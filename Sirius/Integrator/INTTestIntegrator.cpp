...
/*  int type = 1;
  ...
  STLDoubleVectorVector corr;
  double corr_min = 0.05, corr_max = 0.1;
  unicorr(corr, k, corr_min, corr_max);

  CIntegrand *f = new CBasketintegrand(S0, vol, corr, type, K, rho, T);
  ...
  // Alpha
  int s = 2;
  CAlpha *a = new CHaselgrovealpha(s);
  
  // Weights
  int r = 2;
  CWeights *w = new CHaselgroveweights(r);

  // Periodization
  int d = 3;
  CPeriodization *p = new CTrigonometricperiodization(d);

  CNTparameters NTpar(a, w, p);
  ...
  std::cout << "Analytic result = " << f->I(k) << std::endl;
  std::cout << "MC result = " << MCint.integrate(*f, *D, N) << std::endl;
  std::cout << "NT result = " << NTint.integrate(*f, *D, N, NTpar) << std::endl;
...
void testsystematic(int k, int optionclass, int Q, double corr)
{

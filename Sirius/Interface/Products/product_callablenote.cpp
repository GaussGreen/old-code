// product_callableNote.cpp : Implementation of CCallableNote
#include "stdafx.h"
#include "Products.h"
...
STDMETHODIMP CCallableNote::Evaluate(IResult* pVal)
{
	begin_function	
	...
	if( resetDates[ndates-1] > m_dateMaturity ){
		throw "the maturity must be greater than the coupon schedule";
	}

	// build the whole date setting:
	if( resetDates[ndates-1] < m_dateMaturity )
	{
	
		tmpDates.resize(ndates+1);
		for(int i=0; i<ndates; ++i){
			tmpDates[i] = resetDates[i] ;
		}
		tmpDates[ndates] = m_dateMaturity;
	}
...
HRESULT CCallableNote::FinalConstruct(void)
{	
	m_fStrike = 0.0;
	m_dateMaturity = 0.0;
	m_fGearing = 1.0;
...
STDMETHODIMP CCallableNote::InterfaceSupportsErrorInfo(REFIID riid)
{

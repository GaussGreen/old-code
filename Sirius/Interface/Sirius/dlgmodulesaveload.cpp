//	dlgmodulesaveload.cpp : Implementation of the save and load module dialogs
//
/////////////////////////////////////////////////////////////////////////////
...
/*static*/ WINDOWPLACEMENT				CDlgModuleSaveLoad::s_wp = {sizeof(WINDOWPLACEMENT), 0, SW_SHOWNORMAL, {-1, -1}, {-1, -1}, {0, 0, 0, 0}};


/////////////////////////////////////////////////////////////////////////////
//	CDlgModuleSaveLoad
//
//	This is common code to the save and load dialog special cases.
//
long CDlgModuleSaveLoad::MapImageModuleType(int* pnImage, long* pnModuleType)
...
LRESULT CDlgModuleSaveLoad::OnCancel(WORD wNotifyCode, WORD wID, HWND hWndCtl, BOOL& bHandled)
{
	EndDialog(wID);
	return 0L;
}
...
LRESULT CDlgModuleSaveLoad::OnClickedDelete(WORD wNotifyCode, WORD wID, HWND hWndCtl, BOOL& bHandled)
{	
	return 0L;
}

//	Dialog Startup
LRESULT CDlgModuleSaveLoad::OnInitDialog(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{	
	...
	m_wp_init.length = m_wp_tree_init.length = m_wp_preview_init.length = m_wp_edit_init.length = m_wp_saveload_init.length = m_wp_close_init.length = sizeof(WINDOWPLACEMENT);
	...
	::GetWindowPlacement(GetDlgItem(1), &m_wp_saveload_init);
...
LRESULT CDlgModuleSaveLoad::OnItemExpandingTreeModule(int idCtrl, LPNMHDR pnmh, BOOL& bHandled)
{
	return 0L;	
}

LRESULT CDlgModuleSaveLoad::OnSize(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
// Don't use wParam and lParam without changing code in CDlgModuleLoad::OnInitDialog
{			
	...
	// Load button
	wpuse.rcNormalPosition = m_wp_saveload_init.rcNormalPosition;
...
LRESULT CDlgModuleSaveLoad::OnSizing(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
...
//	CDlgModuleLoad
//
//	Specific functionality for the load dialog.
//
LRESULT CDlgModuleLoad::OnClickedDelete(WORD wNotifyCode, WORD wID, HWND hWndCtl, BOOL& bHandled)
{
	...
	// prompt
	if (::MessageBox(::GetActiveWindow(), CStringResource(IDS_DELETE_CONFIRM).str().c_str(), _T("Delete Module"), MB_YESNOCANCEL | MB_ICONQUESTION) != IDYES) return 0L;
...
{
	// Get initial window placements for extra controls (before calling base dialog init)
	m_wp_combo_init.length = m_wp_downloadto_init.length = m_wp_delete_init.length = sizeof(WINDOWPLACEMENT);
	GetWindowPlacement(&m_wp_init);
	...
	::ShowWindow(GetDlgItem(IDC_COMBOBOXEX), SW_SHOW);
	::ShowWindow(GetDlgItem(IDC_DOWNLOAD_TO), SW_SHOW);	
	::ShowWindow(GetDlgItem(IDC_DELETE), SW_SHOW);	
	CDlgModuleSaveLoad::OnInitDialog(uMsg, wParam, lParam, bHandled);	
	m_wp_edit_init.rcNormalPosition.bottom = m_wp_combo_init.rcNormalPosition.top - 7;
	
	// Must redraw the dialog since we have resized a control.
	// Note that we don't supply valid lParam and wParam values.
	// They are not used in the WM_SIZE handlers (see notes in ::OnSize() functions).
	::SendMessage(m_hWnd, WM_SIZE, 0, 0);
	
	SetWindowText("Load Module");
	::SetWindowText(GetDlgItem(1), "&Load");
...
LRESULT CDlgModuleLoad::OnSelChangedTree(int idCtrl, LPNMHDR pnmh, BOOL& bHandled)
{
	...
	if (!m_nModuleSelected) return 0L;
	ssQuery << "sp_user_get_vbmodule_code " << m_nModuleSelected;
	try {
		_Module.GetConnection(spConnection);
		prs = spConnection->Execute(_bstr_t(ssQuery.str().c_str()), NULL, -1);
...
// Don't use wParam and lParam without changing code in CDlgModuleLoad::OnInitDialog
{				
	CDlgModuleSaveLoad::OnSize(uMsg, wParam, lParam, bHandled);
...
//	CDlgModuleSave
//
//	Specific functionality for the save dialog.
//
LRESULT CDlgModuleSave::OnInitDialog(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
	CDlgModuleSaveLoad::OnInitDialog(uMsg, wParam, lParam, bHandled);
	SetWindowText("Save Module");
	::SetWindowText(GetDlgItem(1), "&Save");	

	CComDispatchDriverEx				ddVBE, ddVBProjects, ddVBProject, ddVBComponent, ddCodeModule;
	CComVariant							ProjectsCount, Name, ComponentsCount, Type;
	CParameterMap						pmName;
	std::string							szProject, szModule;
	TVINSERTSTRUCT						tvis;	
	HTREEITEM							hModule = NULL;
	
	m_md.m_nModuleSelected = 0;	
	m_vector.clear();
	memset(&tvis, NULL, sizeof(tvis));
	tvis.hInsertAfter = TVI_SORT;
	...
	SendMessage(GetDlgItem(IDC_EDIT_PREVIEW), WM_SETFONT, (WPARAM)_Module.m_hFont, MAKELPARAM(FALSE, 0));
	TreeView_SetImageList(GetDlgItem(IDC_TREE_MODULE), _Module.m_hImageList, TVSIL_NORMAL);	
	if (!m_ddExcel.GetPropertyByName(L"VBE", ddVBE)){
		if (!ddVBE.GetPropertyByName(L"VBProjects", ddVBProjects)){
			if (!ddVBProjects.GetPropertyByName(L"Count", &ProjectsCount)){
				ATLASSERT(ProjectsCount.vt == VT_I4);
				for (long nProject = 1; nProject <= ProjectsCount.lVal; nProject++){
					if (ddVBProjects.Invoke1(L"Item", &CComVariant(nProject), ddVBProject)) continue;
					if (ddVBProject.GetPropertyByName(L"Name", &Name)) continue;
					if (pmName.SetValue(Name)) continue;
					if (pmName.GetString(&szProject)) continue;
																					
					tvis.hParent = TVI_ROOT;					
					tvis.item.pszText = (LPTSTR)szProject.c_str();					
					tvis.item.lParam = NULL;
										
					CComDispatchDriverEx	ddVBComponents;
					if (ddVBProject.GetPropertyByName(L"VBComponents", ddVBComponents)){
						// cannot access components
						tvis.item.iImage = tvis.item.iSelectedImage = 3;						
					} else {
						tvis.item.iImage = tvis.item.iSelectedImage = 0;
					}
					HTREEITEM hProject = (HTREEITEM)SendMessage(GetDlgItem(IDC_TREE_MODULE), TVM_INSERTITEM, 0, (LPARAM)&tvis);
					if (!ddVBComponents) continue;
					if (ddVBComponents.GetPropertyByName(L"Count", &ComponentsCount)) continue;
					ATLASSERT(ComponentsCount.vt == VT_I4);
					for (long nComponent = 1; nComponent <= ComponentsCount.lVal; nComponent++){
						if (ddVBComponents.Invoke1(L"Item", &CComVariant(nComponent), ddVBComponent)) continue;
						if (ddVBComponent.GetPropertyByName(L"Type", &Type)) continue;												
						if (Type.vt != VT_I4 || Type.lVal != CExcelInterface::vbext_ct_StdModule && Type.lVal != CExcelInterface::vbext_ct_ClassModule) continue;						
						if (ddVBComponent.GetPropertyByName(L"CodeModule", ddCodeModule)) continue;
						if (ddVBComponent.GetPropertyByName(L"Name", &Name)) continue;
						if (pmName.SetValue(Name)) continue;
						if (pmName.GetString(&szModule)) continue;
												
						tvis.hParent = hProject;						
						tvis.item.pszText = (LPTSTR)szModule.c_str();
						tvis.item.iImage = tvis.item.iSelectedImage = MapImageModuleType(NULL, &Type.lVal);												
						m_vector.push_back(CComPtr<IDispatch>(ddCodeModule.p));
						tvis.item.lParam = (LPARAM)m_vector.size();												
						hModule = (HTREEITEM)SendMessage(GetDlgItem(IDC_TREE_MODULE), TVM_INSERTITEM, 0, (LPARAM)&tvis);
						SendMessage(GetDlgItem(IDC_TREE_MODULE), TVM_EXPAND, TVE_EXPAND, (LPARAM)hProject);												
					}
				}
			}
		}
...
LRESULT CDlgModuleSave::OnOK(WORD wNotifyCode, WORD wID, HWND hWndCtl, BOOL& bHandled)
{	
	if (!m_md.m_nModuleSelected){
		::MessageBeep(MB_ICONEXCLAMATION);
		CParameterMap::DisplayError(CStringResource(IDS_NO_MODULE_SELECTED).str(), MB_ICONEXCLAMATION);
		return 0L;
	}
	
	CComDispatchDriverEx				ddCodeModule(m_vector[m_md.m_nModuleSelected - 1].m_T);	
	CComVariant							Name;	
	_ConnectionPtr						spConnection;
	std::stringstream					ssQuery;						// SQL query to execute
	
	ddCodeModule.GetPropertyByName(L"Name", &Name);			
	
	estring szModuleText(m_md.m_szModuleText);
	szModuleText.ReplaceStrInStr("'", "''");
	
	ssQuery << "sp_user_add_vbmodule '" << estring(Name) << "', '" << szModuleText << "', " << m_md.m_nModuleType;
	try {
		_Module.GetConnection(spConnection);
		spConnection->Execute(_bstr_t(ssQuery.str().c_str()), NULL, -1);
	} catch (_com_error& e){		
	...
	// Grey the save button (which gives the user indication that the save has worked) and move the focus.
	NextDlgCtrl();	
	::EnableWindow(GetDlgItem(IDOK), FALSE);			
	return 0L;
}

LRESULT CDlgModuleSave::OnSelChangedTree(int idCtrl, LPNMHDR pnmh, BOOL& bHandled)
{
	// Preview any new module and set m_md		
	::EnableWindow(GetDlgItem(IDOK), TRUE);
	LPNMTREEVIEW						pnmtv = (LPNMTREEVIEW)pnmh;
	
	if (!pnmtv->itemNew.lParam) return 0L;
	
	CComDispatchDriverEx				ddCodeModule(m_vector[pnmtv->itemNew.lParam - 1].m_T);
	CComVariant							av[2] = {0L, 1L};				// parameters of "Lines" property
	CComVariant							vModuleText;
	
	m_md.m_nModuleSelected = pnmtv->itemNew.lParam;	
	ATLASSERT(pnmtv->itemNew.mask & TVIF_HANDLE);

	TVITEM								tvi;
	
	::memset(&tvi, NULL, sizeof(tvi));
	tvi.hItem = pnmtv->itemNew.hItem;
	tvi.mask = TVIF_SELECTEDIMAGE;
	SendMessage(GetDlgItem(IDC_TREE_MODULE), TVM_GETITEM, 0, (LPARAM)&tvi);	
	m_md.m_nModuleType = MapImageModuleType(&tvi.iSelectedImage, NULL);		
	ddCodeModule.GetPropertyByName(L"CountOfLines", av);		// this sets the zeroth parameter		
	ddCodeModule.GetPropertyByName(L"Lines", av, 2, &vModuleText);
	m_md.m_szModuleText = vModuleText;
	SetDlgItemText(IDC_EDIT_PREVIEW, m_md.m_szModuleText.c_str());
	return 0L;
}
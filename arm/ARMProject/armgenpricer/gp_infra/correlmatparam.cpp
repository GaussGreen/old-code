...
////////////////////////////////////////////////////
std::vector<double>* ARM_CorrelMatParam::GetData( ARM_DataType type, long& rows, long& cols ) const
{
	switch(type)
	{
		case ARM_ModelParamType::BreakPointTimes:
			{
				...
				rows=result.size();
				cols=1;
				return new std::vector<double>(result.GetValues());
			}
        case ARM_ModelParamType::Values:
			{
				...
				std::vector<double>* result = new std::vector<double>(rows*cols,0.0);
				for(size_t i=0,k=0;i<rows;++i)
				{
					const std::vector<double>& val = (itsCurves->GetOrdinates())[i];
...
///	Action : computes the output matrix
////////////////////////////////////////////////////
void ARM_CorrelACPMatParam::UpdateMultiCurves( std::vector<double>* breakPointTimes )
{
	...
	ARM_GP_T_Vector< std::vector<double> > ordinates(breakPointTimes->size() );
	ARM_MultiCurve* mCurve = new ARM_MultiCurve( *breakPointTimes, ordinates, interpolator );
	for(i=0;i<size; ++i)
	{
        std::vector<double> factor(itsFactorCount);
...
ARM_CorrelTrigoMatParam::ARM_CorrelTrigoMatParam( 
	...
	bool adviseBreakPointTimes )
:	
	ARM_CorrelACPMatParam( &std::vector<double>(1,0.0), &std::vector<double>(1,theta), interpolatorName, lowerBound, upperBound ), 
	itsTheta(theta),
	itsCorrelBreakPointTimes( NULL )
{
	itsCorrelBreakPointTimes = datestrip.GetResetDates();
...
:	
	...
	itsCorrelBreakPointTimes( NULL )
{
	itsCorrelBreakPointTimes = (std::vector<double>*) resetDates.Clone();
	*itsCorrelBreakPointTimes -= julianAsOfDate;
...
:	ARM_CorrelACPMatParam(rhs),
	...
	itsCorrelBreakPointTimes( rhs.itsCorrelBreakPointTimes? (std::vector<double>*) rhs.itsCorrelBreakPointTimes->Clone(): NULL )
{}
...
void ARM_CorrelTrigoMatParam::MergeModelParam( ARM_ModelParam* NewValue )
{
	...
	std::vector<double>* values = GetCurve()->GetOrdinates();
...
///	Action : 
////////////////////////////////////////////////////
void ARM_BrownianCorrelationParam::ComputeMultiCurves( std::vector<double>* values )
{
	const std::vector<double>* times	= GetCurve()->GetAbscisses();
	...
	ARM_MultiCurveInterpolator* interpolator = (ARM_MultiCurveInterpolator*) GetMultiCurve()->GetInterpolator()->Clone();
	ARM_GP_T_Vector< std::vector<double> > ordinates(times.size() );
	ARM_MultiCurve* mCurve = new ARM_MultiCurve( times, ordinates, interpolator );
	
	for( size_t i=0; i<size; ++i )
	{
        std::vector<double> factors(2);

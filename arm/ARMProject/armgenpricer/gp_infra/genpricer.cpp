...
ARM_GenPricer::ARM_GenPricer( ARM_GenSecurity* sec, ARM_PricingModel* mod, 
	const ARM_StringVector& controlVariatesColumns,	const std::vector<double>& controlVariatesPrices,	const string& refPriceColumn, 
...
double ARM_GenPricer::Price()
{
	...
	std::vector<double>::iterator toTime		= pricingAdviser->GetFirstToTime();
	std::vector<double>::iterator endToTime	= pricingAdviser->GetLastToTime();

	// from times and to times defines the time interval on which we induct
	ARM_VectorPtr fromTimes				= pricingAdviser->GetFromTimes();
	std::vector<double>::iterator fromTime	= pricingAdviser->GetFirstFromTime();
	std::vector<double>::iterator endFromTime	= pricingAdviser->GetLastFromTime();
    ...
    else
    {
	    ...
	    for(size_t i=0; i<bucketsNb; ++i )
	    {
			...
			///if( (*itsPricingMod->GetNumMethod()->GetBuckets())[i] )
			{
				...
				itsPricerInfo->StoreCurrentLoopPrice( states, (*itsPricingMod->GetNumMethod()->GetBuckets())[ itsPricingMod->GetNumMethod()->GetBucketIndex() ], i, itsPricingMod->GetNumMethod()->GetPricingDirection() );

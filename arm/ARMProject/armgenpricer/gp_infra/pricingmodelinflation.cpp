...
ARM_VectorPtr ARM_PricingModelInflation::DefaultAnnuity(const string& curveName, double evalTime,const ARM_DateStripPtr& PayDateStrip, 
													const ARM_PricingStatesPtr& states) const
{
	std::vector<double> * PayDates = PayDateStrip->GetFlowEndDates();
	std::vector<double> * InterestTerms = PayDateStrip->GetInterestTerms();
	...
	if( !itsIRModel )
	{
		...
		std::vector<double>::iterator resultIter,DFsIter;
	...
	else
	{
		...
		std::vector<double> * newPayDates = new std::vector<double>( *PayDates );
		for( std::vector<double>::iterator iter = newPayDates->begin() ; iter != newPayDates->end() ; iter++ )
			*iter -= AsOfDate;
	...
	const ARM_PricingStatesPtr& states) const
{
	...
	std::vector<double>::iterator iter,iterAnnuity;

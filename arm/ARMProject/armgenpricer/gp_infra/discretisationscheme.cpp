...
////////////////////////////////////////////////////
std::vector<double>* ARM_EventTime::ModelTimesFromTimesInfo(
	...
	case ARM_NumMethod::GP_FWDBCKWDLOOKING:
		{
			...
			{
				timeSteps=new std::vector<double>(nbEvents+1);
				(*timeSteps)[0]=0.0;
				j=1;
			}
			else
			{
				timeSteps=new std::vector<double>(nbEvents);
				j=0;
			}
...
////////////////////////////////////////////////////
std::vector<double>* ARM_EventAndModelTime::ModelTimesFromTimesInfo(
	const ARM_TimeInfoPtrVector& timeInfos,
	ARM_PricingModel& model ) const 
{
	/// first computes eventTimes
	CC_NS(std,auto_ptr)<std::vector<double>> pEventTimes( ARM_EventTime().ModelTimesFromTimesInfo( timeInfos, model ) );
	
	/// get model times only if appropriate
	if( !model.SupportAnalyticMarginal() )
	{
		CC_NS(std,auto_ptr)<std::vector<double>> pModelTimes( model.ComputeModelTimes( timeInfos ) );

		return MergeSortedVectorNoDuplicates( pEventTimes.get(), pModelTimes.get() );
	}
	else
		return VectorUnique( pEventTimes.get() );
}
...
ARM_GP_Vector& ARM_FixStepTimeInserter::AddTimes(const ARM_GP_Vector& initialTimes) const
{
	...
	return ARM_GP_Vector(tmpResult.size(),&tmpResult[0]);
}

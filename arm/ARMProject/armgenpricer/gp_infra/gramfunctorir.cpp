...
{
	/// NDC : test if that feature is really used
	/*
	...
	if( arg[1].GetType() == GFAT_STRING_TYPE ) {
		...
		itsCurveName = arg[0].GetString();		/// Modification of nodes if necessary 
		...
		ARM_INDEX_TYPE defaultIndex = LIBOR3M;//( ccyName );
	...
	double evalDate, vector< ARM_ExpNodePtr >& nodes, size_t statesSize   )
{
	...
	if(!GetAlreadyComputed())
	{
		...
		GPAF_CheckArgType( arg[1], GFAT_DATE_OR_VECTOR_TYPE,ARM_GP_DF::itsFuncName );
		...
		if ( (arg[1].GetType() ==  GFAT_DATE_TYPE) || (arg[1].GetType() ==  GFAT_STRING_TYPE))
		{
			ARM_Date endDate = ComputeEndDate( evalDate, arg[1], calendar );
			double itsMaturityTime = mod->GetTimeFromDate( endDate );
			CheckNbSmaller( itsEvalTime, itsMaturityTime, "EvalTime", "MaturityTime", ARM_GP_DF::itsFuncName,__LINE__,__FILE__  );
			itsMaturityTimeVector		= ARM_VectorPtr(new std::vector<double>(statesSize ,itsMaturityTime));
		}
		else if (arg[1].GetType() ==  GFAT_DOUBLE_TYPE)
		{
			ARM_Date endDate = ARM_Date(arg[1].GetDouble());
			double itsMaturityTime = mod->GetTimeFromDate( endDate );
			CheckNbSmaller( itsEvalTime, itsMaturityTime, "EvalTime", "MaturityTime", ARM_GP_DF::itsFuncName,__LINE__,__FILE__  );
			itsMaturityTimeVector		= ARM_VectorPtr(new std::vector<double>(statesSize ,itsMaturityTime));
		}
		else if ( arg[1].GetType() ==  GFAT_VECTOR_TYPE ) {
			itsMaturityTimeVector		= arg[1].GetVector();
			size_t i, sz = itsMaturityTimeVector->size();

			if ( sz != statesSize)
				Exception(__LINE__, __FILE__, ERR_INVALID_ARGUMENT, ARM_USERNAME + "something wrong in ARM_GP_DF::GrabInputs(...)" );

			if (sz>0) {
				// Convert vector of julian in vector of year fraction
				for ( i = 0; i < sz; ++i) {
					ARM_Date dummyDate = ARM_Date( (*itsMaturityTimeVector)[i] );
					(*itsMaturityTimeVector)[i] = mod->GetTimeFromDate(dummyDate);
					CheckNbSmaller( itsEvalTime, (*itsMaturityTimeVector)[i],	"EvalTime", "ExpiryTime",  string("ARM_GP_DF") );
				}
			}
		}
		else throw Exception(__LINE__, __FILE__, ERR_INVALID_ARGUMENT, ARM_USERNAME + "maturity should be vector or date" );
	...
	double evalDate, const ARM_PricingStatesPtr& states, vector< ARM_ExpNodePtr >& nodes )
{
	...
	ARM_VectorPtr df =  itsModel->DiscountFactor( itsCurveName, itsEvalTime , *itsMaturityTimeVector, states );
	...
	double evalDate, vector<ARM_ExpNodePtr>& nodes )
{
	...
	return ARM_NodeInfo( itsMaturityTimeVector,ARM_AdditionalTimeInfoPtr(NULL) );
}
...
{
	/// checking of the size and type
	GPAF_CheckArgSize( arg, 9, ARM_GP_Annuity::itsFuncName );
	...
	double evalDate, vector<ARM_ExpNodePtr>& nodes  )
{
	...
	if(!GetAlreadyComputed())
	{
		...
		int dateStripWidth = static_cast<int>(arg[8].GetDouble());
        ...
        if(sched == ARM_DateStripPtr(NULL))
        {
		    ...
		    std::vector<double>* pfixPayTimes   = dateStrip.GetFlowEndDates();
		    std::vector<double>* pfixPayPeriods = dateStrip.GetInterestTerms();

		    /// copy constructor
		    itsFixPayTimes	= std::vector<double>( *pfixPayTimes );
		    itsFixPayPeriods= std::vector<double>( *pfixPayPeriods );
		    ...
		    else if( arg[5].GetType() ==  GFAT_CURVE_TYPE )
		    {
			    ...
			    for(size_t i=0; i<sizefixflows; ++i)			
        ...
        else
        {
			...
			if (dateStripWidth != -1)
				nbActFixFlows = dateStripWidth < nbActFixFlows ? dateStripWidth : nbActFixFlows;

            /// Fill arguments from the dateStrip
			itsFixPayTimes.resize(nbActFixFlows);
		    itsFixPayPeriods.resize(nbActFixFlows);
		    for(size_t i=0;i<nbActFixFlows;++i)
            {
	...
	double evalDate, const ARM_PricingStatesPtr& states, vector< ARM_ExpNodePtr >& nodes )
{
	...
	if (itsIsVariableNotional)
		Annuity =  itsModelIR->AnnuityWithNominal( itsCurveName, itsEvalTime, itsFixPayTimes, itsFixPayPeriods, itsVariableNotional->GetValues(), states );
	else
...
void ARM_GP_SwapRate::GrabInputs( ARM_GramFctorArgVector& arg, ARM_PricingModel* mod,	double evalDate, vector< ARM_ExpNodePtr >& nodes )
{
	...
	if(!GetAlreadyComputed())
	{
		...
		ARM_Currency* ccy		    = mod->GetCurrency( itsCurveName );
		char* ccyName			    = ccy->GetCcyName();
		ARM_INDEX_TYPE defaultIndex =  LIBOR6M;//GetDefaultIndexFromCurrency( ccyName );
		...
		/// not cloned hence no need to delete this!
		std::vector<double>* pfixResetTimes  = fixDateStrip.GetResetDates();
		...
		/// copy constructor
		itsFixResetTimes	= ( *pfixResetTimes );  
		itsFixPayTimes	    = ( *pfixPayTimes );
		itsFixPayPeriods   = ( *pfixPayPeriods );
		...
		/// not cloned hence no need to delete this!
		std::vector<double>* pFwdResetTimes      = floatDateStrip.GetResetDates();
		std::vector<double>* pFwdStartTimes      = floatDateStrip.GetFwdRateStartDates();
		std::vector<double>* pFwdEndTimes        = floatDateStrip.GetFwdRateEndDates();
		std::vector<double>* pFloatPayTimes      = floatDateStrip.GetPaymentDates();
		std::vector<double>* pFloatPayPeriods    = floatDateStrip.GetInterestTerms();

		/// copy constructor
		itsFwdResetTimes	= *pFwdResetTimes;
		itsFwdStartTimes	= *pFwdStartTimes;
		itsFwdEndTimes      = *pFwdEndTimes;
		itsFwdPayPeriods.resize(pFwdStartTimes->size());
		itsFloatPayTimes	= *pFloatPayTimes;
		itsFloatPayPeriods  = *pFloatPayPeriods;

		/// get time from date! 
		for(int i=0; i<pFwdStartTimes->size(); ++i )
		{
		...
		else if( arg[7].GetType() ==  GFAT_VECTOR_TYPE )
		{
			...
			for(size_t i=0; i<sizefloatflows; ++i)
	...
	double evalDate, const ARM_PricingStatesPtr& states, vector< ARM_ExpNodePtr >& nodes )
{
	...
	ARM_VectorPtr SwapRate =  itsModelIR->SwapRate( itsCurveName, itsEvalTime,
		...
		itsFloatPayTimes, itsFloatPayPeriods, itsMarginVector->GetValues(), itsDbleNotional, states );
	...
	double evalDate, const ARM_PricingStatesPtr& states, vector< ARM_ExpNodePtr >& nodes )
{
	...
	else if(nbRows == 1 &&  nbRows != statesSize)
	{
        ...
        itsFloatPayTimes,
		itsFloatPayPeriods,		
		itsMarginVector->GetValues(),
		itsDbleNotional,
		itsFixNotionalVector->GetValues(),
		itsFloatNotionalVector->GetValues(),
	    *itsStrikeMatrix,
		itsPayRec,
		states );
...
/// First Swap Rate
		...
		/// copy constructor
		itsFixResetTimes1	= std::vector<double>( *(fixDateStrip1.GetResetDates()) );  
		itsFixPayTimes1	    = std::vector<double>( *(fixDateStrip1.GetPaymentDates()) );
		itsFixPayPeriods1   = std::vector<double>( *(fixDateStrip1.GetInterestTerms()) );
		...
		/// copy constructor
		itsFwdResetTimes1	= std::vector<double>( *(floatDateStrip1.GetResetDates()) );
		itsFwdStartTimes1	= std::vector<double>( *(floatDateStrip1.GetFwdRateStartDates()) );
		itsFwdEndTimes1     = std::vector<double>( *(floatDateStrip1.GetFwdRateEndDates()) );
		itsFwdPayPeriods1   = std::vector<double>( itsFwdStartTimes1.size());
		itsFloatPayTimes1	= std::vector<double>( *(floatDateStrip1.GetPaymentDates()) );
		itsFloatPayPeriods1 = std::vector<double>( *(floatDateStrip1.GetInterestTerms()) );

		/// get time from date! 
		for(size_t i=0; i<itsFwdStartTimes1.size(); ++i )
		{
...
/// Second Swap Rate
		...
		/// copy constructor
		itsFixResetTimes2	= std::vector<double>( *(fixDateStrip2.GetResetDates()) );  
		itsFixPayTimes2	    = std::vector<double>( *(fixDateStrip2.GetPaymentDates()) );
		itsFixPayPeriods2   = std::vector<double>( *(fixDateStrip2.GetInterestTerms()) );

		/// get time from date!
		for(size_t i=0; i<itsFixPayTimes2.size(); ++i )
		{
		...
		/// copy constructor
		itsFwdResetTimes2	= std::vector<double>( *(floatDateStrip2.GetResetDates()) );
		itsFwdStartTimes2	= std::vector<double>( *(floatDateStrip2.GetFwdRateStartDates()) );
		itsFwdEndTimes2     = std::vector<double>( *(floatDateStrip2.GetFwdRateEndDates()) );
		itsFwdPayPeriods2   = std::vector<double>(itsFwdStartTimes2.size());
		itsFloatPayTimes2	= std::vector<double>( *(floatDateStrip2.GetPaymentDates()) );
		itsFloatPayPeriods2 = std::vector<double>( *(floatDateStrip2.GetInterestTerms()) );

		/// get time from date! 
		for(size_t i=0; i<itsFwdStartTimes2.size(); ++i )
		{
	...
	double evalDate, const ARM_PricingStatesPtr& states, vector< ARM_ExpNodePtr >& nodes )
{
	...
	ARM_VectorPtr Spread =  itsModelIR->Spread( itsCurveName, itsEvalTime,
        ...
        itsFloatPayTimes1, itsFloatPayPeriods1, itsMarginVector1->GetValues(),
		itsCoeff2,
		itsFloatStartTime2, itsFloatEndTime2,
		itsFixPayTimes2, itsFixPayPeriods2,
        itsFwdStartTimes2, itsFwdEndTimes2, itsFwdPayPeriods2,
        itsFloatPayTimes2, itsFloatPayPeriods2, itsMarginVector2->GetValues(),
		states );
...
void ARM_GP_BasisSwap::GrabInputs( ARM_GramFctorArgVector& arg, ARM_PricingModel* mod,	double evalDate, vector< ARM_ExpNodePtr >& nodes )
{
	...
	if(!GetAlreadyComputed())
	{
		...
		/// copy constructor
		itsDomResetTimes		= std::vector<double>( *dateStrip.GetResetDates() ); 
		itsDomFwdStartTimes		= std::vector<double>( *dateStrip.GetFwdRateStartDates() ); 
		itsDomFwdEndTimes		= std::vector<double>( *dateStrip.GetFwdRateEndDates() ); 
		itsDomFlowStartTimes	= std::vector<double>( *dateStrip.GetFlowStartDates() );//;
		itsDomFlowEndTimes		= std::vector<double>( *dateStrip.GetFlowEndDates() );//;
		itsDomPayTimes			= std::vector<double>( *dateStrip.GetPaymentDates() );//;
		itsDomPayPeriods		= std::vector<double>( *dateStrip.GetInterestTerms() );
								
		ARM_INDEX_TYPE defaultIndex = LIBOR3M;//GetDefaultIndexFromCurrency( domCcy->GetCcyName() );
		int fwdDayCount	= GetFloatDayCountFtor(domCcy)();
		itsDomFwdPeriods	= std::vector<double>(itsDomFwdStartTimes.size()); 
		...
		itsForResetTimes		= std::vector<double>( *dateStrip.GetResetDates() );//; 
		itsForFwdStartTimes		= std::vector<double>( *dateStrip.GetFwdRateStartDates() );//; 
		itsForFwdEndTimes		= std::vector<double>( *dateStrip.GetFwdRateEndDates() );//; 
		itsForFlowStartTimes	= std::vector<double>( *dateStrip.GetFlowStartDates() );//; 
		itsForFlowEndTimes		= std::vector<double>( *dateStrip.GetFlowEndDates() );//; 
		itsForPayTimes			= std::vector<double>( *dateStrip.GetPaymentDates() );//; 
		itsForPayPeriods		= std::vector<double>( *dateStrip.GetInterestTerms() ); 
								
		defaultIndex = LIBOR3M;// GetDefaultIndexFromCurrency( forCcy->GetCcyName() );
		...
		if( arg[14].GetType() ==  GFAT_DOUBLE_TYPE )
			...
			std::vector<double>::iterator begin = Vect.begin(), iter;
	...
	double evalDate, const ARM_PricingStatesPtr& states, vector< ARM_ExpNodePtr >& nodes )
{
	...
	// DARK SIDE IS VICIOUS
	ARM_GP_T_Matrix<double> tmp1, tmp2;
	ARM_GP_T_Matrix<double>* dummyDomStrikeMatrix = itsDomStrikeMatrix.IsNull()? &tmp1: &*itsDomStrikeMatrix;
	ARM_GP_T_Matrix<double>* dummyForStrikeMatrix = itsForStrikeMatrix.IsNull()? &tmp2: &*itsForStrikeMatrix;
	...
	ARM_VectorPtr NPVFloatLeg =  itsModelIR->NPVBasisSwap( 
							...
							itsDomPayTimes,
							itsDomPayPeriods,
							itsDomMarginVector->GetValues(),
							itsDomNotionalVector->GetValues(),
							itsIsDomFlottant,
							itsForResetTimes,       
							itsForFwdStartTimes,
							itsForFwdEndTimes,
							...
							itsForPayTimes,
							itsForPayPeriods,
							itsForMarginVector->GetValues(),
							itsForNotionalVector->GetValues(),
							itsIsForFlottant,
							itsExNotionalType,
							itsFxResetTimes,
							itsFxSettlTimes,  
							itsFxPayTimes,
							*dummyDomStrikeMatrix,
							*dummyForStrikeMatrix,
							states );
	...
	double evalDate, vector<ARM_ExpNodePtr>& nodes )
{
	...
	/// sort and remove duplicates!
    CC_NS( std, sort )( result.begin(), result.end() );
    std::vector<double>::iterator pos = CC_NS( std, unique )( result.begin(), result.end() );
    result.resize( CC_NS( std, CC_DISTANCE )( result.begin(), pos ) );
	...
	double evalDate, const ARM_PricingStatesPtr& states, vector< ARM_ExpNodePtr >& nodes )
{
	...
	{
		size_t statesSize	= states != ARM_PricingStatesPtr( NULL )? states->size() : 1;
		itsStrikeVector		= ARM_VectorPtr( new std::vector<double>( statesSize, itsStrikeDouble ) );
	}
	...
	double evalDate, vector<ARM_ExpNodePtr>& nodes )
{
	...
	if(!GetAlreadyComputed())
	{
		...
		/// not cloned hence no need to delete this!
		std::vector<double>* pfixPayTimes   = fixDateStrip.GetFlowEndDates();
		std::vector<double>* pfixPayPeriods = fixDateStrip.GetInterestTerms();
		...
		std::vector<double>* pfloatResetTimes	= floatDateStrip.GetResetDates();
		std::vector<double>* pfloatStartTimes	= floatDateStrip.GetFlowStartDates();
		std::vector<double>* pfloatEndTimes	= floatDateStrip.GetFlowEndDates();
		std::vector<double>* pfloatIntTerms   = floatDateStrip.GetInterestTerms();

		itsFloatResetTimes	= std::vector<double>( *pfloatResetTimes );
		itsFloatStartTimes= std::vector<double>( *pfloatStartTimes );
		itsFloatPayTimes	= std::vector<double>( *pfloatEndTimes );
		itsFloatIntTerms= std::vector<double>( *pfloatIntTerms );

		/// get time from date! 
		for(size_t i=0; i<pfixPayTimes->size(); ++i )
		{
			itsFixPayTimes[i] = mod->GetTimeFromDate( itsFixPayTimes[i] );
		}

		for(size_t i=0; i<pfloatResetTimes->size(); ++i )
		{
	...
	double evalDate, const ARM_PricingStatesPtr& states, vector< ARM_ExpNodePtr >& nodes )
{
	...
	ARM_VectorPtr Swaption =  itsModelIR->VanillaSwaption( itsCurveName, itsEvalTime, itsSwapResetTime, itsFixNotionalVector->GetValues(), itsFloatNotionalVector->GetValues(),
	...
	double evalDate, vector<ARM_ExpNodePtr>& nodes )
{
	...
	int i;
	for(size_t i=0; i< sizeFixPayTimes; ++i ){
...
void ARM_GP_Cap::GrabInputs( const ARM_GramFctorArgVector& arg, ARM_PricingModel* mod,	double evalDate, vector< ARM_ExpNodePtr >& nodes )
{
	...
	if(!GetAlreadyComputed())
	{
		...
		/// not cloned hence no need to delete this!
		std::vector<double>* pFwdStartTimes      = cpnDateStrip.GetFwdRateStartDates();
		std::vector<double>* pFwdEndTimes        = cpnDateStrip.GetFwdRateEndDates();
		std::vector<double>* pPayTimes           = cpnDateStrip.GetPaymentDates();
		std::vector<double>* pPeriods            = cpnDateStrip.GetInterestTerms();
		std::vector<double>* pFwdResetTimes      = cpnDateStrip.GetResetDates();
		...
		for(size_t i=0; i<pFwdStartTimes->size(); ++i )
        {
		...
		itsPayTimes	        = ( *pPayTimes );
		itsPeriods          = ( *pPeriods );
		itsFwdResetTimes    = (*pFwdResetTimes);
		itsFwdStartTimes	= ( *pFwdStartTimes );
		itsFwdEndTimes      = ( *pFwdEndTimes );
		itsFwdPeriods.resize(pFwdStartTimes->size());										
...
void ARM_GP_SumOption::GrabInputs(ARM_GramFctorArgVector& arg, ARM_PricingModel* mod,	double evalDate, vector< ARM_ExpNodePtr >& nodes )
{
	...
	if(!GetAlreadyComputed())
	{
		...
		std::vector<double>* tmpFwdStartDates = cpnDateStrip.GetFwdRateStartDates();
		std::vector<double>* tmpFwdEndDates = cpnDateStrip.GetFwdRateEndDates();
		std::vector<double>* tmpFwdResetDates = cpnDateStrip.GetResetDates();
		...
		/// not cloned hence no need to delete this!
		std::vector<double>* pFwdStartTimes      = new std::vector<double>(lastIdx-firstIdx+1);
		std::vector<double>* pFwdEndTimes        = new std::vector<double>(lastIdx-firstIdx+1);
		itsPayTime = arg[3].GetDate().GetJulian();
		std::vector<double>* pFwdResetTimes      =new std::vector<double>(lastIdx-firstIdx+1);
		...
		itsFwdStartTimes	= ( *pFwdStartTimes );
		itsFwdEndTimes      = ( *pFwdEndTimes );
		itsFwdPeriods.resize(lastIdx-firstIdx+1);
	...
	if(	firstVectorIndex != ARM_GramFctorArgVectorHelper::IndexNotFound )
	{
		...
		for( size_t* iter=positionBegin; iter<positionEnd; ++iter )
		{
			if(arg[*iter].GetType() == GFAT_DOUBLE_TYPE )
				argVec[iter-positionBegin] = ARM_VectorPtr( new std::vector<double>(refSize,arg[*iter].GetDouble() ) );
			else
			{
...
void ARM_GP_SpreadOption::GrabInputs( const ARM_GramFctorArgVector& arg, ARM_PricingModel* mod,	double evalDate, vector< ARM_ExpNodePtr >& nodes )
{
	...
	if(!GetAlreadyComputed())
	{
		...
		/// not cloned hence no need to delete this!
		std::vector<double>* pPayTimes        = cpnDateStrip.GetPaymentDates();
		std::vector<double>* pPayPeriods      = cpnDateStrip.GetInterestTerms();
		std::vector<double>* pResetTimes      = cpnDateStrip.GetResetDates();
		...
		itsResetTimes		= ( *pResetTimes );
		itsPayTimes			= ( *pPayTimes );
		itsPayPeriods       = ( *pPayPeriods );
		...
		/// get time from date! 
		for(size_t i=0; i<itsPayTimes.size(); ++i )
		{			
			itsPayTimes[i]	 = mod->GetTimeFromDate( itsPayTimes[i] );
			...
			for (size_t j=0; j<itsSwapShortFixPayTimes[i]->size(); j++)
...
{	
	...
	ARM_VectorPtr SpreadOption =  itsModelIR->VanillaSpreadOption ( itsCurveName, 
																	...
																	itsPayTimes, 
																	itsPayPeriods, 
																	itsNotionals->GetValues(), 
																	itsCoeffLong->GetValues(), 
																	itsCoeffShort->GetValues(), 
																	*itsStrikes, 
...
//															startDate.GetJulian(), 
//															endDate1.GetJulian(),
//															std::vector<double>(1, endDate1.GetJulian()),
//															std::vector<double>(1, payPeriod));
...
//															startDate.GetJulian(), 
//															endDate2.GetJulian(),
//															std::vector<double>(1, endDate2.GetJulian()),
//															std::vector<double>(1, payPeriod));
...
//															startDate.GetJulian(), 
//															endDate.GetJulian(),
//															std::vector<double>(1, endDate.GetJulian()),
//															std::vector<double>(1, payPeriod));
...
void ARM_GP_MaxRate::GrabInputs( const ARM_GramFctorArgVector& arg, ARM_PricingModel* mod,	double evalDate, vector< ARM_ExpNodePtr >& nodes )
{
	...
	if(!GetAlreadyComputed())
	{
		...
		if( arg[8].GetType() ==  GFAT_DOUBLE_TYPE )
			itsStrikes = ARM_VectorPtr(new std::vector<double>(itsFirstRate->size(),arg[8].GetDouble()));
		else
		{
...
/// First Swap Rate
		...
		/// copy constructor
		itsFixResetTimes	= ( *(fixDateStrip.GetResetDates()) );  
		itsFixPayTimes	    = ( *(fixDateStrip.GetPaymentDates()) );
		itsFixPayPeriods    = ( *(fixDateStrip.GetInterestTerms()) );
		...
		/// copy constructor
		itsFwdResetTimes	= ( *(floatDateStrip.GetResetDates()) );
		itsFwdStartTimes	= ( *(floatDateStrip.GetFwdRateStartDates()) );
		itsFwdEndTimes		= ( *(floatDateStrip.GetFwdRateEndDates()) );
		itsFwdPayPeriods.resize( itsFwdStartTimes.size());
		itsFloatPayTimes	= ( *(floatDateStrip.GetPaymentDates()) );
		itsFloatPayPeriods	= ( *(floatDateStrip.GetInterestTerms()) );

		/// get time from date! 
		for(size_t i=0; i<itsFwdStartTimes.size(); ++i )
		{
		    itsFwdPayPeriods[i] = CountYearsWithoutException( fwdDayCount, ARM_Date(itsFwdStartTimes[i]), ARM_Date(itsFwdEndTimes[i]) );
			itsFwdResetTimes[i] = mod->GetTimeFromDate( itsFwdResetTimes[i] );
			itsFwdStartTimes[i] = mod->GetTimeFromDate( itsFwdStartTimes[i] );
	...
	double evalDate, const ARM_PricingStatesPtr& states, vector< ARM_ExpNodePtr >& nodes )
{
	...
	ARM_VectorPtr MaxRate =  itsModelIR->MaxRate( itsCurveName, itsEvalTime,
		...
		itsFixPayTimes, itsFixPayPeriods,
        itsFwdStartTimes, itsFwdEndTimes, itsFwdPayPeriods,
		itsFloatPayTimes, itsFloatPayPeriods, itsMarginVector->GetValues(),
...
void ARM_GP_DoubleDigital::GrabInputs( const ARM_GramFctorArgVector& arg, ARM_PricingModel* mod,	double evalDate, vector< ARM_ExpNodePtr >& nodes )
{
	...
	if(!GetAlreadyComputed())
	{
		...
		if(arg[2].GetType() ==  GFAT_DOUBLE_TYPE)
			itsFirstStrikeDown = ARM_VectorPtr(new std::vector<double>(itsFirstRate->size(),arg[2].GetDouble()));
		else
		{
		...
		if(arg[3].GetType() ==  GFAT_DOUBLE_TYPE)
			itsFirstStrikeUp = ARM_VectorPtr(new std::vector<double>(itsFirstRate->size(),arg[3].GetDouble()));
		else
		{
		...
		if(arg[5].GetType() ==  GFAT_DOUBLE_TYPE)
			itsSecondStrikeDown = ARM_VectorPtr(new std::vector<double>(itsSecondRate->size(),arg[5].GetDouble()));
		else
		{
		...
		if(arg[6].GetType() ==  GFAT_DOUBLE_TYPE)
			itsSecondStrikeUp = ARM_VectorPtr(new std::vector<double>(itsSecondRate->size(),arg[6].GetDouble()));
		else
		{

...
/////////////////////////////////////////////////////////////////
std::vector<double>* ARM_MarketData_RetrieverImp::ConvertVectorFromMatuOrDateToYearFrac( const vector<string>& vecData, 
	const ARM_Date& asOfDate,
	const string& ccy ) const
{
	std::vector<double>* result = new std::vector<double>(vecData.size());
...
std::vector<double>* ARM_MarketData_RetrieverImp::ConvertVectorFromMatuOrDateWRefDateToYearFrac( const vector<string>& vecData, 
	const ARM_Date& asOfDate,
	...
	const ARM_Date& asOfDate ) const
{
	...
	ARM_ZeroCurve* curve = NULL;//new ARM_ZeroLInterpol( const_cast<ARM_Date&>(asOfDate), mktMatu.get(), mktData.get(), 0, 0, K_LINEAR );
	...
	int volType ) const
{
	...
	/// read the file!
	std::vector<double>& vMaturitiesOrStrikes = new std::vector<double>;
	...
	std::vector<double>& tmpYearTerms = ConvertVectorFromMatuOrDateWRefDateToYearFrac( vYearTerms, asOfDate, settleDate, ccy );
...
ARM_VolCube* ARM_MarketData_RetrieverImp::GetVolCubeFromSummit(
	...
	const string& volMktType ) const
{
	//ARM_VolCurve* ATMVolCrv	= NULL;//GetVolATMFromSummit( index, ccy, cvName, asOfDate, volMktType );
	...
	return NULL;//new ARM_VolCube(ATMVolCrv, &inVols[0], totalSize, &tenors );
}
	...
	ARM_MarketData::VolType volType ) const
{
	...
	case ARM_MarketData::MKT_ATM_AND_SMILE_VOL:
		return NULL;//GetVolCubeFromSummit( index, ccy, cvName, asOfDate, ARM_ArgConvReverse_VolMktType.GetString(volMktType) );
	default:
		...
		const string& source ) const
{
	...
	return NULL;// new ARM_BSModel( const_cast<ARM_Date&>(asOf), 0, zcCurve	, zcCurve, volCurv );
}

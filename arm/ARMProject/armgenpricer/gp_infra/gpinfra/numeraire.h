...
#include "gpbase/gpvector.h" /// for accessor to the std::vector<double>
...
public:
	...
	virtual void ProcessUnPaidPayoffs( const string& payModelName, ARM_GP_VectorPtr& payoffs, double evalTime, 
		const ARM_PricingStatesPtr& states, const ARM_PricingModel& model ) const= 0;
	...
	virtual void DefaultProcessUnPaidPayoffs( const string& payModelName, ARM_GP_VectorPtr& payoffs, double evalTime, 
		const ARM_PricingStatesPtr& states, const ARM_PricingModel& model ) const;
	/// ARM_Object part
...
public:
	...
	/// accessors
	ARM_GP_VectorPtr GetDiscount(double time) const;
...
protected:
	typedef map<double,ARM_VectorPtr> DiscountMap;

...
struct ARM_GP_StatAverageVector : public ARM_GramFctor
{
	/// default constructor to allow its creation!
	ARM_GP_StatAverageVector() 
		: ARM_GramFctor(), isInitialized_(false)
	{
		// Debug
	}
    ARM_GP_StatAverageVector(const ARM_GP_StatAverageVector& rhs) 
		: ARM_GramFctor(rhs), isInitialized_(rhs.isInitialized_)
	{
		//Debug
	}

	virtual ARM_GramFctorArg operator()(ARM_GramFctorArgVector& arg, 
	...
	virtual ARM_GramFctorPtr Clone() const 
	{ 
		return ARM_GramFctorPtr( new ARM_GP_StatAverageVector( *this ) ); 
	}
private:
	ARM_GramFctorArg pushNewValuesAndComputeAverage(ARM_GramFctorArgVector& arg, size_t histoSize, bool cloneResult);

	bool isInitialized_;
	static std::vector< std::vector<double> > initialValuesPerStates_;

    static string itsFuncName;
};


struct ARM_GP_StatStdDevVector : public ARM_GramFctor
{
	/// default constructor to allow its creation!
	ARM_GP_StatStdDevVector() 
		: ARM_GramFctor(), isInitialized_(false)
	{
		// Debug
	}
    ARM_GP_StatStdDevVector(const ARM_GP_StatStdDevVector& rhs) 
		: ARM_GramFctor(rhs), isInitialized_(rhs.isInitialized_)
	{
		//Debug
	}
	...
	virtual ARM_GramFctorArg operator()(ARM_GramFctorArgVector& arg, 
		...
		virtual ARM_GramFctorPtr Clone() const 
	{ 
		return ARM_GramFctorPtr( new ARM_GP_StatStdDevVector( *this ) ); 
	}
private:
	ARM_GramFctorArg pushNewValuesAndComputeStdDev(ARM_GramFctorArgVector& arg, size_t histoSize, bool cloneResult);

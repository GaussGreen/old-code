...
{
    itsTimes	= rhs.itsTimes  ? new std::vector<double>(*rhs.itsTimes)  : NULL;
...
:	itsTimes( new std::vector<double>(1,1.0) ),
	itsValues0(0.0),
	itsMatIdx(0)
{
...
#endif
		itsTimes = new std::vector<double>( *numeraireTimes );
		itsMatIdx=0;
	}
	else
	{
		...
		itsTimes = new std::vector<double>(1,lastPayTime);
	}
...
void ARM_NumeraireTerminalEventZc::Init(const ARM_ZeroCurveFunctor& discFunctor,const string& modelName,const ARM_TimeInfoPtrVector& timeInfos, const ARM_VectorPtr& numeraireTimes )
{
	...
	else
	{
		...
		itsTimes = new std::vector<double>(1,lastEventTime);
	}
...
{
	CleanUp();
	itsTimes = new std::vector<double>(1,0.0);
	itsMatIdx=0;
	itsValues0 = 1.0;
	...
	const ARM_PricingModel& model )
{
	...
	if( itsRenormalisationIsOn )
	{
		...
		double df0 = 0;//zc->DiscountPrice(evalTime/K_YEAR_LEN);
		...
		else if(pricingDirection== ARM_NumMethod::GP_FWDLOOKING || pricingDirection==ARM_NumMethod::GP_FWDBCKWDLOOKING)
		{
			if (loopPricingDirection == ARM_NumMethod::GP_FWDLOOKING)
			{
				...
				else
				{
					...
					(*discountingTerm) *= ARM_GP_Vector(*model.LocalDiscounts(timeIdx,nextTime-time,states));
...
ARM_GP_VectorPtr ARM_NumeraireCash::GetDiscount(double time) const
{
	...
	if (it != itsDiscountMap.end())
		return ARM_GP_VectorPtr(new ARM_GP_Vector(*it->second));
	else
	{
...
void ARM_NumeraireRolling::ProcessUnPaidPayoffs( const string& payModelName, ARM_GP_VectorPtr& payoffs, double evalTime, 
	const ARM_PricingStatesPtr& states, const ARM_PricingModel& model ) const
{
	const ARM_NumMethodPtr numMethod = model.GetNumMethod();
		...
		size_t timeIdx )
{
	...
	else if(pricingDirection== ARM_NumMethod::GP_FWDLOOKING || pricingDirection==ARM_NumMethod::GP_FWDBCKWDLOOKING)
	{
		...
		{
			if( !itsMatIdx )
				itsRollingDiscount[itsMatIdx] = ARM_GP_VectorPtr(new ARM_GP_Vector(*model.GetRefModel()->GetDiscountFunctor()->DiscountFactor( "",startTime, endTime, states)));
			else
			{
				ARM_GP_VectorPtr rollingDiscount = ARM_GP_VectorPtr(new ARM_GP_Vector(*model.GetRefModel()->GetDiscountFunctor()->DiscountFactor("",evalTime, endTime, states)));
				(*rollingDiscount) /= ARM_GP_Vector(*model.GetRefModel()->GetDiscountFunctor()->DiscountFactor("",evalTime, startTime, states));
...
void ARM_NumeraireRollingPayment::Init(const ARM_ZeroCurveFunctor& discFunctor,const string& modelName,const ARM_TimeInfoPtrVector& timeInfos, const ARM_VectorPtr& numeraireTimes )
{
	...
	{
		size_t nbEvents = timeInfos.size();
		itsTimes = new std::vector<double>(nbEvents);
	...
	if( model.GetNumMethod() != ARM_NumMethodPtr(NULL) )
	{
		...
		else if(pricingDirection== ARM_NumMethod::GP_FWDLOOKING || pricingDirection==ARM_NumMethod::GP_FWDBCKWDLOOKING)
		{
			if (loopPricingDirection == ARM_NumMethod::GP_FWDLOOKING)
			{
				...
				{
					ARM_GP_VectorPtr discountingTerm = CreateClonedPtr<ARM_GP_Vector>(&*GetDiscount(time));
					(*discountingTerm) *= ARM_GP_Vector(*model.GetRefModel()->GetDiscountFunctor()->DiscountFactor("",time, nextTime, states));
					itsDiscountMap.insert(make_pair(nextTime,discountingTerm));
				}
			}
		}

...
{
	/*if(itsIntegrandType== GL)
	{*/
		if (m_isDigital)
		{
			double p = 0.5 + m_sens * value() / ARM_NumericConstants::ARM_PI;
			return p;
		}
		else
		{
...
void ARM_HestonOptionPricer::prices(std::vector<double>& pricesVec)
{
	pricesVec.resize(getNbIntegrals());
	std::vector<double> valuesVec(getNbIntegrals());
	values(valuesVec);
	...
	if (m_isDigital)
	{
		for (i = 0; i < getNbIntegrals(); ++i)
			pricesVec[i] = 0.5 + m_sens * valuesVec[i] / ARM_NumericConstants::ARM_PI;
	}
	else
	{
		for (i = 0; i < getNbIntegrals(); ++i)
		{
			if(m_shstrike < K_DOUBLE_TOL)
			{
				pricesVec[i] = m_sens == 1 ? m_forward : 0.;
			}

			pricesVec[i] = m_shfwd - m_shstrikes[i] * valuesVec[i] / ARM_NumericConstants::ARM_PI;

			if(m_shift > 0.)
				pricesVec[i] = m_sens == 1 ? valuesVec[i] : valuesVec[i] - m_shfwd + m_shstrikes[i];
			else
				pricesVec[i] = m_sens == 1 ? valuesVec[i] - m_shfwd + m_shstrikes[i] : valuesVec[i];
		}
	}
}
										  ...
										  double kappa, double theta, double rho, double nu)
{
	if (0 == m_idx) 
	{
		ComputeCoeffs(m_idx , m_reset, u, A, B, kappa, theta, rho, nu);
	}
	else {
		double deltat = m_reset - m_times[m_idx-1];

		ComputeCoeffs(m_idx , deltat, u, A, B, kappa, theta, rho, nu);
		
		for(int k = m_idx - 1; k >= 0; k--)
		{
			B *= m_levels[k+1] * m_levels[k+1] / (m_levels[k]*m_levels[k]);
...
void ARM_HestonOptionPricer::setStrikes(const std::vector<double>& strikes)
{
	m_strikes = strikes;

	m_shstrikes.resize(strikes.size());
	m_logFKs.resize(strikes.size());

	for (int i = 0; i < strikes.size(); ++i)
	{
		m_shstrikes[i] = m_shift < 0. ? m_shfwd * (1. + fabs(m_shift)) - m_strikes[i] : m_strikes[i] + (1. - m_shift) * m_shfwd;
		m_logFKs[i] = std::log(complex<double>(m_shfwd,0.0)/complex<double>(m_shstrikes[i],0.0));
	}
}

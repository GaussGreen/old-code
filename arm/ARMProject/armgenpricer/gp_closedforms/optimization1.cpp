	...
	Integer tdfjac, Nag_Comm *comm )
{
	...
	std::vector<double> weigths(cm->weigthlist);
	std::vector<double> prices(cm->pricelist);
	...
	std::vector<double>* deriv0=new(std::vector<double>)(m*n);
	...
	{
		std::vector<double> param_weigths(cm->paramweightlist);
		std::vector<double> param_previousvalues(cm->previousparamlist);

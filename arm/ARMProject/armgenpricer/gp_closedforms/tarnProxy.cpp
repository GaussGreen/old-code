...
///	Action : Constructor
////////////////////////////////////////////////////
ARM_TarnProxy::ARM_TarnProxy(	const std::vector<double>* resetDates,
	...
	itsGrid(0)
{
	...
	int nb			= 400;
	double stdDev	= 5.;
	itsGrid = ARM_GP_VectorPtr(new std::vector<double>(2*nb+1));
...
void ARM_TarnProxy::Build(const std::vector<double>* fwds,const vector< ARM_DensityFunctor* >& densityFunctors)
{
	...
	int i;
	itsFunc.resize(n);
	ARM_GP_VectorPtr proba=ARM_GP_VectorPtr(new std::vector<double>(itsGrid->size()));
...
void ARM_TarnProxy::Price(double correl, int simul)
{
	...
	std::vector<double> vecIndep(n);
	std::vector<double> vecReal(n);

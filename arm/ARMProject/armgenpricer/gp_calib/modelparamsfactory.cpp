...
// FIXMEFRED: mig.vc8 (22/05/2007 18:05:12): sqrt(1.+8.
				...
				std::vector<double> firstValues(nbTimes),valueAtpoint(nbCurves);
				ARM_GP_T_Vector<std::vector<double>> curvesValues(nbTimes);
				...
				ARM_MultiCurve *curves = new ARM_MultiCurve(ARM_GP_Vector(*breakPointTimes),curvesValues,static_cast<ARM_MultiCurveInterpolator*>(correlParam->GetMultiCurve()->GetInterpolator()->Clone()));
		...
		else
		{
			CC_NS(ARM_Check,CheckSameArgSize)(*values,*breakPointTimes,"values","breakPointTimes");
				return NULL;//ARM_CurveModelParam( (ARM_ModelParamType::ParamNb) type, values, breakPointTimes, name, interpolatorName,lowerbound, upperbound, adviseBreakPointTimes, currency );
		}
	}

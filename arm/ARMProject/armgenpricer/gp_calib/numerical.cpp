	...
	itsSolver				( NULL )
{
	...
	{
		if ( itsCalibSecDensities[i]->getDensityFunctor()->GetIsDirect() )
...
{	
	...
	///
	double asOf = model->GetAsOfDate().GetJulian();
	ARM_GP_VectorPtr resetDates( new ARM_GP_Vector(*itsCalibSchedule->GetResetDates()));
	...
	size_t sizePf = 0;//GetPortfolio()->size();

	/// get time steps from num method
	ARM_GP_VectorPtr timeSteps = ARM_GP_VectorPtr(static_cast<ARM_GP_Vector*>(numMethod->GetTimeSteps()->Clone()));
	...
	for (size_t i = sizePf-1 ; i>=0 ; i-- )
	{
		...
		if (true)// fabs( GetPortfolio()->GetWeights()->Elt(i) ) > K_DOUBLE_TOL )
		{
			...
			/// Build vanilla arg
			ARM_VanillaArg* arg = NULL;//ARM_ConverterFromKernel::ConvertSecuritytoArgObject(GetPortfolio()->GetAsset(i), asOf );
			arg->SetIndex(i);
	...
	for (size_t i = (*portfolioIdx)[0] - 1 ; i>=0 ; i-- )
	{
...
void ARM_NumericalModelFitter::Initialise() const
{
	...
	if ( ARM_CurveModelParam* curveParam = dynamic_cast<ARM_CurveModelParam*>(param) )
	{
		...
		{
			ARM_GP_Vector* resetDates     = new ARM_GP_Vector(*itsCalibSchedule->GetResetDates());
...
ARM_IntVectorPtr ARM_NumericalModelFitter::ComputePortfolioSecuritiesIdx() const 
{
	...
	{
		int idx = -1;
		double secResetDate = 0;//GetPortfolio()->GetAsset(i)->GetResetDates()->Elt(0);
...
///	Action :
////////////////////////////////////////////////////
const std::vector<double>& ARM_NumericalModelFitter::getCalibSecInterestTerms (size_t resetIndex) const
{		
...
///----------------------------------------------------------------
void BootstrapPriceFunc::Init (	ARM_PricingModel*			model,

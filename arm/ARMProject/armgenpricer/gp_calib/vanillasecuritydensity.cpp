...
void ARM_VanillaSecurityDensity::AssociateCalibSchedule(ARM_DateStripPtr calibSchedule, 
														...
														std::vector<double>& InterestTerms, ARM_IntVector& PayDatesRelIndexes, ARM_IntVector& PayDatesAbsIndexes)
{
	...
	{				
		...
		for (i=0; i<nbPeriods; i++)
			InterestTerms[i] = (*itsCalibSchedule->GetInterestTerms())[startIndex + i];
	}
	...
	{	
		...
		double startDate = (*itsCalibSchedule->GetFlowStartDates())[startIndex];

		for (i=0; i<InterestTerms.size(); i++)
		{
			double endDate		= (*itsCalibSchedule->GetFlowEndDates())[PayDatesAbsIndexes[i]];
...
{ 
	...
	/// consistency checks
	std::vector<double>* resetDates	= itsCalibSchedule->GetResetDates();
	std::vector<double>* startDates	= itsCalibSchedule->GetFlowStartDates();
	std::vector<double>* endDates		= itsCalibSchedule->GetFlowEndDates();
	size_t N = resetDates->size();
	
	int resetIndex (-1);
	for (size_t i(0); i<N; i++)
		if ((*resetDates)[i] == itsResetDate)
		{
			resetIndex = i;
			break;
		}
	...
	if (StartDate != (*resetDates)[resetIndex])
...
////////////////////////////////////////////////////
double ARM_VanillaSecurityDensity::ComputeForwardRate (double StartDate, double EndDate, const ARM_IntVector& PayDatesAbsIndexes, const std::vector<double>& InterestTerms) const
{	
	if (itsCalibSchedule.IsNull())
	...
	double asOf			= 0;//itsZeroCurve->GetAsOfDate().GetJulian();
	std::vector<double>* ends = itsCalibSchedule->GetFlowEndDates();
		
	double fwd =	0;/*getZeroCurve()->DiscountPrice((StartDate-asOf)/K_YEAR_LEN)
	...
	for (size_t i = 0; i<PayDatesAbsIndexes.size(); i++)
	{
		...
		level += 0;// InterestTerms[i] * getZeroCurve()->DiscountPrice(((*ends)[idx]-asOf)/K_YEAR_LEN);
	}
...
///	Action : sensi
////////////////////////////////////////////////////
double ARM_VanillaSecurityDensity::ComputeLevel (double startdate, double enddate, const ARM_IntVector& PayDatesAbsIndexes, const std::vector<double>& InterestTerms) const
{	
	/*if (itsCalibSchedule.IsNull())
...
{		
	...
	if( states->GetPayoffsSize() == 1 )
	{
		...
		std::vector<double>::iterator vecIter = vec->begin(), vecEnd = vec->end();
	...
	else
	{
		...
		std::vector<double>::iterator vecIter, vecEnd;

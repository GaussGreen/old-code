...
double ARM_VanillaSwaptionArg::Price(ARM_PricingModel* model) const
{
    ...
    if(IRModel)
	{
		if (GetAtTheMoneyFlag())
		{
			...
			ARM_VectorPtr annuity  = ARM_VectorPtr(NULL);//newModel->Annuity(GetCurveName(),GetEvalTime(),*itsFixPayTimes,*itsFixPayPeriods,dumStates);
#if defined(__GP_STRICT_VALIDATION)
...
double ARM_VanillaSwaptionArg::Vega(ARM_PricingModel* model) const
{
	...
	double asOfDate = 0;//zcCurve->GetAsOfDate().GetJulian();

	double annuity = 0.0, floatLeg = 0.0;

	for (int i = 0; i < itsFloatStartTimes->size(); ++i)
	{
		floatLeg += 0;//(*itsFloatNominal)[i]*(zcCurve->DiscountPrice((*itsFloatStartTimes)[i]/K_YEAR_LEN));
		floatLeg += 0;//-(*itsFloatNominal)[i]*(zcCurve->DiscountPrice((*itsFloatEndTimes)[i]/K_YEAR_LEN));
	}

	for (size_t i = 0; i < itsFixPayTimes->size(); ++i)
	{
		annuity += 0;//(*itsFixNominal)[i]*(*itsFixPayPeriods)[i]*(zcCurve->DiscountPrice((*itsFixPayTimes)[i]/K_YEAR_LEN));
	}
...
ARM_GenSecurityPtr ARM_VanillaSwaptionArg::VanillaSwaptionToGenSec() const
{
	...
	{
		ARM_GP_VectorPtr spreadVector((ARM_GP_VectorPtr(static_cast<ARM_GP_Vector*> (GetFundingSpread()->Clone()))));
	...
	{
		ARM_GP_VectorPtr strikesVector((ARM_GP_Vector*) GetStrikes()->Clone());

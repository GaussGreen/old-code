...
double ARM_VanillaSmiledSwaption::Price(ARM_PricingModel* model) const
{
    ...
    if(IRModel)
	{
		Price = IRModel->VanillaSmiledSwaption(
			...
			GetEvalTime(),
			itsResetTime,
			itsFixNominal->GetValues(),
			itsFloatNominal->GetValues(),
			itsStartTime,
			itsEndTime,
			itsFloatResetTimes->GetValues(),
			itsFloatStartTimes->GetValues(),
			itsFloatEndTimes->GetValues(),
			itsFloatIntTerms->GetValues(),
			itsFixPayTimes->GetValues(),
			itsFixPayPeriods->GetValues(),
			itsStrikes->GetValues(),
			GetCallPut(),
			dumStates,
			itsTargetParams,

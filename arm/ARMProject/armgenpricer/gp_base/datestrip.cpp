...
/////////////////////////////////////////////////////////////////
void ARM_DateStrip::CheckSizeAndSorted( size_t defaultSize, std::vector<double>* vec, const char* objName )
{
...
/////////////////////////////////////////////////////////////////
void ARM_DateStrip::CheckSize( size_t defaultSize, std::vector<double>* vec, const char* objName )
{
...
///	Returns: void
///	Action : constructor with the already given std::vector<double>
...
ARM_DateStrip::ARM_DateStrip( 
	...
	std::vector<double>* FlowEndDates,			/// period of interest 
	...
	itsIsBuiltFromOutside( true )		/// says that it is built from input and not from outside
{
	...
	itsFlowStartDates->assign(FlowStartDates->begin(),FlowStartDates->end());
    itsFlowEndDates		= (std::vector<double>*) FlowEndDates->Clone();
	if (FwdRateStartDates)
		itsFwdRateStartDates= (std::vector<double>*) FwdRateStartDates->Clone();
	if (FwdRateEndDates)
		itsFwdRateEndDates	= (std::vector<double>*) FwdRateEndDates->Clone();
	itsResetDates		= (std::vector<double>*) ResetDates->Clone();
	itsPaymentDates		= (std::vector<double>*) PaymentDates->Clone();
    itsInterestDays		= (std::vector<double>*) InterestDays->Clone();
	itsInterestTerms	= (std::vector<double>*) InterestTerms->Clone();*/
	Init();
}
...
std::vector<double>* ARM_DateStrip::GenerateDatesFromTimingAndGap(
	int timing,
	int gap,
	const char* calendar,
	...
	std::vector<double>* FlowEndDates ) const
{
	...
	std::vector<double>* result = new std::vector<double>( nFlows );
	int i;

    for (i = 0; i < nFlows; ++i) 
    {
        ARM_Date tempDate = ARM_Date((*flowDates)[i]);
        tempDate.GapBusinessDay(gap, const_cast<char*>(calendar) ); /// UGLY but forced to const cast
        (*result)[i]= tempDate.GetJulian();
    }
...
///	Routine: ARM_DateStrip
///	Returns: constructor
///	Action : initialises correctly all the std::vector<double> member objects
...

{
    ...
    {
	    itsFlowStartDates   = new std::vector<double>;
        itsFlowEndDates     = new std::vector<double>;
        itsFwdRateStartDates= new std::vector<double>;
        itsFwdRateEndDates  = new std::vector<double>;
	    itsResetDates       = new std::vector<double>;
	    itsPaymentDates     = new std::vector<double>;
        itsInterestDays     = new std::vector<double>;
        itsInterestTerms    = new std::vector<double>;
    }
    else
    {
...
void ARM_DateStrip::CptCashFlowDates()
{
	...
	//std::vector<double>* tmpFlowStartDates = CptStripOfStartDates( 
	...
	//std::vector<double>* tmpResetFlowStartDates;
	...
	//	}
	//	else
	//		tmpResetFlowStartDates = (std::vector<double>*) tmpFlowStartDates->Clone();
	//}

	///// initialise all vectors
	//int nFlows			= MAX(tmpFlowStartDates->size(), tmpResetFlowStartDates->size());

	//itsFlowStartDates	= new std::vector<double>( nFlows );
	//itsFlowEndDates		= new std::vector<double>( nFlows );
	//itsInterestDays		= new std::vector<double>( nFlows );
	//itsInterestTerms	= new std::vector<double>( nFlows );


	///// temporary variable
	//std::vector<double>* resetFlowStartDates = new std::vector<double>( nFlows );
	//std::vector<double>* resetFlowEndDates	= new std::vector<double>( nFlows );
 ...
 //   /// Fill cash flow period start and end dates, reset and payment dates
	...
	///// a new std::vector<double>( nFlows ) for itsResetDates
	...
	//	itsPaymentDates		= new std::vector<double>( itsFlowEndDates->size(), finalPayDate.GetJulian() );
	//}
	//else
	//{
	...
	//{
	//	itsFwdRateStartDates = (std::vector<double>*) itsFlowStartDates->Clone();
	//	itsFwdRateEndDates	 = (std::vector<double>*) itsFlowEndDates->Clone();
	//}
	//else 
	///// otherwise the logic comes from the reset timing 
	///// and the fwdRateEndDates are computed adding the resetFreq
	//{
	//	itsFwdRateStartDates = (std::vector<double>*) itsResetDates->Clone();
	//	itsFwdRateEndDates	 = new std::vector<double>(itsResetDates->size());
...
std::vector<double>* ARM_DateStrip::GetMemberData( int type )
{
	switch( type )
    {
        case K_START_DATES:
...
std::vector<double>* ARM_DateStrip::CptStripOfStartDates(
	ARM_Date& StartDate, 
	ARM_Date& EndDate, 
	int frequency,
	int fwdRule, 
	...
	int adjFirstdate)
{
	...
	if ( frequency == K_ZEROCOUPON )
		return new std::vector<double>(1, StartDate.GetJulian());
	...
	std::vector<double> result(SizeMax);
	...
	///  Adjusting the dates
	std::vector<double> resultAdj(nbFlow);
...
// FIXMEFRED: mig.vc8 (22/05/2007 15:51:14): explicit cast
	return new std::vector<double>(resultAdj.begin(), resultAdj.end());
}
...
void ARM_DateStrip::SetFlowStartDates( std::vector<double>* FlowStartDates )
{
	/*delete itsFlowStartDates; 
	itsFlowStartDates = (std::vector<double>*) FlowStartDates->Clone(); */
}

void ARM_DateStrip::SetFlowEndDates( std::vector<double>* FlowEndDates )
{
	/*delete itsFlowEndDates; 
	itsFlowEndDates = (std::vector<double>*) FlowEndDates->Clone();*/
}

void ARM_DateStrip::SetFwdRateStartDates( std::vector<double>* FwdRateStartDates )
{
	/*delete itsFwdRateStartDates;
	itsFwdRateStartDates = (std::vector<double>*) FwdRateStartDates->Clone();*/
}

void ARM_DateStrip::SetFwdRateEndDates( std::vector<double>* FwdRateEndDates)
{
	/*delete itsFwdRateEndDates;
	itsFwdRateEndDates = (std::vector<double>*) FwdRateEndDates->Clone();*/
}

void ARM_DateStrip::SetResetDates( std::vector<double>* ResetDates )
{
	/*delete itsResetDates;
	itsResetDates = (std::vector<double>*) ResetDates->Clone();*/
}

void ARM_DateStrip::SetPaymentDates( std::vector<double>* PaymentDates )
{
	/*delete itsPaymentDates;
	itsPaymentDates = (std::vector<double>*) PaymentDates->Clone();*/
}

void ARM_DateStrip::SetInterestDays( std::vector<double>* InterestDays )
{
	/*delete itsInterestDays; 
	itsInterestDays  = (std::vector<double>*) InterestDays->Clone();*/ 
}

void ARM_DateStrip::SetInterestTerms( std::vector<double>* InterestTerms ) 
{
	/*delete itsInterestTerms;
	itsInterestTerms = (std::vector<double>*) InterestTerms->Clone();*/ 
}
...
void ARM_DateStrip::SetFlowStartDatesNoClone( std::vector<double>* FlowStartDates ) 
{
	delete itsFlowStartDates;
	itsFlowStartDates =  FlowStartDates; 
}

void ARM_DateStrip::SetFlowEndDatesNoClone( std::vector<double>* FlowEndDates ) 
{
	delete itsFlowEndDates;
	itsFlowEndDates =  FlowEndDates; 
}

void ARM_DateStrip::SetFwdRateStartDatesNoClone( std::vector<double>* FwdRateStartDates ) 
{
	delete itsFwdRateStartDates;
	itsFwdRateStartDates = FwdRateStartDates; 
}

void ARM_DateStrip::SetFwdRateEndDatesNoClone( std::vector<double>* FwdRateEndDates) 
{
	delete itsFwdRateEndDates; 
	itsFwdRateEndDates = FwdRateEndDates; 
}

void ARM_DateStrip::SetResetDatesNoClone( std::vector<double>* ResetDates ) 
{
	delete itsResetDates; 
	itsResetDates =  ResetDates; 
}

void ARM_DateStrip::SetPaymentDatesNoClone( std::vector<double>* PaymentDates ) 
{
	delete itsPaymentDates; 
	itsPaymentDates =  PaymentDates;
}

void ARM_DateStrip::SetInterestDaysNoClone( std::vector<double>* InterestDays ) 
{
	delete itsInterestDays;
	itsInterestDays  =  InterestDays; 
}

void ARM_DateStrip::SetInterestTermsNoClone( std::vector<double>* InterestTerms ) 
{
	delete itsInterestTerms;
...
{
	//std::vector<double> FlowStartDates(&(*itsFlowStartDates->begin())+begin, &(*itsFlowStartDates->begin())+end);
	//delete itsFlowStartDates;
	//itsFlowStartDates = (std::vector<double>*)FlowStartDates.Clone();

	//std::vector<double> FlowEndDates(&(*itsFlowEndDates->begin())+begin, &(*itsFlowEndDates->begin())+end);
	//delete itsFlowEndDates;
	//itsFlowEndDates = (std::vector<double>*)FlowEndDates.Clone();

	//std::vector<double> FwdRateStartDates(&(*itsFwdRateStartDates->begin())+begin, &(*itsFwdRateStartDates->begin())+end);
	//delete itsFwdRateStartDates;
	//itsFwdRateStartDates = (std::vector<double>*)FwdRateStartDates.Clone();

	//std::vector<double> FwdRateEndDates(&(*itsFwdRateEndDates->begin())+begin, &(*itsFwdRateEndDates->begin())+end);
	//delete itsFwdRateEndDates;
	//itsFwdRateEndDates = (std::vector<double>*)FwdRateEndDates.Clone();

	//std::vector<double> ResetDates(&(*itsResetDates->begin())+begin, &(*itsResetDates->begin())+end);
	//delete itsResetDates;
	//itsResetDates = (std::vector<double>*)ResetDates.Clone();

	//std::vector<double> PaymentDates(&(*itsPaymentDates->begin())+begin, &(*itsPaymentDates->begin())+end);
	//delete itsPaymentDates;
	//itsPaymentDates = (std::vector<double>*)PaymentDates.Clone();

	//std::vector<double> InterestDays(&(*itsInterestDays->begin())+begin, &(*itsInterestDays->begin())+end);
	//delete itsInterestDays;
	//itsInterestDays = (std::vector<double>*)InterestDays.Clone();

	//std::vector<double> InterestTerms(&(*itsInterestTerms->begin())+begin, &(*itsInterestTerms->begin())+end);
	//delete itsInterestTerms;
	//itsInterestTerms = (std::vector<double>*)InterestTerms.Clone();
}

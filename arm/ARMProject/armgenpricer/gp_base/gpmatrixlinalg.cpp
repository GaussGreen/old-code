...
ARM_GP_Matrix* ACPTransformation(ARM_GP_Matrix* matrix, ARM_GP_Vector& eigenvalues, int nbFactors)
{
	...
	// MARCHE PAS ...
	{
        ...
        std::vector<double> oldEigenValues(nbDims);
		for(i = 0; i < nbDims; ++i)
        {
        ...
        std::vector<double>::iterator pos,prevPos=oldEigenValues.end(),beginPos=oldEigenValues.begin();
        size_t offset=0;
		for (i = 0; i < (nbFactors==-1?nbDims:nbFactors); ++i)
		{
            pos = find(oldEigenValues.begin(), oldEigenValues.end(), eigenvalues[i]);
			if(pos==prevPos)
...
double ACPTransformationWithRescalling(ARM_GP_Matrix* matrix, std::vector<double>& eigenvalues, ARM_GP_Matrix& ACPMatrix)
{
...
void LUBackSubstitution( ARM_GP_Matrix& a, vector<int>* indx, std::vector<double>* b )
{
    size_t ii,ip,j;
	signed int i;
	size_t itsCols = a.cols();
...
/// is B ;) it is return as an std::vector<double> * 
////////////////////////////////////////////////////
std::vector<double> * LeastSquareRegression( const ARM_GP_Matrix& X , const std::vector<double>& Y )
{
	...
	std::vector<double> * B = nullptr;/*XPrime*Y;
	XPrime *= X;*/
...
void SingularValuesDecomposition(ARM_GP_Matrix& a,std::vector<double>& w,ARM_GP_Matrix& v)

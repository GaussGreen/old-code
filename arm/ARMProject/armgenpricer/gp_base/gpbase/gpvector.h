...
private:
	...
	void CheckRange(size_t i) const
	{
		...
		if ( 0 == itsValues.size() )
		{
			std::string msg("ARM_GP_T_Vector::CheckRange(size_t i), try to access element of an empty vector");
			...
			{
				char msg[255];
				sprintf( msg, "Range error in ARM_GP_T_Vector: index %d  not in the range [0, %d[ ", i, itsValues.size() );

...
public:
    ...
    //inline void push_backRow(const std::vector<double>& RowValues);
	inline void push_backRow(const ARM_GP_T_Vector<T>& RowValues);
    //inline void push_backColumn(const std::vector<double>& ColValues);
	inline void push_backColumn(const ARM_GP_T_Vector<T>& ColValues);
...
private:
	...
	{
#if !defined( __ARM_MATRIX_NO_RANGE_CHECK )
		if ( (0 == itsRowsNb) ||  (0 == itsColsNb) )
		{
			std::string msg("ARM_GP_T_Matrix::CheckRange(size_t i, size_t j), try to access element of an empty matrix");
			throw Exception(__LINE__, __FILE__, ERR_INVALID_DATA, msg.c_str() );
			...
			{
				char msg[255];
				sprintf( msg, "Range error in ARM_GP_T_Matrix: row index %d  not in the range [0, %d[ ", i, itsRowsNb );
				...
				{
					char msg[255];
					sprintf( msg, "Range error in ARM_GP_T_Matrix: column index %d  not in the range [0, %d[ ", j, itsColsNb );
					throw Exception(__LINE__, __FILE__, ERR_INVALID_DATA, msg );
				}
			}
		}
...
///added new row
template <typename T> void ARM_GP_T_Matrix<T>::push_backRow(const ARM_GP_T_Vector<T>& RowValues)
{
	if( empty() )
	{
        itsRowsNb = 1;
...
///added new line
template <typename T> void ARM_GP_T_Matrix<T>::push_backColumn(const ARM_GP_T_Vector<T>& ColValues)
{

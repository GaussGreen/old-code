...
void ARM_CCSCalculator::ComputeProductVectorsFromCurves()
{
	...
	std::vector<double>& forResetDates = itsForDateStrip->GetResetDates() ;
	...
	std::vector<double>& domResetDates = itsDomDateStrip->GetResetDates() ;
	...
	std::vector<double>& domStartDates  = itsDomDateStrip->GetFlowStartDates() ;
	std::vector<double>& forStartDates = itsForDateStrip->GetFlowStartDates() ;
...
ARM_CstManagerPtr ARM_CCSCalculator::ComputeCstManager()
{
	...
	values.push_back(ARM_GramFctorArg(ARM_GP_VectorPtr(static_cast<ARM_GP_Vector*>itsvDomNominal.Clone() )));

	names.push_back(CCSProfileNamesTable[ForNominalProfile]);
	values.push_back(ARM_GramFctorArg(ARM_GP_VectorPtr(static_cast<ARM_GP_Vector*>itsvForNominal.Clone() )));

	names.push_back(CCSProfileNamesTable[ForMarginProfile]);
	values.push_back(ARM_GramFctorArg(ARM_GP_VectorPtr(static_cast<ARM_GP_Vector*>itsvForSpread.Clone() )));

	names.push_back(CCSProfileNamesTable[DomMarginProfile]);
	values.push_back(ARM_GramFctorArg(ARM_GP_VectorPtr(static_cast<ARM_GP_Vector*>itsvDomSpread.Clone() )));
...
ARM_DateStripCombiner ARM_CCSCalculator::DatesStructure() const
{
	...
	size_t nbPastNoCall;
	std::vector<double>& resetDates = NULL;
	...
	if (itsDomDateStrip == ARM_DateStripPtr(NULL))
	{
		...
		ARM_DateStrip ForexSched(itsStartDate,itsEndDate,itsDomFreq,itsDomDaycount,resetCalendar,fwdRule,intRule,itsStubRule,
								-abs(itsFxResetGap) ,itsDomFreq, GETDEFAULTVALUE, payCalendar, payTiming, payTiming);

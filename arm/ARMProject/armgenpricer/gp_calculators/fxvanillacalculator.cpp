...
void ARM_FXVanillaCalculator::CreateAndSetModel()
{
	...
	std::vector<double> ordinates(sizeCv,1.0);
...
double ARM_FXVanillaCalculator::Price()
{
	...
	std::vector<double>& resetDates = itsDateStrip->GetResetDates() ;
	std::vector<double>& payDates = itsDateStrip->GetPaymentDates();
	...
	std::vector<double> slidingExpiryTimes, slidingSettelTimes, slidingStrikes, slidingCoeffs, slidingLeverages, slidingCouponMin, slidingCouponMax;
	...
	for ( size_t i(0); i<size; i++)
	{
		...
		//From Curves to doubles
		double strike   = itsStrikeCv.Interpolate(expiryTime);
		std::vector<double> strikePerState(1,strike);
		double strike2 = itsStrike2Cv.Interpolate(expiryTime);
		double alpha = itsAlphaCv.Interpolate(expiryTime);
		double beta = itsBetaCv.Interpolate(expiryTime);
		...
		else if(itsVanillaType == ARM_FXVanillaType::perf)
		{
			...
			if (barrier!=0.)
			{
				std::vector<double> invbarrierPerState(1, 1./barrier);

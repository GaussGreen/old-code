	...
	itsProba(ARM_VectorPtr(NULL))
{
	...
	/// nb fix flows
	std::vector<double>& resetDates = GetCalibDateStrip()->GetResetDates();
	...
	std::vector<double>& callDates = GetExerciseDateStripUnadj()->GetResetDates();
	...
	itsProba			= ARM_VectorPtr(new std::vector<double>(itsNbComputedProba,0.0));
...
void ARM_LocalCSOCalculator::ComputeProductVectorsFromCurves()
{
	...
	std::vector<double>& exerciseDates = GetExerciseDateStrip()->GetResetDates() ;
...
ARM_CstManagerPtr ARM_LocalCSOCalculator::CreateCstManager()
{
	...
	std::vector<double> cpnStrikePlusFloor = itsvCpnFloor + itsvCpnStrike;
	std::vector<double> cpnStrikePlusCap   = itsvCpnCap + itsvCpnStrike;
	std::vector<double> cpnFloorTimesNotio = itsvCpnFloor * itsvCpnNominal;
    std::vector<double> spreadTimesNotio	 = itsvFundSpread * itsvFundNominal;
	std::vector<double> leverageTimesNotio = itsvFundLeverage * itsvFundNominal;
	...
	{		
		...
		/// values
// FIXMEFRED: mig.vc8 (25/05/2007 14:53:19):cast
		values.push_back( ARM_GramFctorArg(ARM_VectorPtr(new std::vector<double>(&(*(itsvFundNominal.begin()		+ itsvFundIndex[i])), &(*(itsvFundNominal.end()))))) );/// first value
		values.push_back( ARM_GramFctorArg(ARM_VectorPtr(new std::vector<double>(&(*(leverageTimesNotio.begin()	+ itsvFundIndex[i])),	&(*(leverageTimesNotio.end()))))) );   //// ...
        values.push_back( ARM_GramFctorArg(ARM_VectorPtr(new std::vector<double>(&(*(itsvCpnNominal.begin()		+ itsvCpnIndex[i])),	&(*(itsvCpnNominal.end()))))) );   //// ...
		values.push_back( ARM_GramFctorArg(ARM_VectorPtr(new std::vector<double>(&(*(itsvCpnLeverageLong.begin()	+ itsvCpnIndex[i])),	&(*(itsvCpnLeverageLong.end()))))) );
		values.push_back( ARM_GramFctorArg(ARM_VectorPtr(new std::vector<double>(&(*(itsvCpnLeverageShort.begin()	+ itsvCpnIndex[i])),	&(*(itsvCpnLeverageShort.end()))))) );
		values.push_back( ARM_GramFctorArg(ARM_VectorPtr(new std::vector<double>(&(*(cpnFloorTimesNotio.begin()	+ itsvCpnIndex[i])),	&(*(cpnFloorTimesNotio.end()))))) );
		values.push_back( ARM_GramFctorArg(ARM_VectorPtr(new std::vector<double>(&(*(cpnStrikePlusFloor.begin()	+ itsvCpnIndex[i])),	&(*(cpnStrikePlusFloor.end()))))) );
		values.push_back( ARM_GramFctorArg(ARM_VectorPtr(new std::vector<double>(&(*(cpnStrikePlusCap.begin()		+ itsvCpnIndex[i])),	&(*(cpnStrikePlusCap.end()))))) );
	}

	names.push_back("SpreadTimesNotio"); /// ....
	values.push_back( ARM_GramFctorArg(ARM_VectorPtr((std::vector<double>&)spreadTimesNotio.Clone() ) ));   //// ...

	if (IsSwitchable())
	{
		int size = itsvFundSpread.size();
		std::vector<double> auxVector(size,1000.);
		std::vector<double> auxNull(size,0.);
		std::vector<double> cpnStrikePlusFloorFund = auxNull-itsvFundSpread-auxVector;
		std::vector<double> cpnStrikePlusCapFund   = auxNull-itsvFundSpread+auxVector;
		std::vector<double> cpnFloorFundTimesNotio = itsvCpnNominal*cpnStrikePlusFloorFund;
		...
		{		
			...
			/// values
// FIXMEFRED: mig.vc8 (25/05/2007 14:53:16):cast
			values.push_back( ARM_GramFctorArg(ARM_VectorPtr(new std::vector<double>(&(*(cpnFloorFundTimesNotio.begin()	+ itsvCpnIndex[i])),	&(*(cpnFloorFundTimesNotio.end()))))) );
			values.push_back( ARM_GramFctorArg(ARM_VectorPtr(new std::vector<double>(&(*(cpnStrikePlusFloorFund.begin()	+ itsvCpnIndex[i])),	&(*(cpnStrikePlusFloorFund.end()))))) );
			values.push_back( ARM_GramFctorArg(ARM_VectorPtr(new std::vector<double>(&(*(cpnStrikePlusCapFund.begin()		+ itsvCpnIndex[i])),	&(*(cpnStrikePlusCapFund.end()))))) );
		}
	}
...
{	
	std::vector<double>& resetDates = GetStructDateStrip()->GetResetDates();
	std::vector<double>& startDates = GetStructDateStrip()->GetFlowStartDates();
...
void ARM_LocalCSOCalculator::SetDiagStrikeToFrontier(ARM_StdPortfolioPtr& swaptionPF)
{
	...
	ARM_VectorPtr lastFrontier(new std::vector<double>(nbSwaptions));
	ARM_VectorPtr atmStrikes(new std::vector<double>(nbSwaptions));
	...
	vector<std::vector<double>> frontiers;
...
ARM_StdPortfolioPtr ARM_LocalCSOCalculator::CreateSOPortfolioFor2F(bool isSOCond,bool isLong)
{
	...
	std::vector<double>& startDates = GetExerciseDateStripUnadj()->GetFlowStartDates();
	std::vector<double>& endDates = GetExerciseDateStripUnadj()->GetFlowEndDates();
	std::vector<double>& cpnResetDates = GetStructDateStrip()->GetResetDates() ;
...
ARM_StdPortfolioPtr ARM_LocalCSOCalculator::CreateSwaptionPortfolioFor2F(CalibStrikeType cst)
{
	...
	std::vector<double>& startDates = GetExerciseDateStrip()->GetFlowStartDates();
	size_t i,nbFlows = startDates->size();
...
ARM_Swaption* ARM_LocalCSOCalculator::CreateVarNotionalSwaptionAtExer (int exerIdx, ARM_Swaption*& fwdStartSwaption,CalibStrikeType calibStrikeType,bool isUpdateStrike)
{
	...
	std::vector<double>& exerDates = itsExerciseDateStrip->GetResetDates();	
	...
	// precomputation
	std::vector<double> A(itsCpnSize), Bshort(itsCpnSize), Blong (itsCpnSize);
	std::vector<double> Afund(itsCpnSize), Bfund(itsCpnSize);

	std::vector<double>& cpnCoverages = GetStructDateStrip()->GetInterestTerms();
	...
	std::vector<double> df0(itsCpnSize), Slong0(itsCpnSize), Sshort0(itsCpnSize), Sfund0(itsCpnSize);
	double dfFee0;
	...
	for (i = itsvCpnIndex[exerIdx]; i<itsCpnSize; i++)
	{
		...
		if (!IsFixCoupon(i, notUsed))
		{
			...
			std::vector<double>& longPayTimes    = soVanillaArg->GetSwapLongFixPayTimes()[0];
			std::vector<double>& longPayPeriods  = soVanillaArg->GetSwapLongFixPayPeriods()[0];
			...
			std::vector<double>& shortPayTimes    = soVanillaArg->GetSwapShortFixPayTimes()[0];
			std::vector<double>& shortPayPeriods  = soVanillaArg->GetSwapShortFixPayPeriods()[0];
	...
	std::vector<double> fundingLeverage (0) ;

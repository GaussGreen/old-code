...
ARM_CRASpreadCalculator::ARM_CRASpreadCalculator( 
		...
		const std::deque<bool>& productsToPrice, // FIXMEFRED: mig.vc8 (23/05/2007 14:27:19): dont use std::vector<bool>. see item 18 in "Effective STL" from Scott Meyers
		const std::deque<bool>& localCalibFlags, // FIXMEFRED: mig.vc8 (23/05/2007 14:27:19): dont use std::vector<bool>. see item 18 in "Effective STL" from Scott Meyers
		const std::vector<double>& optimResetData,
												 ...
												 const std::deque<bool> /* FIXMEFRED: mig.vc8 (23/05/2007 14:27:19): dont use std::vector<bool>. see item 18 in "Effective STL" from Scott Meyers*/& localCalibFlags)
{
	...
	std::vector<double> breakPointTimesExt(2);breakPointTimesExt[0]=0;breakPointTimesExt[1]=1;
	std::vector<double> valuesExt(2);valuesExt[0]=correl;valuesExt[1]=correl;
...
ARM_DateStripCombiner ARM_CRASpreadCalculator::DatesStructure() const
{
    ...
    else if(!itsExerDateStrip.IsNull())
    {
        SchedVect[CALL_CRASPREAD_SCHED] = &(*itsExerDateStrip);

	    itsExerDateStripUnadj = ARM_DateStripPtr(static_cast<ARM_DateStrip*>(itsExerDateStrip->Clone()));
    }
	else
	{
		ARM_THROW( ERR_INVALID_ARGUMENT, ARM_USERNAME+" => execise date strip is missing in DatesStructure()");
	}
	...
	std::vector<double>& fundResetDates = GetFundDateStrip()->GetResetDates() ;
	std::vector<double>& fundStartDates = GetFundDateStrip()->GetFlowStartDates();
	std::vector<double>& exerciseDates = GetExerDateStrip()->GetResetDates() ;
	std::vector<double>& startDates = GetExerDateStrip()->GetFlowStartDates();
...
ARM_StdPortfolioPtr ARM_CRASpreadCalculator::CreateSOPortfolio(bool isSOCond,bool isDouble,bool isLong)
{
	...
	std::vector<double>& resetDates = itsExerDateStrip->GetResetDates();
	std::vector<double>& startDates = itsExerDateStripUnadj->GetFlowStartDates();
	std::vector<double>& endDates = itsExerDateStripUnadj->GetFlowEndDates();
...
void ARM_CRASpreadCalculator::ComputeProductVectorsFromCurves()
{
	...
	std::vector<double>& cpnResetDates = GetStructDateStrip()->GetResetDates() ;
	...
	std::vector<double>& cpnStartDates  = GetStructDateStrip()->GetFlowStartDates();
	std::vector<double>& cpnPayDates  = GetStructDateStrip()->GetPaymentDates();
	double cpnEndDate = GetStructDateStrip()->GetFlowEndDates()->Elt(itsCpnSize-1);
	std::vector<double>& fundStartDates = GetFundDateStrip()->GetFlowStartDates();
	std::vector<double>& fundPayDates = GetFundDateStrip()->GetPaymentDates();
	...
	/// To avoid SpreadOption schedule synchronisation pb (the start date is
	/// not adjusted by default) that may appears if cpnStartDates comes
	/// from a StructDateStrip built with itsDateStrip3...!!!!!
	std::vector<double>& startDatesForSynchro = GetExerDateStripUnadj()->GetFlowStartDates();
	i = 0;
	for (k=0; k<itsExerSize; k++)
	{
		while ((*startDatesForSynchro)[k]>(*cpnStartDates)[i]) 
			i++;
	...
	for(int i=0;i<pf->GetSize();++i)
	{
		...
		else if (itsCalibType == BASKET_CALIBRATION || itsCalibType == BASKET_CALIBRATION_SIMPLIFIED)
		{
			...
			if (itsModelType == ARM_PricingModelType::HWM1F)
			{
				...
				std::vector<double> defaultSigmas(1,SIGMA_DEFAULT_VALUE_FOR_VEGA);
				std::vector<double> defaultSigmasShifted(1,SIGMA_DEFAULT_VALUE_FOR_VEGA+SIGMA_SHIFT_FOR_VEGA);
				std::vector<double> defaultMeanRevs(1,mrs);
			...
			else
			{
				...
				std::vector<double> defaultSigmasShifted(1,SIGMA_DEFAULT_VALUE_FOR_VEGA+SIGMA_SHIFT_FOR_VEGA);
				std::vector<double> defaultMeanRevs(1,mrs);
				std::vector<double> defaultMRspread(1,mrs_spread);
				std::vector<double> defaultCorrel(1,correl);
				std::vector<double> defaultVolRatio(1,volratio);
...
/////////////////////////////////////////////////////////////////
void ARM_CRASpreadCalculator::SkipResetDates(const std::vector<double>& resets,const std::vector<double>& payments,
...
void ARM_CRASpreadCalculator::OptimiseResetDates(ARM_SpreadOption* sec,ARM_DateStripPtr& sched) const
{
	...
	std::vector<double> resets(sec ? To_ARM_GP_Vector(*(sec->GetSwapLeg()->GetResetDates())) : *(sched->GetResetDates()));
	size_t nbResets = resets.size();
	if(nbResets <= 1)
		return;
	...
	std::vector<double> payments(sec ?  To_ARM_GP_Vector(*(sec->GetSwapLeg()->GetPaymentDates())) : *(sched->GetPaymentDates()));
	size_t resetIdx = 0;
												 ...
												 ARM_SwapLeg* leg,ARM_DateStripPtr& sched) const
{
	...
	std::vector<double> newResets,newPayments,newFwdRateStarts,newFwdRateEnds;
	std::vector<double> newFlowStarts,newFlowEnds,newInterestDays,newInterestTerms;
...
{
	std::vector<double> resets(To_ARM_GP_Vector(*(sec->GetSwapLeg()->GetResetDates())));
	size_t nbResets = resets.size();
	if(nbResets <= 1)
		return;
...
void ARM_CRASpreadCalculator::CreateCSOPortfolio_FakeForDateStripOnly()
{
	...
	else
	{
		...
		std::vector<double> resetTimesForStruct(nbPeriods);
		std::vector<double> startTimesForStruct(nbPeriods);
		std::vector<double> endTimesForStruct(nbPeriods);
		std::vector<double> payTimesForStruct(nbPeriods);
		std::vector<double> payPeriodsForStruct(nbPeriods);

		std::vector<double> resetTimesForPay(nbPeriods);
		std::vector<double> startTimesForPay(nbPeriods);
		std::vector<double> endTimesForPay(nbPeriods);
		std::vector<double> payTimesForPay(nbPeriods);
		std::vector<double> payPeriodsForPay(nbPeriods);

		std::vector<double> resetTimesForFix(nbFlows);
		std::vector<double> startTimesForFix(nbFlows);
		std::vector<double> endTimesForFix(nbFlows);
		std::vector<double> fwdStartTimesForFix(nbFlows);
		std::vector<double> fwdEndTimesForFix(nbFlows);
		std::vector<double> payTimesForFix(nbFlows);
		std::vector<double> itForFix(nbFlows);
		std::vector<double> weightForFix(nbFlows);
...
void ARM_CRASpreadCalculator::CreateCSOPortfolio_FakeForDateStripOnly_ForVMS()
{
	...
	std::vector<double>& startDatesAdj = GetExerDateStrip()->GetFlowStartDates();
	std::vector<double>& resetDatesAdj = GetExerDateStrip()->GetResetDates();

	std::vector<double>& startDates = GetExerDateStripUnadj()->GetFlowStartDates();
	std::vector<double>& resetDates = GetExerDateStripUnadj()->GetResetDates();

	std::vector<double> resetTimesForStruct;
	std::vector<double> startTimesForStruct;
	std::vector<double> endTimesForStruct;
	std::vector<double> payTimesForStruct;
	std::vector<double> payPeriodsForStruct;

	std::vector<double> resetTimesForPay;
	std::vector<double> startTimesForPay;
	std::vector<double> endTimesForPay;
	std::vector<double> payTimesForPay;
	std::vector<double> payPeriodsForPay;

	std::vector<double> resetTimesForFix;
	std::vector<double> startTimesForFix;
	std::vector<double> endTimesForFix;
	std::vector<double> fwdStartTimesForFix;
	std::vector<double> fwdEndTimesForFix;
	std::vector<double> payTimesForFix;
	std::vector<double> itForFix;
	std::vector<double> weightForFix;
...
void ARM_CRASpreadCalculator::CreateEmptyCalibration()
{
	...
	else
	{
		...
		std::vector<double> calibTimes(1, 0.00);
...
void ARM_CRASpreadCalculator::SetDiagStrikeToFrontier(ARM_StdPortfolioPtr& swaptionPF)
{
	...
	if (oswBSModel)
	{
		...
		std::vector<double>& exerciseDates = itsExerDateStrip->GetResetDates() ;
...
void ARM_CRASpreadCalculator::CreateEmptyCalibrationFor2F()
{
	...
	std::vector<double> volatilityRatio(2, VOLRATIO_DEFAULT_VALUE);
	...
	std::vector<double> correl(2, CORREL_DEFAULT_VALUE);
...
void ARM_CRASpreadCalculator::ComputeCSONormalSensitivities()
{
	...
	std::vector<double>& payPeriods = soVanillaArg->GetPayPeriods();
...
ARM_Swaption* ARM_CRASpreadCalculator::CreateVarNotionalSwaptionAtExer (int exerIdx)
{
	...
	std::vector<double>& cpnCoverages = soVanillaArg->GetPayPeriods();
	std::vector<double>& cpnPaydates  = GetStructDateStrip()->GetPaymentDates();
	...
	// precomputation
	std::vector<double> A(itsCpnSize), Bshort(itsCpnSize), Blong (itsCpnSize), Bpay (itsCpnSize);
	...
	std::vector<double> df0(itsCpnSize), Slong0(itsCpnSize), Sshort0(itsCpnSize), Spay0(itsCpnSize);
	double dfFee0;
	...
	for (i = itsvCpnIndex[exerIdx]; i<itsCpnSize; i++)
	{
		...
		std::vector<double>& longPayTimes    = soVanillaArg->GetSwapLongFixPayTimes()[i];
		std::vector<double>& longPayPeriods  = soVanillaArg->GetSwapLongFixPayPeriods()[i];
		...
		std::vector<double>& shortPayTimes    = soVanillaArg->GetSwapShortFixPayTimes()[i];
		std::vector<double>& shortPayPeriods  = soVanillaArg->GetSwapShortFixPayPeriods()[i];
		...
		if (IsVariableCpn(i))
		{
			...
			if (i==0 || ( (i>0) && ((*periodIndexes)[i]!=(*periodIndexes)[i-1]) ) )
			{
				...
				std::vector<double>& payTimes    = soVanillaArg->GetSwapPayFixPayTimes()[periodIndex];
				std::vector<double>& payPeriods  = soVanillaArg->GetSwapPayFixPayPeriods()[periodIndex];
	...
	std::vector<double> fundingStarts (0) ;
	std::vector<double> fundingEnds	(0) ;
	std::vector<double> fundingPeriods(0) ;
	std::vector<double> fundingNotios (0) ;
	std::vector<double> fundingMargin (0) ;
...
ARM_Swaption* ARM_CRASpreadCalculator::ConvertToVarNotionalSwaption(ARM_ZeroCurve* curve, const ARM_VanillaSwaptionArg& oswVanillaArg)
{
	...
	const std::vector<double>& fixPayRates	= oswVanillaArg.GetStrikes();
	const std::vector<double>& fixPayPeriods	= oswVanillaArg.GetFixPayPeriods();
	const std::vector<double>& fixPayTimes	= oswVanillaArg.GetFixPayTimes();
	const std::vector<double>& fixNotionals	= oswVanillaArg.GetFixNotional();
	
	double fixLeg0=0.0;

	std::vector<double> A(nbFixFlows);
	...
	std::vector<double> df0(nbFixFlows);
...
/////////////////////////////////////////////////////////////////
///	Class  : ARM_CRASpreadCalculator
///	Routine: GetNextNoticeDate
///	Returns: ARM_Date
///	Action : returns first notice date
/////////////////////////////////////////////////////////////////
ARM_Date ARM_CRASpreadCalculator::GetNextNoticeDate() const
{
	for(size_t firstEventIdx(0); firstEventIdx<itsExerSize; firstEventIdx++)
	{
		if(itsvIsExerDate[firstEventIdx]) 
	...
	std::vector<double>& exerciseDates = GetExerDateStrip()->GetResetDates();
	return ARM_Date(exerciseDates->Elt(firstEventIdx));
}

/////////////////////////////////////////////////////////////////
///	Class  : ARM_CRASpreadCalculator
///	Routine: ComputeUnderlying
///	Returns: void
///	Action : returns underlying (market and model)
/////////////////////////////////////////////////////////////////
void ARM_CRASpreadCalculator::ComputeUnderlying(double& mktPrice,double& modelPrice) const
{
	modelPrice = itsFundingPrice - itsCorridorPrice;
	...
	for (size_t i = 0; i < itsCSOPF->GetSize(); ++i)
	{
		...
		if(itsCSOPF->GetAsset(i))
		{
			ARM_SpreadOption* spreadOption=static_cast< ARM_SpreadOption* >(itsCSOPF->GetAsset(i));
			spreadOption->SetModel(BSModel);
			underlyingPrice += spreadOption->ComputePrice();
		}
	}

	mktPrice = itsFundingPrice - underlyingPrice;
}

/////////////////////////////////////////////////////////////////
///	Class  : ARM_CRASpreadCalculator
///	Routine: GetFirstProba
///	Returns: double
///	Action : returns first exercise proba
/////////////////////////////////////////////////////////////////
double ARM_CRASpreadCalculator::GetFirstProba() const
{
	for(size_t firstEventIdx(0); firstEventIdx<itsExerSize; firstEventIdx++)
	{
		if(itsvIsExerDate[firstEventIdx]) 
	...
	if (itsIsExerciseProbas)
		return itsExerciseProbas[firstEventIdx];
	else
		ARM_THROW( ERR_INVALID_ARGUMENT, " ARM_CRASpreadCalculator:: proba not computed");
}

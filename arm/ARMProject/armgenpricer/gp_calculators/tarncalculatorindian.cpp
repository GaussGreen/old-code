...
ARM_DateStripCombiner ARM_TARNCalculatorIndian::DatesStructure() const
{
	...
	if ((firstEventIdx>0) && const_cast<ARM_TARNCalculatorIndian*>(this)->GetFixings() != NULL)
		firstEventIdx--;
...
//	bool isFirstEvent = ((itsCpnTiming == K_ARREARS)&&(itsFirstEventIdx==itsNbPastFixings)?(eventIdx == itsNbPastFixings+1):(eventIdx == itsFirstEventIdx));
	...
	if (isFirstEvent && itsNbPastFixings>0)
	{
		vResetDate = asOfDate+1;
		if (itsCpnIntRule == K_ADJUSTED)
			vResetDate = ARM_Date(vResetDate).GoodBusinessDay(K_FOLLOWING, (char*)itsCpnResetCal.c_str()).GetJulian();
	}
	...
	if (isFirstEvent && itsNbPastFixings>0)
	{
		vPayDate = ARM_Date(vResetDate).AddPeriodMult(K_DAILY, itsCpnResetGap, (char*)itsCpnResetCal.c_str()).GetJulian();
	}
	...
	if (itsIndianType == DownUp)
	{
		...
		paidCouponDesc	<< "(" << IndianColNamesTable[IndianColAlias::TwiceCallPut] << "[i]"
						...
						<< "*DF(" << YC_BASIS_KEY_NAME << itsCouponCcy.GetCcyName() << ","

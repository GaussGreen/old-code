...
:	ARM_GenCalculator( rhs ),
	...
	itsCompFreq(rhs.itsCompFreq),
	itsCompType(rhs.itsCompType),
...
:	
	...
	itsCompFreq(compFreq),
	itsCompType(compType),
...
void ARM_PRDCalculator::Init(const ARM_MarketData_ManagerRep& mktDataManager,
		...
		const std::vector<double>& calibDatas)
{
	...
	if(IsBasis()){
		...
		ARM_GramFctorArg cst(ARM_VectorPtr((std::vector<double>&)itsvFundSpread.Clone()));
...
:	
	...
	itsFundingLeg( CreateClone(powRev? powRev->GetItsRealFundLeg() : NULL)),
	...
	itsCompFreq(powRev->GetItsRealFundLeg()->GetDecompFreq()),
	itsCompType(powRev->GetItsRealFundLeg()->GetCompoundingType()),
	itsExerciseFreq(),
	itsNoticeGap(),
	itsPayRec(),
	itsFees(),
...
void ARM_PRDCalculator::ComputeDomesticBasis()
{
	...
	if(itsCompFreq != K_COMP_PROP || itsCompType != K_COMP_NONE) 
		fundingLeg = itsFundingLeg;

	std::vector<double> vdomMargin = basisConveter.ComputeDomMargin(fundingLeg);
	...
	std::vector<double> timeLags(itsCpnSize);
...
void ARM_PRDCalculator::ComputeProductVectorsFromCurves()
{
	...
	std::vector<double>& originalfundResetDates = itsFundDateStripFrom->GetResetDates() ;
	for (i=0; i<originalfundResetDates->size(); i++)
	{
		...
		if(IsBasis()){
			...
			itsvInitialFundNominal.push_back(fundLvge*itsFundnominalCv.Interpolate(lag));
		}
	}
	for (i=0; i<itsFundSize; i++)
	{
		...
		itsvFundNominal.push_back(fundLvge*itsCpnnominalCv.Interpolate(lag));
		itsvFundLeverage.push_back(fundLvge);
	}
	...
	std::vector<double>& exerciseDates = itsExerciseDateStrip->GetResetDates() ;
	if(!itsFees.empty())
	{
		double fee; 
	...
	std::vector<double>& cpnStartDates  = itsStructDateStrip->GetFlowStartDates() ;
...
void ARM_PRDCalculator::CreateDataFromUnderlying()
{
	...
	std::vector<double>& exerciseDates = OriginalFundingSched->GetResetDates() ;
	...
	std::vector<double> abscisses = itsDomesticCpnCv.GetAbscisses();
...
{
    std::vector<double> fxExpiryDates;
    std::vector<double> fxPaymentDates;
	 ARM_GP_VectorPtr vnominal(new std::vector<double>());
    ARM_GP_VectorPtr fxLowStrikes(new std::vector<double>());
    ARM_GP_VectorPtr fxHighStrikes(new std::vector<double>());
    ARM_GP_VectorPtr fxLeverages(new std::vector<double>());
    std::vector<double> fundingPaymentDates;

	for(size_t eventIdx = 0;eventIdx<itsCpnSize;++eventIdx)
    {
		/// Coupon pay date
		...
		/// forex reset date
		double fxResetDate = (*(itsForexDateStrip->GetResetDates()))[eventIdx];
        fxExpiryDates.push_back(  fxResetDate);
       
		/// low strike of call spread
		double lowStrike = itsvLowStrikeCpn[eventIdx];
        fxLowStrikes->push_back(lowStrike);

		/// high strike of call spread
		double highStrike = itsvHighStrikeCpn[eventIdx];
        fxHighStrikes->push_back(highStrike);
		
		/// levrage times interestperiod for callStrip Option
		double term=(*(itsStructDateStrip->GetInterestTerms()))[eventIdx];
		double leverage = itsvLeverage[eventIdx];
        fxLeverages->push_back(leverage*term);

		/// domestic nominal
		double nominal = itsvCpnNominal[eventIdx];
	...
	//ARM_GP_VectorPtr  vMarginptr ((std::vector<double>&)itsvFundSpread.Clone());
	ARM_GP_VectorPtr  vMarginptr (new std::vector<double>(itsvFundSpread));
	...
	std::vector<double> cpnFloorTimesNotio = itsvFixCpn * itsvCpnNominal;
	...
	{		
		...
		/// values
		values.push_back( ARM_GramFctorArg(ARM_VectorPtr(new std::vector<double>(cpnFloorTimesNotio.begin() + i,	 cpnFloorTimesNotio.end()))) );
	}
...
{	
    ...
    std::vector<double> noticeDates(nbFlows,0.0);

    int lastNoticeIdx=-1;
    int flowIdx=0;
    int firstNoticeIdx=-1;
	itsNbNoCall = 0;
    for(size_t noticeIdx=0;noticeIdx<nbFlows && (noticeDate = floor(BoosterDatas(noticeIdx,NOTICE_DATE))) > 0;++noticeIdx)
    {
	...
	std::vector<double> realNoticeDates;
    if(firstNoticeIdx > -1)
    {
	...
	std::vector<double>& margin = BoosterDatas.GetColumn(FUNDING_SPREAD);
	CC_NS(std,auto_ptr)<std::vector<double>> holdmargin(margin);
...
ARM_DateStripCombiner ARM_PRDCalculator::DatesStructure() const
{
	...
	size_t nbPastNoCall;
	std::vector<double>& exerciseDates = NULL;
	...
	if (itsExerciseDateStrip == ARM_DateStripPtr(NULL))
	{
		ARM_DateStrip ExerSched(itsStartDate,itsEndDate,itsExerciseFreq,itsCpnDaycount,resetCalendar,fwdRule,intRule,itsStubRule,
			-abs(itsNoticeGap),itsExerciseFreq, GETDEFAULTVALUE, payCalendar, resetTiming, payTiming);
	...
	if (itsStructDateStrip == ARM_DateStripPtr(NULL))
	{
		...
		if(abs(itsFxResetGap) > abs(itsNoticeGap) + K_NEW_DOUBLE_TOL)
		...
		ARM_DateStrip ForexSched(itsStartDate,itsEndDate,itsCpnFreq,itsCpnDaycount,resetCalendar,fwdRule,intRule,itsStubRule,
								-abs(itsFxResetGap) ,itsCpnFreq, GETDEFAULTVALUE, payCalendar, itsResetTiming, payTiming);
		...
		const_cast< ARM_PRDCalculator* >(this)->itsResetRedemptionDate.PreviousBusinessDay(abs(itsRedemptionGap),const_cast<char*> (resetCalendar));
	}
...
ARM_RowInfo ARM_PRDCalculator::MiddleRows( size_t eventIdx, const ARM_DateStripCombiner& datesStructure ) const
{
	...
	fxLowCallStripDesc << itsvCpnIndex[eventIdx]  << ")";
	...
	for(size_t idx=eventIdx;idx<eventSize;++idx)
	{
		if(itsvHighStrikeCpn[itsvCpnIndex[idx]] < MAX_FX_STRIKE)
		{
			...
			fxHighCallStripDesc << itsvCpnIndex[eventIdx] << ")";
...
void ARM_PRDCalculator::CreateAndSetModel()
{
    ...
    ARM_CurveMatrix* correlMatrixCurve;
	ARM_GP_Matrix* correlMatrix;
		
	if (!(correlMatrixCurve = dynamic_cast<ARM_CurveMatrix*>(GetMktDataManager()->GetData(GetKeys()[CorrelMatrixKey]))))
	{
    ...
    models[ARM_2IRFXModel::FxModel] = ARM_PricingModelPtr( ARM_EqFx_ModelFactory.Instance()->CreateModel( CreateClonedPtr(ycBasisDomCurve), 
									...
									(correlMatrixCurve?*correlMatrixCurve:*correlMatrix),
    ...
    ARM_PricingModelPtr hybridModel = ARM_PricingModelPtr( new ARM_2IRFXModel(modelMap,(correlMatrixCurve?*correlMatrixCurve:*correlMatrix) ) );
    ...
    ARM_TreeBase* tree = ARM_TreeFactory.Instance()->CreateTreeND(3,schedulerType,itsSchedulerDatas,
						samplerType,std::vector<double>(0),truncatorType,itsTruncatorDatas,false,reconnectorType,smootherType);
...
void ARM_PRDCalculator::CreateAndSetCalibration()
{
	...
	if(itsCalibType == ARM_PRCSCalibTypes::HybridBasketCalib)
	{
		...
		ARM_GP_VectorPtr hybridDatas( new std::vector<double>(ARM_MarketHybridModel::NbDatas,0.0) );
...
pair< ARM_StdPortfolioPtr, ARM_StdPortfolioPtr > ARM_PRDCalculator::CreateDiagonalSwaption()
{
    ...
    std::vector<double> domStdExp(nbExp+1);
    ...
    nbExp = oswBSVol->GetExpiryTerms()->GetSize();
    std::vector<double> forStdExp(nbExp+1);
			  ...
			  bool isInitVolParam)
{
    ...
    if(isInitVol)
    {
        ...
        std::vector<double> volLowerBound(nbOSW,isModelHW ? HWVOL_LOWER_BOUND : QVOL_LOWER_BOUND);
        std::vector<double> volUpperBound(nbOSW,isModelHW ? HWVOL_UPPER_BOUND : QVOL_UPPER_BOUND);
...
ARM_StdPortfolioPtr ARM_PRDCalculator::CreateFxOption()
{
	...
	std::vector<double>& exerResetDates = itsExerciseDateStrip->GetResetDates();
    if(itsCalibType == ARM_PRCSCalibTypes::ATMCalib)
    {
		...
		std::vector<double> fxStdExp(nbExp);
...
/// Restore models
	...
	std::vector<double> fundingStarts(nbFunds),fundingEnds(nbFunds),fundingPeriods(nbFunds);
	std::vector<double> fundingMargins(nbFunds),fundingNotios(nbFunds,1.0);
	...
	std::vector<double> fxSensis(nbEvents),zcSensis(nbEvents),fxPrices(nbEvents),cpnPrices(nbEvents);
	std::vector<double> fwdFxs(nbEvents),zcFxSets(nbEvents),zcCpnPays(nbEvents); /// for dump !
	...
	std::vector<double> *startDates,*endDates,*interestTerms;
	std::vector<double> dfs,swaps,stdSwapSensis,stdZcSensis,df0s(nbEvents);
...
void ARM_PRDCalculator::ComputeHybridBasketPrices()
{
    ...
    std::vector<double> initTimes(nbBaskets);
    std::vector<double> initVols(nbBaskets);
    ...
    std::vector<double> volLowerBound(nbBaskets,QVOL_LOWER_BOUND);
    std::vector<double> volUpperBound(nbBaskets,QVOL_UPPER_BOUND);
    ARM_CurveModelParam* vol = new ARM_CurveModelParam(ARM_ModelParamType::QVol,&initVols,&initTimes,
        "QVOL","STEPUPRIGHT",&volLowerBound,&volUpperBound);
    if(volParamSize==0)
														 ...
														 const vector< ARM_VanillaIrFxSwaption* >& hybridBaskets)
{
	...
	std::vector<double> domFloatZcCoefs,domFloatZcTimes,domFixZcCoefs,forZcCoefs;
	...
	for(idx=0;idx<nbBaskets;++idx)
	{
		...
		const std::vector<double>& fxNotionals = irFxSwaption->GetFxNominals();
		const std::vector<double>& fxSettlementTimes = irFxSwaption->GetFxSettlementTimes();
		const std::vector<double>& fxPayTimes = irFxSwaption->GetFxPayTimes();
		...
		const std::vector<double>& floatNotionals = *(irFxSwaption->GetIrSwaption().GetFloatNotional());
		const std::vector<double>& floatStartTimes = *(irFxSwaption->GetIrSwaption().GetFloatStartTimes());
		const std::vector<double>& floatEndTimes = *(irFxSwaption->GetIrSwaption().GetFloatEndTimes());
		const std::vector<double>& fixPayPeriods = *(irFxSwaption->GetIrSwaption().GetFixPayPeriods());
		const std::vector<double>& fixPayTimes = *(irFxSwaption->GetIrSwaption().GetFixPayTimes());
		const std::vector<double>& fixNotionals = *(irFxSwaption->GetIrSwaption().GetFixNotional());
		...
		hybridDatas = new std::vector<double>(ARM_MarketHybridModel::NbDatas);
...
void ARM_PRDCalculator::CalibrateLocalFxModel()
{
    if(itsFxLocalModelFlag)
    {
        ...
        std::vector<double> evalTimes(nbResets);
        std::vector<double> resetTimes(nbResets,0.0);
        ...
        if(itsFlooredFxOptionPF)
        {
            ...
            std::vector<double> noticeTimes(0);
            resetIdx=0;
...
/////////////////////////////////////////////////////////////////
///	Class  : ARM_PRDCalculator
///	Routine: CreateFundingLeg
///	Returns: 
///	Action : Create a Power Reverse Swap
/////////////////////////////////////////////////////////////////
void ARM_PRDCalculator::CreateFundingLeg()
{
	if(itsFundingLeg){
		itsFundingLeg->SetNxFlag(K_NX_NONE);
		return;
	}
	...
	/// funding spread  from Curve to refrence value
	double asOfDate = GetMktDataManager()->GetAsOfDate().GetJulian();
	ARM_ReferenceValue rspread(*CurveToRefValue(itsFundSpreadCv,asOfDate,100.0));
	itsFundingLeg->SetVariableSpread(&rspread);

	/// funding nominal from Curve to refrence value
	ARM_ReferenceValue rfundNotional(*CurveToRefValue(itsFundnominalCv,asOfDate));
	itsFundingLeg->SetAmount(&rfundNotional);

	itsFundingLeg->SetDecompFreq(itsCompFreq);
	itsFundingLeg->SetCompoundingType(itsCompType);
}
/////////////////////////////////////////////////////////////////
///	Class  : ARM_PRDCalculator
...
///	Action : Create a Power Reverse Swap
/////////////////////////////////////////////////////////////////
void ARM_PRDCalculator::CreateUnderlying(const ARM_Date& fixDate)
{
	///
	double asOfDate = GetMktDataManager()->GetAsOfDate().GetJulian();
	...
	ARM_Vector* resetDates = FxUnderTmpLeg.GetResetDates();	
	/// foreign coupon from Vector to refrence value
	std::vector<double>& fxResetDates = itsForexDateStrip->GetResetDates();
	ARM_ReferenceValue rForeignCpn(*CurveToRefValue(itsForeignCpnCv,asOfDate,100.0));
	(*rForeignCpn.GetDiscreteDates())-=dayLag;
	...
	*FxUnderLeg = FxUnderTmpLeg;
	...
	std::vector<double>& payDates = itsStructDateStrip->GetPaymentDates();

	/// domestic nominal by initial Fx to refrence value
	std::vector<double> cpnNominalbyInitialFx = itsvCpnNominal/itsInitialFx;
	...
	ARM_ReferenceValue rDomesticCpn(*CurveToRefValue(itsDomesticCpnCv,asOfDate,100.0));
	...
	*FxNumLeg = FxNumTmpLeg;
	...
	ARM_ReferenceValue rNominal(*CurveToRefValue(itsCpnnominalCv,asOfDate));
	FxNumLeg->SetAmount(&rNominal);
	
	/// initial forex from Vector to refrence value
	ARM_ReferenceValue rFx(*CurveToRefValue(itsInitialFxCv,asOfDate));

	resetDates = FxUnderLeg->GetResetDates();
	/// fix period managing by fixEndDate
	size_t nbFixFlows = 0;
	while(nbFixFlows < FxUnderLeg->GetFlowEndDates()->size() && 
		(*FxUnderLeg->GetFlowEndDates())[nbFixFlows] < fixDate.GetJulian()+ ARM_GlobalConstant::ARM_SEVENDAYS_LAG )
		nbFixFlows++;
	for(int i(0); i<nbFixFlows; ++i)
	{
		double fixRate = itsDomesticCpnCv.GetOrdinates()[i];
		itsMinCpnCv.GetOrdinates()[i] = fixRate;
		itsMaxCpnCv.GetOrdinates()[i] = fixRate;	
	}
	/// coupon min coupon from Vector to refrence value
	ARM_ReferenceValue rFloor(*CurveToRefValue(itsMinCpnCv,asOfDate,100.0));
	rFloor.SetDiscreteDates(resetDates);
	(*rFloor.GetDiscreteDates())-=dayLag;
	///  coupon max from Vector to refrence value
	ARM_ReferenceValue rCap(*CurveToRefValue(itsMaxCpnCv,asOfDate,100.0));
	rCap.SetDiscreteDates(rFloor.GetDiscreteDates());
	...
	/// funding nominal from Curve to refrence value
	ARM_Vector* vpayDates = fundinLeg.GetPaymentDates();
	ARM_ReferenceValue rfundNotional(*CurveToRefValue(itsFundnominalCv,asOfDate));
	ARM_ReferenceValue* pfundNotional = rfundNotional.CptReferenceValues(vpayDates);
	fundinLeg.SetAmount(pfundNotional);
	delete pfundNotional;

	fundinLeg.SetDecompFreq(itsCompFreq);
	fundinLeg.SetCompoundingType(itsCompType);
	
	std::vector<double>& exerResetDates = itsExerciseDateStrip->GetResetDates();	
	ARM_Vector* noticeDates = To_pARM_Vector(&std::vector<double>(exerResetDates->begin() + itsNbNoCall,exerResetDates->end()));
	ARM_AutoCleaner<ARM_Vector> HoldNotice(noticeDates );

	ARM_Vector* cancelDates = To_pARM_Vector(&std::vector<double>(itsStructDateStrip->GetFlowStartDates()->begin() + itsNbNoCall,
																itsStructDateStrip->GetFlowStartDates()->end()));
...
ARM_BS_Model* ARM_PRDCalculator::CreateConvAdjstModel()
{
	...
	std::vector<double> x1Vec(To_ARM_GP_Vector(*bsVol->GetExpiryTerms()));
	std::vector<double> x2Vec(To_ARM_GP_Vector(*(bsVol->GetStrikes())));
...
ARM_DFBSModel* ARM_PRDCalculator::CreateAnalyticalModel(mdmKeysAlias key,bool useMktDataMgrCorrels)
{
	...
	{
		ARM_CurveMatrix* correlMatrixCurve = dynamic_cast<ARM_CurveMatrix*>(GetMktDataManager()->GetData(GetKeys()[CorrelMatrixKey]));
		if (correlMatrixCurve)
			correlMatrix = correlMatrixCurve->Interpolate(0.0);
		else
			correlMatrix = (*dynamic_cast<ARM_GP_Matrix*>(GetMktDataManager()->GetData(GetKeys()[CorrelMatrixKey])));
	}
	else
	...
	ARM_ZeroCurve* basisFundCurve = dynamic_cast<ARM_ZeroCurve*>(GetMktDataManager()->GetData(GetKeys()[YcBasisFundKey]));
	ARM_Forex* fundforex = dynamic_cast<ARM_Forex*>(GetMktDataManager()->GetData(GetKeys()[FunForexKey]));
	double fundspoValue = fundforex->GetMarketPrice();
...
string ARM_PRDCalculator::HybridBasketDump()
{
		...
		(hybridBaskets = itsHybridBasketCalibMethod->GetPortfolio()) != ARM_StdPortfolioPtr(NULL) )
	{
		...
		for(i=0;i<nbBaskets;++i)
		{
			...
			std::vector<double> floatNotio(*(irFxSwaption->GetIrSwaption().GetFloatNotional()));
			std::vector<double> fixNotio(*(irFxSwaption->GetIrSwaption().GetFixNotional()));
			std::vector<double> floatEndTimes(*(irFxSwaption->GetIrSwaption().GetFloatEndTimes()));
			std::vector<double> fixPeriods(*(irFxSwaption->GetIrSwaption().GetFixPayPeriods()));
			zcCoef = - floatNotio[0];
...
ARM_VolLInterpol* ARM_PRDCalculator::ComputeATMFxOptionVols()
{
    ...
    if(!isATMCalib)
    {
        ...
        std::vector<double> prevStrike(nbFx);
        ARM_Option* fxOption;
        for(i=0;i<nbFx;++i)
        {
            fxOption = static_cast< ARM_Option* >(fxPortfolio->GetAsset(i));
    ...
    int callPut=K_CALL;
    std::vector<double> strikePerStates(1);
    ...
    std::vector<double> fxResetYfs(nbResets);
    ...
    std::vector<double> strikes(1,0.0);
    ...
    /// Convert std::vector<double> to ARM_Vector

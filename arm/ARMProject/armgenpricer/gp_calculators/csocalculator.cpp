...
//    SetModelKeys();
	...
	std::vector<double>& resetDates = itsCalibDateStrip->GetResetDates();
...
//	const ARM_DateStripCombiner& dateStructure = DatesStructure();
	...
	std::vector<double> defaultTimes(1,firstCalibTime);
...
/////////////////////////////////////////////////////////////////
void ARM_CSOCalculator::ComputeEquivalentSwoptStrikes(std::vector<double>& equivStrikes1, std::vector<double>& equivStrikes2)
{
	std::vector<double>& startDates = itsCalibDateStrip->GetFlowStartDates();
	std::vector<double>& resetDates = itsCalibDateStrip->GetResetDates();
...
void ARM_CSOCalculator::CreateCSOSwaptionCap()
{
	...
	std::vector<double> equivStrikes1(resetSize);
	std::vector<double> equivStrikes2(resetSize);
...
void ARM_CSOCalculator::computeSOPortfolioPrices()
{
	...
	/*std::vector<double> lowerBound(1,CORREL_LOWER_BOUND);
	std::vector<double> upperBound(1,CORREL_UPPER_BOUND);
...
void ARM_CSOCalculator::computeSigmaPortfolioPrices()
{
	...
	if (itsSigmaCalib == BootstrapCalib)
	{
		...
		std::vector<double> calibTimes(calibSize);
		std::vector<double> volatility(calibSize);
		std::vector<double> lowerVolatility(calibSize,SIGMA_LOWER_BOUND);
		std::vector<double> upperVolatility(calibSize,SIGMA_UPPER_BOUND);
	...
	else
	{
		...
		std::vector<double> breakpointTimes(1,lastCalibTime);
		std::vector<double> volatility(1,lastVol);
		...
		for (int i = size-2; i>=GetNbFixFlows(); i--)
		{
			...
			std::vector<double> breakpointTimes_i(1,calibTime_i);
			double vol_i = CFBSModel->GetVolatility()->ComputeVolatility(calibTime_i/K_YEAR_LEN,tenor)/100.0;
			std::vector<double> volatility_i(1,vol_i);
			std::vector<double> lowerVolatility_i(1,SIGMA_LOWER_BOUND);
			std::vector<double> upperVolatility_i(1,SIGMA_UPPER_BOUND);

...
{
	std::vector<double> values(1, mrsValue);
	std::vector<double> terms(1, 0.0);
...
ARM_DateStripCombiner ARM_CRFCalculator::DatesStructure() const
{
    ...
    if(itsExerGap==NOT_DEF_EXER_GAP)
    {
        ...
        std::vector<double>& fixResetDates = FixLegSched.GetResetDates();
    ...
    std::vector<double>& cpnResetDates = RFLegSched.GetResetDates();
...
{
    std::vector<double>& resetDates = schedule->GetResetDates();
    if ( !resetDates || !resetDates->size() )
        return; // nothing to do !
	
	ARM_Vector* tmpExerDates  = itsExerFee.GetDiscreteDates();
    std::vector<double>& exerDates = To_pARM_GP_Vector(tmpExerDates);
	CC_NS(std,auto_ptr)<std::vector<double>> exerDatesTmp(exerDates);

    if(exerDates->size()==0)
		ARM_THROW( ERR_INVALID_ARGUMENT, ARM_USERNAME+" : At least one exercise date is needed in the fees schedule" );

    std::vector<double> newResetDates;
    std::vector<double> nonCallResetDates;
    ...
    std::vector<double>& cpnResetDates = cpnSchedule->GetResetDates();
    std::vector<double> tmpResetDates;
    ...
    if(nonCallResetDates.size()>0)
    {
        ...
        std::vector<double> newDates(newExerFee.size());
        std::vector<double>& newValues = new std::vector<double>(newExerFee.size());
        for(i=0;it!=end;++it,++i)
        {
            newDates[i]=it->first;
...
void ARM_CRFCalculator::ReplaceExerDatesWithExerGap(ARM_DateStrip* schedule) const
{
	...
	ARM_Vector* tmpExerDates  = itsExerFee.GetDiscreteDates();
    std::vector<double>& exerDates = To_pARM_GP_Vector(tmpExerDates);
	CC_NS(std,auto_ptr)<std::vector<double>> exerDatesPtr(exerDates);
	...
	int exerGap;
    std::vector<double>& startDates = schedule->GetFlowStartDates();
...
void ARM_CRFCalculator::PreCalibrateSkew()
{
	...
	std::vector<double> SkewTimes(sizepf),
		 VolTimes(sizepf),	
		 SkewValues(sizepf),
		 VolValues(sizepf),
		 weights(sizepf);
	...
	std::vector<double> precisions(sizepf,SKEW_CALIB_PRECISION);
	std::vector<double> LBoundVector(sizepf,SKEW_LOWER_BOUND);
	std::vector<double> UBoundVector(sizepf,SKEW_UPPER_BOUND);
	...
	std::vector<double>& Skews = qgmSet->get_X();

	//debug
	double objec = qgmSet->get_objective();
	
	///Update Model and CalibMethod
	std::vector<double> LBoundVol(sizepf,SIGMA_MIN_INIT_VALUE);
	std::vector<double> UBoundVol(sizepf,SIGMA_MAX_INIT_VALUE);
...
ARM_StdPortfolio* ARM_CRFCalculator::CreateDiagFWDVanilla(const ARM_StdPortfolio& portfolio)
{
	...
	std::vector<double>& prices = To_pARM_GP_Vector(vanillaPF->GetMktPrices());
	CC_NS(std,auto_ptr)<std::vector<double>> pricesPtr(prices);
...
void ARM_CRFCalculator::ComputeDiagonalSwaptionPrice(bool isFreezeWeights, bool isInitParam, bool isUpdateStrike)
{
    ...
    if(isUpdateStrike)
    {
	    ...
	    std::vector<double> volTimes;
	    std::vector<double> volValues;
    ...
    std::vector<double> initSigmas(nbOSW);
    ARM_Swaption* swaption;
	...
	if(isInitSigma)
    {
		...
		if(removedOSWIdx.size()>0)
		{
			...
			std::vector<double> newInitTimes(newNbOSW),newInitSigmas(newNbOSW);
			ARM_IntVector newKeepOSWIdx(newNbOSW);
			std::vector<double> newStrike(newNbOSW);
        ...
        std::vector<double> sigmaLowerBound(nbOSW,SIGMA_LOWER_BOUND);
        std::vector<double> sigmaUpperBound(nbOSW,SIGMA_UPPER_BOUND);
...
ARM_CurveModelParam* ARM_CRFCalculator::CheckPortfolioOSW( ARM_IntVector& removedOSWIdx )
{
	...
	ARM_StdPortfolioPtr swaptionPF = GetOSWPortfolio();
	std::vector<double> RepairedSigma,times;
...
void ARM_CRFCalculator::ComputeShortTermVanillaPrice(bool isFreezeWeights, bool isInitParam,bool isUpdateStrike)
{
    ...
    if(isInitMRS && nbMRS > 1)
        nbMRS=nbVanilla;
    std::vector<double> initTimes(nbMRS,0.0);
    std::vector<double> initValues(nbMRS,MRS_DEFAULT_VALUE);
    ...
    if(isInitMRS)
    {
        ...
        std::vector<double> mrsLowerBound;
        std::vector<double> mrsUpperBound;

		mrsLowerBound = std::vector<double>(nbMRS,MRS_LOWER_BOUND);
		mrsUpperBound = std::vector<double>(nbMRS,MRS_UPPER_BOUND);
...
void ARM_CRFCalculator::ComputeDiagFWDVanillaPrice(bool isFreezeWeights, bool isInitParam)
{
	...
	std::vector<double> lowerBound(1,MRS_LOWER_BOUND);
	std::vector<double>  upperBound(1,MRS_UPPER_BOUND);
	mrsParam->SetLowerBound((std::vector<double>&)lowerBound.Clone());
	mrsParam->SetUpperBound((std::vector<double>&)upperBound.Clone());

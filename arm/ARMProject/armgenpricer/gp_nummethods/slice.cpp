...
double Slice1DFunct::operator() (double x) const
{
    ...
    ARM_VectorPtr localDf = itsModel->LocalDiscounts(itsTimeIdx,itsDt,itsStates);
    double fx=0.0;
    for(i=0;i<nbStates;++i)
...
ARM_GP_VectorPtr Slice1DFunct::GetLocalDf(double x) const
{
    size_t i,nbStates = itsXStates->size();
    for(i=0;i<nbStates;++i)
        itsStates->SetNumMethodState(i,0,(*itsXStates)[i]+x);
	itsModel->TreeStatesToModelStates(itsStates,itsTimeIdx);
    ARM_VectorPtr localDf = itsModel->LocalDiscounts(itsTimeIdx,itsDt,itsStates);
    return ARM_GP_VectorPtr(new ARM_GP_Vector(*localDf));
}
...
double SliceNDFunct::operator() (double x) const
{
    ...
    ARM_VectorPtr localPayoff = itsModel->LocalPayoffs(itsTimeIdx,itsDt,itsStates);
...
ARM_GP_VectorPtr SliceNDFunct::GetLocalDf(double x) const
{
    ...
    ARM_VectorPtr localPayoff = itsModel->LocalDiscounts(itsTimeIdx,itsDt,itsStates);
    return ARM_GP_VectorPtr( new ARM_GP_Vector(*localPayoff));
}
...
double ARM_SliceNDBase::ComputeExpectation( size_t stateIdx, const ARM_SliceBase* nextSlice, const PayoffFunc& payoffFunc, ARM_TransitionStates& transStates ) const
{
    ...
    const std::vector<double>& transProbas = transStates.GetProbas();
    double payoff = 0;
...
void ARM_SliceND::LinkedWithNextSlice( ARM_SliceBase* nextSlice, ARM_SamplerBase* sampler, ARM_TruncatorBase* truncator, bool needProbas, ARM_TransitionStates& transStates, const ARM_GP_VectorPtr& driftCorrection, size_t driftOffset, double targetPayoff)
{
    ...
    SetXStates(xStates);
    const std::vector<double> var    = samplerND->GetLocalVar( timeIdx );

    std::vector<double> spaceStepSquare(nextSliceND->GetSpaceStep());
    for( i=0; i<nbDims; ++i )
    ...
    double meanError;
    std::vector<double> meanRel(nbDims),varRel(nbDims);
...
if(needProbas)
{
    ...
    while(isPrintable)
    {
        ...
        std::vector<double> treeMeanZ(nbDims,0.0),treeVarZ(nbDims,0.0);
        std::vector<double> treeMeanX(nbDims,0.0),treeVarX(nbDims,0.0);
        std::vector<double> treeSkewX(nbDims,0.0),treeKurtX(nbDims,0.0);
        ...
        ARM_MatrixVector localVariances,variances;
        model->NumMethodStateLocalGlobalVariances(*(model->GetNumMethod()->GetTimeSteps()),localVariances,variances);
        std::vector<double> theoVar(nbDims),theoSkew(nbDims,0.0),theoKurt(nbDims);
        std::vector<double> errRelVar(nbDims,0.0),errRelKurt(nbDims,0.0);

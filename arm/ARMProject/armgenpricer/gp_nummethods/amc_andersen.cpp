...
double ARM_AndersenCalculate::operator () ( double x ) const
{
	...
	for ( i = 0 ; i < size ; i ++ )
		result += ( (*itsTrigValue)[i] >= x ) ? (*itsPayoff)[i] : (*itsContOpt)[i];
...
pair<double,bool> ARM_AMCAndersen::ComputeStdExerciseBoundary( const ARM_VectorPtr& payoff, const ARM_VectorPtr& contOpt, const ARM_VectorPtr& trigValue )
{
	...
	maxopt = (*(trigValue.IsNull()?payoff:trigValue))[0];
	minopt = maxopt;
	size = (trigValue.IsNull()?payoff:trigValue)->size();

	for ( i = 0 ; i < size ; i ++ )
	{
		double &x = (*(trigValue.IsNull()?payoff:trigValue))[i];
		maxopt = MAX( maxopt, x);
		minopt = MIN( minopt, x );
	}
...
{	
	double	maxopt = (*(trigValue.IsNull()?payoff:trigValue))[0],
			minopt = maxopt,
			sum	   = 0.0,
			sumCont= 0.0;
	...
	else
	{
		...
		ARM_GP_VectorPtr trigValueSorted(new ARM_GP_Vector(*(trigValue.IsNull()?payoff:trigValue)));
		ARM_GP_VectorPtr payoffSorted(new ARM_GP_Vector(*payoff));
		ARM_GP_VectorPtr contOptSorted(new ARM_GP_Vector(*contOpt));
		...
		if ((maxIndex >= nbPoints) && (maxIndex <= size-nbPoints-1))
		{
			...
			std::vector<double>* X = LeastSquareRegression( Mat, Y );

...
double ARM_ConstantVarianceMeanRevertingScheduler::YtoX(std::vector<double>& X,std::vector<double>& Y,double y,int& lastIdx,int nextIdx) const
{
...
ARM_GP_VectorPtr ARM_ConstantVarianceMeanRevertingScheduler::ComputeTimeSteps(const ARM_PricingModel& model) const
{
	...
	if(varIncrSum < itsMinStdDev*itsMinStdDev*modelSchedule->Elt(nbModelSteps)/K_YEAR_LEN)
    {
...
{
    const ARM_GP_Vector* const modelSchedule = model.GetNumMethod()->GetTimeSteps();
    size_t nbModelSteps= modelSchedule->size()-1; /// Actual model steps
    int nbSteps     = model.GetNumMethod()->GetNbSteps();
	...
	model.NumMethodStateLocalGlobalVariances(modelSchedule->GetValues(),localVCV,globalVCV);
        ...
        if(nextModelTime - timeErr <= time)
        {
            ...
            if(nextModelIdx < nbModelSteps)
            {
                if(time <= nextModelTime + timeErr)
                {
                    ...
                    var = nextVar + varStep;
					time=model.VarianceToTime(var,time,modelSchedule->Elt(nextModelIdx+1));
                ...
                nextModelIdx++;
				nextModelTime=modelSchedule->Elt(nextModelIdx);
	...
	for (;nextModelIdx < nbModelSteps+1;++nextModelIdx,++idx)
		timeSteps[idx]=modelSchedule->Elt(nextModelIdx);
    ...
    for(idx=0;idx<finalTimeSteps->size();++idx)
		finalTimeSteps->Elt(idx)=timeSteps[idx];
...
ARM_GP_VectorPtr ARM_TimeStepPerYearScheduler::ComputeTimeSteps(const ARM_PricingModel& model) const
{
	if( itsTimeStepPerYear >= 0 )
	{
		...
		ARM_GP_VectorPtr pNewTimeSteps( static_cast<ARM_GP_Vector*>(timeInserter.AddTimes(modelSchedule).Clone()) );

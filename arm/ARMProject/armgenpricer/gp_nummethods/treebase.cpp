...
////////////////////////////////////////////////////
ARM_PricingStatesPtr ARM_TreeBase::Init( ARM_PricingModel& model, double firstInductTime, const ARM_VectorPtrVector& prevDriftCorrections, size_t driftOffset, const std::vector<double>& targetLocalPayoffs, ARM_GP_VectorPtr& timeSteps)
{
...
////////////////////////////////////////////////////
ARM_GP_MatrixPtr ARM_TreeBase::GetSpotProbabilities(const std::vector<double>& eventTimes) const
{
...
ARM_PricingStatesPtr ARM_TreeBase::Induct( const ARM_PricingModel& model, ARM_PricingStatesPtr& states,  double toTime)
{
    ...
    for(;timeIdx>=endTimeIdx;--timeIdx)
    {
        ...
        if(model.NeedLocalDiscount())
        {
			...
			ARM_VectorPtr localDiscounts = model.LocalDiscounts(timeIdx,nextTime-time,curPricingStates);
...
////////////////////////////////////////////////////
void ARM_TreeBase::ExerciseSmoothing1D(const std::vector<double>& exerFct, double coef, std::vector<double>& smoothValues, std::vector<double>& exerStates) const
{

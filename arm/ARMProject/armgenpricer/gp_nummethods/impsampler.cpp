	...
	ARM_SamplerBase* sampler)
{
	...
	std::vector<double> meanRevCoeff(factorNb,1.0);
	
	size_t timeIdx, factorIdx;

	std::vector<double> alpha(factorNb,0.0);
	...
	for(timeIdx = 0; timeIdx < nbTimeSteps-1; ++timeIdx)
	{
		...
		if (alpha.size() < factorNb)
			alpha = std::vector<double>(factorNb,0.0);
...
void ARM_PropImpSampler::BootstrapAlpha(
	int factorNb,
	const std::vector<double>& timeSteps,
	const std::vector<double>& rowSteps,
	const ARM_GP_T_Vector<std::vector<double>>& globalAlpha,
	ARM_SamplerBase* sampler)
{
	ARM_SamplerNDBase* samplerND = sampler->ToSamplerNDBase();
	...
	std::vector<double> lastDrift(factorNb,0.0);
	std::vector<double> lastDriftWithAlpha(factorNb,0.0);
	std::vector<double> incDrift(factorNb,0.0);
	...
	ARM_SamplerBase* sampler)
{
	...
	std::vector<double> alpha(factorNb,0.0);
	std::vector<double> meanRevCoeff(factorNb,1.0);
	std::vector<double> expMat(nbStates,1.0);
	...
	for(timeIdx = 0; timeIdx < nbTimeSteps-1; ++timeIdx)
	{
		...
		itsExpProcessStates.insert(make_pair(timeSteps[timeIdx+1], ARM_GP_VectorPtr(new ARM_GP_Vector(expMat))));
	}
}

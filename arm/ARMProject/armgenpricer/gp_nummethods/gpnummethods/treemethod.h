...
private:
    ...
    double YtoX(std::vector<double>& X,std::vector<double>& Y,double y,int& lastIdx,int nextIdx);
...
public:
	...
	const std::vector<double>& GetProbaSpotForward(int i) {return itsProbaSpotForward[i]->GetValues();}
	...
	{return ARM_GP_VectorPtr( timeIdx > 0 ? static_cast<ARM_GP_Vector*>(itsProbaSpotForward[timeIdx-1]->Clone()) : NULL );}
	...
	void TreeConstruction(const ARM_GP_Vector& timeSteps, ARM_VectorVector& probaSoptFwd, 

...
protected:
    void ExerciseSmoothing1D(const std::vector<double>& exerFct, double coef, std::vector<double>& smoothValues, std::vector<double>& exerStates) const;
...
public:
    ...
    { return Init(model,firstInductTime,ARM_VectorPtrVector(0),0,std::vector<double>(0),ARM_GP_VectorPtr(NULL)); }

    ARM_PricingStatesPtr Init( ARM_PricingModel& model, double firstInductTime, const ARM_VectorPtrVector& prevDriftCorrections, size_t driftOffset, const std::vector<double>& targetLocalPayoffs, ARM_GP_VectorPtr& timeSteps);

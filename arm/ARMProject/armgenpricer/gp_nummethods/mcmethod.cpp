...
size_t MaxBucketSize )
{
	...
	itsBuckets = ARM_VectorPtr( new std::vector<double>(bucketsNb) );

	for ( std::vector<double>::iterator iter = itsBuckets->begin() ; iter != itsBuckets->end() ; iter++ )
		*iter = EvenBucketSize;
...
ARM_PricingStatesPtr ARM_MCMethod::Init( ARM_PricingModel& model, double firstInductTime )
{
	...
	model.ModelStateLocalVariancesAndStdDev(GetTimeSteps()->GetValues());
	model.NumMethodStateLocalVariancesAndStdDev(GetTimeSteps()->GetValues());
...
//	std::vector<double> localStdDevs(dim);
...
////////////////////////////////////////////////////
ARM_GP_MatrixPtr ARM_MCMethod::GetSpotProbabilities(const std::vector<double>& eventTimes) const
{
...
ARM_GP_VectorPtr ARM_MCMethod::GetArrowDebreuPrices(size_t timeIdx, const ARM_PricingModel& model ) const
{
	...
	else
	{
		...
		double zcT =0;// ZcCurve->DiscountPrice((*GetTimeSteps())(timeIdx)/K_YEAR_LEN);

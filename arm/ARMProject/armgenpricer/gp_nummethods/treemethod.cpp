...
ARM_PricingStatesPtr ARM_TreeMethod::Init( ARM_PricingModel& model, double firstInductTime)
{
	...
	const ARM_GP_Vector& vec = *GetTimeSteps(); 
	TreeConstruction(vec, itsProbaSpotForward, itsTransitionStates, model.NeedArrowDebreuPrices() || itsIsProbabilityComputation);
...
void ARM_TreeMethod::ComputeMixTimeVarSchedule(const ARM_PricingModel& model)
{
	...
	model.NumMethodStateLocalVariances(modelSchedule.GetValues(),localVCV);

    std::vector<double> varIncr(localVCV.size());
    ...
    std::vector<double> timeSteps(nbSteps+nbModelSteps);
    timeSteps[0]=0.0;
    int idx=1;
    ...
    nbSteps=idx;
    std::vector<double> X(nbSteps),Y(nbSteps);
    Y[0]=0.0;
    for(j=0;j<nbSteps;++j)
        X[j]=timeSteps[j];
...
double ARM_TreeMethod::YtoX(std::vector<double>& X,std::vector<double>& Y,double y,int& lastIdx,int nextIdx)
{
...
void ARM_TreeMethod::ComputeCstVarSchedule(const ARM_PricingModel& model)
{
    ...
    std::vector<double> timeSteps(nbSteps+1); // +1 for [0]=0
    timeSteps[0]=0.0;
    int idx=1;
...
void ARM_TreeMethod::InitContinuousMapping( const ARM_PricingModel& model )
{
	...
	model.NumMethodStateLocalGlobalVariancesAndStdDev(GetTimeSteps()->GetValues(),tmpLocalVar,itsNumMethodStateGlobalVars,
...
void ARM_TreeMethod::ComputeMaxIndex()
{
    ...
    std::vector<double> nextExpectedState(nbDir);
...
void ARM_TreeMethod::ComputeBinomialTransition(int dirIdx,
                                               double driftErr,
                                               ARM_GP_Vector& proba,
                                               std::vector<double>& nextStateVariance) const
{
        ...
        int timeIdx) const
{
    ...
    std::vector<double> nextExpectedState(nbDir);
    std::vector<double> nextStateVariance(nbDir);
    ...
    std::vector<double> invSpaceii(nbDir);
...
double ARM_TreeMethod::ComputeStateProba(
        ...
        std::vector<double>& lastProbas) const
{
...
{      
    ...
    std::vector<double> payoff(nbPayoffs);
...
{      
	...
	std::vector<double> nextPayoffs(nbNextStates);
	
	std::vector<double> currPayoffs = payoffs;
    ...
    for(;curState<=maxState;++curState)
    {
        ...
        while(relState <= relMaxState)
        {
            ...
            if(stateProba > 0.0)
            {
               nextPayoffs[nextPosition] += stateProba  * currPayoffs[curPosition];
            }
            ++relState;
        }
...
///	Action : 
////////////////////////////////////////////////////
void ARM_TreeMethod::TreeConstruction(const ARM_GP_Vector& timeSteps, ARM_VectorVector& probaSoptFwd, 
	 ...
	 
{
	...
	{
		std::vector<double> payoffs(1,1.0);	
...
ARM_VectorPtr ARM_TreeMethod::GetBuckets() const
{
	...
	return static_cast<ARM_VectorPtr>(new std::vector<double>(1,1)); 
}
...
////////////////////////////////////////////////////
ARM_GP_MatrixPtr ARM_TreeMethod::GetSpotProbabilities(const std::vector<double>& eventTimes) const
{

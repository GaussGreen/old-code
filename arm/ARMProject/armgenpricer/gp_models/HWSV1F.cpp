...
///			 parameter inside)
////////////////////////////////////////////////////
int ARM_RiccatiHWSV1F::ComputeNbSteps(const std::vector<double>& solverParams)
{
...
////////////////////////////////////////////////////
void ARM_RiccatiHWSV1F::derivs(double t, std::vector<double>& yt, std::vector<double>& dyt) const
{
	++itsNbDerivCalls;
...
///	Action : Compute derivatives of the Riccati system
////////////////////////////////////////////////////
void ARM_RiccatiHWSV1F_SO::derivs(double t, std::vector<double>& yt, std::vector<double>& dyt) const
{
	++itsNbDerivCalls;
...
///	Action : Constructor
////////////////////////////////////////////////////
ARM_HWSV1F::ARM_HWSV1F(const ARM_ZeroCurvePtr& zc,const ARM_ModelParams* params,int solverType,const std::vector<double>& solverParams,
					   int formulaType,const std::vector<double>& formulaParams,
					   int formulaTypeSO,const std::vector<double>& formulaParamsSO,
...
void ARM_HWSV1F::MCModelStatesFromToNextTime(ARM_PricingStatesPtr& states,int timeIndex) const
{
	...
	const std::vector<double>& modelSchedule = GetModelSchedule();
...
////////////////////////////////////////////////////
void ARM_HWSV1F::ComputeRiccatiSchedule(double T, std::vector<double>& schedule, double Tref, bool isSOFormula, double Tstart) const
{
	schedule.empty();
	std::vector<double> modelSched = static_cast<const ARM_ModelParamsHWSV* const>(GetModelParams())->GetSchedule();
	if(modelSched[0] != 0.0)
	...
	if(maxDecay>0)
	{
		...
		std::vector<double> tmpSched(schedule.size()+additionalPoints.size());
		CC_NS(std,merge)(schedule.begin(),schedule.end(),additionalPoints.begin(),additionalPoints.end(),tmpSched.begin());
		std::vector<double>::iterator last=CC_NS(std,unique)(tmpSched.begin(),tmpSched.end());
...
void ARM_HWSV1F::InitAnalyticalData(double evalTime, double F0, double Te, double expT0,
									bool isSOFormula, std::vector<double>& schedule, double mu0) const
{
	double Tref = Te;
	if(schedule.size()==0)
...
////////////////////////////////////////////////////
void ARM_HWSV1F::UpdateUDependentSystemData(const std::vector<double>& ImU, bool isStdFormula, bool isSOFormula) const
{
...
void ARM_HWSV1F::GenerateFunction(double expTe,
								  ...
								  std::vector<double>& angularShifts) const
{
...
ARM_VectorPtr ARM_HWSV1F::ComputeCapletF0(	double evalTime,
									...
									std::vector<double>& newStrikes) const
{	
	...
	ARM_VectorPtr F0(new std::vector<double>(nbStates,F00));
...
for(size_t tIdx=0;tIdx<PRICING_NB_TIMES;++tIdx)
{
	...
	std::vector<double> newStrikes(nbStates);
	...
	if(nbStates==1 &&  newStrikes[0] <= 0.0)
	{
		if(capFloor==K_FLOOR)
		{
			values = ARM_VectorPtr(new std::vector<double>(1, 0.0));
		}
		else
		{
			ARM_VectorPtr zcStart = GetDiscountFunctor()->DiscountFactor(curveName,evalTime,fwdStartTime,newStates);
			values = ARM_VectorPtr(new std::vector<double>(1, ((*zcStart)[0]/newStrikes[0])*(newStrikes[0]-1)*payNotional*period/fwdPeriod));
		}
	}
	else
	{
								...
								std::vector<double>& newStrikes) const
{
	...
	std::vector<double> expMrs(nbPeriods);
	...
	ARM_VectorPtr F0(new std::vector<double>(nbStates,0.0));
...
for(size_t tIdx=0;tIdx<PRICING_NB_TIMES;++tIdx)
{
	...
	if(nbStates == 1 && newStrikes[0] <= 0.0)
	{
		...
		else
		{
			...
			values = ARM_VectorPtr(new std::vector<double>(1, ((*zcStart)[0]/newStrikes[0])*(newStrikes[0]-1)*refNotional));
		}
	}
	else
	{
...
/****
		...
		ARM_VectorPtr zcStart = GetDiscountFunctor()->DiscountFactor(curveName,evalTime,startTime,newStates);
		ARM_VectorPtr approxPrices(new std::vector<double>(nbStates,0.0));
		if(callPut==K_CALL)
		{
...
//	fclose(f);
    ...
    ARM_VectorPtr values(new std::vector<double>(1, (zcStart/newStrike)*expect*payNotional));
...
ARM_VectorPtr ARM_HWSV1F::ComputeRungeKuttaOptionPrice(	double evalTime,
														...
														double T0,
														double Te,
														const std::vector<double>& newStrikes,
														int	RecPay,
														double payNotional,
														const ARM_PricingStatesPtr& states) const
{
	...
	const std::vector<double>& numericals = itsNumericals.GetFormulaParams();
	...
	std::vector<double> ImU,PsiX1,PsiY1,PsiX0,PsiY0;
														...
														const ARM_PricingStatesPtr& states) const
{
	...
	std::vector<double> schedule;
	ComputeRiccatiSchedule(Te,schedule,Te,isSOFormula,evalTime);
	std::vector<double> times(schedule.size()-1);
	size_t i;
        ...
        const ARM_PricingStatesPtr& states) const
{
	...
	ARM_VectorPtr St = ARM_VectorPtr(new std::vector<double>(*firstRate));
...
****/
	...
	std::vector<double> volFactorStart(nbFixFlows),volFactorEnd(nbFixFlows);
	...
	ARM_VectorPtr ST = ARM_VectorPtr(new std::vector<double>(nbStates));

	std::vector<double> condShiftStart(nbStates);
	std::vector<double> condVarStart(nbStates);
	std::vector<double> condShiftEnd(nbStates);
	std::vector<double> condVarEnd(nbStates);
...
//fclose(f);
	...
	std::vector<double> totalUnif(2*nbTotalPaths);
	randGen.draw(totalUnif);

	std::vector<double> unif(2*nbStates);
												...
												ARM_VectorPtr& swapRate) const
{
	...
	ARM_VectorPtr F0(new std::vector<double>(2*nbStates,0.0));
...
for(size_t tIdx=0;tIdx<nbPrices;++tIdx)
{
	...
	ARM_VectorPtr longSwapRateValue(new std::vector<double>(0)),shortSwapRateValue(new std::vector<double>(0));
	...
	std::vector<double> F0(nbStates);
	std::vector<double> Mu0(nbStates);
	std::vector<double> stdDev(nbStates),newStrikes(nbStates),cvxFwdSpread(nbStates);
	...
	if(nbStates==1 && statusITM[0] == DEEP_OTM)
		return ARM_VectorPtr(new std::vector<double>(1,0.0));
													...
													const ARM_IntVector& statusITM) const
{
	...
	std::vector<double> numericals = itsNumericalsSO.GetFormulaParams();
	...
	vector<std::vector<double>> solverVars(RK5_NBTMP_VAR);
	size_t nbFcts = maxNbPts * fctMultiplier;
    size_t i;
	for(i=0;i<solverVars.size();++i)
		solverVars[i].reserve(nbFcts);

	std::vector<double> ImU,PsiX,PsiY,PsiTildeX,PsiTildeY;

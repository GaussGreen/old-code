...
////////////////////////////////////////////////////
size_t ARM_ModelParamsQ1F::GetModelCurves(std::vector<double>& times, std::vector<double>& sigmas, std::vector<double>& qs, std::vector<double>& dqs) const
{
    /// Get model curves
    const ARM_Curve& sigmaCurve = * ((ARM_CurveModelParam&) GetModelParam(GetVolatilityType())).GetCurve();
    const std::vector<double>& sigmaValues    = sigmaCurve.GetOrdinates();
    const std::vector<double>& sigmaTimes     = sigmaCurve.GetAbscisses();

    const ARM_Curve& qCurve = * ((ARM_CurveModelParam&) GetModelParam(ARM_ModelParamType::QParameter)).GetCurve();
    const std::vector<double>& qValues        = qCurve.GetOrdinates();
    const std::vector<double>& qTimes         = qCurve.GetAbscisses();
...
double ARM_ModelParamsQ1F::Q1FStateCovariance( const ARM_ModelParamsQ1F* lhs, const ARM_ModelParamsQ1F* rhs, double a, double b, double T, bool isrhsQ )
{
    ...
    /// 1st model curves
    std::vector<double> sigmas1,qs1,dqs1,times1;
    ...
    /// 2nd model curves
    std::vector<double> sigmas2,qs2,dqs2,times2;
    int na2,nb2;
    size_t nbTimes2;
    double MRSValue2;
    if(rhs != lhs)
...
double ARM_ModelParamsQ1F::Q1FStateZcCovariance( const ARM_ModelParamsQ1F* lhs, const ARM_ModelParamsHW1F* rhs, double a, double b, double T1, double T2 )
{
    ...
    /// 2nd model curves
    std::vector<double> sigmas2,times2;
    int na2,nb2;
    size_t nbTimes2;
    double MRSValue2;
    if(rhs != lhs)

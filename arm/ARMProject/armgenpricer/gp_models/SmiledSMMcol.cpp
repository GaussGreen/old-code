...
void ARM_SmiledSMMcol::MCModelStatesFromToNextTime_std(ARM_PricingStatesPtr& states,int timeIndex) const
{
	...
	std::vector<double> G(fwdsNb,0.);
	std::vector<double> D(fwdsNb,0.);
	std::vector<double> S(fwdsNb,0.);

	vector<std::vector<double>::iterator> rateIter(iLast-iFirst+1);
	vector<std::vector<double>::iterator> dxRateIter(iLast-iFirst+1);
	for (k = iFirst;k<=iLast;k++)
	{
...
// FIXMEFRED: mig.vc8 (30/05/2007 16:32:52):cast
	...
	if( DoesResetDateExist( evalTime ) )
	{
		...
		std::vector<double>::iterator iResult = result->begin();
		std::vector<double>::iterator iFwdStart = fwdStart->begin();
		std::vector<double>::iterator iFwdEnd = fwdEnd->begin();
		...
		if (maturityTime != itsStartDates[EndIdx])
		{
			...
			std::vector<double>::iterator iResult = result->begin();
			std::vector<double>::iterator iFwdEnd = fwdEnd->begin();
			std::vector<double>::iterator iFwdEndPlus = fwdEndPlus->begin();
...
ARM_VectorPtr ARM_SmiledSMMcol::ForwardDiscountFactorFromIdx_cache( const string& curveName, double evalTime, size_t Idx, const ARM_PricingStatesPtr& states) const
{
	size_t fwdsNb  = itsResetDates.size();
	size_t modelNb = GetModelNb();
	std::vector<double>& result = new std::vector<double>( states->size(),1.);
	
	if (Idx<fwdsNb)
	{
		std::vector<double>::iterator iResult;
...
ARM_VectorPtr ARM_SmiledSMMcol::ForwardDiscountFactorFromIdx_std( const string& curveName, double evalTime, size_t Idx, const ARM_PricingStatesPtr& states) const
{
	...
	std::vector<double>::iterator iRate;
	std::vector<double>::iterator iResult;
	std::vector<double>::iterator iInit;
	std::vector<double>::iterator iInitZC;
	...
	{
		result = new std::vector<double>( states->size(),itsTheta[itsLastRate[Idx]]);
	...
	{
		rate = itsProcess[Idx]->Rate(evalTime,states,modelNb,Idx);
		result = new std::vector<double>( states->size(),1.);
	...
	if (numericalModelFitter)
	{
		...
		if (size>0)
		{
			if (densityVector[0]->getInterestTerms().size()>1)
			{
				...
				std::vector<double> EndDates(nbFamily);

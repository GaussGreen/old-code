...
////////////////////////////////////////////////////
double ARM_QModel1F_Fx::ComputeForwardFxQ(double t, double T, const std::vector<double>& times, const std::vector<double>& sigmas, const std::vector<double>& qs, const std::vector<double>& dqs, double& qDrift)
{
...
/****
       ...
       /// Compute drift correction (only spot vol part is used in fwd vol)
        ...
        std::vector<double> times,sigmas,qs,dqs;
	...
	const ARM_PricingStatesPtr& states) const
{
	...
	return ARM_VectorPtr( new std::vector<double>( stateSize, zcT/zct ) );
}
...
{
    std::vector<double>& timeSteps = GetNumMethod()->GetTimeSteps();
        ...
        || states == ARM_PricingStatesPtr(NULL) || (evalTime<=K_NEW_DOUBLE_TOL))
	{
		...
		else
		{
			...
			if(isFwdAndInteg)
				...
				fwdValue = ARM_GP_VectorPtr( new std::vector<double>(stateSize,forwardValueMaturityTime) );
                                      ...
                                      double& fwdZcFwdFxCov) const
{
	...
	std::vector<double> correlTimes = curveCorrel.GetTimes();
        ...
        ARM_PricingContext* context) const
{
    ...
    else if(!isIVCalc)
    {
        ...
        const std::vector<double>& domTimes     = ((ARM_CurveModelParam&) domParams->GetModelParam(domParams->GetVolatilityType())).GetCurve()->GetAbscisses();

        const ARM_ModelParamsQ1F* forParams   = static_cast< const ARM_ModelParamsQ1F* >(forRefModel->GetModelParams());
        const std::vector<double>& forTimes     = ((ARM_CurveModelParam&) forParams->GetModelParam(forParams->GetVolatilityType())).GetCurve()->GetAbscisses();

        const std::vector<double>& fxTimes     = ((ARM_CurveModelParam&) fxParams->GetModelParam(fxParams->GetVolatilityType())).GetCurve()->GetAbscisses();

		const std::vector<double> correlTimes = GetCorrelMatrix().GetTimes();
    ...
    std::vector<double>& fxOption = new std::vector<double>(nbStates);
...
ARM_VectorPtr ARM_QModel1F_Fx::ComputeFwdZCModelParam(
		double t1,
		double t2,
	    double T1,
	    double T2,
...

{
	...
	double varDom = domModelParams->FwdZcLocalVariance(t1, t2, T1, T2);
	double varFgn = forModelParams->FwdZcLocalVariance(t1, t2, T1, T2);

	double volDom = sqrt(varDom / ((t2-t1)/K_YEAR_LEN));
	double volFgn = sqrt(varFgn / ((t2-t1)/K_YEAR_LEN));	

	std::vector<double>& fwdZCModelParam = new std::vector<double>(2);
	(*fwdZCModelParam)[0] = volDom; 
	(*fwdZCModelParam)[1] = volFgn;
    
    return ARM_VectorPtr(fwdZCModelParam);
}
		...
		ARM_PricingContext* context) const
{
	...
	std::vector<double>& fwdFXModelParam = new std::vector<double>(2);
...
////////////////////////////////////////////////////
void ARM_QModel1F_Fx::VolatilitiesAndCorrelations( const std::vector<double>& timeSteps, 
	...
	bool linearVol) const
{
	...
	std::vector<double> times = GetModelParams()->GetModelParam( ARM_ModelParamType::QVol).ToCurveModelParam().GetCurve()->GetAbscisses();
	std::vector<double> values= GetModelParams()->GetModelParam( ARM_ModelParamType::QVol).ToCurveModelParam().GetCurve()->GetOrdinates();
...
////////////////////////////////////////////////////
void ARM_QModel1F_Fx::EulerLocalDrifts( const std::vector<double>& timeSteps,
		ARM_GP_MatrixPtr& relativeDrifts,
		ARM_GP_MatrixPtr& absoluteDrifts) const
{

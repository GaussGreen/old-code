...
///	Action  : 
////////////////////////////////////////////////////
std::vector<double>& ARM_SmiledFRM::ComputeModelTimes(  const ARM_TimeInfoPtrVector& timeInfos )
{
...
ARM_VectorPtr ARM_SmiledFRM::ForwardDiscountFactorFromIdx( const string& curveName, double evalTime, size_t IdxFrom, size_t IdxTo , size_t modelNb, const ARM_PricingStatesPtr& states) const
{
	...
	for (size_t k = IdxFrom; k < IdxTo ; k++ )
	{
		if ( itsWeight(IdxFrom,k) == 1. )
		{
			...
			std::vector<double>::iterator iter2=dfratio->begin();

			for (std::vector<double>::iterator iter = result->begin() ; iter != result->end() ; ++iter, ++iter2)
				(*iter) /= (*iter2);
		}
	}
	return static_cast<ARM_VectorPtr>(result);
...
void ARM_SmiledFRM::MCModelStatesFromToNextTime(ARM_PricingStatesPtr& states,int timeIndex) const
{
	...
	for(k = 0 ; k < statesNb ; ++k )
	{
		...
		for (ARM_VectorPtrVector::reverse_iterator iterDx = dxLog.rbegin(); iterDx != dxLog.rend() ; ++iterDx,--i)
		{
			...
			for (std::vector<double>::iterator iter = eigen->begin(); iter != eigen->end() ; ++iter,++j )
			{
...
////////////////////////////////////////////////////
void ARM_SmiledFRM::NumMethodStateLocalVariances( const std::vector<double>& timeSteps,
...
////////////////////////////////////////////////////
void ARM_SmiledFRM::NumMethodStateGlobalVariances( const std::vector<double>& timeSteps,
...
////////////////////////////////////////////////////
void ARM_SmiledFRM::ModelStateLocalVariancesAndStdDev( const std::vector<double>& timeSteps)
{
...
///	Action  : computes the local variance and std dev 
////////////////////////////////////////////////////
void ARM_SmiledFRM::NumMethodStateLocalVariancesAndStdDev( const std::vector<double>& timeSteps )
{
...
void ARM_SmiledFRM::ModelStateLocalStdDev(const std::vector<double>& timeSteps, const ARM_MatrixVector& localVariances)
{
...
////////////////////////////////////////////////////
void ARM_SmiledFRM::ModelStateLocalVariances( const std::vector<double>& timeSteps,
		ARM_MatrixVector& localVariances ) const
{
...
double ARM_SmiledFRM::GetEquivalentShift(const std::vector<double>& weight) const
{
	double shift = 0;
	size_t k = 0;
...
ARM_VectorPtr ARM_SmiledFRM::ComputeDSwapDLib(	const std::vector<double>& startDates,const std::vector<double>& endDates,
...
ARM_VectorPtr ARM_SmiledFRM::ComputeDLogShiftSwapDLib(double shift, const std::vector<double>& startDates,const std::vector<double>& endDates,
					const std::vector<double>& fixPayTimes,const std::vector<double>& fixPayPeriods) const
{
	size_t	k,
			size		= startDates.size(),
...
ARM_PricingStatesPtr ARM_SmiledFRM::Init(const string& payModelName, const ARM_TimeInfoPtrVector& timeInfos)
{
	if (itsCalibrationStatus){
		...
		CC_NS(std,auto_ptr)<std::vector<double>> ptimeSteps(PricingTimeSteps(timeInfos));
...
////////////////////////////////////////////////////
std::vector<double>* ARM_SmiledFRM::PricingTimeSteps(const ARM_TimeInfoPtrVector& timeInfos)
{

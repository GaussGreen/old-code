...
{	
	...
	if (itsIsSwapRate )
	{
		...
		std::vector<double>& fixPayTimes   = pFixDateStrip->GetPaymentDates();
		std::vector<double>& fixPayPeriods = pFixDateStrip->GetInterestTerms();
		std::vector<double>& resetTimes    = pFixDateStrip->GetResetDates();
		std::vector<double>& startTimes	 = pFixDateStrip->GetFlowStartDates();

		size_t nbFixFlows = fixPayTimes->size();
		std::vector<double> rates(nbFixFlows, 0.0);
		std::vector<double> ratesDerivatives(nbFixFlows, 0.0);
		std::vector<double> dates(nbFixFlows, 0.0);
		...
		bool isConstantStrike) const 
{
	...
	return ARM_VectorPtr( new std::vector<double>(1,result));
	...
	const ARM_PricingStatesPtr& states) const
{
	...
	std::vector<double> fixNotional(1,payNotional);
	std::vector<double> floatNotional(1,payNotional);
	std::vector<double> floatResetTimes(1,fwdResetTime);
	std::vector<double> floatStartTimes(1,fwdStartTime);
	std::vector<double> floatEndTimes(1,fwdEndTime);
	std::vector<double> floatIntTerms(1,delta);
	std::vector<double> fixPayTimes(1,fwdEndTime);
	std::vector<double> fixPayPeriods(1,delta);
...
////////////////////////////////////////////////////
std::vector<double>& ARM_MarkovSV1F::ComputeFwdTerm_t(
					double evalTime,
					double F0,
					const ARM_PricingStatesPtr& states) const
{	
	size_t payoffSize = states != ARM_PricingStatesPtr(NULL) ? states->size(): 1;
	std::vector<double>& values = new std::vector<double>(payoffSize);
	double Ft,Xt,Yt;
	if (!itsIsSwapRate)
	{
		double evalTerm = evalTime/K_YEAR_LEN;
	...
	else // SwapRate 
	{
		...
		std::vector<double>& resetTimes    = pFixDateStrip->GetResetDates();
		std::vector<double>& startTimes	 = pFixDateStrip->GetFlowStartDates();
		std::vector<double> margin(startTimes->size(),0.0);
		...
		const ARM_PricingStatesPtr& states) const
{
	...
	double Beta = ((const ARM_ModelParamsMSV1F*) GetModelParams())->BetatT(evalTime,endFwdTime);	
	double var_3 = Volatility_t /DBeta;
	std::vector<double>& FtVector = ComputeFwdTerm_t(evalTime,F0,states);
...
void ARM_MarkovSV1F::EulerLocalDrifts( const std::vector<double>& timeSteps,
		ARM_GP_MatrixPtr& relativeDrifts,
		ARM_GP_MatrixPtr& absoluteDrifts) const
{
...
////////////////////////////////////////////////////
void ARM_MarkovSV1F::VolatilitiesAndCorrelations( const std::vector<double>& timeSteps, 
...
///	Action  :  
////////////////////////////////////////////////////
void ARM_MarkovSV1F::ModelStateLocalCorrels( const std::vector<double>& timeSteps,

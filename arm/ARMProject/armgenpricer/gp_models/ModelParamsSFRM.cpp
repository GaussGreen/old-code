...
{
    itsFwdValues = rhs.itsFwdValues ? (std::vector<double>&)rhs.itsFwdValues->Clone() : NULL;
...
double ARM_ModelParamsSFRM::IntegratedCovariance(double s, double t, double T1, double T2) const
{
    ...
    CC_NS(std,auto_ptr)<std::vector<double>> correlT1(InterpolateCorrelation(T1));
    CC_NS(std,auto_ptr)<std::vector<double>> correlT2(InterpolateCorrelation(T2));
...
double ARM_ModelParamsSFRM::IntegratedCovariance(double s, double t, double T1, double T2, double T3) const
{
    ...
    size_t i;
    CC_NS(std,auto_ptr)<std::vector<double>> correl1;
    CC_NS(std,auto_ptr)<std::vector<double>> correl2;
    ...
    if( (shortLibor = (T3 <= T1 + K_NEW_DOUBLE_TOL)) )
    {
        ...
        correl2 = CC_NS(std,auto_ptr)<std::vector<double>>(InterpolateCorrelation(T1));
        term2 = K_YEAR_LEN;
    }
    ...
    while(t1 < term1)
    {
        ...
        correl1 = CC_NS(std,auto_ptr)<std::vector<double>>(InterpolateCorrelation(u1));
        if(shortLibor)
        {
            cor = 0.0;
        ...
        else
        {
            while(t2 < term2)
            {
                ...
                correl2 = CC_NS(std,auto_ptr)<std::vector<double>>(InterpolateCorrelation(u2));
...
////////////////////////////////////////////////////
std::vector<double>& ARM_ModelParamsSFRM::InterpolateCorrelation(double T) const
{
    if(itsFactorsNb<=1)
        return new std::vector<double>(1,1.0);
    std::vector<double> correl			  = ((ARM_CorrelMatParam&)GetModelParam( itsCorrelType ) ).GetMultiCurve()->Interpolate(T);
    std::vector<double>& renormalizedcorrel = new std::vector<double>(itsFactorsNb);
	double sum = 0.;
    size_t i;
	for(i=0;i<itsFactorsNb;++i)
...
double ARM_ModelParamsSFRM::IntegratedCorrelation( double s, double t, double T1, double T2) const
{
    ...
    if(fabs(t-s)<K_NEW_DOUBLE_TOL)
    {
        ...
        CC_NS(std,auto_ptr)<std::vector<double>> instvol1(InstantaneousVolatility(0.0,T1));
        CC_NS(std,auto_ptr)<std::vector<double>> instvol2(InstantaneousVolatility(0.0,T2));
        double sum = 0.0;
        for(size_t i=0;i<itsFactorsNb;++i)
...
{
    double correl = 0.0;
    std::vector<double> correlT1 = ((ARM_CorrelMatParam&)GetModelParam( itsCorrelType )).GetMultiCurve()->Interpolate(T1);
    std::vector<double> correlT2 = ((ARM_CorrelMatParam&)GetModelParam( itsCorrelType )).GetMultiCurve()->Interpolate(T2);
...
{
    ARM_VectorPtr VolResult(new std::vector<double>(itsFactorsNb));
...
{
	ARM_VectorPtr result(new std::vector<double>(itsFactorsNb));
...
////////////////////////////////////////////////////
std::vector<double>& ARM_ModelParamsSFRM::InstantaneousVolatility(double t, double T) const
{
    double normeVol = VolatilityFunction(t,T);
    std::vector<double>& result = new std::vector<double>(itsFactorsNb);
    if(itsFactorsNb==1)
		(*result)[0]=normeVol;
    else
    {
        CC_NS(std,auto_ptr)<std::vector<double>> correl(InterpolateCorrelation(T));
		for(size_t i=0;i<itsFactorsNb;++i)
...
ARM_VectorPtr ARM_ModelParamsSFRM::FwdsVarCorrelCoeffs(double T_1, double T_2)
{
	...
	{
        CC_NS(std,auto_ptr)<std::vector<double>> correl_1(InterpolateCorrelation(T_1));
		CC_NS(std,auto_ptr)<std::vector<double>> correl_2(InterpolateCorrelation(T_2));
        for(size_t i=0;i<itsFactorsNb;++i)
				...
				bool useFixFrequency)
{
	...
	std::vector<double>& Mu = new std::vector<double>(nbVarFlow);
	double averageShift= AverageShift(*arg.GetFloatResetTimes());

	std::vector<double> DFFloatStarti(nbVarFlow+1);
	int i;

	//Determination of fix leg
	double DFix=0.0;
	double SwapNotional = 1;
	std::vector<double> FixFlow(nbFixFlow);
	std::vector<double> FixStarti(nbFixFlow+1);
	std::vector<double> DFFixStarti(nbFixFlow+1);
	...
	if(mode ==0)
	{
		...
		else
		{
			...
			std::vector<double> VarFlow(NbVarFlow);
			std::vector<double> VarStarti(NbVarFlow+1);
			std::vector<double> VarEndi(NbVarFlow+1);
...
// FIXMEFRED: mig.vc8 (25/05/2007 15:46:15):cast
		((ARM_VanillaSwaptionArgSFRM&)arg).SetSwapFwd(static_cast<ARM_VectorPtr>(new std::vector<double>(1,Sij)));
	...
	{	
		...
		std::vector<double> Fwdvectori(nbVarFlow);
		std::vector<double> DFFloatStartAtZeroi(nbVarFlow+1);
		...
		((ARM_VanillaSwaptionArgSFRM&)arg).SetSwapFwd(ARM_VectorPtr(new std::vector<double>(1,DFloatAtZero/(DFix+stubFix))));
	...
	else 
	{
		...
		((ARM_VanillaSwaptionArgSFRM&)arg).SetSwapFwd(ARM_VectorPtr(new std::vector<double>(1,Sij)));
		double Spj,DFpj;
		double Cst0= Sij/(Sij+averageShift);
		double Cst = Cst0/DFloat;		
												 ...
												 const ARM_VanillaSwaptionArg& argProba )
{
	...
	double DFix2=0.0;
	std::vector<double> FixFlow2(nbFixFlow2);
	std::vector<double> FixStarti2(nbFixFlow2+1);
	std::vector<double> DFFixStarti2(nbFixFlow2+1);
	...
	std::vector<double>& Mu = new std::vector<double>(nbVarFlow2);
	// calcul des coeff vol swap vol fra 
	DFix2 = TotalDFix2;
...
////////////////////////////////////////////////////
double ARM_ModelParamsSFRM::AverageShift(const std::vector<double>& times )
{
...
{
    std::vector<double> breakPointTimes(( (ARM_CurveModelParam&) GetModelParam(ARM_ModelParamType::Volatility)).GetCurve()->GetAbscisses());
    ...
    if(!itsFwdValues || fwdsSize != itsFwdValues->size())
    {
        ...
        delete itsFwdValues;
        itsFwdValues = new std::vector<double>(fwdsSize);
		...
		char* resetCalendar = pCcy->GetResetCalName( LIBOR3M);//GetDefaultIndexFromCurrency(pCcy->GetCcyName()) ); 
...
void ARM_ModelParamsSFRM::ConvertToBetaParam(const ARM_SFRM& model,const ARM_Portfolio& shiftConvPort)
{
    if( DoesModelParamExist(ARM_ModelParamType::Shift) )
	{
		...
		std::vector<double> breakPointTimes(( (ARM_CurveModelParam&) GetModelParam(ARM_ModelParamType::Volatility)).GetCurve()->GetAbscisses());
		size_t fwdsSize = breakPointTimes.size();
		std::vector<double> values(fwdsSize);
...
{
	std::vector<double> sigmaTimes(GetVolCurve()->GetAbscisses());

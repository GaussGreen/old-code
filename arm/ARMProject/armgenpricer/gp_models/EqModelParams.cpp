...
ARM_EQHWSV_ModelParams::ARM_EQHWSV_ModelParams( const ARM_ModelParamVector& params ): ARM_ModelParams(params ){
	...
	const std::vector<double>& schedAt	= GetModelParam( ARM_ModelParamType::MeanReversion).ToCurveModelParam().GetCurve()->GetAbscisses();
...
void ARM_EQHWSV_ModelParams::MergeSchedule(){
    ...
    const std::vector<double>& schedCt	= GetModelParam( ARM_ModelParamType::CompoundVol		).ToCurveModelParam().GetCurve()->GetAbscisses();
 	const std::vector<double>& schedDt	= GetModelParam( ARM_ModelParamType::VolOfVol			).ToCurveModelParam().GetCurve()->GetAbscisses();
 	const std::vector<double>& schedEt	= GetModelParam( ARM_ModelParamType::VolMeanReversion	).ToCurveModelParam().GetCurve()->GetAbscisses();
	const std::vector<double>& schedFt	= GetModelParam( ARM_ModelParamType::LongTermVol		).ToCurveModelParam().GetCurve()->GetAbscisses();
...
////////////////////////////////////////////////////
const	std::vector<double> & ARM_EQHWSV_ModelParams::CptSchedule( const double& t, const double & T) const{

	double tmin= t<T? t:T;
	double tmax= t<T? T:t;
	std::vector<double> res(1,tmin);

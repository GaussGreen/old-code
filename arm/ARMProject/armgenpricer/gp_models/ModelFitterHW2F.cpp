...
void ARM_ModelFitterHW2F::StoreDatas()
{
	...
	std::vector<double> breakPointTimes(nopt+1,0.);
...
void ARM_ModelFitterHW2F::SolveEquations()
{
	...
	size_t nopt = GetPortfolio()->size();
	const std::vector<double>& breakPointTimes = volParam->GetCurve()->GetAbscisses();
	...
	for (size_t index=0;index<nopt;index++)
	{
		...
		{
			std::vector<double>& localVarianceToPrec = hw2Params->Variance(0,breakPointTimes[index],scale);
			std::vector<double>& localVarianceToNext = hw2Params->Variance(0,breakPointTimes[index+1],scale);
			std::vector<double> localVariance(3);
			...
			std::vector<double> a(nbEq);
			std::vector<double> b(nbEq);
			std::vector<double> c(nbEq);

			std::vector<double> d(nbEq);
			bool ok=true;
...
void ccalib::solveOptim()
{
	...
	std::vector<double> hx(dim, 0.);
	std::vector<double> fguess(dim,0.);
...
void ccalib3eq::initOptim(std::vector<double>& fguess)
{
	int k=0;
	fguess[k++]=0.005;
	fguess[k++]=1.5;
...
void ccalib2eqVol::initOptim(std::vector<double>& fguess)
{
	int k=0;
	fguess[k++]=1.5;
	fguess[k++]=0.;
...
void ccalib2eqRho::initOptim(std::vector<double>& fguess)
{
	int k=0;
	fguess[k++]=0.005;
	fguess[k++]=1.5;
...
void ccalib2eqRatio::initOptim(std::vector<double>& fguess)
{
	int k=0;
	fguess[k++]=0.005;
	fguess[k++]=0;
...
void ccalib1eqVol::initOptim(std::vector<double>& fguess)
{
	int k=0;
	fguess[k++]=0.005;
}
...
void ccalib1eqRho::initOptim(std::vector<double>& fguess)
{
	int k=0;
	fguess[k++]=0.;
}
...
void ccalib1eqRatio::initOptim(std::vector<double>& fguess)
{
	int k=0;
	fguess[k++]=1.5;
}

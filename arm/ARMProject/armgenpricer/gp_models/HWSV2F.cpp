...
////////////////////////////////////////////////////
void ARM_RiccatiHWSV2F::derivs(double t, std::vector<double>& yt, std::vector<double>& dyt) const
{
	++itsNbDerivCalls;
...
///	Action : Compute derivatives of the Riccati system
////////////////////////////////////////////////////
void ARM_RiccatiHWSV2F_SO::derivs(double t, std::vector<double>& yt, std::vector<double>& dyt) const
{
	++itsNbDerivCalls;
...
///	Action : Constructor
////////////////////////////////////////////////////
ARM_HWSV2F::ARM_HWSV2F(const ARM_ZeroCurvePtr& zc,const ARM_ModelParams* params,const std::vector<double>& solverParams,
    ...
    ARM_MatrixVector& localVariances ) const
{
	...
	std::vector<double> correlValues;
...
void ARM_HWSV2F::MCModelStatesFromToNextTime(ARM_PricingStatesPtr& states,int timeIndex) const
{
	...
	/// Correlations
	std::vector<double> correls(static_cast<const ARM_CorrelMatParam&>(GetModelParams()->GetModelParam( ARM_ModelParamType::Correlation)).GetMultiCurve()->Interpolate(time));
...
////////////////////////////////////////////////////
void ARM_HWSV2F::ComputeRiccatiSchedule(double T, std::vector<double>& schedule, double Tref, bool isSOFormula, double Tstart) const
{
	schedule.empty();
...
void ARM_HWSV2F::InitAnalyticalData(double evalTime,const ARM_VectorPtr& F0, double Te, double exp1T0, double exp2T0,
									bool isSOFormula, std::vector<double>& schedule) const
{
	double Tref = Te;
	if(schedule.size()==0)
...
////////////////////////////////////////////////////
void ARM_HWSV2F::UpdateUDependentSystemData(const std::vector<double>& ImU, bool isStdFormula, bool isSOFormula) const
{
...
void ARM_HWSV2F::UpdateStdHWModel(double volFactor) const
{
	...
	std::vector<double> correlTimes(correls->GetAbscisses());
	size_t nbTimes = correlTimes.size();
	std::vector<double> correlValues(nbTimes);
	for(i=0;i<nbTimes;++i)
	...
	const ARM_PricingStatesPtr& states) const
{
	...
	size_t nbStates		= states->size();
	std::vector<double>& result	= new std::vector<double>(nbStates,0.0);
	size_t modelIdx			= GetModelNb();
...
{	
	...
	ARM_VectorPtr F0(new std::vector<double>(NB_X_VALUES*nbStates,0.0));
...
/****/
	...
	std::vector<double> expMrs1(nbPeriods),expMrs2(nbPeriods);
														...
														const ARM_PricingStatesPtr& states) const
{
	...
	ARM_VectorPtr stateF0(new std::vector<double>(NB_X_VALUES));
								...
								ARM_VectorPtr& swapRate) const
{
	...
	ARM_VectorPtr F0(new std::vector<double>(2*NB_X_VALUES*nbStates,0.0));
...
for(size_t tIdx=0;tIdx<nbPrices;++tIdx)
{
	...
	std::vector<double> F0(NB_X_VALUES*nbStates);
	std::vector<double> Mu0(2*NB_X_VALUES*nbStates);

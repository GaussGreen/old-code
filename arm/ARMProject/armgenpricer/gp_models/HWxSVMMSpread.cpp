...
ARM_HWxSVMMSpread::ARM_HWxSVMMSpread(const ARM_ModelNameMap& modelNameMap, ARM_HullWhite2F * correlModel,
									 const std::vector<double>& corrEndTimes, double ConstantCrossCorrel) : 
...
itsConvexAdjDone(false), itsFakeParam(ARM_ModelParamType::Volatility, 0.)
{
	...
	std::vector<double> times(1,0.), values(1, 1.);
...
itsConvexAdjDone(false), itsLastTHW(rhs.itsLastTHW), itsSpreadGVol(rhs.itsSpreadGVol), itsFakeParam(rhs.itsFakeParam)
{
	...
	DuplicateCloneablePointorVectorInPlace<std::vector<double>>( rhs.itsxCorrel, itsxCorrel );
...
ARM_HWxSVMMSpread::~ARM_HWxSVMMSpread()
{
	...
	DeletePointorVector<std::vector<double>>( itsxCorrel );
}
...
void ARM_HWxSVMMSpread::computeCrossCorrel(ARM_HullWhite2F * correlModel, double ConstantCrossCorrel)
{
	...
	for(i = 0; i < size; i++)
	{
		...
		itsxCorrel[i] = new std::vector<double>(dim, 0.);
...
void ARM_HWxSVMMSpread::computeConvexAdj() const
{
    ...
    std::vector<double> sigmaTimes( ( (ARM_CurveModelParam&) itsHWModel->GetModelParams()->GetModelParam( ARM_ModelParamType::Volatility )).GetCurve()->GetAbscisses());  // Ui
    std::vector<double> sigmaValues(( (ARM_CurveModelParam&) itsHWModel->GetModelParams()->GetModelParam( ARM_ModelParamType::Volatility )).GetCurve()->GetOrdinates()); // Sigma(Ui)
...
void ARM_HWxSVMMSpread::NumMethodStateLocalVariances(const std::vector<double>& timeSteps, ARM_MatrixVector& localVariances) const
{
...
void ARM_HWxSVMMSpread::NumMethodStateGlobalVariances(const std::vector<double>& timeSteps, ARM_MatrixVector& globalVariances) const
{
...
void ARM_HWxSVMMSpread::ModelStateLocalVariancesAndStdDev(const std::vector<double>& timeSteps)
{
...
//	return;
	...
	DuplicateCloneablePointorVectorInPlace<std::vector<double>>(itsSpreadModel->GetEigenValues(), itsEigenValues);

//	SetModelStateLocalVars(itsSpreadModel->GetModelStateLocalVars());
}

void ARM_HWxSVMMSpread::NumMethodStateLocalVariancesAndStdDev( const std::vector<double>& timeSteps )
{
...
void ARM_HWxSVMMSpread::ModelStateLocalVariances(const std::vector<double>& timeSteps, ARM_MatrixVector& localVariances) const
{
	int totalFwds		= itsNbEffectiveReset;
	int timeStepsSize	= timeSteps.size();
...
//	return;
	...
	std::vector<double> fachw(aliveFwds), facx(aliveFwds);

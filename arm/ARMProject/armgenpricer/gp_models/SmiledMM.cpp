...
///	Action  : 
////////////////////////////////////////////////////
std::vector<double>& ARM_SmiledMM::ComputeModelTimes(  const ARM_TimeInfoPtrVector& timeInfos )
{
...
////////////////////////////////////////////////////
void ARM_SmiledMM::NumMethodStateLocalVariances( const std::vector<double>& timeSteps,
...
////////////////////////////////////////////////////
void ARM_SmiledMM::NumMethodStateGlobalVariances( const std::vector<double>& timeSteps,
...
////////////////////////////////////////////////////
void ARM_SmiledMM::ModelStateLocalVariancesAndStdDev( const std::vector<double>& timeSteps)
{
	...
	else
	{
		...
		std::vector<double> eigenValues(totalFwds);
		...
		for(i=0;i<timeStepsSize-1 ;++i)
		{
			...
			itsEigenValues[i] = new std::vector<double>(factorsNb);
...
///	Action  : computes the local variance and std dev 
////////////////////////////////////////////////////
void ARM_SmiledMM::NumMethodStateLocalVariancesAndStdDev( const std::vector<double>& timeSteps )
{
...
////////////////////////////////////////////////////
void ARM_SmiledMM::ModelStateLocalVariances( const std::vector<double>& timeSteps,
		ARM_MatrixVector& localVariances ) const
{
	size_t totalFwds     = itsResetDates.size(),
		   factorsNb     = FactorCount(),
...
double ARM_SmiledMM::GetEquivalentShift(const std::vector<double>& weight) const
{
	double shift = 0;
	size_t k = 0;

	for (std::vector<double>::const_iterator iter = weight.begin() ; iter != weight.end() ;++iter,++k)
...
{
	std::vector<double>& res=new std::vector<double>(itsResetDates.size());

	size_t k = 0;
	
	for (std::vector<double>::iterator iter = res->begin() ; iter != res->end() ;++iter,++k)
...
void ARM_SmiledMM::AdviseCurrentCalibSecIndex(size_t index,ARM_ModelFitter& modelFitter)
{
	...
	std::vector<double> date = ((ARM_ModelParamsSmiled* ) GetModelParams())->GetCorrelCurve()->GetAbscisses();
...
ARM_VectorPtr ARM_SmiledMM::ComputeDSwapDRate(	const std::vector<double>& startDates,const std::vector<double>& endDates,
					const std::vector<double>& fixPayTimes,const std::vector<double>& fixPayPeriods) const
{
	...
	std::vector<double>& result	=	new std::vector<double>(fwdsNb,0.);
	...
	if (nbFixPay>0)
	{
		...
		std::vector<double> dswapdzc(nbFixPay+2);
...
ARM_VectorPtr ARM_SmiledMM::ComputeDLogShiftSwapDRate(double shift, const std::vector<double>& startDates,const std::vector<double>& endDates,
					const std::vector<double>& fixPayTimes,const std::vector<double>& fixPayPeriods) const
{
	size_t	k,
			size		= startDates.size(),
...
/*	if( fixNotional.size() != fixPayTimes.size() )
	...
	ARM_VectorPtr fwd		= SwapRate(	curveName, evalTime,floatStartTime,floatEndTime,
										...
										std::vector<double>(1,0.), false, states);
		...
		bool isConstantStrike) const
{
	...
	std::vector<double>::iterator iterFwd = fwd->begin();
	std::vector<double>::iterator iterLevel = level->begin();
	...
	if (itsCalibProxy==ARM_ModelParamsSmiled::GaussBasketMoneyness || itsCalibProxy==ARM_ModelParamsSmiled::GaussBasketAtm)
	{
		...
		{
			for (std::vector<double>::iterator iter = result->begin() ; iter != result->end() ; ++iter,++iterFwd,++iterLevel,++k)
...
//CMSLong (Fix Leg)
	...
	std::vector<double>& longFixPayTimes	 = fixDateStrip1.GetPaymentDates();
	std::vector<double>& longFixPayPeriods = fixDateStrip1.GetInterestTerms(); 
	std::vector<double>& floatStartTimes1	= fixDateStrip1.GetFlowStartDates();
	std::vector<double>& floatPayTimes1	= fixDateStrip1.GetFlowEndDates();
	std::vector<double>& floatIntTerms1	= fixDateStrip1.GetInterestTerms();
	std::vector<double>& floatResetDate1	= fixDateStrip1.GetResetDates();
...
//CMSShort (Fix leg)
	...
	std::vector<double>& shortFixPayTimes   = fixDateStrip2.GetPaymentDates();
	std::vector<double>& shortFixPayPeriods = fixDateStrip2.GetInterestTerms();
	std::vector<double>& floatStartTimes2	= fixDateStrip2.GetFlowStartDates();
	std::vector<double>& floatPayTimes2	= fixDateStrip2.GetFlowEndDates();
	std::vector<double>& floatIntTerms2	= fixDateStrip2.GetInterestTerms();
	std::vector<double>& floatResetDate2	= fixDateStrip2.GetResetDates();
...
//Create ARM_VanillaSpreadOptionArg
	...
	std::vector<double>& floatResetDateToTime	= fixDateStripToTime.GetResetDates();
	std::vector<double>& floatStartDateToTime	= fixDateStripToTime.GetFlowStartDates();
	std::vector<double>& floatEndDateToTime	= fixDateStripToTime.GetFlowEndDates();
	std::vector<double>& floatIntTermsToTime	= fixDateStripToTime.GetInterestTerms();
	...
	std::vector<double>& floatResetDateStartTime	= fixDateStripStartTime.GetResetDates();
	std::vector<double>& floatStartDateStartTime	= fixDateStripStartTime.GetFlowStartDates();
	std::vector<double>& floatEndDateStartTime	= fixDateStripStartTime.GetFlowEndDates();
	std::vector<double>& floatIntTermsStartToTime	= fixDateStripStartTime.GetInterestTerms();

							...
							const ARM_PricingStatesPtr& states)  const
{
		...
		&&	fabs(coeffShort)<K_NEW_DOUBLE_TOL)
	{
		...
		ARM_VectorPtr cmsRateLong (new std::vector<double>(nbStates)), cmsRateShort (new std::vector<double>(nbStates));;
		...
		// spread computation
		ARM_VectorPtr spread ( new std::vector<double>(nbStates) );
								...
								const   ARM_PricingStatesPtr& states)
{
	...
	ARM_VectorPtr cmsRate (new std::vector<double>(nbStates));
...
ARM_VectorPtr ARM_Local_Normal_Model::VanillaCMSCorridorlet(
	...
	const std::vector<double>& spreadDownBarriers,
    const std::vector<double>& spreadUpBarriers,
    double  payNotional,
    int     rcvPay,
	const ARM_SwapRatePtrVector& thirdIndex, // Single rate index for double condition
	const std::vector<double>& rateDownBarriers,
	const std::vector<double>& rateUpBarriers,
    const   ARM_PricingStatesPtr& states) const
{
/****
...
****/
	...
	ARM_VectorPtr spreads ( new std::vector<double>(nbStates) );

	std::vector<double> spreadAdjFlt(DBLECOR_NB_BARRIERS),rateAdjFlt(DBLECOR_NB_BARRIERS);
	std::vector<double> spreadAdj(1),rateAdj(1);
...
fclose(f);*/
	...
	if(isPVCalib)
	{
		...
		std::vector<double> initialValues(*values);
        ...
        const ARM_PricingStatesPtr& states) const
{
	...
	const std::vector<double>& resets = ResetTimes();
	...
	std::vector<double>& result = new std::vector<double>(nbStates);
...
{
	std::vector<double> reset = ResetTimes();
...
void ARM_Local_Normal_Model::CalibrateLocalModel (const ARM_Security& security, ARM_DensityFunctor& density, bool rescaling)
{
	...
	if(ra2)
	{
		...
		std::vector<double> soLongFixPayTimes,soLongFixPayPeriods,soShortFixPayTimes,soShortFixPayPeriods; 
		...
		std::vector<double> libLongFixPayTimes,libLongFixPayPeriods,libShortFixPayTimes,libShortFixPayPeriods; 

		std::vector<double> resets(To_ARM_GP_Vector(*(ra2->GetSwapLeg()->GetResetDates())));
	...
	else if (spreadOption)
	{
		...
		if (soSize==1)
		{
			...
			std::vector<double> longFixPayTimes, longFixPayPeriods, shortFixPayTimes, shortFixPayPeriods; 
		...
		size_t oldbarrierType)
{
	...
	std::vector<double> payTimes, resetTimes, coeffsLong, coeffsShort, strikes, payPeriods, payNotionals, fixValues, payIndexLeverages;
	std::vector<double> longFloatStartTimes, longFloatEndTimes, shortFloatStartTimes, shortFloatEndTimes;
	std::vector<double> payFloatStartTimes, payFloatEndTimes;
	vector<std::vector<double>> longSwapsFixPayTimes, longSwapsFixPayPeriods, shortSwapsFixPayTimes, shortSwapsFixPayPeriods; 
	vector<std::vector<double>> paySwapsFixPayTimes, paySwapsFixPayPeriods; 
	...
	int rateCallPut;
	std::vector<double> rateStrikes,rateFloatStartTimes,rateFloatEndTimes;
	vector<std::vector<double>> rateFixPayTimes,rateFixPayPeriods;
		...
		size_t barrierType)
{
	...
	int spreadCallPut;
	double spread1, spread2;
	std::vector<double> payTimes, resetTimes, coeffsLong, coeffsShort, spreadStrikes, payPeriods, payNotionals, fixValues, payIndexLeverages;
	std::vector<double> longFloatStartTimes, longFloatEndTimes, shortFloatStartTimes, shortFloatEndTimes;
	std::vector<double> payFloatStartTimes, payFloatEndTimes;
	vector<std::vector<double>> longSwapsFixPayTimes, longSwapsFixPayPeriods, shortSwapsFixPayTimes, shortSwapsFixPayPeriods; 
	vector<std::vector<double>> paySwapsFixPayTimes, paySwapsFixPayPeriods; 
	...
	std::vector<double> unsedRateFloatStartTimes(rateFloatStartTimes);
	std::vector<double> unsedRateFloatEndTimes(nbFlows);
	vector<std::vector<double>> unsedRateFixPayTimes(nbFlows),unsedRateFixPayPeriods(nbFlows);
	for(i=0;i<nbFlows;++i)
	{
		unsedRateFloatEndTimes[i]=rateFixPayTimes[i][0];
		unsedRateFixPayTimes[i]=std::vector<double>(1,rateFixPayTimes[i][0]);
		unsedRateFixPayPeriods[i]=std::vector<double>(1,rateFixPayPeriods[i][0]);
	}
...
{		
	...
	else if (itsModelType == ARM_PricingModelType::QGM1F)
	{
		...
		ARM_VectorPtr caplet = ((ARM_QGM1F*)itsNumericalModel)->VanillaCaplet("", 0 /*as of date*/, fwdEndTime, 1.0, 1.0, expiryTime, fwdStartTime, fwdEndTime, fwdPeriod, std::vector<double>(1, strike), callPut, ARM_PricingStatesPtr( new ARM_PricingStates(1,1,0) ) );
																	...
																	double* factor1, double* factor2) const
{
	...
	std::vector<double> swapFixPayDfs (size);
																		...
																		double* factor1, double* factor2) const
{
	...
	std::vector<double> swapLongFixPayDfs (sizeLong);
	...
	std::vector<double> swapShortFixPayDfs (sizeShort);
...
{	
	...
	std::vector<double> swapFixPayDfs (size);
	std::vector<double> payIndexFixPayDfs (paySize);
...
double Local_Normal_Model_Calibration_Helper::SpreadRateNormalCovarianceFromNumericalModel(
														...
														const std::vector<double>& rateShortFixPayTimes,
														const std::vector<double>& rateShortFixPayPeriods) const
{

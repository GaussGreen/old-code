...
void ARM_HWSBGMQtoModel::NumMethodStateLocalVariances(const std::vector<double>& timeSteps, ARM_MatrixVector& localVariances) const
{
...
void ARM_HWSBGMQtoModel::NumMethodStateGlobalVariances(const std::vector<double>& timeSteps, ARM_MatrixVector& globalVariances) const
{
	size_t totalFwds     = itsSBGMModel->GetResetDates().size(),
		   timeStepsSize = timeSteps.size(),
		   modelNb		 = GetModelNb(),
...
void ARM_HWSBGMQtoModel::NumMethodStateLocalVariancesAndStdDev( const std::vector<double>& timeSteps )
{
...
void ARM_HWSBGMQtoModel::ModelStateLocalVariancesAndStdDev(const std::vector<double>& timeSteps)
{
	...
	DuplicateCloneablePointorVectorInPlace<std::vector<double>>(itsSBGMModel->GetEigenValues(), itsEigenValues);
}

void ARM_HWSBGMQtoModel::ModelStateLocalVariances(const std::vector<double>& timeSteps, ARM_MatrixVector& localVariances) const
{
...
void ARM_HWSBGMQtoModel::MCModelStatesFromToNextTime(ARM_PricingStatesPtr& states, int timeIndex) const
{
	...
	std::vector<double> qtoAdj = GetQuantoConvexAdj(currTime, nextTime, iFirst, fwdsNb-1);

	int i,j;
	double old,next,modStd;
	double x;

	std::vector<double> stddev(fwdsNb,0.);
...
std::vector<double> ARM_HWSBGMQtoModel::GetQuantoConvexAdj(double t, double T, int iFirst, int iLast) const
{
	std::vector<double> adj(iLast+1);

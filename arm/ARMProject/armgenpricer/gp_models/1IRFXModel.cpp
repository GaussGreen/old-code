...
void ARM_1IRFXModel::CalibrateCorrelation( ARM_2IRFXModelPtr Model2IRFX )
{
	...
	{
		ARM_SmiledModel_Fx* fxModel = dynamic_cast<ARM_SmiledModel_Fx*>(&*GetModel(FxModel));
		std::vector<double> resetTimes = fxModel->getResetTimes();
		std::vector<double> settlementTimes = fxModel->getSettlementTimes();
		ARM_GP_Matrix hump;
		std::vector<double> totalVol;
        ...
        ARM_GP_Matrix& variances) const
{
	...
	for (i = startTimePos; i < nbFwds; ++i)
	{
		...
		{
			std::vector<double> fxVolTimes = fxModelParams->GetVolCurve(i)->GetAbscisses();
			std::vector<double> fxVolValues(fxVolTimes.size());
...
{ 
	...
	const std::vector<double>& resetTimes = fxModel->getResetTimes();
	const std::vector<double>& settlementTimes = fxModel->getResetTimes();
	size_t nbFwds = resetTimes.size();
	...
	for(size_t i=0;i<nbSteps-1;++i)
	{
		...
		else
		{
			...
			itsEigenValues[i]	= new std::vector<double>(factorNb);
...
////////////////////////////////////////////////////
void ARM_1IRFXModel::ModelStateLocalVariancesAndStdDev( const std::vector<double>& timeSteps)
{
...
////////////////////////////////////////////////////
void ARM_1IRFXModel::NumMethodStateLocalVariances( const std::vector<double>& timeSteps,
		ARM_MatrixVector& localVariances ) const
{
	ARM_SmiledModel_Fx* fxModel = dynamic_cast<ARM_SmiledModel_Fx*>(&*GetModel(FxModel));
	ARM_ModelParams* modelParams= fxModel->GetModelParams();
...
////////////////////////////////////////////////////
void ARM_1IRFXModel::NumMethodStateGlobalVariances( const std::vector<double>& timeSteps,
...
string ARM_1IRFXModel::toString(const string& indent, const string& nextIndent) const
{
    ...
    os << "\n\n";
    os << indent << "Hybrid 1 Interest Rates & Forex model (1IR+FX)\n";
    os << indent << "----------------------------------------------\n\n";

...
void ARM_ShiftedSABR::calibre(double resetTime, double forward, double atmvol,
							  std::vector<double> * mktVols, std::vector<double> * strikes,
...
//	build(resetTime, forward, forward, 1, atmvol, DefaultAlpha, calibrho ? 0. : rho, calibnu ? 0.1 : nu, calibshift ? 1. : shift);
	...
	std::vector<double> fguess(calibshift + calibrho + calibnu);
	std::vector<double> ubound(fguess.size(), 100.), lbound(fguess.size(), -100.);
	std::vector<double> fatfguess(strikes->size(), 0.);
...
ARM_PricingStatesPtr ARM_SVBGM::Induct(ARM_PricingStatesPtr& states,double toTime)
{
	...
	{		
		if(itsCalibratedStatus == false)
		{
			...
			std::vector<double> calibshift(size), calibalpha(size), calibrho(size), calibnu(size);
...
std::vector<double>& ARM_SVBGM::ComputeModelTimes(  const ARM_TimeInfoPtrVector& timeInfos )
{
...
void ARM_SVBGM::NumMethodStateLocalVariances(const std::vector<double>& timeSteps, ARM_MatrixVector& localVariances ) const
{
...
void ARM_SVBGM::NumMethodStateGlobalVariances(const std::vector<double>& timeSteps, ARM_MatrixVector& globalVariances ) const
{
...
void ARM_SVBGM::ModelStateLocalVariancesAndStdDev( const std::vector<double>& timeSteps)
{
...
void ARM_SVBGM::NumMethodStateLocalVariancesAndStdDev( const std::vector<double>& timeSteps )
{
...
void ARM_SVBGM::ModelStateLocalVariances(const std::vector<double>& timeSteps, ARM_MatrixVector& localVariances ) const
{
	int totalFwds		= itsNbEffectiveReset;
	int timeStepsSize	= timeSteps.size();
	int modelNb			= GetModelNb();
...
void ARM_SVBGM::MCModelStatesFromToNextTime(ARM_PricingStatesPtr& states,int timeIndex) const
{
	...
	std::vector<double> x(2 * aliveFwds);
...
void ARM_SVBGM::setNumericalModelFitter(ARM_NumericalModelFitter * numericalModelFitter)
{
	if(numericalModelFitter)
	{
		...
		std::vector<double> fwdRate(size);
...
void ARM_SVBGM::getkthStrikesAndVols(int k, std::vector<double>& strikes, std::vector<double>& vols,double& atmvol)
{
	double price;
	// vol monnaie

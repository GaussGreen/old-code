...
public:
	...
	struct ARM_SystemData
	{
		...
		std::vector<double> itsModB2y;	/// -Rho(t)*Nu(t)*Epsilon(t)*F0*Im(u) (Heston formula)

		double itsModC;				/// 0.5*F0²*Epsilon(t)²
		std::vector<double> itsModCx;		/// 0.5*F0²*Epsilon(t)²*Im(u)²
		std::vector<double> itsModCy;		/// 0.5*F0²*Epsilon(t)²*Im(u) (Heston formula)
	}; 
...
public:
	...
	virtual void derivs(double t, std::vector<double>& yt, std::vector<double>& dyt) const;
};
...
struct ARM_RiccatiHWSV1F_SO : public ARM_RiccatiHWSV1F
{
	...
	struct ARM_SystemData
	{
		...
		std::vector<double> itsModDx;		/// { modD * Im(u) }
	}; 
...
protected:
	...
	CC_IS_MUTABLE std::vector<double> itsAngularShifts1;
	CC_IS_MUTABLE std::vector<double> itsPrevValues1;
	CC_IS_MUTABLE std::vector<double> itsAngularShifts2;
	CC_IS_MUTABLE std::vector<double> itsPrevValues2;
	...
	void ComputeRiccatiSchedule(double T, std::vector<double>& schedule, double Tref, bool isSOFormula=false, double Tstart=0.0) const;
	...
	void InitAnalyticalData(double evalTime, double F0, double Te, double expT0,
							bool isSOFormula,
							std::vector<double>& schedule=std::vector<double>(0), double mu0=0.0) const;
	...
	void UpdateUDependentSystemData(const std::vector<double>& ImU, bool isStdFormula, bool isSOFormula=false) const;
...
public:
	...
	//	int forumalType=HWSVNumericals::Lewis,const std::vector<double>& formulaParams=std::vector<double>(0),
	//	double maxDecay=0.0);
	ARM_HWSV1F(const ARM_ZeroCurvePtr& zc,const ARM_ModelParams* params=NULL,
		int solverType=ARM_ODEFunc::RK5Adaptative,const std::vector<double>& solverParams=std::vector<double>(0),
		int forumalType=HWSVNumericals::Lewis,const std::vector<double>& formulaParams=std::vector<double>(0),

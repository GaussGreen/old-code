...
private:
	...
	void UpdateUDependentSystemData(const std::vector<double>& ImU, bool isStdFormula=true, bool isSOFormula=false) const;
...
public:
	...
	ARM_HWSV2F(const ARM_ZeroCurvePtr& zc,const ARM_ModelParams* params=NULL,
		const std::vector<double>& solverParams=std::vector<double>(0),

...
private:
	...
	std::vector<double>							itsSettlementDates;

	std::vector<double>							itsTotalVar;
...
public:
	...
	inline const std::vector<double>& getResetTimes()	const					{ return itsResetDates; }
	inline void	 setResetTimes( const std::vector<double>& resetTimes )		{ itsResetDates = resetTimes;	}

	inline const std::vector<double>& getSettlementTimes()						{ return itsSettlementDates; }
	inline void setSettlementTimes( const std::vector<double>& settlementTimes )	{ itsSettlementDates = settlementTimes; }
	...
	const std::vector<double>& GetTotalVar() const { return itsTotalVar; }
	...
	virtual void VolatilitiesAndCorrelations( const std::vector<double>& timeSteps, ARM_GP_MatrixPtr& vols, 
											ARM_GP_MatrixPtr& d1Vols, ARM_GP_MatrixPtr& correls, 
											bool linearVol = true) const;
	void EulerLocalDrifts( const std::vector<double>& timeSteps, ARM_GP_MatrixPtr& relativeDrifts, 
						ARM_GP_MatrixPtr& absoluteDrifts) const;

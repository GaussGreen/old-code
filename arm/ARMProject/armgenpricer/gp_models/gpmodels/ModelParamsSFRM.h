...
private:
    ...
    std::vector<double>& itsFwdValues;                            /// Local vector to store Fwds values.
...
public:
	...
	/// pricing function
    std::vector<double>& InstantaneousVolatility(double t, double T) const;  
    ...
    std::vector<double>& InterpolateCorrelation(double T) const;
	...
	double AverageShift(const std::vector<double>& times );

...
void ARM_QGM1F::InitFunctionDatas()
{
    ...
    const std::vector<double>& schedVol  = ((ARM_CurveModelParam&)GetModelParams()->GetModelParam(ARM_ModelParamType::Volatility)).GetCurve()->GetAbscisses();
    const std::vector<double>& schedMrs  = ((ARM_CurveModelParam&)GetModelParams()->GetModelParam(ARM_ModelParamType::MeanReversion)).GetCurve()->GetAbscisses();
    const std::vector<double>& schedSkew = ((ARM_CurveModelParam&)GetModelParams()->GetModelParam(ARM_ModelParamType::Skew)).GetCurve()->GetAbscisses();
    std::vector<double> tmpSched(schedVol.size()+schedMrs.size()),sched(tmpSched.size()+schedSkew.size());
    ...
    std::vector<double>::iterator last=CC_NS(std,unique)(sched.begin(),sched.end());
                              ...
                              const ARM_PricingStatesPtr& states,double absoluteDrift) const
{
    ...
    std::vector<double>& lawParams = new std::vector<double>(means->size()+1);
...
///	Action : 
////////////////////////////////////////////////////
void ARM_QGM1F::InitABC(const std::vector<double>& times, const vector< std::vector<double> >& maturities)
{
}
...
////////////////////////////////////////////////////
std::vector<double>& ARM_QGM1F::ComputeModelTimes(  const ARM_TimeInfoPtrVector& timeInfos )
{
...
{
    std::vector<double>& zeros = new std::vector<double>;
...
// FIXMEFRED: mig.vc8 (25/05/2007 15:28:14):cast
	...
	std::vector<double> ATTpT0(nbFlows+1),BTTpT0(nbFlows+1),CTTpT0(nbFlows+1);
    std::vector<double> zc0TpT0(nbFlows+1);
    ...
    std::vector<double> AA(nbFlows+1),sAA(nbFlows+1),invSsAA(nbFlows+1),BB(nbFlows+1);
    double payTime;
    ...
    ARM_VectorPtr values(new std::vector<double>(nbStates));
    double zctT0,price;
    std::vector<double> zcCoef(nbFlows),fwd(nbFlows+1),shift(nbFlows+1);

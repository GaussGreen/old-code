...
////////////////////////////////////////////////////
std::vector<double>& ARM_InflationEquityMod::PricingTimeSteps(const ARM_TimeInfoPtrVector& timeInfos)
{
	...
	std::vector<double>& newptimeSteps = SortSTLBased( *ptimeSteps );
	delete ptimeSteps;
...
///          each time step
////////////////////////////////////////////////////
void ARM_InflationEquityMod::NumMethodStateLocalVariances( const std::vector<double>& timeSteps,
...
///          each time step
////////////////////////////////////////////////////
void ARM_InflationEquityMod::ModelStateLocalVariances( const std::vector<double>& timeSteps,
	ARM_MatrixVector& localVariances ) const
{
	size_t nbSteps	= timeSteps.size();
...
void ARM_InflationEquityMod::NumMethodStateLocalCovariances( const std::vector<double>& timeSteps, ARM_MatrixVector& localCorrels, const ARM_MultiAssetsModel& multiassets )
{
	size_t modelRank = GetModelRank();
...
std::vector<double>& ARM_InflationEquityMod::ComputeModelTimes(  const ARM_TimeInfoPtrVector& timeInfos )
{
...
void ARM_InflationEquityMod::ComputeCPISpotAndStore( const string& InfcurveName, double evalTime, double CPITime, string DCFLag, long DailyInterp,string ResetLag,const ARM_PricingStatesPtr& states) const
{
	...
	if( returnedVector == ARM_GP_VectorPtr(NULL) )
	{
		...
		returnedVector = ARM_GP_VectorPtr( new std::vector<double>( statesNb ) );
...
ARM_GP_VectorPtr ARM_InflationEquityMod::CPISpot( const string& InfcurveName, double evalTime, double CPITime, string DCFLag, long DailyInterp,string ResetLag,const ARM_PricingStatesPtr& states) const
{
	...
	if( returnedVector != ARM_GP_VectorPtr(NULL) )
		...
		{
			returnedVector = ARM_GP_VectorPtr( new std::vector<double>( states->size() ));
			double result = getInfCurve()->CPIInterpolate( CPITime,DCFLag, DailyInterp, ResetLag );
			for( std::vector<double>::iterator iter = returnedVector->begin() ; iter != returnedVector->end() ; iter++ )
				(*iter) = result;
		}
		else
			ARM_THROW( ERR_INVALID_ARGUMENT, ARM_USERNAME + ": CPI did not reset. Please advise.");

	return static_cast<std::vector<double>&> (returnedVector->Clone());
}
...
{ 
	...
	if( evalTime > K_DOUBLE_TOL )
	{
		...
		std::vector<double> * result = new std::vector<double>( statesNb );
	...
	{
		std::vector<double> * result = new std::vector<double>( statesNb );
		std::vector<double>::iterator iter = result->begin(), iterEnd = result->end();
...
{ 
	...
	ARM_GP_VectorPtr result( new std::vector<double>( statesSize ) );
...
{ 
	...
	std::vector<double>::iterator iter1,iter2,iter3, iterEnd = numVector->end();
...
ARM_GP_VectorPtr ARM_InflationEquityMod::YoYCapFloor( const string& irCurveName,	const string& infCurveName, double evalTime,double Strike,double FloatMargin, int CapFloor, const ARM_DateStripPtr& numDateStrip,const ARM_DateStripPtr& denomDateStrip,double itsSpread,	const ARM_PricingStatesPtr& states) const
{
	...
	std::vector<double>::iterator iterResult, iterCaplets; 
...
ARM_GP_VectorPtr ARM_InflationEquityMod::YoYCaplet( const string& irCurveName, const string& infCurveName, double EvalTime, double tenor, double CPITime, double maturityTime, double Strike, int CapFloor, const ARM_PricingStatesPtr& states ) const
{
	...
	std::vector<double>::iterator iter = result->begin(), iterEnd = result->end(), iterDFs = DFs->begin();

		 ...
		 ARM_result& result )
{
	...
	try
	{
		...
		std::vector<double>& tmpData = dateStripCombiner->GetDateStrip(dateStripNb)->GetMemberData(dataType);
		size_t i;
    ...
    ARM_result& result )
{
	...
	try
	{
		...
		std::vector<double> tmpData = *dateStripCombiner->GetMergeData();
	...
	ARM_result& result)
{
	...
	try
	{
		...
		std::vector<double>& domStartDates = domDateStrip->GetFlowStartDates();
		std::vector<double>& forStartDates = forDateStrip->GetFlowStartDates();
		std::vector<double>& fundStartDates = fundDateStrip->GetFlowStartDates();

		std::vector<double> domNotionalVec(domSize);
		std::vector<double> forNotionalVec(forSize), forSpreadVec(forSize);
		...
		*forex, 
		...
		std::vector<double> margin = basisConverter.ComputeDomMargin();

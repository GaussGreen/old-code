...
long ARM_TARNFXCalculator_CreateFunctor::operator()( ARM_result& result, long objId )
{
	...
	try
	{
		...
		std::vector<double> redemptionStrikes;
		if (genericParams->GetParamValue("RedeemStrike").GetType() == GA_OBJECT)
		{
			std::vector<double>& vector = dynamic_cast<std::vector<double>&>(LOCAL_PERSISTENT_OBJECTS->GetObject(genericParams->GetParamValue("RedeemStrike").GetObjectId()));
			int i=0;
			for (i = 0; i < vector->size(); i++)

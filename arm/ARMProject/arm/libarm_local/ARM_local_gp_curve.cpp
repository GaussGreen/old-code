    ...
    ARM_result&	result )
{
	...
	try
	{
		...
		{
			std::vector<double> res = multiCurve->Interpolate( abscisse );
    ...
    long					objId )
{
	...
	try
	{
		...
		if( ARM_Curve*  simpleCurve = dynamic_cast< ARM_Curve* >( armObj ) )
		{
			genericCurve = simpleCurve->CptCurve( std::vector<double>(C_newAbscisses));
		}
		else if( ARM_MultiCurve* multiCurve = dynamic_cast< ARM_MultiCurve* >( armObj ) )
		{
			genericCurve = multiCurve->CptCurve( std::vector<double>(C_newAbscisses));
		}
		else
		{
			CCString local_msg ("unknown curve: permitted are simple curve and multicurve");
    ...
    long					objId )
{
	...
	try
	{
		...
		else if( ARM_MultiCurve* multiCurve = dynamic_cast< ARM_MultiCurve* >( armObj ) )
		{
			...
			multiCurve->insert(C_newAbscisse,std::vector<double>(C_newOrdinate));
	...
	long objId )
{
	...
	try
	{
		std::vector<double> x1Vec(X1);
		std::vector<double> x2Vec(X2);
	...
	long objId )
{
	...
	try
	{
		...
		std::vector<double> x1Vec(To_ARM_GP_Vector(*expiries));
		std::vector<double> x2Vec(To_ARM_GP_Vector(*(volatility->GetStrikes())));
	...
	long objId )
{
	...
	try
	{
		...
		std::vector<double> abscisses = To_ARM_GP_Vector(volatility->GetExpiryTerms());
		std::vector<double> ordinates = To_ARM_GP_Vector(volatility->GetVolatilities()->GetColumn(0));

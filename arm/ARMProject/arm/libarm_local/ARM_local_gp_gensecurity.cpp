	...
	long        objId )
{
	...
	for (int i=0;i<objIds.size();i++)
	{
		...
		std::vector<double>& tmpVector = dynamic_cast< std::vector<double>& >(object);
		ARM_GP_Matrix* tmpMatrix = dynamic_cast< ARM_GP_Matrix* >(object);
		ARM_Curve* tmpCurve = dynamic_cast< ARM_Curve* >(object);
		if (tmpVector )
			objVector.push_back(ARM_GramFctorArg(ARM_VectorPtr(static_cast<std::vector<double>&> (tmpVector->Clone()))));
		else if (tmpMatrix )

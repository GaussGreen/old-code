								...
								int adjStartDate)
{
 ...
 if ( resetGap <= 0 )
	...
	ARM_Date indDate= ARM_Date( itsResetDates->Elt(0) );
		if ( itsResetDates->size() >1 ){
			for ( int i=0; i< itsResetDates->size();i++){
				tmp = ARM_Date( itsResetDates->Elt(i) );
				if ( tmp.GetMonth()== 6 || tmp.GetMonth()== 12) 
					indDate= ARM_Date( itsResetDates->Elt(i) );
				else
					itsResetDates->Elt(i)=indDate.GetJulian();

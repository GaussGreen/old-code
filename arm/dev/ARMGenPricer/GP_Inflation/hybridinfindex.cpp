...
ARM_InfIrIndex::ARM_InfIrIndex( const string			&	indexName,
	...
	if( indexName =="N"){
		...
		vCahFlows = InfFloatingCouponVector(nominals,payoff, cpiPerf, mDate["startDate"],
							    mDate["endDate"], mInt["resetFreq"],
							    mInt["payFreq"], tmpDayCount,
							    mInt["fwdRule"], mInt["stubRule"],
								mInt["intRule"], resetRollConv,
								tmpInfResetCal,	payRollConv,
								tmpPayCal, monthBegin,demGap,numGap);
...
ARM_GP_VectorPtr ARM_InfIrIndex::GetDemDates( ) const { 
	...
	return tmpLeg->GetDemJulianDates();
...
ARM_CountedPtr<ARM_IRIndex>		ARM_InfIrIndex::GetIndex( ) const{
	...
	if( dynamic_cast<InfLeg*> (&*itsIndex) )
		return dynamic_cast<InfLeg*> (&*itsIndex)->GetIndex();
	else
...
ARM_GP_VectorPtr  ARM_InfIrIndex::GetResLags( ) const {
	...
	ARM_CountedPtr<InfLeg> infLeg = itsIndex ;
	...
	else ARMTHROW(ERR_INVALID_ARGUMENT," ARM_InfIrIndex::itsIndex must be an InfLeg or ARM_SwapLeg");
...
string ARM_InfIrIndex::GetIndexName( ) const { 
	...
	if ( dynamic_cast<InfLeg*> ( &*itsIndex ) )
		return dynamic_cast<InfLeg*> ( &*itsIndex )->GetIndexName();
	else 
...
void	ARM_InfIrIndex::SetMod( ARM_BSSmiledModel* mod){
	...
	if ( !infLeg.IsNull())
	{
		ARM_CountedPtr<InfCapFloor>	itsCF (new InfCapFloor(	infLeg->GetLeg(), 1, .02, 
															infLeg->GetInterpType(), 
															infLeg->GetFirstReset()));
		
		itsCF->Instrument::ComputePrice(itsMod);
	}
	*/
}

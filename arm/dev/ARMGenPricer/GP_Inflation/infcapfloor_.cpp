...
: /*ARM_CapFloor( new ARM_SwapLeg(InfLeg(vCahFlow,interpType,	firstReset)), capFloor, strike ), Instrument(vCahFlow)*/
InfLeg(vCahFlow,interpType,	firstReset), itsCapFloor(capFloor), itsStrike(strike)
{
	...
	itsCFValues	 = ARM_GP_Vector( nbCapLet, 0.0 );
}


InfCapFloor::InfCapFloor(const InfCapFloor& rhs)
: /*ARM_CapFloor( rhs ), Instrument(rhs.GetLeg()), */ 
...
void InfCapFloor::CptCashFlowValues()
{
	...
	ARM_CountedPtr<ARM_InfBSSmiledModel> pricingMod = itsInstrumentModel ;
	if( pricingMod.IsNull() )
		ARMTHROW(ERR_INVALID_ARGUMENT," wrongt model type in InfCapFloor::ComputePrice");	
	ARM_CountedPtr<ARM_InfIdx> infidx ( GetIndex()->GetIndex() );
	...
	for (k = 0; k<nbCapLet; k++) 
	{
		...
		yoy				= itsExpectedFwdRates[k] ;
		payTime			= itsPaymentTimes[k];
		interestTerm	= itsAccrualTimes[k];

		timeToStart		= pricingMod->GetModelTimeWPublishLag( denomDate, infidx.operator->() );
		
		timeToExpiry	= pricingMod->GetModelTimeWPublishLag( numDate, infidx.operator->() );
		...
		itsCFValues[k]	= optLet/discountfactor*interestTerm;

...
double ARM_InfBSSmiledModel::SingleAssetOptionPrice(	double CPIForward,
														...
														int callput,
														double discounting, 
														ARM_CountedPtr<ARM_InfIdx> infIdx,
														ARM_CountedPtr<InfCapFloorContext> infCapFloorContext,
														ARM_CountedPtr<StoreVolInfoWithNewCF> storeInfo)
{	
/*	double totalVol = 0., pricingStrike = 0., tenor = 0.;
	...
	ComputeVolAndStrikeForCap( 
			CPIForward-1,	strike,	callput,
			infIdx,	storeInfo,	
			infCapFloorContext->GetNumDate(), infCapFloorContext->GetDenomDate(),
			totalVol,
			pricingStrike, 
			tenor );
	...
	return CC_NS( ARM_Constants, rateBase ) * result;	*/
	ARM_THROW( ERR_INVALID_ARGUMENT, ARM_USERNAME + " : not implemented you should look to the none ptr sample" );
	return	0.0;
}

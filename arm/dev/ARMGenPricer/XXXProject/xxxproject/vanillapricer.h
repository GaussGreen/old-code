...
// All the ARM calculator products pricer inherit from
// this pricer
/*
class ARM_CalculatorPricer : public ARM_Pricer
{
public:

	ARM_CalculatorPricer(ARM_GenCalculator* genCalculator);

	void				SetMkt	(	ARM_MktData * );
	
	double				Price	( );

	virtual ARM_Observator*		GetObservator(	 ) const{ return new ARM_Observator; }

private:

	ARM_GenCalculator*	itsGenCalculator;
...
*/
// The INF option pricer
class ARM_INF_Pricer : public ARM_VanillaPricer
{

public:
	ARM_INF_Pricer				(	ARM_InfCorridorLeg*	  );
	void				SetMkt	(	ARM_MktData * );
	double				Price	(	);
};

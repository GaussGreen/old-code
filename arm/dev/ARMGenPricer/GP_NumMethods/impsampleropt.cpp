...
public:
	ImpSamplerOptFunc(
		...
		bool withMC,
		const ARM_GP_VectorPtr& rowTimes=ARM_VectorPtr(NULL),
		int timeStep=-1)
		:
	itsGenSec(genSec),
	...
	virtual double operator()(ARM_GP_Vector x) const;
...
double ImpSamplerOptFunc::operator()(ARM_GP_Vector x) const
{
	if (itsTimeStep == -1)
	{
		...
		bool bootstrap)
{
	...
	if (bootstrap)
	{
		ARM_GP_VectorPtr rowSteps = ARM_GP_VectorPtr(static_cast<ARM_GP_Vector*>(genSec->GetRowDates()->Clone()));
		...
		*(rowSteps) -= newModel->GetAsOfDate().GetJulian();
		...
		for (size_t i = 0; i < rowSteps->size(); ++i)
		{
			...
			ARM_GP_Vector guess = initGuess.Interpolate((*rowSteps)[i]);
			ARM_GP_Vector lowerBound = lowerBounds.Interpolate((*rowSteps)[i]);
			ARM_GP_Vector upperBound = upperBounds.Interpolate((*rowSteps)[i]);
	...
	else
	{
		...
		// Optimize the alpha curve for each row of the gensec
		ARM_GP_Vector guess = initGuess.Interpolate(0);
		ARM_GP_Vector lowerBound = lowerBounds.Interpolate(0);
		ARM_GP_Vector upperBound = upperBounds.Interpolate(0);

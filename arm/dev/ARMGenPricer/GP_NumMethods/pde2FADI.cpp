...
void ARM_PDE2FADINumericalScheme::Induct( const ARM_PricingModel& model, ARM_PricingStatesPtr& states, int toTimeIdx)
{
	...
	for( i=0 ; i<PayoffStatesSize ; ++i)
	{
		...
		UpdatePayoffs( vec, *(diffPayoffStatesPayoffs[i]), toTimeIdx  );
	...
	for( i=0 ; i<PayoffsSize ; ++i)
	{
		...
		UpdatePayoffs( *vec, *(diffPayoffs[i]), toTimeIdx  );
...
////////////////////////////////////////////////////
void ARM_PDE2FADINumericalScheme::UpdatePayoffs(ARM_GP_Vector& vec, ARM_GP_Vector& PredictorCorrector, int toTimeIdx)
{
	...
	ARM_GP_Vector::iterator iter, iterEnd;
	ARM_GP_Vector::iterator DiagIter, UIter, LIter;
...
void ARM_PDE2FADINumericalScheme::UpdatePayoffs(ARM_GP_Matrix& vec, int payoffStateIdx, int toTimeIdx)
{
	...
	for( size_t j=0 ; j<vec.rows() ; ++j )
	{
		...
		UpdatePayoffs( vec2, *((diffPayoffStates[payoffStateIdx])->GetRow(j)), toTimeIdx );
...
void ARM_PDE2FADINumericalScheme::PrecomputeCrossedTerms( const ARM_PricingStatesPtr& states )
{
	...
	size_t PayoffStatesSize = states->GetPayoffStatesSize();
	size_t i;
	ARM_GP_Vector::iterator storedIter, otherIter, endIter, diffIter;
	...
	if( nextPayoffs.size() == 0 )
	{
		...
		{
			ARM_VectorPtr vec = states->GetPayoffVec(i);
			nextPayoffs.push_back( ARM_GP_VectorPtr( static_cast<ARM_GP_Vector*>( vec->Clone() ) ) );
			diffPayoffs.push_back( ARM_GP_VectorPtr( static_cast<ARM_GP_Vector*>( vec->Clone() ) ) );
		}
	}
	...
	if( nextPayoffStates.size() == 0 )
	{
		for( i=0 ; i<PayoffStatesSize ; ++i)
		{
			...
			nextPayoffStatesPayoffs.push_back( ARM_GP_VectorPtr( static_cast<ARM_GP_Vector*>( payoffStates.GetPayoffs().Clone() ) ) );
			diffPayoffStatesPayoffs.push_back( ARM_GP_VectorPtr( static_cast<ARM_GP_Vector*>( payoffStates.GetPayoffs().Clone() ) ) );
		}
	}

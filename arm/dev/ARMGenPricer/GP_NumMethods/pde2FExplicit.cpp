...
void ARM_PDE2FExplicitNumericalScheme::UpdatePayoffs(ARM_GP_Vector& vec, int toTimeIdx)
{
	...
	double crossTerm = itsCrossCoeffs->Elt(matIdx);
	...
	for( size_t i=0 ; i < matrixSize ; ++i)
		vec.Elt(i) = 0.5*(itsPayoffs2->Elt(i) + vec.Elt(i));
}
...
{
	ARM_GP_Vector::iterator prevXY, prevXprevY, prevXnextY, nextXprevY, nextXY, nextXnextY, XnextY, XprevY, curPos, XY;

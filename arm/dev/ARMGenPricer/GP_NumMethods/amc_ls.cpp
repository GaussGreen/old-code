...
ARM_ExerciseBoundary * ARM_AMCLongstaffSchwartz::ComputeExerciseBoundary( const ARM_GP_VectorPtr& payoff, const ARM_GP_VectorPtr& contOpt, 
			const ARM_GP_MatrixPtr& StatesVector )
{
	...
	if (!payoff.IsNull()&&!contOpt.IsNull()&&!StatesVector.IsNull())
	{
		...
		if (itsRegressionMode == ARM_Regression::LS)
			regression = ARM_RegressionPtr(new ARM_LSRegression(contOpt, StatesVector ));
		else if (itsRegressionMode == ARM_Regression::LOESS)
			regression = ARM_RegressionPtr(new ARM_LOESSRegression(contOpt, StatesVector, itsSpan ));
...
ARM_GP_VectorPtr& ARM_AMCLongstaffSchwartz::ComputeRegression
	( const ARM_VectorPtrDbleFuncPtrVector& Fcts, const ARM_VectorPtrVector& X, 
	const ARM_VectorPtr& Y )
{
	...
	ARM_GP_Vector * A = LeastSquareRegression( Mat, *Y );
	ARM_GP_VectorPtr * B = new ARM_GP_VectorPtr( A );

...
void ARM_PDEMethod::ComputeAndSetTimeSteps(const ARM_PricingModel& model) const
{
	...
	double lastTimeStep = timeSteps->Elt(timeSteps->size()-1);
	...
	/// Merge
	ARM_GP_Vector* pNewTimeSteps = MergeSortedVectorNoDuplicates( *timeSteps, *VolTimeSteps );
	timeSteps = NULL;
	...
	{
		if ( pNewTimeSteps->Elt(i) > lastTimeStep + 1.e-7 )
		{
			resizing = i;
			break;
		}
	...
	const_cast< ARM_PDEMethod* >(this)->SetTimeSteps(*pNewTimeSteps);
	delete pNewTimeSteps;

	/// Adds TimeSteps from what users asked to. 
	ARM_GP_Vector AdditionalTimeSteps( itsTimeItersNb );
	ARM_GP_Vector * currentTimeSteps = const_cast< ARM_PDEMethod* >(this)->GetTimeSteps();
	double OtherDatesStep = currentTimeSteps ->Elt(currentTimeSteps ->size() -1)/(itsTimeItersNb+2);

	if (AdditionalTimeSteps.size())
	{
		AdditionalTimeSteps[0] = OtherDatesStep;
		for( i=1 ; i< itsTimeItersNb ; ++i)
			AdditionalTimeSteps[i] = AdditionalTimeSteps[i-1]+OtherDatesStep;
	}

	ARM_GP_Vector * newTimeSteps = MergeSortedVectorNoDuplicates( AdditionalTimeSteps, *currentTimeSteps );
	currentTimeSteps = NULL;

	const_cast< ARM_PDEMethod* >(this)->SetTimeSteps( *newTimeSteps );
	delete newTimeSteps;
}
...
////////////////////////////////////////////////////
ARM_GP_MatrixPtr ARM_PDEMethod::GetSpotProbabilities(const ARM_GP_Vector& eventTimes) const
{
...
{
	return static_cast<ARM_GP_VectorPtr>(new ARM_GP_Vector(0));
}

...
public:
    ...
    ARM_TruncatorND(const ARM_GP_Vector& stdDevs)
    ...
    /// Accessors
    const ARM_GP_Vector& GetStdDevRatio() const { return itsStdDevRatio; }
    ...
    /// Reconnection is truncation occurs
    virtual void UpdateNode( double sliceTime, ARM_SliceBase* tmpSlice, size_t stateIdx, ARM_SliceBase* tmpNextSlice, const ARM_GP_Vector& meanRel, const ARM_GP_Vector& varRel ) const;
...
public:
    ...
    ARM_TruncatorNDArrowDebreu(const ARM_GP_Vector& stdDevs, const ARM_IntVector& maxMaxIndex,
        double arrowDebreuThreshold=1.0e-7, double timeThreshold=10.0 )

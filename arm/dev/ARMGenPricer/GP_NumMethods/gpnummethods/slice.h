...
private:
	...
	ARM_GP_Vector itsProbaUp;
	ARM_GP_Vector itsProbaDown;
...
private:
    ...
    ARM_GP_Vector itsProbaUpDown;   // to optimise computation time : may be 0 if truncated hedge
...
public:
    ARM_SliceNDBase( size_t nbDims, size_t index, const ARM_IntVector& min, const ARM_IntVector& max, const ARM_GP_Vector& spaceStep)
    ...
    const ARM_GP_Vector& GetSpaceStep() const { return itsSpaceStep; }
    double GetSpaceStep(size_t i) const { return itsSpaceStep[i]; }
    void SetSpaceStep(const ARM_GP_Vector& spaceStep) { itsSpaceStep=spaceStep; }
...
public:
    ARM_SliceND( size_t nbDims, size_t index, const ARM_IntVector& min, const ARM_IntVector& max, const ARM_GP_Vector& spaceStep)
...
{
private:
	ARM_GP_Vector itsProba;             // cst proba to go up or down for the whole slice
    ARM_BoolMatrix itsTruncatedHedge;   // to say if (state,dim) is truncated

public:
    ARM_SliceNDCstSymProba( size_t nbDims, size_t index, const ARM_IntVector& min,  const ARM_IntVector& max, const ARM_GP_Vector& spaceStep, const ARM_GP_Vector& proba)

...
{
	itsP = x->GetColsNb();
	ARM_GP_Vector * regressedCoeffs = LeastSquareRegression( *x, *y );
	itsRegressedCoeffs = ARM_GP_VectorPtr(regressedCoeffs);
}
...
ARM_GP_VectorPtr ARM_LSRegression::ComputeValues(const ARM_GP_MatrixPtr& x) const
{
	...
	ARM_GP_Vector *y = (*x)*(*itsRegressedCoeffs);
...
ARM_GP_VectorPtr ARM_LOESSRegression::ComputeValues(const ARM_GP_MatrixPtr& x) const
{
	...
	ARM_GP_VectorPtr ret(new ARM_GP_Vector(m));

	for (i = 0; i < m; ++i)
	{
		(*ret)(i) = loessPred->fit[i];
	}

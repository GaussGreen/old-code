...
/// double operator()( const ARM_GP_Vector& x ) const;

template <typename T>
	struct GradientFunction : public CC_NS( ARM_GP, UnaryFunc)<ARM_GP_Vector,ARM_GP_Matrix> 
{
	...
	virtual ARM_GP_Matrix operator()( const ARM_GP_Vector& x ) const;
...
template <typename T>
	ARM_GP_Matrix GradientFunction<T>::operator()( const ARM_GP_Vector& x ) const
{
	ARM_GP_Vector tmpX(x), fx_plus, fx_minus;

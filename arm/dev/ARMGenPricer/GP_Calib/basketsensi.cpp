...
///	Action : 
////////////////////////////////////////////////////
void ARM_BasketSensi::FillCpnIndex(const ARM_GP_Vector& exerciseDates,const ARM_GP_Vector& startDates)
{
	double days	= 7.;
...
double ARM_BasketSensiCSO::GetDCpn(int i, const ARM_STRIPPER& stripper)
{
	...
	double Long = stripper.swapRate(longStartTime, longEndTime, *longPayTimes, *longPayPeriods);
	...
	double Short = stripper.swapRate(shortStartTime, shortEndTime, *shortPayTimes, *shortPayPeriods);
...
void ARM_BasketSensiCSO::Init(int i, const ARM_STRIPPER& stripper)
{
	...
	ARM_GP_Vector* longPayTimes    = itsArg->GetSwapLongFixPayTimes()[i];
	ARM_GP_Vector* longPayPeriods  = itsArg->GetSwapLongFixPayPeriods()[i];
	itsLong0[i] = stripper.swapRate(longStartTime, longEndTime, *longPayTimes, *longPayPeriods);

	double shortStartTime = itsArg->GetSwapShortFloatStartTime()->Elt(i);
	double shortEndTime   = itsArg->GetSwapShortFloatEndTime()->Elt(i);
	ARM_GP_Vector* shortPayTimes    = itsArg->GetSwapShortFixPayTimes()[i];
	ARM_GP_Vector* shortPayPeriods  = itsArg->GetSwapShortFixPayPeriods()[i];
	itsShort0[i] = stripper.swapRate(shortStartTime, shortEndTime, *shortPayTimes, *shortPayPeriods);
	...
	if (IsVariableCpn(i))
	{
		...
		if (i==0 || ( (i>0) && ((*periodIndexes)[i]!=(*periodIndexes)[i-1]) ) )
		{
			...
			itsPay0[i] = stripper.swapRate(payStartTime, payEndTime, *payTimes, *payPeriods);
		}
		else
			itsPay0[i] = 0.0;
	}

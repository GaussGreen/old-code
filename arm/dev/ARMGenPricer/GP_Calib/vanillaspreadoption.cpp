...
void ARM_VanillaSpreadOptionArg::ComputeIndexSchedulesAndAdjustDates (ARM_Currency* ccy, double asOfDate) 
{
	...
	if (!itsLongIndexIsLibor)
	{
		...
		for (size_t i(0); i<size; i++)
		{
			...
			// push
			itsSwapLongFixPayTimes[i] = (ARM_GP_Vector*)longDateStrip.GetPaymentDates()->Clone();
			itsSwapLongFixPayPeriods[i] = (ARM_GP_Vector*)longDateStrip.GetInterestTerms()->Clone();
	...
	if (!itsShortIndexIsLibor)
	{
		...
		for (size_t i(0); i<size; i++)
		{
			...
			// push
			itsSwapShortFixPayTimes[i] = (ARM_GP_Vector*)shortDateStrip.GetPaymentDates()->Clone();
			itsSwapShortFixPayPeriods[i] = (ARM_GP_Vector*)shortDateStrip.GetInterestTerms()->Clone();
	...
	if (itsPayIndexType == K_CMS)
	{
		...
		for (size_t i(0); i<size; i++)
		{
			...
			// push
			itsSwapPayFixPayTimes[i]   = (ARM_GP_Vector*)payDateStrip.GetPaymentDates()->Clone();
			itsSwapPayFixPayPeriods[i] = (ARM_GP_Vector*)payDateStrip.GetInterestTerms()->Clone();

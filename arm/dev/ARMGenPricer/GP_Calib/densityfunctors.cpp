...
{
	ARM_GP_Vector::iterator iter1, iter2, iterEnd;
	...
	itsLambda(lambda)
{
	...
	//itsVol2 = Inverse(func,Inverse::REAL)(priceATM,guess,guess/5.,1e-12);
}
		...
		double lambda)
{
	if ((1.0-lambda) >= K_NEW_DOUBLE_TOL)
		return lambda*BlackSholes_Formula(fwd-alpha ,vol1*sqrt(maturity), 1.,strike,1)
			+(1.0-lambda)*BlackSholes_Formula(fwd+lambda/(1.0-lambda)*alpha,vol2*sqrt(maturity), 1.,strike,1);
	else
...
ARM_SplineDensityFunctor::ARM_SplineDensityFunctor( const ARM_GP_Vector& money, const ARM_GP_Vector& vol, ARM_SmileViewer::MoneyType type)
: ARM_DensityFunctor(),
	itsSize(vol.size()),
	itsVar(0),
	itsMoney(0),
...
double ARM_SplineDensityFunctor::Quantile( double proba, double fwd, double mat) const
{
	...
	public: 
		double m_fwd;
		ARM_GP_Vector m_pMoney;
		ARM_GP_Vector m_pVar;
		ARM_GP_Vector m_pDer2;
		size_t		   m_type;
		DistributionToInverse(double fwd,const ARM_GP_Vector& pMoney, 
			const ARM_GP_Vector& pVar, 
			const ARM_GP_Vector& pDer2,
			size_t type):
...
double ARM_SplineDensityFunctor::Proba(double strike, double fwd, const ARM_GP_Vector& money, const ARM_GP_Vector& var, const ARM_GP_Vector& der2, size_t type)
{
	double eps = 1e-6;
...
double ARM_SplineDensityFunctor::ComputeVol(double strike, double fwd, const ARM_GP_Vector& money, const ARM_GP_Vector& var, const ARM_GP_Vector& der2, size_t type)
{
...
double ARM_SplineDensityFunctor::ComputeDVol(double strike, double fwd, const ARM_GP_Vector& money, const ARM_GP_Vector& var, const ARM_GP_Vector& der2, size_t type)
{
...
double ARM_SplineDensityFunctor::ComputeVol(double x, const ARM_GP_Vector& money, const ARM_GP_Vector& var, const ARM_GP_Vector& der2)
{
...
double ARM_SplineDensityFunctor::ComputeDVol(double x, const ARM_GP_Vector& money, const ARM_GP_Vector& var, const ARM_GP_Vector& der2)
{
...
//-----------------------------------------------------------------------------------------
ARM_SmileViewer::ARM_SmileViewer(const ARM_GP_Vector& money,MoneyType type,const ARM_GP_Vector& strikes)
:	itsMoney(money),
	itsStrike(0),
	itsVol(0),
	itsType(type),

...
ARM_VanillaCapArg* ARM_ConverterFromKernel::ConvertVanillaCapFloor( ARM_CapFloor* capFloor, double asOfDate, const string& modelName )
{
	...
	ARM_GP_Vector* ResetTimes    = To_pARM_GP_Vector( capFloor->GetResetDates() );
	ARM_GP_Vector* StartTimes    = To_pARM_GP_Vector( capFloor->GetSwapLeg()->GetFwdRateStartDates() );
	ARM_GP_Vector* EndTimes      = To_pARM_GP_Vector( capFloor->GetSwapLeg()->GetFwdRateEndDates() );
	ARM_GP_Vector* PayTimes      = To_pARM_GP_Vector( capFloor->GetPaymentDates() );
	ARM_GP_Vector* PayPeriods    = To_pARM_GP_Vector( capFloor->GetSwapLeg()->GetInterestTerms() );
	ARM_GP_Vector* Nominals      = new ARM_GP_Vector(size);
	ARM_GP_Vector* Strikes       = new ARM_GP_Vector(size);
...
ARM_VanillaDigitalArg* ARM_ConverterFromKernel::ConvertVanillaDigital( ARM_Digital* digital, double asOfDate, const string& modelName )
{
	...
	ARM_GP_Vector* ResetTimes    = To_pARM_GP_Vector( digital->GetResetDates() );
	ARM_GP_Vector* StartTimes    = To_pARM_GP_Vector( digital->GetSwapLeg()->GetFwdRateStartDates() );
	ARM_GP_Vector* EndTimes      = To_pARM_GP_Vector( digital->GetSwapLeg()->GetFwdRateEndDates() );
	ARM_GP_Vector* PayTimes      = To_pARM_GP_Vector( digital->GetSwapLeg()->GetPaymentDates() );
	ARM_GP_Vector* PayPeriods    = To_pARM_GP_Vector( digital->GetSwapLeg()->GetInterestTerms() );
	ARM_GP_Vector* Nominals      = new ARM_GP_Vector(size);
	ARM_GP_Vector* Strikes       = new ARM_GP_Vector(size);
...
ARM_VanillaCorridorLegArg* ARM_ConverterFromKernel::ConvertVanillaCorridorLeg(ARM_CorridorLeg* corridorleg, double asOfDate, const string& modelName )
{
	...
	ARM_GP_Vector* ResetTimes       = To_pARM_GP_Vector( corridorleg->GetResetDates() );
	ARM_GP_Vector* StartTimes       = To_pARM_GP_Vector( corridorleg->GetPaymentStartDates() );
	ARM_GP_Vector* EndTimes         = To_pARM_GP_Vector( corridorleg->GetPaymentEndDates() );
	ARM_GP_Vector* PayTimes         = To_pARM_GP_Vector( corridorleg->GetPaymentDates() );
    int indexPaymentType            = corridorleg->GetPaymentIndex()->GetIndexType();
    ARM_GP_Vector* fwdPaymentPeriod = To_pARM_GP_Vector( corridorleg->GetPaymentInterestterms() );

    ARM_VectorVector DownBarriers;
	ARM_VectorVector UpBarriers  ;

	ARM_GP_Vector* Nominals      = new ARM_GP_Vector(size);
	ARM_GP_Vector* CouponMargin  = new ARM_GP_Vector(size);
    ...
    for(i=0; i< size; ++i )
    {
        ARM_GP_Vector DownBarrier(0);
	    ARM_GP_Vector UpBarrier(0)  ;

        ARM_GP_Vector RefIdxResettimes(0);
        ARM_GP_Vector RefIdxStarttimes(0);
        ARM_GP_Vector RefIdxEndtimes(0);
        ARM_GP_Vector RefFwdPeriods(0);
        ARM_GP_Vector RefIndexWeight(0);
        ...
        DownBarriers.push_back((ARM_GP_Vector*)DownBarrier.Clone());
        UpBarriers.push_back((ARM_GP_Vector*)UpBarrier.Clone());

        VectRefIdxResettimes.push_back((ARM_GP_Vector*)RefIdxResettimes.Clone());
        VectRefIdxStarttimes.push_back((ARM_GP_Vector*)RefIdxStarttimes.Clone());
        VectRefIdxEndtimes.push_back((ARM_GP_Vector*)RefIdxEndtimes.Clone());
        VectRefFwdPeriods.push_back((ARM_GP_Vector*)RefFwdPeriods.Clone());
        VectRefIndexWeight.push_back((ARM_GP_Vector*)RefIndexWeight.Clone());
    }
...
ARM_VanillaSwaptionArg* ARM_ConverterFromKernel::ConvertVanillaSwaption( ARM_Swaption* swaption, double asOfDate, const string& modelName )
{
	...
	ARM_GP_Vector* fixPayTimes = To_pARM_GP_Vector( swaption->GetFixedLeg()->GetFlowEndDates() );
	if( fixPayTimes )
		(*fixPayTimes) -=asOfDate;

	ARM_GP_Vector* fixPayPeriods = To_pARM_GP_Vector( swaption->GetFixedLeg()->GetInterestTerms() );
	
	ARM_GP_Vector* floatResetTimes = To_pARM_GP_Vector( swaption->GetFloatLeg()->GetResetDates() );
	if( floatResetTimes)
		(*floatResetTimes) -=asOfDate;
	
	ARM_GP_Vector* floatStartTimes = To_pARM_GP_Vector( swaption->GetFloatLeg()->GetFwdRateStartDates() );
	if( floatStartTimes )
		(*floatStartTimes) -=asOfDate;
	
	ARM_GP_Vector* floatEndTimes   = To_pARM_GP_Vector( swaption->GetFloatLeg()->GetFwdRateEndDates() );
	if( floatEndTimes )
		(*floatEndTimes) -=asOfDate;
	
	ARM_GP_Vector* floatInterestTerms = To_pARM_GP_Vector( swaption->GetFloatLeg()->GetInterestTerms() );
	...
	if(!fixLeg->GetVarCoupons())
		Strikes = new ARM_GP_Vector(sizefix, swaption->GetStrike()/CC_NS(ARM_Constants,rateBase));
    else
    {
        Strikes = new ARM_GP_Vector(sizefix);
	...
	ARM_GP_Vector*  FixNotional =new ARM_GP_Vector(nbFxFlows);
	ARM_GP_Vector*  FloatNotional = new ARM_GP_Vector(nbFloatFlows);
	...
	if ((swaption->GetFloatLeg()->GetVariableSpread()) && !(swaption->GetFloatLeg()->GetVariableSpread()->IsConstant()))
	{
		...
		MarginVector = new ARM_GP_Vector(floatResetTimes->size(),swaption->GetFloatLeg()->GetSpread()/CC_NS(ARM_Constants,rateBase));
...
ARM_VanillaStripArg* ARM_ConverterFromKernel::ConvertToStripOption( ARM_StripOption* stripOption,double asOfDate, const string& modelName )
{
	...
	ARM_GP_Vector coeffs(strip.size());

	ARM_GP_Vector* startDates = stripOption->GetSchedule()->GetFlowStartDates();
	ARM_GP_Vector* payDates   = stripOption->GetSchedule()->GetPaymentDates();
	ARM_GP_Vector* terms      = stripOption->GetSchedule()->GetInterestTerms();
...
ARM_VanillaStripArg* ARM_ConverterFromKernel::ConvertToStripDigitalOption( ARM_StripDigitalOption* stripDigitalOption,double asOfDate, const string& modelName )
{
	...
	ARM_GP_T_Vector<ARM_VanillaArg*> vanillas;
	ARM_GP_Vector coeffs(stripUp.size());
	ARM_GP_Vector* resetDates   = stripDigitalOption->GetSchedule()->GetResetDates();
	ARM_GP_Vector* startDates   = stripDigitalOption->GetSchedule()->GetFlowStartDates();
	ARM_GP_Vector* payDates   = stripDigitalOption->GetSchedule()->GetPaymentDates();
	ARM_GP_Vector* terms      = stripDigitalOption->GetSchedule()->GetInterestTerms();
...
ARM_VanillaSpreadOptionArg* ARM_ConverterFromKernel::ConvertVanillaSpreadOption( ARM_SpreadOption* spreadoption, double asOfDate, const string& modelName )
{
	...
	ARM_GP_Vector* coeffShort = new ARM_GP_Vector(nbCapLet,coeffShortValue);
	ARM_GP_Vector* coeffLong = new ARM_GP_Vector(nbCapLet,coeffLongValue);
	
	ARM_GP_Vector* resetTimes		= To_pARM_GP_Vector(spreadoption->GetSwapLeg()->GetResetDates());
	ARM_GP_Vector* payTimes			= To_pARM_GP_Vector(spreadoption->GetSwapLeg()->GetPaymentDates()); 
	ARM_GP_Vector* swapShortFloatStartTime	= To_pARM_GP_Vector(spreadoption->GetSpreadLeg()->GetFirstLeg()->GetFwdRateStartDates());
	ARM_GP_Vector* swapLongFloatStartTime	= To_pARM_GP_Vector(spreadoption->GetSpreadLeg()->GetSecondLeg()->GetFwdRateStartDates());
	...
	ARM_GP_Vector* payPeriods = new ARM_GP_Vector(nbCapLet);
	ARM_GP_Vector* strikes    = new ARM_GP_Vector(nbCapLet);
	ARM_GP_Vector* notional   = new ARM_GP_Vector(nbCapLet);
	ARM_GP_Vector* swapShortFloatEndTime  = new ARM_GP_Vector(nbCapLet);
	ARM_GP_Vector* swapLongFloatEndTime   = new ARM_GP_Vector(nbCapLet);
...
// FIXMEFRED: mig.vc8 (22/05/2007 18:07:17): floor(int) doesnt mean anything
    ...
    for (k = 0; k < nbCapLet; k++) 
	{
		...
		else
		{
			...
			swapShortFixPayTimes.push_back(new ARM_GP_Vector(1,(*swapShortFloatEndTime)[k]));
			swapShortFixPayPeriods.push_back(new ARM_GP_Vector(1,period));
		}
		...
		else
		{
			...
			swapLongFixPayTimes.push_back(new ARM_GP_Vector(1,(*swapLongFloatEndTime)[k]));
			swapLongFixPayPeriods.push_back(new ARM_GP_Vector(1,period));
		}
	...
	if( spreadoption->IsCorridorSpread() || spreadoption->IsDigitalFLT() )
	{
		...
		swapPayFloatEndTime    = new ARM_GP_Vector(nbPeriods);
		swapPayFloatStartTime  = new ARM_GP_Vector(nbPeriods);
		...
		fixValues				= new ARM_GP_Vector(nbCapLet);
		
		double tenor_p;

		payIndexLeverages	= new ARM_GP_Vector(nbPeriods);
		payIndexResetTimes	= new ARM_GP_Vector(nbPeriods);
		...
		{			
			...
			else if (payIndexType == K_LIBOR)
			{
				...
				swapPayFixPayTimes.push_back(new ARM_GP_Vector(1,(*swapPayFloatEndTime)[k]));
				swapPayFixPayPeriods.push_back(new ARM_GP_Vector(1,period));
			}
			else
			{
				...
				swapPayFixPayTimes.push_back(new ARM_GP_Vector(1, 0.0));
				swapPayFixPayPeriods.push_back(new ARM_GP_Vector(1, 0.0));
			}
...
ARM_VanillaIRSwaplegArg* ARM_ConverterFromKernel::ConvertVanillaIRSwapleg( ARM_SwapLeg* swapleg, double asOfDate, const string& modelName )
{
	...
	ARM_GP_Vector* ResetTimes    = To_pARM_GP_Vector( swapleg->GetResetDates() );
	ARM_GP_Vector* StartTimes    = To_pARM_GP_Vector( swapleg->GetFwdRateStartDates() );
	ARM_GP_Vector* EndTimes      = To_pARM_GP_Vector( swapleg->GetFwdRateEndDates() );
	ARM_GP_Vector* PayTimes      = To_pARM_GP_Vector( swapleg->GetPaymentDates() );
	ARM_GP_Vector* PayPeriods    = To_pARM_GP_Vector( swapleg->GetInterestTerms() );
	ARM_GP_Vector* Nominals      = new ARM_GP_Vector(size);
	ARM_GP_Vector* Strikes       = new ARM_GP_Vector(size);
...
{
	ARM_GP_Vector* numResetDates = infleg->GetNumResetDates();
	ARM_GP_Vector* numFlowStartDates = infleg->GetNumPublicationDates();
	ARM_GP_Vector* numFlowPaymentDates = To_pARM_GP_Vector(infleg->GetPaymentDates() );
	ARM_GP_Vector* numInterestTerms = To_pARM_GP_Vector( infleg->GetInterestTerms() );
	ARM_GP_Vector* numInterestDays = To_pARM_GP_Vector( infleg->GetInterestDays() );
	ARM_GP_Vector* denomResetDates = infleg->GetDenomResetDates();
...
ARM_VanillaPortfolio* ARM_ConverterFromKernel::ConvertPortoflio( ARM_Portfolio* port, double asOfDate, const string& modelName )
{
	...
	if( portSize )
	{
		ARM_GP_Vector* weights		= To_pARM_GP_Vector( port->GetWeights()	);
		ARM_AutoCleaner<ARM_GP_Vector> HoldWeights(weights);
		ARM_GP_Vector* mktprices	= To_pARM_GP_Vector( port->GetMktPrices() );
		ARM_AutoCleaner<ARM_GP_Vector> HoldMktPrices(mktprices);
		ARM_GP_Vector* vegas		= To_pARM_GP_Vector( port->GetPrecision() );
		ARM_AutoCleaner<ARM_GP_Vector> HoldVegas(vegas);
	...
	{
		ARM_GP_Vector weights(0);
		ARM_GP_Vector mktprices(0);
		ARM_GP_Vector vegas(0);
...
ARM_VanillaSmiledSwaption* ARM_ConverterFromKernel::ConvertVanillaSmiledSwaption	( ARM_SmiledSwaption* opt,double asOfDate, const string& modelName)
{
	...
	ARM_GP_Vector data = opt->GetDatas();
...
ARM_VanillaIrFxSwaption* ARM_ConverterFromKernel::ConvertIrFxSwaption(ARM_OptionPortfolio* irFxSwaption,double asOfDate, const string& modelName)
{
	...
	ARM_GP_Vector fxResetTimes(nbFwdFxs);
	ARM_GP_Vector fxSettlementTimes(nbFwdFxs);
	ARM_GP_Vector fxPayTimes(nbFwdFxs);
	ARM_GP_Vector fxNominals(nbFwdFxs);

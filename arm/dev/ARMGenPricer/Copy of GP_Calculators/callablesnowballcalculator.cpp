...
/////////////////////////////////////////////////////////////////
bool FindIdxInVector( const ARM_GP_Vector& timeVector, double time, double daysNb, size_t& idx)
{
	bool found = false;
...
:	ARM_GenCalculator(mktDataManager),
		...
		itsCpnResetLag(-fabs(cpnIndexResetLag)),
		itsPayRec(payRec),
		itsCapOrFloor(CF),
...

{
	...
	if (itsControlVariableFlag)
	{
		...
		itsCVBetas = ARM_VectorPtr(new ARM_GP_Vector(0));
	}
																			...
																			// By default=1(Sigma))
{
	...
	values = ARM_GP_Vector(1,BETA_DEFAULT_VALUE);
	...
	values = ARM_GP_Vector(1,CORREL_DEFAULT_VALUE);
    tCorrel = ARM_CurveModelParam(ARM_ModelParamType::Correlation,&values,&times,"CORRELATION");
	marketDatas.push_back(&tCorrel);

	if ( theHump != -10000.0 )
	{
		values= ARM_GP_Vector(1, theHump);
    }
    else
    {
		values= ARM_GP_Vector(1, HUMP_DEFAULT_VALUE);
	}
	...
	{
		values= ARM_GP_Vector(1, theBetaCorrel);
    }
    else
    {
		values= ARM_GP_Vector(1, BETA_CORREL_DEFAULT_VALUE);
	}
	...
	{
		values= ARM_GP_Vector(1, theReCorrel);
    }
    else
    {
		values= ARM_GP_Vector(1, RECORREL_DEFAULT_VALUE);
	}
...
ARM_DateStripCombiner ARM_CallableSnowBallCalculator::InitDateStrip()
{
	...
	ARM_DateStrip ExerDateStrip(itsStartDate,itsEndDate, itsCpnFreq, itsCpnDayCount, itsCpnResetCal.c_str(), K_MOD_FOLLOWING, itsCpnIntRule,
					K_SHORTSTART, -fabs(itsCpnResetLag),itsCpnFreq,GETDEFAULTVALUE,itsCpnPayCal.c_str(),itsCpnIndexTiming, K_ARREARS);

	/// past  no call
	size_t nbPastNoCall=0;
	ARM_GP_Vector* dates = ExerDateStrip.GetResetDates() ;
	...
	ARM_DateStrip CpnDateStrip(itsStartDate,itsEndDate, itsCpnFreq, itsCpnDayCount, itsCpnResetCal.c_str(), K_MOD_FOLLOWING, 
				itsCpnIntRule,K_SHORTSTART, -fabs(itsCpnResetLag),itsCpnFreq,GETDEFAULTVALUE,itsCpnPayCal.c_str(),itsCpnIndexTiming, K_ARREARS);
	...
	ARM_GP_Vector* cpnResetDates = EventSchedule.GetDateStrip(CPN_SCHED)->GetResetDates();
	ARM_GP_Vector* exerDates = EventSchedule.GetDateStrip(EXER_SCHED)->GetResetDates();
	...
	itsIsCpnFlow = ARM_GP_T_Vector<bool>(cpnResetDates->size(),false);
	...
	itsConstVec = ARM_GP_Vector(itsResetDatesVec.size(),0.0);
	itsLevPrevVec = ARM_GP_Vector(itsResetDatesVec.size(),0.0);
	itsLevNewVec = ARM_GP_Vector(itsResetDatesVec.size(),0.0);
	itsStrikeOptVec = ARM_GP_Vector(itsResetDatesVec.size(),0.0);
	itsCpnMinVec = ARM_GP_Vector(itsResetDatesVec.size(),0.0);
	itsCpnMaxVec = ARM_GP_Vector(itsResetDatesVec.size(),0.0);
	itsvCpnNominal= ARM_GP_Vector(itsResetDatesVec.size(),0.0);
	itsStrikeSum = ARM_GP_Vector(itsResetDatesVec.size(),0.0);
	itsFeesVec = ARM_GP_Vector(exerDates->size(),0.0);

	ARM_GP_Vector* flowStartDates = CpnDateStrip.GetFlowStartDates();
	...
	ARM_GP_Vector* exerciseDates = ExerDateStrip.GetResetDates();
	for (i = 0; i < exerciseDates->size(); ++i)
	{
		itsFeesVec[i] = itsFeesProfile.Interpolate((*exerciseDates)[i]-asOfDate);
	}

	ARM_GP_Vector* fundResetDates = FundingSched.GetResetDates() ;
	double lag, margin;
	...
	if(itsCalibMode == CAPSWAPTION)
	{
		...
		if (!itsFixBeta || !itsFixBetaTimes.size() || !itsFixBetaValues.size())
		{
			if (!initBetaCorrel)
			{
				...
				ARM_GP_Vector initBetas(1,BETA_CORREL_DEFAULT_VALUE);
				ARM_GP_Vector betaLowerBound(1,BETA_CORREL_LOWER_BOUND);
				ARM_GP_Vector betaUpperBound(1,BETA_CORREL_UPPER_BOUND);
...
ARM_CalibMethod* ARM_CallableSnowBallCalculator::CreateEmptyCalibration( CalibMode calibMode, bool betaCalib)
{
	...
	if (betaCalib)
	{
		if (calibMode == SUMOPT )
		{
			...
			ARM_GP_Vector initTimes(1,itsResetDatesVec[0]-asOf);
			ARM_GP_Vector initBeta(1,BETA_DEFAULT_VALUE);
			ARM_GP_Vector BetaLB(1,BETA_LOWER_BOUND);
			ARM_GP_Vector BetaUB(1,BETA_UPPER_BOUND);
...
void ARM_CallableSnowBallCalculator::CreateAndSetDensityCalibration(ARM_StdPortfolioPtr pfPtr)
{
	...
	ARM_GP_Vector* pRD		= pDS->GetResetDates();	//not cloned, do not delete
	
	ARM_VanillaSecDensityPtrVector vanillaSecDensities;
	ARM_GP_Vector::iterator resetD	= pRD->begin();
	ARM_GP_Vector::iterator startD	= pDS->GetFlowStartDates()->begin();
	ARM_GP_Vector::iterator endD	= pDS->GetFlowEndDates()->begin();
...
{	
	...
	pDS = new ARM_DateStrip(Date1, Date2, maxFreq,
							...
							itsCpnIntRule, K_SHORTSTART, -fabs(itsCpnResetLag),
							maxFreq, GETDEFAULTVALUE, itsCpnPayCal.c_str(),
							K_ADVANCE, K_ARREARS);

	/// 
	double asOf = GetMktDataManager()->GetAsOfDate().GetJulian();
	ARM_GP_Vector* resetDates = pDS->GetResetDates();
	...
	ARM_GP_Vector* fwdEndDates    = pDS->GetFwdRateEndDates();
	ARM_GP_Vector* fwdStartDates  = pDS->GetFwdRateStartDates();
	ARM_GP_Vector* flowStartDates = pDS->GetFlowStartDates();
	ARM_GP_Vector* flowEndDates   = pDS->GetFlowEndDates();
	...
	if(isArrearsCpn){
		ARM_GP_Vector* resetDates    = pDS->GetResetDates();
		ARM_GP_Vector* eventDates    = itsDateStructure.GetDateStrip(EXER_SCHED)->GetResetDates();
...
void ARM_CallableSnowBallCalculator::ComputePortPrices(bool isInitParam, CalibMode calibMode, ARM_CalibMethod* calibMethod)
{
	...
	ARM_GP_Vector initTimes(nbProducts);
	ARM_GP_Vector initSigmas(nbProducts);
	ARM_GP_Vector sigmaLB(nbProducts);
	ARM_GP_Vector sigmaUB(nbProducts);
...
ARM_PricingModel* ARM_CallableSnowBallCalculator::BuildModel_HK(ARM_ZeroCurve* pCurve)
{
	...
	ARM_GP_Vector defaultSigmas(1,0.01);
...
{	
	...
	ARM_GP_Vector* startDates = dateStrip.GetFwdRateStartDates();
...
ARM_StdPortfolioPtr ARM_CallableSnowBallCalculator::CreateSwoptPortfolio()
{
	...
	ARM_GP_Vector* startDates = swoptDateStrip.GetFwdRateStartDates();
	int nbProducts = startDates->size();
...
ARM_RowInfo ARM_CallableSnowBallCalculator::MiddleRows( size_t eventIdx, const ARM_DateStripCombiner& datesStructure ) const
{
	...
	ARM_GP_Vector* cpnResetDates = datesStructure.GetDateStrip(CPN_SCHED)->GetResetDates();

	ARM_GP_Vector* cpnStartDates = datesStructure.GetDateStrip(CPN_SCHED)->GetFlowStartDates();
	ARM_GP_Vector* cpnEndDates = datesStructure.GetDateStrip(CPN_SCHED)->GetFlowEndDates();
	ARM_GP_Vector* cpnFwdStartDates = datesStructure.GetDateStrip(CPN_SCHED)->GetFwdRateStartDates();
	ARM_GP_Vector* cpnFwdEndDates = datesStructure.GetDateStrip(CPN_SCHED)->GetFwdRateEndDates();
	ARM_GP_Vector* cpnPayDates = datesStructure.GetDateStrip(CPN_SCHED)->GetPaymentDates();
	ARM_GP_Vector* cpnIT = datesStructure.GetDateStrip(CPN_SCHED)->GetInterestTerms();

	ARM_GP_Vector* fundingStartDates = datesStructure.GetDateStrip(EXER_SCHED)->GetFlowStartDates();
	ARM_GP_Vector* fundingEndDates = datesStructure.GetDateStrip(EXER_SCHED)->GetFlowEndDates();

	ARM_GP_Vector* exerDates = datesStructure.GetDateStrip(EXER_SCHED)->GetResetDates();
	...
	if (itsSBPos2Rank[eventIdx] != ARM_DateStripCombiner::DateStripCombiner_BlankData)
    {
		...
		const_cast< ARM_GP_Vector& >(itsStrikeSum)[itsSBPos2Rank[eventIdx]] = itsStrikeOptVec[itsSBPos2Rank[eventIdx]]*itsLevNewVec[itsSBPos2Rank[eventIdx]]-itsCapOrFloor*itsConstVec[itsSBPos2Rank[eventIdx]];
		if ((itsSBPos2Rank[eventIdx]>0))
		{
			const_cast< ARM_GP_Vector& >(itsStrikeSum)[itsSBPos2Rank[eventIdx]]+=itsStrikeSum[itsSBPos2Rank[eventIdx]-1]*itsLevPrevVec[itsSBPos2Rank[eventIdx]];
		}

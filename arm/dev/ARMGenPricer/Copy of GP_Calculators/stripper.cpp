...
#include "gpcalculators/stripper.h"
...
{
	ARM_GP_Vector* startDates = itsDatestrip->GetFlowStartDates();
	int size = startDates->size(); 
...
{	
	...
	ARM_GP_Vector* interestTerm = itsDatestrip->GetInterestTerms();
	ARM_GP_Vector* endDates = itsDatestrip->GetFlowEndDates();
...
///	Action : 
////////////////////////////////////////////////////
double ARM_STRIPPER::swapRate (double startFloatTime,double endFloatTime, const ARM_GP_Vector& FixPayTimes, const ARM_GP_Vector& FixPayPeriods) const
{
...
///	Action : 
////////////////////////////////////////////////////
double ARM_STRIPPER::stdFundingLeg (const ARM_GP_Vector& startFlowTimes, const ARM_GP_Vector& endFlowTimes,  const ARM_GP_Vector& periods, const ARM_GP_Vector& Notional, const ARM_GP_Vector& Margin, ARM_GP_Vector* LiborLeverage ) const
{
	int sizeFund = startFlowTimes.size();
	double floatPV = 0.0;
	for(int i=0; i<sizeFund;i++)
	{	
		double mult = LiborLeverage ? LiborLeverage->Elt(i) : 1.0;

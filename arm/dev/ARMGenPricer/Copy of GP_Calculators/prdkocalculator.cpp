...

{
	...
	/// PRDC period managing by switch date
	while(itsNbCallable < itsStructDateStrip->GetFlowEndDates()->size() && 
		(*itsStructDateStrip->GetFlowEndDates())[itsNbCallable] < switchDate.GetJulian()+ ARM_GlobalConstant::ARM_SEVENDAYS_LAG )
		itsNbCallable++;
	/// convert barrier from curve to vector
	ComputeProductVectorsFromCurves(barrier);
...
void ARM_PRDKOCalculator::ComputeProductVectorsFromCurves(const ARM_Curve& barrier)
{
	...
	for (i=0; i<itsCpnSize; i++)
	{
		...
		if(i<itsNbCallable) itsvCpnIsCallable[i] = true;
...
ARM_StdPortfolioPtr ARM_PRDKOCalculator::CreateFxOption()
{
    ...
    else
    {
        ...
        for(size_t exerIdx=0; exerIdx<itsExerSize; ++exerIdx)
        {
            ...
            {
				if(itsvCpnIsCallable[exerIdx] && !itsvCpnIsFixed [exerIdx]){
				...
				else if(itsvCpnIsCallable[exerIdx] && itsvCpnIsFixed [exerIdx]){
					continue;
				}
					...
					fxResetDate = ARM_Date((*(itsExerciseDateStrip->GetResetDates()))[itsvCpnIndex[exerIdx]]);
				}

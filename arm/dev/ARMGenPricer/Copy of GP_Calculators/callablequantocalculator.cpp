                                          ...
                                          const ARM_DateStripCombiner& datesStructure) const
{
	try
	{
		...
		ARM_GP_Vector* exerciseDates = datesStructure.GetDateStrip(EXER_SCHED)->GetResetDates();
...
void ARM_CallableQuantoCalculator::CreateAndSetModel()
{
	...
	ARM_Local_Normal_ModelParams* defaultLocalParams = static_cast< ARM_Local_Normal_ModelParams* >( ARM_Local_Normal_Model::CreateDefaultModelParams() );
	CC_NS(std,auto_ptr)<ARM_Local_Normal_ModelParams> holdLocalParams(defaultLocalParams);
	models[ARM_HWHWQtoModel::ForLocalModel] = ARM_PricingModelPtr( new ARM_Local_Normal_Model(CreateClonedPtr(ycForCurve), *defaultLocalParams) );

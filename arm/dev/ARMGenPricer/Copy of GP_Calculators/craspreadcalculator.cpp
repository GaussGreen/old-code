...
ARM_CRASpreadCalculator::ARM_CRASpreadCalculator( 
		...
		const vector<bool>& productsToPrice,
		const vector<bool>& localCalibFlags,
		double fundLeverage,
...
/////////////////////////////////////////////////////////////////////////////////
ARM_CRASpreadCalculator::InitCRASpreadFromSummit(ARM_ZeroCurve* zc,
												 ARM_VolCurve* capVol,
												 ARM_VolCurve* rhoCap,
												 ARM_VolCurve* nuCap,
...
void ARM_CRASpreadCalculator::Init(void)
{
	...
	if(GetKeys().size() != 0)
	{
		...
		CreateAndSetDealDescriptionAndTimeIt(ccyStr, pricedColumns, cstManagerPtr,false,isOtherPayoff);
	...
	try
	{
		...
		else
		{
			...
			if (IsTripleRange())
			{
				...
				cstNames.push_back("BarrierDown2Cst");
				cstNames.push_back("BarrierDown3Cst");
				cstNames.push_back("BarrierUp2Cst");
				cstNames.push_back("BarrierUp3Cst");
			}
		}
...
void ARM_CRASpreadCalculator::CreateCSOPortfolio_FakeForDateStripOnly()
{
	...
	// DATESTRIP fixings
		...
		startTimesForFix[i]		= spreadOption.GetSpreadLeg()->GetFirstLeg()->GetFwdRateStartDates()->Elt(i);

		//récupéré par le gram functor pour first index = LIBOR
		endTimesForFix[i]		= spreadOption.GetSpreadLeg()->GetFirstLeg()->GetFwdRateEndDates()->Elt(i);
		...
		//récupéré par le gram functor, et utilisé par le modèle local
		start					= spreadOption.GetSpreadLeg()->GetFirstLeg()->GetFlowStartDates()->Elt(i);
		end						= spreadOption.GetSpreadLeg()->GetFirstLeg()->GetFlowEndDates()->Elt(i);
                                          ...
                                          const ARM_DateStripCombiner& datesStructure) const
{
	try
	{
		...
		if(IsVms())
		{
			...
			int vmsIndex = (int) const_cast< ARM_ReferenceValue& >(itsTenor).Interpolate(sd);
			int intVmsType;
			...
			corridorLeg1Desc << "StartDate[i]" << ",";				// StartDate
			corridorLeg1Desc << "MaturityDate[i]" << ",";			// EndDate
			...
			corridorLeg1Desc << "BarrierDownCst" << ",";			// Barrier Down
			corridorLeg1Desc << "BarrierUpCst" << ",";				// Barrier Up
		...
		else
		{
			...
			corridorLeg1Desc << cpnTerm1 << ",";					// Fixing Index Term 1
			corridorLeg1Desc << "Coef1Cst" << ",";					// Coeff 1
			if(IsDbleCorridor())
			{
		...
		if (IsTripleRange())
		{
			...
			corridorLeg2Desc << "StartDate[i]" << ",";				// StartDate
			corridorLeg2Desc << "MaturityDate[i]" << ",";			// EndDate
			...
			{
				corridorLeg2Desc << "BarrierDown2Cst" << ",";		// Barrier Down
				corridorLeg2Desc << "BarrierUp2Cst" << ",";			// Barrier Up
			}
			...
			corridorLeg3Desc << "StartDate[i]" << ",";				// StartDate
			corridorLeg3Desc << "MaturityDate[i]" << ",";			// EndDate
			...
			{
				corridorLeg3Desc << "BarrierDown3Cst" << ",";		// Barrier Down
				corridorLeg3Desc << "BarrierUp3Cst" << ",";			// Barrier Up
			}
		...
		if(itsIsExerciseProbas && itsExerciseProbaOffset <= eventIdx &&
			eventIdx+1 <= itsExerciseProbaOffset + nbProbas)
		{
			//NEXT NOTICE DATE
			...
			if(itsExerciseProbaOffset == eventIdx)
			{
				...
				for(pIdx=0;pIdx<nbProbas;++pIdx,probaOffset+=2)
				{
					...
					probaDesc << LocalCapFloorCRALocalExerciseProbaNamesTable[pIdx] << "[i]*DF("
							  << ccy << "," << LocalCapFloorCRAColNamesTable[localCfMaturityDate] << "[i])";
			...
			/// LOCAL PROBABILITY
			probaOffset = localCfLocalProba1+2*(eventIdx-itsExerciseProbaOffset);
			for(pIdx=eventIdx-itsExerciseProbaOffset;pIdx < nbProbas;++pIdx,probaOffset+=2)
			{
				...
				else
				{
					localProbaDesc	<< "MAX(" << LocalCapFloorCRAColNamesTable[localCfIndicExer] << "[i]"
									...
									<< ccy << "," << LocalCapFloorCRAColNamesTable[localCfNextResetDate] << "[i]))";
				}

...
public:
	...
	ARM_CRASpreadCalculator( 
		...
		const vector<bool>& productsToPrice = vector<bool>(),
		const vector<bool>& localCalibFlags = vector<bool>(5,false),
		double fundLeverage=1.0,
		bool isExerciseProbas=false,
		size_t exerciseProbaOffset=0,
		const ARM_Currency& fundccy = ARM_Currency(),
	...
	InitCRASpreadFromSummit(ARM_ZeroCurve* zc,
							ARM_VolCurve* capVol,
							ARM_VolCurve* rhoCap,
							ARM_VolCurve* nuCap,

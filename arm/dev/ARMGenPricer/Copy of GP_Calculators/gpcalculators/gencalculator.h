 ...
 *
 *  \brief base class for all the calculators
...
public:
    ...
    /// to test if basis
    inline bool IsBasis() const {	return ( string(itsFundingCcy.GetCcyName()) != string(itsCurrency->GetCcyName()) );}
	...
	/// Vector
	inline virtual ARM_GP_Vector GetvCpnNominal() const  = 0;
	inline virtual ARM_GP_Vector GetvFundNominal() const = 0;
	inline virtual ARM_GP_Vector GetvFundSpread() const = 0;
	...
	ARM_GP_Vector ComputeDomesticBasis() const ;
    ...
    ARM_FixingSched* GetFixings(void)
    {
        return(itsLiborFxFixings);
    }

    void SetFixings(ARM_FixingSched* theFixings)
    {
        if (itsLiborFxFixings)
        {
        ...
        if (theFixings)
           itsLiborFxFixings = (ARM_FixingSched *) theFixings->Clone();
    }

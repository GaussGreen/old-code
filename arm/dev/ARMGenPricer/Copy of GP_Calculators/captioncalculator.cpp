	...
	itsNbdDeedResets(0)
{
	...
	ARM_GP_Vector* resetDates = itsPrincipalDateStrip->GetResetDates();
...
: ARM_GenCalculator(rhs),
	...
	itsCapPrice(ARM_VectorPtr((rhs.itsCapPrice != ARM_VectorPtr(NULL)) ? (ARM_GP_Vector*) rhs.itsCapPrice->Clone() : NULL)),
	itsCouponLegPrice(ARM_VectorPtr((rhs.itsCouponLegPrice != ARM_VectorPtr(NULL)) ? (ARM_GP_Vector*) rhs.itsCouponLegPrice->Clone() : NULL)),
	itsFundingLegPrice(ARM_VectorPtr((rhs.itsFundingLegPrice != ARM_VectorPtr(NULL)) ? (ARM_GP_Vector*) rhs.itsFundingLegPrice->Clone() : NULL)),
	itsCaptionStrikes(ARM_VectorPtr((rhs.itsCaptionStrikes != ARM_VectorPtr(NULL)) ? (ARM_GP_Vector*) rhs.itsCaptionStrikes->Clone() : NULL)),
...
{
	ARM_GP_Vector Abscisses = const_cast< ARM_Curve& >(exerciseProfile).GetAbscisses();
	ARM_GP_Vector Ordinates = const_cast< ARM_Curve& >(exerciseProfile).GetOrdinates();
	int size = Abscisses.size();
	int newSize = size+2;
	ARM_GP_Vector newAbscisses(newSize); 
	ARM_GP_Vector newOrdinates(newSize);
...
void ARM_CaptionCalculator::GenerateCapStrikes(void)
{
	...
	ARM_GP_Vector CouponAbscisses = const_cast< ARM_Curve& >(itsStrike).GetAbscisses();
	int size = CouponAbscisses.size();

	ARM_GP_Vector CouponOrdinates(size);
...
void ARM_CaptionCalculator::GenerateEquivalentFundSpreads(void)
{
	if(itsFundFreq>itsCpnFreq)
	{
		...
		ARM_GP_Vector* fundEndDates = FundDateStrip.GetFlowEndDates();
		ARM_GP_Vector* resetDates = FundDateStrip.GetResetDates();
		int newSize = resetDates->size();
		
		ARM_GP_Vector SpreadAbscisses(newSize);
		ARM_GP_Vector spreadOrdinates(newSize);

		//transfor fundSpread to refvalue
		ARM_GP_Vector FundSpreadAbscisses = const_cast< ARM_Curve& >(itsFundSpread).GetAbscisses();
		ARM_GP_Vector FundSpreadOrdinates = const_cast< ARM_Curve& >(itsFundSpread).GetOrdinates();
		...
		ARM_GP_Vector notionalAbscisses = const_cast< ARM_Curve& >(itsNotional).GetAbscisses();		
		ARM_GP_Vector notionalOrdinates = const_cast< ARM_Curve& >(itsNotional).GetOrdinates();
...
{
    ARM_GP_Vector* resetDates = itsCalibPorfolioDateStrip->GetResetDates();
	ARM_GP_Vector* startDates = itsCalibPorfolioDateStrip->GetFlowStartDates();
...
void ARM_CaptionCalculator::CreateDiagonalSwaption()
{
	...
	ARM_GP_Vector equivStrikes(portfolioSize);
	...
	bool computeSwaptionPrice)
{
	...
	ARM_GP_Vector initTimes(nbCapFloor);
	ARM_GP_Vector initSigmas(nbCapFloor);
	...
	ARM_GP_Vector sigmaLowerBound(nbCapFloor,SIGMA_LOWER_BOUND);
    ARM_GP_Vector sigmaUpperBound(nbCapFloor,SIGMA_UPPER_BOUND);
...
void ARM_CaptionCalculator::ComputeDiagonalSwaptionPrice(bool isFreezeWeights, bool isArrearCpn)
{
	...
	if(itsSwoptCalibMode==SWOPT_At_Exer) 
	{
		...
		ARM_GP_Vector* NotifDates = itsPrincipalDateStrip->GetResetDates();
...
void ARM_CaptionCalculator::ComputeEquivalentSwoptStrikes(ARM_GP_Vector& equivStrikes)
{
	string fundingCcyName(itsFundingCcy.GetCcyName());
    string basisCcyName(itsBasisCcy.GetCcyName());
	...
	ARM_GP_Vector* resetDates = itsCalibPorfolioDateStrip->GetResetDates();
	ARM_GP_Vector* startDates = itsCalibPorfolioDateStrip->GetFlowStartDates();
	ARM_GP_Vector* payDates = itsCalibPorfolioDateStrip->GetPaymentDates();

	ARM_GP_Vector* fundStartDates = itsFundDateStrip->GetFlowStartDates();
	ARM_GP_Vector* fundEndDates = itsFundDateStrip->GetFlowEndDates();
	ARM_INDEX_TYPE fundLiborType = GetFundIndexType();


	//transform CouponSpread in refvalue
	ARM_GP_Vector CouponAbscisses = const_cast< ARM_Curve& >(itsCoupon).GetAbscisses();
	ARM_GP_Vector CouponOrdinates = const_cast< ARM_Curve& >(itsCoupon).GetOrdinates();
    ...
    for (size_t i = 0; i < NbProductsToPrice; ++i)
	{
		if (itsProductsToPrice[i])
		{
			...
			else if(i == CapPrice)
			{
				...
				ARM_VectorPtr  newItermPrices(new ARM_GP_Vector(itermPrices->size()));

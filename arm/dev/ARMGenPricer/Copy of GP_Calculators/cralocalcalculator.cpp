...
itsExerciseProbas(ARM_GP_Vector(1,0.0))
{
}
...
///          called from a CCSO
/////////////////////////////////////////////////////////////////
ARM_GP_Vector* ARM_CRALocalCalculator::ComputeExerciseDatesAndSetFees() const
{
	try
	{
...
void ARM_CRALocalCalculator::CreateAndSetLocalModel()
{
	try
	{
		...
		fwdAdjValue[0] = 1.0;
		ARM_GP_T_Vector<double> vecFwdAdjCall(fwdAdjCall);
		ARM_GP_T_Vector<double> vecFwdAdjFix(fwdAdjFix);
		...
		volFix[0] = 0.0;
		volValue[0] = 0.2;
		ARM_GP_T_Vector<double> vecVolCall(volCall);
		ARM_GP_T_Vector<double> vecVolFix(volFix);
		...
		shiftValue[0] = 0.0;
		ARM_GP_T_Vector<double> vecShiftCall(shiftCall);
		ARM_GP_T_Vector<double> vecShiftFix(shiftFix);
...
void ARM_CRALocalCalculator::CalibrateLocalModel()
{
	try
	{
		...
		ARM_GP_Vector evalTimes (portfolioSize);
		...
		if (itsLocalModelType == LocalDownUp)
		{
			...
			for (size_t i(0); i < NbProductsToPrice; ++i)
			{
				if (itsProductsToPrice[i])
				{
					...
					{
						itsCorridorLegPrices = ARM_GP_Vector( * (genPricer->GetPricerInfo()->GetContents( LocalCapFloorCRAColNamesTable[ LocalCapFloorCRAProductToPriceColumns[i] ] ).GetData("IntermediatePrices").GetVector()) );
					}
				}
			}
			if(itsIsExerciseProbas)
...
void ARM_CRALocalCalculator::ComputePricingData() const
{
	...
	GetPricingData()[ "CorridorLeg" ]		= ARM_VectorPtr(new ARM_GP_Vector(itsCorridorLegPrices));
	
	/// Exercice probabilities
	if (itsIsExerciseProbas)
		GetPricingData()[ "ExerciseProbas" ] = ARM_VectorPtr(new ARM_GP_Vector(itsExerciseProbas));

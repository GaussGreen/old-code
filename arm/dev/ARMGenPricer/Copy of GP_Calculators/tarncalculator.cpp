...
ARM_TARNCalculator::ARM_TARNCalculator(const ARM_Date& startDate,
	...
	const ARM_GP_Vector& nbIterations,
...
ARM_TARNCalculator::ARM_TARNCalculator(const ARM_Date& asOfDate,
	...
	ARM_GP_Vector& customResetDates
...
*/
    ...
    if ( modelType == ARM_PricingModelType::SBGM )
    {
        ...
        if ( theHump != -10000.0 )
        {
           ARM_GP_Vector       values(1, theHump);
	       ARM_GP_Vector       times (1, 0.0);
        ...
        else
        {
           ...
           {
	          ARM_GP_Vector       values(1, HUMP_DEFAULT_VALUE);
	          ARM_GP_Vector       times (1, 0.0);
        ...
        {
	       ARM_GP_Vector       values(1, theBetaCorrel);
	       ARM_GP_Vector       times (1, 0.0);
        ...
        else
        {
            ...
            {
	            ARM_GP_Vector       values(1, BETACORREL_DEFAULT_VALUE);
	            ARM_GP_Vector       times (1, 0.0);
...
ARM_DateStripCombiner ARM_TARNCalculator::DatesStructure() const
{
	...
	if (itsCpnTiming == K_ARREARS)
	{
		...
		firstFundingResetDate.PreviousBusinessDay(fabs(itsCpnResetGap),ccyName);
	...
	if (GetCustomResetFlag() == true)
	{
		...
		if (itsCpnTiming == K_ARREARS)
		{
			ARM_GP_Vector custSmallResetDates = GetCustomResetDates();
			ARM_GP_Vector currResetDates = *(RFLegSched.GetResetDates());
			
			ARM_GP_Vector customResetDates = ARM_GP_Vector(custSmallResetDates.size()+1);
			...
			ARM_GP_Vector customFwdStartDates = customResetDates;
			ARM_GP_Vector customFwdEndDates = ARM_GP_Vector(customResetDates.size());
		...
		else
		{
			...
			ARM_GP_Vector customResetDates = GetCustomResetDates();
	...
	ARM_GP_Vector* rfResetDates = EventSchedule.GetDateStrip(RF_CPN_SCHED)->GetResetDates();
...
///	Action : 
////////////////////////////////////////////////////
double ARM_TARNCalculator::FindValueInVector( const ARM_GP_Vector& timeVector, double time, double daysNb ) const
{
	bool found = false;
...
///	Action : 
////////////////////////////////////////////////////
bool ARM_TARNCalculator::FindIdxInVector( const ARM_GP_Vector& timeVector, double time, double daysNb, size_t& idx) const
{
	bool found = false;
...
void ARM_TARNCalculator::CreateAndSetDensityCalibration(ARM_StdPortfolioPtr pfPtr)
{
	...
	ARM_GP_Vector::iterator startD	= pDS->GetFwdRateStartDates()->begin();
	ARM_GP_Vector::iterator endD	= pDS->GetFwdRateEndDates()->begin();
	ARM_GP_Vector::iterator strike	= pStrike->begin();
...
///	Action : 
////////////////////////////////////////////////////
ARM_DateStrip* ARM_TARNCalculator::GetCalibSchedule(ARM_GP_Vector*& pStrike) const
{
	bool isArrearsCpn					= (itsCpnTiming == K_ARREARS);
	...
	ARM_GP_Vector julianResetDates(*pDS->GetResetDates());
	ARM_GP_Vector julianStartDates(*pDS->GetFlowStartDates());
	ARM_GP_Vector julianEndDates(*pDS->GetFlowEndDates());
	ARM_GP_Vector julianFwdStartDates(*pDS->GetFwdRateStartDates());
	ARM_GP_Vector julianFwdEndDates(*pDS->GetFwdRateEndDates());

	vector<int> vFreq(size,-1);
	pStrike = new ARM_GP_Vector(size,-1.);
	...
	idx = 0;
	for (ARM_GP_Vector::iterator iter = julianResetDates.begin() ; iter != julianResetDates.end() ; ++iter,++idx)
	{
...
/////////////////////////////////////////////////////////////////
void ReplaceSwaptionDates(ARM_Vector* swaptionResetDates, const ARM_GP_Vector* modelDates)
{
	size_t i;
...
ARM_StdPortfolioPtr ARM_TARNCalculator::CreateDiagonalSwaption()
{
	...
	ARM_GP_Vector* modelDates = dateStructure.GetDateStrip(0)->GetResetDates();
...
ARM_StdPortfolioPtrVector ARM_TARNCalculator::CreateTARNCapletFloorletAndDigital()
{
	...
	ARM_GP_Vector initBreakpointTimes;
	ARM_GP_Vector initVolatility;
	ARM_GP_Vector initBeta;
...
void ARM_TARNCalculator::ComputeEquivalentDatas(const ARM_DealDescription& dealDesc,
    ...
    ARM_Swaption* swaption, const string& evalDateStr, ARM_GP_Vector& equivDatas)
{
...
void ARM_TARNCalculator::Calibrate_SBGM()
{
	...
	if(itsOSWCalibFlag)
	{
		...
		ARM_GP_Vector initTimes(1,0.);
		ARM_GP_Vector initBetas(1,0.1);
		ARM_GP_Vector betaLowerBound(1,0.);
		ARM_GP_Vector betaUpperBound(1,4.);
...
double ARM_TARNCalculator::Price()
{
	...
	if( itsControlVariableFlag )
	{
		...
		ARM_GP_Vector columnPrices( 2 );
		ComputeControlVariable( columnPrices );
		itsCVRefPrices = ARM_GP_VectorPtr( new ARM_GP_Vector(columnPrices) );
			...
			if (itsProductsToPrice[i])
			{
				...
				else if(i == ExerciseStrikesPrice || i == ExerciseProbasPrice)
				{
					...
					if( newItermPricesSize && isArrearsCpn ) 
						--newItermPricesSize;
					ARM_VectorPtr  newItermPrices( new ARM_GP_Vector(newItermPricesSize ) );
	...
	bool computeSwaptionPrice)
{
    ...
    while (cfCalibMethod != NULL)
    {
		...
		if (isInitParam)
		{
			...
			ARM_GP_Vector initTimes(1,expiry);
			ARM_GP_Vector initSigmas(1,vol/100.0);
			ARM_GP_Vector sigmaLowerBound(1,SIGMA_LOWER_BOUND);
			ARM_GP_Vector sigmaUpperBound(1,SIGMA_UPPER_BOUND);
			...
			/// Replace the sigma param with new initialisations
			ARM_GP_Vector initBetas(1,BETA_DEFAULT_VALUE);
			ARM_GP_Vector betaLowerBound(1,BETA_LOWER_BOUND);
			ARM_GP_Vector betaUpperBound(1,BETA_UPPER_BOUND);
...
void ARM_TARNCalculator::ComputeControlVariable(ARM_GP_Vector& cvprices)
{

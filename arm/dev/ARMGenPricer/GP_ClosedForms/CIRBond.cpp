...
ARM_CIRModelParams::ARM_CIRModelParams( const ARM_ModelParamVector & params ):ARM_ModelParams(params){
	...
	const ARM_GP_Vector& schedBt	= GetModelParam( ARM_ModelParamType::MeanReversion	).ToCurveModelParam().GetCurve()->GetAbscisses();
	if( schedBt.size() == 0)		ARMTHROW(ERR_INVALID_ARGUMENT, "MeanReversion Model Params should contaions at least one element" );
    const ARM_GP_Vector& schedCt	= GetModelParam( ARM_ModelParamType::LongTermVol	).ToCurveModelParam().GetCurve()->GetAbscisses();
...
Complex ARM_CIRModelParams::CptGlobal(	const	ARM_GP_Vector	& schedule,	
...
Complex	ARM_CIRModelParams::CptLaplace_Bond	(	const	ARM_GP_Vector	& schedule,	
												const	mapParamType	& mapParam,
												const	Complex			& x0,
												const	Complex			& x){
...
ARM_GP_Vector ARM_CIRModelParams::GenerateSchedule( const double & time) const{
...
struct ARM_CIR_InvLaplace:public ARM_GP::UnaryFunc<Complex,Complex>{
	...
	ARM_CIR_InvLaplace(		const ARM_GP_Vector	& schedule, 
							const mapParamType	& mapParam,
							const double		& x0):
...
double ARM_CIRModelParams::CptDensity_Bond	(	const	ARM_GP_Vector	& schedule,	
												const	mapParamType	& mapParam,
												const	double			& x0,
												const	double			& x){
...
public: 
	InvLaplace(		const ARM_GP_Vector	&	schedule,	
					const mapParamType	&	mapParam, 
					const double		&	x0,
					const double		&	target):
...
DistribSet	ARM_CIRModelParams::CptDistribSet(const double & time,	const double & x0, const int& nbDisc, const double & factor ) { 
	itsLaplaceParam.isOptimized		= false;
	ARM_GP_Vector		schedule	= GenerateSchedule(time);
	...
	itsDistribSet.itsWeight		= ARM_GP_Vector(nbDisc);
	itsDistribSet.itsPoint		= ARM_GP_Vector(nbDisc);
	itsDistribSet.itsDensity	= ARM_GP_Vector(nbDisc);

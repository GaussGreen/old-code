...
protected:
	...
	ARM_GP_Vector		m_resets;
	ARM_GP_Vector		m_forwards;

	ARM_VectorVector	m_mktvols;
	ARM_VectorVector	m_strikes;
	ARM_GP_Vector		m_weights;
	
	bool				m_calibConstraint;
	ARM_GP_Vector		m_constraintStrikes;
	ARM_GP_Vector		m_constraintVols;
...
public:
	...
	void	Init(double reset, double forward, 
				 const ARM_GP_Vector& mktvols, const ARM_GP_Vector& strikes,
				 bool calibConstraint, double constraintK, double constraintVol,
				 ARM_SmileCalibration_Params * params);

	void	Init(const ARM_GP_Vector& resets, const ARM_GP_Vector& forwards,
				 const ARM_VectorVector& mktvols, const ARM_VectorVector& strikes,
				 bool calibConstraint, const ARM_GP_Vector& constraintStrikes, const ARM_GP_Vector& constraintVols,
...
protected:
	...
	virtual void	InitfGuess(ARM_GP_Vector& fguess) = 0;
...
protected:
	...
	void AdjustVolToBP()
	{
		for(int k = 0; k < m_mktvols.size(); k++) (*m_mktvols[k]) /= VolToBP;

		m_constraintVols /= VolToBP;
	}
...
protected:
	...
	ARM_GP_Vector	m_shifts;
	ARM_GP_Vector	m_rhos;
...
public:
	...
	const ARM_GP_Vector&	shifts() const	{return m_shifts;};
	const ARM_GP_Vector&	levels() const	{return m_levels;};
	const ARM_GP_Vector&	rhos() const	{return m_rhos;};
...
protected:
	...
	void	InitfGuess(ARM_GP_Vector& fguess);
};
...
public:
	...
	{
		build(v0, kappa, theta, rho, nu, shift, level, ARM_GP_Vector(1,0.), calibkappa, calibtheta, calibrho, calibnu, calibshift, calibeachrho, bootstraplevel, normalHeston);
	}

	ARM_SmileCalibration_Params_Heston(double v0, double kappa, double theta, double rho, double nu, double shift, double level, const ARM_GP_Vector& sigmas,
		bool calibkappa, bool calibtheta, bool calibrho, bool calibnu, bool calibshift, bool calibeachrho = false, bool bootstraplevel = false, bool normalHeston = false) 
	{
...
public:
	...
	{
		build(v0, kappa, theta, rho, nu, shift, level, ARM_GP_Vector(1,0.), calibkappa, calibtheta, calibrho, calibnu,
			calibshift, calibeachrho, bootstraplevel, normalHeston);
	}
					
	void		build(double v0, double kappa, double theta, double rho, double nu, double shift, double level,
					const ARM_GP_Vector& sigmas,
					bool calibkappa, bool calibtheta, bool calibrho, bool calibnu, bool calibshift, 
					bool calibeachrho = false, bool bootstraplevel = false, bool normalHeston = false)
	{
		m_v0				= v0;
...
protected:
	...
	ARM_GP_Vector	m_shifts;
	ARM_GP_Vector	m_levels;
	ARM_GP_Vector	m_rhos1;
	ARM_GP_Vector	m_rhos2;
...
public:
	...
	const ARM_GP_Vector&	levels() const	{return m_levels;};
	const ARM_GP_Vector&	shifts() const	{return m_shifts;};
	const ARM_GP_Vector&	rhos1() const	{return m_rhos1;};
	const ARM_GP_Vector&	rhos2() const	{return m_rhos2;};
...
private:
	...
	ARM_GP_Vector	m_mktvols1;
	ARM_GP_Vector	m_strikes1;
	double			m_constrvol1;
	double			m_constrK1;
	ARM_GP_Vector	m_mktvols2;
	ARM_GP_Vector	m_strikes2;
	double			m_constrvol2;
	double			m_constrK2;

	ARM_GP_Vector	m_mktvolspread;
	ARM_GP_Vector	m_strikespread;
...
public:
	...
	void	Init(double reset, 
				double fwd1, const ARM_GP_Vector& mktvols1, const ARM_GP_Vector& strikes1, double constrvol1, double constrK1, 
				double fwd2, const ARM_GP_Vector& mktvols2, const ARM_GP_Vector& strikes2, double constrvol2, double constrK2,
				const ARM_GP_Vector& mktvolspread, const ARM_GP_Vector& strikespread, double constrvolSpread, double constrKSpread,
...
protected:
	...
	virtual void	InitfGuess(ARM_GP_Vector& fguess) {};
...
public:
	...
	ARM_Spread2Heston_CAPCalibration_Params(const ARM_GP_Vector& fwdReset, const ARM_GP_Vector& fwd1, 
		const ARM_GP_Matrix& mktvols1, const ARM_GP_Matrix& strikes1, const ARM_GP_Vector& fwd2,
...
private:
	...
	ARM_GP_Vector		m_capprices;
	ARM_GP_Vector		m_capstrikes;
...
public:
	...
	void	Init(const ARM_GP_Vector& resets, const ARM_GP_Vector& dfs, const ARM_GP_Vector& fwd1, const ARM_GP_Vector& fwd2,
				 const ARM_GP_Vector& capprices, const ARM_GP_Vector& capstrikes, double constrCapPrice, double constrCapStrike,
...
void ARM_Spread2Heston_TOTEMCalibration(const ARM_GP_Vector& TOTEMMaturities, const ARM_GP_Vector& TOTEMStrikes,
										const ARM_GP_Vector& TOTEMPrices,
										const ARM_GP_Vector& FullSchedReset,
										const ARM_GP_Vector& FullSchedAnnuity,
										const ARM_GP_Vector& FullSchedFwd1, const ARM_GP_Vector& FullSchedFwd2,
										const ARM_GP_Vector& FwdCalibReset,
										const ARM_GP_Vector& LongFwds, const ARM_GP_Matrix& LongVols, const ARM_GP_Matrix& LongK,
										const ARM_GP_Vector& ShortFwds, const ARM_GP_Matrix& ShortVols, const ARM_GP_Matrix& ShortK,
										double v0, double kappa, double theta, bool locShifts,
										ARM_GP_Vector& Mat, ARM_GP_Vector& Theta, ARM_GP_Vector& Nu, 
										ARM_GP_Vector& LongRho, ARM_GP_Vector& LongShift, ARM_GP_Vector& LongLevel,
										ARM_GP_Vector& ShortRho, ARM_GP_Vector& ShortShift, ARM_GP_Vector& ShortLevel,
										ARM_GP_Vector& Correl);

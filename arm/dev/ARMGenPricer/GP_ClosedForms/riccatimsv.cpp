...
void ARM_Riccati::InitFunctionDatas()
{
    ...
    ARM_GP_Vector tmpSched(schedAt.size()+schedBt.size()),sched(tmpSched.size()+schedCt.size());
    CC_NS(std,merge)(schedAt.begin(),schedAt.end(),schedBt.begin(),schedBt.end(),tmpSched.begin());
    CC_NS(std,merge)(tmpSched.begin(),tmpSched.end(),schedCt.begin(),schedCt.end(),sched.begin());

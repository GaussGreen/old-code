...

{
	...
	ArgumentList a1(F_Vec->Elt(i-1),K,
...
void fillVector(int size, ARM_GP_Vector& gpvec, const vector<double>& vec)
{
	gpvec.resize(size);
...
double Export_Heston_OptionPrice(double resetTime, double forward, double strike, int callorput, double v0, double kappa, 
								 ...
								 const ARM_GP_Vector& times, const ARM_GP_Vector& levels)
{
...
double Export_Heston2b_OptionPrice(double resetTime, double forward, double strike, int callorput, 
								   ...
								   double shift, const ARM_GP_Vector& times, const ARM_GP_Vector& levels)
{

...
void ARM_SmileCalibration::Init(double reset, double forward, const ARM_GP_Vector& mktvols, const ARM_GP_Vector& strikes,
								...
								ARM_SmileCalibration_Params *  params)
{
	...
	m_mktvols.resize(1);
	m_mktvols[0] = new ARM_GP_Vector(mktvols.size());
	for(int k = 0; k < mktvols.size(); k++) (*(m_mktvols[0]))[k] = mktvols[k];

	m_strikes.resize(1);
	m_strikes[0] = new ARM_GP_Vector(strikes.size());
	for(k = 0; k < strikes.size(); k++) (*(m_strikes[0]))[k] = strikes[k];
...
void ARM_SmileCalibration::Init(const ARM_GP_Vector& resets, const ARM_GP_Vector& forwards,
...
bool ARM_SmileCalibration::Calibrate()
{
	...
	ARM_GP_Vector fguess, hx(size, 0.);
...
void ARM_SmileCalibration_SABR::InitfGuess(ARM_GP_Vector& fguess)
{
	if(m_params->m_levels.size() > 0)
	{
...
void ARM_SmileCalibration_Heston::InitfGuess(ARM_GP_Vector& fguess)
{
...
void ARM_SmileCalibration_NormalHeston::InitfGuess(ARM_GP_Vector& fguess)
{
	fguess.resize(m_params->m_calibkappa 
				+ m_params->m_calibtheta 
				+ (m_params->m_calibrho && m_params->m_calibeachrho == false)
...
void ARM_SmileCalibration_BiSABR::InitfGuess(ARM_GP_Vector& fguess)
{
...
void ARM_SmileCalibration_Heston2b::InitfGuess(ARM_GP_Vector& fguess)
{
...
void ARM_SmileCalibration_SABR2beta::InitfGuess(ARM_GP_Vector& fguess)
{
	fguess.resize(	m_params->m_calibalpha + 
...
void ARM_SmileCalibration_Merton::InitfGuess(ARM_GP_Vector& fguess)
{
...
void ARM_SmileCalibration_Spread2Heston::Init(double reset,
											  double fwd1, const ARM_GP_Vector& mktvols1, const ARM_GP_Vector& strikes1,
											  double constrvol1, double constrK1,
											  double fwd2, const ARM_GP_Vector& mktvols2, const ARM_GP_Vector& strikes2,
											  double constrvol2, double constrK2,
											  const ARM_GP_Vector& mktvolspread, const ARM_GP_Vector& strikespread, 
											  ...
											  ARM_SmileCalibration_Params_Spread2Heston * params)
{
	...
	m_constrK2		= constrK2;
	m_mktvolspread	= mktvolspread / VolToBP;
...
bool ARM_SmileCalibration_Spread2Heston::Calibrate()
{
	...
	ARM_GP_Vector fguess(2), hx(size, 0.);
...
ARM_Spread2Heston_CAPCalibration_Params::ARM_Spread2Heston_CAPCalibration_Params(const ARM_GP_Vector& fwdReset, 
	const ARM_GP_Vector& fwd1, const ARM_GP_Matrix& mktvols1, const ARM_GP_Matrix& strikes1, 
	const ARM_GP_Vector& fwd2, const ARM_GP_Matrix& mktvols2, const ARM_GP_Matrix& strikes2, 
...
void ARM_Spread2Heston_CAPCalibration_Params::Calibrate(double firstCapReset, double lastCapReset)
{
	...
	for(k = 0; k < size; k++)
	{
		...
		calibtool.Init(m_fwdReset[i], m_fwd1[i], (*m_mktvols1[i]), (*m_strikes1[i]), true, m_fwd1[i], m_atm1[i], &m_params1[i]);
		calibtool.Calibrate();
		...
		calibtool.Init(m_fwdReset[i], m_fwd2[i], (*m_mktvols2[i]), (*m_strikes2[i]), true, m_fwd2[i], m_atm2[i], &m_params2[i]);
		calibtool.Calibrate();
	}
}
...
void ARM_SmileCalibration_Spread2HestonCAP::Init(const ARM_GP_Vector& resets, const ARM_GP_Vector& dfs,
												 const ARM_GP_Vector& fwd1, const ARM_GP_Vector& fwd2,
												 const ARM_GP_Vector& capprices, const ARM_GP_Vector& capstrikes, 
										...
										ARM_GP_Vector& Correl)
{
	...
	ARM_GP_Vector currCalibPrices, currTOTEMStrikes, currTOTEMPrices, prevTOTEMStrikes(0), prevTOTEMPrices(0);
	ARM_GP_Vector currReset, currFwd1, currFwd2, currAnnuity;

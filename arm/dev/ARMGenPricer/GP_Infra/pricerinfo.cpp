...
void ARM_PInfo_SingleLoop::StoreCurrentLoopPrice( const ARM_PricingStatesPtr& states, size_t bucketSize, size_t currentBucketIndex, ARM_PricingAdviser::PricingDir pricingDir, int colIdx )
{
    ...
    int i;
	for (i = 0;i < nbIntermediatePayoffs;++i)
...
void ARM_PInfo_SingleLoop::PostProcess(const ARM_TimeInfoPtrVector& timeInfo,const const ARM_NumMethodPtr& numMethod)
{
	...
	ARM_GP_Vector eventTimes(timeInfo.size());
...
void ARM_PInfo_MultipleLoop::StoreCurrentLoopPrice( const ARM_PricingStatesPtr& states, size_t bucketSize, size_t currentBucketIndex, ARM_PricingAdviser::PricingDir pricingDir, int colIdx)
{
	...
	{	size_t totalSize = 0;
		...
		itsIntermediatePrices			= ARM_VectorPtr(new ARM_GP_Vector(nbIntermediatePayoffs));
		itsIntermediateStdDevs			= ARM_VectorPtr(new ARM_GP_Vector(nbIntermediatePayoffs));
		itsIntermediateEmpiricalStdDevs	= ARM_VectorPtr(new ARM_GP_Vector(nbIntermediatePayoffs));
		itsIntermediateRunningAverages	= ARM_GP_MatrixPtr(new ARM_GP_Matrix);

		if (otherPayoffFlag)
		{
			itsValues				= ARM_VectorPtr( new ARM_GP_Vector(totalSize,0.0) );
			if (ivFlag)
			{
	...
	ARM_GP_Vector curBucketValues(bucketSize);
	...
	for (i=0;i<nbIntermediatePayoffs;++i)
	{
		...
		itsInterMomentFunc[i]->addBucket(*tmpRow);
		delete tmpRow;
	}
}
...
void ARM_PInfo_MultipleLoop::PostProcess(const ARM_TimeInfoPtrVector& timeInfo,const ARM_NumMethodPtr& numMethod)
{
	...
	for (i=0;i<itsInterMomentFunc.size();++i)
...
////////////////////////////////////////////////////
ARM_PInfo_Cov::ARM_PInfo_Cov( ARM_PInfo_Map* pinfoMap, const string& itsRefColumn, const ARM_StringVector& itsCVColumns, const ARM_GP_Vector& CVPrices, const ARM_GP_Vector& userBetas )
:
	ARM_PricerInfo(),
...
////////////////////////////////////////////////////
void ARM_PInfo_Cov::ComputeBetas( ARM_PInfo_Map* pinfoMap, ARM_PInfo_MultipleLoop* pinfoMultiLoop, const ARM_GP_Vector& userBetas )
{
	if( userBetas.size() )
	{
#if defined(__GP_STRICT_VALIDATION)
...
////////////////////////////////////////////////////
void ARM_PInfo_Cov::PostProcessCV( ARM_PInfo_Map* pinfoMap, ARM_PInfo_MultipleLoop* pinfoMultiLoop, const ARM_GP_Vector& userBetas ) 
{
	...
	itsRunningAverage	= ARM_GP_VectorPtr( new ARM_GP_Vector( int( totalSize/itsRunningAvgStep ), 0.0 ) );

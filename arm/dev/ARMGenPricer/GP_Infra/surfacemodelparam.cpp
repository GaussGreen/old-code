...
void ARM_SurfaceModelParam::SetAndUpDate(ARM_GP_Vector* values)
{
	size_t k=0;
...
ARM_GP_Vector* ARM_SurfaceModelParam::GetData( ARM_DataType type, long& rows, long& cols ) const
{
	switch(type)
	{
		case ARM_ModelParamType::BreakPointTimes:
			{
				const ARM_GP_Vector& result = itsSurface->GetX1();
				rows=result.size();
				cols=1;
        ...
        case ARM_ModelParamType::Tenors:
            {
				const ARM_GP_Vector& result = itsSurface->GetX2();
				rows=result.size();
				cols=1;
				return new ARM_GP_Vector(result);
			}

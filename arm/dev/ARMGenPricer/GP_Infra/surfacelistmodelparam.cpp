...
void ARM_SurfaceListModelParam::SetAndUpDate(ARM_GP_Vector* values)
{
...
void ARM_SurfaceListModelParam::SetSurfaceListIndex( const ARM_GP_Vector& index )
{ 
...
ARM_GP_Vector* ARM_SurfaceListModelParam::GetData( ARM_DataType type, double index, long& rows, long& cols ) const
{
	size_t i = 0; 
	...
	switch(type)
        ...
        case ARM_ModelParamType::Tenors:
            {
				const ARM_GP_Vector& result = itsSurfaceList[i]->GetX2();
				rows=result.size();
				cols=1;
				return new ARM_GP_Vector(result);
			}
        case ARM_ModelParamType::Values:
			{
				...
				rows = values.rows();
				cols = values.cols();
				ARM_GP_Vector* result = new ARM_GP_Vector( values.size() );

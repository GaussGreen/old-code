...
public:
	...
	ARM_GP_Vector* GetData( ARM_DataType type, double index, long& rows, long& cols ) const;
	virtual void SetSurfaceList( const ARM_SurfacePtrVector& surfacelist );
	void SetSurfaceListIndex( const ARM_GP_Vector& index );

	 /// Replace old values using interpolate if necessairy
    void SetAndUpDate(ARM_GP_Vector* values);    

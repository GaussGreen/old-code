...
ARM_GP_Vector* MergeARM_Vector( const ARM_GP_Vector& vec1, const ARM_GP_Vector& vec2 );
...
{
	virtual ARM_GP_Vector* ModelTimesFromTimesInfo(const ARM_TimeInfoPtrVector& timeInfos, ARM_PricingModel& pricingModel) const = 0;
};

/// event time only inserts eventtime from time infos
struct ARM_EventTime: ARM_DiscretisationScheme
{
	virtual ARM_GP_Vector* ModelTimesFromTimesInfo(const ARM_TimeInfoPtrVector& timeInfos, ARM_PricingModel& pricingModel) const;
};

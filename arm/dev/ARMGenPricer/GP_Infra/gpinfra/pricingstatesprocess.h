...
struct AddToState0
{
	...
	void ProcessStates( ARM_PricingStatesPtr& currentStates, size_t colIdx, ARM_VectorPtr& payoffs, ARM_PricingModel* model, double eventTime, bool isAZero )
	{
		...
		if(isOtherPayoffs)
			currentStates->GetPayoffStates(colIdx).push_backPayoffSnapshot( ARM_VectorPtr( (ARM_GP_Vector*) payoffs->Clone() ) );

		// To prevent in place effect in the graph nodes
		ARM_VectorPtr newPayoffs( new ARM_GP_Vector( *payoffs ) );
...
struct CreateFirstPayoff
{
	...
	void ProcessStates( ARM_PricingStatesPtr& currentStates, size_t colIdx, ARM_VectorPtr& payoffs, ARM_PricingModel* model, double eventTime, bool isAZero )
	{
        ...
        /// Save a snapshot of current payoff states before any numeraire treatment
        if(isOtherPayoffs)
		    currentStates->GetPayoffStates(colIdx).push_backPayoffSnapshot(ARM_VectorPtr(new ARM_GP_Vector(*payoffs)));
    ...
    case GFAT_DOUBLE_TYPE:
		{
			if( fabs( payoff.GetDouble()  )< K_NEW_DOUBLE_TOL ) 
				isAZero = true;
			payoffVec = ARM_VectorPtr(new ARM_GP_Vector(states->size(),payoff.GetDouble()));
		}
		break;

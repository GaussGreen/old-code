	...
	ARM_PricingModel* model )
{
    ...
    if( !itsModelCalls.empty() )
    {
        ...
        for( pos = 0; pos < endPos; ++pos )
		{
            ...
            tmpResult.push_back( ARM_TimeInfoPtr( new ARM_TimeInfo( eventTime, ARM_VectorPtr( new ARM_GP_Vector( payTimes ) ),AdditionalTimeInfos ) ) );	
        }
	...
	ARM_VectorPtr tmpTimes( (ARM_GP_Vector*) TimeInfoEventTimes->Clone() );
	CC_NS( std, sort )( tmpTimes->begin(), tmpTimes->end() );
	ARM_GP_Vector::iterator times		= tmpTimes->begin();
	ARM_GP_Vector::iterator endTimes	= tmpTimes->end();
	...
	for( ; times != endTimes; ++times )
	{
			...
			&& (*modTimeInfos)->GetEventTime() == (*times) )
		{
			...
			result.push_back( ARM_TimeInfoPtr( new ARM_TimeInfo( *times, ARM_VectorPtr( new ARM_GP_Vector(1,*times) ) ) ) );
	}
	...
	case ARM_NumMethod::GP_FWDLOOKING: case ARM_NumMethod::GP_FWDBCKWDLOOKING: 
		{
			...
			ARM_GP_Vector tmpToTimes(realSize);
			ARM_GP_Vector tmpFromTimes(realSize);
			...
			itsToTimes		= ARM_VectorPtr( new ARM_GP_Vector( tmpToTimes ) );
			itsFromTimes	= ARM_VectorPtr( new ARM_GP_Vector( tmpFromTimes ) );

			break;
		}
	case ARM_NumMethod::GP_BCKWDLOOKING:
		{
			ARM_GP_Vector tmpToTimes(rowDateSize);
			ARM_GP_Vector tmpFromTimes(rowDateSize);

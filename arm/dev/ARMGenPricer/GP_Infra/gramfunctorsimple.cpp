...
void IfInPlace( ARM_VectorPtr& v1, const ARM_VectorPtr& v2, const ARM_VectorPtr& v3 )
{
	ARM_GP_Vector::iterator v1Elem		= (*v1).begin();
	ARM_GP_Vector::const_iterator v2Elem	= (*v2).begin();
	ARM_GP_Vector::const_iterator v3Elem	= (*v3).begin();
	ARM_GP_Vector::iterator end			= (*v1).end();
											...
											double evalDate, const ARM_PricingStatesPtr& states, vector< ARM_ExpNodePtr >& nodes )
{
	...
	{
		ARM_GP_VectorPtr newvec( new ARM_GP_Vector( *(arg[0].GetVector()) ) );
											 ...
											 double evalDate, const ARM_PricingStatesPtr& states, vector< ARM_ExpNodePtr >& nodes )
{
	...
	{	
		ARM_GP_VectorPtr newvec( ARM_GP_VectorPtr( new ARM_GP_Vector( *(arg[0].GetVector()) ) ) );
		...
		if( arg[0].GetType() == GFAT_DOUBLE_TYPE )
		{
			...
			{
				GPAF_CheckArgVecSameSize( arg, ARM_GP_IfVector::itsFuncName );
				ARM_VectorPtr vec( new ARM_GP_Vector( arg[0].GetVector()->size(), arg[1].GetDouble() ) );
				arg[1].SetVector( vec );
				ARM_GP_VectorPtr newvec( new ARM_GP_Vector( *(arg[0].GetVector()) ) );
				IfInPlace( newvec, arg[1].GetVector(), arg[2].GetVector() );
				return ARM_GramFctorArg( newvec );
				...
				{
					GPAF_CheckArgVecSameSize( arg, ARM_GP_IfVector::itsFuncName );
					ARM_VectorPtr vec( new ARM_GP_Vector( arg[0].GetVector()->size(), arg[2].GetDouble() ) );
					arg[2].SetVector( vec );
					ARM_GP_VectorPtr newvec( new ARM_GP_Vector( *(arg[0].GetVector()) ) );
					IfInPlace( newvec, arg[1].GetVector(), arg[2].GetVector() );
					return ARM_GramFctorArg( newvec );
					...
					if(	   arg[0].GetType() == GFAT_VECTOR_TYPE 
						...
						&& arg[2].GetType() == GFAT_DOUBLE_TYPE )
					{
						ARM_VectorPtr vec1( new ARM_GP_Vector( arg[0].GetVector()->size(), arg[1].GetDouble() ) );
						ARM_VectorPtr vec2( new ARM_GP_Vector( arg[0].GetVector()->size(), arg[2].GetDouble() ) );
					...
					////////////////////
						...
						{
							GPAF_CheckArgVecSameSize( arg, ARM_GP_IfVector::itsFuncName );
							ARM_VectorPtr vec( new ARM_GP_Vector( arg[0].GetVector()->size(), arg[1].GetDate().GetJulian() ) );
							arg[1].SetVector( vec );
							ARM_GP_VectorPtr newvec( new ARM_GP_Vector( *(arg[0].GetVector()) ) );
							IfInPlace( newvec, arg[1].GetVector(), arg[2].GetVector() );
							return ARM_GramFctorArg( newvec );
							...
							{
								GPAF_CheckArgVecSameSize( arg, ARM_GP_IfVector::itsFuncName );
								ARM_VectorPtr vec( new ARM_GP_Vector( arg[0].GetVector()->size(), arg[2].GetDate().GetJulian() ) );
								arg[2].SetVector( vec );
								ARM_GP_VectorPtr newvec( new ARM_GP_Vector( *(arg[0].GetVector()) ) );
								IfInPlace( newvec, arg[1].GetVector(), arg[2].GetVector() );
								return ARM_GramFctorArg( newvec );
								...
								if(	   arg[0].GetType() == GFAT_VECTOR_TYPE 
									...
									&& arg[2].GetType() == GFAT_DATE_TYPE )
								{
									ARM_VectorPtr vec1( new ARM_GP_Vector( arg[0].GetVector()->size(), arg[1].GetDate().GetJulian() ) );
									ARM_VectorPtr vec2( new ARM_GP_Vector( arg[0].GetVector()->size(), arg[2].GetDate().GetJulian() ) );
									arg[1].SetVector( vec1 );
									arg[2].SetVector( vec2);
									ARM_GP_VectorPtr newvec( new ARM_GP_Vector( *(arg[0].GetVector()) ) );
									IfInPlace( newvec, arg[1].GetVector(), arg[2].GetVector() );
													  ...
													  double evalDate, const ARM_PricingStatesPtr& states, vector< ARM_ExpNodePtr >& nodes )
{
		...
		&& arg[1].GetType() == GFAT_VECTOR_TYPE )
	{
		...
		if(    arg[0].GetType() == GFAT_DOUBLE_TYPE 
			&& arg[1].GetType() == GFAT_VECTOR_TYPE )
		{
			ARM_GP_VectorPtr newvec( new ARM_GP_Vector( *(arg[1].GetVector()) ) );
	...
	{	
			...
			&& arg[1].GetType() == GFAT_VECTOR_TYPE )
		{
			...
			if( doesSupportInPlace )
				v1 = ARM_VectorPtr( new ARM_GP_Vector( *(arg[1].GetVector()) ) );
			else
				v1 = arg[1].GetVector();
				...
				&& arg[1].GetType() == GFAT_DATE_TYPE )
			{
				ARM_VectorPtr v0;
				if( doesSupportInPlace )
					v0 = ARM_VectorPtr( new ARM_GP_Vector( *(arg[0].GetVector()) ) );
				else
					v0 = arg[0].GetVector();
									   ...
									   double evalDate, const ARM_PricingStatesPtr& states, vector< ARM_ExpNodePtr >& nodes )
{
	...
	if( ARM_ExpNode::ModelDoesNotSupportInPlace( mod ) )
		payoff = ARM_GP_VectorPtr( new ARM_GP_Vector( *(arg[0].GetVector()) ) );
	else
										  ...
										  double evalDate, const ARM_PricingStatesPtr& states, vector< ARM_ExpNodePtr >& nodes )
{
	...
	case GFAT_VECTOR_TYPE:
		{
			if( ARM_ExpNode::ModelDoesNotSupportInPlace( mod ) )
				payoff = ARM_VectorPtr( new ARM_GP_Vector( *(arg[0].GetVector()) ) );
			else 
				payoff = arg[0].GetVector();
			/// unpaid the payoff
			mod->ProcessUnPaidPayoffs( GetPayModelName(), payoff, evalTime, states );
	...
	case GFAT_DOUBLE_TYPE:
		{
			payoff = ARM_VectorPtr( new ARM_GP_Vector( states->size(), arg[0].GetDouble() ) );
			/// unpaid the payoff
			mod->ProcessUnPaidPayoffs( GetPayModelName(), payoff, evalTime, states );
			/// SetTheValue
			break;
...
****/
	...
	ARM_VectorPtr payoff1,payoff2;

	if (arg[0].GetType() == GFAT_VECTOR_TYPE)
	{
		payoff1 = ARM_VectorPtr( new ARM_GP_Vector( *(arg[0].GetVector()) ) );
	}
	else if (arg[0].GetType() == GFAT_DOUBLE_TYPE)
	{
		payoff1 = ARM_VectorPtr( new ARM_GP_Vector( states->size(), arg[0].GetDouble() ) );
	}
	else
	{
	...
	{
		payoff2 = ARM_VectorPtr( new ARM_GP_Vector( *(arg[1].GetVector()) ) );
	}
	else if (arg[1].GetType() == GFAT_DOUBLE_TYPE)
	{
		payoff2 = ARM_VectorPtr( new ARM_GP_Vector( states->size(), arg[1].GetDouble() ) );
	}
	else
	{
	...
	if (pricingDir == ARM_NumMethod::GP_BCKWDLOOKING)
	{
		...
		{
			contOpt = ARM_VectorPtr( new ARM_GP_Vector( states->size(), arg[2].GetDouble() ) );
		}
		else
		{
	...
	else
	{
		...
		return ARM_GramFctorArg(ARM_VectorPtr(new ARM_GP_Vector(states->size(),0.0)));
	}
}
									   ...
									   double evalDate, const ARM_PricingStatesPtr& states, vector< ARM_ExpNodePtr >& nodes )
{
	if (mod->GetNumMethod()->SupportFrontierComputation())
	{
		...
		if (arg[2].GetType() == GFAT_VECTOR_TYPE)
		{
			index = ARM_VectorPtr( new ARM_GP_Vector( *(arg[2].GetVector()) ) );
		}
		else if (arg[2].GetType() == GFAT_DOUBLE_TYPE)
		{
			index = ARM_VectorPtr( new ARM_GP_Vector( states->size(), arg[2].GetDouble() ) );
		}
		else
		{
			ARM_THROW( ERR_INVALID_ARGUMENT, ARM_USERNAME + ": execise payoff 2 option should be double or vector!" );
		...
		if (pricingDir == ARM_NumMethod::GP_BCKWDLOOKING)
		{
			...
			{
				intrinsic = ARM_VectorPtr( new ARM_GP_Vector( states->size(), arg[0].GetDouble() ) );
			}
			else
			{
				ARM_THROW( ERR_INVALID_ARGUMENT, ARM_USERNAME + ": execise continuation option should be double or vector!" );
			...
			{
				continuation = ARM_VectorPtr( new ARM_GP_Vector( states->size(), arg[1].GetDouble() ) );
			}
			else
			{
			...
			ARM_GP_Vector::iterator iterIdx = index->begin();
			ARM_GP_Vector::iterator iterIV = intrinsic->begin();
			ARM_GP_Vector::iterator iterCV = continuation->begin();
			double sgn = (*iterIV)-(*iterCV);
			double value;
			
			ARM_GP_Vector idxFrontier;
			int i = 0;
									   ...
									   double evalDate, const ARM_PricingStatesPtr& states, vector< ARM_ExpNodePtr >& nodes )
{
	...
	{
		ARM_VectorPtr contOpt;
		ARM_VectorPtr testVar1, payoff1, payoff2;
		...
		if (arg[0].GetType() == GFAT_VECTOR_TYPE)
			...
			testVar1 = ARM_VectorPtr( new ARM_GP_Vector( states->size(), arg[0].GetDouble() ) );
		else
			ARM_THROW( ERR_INVALID_ARGUMENT, ARM_USERNAME + ": execise testVar1 option should be double or vector!" );

		/// testVar2
		if (arg[1].GetType() == GFAT_VECTOR_TYPE)
			testVar2 = ARM_GP_VectorPtr( new ARM_GP_Vector( *(arg[1].GetVector()) ) );
		else if (arg[1].GetType() == GFAT_DOUBLE_TYPE)
		...
		// payoff1
		if (arg[2].GetType() == GFAT_VECTOR_TYPE)
			payoff1 = ARM_GP_VectorPtr( new ARM_GP_Vector( *(arg[2].GetVector()) ) );
		else if (arg[2].GetType() == GFAT_DOUBLE_TYPE)
			payoff1 = ARM_VectorPtr( new ARM_GP_Vector( states->size(), arg[2].GetDouble() ) );
		else
		...
		if (arg[3].GetType() == GFAT_VECTOR_TYPE)
			payoff2 = ARM_GP_VectorPtr( new ARM_GP_Vector( *(arg[3].GetVector()) ) );
		else if (arg[3].GetType() == GFAT_DOUBLE_TYPE)
			payoff2 = ARM_VectorPtr( new ARM_GP_Vector( states->size(), arg[3].GetDouble() ) );
		else
		...
		if (arg[4].GetType() == GFAT_VECTOR_TYPE)
			...
			contOpt = ARM_VectorPtr( new ARM_GP_Vector( states->size(), arg[4].GetDouble() ) );
		else
			ARM_THROW( ERR_INVALID_ARGUMENT, ARM_USERNAME + ": execise continuation option should be double or vector!" );

		ARM_VectorPtr newvec( new ARM_GP_Vector( (*payoff2) ) ) ;
											  ...
											  double evalDate, const ARM_PricingStatesPtr& states, vector< ARM_ExpNodePtr >& nodes )
{
	...
	else
	{
		int k, size = (int)(arg[1].GetDouble());
		int n, nbStates = arg[0].GetVector()->size();
		ARM_GP_VectorPtr newvec(new ARM_GP_Vector(nbStates,0.));

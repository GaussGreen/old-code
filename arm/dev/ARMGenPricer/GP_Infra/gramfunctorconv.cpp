...
ARM_GP_CurvePtr GetCurve(const ARM_GramFctorArg& arg)
{
	...
	else if (arg.GetType() == GFAT_DOUBLE_TYPE)
	{
		...
		ARM_GP_Vector ord(1,arg.GetDouble());
		return ARM_GP_CurvePtr(new ARM_Curve(abs,ord,new ARM::ARM_StepUpRightOpenCstExtrapolDble));
	}
	else
	{
                               ...
                               const ARM_GP_Vector& times, const string& msg)
{
    ...
    if( arg.GetType() ==  GFAT_DOUBLE_TYPE )
		values	= ARM_VectorPtr(new ARM_GP_Vector(nbOutValues,arg.GetDouble()));

	else if( arg.GetType() ==  GFAT_VECTOR_TYPE )
    {
        values = ARM_VectorPtr(new ARM_GP_Vector(nbOutValues));
    ...
    else if( arg.GetType() ==  GFAT_CURVE_TYPE )
	{
        ...
        values = ARM_VectorPtr(new ARM_GP_Vector(outValues));
	}
	else
    {

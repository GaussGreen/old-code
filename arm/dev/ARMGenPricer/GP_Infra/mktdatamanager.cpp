...
{
	ValidateAsOf( asOf );
	return RegisterData( CoinZCObjKey( indexName, ccy, cvName, asOf, source ),curve);
}
...
{
	ARM_ZeroCurve* zc = static_cast<ARM_ZeroCurve*>( GetData( CoinZCObjKey( indexName, ccy, cvName, asOf, source ) ) );
	if(!zc)
	{
	...
	const string& source ) const
{
	...
	/// other cases
	return (ARM_ZeroCurve* ) myCurve->Clone();
}
...
{
	ValidateAsOf( asOf );
	return RegisterData( CoinVolObjKey( indexName, ccy, cvName, asOf, volMktType, volType, source ), volCurve);
}
...
ARM_VolCurve* ARM_MarketData_ManagerImp::GetVolCurve( 
	...
	const string& source ) const
{
	ARM_VolCurve* volCurve	= static_cast<ARM_VolCurve*>( GetData( CoinVolObjKey( indexName, ccy, cvName, asOf, volMktType, volType, source ) ) );
	if(!volCurve)
	{
		if( ARM_MarketData_ManagerImp::UseMktDataRetriever && ARM_MarketData_ManagerImp::isAuthorizedToChangedMarketData)
		{
			...
			volCurve	= ARM_TheMarketDataRetriever.Instance()->GetVolFromSummit( 
				indexName, 
				ccy, 
				cvName, 
				asOf, 
	...
	const string& source ) const
{
	...
	/// other cases
	return (ARM_VolCurve*) myVolCurve->Clone();
}
...
{
	ValidateAsOf( asOf );
	return RegisterData( CoinVolMktModelObjKey( indexName, ccy, cvName, asOf, volMktType, source ), volMktModel);
}
...
ARM_Model* ARM_MarketData_ManagerImp::GetVolMktModel( 
	...
	const string& source ) const
{
	ARM_Model* volMktModel = static_cast<ARM_Model*>( GetData( CoinVolMktModelObjKey( indexName, ccy, cvName, asOf, volMktType, source ) ) );
	if(!volMktModel)
	{
		if( ARM_MarketData_ManagerImp::UseMktDataRetriever && ARM_MarketData_ManagerImp::isAuthorizedToChangedMarketData)
		{

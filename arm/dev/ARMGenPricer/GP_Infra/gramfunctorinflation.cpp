	...
	double evalDate, vector<ARM_ExpNodePtr>& nodes )
{
	...
	return ARM_NodeInfo( ARM_GP_VectorPtr(new ARM_GP_Vector(1,evalDate-mod->GetAsOfDate().GetJulian() )),ARM_AdditionalTimeInfoPtr(cpiTimeInfo) );
}
...
void ARM_GP_InfSwapCommon::ComputeDateStrips( ARM_PricingModel* mod )
{
		...
		ARM_GP_Vector * numStartDates = numDateStrip->GetFlowStartDates();
		ARM_GP_Vector * numResetDates = numDateStrip->GetResetDates();
...
ARM_NodeInfo ARM_GP_InfSwapRate::GetUsedTimeLags( ARM_GramFctorArgVector& arg, ARM_PricingModel* mod, 
	double evalDate, vector<ARM_ExpNodePtr>& nodes )
{
	return ARM_NodeInfo( ARM_GP_VectorPtr(new ARM_GP_Vector( 1, evalDate-mod->GetAsOfDate().GetJulian() )),ARM_AdditionalTimeInfoPtr(NULL) );
}

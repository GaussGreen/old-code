...
public:
	...
	virtual void derivs(double t, ARM_GP_Vector* yt, ARM_GP_Vector* dyt) const = 0;
};
...
public:
	...
	HWSVNumericals(size_t solverType=ARM_ODEFunc::RK5Adaptative,const ARM_GP_Vector& solverParams=ARM_GP_Vector(0),
		int formulaType=Heston,const ARM_GP_Vector& formulaParams=ARM_GP_Vector(0),
	...
	const ARM_GP_Vector& GetSolverParams() const {return itsSolverParams;}
	...
	const ARM_GP_Vector& GetFormulaParams() const {return itsFormulaParams;}
	...
	static void BuildHestonDefaultParams(ARM_GP_Vector& formulaParams,bool isSOFormula=false);
	static void BuildLewisDefaultParams(ARM_GP_Vector& formulaParams,bool isSOFormula=false);
	...
	void SetScheduleTrace(const ARM_GP_Vector& times) {itsScheduleTrace=times;}


	inline void SolveAnalyticalHestonSystem(const vector< ARM_AnalyticalRiccati >& analyticalDatas,
					 const ARM_GP_Vector& ImU,
					 ARM_GP_Vector& PsiX1,ARM_GP_Vector& PsiY1,
					 ARM_GP_Vector& PsiX0,ARM_GP_Vector& PsiY0,
					 double var=1.0) const;

	inline void SolveAnalyticalLewisSystem(const vector< ARM_AnalyticalRiccati >& analyticalDatas,
					 const ARM_GP_Vector& ImU,
					 ARM_GP_Vector& PsiX,ARM_GP_Vector& PsiY,
					 double var=1.0, double factor=0.5) const;

	inline void IntegrateHestonSystem(GaussLegendre_Coefficients& GL,const ARM_GP_Vector& ImU,
						 double scalet,double lnK,
						 const ARM_GP_Vector& PsiX1,const ARM_GP_Vector& PsiY1,
						 const ARM_GP_Vector& PsiX0,const ARM_GP_Vector& PsiY0,
						 double& localIntegral_1,double& localIntegral_2) const;

	inline void IntegrateLewisSystem(GaussLegendre_Coefficients& GL,const ARM_GP_Vector& ImU,
						 double scalet,double lnK,
						 const ARM_GP_Vector& PsiX,const ARM_GP_Vector& PsiY,
						 double& localIntegral,
						 double factor=0.5) const;

	void SolveSystem(ARM_RiccatiHWSV& riccatiSystem,
					 vector<ARM_GP_Vector>& solverVars,
					 ARM_GP_Vector& PsiX1,ARM_GP_Vector& PsiY1,
					 ARM_GP_Vector& PsiX0,ARM_GP_Vector& PsiY0,
					 double var=1.0) const;

	void IntegrateSystem(GaussLegendre_Coefficients& GL,const ARM_GP_Vector& ImU,
						 double scalet,double lnK,
						 const ARM_GP_Vector& PsiX1,const ARM_GP_Vector& PsiY1,
						 const ARM_GP_Vector& PsiX0,const ARM_GP_Vector& PsiY0,
						 double& localIntegral_1,double& localIntegral_2) const;
...
inline void HWSVNumericals::IntegrateHestonSystem(GaussLegendre_Coefficients& GL,const ARM_GP_Vector& ImU,
									 double scalet,double lnK,
									 const ARM_GP_Vector& PsiX1,const ARM_GP_Vector& PsiY1,
									 const ARM_GP_Vector& PsiX0,const ARM_GP_Vector& PsiY0,
									 double& localIntegral_1,double& localIntegral_2) const
...
inline void HWSVNumericals::IntegrateLewisSystem(GaussLegendre_Coefficients& GL,const ARM_GP_Vector& ImU,
									 double scalet,double lnK,
									 const ARM_GP_Vector& PsiX,const ARM_GP_Vector& PsiY,
									 double& localIntegral,
									 double factor) const
...
private:
	...
	ARM_GP_Vector itsModelSchedule;
...
public:
	...
	const ARM_GP_Vector& GetModelSchedule() const { return itsModelSchedule; }
	...
	void ModelStateLocalVariances(const ARM_GP_Vector& timeSteps,ARM_MatrixVector& localVariances) const {}
	...
	virtual ARM_GP_Vector* ComputeModelTimes(const ARM_TimeInfoPtrVector& timeInfos ) {return new ARM_GP_Vector(0);}
// FIXMEFRED: mig.vc8 (24/05/2007 10:46:11):cast
	...
	void IntegratedLocalDrifts(const ARM_GP_Vector& timeSteps,ARM_GP_MatrixPtr& relativeDrifts,ARM_GP_MatrixPtr& absoluteDrifts ) const;
	void EulerLocalDrifts(const ARM_GP_Vector& timeSteps,ARM_GP_MatrixPtr& relativeDrifts,ARM_GP_MatrixPtr& absoluteDrifts) const;
	virtual void ModelStateLocalCorrels( const ARM_GP_Vector& timeSteps,ARM_MatrixVector& localCorrels, const ARM_MultiAssetsModel& map);
	ARM_GP_VectorPtr VolatilitiesAndCorrelationTimesSteps() const;
	virtual void VolatilitiesAndCorrelations( const ARM_GP_Vector& timeSteps,ARM_GP_MatrixPtr& vols,ARM_GP_MatrixPtr& d1Vols,ARM_GP_MatrixPtr& correls ) const;

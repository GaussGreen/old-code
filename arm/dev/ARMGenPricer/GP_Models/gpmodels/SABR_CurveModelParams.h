...
#ifndef _INGPMODELS_SABR_CURVEMODELPARAMS_H
#define _INGPMODELS_SABR_CURVEMODELPARAMS_H
...
//-----------------------------------------------------------------------------
class ARM_SABR_CurveModelParams : public ARM_AnalyticModelParams
{
private:
	void ValidateModelParams() const;

public:
	ARM_SABR_CurveModelParams( const ARM_SABR_CurveModelParams& rhs );
	ARM_SABR_CurveModelParams( const ARM_ModelParamVector& params=ARM_ModelParamVector() );
	virtual ~ARM_SABR_CurveModelParams();
    ARM_SABR_CurveModelParams& operator = (const ARM_SABR_CurveModelParams& rhs);

	/// How many factors?
    virtual size_t FactorCount() const { return 1; };

    double ComputeImpliedVolatility(double undelying, 
        double strike, 
        double time,
        double tenor,
        int type,

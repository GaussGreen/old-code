...
////////////////////////////////////////////////////
ARM_GP_VectorPtr ARM_DiscountFactorMap::DiscountFactorInterpolate( double ResetTime, double EndTime, const ARM_GP_Vector& states, bool dividedByTerminalDf) const
{
...
ARM_GP_VectorPtr ARM_DiscountFactorMap::DiscountFactor( double ResetTime, double EndTime, const ARM_PricingStatesPtr& states, bool statesAreSorted ) const
{
	...
	if( abs ( ResetTime - EndTime ) < 1. )
		return ARM_GP_VectorPtr( new ARM_GP_Vector( states->size(), 1. ) );
...
ARM_GP_VectorPtr ARM_DiscountFactorMap::ReturnCalibrationDiscountFactors( double ResetTime, double EndTime ) const
{
	...
	/// ResetTime == EndTime
	if( abs ( ResetTime - EndTime ) < 1. )
		return ARM_GP_VectorPtr( new ARM_GP_Vector( discountFactors->cols(), 1. ) );
	...
	if( DoesStartTimeExist( EndTime ) )
	{
		...
		for( ARM_GP_Vector::iterator iter = result->begin() ; iter != result->end() ; ++iter,++miter)
			(*iter) = (*iter)/(*miter);
	...
	{	
		...
		if (ReverseEndIdx != 0)
		{
			...
			for( ARM_GP_Vector::iterator iter = result->begin() ; iter != result->end() ; ++iter,++nextIter,++termIter)
			{			
		...
		{			
			for( ARM_GP_Vector::iterator iter = result->begin() ; iter != result->end() ; ++iter, ++termIter)
			{			
				prevZcRate = - log( (*iter)/(*termIter)  );
...
ARM_GP_VectorPtr ARM_DiscountFactorMap::ReturnCalibrationDiscountFactorRatios( double ResetTime, double EndTime ) const
{
	...
	else
	{
		...
		if (ReverseEndIdx != 0)
		{
			...
			for( ARM_GP_Vector::iterator iter = result->begin() ; iter != result->end() ; ++iter,++nextIter)
			{			
				prevZcRate = - log( (*iter) );

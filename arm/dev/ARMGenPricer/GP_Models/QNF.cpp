		...
		const ARM_PricingStatesPtr& states) const
{
	...
	else
	{
		...
		ARM_GP_Vector decay( nbFactors, 0.0 );
		ARM_GP_Vector stdDevs( nbFactors, 0.0 );
	...
	bool isConstantStrike) const
{
    ...
    ARM_GP_VectorPtr swapRate( new ARM_GP_Vector(nbStates,0.0) );
    ARM_GP_VectorPtr fixAnnuity( new ARM_GP_Vector(nbStates,0.0) );
	...
	ARM_GP_Vector var( factorNb, 0.0 );
...
////////////////////////////////////////////////////
void ARM_QModelNF::VolatilitiesAndCorrelations( const ARM_GP_Vector& timeSteps, 
	...
	bool linearVol) const
{
	...
	{
		ARM_GP_Vector times = ((ARM_CurveModelParam&) modelParams->GetModelParamsPerDim(i)->GetModelParam( ARM_ModelParamType::QVol)).GetCurve()->GetAbscisses();
		ARM_GP_Vector values= ((ARM_CurveModelParam&) modelParams->GetModelParamsPerDim(i)->GetModelParam( ARM_ModelParamType::QVol)).GetCurve()->GetOrdinates();
...
void ARM_QModelNF::EulerLocalDrifts( const ARM_GP_Vector& timeSteps,
		ARM_GP_MatrixPtr& relativeDrifts,
		ARM_GP_MatrixPtr& absoluteDrifts) const
{
	size_t factorNb	= FactorCount();

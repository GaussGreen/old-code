...
{
	DuplicateCloneablePtrVectorInPlace<ARM_GP_Vector> (rhs.itsGrids, itsGrids);
		...
		ARM_DensityFunctor& density)
{
	...
	double minVal = hestonModelFx->Quantile(resetTime,minProb);
	...
	ARM_GP_Vector::iterator iter2 = grid->begin();
	for (;iter!=proba->end();++iter,++iter2)
		(*iter) = hestonModelFx->Proba(resetTime,(*iter2));

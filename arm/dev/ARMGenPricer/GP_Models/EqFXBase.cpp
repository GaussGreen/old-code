		...
		const ARM_PricingStatesPtr& states) const
{
	...
	return ARM_VectorPtr( new ARM_GP_Vector(nbStates,zcT/zct) );
}
    ...
    ARM_PricingContext* context) const
{
	...
	if(	evalTime<=K_NEW_DOUBLE_TOL || states == ARM_PricingStatesPtr(NULL) )
	{
		...
		if ( (calltype == Quanto) || (calltype == InvQuanto) )
		{
			...
			return  ARM_VectorPtr( new ARM_GP_Vector(1.0,callprice) );
		}
		else
        ...
        const ARM_PricingStatesPtr& states) const
{
	...
	if(	evalTime<=K_NEW_DOUBLE_TOL || states == ARM_PricingStatesPtr(NULL) )
	{
		...
		return ARM_VectorPtr( new ARM_GP_Vector(payoffSize,forward*payLagAdjst) );
	}
	else
	{
...
ARM_GP_VectorPtr ARM_EqFxBase::NormalDistribution(GaussLegendre_Coefficients glc, double forward, double expiryTime) const
{
	...
	ARM_GP_VectorPtr normaldist(new ARM_GP_Vector(NbPoints,0.0));
	double xi, proba_i;
		...
		bool IsInv) const
{
	...
	int NbPoints = glc.get_order();
	ARM_GP_VectorPtr fwd_mat(new ARM_GP_Vector(NbPoints,0.0));
	double xi;
	double mat = expiryTime/K_YEAR_LEN;
	if(IsInv)
	{
		...
		const ARM_GP_Vector& glparams) const
{
	...
	//fill
	ARM_GP_Vector x_w_fwd= glc.get_points();
    ...
    ARM_PricingContext* context) const
{
	...
	size_t payoffSize = states!=ARM_PricingStatesPtr(NULL)? states->size(): 1;
	ARM_GP_VectorPtr digit(new ARM_GP_Vector(payoffSize,0.0));
	...
	case ARM_FXDigitType::analytic:
		{
			...
			else
			{
				...
				ARM_GP_Vector newstrikePerState = ARM_GP_Vector(strikePerState.size(),newstrike);
...
////////////////////////////////////////////////////
void ARM_EqFxBase::VolatilitiesAndCorrelations( const ARM_GP_Vector& timeSteps, 
...
void ARM_EqFxBase::EulerLocalDrifts( const ARM_GP_Vector& timeSteps,
...
{	
	...
	//discounting
	rangeAccrual*=norm;
	return ARM_GP_VectorPtr( new ARM_GP_Vector(payoffSize,rangeAccrual) );
}

...
#endif
{
	...
	itsFactors		= ARM_VectorPtr( new ARM_GP_Vector(factorsNb) );
...
ARM_PricingStatesPtr ARM_SFRM::Init(const string& payModelName, const ARM_TimeInfoPtrVector& timeInfos)
{
    ...
    if(!isSpotUse)
    {
		...
		else if(numMethod->GetPricingDirection() ==  ARM_NumMethod::GP_BCKWDLOOKING)
        {
			...
			CC_NS(std,auto_ptr)<ARM_GP_Vector> pModelTimes( (*this).ComputeModelTimes( timeInfos ) );
	...
	case ARM_Numeraire::TerminalZc:
		{
			...
			return static_cast<ARM_VectorPtr>(new ARM_GP_Vector(1,(*itsFwdEndTimes)[payIndex] ));
		}
		break;
	case ARM_Numeraire::TerminalEventZc:
		{
			...
			return static_cast<ARM_VectorPtr>(new ARM_GP_Vector(1,(*itsFwdEndTimes)[resetIndex] ));
		}
		break;
	case ARM_Numeraire::RollingPayment:
		{
			ARM_GP_Vector* paymentDates = new ARM_GP_Vector;
			for (size_t i = 0; i < nbEvents; ++i)
			{
				const ARM_GP_Vector& payTimes = timeInfos[i]->GetPayTimes();
		...
		const ARM_PricingStatesPtr& states) const
{
    ...
    else
    {
        ...
        else
        {
		    ...
		    ARM_VectorPtr coVarSP(new ARM_GP_Vector(nbStates));
...
// FIXMEFRED: mig.vc8 (25/05/2007 15:54:21):cast
	...
	ARM_VectorPtr caplet(new ARM_GP_Vector(nbStates));
...
// FIXMEFRED: mig.vc8 (30/05/2007 15:55:57):cast
	...
	ARM_VectorPtr digitalCaplet(new ARM_GP_Vector(nbStates));
	double d2;
	for(i=0;i<nbStates;i++)
	{
...
// FIXMEFRED: mig.vc8 (30/05/2007 15:58:05):cast
    ...
    ARM_VectorPtr corridor(new ARM_GP_Vector(nbStates));
    ARM_GP_Vector ExerciseProbability(nbStates,0.0); 
    ...
    else
	{
        ...
        {
            libor = ARM_VectorPtr( new ARM_GP_Vector(nbStates, couponMargin ) );
        }
    }
    ...
    for( k = 0; k < RefIdxResettimes.size(); ++k)
    {
        ...
        if(itsCurrentArg)
        {
            ref_Fwd = ARM_VectorPtr((ARM_GP_Vector*)((((ARM_VanillaCorridorLegArgSFRM*)itsCurrentArg)->itsRef_Libors[index])[k])->Clone());
        ...
        ARM_VectorPtr DownProbability(new ARM_GP_Vector(nbStates,1.0));
        ARM_VectorPtr UpProbability(new ARM_GP_Vector(nbStates,1.0));
        for(i=0 ; i < nbStates; ++i)
        {
...
// FIXMEFRED: mig.vc8 (30/05/2007 15:58:14):cast
	...
	ARM_VectorPtr corridor(new ARM_GP_Vector(nbStates,result));
    return corridor;
}
...
// FIXMEFRED: mig.vc8 (30/05/2007 15:58:27):cast
	...
	ARM_VectorPtr swaption(new ARM_GP_Vector(nbStates));
	size_t i;
	...
	bool isConstantNotional	) const
{
	...
	ARM_GP_Vector* pFixPayTimes	 = static_cast<ARM_GP_Vector*>(const_cast<ARM_GP_Vector&>(fixPayTimes).Clone());
	ARM_GP_Vector* pFixPayPeriods	 = static_cast<ARM_GP_Vector*>(const_cast<ARM_GP_Vector&>(fixPayPeriods).Clone());
	ARM_GP_Vector* pFloatResetTimes = static_cast<ARM_GP_Vector*>(const_cast<ARM_GP_Vector&>(floatResetTimes).Clone());
	ARM_GP_Vector* pFloatStartTimes = static_cast<ARM_GP_Vector*>(const_cast<ARM_GP_Vector&>(floatStartTimes).Clone());
	ARM_GP_Vector* pFloatEndTimes	 = static_cast<ARM_GP_Vector*>(const_cast<ARM_GP_Vector&>(floatEndTimes).Clone());
	ARM_GP_Vector* pFloatIntTerms	 = static_cast<ARM_GP_Vector*>(const_cast<ARM_GP_Vector&>(floatIntTerms).Clone());

	ARM_GP_Vector* pfixNotional		 = static_cast<ARM_GP_Vector*>(const_cast<ARM_GP_Vector&>(fixNotional).Clone());
	ARM_GP_Vector* pfloatNotional	 = static_cast<ARM_GP_Vector*>(const_cast<ARM_GP_Vector&>(floatNotional).Clone());
	...
	ARM_GP_Vector* fwdStartTimes	= static_cast<ARM_GP_Vector*>(const_cast<ARM_GP_Vector&>(floatStartTimes).Clone());
	ARM_GP_Vector* fwdEndTimes		= static_cast<ARM_GP_Vector*>(const_cast<ARM_GP_Vector&>(floatEndTimes).Clone());
	ARM_GP_Vector* floatPayTimes	= static_cast<ARM_GP_Vector*>(const_cast<ARM_GP_Vector&>(floatEndTimes).Clone());


	ARM_GP_Vector fwdPayPeriods = ARM_GP_Vector(fwdStartTimes->size());
	...
	if (isConstantNotional)
	{
		...
		ARM_VectorPtr clonedfixAnnuity = ARM_CountedPtr<ARM_GP_Vector>( (ARM_GP_Vector*) fixAnnuity->Clone() );
	...
	else
	{
		...
		ARM_VectorPtr clonedfixAnnuity = ARM_CountedPtr<ARM_GP_Vector>( (ARM_GP_Vector*) fixAnnuityWithNominal->Clone() );
		///Compute a stub to add to fixAnnuity
		ARM_Date tmpDate(asOfDate + pFixPayTimes->Elt(0));
		char* CurrencyName = pCcy->GetCcyName();  
		tmpDate.AddPeriod(-fixFreq,CurrencyName);
	...
	ARM_VectorPtr nullstrike(new ARM_GP_Vector(1));
		...
		const ARM_PricingStatesPtr& states) const
{
	...
	ARM_VectorPtr SumOption(new ARM_GP_Vector(statesSize));
...
{	
        ...
        /// Insert the new function in the map
	{
		...
		if(	evalTime<=K_NEW_DOUBLE_TOL )
		{
			...
			size_t payoffSize = states != ARM_PricingStatesPtr(NULL) ? states->size(): 1;
			return ARM_VectorPtr( new ARM_GP_Vector(payoffSize,zc) );
		...
		else if( states == ARM_PricingStatesPtr(NULL) )
		{
			...
			return ARM_VectorPtr( new ARM_GP_Vector(1,zc/zct) );
		}
		else
		{
			size_t nbStates = states->size();
			...
			else
			{
				...
				{	
					...
					ARM_VectorPtr values2(new ARM_GP_Vector(*values));
					...
					values = ARM_VectorPtr(new ARM_GP_Vector(*(found2->second)));
			...
			if(found == itsDFMap.end())	
			{
				...
				if(!result.second)
					ARM_THROW( ERR_INVALID_ARGUMENT, ARM_USERNAME + ": can't insert a DF Vector in the map");
				return ARM_VectorPtr(new ARM_GP_Vector(*(result.first->second)));
			}
			else
				return ARM_VectorPtr(new ARM_GP_Vector(*(found->second)));
		}
	}
...
void ARM_SFRM::PostInit()
{
	...
	itsFwdValues=ARM_VectorPtr(new ARM_GP_Vector(totalFwds));
	itsFwdShifts=ARM_VectorPtr(new ARM_GP_Vector(totalFwds));
...
void ARM_SFRM::ComputeFwdsCorrelVarCoeffs( void ) const
{
	...
	ARM_GP_Vector tmpFwdsCoeffs(totalFwds);
	...
	for(size_t k=0; k<totalFwds; ++k)
	{
		...
		CC_NS(std,auto_ptr)<ARM_GP_Vector> correl_k(((ARM_ModelParamsSFRM*)GetModelParams())->InterpolateCorrelation((*itsFwdResetTimes)[k]));
	...
	const_cast<ARM_SFRM *>(this)->itsFwdsVarCoeffs  = ARM_VectorPtr(new ARM_GP_Vector(tmpFwdsCoeffs));
}
...
void ARM_SFRM::ComputeFwdDrifts(void)
{
	...
	itsFwdDrifts = ARM_VectorPtr(new ARM_GP_Vector(nbFwds));
...
////////////////////////////////////////////////////
ARM_GP_Vector* ARM_SFRM::PricingTimeSteps(const ARM_TimeInfoPtrVector& timeInfos)
{
...
///
////////////////////////////////////////////////////
ARM_GP_Vector* ARM_SFRM::ComputeModelTimes(  const ARM_TimeInfoPtrVector& timeInfos )
{
	...
	ARM_GP_Vector* tmpDateStripResetDates = pFloatDateStrip->GetResetDates();
	*tmpDateStripResetDates-= asOfDate;
	...
	itsFwdResetTimes  = ARM_VectorPtr(new ARM_GP_Vector(tmpResetDates));
	itsFwdStartTimes  = ARM_VectorPtr(new ARM_GP_Vector(tmpStartDates));
	itsFwdEndTimes	  = ARM_VectorPtr(new ARM_GP_Vector(tmpEndDates));
	itsFwdIntTerms	  = ARM_VectorPtr(new ARM_GP_Vector(tmpIntTerms));
...
void ARM_SFRM::MCModelStatesFromToNextTime(ARM_PricingStatesPtr& states,int timeIndex) const
{
	...
	ARM_GP_Vector lastDriftPerFactor(factorsNb);
...
void ARM_SFRM::PrepareForMarkovDriftCalculation(int timeIndex, double var_At_timeIndex,
	int itsFwdMinIndex,	 int NbFwds, int nbIntegrationSteps, ARM_GP_Vector*& VarCoeff,
	ARM_GP_Vector*& SpotVarCoeff, ARM_GP_Matrix*& FwdVolCoeff, 
	double*& x, double*& w) const
{
	...
	VarCoeff=new ARM_GP_Vector(nbIntegrationSteps);
    SpotVarCoeff=new ARM_GP_Vector(nbIntegrationSteps);
		...
		double var) const
{
	...
	size_t factorsNb = FactorCount();
	size_t nbStates = states->size();
	ARM_GP_Vector FwdsCorrelVarCoeffs(factorsNb);
	...
	ARM_GP_Vector expTab(nbStates), stochasticTerm(nbStates), probaChanges(nbStates), decStates(nbStates), decStates1(nbStates), decStates2(nbStates);
	...
	ARM_GP_Vector stochTermVec2(2);
	ARM_GP_Vector probaChangesVec2(2);
	...
	ARM_GP_Vector newProbaChanges(nbStates);
...
void ARM_SFRM::TreeStatesToModelStatesNonParamDrift(ARM_PricingStatesPtr& states, int timeIndex) const
{
	...
	ARM_GP_Vector savAcc(NbStates,1.0), prevSavAcc(NbStates,1.0);
...
void ARM_SFRM::TreeStatesToModelStatesDriftMarkovian(ARM_PricingStatesPtr& states, int timeIndex) const
{
	...
	else
	{
		...
		ARM_GP_Vector *VarCoeff=NULL,*SpotVarCoeff=NULL;
	...
	double endTime2, ARM_GP_Vector& VarCovar, ARM_GP_Vector& swapfwd) const
{
	...
	ARM_GP_Vector* floatResetDate1 = dateStrip1->GetResetDates();
	double swapResetTime1 = (*floatResetDate1)[0]-asOfDate;	
	int size1 = floatResetDate1->size();
	ARM_GP_Vector* floatStartTimes1		= dateStrip1->GetFlowStartDates();
	ARM_GP_Vector* floatPayTimes1		= dateStrip1->GetFlowEndDates();
	ARM_GP_Vector* floatIntTerms1		= dateStrip1->GetInterestTerms();
	...
	ARM_GP_Vector* floatResetDate2 = dateStrip2->GetResetDates();
	double swapResetTime2 = (*floatResetDate2)[0]-asOfDate;
	int size2 = floatResetDate2->size();
	ARM_GP_Vector* floatStartTimes2		= dateStrip2->GetFlowStartDates();
	ARM_GP_Vector* floatPayTimes2		= dateStrip2->GetFlowEndDates();
	ARM_GP_Vector* floatIntTerms2		= dateStrip2->GetInterestTerms();
									   ...
									   ARM_GP_Vector& swapfwd) const
{
	...
	var1 = var1*sqrt((toTime-fromTime)/K_YEAR_LEN);
	ARM_GP_Vector* floatResetDate1 = const_cast<ARM_GP_Vector* const>(swaptionArg1.GetFloatResetTimes());
	int size1 = floatResetDate1->size();
	ARM_VectorPtr swapfwd1	= swaptionArg1.GetSwapFwd();
	double shift1 = swaptionArg1.GetAverageShift();
	...
	var2 = var2*sqrt((toTime-fromTime)/K_YEAR_LEN);
	ARM_GP_Vector* floatResetDate2 = const_cast<ARM_GP_Vector* const>(swaptionArg2.GetFloatResetTimes());
	int size2 = floatResetDate2->size();
	ARM_VectorPtr swapfwd2	= swaptionArg2.GetSwapFwd();
	double shift2 = swaptionArg2.GetAverageShift();
												...
												const ARM_PricingStatesPtr& states) const
{
	...
	ARM_GP_Vector* floatResetDate_Long = const_cast<ARM_GP_Vector* const>(swaptionArg_Long->GetFloatResetTimes());
	...
	ARM_GP_Vector* floatResetDate_Short = const_cast<ARM_GP_Vector* const>(swaptionArg_Short->GetFloatResetTimes());
	...
	ARM_GP_Vector* floatResetDate_ToTimePayTime = const_cast<ARM_GP_Vector* const>(swaptionArg_ToTimePayTime->GetFloatResetTimes());
	...
	ARM_GP_Vector* floatResetDate_StartTimePayTime = const_cast<ARM_GP_Vector* const>(swaptionArg_StartTimePayTime->GetFloatResetTimes());
	...
	ARM_GP_Vector VarCovar_P(3);
	ARM_GP_Vector swapfwd_P(4);

	ARM_GP_Vector VarCovar_P_1(3);
	ARM_GP_Vector swapfwd_P_1(4);

	ARM_GP_Vector VarCovar_P_2(3);
	ARM_GP_Vector swapfwd_P_2(4);
		...
		const ARM_GP_Vector& swapShortFixPayPeriods) const
{
	...
	ARM_DateStrip fixDateStrip1( startDate1, endDate1, fixFreq, fixDayCount, fixCalendar,
		K_MOD_FOLLOWING, K_ADJUSTED, K_SHORTSTART, GETDEFAULTVALUE, fixFreq, GETDEFAULTVALUE,
		fixCalendar );
	ARM_GP_Vector* longFixPayTimes= static_cast<ARM_GP_Vector*>(fixDateStrip1.GetPaymentDates()->Clone());
	ARM_GP_Vector* longFixPayPeriods = static_cast<ARM_GP_Vector*>(fixDateStrip1.GetInterestTerms()->Clone()); 
	...
	ARM_DateStrip fixDateStrip2( startDate2, endDate2, fixFreq, fixDayCount, fixCalendar,
		K_MOD_FOLLOWING, K_ADJUSTED, K_SHORTSTART, GETDEFAULTVALUE, fixFreq, GETDEFAULTVALUE,
		fixCalendar );
	ARM_GP_Vector* shortFixPayTimes   = static_cast<ARM_GP_Vector*>(fixDateStrip2.GetPaymentDates()->Clone());
	ARM_GP_Vector* shortFixPayPeriods = static_cast<ARM_GP_Vector*>(fixDateStrip2.GetInterestTerms()->Clone());
	...
	ARM_GP_Vector* floatResetDateToTime = dateStripToTime->GetResetDates();
	ARM_GP_Vector* floatStartDateToTime		= dateStripToTime->GetFlowStartDates();
	ARM_GP_Vector* floatEndDateToTime		= dateStripToTime->GetFlowEndDates();
	ARM_GP_Vector* floatIntTermsToTime		= dateStripToTime->GetInterestTerms();
	...
	ARM_GP_Vector* floatResetDateStartTime = dateStripStartTime->GetResetDates();
	ARM_GP_Vector* floatStartDateStartTime = dateStripStartTime->GetFlowStartDates();
	ARM_GP_Vector* floatEndDateStartTime = dateStripStartTime->GetFlowEndDates();
	ARM_GP_Vector* floatIntTermsStartToTime		= dateStripStartTime->GetInterestTerms();
...
///	Action  : No default implementation since inappropriate for SFRM  model
////////////////////////////////////////////////////
void ARM_SFRM::NumMethodStateGlobalVariances( const ARM_GP_Vector& timeSteps,
...
ARM_VanillaCorridorLegArgSFRM* ARM_SFRM::ComputeCachedCorridorLegData(ARM_CorridorLeg* corridorLeg )
{
	...
	for( size_t i=0; i<size; ++i )
	{
        ...
        for( size_t k=0; k<size_i; ++k )
        {
            ...
            ref_DfFwd  = ARM_VectorPtr(new ARM_GP_Vector(nbStates));
...
ARM_VanillaSpreadOptionletArgSFRM* ARM_SFRM::ComputeCachedSpreadOptionletData( ARM_SpreadOption* spreadoption)
{
	...
	const ARM_GP_Vector& strikes = arg->GetStrikes()[0];
	double swapLongFloatStartTime = (*(arg->GetSwapLongFloatStartTime()))[0];
	double swapLongFloatEndTime = (*(arg->GetSwapLongFloatEndTime()))[0];
	const ARM_GP_Vector& swapLongFixPayTimes= *arg->GetSwapLongFixPayTimes()[0];
	const ARM_GP_Vector& swapLongFixPayPeriods = *arg->GetSwapLongFixPayPeriods()[0];
	...
	const ARM_GP_Vector& swapShortFixPayTimes = *arg->GetSwapShortFixPayTimes()[0];
	const ARM_GP_Vector& swapShortFixPayPeriods = *arg->GetSwapShortFixPayPeriods()[0];
...
////////////////////////////////////////////////////
void ARM_SFRM::VolatilitiesAndCorrelations( const ARM_GP_Vector& timeSteps, 
		...
		bool linearVol) const
{
	...
	ARM_GP_Vector times = ((ARM_CurveModelParam&) GetModelParams()->GetModelParam( ARM_ModelParamType::Volatility)).GetCurve()->GetAbscisses();
	ARM_GP_Vector values= ((ARM_CurveModelParam&) GetModelParams()->GetModelParam( ARM_ModelParamType::Volatility)).GetCurve()->GetOrdinates();

	/// Since volatilies are stepUpRight!!
	for( ARM_GP_Vector::iterator iter = values.begin() ; iter != values.end()-1 ; ++iter)
		(*iter) = *(iter+1);

	ARM_GP_Vector volsVec,d1VolsVec;
...
///	Action  : function for the generic tree (2nd generation)
////////////////////////////////////////////////////
void ARM_SFRM::EulerLocalDrifts(const ARM_GP_Vector& timeSteps,
		ARM_GP_MatrixPtr& relativeDrifts,
		ARM_GP_MatrixPtr& absoluteDrifts) const
{

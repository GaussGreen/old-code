		...
		const ARM_PricingStatesPtr& states) const
{
    ...
    else
    {
        ...
        if(evalTime < maturityTime)
            ...
            stateFact = new ARM_GP_Vector(2,0.0);
        zct=ZcCurve->DiscountPrice(evalTime/K_YEAR_LEN);
		...
		bool isConstantStrike) const
{
    ...
    ARM_GP_Vector *betatT0=((const ARM_ModelParamsHW2F* const) GetModelParams())->BetatT(swapResetTime,effFloatStartTime);
													...
													const ARM_PricingStatesPtr& states) const
{
	...
	ARM_GP_Vector longFlowStoc(longNbFlows+1),shortFlowStoc(shortNbFlows+1);
	double longStartStoc,shortStartStoc;
	ARM_GP_Vector condValues(nbStates);
	for(i1=0;i1<n1;++i1)
	{
		...
		bool isConstantStrike) const
{
    ...
    /// Compute fwd Zc coef in a uncorrelated world (i.e. ACP)
    ARM_GP_Vector *betatT0=((const ARM_ModelParamsHW2F* const) GetModelParams())->BetatT(swapResetTime,floatStartTime);
    ARM_VectorVector betatTiT0(nbFlows);
    vector< double > logDrift(nbFlows);
    vector< double > stdDev1(nbFlows);
    vector< double > stdDev2(nbFlows);
...
void ARM_HullWhite2F::EulerLocalDrifts( const ARM_GP_Vector& timeSteps,
		ARM_GP_MatrixPtr& relativeDrifts,
		ARM_GP_MatrixPtr& absoluteDrifts) const
{
...
////////////////////////////////////////////////////
void ARM_HullWhite2F::VolatilitiesAndCorrelations( const ARM_GP_Vector& timeSteps, 
...
{	
	...
	else
	{
		...
		ARM_GP_Vector volsVec(timeStepsSize);
...
ARM_GP_Vector* csecurity::getRateWeight(double t_start,double t_end,const ARM_GP_Vector& payTimes,const ARM_GP_Vector& periods,double mrs1,double mrs2)
{
	...
	ARM_GP_Vector* pRes = new ARM_GP_Vector(4,0.);
	(*pRes)[0]=fwd;
	(*pRes)[1]=w1;
	(*pRes)[2]=w2;
	(*pRes)[3]=level;
...
ARM_GP_Vector* csecurity::getRateWeight(double t_start,const ARM_GP_Vector& floatPayTimes,const ARM_GP_Vector& floatNotionals,
		const ARM_GP_Vector& fixPayTimes,const ARM_GP_Vector& fixNotionals,const ARM_GP_Vector& fixPeriods,
...
ARM_GP_Vector* csecurity::getChangeNumWeight(double t_pay,double t_start,double t_end,const ARM_GP_Vector& payTimes,const ARM_GP_Vector& periods,double mrs1,double mrs2)
{
	ARM_ZeroCurvePtr ZcCurve = m_pmodel->GetZeroCurve();
	...
	ARM_GP_Vector* pRes = new ARM_GP_Vector(2,0.);
	(*pRes)[0]=w1;
	(*pRes)[1]=w2;
	return pRes;
}
...
void cswaption::BuildCoeffs(const ARM_GP_Vector& lv, const ARM_GP_Vector& lv0, const ARM_GP_Vector& lv1)
{
	m_A  = lv[0]*m_w1*m_w1;
	m_B  = lv[1]*m_w2*m_w2;
...
/*****
	...
	const ARM_GP_Vector& swapLongFixPayTimes	= *(argSO->GetSwapLongFixPayTimes()[0]);
	const ARM_GP_Vector& swapLongFixPayPeriods	= *(argSO->GetSwapLongFixPayPeriods()[0]);
	const ARM_GP_Vector& swapShortFixPayTimes	= *(argSO->GetSwapShortFixPayTimes()[0]);
	const ARM_GP_Vector& swapShortFixPayPeriods	= *(argSO->GetSwapShortFixPayPeriods()[0]);
	

	ARM_GP_Vector* rateShortWeight = getRateWeight(	swapShortFloatStartTime, 
													swapShortFloatEndTime,
													swapShortFixPayTimes,
													swapShortFixPayPeriods,
													mrs1,mrs2);
			
	ARM_GP_Vector* chNumShortWeight = getChangeNumWeight(	payTime,
															swapShortFloatStartTime, 
															swapShortFloatEndTime,
															swapShortFixPayTimes,
															swapShortFixPayPeriods,
	...
	ARM_GP_Vector* rateLongWeight = getRateWeight(	swapLongFloatStartTime, 
													swapLongFloatEndTime,
													swapLongFixPayTimes, 
													swapLongFixPayPeriods,
													mrs1,mrs2);
			
	ARM_GP_Vector* chNumLongWeight = getChangeNumWeight(	payTime,
															swapLongFloatStartTime, 
															swapLongFloatEndTime,
															swapLongFixPayTimes, 
															swapLongFixPayPeriods,
...
*****/
	...
	ARM_GP_Vector* rateShortWeight = getRateWeight(	shortSwapRate->floatStartTime, 
	...
	ARM_GP_Vector* rateLongWeight = getRateWeight(	longSwapRate->floatStartTime, 
...
void cspreadoption::BuildCoeffs(const ARM_GP_Vector& lv, const ARM_GP_Vector& lv0, const ARM_GP_Vector& lv1)
{

...
{
	ARM_GP_Vector timeSteps(itsVolCurve.GetAbscisses());
	ARM_GP_Vector volFunc(itsVolCurve.GetOrdinates());
	size_t nbSteps = timeSteps.size();
	ARM_GP_Vector varFunc(nbSteps);
	double prec,time,dt;
...
void ARM_ProcessBuilder::Calibrate(	double horizonDate,
												...
												const ARM_GP_Vector& evalDates,
												const ARM_GP_Vector& numDates,
												bool doPDE)
{
		itsHorizonDate			= horizonDate;
		ComputeIntegratedVariance();

		ARM_GP_Vector delta = terminalDensity.getInterestTerms();
...
{
	DuplicateCloneablePtrVectorInPlace<ARM_GP_Vector> (rhs.itsDFRatio, itsDFRatio);
	DuplicateCloneablePtrVectorInPlace<ARM_GP_Vector> (rhs.itsDxLogDFRatio, itsDxLogDFRatio);
	DuplicateCloneablePtrVectorInPlace<ARM_GP_Vector> (rhs.itsRate, itsRate);
	DuplicateCloneablePtrVectorInPlace<ARM_GP_Vector> (rhs.itsDxRate, itsDxRate);
}
...
double ARM_ProcessBuilderPDE::GetIthMoment(size_t n) const
{
	...
	ARM_GP_Vector states (GLPOINTSNB);
...
{
	ARM_GP_Vector old = volatilityCalib->GetOrdinates();
	ARM_GP_Vector date = volatilityCalib->GetAbscisses();
...
////////////////////////////////////////////////////
size_t ARM_ProcessBuilderPDE::IdxFromDate( const ARM_GP_Vector& dateVector, double date ) const
{
	size_t k=0;
...
ARM_GP_VectorPtr ARM_ProcessBuilderPDE::DFRatio(double mat,const ARM_PricingStatesPtr& states, size_t modelNb, size_t k, bool extrapol) const
{
	...
	else
	{
		...
		ARM_VectorPtr interpol( new ARM_GP_Vector(states->size()) );

		ARM_GP_Vector::iterator iter2 = dxRight->begin();
		ARM_GP_Vector::iterator iter3 = interpol->begin();

		for(ARM_GP_Vector::iterator iter1 = dxLeft->begin(); iter1!=dxLeft->end() ; ++iter1, ++iter2, ++iter3)
		{
...
ARM_GP_VectorPtr ARM_ProcessBuilderPDE::Rate(double mat,const ARM_PricingStatesPtr& states, size_t modelNb,size_t k, bool extrapol) const
{
	...
	{
		size_t index = IdxFromValueWithTol(itsDates,t,1);
		ARM_GP_Vector* result = RateAtResetDate(index,states,modelNb,k);
	...
	else
	{
		...
		size_t iLeft = iRight - 1;
		double ratio = ( t - itsDates[iLeft] ) / ( itsDates[iRight] - itsDates[iLeft] );
		ARM_GP_Vector* interpol = RateBetweenResetDates(ratio,iLeft,iRight,states,modelNb,k);
...
ARM_GP_VectorPtr ARM_ProcessBuilderPDE::DxRate(double mat,const ARM_PricingStatesPtr& states, size_t modelNb, size_t k, bool extrapol) const
{
	...
	{
		size_t index = IdxFromValueWithTol(itsDates,t,1);
		ARM_GP_Vector* result = DxRateAtResetDate(index,states,modelNb,k);
		return ARM_GP_VectorPtr(result);
	}
	else
	{
		...
		size_t iLeft = iRight - 1;
		double ratio = ( t - itsDates[iLeft] ) / ( itsDates[iRight] - itsDates[iLeft] );
		ARM_GP_Vector* interpol = DxRateBetweenResetDates(ratio,iLeft,iRight,states,modelNb,k);
...
///	Action : 
////////////////////////////////////////////////////
ARM_GP_Vector* ARM_ProcessBuilderPDE::RateAtResetDate(size_t index,const ARM_PricingStatesPtr& states, size_t modelNb, size_t k) const
{
	size_t i, j;
...
///	Action : 
////////////////////////////////////////////////////
ARM_GP_Vector* ARM_ProcessBuilderPDE::RateBetweenResetDates(double ratio,size_t iLeft,size_t iRight,const ARM_PricingStatesPtr& states, size_t modelNb, size_t k) const
{
	size_t i, j;
	size_t newSize							= states->size();
	ARM_GP_MatrixPtr newNumMethodStates		= states->GetModelStates();
...
///	Action : 
////////////////////////////////////////////////////
ARM_GP_Vector* ARM_ProcessBuilderPDE::DxRateBetweenResetDates(double ratio,size_t iLeft,size_t iRight,const ARM_PricingStatesPtr& states, size_t modelNb, size_t k) const
{
	size_t i, j;
	size_t newSize							= states->size();
	ARM_GP_MatrixPtr newNumMethodStates		= states->GetModelStates();
...
///	Action : 
////////////////////////////////////////////////////
ARM_GP_Vector* ARM_ProcessBuilderPDE::DxRateAtResetDate(size_t index,const ARM_PricingStatesPtr& states, size_t modelNb, size_t k) const
{
	size_t i, j;
	size_t newSize							= states->size();
	ARM_GP_MatrixPtr newNumMethodStates		= states->GetModelStates();
...
////////////////////////////////////////////////////
ARM_GP_VectorPtr ARM_ProcessBuilderPDE::DFRatioAtResetDate(size_t index,const ARM_GP_Vector& states) const
{
	size_t i, j;
	size_t newSize							= states.size();
...
{
	double cumVar		= RescaleVolatilityCalib(volatilityCalib);
	ARM_GP_Vector vol	= volatilityCalib->GetOrdinates();
	ARM_GP_Vector date	= volatilityCalib->GetAbscisses();
	size_t	size		= IdxFromValueWithTol(date,Horizon(),1) + 1;

	ARM_GP_Vector v(size,0.);
...
///	Action : computes and stores E[f(XT)|Ft] (implies solving a 1F PDE)
////////////////////////////////////////////////////
void ARM_ProcessBuilderPDE::Backward(double first,double last,ARM_GP_VectorPtr& func,const ARM_GP_Vector& numDates)
{
...
///	Action : 
////////////////////////////////////////////////////
size_t ARM_ProcessBuilderPDE::GetIndex(double x, const ARM_GP_Vector& v) const
{
	size_t size = v.size();

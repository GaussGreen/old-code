...
ARM_PricingStatesPtr ARM_HullWhite::Init(const string& payModelName, const ARM_TimeInfoPtrVector& timeInfos)
{
    ...
    if(!isSpotUse)
    {
		...
		ARM_GP_Vector* timeSteps = numMethod->GetTimeSteps();
...
////////////////////////////////////////////////////
bool ARM_HullWhite::isFwdStart(const ARM_GP_Vector& fixNotional, int& idx) const
{
	bool ok  = false;
	bool cst = true;
	int lastPosIdx = -1;
...
// FIXMEFRED: mig.vc8 (25/05/2007 15:26:45):cast
    ...
    ARM_VectorPtr values = ARM_VectorPtr(new ARM_GP_Vector(nbStates));
...
////////////////////////////////////////////////////
ARM_GP_Vector* ARM_HullWhite::ComputeModelTimes(  const ARM_TimeInfoPtrVector& timeInfos )
{
											   ...
											   double   endTime4) const
{
	...
	else if (underlyingType == "CMS" || underlyingType == "SO/CMS" || underlyingType == "SO/SO")
	{
		...
		ARM_GP_Vector longFixPayDfs    (longSched.GetFlowEndDates()->size()) ;
		ARM_GP_Vector longFixPayTimes  (longSched.GetFlowEndDates()->size()) ;
		ARM_GP_Vector shortFixPayDfs   (shortSched.GetFlowEndDates()->size()) ;
		ARM_GP_Vector shortFixPayTimes (shortSched.GetFlowEndDates()->size()) ;
		...
		ARM_GP_Vector rateLongFixPayDfs,rateLongFixPayTimes,rateLongInterestTerms;
		...
		ARM_GP_Vector rateShortFixPayDfs,rateShortFixPayTimes,rateShortInterestTerms;
													...
													const ARM_PricingStatesPtr& states) const
{
	...
	return ARM_VectorPtr( new ARM_GP_Vector(payoffSize, price) );
}

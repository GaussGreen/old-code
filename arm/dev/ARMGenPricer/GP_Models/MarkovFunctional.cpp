...
void ARM_MarkovFunctional::setNumericalModelFitter( ARM_NumericalModelFitter* numericalModelFitter ) 
{
	if (numericalModelFitter)
	{
		...
		ARM_GP_Vector* resetDates = sched->GetResetDates(); 
		ARM_GP_Vector* startDates = sched->GetFlowStartDates(); 
		ARM_GP_Vector* endDates	  = sched->GetFlowEndDates(); 
...
ARM_PricingStatesPtr ARM_MarkovFunctional::Init(const string& payModelName, const ARM_TimeInfoPtrVector& timeInfos)
{
	...
	CC_NS(std,auto_ptr)<ARM_GP_Vector> ptimeSteps;

	if( !itsCalibrationStatus )
		ptimeSteps = CC_NS(std,auto_ptr)<ARM_GP_Vector>( discretisationScheme.ModelTimesFromTimesInfo(timeInfos, *this) );
	else
		ptimeSteps = CC_NS(std,auto_ptr)<ARM_GP_Vector>( static_cast<ARM_GP_Vector*> (itsDiscountFactorMap.getResetTimes()->Clone()) );
	
	/// Initialise the numeraire
	numeraire->Init(*(GetDiscountFunctor()),payModelName,timeInfos,ARM_GP_VectorPtr( new ARM_GP_Vector( 1, itsDiscountFactorMap.getTerminalTime() )));
		...
		const ARM_PricingStatesPtr& states) const
{
	...
	{
		ARM_GP_VectorPtr resultVector( new ARM_GP_Vector( MAX(1,states->size() )));
		...
		for( ARM_GP_Vector::iterator iter = resultVector->begin() ; iter != resultVector->end() ; ++iter )
			(*iter) = result;
		...
		bool isConstantStrike) const
{
	...
	{
		ARM_GP_VectorPtr resultVector( new ARM_GP_Vector( MAX(1,states->GetNumMethodStates()->size() )));
...
////////////////////////////////////////////////////
ARM_GP_Vector* ARM_MarkovFunctional::ComputeModelTimes(  const ARM_TimeInfoPtrVector& timeInfos )
{
...
////////////////////////////////////////////////////
void ARM_MarkovFunctional::VolatilitiesAndCorrelations( const ARM_GP_Vector& timeSteps, 
...
ARM_VectorPtr ARM_MarkovFunctional::DiscountFactor( const string& curveName, double evalTime, double maturityTime, const ARM_PricingStatesPtr& states) const
{
	...
	if( abs( evalTime - maturityTime ) < K_DOUBLE_TOL )
		return static_cast<ARM_VectorPtr>(new ARM_GP_Vector(states->size(), 1. ));
...
////////////////////////////////////////////////////
void ARM_MarkovFunctional::NumMethodStateLocalVariances( const ARM_GP_Vector& timeSteps,
...
////////////////////////////////////////////////////
void ARM_MarkovFunctional::NumMethodStateGlobalVariances( const ARM_GP_Vector& timeSteps,
...
////////////////////////////////////////////////////
void ARM_MarkovFunctional::EulerLocalDrifts(const ARM_GP_Vector& timeSteps,ARM_GP_MatrixPtr& relativeDrifts,ARM_GP_MatrixPtr& absoluteDrifts) const
{
...
////////////////////////////////////////////////////
void ARM_MarkovFunctional::ModelStateLocalVariances( const ARM_GP_Vector& timeSteps,
		ARM_MatrixVector& localVariances ) const
{
...
////////////////////////////////////////////////////
void ARM_MarkovFunctional::ModelStateLocalCorrels( const ARM_GP_Vector& timeSteps,
...
void ARM_MarkovFunctional::BuildDFs( const ARM_PricingStatesPtr& states, double toTime, size_t lineIdx )
{
	...
	ARM_GP_VectorPtr probas( new ARM_GP_Vector( states->GetNumMethodStates()->size() ));
...
void ARM_MarkovFunctional::buildNormalProbabilities( ARM_GP_VectorPtr& probas, const ARM_PricingStatesPtr& states, double toResetTime ) const
{
	...
	{		
		...
		const ARM_GP_Vector&  interestTerms = itsNumericalModelFitter->getCalibSecInterestTerms(resetIdx);
...
{
	return static_cast<ARM_VectorPtr>(new ARM_GP_Vector(1,itsDiscountFactorMap.getTerminalTime()));
}

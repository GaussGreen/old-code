...
ARM_ModelParamsBGMSV1F::ARM_ModelParamsBGMSV1F(const ARM_ModelParamVector& params, const ARM_GP_Vector& stddev, 
...
void ARM_ModelParamsBGMSV1F::SetParams(const ARM_GP_Vector& resetTimes, double v0, double kappa, double theta,
									   const ARM_GP_Vector& shifts, const ARM_GP_Vector& levels, 
									   const ARM_GP_Vector& vvols, const ARM_GP_Vector& rhos)
{
	( (ARM_CurveModelParam&) GetModelParam(ARM_ModelParamType::VolMeanReversion) ).GetCurve()->SetOrdinates(ARM_GP_Vector(1,kappa));
	( (ARM_CurveModelParam&) GetModelParam(ARM_ModelParamType::InitialVol) ).GetCurve()->SetOrdinates(ARM_GP_Vector(1,v0));
	( (ARM_CurveModelParam&) GetModelParam(ARM_ModelParamType::LongTermVol) ).GetCurve()->SetOrdinates(ARM_GP_Vector(1,theta));
	...
	if(DoesModelParamExist(ARM_ModelParamType::Shift))
	{
		...
		{
			( (ARM_CurveModelParam&) GetModelParam(ARM_ModelParamType::Shift) ).GetCurve()->SetOrdinates(ARM_GP_Vector(resetTimes.size(), shifts[0]));
		}
	}
	...
	{
		( (ARM_CurveModelParam&) GetModelParam(ARM_ModelParamType::Alpha) ).GetCurve()->SetOrdinates(ARM_GP_Vector(resetTimes.size(),levels[0]));
	}
	...
	{
		( (ARM_CurveModelParam&) GetModelParam(ARM_ModelParamType::QParameter) ).GetCurve()->SetOrdinates(ARM_GP_Vector(resetTimes.size(),rhos[0]));
	}


	if(vvols.size() == 1)
	{
		( (ARM_CurveModelParam&) GetModelParam(ARM_ModelParamType::VolOfVol) ).GetCurve()->SetAbscisses(ARM_GP_Vector(1,0.));
		( (ARM_CurveModelParam&) GetModelParam(ARM_ModelParamType::VolOfVol) ).GetCurve()->SetOrdinates(ARM_GP_Vector(1,0.));
	}
...
ARM_Curve * ARM_ModelParamsBGMSV1F::GetVVolt(double evalTime) const
{
	...
	else
	{
		...
		ARM_GP_Vector times(size-i), values(size-i);
...
void ARM_ModelParamsBGMSV1F::Calibrate(const ARM_GP_Vector& resetTimes, const ARM_GP_Vector& fwdRates,
									   const ARM_VanillaSecDensityPtrVector& CalibSecDensities)
{
	...
	else
	{
		...
		ARM_GP_Vector shifts(size), levels(size), rhos(size), vvols(size);
...
void ARM_ModelParamsBGMSV1F::GetCalibrationData(const ARM_GP_Vector& resetTimes, const ARM_GP_Vector& fwdRates,
												const ARM_VanillaSecDensityPtrVector& CalibSecDensities,
...
void ARM_ModelParamsBGMSV1F::finishGlobalCalibration(const ARM_GP_Vector& resetTimes, 
													 const ARM_GP_Vector& fwdRates,
													 const ARM_GP_Vector& atmvols, ARM_GP_Vector& calibWeights, 
													 const ARM_GP_Vector& calibTimes, 
													 const ARM_SmileCalibration_Params_Heston& params)
{
	...
	ARM_GP_Vector shifts(size), rhos(size), levels(size);
...
void ARM_ModelParamsBGMSV1F::finishLocalCalibration(const ARM_GP_Vector& resetTimes, 
													const ARM_GP_Vector& fwdRates, 
													const ARM_GP_Vector& atmvols, ARM_GP_Vector& calibWeights,
													const ARM_GP_Vector& calibTimes, 
													const ARM_GP_Vector& calibShifts, 
													const ARM_GP_Vector& calibLevels, 
													const ARM_GP_Vector& calibRhos, 
													const ARM_GP_Vector& calibVvols)
{

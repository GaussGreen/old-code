...
ARM_PricingStatesPtr ARM_IrFwdMod::Init(const string& payModelName, const ARM_TimeInfoPtrVector& timeInfos)
{
    ...
    ARM_GP_Vector timeSteps(nbEvents);
	for( int i=0; i<nbEvents; ++i)
		timeSteps[i] = timeInfos[i]->GetEventTime();

    GetNumMethod()->SetTimeSteps( ARM_GP_Vector(timeSteps) );
...
// FIXMEFRED: mig.vc8 (30/05/2007 16:11:16):cast
	return static_cast<ARM_VectorPtr>(new ARM_GP_Vector(1,fwdDf));
}
...
// FIXMEFRED: mig.vc8 (30/05/2007 16:11:29):cast
	return static_cast<ARM_VectorPtr>(new ARM_GP_Vector(1,result));
}
...
void ARM_IrFwdMod::NumMethodStateLocalVariances( const ARM_GP_Vector& timeSteps,
...
void ARM_IrFwdMod::ModelStateLocalVariances( const ARM_GP_Vector& timeSteps,
	ARM_MatrixVector& localVariances ) const
{
    throw Exception(__LINE__, __FILE__, ERR_UNIMP_METHOD_CALL,
...
ARM_GP_Vector* ARM_IrFwdMod::ComputeModelTimes(  const ARM_TimeInfoPtrVector& timeInfos )
{

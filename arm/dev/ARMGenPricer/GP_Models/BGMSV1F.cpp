...
ARM_PricingStatesPtr ARM_BGMSV1F::Induct(ARM_PricingStatesPtr& states,double toTime)
{
	...
	{	
		if(itsCalibratedStatus == false)
		{
			...
			/*
			ARM_GP_Vector calibTimes(size);
			calibTimes[0] = 0.;
			for(k = 1; k < size; k++) calibTimes[k] = itsResetTimes[k-1];

			ARM_VectorVector strikes, mktvols;
			ARM_GP_Vector resettimes, forwards, atmvols;
			ARM_GP_Vector shifts, levels, weights;
...
ARM_GP_Vector* ARM_BGMSV1F::ComputeModelTimes(  const ARM_TimeInfoPtrVector& timeInfos )
{
...
void ARM_BGMSV1F::CalcNbEffectiveReset(const ARM_GP_Vector& timeSteps)
{
	if(itsProxyStatus)
	{
...
void ARM_BGMSV1F::ModelStateLocalVariancesAndStdDev( const ARM_GP_Vector& timeSteps)
{
...
void ARM_BGMSV1F::NumMethodStateLocalVariances(const ARM_GP_Vector& timeSteps, ARM_MatrixVector& localVariances ) const
{
...
void ARM_BGMSV1F::NumMethodStateGlobalVariances(const ARM_GP_Vector& timeSteps, ARM_MatrixVector& globalVariances ) const
{
...
void ARM_BGMSV1F::ModelStateLocalStdDev( const ARM_GP_Vector& timeSteps, const ARM_MatrixVector& auxLocalCov)
{
...
void ARM_BGMSV1F::NumMethodStateLocalVariancesAndStdDev( const ARM_GP_Vector& timeSteps )
{
...
void ARM_BGMSV1F::ModelStateLocalVariances(const ARM_GP_Vector& timeSteps, ARM_MatrixVector& localVariances ) const
{
	int totalFwds		= itsNbEffectiveReset;
	int timeStepsSize	= timeSteps.size();
	int modelNb			= GetModelNb();
	...
	else
	{
		...
		if(ExistsInVector(itsResetTimes, fwdResetTime))
		{
			...
			else if(evalTime > K_DOUBLE_TOL)
			{
				...
				if(fabs(fwdEndTime - fwdStartTime + itsStartTimes[idx] - itsEndTimes[idx]) < 7)
				{
					ARM_VectorPtr result(new ARM_GP_Vector(states->size()));
	...
	if((idx = IdxFromValue(itsResetTimes, swapResetTime, 7)) != -1)
	{
		...
		else if(itsCalibSecDensities[idx]->getInterestTerms().size() == 1)
		{
			...
			double fwdrate = (*SwapRate(curveName, evalTime, floatStartTime, floatEndTime, fixPayTimes, fixPayPeriods, floatStartTimes, 
								floatEndTimes, floatIntTerms, floatEndTimes, floatIntTerms, ARM_GP_Vector(1,0.),isConstantNotional, states))[0];
			...
			ARM_VectorPtr result(new ARM_GP_Vector(1, swopt * annuity));
        ...
        const ARM_PricingStatesPtr& states) const
{
	...
	if((idx = IdxFromValue(itsResetTimes, fwdResetTime, 7)) != -1)
	{
		if(fabs(fwdEndTime - fwdStartTime + itsStartTimes[idx] - itsEndTimes[idx]) < 7)
		{
			...
			ARM_VectorPtr result(new ARM_GP_Vector(size));
...
void ARM_BGMSV1F::ComputeUnderlyingVarCovar(double fromTime, double toTime, 
											double startTime1, double endTime1, 
											double startTime2, double endTime2,
											ARM_GP_Vector& varcovar, ARM_GP_Vector& swapfwd) const
{
	double asOfDate	= GetAsOfDate().GetJulian();
	...
	double fwdrate1 = (*ARM_PricingModelIR::SwapRate(curveCcy, evalTime, startTime1, endTime1, 
								...
								*floatStartTimes1, *floatPayTimes1, *floatIntTerms1, *floatPayTimes1, *floatIntTerms1,
								ARM_GP_Vector(1,0.),true, states))[0];
...
/*
void ARM_BGMSV1F::getCalibrationData(ARM_GP_Vector& resettimes, ARM_GP_Vector& forwards, ARM_VectorVector& strikes, ARM_VectorVector& vols, ARM_GP_Vector& atmvols, ARM_GP_Vector& weights)
{
	double asof = GetAsOfDate().GetJulian();
...
void ARM_BGMSV1F::finishCalibration(ARM_GP_Vector * calibTimes, ARM_SmileCalibration_Params_Heston& params)
{
	ARM_SmileCalibration_Heston func;

	int k, size = itsResetTimes.size(), idx = 0;
	ARM_GP_Vector calibShifts(size), calibLevels(size), calibRhos(size);
	ARM_GP_Vector strikes, mktvols;
	...
	ARM_GP_Vector calibTheta(size, params.theta()), calibvvol(size, params.nu());
...
void ARM_BGMSV1F::getkthStrikesAndVols(int k, ARM_GP_Vector& strikes, ARM_GP_Vector& vols,double& atmvol)
{
	double price;
	// vol monnaie
...
void ARM_BGMSV1F::GetWeightSwaptionApprox(const string& curveName, double evalTime, const ARM_GP_Vector& floatNotional, 
										  const ARM_GP_Vector& floatStartTimes, const ARM_GP_Vector& floatEndTimes, const ARM_GP_Vector& floatPayPeriods,
										  int idx, double fwdrate, double annuity, ARM_GP_Vector& weight) const
{
...
double ARM_BGMSV1F::GetEquivSwaptionShift(int idx, const ARM_IntVector& idxs, ARM_GP_Vector& weight) const
{
	double shift = 0.;
...
double ARM_BGMSV1F::GetEquivSwaptionLevel(int idx, const ARM_IntVector& idxs, const ARM_GP_Vector& weight, const ARM_GP_Vector * optweight) const
{
	int i, j, k;
	double dt, corr, var = 0.;
	double voli, volj;

	const ARM_GP_Vector * tmpweight = optweight == NULL ? &weight : optweight;
...
double ARM_BGMSV1F::GetEquivSwaptionRho(int idx, const ARM_IntVector& idxs, const ARM_GP_Vector& weight, double swlevel) const
{
	int k;

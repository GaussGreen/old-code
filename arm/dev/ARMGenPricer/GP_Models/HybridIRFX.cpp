		...
		ARM_PricingContext* context) const
{
	...
	{	
		...
		ARM_GP_Vector optionPrice_i(stateSize, 0.), value(stateSize, 0.);
		...
		for(i=0; i<couponSize; i++)
		{
			...
			if (isPerformOrNo)
			{
				if ( fabs(couponMin[i])<1. )
				{
					...
					optionPrice_i	= (*(this)->CallVectorial (invFXName, evalTime, fixingTimes[i], settelmentTimes[i], ARM_GP_Vector (1, 1.0/strike_i), -callPut, payTime, states))*strikes[i];
				} else
					optionPrice_i	= ARM_GP_Vector(stateSize, 0.);
				
			} else {
				strike_i		= strikes[i]+callPut*couponMin[i];

				optionPrice_i	= (*(this)->CallVectorial (modelName, evalTime, fixingTimes[i], settelmentTimes[i], ARM_GP_Vector (1, strike_i), callPut, payTime, states));
			}
		...
		return ARM_GP_VectorPtr( new ARM_GP_Vector(value) );
	} else {
...
{	
	...
	else
	{
		...
		for( int k = 0; k < nbFixing; ++k)
		{
			...
			irModel->UpdateDensityFunctor(irfwd,expiryTime,fwdPeriod*K_YEAR_LEN);
			...
			double price3 = irdensity->Call_Option(irDownBarrier, irfwd, maturity);

		...
		const ARM_PricingStatesPtr& states) const
{
	...
	if(maturityTime - K_NEW_DOUBLE_TOL <= evalTime)
	    ...
	    /// AsOf Df
		return ARM_VectorPtr( new ARM_GP_Vector(nbStates,targetPayoff) );
    ...
    else if(!isValidPayoff)
    {
        ...
        if(evalTime < maturityTime)
        {
            ...
            values = ARM_VectorPtr(new ARM_GP_Vector(nbStates));
            for(i=0;i<nbStates;++i)
                (*values)[i]=zcT/zct*exp(-0.5*zcVar-stateFact * states->GetModelState(i,modelNb));
        }
        else
            values = ARM_VectorPtr(new ARM_GP_Vector(nbStates,1.0));
...
// FIXMEFRED: mig.vc8 (25/05/2007 15:48:07):cast
	...
	ARM_VectorPtr zcEnd,libor(new ARM_GP_Vector(nbStates));
	...
	bool isConstantStrike) const
{
    ...
    ARM_GP_Vector fixAnnuity(nbStates,0.0);
    size_t iFix,nbFixFlows=fixPayTimes.size();
    ARM_GP_Vector decayFixLeg(nbStates,0.0);
    ...
    ARM_GP_Vector decayBasisLeg(nbStates,0.0);
    ARM_GP_Vector basisFlow(nbFixFlows,0.0);
    ...
    ARM_GP_Vector floatLeg(nbStates,0.0);
    ARM_GP_Vector basisAnnuity(nbStates,0.0);
    if(isBasis)
    {
    ...
    ARM_GP_Vector swapRate(nbStates,0.0);
    ARM_GP_Vector swapDecay(nbStates,0.0);
    if(IsDegenerateInHW())
    {
        /// ATM normal vol <=> Zc vol relation + normal price
        for(i=0;i<nbStates;++i)
...
////////////////////////////////////////////////////
void ARM_QModel1F::VolatilitiesAndCorrelations( const ARM_GP_Vector& timeSteps, 
	...
	bool linearVol) const
{
	...
	ARM_GP_Vector times = ((ARM_CurveModelParam&) GetModelParams()->GetModelParam( ARM_ModelParamType::QVol)).GetCurve()->GetAbscisses();
	ARM_GP_Vector values= ((ARM_CurveModelParam&) GetModelParams()->GetModelParam( ARM_ModelParamType::QVol)).GetCurve()->GetOrdinates();
...
void ARM_QModel1F::EulerLocalDrifts( const ARM_GP_Vector& timeSteps,
	...
	const ARM_PricingStatesPtr& states) const
{
	...
	if( IsDegenerateInHW() )
	{
		...
		if(GetNumMethod()->GetPricingDirection() == ARM_NumMethod::GP_BCKWDLOOKING)
		{
			double f0			= (*GetPrecomputedFwds())[timeIdx];
   			double q0			= GetModelParams()->GetModelParam( ARM_ModelParamType::QParameter).GetValueAtPoint(0);
			result				= ARM_GP_VectorPtr( new ARM_GP_Vector(statesSize,0.0) );

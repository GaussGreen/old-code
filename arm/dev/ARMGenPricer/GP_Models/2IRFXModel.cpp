		...
		ARM_GP_Vector& totalVar) const
{
	...
	step = 0.0;
	// Just for test
	ARM_GP_Vector totalVol(nbFwds, 0.);
	double sum(0.);
...
void ARM_2IRFXModel::NumMethodStateLocalGlobalVariances( const ARM_GP_Vector& timeSteps,
	ARM_MatrixVector& localVariances,
	ARM_MatrixVector& variances ) const
{
...
void ARM_2IRFXModel::NumMethodStateGlobalVariances( const ARM_GP_Vector& timeSteps,
	ARM_MatrixVector& variances ) const
{
	ARM_MatrixVector localVariances;
...
ARM_PricingStatesPtr ARM_2IRFXModel::BackwardLookingInit(ARM_NumMethodPtr& numMethod, double firstInductTime)
{
	...
	if( treebase)
	{
		if (numeraire->GetType() == ARM_Numeraire::Cash)
		{
			...
			ARM_GP_Vector defaultTargetPayoffs(0);
			...
			ARM_GP_Vector targetDomDf(nbSteps);
			...
			ARM_GP_Vector forBasisDf(nbSteps);
			for(timeIdx=0;timeIdx+1<nbSteps;++timeIdx)
			{
				nextTime = (*timeSteps)[timeIdx+1];
		...
		****/
			...
			ARM_GP_Vector targetLocalPayoffs(nbSteps);
			for(timeIdx=0;timeIdx<nbSteps;++timeIdx)
			{
				time = (*timeSteps)[timeIdx];
...
void ARM_2IRFXModel::AdjustVolToSchedule(const ARM_GP_VectorPtr& timeSteps, ARM_PricingModelPtr& domModel, ARM_PricingModelPtr& forModel, ARM_PricingModel& fxModel)
{
    ...
    ARM_GP_Vector newTimeSteps(newNbSteps);
    ARM_GP_Vector domVols(newNbSteps);
    ARM_GP_Vector forVols(newNbSteps);
    ARM_GP_Vector fxVols(newNbSteps);
    ...
    const ARM_GP_Vector& domInitTimes =  domInitVol.GetCurve()->GetAbscisses();
    ARM_GP_Vector domTimeSteps(newTimeSteps);
    ...
    const ARM_GP_Vector& forInitTimes =  forInitVol.GetCurve()->GetAbscisses();
    ARM_GP_Vector forTimeSteps(newTimeSteps);
    ...
    const ARM_GP_Vector& fxInitTimes =  fxInitVol.GetCurve()->GetAbscisses();
    ARM_GP_Vector fxTimeSteps(newTimeSteps);
    ...
    ARM_GP_Vector domVolLowerBound(domTimeSteps.size(),(*(domInitVol.GetLowerBound()))[0]);
    ARM_GP_Vector domVolUpperBound(domTimeSteps.size(),(*(domInitVol.GetUpperBound()))[0]);
    ...
    ARM_GP_Vector forVolLowerBound(forTimeSteps.size(),(*(forInitVol.GetLowerBound()))[0]);
    ARM_GP_Vector forVolUpperBound(forTimeSteps.size(),(*(forInitVol.GetUpperBound()))[0]);
    ...
    ARM_GP_Vector fxVolLowerBound(fxTimeSteps.size(),(*(fxInitVol.GetLowerBound()))[0]);
    ARM_GP_Vector fxVolUpperBound(fxTimeSteps.size(),(*(fxInitVol.GetUpperBound()))[0]);
...
ARM_VectorPtr ARM_2IRFXModel::ComputeNumeraireTimes( const ARM_TimeInfoPtrVector& timeInfos ) const
{
	...
	switch( GetNumeraire()->GetType() )
	{
		...
		{
            ARM_GP_Vector* times = new ARM_GP_Vector(nbEvents);
...
ARM_PricingStatesPtr ARM_2IRFXModel::ForwardLookingInit(ARM_NumMethodPtr& numMethod, double firstInductTime)
{
	...
	if(!isLnFwdFx)
	{
		...
		else
		{
			...
			ARM_GP_Vector *modelTimeSteps,*timeSteps;
			...
			const ARM_GP_Vector& domVolTimes = (static_cast< const ARM_CurveModelParam& >(domModel->GetModelParams()->GetModelParam(ARM_ModelParamType::QVol).ToCurveModelParam())).GetCurve()->GetAbscisses();
			timeSteps = MergeSortedVectorNoDuplicates(*(mcMethod->GetTimeSteps()),domVolTimes);

			const ARM_GP_Vector& forVolTimes = (static_cast< const ARM_CurveModelParam& >(forModel->GetModelParams()->GetModelParam(ARM_ModelParamType::QVol).ToCurveModelParam())).GetCurve()->GetAbscisses();
			modelTimeSteps = MergeSortedVectorNoDuplicates(*timeSteps,forVolTimes);
			delete timeSteps;
			timeSteps = modelTimeSteps;

			const ARM_GP_Vector& fxVolTimes = (static_cast< const ARM_CurveModelParam& >(fxModel->GetModelParams()->GetModelParam(ARM_ModelParamType::QVol).ToCurveModelParam())).GetCurve()->GetAbscisses();
			modelTimeSteps = MergeSortedVectorNoDuplicates(*timeSteps,fxVolTimes);
			delete timeSteps;
			timeSteps = modelTimeSteps;

			const ARM_GP_Vector& fxQTimes = (static_cast< const ARM_CurveModelParam& >(fxModel->GetModelParams()->GetModelParam(ARM_ModelParamType::QParameter).ToCurveModelParam())).GetCurve()->GetAbscisses();
			modelTimeSteps = MergeSortedVectorNoDuplicates(*timeSteps,fxQTimes);
			delete timeSteps;
			timeSteps = modelTimeSteps;

			ARM_GP_Vector correlTimes = GetCorrelMatrix()->GetTimes();
			modelTimeSteps = MergeSortedVectorNoDuplicates(*timeSteps,correlTimes);

			/// Erase model steps beyong last event time
			ARM_GP_Vector::iterator pos=modelTimeSteps->end();
			--pos;
...
void ARM_2IRFXModel::IntegratedLocalDrifts( const ARM_GP_Vector& timeSteps,
...
///	Action  : computes the relative and absolute drift
////////////////////////////////////////////////////
void ARM_2IRFXModel::EulerLocalDrifts( const ARM_GP_Vector& timeSteps,
		ARM_GP_MatrixPtr& relativeDrifts,
		ARM_GP_MatrixPtr& absoluteDrifts) const
{
...
/**** No quanto markovian drift correction because already done through the above fct
        ...
        const ARM_GP_Vector& qTimes  = qFxCurve.GetAbscisses();
        const ARM_GP_Vector& qValues = qFxCurve.GetOrdinates();
...
void ARM_2IRFXModel::InitDomCalibration(double evalTime, double maturityTime, const ARM_PricingStatesPtr& states) const
{
	if (GetNumeraire()->GetType() == ARM_Numeraire::Cash)
	{
		...
		domModel.SetTargetPayoff(maturityTime,targetDf,ARM_GP_VectorPtr( new ARM_GP_Vector(1,basisRatio) ));
	}
	...
	const InitCalibrationFunc& initFunc) const
{
	...
	ARM_GP_Vector alreadyKnownTimes; /// to avoid repetitive calls even if calibration will occur once
	ARM_GP_Vector::iterator found;
...
{	
    ...
    /// Compute the option
    return eqFxModel.CallVectorial(modelName,evalTime,expiryTime,settlementTime,strikePerState,callPut,payTime,states,context);
}
        ...
        ARM_PricingContext* context) const
{
	...
	ARM_PricingModelPtr forModel     = (*modelMap)[ForModel]->Model();
	ARM_PricingModelPtr fxModel     = (*modelMap)[FxModel]->Model();
	ARM_QModel1F_Fx* q1fFXModel		 = dynamic_cast<ARM_QModel1F_Fx*>(&*fxModel);
	...
	ARM_VectorPtr rangeAccrual (new ARM_GP_Vector(nbStates));
	...
	//{
		...
		for (size_t i=0; i<nbFixings; i++)
		{
			...
			double stddevFX = sqrt(varFX);
			double stddevFX_Divide_2 = 0.5 * stddevFX;
			...
			double varfwdZCDom = varZCDomEnd + varZcDom - 2*covarZCDomEnd;
			double stddevfwdZCDom=sqrt(varfwdZCDom);
			double stddevfwdZCDom_divide_2 = 0.5 * stddevfwdZCDom;
			...
			rhoj =  covar / (stddevfwdZCDom * stddevFX);


			//State Variable IR		
			double varXDom = domModelParams->StateLocalVariance(evalTime,fixing,fixing);;
			double stddevXDom = sqrt(varXDom);
			double stateFact = domModelParams->BetatT(fixing,irIndexEndTimes[i]);
			
			//Calcul de la barrière IR	
			...
			for (size_t j=0; j<nbStates; j++)
			{
				...
				/*
				//Check formula DF(Tfix,Tfix,Tend) using state variable X and B(t,T) -> give the same strike
				  if (computeZCUpK)
					K_ZC_Up_j	= (log(zcUpBarrier/fwdZC)	+ 0.5*varfwdZCDom) / (stateFact*stddevXDom);
				if (computeZCDownK)
					K_ZC_Down_j = (log(zcDownBarrier/fwdZC)	+ 0.5*varfwdZCDom) / (stateFact*stddevXDom);
	*/

				if (computeZCUpK)
					K_ZC_Up_j	= log(zcUpBarrier/fwdZC)/stddevfwdZCDom + stddevfwdZCDom_divide_2;	
				if (computeZCDownK)
					K_ZC_Down_j = log(zcDownBarrier/fwdZC)	/stddevfwdZCDom + stddevfwdZCDom_divide_2;

				//strikes associés à binaire sur FX
				double fwdFX = (*forwardFX)[j];
				if (computeFXUpK)	
					K_FX_Up_j	= log(fxUpBarrier/fwdFX)	/stddevFX + stddevFX_Divide_2;
				if (computeFXDownK) 
					K_FX_Down_j = log(fxDownBarrier/fwdFX)	/stddevFX + stddevFX_Divide_2;

						
				double proba1=NormalCDF(K_FX_Up_j,K_ZC_Up_j,rhoj);
				double proba2=NormalCDF(K_FX_Up_j,K_ZC_Down_j,rhoj);
				double proba3=NormalCDF(K_FX_Down_j,K_ZC_Up_j,rhoj);
				double proba4=NormalCDF(K_FX_Down_j,K_ZC_Down_j,rhoj);
		...
		) const
{
	...
	ARM_GP_Vector::iterator qxxit = qxx->begin();
	ARM_GP_Vector::iterator qyyit = qyy->begin();
	ARM_GP_Vector::iterator qzzit = qzz->begin();
	ARM_GP_Vector::iterator qxyit = qxy->begin();
	ARM_GP_Vector::iterator qyzit = qyz->begin();
	ARM_GP_Vector::iterator qzxit = qzx->begin();
	ARM_GP_Vector::iterator pxit  = px->begin();
	ARM_GP_Vector::iterator pyit  = py->begin();
	...
	else if(lambda==-2.5)//Tollaly Centred for the rates LogSpotFX
	{
		...
		ARM_GP_Vector::iterator pzit  = pz->begin();
		ARM_GP_Vector::iterator rdit  = rd->begin();
		ARM_GP_Vector::iterator rfit  = rf->begin();
	...
	else if(lambda==-1)//Centred Qmapping
	{
		...
		{
			*pz= 0.0 - *(pz);
			*o = ARM_GP_T_Vector<double>(N,0.0);
		}
		else if(GetNumeraire()->GetType() == ARM_Numeraire::Cash)
		{

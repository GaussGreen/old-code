...
ARM_GP_Vector* ARM_SVMMSpread::ComputeModelTimes(  const ARM_TimeInfoPtrVector& timeInfos )
{
...
void ARM_SVMMSpread::NumMethodStateLocalVariances(const ARM_GP_Vector& timeSteps, ARM_MatrixVector& localVariances ) const
{
...
void ARM_SVMMSpread::NumMethodStateGlobalVariances(const ARM_GP_Vector& timeSteps, ARM_MatrixVector& globalVariances ) const
{
...
void ARM_SVMMSpread::CalcNbEffectiveReset(const ARM_GP_Vector& timeSteps)
{
...
void ARM_SVMMSpread::ModelStateLocalVariancesAndStdDev( const ARM_GP_Vector& timeSteps)
{
...
void ARM_SVMMSpread::ModelStateLocalStdDev( const ARM_GP_Vector& timeSteps, const ARM_MatrixVector& auxLocalCov)
{
...
void ARM_SVMMSpread::NumMethodStateLocalVariancesAndStdDev( const ARM_GP_Vector& timeSteps )
{
...
void ARM_SVMMSpread::ModelStateLocalVariances(const ARM_GP_Vector& timeSteps, ARM_MatrixVector& localVariances ) const
{
...
void ARM_SVMMSpread::setNumericalModelFitter(ARM_NumericalModelFitter * numericalModelFitter)
{
	if(numericalModelFitter)
	{
		...
		ARM_GP_Vector rhos(size), levels(size), vvols(size);	
		...
		((ARM_ModelParamsBGMSV1F*) GetModelParams())->SetParams(itsResetTimes, v0, kappa, theta, ARM_GP_Vector(1,0.), levels, vvols, rhos);

//		((ARM_ModelParamsBGMSV1F*) GetModelParams())->UpdateParamValues(&itsResetTimes, NULL, &levels, &rhos, &ARM_GP_Vector(1, vvol),
//				&ARM_GP_Vector(1,theta), v0);
	...
	const ARM_PricingStatesPtr& states) const
{
	...
	if(fabs(swapLongFloatStartTime - swapShortFloatStartTime) < 7)
	{
		...
		&& fabs(itsEndTimes2[idx] - itsStartTimes[idx] + swapLongFloatStartTime - swapShortFloatEndTime) < 7)
		{
			...
			ARM_VectorPtr result(new ARM_GP_Vector(1, spreadopt * notional * GetZeroCurve()->DiscountPrice(payTime/K_YEAR_LEN)));
...
ARM_GP_Vector ARM_SVMMSpread::GetATMGaussianVol()
{
	int k, size = itsResetTimes.size();

	ARM_GP_Vector gvol(size);

		...
		const ARM_PricingStatesPtr& states ) const
{
	...
	for(i=0;i<nbStates;++i){
		ARM_GP_Vector* column = foreignLeg->GetColumn(i);
	...
	ARM_GP_Vector domFlows(nbStates);
	for(i=0;i<nbStates;++i)
		domFlows[i]= ( beta*(*domZcEnd)[i]*(domNotionalVector)[size-1] - alpha*(*domZcStart)[i] *(domNotionalVector)[0] );

	for(i=0;i<nbStates;++i){
		ARM_GP_Vector* column = domesticLeg->GetColumn(i);
		domFlows[i] += column->sum() ;
		delete column;
	}

	ARM_GP_VectorPtr values(new ARM_GP_Vector(nbStates));
	for(i=0;i<nbStates;++i)
...
{
	return ARM_GP_VectorPtr( new ARM_GP_Vector( 0 ) );
}
...
ARM_GP_Vector* ARM_MultiAssetsModel::ComputeModelTimes( const ARM_TimeInfoPtrVector& timeInfos )
{
	ARM_ModelNameMap::const_iterator 
		iter, 
		start	= itsModelMap->begin(), 
	...
	ARM_GP_Vector* tmpModelTimes=NULL;
	ARM_GP_Vector* result = new ARM_GP_Vector(0);
...
ARM_PricingStatesPtr ARM_MultiAssetsModel::Init(const string& payModelName, const ARM_TimeInfoPtrVector& timeInfos)
{
    ...
    if(!isSpotUse)
    {
		...
		ARM_GP_Vector* ptimeSteps = new ARM_GP_Vector(0), 
...
////////////////////////////////////////////////////
void ARM_MultiAssetsModel::NumMethodStateLocalVariances( const ARM_GP_Vector& timeSteps,
...
///				To be improved!!!
////////////////////////////////////////////////////
void ARM_MultiAssetsModel::NumMethodStateGlobalVariances( const ARM_GP_Vector& timeSteps,
...
void ARM_MultiAssetsModel::VolatilitiesAndCorrelations( const ARM_GP_Vector& timeSteps, 
...
void ARM_MultiAssetsModel::ModelStateLocalVariances( const ARM_GP_Vector& timeSteps,
	ARM_MatrixVector& localVariances ) const
{
	size_t nbSteps = timeSteps.size();
...
void ARM_MultiAssetsModel::EulerLocalDrifts( const ARM_GP_Vector& timeSteps,
		ARM_GP_MatrixPtr& relativeDrifts,
		ARM_GP_MatrixPtr& absoluteDrifts) const
{
	size_t nbSteps	= timeSteps.size();

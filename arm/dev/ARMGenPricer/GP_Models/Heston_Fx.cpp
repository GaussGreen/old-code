...
double alpha,
ARM_GP_Vector& avgTimes,
ARM_GP_Vector& avgLevels
)
{
    ...
    ARM_PricingContext* context) const
{
		 ...
		 || states == ARM_PricingStatesPtr(NULL) )
	{
		...
		ARM_GP_Vector levels = ((ARM_CurveModelParam&)GetModelParams()->GetModelParam(ARM_ModelParamType::Volatility)).GetCurve()->GetOrdinates();
		...
		return ARM_VectorPtr( new ARM_GP_Vector(nbStates,price*zcT) );
	}
	else
	{
    ...
    ARM_PricingContext* context) const
{
		 ...
		 || states == ARM_PricingStatesPtr(NULL) )
	{
		...
		return ARM_VectorPtr( new ARM_GP_Vector(nbStates,digit) );
	}
	else
	{
		ARM_THROW( ERR_INVALID_ARGUMENT, ARM_USERNAME + ": Heston FX can be used just for its closed forms." );
		...
		double strike) const
{
	...
	double strikeUp = strike+CALL_SPREAD_SHIFT_FX;
	double strikeDown = strike-CALL_SPREAD_SHIFT_FX;
	...
	if (domModel && forModel)
	{
		...
		priceUp = pricerUp.price();
		priceDown = pricerDown.price();
	}
	else
	{
		ARM_MixteHestonOptionPricer pricerUp(
	...
	return (priceUp-priceDown)/2.0/CALL_SPREAD_SHIFT_FX;
}

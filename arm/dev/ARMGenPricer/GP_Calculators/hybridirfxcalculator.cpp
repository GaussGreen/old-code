...
ARM_DateStripCombiner ARM_HybridIRFXCalculator::DatesStructure() const
{
	...
	if (itsFundDateStrip.IsNull())
	{
		itsFundDateStrip = ARM_DateStripPtr(new ARM_DateStrip(itsStartDate, itsEndDate,itsFundFreq, itsFundDayCount,resetCalendar,fwdRule, intRule, 
													stubRule,-fabs(spotDays), itsFundFreq,payGap, payCalendar,fundResetTiming, payTiming));
		// past  no call
		nbPastNoCall=0;
...
///	Routine: FXValue
///	Returns: string
///	Action : fill the FX value(s) to pass to FUNDING
/////////////////////////////////////////////////////////////////
...
ARM_1IRFXModel* ARM_HybridIRFXCalculator::CreateModel1IRFX()
{
	...
	/// Create the Q1F model for domestic IR market
	ARM_ZeroCurve* ycDomCurve = dynamic_cast<ARM_ZeroCurve*>(GetMktDataManager()->GetData(YcDomKey));
	models[ARM_1IRFXModel::DomModel] = ARM_PricingModelPtr( new ARM_QModel1F( CreateClonedPtr(ycDomCurve),ARM_ModelParamsQ1F(IRModelParams),true) );
...
void ARM_HybridIRFXCalculator::CreateAndSetModel()
{
	...
	ARM_PathSchemePtr pathScheme(ARM_PathSchemeFactory.Instance()->CreatePathScheme((itsPayOffName == ARM_TARNFXPayoffType::INDIANFX)?
																					ARM_PathScheme::Incremental:
																					ARM_PathScheme::BrownianBridge));

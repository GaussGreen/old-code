...
/////////////////////////////////////////////////////////////////
void ARM_FXRACalculator::FixingsStructure(ARM_Date startDate, ARM_Date endDate, char* ccyName, ARM_IRIndex irIndex, ARM_INDEX_TYPE irIndexType, ARM_HybridIRFX* model)  
{
...
double ARM_FXRACalculator::Price()
{
	...
	//dates
	ARM_GP_Vector* startDates = itsDateStrip->GetFwdRateStartDates();
	ARM_GP_Vector* endDates = itsDateStrip->GetFwdRateEndDates();
	...
	ARM_GP_Vector fxDownBarriers, fxUpBarriers, irDownBarriers, irUpBarriers, notionals;
	...
	for ( size_t i(0); i<size; i++)
	{
		...
		FixingsStructure(startDate, endDate, ccyName, irIndex, irIndexType, model);
		...
		temp = (*interestTerms)[i];
		temp = temp*itsLeverageCv.Interpolate(lag);
		...
		ARM_GP_VectorPtr price = model->RangeAccrualVectorial(
			...
			(*itsIntermediatePrices)[i] = (*price)[0];//storage for ComputeAll

		...
		double strike2)
{
	...
	if ( vanillaType == ARM_FXVanillaType::spread )
		...
		opt =  new ARM_FXQuotient( strike, callPut,fxvanilla2DType, alpha, beta, strike2, isInvG1, isInvG2);
	else
		ARM_THROW( ERR_INVALID_ARGUMENT, ARM_USERNAME+" : Vanilla non computed yet");

	ARM_AutoCleaner<ARM_FXVanilla> HOLDOpt(opt);
	//Gauss Legendre
	return ARM_GaussReplic2D(opt, rho, quantoType);
}

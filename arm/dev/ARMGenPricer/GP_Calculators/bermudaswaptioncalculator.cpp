	...
	itsPreCalibMethod(0)
{
	...
	itsCumProba					 = ARM_VectorPtr(new ARM_GP_Vector(itsNbCalculatedProbabilities,0.0));
...
: ARM_GenCalculator(asOfDate),
	...
	itsFundDateStrip(0),
	itsStructDateStrip(0),
	itsBetasCoeff(new ARM_GP_Vector(NULL))
{
	/// Creation of the Exercise Style for the Exo-Swaption Creation
	ARM_ExerciseStyle* exoDates  = CreateExerciseStyle();
...
ARM_ExerciseStyle* ARM_BermudaSwaptionCalculator::CreateExerciseStyle() const
{
	...
	ARM_GP_Vector* resetDates = calculatedExerciseDates.GetDateStrip(CALL_BS_SCHED)->GetFlowStartDates();
	ARM_GP_Vector* startDates = calculatedExerciseDates.GetDateStrip(CALL_BS_SCHED)->GetFlowStartDates();
...
{	
	...
	ARM_GP_Vector* endDates = FixSched.GetFlowEndDates();
	ARM_GP_Vector* newDates = newSched.GetFlowEndDates();
...
ARM_ReferenceValue* ARM_BermudaSwaptionCalculator::ConvertVariableMarginToStrike(ARM_ReferenceValue& marginCurve )
{
	...
	ARM_GP_Vector* resetDates = FixSched.GetResetDates();
	ARM_GP_Vector* startDates = FixSched.GetFlowStartDates();
	ARM_GP_Vector* endDates = FixSched.GetFlowEndDates();
...
ARM_RowInfo ARM_BermudaSwaptionCalculator::MiddleRows( size_t eventIdx, const ARM_DateStripCombiner& datesStructure ) const
{
	...
	else if (genSecType == 4) // (Zero Coupon)
	{
		...
		ARM_GP_Vector fixSchedVector = *(fixSched.GetFlowStartDates());
		double fixStartDate;
...
ARM_DateStripCombiner ARM_BermudaSwaptionCalculator::DatesStructure() const
{
	...
	if (itsExoSwaption && (itsCallNotice == 9999)) //case swaption loaded from Summit !!!
	{
		...
		ARM_GP_Vector* FlowStartDates		= new ARM_GP_Vector(noticeSize);
		ARM_GP_Vector* FlowEndDates			= new ARM_GP_Vector(noticeSize);
		ARM_GP_Vector* ResetDates			= new ARM_GP_Vector(noticeSize);
		ARM_GP_Vector* FwdRateStartDates	= new ARM_GP_Vector(noticeSize);
		ARM_GP_Vector* FwdRateEndDates	    = new ARM_GP_Vector(noticeSize);
		ARM_GP_Vector* PaymentDates			= new ARM_GP_Vector(noticeSize);
		ARM_GP_Vector* InterestTerms		= new ARM_GP_Vector(noticeSize);
		ARM_GP_Vector* InterestDays			= new ARM_GP_Vector(noticeSize);
...
void ARM_BermudaSwaptionCalculator::ComputeDomesticBasis()
{
	...
	CC_NS(std,auto_ptr)<ARM_GP_Vector> HOLDCVDM(vdomMargin);	

	double asOfDate = GetMktDataManager()->GetAsOfDate().GetJulian();	
	ARM_GP_Vector timeLags(*itsStructDateStrip->GetResetDates());
	timeLags -= asOfDate;
...
ARM_Curve* ARM_BermudaSwaptionCalculator::RefValueToARM_Curve(ARM_ReferenceValue* refVal, ARM_Date& date, ARM_Interpolator<double,double>* interpolator)
{
	...
	ARM_GP_Vector* gpDates  = NULL;
	ARM_GP_Vector* gpValues = NULL;
	...
	if (size == 0)
	{
		gpDates = new ARM_GP_Vector(1, 0.0);
	}
	else
	{
		gpDates  = new ARM_GP_Vector(dates->GetSize(),dates->GetElt());
		*gpDates -= date.GetJulian();
	}

	values   = refVal->GetDiscreteValues();
	gpValues = new ARM_GP_Vector(values->GetSize(),values->GetElt());
...
/////////////////////////////////////////////////////////////////
void ARM_BermudaSwaptionCalculator::ComputeControlVariate(ARM_GP_Vector& cvprices)
{
...
void ARM_BermudaSwaptionCalculator::CreateAndSetModel()
{
	...
	ARM_GP_Vector defaultSigmas(1,GetInitVol());
	ARM_GP_Vector defaultMeanReversion(1,GetInitMrs());
	...
	{
		ARM_GP_Vector defaultBetas(1,GetInitBeta());
		...
		{
			ARM_GP_Vector correlLowerBound(1,CORREL_LOWER_BOUND);
			ARM_GP_Vector correlUpperBound(1,CORREL_UPPER_BOUND);
    ...
    else if(GetModelType()==ARM_PricingModelType::QGM1F)
	{
		ARM_CurveModelParam  paramvol(ARM_ModelParamType::Volatility, &defaultSigmas, &defaultTimes );
		ARM_CurveModelParam  paramMRS(ARM_ModelParamType::MeanReversion, &defaultMeanReversion, &defaultTimes );
		ARM_GP_Vector defaultSkews(1,GetInitSkew());
...
ARM_StdPortfolioPtr ARM_BermudaSwaptionCalculator::SwaptionPortfolio_Frontier()
{
	...
	ARM_GP_Vector* resetDates	= datesStructure.GetDateStrip(CALL_BS_SCHED)->GetResetDates();
	ARM_GP_Vector* startDates	= datesStructure.GetDateStrip(CALL_BS_SCHED)->GetFlowStartDates();
...
void ARM_BermudaSwaptionCalculator::CreateAndSetCalibration_Frontier()
{
	...
	ARM_GP_Vector defaultTimes(1,0.0);
	ARM_GP_Vector defaultSigmas(1,GetInitVol());
	ARM_GP_Vector volLowerBound(1,SIGMA_LOWER_BOUND);
	ARM_GP_Vector volUpperBound(1,SIGMA_UPPER_BOUND);
...
void ARM_BermudaSwaptionCalculator::CreateAndSetCalibration()
{
	...
	else
	{
		...
		if(GetMrsCalibFlag())
		{
			...
			ARM_GP_Vector mrsLowerBound(1,GetInitMrsL());
			ARM_GP_Vector mrslUpperBound(1,GetInitMrsU());
			ARM_GP_Vector initMrs (1,GetInitMrs());		
...
/////////////////////////////////////////////////////////////////
ARM_GP_Vector ARM_BermudaSwaptionCalculator::GetCVPrices()
{
	ARM_StdPortfolioPtr swaptionPortfolio = GetOSWPortfolio();
	size_t nbCv = GetNbControlVariate();
	ARM_GP_Vector CVPrices(nbCv,0.0);
...
double ARM_BermudaSwaptionCalculator::Price()
{
	...
	if ((genSecType == 2) || (genSecType == 4))
	{
		...
		if (itsCalculateProbaFlag && (genSecType == 2))
		{
			...
			ARM_GP_Vector wRows(itsProbaMatrix.rows(),0.0);
	...
	{
		const ARM_GP_Vector cvPrices = GetCVPrices();
		...
		else
		{
			...
			{
				itsBetasCoeff = ARM_VectorPtr(new ARM_GP_Vector(genPricer->GetCVInfo()->GetBetas()));
			}
		}
		ARM_AutoCleaner<ARM_GenPricer> HoldGP(genPricer );

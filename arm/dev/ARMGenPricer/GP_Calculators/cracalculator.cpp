...
itsCorridorLegPrices(ARM_GP_Vector(1,0.0)),
...
/////////////////////////////////////////////////////////////////
ARM_GP_Vector* ARM_CRACalculator::ComputeExerciseDatesAndSetFees() const
{
	ARM_Vector* StartDates = NULL;
	ARM_Vector* fees = NULL;
	ARM_GP_Vector* NewResetDates = NULL;
	...
	try
	{
		...
		if (itsIsPortfolioNoticeDays)
		{
			if (itsOptionPortfolio)
			{
				...
				ARM_GP_Vector* FlowStartDates		= new ARM_GP_Vector();
				ARM_GP_Vector* FlowEndDates			= new ARM_GP_Vector();
				ARM_GP_Vector* FwdRateStartDates	= new ARM_GP_Vector();
				ARM_GP_Vector* FwdRateEndDates		= new ARM_GP_Vector();
				ARM_GP_Vector* PaymentDates			= new ARM_GP_Vector();
				ARM_GP_Vector* InterestTerms		= new ARM_GP_Vector();
				ARM_GP_Vector* ResetDates			= ComputeExerciseDatesAndSetFees();
				...
				ARM_GP_Vector* FlowResetDates		= new ARM_GP_Vector();

				int size = corridor->GetFlowStartDates()->size();
				
				ARM_GP_Vector* InterestDays = new ARM_GP_Vector();
				for (int i=0; i<size; i++)
				{
...
void ARM_CRACalculator::CreateAndSetModel()
{
	try
	{
		...
		if (skipCalib) // for tests
		{
			...
			ARM_GP_Vector values(1, 0.0 ); // MeanRev = 0.0
		...
		else
        {
			...
			ARM_GP_Vector sigmatimes(size_sig); 
			ARM_GP_Vector sigmavalues(size_sig);
			...
			ARM_GP_Vector lowerbound(size_sig, 1.0e-5);
			ARM_GP_Vector upperbound(size_sig, UPPER_INFINITE_BOUND);
			...
			calibParamMrs = ARM_CurveModelParam(ARM_ModelParamType::MeanReversion, 
												  &ARM_GP_Vector(1, (itsMeanRevMin + itsMeanRevMax)/2), 
												  &ARM_GP_Vector(1, 0.0), 
												  "MEANREV", 
												  "STEPUPRIGHT",
												  &ARM_GP_Vector(1, itsMeanRevMin), 
												  &ARM_GP_Vector(1, itsMeanRevMax));

			ModelParams.push_back(&calibParamMrs);

            // To create Beta params
			size_t size_beta = BetaResetDates->GetSize();
			ARM_GP_Vector betatimes(size_beta); 
			ARM_GP_Vector betavalues(size_beta); 
			...
			calibParamBeta = ARM_CurveModelParam(ARM_ModelParamType::Beta, 
											   ...
											   "BETA", 
											   "LINEAR",
											   &ARM_GP_Vector(size_beta, itsBetaMin),
											   &ARM_GP_Vector(size_beta, itsBetaMax)); 
		...
		ARM_GP_Vector truncatorDatas(1,treeParams[2]);
...
void ARM_CRACalculator::Calibrate()
{
	try
	{
		if (!skipCalib)
		{
			...
			ARM_GP_Vector sigmavalues(sizeref);
			int i;
			for(i = 0; i< sizeref; ++i)
			...
			if (itsReCalibMrs == 1) 
			{
				ARM_GP_Vector meanreversion(1, (itsMeanRevMin+itsMeanRevMax)/2);
			...
			if (itsReCalibBeta == 1) 
			{
				ARM_GP_Vector shift = ( (ARM_CurveModelParam&) (&*sfrmModel)->GetModelParams()->GetModelParam(ARM_ModelParamType::Shift)).GetCurve()->GetOrdinates();
				...
				ARM_GP_Vector beta = ( (ARM_CurveModelParam&) (&*sfrmModel)->GetModelParams()->GetModelParam(ARM_ModelParamType::Beta)).GetCurve()->GetOrdinates();
							...
							ARM_VolCurve* BetaSwopt)
{
	...
	ARM_GP_Vector mrs(1, itsMeanRev );
	ARM_GP_Vector breakPointTimes(1, 0.0);

...
{
	ARM_GP_Vector calibTimes(1, 0.0);
...
ARM_StdPortfolioPtr ARM_CraQuantoCalculator::CreateSwaptionPortfolio(ARM_Currency& ccy)
{
	...
	ARM_GP_Vector* resetDates = datesStructure.GetDateStrip(EXER_SCHED)->GetResetDates();
	ARM_GP_Vector* startDates = datesStructure.GetDateStrip(EXER_SCHED)->GetFlowStartDates();

...
public:
	...
	ARM_CRASpreadCalculator( 
		...
		const ARM_GP_Vector& optimResetData = ARM_GP_Vector(0),
...
// FIXMEFRED: mig.vc8 (25/05/2007 14:53:08):missing return type
	...
	inline virtual ARM_ZeroCurve* GetForeignZeroCurve()	const			{ return dynamic_cast<ARM_ZeroCurve*>(GetMktDataManager()->GetData(GetKeys()[YcFundKey]));     };
	inline virtual ARM_ZeroCurve* GetDomesticDiscountZeroCurve() const  { return dynamic_cast<ARM_ZeroCurve*>(GetMktDataManager()->GetData(GetKeys()[YcBasis]));		};
	inline virtual ARM_ZeroCurve* GetForeignDiscountZeroCurve()	const	{ return dynamic_cast<ARM_ZeroCurve*>(GetMktDataManager()->GetData(GetKeys()[YcFundBasis]));   };
	...
	void ComputeCRA2Probabilities(ARM_CorridorDblCondition* dblCorridor,
								  ARM_GP_Vector& probaIn,ARM_GP_Vector& probaInFLT);
	...
	inline virtual ARM_GP_Vector GetvFundNominal() const { return itsvRealFundNominal;};
	inline virtual ARM_GP_Vector GetvFundSpread() const { return itsvRealFundSpread;};
...
private:
	...
	ARM_GP_Vector		itsvRealFundSpread;
	ARM_GP_Vector		itsvRealFundNominal;
	size_t				itsFundSize;
	...
	ARM_GP_Vector itsPayIndexFwd;/// size = itsCpnSize...
	
	// cpn value (no discount)
	ARM_GP_Vector itsFixedCpnValue;
	ARM_GP_Vector itsVarCpnValue;

	ARM_GP_Vector itsFixedCpnLongSensi2;
	ARM_GP_Vector itsFixedCpnShortSensi2;
	ARM_GP_Vector itsVarCpnLongSensi2;
	ARM_GP_Vector itsVarCpnShortSensi2;
	ARM_GP_Vector itsPayIndexFwd2;
	ARM_GP_Vector itsFixedCpnValue2;
	ARM_GP_Vector itsVarCpnValue2;
	ARM_GP_Vector itsFixedValues2;

	ARM_GP_Vector itsFixedCpnLongSensi3;
	ARM_GP_Vector itsFixedCpnShortSensi3;
	ARM_GP_Vector itsVarCpnLongSensi3;
	ARM_GP_Vector itsVarCpnShortSensi3;
	ARM_GP_Vector itsPayIndexFwd3;
	ARM_GP_Vector itsFixedCpnValue3;
	ARM_GP_Vector itsVarCpnValue3;
	ARM_GP_Vector itsFixedValues3;
	...
	ARM_GP_Vector itsOptimResetData;
	void SkipResetDates(const ARM_GP_Vector& resets,const ARM_GP_Vector& payments,

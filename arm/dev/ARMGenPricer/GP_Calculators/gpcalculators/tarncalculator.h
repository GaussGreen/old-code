...
public:
	...
	ARM_TARNCalculator(const ARM_Date& asOfDate,
		...
		ARM_GP_Vector& customResetDates = ARM_GP_Vector(0));
	...
	ARM_GP_Vector GetNbIterations() const { return itsNbIterations; }
	...
	void SetCustomResetDates(ARM_GP_Vector customResetDates){itsCustomResetDates = customResetDates;}
	ARM_GP_Vector const GetCustomResetDates()const{return itsCustomResetDates;}
	...
	bool	FindIdxInVector( const ARM_GP_Vector& timeVector, double time, double daysNb, size_t& idx) const;
	double  FindValueInVector( const ARM_GP_Vector& timeVector, double time, double daysNb ) const;
	...
	/// Vector
	inline virtual ARM_GP_Vector GetvCpnNominal() const  { return itvCpnsNominal;};
	inline virtual ARM_GP_Vector GetvFundNominal() const { return itsvFundNominal;};
	...
	inline virtual ARM_ZeroCurve* GetDomesticDiscountZeroCurve() const  { return dynamic_cast<ARM_ZeroCurve*>(GetMktDataManager()->GetData(GetKeys()[BasisKey]));    };
	inline virtual ARM_ZeroCurve* GetForeignDiscountZeroCurve()	const	{ return dynamic_cast<ARM_ZeroCurve*>(GetMktDataManager()->GetData(GetKeys()[FundBasisKey]));     };
...
private:
	...
	void ComputeControlVariable(ARM_GP_Vector& cvprices);
	...
	ARM_DateStrip* GetCalibSchedule(ARM_GP_Vector*& pStrike) const;

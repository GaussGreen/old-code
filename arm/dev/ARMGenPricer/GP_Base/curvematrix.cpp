...
ARM_CurveMatrix::ARM_CurveMatrix(const ARM_GP_Matrix& matrix)
{
	...
	for (size_t i = 0; i < itsNbRows; ++i)
		for (size_t j = 0; j < itsNbCols; ++j)
		{
			...
			ARM_GP_Vector ord(1,matrix(i,j));
...
ARM_GP_Vector ARM_CurveMatrix::GetTimes() const
{
	ARM_GP_Vector prevVec(0);
	ARM_GP_Vector* newVec;

	for (size_t i = 0; i < itsNbRows*itsNbCols;++i)
	{
		ARM_GP_Vector tmpVec = itsCurves[i].GetAbscisses();

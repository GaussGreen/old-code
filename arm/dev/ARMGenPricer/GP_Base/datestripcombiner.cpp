...
void ARM_DateStripCombiner::MergeData( const MergeFunc& func ) 
{
	...
	{
		ARM_GP_Vector* result = ((*itsContents[0]).*func)();
		itsMergeData = ARM_VectorPtr( static_cast<ARM_GP_Vector*>( result->Clone() ) );
	}
	else
	{
		...
		itsMergeData = ARM_VectorPtr( new ARM_GP_Vector( mergeDataSize, &tmpMergeData[0] ) );
		...
		{
			/// create data
			ARM_GP_Vector* newFlowStartDates	= new ARM_GP_Vector(mergeDataSize);
			ARM_GP_Vector* newFlowEndDates		= new ARM_GP_Vector(mergeDataSize);
			ARM_GP_Vector* newFwdStartDates	= new ARM_GP_Vector(mergeDataSize);
			ARM_GP_Vector* newFwdEndDates		= new ARM_GP_Vector(mergeDataSize);
			ARM_GP_Vector* newResetDates		= new ARM_GP_Vector(mergeDataSize);
			ARM_GP_Vector* newPaymentDates		= new ARM_GP_Vector(mergeDataSize);
			ARM_GP_Vector* newInterestDays		= new ARM_GP_Vector(mergeDataSize);
			ARM_GP_Vector* newInterestTerms	= new ARM_GP_Vector(mergeDataSize);
	
			ARM_GP_Vector* initialVector	= ((*itsContents[i]).*func)();
			...
			{	
				...
				{
					newFlowStartDates->Elt(j)	= (*itsContents[i]).GetFlowStartDates()->Elt(k);

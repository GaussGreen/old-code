/////////////////////////////////////////////////////////////////////////////
// Name:        ihg/src/calibration/voltanhhrpowercalibrator.cpp
// Purpose:     calibrator on a vol tanh and a hr with spot component power
// Created:     2005/02/07
// RCS-ID:      $Id: voltanhhrpowercalibrator.cpp,v 1.10 2006/08/20 09:31:04 wang Exp $
...
void 
VolTanhHRPowerCalibrator::Calibrate
...
{ 
  ...
  //     - find new tanh vol to fit the two derivatives using the
  //       hazard rate
  //     - find new hazard rate (alpha, beta, time components) to fit the 
  //       term structure using using the current tanh vol guess
  ...
  double dS0HR = deriv1.GetSessionData()->GetSpotSharePrice();
  m_pHRSpotComponentPower = make_ptr( new HRSpotComponentPower(0.0, dS0HR) );
  ...
  // a reasonable proxy for alpha in the tanh volatility
  double dVolatility = 0.0;
  try
  {
    // todo: improve the initial guess
  ...
  if ( !bConverged ) 
    throw EXCEPTION_MSG
          (
            ITO33_MAX_ITER,
            TRANS("Cannot calibrate a tanh volatility and spot component"
                  " tanh hazard rate to the specified derivatives and"
                  " termstructure.")
          );

}

bool VolTanhHRPowerCalibrator::IsConverged
                (const finance::Derivative& deriv1, 
                 const finance::Derivative& deriv2, 
                 const finance::TermStructureDerivative& tsDerivs)
{

...
int main()
{
  try
  {
    ...
    // Actually price
    pModel->SetDebugOutputFile("ihg_reset.xml");
    shared_ptr<ModelOutput> output = pModel->Compute(*pReset);

    // Report
    std::cout.precision(15);
    std::cout << "IHG reset test" << std::endl;
    std::cout << "  Issue date = " << pReset->GetBondLikeTerms()->GetIssueDate() << std::endl;
    std::cout << "  Maturity date = " << pReset->GetBondLikeTerms()->GetMaturityDate() << std::endl;

...
int   Hyb4_Par_Yield_t(
            ...
            HYB4_TREE_DATA const* tree_data) /**< (I) Tree data structure   */
{
    ...
    if (ParYield == NULL)
    {
        DR_Error("Hyb4_Par_Yield_t:  ParYield slice pointer to store result of "
                 "ParYield calculation is NULL");
        goto FREE_MEM_AND_RETURN;
    }
    if (Zero == NULL)
    {
        DR_Error("Hyb4_Par_Yield_t: input Zero bank is invalid - "
    ...
    if (   (DayCntFrn   == NULL)
        ...
        || (InterpIndex == NULL))
    {
        DR_Error("Hyb4_Par_Yield_t: could not allocate memory!");
        goto FREE_MEM_AND_RETURN;
    }
    ...
    for (l = 0; l <= NbReset; l++)
    {
        ...
        if (DrDayCountFraction( AccStart,     
                                ...
                                &(DayCntFrn[l])) == FAILURE)
        {
            DR_Error("Hyb4_Par_Yield_t: could not calculate day count fractions!");
            goto FREE_MEM_AND_RETURN;
        }
    ...
    {        
        DR_Error("Hyb4_Par_Yield_t: not enough zeros to calculate the index "

...
int   	Hyb4_Floater_Price(
            ...
            HYB4_DEV_DATA      *dev_data,      /* (I) Data kept for DEV'ing       */
            HYB4_TREE_DATA     *tree_data)     /* (I) Structure of tree data      */
{
    ...
    if (DMode == DISC_1D_NOCUPS)
    {
        ...
        if (PrincipalFlag)
        {
            Floater = (double *)FloaterPtr + tree_data->NodeOffset0[t];
        ...
        if (FixingFlag)	         /* Add fixing  */
        {
            ...
            if (CoS == 'S')	     /* Simple rate */
            {
                ...
                {                                                       
                    for (i = Bottom1; i <= Top1; i ++)
                    {
                        IndexAtNode = Hyb4_GetValueAtNode(IndexDim,IndexPtr,
                                                     i,0,0,0,t,tree_data);
    ...
    else if (DMode == DISC_2D_CUPS || DMode == DISC_2D_NOCUPS)
    {
        ...
        if (PrincipalFlag)
        {
            ...
            for (i = Bottom1; i <= Top1; i ++)
            {
                Floater =(double *)FloaterPtr + tree_data->NodeOffset1[t][i];
    ...
    else if (DMode == DISC_3D_CUPS)
    {
        ...
        if (PrincipalFlag)
        {
            ...
            for (i = Bottom1; i <= Top1; i ++)
            {
                for (j = Bottom2[i]; j <= Top2[i]; j++)
                {
                    Floater =(double *)FloaterPtr + tree_data->NodeOffset2[t][i][j];
                    ...
                    for (i = Bottom1; i <= Top1; i ++)
                    {
                        for (j = Bottom2[i]; j <= Top2[i]; j++)
                        {
                            ...
                            for (k = Bottom3[i][j]; k <= Top3[i][j]; k++)
                            {
                                IndexAtNode = Hyb4_GetValueAtNode(IndexDim,IndexPtr,
                                                             i,j,k,0,t,tree_data);
                                Floater[k] += Outstanding * Zero[k] * (pow 
                                  (1. + (IndexAtNode + FloatSpd), DayCntFtn) - 1.);
                            }                        
                        }  /* for j */
                    }	
        ...
        if (PrincipalFlag)
        {
            ...
            for (i = Bottom1; i <= Top1; i ++)
            {
                for (j = Bottom2[i]; j <= Top2[i]; j++)
                {
                    for (k = Bottom3[i][j]; k <= Top3[i][j]; k++)
                    {
                        Floater =(double *)FloaterPtr + tree_data->NodeOffset3[t][i][j][k];
                        ...
                        for (j = Bottom2[i]; j <= Top2[i]; j++)
                        {
                            for (k = Bottom3[i][j]; k <= Top3[i][j]; k++)
                            {
                                ...
                                for (L = Bottom4[i][j][k]; L <= Top4[i][j][k]; ++L)
                                {
                                    IndexAtNode = Hyb4_GetValueAtNode(IndexDim,IndexPtr,
                                                                 i,j,k,L,t,tree_data);
                                    Floater[L] += Outstanding * DayCntFtn 
                                                    * (IndexAtNode + FloatSpd);
                                }
                            }
                        }  /* for j */	
                        ...
                        for (j = Bottom2[i]; j <= Top2[i]; j++)
                        {
                            for (k = Bottom3[i][j]; k <= Top3[i][j]; k++)
                            {
                                ...
                                for (L = Bottom4[i][j][k]; L <= Top4[i][j][k]; ++L)
                                {
                                    IndexAtNode = Hyb4_GetValueAtNode(IndexDim,IndexPtr,
                                                                 i,j,k,L,t,tree_data);
                                    Floater[L] += Outstanding * Zero[L] * (pow 
                                      (1. + (IndexAtNode + FloatSpd), DayCntFtn) - 1.);
                                }
                            }                        
                        }  /* for j */
          ...
          HYB4_TREE_DATA  *tree_data)        /* (I) Tree data structure           */
{
    ...
    if (!ExerFlag)                                              
    {
        if (Hyb4_Dev (FwdFloater,/* Disc expd value of fwd flaoter starting on */
    ...
    if (FltCpnAccSt <= CurrentDate)
    {
        if (DrDayCountFraction (FltCpnAccSt,
                                ...
                                &FltAccrued) == FAILURE)
        {
            DR_Error("Unable to calculate dcf for flt accrued (Hyb4_FwdFloater_t)");
            goto RETURN;
        }        
    }  /* if */
    ...
    if ((FltCpnAccSt < CurrentDate) && (DMode == DISC_1D_NOCUPS))
    {
        ...
        if (CoS == 'S')
        {
            if (ArreasReset == 'Y')
            {
                if (FltResetFlag)   /* Subtract flt cpn just made  */
                {
                    for (i = Bottom1; i <= Top1; i ++)
                    {
                        IndexAtNode = Hyb4_GetValueAtNode(IndexDim,FltIndex,
                                                     i,0,0,0,t,tree_data);
                ...
                {                          
                    for (i = Bottom1; i <= Top1; i ++)
                    {
                        IndexAtNode = Hyb4_GetValueAtNode(IndexDim,FltIndex,
                                                     i,0,0,0,t,tree_data);
                        ZeroAtNode  = Hyb4_GetValueAtNode(ZeroDim,FltZeroToPmt,
                                                     i,0,0,0,t,tree_data);
        ...
        else
        {
            ...
            else
            {
             ...
             ((DMode == DISC_2D_CUPS) || (DMode == DISC_2D_NOCUPS)))
    {
            ...
            if (ArreasReset == 'Y')
            {
                ...
                else                       /* Stub : Not supported */
                {
                    DR_Error("Accrued not supported for "
                             "comp flt payment (Hyb4_FwdFloater_t)");
                    goto RETURN;
                }        
            }
            else
    ...
    else if ((FltCpnAccSt < CurrentDate) && (DMode == DISC_3D_CUPS))
    {
        if (CoS == 'S')
        {
            if (ArreasReset == 'Y')
    ...
    else if ((FltCpnAccSt < CurrentDate) && (DMode == DISC_4D_CUPS))
    {
        if (CoS == 'S')
        {
            if (ArreasReset == 'Y')
            {
                if (FltResetFlag)     /* Subtract flt cpn just made  */
                {
			        ...
			        IndexAtNodeSlice = (double *)FltIndex + offset;
                                ...
                                for (L = Bottom4[i][j][k]; L <= Top4[i][j][k]; ++L)
                                {
                                    ...
                                    FwdFloaterL[L] -= (IndexAtNode + FloatSpd) 
                                                     * FltCpnDcf * Outstanding;
                                }
                            }  
                        }
                ...
                {                          
			        ...
			        ZeroAtNodeSlice = (double *)FltZeroToPmt + offset;
                                ...
                                {
                                    IndexAtNode = IndexAtNodeSlice[L];
                                    ZeroAtNode  = ZeroAtNodeSlice[L];
                                    FwdFloaterL[L] -= (IndexAtNode + FloatSpd * ZeroAtNode) 
                                                          * FltAccrued * Outstanding ;
                                }                        
                            }
                        }
                ...
                {                         /*             add paid PVed coupon   */
			        ...
			        ZeroAtNodeSlice = (double *)FltZeroToPmt + offset;
                                ...
                                for (L = Bottom4[i][j][k]; L <= Top4[i][j][k]; ++L)
                                {
                                    IndexAtNode = IndexAtNodeSlice[L];
                                    ZeroAtNode  = ZeroAtNodeSlice[L];
                                    FwdFloaterL[L] += (IndexAtNode + FloatSpd) * Outstanding 
                                                    * (FltCpnDcf * ZeroAtNode - FltAccrued);
                                }
                            } 
                        }
                ...
                if (FltResetFlag)         /* Subtract flt cpn just paid  */
                {
			        ...
			        ZeroAtNodeSlice = (double *)FltZeroToPmt + offset;
                                ...
                                for (L = Bottom4[i][j][k]; L <= Top4[i][j][k]; ++L)
                                {
                                    IndexAtNode = IndexAtNodeSlice[L];
                                    ZeroAtNode  = ZeroAtNodeSlice[L];
                                    FwdFloaterL[L] -= (pow(1.+(IndexAtNode + FloatSpd), 
                                                         FltCpnDcf) -1.) * Outstanding;
                                }                        
                            } 
                        }
...
*       Single collaret.
*/
int     Hyb4_Collaret_t (TSLICE      Collaret,     /* (I/O) Collaret              */
                ...
                for (i = Bottom1; i <= Top1; i ++)
                {
                    for (j = Bottom2[i]; j <= Top2[i]; j++)             
                    {
                        for (k = Bottom3[i][j]; k <= Top3[i][j]; k++)               
                        {
                            ...
                            for (L = Bottom4[i][j][k]; L <= Top4[i][j][k]; L++)               
                            {
                                CollaretL[L] = Notional * DayCntFtn 
                                             * MINMAX (IndexL[L] + FloatSpd, FloorRate, CapRate);
                            }
                        }
                    }  /* for j */
                }
                ...
                for (i = Bottom1; i <= Top1; i ++)
                {
                    for (j = Bottom2[i]; j <= Top2[i]; j++)
                    {
                        for (k = Bottom3[i][j]; k <= Top3[i][j]; k++)               
                        {
                            ...
                            for (L = Bottom4[i][j][k]; L <= Top4[i][j][k]; L++)               
                            {
                                CollaretL[L] = Notional * ZeroL[L] * DayCntFtn 
                                             * MINMAX (IndexL[L] + FloatSpd, FloorRate, CapRate);
                            }
                        }
                    }  /* for j */
                ...
                for (i = Bottom1; i <= Top1; i ++)
                {
                    for (j = Bottom2[i]; j <= Top2[i]; j++)
                    {
                        for (k = Bottom3[i][j]; k <= Top3[i][j]; k++)               
                        {
                            ...
                            for (L = Bottom4[i][j][k]; L <= Top4[i][j][k]; L++)               
                            {
                                CollaretL[L] = Notional 
                                             * MINMAX ( pow (1. + IndexL[L] + FloatSpd, DayCntFtn) - 1., CompFloorRate, CompCapRate);
                            }
                        }
                    }  /* for j */
                }  /* for i */
...
*
*       Similar to Hyb4_Floater_Price, but no dev is done here. Also, anything 
*       already stored in FloaterPtr Slice will be overwritten.
*/
int   	Hyb4_Floater_Pmt(
            TSLICE         FloaterPtr,    /* (I/O) Floater                   */
            int            IndexDim,      /* (I) Dimension of the Index slice*/
            TSLICE         IndexPtr,      /* (I) Floating index              */
            TSLICE         ZeroPtr,       /* (I) Zero maturing on nxt pmt    */

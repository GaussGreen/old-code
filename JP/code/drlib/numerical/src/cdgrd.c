/*Translated by FOR_C++, v0.1, on 08/16/90 at 13:10:49 */
...
#endif
/* Structured by FOR_STRUCT, v0.2, on 08/16/90 at 13:10:47
...
void imsl_cdgrd(Mfloat (*fcn) (Mint, Mfloat*, Mfloat*), Mint n, Mfloat xc[],
...
#endif
#else
void imsl_cdgrd(fcn, n, xc, xscale, epsfcn, gc)
	Mfloat           (*fcn) ();
	Mint             n;
...
/*				imsl_ermes(5, 4, "The values for the diagonal scaling matrix must be positive while XSCALE(%(i1)) = %(r1) is given.");
*/
	...
	if (imsl_n1rcd(0) == 0) {
		...
		for (j = 1; j <= n; j++) {
			...
			(*fcn) (n, xc, &fnew2);
			xc[j - 1] = xtempj;
			gc[j - 1] = (fnew1 - fnew2) / (F_TWO * stepsz);
		}
	}
L_9000:
	imsl_e1pop("CDGRD ");

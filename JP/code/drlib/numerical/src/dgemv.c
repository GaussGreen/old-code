...
/* Structured by FOR_STRUCT, v0.2, on 08/01/90 at 15:47:43
    Options SET: fmt=t s=n
  -----------------------------------------------------------------------
    IMSL Name:  DGEMV  (Double precision version)
    ...
    Usage:      CALL DGEMV (TRANS, M, N, ALPHA, A, LDA, X, INCX, BETA, Y,
                            INCY)
    ...
    Arguments:
       ...
       ALPHA  - Double precision scalar multiplier for a matrix-vector
                product.  (Input)
       A      - Double precision array of size M by N.  (Input)
       LDA    - Leading dimension of A exactly as specified in the
                calling routine.  (Input)
       X      - Double precision vector of length (N-1)*IABS(INCX)+1 when
                TRANS is 'N' or 'n' and of length (M-1)*IABS(INCX)+1
                otherwise.
                (Input)
       INCX   - Displacement between elements of X.  (Input)
       BETA   - Double precision scalar multiplier for Y.  (Input)
                When BETA is zero, Y need not be set on input.
                In that case, BETA*Y is defined as the zero vector.
       Y      - Double precision vector of length (N-1)*IABS(INCY)+1 when
                TRANS is 'M' or 'm' and of length (M-1)*IABS(INCY)+1
                otherwise.
                (Input/Output)
 ...
 */
#ifdef ANSI
void imsl_dgemv(Mchar *trans, Mint trans_s, Mint *m, Mint *n, Mdouble *alpha,
                Mdouble *a, Mint *lda, Mdouble *x, Mint *incx, 
	        Mdouble *imsl_beta, Mdouble *y, Mint *incy)
#else
void imsl_dgemv(trans, trans_s, m, n, alpha, a, lda, x, incx,
	   imsl_beta, y, incy)
	Mchar           *trans;
	unsigned        trans_s;
	Mint            *m, *n;
...
*/
		...
		IMSL_TRANS_MUST_EQUAL_N_T_OR_C);
		imsl_e1pop("DGEMV ");
		goto L_9000;
	}

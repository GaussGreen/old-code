...
/* Structured by FOR_STRUCT, v0.2, on 01/08/90 at 17:19:44
    Options SET: fmt=t s=n
  -----------------------------------------------------------------------
    IMSL Name:  SGER  (Single precision version)
    ...
    Usage:      CALL SGER (M, N, ALPHA, X, INCX, Y, INCY, A, LDA)

    Arguments:
       M      - Number of rows in A.  (Input)
       N      - Number of columns in A.  (Input)
       ALPHA  - Scalar.  (Input)
       X      - Vector of length (M-1)*IABS(INCX)+1.  (Input)
       INCX   - Displacement between elements of X.  (Input)
       Y      - Vector of length (N-1)*IABS(INCY)+1.  (Input)
       INCY   - Displacement between elements of Y.  (Input)
       A      - Array of size M by N.  (Input/Output)
 ...
 */
#ifdef ANSI
void imsl_sger(Mint m, Mint n, Mfloat alpha, Mfloat *x, Mint incx, Mfloat *y,
               Mint incy, Mfloat *a, Mint lda)
#else
void imsl_sger(m, n, alpha, x, incx, y, incy, a, lda)
    Mint             m, n;
    Mfloat           alpha, x[];
    Mint             incx;
    Mfloat           y[];
...
*/
	...
	i1x = 1;
	for (j = 1; j <= n; j++) {
		saxpy(m, alpha * y[iy - 1], x, incx, &a[i1x - 1], 1);
		iy += incy;
		i1x += lda;
	}

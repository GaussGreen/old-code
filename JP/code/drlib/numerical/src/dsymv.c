...
/* Structured by FOR_STRUCT, v0.2, on 08/01/90 at 15:48:29
    Options SET: fmt=t s=n
  -----------------------------------------------------------------------
    IMSL Name:  DSYMV (Double precision version)
    ...
    Usage:      CALL DSYMV (UPLO, N, ALPHAM A, LDA, X, INCX, BETA,
                            Y, INCY)
    ...
    Arguments:
       ...
       ALPHA  - Double precision scalar multiplier for the matrix-vector
                product.  (Input)
 ...
 */
#ifdef ANSI
void imsl_dsymv(Mchar *uplo, Mint uplo_s, Mint *n, Mdouble *alpha, Mdouble *a,
                Mint *lda, Mdouble x[], Mint *incx, Mdouble *imsl_beta,
	        Mdouble y[], Mint *incy)
#else
void imsl_dsymv(uplo, uplo_s, n, alpha, a, lda, x, incx, imsl_beta,
	   y, incy)
	Mchar           *uplo;
	Mint            uplo_s;
	Mint            *n;

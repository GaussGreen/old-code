 ...
 */
#ifdef ANSI
void imsl_trncr(Mint nra, Mint nca, Mf_complex *a, Mint lda, Mint nrb, Mint ncb,
                Mf_complex *b, Mint ldb)
#else
void imsl_trncr(nra, nca, a, lda, nrb, ncb, b, ldb)
	Mint             nra, nca;
	Mf_complex          *a;
	Mint             lda, nrb, ncb;
	Mf_complex          *b;
...
*/
	...
	if (nra == nca && lda == ldb) {
		for (i = 1; i <= nca; i++) {
			imsl_ccopy(&nra, A(i - 1, 0), &one, B(i - 1, 0), &one);
		}
		for (i = 1; i <= (nca - 1); i++) {
			temp = nra-i;
			imsl_cswap(&temp, B(i - 1, i), &one, B(i, i - 1), &ldb);
		}
	} else {
		for (i = 1; i <= nca; i++) {
			imsl_ccopy(&nra, A(i - 1, 0), &one, B(0, i - 1), &ldb);
		}
	}
L_9000:
	imsl_e1pop("TRNCR");

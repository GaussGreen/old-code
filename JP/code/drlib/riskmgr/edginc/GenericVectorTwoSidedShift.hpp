/**
 * @file GenericVectorTwoSidedShift.hpp
 *
 *
 */

#ifndef EDG_GENERIC_VECTOR_TWO_SIDED_SHIFT_HPP
#define EDG_GENERIC_VECTOR_TWO_SIDED_SHIFT_HPP
...
/**
 * Template to automate the boilerplate aspects of a
 * VectorShiftTwoSided implementation
 */
template <class TWEAK, bool isAbstract = false>
class GenericVectorTwoSidedShift: public VectorShiftTwoSided,
...
public:
    ...
    GenericVectorTwoSidedShift(double shiftSize): 
        VectorShiftTwoSided(TYPE, NAME, shiftSize) {}

    //// constructor with default shift size
    GenericVectorTwoSidedShift(): 
        VectorShiftTwoSided(TYPE, NAME, DEFAULT_SHIFT) {}

protected:
    //// for derived types
    GenericVectorTwoSidedShift(const CClassConstSP& clazz,
                               const string&        sensName,
                               const double&        shiftSize):
        VectorShiftTwoSided(clazz, sensName, shiftSize) {}
...
private:
    ...
    static void load(CClassSP &clazz){
        ...
        REGISTER(GenericVectorTwoSidedShift, clazz);
        SUPERCLASS(VectorShiftTwoSided);
        IMPLEMENTS(Additive);
        if (!isAbstract){
            ...
            SensitivityFactory::addSens(
                NAME,
                new GenericSensitivityFactory<GenericVectorTwoSidedShift>(),
                new GenericVectorTwoSidedShift(DEFAULT_SHIFT),
                Tweakable::TYPE);
        }
    }

    static IObject* defaultConstructor() {
        return new GenericVectorTwoSidedShift();
    }
};

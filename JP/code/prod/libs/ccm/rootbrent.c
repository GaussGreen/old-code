...
#define TRUE 1
#define FALSE 0
#define GTO_EXPORT(a) a
#define IS ==
#define ABS(a) ((a)>0 ? (a) : -(a))

PRIVATE int secantMethod 
...
static char routine[]="RootFindBrent"; 
...

{
   ...
   if (boundLo >= boundHi)
   {
       DR_Error ("%s: Lower bound (%2.6e) >= higher bound (%2.6e).\n",
   ...
   {
       DR_Error ("%s: Guess (%2.6e) is out of range [%2.6e,%2.6e].\n", 
   ...
   {
       DR_Error("%s: Supplied function failed at point %2.6e.\n",
                 routine, xPoints[0]);
       return FAILURE;
   }
   ...
   if (secantMethod (funcd, data, numIterations,
                     ...
                     xPoints, yPoints, 
                     &foundIt, &bracketed, solution) IS FAILURE)
   {
       return FAILURE;
   }
   else if (foundIt)
   ...
   /* Root is still not bracketed, so try at the upper bound now. */
   {
       ...
       {
           DR_Error("%s: Function values (%2.6e,%2.6e) at bounds\n"
     ...
     double      *solution)     /* (O) X root found */
{
    ...
    for (j=1; j<=numIterations; j++) 
    {
         ...
         */
        ratio = (x3-x1)/(x2-x1);
        if (f3*f31<ratio*f2*f21 || f21 IS 0. || f31 IS 0. || f32 IS 0.) 
        {
    ...
    DR_Error("%s: Maximum number of iterations exceeded.\n", routine);
    return FAILURE;
}    

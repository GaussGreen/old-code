...
DLL_EXPORT(int)
DrlFScanVType(FILE *fp, TVType type, void *valPtr)
{
...
DLL_EXPORT(int)
DrlFPrintVType(FILE *fp, TVType type, void *valPtr)
{
static	char	buf[256];
...
DrlFScanStruct(
	...
	int *line,
	char *toks,
	/* DRL_CVAR_T, char *tagName, TVType varType, void *var,
	 * DRL_CARRAY_T, char *tagName, int *nItems, int nVar,
	 *	TVType varType1, int varAllocType1, void* var1,
	 *	...
	 *	TVType varTypeN, int varAllocTypeN, void* varN,
	 * DRL_CVECTOR_T, char *tagName, int *nItems,
	 *	TVType varType, void* var,
	 * DRL_CDVECTOR_T, char *tagName,
	 *	int *nItems, double **v,
	 * DRL_CDMATRIX_T, char *tagName,
	 *	int *nx, int *ny, double ***v,
	 * DRL_LILARRAY_L, char *tagName, TVType varType, void **var,
	 * 0)
	 */ ...)
{
...
#undef	ERROR
#define	ERROR(str)	{GtoErrMsg("%s: %s (name `%s', line %d)\n",\
	...
	while ((strType = (TVType) va_arg(ap, TVType)) != 0) {
		...
		for (m=0; m<=*nItems-1; m++) {
			...
			if ((DrlFScanString(fp, t0) != SUCCESS) ||
				...
				m, varType[i])) != SUCCESS)) {
			    GtoErrMsg("%s: CARRAY_T `%s': can't read "
		...
		for (i=0; i <= *nItems-1; i++) {
			if (DrlFScanVType(fp, varType[0], 
				...
				!= SUCCESS) {
				GtoErrMsg("%s: can' read DRL_CVECT_T element "
					" #%d/%d (name `%s', line %d)\n",
					routine, i+1, *nItems,
					(tagName != NULL ? tagName : "N/A"),
					*line);
...
#define	ERROR(str)	{GtoErrMsg("%s: %s (name `%s', line %d)\n",\
	...
	while ((strType = (TVType) va_arg(ap, TVType)) != 0) {
		...
		for (i=0; i <= nVar-1; i++) {
			...
			GtoErrMsg("%s: array argument # %d has invalid "
				"type.\n", routine, i);
			goto done;
		    }
		}
	    ...
	    case DRL_CMATRIX_T:
		nx         = (int)      va_arg(ap, int);
		varType[1] = (TVType) va_arg(ap, TVType);
		varFmt[1]  = (char*)    va_arg(ap, char*);
		vtv[1]     = (void*)    va_arg(ap, void*);
		ny         = (int)      va_arg(ap, int);
		varType[2] = (TVType) va_arg(ap, TVType);

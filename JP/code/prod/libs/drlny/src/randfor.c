 ...
 * Author:	C. Daher
 * Revision:	$Id: randfor.c 55172 2006-03-27 20:33:17Z shuxiang.a.liu $
 ************************************************************************/
...
#define	MAXDIM	20
	...
	WRAP_CHECK_VECTOR_LEN(pL, nDim);
	WRAP_CHECK_SCALAR(texpL);
	WRAP_CHECK_VECTOR_LEN(volL, nDim);
	WRAP_CHECK_VECTOR_LEN(corrMatL, nDim*nDim);
	WRAP_CHECK_SCALAR(formL);
	WRAP_CHECK_SCALAR(nSampleL);
	WRAP_CHECK_SCALAR(doStatFlagL);
        WRAP_CHECK_SCALAR(traceL);
	...
	fprintf(stdout, "#\n# MC_FORMULA\n#\n"); fflush(stdout);
        ...
        if (traceL[1] > 0)
            DrlLilVectLoggingFile(
                    ...
                    (traceL[1] == 1 ? "w" : "a"),
                    routine,
                    DRL_LONG_L,         nDimL,          "nDim",
                    DRL_CHAR_BLOCK_L,   whatL,          "model",
                    DRL_DOUBLE_L,       pL,             "forwards",
                    DRL_DOUBLE_L,       texpL,          "maturity",
                    DRL_DOUBLE_L,       volL,           "vols",
                    DRL_DOUBLE_L,       corrMatL,       "corrMatrix",
                    DRL_CHAR_BLOCK_L,   formL,          "formula",
                    DRL_LONG_L,         nSampleL,       "nSample",
                    DRL_LONG_L,         doStatFlagL,    "doStat",
                    DRL_LONG_L,         traceL,         "trace",
                    0);

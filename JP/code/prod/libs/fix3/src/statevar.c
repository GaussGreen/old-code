	...
	char				cMethod)			/* (I) Method to use 'G'eom or 'L'inear */
{
    ...
    {
        DR_Error("Fix3_StateVar_Generate: nb of state variables exceeds limit (200)!");
        goto RETURN;
    }
    ...
    if (NbStates < 2)
    {
        DR_Error("Fix3_StateVar_Generate: nb of state variables must be at least two.");
        goto RETURN;
    }
    ...
    {
        DR_Error("Fix3_StateVar_Generate: max state smaller than min state!");
        goto RETURN;
    }
    ...
    {
        DR_Error("Fix3_StateVar_Generate: could not allocate memory for State!");
        goto RETURN;
    }
	...
	case 'G':	/* Geometric spacing */
				...
				{
					DR_Error("Fix3_StateVar_Generate: Minimum state cannot be zero for geometric interpolation!");
					goto RETURN;
				}
	...
	default:	/* Unknown */
				DR_Error("Fix3_StateVar_Generate: Generation must be either 'L'inear or 'G'eometric.");
				status=FAILURE;
				goto RETURN;
	}

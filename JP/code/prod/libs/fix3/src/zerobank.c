...
#define Zbk_INTERP_ZERO_FF(x)                               \
    ...
    {                                                       \
        ZeroLocal[x] = pow(ZeroR[x], tFactor);              \
    }                                                       \
    else                                                    \
    {                                                       \
        ZeroLocal[x] = ZeroL[x] *                           \
...
#define Zbk_INTERP_ZERO_LC(x)                               \
    ...
    idx = Fix3_CbkGetOffset(ZBK, MatDate, CbkHIGHER);
    if (idx < 0) 
        goto RETURN;
    ...
    if  (!interpON) 
    {
        DR_Error("Fix3_ZbkReadZero: No exact match and interpON is false");
        goto RETURN;
    }
    ...
    if (idx>0)
    {
        ...
        ZeroLSlice = ZeroRSlice; /* ZeroLSlice is not needed, 
                                    but provide a valid pointer anyway */

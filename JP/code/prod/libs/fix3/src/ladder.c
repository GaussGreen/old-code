              ...
              FIX3_TREE_DATA   *tree_data)     /* (I) Tree data structure     */
{
    ...
    if (ResetFlagSt)
    {
        if (PrevStates == NULL ||
            CurrStates == NULL)
        {
            DR_Error("Fix3_LadderSwap_t: No previous or current states on %8ld!", 
                     CurrDate);
            goto RETURN;
        }
...
*/
int     Fix3_LadderStep_t (double      *Step,       /* (I/O) Smooth step          */

...
#define Q3TMX0  2.33520497626869185443E-3    /* Denominator */
#define Q3TMX1  6.05183413124413191178E-2
#define Q3TMX2  5.27905102951428412248E-1
#define Q3TMX3  1.87295284992346047209
#define Q3TMX4  2.56852019228982242072
#define Q3TMX5  1.0
...
{
    return (Q3TMX_INVSQRT2PI * exp (-0.5 * x * x));
}
...
{
   return (x>0.0) ? 1. - 0.5 * Q3TMXExpCErrFcn(0,0,x) : 0.5 * Q3TMXExpCErrFcn(0,0,x);
}
...
double  Q3TMXBSQInt (double  a, double b, double x1, double x2)
{
    double erfc1, erfc2; 
    double erfc = 0; 
    int    sign = 0;

    erfc1 = Q3TMXExpCErrFcn (a,b,x1); 
    erfc2 = Q3TMXExpCErrFcn (a,b,x2);
    ...
    /* Q3TMXNormCum has an extra coefficient of 1/2 */
    erfc *= 0.5;

    /* put back value at infinity as in Q3TMXNormCum */
    if (sign) erfc += sign * exp (b + 0.5 * a * a);
 
    return erfc;
} /* ExpA2DiffQ3TMXNormCum */
    ...
    double *P)                  /* Price & Vega                  */
{
    ...
    if (Y < Q3TMX_MIN_FWD) return FAILURE;

    /* compute vega? */
    vOn = (Q3TMX_PAY_TYPE(I) == Q3TMX_VEGA);

    /* for zero volatility return intrinsic */
    if (s * sqrt(T) < Q3TMX_MIN_VOL) {
        C = MAX(Y-K, 0.);
        V = 0.;

        switch(Q3TMX_COP_TYPE(I)) {
            case Q3TMX_CALL: 
                P[0] = C; 
                if (vOn) P[1] = V; 
                break;
    ...
    if (fabs(Q) > Q3TMX_Q_SHIFT) {
	    ...
	    C = MAX(Y-K, 0.);
            ...
            C   = Y1 * Q3TMXNormCum(d+st) - K1 * Q3TMXNormCum(d-st);        
            if (vOn) V = Y * exp(-(d*d+st*st)/2.) * sqrt(r*T) * Q3TMX_INVSQRT2PI;
	}
    } else {
        st  = s * sqrt(T);
        d   = - (K/Y-1.)/st;

        C   = Y * st * exp(-d*d/2) * Q3TMX_INVSQRT2PI + (Y-K) * Q3TMXNormCum(d);
        if (vOn) V = Y * exp(-(d*d)/2.) * sqrt(T) * Q3TMX_INVSQRT2PI;
    }
    ...
    double *impVol)                   /* (O) Implied BS vol         */
{
    ...
    static char      routine[]="Q3TMXBSQImpVol";
    int              status = FAILURE;
    ...
    do {
        ...
        if (fabs(Pr - price) > scale * PREQERR) {
            dPr = (Pr2 - Pr) / VOLDELTA;
            if (fabs(dPr) < Q3TMX_MQ_RESN) {
                Q3TMXErrMsg("%s: numerical derivative is 0.\n", routine);
                goto RETURN;
            }
    ...
    if (found == TRUE) {
        *impVol = iVol;
        ...
        Q3TMXErrMsg ("%s: exceeded maximum nb iterations.\n", routine);        
        *impVol = -999;
    }
...
double Q3TMXExpCErrFcn (double a, double b, double x)
{
    ...
    if (y < 0.46875) {
        ...
        denominator = Q3TMX0 + z2*(Q3TMX1 + z2*(Q3TMX2 + z2*(Q3TMX3 + z2*(Q3TMX4 +z2*Q3TMX5))));
        erfc = (exp(-y2 + W)/y) * 

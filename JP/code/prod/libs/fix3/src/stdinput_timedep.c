                              ...
                              char const     *FileName)      /* (I) File name including extension */           
{
    ...
    for ( t = 0; t < NbTDInp; t++)
    {
        ...
        mktvol_data->TDInpDate[t] = IRDateFromYMDDate(currExp);
        prevExp = currExp;
    ...
    for (t = 0; t < mktvol_data->NbSmileDates; t++)
    {
        ...
        mktvol_data->SmileDate[t] = IRDateFromYMDDate(mktvol_data->SmileDate[t]);
                     ...
                     FIX3_TREE_DATA   *tree_data)   /* (I) Tree data structure                   */
{
    ...
    if ((mktvol_data->CalibFlag == TRUE) && (mktvol_data->NbTDInp > 1))
    {
        for (i = 0; i < mktvol_data->NbTDInp; i++)
        {
            ...
            {
                DR_Error ("mktvol_data->TDInpDate[%d]=%ld (taken from VNFM dates) "
                    "does not appear in the list of benchmark vol dates !", 

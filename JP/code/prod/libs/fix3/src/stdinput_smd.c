             ...
             char const*   FileNameModelPar)            /* (I) File name model parameters    */
{
    ...
    if (Fix3_Model_Input_Smd (mktvol_data, 
                              tree_data,
    ...
    {
        DR_Error ("SMD model: number of factors must be equal to 2");
        goto RETURN;
    }
    ...
    if (Fix3_Param_Check_Smd (tree_data->NbFactor,
                              mktvol_data,
                              tree_data) == FAILURE)
    {
        goto RETURN;
...
/*****Fix3_Model_Input_Smd  *********************************************************/
/*
* Read time-dependent mean-reversion, correlation, weight input      
*/
int Fix3_Model_Input_Smd (MKTVOL_DATA     *mktvol_data, /* (O) Volatility data               */
                          ...
                          char const      *FileName)    /* (I) File name including extension */           
{
    ...
    if (readerror != 1)
    {      
        sprintf (ErrorMsg, "Could not find Bbq in file %s! (Fix3_Model_Input_Smd)", FileName);
        DR_Error(ErrorMsg);
        goto RETURN;
    }
    ...
    if (readerror != 1)
    {      
        sprintf (ErrorMsg, "Could not find Smile Nb entries in file %s! (Fix3_Model_Input_Smd)", FileName);
        DR_Error(ErrorMsg);
        goto RETURN;
    }
    ...
    if(SmileNbEntries != 1)
    {
        sprintf (ErrorMsg, "Number of entries in Smile section has to be 1 in file %s! (Fix3_Model_Input_Smd)", FileName);
        DR_Error(ErrorMsg);
        goto RETURN;
    }
    ...
    if (readerror != 4)
    {        
        sprintf (ErrorMsg, "Could not read smile info in file %s! (Fix3_Model_Input_Smd)", FileName);
        DR_Error(ErrorMsg);
        goto RETURN;
    }
    ...
    if (Dateok(SmileDate))
    {
        sprintf(ErrorMsg, "Incorrect format for smile date in file %s (Fix3_Model_Input_Smd)!", FileName);
        DR_Error(ErrorMsg);
        goto RETURN;
    }
   ...
   if (FindAndSkipSectionLine (
    ...
    if (readerror != 4)
    {        
        DR_Error("Could not read SMD info in file %s! (Fix3_Model_Input_Smd)", FileName);
        goto RETURN;
    }

                      ...
                      double	        *RateJump)      /* (O) */
{
    ...
    if (0 == strcmp(LoR,"Left"))
        Q = QLeft;
    else if (0 == strcmp(LoR,"Right"))
        Q = QRight;
    else
        DR_Error ("computeMandS: wrong input");


    if (IS_Q(Q))
    {
        *M          = (Amap + Bmap * FwdRateA) / Q;
        *S          = 1 + FwdRateA - (*M);      
        *RateJump   = exp(Q * VolBbq * PreviousJump);
    }
    else
    {
        *M          = Amap + Bmap * FwdRateA;
        *S          = 1 + FwdRateA;
        *RateJump   = (*M) * VolBbq * PreviousJump;
    }
}   /* computeMandS */
...
/*****  getU   ******************************************************************/
/*
*       computes U parameter for the drift equation
*/
double	getU (  double	Q,                              /* (I) */
                double	a,                              /* (I) */
                double	b,                              /* (I) */
                double	FwdRateA,                       /* (I) */
                double	VolBbq)                         /* (I) */
{
    return (a + b * FwdRateA - Q * (1. + FwdRateA)) * VolBbq;
}
...
/*****  ExpansionParameters   ******************************************************/
/*
*       Calculate paramaters for polynomial coefficients.
*/
void	ExpansionParameters (double Q,                  /* (I) */
                            double  a,                  /* (I) */
                            double  b,                  /* (I) */
                            double  VolBbq,             /* (I) */
                            double  FwdRateA,           /* (I) */
                            ...
                            double  *a22)               /* (O) */
{
    ...
    Q1 = Q * VolBbq;
    U = getU (Q, a, b, FwdRateA, VolBbq);

    *a00 =  1.;
    *a10 = -Q1;          *a11 = -U;
    *a20 =  0.5*Q1*Q1;    *a21 =  1.5*U*Q1;    *a22 = U*U;
}   /* ExpansionParameters */



/*****  computePolynomialCoeff   **************************************************/
/*
*       Calculate polynomial coefficients for drift computation.
*/
void	computePolynomialCoeff (
                                double          QLeft,          /* (I) */
                                double          QRight,         /* (I) */
                                double          Amap,           /* (I) */
                                double          Bmap,           /* (I) */
                                ...
                                int             Mid,            /* (I) */
                                int             Top,            /* (I) */
                                double* const   DiscountL,      /* (I) */
                                double* const   StatePrL,       /* (I) */
                                double  P[4])                   /* (O) */
{
    int     i;
    ...
    ExpansionParameters (QLeft,Amap, Bmap, 
                        VolBbq, FwdRateA,
                        &a00,
                        &a10,
                        &a11,
    ...
    for (i = Bottom; i <= Mid; i++)
    {
        ...
        P[2] += (a20 + (a21 + a22 * x) * x) * D0;                  
    }

    ExpansionParameters (QRight, Amap, Bmap, 
                        VolBbq, FwdRateA,
                        &a00,
                        &a10,
                        &a11,
...
*       compute Grid
*/
void	computeDiscFactors (double  Q,                  /* (I) */
                            double  VolBbq,             /* (I) */
                            double  FwdRateA,           /* (I) */
                            double  M,                  /* (I) */
                            double  S,                  /* (I) */
...

{
    ...
    if (IS_Q(Q))
    {
        Grid = M * exp (Q * VolBbq * Zidx);
        
        for (i = Ifirst; i <= Ilast; i++)
        {
            DiscountL[i] = 1. / (S + Grid);
            Grid *= RateJump;
        }           
    }
    else
...
*        
*/
double  Drift_Equation (double          QLeft,          /* (I) */
                        double          QRight,         /* (I) */
                        double          a,              /* (I) */
                        double          b,              /* (I) */
                        double          VolBbq,         /* (I) */
                        ...
                        double          drift)          /* (I) */
{
    int     i;
    double  denominator;
    double  temp = -ZeroCoupon;
    double  U;

    /* Left part */
    U = getU (QLeft, a, b, FwdRateA, VolBbq);

    for (i = Bottom; i <= Mid; i++)
    {
        if (IS_Q(QLeft))
        {
            denominator = (U + QLeft / DiscountL[i]) * exp(QLeft * drift) - U;
            temp += StatePrL[i] * QLeft / denominator;
        }
        else
        {
            temp += StatePrL[i] * DiscountL[i] / (1 + DiscountL[i] * (a + b * FwdRateA) * drift);
        }
    }

    /* Right part */
    U = getU (QRight, a, b, FwdRateA, VolBbq);

    for (i = Mid + 1; i <= Top; i++)
    {
        if (IS_Q(QRight))
        {
            denominator = (U + QRight / DiscountL[i]) * exp(QRight * drift) - U;
            temp += StatePrL[i] * QRight / denominator;
        }
        else
        {
            temp += StatePrL[i] * DiscountL[i] / (1 + DiscountL[i] * (a + b * FwdRateA) * drift);
...
*/
int     computeStatePrices (int             CurrentBottom,      /* (I) */
                            int             CurrentTop,         /* (I) */
                            int             NextBottom,         /* (I) */
                            int             NextTop,            /* (I) */
                            int             OutBottom,          /* (I) */
                            ...
                            double          JumpCoeff,          /* (I) */
                            double* const   DiscountL,          /* (I) */
                            double* const   CurrentStatePrL,    /* (I) */
                            double*         NextStatePrL)       /* (O) */
{
    int     i;
    int     status = FAILURE;
    int     lMin,
    ...
    double  x;
        ...
        for (i = NextBottom; i <= NextTop; i++)
            NextStatePrL[i] = 0.;                        


        for (i = CurrentBottom; i <= CurrentTop; i++)
        {                        
            Pi = (d - BetaLocal) * i;
            ...
            x = CurrentStatePrL[i] * DiscountL[i];

            NextStatePrL[i+1+l] += pu * x;
            NextStatePrL[i  +l] += p0 * x;
            NextStatePrL[i-1+l] += pd * x;
        }
...
*/
int     Fix3_Drift_1D_E2Q ( MKTVOL_DATA     *mktvol_data,       /* (I) Volatility data */
                            FIX3_TREE_DATA  *tree_data)         /* (I/O) Tree data     */
{
    ...
    /* Amap must be updated each time in the loop	*/
    ...
    {   
        ...
        */
        ...
        /*************************************************************************/
        Amap = mktvol_data->Amap * Length[t]; /* need to rescale */
        /*************************************************************************/
        ...
        computeMandS (  QLeft, QRight, Amap, Bmap, "Left", 
                        VolBbq, PreviousJump, FwdRateA,
                        &MLeft,
                        &SLeft,
                        &RateJumpLeft);

        computeMandS (  QLeft, QRight, Amap, Bmap, "Right",
                        VolBbq, PreviousJump, FwdRateA,
                        &MRight,
                        &SRight,
                        &RateJumpRight);
        ...
        */
        ...
        computeDiscFactors (QLeft, VolBbq, FwdRateA, 
                            MLeft, SLeft, RateJumpLeft,
                            Bottom[t], Mid, Zidx,
                            DiscountL);
        ...
        computeDiscFactors (QRight, VolBbq, FwdRateA, 
                            MRight, SRight, RateJumpRight,
                            Mid + 1, Top[t], Zidx,
                            DiscountL);      
        ...
        */
        ...
        computePolynomialCoeff (QLeft, QRight, Amap, Bmap,
                                VolBbq, FwdRateA,
                                ZeroCoupon[t+1],
                                Bottom[t], Mid, Top[t],
                                DiscountL,
        ...
        *   Compute state prices for next period.
        */
        if (computeStatePrices (Bottom[t],
                                Top[t],
                                Bottom[t+1],
                                Top[t+1],
                                OutBottom[t+1],
...
*/
int     Fix3_Drift_2D_E2Q ( MKTVOL_DATA     *mktvol_data,   /* (I) Volatility data */
                            FIX3_TREE_DATA  *tree_data)     /* (I/O) Tree data     */
{
    ...
    {                                   
        ...
        computeMandS (QLeft, QRight, Amap, Bmap, "Left",
                        VolBbq, PreviousJump[2], FwdRateA,
                        &MLeft,
                        &SLeft,
                        &RateJumpLeft2);
        ...
        for (i = Bottom1[t]; i <= Top1[t]; i++)
        {
            ...
            computeDiscFactors (QLeft, VolBbq, FwdRateA, 
                            MLeft, SLeft, RateJumpLeft2,
                            Bottom2[t][i], Mid, Zidx,
                            DiscountL);
        ...
        Q = QLeft * VolBbq;
        U = getU (QLeft, Amap, Bmap, FwdRateA, VolBbq);
        ...
        Q = QRight * VolBbq;
        U = getU (QRight, Amap, Bmap, FwdRateA, VolBbq);
...
*/
int     Fix3_Drift_3D_E2Q ( MKTVOL_DATA     *mktvol_data,   /* (I) Volatility data */
                            FIX3_TREE_DATA  *tree_data)     /* (I/O) Tree data     */
{
    ...
    {                                   
        ...
        computeMandS (QLeft, QRight, Amap, Bmap, "Left",
                        VolBbq, PreviousJump[5], FwdRateA,
                        &MLeft,
                        &SLeft,
                        &RateJumpLeft5);
        ...
        for (i = Bottom1[t]; i <= Top1[t]; i++)
        {
            for (j = Bottom2[t][i]; j <= Top2[t][i]; j++)
            {
                ...
                computeDiscFactors (QLeft, VolBbq, FwdRateA, 
                            MLeft, SLeft, RateJumpLeft5,
                            Bottom3[t][i][j], Mid, Zidx,
                            DiscountL);
                ...
                computeDiscFactors (QRight, VolBbq, FwdRateA, 
                            MRight, SRight, RateJumpRight5,
                            Mid + 1, Top3[t][i][j], Zidx,
                            DiscountL);      

 ...
 */
int     Fix3_KoCap_t (double      *KoCap,       /* (O) Knock out cap              */
                 ...
                 FIX3_TREE_DATA   *tree_data)   /* (I) Tree data structure        */
{
    ...
    if (Fix3_Dev (KoCap,    /* Disc expd value of a cap starting */
    ...
    if (KoFlag)
    {
        KoCapRebateSl  = Fix3_Alloc_Slice (tree_data);
        KoPwayRebateSl = Fix3_Alloc_Slice (tree_data);

        if ((KoCapRebateSl == NULL) || (KoPwayRebateSl == NULL))
        {
            DR_Error("Fix3_KoCap_t: could not allocate memory for rebate slices!");
            goto FREE_MEM_AND_RETURN;
        }
        ...
        if (FltCpnAccSt <= CurrentDate)
        {
            if (DrDayCountFraction (FltCpnAccSt,
                                    ...
                                    &FltAccrued) == FAILURE)
            {
                DR_Error("Fix3_KoCap_t: unable to calculate dcf for flt accrued");
                goto FREE_MEM_AND_RETURN;
            }        
        ...
        if (Fix3_KoOptionVarRebate_t(KoCap,
                                KoIndex,
                                KoCapRebatePtr,
                                1,          /* This is Ko date */
                                LoBarrier,
        ...
        {
            if (Fix3_KoOptionVarRebate_t(KoPway,
                                    KoIndex,
                                    KoPwayRebatePtr,
                                    1,
                                    LoBarrier,
    ...
    if (KoFlag)
    {
        Fix3_Free_Slice (KoCapRebateSl, tree_data);
        Fix3_Free_Slice (KoPwayRebateSl, tree_data);
    }
 ...
 */
int  Fix3_KoSwap_t (double      *KoSwap,       /* (O) Knock out swap              */
               double      *KoPway,       /* (O) Knock out partway           */
               ...
               FIX3_TREE_DATA   *tree_data)    /* (I) Tree data structure         */
{
    ...
    if (Fix3_Dev (KoSwap,   /* Disc expd value of a swap starting */
             t,        /* on the  last ko date               */
             T,
             DCurve,
             dev_data,
    ...
    if (KoFlag)
    {
        KoSwapRebate = Fix3_Alloc_Slice (tree_data);
        KoPwayRebate = Fix3_Alloc_Slice (tree_data);

        if ((KoSwapRebate == NULL) || (KoPwayRebate == NULL))
        {
            DR_Error("Fix3_KoSwap_t: could not allocate memory for rebate slices!");
            goto FREE_MEM_AND_RETURN;
        }
        ...
        if (FltCpnAccSt <= CurrentDate)
        {
            if (DrDayCountFraction (FltCpnAccSt,
                                    ...
                                    &FltAccrued) == FAILURE)
            {
                DR_Error("Fix3_KoSwap_t: unable to calculate dcf for flt accrued!");
                goto FREE_MEM_AND_RETURN;
            }        
            FltAccRatio = FltAccrued / FltCpnDcf;            
        }  
        ...
        if (FixCpnAccSt <= CurrentDate)
        {
            if (DrDayCountFraction (FixCpnAccSt,
                                    ...
                                    &FixAccrued) == FAILURE)
            {
                DR_Error("Fix3_KoSwap_t: unable to calculate dcf for fix accrued!");
                goto FREE_MEM_AND_RETURN;
            }        
        ...
        if ((FltCpnAccSt <= CurrentDate) && (tree_data->NbFactor == 1))
        {
            ...
            if (ArrearsReset == 'Y')
            {
                ...
                {
                    DR_Error("Fix3_KoSwap_t: bond stub is not supported for "
        ...
        if (Fix3_KoOptionVarRebate_t(KoSwap,
                                KoIndex,
                                KoSwapRebate,
                                1,          /* This is Ko date */
                                LoBarrier,
    ...
    if (KoFlag)
    {
        Fix3_Free_Slice (KoSwapRebate, tree_data);
        Fix3_Free_Slice (KoPwayRebate, tree_data);
    }
             ...
             FIX3_TREE_DATA   *tree_data)  /* (I) Structure of tree data          */
{
    ...
    /* if the barries are equal save some effort */
    if( IS_EQUAL (LowBarrier, HighBarrier))
    {
        /* if "out"  then get rebate */
        if (IoO == 'O') 
        {
            if (RebateDim == 0)
            {
                RebateN = Rebate;
                if (Fix3_Set_Slice (KoOpt,
                                    *RebateN,
                                    t,
                                    tree_data) == FAILURE)
                {
...
/*****  Fix3_KoFwdSwap_t  *******************************************************/
/*
 *  Knock out fwd swap function. This is the same as the Fix3_KoSwap_t except
 *  it supports separate fixed and floating stub conventions
 */
int  Fix3_KoFwdSwap_t(double    *KoSwap,       /* (O) Knock out swap              */
                 double    *KoPway,       /* (O) Knock out partway           */
                 double    *FltSwaplet,   /* (I) Flt part of current swaplet */
                 double    *FltZeroToPmt, /* (I) Zero w/ mat on next flt cpn */
                 double    *FixZeroToPmt, /* (I) Zero w/ mat on next fix cpn */
                 ...
                 FIX3_TREE_DATA *tree_data)    /* (I) Tree data structure         */
{
    ...
    if (KoFlag)
    {
        ...
        if ((KoSwapRebate == NULL) || (KoPwayRebate == NULL))
        {
            DR_Error("Fix3_KoFwdSwap_t: could not allocate memory "
                     "for rebate slices!");
            goto FREE_MEM_AND_RETURN;
        }
        ...
        if (FltCpnAccSt <= CurrentDate)
        {
            if (DrDayCountFraction (FltCpnAccSt,
                                    ...
                                    &FltAccrued) == FAILURE)
            {
                DR_Error("Fix3_KoFwdSwap_t: unable to calculate dcf "
                         "for flt accrued!");
                goto FREE_MEM_AND_RETURN;
            }        
            FltAccRatio = FltAccrued / FltCpnDcf;            
        ...
        if ((FltCpnAccSt <= CurrentDate) && (tree_data->NbFactor == 1))
        {
            ...
            if (ArrearsReset == 'Y')
            {
                ...
                {
                    DR_Error("Fix3_KoFwdSwap_t: bond stub is not supported for "
                                "reset-in-arrears swap!");
                    goto FREE_MEM_AND_RETURN;
                }

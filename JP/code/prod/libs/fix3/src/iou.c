            ...
            FIX3_TREE_DATA   *tree_data)   /* (I) Tree data structure          */
{
    ...
    {
        DR_Error("Fix3_Ioucap_t: nb of state variables exceeds limit (200)!");
        goto RETURN;
    }

    if (SMax < SMin)
    {
        DR_Error("Fix3_Ioucap_t: spent amount is bigger than level amount!");
        goto RETURN;
    }
            ...
            FIX3_TREE_DATA   *tree_data)   /* (I) Tree data structure          */
{
    ...
    if (NbStates > MAXNBSTATES)
    {
        DR_Error("Fix3_Iouswap_t: nb of state variables exceeds limit (200)!");
        goto RETURN;
    }
    ...
    if (SwapletFlag)
    {
        if (SMax < SMin)
        {
            DR_Error("Fix3_Iouswap_t: max state smaller than min state!");
            goto RETURN;
        }
        ...
        if (State == NULL)
        {
            DR_Error("Fix3_Iouswap_t: could not allocate memory for State!");
            goto RETURN;
        }

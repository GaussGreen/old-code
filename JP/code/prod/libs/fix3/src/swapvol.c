...
/*****  Fix3_BFactor_Classic    *****************************************/
/*
*       Determine the value of the B coefficient (see Vladimir's memo)
*/
int     Fix3_BFactor_Classic (
        double*         B,            /* (O) B coeff for each factor       */
        ...
        T_CURVE const*  crv)          /* (I) zero curve                    */
{
    ...
    if (status != SUCCESS)
        DR_Error("Fix3_BFactor_Classic failed\n");
...
/*****  Fix3_SpotVol_Classic    *************************************************/
/*
*       Bootstrap one volatility curve to extract spot volatilities.
...
{   
    ...
    */
    ...
    for (i = 0; i < NbVol; i++)
    {
        ...
        if (VolT[i] < 0.0)
        {
            DR_Error("Fix3_SpotVol_Classic: time to expiry in years for "
                     "calibration vol is negative (%lf).  Supplied calibration VolBaseDate = %ld, "
                     "VolDate[%d] = %ld", VolT[i], VolBaseDate, i, VolDate[i]);
            goto RETURN;
        }
    ...
    if (status != SUCCESS)
        DR_Error("Fix3_SpotVol_Classic failed\n");
    ...
    if (CalibFlag == FALSE)
    {
        ...
        for (i = 0; i < NbTP; i++)                                           
        {
            for (k = 0; k < NbAweight; k++)
            {
                ...
                AweightCurve[k][i]*=(1.+FwdRate[i+1])*BbqAdj*Fix3_ExpDecay(Beta[l], Length[i+1]);
            }
        }
...
*/
int     Fix3_Filtered_SpotVol_Classic (
                MKTVOL_DATA *mktvol_data,        /* (I/O) Volatility data            */
                T_CURVE const* crv)              /* (I) zero curve                   */
{
    ...
    */
    ...
    for (i = 0; i < NbVolUsed; i++)
    {
        ...
        if (VolT[i] < 0.0)
        {
            DR_Error("Fix3_Filtered_SpotVol_Classic: time to expiry in years for "
                     "calibration vol is negative (%lf).  Supplied calibration VolBaseDate = %ld, "
                     "VolDate[%d] = %ld", VolT[i], VolBaseDate, i, VolDate[i]);
            goto RETURN;
        }

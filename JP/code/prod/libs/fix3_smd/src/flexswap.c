...
*       Flexi swap price.
*/
int     Fix3_Flexswap_t (double      **Swap,      /* (I/O) Flexi swaps            */
                    ...
                    FIX3_TREE_DATA   *tree_data)  /* (I) Tree data structure      */
{
    ...
    if (SwapL == NULL)
    {
        DR_Error ("Fix3_Flexswap_t: could not allocate memory!");
    ...
    {
        CombCoupon = Fix3_Alloc_Slice (tree_data);
        if (CombCoupon == NULL)
        {
            DR_Error ("Fix3_Flexswap_t: could not allocate memory!");
            goto FREE_MEM_AND_RETURN;        
    ...
    /* Fix3_Dev swaps for relevant outstanding levels */
    for (idx = LastLow; idx <= LastHigh; idx++)
    {
        if (Fix3_Dev (Swap[idx],
                 t,
                 T,
                 DCurve,
                 dev_data,
    ...
    if (Weight[0] || Weight[1])
    {
        for (idx = LastLow; idx <= LastHigh; idx++)
        {
            if (Fix3_LCombTwoSlices(CombCoupon,
                               Coupon[0],
    ...
    if (OptExerFlag)
    {
        ...
        if (tree_data->NbFactor == 1)
        {
            for (idx = 0; idx < NbOuts; idx ++)
            {
                SwapL[idx] = Swap[idx] + Fix3_Node_Offset(1, 0, 0, t, tree_data);
            }
        ...
        else if (tree_data->NbFactor == 2)
        {
            for (idx = 0; idx < NbOuts; idx ++)
            {
                SwapL[idx] = Swap[idx] + Fix3_Node_Offset(2, 0, 0, t, tree_data);
            }
        ...
        else if (tree_data->NbFactor == 3)
        {
            for (idx = 0; idx < NbOuts; idx ++)
            {
                SwapL[idx] = Swap[idx] + Fix3_Node_Offset(3, 0, 0, t, tree_data);
            }
    ...
    if (ResetFlag[0] || ResetFlag[1])
    {
        Fix3_Free_Slice (CombCoupon, tree_data);
    }

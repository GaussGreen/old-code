...
void    Fix3_CbkInit(CLAIM_BANK *CBK)      /* (I/O) the target claim bank   */
{
    if (CBK != NULL)
    {
        CBK->Slices = NULL;
             ...
             FIX3_TREE_DATA  *tree_data)    /* (I) Tree data structure       */
{
    ...
    if ((CBK->Slices   != NULL) ||
        ...
        (CBK->auxSlice != NULL))
    {
        DR_Error("Fix3_CbkAlloc: claim bank is not empty!\n");
    ...
    CBK->auxSlice = Fix3_Alloc_Slice(tree_data);
    ...
    {
        (CBK->Slices)[i] = Fix3_Alloc_Slice(tree_data);
...
FREE_MEM_AND_RETURN:
    ...
    if (status != SUCCESS) 
    {
        DR_Error("Fix3_CbkAlloc: Unable to allocate memory!\n");
        Fix3_CbkFree(CBK, tree_data);
    }
    return (status);
             ...
             FIX3_TREE_DATA  *tree_data)
{
    ...
    if (Fix3_Free_Slice(CBK->auxSlice,tree_data) == FAILURE) goto RETURN;

    if (CBK->Slices != NULL)
    {
        for (i=0;i<CBK->TotNbSlices;i++)
        {
            if (Fix3_Free_Slice(CBK->Slices[i],tree_data) == FAILURE) goto RETURN;
        }
...
RETURN:
    ...
    if (status != SUCCESS) DR_Error("Fix3_CbkFree: Failed!\n");
    return (status);
             ...
             int     NbErDates)         /* (I) size of the usage list     */
{
    ...
    for (i=0; i<NbEvDates; i++)
    {
        ...
        {
            sprintf (ErrorMsg, 
                     "Fix3_CbkSizeFromDL: Earliest usage date #%d (%ld) > "
                     "evaluation date (%ld)!\n",
                     i+1, ErDL[i], EvDL[i]);
            DR_Error (ErrorMsg);
            return(-999);
             ...
             int         ErDateIdx)           /* suppdate idx       */
{
    ...
    MaxSize = Fix3_CbkSizeFromDL(EvDL,
                            NbEvDates,
                            ErDL,
                            NbErDates);
...
/*****  Fix3_CbkDev  *****************************************************/
/*
 *      Fix3_Dev the active elements of the bank
 *      Returns SUCCESS or FAILURE
 */
...
RETURN:
    ...
    if (status == FAILURE) DR_Error("Fix3_CbkDev: Failed!\n");
    return (status);
 ...
 *      The bank is locked for all activities except for Fix3_CbkPushSlice.
 *      Caller is expected to free the slice unless he/she/it pushes it
 *      back to the bank via Fix3_CbkPushSlice
 *      Returns NULL if failed, or pointer to free slice
 */

double  *Fix3_CbkPopSlice(CLAIM_BANK *CBK)
{
...
RETURN:
    ...
    if (status == FAILURE) DR_Error("Fix3_CbkPushSlice: Failed!\n"); 
    return (status);
...
double  *Fix3_CbkReadSlice(CLAIM_BANK *CBK,
                      long        EvDate)
{
    int ofs;

    ofs = Fix3_CbkGetOffset(CBK, EvDate, CbkEXACT);
...
int     Fix3_CbkGetOffset(CLAIM_BANK const *CBK,
                     long              EvDate,
                     int                mode)
{
    int  offset = -999;

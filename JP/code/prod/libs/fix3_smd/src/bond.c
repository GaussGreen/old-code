...
*       Bond price. 
*/
int     Fix3_Bond_t (double     *Bond,           /* (I/O) Bond prices             */
                ...
                FIX3_TREE_DATA   *tree_data)     /* (I) Tree data structure       */
{
    ...
    {
        BondL = Bond + Fix3_Node_Offset(1, 0, 0, t, tree_data);
    ...
    else if (tree_data->NbFactor == 2)
    {
        if (PrincipalFlag)
        {
            for (i = Bottom1; i <= Top1; i ++)
            {
                BondL = Bond + Fix3_Node_Offset(2, i, 0, t, tree_data);
    ...
    else if (tree_data->NbFactor == 3)
    {
        if (PrincipalFlag)
        {
            for (i = Bottom1; i <= Top1; i ++)
                for (j = Bottom2[i]; j <= Top2[i]; j++)
                {
                    BondL = Bond + Fix3_Node_Offset(3, i, j, t, tree_data);
...
*
*/
int     Fix3_FwdBond_t (double      *FwdBond,    /* (O) Forward bond              */
    ...
    */              
    ...
    if (CurrentDate > CpAccStDate)
    {
        if (DrDayCountFraction (CpAccStDate,
                                ...
                                &Accrued) == FAILURE)
        {
            DR_Error("Fix3_FwdBond_t: unable to calculate dcf for accrued!");
            goto RETURN;
        }

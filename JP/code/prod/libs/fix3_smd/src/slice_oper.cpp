...
// slices (rhs), 'count' of result slices (lhs), one or two (for binary) argument 
// slices (x and y) and a function object (fun) the functions do the following:
//
         ...
         FIX3_TREE_DATA const*  tree_data)    /**< (I) Tree data              */
{
    ...
    Fix3_SliceOper(tree_data, t, indicator, index, KOIndicator(low, high, io=='I'));

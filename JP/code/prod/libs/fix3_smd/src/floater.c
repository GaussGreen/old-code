...
*/
int     Fix3_Floater_t ( double      *Floater,      /* (I/O) Floater              */
            ...
            FIX3_TREE_DATA   *tree_data)    /* (I) Tree data structure           */
{
    ...
    if (!ExerFlag)                                              
    {
        if (Fix3_Dev (FwdFloater,/* Disc expd value of fwd flaoter starting on */
    ...
    if (FltCpnAccSt <= CurrentDate)
    {
        if (DrDayCountFraction (FltCpnAccSt,
                                ...
                                &FltAccrued) == FAILURE)
        {
            DR_Error("Fix3_FwdFloater_t: Unable to calculate dcf for flt accrued!");
            goto RETURN;
        }        
    }  /* if */
    ...
    if ((FltCpnAccSt < CurrentDate) && (tree_data->NbFactor == 1))
    {
        ...
        else
        {
            if (ArreasReset == 'Y')
            {
                ...
                {
                    DR_Error("Fix3_FwdFloater_t: accrued not supported for comp flt payment!");
                    goto RETURN;
                }        
            }
            else
...
*/
int     Fix3_CmpFloater_t (double      *CmpFloater,   /* (I/O) Floater           */
                      double      *Index,        /* (I) Floating index      */

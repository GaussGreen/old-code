...
int     Fix3_Cet_Main 
            (int            CetOutputFlag,     /* (I) On TRUE write to file */          
             ...
             FIX3_TREE_DATA      *tree_dataProd)    /* (I) Tree data             */
{
    ...
    for (IterIdx = 1; IterIdx <= mktvol_dataProd->CetNbIter; IterIdx++)
    {
        ...
        for (i=0; i<NbInstr; i++)
        {
            if (mktvol_data.VolUsed[i]) 
            {
                ...
                if (Smd_IndexVol(&(cet_out_data.FilterVol[i]),
                             mktvol_data.SwapSt[i],
                             mktvol_data.SwapMat[i],
                             mktvol_data.Freq,
                             mktvol_data.DCC,
    ...
    {        
        if (mktvol_data.VolUsed[i])
        {
            if (fabs(cet_out_data.PriceDiffInVega[i]) > CET_ERROR_TOLERANCE)
            {
                ...
                DR_Error("Fix3_Cet_Main Error: Final CET vol has not converged to within %4.2lf vega!\n"
...
int     Fix3_Cet_Schedule 
            (long            ValueDate,      /* (I) Value date             */
             ...
             FIX3_TREE_DATA       *tree_dataProd, /* (I) Tree data for product  */
...

{
    ...
    if (NbInstr > NBCRITDATE)
    {
        DR_Error("The nb of swaptions to calibrate exceeds the pre-set\n"
                 "memory allocated for it. (Fix3_Cet_Schedule)\n");
        goto FREE_MEM_AND_RETURN;
    }
    ...
    if (CritDate == NULL)
    {
        DR_Error("Fix3_Cet_Schedule: unable to allocate memory for critical "
                    "dates array !");
        goto FREE_MEM_AND_RETURN;
    }
    ...
    if (status ==FAILURE)
    {
        DR_Error("Fix3_Cet_Schedule: Failed.\n");
    }

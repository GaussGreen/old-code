	...
	FIX3_TREE_DATA			*tree_data)			/* (I) Tree data structure							*/
{
    ...
    {
        DR_Error("Fix3_ZeroBinary_t: nb of state variables exceeds limit (200)!");
        goto RETURN;
    }
        ...
        if (tree_data->NbFactor == 1)
        {
            ...
            {                
                ...
                for (s = 0; s < NbStates; s++)
                {
					...
					ZBPayoff[s] = Fix3_StateVar_Interp(i, NbStates, NotlsTPlus1, ZeroBinaryL, D, NewNotional);
                } /* for state idx */
            ...
            for (i = Bottom1; i <= Top1; i ++)
            {
                ...
                for (j = Bottom2[i]; j <= Top2[i]; j++)
                {
                    ...
                    for (s = 0; s < NbStates; s++)
                    {
						...
						/* Work backwards to the payoff */
						ZBPayoff[s] = Fix3_StateVar_Interp(j, NbStates, NotlsTPlus1, ZeroBinaryL, D, NewNotional);
                    } /* for state idx */
                ...
                for (j = Bottom2[i]; j <= Top2[i]; j++)
                {
                    ...
                    for (k = Bottom3[i][j]; k <= Top3[i][j]; k++)
                    {
                        ...
                        for (s = 0; s < NbStates; s++)
                        {
							...
							/* Work backwards to the payoff */
							ZBPayoff[s] = Fix3_StateVar_Interp(k, NbStates, NotlsTPlus1, ZeroBinaryL, D, NewNotional);
                        } /* for state idx */

              ...
              FIX3_TREE_DATA   *tree_data)     /* (I)Tree data structure       */
{
    ...
    if (*PrevState == NULL)
    {
        DR_Error("Fix3_IRRLoanSwap_t: No previous states !");
        goto RETURN;
    }
    ...
    {
        DR_Error("Fix3_IRRLoanSwap_t: nb of state variables exceeds limit (200)!");
        goto RETURN;
    }
    ...
    if (ResetCx)
    {
        ...
        {
            DR_Error("Fix3_IRRLoanSwap_t: max state smaller than min state!");
            goto RETURN;
        }
        ...
        if (State == NULL)
        {
            DR_Error("Fix3_IRRLoanSwap_t: could not allocate memory for State!");
            goto RETURN;
        }

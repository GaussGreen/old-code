...
static  char    swapsmFnam[]  = "swapsm.dat";
static  char    basesmFnam[]  = "basesm.dat";
static  char    xparamFnam[]  = "xsmileparameters.dat";
...
*/
	...
	(*drWrapper)->fSwapSmile    = NULL;
	(*drWrapper)->fBaseSmile    = NULL;
	...
	if (DrlTSwaptionMatrix2DFileRead(&(*drWrapper)->fCmsSwMat, fnam,
...
#define	READ_DATA(type,ptr,str)	\
	...
	if (DrlTCurveLondonBaseVolFileRead(&((*drWrapper)->fBvCurve), fnam)
			!= SUCCESS) goto done;
    ...
    /*
	 * Read TMX model parameters
	 */
	if (pathdir != NULL) 
    {
		sprintf(fnam, "%s/%s", pathdir, xparamFnam);
	} 
    else 
    {
		strcpy(fnam, xparamFnam);
	}
    
    if ((fp = fopen(fnam, "r")) != NULL) 
    {
       
        READ_DATA(DRL_DOUBLE_T, &((*drWrapper)->MQNormT),
            "MQ NormT");
        READ_DATA(DRL_DOUBLE_T, &((*drWrapper)->MQNck),
            "MQ Nck");
        READ_DATA(DRL_DOUBLE_T, &((*drWrapper)->MQNbDevL),
            "MQ NbDevL ");
        READ_DATA(DRL_DOUBLE_T, &((*drWrapper)->MQNbFwdL),
            "MQ NbFwdL");
        READ_DATA(DRL_DOUBLE_T, &((*drWrapper)->MQBinMapStdL),
            "MQ Bin Map Std Left");
        READ_DATA(DRL_DOUBLE_T, &((*drWrapper)->MQBinMapStdR),
            "MQ Bin Map Std Right");
        (*drWrapper)->MQParamFlag = TRUE;
        fclose(fp); fp = NULL;
    }
    else
	{
        (*drWrapper)->MQParamFlag = FALSE;
	}

	/* TMX Swaption Smile */
	if (pathdir != NULL) {
		sprintf(fnam, "%s/%s", pathdir, swapsmFnam);
	} else {
		strcpy(fnam, swapsmFnam);
	}
    ...
    else if (DrlTSwaptionMatrix2DFileRead_Mod(&(*drWrapper)->fSwapSmile, fnam,
		    TSWAPTION_MATRIX_FMT_LON, NBMQSMLPAR) != SUCCESS) 
                goto done;
    
    /* TMX Base Smile */
	if (pathdir != NULL) {
		sprintf(fnam, "%s/%s", pathdir, basesmFnam);
	} else {
		strcpy(fnam, bvFnam);
	}
    if (fopen(fnam, "r") == NULL)
	...
	else if (DrlTCurveLondonBaseVolFileRead_Mod(&((*drWrapper)->fBaseSmile), fnam, 
            NBMQSMLPAR) != SUCCESS) goto done;
...
DriTDrWrapperDataFree(TDrWrapperData *drWrapper)
{
	if (drWrapper != NULL) {
        ...
        /* TMX smile */
        if (drWrapper->fBaseSmile != NULL)
        {
            for (i = 0; i < NBMQSMLPAR; i++)
            {
                GtoFreeTCurve(drWrapper->fBaseSmile[i]);
            }
            FREE_ARRAY(drWrapper->fBaseSmile);
        }
        if (drWrapper->fSwapSmile != NULL)
        {
            for ( i = 0; i < NBMQSMLPAR; i++)
            {
                DrlTSwaptionMatrix2DFree(drWrapper->fSwapSmile[i]);
            }
            FREE_ARRAY(drWrapper->fSwapSmile);
        }
		FREE(drWrapper);
	}
	return(SUCCESS);

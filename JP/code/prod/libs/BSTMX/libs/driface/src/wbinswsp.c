...
#include "driwbsp.h"		/* Prototype consistency */
...
{
static	char	routine[] = "DrlSpreadBarrierBinary";
	int	status = FAILURE;
	...
	spotY1 = spotYDisc - spotYIndx;
	fwdY1  = fwdYDisc  - fwdYIndx;
	...
	DrlFPrintStruct(NULL, NULL, '\t',
	    DRL_CVAR_T, "fwdYDisc",     DRL_PERCENT_T, (void*) &fwdYDisc,
	    DRL_CVAR_T, "spotYDisc",    DRL_PERCENT_T, (void*) &spotYDisc,
...
/*f---------------------------------------------------------------------
 * Dr-Wrapper for {\tt DrlSpreadBarrierBinary}.
 * The argument {\tt dataFnam} specifies the name of the file
 * containing the data (if it is NULL, the default name
 * "wbinswsp.dat" is used.
 * Returns SUCCESS/FAILURE.
 */

int
DriSpreadBarrierBinaryW(char *dataFnam)
{
static	char	routine[] = "DriSpreadBarrierBinaryW";
	int	status = FAILURE;
	...
	FILE		*fp = NULL;
static	char		defDataFnam[] = "wbinswsp.dat";

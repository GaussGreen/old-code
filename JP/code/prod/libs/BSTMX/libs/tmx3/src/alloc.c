...
int     Tree_Free ( TREE_DATA   *tree_data) /* Tree building data structure */
{
    ...
    {
        Free_Slice(tree_data->LastZero, tree_data);
    }
...
void    MktVol_Init (MKTVOL_DATA    *mktvol_data)
{
    ...
    /* To print out Nmr Schedule   */
    mktvol_data->Trace             = 1;

    /* Set default for Nb Cet Iterations (will be overwritten IF there is    */
    /* a modelparameters.dat file)                                           */
    mktvol_data->CetNbIter         = DEFAULT_CETNBITER;

    
    /* Initialise the libor calibration flag     */

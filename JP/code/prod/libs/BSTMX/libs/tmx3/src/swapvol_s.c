...
*/
double  TMX_ExpDecay (  double  a,
                    double  t)
{
    double  x;


    /* If a=0 or t=0, TMX_ExpDecay(a,t)=1 */
    if (fabs (a * t) < ERROR)
    {
        return (1.);
    }
       ...
       long    ZeroBaseDate)          /* (I) Zero curve base date            */
{
    ...
    {       
        ...
        M = D[0][0] * D[0][0] * TMX_ExpDecay (2. * Beta[0], t) * t;
        ...
        L[0][0] += lambda * lambda * TMX_ExpDecay (2. * Beta[0], t) * t;
    ...
    for (i=1 ;i < NbNmr - 1; i++)
    {
        ...
        {
            L[0][0]            *= exp (-2. * Beta[0] * t);
            L[0][0]            += Aweight[0][j] * Aweight[0][j] * TMX_ExpDecay (2. * Beta[0], t) * t;       
        }
        else if (NbFactor == 2)
        {
            ;
          ...
          long          ZeroBaseDate)         /* (I) Zero curve base date         */
{
    ...
    for (i = 0; i < NbVol; i++)
    {
         ...
         */
        L[0][0] *= exp (-2. * Beta[0] * t);
        L[0][0] += Aweight[0][i] * Aweight[0][i] * TMX_ExpDecay (2. * Beta[0], t) * t;

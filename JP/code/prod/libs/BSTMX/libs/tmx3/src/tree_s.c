                        ...
                        double  *LengthJ)   /* (I) Time step for jump size */
{
    ...
    {   
        ...
        Var1      += Aweight[0][t] * Aweight[0][t] * Length[t] * TMX_ExpDecay (2. * Beta[0], Length[t]);
        ...
        {  
            ...
            Var2 += Aweight[1][t] * Aweight[1][t] * Length[t] * TMX_ExpDecay (2. * Beta[1], Length[t]);
            Var2 += Aweight[2][t] * Aweight[2][t] * Length[t] * TMX_ExpDecay (2. * Beta[1], Length[t]);

            Vol2 = sqrt (Var2);

            Cov1 *= exp (- (Beta[0] + Beta[1]) * Length[t]);
            Cov1 += Aweight[0][t] * Aweight[1][t] * Length[t] * TMX_ExpDecay (Beta[0] + Beta[1], Length[t]);
        ...
        {    
            ...
            Var3 += Aweight[3][t] * Aweight[3][t] * Length[t] * TMX_ExpDecay (2. * Beta[2], Length[t]);
            Var3 += Aweight[4][t] * Aweight[4][t] * Length[t] * TMX_ExpDecay (2. * Beta[2], Length[t]);
            Var3 += Aweight[5][t] * Aweight[5][t] * Length[t] * TMX_ExpDecay (2. * Beta[2], Length[t]);

            Vol3 = sqrt (Var3);

            Cov2 *= exp (- (Beta[0] + Beta[2]) * Length[t]);
            Cov2 += Aweight[0][t] * Aweight[3][t] * Length[t] * TMX_ExpDecay (Beta[0] + Beta[2], Length[t]);

            Rho2   = Cov2 / Vol1 / Vol3;

            Cov3 *= exp (- (Beta[1] + Beta[2]) * Length[t]);
            Cov3 += Aweight[1][t] * Aweight[3][t] * Length[t] * TMX_ExpDecay (Beta[1] + Beta[2], Length[t]);
            Cov3 += Aweight[2][t] * Aweight[4][t] * Length[t] * TMX_ExpDecay (Beta[1] + Beta[2], Length[t]);
                    ...
                    TREE_DATA       *tree_data)     /* (O) Tree data       */
{
    ...
    if (SpotVol (       mktvol_data->Aweight,
                        mktvol_data->NmrLibVol,
                        mktvol_data->NbNmr,
                        mktvol_data->NmrDate,
                        mktvol_data->SmlLiqDate,

...
static int Nmr_t(double **,double *,int,int,MQDATA *,MKTVOL_DATA *,TREE_DATA *);                  
static int Nmr_Print0(MKTVOL_DATA *);
static int Nmr_Print1(MKTVOL_DATA *,MQDATA *);
static int Nmr_Vanl(long,MQDATA *,MKTVOL_DATA *, TREE_DATA *, T_CURVE *);
                  ...
                  MKTVOL_DATA *mvd)            /* (I/O) Mkt vol data      */
{
    ...
    for (i=0; i < NbExpiry - 1; i++)
    {
        ...
        while (LibMatDate < mvd->SwapSt[i+1]) 
        {
            ...
            /* could potentially result in dccfrac = 0 if DCC = 3 */        
            /* & MatDate = 31st. Assumes intvl > 1 day            */
            if (LibMatDate >= mvd->SwapSt[i+1] - 2)
                LibMatDate -= 1L;
               ...
               TREE_DATA   *tree_data)       /* (I/O)  Tree data         */
{
    ...
    tree_data->LastZero = Alloc_Slice(tree_data);
...
int Nmr_Vanl (long        ValueDate,       /* (I) Value date              */
              MQDATA      *NmrMQ,          /* (O) MultiQ for vanilla      */
              MKTVOL_DATA *mvd,            /* (I/O) Mkt vol data          */
              TREE_DATA   *tree_data,      /* (I) Tree data structure     */
              T_CURVE     *t_curve)        /* (I) T_curve data structure  */
{
    int     NbNmr, i, s;
    int     CvDiff;
    int     status = FAILURE;         /* Error status = FAILURE initially */  
   ...
   /* Compute terminal zero      */
    mvd->TermZero0 = ZeroPrice (mvd->TermDate,
                                ValueDate,
    ...
    for (i = 1; i < NbNmr - 1; i++) 
    {
        ...
        /* Get Par Libor */     
        if (Par_Yield_From_Dates (&(mvd->ParLibor0[i]),
                                  &(mvd->AnnuityLibor0[i]),
                                  mvd->NmrDate[i],
                                  mvd->NmrDate[i+1],
                                  mvd->DCC,
              ...
              TREE_DATA            *tree_data)     /* (O) Tree data        */
{
    ...
    for (t = T; t >= 0; t--)
    {
        ...
        if (NmrFlag)
        {
            ...
            {
                FirstZero = ZbkReadZero (&ZeroBank,

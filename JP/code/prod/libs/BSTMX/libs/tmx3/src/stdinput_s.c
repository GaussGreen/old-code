            ...
            char           *File)           /* (I) File name                 */
{
    ...
    if (TMX_FindAndSkipComLine (stream, "frequency", Code, File) == FAILURE) 
        goto RETURN;
    ...
    if (TMX_FindAndSkipComLine (stream, "nb of expiries", Code, File) == FAILURE)
        goto RETURN;
    ...
    {
        if (TMX_FindAndSkipComLine (stream, "vol info", Code, File) == FAILURE)
            goto RETURN;
            ...
            char           *File)           /* (I) File name                 */
{
    ...
    if (TMX_FindAndSkipComLine (stream, "number of tenors", Code, File) == FAILURE)
        goto RETURN;
    ...
    for (s = 0; s <= idxHi-idxLo; s++)
    {
        if (TMX_FindAndSkipComLine (stream, "vol matrix", Code, File) == FAILURE)
        goto RETURN;
             ...
             char          *FileNameXSMILE)    /* (I) File name for XSMILE   */
{
    ...
    if (stream == NULL)
    {
        ...
        if (readerror != 1)
        {        
            sprintf (ErrorMsg, "Could not find file %s: Ppy overwrite required! (TMX_Param_Input)", FileName);
            DR_Error(ErrorMsg);
            goto RETURN;
        }
        ...
        if (NbFactor == 1)
        {
            ...
            if (readerror != 1)
            {        
                sprintf (ErrorMsg, "Could not find file %s: factor weight overwrite required! (TMX_Param_Input)", FileName);
                DR_Error(ErrorMsg);
                goto RETURN;
            }
            ...
            if (readerror != 1)
            {        
                sprintf (ErrorMsg, "Could not find file %s: mean reversion overwrite required! (TMX_Param_Input)", FileName);
                DR_Error(ErrorMsg);
                goto RETURN;
            }
        ...
        else if (NbFactor == 2)
        {
            ...
            if (readerror != 1)
            {        
                sprintf (ErrorMsg, "Could not find file %s: correlation overwrite required! (TMX_Param_Input)", FileName);
                DR_Error(ErrorMsg);
                goto RETURN;
            }
        ...
        if (readerror != 1)
        {      
            sprintf (ErrorMsg, "Could not find file %s: Bbq overwrite required! (TMX_Param_Input)", FileName);
            DR_Error(ErrorMsg);
            goto RETURN;
        }
    ...
    {        
        ...
        if (NbFactor == 1)
        {
            ...
            if (TMX_FindAndSkipComLine (stream, "one factor mean reversion", "TMX_Param_Input", FileName) == FAILURE)
            {        
                goto RETURN;
            }
            ...
            if (readerror != 1)
            {        
                sprintf (ErrorMsg, "Could not find mean reversion in file %s! (TMX_Param_Input)", FileName);
                DR_Error(ErrorMsg);
                goto RETURN;
            }
            ...
            if (strstr (OWS[3], "nil") == NULL)  /* Need to use strstr() here, strcmp won't do */
            {
                ...
                if (readerror != 1)
                {        
                    sprintf (ErrorMsg, "Could not read overwrite string for mean reversion! (TMX_Param_Input)");
                    DR_Error(ErrorMsg);
                    goto RETURN;
                }
            }

            if (TMX_FindAndSkipComLine (stream, "one factor weight", "TMX_Param_Input", FileName) == FAILURE)
            {        
                goto RETURN;
            }
            ...
            if (readerror != 1)
            {        
                sprintf (ErrorMsg, "Could not find factor weight in file %s! (TMX_Param_Input)", FileName);
                DR_Error(ErrorMsg);
                goto RETURN;
            }
            ...
            if (strstr (OWS[2], "nil") == NULL)
            {
                ...
                if (readerror != 1)
                {        
                    sprintf (ErrorMsg, "Could not read overwrite string for factor weight! (TMX_Param_Input)");
                    DR_Error(ErrorMsg);
                    goto RETURN;
                }
            }

            if (TMX_FindAndSkipComLine (stream, "one factor ppy", "TMX_Param_Input", FileName) == FAILURE)
            {        
                goto RETURN;
            }
            ...
            if (readerror != 1)
            {        
                sprintf (ErrorMsg, "Could not find Ppy in file %s! (TMX_Param_Input)", FileName);
                DR_Error(ErrorMsg);
                goto RETURN;
            }
            ...
            if (strstr (OWS[0], "nil") == NULL)
            {
                ...
                if (readerror != 1)
                {        
                    sprintf (ErrorMsg, "Could not read overwrite string for Ppy! (TMX_Param_Input)");
                    DR_Error(ErrorMsg);
                    goto RETURN;
                }
            }
            ...
            for (i = 0; i < 12; i++)
            {
                ...
                if (getserror  == NULL)
                {        
                    sprintf (ErrorMsg, "Could not find two factor parameters in file %s! (TMX_Param_Input)", FileName);
                    DR_Error(ErrorMsg);
                    goto RETURN;
                }
            }
            ...
            for (i = 0; i < 20; i++)
            {
                ...
                if (getserror  == NULL)
                {        
                    sprintf (ErrorMsg, "Could not find three factor parameters in file %s! (TMX_Param_Input)", FileName);
                    DR_Error(ErrorMsg);
                    goto RETURN;
                }
            }
        ...
        else if (NbFactor == 2)
        {
            ...
            for (i = 0; i < 6; i++)
            {
                ...
                if (getserror  == NULL)
                {        
                    sprintf (ErrorMsg, "Could not find one factor parameters in file %s! (TMX_Param_Input)", FileName);
                    DR_Error(ErrorMsg);
                    goto RETURN;
                }
            }

            if (TMX_FindAndSkipComLine (stream, "two factor mean reversion", "TMX_Param_Input", FileName) == FAILURE)
            {        
                goto RETURN;
            }
            ...
            if (TMX_FindAndSkipComLine (stream, "two factor weight", "TMX_Param_Input", FileName) == FAILURE)
            {        
                goto RETURN;
            }
            ...
            if (TMX_FindAndSkipComLine (stream, "two factor correlation", "TMX_Param_Input", FileName) == FAILURE)
            {        
                goto RETURN;
            }
            ...
            if (readerror != 1)
            {        
                sprintf (ErrorMsg, "Could not find correlation in file %s! (TMX_Param_Input)", FileName);
                DR_Error(ErrorMsg);
                goto RETURN;
            }
            ...
            if (strstr (OWS[4], "nil") == NULL)
            {
                ...
                if (readerror != 1)
                {        
                    sprintf (ErrorMsg, "Could not read overwrite string for correlation! (TMX_Param_Input)");
                    DR_Error(ErrorMsg);
                    goto RETURN;
                }
            }

            if (TMX_FindAndSkipComLine (stream, "two factor ppy", "TMX_Param_Input", FileName) == FAILURE)
            {        
                goto RETURN;
            }
        ...
        else if (NbFactor == 3)
        {
            ...
            if (TMX_FindAndSkipComLine (stream, "three factor mean reversion", "TMX_Param_Input", FileName) == FAILURE)
            {        
                goto RETURN;
            }
            ...
            if (TMX_FindAndSkipComLine (stream, "three factor weight", "TMX_Param_Input", FileName) == FAILURE)
            {        
                goto RETURN;
            }
            ...
            if (TMX_FindAndSkipComLine (stream, "three factor correlation", "TMX_Param_Input", FileName) == FAILURE)
            {        
                goto RETURN;
            }
            ...
            if (TMX_FindAndSkipComLine (stream, "three factor ppy", "TMX_Param_Input", FileName) == FAILURE)
            {        
                goto RETURN;
            }
        ...
        if (TMX_FindAndSkipComLine (stream, "QLeft", "TMX_Param_Input", FileName) == FAILURE)
        {        
            goto RETURN;
        }
        ...
        if (readerror != 1)
        {      
            sprintf (ErrorMsg, "Could not find QLeft in file %s! (TMX_Param_Input)", FileName);
            DR_Error(ErrorMsg);
            goto RETURN;
        }

        if (TMX_FindAndSkipComLine (stream, "QRight", "TMX_Param_Input", FileName) == FAILURE)
        {        
            goto RETURN;
        }
        ...
        if (readerror != 1)
        {      
            sprintf (ErrorMsg, "Could not find QRight in file %s! (TMX_Param_Input)", FileName);
            DR_Error(ErrorMsg);
            goto RETURN;
        }

        if (TMX_FindAndSkipComLine (stream, "FwdShift", "TMX_Param_Input", FileName) == FAILURE)
        {        
            goto RETURN;
        }
        ...
        if (readerror != 1)
        {      
            sprintf (ErrorMsg, "Could not find FwdShift in file %s! (TMX_Param_Input)", FileName);
            DR_Error(ErrorMsg);
            goto RETURN;
        }

        if (TMX_FindAndSkipComLine (stream, "CetNbIter", "TMX_Param_Input", FileName) == FAILURE)
        {        
            goto RETURN;
        }
        ...
        if (readerror != 1)
        {      
            sprintf (ErrorMsg, "Could not find CetNbIter in file %s! (TMX_Param_Input)", FileName);
            DR_Error(ErrorMsg);
            goto RETURN;
        }
        ...
        {
            if (TMX_FindAndSkipComLine (stream, "Bbq", "TMX_Param_Input", FileName) == FAILURE)
            {        
                goto RETURN;
            }
            ...
            if (readerror != 1)
            {      
                sprintf (ErrorMsg, "Could not find Bbq in file %s! (TMX_Param_Input)", FileName);
                DR_Error(ErrorMsg);
                goto RETURN;
            }
        ...
        else
        {
            ...
            if (readerror != 1)
            {      
                sprintf (ErrorMsg, "Could not read overwrite string for Bbq! (TMX_Param_Input)");
                DR_Error(ErrorMsg);
                goto RETURN;
            }       
    ...
    if (TMX_FindAndSkipComLine (streamXSMILE, "Header/MultiQ Nb Std", "TMX_Param_Input",
    ...
    {
        DR_Error ("Could not find 2nd line - comment or value - in file %s! (TMX_Param_Input)", 
                  FileNameXSMILE);
        goto RETURN;    
    }
    ...
    {    
        ...
        if (fgets (string, MAXBUFF, streamXSMILE) == NULL)
        {
            DR_Error ("Could not find MultiQ Nb Std in file %s! (TMX_Param_Input)", 
                      FileNameXSMILE);       
            goto RETURN;
        }
    }
    ...
    if (TMX_FindAndSkipComLine (streamXSMILE, "MultiQ Nb Nck", "TMX_Param_Input", 
                            FileNameXSMILE) == FAILURE)        
    {        
        goto RETURN;
    }
    ...
    if (readerror != 1)
    {      
        DR_Error ("Could not find MultiQ Nck in file %s! (TMX_Param_Input)", 
                  FileNameXSMILE);
        goto RETURN;
    }

    /* Check validity of input */
    if (TMX_Param_Check (   NbFactor,
                        mvd,
                        tree_data) == FAILURE)              
    {        
        goto RETURN;
...
*/
int     TMX_Param_Check (int             NbFactor,    /* (I) Number of factors   */

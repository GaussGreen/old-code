...
KVPFloatLegNew::KVPFloatLegNew(
	...
	: KVPInstr(name)
{
static	char	routine[] = "KVPFloatLegNew::KVPFloatLegNew";
	...
	for (idx=0; idx < datesDL->fNumItems-1; idx++) {
		...
		mResetDates.insert(mResetDates.end(), 
        			datesDL->fArray[idx]-(floatRate->instrIdx(0)).SpotOffset());
...
static	char	routine[] = "KVPFloatLegNew::KVPFloatLegNew";
	...
	for (idx=0; idx < numDates; idx++) {
	    mResetDates.insert(   mResetDates.end(),
				  resetEffDates[idx]-(floatRate->instrIdx(0)).SpotOffset());
...
static	char	routine[] = "KVPFloatLegNew::KVPFloatLegNew";
	...
	KCplxRate		tmpRate(*floatRate);	// make a copy
	...
	// Original spread
	oldSprd = (floatRate->instrIdx(0)).Spread();
	...
	for (idx=0; idx < numDates; idx++) {
		...
		tmpRate.instrIdx(0).SetSpread(newSprd);
...
KVPFloatLegNew::KVPFloatLegNew(
	...
	const KVector(double)& notionals,	// (I) notionals array
	const KVector(String)& formulas,	// (I) pay formula array
	const KVector(SharedPointer<KCplxRate>) &floatRates, // (I) floating rate array
	KDayCc dayCc,				// (I) pay day count conv
	KStubConv stubConv,			// (I) stub conv
	const char *discZcName)			// (I) discount curve name
	: KVPInstr(name)
...
/*	for(KVector(KRateReset*)::iterator it=mRateResets.begin();
                ...
                delete (*it);*/
	for(KVector(KCplxRateReset*)::iterator it=mRateResets.begin();
                        it!=mRateResets.end(); ++it)
                delete (*it);
...
void
KVPFloatLegNew::CheckValid() const
{
static	char	routine[] = "KVPFloatLegNew::CheckValid";
    int	     idx, numDates;
	...
	throw KFailure("KVPFloatLegNew::CheckValid: failed for %s.\n", GetName());
    }
...
istream&
KVPFloatLegNew::Get(istream& is, int drw)
{
	...
	if (drw) {
	    ...
	    mDayCc.Get(is, "KVPFloatLegNew::Get: day count conv.");
	    mStubConv.Get(is, "KVPFloatLegNew::Get: stub conv.");

	    numDates = getInt(is, "KVPFloatLegNew::Get: number of dates.");

	    for (idx=0; idx<numDates; idx++) {
		mResetDates.insert(mResetDates.end(),
			getTDate(is, "KVPFloatLegNew::Get: reset date."));
		mResetEffDates.insert(mResetEffDates.end(),
			getTDate(is, "KVPFloatLegNew::Get: reset eff date."));
		mAccStartDates.insert(mAccStartDates.end(),
			getTDate(is, "KVPFloatLegNew::Get: acc start date."));
		mAccEndDates.insert(mAccEndDates.end(),
			getTDate(is, "KVPFloatLegNew::Get: acc end date."));
		mPayDates.insert(mPayDates.end(),
			getTDate(is, "KVPFloatLegNew::Get: pay date."));
		mNotionals.insert(mNotionals.end(),
			getTDate(is, "KVPFloatLegNew::Get: notional."));
	    }

	} else {
		throw KFailure("KVPFloatLegNew::Get: format N/A.\n");
	}
	...
	throw KFailure("KVPFloatLegNew::Get: failed.\n");
    }
}
...
ostream&
KVPFloatLegNew::Put(ostream& os, int indent) const
{
    int idx, numDates;
    ...
    return(os);
        ...
        throw KFailure("KVPFloatLegNew::Get: failed for %s.\n", GetName());
    }
}
...
/*
void
KVPFloatLegNew::SetWriteFlag(bool flag)
{
	int     idx, numDates;
...
// 
ostream&
KVPFloatLegNew::YacctionWriteRecursive(ostream& os)
{
...
ostream&
KVPFloatLegNew::YacctionWrite(ostream& os, int indent) 
{
	int	idx, numDates;
	bool	isOneRate = true;
	double	spread;
...
/*		if (!(mRateResets[0]->Rate() == mRateResets[idx]->Rate()) &&
		...
		if (!(mRateResets[0]->RateIndex(0) == mRateResets[idx]->RateIndex(0)) &&
		    mRateResets[idx]->RateIndex(0).IsFloating())
			isOneRate = false;
	    }
...
/*
	    if (!isOneRate)
		throw KFailure("KVPFloatLegNew::YacctionWrite: different "
...
*/
		...
		if (mRateResets[idx]->RateIndex(0).IsFloating())
			spread = mRateResets[idx]->RateIndex(0).Spread();
		else 	// fixed rate
			spread = mRateResets[idx]->RateIndex(0).Spread()
			       - mRateResets[0]->RateIndex(0).Spread();
	...
	throw KFailure("KVPFloatLegNew::YacctionWrite: failed for %s.\n", GetName());
    }
}

...
KVPToolFloatLeg2Idx::KVPToolFloatLeg2Idx(
        SharedPointer<KVPFloatLeg2Idx> ins,        // (I) inetrument to value
        KVTree &vt)                                // (I) virtual tree
        : KVPToolFloatLeg(vt)
{
static        char        routine[] = "KVPToolFloatLeg2Idx::KVPToolFloatLeg2Idx";
        int        idx, n;
        int         cIdx, cIdxMax;
...
const        String&        discCurveName = ins->mDiscZcName;
    ...
    try {
        ...
        n = mFloatLeg2Idx->mResetDates.size();

        mFloatLeg2Idx->CheckValid();

        // enough space to store the 2nd set of model reset dates
        ...
        KVector(TDate)::iterator firstResetDate = min_element(
                                mFloatLeg2Idx->mResetDates.begin(),
                                mFloatLeg2Idx->mResetDates.end());
        ...
        for (idx=0; idx<n; idx++) {
            ...
            resetDate = mFloatLeg2Idx->mResetDates[idx];

            if (mFloatLeg2Idx->mRateResets2[idx]->Rate().IsFloating())
                curveName = mFloatLeg2Idx->mRateResets2[idx]->Rate().CurveName();
            else
                curveName = mFloatLeg2Idx->mDiscZcName;

            // Value date is curve dependent
            valueDate2 = vt.GetValueDate(vt.GetCurveIdx(curveName));
            ...
            if (mFloatLeg2Idx->mAccEndDates[idx] <= valueDate2) // obsolete events
                continue;
            else         // insert valid events
            {
                    ...
                    resetDate >= todayDate)        
                {
                        mModelResetDates2[idx] 
                                = vt.Insert(*(mFloatLeg2Idx->mRateResets2[idx]),
                                            mFloatLeg2Idx->mAccEndDates[idx]);
                }
                ...
                if (resetDate < todayDate &&
                    mFloatLeg2Idx->mRateResets2[idx]->Rate().IsFloating())
                                                        // Stub period
                {
                    ...
                    switch ((long)(mFloatLeg2Idx->mStubConv)) {
                    case GTO_STUB_NONE:
                    ...
                    case KV_STUB_SPOT_SIMPLE:
                        ...
                        stubRate = new KRateReset(
                                      todayDate,
                                      valueDate2,
                                      mFloatLeg2Idx->mRateResets2[idx]->Rate());
        ...
        KVector(TDate)::iterator lastDate = max_element(
                                        mFloatLeg2Idx->mAccEndDates.begin(),
                                        mFloatLeg2Idx->mAccEndDates.end());

        KRateReset extraRateReset(
                        (*lastDate),
                        (*lastDate) + 
                        mFloatLeg2Idx->mRateResets2[n-1]->Rate().SpotOffset(),
                        mFloatLeg2Idx->mRateResets2[n-1]->Rate());

        vt.Insert(extraRateReset);

        //
        // Curve name of the leg is the one with largest slice geometry
        // among the 2 sets of index rates
        ...
        if((long)(mFloatLeg2Idx->mStubConv) == KV_STUB_SPOT_SIMPLE) 
                delete stubRate;
...
void
KVPToolFloatLeg2Idx::Update()
{
static        char        routine[] = "KVPToolFloatLeg2Idx::Update";
    ...
    try {
        ...
        for (idx=0; idx<n; idx++) {
          ...
          else         // Valid events
          {
            ...
            accStartDate = mFloatLeg2Idx->mAccStartDates[idx];
            accEndDate   = mFloatLeg2Idx->mAccEndDates[idx];
            payDate      = mFloatLeg2Idx->mPayDates[idx];
            notional     = mFloatLeg2Idx->mNotionals[idx];
            rateReset1   = mFloatLeg2Idx->mRateResets[idx];
            rateReset2   = mFloatLeg2Idx->mRateResets2[idx];
...
KTSlice&
KVPToolFloatLeg2Idx::GetValue()
{
static        char        routine[] = "KVPToolFloatLeg2Idx::GetValue";
        int        idx, n = mFloatLeg->mResetDates.size();
        TDate        currDate = mVTree->TPDateCurrent(),
                    ...
                    (rateReset2->Rate().IsFloating()))
                {
                    ...
                    case KV_STUB_SPOT_SIMPLE:
                        ...
                        else
                        {
                            ...
                            mVTree->Get(rateArrayTs[0], *stubRateReset1);
                            mVTree->Get(rateArrayTs[1], *stubRateReset1);

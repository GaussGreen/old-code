...
/****************************************************************
 * Constructs the object from a complex rate
 */
KVPKnockIONew::KVPKnockIONew(
	...
	: KVPInstr(name)
{
static	char	routine[] = "KVPKnockIONew::KVPKnockIONew";
...
//	KVector(double)	barrierHiA = barrierHi;
    ...
    // The barrier is set in the formula of rateIndex.  
    // 
    barHi = koind->getHBarrier();
    barLo = koind->getLBarrier(); 

//    cout << routine << *koind << endl;
    try {
...
//	ASSERT_OR_THROW(barrierDates.size() == barrierHi.size());
	...
	for (idx=0; idx<=numDates-1; idx++) {
		...
		// Rate Reset
		rateReset = new KCplxRateReset(
                            obsDate,
					        koind->CplxRate());

//        cout << routine << '\t' << " curve name "
//             << koind->CurveName() << endl;
...
/*		if (barLo > barHi * (1. - BARRIER_TOL))
	...
	// Add rate as dependency
	SharedPointerConvertTo(koind, koRate);
	AddDep(koRate);
...
static	char	routine[] = "KVPKnockIONew::KVPKnockIONew";
	...
	for (idx=0; idx<=numDates-1; idx++) {
		...
		// Rate Reset
		rateReset = new KCplxRateReset(obsDate,
					   *rateIndex);
...
static	char	routine[] = "KVPKnockIO::KVPKnockIO";
	...
	for (i=0; i<=obsDates.size()-1; i++)
	{
		...
		// Rate Reset
		rateReset = new KCplxRateReset(obsDates[i],
					   *rateIndex);
...
KVPKnockIONew::~KVPKnockIONew()
{
	for(KVector(KCplxRateReset*)::iterator it=mRateIndex.begin();
...
void
KVPKnockIONew::ValidEvents(TDate today)		// (I) today's date
{
static	char	routine[] = "KVPKnockIONew::ValidEvents";
	...
	KVector(KCplxRateReset*) rateIndex;
...
istream&
KVPKnockIONew::Get(istream& is, int drw)
{
...
ostream&
KVPKnockIONew::Put(ostream& os, int indent) const
{
	int	idx, numDates;
...
ostream&
KVPKnockIONew::YacctionWrite(ostream& os, int indent)
{
	int	idx, numDates;
	...
	if (GetWriteFlag())
	{
		...
		{                        
			os << " ";
			mRateIndex[idx]->CplxRate().WriteSimple(os);
		}

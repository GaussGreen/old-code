...
//---------------------------------------------------------------
void
KVPToolFloatLegNew::CreateVPToolFloatLegNew(
        SharedPointer<KVPFloatLegNew> ins,        // (I) instrument to value
        KVTree &vt)                              // (I) virtual tree
{
static  char        routine[] = "KVPToolFloatLegNew::CreateVPToolFloatLegNew";
        int         idx, n;
...
const   String&     discCurveName = ins->mDiscZcName;
    ...
    try {
        ...
        // set value date to the minimum value date of all curves
        setValueDate(vt);
        valueDate = mValueDate;
        // set mCurveName associated to the largest curve index
        setCurveName();
        //
        // Add events in tree
        //
         for (idx=0; idx<n; idx++) 
         {
             ...
             cIdx = mVTree->GetCurveIdx(mCurveName);
             cIdxMax = cIdx;
             ...
             else         // insert valid events
             {
                ...
                // anywhere between reset date and accrual end date.
                ...
                // later on.
                //
                if (resetDate < todayDate )          // Stub period
                    //IsFloating() is checked in vt.Insert();
                {
                    // The value date of the leg is determined from
                    // the first valid period.
                   // mValueDate = MIN( valueDate, mValueDate) ;
...
/*                        stubRate = new KRateReset(
                        ...
                        stubRate = new KCplxRateReset(
                                        todayDate,
                                        valueDate,
                                        mFloatLeg->mRateResets[idx]->CplxRate());
...
/*        KRateReset extraRateReset((*lastDate),
        ...
        KCplxRateReset extraRateReset(
                    (*lastDate),
                    mFloatLeg->mRateResets[n-1]->CplxRate());
...
KVPToolFloatLegNew::~KVPToolFloatLegNew()
{
...
void
KVPToolFloatLegNew::Update()
{
static        char        routine[] = "KVPToolFloatLegNew::Update";
        int        idx, n = mFloatLeg->mResetDates.size();
...
KTSlice&
KVPToolFloatLegNew::GetValue()
{
static        char        routine[] = "KVPToolFloatLegNew::GetValue";
        int        idx, n = mFloatLeg->mResetDates.size();
        TDate        currDate = mVTree->TPDateCurrent(),
        ...
        KTSlice           *zTs = NULL;
        KCplxRateReset        *stubRateReset = NULL;
...
/*                        stubRateReset = new KRateReset(currDate, 
                                                       currEffDate,
                                                       rateReset->Rate());*/
                        stubRateReset = new KCplxRateReset(currDate, 
                                                       currEffDate,
                                                       rateReset->CplxRate());
...
void
KVPToolFloatLegNew::setValueDate( KVTree& vt )
{
static  char    routine[] = "KVPToolFloatLegNew::setValueDate";
    TDate   valueDate;
    String  curveName;
    int     nbinstr, i;
    try
    {
        curveName = mFloatLeg->mDiscZcName;
        mValueDate = vt.GetValueDate(vt.GetCurveIdx(curveName));
          
        nbinstr = mFloatLeg->nbIndexInstr();
        for ( i = 0; i < nbinstr; i++)
        {
           if (mFloatLeg->mRateResets[0]->RateIndex(i).IsFloating())
                curveName = 
                       mFloatLeg->mRateResets[0]->RateIndex(i).CurveName();
                
           valueDate = vt.GetValueDate(vt.GetCurveIdx(curveName));
        }
        mValueDate = MIN(mValueDate, valueDate);
    }
    catch(KFailure) 
    {
        throw KFailure("%s: failed.\n",routine);
...
void
KVPToolFloatLegNew::setCurveName()
{
static  char    routine[] = "KVPToolFloatLegNew::setCurveName";
    String       curveName;
    int          nbinstr, i;
    int          cIdx, cIdxMax, discIdx;
    try
    {
        ...
        for (i = 0; i < mFloatLeg->mResetDates.size(); i++)
            mFloatLeg->mRateResets[i]->CplxRate().SetCurveName(mCurveName);
   }
   catch(KFailure) 
   {
       throw KFailure("%s: failed.\n",routine);

...
/* function n-dim to n-dim */
#ifdef	_SKIP
/* defined in header drlroot.h */
typedef	void (*TMDFunc)(
	int n,			/* (I) # dim */
	double *v,		/* (I) point where evaluated */
	double *f);		/* (O) vector values */
#endif	/*_SKIP*/
...
#define FREERETURN {status = SUCCESS; goto done;}
	...
	*check = 1;
...
#endif
	    	if (sing) {
		    GtoErrMsg("%s: singular Jacobian:\n", routine);
	    	    GtoErrMsg("NL:%d\nNC:%d\n", n, n);
	    	    for(idx1=0; idx1<=n-1; idx1++) {
			for(idx2=0; idx2<=n-1; idx2++) {
	    	    		GtoErrMsg("\t%lf", r[idx1+1][idx2+1]);
			}
	    	        GtoErrMsg("\n");
	    	    }
		    goto done;
	...
	GtoErrMsg("%s: maximum # of iterations exceeded (%s).\n",
		routine, maxIter);
	goto done;

...
static	char	routine[] = "DrlMatrixNew";
	...
	if (status != SUCCESS) {
	    GtoErrMsg("%s: %d x %d matrix failed\n",
		routine, nl, nc);
	    return(NULL);
	}
	return(p);
	...
	int nc)			/* (I) # of columns */
{
	int	i, j;
#undef	PRINT
#define	PRINT(fmt, arg)	{if (fp == (FILE*)NULL) GtoErrMsg(fmt, arg);\
...
static	char	routine[] = "DrlMatrixCopy";
	...
	if ((*toMat = DrlMatrixNew(nl, nc)) == NULL) {
		GtoErrMsg("%s: matrix allocation failed\n", routine);
		return(-4);
	}
...
register int	i, j;
	...
	if ((aNl != bNl) || (aNc != bNc)) {
		GtoErrMsg("%s: matrix (%d, %d) not same type as ",
			" matrix (%d, %d)\n",
	...
	case '-':
		...
		break;
	default:
	    GtoErrMsg("%s: operation `%s' not availble.\n",
		routine, oper);
	    return (FAILURE);
	}
...
static	char	routine[] = "DrlMatrixProduct";
	...
	if (aNc != bNl) {
		GtoErrMsg("%s: can't multiply (aNc = %d, bNl = %d)\n",
			routine, aNc, bNl);
		*prodMat = NULL;
		return(1);
	}
...
static	char	routine[] = "DrlMatrixCond";
	...
	if (DrlMatrixInvert(&invMat, mat, nl) != 0) {
		GtoErrMsg("%s: matrix inversion failed\n", routine);
		return(FAILURE);
	}

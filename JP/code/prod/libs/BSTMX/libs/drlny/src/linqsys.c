...
static	char	routine[] = "DrlRealLinSys";
	...
	if (n == 1) {
		if (IS_ALMOST_ZERO(a[0][0])) {
			GtoErrMsg("%s: singular 1x1 matrix.\n", routine);
			goto done;
		}
		x[0] = b[0] / a[0][0];
		status = SUCCESS;
	...
	if ((xSol == NULL) || (imslErrCode != 0L)) {
	    ...
	    GtoErrMsg("%s: [imsl] solving linear system "
			"failed (code %d)\n",
			routine, imslErrCode);
...
static	char	routine[] = "DrlRealLinSysPivot";
	...
	*dpo = NULL;
	...
	if ((m <= 1) || (n <= 1) || (npiv >= m) || (npiv >= n)) {
		GtoErrMsg("%s: can't %d-pivot system %d x %d.\n",
			routine, npiv, m, n);
		goto done;
	}
	for (i=0; i<=npiv-1; i++) {
	    if ((ipiv[i] < 0) || (ipiv[i] >= m)) {
		GtoErrMsg("%s: wrong i-pivot #%d (%d) for system %d x %d.\n",

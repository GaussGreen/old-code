...
#include "drltime.h"		/* DrlTDateIntervalScan() */
#include "drlvtype.h"
...
DrlStrParseFuncScan(
	...
	/*  char *varToken, TVType varType, void* varPtr,
...
static	char	routine[] = "DrlStrParseFuncScan";
	 ...
	 */
	if (nVar > MAXVAR-1) {
	    GtoErrMsg("%s: too any arguments (max %d).\n",
		routine, MAXVAR);
	    return(FAILURE);
	}
	strcpy(buf, s);
...
printf("v0=`%s'  v1=`%s'\n", v0, v1);
		...
		for (i=0; i<=nVar-1; i++) {
			...
			if (DrlVTypeScan(v1, varType[i], varPtr[i]) != 0) {
			    GtoErrMsg("%s: error arg `%s' (type %s)\n",
	...
	for (i=0; i <= nVar-1; i++) {
		...
		if (varDefFlag[i] != 0) {
			 ...
			 * value. ERROR  */
			GtoErrMsg("%s: mandatory variable `%s' "
				"not set for `%s'\n",
...
static	char	routine[] = "DrlStrParseOptions";
	...
	numVar = 0;
	va_start(ap, s);
	while ((varType[numVar] = (TVType) va_arg(ap, TVType)) != 0L) {
...
DLL_EXPORT(int)
DrlStrTokScan(char *s, TVType varType, void *var)
{
...
static	char	st0[512], *st1;
	...
	if ((p = DrlStrToken(s, tok, st0, &st1)) == NULL) {
		GtoErrMsg("%s: end of line\n", routine);
		return(1);
	}

	if (DrlVTypeScan(p, varType, var) != 0) {
		GtoErrMsg("%s: can't read type %s in `%s'\n",
...
DLL_EXPORT(char*)
DrlStrTokPrint(char *s, TVType varType, void *var)
{
	strcat(s, "\t");
...
static	char		tok[] = " \t;\n";
	...
	while ((varType = (TVType) va_arg(ap, TVType)) != 0L) {
	    argNo++;
		...
		GtoErrMsg("%s: can't read variable # %d "
...
#define	ISNCHAR(c)	(isalnum(c) || (c =='_'))
	...
	for (;;) {
		...
		if ((varval = getenv(varname)) == NULL) {
			GtoErrMsg("%s: in string `%s', variable `%s' "
				"undefined.\n", routine, s, varname);
			return(FAILURE);
		}
		strcpy(p, varval);
...
DrlDoubleIntervalScan(char *s, double *value)
{
	...
	if (isalpha(s[strlen(s)-1])) {
	    if (DrlTDateIntervalScan(buf, &interval) != SUCCESS)
		goto done;
	    if (GtoDateIntervalToYears(&interval, value) != SUCCESS)
	    	goto done;
	} else {
	...
	if (status != SUCCESS)
		GtoErrMsg("DoubleIntervalScan: failed on `%s'.\n", s);
	return(status);
}
...
DrlDoubleIntervalPrint(char *s, double value)
{
	...
	if (GtoYearsToDateInterval(value, &interval) != SUCCESS) {
		strcpy(s, "ERR");
		goto done;
	}

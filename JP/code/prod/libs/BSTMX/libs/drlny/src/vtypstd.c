...
DLL_EXPORT(void*)
DrlVTypeVectAlloc(int size, TVType type)
{
static	char	routine[] = "DrlVTypeVectAlloc";
	...
	case DRL_STRING_T:
		...
		if (p == NULL) {
			GtoErrMsg("%s: malloc DRL_STRING_T "
				"length %d failed.\n", routine, (int) (size));
			return(NULL);
		}
	    	for (i=0; i<=size-1; i++) {
	...
	default:
		...
		if (DrlVTypeCheckValidCType(type) != 0) {
			GtoErrMsg("%s: bad C type.\n", routine);
			return(NULL);
		}

	 	/* get type size */
		if ((typeSize = DrlVTypeSizeof(type)) <= 0) {
			GtoErrMsg("%s: bad type size.\n");
			return(NULL);
		}

		/* allocate memory */
		if ((p = (void*) MALLOC((size_t) (typeSize*size))) == NULL) {
			GtoErrMsg("%s: malloc length %d failed.\n", routine, 
					(int) (typeSize*size));
			return(NULL);
		}
		break;
...
DLL_EXPORT(int)
DrlVTypeVectFree(void* p, int size, TVType type)
{
	int	i;
	...
	default:
		if (DrlVTypeCheckValidCType(type) != 0) {
			GtoErrMsg("DrlVTypeVectFree: bad C type\n");
			return(1);
		}
...
DLL_EXPORT(void*)
DrlVTypeMatrAlloc(int nx, int ny, TVType type)
{
static	char	routine[] = "DrlVTypeMatrAlloc";
	...
	if (errCode != 0) {
		GtoErrMsg("%s: failed (type %d, size %dx%d).\n",
			routine, type, nx, ny);
	}
	return((void*) p);
}
...
DLL_EXPORT(int)
DrlVTypeMatrFree(void* p, int nx, int ny, TVType type)
{
	int	i, j;

	/* check type size */
	if (DrlVTypeCheckValidCType(type) != 0) {
		GtoErrMsg("DrlVTypeMatrFree: bad C type.\n");
		return(1);
	}
...
DLL_EXPORT(int)
DrlVTypeScan(char *str, TVType type, void *p)
{
static	char	routine[] = "DrlVTypeScan";
	...
	case DRL_TDATE_T:
		...
		if (DrlTDateScan(buf, (TDate*)p) != 0) goto error;
		break;
	case DRL_TDATEINTERVAL_T:
		if (sscanf(str, "%s",  buf) != 1) goto error;
		if (DrlTDateIntervalScan(buf, (TDateInterval*)p)
				!= SUCCESS) goto error;
		break;
	...
	return(0);
error:
	GtoErrMsg("%s: can't scan `%s' (type %s)\n",
...
DLL_EXPORT(char*)
DrlVTypePrint(char *str, TVType type, void *p)
{
...
DLL_EXPORT(char*)
DrlVTypePrintFmt(char *str, TVType type, void *p, char *fmt)
{
...
static	char	buf[64];
	...
	case DRL_STRING_T:
		...
		break;
	case DRL_TDATE_T:
		sprintf(str, fmt, GtoFormatDate(*((TDate*) p)));
		break;
	case DRL_TDATEINTERVAL_T:
		sprintf(str, fmt, GtoFormatDateInterval(((TDateInterval*) p)));
		break;
	case DRL_TDAYCOUNT_T:
		sprintf(str, fmt,
		   GtoFormatDayCountConv(*((TDayCount*) p)));
...
DLL_EXPORT(char*)
DrlVTypePrintSmart(char *str, TVType type, void *p)
{
...
DLL_EXPORT(void*)
DrlVTypeOffsetVect(void *p, int offset, TVType type)
{
	...
	default:
		GtoErrMsg("OffsetType: bad type %ld.\n", (long)type);
		return(NULL);
	}
}
...
DLL_EXPORT(void*)
DrlVTypeOffsetMatrix(void *p, int nx, int ny, TVType type)
{
#undef	ELEM
#define	ELEM(t)	((void*)&((t**)p)[nx][ny])
	...
	case DRL_TDATE_T:
		return ELEM(TDate);
	case DRL_TDATEINTERVAL_T:
		return ELEM(TDateInterval);
	case DRL_TDAYCOUNT_T:
		return ELEM(TDayCount);
	default:
		GtoErrMsg("OffsetType: bad type %ld.\n", (long)type);
		return(NULL);
	}
...
DLL_EXPORT(int)
DrlVTypeLet(void *p, int offset1, void *q, int offset2, TVType type)
{
...
DLL_EXPORT(int)
DrlVTypeCompare(const void *p, const void *q, TVType type)
{
 ...
 * must  be the address of a <i> TDate</i> that is used
 * as reference date to convert an interval to a double value.
 * (the routine <i> DrlTDateIntervalToYears</i> is used
...
DLL_EXPORT(int)
DrlVTypeNumValue(void *p, TVType type, void *arg, double *value)
{
	...
	case DRL_TDATEINTERVAL_T:
		if (DrlTDateIntervalToYears(
			...
			*((TDateInterval*)p),
			...
			value) != SUCCESS)
				goto done;
		/*GtoDateIntervalToYears((TDateInterval*)p, value);*/
		break;
	case DRL_TDAYCOUNT_T:
	default:
		GtoErrMsg("%s: can't get numerical value for type %s.\n",
...
static	TVType	_DrlVTypeVectSort_type;
...
DLL_EXPORT(int)
DrlVTypeNameScan(TVType *type, char *s)
{
...
#define	CHECK(str, val)		{if (!strcmp(str, s)) \
	...
	GtoErrMsg("%s: can't read type in `%s'.\n", routine, s);
	return(FAILURE);
...
DLL_EXPORT(int)
DrlVTypeCheckValidCType(TVType type)
{
	switch (type) {
	/* C types */
	case DRL_POINTER_T:
...
DLL_EXPORT(int)
DrlVTypeCheckValidLType(TVType type)
{
	switch (type) {
	case DRL_FLOAT_L:
	case DRL_INT_L:
...
DLL_EXPORT(void*)
DrlVTypeTensorAlloc(TVType type, int nDim, int *nSize)
{
...
DLL_EXPORT(int)
DrlVTypeTensorFree(void *ptr, TVType type, int nDim, int *nSize)
{
static	char	routine[] = "DrlVTypeTensorAlloc";
	int	i;
...
DLL_EXPORT(void*)
DrlVTypeTensorAllocV(TVType type, int nDim,
...
DLL_EXPORT(int)
DrlVTypeTensorFreeV(void *ptr, TVType type, int nDim,
		... /* int size1, ..., sizeNDim */)
{
...
static	char	routine[] = "DrlVTypeTensorOperScalar";
	...
	if (type != DRL_DOUBLE_T) {
	    GtoErrMsg("%s: type must be double.\n", routine);
	    goto done;
	}
...
#define	IF_OPER(name)	if (!strcmp(operName, name))
	...
	if (!strcmp(operName, "sum")) {
		...
		*value /= (double)cnt;
		...
		GtoErrMsg("%s: unknown operation %s.\n", routine, operName);
		goto done;
	}

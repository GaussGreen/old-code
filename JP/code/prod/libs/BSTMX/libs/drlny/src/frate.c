...
/*ARGSUSED*/
DLL_EXPORT(int)
DrlTFloatRateIsValid(TFloatRate *that)
{
	return(TRUE);
}
...
/*f-------------------------------------------------------------
 * Reads a <i> TFloatRate</i> in a string <i> s</i> and puts the result
...
DLL_EXPORT(int)
DrlTFloatRateScan(char *s, TFloatRate *that)
{
/*static	char	routine[] = "DrlTFloatRateScan";*/
	int	status = FAILURE;
...
/*f-------------------------------------------------------------
 * Prints a <i> TFloatRate</i> <i> that</i> in a string <i> s</i>.
 * The output format is the same as in <i> DrlTFloatRateScan</i>.
 * Returns <i> s</i> (or a private copy if <i> s</i> is NULL).
 */

DLL_EXPORT(char*)
DrlTFloatRatePrint(char *s, TFloatRate *that)
{
static	char	buf[256];
	s =  (s == NULL ? buf : s);
...
/*f--------------------------------------------------------------
 * Prints a <i> TFloatRateArray</i> <i> that</i> in a string <i> s</i>.
 * The output format is the same as in <i> DrlTFloatRateScan</i>.
 * Returns <i> s</i> (or a private copy if <i> s</i> is NULL).
 */

DLL_EXPORT(int)
DrlTFloatRateArrayFpWrite(TFloatRateArray *that, FILE *fp)
{
	int	idx;
	...
	for (idx=0; idx<= that->numRates-1; idx++) {
		...
		DrlTFloatRatePrint(NULL, &that->defs[idx])
	    );
	}
	return(SUCCESS);
}
...
DLL_EXPORT(int)
DrlTFloatRateVolPointScan(char *s, TFloatRateVolPoint *that)
{
/*static	char	routine[] = "DrlTFloatRateVolPointScan";*/
	int	freq, freqDef = 2,
		status = FAILURE;
	TDateInterval	startDef;

	if (GtoMakeDateInterval(0, 'D', &startDef) != SUCCESS)
		goto done;
	...
	if (freq == 0) {
	    ...
	    if (GtoFreq2TDateInterval((long)freq, &that->fRate.payInterval)
		!= SUCCESS) goto done;
	}
	...
	that->fRate.dayCountConv = (freq > 0 ? GTO_B30_360 : GTO_ACT_360);

	/* made it through */
	status = SUCCESS;
done:
	/*if (status != SUCCESS) {
	    GtoErrMsg("%s: failed (can't read string `%s')\n", routine, s);
	}*/
	return(status);
}
 ...
 * The output format is the same as in <i> DrlTFloatRateVolPointScan</i>.
 */

DLL_EXPORT(char*)
DrlTFloatRateVolPointPrint(char *s, TFloatRateVolPoint *that)
{
static	char	buf[256];
	int	freq;
	double	dfreq;
	...
	if (GtoDateIntervalToFreq(&that->fRate.payInterval, &dfreq)
	    != SUCCESS) {
...
DLL_EXPORT(int)
DrlTFloatRateCorrPointScan(char *s, TFloatRateCorrPoint *that)
{
/*static	char	routine[] = "DrlTFloatRateCorrPointScan";*/
	...
	if (freq1 == 0) {
	    ...
	    if (GtoFreq2TDateInterval((long)freq1, &that->fRate1.payInterval)
		!= SUCCESS) goto done;
	}
	...
	if (freq2 == 0) {
	    ...
	    if (GtoFreq2TDateInterval((long)freq2, &that->fRate2.payInterval)
		!= SUCCESS) goto done;
	}
	...
	/* set proper day count */
	that->fRate1.dayCountConv = (freq1 > 0 ? GTO_B30_360 : GTO_ACT_360);
	that->fRate2.dayCountConv = (freq2 > 0 ? GTO_B30_360 : GTO_ACT_360);
 ...
 * The output format is the same as in <i> DrlTFloatRateCorrPointScan</i>.
 */

DLL_EXPORT(char*)
DrlTFloatRateCorrPointPrint(char *s, TFloatRateCorrPoint *that)
{
static	char	buf[256];
	int	freq1, freq2;
	double	dfreq1, dfreq2;
	s =  (s == NULL ? buf : s);

	if ((GtoDateIntervalToFreq(&that->fRate1.payInterval, &dfreq1)
		!= SUCCESS) || 
	    (GtoDateIntervalToFreq(&that->fRate2.payInterval, &dfreq2)
		!= SUCCESS)) {
		sprintf(s, "FLRATE_CORR=ERROR");
		return(s);
	}
...
/*f-------------------------------------------------------------
 * Convenience routine to construct set a TVolBenchmark
 * to be a cap/swaption volatility point.
 * Returns 0 iff successful.
 */

DLL_EXPORT(int)
DrlTVolBenchmarkSetVol(
	...
	TDayCount rateDcc)		/* (I) rate dcc (-1L for default) */
{
	...
	IF_FAILED_DONE( GtoMakeDateInterval(0, 'D', &startDef));
	...
	if (rateFreq == 0) {
	    ...
	    IF_FAILED_DONE( GtoFreq2TDateInterval(
		(long)rateFreq,
	...
	vpt->fRate.dayCountConv = (rateDcc == -1L ? 
			(rateFreq > 0 ? GTO_B30_360 : GTO_ACT_360) :
			rateDcc);
	...
	TDayCount rateDcc2)		/* (I) rate dcc (-1L for default) */
{
	...
	cpt->fRate1.dayCountConv = (rateDcc1 == -1L ? 
			(rateFreq1 > 0 ? GTO_B30_360 : GTO_ACT_360) :
			rateDcc1);
	cpt->fRate2.dayCountConv = (rateDcc2 == -1L ? 
			(rateFreq2 > 0 ? GTO_B30_360 : GTO_ACT_360) :
			rateDcc2);
...
DLL_EXPORT(int)
DrlTVolBenchmarkIsValid(TVolBenchmark *pt)
{
static 	char	routine[] = "DrlTVolBenchmarkIsValid";
	switch(pt->fType) {
...
DLL_EXPORT(int)
DrlTVolBenchmarkScan(char *s, TVolBenchmark *pt)
{
	int	errCode;

	errCode = DrlTFloatRateVolPointScan(s, &pt->fU.fVol);
	if (errCode == 0) {
	...
	errCode = DrlTFloatRateCorrPointScan(s, &pt->fU.fCorr);
	if (errCode == 0) {
	...
	if (errCode != 0) {
	    GtoErrMsg("DrlTVolBenchmarkScan: can't parse `%s'\n", s);
	}
 ...
 * in the string "s"
 * (same format as in <i> DrlTVolBenchmarkScan</i>).
 * Returns a pointer to "s".
 */

DLL_EXPORT(char*)
DrlTVolBenchmarkPrint(char *s, TVolBenchmark *pt)
{
static 	char	routine[] = "DrlTVolBenchmarkPrint";
	switch(pt->fType) {
	case DRL_TVOLBENCHMARK_VOL:
		return DrlTFloatRateVolPointPrint(s, &pt->fU.fVol);
	case DRL_TVOLBENCHMARK_CORR:
		return DrlTFloatRateCorrPointPrint(s, &pt->fU.fCorr);
	default:
...
/*--------------------------------------------------------------
 * Stream I/O for an array of TVolBenchmark
 */

static	int
_vTVolBenchmarkFpRead(TVolBenchmark *pt, int *nPt, FILE *fp,
		va_list arg)
{
#ifdef	_SKIP
	int	line=0, i, j, jmax;
...
static	char	toks[] = " \t;\n";
	...
	for (i=0; i<=*nPt-1; i++) {
		...
		if ((s1 = StrToken(s0, toks, sTmp0, &sTmp1)) == NULL)
			return(line);
		s0 = NULL;
		if (TVolBenchmarkScan(s1, pt+i) != 0) return(line);
	...
	return(0);
#endif
	GtoErrMsg("_vTVolBenchmarkFpRead: not implemented\n");
	return(1);
}

static	int
_vTVolBenchmarkFpWrite(TVolBenchmark *pt, int nPt, FILE *fp,
		va_list arg)
{
	int	i, j, jmax;
	double	*x[32];
	...
	for (i=0; i<=nPt-1; i++) {
		DrlFPrintf(fp, "\t%s", DrlTVolBenchmarkPrint(NULL, pt+i));
...
DLL_EXPORT(int)
DrlTVolBenchmarkFpRead(TVolBenchmark *pt, int *nPt, FILE *fp, ...)
{
	int	errCode;
	va_list	ap;

	va_start(ap, fp);
	errCode = _vTVolBenchmarkFpRead(pt, nPt, fp, ap);
	va_end(ap);
	return(errCode);
}
...
DLL_EXPORT(int)
DrlTVolBenchmarkFpWrite(TVolBenchmark *pt, int nPt, FILE *fp, ...)
{
	int	errCode;
	va_list	ap;

	va_start(ap, fp);
	errCode = _vTVolBenchmarkFpWrite(pt, nPt, fp, ap);
	va_end(ap);
	return(errCode);
}
...
/*--------------------------------------------------------------
 * File I/O for an array of TVolBenchmark
 */

DLL_EXPORT(int)
DrlTVolBenchmarkFileRead(TVolBenchmark *pt, int *nPt, char *fnam, ...)
{
	FILE	*fp ;
	int	errCode;
	va_list	ap;
...
DLL_EXPORT(int)
DrlTVolBenchmarkFileWrite(TVolBenchmark *pt, int nPt, char *fnam, ...)
{
	FILE	*fp ;
	int	errCode ;
	va_list	ap;
 ...
 * of the benchmarks (same as in <i> DrlTVolBenchmarkScan</i>).
...
DLL_EXPORT(int)
DrlTVolBenchmarkWrapRead(
	TVolBenchmark *pt,	/* (O) array of benchmarks */
	int *nPt,		/* (O) actual # of benchmarks */
	IntL *nPtL,		/* (I) LIL vector (or NULL) */
	CharBlockL *ptL)	/* (I) LIL vector */
{
static	char	routine[] = "DrlTVolBenchmarkWrapRead";
	int	i, status = FAILURE;

	/* get # of arguments */
	WRAP_CHECK_VECTOR(ptL);
	*nPt = ARGSIZE(ptL);

	if (nPtL != NULL) {
	    WRAP_CHECK_VECTOR(nPtL);
	    *nPt = MIN(*nPt, (int) nPtL[1]);
	}

	/* scan strings */
	for (i=0; i<=*nPt-1; i++) {
	    if (DrlTVolBenchmarkScan(&ptL[WRAP_STR_IDX(i+1)],
		&pt[i]) != 0) {
		goto done;
	    }
	    if (!DrlTVolBenchmarkIsValid(&pt[i])) {
		goto done;
	    }
	}
	...
	/* made it through OK */
#ifdef	__DEBUG__
	DrlTVolBenchmarkFpWrite(pt, *nPt, stdout, NULL);
#endif
	status = SUCCESS;
done:
	if (status != SUCCESS) {

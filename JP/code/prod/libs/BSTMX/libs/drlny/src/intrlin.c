...
static	char	routine[] = "DrlLinearInterp1d";
	...
	if (m < 1) {
		GtoErrMsg("%s: not enough points (%d).\n",
			routine, m);
		return(1);
	}
...
/*f---------------------------------------------------------------------
 * Interpolation 1D linear on TDate.
 * 
 * <br><br>
 * Linearly interpolates an array of points
 * $(x^a_i,y^a_i)_{i=0,\dots,m-1}$ at a point $x$.
...
static	char	routine[] = "DrlLinearInterp2d";
	...
	/* x1-coord */
	if (n1 < 1) {
		GtoErrMsg("%s: not enough points in dim 1 (%d).\n",
			routine, n1);
		return(FAILURE);
	} else if (n1 == 1) {
		i = ip1 = 0;
	...
	/* x2-coord */
	if (n2 < 1) {
		GtoErrMsg("%s: not enough points in dim 2 (%d).\n",
			routine, n2);
		return(FAILURE);
	} else if (n2 == 1) {
		j = jp1 = 0;

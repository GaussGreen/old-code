    ...
    KOptResult optResult)                 /* Return result type                 */
{
    ...
    if (Y < OPT_MIN_FWD) return FAILURE;

    /* compute vega? */
    vOn = (optResult == OPT_VEGA);
 ...
 *
 * Alib comment:  The funtion has a relative accuracy no worse than 1.0E-14, 
...
int Crx2QCalibrate(
    ...
    static char routine[] = "Crx2QCalibrate";
    int status = FAILURE;
...
{
    int status = FAILURE;
    static char routine[] = "Crx2QForward";
    double alpha;
    ...
    if (sigQ<0)
    {
        DR_Error("%s failed - negative driver volatility %lf", routine, sigQ);
        goto RETURN;
    }
...
{
    int status = FAILURE;
    static char routine[] = "Crx2QOptionPrice";
    ...
    if (s<0)
    {
        DR_Error("%s failed - negative total volatility %lf", routine, s);
        goto RETURN;
    }
...
{
    static char routine[] = "Crx2QVolFinder";
    int status = FAILURE;
    ...
    ) 
{
    ...
    static char routine[] = "Crx2QImpliedVol";
    int status = FAILURE;
    double data[9];
    data[0] = price;
    ...
    {
        DR_Error("%s failed - non positive price %lf", routine, price);
        goto RETURN;
    }
    else if (FAILURE==

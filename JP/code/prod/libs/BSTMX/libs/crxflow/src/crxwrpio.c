    ...
    CR_INPUT **CrInp)                     /* (O)                                */
{
    ...
    if (NbSpInpL > 0)
    {
        ...
        for (i=0; i<NbSpInpL; i++)
        {
            ...
            if (readerror != 2)
            {
                DR_Error("summaryInputEnvData: Cannot read Base IR id / reference curve.");
                goto RETURN;
            }
        }
    }
...

    {
        ...
        if (isFileNeeded)
        {
            if (Param_Input (&(irInput->NbFactor),
                             ...
                             &(irInput->FwdShift),
                             3,
                             FNameModlPar) == FAILURE) goto RETURN;
                ...
                char      *SmileOWS)      /* (I) 2q string                      */
{
    ...
    /* Optional additional recovery parameters for recovery correlation model */
    if (FindAndSkipSectionLine(2, stream, "recovery dispersion", "crInputEnvData", 
        FNameInfo) != FAILURE)
    {        

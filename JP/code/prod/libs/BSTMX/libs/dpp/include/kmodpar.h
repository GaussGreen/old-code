...
virtual void	ReadDrw(
	...
	/**
	 * Reads the both FIX and TMX smile parameters in a 
     * DRW data file stream is and performs the overwrites (
     * given a DRW environment drWrapData). 
     * Firstly, identify which tree model is using, TMX or FIX
     * Secondly, read smile parameters.
     * The recognized format for the data file is
	 * L, N or a series of eleven numbers (skew, vov, bbv, bbr,
	 * deltaL, tauL, deltaR, tauR, cutoff, NCK, nbIter) for TMX or
     * four numbers (q1, q2, forward shift
	 * and Black volatilities iterations) for FIX. 
	 */
virtual void	ReadDrw_x(
		istream& is,			// (I) wrapper data stream
		TDrWrapperData *drWrapData);	// (I) wrapper market data 
...
virtual void	ReadMagnet(const KVector(double) &smileParams);
	 ...
	 * Reads the parameters from Magnet interface.
	 * The array of the smile parameters are:
	 * skew, vov, bbv, bbr, deltaL, tauL, deltaR, tauR
     * MQNormT, MQNck, MQNbDevL, MQBinMapStdL, MQBinMapStdR;
     * mNumIter
	 */
virtual void	ReadMagnet_tmx(const KVector(double) &smileParams,
                               const KVector(TDate)  &bmkLiqDates,
                               const KVector(TDate)  &smlDates);

virtual void    SetSmlDates(const KVector(TDate) &SmlDate);
    /**
     * Interpolate TMX smile parameters
     * and setup liquid benchmark dates
     */
virtual void    SmileDiag(
        const KVolCalibIndex &calibIdx,
        TDrWrapperData *drWrapData);

    /**
     * Check liquid benchmark dates
     */
virtual void    CheckLiqBmkDates(
    ...
    /* Default MultiQ parameters*/
    double  mSkew, mVov, mBBV, mBBR, 
            mDeltaL, mTauL, mDeltaR, mTauR,
            mCutoff, mNCK;

    double  MQNormT, MQNck, MQNbDevL, MQNbFwdL,
            MQBinMapStdL, MQBinMapStdR;

    /* Liquid benchmark dates*/
    KVector(TDate)      mLiqBmkDates;

    /* MQ Smile (skew, vov, bbv, bbr, deltaL, taulL, deltaR, tauR, 
     * cutoff, NCK)
     */
    KVector(TDate)      mSmlDates;
    
    KVector(double*)    mMQSmile;

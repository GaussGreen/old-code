...
static        char        routine[] = "BasisTWrapperRead";
    ...
    try {
        ...
        DppReadFromString(idx2s, volIdx2);
...
TmxBasisTWrapperRead(
        ...
        KSmileParam&     bsSmileParam,     // (O) Basis skew data.
        double&          irBsCorr,         // (O) IR basis correlation.
        int*             TmxOnFlag,        // (O) Tmx or Fix
        char*            CalibIdx)         // (O) Calibration index

{
static        char        routine[] = "TmxBasisTWrapperRead";
    ...
    try {
        ...
        // Distribution type
        //----------------------------------------------
        irSmileParam.ReadDrw_x(is, drWrapData);
        *TmxOnFlag = FALSE;
        if (irSmileParam.IsTmx())
        {
            *TmxOnFlag = TRUE;
            if (drWrapData->fSwapSmile == NULL ||
                drWrapData->fBaseSmile == NULL)
            {
                throw KFailure("%s Failed: cannot find TMX smile files: "
                        "swapsm.dat,  basesm.dat \n", routine);
            }
            if (drWrapData->MQParamFlag == FALSE)
            {
                throw KFailure("%s Failed: cannot find TMX parameter file: "
                        "xsmileparameters.dat \n", routine);
            }
            // Set Smile Date
            irSmileParam.SetSmlDates(irVolDiag.mVolDates);
            // Interpolate irSmile parameters
            irSmileParam.SmileDiag(volIdx1, drWrapData);
            irSmileParam.CheckLiqBmkDates(irVolDiag);
            // MultiQ constants
            irSmileParam.MQNormT      = drWrapData->MQNormT;
            irSmileParam.MQNck        = drWrapData->MQNck;
            irSmileParam.MQNbDevL     = drWrapData->MQNbDevL;
            irSmileParam.MQNbFwdL     = drWrapData->MQNbFwdL;
            irSmileParam.MQBinMapStdL = drWrapData->MQBinMapStdL;
            irSmileParam.MQBinMapStdR = drWrapData->MQBinMapStdR;
        }

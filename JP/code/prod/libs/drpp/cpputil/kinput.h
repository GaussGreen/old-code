...
public:
	...
	/** Opens a file and reads in the KInput
		...
		*/
	void open (string filename); // Create from file
	...
	friend ostream& operator<<(ostream&, const KInput&); //
	friend istream& operator>>(istream&, KInput&);

	/**@#-*/
	void print(ostream&, int level = 0) const ; // Level is the number of tabs
	/**@#+*/
...
protected:
	...
	KInput Parse(istream&);
	void ParseFile(string fileName);
	void ParseVector (istream& inFile);
	void ParseMap (istream& inFile);
	void ParseString (istream& inFile);
	void ParseQuote (istream& inFile);
	
	char PeekNextChar (istream& inFile);
	upper_string GetNextString (istream& inFile);
};
...
// It doesn't have a string class and nothing makes any fucking sense.
	string temp;
...
public:
	...
	friend ostream& operator<<(ostream&, const KInputVector&); //
/*@#-*/
	void print(ostream&, int level = 0) const ; // Level is the number of tabs
/*@#+*/
};
...
public:
	...
	upper_string get (upper_string name, upper_string defaultValue); // Gets a string; if doesn't exist, returns default
	/** Gets a string without default value */
	upper_string get (upper_string name); // Gets a string; if it doesn't exist, error
	...
	friend ostream& operator<<(ostream&, const KInputMap&); //
/*@#-*/
	void print(ostream&, int level = 0) const ; // Level is the number of tabs
/*@#+*/

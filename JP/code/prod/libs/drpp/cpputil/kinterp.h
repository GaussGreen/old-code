...
class KInterp {
public:
	/** Gets a step interpolation */
	static KInterp step() {return KInterp(STEP);}
	...
	*/
	Result operator()(const Arg& a, const pair<Arg,Result>& p1, const pair<Arg,Result>& p2) const
	{
		if (_t == STEP) {
			if (a < p1.first) return p1.second;

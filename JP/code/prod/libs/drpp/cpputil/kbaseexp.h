...
public:
	...
	virtual Arg Eval (const SymbolMap& m)
	{
		SymbolMap::const_iterator iter = m.find(_s);
		if (iter == m.end()) throw DRException ("Undefined symbol ") << _s;
		return (*iter).second;
	}
private:
	upper_string _s;
...
public:
	...
	virtual Arg Eval (const SymbolMap& m) {if ((*_p).Eval(m)) return (*_r1).Eval(m); else return (*_r2).Eval(m);}
private:
...
public:
	...
	virtual Arg Eval (const SymbolMap&) {return _a;}
private:
	Arg _a;
};
...
public:
	...
	virtual Arg Eval (const SymbolMap& m) {return (*_p).Eval(m);}
private:
	BaseExpPtr _p;
};
...
public:
	...
	virtual Arg Eval (const SymbolMap& m) {return _f((*_p).Eval(m));}
private:
	BaseExpPtr _p;
	UnaryFunc _f;
};
...
public:
	...
	virtual Arg Eval (const SymbolMap& m) {return _f((*_p1).Eval(m), (*_p2).Eval(m));}
private:
	BaseExpPtr _p1, _p2;
	BinaryFunc _f;
};
...
public:
	...
	virtual Arg Eval (const SymbolMap& m) {return (_t==PLUS) ? +(*_p).Eval(m) : - (*_p).Eval(m);}
private:
	BaseExpPtr _p;
	Type _t;
};

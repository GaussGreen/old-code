 ...
 * using the system library version would almost certainly be quicker
 */
int CxStrcmpi (const char *p1, const char *p2)
{
    int c1;
    int c2;
...
{
    static char routine[] = "CxStringConcatenate";
...
{
   static char routine[] = "CxStringToUpper";
   char *out=NULL;
   char *outPtr;
...
{
    static char routine[] = "CxStringToLower";
    char *out;
    char *outPtr;
...
{
    static char routine[] = "CxStringParser";
    int         status    = FAILURE;
...
{
    static char routine[] = "CxStringSplit";
    int         status    = FAILURE;
    ...
    */
    mySplit  = (char**)(CxMemAlloc(sizeof(char*)*(myNumItems+1)+len+1));
    if (mySplit == NULL)
        goto done; /* failure */

...
static CxTDateCacheEntry DATE_CACHE[] = {
...
#define DATE_CACHE_SIZE  (sizeof(DATE_CACHE)/sizeof(CxTDateCacheEntry))
...
)
{
    ...
    if (CxYMDToDate (&ymd, &date) != SUCCESS)
    {
...
(TDate          date,               /* (I) Days since 1/1/BASE_YEAR. */
 CxTYearMonthDay *ymd)                /* (O) Month/Day/Year format */
{
    static char routine[] = "CxDateToYMD";
    int         status    = FAILURE;
...
*/
int CxYMDToDate
(CxTYearMonthDay *ymd,                /* (I) Month/Day/Year format */
 TDate         *odate)              /* (O) Days since 1/1/BASE_YEAR. */
{
    static char routine[] = "CxYMDToDate";
    int         status    = FAILURE;
        ...
        ymd->year <= DATE_CACHE[DATE_CACHE_SIZE-1].year)
    {
        ...
        {
            GtoErrMsg("%s: Invalid date: %d/%d/%d\n",
    ...
    {
        GtoErrMsg("%s: date %ld/%ld/%ld  not valid.\n",
    ...
    if (month < 1 || month > 12 ||
        ymd->year < BASE_YEAR)
    {
        GtoErrMsg("%s: date %ld/%ld/%ld is not valid.\n",
                routine, ymd->month, ymd->day, ymd->year);
        goto done;
    }
...
***************************************************************************
*/
int CxNormalizeYMD(CxTYearMonthDay *ymd)  /* (I/O) */
{
    static char routine[]="CxNormalizeYMD";
...
{
    static char routine[] = "CxDateAddMonths";
    int         status    = FAILURE;

    TDate output;
    CxTYearMonthDay ymd;
    TBoolean       lastDay = FALSE;

    if (months == 0) return date;

    if (CxDateToYMD(date, &ymd) != SUCCESS) goto done;
    if (eom)
    {
        lastDay = ymd.day == CxDaysInMonth (ymd.year, ymd.month);
    }
    
    ymd.month += months;
    if (CxNormalizeYMD (&ymd) != SUCCESS) goto done; /* failure */
    
    if (CxYMDToDate (&ymd, &output) != SUCCESS) goto done; /* failure */
    if (eom && lastDay)
    {
        output = CxDateToEndOfMonth(output);
    }
    status = SUCCESS;
...
)
{
    ...
    if (CxDateToYMD (date, &ymd) != SUCCESS) return -1;
    day = CxDaysInMonth (ymd.year, ymd.month);
    return CxDate(ymd.year, ymd.month, day);
}
...
)
{
    ...
    if (CxDateToYMD (date, &ymd) != SUCCESS) return -1;
    return CxDate(ymd.year, ymd.month, 1);
}
...
{
    static char routine[] = "CxDateNthWeekDay";
    int         status =    FAILURE;
    TDate date = FAILURE;
    long dow;
    long day;

    REQUIRE(weekday >= CX_SUNDAY && weekday <= CX_SATURDAY);

    /*
    ** Intercept some inputs to ensure that we do not end up with unpleasant
    ** messages from CxYMDToDate.
    */
    if (whichOne < 1 || whichOne > 5 ||
        month < 1 || month > 12 ||
        year < 1601)
    {
        goto done; /* failure - the final message will capture this */
    }

    day = (whichOne-1)*7 + 1;
    date = CxDate(year, month, day);
    if (CX_INVALID_DATE(date)) goto done;
    dow  = CX_DAY_OF_WEEK(date);
     ...
     */
    if (day > 28 && day > CxDaysInMonth(year, month))
    {
        goto done;                      /* Failed */
    }
    ...
    if (status != SUCCESS)
    {
        ...
        GtoErrMsg("%s: %ld%s %s of %ld/%ld does not exist.\n",
                routine, whichOne, str, CxFormatDayOfWeek(weekday),
                month, year);
        return -1;
    }
...
char* CxFormatDayOfWeek(long dayOfWeek) /* (I) */
{
    ...
    default: 
        GtoErrMsg("CxFormatDayOfWeek: Unknown day of week %ld.\n", dayOfWeek);
        return("Unknown");
    }
    /*NOTREACHED*/
}
...
{
    static char routine[] = "CxDateFormat";
    ...
    if (CxDateToYMD (date, &ymd) != SUCCESS) goto done;
    dow = CxDayOfWeek (date);

    myFormatAlloc = CxStringToUpper (format);
    if (myFormatAlloc == NULL) goto done; /* failure */
    ...
    while ((c = *myFormat) != '\0')
    {
        ...
        if (isalpha((int)c))
        {
            ...
            case 'D':
                ...
                case 3:
                case 4:
                    strcpy (add, CxFormatDayOfWeek(dow));
                    if (count == 3) add[3] = '\0';
                    convertCase (add, textCase);
                    break;
                default:
                    GtoErrMsg ("%s: Cannot have more than 4 adjacent "
            ...
            case 'Q':
                ...
                case 2:
                    ...
                    break;
                default:
                    GtoErrMsg ("%s: Cannot have more than 2 adjacent "
            ...
            case 'Y':
                ...
                case 4:
                    ...
                    break;
                default:
                    GtoErrMsg ("%s: Must have either 2 or 4 adjacent letter "
            ...
            case 'X':
                ...
                continue; /* no text to add */
            default:
                GtoErrMsg ("%s: Invalid letter (%c) in date format string %s. "
                           "Must be D,M,Q,Y,U,L or X\n",
                           routine, c, format);
                goto done; /* failure */
            }
            tmp = buf;
            buf = CxStringConcatenate (2, tmp, add);
            if (buf == NULL) goto done; /* failure */
            FREE(tmp);
        }
        else if (isdigit((int)c))
        {
            GtoErrMsg ("%s: Cannot have numbers in date format string %s\n",
                     routine, format);
            goto done; /* failure */
        }
        else if (isprint((int)c))
        {
            ...
            punct[1] = '\0';
            tmp = buf;
            buf = CxStringConcatenate (2, buf, punct);
            FREE (tmp);
            if (buf == NULL)
                goto done; /* failure */
        }
        ...
        {
            GtoErrMsg ("%s: Cannot have unprintable characters in date format"
                     "string %s\n", routine, format);
            goto done; /* failure */
        }
    }
    ...
    if (str == NULL) GtoErrMsgFailure (routine);
...
(const char* str,
 ...
 TDate*     result)
{
    static char routine[] = "CxStringToDate";
    int         status    = FAILURE;
    ...
    {
        char *ep;
        long yyyymmdd = strtol(str, &ep, 10);
        long year;
        long month;
        long day;
        if (*ep != '\0')
        {
            GtoErrMsg ("%s: Date string \"%s\" is not an integer\n",
                     routine, str);
            goto done; /* failure */
        }
        year  = yyyymmdd / 10000;
        ...
        day   = yyyymmdd % 100;
        date  = CxDate (year, month, day);
        if (date == FAILURE) goto done; /* failure */
    }
    else
    {
        GtoErrMsg ("%s: Invalid string format \"%s\" - must be \"YYYYMMDD\"\n",
                 routine, format);
        goto done; /* failure */
    }

...
(long    xDesired,  /* (I) Value for which we seek */
 ...
 long   *hiBound)   /* (O) Index of first value strictly greater than xDesired,
                       -1 if no such value exists. */
{
    static char routine[] = "CxBinarySearchLong";
    int         status    = FAILURE;
...
#define ELEMENT(idx) *(long*)(xp + skip*(idx))
    ...
    if (count == 0)
    {
        GtoErrMsg("%s: x array not in increasing order.n", routine);
        return FAILURE;
    }
...
**
** For details see comments for CxBinarySearchDouble, replacing long with
** double throughout.
***************************************************************************
*/
int CxBinarySearchDouble
(double  xDesired,  /* (I) Value for which we seek */
 ...
 long   *hiBound)   /* (O) Index of first value strictly greater than xDesired,
                       -1 if no such value exists. */
{
    static char routine[] = "CxBinarySearchDouble";

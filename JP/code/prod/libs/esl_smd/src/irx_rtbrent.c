...
$Header: /home/alibcvs/alibcvs/.cvsroot-alib/analyts/src/rtbrent.c,v 1.19 2000/11/06 17:15:16 smeldrum Exp $
-------------------------------------------------------------------------  */

#include "irx/rtbrent.h"                       /* Prototype consistency */

#include <math.h>
#include <ctype.h> 
#include "irx/macros.h"
#include "irx/cgeneral.h" 
#include "irx/error.h" 

static int secantMethod 
    (IrxTObjectFunc funcd,     /* (I) Name of function to call */
     ...
     IrxTBool   *foundIt,      /* (O) If solution was found */
     IrxTBool   *bracketed,    /* (O) If root was bracketed */
...
static char routine[]="irxRootFindBrent"; 
...
-------------------------------------------------------------------------  */
int   irxRootFindBrent(           
   IrxTObjectFunc funcd,                /* (I) Function to call */
   void       *data,                    /* (I) Data to pass into funcd */
...

{
   ...
   IrxTBool    bracketed;              /* If root was bracketed by secant */
   IrxTBool    foundIt;                /* If root was found by secant */
   ...
   if (boundLo >= boundHi)
   {
       irxError ("%s: Lower bound (%2.6e) >= higher bound (%2.6e).\n",
   ...
   {
       irxError ("%s: Guess (%2.6e) is out of range [%2.6e,%2.6e].\n", 
   ...
   {
       irxError("%s: Supplied function failed at point %2.6e.\n",
                 routine, xPoints[0]);
       return FAILURE;
   }

   /* Check if guess is the root (use bounds for x-accuracy) */
   if (IS_ALMOST_ZERO(yPoints[0]) || 
   ...
   if (IS_ALMOST_ZERO(initialXStep)) 
   {
   ...
   /* Take a step of the size passed in, if the derivative is not
      passed in */
   if (IS_ALMOST_ZERO(initialFDeriv))
   {
   ...
   if ( xPoints[2] < boundLo || xPoints[2] > boundHi ) 
   {
       ...
       if ( ARE_ALMOST_EQUAL(xPoints[2], xPoints[0]) )
       {
           /* We cannot have xPoints[0] and 
            * xPoints[2] be the same. 
            */ 
           if ( ARE_ALMOST_EQUAL(xPoints[2], boundLo) ) 
           {
   ...
   if (IS_ALMOST_ZERO(fLo) || (ABS(fLo) <= facc && ABS(boundLo - xPoints[0]) <= xacc))
   {
       *solution = boundLo; 
       return SUCCESS;
   }
   ...
   /* Root is still not bracketed, so try at the upper bound now. */
   {
       ...
       if (IS_ALMOST_ZERO(fHi) || (ABS(fHi) <= facc && ABS(boundHi-xPoints[0]) <= xacc)) 
       {
           *solution = boundHi; 
           return SUCCESS;
       }
       ...
       {
           irxError("%s: Function values (%2.6e,%2.6e) at bounds\n"
...
---------------------------------------------------------------------- */
static int brentMethod 
    (IrxTObjectFunc  funcd,     /* (I) Function to evaluate */
     void        *data,         /* (I) Data to pass into funcd */
     int         numIterations, /* (I) Maximum number of iterations*/
     double      xacc,          /* (I) X accuracy tolerance */
     double      facc,          /* (I) Function accuracy tolerance */
     ...
     double      *solution)     /* (O) X root found */
{
    ...
    for (j=1; j<=numIterations; j++) 
    {
        ...
        else 
        {
            ...
            if (IS_ALMOST_ZERO(fm) || (ABS(fm) <= facc && ABS(xm-x1) <= xacc))
            {
                *solution=xm;
                return SUCCESS;
            }
        ...
        if (IS_ALMOST_ZERO(f2) || (ABS(f2) <= facc && ABS(x2 - x1) <= xacc))
        {
            *solution = x2;
            return SUCCESS;
        }
    ...
    irxError("%s: Maximum number of iterations exceeded.\n", routine);
    return FAILURE;
}    

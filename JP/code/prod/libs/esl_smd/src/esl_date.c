...
int  ExtendTCurve(T_CURVE *tc, ESL_DATE fromDate, ESL_DATE toDate)
{
    ...
    if ( tc->ValueDate < fromDate )
    {
        DR_Error("%s: curve value date %d < from date %d\n",routine, tc->ValueDate, fromDate);
        return FAILURE;
    }
    ...
    for (i=0; i<nbZero; i++)
    {
        AZero = ZeroPrice(tmpTc.ZeroDate[i],
                          tmpTc.ValueDate,
                          tmpTc.NbZero,
                          tmpTc.ZeroDate,
                          tmpTc.Zero);
        ...
        if (AZero <= TINY)
        {
            DR_Error("%s: zero < TINY for date %d\n",routine, tc->ZeroDate[i]);
        }

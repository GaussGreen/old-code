...
int irxMultiDimMinStateValidate (IrxTMultiDimMinState *state)
{
    static char routine[] = "IrxMultiDimMinStateValidate";
    int         status    = FAILURE;
    ...
    {
        state->direction = irxMatrixNewEmpty(state->n, state->n);
        if (state->direction == NULL) goto RETURN; /* failure */
        for (i = 0; i < state->n; ++i)
    ...
    else
    {
        ...
        {
            irxError ("%s: direction matrix has wrong dimensions [%d,%d] "
                       "instead of [%d,%d]\n", routine,
...
IrxTMultiDimMinState* irxMultiDimMinimization
(IrxTMultiDimMinState *initState,
 IrxTMultiDimObjFunc   func,
 IrxTMultiDimObjFunc   dfunc,
 void                *data,
 double               ftol,
 int                  maxiter,
 double               maxtime,
 char                *logfilename)
{
    static char routine[] = "irxMultiDimMinimization";
    int         status    = FAILURE;

    IrxTMultiDimMinState *state = NULL;

    FILE       *log = NULL;
    
    if (dfunc != NULL)
    {
        irxError ("%s: Methods using derivative function not yet supported\n",
                   routine);
        goto RETURN; /* failure */
    }
    ...
    if (logfilename != NULL && *logfilename != '\0')
    {
        ...
        if (log == NULL)
        {
            irxError ("%s: Could not open logfilename %s\n", routine, 
                       logfilename);
            goto RETURN; /* failure */
        }
    }

    state = irxMultiDimMinStateCopy (initState);
    if (state == NULL)
        goto RETURN; /* failure */

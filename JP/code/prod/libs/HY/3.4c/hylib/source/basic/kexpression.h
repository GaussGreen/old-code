...
public:
	...
	*/
	Arg eval (const SymbolMap& m) const {if (_e.null()) throw KException ("Can't eval null exp"); return (*_e).Eval(m);}
	
	friend std::ostream& operator<< (std::ostream& s, const KExpression& e) {s << e._text; return s;}
private:
	upper_string _text;
	SymbolSet _s;
	...
	BaseExpPtr Parse (std::istream& s)
	{
	...
	BinaryOpExp<Arg>::Type GetBinaryOp(std::istream& s)
	{
		char c = s.get();
		...
		if (c == '+') t = BinaryOpExp<Arg>::PLUS;
			...
			throw KException ("Invalid binary operator ") << c;
		
		CleanWhite(s);
		return t;
	}
	
	BaseExpPtr GetNextExpression (std::istream& s)
	{
		BaseExpPtr p;
		char c = s.peek();
		if (c == '-') {
			...
			if (c2 == '(') {
				...
				if (text == "IF") {
					...
					if (uIter != _uf.end()) {
						...
						if (bIter == _bf.end())
							throw KException ("Bad function name ") << text;
	...
	upper_string GetTillCharInSameParenScope (std::istream& s, char t)
	{
		int count = 0;
		upper_string a;
		char c;
		...
		if (s.eof() || s.peek() < 0)
			throw KException ("Failed to find ") << c;
		s.get();
		CleanWhite(s);
		return a;
	}
	
	upper_string GetText(std::istream& s)
	{
		upper_string ans;
	...
	static void CleanWhite (std::istream& s) 

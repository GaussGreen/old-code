...
void	AssetTree::treeSpotVol(void) 
{
	...
	double minJump = sqrt(Y_JUMPCOEFF*100);		//HY3.4
	double maxJump = 0.0;
	double currJump;
//	int i=1;
...
{
	int		dim, i;
	double	volShift, probCenterShift, fwdFactorShift;
...
//	out<<" dimFwd: "<<dim<<" disc: "<<1/fwdFactor<<"\n"<<std::endl;
	...
	if(m_isOneAssetVol == false)		// i.e., if it is a bond, see "defswap.cpp"
	{
		volShift = tp[__d1TreeSpotVolShift];											//HY3.4v
		probCenterShift = 1.0 - volShift*volShift*timeTillNextTp/maxJump/maxJump;		//HY3.4v
		fwdFactorShift = get_fwd_factor(true);											//HY3.4v
	}
	...
	{ 	
		...
		else
		{
			...
			//HY3.4v
			//prob with vol shift and zero beta
			...
			else
			{
				...
				param.m_probDvolShift[1] = param.m_probDvol[1];			//per 1% vol

...
//		double assetToBarrier = log(level*dps/asset_ts[ts_idx]);
		...
		for (i = k1; i <= k2; i++)
		{
			...
			{
				double assetDist = log(asset_ts[i]/asset_ts[ts_idx]);		//HY4.1a
...
//				temp2 = log(asset_ts[i] * asset_ts[ts_idx] / level / level);
//				prob1 = exp(-temp1*temp1/(2*vollim*vollim));
//				prob2 = exp(-temp2*temp2/(2*vollim*vollim));
//				prob = prob1 - prob2;
...
//			double dist = asset_ts[ts_idx]/level/dps * exp(vollim * vollim);
			...
			integratedValue = integratedValue * p_nd + get_defaultValue() * (1.0 - p_nd);

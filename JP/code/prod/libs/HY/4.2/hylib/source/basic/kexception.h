...
public:
	...
	KException (const char* msg,const char* rtname); 
...
/** Appends textual KString to exception message. 
...
inline
KException::KException (const char* msg,const char* rtname):mMsg(NULL)
{
	mMsg = new char[1]; mMsg[0] = 0;
	add (rtname, strlen(rtname));
	add (": ", strlen(": "));
	add (msg, strlen(msg));
}
...
inline
KException::~KException() {delete [] mMsg;}
...
inline
const char* KException::what() const {return mMsg;}

...
{
	StrikesClass::const_iterator iter;
	double returnValue;
	double strike;
	if(m_longShort == 0)
	{
	...
	//		out<<GtoFormatDate(currDate)<<std::endl;
			// if there is only one strike, american always
			// if there are multiple strikes, the period before first strike date is European

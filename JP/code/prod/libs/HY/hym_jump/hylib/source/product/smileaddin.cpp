...
MAGNET_FUNCTION15(Array<double>*,  Gen_Pricer, double spotStockPrice,CM::SharedPointer<DDMap> dividentYield, 
				 ...
				 double vollim, double jumpProb, Date settleDate, bool isCVOption)
{
...
MAGNET_FUNCTION15(Array<double>*,  Gen_Pricer2, double spotStockPrice,CM::SharedPointer<DDMap> dividentYield, 
				 CM::SharedPointer<KRateCurve> repoCurve,CM::SharedPointer<KRateCurve> irCurve,
				 CM::SharedPointer<KRateCurve> volCurve, CM::SharedPointer<KRateCurve> volCurveShift,
				 CM::SharedPointer<Instrument> instrument, 
				 CM::SharedPointer<BaseFunction> assetToStockMapping, 
				 CM::SharedPointer<BaseFunction> assetProcess, double ppy, double beta,
				 double vll, double jumpProb, Date settleDate, bool isCVOption)
{
	Array<double>* result = new Array<double>;
	TDate tSettleDate = settleDate;
	KDate kSettleDate = tSettleDate;
	...
	double jumpProbConst[2];
	jumpProbConst[0] = 1;
	jumpProbConst[1] = jumpProb;
	...
	if(HYMCapitalWrapper(spotPrice,
						 ...
						 lim2,
						 vollim,
//						 jumpProbConst,					//need to change Kapital interface to accept this additional parameter.
						 x,
						 lim,
						 betaa,
						 assetProcessType,

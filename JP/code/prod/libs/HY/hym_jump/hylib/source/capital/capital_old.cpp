...
HY_EXPORT char*  HY_version(){return (char*)"HY V3.3";}
...
(double*	spotPrice,	/*	1	(I)	equity spot price  */
 ...
 long*		volDates,	/*	9	(I) asset vol curve dates*/
 double*	volRates,	/*	10	(I) asset vol rates if volRefSpot==0, otherwise stock vols*/
 double*    volShift,   /*  11  (I) asset vol shift */
 /*
  *  instrument description 
  */
  long*		instType,			/*	12	(I) instrument type */
...
//	KRateCurve   irCurve(valueDates[2],&swapDates[1],&swapRates[1],swapDates[0],1,1)
	...
	KRateCurve  kvolCurve1(valueDates[1],&volDates[1],assetVols,volDates[0]);
...
/*		if(couponAmounts[i] < 0 || amortAmounts[i] <0)
	...
	case  BONDOPT:
		...
		for(i=0; i<exerEndDates[0]; i++)
		{
			switch(optionType[i+1])
			{
			case HY_CAPITAL_CLEAN:
				strikeDates[numStrikes] = exerEndDates[i+1];
				strikes[numStrikes] = exerEndStrikes[i+1];
				numStrikes++;
				longShort = optionDirections[i+1];
				exerType = exerTypes[i+1];
				break;
			case HY_CAPITAL_SOFTCALL:
				softStrikeDates[numSoftStrikes] = exerEndDates[i+1];
				softStrikes[numSoftStrikes] = exerEndStrikes[i+1];
				numSoftStrikes++;
				break;
			case HY_CAPITAL_SHARES:
				convertDates[numConvertStrikes] = exerEndDates[i+1];
				convertStrikes[numConvertStrikes] = exerEndStrikes[i+1];
				numConvertStrikes++;
				convertLongShort = optionDirections[i+1];
				exerTypeConvert = exerTypes[i+1];
				break;
			default:
		...
		if(numStrikes != 0)
		{
			sc = new StrikesClass(strikeDates,strikes,numStrikes,"S");
		}
		if(numSoftStrikes != 0)
		{
			scSoft = new StrikesClass(softStrikeDates,softStrikes,numSoftStrikes,"S");
		}
		if(numConvertStrikes != 0)
		{
			scConvert = new StrikesClass(convertDates,convertStrikes,numConvertStrikes,"S");
		}
		if(numStrikes != 0)
		{
...
// HY3.4v
	...
	double assetVol_opt = volRate - *volShift;
	if(assetVol_opt <= 0.0)
	{

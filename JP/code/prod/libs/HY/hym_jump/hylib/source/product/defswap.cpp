...
//	KValarray<double>  result(9);
	...
	AssetTree 	tree(spotStockPrice,
					 ...
					 *repoCurve,
					 beta,
					 vollim,				//HY4.1v
					 jumpProb,				//jump version	
					 dividentYield,
					 isCVOption);

	if(Bond *pt_instrument = dynamic_cast<Bond*>(instrument))
	{
		tree.set_isOneAssetVolFalse();
	}
...
//	double timeShift = vollim * vollim/assetVol0/assetVol0 * 365.0;					//HY4.1v
	double timeShift = 0.0;												//jump version
...
//		AnnuCalFlag = -1;					//don't need annuity for risk management s/s
	}
...
//		dsAverageLife.update();			//old annuity calculation
	...
	double temp1 = instrument->get_spot_ts_value();
	double temp2 = instrument->get_option_ts_value();

	result[0] = (temp1 + temp2)/settleFactor;		//HY3.4v

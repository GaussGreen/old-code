...
extern	double	DrlDoubleRand(long *idum,
					double xLow, double xHigh);
extern	double	DrlGaussSimul(long *idum);
extern	int		DrlIntRand(long *idum, int iLow, int iHigh);


/* Generation of multidimensional normal deviates */

extern	int	DrlMultiNormSimulInit(long seed, int nDim,
				double **corrMat);
extern	int	DrlMultiNormSimulGet(double *deviate);
extern	int	DrlMultiNormSimulDispose(void);

/* Monte-Carlo on a parsed formula (The Parser) */

extern	int	DrlFormMonteCarlo(
...
extern	int	DrlSobolSequence(int *n, double *x);

...
static	long	inf[3];
	...
	if (ifail != 0) {
	    DrlErrMsg("CumTriNorm: x=(%lf, %lf, %lf) "
		"r=(%lf, %lf, %lf) N3=%lf\n",
		x1, x2, x3, r12, r13, r23, prob);
	     DrlErrMsg("            errCode = %d    IFAULT = %d\n",
		errCode, ifail);
...
#else
	DrlErrMsg("CumTriNorm: not supported.\n");
	return (1e39);
#endif
}

...
DrlFilePrintf(char *fnam, char *fmt,...)
{
	...
	if (!strcmp(fnam, "stdout")) {
		...
		DrlErrMsg("FilePrintf: can't open `%s'\n", fnam);
		return(-1);
	    } else {
		fputs(buf, fp);
		fclose(fp);
...
#if defined(DRL_CLIB) && (defined(_WINDLL) && !(defined(_WIN32) && defined(WIN32)))
int
_dummyFscanf(FILE *fp, char *fmt,...)
{
	DrlErrMsg("fscanf not implemented\n");
	return(0);
}
#endif
...
static	char	devNullFnam[] = "/dev/null";
	...
	if ((newfd = dup(STDOUT_FILENO)) < 0) {
		DrlErrMsg("%s: duplicate failed (%s)\n",
			routine, DrlStrerror());
		return(-1);
	}
	_stdoutSaveFd = newfd;

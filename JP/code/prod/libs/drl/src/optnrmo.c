...
static	char	routine[] = "DrlNormOption";
	...
	if (texp < 0e0) {
		DrlErrMsg("%s: time to expiration (%lf) < 0e0.\n",
			routine, texp);
		goto done;
	}

	if (vol < 0e0) {
		DrlErrMsg("%s: normal volatility (%lf) < 0e0.\n",
			routine, vol);
		goto done;
	}


	d1 = (p-k)/(vol*sqrt(texp));
	...
	case 'c':
		...
		case 'p': /* price */
		case 'P':
			v = (p-k)*N(d1) + vol*sqrt(texp)*N1(d1);
			break;
		case 'd': /* delta */
		case 'D':
			v = N(d1);
...
#define	BLPRICE(z)	(DrlNormOption(texp, fp, (z), k, cp, what, &value),\
	...
	DrlErrMsg("%s: too many iterations.\n", routine);
	return(FAILURE);

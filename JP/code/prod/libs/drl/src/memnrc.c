 ...
 * <tr> <td>DrlDDate	 </td> <td> DDate	</td> </tr>
 * <tr> <td>DrlDDateP	 </td> <td> DDate*	</td> </tr>
 * <tr> <td>DrlDDatePP	 </td> <td> DDate**	</td> </tr>
 * </table>
 */
...
type_name* routine_name(long nl, long nh) \
{ \
	type_name *v; \
	v = (type_name*) \
		MALLOC((size_t) ((nh-nl+1+NR_END)*sizeof(type_name))); \
	if (!v) { \
		DrlErrMsg("%s: alloc failure (size=%d)\n", \
...
void routine_name(type_name *v, long nl, long nh) \
{ \
	if (v) \
...
type_name** routine_name(long nrl, long nrh, long ncl, long nch) \
{ \
 ...
 \
	...
	if (!m) { \
		DrlErrMsg("%s: alloc failure 1 (nrow=%d)\n", \
			#routine_name, nrow); \
		return(NULL); \
	} \
	m += NR_END; \
 ...
 \
	...
	if (!m[nrl]) { \
		DrlErrMsg("%s: alloc failure 2 (ncol=%d)\n", \
			#routine_name, ncol); \
		return(NULL); \
	} \
	m[nrl] += NR_END; \
...
void routine_name(type_name**m, long nrl, long nrh, long ncl, long nch) \
{ \
	if (m) { \
...
{ \
 ...
 \
	...
	if ((m = (type_name***)MALLOC(sz)) == NULL) { \
		DrlErrMsg("%s: alloc failure 1 (n=%d)\n", \
			#routine_name, n);  \
		return(NULL); \
	} \
	m += NR_END; \
 ...
 \
	...
	for(i=nl; i<=nh; i++) { \
	    if ((m[i] = (type_name**)MALLOC(sz)) == NULL) { \
		DrlErrMsg("%s: alloc failure 2 (nrow=%d)\n", \
			#routine_name, nrow);  \
		return(NULL); \
	    } \
	    m[i] += NR_END; \

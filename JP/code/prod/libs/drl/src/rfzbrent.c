...
static	char	routine[] = "DrlRootFinderBrent";
	...
	if ((fa > 0.0 && fb > 0.0) || (fa < 0.0 && fb < 0.0)) {
		DrlErrMsg("%s: low  bound f(%6.4e) = %6.4e\n", routine, a, fa);
		DrlErrMsg("%s: high bound f(%6.4e) = %6.4e\n", routine, b, fb);
		DrlErrMsg("%s: root must be bracketed\n", routine);
		return(FAILURE);
	}
	fc=fb;
	...
	DrlErrMsg("%s: maximum number of iterations exceeded\n",
		routine);
	return(FAILURE);
}
#undef EPS
...
#define	IF_ERRMSG(statement)	if (errMsgFlag) { statement;}
	...
	*indic = FALSE;
	...
	case 2:
		 ...
		 */
		if (fa * fb > 0e0) {
		    IF_ERRMSG(DrlErrMsg("%s: failed bracketing: "
	...
	default:
		...
		*X = b ;
		...
		/* too many iterations */
		if (ITER > itmax) {
		    IF_ERRMSG(DrlErrMsg("%s: too many itertions "
			"(iter=%d > itmax=%d)\n", routine, ITER, itmax));
		    goto done;
		}
	}

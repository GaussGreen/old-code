...
#include "drltime.h"		/* DrlDIntervalNil() */
#include "drlts.h"
...
/*
static	int	_getInterpCoeffs(DSwopMat *that,
			double tExp, double tMat,
			int *ie, int *im, double *w);
static	int	_getMatInterpCoeffs(DSwopMat *that,
			int ie, double tMat,
			int *imlo, int *imhi,
...
{
static	char	routine[] = "DrlDSwopMatInterpMatTime";
	int	status = FAILURE;
	DDate	expDate;
	double	fm;
	...
	if ((finalFlag == TRUE) && (that->diagonal == FALSE)) {
	    ...
	    *interpMat -= tExp;
	    ...
	    if (DrlDayCountFract(baseDate, matDate,
		DRL_ACT_365F, interpMat)
		!= SUCCESS) goto done;

	} else if ((finalFlag == FALSE) && (that->diagonal == FALSE)) {
	    /* constant maturity in vertical matrix */
	    if (DrlDIntervalToYears(&matInt, interpMat) != SUCCESS)
		goto done;
...
#else
	    IF_FAILED_DONE( DrlDDateAdvanceYears(baseDate, tExp, &expDate));
#endif

	    if (DrlDDateFwdAny(expDate, &matInt, &matDate)
		!= SUCCESS) goto done;
	...
	if (tMat != NULL) {
            if (finalFlag == FALSE) {
                if (DrlDIntervalToYears(&matInt, tMat) != SUCCESS)
                        goto done;
            } else {
#ifdef	DRL_CLIB
		expDate = baseDate + (long) (tExp*365e0);
	        ...
	        IF_FAILED_DONE( DrlDDateAdvanceYears(baseDate, tExp, &expDate));
#endif
                if (DrlDayCountFract(expDate, matDate,
                        DRL_B30_360, tMat) != SUCCESS)
                        goto done;
            }
	}
	...
	if (freq != NULL) {
            if ((finalFlag == FALSE) && (that->diagonal == FALSE)) {
                if (DrlDIntervalToYears(&matInt, &fm) != SUCCESS)
                        goto done;
	...
	fprintf(stdout, "%s: tExp=%5.2f x %10s : interpMat=%lf\n",
		routine, tExp,
		(finalFlag == FALSE ? DrlDIntervalPrint(NULL, &matInt) :
...
int
DrlDSwopMatInterpMatrix(
	DSwopMat *that,	/* (B) swaption matrix */
	DSwopMat *interpMat,	/* (B) interp swaption matrix */
        DDate baseDate,			/* (I) base date */
	int adjointFlag)		/* (I) TRUE=rebucket, FALSE=interp */
{
static	char	routine[] = "DrlDSwopMatInterpMatrix";
	int	status = FAILURE;
...
#define	IMVOL(idxExp, idxMat) \
	...
	fprintf(stdout, "%s: Base date %s.\n", routine,
		DrlDDatePrint(NULL, baseDate));
#endif

	IF_FAILED_DONE( DrlDIntervalSet(0, 'M', &matInt));

	if (adjointFlag != FALSE) {
	    /* adjoint interpolation */
	    /* reset to zero */
	    if (DrlDSwopMatOperScalar(interpMat, "=", 0e0) != SUCCESS)
		goto done;
	    ...
	    fprintf(stdout, "%s: output matrix (interpMat)\n", routine);
            DrlDSwopMatFpWrite(interpMat, stdout,
...
#else
	   		IF_FAILED_DONE( DrlDDateAdvanceYears(baseDate,
						SWTMAT(j), &matDate));
#endif
		} else {
			if (DrlYearsToDInterval(SWTMAT(j), &matInt)
			    != SUCCESS) goto done;
		}

		/* compute interpolation maturity */
		if (DrlDSwopMatInterpMatTime(
			interpMat,
			baseDate,
			tExp,
		...
		if (IMDIAG == TRUE) {
			...
			if (DrlYearsToDInterval(IMTMAT(j), &matInt)
			    != SUCCESS) goto done;
		}
...
{
static	char	routine[] = "DrlDSwopMatInterpExpMat";
	int	status = FAILURE;
	int	k, ie[4], im[4];
	double	w[4];
...
/*f--------------------------------------------------------------
 * Same as <i> DrlDSwopMatInterpExpMat</i>,
 * but takes expiration date.
 */

int
DrlDSwopMatInterpDate(
	DSwopMat *that,/* (B) swaption matrix */
	double *value,		/* (B) interpolated value */
	DDate baseDate,		/* (I) base date */
	...
	int adjointFlag)	/* (I) TRUE=adjoint, FALSE=direct*/
{
	...
	if (DrlDayCountFract(baseDate, expDate, DRL_B30_360,
			&tExp) != SUCCESS)
				return(FAILURE);
...
DrlDSwopMatInterpValue(
	...
	int adjointFlag)	/* (I) TRUE=rebucket, FALSE=interp */
{
static	char	routine[] = "DrlDSwopMatInterpVol";
	int	status = FAILURE;
...
DrlDSwopMatDateInterpValue(
	...
	int adjointFlag)	/* (I) TRUE=rebucket, FALSE=interp */
{
static	char	routine[] = "DrlDSwopMatDateInterpValue";
	int	status = FAILURE;
...
{
static	char	routine[] = "DrlDSwopMatInterpVolCurve";
	int	status = FAILURE;
	...
	if (finalFlag == TRUE) {
		...
		if (DrlDayCountFract(baseDate, matDate, DRL_B30_360,
			&tMat) != SUCCESS) goto done;

		/* check maturity OK */
		if (tMat <= 0e0) {
		     DrlErrMsg("%s: too short maturity date %s (%lf).\n",
			routine, DrlDDatePrint(NULL, matDate), tMat);
		     goto done;
	        }
		...
		if (nActive == 0) {
		     DrlErrMsg("%s: too short maturity date %s (%lf yrs),"
			"first maturity on matrix is %lf yrs.\n",
			routine, DrlDDatePrint(NULL, matDate), tMat, SWTEXP(0));
		     goto done;
	        }
#endif/*NO_MIN_MAT*/
	...
	if (adjointFlag == FALSE) {
	    ...
	    if (volDates != NULL) {
		if ((*volDates = NEW_ARRAY(DDate, nActive)) == NULL)
			goto done;
		for (n=0; n<=nActive-1; n++) {
		    if (DrlDDateAdvanceYears(
			baseDate,
			SWTEXP(n),
			&(*volDates)[n]) != SUCCESS)
				goto done;
		...
		for (n=0; n<=nActive-1; n++) {
			if (DrlDDateAdvanceYears(
				baseDate,
				SWTEXP(n),
				&DRLTCURVE_DATE(*volCurve, n)) != SUCCESS)
...
#if defined(__DEBUG__)
	    fprintf(stdout, "%s:\n", routine);
	    if (volCurve != NULL) DrlDCurveFpWrite(*volCurve, stdout,
			DRL_TCURVE_FMT_STD);
#endif
		...
		if (volRates != NULL) {
			...
			vol = DRLTCURVE_RATE(*volCurve, n);
		} else {
			DrlErrMsg("%s: adjoint interpolation, but volRates and"
				" volCurve both NULL.\n", routine);
				goto done;
		}
...
int
DrlDSwopMatL2Product(
	DSwopMat *that,	/* (I) matrix #1 */
	DSwopMat *mat2,	/* (I) matrix #2 */
	double *val)			/* (O) */
{
static	char	routine[] = "DrlDSwopMatL2Product";
register int	i, j;
...
#define	SW2VOL(idxExp, idxMat) \
	...
	if (!DrlDSwopMatIsSameType(that, mat2)) {
		DrlErrMsg("%s: matrices of different type.\n", routine);
		return(FAILURE);
	}

...
#include "drltime.h"		/* DrlStrerror() */

static	char*	strReplaceNonPrintChar(char *s);

#define	CASTSTR(ptr, nidx)	(&(((char*)ptr)[DRL_WRAP_STR_IDX(nidx+1)]))
...
int
DrlLilToCType(DVType varType)		/* (I) variable type */
{
...
int
DrlLilVectSize(DVType varType, void *lptr, char *argName)
{
static	char	routine[] = "DrlLilVectSize";
	...
	case DRL_PERCENT_L:
	case DRL_BPOINT_L:
		return DRL_ARGSIZE((double*) lptr);
	case DRL_INT_L:
		return DRL_ARGSIZE((long*)   lptr);
	case DRL_TDATE_L:
		return DRL_ARGSIZE((long*)   lptr);
	...
	case DRL_TDAYCOUNT_T:
		DrlErrMsg("%s: [%s] non LIL type (%s).\n",
			routine, argName, DrlVTypeName(varType));
		return(-1);

	default:
		DrlErrMsg("%s: [%s] bad type %ld.\n",
...
int
DrlLilVectActiveSize(DVType varType, void *lptr, char *argName)
{
static	char	routine[] = "DrlLilVectActiveSize";
	...
	case DRL_PERCENT_L:
	case DRL_BPOINT_L:
		sz = DRL_ARGSIZE((double*) lptr);
		break;
	case DRL_INT_L: /* long -> int */
		sz = DRL_ARGSIZE((long*) lptr);
		break;

	case DRL_TDATE_L: /* DDate: check > 1 */
		sz = DRL_ARGSIZE((long*) lptr);
		for (i=0; i<=sz-1; i++) {
	...
	case DRL_TDATEINTERVAL_L: /* DInterval */
		sz = DRL_ARGSIZE((double*) lptr);
		break;

	case DRL_CHAR_BLOCK_L:
	case DRL_CHAR_L:
		/*sz = DRL_ARGSIZE((char*) lptr);*/
		sz  = (int)(lptr == NULL ? 0 :
			(unsigned char)((char*) lptr)[0]);
		break;
	 ...
	 */
	if (sz <= 0) {
	    DrlErrMsg("%s: argument `%s' bad array length %d (type %s)\n",
...
#define	LSTR	(&(((char*)lptr)[DRL_WRAP_STR_IDX(loffset+lskip*i+1)]))
...
#define	CVAL(t)	(((t) cptr)[i])
	...
	case DRL_TDATEINTERVAL_L: /* LIL: double <-> C: DInterval */
		FORALL {
			dVal = LVAL(double*);
			if (!DrlDoubleIsFinite(dVal)) {
			    DrlErrMsg("%s: bad DInterval value.\n",
				routine);
				return(5);
			}
			if (DrlYearsToDInterval(dVal, 
				((DInterval*) cptr)+i) != SUCCESS)
					return(FAILURE);
		}
		break;

	case DRL_TDATE_L: /* DDate */
		FORALL {
			CVAL(DDate*) = LVAL(DDate*);
		}
		break;
...
DrlLilVectPut(
	...
	DVType varType,		/* (I) variable type (see header) */
	int loffset,		/* (I) offset on LIL vector */
	int lskip,		/* (I) interval bet. LIL values */
	int n,			/* (I) size */
...
#define	CVAL(t)	(((t) cptr)[i])
	...
	case DRL_TDATEINTERVAL_L: /* DInterval */
	    FORALL {
		if (DrlDIntervalToYears(&(CVAL(DInterval*)),
		    &LVAL(double*)) != SUCCESS)
			goto done;
	    }
	    break;
...
#define	LSTR	(&((char*)lptr)[DRL_WRAP_STR_IDX(n2*i+j+1)])
...
#define	CHECKSZ(sz0)	{if (((sz0) < nMinItems) || ((sz0) > nMaxItems) ||\
		(nMinItems > nMaxItems)) {\
		DrlErrMsg("%s: [%s] expected %d <= #items <= %d, got %d\n",\
	...
	while ((strType = (DVType) va_arg(ap, DVType)) != 0) {
		...
		*nItems = sz;
...
#ifdef	__DEBUG__
		DrlErrMsg("%s: read `%s', type=%s, nItems=%d, allocType=%d\n",
			routine, argName[0], DrlVTypeName(varType[0]),
			*nItems, allocType[0]);
		...
		sz = DrlLilVectActiveSize(varType[0], lptr[0], argName[0]);
		if (sz != nx*ny) {
		    DrlErrMsg("%s: [%s] expected matrix size %dx%d=%d, "
			" got range size %d\n",
		...
		*nItems = sz;
		...
		if ((sz = DrlLilVectSize(varType[0], lptr[0], argName[0])) <= 0) {
		    DrlErrMsg("%s: [%s] LIL array of length %d\n",
			routine, sz);
		    goto done;
		}
		if ((sz - nVar*((int) (sz/nVar))) != 0) {
		    DrlErrMsg("%s: [%s] expected range of width %d, "
...
#define	MAXARG	32	/* maximum # of args */
	...
	while ((strType = (DVType) va_arg(ap, DVType)) != 0) {
		...
		*nItems = sz;
...
#endif
		DrlErrMsg("%s: [XXX] not implemented\n", routine);
		goto done;
		 ...
		 * For Excel
		 ************************************************/
		DrlErrMsg("%s: MATR2VECT not implemented\n", routine);
		goto done;
...
#define	FORALL	for (i=0; i<=n-1; i++)
#define	LSTR	(&(((char*)lptr)[DRL_WRAP_STR_IDX(i+1)]))
#define	RSTR	(&(((char*)rptr)[DRL_WRAP_STR_IDX(i+1)]))
...
#define	PRINTF(fmt, val)	{if (fp != NULL) fprintf(fp, fmt, val);\
	...
	if (sz > 0) {
		...
		case DRL_TDATE_L:
			FORALL(PRINTF("\t%s",
				DrlDDatePrint(NULL, CASTTYPE(lptr, idx, DDate))));
			break;
...
DrlLilVectArrayFpRead(
	...
	DVType *varType,	/* (I) array of variable types [0..numVect-1] */
...
static	char	routine[] = "DrlLilVectArrayFpRead";
	...
	for (idx=0; idx<=numItems-1; idx++) {
		...
		if (DrlFScanVType(fp, varType[idxV], vptr) != SUCCESS) {
		    DrlErrMsg("%s: can't read element #%d/%d "
			"in column # %d/%d "
			"(expecting type %s).\n", routine,
...
#define	MAX_ARGS	32
	...
	while ((varType[numVect] = (DVType) va_arg(ap, DVType))
			!= DRL_NULL_T) {
		lptr[numVect] = (void**) va_arg(ap, void*);
		numVect++;
		if (numVect >= MAX_ARGS) {
			DrlErrMsg("%s: too many arguments (max %d).\n",
				routine, MAX_ARGS);
			goto done;
		}
	}

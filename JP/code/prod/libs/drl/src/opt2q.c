...
static	double Normal_InvH (double prob)
{
    ...
    if (prob < 0e0 ||
	prob > 1e0)
	DrlErrMsg("Normal_InvH failed.  Input probability (%lf) is invalid.\n",
		  prob);
...
static	char	routine[] ="DrlOptBS2Q";
    ...
    {
        DrlErrMsg("Could not find calib const for Call_BS2Q !");
        C = P = -1.0e10;
	return (FAILURE);
    }
...
static	char	routine[] = "DrlATMTo2QVol";
	    ...
	    fwd < 0e0)
	{
	    ...
	    {
		DrlErrMsg("%s: in the case of fwd<=0 (at texp =%lf), both "
	    ...
	    if(toupper(vType) != 'N')
	    {
		DrlErrMsg("%s: in the case of fwd<=0 (at texp =%lf), only "
	...
	{
		DrlErrMsg("%s: invalid vol type (vType=%c). "
		  "only lognormal (%c) or normal vol (%c) allowed.\n",
		  routine,
		  vType,
		  'L',
		  'N');
	...
	/* "y" is the bp vol 				       */
		...
		if (prob < 0e0 ||
		    prob > 1e0)
		{
		    DrlErrMsg("%s: can not solve for 2q vol.  The "
			      "cumulative probability (%lf) is invalid.\n",
			      routine, prob);

	    	    DrlErrMsg("QLeft = QRight = %lf, exp = %lf, fwd=%lf, "
			      "atmPr=%lf.\n", 
			      QLeft, T, fwd, atmPr);

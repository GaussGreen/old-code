...
typedef enum
{
    IRX_BOOTSTRAP_LINEAR_ZEROES,              /* Linear in zero rate space */
...
typedef struct _IrxTZeroCurve
{
    ...
    IrxTDate        baseDate;
    /** Base rate of the curve. In the curve construction from zero rates, the
        discount factors are calculated, with a df = 1.0 at the baseDate, hence
        we lose what the actual zero rate was, so we need to store this zero rate
        at base date separately for the linear zeroes interpolation method */
    double          baseRate;
...
***************************************************************************
*/
const char* irxBootstrapMethodToString (IrxTBootstrapMethod value);
...
***************************************************************************
*/
const char* irxRateTypeTypeToString (IrxTRateTypeType value);
...
***************************************************************************
*/
const char* irxStubPaymentToString (IrxTStubPayment value);
...
/**
***************************************************************************
** Initializer for IrxTZeroCurve (to avoid potentially "destroying" garbage
***************************************************************************
*/
void irxZeroCurveInit(IrxTZeroCurve *p);
/**
...
int irxTypeAndFreqToRateType(
/** Type of the rate - e.g. simple, continously compounded, compounded N periods
    per year etc. */
IrxTRateTypeType type,
/** Frequency - only used for rates with N periods per year. This is N. */
int              frequency,
/** Empty shell for the result */

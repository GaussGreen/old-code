...
public:
	...
	EslEx(void) {}
	EslEx(const char * aMsg) : msg(aMsg) {}
	EslEx(const std::string& aMsg) : msg(aMsg) {}
	EslEx(const std::string& aMsg, const std::exception& e) { 
		this->msg = aMsg + "\n" + e.what();
	}
...
#if 0
/*
   REPLACED WITH A GLOBAL FUNCTION (see below) TO AVOID UNDEFINED ORDER OF

...
IrxTZeroCurve* irxZeroCurveCopy(IrxTZeroCurve const* src)
{
    ...
    dst->interpMethod = src->interpMethod;
    return dst;
}
...
)
{
    ...
    status = irxTypeAndFreqToRateType(type, frequency, p); /* either SUCCESS or FAILURE */
...
{
    rateType->type      = type;
    rateType->frequency = frequency;

    return irxRateTypeValidate(rateType); /* either SUCCESS or FAILURE */
}

/*****************************************************************************
** Utility functions for DLLs that use a counted-array interface.
**
** IMPORTANT: This file contains conditionally compiled code depending upon
**            whether one is using IRX dates and curves (i.e.
**            -DESL_NEW_DATE -DESL_NEW_CURVE).
**
**            With the exception of common include files and some common
**            definitions, the code is organized so that the conditionally
**            compiled code is placed near the top of the file, and common
**            code (without any conditional compilation) is placed near the
**            end.  PLEASE respect this convention when adding/modifying
**            this file.
**
*****************************************************************************/
#include "dll_util.h"
#include "esl_alloc.h"
#include "esl_date.h"
#include "esl_zeros.h"

#ifdef ESL_NEW_CURVE
#include "irx/irxflow.h"
#include "irx/zerocurve.h"
#endif

/* Task  : Packs up a single T_CURVE structure.  
 */  
int PackSingleTCurve( T_CURVE*      crv,  
                      IRDate         Today,
                      IRDate         ValueDate,   
                      char const*   SwapFreq, 
                      char const*   DCC, 
                      char const*   MMB, 
                      long          SpotDays,
                      IRDate const*  ZeroDatesL,
                      double const* ZeroRatesL )
{  
#ifdef ESL_NEW_CURVE
    IRDate dates[MAXNBDATE];
#else
    IRDate LastDate;
#endif

   ASSERT_OR_FAIL ((int)ZeroDatesL[0] == (int)ZeroRatesL[0]);
   ASSERT_OR_FAIL ((int)ZeroDatesL[0] >= 1);

   crv->Today       = DRDate(Today);  
   crv->ValueDate   = DRDate(ValueDate);  
   
   crv->SwapFreq   = toupper(SwapFreq[0]) ; 

   if ( DCC[0] == '0' ) 
   { 
      strcpy( crv->SwapDCC, "360" ) ; 
   } 
   else if ( DCC[0] == '3' ) 
   { 
      strcpy( crv->SwapDCC, "ACT" ) ; 
   } 
   else if ( DCC[0] == '5' ) 
   { 
      strcpy( crv->SwapDCC, "365" ) ; 
   } 
   else 
   { 
      DR_Error( "Value Of Zero DCC not valid" ) ; 
      return( FAILURE ) ; 
   }

   if ( MMB[0] == '0' ) 
   { 
      crv->MMB = 360 ; 
   } 
   else if ( MMB[0] == '5' ) 
   { 
      crv->MMB = 365 ; 
   } 
   else 
   { 
      DR_Error( "Value Of Zero MMB not valid" ) ; 
      return( FAILURE ) ; 
   } 
 
   crv->SpotDays = SpotDays; 

#ifdef ESL_NEW_CURVE
   COPY_FROM_DATES_L (ZeroDatesL, MAXNBDATE, dates);

   return irxZeroCurveConstructFromRates(
        crv,
        crv->ValueDate,
        (int)ZeroDatesL[0],
        dates,
        &ZeroRatesL[1],
        IRX_ANNUAL_RATE,
        IRX_ACT_365F);
#else
   crv->NbZero = (int)ZeroDatesL[0] ;
   COPY_FROM_DATES_L (ZeroDatesL, MAXNBDATE, crv->ZeroDate);
   COPY_FROM_ARRAY_L (ZeroRatesL, MAXNBDATE, crv->Zero);

   /* Extend curve to 100 years if necessary */
   LastDate = Nxtmth (crv->ValueDate, 1200L, 1L);
   if (ZeroDatesL[ZeroDatesL[0]] < LastDate)
   {
       (crv->NbZero)++;
       crv->ZeroDate[crv->NbZero-1] = LastDate;
       crv->Zero[crv->NbZero-1]     = crv->Zero[crv->NbZero-2];
   }
   crv->InterpType = EslGetZeroInterpolation();

   /* printf("Pack\n"); EslPrintZeroCurve(crv, stdout); */

   return SUCCESS;
#endif
}  


/* Task  : Unpacks up a single T_CURVE structure.  
 */  
int UnPackSingleTCurve(T_CURVE const* crv,  
                      IRDate*          ValueDate,   
                      char*           SwapFreq, 
                      char*           DCC, 
                      char*           MMB, 
                      long*           SpotDays,
                      IRDate*          ZeroDatesL,
                      double*         ZeroRatesL )
{  
#ifdef ESL_NEW_CURVE
    int i, j;
#endif

   *ValueDate = YMDDateFromIRDate(crv->ValueDate);  
   *SwapFreq  = crv->SwapFreq;

/* printf("UnPack\n"); EslPrintZeroCurve(crv, stdout); */

   if (toupper(crv->SwapDCC[0]) == 'A')
   {
        strcpy(DCC, "3");
   }
   else if (!strcmp (crv->SwapDCC, "360"))
   {
        strcpy(DCC, "0");
   }
   else if (!strcmp (crv->SwapDCC, "365"))
   {
        strcpy(DCC, "5");
   }
   else 
   { 
        DR_Error( "Value Of Zero DCC not valid" ) ; 
        return FAILURE; 
   }

   if ( crv->MMB == 360 )
   { 
        strcpy(MMB, "0");
   } 
   else if ( crv->MMB == 365 ) 
   { 
        strcpy(MMB, "5");
   } 
   else 
   { 
      DR_Error( "Value Of Zero MMB not valid" ) ; 
      return FAILURE; 
   } 
 
   *SpotDays = crv->SpotDays; 
       
#ifdef ESL_NEW_CURVE 
   /* one of the start dates is a base date */
   ZeroDatesL[0] = (YMDDate)crv->numItems - 1;
   ZeroRatesL[0] = (double)crv->numItems - 1;
   
   j = 1;
   for (i=0; i<crv->numItems; ++i)
   {
       if (crv->startDates[i] == crv->baseDate)
           continue;

       ZeroDatesL[j] = YMDDateFromIRDate(crv->startDates[i]);
       if (irxZeroRate(crv,
                       crv->startDates[i],
                       IRX_ACT_365F,
                       IRX_ANNUAL_RATE,
                       &ZeroRatesL[j]) != SUCCESS)
           return FAILURE;

       ++j;
   }
#else
   EslSetZeroInterpolation((ESL_INTERP_TYPE)crv->InterpType);
   ZeroDatesL[0] = (YMDDate)crv->NbZero;
   ZeroRatesL[0] = (double)crv->NbZero;
   
   COPY_TO_DATES_L (ZeroDatesL, crv->NbZero, crv->ZeroDate);
   COPY_TO_ARRAY_L (ZeroRatesL, crv->NbZero, crv->Zero);
#endif

   return SUCCESS;  
}  


/*****************************************************************************
** IMPORTANT:  All code below this point should work whether or not we are
**             using IRX dates and curves without requiring any conditional
**             compilation.
*****************************************************************************/

/* By default, date conversion function (see DRDate, below) is IRDateFromYMDDate */
/* NOTE: That IRDateFromYMDDate simply returns the input date unless using IRX (ALib) dates with -DESL_NEW_DATE */
/*       Unfortunately, this makes the assumption that all dates may be implicitly converted */
/*       to and from long (which is currently a valid assumption).                           */
#ifdef __cplusplus
extern "C" {
#endif
static long (*DateConvFcn) (long) = IRDateFromYMDDate;
int SetDateConv (long (*f) (long))
{
    if (f == NULL) return (FAILURE);
    DateConvFcn = f;
    return (SUCCESS);
}
#ifdef __cplusplus
}
#endif

/* Note that this function is used by COPY_FROM_DATES_L (see dll_util.h) */
long DRDate (long inputDate)
{
    return (DateConvFcn (inputDate));
}

/* Create counted array of strings for regression test */
char **Alloc_Strng_L (long NbStrings,    /* size of string array      */
                      long MaxStrLen)    /* length of each string     */
{
    int  i;
    char **StrngL;

    StrngL = (char **) DR_Array (CHAR_PTR, 0, NbStrings);
    if (StrngL == NULL) 
    {
        DR_Error ("Failed to allocate CHAR_PTR array");
        return (NULL);
    }

    /* do not allocate 0-th pointer */
    for (i=1;i<=NbStrings;i++)
    {
        StrngL[i] = (char *) DR_Array (CHAR, 0, MaxStrLen);
        if (StrngL[i] == NULL)
        {
            DR_Error ("Failed to allocate CHAR array");
            return (NULL);
        }
    }

    /* instead, use the address to store number of strings */
    StrngL[0] = (char *) NbStrings;

    return (StrngL);
}

/* Free counted array of strings; make sure not to free 0th string! */
int Free_Strng_L (char **StrngL,         /* string array to be freed */        
                  long NbStrings,        /* size of string array     */
                  long MaxStrLen)        /* length of each string    */
{
    int i;

    /* do not free 0th pointer, which is not allocated! */
    for (i=1; i <= NbStrings; i++)
    {
        if (StrngL[i] != NULL)
        {
            Free_DR_Array (StrngL[i], CHAR, 0, MaxStrLen);
        }
    }

    if (StrngL != NULL)
    {
        Free_DR_Array (StrngL, CHAR_PTR, 0, NbStrings);
    }

    return (SUCCESS);
}


/* Task : Packs up all three zero curves.  
*/  
int PackTCurves(  T_CURVE    *Curves,  
                  IRDate      *BaseDatesL,  
                  char       **ZeroFrequenciesL,
                  char       **ZeroDCCsL,
                  char       **ZeroMMBsL,
                  long       *ZeroSpotDaysL,
                  IRDate      *Zero0DatesL,
                  double     *Zero0RatesL,
                  IRDate      *Zero1DatesL,
                  double     *Zero1RatesL,
                  IRDate      *Zero2DatesL,
                  double     *Zero2RatesL )
{ 
   ASSERT_OR_FAIL ((int)BaseDatesL[0] == (L_BASEDATE_SIZE-1));

   if (PackSingleTCurve(&Curves[0],  
                         BaseDatesL[L_Today],  
                         BaseDatesL[L_ValueDate0], 
                         ZeroFrequenciesL[1], 
                         ZeroDCCsL[1],
                         ZeroMMBsL[1],       
                         ZeroSpotDaysL[1],  
                         Zero0DatesL,
                         Zero0RatesL) != SUCCESS ||
       PackSingleTCurve(&Curves[1],    
                         BaseDatesL[L_Today],    
                         BaseDatesL[L_ValueDate1],   
                         ZeroFrequenciesL[2], 
                         ZeroDCCsL[2],
                         ZeroMMBsL[2],         
                         ZeroSpotDaysL[2],    
                         Zero1DatesL,
                         Zero1RatesL) != SUCCESS ||    
       PackSingleTCurve(&Curves[2],    
                         BaseDatesL[L_Today],    
                         BaseDatesL[L_ValueDate2],   
                         ZeroFrequenciesL[3],   
                         ZeroDCCsL[3],
                         ZeroMMBsL[3],
                         ZeroSpotDaysL[3],    
                         Zero2DatesL,
                         Zero2RatesL) != SUCCESS)    
      return FAILURE;    

   return SUCCESS;
}  


/* Task : Unpacks up all three zero curves.  
*/  
int UnPackTCurves(  
                  T_CURVE const* Curves,  
                  IRDate      *BaseDatesL,  
                  char       **ZeroFrequenciesL,
                  char       **ZeroDCCsL,
                  char       **ZeroMMBsL,
                  long       *ZeroSpotDaysL,
                  IRDate      *Zero0DatesL,
                  double     *Zero0RatesL,
                  IRDate      *Zero1DatesL,
                  double     *Zero1RatesL,
                  IRDate      *Zero2DatesL,
                  double     *Zero2RatesL )
{ 
   BaseDatesL[0] = (int)(L_BASEDATE_SIZE - 1);
   BaseDatesL[L_Today] = YMDDateFromIRDate(Curves[0].Today);

   if ( UnPackSingleTCurve( &Curves[0],  
                            &(BaseDatesL[L_ValueDate0]),
                            ZeroFrequenciesL[1],
                            ZeroDCCsL[1],
                            ZeroMMBsL[1],   
                            &(ZeroSpotDaysL[1]),
                            Zero0DatesL,
                            Zero0RatesL  
                         )  == FAILURE )  
   {  
      return( FAILURE ) ;  
   }  
   if ( UnPackSingleTCurve( &Curves[1],    
                            &(BaseDatesL[L_ValueDate1]),
                            ZeroFrequenciesL[2],
                            ZeroDCCsL[2],
                            ZeroMMBsL[2], 
                            &(ZeroSpotDaysL[2]),
                            Zero1DatesL,
                            Zero1RatesL 
                         )  == FAILURE )    
   {    
      return( FAILURE ) ;    
   }    
   if ( UnPackSingleTCurve( &Curves[2],    
                            &(BaseDatesL[L_ValueDate2]),
                            ZeroFrequenciesL[3],
                            ZeroDCCsL[3],
                            ZeroMMBsL[3],   
                            &(ZeroSpotDaysL[3]),
                            Zero2DatesL,
                            Zero2RatesL    
                         )  == FAILURE )    
   {    
      return( FAILURE ) ;    
   }    

   return( SUCCESS ) ;
}  

/************************************************************************************
**
** IMPORTANT:  READ COMMENTS AT TOP OF FILE BEFORE ADDING/MODIFYING CODE TO THIS FILE.
**
************************************************************************************/

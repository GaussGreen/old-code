...
**
** Based on code by Doug Gallager, Greg Garber, Tom Robbins-Milne, Peter
** Taylor, Simon Meldrum etc.
***************************************************************************
*/
...
#define CROSSOVER_YEAR  60          /* If double digit year less than */
                                    /* this, assume year */
...
/* Make these long because they're compared against CXDates, which
 ...
 *
 * Basic idea:
 *    The time consuming part of date conversion always arises from the
 *    variable number of days in a month.  Having leap years makes it worse.
 *    It is faster if we store the starting dates for months.  We can
 *    make finding cached items very fast.
 *    Keep an array of <CXDate, m/y> pairs, one for each month for many years.
 *    To convert from CXDate d to m/d/y:
 *         find element e in cache such that e.date <= d
 *         return e.ymd, after replacing  e.day with d - e.date + 1
 *    To convert from m/d/y m to CXDate d:
 *         find element e in cache such that e.month=m.month && e.year==m.year
 *         return e.date + m.day - 1
 *
 *  DATE_CACHE  is initialized to contain the TDate,Month,Year for the
 *   first day of each month from 1/1/1995 until 12/1/2119
 */

typedef struct                         /* One entry for each month in cache */
{
    IrxTDate        date;              /* Date of first day in cached month  */
    short int       month;             /* month= 1..12 */
    short int       year;              /* E.g. 1996  (NOT 96) */
} IrxTDateCacheEntry;

static IrxTDateCacheEntry DATE_CACHE[] = {
...
#define DATE_CACHE_SIZE  (sizeof(DATE_CACHE)/sizeof(IrxTDateCacheEntry))

#define MIXED_CASE 0
#define UPPER_CASE 1
#define LOWER_CASE 2

static void convertCase (char *str, long textCase);
...
IrxTDate irxToday(void)
{
   ...
   time(&timer);                /* time() and localtime() both use TZ var, */
   pSTM = localtime(&timer);    /* so they must cancel each other, that is */
                                /* we have good struct tm without TZ */

   month = pSTM->tm_mon + 1;     /* tm_mon is zero relative */
   day   = pSTM->tm_mday;
   year  = pSTM->tm_year + 1900; /* tm_year is 1900 relative */
...
{
    static char routine[]="irxDate";

    IrxTYearMonthDay ymd;
    IrxTDate         date; /* returned */

    ymd.month = month;
    ymd.day   = day;
    ymd.year  = year;

    if (irxYMDToDate (&ymd, &date) != SUCCESS)
    {
...
/*
***************************************************************************
** Converts IrxTDate to Month, Day, Year.
**
...
*/
int irxDateToYMD
(IrxTDate          date,               /* (I) Days since 1/1/BASE_YEAR. */
 IrxTYearMonthDay *ymd)                /* (O) Month/Day/Year format */
{
    static char routine[] = "irxDateToYMD";
    int         status    = FAILURE;
    ...
    if (DATE_CACHE[0].date <= date &&
        date <= DATE_CACHE[DATE_CACHE_SIZE-1].date)
    {
        int i = (int) (date - DATE_CACHE[0].date);
        /* i = 12*(i/365) + (i%365)/29; */ /* make guess at proper index */
        i /= 29;                /* make guess at proper index */
        if (i>(int)DATE_CACHE_SIZE-1)
        {
            i = DATE_CACHE_SIZE-1;
        }
        while (DATE_CACHE[i].date > date) /* can guess too high */
        {
            i--;
        }

        ymd->year  = DATE_CACHE[i].year;
        ymd->month = DATE_CACHE[i].month;
        ymd->day   = date - DATE_CACHE[i].date + 1;
        return SUCCESS;
    }
...
/*
***************************************************************************
** Converts  Month, Day, Year to IrxTDate.
**
** NOTE: routine was written to avoid divides (especially) and multiplies.
***************************************************************************
*/
int irxYMDToDate
(IrxTYearMonthDay *ymd,                /* (I) Month/Day/Year format */
 IrxTDate         *odate)              /* (O) Days since 1/1/BASE_YEAR. */
{
    static char routine[] = "irxYMDToDate";
    int         status    = FAILURE;

    IrxTDate   date = 0;             /* Assign to odate at end */
    long     fourYearBlocks;
    int      year =  (int) ymd->year;
    int      month = (int) ymd->month;
    int      day =   (int) ymd->day;
    IrxTBool   isLeap;

    /* Check if date is covered by cache */
    if (DATE_CACHE[0].year <= ymd->year &&
        ymd->year <= DATE_CACHE[DATE_CACHE_SIZE-1].year)
    {
        int i = 12*(year - DATE_CACHE[0].year) + month-1;  /* index */
        
        if (day < 1  ||  31 < day || month < 1  ||  12 < month)
        {
            irxError("%s: Invalid date: %d/%d/%d\n",
                    routine,month,day,year);
            return(FAILURE);
        }
        
        *odate = DATE_CACHE[i].date + day - 1;
        if (i < (int)DATE_CACHE_SIZE-1 &&
            *odate > DATE_CACHE[i+1].date)
    ...
    {
        irxError("%s: date %ld/%ld/%ld  not valid.\n",
                routine, ymd->month, ymd->day, ymd->year);
        goto RETURN;
    }
    ...
    if (month < 1 || month > 12 ||
        ymd->year < BASE_YEAR)
    {
        irxError("%s: date %ld/%ld/%ld is not valid.\n",
                routine, ymd->month, ymd->day, ymd->year);
        goto RETURN;
    }
...
***************************************************************************
*/
int irxNormalizeYMD(IrxTYearMonthDay *ymd)  /* (I/O) */
{
    static char routine[]="irxNormalizeYMD";

    int    month = (int) ymd->month;            /* store in local vars for */
    int    year =  (int) ymd->year;             /* ... speed */
    int    day =   (int) ymd->day;
...
**
** The truly remarkable and bizarre behaviour of the SRM3 Nxtmth function
** is not supported.
***************************************************************************
*/
...
{
    static char routine[] = "irxDateAddMonths";
    int         status    = FAILURE;

    IrxTDate output;
    IrxTYearMonthDay ymd;
    IrxTBool       lastDay = FALSE;

    if (months == 0) return date;

    if (irxDateToYMD(date, &ymd) != SUCCESS) goto RETURN;
    if (eom)
    {
        lastDay = ymd.day == irxDaysInMonth (ymd.year, ymd.month);
    }
    
    ymd.month += months;
    if (irxNormalizeYMD (&ymd) != SUCCESS) goto RETURN; /* failure */
    
    if (irxYMDToDate (&ymd, &output) != SUCCESS) goto RETURN; /* failure */
    if (eom && lastDay)
    {
        output = irxDateToEndOfMonth(output);
    }
    status = SUCCESS;
...
)
{
    ...
    if (irxDateToYMD (date, &ymd) != SUCCESS) return -1;
    day = irxDaysInMonth (ymd.year, ymd.month);
    return irxDate(ymd.year, ymd.month, day);
}
...
)
{
    ...
    if (irxDateToYMD (date, &ymd) != SUCCESS) return -1;
    return irxDate(ymd.year, ymd.month, 1);
}
...
{
    static char routine[] = "irxDateNthWeekDay";
    int         status =    FAILURE;
    IrxTDate date = FAILURE;
    long dow;
    long day;

    REQUIRE(weekday >= IRX_SUNDAY && weekday <= IRX_SATURDAY);

    /*
    ** Intercept some inputs to ensure that we do not end up with unpleasant
    ** messages from irxYMDToDate.
    */
    if (whichOne < 1 || whichOne > 5 ||
        month < 1 || month > 12 ||
        year < 1601)
    {
        goto RETURN; /* failure - the final message will capture this */
    }

    day = (whichOne-1)*7 + 1;
    date = irxDate(year, month, day);
    if (IRX_INVALID_DATE(date)) goto RETURN;
    dow  = IRX_DAY_OF_WEEK(date);
     ...
     * Check 28 first so we don't usually have to make function call.
     */
    if (day > 28 && day > irxDaysInMonth(year, month))
    {
        goto RETURN;                      /* Failed */
    }
  ...
  RETURN:
    ...
    if (status != SUCCESS)
    {
        ...
        irxError("%s: %ld%s %s of %ld/%ld does not exist.\n",
                routine, whichOne, str, irxFormatDayOfWeek(weekday),
                month, year);
        return -1;
    }
...
const char* irxFormatDayOfWeek(long dayOfWeek) /* (I) */
{
    ...
    default: 
        irxError("irxFormatDayOfWeek: Unknown day of week %ld.\n", dayOfWeek);
        return("Unknown");
    }
    /*NOTREACHED*/
}
...
{
    static char routine[] = "irxDateFormat";

    char          add[32]; /* new string to be concatenated */
    char         *myFormat = NULL;
    char         *myFormatAlloc = NULL;
    char         *tmp = NULL; /* Temporary string */
    char         *buf = NULL; /* Worker string */
    char          c; /* current character in format string */
    char         *str = NULL; /* To be returned */
    long          textCase = MIXED_CASE;

#define MAX_STR_LEN 32
#define MAX_AT_ONCE 8                  /* Must be a power of 2 */
    static int ibuf;
    char          dateBuf[MAX_AT_ONCE][MAX_STR_LEN];  /* date string buffer */
    ...
    if (irxDateToYMD (date, &ymd) != SUCCESS) goto RETURN;
    dow = irxDayOfWeek (date);

    myFormatAlloc = irxStringToUpper (format);
    if (myFormatAlloc == NULL) goto RETURN; /* failure */
    myFormat = myFormatAlloc;
    textCase = MIXED_CASE;
    ...
    while ((c = *myFormat) != '\0')
    {
        ...
        if (isalpha((int)c))
        {
            ...
            case 'D':
                ...
                {
                case 1:
                    sprintf (add, "%ld", ymd.day);
                    break;
                case 2:
                    sprintf (add, "%02ld", ymd.day);
                    break;
                case 3:
                case 4:
                    strcpy (add, irxFormatDayOfWeek(dow));
                    if (count == 3) add[3] = '\0';
                    convertCase (add, textCase);
                    break;
                default:
                    irxError ("%s: Cannot have more than 4 adjacent "
                               "letter D in date format string %s\n",
                               routine, format);
                    goto RETURN; /* failure */
                }
                break;
            case 'M':
                ...
                {
                case 1:
                    sprintf (add, "%ld", ymd.month);
                    break;
                case 2:
                    sprintf (add, "%02ld", ymd.month);
                    break;
                case 3:
                case 4:
                    switch (ymd.month)
                ...
                default:
                    irxError ("%s: Cannot have more than 4 adjacent "
                             "letter M in date format string %s\n",
                             routine, format);
                    goto RETURN; /* failure */
                }
                break;
            case 'Q':
                ...
                {
                case 1:
                    sprintf (add, "%ld", (long)((ymd.month+2)/3));
                    break;
                case 2:
                    sprintf (add, "q%ld", (long)((ymd.month+2)/3));
                    convertCase (add, textCase);
                    break;
                default:
                    irxError ("%s: Cannot have more than 2 adjacent "
                               "letter Q in date format string %s\n",
                               routine, format);
                    goto RETURN; /* failure */
                }
                break;
            case 'Y':
                ...
                {
                case 2:
                    sprintf (add, "%02ld", ymd.year % 100);
                    break;
                case 4:
                    sprintf (add, "%04ld", ymd.year);
                    break;
                default:
                    irxError ("%s: Must have either 2 or 4 adjacent letter "
                             "Y in date format string %s\n", routine,
                             format);
                    goto RETURN; /* failure */
                }
				break;
            ...
            case 'X':
                ...
                continue; /* no text to add */
            default:
                irxError ("%s: Invalid letter (%c) in date format string %s. "
                           "Must be D,M,Q,Y,U,L or X\n",
                           routine, c, format);
                goto RETURN; /* failure */
            }
            tmp = buf;
            buf = irxStringConcatenate (2, buf, add);
            if (buf == NULL) goto RETURN; /* failure */
            FREE(tmp);
        }
        else if (isdigit((int)c))
        {
            irxError ("%s: Cannot have numbers in date format string %s\n",
                     routine, format);
            goto RETURN; /* failure */
        }
        else if (isprint((int)c))
        {
            ...
            punct[1] = '\0';
            tmp = buf;
            buf = irxStringConcatenate (2, buf, punct);
            FREE (tmp);
            if (buf == NULL)
                goto RETURN; /* failure */
        }
        ...
        {
            irxError ("%s: Cannot have unprintable characters in date format"
                     "string %s\n", routine, format);
            goto RETURN; /* failure */
        }
    }
...
/*--------------------------------------------------------------------
 * Helper function to convert 3-character representation
 * of month to a long integer.
 */
static int
NStringToMonth(
    char *cp,
    long *monthN)
{
    cp[0] = (char)tolower(cp[0]);
    cp[1] = (char)tolower(cp[1]);
    cp[2] = (char)tolower(cp[2]);
    if (strcmp(cp,"jan") == 0)
    {
        *monthN = 1;
    }
...
***************************************************************************
*/
int irxStringToDate(const char *mmddyyyy, IrxTDate *date)
{
    static char routine[] = "irxStringToDate";
    int status = FAILURE;
     ...
     */
    (void)strncpy(dateCopy, mmddyyyy, sizeof(dateCopy)-1); 
    dateCopy[sizeof(dateCopy)-1] = '\0';
 
    /* Only support the following formats:
     * YYYYMMDD, MM/DD/YYYY, MM-DD-YYYY, Mth-DD-YYYY, DD-Mth-YYYY
     */
    if( strpbrk(dateCopy, "-/. ") == NULL ) /* London long YYYYMMDD format */
    {
        char *ep;
        long yyyymmdd = strtol(dateCopy, &ep, 10);
        if (*ep != '\0')
        {
            irxError ("%s: Date string \"%s\" is not one of the allowed formats:\n"
                      "YYYYMMDD, MM/DD/YYYY, MM-DD-YYYY, Mth-DD-YYYY, DD-Mth-YYYY.\n",
                      routine, mmddyyyy);
            goto RETURN;
        }
    ...
    {
        /* This block parses the following delimited formats:
         * MM/DD/YYYY, MM-DD-YYYY, Mth-DD-YYYY, DD-Mth-YYYY   
         */
        ...
        /* If not integer try string */
        if (sscanf(cp,"%ld", &mm) != 1)  
        {
            if (NStringToMonth(cp, &mm) == FAILURE)
                goto RETURN;
        ...
        if (cp == NULL)
            goto RETURN;
        if (sscanf(cp,"%ld", &dd) != 1)   /* If not integer try string */
        {
            dd = mm;              /* Month was really day */
            if (isMonth == TRUE || NStringToMonth(cp, &mm) == FAILURE)
            {
                goto RETURN;
            }
        }
        ...
        if (mm > 12 || mm < 1)
        {
            irxError("%s: Month (%ld) in date %s is out of range",
                     routine, mm, mmddyyyy);
            goto RETURN;
        }
        ...
        yyyy = atol(cp);
        if (yyyy > 99 && yyyy < BASE_YEAR)
        {
            irxError("%s: Year %ld out of range.\n", routine, yyyy);
            goto RETURN; 
        }
 
        if (yyyy < CROSSOVER_YEAR)      /* Assume its past year 2000 */
        yyyy += 2000;           /* To get years since 1900 */
        else if (yyyy < BASE_YEAR)
        yyyy += 1900;        /* Else assume we got only 2 digits */
 
    }

    /* Convert to date
     */
    *date  = irxDate (yyyy, mm, dd);
    if (*date == FAILURE) goto RETURN; /* failure */
...
/*
***************************************************************************
** Converts case in the requisite manner.
***************************************************************************
*/
static void convertCase (char *str, long textCase)
{

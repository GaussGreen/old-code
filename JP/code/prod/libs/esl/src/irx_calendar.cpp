 ...
 */
    ...
    */
    ...
    if (cal->dateList != NULL && cal->dateList->fNumItems > 0)
    {
...
(const char   *name,    /* (I) Name to associate calendars with */
...
)
{
    ...
    static char NO_WEEKENDS[] = "NO_WEEKENDS";
    static char NONE[]        = "NONE";

    if (irxStrcmpi(fileName, NO_WEEKENDS) == 0)
    {
        cal = irxCalendarMake(dl, IRX_WEEKEND_NO_WEEKENDS);
        goto RETURN; /* Success */
    }

    if (irxStrcmpi(fileName, NONE) == 0)
    {
        cal = irxCalendarMake(dl, IRX_WEEKEND_STANDARD);
        goto RETURN; /* Success */
    }
...
)
{
    ...
    long                 numHols = cal->dateList == NULL ? 0 : cal->dateList->fNumItems;
    IrxTDate              *holArray = NULL;

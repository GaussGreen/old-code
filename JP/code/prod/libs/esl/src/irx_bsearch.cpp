...
**
** Binary search functions on sorted lists.
**
** We cannot just use bsearch in stdlib.h because we want to return the
** bounds.
**
** $Header$
***************************************************************************
...
(long    xDesired,  /* (I) Value for which we seek */
 ...
 long   *hiBound)   /* (O) Index of first value strictly greater than xDesired,
                       -1 if no such value exists. */
{
    static char routine[] = "irxBinarySearchLong";
    int         status    = FAILURE;
...
#define ELEMENT(idx) *(long*)(xp + skip*(idx))
    ...
    if (count == 0)
    {
        irxError("%s: x array not in increasing order.n", routine);
        return FAILURE;
    }
...
/*f
***************************************************************************
** Searches for a double value within an array.
**
** For details see comments for irxBinarySearchDouble, replacing long with
** double throughout.
***************************************************************************
*/
int irxBinarySearchDouble
(double  xDesired,  /* (I) Value for which we seek */
 double *xArray,    /* (I) Array in which we seek - assumes sorted in
                       ascending order.*/
 size_t  skip,      /* (I) Size of elements in the array */
 long    arraySize, /* (I) Size of the array */
 long   *exact,     /* (O) Index of a value which is equal to xDesired,
 ...
 long   *hiBound)   /* (O) Index of first value strictly greater than xDesired,
                       -1 if no such value exists. */
{
  /*static char routine[] = "irxBinarySearchDouble"; */

#undef ELEMENT
#define ELEMENT(idx) *(double*)(xp + skip*(idx))

    irxErrorFailure("irxBinarySearchDouble not implemented - use STL instead");
    return FAILURE;
}

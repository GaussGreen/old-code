...
static int irxDayCountFractionActActAFB(IrxTDate date1, IrxTDate date2, double *time);
 ...
 double           *time)
{
    ...
    case IRX_ACT_ACT_AFB:
        return irxDayCountFractionActActAFB(date1, date2, time);
    case IRX_ACT_365F:
    ...
    case IRX_B30_360_FIXED:
    {
        double yearFrac = (date2 - date1) / 365.25;
        *time =  floor(yearFrac * 12 + 0.5) / 12;
        return SUCCESS;
    }
    }
 ...
 double  *time)
{
    ...
    else
    {
        ...
        days = date2 - irxDateToStartOfYear(date2);
...
/*
***************************************************************************
** Computes the day count fraction between date1 and date2 according to the
** Actual/Actual (AFB) [a.k.a. Actual/Actual (Euro)] convention.
**
** The rule is that we move date2 back by a number of years until it is
** within one year of date1. Then, we compute the fractional period which
** remains based on the rule that if the period contains Feb 29, then the
** denominator is 366; otherwise, the denominator is 365.
** i.e. if (date1 <= Feb29 && fracPeriodEndDate > Feb29)
**          denom = 366;
**      else
**          denom = 365;
***************************************************************************
*/
static int irxDayCountFractionActActAFB
(IrxTDate   date1,                    /* (I) start date */
 IrxTDate   date2,                    /* (I) end date   */ 
...
)
{
    ...
    IrxTYearMonthDay  ymd1;
    IrxTYearMonthDay  ymd2;
    IrxTYearMonthDay  ymdFracPeriodEnd;
    IrxTDate          fracPeriodEndDate;
    double            sign;
    double            denom;
    int               numYears;
    int               isDate2EndOfFeb;

    /* Ensure that dates are specified in ascending order.  Otherwise, swap them and
       negate the sign of the result. */
    if (date1 <= date2)
        sign = 1.0;
    else
    {
        IrxTDate tmpDate = date1;
        date1 = date2;
        date2 = tmpDate;
        sign = -1.0;
    }

    if (irxDateToYMD(date1, &ymd1) != SUCCESS)
        goto RETURN;
    if (irxDateToYMD(date2, &ymd2) != SUCCESS)
        goto RETURN;


    /* Compute the end date of the fractional period.  We use an equivalent (but more
       efficient) algorithm that jumps directly to the fractional end date (instead of
       repeatedly subtracting one year from each period end date), but we take care
       to handle an fractional period from Feb 29 to Feb 28 according to the rules. */ 
    ymdFracPeriodEnd.year = ymd1.year;
    ymdFracPeriodEnd.month = ymd2.month; 
    if (ymd2.month < ymd1.month || (ymd2.month == ymd1.month && ymd2.day < ymd1.day))
        ++ymdFracPeriodEnd.year;

    /* Determine whether date2 is the last day of February, taking into account leap years. */
    isDate2EndOfFeb = (ymd2.month == 2 && ymd2.day == irxDaysInMonth(ymd2.year, ymd2.month));

    if (!isDate2EndOfFeb)
        ymdFracPeriodEnd.day = ymd2.day;
    else
        /* Set day to last day of Feb in the fractional period end date's year, taking into
           account leap years.  This handles all of [Feb29, Feb28), [Feb28, Feb29), and
           [Feb28, Feb28). */ 
        ymdFracPeriodEnd.day = irxDaysInMonth(ymdFracPeriodEnd.year, ymd2.month);

    if (irxYMDToDate(&ymdFracPeriodEnd, &fracPeriodEndDate) != SUCCESS)
        goto RETURN;


    /* Next, see if the period between [date1, fracPeriodEndDate) contains February 29th
       (to compute the appropriate denomiator for the fractional period). */
    if (ymdFracPeriodEnd.month > 2 &&
        ((ymd1.month <= 2 && IRX_IS_LEAP(ymd1.year)) ||
         (ymdFracPeriodEnd.year > ymd1.year && IRX_IS_LEAP(ymdFracPeriodEnd.year))))
            denom = 366.0;  /* It does. */
    else
            denom = 365.0;  /* Is does NOT. */


    /* Finally, compute the year fraction from the number of whole years plus any fractional
       period.  Note that fracPeriodEndDate - date1 gives the actual number of days in
       the fractional period, which is always what we want for the numerator. */
    numYears = ymd2.year - ymdFracPeriodEnd.year;
    *result = sign * (numYears + (fracPeriodEndDate - date1) / denom);

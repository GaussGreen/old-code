    ...
    )
{
    ...
    if (GtoDiscountDate(
        ...
        &temp) != SUCCESS)
    {
        DR_Error("%s: failed to compute discount factor.", routine);
        goto RETURN;
    }
    ...
    KProtLeg_D *protLeg)
{
    ...
    {
        FREE(protLeg->mDates);
        FREE(protLeg->mNotionals);
        FREE(protLeg->mRecoveries);
...
RETURN:
    ...
    FREE(notionals);
    FREE(coupons);
    FREE(dates);            // NB also frees memory for useDates
    ...
    KFeeLeg_D *feeLeg)
{
    if(feeLeg != NULL)
    {
        FREE(feeLeg->mAccStDates);
        FREE(feeLeg->mAccEndDates);
        FREE(feeLeg->mPayDates);
        FREE(feeLeg->mNotionals);
        FREE(feeLeg->mCoupons);
    }

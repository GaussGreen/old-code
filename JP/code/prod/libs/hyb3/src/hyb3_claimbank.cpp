...
void    Hyb3_CbkInit(CLAIM_BANK *CBK)      /* (I/O) the target claim bank   */
{
    Esl_CbkInit(CBK);
}
             ...
             int         dimension)    /* (I) Dim of desired slice      */
{
    ...
    if ((CBK->Slices   != NULL) ||
        ...
        (CBK->auxSlice != NULL))
    {
        DR_Error("Hyb3_CbkAlloc: claim bank is not empty!\n");
    ...
    CBK->auxSlice = Hyb3_Alloc_Slice(tree_data, dimension);
    ...
    {
        (CBK->Slices)[i] = Hyb3_Alloc_Slice(tree_data, dimension);
...
FREE_MEM_AND_RETURN:
    ...
    if (status != SUCCESS) 
    {
        DR_Error("Hyb3_CbkAlloc: Unable to allocate memory!\n");
        Hyb3_CbkFree(CBK, tree_data, dimension);
    }
    return (status);
             ...
             int         dimension)     /* (I) Dim of desired slice      */
{
    ...
    Hyb3_Free_Slice(CBK->auxSlice,tree_data, dimension);

    if (CBK->Slices != NULL)
    {
        for (i=0;i<CBK->TotNbSlices;i++)
        {
            Hyb3_Free_Slice(CBK->Slices[i],tree_data, dimension);
        }
...
RETURN:
    ...
    if (status != SUCCESS) DR_Error("Hyb3_CbkFree: Failed!\n");
    return (status);
             ...
             int     NbErDates)         /* (I) size of the usage list     */
{
    ...
    for (i=0; i<NbEvDates; i++)
    {
        ...
        {
            sprintf (ErrorMsg, 
                     "Hyb3_CbkSizeFromDL: Earliest usage date #%d (%ld) > "
                     "evaluation date (%ld)!\n",
             ...
             int         ErDateIdx)           /* suppdate idx       */
{
    ...
    MaxSize = Hyb3_CbkSizeFromDL(EvDL,
                            NbEvDates,
                            ErDL,
                            NbErDates);
...
/*****  Hyb3_CbkDev  *****************************************************/
/*
 *      Hyb3_Dev the active elements of the bank
 *      Returns SUCCESS or FAILURE
 */
...
RETURN:
    ...
    if (status == FAILURE) DR_Error("Hyb3_CbkDev: Failed!\n");
    return (status);
 ...
 *      The bank is locked for all activities except for Hyb3_CbkPushSlice.
 *      Caller is expected to free the slice unless he/she/it pushes it
 *      back to the bank via Hyb3_CbkPushSlice
 *      Returns NULL if failed, or pointer to free slice
 */

TSLICE  Hyb3_CbkPopSlice(CLAIM_BANK *CBK)
{
...
RETURN:
    ...
    if (status == FAILURE) DR_Error("Hyb3_CbkPushSlice: Failed!\n"); 
    return (status);
...
TSLICE   Hyb3_CbkReadSlice(CLAIM_BANK *CBK,
                      long        EvDate)
{
    int ofs;

    ofs = Hyb3_CbkGetOffset(CBK, EvDate, CbkEXACT);
...
int     Hyb3_CbkGetOffset(CLAIM_BANK *CBK,
                     long        EvDate,
                     int         mode)
{
    int  offset = -999;

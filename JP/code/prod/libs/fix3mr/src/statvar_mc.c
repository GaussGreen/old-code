          ...
          int     NbPaths)               /* (I) Nb of paths                  */
{
    ...
    {
        DR_Error("Initialize_IR_SIM: NbPaths (%d) > %d the maximum of "
...
{
static  char    routine[] = "SwapYield_MC";
    ...
    {
        DR_Error ("%s: Swap state date %8d < current simulation date %8d",
                  routine,
    ...
    for (p=0; p<ir_sim->NbPaths; p++)
    {
        ir_sim->X[p] = Corr * ir_sim->X[p]
                     + sqrt(1. - Corr * Corr) * gasdev(&(ir_sim->Seed));
...
static  char    routine[] = "IndexCovar";
    int     status = FAILURE;  /* Error status = FAILURE initially      */
    ...
    LCoef ( L1,
            ...
            CalibFlag,
            NbVol,
            (CalibFlag == TRUE ? VolBaseDate : ZeroBaseDate),
            VolDate,
            Aweight,
            NbFactor,
            Beta); 
          ...
          double  *Beta)                 /* (I) Mean reversions              */
{
    ...
    if (CalibFlag == FALSE)
    {
        ...
        L[0][0] = Aweight[0][0] * Aweight[0][0] * ExpDecay(2. * Beta[0], t) * t;
    
        if (NbFactor > 1) 
        {
            L[1][1]  = Aweight[1][0] * Aweight[1][0] * ExpDecay (2. * Beta[1], t) * t;
            L[1][1] += Aweight[2][0] * Aweight[2][0] * ExpDecay (2. * Beta[1], t) * t;
            L[0][1]  = Aweight[0][0] * Aweight[1][0] * ExpDecay ((Beta[0]+Beta[1]), t) * t;
            L[1][0]  = Aweight[1][0] * Aweight[0][0] * ExpDecay ((Beta[0]+Beta[1]), t) * t;

        }
        
        if (NbFactor > 2)
        {
            L[2][2]  = Aweight[3][0] * Aweight[3][0] * ExpDecay (2. * Beta[2], t) * t;
            L[2][2] += Aweight[4][0] * Aweight[4][0] * ExpDecay (2. * Beta[2], t) * t;
            L[2][2] += Aweight[5][0] * Aweight[5][0] * ExpDecay (2. * Beta[2], t) * t;
            L[0][2]  = Aweight[0][0] * Aweight[3][0] * ExpDecay ((Beta[0]+Beta[2]), t) * t;
            L[2][0]  = Aweight[3][0] * Aweight[0][0] * ExpDecay ((Beta[2]+Beta[0]), t) * t;
            L[1][2]  = Aweight[1][0] * Aweight[3][0] * ExpDecay ((Beta[1]+Beta[2]), t) * t;
            L[1][2] += Aweight[2][0] * Aweight[4][0] * ExpDecay ((Beta[1]+Beta[2]), t) * t;
            L[2][1]  = Aweight[3][0] * Aweight[1][0] * ExpDecay ((Beta[2]+Beta[1]), t) * t;
            L[2][1] += Aweight[4][0] * Aweight[2][0] * ExpDecay ((Beta[2]+Beta[1]), t) * t;
    ...
    for (i = 0; i < NbVol; i++)
    {
        ...
        if (NbFactor > 1)
        {
            ...
            L[0][1] += Aweight[0][i] * Aweight[1][i] * ExpDecay ((Beta[0]+Beta[1]), t) * t;
            L[1][0] *= exp (-(Beta[1] + Beta[0]) * t);
            L[1][0] += Aweight[1][i] * Aweight[0][i] * ExpDecay ((Beta[1]+Beta[0]), t) * t;
        }
        ...
        if (NbFactor > 2)
        {
            ...
            L[0][2] += Aweight[0][i] * Aweight[3][i] * ExpDecay ((Beta[0]+Beta[2]), t) * t;
            L[2][0] *= exp (-(Beta[2] + Beta[0]) * t);
            L[2][0] += Aweight[3][i] * Aweight[0][i] * ExpDecay ((Beta[2]+Beta[0]), t) * t;
            L[1][2] *= exp (-(Beta[1] + Beta[2]) * t);
            L[1][2] += Aweight[1][i] * Aweight[3][i] * ExpDecay ((Beta[1]+Beta[2]), t) * t;
            L[1][2] += Aweight[2][i] * Aweight[4][i] * ExpDecay ((Beta[1]+Beta[2]), t) * t;
            L[2][1] *= exp (-(Beta[2] + Beta[1]) * t);
            L[2][1] += Aweight[3][i] * Aweight[1][i] * ExpDecay ((Beta[2]+Beta[1]), t) * t;
            L[2][1] += Aweight[4][i] * Aweight[2][i] * ExpDecay ((Beta[2]+Beta[1]), t) * t;
        }

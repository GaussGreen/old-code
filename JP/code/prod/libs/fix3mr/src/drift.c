...
/*
$Header: /nasdev/export2/home/drdev/cvsadmin/cvs/libs/fix3/src/drift.c,v 1.13 2001/08/10 09:17:01 lcooper Exp $
*/
...
//    double  Beta         = mktvol_data->Beta[0];
    ...
    double  *StPrice = NULL;
    double  *StPrice1 = NULL;
    double  *StPriceL = NULL;
    double  *StPrice1L = NULL;
    double  eX, varX;
    double  sumStPrice;
    double  Var1, Var2, intLength;
    int     bmkIdx;

    StPrice  = Alloc_Slice (tree_data);
    StPrice1 = Alloc_Slice (tree_data);
    if (   (StPrice  == NULL)
        || (StPrice1 == NULL))
    {
    ...
    Discount = Alloc_Slice (tree_data);
    ...
    StatePrL = StatePr + Node_Offset(1, 0, 0, 0, tree_data);
    StatePrL[0] = 1.;

    StPriceL = StPrice + Node_Offset(1, 0, 0, 0, tree_data);
    StPriceL[0] = 1.;
    ...
    {   
        ...
        DiscountL = Discount + Node_Offset(1, 0, 0, t,   tree_data);
        
        StPrice1L = StPrice1 + Node_Offset(1, 0, 0, t+1, tree_data);
        StPriceL = StPrice + Node_Offset(1, 0, 0, t, tree_data);
        ...
        */
        ...
        /* Find change in tree centre at current time step*/
        {
            ...
            {
                DR_Error ("Drift_1D: no solution exists to drift equation!");
                goto FREE_MEM_AND_RETURN;
            }
            ...
            {
                DR_Error ("Drift_1D: change in drift is too large");
                goto FREE_MEM_AND_RETURN;
            }
        }
        ...
        */
        ...
        if (lMin > lMax)
        {
            DR_Error ("Drift_1D: problem in building the tree (lMin > lMax)!");
            goto FREE_MEM_AND_RETURN;
        }
        ...
        {                        
            ...
            StPrice1L[i+1+l] += pu * StPriceL[i];
            StPrice1L[i  +l] += p0 * StPriceL[i];
            StPrice1L[i-1+l] += pd * StPriceL[i];
        }
        ...
        /* for (i = Bottom[t+1]; i <= Top[t+1]; i++)
        {                        
            eX += i*Jump*StPrice1L[i];
            sumStPrice += StPrice1L[i];
        }

        for (i = Bottom[t+1]; i <= Top[t+1]; i++)
        {                        
            varX += (i*Jump - eX)*(i*Jump - eX)*StPrice1L[i];
        }

        Var1 *= exp (- 2. * tree_data->BetaTD[0][t] * Length[t]);
        Var1 += Aweight[0][t] * Aweight[0][t] * Length[t] * ExpDecay (2. * tree_data->BetaTD[0][t], Length[t]);

        while ((bmkIdx < mktvol_data->NbBmkMr)&&(mktvol_data->BmkDate[bmkIdx] <= tree_data->TPDate[t]))
            bmkIdx ++;

        if (bmkIdx == mktvol_data->NbBmkMr)
            bmkIdx--;

        if ((mktvol_data->BmkDate[bmkIdx] > tree_data->TPDate[t])&&(mktvol_data->BmkDate[bmkIdx] < tree_data->TPDate[t+1]))
        {
            intLength = Daysact(tree_data->TPDate[t], mktvol_data->BmkDate[bmkIdx])/365.; 

            Var2 *= exp (- 2. * mktvol_data->BetaBmk[0][bmkIdx] * intLength);
            Var2 += mktvol_data->Aweight[0][bmkIdx] * mktvol_data->Aweight[0][bmkIdx] * intLength * ExpDecay (2. * mktvol_data->BetaBmk[0][bmkIdx], intLength);

            if (bmkIdx < mktvol_data->NbBmkMr - 1)
                bmkIdx++;

            intLength = Daysact(mktvol_data->BmkDate[bmkIdx], tree_data->TPDate[t+1]); 
        ...
        /* printf("\nt = %d, sumProb = %20.15lf, Var = %20.15lf, VarTree = %20.15lf, err = %20.15lf", t,
               sumStPrice, Var1, varX, varX - Var1);

        if (fabs (varX - Var1) > 0.0001)
        {
            printf(" !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!");
            fflush(stdout);
            fflush(stdout);
    ...
    Free_Slice (StPrice,  tree_data);
    Free_Slice (StPrice1,  tree_data);

    Free_Slice (StatePr,  tree_data);
    Free_Slice (Discount, tree_data);
...
*/
int     Drift_2D (  MKTVOL_DATA     *mktvol_data,   /* (I) Volatility data */
                    TREE_DATA       *tree_data)     /* (I/O) Tree data     */
{
...
//    double  *Beta        = mktvol_data->Beta;
    ...
    if (   (StatePr  == NULL)
        ...
        || (StatePr1 == NULL))
    {
        DR_Error ("Drift_2D: could not allocate memory!");
        goto FREE_MEM_AND_RETURN;        
    }
    ...
    StatePrL = StatePr + Node_Offset(2, 0, 0, 0, tree_data);
    StatePrL[0] = 1.;
    ...
    {                                   
        ...
        for (i = Bottom1[t]; i <= Top1[t]; i++)
        {
            ...
            DiscountL = Discount + Node_Offset (2, i, 0, t, tree_data);
        ...
        /* Find change in tree centre at current time step*/
        {
            ...
            if (Quadratic_Solve(Root, P) == FAILURE)
            {
                DR_Error ("Drift_2D: no solution exists to drift equation!");
                goto FREE_MEM_AND_RETURN;
            }
            ...
            if (fabs(DelZt * VolBbq) > 1.)
            {
                DR_Error ("Drift_2D: change in drift is too large");
                goto FREE_MEM_AND_RETURN;
            }
        }
        ...
        if ((EDevOn) && (EDevIdx<tree_data->NbEDevDates))
        {
            if (tree_data->TPDate[t+1] == tree_data->EDevDate[EDevIdx])
            {
                ...
                for (i = Bottom1[t+1]; i <= Top1[t+1]; i++)
                {
                    offset =  Node_Offset(2, i, 0, t+1, tree_data);
...
*/
int     Drift_3D (  MKTVOL_DATA     *mktvol_data,   /* (I) Volatility data */
                    TREE_DATA       *tree_data)     /* (I/O) Tree data     */
{
    ...
    if (   (StatePr  == NULL)
        ...
        || (StatePr1 == NULL))
    {
        DR_Error ("Drift_3D: could not allocate memory!");
        goto FREE_MEM_AND_RETURN;        
    }
    ...
    StatePrL = StatePr + Node_Offset(3, 0, 0, 0, tree_data);
    StatePrL[0] = 1.;
    ...
    {                                   
        ...
        Beta11 = Beta[0] * Length[t] * PreviousJump[0] / Jump[0];
        Beta21 = Beta[1] * Length[t] * PreviousJump[1] / Jump[2];
        Beta22 = Beta[1] * Length[t] * PreviousJump[2] / Jump[2];
        Beta31 = Beta[2] * Length[t] * PreviousJump[3] / Jump[5];
        Beta32 = Beta[2] * Length[t] * PreviousJump[4] / Jump[5];
        Beta33 = Beta[2] * Length[t] * PreviousJump[5] / Jump[5];
        ...
        for (i = Bottom1[t]; i <= Top1[t]; i++)
        {
            for (j = Bottom2[t][i]; j <= Top2[t][i]; j++)
            {
                ...
                DiscountL = Discount + Node_Offset (3, i, j, t, tree_data);
        ...
        for (i = Bottom1[t]; i <= Top1[t]; i++)
        {
            for (j = Bottom2[t][i]; j <= Top2[t][i]; j++)
            {
                StatePrL  = StatePr  + Node_Offset (3, i, j, t, tree_data);
                DiscountL = Discount + Node_Offset (3, i, j, t, tree_data);
        ...
        /* Find change in tree centre at current time step*/
        {
            ...
            if (Quadratic_Solve(Root, P) == FAILURE)
            {
                DR_Error ("Drift_3D: no solution exists to drift equation!");
                goto FREE_MEM_AND_RETURN;
            }
            ...
            if (fabs(DelZt * VolBbq) > 1.)
            {
                DR_Error ("Drift_3D: change in drift is too large");
                goto FREE_MEM_AND_RETURN;
            }
        }
        ...
        for (i = Bottom1[t+1]; i <= Top1[t+1]; i++)
        {
            for (j = Bottom2[t+1][i]; j <= Top2[t+1][i]; j++)
            {                                   
                StatePr11L = StatePr1 + Node_Offset (3, i, j, t+1, tree_data);
        ...
        if (lMin > lMax)
        {
            DR_Error ("Drift_3D: problem in building the tree (lMin > lMax)!");
            goto FREE_MEM_AND_RETURN;
        }
        ...
        {                        
            ...
            if (mMin > mMax)
            {
                DR_Error ("Drift_3D: problem in building the tree (mMin > mMax)!");
                goto FREE_MEM_AND_RETURN;
            }
            ...
            for (j = Bottom2[t][i]; j <= Top2[t][i]; j++)
            {
                ...
                if (nMin > nMax)
                {
                    DR_Error ("Drift_3D: problem in building the tree (nMin > nMax)!");
                    goto FREE_MEM_AND_RETURN;
                }
                ...
                StatePr11L = StatePr1 + Node_Offset (3, i+1+l, j+1+m, t+1, tree_data);
                StatePr12L = StatePr1 + Node_Offset (3, i+1+l, j  +m, t+1, tree_data);
                StatePr13L = StatePr1 + Node_Offset (3, i+1+l, j-1+m, t+1, tree_data);
                StatePr21L = StatePr1 + Node_Offset (3, i  +l, j+1+m, t+1, tree_data);
                StatePr22L = StatePr1 + Node_Offset (3, i  +l, j  +m, t+1, tree_data);
                StatePr23L = StatePr1 + Node_Offset (3, i  +l, j-1+m, t+1, tree_data);
                StatePr31L = StatePr1 + Node_Offset (3, i-1+l, j+1+m, t+1, tree_data);
                StatePr32L = StatePr1 + Node_Offset (3, i-1+l, j  +m, t+1, tree_data);
                StatePr33L = StatePr1 + Node_Offset (3, i-1+l, j-1+m, t+1, tree_data);
        ...
        if (fabs (p - ZeroCoupon[t+1]) > 0.0001)
        {
            DR_Error ("Drift_3D: state prices don't add up to 1: "
                        "increase sigma or Ppy!");
            goto FREE_MEM_AND_RETURN;
        }
        ...
        if ((EDevOn) && (EDevIdx<tree_data->NbEDevDates))
        {
            if (tree_data->TPDate[t+1] == tree_data->EDevDate[EDevIdx])
            {
                ...
                for (i = Bottom1[t+1]; i <= Top1[t+1]; i++)
                {
                    for (j = Bottom2[t+1][i]; j <= Top2[t+1][i]; j++)
                    {   
                        offset =  Node_Offset (3, i, j, t+1, tree_data);                               

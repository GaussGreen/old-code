        ...
        if (tree_data->NbFactor == 1)
        {
            ...
            {   
                ...
                for (s = 0; s < NbStates; s++)
                {
					...
					/* Work backwards to the payoff */
					OptionPayoff[s] = StateVar_Interp(i, NbStates, StatesTPlus1, MultiSerialL, D, LiborPairValue);
                } /* for state idx */
            ...
            for (i = Bottom1; i <= Top1; i ++)
            {
                ...
                for (j = Bottom2[i]; j <= Top2[i]; j++)
                {
					...
					for (s = 0; s < NbStates; s++)
					{
						...
						/* Work backwards to the payoff */
						OptionPayoff[s] = StateVar_Interp(j, NbStates, StatesTPlus1, MultiSerialL, D, LiborPairValue);
					} /* for state idx */
                ...
                for (j = Bottom2[i]; j <= Top2[i]; j++)
                {
                    ...
                    for (k = Bottom3[i][j]; k <= Top3[i][j]; k++)
                    {
						...
						for (s = 0; s < NbStates; s++)
						{
							...
							/* Work backwards to the payoff */
							OptionPayoff[s] = StateVar_Interp(k, NbStates, StatesTPlus1, MultiSerialL, D, LiborPairValue);
						} /* for state idx */
	...
	else if (iResetNumber == 0)
	{
        ...
        if (tree_data->NbFactor == 1)
        {
            ...
            {   
				...
				MultiSerialL[0][i] = StateVar_Interp(i, NbStates, StatesTPlus1, MultiSerialL, D, LiborPairFirstPart);
            } /* for i */
        ...
        else if (tree_data->NbFactor == 2)
        {
            ...
            for (i = Bottom1; i <= Top1; i ++)
            {
                ...
                for (j = Bottom2[i]; j <= Top2[i]; j++)
                {
					...
					/* of the whole option.  We stick this into the slice for   */
					/* the first state variable.                                */
					MultiSerialL[0][j] = StateVar_Interp(j, NbStates, StatesTPlus1, MultiSerialL, D, LiborPairFirstPart);
                }  /* for j */
            }  /* for i */
        }
        /************************  3 FACTORS   ****************************/
            ...
            for (i = Bottom1; i <= Top1; i ++)
                ...
                for (j = Bottom2[i]; j <= Top2[i]; j++)
                {
                    ...
                    for (k = Bottom3[i][j]; k <= Top3[i][j]; k++)
                    {
						...
						/* of the whole option.  We stick this into the slice for   */
						/* the first state variable.                                */
						MultiSerialL[0][j] = StateVar_Interp(k, NbStates, StatesTPlus1, MultiSerialL, D, LiborPairFirstPart);
                    }  /* for k */
                }  /* for j */
        }  /* if then else */
	}

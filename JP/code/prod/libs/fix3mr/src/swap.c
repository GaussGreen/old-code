...
/*
$Header: /nasdev/export2/home/drdev/cvsadmin/cvs/libs/fix3/src/swap.c,v 1.9 2001/04/09 09:54:37 lcooper Exp $
*/
...
*/
int     ParSwap_t (double      *Swap,       /* (O) Forward swap              */
    ...
    */              
    if (CpAccStDate <= CurrentDate)
    {
        if (DrDayCountFraction (CpAccStDate,
                                ...
                                &Accrued) == FAILURE)
        {
            DR_Error("ParSwap_t: unable to calculate dcc for accrued!");
            goto RETURN;
        }
    ...
    {
        DR_Error("ParSwap_t: version does not allow for fwd starting swap!");
        goto RETURN;
             ...
             TREE_DATA   *tree_data)    /* (I) Tree data structure           */
{
    ...
    {   
        if (DrDayCountFraction (FixCpnAccSt,
                                ...
                                &FixAccrued) == FAILURE)
        {
            DR_Error("AmortSwap_t: unable to calculate dcf for fix accrued!");
            goto RETURN;
        }        
    }  /* if */
    ...
    if (FltCpnAccSt <= CurrentDate)
    {
        if (DrDayCountFraction (FltCpnAccSt,
                                ...
                                &FltAccrued) == FAILURE)
        {
            DR_Error("AmortSwap_t: unable to calculate dcf for flt accrued!");
            goto RETURN;
        }        
    }  /* if */
    ...
    if ((FltCpnAccSt < CurrentDate) && (tree_data->NbFactor == 1))
    {
        ...
        else
        {
            if (ArreasReset == 'Y')
            {
                ...
                {
                    DR_Error("AmortSwap_t: accrued not supported for comp flt payment!");
                    goto RETURN;
                }        
            }
            else
...
*       Single swaplet.
*/
int   Swaplet_t (double      *Swaplet,    /* (I/O) Swaplet                   */
...
/*****  FwdSwap_t  ***********************************************************/
/*
*       Forward swap price. Floating leg is arbitrary. Same as AmortSwap_t
             ...
             TREE_DATA   *tree_data)    /* (I) Tree data structure           */
{
        ...
        if (CoS == 'S')
        {
            if (ArreasReset == 'Y')
            {
                if (FltResetFlag)
                {
                    if (FltIndex == NULL)
                    {
                        DR_Error("FwdSwap_t: Flt Index is not supplied.");
                        goto RETURN;
                    }
                }
                else if (FltOptStub == 'P')
                {
                    ...
                    {
                        DR_Error("FwdSwap_t: Idx Zero is not supplied.");
                        goto RETURN;
                    }
                    if (FltZeroToPmt  == NULL)
                    {
                        DR_Error("FwdSwap_t: Pmt Zero is not supplied.");
                        goto RETURN;
                    }
                }
    ...
    {   
        if (DrDayCountFraction (FixCpnAccSt,
                                ...
                                &FixAccrued) == FAILURE)
        {
            DR_Error("FwdSwap_t: unable to calculate dcf for fix accrued!");
            goto RETURN;
        }        
    }  /* if */
    ...
    if (FltCpnAccSt < CurrentDate)
    {
        if (DrDayCountFraction (FltCpnAccSt,
                                ...
                                &FltAccrued) == FAILURE)
        {
            DR_Error("FwdSwap_t: unable to calculate dcf for flt accrued!");
            goto RETURN;
        }        
    }  /* if */
    ...
    if ((FltCpnAccSt < CurrentDate) && (tree_data->NbFactor == 1))
    {
        ...
        if (CoS == 'S')
        {
            if (ArreasReset == 'Y')
            {
                ...
                {
                    DR_Error("FwdSwap_t: Bond stub not supported "
                             "for arrears reset!");
                    goto RETURN;
                }                             
        ...
        else /* pmt convention is compounding */
        {
            if (ArreasReset == 'Y')
            {
                ...
                {
                    DR_Error("FwdSwap_t: accrued not supported for comp flt payment!");
                    goto RETURN;
                }        
            }
            else

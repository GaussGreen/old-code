...
*/
int     Annuity_t   (double      *Annuity,      /* (O) Par yield             */
                     int         NbZero,        /* (I) Number of zeros       */
                     double      **Zero,        /* (I) Zero bank             */
                     long        *ZeroMaturity, /* (I) Zero maturities       */
                     long        Reset,         /* (I) Reset flag            */
                     ...
                     TREE_DATA   *tree_data)    /* (I) Tree data structure   */
{
    ...
    /* by the user. The cost of this approach is not justifiable  in */
    /* view of the rarity of such cases.            (LP/LB April'98) */
    #define   DR_ANNUITY_CUTOFF   999

     
    double  *AnnuityL;        /* Local slice pointer                   */
    double  *ZeroL, *ZeroR;

    double  *DayCntFrn=NULL;   /* Accrual periods                       */

    double  Ann;               /* Ann price                             */
    double  ZRate, ZPrice;     /* Price and rate of current zero coupon */
    double  ZRateL, ZRateR;    /* Zero rates (ACT/365 basis)            */
	...
	if (NbReset == 1)
	{
		...
		AccEnd   = Nxtmth (StartDate, IndexMat, 1L);

		for (m=0; m < NbZero; m++)
		{
			if (ZeroMaturity[m] == AccEnd)
				break;
		}

		if ( m >= NbZero)
		{
			DR_Error("Annuity_t : can not find Zeros in the Zero Bank!");
			return(FAILURE);
		}

		if (Copy_Slice(Annuity,
					   Zero[m],
					   t,
					   tree_data) == FAILURE)
		{
		...
		if (DrDayCountFraction(StartDate,     
							   AccEnd,
							   DayCount,
							   &(DCFr)) == FAILURE)
        {
            DR_Error("Annuity_t: could not calculate day count fractions!");
			return(FAILURE);
        }

		if (MultiplyScalar(Annuity,
						   DCFr,
						   t,
						   tree_data) == FAILURE)
		{
        ...
        if (tree_data->NbFactor == 1)
        {
            ...
            for (i = Bottom1; i <= Top1; i ++)
            {
                ...
                for (l = 0; l <= NbReset; l++)
                {
					...
					if (l != 0)
                        Ann += DayCntFrn[l] * ZPrice;

                }  /* for l */

                if (CutOffLevelReached)
                {
                    AnnuityL[i] = DR_ANNUITY_CUTOFF; /* See note above */
                }
                else
                {
					AnnuityL[i] = Ann;

                    if (AnnuityL[i] > DR_ANNUITY_CUTOFF)
                    {
                        AnnuityL[i] = DR_ANNUITY_CUTOFF;
                    }
                }
            }  /* for i */
        }
        else if (tree_data->NbFactor == 2)
        {
            for (i = Bottom1; i <= Top1; i ++)
            {
                ...
                for (j = Bottom2[i]; j <= Top2[i]; j++)
                {
                    ...
                    if (CutOffLevelReached)
                    {
                        AnnuityL[j] = DR_ANNUITY_CUTOFF;
                    }
                    else
                    {
                        AnnuityL[j] /= Ann;

                        if (AnnuityL[j] > DR_ANNUITY_CUTOFF)
                        {
                            AnnuityL[j] = DR_ANNUITY_CUTOFF;
                        }
                    }
            ...
            for (i = Bottom1; i <= Top1; i ++)
            {
                for (j = Bottom2[i]; j <= Top2[i]; j++)
                {
                    ...
                    for (k = Bottom3[i][j]; k <= Top3[i][j]; k++)
                    {
                        ...
                        if (CutOffLevelReached)
                        {
                            AnnuityL[k] = DR_ANNUITY_CUTOFF;
                        }
                        else
                        {
							AnnuityL[k] = Ann;

                            if (AnnuityL[k] > DR_ANNUITY_CUTOFF)
                            {
                                AnnuityL[k] = DR_ANNUITY_CUTOFF;
                            }
                        }

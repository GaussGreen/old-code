    ...
    )
{
    ...
    double step, xval, y, d, z, x, ibpv, zero, yCut, p;
    long   i;
    ...
    for (i = 0; i < numGridPts; i++)
    {
        ...
        if (y > TINY) 
        {
            ...
            d    = pow(x, -freqAnn * matAnn);
            ibpv = yCut / (1.-d);

            /* interpolate between 0 and yCutoff */
            ibpv = p * ibpv + (1 - p) / matAnn;
        } 
        else 
        {
            ibpv = 1. / matAnn;
        ...
        if (y > TINY) 
        {
            yCut = MAX (y, yCutoff);
            z = pow(yCut, powerDel);
            x = 1. + alphaDel * z / freqDel;
            zero = 1. / pow(x, freqDel * matDel);
        ...
        faDens[i] = NormDens(xval) * ibpv * zero;
    }
    ...
    *normC = Q3SimpsIntegral(
    ...
    if (*normC < TINY) 
    {
        Q3ErrMsg("%s: Non-positive total FA mass.\n", routine);
        goto RETURN;
    }
...
{   
    ...
    else 
    {
        ...
        /* relate vol ratio to power ratio as in Tonge-Cooper note */
    ...
    else 
    {
        ...
        fa->alphaDel = zeroRateSwap / pow(swapRate, fa->powerDel) *
...
{   
    ...
    case 9:         /* 3 fact */
        ...
        break;
    default:
        Q3ErrMsg("%s: Number of parameters (%d) must be 1, 2, 5, or 9.\n",
...
{   
    ...
    for (i=0; i< numFact; i++) 
    {
        for (j=0; j < numFact; j++) 
        {
            ...
            betaSum = beta[i] + beta[j];
            betaDur = (betaSum*expiry>TINY)?
                (1-exp(-betaSum*expiry))/(betaSum*expiry): 1.;

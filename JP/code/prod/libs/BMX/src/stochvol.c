    ...
    double            *price)        /* (O) option price & vega    */
{
    ...
    if (fwdRate                   < Q3_MIN_FWD     ||
        ...
        sigATM   * sqrt(expiry)   > Q3_MAX_VOL       ) goto RETURN;
    ...
    SVDATA            *sv)              /* (0) SV measure  */
{
    ...
    /* skew limits */
    if (smile[0] < Q3_MIN_SKEW || smile[0] > Q3_MAX_SKEW) 
    {
        Q3ErrMsg("%s: Stoch Vol skew (%f) must be between %1.2f and %1.2f.\n",
                 routine, smile[0], Q3_MIN_SKEW, Q3_MAX_SKEW);
        goto RETURN;
    }

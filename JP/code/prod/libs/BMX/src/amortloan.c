            ...
            TREE_DATA   *tree_data)   /* (I) Tree data structure           */
{
    ...
    {
        DR_Error("AnnAmortSwap_t: nb of state vars exceeds limit (200)!");
        goto RETURN;
    }
    ...
    if (OutsMax < TINY)
    {
        if (*PrevState != NULL)
        {
            DR_Error("AnnAmortSwap_t: outstanding is zero!");
            status = FAILURE;
        }
        else
            status = SUCCESS;
    ...
    if (CollaretFlag)
    {
        if (OutsMax < OutsMin)
        {
            DR_Error("AnnAmortSwap_t: max outs smaller than min outs!");
            goto RETURN;
        }
        ...
        if (State == NULL)
        {
            DR_Error("AnnAmortSwap_t: could not allocate memory for State!");
            goto RETURN;
        }

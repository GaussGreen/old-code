...
*/
int    Fix3_Lattice_Tmx (
                FIX3_DEV_DATA   *dev_data,     /* (O) Dev data structure     */
                ...
                FIX3_TREE_DATA  *tree_data)    /* (I) Tree data structure    */
{
    ...
    if (t == T)                                                        
    {
        ...
        {
            if (Fix3_Copy_Slice (dev_data->NmrInv[CvDiff],
                                 tree_data->NmrInv[NmrIdx],
                                 t,
                                 tree_data) == FAILURE) goto RETURN;

            if (Fix3_SliceTimesScalar (dev_data->NmrInv[CvIdx1],
                                       dev_data->NmrInv[CvDiff],
                                       ZRatio1,
                                       t,
                                       tree_data) == FAILURE) goto RETURN;

            if (Fix3_SliceTimesScalar (dev_data->NmrInv[CvIdx2],
                                       dev_data->NmrInv[CvDiff],
                                       ZRatio2,
                                       t,
                                       tree_data) == FAILURE) goto RETURN;
    ...
    {
        if (Fix3_Copy_Slice (dev_data->NmrInvLag[CvDiff],
                             dev_data->NmrInv[CvDiff],
                             t+1,
                             tree_data) == FAILURE)
        {
        ...
        if (Fix3_Copy_Slice (dev_data->NmrInvLag[CvIdx1],
        ...
        if (Fix3_Copy_Slice (dev_data->NmrInvLag[CvIdx2],
    ...
    if (CurrentDate == mktvol_data->NmrDate[NmrIdx])
    {
        ...
        if (Fix3_Copy_Slice (tree_data->LastZero,
                             tree_data->NmrInv[NmrIdx],
                             t,
                             tree_data) == FAILURE) goto RETURN;
    ...
    else /* In-between numeraire dates */
    {
        ...
        if (Fix3_Ev (tree_data->LastZero,t,T,dev_data,tree_data) == FAILURE)
        {
            goto RETURN;
        }
...
/*****  Fix3_NmrToCcy *****************************************************/
/*
*       Conversion to numeraire units if necessary
*/
int    Fix3_NmrToCcy (
                FIX3_DEV_DATA   *dev_data,     /* (O) Dev data structure     */
                int             t,             /* (I) Current time point     */
                ...
                FIX3_TREE_DATA  *tree_data)    /* (I) Tree data structure    */
{
    ...
    if (!mktvol_data->IsNmrModel) return (SUCCESS);

    /* Numeraire model: set flag for NmrToCcy conversion: 
     * always on at t = 0, otherwise on if a product critical date */
    dev_data->NmrToCcy = (t==0);

    for (i = 0; i <= 2; i++)
    {
        if (tree_data->TPtype[i+ZbkEVENT][t])
        {
            dev_data->NmrToCcy = TRUE;
            break;
        }
    }

    for (i = 0; i <= MAXPRODEVENT; i++)
    {
        if (tree_data->TPtype[i][t])
        {
            dev_data->NmrToCcy = TRUE;
            break;
        }
                ...
                FIX3_TREE_DATA  *tree_data)    /* (I) Tree data structure    */
{
    ...
    /* No more conversions needed at t = 0 */
    if (t==0)
    {

       ...
       T_CURVE const* crv)
{
    ...
    double  BetaL[3];
	double  WeightL[3];
    long     mrIdx, prevMrIdx, lastMrIdx;
    int     lastMrFlag;
    ...
    {       
        ...
        if (NbFactor > 1) 
        {
            y -= D[1][0] * D[1][0] * L[1][0] * exp (-2. * BetaL[1] * t);
        ...
        if (NbFactor > 2)
        {
            ...
            M += 2. * D[1][0] * D[2][0] * Fix3_ExpDecay (BetaL[1] + BetaL[2], t) * t *
                WeightL[1] * WeightL[2] * aw[1][0] * aw[2][0];
            ...
            M += 2. * D[1][1] * D[2][1] * Fix3_ExpDecay (BetaL[1] + BetaL[2], t) * t * 
                WeightL[1] * WeightL[2] * aw[1][1] * aw[2][1];
          ...
          T_CURVE const* crv)                 /* (I) zero curve                   */
{
    ...
    TtoSt = Daysact (VolBaseDate, SwapSt) / 365.;
    ...
    for (i = 0; i < NbBmkMr; i++)
    {
        ...
        EndDate = (i < NbBmkMr-1) ? MIN(SwapSt, BmkDate[i]) : SwapSt;
          ...
          T_CURVE const* crv)           /* (I) zero curve */
{
    ...
    /* Avoid spot index expiration */
   /* if (SwapSt1 <= BaseDateL) 
    {
        *Vol = 0.00;        
        return(SUCCESS);        
    }*/

    if (MIN(SwapSt1, SwapSt2) != SwapSt1)
    {
        DR_Error( "Need to sepcify increasin swap starts");
        goto RETURN;
    }
    TtoSt = Daysact (VolBaseDate, SwapSt1) / 365.;
    TtoSt2 = Daysact (VolBaseDate, SwapSt2) / 365.;
    ...
    PrevT = 0;
    TSwapSt = Daysact(SwapSt1, SwapSt2) / 365.;
    for (i = 0; i < NbBmkMr; i++)
    {
        /* End of integration bucket: if not last bucket, then stop at end */
        /* of bucket or expiry, whichever is smaller; if last bucket, stop */
        /* at expiry (as spot volatility is constant after last bucket).   */
        EndDate = (i < NbBmkMr-1) ? MIN(SwapSt1, BmkDate[i]) : SwapSt1;
        ...
        L[0][0] *= exp (-2. * BetaBmk[0][i] * t) ;
        L[0][0] += Aweight[0][i] * Aweight[0][i] * Fix3_ExpDecay (2. * BetaBmk[0][i], t) * t
            * exp(-BetaBmk[0][i] * TSwapSt);

        if (NbFactor > 1)
        {
            L[1][1] *= exp (-2. * BetaBmk[1][i] * t) ;
            L[1][1] += Aweight[1][i] * Aweight[1][i] * Fix3_ExpDecay (2. * BetaBmk[1][i], t) * t
                       * exp(-BetaBmk[1][i] * TSwapSt);;
            L[1][1] += Aweight[2][i] * Aweight[2][i] * Fix3_ExpDecay (2. * BetaBmk[1][i], t) * t
                       * exp(-BetaBmk[1][i] * TSwapSt);
        ...
        /* End of integration loop */
        if (EndDate == MIN(SwapSt1, SwapSt2))
        {
            break;
        }
    }  /* for i */

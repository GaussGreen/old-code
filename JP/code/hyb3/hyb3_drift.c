                     ...
                     long        step)       /**<(I) number of steps between smoothing pts */
{
    ...
    double AcurveT[MAXNBPOINTS], AcurveVal[MAXNBPOINTS], AcurveSPL[MAXNBPOINTS];
                                ...
                                FX_DATA         *fx_data)       /* (I) FX Data for flat smile test */
{
    int status = FAILURE;
    int isLN = TRUE, i;
    double *scale1st, *smoothScale;
    double newDrift[MAXNBPOINTS];
    double adjZeroCoupons[MAXNBPOINTS];
    double adjust[MAXNBPOINTS], timeLine[MAXNBPOINTS] ; 

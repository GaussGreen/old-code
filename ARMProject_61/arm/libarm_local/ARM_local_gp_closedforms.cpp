...
extern long ARMLOCAL_Gaussian_SABR_Power_SpreadOption(
	...
	/// used in the MACRO ARM_RESULT
	CCString msg ("");
	ARM_GP_Vector*		Parameters_vec		= NULL;	
	try
	{
		...
		ARM_result&  result)
{
	/// used in the MACRO ARM_RESULT
	CCString msg ("");
	ARM_GP_Vector* Maturity_Vec	= NULL;	
	ARM_GP_Vector* F_Vec	= NULL;	
	ARM_GP_Vector* InitialVol_Vec = NULL;
	ARM_GP_Vector* longtermV_Vec = NULL;
	ARM_GP_Vector* theta_Vec = NULL;
	ARM_GP_Vector* ksi_Vec = NULL;
	ARM_GP_Vector* rho_Vec = NULL;
	ARM_GP_Vector* lambda_Vec = NULL;
	ARM_GP_Vector* muJ_Vec = NULL;
	ARM_GP_Vector* sigmaJ_Vec = NULL;
		...
		ARM_result&  result)
{
	/// used in the MACRO ARM_RESULT
	CCString msg ("");
	ARM_GP_Vector* K_Vec	= NULL;	
	ARM_GP_Vector* ImpVol_Vec	= NULL;	
	ARM_GP_Vector* Weigth_Vec	= NULL;	
	...
	ARM_result&				result)
{
	...
	/// used in the MACRO ARM_RESULT
	CCString msg ("");
	ARM_GP_Vector* optimizedVector = NULL;
	ARM::QGM_ParameterSet* qgmSet= NULL;

	ARM_GP_Vector* tagetVect = new ARM_GP_Vector(C_tagetVect);
	ARM_GP_Vector* weights = new ARM_GP_Vector(C_weights);
    ARM_GP_Vector* precisions = new ARM_GP_Vector(C_presicions);
	ARM_GP_Vector* InitVector = new ARM_GP_Vector(C_InitVector);
	ARM_GP_Vector* LBoundVector = new ARM_GP_Vector(C_LBoundVector);
	ARM_GP_Vector* UBoundVector = new ARM_GP_Vector(C_UBoundVector);
										   ...
										   ARM_result&  result)
{
	/// used in the MACRO ARM_RESULT
	CCString msg ("");
	ARM_GP_Vector*  C_l1a_VVec	= NULL;	
	ARM_GP_Vector*  C_l2a_VVec	= NULL;
	ARM_GP_Vector*  C_l3a_VVec	= NULL;
	ARM_GP_Vector*  C_l4a_VVec	= NULL;
	ARM_GP_Vector*  C_l5a_VVec	= NULL;
	ARM_GP_Vector*  C_l6a_VVec	= NULL;
	ARM_GP_Vector*  C_l1b_VVec	= NULL;
	ARM_GP_Vector*  C_l2b_VVec	= NULL;
	ARM_GP_Vector*  C_l3b_VVec	= NULL;
	ARM_GP_Vector*  C_l4b_VVec	= NULL;
	ARM_GP_Vector*  C_l5b_VVec	= NULL;
	ARM_GP_Vector*  C_l6b_VVec	= NULL;
	ARM_GP_Vector*  C_Dis_VVec	= NULL;	
	try
	{
										...
										ARM_result& result)
{
		/// used in the MACRO ARM_RESULT
	CCString msg ("");
	ARM_GP_Vector* A_Vec	= NULL;	
	ARM_GP_Vector* B_Vec	= NULL;	
	ARM_GP_Vector* C_Vec	= NULL;	
	ARM_GP_Vector* R_Vec	= NULL;	
										...
										ARM_result& result)
{
		/// used in the MACRO ARM_RESULT
	CCString msg ("");
	ARM_GP_Vector*		F1_vec					= NULL;	
	ARM_GP_Vector*		alpha1_vec				= NULL;	
	ARM_GP_Vector*		beta1_vec				= NULL;	
	ARM_GP_Vector*		rho1_vec				= NULL;	
	ARM_GP_Vector*		nu1_vec					= NULL;	
	ARM_GP_Vector*		F2_vec					= NULL;	
	ARM_GP_Vector*		alpha2_vec				= NULL;	
	ARM_GP_Vector*		beta2_vec				= NULL;	
	ARM_GP_Vector*		rho2_vec				= NULL;	
	ARM_GP_Vector*		nu2_vec					= NULL;	
	ARM_GP_Vector*		strike_vec				= NULL;	
	ARM_GP_Vector*		maturity_vec			= NULL;	
	ARM_GP_Vector*		callput_vec				= NULL;	
	ARM_GP_Vector*		price_vec				= NULL;	
	ARM_GP_Vector*		weight_vec				= NULL;	
	ARM_GP_Vector*		initialparams_vec		= NULL;	
		...
		ARM_result& result)
{
	/// used in the MACRO ARM_RESULT
	CCString msg ("");
		ARM_GP_Vector*		Correlations_vec		= NULL;	
		ARM_GP_Vector*		Params_vec		= NULL;	
	...
	long			objId )
{
	...
	try
	{
		ARM_GP_Vector rd(C_ResetDates);
		ARM_GP_Vector fwd(C_Fwds);
		ARM_GP_Vector df(C_Df);
		ARM_GP_Vector levprec(C_LevPrec);
		ARM_GP_Vector lev(C_Lev);
		ARM_GP_Vector fix(C_Fix);
		ARM_GP_Vector cap(C_Cap);
		ARM_GP_Vector floor(C_Floor);
		ARM_GP_Vector fees(C_Fees);
		ARM_GP_Vector dcf(C_dcf);
	...
	long					objId)
{
	...
	{
		ARM_GP_Vector rd(C_resetDates);
		ARM_GP_Vector fwd(C_fwdRates);
		ARM_GP_Vector totalvol(C_totalVol);
		ARM_GP_Vector leftvol(C_leftVol);
		ARM_GP_Vector rightvol(C_rightVol);
		ARM_GP_Vector rho(C_rho);
		ARM_GP_Vector nu(C_nu);
		ARM_GP_Vector rate1Lev(C_rate1Lev);
		ARM_GP_Vector rate1Add(C_rate1Add);
		ARM_GP_Vector caplev(C_capRateLev);
		ARM_GP_Vector capadd(C_capRateAdd);
		ARM_GP_Vector vblev(C_vbLev);
	...
	long					objId)
{
	...
	try
	{
		ARM_GP_Vector rd(C_resetDates);
		ARM_GP_Vector fwd(C_fwdRates);
		ARM_GP_Vector dfs(C_DFs);
		ARM_GP_Vector fwdvol(C_fwdRatesVol);
		ARM_GP_Vector vols(C_vols);
		ARM_GP_Vector vvols(C_volvols);
		ARM_GP_Vector rho(C_rho);
		ARM_GP_Vector fwdpartvol(C_fwdRatesPartVol);
	...
	long					objId)
{
	...
	{
		ARM_GP_Vector rd(C_resetDates);
		ARM_GP_Vector fwd1(C_fwdRates1);
		ARM_GP_Vector fwdvol1(C_fwdRatesVol1);
		ARM_GP_Vector vols1(C_vols1);
		ARM_GP_Vector nu1(C_nu1);
		ARM_GP_Vector rho1(C_rho1);
		ARM_GP_Vector fwdpartvol1(C_fwdRatesPartVol1);
		ARM_GP_Vector fwd2(C_fwdRates2);
		ARM_GP_Vector fwdvol2(C_fwdRatesVol2);
		ARM_GP_Vector vols2(C_vols2);
		ARM_GP_Vector nu2(C_nu2);
		ARM_GP_Vector rho2(C_rho2);
		ARM_GP_Vector fwdpartvol2(C_fwdRatesPartVol2);
		ARM_GP_Vector rcorrel(C_Rate1Rate2Correl);
		ARM_GP_Vector fwdLev(C_fwdLev);
		ARM_GP_Vector strikes(C_fwdStrikes);
		ARM_GP_Vector levier(C_Levier);
		ARM_GP_Vector fix(C_Fixed);
											   ...
											   ARM_result& result)
{
	/// used in the MACRO ARM_RESULT
	CCString msg ("");
	ARM_GP_Vector*		S3params_vec		= NULL;	
	try
	{
											...
											ARM_result& result)
{
	/// used in the MACRO ARM_RESULT
	CCString msg ("");
	ARM_GP_Vector*		S1params_vec		= NULL;	
	ARM_GP_Vector*		S2params_vec		= NULL;	
	ARM_GP_Vector*		S3params_vec		= NULL;	
												...
												ARM_result& result )
{
	...
	try
	{
		ARM_GP_Vector times(C_Times);
		ARM_GP_Vector levels(C_Levels);
		times -= C_AsOfDate;
		times /= 365.;
	...
	ARM_result& result )
{
	...
	try
	{
		ARM_GP_Vector calibtimes(C_CalibTimes);
		calibtimes -= C_AsOfDate;
		calibtimes /= 365.;

		ARM_GP_Vector forwards(C_Forwards);
		ARM_GP_Vector weights(C_Weights);
		ARM_GP_Vector constraintstrikes(C_ConstraintStrikes);
		ARM_GP_Vector constraintvols(C_ConstraintVols);

		vector< ARM_GP_Vector* > mktvols, strikes;

		if(C_MktVols.size() > 0 && C_MktVols[0].size() == 1)
		{
			mktvols.resize(1);
			mktvols[0] = new ARM_GP_Vector;
		...
		if(type == 1)
		{
			...
			int col = 2;
			for(int k = 0; k < sabrparamObj->rhos().size(); k++) 
			{
		...
		else if(type == 2)
		{
			...
			int col = 3;
			for(int k = 0; k < hestonparamObj->rhos().size(); k++) 
			{
	...
	ARM_result& result)
{
	...
	try
	{
		ARM_GP_Vector mktvols1(C_MktVols1);
		ARM_GP_Vector strikes1(C_Strikes1);
		ARM_GP_Vector mktvols2(C_MktVols2);
		ARM_GP_Vector strikes2(C_Strikes2);
		ARM_GP_Vector mktvolspread(C_MktVolsSpread);
		ARM_GP_Vector strikespread(C_StrikesSpread);
	...
	)
{
	...
	try
	{
		ARM_GP_Vector TotemMat(TOTEMMat), TotemStrikes(TOTEMStrikes), TotemPrices(TOTEMPrices);
		ARM_GP_Vector FullSchedReset(FullScheduleReset), FullSchedAnnuity(FullScheduleAnnuity), FullSchedFwd1(FullScheduleFwd1), FullSchedFwd2(FullScheduleFwd2);
		ARM_GP_Vector longFwds(LongFwds), shortFwds(ShortFwds);
		ARM_GP_Vector fwdCalibReset(FwdCalibReset);
		ARM_GP_Matrix * longVols, * longK, * shortVols, * shortK;
		ARM_GP_Vector Mat, Theta, Nu, LongRho, LongShift, LongLevel, ShortRho, ShortShift, ShortLevel, Correl;
											...
											ARM_result& result)
{
	/// used in the MACRO ARM_RESULT
	CCString msg ("");
	ARM_GP_Vector*		strike1_vec			= NULL;	
	ARM_GP_Vector*		vol1_vec			= NULL;	
	ARM_GP_Vector*		strike2_vec			= NULL;	
	ARM_GP_Vector*		vol2_vec			= NULL;	
	ARM_GP_Vector*		S1params_vec		= NULL;	
	ARM_GP_Vector*		S2params_vec		= NULL;	

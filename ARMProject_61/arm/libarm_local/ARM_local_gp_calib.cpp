    ...
    long					objId )
{
	...
	try
	{
		...
		{
			ARM_GP_Vector money(C_Money);
			ARM_GP_Vector vol(C_Vol);
	...
	ARM_result&	result)
{
	...
	try
	{
		...
		ARM_GP_VectorPtr pricePtr = fctor->Quantile(ARM_GP_VectorPtr(new ARM_GP_Vector(1,C_Proba)),C_Forward,C_Maturity);
    ...
    long					objId)
{
	...
	try
	{
		...
		ARM_GP_Vector moneyness(C_moneyness);
		ARM_GP_Vector strikes(C_strikes);

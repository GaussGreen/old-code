	...
	long                    objId )
{
	...
	try
	{ 
        times   = paramTimes.empty() ?  NULL : new ARM_GP_Vector(paramTimes);
		ARM_AutoCleaner<ARM_GP_Vector> HoldTimes(times);
        values  = paramValues.empty() ?  NULL : new ARM_GP_Vector(paramValues);
		ARM_AutoCleaner<ARM_GP_Vector> HoldValues(values);


        lowerBound  = C_LowerBoundary.empty() ?  NULL : new ARM_GP_Vector(C_LowerBoundary);
		ARM_AutoCleaner<ARM_GP_Vector> HoldLowerBound(lowerBound);
        upperBound  = C_UpperBoundary.empty() ?  NULL : new ARM_GP_Vector(C_UpperBoundary);
		ARM_AutoCleaner<ARM_GP_Vector> HoldUpperBound(upperBound);
	...
	long objId )
{
	...
	{ 
		...
		ARM_GP_Vector* datesVector=NULL;
		ARM_GP_Vector* julianDatesVector=NULL;

		if( !GetObjectFromId( &dateStrip, dateStripId, ARM_DATESTRIP ) )
		{
			dateStrip = NULL;
			if (!(datesVector = dynamic_cast<ARM_GP_Vector *>(LOCAL_PERSISTENT_OBJECTS->GetObject(dateStripId))))
			{
		...
		if (datesVector)
		{
			julianDatesVector = new ARM_GP_Vector(datesVector->size());
			for (size_t i = 0; i < datesVector->size(); ++i)
				(*julianDatesVector)[i] = XLDateToJulian((*datesVector)[i]);
		}

		ARM_GP_Vector* lowerBoundVec = lowerBound.empty()? NULL : new ARM_GP_Vector( lowerBound );
		ARM_GP_Vector* upperBoundVec = upperBound.empty()? NULL : new ARM_GP_Vector( upperBound );
	...
	long        objId )
{
	...
	try
	{
		...
		ARM_GP_Vector solverDatas(solverParams);
		ARM_GP_Vector formulaDatas(formulaParams);
		ARM_GP_Vector formulaDatasSO(formulaParamsSO);
	...
	ARM_result&		result)
{
	...
	try
	{
		...
		ARM_VectorPtr times(new ARM_GP_Vector);
		ARM_VectorPtr volatilies(new ARM_GP_Vector);
	...
	long					objId )
{
	...
	{ 		
        ...
        if(!portfolio)
		{
			...
			ARM_GP_Vector evalTimes (evalDates.size());
	...
	long					objId)
{
	...
	{	
		...
		ARM_GP_Vector corrIdxEndTimes(0);
...
long ARM_ParamsMixtureFx_CreateFunctor::operator()( ARM_result& result, long objId )
{
	...
	try
	{
		...
		ARM_GP_Vector* lags = dynamic_cast<ARM_GP_Vector*>(LOCAL_PERSISTENT_OBJECTS->GetObject(genericParams->GetParamValue("Lags").GetObjectId()));
		if (!lags)
		{
		...
		ARM_GP_Vector* volATM = dynamic_cast<ARM_GP_Vector*>(LOCAL_PERSISTENT_OBJECTS->GetObject(genericParams->GetParamValue("VolATM").GetObjectId()));
		if (!volATM)
		{
		...
		ARM_GP_Vector* decVol = dynamic_cast<ARM_GP_Vector*>(LOCAL_PERSISTENT_OBJECTS->GetObject(genericParams->GetParamValue("DecVol").GetObjectId()));
		if (!decVol)
		{
		...
		ARM_GP_Vector* shift = dynamic_cast<ARM_GP_Vector*>(LOCAL_PERSISTENT_OBJECTS->GetObject(genericParams->GetParamValue("Shift").GetObjectId()));
		if (!shift)
		{
		...
		ARM_GP_Vector* lambda = dynamic_cast<ARM_GP_Vector*>(LOCAL_PERSISTENT_OBJECTS->GetObject(genericParams->GetParamValue("Lambda").GetObjectId()));
		if (!shift)
		{
...
long ARM_FXModelDensity_CreateFunctor::operator()( ARM_result& result, long objId )
{
	...
	try
	{
		...
		ARM_GP_Vector* densityOrd = (ARM_GP_Vector*) (eqfxmodel->NormalDistribution(glc,fwd,expiry))->Clone();
	...
	long					objId )
{
	...
	{ 		
		...
		ARM_GP_VectorPtr ResetTimes(ResetDates.empty() ?  NULL : new ARM_GP_Vector(ResetDates));
...
long ARM_2IRFX_ComputeFwdFxVolFunctor::operator()( ARM_result& result, long objId )
{
	...
	try
	{
		...
		ARM_GP_Vector* resetTimes = dynamic_cast<ARM_GP_Vector*>(LOCAL_PERSISTENT_OBJECTS->GetObject(genericParams->GetParamValue("ResetTimes").GetObjectId()));
		if (!resetTimes)
		{
			result.setMsg ("ARM_ERR: lags should be a vector");
			return ARM_KO;
		...
		ARM_GP_Vector* settlTimes = dynamic_cast<ARM_GP_Vector*>(LOCAL_PERSISTENT_OBJECTS->GetObject(genericParams->GetParamValue("SettlTimes").GetObjectId()));
		if (!settlTimes)
		{
			result.setMsg ("ARM_ERR: lags should be a vector");
			return ARM_KO;

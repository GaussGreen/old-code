...

{
	...
	CCString msg ("");
	ARM_GP_Vector* gpVector = NULL;
	try
	{			
		gpVector = new ARM_GP_Vector(values);
		/// assign object
    ...
    ARM_result&				result)
{
	...
	ARM_GP_Vector YVector(Y.size());
	ARM_GP_Vector* CoeffsVector = NULL;
    ...
    ARM_result&				result)
{
	...
	ARM_GP_Vector eigenValues(colsInput);
    ...
    ARM_result& result)
{
	...
	{ 
		...
		ARM_GP_VectorPtr YVector(new ARM_GP_Vector(Y.size()));
		for ( i=0;i<Y.size(); ++i)
			(*YVector)[i] = Y[i];
		...
		for( size_t i=0; i<expiries.size(); ++i )
			for( size_t j=0; j<tenors.size(); ++j )
				correlation(i,j) = mod->UnderlyingCorrelation( "CMS", 
					...
					int( expiries[i]*365 ), 
					int( (expiries[i]+tenor1)*365 ) );  
		surface = new ARM_SurfaceWithInterpol( ARM_GP_Vector(expiries), ARM_GP_Vector(tenors), correlation,type);
	...
	long objId )
{
	...
	ARM_GP_Vector*  initVariable = NULL;
	ARM_AutoCleaner< ARM_GP_Vector > HoldIV(initVariable);

	ARM_GP_Vector*  lowBound = NULL;
	ARM_AutoCleaner< ARM_GP_Vector > HoldLB(lowBound);

	ARM_GP_Vector*  upBound = NULL;
	ARM_AutoCleaner< ARM_GP_Vector > HoldUB(upBound);
	...
	try
	{
		...
		ARM_GP_Vector vdeltaCalls(deltaCalls);
		ARM_GP_Vector vdeltaPuts(deltasPuts);
		...
		//Res = (ARM_GP_Vector*)fxMkt.GetUnKnown().Clone();
		if(OutPutId == string("MKTMODEL"))
			Res = (ARM_BSModel*)fxMkt.GetMktModel()->Clone();
		
		else if(OutPutId == string("GENCURVE")){
			ARM_GP_Vector values(fxMkt.GetUnKnown());
			values/=100.0;
			ARM_GP_Vector deltas(values.size());

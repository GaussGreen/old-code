	...
	ARM_Object* BuildAsset(MSXML2::IXMLDOMNode *node,const ARM_Date& date,string& assetId,int isEtkOrNot = 1)
	{
		...
		try
		{
			...
			if (dates)
			{
				...
				ARM_GP_Vector gpDates(payDates.GetSize(),payDates.GetElt());
				gpDates -= date.GetJulian();
				ARM_GP_Vector gpValues(values->GetSize(),values->GetElt());
				amortizCurve = new ARM_Curve(gpDates,gpValues,new ARM::ARM_StepUpRightOpenCstExtrapolDble);
			}
			else
			{
				ARM_GP_Vector gpDates(values->GetSize(),0.0);
				ARM_GP_Vector gpValues(values->GetSize(),values->GetElt());
				amortizCurve = new ARM_Curve(gpDates,gpValues,new ARM::ARM_StepUpRightOpenCstExtrapolDble);
			}
...
public:
	...
	ARM_Object* BuildTARNAsset(MSXML2::IXMLDOMNode *node,const ARM_Date& date,string& assetId,int isEtkOrNot, bool oswCalibFlag)
	{
		...
		//ARM_GP_Vector nbIterations(2, 10000);
		...
		try
		{
			...
			ARM_GP_Vector cpnMinAbs(1,0.0),cpnMinOrd(1,0.0), cpnMaxAbs(1,0.0), cpnMaxOrd(1,1000.0);
			...
			ARM_GP_Vector nbIterations(3, nbIter);
			...
			ARM_GP_Vector dates(1,start.GetJulian());
			...
			ARM_GP_Vector* pastFixings = GetPastFixings(node, date);
			ARM_GP_Vector* pastStart   = GetPastStart(node, date);
			...
			ARM_GP_Vector mrsValues(1, meanRev);
			ARM_GP_Vector mrsTimes (1, 0.0);
			...
			ARM_GP_Vector* correlTimes = NULL;
			if (isCust)
			...
			*correlTimes -= date.GetJulian(); // times will be deleted in DateStrip destructor
			...
			ARM_GP_Vector* correlValues = new ARM_GP_Vector(n*n);
...
// -----> SBGM case: See SBGM constructor in tarncalculator.cpp
            ...
            ARM_GP_Vector       hValues(1, theHUMP_DEFAULT_VALUE);
	        ARM_GP_Vector       hTimes (1, 0.0);
            ...
            // Set the Beta Correl
            ARM_GP_Vector       bcValues(1, meanRev);
	        ARM_GP_Vector       bcTimes (1, 0.0);

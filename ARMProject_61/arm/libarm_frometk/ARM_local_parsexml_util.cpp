...
ARM_GP_Vector* GetTarnCustomDates(MSXML2::IXMLDOMNode* node)
{
	...
	ARM_GP_Vector*     genDates  = NULL;
	ARM_GP_Vector*     gen       = NULL;
	...
	if (resultList!=NULL)
	{
		...
		CCString sun;
		int nbDates = 0;
		genDates = new ARM_GP_Vector(nbEvents,0.0);
		for (int i = 0; i < nbEvents; i++)
		{
			hr=resultList->get_item(i, &listItem);
		...
		if (nbDates != 0)
		{
			gen = new ARM_GP_Vector(nbDates, 0.0);
			int loop = 0;
			double currVal=0;
...
ARM_GP_Vector* GetPastFixings(MSXML2::IXMLDOMNode* node, ARM_Date asOfDate)
{
	...
	ARM_GP_Vector*     pastFixings = NULL;
	ARM_GP_Vector*	   fixings	   = NULL;
	long               nbEvents;
	double			   currFixing;
	...
	if (resultList!=NULL)
	{
		...
		CCString type;
		int nbFixing = 0;
		pastFixings = new ARM_GP_Vector(nbEvents,0.0);
		for (int i = 0; i < nbEvents; i++)
		{
			hr=resultList->get_item(i, &listItem);
			listItem->selectSingleNode(_bstr_t("Type"), &theNode);
		...
		if (nbFixing != 0)
		{
			fixings = new ARM_GP_Vector(nbFixing, 0.0);
			int loop = 0;
			double currVal=0;
			for (i=0; i<nbEvents; i++)
			{
...
ARM_GP_Vector* GetPastStart(MSXML2::IXMLDOMNode* node, ARM_Date asOfDate)
{
	...
	ARM_GP_Vector*     pastStart = NULL;
	ARM_GP_Vector*	   start     = NULL;
	long               nbEvents;
	...
	if (resultList!=NULL)
	{
		...
		CCString type;
		int nbStart = 0;
		pastStart = new ARM_GP_Vector(nbEvents,0.0);
		for (int i = 0; i < nbEvents; i++)
		{
			hr=resultList->get_item(i, &listItem);
			listItem->selectSingleNode(_bstr_t("Type"), &theNode);
		...
		if (nbStart != 0)
		{
			start = new ARM_GP_Vector(nbStart, 0.0);
			int loop = 0;
			double currVal=0;
			for (i=0; i<nbEvents; i++)
			{

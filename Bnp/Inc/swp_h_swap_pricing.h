...
Err swp_f_ForwardRate_SwapDP(SwapDP* swapdp, String ycName, String refRateCode, double* forward);
...
/* -----------------------------  MARGIN (ON ONE LEG) -------------------------------- */
/* Computes a margin on the leg of a swap to reach a given PV (for use outside SORT) */
Err swp_f_SwapMargin(
    double  pv,
    long    start,
    long    end_nfp,
    String  compStr,
...
Err swp_f_SwapMargin_SwapDP(double pv, SwapDP* swapdp, String ycName, double* margin);

/* -----------------------------  PV (SWAP) -------------------------------- */

/* Computes the PV of a Swap, taking spreads into account (for use outside SORT) */
Err swp_f_SwapPv(
    double  coupon,
    long    start,
    long    end_nfp,
...
Err swp_f_SwapPv_SwapDP(
    double coupon, SwapDP* swapdp, String ycName, String refRateCode, double* pv);

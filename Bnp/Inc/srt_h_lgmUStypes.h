...
/* Exercise information */
	...
	long* iSet;				/* [0,1,...,nEx-1] on exer, settlement is tCpnStart[iSet[j]] */
	double* strike;			/* [0,1,...,nEx-1] fee paid by option holder to exercise */
	SrtReceiverType PayRec;	/* RECEIVER or PAYER */
/*	Coupons: each coupon = marg*cvg + (marg_accrued + gear*LiborCash)*accrued*cvg_accrued */
	...
	double *gear;			/* in general, 0 (fixed coupon) or 1 (floating digital) - [0,1,...,nCpn-1]  margins accrued*/
	double *tCvgCpn;				/* [0,1,...,nCpn-1]  margins accrued  */
/* Barriers details */
	double call_spread_up;
	double call_spread_low;
	double **barriers;   /* [i,0]: lower barrier, [i,1]: upper barrier */
/* Observation details */
	long observation_freq;
	Date ***observationdays;				/* [0,..,nCpn-1]*[0..observation_freq-1][0..1], 0 for the Start, 1 for the End  */
	double **ratiodays_for_subperiod;		/* [0,..,nCpn-1]*[0..observation_freq-1] */
/* Index details */
	long undIndex;
	long undAccrued;
	double CorrelStart;  /* for floating digital, correlation between the cash libor and the first fixing of the period; */
	double CorrelEnd;    /* for floating digital, correlation between the cash libor and the last exercise of the period */
/* Time swap forward pricing */
	double **tForwardTS;		/* [0..1]*[0..nEx-1] for each exercise date. Filled in LGMExtractInfoFromDeal  */
	...
	double dMatStd;						/* 0 if no max, otherwise vol is retrieved for strikes bounded by Fe^{+/-dMaxStd Vol sqrt{t}} */

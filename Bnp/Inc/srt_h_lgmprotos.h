...
/* NOTE: this program is a temporizing step, which does not use the flexibility of the new
program, and it should be replaced with programs which call the new LGMautocal directly from
MAD and Westminster, which can expose the new functionality, ASAP */
...
- the exotic side also includes a cap paying gear * max (0, Libor CASH - cap_str) * libor_cvg */
...
/* Copies the input data into the Simple MidAtlantic Structure pointed to by MidAtPtr */
...
char* LGMFillCallCapFloater(
    ...
    Date*           t,
    double*         amax,
    double*         amin, /* coupon leg */
    double*         marg,
    double          lvg,
    SrtBasisCode    cpnBasis, /* more coupon leg */
...
char* LGMNewAmerican(
    ...
    char* (*RecVol)(Date, Date, double), /* function called whenever LGM uses a vol for pricing */
                                         /* the deal */
...
char* LGMnewautocal(
    ...
    /* calibration and eval parameters */
    LGMCalParm* CalReq,    /* structure defining calibration method to be used */
    ConvParams* EvalParms, /* structure containing numerical evaluation parameters */
...
SrtSimMidAtPtr LGMCreateMidAtFromAmer(
    Date tfirst, double rfltCur, String ycName, LGMMarkConv* conv, int dmos, SrtSimAmer* AmerPtr);
...
char* LGMMakeRefDeals(
    LGMDealType  DealType,
    void*        dealptr,
    Date         tNow,   /* EVALUTAION DATE */
    ...
    char* (*GetVol)(Date, Date, double, SRT_Boolean, double*), /* SWAPTION CAP/VOLS*/
    char* (*GetBeta)(Date, Date, double*), /* EXPONENT BETA TO INTERPRET VOLto interpret vols */
    LGMCalSet** RefDealsPtrPtr);           /* REFERENCE INSTRUMENT (OUPUT) */

/******************************************************************/
/* Main routine to find the market prices of the reference instruments */
char* LGMPriceRefDeals(
    char* (*GetVol)(
        Date, Date, double, SRT_Boolean, double*), /* function to provide swaption/cap vols */
    char* (*GetBeta)(
        Date, Date, double*),            /* function to provide exponent (beta) to interpret vols */
    char* (*RecVol)(Date, Date, double), /* function to provide swaption/cap vols */
...
char* NewMidAtEval(
    ...
    char* (*GetVol)(Date, Date, double, SRT_Boolean, double*), /* swaption/cap vols */
    char* (*GetBeta)(Date, Date, double*),                     /* swaption/cap exponents (beta) */
...
char* LGMValidGenEur(SrtGenEur* ptr, Date tfirst, long* nExleft);
...
char* LGMValidSimMidAt(SrtSimMidAt* ptr, Date tfirst, long* nExleft);
...
char* LGMValidGenMidAt(SrtGenMidAt* ptr, Date tfirst, long* nExleft);
...
char* LGMValidSimAmer(SrtSimAmer* ptr, Date tfirst);
...
char* LGMValidCallInvFlt(SrtCallInvFlt* ptr, Date tfirst, long* ExFirst);
...
/* Check validity of a Bermudan inverse floater */
char* LGMValidCallCapFlt(SrtCallCapFlt* ptr, Date tfirst, long* ExFirst);
...
char* LGMVerifyLGM_TS(LGM_TS* tsPtr);

char* LGMVerifyLGM_TS_For_DiagSwpts(LGMCalSet* CSPtr, LGM_TS* tsPtr);

/* Routine to verify that sigma-kappa term structure is sensible */
char* LGMVerifySigKapTS(SigKapTS* tsPtr);
...
char* LGMCcyDefaults(SrtCurvePtr yldcrv, LGMMarkConv* conventions);
char* LGMMuniDefaults(char* szRefRate, LGMMarkConv* conventions);
...
char* LGMGetRfixFromX(
    Date tEx, Date tEnd, double x, Date tNow, String ycname, LGM_TS* tsPtr, double* Rfix);

char* LGMGetFloatRateFromX(
    Date tStart, Date tPay, double x, String ycName, LGM_TS* tsPtr, double* FloatRate);

/* LGM value of a receiver swaption */
double LGMRecVal(
    double* ystar, long n, double* a, double DStart, double sqzeta, double* Gpay, double Gst);
...
double LGMNormalVolFromCEV(double fwd, double strike, double mat, double CEVvol, double beta);

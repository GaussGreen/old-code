...
typedef struct
{
    ...
    double MMYield[6],           /* Money market rates (O/N,1m,2m,3m,6m,1yr) for tree building */
    ...
    double OAS,               /* Option adjusted spread (cf Lattice() in dev2.c */
        Zero[2 * MAXMAT + 4], /* Output: Zeros at money market points followed by 6m interval points
                               */
        ZeroS[2 * MAXMAT + 4]; /* Output: Zeros at mm points followed by 6m interval points for
                                  YieldS*/
...
double linterpFlat(
    int n, double* xs, double* ys, double x);  // interpolate or do flat extrapolation

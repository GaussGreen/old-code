...
#define         JUMPCOEFF2      2.                                              /* Coefficient used to determine the size of the second jump */
#define         NBEXER          50                                              /* Maximum number of call dates */
#define         NBFWD           50                                              /* Maximum number of forward prices */
#define         NBSWAPTION      5                                               /* Number of benchmark swaptions used to compute spot volatility */
...
typedef struct
{
	...
	int
		MMB,                                                            /* Money market basis (360 or 365) */
		IndexMat,                                                       /* Index maturity in years */
		SwaptionExp[NBSWAPTION],                                        /* Expiration (in years) of the benchmark swaptions */
 		TStree,                                                         /* 0 for treasury tree, 1 for swap tree */
		TSdiscount;							/* 0 for treasury discount, 1 for swap discount */
	char
		AoS,                                                            /* Yield curve type (annual or semi-annual) */
		YearBase[4],                                                    /* Year basis convention for the swap curve */
		IndexFreq;                                                      /* Index frequency */
	double
		MMYield[6],                                                     /* Money market rates (O/N,1m,2m,3m,6m,1yr) for tree building */
		MSYield[6],							/* Libor rates if above is treasury, vice versa, for discounting. */
	...
	long
		ValueDate,                                                      /* Output: value date */
		Period[2*MAXMAT+4],                                             /* Output: length of money market periods followed by 6m increments */
		Holidays[ MAXHOLIDAYS ];
	int numHolidays;
	double
		OAS,								/* Option adjusted spread (cf Lattice() in dev2.c */
		Zero[2*MAXMAT+4],                                               /* Output: Zeros at money market points followed by 6m interval points */
		ZeroS[2*MAXMAT+4];						/* Output: Zeros at mm points followed by 6m interval points for YieldS*/
...
{
	double
		*ZeroCoupon,                                                    /* Zero coupon price maturing at each node */
		...
		*Delta,								/* convexity adjustment for discounting rates when tree rates differ */
...
{
	int
		NbFwd;                                                          /* Number of forward prepayments */
	long
		FwdMat[NBFWD];                                                  /* Maturity of the forward prepayments */
	double
		Spot,                                                           /* Spot value of the prepayment */
...
/* Number of days of payment delay */
	...
	double
		...
		AmortLevel[MAXNUMP][NBPOINTS],                                  /* Corresponding annual CPR level or Avg Life*/
		AmortLevel1[MAXNUMP][NBPOINTS],
		Amort[MAXSFSNB],                                                /* Additional amortization once the trigger is reached for each SFS */
		Speedup,							/* Speed up the PPF */   
		rational,							/* rationality speed up */
		TriggerRate[MAXSFSNB],                                          /* Trigger rate for each SFS */
		SeasoningLevel[NUMRAMPS],                                              /* 3 Levels of Refi/Coupon that define seasoning (cf SeasoningMat) */
	...
	char
		Freq,                                                           /* Frequency of mortgage payments */
...
double linterpFlat(int n, double * xs, double * ys, double x); // interpolate or do flat extrapolation

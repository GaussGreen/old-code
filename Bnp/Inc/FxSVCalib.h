...
typedef struct _SFxSVComm_Calib
{
    ...
    int           idx_from;  // Index of the first piece smile params of which are being calibrated
    int           idx_to;    // Index of the last piece smile params of which are being calibrated
    int** calib;  // Flags "calibrate parameters" : { beta, alpha, gamma, rho } (per ex date)

    int*     nk;       // Number of strikes (per ex date)
    double** k;        // Strikes ( log(K/FFX0) ) (per ex date)
    double** mkt_val;  // Market prices (per ex date)

    double bl[4], bu[4];  // Lower and upper bounds { beta, alpha, gamma, rho }

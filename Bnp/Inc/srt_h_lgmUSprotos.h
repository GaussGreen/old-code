...
LGMErr calcRangeAccrual( long lPayDate, double dCoupon, int iEODFixFlag, int iAccrueOnBarrier, int nDaysInCoupon, 
							long* lvFixingDates, double* dvWeights, double* dvCouponFixingOrSpread, 
							double dUpperBarrier, double dLowerBarrier, double dUpperSpread, double dLowerSpread, char* szRefRate, double dCvg, 
							double* out_pdFixed );
...
LGMErr LGMCallableTimeSwapCaller2(
  ...
  - on the exotic side, (cpn + gear*accrued_index)*accrued_ratio*coverage */
	...
	double	**forwardTimeSwap,	/*[0..1]*[0..nEx-1]  the first is the forward computed
								today; the second is with the call put parity */
...
LGMErr LGMFillCallableTimeSwap2(
		...
		long n, Date *tStartCpn, Date *tPayCpn,						/* coupon leg */
		double	*tCpn,
		double *tCpn2,  /* should be 0 unless it is a double range */
		double	*tgear_floatdigital,
		double	*tCvgCpn,
		double	*tFundingPayment,
		long nEx, Date *tEx, Date *tExStart, double *exFee,		/* exercise info */

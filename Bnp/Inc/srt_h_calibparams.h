...
/* ------------------------------------------------------------------------
   ...
   FIELDS:
                - eAlgoType: LEVENBERG_MARQUARDT, SIMPLEX, SIMULATED_ANNEALING, SOBENBERG
                        Which algorithm to use in the calibration: LM, SimuAnn, Simplex...
                - eCalibType: GLOBAL, FIXED
                        Which correlation calibration to go for: fixed corr, global, blind on all...
                ...
                - bOneTau: SRT_NO or SRT_YES
                        Use a TermStruct of tau (NO) or just one single value (YES)
                - bFreezeTau: SRT_NO or SRT_YES
                        Freeze the tau structure
                - lNumSobolPaths: 0 or any positive long
                        Number of paths used in the presampling with Sobol (0== do not use)
                - bLgmStartPoint:  SRT_YES or SRT_NO
                        Use LGM as a starting point for the Calibration (Cheyette)
                - lLgmNumIter:
                        Number of iterations used for the LGM starting point calibration
                - bSmoothSigma: add an extra criteria for smoothness of sigma
                - dSigmaMin: >=0.0
                        Minimum value authorised for sigma in the Sobol pre-sampling
                - dSigmaMax;
                        Maximum value authorised for sigma in the Sobol pre-sampling
                - dLambdaMin;
                        Minimum value authorised for lambda in the Sobol pre-sampling
                - dLambdaMax;
                        Maximum value authorised for lambda in the Sobol pre-sampling
                - dOptionsWeight;
                        Weight of the Option Prices matrix in the minimisation criteria
                - dBetaMin;
...
Err srt_f_set_CalibParams(
    String*        pszParamStrings,
    String*        pszValueStrings,
    int            iNumParams,
    SrtCalibParam* psCalibParam,

...
struct tagMBSPT_CashFlowStruct
{
	long DealMaturityDate; //for FNM, this is always the first of the month, this plus 24 days = last payment day
	long SettleDate; //spotsettle, i.e., trade date
	int oterm; //in months, for 30Y it is 360
	int cpnFreq;
	double cpn; //6 for 6%
	int PayDelay; // in days, for FNM it is 24
	double accrualFactor; //this multiplied with face amount (100 for $100) and cpn (5 for 5%) gives the accrued interest
	int numCashFlows; //number of coupons that the new owner on SettleDate will own
	int NbInitialCashFlowToSkip;
	long * duePeriodEndDates; //always the first of month for FNM, last duePeriodEndDate = DealMaturityDate

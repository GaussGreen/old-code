...
/* ------------------------------------------------------------------------
   ...
   ------------------------------------------------------------------------ 
		...
		double 		max_time_per_slice;
				Maximum time in years authorised between two time steps .
		long 		min_nodes_per_path;
				Minimum number of time steps to create for discretisation .
		SRT_Boolean 	force_mc;
				Force the use of Monte Carlo for a "neutral" tableau .
		SRT_Boolean     Jumping;
				Force the use if possible of the Jumping Numeraire method (MC Core).
		SrtMCSamType 	sample_type;
				Monte Carlo random number generation method (ABS, Sobol, ...) .
		SrtMCDfSchType 	difference_scheme;
				Monte Carlo discretisation scheme (Euler or Milshtein) .
		SRT_Boolean 	mc_renormalize;
				Force the Cheyette type Monte Carlo control variate on dfs .
...
- ------------------------------------------------------------------------
   ...
   ------------------------------------------------------------------------ .
		...
		long 		width_phi;
				Number of discrete phi used in a tree discretisation of 
				Cheyette .
		SRT_Boolean		imm_exit;
				Prevents the calculation of the grfn function .
		SRT_Boolean		skip_evaluation;
				Skip the evaluation of the grfn deal: initialisation is done .
		SRT_Boolean 	insert_vol_dates;
				Force the time discretisation to incorporate the volatility 
				dates .
   ...
   ------------------------------------------------------------------------ .
		SRT_Boolean		spline ;
				Only for backward compatibility : not used any longer .
		SRT_Boolean 	dbg;   
				Only for backward compatibility : not used any longer .
   ...
   ------------------------------------------------------------------------ .
		long 		step_num;
				Number of steps in a tree discretisation: set automatically .
		SrtMdlType		imp_mdl_type;
				New field added for backward compatibility : the model used .
		SrtImpType 	imp_type;
				This will be set to either IMPMONTECARLO or IMPBACKWARDLATTICE .
		int 		node_dim;
				Dimension of a tree node according to the length of the 
				grfn tableau .
...
typedef struct {
	...
	SRT_Boolean          exfrontierfirst_time;/*parameters needed for the exercise frontier optimization*/

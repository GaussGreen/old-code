...
typedef enum { TN_ZC, TN_SWAP, TN_FINAL, TN_SWAPKO } TARGET_NOTE_COUPON;

/* Different models */
typedef enum { TN_2F, TN_1F_SV, TN_2F_SV } TARGET_NOTE_MODEL;
...
typedef struct {
  ...
  char *szLGMSVUnd;
  /* Market Functions */
  char *(*getCashVol)(char *szVolCurve, double dStartDate, double dEndDate,
                      double cashStrike, int iZero, char *szRefRate,
                      double *dVol, double *dPower);
  char *(*getDF)(char *szYieldCurve, double dStart, double dEnd, double *dDF);
  /* EOD flags */
  int eodFixFlag; /*	0: do not check for fixings today; 1: check for fixings
                     today */
  int eodPayFlag; /*	0: ignore cash-flows with payment date today; 1: include
                     cash-flows */

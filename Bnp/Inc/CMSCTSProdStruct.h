...
typedef struct
{
	...
	char	*(*GetCashVol)(	char	*cVolCurve,
							double	dStartDate,
							double	dEndDate, 
							double	dCashStrike,
							int		iZero,
...
typedef struct
{
	...
	double			*dCoverage;	
	double			*dMargin;	
	double			*dFixCoupon;		/*	Past coupon fixing if relevant,	includes spr, but not mrg, cvg and notional */
...
Err cmscts_fill_check_all_struct(CMSCTS_DEAL			sDeal,

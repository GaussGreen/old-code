...
typedef struct
{
    ...
    SrtDiffusionType eVolType; /*	Type of vol in matrix, SRT_NORMAL or SRT_LOGNORMAL */
...
typedef struct
{
    ...
    double* dCoverage;
    double* dMargin;
    double* dFixCoupon; /*	Past coupon fixing if relevant,	includes spr, but not mrg, cvg and
                           notional */
...
Err cmscts_interp_coupon_type(char* cCouponType, CMSCTS_COUPON_TYPE* eCouponType);
...
typedef struct
{
    ...
    /* Accrual Def */
    int* iFloatingType; /* 0: no floating, 1: floating in advance, 2: floating in arrears */
...
Err cmscts_interp_TARGET_type(const char* constStr, CMSCTS_TARGET_TYPE* eTargetType);
...
Err cmscts_fill_check_all_struct(
    CMSCTS_DEAL sDeal, CMSCTS_MARKET sMarket, CMSCTS_CPN_NUMPARAMS sNumerParams);

...
/* Second step : define the method + (optional params szVolCurveName, numstrikes, strikes, vol,
beta, etc....if necessary
...
/* 2. approx smile log	: myVol.method = 1, myVol.voltype = SRT_LOGNORMAL, + fill myVol.start,
myVol.end, myVol.szVolCurveName, myVol.numstrikes, myVol.strikes
/* 3. full sabr			: myVol.method = 2, myVol.szVolCurveName = vol_name(mkt),
myVol.start, myVol.end , myVol.voltype = Inputtype atm vols (norm, log, beta);
/* 4. strike vol		: myVol.method = 4; myVol.atm = 0; myVol.szVolCurveName =
vol_name(mkt); myVol.voltype = type of vol in the mkt(norm, log);
*/

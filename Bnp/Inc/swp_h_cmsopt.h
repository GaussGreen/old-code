...
/* This the New version including Smile and swaption replication for GRFN Use only 
	-> Flat Vol*/
Err swp_f_cmsoption(double, double, SrtCompounding, double, double, double,
		SrtReceiverType, double, double, int, SrtDiffusionType, double * );
...
/* Second step : define the method + (optional params szVolCurveName, numstrikes, strikes, vol, beta, etc....if necessary
/* Third step  : call the function
/* Last step   : free the structure :  CMSTECfreevol(&myVol);
/*
/* Example of second step :
/* 1. flat log vol case	: myVol.method = 0, myVol.vol = 10%, myVol.voltype = SRT_LOGNORMAL */
/* 2. approx smile log	: myVol.method = 1, myVol.voltype = SRT_LOGNORMAL, + fill myVol.start, myVol.end, myVol.szVolCurveName, myVol.numstrikes, myVol.strikes
/* 3. full sabr			: myVol.method = 2, myVol.szVolCurveName = vol_name(mkt), myVol.start, myVol.end , myVol.voltype = Inputtype atm vols (norm, log, beta);
/* 4. strike vol		: myVol.method = 4; myVol.atm = 0; myVol.szVolCurveName = vol_name(mkt); myVol.voltype = type of vol in the mkt(norm, log);
*/
...
Err swp_f_CMSTEC_Weights(/* underlying */
					 ...
					 /* delay parameters */
					 double				dCvgZC,	/* cvg of the delay */
					 double             dSpdZC, /* B(0,Tpay) / B(0,Tstart) */
					 /* numerical param */
					 CMSParams			*params,
					 /* Output */
					 double				*pdW,

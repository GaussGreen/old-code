...
/* Integral of (1.0 - exp(-x*(T-u))) / x * (1.0 - exp(-y*(T-u))) / x between s
 * and t */
...
/* Integral of (1.0 - exp(-x*(Tx-u))) / x * (1.0 - exp(-y*(Ty-u))) / y between s
 * and t */
...
Err Fx3DDefImpVol2(/*	Total number of options */
                   ...
                   long maturity_date, char *yc_dom, char *vol_dom,
                   char *ref_rate_dom, char *basis_dom, char *cpd_dom,
                   char *ccy_dom, char *yc_for, char *vol_for,
...
/* Fx3D functions that can be used with the name of the underlying Fx. Used in
 * exportation to Xl */
...
Err Fx3DtsFxFwdCov(double option_maturity, double start_date, double end_date,
                   ...
                   double *sig_curve_fx2, double **correlations, double *covar);
...
Err Get_LGM_TermStructure(char *underlying, double **sigma_time, double **sigma,
                          long *sigma_n, double **tau_time, double **tau,
                          long *tau_n);

Err Get_LGM_TermStructure2(char *underlying, double **sigma_time,
                           double **sigma, long *sigma_n, double *fixed_tau);

Err Get_LGM2F_TermStructure(char *und_name, double **sigma_time, double **sigma,
                            long *sigma_n, double *fixed_tau,
...
Err Get_FX_StochRate_TermStructures(
    ...
    double **sigma_date_fx, double **sigma_fx, long *sigma_n_fx, double *rho_df,
    double *rho_dfx, double *rho_ffx);

Err display_FX1F_TermStruct(char *szFxUndName, long *sigma_n,
                            double **sigma_date, double **sigma);
...
Err Fx3DFwdTpay_corr(char *fx_underlying, double Tfix, double Tval, double Tpay,
                     int log_flag, double *expect);

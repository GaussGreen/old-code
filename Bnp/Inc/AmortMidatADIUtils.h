...
#define _INIT_MATRIX_(                                                             \
    _matrix_, _row_begin_, _row_end_, _col_begin_, _col_end_, _value_, _nr_, _nc_) \
    {                                                                              \
...
typedef int(__stdcall* PFN_COMPARE_BINARY)(const void* /*pvRight*/, const void* /*pvLeft */);
...
double exp_integrate_pc(
    double        dLB,  // integration lower bound
...
//+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+--+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-
// performs binary search to determine least n s.t. compare(key,begin + m) <= 0 for all m >= n
...
//+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+--+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-
// performs binary search to determine n s.t. compare(key,begin + m) < 0 for all m >= n
...
//+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+--+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-
// look up psubcBegin in pcBegin and return corresponding pvBegin in psubvOut
// if elements in psubcBegin are not found in pcBegin, interpolate according to SORT convention
void _bmatch_ex(
    const void* pcBegin,    // linear coordinates
    unsigned    ncSize,     // size of coordinates (in bytes)
    const void* pvBegin,    // # elements in values
...
//+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+--+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-
// perform linear interpolation according to the SORT convention : (left open,righ close]
// results returned in pv

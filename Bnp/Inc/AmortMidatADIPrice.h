...
typedef struct _Precomputed_Quantities_TimeSequence_ {
  ...
  const double *m_pdDF_Begin; /// discount factors observed at today = 0 for the
                              /// time sequence
...
typedef struct _Precomputed_Quantities_FloatingLegPay_ {
  ...
  const double *const m_pdCoverage_Begin;
  const double
      *const m_pdPaymentAdj_Begin; // notional * coverage * (spread + margin)
...
const char *Price_GenMidAt(
    ...
    const double *pdFixCvg_Begin, const double *pdFixNotional_Begin,
    ...
    // term structure
    const long *plSigDate_Begin, const long *plSigDate_End,
    const double *pdSig_Begin, const long *plLamDate_Begin,
    const long *plLamDate_End, const double *pdLam_Begin, const double *pdAlpha,
    const double *pdGamma, const double *pdRho,
    // grid
...
const char *_Price_GenMidAt(
    ...
    const double *pdLamT_End, const double *pdLam_Begin, const double *pdAlpha,
    const double *pdGamma, const double *pdRho,
    /// Call
    ...
    const double *pdFixNotional_Begin, const double *pdFixCoupon_Begin,
    const double *pdFixFee_Begin,
    //// floating leg
    const long *plFltPay_Begin, const long *plFltPay_End,
    const long *plFltStart_Begin, const long *plFltEnd_Begin,
    ...
    double dPayRec,
    /// result
    double *pdFixPV, double *pdFltPV, double *pOptionPV, double *pdExProb,
    double *pdExBoundary, int nUse_Backward);
...
const char *_alloc_Option(int nExSize, int nFixSize, int nFltSize,
                          /// output
                          _GenMidAt *pOption);
...
const char *_alloc_Seq(int nDateSize,
                       /// output
                       _PCQ_Seq *pSeq, const double **ppdEIL1_Begin,
                       const double **ppdEIL2_Begin);
void _preinit_Seq(const char *szYC, long lToday, const long *plDate_Begin,
                  const long *plDate_End,
                  /// output
                  const _PCQ_Seq *pSeq);

void _free_Seq(const _PCQ_Seq *pSeq);

const char *_GenMidAt_OptionPV(const _GenMidAt *pOption, const _Model *pModel,
                               const _PCQ_Seq *pNumeraire, int nNumPointX,
                               int nNumPointT, double *pOptionPV,
                               double *pdExProb, double *pdExBoundary,
                               int nUse_Backward);

...
/******************************************************************************\
    ...
    NOTE            :   [1] Expressions are simplified as mush as possible. 
                            e.g.  2+5 is reduced to 7, as is d[i], i+2 (since
                            the symbol i is known), max, pow, cvg, etc...
...
struct COMLL_STRUCT {
	...
	long                *dfindfloat;                  /* Indices of df needed by COMMLL
													     for floating leg of swap (spreads)
														                                */
...
#define test_real(I,N,C) { if(C->type != COMLL_REAL) {\
...
#define CheckAuxRange1(I,GD){if((I) < 0 || (I) > GD->auxwidth-1)\
...
#define CheckAuxRange2(I,J,GD){\
    CheckAuxRange1(I,GD);CheckAuxRange1(J,GD);\
    if(GD->auxlen[(I)] != GD->auxlen[(J)]){\
      return serror("%s: lengths %d,%d",\
                    GRERR_INTERP_SAME_SIZE,\
                    gd->auxlen[(I)],\
                    gd->auxlen[(J)]);}}

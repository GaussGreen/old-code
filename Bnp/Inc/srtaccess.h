...
char *SrtGrfnMainExFrontier(char *domestic,
 ...
 char *SrtGrfAmer(int numParams, char **paramStrings, char **valueStrings, char *undName,
 				  double start, double end, int nfp, int delay, char *cpdStr, char *basisStr,
				  double *strikes, int nStrikes, char *recPayStr, double *price);
 ...
 char *SrtClosedForm(long startLong, long endLong, char *compound,
	   			     char *basis, char *recPay, char *dealType,
	                 double strike, double bondStrike, char *refRateCodeStr, 
	                 int mdlRows, char **paramStrings, char **valueStrings,
	                 char *undName, double *price,
...
char *WrapSrtBootstrap(int numParams, char **paramStrings, char **valueStrings, int *nInputs,
                   long *startDates, long *endDates, char **cpdStr, char **basisStr,
		           double *strike, double *bondStrike, char **typeStr, char **recPayStr,
			       String     *refRateStr,
				   double *undPrice,
				   Err  (*pfGetVol)(double dStart, double dEnd, double dStrike, 
						double dForward, double dSpread, double *pdBsVol),
...
char *SrtBootstrap(int numParams, char **paramStrings, char **valueStrings, int *nInputs,
                   long *startDates, long *endDates, char **cpdStr, char **basisStr,
		           double *strike, double *bondStrike, char **typeStr, char **recPayStr,
			       String     *refRateStr,
					double *undPrice,
...
Err srt_f_get_fx_implied_vol(
 ...
 char *SrtInitIRUnd(char *undName, char *ycName, char *model, 
					...
					double vovol, double etaOrBeta2,double vasicek_init_cond,
					int	vasicek_mean_rev_level_rows,int	vasicek_mean_rev_level_cols,double ** vasicek_mean_rev_level_vals);
...
/* OUTPUT */
 ...
 char *SrtInitEQUnd(char *undName, char *model, double spot, char *ycName1, char *ycName2,char *repoName, 
                    int volCrvRows, int volCrvCols, double **volCrvVals,
					double omega,double beta,double gamma,double voldrift,double vovol,double rho);

...
Err srt_f_init_FX_STOCH_RATES_TermStruct(char *undName, SrtUndPtr dom_und,
                                         SrtUndPtr for_und, TermStruct **ts,
...
/* ---------------------------------------------------------------------------------------
                                                        For FX Stoch Rates with
   Jumping Numeraire
   -----------------------------------------------------------------------------------------*/

/* Compute the different functions needed and hang them to each point of the
 * term structure  */
...
Err srt_f_get_fx_implied_vol(double yr_to_exp, String fx_und_name,
                             double *fx_bs_implied_vol);

Err srt_f_get_fx_stoch_rates_ind_correl(double start_date, double end_date,
                                        String first_und_name,
                                        String second_und_name,
                                        String fx_und_name, double *corr);
/* To be find with "find in file":  PVMPI*/
/* The following lines must be included in changes */
#ifdef __cplusplus
}

...
double MBS_GetVolTermTenor(void *VC, // VolatilityCurve
                           long today, int TermNumUnits, SrtUnit TermUnitType,
...
unsigned MBS_SwapRate_Create( // NB: returns 0 on error
    SrtBasisCode eBasis_Fixed, SrtCompounding eFreq_Fixed, const char *szYC,
...
unsigned MBS_ForwardRate_Create( // NB: returns 0 on error
    const char *szYC, const char *szRefRate);
...
_MBS_EXTERN_C
double MBS_SwapRateEvaluate_TermTenor(void *YC_in, // IYieldCurve
                                      int TermNumUnits,

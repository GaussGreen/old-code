   ...
   PURPOSE		: Defines the structure used to store all the pointer to
                  functions that have to be set before calling any SRT
                                  ...
                                  This include:
                                        ...
                                        - VolFunc ( vol_curve_id, start, end, strike, *vol )
                                        - ....
...
typedef Err (*SpreadFuncType)(char* rate_id, double start, double end, double* spread);

/* The RateInformation function type */

typedef Err (*RateInfoFuncType)(
    char* rate_id,
    char* family_code,
    char* ccy,
    char* tenor,
...
typedef Err (*VolFuncType)(
    char* vol_id, double start, double end, double strike, double* vol, double* power);

/* The Correlation function type */

typedef Err (*CorrelFuncType)(char* vol_id, double start, double end, double strike, double* vol);

/* The Volatility component function type */
/* ATMlog, ATMNorm, ATMBeta, Alpha, Beta, Rho */
/* Log, Norm for a given Strike */

typedef Err (*SABRVolFuncType)(
    char*   vol_id,
    double  start,
    double  end,
    double  strike,
...
/* -----------------------------------------------------------------------------
   Sets or Gets the Official SABRVol Component Interpolation Function used in the pricing
   ----------------------------------------------------------------------------- */

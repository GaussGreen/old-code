...
typedef struct {
  ...
  double *Yields;
  long Ref_Date;
} Yield_Data; // Yield_Data enables construction of function Y: Days --> Y(t) =
              // yield to maturity from RefDate to (t + Ref_Date)
...
char *
MMYields_To_ZeroRates(Zero_Data *zero_data, MM_Data *mm_data, int frequency,
                      YBASIS YearBase); // convert MMYields into zero_rates
...
char *
Yield_Data_Interpolate(Yield_Data *yield_data,
                       int interp_frequency); // intrep_frequency in months
...
char *MBS_Strip(Zero_Data *zero_data, MM_Data *mm_data, Yield_Data *yield_data,
                ...
                long zero_Ref_Date);
typedef enum VOL_TYPE { LOGNORMAL = 0, NORMAL } VOL_TYPE;
...
// then zeros[i][j] has meaning only for j >= currIndex and it means the value
// at time slice currIndex and space node i of a zero-coupon maturiing at time
// slice j

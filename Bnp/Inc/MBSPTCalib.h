...
typedef struct
{
    ...
    long    Ref_Date;
} Zero_Data;  // Zero_Data enables us to construct a func df: Days --> R, df(t) = df spanning from
              // Ref_Date to (Ref_Date + t)
...
typedef struct
{
    ...
    double*    Yields;
    long       Ref_Date;
} Yield_Data;  // Yield_Data enables construction of function Y: Days --> Y(t) = yield to maturity
               // from RefDate to (t + Ref_Date)
...
char* Yield_Data_Interpolate(
    Yield_Data* yield_data, int interp_frequency);  // intrep_frequency in months
...
// then zeros[i][j] has meaning only for j >= currIndex and it means the value at time slice
// currIndex and space node i of a zero-coupon maturiing at time slice j
...
{
    long RefDate;
    int  PPY;  // parts per year, 12 for roughly month spacing; for cashflow times interval dT, N =
              // number of time slice intervals = round(PPY*dT); ASSUME dT is close to integer
              // multiple of 1/PPY
    double DX_COEFF;
    double beta;
    ...
    int    NbTimeSlices;
    int    NbSpaceNodes;
    int    EndTimeSlices;  // = NbTimeSlices - 1, we start backward induction for zeros at
                           // tree_times[EndTimeSlices-1]
    double* tree_times;    // always starts from 0.0 <-> RefDate
    double*
        DFS;  // DFS[i] = values at tree_times[0] of $1 payoff at tree_times[i]; i.e., DFS[0] = 1.0
    double* sigmas;  // sigmas[i] = constant sigma value prevailing over [tree_times[i],
                     // tree_times[i+1]); if i = NbTimeSlices, treat tree_Times[i+1] = infinity, this
                     // last one is most likely not used
    int               isZV;  // zero-vol
    int*              Bottom;
    int*              Top;
    double*           Drift;

...
Err get_bgm_fra(long Fradate, int spotlag, char *cRefRateCode,
                char *szYieldCurveName, double *dFra);

Err get_bgm_SwapFraWeights(double maturity, double underlying,
                           char *szYieldCurveName, char *cRefRateCode,
                           double *weights);

Err get_bgm_SwapFraHatWeights(double maturity, double underlying,
                              char *szYieldCurveName, char *cRefRateCode,
                              double *weights, double *hatweights);

Err srt_BGMF_Weight(long MaxNumPeriod, long MaturityInPeriod, char *szRefRate,
                    char *szYieldCurveName, long today, long *tStart,
                    long *tPay, double *tCvg, double ***pppdWeights);

Err srt_BGMF_WeightNew(long MaxNumPeriod, long MaturityInPeriod,
                       char *szRefRate, char *szYieldCurveName,
                       char *szFrequency, char *szBasis, long today,
                       long *tStart, long *tPay, double *tCvg,
                       double ***pppdWeights);

Err build_instrument(double *vecs, double **matres, int dim);

Err get_bgm_AllCoverages(double **instruments, int num_of_fras,
                         char *szYieldCurveName, char *cRefRateCode,
                         double *cvgs);

Err get_bgm_SwaptionOMEGAMatrix(double *hatweights, int num_of_tenors,
                                double **OmegaMatrix, double *coverages);

Err get_bgm_CapletOMEGAMatrix(int CapletIndex, int num_of_tenors,
                              double **OmegaMatrix, double *coverages);

Err srt_bgm_OMEGA_Constraints(double **instruments, int num_of_instruments,
                              int num_of_caplets, double ***OMEGA,
                              char *szYieldCurveName, char *cRefRateCode);

Err get_bgm_SwapNumOfPeriods(double maturity, double underlying,
                             char *szYieldCurveName, char *cRefRateCode,
                             int *VectorSize);
...
Err get_bgm_HistoricalVarianceCovarianceMatrix(
    double **capletsinstruments, int num_of_fras, double strike,
    double **historicalcorrelation, double **histvarcovar,
    char *szYieldCurveName, char *cRefRateCode, char *szVolCurveName);

Err srt_bgm_Calibration(double **instruments, int num_of_instruments,
                        double **capletsinstruments, int num_of_fras,
                        double strike, double **histcorrelation,
                        char *cRefRateCode, char *szYieldCurveName,
                        char *szVolCurveName, double **calibratedmatrix,
                        double *result, double *error, int *return_err,
                        double toler, int printlevel, int niter);

Err srt_bgm_Calibration_with_PCA(double **instruments, int num_of_instruments,
                                 double **capletsinstruments, int num_of_fras,
                                 double strike, double **histcorrelation,
                                 char *cRefRateCode, char *szYieldCurveName,
                                 char *szVolCurveName, int *num_of_factors,
                                 double **volfunc, double *result,
                                 double *error, int *return_err, double toler,
                                 int printlevel, int niter);

Err srt_bgm_MatrixReduction(double *eigen_val, int dim, int *numfactors,
                            double **VolMatrix);

Err srt_bgm_VarCovarRebuild(double **VolMatrix, double ***NewVarCovar,
                            int dimrow, int dimcol);

Err srt_bgm_MCEvolve(
    double **LiborMatrix, int ndiscretization, int *Indices, double *maturity,
    double *coverages,
    int LastTenor, /* Last tenor necessary to compute all the discount factors
                      , numeraire of the measure in which we price*/
    int FirstTenor, double **Brownianincs, int num_of_factors,
    double **VolMatrix, double **VarCovar);
...
Err srt_bgm_ComputeRollingNumeraire(double **LiborMatrix,
                                    int *TenorIndicesinDisc, int LastTenorIndex,
...
Err srt_f_BGMVolMatrix(long MaxNumPeriod, long MaturityInPeriod,
                       long nCorrelRow, double **ppdHistCorrel, double *pdBeta,
                       char *szRefRate, char *szYieldCurveName,
...
Err srt_f_BGMImpliedCorrel(
    long MaxNumPeriod, char *szRefRate, char *szYieldCurveName,
    char *szVolCurveName, char *szCorrelMode, /* SLIDING or CONVERGING */
...
Err srt_f_FwdVolMatrix(
    long nCorrelRow, double **histcorrel, long nInstr, long **Instr,
    long **InstrDate, char *szRefRate, long NumPeriodEnd,
...
Err srt_f_BGMGetATMSwaptionVolNew(
    double **ppdCorrelInput, long nCorrelSize, char *szCorrelType,
    double **ppdVolMatrix, /*This matrix is in Normal Vols!*/
    long nVolInput, char *szRefRateInput, char *szYieldCurveNameInput,
    double *Expiry, long nExpiry, double *Underlying, long nUnderlying,
    double
...
Err srt_bgm_GetVolMatrixSlidingSliding(double **VolMatrix, double **correl,
                                       int num_tenors, int *factorsofmatrix,
                                       double **VolMatrixPCA);
...
Err srt_BGMFwdVolComputeTenorsNew(long MaxNumPeriod, long MaturityInPeriod,
                                  long *tStart, long *tPay, char *szRefRate,
...
/* Needed for BGMStripATM.c */
Err srt_BGMFwdVol_computeCoeff(long indexTime, long indexTenor, double *tLibor,
                               double *tExp, double **tFwdSwap,
                               double ***pppdWeights, double **ppdHistCorrel,
                               SMILE_MOVE_TYPE ModelCorrelMove,
                               double **ppdTempVol, double *A, double *B,
                               double *C);

/* Needed for BGMStripATM.c */
Err BGM_FillMarketVol(long MaxNumPeriod, long MaturityInPeriod, long *tStart,
                      long *tPay, double **tFwdSwap, double **tFwdSwapSpread,
                      int CashOrLibor, double CapStrike,
                      STRIKE_TYPE CapStrikeType, double SwapStrike,
                      STRIKE_TYPE SwapStrikeType, char *szRefRate,
                      char *szVolCurveName, double **ppdMarketVols);

/* Needed for BGMStripATM.c */
double srt_BGMFwdVol_computecovar(
    long indexTime, long indexStart, long indexEnd, /* For the Swap */
    long indexTenor, /* on the libor indexStart up to indexTenor - 1*/
    double *tLibor, double ***pppdWeights, double **ppdHistCorrel,
    SMILE_MOVE_TYPE CorrelMode, double **ppdTempVol);
#endif
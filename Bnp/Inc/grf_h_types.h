...
/******************************************************************************\
    ...
    ELEMENTS            :   t       Summary of grammatical information
							...
							sprdlen Length of spread 
									( = 0 if no spread, = dflen otherwise)
							spread  Spread of the reference rate to cash rate,
							        stored on payment date (end of period), for 
									all the df dates
\******************************************************************************/
...
/* --------------------------------------------------------------
   ...
   DESCRIPTION  :  Struct to wrap a pointer to a function.
				   ...
				   So we use the name field to find the new address.
  --------------------------------------------------------------*/
...
typedef struct
{
	...
	int			end_of_day_payment; /* Due to misuse of the two previous ones.
													This one is only use by the GRFN language PAY function */
...
#define GrfnAddStatus(status,mask)  {status[mask] = SRT_YES;}

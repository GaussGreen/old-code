...
char		*ref,							//	ref rate (only if
calib)
char		*swap_freq,						//	swap freq (only if
calib) char		*swap_basis,					//	swap basis (only if
calib)
int			lam_ts,							//
...
double		*tslam,							//
corresponding lambdas if ts
double		alpha,							//	alpha
double		gamma,							//	gamma
double		rho,							//	rho
//	Calib params
int			force_atm,						//	force atm
calib double		max_std_long, double		max_std_short,
int			fix_lambda,						//	0: calib
lambda to cap, 1: fix lambda calib
                                                                                        //
to diagonal
int			one_f_equi,						//	1F
equivalent flag:
...
int			skip_last,						//	If 1, the
last option is disregarded
                                                                                        //
...
structure
Err			(*get_cash_vol)(				//	function to get IR
cash vol from the markets char	*vol_curve_name, double	start_date, double	end_date, double
cash_strike, int		zero, char	*ref_rate_name, double	*vol, double	*power),
LGMQTO_UND		und,
long		spread_vol_n,

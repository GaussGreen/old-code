...
SrtProduct* srt_f_init_range_accrual(
    char*   name,          /* name given to the product */
    ...
    char*   refrate_d,     /* standard domestic ref rate */
    char*   refrate_f,     /* index ref rate */
    int     n_fxvol_dates, /*	\							*/
    long*   fxvol_dates,   /*	 }	FX vol curve			*/
    double* fxvol,         /*	/	(NULL if not quanto)	*/
    double* qtocorrel,     /* quanto correl term structure */
    int     n_periods,
    long*   dates,      /* n_periods + 1 : a start + payment dates */
    double* cpns,       /* coupons for each period */
    char*   basis,      /* day count basis */
    double* upper_barr, /* upper barrier for each period */
    double* lower_barr, /* lower barrier for each period */
    char*   recpay,     /* Payer - receiver */
    double  c_spread,   /* call spread margin */
    int     obs_freq,   /* observation frequency in days */
    double  rho_df);     /* domestic mkt - index correlation */
...
struct _SrtRAObservStep
{
    ...
    double K[4];                 /* strikes corresponding to barriers */
                                 /*	double		vol_atK[4];			/* volatilities at the barriers */
...
typedef struct _SrtRangeAccrualSpec
{
    ...
    SrtRAObservStep** obs_steps; /* pointers to first observation steps in periods */

...
typedef struct
{
	...
	/*	CMS Adjustment */
	int				adj_type;		/*	0: no adjustment, 1: atm normal adjustment, 2: smile adjustment */
	/*	If smile is used, CMS = F+lambda*F^(2*beta)*mat
		beta is input, lambda = (CMS-F)/(F^(2*beta)*mat)
		
		if shifted log is used, if smile is used, ignores the input beta, CMS = F + lambda*(F+slshift)^2*mat
...
typedef struct
{
	...
	/*	Coupon is of the form alpha * CMS1 + beta * CMS2 + gamma [floored, capped] */
...
typedef struct
{
	...
	1:	CIF				(all coupons are Midat and at least 1 is CIF)
	2:	CMS				(all coupons are Midat or CIF and at least 1 is CMS)
	3:	CMS Spread		(at least 1 coupon is CMS spread) */
	int				type;
...
typedef struct
{
	...
	double			cf_spr_opt_std[CCF_NCPN][CCF_NOPT];	/* Normal variance of the spread option */

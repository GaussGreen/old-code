...
String
LGMamerican(long nfix,         /* number of fixed periods         */
            Date tfix_start[], /* [0      ,1      ,...      ,nfix-1]  start
                                * dates for fixed coupons
                                */
            Date tfix_end[],   /* [0      ,1      ,...      ,nfix-1]  end dates
                                  for fixed coupons   */
            Date tfix_pay[],   /* [0      ,1      ,...      ,nfix-1]  pay dates
                                  for fixed coupons   */
            ...
            int early_flag_fix, /* 0=subtract accrual from frst pymnt; 1=add
                               accrual to fee */
            ...
            int usecaps, /* if fixed tau      , then 1=use caplets to calibrate
                        , 0=use long swptns */
            double In_tau, /* if fixed tau      , use this value for tau (in
                              years) */
            String (*GetVol)(Date, Date, double, SRT_Boolean,
                             double *), /* function to get swaption vols */
            char *char_vol_type,        /*       */
            int update_ts_flag,   /* 1=cmpute new sigmas and taus & store in
                                 yldcrv;   0=don't bother */
            int find_exer_bdry,   /* 1=find swap rates at exercise bocrvary;
                                 0=don't   bother       */
            String outfile,       /* output log file (unused)       */
            double *LGMamervalue, /* answer */
            SrtLgmExerBdryData *lgmExerBdryData); /* ptr to exercise boundary
                                                     data structure         */

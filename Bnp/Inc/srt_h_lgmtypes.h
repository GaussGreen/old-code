...
typedef struct LGM_TS_ {
  ...
  /* ONE FACTOR CASE */
  double
      *zeta; /* zeta[0  ,1  , ...  , numZ-1] are zeta values at above dates */
  double *G; /* G[0  ,1  , ...  , numG-1] are G values at above dates */

  /* TWO FACTOR CASE */
  double *
      Zeta1; /* zeta1[0  ,1  , ...  , numZ-1] are zeta1 values at above dates */
  double *
      Zeta2; /* zeta2[0  ,1  , ...  , numZ-1] are zeta2 values at above dates */
  double *Zeta12; /* zeta12[0  ,1  , ...  , numZ-1] are zeta12 values at above
                     dates */
...
/* If exercised at tEx  , the holder receives all positive payments  , and pays
   all negative payments. For receivers  , all the payments should be positive
   except for the first payment (a negative payment of the strike) made at the
        settlement date. For payers  , all payments should be negative except
   for the strike. */
...
typedef struct SrtSimMidAt_ {
  ...
  Date *tStart;   /* [0  ,...  ,nEx-1]  , settlement (start) dates for each
                     exercise */
  double *Strike; /* [0  ,...  ,nEx-1]  , amount paid on settle for fixed leg at
                     each exercise */
  long *FirstPay; /* [0  ,...  ,nEx-1]  , payment number of first payment
                     received for each exercise */
  double *RedFirstPay; /* [0  ,...  ,nEx-1]  , reduction in amount of first
                          payment received after exercise */
...
/* The SrtSimMidAt structure assumes that a deal can only be exercised once  ,
on one of the dates tEx[0  ,1  ,...  ,nEx-1]. Suppose the deal is exercised at
, say  , tEx[j]  ,
...
If PayRec = SRT_RECEIVER  , then the receiver of the fixed leg has the option to
exercise. If PayRec = SRT_PAYER  , then the payer of the fixed leg has the
option to exercise.

WARNING: The first payment is payment 0  , so for vanilla deals  , the FirstPay
array would start FirstPay[0]=0  , meaning that if the deal is exercised at
tEx[0]  , then it will receive payments at tPay[0]  , tPay[1]  , ...
...
typedef struct SrtGenMidAt_ {
  ...
  long *nPay; /* [0  ,...  ,nEx-1]  , number of payments if exercised at each
                 date */
  double **Payment; /* Payment[j][i]  , j=0  ,1  ,...  ,nEx-1  , i=0  ,1  ,...
                       ,nPay[j]-1 at */
  Date **tPay;      /* tPay[j][i]  , j=0  ,1  ,...  ,nEx-1  , i=0  ,1  ,...
                       ,nPay[j]-1 are the */
                    /* the payments received if exercised at tEx[j] */
} SrtGenMidAt, *SrtGenMidAtPtr;

/* This structure assumes that a deal can only be exercised once  , on any of
the dates tEx[0  ,1  ,...  ,nEx-1]. If the deal is exercised at  , say  , tEx[j]
, then the option exerciser will receive the series of payments Payment[j][i]
paid at tPay[j][i]  , i=0  ,1  ,...  ,nPay[j]-1. If a Payment is negative  ,
then it represents an amount the exerciser has to pay on tPay. To change between
payers & receivers  , we just need to flip the signs of the payments.
...
typedef struct SrtSimAmer_ {
  ...
  Date *tfixStart; /* [0  ,1  , ...  , nfix-1]  , start date of each fixed
                      period */
  Date *tfixEnd; /* [0  ,1  , ...  , nfix-1]  , end date of each fixed period */
  Date
      *tfixPay; /* [0  ,1  , ...  , nfix-1]  , pay date for each fixed period */
  double *fixCoupon; /* [0  ,1  , ...  , nfix-1]  , fixed leg interest payments
                        (no notional in final payment) */
  ...
  Date *tfltFixing; /* [0  ,1  , ...  , nflt-1]  , fixing dates for each
                       floating period */
  Date *tfltPay;    /* [0  ,1  , ...  , nflt-1]  , pay dates for each floating
                       period */
  SrtBasisCode
  ...
  BusDayConv convSettle;  /* business day convention for the settlement date (if
                             calendar) */
  double *ExtraPrem;      /* [0  ,1  , ...  , nfix-1]  , extra amount paid on
                             settlement */
} SrtSimAmer, *SrtSimAmerPtr;

/*
CAUTION: SrtSrtSimAmer swaption structure assumes that the notional is 1.
CAUTION: The final fixPayment should include only the interest rate coupon  ,
NOT the notional. CAUTION: Any floating leg spread (the difference between the
...
ExtraPrem<0 decreases the value of the floating leg  , regardless of whether the
deal is a payer of receiver. If the deal has an "exercise premium" which is paid
by the option owner to exercise  , then the sign must be changed if it is a
PAYER.
...
"exercise into" swaptions should have EarlyFlags 0  , and ResetFlt may or may
not be 1. See below.

Fixed leg:
        The interest rate coupon for the period tfixStart[i] to tfixEnd[i] is
fixCoupon[i] and is paid on tfixPay[i]  , i=0  ,1  , ...  , nfix-1. The notional
of 1 is also paid on tfixPay[nfix-1] Floating leg: The interest rate for the
period tfltPay[i-1] to tfltPay[i] is set (fixed) on tfltFixing[i] and paid on
tfltPay[i]  , i=0  ,1  , ...  , nflt-1. The notional is also paid on
tfltPay[nflt-1]. The code assumes that the start date of the first period is
tfixStart[0]  , which causes no appreciable errors.

Exercise:
        If PayRec = SRT_RECEIVER  , then the receiver of the fixed leg has the
option to exercise. If PayRec = SRT_PAYER  , then the payer of the fixed leg has
the option to exercise. This structure assumes the deal can be exercised on any
day that is on or after both tFirstExer and tFirst  , where tFirst is either the
evaluation date (if end-of-day is off) or the evaluation date plus 1 business
day (if end-of-day is on). Neither the evaluation date nor the end-of-day flag
are part of the deal structure; they are supplied to the valuation code
independently.

If the deal is exercised on tEx  , the theoretical settlement date is
tSettle_theor  , which is lagExerSettle (calendar or business) days after tEx.
If business days  , then the actual settlement date tSettle is the same as the
theoretical settlement date. If calendar days  , then the business day rule
convSettle is applied to get the actual settlement date from the theoretical
date.

Suppose that the deal is exercised on tEx  , with actual settlement date tSettle
, and suppose that the settlement date falls in the k-th fixed period  ,
...
reduced by the interest that has already accrued in the then current period  ,
k. So the receiver gets the payments fixCoupon[k](1 - coverage(tfixStart[k]  ,
...
If EarlyFlagFix is 1  , then the receiver must pay the accrued interest
        fixCoupon[k] * coverage(tfixStart[k]  , tSettle)/coverage(tfixStart[k]
, tfixEnd[k])) on the settlement date tSettle  , but then receives the entire
amount of all remaining coupons: fixCoupon[i] paid at tfixPay[i]  , i=k  , k+1
, ...  , nfix-2 1 + fixCoupon[nfix-1] paid at tfixPay[nfix-1]

In return for receiving the fixed leg payments  , the receiver makes floating
leg payments. Apart from some corrections (see below)  , these floating rate
payments are assumed to be worth the notional (of 1) plus any extra exercise
premium paid at settlement. ANY SPREAD due to the difference between cash and
the floating leg has to be accounted for BEFORE calling LGMAmerican.

To be more precise  , suppose that the settlement date occurs in the k-th fixed
period: tfixEnd[k-1] <= tSettle < tfixEnd[k]. Then the floating leg is assumed
to be worth 1 + ExtraPremium[k] + "corrections" paid at tSettle.

Note that ExtraPrem[k]>0 INCREASES the value of the floating leg. If ExtraPrem
is an exercise premium  , paid by the exerciser  , then the sign of ExtraPrem
must be adjusted depending on whether the deal is a Payer or Receiver.

The corrections are:
        If EarlyFlagFlt is 0  , then the first floating payment received is the
interest for the period tSettle to tfltPay[k]. If ResetFlt is 0  , then the rate
for this first period is the previously fixed LIBOR rate which was fixed at par
on tfltFixing[k] for the period tfltPay[k-1] to tfltPay[k]); this causes the
floating leg to not be worth the same as 1 paid on tSettle. If ResetFlt is 1  ,
then the rate for the first period is fixed on or after the exercise date for
the period tSettle to tfltPay[k]  , and the floating leg  is worth exactly 1
paid on tSettle. If EarlyFlagFlt is 1  , then the first payment received is the
floating interest rate applied to the whole period tfltPay[k-1] to tfltPay[k]
paid on tfltPay[k]  , but the floating leg has to pay on the floating interest
that has accrued from tfltPay[k-1] to tSettle on tSettle. This is also slightly
different than 1 being paid at tSettle.
...
already-fixed current floating rate. This is supplied to the code outside the
deal structure
*/
...
typedef struct SrtCallInvFlt_ {
  ...
  double
      *strike; /* [0  ,1  ,...  ,nEx-1] fee paid by option holder to exercise */
  ...
  double *a; /* [0  ,1  ,...  ,nCpn-1] fixed coupon  , total coupon is a - gear
                * cash libor * cvg */
  double *gear;
  double *cvg;
  double *lcvg;
...
typedef struct SrtCallCapFlt_ {
  ...
  double *
      amax; /* [*  ,1  , ...  , nCpn]  , effective max handle for each coupon */
  double *
      amin; /* [*  ,1  , ...  , nCpn]  , effective min handle for each coupon */
...
can exercise. If exercised at tEx[j] the settlement date is tCpn[iSet[j]]  , the
next coupon date on or after tEx[j].

The receiver
        receives all remaining coupons C[i]  , i = iSet[j]+1  , iSet[j]+2  ,...
, n receives notional ($1) at the end date tCpn[n] pays strike[j] on the
settlement date tCpn[i0-1]

In the coupon  , rate[i] is ASSUMED to be the cash FRA rate for the period
t[i-1] to t[i]; if the rate has a basis spread  , then this must be incorporated
into the handle a[i]. Any margin or basis spread on the floating leg must be
moved into the margin on the coupon leg */
...
typedef enum LGMCalMeth_ {
  FixKappa = 0, /* Use constant kappa (=1/tau) to get G(t)  , calib on swaptions
                   or caplets for zeta(t) */
  ...
  FixExp,    /* Calib on 1 into k swaptions to get G(t)  , calib on swaptions or
                caplets for zeta */
...
typedef struct LGMCalParm_ {
  ...
  int respectExer; /* 1 = respect exer dates of deal in calib  , 0 = use regular
                      set of dates */
  ...
  long MinMonToEx; /* time to earliest allowed exercise date for 1 into k
                      swaptions */
  int keep1intok;  /* 1 = give preference to zeta from 1 into k  , if conflict
                      exists */
  ...
  /* strike information for addshift  , propshift  , fracstd  , abs strike
   * methods */
...
typedef struct LGMCalSet_ {
  ...
  long *ifirst; /* ifirst[*  ,1  ,...  ,nEx] fixed leg is i = ifirst[j]  , ...
                   , nlong[j] */
  long *nlong;
  double *cvgfirst; /* cvgfirst[*  ,1  ,...  ,nEx] cvg for first payment of
                       swaption j */
  double *FrLong;
  ...
  double
      *Rfshort; /* Rfshort[*  ,1  ,...  ,nEx] fixed rate of short swaption j */
  double *Vshort; /* Vsh[*  ,1  ,...  ,nEx] market value of short swaption j */

  /* caplets */
  int capflag; /* 1 means caps have been created */
  Date *tEnd; /* tEnd[*  ,1  ,...  ,nEx] fixing tEx[j]  , start tStart[j]  , end
                 tEnd[j] */
...
LGMErr Convolver2D(
    ...
    Date *tEx, double *zeta11, /* [0  , 1  , ...  , nEx-1]  , values of zeta11
                                  at the exercise dates */
    double *zeta12, /* [0  , 1  , ...  , nEx-1]  , values of zeta12 at the
                       exercise dates */
    double *zeta22, /* [0  , 1  , ...  , nEx-1]  , values of zeta22 at the
                       exercise dates */
    double *
        G1, /* [0  , 1  , ...  , nEx-1]  , values of G1 at the exercise dates */
    double *
        G2, /* [0  , 1  , ...  , nEx-1]  , values of G2 at the exercise dates */

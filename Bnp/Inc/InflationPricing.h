...
typedef enum {Infl3F} InflationModelType;
...
typedef struct
{
	...
	/* at Tpay we receive if floored  Coupon*(max((alpha * CPI(Tf1) + beta * CPI(Tf2))/DRIRef + gamma, Floor)) */
...
/* AssetSwap type */
/* ---------------*/
typedef enum {FlatNotional, FlatIndexedNotional, IndexedNotional, FlatDirtyNotional, IndexedDirtyNotional} InflationAssetSwapType;

...
static Err try_sigma(double sig, double *diff, void *static_data) {
  ...
  for (i = 0, sum = 0.0; i < p->nx; i++) {
    ...
    err = p->product->Payoff(p->product, p->ex_data->d, p->ex_data->d, dfs,
                             &payoff);
    if (err)
      return err;
...
static Err calib_sigma(int mkt_idx, SrtProduct *product, int isig,
                       LGMExData *ex_data, double lam, double *sig, int nx,
                       double qto_fudge) {
  Err err = NULL;
  TrySigmaData data;
  double x1, x2, f1, f2;
  ...
  /* Bracket the root */
  /*	err = num_f_zbrac(try_sigma        , &x1        , &x2        , &f1 , &f2
     , &data); if (err) return err;*/
  ...
  ex_data[isig].phi =
      data.phi_prev * ex_data[isig].exp_2lamdt +
...
Err srt_f_lgm1f_calib_to_product(char **yc_names, int n_mkts, int mkt_idx,
  ...
  for (i = 0, t1 = 0.0; i < nex; i++) {
    ...
    {
      err = product->Payoff(product, today, ex_dates[i], NULL,
                            &(ex_data[i].target));
      if (err)
        goto FREE_RETURN;
    } else /* otherwise ask the product */
    {
      err = product->RequestDfDates(product, ex_dates[i], &n_unds, &n_dfs,
                                    &dates);
      if (err)
        goto FREE_RETURN;
      if (n_unds > n_mkts) {
      ...
      for (j = 0; j < n_unds; j++) {
        ...
        if (j == mkt_idx) {
          ...
          for (k = 0; k < n_dfs[j]; k++) {
            ex_data[i].gammas[k] =
                (1.0 - exp(-lam * (dates[j][k] - ex_dates[i]) * YEARS_IN_DAY)) /
                lam;
          }
        }
      }
      ...
      /* Calculate target */
      err = product->Payoff(product, today, ex_dates[i], ex_data[i].dfs,
                            &(ex_data[i].target));
      if (err)
        goto FREE_RETURN;

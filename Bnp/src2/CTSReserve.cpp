							  ...
							  int		nvol)
{
	...
	if (ind == 0)
	{
		...
		for (j=0; j<nvol; j++)
		{
			...
			Convert_Tstar_model(&(stat_und->model),
								stat_numer_tstar);
			...
			err = cts_launch_algo (stat_model_cts, stat_und, stat_adi_arg, &stat_call, &(stat_euro[0]), NULL);
...
Err	cts_calibrate_european_and_price(	CTS_UND				und,
										CTS					market_cts,
										CTS					model_cts,
										int					for_fund,
										int					calc_fwd_iv,
...
Err	cts_get_one_time_infos(CTS_MKT		mkt,
						   CTS_ADI_ARG	adi_arg_for_midat,
						   CTS			cts,
						   double		vol_shift,
						   double		*onetime_call,
						   int			*one_time_calc,
						   double		*european_prices,
						   double		*european_prices_bumped,
						   double		*european_fees,
						   int			*nb_one_time_calc,
						   double		*most_expensive_pv,
						   CTS_IV		fwd_iv_info)
{
Err				err = NULL;
int				i, j, k, last_k;
...
double			onetime_fwd, onetime_strike, onetime_level,
				onetime_notional, onetime_vol, onetime_pv, onetime_mat;
...
double			temp;
	...
	fwd_iv_info->one_time_call = (double *) calloc(fwd_iv_info->ncall, sizeof(double));
	fwd_iv_info->one_time_coupon = (double *) calloc(fwd_iv_info->ncall, sizeof(double));
	fwd_iv_info->one_time_log_vol = (double *) calloc(fwd_iv_info->ncall, sizeof(double));
	fwd_iv_info->one_time_norm_vol = (double *) calloc(fwd_iv_info->ncall, sizeof(double));
	fwd_iv_info->one_time_strike = (double *) calloc(fwd_iv_info->ncall, sizeof(double));
	fwd_iv_info->one_time_notional = (double *) calloc(fwd_iv_info->ncall, sizeof(double));

	if (!fwd_iv_info->one_time_call || !fwd_iv_info->one_time_coupon || !fwd_iv_info->one_time_log_vol || 
		!fwd_iv_info->one_time_norm_vol || !fwd_iv_info->one_time_strike || !fwd_iv_info->one_time_notional)
	{
	...
	*most_expensive_pv = 0.0;
	...
	for (i=0; i<adi_arg_for_midat->nstp; i++)
	{
		if (adi_arg_for_midat->is_event[i])
		{
			...
			if (cts_prm->is_call)
			{
				fwd_iv_info->one_time_coupon[index_call] = cts->exo_leg->cpn[cts_prm->call->exo_idx].cpn_coupon;
				fwd_iv_info->one_time_notional[index_call] = cts->fund_leg->cpn[cts_prm->call->fund_idx].not;
				...
				if (cts_prm->eval_const_call.eval_one_time)
				{
					...
					{
						temp = cts->exo_leg->cpn[j].cpn_cvg * swp_f_df(today, cts->exo_leg->cpn[j].cpn_pay_date, mkt->yc);

						last_k = k;

						while (k < cts->fund_leg->num_cpn - 1 && cts->fund_leg->cpn[k].end_date < cts->exo_leg->cpn[j].cpn_end_date - 10)
						{
							k++;
						}

						onetime_level += temp * cts->fund_leg->cpn[k].not;
						onetime_strike += temp * cts->exo_leg->cpn[j].cpn_coupon * cts->exo_leg->cpn[j].cpn_not;
						onetime_fwd += (swp_f_df(today, cts->fund_leg->cpn[last_k].start_date, mkt->yc) - swp_f_df(today, cts->fund_leg->cpn[k].pay_date, mkt->yc)) * cts->fund_leg->cpn[k].not;
					...
					if (onetime_fwd < 0.0 && onetime_strike < 0.0 && onetime_level < 0.0)
					{
						onetime_fwd *= -1.0;
						onetime_strike *= -1.0;
						onetime_level *= -1.0;
					...
					if (onetime_type == SRT_CALL)
					{
						...
						onetime_pv = onetime_pv + (onetime_strike - onetime_fwd) * onetime_level;
						onetime_call[index_onetime] = onetime_pv * onetime_notional;
						european_prices[cts_prm->call->exo_idx] = onetime_call[index_onetime];
					}

					err = cts_optimpvol(	onetime_pv,
											onetime_fwd,
											onetime_strike,
											onetime_mat,
											onetime_level,
					...
					if (onetime_fwd > 0.0 && onetime_strike > 0.0 && onetime_vol > 0.0)
					{
						...
						{
							onetime_vol = 0.0;
							normal_vol = fwd_iv_info->one_time_norm_vol[index_call] / (onetime_fwd + onetime_strike) * 2.0;
							normal_vol = (normal_vol + vol_shift) * (onetime_fwd + onetime_strike) / 2.0;
							/*
							one_time_calc[cts_prm->call->exo_idx] = 1;
							*/

							european_prices_bumped[cts_prm->call->exo_idx] = srt_f_optblknrm(	onetime_fwd,
																								onetime_strike,
																								normal_vol,
																								onetime_mat,
																								onetime_level,
						...
						{
							european_prices_bumped[cts_prm->call->exo_idx] = srt_f_optblksch(	onetime_fwd,
																								onetime_strike,
																								onetime_vol + vol_shift,
																								onetime_mat,
																								onetime_level,
																								SRT_PUT,
																								SRT_PREMIUM);
...
Err cts_convert_to_equi_midat(	CTS_MKT	mkt,
								CTS mkt_cts,
								CTS mdl_cts)
{
Err		err = NULL;
...
long	fix;
	...
	for (i=mdl_cts->call[0].exo_idx; i<mdl_cts->exo_leg->num_cpn; i++)
	{
		...
		while (index_funding < mkt_cts->fund_leg->num_cpn  && mkt_cts->fund_leg->cpn[index_funding].pay_date < mkt_cts->exo_leg->cpn[i].cpn_pay_date + 5)
		{
		...
		{					
			fund_pv += swp_f_df(today, mkt_cts->exo_leg->cpn[i].cpn_pay_date, yc) * mkt_cts->fund_leg->cpn[index_funding - 1].not;
		}

		coupon = mkt_cts->exo_leg->cpn[i].mkt_fixed_part / mkt_cts->exo_leg->cpn[i].cpn_not - fund_pv / mkt_cts->fund_leg->cpn[index_funding - 1].not;
		
		coupon /= mkt_cts->exo_leg->cpn[i].cpn_cvg * swp_f_df(today, mkt_cts->exo_leg->cpn[i].cpn_pay_date, yc);

		/* adjust float notional */

		fund_cash_pv = swp_f_df(today, mkt_cts->exo_leg->cpn[i].cpn_start_date, yc) - swp_f_df(today, mkt_cts->exo_leg->cpn[i].cpn_pay_date, yc);
		float_notional = mkt_cts->exo_leg->cpn[i].mkt_float_part / fund_cash_pv;
		float_ratio = (mkt_cts->fund_leg->cpn[index_funding - 1].not - float_notional) / mkt_cts->fund_leg->cpn[index_funding - 1].not;
		...
		err = cts_init_cpn_payoff_for_range(0,
											0,
											-999,
											999,
											0,
...
Err	cts_calibrate_equi_midat(	CTS_MKT	mkt,
								CTS_UND	und,
								CTS		cts,
...
long	*fix_start_dates		= NULL,
		*fix_start_dates_one	= NULL,
		*fix_end_dates			= NULL,
		*fix_end_dates_one		= NULL,
	
		*fix_pay_dates			= NULL,
		*fix_pay_dates_one		= NULL,
		*float_pay_dates		= NULL,
		*float_pay_dates_one	= NULL,

		*float_start_dates		= NULL,
		*float_start_dates_one	= NULL,
		*float_end_dates		= NULL,
		*float_end_dates_one	= NULL;

double	*fix_rates				= NULL,
		*fix_rates_one			= NULL,
		*fix_notionals			= NULL,
		*fix_notionals_one		= NULL,
		*ex_fee					= NULL,		
		*ex_fee_one				= NULL,
		*ex_prices				= NULL,
		*ex_prices_one			= NULL,
		...
		*float_margins			= NULL,
		*float_margins_one		= NULL,
		*float_spreads			= NULL,	
		*float_spreads_one		= NULL,
		*float_notionals		= NULL,
		*float_notionals_one	= NULL;
...
int		next_i, last_i;
	...
	while (i >= 0 && nb_months != 1 && nb_months != 3 && nb_months != 6 && nb_months != 12)
	{
		nb_months = (int) ((cts->exo_leg->cpn[i].cpn_pay_date 
						- cts->exo_leg->cpn[i].cpn_start_date) / 30.0 + 0.5);
		i--;
	}	
	...
	while (i >= 0 && nb_months != 1 && nb_months != 3 && nb_months != 6 && nb_months != 12)
	{
		nb_months = (int) ((cts->fund_leg->cpn[i].pay_date
						- cts->fund_leg->cpn[i].start_date) / 30.0 + 0.5);
		i--;
	}	
	...
	if (!fix_start_dates || !fix_end_dates || !fix_rates || !fix_notionals || !ex_fee || !ex_bool || !ex_prices ||
		!float_start_dates || !float_end_dates || !float_margins || !float_spreads || !float_notionals ||
		!fix_start_dates_one || !fix_end_dates_one || !fix_rates_one || !fix_notionals_one || !ex_fee_one || !ex_bool || !ex_prices_one || !fix_end_dates || !fix_end_dates_one ||
		!float_start_dates_one || !float_end_dates_one || !float_margins_one || !float_spreads_one || !float_notionals_one || !float_pay_dates || !float_pay_dates_one)
	{
	...
	for (i=0; i<cts->exo_leg->num_cpn; i++)
	{
		fix_start_dates[i] = cts->exo_leg->cpn[i].cpn_start_date;
		fix_start_dates_one[i] = fix_start_dates[i];

		fix_end_dates[i] = cts->exo_leg->cpn[i].cpn_end_date;
		fix_end_dates_one[i] = fix_end_dates[i];

		fix_pay_dates[i] = cts->exo_leg->cpn[i].cpn_pay_date;
		fix_pay_dates_one[i] = fix_pay_dates[i];

		fix_rates[i] = cts->exo_leg->cpn[i].cpn_coupon;
		fix_rates_one[i] = fix_rates[i];

		fix_notionals[i] = cts->exo_leg->cpn[i].cpn_not;
	...
	if (i < cts->exo_leg->num_cpn)
	{
		...
		err = amortMidat_cpd_calib_diagonal_new(notperiod,
												mkt->yc,
												mkt->vc,
												mkt->ref,
												mkt->swap_basis,
												...
												fund_freq,
												fund_basis,
												cts->fund_leg->num_cpn,
												float_start_dates_one,
												float_end_dates_one,
												float_pay_dates_one,
												float_margins_one,
												float_spreads_one,
		...
		if (und->model.dPWTime) free (und->model.dPWTime);
		und->model.dPWTime = NULL;
		if (und->model.dSigma) free (und->model.dSigma);
	...
	und->model.dLambdaEps = (double *) calloc(und->model.iNbPWTime, sizeof(double));
...
FREE_RETURN:
	...
	if (fix_start_dates_one) free (fix_start_dates_one);

	if (fix_end_dates) free (fix_end_dates);
	if (fix_end_dates_one) free (fix_end_dates_one);

	if (fix_pay_dates) free (fix_pay_dates);
	if (fix_pay_dates_one) free (fix_pay_dates_one);

	if (fix_rates) free (fix_rates);
	if (fix_rates_one) free (fix_rates_one);

	if (fix_notionals) free (fix_notionals);
	if (fix_notionals_one) free (fix_notionals_one);

	if (ex_fee) free (ex_fee);
	if (ex_fee_one) free (ex_fee_one);
	
	if (ex_bool) free (ex_bool);
	if (ex_bool_one) free (ex_bool_one);

	if (ex_prices) free (ex_prices);
	if (ex_prices_one) free (ex_prices_one);

	if (float_start_dates) free (float_start_dates);
	if (float_start_dates_one) free (float_start_dates_one);

	if (float_end_dates) free (float_end_dates);
	if (float_end_dates_one) free (float_end_dates_one);

	if (float_pay_dates) free (float_pay_dates);
	if (float_pay_dates_one) free (float_pay_dates_one);

	if (float_margins) free (float_margins);
	if (float_margins_one) free (float_margins_one);

	if (float_spreads) free (float_spreads);
	if (float_spreads_one) free (float_spreads_one);

	if (float_notionals) free (float_notionals);
	if (float_notionals_one) free (float_notionals_one);
...
Err	cts_calc_reserve_midat(	/*	Initial results */
							...
							int					short_strike_flag,				/*	0: ATM, 1: implied digital caplet strike 2: same number of std */
							...
							int					reserve_nstpt,			/*	number of time steps for reserve calculation */
							int					reserve_nstpx,			/*	number of space steps for reserve calculation */
							double				reserve_integ_mintime,
												
							/*	Results */
							double				*switch_reserve,
							double				*onetime_reserve,
...
cts_adi_arg	*adi_arg_init = NULL,
			*adi_arg_bump = NULL,
			*adi_arg_switch = NULL;

int			free_adi_arg_init = 0,
			free_adi_arg_bump = 0,
			free_adi_arg_switch = 0;
...
double		*european_prices		= NULL,
			*european_prices_vega	= NULL,
			*european_fees			= NULL;
...
Err			err = NULL;
	...
	if (fabs(onetime_vega) < 1.0E-08 && fabs(lambda_reserve - lambda_init) < 1.0E-08)
	{
		*onetime_reserve = 0.0;
		*switch_reserve = 0.0;
		goto FREE_RETURN;
	...
	err = cts_convert_to_equi_midat(mkt,
									cts_mkt,
									cts_mdl);

	if (err) goto FREE_RETURN;

	err = cts_adjust_model_fwd_iv(	und,
									cts_mkt,
									cts_mdl,
									0,
									0,
	...
	err = translate_basis(&fund_basis, cts_mkt->fund_leg->cpn[cts_mkt->fund_leg->num_cpn-1].basis);

	if (err) goto FREE_RETURN;

	err = translate_basis(&exo_basis, cts_mkt->exo_leg->cpn[cts_mkt->exo_leg->num_cpn-1].cpn_basis);

	if (err) goto FREE_RETURN;

	notperiod = 0;
	temp_date = cts_mdl->exo_leg->cpn[cts_mdl->call[0].exo_idx].cpn_start_date;
	...
	if (!one_time_calc || !european_prices || !european_prices_vega || !european_fees ||
		!adi_arg_init || !adi_arg_bump || !adi_arg_switch)
	{
		err = "memory allocation faillure in cts_calc_reserve";
		goto FREE_RETURN;
	}

	err = cts_get_one_time_infos(mkt,
								 adi_arg_ts,
								 cts_mdl,
								 onetime_vega,
								 onetime_call,
	...
	if (recalib_euro)
	{
		err = cts_calibrate_equi_midat(	mkt,
										und,
										cts_mdl,
										exo_basis,
										fund_basis,
	...
	smessage ("Launching adi, time steps requested: %d, actual: %d", reserve_nstpt, adi_arg_init->nstp);
		
	err = cts_launch_algo (cts_mdl, und, adi_arg_init, &call_init, &(one_time_call_temp[0]), NULL);
	...
	if (fabs(onetime_vega) > 1.0E-08)
	{
		...
		smessage ("Launching adi, time steps requested: %d, actual: %d", reserve_nstpt, adi_arg_bump->nstp);
			
		err = cts_launch_algo (cts_mdl, und, adi_arg_bump, &call_new, &(one_time_call_temp[0]), NULL);
	...
	if (fabs(lambda_reserve - lambda_init) > 1.0E-08 && nb_one_time_calc > 1)
	{
		...
		smessage ("Launching adi, time steps requested: %d, actual: %d", reserve_nstpt, adi_arg_switch->nstp);
			
		err = cts_launch_algo (cts_mdl, und, adi_arg_switch, &call_new, &(one_time_call_temp[0]), NULL);
...
FREE_RETURN:
	...
	if (adi_arg_init)
	{
		if (free_adi_arg_init) cts_free_adi_arg(adi_arg_init);
		free(adi_arg_init);
	}

	if (adi_arg_bump)
	{
		if (free_adi_arg_bump) cts_free_adi_arg(adi_arg_bump);
		free(adi_arg_bump);
	}

	if (adi_arg_switch)
	{
		if (free_adi_arg_switch) cts_free_adi_arg(adi_arg_switch);
		free(adi_arg_switch);
	}	

	if (one_time_calc) free (one_time_calc);
	if (european_prices) free (european_prices);
	if (european_prices_vega) free (european_prices_vega);
	if (european_fees) free (european_fees);
...
Err	cts_calc_reserve_timeswap(	/*	Initial results */
								...
								int					fix_smile,
								int					smile_calib_months,				/* 0: co-terminal swaption, otherwise underlyings with required nb months */
								LGMSV_CalibParams	*lgmsv_calib_params,			/*	shift on Rho SV */
								double				min_time,
								int					skip_last,						/*	If 1, the last option is disregarded and the forward volatility is flat from option n-1 */
								double				min_fact,						/*	Maximum down jump on variance */
								double				max_fact,						/*	Maximum up jump on variance */
								...
								int					one_time_index,			/*	One Time callable used for reserve calculation */
								
								int					reserve_nstpt,			/*	number of time steps for reserve calculation */
								int					reserve_nstppsi,		/*	number of psi steps for reserve calculation */
								int					reserve_nstpx,			/*	number of space steps for reserve calculation */
								int					reserve_nstpvol,		/*	number of vol steps for reserve calculation */								
								double				reserve_integ_mintime,
...
Err			err = NULL;
	...
	/*	Step 2: Pricing of the switch reserve by recalibrating european time swap */
	/*	************************************************************************* */	
	...
	err = cts_calibrate_european_and_price(	und,
											cts_mkt,
											cts_mdl,
											for_fund,
											calc_fwd_iv,
...
FREE_RETURN:	
	...
	if (adi_arg)
	{
		if (free_adi_arg) cts_free_adi_arg(adi_arg);
		free(adi_arg);
	}
...
Err	cts_calc_reserve(	/*	Initial results */
						...
						int					reserve_nstpt,			/*	number of time steps for reserve calculation */
						int					reserve_nstpx,			/*	number of space steps for reserve calculation */
						int					reserve_nstppsi,		/*	number of time steps for reserve calculation */
						int					reserve_nstpvol,		/*	number of space steps for reserve calculation */
						double				reserve_integ_mintime,
...
int			free_adi_arg_lgm = 0,
			free_adi_arg_mc = 0;

cts_adi_arg	*adi_arg_lgm	= NULL,
			*adi_arg_mc		= NULL;

int			init_nsmilepar;
double		*init_smilepartime	= NULL,
			*init_alpha			= NULL,
			*init_rho			= NULL,
			*init_ldaeps		= NULL,
			*init_rho2			= NULL;
...
LGMSV_CalibParams	*local_lgm_sv_calib_params;
	...
	if (!local_lgm_sv_calib_params || !init_smilepartime || !init_alpha || !init_rho || !init_ldaeps || !init_rho2)
	{
		err = "Memory allocation faillure in cts_calc_reserve";
		goto FREE_RETURN;
	}
	...
	switch (reserve_method)
	{
		...
		case 0:		
		{
			...
			if (lgm_reserve)
			{
				...
				/* Launch the adi */
				smessage ("Launching adi, time steps requested: %d, actual: %d", reserve_nstpt, adi_arg_lgm->nstp);
						
				err = cts_launch_algo (cts_mdl, und, adi_arg_lgm, &call, &(onetime_call[0]), NULL);
			...
			if (midat_reserve)
			{
				err = 	cts_calc_reserve_midat(	cts_mkt,
												cts_mdl,
												und,
												adi_arg_init,
												call,
			...
			{
				cts_calc_reserve_timeswap(	cts_mkt,
											cts_mdl,
											und,
											call,
											onetime_call,
											adi_arg_init->has_one_time,
											for_fund,
											nsmilepar,
											smilepartime,
											alphaeps,
			...
			if (save_extra_infos)
			{
				...
				ConvertTS_LGMSV_to_LGM(	und->model.iNbPWTime,
										und->model.dPWTime,
										und->model.dSigma,
										und->model.dLambdaX,
										und->model.dTStar);
				...
				extra_infos->extra_infos = dmatrix(0, extra_infos->nrow - 1, 0, extra_infos->ncol - 1);
				...
				for (i=0; i<und->model.iNbPWTime; i++)
				{
					extra_infos->extra_infos[i+1][0] = mkt->today + und->model.dPWTime[i] * DAYS_IN_YEAR;
		...
		{						
			...
			else
			{
				...
				if (recalc_one_factor)
				{
					...
					if (free_adi_arg_mc) cts_free_adi_arg(adi_arg_mc);
					free_adi_arg_mc = 0;
				}
				else
				{
...
FREE_RETURN:
	...
	if (adi_arg_lgm)
	{
		if (free_adi_arg_lgm) cts_free_adi_arg(adi_arg_lgm);
		free(adi_arg_lgm);
	}
	...
	if (local_lgm_sv_calib_params) free(local_lgm_sv_calib_params);

	if (init_smilepartime) free(init_smilepartime);
	if (init_alpha) free(init_alpha);
	if (init_rho) free(init_rho);
	if (init_ldaeps) free(init_ldaeps);
	if (init_rho2) free(init_rho2);

	return err;
}

void shift_LGMSV_model(	long		today,
						char		*yc,
						LGMSV_MODEL	model,
						double		shift,
						double		log_norm)
...
double	short_rate;
	...
	else
	{
		for (i=0; i<model->iNbPWTime; i++)
		{
			short_rate = swp_f_zr(today + model->dPWTime[i] * DAYS_IN_YEAR, today + model->dPWTime[i] * DAYS_IN_YEAR + 1, yc);
	...
	ConvertTS_LGM_to_LGMSV(	model->iNbPWTime,
							model->dPWTime,
							model->dSigma,
							model->dLambdaX,
							model->dTStar,

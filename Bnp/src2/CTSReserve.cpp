      ...
      if (cts_prm->is_call) {
        ...
        if (cts_prm->eval_const_call.eval_one_time) {
          ...
          for (j = cts_prm->call->exo_idx; j < cts->exo_leg->num_cpn; j++) {
            ...
            onetime_strike += temp * cts->exo_leg->cpn[j].cpn_coupon *
                              cts->exo_leg->cpn[j].cpn_not;
            onetime_fwd +=
...
Err cts_convert_to_equi_midat(CTS_MKT mkt, CTS mkt_cts, CTS mdl_cts) {
  ...
  for (i = mdl_cts->call[0].exo_idx; i < mdl_cts->exo_leg->num_cpn; i++) {
    ...
    fund_cash_pv =
        swp_f_df(today, mkt_cts->exo_leg->cpn[i].cpn_start_date, yc) -
        swp_f_df(today, mkt_cts->exo_leg->cpn[i].cpn_pay_date, yc);
...
Err cts_calibrate_equi_midat(CTS_MKT mkt, CTS_UND und, CTS cts,
  ...
  while (i >= 0 && nb_months != 1 && nb_months != 3 && nb_months != 6 &&
    ...
    nb_months = (int)((cts->exo_leg->cpn[i].cpn_pay_date -
                       cts->exo_leg->cpn[i].cpn_start_date) /
                          30.0 +
                      0.5);
    i--;
  }
...
Err cts_calc_reserve_midat(/*	Initial results */
  ...
  err = translate_basis(
      &exo_basis,
      cts_mkt->exo_leg->cpn[cts_mkt->exo_leg->num_cpn - 1].cpn_basis);

...
static double stat_price[MAX_EURO], *stat_target, stat_sensi[MAX_EURO][MAX_EURO];

static double stat_call, stat_numer_tstar, stat_euro[MAX_EURO];

Err static_cts_price_european_r2(double index, double vol[], double* price, double* gradient, int nvol)
{
    static int    i, j, ind;
    static double sum2;
    Err           err = NULL;
    ...
    if (ind == 0)
    {
        ...
        for (j = 0; j < nvol; j++)
        {
            ...
            err = cts_calc_model_fwd_iv(
                stat_und, stat_model_cts, stat_calc_fwd_iv, stat_adj_fee, stat_num_params);
            ...
            err = cts_launch_algo(
                stat_model_cts, stat_und, stat_adi_arg, &stat_call, &(stat_euro[0]), NULL);
    ...
    double*          implied_vol)
{
    ...
    *implied_vol = 0.0;
    ...
    intrinsic = optval(fwd_price, strike, NULL_VOL, mat, disc, call_put, PREMIUM, lognormal_normal);
    ...
    prem_up = optval(fwd_price, strike, vol_up, mat, disc, call_put, PREMIUM, lognormal_normal);

    prem_down = optval(fwd_price, strike, vol_down, mat, disc, call_put, PREMIUM, lognormal_normal);
    ...
    if ((prem_down - premium) * (prem_up - premium) > 0.0)
    {
        ...
        if (prem_up < premium)
        {
            ...
            while (i < VOL_MULT_ITER && prem_up < premium)
            {
                vol_up *= VOL_MULT;
                prem_up = optval(
                    fwd_price, strike, vol_up, mat, disc, call_put, PREMIUM, lognormal_normal);
                i++;
            }
    ...
    prem_new =
        optval(fwd_price, strike, vol_middle, mat, disc, call_put, PREMIUM, lognormal_normal);
    ...
    CTS_IV      fwd_iv_info)
{
    ...
    double onetime_fwd, onetime_strike, onetime_level, onetime_notional, onetime_vol, onetime_pv,
        onetime_mat;
        ...
        if (adi_arg_for_midat->is_event[i])
        {
            ...
            if (cts_prm->is_call)
            {
                fwd_iv_info->one_time_coupon[index_call] =
                    cts->exo_leg->cpn[cts_prm->call->exo_idx].cpn_coupon;
                ...
                if (cts_prm->eval_const_call.eval_one_time)
                {
                    ...
                    {
                        temp = cts->exo_leg->cpn[j].cpn_cvg *
                               swp_f_df(today, cts->exo_leg->cpn[j].cpn_pay_date, mkt->yc);

                        last_k = k;

                        while (k < cts->fund_leg->num_cpn - 1 &&
                               cts->fund_leg->cpn[k].end_date <
                                   cts->exo_leg->cpn[j].cpn_end_date - 10)
                        {
                            k++;
                        }
                        ...
                        onetime_strike +=
                            temp * cts->exo_leg->cpn[j].cpn_coupon * cts->exo_leg->cpn[j].cpn_not;
                        onetime_fwd +=
                            (swp_f_df(today, cts->fund_leg->cpn[last_k].start_date, mkt->yc) -
                    ...
                    if (onetime_fwd > 0.0 && onetime_strike > 0.0 && onetime_vol > 0.0)
                    {
                        ...
                        if (err && fwd_iv_info->one_time_norm_vol[index_call] > 1.0E-10)
                        {
                            ...
                            normal_vol =
                                (normal_vol + vol_shift) * (onetime_fwd + onetime_strike) / 2.0;
                            /*
                            one_time_calc[cts_prm->call->exo_idx] = 1;
                            */
    ...
    for (i = mdl_cts->call[0].exo_idx; i < mdl_cts->exo_leg->num_cpn; i++)
    {
        ...
        while (index_funding < mkt_cts->fund_leg->num_cpn &&
               mkt_cts->fund_leg->cpn[index_funding].pay_date <
                   mkt_cts->exo_leg->cpn[i].cpn_pay_date + 5)
        {
        ...
        {
            fund_pv += swp_f_df(today, mkt_cts->exo_leg->cpn[i].cpn_pay_date, yc) *
                       mkt_cts->fund_leg->cpn[index_funding - 1].not ;
        }

        coupon = mkt_cts->exo_leg->cpn[i].mkt_fixed_part / mkt_cts->exo_leg->cpn[i].cpn_not -
                 fund_pv / mkt_cts->fund_leg->cpn[index_funding - 1].not ;

        coupon /= mkt_cts->exo_leg->cpn[i].cpn_cvg *
                  swp_f_df(today, mkt_cts->exo_leg->cpn[i].cpn_pay_date, yc);

        /* adjust float notional */

        fund_cash_pv = swp_f_df(today, mkt_cts->exo_leg->cpn[i].cpn_start_date, yc) -
                       swp_f_df(today, mkt_cts->exo_leg->cpn[i].cpn_pay_date, yc);
        float_notional = mkt_cts->exo_leg->cpn[i].mkt_float_part / fund_cash_pv;
        float_ratio    = (mkt_cts->fund_leg->cpn[index_funding - 1].not -float_notional) /
                      mkt_cts->fund_leg->cpn[index_funding - 1].not ;
        ...
        err = cts_init_cpn_payoff_for_range(0, 0, -999, 999, 0, 9999999999, 1.0, 10.0E-04, cpn);
...
{
    Err   err             = NULL;
    long *fix_start_dates = NULL, *fix_start_dates_one = NULL, *fix_end_dates = NULL,
         *fix_end_dates_one = NULL,

         *fix_pay_dates = NULL, *fix_pay_dates_one = NULL, *float_pay_dates = NULL,
         *float_pay_dates_one = NULL,

         *float_start_dates = NULL, *float_start_dates_one = NULL, *float_end_dates = NULL,
         *float_end_dates_one = NULL;

    double *fix_rates = NULL, *fix_rates_one = NULL, *fix_notionals = NULL,
           *fix_notionals_one = NULL, *ex_fee = NULL, *ex_fee_one = NULL, *ex_prices = NULL,
           *ex_prices_one = NULL,

           *float_margins = NULL, *float_margins_one = NULL, *float_spreads = NULL,
           *float_spreads_one = NULL, *float_notionals = NULL, *float_notionals_one = NULL;
    ...
    {
        nb_months =
            (int)((cts->exo_leg->cpn[i].cpn_pay_date - cts->exo_leg->cpn[i].cpn_start_date) / 30.0 + 0.5);
        i--;
    }
    ...
    {
        nb_months =
            (int)((cts->fund_leg->cpn[i].pay_date - cts->fund_leg->cpn[i].start_date) / 30.0 + 0.5);
        i--;
    }
    ...
    /* Recopy Fixed Leg */
    fix_start_dates     = (long*)calloc(cts->exo_leg->num_cpn, sizeof(long));
    fix_start_dates_one = (long*)calloc(cts->exo_leg->num_cpn, sizeof(long));
    fix_end_dates       = (long*)calloc(cts->exo_leg->num_cpn, sizeof(long));
    fix_end_dates_one   = (long*)calloc(cts->exo_leg->num_cpn, sizeof(long));
    fix_pay_dates       = (long*)calloc(cts->exo_leg->num_cpn, sizeof(long));
    fix_pay_dates_one   = (long*)calloc(cts->exo_leg->num_cpn, sizeof(long));

    fix_rates         = (double*)calloc(cts->exo_leg->num_cpn, sizeof(double));
    fix_rates_one     = (double*)calloc(cts->exo_leg->num_cpn, sizeof(double));
    fix_notionals     = (double*)calloc(cts->exo_leg->num_cpn, sizeof(double));
    fix_notionals_one = (double*)calloc(cts->exo_leg->num_cpn, sizeof(double));

    ex_fee        = (double*)calloc(cts->exo_leg->num_cpn, sizeof(double));
    ex_fee_one    = (double*)calloc(cts->exo_leg->num_cpn, sizeof(double));
    ex_bool       = (int*)calloc(cts->exo_leg->num_cpn, sizeof(int));
    ex_bool_one   = (int*)calloc(cts->exo_leg->num_cpn, sizeof(int));
    ex_prices     = (double*)calloc(cts->exo_leg->num_cpn, sizeof(double));
    ex_prices_one = (double*)calloc(cts->exo_leg->num_cpn, sizeof(double));

    float_start_dates     = (long*)calloc(cts->fund_leg->num_cpn, sizeof(long));
    float_start_dates_one = (long*)calloc(cts->fund_leg->num_cpn, sizeof(long));
    float_end_dates       = (long*)calloc(cts->fund_leg->num_cpn, sizeof(long));
    float_end_dates_one   = (long*)calloc(cts->fund_leg->num_cpn, sizeof(long));
    float_pay_dates       = (long*)calloc(cts->fund_leg->num_cpn, sizeof(long));
    float_pay_dates_one   = (long*)calloc(cts->fund_leg->num_cpn, sizeof(long));

    float_margins       = (double*)calloc(cts->fund_leg->num_cpn, sizeof(double));
    float_margins_one   = (double*)calloc(cts->fund_leg->num_cpn, sizeof(double));
    float_spreads       = (double*)calloc(cts->fund_leg->num_cpn, sizeof(double));
    float_spreads_one   = (double*)calloc(cts->fund_leg->num_cpn, sizeof(double));
    float_notionals     = (double*)calloc(cts->fund_leg->num_cpn, sizeof(double));
    float_notionals_one = (double*)calloc(cts->fund_leg->num_cpn, sizeof(double));

    if (!fix_start_dates || !fix_end_dates || !fix_rates || !fix_notionals || !ex_fee || !ex_bool ||
        !ex_prices || !float_start_dates || !float_end_dates || !float_margins || !float_spreads ||
        !float_notionals || !fix_start_dates_one || !fix_end_dates_one || !fix_rates_one ||
        !fix_notionals_one || !ex_fee_one || !ex_bool || !ex_prices_one || !fix_end_dates ||
        !fix_end_dates_one || !float_start_dates_one || !float_end_dates_one ||
        !float_margins_one || !float_spreads_one || !float_notionals_one || !float_pay_dates ||
        !float_pay_dates_one)
    {
...
Err cts_calc_reserve_midat(/*	Initial results */
                           ...
                           int    short_strike_flag, /*	0: ATM, 1: implied digital caplet strike 2:
                                                        same number of std */
                           ...
                           int reserve_nstpt, /*	number of time steps for reserve calculation
                                               */
                           int reserve_nstpx, /*	number of space steps for reserve
                                                 calculation */
                           ...
                           CTS_EXTRA_INFOS extra_infos)
{
    ...
    cts_adi_arg *adi_arg_init = NULL, *adi_arg_bump = NULL, *adi_arg_switch = NULL;
    ...
    double *european_prices = NULL, *european_prices_vega = NULL, *european_fees = NULL;
    ...
    err =
        translate_basis(&fund_basis, cts_mkt->fund_leg->cpn[cts_mkt->fund_leg->num_cpn - 1].basis);

    if (err)
        goto FREE_RETURN;

    err =
        translate_basis(&exo_basis, cts_mkt->exo_leg->cpn[cts_mkt->exo_leg->num_cpn - 1].cpn_basis);
    ...
    one_time_calc        = (int*)calloc(cts_mdl->exo_leg->num_cpn, sizeof(int));
    european_prices      = (double*)calloc(cts_mdl->exo_leg->num_cpn, sizeof(double));
    european_prices_vega = (double*)calloc(cts_mdl->exo_leg->num_cpn, sizeof(double));
    european_fees        = (double*)calloc(cts_mdl->exo_leg->num_cpn, sizeof(double));

    adi_arg_init   = (cts_adi_arg*)calloc(1, sizeof(cts_adi_arg));
    adi_arg_bump   = (cts_adi_arg*)calloc(1, sizeof(cts_adi_arg));
    adi_arg_switch = (cts_adi_arg*)calloc(1, sizeof(cts_adi_arg));
    ...
    smessage(
        "Launching adi, time steps requested: %d, actual: %d", reserve_nstpt, adi_arg_init->nstp);
    ...
    if (fabs(onetime_vega) > 1.0E-08)
    {
        ...
        err =
            cts_launch_algo(cts_mdl, und, adi_arg_bump, &call_new, &(one_time_call_temp[0]), NULL);
    ...
    if (fabs(lambda_reserve - lambda_init) > 1.0E-08 && nb_one_time_calc > 1)
    {
        ...
        err = cts_launch_algo(
            cts_mdl, und, adi_arg_switch, &call_new, &(one_time_call_temp[0]), NULL);
...
Err cts_calc_reserve_timeswap(/*	Initial results */
                              ...
                              int euro_nb_iter,   /*	Number of Levenberg iteration for
                                                     calibration of european */
                              int one_time_index, /*	One Time callable used for reserve
                                                     calculation */
...
Err cts_calc_reserve(/*	Initial results */
                     ...
                     char* short_basis,
                     int   fix_smile,
                     int smile_calib_months, /* 0: co-terminal swaption, otherwise underlyings with
                                                required nb months */
                     LGMSV_CalibParams* lgmsv_calib_params, /*	shift on Rho SV */
                     double             min_time,
                     int skip_last,    /*	If 1, the last option is disregarded and the forward
                     ...
                     int    keep_first,
                     int    long_strike_flag,  /*	0: ATM 1: Coupon 2: Eq (PV/Lvl) */
                     int    short_strike_flag, /*	0: ATM, 1: implied digital caplet strike 2: same
                                                  number of std */
                     ...
                     int recalib_european,  /*	If set to 1, then european are recalibrated when
                                               changing lambda */
                     ...
                     CTS_EXTRA_INFOS extra_infos)
{
    ...
    int     init_nsmilepar;
    double *init_smilepartime = NULL, *init_alpha = NULL, *init_rho = NULL, *init_ldaeps = NULL,
           *init_rho2 = NULL;
    ...
    init_smilepartime = (double*)calloc(init_nsmilepar, sizeof(double));
    init_alpha        = (double*)calloc(init_nsmilepar, sizeof(double));
    init_rho          = (double*)calloc(init_nsmilepar, sizeof(double));
    init_ldaeps       = (double*)calloc(init_nsmilepar, sizeof(double));
    init_rho2         = (double*)calloc(init_nsmilepar, sizeof(double));

    local_lgm_sv_calib_params = (LGMSV_CalibParams*)calloc(1, sizeof(LGMSV_CalibParams));

    if (!local_lgm_sv_calib_params || !init_smilepartime || !init_alpha || !init_rho ||
        !init_ldaeps || !init_rho2)
    {
        err = "Memory allocation faillure in cts_calc_reserve";
        goto FREE_RETURN;
    }
    ...
    case 0:
    {
        ...
        adi_arg_lgm = (cts_adi_arg*)calloc(1, sizeof(cts_adi_arg));
        ...
        if (lgm_reserve)
        {
            ...
            err = cts_calc_model_fwd_iv(und, cts_mdl, calc_fwd_iv, adj_fee, NumerParams);
            ...
            err = cts_adjust_model_fwd_iv(
                und, cts_mkt, cts_mdl, for_fund, calc_fwd_iv, adj_fee, 0, 0, NULL);
    ...
    case 1:
    {
        ...
        adi_arg_mc = (cts_adi_arg*)calloc(1, sizeof(cts_adi_arg));
        ...
        else
        {
            ...
            err = cts_adjust_model_fwd_iv(
                und, cts_mkt, cts_mdl, for_fund, calc_fwd_iv, adj_fee, 1, 0, NULL);
        ...
        if (fabs(onetime_vega) > 1.0E-08)
        {
            ...
            err = cts_adjust_model_fwd_iv(
                und, cts_mkt, cts_mdl, for_fund, calc_fwd_iv, adj_fee, adi_arg->pde_or_mc, 0, NULL);
...
void shift_LGMSV_model(long today, char* yc, LGMSV_MODEL model, double shift, double log_norm)
{
    int    i;
    double short_rate;
    ...
    else
    {
        ...
        {
            short_rate = swp_f_zr(
                today + model->dPWTime[i] * DAYS_IN_YEAR,
                today + model->dPWTime[i] * DAYS_IN_YEAR + 1,
                yc);

...
char *SrtGrfn3DFXQuadTree(char *und3dfx, double beta, double gamma, double sig0,
  ...
  double dom_lam, for_lam, corr_dom_for, corr_dom_fx, corr_for_fx;
  double spot_fx;
  char *dom_yc, *for_yc;
  ...
  err = srt_f_get_corr_from_CorrList(sCorrlist, domname, forname, 1.0,
                                     &corr_dom_for);

  err = srt_f_get_corr_from_CorrList(sCorrlist, domname, und3dfx, 1.0,
                                     &corr_dom_fx);

  err = srt_f_get_corr_from_CorrList(sCorrlist, forname, und3dfx, 1.0,
                                     &corr_for_fx);
  ...
  // err = allocate_local_model_cust(NB_POINTS        , nstp        ,
  // &model_func        , &model_param);
  if (err) {
    goto FREE_RETURN;
  }

  err = fill_local_model(model_func, model_param, nstp, time, alpha2, beta2,
                         gamma2, sig0, fx_fwd0);
  // err = fill_local_model_cust(model_func        , model_param        ,
  // NB_POINTS        , time
  //       , nstp        , alpha2        , beta2        , gamma2        , sig0
  //       , fx_fwd0);
  if (err) {
    goto FREE_RETURN;
  }
  ...
  */
  ...
  for (i = nstp - 1; i >= 0; i--) {
    if (j >= 0 && fabs(date[i] - xStr.dts[j]) < 1.0e-04) {
      ...
      if (barrier[j] > 1.0e-08) {
        next_bar = model_func->Z_to_U(
            i, model_func->S_to_Z(i, barrier[j], fx_fwd0[i], model_param),
            model_param);
      ...
      if (barrier[j - 1] > 1.0e-08) {
        next_bar = model_func->Z_to_U(
            i, model_func->S_to_Z(i, barrier[j - 1], fx_fwd0[i], model_param),
            model_param);
    ...
    if (fabs(next_bar) > 1.0e-08) {
      ...
      bar_lvl[i] = model_func->Z_to_U(
          i, model_func->S_to_Z(i, fx_fwd0[i], fx_fwd0[i], model_param),
          model_param);
    }
  ...
  is_bar[1] = 0;
  bar_lvl[1] = model_func->Z_to_U(
      1, model_func->S_to_Z(i, fx_fwd0[1], fx_fwd0[1], model_param),
      model_param);
  bar_cl[1] = -1;
  ...
  *prod_val = (double *)calloc(*num_prod + 1, sizeof(double));
  ...
  err = treeQuad_main_3dfx(
      ...
      dom_var, for_ifr, for_fwd, for_var, fx_fwd, fx_fwd0, fx_var, void_prm,
      is_event, bar_lvl, bar_cl, is_bar, dom_lam, for_lam, corr_dom_for,
      corr_dom_fx, corr_for_fx, spot_fx, dom_yc, for_yc, model_func,
      model_param, grfn_payoff_4_3dfxQuad_tree, *num_prod, discount, *prod_val);
...
char *SrtGrfn3DFXQuadMc(char *und3dfx, double beta, double gamma, double sig0,
  ...
  if (!sig_dom || !sig_for || !sig_fx || !dom_vol || !for_vol || !fx_vol ||
      !fx_fwd || !corr_dom_for || !corr_dom_fx || !corr_for_fx || !dom_ifr ||
      !dom_phi || !for_ifr || !for_phi) {
  ...
  for (i = 0; i < nstp; i++) {
    fx_fwd[i] =
        (swp_f_zr(today, date[i], dom_yc) - swp_f_zr(today, date[i], for_yc)) *
        time[i];
  ...
  *prod_val = dmatrix(0, num_col - 1, 0, 1);
  ...
  err = mcLocal_main_3dfx(
      ...
      dom_vol, dom_phi, for_ifr, for_vol, for_phi, fx_fwd, fx_vol, model_func,
      model_param, void_prm, has_evt,
      /*	Model data */
      dom_lam, for_lam, correl_dom_for, correl_dom_fx, correl_for_fx,
      /*	Market data */

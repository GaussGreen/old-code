...
static double local_price_fct(double irr, int i) {
  ...
  disc1 = 1 / (1 + irr / (double)glob_sdp[i].cxxompd);

  if (glob_sdp[i].basis_code == BASIS_ACT_USD) {
    dr = day_count_date(glob_dl[i].date[0], glob_dl[i].date[1],
                        glob_sdp[i].basis_code);
    dr = dr / day_count_date(glob_dl[i].prev, glob_dl[i].date[1],
                             glob_sdp[i].basis_code);
    dr = dr / glob_sdp[i].cxxompd;
  } else {
    dr = coverage(glob_dl[i].date[0], glob_dl[i].date[1],
                  glob_sdp[i].basis_code);
  }

  if (disc1 != 1.0) {
    price =
        (NI_cpn[i] / (double)glob_sdp[i].cxxompd) *
            (pow(disc1, glob_dl[i].len - (glob_dl[i].type == BROKEN)) - disc1) /
            (disc1 - 1) +
        redemption *
            pow(disc1, glob_dl[i].len - 1 - (glob_dl[i].type == BROKEN));
  } else {
    price = (NI_cpn[i] / (double)glob_sdp[i].cxxompd) *
                (glob_dl[i].len - 1 - (glob_dl[i].type == BROKEN)) +
            redemption;
  }

  if (glob_dl[i].type == BROKEN) {
    price = (price + NI_cpn[i] / (double)glob_sdp[i].cxxompd) *
            pow(disc1, glob_sdp[i].cxxompd * dr);
  }
...
static double bond_opt_integrand(double logy) {
  ...
  if (NI_ext_flg != 0) {
    dens -= INV_SQRT_TWO_PI * exp(NI_ext_adj2 - (logy - NI_ext_adj1) *
                                                    (logy - NI_ext_adj1) / 2.0);
  }
...
SrtErr swp_f_bndcms(Date today, Date fixdt, Date paydt, SwapDP *sdp, double cpn,
                    double vol, double fwd, SrtDiffusionType lognrm_nrm,
                    double *answer) {
  SrtErr err;
  double strike = 0;
  ...
  sdp->end = add_unit(sdp->start, 12 / (int)sdp->compd * sdp->nfp, SRT_MONTH,
  ...
  err = swp_f_optbndyld(today, fixdt, sdp, cpn, strike, vol, x0, ext_barrier,
                        x0, call_put, ext_flg, greek, lognrm_nrm, &y0);
  if (err)
    return err;

  y0 -= fwd_pr;

  err = swp_f_optbndyld(today, fixdt, sdp, cpn, strike, vol, x1, ext_barrier,
                        x1, call_put, ext_flg, greek, lognrm_nrm, &y1);
  if (err)
    return err;
 ...
 * payment date.
 */
SrtErr swp_f_optbndyld(Date today, Date fixdt, SwapDP *sdp, double cpn,
                       double strike, double vol, double fwd,
                       double ext_barrier, double spot_yld,
                       SrtCallPutType call_put, int ext_flg, SrtGreekType greek,
                       SrtDiffusionType lognrm_nrm, double *answer) {
  double (*f)(double);
  ...
  if (ext_flg != 0) {
    if (lognrm_nrm == SRT_LOGNORMAL) {
      ...
      NI_ext_adj2 = (2.0 * (NI_logy0 - log(spot_yld)) / (NI_sig * NI_sig) *
                     log(ext_barrier / spot_yld));
      NI_ext_flg = ext_flg;
    } else {
      NI_ext_adj1 = 2 * (ext_barrier - spot_yld) / NI_sig;
      NI_ext_adj2 = (2.0 * (NI_logy0 - spot_yld) / (NI_sig * NI_sig) *

...
Err srt_f_optbetavoltoblkvol(double forward, double strike, double betavol,
  ...
  *bsvol = betavol * pow(middle, beta - 1.0) *
...
Err srt_f_optbetastochvoltoblkvol(double forward, double strike, double betavol,
  ...
  zz = vovol / betavol * log(forward / strike) *
       pow(forward * strike, (1.0 - beta) / 2.);

  xx = log((sqrt(1.0 - 2.0 * rho * zz + zz * zz) + zz - rho) / (1.0 - rho));

  /* First term */
  first_term =
      pow(forward * strike, (1.0 - beta) / 2.0) *
      (1.0 + 1.0 / 24.0 * pow((1.0 - beta) * log(forward / strike), 2.0));

  /* Second Term */
  second_term = pow(1.0 - beta, 2.0) / 24.0;
  second_term += 0.25 * rho * beta * vovol / betavol *
                 pow(forward * strike, (1.0 - beta) / 2.0);
  second_term += (2.0 - 3.0 * rho * rho) / 24.0 * vovol * vovol /
                 (betavol * betavol) * pow(forward * strike, (1.0 - beta));
  second_term *= betavol * betavol * mat / pow(forward * strike, (1.0 - beta));
  second_term += 1.0;

  /* Third Term */
  if (fabs(zz) < 0.00005) {
    third_term = betavol / (1 + 0.5 * zz * rho +
                            1. / 6. * zz * zz * (3. * rho * rho - 1.));
  } else
    third_term = betavol * (zz / xx);

  /* The equivalent BlackScholes betastoch vol (constant elasticity and stoch
   * vol) */

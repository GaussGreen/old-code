...
/*                                                                            */
/*      AUTHORS:        G.Amblard        , E.Auld        , K.Chau        ,
 * J.Malhi        , A. Sahuguet
 */
/*                                                                            */
/******************************************************************************/
...
static SRT_Boolean is_within(Ddate today, double left, double date_or_time,
                             double right);
...
However        , when we deal with sigma shifts        , the volatility at each
date may be different from one shifted price to another. This would lead to
different values of the u-parameter ( rate spacing between the nodes ). see
LGM_calc_delta_x() function We want the tree to be frozen: the u is chosen
constant for each shifted price. The first time the function is called        ,
it's for the premium - the real price       , without any shift. The value u
computed then is stored and next calls to this function will return this
particular u instead of recomputing it. The first time        , *old_u = -1.
*/

static Err srt_f_vegashiftlgm1dtrelim(SrtStpPtr stp, SrtTreStpInf *maxtrinf,
                                      double *old_u);
...
static Err srt_f_vegashiftlgministp(
    SrtUndPtr und, SrtUndInfo *undinfo, SrtStpPtr stp, TermStruct *ts,
    double sigma_bucket_start, double sigma_bucket_end, double sigma_shift,
    int sigma_shift_type, double tau_bucket_start, double tau_bucket_end,
    double tau_shift, int tau_shift_type);
...
Err srt_f_vegashifttrelgm1d(SrtUndPtr und, SrtGrfnParam *grfnparam,
                            SrtStpPtr stp, GrfnDeal *gd, EvalEventFct evalcf,
                            SrtIOStruct *iolist, SrtUndInfo *und_info) {
  ...
  /* 3D arrays: r dimension        , grfn_cols dimension        , shift
   * dimension */
  ...
  /* current and constant value for rate spacing in the tree        , for the
  prices and all the schifted prices */
  ...
  for (io_request = iolist->head; io_request != NULL;
    ...
    /* find out whether or not the requested value can be computed by this tree
     * function */
    ...
    if (srt_f_vegatrelgm1d_requestOK(io_request_val_p->type) == SRT_NO) {
      ...
      case IO_SIGMA_SHIFT:
        my_pointer =
            (SrtStpPtr)(((SrtIOVal *)(io_request->element->val.pval))->pval);

        if (err = srt_f_vegashiftirministp(
                my_pointer, und, und_info, io_request_val_p->bucket_start,
                io_request_val_p->bucket_end, io_request_val_p->shift_value,
                io_request_val_p->shift_type, -1, -1, 0, SH_NONE)) {
          return err;
        }
        break;
      case IO_TAU_SHIFT:
        ...
        if (err = srt_f_vegashiftirministp(
                my_pointer, und, und_info, -1, -1, 0, SH_NONE,
                io_request_val_p->bucket_start, io_request_val_p->bucket_end,
                io_request_val_p->shift_value, io_request_val_p->shift_type)) {
          return err;
        }
        break;
      default:
      ...
      structure. For this structure        , the parameters have been computed
      with the shift corresponding to the request */
  ...
  prev_assets = f3tensor(0, xtra_calc_num - 1, maxtrinf.min_x_index,
                         maxtrinf.max_x_index, 0, grfnparam->node_dim - 1);
  cur_assets = f3tensor(0, xtra_calc_num - 1, maxtrinf.min_x_index,
                        maxtrinf.max_x_index, 0, grfnparam->node_dim - 1);
  ...
  if (!prev_assets || !cur_assets || !prev_x || !cur_x) {
    return serror("allocation failure srt_f_vegatrelgm.cxx");
  }
  ...
  for (stp = bot; stp != NULL; stp = stp->prev) {
    ...
    for (i = trinf->min_x_index; i <= trinf->max_x_index; i++) {
      ...
      sam_get(node.cxxur_sam, 0, STATEVAR) = cur_x[i] + tminf->rf.onef.H;

      /*
      sam_get(tminf->fwd_sam        ,0        ,F_0_t) + tminf->H  +  ...
      */
      sam_get(node.cxxur_sam, 0, SHORT_RATE) =
          sam_get(node.cxxur_sam, 0, STATEVAR) +
          sam_get(tminf->fwd_sam, 0, F_0_t);

      sam_get(node.cxxur_sam, 0, PHI) = sam_get(tminf->fwd_sam, 0, PHI);
      ...
      for (io_request = iolist->head; io_request != NULL;
        ...
        if (stp->next) {
          ...
          /*  Y_T_at_t_compute uses node->cur_sam...[STATEVAR] which correponds
             to r(t) - f(0        ,t)  that is X(t) + H(t) */
          ...
          sam_get(node.drift_sam, 0, PHI) = sam_get(nxttminf->fwd_sam, 0, PHI);
          /*        		sam_get(node.drift_sam        ,0 ,SHORT_RATE) =
             sam_get(node.cxxur_sam        ,0 ,SHORT_RATE)
                                          + sam_get(nxttminf->fwd_sam        ,0
                   ,F_0_t)
                                          - sam_get(tminf->fwd_sam        ,0
             ,F_0_t)
                                          + nxttminf->H - tminf->H
                                          - tminf->lambda * my_pointer->delta_t
                                                  * (sam_get(node.cxxur_sam ,0
                   ,SHORT_RATE)
                                                  - sam_get(tminf->fwd_sam ,0
                   ,SHORT_RATE)
                                                          - tminf->H);
          */
          /*  The tree is built on X = r(t)-f(0        ,t)-H(t)
              Therefore        , when drift_sam is used        , STATEVAR
             corresponds to X - not to r-f(0        ,t) */
          sam_get(node.drift_sam, 0, STATEVAR) =
              cur_x[i] * (1 - tminf->ev.onef.lambda * my_pointer->delta_t);
          ...
          */
          /* Modified node.mid_son_index  - Please note that this function uses
             only node->drift_sam...[STATEVAR] which is consistent with the
             previous assumption that the tree is build on X        , not really
             on r(t)-f(0        ,t)*/
          srt_f_trintrexindex(my_pointer, prev_x, &node, grfnparam, CLOSESTINX);

          /** calculate variance at this node **/

          node.var_at_sam = tminf->ev.onef.sig2 * my_pointer->delta_t;
          /* CHECK if not better putting stdev_r */
          /** calculate probabilities **/

          /*
                                  srt_f_chetreprob(my_pointer        ,prev_x
             ,&node ,mdl);
          */
          ...
          srt_f_trintredcntvec(my_pointer, cur_assets[xtra_calc_index][i],
                               &node, prev_assets[xtra_calc_index],
                               grfnparam->node_dim);
        ...
        err = evalcf((GrfnEvent *)my_pointer->e, &node.cxxur_sam, gd,
  ...
  for (io_request = iolist->head; io_request != NULL;
    ...
    strncpy(io_request_val_p->computation_origin, "Tree_LGM_1d",
            strlen("Tree_LGM_1d"));
    io_request_val_p->dval =
        prev_assets[xtra_calc_index][0][grfnparam->node_dim - 1];
  ...
  err = srt_f_IOstructsetcolpvs((iolist), SRT_NO, (double *)prev_assets[0][0],
                                grfnparam->node_dim, "");

  /* Free memory */
  free_f3tensor(prev_assets, 0, xtra_calc_num - 1, maxtrinf.min_x_index,
                maxtrinf.max_x_index, 0, grfnparam->node_dim - 1);
  free_f3tensor(cur_assets, 0, xtra_calc_num - 1, maxtrinf.min_x_index,
                maxtrinf.max_x_index, 0, grfnparam->node_dim - 1);
...
static SRT_Boolean is_within(Ddate today, double left, double date_or_time,
  ...
  /* if today != -1        , the date is actually a time -> conversion into a
     date */
...
static Err
srt_f_vegashiftlgm1dtrelim(SrtStpPtr stp, SrtTreStpInf *maxtrinf,
  ...
  while (cur->next) {
    ...
    /* trinf->rmin = sam_get(tminf->fwd_sam        ,0        ,F_0_t) + tminf->H
     * - cur->index...;*/
    cur = cur->next;
  }
  trinf = (SrtTreStpInf *)cur->trinf;
...
static Err srt_f_vegashiftlgministp(
    SrtUndPtr und, SrtUndInfo *und_info, SrtStpPtr stp, TermStruct *ts,
    double sigma_bucket_start, double sigma_bucket_end, double sigma_shift,
    int sigma_shift_type, double tau_bucket_start, double tau_bucket_end,
    double tau_shift, int tau_shift_type) {
  ...
  for (ls = ts->head; ls != NULL; ls = ls->next) {
    ...
    case SH_NONE:
      break;
    case SH_ABSOLUTE:
      if (is_within(-1, sigma_bucket_start, tsval->date, sigma_bucket_end) ==
          SRT_YES)
    ...
    case SH_NONE:
      break;
    case SH_ABSOLUTE:
      if (is_within(-1, tau_bucket_start, tsval->date, tau_bucket_end) ==
          SRT_YES)
        tsval->tau += tau_shift;
      break;
    case SH_RELATIVE:
...
Err srt_f_vegashiftirministp(SrtStpPtr stp,                       /* output */
                             SrtUndPtr und, SrtUndInfo *und_info, /* und info */
                             double sigma_bucket_start, /* shift info */
                             double sigma_bucket_end, double sigma_shift,
                             int sigma_shift_type, double tau_bucket_start,
                             double tau_bucket_end, double tau_shift,
                             int tau_shift_type)

{
  ...
  while (stp->next) {
    ...
    if (is_within(today, sigma_bucket_start, stp->next->time,
    ...
    if (is_within(today, tau_bucket_start, stp->next->time, tau_bucket_end) ==
        SRT_YES) {
      switch (tau_shift_type) {
      case SH_NONE:
        break;
    ...
    if (mdl_type == CHEY) {
      ...
      if (prvtminf) {
        /* initialises the deterministic part of exp(r)        , exp(X) and
         * exp(phi)
         */
        srt_f_chedrfatsam(stp->prev, &(prvtminf->fwd_sam), &(tminf->fwd_sam),
                          0);
      } else {
    ...
    if (err =
            Y_T_at_t_param(stp->ddate, &stp->next->ddate, 1, und, &tminf->yp)) {
      return err;
    }
  ...
  if (is_within(today, sigma_bucket_start, stp->time, sigma_bucket_end) ==
      SRT_YES) {
    switch (sigma_shift_type) {
    case SH_NONE:
      break;
  ...
  if (is_within(today, tau_bucket_start, stp->time, tau_bucket_end) ==
      SRT_YES) {
    switch (tau_shift_type) {
    case SH_NONE:
      break;
  ...
  if (mdl_type == LGM) {
    if (err = srt_f_vegashiftlgministp(
            und, und_info, top, ts, sigma_bucket_start, sigma_bucket_end,
            sigma_shift, sigma_shift_type, tau_bucket_start, tau_bucket_end,
            tau_shift, tau_shift_type)) {
      return err;
    }
  }
  return err;

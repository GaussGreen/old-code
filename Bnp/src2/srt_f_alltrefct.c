...
double interp_linear(double x, double x_lower, double prem_lower, double x_upper, double prem_upper)
{
    double coef, premium;
...
/*<%%STA-----------------------------------------------------------------
  ...
  DESCRIPTION     :find guess such that val_array[index] is the closest to target
  CALL            :

<%%END---------------------------------------------------------------------*/
void find_closest(double target, double* val_array, long min_index, long max_index, long* guess)
{
    long index;
...
/* -----------------------------------------------------------------
   ...
   AUTHOR          :O. Van Eyseren
   DESCRIPTION     :find guess such that val_array[index] is the closest to target
                    but always inferior to it
   CALL            :

<%%END---------------------------------------------------------------------*/
void find_closest_strictly_below(
    double target, double* val_array, long min_index, long max_index, long* guess)
{
    long index;
    ...
    else
    {
        if (node->mid_son_index < trinf->max_x_index &&
            ((prev_x[node->mid_son_index] * prev_x[node->mid_son_index + 1]) <
             (sam_get(node->drift_sam, 0, STATEVAR) * sam_get(node->drift_sam, 0, STATEVAR))))
            node->mid_son_index++;
    }
    return;
}
...
<%%END>---------------------------------------------------------------------*/
/*<%%STADEC>*/
void srt_f_trintredcntvec(
    SrtStpPtr stp, double* cur, SrtTrinTreNdInf* node, double** assets, long dim)
/*<%%ENDDEC>*/
{
    SrtTreStpInf* trinf;
    int           i, j, x_i;
...
/*<%%STADEC>*/
void srt_f_trintreprob(double* prev_x, SrtTrinTreNdInf* node, SrtGrfnParam* grfnparam)
/*<%%ENDDEC>*/
{
...
/*-------------------------------------------------------------------------
  ...
  DESCRIPTION     :calculates probabilities for a nononomial tree,
                                   assuming that node -> moments are correctly set,
                                   as well as node -> mid_son_index
  MODIFIES		  :node -> p[0..2][0..2]
---------------------------------------------------------------------------*/

void srt_f_nonotreprob(SrtTwoFacTreInf* trinf, SrtNonoTreeNodeInfo* node, double** next_x)
{
    double m, n, mn, m2, n2, m2n, mn2, m2n2, rho, e_x1, e_x2, e_x1x1, e_x2x2, e_x1x2, midx1, midx2;
    ...
    e_x1x2  = moments->E_X1X2 - midx2 * moments->E_X1 - midx1 * moments->E_X2 + midx1 * midx2;

    rho = (e_x1x2 - e_x1 * e_x2) / sqrt((e_x1x1 - e_x1 * e_x1) * (e_x2x2 - e_x2 * e_x2));

    e_x1x1x2 =
        e_x1 * e_x1 * e_x2 + 2 * e_x1 * (e_x1x2 - e_x1 * e_x2) + e_x2 * (e_x1x1 - e_x1 * e_x1);
    e_x1x2x2 =
        e_x2 * e_x2 * e_x1 + 2 * e_x2 * (e_x1x2 - e_x1 * e_x2) + e_x1 * (e_x2x2 - e_x2 * e_x2);
    e_x1x1x2x2 = e_x1 * e_x1 * e_x2 * e_x2 +
                 (1.0 + 2 * rho * rho) * (e_x1x1 - e_x1 * e_x1) * (e_x2x2 - e_x2 * e_x2) +
                 e_x1 * e_x1 * (e_x2x2 - e_x2 * e_x2) + e_x2 * e_x2 * (e_x1x1 - e_x1 * e_x1) +
                 4 * e_x1 * e_x2 * (e_x1x2 - e_x1 * e_x2);

    /* compute probabilities */
    node->p[0][0] = e_x1x2 / mn + (-(e_x1x2x2 * mn) - e_x1x2 * mn2) / (2 * mn * mn2) + /* Pdd */
    ...
    node->p[0][2] = (-(e_x1x2x2 * mn) - e_x1x2 * mn2) / (2 * mn * mn2) - /* Pdu */
                    (4 * mn * mn * mn2 * (-(e_x1x1x2 * mn) - e_x1x2 * m2n) * m2n2 -
                     2 * mn * m2n *
                         (-2 * mn * (-(e_x1x2x2 * mn) - e_x1x2 * mn2) * m2n2 -
                          2 * mn * mn2 * (-(e_x1x1x2x2 * mn) + e_x1x2 * m2n2))) /
                        (16 * mn * mn * mn * mn2 * m2n * m2n2);
    node->p[1][0] = -(e_x1x2 / mn) - (-(e_x1x2x2 * mn) - e_x1x2 * mn2) / (2 * mn * mn2) - /* P0d */
    ...
    node->p[1][1] = 1 + e_x1x2 / mn + (-(e_x1x2x2 * mn) - e_x1x2 * mn2) / (mn * mn2) - /* P00 */
                    ...
                    e_x2x2 / n2;
    node->p[1][2] = -(-(e_x1x2x2 * mn) - e_x1x2 * mn2) / (2 * mn * mn2) + /* P0u */
                    (4 * mn * mn * mn2 * (-(e_x1x1x2 * mn) - e_x1x2 * m2n) * m2n2 -
                     2 * mn * m2n *
                         (-2 * mn * (-(e_x1x2x2 * mn) - e_x1x2 * mn2) * m2n2 -
                          2 * mn * mn2 * (-(e_x1x1x2x2 * mn) + e_x1x2 * m2n2))) /
    ...
    node->p[2][0] = (-(e_x1x1x2 * mn) - e_x1x2 * m2n) / (2 * mn * m2n) - /* Pud */
                    (4 * mn * mn * mn2 * (-(e_x1x1x2 * mn) - e_x1x2 * m2n) * m2n2 -
                     2 * mn * m2n *
                         (-2 * mn * (-(e_x1x2x2 * mn) - e_x1x2 * mn2) * m2n2 -
                          2 * mn * mn2 * (-(e_x1x1x2x2 * mn) + e_x1x2 * m2n2))) /
                        (16 * mn * mn * mn * mn2 * m2n * m2n2);
    node->p[2][1] = -(-(e_x1x1 * m) - e_x1 * m2) / (2 * m * m2) - /* Pu0 */
                    (-(e_x1x1x2 * mn) - e_x1x2 * m2n) / (2 * mn * m2n) +
                    (4 * mn * mn * mn2 * (-(e_x1x1x2 * mn) - e_x1x2 * m2n) * m2n2 -
                     2 * mn * m2n *
                         (-2 * mn * (-(e_x1x2x2 * mn) - e_x1x2 * mn2) * m2n2 -
                          2 * mn * mn2 * (-(e_x1x1x2x2 * mn) + e_x1x2 * m2n2))) /
                        (8 * mn * mn * mn * mn2 * m2n * m2n2);
    node->p[2][2] = -(4 * mn * mn * mn2 * (-(e_x1x1x2 * mn) - e_x1x2 * m2n) * m2n2 - /* Puu */
                      2 * mn * m2n *
                          (-2 * mn * (-(e_x1x2x2 * mn) - e_x1x2 * mn2) * m2n2 -
                           2 * mn * mn2 * (-(e_x1x1x2x2 * mn) + e_x1x2 * m2n2))) /
                    (16 * mn * mn * mn * mn2 * m2n * m2n2);
...

{
    ...
    {
            cur_x[0][trinf->min_x_index[0]] = cur_x[1][trinf->min_x_index[1]] = 0.0;
            return;
    }
    */
...
void srt_f_nonotredcntvec(
    SrtStpPtr stp, double* cur, SrtNonoTreeNodeInfo* node, double*** assets, long dim)

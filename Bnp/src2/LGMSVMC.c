...

{
        ...
        SimulationCIR(const BasicParameters & basicParameters0, const SimulationParameters &
simulationParameters0)
    ...
    double** res)
{
    ...
    double *sum_payoff = NULL, *sum_payoff2 = NULL, *path_payoff = NULL, *res_evt = NULL,
           **matrix = NULL, *matrixi = NULL, yInterpolated,

           *muf = NULL, *sigf = NULL, *sigf2 = NULL, *sigf3 = NULL, *sigf4 = NULL, *sigf5 = NULL,
           *sigf6 = NULL, *mupsi = NULL, *mupsi2 = NULL, *sigpsi = NULL, *muv1 = NULL, *muv2 = NULL,
           *sigv = NULL, *sigv2 = NULL, *sigv3 = NULL, *sigv4 = NULL, *sigv5 = NULL, *sigv6 = NULL,
           *sigv7 = NULL, *sigv8 = NULL, *sigv9 = NULL, *sigv10 = NULL, *sigv11 = NULL,
           *sigv12 = NULL, *sigv13 = NULL, *coef1 = NULL, *coef2 = NULL,

           *b1 = NULL, *b2 = NULL, *b3 = NULL,

           *c2 = NULL, *c3 = NULL, *c1 = NULL, *c4 = NULL, *c5 = NULL, *c6 = NULL, *c7 = NULL,

           *n0 = NULL, *n2 = NULL, *n4 = NULL, *d1 = NULL, **gamma = NULL, **cumulant1 = NULL,
           **cumulant2 = NULL, **cumulant3 = NULL, *nu = NULL, *xAux = NULL,
           **coefBesselApprox1 = NULL, **coefBesselApprox2 = NULL, **xValues = NULL,
           **TabulatedBesselFunction = NULL, **TabulatedBesselFunctionSecondDerivate = NULL,
    ...
    int    NbCumulants = 7;
    double v0, sqrtV, sumV, difV, x, argGamma1, argGamma2, besselApproxNum, besselApproxDen,
        SimpleBesselFunction, BigBesselFunction;
    ...
    if (!matrix || !path_payoff || !sum_payoff || !sum_payoff2 || !res_evt || !sigf || !sigpsi ||
        !muv1 || !muv2 || !sigv || !coef1 || !coef2)
    {
    ...
    if (do_optimisation)
    {
        err = mceb_allocate_savevalues_for_GRFN(iNumPaths, iNbEvent, params, &save_values);
...

    {
        ...
        if (!muf || !sigf2 || !sigf3 || !sigf4 || !sigf5 || !sigf6 || !sigv2 || !sigv3 || !mupsi ||
            !mupsi2)
        {
            err = "Memory allocation failure in lgmSV_mc_balsam";
            goto FREE_RETURN;
    ...
    case 3:
    {
        ...
        if (!muf || !sigf2 || !sigf3 || !sigf4 || !sigf5 || !sigf6 || !sigv2 || !sigv3 || !sigv4 ||
            !sigv5 || !sigv6 || !sigv7 || !sigv8 || !sigv9 || !sigv10 || !sigv11 || !sigv12 ||
            !sigv13 || !mupsi || !mupsi2)
        {
            err = "Memory allocation failure in lgmSV_mc_balsam";
            goto FREE_RETURN;
        ...
        for (j = 1; j < iNbTime; j++)
        {
            ...
            sigv6[j] = (-11 * pow(dAlpha[j], 6) + 56 * pow(dAlpha[j], 4) * dLvlEps[j] -
            ...
            sigv9[j] =
                ...
                (576 * dAlpha[j] * dAlpha[j] * dLvlEps[j] * dLvlEps[j] * dLvlEps[j] +
                 ...
                 944 * dLvlEps[j] * dLvlEps[j] * pow(dAlpha[j], 4) - 41 * pow(dAlpha[j], 8)) *
                dt * dt;
            ...
            sigv12[j] = (1 / (36864 * pow(dAlpha[j], 2))) *
                        ...
                        dt * dt;
            sigv13[j] =
                (1 / (36864 * pow(dAlpha[j], 2))) * (3072 * pow(dLambdaEps[j], 4)) * dt * dt;
        }
    ...
    case 4:
    {
        ...
        if (!muf || !sigf2 || !sigf3 || !sigf4 || !sigf5 || !sigf6 || !sigv2 || !sigv3 || !mupsi ||
            !b1 || !b2 || !b3 || !c1 || !c2 || !c3 || !c4 || !c5 || !c6 || !c7 || !nu || !xAux)
        {
            err = "Memory allocation failure in lgmSV_mc_balsam";
            goto FREE_RETURN;
        }
    ...
    case 5:
    {
        ...
        if (!muf || !sigf2 || !sigf3 || !sigf4 || !sigf5 || !sigf6 || !sigv2 || !sigv3 || !mupsi ||
            !b1 || !b2 || !b3 || !c1 || !c2 || !c3 || !c4 || !c5 || !c6 || !c7 || !nu || !xAux ||
            !coefBesselApprox1 || !coefBesselApprox2)
        {
            err = "Memory allocation failure in lgmSV_mc_balsam";
            goto FREE_RETURN;
        }
        ...
        for (j = 1; j < iNbTime; j++)
        {
...

            {
                ...
                coefBesselApprox1[j][iBesselApprox] = pow(0.5, nu[j] + 1) *
                                                      pow(0.25, iBesselApprox) /
                                                      (fact(iBesselApprox) * gamma_(&argGamma1));
                argGamma2                           = nu[j] + iBesselApprox + 1;
                coefBesselApprox2[j][iBesselApprox] = pow(0.5, nu[j]) * pow(0.25, iBesselApprox) /
    ...
    case 6:
    {
        ...
        if (TabulatedBesselFunctionSecondDerivate)
            free_dmatrix(TabulatedBesselFunctionSecondDerivate, 0, iNbTime - 1, 1, NbTabCount + 1);
        ...
        if (!muf || !sigf2 || !sigf3 || !sigf4 || !sigf5 || !sigf6 || !sigv2 || !sigv3 || !sigv4 ||
            !sigv5 || !sigv6 || !sigv7 || !sigv8 || !sigv9 || !sigv10 || !sigv11 || !sigv12 ||
            !sigv13 || !mupsi || !mupsi2 || !b1 || !b2 || !b3 || !c1 || !c2 || !c3 || !c4 || !c5 ||
            !c6 || !c7 || !nu || !xAux)
        ...
        for (j = 1; j < iNbTime; j++)
        {
            ...
            {
                xValues[j][iTabCount] =
                    xInfBound + ((xSupBound - xInfBound) / NbTabCount) * (iTabCount - 1);
            ...
            spline(
                xValues[j],
                TabulatedBesselFunction[j],
                NbTabCount + 1,
                1.0e30,
                1.0e30,
                TabulatedBesselFunctionSecondDerivate[j]);
    ...
    time2 = clock();
    smessage(
        "Phase 1 -BalSam generation, time in sec: %.2f", (double)(time2 - time1) / CLOCKS_PER_SEC);
    ...
    for (i = 0; i < iNumPaths; i++)
    {
        ...
        if (func_parm_tab[0] && EvalEvent[0])
        {
            ...
            if (do_optimisation)
            {
                mceb_fill_savevalues_from_GRFN(save_values[evtindex], res_evt, i, df, params);
            }
        ...
        for (j = 1; stop_path == 0 && j < iNbTime; j++)
        {
...

            {
                ...
                v0 = v;
                v  = muv1[j] + muv2[j] * v0 -
                    (sigv4[j] / v0 + sigv5[j] + sigv9[j] / (v0 * v0) + sigv10[j] / v0 + sigv11[j] +
                ...
                v += (sigv[j] * sqv + sigv2[j] / sqv + sigv3[j] * brow2 +
                      ...
                      (sqdt * (sigv4[j] / v0 + sigv5[j]) +
                       dt * (sigv9[j] / (v0 * v0) + sigv10[j] / v0 + sigv11[j] + sigv12[j] * v0 +
                             sigv13[j] * v0 * v0)) *
                          brow2) *
                     brow2;
            ...
            case 4:
            {
                ...
                x = xAux[j] * sqrtV;
                SimpleBesselFunction =
                    (ModifiedBesselFunction(nu[j] + 1, x) / ModifiedBesselFunction(nu[j], x) +
                     nu[j] / x);
            ...
            case 5:
            {
                ...
                {
                    besselApproxNum +=
                        coefBesselApprox1[j][iBesselApprox] * pow(x, nu[j] + 1 + 2 * iBesselApprox);
                    besselApproxDen +=
                        coefBesselApprox2[j][iBesselApprox] * pow(x, nu[j] + 2 * iBesselApprox);
                }
                SimpleBesselFunction =
                    (ModifiedBesselFunction(nu[j] + 1, x) / ModifiedBesselFunction(nu[j], x) +
                     nu[j] / x);
    ...
    if (do_optimisation)
    {
        ...
        time2 = clock();
        smessage(
            "Phase 3 -optimisation, time in sec: %.2f", (double)(time2 - time1) / CLOCKS_PER_SEC);
    }
    ...
    double** res)
{
    ...
    double *sum_payoff = NULL, *sum_payoff2 = NULL, *path_payoff = NULL, *res_evt = NULL,
           **matrix = NULL, *matrixi = NULL,

           *sigf1 = NULL, *sigf2 = NULL, *sigpsi1 = NULL, *sigpsi2 = NULL, *sigpsi12 = NULL,
           *muv1 = NULL, *muv2 = NULL, *sigv = NULL, *coef1 = NULL, *coef2 = NULL, *coef3 = NULL,
           *coef4 = NULL, *coef5 = NULL,
    ...
    if (!matrix || !path_payoff || !sum_payoff || !sum_payoff2 || !res_evt || !sigf1 || !sigf2 ||
        !sigpsi1 || !sigpsi2 || !sigpsi12 || !muv1 || !muv2 || !sigv || !coef1 || !coef2 ||
        !coef3 || !coef4 || !coef5)
    {
        err = "Memory allocation failure in lgmSV_mc_balsam";
        goto FREE_RETURN;
    ...
    for (i = 0; i < iNumPaths; i++)
    {
        ...
        if (func_parm_tab[0] && EvalEvent[0])
        {
            ...
            df =
                exp(dff_star[0] + gam1_star[0] * ft1 + gam2_star[0] * ft2 + gam1_2_star[0] * psi1 +
                    gam2_2_star[0] * psi2 + gam12_star[0] * psi12);
        ...
        for (j = 1; stop_path == 0 && j < iNbTime; j++)
        {
            ...
            if (EvalEvent[j])
            {
                ...
                df =
                    exp(dff_star[evtindex] + gam1_star[evtindex] * ft1 + gam2_star[evtindex] * ft2 +
                        gam1_2_star[evtindex] * psi1 + gam2_2_star[evtindex] * psi2 +
                        gam12_star[evtindex] * psi12);
    ...
    double** res)
{
    ...
    clock_t  time1, time2;
    long     i, j, k;
    double **payoff = NULL, *res_evt = NULL, *init_gauss = NULL, *ft = NULL, *v = NULL, *psi = NULL,
           **matrix = NULL, *matrixk = NULL, ***save_values = NULL, **save_valuesj = NULL;
    ...
    for (j = 1; j < iNbTime; j++)
    {
        ...
        for (i = 0; i < iNumPaths; i++)
        {
            if (!stop_path[i])
            {
                ...
                if (EvalEvent[j])
                {
                    ...
                    {
                        df =
                            exp(dff_star[evtindex] + gam_star[evtindex] * ft[i] +
        ...
        {
            err = payoff_adjust_function(
                evtindex, iNumPaths, iNbProduct, save_values, func_parm_tab[j], params);
    ...
    double** res)
{
    ...
    clock_t  time1, time2;
    long     i, j, k;
    double **payoff = NULL, *res_evt = NULL, *init_gauss = NULL, *ft1 = NULL, *ft2 = NULL,
           *v = NULL, *psi1 = NULL, *psi2 = NULL, *psi12 = NULL, **matrix = NULL, *matrixk = NULL,
    ...
    if (!payoff || !res_evt || !init_gauss || !ft1 || !ft2 || !v || !psi1 || !psi2 || !psi12 ||
        !matrix || !stop_path)
    {
        err = "Memory allocation failure in lgmSV_mc_balsam";
        goto FREE_RETURN;
    ...
    if (func_parm_tab[0] && EvalEvent[0])
    {
        ...
        {
            df =
                exp(dff_star[0] + gam1_star[0] * ft1[0] + gam2_star[0] * ft2[0] +
        ...
        {
            err = payoff_adjust_function(
                0, iNumPaths, iNbProduct, save_values, func_parm_tab[0], params);
        ...
        for (i = 0; i < iNumPaths; i++)
        {
            if (!stop_path[i])
            {
                ...
                if (EvalEvent[j])
                {
                    ...
                    {
                        df =
                            exp(dff_star[evtindex] + gam1_star[evtindex] * ft1[i] +
                                gam2_star[evtindex] * ft2[i] + gam1_2_star[evtindex] * psi1[i] +
                                gam2_2_star[evtindex] * psi2[i] + gam12_star[evtindex] * psi12[i]);
                    }
                    else
                    {
                        df = 1.0;
    ...
    double** res)
{
    ...
    clock_t time1, time2;
    long    i, j, k;
    double *sum_payoff = NULL, *sum_payoff2 = NULL, *res_evt = NULL, *path_payoff = NULL,
           *init_gauss = NULL, **ft1 = NULL, **ft2 = NULL, **v = NULL, **psi1 = NULL, **psi2 = NULL,
           **psi12 = NULL, **matrix = NULL, *matrixk = NULL, ***save_values = NULL,
           **save_valuesj = NULL;
    ...
    if (!init_gauss || !matrix || !sum_payoff | !sum_payoff2 || !res_evt || !path_payoff || !ft1 ||
        !ft2 || !v || !psi1 || !psi2 || !psi12 || !index_event)
    {
    ...
    time2 = clock();
    smessage(
        "Phase 1 -Path Generation, time in sec: %.2f", (double)(time2 - time1) / CLOCKS_PER_SEC);
    ...
    for (i = 0; i < iNumPaths; i++)
    {
        ...
        if (func_parm_tab[0] && EvalEvent[0])
        {
            ...
            df =
                exp(dff_star[0] + gam1_star[0] * ft1[evtindex][i] +
                    gam2_star[0] * ft2[evtindex][i] + gam1_2_star[0] * psi1[evtindex][i] +
                    gam2_2_star[0] * psi2[evtindex][i] + gam12_star[0] * psi12[evtindex][i]);
        ...
        for (evtindex; stop_path == 0 && evtindex < iNbEvent; evtindex++)
        {
            ...
            df = exp(
                dff_star[evtindex] + gam1_star[evtindex] * ft1[evtindex][i] +
                gam2_star[evtindex] * ft2[evtindex][i] + gam1_2_star[evtindex] * psi1[evtindex][i] +
             ...
             double** res)
{
    ...
    for (i = 0; i < iNumPaths; i++)
    {
        ...
        if (func_parm_tab[0] && EvalEvent[0])
        {
            err = payoff_func(dDate[0], dTime[0], func_parm_tab[0], ft, psi, iNbProduct, temp);
        ...
        for (j = 1; j < iNbTime; j++)
        {
            ...
            if (EvalEvent[j])
            {
                /* Modification of the Payoff at t */
                err =
                    payoff_func(dDate[j], dTime[j], func_parm_tab[j], ft, psi, iNbProduct, res_evt);
    ...
    double** res)
{
    ...
    if (!temp || !res_evt || !sum_price || !sum_2price || !temp_bs || !res_evt_bs ||
        !sum_price_bs || !sum_2price_bs || !cov_price_bs)
    {
    ...
    for (i = 0; i < iNumPaths; i++)
    {
        ...
        for (j = 1; j < iNbTime; j++)
        {
            ...
            if (EvalEvent[j])
            {
                ...
                /* Modification of the Payoff at t */
                err = payoff_func(
                    dDate[j], dTime[j], func_parm_tab[j], ft_bs, psi_bs, iNbProduct, res_evt_bs);
    ...
    for (k = 0; k < iNbProduct; k++)
    {
        ...
        res[k + iNbProduct][1] =
            sqrt((sum_2price_bs[k] - sum_price_bs[k] * sum_price_bs[k]) / iNumPaths);
    ...
    double* pStd)
{
    ...
    int     dNbSig;
    double *dSigTime = NULL, *dSig = NULL, *dAlphaTS = NULL, *dRhoTS = NULL, *dLambdaEpsTS = NULL,
           **Coupon = NULL;
    ...
    for (i = 1; i < ncpn; i++)
    {
        ...
        swp_cash =
            (swp_f_df(today, cpn_date[i - 1], yc_name) - swp_f_df(today, cpn_date[i], yc_name)) /
            (cpn_cvg[i] * swp_f_df(today, cpn_date[i], yc_name));

        for (j = 0; j < nb_strike; j++)
        {
            Coupon[j][i] = -cpn_cvg[i] * pay_rec * (dStrike[j] - (swp_rte - swp_cash)) *
    ...
    for (i = 1; i < dNbSig; i++)
    {
        if ((fabs(dAlphaTS[i] - dAlpha) > 1.0E-08) || (fabs(dRhoTS[i] - dRho) > 1.0E-08) ||
    ...
    double* Std)
{
    Err     err = NULL;
    int     i, j, k, nbstep;
    double *time = NULL, *vol = NULL, *cpn = NULL, *lamdt = NULL, *dti = NULL, *beta = NULL,
           *volX = NULL, *volX2 = NULL, *sig = NULL, *var = NULL, *drift = NULL, *init_gauss = NULL,
           *sum = NULL, *sum2 = NULL, *covar = NULL;
    ...
    if (!vol || !lamdt || !nbpT || !cpn || !beta || !volX || !volX2 || !dti || !sum || !sum2 ||
        !covar || !init_gauss || !sig || !var || !drift)
    {
    ...
    for (i = 0; i < iNumPaths; i++)
    {
        for (j = 0; j < nb_strike; j++)
        {
            lgm_price = lgmopval_stochvol(
                iNbCoupon, Coupon[j], beta, var[i], dRho * drift[i], rho2, sqrho2);

...
static LGMErr
FitAllZetaCaps(LGM_TS *tsPtr,     /* Return: zeta's in LGM term structure ts */
...
static LGMErr
FitAllGCaps(LGM_TS *tsPtr,     /* Return: calibrated LGM term structure tsPtr */
            LGMCalSet *CSPtr); /* Reference caplets */

/* Fits the LGM price of a caplet to the market price */
static double FitOneCap(double price, double cvg, double Rfix, double DStart,
                        double Dend, SrtReceiverType recpay);
...
static LGMErr
FitAllZetaSwaps(LGM_TS *tsPtr,     /* Return: zeta's in LGM term structure ts */
                LGMCalSet *CSPtr); /* Reference caplets */

/* Fits value of zeta at exercise date for a swaption */
static double FitOneZetaSwap(long n, double *a, double DStart,
                             SrtReceiverType recpay, double *Gpay, double Gst,
                             double price);
...
static LGMErr FitAllGPairs(long npair, long *jpair, long usecaps, LGM_TS *tsPtr,
                           LGMCalSet *CSPtr);

/* Picks coef so the LGM price of a long swaption matches the market price when
        Gst = Gst + coef*dGst
   (G at start date)
        Gpay[i] = Gpay[i] + coef*dGpay[i]        , i=1        , ...        ,n
   (G at pay dates) */
static double FitOneSwap(long n, double *a, double DStart,
                         SrtReceiverType recpay, /* the swaption */
                         double zeta,            /* zeta */
                         double *Gstptr,
                         double *Gpay, /* the existing term structure */
                         double dGst, double *dGpay, /* direction of change */
                         double price); /* market price of swaption */

/* Picks coef so the LGM price of a long swaption matches the market price when
        Gst = Gst + coef*dGst
(G at start date) Gpay[i] = Gpay[i] + coef*dGpay[i]        , i=1        , ... ,n
(G at pay dates) and sqrt(zeta)*Gst is held constant at product */
static double
FitOneSwapCap(long n, double *a, double DStart,
              SrtReceiverType recpay,       /* the swaption */
              double zeta, double product,  /* zeta        , product */
              double *Gstptr, double *Gpay, /* the existing term structure */
              double dGst, double *dGpay,   /* direction of change */
              double price);                /* market price of swaption */
...
swaptions match their market price when
        Gst = Gst + coef*dGst
(G at start date) Gpay[i] = Gpay[i] + coef*dGpay[i]        , i=1        , ... ,n
(G at pay dates) zeta(tEx) = sqzeta*sqzeta */
static double
FitOneLongShort(long n1, double *a1, /* swaption 1 */
                long n2, double *a2, /* swaption 2 */
                double DStart,
                SrtReceiverType recpay, /* PV of strike for both swaptions */
                double *Gstptr, double *Gpay,  /* the existing term structure */
                double dGst, double *dGpay,    /* direction of change */
                double price1, double price2); /* market prices of swaptions */
...
static double RecSqZetaDer(double ystar, long n, double *a,
                           double DStart, /* receiver swaption */
                           double sqzeta, double *Gpay,
...
static double
RecGDer(double ystar, long n, double *a, double DStart, /* receiver swaption */
        double sqzeta, double *Gpay, double Gst,        /* term structure */
        double dGst, double *dGpay); /* direction of differentiation */

/* Create a term structure and calculate G(t) from fixed kappa */
static LGM_TSPtr
CreateTSwithKappa(double kap,    /* Fixed kappa to use to construct G */
...
static LGM_TSPtr CreateTSwithGs(long numG,   /* number of values of G */
...
static LGM_TSPtr
CreateTSwithSigma(LGMCalSet *CSPtr); /* Reference instruments */

/* Create a term structure and fills in the input zeta(t) */
static LGM_TSPtr
CreateTSfromZetas(long nZ,           /* number of zeta values */
...
LGMErr LGMCalibration(
    ...
    Date tlast, Date *TauArr, /* calibrate on these exercise dates */
                                            ...
                                            structure (NULL => not req'd) */
{
  ...
  *LGMtsPtrPtr = NULL;
  ...
  if (CalReq->LGMOneTwoFactor == 1) {
    ...
    error = LGMMakeRefDeals(
        dealType, dealptr, tNow, tlast, nEx, TauArr,
        FVArr,           /* exer dates and relative PV of fixed leg of deal */
        ycname, &conv,   /* discount curve & market conventions */
        CalReq,          /* method and data for choosing strikes */
        GetVol, GetBeta, /* information for choosing strikes */
    ...
    error =
        LGMPriceRefDeals(GetVol, GetBeta, RecVol, RefDealPtr, lgmRefSwptnData);
    if (error != NULL) {
      LGMFreeCalSet(&RefDealPtr);
      return (error);
    }
  ...
  */
    ...
    case GivenG:
      error =
          LGMCalGivenG(&LGMtsPtr,       /* Return: Calibrated term structure */
    ...
    /* Simultaneously calibrate on the long "k into n-k" swaptions and either
    the short "k into 1" swaptions or caplets depending on usecaps */
    ...
    /* Use constant sigma to construct zeta(t)        , then get G(t) by
    calibrating on the long "k into n-k" swaptions or the caplets        ,
    depending on usecaps */
    case FixSigma:
      error =
          LGMCalFixSig(&LGMtsPtr,       /* Return: Calibrated term structure */
                       CalReq->usecaps, /* calibrate on caplets or swaptions */
                       RefDealPtr);     /* Reference instruments */
      break;
  ...
  */
    ...
    /* Calibrate on the "1 into k" swaptions to get G(t)        , then get
    zeta(t) by calibrating on the long "k into n-k" swaptions or the caplets ,
    depending on usecaps */
    case FixExp:
      error =
          LGMCalFixExp(&LGMtsPtr,       /* Return: Calibrated term structure */
                       CalReq->usecaps, /* calibrate on caplets or swaptions */
    ...
    *LGMtsPtrPtr = LGMtsPtr;
    ...
    error = lgm_2f_autocal_calibrate(dealptr, LGMtsPtrPtr, lgmRefSwptnData,
                                     CalReq, GetVol, GetBeta, ycname);
    if (error != NULL) {
      if (LGMtsPtr)
        LGMFreeLGM2F_TS(&LGMtsPtr);
      LGMFreeCalSet(&RefDealPtr);
...
static void checkcalibrationmethod(LGMCalParm *CR) {
  ...
  if (CR->MinMonToEx >
      12)                /* minimum time to exercise less than twelve months */
  ...
  if (CR->usecaps ==
      1)           /* All methods except Tenor & Diagonal and Fixed Expiry */
...
/**************************************************************/
/* The following routine first creates a term structure  and constructs G(t)
from the incoming fixed kappa: If usestarts != 0        , it will construct G(t)
on each start date of the caplets or swaptions        , and on the end date of
the last caplet (if usecaps=1) or tPay[n] (if usecaps!=1). If usestarts = 0 , it
will construct G(t) on the start date of the first caplet or swaption and on the
end date of the last caplet or tPay[n]         , and on the dates Gdates[j] ,
j=1        ,... , which fall between the first start date and the last end date.
Note that Gdate[0] is ignored.  In the call from LGMCalibration        , Gdate[]
are the pay dates of the fixed leg of the longest swaption used to calibrate It
then finds zeta(t) at tNow        , and at each exercise date of the set of
caplets or swaptions by calibrating to the set of caplets  (if usecaps=1) or
long swaptions (if usecaps!=1) in the calibration set CSPtr. */

/* If usecaps == 1        , the following information must be set in CSPtr:
tNow							evaluation date
nEx>=1							number of reference
caplets tEx[j] for j=1        ,...        ,nEx			exercise date
for caplet j tStart[j] for j=1        ,...        ,nEx		start date for
caplet j DStart[j] for
j=1        ,...        ,nEx		dis factor from tNow to the start date
tEnd[j] for j=1
      ,...        ,nEx			end date for caplet j Dcap[j] for j=1        ,...
,nEx dis factor to the end date of caplet j cvgcap[j] for j=1        ,... ,nEx
coverage from start date to end date for caplet j Rfcap[j] for j=1        ,...
,nEx		fixed rate for caplet j Vcap[j] for j=1        ,...        ,nEx
market value at tNow for caplet j Note the index j=0 is ignored in the above
arrays. */
...
nEx>=1							number of reference
swaptions
tEx[j] for j=1        ,...        ,nEx			exercise date for
swaption j tStart[j] for j=1        ,...        ,nEx		start date for
swaption j DStart[j] for j=1        ,...        ,nEx		dis factor from
tNow to the start date
nPay							total number of fixed
leg pay dates tPay[i] for i=1        ,...        ,nPay		fixed leg pay
dates
cvgpay[i] for i=1        ,...        ,nPay		coverage from tPay[i-1]
to tPay[i] Dpay[i] for i=1        ,...        ,nPay		dis factor from
tNow to tPay[i] ifirst [j] for j=1        ,...        ,nEx		fixed
leg pay dates are i = ifirst[j] to nlong[j] nlong [j] for j=1        ,... ,nEx
cvgfirst t[j] for j=1        ,...        ,nEx	first period coverage (from
tStart[j] to tPay[ifirst[j]]) Rlong [j] for j=1        ,...        ,nEx
fixed rate for swaption j Vlong [j] for j=1        ,...        ,nEx
market price of swaption j Note the
indices j=0 and i=0 are ignored in the above arrays */
LGMErr
             ...
             LGMCalSet *CSPtr)     /* Reference caplets */
{
  ...
  *LGMtsPtrPtr = NULL;
  ...
  /* Create term structure        , fill in  G dates        , calculate and fill
   * in G values from kappa */
  tsPtr = CreateTSwithKappa(kap, usestarts, ndate, Gdate, usecaps, CSPtr);
  if (tsPtr == NULL)
    return ("error in CreateTSwithKappa");

  /* Pick the zeta's in the term structure *ts by calibrating to the caplets or
     swaptions in the calibration set CS */
  if (usecaps == 1)
...
/*************************************************************************/
/* This routine is the same as above        , except that it uses the input
arrays G(t) = G[i] at t=Gdate[i]        , i=1        ,...        ,ndate to
define G(t) in the term structure */
LGMErr
             ...
             LGMCalSet *CSPtr)     /* Reference instruments */
{
  ...
  *LGMtsPtrPtr = NULL;
  ...
  /* Pick the zeta's in the term structure *ts by calibrating to the caplets in
   * CS */
  if (usecaps == 1)
    error = FitAllZetaCaps(tsPtr, CSPtr);
  else
...
/**************************************************/
/* This routine creates a term structure and calibrates it to the set of caplets
or the set of long (k into n-k) swaptions calibration set CSPtr. It first
constructs zeta(t) using a fixed value of sigma: zeta[j] =
sigma*sigma*(tEx[j]-tNow) on the exercise dates tEx[j]        , j=1        ,...
,nEx. The particular value of sigma used is irrelevent.
/* If usecaps ==1        , the following information must be set in CSPtr:
tNow						evaluation date
...
DStart[j] for j=1        ,...        ,nEx		dis factor from tNow to
the start date
tEnd[j] for j=1        ,...        ,nEx		end date for caplet j
Dcap[j] for j=1        ,...        ,nEx		dis factor to the end date of
caplet j cvgcap[j] for j=1        ,...        ,nEx	coverage from start date
to end date for caplet
j Rfcap[j] for j=1        ,...        ,nEx	fixed rate for caplet j Vcap[j]
for j=1       ,... ,nEx		market value at tNow for caplet j
...
tStart[j] for j=1        ,...        ,nEx		start date for swaption
j DStart[j] for j=1        ,...        ,nEx		dis factor from tNow to
the start date
n							tPay[n] is the last date
we need to calibrate to tPay[i] for i=1        ,...        ,nPay	fixed
leg pay dates cvgpay[i] for i=1        ,...        ,nPay	coverage from
tPay[i-1] to tPay[i] Dpay[i] for i=1        ,...        ,nPay	dis factor from
tNow to tPay[i] ifirst [j] for j=1        ,...        ,nEx	fixed leg pay
dates are i = ifirst[j] to nlong[j] nlong [j] for j=1        ,...        ,nEx
cvgfirst t[j] for j=1        ,...        ,nEx  first period coverage (from
tStart[j] to tPay[ifirst[j]]) Rlong [j] for j=1        ,...       ,nEx fixed
rate for swaption j Vlong [j] for j=1        ,...        ,nEx	market price of
swaption j Note the indices j=0 and i=0 are ignored in the above arrays */
LGMErr LGMCalFixSig(
    ...
    LGMCalSet *CSPtr) /* Reference instruments */
{
  ...
  *LGMtsPtrPtr = NULL;
  ...
  /* Create term structure        , fill in  zeta dates        , calculate and
   * fill in zeta values */
  tsPtr = CreateTSwithSigma(CSPtr);
  if (tsPtr == NULL)
    return ("error in CreateTSwithSigma");

  /* Pick the G(t) values in the term structure *tsPtr by calibrating to the
   * caplets in *CSPtr */
  if (usecaps == 1)
...
/*************************************************************************/
/* This routine is the same as above        , except that it uses the input
arrays zeta(t) = Zval[i] at t=Zdate[i]        , i=1        ,...        ,numZ
    ...
    LGMCalSet *CSPtr) /* Reference caplets */
{
  ...
  *LGMtsPtrPtr = NULL;
  ...
  /* Create term structure        , fill in  numZ        , zeta dates        ,
   * and zeta values */
...
/*****************************************************************************/
/* After the term structure has been created        , and the zeta values filled
in        , this routine finds the G values by calibrating to the caplets , and
then fills them into the term structure */
static LGMErr
FitAllGCaps(LGM_TS *tsPtr,    /* Return: calibrated LGM term structure ts */
            LGMCalSet *CSPtr) /* Reference caplets */
{
  LGMErr error;
  ...
  for (j = nEx; j > 0; j--) {
    stddev = FitOneCap(CSPtr->Vcap[j], CSPtr->cvgcap[j], CSPtr->Rfcap[j],
                       CSPtr->DStart[j], CSPtr->Dcap[j], recpay);
    if (stddev < 0.0) /* error */
    ...
    if (CSPtr->tEnd[j] <=
        tsPtr->Gdate[j]) /* tEnd[j] betweeen Gdate[j-1] and Gdate[j] */
    {
      ...
      for (k = j; tsPtr->Gdate[k] < CSPtr->tEnd[j] && k < tsPtr->numG; k++)
        ;
      dt1 = (double)(tsPtr->Gdate[k] -
                     tsPtr->Gdate[k - 1]); /* tEnd[j] between Gdate[k-1] and
                                              Gdate[k] */
...
/******************************************************/
/* Fits the LGM price of a caplet to the market price */
/* price is market price        , cvg = coverage of period        , Rfix = fixed
rate        , DStart        , Dend are dis factors to start and end dates The
output is stddev = sqrt(zeta(tEx))*|G(tStart) - G(tEnd)| */
static double FitOneCap(double price, double cvg, double Rfix, double DStart,
                        double Dend, SrtReceiverType recpay) {
  double fwd, stddev;
...
/*****************************************************************************/
/* After the term structure has been created        , and the G values filled in
, this routine finds the zeta values by calibrating to the caplets        , and
then fills them into the term structure */
static LGMErr
FitAllZetaCaps(LGM_TS *tsPtr,    /* Return: zeta's in LGM term structure ts */
               LGMCalSet *CSPtr) /* Reference caplets */
...
/*************************************************************************************/
/* After the term structure has been created        , and the G values filled in
, this routine finds the zeta values by calibrating to the long swaptions , and
then fills them into the term structure */
static LGMErr FitAllZetaSwaps(LGM_TS *tsPtr, LGMCalSet *CSPtr) {
  ...
  for (j = 1; j <= nEx; j++) {
    ifirst =
        CSPtr->ifirst[j]; /* discounted payments of fixed leg of swaption j */
    nlong = CSPtr->nlong[j];
    dispayment[1] = CSPtr->Rflong[j] * CSPtr->cvgfirst[j] * CSPtr->Dpay[ifirst];

    for (i = ifirst + 1; i <= nlong; i++)
      dispayment[i + 1 - ifirst] =
          CSPtr->Rflong[j] * CSPtr->cvgpay[i] * CSPtr->Dpay[i];

    n = nlong + 1 - ifirst;
    dispayment[n] =
        dispayment[n] + CSPtr->Dpay[nlong]; /* add notional to final payment */

    Gst = LGMGFromTS(CSPtr->tStart[j], tsPtr);

    ind = ifirst - 1;
    zeta = FitOneZetaSwap(n, dispayment, CSPtr->DStart[j], recpay, &(Gpay[ind]),
                          Gst, CSPtr->Vlong[j]);
    if (zeta < 0) {
      srt_free(Gpay);
      srt_free(dispayment);
...
/********************************************************************************/
/* Fits value of zeta at exercise date for a swaption */
/* The fixed leg has the disc payments a[i] = payment[i]*df(tPay[i])        ,
i=1       ,... ,n. The strike is worth DStart = df(tStart)        , and  Gpay[i]
= G(t) at tPay[i]        , i=1        ,...        ,n and Gst = G(tStart) */
static double FitOneZetaSwap(long n, double *a, double DStart,
                             SrtReceiverType recpay, double *Gpay, double Gst,
                             double price) {
  SrtCallPutType callput;
  ...
  /* check for intrinsic */
  if (fabs(LGMRecVal(&ystar, n, a, DStart, 1.0e-15, Gpay, Gst) - price) <
      1.0e-08) {
    return 1.0e-15;
  }
  ...
  favg = 0.5 * (fwd + DStart);
  stddev =
      LGMNormImpVol(price / favg, fwd / favg, DStart / favg, 1.0, 1.0, callput);

  /* Use global Newton on sqzeta = sqrt(zeta(tEx)) to match LGM price to market
   * price */
  ...
  for (iter = 1; iter <= itermax && success <= 0; iter++) {
    newvalue =
        LGMRecVal(&ystar, n, a, DStart, sqzeta + dsqzeta, Gpay, Gst) - price;
...
/******************************************************************************/
/* After the term structure has been created and the zeta values filled in ,
this routine finds the G(t) values by calibrating to the long swaptions and
fills them into the term structure */
static LGMErr
FitAllGBkwdSwap(LGM_TS *tsPtr, /* Return: zeta's in LGM term structure tsPtr */
                LGMCalSet *CSPtr) /* Reference instruments */
{
  LGMErr error;
  SrtReceiverType recpay;
  ...
  /* For each j        , find G(tStart[j]) so that the LGM value of long
  swaption j agrees with the market price Vlong[j] */
  previfirst = CSPtr->nPay + 1;
  for (j = nEx; j > 0; j--) {
    ifirst = CSPtr->ifirst[j]; /* compute discounted payments of fixed leg of
                                  swaption j */
    nlong = CSPtr->nlong[j];
    dispayment[1] = CSPtr->Rflong[j] * CSPtr->cvgfirst[j] * CSPtr->Dpay[ifirst];
    ...
    ind = ifirst - 1;
    zeta =
        FitOneSwap(n, dispayment, CSPtr->DStart[j], recpay, /* the swaption */
                   zeta,                                    /* zeta(tEx) */
                   &Gst, &(Gpay[ind]),  /* the term structure */
                   dGst, &(dGpay[ind]), /* direction G */
...
/******************************************************************/
/* This routine creates a term structure and calibrates it to the set of fixed
expiry (1 into k) swaptions and the set of caplets in the calibration set CSPtr.
It first constructs G(t) from by calibrating the 1 into k swaptions to their
market value. It then finds zeta(t) at tNow        , and at each exercise date
tEx by calibrating the caplets to their market values */
/* The following information must be set in CSPtr:
        tNow						evaluation date
Definition of the 1 into k swaptions:
        ftEx							exercise date
for all 1 into k swaptions
        ftStart							start date for
all 1 into k swaptions
        DfixStart							dis
factor from tNow to common start date fstt tPay[i] for i=ffirst        ,...
,nlast common set of pay dates for 1 into k swaptions
        ffirst							pay dates of
swaption j are tPay[i] for
        nfirst							i = ffirst , ...
      , j
        nlast
for j=nfirst        , ...        , nlast
        cvgfix							coverage from
start date to first pay date cvgpay[i]        , i=ffirst+1        ,... ,nlast
coverage from tPay[i-1] to tPay[i] Dpay[i]        , i=ffirst        , ... ,
nlast	dis factor from tNow to tPay[i] Rfix[j] for j=nfirst        ,... ,nlast
fixed rate for caplet j Vfix[j]	for j=nfirst        ,...        ,nlast market
value at tNow for caplet
j
*/
/* If usecaps == 1        , the caplets must also be defined:
        nEx>1						number of reference
caplets tEx[j] for j=1        ,...        ,nEx		exercise date for caplet
j tStart[j] for j=1        ,...        ,nEx		start date for caplet j
        DStart[j] for j=1        ,...        ,nEx		dis factor from
tNow to the start date tEnd[j] for j=1        ,...        ,nEx		end date
for caplet j Dcap[j]
for j=1        ,...        ,nEx		dis factor to the end date of caplet j
cvgcap[j] for j=1        ,...        ,nEx	coverage from start date to end
date for caplet j Rfcap[j] for j=1        ,...        ,nEx	fixed rate for
caplet j Vcap[j]	for j=1        ,...        ,nEx		market value at
tNow for caplet j Note the indices i=0 and j=0 is ignored in the above arrays
*/
/* If usecaps != 1        , the long "k into n-k" swaptions must also be
   defined: nEx>1						number of
   reference
   swaptions tEx[j] for j=1        ,...        ,nEx		exercise date for
   swaption j tStart[j] for j=1        ,...        ,nEx		start date for
   swaption j
        DStart[j] for j=1        ,...        ,nEx		dis factor from
   tNow to the start date
        nPay						total number of fixed
   leg pay dates tPay[i] for i=1        ,...        ,nPay	fixed leg pay
   dates cvgpay[i] for i=1        ,...        ,nPay	coverage from tPay[i-1]
   to tPay[i] Dpay[i] for i=1        ,... ,nPay	dis factor from tNow to tPay[i]

        ifirst [j] for j=1        ,...        ,nEx	fixed leg pay dates are
   i = ifirst[j] to nlong[j] nlong [j] for j=1        ,...        ,nEx cvgfirst
   t[j] for j=1        ,...        ,nEx first period coverage (from tStart[j] to
   tPay[ifirst[j]]) Rlong [j] for j=1
         ,...        ,nEx	fixed rate for swaption j Vlong [j] for j=1 ,...
   ,nEx market price of swaption j Note the indices j=0 and i=0 are ignored in
   the above arrays
*/
LGMErr LGMCalFixExp(
    LGM_TS **LGMtsPtrPtr, /* Return: Calibrated term structure */
    ...
    LGMCalSet *CSPtr) /* Reference caplets */
{
  ...
  else if (zdatefix >= tsPtr->zdate[nEx] + 365) /* append to end */
  {
    ...
    for (j0 = 0; tsPtr->zdate[j0] <= zdatefix; j0++)
      ;
    if (keep > 0) {
      if (j0 > 1 && (abs(tsPtr->zdate[j0 - 1] - zdatefix) <
                     abs(tsPtr->zdate[j0] - zdatefix)))
        j0 = j0 - 1;
    ...
    else /* insert at j0 */
    {
      ...
      zmax = min(tsPtr->zeta[j0 - 1] + 2.5 * slope * dt,
                 tsPtr->zeta[j0] - 0.4 * slope * (dint - dt));
      zmin = max(tsPtr->zeta[j0 - 1] + 0.4 * slope * dt,
...
by calibrating to the "1 into k" swaptions and fills them into the term
structure */
LGMErr FitAllGFwdSwap(LGM_TS *tsPtr, LGMCalSet *CSPtr, Date *zdatePtr,
                      double *zetaPtr) {
  LGMErr error;
  SrtReceiverType recpay;
  ...
  for (j = nfirst; j <= nlast; j++) {
    dispayment[1] = CSPtr->Rfix[j] * CSPtr->cvgfix *
                    CSPtr->Dpay[ifirst]; /* compute discounted payments */
    for (i = ifirst + 1; i <= j; i++)    /* of fixed leg of swaption j */
      dispayment[i + 1 - ifirst] =
          CSPtr->Rfix[j] * CSPtr->cvgpay[i] * CSPtr->Dpay[i];
    n = j + 1 - ifirst;
    dispayment[n] =
        dispayment[n] + CSPtr->Dpay[j]; /* add notional to final payment */
    ...
    dummy =
        FitOneSwap(n, dispayment, CSPtr->DfixStart, recpay, /* the swaption */
                   zeta,                                    /* zeta(tEx) */
                   &Gst, &(Gpay[ind]),  /* the term structure */
                   dGst, &(dGpay[ind]), /* direction G */
...
Definition of the long "k into n-k" swaptions:
        nEx>1						number of reference
swaptions tEx[j] for j=1        ,...        ,nEx		exercise date
for swaption j
        tStart[j] for j=1        ,...        ,nEx		start date for
swaption j DStart[j] for j=1        ,...        ,nEx		dis factor from
tNow to the start date
        nPay						total number of fixed
leg pay dates tPay[i] for i=1        ,...        ,nPay	fixed leg pay dates
cvgpay[i] for i=1        ,...        ,nPay	coverage from tPay[i-1] to
tPay[i] Dpay[i] for i=1        ,... ,nPay	dis factor from tNow to tPay[i]

        ifirst [j] for j=1        ,...        ,nEx	fixed leg pay dates are
i = ifirst[j] to nlong[j] nlong [j] for j=1        ,...        ,nEx cvgfirst
t[j] for j=1        ,...        ,nEx  first period coverage (from tStart[j] to
tPay[ifirst[j]]) Rlong [j] for j=1        ,...       ,nEx fixed rate for
swaption j Vlong [j] for j=1        ,...        ,nEx	market price of swaption
j Note the indices j=0 and i=0 are ignored in the above arrays Definition of the
caplets also requires:
        tEnd[j] for j=1        ,...        ,nEx		end date for caplet j
        Dcap[j] for j=1        ,...        ,nEx		dis factor to the end
date of caplet j cvgcap[j] for j=1        ,...        ,nEx	coverage from
start date to end date for caplet j Rfcap[j] for j=1        ,...        ,nEx
fixed rate for caplet j Vcap[j] for j=1        ,...        ,nEx		market
value at tNow for caplet j */
...
static LGMErr FitAllGPairs(long npair, long *jpair, long usecaps, LGM_TS *tsPtr,
  ...
  if (dispaylong == NULL || dispayshort == NULL || Gpay == NULL ||
      dGpay == NULL) {
    srt_free(dispaylong);
  ...
  tsPtr->G[npair] = 0.0;
  tsPtr->G[npair - 1] =
      ((double)(tsPtr->Gdate[npair] - tsPtr->Gdate[npair - 1])) / 365.0;
  ...
  /* For k = npair-1        , npair-2        , ...        , 1        , for each
  j=jpair[k] find G(tStart[j]) so that the LGM value of long swaption j and
  either the LGM value
  of the caplet or short swaption agrees with their market prices */
  ...
  for (k = npair - 1; k >= 1; k--) {
    ...
    if (usecaps == 1) {
      ...
      ind = ifirst - 1;
      zeta = FitOneSwapCap(
          nsw, dispaylong, CSPtr->DStart[j], recpay, /* the swaption */
          zeta, product,       /* unused        , product of sqzeta*dGst */
          &Gst, &(Gpay[ind]),  /* the existing term structure */
          dGst, &(dGpay[ind]), /* direction of changes in G */
          CSPtr->Vlong[j]);    /* market price */
    }
    /* if usecaps!=1        , compute discounted payments of short swaption j */
      ...
      dispayshort[1] =
          CSPtr->Rfshort[j] * CSPtr->cvgfirst[j] * CSPtr->Dpay[ifirst];
      for (i = ifirst + 1; i <= nshort; i++)
        dispayshort[i + 1 - ifirst] =
            CSPtr->Rfshort[j] * CSPtr->cvgpay[i] * CSPtr->Dpay[i];
      ...
      ind = ifirst - 1;
      zeta =
          FitOneLongShort(nsw, dispaylong,          /* the swaption 1 */
                          nsw2, dispayshort,        /* the swaption 2 */
                          CSPtr->DStart[j], recpay, /* the swaptions */
                          &Gst, &(Gpay[ind]),  /* the existing term structure */
                          dGst, &(dGpay[ind]), /* direction of changes in G */
                          CSPtr->Vlong[j], CSPtr->Vshort[j]); /* market price */
    }
...
matches the market price when
        Gst = Gst + C*dGst
(G at start date) Gpay[i] = Gpay[i] + C*dGpay[i]        , i=1        , ... ,n
(G at pay dates)
...
/* The fixed leg has discounted payments a[i] = payment[i]*df(tPay[i])        ,
   i=1
      ,...        ,n. The strike is worth DStart = dis factor(tStart) */
double FitOneSwap(long n, double *a, double DStart,
                  SrtReceiverType recpay, /* the swaption */
                  double zeta,            /* zeta */
                  double *Gstptr,
                  double *Gpay,               /* the existing term structure */
                  ...
                  double price)               /* market price of swaption */
{
  ...
  /* check for intrinsic */
  /*	if (fabs (LGMRecVal(&ystar        , n        , a        , DStart
     , 1.0e-15        , Gpay        , Gst) - price) < 1.0e-08)
          {
                  return 1.0e-15;
          }*/
  ...
  for (iter = 1; iter <= itermax && success <= 0; iter++) {
    ...
    for (i = 1; i <= n; i++) {
      ...
      if (fabs(theta) <= 1.e-5 || fabs(arg) <= 1.e-4)
        newvalue =
            newvalue +
            a[i] * arg * (1. - 0.5 * arg * theta * (1.0 - arg * theta / 3.0));
      else
...
chosen        , this keeps the caplet with fixing date tEx        , start date
tStart and end date calibrated to its market value
                ...
                double price1, double price2) /* market prices of swaptions */
{
  ...
  stddev1 = favg * LGMNormImpVol(price1 / favg, fwd1 / favg, DStart / favg, 1.0,
                                 1.0, callput);

  favg = 0.5 * (fwd2 + DStart);
  stddev2 = favg * LGMNormImpVol(price2 / favg, fwd2 / favg, DStart / favg, 1.0,
                                 1.0, callput);
  ...
  for (iter = 1; iter <= itermax && success <= 0; iter++) {
    ...
    df1cz = 0.;
    for (i = 1; i <= n1; i++) {
      arg = (Gst - Gpay[i]) * stddev2 * (z + dz) +
            (dGst - dGpay[i]) * stddev2 * (cz + dcz);
      expfac = exp(-arg * theta1);
      if (fabs(arg * theta1) < 1.e-5)
        f1 = f1 +
             a1[i] * arg *
                 (1.0 - 0.5 * arg * theta1 + arg * arg * theta1 * theta1 / 6.0);
      else
    ...
    df2cz = 0;
    for (i = 1; i <= n2; i++) {
      arg = (Gst - Gpay[i]) * stddev1 * (z + dz) +
            (dGst - dGpay[i]) * stddev1 * (cz + dcz);
      expfac = exp(-arg * theta2);
      if (fabs(arg * theta2) < 1.e-5)
        f2 = f2 +
             a2[i] * arg *
                 (1.0 - 0.5 * arg * theta2 + arg * arg * theta2 * theta2 / 6.0);
      else
  ...
  for (iter = 1; iter <= itermax && success <= 0; iter++) {
    ...
    newerror = value1 * value1 / (price1 * price1) +
               value2 * value2 / (price2 * price2);
    if (newerror < 1.e-10)
      success++;
...
double LGMRecVal(double *ystar, long n, double *a,
                 double DStart, /* receiver swaption */
                 double sqzeta, double *Gpay, double Gst) /* term structure */
{
        ...
        double dGst, double *dGpay) /* direction of differentiation */
{
  ...
  der = 0.0;
  for (i = 1; i <= n; i++)
    der = der + (dGst - dGpay[i]) * a[i] *
                    LGMsafeGauss(arg + (Gst - Gpay[i]) * sqzeta);
...
/*****************************************************************************/
/* Create a term structure        , calculate G(t) from fixed kappa        ,
fill in numG        , Gdates        , G values */
/*****************************************************************************/
static LGM_TSPtr
CreateTSwithKappa(double kap,    /* Fixed kappa to use to construct G */
                  int usestarts, /* construct G on start dates or pay dates */
...
/*****************************************************************************/
/* Create a term structure        , check out G(t) from user input        , fill
in numG        , Gdates        , G values */
/*****************************************************************************/
static LGM_TSPtr CreateTSwithGs(long numG,   /* number of values of G */
                                Date *Gdate, /* construct G on these dates */
                                double *G,   /* values of G(t) */
                                LGMCalSet *CSPtr) /* Reference instruments */
{
  ...
  for (i = 1; i < numG; i++) {
    dt = (double)(tsPtr->Gdate[i] -
                  tsPtr->Gdate[i - 1]); /* G values too ragged? */
...
/*****************************************************************************/
/* Create a term structure        , compute zeta from constant sigma at the
 exercise dates        , and fill in numZ        , zdate[]        , and zeta[]
 */
/*****************************************************************************/
...
/* Create a term structure        , check out input zeta(t) and fill in numZ ,
   zdate[]        , and zeta[]   */
/*****************************************************************************/
static LGM_TSPtr CreateTSfromZetas(long nZ,      /* number of zeta values */
                                   Date *Zdate,  /* zeta given on these dates */
                                   double *Zval, /* zeta values */
                                   LGMCalSet *CSPtr) /* Reference instruments */
{

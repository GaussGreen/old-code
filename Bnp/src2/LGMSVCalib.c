...
Err	err = NULL;	
	...
	{		
		...
		{
			vol[i] = sqrt ((ex_zeta[i] - ex_zeta[i-1]) * 2 * lambda
	...
	for (i=0; i<nex-skip_last; i++)
	{
		...
		err = srt_f_optimpvol(	ex_lprice[i],
								fwd,
								ex_lstrike[i],
								ex_time[i],						
								level,
		...
		{
			alphaeq = alpha * sqrt((1.0 - exp(-2.0 * lambdaEps * ex_time[i])) / (2.0 * lambdaEps * ex_time[i]));
		}
		else
		{
			alphaeq = alpha;
		...
		if (i > 0)
		{			
			newVar = vol[i-1] * vol[i-1] * ex_time[i-1] + vol[i] * vol[i] * ex_time[i] - save_vol * save_vol * ex_time[i-1];
			save_vol = vol[i];
		...
		if (error > precision)
		{
			...
			if (i > 0)
			{
				vol2 = (ex_lprice[i] * ex_lprice[i] / price1 / price1 * newVar - Var) / (ex_time[i] - ex_time[i-1]);
			}
			else
			{
...
FREE_RETURN:
	...
	if (date) free_dvector(date, 0, nbT-1);
	if (time) free_dvector(time, 0, nbT-1);
	if (ifr) free_dvector(ifr, 0, nbT-1);
	if (is_event) free(is_event);
	if (func_parm) free (func_parm);

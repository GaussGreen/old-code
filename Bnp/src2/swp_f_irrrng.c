...
/* -----------------------------------------------------------------------
    ...
    conv = (1/comp)^3 sum[i=0;i<len] { cf[i]*cvg[i]*(cvg[i]+1)*(cvg[i]+2)/(1 + irr/comp)^(i + alpha + 3) } 
	(i.e. from d3PV/dIRR3)
   ----------------------------------------------------------------------- */
...
double  pv;
	...
	sum = 0.0;
	for(i=1;i<len;i++)
		sum += cf[i] * ((i-1) + first_period_fraction)* ( (i-1) + first_period_fraction) * exp(l * (i-1));

...
/* -------------------------------------------------------------------------
   Computes the PV of a series of cash-flows using the following formula:
        sum(i=1; i<len) { adj_cf[i]/(1+irr/comp)^i} / (1+irr/comp)^first_day_count
   where:
                - irr/comp is entered as the adjusted irr ,
                - the cashflows are the coupons adjusted by the compounding
                - first_period_fraction represents the exact fraction of period from start
                  to first coupon date computed using the coverage and the basis
   ------------------------------------------------------------------------- */

static double pv_with_irr(double adjusted_irr, double first_period_fraction, double* cf, int len)
{
    int    i;
    double sum = 0;
    double l;
...
/* -----------------------------------------------------------------------
    ...
    conv = (1/comp)^2 sum[i=0;i<len] { cf[i]*cvg[i]*(cvg[i]+1)/(1 + irr/comp)^(i + alpha + 2) }
        (i.e. from d2PV/dIRR2)
   ----------------------------------------------------------------------- */
    ...
    double*        convexity)
{
    ...
    sum = 0.0;
    for (i = 1; i < len; i++)
        sum += cf[i] * ((i - 1) + first_period_fraction) * ((i - 1) + first_period_fraction + 1) *
               exp(l * ((i - 1) + 2));
...
/* -----------------------------------------------------------------------
    ...
    conv = (1/comp)^3 sum[i=0;i<len] { cf[i]*cvg[i]*(cvg[i]+1)*(cvg[i]+2)/(1 + irr/comp)^(i + alpha
   + 3) } (i.e. from d3PV/dIRR3)
   ----------------------------------------------------------------------- */
...
/* -------------------------------------------------------------------------------
    ...
    mod_dur = sum[i=1;i<len] { cf[i]*(i+alpha)/(1 + irr/comp)^(i+alpha) } / comp / PV
        (i.e. from -(1+IRR/comp)/PV * dPV/dIRR)
   ------------------------------------------------------------------------------- */
    ...
    double*        mod_convexity)
{
    ...
    sum = 0.0;
    for (i = 1; i < len; i++)
        sum += cf[i] * ((i - 1) + first_period_fraction) * ((i - 1) + first_period_fraction) *
               exp(l * (i - 1));

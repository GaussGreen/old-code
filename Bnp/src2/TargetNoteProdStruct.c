...
char* init_TARN_AUX(TARN_Struct* tarn, TARN_AUX* aux)
{
    ...
    for (i = 0; i < tarn->deal.nCouponDates; i++)
    {
        ...
        if (tarn->deal.dFixedCvg > 0.0001)
            ...
            aux->dvCouponCvg[i] = coverage(
                tarn->deal.lvCpnStartDates[i], tarn->deal.lvCpnPayDates[i], tarn->deal.basisCpn);
        ...
        if (tarn->deal.lvCpnPayDates[i] >= tarn->market.lToday)
        {
            if (err = tarn->market.getDF(
                    ...
                    tarn->market.lToday,
                    (double)tarn->deal.lvCpnPayDates[i],
                    &dDF))
                return err;
    ...
    for (i = 0; i < tarn->deal.nFundDates; i++)
    {
        ...
        if (tarn->deal.lvFundStartDates[i] >= tarn->market.lToday)
        {
            if (err = tarn->market.getDF(
                    tarn->market.szYieldCurve,
                    tarn->market.lToday,
                    (double)tarn->deal.lvFundStartDates[i],
                    &dDF))
                return err;
        ...
        if (tarn->deal.lvFundEndDates[i] >= tarn->market.lToday)
        {
            if (err = tarn->market.getDF(
                    tarn->market.szYieldCurve,
                    tarn->market.lToday,
                    (double)tarn->deal.lvFundEndDates[i],
                    &dDF))
                return err;
    ...
    /* Loop over the coupons calculating the floating periods that can be calculated .
            A floating payment belongs to period n if it starts strictly before the nth coupon
       payment date */
    iFund                 = 0;
    ...
    if (tarn->deal.fund_ccy == 1)
    {
        ...
        double dNotRatio =
            tarn->deal.dFundingNotional / tarn->deal.dCpnNotional * tarn->market.fx_fund_dom;
        ...
        /*		if ( i < tarn->deal.nFundDates )
           ...
           tarn->market.lToday, tarn->deal.lvFundStartDates[i], &dFundDF ) ) goto FREE_RETURN; if (
           err = tarn->market.getDF( tarn->market.szYieldCurve, tarn->market.lToday,
                                                                                                tarn->deal.lvFundStartDates[i],
           &dDomDF ) ) goto FREE_RETURN; aux->dHistFundPV = tarn->deal.dFundingNotional /
        ...
        */
        /*
                        if ( err = tarn->market.getDF( tarn->market.szFundYieldCurve,
           tarn->market.lToday, tarn->deal.lvFundStartDates[aux->ivFundStartIndex[aux->i1stCpn+1]],
           &dFundDF ) ) goto FREE_RETURN; if ( err = tarn->market.getDF( tarn->market.szYieldCurve,
           tarn->market.lToday, tarn->deal.lvFundStartDates[aux->ivFundStartIndex[aux->i1stCpn+1]],
           &dDomDF ) ) goto FREE_RETURN; aux->dHistFundPV = tarn->deal.dFundingNotional /
           tarn->deal.dCpnNotional * dFundDF * tarn->market.fx_fund_dom
                                                                        - dDomDF;
        */
        ...
        /* calculate the potential notional exchange at each of the KO settle dates and store it in
         * the KO fee vector */
        /*		for ( i=aux->i1stCpn; i<tarn->deal.nCouponDates; i++ )
                        {

                                if ( tarn->deal.lvFundEndDates[aux->ivFundEndIndex[i]] >=
           tarn->market.lToday )
                                {
                                        if ( err = tarn->market.getDF(
           ...
           tarn->market.lToday, tarn->deal.lvFundEndDates[aux->ivFundEndIndex[i]], &dDomDF ) ) goto
           FREE_RETURN; aux->dvKnockOutFee[i] = 1.0 - tarn->deal.dFundingNotional /
           tarn->deal.dCpnNotional * dFundDF / dDomDF
        ...
        for (i = 0; i < tarn->deal.nFundDates; i++)
        {
            if (tarn->deal.lvFundFixingDates[i] < lFirstFixing)
            {
                if (tarn->deal.lvFundEndDates[i] >= tarn->market.lToday)
                {
                    ...
                    tarn->deal.dvFundHistFixings[i] =
                        (tarn->deal.dvFundHistFixings[i] + tarn->deal.dvFundMargin[i]) * dFundDF /
                        dDomDF * dNotRatio;
            ...
            if (tarn->deal.lvFundFixingDates[i] >= lFirstFixing)
            {
                ...
                tarn->deal.dvFundMargin[i] =
                    (tarn->deal.dvFundMargin[i] + tarn->deal.dvFundSpread[i]) * dFundDF / dDomDF *
                    dNotRatio;
...
char* alloc_TARN_MC_AUX_2F(TARN_Struct* tarn, TARN_AUX* aux, TARN_MC_AUX* mc_aux)
{
    ...
    if (err = Get_LGM2F_TermStructure(
            mc_aux->und_name, &sig_date, &sig, &num_sig, &tau, &alpha, &gamma, &rho))
        goto FREE_RETURN;
    ...
    int*        stop_path)
{
    ...
    *stop_path = 0;
    ...
    /* If the cumulative coupon exceeds the strike - spread * notional, calculate the notional
     * repayment */

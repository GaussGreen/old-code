...
/* Generate the coverages and DF */
	...
	aux->dvCouponPayDF = (double*) calloc( tarn->deal.nCouponDates, sizeof(double) );

	aux->dvKnockOutFee = (double*) calloc( tarn->deal.nCouponDates, sizeof(double) );
	for ( i=0; i< tarn->deal.nCouponDates; i++ )
	{
		...
		if ( tarn->deal.dFixedCvg > 0.0001 )
			...
			aux->dvCouponCvg[i] = coverage( tarn->deal.lvCpnStartDates[i], tarn->deal.lvCpnPayDates[i], 
												tarn->deal.basisCpn );
		aux->dvCumulCvg[i] = aux->dvCouponCvg[i];
		if ( tarn->deal.lvCpnPayDates[i] >= tarn->market.lToday )
		{
			if ( err = tarn->market.getDF( tarn->market.szYieldCurve, tarn->market.lToday, 
	...
	aux->dvFundDomStartDF = (double*) calloc( tarn->deal.nFundDates, sizeof(double) );
	aux->dvFundForStartDF = (double*) calloc( tarn->deal.nFundDates, sizeof(double) );
	for ( i=0; i< tarn->deal.nFundDates; i++ )
	{
		aux->dvFundCvg[i] = coverage( tarn->deal.lvFundStartDates[i], tarn->deal.lvFundEndDates[i], 
											tarn->deal.basisFund );
		...
		if ( tarn->deal.lvFundStartDates[i] >= tarn->market.lToday )
		{
			...
			if ( err = tarn->market.getDF( tarn->market.szFundYieldCurve, tarn->market.lToday, 
															(double) tarn->deal.lvFundStartDates[i], &dDF )  )
				return err;
...
/* Loop over the coupons calculating the floating periods that can be calculated .
	A floating payment belongs to period n if it starts strictly before the nth coupon payment date */
	iFund = 0;
	...
	{		
		...
		while ( iFund < tarn->deal.nFundDates && tarn->deal.lvFundStartDates[iFund] < tarn->deal.lvCpnPayDates[i] )
			iFund++;
...
/* The last period gets the final bit */
	...
	{
		aux->ivFundStartIndex[tarn->deal.nCouponDates-1] = aux->ivFundEndIndex[tarn->deal.nCouponDates-2] + 1;
		aux->ivFundEndIndex[tarn->deal.nCouponDates-1] = tarn->deal.nFundDates - 1;
	}
...
/* check if today is a fixing date */
	...
	for ( i=0; i<tarn->deal.nCouponDates; i++ )
		if ( tarn->deal.lvFltrFixingDates[i] == tarn->market.lToday ) aux->iIsTodayFixing = 1;
...
//		double eq_final_ex, eq_init_ex, dFundingPV, dCouponPV;
		...
		double dNotRatio = tarn->deal.dFundingNotional / tarn->deal.dCpnNotional * tarn->market.fx_fund_dom;

/* Find the first funding coupon not yet fixed */
		i=0;
		while ( i < tarn->deal.nFundDates && tarn->deal.lvFundFixingDates[i] <= lFirstFixing )
			++i;
...
/*		if ( i < tarn->deal.nFundDates )
		{
			...
			if ( err = tarn->market.getDF( tarn->market.szYieldCurve, tarn->market.lToday, 
				...
				goto FREE_RETURN;
			aux->dHistFundPV = tarn->deal.dFundingNotional / tarn->deal.dCpnNotional * dFundDF * tarn->market.fx_fund_dom 
									- dDomDF;
		}
*/
/*
...
/* calculate the potential notional exchange at each of the KO settle dates and store it in the KO fee vector */
/*		for ( i=aux->i1stCpn; i<tarn->deal.nCouponDates; i++ )
		{

			if ( tarn->deal.lvFundEndDates[aux->ivFundEndIndex[i]] >= tarn->market.lToday )
			{
				if ( err = tarn->market.getDF( tarn->market.szFundYieldCurve, tarn->market.lToday,
				...
				if ( err = tarn->market.getDF( tarn->market.szYieldCurve, tarn->market.lToday,
					...
					goto FREE_RETURN;
				aux->dvKnockOutFee[i] = 1.0 - tarn->deal.dFundingNotional / tarn->deal.dCpnNotional * dFundDF / dDomDF 
		...
		for ( i=0; i<tarn->deal.nFundDates; i++ )
		{
			if ( tarn->deal.lvFundFixingDates[i] < lFirstFixing )
			{
				if ( tarn->deal.lvFundEndDates[i] >= tarn->market.lToday )
				{
					...
					if ( err = tarn->market.getDF( tarn->market.szYieldCurve, tarn->market.lToday,
						...
						goto FREE_RETURN;
					tarn->deal.dvFundHistFixings[i] = ( tarn->deal.dvFundHistFixings[i] + tarn->deal.dvFundMargin[i] ) 
			...
			if ( tarn->deal.lvFundFixingDates[i] >= lFirstFixing )
			{
				...
				if ( err = tarn->market.getDF( tarn->market.szYieldCurve, tarn->market.lToday,
												tarn->deal.lvFundEndDates[i], &dDomDF ) )
					goto FREE_RETURN;
				tarn->deal.dvFundMargin[i] = ( tarn->deal.dvFundMargin[i] + tarn->deal.dvFundSpread[i] ) 
												* dFundDF / dDomDF * dNotRatio;
...
/*		if ( err = convert_funding_to_domestic(
								tarn->market.lToday,
								tarn->deal.lvFundStartDates[aux->i1stCpn],
								tarn->market.eodFixFlag,
								tarn->market.eodPayFlag,
								tarn->market.fx_fund_dom,
								tarn->market.fx_fund_dom_spot_date,
								tarn->deal.dCpnNotional,
								tarn->market.szYieldCurve,
								tarn->deal.nFundDates,
								tarn->deal.lvFundFixingDates,
								tarn->deal.lvFundStartDates,
								tarn->deal.lvFundEndDates,
								tarn->deal.szvBasisFund,
								tarn->market.szFundYieldCurve,		
								&tarn->deal.dFundingNotional,
								tarn->deal.dvFundSpread,
								tarn->deal.dvFundMargin,
								tarn->deal.dvFundHistFixings,
								&lStartDate,
								&eq_final_ex,
								&eq_init_ex)  )
			goto FREE_RETURN;

		TARGET_NOTE.m_dFundingNotionalExchange = eq_init_ex - eq_final_ex;
		TARGET_NOTE.m_dCouponNotionalExchange = tarn->deal.dCpnNotional;
*//*
...
/* Free the allocated memory */
	if ( aux->dvCouponCvg ) free_and_zero( &aux->dvCouponCvg );
	if ( aux->dvCouponPayDF ) free_and_zero( &aux->dvCouponPayDF );
	if ( aux->dvFundCvg ) free_and_zero( &aux->dvFundCvg );
	free_and_zero( &aux->dvFundDomPayDF );
	free_and_zero( &aux->dvFundDomStartDF );
	free_and_zero( &aux->dvFundForStartDF );
	if ( aux->ivFundStartIndex ) free_and_zero( &aux->ivFundStartIndex );
	if ( aux->ivFundEndIndex ) free_and_zero( &aux->ivFundEndIndex );
...
// set the cumulative coupon to zero
	...
	TARGET_NOTE.m_dCouponNotionalRepaid = TARGET_NOTE.tarn->output.iIsKnockedOut > 0 ? 1 : 0;
	TARGET_NOTE.m_dPrevAccrual = 1.0;
}
/* Setup function for the global variable */
/* ----------------------------------------------------------------------------------------------------------------- */
...
char* alloc_TARN_MC_AUX(	TARN_Struct* tarn, 
							TARN_AUX* aux, 
							TARN_MC_AUX* mc_aux )
{
/* Variable declaration */
...
char* alloc_TARN_MC_AUX_2F( TARN_Struct* tarn, TARN_AUX* aux, TARN_MC_AUX* mc_aux )
{
/* Variable declaration */
...
/* Get the term structure */
	if ( err = Get_LGM2F_TermStructure(		mc_aux->und_name,
											&sig_date,
											&sig,
											&num_sig,
											&tau,
...
{
	if (event->fund_start_gam1) free_dvector (event->fund_start_gam1, 0, event->num_fund_df-1);
	if (event->fund_start_gam2) free_dvector (event->fund_start_gam2, 0, event->num_fund_df-1);
	if (event->fund_start_gam12) free_dvector (event->fund_start_gam12, 0, event->num_fund_df-1);
	if (event->fund_end_gam1) free_dvector (event->fund_end_gam1, 0, event->num_fund_df-1);
	if (event->fund_end_gam2) free_dvector (event->fund_end_gam2, 0, event->num_fund_df-1);
	if (event->fund_end_gam12) free_dvector (event->fund_end_gam12, 0, event->num_fund_df-1);
}
...
void free_TARN_EVENT_SV( TARN_EVENT_SV* event )
{
	if ( event->fund_start_gam1 ) free_dvector( event->fund_start_gam1, 0, event->num_fund_df-1 );
	if ( event->fund_start_gam2 ) free_dvector( event->fund_start_gam2, 0, event->num_fund_df-1 );
	if ( event->fund_start_gam1_2 ) free_dvector( event->fund_start_gam1_2, 0, event->num_fund_df-1 );
	if ( event->fund_start_gam2_2 ) free_dvector( event->fund_start_gam2_2, 0, event->num_fund_df-1 );
	if ( event->fund_start_gam12 ) free_dvector( event->fund_start_gam12, 0, event->num_fund_df-1 );
	if ( event->fund_end_gam1 ) free_dvector( event->fund_end_gam1, 0, event->num_fund_df-1 );
	if ( event->fund_end_gam2 ) free_dvector( event->fund_end_gam2, 0, event->num_fund_df-1 );
	if ( event->fund_end_gam1_2 ) free_dvector( event->fund_end_gam1_2, 0, event->num_fund_df-1 );
	if ( event->fund_end_gam2_2 ) free_dvector( event->fund_end_gam2_2, 0, event->num_fund_df-1 );
	if ( event->fund_end_gam12 ) free_dvector( event->fund_end_gam12, 0, event->num_fund_df-1 );
}
...
{		
	if (event->fund_start_tms) free_dvector (event->fund_start_tms, 0, event->num_fund_df-1);
	if (event->fund_start_dts) free_dvector (event->fund_start_dts, 0, event->num_fund_df-1);
	if (event->fund_start_logdff) free_dvector (event->fund_start_logdff, 0, event->num_fund_df-1);
	if (event->fund_start) free_dvector (event->fund_start, 0, event->num_fund_df-1);
	if (event->fund_end_tms) free_dvector (event->fund_end_tms, 0, event->num_fund_df-1);
	if (event->fund_end_dts) free_dvector (event->fund_end_dts, 0, event->num_fund_df-1);
	if (event->fund_end_logdff) free_dvector (event->fund_end_logdff, 0, event->num_fund_df-1);
	if (event->fund_end) free_dvector (event->fund_end, 0, event->num_fund_df-1);
	if (event->fund_cvgMargin) free_dvector (event->fund_cvgMargin, 0, event->num_fund_df-1);
	if ( event->ptrModel ) 
...
/* If it is the final period, then pay the final coupon */
	...
	{
		res[1] = TARGET_NOTE.m_dCouponNotional * event->fixed_pay * TARGET_NOTE.m_dTarget;
		*stop_path = 1;
...
/* If the cumulative coupon exceeds the strike - spread * notional, calculate the notional repayment */
...
{
/* If it is the final period and the coupon is guaranteed, then calculate the coupon */
	if ( event->iIsFinal && TARGET_NOTE.tarn->deal.bRepayCoupon )
	{
		res[1] = TARGET_NOTE.m_dCouponNotional * event->fixed_pay 
					* /*event->fixed_cvg **/ ( TARGET_NOTE.m_dTarget - TARGET_NOTE.m_dCumulCoupon );
		*stop_path = 1;
		return 0;
	}
...
/* If the cumulative coupon exceeds the strike - spread * notional, calculate the notional repayment */
	...
	{
		double dIncPay = dNotionalRepay - TARGET_NOTE.m_dCouponNotionalRepaid;
		res[1] = dIncPay * /* event->fixed_cvg * */ ( TARGET_NOTE.m_dTarget - TARGET_NOTE.m_dCumulCoupon )
					* event->fixed_pay;		
		TARGET_NOTE.m_dCouponNotionalRepaid = dNotionalRepay;
		TARGET_NOTE.m_dCouponNotional = 1.0 - dNotionalRepay;
		TARGET_NOTE.m_dFundingNotional = TARGET_NOTE.m_dCouponNotional;
...
/* Calculate any coupon payments for the TN_SWAP case */
	res[1] += TARGET_NOTE.m_dCouponNotional * event->fixed_pay * event->fixed_cvg * dCoupon;
...
/* if the target is exceeded pay the remaining coupon and cancel the deal */
	if ( dNotionalRepay >= 1.0 )
	{
		res[1] = event->fixed_pay * event->fixed_cvg * dCoupon * TARGET_NOTE.m_dCouponNotional;
		res[0] += event->dKnockOutFee * event->fixed_pay;
		*stop_path = 1;
		return 0;
	}

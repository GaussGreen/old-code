...
Err OptFXSVMC(char *fxundname, double gamma, double alpha, double **rho, long mat, double *K, int nK,
			  long npaths, int nsteps, double *res, double *std)
{
	Err			err = NULL;
	...
	if (!und) { err = serror("Couldn't find underlying named %s", fxund);  goto FREE_RETURN; }
	if (get_underlying_type(und) != FOREX_UND)
	{ err = serror ("Underlying %s is not of type FX", fxund);  goto FREE_RETURN; }
	if (get_mdltype_from_fxund(und) != FX_STOCH_RATES)
	{ err = serror ("Underlying %s is not of type FX Stoch Rates", fxund); goto FREE_RETURN; }
	...
	while (times[ntimes-1] > ExTime + 1e-5) ntimes--;
	...
	ffx0 = get_spot_from_fxund(und) * swp_f_df(spot_date, mat, for_yc) / swp_f_df(spot_date, mat, dom_yc);
	...
	if (!sv || !chol || !pvs) { err = serror("Memory failure");  goto FREE_RETURN; }
	...
	for (i=0; i < ntimes-1; i++)
	{
		...
		sqrtdt = sqrt(dt);
		while ( idx_d < nsig_d-1 && sigtms_d[idx_d] < times[i] + 1e-5 ) idx_d++;
		while ( idx_f < nsig_f-1 && sigtms_f[idx_f] < times[i] + 1e-5 ) idx_f++;
		while ( idx_x < nsig_x-1 && sigtms_x[idx_x] < times[i] + 1e-5 ) idx_x++;

		Gamma_d = -sig_d[idx_d] * ( 1.0 - exp(-lam_d * (MatTime - times[i])) ) / lam_d;
		Gamma_f = -sig_f[idx_f] * ( 1.0 - exp(-lam_f * (MatTime - times[i])) ) / lam_f;

		V12 = Gamma_f*Gamma_f/2.0 + Gamma_d*Gamma_d/2.0 - rho[0][1]*Gamma_f*Gamma_d;
		...
		for (j=0; j < npaths; j++)
		{
			...
			sv[j][0] += -V123 * dt + sig_x[idx_x] * sv[j][1] * cbr[2] + Gamma_f * cbr[1] - Gamma_d * cbr[0];
			sv[j][1] += gamma * (1.0 - sv[j][1]) * dt + alpha * cbr[3];
			sv[j][2] += -V123_ln * dt + sig_x[idx_x] * cbr[2] + Gamma_f * cbr[1] - Gamma_d * cbr[0];
		}
	}
	...
	for (k=0; k < nK; k++)
	{
		...
		// Calculate std
		std[k] = 0.0;
		for (j=0; j < npaths; j++) std[k] += (pvs[j] - res[k]) * (pvs[j] - res[k]) / (npaths-1);
...
FREE_RETURN:
	free(sig_d);	free(sigtms_d);
	free(sig_f);	free(sigtms_f);
	free(sig_x);	free(sigtms_x);
	free(times);

	ABS_Free(&rg);
	if (sv) free_dmatrix(sv, 0, npaths-1, 0, 2);

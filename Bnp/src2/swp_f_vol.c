/* -------------------------------------------------------------------------
   ...
   PURPOSE		: compute volatilities via a pointer to a function VolFunc
...
/* --------------------------------------------------------------------------- 
	Calculate volatility , using the SABRVolFunc that has been attached to the 
	library (see srt_f_external_fct.c)
  --------------------------------------------------------------------------- 
	case 0:"ATMLOGNORMAL";
	case 1:"ATMNORMAL";
...
/*	if (component == 9)
	{
		err = sabrvol_func(vol_id, start, end, strike, &dtemp, power, (int)8);
		if (err) return err;
...
/* --------------------------------------------------------------------------- 
  ...
  --------------------------------------------------------------------------- */
Err 	swp_f_transformvol( char* yc_id, char* vol_id, 
						   char* freq_std, char* basis_std,
						   char* refRateCode,
						   Ddate start, Ddate end, double strike,  
						   char* freq, char* basis,
						   double* volatility, double *power )
{		
Err			    err;
VolFuncType		vol_func;
...
/* Gets volatility function attached to the library */
	...
	err = swp_f_ForwardRate((long)(start), (long)(end), freq_std, basis_std, yc_id, refRateCode, &R_std);
	err = swp_f_ForwardRate((long)(start), (long)(end), freq, basis, yc_id, refRateCode, &R);
...
/* --------------------------------------------------------------------------- 
	Calculate non standard volatility , using Pat Hagan's trick
  --------------------------------------------------------------------------- */
Err 	swp_f_transformvol_func( char* yc_id, 
						    char* vol_id,
...
Err 	swp_f_IsSmileVol( char* vol_id, double* yesno) // return 1 if Model is SABR or SABRAFor any smile model.
{
	double dontcare;

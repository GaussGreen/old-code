...
Err analyze_und_info(SrtUndInfo *und_info) {
  ...
  /* there should at least be one underlying returned from the call        ,
     which is the domestic underlying */
  ...
  /* assume that a jumping discretisation is used        , until we know
   * otherwise */
  ...
  for (i = 0; i < und_info->no_of_underlyings; i++) {
    ...
    else /* stochastic */
    {
      ...
      if (mdl_dim == TWO_FAC) {
        ...
        /* Stochastic volatility model not implemented in Multi-Underlying Exept
         * in EQ_STOCH_RATES_SRVGS (fudge) */
        if ((!(mdl_type == EQ_STOCH_RATES_SRVGS)) &&
            (!(mdl_type == LGM_STOCH_VOL))) {
          if (und_info->no_of_underlyings > 1)
            return (serror(
                "Stochastic Volatility Model not available in Multi Asset"));
          else {
    ...
    if ((mdl_type == CHEY) || (mdl_type == CHEY_BETA))
      ...
      und_info->jumping = SRT_NO;
        ...
        /* OVE && REGIS && JL: FX_STOCH_RATES model does not allow jumping
           numeraire YET */
  ...
  if (und_info->no_of_brownians > 1 && und_info->use_corr_ts == SRT_YES) {
    if (und_info->two_factor_model == SRT_YES ||
        und_info->use_stochastic_vol == SRT_YES) {
      ...
      err = srt_f_make_deal_corrlist(
          used_und_names, und_info->no_of_underlyings,
          "Local deal correlation list", srt_f_GetTheCorrelationList(), &cls);
      if (err) {
...
Err add_more_underlyings_to_und_info(SrtUndInfo *und_info) {
  ...
  if (und == NULL) {
    return serror("Underlying %s not initialise",
                  und_info->und_data[0].und_name);
  }
  ...
  */
  ...
  for (i = 0; i < und_info->no_of_underlyings; i++) {
    ...
    if ((mdl_type == EQ_STOCH_RATES) || (mdl_type == EQ_STOCH_RATES_SRVGS)) {
      ...
      /* If EQ_UND is P&L underlying        , change NUMERAIRE index to domestic
       * IR_UND for discount */
      if (i == 0)
    ...
    /* FX_UND with stochastic rates : dom and for IR_UND need to be there */
      ...
      /* If FX_UND is P&L underlying        , change NUMERAIRE index to domestic
       * IR_UND for discount */
      if (i == 0)
        und_info->numeraire_index = j;
    ...
    if ((und_type == FOREX_UND) && (mdl_type == FX_STOCH_RATES)) {
      ...
      if (strcmp(und_ccy, dom_ccy_str) != 0) {
        ...
        /* Go through the correlation list to find an FX und with both
         * currencies */
        for (k = 0; k < corrval->nund; k++) {
          /* Get the underlying associated to the k-th name in the correlation
           * list */
          ...
          if (und_type == FOREX_UND) {
            ...
            if (((strcmp(tempdomccy, dom_ccy_str) == 0) &&
              ...
              for (j = 0; j < und_info->no_of_underlyings; j++) {
                if (strcmp(und_info->und_data[j].und_name,
                           corrval->und_names[k]) == 0)
                  break;
              }

              /* OVE: the FX underlying should not be added to the list of
                 underlyings to be diffused if it is not there
                                                              /* It is not in
                 the list        , we add it ( it will have to be diffused...)
                 if (j
                 >= und_info->no_of_underlyings)
                                                              {
                                                                      strcpy(und_info->und_data[und_info->no_of_underlyings].und_name
        ...
        if (found == SRT_NO) {
          return serror("Need a FOREX underlying [%s/%s]", dom_ccy_str,
                        und_ccy);
        }

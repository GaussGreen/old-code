...
static Err funcs1(double x, double a[], double *yfit, int ma) {
  int i, j;
  double *temp_fvol;
  double temp_ivol;
  ...
  /*	Bounds */
  if (do_bnd_ && j >= ndata_bnd_first_ &&
      j < ndata_bnd_first_ + ndata_bnd_num_) {
    /*	Number of bnd equation */
    j -= ndata_bnd_first_;

    if (*(ndata_bnd_val_[j]) < ndata_bnd_min_[j]) {
      *yfit = ndata_bnd_min_[j] - *(ndata_bnd_val_[j]);
    } else if (*(ndata_bnd_val_[j]) > ndata_bnd_max_[j]) {
  ...
  /*	Implied volatility */
  if (do_ivol_ && j >= ndata_ivol_first_ &&
      j < ndata_ivol_first_ + ndata_ivol_num_) {
    /*	Number of ivol equation */
    j -= ndata_ivol_first_;

    if (do_calib_ && num_calib_) {
      /*	Calibration */
      err = Fx3DtsCalibration(opt_exe_, opt_mat_, ivol_, num_calib_, &ONE, 1,
                              sig_dom_, *lam_dom_, sig_for_, *lam_for_,
                              *rho_dom_for_, *rho_dom_fx_, *rho_for_fx_,
                              &temp_fvol);
      ...
      for (i = num_calib_; i < num_opt_; i++) {
        if (ext_) {
          fvol_[i] =
              fvol_[num_calib_ - 1] *
                  exp(-exta_ * (opt_mat_[i] - opt_mat_[num_calib_ - 1])) +
              extb_ * (1.0 -
                       exp(-exta_ * (opt_mat_[i] - opt_mat_[num_calib_ - 1])));
        } else {
          fvol_[i] = fvol_[i - 1];
    ...
    err = Fx3DtsImpliedVol(opt_exe_[ndata_ivol_first_opt_ + j], 0.0,
                           opt_mat_[ndata_ivol_first_opt_ + j], &ONE, 1,
                           sig_dom_, *lam_dom_, sig_for_, *lam_for_, opt_mat_,
                           fvol_, num_opt_, *rho_dom_for_, *rho_dom_fx_,
                           *rho_for_fx_, yfit);
  ...
  if (do_ivol_bnd_ && j >= ndata_ivol_bnd_first_ &&
      j < ndata_ivol_bnd_first_ + ndata_ivol_bnd_num_) {
    ...
    err = Fx3DtsImpliedVol(opt_exe_[ndata_ivol_bnd_first_opt_ + j], 0.0,
                           opt_mat_[ndata_ivol_bnd_first_opt_ + j], &ONE, 1,
                           sig_dom_, *lam_dom_, sig_for_, *lam_for_, opt_mat_,
                           fvol_, num_opt_, *rho_dom_for_, *rho_dom_fx_,
                           *rho_for_fx_, &temp_ivol);
    ...
    } else if (temp_ivol > offer_[ndata_ivol_bnd_first_opt_ + j]) {
  ...
  if (do_fvol_ && j >= ndata_fvol_first_ &&
      j < ndata_fvol_first_ + ndata_fvol_num_) {
...
static Err funcs2(double x, double a[], double *yfit, double dyda[], int ma) {
  int i;
  double y;
  Err err;
...
Err Fx3DDefImpVol(/*	Total number of options */
                  ...
                  int num_calib,
                  /*	Implied volatility */
                  double *opt_mat, double *ivol, double *bid, double *offer,
                  /*	Model parameters and bounds */
                  double *fvol, double *sig_dom, double sig_dom_min,
                  double sig_dom_max, double *sig_for, double sig_for_min,
                  double sig_for_max, double *lam_dom, double lam_dom_min,
                  double lam_dom_max, double *lam_for, double lam_for_min,
                  double lam_for_max, double *rho_dom_for,
                  double rho_dom_for_min, double rho_dom_for_max,
                  double *rho_dom_fx, double rho_dom_fx_min,
                  double rho_dom_fx_max, double *rho_for_fx,
                  double rho_for_fx_min, double rho_for_fx_max, int ext,
                  double exta, double extb,
                  /*	Number of iterations */
                  int num_iter,
                  /*	What to optimise on */
                  int opt_sigma, int opt_lambda, int opt_correl,
                  int force_eq_sig, int force_eq_lam,
                  /*	Method 1 to 5 */
                  int method) {
  ...
  if (method == 1 || method == 2 || method == 5) {
    ...
    err = Fx3DtsCalibration(opt_exe_, opt_mat_, ivol_, num_opt_, &ONE, 1,
                            sig_dom_, *lam_dom_, sig_for_, *lam_for_,
                            *rho_dom_for_, *rho_dom_fx_, *rho_for_fx_,
                            &temp_fvol);
      ...
      if (ext_) {
        fvol_[i] =
            fvol_[num_calib_ - 1] *
                exp(-exta_ * (opt_mat_[i] - opt_mat_[num_calib_ - 1])) +
            extb_ *
                (1.0 - exp(-exta_ * (opt_mat_[i] - opt_mat_[num_calib_ - 1])));
      } else {
        fvol_[i] = fvol_[i - 1];
      }
    }
  ...
  if (ndata_ >= 1 && ma_ >= 1) {
    err = levenberg_marquardt(data - 1, target - 1, weight - 1, ndata_,
                              parm - 1, ma_, num_iter, funcs2, &chisq);
  }
  ...
  if (!err) {
    ...
    for (i = 0; i < num_opt_; i++) {
      err = Fx3DtsImpliedVol(opt_exe_[i], 0.0, opt_mat_[i], &ONE, 1, sig_dom_,
                             *lam_dom_, sig_for_, *lam_for_, opt_mat_, fvol_,
                             num_opt_, *rho_dom_for_, *rho_dom_fx_,
                             *rho_for_fx_, &(ivol_[i]));
    }
  }
...
static Err Get_Calib_LGM_Underlying(
    SrtUndPtr undptr, char *und_name, double tau, long today, long spot_date,
    double maturity_date, char *yc, char *ref_rate, char *basis, char *cpd,
    char *ccy, char *call_freq, long last_opt_date,
    Err (*pfGetVol)(double dStart, double dEnd, double dStrike, double dForward,
                    double dSpread, double *pdBsVol),
    char *log_nor, SrtGrfnParam *grfnparam)

{
  /* params of calboot */
  double **vol_curve = NULL, **tau_curve = NULL, *undPrice = NULL,
         *strike = NULL, *bondStrike = NULL;

  char **paramStrings = NULL, **valueStrings = NULL, **refRateStr = NULL;

  SrtReceiverType *recPayStr = NULL;

  int nInput, i, *type = NULL;
  ...
  if (maturity_date_adj < (today + 365.0 / 2.0)) {
    err = "Maturity date is smaller than 6m from today        , we cannot "
          "calibrate";
    goto FREE_RETURN;
  }
  ...
  err = SrtInitIRUnd(und_name, yc, "LGM1F", 1, 3, vol_curve, 1, 2, tau_curve, 0,
  ...
  if (nInput == 1) {
    ...
    /* get the schedule */
    err = swp_f_initSwapDP(call_start, call_end, call_freq, basis, &Swap);
    if (err)
      goto FREE_RETURN;
  ...
  for (i = 0; i < nInput; i++) {
    if (nInput > 1) {
      ...
      err = swp_f_initSwapDP(dateList.date[i + 1], call_end, cpd, basis,
                             &(sdp[i]));

      if (err)
        goto FREE_RETURN;

      err = swp_f_ForwardRate(dateList.date[i + 1], call_end, cpd, basis, yc,
                              ref_rate, &(strike[i]));
      ...
      err = pfGetVol(dateList.date[i + 1], call_end, strike[i], strike[i],
                     Spread, &vol);
      if (err)
        goto FREE_RETURN;

      err = swp_f_Swaption(dateList.date[i + 1], call_end, cpd, basis, vol,
                           strike[i], "PAY", ref_rate, yc, "PREMIUM", log_nor,
                           &(undPrice[i]));
      if (err)
        goto FREE_RETURN;
    } else {
      ...
      if (err)
        goto FREE_RETURN;
      err = swp_f_ForwardRate(call_start, call_end, cpd, basis, yc, ref_rate,
                              &(strike[i]));
      if (err)
        goto FREE_RETURN;
      ...
      if (err)
        goto FREE_RETURN;
      err = swp_f_CapFloor(call_start, call_end, strike[i], pfGetVol, "CAP",
                           ref_rate, yc, "PREMIUM", log_nor, &(undPrice[i]));
      if (err)
        goto FREE_RETURN;
    }
  ...
  err = srt_f_bootstrap_calibrate(undptr, grfnparam, sdp, strike, bondStrike,
                                  type, recPayStr, refRateStr, undPrice,
                                  undPrice, tau, 0, 0, 0, &nInput);
...
Err Fx3DDefImpVol2(/*	Total number of options */
                   ...
                   /*	Implied volatility */
                   double *opt_exe, double *opt_mat, double *ivol,

                   /*	Market parameters */
                   long today, long spot_date_dom, long spot_date_for,
                   long maturity_date, char *dom_yc, char *vol_dom,
                   char *ref_rate_dom, char *basis_dom, char *cpd_dom,
                   char *ccy_dom, char *for_yc, char *vol_for,
                   char *ref_rate_for, char *basis_for, char *cpd_for,
                   char *ccy_for,

                   /* Model parameters */
                   double lam_dom, double lam_for, double rho_dom_for,
                   double rho_dom_fx, double rho_for_fx,

                   /* Extrapolation parameters */
                   double vol_lim, double vol_lam,

                   /* Calibration parameters */
                   char *call_freq, double dom_vol_shift, double for_vol_shift,
  ...
  double *sigma_date_dom = NULL, *sigma_dom = NULL, *sigma_date_for = NULL,
         *sigma_for = NULL, *merge_dates = NULL, *sig_dom = NULL,
         *sig_for = NULL;
  ...
  err = old_cpd_calib_diagonal_wrapper(
      dom_yc, vol_dom, ref_rate_dom, get_cash_vol, dom_vol_shift, 1, nb_exe,
      ex_date, maturity_calib, NULL, NULL, 0, 1.0, 1.0, cpd_dom, basis_dom, 1,
      0, 0, 0, 0, 0, 0, NULL, &lam_dom, 1, 0.0, 0.0, 0.0, &sigma_n_dom,
  ...
  err = old_cpd_calib_diagonal_wrapper(
      for_yc, vol_for, ref_rate_for, get_cash_vol, for_vol_shift, 1, nb_exe,
      ex_date, maturity_calib, NULL, NULL, 0, 1.0, 1.0, cpd_for, basis_for, 1,
      0, 0, 0, 0, 0, 0, NULL, &lam_for, 1, 0.0, 0.0, 0.0, &sigma_n_for,
  ...
  err = Fx3DtsCalibration(opt_exe, opt_mat, ivol, num_calib, merge_dates,
                          nb_merge_dates, sig_dom, lam_dom, sig_for, lam_for,
                          rho_dom_for, rho_dom_fx, rho_for_fx, &fx_vol_curve);
  ...
  err = Fx3DtsImpliedVolExtrapol(
      opt_maturity, 0, exe_maturity, merge_dates, nb_merge_dates, sig_dom,
      lam_dom, sig_for, lam_for, opt_exe, fx_vol_curve, num_calib, vol_lim,
      vol_lam, rho_dom_for, rho_dom_fx, rho_for_fx, vol);
...
Err Fx3DDefImpVol2_corr(/*	Total number of options */
                        ...
                        /* Model parameters */
                        double lam_dom, double lam_for, double *rho_times,
                        double *rho_dom_for_ts, double *rho_dom_fx_ts,
                        double *rho_for_fx_ts, long nb_rho,

                        /* Extrapolation parameters */
                        double vol_lim, double vol_lam,

                        /* Calibration parameters */
                        char *call_freq, double dom_vol_shift,
  ...
  err = Fx3DtsCalibration_corr(
      opt_exe, opt_mat, ivol, num_calib, merge_dates, nb_merge_dates, sig_dom,
      lam_dom, sig_for, lam_for, rho_times, rho_dom_for_ts, rho_dom_fx_ts,
      rho_for_fx_ts, nb_rho, &fx_vol_curve);
  ...
  err = Fx3DtsImpliedVolExtrapol_corr(
      opt_maturity, 0, exe_maturity, merge_dates, nb_merge_dates, sig_dom,
      lam_dom, sig_for, lam_for, opt_exe, fx_vol_curve, num_calib, vol_lim,
      vol_lam, rho_times, rho_dom_for_ts, rho_dom_fx_ts, rho_for_fx_ts, nb_rho,
      vol);
...
Err Fx3DDefImpVolBeta(/*	Total number of options */
                      ...
                      /*	Market parameters */
                      long today, long spot_date_dom, long spot_date_for,
                      long maturity_date, double spot_fx, char *dom_yc,
                      char *vol_dom, char *ref_rate_dom, char *basis_dom,
                      char *cpd_dom, char *ccy_dom, char *for_yc, char *vol_for,
                      char *ref_rate_for, char *basis_for, char *cpd_for,
                      char *ccy_for,

                      /* Model parameters */
                      double lam_dom, double lam_for, double rho_dom_for,
                      double rho_dom_fx, double rho_for_fx, double beta,
                      ...
                      long nbSteps, double disc_dt, double fx_dt,
                      long nbIterMax,
  ...
  double *sigma_date_dom = NULL, *sigma_dom = NULL, *sigma_date_for = NULL,
         *sigma_for = NULL, *merge_dates = NULL, *sig_dom = NULL,
         *sig_for = NULL, *beta_tab = NULL, *new_vol_mat = NULL;
  ...
  spot_fx *= swp_f_df(today, spot_date_dom, dom_yc) /
             swp_f_df(today, spot_date_for, for_yc);
  df = swp_f_df(today, exercise_date, dom_yc);
  fwd = spot_fx * swp_f_df(today, exercise_date, for_yc) / df;

  err = old_cpd_calib_diagonal_wrapper(
      dom_yc, vol_dom, ref_rate_dom, get_cash_vol, dom_vol_shift, 1, 0, NULL,
      maturity_calib, NULL, NULL, 0, 1.0, 1.0, cpd_dom, basis_dom, 1, 0, 0, 0,
      0, 0, 0, NULL, &lam_dom, 1, 0.0, 0.0, 0.0, &sigma_n_dom, &sigma_date_dom,
      &sigma_dom, NULL);
  ...
  err = old_cpd_calib_diagonal_wrapper(
      for_yc, vol_for, ref_rate_for, get_cash_vol, for_vol_shift, 1, 0, NULL,
      maturity_calib, NULL, NULL, 0, 1.0, 1.0, cpd_for, basis_for, 1, 0, 0, 0,
      0, 0, 0, NULL, &lam_for, 1, 0.0, 0.0, 0.0, &sigma_n_for, &sigma_date_for,
      &sigma_for, NULL);
  ...
  err = Fx3DBetatsCalibration(today, opt_exe, opt_mat, ivol, num_calib,
                              merge_dates, nb_merge_dates, sig_dom, lam_dom,
                              sig_for, lam_for, beta_tab, spot_fx, rho_dom_for,
                              rho_dom_fx, rho_for_fx, dom_yc, for_yc,
  ...
  nb_fx_vol =
      num_calib + (int)((exe_maturity - opt_exe[num_calib - 1]) / 0.5) + 1;

  if (fabs(opt_exe[num_calib - 1] + (nb_fx_vol - num_calib - 1) * 0.5 -
           exe_maturity) < 0.01) {
    nb_fx_vol -= 1;
  }
  ...
  for (i = num_calib; i < nb_fx_vol - 1; i++) {
    ...
    fx_vol_curve[i] =
        vol_lim +
        (fx_vol_curve[num_calib - 1] - vol_lim) *
            exp(-vol_lam * (new_vol_mat[i] - new_vol_mat[num_calib - 1]));
  }

  new_vol_mat[nb_fx_vol - 1] = exe_maturity;
  fx_vol_curve[nb_fx_vol - 1] =
      vol_lim + (fx_vol_curve[num_calib - 1] - vol_lim) *
                    exp(-vol_lam * (exe_maturity - new_vol_mat[num_calib - 1]));

  err = Fx3DBetatsTreeFxOptions(today, exercise_date, &fwd, 1, merge_dates,
                                nb_merge_dates, sig_dom, lam_dom, sig_for,
                                lam_for, new_vol_mat, nb_fx_vol, fx_vol_curve,
                                0.0, beta, spot_fx, rho_dom_for, rho_dom_fx,
                                rho_for_fx, dom_yc, for_yc, &price, nbSteps);
  if (err)
    goto FREE_RETURN;

  err = srt_f_optimpvol(price, fwd, fwd, exe_maturity, df, SRT_CALL,
                        SRT_LOGNORMAL, vol);

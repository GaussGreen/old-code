...
static Err populate_knot_point (SrtBndCrv * crv,
								int *		numknot, 
								int	*		num_coupons,
								Ddate **	coupon_dates,
								int			numbond,
...
int		found; /* boolean */
	...
	for (i = 0 ; i < numbond ; i++)
	{
		...
		modified_end = bus_date_method((Date)coupon_dates[i][num_coupons[i]-1], MODIFIED_SUCCEEDING); 
		...
		{	
			...
			(crv->knot_point) = (double *) realloc ((crv->knot_point), (*numknot+1) * sizeof(double));
 ...
 * subtracted
 */
static double fwdclnprc (SrtBndCrv *	crv,         
						 Ddate			prev_cpn_date,    
						 Ddate *		cpn_date,
						 int			num_cpn_date,
						 double	*		cpn,
...
int		i, 
	...
	if (global_algo_flag == 2)
	{
		...
		*/
		...
		for(i=1 ; i <= crv->lastknot-1 ; i++)
		{
			tmp += crv->fwd_rate_coef[i+2] * (crv->knot_point[crv->lastknot+1] - crv->knot_point[i]);
		}
    
		tmp = (-crv->fwd_rate_coef[1] - 2.0 * crv->fwd_rate_coef[2] * crv->knot_point[crv->lastknot+1] - 2.0 * tmp)/
...
Err			err;
	...
	*/
	...
	else
	{
		...
		knot_date_last  = (Date)add_unit(DTOL(crv->spot),DTOL(12*knot_point[numknot-1]),SRT_MONTH,MODIFIED_SUCCEEDING);    
		...
		{	
			crv->knot_date[i]  = (Ddate)add_unit(DTOL(crv->spot),DTOL(12*knot_point[i-1]),SRT_MONTH,MODIFIED_SUCCEEDING);    
		...
		if (lastdate > knot_point_last)
		{
			...
			knot_point_last = (double)(knot_date_last-today)*YEARS_IN_DAY;
			crv->knot_date[numknot] = (Ddate)add_unit(DTOL(crv->spot),DTOL(12*knot_point_last),SRT_MONTH,MODIFIED_SUCCEEDING);    
		}
		else
		{
			crv->knot_date[numknot] = (Ddate)add_unit(DTOL(crv->spot),DTOL(12*knot_point[numknot]),SRT_MONTH,MODIFIED_SUCCEEDING);    
		}
	...
	*/
	...
	/************************/
	if (err = levenberg_marquardt_select(	x,y,sig,PRICE_NUMB,
											p,ia,PARA_NUMB,
...
*/
SrtErr srt_f_bndcrv_zr(SrtBndCrv *crv,
					   Ddate start, Ddate end, SrtBasisCode basis,
					   double *answer)
{
...
/*
  fwd par bond yield 
*/
SrtErr srt_f_bndcrv_fwdtr(SrtBndCrv *crv,Date s,Date e_nfp,
						  char *f, char *b, double *answer)
{
SrtRtFnc	rt;
SrtErr		err;
int			i, len;
...
/*
   fwd bond price
*/
SrtErr srt_f_bndcrv_fwdpr(SrtBndCrv *crv, SwapDP *sdp, double cpn, double redemption,
						  double *answer)
{
int i;
DateList	dl; 
...
/*
   fwd bond yield
*/
SrtErr srt_f_bndcrv_fwdyld(SrtBndCrv *crv, SwapDP *sdp, double cpn, double redemption,
						   double *answer)
{
SrtErr err;

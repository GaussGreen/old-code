...
/* ----------------------------------------------------------------------------------
   ...
   sym_mat[0..n-1][0..n-1] On output        , elements of sym_mat above the
   diagonal are destroyed. eigen_val[0..n-1] returns the eigen values of
   sym_mat. eigen_vec[0..n-1][0..n-1] is a matrix that stores all the
   corresponding renormalised eigen vectors of sym_mat  ( eigen_vec[...][i]
   corresponds to eigen_val[i] ) nrot returns the number of Jacobi rotations
   that were required
   ----------------------------------------------------------------------------------
 */
Err jacobi_diagonalisation(double **sym_mat, int n, double eigen_val[],
                           double **eigen_vec, int *nrot)
  ...
  for (i = 1; i <= 50; i++) {
    ...
    for (ip = 0; ip < n - 1; ip++) {
      for (iq = ip + 1; iq < n; iq++) {
        ...
        if ((i > 4) &&
            (double)(fabs(eigen_val[ip]) + g) == (double)fabs(eigen_val[ip]) &&
            (double)(fabs(eigen_val[iq]) + g) == (double)fabs(eigen_val[iq]))
          sym_mat[ip][iq] = 0.0;
        else if (fabs(sym_mat[ip][iq]) > tresh) {
          h = eigen_val[iq] - eigen_val[ip];
          if ((double)(fabs(h) + g) == (double)fabs(h))
            t = (sym_mat[ip][iq]) / h;
          else {
            theta = 0.5 * h / (sym_mat[ip][iq]);
...
Err jacobi_diagonalisation2(double **sym_mat, int n, double *eigen_val,
                            double **eigen_vec, int *nrot)

...
)
{
    ...
    /* Check the model : BLACK_SCHOLES (defaulted if no model is passed) or FX_STOCH_RATES */
    if (!strcmp(model, ""))
        model = "BS";
    ...
    if ((modelType == EQ_STOCH_RATES) || (modelType == EQ_STOCH_RATES_SRVGS))
    {
        ...
        if (!dom_und)
            return serror("With Eq_Stoch_Rates, the first curve should be an UNDERLYING");
        if (!ISUNDTYPE(dom_und, INTEREST_RATE_UND))
            return serror("With Eq_Stoch_Rates, the first curve should be an IR UNDERLYING");
    ...
    else
    {
        ...
        {
            return serror("Fatal: (SrtInitEQUnd) Cannot find discount curve %s", discName);
        }
        if (!ISCURVETYPE(crv, YIELD_CURVE))
        {
            return serror("Fatal: (SrtInitEQUnd) discount curve must be a yield curve curve");
        }
    ...
    {
        return serror("Fatal: (SrtInitEQUnd) Cannot find dividend curve %s", dividendName);
    }
    if (!ISCURVETYPE(crv, YIELD_CURVE) && !ISCURVETYPE(crv, DVD_CURVE))
    {
        return serror(
            "Fatal: (SrtInitEQUnd) growth curve must be a yield curve or a dividend curve");
    }
    ...
    if (err = srt_f_init_EQ_TermStruct(
            today,
            volCrvVals,
            volCrvCols,
            volCrvRows,
    ...
    err = srt_f_addundtolist(
        und_list, undName, "EQUITY_UND", ccy, model, discName, dividendName, repoName, ts, spot);
    if (err)
    {

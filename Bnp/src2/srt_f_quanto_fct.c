...
Err srt_f_quanto_fra(long			FraStartDate,
					 long			FraEndDate,
					 String			FloatRefRate,
					 char			*FxUndName,
					 double			*AdjFra)
...
/*---------------Compute the different elements for the quanto-adjustment---------------*/
	...
	err = swp_f_setSwapDP(FraStartDate, 
						  FraEndDate, 
						  FraFreq, 
						  FraBasis, 
						  FloatLegDP);
	...
	LnFutureAdj+=
	-Lambda_func(FraStartMat,FraEndMat,psForTs)*H_fd_func(FraStartMat,psDomTs,psForTs,psFxTs);

	LnFutureAdj+=
	Lambda_func(FraStartMat,FraEndMat,psForTs)*
	Lambda_func(FraStartMat,FraEndMat,psDomTs)*Phi_fd_func(FraStartMat,psFxTs);

	LnFutureAdj+=
	-pow(Lambda_func(FraStartMat,FraEndMat,psForTs)*F_func(FraStartMat,psForTs),2)*G;

	LnFutureAdj+=
	-Lambda_func(FraStartMat,FraEndMat,psForTs)*
	F_func(FraStartMat,psForTs)*Psi_func(FraStartMat,psForTs)*G;
	...
	LnFxAdj+=
	Psi_func(FraStartMat,psForTs)*V_fx_func(FraStartMat,psFxTs);
	
	LnFxAdj+=
	-W_fx_func(FraStartMat,psFxTs);

	LnFxAdj+=
	-Psi_func(FraEndMat,psForTs)*V_fx_func(FraEndMat,psFxTs);

	LnFxAdj+=
	W_fx_func(FraEndMat,psFxTs);
...
/*--------------Compute the Foreign Forward Rate and adjust it ---------------------*/
	...
	*AdjFra=((df_start/df_end)*FutureAdj*FxAdj-1)/cov;
	...
	if(PayDates)	srt_free(PayDates);
	if(FixingDates) srt_free(FixingDates);
	if(StartDates)	srt_free(StartDates);
	if(EndDates)	srt_free(EndDates);
	if(Coverage)	srt_free(Coverage);
	if (FloatLegDP) free(FloatLegDP);
...
Err srt_f_quanto_swap(long				SwapStartDate,
					 long				SwapEndDate,
...
/* double			SwapLevel,Weights; */
	...
	err = srt_SwapFraWeigths(SwapStartDate,
							 SwapEndDate,
							 FixedFreq,
							 FixedBasis,
							 FloatFreq,
	...
	{
		err = srt_f_quanto_fra(StartDates[i],
							 EndDates[i],
							 FloatRefRate,
							 FxUndName,
							 &AdjFra);
	...
	if(WeightsVector) srt_free(WeightsVector);
	if(PayDates)	srt_free(PayDates);
	if(FixingDates) srt_free(FixingDates);
	if(StartDates)	srt_free(StartDates);
	if(EndDates)	srt_free(EndDates);
...
/*------------- COMPUTE THE WEIGHTS OF A SWAP (FOR GRFN USE)---------------------------------*/

Err srt_SwapFraWeigths(Date					SwapStartDate,
					   Date					SwapEndDate,
					   SrtCompounding		FixedFreq,
					   SrtBasisCode			FixedBasis,
					   SrtCompounding		FloatFreq,
				       ...
				       long					*WeightsVectorSize)
{
	...
	if (err = swp_f_setSwapDP(SwapStartDate, 
							  SwapEndDate, 
							  FixedFreq, 
							  FixedBasis, 
							  FixedLegDP))
	...
	if (err = swp_f_Level_SwapDP(FixedLegDP, 
								 YieldCurveName, 
								 &SwapLevel))
		return err;
	...
	for (ScheduleIndex = 0; 
		 ScheduleIndex <VectorSize; 
		 ScheduleIndex ++)
		
		(*WeightsVector)[ScheduleIndex] 
		= swp_f_df(Today,EndDates[ScheduleIndex],YieldCurveName)* Coverage[ScheduleIndex] 
		/SwapLevel;
...
/*------------- COMPUTE THE WEIGHTS OF A SWAP (FOR WESTMINSTER USE ONLY)------------------------------------------*/


Err srt_Weigths(Date SwapStartDate,
				Date SwapEndDate,
			   String FixedFreqStr,
			   String FixedBasisStr,
			   String FloatFreqStr,
			   String FloatBasisStr,
			   String YieldCurveName,
			   double **WeightsVector,
			   long *WeightsVectorSize)
{
	...
	srt_SwapFraWeigths(SwapStartDate,
					   SwapEndDate,
					   FixedComp,
					   FixedBasis,
					   FloatComp,
...
Err		srt_FraSwapWeights(long					SpotDate,
						   long					EndDate,
						   SrtCompounding		FixedFreq,
						   SrtBasisCode			FixedBasis,
						   SrtCompounding		FloatFreq,
...
long		SwapTheoEndDate;
	...
	if(!CrvPtr) return serror("Yield Curve Not Found");
	...
	{	
		SwapTheoEndDate  = add_unit (DTOL (SpotDate), 
									  (i+1)*(12/(int)FloatFreq), SRT_MONTH, 								  
									  NO_BUSDAY_CONVENTION);

		err = srt_SwapFraWeigths(SpotDate,
								SwapTheoEndDate,
								FixedFreq,
								FixedBasis,
								FloatFreq,
	...
	/*THE FRA WEIGHTS IN THE DECOMPOSITION OF THE SWAP */
		if(err)
			return err;
	/*SwapFraWeightsMatrix STORES THE FRA WEIGHTS OF THE DECOMPOSITION OF SWAP[i] */
	...
	(*FraSwapWeightsMatrix) = inverse_matrix (SwapFraWeightsMatrix, 0, NumEndDates-1);

...
Err srt_f_quanto_fra(
    long FraStartDate, long FraEndDate, String FloatRefRate, char* FxUndName, double* AdjFra)
{
    Err               err = NULL;
    ...
    err = swp_f_setSwapDP(FraStartDate, FraEndDate, FraFreq, FraBasis, FloatLegDP);
    ...
    LnFutureAdj += -Lambda_func(FraStartMat, FraEndMat, psForTs) * F_func(FraStartMat, psForTs) *
                   Psi_func(FraStartMat, psForTs) * G;

    LnFutureAdj += Lambda_func(FraStartMat, FraEndMat, psForTs) * F_func(FraStartMat, psForTs) *
                   S_func(FraStartMat, psForTs);
    ...
    double*        AdjSwap)
{
    ...
    err = swp_f_setSwapDP(SwapStartDate, SwapEndDate, FloatFreq, FloatBasis, FloatLegDP);
    ...
    {
        err = srt_f_quanto_fra(StartDates[i], EndDates[i], FloatRefRate, FxUndName, &AdjFra);
        if (err)
            return err;
    ...
    long*          WeightsVectorSize)
{
    ...
    if (err = swp_f_setSwapDP(SwapStartDate, SwapEndDate, FixedFreq, FixedBasis, FixedLegDP))
        return err;

    FixedLegDP->spot_lag = SpotLag;

    if (err = swp_f_setSwapDP(SwapStartDate, SwapEndDate, FloatFreq, FloatBasis, FloatLegDP))
        return err;
    ...
    for (ScheduleIndex = 0; ScheduleIndex < VectorSize; ScheduleIndex++)
        ...
        (*WeightsVector)[ScheduleIndex] = swp_f_df(Today, EndDates[ScheduleIndex], YieldCurveName) *
...

{
    ...
    {
        SwapTheoEndDate = add_unit(
            DTOL(SpotDate), (i + 1) * (12 / (int)FloatFreq), SRT_MONTH, NO_BUSDAY_CONVENTION);

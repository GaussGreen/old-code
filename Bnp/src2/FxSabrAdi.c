...
Err FxSabrPrecalculations(
    ...
    /*	Output			*/
    double *exp_h, double *expect_z, double *drift_z, double *std1) {
  ...
  for (i = 0; i < nstp - 1; i++) {
    ...
    expect_z[i] = exp_z;
    drift_z[i] = const3 * sig + const4 * sig * exp(-const5 * (time[i] + dt)) +
                 const2 / sig * drift[i];
    exp_h[i] = sig;
...
Err FxSabr_adi(
  ...
  double *exp_h = NULL, *expect_z = NULL, *expect_z2 = NULL, *drift_z = NULL,
         ...
         **varz = NULL, *varxinit = NULL, *muzinit1 = NULL, *muzinit2 = NULL,
         *sigBeta = NULL, **r = NULL;
  ...
  err =
      FxSabrPrecalculations2(nstp, time, sig0, drift, alpha, beta, rho, lambda,
                             spot_fx, exp_h, expect_z, drift_z, &std1);
  ...
  */
  ...
  if (bar_lvl) {
    disc_linleft_logright_bar(&x, &nstepx, spot_fx, flx, fux, std1, NSTD_FX,
                              bar_lvl, nstp, &bar_pres, &index_x);
  } else {
    if (beta >= 0.5) {
      disc_linleft_logright_center(x, nstepx, spot_fx, flx, fux, std1, NSTD_FX,
                                   &index_x);
    } else {
      disc_linleft_linright_center(x, nstepx, spot_fx, flx, fux, std1, NSTD_FX,
                                   &index_x);
    }
  }
  ...
  */
  ...
  if (!values || !values_p1 || !mux || !muz || !muzinit1 || !muzinit2 ||
      !sigBeta || !varx || !varxinit || !varz || !r) {
...
FREE_RETURN:
  ...
  if (muzinit1)
    free_dvector(muzinit1, 0, nstepx - 1);
  if (muzinit2)
    free_dvector(muzinit2, 0, nstepx - 1);
  if (sigBeta)
...
Err FxSabrCalibration(
  ...
  if (!date || !drift_res || !drift_adi || !eval_evt || !func_parm || !sigma ||
      !res_iter) {
  ...
  premium_tgt = srt_f_optblksch(spot_fx, spot_fx, volatility[0], exercise[0],
                                df, SRT_CALL, PREMIUM);

  eval_evt[nstpt - 1] = 1;

  err = FxSabr_adi(nstpt, time, date, nstpfx, nstpvol, sig0, drift_adi, alpha,
                   beta, rho, lambda, floormu, func_parm, eval_evt, NULL, NULL,
                   NULL, spot_fx, dom_yc, for_yc, payoff_fx_sabr_adi_ATM_opt, 1,
                   &premium);
  ...
  err = srt_f_optimpvol(premium, spot_fx, spot_fx, exercise[0], df, SRT_CALL,
                        SRT_LOGNORMAL, &bsvol);
  ...
  while ((k < nbIter) && (errorv >= precision)) {
    err = FxSabr_adi(nstpt, time, date, nstpfx, nstpvol, newSig0, drift_adi,
                     alpha, beta, rho, lambda, floormu, func_parm, eval_evt,
                     NULL, NULL, NULL, spot_fx, dom_yc, for_yc,
                     payoff_fx_sabr_adi_ATM_opt, 1, &premium2);
    if (err) {
      goto FREE_RETURN;
    }

    err = srt_f_optimpvol(premium2, spot_fx, spot_fx, exercise[0], df, SRT_CALL,
                          SRT_LOGNORMAL, &bsvol);
  ...
  for (i = 1; i < nbOpt; i++) {
    ...
    if (var_diff < 0.0) {
      ...
      if (delta >= 0.0) {
        ...
        coef =
            (var_diff / ((*sigma)[i - 1] * (*sigma)[i - 1]) - dt) / (dt * dt);
      }
    }
    ...
    premium_tgt = srt_f_optblksch(spot_fx, spot_fx, volatility[i], exercise[i],
                                  df, SRT_CALL, PREMIUM);
    ...
    err = FxSabr_adi(nstpt, time, date, nstpfx, nstpvol, sig0, drift_adi, alpha,
                     beta, rho, lambda, floormu, func_parm, eval_evt, NULL,
                     NULL, NULL, spot_fx, dom_yc, for_yc,
                     payoff_fx_sabr_adi_ATM_opt, 1, &premium);
    ...
    error = fabs(premium - premium_tgt);
    err = srt_f_optimpvol(premium, spot_fx, spot_fx, exercise[i], df, SRT_CALL,
                          SRT_LOGNORMAL, &bsvol);
    ...
    if (errorv >= precision || err) {
      if (err) {
        ...
        if (delta >= 0.0) {
          ...
          newCoef =
              ((newVar2 - var) / ((*sigma)[i - 1] * (*sigma)[i - 1]) - dt) /
              (dt * dt);
        }
      }
    ...
    k = 0;
    while ((k < nbIter) && (errorv >= precision)) {
      err = FxSabr_adi(nstpt, time, date, nstpfx, nstpvol, sig0, drift_adi,
                       alpha, beta, rho, lambda, floormu, func_parm, eval_evt,
                       NULL, NULL, NULL, spot_fx, dom_yc, for_yc,
                       payoff_fx_sabr_adi_ATM_opt, 1, &premium2);
      ...
      err = srt_f_optimpvol(premium2, spot_fx, spot_fx, exercise[i], df,
      ...
      if (premium2 > 100 * premium_tgt || premium2 < 0.0 || err) {
        ...
        newCoef2 = solve_for_next_coef(res_iter, nb_iter, premium_tgt, 0);
    ...
    (*sigma)[i] =
        (*sigma)[i - 1] * exp(newCoef * (exercise[i] - exercise[i - 1]));

    var += (*sigma)[i - 1] * (*sigma)[i - 1] / (2.0 * newCoef) *
           (exp(2.0 * newCoef * (exercise[i] - exercise[i - 1])) - 1.0);
  ...
  *x = realloc(*x, (*nstp + nb_bar2) * sizeof(double));
  ...
  while (exit) {
    ...
    if (j < nb_bar2 && bar[j] < (*x)[i - 1] + meshx + 1.0E-08) {
      ...
      if (bar[j] - (*x)[i - 1] > meshx / 2 ||
          (j > 0 && fabs((*x)[i - 1] - bar[j - 1]) < 1.0E-06)) {
        (*x)[i] = bar[j];
      } else {
        i--;
        (*x)[i] = bar[j];
                           ...
                           int method) /* 0: linear 1: quadratic */
{
  ...
  if (nb_iter == 2 || method == 0) {
    ...
    if (delta > 0) {
      ...
      coef = solve_for_next_coef(res_iter, nb_iter, premium_tgt, 0);
    }
  } else {
...
double find_beta_lim(double forward, double std_beta, double beta,
  ...
  perc1 = -srt_f_optblkschbeta(forward, level1 + eps, std_beta, 1.0, beta, 1.0,
                               SRT_CALL, PREMIUM) +
          srt_f_optblkschbeta(forward, level1 - eps, std_beta, 1.0, beta, 1.0,
                              SRT_CALL, PREMIUM);

  level2 = forward * 1.01;

  perc2 = -srt_f_optblkschbeta(forward, level2 + eps, std_beta, 1.0, beta, 1.0,
                               SRT_CALL, PREMIUM) +
          srt_f_optblkschbeta(forward, level2 - eps, std_beta, 1.0, beta, 1.0,
                              SRT_CALL, PREMIUM);
  ...
  while (i < ITER_MAX && fabs(perc2 - perc_tgt) > pres) {
    ...
    if (level2 > eps_lim) {
      perc2 = -srt_f_optblkschbeta(forward, level2 + eps, std_beta, 1.0, beta,
                                   1.0, SRT_CALL, PREMIUM) +
              srt_f_optblkschbeta(forward, level2 - eps, std_beta, 1.0, beta,
                                  1.0, SRT_CALL, PREMIUM);
    } else {
      perc2 = 2.0 * (-srt_f_optblkschbeta(forward, level2 + eps, std_beta, 1.0,
                                          beta, 1.0, SRT_CALL, PREMIUM) +
                     srt_f_optblkschbeta(forward, level2, std_beta, 1.0, beta,
                                         1.0, SRT_CALL, PREMIUM));
    }

    ...
    double* std1)
{
    ...
    for (i = 0; i < nstp - 1; i++)
    {
        ...
        expect_z[i] = exp_z;
        drift_z[i] =
            const3 * sig + const4 * sig * exp(-const5 * (time[i] + dt)) + const2 / sig * drift[i];
        exp_h[i] = sig;
    ...
    double* res)
{
    ...
    double *exp_h = NULL, *expect_z = NULL, *expect_z2 = NULL, *drift_z = NULL, *x = NULL,
           *z = NULL, ***values = NULL, ***values_p1 = NULL, ***values_temp = NULL, **mux = NULL,
           **muz = NULL, **varx = NULL, **varz = NULL, *varxinit = NULL, *muzinit1 = NULL,
           *muzinit2 = NULL, *sigBeta = NULL, **r = NULL;
    ...
    */
    ...
    if (bar_lvl)
    {
        disc_linleft_logright_bar(
            &x, &nstepx, spot_fx, flx, fux, std1, NSTD_FX, bar_lvl, nstp, &bar_pres, &index_x);
    }
    else
    {
        if (beta >= 0.5)
    ...
    */
    ...
    if (!values || !values_p1 || !mux || !muz || !muzinit1 || !muzinit2 || !sigBeta || !varx ||
        !varxinit || !varz || !r)
    {
    ...
    double** sigma)
{
    ...
    premium_tgt =
        srt_f_optblksch(spot_fx, spot_fx, volatility[0], exercise[0], df, SRT_CALL, PREMIUM);
    ...
    err   = srt_f_optimpvol(
        premium, spot_fx, spot_fx, exercise[0], df, SRT_CALL, SRT_LOGNORMAL, &bsvol);
    ...
    while ((k < nbIter) && (errorv >= precision))
    {
        ...
        err = srt_f_optimpvol(
            premium2, spot_fx, spot_fx, exercise[0], df, SRT_CALL, SRT_LOGNORMAL, &bsvol);
    ...
    for (i = 1; i < nbOpt; i++)
    {
        ...
        premium_tgt =
            srt_f_optblksch(spot_fx, spot_fx, volatility[i], exercise[i], df, SRT_CALL, PREMIUM);
        ...
        error = fabs(premium - premium_tgt);
        err   = srt_f_optimpvol(
            premium, spot_fx, spot_fx, exercise[i], df, SRT_CALL, SRT_LOGNORMAL, &bsvol);
        ...
        if (errorv >= precision || err)
        {
            ...
            else
            {
                ...
                {
                    newCoef =
                        ((newVar2 - var) / ((*sigma)[i - 1] * (*sigma)[i - 1]) - dt) / (dt * dt);
                }
            }
        ...
        while ((k < nbIter) && (errorv >= precision))
        {
            ...
            err = srt_f_optimpvol(
                premium2, spot_fx, spot_fx, exercise[i], df, SRT_CALL, SRT_LOGNORMAL, &bsvol);
    ...
    int       interp_bar)
{
    ...
    if (ux + 1 < nstepx && ux > 0)
    {
        ...
        if (nprod != 1)
        {
            ...
            coef1 =
                (barrier * (barrier - x[ux] - x[ux - 1]) + x[ux] * x[ux - 1]) / (d1 * (d1 + d2));
            coef2 =
                (barrier * (barrier - x[ux + 1] - x[ux - 1]) + x[ux + 1] * x[ux - 1]) / (-d1 * d2);
        ...
        for (j = lz; j <= uz; j++)
        {
            ...
            if (interp_bar)
            {
                ...
                {
                    valuesi2[j][k] =
                        coef1 * valuesi1[j][k] + coef2 * valuesi2[j][k] + coef3 * valuesi3[j][k];
                }
            }
            else
            {
...
void disc_linleft_logright_center(
    double* x,
    long    nstp,
    double  fwd,
    double  xmin,
...
void disc_linleft_linright_center(
    double* x,
    long    nstp,
    double  fwd,
    double  xmin,
...
void disc_linleft_logright_bar(
    double** x,
    long*    nstp,
    double   fwd,
    double   xmin,
...
void disc_linleft_logright_strike(
    double* x,
    long    nstp,
    double  fwd,
    double  xmin,
...
void disc_linleft_logright_center_strike(
    double* x,
    long    nstp,
    double  fwd,
    double  xmin,
...
void disc_linleft_logright_bar2(
    double** x,
    long*    nstp,
    double   fwd,
    double   xmin,
...
double find_beta_lim(double forward, double std_beta, double beta, double percent)
{
    ...
    while (i < ITER_MAX && fabs(perc2 - perc_tgt) > pres)
    {
        ...
        if (level2 > eps_lim)
        {
            perc2 = -srt_f_optblkschbeta(
                        forward, level2 + eps, std_beta, 1.0, beta, 1.0, SRT_CALL, PREMIUM) +
                    srt_f_optblkschbeta(
                        forward, level2 - eps, std_beta, 1.0, beta, 1.0, SRT_CALL, PREMIUM);
        }
        else
        {
            perc2 = 2.0 * (-srt_f_optblkschbeta(
                               forward, level2 + eps, std_beta, 1.0, beta, 1.0, SRT_CALL, PREMIUM) +
                           srt_f_optblkschbeta(
                               forward, level2, std_beta, 1.0, beta, 1.0, SRT_CALL, PREMIUM));
        }

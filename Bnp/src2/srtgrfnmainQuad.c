    ...
    double** prod_val)
{
    ...
    double *dom_ifr = NULL, *dom_fwd = NULL, *dom_var = NULL, *for_ifr = NULL, *for_fwd = NULL,
           *for_var = NULL, *fx_fwd = NULL, *fx_fwd0 = NULL, *fx_var = NULL;
    ...
    err = srt_f_get_corr_from_CorrList(sCorrlist, domname, forname, 1.0, &corr_dom_for);

    err = srt_f_get_corr_from_CorrList(sCorrlist, domname, und3dfx, 1.0, &corr_dom_fx);

    err = srt_f_get_corr_from_CorrList(sCorrlist, forname, und3dfx, 1.0, &corr_for_fx);
    ...
    if (!dom_ifr || !dom_fwd || !dom_var || !for_ifr || !for_fwd || !for_var || !fx_fwd ||
        !fx_fwd0 || !fx_var || !sig_fx_approx || !fx_fwd_approx)
    {
    ...
    for (i = 1; i < nstp; i++)
    {
        fx_fwd0[i] = spot_fx / swp_f_df(today, date[i], dom_yc) * swp_f_df(today, date[i], for_yc);
    }
    ...
    err =
        fill_local_model(model_func, model_param, nstp, time, alpha2, beta2, gamma2, sig0, fx_fwd0);
    // err = fill_local_model_cust(model_func, model_param, NB_POINTS, time, nstp, alpha2, beta2,
    // gamma2, sig0, fx_fwd0);
    if (err)
    {
        goto FREE_RETURN;
    ...
    */
    ...
    for (i = nstp - 1; i >= 0; i--)
    {
        if (j >= 0 && fabs(date[i] - xStr.dts[j]) < 1.0e-04)
        {
            ...
            if (barrier[j] > 1.0e-08)
            {
                next_bar = model_func->Z_to_U(
                    i, model_func->S_to_Z(i, barrier[j], fx_fwd0[i], model_param), model_param);
        ...
        else if (j > 0 && xStr.am[j - 1])
        {
            ...
            {
                next_bar = model_func->Z_to_U(
                    i, model_func->S_to_Z(i, barrier[j - 1], fx_fwd0[i], model_param), model_param);
        ...
        else
        {
            bar_lvl[i] = model_func->Z_to_U(
                i, model_func->S_to_Z(i, fx_fwd0[i], fx_fwd0[i], model_param), model_param);
        }
    ...
    bar_lvl[1] = model_func->Z_to_U(
        1, model_func->S_to_Z(i, fx_fwd0[1], fx_fwd0[1], model_param), model_param);
    bar_cl[1] = -1;
    ...
    double*** prod_val)
{
    ...
    if (!sig_dom || !sig_for || !sig_fx || !dom_vol || !for_vol || !fx_vol || !fx_fwd ||
        !corr_dom_for || !corr_dom_fx || !corr_for_fx || !dom_ifr || !dom_phi || !for_ifr ||
        !for_phi)
    {
    ...
    for (i = 0; i < nstp; i++)
    {
        fx_fwd[i] = (swp_f_zr(today, date[i], dom_yc) - swp_f_zr(today, date[i], for_yc)) * time[i];
    ...
    err =
        fill_local_model(model_func, model_param, nstp, time, alpha2, beta2, gamma2, sig0, fx_fwd);
    if (err)
    {
        goto FREE_RETURN;
    }

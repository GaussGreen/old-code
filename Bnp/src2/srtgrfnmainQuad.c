...
char *SrtGrfn3DFXQuadTree(char *und3dfx, double beta, double gamma, double sig0,
  ...
  // err = allocate_local_model_cust(NB_POINTS  , nstp  , &model_func  ,
  // &model_param);
  if (err) {
    goto FREE_RETURN;
  }
  ...
  err = fill_local_model(model_func, model_param, nstp, time, alpha2, beta2,
                         gamma2, sig0, fx_fwd0);
  // err = fill_local_model_cust(model_func  , model_param  , NB_POINTS  , time
  // , nstp  , alpha2  , beta2  , gamma2  , sig0  , fx_fwd0);
  if (err) {
    goto FREE_RETURN;
  }

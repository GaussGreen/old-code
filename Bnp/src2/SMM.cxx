...
static Err gf_calc_deriv(double x, smm_swap *swp, double shift, double *g,
                         double *f) {
  Err err = NULL;
  int i, j;
  ...
  for (i = 0; i < swp->nfix; i++) {
    ...
    lvlc2 += swp->fix_cvg[i] * swp->fix_cvg[i] * swp->fix_cvg[i] * (i + 1) *
             (i + 2) / (tmpi * tmp * tmp);
  }
  g[0] = 1.0 / lvlc0;
  g[1] = g[0] * g[0] * lvlc1;
  ...
  err = BMM_Option_Price_From_States(x, swp->fxg_time, swp->beta, swp->fwd1,
                                     swp->fwd2, swp->sigbeta1, swp->sigbeta2,
                                     swp->pi, SRT_CALL, &f[0]);
  if (err)
    return err;

  err = BMM_Option_Price_From_States(x - shift, swp->fxg_time, swp->beta,
                                     swp->fwd1, swp->fwd2, swp->sigbeta1,
                                     swp->sigbeta2, swp->pi, SRT_CALL, &p[0]);
  if (err)
    return err;

  err = BMM_Option_Price_From_States(x + shift, swp->fxg_time, swp->beta,
                                     swp->fwd1, swp->fwd2, swp->sigbeta1,
                                     swp->sigbeta2, swp->pi, SRT_CALL, &p[1]);
  if (err)
    return err;
...
static Err smm_BMM_linterp_cumulative(smm_swap *swp,

                                      double dMaxError, double dNbStdLeft,
                                      double dNbStdRight,
  ...
  srt_f_optbmmvolfromstates(swp->cms_fwd, //		 swp->swap0        ,
                            swp->fxg_time, swp->beta, swp->fwd1, swp->fwd2,
                            swp->sigbeta1, swp->sigbeta2, swp->pi,
                            SRT_LOGNORMAL, &dLogStd);
  ...
  dMinPoint =
      /*swp->swap0 */ swp->cms_fwd * exp(dLogCvx - dNbStdDevLeft * dLogStd);

  do {
    dNbStdDevRight -= 1.0;
    dMaxPoint =
        /*swp->swap0 */ swp->cms_fwd * exp(dLogCvx + dNbStdDevRight * dLogStd);

    err = BMM_Option_Price_From_States(
        dMaxPoint, swp->fxg_time, swp->beta, swp->fwd1, swp->fwd2,
        swp->sigbeta1, swp->sigbeta2, swp->pi, SRT_CALL, &dNewF[0]);
    if (err)
      return err;
  ...
  (*dCumulative)[0] =
      (dNewG[0] * dNewF[1] - dLastG[0] * dLastF[1] - dNewG[1] * dNewF[0] +
       dLastG[1] * dLastF[0] +
  ...
  while (dNewPoint < dMaxPoint) {
    ...
    dNewCumul =
        dLastCumul + (dNewG[0] * dNewF[1] - dLastG[0] * dLastF[1] -
...
static Err smm_swap_density(smm_swap *swp, double x, double shift,
                            double *density) {
  Err err = NULL;
  double p[3];
  ...
  err = BMM_Option_Price_From_States(x + shift, swp->fxg_time, swp->beta,
                                     swp->fwd1, swp->fwd2, swp->sigbeta1,
                                     swp->sigbeta2, swp->pi, SRT_CALL, &p[2]);
  if (err)
    return err;
...
static Err smm_BMM_linterp_cumulative2(smm_swap *swp,
  ...
  double dMinPoint, dMaxPoint;
  double dNbStdDevLeft, dNbStdDevRight, dLogStd, dLogCvx;
  double dLastPoint, dNewPoint, dInterpPoint, dLastCumul, dNewCumul,
      dInterpCumul, dRealCumul, dLastDensity, dNewDensity, dRealDensity;
  long i, lAllocTotPoints;
  double dError;

  // First Guess
  srt_f_optbmmvolfromstates(swp->swap0, swp->fxg_time, swp->beta, swp->fwd1,
                            swp->fwd2, swp->sigbeta1, swp->sigbeta2, swp->pi,
                            SRT_LOGNORMAL, &dLogStd);
  ...
  while (dNewPoint < dMaxPoint) {
    ...
    dNewCumul = dLastCumul +
                0.5 * (dLastDensity + dNewDensity) * (dNewPoint - dLastPoint);
    ...
    while (dError > dMaxError) {
      ...
      dRealCumul = dLastCumul + 0.5 * (dLastDensity + dRealDensity) *
                                    (dInterpPoint - dLastPoint);
...
static Err smm_simulate(int nswaps, smm_swap *swaps, double **corr_matrix,
                        smm_params *params, double **samples) {
  Err err = NULL;
  SRandomGen rg;
  ...
  for (i = 0; i < params->npaths; i++) {
    for (j = 0; j < nswaps; j++) {
      ...
      samples[i][j] = swaps[j].cxxms_fwd /*swaps[j].swap0 */ *
...
static Err smm_simulate2(int nswaps, smm_swap *swaps, double **corr_matrix,
                         smm_params *params, double **samples) {
  Err err = NULL;
  int i;
  long *npts = NULL;
  ...
  for (i = 0; i < nswaps; i++) {
    err =
        smm_BMM_linterp_cumulative(&swaps[i], params->maxerror, params->nstd,
                                   params->nstd, &npts[i], &pts[i], &cumul[i]);
    if (err)
      goto FREE_RETURN;
  }
  ...
  err = copula_gaussian_numer(nswaps, pts, cumul, npts, 0, corr_matrix,
                              &copula_prm, samples);
  if (err)
    goto FREE_RETURN;
...
static Err smm_setup_swaps(char *yc, char *vc, smm_params *params,
                           char *freq_str, char *bas_str, char *ref_rate,
                           long ex_date, long last_date, smm_swap **swaps,
                           int *nswaps) {
  Err err = NULL;
  ...
  do {
    theo_date =
        add_unit(theo_date, 12 / fix_freq, SRT_MONTH, NO_BUSDAY_CONVENTION);
  ...
  *swaps = (smm_swap *)calloc(*nswaps, sizeof(smm_swap));
  ...
  for (i = 0; i < *nswaps; i++) {
    ...
    if (!swp->fix_d || !swp->fix_t || !swp->fix_cvg || !swp->fix_df ||
        !swp->flt_d || !swp->flt_t || !swp->flt_cvg || !swp->flt_df ||
        !swp->flt_spr)
      return serror("Memory failure");
    ...
    for (j = 0; j < swp->nfix; j++) {
      ...
      swp->fix_cvg[j] = coverage((j == 0 ? swp->start_date : swp->fix_d[j - 1]),
                                 swp->fix_d[j], fix_basis);
    ...
    for (j = 0; j < swp->nflt; j++) {
      ...
      swp->flt_cvg[j] = coverage((j == 0 ? swp->start_date : swp->flt_d[j - 1]),
                                 swp->flt_d[j], flt_basis);
      ...
      for (k = flt_freq / fix_freq - 1; k >= 0; k--) {
        theo_date =
            add_unit(theo_date, 12 / flt_freq, SRT_MONTH, NO_BUSDAY_CONVENTION);
        temp_date = (k == 0 ? swp->flt_d[j]
                            : bus_date_method(theo_date, MODIFIED_SUCCEEDING));
    ...
    err = swp_f_SABRvol(vc, swp->start_date, swp->end_date, 0.0, &sabr_sigbeta,
                        &power, SABR_BETAVOL);
    if (err)
      return err;

    err = swp_f_SABRvol(vc, swp->start_date, swp->end_date, 0.0, &sabr_alpha,
                        &power, SABR_ALPHA);
    if (err)
      return err;

    err = swp_f_SABRvol(vc, swp->start_date, swp->end_date, 0.0, &sabr_beta,
                        &power, SABR_BETA);
    if (err)
      return err;

    err = swp_f_SABRvol(vc, swp->start_date, swp->end_date, 0.0, &sabr_rho,
                        &power, SABR_RHO);
    if (err)
      return err;

    err = srt_f_optsarbvol(swp->swap0, swp->swap0, swp->fxg_time, sabr_sigbeta,
                           sabr_alpha, sabr_beta, sabr_rho, SRT_BETAVOL,
                           SRT_LOGNORMAL, &imp_vols[0]);
    if (err)
      return err;
    ...
    err = srt_f_optsarbvol(swp->swap0, swp->swpn_strikes[1], swp->fxg_time,
                           imp_vols[0], sabr_alpha, sabr_beta, sabr_rho,
                           SRT_LOGNORMAL, SRT_LOGNORMAL, &imp_vols[1]);
    if (err)
      return err;

    err = srt_f_optsarbvol(swp->swap0, swp->swpn_strikes[2], swp->fxg_time,
                           imp_vols[0], sabr_alpha, sabr_beta, sabr_rho,
                           SRT_LOGNORMAL, SRT_LOGNORMAL, &imp_vols[2]);
    if (err)
      return err;

    for (j = 0; j < 3; j++)
      swp->swpn_values[j] =
          srt_f_optblksch(swp->swap0, swp->swpn_strikes[j], imp_vols[j],
                          swp->fxg_time, swp->lvl0, SRT_CALL, PREMIUM);
    ...
    //		err = swp_f_cmsrate(swp->swap0        , swp->nfix        , fix_freq
    //, sabr_sigbeta        , swp->fxg_time        , 			0.0 ,
    // 0.0        , 0        , SRT_LOGNORMAL        , &swp->cms_fwd);

    err = swp_f_CMSTEC_rate(swp->swap0, // forward
                            swp->nfix,  // nfp - (we suppose that we're always
                                        // taking standard swaps as underlyings)
                            ...
                            0.0,           // delay
                            1.0,           // df_pay / df_start
                            &cms_vol, NULL, &cms_params, &swp->cms_fwd);
    if (err)
      return err;

    err =
        swp_f_CMSTEC_Option(swp->swap0, swp->nfix, fix_freq, 1.0, swp->fxg_time,
                            swp->cms_fwd, SRT_RECEIVER, 1.0, 0.0, 0.0, 1.0,
                            &cms_vol, NULL, &cms_params, &cms_opt_atm);
    if (err)
      return err;

    err =
        srt_f_optimpvol(cms_opt_atm, swp->cms_fwd, swp->cms_fwd, swp->fxg_time,
                        1.0, SRT_PUT, SRT_LOGNORMAL, &imp_vols[0]);
    if (err)
      return err;

    err = srt_f_optsarbvol(swp->cms_fwd, swp->cms_fwd, swp->fxg_time,
                           imp_vols[0], sabr_alpha, sabr_beta, sabr_rho,
                           SRT_LOGNORMAL, SRT_BETAVOL, &sabr_sigbeta);
    if (err)
      return err;

    //		swp->cms_fwd *= exp(-0.0010 * (swp->end_time -
    // swp->start_time));

    err = BMMCalibOnSabrStates(
        swp->cms_fwd, // swp->swap0        ,
        swp->fxg_time, sabr_sigbeta, sabr_beta, sabr_alpha, sabr_rho, swp->pi,
        params->smile_nstd, &swp->fwd1, &swp->fwd2, &swp->sigbeta1,
        &swp->sigbeta2, &swp->pi, SRT_BETAVOL, &calib_pres);
...
PASS_START:
  ...
  for (i = 0; i < nswaps; i++) {
    ...
    flt_tmp = swaps[i].flt_spr[swaps[i].nflt - 1] *
              swaps[i].flt_cvg[swaps[i].nflt - 1];
    swp_dfs[i + 1] = (swp_dfs[0] - samples[i] * fix_leg + flt_leg) /
                     (samples[i] * fix_tmp + 1.0 - flt_tmp);
    //		(swp_dfs[0] - samples[i] * level_cash(samples[i]        ,
    //swaps[i].nfix        , swaps[i].fix_cvg) + flt_leg) / 		(1.0 -
    // flt_tmp);

    if (swp_dfs[i + 1] > 0)
      zc_rate[i + 1] =
          -log(swp_dfs[i + 1]) / (swp_times[i + 1] - swaps[0].fxg_time);
    else
      zc_rate[i + 1] = 999.99;
    ...
    if (ipass < NPASS)
      samples[i] =
          orig_samples[i] * swaps[i].lvl0 / swaps[i].df_start / fix_leg;
  }
...
static void smm_interp_dfs(int swp_ndfs, double *swp_times, double *swp_dfs,
                           double *zc_rate, double mat, int ndfs, double *times,
                           double *dfs) {
  int i, idx = 0;
  double zcr, coef;
  ...
  for (i = 0; i < ndfs; i++) {
    if (times[i] <= swp_times[0])
      ...
      while (times[i] >= swp_times[idx + 1])
        idx++;
      coef =
          (times[i] - swp_times[idx]) / (swp_times[idx + 1] - swp_times[idx]);
...
static Err smm_match(int nswaps, smm_swap *swaps, smm_params *params,
                     double **swp_dfs, double *weights) {
  Err err = NULL;
  ...
  for (k = 0; k < nswaps; k++) {
    ...
    for (j = 0; j < N; j++) {
      ...
      for (n = 0; n < swaps[k].nfix; n++) {
        ...
        flt_leg +=
            swaps[k].flt_cvg[n] * swp_dfs[j][n + 1] * swaps[k].flt_spr[n];
      }
      ...
      for (n = 0; n < 3; n++) {
        A[i + n + 1][j] =
            (flt_leg - swaps[k].swpn_strikes[n] * fix_leg) / swp_dfs[j][0];
        if (A[i + n + 1][j] < 0.0)
          A[i + n + 1][j] = 0.0; // Swaptions
        sum_s[n] += A[i + n + 1][j] * weights[j];
      }
    }
    b[i++] = sum - swaps[k].cxxms_fwd;
    for (n = 0; n < 3; n++)
  ...
  nag_real_svd(M, M, &AA[0][0], M, 1, b, 1, 0, NULL, 0, sv, 1, NULL, 0, &iter,
               e, &failinfo, &fail);
...
Err smm_otc_price(char *yc, char *vc, char *freq_str, char *bas_str,
                  char *ref_rate, char *corr_cube, smm_params *params,
                  SProductDesc *g, int iex, double *pv, double *stddev) {
  Err err = NULL;
  smm_swap *swaps = NULL;
  ...
  SSabrMktDesc mkt_sabr_desc = {
      &sabr_sigbeta_, &sabr_alpha_, &sabr_beta_, &sabr_rho_, 0, NULL};
  ...
  if (g->nvol)
    for (i = 0; i < g->nvol[0][iex]; i++) {
      err = swp_f_SABRvol(vc, g->vol_start[0][iex][i], g->vol_end[0][iex][i],
                          0.0, &sabr_sigbeta[i], &power, SABR_BETAVOL);
      if (err)
        goto FREE_RETURN;

      err = swp_f_SABRvol(vc, g->vol_start[0][iex][i], g->vol_end[0][iex][i],
                          0.0, &sabr_alpha[i], &power, SABR_ALPHA);
      if (err)
        goto FREE_RETURN;

      err = swp_f_SABRvol(vc, g->vol_start[0][iex][i], g->vol_end[0][iex][i],
                          0.0, &sabr_beta[i], &power, SABR_BETA);
      if (err)
        goto FREE_RETURN;

      err = swp_f_SABRvol(vc, g->vol_start[0][iex][i], g->vol_end[0][iex][i],
                          0.0, &sabr_rho[i], &power, SABR_RHO);
      if (err)
        goto FREE_RETURN;
    }
  ...
  {
    for (i = 0; i < g->nvol[0][iex]; i++) {
      err = swp_f_SABRvol(vc, g->vol_start[0][iex][i] - slide,
                          g->vol_end[0][iex][i] - slide, 0.0, &sabr_sigbeta[i],
                          &power, SABR_BETAVOL);
      if (err)
        goto FREE_RETURN;
    }
  ...
  if (g->nvol && (params->vol_hyp & 2)) // SABR params sliding
  {
    for (i = 0; i < g->nvol[0][iex]; i++) {
      err = swp_f_SABRvol(vc, g->vol_start[0][iex][i] - slide,
                          g->vol_end[0][iex][i] - slide, 0.0, &sabr_alpha[i],
                          &power, SABR_ALPHA);
      if (err)
        goto FREE_RETURN;

      err = swp_f_SABRvol(vc, g->vol_start[0][iex][i] - slide,
                          g->vol_end[0][iex][i] - slide, 0.0, &sabr_beta[i],
                          &power, SABR_BETA);
      if (err)
        goto FREE_RETURN;

      err = swp_f_SABRvol(vc, g->vol_start[0][iex][i] - slide,
                          g->vol_end[0][iex][i] - slide, 0.0, &sabr_rho[i],
                          &power, SABR_RHO);
      if (err)
        goto FREE_RETURN;
    }
  ...
  err =
      smm_setup_swaps(yc, vc, params, freq_str, bas_str, ref_rate, g->ex_d[iex],
                      g->mat_d[0][iex][g->nmat[0][iex] - 1], &swaps, &nswaps);
  if (err)
    goto FREE_RETURN;
  ...
  for (i = 0; i < nswaps; i++) {
    for (j = 0; j < i; j++) {
      ...
      err = swp_f_vol(corr_cube, start_corr, end_corr, g->ex[iex],
                      &corr_matrix[i][j], &tmp);
      if (err)
        goto FREE_RETURN;
  ...
  memcpy(swp_times + 1, swaps[nswaps - 1].fix_t,
         swaps[nswaps - 1].nfix * sizeof(double));

  for (ipath = 0; ipath < params->npaths; ipath++)
    smm_strip(nswaps, swaps, samples[ipath], swp_times, swp_dfs[ipath],
              zc_rate[ipath]);
  ...
  for (ipath = 0; ipath < params->npaths; ipath++) {
    smm_interp_dfs(swp_ndfs, swp_times, swp_dfs[ipath], zc_rate[ipath],
                   swaps[0].fxg_time, g->nmat[0][iex], g->mat[0][iex], dfs);
...
static Err IndexDates(int ndates, long *dates, int *new_ndates, long *new_dates,
                      long *index) {
  Err err = NULL;
...
static Err
smm_swaptions_payoff(SProductDesc *g,
                     int idx, // must be = 0
                     double time,
                     ...
                     double *pv)         // ninst sized vector with PVs (output)
{
  ...
  for (i = 0; i < g->ninst; i++) {
    ...
    // test CMS:
    lvl = 0.0;
    /*		for (j = swp_desc->cffix_idx[i] - swp_desc->cf_idx[i]; j <
       swp_desc->ncf[i]; j++)
                    {
                            cf_index = swp_desc->cf_idx[i] + j;
...
static Err smm_cms_payoff(SProductDesc *g,
                          int idx, // must be = 0
                          double time,
                          SMktData *mkt_data, // market data (dfs)
                          double *pv) // ninst sized vector with PVs (output)
...
static Err
smm_levelcash_payoff(SProductDesc *g,
                     int idx, // must be = 0
                     double time,
                     SMktData *mkt_data, // market data (dfs)
                     double *pv)         // ninst sized vector with PVs (output)
...
Err smm_cif_init_product(SProductDesc *g, char *yc,

                         // Coupons:
                         char *freq_str, // CMS fix freq
                         char *bas_str,  // CMS fix basis
                         char *ref_rate, // CMS ref rate
                         int ncpn, long *starts, long *ends, double *alpha,
                         double *beta, int *floored, double *floor, int *capped,
                         double *cap, long *cpn_pay, double *cpn_cvg,
  ...
  for (i = 0; i < fund_ncpn; i++) {
    ...
    cif->fund_cpn[i].cxxpn = cvg * (fund_spr[i] + fund_mrg[i]);
...
Err smm_swaptions_init_product(SProductDesc *g, char *yc, char *freq_str,
                               char *bas_str, char *ref_rate, char *pay_rec,
                               int is_cms, long start, int nends, long *ends,
                               int *nK, double **K) {
  Err err = NULL;
  ...
  for (i = 0; i < nends; i++) {
    for (j = 0; j < nK[i]; j++) {
      ...
      while (act_date < ends[i]) {
        ...
        all_dates[n_all_dates] = act_date;
        all_cash_flows[n_all_dates] =
            -K[i][j] * coverage(temp_date, act_date, fix_basis);
        n_all_dates++;
        swp_desc->ncf[inst_idx]++;
        temp_date = act_date;
      }
  ...
  err = IndexDates(n_all_dates, all_dates, &g->nmat[0][0], new_dates,
                   swp_desc->dfs_idx);
  if (err)
    return err;
...
static Err smm_dfs_payoff(SProductDesc *g,
                          int idx, // must be = 0
                          double time,
                          SMktData *mkt_data, // market data (dfs)
                          double *pv) // ninst sized vector with PVs (output)

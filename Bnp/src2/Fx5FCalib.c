...
static Err Coefs_Partial_Var(	double	T,
								double	T1,
								double	T2,
								double	sig_dom1,
								double	sig_dom2,
...
static Err Partial_Var(	double	T,
						double	T1,
						double	T2,
						double	sig_dom1,
						double	sig_dom2,
...
Err err = NULL;
	...
	/* Get Coefs */
	err = Coefs_Partial_Var(T, T1, T2,
							sig_dom1, sig_dom2, lda_dom1, lda_dom2,
							sig_for1, sig_for2, lda_for1, lda_for2,
							correl,
							&a, &b, &c);
...
Err Fx5DtsImpliedVol(	double	opt_maturity,
						...
						double	*fx_vol)
{
double	sig_dom1, sig_dom2, lda_dom2,
		sig_for1, sig_for2, lda_for2,
		sig_fx;
...
Err			err = NULL;
	...
	err = Get_FX_StochRate_TermStructures5F(fx_underlying,
...
double		tau_dom, lda_dom, alpha_dom, gamma_dom, rho_dom,
			tau_for, lda_for, alpha_for, gamma_for, rho_for;

Err			err = NULL;

	err = Get_LGM2F_TermStructure (	dom_underlying,
									&sigma_date_dom,
									&sigma_dom,
									&sigma_n_dom,
									&tau_dom,
	...
	err = Get_LGM2F_TermStructure (	for_underlying,
									&sigma_date_for,
									&sigma_for,
									&sigma_n_for,
									&tau_for,
...
Err			err = NULL;
	...
	*sigma_fx		= NULL;
	...
	err = Get_LGM2F_TermStructure (	domname,
									sigma_date_dom,
									sigma_dom,
									sigma_n_dom,
									fixed_tau_dom,
	...
	err = Get_LGM2F_TermStructure (	forname,
									sigma_date_for,
									sigma_for,
									sigma_n_for,
									fixed_tau_for,
...
Err		err = NULL;
	...
	if (calib_rho_dom)
	{
		...
		c = corr2 * (1.0 + alpha_dom * alpha_dom) * (1.0 + alpha_dom * alpha_dom * ex_dom * ex_dom)
	...
	if (calib_rho_for)
	{
		...
		c = corr2 * (1.0 + alpha_for * alpha_for) * (1.0 + alpha_for * alpha_for * ex_for * ex_for)
...
FREE_RETURN:
	...
	if (matrix) free_dmatrix(matrix, 0, 3, 0, 3);
	if (matrix_inv) free_dmatrix(matrix_inv, 1, 4, 1, 4);
...
double	a, b, c, d;
	...
	/* SD / LD */
	corr_sd[0][1] = (1.0 + corr_model[0][1] * alpha_dom * (1.0 + ex_dom) + alpha_dom * alpha_dom * ex_dom) / (a * b);
	/* SD / Fx */
	...
	/* SF / LF */
	corr_sd[2][3] = (1.0 + corr_model[2][3] * alpha_for * (1.0 + ex_for) + alpha_for * alpha_for * ex_for) / (c * d);
	/* SF / Fx */
	...
	/* SD / SF */
	corr_sd[0][2] = (corr_model[0][2] + corr_model[0][3] * alpha_for + corr_model[1][2] * alpha_dom + corr_model[1][3] * alpha_dom * alpha_for) / (a * c);
	/* LD / LF */
	corr_sd[1][3] = (corr_model[0][2] + corr_model[0][3] * alpha_for * ex_for + corr_model[1][2] * alpha_dom * ex_dom + corr_model[1][3] * alpha_dom * alpha_for * ex_dom * ex_for) / (b * d);
	/* SD / LF */
	corr_sd[0][3] = (corr_model[0][2] + corr_model[0][3] * alpha_for * ex_for + corr_model[1][2] * alpha_dom + corr_model[1][3] * alpha_dom * alpha_for * ex_for) / (a * d);
	/* LD / SF */
	corr_sd[1][2] = (corr_model[0][2] + corr_model[0][3] * alpha_for + corr_model[1][2] * alpha_dom * ex_dom + corr_model[1][3] * alpha_dom * alpha_for * ex_dom) / (b * c);

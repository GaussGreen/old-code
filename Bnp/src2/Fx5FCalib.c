/* ==========================================================================
   ...
   PURPOSE:		Modelling of the spot FX vol by taking in consideration a
   LGM 2 factor on the domestic and foreign market and a lognormal model on the
   Spot Fx
...
static Err Coefs_Partial_Var(double T, double T1, double T2, double sig_dom1,
                             double sig_dom2, double lda_dom1, double lda_dom2,
                             double sig_for1, double sig_for2, double lda_for1,
                             double lda_for2, double **correl, double *a,
                             double *b, double *c) {
  ...
  (*c) = sig_for1 * sig_for1 * Psi_Func(lda_for1, lda_for1, T, T1, T2) +
         ...
         2.0 * correl[0][2] * sig_dom1 * sig_for1 *
             Psi_Func(lda_for1, lda_dom1, T, T1, T2) -
         2.0 * correl[0][3] * sig_dom1 * sig_for2 *
             Psi_Func(lda_for2, lda_dom1, T, T1, T2) +
         2.0 * correl[0][1] * sig_dom1 * sig_dom2 *
             Psi_Func(lda_dom1, lda_dom2, T, T1, T2) -
         2.0 * correl[1][2] * sig_dom2 * sig_for1 *
             Psi_Func(lda_for1, lda_dom2, T, T1, T2) -
         2.0 * correl[1][3] * sig_dom2 * sig_for2 *
             Psi_Func(lda_for2, lda_dom2, T, T1, T2) +
         2.0 * correl[2][3] * sig_for2 * sig_for1 *
             Psi_Func(lda_for1, lda_for2, T, T1, T2);

  return NULL;
}

static Err Partial_Var(double T, double T1, double T2, double sig_dom1,
                       double sig_dom2, double lda_dom1, double lda_dom2,
                       double sig_for1, double sig_for2, double lda_for1,
                       double lda_for2, double sig_fx, double **correl,
                       double *var) {
  double a;
  double b;
  double c;
  ...
  /* Get Coefs */
  err = Coefs_Partial_Var(T, T1, T2, sig_dom1, sig_dom2, lda_dom1, lda_dom2,
                          sig_for1, sig_for2, lda_for1, lda_for2, correl, &a,
                          &b, &c);
...
Err Fx5DtsImpliedVol(double opt_maturity, double start_date, double end_date,
                     double *maturity_rates, long nbMat, double *sig_curve_dom,
                     double lda_dom, double alpha_dom, double gamma_dom,
                     double *sig_curve_for, double lda_for, double alpha_for,
                     double gamma_for, double *maturity_fx,
                     double *sig_curve_fx, long nbrMat_fx, double **correl,
                     double *fx_vol) {
  ...
  for (i = StartIndex; i < EndIndex + 1; i++) {
    ...
    for (j = StartIndex2; j < EndIndex2 + 1; j++) {
      ...
      err = Partial_Var(opt_maturity, t1, t2, sig_dom1, sig_dom2, lda_dom,
                        lda_dom2, sig_for1, sig_for2, lda_for, lda_for2, sig_fx,
                        correl, &var_partial);
...
Err Fx5DtsCalibration(double *exercise_opt, double *maturity_opt,
                      double *vol_opt, long nbrOpt, double *maturity_rates,
                      long nbrMat, double *sig_curve_dom, double lda_dom,
                      double alpha_dom, double gamma_dom, double *sig_curve_for,
                      double lda_for, double alpha_for, double gamma_for,
                      double **correl, double **fx_vol_curve)

{
  ...
  for (idxopt = 0; idxopt <= nbrOpt - 1; idxopt++) {
    ...
    if (idxopt > 0) {
      ...
      err = Fx5DtsImpliedVol(val_opt, 0, T1, maturity_rates, nbrMat,
                             sig_curve_dom, lda_dom, alpha_dom, gamma_dom,
                             sig_curve_for, lda_for, alpha_for, gamma_for,
                             exercise_opt, *fx_vol_curve, nbrOpt, correl, &vol);
    ...
    for (i = StartIndex; i < EndIndex + 1; i++) {
      ...
      err = Coefs_Partial_Var(val_opt, t1, t2, sig_dom1, sig_dom2, lda_dom,
                              lda_dom2, sig_for1, sig_for2, lda_for, lda_for2,
                              correl, &a_part, &b_part, &c_part);
...
Err Fx5DImpliedVol(char *fx_underlying, double **correl, double val_time,
                   double start_time, double end_time, double *vol) {
  ...
  err = Get_FX_StochRate_TermStructures5F(
      fx_underlying, &sigma_date_dom, &sigma_dom, &sigma_n_dom, &tau_dom,
      &alpha_dom, &gamma_dom, &rho_dom, &sigma_date_for, &sigma_for,
      &sigma_n_for, &tau_for, &alpha_for, &gamma_for, &rho_for, &sigma_date_fx,
      &sigma_fx, &sigma_n_fx, &correl_dom_for, &correl_dom_fx, &correl_for_fx);
  if (err) {
    goto FREE_RETURN;
  }
  ...
  err = Fx5DtsImpliedVol(val_time, start_time, end_time, merge_dates,
                         nb_merge_dates, sig_dom, lda_dom, alpha_dom, gamma_dom,
                         sig_for, lda_for, alpha_for, gamma_for, sigma_date_fx,
                         sigma_fx, sigma_n_fx, correl, vol);
...
Err Fx5DCalibration(char *dom_underlying, char *for_underlying, double **correl,
                    double *exercise_opt, double *maturity_opt, double *vol_opt,
                    long nbropt, double **fx_vol_curve) {
  ...
  double tau_dom, lda_dom, alpha_dom, gamma_dom, rho_dom, tau_for, lda_for,
      alpha_for, gamma_for, rho_for;

  Err err = NULL;

  err = Get_LGM2F_TermStructure(dom_underlying, &sigma_date_dom, &sigma_dom,
  ...
  err = Get_LGM2F_TermStructure(for_underlying, &sigma_date_for, &sigma_for,
                                &sigma_n_for, &tau_for, &alpha_for, &gamma_for,
                                &rho_for);
  if (err) {
    goto FREE_RETURN;
  }
  ...
  err = Fx5DtsCalibration(exercise_opt, maturity_opt, vol_opt, nbropt,
                          merge_dates, nb_merge_dates, sig_dom, lda_dom,
                          alpha_dom, gamma_dom, sig_for, lda_for, alpha_for,
                          gamma_for, correl, fx_vol_curve);
...
Err Get_FX_StochRate_TermStructures5F(
    ...
    long *sigma_n_dom, double *fixed_tau_dom, double *fixed_alpha_dom,
    double *fixed_gamma_dom, double *fixed_rho_dom, double **sigma_date_for,
    double **sigma_for, long *sigma_n_for, double *fixed_tau_for,
    double *fixed_alpha_for, double *fixed_gamma_for, double *fixed_rho_for,
  ...
  *sigma_fx = NULL;
  ...
  err = Get_LGM2F_TermStructure(domname, sigma_date_dom, sigma_dom, sigma_n_dom,
                                fixed_tau_dom, fixed_alpha_dom, fixed_gamma_dom,
                                fixed_rho_dom);
  if (err) {
    goto FREE_RETURN;
  }
  ...
  err = Get_LGM2F_TermStructure(forname, sigma_date_for, sigma_for, sigma_n_for,
                                fixed_tau_for, fixed_alpha_for, fixed_gamma_for,
                                fixed_rho_for);
...
Err calibrate_correl5D_from_histo(
    ...
    double lam_dom, double alpha_dom, double gamma_dom, double rho_dom,
                          ...
                          */
{
  ...
  if (calib_rho_dom) {
    ...
    a = alpha_dom * alpha_dom *
        (4 * corr2 * ex_dom - (1 + ex_dom) * (1 + ex_dom));
    b = alpha_dom * (1 + ex_dom) * (1 + alpha_dom * alpha_dom * ex_dom) *
        (corr2 - 1.0);
    c = corr2 * (1.0 + alpha_dom * alpha_dom) *
            (1.0 + alpha_dom * alpha_dom * ex_dom * ex_dom) -
        (1.0 + alpha_dom * alpha_dom * ex_dom) *
            (1.0 + alpha_dom * alpha_dom * ex_dom);
  ...
  c = 1.0 /
      sqrt(1.0 + alpha_dom * ex_dom * (2.0 * rho_dom + alpha_dom * ex_dom));
  ...
  if (fabs(delta) < 1.0E-10) {
    ...
    corr_model[0][4] =
        1.0 / delta * (d * corr_histo[0][4] - b * corr_histo[1][4]);
    corr_model[1][4] =
        1.0 / delta * (a * corr_histo[1][4] - c * corr_histo[0][4]);
  }
  ...
  if (calib_rho_for) {
    ...
    a = alpha_for * alpha_for *
        (4 * corr2 * ex_for - (1 + ex_for) * (1 + ex_for));
    b = alpha_for * (1 + ex_for) * (1 + alpha_for * alpha_for * ex_for) *
        (corr2 - 1.0);
    c = corr2 * (1.0 + alpha_for * alpha_for) *
            (1.0 + alpha_for * alpha_for * ex_for * ex_for) -
        (1.0 + alpha_for * alpha_for * ex_for) *
            (1.0 + alpha_for * alpha_for * ex_for);
  ...
  c = 1.0 /
      sqrt(1.0 + alpha_for * ex_for * (2.0 * rho_for + alpha_for * ex_for));
  ...
  if (fabs(delta) < 1.0E-10) {
    ...
    corr_model[2][4] =
        1.0 / delta * (d * corr_histo[2][4] - b * corr_histo[3][4]);
    corr_model[3][4] =
        1.0 / delta * (a * corr_histo[3][4] - c * corr_histo[2][4]);
  }

  /* Fifth step  , solve for the others in a big system */

  a = sqrt((1.0 + alpha_dom * (2.0 * rho_dom + alpha_dom)) *
           (1.0 + alpha_for * (2.0 * rho_for + alpha_for)));
  b = sqrt((1.0 + alpha_dom * ex_dom * (2.0 * rho_dom + alpha_dom * ex_dom)) *
           (1.0 + alpha_for * ex_for * (2.0 * rho_for + alpha_for * ex_for)));
  c = sqrt((1.0 + alpha_dom * (2.0 * rho_dom + alpha_dom)) *
           (1.0 + alpha_for * ex_for * (2.0 * rho_for + alpha_for * ex_for)));
  d = sqrt((1.0 + alpha_dom * ex_dom * (2.0 * rho_dom + alpha_dom * ex_dom)) *
           (1.0 + alpha_for * (2.0 * rho_for + alpha_for)));
  ...
  corr_model[0][2] = matrix_inv[1][1] * corr_histo[0][2] +
                     matrix_inv[1][2] * corr_histo[1][3] +
                     matrix_inv[1][3] * corr_histo[0][3] +
                     matrix_inv[1][4] * corr_histo[1][2];

  corr_model[0][3] = matrix_inv[2][1] * corr_histo[0][2] +
                     matrix_inv[2][2] * corr_histo[1][3] +
                     matrix_inv[2][3] * corr_histo[0][3] +
                     matrix_inv[2][4] * corr_histo[1][2];

  corr_model[1][2] = matrix_inv[3][1] * corr_histo[0][2] +
                     matrix_inv[3][2] * corr_histo[1][3] +
                     matrix_inv[3][3] * corr_histo[0][3] +
                     matrix_inv[3][4] * corr_histo[1][2];

  corr_model[1][3] = matrix_inv[4][1] * corr_histo[0][2] +
                     matrix_inv[4][2] * corr_histo[1][3] +
                     matrix_inv[4][3] * corr_histo[0][3] +
                     matrix_inv[4][4] * corr_histo[1][2];
...
Err get_correlSL_from_model(
    ...
    double **
        corr_sd) /* ouput for correl between SD  , LD  , SF  , LF  , Fx	*/
{
  ...
  b = sqrt(1.0 +
           alpha_dom * ex_dom * (2.0 * corr_model[0][1] + alpha_dom * ex_dom));
  c = sqrt(1.0 + alpha_for * (2.0 * corr_model[2][3] + alpha_for));
  d = sqrt(1.0 +
           alpha_for * ex_for * (2.0 * corr_model[2][3] + alpha_for * ex_for));

  /* SD / LD */
  corr_sd[0][1] = (1.0 + corr_model[0][1] * alpha_dom * (1.0 + ex_dom) +
                   alpha_dom * alpha_dom * ex_dom) /
                  (a * b);
  /* SD / Fx */
  corr_sd[0][4] = (corr_model[0][4] + corr_model[1][4] * alpha_dom) / a;
  /* LD / Fx */
  corr_sd[1][4] =
      (corr_model[0][4] + corr_model[1][4] * alpha_dom * ex_dom) / b;

  /* SF / LF */
  corr_sd[2][3] = (1.0 + corr_model[2][3] * alpha_for * (1.0 + ex_for) +
                   alpha_for * alpha_for * ex_for) /
                  (c * d);
  /* SF / Fx */
  corr_sd[2][4] = (corr_model[2][4] + corr_model[3][4] * alpha_for) / c;
  /* LF / Fx */
  corr_sd[3][4] =
      (corr_model[2][4] + corr_model[3][4] * alpha_for * ex_for) / d;

  /* SD / SF */
  corr_sd[0][2] = (corr_model[0][2] + corr_model[0][3] * alpha_for +
                   corr_model[1][2] * alpha_dom +
                   corr_model[1][3] * alpha_dom * alpha_for) /
                  (a * c);
  /* LD / LF */
  corr_sd[1][3] = (corr_model[0][2] + corr_model[0][3] * alpha_for * ex_for +
                  ...
                  (b * d);
  /* SD / LF */
  corr_sd[0][3] = (corr_model[0][2] + corr_model[0][3] * alpha_for * ex_for +
                   corr_model[1][2] * alpha_dom +
                   corr_model[1][3] * alpha_dom * alpha_for * ex_for) /
                  (a * d);
  /* LD / SF */
  corr_sd[1][2] = (corr_model[0][2] + corr_model[0][3] * alpha_for +
                   corr_model[1][2] * alpha_dom * ex_dom +

...
/* ------------------------------------------------------------------------- */
/* Create an object and insert it in the list from its components (ticker
 * update) */
Err srt_f_lstins(SrtList *l, char *name, double key, int type, void *Ptr,
                 Err (*ObjValPvalFreeFct)(void *), long *ticker) {
  SrtObject *objPtr;
  SrtObjVal *objval;
  Err err = 0;
...
SRT_Boolean srt_f_objequal(SrtObject *obj1, SrtObject *obj2) {
  int res;
  res = (((strcmp(obj1->name, obj2->name) == 0) ? 1 : 0) &&
         (obj1->key == obj2->key));
  if (res == 1)
    return SRT_YES;
  else
...
Err srt_f_objset(char *name, double key, int type, SrtObjVal *val,
                 SrtObject **Ptr, SRT_Boolean ObjValPtr_allocated,

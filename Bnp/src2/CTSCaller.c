...
Err	cts_set_default_params(	int						*accrue_on_barrier,
							int						*value_zero,
							int						*use_cmsopt,
							double					*correl_start,
							double					*correl_end,
							...
							int						*compatibility_flag)
{
	...
	*numer_tstar = 0;
	...
	lgmsv_app_set_default_params(	iNbX,
									iNbSigmaXLeft,
									iNbSigmaXRight,
									dIntegParam,
									iIntegMethod,
...
double			*fund_mrg2		= NULL,
	...
	if (!cts_pricing || !cts_on_iv || !cts_iv_adj || !und || !und_iv || !und_iv_adj ||
		 !adi_arg || !fwd_iv_secu || !fwd_iv_reserve || !cpn2 || !mkt || !NumerParams)
	{
	...
	lgmsv_init_numer_params(	NumerParams,
								iNbX,
								iNbSigmaXGridLeft,
								iNbSigmaXGridRight,
								dIntegParam,
	...
	if (cts_on_iv->fund_leg->num_cpn > 0)
	{
		if (for_fund)
		{
			fund_leg_pv += swp_f_df (today, cts_on_iv->fund_leg->cpn[0].start_date, yc) * eq_final_ex;
			fund_leg_pv += swp_f_df (today, cts_on_iv->fund_leg->cpn[cts_on_iv->fund_leg->num_cpn-1].pay_date, mkt->yc)
						* cts_on_iv->fund_leg->cpn[cts_on_iv->fund_leg->num_cpn-1].not;
		}
		else
		{
			fund_leg_pv += swp_f_df (today, cts_on_iv->fund_leg->cpn[0].start_date, yc) * fund_not_ts2[0];
		}
	}
	else
	{
	...
	if (!compatibility_flag)
	{
		...
		while (i < ncpn && fix_dates[i][0] < today + eod_fix_flag)
		{
			if (cpn_pay_date[i] >= today + eod_pay_flag)
			{
				...
				err = cts_calc_fixed_cpn_mkt_value(	mkt,
													cpn_start_date[i],
													cpn_end_date[i],
													cpn_pay_date[i],
													cpn_coupon[i],
				...
				if (j0 < nfix[i])
				{
					...
					if (pay_fix_date[i] < today + eod_fix_flag && (cpn_type[i] == 1 || cpn_type[i] == 2))
					{
		...
		if (for_fund)
		{
			...
			{
				exo_leg_pv += swp_f_df (today, cts_on_iv->exo_leg->cpn[cts_on_iv->exo_leg->num_cpn-1].cpn_pay_date, yc) * cts_on_iv->exo_leg->cpn[cts_on_iv->exo_leg->num_cpn-1].cpn_not;
			}
			else
			{
				fin_not_date = fund_pay[fund_ncpn-1];
	...
	{		
		...
		{
			j = 0;
			while (j < cts_on_iv->exo_leg->num_cpn && cts_on_iv->exo_leg->cpn[j].cpn_start_date < ex_date[i])
			{
				j++;
			}
	...
	if ((exercised) && (fund_pay[fund_ncpn-1] != ex_date_set))
	{
		...
		if (for_fund)
		{
			exo_leg_pv += (swp_f_df (today, ex_date_set, yc) - 
							 swp_f_df (today, fund_pay[fund_ncpn-1], yc)) * cpn_not_ts[ncpn - 1];
		}
	...
	*call_val = call;
	...
	if (export_und && call_feat == 1)
	{
		...
		ConvertTS_LGMSV_to_LGM(	und_exp->model.iNbPWTime,
	...
	{				
		err = cts_calc_reserve(	cts_for_iv,
								cts_pricing,
								und,
								adi_arg,
								for_fund,
...
FREE_RETURN:
	...
	if (cts_pricing) free(cts_pricing);
	if (cts_on_iv) free(cts_on_iv);
	if (cts_iv_adj) free(cts_iv_adj);

	if (und) free(und);
	if (und_iv) free(und_iv);
	if (und_iv_adj) free(und_iv_adj);

	if (adi_arg) free(adi_arg);

	if (fwd_iv_secu) free(fwd_iv_secu);
	if (fwd_iv_reserve) free(fwd_iv_reserve);
	...
	if (fund_not_ts2) free (fund_not_ts2);

	if (NumerParams) free(NumerParams);

        ...
        inst_data) /*	NULL = don't save calibration instrument data */
{
  ...
  if (fix_lambda == 0)
    return serror("old_cpd_calib_diagonal_wrapper can only be used with "
                  "fix_lambda = 1 for now");
  ...
  err = cpd_calib_diagonal_dlm(
      yc_name, vol_curve_name, get_cash_vol, ref_rate_name, swaption_freq,
      swaption_basis, ref_rate_name, num_ex_dates, used_ex_date, cal_datel,
      end_tenorl, end_date, (long_strike ? long_strike : strike), &calib_param,
      NULL, NULL, NULL, 0, NULL, NULL, NULL, 0, NULL, NULL, NULL, 1, one_f_equi,
      1, &lam_time, lambda, &lam_shift, one2F, alpha, gamma, rho,

      0, NULL, NULL, 0, NULL, NULL,

      num_sig, sig_time, sig, &lm_params, inst_data);

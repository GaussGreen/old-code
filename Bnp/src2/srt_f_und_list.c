    ...
    double        spot)
{
    ...
    case INTEREST_RATE_UND:
        ...
        if (mdl_lbl != 0)
        {
            if (err = srt_f_interp_model(mdl_lbl, &(undint->mdl_type), &(undint->mdl_dim)))
                return err;
    ...
    case EQUITY_UND:
        ...
        if (mdl_lbl != 0)
        {
            if (err = srt_f_interp_model(mdl_lbl, &(undeq->mdl_type), &(undeq->mdl_dim)))
                return err;
    ...
    case FOREX_UND:
        ...
        if (mdl_lbl != 0)
        {
            if (err = srt_f_interp_model(mdl_lbl, &(undfx->mdl_type), &(undfx->mdl_dim)))
                return err;
    ...
    err = srt_f_lstins(
        und_list, und_name, 0.0, OBJ_PTR_UND, und, &srt_f_undvalfree, &(und->underl_ticker));
    if (err)

						...
						int		*stop_path)
{
	...
	if (total->do_dom)
	{
		...
		{			
			local->evt->df[total->dom_idx][l] = total->dom_dff[l] * exp(	total->dom_gam12[l]	- total->dom_gam1[l] * R1D +
	...
	if (total->do_fx)
	{
		...
		{
			local->evt->df[total->fx_idx][l] = total->fx_dff[l] * exp(	total->fx_gam12[l]	- total->fx_gam1[l] * R1D +
																				- total->fx_gam2[l] * R2D);
		}
	}
	if (total->do_for)
	{
		...
		{
			local->evt->df[total->for_idx][l] = total->for_dff[l] * exp(	total->for_gam12[l]	- total->for_gam1[l] * R1F +

    ...
    if (total->do_dom)
    {
        for (l = 0; l < total->num_dom_df; l++)
        {
            local->evt->df[total->dom_idx][l] =
                total->dom_dff[l] *
                exp(total->dom_gam12[l] - total->dom_gam1[l] * R1D + -total->dom_gam2[l] * R2D);
        }
    }
    if (total->do_fx)
    {
        for (l = 0; l < total->num_fx_df; l++)
        {
            local->evt->df[total->fx_idx][l] =
                total->fx_dff[l] *
                exp(total->fx_gam12[l] - total->fx_gam1[l] * R1D + -total->fx_gam2[l] * R2D);
        }
    }
    if (total->do_for)
    {
        for (l = 0; l < total->num_for_df; l++)
        {
            local->evt->df[total->for_idx][l] =
                total->for_dff[l] *
                exp(total->for_gam12[l] - total->for_gam1[l] * R1F + -total->for_gam2[l] * R2F);
        }
    }

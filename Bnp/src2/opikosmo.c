...
static double 	iko_ext_fct_binomial3	(
				double 	fwd_price2, 
				double 	fwd_price1, 
				double 	spot_price,
				double 	barrier,
...
double answer;
	...
	else
	{
		...
		for (k = 1; k <= 25; k++)
		{
					...
					***/
	    			...
	    			{           
						...
						***/
		   				...
		   				{ 
		   					strike_end = ( (cp * strikes[q]) > (cp * strikes[1]) ? strikes[q] : strikes[1]);
		   				}
	    			}

	    			dd = (double)j;
	    			prob =	comb(dd, step_num, p, i) - 
			       			comb(dd - 1, step_num, p, i);

	    			opt = srt_f_optexting(	spot_end / disc2,
											spot_end,
											strike_end,
											barrier,
											vol2,
			...
			/* binomial tree with payoffs at t=T1 set at premium[step_num] obtained above */
...
double 	cp = ( call_put == SRT_CALL ) ? 1 : -1 ;
	...
	***/
	...
	premium = iko_ext_fct_binomial3(	fwd_price1, 
										fwd_price2, 
										spot_price,
										barrier, 
					       				vol1, 
	...
	{	
		...
		case DELTA_FWD1:
			...
			answer = (srt_f_optikosmo (	fwd_price2, 
										fwd_price1 + shift,
										spot_price, 
										barrier,
										hist_extreme, 
		...
		case DELTA_FWD2:
			...
			answer = (srt_f_optikosmo (	fwd_price2 + shift, 
										fwd_price1,
										spot_price, 
										barrier,
										hist_extreme, 

...
#define FREE_MC_CORE                                                                        \
    {                                                                                       \
        ...
        if (path_cash_flows)                                                                \
            free_dmatrix(path_cash_flows, 0, num_time_pts - 1, 0, grfnparam->node_dim - 1); \
    }
    ...
    SrtUndInfo*   und_info)
{
                ...
                if (mdl_type == FX_STOCH_RATES)
                {
                        ...
                        (und_info->jumping == SRT_YES))
                    {
                        err = srt_f_fx_initstp(
                            top, und_info, und, dom_und, for_und, i, numeraire_und);
                        if (err)
                        {
                            FREE_MC_CORE;
                            return err;
    ...
    if ((und_info->no_of_underlyings > 1) && (und_info->two_factor_model == SRT_NO) &&
    ...
    if ((dfs == NULL) || (sum_disc_cf == NULL) || (Z_sum == NULL) || (Pt_by_Zt == NULL))
    {
        FREE_MC_CORE;
    ...
    memset(
        &deal_cash_flows_pv[0][0], 0, (int)(num_time_pts * grfnparam->node_dim * sizeof(double)));
    ...
    for (n = 0; n < grfnparam->num_MCarlo_paths; n++)
    {
        ...
        for (j = 0; j < 2; j++)
        {
            for (i = 0; i < und_info->no_of_underlyings; i++)
            {
                ...
                if (((j == 0) &&
                     (!((mdl_type == EQ_STOCH_RATES) || (mdl_type == EQ_STOCH_RATES_SRVGS))) &&
                    ...
                    ((j == 1) && (mdl_type == EQ_STOCH_RATES_SRVGS)))
                {
                    ...
                    if (und_info->und_data[i].stochastic == SRT_YES)
                    {
                        ...
                        case EQ_STOCH_RATES:
                            ...
                            monte_EQ_STOCH_RATES_evolve(
                                mcrs.r[mcrs.cur_path][i], top, sam, i, dom_ind);
                            break;
                        case EQ_STOCH_RATES_SRVGS:
                            err = get_index_from_und_info(
                                und_info, get_discname_from_underlying(und), &dom_ind);
                            ...
                            monte_EQ_STOCH_RATES_SRVGS(
                                mcrs.r[mcrs.cur_path][i],
                                mcrs.r[mcrs.cur_path][und_info->no_of_brownians - 1],
                                top,
                                sam,
                                i,
                                dom_ind);
                        ...
                        case FX_STOCH_RATES: /* DONE ONLY FOR FOREX UNDERLYING */
                                ...
                                (for_mdl_type == LGM) && (und_info->jumping == SRT_YES))
                            {
                                monte_FX_STOCH_RATES_Jumping_evolve(
                                    mcrs.r[mcrs.cur_path][i], top, sam, i, dom_ind, for_ind);
                            }
                            else

                                monte_FX_STOCH_RATES_evolve(
                                    mcrs.r[mcrs.cur_path][i], top, sam, i, dom_ind, for_ind);
                        ...
                        case LGM:
                            ...
                            {
                                if (und_info->jumping == SRT_YES)
                                    monte_LGM_1f_Jumping_evolve(
                                        mcrs.r[mcrs.cur_path][i], top, sam, i);
                                else if (und_info->jumping == SRT_NO)
                                    monte_LGM_1f_Euler_evolve(
                                        mcrs.r[mcrs.cur_path][i], top, sam, i);
                            }
                        ...
                        case VASICEK:
                            ...
                            break;
                        case NEWCHEYBETA:
                            monte_NEWCHEYBETA_1f_Euler_evolve(
                                mcrs.r[mcrs.cur_path][i], top, sam, i);
                            break;
                        case LGM_STOCH_VOL:
                            monte_LGM_1f_STOCHVOL_evolve(
                                mcrs.r[mcrs.cur_path], top, sam, Pt_by_Zt, i);
                            break;
                        case CHEY_STOCH_VOL:
                            monte_CHE_1f_STOCHVOL_evolve(
                                mcrs.r[mcrs.cur_path], top, sam, Pt_by_Zt, i);
                            break;
                        case CHEY_BETA_STOCH_VOL:
                            monte_CHEYBETA_1f_STOCHVOL_evolve(
                                mcrs.r[mcrs.cur_path], top, sam, Pt_by_Zt, i);
                            break;
                        case CHEY_BETA:
                            if (mdl_dim == ONE_FAC)
                            {
                                monte_CHEYBETA_1f_Euler_evolve(
                                    mcrs.r[mcrs.cur_path][i], top, sam, i);
                            }
                            else if (mdl_dim == TWO_FAC)
                            {
                        ...
                        default:
                            if (mdl_dim == ONE_FAC)
                            {
                                ...
                                {
                                    monte_CHE_1f_Milshtein_evolve(
                                        mcrs.r[mcrs.cur_path][i], top, sam, i);
                                }
                                else /* EULER */
                                {
                                    monte_CHE_1f_Euler_evolve(
                                        mcrs.r[mcrs.cur_path][i], top, sam, i);
                                }
                            }
                            else if (mdl_dim == TWO_FAC)
        ...
        for (t = 0, step = top; t < num_time_pts; step = step->next, t++)
        {
            ...
            {
                /* Here we multiply by dfs[t] that has been removed from the numeraire in HJM type
                   model, (it has been computed with exp(r - f(0,t))  or exp(A(t)+h(t,x))  */
            ...
            /* The zero-coupon price computed for this time step (sum on all paths) (without its
             * forward) */
    ...
    {
        /* The correction factor is (at each step) the zero-coupon bond price computed by full MC
         * simulation  */
    ...
    exp_disc_pay_sqr *=
        1.0 + DBL_EPSILON * grfnparam->num_MCarlo_paths; /* To prevent rounding errors */
    stdev = (sqrt(
                exp_disc_pay_sqr / grfnparam->num_MCarlo_paths - exp_disc_pay * exp_disc_pay /
    ...
    err =
        srt_f_IOstructsetcolpvs((SrtIOStruct*)iorequest, SRT_NO, col_pvs, grfnparam->node_dim, "");
    if (err)
    {
        FREE_MC_CORE;
        return err;
    ...
    for (n = nstart; n < nend; n++)
    {
        ...
        for (j = 0; j < 2; j++)
        {
            for (i = 0; i < und_info->no_of_underlyings; i++)
            {
                ...
                if (((j == 0) && (!(mdl_type == EQ_STOCH_RATES)) && (!ISUNDTYPE(und, FOREX_UND))) ||
                    ((j == 1) && (ISUNDTYPE(und, FOREX_UND))) ||
                    ((j == 1) && (mdl_type == EQ_STOCH_RATES)))
                {
                    /* if underlying is stochastic, set up sam[t]; otherwise sam[t]. pv_money_mkt

...
Err MCCore(SrtGrfnParam *grfnparam, SrtStpPtr step, GrfnDeal *gd,
  ...
  /* Attaches the right correlation coefficients to the right steps  , when
   * needed  */
    ...
    for (j = 0; j < 2; j++) {
      for (i = 0; i < und_info->no_of_underlyings; i++) {
        ...
        if (((j == 0) &&
          ...
          /* if underlying is stochastic  , set up sam[t]; otherwise sam[t].
          pv_money_mkt
          ...
          if (und_info->und_data[i].stochastic == SRT_YES) {
            ...
            case EQ_STOCH_RATES:
              ...
              monte_EQ_STOCH_RATES_evolve(mcrs.r[mcrs.cur_path][i], top, sam, i,
                                          dom_ind);
              break;
            case EQ_STOCH_RATES_SRVGS:
              err = get_index_from_und_info(
              ...
              monte_EQ_STOCH_RATES_SRVGS(
                  ...
                  mcrs.r[mcrs.cur_path][und_info->no_of_brownians - 1], top,
                  sam, i, dom_ind);
              break;
            case FX_STOCH_RATES: /* DONE ONLY FOR FOREX UNDERLYING */
              ...
              if ((mdl_type == FX_STOCH_RATES) && (dom_mdl_type == LGM) &&
                ...
                monte_FX_STOCH_RATES_evolve(mcrs.r[mcrs.cur_path][i], top, sam,
                                            i, dom_ind, for_ind);
            ...
            case LGM:
              if (mdl_dim == ONE_FAC) {
                if (und_info->jumping == SRT_YES)
                  monte_LGM_1f_Jumping_evolve(mcrs.r[mcrs.cur_path][i], top,
                                              sam, i);
                else if (und_info->jumping == SRT_NO)
                  monte_LGM_1f_Euler_evolve(mcrs.r[mcrs.cur_path][i], top, sam,
                                            i);
              ...
              break;
            case NEWLGM:
              monte_NEWLGM_1f_Euler_evolve(mcrs.r[mcrs.cur_path][i], top, sam,
                                           i);
              break;
            case VASICEK:
              monte_vasicek_1f_euler_evolve(mcrs.r[mcrs.cur_path][i], top, sam,
                                            i);
              break;
            case NEWCHEYBETA:
              monte_NEWCHEYBETA_1f_Euler_evolve(mcrs.r[mcrs.cur_path][i], top,
                                                sam, i);
              break;
            case LGM_STOCH_VOL:
              monte_LGM_1f_STOCHVOL_evolve(mcrs.r[mcrs.cur_path], top, sam,
                                           Pt_by_Zt, i);
              break;
            case CHEY_STOCH_VOL:
              monte_CHE_1f_STOCHVOL_evolve(mcrs.r[mcrs.cur_path], top, sam,
                                           Pt_by_Zt, i);
              break;
            case CHEY_BETA_STOCH_VOL:
              monte_CHEYBETA_1f_STOCHVOL_evolve(mcrs.r[mcrs.cur_path], top, sam,
                                                Pt_by_Zt, i);
              break;
            case CHEY_BETA:
              if (mdl_dim == ONE_FAC) {
                monte_CHEYBETA_1f_Euler_evolve(mcrs.r[mcrs.cur_path][i], top,
                                               sam, i);
              } else if (mdl_dim == TWO_FAC) {
                monte_CHEYBETA_2f_Euler_evolve(mcrs.r[mcrs.cur_path], top, sam,
                                               i);
              }
              break;
            case MIXED_BETA:
              if (mdl_dim == ONE_FAC) {
                ...
                monte_MIXEDBETA_2f_Euler_evolve(mcrs.r[mcrs.cur_path], top, sam,
                                                i);
              }
              break;
            case CHEY:
            default:
              if (mdl_dim == ONE_FAC) {
                if (grfnparam->difference_scheme == MILSHTEIN) {
                  monte_CHE_1f_Milshtein_evolve(mcrs.r[mcrs.cur_path][i], top,
                                                sam, i);
                } else /* EULER */
                {
                  monte_CHE_1f_Euler_evolve(mcrs.r[mcrs.cur_path][i], top, sam,
                                            i);
                }
              } else if (mdl_dim == TWO_FAC) {
  ...
  /* The Real price is the price obtained by Monte Carlo  , renormalised step by
   * step */
  sum_price = 0.0;

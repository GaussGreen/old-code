...
#define FREE_MC_CORE {	\
			if (grfnparam->calib == SRT_NO) SrtMCRanSrc_end(&mcrs);\
			if (dfs) free_dvector(dfs,0,num_time_pts);\
			if (sum_disc_cf) free_dvector(sum_disc_cf,0,num_time_pts);\
			if (Z_sum) free_dvector(Z_sum,0,num_time_pts);\
			if (Pt_by_Zt) free_dvector(Pt_by_Zt,0,num_time_pts);\
			if (sam) srt_free(sam);\
			if (col_pvs) free_dvector(col_pvs,0, grfnparam->node_dim-1);\
			if (path_cash_flows) free_dmatrix(path_cash_flows, 0, num_time_pts - 1, 0, grfnparam->node_dim-1);\
	}


#define FREE_MC_CORE_EXFR {	\
			if (grfnparam->exfrontierlast_time==SRT_YES) SrtMCRanSrc_end(&mcrs);\
			if (dfs) free_dvector(dfs,0,num_time_pts);\
			if (sum_disc_cf) free_dvector(sum_disc_cf,0,num_time_pts);\
			if (Z_sum) free_dvector(Z_sum,0,num_time_pts);\
			if (sam) srt_free(sam);\
...
/* ----------------------------------------------------------------
  ...
  DESCRIPTION     :computes prices of assets/contingent claims
   ...
   VECTOR of sample structures, and is called each time for iterating through 
   each one of the (num_mcarlo_paths). 
  MODIFIES        : gd,price,mcrs
  CALL            :
...
int		dom_ind, for_ind;	/* index for domestic and foreign underlying when quanto adjustment */
...
/* Initialises mcrs according to inputs, makes memory allocation or
   even generates fully the non correlated random numbers */
	...
	{
		err = SrtMCRanSrc_start(	&mcrs,
									0, grfnparam->num_MCarlo_paths-1,
									1, last_index  ,
									und_info->no_of_brownians, 
									grfnparam->sample_type,	
									seed, 
									top);
		if (err)
		{
...
/* Keep track of the Main Currency (for Quanto adjustments)  (the One of the Numeraire) */
	...
	/* Gets the Relevant Underlying (in the order of the Grfn Tableau) */
		...
		/* Initialisation for Black Sholes like diffusions */
			if ((mdl_type == BLACK_SCHOLES) || (mdl_type == NORMAL_BS) || 
				(mdl_type == FX_STOCH_RATES) || (mdl_type == EQ_STOCH_RATES) || (mdl_type == EQ_STOCH_RATES_SRVGS))
			{
				...
				/* Force the Jumping if possible  and set in srt_f_grfn_und*/
					if((dom_mdl_type == LGM) && (for_mdl_type == LGM) && (und_info->jumping == SRT_YES))
					{
						err = srt_f_fx_initstp(top,und_info,und,dom_und,for_und,i, numeraire_und);
						if(err)
						{
							FREE_MC_CORE;
							return err;
		...
		/* This is required for Y_T_at_t_compute to work properly */
			...
			if ( (mdl_type == LGM) || (mdl_type == LGM_STOCH_VOL))
			{
					...
					switch (mdl_dim)
					{
			  		case ONE_FAC:
    			     		sam_get(sam[t],i,PHI) = 
							sam_get(tminf->fwd_sam,i,PHI);
			     			break;
			  		case TWO_FAC:
    			     		sam_get(sam[t],i,PHI1) = 
							sam_get(tminf->fwd_sam,i,PHI1);
    			     		sam_get(sam[t],i,PHI2) = 
							sam_get(tminf->fwd_sam,i,PHI2);
    			     		sam_get(sam[t],i,CROSSPHI) = 
							sam_get(tminf->fwd_sam,i,CROSSPHI);
			     			break;
			  		default:
...
/* Attaches the right correlation coefficients to the right steps, when needed  */
	...
	if (  (dfs == NULL) || (sum_disc_cf == NULL)
		|| (Z_sum == NULL) || (Pt_by_Zt == NULL))
	{
		FREE_MC_CORE;
...
/* If we just want the intrinsic value i.e. no brownian is defined, we just
  need one path to evaluate the answer.  Therefore we modified the no. of
  Monte Carlo paths below if this is the case.  */
	if (und_info->no_of_brownians==0)
	  	grfnparam->num_MCarlo_paths = 1 ;

 /* Compute discount factors from 0 to the step date for discounting using dom und */
	t=0;
  	step=top;
...
/* Memory allocation for a vector where all the Columns PV's will be stored */
	...
	memset( &deal_cash_flows_pv[0][0], 0, (int)(num_time_pts *grfnparam->node_dim * sizeof(double) ) );
	memset( &path_cash_flows[0][0], 0, (int)(num_time_pts *grfnparam->node_dim * sizeof(double) ) );
			...
			for (i=0 ; i<und_info->no_of_underlyings ; i++)
	  		{
				...
				if (  ((j == 0) && (! ( (mdl_type == EQ_STOCH_RATES) || (mdl_type == EQ_STOCH_RATES_SRVGS) ) ) 
				...
				/* if underlying is stochastic, set up sam[t]; otherwise sam[t]. pv_money_mkt
				should already be set up above because it is determinstic */
					...
					if (und_info->und_data[i].stochastic==SRT_YES)
	    			{
						...
						{
		  				case NORMAL_BS:
							monte_NORMALBS_evolve(mcrs.r[mcrs.cur_path][i],top,sam,i) ;
							break;
		  				case BLACK_SCHOLES:
							monte_BLACKSCHOLES_evolve(mcrs.r[mcrs.cur_path][i],top,sam,i) ; 
							break ;
						case EQ_STOCH_RATES:
							err = get_index_from_und_info(	und_info,
															get_discname_from_underlying(und),
															&dom_ind);
							if (err)
							{
								FREE_MC_CORE;
							...
							monte_EQ_STOCH_RATES_evolve(mcrs.r[mcrs.cur_path][i],top,sam,i,dom_ind);
							break;
						case EQ_STOCH_RATES_SRVGS:
							err = get_index_from_und_info(	und_info,
															get_discname_from_underlying(und),
							...
							if (err)
							{
							 ...
							 monte_EQ_STOCH_RATES_SRVGS(mcrs.r[mcrs.cur_path][i],mcrs.r[mcrs.cur_path][und_info->no_of_brownians-1],top,sam,i,dom_ind);
							 break;
						case FX_STOCH_RATES: /* DONE ONLY FOR FOREX UNDERLYING */
							err = get_index_from_und_info(	und_info,
															get_domname_from_fxund(und),
															&dom_ind);
							if (err)
							{
								FREE_MC_CORE;
							...
							if((mdl_type == FX_STOCH_RATES)&&(dom_mdl_type==LGM)&&(for_mdl_type==LGM)
								&& (und_info->jumping == SRT_YES) )
							{
								
								monte_FX_STOCH_RATES_Jumping_evolve(mcrs.r[mcrs.cur_path][i],top,sam,i,dom_ind,for_ind);
							}
							else 

							monte_FX_STOCH_RATES_evolve(mcrs.r[mcrs.cur_path][i],top,sam,i,dom_ind,for_ind);
						...
						case ETABETA:
							...
							{
								monte_BETAETA_1f_evolve( mcrs.r[mcrs.cur_path][i],top, sam, i);
							}
							break;
		  				case LGM:
    						if (mdl_dim == ONE_FAC)
							{
			  					if (und_info->jumping == SRT_YES)
			     						monte_LGM_1f_Jumping_evolve(mcrs.r[mcrs.cur_path][i],top,sam,i);  
			  					else
			  					if (und_info->jumping == SRT_NO)
			     						monte_LGM_1f_Euler_evolve(mcrs.r[mcrs.cur_path][i],top,sam,i);  
							}
							else if (mdl_dim == TWO_FAC)
							{
			   					monte_LGM_2f_evolve(mcrs.r[mcrs.cur_path],top,sam, i); 
							}
							break ;
						case NEWLGM:
							monte_NEWLGM_1f_Euler_evolve(mcrs.r[mcrs.cur_path][i],top,sam,i); 
							break ;
						case VASICEK:
							monte_vasicek_1f_euler_evolve(mcrs.r[mcrs.cur_path][i],top,sam,i);
							break;
						case NEWCHEYBETA:
							monte_NEWCHEYBETA_1f_Euler_evolve(mcrs.r[mcrs.cur_path][i],top,sam,i); 
							break ;
						case LGM_STOCH_VOL:
							monte_LGM_1f_STOCHVOL_evolve(mcrs.r[mcrs.cur_path],top,sam,Pt_by_Zt,i); 
							break ;
						case CHEY_STOCH_VOL:
							monte_CHE_1f_STOCHVOL_evolve(mcrs.r[mcrs.cur_path],top,sam,Pt_by_Zt,i); 
							break ;
						case CHEY_BETA_STOCH_VOL:
							monte_CHEYBETA_1f_STOCHVOL_evolve(mcrs.r[mcrs.cur_path],top,sam,Pt_by_Zt,i); 
							break ;
						case CHEY_BETA:
							if (mdl_dim == ONE_FAC)
							{
								monte_CHEYBETA_1f_Euler_evolve(mcrs.r[mcrs.cur_path][i],top,sam,i);
							}
							else 
							if (mdl_dim == TWO_FAC)
							{
								monte_CHEYBETA_2f_Euler_evolve(mcrs.r[mcrs.cur_path],top,sam,i);
							}
							break;
						case MIXED_BETA:
							if (mdl_dim == ONE_FAC)
							...
							if (mdl_dim == TWO_FAC)
							{
								monte_MIXEDBETA_2f_Euler_evolve(mcrs.r[mcrs.cur_path],top,sam,i);
							}
							break;
						case CHEY:  
						default :
    						if (mdl_dim == ONE_FAC)
							{
    			   				if(grfnparam->difference_scheme==MILSHTEIN)
      			   				{
									monte_CHE_1f_Milshtein_evolve(mcrs.r[mcrs.cur_path][i],  
										top, sam, i); 
   			   					}                                    
    			   				else /* EULER */
    			   				{
      								monte_CHE_1f_Euler_evolve(mcrs.r[mcrs.cur_path][i],  
									top, sam, i);
    			   				}
							}
							else if (mdl_dim == TWO_FAC)
							{
			   					monte_CHE_2f_Euler_evolve(mcrs.r[mcrs.cur_path],top,sam, i); 
							}
							break ;
	...
	/* Loops on all the time steps of this path to compute the cash flow */
		...
		{ 
      	...
      	/* Call to the main Grfn COMLL evaluation function */
			...
			{
			/* Here we multiply by dfs[t] that has been removed from the numeraire in HJM type model,
			   (it has been computed with exp(r - f(0,t))  or exp(A(t)+h(t,x))  */
				cash_flow_pv = cash_flow / (sam[t].numeraire /dfs[t] ) ;
				sum_disc_cf[t] += cash_flow_pv;
				temp += cash_flow_pv;
			}

		/* Store the information for all the columns, in a global deal pv calculation */
			for (i =0; i < grfnparam->node_dim; i++)
			{
				...
				{
					deal_cash_flows_pv[t][i] += path_cash_flows[t][i] / ( sam[t].numeraire / dfs[t] );
				}
			/* Resets the path_cash_flow to 0.0 */
				path_cash_flows [t][i] = 0.0;
			}
    	
		/* The zero-coupon price computed for this time step (sum on all paths) (without its forward) */	
			Z_sum[t] += 1.0 / sam[t].numeraire;
		
		}/* END for t loop on all time steps of current path */
		
	/* Increments the current sums of all cashflows and cash flow squares for all paths */
		exp_disc_pay += temp;
...
/* Computes the renormalisation factor for the price */
	...
	{
	/* The correction factor is (at each step) the zero-coupon bond price computed by full MC simulation  */
...
/* The Real price is the price obtained by Monte Carlo, renormalised step by step */
	sum_price=0.0;
...
/* Stores the premium in the Input/Output list */
	err = srt_f_IOstructsetpremium((SrtIOStruct *)iorequest,SRT_NO,sum_price,"");
	if (err) 
	{
		FREE_MC_CORE;
		return (err);
...
/* Computes and stores the standard deviation as well */
	exp_disc_pay_sqr *= 1.0 + DBL_EPSILON * grfnparam->num_MCarlo_paths; /* To prevent rounding errors */
	stdev = (sqrt(exp_disc_pay_sqr/grfnparam->num_MCarlo_paths
				- exp_disc_pay*exp_disc_pay/grfnparam->num_MCarlo_paths/grfnparam->num_MCarlo_paths))
			/ sqrt(grfnparam->num_MCarlo_paths);
	err = srt_f_IOstructsetstdev((SrtIOStruct *)iorequest,SRT_NO,
								stdev,"");
	if (err)
	{
		FREE_MC_CORE;
...
{
int numex; /*number of exercize dates, equal to the length of the appropriate aux*/

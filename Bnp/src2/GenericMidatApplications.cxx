...
Err GMA_Midat_allocate_params(int iNbShortVolShift, double *dShortVolShiftTime,
  ...
  if (dShortVolShift && dShortVolShiftTime) {
    ...
    memcpy(sParams->dShortVolShiftTime, dShortVolShiftTime,
           iNbShortVolShift * sizeof(double));
    memcpy(sParams->dShortVolShift, dShortVolShift,
           iNbShortVolShift * sizeof(double));
  }
...
void GMA_MIDAT_free_params(GMA_MIDATPARAMS sParams) {
  if (sParams) {
    if (sParams->dShortVolShift)
      free(sParams->dShortVolShift);
    if (sParams->dShortVolShiftTime)
      free(sParams->dShortVolShiftTime);
  }
}
...
Err GMA_MidatAutocal(/* Cash Flows description */
  ...
  double *dFras = NULL, *dLongLevel = NULL, *dShortLevel = NULL,
         *dExeTimes = NULL,

         *dLongForward = NULL, *dShortForward = NULL, *dLongOptions = NULL,
         *dShortOptions = NULL,

         *dATMLongForward = NULL, *dATMShortForward = NULL,
         *dATMLongOptions = NULL, *dATMShortOptions = NULL;

  int i, j, i0;
  double dDfStart, dDfEnd, dSpread, dCoverage;
  double dShortForwardi, dLongForwardi, dStrike, dShortVolatility,
      dLongVolatility, dCorrel;
  ...
  *dCall = 0.0;
  ...
  if (!dFras || !dExeTimes || !dShortLevel || !dLongLevel ||
      !dATMShortForward || !dATMLongForward || !dShortForward ||
      !dLongForward || !dATMShortOptions || !dATMLongOptions ||
      !dShortOptions || !dLongOptions) {
  ...
  for (i = 0; i < iNbFixedFunding; i++) {
    ...
    dShortForward[iIndexCoupon] -=
        (dFras[i] + dFundingMargin[i]) * dCoverage * dDfEnd;

    if (i < iNbFixedFunding - 1 &&
        lCouponPayDate[iIndexCoupon] < lFundingPayDate[i + 1] - 5) {
  ...
  *dIV = -dLongForward[0] * iPayRec * 1.0;
  ...
  for (i = i0; i < iNbFixedCoupon; i++) {
    ...
    if (sMidatParams->dShortVolShift && sMidatParams->dShortVolShiftTime) {
      ...
      constant_or_linear_interpolation_dlm(
          sMidatParams->dShortVolShiftTime, sMidatParams->dShortVolShift,
          sMidatParams->iNbShortVolShift, dExeTimes[i], &dVolShift);
    } else {
      dVolShift = 0.0;
    }

    if (sMidatParams->iCalibATM) {
      err = swp_f_vol(cVolCurve, lCouponStartDate[i], lCouponEndDate[i],
                      dShortForwardi, &dShortVolatility, &power);
      ...
      if (power > 0.5) {
        dATMShortOptions[i] =
            srt_f_optblksch(dShortForwardi, dShortForwardi, dShortVolatility,
                            dExeTimes[i], dShortLevel[i], sOptType, PREMIUM);

        if (sCalibParams->iShortIsCap == 0) {
          /* Convert into Normal Vol */
          err = srt_f_optimpvol(dATMShortOptions[i], dShortForwardi,
                                dShortForwardi, dExeTimes[i], dShortLevel[i],
                                sOptType, SRT_NORMAL, &dShortVolatility);
        ...
        dATMShortOptions[i] =
            srt_f_optblknrm(dShortForwardi, dShortForwardi, dShortVolatility,
                            dExeTimes[i], dShortLevel[i], sOptType, PREMIUM);
      }
    } else {
      ...
      err = swp_f_vol(cVolCurve, lCouponStartDate[i], lCouponEndDate[i],
                      dStrike, &dShortVolatility, &power);
      ...
      if (power > 0.5) {
        dShortOptions[i] =
            srt_f_optblksch(dShortForwardi, dStrike, dShortVolatility,
                            dExeTimes[i], dShortLevel[i], sOptType, PREMIUM);
      } else {
        dShortOptions[i] =
            srt_f_optblknrm(dShortForwardi, dStrike, dShortVolatility,
                            dExeTimes[i], dShortLevel[i], sOptType, PREMIUM);
      }
    }
    ...
    /* LONG */
    if (sMidatParams->iCalibATM) {
      err = swp_f_vol(cVolCurve, lCouponStartDate[i], lTheoEndDate,
                      dLongForwardi, &dLongVolatility, &power);
      if (err)
        goto FREE_RETURN;
      ...
      if (power > 0.5) {
        dATMLongOptions[i] =
            srt_f_optblksch(dLongForwardi, dLongForwardi, dLongVolatility,
                            dExeTimes[i], dLongLevel[i], sOptType, PREMIUM);

        if (sCalibParams->iShortIsCap == 0) {
          /* Convert into Normal Vol */
          err = srt_f_optimpvol(dATMLongOptions[i], dLongForwardi,
                                dLongForwardi, dExeTimes[i], dLongLevel[i],
                                sOptType, SRT_NORMAL, &dLongVolatility);
        ...
        dATMLongOptions[i] =
            srt_f_optblknrm(dLongForwardi, dLongForwardi, dLongVolatility,
                            dExeTimes[i], dLongLevel[i], sOptType, PREMIUM);
      }
    }
    ...
    if (sCalibParams->iShortIsCap == 0 && sMidatParams->iCalibATM) {
      if (i < iNbFixedCoupon - 1) {
        ...
        err = swp_f_vol(sMidatParams->cCorrelName, lCorrelStart, lCorrelEnd,
                        dCorrelStrike, &dCorrel, &power2);
        if (err)
          goto FREE_RETURN;
        ...
        dPartialVolatility =
            dLongWeight * dLongWeight * dLongVolatility * dLongVolatility +
            dShortWeight * dShortWeight * dShortVolatility * dShortVolatility -
            2.0 * dCorrel * dLongWeight * dShortWeight * dLongVolatility *
                dShortVolatility;

        if (dPartialVolatility < 1.0E-08) {
          err = serror("Correlation %.2f between %.2f y %.2f y and %.2f y %.2f "
                       "is too low",
                       dCorrel, dExeTimes[i],
                       (lCouponEndDate[i] - lCouponStartDate[i]) * DAYS_IN_YEAR,
                       dExeTimes[i],
                       (lTheoEndDate - lCouponStartDate[i]) * DAYS_IN_YEAR);
        }

        dPartialVolatility = sqrt(dPartialVolatility);

        dATMShortOptions[i] = srt_f_optblknrm(
            dATMLongForward[i + 1] / dLongLevel[i + 1],
            dATMLongForward[i + 1] / dLongLevel[i + 1], dPartialVolatility,
            dExeTimes[i], dLongLevel[i + 1], sOptType, PREMIUM);
      } else {
        dShortOptions[i] = 0.0;
      }
    }
    ...
    err = swp_f_vol(cVolCurve, lCouponStartDate[i], lTheoEndDate, dStrike,
                    &dLongVolatility, &power);
    if (err)
      goto FREE_RETURN;
    ...
    if (power > 0.5) {
      dLongOptions[i] =
          srt_f_optblksch(dLongForwardi, dStrike, dLongVolatility, dExeTimes[i],
                          dLongLevel[i], sOptType, PREMIUM);
    } else {
      dLongOptions[i] =
          srt_f_optblknrm(dLongForwardi, dStrike, dLongVolatility, dExeTimes[i],
                          dLongLevel[i], sOptType, PREMIUM);
    }
  }
  ...
  if (sMidatParams->iCalibATM) {
    ...
    err = genmidat_init_model(sModel->dAlpha, sModel->dGamma, sModel->dRho,
                              &(dExeTimes[i0]), &(dATMLongForward[i0]), NULL,
                              NULL, NULL, NULL, sModel->dStartBeta2,
    ...
    err = generic_midat_calib(iNbExercise, &(dATMLongOptions[i0]),
                              &(dATMShortOptions[i0]), sModel, sCalibParams,
                              NULL);
  ...
  /* Initialialise the Model */
  err = genmidat_init_model(sModel->dAlpha, sModel->dGamma, sModel->dRho,
                            &(dExeTimes[i0]), &(dLongForward[i0]), NULL, NULL,
                            NULL, NULL, sModel->dStartBeta2, sModel->dNumeraire,
                            sModel);

  if (err)
    goto FREE_RETURN;

  err = GenericMidatAutocal(iNbExercise, &(dLongOptions[i0]),
                            &(dShortOptions[i0]), NULL, sModel, sAutocalParams,
                            sCalibParams, sPDEParams, dCall, sInfos);
...
FREE_RETURN:
  ...
  if (dLongLevel)
    free(dLongLevel);
  if (dATMShortForward)
    free(dATMShortForward);
  if (dATMLongForward)
    free(dATMLongForward);
  if (dShortForward)
    free(dShortForward);
  if (dLongForward)
    free(dLongForward);
  ...
  if (dATMShortOptions)
    free(dATMShortOptions);
  if (dATMLongOptions)
    free(dATMLongOptions);
  if (dShortOptions)
    free(dShortOptions);
  if (dLongOptions)
    free(dLongOptions);
...
Err GMA_MidatAutocalNew(/* Cash Flows description */
  ...
  double *dFras = NULL, *dExoticPV = NULL, *dFundingPV = NULL,
         *dFlatFundingPV = NULL;

  double *dExeTimes = NULL, *dFwdIV = NULL, *dATMFwdIV = NULL,
         *dLongLevel = NULL, *dLongForward = NULL, *dATMShortOptions = NULL,
         *dATMLongOptions = NULL, *dATMLongVol = NULL, *dLongOptions = NULL;

  int iFundStartIdx, iCouponStartIdx, iExeStartIdx;
  int iFundIdx, iCouponIdx;
  double dMarketIV, dDfStart, dDfEnd, dSpread;
  double dLongForwardi, dLongLeveli, dFlatLongForward, dLongStrike,
      dLongATMVoli, dLongStrikeVol;
  ...
  *dCall = 0.0;
  ...
  if (!dFras || !dExoticPV || !dFundingPV || !dFlatFundingPV || !dExeTimes ||
      !dFwdIV || !dATMFwdIV || !dLongLevel || !dLongForward ||
  ...
  while (iFundStartIdx < iNbFunding &&
         lFundingFixDate[iFundStartIdx] < lToday + iEODFixFlag) {
    if (lFundingPayDate[iFundStartIdx] < lToday + iEODPayFlag) {
      ...
      dFundingPV[iFundStartIdx] =
          (dPastFixings[iFundStartIdx] + dFundingMargin[iFundStartIdx]) *
          dFundingCoverage[iFundStartIdx] * dDfEnd;
    }
  ...
  for (i = iFundStartIdx; i < iNbFunding; i++) {
    ...
    dFundingPV[i] =
        dFlatFundingPV[i] + dFundingMargin[i] * dFundingCoverage[i] * dDfEnd;
  ...
  while (iCouponStartIdx < iNbCoupon &&
         lCouponPayDate[iCouponStartIdx] < lToday + iEODPayFlag) {
  ...
  *dIV = dPayRec * dMarketIV;
  ...
  while (iExeStartIdx < iNbExercise &&
         lExerciseDate[iExeStartIdx] < lToday + iEODExeFlag) {
    iExeStartIdx++;
  }
  ...
  for (i = iExeStartIdx; i < iNbExercise; i++) {
    ...
    while (iFundIdx < iNbFunding &&
           lFundingStartDate[iFundIdx] < lExerciseDate[i]) {
      iFundIdx++;
    }
    ...
    while (iCouponIdx < iNbCoupon &&
           lCouponStartDate[iCouponIdx] < lExerciseDate[i]) {
      iCouponIdx++;
    }
    ...
    if (iCouponIdx > 0) {
      dLongLeveli += (lCouponPayDate[iCouponIdx - 1] - lSettlementDate[i]) /
                     (1.0 * (lCouponPayDate[iCouponIdx - 1] -
                             lCouponStartDate[iCouponIdx - 1])) *
                     dExoticPV[iCouponIdx - 1] / dCoupon[iCouponIdx - 1];
      dLongForwardi += (lCouponPayDate[iCouponIdx - 1] - lSettlementDate[i]) /
                       (1.0 * (lCouponPayDate[iCouponIdx - 1] -
                               lCouponStartDate[iCouponIdx - 1])) *
                       dExoticPV[iCouponIdx - 1];
    }
    ...
    /* Add the Fee */
    dLongForwardi =
        dLongForwardi * dPayRec -
        lExerciseFee[i] * swp_f_df(lToday, lSettlementDate[i], cYieldCurve);
    ...
    err = swp_f_vol(cVolCurve, lSettlementDate[i], lTheoEndDate,
                    dLongForward[i], &dLongATMVoli, &dPower);
    if (err)
      goto FREE_RETURN;

    err = swp_f_vol(cVolCurve, lSettlementDate[i], lTheoEndDate, dLongStrike,
                    &dLongStrikeVol, &dPower);
    if (err)
      goto FREE_RETURN;

    if (dPower > 0.5) {
      dLongOptions[i] =
          srt_f_optblksch(dLongForward[i], dLongStrike, dLongStrikeVol,
                          dExeTimes[i], dLongLevel[i], sOptType, PREMIUM);

      dATMLongOptions[i] =
          srt_f_optblksch(dLongForward[i], dLongForward[i], dLongATMVoli,
                          dExeTimes[i], dLongLevel[i], sOptType, PREMIUM);

      /* Convert into Normal Vol */
      err = srt_f_optimpvol(dATMLongOptions[i], dLongForward[i],
                            dLongForward[i], dExeTimes[i], dLongLevel[i],
                            sOptType, SRT_NORMAL, &dLongATMVoli);
    } else {
      dLongOptions[i] =
          srt_f_optblknrm(dLongForward[i], dLongStrike, dLongStrikeVol,
                          dExeTimes[i], dLongLevel[i], sOptType, PREMIUM);

      dATMLongOptions[i] =
          srt_f_optblknrm(dLongForward[i], dLongForward[i], dLongATMVoli,
                          dExeTimes[i], dLongLevel[i], sOptType, PREMIUM);
    }
    ...
    if (i > iExeStartIdx) {
      ...
      dShortForwardi = (dLongLevel[i - 1] * dLongForward[i - 1] -
                        dLongLevel[i] * dLongForward[i]) /
                       dShortLeveli;

      err = swp_f_vol(cVolCurve, lSettlementDate[i - 1], lSettlementDate[i],
                      dShortForwardi, &dShortATMVoli, &dPower);
      if (err)
        goto FREE_RETURN;

      if (dPower > 0.5) {
        dATMShortOptions[i - 1] =
            srt_f_optblksch(dShortForwardi, dShortForwardi, dShortATMVoli,
                            dExeTimes[i - 1], dShortLeveli, sOptType, PREMIUM);

        /* Convert into Normal Vol */
        err = srt_f_optimpvol(dATMShortOptions[i - 1], dShortForwardi,
                              dShortForwardi, dExeTimes[i - 1], dShortLeveli,
                              sOptType, SRT_NORMAL, &dShortATMVoli);
      } else if (sCalibParams->iShortIsCap) {
        dATMShortOptions[i - 1] =
            srt_f_optblknrm(dShortForwardi, dShortForwardi, dShortATMVoli,
                            dExeTimes[i - 1], dShortLeveli, sOptType, PREMIUM);
      }
      ...
      if (!sCalibParams->iShortIsCap) {
        ...
        dPartialVolatility =
            dLongWeight * dLongWeight * dATMLongVol[i - 1] *
                dATMLongVol[i - 1] +
            dShortWeight * dShortWeight * dShortATMVoli * dShortATMVoli -
            2.0 * dCorrel * dLongWeight * dShortWeight * dATMLongVol[i - 1] *
                dShortATMVoli;

        if (dPartialVolatility < 1.0E-08) {
          err = serror("Correlation %.2f between %.2f y %.2f y and %.2f y %.2f "
                       "is too low",
                       dCorrel, dExeTimes[i - 1],
                       (lSettlementDate[i] - lSettlementDate[i - 1]) *
                           DAYS_IN_YEAR,
                       dExeTimes[i - 1],
                       (lTheoEndDate - lSettlementDate[i - 1]) * DAYS_IN_YEAR);
        }

        dPartialVolatility = sqrt(dPartialVolatility);

        dATMShortOptions[i - 1] = srt_f_optblknrm(
            dLongForward[i], dLongForward[i], dPartialVolatility,
            dExeTimes[i - 1], dLongLevel[i], sOptType, PREMIUM);
      }
    }
  }
  ...
  if (sMidatParams->iCalibATM) {
    ...
    err = genmidat_init_model(
        sModel->dAlpha, sModel->dGamma, sModel->dRho,
        &(dExeTimes[iExeStartIdx]), &(dATMFwdIV[iExeStartIdx]), NULL, NULL,
        NULL, NULL, sModel->dStartBeta2, sModel->dNumeraire, sModel);

    if (err)
      goto FREE_RETURN;

    /* First ATM Calibration */
    err = generic_midat_calib(
        iNbExercise - iExeStartIdx, &(dATMLongOptions[iExeStartIdx]),
        &(dATMShortOptions[iExeStartIdx]), sModel, sCalibParams, NULL);
  ...
  /* Initialialise the Model */
  err = genmidat_init_model(sModel->dAlpha, sModel->dGamma, sModel->dRho,
                            &(dExeTimes[iExeStartIdx]), &(dFwdIV[iExeStartIdx]),
                            NULL, NULL, NULL, NULL, sModel->dStartBeta2,
                            sModel->dNumeraire, sModel);

  if (err)
    goto FREE_RETURN;

  err = GenericMidatAutocal(
      iNbExercise - iExeStartIdx, &(dLongOptions[iExeStartIdx]), NULL, NULL,
      sModel, sAutocalParams, sCalibParams, sPDEParams, dCall, sInfos);

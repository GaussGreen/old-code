/*--------------------------------------------------------------
        FILE: srt_f_cheybeta_diagcalib.cxx
...
Err cheybeta_diagcalib_new(
    ...
    double vol_shift, int shift_type,       /*	0:	Additive
                                                      1:	Multiplicative */
    int nex, long *ex_dates,                /*	Exercise dates */
    long end_date,                          /*	End date for diagonal */
    char **tenors,                          /*	Instruments' tenors */
    char *freq, char *basis, char *refrate, /*	Name of the reference rate */
    double *strikes, int strike_type,       /*	0: ATM
                                                        1: CASH
                                                        2: SWAP
                                                        3: STD */
    double max_std, int nt,                 /*  Parameters of the		*/
  ...
  /* ____For preliminary LGM calibration (interface with
   * cpd_calib_diagonal_2)____ */
  ...
  for (i = 0; i < nex; i++) {
    ...
    /* Strike */
    if (!strikes || strike_type == 0)
      strike = fwd;
    else if (strike_type == 2)
      strike = strikes[i];
    else if (strike_type == 3)
      strike = fwd + strikes[i] * std;
    else {
      err = serror("Unknown strike type");
      goto FREE_RETURN;
    }

    /* Apply max std */
    if (strike > fwd + max_std * std)
      strike = fwd + max_std * std;
    if (strike < fwd - max_std * std)
      strike = fwd - max_std * std;
    ...
    /* Market price */
    mkt_prices[i] = (power > 0.5 ? srt_f_optblksch(fwd, strike, std, g.ex[i],
                                                   lvl, SRT_PUT, PREMIUM)
                                 : srt_f_optblknrm(fwd, strike, std, g.ex[i],
                                                   lvl, SRT_PUT, PREMIUM));

    /* Cash flows */
    err = SwapElements_Init(start, end, freq, basis, refrate, "REC", strike,
                            elements, pmdl->today, &(g.mat[i]), &(g.cxxpn[i]),
                            &(g.nmat[i]));
    if (err)
      goto FREE_RETURN;
  ...
  /* _______Calibrate LGM for later use as a first
   * guess_______________________________________ */
  ...
  if (tenors)
    for (i = 0; i < nex; i++)
      strcpy(tenors_LGM[i], tenors[i]);
  else if (end_date != 0)
    for (i = 0; i < nex; i++)
      strcpy(tenors_LGM[i], "DIAG");
  else
    for (i = 0; i < nex; i++)
      sprintf(tenors_LGM[i], "%dM", 12 / reffreq);

  if (strikes)
    memcpy(strikes_LGM, strikes, nex * sizeof(double));
  else
    memset(strikes_LGM, 0, nex * sizeof(double));

  for (i = 0; i < nex; i++)
    arr_of_1[i] = 1;

  err = cpd_calib_diagonal_2(
      pmdl->ycname, vc_name, refrate, get_cash_vol, freq, basis, nex, ex_dates,
      arr_of_1, tenors_LGM, end_date, strikes_LGM, pmdl->lambda, 1, 0.0, 0.0,
      0.0, &nsig_LGM, &sigtms_LGM, &sig_LGM, &param_LGM, NULL);
  if (err)
    goto FREE_RETURN;

  if (nsig_LGM != nex) {
    err = serror("Wrong nsig after LGM calibration");
    goto FREE_RETURN;
  }

  for (i = 0; i < nex; i++) {
    if (fabs(pmdl->sigtms[i] - sigtms_LGM[i]) > 1e-5) {
      err = serror("Wrong dates in LGM calibration");
      goto FREE_RETURN;
    }
...
FREE_RETURN:
  ...
  if (tenors_LGM)
    free_svector_size(tenors_LGM, 0, nex - 1, 20);
  free(strikes_LGM);
  free(arr_of_1);
  free(sigtms_LGM);
  free(sig_LGM);

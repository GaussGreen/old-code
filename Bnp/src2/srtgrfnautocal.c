...
Err GrfnAutocalCaller(
  ...
  *prod_val = NULL;
  ...
  /*	Calibrate all */
  err = cpd_calib_all(
      today, get_cash_vol, dom_yc_name, dom_vol_curve_name, dom_ref_rate_name,
      dom_instr_freq, dom_instr_basis, dom_lambda, for_yc_name,
      for_vol_curve_name, for_ref_rate_name, for_instr_freq, for_instr_basis,
      for_lambda, fx_mkt_vol_date, fx_mkt_vol, num_fx_mkt_vol, corr_times,
      correl_dom_for, correl_dom_fx, correl_for_fx, corr_n_times, use_ndates,
      use_dates, use_cal, dom_end_tenor, for_end_tenor, end_date, dom_strike,
      for_strike, num_sig, sig_time, dom_sig, for_sig, num_fx_vol, fx_vol_time,
      fx_vol, calib_param, dom_inst_data, for_inst_data);
  if (err) {
    goto FREE_RETURN;
  }
  free_inst_data = 1;
  ...
  err = cpd_calib_all_makeund(
      today, dom_ccy, dom_und_name, dom_yc_name, *dom_sig, dom_lambda, for_ccy,
      for_und_name, for_yc_name, *for_sig, for_lambda, corr_times,
      correl_dom_for, correl_dom_fx, correl_for_fx, corr_n_times, *num_sig,
      *sig_time, *num_fx_vol, spot_fx, *fx_vol_time, *fx_vol, fx_und_name);
  if (err) {
    goto FREE_RETURN;
  }
  free_und = 1;
  ...
  if (mth == 0) {
    if (quantoflag == 1) /*	Quanto Product	*/
    {
      ...
      err = SrtGrfnQuantoPDEWithCorrelTS2(
          fx_und_name, num_ex_dates, ex_date, tableauRows, tableauCols,
          tableauStrings, tableauMask, auxWidth, auxLen, aux,
          pricing_param->num_stp, pricing_param->num_stpx, num_prod, prod_val);
    ...
    else /*	Hybrid Product	*/
    {
      ...
      /*	Tree */
      err = SrtGrfn3DFXTree(fx_und_name, num_ex_dates, ex_date, tableauRows,
                            tableauCols, tableauStrings, tableauMask, auxWidth,
                            auxLen, aux, is_end_of_day_fixing,
                            is_end_of_day_payment, barrier, bar_col,
                            pricing_param->num_stp, num_prod, 1, prod_val);
      free(barrier);
      free(bar_col);
      if (err) {
        goto FREE_RETURN;
    ...
    /*	MC */
    err = SrtGrfn3DFXMc(fx_und_name, num_ex_dates, ex_date, tableauRows,
                        &tableauCols, tableauStrings, tableauMask, auxWidth,
                        auxLen, aux, is_end_of_day_fixing,
                        is_end_of_day_payment, pricing_param->num_paths,
                        pricing_param->do_pecs, &temp_prod_val);
    if (err) {
      goto FREE_RETURN;
    }

...
/*
        TargetNoteUtil.cxx
        Routines:
                TARN_calcHistory:  from the historical fixings        ,
   calculate what has happened.
*/
...
char *TARN_calcHistoryOld(TARN_Struct *tarn, TARN_AUX *aux) {
   ...
   */
  /* then the cash flows are deterministic        , so it is as if we have
   * knocked out
   */
  ...
  /* If the swap has knocked out        , see if there are any remaining cash
   * flows and calculate their PV */
  if (tarn->output.iIsKnockedOut) {
    /* Calculate the coupon PV */
    if (tarn->deal.cxxouponType == TN_SWAP) {
    ...
    if (tarn->deal.cxxouponType == TN_SWAP) {
      ...
      //			aux->dHistCpnPV += ( tarn->deal.dTarget -
      // tarn->output.dCumulCoupon ) * aux->dvCouponPayDF[aux->i1stCpn];
    } else if (tarn->deal.cxxouponType == TN_ZC) {
      aux->dHistCpnPV = tarn->deal.dTarget * aux->dvCouponPayDF[aux->i1stCpn];
    } else if (tarn->deal.cxxouponType == TN_FINAL) {
      if (tarn->deal.lvCpnPayDates[aux->i1stCpn] >= lFirstPayment)
        aux->dHistCpnPV = dCoupon * aux->dvCouponCvg[aux->i1stCpn] *
                          aux->dvCouponPayDF[aux->i1stCpn];
  ...
  /* if the swap has not knocked out        , calculate any residual coupon cash
   * flows
   */
  if (tarn->deal.cxxouponType == TN_SWAP) {
    if ((aux->i1stCpn > 0) &&
  ...
  /* if the swap has not knocked out        , calculate any residual funding PV
   * up to the first fixing date */
...
char *TARN_calcHistory_SWAP(TARN_Struct *tarn, TARN_AUX *aux) {
  ...
  /* if there is only 1 coupon remaining and the coupon is guaranteed        ,
   * then the cash flows are deterministic        , */

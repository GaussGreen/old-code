...
{\
	if (peOptionType) srt_free(peOptionType);\
	peOptionType = NULL;\
	if (peRecPay) srt_free(peRecPay);\
	peRecPay = NULL;\
	if (psSwapDp) srt_free(psSwapDp);\
	psSwapDp = NULL;\
	if (plDealSigmaDates) free_dvector(plDealSigmaDates,0,*plNumSigmas-1);\
	plDealSigmaDates = NULL;\
	if (*pppdSigmaCurve) free_dmatrix(*pppdSigmaCurve, 0, *plNumSigmaCols-1, 0, *plNumSigmas-1);\
	*pppdSigmaCurve = NULL;\
	if (plDealTauDates) free_dvector(plDealTauDates,0,*plNumTaus-1);\
	plDealTauDates = NULL;\
	if (*pppdTauCurve) free_dmatrix(*pppdTauCurve, 0, *plNumTauCols-1, 0, *plNumTaus-1);\
	*pppdTauCurve = NULL;\
	if (pdFraMaturities) free_vector(pdFraMaturities, 0, lNumTenors-1);\
...
#ifdef PVMPI
	if (!PRL_CONTEXT.bLocalComp) RequestChangeUndName(the_und);
#endif
...
{\
	if (peOptionTypeFree) srt_free(peOptionTypeFree);\
	peOptionTypeFree = NULL;\
	if (peRecPayFree) srt_free(peRecPayFree);\
	peRecPayFree = NULL;\
	if (psSwapDpFree) srt_free(psSwapDpFree);\
	psSwapDpFree = NULL;\
	if (plDealSigmaDates) free_dvector(plDealSigmaDates,0,*plNumSigmas-1);\
	plDealSigmaDates = NULL;\
	if (*pppdSigmaCurve) free_dmatrix(*pppdSigmaCurve, 0, *plNumSigmaCols-1, 0, *plNumSigmas-1);\
...
/* Check the Term Structure depending on the model dimension */
	...
	if (eModelDim == TWO_FAC)
	{
		...
		{
			return serror("Need a FULL Two Factor Sigma TS for Calibration: Date - Sig1 - Bet1 - Sig2 - Bet2 - Rho");
		}
	}
...
/* New flag for the calib */
	if (calib_param.bAggregate == SRT_YES) sGrfnParams.calib = SRT_YES;
...
/* Sets an initial guess only for instruments with a fixing date after today */
	...
	for (i=0; i< lNumInstruments; i++)
	{
	   ...
	   /* Extracts the relevant fixing date for the deal */
		...
		if ((peOptionType[iNumUsedInstruments] == SWAPTION) || (peOptionType[iNumUsedInstruments] == BOND_OPTION)
...
///// added by Albert Wang 08/25/03 - end 
		...
		/* The Sigma date is the fixng date of the swaption */
			lLastFixing = add_unit(psSwapDp[iNumUsedInstruments].start, - Spot_Lag, SRT_BDAY, MODIFIED_SUCCEEDING);
		}
   /* We only keep track of the instruments with a fixing date before or equal to today */  
		if (lLastFixing > today)
		{

		/* In the case of the full Cap and if the first fixing is before today we modify the start date and the relevant swapDP */
			if (peOptionType[iNumUsedInstruments] == CAPFLOOR)
			{
				int indexcap = 0;
				...
				err = swp_f_initSwapDP(	first_period_start,
				...
				err = swp_f_Level_SwapDP(&(psSwapDp[iNumUsedInstruments]),szYieldCurveName,&dLevel);
...
///// added by Albert Wang 08/25/03 - end 				
			...
			/*gets the ATM Vol */
				err = pfGetVol((double) DTOL(date_list.date[0]),(double) DTOL(date_list.date[date_list.len-1])
			...
			/* Computes the Distance betwenn Forward and Strike in Num Std Dev */
				...
				{
					dDistance = fabs(dForwardRate - pdStrike[iNumUsedInstruments]) 
						/ dStdDev;
				}
				else
				{
					dDistance = fabs( log(dForwardRate / pdStrike[iNumUsedInstruments]))
						/ dStdDev;
				}
			}

			if (date_list.date) srt_free(date_list.date);
		...
		/* If the Strike is more than MAX_STDEV away from the Forward, discard the instrument */
			if ( dDistance > CALIBRATE_MAX_STDEV )
			{
				...
				/* For the output purpose we need to keep track of the details of the instruments in input */
	...
	/* If only one TAU required, sets a default date */
		...
		else if (calib_param.bFreezeTau == SRT_YES)
		{
			if (eModelDim == ONE_FAC)
			{
				...
				if (plTemSigmaDates) free(plTemSigmaDates);
				if (pdTempSigma) free(pdTempSigma);
				if (pdTempBeta) free(pdTempBeta);
				if (pdTempVovol) free(pdTempVovol);
				if (pdTempRho) free(pdTempRho);
				if (pdTempMeanVol) free(pdTempMeanVol);
				if (pdTempTau) free(pdTempTau);
	...
	/* Allocate *pppdSigmaCurve and sets dates and values for sig from TS */
		...
		/* Populates the Full Sigma Curve according to the Initial TS */
			...
			/* Set values for Sigma */
				(*pppdSigmaCurve)[1][i] = find_sig(
						(plDealSigmaDates[i]-today) * YEARS_IN_DAY,
						ts);
	
			/* Set values for Beta  */
				(*pppdSigmaCurve)[2][i] = find_beta (
						(plDealSigmaDates[i]-today) * YEARS_IN_DAY,
						ts);

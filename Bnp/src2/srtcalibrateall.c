...
/* ------------------------------------------------------------------------
        ...
        PURPOSE: calibrate model acoording to user's inputs:
                ...
                if FixedCalibration:
                        the correlation structure (alpha  , beta  , rho) is
 frozen optimisation is made only on sig1(t)  , lam1(t) the criteria is just
 based on market prices USAGE: to get to a : Global Calibration: DEFAULT
...
Err SrtNewCalibrateAll(
  ...
  if (is_model_Cheyette_type(eModelType) == SRT_NO) {
    return serror(
        "Can only calibrate LGM  , CHEY  , CHEYBETA (with STOCHVOL) so far");
  }
  ...
  for (i = 0; i < lNumInstruments; i++) {
    ...
    if (lLastFixing > today) {
      ...
      /* If the Strike is more than MAX_STDEV away from the Forward  , discard
       * the instrument */

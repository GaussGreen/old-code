    ...
    String szUndName)
{
    ...
    int iNumSigmaRows, iNumSigmaCols, iNumTauRows, iNumTauCols, iNumInstruments, *pDerivType, i;

    double **ppdSigmaCurve, **ppdTauCurve, *pdStrike, *pdBondStrike, *pdOptionPrice,
        *pdATMOptionPrice, forward, dForwardSpread, dCurrentVol;
    ...
    for (i = 0; i < iNumInstruments; i++)
    {
        ...
        dForwardSpread = swp_f_spread(plStartDates[i], plEndDates[i], pszRefRate[i]);

        err = pfGetVol(
            plStartDates[i], plEndDates[i], forward, forward, dForwardSpread, &dCurrentVol);
        ...
        err = pfGetVol(
            plStartDates[i], plEndDates[i], pdStrike[i], forward, dForwardSpread, &dCurrentVol);
    ...
    for (i = 0; i < iNumInstruments; i++)
    {
        if (err = swp_f_initSwapDP(
                plStartDates[i], plEndDates[i], pszFrequency[i], pszBasis[i], &sdpArray[i]))
            return err;

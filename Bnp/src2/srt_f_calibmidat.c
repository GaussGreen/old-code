...
double				**ppdSigmaCurve,
	...
	for (i=0;i <iNumInstruments;i++)
	{
		...
		if (err =  swp_f_ForwardRate(plStartDates[i], 
									plEndDates[i],
									pszFrequency[i],
									pszBasis[i],
									szYieldCurveName,
		...
		dForwardSpread = swp_f_spread(plStartDates[i], 
									  plEndDates[i], 
									  pszRefRate[i]);
		
		err = pfGetVol(plStartDates[i], 
					   plEndDates[i], 
					   forward, 
					   forward, 
					   dForwardSpread, 
		...
		err = swp_f_Swaption(plStartDates[i], 
							 plEndDates[i],
							 pszFrequency[i],
							 pszBasis[i],
							 dCurrentVol,
	...
	for (i=0; i<iNumInstruments; i++)
	{
		if (err =  swp_f_initSwapDP(plStartDates[i],
									plEndDates[i],
									pszFrequency[i],
									pszBasis[i],
									&sdpArray[i]))
	...
	err = srt_f_bootstrap_calibrate(Underlying,
									&sGrfnParams,
									sdpArray,
									pdStrike,
									pdBondStrike,

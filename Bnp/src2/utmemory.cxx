...
/* -----------------------------------------------------------------
  DESCRIPTION     :Numerical Recipes in 'C' allocation routines        ,
modified for SORT purposes in the following ways: (1) we never use floats. (2)
lngvector (vector of longs) added (3) calloc and free replaced with srt_calloc
and srt_free (4) nrerror no longer calls exit() since this could kill something
important. rather        , these functions all return NULL if they fail.  This
is done by redefining nrerror as a call to a function nrerror_print that prints
a message and returns NULL. Any function calling these functions should make
appropriate error checks. (5) static arguments to numerical recipes macros
...
/* KNL: ansi-friendly version added for grf_f_mid_conv_2020.cxx 	    */
/* allocate a int matrix with subscript range m[nrl..nrh][ncl..nch] */
{
  long i, nrow = nrh - nrl + 1, ncol = nch - ncl + 1;
  long **m;
...
/* allocate a double 3tensor with range t[nrl..nrh][ncl..nch][ndl..ndh] */
{
  ...
  /* allocate rows and set pointers to them */
  t[nrl][ncl] =
      (double *)srt_calloc(nrow * ncol * ndep + NR_END, sizeof(double));
  if (!t[nrl][ncl]) {
...
/* Allocates f4tensor */
double ****f4tensor(long m_min, long m_max, long n_min, long n_max, long o_min,
                    long o_max, long p_min, long p_max)
/* allocate a double 4tensor with range
   t[m_min..m_max][n_min..n_max][o_min..o_max][p_min..p_max] */
{
  ...
  t[m_min][n_min][o_min] =
      (double *)srt_calloc(m * n * o * p + NR_END, sizeof(double));
...
void free_smatrix_size(char ***m, long nrl, long nrh, long ncl, long nch,
                       long sz)
...
void free_l3tensor(long ***t, long nrl, long nrh, long ncl, long nch, long ndl,
                   long ndh)
/* free a double f3tensor allocated by f3tensor() */
{
  free((FREE_ARG)(t[nrl][ncl] + ndl - NR_END));
...
/* Frees f4tensor */
void free_f4tensor(double ****t, long m_min, long m_max, long n_min, long n_max,
                   long o_min, long o_max, long p_min, long p_max) {

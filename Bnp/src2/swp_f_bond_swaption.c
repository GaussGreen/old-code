    ...
    double* hedge_ratio)
{
    ...
    *swap_maturity = maturity_date;
    while (stop_flag != SRT_YES)
    {
        ...
        err =
            swp_f_initSwapDP(settle_swap_date, *swap_maturity, strSwapComp, strSwapBasis, &swap_dp);
        if (err)
            return err;
    ...
    swaption_value  = settle_swap_df * srt_f_optblksch(
                                          fwd_clean_price / clean_strike,
                                          1.0,
                                          swap_price_vol,
                                          option_maturity,
                                          1.0,

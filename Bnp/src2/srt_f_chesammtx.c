...
/*-----------------------------------------------------------------------
  ...
  DESCRIPTION		:determine max and min phi for a given sample
                                         tminf and trinf should correspond to stp
                                         does both "trapezoidal" and "rectangular" sampling in phi
  MODIFIES			:sam->phi is set to the phi centered on
                                         *phimin and *phimax are also set if they are not NULL
---------------------------------------------------------------------------*/
...

{
    ...
    /* modified AS */
    max_ri = IMAX(trinf->max_r_index, ((SrtCheTreInf*)(stp->prev->trinf))->max_r_index + 1);
    min_ri = IMIN(trinf->min_r_index, ((SrtCheTreInf*)(stp->prev->trinf))->min_r_index - 1);
    ...
    for (i = min_ri; i <= max_ri; i++)
    {
        ...
        {
            loguphi = log(phimax / phimin) / (trinf->max_phi_index - trinf->min_phi_index);
        }

        for (j = trinf->min_phi_index, logphi = j * loguphi; j <= trinf->max_phi_index;
             j++, logphi += loguphi)
        {

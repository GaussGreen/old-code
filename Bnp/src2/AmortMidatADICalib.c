/*-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+--+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-
        ...
        Description	: implementation of the calibration of Callable Amortizing/Accreting
Swaption (CAS).
...
void _free_Calib_LamTS_DiagEuro_Comm(_Calib_LamTS_DiagEuro_Comm* pCalib_LamTS_DiagEuro)
{
...
static int _initialize_ptr_TS_(const double* pdIn_Begin, const double* pdIn_End, double dKey)
{
    const double* pd =
        find_min_greater_than(&dKey, pdIn_Begin, pdIn_End - pdIn_Begin, sizeof(dKey), dless);
    ...
    double* pdG_Begin)
{
    ...
    // delegate to nag ...
    nag_opt_lsq_no_deriv(m, n, _lsqfunc_, pdTS, &dfsumsq, pdf, pdjac, n, E04_DEFAULT, &comm, &fail);
...
static double _Price_GenEuroBlock(
    const _GenEuropean* pGenEuroBlock_Begin, const _GenEuropean* pGenEuroBlock_End)
{
    ...
    double** ppdSig)
{
    ...
    szErr = cpd_calib_diagonal_dlm(
        ...
        (double*)_memset(
            &dZero,
            _alloca(nSize_Lam * sizeof(double)),
            nSize_Lam,
...
const char* Calibrate_MidAt(
    ...
    const double* pdSpread_Begin,
    // term structure
    int nCalibTauToCap,  // 2 - calibrate lambda ATM, then prim ATS:  1 - calibrate Tau to cap, 0
                         // otherwise
    const double* pdAlpha,
    const double* pdGamma,
    const double* pdRho,
    ...
    double* pdDiagPrice)
{
    ...
    if (nEuroModel != 1)
        return "Only SMM model currently available for pricing amortizing/accreting european "
               "swaptions!";
    ...
    if ((pdLamT_End - pdLamT_Begin) == 1)
    {
        szErr = amortMidat_cpd_calib_diagonal_new(
            ...
            memcpy(
                _alloca(sizeof(long) * nSize_Fix),
                plFixPay_Begin,
                sizeof(long) * nSize_Fix),  //(long*)plFixPay_Begin,
            memcpy(
                _alloca(sizeof(double) * nSize_Fix),
                pdFixRate_Begin,
            ...
            memcpy(
                _alloca(sizeof(long) * nSize_Flt),
                plFltPay_Begin,
                sizeof(long) * nSize_Flt),  //(long*)plFltPay_Begin,
            memcpy(
                _alloca(sizeof(double) * nSize_Flt),
                pdMargin_Begin,
            ...
            dMaxVarJump,
            memcpy(
                _alloca((pdLamT_End - pdLamT_Begin) * sizeof(double)),
                pdLam_Begin,
                (pdLamT_End - pdLamT_Begin) * sizeof(double)),
    ...
    double* pPrice)
{
    ...
    const _Model MyModel = {
        ...
        MyModel.m_dAlpha *
            sqrt(1 - MyModel.m_dRho * MyModel.m_dRho),  // const double m_dAlpha_Sqrt1mRhoSqrd;
        MyModel.m_dGamma * MyModel.m_dRho /
            sqrt(
                1. -
                MyModel.m_dRho * MyModel.m_dRho),  // const double m_dGammaRho_Over_Sqrt1mRhoSqrd;
        0};
    ...
    *pPrice = 0.;
    if (szErr =
            _GenMidAt_OptionPV_Core(nNumPointT, nNumPointX, pOption, &MyModel, pNumeraire, pPrice))
    {
        // free(pdSig_Begin);
        // free(pdSigT_Begin);
        return szErr;
...
_OBJFUN_LAMTS_BOUNDS(long n, double* pdx, double* pdf, double* pdg, Nag_Comm* pNagComm)
{
    ...
    *pdf = 0.;
    ...
    for (; pOption_Begin < pOption_End;
         ++pOption_Begin, ++pOptionPrice_Begin, ++pNumeraire, ++pCalib_LamTS_DiagEuro_Begin, ++pdf_)
    {
...
)
{
    ...
    /// options
    _GenMidAt *pOption_Begin = _alloca(m * sizeof(_GenMidAt)), *pOption_End = pOption_Begin + m,
              *pOption = pOption_Begin;
    ...
    for (; plEx_Begin < plEx_End; ++plEx_Begin,
                                  ++plExStart_Begin,
                                  ++plExFee_Begin,
                                  ++pdCoupon_Begin,
                                  ++pdExFee_Begin,
                                  ...
                                  ++pCalib_LamTS_DiagEuro)
    {
        ...
        if (szErr = _alloc_Option(plEx_End - plEx_Begin, nSize_Fix_nI, nSize_Flt_nI, pOption))
            return szErr;
        ...
        _preinit_Option(
            ...
            (const double*)_memset(
                pdCoupon_Begin,
                _alloca(sizeof(double) * nSize_Fix_nI),
                nSize_Fix_nI,
                sizeof(double)),  // pdFixNotional_Begin+nOffSet_Fix,

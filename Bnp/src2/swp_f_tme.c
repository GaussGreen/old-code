...
Err swp_f_tme(
			...
			double  *dStrikes,
			int     bAdjForSpread,
			Err		(*GetVol)(double dStart, double dEnd, double dStrike, SRT_Boolean bAdjForSpread, 
			...
			double      *dAnswer)
{
	...
	if (err = swp_f_Cms_Rate(dFwdTEC, dMaturity, dNumperiods, Compounding, 0.0 , 1.0, CMSlognorm,
							 0.0, 1 /* Full Smile Approx */, lFraSpot, lBond10YTheoricalEndDate,
							 (SRT_Boolean) bAdjForSpread, bAdjForSpread*dSpread, szBondVolCurveName, iNumStrikesInVol,
							 dStrikes, &dTECCms))

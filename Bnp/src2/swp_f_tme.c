    ...
    double* dAnswer)
{
    ...
    lBond12YTheoricalEndDate = add_unit(TMEDate, 12, SRT_YEAR, MODIFIED_SUCCEEDING);
    lEndDate                 = add_unit(lFraSpot, 10, SRT_YEAR, MODIFIED_SUCCEEDING);
    Compounding              = SRT_ANNUAL;
    Basis                    = BASIS_ACT_360;

    /* Initialise the swap and the two bonds*/
    err = swp_f_setSwapDP(lFraSpot, lSwapTheoricalEndDate, Compounding, Basis, &sdpSwap);
    if (err)
        return err;
    err = swp_f_setSwapDP(lFraSpot, lBond10YTheoricalEndDate, Compounding, Basis, &sdpBond10Y);
    if (err)
        return err;
    err = swp_f_setSwapDP(lFraSpot, lBond12YTheoricalEndDate, Compounding, Basis, &sdpBond12Y);
    if (err)
        return err;
    ...
    err     = swp_f_ForwardRate_SwapDP(&sdpSwap, szSwapYieldCurveName, cSwapRefRateCode, &dFwdSwap);
    dFwdTEC = dFwdSwap + dTECSpread;

    err = swp_f_ForwardRate_SwapDP(&sdpBond10Y, szBondYieldCurveName, cBondRefRateCode, &dBond10Y);

    err = swp_f_ForwardRate_SwapDP(&sdpBond12Y, szBondYieldCurveName, cBondRefRateCode, &dBond12Y);
    ...
    if (err = swp_f_Cms_Rate(
            ...
            1 /* Full Smile Approx */,
            lFraSpot,
            lBond10YTheoricalEndDate,
            (SRT_Boolean)bAdjForSpread,
            bAdjForSpread * dSpread,
            szBondVolCurveName,
            iNumStrikesInVol,
            dStrikes,
            &dTECCms))

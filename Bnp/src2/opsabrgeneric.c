...
//
//vol_bvm()				Specify your local vol of the form vol_mylocvol(double, double,double,double, int)
//
...
double	res;
	...
	if(T > 1.0e-14)
	{
		...
		res *= (1.0 + ((2.0 * gam2 - gam1 * gam1 + 1.0 / Fb / Fb) * Fb * Fb / 24.0
						+ alpha * Fb / sigma / Af * (0.25 * rho * gam1 * Fb +
...
static double res;
	...
	switch (type)
	{
		...
		{
			if(beta==1) res = log(x);
			else res = 1.0/(1.0-beta)*pow(x,1.0-beta);
...
double	vol_bvm2(double F, double F0, double Beta, double not_used2, int type)
{
	...
	switch (type)
	{
		...
		case 2:
		{
			...
			double o3=1.-Beta;
			double o4=pow(F,o3);
			res = o1*exp(-o4)*(-(o1*pow(F,2.))+Beta*(-1.+exp(o4))*pow(F,2.*Beta)-Beta*pow(F,1.+Beta))*pow(F,-2.-Beta);
			res /= F0 * F0;
			return res;
			break;
		}
...
double	vol_bvmh(double F, double F0, double Beta, double notused2, int type)
{
	...
	switch (type)
	{
		...
		case 2:
		{
			...
			double o4=pow(F,o3);
			double o5=tanh(o4);
			res = o2*pow(F,-2.+o1)*(o5*Beta*pow(F,2.*Beta)-F*(2.*F*o2*o5+Beta*pow(F,Beta))*pow(1/cosh(o4),2.));
			res /= F0 * F0;
	...
	switch (type)
	{
		...
		case 1:
			{			
				...
				double o6=tanh(F/F0);
				res = (1/F0*o4*(-(F0*o4*o6*Beta*pow(1/sinh(o4),2.))+1/tanh(o4)*(F0*o6*Beta+F*pow(1/cosh(F/F0),2.))))/F;
		...
		case 2:
		{
				...
				double o10=pow(o9,2.);
				double o11=tanh(o8);
				res = (o4*pow(F,-2.)*(-(F0*o4*o6*Beta*(2.*F*o10+F0*o11*(-1.+3.*Beta)))+1/tanh(o4)*(2.*F*o10*(-(F*o11)+F0*Beta)+o1*o11*Beta*(-1.+Beta+2.*o6*Beta*pow(o3,2.*Beta)))))/o1;
	...
	if (BetaMin==BetaMax && BetaMax==0){
		...
		case 2:
			{	
				...
				res = -(BetaMax*(1.-BetaMax+(1.+BetaMax)*o1)*pow(F,-2.+BetaMax)*pow(1.+o1,-3.));
				res = res / (F0*F0);
				return res;
				break;
			}
		...
		case 2:
			{	
				double o1 = pow(F,BetaMax);
				res = -((-1.+BetaMax)*o1*(-(BetaMax*F)+(-2.+BetaMax)*o1)*pow(F+o1,-3.));
				res = res / (F0*F0);
				return res;
				break;
			}
		...
		case 2:
			{	
				double o1 = pow(F,BetaMin);
				res = -((-1.+BetaMin)*o1*(-(BetaMin*F)+(-2.+BetaMin)*o1)*pow(F+o1,-3.));
				res = res / (F0*F0);
				return res;
				break;
			}
	...
	// Main Case
		...
		case 1:
			{	
				...
				res = (BetaMin*o1+BetaMax*o2)*pow(F,-1.+BetaMax+BetaMin)*pow(o1+o2,-2.);
		...
		case 2:
			{			
				...
				res = pow(F,-2.+BetaMax+BetaMin)*((-1.+BetaMin)*BetaMin*pow(F,2.*BetaMax)+(-1.+BetaMax)*BetaMax*pow(F,2.*BetaMin)-(BetaMax+BetaMin-4.*BetaMax*BetaMin+pow(BetaMax,2.)+pow(BetaMin,2.))*pow(F,BetaMax+BetaMin))*pow(pow(F,BetaMax)+pow(F,BetaMin),-3.);	
...
{
	double alb = - DBL_MAX, aub= DBL_MAX, blb= - DBL_MAX, bub= DBL_MAX, clb= - DBL_MAX, cub= DBL_MAX;

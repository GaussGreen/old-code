...
static void grfn_mk_midat_str(
    String opt_str, double bond_strike, SrtReceiverType rec_pay, SRT_Boolean pv)
{
    String pvstring;
    if (pv == SRT_YES)
        pvstring = "PV[0]";
    ...
    {
        sprintf(opt_str, "max(pvrng(0,1,a[2,i])-%.12lf*df(now,a[2,i]),%s)", bond_strike, pvstring);
    }
    else
    {
        sprintf(opt_str, "max(-pvrng(0,1,a[2,i])+%.12lf*df(now,a[2,i]),%s)", bond_strike, pvstring);
    ...
    GrfnCell***     sprdsht)
{
    ...
    *sprdsht                 = GrfnCellmatrix(*nrows, *ncols, GRFN_DEF_ARGBUFSZ);
    ...
    {
        grfn_mk_midat_str((*sprdsht)[i][0].sval, exercise_premiums[i], rec_pay, SRT_YES);
        (*sprdsht)[i][0].type = GRFNSCELL;
        if (err)
            return err;
    }
    grfn_mk_midat_str((*sprdsht)[i][0].sval, exercise_premiums[i], rec_pay, SRT_NO);
    (*sprdsht)[i][0].type = GRFNSCELL;
    if (err)
        return err;

...
static void grfn_mk_midat_str(String        opt_str, 
                double      bond_strike, 
                SrtReceiverType rec_pay,
                SRT_Boolean     pv)
{           
   String pvstring;
   if(pv==SRT_YES)pvstring = "PV[0]";
   else pvstring = "0.0";

   if(rec_pay == SRT_RECEIVER){
    sprintf(opt_str,"max(pvrng(0,1,a[2,i])-%.12lf*df(now,a[2,i]),%s)",
...
static Err grfn_mk_midat_GrfnCells(
        long num_exercise_dates,
        Date *exercise_dates,
...
/*** only include exercise dates after clcn_date ***/
    if(num_exercise_dates <= 0)return serror("no valid exercise dates");
...
/*** enter payoff string for each date ***/
        ...
        grfn_mk_midat_str((*sprdsht)[i][0].sval, 
            exercise_premiums[i], 
            rec_pay,SRT_YES);
...
/*** create auxiliary vars ***/
   if(num_prod_dates < 1){*answer=0;return NULL;}
...
/* CONSTRUCT A GRFN SPREADSHEET THAT DESCRIBES THE MIDAT SWAPTION */
   err = grfn_mk_midat_GrfnCells(
        num_exercise_dates,exercise_dates,exercise_premiums,
        rec_pay,today,
        &num_eventdates,&eventdates,&nrows,&ncols,&sprdsht);

   if(!err)
    err = srt_f_grfn(und,grfnparam,num_eventdates,&eventdates,&nrows,&ncols,&sprdsht,
    0,0,auxwidth,auxlen,aux,iolist,0,0);

...
static double BlackDerivative(double F, double sigma, double T, double K,
                              int iF, int iSigma) {
  ...
  case 4:
    ...
    case 0:
      ...
      o6 = pow(o5, 2.);
      o7 = o4 * T;
      res = (0.0249338925250895 * K *
             exp((-0.5 * o6) / (o4 * T) - 0.125 * o4 * T) *
  ...
  case 3:
    ...
    case 0:
      ...
      res = -0.199471140200716 * K * (2. * o4 + 3. * o3 * T) *
            exp(-0.125 * o3 * T - (0.5 * pow(o4, 2.)) / (o3 * T)) *
            pow(F, -3.) * pow(sigma, -3.) * pow(T, -3. / 2.) * sqrt(o2);
      break;
    case 1:
      o1 = 1 / K;
      o2 = F * o1;
      ...
      res = 0.0498677850501791 * K * (2. * o4 + o5) *
            ...
            exp(-0.125 * o3 * T - (0.5 * pow(o4, 2.)) / (o3 * T)) *
            pow(F, -3.) * pow(sigma, -6.) * pow(T, -5. / 2.) * sqrt(o2);
      break;
    }
    break;
  ...
  case 2:
    ...
    case 1:
      ...
      res = -0.0997355701003582 * K * (-4. * o5 + o3 * T * (4. + o3 * T)) *
            exp((-0.5 * o5) / (o3 * T) - 0.125 * o3 * T) * pow(F, -2.) *
            pow(sigma, -4.) * pow(T, -3. / 2.) * sqrt(o2);
      break;
    case 2:
      o1 = 1 / K;
      o2 = F * o1;
  ...
  case 1:
    ...
    case 2:
      ...
      o4 = log(o2);
      o5 = pow(o4, 2.);
      res = (-0.04986778505017909 *
             exp((-0.5 * o5) / (o3 * T) - 0.125 * o3 * T) * pow(sigma, -5.) *
             (-4. * o3 * o5 * T - 2. * o3 * o4 * T * (8. + o3 * T) +
              8. * pow(o4, 3.) + pow(sigma, 6.) * pow(T, 3.)) *
             pow(T, -3. / 2.)) /
            sqrt(o2);
      break;
    case 3:
      ...
      o9 = pow(T, 2.);
      res =
          (0.012466946262544772 * exp((-0.5 * o6) / (o4 * T) - 0.125 * o4 * T) *
           (2. * o5 *
                (-(o8 * o9 * (96. + o4 * (12. + o7) * T)) +
                 4. * o5 *
                     (-((6. + o7) * o8 * o9) +
                      2. * o5 *
                          (-2. * o6 + o4 * o5 * T + o4 * (14. + o7) * T))) +
            o3 * (-4. + o7) * pow(T, 4.)) *
           pow(T, -5. / 2.)) /
          (o3 * sqrt(o2));
      break;
  ...
  case 0:
    ...
    case 4:
      ...
      o6 = o3 * T;
      res =
          -0.00623347313127239 * K *
          exp((-0.5 * o5) / (o3 * T) - 0.125 * o3 * T) * pow(sigma, -9.) *
...
static double
First_Order_Integrand_function(double date, double FwdInit, double Fwd,
                               double Vol, double Strike, double Maturity,
                               double gamma, double alpha, double rho,
                               double VolInfinity, double Lambda, int L_or_N,
                               double (*local_vol)(double x, double a, double b,
                                                   double c, int log_or_norm),
                               double *f1, double *f2) {
  double res;
  double d1, d2;
  ...
  if (L_or_N == 1) {
    Gamma = srt_f_optblksch(Fwd, Strike, Vol, Maturity - date, 1.0, SRT_CALL,
                            GAMMA);

    Vega =
        srt_f_optblksch(Fwd, Strike, Vol, Maturity - date, 1.0, SRT_CALL, VEGA);

    Volga = srt_f_optblksch(Fwd, Strike, Vol, Maturity - date, 1.0, SRT_CALL,
                            VOLGA);

    Vanna = srt_f_optblksch(Fwd, Strike, Vol, Maturity - date, 1.0, SRT_CALL,
                            VANNA);

    *f1 = Vol * Vol * Fwd * Fwd *
          (local_vol(Fwd, gamma, 0, 0, L_or_N) /
               local_vol(FwdInit, gamma, 0, 0, L_or_N) -
           1.0) *
          Gamma;
    ...
    *f2 += rho * alpha * Vol * Fwd * Vol * Vanna;
    ...
    Gamma = srt_f_optblknrm(Fwd, Strike, Vol, Maturity - date, 1.0, SRT_CALL,
                            GAMMA);

    Vega =
        srt_f_optblknrm(Fwd, Strike, Vol, Maturity - date, 1.0, SRT_CALL, VEGA);

    Volga = srt_f_optblknrm(Fwd, Strike, Vol, Maturity - date, 1.0, SRT_CALL,
                            VOLGA);

    Vanna = srt_f_optblknrm(Fwd, Strike, Vol, Maturity - date, 1.0, SRT_CALL,
                            VANNA);
...
static double Expectation_In_Fwd(
    double today, double date, double Fwd, double Vol, double Strike,
    double Maturity, double gamma, double alpha, double rho, double VolInfinity,
    double Lambda, int log_or_norm,
    double (*local_vol)(double x, double a, double b, double c,
                        int lgo_or_norm),
    int NQuadrature, double *w, double *x, double *f1, double *f2) {
  int i;
  double res;
  double Forward;
  ...
  for (i = 1; i <= NQuadrature; ++i) {
    if (log_or_norm == 1) {
      Forward = Fwd * exp(-0.5 * (date - today) * Vol * Vol) *
                exp(Vol * sqrt(date - today) * x[i]);
    } else {
      Forward = Fwd + Vol * sqrt(date - today) * x[i];
    }

    res += w[i] * First_Order_Integrand_function(
                      date, Fwd, Forward, Vol, Strike, Maturity, gamma, alpha,
                      rho, VolInfinity, Lambda, log_or_norm, local_vol, &tempf1,
                      &tempf2);
...
static double First_Order_Integral_In_Time(
    double today, double Fwd, double Vol, double Strike, double Maturity,
    double gamma, double alpha, double rho, double VolInfinity, double Lambda,
    int log_or_norm,
    double (*local_vol)(double x, double a, double b, double c,
                        int log_or_norm),
    int NQuad_Her, double *w_her, double *x_her, int NQuad_Leg, double *w_leg,
    double *x_leg, double *f1, double *f2) {
  int i;
  double res;
  double tempf1, tempf2;
  ...
  *f2 = 0.0;
  for (i = 1; i <= NQuad_Leg; ++i) {
    if (today < x_leg[i]) {
      res += w_leg[i] * Expectation_In_Fwd(today, x_leg[i], Fwd, Vol, Strike,
                                           Maturity, gamma, alpha, rho,
                                           VolInfinity, Lambda, log_or_norm,
                                           local_vol, NQuad_Her, w_her, x_her,
                                           &tempf1, &tempf2);
...
static double Second_Order_Integrand_function(
    double date, double FwdInit, double Fwd, double Vol, double Strike,
    double Maturity, double gamma, double alpha, double rho, double VolInfinity,
    double Lambda, int L_or_N,
    double (*local_vol)(double x, double a, double b, double c,
                        int log_or_norm),
    int NQuad_Her, double *w_her, double *x_her, int NQuad_Leg, double *w_leg,
    double *x_leg) {
  ...
  f = First_Order_Integral_In_Time(date, Fwd, Vol, Strike, Maturity, gamma,
                                   alpha, rho, VolInfinity, Lambda, L_or_N,
                                   local_vol, NQuad_Her, w_her, x_her,
                                   NQuad_Leg, w_leg, x_leg, &f1, &f2);

  f_f_up = First_Order_Integral_In_Time(
      date, Fwd + dFwd, Vol, Strike, Maturity, gamma, alpha, rho, VolInfinity,
      Lambda, L_or_N, local_vol, NQuad_Her, w_her, x_her, NQuad_Leg, w_leg,
      x_leg, &f1_f_up, &f2_f_up);

  f_f_down = First_Order_Integral_In_Time(
      date, Fwd - dFwd, Vol, Strike, Maturity, gamma, alpha, rho, VolInfinity,
      Lambda, L_or_N, local_vol, NQuad_Her, w_her, x_her, NQuad_Leg, w_leg,
      x_leg, &f1_f_down, &f2_f_down);

  f_vol_up = First_Order_Integral_In_Time(
      date, Fwd, Vol + dVol, Strike, Maturity, gamma, alpha, rho, VolInfinity,
      Lambda, L_or_N, local_vol, NQuad_Her, w_her, x_her, NQuad_Leg, w_leg,
      x_leg, &f1_vol_up, &f2_vol_up);

  f_vol_down = First_Order_Integral_In_Time(
      date, Fwd, Vol - dVol, Strike, Maturity, gamma, alpha, rho, VolInfinity,
      Lambda, L_or_N, local_vol, NQuad_Her, w_her, x_her, NQuad_Leg, w_leg,
      x_leg, &f1_vol_down, &f2_vol_down);

  f_f_up_vol_up = First_Order_Integral_In_Time(
      date, Fwd + dFwd, Vol + dVol, Strike, Maturity, gamma, alpha, rho,
      VolInfinity, Lambda, L_or_N, local_vol, NQuad_Her, w_her, x_her,
      NQuad_Leg, w_leg, x_leg, &f1_f_up_vol_up, &f2_f_up_vol_up);
  ...
  if (L_or_N == 1) {
    ...
    res += rho * Fwd * (LocVolFwd / LocVolFwdInit - 1.0) * alpha * Vol * Vol *
           Vanna;
...
static double Second_Order_Expectation_In_Fwd(
  ...
  for (i = 1; i <= NQuad_Her; ++i) {
    ...
    res +=
        w_her[i] * Second_Order_Integrand_function(
                       date, Fwd, Forward, Vol, Strike, Maturity, gamma, alpha,
                       rho, VolInfinity, Lambda, log_or_norm, local_vol,
                       NQuad_Her, w_her, x_her, NQuad_Leg, w_leg, x_leg);
  }
...
static double
Second_Order_Integral_In_Time(double today, double Fwd, double Vol,
                              double Strike, double Maturity, double gamma,
                              double alpha, double rho, double VolInfinity,
                              double Lambda, int log_or_norm,
                              double (*local_vol)(double x, double a, double b,
                                                  double c, int log_or_norm),
                              int NQuad_Her, double *w_her, double *x_her,
                              int NQuad_Leg, double *w_leg, double *x_leg) {
  int i;
  double res;
  ...
  for (i = 1; i <= NQuad_Leg; ++i) {
    res +=
        w_leg[i] * Second_Order_Expectation_In_Fwd(
                       today, x_leg[i], Fwd, Vol, Strike, Maturity, gamma,
                       alpha, rho, VolInfinity, Lambda, log_or_norm, local_vol,
                       NQuad_Her, w_her, x_her, NQuad_Leg, w_leg, x_leg);
  }
...
double BVM_First_Order_Perturbation_Price(
  ...
  if (log_or_norm == 1) {
    black = srt_f_optblksch(Fwd, Strike, Vol * locvol, Maturity - today, 1.0,
                            SRT_CALL, PREMIUM);
  } else {
    black = srt_f_optblknrm(Fwd, Strike, Vol * locvol, Maturity - today, 1.0,
                            SRT_CALL, PREMIUM);
  }

  pertvalue = First_Order_Integral_In_Time(
      today, Fwd, Vol * locvol, Strike, Maturity, gamma, alpha, rho,
      VolInfinity, Lambda, log_or_norm, local_vol, NQuad_Her, w_her, x_her,
      NQuad_Leg, w_leg, x_leg, &f1, &f2);
...
Err monte_carlo_sabrbvm(double **rndm_mat, double forw, double vovol,
                        double beta, double rho, double sigma, double num_paths,
                        double maturity, double num_steps, int path_num,
                        double *Forward, int modeltype, int sampletype,
                        int *pathnogood) {
  double g[2];
  double a1;
  double a2;
  ...
  *pathnogood = 0;
...

  {
    ...
    case 0:
      ...
      //			Forward[0]+=Eta*pow(Forward[0]
      //      ,beta)*g[0]*sqrt_delta_t;
    ...
    case 1:
      ...
      Forward[0] +=
          ...
          0.5 * Eta * Eta * beta * exp(-beta * Forward[0]) * (g[0] * g[0] - 1) *
              delta_t;
      Forward[1] +=
    ...
    //			logEta = log(Eta);
    //			logEta = logEta + (-0.5 * vovol*vovol + lambda * ( logEta0 -
    //logEta ) )* delta_t
    //							+
    // vovol*g[1]*sqrt_delta_t; 			Eta = exp(logEta);

    Eta =
        Eta * exp(vovol * g[1] * sqrt_delta_t - 0.5 * vovol * vovol * delta_t);
...
Err monte_carlo_sabrbvm3(double forw, double vovol, double beta, double rho,
                         double sigma, double lambda, double num_paths,
                         double maturity, double num_steps, int path_num,
                         double *Forward, int modeltype, int sampletype,
                         int *pathnogood) {
  double g[2];
  ...
  *pathnogood = 0;
...

  {
    ...
    logEta = log(Eta);
    logEta = logEta +
             (-0.5 * vovol * vovol + lambda * (logEta0 - logEta)) * delta_t +
...
Err monte_carlo_sabrbvm2(double forw, double vovol, double beta, double rho,
                         double sigma, double num_paths, double maturity,
                         double num_steps, int path_num, double *Forward,
                         int modeltype, int sampletype, int *pathnogood) {
  double g[2];
  double a1;
  double a2;
  double Eta;
...
Err sabrBVMmontecarlo2(double forward, double *strike, int nb_strike,
  ...
  for (j = 0; j < nb_strike; j++) {
    ...
    err = srt_f_optimpvol(res[j][0], forward, strike[j], maturity, 1, SRT_CALL,
                          SRT_LOGNORMAL, &(impliedvol));
    if (err) {
      res[j][0] = 0;
...
Err sabrBVMmontecarlo(double forw, double vovol, double beta, double rho,
                      ...
                      int modeltype,
                      int sampletype, /*0: randsam        , 1: abs        ,
                                         2:sobol        , 3: SPECTRUNC*/
                      double *impvol) {
  double ***cube = NULL;
  int cur_path;
  ...
  //	cube = f3tensor(1        ,(long) num_paths        ,1        ,2        ,1
  //,(long) num_steps);
  Forward = dvector(1, 2);
  ...
  /*	if (sampletype == 3)
          {
                  if (err = SpecTruncCube(cube        ,times_at_steps ,0.0001 ,1
           ,(long) num_paths        ,1        ,2        ,1        ,(long)
     num_steps        ,&seed))
                  {
                          free_dmatrix(price        ,1        ,2        ,1
     ,num_strikes); free_f3tensor(cube        ,1        ,(long) num_paths ,1 ,2
     ,1
           ,(long) num_steps); free_dvector(Forward        ,1        ,2);
     free_dvector(blkbeta        ,1 ,num_strikes); smessage("error in sobol
     cube"); return err;
                  }
          }
  */
  ...
  /*	if (sampletype==2)
          {
                  if (err = sobol_init(1        ,(long) num_paths        ,1 ,2
     ,1        ,(long) num_steps))
                  {
                          free_dmatrix(price        ,1        ,2        ,1
     ,num_strikes); free_f3tensor(cube        ,1        ,(long) num_paths ,1 ,2
     ...
     cube"); return err;
                  ...
                  if (err = sobol_cube(cube        ,1        ,(long) num_paths
     ,1        ,2        ,1
           ,(long) num_steps))
                  {
                          free_dmatrix(price        ,1        ,2        ,1
  ...
  */
  ...
  /*	else if (sampletype == 1)
          {
                  if (err = ABSCube(cube        ,1        ,(long) num_paths ,1
     ,2        ,1
           ,(long) num_steps        , &seed))
                  {
                          free_dmatrix(price        ,1        ,2        ,1
     ,num_strikes); free_dvector(Forward        ,1        ,2);
                          free_f3tensor(cube        ,1        ,(long) num_paths
     ,1        ,2        ,1
           ,(long) num_steps); free_dvector(blkbeta        ,1 ,num_strikes);
  ...
  */
  ...
  for (cur_path = 1; cur_path <= num_paths; cur_path++) {
    ...
    if (err =
            monte_carlo_sabrbvm(cube[cur_path], forw, vovol, beta, rho, sigma,
                                num_paths, maturity, num_steps, cur_path + 1,
                                Forward, modeltype, sampletype, &pathnogood)) {
    ...
    if (pathnogood == 0) {
      for (i = 1; i <= num_strikes; i++) {
        ...
        stdev[1][i] +=
            DMAX(Forward[0] - strike[i], 0) * DMAX(Forward[0] - strike[i], 0);
        correl[i] +=
            DMAX(Forward[0] - strike[i], 0) * DMAX(Forward[1] - strike[i], 0);
        price[2][i] += DMAX(Forward[1] - strike[i], 0);
        stdev[2][i] +=
            DMAX(Forward[1] - strike[i], 0) * DMAX(Forward[1] - strike[i], 0);
      }
    } else {
      actnumpaths--;
    }
  ...
  for (i = 1; i <= num_strikes; i++) {
    ...
    correlation = (correl[i] - price[1][i] * price[2][i] / actnumpaths) /
                  sqrt((stdev[1][i] - price[1][i] * price[1][i] / actnumpaths) *
                       (stdev[2][i] - price[2][i] * price[2][i] / actnumpaths));

    price[1][i] = price[1][i] / actnumpaths;

    if (err = srt_f_optimpvol(price[1][i], forw, strike[i], maturity, 1,
                              SRT_CALL, SRT_LOGNORMAL, &(impvol[i]))) {
      impvol[i] = 0;
      smessage("error in implied vol");
      err = NULL;
    }
...
Err sabrBVMmontecarlo3(double forw, double vovol, double beta, double rho,
  ...
  for (cur_path = 1; cur_path <= num_paths; cur_path++) {
    ...
    if (err =
            monte_carlo_sabrbvm3(forw, vovol, beta, rho, sigma, lambda,
                                 num_paths, maturity, num_steps, cur_path + 1,
                                 Forward, modeltype, sampletype, &pathnogood)) {
      free_dmatrix(price, 1, 2, 1, num_strikes);
      free_dvector(Forward, 1, 2);
...
//--------------------------------------------------------------------------------
//-----------------------Pat's formula applied to
// BVM-----------------------------
//--------------------------------------------------------------------------------
double BVMLOGVol(double forward, double strike, double maturity, double betavol,
                 double vovol, double rho, double eta) {
  double favg;
  ...
  zeta = (vovol / betavol) *
         (eta * (forward - strike) + log(1 - exp(-eta * forward)) -
          log(1 - exp(-eta * strike)));
  ...
  logvol = logvol * (1 + ((2 * gamma2 - gamma1 * gamma1 + 1 / (favg * favg)) /
                              24.0 * betavol * betavol * volavg * volavg +
...
double SABRNormalVol(double forward, double strike, double maturity,
  ...
  normvol = normvol * (1 + ((2 * gamma2 - gamma1 * gamma1) / 24.0 * betavol *
...
double BVMApproxSimulUnderlying(double X, double XStar, double GStar,
...
Err BVMApproxPrice(double Xinit, double XStar, double GStar, double Sigma,
                   double Lambda, double Maturity, long NHermite, long NStrikes,
                   double *Strikes, double *ImpliedVol, double *Fwd) {
  Err err = NULL;
  int i, j;
  double *x_her = NULL;
  double *w_her = NULL;
  ...
  *Fwd = 0;
  ...
  //	for(j=0;j<NStrikes;++j)
  //	{
  //		err = srt_f_optimpvol(ImpliedVol[j]        , *Fwd        ,
  //Strikes[j]        , Maturity        , 1        , SRT_CALL        ,
  // SRT_LOGNORMAL        , &(impliedvol)); 		ImpliedVol[j] =
  //impliedvol;
  //	}
...
Err BVMApproxPriceFromFwd(double Fwd, double XStar, double GStar, double Sigma,
                          double Lambda, double Maturity, long NHermite,
                          long NStrikes, double *Strikes, double *ImpliedVol,
                          double *FwdOutPut) {
  Err err = NULL;
  int NIter;
  ...
  err = BVMApproxPrice(Xinit1, XStar, GStar, Sigma, Lambda, Maturity, NHermite,
                       0, NULL, NULL, &Fwd1);
  ...
  err = BVMApproxPrice(Xinit2, XStar, GStar, Sigma, Lambda, Maturity, NHermite,
                       0, NULL, NULL, &Fwd2);
  ...
  if (fabs(Xinit1 - Xinit2) > 1.0e-20) {
    ...
    err = BVMApproxPrice(Xinit1, XStar, GStar, Sigma, Lambda, Maturity,
                         NHermite, 0, NULL, NULL, &Fwd1);
  } else {
    NIter = 20;
  }
  ...
  while ((NIter < 20) && (fabs(Fwd - Fwd1) > 1.0e-7)) {
    ...
    err = BVMApproxPrice(Xinit2, XStar, GStar, Sigma, Lambda, Maturity,
                         NHermite, 0, NULL, NULL, &Fwd2);
  ...
  err = BVMApproxPrice(Xinit, XStar, GStar, Sigma, Lambda, Maturity, NHermite,
                       NStrikes, Strikes, ImpliedVol, FwdOutPut);
...
double g_function(double x, double xStar, double gStar, double lambda,
                  int nderiv) {
  double res;
  double C;
  ...
  case 0:
    if (x > 50.0) {
      ...
      res = log(C) / lambda + exp(-x) / (C * lambda) +
            0.5 * exp(-2 * x) / (C * C * lambda);
    } else {
      res = log(1 + exp(x - xStar) * (exp(lambda * gStar) - 1)) / lambda;
    }
    break;
  ...
  case 4:
    res = exp(x - xStar) * (exp(lambda * gStar) - 1) *
          (1 + exp(2 * (x - xStar)) + exp(2 * (x - xStar + lambda * gStar)) -
           2 * exp(2 * (x - xStar) + lambda * gStar) -
           4 * exp(x - xStar + 2 * lambda * gStar) + 4 * exp(x - xStar)) /
          lambda / (exp(x - xStar) - 1 - exp(x - xStar + lambda * gStar)) /
          (exp(x - xStar) - 1 - exp(x - xStar + lambda * gStar)) /
          (exp(x - xStar) - 1 - exp(x - xStar + lambda * gStar)) /
          (exp(x - xStar) - 1 - exp(x - xStar + lambda * gStar));
  ...
  case 5:
    res =
        ...
        (1 + 11 * exp(x - xStar) - 22 * exp(2 * (x - xStar) + lambda * gStar) -
         ...
         3 * exp(3 * (x - xStar) + 2 * lambda * gStar) -
         11 * exp(x - xStar + lambda * gStar) + exp(3 * (x - xStar)) +
         11 * exp(2 * (x - xStar + lambda * gStar))) /
        lambda / (exp(x - xStar) - 1 - exp(x - xStar + lambda * gStar)) /
        (exp(x - xStar) - 1 - exp(x - xStar + lambda * gStar)) /
        (exp(x - xStar) - 1 - exp(x - xStar + lambda * gStar)) /
        (exp(x - xStar) - 1 - exp(x - xStar + lambda * gStar)) /
...
Err BVMApproxAccurate(double Xinit, double Strike, double Maturity,
                      double XStar, double GStar, double sigma, double lambda,
                      long Order, double *Price) {
  Err err = NULL;
  int i;
  double price;
  ...
  for (i = 1; i < Order; ++i) {
    ...
    facti = i * facti;
    price +=
        g_function(H, XStar, GStar, lambda, i) * powvol / facti * I_func[i];
  }
...
Err BVMApproxAccurate2(double Xinit, double Strike, double Maturity,
                       double XStar, double GStar, double sigma, double lambda,
                       long NQuadrature, double *Price) {
  Err err = NULL;
  int i;
  double price;
  double vol;
  ...
  for (i = 1; i <= NQuadrature; ++i) {
    price +=
        w[i] * DMAX(0, g_function(Xinit + vol * x[i], XStar, GStar, lambda, 0) -
                           Strike);
  }

...
static Err IMM_fut(int nm, int spot,
			int sy, int sm,
			Date *fut_last_trading,
			Date *fut_start, 
			Date *fut_end);

static Err AUD_fut(int nm, int spot,
			int sy, int sm,
			Date *fut_last_trading,
			Date *fut_start, 
			Date *fut_end);

static Err GBP_fut(int nm, int spot,
			int sy, int sm,
			Date *fut_last_trading,
			Date *fut_start, 
			Date *fut_end);
...
static Err PIBOR_fut(int nm, int spot,
			int sy, int sm,
			Date *fut_last_trading,
			Date *fut_start, 
			Date *fut_end);
/* ------------------------------------------------------------------------------------------ */
/* Dynamic ccy data - if present this takes precedence over static data defined below         */
/* ------------------------------------------------------------------------------------------ */
typedef struct {
	SrtCcyParam *this;
	void		*next;
...
CcyParamListItem *item = srt_dyn_ccy_params;
	...
	while ( default_ccy_params[i].currency != LASTCCYCODE )
	{
		if (ccy_code == default_ccy_params[i].currency )
		{
...
Err ccy_string_get_param(SrtCcyParam *ccy_param,
				String param_name, 
				String *param_val)
{
	...
	if(!strcmp(param_name,"CASHBUSDAYCONV"))
	{
		...
		if(err = (translate_bus_day_conv(param_val,val))) return err;
		return 0;
	}					
	...
	if(!strcmp(param_name,"CASHBASIS"))			
	{
		...
		if(err = (translate_basis(param_val,val))) return err;
		return 0;
	}					
	...
	{
		val = ccy_param->compd;
		if(err = (translate_compounding(param_val,val))) return err;
		return 0;
	}					
	if(!strcmp(param_name,"INTERPMETHOD"))			
	{
		val = ccy_param->interp_method;
		if(err = (translate_interp_method(param_val,val))) return err;
		return 0;
	}					
	if(!strcmp(param_name,"INTERLEAVE"))			
	{
		val = ccy_param->ilv_insert_flg;
		if(err = (translate_insert_flg(param_val,val))) return err;
		return 0;
	}					
	...
	if(!strcmp(param_name,"INTERLEAVEOVERWRITE"))			
	{
		...
		if(err = (translate_no_overwrite_flg(param_val,val))) return err;
		return 0;
	}					
...
Err ccy_string_set_param(SrtCcyParam *ccy_param,
					String param_name,
					String param_val)
{
	Err err;
	...
	if(!strcmp(param_name,"CASHBUSDAYCONV"))
	{
		if(err = (interp_bus_day_conv(param_val,&val))) return err;
	...
	if(!strcmp(param_name,"CASHBASIS"))			
	{
		if(err = (interp_basis(param_val,&val))) return err;
	...
	if(!strcmp(param_name,"COMPOUNDING"))			
	{
		if(err = (interp_compounding(param_val,&val))) return err;
		ccy_param->compd = val;
		return 0;
	}					
	if(!strcmp(param_name,"INTERPMETHOD"))			
	{
		if(err = (interp_interp_method(param_val,&val))) return err;
		ccy_param->interp_method = val;
		return 0;
	}					
	if(!strcmp(param_name,"INTERLEAVE"))			
	{
		if(err = (interp_insert_flg(param_val,&val))) return err;
	...
	if(!strcmp(param_name,"INTERLEAVEOVERWRITE"))			
	{
		if(err = (interp_no_overwrite_flg(param_val,&val))) return err;
...
/* FIX for holidays */
	...
	if(nm != 3) return serror("Only understand QUARTERLY Pibor futures.");

	*fut_last_trading = eleventh_thursday(sm-2,sy) - 2; 
	*fut_start = *fut_last_trading + spot;
	*fut_end = eleventh_thursday((sm+1 < 13 ? sm+1 :1),(sm+1 < 13 ? sy: sy+1)) - 2 + spot;

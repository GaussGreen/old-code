...
static Err try_sigma(double sig, double *diff, void *static_data)
{
	...
	if (p->nx > MAXNX) return serror("nx should be no more than %d", MAXNX);
	if (p->ex_data->nmkts > 2) return serror("Maximum two markets supported");
	...
	for (j=0; j < p->ex_data->nmkts; j++) if (j != p->mkt_idx)
	...
	for (i=0, sum=0.0; i < p->nx; i++)
	{
		...
		for (k=0; k < p->ex_data->nmat[p->mkt_idx]; k++)
		{
			...
			dfs[p->mkt_idx][k] = p->ex_data->dfs[p->mkt_idx][k] *
				exp( -gam * (x[i] + 0.5 * gam * phi) );
		}
		/* Calculate payoff at x[i] */
		err = p->product->Payoff( p->product,
			p->ex_data->d, p->ex_data->d, dfs, &payoff );
		if (err) return err;
		...
		double				qto_fudge )
{
	...
	if (!yc_ptr) return serror("Yield curve %s not found", yc_names[0]);
	...
	for (i=0, t1=0.0; i < nex; i++)
	{
		...
		{
			err = product->Payoff(
				product, today, ex_dates[i], NULL, &(ex_data[i].target) );
			if (err) goto FREE_RETURN;
		}
		else		/* otherwise ask the product */
		{
			err = product->RequestDfDates(
				product, ex_dates[i], &n_unds, &n_dfs, &dates );
			if (err) goto FREE_RETURN;
			if (n_unds > n_mkts)
			...
			for (j=0; j < n_unds; j++)
			{
				...
				if (j == mkt_idx)
				{
					...
					for (k=0; k < n_dfs[j]; k++)
					{
						ex_data[i].gammas[k] = (1.0 - exp(
							-lam * (dates[j][k] - ex_dates[i]) * YEARS_IN_DAY )) / lam;
					}
				}
			}
			...
			/* Calculate target */
			err = product->Payoff(
				product, today, ex_dates[i], ex_data[i].dfs, &(ex_data[i].target) );
			if (err) goto FREE_RETURN;

			/* Now replace dfs by forward dfs */
			for (j=0; j < n_unds; j++)
				for (k=0; k < n_dfs[j]; k++) ex_data[i].dfs[j][k] =
					swp_f_df(ex_dates[i], dates[j][k], yc_names[j]);
...
FREE_RETURN:
	...
	{
		if (ex_data[i].dfs)
			for (j=0; j < ex_data[i].nmkts; j++) free(ex_data[i].dfs[j]);
		free(ex_data[i].dfs);
		free(ex_data[i].nmat);

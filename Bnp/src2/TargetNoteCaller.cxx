...
char *TargetNoteCaller(TARN_Struct *in_tarn) {
  ...
  /* if the deal is knocked out (but does not have a final payment)or has
   * finished        , record the remaining PV and return */
  if ((tarn->output.iIsKnockedOut &&
       (tarn->deal.cxxouponType != TN_FINAL ||
        aux->i1stCpn == (tarn->deal.nCouponDates - 1))) ||
  ...
  if (tarn->pricing.iPrice2FCV) {
    smessage("2F");
    if (err = TargetNoteMC_2F(tarn->output.szTARN_LGM2F_UND, tarn, aux,
                              tarn->output.dmLGM2F_PV))
      goto FREE_RETURN;

    smessage("1F");
    if (err = TargetNoteMC_2F(tarn->output.szTARN_LGM1F_UND, tarn, aux,
                              tarn->output.dmLGM1F_PV))
      goto FREE_RETURN;

    smessage("1FSV");
    if (err = TargetNoteMC_SV(tarn->output.szTARN_LGM1FSV_UND, tarn, aux,
                              tarn->output.dmLGM1FSV_PV))
      goto FREE_RETURN;
  }
  ...
  if (tarn->pricing.iPriceSV) {
    smessage("SV");
    if (err = TargetNoteMC_SV(tarn->output.szTARN_LGMSV_UND, tarn, aux,
                              tarn->output.dmLGMSV_PV))
      goto FREE_RETURN;
  }

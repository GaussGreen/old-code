...
Err lgmSVprcapgivenlambda_num(
    ...
    double alpha, double lambdaEps, double rho,
    ...
    int nbT, int nbPhi, int nbX, int nbEps,

    /*	Newton Parameters		*/
    int nbIterMax, double precision,
  ...
  is_event[nbT - 1] = 1;
  memcpy(param.cxxpn_cvg, cpn_cvg, ncpn * sizeof(double));
  ...
  for (i = 0; i < nex - skip_last; i++) {
    ...
    for (j = param.start_index; j < param.ncpn; j++) {
      param.cxxpn_df[j] = cpn_df[j] / df;
      param.cxxpn_time[j] = cpn_time[j] - ex_time[i];
      param.gam[j] = (1.0 - exp(-lambda * param.cxxpn_time[j])) / lambda;
    ...
    err = srt_f_optimpvol(ex_lprice[i], fwd, ex_lstrike[i], ex_time[i], level,
                          SRT_PUT, SRT_LOGNORMAL, &bsvol);

    if (fabs(lambdaEps) > 1.0E-10) {
      alphaeq = alpha * sqrt((1.0 - exp(-2.0 * lambdaEps * ex_time[i])) /
                             (2.0 * lambdaEps * ex_time[i]));
    } else {
      alphaeq = alpha;
    }

    err = vol_conv(bsvol, SABR_ATM_LOG, &sigbeta, SABR_ATM_BETA, fwd,
                   ex_lstrike[i], ex_time[i], alphaeq, 0.0, rho);

    /*	First guess and first evaluation */

    if (i > 0) {
      newVar = vol[i - 1] * vol[i - 1] * ex_time[i - 1] +
               vol[i] * vol[i] * ex_time[i] -
    ...
    err = lgmSV_adi(nbT, time, date, nbPhi, nbX, nbEps, lambda, 1.0, ex_time,
                    vol, i + 1, lambdaEps, alpha, rho, Params, func_parm,
                    is_event, ifr, yc, payoff_lgmsv_rec_swaption, 1, &price1);
    ...
    if (error > precision) {
      ...
      if (i > 0)
      {
              vol2 = (ex_lprice[i] * ex_lprice[i] / price1 / price1 * newVar -
      Var) / (ex_time[i] - ex_time[i-1]);
      }
      else
      {
              vol2 = ex_lprice[i] * ex_lprice[i] / price1 / price1 * newVar /
      ...
      */
      ...
      err = srt_f_optimpvol(price1, fwd, ex_lstrike[i], ex_time[i], level,
                            SRT_PUT, SRT_LOGNORMAL, &bsvol1);

      err = vol_conv(bsvol1, SABR_ATM_LOG, &sigbeta1, SABR_ATM_BETA, fwd,
                     ex_lstrike[i], ex_time[i], alphaeq, 0.0, rho);
      ...
      while (error > precision && nbIter < nbIterMax) {
        err = lgmSV_adi(nbT, time, date, nbPhi, nbX, nbEps, lambda, 1.0,
                        ex_time, vol, i + 1, lambdaEps, alpha, rho,
                        /* Hard coded to be modified */
                        Params, func_parm, is_event, ifr, yc,
                        payoff_lgmsv_rec_swaption, 1, &price2);
  ...
  *sig = NULL;
  ...
  for (i = 0; i < nex; i++) {
    ...
    if ((!long_strike) || (!strike_type)) {
      ...
      if (err = swp_f_ForwardRate(cpn_date[j], theo_end_dates[i], swaption_freq,
  ...
  if (!fix_lambda) {
    ...
    for (i = 0; i < nex; i++) {
      ...
      err = get_cash_vol(vol_curve_name,
                         add_unit(ex_date[i], 2, SRT_BDAY, MODIFIED_SUCCEEDING),
                         cpn_date[ex_cpn[i] + ex_sncpn[i] - 1], ex_sfwd[i], 0,
      ...
      err = get_cash_vol(vol_curve_name,
                         add_unit(ex_date[i], 2, SRT_BDAY, MODIFIED_SUCCEEDING),
                         cpn_date[ex_cpn[i] + ex_sncpn[i] - 1], ex_sstrike[i],
                         0, ref_rate_name, &(ex_svol[i]), &power);
      if (err) {
        goto FREE_RETURN;
      }
  ...
  if (fix_lambda && one_f_equi) {
    ...
    err = lgmcalibzetalambda(
        ...
        ex_lstrike, ex_lprice, ex_sstrike, ex_sweight, 0.0, ex_zeta, 1, lambda,
        1, 0.0, 0.0, 0.0, skip_last, CALPRES, CALPRES, 0.25, 0.25 / 4.0, 0);
  ...
  err = lgmSVprcapgivenlambda_num(
      ncpn, cpn_time, cpn_df, cpn_cvg, nex, ex_time, ex_cpn, ex_endcpn,
      ex_sncpn, ex_lstrike, ex_lprice, ex_sstrike, ex_zeta, *lambda, today,
      yc_name, 2.0 * alpha, 2.0 * lambdaEps, rho, skip_last, 0, NULL, nbT,
      nbPhi, nbX, nbEps, nbIterMax, precision, ModelParams);

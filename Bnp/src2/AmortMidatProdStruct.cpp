...
char* am_check_calls(AM_STR am)
{
    ...
    for (i = 1; i < am->num_calls; i++)
    {
        ...
        {
            //			return serror ("Calls %d and %d -indexed after today- are
            // redundant", i-1, i);
        }
    }
...
char* amortMidat_compute_diagonal_prices(
    char*    yc,
    char*    vc,
    char*    ref,
    char*    cFreq,
...
char* amortMidat_compute_diagonal_prices_new(
    char*    yc,
    char*    vc,
    char*    ref,
    char*    cFreq,
    ...
    AM_ADI_ARG adi_arg)
{
    ...
    for (i = adi_arg->nstp - 1; i >= 0; i--)
    {
        ...
        {
            am_prm      = (am_pay_arg*)malloc(sizeof(am_pay_arg));
            am_prm->und = und;
            am_prm->am  = am;
            ...
            err = am_fill_eval_const_ts(
                nLambda, pdLambdaValue, pdLambdaTime, und, am, j, &(am_prm->eval_const));
...
char* am_free_adi_arg(AM_ADI_ARG adi_arg)
{
    int        i;
    AM_PAY_ARG am_prm;
    ...
    AM_ADI_ARG adi_arg)
{
    ...
    {
        err = am_fill_calls(und->today, eod_ex_flag, ncall, pay_rec, ex_date, set_date, fee, am);
        if (err)
        {
            goto FREE_RETURN;
        }
    ...
    double*** prod_val)
{
    ...
    for (i = l1; i <= u1; i++)
    {
        ...
        for (j = l2; j <= u2; j++)
        {
            ...
            fund_leg_pv +=
                eval_const->df[eval_const->start_idx] * fund_leg->notional[call->fund_idx];

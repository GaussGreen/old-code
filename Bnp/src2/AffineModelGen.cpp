...
static void FillAffineCoefFxSV(SAffinCoef *p, double t)
{
	...
	// Drift:
	p->B[0][0] = -(Gamma_d * Gamma_d / 2.0 + Gamma_f * Gamma_f / 2.0 - pmdl->rho[idx_x][0][1] * Gamma_d * Gamma_f);
	p->B[0][1] = 0.0;
	p->B[0][2] = -pmdl->beta[idx_x] * (pmdl->rho[idx_x][1][2] * Gamma_f - pmdl->rho[idx_x][0][2] * Gamma_d);
	p->B[0][3] = -0.5 * pmdl->beta[idx_x] * pmdl->beta[idx_x];

	p->B[1][0] = pmdl->rho[idx_x][0][3] * pmdl->alpha[idx_x] * Gamma_d + pmdl->gamma[idx_x];
	p->B[1][1] = 0.0;
	...
	for (i=0; i <= 3; i++) p->D[0][0][i] = -2.0 * p->B[0][i];

	p->D[0][1][0] = p->D[1][0][0] =
		pmdl->alpha[idx_x] * (pmdl->rho[idx_x][1][3] * Gamma_f - pmdl->rho[idx_x][0][3] * Gamma_d);
	p->D[0][1][1] = p->D[1][0][1] = 0.0;
	p->D[0][1][2] = p->D[1][0][2] = pmdl->alpha[idx_x] * pmdl->rho[idx_x][2][3] * pmdl->beta[idx_x];
...
static void FillAffineCoefHeston(SAffinCoef *p, double t)
{
	...
	p->D[0][1][1] = p->D[1][0][1] = 0.0;
	p->D[0][1][2] = p->D[1][0][2] = pmdl->alpha[idx] * pmdl->rho[idx][2][3] * pmdl->beta[idx];
...
//	FillAffineCoefFxSV(p, t);
	...
	for (i=1; i <= p->n; i++)
	{
		for (k=0; k <= p->n; k++)
		{
			yp[2*k] -= 0.5 * (y[2*i] * y[2*i] - y[2*i+1] * y[2*i+1]) * p->D[i-1][i-1][k];
		...
		for (j=i+1; j <= p->n; j++)
		{
			for (k=0; k <= p->n; k++)
			{
				yp[2*k] -= (y[2*i] * y[2*j] - y[2*i+1] * y[2*j+1]) * p->D[i-1][j-1][k];
				yp[2*k+1] -= (y[2*i] * y[2*j+1] + y[2*i+1] * y[2*j]) * p->D[i-1][j-1][k];
			}
		}
	}
}
...
FREE_RETURN:
	nag_ode_ivp_rk_free(&opt);
	if (comm_RK.B) free_dmatrix(comm_RK.B, 0, comm_RK.n - 1, 0, comm_RK.n);
	if (comm_RK.D) free_f3tensor(comm_RK.D, 0, comm_RK.n - 1, 0, comm_RK.n - 1, 0, comm_RK.n);
...
Err AffineCalcMoments(SFxSVComm_InvFT *q, double *mean, double *std)
{
	...
	for (i=o->ntimes-2; i >= 0; i--)
	{
		...
		nag_ode_ivp_rk_range(NEQ, EvalDerivativesU, o->times[i], &tgot, y, yp, ymax, &opt, &comm_Nag, &fail);
		if (fail.code != NE_NOERROR) { err = serror(fail.message);  goto FREE_RETURN; }

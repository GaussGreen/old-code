    ...
    double* greeks) /* array 6 * nprod containing delta, gamma, theta, vega, volga and vanna */
{
    ...
    double *expect_z = NULL, *drift_z = NULL, *x = NULL, *z = NULL, ***values = NULL,
           ***values_p1 = NULL, ***values_temp = NULL, **mux = NULL, **muz = NULL, *sigBeta = NULL,
           **varx = NULL, **varz = NULL, *varxinit = NULL, *muzinit = NULL, **r = NULL;
    ...
    {
        /*
        disc_SL_strike(x, nstepx, spot_fx, bar_ex_down, bar_ex_up, 0.9999, 0.00001, stdln,
        NSTD_FX, 1.0 / strike2, 0, &index_x);
        */

        disc_SL_center_linleft_logright(
            x, nstepx, spot_fx, bar_ex_down, bar_ex_up, 0.9999, 0.00001, stdln, NSTD_FX, &index_x);
    }
    else
    {
        disc_SL_strike(
            ...
            &index_x);
        /*
        disc_linleft_logright_center_strike(x, nstepx, spot_fx, bar_ex_down, bar_ex_up, stdln,
    ...
    if (calc_greeks)
    {
        ...
        greeks[0] =
            (values_p1[index_x + 1][index_z][0] - values_p1[index_x - 1][index_z][0]) / (dxu + dxd);
        greeks[1] = 2.0 *
                    (values_p1[index_x + 1][index_z][0] * dxd +
                     values_p1[index_x - 1][index_z][0] * dxu - (dxd + dxu) * res[0]) /
                    (dxu * dxd * (dxu + dxd));
        greeks[2] =
            (values[index_x][index_z][0] - values_p1[index_x][index_z][0]) / dt / DAYS_IN_YEAR;

        greeks[3] =
            (values_p1[index_x][index_z + 1][0] - values_p1[index_x][index_z - 1][0]) / (dzu + dzd);
        greeks[4] = 2.0 *
        ...
        greeks[5] =
            ((values_p1[index_x + 1][index_z + 1][0] - values_p1[index_x - 1][index_z + 1][0]) /
                 (dxu + dxd) -
             (values_p1[index_x + 1][index_z - 1][0] - values_p1[index_x - 1][index_z - 1][0]) /
                 (dxu + dxd)) /
            (dzu + dzd);

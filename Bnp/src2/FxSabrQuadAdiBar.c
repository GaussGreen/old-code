...
double				logx, logmax, logmin, save1, save2, save3, coef;
	...
	df_ratio = swp_f_df (today, settlmt_date, for_yc) / swp_f_df (today, settlmt_date, dom_yc);
	if (is_cvx)
	{
	...
	{
		/*
		disc_SL_strike(x, nstepx, spot_fx, bar_ex_down, bar_ex_up, 0.9999, 0.00001, stdln, NSTD_FX, 1.0 / strike2, 0, &index_x);
		*/
		
		disc_SL_center_linleft_logright(x, nstepx, spot_fx, bar_ex_down, bar_ex_up, 0.9999, 0.00001, stdln, NSTD_FX, &index_x);
	}
	else
	{		
		disc_SL_strike(x, nstepx, spot_fx, bar_ex_down, bar_ex_up, 0.9999, 0.00001, stdln, NSTD_FX, strike2, 0, &index_x);
		/*	
		disc_linleft_logright_center_strike(x, nstepx, spot_fx, bar_ex_down, bar_ex_up, stdln, NSTD_FX, strike2, &index_x);
		*/
	}
	...
	for (step=nstp-2; step>=0; step--)
	{
		...
		if (is_american)
		{		
			df_ratio = swp_f_df (today, date[step], for_yc) / swp_f_df (today, date[step], dom_yc);
			if (is_cvx)
			{
				df_ratio = 1.0 / df_ratio;
			}
	...
	if (calc_greeks)
	{
		...
		greeks[0] = (values_p1[index_x+1][index_z][0] - values_p1[index_x-1][index_z][0]) / (dxu + dxd);
		greeks[1] = 2.0 * (values_p1[index_x+1][index_z][0] * dxd + values_p1[index_x-1][index_z][0] * dxu
			- (dxd + dxu) * res[0]) / (dxu * dxd * (dxu + dxd));
		greeks[2] = (values[index_x][index_z][0] - values_p1[index_x][index_z][0]) / dt / DAYS_IN_YEAR;

		greeks[3] = (values_p1[index_x][index_z+1][0] - values_p1[index_x][index_z-1][0]) / (dzu + dzd);
		greeks[4] = 2.0 * (values_p1[index_x][index_z+1][0] * dzd + values_p1[index_x][index_z-1][0] * dzu
			- (dzu + dzd) * res[0]) / (dzu * dzd * (dzu + dzd));

		greeks[5] = ((values_p1[index_x+1][index_z+1][0] - values_p1[index_x-1][index_z+1][0]) / (dxu + dxd)
						- (values_p1[index_x+1][index_z-1][0] - values_p1[index_x-1][index_z-1][0]) / (dxu + dxd)) / (dzu + dzd);
...
Err KO_Payoff(	double		event_date,
...
double				logx, logmax, logmin, save1, save2, save3, coef;
	...
	{		
		disc_SL_strike(x, nstepx, spot_fx, bar_ex_down, bar_ex_up, 0.9999, 0.00001, stdln, NSTD_FX, bar_ex_up * 1000, 0, &index_x);	
	}

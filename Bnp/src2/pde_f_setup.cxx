...
Err pde_set_up(int dimension, int nb_payoffs, int nb_second_members,
               SrtPdeMeshType mesh_type[3], SrtPdeBoundaryCond bound_cond[3],
               SrtPdeSolvingScheme scheme, double theta, long num_points[3],
               void *info, PdeSetUpModelFunc the_pde_set_up,
               SrtPdeObject *pde) {
  Err err = NULL;
  long taillealloc;
  ...
  /* the procedure of change-of-variables and localization (space boundaries
  setting) pde->pde_set_up = the_pde_set_up; err = (*(pde->pde_set_up))(pde);
*/

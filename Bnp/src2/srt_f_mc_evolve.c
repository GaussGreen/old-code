...
/* function " monte_CHE_1f_Milshtein_evolve " describes the temporal evolution
   ...
   *step : pointer to linked list of structures which contains all preprocessed
            step (which is path-independent) utilized in the evolution
   equations. *sam : pointer to VECTOR of structure which the function fills
   with sample path of the short_rate  , v_square(called phi)  , and the
   money_market account corresponding to THIS path of the short_rate.
  ...
  if (step != NULL) {
    ...
    while (step->next != NULL) {
      ...
      */
      ...
      /* Here  , the Numeraire is the Jumping Numeraire : multiply current by
       * B(Ti  ,Ti+1) | F(Ti) */
      ...
      if (und_index ==
          sam[i + 1].numeraire_index) /* i.e. if domestic underlying */
      {
        /* Here again  , do the Numeraire calculation taking the Renormalisation
         * into account */
        sam[i + 1].numeraire =
            exp(yld -
                sam_get(tminf->fwd_sam, und_index, F_0_t) * step->delta_t) *
  ...
  if (step != NULL) {
    ...
    while (step->next != NULL) {
      ...
      /*
         ...
         ,und_index  ,STATEVAR) < 0.) sprintf(msg  ,"1+beta*x has gone too small
         %f"  , (1.0 + tminf->beta * sam_get(sam[i]  ,und_index  ,STATEVAR)));


                              if   (1.0 + tminf->beta * sam_get(sam[i]
         ,und_index  ,STATEVAR) >= 0.) sprintf(msg  ,"1+beta*x is ok %f"  , (1.0
...
/* The two following functions simulate General Cheyette evolutions  , in a
one-factor world (function 1) and two-factor world (function). They use the
local volatility functions defined in srt_f_ts_init.c and srt_f_ts_init_2f.c  ,
which return the local volatility as a function of t and r  , according to the
model. */

...
/* Initialization */
  ...
  /* The Euler step to get the next short rate */
			...
			drift = ( 
		   		...
		   		+ tminf->quanto_adjustment * tminf->ev.onef.sig	 * sam_get(sam[i],und_index,SHORT_RATE)
		  		) * step->delta_t ;

			variance = tminf->ev.onef.sig 
  ...
  /* The finite difference step to get the next cum. vol, (A.19) */
      		...
      		sam_get(sam[i+1],und_index,PHI) = sam_get(sam[i],und_index,PHI)
...
Err monte_CHE_1f_Milshtein_evolve(double *rndm_vec,  
				SrtStpPtr top, SrtSample *sam,
				int und_index)     
{
long         i;
double       drift;
	...
	if(step != NULL)
	{
		...
		{ 
			...
			if (und_index==sam[i+1].numeraire_index)
				sam[i+1].numeraire = exp( sam_get(sam[i],und_index,STATEVAR)  
											*step->delta_t)*sam[i].numeraire; /* pv_money_mkt */
...
double        alpha, beta;
	...
	/* Please note that all along this function, r(t) = f(0,t) + X1 + X2 */
	  ...
	  /* The Euler step to get the next short rate */
			...
			sam_get(sam[i+1],und_index,X1) = sam_get(sam[i],und_index,X1) 
				+ ( 
			...
			sam_get(sam[i+1],und_index,X2) = sam_get(sam[i],und_index,X2) 
			+ ( 
			...
			sam_get(sam[i+1],und_index,SHORT_RATE) =  sam_get(nxttminf->fwd_sam,und_index,F_0_t)
	       				+ sam_get(sam[i+1],und_index,X1) 
	       				+ sam_get(sam[i+1],und_index,X2);

	  /* The finite difference step to get the next cum. vol, */

			sam_get(sam[i+1],und_index,PHI1) = sam_get(sam[i],und_index,PHI1)*
			  ( 1  - 2 * tminf->ev.twof[0].lambda * step->delta_t )
					  + sam_get(sam[i],und_index,SHORT_RATE) * 
				sam_get(sam[i],und_index,SHORT_RATE) * 
					  tminf->ev.twof[0].sig2 * step->delta_t ;

			sam_get(sam[i+1],und_index,PHI2) = sam_get(sam[i],und_index,PHI2)*
			  ( 1  - 2 * tminf->ev.twof[1].lambda * step->delta_t )
					  + sam_get(sam[i],und_index,SHORT_RATE) * 
				sam_get(sam[i],und_index,SHORT_RATE) * 
					  tminf->ev.twof[1].sig2 * step->delta_t ;

			sam_get(sam[i+1],und_index,CROSSPHI) = sam_get(sam[i],und_index,CROSSPHI)*
			  ( 1  - ( tminf->ev.twof[0].lambda + 
				   tminf->ev.twof[1].lambda ) * step->delta_t )
					  +  sam_get(sam[i],und_index,SHORT_RATE) 
			* sam_get(sam[i],und_index,SHORT_RATE) 
			* tminf->ev.twof[0].sig * tminf->ev.twof[1].sig 
						* tminf->correl_x * step->delta_t ;
	  ...
	  /* The step to get 1/rolling money-market account, 
		product in (A.31)  (using renormalisation techniques)*/
			...
			if (und_index==sam[i+1].numeraire_index) /*i.e. if Domestic Underlying */
			{
				sam[i+1].numeraire = sam[i].numeraire 
				* exp( step->delta_t * ( sam_get(sam[i],und_index,X1) + sam_get(sam[i],und_index,X2) ) ) ;   
											/* pv_money_mkt */ 
			}
			i++;
			step=step->next;
  	...
  	{ 
		...
		{ 
			...
			sam_get(sam[i],und_index,STATEVAR) * nxttminf->ev.onef.F/tminf->ev.onef.F 
	 			+ nxttminf->ev.onef.F*tminf->rf.onef.G
				* (nxttminf->ev.onef.Psi - tminf->ev.onef.Psi) 
				+ nxttminf->fxquanto_adjustment
				+ nxttminf->ffdquanto_adjustment
...
long			i;
	...
	if(step != NULL)
	{
		...
		if (und_index == sam[0].numeraire_index) sam[0].numeraire = 1.0; 
		...
		{ 
			...
			sam_get(sam[i+1],und_index,SHORT_RATE)  =  sam_get(sam[i],und_index,SHORT_RATE)
			...
			if (und_index == sam[i+1].numeraire_index)
				sam[i+1].numeraire = sam[i].numeraire * exp( sam_get(sam[i],und_index,SHORT_RATE)	*step->delta_t);  
		
			sam_get(sam[i+1],und_index,BT) = sam_get(sam[i],und_index,BT)
		...
		{ 
			...
			sam_get(sam[i+1],und_index,STATEVAR)
				 =   sam_get(sam[i],und_index,STATEVAR)
				   + nxttminf->rf.onef.H - tminf->rf.onef.H
...
double        vol_sqdt;
	...
	{ 
		...
		{ 
			...
			product in (A.31)  (using renormalisation techniques)*/
			{
				sam[i+1].numeraire = exp( - sam_get(sam[i+1],und_index,STATEVAR) * nxttminf->rf.onef.H
					+ nxttminf->rf.onef.H * nxttminf->rf.onef.H * sam_get(sam[i+1],und_index,PHI));   /* pv_money_mkt */
			}
...
double        temp_var;
	...
	{ 
		...
		{ 
			...
			sam_get(sam[i+1],und_index,SHORT_RATE) = sam_get(nxttminf->fwd_sam,und_index,F_0_t) 
				-  nxttminf->ev.onef.F * ( temp_var - nxttminf->rf.onef.H * sam_get(sam[i+1],und_index,PHI)); 

			if (und_index==sam[i+1].numeraire_index) /*i.e. if Domestic Underlying */
			/* The step to get 1/rolling money-market account, 
			product in (A.31)  (using renormalisation techniques)*/
			{
				sam[i+1].numeraire = exp( - temp_var * nxttminf->rf.onef.H
					+ nxttminf->rf.onef.H * nxttminf->rf.onef.H * sam_get(sam[i+1],und_index,PHI));   /* pv_money_mkt */
			}
  	...
  	{ 
		...
		/* Here, the Numeraire is the Jumping Numeraire : multiply current by B(Ti,Ti+1) | F(Ti) */
			...
			/* Here again, do the Numeraire calculation taking the Renormalisation into account */
				sam[i+1].numeraire = exp( yld - sam_get(tminf->fwd_sam,und_index,F_0_t) 
						* step->delta_t) * sam[i].numeraire; /* pv_money_mkt */
			}  
...
/*		
			if   (1.0 + tminf->beta * sam_get(sam[i],und_index,STATEVAR) < 0.)
				sprintf(msg,"1+beta*x has gone too small %f",
					(1.0 + tminf->beta * sam_get(sam[i],und_index,STATEVAR)));
			
			
			if   (1.0 + tminf->beta * sam_get(sam[i],und_index,STATEVAR) >= 0.)
				sprintf(msg,"1+beta*x is ok %f",
					(1.0 + tminf->beta * sam_get(sam[i],und_index,STATEVAR)));
...
*/			
		...
		/* The step to get the Numeraire at next step */
			...
			/*	h_t_x = x_power_fct( sam_get(sam[i+1],und_index,STATEVAR)* nxttminf->h_t_power,
					 nxttminf->x_power );
...
int	   numeraire = 0;
	...
	/* Initialization */
		...
		{ 
			...
			volsqdt = volvar * step->sqrt_delta_t * pow(fabs(sam_get(sam[i],und_index,SHORT_RATE)),tminf->ev.onef.beta);
			...
			sam_get(sam[i+1],und_index,STATEVAR) = statevar;
      		sam_get(sam[i+1],und_index,SHORT_RATE) = statevar 
							+ sam_get(nxttminf->fwd_sam,und_index,F_0_t);
	...
	/*	sam_get(sam[i+1],und_index,SIGMA) = 
			DMIN(sam_get(sam[i+1],und_index,SIGMA),nxttminf->ev.onef.sig*
			exp(tminf->vovol*step->sqrt_delta_t*sqrt((double) i+1)-tminf->vovol_sqr*step->delta_t*(i+1)));

	*/		
	/* If domestic underlying, accrue the pv money market as numeraire */
      		if (numeraire) 		
			{
         		sam[i+1].numeraire = exp( statevar * step->delta_t)
									 *sam[i].numeraire; 
			}
...
SrtLogTmInf  *tminf, *nxttminf;
	...
	/* Loop on all the time steps */
		...
		{ 
			nxttminf = step->next->tminf[und_index];
			sam_get(sam[i+1],und_index,SPOT) = sam_get(sam[i],und_index,SPOT)*( 1.0 + tminf->drift)  
					+ (rndm_vec[i+1]+tminf->quanto_adjustment)*tminf->sqrt_int_sig2_dt;
...
SrtLogTmInf    *tminf,*nxttminf;
	...
	if(step !=NULL)
	{
		...
		{
			nxttminf = step->next->tminf[und_index];
			sam_get(sam[i+1],und_index,SPOT) = sam_get(sam[i],und_index,SPOT)*
				nxttminf->init_fwd_val / tminf->init_fwd_val *
							   ...
							   int				dom_index)
{
	...
	if(step != NULL)
	{
		...
		while(step->next != NULL)
		{
			...
			g[1] = tminf->rho*rndm_vec_spot[i+1] + sqrt(1-tminf->rho*tminf->rho)*rndm_vec_vol[i+1];
			...
			sam_get(sam[i+1],und_index,SIGMA) = sam_get(sam[i],und_index,SIGMA)
												+ (nxttminf->basevol-tminf->basevol) 
												+ voldrift*step->delta_t 
												+ vovol*g[1]*step->sqrt_delta_t;	

			sqrt_int_sig2_dt = sam_get(sam[i+1],und_index,SIGMA)*(1 + beta*stdev +  gamma*stdev*stdev)*sqrt(step->delta_t);
...
Err monte_FX_STOCH_RATES_Jumping_evolve(double       *rndm_vect,
										SrtStpPtr	 top,
										SrtSample	 *sam,
										int			 und_index,
										int			 dom_index,
	...
	beta = tminf->ev.onef.beta;
  	...
  	{ 
    	...
    	{ 
			...
			volsqdt = tminf->ev.onef.sig * step->sqrt_delta_t * pow(fabs(sam_get(sam[i],und_index,SHORT_RATE)),tminf->ev.onef.beta);
...
Err			err;
  	...
  	{ 
    	...
    	sam_get(sam[0],und_index,X1) = sam_get(sam[0],und_index,X2) = 0.0;
    	sam_get(sam[0],und_index,SHORT_RATE) = sam_get(tminf->fwd_sam,und_index,F_0_t);
		sam_get(sam[0],und_index,PHI1) 
			= sam_get(sam[0],und_index,PHI2) 
			= sam_get(sam[0],und_index,CROSSPHI) 
			= 0.0;
    	if (und_index == sam[0].numeraire_index)
		{ 
			sam[0].numeraire = 1.0;   
		...
		/* Evolve the ith Sample and store the result in the next one */ 
      	...
      	/* Update the Numeraire (the Bank account unit)(minus the forward rate...)  */
			...
			{
         		sam[i+1].numeraire	
					=	exp ((sam_get(sam[i],und_index,X1)+sam_get(sam[i],und_index,X2)) * step->delta_t)
					*	sam[i].numeraire; 
			}

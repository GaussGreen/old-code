...
static Err populate_knot_point(SrtBndCrv *crv, int *numknot, int *num_coupons,
                               Ddate **coupon_dates, int numbond, Ddate today) {
  long modified_end;
  double times;
  int i, j;
  int found; /* boolean */
  ...
  for (i = 0; i < numbond; i++) {
    ...
    modified_end = bus_date_method((Date)coupon_dates[i][num_coupons[i] - 1],
                                   MODIFIED_SUCCEEDING);
    ...
    if (!found) {
      ...
      (crv->knot_date) =
          (double *)realloc((crv->knot_date), (*numknot + 1) * sizeof(double));
  ...
  /* Order it by date */
  qsort((void *)(&crv->knot_point[1]), *numknot, sizeof(double),
        compare_double);
 ...
 * subtracted
 */
static double fwdclnprc(SrtBndCrv *crv, Ddate prev_cpn_date, Ddate *cpn_date,
                        int num_cpn_date, double *cpn, double redemption,
                        SrtBasisCode bc, SrtCompounding compd) {
...
static void reset_zcpn_yld_coefs(SrtBndCrv *crv) {
  ...
  if (global_algo_flag == 2) {
    /*
     * choose the next to last coefficient (b_lastknot+2) to force f'(0
     *       ,k_lastknot+1) = 0
     */
    ...
    tmp = (-crv->fwd_rate_coef[1] -
           2.0 * crv->fwd_rate_coef[2] * crv->knot_point[crv->lastknot + 1] -
           2.0 * tmp) /
          (2.0 * (crv->knot_point[crv->lastknot + 1] -
                  crv->knot_point[crv->lastknot]));

    crv->fwd_rate_coef[crv->lastknot + 2] = tmp;

    /*
     * between knot points        , -log(df(0        ,t)) is a cubic; coefs are
     * here we say crv->zcpn_yld_coef[i][0..3] is the third order polynomial
  ...
  /**********/
    ...
    */
    /*
     * between knot points        , -log(df(0        ,t)) is a second order
     * polynomial; coefs are here we say crv->zcpn_yld_coef[i][0..3] is the
     * third order polynomial (index 3 is the highest order coefficient) between
     * knot point i and knot point i+1.  Here we consider the zeroth knot point
     * to always have value zero.
     */
...
static void value_jth_bond(int j, double *answer) {
  *answer =
      fwdclnprc(global_crv, global_prev_cpn_date[j], global_coupon_dates[j],
                global_num_coupons[j], global_coupons[j], global_redemption[j],
                global_basis[j], global_compounding[j]);
}
    ...
    int algo_flag) /* 0 = CONSTANT        , 1 = LINEAR        , 2 = SPLINE */
{
  ...
  /*
   * allocate memory for the curve and initialise the different arrays in the
   * curve
   */
  ...
  if (knot_point == NULL) {
    ...
    if (populate_knot_point(crv, &numknot, num_coupons, coupon_dates, numbond,
                            today) != NULL) {
      srt_free(crv);
      return serror("alloc failure in srt_f_bndcrv_init");
    }
  } else {
    /* we have to add one point if the lastdate is superior to the last knot
     * date */
    knot_date_last =
        (Date)add_unit(DTOL(crv->spot), DTOL(12 * knot_point[numknot - 1]),
    ...
    for (i = 1; i <= numknot - 1; i++) {
      crv->knot_date[i] =
          (Ddate)add_unit(DTOL(crv->spot), DTOL(12 * knot_point[i - 1]),
                          SRT_MONTH, MODIFIED_SUCCEEDING);
    ...
    if (lastdate > knot_point_last) {
      ...
      knot_point_last = (double)(knot_date_last - today) * YEARS_IN_DAY;
      crv->knot_date[numknot] =
          (Ddate)add_unit(DTOL(crv->spot), DTOL(12 * knot_point_last),
                          SRT_MONTH, MODIFIED_SUCCEEDING);
    } else {
      crv->knot_date[numknot] =
          (Ddate)add_unit(DTOL(crv->spot), DTOL(12 * knot_point[numknot]),
                          SRT_MONTH, MODIFIED_SUCCEEDING);
    }

    crv->knot_point[numknot] =
        (double)(crv->knot_date[numknot] - today) * YEARS_IN_DAY;

    /* Order it by date */
    qsort((void *)(&crv->knot_point[1]), numknot, sizeof(double),
          compare_double);
    qsort((void *)(&crv->knot_date[1]), numknot, sizeof(double),
          compare_double);
  }
  ...
  /*
   * put knot points in bond curve        , initialize coefficients we are
   * optimizing over        , (these are the coefficients of the quadratic
   * forward rate spline) and calculate the corresponding coefficients of the
   * cubic equation this implies for the zero coupon yields.
   */
  ...
  /************************/
  if (err = levenberg_marquardt_select(x, y, sig, PRICE_NUMB, p, ia, PARA_NUMB,
                                       num_iter, funcs, &chisq)) {
    free_dmatrix(crv->zcpn_yld_coef, 0, numknot, 0, 3);
    free_dvector(crv->fwd_rate_coef, 0, numknot + 2);
    free_dvector(crv->knot_date, 1, numknot);
    free_dvector(crv->knot_point, 1, numknot);
...
SrtErr srt_f_bndcrv_fwdr(SrtBndCrv *crv, Ddate d, double *answer) {
  ...
  fr = crv->fwd_rate_coef[0] +
       t * (crv->fwd_rate_coef[1] + t * crv->fwd_rate_coef[2]);

  /*
   * find index of last knot point >= t
   */
  for (i = 1; i <= crv->num_knot_point && crv->knot_point[i] < t; i++) {
    fr += pow((t - crv->knot_point[i]), crv->algo_flag) *
          crv->fwd_rate_coef[i + 2];
  }
...
SrtErr srt_f_bndcrv_level(SrtBndCrv *crv, SwapDP *sdp, double *answer) {
  ...
  for (i = 0; i < dl.len - 1; i++) {
    if ((err = srt_f_bndcrv_df((void *)crv, dl.date[i], dl.date[i + 1], &df)) !=
        NULL)
      return err;
...
*/
SrtErr srt_f_bndcrv_zr(SrtBndCrv *crv, Ddate start, Ddate end,
...
/*
  fwd par bond yield
*/
SrtErr srt_f_bndcrv_fwdtr(SrtBndCrv *crv, Date s, Date e_nfp, char *f, char *b,
                          double *answer) {
  SrtRtFnc rt;
  SrtErr err;
  int i, len;
...
/*
   fwd bond price
*/
SrtErr srt_f_bndcrv_fwdpr(SrtBndCrv *crv, SwapDP *sdp, double cpn,
                          double redemption, double *answer) {
  int i;
  DateList dl;
  double *cpn_array;
  Ddate *date_array;
  ...
  *answer = fwdclnprc(crv, dl.prev, date_array, dl.len, cpn_array, redemption,
                      sdp->basis_code, sdp->compd);
...
/*
   fwd bond yield
*/
SrtErr srt_f_bndcrv_fwdyld(SrtBndCrv *crv, SwapDP *sdp, double cpn,
                           double redemption, double *answer) {
  SrtErr err;
...
/*
  find out lastdate
*/
Date srt_f_bndcrv_lastdate(SrtBndCrv *crv) { return (Date)DTOL(crv->lastdate); }

/*
  find out today
*/
Date srt_f_bndcrv_today(SrtBndCrv *crv) { return (Date)DTOL(crv->today); }

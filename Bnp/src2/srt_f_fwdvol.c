/* =========================================================================================
	...
	PURPOSE					COMPUTE FORWARD VOLATILITY, EITHER THROUGH AUTOCALIBRATION,
							OR PARABOLAS
   ========================================================================================= 
...
static void free_calib_instr(long		*cal_start,
						long		*cal_end,
						double  	*cal_str,
						double		*cal_bndstr,
						String		*cal_type,
						...
						int			fix_tau_flag)
{
	if(cal_start)free_lngvector (cal_start, 0, cal_numinst-1);
		cal_start = NULL;
	if(cal_end) free_lngvector (cal_end, 0, cal_numinst-1);
		cal_end = NULL;
	if(cal_str)	free_dvector (cal_str, 0, cal_numinst-1);
		cal_str = NULL;
	if(cal_bndstr) free_dvector (cal_bndstr, 0, cal_numinst-1);
		cal_bndstr = NULL;
	if(cal_type) free_svector_size (cal_type, 0, cal_numinst-1, 32);
		cal_type = NULL;
	if(cal_recpay)free_svector_size (cal_recpay, 0, cal_numinst-1, 32);
		cal_recpay = NULL;
	if(cal_refrate) free_svector_size (cal_refrate, 0, cal_numinst-1, 32);
		cal_refrate = NULL;
	if(cal_price) free_dvector (cal_price, 0, cal_numinst-1);
		cal_price = NULL;
	if(cal_vega) free_dvector (cal_vega, 0, cal_numinst-1);
		cal_vega = NULL;
	if(cal_freq) free(cal_freq);
	if(cal_basis) free(cal_basis);
}

static void free_option_computation(Date *und_end_th,
									double *premium_array)
{
	if (und_end_th) free(und_end_th);
	if (premium_array) free(premium_array);
}
...
#define	FREE_GRFN_CALIB_PARAM {													\
	...
	grfn_value = NULL; \
	if (calib_param) free_svector_size (calib_param, 0, num_calib_param-1, 32);	\
	calib_param = NULL; \
	if (calib_value) free_svector_size (calib_value, 0, num_calib_param-1, 32); \
	calib_value = NULL;}
...
Err srt_f_fwdvol_autocal
	...
	double					beta,
	double					rho,
	double					chey_beta,			/* Chey Beta parameter */
	...
	char					*und_tenor,
	SrtCompounding			compd,				/* Und compd */
	...
	SrtPriceType			price_type,
	double					*output_array,	/* Output array */
...
double					*cal_str = NULL, *cal_bndstr = NULL, *cal_price = NULL, *cal_vega = NULL;
String					*grfn_param = NULL, *grfn_value = NULL, *calib_param = NULL, *calib_value = NULL;
...
/*	Model */
	...
	if (err)
	{
		return serror ("Error in srt_f_fwdvol_autocal (srt_f_interp_model): %s", err);
	}
	if ((mdl_dim == TWO_FAC) && (fabs (alpha) < EPS || fabs (beta) < EPS || fabs (rho) < EPS))
	{
		return serror ("Error in srt_f_fwdvol_autocal: two factor model requires alpha, beta and rho");
	}
	if ((mdl_type == CHEY_BETA) && (fabs (chey_beta) < EPS))
	{
		return serror ("Error in srt_f_fwdvol_autocal: CHEY_BETA model requires chey beta parameter");
	}
...
/*	Lognormal / Normal */
	...
	if (err)
	{
		return serror ("Error in srt_f_fwdvol_autocal (interp_diffusion_type): %s", err);
	}
...
/*	Sigma curve */
	if (mdl_dim == ONE_FAC)
	{
		...
		if (!sig_date_value)
		{
			return serror ("Error in srt_f_fwdvol_autocal (sig_date_value): memory allocation");
		}
...
/*	Tau curve */
	if (mdl_dim == ONE_FAC)
	{
		...
		if (!tau_date_value)
		{
			...
			return serror ("Error in srt_f_fwdvol_autocal (tau_date_value): memory allocation");
		}
		for (i=0; i<num_tau; i++)
		{
...
/*	---------- Set calibration instruments ---------- */
	...
	if (err)
	{
		...
		return serror ("Error in srt_f_fwdvol_autocal (fwd_vol_set_calibration_instruments): %s", err);
	}		
...
/*	---------- Set grfn & calib options ---------- */
	...
	/* ------- calib ------- */
	fwd_vol_set_calib_options (mdl_type,option_type,fix_tau_flag, &calib_param, &calib_value, &num_calib_param);

	if (err = srt_f_set_GrfnParams (num_grfn_param, grfn_param, grfn_value, &grfnparam) )
	{
		free_calib_instr(cal_start,
						cal_end,
						cal_str,
...
/*	---------- Calibrate ---------- */
	...
	if (err)
	{
		return serror ("Error in srt_f_fwdvol_autocal (srt_f_CalibrateAll): %s", err);
	}
	if (chisq > 0.25)
	{
		srt_f_destroy_und (und_id);
		return serror ("Error in srt_f_fwdvol_autocal (srt_f_CalibrateAll): calibration failed");
	}
	...
	if(option_type==RESETCAPFLOOR)
	{
		...
		/*	Forward Reset Caplet is the following profile:
			At T4, pay: MAX (F(T2,T3,T4) - F(T1,T3,T4), 0) * cvg (T3, T4)
			Where	T1 = vol start date
					T2 = vol end date
					T3 = fra start date
					T4 = fra end date */
		...
		{
			und_end_th[i] = add_unit (DTOL (und_start_act[i]), 
									  12/(int)compd, SRT_MONTH, 								  
			...
			if (err)
			{
				free_option_computation(und_end_th,premium_array);
				srt_f_destroy_und (und_id);
				return serror ("Error in srt_f_fwdvol_autocal (srt_f_fwd_resetcaplet): %s", err);
			}

			err = swp_f_LevelPayment (DTOL (und_start_act[i]),
									  DTOL (und_end_th[i]), 
									  compdStr, 
									  basisStr, 
									  yc_id, 
			...
			if (err)
			{
				free_option_computation(und_end_th,premium_array);
				srt_f_destroy_und (und_id);
				return serror ("Error in srt_f_fwdvol_autocal (srt_f_ForwardRate): %s", err);
			}
			...
			if (err)
			{
				free_option_computation(und_end_th,premium_array);
				srt_f_destroy_und (und_id);
				return serror ("Error in srt_f_fwdvol_autocal (srt_f_optimpvol): %s", err);
			}
		}
	...
	else if(option_type==RESETCMSOPTION)
	{
		...
		for (i=0; i<num_period; i++)
		{
			...
			add_tenor(DTOL (und_start_act[i]),
							und_tenor,
							NO_BUSDAY_CONVENTION,
							&und_end_th[i]);

			err = srt_f_fwd_resetcms(undptr,
									&grfnparam,
									(long)vol_start[i],
									(long)vol_end[i],
									(long)und_start_act[i],
									(long)und_end_th[i],
									basisStr,
									compdStr,
									ref_rate, 
			...
			if (err)
			{
				free_option_computation(und_end_th,premium_array);
				srt_f_destroy_und (und_id);
				return serror ("Error in srt_f_fwdvol_autocal (srt_f_fwd_resetcms): %s", err);
			}
			
		
			SwpSpread = swp_f_spread(und_start_act[i], und_end_th[i],ref_rate);
			err = GetVol(und_start_act[i],und_end_th[i],und_val[i],und_val[i],SwpSpread,&cmsvol);	 
			...
			if(price_type==SRT_VOLATILITY)
			{
				...
				premium = output_array[i];
				err = srt_f_optimpvol(premium,
									und_val[i],
									und_val[i],
									(vol_end[i] - vol_start[i]) * YEARS_IN_DAY,
									swp_f_df(clcn_date, (long)und_start_act[i], yc_name),
									SRT_CALL, 
									srt_fwd_vol_type, 
									&output_array[i]);
			}
...
/* Function to compute forward volatility through parabolas for normal model on one period */
...
*				  parabolic profiles are priced using (CMS) caplets
*				  caplets are approximated by calls on CMS rate 
*				  caplets are priced with normal or lognormal smile
*                                                                         
* AUTHOR		: Antoine Savine
*                                                                         
* PARAMETERS  	: today 			- today (in days)
*				: vol_start		    - start date (in days)
*				: vol_end			- end date (in days)     	  
*				: rate_tenor		- maturity of the desired rate
*				: fwd_cms_start
*				  fwd_cms_end 		- forward rates at for start and end dates CMS adjusted 
*				: level_slope_correl
*				: slope_vol			- correl level/slope and vol of the slope used for adj
*				: GetVolTenor	    - function that enables to retrieve implied vols
*               : vol_id     		- ivol curve identification
...
Err srt_f_spfwdnormvol(
			...
			double         fwd_cms_start, 
			double         fwd_cms_end,
			double         level_slope_correl, 
			double         slope_vol,
			Err		       (*GetVol)	(Ddate, 
										 Ddate, 
										 double,
...
/*	Main function to compute the forwad volatility for a string of period in a normal model  */
Err srt_f_fwdvol_parabola
	(double					level_slope_correl,	/* Parameters */
	double					slope_vol,
	long					num_period,			/* Number of required forward vols */
...
double				start_rate_st, end_rate_st, atm_vol_st, cms_st, 
					start_rate_nd, end_rate_nd, atm_vol_nd, cms_nd;
long				i;
	...
	/* Get the details of the reference rate: compounding (in value and string) and basis */
	err = swp_f_get_ref_rate_details(ref_rate, &basis, &compd);
	if (err)
	{
		return err;
	...
	if (err)
	{
		return serror ("Volatility start dates should be between 01-Jan-1983 and 01-Jan-2070");
	}

	err = srt_f_test_date_dblarr ((double *) vol_end, (int) num_period);
	if (err)
	{
		return serror ("Volatility end dates should be between 01-Jan-1983 and 01-Jan-2070");
	}
...
/* Compute the fwd volatility for each period */
	for (i=0; i<num_period; i++)
	{
		...
		start_rate_st = add_unit ((Date)vol_start[i], spot_lag, SRT_BDAY, MODIFIED_SUCCEEDING);
		end_rate_st = add_unit ((Date)start_rate_st, 12/(int)compd, SRT_MONTH, NO_BUSDAY_CONVENTION);
		...
		start_rate_nd = add_unit ((Date)vol_end[i], spot_lag, SRT_BDAY, MODIFIED_SUCCEEDING);
		end_rate_nd = add_unit ((Date)start_rate_nd, 12/(int)compd, SRT_MONTH, NO_BUSDAY_CONVENTION);

/* =========================================================================================
        ...
        PURPOSE					COMPUTE FORWARD VOLATILITY, EITHER THROUGH
   AUTOCALIBRATION, OR PARABOLAS
   =========================================================================================
    ...
    String       fwd_vol_type)       /* Lognormal or normal */
{
    ...
    {
        return serror(
            "Error in srt_f_fwdvol_autocal: two factor model requires alpha, beta and rho");
    }
    if ((mdl_type == CHEY_BETA) && (fabs(chey_beta) < EPS))
    {
        return serror(
            "Error in srt_f_fwdvol_autocal: CHEY_BETA model requires chey beta parameter");
    }
    ...
    if (err)
    {
        ...
        return serror(
            "Error in srt_f_fwdvol_autocal (fwd_vol_set_calibration_instruments): %s", err);
    }
    ...
    /* ------- calib ------- */
    fwd_vol_set_calib_options(
        mdl_type, option_type, fix_tau_flag, &calib_param, &calib_value, &num_calib_param);
    ...
    if (option_type == RESETCAPFLOOR)
    {
        ...
        {
            und_end_th[i] =
                add_unit(DTOL(und_start_act[i]), 12 / (int)compd, SRT_MONTH, NO_BUSDAY_CONVENTION);
    ...
    else if (option_type == RESETCMSOPTION)
    {
        ...
        for (i = 0; i < num_period; i++)
        {
            ...
            add_tenor(DTOL(und_start_act[i]), und_tenor, NO_BUSDAY_CONVENTION, &und_end_th[i]);
            ...
            err =
                GetVol(und_start_act[i], und_end_th[i], und_val[i], und_val[i], SwpSpread, &cmsvol);
 ...
 *				: fwd_cms_start
 *				  fwd_cms_end 		- forward rates at for start and end dates CMS
 *adjusted : level_slope_correl : slope_vol			- correl level/slope and vol of the
 *slope used for adj : GetVolTenor	    - function that enables to retrieve implied vols :
 *vol_id     		- ivol curve identification : vol_type			- "normal" or
 *"lognormal"
 *
    ...
    long    nvol)
{
    ...
    *fwd_vol = pc2 + pp2 - pc1 - pp1;
    ...
    adjustment =
        level_slope_correl * slope_vol * sqrt(YEARS_IN_DAY * (vol_start - today) * 4 * (pc1 + pp1));
    ...
    long    nvol)
{
    ...
    double start_rate_st, end_rate_st, atm_vol_st, cms_st, start_rate_nd, end_rate_nd, atm_vol_nd,
        cms_nd;
    long i;
    ...
    for (i = 0; i < num_period; i++)
    {
        ...
        end_rate_st =
            add_unit((Date)start_rate_st, 12 / (int)compd, SRT_MONTH, NO_BUSDAY_CONVENTION);
        ...
        end_rate_nd =
            add_unit((Date)start_rate_nd, 12 / (int)compd, SRT_MONTH, NO_BUSDAY_CONVENTION);

...
static Err num_f_optblkvoltobetavol(double forward, double strike, double bsvol,
  ...
  space_correction = (1.0 - beta) * (2.0 + beta) / 24.0 * (distance / middle) *
                     (distance / middle);
  ...
  *betavol = bsvol * pow(middle, 1.0 - beta) *
             (1.0 - (space_correction + time_correction) /
                        (1.0 + space_correction + 3.0 * time_correction));
...
static Err rtsafe_func(double x, double *fx, double *dfx) {
  ...
  err = srt_f_optbetavoltoblkvol(static_forward, static_strike, x, static_mat,
                                 static_beta, &vol_tmp);

  if (err)
    return err;

  err = srt_f_optbetavoltoblkvol(static_forward, static_strike, x + dx,
                                 static_mat, static_beta, &vol_tmp2);
...
Err srt_f_optblkvoltobetavol(double forward, double strike, double bsvol,
  ...
  err = num_f_optblkvoltobetavol(forward, strike, bsvol, mat, beta, &guess_vol);
  if (err)
    return err;

  err =
      srt_f_optbetavoltoblkvol(forward, strike, guess_vol, mat, beta, &vol_log);
  ...
  while ((setup == 0) && (niter <= 5)) {
    if ((bsvol - vol_bump <= 0.0) && (direc < 0.0))
      vol_bump = 0.9 * bsvol;

    err = num_f_optblkvoltobetavol(forward, strike, bsvol + direc * vol_bump,
                                   mat, beta, &guess_vol_down);
    if (err)
      return err;

    err = srt_f_optbetavoltoblkvol(forward, strike, guess_vol_down, mat, beta,
                                   &vol_log_down);
  ...
  err =
      rtsafe(rtsafe_func, guess_vol_down, guess_vol_up, stop_eps, 10, betavol);
  if (err)
    return err;
...
static Err rtsafe_func2(double x, double *fx, double *dfx) {
  ...
  err = srt_f_optbetastochvoltoblkvol(static_forward, static_strike, x,
                                      static_alpha, static_rho, static_mat,
                                      static_beta, &vol_tmp);

  if (err)
    return err;

  err = srt_f_optbetastochvoltoblkvol(static_forward, static_strike, x + dx,
                                      static_alpha, static_rho, static_mat,
                                      static_beta, &vol_tmp2);
...
Err srt_f_optblkvolATMtobetavolStochVol(double forward, double strike,
  ...
  err = srt_f_optbetastochvoltoblkvol(forward, strike, guess_vol, alpha, rho,
                                      mat, beta, &vol_log);
  ...
  while ((setup == 0) && (niter <= 10)) {
    ...
    err = srt_f_optbetastochvoltoblkvol(forward, strike, guess_vol_down, alpha,
                                        rho, mat, beta, &vol_log_down);
  ...
  err =
      rtsafe(rtsafe_func2, guess_vol_down, guess_vol_up, stop_eps, 40, betavol);
  if (err)
    return err;

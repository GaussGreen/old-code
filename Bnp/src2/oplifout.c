	...
	for (i = -NI_nterms;i<0;i++)
 	{
		...
		d_alpha = (NI_b - 2 * ( NI_a - i * NI_d ) - NI_mu * mat) /
					( NI_vol * sqrt(mat));

		d_beta = (NI_b + 2 * i * NI_d  - NI_mu * mat) /
					( NI_vol * sqrt(mat));
...
if (((spot > b_out) && (b_out > b_life)) ||
	((spot < b_out) && (b_out < b_life)) )
{
	premium = 0.0;
}
else
if (((spot > b_life) && (b_life > b_out)) ||
...
if (fabs(spot-b_life)/spot < 1.0e-04) 
{
	premium = mat*disc;
}
else
...
switch(greek)
{
	...
	case DELTA_FWD:
		shift = fwd/ 10000;
		answer = ( srt_f_optlifout(	fwd+shift,
									spot,
									b_life,
									b_out,
									vol,
	...
	case DELTA:
		shift = spot/ 10000;
		answer = ( srt_f_optlifout(	fwd * (1 + shift/spot),
									spot + shift,
									b_life,
									b_out,
									vol,
	...
	case GAMMA:
		...
		answer = ( srt_f_optlifout(	fwd * ( 1 + shift/spot),
									...
									nterms,
									PREMIUM)
				+ srt_f_optlifout(	fwd * ( 1 - shift/spot),
									spot-shift,
									b_life,
									b_out,
									vol,

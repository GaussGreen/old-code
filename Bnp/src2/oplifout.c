...
static double life_out_time_prob_function(double mat)
{
    ...
    for (i = -NI_nterms; i < 0; i++)
    {
        ...
        d_alpha = (NI_b - 2 * (NI_a - i * NI_d) - NI_mu * mat) / (NI_vol * sqrt(mat));
    ...
    SrtGreekType greek)
{
    ...
    else if (((spot > b_out) && (b_out > b_life)) || ((spot < b_out) && (b_out < b_life)))
    {
        premium = 0.0;
    }
    else if (((spot > b_life) && (b_life > b_out)) || ((spot < b_life) && (b_life < b_out)))
    {
        premium = mat * disc;
    }
    ...
    case DELTA_FWD:
        shift = fwd / 10000;
        answer =
            (srt_f_optlifout(fwd + shift, spot, b_life, b_out, vol, mat, disc, nterms, PREMIUM) -
             premium) /
            shift;
        break;
    ...
    case VEGA:
        shift = GVOPT.vol_add;
        answer =
            (srt_f_optlifout(fwd, spot, b_life, b_out, vol + shift, mat, disc, nterms, PREMIUM) -
             premium) /
            shift;
        break;

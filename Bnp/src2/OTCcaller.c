...
Err otc_caller(
    ...
    int     use_GMA,   //	0: Does not output the multicallable price | 1: GMA1 | 1: GMA2 | 3: GMA1 &
                       //GMA2
    /*	Numerical params */
    ...
    double** Results)
{
    ...
    {
        err = serror(
            "Exercised flag is temporarily not supported in case of option string coupon or "
            "redemption");
        goto FREE_RETURN;
    }
    ...
    time2 = clock();
    smessage(
        "Phase 0 -Precalculations, time in sec: %.2f", (double)(time2 - time1) / CLOCKS_PER_SEC);
    ...
    double* Results)
{
    ...
    forward =
        spot_fx * swp_f_df(spot_date, tstarDate, for_yc) / swp_f_df(spot_date, tstarDate, dom_yc);
    ...
    Payoff
    ================================== */
    forward = spot_fx * swp_f_df(spot_date, valDate, for_yc) / swp_f_df(spot_date, valDate, dom_yc);
    ...
    double**         Results)
{
    ...
    /* =====================
            GMA
    ==================== */
    double *dExeTimes = NULL, *dForward = NULL, *dLongOption = NULL, *dShortOption = NULL;
    ...
    OTC_params->COPULAmatrix =
        dmatrix(0, OTC_params->COPULAnSimul - 1, 0, 3 - 1 + OTC_params->OTCsmileAndFlat);
    ...
    time2 = clock();
    smessage(
        "Phase 2 -PECS adjustment, time in sec: %.2f", (double)(time2 - time1) / CLOCKS_PER_SEC);
    ...
    for (otc = otcStart; otc <= otcEnd; otc++)  // TO do Check if the call has been erased
    {
        if (!erasing_call_done || (erasing_call_done && !erased_call_list[otc]))
        {
            ...
            if (fabs(cpd->call[otc].ex_time) > 1.0e-10)
            {
                if (OTC_params->OTCsmile || OTC_params->OTCsmileAndFlat)
                {
                    ...
                    err = OTCcalibSmileModel(
                        cpd->call[otc].ex_time, /* Maturity in years */
                        forward,                /* Forward */
                        OTC_params,
                        smile_params,
                        &error);
                    ...
                    time2 = clock();
                    smessage(
                        "Phase 3 -Simulation Finished, time in sec: %.2f",
                ...
                if (!OTC_params->OTCsmile || OTC_params->OTCsmileAndFlat)
                {
                    ...
                    for (i = 0; i < OTC_params->COPULAnSimul; i++)
                    {
                        OTC_params->COPULAmatrix[i][k] =
                            forward * exp(-0.5 * smile_params->sigma * smile_params->sigma *
                                              cpd->call[otc].ex_time +
            ...
            else if (OTC_params->OTCpayoff == 1234)
            {
                ...
                {
                    Results[j][0] =
                        forward * exp(OTC_params->CUMULnStd * ((double)j - 4) / 9.0 *
            ...
            time2 = clock();
            smessage(
                "Phase 4 -Payoff evaluation Finished, time in sec: %.2f",
                (double)(time2 - time1) / CLOCKS_PER_SEC);
        }
    }
    ...
    if (OTC_params->OTCpayoff == 4 || OTC_params->OTCpayoff == 6)
    {
        ...
        if (!calib_params || !pde_params || !autocal_params || !sModel || !sInfos || !dExeTimes ||
            !dForward || !dLongOption || !dShortOption)
        {
    ...
    if (OTC_params->OTCpayoff == 4)
    {
        ...
        err = genmidat_allocate_autocalinfos(cpd->num_calls - k, autocal_params, sInfos);
    ...
    double** Results)
{
    ...
    err = balsam_generation(OTC_params->COPULAnSimul, cpd->num_calls + 2, RealisationMatrix);
    ...
    if (OTC_params->COPULAdo_pecs)
    {
        adjust_emp_covar(RealisationMatrix, OTC_params->COPULAnSimul, cpd->num_calls + 2);
    }
    ...
    if (RealisationMatrix)
        free_dmatrix(RealisationMatrix, 0, OTC_params->COPULAnSimul - 1, 0, 2 + cpd->num_calls - 1);
    ...
    */
    ...
    for (otc = 0; otc < cpd->num_calls; otc++)
    {
        ...
        err = OTCutilsCorrelateFxAndRates(
            OTC_params, 0.0, stdDom, meanFor, stdFor, cumulDomForCor, cumulDomFxCor, cumulForFxCor);
    ...
    if (OTC_params->KO_do_optim)
    {
        ...
        time2 = clock();
        smessage(
            "Phase 5 -Optimal Barrier found, time in sec: %.2f",
            (double)(time2 - time1) / CLOCKS_PER_SEC);
        time1 = clock();
        ...
        time2 = clock();
        smessage(
            "Phase 6 -Payoff evaluation Finished, time in sec: %.2f",
            (double)(time2 - time1) / CLOCKS_PER_SEC);
    }
...
FREE_RETURN:
    ...
    if (save_values)
        free_f3tensor(save_values, 0, cpd->num_calls, 0, 1, 0, OTC_params->COPULAnSimul - 1);

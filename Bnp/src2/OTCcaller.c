...
{
	err = serror("Exercised flag is temporarily not supported in case of option string coupon or redemption");
	goto FREE_RETURN;
}
	...
	err = cpd_fill_check_all_struct(	today,
										use_calib,
										fx_spot,
										fx_spot_date,
										dom_calib,
	...
	if(err) goto FREE_RETURN;
	time2 = clock();
	smessage ("Phase 0 -Precalculations, time in sec: %.2f", (double) (time2 - time1) / CLOCKS_PER_SEC);
...
FREE_RETURN:
	...
	if(pd_num_strikes) free(pd_num_strikes);
	if(pd_wcst) free(pd_wcst);
	if(pd_wspot) free(pd_wspot);
	...
	if(pd_not_num_strikes) free(pd_not_num_strikes);
	if(pd_not_wcst) free(pd_not_wcst);
	if(pd_not_wspot) free(pd_not_wspot);
	...
	if(OTC_precalc)	
	{
		cpd_free_otc_precalc(OTC_precalc, cpd->num_calls,cpd->pd_leg->num_cpn, cpd->fund_leg->num_cpn);
		free(OTC_precalc);
	}
...
Err err = NULL;
	...
	{
		err = fwdSABRpayoffAlphaFudge(	today,
										forwardFixDate,
										forwardFixTime,
										valDate,
										fixTime,
...
FREE_RETURN:
	if(matrix)		free_dmatrix (matrix, 0, nSimul - 1, 0, 3 - 1);
	if(mergeTimes) free (mergeTimes);
	if(sigDom) free(sigDom);
	if(sigFor) free(sigFor); 
	if(sigFx) free(sigFx); 
...
Err				err				= NULL;
	...
	OTC_params->COPULAmatrix = dmatrix (0, OTC_params->COPULAnSimul-1, 0, 3 - 1 + OTC_params->OTCsmileAndFlat);
	...
	if(!OTC_params->COPULAmatrix || !OTC_params->COPULAgauss || !OTC_params->CUMUL)
	{
	...
	err = balsam_generation (	OTC_params->COPULAnSimul, 
								3,
	...
	{
		adjust_emp_covar (	OTC_params->COPULAgauss,
	...
	================================== */
	if( OTC_params->OTCpayoff == 4 || OTC_params->OTCpayoff == 6 || OTC_params->OTCpayoff == 123)
	{
		otcStart = 0;
		otcEnd = cpd->num_calls - 1;
	}

	for(otc = otcStart; otc <= otcEnd; otc++) // TO do Check if the call has been erased
	{
		if(!erasing_call_done || (erasing_call_done && !erased_call_list[otc]))
		{
			...
			err = OTCgetMoments(	cpd->call[otc].ex_time, und->lda_dom, und->lda_for, tstarTime,
									mergeTimes, mergeNtimes,
									sigDom, sigFor, sigFx,
									corDF, corDFx, corFFx,
									&stdDom, &meanFor, &stdFor,
			...
			if(fabs(cpd->call[otc].ex_time) > 1.0e-10)
			{
				if(OTC_params->OTCsmile || OTC_params->OTCsmileAndFlat)
				{
					...
					time2 = clock();
					smessage ("Phase 3 -Simulation Finished, time in sec: %.2f", (double) (time2 - time1) / CLOCKS_PER_SEC);
					time1 = clock();
				}
				...
				if(!OTC_params->OTCsmile || OTC_params->OTCsmileAndFlat)
				{
					...
					for(i = 0; i < OTC_params->COPULAnSimul; i++)
					{
						OTC_params->COPULAmatrix[i][k] = forward
							* exp( - 0.5 * smile_params->sigma * smile_params->sigma * cpd->call[otc].ex_time
									+ OTC_params->COPULAmatrix[i][k] * smile_params->sigma * sqrt(cpd->call[otc].ex_time));
					}
			...
			{
				err = OTCfwdSABRCalib(	cpd,
										und,
										OTC_precalc->pd_fwdsmile_vol[OTC_params->OTC],
										...
										tstarTime,
										tstarDate,
										OTC_params->OTCfwdSmileVisuStd,
										OTC_precalc->FxAdj[OTC_params->OTC],
										Results);
			...
			else if (OTC_params->OTCpayoff == 3 || OTC_params->OTCpayoff == 4)
			{
				/*	OUTPUT
					...
					lg fund_leg		pl fund_leg		st fund_leg		lg fund_fee		st fund_fee
					lg pd_leg		pl pd_leg		st pd_leg		lg pd_fee		st pd_fee
					lg otc			pl otc			st otc
					lg std			pl std			st std
				*/
				...
				err = OTCPDpayoffFeeCombinedSmileAndFlat(	cpd,
															und,
															OTC_params,
															OTC_precalc,
															smile_mkt,
			...
			else if (OTC_params->OTCpayoff == 5)
			{
				err = OTCPDcorrel(	cpd,
									...
									tstarDate,
									OTC_precalc->FxAdj[OTC_params->OTC],
									OTC_precalc->FeeFxAdj,
									OTC_params->CORRELfirstIndex,
									OTC_params->CORRELsecondIndex,
									OTC_params->CORRELfirstLong,
									OTC_params->CORRELsecondLong,
									OTC_params->CORRELtStar,
									Results);
			...
			{
				forward = und->spot_fx * swp_f_df( und->today, cpd->call[otc].set_date, und->for_yc)
				...
				for(j = 1; j < 9; j++)
				{
					Results[j][0] = forward * exp(OTC_params->CUMULnStd * ((double)j - 4) / 9.0 * OTC_params->SSLvolUp * sqrt(cpd->call[otc].set_time));
				}
				
				for(j = 0; j < 9; j++)
				{
					err = SSLprice(forward,cpd->call[otc].set_time, Results[j][0],OTC_params->SSLvolUp, OTC_params->SSLvolDown, OTC_params->SSLshift, 1, &(Results[j][2]));

					if (err) goto FREE_RETURN;
			
					err = srt_f_optsarbvol(	forward, 
											Results[j][0], 
											cpd->call[otc].set_time, 
											smile_params->sigma,	
											smile_params->alpha,	
											smile_params->beta,
											smile_params->rho,
					...
					Results[j][3] = srt_f_optblksch(	forward, 
														Results[j][0], 
														Results[j][3], 
														cpd->call[otc].set_time, 
														1.0, 
			...
			time2 = clock();
			smessage ("Phase 4 -Payoff evaluation Finished, time in sec: %.2f", (double) (time2 - time1) / CLOCKS_PER_SEC);
		}
	}
	...
	{
		calib_params = (genmidat_calibparams*) calloc(1, sizeof(genmidat_calibparams));		
		pde_params = (genmidat_pdeparams*) calloc(1, sizeof(genmidat_pdeparams));
		autocal_params = (genmidat_autocalparams*) calloc(1, sizeof(genmidat_autocalparams));
	...
	if (OTC_params->OTCpayoff == 4)
	{
		...
		err = genmidat_allocate_autocalinfos(	cpd->num_calls - k,
												autocal_params,
												sInfos);
		...
		{
			err = GenericMidatAutocal(	cpd->num_calls - k,
										dLongOption,
										dShortOption,
										NULL,
										sModel,
	...
	if (OTC_params->OTCpayoff == 6)
	{
		...
		while(	fabs(dLongOption[cpd->num_calls - 1 - jj - k]) < 1.0e-20 && 
				jj + k < cpd->num_calls && 
				(!erasing_call_done || (erasing_call_done && !erased_call_list[cpd->num_calls - 1 - jj])))
			jj++;
		...
		else
		{
			...
			if(OTC_params->use_GMA == 1 || OTC_params->use_GMA == 3)
			{
				err = GenericMidatAutocal(	cpd->num_calls - jj - k,
											dLongOption,
											dShortOption,
											NULL,
											sModel,
...
FREE_RETURN:
	...
	if (dForward) free(dForward);
	if (dLongOption) free(dLongOption);
	if (dShortOption) free(dShortOption);

	if (calib_params) free (calib_params);
	if (pde_params) free (pde_params);
	if (autocal_params) free (autocal_params);		
	...
	if(partialResults) free_dmatrix(partialResults, 0, 3, 0, 4);
...
Err				err				= NULL;
	...
	{
		save_values = f3tensor(0, cpd->num_calls, 0, 1, 0, OTC_params->COPULAnSimul - 1);
	...
	gaussian simulation 
	================================== */
	OTC_params->KO_FxMatrix = dmatrix (0, OTC_params->COPULAnSimul-1, 0, cpd->num_calls - 1);
	OTC_params->KO_FxGauss = dmatrix (0, OTC_params->COPULAnSimul-1, 0, cpd->num_calls - 1);
	OTC_params->KO_RatesMatrix = dmatrix (0, OTC_params->COPULAnSimul-1, 0, 2 - 1);
	OTC_params->KO_RatesGauss = dmatrix (0, OTC_params->COPULAnSimul-1, 0, 2 - 1);
	OTC_params->CUMUL = dmatrix (0, 2-1, 0, OTC_params->CUMULnPoints - 1);
	RealisationMatrix = dmatrix (0, OTC_params->COPULAnSimul-1, 0, 2 + cpd->num_calls - 1);

	if(	!RealisationMatrix || !OTC_params->KO_FxMatrix || !OTC_params->KO_FxGauss ||
		!OTC_params->KO_RatesMatrix || !OTC_params->KO_RatesGauss)
	{
	...
	if (OTC_params->COPULAdo_pecs)
	{
		adjust_emp_covar (	RealisationMatrix,
							OTC_params->COPULAnSimul,
							cpd->num_calls + 2);
	}
	...
	if(RealisationMatrix) free_dmatrix( RealisationMatrix, 0, OTC_params->COPULAnSimul - 1, 0, 2 + cpd->num_calls - 1);
	...
	*/
	...
	for(i = 0; i < cpd->num_calls; i++)
	{
		...
		{
			err = Fx3DtsFxFwdCov_corr(	cpd->call[i].ex_time,
										cpd->call[j].ex_time,
										0.0,
										cpd->call[i].ex_time,
										mergeTimes,
			...
			err = Fx3DtsImpliedVol_corr(	cpd->call[j].ex_time,
											cpd->call[i].ex_time,
											cpd->call[j].ex_time,
											mergeTimes,
											mergeNtimes,
	...
	for (i = 0; i < OTC_params->COPULAnSimul; i++)
	{
		for (k = 0; k < cpd->num_calls; k++)
		{
			for (l = 0; l <= k; l++)
			{
				OTC_params->KO_FxMatrix[i][k] += cholMat[k][l] * OTC_params->KO_FxGauss[i][l];
			}
		}
	}
			...
			else
			{
				for(i = 0; i < OTC_params->COPULAnSimul; i++)
				{
					OTC_params->KO_FxMatrix[i][OTC_params->OTC] = forward
						* exp( - 0.5 * smile_params->sigma * smile_params->sigma * cpd->call[otc].ex_time
								+ OTC_params->KO_FxGauss[i][OTC_params->OTC] * smile_params->sigma * sqrt(cpd->call[otc].ex_time));
				}
			}
	...
	*/
	...
	for(otc = 0; otc < cpd->num_calls; otc++)
	{
		...
		get the 3F Cumulative correlations at the call date 
		================================== */
		err = OTCgetMoments(	cpd->call[otc].ex_time,
								und->lda_dom,
								und->lda_for,
								tstarTime,
								mergeTimes,
		...
		==================================*/
		err = OTCutilsCorrelateFxAndRates(	OTC_params,
											0.0,
											stdDom,
											meanFor,
											stdFor,
		...
		*/
		...
		if(OTC_params->KO_do_optim)
		{
			...
			time2 = clock();
			smessage ("Phase 4 -Precalculation for optimisation, time in sec: %.2f", (double) (time2 - time1) / CLOCKS_PER_SEC);
			time1 = clock();
		}
		else
		{
	...
	if(OTC_params->KO_do_optim)
	{
		...
		time2 = clock();
		smessage ("Phase 5 -Optimal Barrier found, time in sec: %.2f", (double) (time2 - time1) / CLOCKS_PER_SEC);
		time1 = clock();
		...
		time2 = clock();
		smessage ("Phase 6 -Payoff evaluation Finished, time in sec: %.2f", (double) (time2 - time1) / CLOCKS_PER_SEC);
	}

FREE_RETURN:

	if (correlations) free_f3tensor(correlations, 0, 9, 0, 9, 0, mergeNtimes -1);
	if(	cholMat) free_dmatrix(cholMat, 0, cpd->num_calls -1, 0, cpd->num_calls - 1);
	if(	CORR) free_dmatrix(CORR, 0, cpd->num_calls -1, 0, cpd->num_calls - 1);
	...
	if (save_values) free_f3tensor(save_values, 0, cpd->num_calls, 0, 1, 0, OTC_params->COPULAnSimul - 1);

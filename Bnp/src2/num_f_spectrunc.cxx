/* ======================================================================
   FILENAME		: num_f_spectrunc.cxx

   PURPOSE		: generate randon numbers that sample properly the main
                  components of a Brownian motion (in terms of covariance)
                                  This function requires that memory has already
  been allocated and set to 0 for rand[path][brow][step] For several Brownian's
        , the same covar matrix is used        , and the generation is made in
  the following order for the Sobol dimensions :
                                        - Eigen Vector 1:
                                                - Brownian 1
                                                - Brownian 2
                                                - ...
                  ...
                  The i th step of the j th path for the k th Brownian motion
                                ...
                                rand[j][k][i] = sum[l/factors] ( rand[j][l][i]*
  fac_coord[l] )
...
static Err build_Brownian_covar_matrix(double *time_at_step, long sl, long sh,
                                       double **covar_matrix, long size);

/* -------------------------------------------------------------------------- */
/* --------------------------------------------------------------------------
   Given a covariance matrix        , returns Brownian paths that have been
   generated buy a spectral decomposition (i.e. a PCA). The number of factors
   used to generate the Brownian paths are determined according to the precision
   parameter        , that corresponds to the residual allowed variance (0.05%
   seems a good choice) The random realisations of the factors are generated
   using Sobol. The cube is filled as follows: rand[path][brow][step]
   -------------------------------------------------------------------------- */
Err SpecTruncCube(
    double ***rand,
    double *time_at_steps, /* From [first_step] to [last_step] at least */
    double precision, long first_path, long last_path, long first_brow,
    long last_brow, long first_step, long last_step, long *seed) {
  double ***temp_rand;
  double **covar_matrix;
  double *eigen_val;
  double **eigen_vec;
  ...
  err = build_Brownian_covar_matrix(
      ...
      first_step, last_step, covar_matrix, size);
  if (err)
    return err;
  ...
  if (size < 10)
    err =
        jacobi_diagonalisation(covar_matrix, size, eigen_val, eigen_vec, &nrot);
  else
    err =
        diagonalise_symmetric_matrix(covar_matrix, size, eigen_val, eigen_vec);
  ...
  /* Initialises Sobol to be able to fill in the first dimensions (factors) of
   * the cube with random numbers */
  err = sobol_init(0, npath - 1, 0, nbrow - 1, 0, sobol_fact - 1);
  if (err)
    return err;

  /* Fills the first dimension of the cube temp[0..np-1][0..nbr-1][0..sob-1]
   * with Sobol */
  err = sobol_cube(temp_rand, 0, npath - 1, 0, nbrow - 1, 0, sobol_fact - 1);
  if (err)
    return err;

  /* Fills in the rest of the cube with an antithetic Gaussian noise (not worse
   * than Sobol...) */
  err = gauss_anti_cube(temp_rand, 0, npath - 1, 0, nbrow - 1, sobol_fact,
                        nfact - 1, seed);
  if (err)
    return err;
  ...
  for (i = first_path; i <= last_path; i++) {
    ...
    for (j = first_brow; j <= last_brow; j++) {
      ...
      for (k = 0; k < nfact; k++) {
        ...
        /* Start loop on time steps (coordinate of eigen vectors) affected by
         * this draw */
...
/* Make the Brownian motion covariance matrix as inf(s        ,t) (allocation
 * done outside) */

static Err build_Brownian_covar_matrix(
    double *time_at_step, /* From [sl] to [sh] at least (sorted) */
    long sl, long sh, double **covar_matrix, long size) {
  long i;
  long j;

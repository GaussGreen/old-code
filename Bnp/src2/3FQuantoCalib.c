/* ==========================================================================
   ...
   PURPOSE:		Modelling of the spot FX vol by taking in consideration:
                                ...
                                A set of functions for a calibration of a 3F quanto underlying,
   taking a 4x4 correl tensor as input (index details in 3FQuantoCalib.h) In this case, the 4x4
   correl tensor is converted to a 5x5 correl tensor and the 5F calibration routine is called.
...
Err get_lgm_number_of_factors(char* underlying, int* number_of_factors)
{
    ...
    {
        return serror(
            "get_lgm_number_of_factors: Could not find the underlying named %s", underlying);
    }
    ...
    {
        return serror(
            "get_lgm_number_of_factors: number of factors of underlying %s is undefined",
            underlying);
    }
...
Err convert_4x4_to_5x5_correlation_matrix(
    double** correl_matrix, double** correl_matrix_5x5, int is_lgm2F_dom_for)
{
    ...
    int*     lgm2F_is_dom_for)
{
    ...
    double *lgm1F_sigma_dates = NULL, *lgm1F_sigma_values = NULL, *lgm1F_tau_dates = NULL,
           *lgm1F_tau_values = NULL, *lgm2F_sigma_dates = NULL, *lgm2F_sigma_values = NULL;
    ...
    double**  fx_vol_curve)
{
    ...
    for (i_fx_options = 0; i_fx_options < n_fx_options; i_fx_options++)
    {
        ...
        // Calculates the coefficients of the quadratic equation expressing the variance between T1
        // and T2 We know that fx_sigma will be constant between T1 and T2 but we still need to split
        // by sections [t1, t2] where LGM sigmas are constants
...
{
    Err  err = NULL;
    long start_index_fx, end_index_fx, start_index_ir, end_index_ir, start_index_corr,
        end_index_corr, i, j, k;
    ...
    double**  fx_vol_curve)
{
    ...
    double *dom_sigma_dates = NULL, *dom_sigma_values = NULL, *for_sigma_dates = NULL,
           *for_sigma_values = NULL, *merge_dates = NULL, *dom_merge_sigma_values = NULL,
    ...
    double*   fx_vol)
{
    ...
    int     i;
    double *dom_sigma_dates = NULL, *dom_sigma_values = NULL, *for_sigma_dates = NULL,
           *for_sigma_values = NULL, *fx_sigma_dates = NULL, *fx_sigma_values = NULL,
           *merge_dates = NULL, *dom_merge_sigma_values = NULL, *for_merge_sigma_values = NULL;

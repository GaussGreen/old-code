/* ==========================================================================
   ...
   PURPOSE:		Modelling of the spot FX vol by taking in consideration:
				...
				The calibration is using a 5F approach (2 LGM2F, 1 spot FX)
				and all 3F quanto underlyings must be converted to 2 LGM2F
				using the function convert_3F_Quanto_to_2_LGM2F

				There are 2 set of functions:

				A set of functions for a calibration of a 5F underlying, taking
				a 5x5 correl tensor as input (index details in 3FQuantoCalib.h)

  				A set of functions for a calibration of a 3F quanto underlying, taking
				a 4x4 correl tensor as input (index details in 3FQuantoCalib.h)
				In this case, the 4x4 correl tensor is converted to a 5x5 correl tensor
				and the 5F calibration routine is called.
...
{
	double result;
	result = (t-s-Phi_5F(x,T,s,t)-Phi_5F(y,T,s,t)+Phi_5F(x+y,T,s,t))/(x*y);
	return result;
}
...
Err	get_lgm_number_of_factors(char* underlying, int* number_of_factors)
{
	...
	if(!und)
	{
		return serror("get_lgm_number_of_factors: Could not find the underlying named %s", underlying);
	}
	if(get_underlying_type(und) != INTEREST_RATE_UND)
	{
		return serror("get_lgm_number_of_factors: Underlying %s is not of type IR", underlying);
	}
	if(get_mdltype_from_irund(und) != LGM)
	{
		return serror("get_lgm_number_of_factors: Underlying %s is not of type LGM", underlying);
	}
	...
	{
		return serror("get_lgm_number_of_factors: number of factors of underlying %s is undefined", underlying);
	}
										...
										int is_lgm2F_dom_for)
{
	...
	else return "lgm2F is neither foreign nor domestic";
...
Err convert_3FQuanto_to_2_LGM2F(
								char*		dom_und,
								char*		for_und,
								double**	dom_sigma_dates,
								long*		n_dom_sigma_dates,
								double**	dom_sigma_values,
								double*		dom_fixed_lambda,
								double*		dom_fixed_alpha,
								double*		dom_fixed_gamma,
								double*		dom_fixed_rho,
								double**	for_sigma_dates,
								long*		n_for_sigma_dates,
								double**	for_sigma_values,
								double*		for_fixed_lambda,
								double*		for_fixed_alpha,
								double*		for_fixed_gamma,
								double*		for_fixed_rho,
								int*		lgm2F_is_dom_for)
{
	Err err = NULL;
	int dom_und_number_of_factors, for_und_number_of_factors;

	double	*lgm1F_sigma_dates			= NULL,
			*lgm1F_sigma_values			= NULL,
			*lgm1F_tau_dates			= NULL,
			*lgm1F_tau_values			= NULL,
			*lgm2F_sigma_dates			= NULL,
			*lgm2F_sigma_values			= NULL;
	...
	if (dom_und_number_of_factors == 1 && for_und_number_of_factors == 2)
	{
		...
		err = Get_LGM_TermStructure(dom_und,
									&lgm1F_sigma_dates,
									&lgm1F_sigma_values,
									&n_lgm1F_sigma_dates,
									&lgm1F_tau_dates,
		...
		*lgm2F_is_dom_for = 1;
		...
		err = Get_LGM2F_TermStructure(for_und,
									  &lgm2F_sigma_dates,
									  &lgm2F_sigma_values,
									  &n_lgm2F_sigma_dates,
									  &lgm2F_tau,
	...
	else if (dom_und_number_of_factors == 2 && for_und_number_of_factors == 1)
	{
		...
		*lgm2F_is_dom_for = 0;
		...
		err = Get_LGM2F_TermStructure(dom_und,
									  &lgm2F_sigma_dates,
									  &lgm2F_sigma_values,
									  &n_lgm2F_sigma_dates,
									  &lgm2F_tau,
		...
		err = Get_LGM_TermStructure(for_und,
									&lgm1F_sigma_dates,
									&lgm1F_sigma_values,
									&n_lgm1F_sigma_dates,
									&lgm1F_tau_dates,
...
Err Get_FX_StochRate_TermStructures3FQuanto( char*		underlying,
											 char**		dom_und_name,
											 double**	dom_sigma_dates,
											 long*		n_dom_sigma_dates,
											 double**	dom_sigma_values,
											 ...
											 double*	for_fixed_rho,
											 int*		lgm2F_is_dom_for,
											 double**	fx_sigma_dates,
											 long*		n_fx_sigma_dates,
											 double**	fx_sigma_values)
{
	Err err = NULL;
	SrtUndPtr und;
	...
	*fx_sigma_values=	NULL;
	...
	err = srt_f_display_FX_TermStruct (underlying, n_fx_sigma_dates, fx_sigma_dates, fx_sigma_values, &corr_date_n, &corr_date, &corr);
	if (err)
	{
		goto FREE_RETURN;
	}
...
FREE_RETURN:
	...
	if (err)
	{
		if (*fx_sigma_dates) free(*fx_sigma_dates);
		*fx_sigma_dates = NULL;
	
		if (*fx_sigma_values) free(*fx_sigma_values);
		*fx_sigma_values = NULL;
	}
...
//on an interval [t1, t2[ where ALL parameters (LGM1F, LGM2F, FX sigmas, correls and lambdas)
//are CONSTANT
//DOMESTIC DYNAMICS: sigma1, lambda1 + sigma2, lambda2
//FOREIGN DYNAMICS: sigma3, lambda3 + sigma4, lambda4
//Reminder: in the double** 5x5 correl_matrix, the indices are:
//0: DOM W1, 1: DOM W2, 3: FOR W3, 4: FOR W4, 5: FX
Err Coefs_Partial_Var_5F(
						double		Tk,
						double		t1,
						double		t2,
						double		sigma1,
						...
						double*		c)
{
	...
	(*b) = - 2 * correl_matrix[W3_F][FX_5F] * sigma3 * Ksi_5F(lambda3, Tk, t1, t2)
	...
	(*c) =   sigma3 * sigma3 * Psi_5F(lambda3, lambda3, Tk, t1, t2)
		   ...
		   + 2 * correl_matrix[W3_F][W4_F] * sigma3 * sigma4 * Psi_5F(lambda3, lambda4, Tk, t1, t2)
		   - 2 * correl_matrix[W1_D][W3_F] * sigma1 * sigma3 * Psi_5F(lambda1, lambda3, Tk, t1, t2)
		   - 2 * correl_matrix[W2_D][W3_F] * sigma2 * sigma3 * Psi_5F(lambda2, lambda3, Tk, t1, t2)
		   - 2 * correl_matrix[W1_D][W4_F] * sigma1 * sigma4 * Psi_5F(lambda1, lambda4, Tk, t1, t2)
		   - 2 * correl_matrix[W2_D][W4_F] * sigma2 * sigma4 * Psi_5F(lambda2, lambda4, Tk, t1, t2)
		   + 2 * correl_matrix[W1_D][W2_D] * sigma1 * sigma2 * Psi_5F(lambda1, lambda2, Tk, t1, t2);

	return NULL;
}

//Calculates the partial volatility of a FX fwd maturing at time fx_option_maturity
//on an interval [t1, t2[ where ALL parameters (LGM1F, LGM2F, FX sigmas, correls and lambdas)
//are CONSTANT
Err Partial_Var_5F(
				  double	fx_option_maturity,
				  double	t1,
				  double	t2,
				  double	dom_sigma1,
				  double	dom_lambda1,
...
Err Fx5FtsCalibrationCorr(
						  double*		fx_options_exo_times,
						  double*		fx_options_mat_times,
						  double*		fx_options_vols,
						  long			n_fx_options,
						  double*		ir_dates,
						  long			n_ir_dates,
  						  double*		dom_sigma_values,
						  ...
						  double		for_fixed_alpha,
						  double		for_fixed_gamma,
						  double*		correl_matrix_dates,
						  long			n_correl_matrix_dates,
						  double***		correl_matrix_ts_5x5,
						  double**		fx_vol_curve)
{
	Err err = NULL;
	double a_part, b_part, c_part, a, b, c, c2, T1, T2, t1, t2, tt1, tt2, vol, cum_var, Tk, delta;
	double lambda1, lambda2, lambda3, lambda4, sigma1, sigma2, sigma3, sigma4, VolMin;
	long i, j, i_fx_options, start_index_ir, end_index_ir, start_index_corr, end_index_corr;
	...
	for (i_fx_options = 0; i_fx_options<n_fx_options; i_fx_options++)
	{
		...
		if (i_fx_options>0)
		{
			...
			err = Fx5FImpliedVolCorr(
									 ...
									 for_fixed_alpha,
									 for_fixed_gamma,
									 fx_options_exo_times,
									 *fx_vol_curve,
									 n_fx_options,
									 correl_matrix_dates,
									 n_correl_matrix_dates,
									 correl_matrix_ts_5x5,
									 &vol);
		...
		//Calculates the coefficients of the quadratic equation expressing the variance between T1 and T2
		//We know that fx_sigma will be constant between T1 and T2 but we still need to split by sections
		//[t1, t2] where LGM sigmas are constants
		...
		for (i=start_index_ir; i<end_index_ir + 1; i++)
		{
			...
			start_index_corr =  Get_Index(t1, correl_matrix_dates, n_correl_matrix_dates);
			end_index_corr =  Get_Index(t2, correl_matrix_dates, n_correl_matrix_dates);
...
Err Fx5FImpliedVolCorr(
					   ...
					   double	for_fixed_alpha,
					   double	for_fixed_gamma,
					   double*	fx_sigma_times,
					   double*	fx_sigma_values,
					   long		n_fx_sigma_times,
					   double*	correl_matrix_dates,
					   long		n_correl_matrix_dates,
...
{
	Err err = NULL;
	long start_index_fx, end_index_fx, start_index_ir, end_index_ir, start_index_corr, end_index_corr, i, j, k;
	double var, dom_fixed_lambda1, dom_fixed_lambda2, for_fixed_lambda1, for_fixed_lambda2;
...
Err Fx5FCalibrationCorr(
						char*		dom_und,
						char*		for_und,
						double*	correl_matrix_dates,
						double***	correl_matrix_ts,
						long		n_correl_matrix_ts,
						double*	fx_options_exo_times,
						double*	fx_options_mat_times,
						double*	fx_options_vols,
						long		n_fx_options,
						double**	fx_vol_curve)
{
	...
	double dom_fixed_tau, dom_fixed_lambda, dom_fixed_alpha, dom_fixed_gamma, dom_fixed_rho;
	double for_fixed_tau, for_fixed_lambda, for_fixed_alpha, for_fixed_gamma, for_fixed_rho;
	int i, lgm2F_is_dom_for = 1; //0: dom 1: for

	double	*dom_sigma_dates			= NULL,
			*dom_sigma_values			= NULL,
			*for_sigma_dates			= NULL,
			*for_sigma_values			= NULL,
			*merge_dates				= NULL,
			*dom_merge_sigma_values		= NULL,
			*for_merge_sigma_values		= NULL;
	...
	err = merge_rates_ts(
						 ...
						 n_for_sigma_dates,
						 &merge_dates,
						 &dom_merge_sigma_values,
						 &for_merge_sigma_values,
						 &n_merge_dates);
...
FREE_RETURN:
	...
	if (dom_sigma_dates) free(dom_sigma_dates);
	if (dom_sigma_values) free(dom_sigma_values);
	if (for_sigma_dates) free(for_sigma_dates);
	if (for_sigma_values) free(for_sigma_values);
	if (merge_dates) free(merge_dates);
	if (dom_merge_sigma_values) free(dom_merge_sigma_values);
	if (for_merge_sigma_values) free(for_merge_sigma_values);
	return err;
}

//Gets the implied FX fwd vol from a 5F underlying
//given a TERM STRUCTURE OF CORRELATIONS
Err	Fx5FImpliedVolFrom5FUnderlying(
								   char*	und_5F,
								   double*	correl_matrix_dates,
								   double***	correl_matrix_ts,
								   long		n_correl_matrix_ts,
...
FREE_RETURN:
	...
	if(merge_dates) free(merge_dates);
	if(dom_merge_sigma_values) free(dom_merge_sigma_values);
	if(dom_merge_sigma_values) free(for_merge_sigma_values);
							  ...
							  double**	fx_vol_curve)
{
	...
	//so that we can extract a 5x5 slice from it and pass it to our double** functions.
...
FREE_RETURN:
	...
	if (dom_merge_sigma_values) free(dom_merge_sigma_values);
	if (for_merge_sigma_values) free(for_merge_sigma_values);
	if (correl_matrix_ts_5x5) free_f3tensor(correl_matrix_ts_5x5, 0, n_correl_matrix_ts - 1, 0, 4, 0, 4);
	return err;
}
...
FREE_RETURN:
	...
	if(dom_merge_sigma_values) free(for_merge_sigma_values);
	if(correl_matrix_ts_5x5) free_f3tensor(correl_matrix_ts_5x5, 0, n_correl_matrix_ts - 1, 0, 4, 0, 4);
	if(dom_name) free(dom_name);
	if(for_name) free(for_name);

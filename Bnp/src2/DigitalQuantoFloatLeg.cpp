/* ===================================================================================
   FILENAME:      DigitalQuantoFloatLeg.c / LY J-M
											   ...
											   double* result_cms_rate_quanto)
{
	...
	if (cms_flat_vol_or_smile_approx == 0)
	{
		...
		err = digital_quanto_float_leg_get_lognormal_vol(cms_vc_name,
														 fwd,
														 fwd_spread,
														 cms_fixing_time,
														 cms_start_date,
	...
	if(cms_dom_for == 1)
	{
		...
		err = constant_or_linear_interpolation(cms_quanto_corr_times,
											   cms_quanto_corr_values,
											   n_cms_quanto_corr,
											   LINEAR_METHOD,
											   cms_start_time,
											   &cms_quanto_correlation);

		if (err) goto FREE_RETURN;
			
		err = constant_or_linear_interpolation(fwd_fx_vols_times,
											   fwd_fx_vols_values,
											   n_fwd_fx_vols,
											   LINEAR_METHOD,
											   cms_start_time,
		...
		cms*=exp(-cms_quanto_correlation*cms_atm_log_vol*fwd_fx_vol*cms_fixing_time);
	}
...
Err   digital_quanto_float_leg(char* dom_yc_name,
							   char* dom_vc_name,
							   int dom_spot_lag,
							   ...
							   double* DigitalQuantoFloatLeg)
{
	...
	double cms1_ats_log_vol, cms1_atm_log_vol, cms2_atm_log_vol, cms2_log_vol, cms1_nb_std, cms2_strike;
	...
	while (i<n_exo_coupons)
	{
		...
		if (exo_gearings[i]!=0)
		{
			...
			err = constant_or_linear_interpolation(cms1_cms2_corr_times,
												   ...
												   n_cms1_cms2_corr,
												   LINEAR_METHOD,
												   min(cms1_start_time, cms2_start_time),
			...
			else if (atm_ats_std == 2)
			{
				...
				//where nCMS1 is the number of lognormal stdev between the strike exo_barriers[i] and the fwd1
				...
				cms1_nb_std = log(exo_barriers[i]/fwd1)/(cms1_atm_log_vol*sqrt(cms1_fixing_time));
				...
				cms2_strike = fwd2*exp(cms1_cms2_correlation*cms2_atm_log_vol*cms1_nb_std*sqrt(cms2_fixing_time));
			...
			numeraire_adjustment = exp(cms1_cms2_correlation*cms1_ats_log_vol*cms2_log_vol*min(cms1_fixing_time, cms2_fixing_time));
		}

		//Calculation of the exact option (strike B)
		put_B = exo_gearings[i]*cms2*srt_f_optblksch(cms1*numeraire_adjustment,
													 exo_barriers[i],
													 cms1_ats_log_vol,
													 cms1_fixing_time,
													 1.0,
		...
		put_margin_B = exo_margins[i]*srt_f_optblksch(cms1,
													 exo_barriers[i],
													 cms1_ats_log_vol,
													 cms1_fixing_time,
													 1.0,
		...
		put_B_shift = exo_gearings[i]*cms2*srt_f_optblksch(cms1*numeraire_adjustment,
														   shifted_strike,
														   cms1_ats_log_vol,
														   cms1_fixing_time,
														   1.0,
		...
		put_margin_B_shift = exo_margins[i]*srt_f_optblksch(cms1,
															shifted_strike,
															cms1_ats_log_vol,
															cms1_fixing_time,
															1.0,
		...
		{
			if(pay_rec == 0) //PAY
				exotic_coupon_PV = - (put_B_shift + put_margin_B_shift - put_B - put_margin_B) / call_spread;
			else //REC
				exotic_coupon_PV = (put_B + put_margin_B - put_B_shift - put_margin_B_shift) / call_spread;
		}
		else //CMS1>B
		{
			if(pay_rec == 0) //PAY
				exotic_coupon_PV = - (exo_gearings[i]*cms2 + exo_margins[i]) + (put_B + put_margin_B - put_B_shift - put_margin_B_shift) / call_spread;
			else //REC
				exotic_coupon_PV = exo_gearings[i]*cms2 + exo_margins[i] - (put_B_shift + put_margin_B_shift - put_B - put_margin_B) / call_spread;
		}

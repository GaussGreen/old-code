    ...
    double* result_volatility)
{
    ...
    if (power == 0.0)
    {
        err = srt_f_optsarbvol(
            fwd, strike, fixing_time, vol, 0, 0, 0, SRT_BETAVOL, SRT_LOGNORMAL, &temp_vol);
    ...
    double* result_cms_rate_quanto)
{
    ...
    if (cms_flat_vol_or_smile_approx == 0)
    {
        ...
        {
            err = srt_f_optsarbvol(
                fwd, fwd, cms_fixing_time, flat_vol, 0, 1, 0, SRT_BETAVOL, SRT_NORMAL, &temp_vol);
    ...
    double* DigitalQuantoFloatLeg)
{
    ...
    double         cms1_ats_log_vol, cms1_atm_log_vol, cms2_atm_log_vol, cms2_log_vol, cms1_nb_std,
        cms2_strike;
        ...
        else
        {
            ...
            if (exo_cms1_fixing_dates[i] < today + eod_fix_flag)
            {
                    ...
                    ((exo_cms1_past_fixings[i] == exo_barriers[i]) && (accrue_on_barrier == 1)))
                {
                    ...
                    cms2_start_date = add_unit(
                        exo_cms2_fixing_dates[i], cms2_spot_lag, SRT_BDAY, MODIFIED_SUCCEEDING);
                    if (cms2_tenor_unit == 'M')
                    {
                        cms2_theo_end_date = add_unit(
                            cms2_start_date, cms2_tenor_num, SRT_MONTH, NO_BUSDAY_CONVENTION);
                    }
                    else
                    {
                        cms2_theo_end_date = add_unit(
                            cms2_start_date, cms2_tenor_num, SRT_YEAR, NO_BUSDAY_CONVENTION);
                    }
            ...
            else
            {
                ...
                cms1_start_date = add_unit(
                    exo_cms1_fixing_dates[i], cms1_spot_lag, SRT_BDAY, MODIFIED_SUCCEEDING);
                ...
                put_margin_B_shift = srt_f_optblksch(
                    cms1,
                    shifted_strike,
                    cms1_ats_log_vol,
                    cms1_fixing_time,
                ...
                else  // CMS1>B
                {
                    if (pay_rec == 0)  // PAY
                        exotic_coupon_PV *=
                            -1 +
                            (put_B + put_margin_B - put_B_shift - put_margin_B_shift) / call_spread;
                    else  // REC
                        exotic_coupon_PV *=
                            1 -
    ...
    while (i < n_exo_coupons)
    {
        ...
        if (exo_gearings[i] != 0)
        {
            ...
            else if (atm_ats_std == 2)
            {
                ...
                // where nCMS1 is the number of lognormal stdev between the strike exo_barriers[i]
                // and the fwd1
                ...
                cms1_nb_std =
                    log(exo_barriers[i] / fwd1) / (cms1_atm_log_vol * sqrt(cms1_fixing_time));
                ...
                cms2_strike = fwd2 * exp(cms1_cms2_correlation * cms2_atm_log_vol * cms1_nb_std *
                                         sqrt(cms2_fixing_time));
        ...
        put_margin_B = exo_margins[i] * srt_f_optblksch(
                                            cms1,
                                            exo_barriers[i],
                                            cms1_ats_log_vol,
                                            cms1_fixing_time,
        ...
        put_margin_B_shift = exo_margins[i] * srt_f_optblksch(
                                                  cms1,
                                                  shifted_strike,
                                                  cms1_ats_log_vol,
                                                  cms1_fixing_time,

	...
	)
{
	...
	)
	{
		...
		// locate indeces
		const long *plFixPay_nI_Begin = 
			(const long*)find_min_greater_than(plExStart_Begin,plFixPay_Begin,nSize_Fix,sizeof(long),lless);
		const long *plFltPay_nI_Begin = 
			(const long*)find_min_greater_than(plExStart_Begin,plFltPay_Begin,nSize_Flt,sizeof(long),lless);
	...
	)
{
	...
	double *pdSigT_Begin=_alloca(nSize_Sig*sizeof(double)), *pdSigT_End=pdSigT_Begin+nSize_Sig; 
	
	// lambda fixed
	const int nSize_Lam = plLamDate_End-plLamDate_Begin;
	double *pdLamT_Begin= _alloca(nSize_Lam*sizeof(double)),*pdLamT_End=pdLamT_Begin+nSize_Lam;
	...
	)
{
	...
	const double *pdExFee = _memset(&dFee,_alloca(nSize_Ex*size),nSize_Ex,size);
	
	const double *pdFixFee =_memset(&dFee,_alloca(nSize_Fix*size),nSize_Fix,size);
	//const double *pdCoupon_Begin = 	_memset(&dCoupon,_alloca(nSize_Fix*size),nSize_Fix,size);
	const double *pdFixNotional= _memset(&dNotional,_alloca(nSize_Fix*size),nSize_Fix,size);
	
	const double *pdFltNotional = _memset(&dNotional,_alloca(nSize_Flt*size),nSize_Flt,size);

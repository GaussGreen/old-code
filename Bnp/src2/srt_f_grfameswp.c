...
static void grfn_str0(String opt_str, int nfp, String compd_str,
                      String basis_str, double strike,
                      SrtReceiverType rec_pay) {
  if (rec_pay == SRT_RECEIVER)
    sprintf(opt_str,
            "AM(max((%f-swap(now  ,%d  ,\"%s\"  ,\"%s\"))*lvl(now  ,%d  "
            ",\"%s\"  ,\"%s\")  ,PV[0]))",
            strike, nfp, compd_str, basis_str, nfp, compd_str, basis_str);
  else if (rec_pay == SRT_PAYER)
    sprintf(opt_str,
            "AM(max((swap(now  ,%d  ,\"%s\"  ,\"%s\")-%f)*lvl(now  ,%d  "
            ",\"%s\"  ,\"%s\")  ,PV[0]))",
            nfp, compd_str, basis_str, strike, nfp, compd_str, basis_str);
}

static void grfn_str1(String opt_str, int nfp, String compd_str,
                      String basis_str, double strike,
                      SrtReceiverType rec_pay) {
  if (rec_pay == SRT_RECEIVER)
    sprintf(opt_str,
            "max((%f-swap(now  ,%d  ,\"%s\"  ,\"%s\"))*lvl(now  ,%d  ,\"%s\"  "
            ",\"%s\")  ,0)",
            strike, nfp, compd_str, basis_str, nfp, compd_str, basis_str);
  else if (rec_pay == SRT_PAYER)
    sprintf(opt_str,
            "max((swap(now  ,%d  ,\"%s\"  ,\"%s\")-%f)*lvl(now  ,%d  ,\"%s\"  "
            ",\"%s\")  ,0)",
            nfp, compd_str, basis_str, strike, nfp, compd_str, basis_str);
}
...
static Err grfn_mk_ameswp_GrfnCells(double start, double end, int nfp,
                                    int delay, int compd, int basis,
                                    double strike, SrtReceiverType rec_pay,
                                    Date clcn_date, long *num_eventdates,
                                    Date **eventdates, long *nrows, long *ncols,
                                    GrfnCell ***sprdsht) {
  Err err = NULL;
  String compd_str;
  String basis_str;
...
Err grf_ameswp_clsdfrm(
    double start, double end, int nfp, int delay, int compd, int basis,
    double *strikes, SrtReceiverType rec_pay, /* pay or rec */
  ...
  err = grfn_mk_ameswp_GrfnCells(start, end, nfp, delay, compd, basis,
                                 strikes[0], rec_pay, today, &num_eventdates,
                                 &eventdates, &nrows, &ncols, &sprdsht);

  if (!err) {
    err = srt_f_grfn(und, grfnparam, num_eventdates, &eventdates, &nrows,

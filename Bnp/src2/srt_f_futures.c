    ...
    double* error)
{
    ...
    for (i = 1; i <= iNumFut; i++)
        dWeights[i] = 1.0;  // convexities[i-1];//2*pow(2*i/iNumFut-0.5,2);//pow(0.98,iNumFut-i);
    ...
    case 0:
        ...
        err        = levenberg_marquardt(
            dIndex, dConv, dWeights, iNumFut, dParams, nParams, nIter, eFutFunc_HJM, error);
        *alpha = dParams[1];
        *beta  = dParams[2];
        break;
    case 2:
        ...
        dParams[1] = *beta;
        err        = levenberg_marquardt(
            dIndex, dConv, dWeights, iNumFut, dParams, nParams, nIter, eFutFunc_Rut, error);
        *beta = dParams[1];
        break;
    case 3:
        nParams    = 2;
        ...
        dParams[2] = *beta;
        err        = levenberg_marquardt(
            dIndex, dConv, dWeights, iNumFut, dParams, nParams, nIter, eFutFunc_Bloom, error);
        *alpha = dParams[1];
        *beta  = dParams[2];
        break;
    default:

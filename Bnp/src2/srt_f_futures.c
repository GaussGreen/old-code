                  ...
                  double   *pdRate)
{
    ...
    if (ppdOptFutVol) err=eBuildVols(lToday);
    if (err) {
        free_dvector(dDates,0,iNumFut+1); dDates=NULL;
        free_dvector(dPeriod,0,iNumFut); dPeriod=NULL;
        if (ppdOptFutVol) free_dvector(dVolStruct2, 0, iNumFut-1); dVolStruct2=NULL;
        return err;
    }

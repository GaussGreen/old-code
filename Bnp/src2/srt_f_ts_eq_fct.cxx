...
/* Function to compute : int(s = 0        , s = t)
 * (rho(s)*sig_eq(s)*sig_ir(s)/F(s))
 */
double V_ir_e_func(double time, TermStruct *l) {
  ...
  res = tsval->V_ir_e + tsval->rho_e_ir * tsval->sig * (ir_sig / F) *
                            (exp(time / tau) - 1) * tau;

  return res;
}
/* Function to compute : int(s = 0        , s = t)
 * (rho(s)*sig_eq(s)*sig_ir(s)*Psi(s)/F(s)) */
double W_ir_e_func(double time, TermStruct *l) {
  ...
  res = tsval->W_ir_e +
        tsval->rho_e_ir * tsval->sig * ir_sig * (Psi / F) *
            (exp(time / tau) - 1) * tau +
        tsval->rho_e_ir * tsval->sig * ir_sig * tau *
...
Err srt_f_zc_bond_vol(double expiry, char *und_name,
  ...
  (*zc_bond_vol) =
      2 * (Psi_func(expiry, ts) * L_func(expiry, ts) - O_func(expiry, ts)) /
      expiry;

  return NULL;
}

Err one_expiry_calib_func(char *model, double spot, char *und_name,
                          char *disc_und_name, char *ccy, char *dvd_name,
                          char *repoName,

                          long n_dividend, double **dividend, long n_repo,
                          double **repo,

                          double expiry, int ex_index, int n_ex,
  ...
  err = SrtInitDividends(date_today, dividend_curve_name, ccy, n_dividend, 2,
                         dividend);
  ...
  err = SrtInitEQUnd(und_name, model, spot, disc_und_name, dividend_curve_name,
                     repo_curve_name, n_ex, 2, sig_matrix,
  ...
  (*return_value) =
      ...
      2 * (W_ir_e_func(expiry, ts) -
           Psi_func(expiry, disc_und_ts) * V_ir_e_func(expiry, ts));
...
Err srt_f_eq_forward(Date date, SrtUndPtr und,
  ...
  /* compute the forward */
  (*forward) =
      spot * srt_f_forward_from_fwdcrv(time, dvd_crv, repo_crv) / df_date;
...
/* function to calibrate the local volatility of an equity under in a stoch
 * rates environment */
Err srt_f_eq_calib(long n_ex,

                   double *dates, double *vols,

                   char *disc_und_name, char **und_name) {
  ...
  for (l = 1; l <= n_ex; l++) {
    ...
    err = one_expiry_calib_func("EQ_STOCH_RATES", spot, *und_name,
                                disc_und_name, ccy, dvd_name, repo_name,

                                n_dvd, dvd_matrix, n_repo, repo_matrix,

                                (dates[l - 1] - date_today) * YEARS_IN_DAY, l,
                                n_ex,

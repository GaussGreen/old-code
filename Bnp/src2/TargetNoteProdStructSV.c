...
char* alloc_TARN_MC_AUX_SV(TARN_Struct* tarn, TARN_AUX* aux, TARN_MC_AUX* mc_aux)
{
    ...
    if (!mc_aux->is_event || !mc_aux->date || !mc_aux_sv->sigma || !mc_aux_sv->alpha ||
        !mc_aux_sv->rho || !mc_aux_sv->lameps || !mc_aux_sv->lvleps)
        goto FREE_RETURN;
...
char* set_TARN_TimeSteps_SV(TARN_Struct* tarn, TARN_AUX* aux, TARN_MC_AUX* mc_aux)
{
    ...
    {
        mc_aux->time[i] =
            (tarn->deal.lvFltrFixingDates[i + aux->i1stCpn] - tarn->market.lToday) / 365.0;
    ...
    if (err = fill_time_vector(
            &mc_aux->time, &(mc_aux->nStps), 0, NULL, 0, NULL, tarn->pricing.nStepT))
        goto FREE_RETURN;
...
void setLGM1FSV_star(
    double dTimeEvent, LGMSV_model* model, double* out_gam1_star, double* out_gam1_2_star)
{
    ...
    double*      out_gam12)
{
    ...
    temp1 = (1.0 - exp(model->dLambdaX * (model->dTStar - dTimeEvent - dTimeDF))) / model->dLambdaX;
    temp2 =
        (1.0 - exp(model->dLambdaX2 * (model->dTStar - dTimeEvent - dTimeDF))) / model->dLambdaX2;
    *out_gam1   = temp1 - gam1_star;
    ...
    TARN_EVENT*  event)
{
    ...
    if (event->num_fund_df > 0)
    {
        ...
        if (err = check_ptr(
                9,
                event->fund_start_tms,
                event->fund_start_dts,
                event->fund_start,
                event->fund_end_tms,
                event->fund_end_dts,
                event->fund_end_logdff,
                event->fund_end,
                event->fund_cvgMargin))
            goto FREE_RETURN;
        ...
        for (iRelFund = 0; iRelFund < event->num_fund_df; iRelFund++)
        {
            ...
            event->fund_start_logdff[iRelFund] =
                log(swp_f_df(lEvent, event->fund_start_dts[iRelFund], tarn->market.szYieldCurve));
            ...
            event->fund_end_logdff[iRelFund] =
                log(swp_f_df(lEvent, event->fund_end_dts[iRelFund], tarn->market.szYieldCurve));

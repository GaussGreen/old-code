...
char* alloc_TARN_MC_AUX_SV( TARN_Struct* tarn, 
							TARN_AUX* aux, 
							TARN_MC_AUX* mc_aux )
{
...
/*	Next, set the event dates at each of the floater fixing dates and allocate the time steps */
...
/* Allocate the memory for the event variables */
	...
	if ( !mc_aux_sv->logdff_star || !mc_aux_sv->gam1_star || !mc_aux_sv->gam1_2_star )
		goto FREE_RETURN;		
	...
	if ( iIsTwoF )
	{
		...
		if ( !mc_aux_sv->gam2_star || !mc_aux_sv->gam2_2_star || !mc_aux_sv->gam12_star )
			goto FREE_RETURN;
	}
...
/*	Allocate memory for the time step variables variables */
	...
	if ( !mc_aux->is_event || !mc_aux->date || !mc_aux_sv->sigma || !mc_aux_sv->alpha || !mc_aux_sv->rho 
				|| !mc_aux_sv->lameps || !mc_aux_sv->lvleps )
		goto FREE_RETURN;
	...
	if ( iIsTwoF )
	{
		...
		if ( !mc_aux_sv->dLGMalpha || !mc_aux_sv->dLGMgamma || !mc_aux_sv->dLGMrho || !mc_aux_sv->dRho2 )
			goto FREE_RETURN;
	}
	err = 0;
...
char* set_TARN_TimeSteps_SV( TARN_Struct* tarn, TARN_AUX* aux, TARN_MC_AUX* mc_aux )
{
/* variable declaration */
	int i;
	char* err = 0;
...
/* Loop over the coupon fixing dates setting the dates and times */
	...
	{		
		mc_aux->time[i] = ( tarn->deal.lvFltrFixingDates[i+aux->i1stCpn] - tarn->market.lToday ) /365.0;
...
/* add in the sigma dates */
	...
	while ( i < mc_aux_sv->model.iNbPWTime && mc_aux_sv->model.dPWTime[i] < mc_aux->time[mc_aux->nEvents-1] )
	{
		num_f_add_number( &(mc_aux->nStps), &(mc_aux->time), mc_aux_sv->model.dPWTime[i] );
		i++;
	}

/*	Make sure that the time vector has at least the required number of points */
	if ( err = fill_time_vector( &mc_aux->time, &(mc_aux->nStps), 0, NULL, 0, NULL, tarn->pricing.nStepT)  )				
		goto FREE_RETURN;
...
void setLGM1FSV_star( double dTimeEvent, LGMSV_model *model, double* out_gam1_star, double* out_gam1_2_star )
{
	*out_gam1_star = (1.0 - exp(model->dLambdaX * (model->dTStar - dTimeEvent))) / model->dLambdaX;
	*out_gam1_2_star = 0.5 * (*out_gam1_star) * (*out_gam1_star);
}


void setLGM1FSV_event( double dTimeEvent, 
						double dTimeDF, 
						LGMSV_model *model, 
						double gam1_star, 
						double gam1_2_star,
...
{
	double temp1;
	temp1 = (1.0 - exp(model->dLambdaX * (model->dTStar - dTimeEvent - dTimeDF ))) / model->dLambdaX;
...
void setLGM2FSV_star( double dTimeEvent, 
						LGMSV_model *model, 
						double* out_gam1_star, 
						double* out_gam1_2_star,
						double* out_gam2_star,
						double* out_gam2_2_star,
						double* out_gam12_star
						)
{
	*out_gam1_star = (1.0 - exp(model->dLambdaX * (model->dTStar - dTimeEvent))) / model->dLambdaX;
	*out_gam2_star = (1.0 - exp(model->dLambdaX2 * (model->dTStar - dTimeEvent))) / model->dLambdaX2;
	*out_gam1_2_star = 0.5 * (*out_gam1_star) * (*out_gam1_star);
...
void setLGM2FSV_event( double dTimeEvent, 
						double dTimeDF, 
						LGMSV_model *model, 
						double gam1_star, 
						double gam1_2_star,
						...
						)
{
	...
	temp1 = (1.0 - exp(model->dLambdaX * (model->dTStar - dTimeEvent - dTimeDF))) / model->dLambdaX;
	temp2 = (1.0 - exp(model->dLambdaX2 * (model->dTStar - dTimeEvent - dTimeDF))) / model->dLambdaX2;
	*out_gam1 = temp1 - gam1_star;
...
/* set the 1F values */	
	...
	{	
		setLGM1FSV_event( mc_aux->evt_tms[iEvt], 
							event->fund_start_tms[iFund], 
							&mc_aux_sv->model, 
							mc_aux_sv->gam1_star[iEvt], 
							mc_aux_sv->gam1_2_star[iEvt], 
							&event_sv->fund_start_gam1[iFund],
							&event_sv->fund_start_gam1_2[iFund] );

		setLGM1FSV_event( mc_aux->evt_tms[iEvt], 
							event->fund_end_tms[iFund], 
							&mc_aux_sv->model, 
							mc_aux_sv->gam1_star[iEvt], 
							mc_aux_sv->gam1_2_star[iEvt], 
							&event_sv->fund_end_gam1[iFund],
							&event_sv->fund_end_gam1_2[iFund] );
	}
/* set the 2F values if necessary */	
	else
	{
		setLGM2FSV_event( mc_aux->evt_tms[iEvt], 
							event->fund_start_tms[iFund], 
							&mc_aux_sv->model, 
							mc_aux_sv->gam1_star[iEvt], 
							mc_aux_sv->gam1_2_star[iEvt], 
							mc_aux_sv->gam2_star[iEvt],
							mc_aux_sv->gam2_2_star[iEvt],
							mc_aux_sv->gam12_star[iEvt],
							&event_sv->fund_start_gam1[iFund],
							&event_sv->fund_start_gam1_2[iFund],
							&event_sv->fund_start_gam2[iFund],
		...
		setLGM2FSV_event( mc_aux->evt_tms[iEvt], 
							...
							&event_sv->fund_end_gam1[iFund], 
							&event_sv->fund_end_gam1_2[iFund],
							&event_sv->fund_end_gam2[iFund],
							&event_sv->fund_end_gam2_2[iFund],
							&event_sv->fund_end_gam12[iFund]
				);
	}
}
...
char* fillTargetNoteFundingSV(	
					int iEvtMC, /* MC event date---not deal event date */
					TARN_Struct* tarn,
					TARN_AUX* aux,
					TARN_MC_AUX* mc_aux,
					TARN_EVENT* event )
...
/* Set up the target note */
	...
	if (event->num_fund_df > 0)
	{
		...
		if ( err = check_ptr( 9, event->fund_start_tms, event->fund_start_dts, event->fund_start, event->fund_end_tms,
							event->fund_end_dts, event->fund_end_logdff, event->fund_end, event->fund_cvgMargin ) )
			goto FREE_RETURN;
...
/* start of the float period quantities */
			...
			event->fund_start_tms[iRelFund] = ( event->fund_start_dts[iRelFund] - lEvent ) / 365.0;
			event->fund_start_logdff[iRelFund] = log( swp_f_df(lEvent, event->fund_start_dts[iRelFund], tarn->market.szYieldCurve) );

			event->fund_end_dts[iRelFund] = tarn->deal.lvFundEndDates[iFund]; 
			event->fund_end_tms[iRelFund] = ( event->fund_end_dts[iRelFund] - lEvent) / 365.0;
			event->fund_end_logdff[iRelFund] = log( swp_f_df(lEvent, event->fund_end_dts[iRelFund], tarn->market.szYieldCurve) );

			event->fund_cvgMargin[iRelFund] = 1.0 - aux->dvFundCvg[iFund]
...
char* fillTargetNoteCouponSV(
					int iEvt, /* MC event date */
					TARN_AUX* aux,
					TARN_MC_AUX* mc_aux,
					TARN_EVENT* event )
{
...
/* Find out what the model is and set the model structure pointer */
	if ( mc_aux->tnModel == TN_2F_SV )
	{
		...
		setLGM2FSV_event( mc_aux->evt_tms[iEvt], 
				...
				mc_aux_sv->gam2_2_star[iEvt],
				mc_aux_sv->gam12_star[iEvt],
				&event_sv->fixed_pay_gam1, 
				&event_sv->fixed_pay_gam1_2,
				&event_sv->fixed_pay_gam2,
				&event_sv->fixed_pay_gam2_2,
				&event_sv->fixed_pay_gam12
				);

			setLGM2FSV_event( mc_aux->evt_tms[iEvt], 
				event->fltr_start_tms, 
				&mc_aux_sv->model, 
				mc_aux_sv->gam1_star[iEvt], 
				mc_aux_sv->gam1_2_star[iEvt],
				mc_aux_sv->gam2_star[iEvt],
				mc_aux_sv->gam2_2_star[iEvt],
				mc_aux_sv->gam12_star[iEvt],
				&event_sv->fltr_start_gam1, 
				&event_sv->fltr_start_gam1_2,
				&event_sv->fltr_start_gam2,
				&event_sv->fltr_start_gam2_2,
		...
		setLGM2FSV_event( mc_aux->evt_tms[iEvt], 
				...
				mc_aux_sv->gam2_2_star[iEvt],
				mc_aux_sv->gam12_star[iEvt],
				&event_sv->fltr_end_gam1, 
				&event_sv->fltr_end_gam1_2,
				&event_sv->fltr_end_gam2,
				&event_sv->fltr_end_gam2_2,

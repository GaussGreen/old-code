...
static Err string_to_CMTInstr(
    String iname, CMTInstr* instr, double* spread, CMT_Param* cmt_param, Date spot);

static Err CMT_init(
    CMT_Obj*          cmt,
    Date              cmt_today_date,
    CMT_Param_Struct* cmt_param,
    String*           tenor_names,
    double*           spread_rates,
    int               num_spreads);

static Err FwdT_init(String fwd_T_name, CMT_Obj* cmt, FwdT_Obj* fwdT, CMT_Param* cmt_param);

static Err FwdTEC_init(String fwd_T_name, CMT_Obj* cmt, FwdT_Obj* fwdT, CMT_Param* cmt_param);

static int putoncurve(
    FwdT_Obj* fwdT, Ddate date, double cum_time, double fwd_spread, double fwd_R_rate, long index);

static Err FwdT_fill(
    String      fwd_spread_name,
    double*     fwd_spread_date,
    double*     fwd_spread_value,
    int         num_fwd_spreads,
    FwdT_Obj*   fwdT,
    SrtCurvePtr yc_crv,
    Date        clcn_date,
...
static SrtErr swp_f_addcmtcrv_to_list(
    ...
    int               num_spreads)
{
    ...
    for (i = 0; i < num_spreads; i++)
    {
        ...
        if (err = string_to_CMTInstr(
                tenor_names[i], &cmt_instr, &spread_rates[i], cmt_param, spot_date))
        {
            return err;
        }
    ...
    String iname, CMTInstr* instr, double* spread, CMT_Param* cmt_param, Date spot)
{
    ...
    else
    {
        ...
        instr->cms_dp.swapdates.end        = add_unit(spot, nm, SRT_MONTH, NO_BUSDAY_CONVENTION);
        ...
        instr->cmt_dp.swapdates.end        = add_unit(spot, nm, SRT_MONTH, NO_BUSDAY_CONVENTION);
...
static Err CMT_set_Instr(CMT_Obj* cmt, CMTInstr* instr, int i)
{
    ...
    case SWAPINSTR:
        ...
        CMT_field_Iset(cmt, CMT_SWAP_BASIS_CODE, i, instr->cmt_dp.swapdates.basis_code);
        CMT_field_Iset(cmt, CMS_SWAP_BASIS_CODE, i, instr->cms_dp.swapdates.basis_code);
        ...
        CMT_field_Dateset(
            cmt, CMT_SWAP_FIRST_FULL_FIXING, i, instr->cmt_dp.swapdates.first_full_fixing);
        CMT_field_Dateset(
            cmt, CMS_SWAP_FIRST_FULL_FIXING, i, instr->cms_dp.swapdates.first_full_fixing);

        CMT_field_Iset(cmt, CMT_SWAP_NUM_FULL_PERIOD, i, instr->cmt_dp.swapdates.nfp);
        CMT_field_Iset(cmt, CMS_SWAP_NUM_FULL_PERIOD, i, instr->cms_dp.swapdates.nfp);
        ...
        if (i > 0 && instr->cmt_dp.swapdates.end <= CMT_field_Dateget(cmt, CMT_SWAPEND_DATE, i - 1))
...

{
    ...
    SwapDP      swapdp;
    DateList    datelist;
    SrtCurvePtr crv; /* Can be either a cmt_crv or a yc_crv depending on last use*/
    String      yc_name;
    String      ccy_str;
    ...
    if (CMT_Iget(cmt, CMT_SPOT_EXISTS == 1))
    {
        ...
        swp_f_ForwardRate_SwapDP(&swapdp, yc_name, cmt_param->swap_ref_rate, &fwd_R_rate);
        ...
        */
        /* As there is a spot rate entered
                                        and first_swap_index refers to first swap that
                                        does not overlap*/
    }
    ...
    */
    ...
    if (num_swap > 0)
    {
        ...
        {
            /* Gets the value of the CMS-CMT spread from the spreadsheet (thru CMT_Obj)*/
            ...
            /* STARTS ITERATION ON SPREAD TO SOLVE FOR A PAR VALUE OF THE CMS/LIBOR SWAP */
...
static Err FwdTEC_init(String fwd_T_name, CMT_Obj* tec, FwdT_Obj* fwdT, CMT_Param* cmt_param)

{
    ...
    if (num_swap > 0)
    {
        for (i = first_swap_index; i < num_swap; i++)
        {
            /* Gets the value of the TEC-LIBOR spread from the spreadsheet (thru CMT_Obj)*/
    ...
    CMT_Param** cmt_param)
{
    ...
    if (err = CMT_init(cmt, cmt_today_long, *cmt_param, tenor_names, spread_rates, num_spreads))
    {
        return (err);
    }
    ...
    if (err = swp_f_addcmtcrv_to_list(
            crv_list, fwd_T_name, "CMT", clcn_date, yc_name, fwdT, cmt, *cmt_param))
    {
        return (err);
    }
...

{
    ...
    for (i = 0; i < num_fwd_spreads; i++)
    {
        ...
        swapdp.spot_lag          = cmt_param->spot_lag;
        swapdp.first_full_fixing = swapdp.start;
        swp_f_ForwardRate_SwapDP(
            &swapdp, get_curve_name(yc_crv), cmt_param->swap_ref_rate, &fwd_R_rate);

        /* Computes cum_time from spot_date */
        cum_time = (double)(fwd_spread_date[i] - spot_date) * YEARS_IN_DAY;

        /* Puts the element in the FwdT_Obj */
        putoncurve(fwdT, fwd_spread_date[i], cum_time, fwd_spread_value[i], fwd_R_rate, i);
    ...
    CMT_Param* cmt_param)
{
    ...
    crv_list = get_curve_list();
    if (err = swp_f_addcmtcrv_to_list(
            crv_list, fwd_spread_name, "CMT", clcn_date, yc_name, fwdT, NULL, cmt_param))
    {
        return (err);
    }
    ...
    CMT_Param*    cmtprm)
{
    ...
    */
    ...
    if (curve->curve_type != CMT_CURVE)
        return (serror("Trying to use addCMTcrv_to_list for a %s curve", curve_lbl));
    ...
    CMT_Param** cmt_param)
{
    ...
    if (err = swp_f_addcmtcrv_to_list(
            crv_list, fwd_T_name, "CMT", clcn_date, yc_name, fwdTresult, cmt, *cmt_param))
    {
        return (err);
    }

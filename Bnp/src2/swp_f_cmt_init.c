...
static Err	CMT_init(	CMT_Obj *cmt, 
						Date cmt_today_date, 
...
static Err FwdT_init( 	String fwd_T_name, 
						CMT_Obj *cmt, 
						FwdT_Obj *fwdT, 
						CMT_Param *cmt_param);

static Err FwdTEC_init(	String fwd_T_name, 
						CMT_Obj *cmt, 
						FwdT_Obj *fwdT, 
						CMT_Param *cmt_param);
...
static Err	CMT_init(CMT_Obj *cmt, Date cmt_today_date, CMT_Param_Struct *cmt_param,
					String *tenor_names, double *spread_rates, int num_spreads)
{
int toalloc;
int ncash=0,nswap=0;
CMTInstr	cmt_instr;
...
/* Computes the end dates (a full CMTInstr) of the swap, given the tenor */
	    if(err = string_to_CMTInstr(tenor_names[i],&cmt_instr, 
			&spread_rates[i],cmt_param,spot_date))
		{
			return err;
		}
...
int max_index;
	...
	switch(instr->type)
	{
		...
		/* Default values */
			...
			CMT_field_Dateset(cmt,CMT_SWAP_FIRST_FULL_FIXING,i, instr->cmt_dp.swapdates.first_full_fixing);
			CMT_field_Dateset(cmt,CMS_SWAP_FIRST_FULL_FIXING,i, instr->cms_dp.swapdates.first_full_fixing);
...
/* Sets the initial value for the fwd_T_rate in the curve:
	please note that FwdT_NUMSPREAD is set to  index+1 */
		putoncurve(fwdT, tenor_date, cum_time, fwd_spread, 
						fwd_R_rate, index);
...
/* Init th CMT_Obj using the CMT-CMS spread curve (given by tenors)*/
	if ( err = CMT_init( cmt, cmt_today_long, *cmt_param, 
			    tenor_names, spread_rates, num_spreads))
	{
		return( err);
	}
...
/* Pick up the next available market (in the static list) and attach 
   the CMT_Obj (initialised) and the FwdT_Obj (allocated) to this market*/
	...
	if ( err = swp_f_addcmtcrv_to_list(crv_list, fwd_T_name,"CMT",
		     clcn_date, yc_name, fwdT, cmt,  *cmt_param)) 
	{
		return( err);          
	}	
...
/* Sets a few fields in the FwdT_obj (interp_method, today, spot)*/ 
	...
	/* Computes the (forward) swap rate (start date = fwd_spread_date) */
		...
		swapdp.spot_lag =  cmt_param->spot_lag;
		swapdp.first_full_fixing = swapdp.start;
		swp_f_ForwardRate_SwapDP( &swapdp, get_curve_name(yc_crv), cmt_param->swap_ref_rate, &fwd_R_rate);
		
	/* Computes cum_time from spot_date */
        cum_time = (double)(fwd_spread_date[i] - spot_date)*YEARS_IN_DAY;

	/* Puts the element in the FwdT_Obj */ 
		putoncurve(fwdT, fwd_spread_date[i], cum_time, 
				fwd_spread_value[i], fwd_R_rate, i);
...
/* Pick up the next available market (in the static list) and attach 
   the FwdT_Obj (allocated and filled in) to this market*/
	crv_list = get_curve_list();
	if ( err = swp_f_addcmtcrv_to_list(	crv_list, 
									fwd_spread_name,
									"CMT",
		     						clcn_date,
									yc_name,
...
String	ccy_str;
	...
	if ( err = swp_f_addcmtcrv_to_list(crv_list, fwd_T_name,"CMT",
		     clcn_date, yc_name, fwdTresult, cmt,  *cmt_param)) 
	{
		return( err);          
	}	
	...
	if(CMT_Iget(cmt,CMT_SPOT_EXISTS)==1)
	{
		...
		/* Sets the initial value for the fwd_T_rate in the curve:
		please note that FwdT_NUMSPREAD is set to  index+1 */
		putoncurve(fwdTresult, tenor_date, cum_time, fwd_spread, 
					fwd_R_rate, index);
  	}
	...
	if (num_swap >0 )
	{
		for (i = first_swap_index; i<num_swap; i++)
		{
			...
			putoncurve(fwdTtemp, tenor_date, cum_time, fwd_spread, 
					   fwd_R_rate, 0);

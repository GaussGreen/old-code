...
{
	free(g->phi);
	if (g->ad) free_dmatrix(g->ad, 0, g->nphi-1, 0, g->nx-1);
	if (g->pv) free_f3tensor(g->pv, 0, g->nphi-1, 0, g->nx-1, 0, g->ninst-1);
...
static Err CopyGrid(SCheyBetaPDEGrid *g1, SCheyBetaPDEGrid *g2)
{
	...
	if (g1->pv)
	{
		...
		if (!g2->pv) return serror("Memory failure");
		...
		SProductDesc *g, int nt, int nx, int nphi, double cutcoef, int isfwd, SCheyBeta *pmdl)
{
	...
	if (g->nccy > 1) return serror("Error (CheyBeta PDE): Product is multi-currency");
	...
	while (pde->times[ntimes-1] > g->ex[g->nex-1] + 1e-5) ntimes--;
	...
	if ( !pde->fr || !pde->gamT || !pde->df ||
		 !pde->phi_max || !pde->phi_min || !pde->phi_mid || !pde->cumxvar )
	...
	for (i=0; i < ntimes-1; i++)
	{
		...
		pde->gamT[i] = ( 1.0 - exp(-pmdl->lambda *
			(pde->times[ntimes-1] - pde->times[i])) ) / pmdl->lambda;
		d1 = d2;
	}
	...
	if (!pde->x) return serror("Memory failure");

	pde->tmppde = (CNPDE_TEMP *) malloc(sizeof(CNPDE_TEMP));
	if (!pde->tmppde) return serror("Memory failure");
...
{
	int i;
	for (i=0; i < 3; i++) FreeGrid(&pde->grids[i]);
	free(pde->times);
	free(pde->df);  free(pde->gamT);  free(pde->fr);
	free(pde->phi_max);  free(pde->phi_min);  free(pde->phi_mid);
	free(pde->x);  free(pde->cumxvar);

	if (pde->tmppde) num_f_pde_free(pde->tmppde, pde->nx, pde->isfwd ? 1 : pde->g->ninst);
	free(pde->tmppde);
...
static void MakeGrid(SCheyBetaPDE *pde)
{
	...
	pde->x[pde->ix0] = 0.0;
	for (i = pde->ix0 + 1; i < pde->nx; i++) pde->x[i] = pde->x[i-1] + dx;
	for (i = pde->ix0 - 1; i >= 0; i--) pde->x[i] = pde->x[i+1] - dx;
}
...
static void ExpandBounds(SCheyBetaPDE *pde, int start, int end)
{
	...
	for (i=start; i < end; i++)
	{
		...
		while ( isig < pde->pmdl->nsig - 1 &&
			pde->pmdl->sigtms[isig] < pde->times[i] + 1e-5 ) isig++;
		...
		v_mid = CheyBeta_Vol(0.0, pde->phi_mid[i], sig, beta, pde->gamT[i], pde->fr[i], xcut_u, xcut_d);
		v_max = CheyBeta_Vol(xcut_u, 0.0, sig, beta, pde->gamT[i], pde->fr[i], xcut_u, xcut_d);
		v_min = CheyBeta_Vol(xcut_d, 0.0, sig, beta, pde->gamT[i], pde->fr[i], xcut_u, xcut_d);

		pde->cumxvar[i+1] = pde->cumxvar[i] + v_mid * v_mid * dt;
		pde->phi_mid[i+1] = pde->phi_mid[i] + (v_mid * v_mid - 2.0 * lam * pde->phi_mid[i]) * dt;
		pde->phi_max[i+1] = pde->phi_max[i] + (v_max * v_max - 2.0 * lam * pde->phi_max[i]) * dt;
		pde->phi_min[i+1] = pde->phi_min[i] + (v_min * v_min - 2.0 * lam * pde->phi_min[i]) * dt;
	}
}
...
static Err InitGridAt_i(SCheyBetaPDE *pde, int i)
{
	...
	{
		itmp = 3 + (int)( (pde->nphi-3) *
			pde->times[i] / pde->times[pde->nt-1] + 1e-8 );
		dphi = (pde->phi_max[i] - pde->phi_min[i]) / (itmp-1);
		pde->grids[!pde->dir].phi = (double *) calloc(itmp, sizeof(double));
		if (!pde->grids[!pde->dir].phi) return serror("Memory failure");
		pde->grids[!pde->dir].nphi = itmp;
		for (j=0; j < itmp; j++) pde->grids[!pde->dir].phi[j] = pde->phi_min[i] + j * dphi;
	}
	...
	if (pde->isfwd)
	{
		...
		if (!pde->grids[!pde->dir].ad) return serror("Memory failure");
	...
	{
		pde->grids[!pde->dir].pv =
			f3tensor(0, pde->grids[!pde->dir].nphi-1, 0, pde->nx-1, 0, pde->g->ninst-1);
		if (!pde->grids[!pde->dir].pv) return serror("Memory failure");
...
Err CheyBetaPDE_ExpandFwd(SCheyBetaPDE *pde, double T)
{
	...
	if (!pde->isfwd) return serror("The PDE was initialized as backward");

	// Find new last
	new_last = pde->last;
	while (new_last < pde->nt - 1 && pde->times[new_last] < T - 1e-5) new_last++;
	if (new_last == pde->last) return NULL;
	...
	for (i = pde->last; i < new_last; i++)
	{
		...
		{
			// Calculate mu
			for (xi=0; xi < pde->nx; xi++) mu[xi] = -lam * pde->x[xi] * dt;
			...
			for (k=0; k < pde->grids[pde->dir].nphi; k++)
			{
				...
				for (xi=0; xi < pde->nx; xi++)
				{
					std = CheyBeta_Vol(pde->x[xi], phi_k, sig, beta, pde->gamT[i], pde->fr[i], xcut_u, xcut_d);
					var[xi] = std * std * dt;
				}

				// Convolve slice into the ad vector using Crank-Nicholson
				num_f_pde_one_step_forward( pde->tmppde,
											pde->nx,
											pde->x,
											pde->grids[pde->dir].ad[k],
											mu,
											var,
											r,
											0.55,
...
Err CheyBetaPDE_ResetFwd(SCheyBetaPDE *pde, double T)
{
	...
	// Find new last
	new_last = 0;
	while (new_last < pde->last && pde->times[new_last] < T - 1e-5) new_last++;
	if (new_last == pde->last) return NULL;
...
static Err PayoffAt_i(SCheyBetaPDE *pde, int idx, int i, double ***pv)
{
	...
	int		xi, j, k;
	long	d1, d2;
	double	x, phi, dffs[MAXCPN], gams[MAXCPN], dfs[MAXCPN], dff, gam, num, *dfs_ = dfs;
	...
	{
		d2 = (long)(pde->pmdl->today + pde->g->mat[0][idx][k] * DAYS_IN_YEAR + 1e-5);
		dffs[k] = swp_f_df(d1, d2, pde->pmdl->ycname);
		gams[k] = ( 1.0 - exp(-pde->pmdl->lambda *
			(pde->g->mat[0][idx][k] - pde->times[i])) ) / pde->pmdl->lambda;
	}
	...
	for (j=0; j < pde->grids[pde->dir].nphi; j++)
	{
		...
		for (xi=0; xi < pde->nx; xi++)
		{
			...
			for (k=0; k < pde->g->ninst; k++) pv[j][xi][k] *= num;
...
Err CheyBetaPDE_PriceSwaption(SCheyBetaPDE *pde, int iswp, double *result)
{
	...
	if (!pde->isfwd) return serror("The PDE was initialized as backward");
	if (pde->g->type != PRODUCT_SWAPTIONS) return serror("Product is not a swaption");

	if (pde->times[pde->last] > pde->g->ex[iswp] + 1e-5)
		err = CheyBetaPDE_ResetFwd(pde, pde->g->ex[iswp]);
	else err = CheyBetaPDE_ExpandFwd(pde, pde->g->ex[iswp]);
	if (err) return err;

	payoff = f3tensor(0, pde->grids[pde->dir].nphi-1, 0, pde->nx-1, 0, pde->g->ninst-1);
	if (!payoff) return serror("Memory failure");
...
FREE_RETURN:
	if (payoff)
		free_f3tensor(payoff, 0, pde->grids[pde->dir].nphi-1, 0, pde->nx-1, 0, pde->g->ninst-1);
	return err;
}
...
Err CheyBetaPDE_PriceBkwd(SCheyBetaPDE *pde, double *pv)
{
	...
	if (pde->isfwd) return serror("The PDE was initialized as forward");
	...
	if ( fabs(pde->g->ex[iex = pde->g->nex-1] - pde->times[pde->nt-1]) > 1e-5 )
	...
	{
		dt = pde->times[i+1] - pde->times[i];
		while ( isig > 0 && pde->pmdl->sigtms[isig-1] > pde->times[i] + 1e-5 ) isig--;
		sig = pde->pmdl->sig[isig];
		beta = pde->pmdl->beta[isig];

		while ( iex >= 0 && pde->g->ex[iex] > pde->times[i] + 1e-5 ) iex--;
		...
		for (k=0; k < pde->grids[pde->dir].nphi; k++)
		{
			...
			for (xi=0; xi < pde->nx; xi++)
			{
				...
				if ( phi_nxt < pde->grids[!pde->dir].phi[0] )
					...
					pde->grids[!pde->dir].phi[pde->grids[!pde->dir].nphi-1] )
					memcpy( pvinst[xi], pde->grids[!pde->dir].pv[pde->grids[!pde->dir].nphi-1][xi],
						pde->g->ninst * sizeof(double) );
				else	// find j: phi[j] <= phi_nxt < phi[j+1]
				{
					for (j=0; pde->grids[!pde->dir].phi[j+1] <= phi_nxt; j++);
					...
					for (m=0; m < pde->g->ninst; m++) pvinst[xi][m] =
			...
			num_f_pde_one_step_backward( pde->tmppde,
										 pde->nx,
										 pde->x,
										 pde->g->ninst,
										 pvinst,
		...
		if ( iex >= 0 &&
			 ( (fabs(pde->g->ex[iex] - pde->times[i]) < 1e-5) || (pde->g->am && pde->g->am[iex]) ) )
		{
...
FREE_RETURN:
	...
	free(mu);
	free(r);
	if (pvinst) free_dmatrix(pvinst, 0, pde->nx-1, 0, pde->g->ninst-1);

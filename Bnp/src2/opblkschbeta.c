...
{ /* exponent >= .5 */
    ...
    ans = (pow(y, 2 * nu) - pow(k_bar, 2 * nu)) * pow(y, .5 - nu) *
          safe_exp(-x_0 * x_0 * ((y - 1) * (y - 1))) * (2 * x_0 * x_0) * sqrt(y) * I_posnu;
    ...
    SrtGreekType   greek)
{
    ...
    case DELTA:
        ...
        ans   = (srt_f_optblkschbeta(
                   fwd_price + shift, strike, vol, mat, beta, disc, call_put, PREMIUM) -
               premium) /
              shift;
        break;
    ...
    case GAMMA:
        shift = fwd_price / 10000;
        ans =
            srt_f_optblkschbeta(fwd_price + shift, strike, vol, mat, beta, disc, call_put, PREMIUM);
        ans +=
            srt_f_optblkschbeta(fwd_price - shift, strike, vol, mat, beta, disc, call_put, PREMIUM);
        ans -= 2 * premium;
        ans /= shift * shift;
    ...
    case VEGA:
        shift = GVOPT.vol_add;
        ans   = (srt_f_optblkschbeta(
                   fwd_price, strike, vol + shift, mat, beta, disc, call_put, PREMIUM) -
               premium) /
              shift;
        break;

    ...
    double* fut_price)
{
    ...
    for (i = 0; i < num_fut_dates; i++)
    {
        ...
        end_futdates[i] =
            add_unit(fut_dates[i], (int)(12 / float_compounding), SRT_MONTH, MODIFIED_SUCCEEDING);
    }
    ...
    for (i = 0; i < num_fut_dates; i++)
    {
        err = srt_f_lgmcashfuture(
            fut_dates[i], end_futdates[i], float_basis, und_name, &fut_price[i]);
        if (err)
            goto e1;
    ...
    int     bumpinst)
{
    ...
    if (mdl_typ == LGM)
    {
        for (i = 0; i < num_fut_dates; i++)
        {
            if (err = srt_f_lgmcashfuture(
                    fut_dates[i], end_futdates[i], basis[i], und_name, &fut_price[i]))
            {
                goto e1;
            }
            spread = swp_f_spread(fut_dates[i], end_futdates[i], ref_rate_code);
    ...
    else if (mdl_typ == CHEY)
    {
        for (i = 0; i < num_fut_dates; i++)
        {
            if (err = srt_f_cheycashfuture(
                    fut_dates[i], end_futdates[i], "MM", und_name, numpaths, &fut_price[i]))
            {
                goto e1;
            }
            spread = swp_f_spread(fut_dates[i], end_futdates[i], ref_rate_code);
    ...
    double* fut_price)
{
    ...
    if ((!end_dates) || (!compd_str) || (!basis_str) || (!basis) || (!compounding) || (!strike) ||
        (!bondstrike) || (!type_str) || (!recpay_str) || (!price) || (!refratecode))
    {
        err = "Memory Allocation Failure";
        goto e1;
    }
    ...
    for (i = 0; i < num_fut_dates; i++)
    {
        ...
        strike[i] =
            swp_f_fra((Ddate)fut_dates[i], (Ddate)end_dates[i], basis[i], yc_name, ref_rate_code);
    ...
    if (mdl_typ == LGM)
    {
        ...
        {
            if (err = srt_f_lgmcashfuture(
                    fut_dates[i], end_dates[i], basis[i], und_name, &fut_price[i]))
            {
                goto e1;
            }
    ...
    else if (mdl_typ == CHEY)
    {
        ...
        {
            if (err = srt_f_cheycashfuture(
                    fut_dates[i], end_dates[i], basis_str[i], und_name, numpaths, &fut_price[i]))
            {
                goto e1;
            }
            spread = swp_f_spread(fut_dates[i], end_dates[i], ref_rate_code);
...
/* in a LGM model */
Err srt_f_lgmcashfuture(
    Ddate start_date, Ddate end_date, SrtBasisCode basis, char* und_name, double* future_price)
{
    SrtErr      err;
    SrtUndPtr   und;
    TermStruct* ts;
    ...
    else if (mdldim == TWO_FAC)
    {
        ...
        zc_exp = (YT[1].phi_coeff[0][0] - YT[0].phi_coeff[0][0]) * phi[0][0] +
                 ...
                 (YT[1].x_coeff[0] - YT[0].x_coeff[0]) * (Two_fac_H[0][0] + Two_fac_H[0][1]) +
                 (YT[1].x_coeff[1] - YT[0].x_coeff[1]) * (Two_fac_H[1][1] + Two_fac_H[1][0]);
    ...
    double* future_price)
{
    ...
    err = srt_f_makehistogram(
        (char*)histo_name, 0, "", 0, &histo_x, &x_size, &histo_y, &y_size, &histo_values);
    if (err)
        goto e4;

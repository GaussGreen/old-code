...
Err err = NULL;
	...
	if (!mkt->strikes_in_vol) return "Allocation error in cts_init_mkt";
...
CTS_MKT						dest)
{
	...
	if (!dest->strikes_in_vol) return "Allocation error in cts_copy_mkt";
...
CTS_MKT						mkt)
{
	if (mkt)
	{
		if (mkt->strikes_in_vol) free (mkt->strikes_in_vol);
		mkt->strikes_in_vol = NULL;
	}
}
...
static	Err Construct_Schedule(	long			lTodayDate,
								char			*cYcName,
								long			lStartDate,
								long			lTheoEndDate,
...
{	
	...
	if (fixing->ref_fix_date >= today)
	{
		...
		err = Construct_Schedule(	today,
									yc,
									ref_start_date,
									ref_theo_end_date,
									srt_freq,
									srt_basis,
									&fixing->schedule.iNCpn,
		...
		fixing->ref_cpnd = 
			(fixing->ref_cpnd - floor (fixing->ref_cpnd) < 0.50? 
			floor (fixing->ref_cpnd): 
...
/*	Check */
Err cts_check_fixing(
CTS_FIX				fixing)
{
	if (fixing->schedule.iNCpn > 0)
	{
		if (fixing->schedule.lCpnDate[0] >= fixing->schedule.lCpnDate[fixing->schedule.iNCpn-1]) return "Date inconsistency in cts_check_fixing";
		if (fixing->ref_fix_date > fixing->schedule.lCpnDate[0]) return "Date inconsistency in cts_check_fixing";	
	}

	return NULL;
}

Err cts_init_cpn_payoff(double	alpha,
						double	beta,
						int		nstr,
						double	*str,
						double	*nbopt,
...
Err cts_init_cpn_payoff_for_range(	int		buy_sell,		/*	1: BNPP buys, -1: BNPP sells */
...
double	*str	= NULL,
	...
	if (lb >= ub - 2.0 * call_spread) return "Range inconsistency in cts_init_fixing_for_range";
	if (ub <= call_spread && !value_zero) return "Range inconsistency in cts_init_fixing_for_range";
	if (call_spread < 1.0e-08) return "Zero call spread in cts_init_fixing_for_range";

	/*	Negative strike means no strike */
	if (lb < min_barrier) value_zero = 0;
...
Err cts_init_cpn_payoff_for_cif(int		value_zero,
								double	min_barrier,
								double	max_barrier,
								double	gearing,
								double	margin,
...
double	*str	= NULL,
	...
	/*	Quick check on range */
	if (floor > cap) return "Cap / Floor inconsistency in cts_init_cpn_payoff_for_cif";
...
Err cts_calc_shifted_range_pv(	int		buy_sell,		/*	1: BNPP buys, -1: BNPP sells */
								int		value_zero,		/*	0: Do not value low strike options, 1: do */
								double	min_barrier,
								double	max_barrier,
...
CTS_FIX	fix;
	...
	/*	Negative strike means no strike */
	if (lb_init < -min_barrier) value_zero = 0;
	...
	for (i=0; i<cpn->used_nfix; i++)
	{
		...
		{		
			...
			if (power > 0.5)
			{
				opt =  srt_f_optblksch(	ref_swp_fwd,
										str[j],
										vol,
										fix->ref_fix_time,
										1.0,
			...
			{
				opt =  srt_f_optblknrm(	ref_swp_fwd,
										str[j],
										vol,
										fix->ref_fix_time,
										1.0,
...
Err cts_adjust_call_spread(	int		buy_sell,		/*	1: BNPP buys, -1: BNPP sells */
							int		value_zero,		/*	0: Do not value low strike options, 1: do */
							double	min_barrier,
							double	max_barrier,
							double	lb,				/*	Lower bound of range */
...
double	alpha, beta;
int		nstr;
double	nbopt[CTS_CALLSPREADMAXSTRIKE],
		str[CTS_CALLSPREADMAXSTRIKE];
	...
	err = cts_calc_shifted_range_pv(buy_sell,
									value_zero,
									min_barrier,
									max_barrier,
									lb,
	...
	while (fabs(price_tgt - price1) > precision && i < maxiter)
	{
		...
		shift2 = solve_for_next_coef_dlm(	res_iter,
										i,
										price_tgt,
										2);
	}
...
FREE_RETURN:
	...
	if (res_iter) free_dmatrix(res_iter, 0, maxiter, 0, 1);
...
CTS_CPN				cpn)
{
	...
	{		
		...
		cpn->pay_start_date = add_unit (cpn->pay_fix_date, fix_lag_bd, SRT_BDAY, MODIFIED_SUCCEEDING);

		if (cpn->type == 3 || cpn->type == 4)
		{
			err = add_tenor (cpn->pay_start_date, fix_tenor, NO_BUSDAY_CONVENTION, &cpn->pay_theo_end_date);		
		}
		else
		{
			cpn->pay_theo_end_date = add_unit (cpn->pay_start_date, pay_months, SRT_MONTH, NO_BUSDAY_CONVENTION);
		}
	...
	/*	Range */
	{
		...
		{
			/* classical Time Swap */
			err = cts_init_cpn_payoff_for_range(buy_sell,
												value_zero,
												CTS_MINBARRIER,
												CTS_MAXBARRIER,
												lb,
			...
			if (numer_spread - call_spread > 1.0E-08)
			{
				...
				err = cts_adjust_call_spread(	buy_sell,
												value_zero,
												CTS_MINBARRIER,
												CTS_MAXBARRIER,
												lb,
		...
		{
			/* CIF type */
			err = cts_init_cpn_payoff_for_cif(	value_zero,
												1.0E-08,
												CTS_MAXBARRIER,												
												pay_gearing,
												cpn_coupon,
	...
	if (tstype)
	{
		if (cpn->type < 3)
		{
			...
			if (lb < -CTS_MINBARRIER) value_zero = 0;
	...
	if (calc_mkt_iv)
	{
		err = cts_calc_cpn_mkt_value (cpn, mkt, use_cmsopt, correl_start, correl_end, float_adjust_type);
		if (err) goto FREE_RETURN;
	}
		...
		if (cpn_type < 3)
		{
			...
			switch (cpn_type)
			{
				...
				case 2:					
					...
					/*
					err = add_tenor (cpn->pay_start_date, fix_tenor, NO_BUSDAY_CONVENTION, &cpn->pay_end_date);
					*/
					...
					{
						pay_start_date = add_unit (pay_fix_date, fix_lag_bd, SRT_BDAY, MODIFIED_SUCCEEDING);
						pay_end_date = add_unit (pay_start_date, pay_months, SRT_MONTH, MODIFIED_SUCCEEDING);

						err = interp_basis (pay_basis, &bas);
						pay_cvg = coverage(pay_start_date, pay_end_date, bas);

						libor = (swp_f_df(mkt->today, pay_start_date, mkt->yc) / swp_f_df(mkt->today, pay_end_date, mkt->yc) - 1.0) / pay_cvg + pay_fwd_spread;
					}
...
double		ref_swp_cash, ref_swp_fwd, ref_swp_drs,
			ref_delay, pay_adjlogvol, ref_adjlogvol, ref_atmlogvol, ref_atmlogstd, ref_strikelogvol, ref_adj;
...
double		temp;
	...
	if (cpn->type == 1 || cpn->type == 2)
	{
		...
		err = swp_f_Cms_Rate(	pay_swp_fwd,
								cpn->pay_fix_time,
								1,
								cpn->pay_cpnd,
								pay_delay,
		...
		if (cpn->pay_fix_time > 0.0)
		{
			...
			err = mkt->get_cash_vol_convert(mkt->get_cash_vol,
											mkt->vc,
											cpn->pay_start_date,
											cpn->pay_theo_end_date,
											cpn->pay_fix_time,
	...
	for (i=0; i<cpn->used_nfix; i++)
	{
		...
		if (cpn->beta || cpn->type == 1 || cpn->type == 2 ||
			((cpn->type == 0 || cpn->type == 3 || cpn->type == 4) && cpn->nstr && (!use_cmsopt || cpn->type == 4)))
		{
			err = swp_f_Cms_Rate(	ref_swp_fwd,
									fix->ref_fix_time,
		...
		if (cpn->type == 0 || do_margin)
		{
			for (j=0; j<cpn->nstr; j++)
			{
				if (use_cmsopt)
				{
					...
					err = swp_f_Cms_Option(	ref_swp_fwd,
											fix->ref_fix_time,
											fix->schedule.iNCpn - 1,
											cpn->str[j],
											fix->ref_cpnd,
				...
				else
				{
					if (cpn->str[j] - fix->ref_fwd_spread > 1.0E-08)
					{
						...
						opt =  srt_f_optblksch(	ref_swp_drs - fix->ref_fwd_spread,
												cpn->str[j] - fix->ref_fwd_spread,
												ref_strikelogvol,
												fix->ref_fix_time,
												1.0,
				...
				if (cpn->type == 0 || cpn->type == 3 || cpn->type == 4)
				{
					if (cpn->type == 3)
					{
						...
						{	
							...
							{
								/* Price the option with BS */
								delta =  srt_f_optblksch(	ref_swp_drs - fix->ref_fwd_spread,
															cpn->str[j] - fix->ref_fwd_spread,
															ref_strikelogvol,
															fix->ref_fix_time,
															1.0,
		...
		if (cpn->type == 1 || cpn->type == 2)
		{
			...
			ref_correl = (correl_end - correl_start) / pay_regpaydelay * (fix->schedule.dCpnTime[0] - cpn->cpn_start_time) + correl_start;
			...
			for (j=0; j<cpn->nstr; j++)
			{
				...
				if (j % 2 == 0)
				{
					...
					if (cpn->type == 1)
					{
						ref_adj = exp(ref_correl * ref_adjlogvol * pay_adjlogvol * cpn->pay_fix_time);
					}
					else
					{
						ref_adj = exp(ref_correl * ref_adjlogvol * pay_adjlogvol * fix->ref_fix_time);
					}
				}
				...
				if (cpn->str[j] - fix->ref_fwd_spread > 1.0E-08)
				{
					opt =  srt_f_optblksch((ref_swp_drs - fix->ref_fwd_spread) * ref_adj,
											cpn->str[j] - fix->ref_fwd_spread,
											ref_strikelogvol,
											fix->ref_fix_time,
											1.0,
				...
				{
					opt = (ref_swp_drs - fix->ref_fwd_spread) * ref_adj - (cpn->str[j] - fix->ref_fwd_spread);
				}
...
Err cts_calc_equivalent_strikes(CTS					cts_iv,
								CTS					cts_call,
								CTS_MKT				mkt)
{
Err		err = NULL;
...
long	end_exo;
	...
	for (i=cts_call->num_calls-1; i>=00; i--)
	{
		...
		swap_pv = exo_pv / cts_iv->exo_leg->cpn[last_exo].cpn_not  - fund_pv / cts_iv->fund_leg->cpn[last_fund].not;
		
		err = swp_f_LevelPayment(	cts_iv->exo_leg->cpn[last_exo].cpn_start_date,
									end_exo,
									mkt->swap_freq,
									mkt->swap_basis,
									mkt->yc,
...
CTS_CPN				cpn)
{
	...
	if (cpn->type)
	{
		if (cpn->pay_fix_date > cpn->pay_start_date) return "Date inconsistency in cts_check_coupon";
		if (cpn->pay_start_date >= cpn->pay_end_date) return "Date inconsistency in cts_check_coupon";
	}

	if (cpn->cpn_start_date >= cpn->cpn_pay_date) return "Date inconsistency in cts_check_coupon";
...
/*	Free */
void cts_free_coupon(
CTS_CPN				cpn)
{
	if (cpn)
	{
		if (cpn->weights) free (cpn->weights);
		cpn->weights = NULL;

		if (cpn->str) free (cpn->str);
		cpn->str = NULL;
		if (cpn->nbopt) free (cpn->nbopt);
		cpn->nbopt = NULL;
		...
		if (cpn->used_fixidx) free (cpn->used_fixidx);
		cpn->used_fixidx = NULL;
		if (cpn->used_fixweights) free (cpn->used_fixweights);
		cpn->used_fixweights = NULL;
	}
}
...
CTS_EXO_LEG	exo_leg)
{
	...
	j = 0;
	while (j < ncpn 
		&& ref_fix_dates[j][0] < today + eod_flag)
	{
		j++;
	}
...
Err				err = NULL;
	...
	{
		if (exo_leg->cpn[j].nstr > 0 || fabs(exo_leg->cpn[j].beta) > 1.0E-08 || exo_leg->cpn[j].type > 0)
		{
			is_midat = 0;
			break;
		}
...
Err cts_adjust_floating_fixing(	CTS		cts,
								int		pde_or_mc)
{
	int				i;
	...
	if (pde_or_mc == 1)
	{
		for (i=0; i<cts->num_calls; i++)
		{
			if (cts->call[i].exo_idx > 0)
			{
				...
				{
					if (cts->exo_leg->cpn[cts->call[i].exo_idx-1].pay_fix_date >= cts->call[i].ex_date)
					{													
						cts->exo_leg->cpn[cts->call[i].exo_idx-1].pay_fix_date = cts->call[i].ex_date - 1;
						cts->exo_leg->cpn[cts->call[i].exo_idx-1].pay_fix_time = cts->call[i].ex_time - YEARS_IN_DAY;
					}
				}
			}		
		}	
	...
	for (i=1; i<cts->num_calls; i++)
	{
		...
		if (cts->call[i].exo_idx > 0)
		{
			if (cts->exo_leg->cpn[cts->call[i].exo_idx-1].used_nfix > 0 
				&& cts->exo_leg->cpn[cts->call[i].exo_idx-1].fix[cts->exo_leg->cpn[cts->call[i].exo_idx-1].used_nfix - 1].ref_fix_date > cts->call[i].ex_date)
			{
				return serror("Either too many fixings requested, or Floating Rate fixe after call date");
			}
		}
	}
...
Err cts_get_mkt_sabr_beta_param(	CTS_MKT	mkt,
									long	exe_date,
									long	start_date,	
									long	theo_end_date,
									double	fixed_beta,
...
int		nb_months;
	...
	{
		err = calib_sabr_rr_bt_given_beta(	swap_cash,
											maturity,
											atm_log_vol,
											Strikes[1],
											Vols[1],
...
Err cts_calib_const_alpha_and_rho(CTS_MKT				mkt,
								  CTS					cts,
...
Err		err = NULL;
int		i, i0, j, k;
double	*mkt_alpha	= NULL,
		*mkt_rho	= NULL;

long	exercise_dates_[MAX_INST];

long	*exercise_dates	= NULL,
		*start_dates	= NULL,
		*end_dates		= NULL;
double	*exercise_times	= NULL,
		*weights		= NULL;
...
double	temp;
	...
	mkt_alpha = calloc(num_ex_date, sizeof(double));
	mkt_rho = calloc(num_ex_date, sizeof(double));
	exercise_times = calloc(num_ex_date, sizeof(double));
	weights = calloc(num_ex_date, sizeof(double));
	start_dates = calloc(num_ex_date, sizeof(long));
	end_dates = calloc(num_ex_date, sizeof(long));
	...
	for (i=0; i<num_ex_date; i++)
	{
		...
		err = cts_get_mkt_sabr_beta_param(	mkt,
											exercise_dates[i],
											start_date,
											theo_end_date,
											fixed_beta,
	...
	if (num_ex_date > 1)
	{
		...
		*rho = temp / num_ex_date;
		...
		err = levenberg_marquardt_select(	exercise_times - 1,
											mkt_alpha - 1,
											weights - 1,
											num_ex_date,
											param,
	...
	if (save_inst_data)
	{
		...
		if (num_ex_date > 0)
		{
			...
			if (!inst_data->start_dates_smile || !inst_data->end_dates_smile || 
				!inst_data->alpha || !inst_data->rho)
			{
...
FREE_RETURN:
	...
	if (exercise_times) free (exercise_times);
	if (weights) free (weights);
	if (start_dates) free (start_dates);
	if (end_dates) free (end_dates);
	if (mkt_alpha) free (mkt_alpha);
	if (mkt_rho) free (mkt_rho);
...
int			smile_calib_months,				/*	0: co-terminal swaption, otherwise underlyings with required nb months */
...
char		**prim_tenor	= NULL,
			**sec_tenor		= NULL;

int			used_nsmilepar;
double		*used_smilepartime	= NULL,
			*used_alpha			= NULL,
			*used_rho			= NULL,
			*used_ldaeps		= NULL,
			*used_rho2			= NULL;
...
cpd_diag_calib_param	*prim_param = NULL,
						*sec_param = NULL;
...
Err			err = NULL;
	...
	/* Initialisation */
	prim_param = calloc(1, sizeof(cpd_diag_calib_param));
	sec_param = calloc(1, sizeof(cpd_diag_calib_param));
	lm_params = calloc(1, sizeof(diag_calib_lm_params));
	calib_params = calloc(1, sizeof(LGMSV_CalibParams));

	used_nsmilepar = nsmilepar;
	used_smilepartime = calloc(used_nsmilepar, sizeof(double));
	used_alpha = calloc(used_nsmilepar, sizeof(double));
	used_rho = calloc(used_nsmilepar, sizeof(double));
	used_ldaeps = calloc(used_nsmilepar, sizeof(double));
	used_rho2 = calloc(used_nsmilepar, sizeof(double));

	if (!prim_param || !sec_param || !lm_params || !calib_params ||
		!used_smilepartime || !used_alpha || !used_rho || !used_ldaeps || !used_rho2)
	{
	...
	/* Recopy */
	if (smilepartime) memcpy(used_smilepartime, smilepartime, nsmilepar * sizeof(double));
	if (alpha) memcpy(used_alpha, alpha, nsmilepar * sizeof(double));
	if (rho) memcpy(used_rho, rho, nsmilepar * sizeof(double));
	if (ldaeps) memcpy(used_ldaeps, ldaeps, nsmilepar * sizeof(double));
	if (rho2) memcpy(used_rho2, rho2, nsmilepar * sizeof(double));
	...
	{			
		err = cts_calib_const_alpha_and_rho(mkt,
											cts,
											smile_calib_months,
											LGMSV_SABRBETA,
											min_time,
	...
	for (i=0; i<nsmilepar; i++)
	{
		...
		if (rho2) used_rho2[i] = DMIN(DMAX(used_rho2[i], -0.99), 0.99);
	}	
	...
	err = cpd_calib_diagonal_LGMSV_new_dlm(	mkt->yc,
										mkt->vc,
										mkt->get_cash_vol,
										mkt->ref,
	...
	und->model.dPWTime = (double *) realloc(und->model.dPWTime, und->model.iNbPWTime * sizeof(double));
	...
	und->model.dLambdaEps = (double *) realloc(und->model.dLambdaEps, und->model.iNbPWTime * sizeof(double));
	...
	{
		und->model.dRho2 = (double *) realloc(und->model.dRho2, und->model.iNbPWTime * sizeof(double));
	...
	{
		und->model.dPWTime[und->model.iNbPWTime - nb_new_dates + i] = und->model.dPWTime[und->model.iNbPWTime - nb_new_dates + i - 1] + step_calib;
		und->model.dSigma[und->model.iNbPWTime - nb_new_dates + i] = und->model.dSigma[und->model.iNbPWTime - nb_new_dates + i - 1];
		und->model.dAlpha[und->model.iNbPWTime - nb_new_dates + i] = und->model.dAlpha[und->model.iNbPWTime - nb_new_dates + i - 1];
		und->model.dRho[und->model.iNbPWTime - nb_new_dates + i] = und->model.dRho[und->model.iNbPWTime - nb_new_dates + i - 1];
		und->model.dLambdaEps[und->model.iNbPWTime - nb_new_dates + i] = und->model.dLambdaEps[und->model.iNbPWTime - nb_new_dates + i - 1];

		if (und->model.iOne2F == 2)
		{
			und->model.dRho2[und->model.iNbPWTime - nb_new_dates + i] = und->model.dRho2[und->model.iNbPWTime - nb_new_dates + i - 1];
		}
	}

	memcpy(und->model.dLvlEps, und->model.dLambdaEps, und->model.iNbPWTime * sizeof(double));

	ConvertTS_LGM_to_LGMSV(	und->model.iNbPWTime,
							und->model.dPWTime,
							und->model.dSigma,
							und->model.dLambdaX,
							und->model.dTStar,
...
FREE_RETURN:
	...
	if (prim_param) free(prim_param);
	if (sec_param) free(sec_param);
	if (lm_params) free(lm_params);
	if (calib_params) free(calib_params);

	if (cal_dates) free (cal_dates);
	if (ex_dates) free (ex_dates);
	if (prim_strikes) free (prim_strikes);
	if (prim_strikesS1) free (prim_strikesS1);
	if (prim_strikesS2) free (prim_strikesS2);

	if (sec_strikes) free (sec_strikes);
	if (sec_strikesS1) free (sec_strikesS1);
	if (sec_strikesS2) free (sec_strikesS2);

	if (prim_tenor) free (prim_tenor);
	if (sec_tenor) free (sec_tenor);

	if (used_smilepartime) free(used_smilepartime);
	if (used_alpha) free(used_alpha);
	if (used_rho) free(used_rho);
	if (used_ldaeps) free(used_ldaeps);
	if (used_rho2) free(used_rho2);

	if (err) cts_free_und (und);
...
Err	err = NULL;
	...
	if (dest && src)
	{
		...
		err = init_LGMSV_model(	&(dest->model),
								src->model.lToday,
								src->model.iOne2F,
								src->model.iNbPWTime,
								src->model.dLambdaX,
...
void cts_free_cts_iv(CTS_IV iv_infos)
{
	if (iv_infos)
	{
		if (iv_infos->call_date) free (iv_infos->call_date);
		iv_infos->call_date = NULL;

		if (iv_infos->market_fwdiv) free (iv_infos->market_fwdiv);
		iv_infos->market_fwdiv = NULL;

		if (iv_infos->model_fwdiv) free (iv_infos->model_fwdiv);
		iv_infos->model_fwdiv = NULL;

		if (iv_infos->extra_fee) free (iv_infos->extra_fee);
		iv_infos->extra_fee = NULL;

		if (iv_infos->has_one_time)
		{
			if (iv_infos->one_time_call) free (iv_infos->one_time_call);
			iv_infos->one_time_call = NULL;

			if (iv_infos->one_time_strike) free (iv_infos->one_time_strike);
			iv_infos->one_time_strike = NULL;

			if (iv_infos->one_time_norm_vol) free (iv_infos->one_time_norm_vol);
			iv_infos->one_time_norm_vol = NULL;

			if (iv_infos->one_time_log_vol) free (iv_infos->one_time_log_vol);
			iv_infos->one_time_log_vol = NULL;

			if (iv_infos->one_time_coupon) free (iv_infos->one_time_coupon);
			iv_infos->one_time_coupon = NULL;

			if (iv_infos->one_time_notional) free (iv_infos->one_time_notional);
			iv_infos->one_time_notional = NULL;
...
void cts_free_extra_infos(CTS_EXTRA_INFOS extra_infos)
{
	if (extra_infos)
	{
		...
		{
			free_dmatrix(extra_infos->extra_infos, 0, extra_infos->nrow - 1, 0, extra_infos->ncol - 1);
...
CalibCpnScheduleDLM	*cCpnSchedule = NULL,
					*cCpnSchedule2;

double				InstPrices_[10],
					InstPrices2_[10],
					InstPrices_temp_[10],
					InstStrikes_[10],
					InstStds_[10],
					*InstPrices = &(InstPrices_[0]),
					*InstPrices2 = &(InstPrices2_[0]),
					*InstPrices_temp = &(InstPrices_temp_[0]),
					*InstStrikes = &(InstStrikes_[0]),
...
Err					err				= NULL;
	...
	/* Memory allocations */
	sPricingConst = calloc(1, sizeof(LGMSV_PricingConst));
	sNumerParams = calloc(1, sizeof(LGMSV_NumerInst));
	cCalibInst = calloc(1, sizeof(CalibInstrumentDLM));
	cHestonInst = calloc(1, sizeof(LGMSV_HestonInst));
	cCpnSchedule = calloc(1, sizeof(CalibCpnScheduleDLM));
	...
	if (calc_fwd_iv)
	{
		...
		err = Initialise_PricingConst(	&(und->model),
										NumerParams,
										sPricingConst);
		...
		for (i=first_coupon; i<cts->exo_leg->num_cpn; i++)
		{
			...
			else
			{
				...
				cCalibInst->dFwdCash = (cCpnSchedule->dCpnDf[0] - cCpnSchedule->dCpnDf[1]) / cCalibInst->dLevel;
				cCalibInst->dSpread = exo_cpn->pay_fwd_spread;

				cCalibInst->iNbStrike = 0;

				/* Calculate the constant */
				err = Initialise_NumerInst(	&(und->model),
											cCalibInst,
											cHestonInst,
											sNumerParams);
				...
				err = Calculate_HestonEquivalent_LGMSV_FromLGM_struct(	cCalibInst,
																		cCpnSchedule,
																		&(und->model),
																		cHestonInst);
				...
				err = Calculate_ShiftAndVol_LGMSV_FromLGM_struct(	cCalibInst,
																	cCpnSchedule,
																	&(und->model),
																	cHestonInst);
				...
				Update_NumerInst(	cCalibInst,
									cHestonInst,
									sNumerParams);

				/* Call the Fwd Pricer */				

				LGMSVFwdClosedFormApprox_struct(	&(und->model),
													cCalibInst,
													cHestonInst,
													sPricingConst,
													sNumerParams,
			...
			for (j=0; j<exo_cpn->used_nfix ; j++)
			{
				...
				while (first_strike < exo_cpn->nstr && exo_cpn->str[first_strike] < cHestonInst->dMinStrike)
				{
					first_strike++;
				}
				...
				while (last_strike < exo_cpn->nstr && exo_cpn->str[last_strike] < cHestonInst->dMaxStrike)
				{
				...
				if (cCalibInst->iIsLiborOption)
				{
					...
					sNumerParams->iSwitchIndex = Get_Index(sNumerParams->dNewSwitchTime, und->model.dPWTime, und->model.iNbPWTime);
					...
					{
						if (fabs(sNumerParams->dNewSwitchTime - und->model.dPWTime[sNumerParams->iSwitchIndex]) > 1.0E-08 || sNumerParams->iSwitchIndex == sNumerParams->iEndIndex)
						{
							/* we add the switch */
				...
				{
					LGMSVClosedFormApprox_struct(	&(und->model),
													cCalibInst,
													cHestonInst,
													sPricingConst,
													sNumerParams,
				...
				if (do_min_strike)
				{
					err = srt_f_optimpvol(	InstPrices_temp[0],
											dLiborRefFwd + 100,
					...
					{
						InstPrices[k] = srt_f_optblknrm(dLiborRefFwd,
														exo_cpn->str[k],
														min_vol,
														cCalibInst->dExeTime,
														df_pay,
				...
				if (do_max_strike)
				{
					err = srt_f_optimpvol(	InstPrices_temp[nb_strike-1],
											dLiborRefFwd + 100,
				...
				if (cCalibInst->iIsLiborOption)
				{
					...
					if (do_min_strike)
					{
						...
						for (k=0; k<first_strike; k++)
						{
							InstPrices2[k] = srt_f_optblknrm(dLiborRefFwd,
															exo_cpn->str[k],
															min_vol,
															cCalibInst->dExeTime,
															df_pay,
					...
					{
						InstPrices[k] = 1.0 / exo_cpn->pay_cvg * (InstPrices[k] * (dLiborFwd * exo_cpn->pay_cvg + 1.0) - InstPrices2[k]) + exo_cpn->cpn_coupon;
					}

					fixing_iv += exo_cpn->alpha * dLiborFwd + exo_cpn->beta * InstPrices[exo_cpn->nstr];
				...
				else
				{
					...
					{
						fixing_iv += exo_cpn->alpha + exo_cpn->beta * InstPrices_temp[exo_cpn->nstr];
					}
					else
					{
	...
	if (sNumerParams) free(sNumerParams);
	if (cCalibInst) free(cCalibInst);
	if (cHestonInst) free(cHestonInst);
	if (cCpnSchedule) free(cCpnSchedule);
...
Err		err = NULL;
	...
	if (calc_fwd_iv)
	{
		...
		{				
			...
			fund_iv = swp_f_df(today, model_cts->fund_leg->cpn[model_cts->call[i].fund_idx].start_date, und->mkt->yc);
			...
			model_cts->call[i].extra_fee = model_cts->call[i].pay_rec * (mdl_exo_iv - mkt_exo_iv) / swp_f_df(today, model_cts->call[i].set_date, und->mkt->yc);

			if (adj_fee)
			{
				model_cts->call[i].total_fee = model_cts->call[i].fee + model_cts->call[i].extra_fee;
			}
			else
			{
		...
		if (pde_or_mc == 1)
		{
			...
			{
				model_cts->call[i].total_fee -= model_cts->call[i+1].total_fee
					* swp_f_df(model_cts->call[i].set_date, model_cts->call[i+1].set_date, und->mkt->yc);
			}
		}
	}
	else
...
long	today;
	...
	eval_const_fix->tpay_tstar_alpha = swp_f_df(today, cpn->cpn_pay_date, und->mkt->yc) / swp_f_df(today, und->model.lTStarDate, und->mkt->yc);
	eval_const_fix->tpay_tstar_alpha = -log(eval_const_fix->tpay_tstar_alpha);
	eval_const_fix->tpay_tstar_beta = (1.0 - exp(-und->model.dLambdaX * (cpn->cpn_pay_time - und->model.dTStar))) / und->model.dLambdaX;
	eval_const_fix->tpay_tstar_gamma = 0.5 * eval_const_fix->tpay_tstar_beta * eval_const_fix->tpay_tstar_beta;	

	/* reconstruction of DF(t,Ts) / DF(t,Te) */
	eval_const_fix->ts_te_alpha = swp_f_df(today, fix->schedule.lCpnDate[0], und->mkt->yc) / swp_f_df(today, fix->schedule.lCpnDate[1], und->mkt->yc);
	eval_const_fix->ts_te_alpha = -log(eval_const_fix->ts_te_alpha);
	
	beta1 = (1.0 - exp(-und->model.dLambdaX * (fix->schedule.dCpnTime[0] - und->model.dTStar))) / und->model.dLambdaX;
	beta2 = (1.0 - exp(-und->model.dLambdaX * (fix->schedule.dCpnTime[1] - und->model.dTStar))) / und->model.dLambdaX;
	...
	{
		/* Fill the Extras */
		eval_const_fix->tpay_tstar_beta2 = (1.0 - exp(-und->model.dLambdaX2 * (cpn->cpn_pay_time - und->model.dTStar))) / und->model.dLambdaX2;
		eval_const_fix->tpay_tstar_gamma2 = 0.5 * eval_const_fix->tpay_tstar_beta2 * eval_const_fix->tpay_tstar_beta2;
		eval_const_fix->tpay_tstar_gamma12 = eval_const_fix->tpay_tstar_beta * eval_const_fix->tpay_tstar_beta2;

		beta1_2 = (1.0 - exp(-und->model.dLambdaX2 * (fix->schedule.dCpnTime[0] - und->model.dTStar))) / und->model.dLambdaX2;
		beta2_2 = (1.0 - exp(-und->model.dLambdaX2 * (fix->schedule.dCpnTime[1] - und->model.dTStar))) / und->model.dLambdaX2;
	...
	/*	Precalculations */	
	eval_const_fix->a = cpn->alpha + cpn->beta * (fix->ref_fwd_spread - 1.0 / fix->schedule.dCpnCvg[1]);
...
long	today;
	...
	/* reconstruction of DF(t,Tpay) / DF(t,Tstar) */
	eval_const_fix_cms->tpay_tstar_alpha = swp_f_df(today, cpn->cpn_pay_date, und->mkt->yc) / df_tstar;
	eval_const_fix_cms->tpay_tstar_alpha = -log(eval_const_fix_cms->tpay_tstar_alpha);
	eval_const_fix_cms->tpay_tstar_beta = (1.0 - exp(-und->model.dLambdaX * (cpn->cpn_pay_time - und->model.dTStar))) / und->model.dLambdaX;
	eval_const_fix_cms->tpay_tstar_gamma = 0.5 * eval_const_fix_cms->tpay_tstar_beta * eval_const_fix_cms->tpay_tstar_beta;	

	/* reconstruction of DF(t,Ti) / DF(t,Tstar) * coverage ! */
	for (i=1; i<fix->schedule.iNCpn-1; i++)
	{
		eval_const_fix_cms->ti_tstar_alpha[i] = fix->schedule.dCpnCvg[i] * swp_f_df(today, fix->schedule.lCpnDate[i], und->mkt->yc) / df_tstar;
		eval_const_fix_cms->ti_tstar_alpha[i] = -log(eval_const_fix_cms->ti_tstar_alpha[i]);
		eval_const_fix_cms->ti_tstar_beta[i] = (1.0 - exp(-und->model.dLambdaX * (fix->schedule.dCpnTime[i] - und->model.dTStar))) / und->model.dLambdaX;
		eval_const_fix_cms->ti_tstar_gamma[i] = 0.5 * eval_const_fix_cms->ti_tstar_beta[i] * eval_const_fix_cms->ti_tstar_beta[i];
	}

	if (fix->schedule.iNCpn > 0)
	{
		/* reconstruction of the first DF without the coverave */
		eval_const_fix_cms->ti_tstar_alpha[0] = swp_f_df(today, fix->schedule.lCpnDate[0], und->mkt->yc) / df_tstar;
		eval_const_fix_cms->ti_tstar_alpha[0] = -log(eval_const_fix_cms->ti_tstar_alpha[0]);
		eval_const_fix_cms->ti_tstar_beta[0] = (1.0 - exp(-und->model.dLambdaX * (fix->schedule.dCpnTime[0] - und->model.dTStar))) / und->model.dLambdaX;
		eval_const_fix_cms->ti_tstar_gamma[0] = 0.5 * eval_const_fix_cms->ti_tstar_beta[0] * eval_const_fix_cms->ti_tstar_beta[0];

		/* reconstruction of the last DF without the coverave */
		eval_const_fix_cms->ti_tstar_alpha[fix->schedule.iNCpn-1] = swp_f_df(today, fix->schedule.lCpnDate[fix->schedule.iNCpn-1], und->mkt->yc) / df_tstar;
		eval_const_fix_cms->ti_tstar_alpha[fix->schedule.iNCpn-1] = -log(eval_const_fix_cms->ti_tstar_alpha[fix->schedule.iNCpn-1]);
		eval_const_fix_cms->ti_tstar_beta[fix->schedule.iNCpn-1] = (1.0 - exp(-und->model.dLambdaX * (fix->schedule.dCpnTime[fix->schedule.iNCpn-1] - und->model.dTStar))) / und->model.dLambdaX;
		eval_const_fix_cms->ti_tstar_gamma[fix->schedule.iNCpn-1] = 0.5 * eval_const_fix_cms->ti_tstar_beta[fix->schedule.iNCpn-1] * eval_const_fix_cms->ti_tstar_beta[fix->schedule.iNCpn-1];
	}

	if (und->model.iOne2F == 2)
	{
		/* Fill the Extras */
		eval_const_fix_cms->tpay_tstar_beta2 = (1.0 - exp(-und->model.dLambdaX2 * (cpn->cpn_pay_time - und->model.dTStar))) / und->model.dLambdaX2;
		eval_const_fix_cms->tpay_tstar_gamma2 = 0.5 * eval_const_fix_cms->tpay_tstar_beta2 * eval_const_fix_cms->tpay_tstar_beta2;	
		eval_const_fix_cms->tpay_tstar_gamma12 = eval_const_fix_cms->tpay_tstar_beta * eval_const_fix_cms->tpay_tstar_beta2;

		for (i=1; i<fix->schedule.iNCpn-1; i++)
		{	
			eval_const_fix_cms->ti_tstar_beta2[i] = (1.0 - exp(-und->model.dLambdaX2 * (fix->schedule.dCpnTime[i] - und->model.dTStar))) / und->model.dLambdaX2;
			eval_const_fix_cms->ti_tstar_gamma2[i] = 0.5 * eval_const_fix_cms->ti_tstar_beta2[i] * eval_const_fix_cms->ti_tstar_beta2[i];
			eval_const_fix_cms->ti_tstar_gamma12[i] = eval_const_fix_cms->ti_tstar_beta[i] * eval_const_fix_cms->ti_tstar_beta2[i];
		}

		if (fix->schedule.iNCpn > 0)
		{
			/* reconstruction of the first DF without the coverave */
			eval_const_fix_cms->ti_tstar_beta2[0] = (1.0 - exp(-und->model.dLambdaX2 * (fix->schedule.dCpnTime[0] - und->model.dTStar))) / und->model.dLambdaX2;
			eval_const_fix_cms->ti_tstar_gamma2[0] = 0.5 * eval_const_fix_cms->ti_tstar_beta2[0] * eval_const_fix_cms->ti_tstar_beta2[0];
			eval_const_fix_cms->ti_tstar_gamma12[0] = eval_const_fix_cms->ti_tstar_beta[0] * eval_const_fix_cms->ti_tstar_beta2[0];

			/* reconstruction of the last DF without the coverave */
			eval_const_fix_cms->ti_tstar_beta2[fix->schedule.iNCpn-1] = (1.0 - exp(-und->model.dLambdaX2 * (fix->schedule.dCpnTime[fix->schedule.iNCpn-1] - und->model.dTStar))) / und->model.dLambdaX2;
			eval_const_fix_cms->ti_tstar_gamma2[fix->schedule.iNCpn-1] = 0.5 * eval_const_fix_cms->ti_tstar_beta2[fix->schedule.iNCpn-1] * eval_const_fix_cms->ti_tstar_beta2[fix->schedule.iNCpn-1];
			eval_const_fix_cms->ti_tstar_gamma12[fix->schedule.iNCpn-1] = eval_const_fix_cms->ti_tstar_beta[fix->schedule.iNCpn-1] * eval_const_fix_cms->ti_tstar_beta2[fix->schedule.iNCpn-1];
		}
	}
...
long	today;
	...
	/* reconstruction of DF(t,Tpay) / DF(t,Tstar) */
	eval_const_cpn->tpay_tstar_alpha = swp_f_df(today, cpn->cpn_pay_date, und->mkt->yc) / swp_f_df(today, und->model.lTStarDate, und->mkt->yc);
	eval_const_cpn->tpay_tstar_alpha = -log(eval_const_cpn->tpay_tstar_alpha);
	eval_const_cpn->tpay_tstar_beta = (1.0 -exp(-und->model.dLambdaX * (cpn->cpn_pay_time - und->model.dTStar))) / und->model.dLambdaX;
	eval_const_cpn->tpay_tstar_gamma = 0.5 * eval_const_cpn->tpay_tstar_beta * eval_const_cpn->tpay_tstar_beta;

	/* reconstruction of DF(t,Ts) / DF(t,Te) */
	eval_const_cpn->ts_te_alpha = swp_f_df(today, cpn->pay_start_date, und->mkt->yc) / swp_f_df(today, cpn->pay_end_date, und->mkt->yc);
	eval_const_cpn->ts_te_alpha = -log(eval_const_cpn->ts_te_alpha);

	beta1 = (1.0 - exp(-und->model.dLambdaX * (cpn->pay_start_time - und->model.dTStar))) / und->model.dLambdaX;
	beta2 = (1.0 - exp(-und->model.dLambdaX * (cpn->pay_end_time - und->model.dTStar))) / und->model.dLambdaX;	
	...
	{
		/* Fill the Extras */
		eval_const_cpn->tpay_tstar_beta2 = (1.0 -exp(-und->model.dLambdaX2 * (cpn->cpn_pay_time - und->model.dTStar))) / und->model.dLambdaX2;
		eval_const_cpn->tpay_tstar_gamma2 = 0.5 * eval_const_cpn->tpay_tstar_beta2 * eval_const_cpn->tpay_tstar_beta2;
		eval_const_cpn->tpay_tstar_gamma12 = eval_const_cpn->tpay_tstar_beta * eval_const_cpn->tpay_tstar_beta2;

		beta1_2 = (1.0 - exp(-und->model.dLambdaX2 * (cpn->pay_start_time - und->model.dTStar))) / und->model.dLambdaX2;
		beta2_2 = (1.0 - exp(-und->model.dLambdaX2 * (cpn->pay_end_time - und->model.dTStar))) / und->model.dLambdaX2;	
	...
	eval_const_cpn->shift = cpn->pay_gearing * (-1.0 / cpn->pay_cvg + cpn->pay_fwd_spread) + cpn->cpn_coupon;
...
int				i;
	...
	for (i=0; i<call->num_fund_cpn; i++)
	{
		...
		eval_const_call->tpay_tstar_alpha[i] = swp_f_df(today, cpn->pay_date, und->mkt->yc) / swp_f_df(today, und->model.lTStarDate, und->mkt->yc);
		eval_const_call->tpay_tstar_alpha[i] = -log(eval_const_call->tpay_tstar_alpha[i]);
		eval_const_call->tpay_tstar_beta[i] = (1.0 -exp(-und->model.dLambdaX * (cpn->pay_time - und->model.dTStar))) / und->model.dLambdaX;
		eval_const_call->tpay_tstar_gamma[i] = 0.5 * eval_const_call->tpay_tstar_beta[i] * eval_const_call->tpay_tstar_beta[i];
	}
	...
	i = call->num_fund_cpn;
	eval_const_call->tpay_tstar_alpha[i] = swp_f_df(today, cpn->start_date, und->mkt->yc) / swp_f_df(today, und->model.lTStarDate, und->mkt->yc);
	eval_const_call->tpay_tstar_alpha[i] = -log(eval_const_call->tpay_tstar_alpha[i]);
	eval_const_call->tpay_tstar_beta[i] = (1.0 -exp(-und->model.dLambdaX * (cpn->start_time - und->model.dTStar))) / und->model.dLambdaX;
	eval_const_call->tpay_tstar_gamma[i] = 0.5 * eval_const_call->tpay_tstar_beta[i] * eval_const_call->tpay_tstar_beta[i];

	/* reconstruction of DF(t,Tset) / DF(t,Tstar) */
	eval_const_call->tset_tstar_alpha = swp_f_df(today, call->set_date, und->mkt->yc) / swp_f_df(today, und->model.lTStarDate, und->mkt->yc);
	eval_const_call->tset_tstar_alpha = -log(eval_const_call->tset_tstar_alpha);
	eval_const_call->tset_tstar_beta = (1.0 -exp(-und->model.dLambdaX * (call->set_time - und->model.dTStar))) / und->model.dLambdaX;
	eval_const_call->tset_tstar_gamma = 0.5 * eval_const_call->tset_tstar_beta * eval_const_call->tset_tstar_beta;
	...
	if (call->is_midat)
	{
		for (i=0; i<call->num_exo_cpn; i++)
		{
			...
			eval_const_call->tpay_tstar_alpha2[i] = swp_f_df(today, exo_cpn->cpn_pay_date, und->mkt->yc) / swp_f_df(today, und->model.lTStarDate, und->mkt->yc);
			eval_const_call->tpay_tstar_alpha2[i] *= exo_cpn->cpn_cvg * exo_cpn->cpn_coupon * exo_cpn->cpn_not;
			eval_const_call->tpay_tstar_alpha2[i] = -log(eval_const_call->tpay_tstar_alpha2[i]);
			eval_const_call->tpay_tstar_beta2[i] = (1.0 -exp(-und->model.dLambdaX * (exo_cpn->cpn_pay_time - und->model.dTStar))) / und->model.dLambdaX;
			eval_const_call->tpay_tstar_gamma2[i] = 0.5 * eval_const_call->tpay_tstar_beta2[i] * eval_const_call->tpay_tstar_beta2[i];
		}
	}
	...
	if (und->model.iOne2F == 2)
	{
		...
		for (i=0; i<call->num_fund_cpn; i++)
		{
			...
			eval_const_call->tpay_tstar_beta_2[i] = (1.0 -exp(-und->model.dLambdaX2 * (cpn->pay_time - und->model.dTStar))) / und->model.dLambdaX2;
			eval_const_call->tpay_tstar_gamma_2[i] = 0.5 * eval_const_call->tpay_tstar_beta_2[i] * eval_const_call->tpay_tstar_beta_2[i];
			eval_const_call->tpay_tstar_gamma_12[i] = eval_const_call->tpay_tstar_beta[i] * eval_const_call->tpay_tstar_beta_2[i];
		}
		...
		i = call->num_fund_cpn;
		eval_const_call->tpay_tstar_beta_2[i] = (1.0 -exp(-und->model.dLambdaX2 * (cpn->start_time - und->model.dTStar))) / und->model.dLambdaX2;
		eval_const_call->tpay_tstar_gamma_2[i] = 0.5 * eval_const_call->tpay_tstar_beta_2[i] * eval_const_call->tpay_tstar_beta_2[i];
		eval_const_call->tpay_tstar_gamma_12[i] = eval_const_call->tpay_tstar_beta[i] * eval_const_call->tpay_tstar_beta_2[i];

		/* reconstruction of DF(t,Tset) / DF(t,Tstar) */		
		eval_const_call->tset_tstar_beta2 = (1.0 -exp(-und->model.dLambdaX2 * (call->set_time - und->model.dTStar))) / und->model.dLambdaX2;
		eval_const_call->tset_tstar_gamma2 = 0.5 * eval_const_call->tset_tstar_beta2 * eval_const_call->tset_tstar_beta2;
		eval_const_call->tset_tstar_gamma12 = eval_const_call->tset_tstar_beta * eval_const_call->tset_tstar_beta2;
		...
		if (call->is_midat)
		{
			for (i=0; i<call->num_exo_cpn; i++)
			{
				...
				eval_const_call->tpay_tstar_beta2_2[i] = (1.0 -exp(-und->model.dLambdaX2 * (exo_cpn->cpn_pay_time - und->model.dTStar))) / und->model.dLambdaX2;
				eval_const_call->tpay_tstar_gamma2_2[i] = 0.5 * eval_const_call->tpay_tstar_beta2_2[i] * eval_const_call->tpay_tstar_beta2_2[i];
				eval_const_call->tpay_tstar_gamma2_12[i] = eval_const_call->tpay_tstar_beta2[i] * eval_const_call->tpay_tstar_beta2_2[i];
			}
		}
	}
...
double				local_min_time;
	...
	else
	{
		...
		adi_arg->nstp = nstp;
		adi_arg->time = calloc(adi_arg->nstp, sizeof(double));
		adi_arg->optimise = calloc(adi_arg->nb_event, sizeof(int));
		adi_arg->mcebparams = calloc(1, sizeof(MCEBParams));

		adi_arg->dSigma = calloc(adi_arg->nstp, sizeof(double));
		adi_arg->dAlpha = calloc(adi_arg->nstp, sizeof(double));
		adi_arg->dLambdaEps = calloc(adi_arg->nstp, sizeof(double));
		adi_arg->dLvlEps = calloc(adi_arg->nstp, sizeof(double));
		adi_arg->dRho = calloc(adi_arg->nstp, sizeof(double));
		adi_arg->dRho2 = calloc(adi_arg->nstp, sizeof(double));
		adi_arg->dLGMAlpha = calloc(adi_arg->nstp, sizeof(double));
		adi_arg->dLGMRho = calloc(adi_arg->nstp, sizeof(double));

		if (!adi_arg->time || !adi_arg->optimise || !adi_arg->mcebparams ||
			!adi_arg->dSigma || !adi_arg->dAlpha || !adi_arg->dLambdaEps ||
			!adi_arg->dLvlEps || !adi_arg->dRho || !adi_arg->dRho2 || !adi_arg->dLGMAlpha || !adi_arg->dLGMRho)
		{
	...
	{
		if (cts->exo_leg->cpn[cts->call[0].exo_idx+i].type == 1 || cts->exo_leg->cpn[cts->call[0].exo_idx+i].type == 2)
		{
			is_floating = 1;
			break;
		}
	...
	if (is_floating && pde_or_mc == 1)
	{
		...
		adi_arg->dPathInfos = calloc(req_paths+1, sizeof(double));
	...
	for (i=0; i<adi_arg->nstp; i++)
	{
		...
		cts_prm = calloc(1, sizeof (cts_pay_arg));
		...
		if (index_call < cts->num_calls && fabs(adi_arg->date[i] - call->ex_date) < 1.0e-08)
		{
			...
			err = cts_fill_eval_const_call(	und,
											cts,
											index_call,
											&(cts_prm->eval_const_call));			
		...
		if (index_cpn < cts->exo_leg->num_cpn && fabs(adi_arg->date[i] - cts->exo_leg->cpn[index_cpn].pay_fix_date) < 1.0e-08 && is_midat == 0)
		{
			/* this is a cpn date */
			...
			{				
				...
				err = cts_fill_eval_const_cpn(	und,
												cts,
												index_cpn,
												&(cts_prm->eval_const_cpn));
		...
		if (index_cpn_for_fix < cts->exo_leg->num_cpn 
			&& index_fix < cpn_for_fix->used_nfix
			&& fabs(adi_arg->date[i] - cpn_for_fix->fix[index_fix].ref_fix_date) < 1.0e-08 && is_midat == 0)
		{
			...
			if (cts_prm->fix->schedule.iNCpn == 2)
			{
				...
				err = cts_fill_eval_const_fix(	und,
												cts,
												index_cpn_for_fix,
												index_fix,
												&(cts_prm->eval_const_fix));
			}
			else
			{
				cts_prm->is_fixing_cms = 1;

				err = cts_fill_eval_const_fix_cms(	und,
													cts,
													index_cpn_for_fix,
													index_fix,
													&(cts_prm->eval_const_fix_cms));
			}
					...
					CTS_ADI_ARG	adi_arg)
{
	...
	if (adi_arg)
	{
		if (adi_arg->time) free (adi_arg->time);
		if (adi_arg->date) free (adi_arg->date);
		if (adi_arg->ifr) free (adi_arg->ifr);
		if (adi_arg->is_event) free (adi_arg->is_event);
		...
		if (adi_arg->params) free(adi_arg->params);
		...
		if (adi_arg->optimise) free (adi_arg->optimise);

		if (adi_arg->dSigma) free(adi_arg->dSigma);
		if (adi_arg->dAlpha) free(adi_arg->dAlpha);
		if (adi_arg->dLambdaEps) free(adi_arg->dLambdaEps);
		if (adi_arg->dLvlEps) free(adi_arg->dLvlEps);
		if (adi_arg->dRho) free(adi_arg->dRho);
		if (adi_arg->dRho2) free(adi_arg->dRho2);
		if (adi_arg->dLGMAlpha) free(adi_arg->dLGMAlpha);
		if (adi_arg->dLGMRho) free(adi_arg->dLGMRho);

		if (adi_arg->dPathInfos) free (adi_arg->dPathInfos);
...
/*	Find Numer TStar */
void cts_get_numer_tstar(	long	today,
							CTS		cts,
							double	*numer_tstar)
{
long	temp_date;
double	temp_numer_tstar;
	...
	{
		temp_numer_tstar = 0.5 * (cts->exo_leg->cpn[cts->call[0].exo_idx].cpn_start_time + cts->exo_leg->cpn[cts->call[cts->num_calls-1].exo_idx].cpn_pay_time);
...
CTS_IV		fwd_iv_info)
{
	...
	if (!just_recalib)
	{
		...
		{		
			...
			err = cts_calc_equivalent_strikes(	cts_iv,
												cts,
												mkt);
			...
			err = cts_adjust_floating_fixing(	cts,
												pde_or_mc);
	...
	if (cts->num_calls > 0 && cts->call)
	{
		...
		cts_get_numer_tstar(und->mkt->today,
							cts,
							&numer_tstar);
	...
	if (cts->num_calls > 0 && cts->call)
	{
		if (do_one_time && one_time_index == 0)
		{
			...
			err = cts_find_one_time_index(	und,
											cts,
											&one_time_index);
		}
		...
		if (call->is_midat)
		{
			for (i=lz; i<=uz; i++)
			{
				...
				for (j=lx; j<=ux; j++)
				{
					...
					for (k=lpsi; k<=upsi; k++)
					{
						DFR_set = exp(-eval_const_call->tset_tstar_alpha - eval_const_call->tset_tstar_beta * tmpf - eval_const_call->tset_tstar_gamma * psi[k]);
						...
						{
							DFR_pay = exp(-eval_const_call->tpay_tstar_alpha[l] - eval_const_call->tpay_tstar_beta[l] * tmpf - eval_const_call->tpay_tstar_gamma[l] * psi[k]);
						...
						{
							DFR_pay = exp(-eval_const_call->tpay_tstar_alpha2[l] - eval_const_call->tpay_tstar_beta2[l] * tmpf - eval_const_call->tpay_tstar_gamma2[l] * psi[k]);
							exotic += DFR_pay;
						}
			...
			for (i=lz; i<=uz; i++)
			{
				...
				for (j=lx; j<=ux; j++)
				{
					...
					for (k=lpsi; k<=upsi; k++)
					{
						...
						prod_val[i][j][k][0] = max(prod_val[i][j][k][0], call->pay_rec * (prod_val[i][j][k][1] - funding) - fee);

						/* Value the one time call if required */
						if (eval_one_time)
						{
							prod_val[i][j][k][one_time_col] = max(0.0, call->pay_rec * (prod_val[i][j][k][1] - funding) - fee);
						}
					}
				}
			}
	...
	if (cts_arg->is_fixing)
	{
		...
		for (i=lz; i<=uz; i++)
		{
			...
			for (j=lx; j<=ux; j++)
			{
				...
				for (k=lpsi; k<=upsi; k++)
				{
					DFR_fix = exp(-eval_const_fix->ts_te_alpha - eval_const_fix->ts_te_beta * tmpf - eval_const_fix->ts_te_gamma * psi[k]);
					DFR_pay = exp(-eval_const_fix->tpay_tstar_alpha - eval_const_fix->tpay_tstar_beta * tmpf - eval_const_fix->tpay_tstar_gamma * psi[k]);
		...
		for (i=lz; i<=uz; i++)
		{
			...
			for (j=lx; j<=ux; j++)
			{
				...
				for (k=lpsi; k<=upsi; k++)
				{
					...
					{
						DFR_start = exp(-eval_const_fix_cms->ti_tstar_alpha[0] - eval_const_fix_cms->ti_tstar_beta[0] * tmpf - eval_const_fix_cms->ti_tstar_gamma[0] * psi[k]);
						cms = 0.0;

						/* Intermediary DF, coverage already included in alpha */
						for (l=1; l<schedule->iNCpn-1; l++)
						{
							DFR_fix = exp(-eval_const_fix_cms->ti_tstar_alpha[l] - eval_const_fix_cms->ti_tstar_beta[l] * tmpf - eval_const_fix_cms->ti_tstar_gamma[l] * psi[k]);
							cms += DFR_fix;
						}

						/* Last DF */
						DFR_end = exp(-eval_const_fix_cms->ti_tstar_alpha[schedule->iNCpn-1] - eval_const_fix_cms->ti_tstar_beta[schedule->iNCpn-1] * tmpf - eval_const_fix_cms->ti_tstar_gamma[schedule->iNCpn-1] * psi[k]);
					...
					DFR_pay = exp(-eval_const_fix_cms->tpay_tstar_alpha - eval_const_fix_cms->tpay_tstar_beta * tmpf - eval_const_fix_cms->tpay_tstar_gamma * psi[k]);
	...
	if (cts_arg->is_cpn)
	{
		...
		for (i=lz; i<=uz; i++)
		{
			...
			for (j=lx; j<=ux; j++)
			{
				...
				for (k=lpsi; k<=upsi; k++)
				{
					DFR_fix = exp(-eval_const_cpn->ts_te_alpha - eval_const_cpn->ts_te_beta * tmpf - eval_const_cpn->ts_te_gamma * psi[k]);					
					...
					{						
						case 1:							
							...
							break;
						case 2:
							DFR_pay = exp(-eval_const_cpn->tpay_tstar_alpha - eval_const_cpn->tpay_tstar_beta * tmpf - eval_const_cpn->tpay_tstar_gamma * psi[k]);
...
int				l;
double			DFR_start, DFR_end, DFR_fix, DFR_pay, DFR_set, coupon, libor, partial_funding, funding, exotic, iv, fee;
double			cms;
...
Err				err = NULL;
	...
	if (cts_arg->is_call)
	{
		...
		if (call->is_midat)
		{			
			DFR_set = exp(-eval_const_call->tset_tstar_alpha - eval_const_call->tset_tstar_beta * ft - eval_const_call->tset_tstar_gamma * psi);
			...
			for (l=0; l<call->num_fund_cpn; l++)
			{
				DFR_pay = exp(-eval_const_call->tpay_tstar_alpha[l] - eval_const_call->tpay_tstar_beta[l] * ft - eval_const_call->tpay_tstar_gamma[l] * psi);
				funding += fund_cpn->cpn_plus_ex * DFR_pay;
			...
			{
				DFR_pay = exp(-eval_const_call->tpay_tstar_alpha2[l] - eval_const_call->tpay_tstar_beta2[l] * ft - eval_const_call->tpay_tstar_gamma2[l] * psi);
				exotic += DFR_pay;
			}
	...
	if (cts_arg->is_cpn)
	{
		...
		DFR_fix = exp(-eval_const_cpn->ts_te_alpha - eval_const_cpn->ts_te_beta * ft - eval_const_cpn->ts_te_gamma * psi);					
		...
		{						
			case 1:
				...
				break;
			case 2:
				DFR_pay = exp(-eval_const_cpn->tpay_tstar_alpha - eval_const_cpn->tpay_tstar_beta * ft - eval_const_cpn->tpay_tstar_gamma * psi);
	...
	if (cts_arg->is_fixing)
	{
		...
		DFR_fix = exp(-eval_const_fix->ts_te_alpha - eval_const_fix->ts_te_beta * ft - eval_const_fix->ts_te_gamma * psi);
		DFR_pay = exp(-eval_const_fix->tpay_tstar_alpha - eval_const_fix->tpay_tstar_beta * ft - eval_const_fix->tpay_tstar_gamma * psi);
	...
	if (cts_arg->is_fixing_cms)
	{
		...
		if (schedule->iNCpn-1 > 0)
		{
			DFR_start = exp(-eval_const_fix_cms->ti_tstar_alpha[0] - eval_const_fix_cms->ti_tstar_beta[0] * ft - eval_const_fix_cms->ti_tstar_gamma[0] * psi);
			cms = 0.0;

			/* Intermediary DF, coverage already included in alpha */
			for (l=1; l<schedule->iNCpn-1; l++)
			{
				DFR_fix = exp(-eval_const_fix_cms->ti_tstar_alpha[l] - eval_const_fix_cms->ti_tstar_beta[l] * ft - eval_const_fix_cms->ti_tstar_gamma[l] * psi);
				cms += DFR_fix;
			}

			/* Last DF */
			DFR_end = exp(-eval_const_fix_cms->ti_tstar_alpha[schedule->iNCpn-1] - eval_const_fix_cms->ti_tstar_beta[schedule->iNCpn-1] * ft - eval_const_fix_cms->ti_tstar_gamma[schedule->iNCpn-1] * psi);
			cms += DFR_end * schedule->dCpnCvg[schedule->iNCpn-1];
		...
		DFR_pay = exp(-eval_const_fix_cms->tpay_tstar_alpha - eval_const_fix_cms->tpay_tstar_beta * ft - eval_const_fix_cms->tpay_tstar_gamma * psi);
...
Err				err = NULL;
	...
	if (cts_arg->is_call)
	{
		...
		if (call->is_midat)
		{			
			DFR_set = exp(-eval_const_call->tset_tstar_alpha - eval_const_call->tset_tstar_beta * ft1 - eval_const_call->tset_tstar_beta2 * ft2 
				- eval_const_call->tset_tstar_gamma * psi1 - eval_const_call->tset_tstar_gamma2 * psi2 - eval_const_call->tset_tstar_gamma12 * psi12);
			...
			for (l=0; l<call->num_fund_cpn; l++)
			{
				DFR_pay = exp(-eval_const_call->tpay_tstar_alpha[l] - eval_const_call->tpay_tstar_beta[l] * ft1 - eval_const_call->tpay_tstar_beta_2[l] * ft2 
					- eval_const_call->tpay_tstar_gamma[l] * psi1 - eval_const_call->tpay_tstar_gamma_2[l] * psi2 - eval_const_call->tpay_tstar_gamma_12[l] * psi12);
				funding += fund_cpn->cpn_plus_ex * DFR_pay;
			...
			for (l=0; l<call->num_partial_exo_cpn; l++)
			{
				DFR_pay = exp(-eval_const_call->tpay_tstar_alpha2[l] - eval_const_call->tpay_tstar_beta2[l] * ft1 - eval_const_call->tpay_tstar_beta2_2[l] * ft2
							- eval_const_call->tpay_tstar_gamma2[l] * psi1 - eval_const_call->tpay_tstar_gamma2_2[l] * psi2 - eval_const_call->tpay_tstar_gamma2_12[l] * psi12);
				exotic += DFR_pay;
			}
	...
	if (cts_arg->is_cpn)
	{
		...
		DFR_fix = exp(-eval_const_cpn->ts_te_alpha - eval_const_cpn->ts_te_beta * ft1 - eval_const_cpn->ts_te_beta2 * ft2
			- eval_const_cpn->ts_te_gamma * psi1 - eval_const_cpn->ts_te_gamma2 * psi2 - eval_const_cpn->ts_te_gamma12 * psi12);
		...
		{						
			case 1:
				...
				break;
			case 2:
				DFR_pay = exp(-eval_const_cpn->tpay_tstar_alpha - eval_const_cpn->tpay_tstar_beta * ft1 - eval_const_cpn->tpay_tstar_beta2 * ft2
					- eval_const_cpn->tpay_tstar_gamma * psi1 - eval_const_cpn->tpay_tstar_gamma2 * psi2 - eval_const_cpn->tpay_tstar_gamma12 * psi12);
				prod_val[0] += -cts->call->pay_rec * libor * DFR_pay * cts_arg->dPathInfos[path_index];
				cts_arg->dPathInfos[path_index] = 0.0;
				break;
		}
	...
	if (cts_arg->is_fixing)
	{
		...
		DFR_fix = exp(-eval_const_fix->ts_te_alpha - eval_const_fix->ts_te_beta * ft1 - eval_const_fix->ts_te_beta2 * ft2
			- eval_const_fix->ts_te_gamma * psi1 - eval_const_fix->ts_te_gamma2 * psi2 - eval_const_fix->ts_te_gamma12 * psi12);
		DFR_pay = exp(-eval_const_fix->tpay_tstar_alpha - eval_const_fix->tpay_tstar_beta * ft1 - eval_const_fix->tpay_tstar_beta2 * ft2 
			- eval_const_fix->tpay_tstar_gamma * psi1 - eval_const_fix->tpay_tstar_gamma2 * psi2 - eval_const_fix->tpay_tstar_gamma12 * psi12);
	...
	if (cts_arg->is_fixing_cms)
	{
		...
		if (schedule->iNCpn-1 > 0)
		{
			DFR_start = exp(-eval_const_fix_cms->ti_tstar_alpha[0] - eval_const_fix_cms->ti_tstar_beta[0] * ft1  - eval_const_fix_cms->ti_tstar_beta2[0] * ft2 
				- eval_const_fix_cms->ti_tstar_gamma[0] * psi1 - eval_const_fix_cms->ti_tstar_gamma2[0] * psi2 - eval_const_fix_cms->ti_tstar_gamma12[0] * psi12);
			cms = 0.0;

			/* Intermediary DF, coverage already included in alpha */
			for (l=1; l<schedule->iNCpn-1; l++)
			{
				DFR_fix = exp(-eval_const_fix_cms->ti_tstar_alpha[l] - eval_const_fix_cms->ti_tstar_beta[l] * ft1 - eval_const_fix_cms->ti_tstar_beta2[l] * ft2
					- eval_const_fix_cms->ti_tstar_gamma[l] * psi1 - eval_const_fix_cms->ti_tstar_gamma2[l] * psi2 - eval_const_fix_cms->ti_tstar_gamma12[l] * psi12);
				cms += DFR_fix;
			}

			/* Last DF */
			DFR_end = exp(-eval_const_fix_cms->ti_tstar_alpha[schedule->iNCpn-1] - eval_const_fix_cms->ti_tstar_beta[schedule->iNCpn-1] * ft1 - eval_const_fix_cms->ti_tstar_beta2[schedule->iNCpn-1] * ft2 
				- eval_const_fix_cms->ti_tstar_gamma[schedule->iNCpn-1] * psi1 - eval_const_fix_cms->ti_tstar_gamma2[schedule->iNCpn-1] * psi2 - eval_const_fix_cms->ti_tstar_gamma12[schedule->iNCpn-1] * psi12);
			cms += DFR_end * schedule->dCpnCvg[schedule->iNCpn-1];
		...
		DFR_pay = exp(-eval_const_fix_cms->tpay_tstar_alpha - eval_const_fix_cms->tpay_tstar_beta * ft1 - eval_const_fix_cms->tpay_tstar_beta2 * ft2
			- eval_const_fix_cms->tpay_tstar_gamma * psi1 - eval_const_fix_cms->tpay_tstar_gamma2 * psi2 - eval_const_fix_cms->tpay_tstar_gamma12 * psi12);
...
Err		err = NULL;
	...
	{
		smessage ("Launching adi, time steps requested: %d, actual: %d", adi_arg->initnstp, adi_arg->nstp);
	}
	else
	{
	...
	if (adi_arg->pde_or_mc == 0)
	{
		...
		temp_val = calloc(ncol_tot, sizeof(double));
		...
		err = lgmSV_adi_UtPieceWise(adi_arg->nstp,
									adi_arg->time,
									adi_arg->date,
									adi_arg->nstppsi,
									adi_arg->nstpx,
	...
	else
	{
		...
		temp_val_mc = dmatrix(0, adi_arg->nb_event + 4, 0, 2 + adi_arg->mcebparams->iNbIndex + adi_arg->mcebparams->iCalcOneTime);
		...
		{
			err = lgmSV_mc_balsam_rev(	adi_arg->nstp,
										adi_arg->nb_event,
										adi_arg->time,
										adi_arg->date,
										adi_arg->npaths,
		...
		{
			err = lgmSV2F_mc_balsam_rev(adi_arg->nstp,
										adi_arg->nb_event,
										adi_arg->time,
										adi_arg->date,
										adi_arg->npaths,
...
FREE_RETURN:
	...
	if (temp_val_mc) free_dmatrix(temp_val_mc, 0, adi_arg->nb_event + 4, 0, 2 + adi_arg->mcebparams->iNbIndex + adi_arg->mcebparams->iCalcOneTime);

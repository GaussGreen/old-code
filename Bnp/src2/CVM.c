...
Err cvm_fill_struct(
					...
					//TS Dates
					double			shift,
					int				converging_sliding,
					int				interpollation_mode,
					int				nbOfDates,
					double			*vol_dates, 
					...
					cvm_str *cvm)
{
	...
	for(i=0;i<nbOfDates;++i)
	{
		...
		for(j=0;j<nbOfPillars;++j)
		{
			...
			cvm->pillars_correls[i][j] = (double*) calloc(nbOfPillars, sizeof(double));
	...
	if( (sensitype==0) || (sensitype==2) ||
		((sensitype==1) && ((nbOfSensiTenors==0)||(sensi_tenors==NULL)||(sensitivities==NULL)))
	  )
	{
		...
		for(i=0;i<nbOfPillars;++i)
		{
			spline(cvm->pillars_times-1, cvm->sensitivities[i]-1, nbOfPillars, 1.0e30, 1.0e30, cvm->sensitivities2[i]-1);
		}
	...
	else if(sensitype==1)
	{
		...
		for(i=0;i<nbOfPillars;++i)
		{
			spline(cvm->sensi_times-1, cvm->sensitivities[i]-1, nbOfSensiTenors, 1.0e30, 1.0e30, cvm->sensitivities2[i]-1);
		}
	}
...
Err cvm_copy_struct(cvm_str *cvm_target, cvm_str *cvm_source)
{
	...
	err = cvm_fill_struct(cvm_source->ycid, cvm_source->vcid, 
					cvm_source->srtFreq, cvm_source->srtBasis, 
					cvm_source->refRate, 
					cvm_source->shift,
					cvm_source->converging_sliding, cvm_source->interpollation_mode,
					cvm_source->nbOfDates, cvm_source->vol_dates,
					cvm_source->nbOfPillars, pillars_tenors,
					cvm_source->pillars_correls, cvm_source->pillars_vols_norm,
					cvm_source->sensi_type, 
					cvm_source->lambdas, 
					cvm_source->nbOfSensiTenors, sensi_tenors, cvm_source->sensitivities, 
					cvm_source->nQuadLegendre,
					cvm_target);

	if(pillars_tenors) free_svector_size(pillars_tenors, 0, cvm_source->nbOfPillars-1, 256);
	if(sensi_tenors) free_svector_size(sensi_tenors, 0, cvm_source->nbOfSensiTenors-1, 256);

	return err;
}


Err cvm_set_pillars_vols(int dateIndex, 
						 int nPillars, double *pillarsVols, 
						 cvm_str *cvm)
{
	Err err=NULL;
	int i, k;
	...
	for(i=0;i<nPillars;++i)
	{
		...
		{
			cvm->pillars_vols[dateIndex][k][i] = cvm->pillars_chol[dateIndex][k][i] * pillarsVols[k];
		}
	}
...
Err cvm_bump_pillars_correls(int dateIndex, 
						 int nPillarsMinusOne, double *correls, 
						 cvm_str *cvm)
{
	Err err=NULL;
	int i, k;
	...
	for(i=0;i<nPillarsMinusOne;++i)
	{
		...
		{
			cvm->pillars_correls[dateIndex][k][i+k] = DMAX(-0.9999, DMIN(0.9999 , cvm->pillars_correls[dateIndex][k][i+k] + correls[i]));
		}
	}
	...
	CholDec(&CholAlg,
				 cvm->pillars_correls[dateIndex],
				 cvm->pillars_chol[dateIndex],
				 cvm->nbOfPillars);
	...
	for(i=0;i<cvm->nbOfPillars;++i)
	{
		...
		{
			cvm->pillars_vols[dateIndex][k][i] = cvm->pillars_chol[dateIndex][k][i] * cvm->pillars_vols_norm[dateIndex][k];
		}
	}
...
Err cvm_set_pillars_correls(int dateIndex, 
						 int nPillars, double **correls, 
						 cvm_str *cvm)
{
	Err err=NULL;
	int i, k;
...
Err cvm_compute_fwdvol_at_voldate(int dateIndex, double maturity, 
						  int nPillars, double *fwdvol, cvm_str *cvm)
{
	Err err = NULL;
	int i,k;
	double *sensi=NULL;
	...
	if(time > maturity)
	{
		if(dateIndex>0)
		{
			err = cvm_compute_fwdvol_at_voldate(dateIndex-1, maturity, 
						  nPillars, fwdvol, cvm);
			return err;
		}
		else
...
FREE_RETURN:
	if(sensi) free_dvector(sensi,0, cvm->nbOfPillars-1);

	return err;
}


Err cvm_compute_pillarsvols(double time, double maturity, 
					   int nPillars, double **pillarsvols, cvm_str *cvm)
{
	Err err=NULL;
	int i, j, k;
		...
		else
		{
			for(j=0;j<nPillars;++j)
			{
				for(k=0;k<nPillars;++k)
				{
					...
					{
						pillarsvols[j][k] = cvm->pillars_vols[i-1][j][k] 
...
Err cvm_compute_fwdvol_sliding(double time, double maturity, 
					   int nPillars, double *fwdvol, cvm_str *cvm)
{
	Err err=NULL;
	double **vol = NULL;
...
FREE_RETURN:
	...
	if(vol) free_dmatrix(vol, 0, cvm->nbOfPillars-1, 0, cvm->nbOfPillars-1);
	if(sensi) free_dvector(sensi, 0, cvm->nbOfPillars-1);
...
Err cvm_compute_fwdvol_converging(double time, double maturity, 
					   int nPillars, double *fwdvol, cvm_str *cvm)
{
	Err err=NULL;
	double *vol1=NULL;
	...
	{
		err = cvm_compute_fwdvol_converging(time+cvm->vol_times[0]-maturity, cvm->vol_times[0], cvm->nbOfPillars, fwdvol, cvm);
		return err;
	}
	...
	else
	{
		...
		if(i == cvm->nbOfDates)
		{
			err = cvm_compute_fwdvol_at_voldate(cvm->nbOfDates-1, maturity, nPillars, vol2, cvm);
			if(err) goto FREE_RETURN;
			for(k=0;k<nPillars;++k)
			{
				fwdvol[k] = vol2[k];
		...
		else
		{
			...
			for(k=0;k<nPillars;++k)
			{
				if(cvm->interpollation_mode==0)
				{
					fwdvol[k] = vol2[k];//vol1[k] + (vol2[k] - vol1[k]) * (time - cvm->vol_times[i-1]) / (cvm->vol_times[i] - cvm->vol_times[i-1]);
				}
				else
				{
					fwdvol[k] = vol1[k] + (vol2[k] - vol1[k]) * (time - cvm->vol_times[i-1]) / (cvm->vol_times[i] - cvm->vol_times[i-1]);
				}
			}
		}
	}
...
FREE_RETURN:
	...
	if(vol1) free_dvector(vol1, 0, cvm->nbOfPillars-1);
	if(vol2) free_dvector(vol2, 0, cvm->nbOfPillars-1);

	return err;
}


Err cvm_compute_fwdvol(double time, double maturity, 
					   int nPillars, double *fwdvol, cvm_str *cvm)
{
	Err err = NULL;
...
Err cvm_vol(double start, double end, double *vol, cvm_str *cvm)
{
    ...
    err = swp_f_setSwapDP((long)(start), (long)(end), cvm->srtFreq, cvm->srtBasis, &Swap);
	if(err) goto FREE_RETURN;
	...
	err = swp_f_make_FloatLegDatesCoveragesAndSpreads(&Swap, lToday, cvm->refRate, 
												&lFloatPayDates,
												&iNFloatPayDates, 
												&lFloatFixingDates, 
												&lFloatStartDates,
	...
	{
		level += dFixCoverages[i] * swp_f_df(lToday, lFixPayDates[i+1], cvm->ycid);
	}
	...
	for(i=0;i<iNFloatDates;++i)
	{
		if(cvm->shift==-1)
		{
			weights[i] = dFloatCoverages[i] * swp_f_df(lToday, lFloatPayDates[i+1], cvm->ycid) / level;
		}
		else
		{
			frai =	(
						swp_f_df(lToday, lFloatPayDates[i], cvm->ycid) 
						/ swp_f_df(lToday, lFloatPayDates[i+1], cvm->ycid)
						- 1
						) / dFloatCoverages[i] + dFloatSpreads[i];

			spreadleg += dFloatCoverages[i] * dFloatSpreads[i] * swp_f_df(lToday, lFloatPayDates[i+1], cvm->ycid);

			weights[i] = (cvm->shift + frai) * dFloatCoverages[i] * swp_f_df(lToday, lFloatPayDates[i+1], cvm->ycid) / level;
		}
	}
	...
	{
		fwdswap =	(
					swp_f_df(lToday, lFloatPayDates[0], cvm->ycid) 
	...
	exercise = add_unit((long)(start), -cvm->spotLag, SRT_BDAY, MODIFIED_SUCCEEDING);
	...
	if(exercise <= cvm->vol_dates[0])
	{
		...
		for(k=0;k<cvm->nQuadLegendre;++k)
		{
			...
			for(i=0;i<iNFloatDates;++i)
			{
				err = cvm_compute_fwdvol(x[k], (lFloatStartDates[i]-lToday)/365.0, cvm->nbOfPillars, volloc, cvm);
				if(err) goto FREE_RETURN;
				for(j=0;j<cvm->nbOfPillars;++j)
				{
					if(cvm->shift==-1)
	...
	{
		while( (dateIndex<cvm->nbOfDates) && ( exercise > cvm->vol_dates[dateIndex]) )
		{
		...
		{
			GaussLeg(cvm->vol_times[l-1], cvm->vol_times[l], x-1, w-1, cvm->nQuadLegendre);
			for(k=0;k<cvm->nQuadLegendre;++k)
			{
				for(j=0;j<cvm->nbOfPillars;++j)
				{
		...
		GaussLeg(cvm->vol_times[dateIndex-1], exercise_time, x-1, w-1, cvm->nQuadLegendre);
		for(k=0;k<cvm->nQuadLegendre;++k)
		{
			for(j=0;j<cvm->nbOfPillars;++j)
			{
...
FREE_RETURN :
	...
	if(weights) free_dvector(weights, 0, iNFloatDates-1);

	if(volvect)	free_dvector(volvect, 0, cvm->nbOfPillars-1);
	if(volloc)	free_dvector(volloc, 0, cvm->nbOfPillars-1);

	if(x) free_dvector(x, 0, cvm->nQuadLegendre-1);
	if(w) free_dvector(w, 0, cvm->nQuadLegendre-1);

	return err;
}


Err cvm_partial_forward_vol(double date, double expiry, double start, double end, double *vol, cvm_str *cvm)
{
	Err err=NULL;
	...
	else
	{
		...
		while( (dateIndex2<cvm->nbOfDates) && ( date > cvm->vol_dates[dateIndex2]) )
		{
		...
		{
			GaussLeg(time, cvm->vol_times[dateIndex2], x-1, w-1, cvm->nQuadLegendre);
			for(k=0;k<cvm->nQuadLegendre;++k)
			{
				for(j=0;j<cvm->nbOfPillars;++j)
				{
...
Err cvm_partial_forward_cov(double date, double expiry, double start1, double end1, double start2, double end2, double *vol1, double *vol2, double *cov, cvm_str *cvm)
{
	Err err=NULL;
	int i, j, k, l, dateIndex, dateIndex2;
	SwapDP Swap1, Swap2;
	...
	long *lFloatFixingDates1=NULL, *lFloatPayDates1=NULL, *lFloatStartDates1=NULL, *lFloatEndDates1=NULL;
	...
	long *lFloatFixingDates2=NULL, *lFloatPayDates2=NULL, *lFloatStartDates2=NULL, *lFloatEndDates2=NULL;
	...
	//Underlying 1
    err = swp_f_setSwapDP((long)(start1), (long)(end1), cvm->srtFreq, cvm->srtBasis, &Swap1);
	if(err) goto FREE_RETURN;
														
	err = swp_f_make_FixedLegDatesAndCoverages(&Swap1,lToday,&lFixPayDates1,&iNFixPayDates1,
								  			   &lFixStartDates1,&lFixEndDates1,
								  			   &dFixCoverages1,&iNFixDates1);
	if(err) goto FREE_RETURN;

	err = swp_f_make_FloatLegDatesCoveragesAndSpreads(&Swap1, lToday, cvm->refRate, 
												&lFloatPayDates1,
												&iNFloatPayDates1, 
												&lFloatFixingDates1, 
												&lFloatStartDates1,
	...
	{
		level1 += dFixCoverages1[i] * swp_f_df(lToday, lFixPayDates1[i+1], cvm->ycid);
	}

	weights1 = dvector(0,iNFloatDates1-1);
	for(i=0;i<iNFloatDates1;++i)
	{
		weights1[i] = dFloatCoverages1[i] * swp_f_df(lToday, lFloatPayDates1[i+1], cvm->ycid) / level1;
	}

	volvect1 = dvector(0,cvm->nbOfPillars-1);
	volloc1 = dvector(0,cvm->nbOfPillars-1);

	exercise1 = add_unit((long)(start1), -cvm->spotLag, SRT_BDAY, MODIFIED_SUCCEEDING);
	exercise_time1 = (exercise1 - lToday) / 365.0;

	//Underlying 2
    err = swp_f_setSwapDP((long)(start2), (long)(end2), cvm->srtFreq, cvm->srtBasis, &Swap2);
	if(err) goto FREE_RETURN;
														
	err = swp_f_make_FixedLegDatesAndCoverages(&Swap2,lToday,&lFixPayDates2,&iNFixPayDates2,
								  			   &lFixStartDates2,&lFixEndDates2,
								  			   &dFixCoverages2,&iNFixDates2);
	if(err) goto FREE_RETURN;

	err = swp_f_make_FloatLegDatesCoveragesAndSpreads(&Swap2, lToday, cvm->refRate, 
												&lFloatPayDates2,
												&iNFloatPayDates2, 
												&lFloatFixingDates2, 
												&lFloatStartDates2,
	...
	{
		level2 += dFixCoverages2[i] * swp_f_df(lToday, lFixPayDates2[i+1], cvm->ycid);
	}

	weights2 = dvector(0,iNFloatDates2-1);
	for(i=0;i<iNFloatDates2;++i)
	{
		weights2[i] = dFloatCoverages2[i] * swp_f_df(lToday, lFloatPayDates2[i+1], cvm->ycid) / level2;
	}

	volvect2 = dvector(0,cvm->nbOfPillars-1);
	volloc2 = dvector(0,cvm->nbOfPillars-1);

	exercise2 = add_unit((long)(start2), -cvm->spotLag, SRT_BDAY, MODIFIED_SUCCEEDING);
	...
	if(exercise <= cvm->vol_dates[0])
	{
		...
		for(k=0;k<cvm->nQuadLegendre;++k)
		{
			...
			for(i=0;i<iNFloatDates1;++i)
			{
				err = cvm_compute_fwdvol(x[k], (lFloatStartDates1[i]-lToday)/365.0, cvm->nbOfPillars, volloc1, cvm);
				if(err) goto FREE_RETURN;
				for(j=0;j<cvm->nbOfPillars;++j)
				{
			...
			for(i=0;i<iNFloatDates2;++i)
			{
				err = cvm_compute_fwdvol(x[k], (lFloatStartDates2[i]-lToday)/365.0, cvm->nbOfPillars, volloc2, cvm);
				if(err) goto FREE_RETURN;
				for(j=0;j<cvm->nbOfPillars;++j)
				{
...
FREE_RETURN :
	...
	if(lFixPayDates1) free(lFixPayDates1);
	if(lFixStartDates1) free(lFixStartDates1);
	if(lFixEndDates1) free(lFixEndDates1);
	if(dFixCoverages1) free(dFixCoverages1);

	if(lFloatPayDates1) free(lFloatPayDates1);
	if(lFloatFixingDates1) free(lFloatFixingDates1);
	if(lFloatStartDates1) free(lFloatStartDates1);
	if(lFloatEndDates1) free(lFloatEndDates1);
	if(dFloatCoverages1) free(dFloatCoverages1);
	if(dFloatSpreads1) free(dFloatSpreads1);

	if(weights1) free_dvector(weights1, 0, iNFloatDates1-1);

	if(volvect1)	free_dvector(volvect1, 0, cvm->nbOfPillars-1);
	if(volloc1)	free_dvector(volloc1, 0, cvm->nbOfPillars-1);

	if(lFixPayDates2) free(lFixPayDates2);
	if(lFixStartDates2) free(lFixStartDates2);
	if(lFixEndDates2) free(lFixEndDates2);
	if(dFixCoverages2) free(dFixCoverages2);

	if(lFloatPayDates2) free(lFloatPayDates2);
	if(lFloatFixingDates2) free(lFloatFixingDates2);
	if(lFloatStartDates2) free(lFloatStartDates2);
	if(lFloatEndDates2) free(lFloatEndDates2);
	if(dFloatCoverages2) free(dFloatCoverages2);
	if(dFloatSpreads2) free(dFloatSpreads2);

	if(weights2) free_dvector(weights2, 0, iNFloatDates2-1);

	if(volvect2)	free_dvector(volvect2, 0, cvm->nbOfPillars-1);
	if(volloc2)	free_dvector(volloc2, 0, cvm->nbOfPillars-1);
...
Err cvm_compute_sensitivities(double time, double maturity, 
					  int nPillars, double *sensi, cvm_str *cvm)
{
	Err err=NULL;
	...
	if(cvm->sensi_type==0)		//Parametric Sensitivities
	{
		...
		{
			dateIndex = 0;
			while( (dateIndex<cvm->nbOfDates) && (time > cvm->vol_times[dateIndex]))
			{
				dateIndex = dateIndex +1;
			}
		...
		for(i=0;i<cvm->nbOfPillars-1;++i)
		{
			err = splint(cvm->pillars_times-1, 
						cvm->sensitivities[i]-1, cvm->sensitivities2[i]-1, 
						cvm->nbOfPillars, maturity - time, &sensi[i]);
			if(err) goto FREE_RETURN;
			sensi[i] = sensi[i] * exp( - lambda[i] * (maturity-time - cvm->pillars_times[i]));
			sum += sensi[i];
		}
		sensi[i] = 1.0 - sum;
	}
	else if(cvm->sensi_type==1)		//Custom Sensitivities
	{
		...
		for(i=0;i<cvm->nbOfPillars-1;++i)
		{
			err = splint(cvm->sensi_times-1, 
						cvm->sensitivities[i]-1, cvm->sensitivities2[i]-1, 
						cvm->nbOfSensiTenors, maturity - time, &sensi[i]);
			if(err) goto FREE_RETURN;
			sum += sensi[i];
		}
		sensi[i] = 1.0 - sum;
	...
	{
		index = 0;
		while( (index<cvm->nbOfPillars) && ((maturity-time) > cvm->pillars_times[index]))
		{
			index=index+1;
		}
		...
		else
		{
			...
			sensi[index] = cvm->sensitivities[index][index-1] 
					+ (cvm->sensitivities[index][index] - cvm->sensitivities[index][index-1]) 
...
FREE_RETURN:
	if(lambda) free_dvector(lambda, 0, cvm->nbOfPillars-1);
...
Err	cvm_free_struct(cvm_str *cvm)
{
	...
	if(cvm)
	{
		...
		if(cvm->lambdas)
		{
			free_dmatrix(cvm->lambdas, 0, cvm->nbOfDates-1, 0, cvm->nbOfPillars-1);
			cvm->lambdas = NULL;
		}

		if(cvm->sensitivities)
		{
			free_dmatrix(cvm->sensitivities, 0, cvm->nbOfPillars-1, 0, cvm->nbOfSensiTenors-1);
			cvm->sensitivities = NULL;
		}

		if(cvm->sensitivities2)
		{
			free_dmatrix(cvm->sensitivities2, 0, cvm->nbOfPillars-1, 0, cvm->nbOfSensiTenors-1);
			cvm->sensitivities2 = NULL;
		}
		...
		if(cvm->market_vols)
		{
			free_dmatrix(cvm->market_vols, 0, cvm->nbOfOptionTenors-1, 0, cvm->nbOfUnderlyingTenors-1);
			cvm->market_vols = NULL;
		}
		
		if(cvm->model_vols)
		{
			free_dmatrix(cvm->model_vols, 0, cvm->nbOfOptionTenors-1, 0, cvm->nbOfUnderlyingTenors-1);
			cvm->model_vols = NULL;
		}
...
Err sortspline(int nbasis, double *xbasis, double *ybasis, 
			   int n, double *x, double *y)
{
	...
	for(i=0;i<n;++i)
	{
		err = splint(xbasis-1, ybasis-1, ybasis2-1, 
					nbasis, x[i], &y[i]);
	}

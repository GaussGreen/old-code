...
Err SwapElements_Init(long start, long end, char *freqStr, char *basisStr,
                      char *refrateStr, char *recpayStr, double fixed,
                      int elements, long today, double **times, double **cflows,
                      int *len) {
  Err err = NULL;
  ...
  if (elements & SE_FIXED) {
    ...
    while (act_date > start_date) {
      theo_date =
          add_unit(theo_date, -12 / freq, SRT_MONTH, NO_BUSDAY_CONVENTION);

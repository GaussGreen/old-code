...
********************************************************************************/
Err srt_f_GCRepoCurve(long TradeDate, int N, long *TermDates, /* [0 to N-1] */
  ...
  // Step 2: Get discount factor and forward overnight and term rates for every
  // calendar day.
  NumDays = (int)x[N];
  ...
  if ((int)(x[1]) > 1) // Cases when O/N rate applies for more than 1 day --
                       // weekend        , etc
  {
    for (i = 1; i < (int)(x[1]); i++)
      Gfit[i] = G[1] * i / x[1];
...
/********************************************************************************
  ...
  Output: Arrays of consecutive dates and corresponding forward and term rates
********************************************************************************/
Err srt_f_SpecialRepoCurve(long TradeDate, int N,
  ...
  for (i = 2; i <= N; i++) {
    rateTemp =
        (pow(disc[i - 1] / disc[i], 1.0 / (xn[i] - xn[i - 1])) - 1) * 36000;

...
/* ==========================================================================
   ...
   PURPOSE:		Modelling of the spot FX vol by taking in consideration:
                                ...
                                - A term structure of correlations (4x4
   correlation tensor)
...
static Err calculate_all_boundaries(
    ...
    long nprod, long **nb_nodes_tab, long *nb_nodes_max,
  ...
  for (step = nstp - 1; step >= 1; step--) {
    ...
    err = calculate_boundaries(
        sqt_prev_inv, expectation, vart, correl[step], transf_matrix_inv,
    ...
    for (i = 0; i < 3; i++) {
      ...
      if (nb_nodes_tab[step][i] > MAXNODE3FQUANTO) {
        err = serror(
            ...
            nb_nodes_tab[step][i], MAXNODE3FQUANTO);
        goto FREE_RETURN_BOUNDARIES;
      }
    }
...
Err tree_main_3fquanto(
    ...
    int *vol_change, /*	1 if one of the volatilities has changed        , 0
                      * otherwise
                      */

    /*	Term structures & Distributions */
    double r1_lam, double *r1_sig, double *r1_fwd, double *r1_var,

    double r2_lam, double *r2_sig, double *r2_fwd, double *r2_var,

    double *r1_r2_covar,

    double *lgm2F_ifr, char *lgm2F_yc, int lgm2F_is_dom_for,

    double r3_lam, double *r3_sig, double *r3_fwd, double *r3_var,

    double *lgm1F_ifr, char *lgm1F_yc,
    ...
    /*	Correlations */
    double *corr_r1_r2, double *corr_r1_r3, double *corr_r1_fx,
    double *corr_r2_r3, double *corr_r2_fx, double *corr_r3_fx,
  ...
  if (!lgm2F_ifr || !lgm1F_ifr || !transf_matrix || !transf_matrix_next ||
  ...
  for (i = 0; i < nstp; i++) {
    // The important correlations for the boundaries are the correlations
    // between r1        , r2        , r3 {0: r1        , 1: r2        , 2: r3}
  ...
  err = calculate_all_boundaries(
      nstp, time, vol_change, r1_sig, r2_sig, r3_sig, r1_fwd, r1_var, r2_fwd,
      r2_var, r3_fwd, r3_var, correl, nprod, nb_nodes_tab, nb_nodes_max,
  ...
  err =
      payoff_func(date[nstp - 1], time[nstp - 1], func_parm_tab[nstp - 1],
                  lgm2F_yc, r1_lam, r2_lam, r1_var[nstp - 1], r2_var[nstp - 1],
                  r1_r2_covar[nstp - 1], corr_r1_r2[nstp - 1], lgm1F_yc, r3_lam,
                  r3_var[nstp - 1], lgm2F_is_dom_for, nb_nodes_next[0],
                  nb_nodes_next[1], nb_nodes_next[2], sv, nprod, values_next);
  ...
  for (step = nstp - 2; step >= 1; step--) {
    ...
    /*	Precalculate quantities used to compute original state variables and
       dual expectations E[R_t+dt_in_base_t+dt] = transf_matrix_inv_t+dt * A
                                                       + transf_matrix_inv_t+dt
       * B * transf_matrix_t * R_t_in_base_t*/

    // A = vector (a1        , a2        , a3) and calculate
    // transf_matrix_inv_t+dt * A
    ...
    // B = diag(b1        , b2        , b3) and calculate transf_matrix_inv_t+dt
    // * B * transf_matrix_t
    ...
    /*	Eval payoff */
    if (eval_evt[step]) {
      err = payoff_func(date[step], time[step], func_parm_tab[step], lgm2F_yc,
                        r1_lam, r2_lam, r1_var[step], r2_var[step],
                        r1_r2_covar[step], corr_r1_r2[step], lgm1F_yc, r3_lam,
                        r3_var[step], lgm2F_is_dom_for, nb_nodes[0],
                        nb_nodes[1], nb_nodes[2], sv, nprod, values);
      if (err) {
        goto FREE_RETURN;
      }
    }
  ...
  /*	Precalculate quantities used to compute original state variables and
     dual expectations E[R_t1_in_base_t1] = transf_matrix_inv_t1 * (A + B *
     transf_matrix_t0 * R_t0_in_base_t0) */
  ...
  if (eval_evt[0]) {
    ...
    err = payoff_func(date[0], time[0], func_parm_tab[0], lgm2F_yc, r1_lam,
                      r2_lam, r1_var[0], r2_var[0], r1_r2_covar[0],
                      corr_r1_r2[0], lgm1F_yc, r3_lam, r3_var[0],
                      lgm2F_is_dom_for, 1, 1, 1, sv, nprod, values);

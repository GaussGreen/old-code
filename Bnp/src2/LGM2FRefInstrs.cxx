...
typedef struct {
  Date date_today, *ex_dates, *start_dates, *caplet_end_dates, *swp_pay_dates,
      last_swp_pay_date, cap_theo_end_date, *float_start_dates,
      *float_end_dates, *float_fixing_dates;

  double *cvg_cap, *cvg_pay, *cvg_first, *df_start, *df_pay, *df_cap,
      **long_swps_payts, *long_swps_strikes, *long_swps_fwd, *long_swps_beta,
      *long_swps_cev, *long_swps_stdev, **short_swps_payts, *short_swps_strikes,
      *short_swps_fwd, *short_swps_beta, *short_swps_cev, *short_swps_pv,
      *short_swps_vega, *long_swps_pv, *long_swps_vega, *float_coverages,
      *caplets_strikes, *caplets_pv, *caplets_vega, *lgm_cap_vol,
      *float_start_dates_discount_factors, *float_end_dates_discount_factors,
      hybrid_swpts_pv, hybrid_swpts_vega, pv_full_cap, vega_full_cap;
...
static double *s_PVs, *s_WeighedPVs, *s_zeta1_ptr, *s_du, *s_dv,
    *s_optim_lambda, **s_long_swpts_cond_prob, **s_short_swpts_cond_prob,
...
double Zeta1_Func(Date tRef, Date *ex_dates, long nEx, Date date_today,
                  double *Zeta1) {
  double tRefZeta, W;
  int i;
  ...
  if (tRef > ex_dates[nEx]) {
    ...
    tRefZeta =
        Zeta1[i - 1] +
        W * (Zeta1[i] - Zeta1[i - 1]); /* Zeta1[i-1] = Zeta1(ex_dates[i]) */
  }

  return tRefZeta;
}

double XL_New_Zeta2_Func(Date tRef, Date *ex_dates, long nEx, Date date_today,
  ...
  tRefZeta = New_Zeta2_Func(tRef, ex_dates, nEx, date_today, Zeta1, Zeta2,
                            alpha, gamma, rho);
...
double XL_New_Zeta12_Func(Date tRef, Date *ex_dates, long nEx, Date date_today,
                          double *Zeta1, double alpha, double gamma,
                          double rho) {
  long i;
  ...
  tRefZeta = New_Zeta12_Func(tRef, ex_dates, nEx, date_today, Zeta1, Zeta12,
                             alpha, gamma, rho);
...
double New_Zeta2_Func(Date tRef, Date *ex_dates, long nEx, Date date_today,
                      double *Zeta1, double *Zeta2, double alpha, double gamma,
                      double rho) {
  int i;
  ...
  if (i == 1) /* CASE WHERE ex_dates[1] =< tRef < ex_dates[2] */
  {
    ...
    if (tRef > ex_dates[1]) /* CASE WHERE tRef > ex_dates[1] */
    {
      ...
      Weight = AlphaSq / (2 * gamma) *
               (exp(2 * gamma * t) - exp(2 * gamma * Prevt)) / dt;
  ...
  } else /* CASE WHERE tRef >= ex_dates[2] */
  {
    ...
    if ((tRef > ex_dates[i]) &&
        (i < nEx)) /* CASE WHERE tRef > ex_dates[i]         , i < nEx */
    {
      Nextt = s_yr_to_ex_dates_ptr[i + 1];
      Prevt = s_yr_to_ex_dates_ptr[i];
      dt = (Nextt - Prevt);
...
double New_Zeta12_Func(Date tRef, Date *ex_dates, long nEx, Date date_today,
                       double *Zeta1, double *Zeta12, double alpha,
  ...
  } else /* CASE WHERE tRef >= ex_dates[2] */
  {
    ...
    if (tRef >
        ex_dates[i]) /* CASE WHERE tRef > ex_dates[i]         , i < nEx */
    {
      Nextt = s_yr_to_ex_dates_ptr[i + 1];
      Prevt = s_yr_to_ex_dates_ptr[i];
      dt = (Nextt - Prevt);
...
double H1_Func(Date tRef, Date *start_dates, Date date_today, long nEx,
               double *H1) {
  double tRefH, W;
  long i;
  ...
  /* start_dates[nEx] IS THE START DATE OF THE LAST SWAPTION &&
   * last_swp_pay_date IS THE LAST PAYMENT DATE */

  if ((tRef >= start_dates[nEx]) && (tRef <= last_swp_pay_date)) {
    W = (double)(tRef - start_dates[nEx]) /
        (last_swp_pay_date - start_dates[nEx]);
    tRefH = (1 - W) * H1[nEx - 1];
...
double XLH1_Func(Date tRef, Date *start_dates, Date date_today,
                 Date last_swp_pay_date, long nEx, double *H1) {
  double tRefH1;
...
double XLH2_Func(Date tRef, Date *start_dates, Date date_today,
                 Date last_swp_pay_date, long nEx, double *H1, double gamma) {
  double tRefH2;
...
double H2_Func(Date tRef, Date *start_dates, Date date_today, long nEx,
  ...
  if (tRef == last_swp_pay_date) {
    ...
    Weight = (exp(-gamma * Nextt) - exp(-gamma * t)) /
             (gamma * dt); /* Weight > 0  */
    tRefH = -Weight * H1[nEx - 1];
    ...
    if (j == nEx) {
      ...
      if (tRef > start_dates[j]) {
        ...
        dt = (Nextt - Prevt);
        t = (double)(tRef - date_today) * YEARS_IN_DAY; /* Prevt < t < Nextt  */
        Weight = (exp(-gamma * Prevt) - exp(-gamma * t)) /
                 (gamma * dt); /* Weight > 0 */
        tRefH += Weight * (H1[j] - H1[j - 1]);
      }
    }
...
static Err srt_f_autocal_df(Date swp_pay_dates, double df_pay, double U,
                            double V, double zeta1_ex, double zeta2_ex,
                            double zeta12_ex, double H1tPay, double H2tPay,
                            double *ZC) {
  double X1tEx, X2tEx;
  ...
  if ((zeta1_ex > 0.0) && (zeta2_ex > 0.0)) {
    ...
    (*ZC) =
        df_pay *
        exp(H1tPay * X1tEx + H2tPay * X2tEx - H1tPay * H2tPay * zeta12_ex -
...
static Err srt_f_autocal_swap_pv(long ex_index, long num_cpn, double Rf,
                                 double U, double V, double zeta1_ex,
                                 double zeta2_ex, double zeta12_ex, double *H1,
                                 double *SWAP) {
  ...
  double *cvg_pay, ak, cvg_first,
      *df_start = SParms.df_start, *df_pay = SParms.df_pay, H1tStart, H2tStart,
      H1tPay, H2tPay, ZCtStart, ZCtPayk, gamma = SParms.gamma;
  Err err = NULL;
  ...
  H2tStart =
      H2_Func(start_dates[ex_index], start_dates, date_today, nEx, H1, gamma);

  err = srt_f_autocal_df(start_dates[ex_index], df_start[ex_index], U, V,
                         zeta1_ex, zeta2_ex, zeta12_ex, H1tStart, H2tStart,
                         &ZCtStart);
  if (err)
    return err;
  ...
  cvg_first = SParms.cxxvg_first[ex_index];
  cvg_pay = SParms.cxxvg_pay;

  for (k = 0; k <= num_cpn; k++) {
    ak = 0.;
    H1tPay = H1_Func(swp_pay_dates[first_cpn_index + k], start_dates,
                     date_today, nEx, H1);
    H2tPay = H2_Func(swp_pay_dates[first_cpn_index + k], start_dates,
                     date_today, nEx, H1, gamma);

    err = srt_f_autocal_df(swp_pay_dates[first_cpn_index + k],
                           df_pay[first_cpn_index + k], U, V, zeta1_ex,
                           zeta2_ex, zeta12_ex, H1tPay, H2tPay, &ZCtPayk);
    if (err)
      return err;
...
Err make_autocal_cond_prob(double *gi, double *Wi, double du_s, double *s_du,
                           long jEx, long num_cpn, double Rf, double *Zeta1,
                           double *Zeta2, double *Zeta12, double *H1, long nher,
                           double ***cond_prob) {
  Err err = NULL;
  ...
  for (l = 1; l <= nher; l++) {
    nstop = 0.0;
    a[0] = 1;
    srt_f_autocal_swap_pv(jEx, num_cpn, Rf, a[0], gi[l], Zeta1[jEx - 1],
                          Zeta2[jEx - 1], Zeta12[jEx - 1], H1, &b[0]);
    a[1] = 1.01;
    srt_f_autocal_swap_pv(jEx, num_cpn, Rf, a[1], gi[l], Zeta1[jEx - 1],
                          Zeta2[jEx - 1], Zeta12[jEx - 1], H1, &b[1]);
    ...
    while (nstop < 1.0 && k < MAX_ITERATION) {
      srt_f_autocal_swap_pv(jEx, num_cpn, Rf, a[2], gi[l], Zeta1[jEx - 1],
                            Zeta2[jEx - 1], Zeta12[jEx - 1], H1, &b[2]);
...
static Err make_static_allocations(long nEx, long nPay) {
  ...
  if ((s_PVs == NULL) || (s_WeighedPVs == NULL) || (s_zeta1_ptr == NULL) ||
      (s_yr_to_ex_dates_ptr == NULL) || (s_yr_to_start_ptr == NULL) ||
      (s_du == NULL) || (s_dv == NULL) || (s_long_swpts_cond_prob == NULL) ||
...
static Err make_levenberg_struct(long nEx) {
  ...
  for (i = nEx + 1; i <= nEx + nEx; i++) {
    ...
    s_levenberg_marquard_weight_for_calibrate[i] =
        SParms.long_swps_vega[i - nEx];
  }
...
static Err srt_f_autocal_caplet_greeks(long jEx, double zeta1_ex,
                                       double *caplet_pv) {
  Err err = NULL;
  double *H1 = SParms.H1, *H2 = SParms.H2, *Zeta1 = SParms.Zeta1,
         *Zeta2 = SParms.Zeta2, *Zeta12 = SParms.Zeta12,
         *df_start = SParms.df_start, *df_cap = SParms.df_cap,
         *cvg_cap = SParms.cxxvg_cap;
  Date *ex_dates = SParms.ex_dates, date_today = SParms.date_today,
       *start_dates = SParms.start_dates,
       *caplet_end_dates = SParms.cxxaplet_end_dates;
  double zeta2_ex, zeta12_ex, H1tStart, H1tEnd, H2tStart, H2tEnd, cap_vol,
      sq_cap_vol, gamma = SParms.gamma, t;
  long nEx = SParms.nEx;
  ...
  H2tStart = H2[jEx - 1];
  H2tEnd =
      H2_Func(caplet_end_dates[jEx], start_dates, date_today, nEx, H1, gamma);
  ...
  (*caplet_pv) = 0.;
  (*caplet_pv) =
      srt_f_optblksch(df_start[jEx] / df_cap[jEx],
                      (1 + cvg_cap[jEx] * SParms.short_swps_strikes[jEx]),
                      cap_vol, t, df_cap[jEx], SRT_PUT, SRT_PREMIUM);

  return err;
}

static Err srt_f_autocal_swpts_greeks(long jEx, long num_cpn, double Rf,
                                      double **SwapPayts, double zeta1_ex,
                                      double *swption_pv) {
  Err err = NULL;
  double *Zeta1 = SParms.Zeta1, *Zeta2 = SParms.Zeta2, *Zeta12 = SParms.Zeta12,
         ...
         *df_start = SParms.df_start, RootZeta1tEx, zeta2_ex, RootZeta2tEx,
         zeta12_ex, RHO, H1tStart, H2tStart, duStart, dvStart, dvStartSq, H1tk,
         H2tk, SwpPayoff;
  ...
  long nPay = SParms.nPay, nEx = SParms.nEx,
       *first_cpn_index = SParms.first_cpn_index, k, l, PayIndex;

  Zeta1[jEx - 1] = zeta1_ex;
  zeta2_ex = New_Zeta2_Func(ex_dates[jEx], ex_dates, nEx, date_today, Zeta1,
                            Zeta2, alpha, gamma, rho);
  Zeta2[jEx - 1] = zeta2_ex;
  zeta12_ex = New_Zeta12_Func(ex_dates[jEx], ex_dates, nEx, date_today, Zeta1,
                              Zeta12, alpha, gamma, rho);
  ...
  for (k = 0; k <= num_cpn; k++) {
    ...
    H2tk = H2_Func(swp_pay_dates[PayIndex], start_dates, date_today, nEx, H1,
                   gamma);
  ...
  err = make_autocal_cond_prob(gi, Wi, duStart, s_du, jEx, num_cpn, Rf, Zeta1,
                               Zeta2, Zeta12, H1, NBR_HERMITE_POINTS,
  ...
  for (l = 1; l <= NBR_HERMITE_POINTS; l++) {
    ...
    SwpPayoff -= df_start[jEx] * s_long_swpts_cond_prob[0][l] *
                 exp(gi[l] * dvStart - 0.5 * dvStartSq);
...
static Err srt_f_autocal_cap_greeks(double *cap_pv) {
  Err err = NULL;
  Date *float_start_dates = SParms.float_start_dates,
       *float_end_dates = SParms.float_end_dates,
       *float_fixing_dates = SParms.float_fixing_dates,
       *start_dates = SParms.start_dates, *ex_dates = SParms.ex_dates,
       date_today = SParms.date_today;
  double *H1 = SParms.H1, *Zeta1 = SParms.Zeta1, *Zeta2 = SParms.Zeta2,
         *Zeta12 = SParms.Zeta12, alpha = SParms.alpha, gamma = SParms.gamma,
         rho = SParms.rho,
         *float_start_dates_discount_factors =
             SParms.float_start_dates_discount_factors,
         *float_end_dates_discount_factors =
             SParms.float_end_dates_discount_factors,
         *float_coverages = SParms.float_coverages, zeta1_ex, zeta2_ex,
         zeta12_ex, H1tStart, H1tEnd, H2tStart, H2tEnd, DtStart, DtEnd, cap_vol,
         sq_cap_vol, t, caplet_pv, CvgFloat;
  ...
  for (j = 1; j <= num_float_dates; j++) {
    ...
    caplet_pv = 0.;
    H1tStart =
        H1_Func(float_start_dates[j - 1], start_dates, date_today, nEx, H1);
    H1tEnd = H1_Func(float_end_dates[j - 1], start_dates, date_today, nEx, H1);

    H2tStart = H2_Func(float_start_dates[j - 1], start_dates, date_today, nEx,
                       H1, gamma);
    H2tEnd = H2_Func(float_end_dates[j - 1], start_dates, date_today, nEx, H1,
                     gamma);

    t = (double)(float_fixing_dates[j - 1] - date_today) * YEARS_IN_DAY;

    zeta1_ex =
        Zeta1_Func(float_fixing_dates[j - 1], ex_dates, nEx, date_today, Zeta1);
    zeta2_ex = New_Zeta2_Func(float_fixing_dates[j - 1], ex_dates, nEx,
                              date_today, Zeta1, Zeta2, alpha, gamma, rho);
    zeta12_ex = New_Zeta12_Func(float_fixing_dates[j - 1], ex_dates, nEx,
                                date_today, Zeta1, Zeta12, alpha, gamma, rho);
    ...
    if (num_float_dates > 1)
      ...
      CvgFloat = coverage(float_start_dates[j - 1], float_end_dates[j - 1],
                          SParms.float_basis);

    caplet_pv = srt_f_optblksch(DtStart / DtEnd,
                                (1 + CvgFloat * SParms.short_swps_strikes[i]),
                                cap_vol, t, DtEnd, SRT_PUT, SRT_PREMIUM);
...
static Err srt_f_autocal_hybrid_swpts_greeks(double *hybrid_swpts_pv) {
  ...
  double **short_swps_payts = SParms.short_swps_payts, *Zeta1 = SParms.Zeta1,
         hybrid_swaplet_pv;
  double Rf;
  ...
  for (i = 1; i < nEx; i++) {
    ...
    err = srt_f_autocal_swpts_greeks(i, num_short_swp_cpn[i], Rf,
                                     short_swps_payts, Zeta1[i - 1],
                                     &hybrid_swaplet_pv);
    if (err)
      return err;
...
static Err levenberg_func_for_tau(double diag_swp_index, double *zeta1_param,
                                  double *levenberg_value,
                                  double *levenberg_value_der, int nparm) {
  Err err = NULL;
  long jEx = SParms.ex_index, nEx = SParms.nEx, nPay = SParms.nPay,
       *first_cpn_index = SParms.first_cpn_index, num_cpn;
  double *Zeta1 = SParms.Zeta1, *Zeta2 = SParms.Zeta2, *Zeta12 = SParms.Zeta12,
         alpha = SParms.alpha, gamma = SParms.gamma, rho = SParms.rho, zeta1_ex,
         diag_swaption_pv, param_shift, zero_shift = 1e-6, prop_shift = 1e-3,
  ...
  if ((*levenberg_value) == 0) {
    Zeta2[jEx - 1] = New_Zeta2_Func(ex_dates[jEx], ex_dates, nEx, date_today,
                                    Zeta1, Zeta2, alpha, gamma, rho);
    Zeta12[jEx - 1] = New_Zeta12_Func(ex_dates[jEx], ex_dates, nEx, date_today,
                                      Zeta1, Zeta12, alpha, gamma, rho);

    num_cpn = nPay - first_cpn_index[jEx] - 1;

    Rf = SParms.long_swps_strikes[jEx];
    err = srt_f_autocal_swpts_greeks(jEx, num_cpn, Rf, long_swps_payts,
                                     Zeta1[jEx - 1], &diag_swaption_pv);
    if (err)
      return err;
...
static Err srt_f_autocal_calboot(double *optim_params, LGMCalMeth cal_method) {
  ...
  double *H1 = SParms.H1, *H2 = SParms.H2, alpha = SParms.alpha,
         gamma = SParms.gamma, rho = SParms.rho, *Zeta1 = SParms.Zeta1,
         *Zeta2 = SParms.Zeta2, *Zeta12 = SParms.Zeta12,
         *df_start = SParms.df_start, *df_cap = SParms.df_cap,
         *cvg_cap = SParms.cxxvg_cap;
  ...
  default:
    ...
    for (i = 1; i <= nEx; i++)
      H2[i - 1] =
          H2_Func(start_dates[i], start_dates, date_today, nEx, H1, gamma);
  ...
  if (SParms.use_start_point_for_zeta1s == SRT_NO) {
    ...
    H2tStart = H2[0];
    H2tEnd =
        H2_Func(caplet_end_dates[1], start_dates, date_today, nEx, H1, gamma);

    zeta_den = (H1tEnd - H1tStart) * (H1tEnd - H1tStart) +
               (alpha * alpha) / (2 * gamma * s_yr_to_ex_dates_ptr[1]) *
                   (exp(2 * gamma * s_yr_to_ex_dates_ptr[1]) - 1) *
                   (H2tEnd - H2tStart) * (H2tEnd - H2tStart) +
               2 * (alpha * rho) / (gamma * s_yr_to_ex_dates_ptr[1]) *
                   (exp(gamma * s_yr_to_ex_dates_ptr[1]) - 1) *
                   (H1tEnd - H1tStart) * (H2tEnd - H2tStart);
  }
  ...
  for (i = 1; i <= nEx; i++) {
    ...
    if (i == 1) {
      zeta_bound_post_fact =
          ONE_HALF * (exp(2 * s_yr_to_ex_dates_ptr[1] * optim_params[1]) - 1) /
          optim_params[1];

      SParms.zeta1_min = zeta1_min =
          FIRST_EX_SIGMA_MIN_SQ * zeta_bound_post_fact;
      SParms.zeta1_max = zeta1_max = SIGMA_MAX_SQ * zeta_bound_post_fact;

      if (SParms.use_start_point_for_zeta1s == SRT_NO)
        s_zeta1_ptr[1] = min(max(s_yr_to_ex_dates_ptr[1] * lgm_cap_vol[1] *
                                     lgm_cap_vol[1] / zeta_den,
                                 zeta1_min),
                             zeta1_max);
      ...
      if (SParms.use_start_point_for_zeta1s == SRT_NO) {
        ...
        H2tStart = H2[i - 1];
        H2tEnd = H2_Func(caplet_end_dates[i], start_dates, date_today, nEx, H1,
                         gamma);
        ...
        zeta_den = (H1tEnd - H1tStart) * (H1tEnd - H1tStart) +
                   ...
                   2 * (alpha * rho) / (gamma * s_yr_to_ex_dates_ptr[i]) *
                       (exp(gamma * s_yr_to_ex_dates_ptr[i]) - 1) *
                       (H1tEnd - H1tStart) * (H2tEnd - H2tStart);
      }
      ...
      SParms.zeta1_max = zeta1_max =
          ONE_HALF * SIGMA_MAX_SQ *
          (exp(2 * s_yr_to_ex_dates_ptr[i] * optim_params[1]) - 1) /
          optim_params[1];

      if (SParms.use_start_point_for_zeta1s == SRT_NO)
        s_zeta1_ptr[1] = min(max(s_yr_to_ex_dates_ptr[i] * lgm_cap_vol[i] *
                                     lgm_cap_vol[i] / zeta_den,
                                 zeta1_min),
                             zeta1_max);

      else
        s_zeta1_ptr[1] =
            min(max(SParms.start_zeta1s[i - 1], zeta1_min), zeta1_max);
    ...
    while ((nstop < 1.0) && (j < MAX_ITERATION)) {
      ...
      err = levenberg_marquardt(s_levenberg_marquard_data_for_swpcalboot,
                                s_levenberg_marquard_target_for_swpcalboot,
                                s_levenberg_marquard_weight_for_swpcalboot, 1,
                                s_zeta1_ptr, 1, niter, &levenberg_func_for_tau,
                                &chisq);
      if (err) {
...
static Err make_swaptions_payts(long nEx, long nPay, long *first_cpn_index,
                                double *cvg_first, double *cvg_pay,
                                double *df_pay, double *K, double **SwapPayts) {
  Err err = NULL;
  long j, k, num_cpn;

  for (j = 1; j <= nEx; j++) /* Loop on all the swaption        , i.e loop on
                                all the exercise dates */
  {
    ...
    for (k = 0; k <= num_cpn; k++) /* Loop on Cpns */
    {
      if ((k == 0) && (num_cpn > 0))
        ...
        SwapPayts[j][k] = df_pay[first_cpn_index[j] + k] *
                          cvg_pay[first_cpn_index[j] + k - 1] * K[j];
      else if (k == num_cpn) {
        if (num_cpn > 0)
          SwapPayts[j][k] = df_pay[first_cpn_index[j] + k] *
          ...
          SwapPayts[j][k] =
              df_pay[first_cpn_index[j] + k] * (1 + K[j] * cvg_first[j]);
      }
    } /* End of loop on k */
...
static Err make_short_swaptions_payts(long nEx, long *num_cpn,
                                      long *first_cpn_index, double *cvg_first,
                                      double *cvg_pay, double *df_pay,
                                      double *K, double **SwapPayts) {
  Err err = NULL;
  long j, k;
...
static Err make_static_struct_allocations(
    long nEx, long nPay, Date **caplet_end_dates, long **first_cpn_index,
    double **df_cap, double **df_start, double **df_pay, double **cvg_pay,
    double **cvg_cap, double **cvg_first, double **long_swps_strikes,
    double ***long_swps_payts, double **long_swps_fwd, double **long_swps_cev,
    double **long_swps_beta, double **long_swps_pv, double **long_swps_vega,
    double **long_swps_stdev, double **short_swps_strikes,
    double ***short_swps_payts, double **short_swps_fwd,
    double **short_swps_cev, double **short_swps_beta, double **short_swps_pv,
    double **short_swps_vega, long **num_short_swp_cpn, double **caplets_pv,
    double **caplets_vega, double **caplets_strikes, double **lgm_cap_vol) {
  Err err = NULL;
...
static Err make_static_struct_desallocations(long nEx, long nPay) {
  ...
  if (SParms.cxxaplet_end_dates)
    free_lngvector(SParms.cxxaplet_end_dates, 1, nEx);
  SParms.cxxaplet_end_dates = NULL;
  ...
  SParms.df_pay = NULL;
  if (SParms.cxxvg_pay)
    free_dvector(SParms.cxxvg_pay, 0, nPay - 1);
  SParms.cxxvg_pay = NULL;
  if (SParms.cxxvg_cap)
    free_dvector(SParms.cxxvg_cap, 1, nEx);
  SParms.cxxvg_cap = NULL;
  if (SParms.cxxvg_first)
    free_dvector(SParms.cxxvg_first, 1, nEx);
  SParms.cxxvg_first = NULL;
  ...
  if (SParms.cxxaplets_pv)
    free_dvector(SParms.cxxaplets_pv, 1, nEx);
  SParms.cxxaplets_pv = NULL;
  if (SParms.cxxaplets_vega)
    free_dvector(SParms.cxxaplets_vega, 1, nEx);
  SParms.cxxaplets_vega = NULL;
  if (SParms.cxxaplets_strikes)
    free_dvector(SParms.cxxaplets_strikes, 1, nEx);
  SParms.cxxaplets_strikes = NULL;
...
Err srt_f_get_autocal_ref_instrs(
    long nEx, long nPay, Date *start_dates, Date *swp_pay_dates, char *ycname,
    Err (*srt_f_get_vol)(Date, Date, double, SRT_Boolean, double *),
    Err (*srt_f_get_beta)(Date, Date, double *), double *LongKs,
    LGMRMeth strike_method, LGMCalMeth calib_method,
    long *likely_most_expensive_index, SrtLgmRefSwptnData *lgm_ref_swp_data)

{
  ...
  Date date_today, *ex_dates, *caplet_end_dates, cap_theo_end_date,
      cap_real_end_date, *float_pay_dates, *float_fixing_dates,
      *float_start_dates, *float_end_dates;
  Err err = NULL;
  long i, j, k, num_cpn, num_cap_fp, last_cap_ex_index, spot_lag,
      most_exp_index, *first_cpn_index, *num_short_swp_cpn;
  int num_float_dates, num_float_pay_dates;
  double cev_vol, swp_level, swp_rate, fra_rate, yr_to_exp, atm_vol, norm_vol,
      beta, caplet_pv, caplet_vega, cap_pv, cap_vega, hybrid_swpts_pv,
      hybrid_swpts_vega, yr_from_prev_ex_date, nbr_of_stdev_from_prev_strike,
      sgn, loc_max_stdev, *float_coverages, *float_start_dates_discount_factors,
      *float_end_dates_discount_factors;

  double *df_cap, *df_start, *df_pay, *cvg_pay, *cvg_cap, *cvg_first,
      *long_swps_strikes, **long_swps_payts, *long_swps_fwd, *long_swps_cev,
      *long_swps_beta, *long_swps_pv, *long_swps_vega, *long_swps_stdev,
      *short_swps_strikes, **short_swps_payts, *lgm_cap_vol, *short_swps_fwd,
      *short_swps_cev, *short_swps_beta, *short_swps_pv, *short_swps_vega,
      *caplets_pv, *caplets_vega, *caplets_strikes;

  /* Make the allocations  (the static first) */
  err = make_static_struct_allocations(
      nEx, nPay, &caplet_end_dates, &first_cpn_index, &df_cap, &df_start,
      &df_pay, &cvg_pay, &cvg_cap, &cvg_first, &long_swps_strikes,
      &long_swps_payts, &long_swps_fwd, &long_swps_cev, &long_swps_beta,
      &long_swps_pv, &long_swps_vega, &long_swps_stdev, &short_swps_strikes,
      &short_swps_payts, &short_swps_fwd, &short_swps_cev, &short_swps_beta,
      &short_swps_pv, &short_swps_vega, &num_short_swp_cpn, &caplets_pv,
      &caplets_vega, &caplets_strikes, &lgm_cap_vol);
  if (err)
    return err;
  ...
  if (lgm_ref_swp_data != NULL) {
    ...
    lgm_ref_swp_data->refLongSwptnArr = (SrtLgmRefSwptn *)srt_calloc(
        lgm_ref_swp_data->NrefLongSwptn, sizeof(SrtLgmRefSwptn));
  ...
  /* Compute the exercise dates from the start dates - ex dates is spot_lag bd
  ...
  /* Compute the caplets end dates (caplet_end_dates)         ,the caplets
    coverages (cvg_cap)        , the long swaptions first coverages (cvg_first)
    , the discount factors at the start dates (df_start) and the discount
    factors at the caplets end dates (df_cap) */
  for (j = 1; j <= nEx; j++) {
    ...
    caplet_end_dates[j] = add_unit(start_dates[j], 12 / (int)float_freq,
                                   SRT_MONTH, float_bus_day_conv);

    cvg_cap[j] = coverage(start_dates[j], caplet_end_dates[j], float_basis);

    cvg_first[j] = coverage(start_dates[j], swp_pay_dates[first_cpn_index[j]],
                            fixed_basis);
  ...
  for (i = 1; i <= nEx; i++) {
    ...
    swp_level = 0.;
    num_cpn = nPay - first_cpn_index[i] -
              1; /* num_cpn is the number of coupon after the first one */
    ...
    for (k = 0; k <= num_cpn; k++) /* k = 0 corresponds to the first coupon */
    {
      if ((k == 0) && (num_cpn > 0))
        ...
        swp_level += cvg_pay[first_cpn_index[i] + k - 1] *
                     df_pay[first_cpn_index[i] + k];
      else if (k == num_cpn) {
        if (num_cpn > 0)
          swp_level += cvg_pay[first_cpn_index[i] + k - 1] *
                       df_pay[first_cpn_index[i] + k];
    ...
    err = srt_f_get_vol(start_dates[i], swp_pay_dates[nPay - 1], swp_rate,
                        SRT_FALSE, &atm_vol);
    if (err)
      return err;
    ...
    err = srt_f_optsarbvol(swp_rate, swp_rate, yr_to_exp, atm_vol, 0.0, 0.0,
                           0.0, vol_type, SRT_NORMAL, &norm_vol);
    if (err)
      return err;

    long_swps_stdev[i] =
        (long_swps_strikes[i] - swp_rate) / (norm_vol * sqrt(yr_to_exp));
    ...
    /* VERIFY IF THE STRIKE IS AT LESS THAN MAX_NR_STDEV_FROM_PREV_STRIKE FROM
    THE PREVIOUS ONE
    - FOR STEP UP AND ZERO COUPON MIDAT */
    if (i > 1) {
      yr_from_prev_ex_date = YEARS_IN_DAY * (ex_dates[i] - ex_dates[i - 1]);
      nbr_of_stdev_from_prev_strike =
          (long_swps_strikes[i] - long_swps_strikes[i - 1]) /
      ...
      if (fabs(nbr_of_stdev_from_prev_strike) > MAX_NR_STDEV_FROM_PREV_STRIKE) {
        ...
        long_swps_strikes[i] = sgn * MAX_NR_STDEV_FROM_PREV_STRIKE * norm_vol *
                                   sqrt(yr_from_prev_ex_date) +
    ...
    err = srt_f_get_vol(start_dates[i], swp_pay_dates[nPay - 1],
                        long_swps_strikes[i], SRT_FALSE, &cev_vol);
    if (err)
      return err;
    ...
    if (beta == 1.0) /* THEN THE DIFFUSION IS LOGNORMAL */
    {
      vol_type = SRT_LOGNORMAL;
      /* COMPUTE THE PREMIUM */
      long_swps_pv[i] = srt_f_optblksch(swp_rate, long_swps_strikes[i], cev_vol,
                                        yr_to_exp, 1.0, SRT_PUT, SRT_PREMIUM);
      ...
      /* COMPUTE THE VEGA */
      long_swps_vega[i] = 0.;
      long_swps_vega[i] = srt_f_optblksch(swp_rate, long_swps_strikes[i],
                                          cev_vol + CEV_VOL_SHIFT, yr_to_exp,
      ...
      long_swps_pv[i] = srt_f_optblknrm(swp_rate, long_swps_strikes[i], cev_vol,
                                        yr_to_exp, 1.0, SRT_PUT, SRT_PREMIUM);
      long_swps_pv[i] *= swp_level;
      ...
      /* COMPUTE THE VEGA */
      long_swps_vega[i] = 0.;
      long_swps_vega[i] = srt_f_optblknrm(swp_rate, long_swps_strikes[i],
                                          cev_vol + CEV_VOL_SHIFT, yr_to_exp,
                                          1.0, SRT_PUT, SRT_PREMIUM);
      long_swps_vega[i] *= swp_level;
    ...
    err = srt_f_get_vol(start_dates[i], caplet_end_dates[i], fra_rate, SRT_TRUE,
                        &atm_vol);
    if (err)
      return err;

    err = srt_f_optsarbvol(fra_rate, fra_rate, yr_to_exp, atm_vol, 0.0, 0.0,
                           0.0, vol_type, SRT_NORMAL, &norm_vol);
    if (err)
      return err;
    ...
    if (strike_method == EMK1)
      ...
      short_swps_strikes[i] =
          fra_rate + long_swps_stdev[i] * norm_vol * sqrt(yr_to_exp);
    ...
    if (i > 1) {
      yr_from_prev_ex_date = YEARS_IN_DAY * (ex_dates[i] - ex_dates[i - 1]);
      nbr_of_stdev_from_prev_strike =
          (short_swps_strikes[i] - short_swps_strikes[i - 1]) /
          (norm_vol * sqrt(yr_from_prev_ex_date));
      ...
      if (fabs(nbr_of_stdev_from_prev_strike) > MAX_NR_STDEV_FROM_PREV_STRIKE) {
        ...
        short_swps_strikes[i] = sgn * MAX_NR_STDEV_FROM_PREV_STRIKE * norm_vol *
                                    sqrt(yr_from_prev_ex_date) +
                                short_swps_strikes[i - 1];
      }
    }
    /* GET THE VOLATILITY AT THIS STRIKE AND THE CAPLET PRICE */
    err = srt_f_get_vol(start_dates[i], caplet_end_dates[i],
                        short_swps_strikes[i], SRT_TRUE, &cev_vol);
    if (err)
      return err;
    ...
    if (beta == 1.0) {
      short_swps_pv[i] = 0.;
      short_swps_pv[i] =
          cvg_cap[i] * srt_f_optblksch(fra_rate, short_swps_strikes[i], cev_vol,
                                       yr_to_exp, df_cap[i], SRT_PUT,
                                       SRT_PREMIUM);

      short_swps_vega[i] = 0.;
      short_swps_vega[i] =
          cvg_cap[i] * srt_f_optblksch(fra_rate, short_swps_strikes[i],
                                       cev_vol + CEV_VOL_SHIFT, yr_to_exp,
                                       df_cap[i], SRT_PUT, SRT_PREMIUM);
      ...
      short_swps_pv[i] =
          cvg_cap[i] * df_cap[i] *
          srt_f_optblknrm(fra_rate, short_swps_strikes[i], cev_vol, yr_to_exp,
                          1.0, SRT_PUT, SRT_PREMIUM);

      short_swps_vega[i] = 0.;
      short_swps_vega[i] = cvg_cap[i] * df_cap[i] *
                           srt_f_optblknrm(fra_rate, short_swps_strikes[i],
                                           cev_vol + CEV_VOL_SHIFT, yr_to_exp,
                                           df_cap[i], SRT_PUT, SRT_PREMIUM);
      short_swps_vega[i] -= short_swps_pv[i];
      short_swps_vega[i] /= CEV_VOL_SHIFT;
  ...
  err = make_swaptions_payts(nEx, nPay, first_cpn_index, cvg_first, cvg_pay,
                             df_pay, long_swps_strikes, long_swps_payts);
  if (err)
    return err;
  ...
  if (calib_method == FullCapAndDiag) {
    ...
    cap_real_end_date =
        add_unit(start_dates[last_cap_ex_index], 12 / (int)float_freq,
    ...
    err = swp_f_setSwapDP(start_dates[1], cap_theo_end_date, float_freq,
                          float_basis, float_leg_DP);
    if (err)
      return err;

    err = swp_f_make_FloatLegDatesAndCoverages(
        float_leg_DP, date_today, &float_pay_dates, &num_float_pay_dates,
        &float_fixing_dates, &float_start_dates, &float_end_dates,
        &float_coverages, &num_float_dates);
    if (err)
      return err;

    if (float_start_dates[0] == float_end_dates[0]) {
      num_cap_fp = num_float_dates;
      while (add_unit(start_dates[1], 12 * num_cap_fp / (int)float_freq,
                      SRT_MONTH,
      ...
      cap_theo_end_date =
          add_unit(start_dates[1], 12 * num_cap_fp / (int)float_freq, SRT_MONTH,
    ...
    if (lgm_ref_swp_data != NULL) {
      ...
      lgm_ref_swp_data->refCapSwptnArr = (SrtLgmRefSwptn *)srt_calloc(
          lgm_ref_swp_data->NrefCapSwptn, sizeof(SrtLgmRefSwptn));
    ...
    for (i = 1; i <= num_float_dates; i++) {
      ...
      err = srt_f_get_vol(float_start_dates[i - 1], float_end_dates[i - 1],
                          short_swps_strikes[j], SRT_TRUE, &cev_vol);
      if (err)
        return err;

      yr_to_exp =
          (double)(float_fixing_dates[i - 1] - date_today) * YEARS_IN_DAY;
      ...
      caplet_pv = LGMCEVCapletPrice(
          date_today, float_fixing_dates[i - 1], SRT_RECEIVER,
          short_swps_strikes[j], float_coverages[i - 1],
          float_start_dates_discount_factors[i - 1],
          float_end_dates_discount_factors[i - 1], cev_vol, beta);

      cap_pv += caplet_pv;

      caplet_vega = 0.;
      caplet_vega =
          LGMCEVCapletPrice(date_today, float_fixing_dates[i - 1], SRT_RECEIVER,
                            short_swps_strikes[j], float_coverages[i - 1],
                            float_start_dates_discount_factors[i - 1],
                            float_end_dates_discount_factors[i - 1],
                            cev_vol + CEV_VOL_SHIFT, beta);
      ...
      if (lgm_ref_swp_data != NULL) {
        lgm_ref_swp_data->refCapSwptnArr[i - 1].bgnDt =
    ...
    for (j = 1; j < nEx; j++) {
      ...
      if (most_exp_index <= nEx) {
        i = BasicDichotomie(
            swp_pay_dates, 0, (nPay - 1),
            start_dates[most_exp_index]); /* start_dates[most_exp_index] IS THE
                                             REAL END DATE OF THE SHORT
                                             SWAPTIONS */
        num_short_swp_cpn[j] =
            i - first_cpn_index[j]; /* num_short_swp_cpn IS THE NUMBER OF CPN
                                       AFTER THE FIRST ONE */
      }

      swp_level = 0.0;
      for (k = 0; k <= num_short_swp_cpn[j];
           k++) /* k = 0 REPRESENTS THE FIRST COUPON */
      {
        if ((k == 0) && (num_short_swp_cpn[j] > 0))
          swp_level += cvg_first[j] * df_pay[first_cpn_index[j]];
        else if ((k > 0) && (k < num_short_swp_cpn[j]))
          swp_level += cvg_pay[first_cpn_index[j] + k - 1] *
                       df_pay[first_cpn_index[j] + k];
      ...
      /* GET THE FORWARD */
      short_swps_fwd[j] =
          (df_start[j] - df_pay[first_cpn_index[j] + num_short_swp_cpn[j]]) /
          swp_level;
      yr_to_exp = (double)(ex_dates[j] - date_today) * YEARS_IN_DAY;

      /* GET THE SHORT SWPTIONS STRIKES */
      err = srt_f_get_vol(start_dates[j], start_dates[most_exp_index],
                          short_swps_fwd[j], SRT_FALSE, &atm_vol);
      if (err)
        return err;

      err = srt_f_optsarbvol(short_swps_fwd[j], short_swps_fwd[j], yr_to_exp,
                             atm_vol, 0.0, 0.0, 0.0, vol_type, SRT_NORMAL,
                             &norm_vol);
      if (err)
        return err;
      ...
      err = srt_f_get_vol(start_dates[j], start_dates[most_exp_index],
                          short_swps_strikes[j], SRT_FALSE, &cev_vol);
      if (err)
        return err;
      ...
      if (vol_type == SRT_LOGNORMAL) {
        short_swps_pv[j] = 0.0;
        short_swps_pv[j] =
            srt_f_optblksch(short_swps_fwd[j], short_swps_strikes[j], cev_vol,
                            yr_to_exp, 1.0, SRT_PUT, SRT_PREMIUM);
        short_swps_pv[j] *= swp_level;

        short_swps_vega[j] = 0;
        short_swps_vega[j] = srt_f_optblksch(
            short_swps_fwd[j], short_swps_strikes[j], cev_vol + CEV_VOL_SHIFT,
            yr_to_exp, 1.0, SRT_PUT, SRT_PREMIUM);
        ...
        short_swps_pv[j] = 0.0;
        short_swps_pv[j] =
            srt_f_optblknrm(short_swps_fwd[j], short_swps_strikes[j], cev_vol,
                            yr_to_exp, 1.0, SRT_PUT, SRT_PREMIUM);
        short_swps_pv[j] *= swp_level;
      ...
      err = make_short_swaptions_payts(nEx, num_short_swp_cpn, first_cpn_index,
                                       cvg_first, cvg_pay, df_pay,
                                       short_swps_strikes, short_swps_payts);
      if (err)
        return err;

      if (lgm_ref_swp_data != NULL) {
        lgm_ref_swp_data->refShortSwptnArr[j - 1].bgnDt = start_dates[j];
        lgm_ref_swp_data->refShortSwptnArr[j - 1].endDt =
            start_dates[most_exp_index];
        lgm_ref_swp_data->refShortSwptnArr[j - 1].strike =
            short_swps_strikes[j];
  ...
  for (i = 1; i <= nEx; i++) {
    if ((SParms.cxxal_method == FullCapAndDiag) ||
        (SParms.cxxal_method == FixKappa)) {
      err = srt_f_optimpvol((short_swps_pv[i] / df_cap[i]),
                            df_start[i] / df_cap[i],
                            (1 + cvg_cap[i] * short_swps_strikes[i]),
                            (double)(ex_dates[i] - date_today) * YEARS_IN_DAY,
                            1.0, SRT_PUT, SRT_LOGNORMAL, &lgm_cap_vol[i]);

      if (err)
        return err;

    } else if (SParms.cxxal_method == HybridShortAndDiag) {
      err =
          srt_f_optimpvol((caplets_pv[i] / df_cap[i]), df_start[i] / df_cap[i],
  ...
  SParms.cxxap_theo_end_date = cap_theo_end_date;
  ...
  SParms.cxxaplet_end_dates = caplet_end_dates;
  SParms.first_cpn_index = first_cpn_index;
  SParms.df_start = df_start;
  SParms.df_cap = df_cap;
  SParms.df_pay = df_pay;
  SParms.cxxvg_pay = cvg_pay;
  SParms.cxxvg_cap = cvg_cap;
  SParms.cxxvg_first = cvg_first;
  SParms.cxxvg_first = cvg_first;
  ...
  SParms.cxxaplets_pv = caplets_pv;
  SParms.cxxaplets_vega = caplets_vega;
  SParms.cxxaplets_strikes = caplets_strikes;
...
static Err newtonfuncforcalbootonfullcap(double lambda, double *f) {
  Err err = NULL;
  double theo_hybrid_swpts_pv, theo_cap_pv,
      mkt_cap_pv = SParms.pv_full_cap, mkt_cap_vega = SParms.vega_full_cap,
      mkt_hybrid_swpts_pv = SParms.hybrid_swpts_pv,
      mkt_hybrid_swpts_vega = SParms.hybrid_swpts_vega;
  LGMCalMeth cal_method = SParms.cxxal_method;
...
Err lgm_2f_autocal_calibrate(
  ...
  double *H1 = (tsPtr->H1), *H2 = (tsPtr->H2), *Zeta1 = (tsPtr->Zeta1),
         *Zeta2 = (tsPtr->Zeta2), *Zeta12 = (tsPtr->Zeta12),
         alpha = cal_req->alpha, gamma = cal_req->gamma, rho = cal_req->rho,
         *lambdaPtr;
  ...
  double *kappa_iteration_values = NULL, two_last_iter_diff_tau,
         last_iter_diff_tau;
  ...
  err = update_autocal_midat_struct(MidAt, yc_name, srt_f_get_vol,
                                    srt_f_get_beta);
  if (err)
    return err;
  ...
  SParms.cxxal_method = cal_req->calmeth;
  ...
  err = srt_f_get_autocal_ref_instrs(
      nEx, nPay, start_dates, swp_pay_dates, yc_name, srt_f_get_vol,
      srt_f_get_beta, LongKs,
      (cal_req->Rmeth), /* TO DETERMINE THE SHORT SWAPTIONS INSTRUMENT  */
      (cal_req->calmeth), likely_most_expensive_index, lgm_ref_swp_data);
  ...
  s_yr_to_last_pay_date =
      (double)(swp_pay_dates[nPay - 1] - date_today) * YEARS_IN_DAY;
  ...
  if (nEx == 1) {
    ...
    case FixKappa:
      ...
      kappa = lambdaPtr[1];
      lEndTime = time(NULL);
      smessage("Calibration achieved in %.2f s",
    ...
    case HybridShortAndDiag:
      ...
      while ((nstop < 1.0) && (k < MAX_ITERATION) &&
        ...
        if ((k > 2) && (nstop < 1.0) && (k < MAX_ITERATION) &&
            (swith_to_fix_kappa_calibration == SRT_NO)) {
          last_iter_diff_tau = 1.0 / kappa_iteration_values[k] -
                               1.0 / kappa_iteration_values[k - 1];
          two_last_iter_diff_tau = 1.0 / kappa_iteration_values[k] -
                                   1.0 / kappa_iteration_values[k - 2];
      ...
      for (j = nEx; j > 0; j--)
        H1[j - 1] = (exp(-s_yr_to_start_ptr[j] * kappa) -
                     exp(-s_yr_to_last_pay_date * kappa)) /
                    kappa;
      for (j = 1; j <= nEx; j++)
        H2[j - 1] =
            H2_Func(start_dates[j], start_dates, date_today, nEx, H1, gamma);

      /* COMPUTE ZETA2        , ZETA12 AND H2 */
      for (j = 1; j <= nEx; j++) {
        Zeta2[j - 1] = New_Zeta2_Func(ex_dates[j], ex_dates, nEx, date_today,
                                      Zeta1, Zeta2, alpha, gamma, rho);
        Zeta12[j - 1] = New_Zeta12_Func(ex_dates[j], ex_dates, nEx, date_today,
                                        Zeta1, Zeta12, alpha, gamma, rho);
      }

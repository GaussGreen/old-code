...
Err mcBeta_main_3dfx(
  ...
  for (i = 0; i < npaths; i++) {
    ...
    for (j = 1; stop_path == 0 && j < nsteps; j++) {
      ...
      if (Z > -100) {
        Z2 = fx_std[j] * exp((beta[j] - 1.0) * Z);
        Z += (X + dom_ifr[j - 1] - Y - for_ifr[j - 1]) * dt - 0.5 * Z2 * Z2 +

...
double gammp(double a, double x)
{
        ...
        if (x < 0.0 || a <= 0.0) serror("Invalid arguments in routine gammp");
        if (x < (a+1.0)) {
                gser(&gamser,a,x,&gln);
                return gamser;
        } else {
...
void gcf(double *gammcf, double a, double x, double *gln)
{
        ...
        *gln=gammln(a);
        ...
        if (i > ITMAX) serror("a too large, ITMAX too small in gcf");
...
void gser(double *gamser, double a, double x, double *gln)
{
        ...
        *gln=gammln(a);
        if (x <= 0.0) {
                if (x < 0.0) serror("x less than 0 in routine gser");
                *gamser=0.0;
                return;
        } else {
                ap=a;
...
double gammq(double a, double x)
{
        ...
        if (x < 0.0 || a <= 0.0) serror("Invalid arguments in routine gammq");
        if (x < (a+1.0)) {
                gser(&gamser,a,x,&gln);
                return 1.0-gamser;
        } else {
...
for (m=1;m<=ITMAX;m++)
{
	...
	de1 = d*c;
	h *= de1;
	if (fabs(de1-1.0) < GAMMA_EPS) break;
}
if (m > ITMAX) smessage("a or b too big, or ITMAX too small in betacf");
return h;
}
...
it is computed up to x*/
{
		...
		if (x< 0.0 || x>1.0) smessage ("Bad x in routine betaincmp");

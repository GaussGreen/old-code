...
{
    return ((t2 * t2 * t2 * t2 * exp(lam * t2) - t1 * t1 * t1 * t1 * exp(lam * t1)) -
    ...
    double** ablinsig)
{
    ...
    for (i = 1; i < nb_sig; ++i)
    {
        ablinsigloc[0][i] = sigtilda[i - 1] - (sigtilda[i] - sigtilda[i - 1]) * sig_time[i - 1] /
        ...
        if ((sig_time[i + 1] - sig_time[i]) > 2.0 / 365.0)
        {
            ...
            C = sigtilda[i] * sigtilda[i] *
                    (Ji0_func(2 * lam, sig_time[i], sig_time[i + 1]) * sig_time[i + 1] *
                     ...
                     Ji2_func(2 * lam, sig_time[i], sig_time[i + 1])) -
                sig[i + 1] * sig[i + 1] * Ji0_func(2 * lam, sig_time[i], sig_time[i + 1]) *
                    (sig_time[i + 1] - sig_time[i]) * (sig_time[i + 1] - sig_time[i]);
    ...
    double*  contcorrel)
{
    ...
    for (i = 0; i < nb_sig - 1; ++i)
    {
        ...
        if (DeltaT > 30.0 / 365.0)
        {
            ...
            {
                smessage(
                    "Cannot convert PW Const correl in continuous PW affine correl, Abs(correl)>1");
                //				err = "Cannot convert PW Const correl in continuous PW
                //affine correl, Abs(correl)>1"; 				goto FREE_RETURN;
            }
    ...
    for (i = 1; i < nb_sig; ++i)
    {
        ablincorrelloc[0][i] = correltilda[i - 1] - (correltilda[i] - correltilda[i - 1]) *
                                                        sig_time[i - 1] /
                                                        (sig_time[i] - sig_time[i - 1]);
        ablincorrelloc[1][i] =
    ...
    double*  contcorrel)
{
    ...
    for (i = 0; i < nb_sig - 1; ++i)
    {
        ...
        if (DeltaT > 30.0 / 365.0)
        {
            ...
            B = forsiga * J1 * sig_time[i + 1] - forsiga * J2 + forsigb * J0 * sig_time[i + 1] -
                forsigb * J1;
    ...
    double** abclinsig)
{
    ...
    {
        abclinsigloc[0][i] =
            sig[i - 1] + alpha[i] * sig_time[i] * sig_time[i - 1] -
    ...
    for (i = 0; i < nstept; ++i)
    {
        ...
        {
            contsig[i] =
                abclinsigloc[0][j] + abclinsigloc[1][j] * t2 + abclinsigloc[2][j] * t2 * t2;
        }
    }
...
void fillVol(int nstept, double* time, int nb_sig, double* sig_time, double* sig, double* sigall)
{
    int    i, j;
    double t1, t2;
    ...
    double* domforphi)
{
    ...
    for (i = 1; i < nstept; i++)
    {
        ...
        while (tb < t2 && j < nb_sig_minus1)
        {
            ...
            std1 += doma * doma * (tb - ta) + domb * domb * (tb * tb * tb - ta * ta * ta) / 3.0 +
        ...
        {
            doma = domsig[j - 1] -
                   (domsig[j] - domsig[j - 1]) * sig_time[j - 1] / (sig_time[j] - sig_time[j - 1]);
            domb = (domsig[j] - domsig[j - 1]) / (sig_time[j] - sig_time[j - 1]);

            fora = forsig[j - 1] -
                   (forsig[j] - forsig[j - 1]) * sig_time[j - 1] / (sig_time[j] - sig_time[j - 1]);
            forb = (forsig[j] - forsig[j - 1]) / (sig_time[j] - sig_time[j - 1]);
        }
        ...
        std1 += doma * doma * (t2 - ta) + domb * domb * (t2 * t2 * t2 - ta * ta * ta) / 3.0 +
    ...
    double* domforphi)
{
    ...
    for (i = 1; i < nstept; i++)
    {
        ...
        while (tb < t2 && j < nb_sig_minus1)
        {
            ...
            domforI2 +=
                domforrhoa * doma * fora * Ji0_func(domlam + forlam, ta, tb) +
                (domforrhoa * doma * forb + domforrhoa * fora * domb + domforrhob * doma * fora) *
                    Ji1_func(domlam + forlam, ta, tb) +
                (domforrhoa * domb * forb + domforrhob * doma * forb + domforrhob * domb * fora) *
        ...
        if (j > 0)
        {
            ...
            quantorhoa = quantorho[j - 1] - (quantorho[j] - quantorho[j - 1]) * sig_time[j - 1] /
                                                (sig_time[j] - sig_time[j - 1]);
            quantorhob = (quantorho[j] - quantorho[j - 1]) / (sig_time[j] - sig_time[j - 1]);

            domforrhoa = domforrho[j - 1] - (domforrho[j] - domforrho[j - 1]) * sig_time[j - 1] /
                                                (sig_time[j] - sig_time[j - 1]);
            domforrhob = (domforrho[j] - domforrho[j - 1]) / (sig_time[j] - sig_time[j - 1]);
        }
        ...
        domforI2 +=
            domforrhoa * doma * fora * Ji0_func(domlam + forlam, ta, t2) +
            (domforrhoa * doma * forb + domforrhoa * fora * domb + domforrhob * doma * fora) *
    ...
    for (i = 1; i < nstept; i++)
    {
        ...
        while (tb < t2 && j < nb_sig_minus1)
        {
            if (j > 0)
            {
                ...
                domc =
                    domsig[j - 1] + domalpha[j] * sig_time[j] * sig_time[j - 1] -
                    sig_time[j - 1] * (domsig[j] - domsig[j - 1]) / (sig_time[j] - sig_time[j - 1]);

                fora = foralpha[j];
                forb = (forsig[j] - forsig[j - 1]) / (sig_time[j] - sig_time[j - 1]) -
                       foralpha[j] * (sig_time[j] + sig_time[j - 1]);
                forc =
                    forsig[j - 1] + foralpha[j] * sig_time[j] * sig_time[j - 1] -
                    sig_time[j - 1] * (forsig[j] - forsig[j - 1]) / (sig_time[j] - sig_time[j - 1]);
            }
            ...
            std1 += doma * doma * (tb * tb * tb * tb * tb - ta * ta * ta * ta * ta) / 5.0 +
                    ...
                    2 * domb * domc * (tb * tb - ta * ta) / 2.0 + domc * domc * (tb - ta);
        ...
        domI1 +=
            doma * doma * Ji4_func(domlam, ta, t2) + 2 * doma * domb * Ji3_func(domlam, ta, t2) +
            (2 * doma * domc + domb * domb) * Ji2_func(domlam, ta, t2) +
            2 * domb * domc * Ji1_func(domlam, ta, t2) + domc * domc * Ji0_func(domlam, ta, t2);
        ...
        forI1 +=
            fora * fora * Ji4_func(forlam, ta, t2) + 2 * fora * forb * Ji3_func(forlam, ta, t2) +
            (2 * fora * forc + forb * forb) * Ji2_func(forlam, ta, t2) +
            2 * forb * forc * Ji1_func(forlam, ta, t2) + forc * forc * Ji0_func(forlam, ta, t2);
    ...
    double* res)
{
    ...
    double *r1_bar = NULL, *r3_bar = NULL, *r1_dim1 = NULL, **r1_dim2 = NULL, **r2 = NULL,
           ***values = NULL, ***values_p1 = NULL, ***values_temp = NULL, **mu_r1 = NULL,
           **mu_r3 = NULL, **var_r1 = NULL, **var_r3 = NULL, **r = NULL, **r_init = NULL,
           *fwd1 = NULL, *fwd3 = NULL, *var1 = NULL, *var3 = NULL, *domphi = NULL, *forphi = NULL,
           *domforphi = NULL;
    ...
    if (!pde || !var1 || !var3 || !domphi || !forphi || !domforphi || !fwd1 || !fwd3 ||
        !domsigtilda || !forsigtilda || !allfxsig || !contdomsig || !contforsig || !domabsig ||
        !forabsig)
    {
    ...
    err = convertVol2(
        domlam, nstept, time, nb_sig, sig_time, domsig, domsigtilda, contdomsig, domabsig);
    if (err)
    {
        goto FREE_RETURN;
    }
    err = convertVol2(
        forlam, nstept, time, nb_sig, sig_time, forsig, forsigtilda, contforsig, forabsig);
    if (err)
    {
        goto FREE_RETURN;
    }
    ...
    std3 = sqrt(
        ((contdomsig[nstept - 1] / contforsig[nstept - 1]) *
             (contdomsig[nstept - 1] / contforsig[nstept - 1]) * forphi[nstept - 1] +
    ...
    double* res)
{
    ...
    if (!pde || !var1 || !var3 || !domphi || !forphi || !domforphi || !fwd1 || !fwd3 || !domalpha ||
        !foralpha || !allfxsig || !contdomsig || !contforsig || !domabcsig || !forabcsig)
    {
    ...
    err = convertVolQuadratic(
        domlam, nstept, time, nb_sig, sig_time, domsig, domalpha, contdomsig, domabcsig);
    if (err)
    {
        goto FREE_RETURN;
    }
    err = convertVolQuadratic(
        forlam, nstept, time, nb_sig, sig_time, forsig, foralpha, contforsig, forabcsig);
    if (err)
    {
        goto FREE_RETURN;
    }
    ...
    double* res)
{
    ...
    if (!pde || !var1 || !var3 || !domphi || !forphi || !domforphi || !fwd1 || !fwd3 ||
        !domsigtilda || !forsigtilda || !domforcorreltilda || !quantocorreltilda || !allfxsig ||
        !contdomsig || !contforsig || !contdomforcorrel || !contquantocorrel || !domabsig ||
        !forabsig || !ablindomforcorrel || !ablinquantocorrel)
    {
        err = "Memory allocation error (1) in doublelgm1fQuanto_adi";
        goto FREE_RETURN;
    }
    ...
    std3 = sqrt(
        ((contdomsig[nstept - 1] / contforsig[nstept - 1]) *
         ...
         2 * contdomforcorrel[nstept - 1] * (contdomsig[nstept - 1] / contforsig[nstept - 1]) *
             domforphi[nstept - 1]) /
    ...
    for (i = 0; i < nstepx; i++)
    {
        ...
        for (j = 0; j < nstepz; j++)
        {
            ...
            r2[i][j]      = fwd3[nstept - 1] + r2_temp +
                       (contforsig[nstept - 1] / contdomsig[nstept - 1]) *
                           sqrt(1 - contdomforcorrel[nstept - 1] * contdomforcorrel[nstept - 1]) *
                           r3_bar[j];
            r_init[i][j] = r1_bar[i];
        }
    }
    ...
    for (step = nstept - 2; step >= 0; step--)
    {
        ...
        for (i = lx; i <= ux; i++)
        {
            ...
            for (j = lz; j <= uz; j++)
            {
                ...
                mu_r3[i][j] =
                    (mu_r3i - forlam * r3_bar[j] +
                     (r3_bar[j] + contdomforcorrel[step] * r1_bar[i] /
                                      sqrt(1 - contdomforcorrel[step] * contdomforcorrel[step])) *
                         (domabsig[1][step] / contdomsig[step] -
                          forabsig[1][step] / contforsig[step]) +
        ...
        if (eval_evt[step])
        {
            ...
            for (i = lx; i <= ux; i++)
            {
                r1_dim1[i] = r1_bar[i] + fwd1[step];
                r2_temp =
                    contdomforcorrel[step] * r1_bar[i] * (contforsig[step] / contdomsig[step]);
    ...
    double* res)
{
    ...
    double *r1_bar = NULL, *r3_bar = NULL, *r1_dim1 = NULL, **r1_dim2 = NULL, **r2 = NULL,
           ***values = NULL, ***values_p1 = NULL, ***values_temp = NULL, **mu_r1 = NULL,
           **mu_r3 = NULL, **var_r1 = NULL, **var_r3 = NULL, **zerovar1 = NULL, **zerovar3 = NULL,
           **r = NULL, **zeror = NULL, **r_init = NULL, *fwd1 = NULL, *fwd3 = NULL, *var1 = NULL,
           *var3 = NULL, *domphi = NULL, *forphi = NULL, *domforphi = NULL;
    ...
    if (!pde || !var1 || !var3 || !domphi || !forphi || !domforphi || !fwd1 || !fwd3 || !allfxsig ||
        !contdomsig || !contforsig || !contdomforcorrel || !contquantocorrel)
    {
        err = "Memory allocation error (1) in doublelgm1fQuanto_adi";
        goto FREE_RETURN;
    }
    ...
    if (!r1_dim2 || !r2 || !values || !values_p1 || !mu_r1 || !mu_r3 || !zerovar1 || !zerovar3 ||
        !var_r1 || !var_r3 || !zeror || !r || !r_init || !r1_bar || !r1_dim1 || !r3_bar)
    {
        err = "Memory allocation error (2) in doublelgm1fQuanto_adi";
        goto FREE_RETURN;
    }
        ...
        for (i = lx; i <= ux; i++)
        {
            ...
            for (j = lz; j <= uz; j++)
            {
                ...
                Jump_r3[i][j] =
                    ((contdomsig[step + 1] /
                      (contforsig[step + 1] *
                       sqrt(1 - contdomforcorrel[step + 1] * contdomforcorrel[step + 1]))) -
                     (contdomsig[step] /
                      (contforsig[step] *
                       sqrt(1 - contdomforcorrel[step] * contdomforcorrel[step])))) *
                        ((0.5 * (contforsig[step + 1] / contdomsig[step + 1]) * r3_bar[j] *
                              sqrt(1 - contdomforcorrel[step + 1] * contdomforcorrel[step + 1]) +
                          0.5 * (contforsig[step + 1] / contdomsig[step + 1]) *
                              contdomforcorrel[step + 1] * r1_bar[i]) +
                         (0.5 * (contforsig[step] / contdomsig[step]) * r3_bar[j] *
                              sqrt(1 - contdomforcorrel[step] * contdomforcorrel[step]) +
                          0.5 * (contforsig[step] / contdomsig[step]) * contdomforcorrel[step] *
                              r1_bar[i])) -

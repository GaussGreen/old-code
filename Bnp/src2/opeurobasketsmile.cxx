 ...
 *				: paths			- number of MC paths used
 *for integration : greek         - greek wanted (only premium) : Points
 *- Num of points for the approx of density of each underlying
 *
 * RETURNS       : ??            - ??
 *
 *******************************************************************************/
...
difference with the preceding one is that        , for each Forward (in total we
have nFdws) it draws a random variable which is compatible with the observed
smile        , that is a variable distributed accrding to the implied marginal
cumulative distributions. In addition        , the copula function define a
joint density for the n forwards. The parameter to input is StudDegree that
  ...
  for (i = 3; i < paths; i++) {
    /*
                    CorrMatr = GetStudentCplDev_Rand (
                                                                    ...
                                                                    mean_v ,
                                                                    Correlation
             , paths+1        , StudDegree+nFwds        , ppdPoints        ,
       ppdCumProbaValue        , 2*nNumPoints+1 , n_conv        , &idum
                                                      );
    */
  }

...
Err srt_InflationSpreadOption( double dRate, double dInflation, struct sInflationArg* argPtr, double* pdOption );
...
/* Calculate the level */
	*pdOption = 0.0;
	for ( i=0; i<argPtr->iNumDates; i++ )
		*pdOption += argPtr->dvForwardDF[i] * exp ( argPtr->dvPsiDiff[i]*dRate/argPtr->dF - 0.5*argPtr->dvPsiDiff2[i]*argPtr->dG );
...
Err setInflationStructure( char *csUndName, Date dtExpiry, Date* dtvMaturity, int iNumDates, struct sInflationArg* sArg )
{
/* Variable declarations */
...
Err srt_InflationSpread( char* csUndName, Date dtExpiry, Date* dtvMaturity, int iNumDates, double* dpIntArg, double* dpPrice )
{
// Variable declarations	
	double dEps, dAcc;
...
/* Set the upper and lower limits, the number of points and the desired accuracy (or use default values) */
	iDim = 2;
...
/* Integrate over a 2D Gaussian */
	nag_multid_quad_adapt_1( iDim, &inflationIntegrand, dvLowerLimits, dvUpperLimits, &iMinPts, iMaxPts, dEps, dpPrice, &dAcc, 
								&nagArg, &neFail );
...
Err srt_f_get_vasicek_var_sr(double			time,
							 TermStruct		*ts,
							 double			*var_sr)
{
	Err err = NULL;
...
Err srt_get_vasicek_risk_neutral_sr_mean(double time,
										TermStruct	*ts,
...
Err srt_get_vasicek_forward_measure_und_mean( double dTime, double dFwdTime, double dCorrelation, 
											 TermStruct *ptsVasicek, TermStruct *ptsLGM, double *pdMean )
{
/* Variable declarations */
...
/* The mean is equal to the mean in the risk neutral measure, plus a correction.  Begin with the risk-neutral mean */
...
/* Move the loops until they are pointing to the element beyond the time */
	while ( (plsVasicek!=NULL) && ( ((IrmTermStructVal*)plsVasicek->element->val.pval)->time < dTime) ) 
		plsVasicek = plsVasicek->next;
	if ( plsVasicek == NULL ) plsVasicek = ptsVasicek->tail;

	while ( (plsLGM!=NULL) && ( ((IrmTermStructVal*)plsLGM->element->val.pval)->time < dTime) ) 
		plsLGM = plsLGM->next;
	if ( plsLGM == NULL ) plsLGM = ptsLGM->tail;
...
/* Calculate the next bit of the integral */	
		...
		dIntegral += pvalLGM->sig / pvalLGM->F * pvalVasicek->sig * pvalVasicek->tau / pvalVasicek->F 
						* (  ( dPsiFwdMeasure - pvalLGM->Psi - pvalLGM->F / lambda ) / (a+lambda) * ( exp((a+lambda)*dTimeDiff) - 1.0 )
...
/* ---------------------------------------------------------------------------------------------------------------------------------- */
/* Calculates the inflation forward */
Err srt_InflationSpreadForward( char* csVasicekUnd, char* csLGMUnd, Date dtExpiry, double dCorrelation, double* pdForward )
{
/* Variable declarations */
...
/* Calculate the mean of the inflation spread */
	return srt_get_vasicek_forward_measure_und_mean( dExpiry, dExpiry, dCorrelation, ptsVasicek, ptsLGM, pdForward );
...
Err srt_CalibrateInflationSpread( char* csYCname, double dSpotSpread, double dRevSpeed, double dCorrelation, double dSpreadVol, 
								  double dTau, Date dtToday, int iNumDates, double* dvFwdSpread, 
								  double* dvLGMvol, Date* dtvDates, double* dvCalibratedLevels )
{
/* Variable declarations */
	int i, j;
...
/* Loop over the dates, calcualating the reversion level up to that time */	
	...
	for ( i=0; i<iNumDates; i++ )
	{
		dSum1 = dvLGMvol[0] * (  ( exp(dRevSpeed*dvTimes[0]) - 1.0 ) / dRevSpeed 
								- exp(dLambda*dvTimes[i]) * ( exp((dRevSpeed+dLambda)*dvTimes[0]) - 1.0 ) / ( dRevSpeed + dLambda )  ); 
		dSum2 = 0.0;
		for ( j=0; j<i; j++ )
		{
			dSum1 += dvLGMvol[j+1] * (  ( exp(dRevSpeed*dvTimes[j+1]) - exp(dRevSpeed*dvTimes[j]) ) / dRevSpeed 
						- exp(dLambda*dvTimes[i]) * ( exp((dRevSpeed+dLambda)*dvTimes[j+1]) - exp((dRevSpeed+dLambda)*dvTimes[j]) ) 
																	/ ( dRevSpeed + dLambda )  ); 
			dSum2 += dvCalibratedLevels[j] * (  exp(dRevSpeed*dvTimes[j+1]) - exp(dRevSpeed*dvTimes[j])  );
		}
		dSum1 *= dCorrelation * dSpreadVol * dTau * exp( -dRevSpeed*dvTimes[i] );
		dSum2 *= exp( -dRevSpeed*dvTimes[i] );

		dvCalibratedLevels[i] = ( dvFwdSpread[i]/swp_f_df( dtToday, dtvDates[i], csYCname ) - dSpotSpread*exp( -dRevSpeed*dvTimes[i] ) - dSum1 - dSum2 ) 

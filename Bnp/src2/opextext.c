...
static double com_bar_fn3 ( double 	xd, 
							double  xu, 	 	
							double  l_b,
							double  mat,
							double  mu ,
...
static double com_bar_fn2 (double x1,       
						   double x2,      
						   double b1,      
						   double b2,      
						   double t1,
...
double 	rtsig2 , rtsig1 ;
	...
	{
		n1 = srt_f_intnrm_i1( 	x1-x2 + mu2*(t2-t1) , 
								rtsig2 ,
								mu1*t1 - x1 , 
								rtsig1 )	;

		n2 =  srt_f_intnrm_i1( 	x1-x2 + mu2*(t2-t1) , 
								rtsig2 ,
								mu1*t1 - x1 + 2.0 * b1, 
								rtsig1 )  	;
...
/******************************************************************************/
static double com_bar_fn1 (double xd1,      
						   double xu1,      
						   double xd2,      
						   double xu2,      
						   double b1,      
...
double 	n1, n2, n3, n4 ;
	...
	n2 = com_bar_fn2(	xu1, xd2,      
						b1, b2,      
						t1, t2, 
						mu1, mu2,
						sig1, sig2 ) ;

	n3 = com_bar_fn2(	xd1, xu2,      
						b1, b2,      
						t1, t2, 
						mu1, mu2,
						sig1, sig2 ) ;
...
static double z_brak (double x1,
					  double x2,
					  double xacc,     
					  int    niter,     
					  double fwd,     
...
double rts ;
	...
	for (i=0; i <= niter; i++)
	{
		...
		for (j=0;j < jmax;j++)
		{
			...
			f = srt_f_optexting(fwd1,rts ,strike2,barrier,sig,mat,disc,
								ot2, sc_type2,PREMIUM)
				- strike1 ;    
...
/*********************************************************************************/
static double 	srt_f_opt_extcombar_pri (double fwd1,
										 double fwd2,
										 double spot, 
										 double strike1,
										 double	strike2,
...
Err err;
	...
	{                       
		...
		{              
			x2 = DMAX (1.1* strike2 + 5*sig2*sqrt(mat2-mat1) + barrier2 , 
						spot );
		} 
		else 
		if (call_put2 == SRT_PUT )  /*** compound put  ***/  
		{              
	...
	{                       
		x1 = DMAX( 0.01*spot, DMIN( 0.5 * spot, spot - 5*sig2*sqrt(mat2-mat1) )  );
	...
	if ( (scud_type2 == SRT_UP) && (call_put2 == SRT_CALL) ) /***  underlying barrier up  ***/     
	{ 
		xacc1 = 10.0 *  xacc ;
	...
	{                       
		...
		{                       
			...
			{              
				...
				{
					xd1 = DMAX( l_xstar_u,l_b1 );  xu1 = 100 ;  
					xd2 = l_k2 ;
					xu2 = 100 ;    
				}
			} 
	...
	n1 = com_bar_fn1 (	xd1, xu1, 	 	
						xd2, xu2, 
						bar1, bar2,    
						mat1, mat2,
						mu1 + du1*sig1*sig1 ,	mu2 + du2*sig2*sig2 ,  
						sig1, sig2);     

	premium =  cp2*fwd2*disc2* n1 ; 

	n2 = com_bar_fn1 (	xd1, xu1, 	 	
						xd2, xu2, 
						bar1, bar2,    
						mat1, mat2,
						mu1, mu2 ,
...
/********************************************************************************/
double srt_f_opt_extcombar (double  fwd1, 
							double 	fwd2,
							double 	spot, 
							double 	strike1,
							double	strike2,   
...
double	delta, gamma, vega, theta ; 
	...
	if (   (greek == PREMIUM) 
		...
		|| (greek == GAMMA_FWD2))
	{
		fval = srt_f_opt_extcombar_pri( fwd1,
										fwd2,
										spot, 
										strike1,
										strike2,
	...
	{   		/*  delta fwd2  */ 
		...
		delta = ( srt_f_opt_extcombar_pri(	fwd1,
											x1, 
											spot, 
											strike1,
											strike2, 
	...
	{   		/*  gamma fwd1  */                    
		...
		gamma = ( srt_f_opt_extcombar_pri(	x1,
											fwd2, 
											spot, 
											strike1,
											strike2, 
	...
	if (greek == GAMMA_FWD2)   		/*  gamma fwd2   */
	...
	{                    
		...
		gamma = ( srt_f_opt_extcombar_pri(	fwd1,
											x1, 
											spot, 
											strike1,
											strike2, 
	...
	{                    
		...
		vega = ( srt_f_opt_extcombar_pri(	fwd1,
											fwd2,			
											spot, 
											strike1,
											strike2, 
	...
	{   		/*  theta1 = theta mat1  */                    
		...
		theta = ( srt_f_opt_extcombar_pri(	fwd1,
											fwd2, 
											spot, 
											strike1,
											strike2, 

...
static double com_bar_fn3(
    double xd, double xu, double l_b, double mat, double mu, double sig, int cp)
{
    double result = 0.0;
    double n1, n2;
    double sigrt;
    ...
    n1 = norm((-cp * xd + mu * mat) / sigrt) -
         exp(2.0 * l_b * mu / (sig * sig)) * norm((-cp * xd + 2.0 * l_b + mu * mat) / sigrt);

    n2 = norm((-cp * xu + mu * mat) / sigrt) -
         exp(2.0 * l_b * mu / (sig * sig)) * norm((-cp * xu + 2.0 * l_b + mu * mat) / sigrt);
    ...
    double sig2)
{
    ...
    {
        n1 = srt_f_intnrm_i1(x1 - x2 + mu2 * (t2 - t1), rtsig2, mu1 * t1 - x1, rtsig1);

        n2 = srt_f_intnrm_i1(x1 - x2 + mu2 * (t2 - t1), rtsig2, mu1 * t1 - x1 + 2.0 * b1, rtsig1);
...
/* routine from Numerical Recipes, pg. 354 */
{
    ...
    for (i = 0; i <= niter; i++)
    {
        ...
        for (j = 0; j < jmax; j++)
        {
            ...
            f = srt_f_optexting(
                    fwd1, rts, strike2, barrier, sig, mat, disc, ot2, sc_type2, PREMIUM) -
                strike1;
...
/*********************************************************************************/
static double srt_f_opt_extcombar_pri(
    double         fwd1,
    double         fwd2,
    double         spot,
    double         strike1,
    ...
    SrtBarrierType scud_type2)
{
    ...
    n2 = com_bar_fn1(xd1, xu1, xd2, xu2, bar1, bar2, mat1, mat2, mu1, mu2, sig1, sig2);
    ...
    SrtGreekType   greek)
{
    ...
    if ((greek == PREMIUM) || (greek == GAMMA) || (greek == GAMMA_FWD1) || (greek == GAMMA_FWD2))
    {

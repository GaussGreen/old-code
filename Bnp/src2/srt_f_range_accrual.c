...
static Err SrtRA_RequestDfDates(
    SrtProduct* product, long date, int* pn_unds, int** pn_dfs, long*** pdates)
{
...
static Err SrtRA_Payoff(SrtProduct* product, long today, long date, double** dfs, double* payoff)
{
    Err                  err = NULL;
    SrtRangeAccrualSpec* RA  = (SrtRangeAccrualSpec*)(product->spec_desc);
    int                  i, j, k, nfwds;
    ...
    *payoff = 0.0;
    ...
    if (date != RA->last_date)
    {
        ...
        if (!RA->quanto)
        {
            ...
            RA->idx = (long*)realloc(RA->idx, (RA->ndfs_d + RA->ndfs_f) * sizeof(long));
    ...
    for (k = 0, stp = RA->obs_steps[i = RA->period_idx]; stp->next; stp = stp->next)
    {
        ...
        for (j = 0; j < 4; j++)
        {
            ...
            else
            {
                ...
                /*				bs[j] = srt_f_optblksch( fwd, stp->K[j], stp->vol_atK[j],
    ...
    double  rho_df)
{
    ...
    RA->obs_steps = (SrtRAObservStep**)calloc(n_periods + 1, sizeof(SrtRAObservStep*));
    ...
    for (i = 0, stp = RA->obs_steps[0]; stp->next; stp = stp->next)
    {
        ...
        err = swp_f_SABRvol(volc_f, stp->date, d, 0.0, &stp->betavol, &power, SABR_BETAVOL);
        if (err)
            goto RETURN_ERR;
        err = swp_f_SABRvol(volc_f, stp->date, d, 0.0, &stp->alpha, &power, SABR_ALPHA);
        if (err)
            goto RETURN_ERR;
        err = swp_f_SABRvol(volc_f, stp->date, d, 0.0, &stp->beta, &power, SABR_BETA);
        if (err)
            goto RETURN_ERR;

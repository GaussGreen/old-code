...
{
	answer = srt_f_optameopt(spot, forward, strike, disc_fact, vol, opt_mat, SRT_CALL, step_num, greek)+
			srt_f_optameopt(spot, forward, strike, disc_fact, vol, opt_mat, SRT_PUT, step_num, greek);
	return (answer);
}
...
{ 
	...
	for (i = 0; i < step_num + 3; i++)
 	{ 
		...
		asset /= uu * uu;
		if(bPayatTheExFlag) disc_factor[i] = 1.0;
	}   
	...
	for (n = step_num + 1; n >= 1; n--)
	{
		...
		for (i = 0; i <= n; i++)
 	 	{ 
			amer_disc_premium = ( p_u * amer_premium[i] + p_d * 
							amer_premium[i+1] ) * df_n; 
			euro_disc_premium = ( p_u * euro_premium[i] + p_d * 
							euro_premium[i+1] ) * df_n; 
			if(bPayatTheExFlag) factor_disc_premium = ( p_u * disc_factor[i] + p_d * disc_factor[i+1] ) * df_n;
			...
			if (amer_disc_premium > intrinsic)
			{
				...
				if(bPayatTheExFlag) disc_factor[i] = factor_disc_premium;
			}
			else
			{
				amer_premium[i] = intrinsic;
		...
		{   
			...
			amer_delta  = ( amer_premium[0] - amer_premium[2] ) / 
					 ( diff_up + diff_do );
			euro_delta  = ( euro_premium[0] - euro_premium[2] ) / 
					 ( diff_up + diff_do );
	...
	switch (greek)
	{
		...
		case VEGA:
			...
			amer_vega  = srt_f_optameopt(spot, 
									forward, 
									strike,
									disc_fact,
				      				vol + GVOPT.vol_add, 

/* =============================================================================
   ...
   FILENAME:    swp_f_swap_generic.cxx
   ...
   PURPOSE:     Provide a few function for generic swaps        , including:
                                ...
                                - CMS        , CMT        , TEC
                                - ...
                                THE FOLLOWING STRUCTURES AND FUNCTIONS SEEMS
   MORE ROBUST (OR AT LEAST EASIER TO WORK WITH) AND SHOULD BE USED IN THE
   FUTURE)
...
static Err create_GenSwapLeg(long today, SwapDP *leg_sdp, double fixed,
                             double initial_exchange, double final_exchange,
                             LegType leg_type, SrtReceiverType rec_pay,
                             String curve_name, String ref_rate_code,
                             GenSwapLeg *leg) {
  Err err = NULL;
  SrtCurvePtr crv;
  int i;
  ...
  /* Allocate space for and generate the times (from today to swap dates) of the
   * swap */
  ...
  err = cvg_list(leg->start_date, leg->end_date, leg_sdp->basis_code,
                 &(leg->cvg));
  if (err)
    return (err);
  ...
  case SPREAD_AND_NOTIONALS_LEG:
    /* Sets the spreads RefRate (Libor        ,...) vs Cash + exchange of
     * notionals */
    leg->initial_exchange = initial_exchange;
    leg->final_exchange = final_exchange;
    err = spread_list(leg->start_date, leg->end_date, ref_rate_code,
                      &(leg->spread));
    if (err)
      return (err);
  ...
  case SPREAD_LEG:
    ...
    break;
  case FLOATING_LEG:
    /* Sets the spreads RefRate (Libor        ,...) vs Cash + cash_fra +
    ...
    if ((leg->spread).len == 0)
      return serror("Error in computing spreads in swap leg");
    err = fwd_cash_list(leg->start_date, leg->end_date, leg_sdp->basis_code,
                        curve_name, &(leg->fwd));
    break;
  case DRS_LEG:
    break;
...
Err swp_f_create_GenFullSwap(SwapDP *rec_leg_sdp, SwapDP *pay_leg_sdp,
                             double fixed, double initial_exchange,
                             double final_exchange, SwapType swap_type,
                             String curve_name, GenFullSwap *swap) {
  Err err;
  ...
  /* Gets the curve */
  if ((swap_type == CMS_MARGIN_FLOATING_SWAP) ||
      (swap_type == CMS_FOR_TEC_MARGIN_FIXED_SWAP) ||
      (swap_type == CMT_MARGIN_FLOATING_SWAP) ||
      (swap_type == CMS_MARGIN_FIXED_SWAP) ||
      (swap_type == CMS_FOR_TEC_MARGIN_FLOATING_SWAP) ||
      (swap_type == CMT_MARGIN_FIXED_SWAP) ||
      (swap_type == TEC_MARGIN_FLOATING_SWAP) ||
      (swap_type == TEC_MARGIN_FIXED_SWAP)) {
    crv = lookup_curve(curve_name);
  ...
  case FIXED_NOTIONALS_SWAP:
    err = create_GenSwapLeg(today, rec_leg_sdp, fixed, 0.0, 0.0, FIXED_LEG,
                            SRT_RECEIVER, curve_name, NULL, &full_swap.leg[0]);
    if (err)
      return (err);
    err = create_GenSwapLeg(today, pay_leg_sdp, 0.0, 1.0, 1.0, NOTIONALS_LEG,
                            SRT_PAYER, curve_name, NULL, &full_swap.leg[1]);
    if (err)
      return (err);
    break;
  ...
  case FIXED_FLOATING_SWAP:
    ...
    if (err)
      return (err);
    err = create_GenSwapLeg(today, pay_leg_sdp, 0.0, 1.0, 1.0, FLOATING_LEG,
                            SRT_PAYER, curve_name, NULL, &full_swap.leg[1]);
    if (err)
      return (err);
    break;
  ...
  case DRS_MARGIN_FLOATING_SWAP:
    err = create_GenSwapLeg(today, rec_leg_sdp, 0.0, 0.0, 0.0, DRS_MARGIN_LEG,
                            SRT_RECEIVER, curve_name, NULL, &full_swap.leg[0]);
    if (err)
      return (err);
    err = create_GenSwapLeg(today, pay_leg_sdp, 0.0, 1.0, 1.0, FLOATING_LEG,
  ...
  case DRS_MARGIN_FIXED_SWAP:
    ...
    if (err)
      return (err);
    err = create_GenSwapLeg(today, pay_leg_sdp, 0.0, 1.0, 1.0, FIXED_LEG,
                            SRT_PAYER, curve_name, NULL, &full_swap.leg[1]);
    if (err)
      return (err);
    break;
  ...
  case CMS_MARGIN_FLOATING_SWAP:
    err = create_GenSwapLeg(today, rec_leg_sdp, 0.0, 0.0, 0.0, CMS_MARGIN_LEG,
                            SRT_RECEIVER, curve_name, NULL, &full_swap.leg[0]);
    if (err)
      return (err);
    err = create_GenSwapLeg(today, pay_leg_sdp, 0.0, 1.0, 1.0, NOTIONALS_LEG,
  ...
  case CMS_FOR_TEC_MARGIN_FLOATING_SWAP:
    err = create_GenSwapLeg(today, rec_leg_sdp, 0.0, 0.0, 0.0,
                            CMS_FOR_TEC_MARGIN_LEG, SRT_RECEIVER, curve_name,
                            NULL, &full_swap.leg[0]);
    if (err)
      return (err);
    err = create_GenSwapLeg(today, pay_leg_sdp, 0.0, 1.0, 1.0, NOTIONALS_LEG,
                            SRT_PAYER, curve_name, NULL, &full_swap.leg[1]);
  ...
  case CMS_FOR_TEC_MARGIN_FIXED_SWAP:
    ...
    if (err)
      return (err);
    err = create_GenSwapLeg(today, pay_leg_sdp, fixed, 1.0, 1.0, FIXED_LEG,
                            SRT_PAYER, curve_name, NULL, &full_swap.leg[1]);
    if (err)
      return (err);
    break;
  ...
  case CMT_MARGIN_FLOATING_SWAP:
    err = create_GenSwapLeg(today, rec_leg_sdp, 0.0, 0.0, 0.0, CMT_MARGIN_LEG,
                            SRT_RECEIVER, curve_name, NULL, &full_swap.leg[0]);
    if (err)
      return (err);
    err = create_GenSwapLeg(today, pay_leg_sdp, 0.0, 1.0, 1.0, NOTIONALS_LEG,
  ...
  case TEC_MARGIN_FLOATING_SWAP:
    err = create_GenSwapLeg(today, rec_leg_sdp, 0.0, 0.0, 0.0, TEC_MARGIN_LEG,
                            SRT_RECEIVER, curve_name, NULL, &full_swap.leg[0]);
    if (err)
      return (err);
    err = create_GenSwapLeg(today, pay_leg_sdp, 0.0, 1.0, 1.0, NOTIONALS_LEG,
  ...
  case CMS_MARGIN_FIXED_SWAP:
    ...
    if (err)
      return (err);
    err = create_GenSwapLeg(today, pay_leg_sdp, fixed, 0.0, 0.0, FIXED_LEG,
                            SRT_PAYER, curve_name, NULL, &full_swap.leg[1]);
    if (err)
      return (err);
    break;
...
*/
static Err populate_GenSwapLeg(double margin, SrtCurvePtr crv,
                               GenSwapLeg *leg) {
  int i;
  Err err = NULL;
  ...
  case FLOATING_LEG:
    for (i = 0; i < leg->end_date.len; i++) {
      leg->payment.d[i + 1] +=
          (leg->spread.d[i] + leg->fwd.d[i]) * leg->cvg.d[i];
    }
    break;
  case CMS_MARGIN_LEG:
    leg->margin = margin;
    err = conv_fwd_list(leg->today, leg->pay_date, leg->fwd, leg->vol, crv,
                        SWAP_RATE, &(leg->cpn));
    if (err)
      return (err);
  ...
  case CMT_MARGIN_LEG:
    ...
    if (err)
      return (err);
    err = treas_vol_list(leg->pay_date, leg->temp_fwd, leg->fwd, crv,
                         &(leg->vol));
    if (err)
      return (err);
    err = conv_fwd_list(leg->today, leg->pay_date, leg->fwd, leg->vol, crv,
                        TREASURY_RATE, &(leg->cpn));
    if (err)
      return (err);
    for (i = 1; i < leg->pay_date.len; i++) {
  ...
  case CMS_FOR_TEC_MARGIN_LEG:
    ...
    for (i = 1; i < leg->pay_date.len; i++) {
      leg->payment.d[i] =
          exp(1.0 / (double)(leg->sdp.cxxompd) * log(1.0 + leg->cpn.d[i])) -
          1.0;
    }
    break;
  case TEC_MARGIN_LEG:
...
Err swp_f_make_FixedLeg(SwapDP *fixedsdp, double fixed, Date today,
                        GenSwapLeg *fixedleg) {
  Err err = NULL;

  /* Create the fixed leg: dates        , cvg        , coupon */
  err = create_GenSwapLeg(today, fixedsdp, fixed, 0.0, 0.0, FIXED_LEG,
                          SRT_RECEIVER, NULL, NULL, fixedleg);
  if (err) {
...
Err swp_f_make_FixedAndNotionalsLeg(SwapDP *fixedsdp, double fixed,
                                    double initial, double final, Date today,
                                    GenSwapLeg *fixedleg) {
  Err err = NULL;

  /* Create the fixed and notionals leg: dates        , cvg        , coupon ,
   * initial        , final */
  err = create_GenSwapLeg(today, fixedsdp, fixed, initial, final,
                          FIXED_AND_NOTIONALS_LEG, SRT_RECEIVER, NULL, NULL,
                          fixedleg);
  if (err) {
    swp_f_freein_GenSwapLeg(fixedleg);
    return err;
  ...
  /* Sets the payments in the fixed & notionals leg (no margin): payment =
   * coupon*cvg */
  err = populate_GenSwapLeg(0.0, NULL, fixedleg);
  if (err) {
    swp_f_freein_GenSwapLeg(fixedleg);
...
Err swp_f_make_NotionalsLeg(SwapDP *fixedsdp, double initial, double final,
                            Date today, GenSwapLeg *fixedleg) {
  Err err = NULL;

  /* Create the notionals leg: initial        , final */
  err = create_GenSwapLeg(today, fixedsdp, 0.0, initial, final, NOTIONALS_LEG,
                          SRT_RECEIVER, NULL, NULL, fixedleg);
  if (err) {
    swp_f_freein_GenSwapLeg(fixedleg);
    return err;
...
Err swp_f_make_FloatingLeg(long start, long end, Date today,
                           String ref_rate_code, SrtCurvePtr crv,
                           GenSwapLeg *floatleg) {
  Err err = NULL;
  SwapDP floatsdp;
  SrtCompounding comp;
  ...
  /* Set up the corresponding SwapDP: start        , end        ,... generated
   * BACKWARD */
  ...
  err = create_GenSwapLeg(today, &floatsdp, 0.0, 0.0, 0.0, FLOATING_LEG,
                          SRT_RECEIVER, get_curve_name(crv), ref_rate_code,
                          floatleg);
  if (err) {
  ...
  /* Sets the payments in the floating leg (no margin): payment = (spread +
   * fra)*cvg */
...
Err swp_f_make_SpreadLeg(long start, long end, Date today, String ref_rate_code,
  ...
  err = create_GenSwapLeg(today, &spreadsdp, 0.0, 0.0, 0.0, SPREAD_LEG,
                          SRT_RECEIVER, NULL, ref_rate_code, spreadleg);
  if (err) {
...
Err swp_f_make_SpreadAndNotionalsLeg(long start, long end, double initial,
                                     double final, Date today,
  ...
  /* Create the floating leg: dates        , cvg        , spreads  */
  err = create_GenSwapLeg(today, &floatsdp, 0.0, initial, final,
                          SPREAD_AND_NOTIONALS_LEG, SRT_RECEIVER, NULL,
                          ref_rate_code, floatleg);
  if (err) {
    swp_f_freein_GenSwapLeg(floatleg);
    return err;
  }
...
Err swp_f_make_FixedLegDatesAndCoverages(SwapDP *swapdp, Date today,
                                         Date **pay_dates, int *num_pay_dates,
  ...
  err = create_GenSwapLeg(today, swapdp, 1.0, 0.0, 0.0, FIXED_LEG, SRT_RECEIVER,
                          NULL, NULL, &fixedleg);
  if (err) {
    swp_f_freein_GenSwapLeg(&fixedleg);
    return err;
...
Err swp_f_make_FloatLegDatesAndCoverages(SwapDP *swapdp, Date today,
                                         Date **pay_dates, int *num_pay_dates,
                                         Date **fixing_dates,
                                         Date **start_dates, Date **end_dates,
                                         double **coverages, int *num_dates) {
  ...
  /* Make a CASH floating leg (without computing the FRA's): dates        ,
   * times        , cvg */
  err = create_GenSwapLeg(today, swapdp, 1.0, 0.0, 0.0, SPREAD_LEG,
                          SRT_RECEIVER, NULL, "CASH", &floatleg);
  if (err) {
    swp_f_freein_GenSwapLeg(&floatleg);
    return err;
  }
...
Err swp_f_make_FloatLegDatesCoveragesAndSpreads(
    ...
    int *num_pay_dates, Date **fixing_dates, Date **start_dates,
    Date **end_dates, double **coverages, double **spreads, int *num_dates) {
  GenSwapLeg floatleg;
  Err err = NULL;

  /* Make the floating leg: dates        , times        , cvg        , spreads
   * ...*/
  err = swp_f_make_SpreadLeg(swapdp->start, swapdp->end, today, refrate,
                             &floatleg);
  if (err)
    return err;
...
/* A weird function that can be needed to merge on one leg two different ones:
   dates are merged        , and payment are computed as : + for recleg        ,
   - for payleg  */
Err swp_f_merge_SwapLegs(GenSwapLeg *recleg, GenSwapLeg *payleg,
  ...
  while ((i_over == 0) || (j_over == 0)) {
    ...
    /* Make sure we do not go beyond the last date of the leg : if we do , set
     * the over flag to 1 */
  ...
  while ((i_over == 0) || (j_over == 0)) {
    ...
    /* Make sure we do not go beyond the last date of the leg: if we do        ,
     * set the over flag to 1 */
    if (i == recleg->leg_length) {
      i = recleg->leg_length - 1;
      i_over = 1;
    }

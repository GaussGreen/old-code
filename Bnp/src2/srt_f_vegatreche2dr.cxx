...
/*---------------------------------------------------------------------------
  ...
  DESCRIPTION     :values a structure according to a 2d tree discretization of
                                   the cheyette interest rate model        , by
which we mean the one where sigma(t        ,r) = sigma(t)*r
-----------------------------------------------------------------------------*/

Err srt_f_vegatreche2dr(SrtUndPtr und,           /* market parameters */
...

{
    ...
    /* if yes */
      ...
      if (io_request_val_p->type == IO_PREMIUM) {
        ...
        if (err = srt_f_vegashiftirministp(
                stp, und, und_info, sigma_bucket_start, sigma_bucket_end,
      ...
      /* vega shifts */
        ...
        if (err = srt_f_vegashiftirministp(
                my_pointer, und, und_info, sigma_bucket_start, sigma_bucket_end,
                sigma_shift, sigma_shift_type, tau_bucket_start, tau_bucket_end,
                tau_shift, tau_shift_type)) {
          return err;
        }

        /* each request of the SrtIOStruct has now a field that points to a
           SrtStp structure. For this structure        , the parameters have
           been computed with the shift corresponding to the request */
  ...
  sz = xtra_calc_num * (maxtrinf.max_r_index - maxtrinf.min_r_index + 1) *
       (maxtrinf.max_phi_index - maxtrinf.min_phi_index + 1) *
       grfnparam->node_dim;

  prev_assets = f4tensor(0, xtra_calc_num - 1, maxtrinf.min_r_index,
                         maxtrinf.max_r_index, maxtrinf.min_phi_index,
                         maxtrinf.max_phi_index, 0, grfnparam->node_dim - 1);
  cur_assets = f4tensor(0, xtra_calc_num - 1, maxtrinf.min_r_index,
                        maxtrinf.max_r_index, maxtrinf.min_phi_index,
                        maxtrinf.max_phi_index, 0, grfnparam->node_dim - 1);

  cur_r = dvector(maxtrinf.min_r_index, maxtrinf.max_r_index);
  prev_r = dvector(maxtrinf.min_r_index, maxtrinf.max_r_index);
  cur_phi = dmatrix(maxtrinf.min_r_index, maxtrinf.max_r_index,
                    maxtrinf.min_phi_index, maxtrinf.max_phi_index);
  prev_phi = dmatrix(maxtrinf.min_r_index, maxtrinf.max_r_index,
                     maxtrinf.min_phi_index, maxtrinf.max_phi_index);

  if (!prev_assets || !cur_assets || !cur_r || !prev_r || !cur_phi || !prev_phi)
    return serror("memory allocation failure in srt_f_treche2dr");

  memset(&cur_assets[0][maxtrinf.min_r_index][maxtrinf.min_phi_index][0], 0,
         sz * sizeof(double));
  ...
  for (stp = bot; stp != NULL; stp = stp->prev) {
    ...
    for (io_request = iolist->head; io_request != NULL;
      ...
      for (i = trinf->min_r_index; i <= trinf->max_r_index; i++) {
        sam_get(node.cxxur_sam, 0, SHORT_RATE) = cur_r[i];

        /* To ensure compatibility with the Y_T_at_t functions */
        sam_get(node.cxxur_sam, 0, STATEVAR) =
            cur_r[i] - sam_get(tminf->fwd_sam, 0, F_0_t);

        /** For each phi **/
        for (j = trinf->min_phi_index; j <= trinf->max_phi_index; j++) {
          /*  Now the time info is taken from the specific shifted price
          structure. The tree being frozen        , the tree info keeps being
          taken from stp.	*/
          sam_get(node.cxxur_sam, 0, PHI) = cur_phi[i][j];
          ...
          if (stp->next) {
            ...
            srt_f_chedrfatsam(my_pointer, &node.cxxur_sam, &node.drift_sam, 0);
            ...
            /* The function uses inside only field tminf -> my_pointer instead
             * of stp */
            srt_f_chevaratsam(my_pointer, &node.cxxur_sam, &node.var_at_sam, 0);
            ...
            /* The function uses inside only field trinf -> stp */
            srt_f_chetredcntvec(stp, cur_assets[xtra_calc_index][i][j], &node,
                                prev_assets[xtra_calc_index],
                                grfnparam->node_dim, prev_phi);
          }
  ...
  for (io_request = iolist->head; io_request != NULL;
    ...
    if (srt_f_treche_requestOK(io_request_val_p->type) == SRT_NO)
      continue;
    strncpy(io_request_val_p->computation_origin, "Tree_CHE_2d",
            strlen("Tree_CHE_2d"));
    io_request_val_p->dval =
        prev_assets[xtra_calc_index][0][0][grfnparam->node_dim - 1];
  ...
  err = srt_f_IOstructsetcolpvs((SrtIOStruct *)(iolist), SRT_NO,
                                (double *)prev_assets[0][0][0],
                                grfnparam->node_dim, "");
  if (err)
    return err;

  /** free the world **/
  free_f4tensor(prev_assets, 0, xtra_calc_num - 1, maxtrinf.min_r_index,
                maxtrinf.max_r_index, maxtrinf.min_phi_index,
                maxtrinf.max_phi_index, 0, grfnparam->node_dim - 1);
  free_f4tensor(cur_assets, 0, xtra_calc_num - 1, maxtrinf.min_r_index,
                maxtrinf.max_r_index, maxtrinf.min_phi_index,
                maxtrinf.max_phi_index, 0, grfnparam->node_dim - 1);
  ...
  free_dmatrix(cur_phi, maxtrinf.min_r_index, maxtrinf.max_r_index,
               maxtrinf.min_phi_index, maxtrinf.max_phi_index);
  free_dmatrix(prev_phi, maxtrinf.min_r_index, maxtrinf.max_r_index,
               maxtrinf.min_phi_index, maxtrinf.max_phi_index);

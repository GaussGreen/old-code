    ...
    double** prod_val)
{
    ...
    err = display_FX1F_TermStruct(underlying, &sigma_n_fx, &sigma_time_fx, &sigma_fx);
    ...
    double** greeks)
{
    ...
    double *bar_lvl_up = NULL, *bar_lvl_down = NULL, *prod_val_ki = NULL, **greeks_ki = NULL;
    ...
    for (i = nstp - 1; i >= 0; i--)
    {
        date[i] = today + time[i] * DAYS_IN_YEAR;
        bar_lvl_up[i] =
            barrier_up * swp_f_df(today, date[i], dom_yc) / swp_f_df(today, date[i], for_yc);
        bar_lvl_down[i] =
            barrier_down * swp_f_df(today, date[i], dom_yc) / swp_f_df(today, date[i], for_yc);
    }
    ...
    double** greeks)
{
    ...
    settlmt_date =
        add_unit((long)(today + maturity * DAYS_IN_YEAR), 2, SRT_BDAY, MODIFIED_SUCCEEDING);
    ...
    double** greeks)
{
    ...
    double *bar_lvl_up = NULL, *bar_lvl_down = NULL, *new_reb_down = NULL, *new_reb_up = NULL,
    ...
    if (!sig || !date || !drift || !bar_lvl_up || !bar_lvl_down || !eval_evt || !new_reb_down ||
        !new_reb_up)
    {
        err = "Memory allocation failure in SrtGrfnFxSabrSLAdiBar";
        goto FREE_RETURN;

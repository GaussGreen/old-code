/**********************************************************************
 *      Name: SrtGrfnMainQuanto.cxx                                     *
 *  Function: Entry point to GRFN with raw data                       *
 *            in the case of the 3Factor model
 ** degenerated in Quanto	2 Factor model * Copyright: (C) BNP Paribas
 *Capital Markets Ltd.                    *
 *--------------------------------------------------------------------*
...
Err Get_FX_TS(char *underlying, double **sigma_time, double **sigma,
              long *sigma_n) {
  SrtUndPtr und;
  TermStruct *ts;
  long corr_date_n;
  ...
  err = srt_f_display_FX_TermStruct(underlying, sigma_n, sigma_time, sigma,
...
void fill_fwd(
    long nstp, double *time, double *date, char *dom_yc, char *for_yc,
    /*	To be allocated by caller        , filled by the function */
    double *dom_ifr, double *for_ifr) {
  int i;
...
Err fill_time_vector_without_barrier(double **time, int *nstp,
                                     int num_vol_times, double *vol_times,
                                     int target_nstp) {
  Err err = NULL;
...
char *SrtGrfnQuantoPDE(char *und3dfx, int numeventdates, long *eventdates,
  ...
  err = fill_time_vector_without_barrier(&time, &nstp, num_vol_times, vol_times,
                                         num_stp);
  if (err) {
    goto FREE_RETURN;
  }
  ...
  *prod_val = (double *)calloc(*num_prod + 1, sizeof(double));
  ...
  err = doublelgm1fQuanto_adi(
      nstp, time, date, num_stpx, dom_lam, for_lam, merge_dates, sigdom, sigfor,
      sigfx, nb_merge_dates, quantocorr, domforcorr, void_prm, is_event,
      dom_ifr, for_ifr, dom_yc, for_yc, payoff_doublelgm1fquanto_pde, *num_prod,
      *prod_val);

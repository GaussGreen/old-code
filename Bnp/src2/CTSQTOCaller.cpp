    ...
    LGMQTO_UND und_exp)
{
    ...
    double vol_fra_cpn, power, vol_fx_cpn, rho_ffx_cpn, vol_d_cpn, coef_cpn, quanto_corr_cpn,
        DRS_corr_cpn, fra_d_cpn, cvg_d_cpn;
    ...
    err = swp_f_get_ref_rate_details(ra_refrate, (SrtBasisCode*)&rabasis, (SrtCompounding*)&rafreq);
    if (err)
    {
    ...
    ctsqto  = (ctsqto_str*)calloc(1, sizeof(ctsqto_str));
    und     = (lgmQto_und*)calloc(1, sizeof(lgmQto_und));
    adi_arg = (lgmQto_adi_arg*)calloc(1, sizeof(lgmQto_adi_arg));
    ...
    if (calc_fwdiv && (ctsqto->num_calls > 0))
    {
        ...
        err = ctsqto_calc_mdl_iv_fwd(ctsqto, und, adi_arg, NUM_HERMITE, und->model_fwdiv);
    ...
    if (fund_leg->num_cpn > 0)
    {
        if (for_fund)
        {
            fund_leg_pv += swp_f_df(today, fund_leg->cpn[0].start_date, dom_yc) * eq_final_ex;
            ...
            if (calc_fwdiv)
            {
                ...
                for (i = 0; i < ctsqto->num_calls; i++)
                {
                    ...
                    und->market_fwdiv[i] =
                        fact *
                        swp_f_df(
                            today, (ctsqto->fund_leg->cpn + callt->fund_idx)->start_date, dom_yc) *
                        eq_final_ex;
                }
            }
        }
        ...
        {
            fund_leg_pv +=
                swp_f_df(today, fund_leg->cpn[0].start_date, dom_yc) * fund_leg->notional;
    ...
    if (for_fund)
    {
        ...
        if (calc_fwdiv)
        {
            for (i = 0; i < ctsqto->num_calls; i++)
            {
                ...
                und->market_fwdiv[i] -=
                    fact *
                    swp_f_df(today, (ctsqto->fund_leg->cpn + callt->fund_idx)->start_date, dom_yc) *
                    eq_final_ex;
            }
        }
    }
    ...
    FwdPVDates = (long*)calloc(nbFwdPVDates, sizeof(long));
    if (!FwdPVDates)
    {
    ...
    while (i < ra_ncpn && ra_fixingdates[i][0] < today + eod_fix_flag)
    {
        if (ra_pay[i] >= today + eod_pay_flag)
        {
            ...
            weight_fixing =
                coverage(ra_start[i], ra_pay[i], bas) /
                ((ra_pay[i] -
                  add_unit(ra_fixingdates[i][0], spotlag_f, SRT_BDAY, MODIFIED_SUCCEEDING)) *
                 1.0) *
            ...
            while (j < ra_nfixings[i] && ra_fixingdates[i][j] < today + eod_fix_flag)
            {
                ...
                if (ra_cpn_type != 0)
                {
                    ...
                    else
                    {
                        ...
                        err = swp_f_get_ref_rate_details(
                            ra_float_refrate, &basis_float_cpn, &freq_float_cpn);
                        if (err)
                            goto FREE_RETURN;

                        tenor_float_cpn = 12 / freq_float_cpn;
                        d_cpn           = add_unit(
                            ra_float_startl[i], tenor_float_cpn, SRT_MONTH, MODIFIED_SUCCEEDING);
                        ...
                        fixing_cpn_float =
                            add_unit(ra_float_startl[i], -spotlag, SRT_BDAY, MODIFIED_SUCCEEDING);
                        ...
                        {
                            // Get the FX ATM vol & FX correlations (using the same ones as for the
                            // quanto index
                            for (; q < n_fxvol_dates - 1 && fxvol_dates[q + 1] < ra_float_startl[i];
                                 q++)
                                ;
                            ...
                            else
                            {
                                ...
                                rho_ffx_cpn =
                                    coef_cpn * qtocorrel[q + 1] + (1.0 - coef_cpn) * qtocorrel[q];
                            }
                        ...
                        else
                        {
                            ...
                            err       = swp_f_SABRvol(
                                dom_vc, d1_cpn, d2_cpn, fra_d_cpn, &vol_d_cpn, &power, SABR_LOGVOL);
                            if (err)
                                return err;
                ...
                // For testing: if( (ra_fixings[i][j] > -0.1) && (ra_fixings[i][j] > lower_barr[i])
                // && (ra_fixings[i][j] < upper_barr[i]) )
                {
                    if (j < ra_nfixings[i] - 1)
                    {
                        ra_leg_pv -=
                            (add_unit(
                                 ra_fixingdates[i][j + 1],
                                 spotlag_f,
                                 SRT_BDAY,
                                 MODIFIED_SUCCEEDING) -
                             add_unit(
                                 ra_fixingdates[i][j], spotlag_f, SRT_BDAY, MODIFIED_SUCCEEDING)) *
            ...
            if (j < ra_nfixings[i])
            {
                ...
                err = RA_FwdPV(
                    dom_yc, for_yc, ra_leg_fixing, today, 1, FwdPVDates, &ra_leg_pv_fixing);
    ...
    if (for_fund)
    {
        ...
        {
            temp = swp_f_df(today, fund_leg->cpn[fund_leg->num_cpn - 1].pay_date, dom_yc) * ra_not;
            ra_leg_pv += -fact * temp;
        ...
        if (calc_fwdiv)
        {
            for (i = 0; i < ctsqto->num_calls; i++)
            {
                callt = ctsqto->call + i;
                und->market_fwdiv[i] -=
                    -fact * swp_f_df(today, ctsqto->ra_leg->dates[callt->ra_idx], dom_yc) * ra_not;
            }
        }
    }
...
FREE_RETURN:
    ...
    if (dates)
        free_lngvector(dates, 0, ra_ncpn);

...
//						needed for updating exercise probability 
//
//	Returns :
//-------------------------------------------------------------------------------------------------------
static void _init_ExProb_Comm(
	...
	)
{
	...
	const double dDeltaX3 = (pdX3_End-pdX3_Begin)>1?pdX3_Begin[1]-pdX3_Begin[0]:1.;
	
	const double dDeltaArea = dDeltaX1*dDeltaX3==0.? dDeltaX1: dDeltaX1*dDeltaX3;
	...
	for(nX1=nX1_Begin;nX1<nX1_End;++nX1)
	{
		...
		for(nX3=nX3_Begin;nX3<nX3_End;++nX3)
		{
			...
			ppdDensity[nX1][nX3]  = _NormDensity(dC,dX1,0.,dVarX1)*_NormDensity(dC,dX3,0.,dVarX3);
...
#ifdef _DEBUG
	{
		const double dSum_Prob= dDeltaArea*_Sum_dMatrix(ppdDensity,nX1_Begin,nX1_End,nX3_Begin,nX3_End);
...
static void _EXPROB_BACKWARD_(
					const double *pdEx, // exercise time
					const double *pdX1_Begin,
					const double *pdX1_End,
					const double *pdX3_Begin,
					const double *pdX3_End,
					...
					)
{
	...
	const int nEx_I = 	(const double*)bsearch(pdEx,pdEx_Begin,nNumEx,sizeof(*pdEx),_dcompare) - pdEx_Begin;
	...
	// interested in the exercise probabilities (as opposed to not exercise at all)
	if(nEx_I == nNumEx-1)
	{
		for(nX3 = nX3_Begin;nX3 <nX3_End;++nX3)
		{
			for(nX1 = nX1_Begin;nX1 <nX1_End;++nX1)
			{
				_ASSERTE(ppdExIndicator[nX1][nX3]==0.||ppdExIndicator[nX1][nX3]==1.);
				ppdU[nX1][nX3] = 	pComm->m_nCompute_ExProb ? ppdExIndicator[nX1][nX3] : 1.-ppdExIndicator[nX1][nX3];
			}
		}
	}
					...
					)
{
	...
	const double dT = *pdT;
	const int nI = 	(const double*)bsearch(&dT,pComm->m_pdT_Begin,nSize,sizeof(dT),_dcompare) - pComm->m_pdT_Begin;
	...
	// if first exercise , then populate the grid with initial probabilities 
	if(nI==0)
		_Init_Density(pdX1_Begin,pdX1_End,pdX3_Begin,pdX3_End,dVar_X1,dVar_X3,ppdU,nX1_Begin,nX3_Begin);

	*pdExProb=0.;

	_ASSERTE(pComm->m_pnExIndicator_RBegin[nI]>=nX1_Begin && nX1_End>=pComm->m_pnExIndicator_REnd[nI]);
	_ASSERTE(pComm->m_pnExIndicator_CBegin[nI]>=nX3_Begin&&nX3_End>=pComm->m_pnExIndicator_CEnd[nI]);
	...
	for(nX1=nX1_Begin;nX1<nX1_End;++nX1)
	{
		for(nX3=nX3_Begin;nX3<nX3_End;++nX3)
		{
			...
			_ASSERTE(nX1<pComm->m_pnExIndicator_RBegin[nI]||nX1>=pComm->m_pnExIndicator_REnd[nI]?dIndiator==0.:1);
			_ASSERTE(nX3<pComm->m_pnExIndicator_CBegin[nI]||nX3>=pComm->m_pnExIndicator_CEnd[nI]?dIndiator==0.:1);
	   ...
	   )
{
	...
	double *pdTKnot_Begin=memcpy(_alloca((nSize+1)*sizeof(double)),pdT_Begin,nSize*sizeof(double));
	int *pnSizeX1_Begin_=memcpy(_alloca((nSize+1)*sizeof(int)),pnSizeX1_Begin,nSize*sizeof(int));
	int *pnSizeX3_Begin_=memcpy(_alloca((nSize+1)*sizeof(int)),pnSizeX3_Begin,nSize*sizeof(int));
	double *pdTotalProb = _alloca(nSize*sizeof(double)), *pdExProb=pdExProb_Begin;
	const char *szErr=0;
		...
		)
{
	...
	const int nNumEx = pdEx_End-pdEx_Begin;
	double *pdT_Begin = memset(_alloca((nNumEx+1)*sizeof(double)),0,(nNumEx+1)*sizeof(double));
	const char *szErr=0;
...
static void _Compute_ExProb_Forward(
	const _Grid *pGrid,
...
static void _Init_ExDensity(
	const double **ppdDensity,
	const double **ppdIndicator,	
	const _Grid *pGrid,
	double **ppdExDensity
	)
{

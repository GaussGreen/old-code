 ...
 **
 *																								  *
 *		Note that the daily PV is always bigger than the period PV. The two prices computed
 *here  * are upper bounds to the real PV.
 **
 *																								  *
    ...
    double*    DBspread)
{
    ...
    /* Calculate the vol type.  This fixes a previous bug wherein the vol type was assumed to be
     * lognormal */
    ...
    if (err = swp_f_Cms_Rate(
            ...
            FraSpot,
            theoenddate,
            passCms.AdjForSpread,
            passCms.AdjForSpread * dspread,
            szVolCurveName,
            passCms.NumStrikesInVol,
            passCms.Strikes,
            &dCms))
    ...
    if (err = (*getLogVol)(
            szVolCurveName, (double)(FraSpot), (double)enddate, lowerstrike, &lowvol, &lognorm))
    ...
    if (err = (*getLogVol)(
            szVolCurveName, (double)(FraSpot), (double)enddate, upperstrike, &upvol, &lognorm))
        smessage("Error in swp_f_vol");
    ...
    else
    {
        if (lognorm == 0.0)
        {
            lowercall = srt_f_optblknrm(
                dCms, lowerstrike, lowvol + lowshift, maturity, 1.0, SRT_CALL, PREMIUM);
            lowercalleps = srt_f_optblknrm(
                dCms,
                lowerstrike - epsilon,
                lowvoleps + lowshift,
                maturity,
        ...
        {
            lowercall = srt_f_optblksch(
                dCms, lowerstrike, lowvol + lowshift, maturity, 1.0, SRT_CALL, PREMIUM);
            lowercalleps = srt_f_optblksch(
                dCms,
                lowerstrike - epsilon,
                lowvoleps + lowshift,
                maturity,
    ...
    {
        uppercall =
            srt_f_optblknrm(dCms, upperstrike, upvol + upshift, maturity, 1.0, SRT_CALL, PREMIUM);
        uppercalleps = srt_f_optblknrm(
            dCms, upperstrike + epsilon, upvoleps + upshift, maturity, 1.0, SRT_CALL, PREMIUM);
    }
    else
    {
        uppercall =
            srt_f_optblksch(dCms, upperstrike, upvol + upshift, maturity, 1.0, SRT_CALL, PREMIUM);
        uppercalleps = srt_f_optblksch(
            dCms, upperstrike + epsilon, upvoleps + upshift, maturity, 1.0, SRT_CALL, PREMIUM);
    }
    ...
    double*    DBspread)
{
    ...
    if (err = (*getLogVol)(
            szVolCurveName, (double)(FraSpot), (double)enddate, dFra, &atmvolindex, &lognorm))
    {
        smessage("Error in swp_f_vol");
    }
    ...
    maturityacc = coverage(
        today, add_unit(Startdate, -spotlag, SRT_BDAY, MODIFIED_SUCCEEDING), BASIS_ACT_365);

    /* Compute the AccFwd */
    /* first initialise the sdpFra */
    err = swp_f_setSwapDP(
        Startdate, theoenddateacc, float_compoundingacc, float_basisacc, &sdpFraacc);
    if (err)
        return err;
    /* input the spot lag  */
    ...
    err = swp_f_ForwardRate_SwapDP(&sdpFraacc, szYieldCurveName, AccRefRateCode, &dFraacc);
    ...
    correl =
        (corrdown - corrup) * ((double)(FraSpot - Startdate)) / ((double)(paydate - Startdate)) +
        corrup;
    ...
    if ((lognorm == 1.0) && (lognormacc == 1.0))
    {
        ...
        uppercall = srt_f_optblksch(
            dCms * correlcorrec, upperstrike, upvol + upshift, maturity, 1.0, SRT_CALL, PREMIUM);
        uppercalleps = srt_f_optblksch(
            dCms * correlcorrec,
            upperstrike + epsilon,
            upvoleps + upshift,
...

{
    ...
    int    i, j;
    double Nstar; /* minimum number of days the index has to be in the period to get more than the
                     minimum*/
    ...
    /* Compute the AccFwd */
    get_fra(
        add_unit(StartDate, -spotlag, SRT_BDAY, MODIFIED_SUCCEEDING),
        spotlag,
        AccRefRateCode,
        szYieldCurveName,
        &dFraacc);
    ...
    ************************************************/
    ...
    2. If it is a minimum PERIOD coupon (dailyorperiod = NO):
            a. an upper bound is given in the case Paribas Pays the minimum Period coupon (RecPay =
    PAY) b. a lower bound is given in the case  Paribas receives the minimum Period coupon (RecPay =
    REC)
    **************************************************************/
...

    {
        ...
        else if (strcmp(approx, "Normal") == 0)
        {
            pv[0] = sqrt(variance / (2 * SRT_PI)) *
                        exp(-(Nstar - TotalNumDays * avep) * (Nstar - TotalNumDays * avep) /
                            (2 * variance)) +
        ...
        else if (strcmp(approx, "Beta") == 0)
        {
            ...
            pv[0] =
                TotalNumDays * betacmp(alpha + 1, beta) *
                (1 - betaincmp(alpha + 1, beta, Nstar / TotalNumDays) / betacmp(alpha + 1, beta)) /
                betacmp(alpha, beta);
            pv[0] = pv[0] / TotalNumDays;
        }
        ...
        else
        {
            ...
            sigma = sqrt(
                log(variance + TotalNumDays * avep * TotalNumDays * avep) -
...
Err srt_f_FlooredTimeSwap(
    ...
    char*  approx,  /*The type of approximation made on the distribution of the number of days spent
                       within the range*/
...

{
    ...
    double* probafix =
        NULL; /* will store the values of the everyday callspreads when the coupon is fixed*/
    double* probafloat =
        NULL; /*will store the values of the everyday callspreads when the coupon is floating*/
    double* isfriday =
        NULL; /* for each day, isfriday will be 1 if that day is a friday and 0 otherwise*/

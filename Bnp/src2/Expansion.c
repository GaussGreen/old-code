...
Err OldInitControl(OldSControl *ctrl, int nfac, int nbit, int nbr, int nstp, int maxmono, int d_size);
...
static Err OldHash(OldSControl *ctrl, long imono, long *hmono)
{
	...
	for (j=imono; j != 0; j >>= ctrl->nbit)
	{
		...
		if (!p->facpow[i])
		{
			...
			if (!p->facpow[i]) return serror("Memory failure");
			memset(p->facpow[i], 0, sizeof(OldSHashTree));
			if (ctrl->pool_pos >= ctrl->pool_size) return serror("Pool size exceeded");
...
static Err AddProdMonoPoly(OldSControl *ctrl, OldSPolynomial *poly,
						   double c, long mono_idx, OldSPolynomial *p)
{
	Err				err;
	int				j, k;
	long			imono, hmono;
	...
	for (j=0; j < p->n_mono; j++)
	{
		...
		if (ctrl->local_pool[hmono] < 0)
		{
			...
			if (poly->n_mono > ctrl->maxmono) return serror("ctrl->maxmono exceeded");
...
static Err AddProdMonoPolyPoly(OldSControl *ctrl, OldSPolynomial *poly,
							   double c, double *pc, long mono_idx, OldSPolynomial *p1, OldSPolynomial *p2)
{
	Err				err;
	int				i, j, k;
	long			imono, hmono;
	...
	for (i=0; i < p1->n_mono; i++)
	{
		for (j=0; j < p2->n_mono; j++)
		{
			...
			for (k=0; k < ctrl->nstp; k++)
				poly->coef[ctrl->local_pool[hmono]][k] += (pc ? c * pc[k] : c) * p1->coef[i][k] * p2->coef[j][k];
		}
	}
	return NULL;
}
...
static Err OldFillStructZ(OldSControl *ctrl, long iZ)
{
	...
	if ( (pZ = ctrl->pool[hZ]) == NULL )
	{
		...
		if (!pZ) return serror("Memory failure");
		...
		for (i=0; i < 2; i++)
		{
			...
			if (!pZ->Z[i].mono || !pZ->Z[i].coef) return serror("Memory failure");

			memset(&pZ->Z[i].coef[0][0], 0, ctrl->maxmono * ctrl->nstp * sizeof(double));
		}
		...
		for (i=0; i < nfac; i++)
		{
			err = AddProdMonoPoly(ctrl, &pZ->Z[0], facpow[i], iZ - ifac[i], &ctrl->pool[hfac[i]]->Z[0]);
			if (err) return err;
			...
			if (facpow[i] >= 2)
			{
				...
				{
					err = AddProdMonoPolyPoly( ctrl, &pZ->Z[0],
						0.5 * facpow[i] * (facpow[i] - 1), NULL, iZ - 2 * ifac[i],
						&ctrl->pool[hfac[i]]->volZ[ki], &ctrl->pool[hfac[i]]->volZ[ki] );
					if (err) return err;

					for (kj=ki+1; kj < ctrl->nbr; kj++)
					{
						err = AddProdMonoPolyPoly( ctrl, &pZ->Z[0],
							facpow[i] * (facpow[i] - 1), ctrl->rho[ki][kj], iZ - 2 * ifac[i],
							&ctrl->pool[hfac[i]]->volZ[ki], &ctrl->pool[hfac[i]]->volZ[kj] );
						if (err) return err;
					}
				}
			}
			...
			for (j=i+1; j < nfac; j++)
			{
				for (ki=0; ki < ctrl->nbr; ki++)
				{
					...
					{
						err = AddProdMonoPolyPoly( ctrl, &pZ->Z[0],
							facpow[i] * facpow[j], ctrl->rho[ki][kj], iZ - ifac[i] - ifac[j],
							&ctrl->pool[hfac[i]]->volZ[ki], &ctrl->pool[hfac[j]]->volZ[kj] );
						if (err) return err;
					}
				}
			}
			...
			for (i=0; i < nfac; i++)
			{
				if (ctrl->pool[hfac[i]]->volZ[ki].n_mono > 0)
				{
					if (!pZ->volZ[ki].mono)
					{
						...
						pZ->volZ[ki].coef = dmatrix(0, ctrl->maxmono-1, 0, ctrl->nstp-1);

						if (!pZ->volZ[ki].mono || !pZ->volZ[ki].coef) return serror("Memory failure");

						memset(&pZ->volZ[ki].coef[0][0], 0, ctrl->maxmono * ctrl->nstp * sizeof(double));
						memset(ctrl->local_pool, -1, ctrl->pool_size * sizeof(int));
					}

					err = AddProdMonoPoly( ctrl, &pZ->volZ[ki],
						facpow[i], iZ - ifac[i], &ctrl->pool[hfac[i]]->volZ[ki] );
					if (err) return err;
				}
			}
		}
		...
		for (ki=0; ki < ctrl->nbr; ki++)
		{
			for (kj=0; kj < ctrl->nbr; kj++)
			{
				err = AddProdMonoPolyPoly( ctrl, &pZ->Z[1],
					1.0, ctrl->rho[ki][kj], 0, &pZ->volZ[ki], &ctrl->pool[ctrl->h_y0]->volZ[kj] );
				if (err) return err;
			}
		}
	...
	for (i=0; i < 2; i++)
	{
		...
		{
			imono = pZ->Z[i].mono[j];
			if (imono == iZ && i == 1) return serror("System unsolvable: <Z,Y0> depends on Z");
			err = OldHash(ctrl, imono, &hmono);
			if (err) return err;

			if ( imono != 0 && imono != iZ && ( !ctrl->pool[hmono] || ctrl->pool[hmono]->nd == 0 ) )
			{
				err = OldFillStructZ(ctrl, imono);
				if (err) return err;
			}
			di = (imono ? ctrl->pool[hmono]->nd : 1) + i;
			if (di > pZ->nd) pZ->nd = di;
		}
	}
	...
	ctrl->d_pos += pZ->nd;
	if (ctrl->d_pos > ctrl->d_size) return serror("d_size exceeded");

	for (di=0; di < pZ->nd; di++)			// allocate memory for equations for each power of u
	{
		dZ[di].mono = (long *) calloc(ctrl->maxmono, sizeof(long));
		dZ[di].coef = dmatrix(0, ctrl->maxmono-1, 0, ctrl->nstp-1);

		if (!dZ[di].mono || !dZ[di].coef) return serror("Memory failure");
	}
	...
	for (i=0; i < 2; i++)
	{
		for (j=0; j < pZ->Z[i].n_mono; j++)
		{
			...
			if (imono == 0)
			{
				...
				for (k=0; k < ctrl->nstp; k++) dZ[i].coef[dZ[i].n_mono][k] = pZ->Z[i].coef[j][k];
				dZ[i].n_mono ++;
				if (dZ[i].n_mono >= ctrl->maxmono) return serror("ctrl->maxmono exceeded (2)");
			}
			else
			{
				for (di=0; di < ctrl->pool[hmono]->nd; di++)
				{
					...
					for (k=0; k < ctrl->nstp; k++) dZ[i+di].coef[dZ[i+di].n_mono][k] = pZ->Z[i].coef[j][k];
					dZ[i+di].n_mono ++;
					if (dZ[i+di].n_mono >= ctrl->maxmono) return serror("ctrl->maxmono exceeded (2)");
				}
			}
		}
	}
...
static void NAG_CALL OldEvalDerivatives(Integer neq, double t, double y[], double yp[], Nag_User *comm)
{
	...
	for (i=0; i < ctrl->d_pos; i++)
	{
		...
		for (j=0; j < ctrl->d_ODE[i].n_mono; j++)
		{
			...
			if (ctrl->d_ODE[i].mono[j] >= 0) c *= y[ctrl->d_ODE[i].mono[j]];
			yp[i] += c;
		}
	}
}
...
static Err OldIntegrateD(OldSControl *ctrl, double *times, double *y)
{
	...
	if (!yp || !ymax || !RKthres) { err = serror("Memory failure");  goto FREE_RETURN; }
	...
	for (i=0, t=0.0; i < ctrl->nstp; t = times[i], i++)
	{
		...
		nag_ode_ivp_rk_setup(neq, t, y, times[i], RKtol, RKthres,
			Nag_RK_4_5, Nag_RK_range, Nag_ErrorAssess_off, 0.0, &opt, &fail);
		if (fail.code != NE_NOERROR) { err = serror(fail.message);  goto FREE_RETURN; }

		nag_ode_ivp_rk_range(neq, OldEvalDerivatives, times[i], &tgot, y, yp, ymax, &opt, &comm_Nag, &fail);
		if (fail.code != NE_NOERROR) { err = serror(fail.message);  goto FREE_RETURN; }
...
Err OldInitControl(OldSControl *ctrl, int nfac, int nbit, int nbr, int nstp, int maxmono, int d_size)
{
	...
	for (i=0; i < nfac; i++)
	{
		...
		for (j=0; j < 2; j++)
		{
			...
			if (!pZ->Z[j].mono || !pZ->Z[j].coef) return serror("Memory failure");
		...
		for (j=0; j < nbr; j++)
		{
			...
			if (!pZ->volZ[j].mono || !pZ->volZ[j].coef) return serror("Memory failure");
...
void OldFreeControl(OldSControl *ctrl)
{
	...
	if (ctrl->pool) for (i=0; i < ctrl->pool_pos; i++) if (ctrl->pool[i])
	{
		for (j=0; j < 2; j++)
		{
			free(ctrl->pool[i]->Z[j].mono);
			if (ctrl->pool[i]->Z[j].coef)
				free_dmatrix(ctrl->pool[i]->Z[j].coef, 0, ctrl->maxmono-1, 0, ctrl->nstp-1);
		}
		for (j=0; j < ctrl->nbr; j++)
		{
			free(ctrl->pool[i]->volZ[j].mono);
			if (ctrl->pool[i]->volZ[j].coef)
				free_dmatrix(ctrl->pool[i]->volZ[j].coef, 0, ctrl->maxmono-1, 0, ctrl->nstp-1);
		}
		free(ctrl->pool[i]);
	}
	free(ctrl->pool);
	...
	if (ctrl->d_ODE) for (i=0; i < ctrl->d_pos; i++)
	{
		free(ctrl->d_ODE[i].mono);
		if (ctrl->d_ODE[i].coef)
			free_dmatrix(ctrl->d_ODE[i].coef, 0, ctrl->maxmono-1, 0, ctrl->nstp-1);
	}
	free(ctrl->d_ODE);

	if (ctrl->rho) free_f3tensor(ctrl->rho, 0, ctrl->nbr-1, 0, ctrl->nbr-1, 0, ctrl->nstp-1);
...
Err sabr_test_get_d(double sigma, double alpha, double rho, double t, double **d, int nd)
{
	...
	if ( (err = OldHash(&C, Y0, &hY0)) ||
		 ...
		 (err = OldHash(&C, Y1, &hY1)) ||
		 (err = OldHash(&C, Y2, &hY2)) ||
		 (err = OldHash(&C, Y1Y1, &hY1Y1)) )
		goto FREE_RETURN;
		...
		for (ki=0; ki < C.nbr; ki++)
		{
			for (kj=0; kj < C.nbr; kj++)
			{
				err = AddProdMonoPolyPoly( &C, &pZ->Z[1],
					1.0, C.rho[ki][kj], 0, &pZ->volZ[ki], &C.pool[hY0]->volZ[kj] );
				if (err) goto FREE_RETURN;
			}
		}
	}
	...
	if (C.pool[hY1]->nd > nd || C.pool[hY2]->nd > nd || C.pool[hY1Y1]->nd + 1 > nd)
	{ err = serror("nd too small");  goto FREE_RETURN; }

	for (i=0; i < C.pool[hY1]->nd; i++) d[0][i] += y[C.pool[hY1]->d_idx + i];
	for (i=0; i < C.pool[hY2]->nd; i++) d[1][i] += y[C.pool[hY2]->d_idx + i];
	for (i=0; i < C.pool[hY1Y1]->nd; i++) d[1][i+1] += 0.5 * y[C.pool[hY1Y1]->d_idx + i];
...
Err heston_test_get_d(double sigma, double gamma, double alpha, double rho, double t, double *d, int nd)
{
	Err				err = NULL;
	OldSControl		C;
	OldSstructZ		*pZ;
	...
	for (i=0; i < C.pool[hY1]->nd; i++) d[i] += y[C.pool[hY1]->d_idx + i];
	for (i=0; i < C.pool[hY2]->nd; i++) d[i] += y[C.pool[hY2]->d_idx + i];
	for (i=0; i < C.pool[hY1Y1]->nd; i++) d[i+1] += 0.5 * y[C.pool[hY1Y1]->d_idx + i];
...
Err sabr_test_get_all_d(double sigma, double alpha, double rho, double t, double **d, int nd)
{
	...
	if (C.pool[hY1VY1V]->nd > nd || C.pool[hY1VY1D]->nd > nd || C.pool[hY1DY1D]->nd > nd)
	{ err = serror("nd too small");  goto FREE_RETURN; }

	for (i=0; i < C.pool[hY1V]->nd; i++) d[0][i] = y[C.pool[hY1V]->d_idx + i];
	for (i=0; i < C.pool[hY1D]->nd; i++) d[1][i] = y[C.pool[hY1D]->d_idx + i];
	for (i=0; i < C.pool[hY2V]->nd; i++) d[2][i] = y[C.pool[hY2V]->d_idx + i];
	for (i=0; i < C.pool[hY2D]->nd; i++) d[3][i] = y[C.pool[hY2D]->d_idx + i];
	for (i=0; i < C.pool[hY1VY1V]->nd; i++) d[4][i] = y[C.pool[hY1VY1V]->d_idx + i] * 0.5;
	for (i=0; i < C.pool[hY1VY1D]->nd; i++) d[5][i] = y[C.pool[hY1VY1D]->d_idx + i];
	for (i=0; i < C.pool[hY1DY1D]->nd; i++) d[6][i] = y[C.pool[hY1DY1D]->d_idx + i] * 0.5;
...
*/
Err heston_test_get_all_d2(double sigma, double gamma, double alpha, double rho, double t, double **d, int nd)
{
	Err				err = NULL;
	OldSControl		C;
	OldSstructZ		*pZ;
...
Err sabr_test4_get_d(double sigma, double alpha, double rho, double t, double **d, int nd)
{
	...
	if ( (err = OldHash(&C, Y0, &hY0))			||
		 ...
		 (err = OldHash(&C, Y2, &hY2))			||
		 (err = OldHash(&C, Y1Y1, &hY1Y1))		||
		 (err = OldHash(&C, X3, &hX3))			||
		 (err = OldHash(&C, Y3, &hY3))			||
		 (err = OldHash(&C, Y1Y2, &hY1Y2))		||
		 (err = OldHash(&C, Y1Y1Y1, &hY1Y1Y1))	||
		 (err = OldHash(&C, X4, &hX4))			||
		 (err = OldHash(&C, Y4, &hY4))			||
		 (err = OldHash(&C, Y2Y2, &hY2Y2))		||
	...
	for (i=0; i < 4; i++) memset(d[i], 0, nd * sizeof(double));

	if (C.pool[hY1]->nd > nd || C.pool[hY2]->nd > nd || C.pool[hY3]->nd > nd ||
		C.pool[hY1Y1]->nd + 1 > nd || C.pool[hY1Y2]->nd + 1 > nd || C.pool[hY1Y1Y1]->nd + 2 > nd
		|| C.pool[hY1Y1Y1Y1]->nd + 3 > nd )			// Anyway, there are the most of equations
	{ err = serror("nd too small");  goto FREE_RETURN; }

	for (i=0; i < C.pool[hY1]->nd; i++) d[0][i] += y[C.pool[hY1]->d_idx + i];
	for (i=0; i < C.pool[hY2]->nd; i++) d[1][i] += y[C.pool[hY2]->d_idx + i];
	for (i=0; i < C.pool[hY3]->nd; i++) d[2][i] += y[C.pool[hY3]->d_idx + i];
	for (i=0; i < C.pool[hY1Y1]->nd; i++) d[1][i+1] += 0.5 * y[C.pool[hY1Y1]->d_idx + i];
	for (i=0; i < C.pool[hY1Y2]->nd; i++) d[2][i+1] += y[C.pool[hY1Y2]->d_idx + i];
	for (i=0; i < C.pool[hY1Y1Y1]->nd; i++) d[2][i+2] += y[C.pool[hY1Y1Y1]->d_idx + i] / 6.0;
	for (i=0; i < C.pool[hY4]->nd; i++) d[3][i] += y[C.pool[hY4]->d_idx + i];
	for (i=0; i < C.pool[hY2Y2]->nd; i++) d[3][i+1] += 0.5 * y[C.pool[hY2Y2]->d_idx + i];
	for (i=0; i < C.pool[hY1Y3]->nd; i++) d[3][i+1] += y[C.pool[hY1Y3]->d_idx + i];
	for (i=0; i < C.pool[hY1Y1Y2]->nd; i++) d[3][i+2] += 0.5 * y[C.pool[hY1Y1Y2]->d_idx + i];
	for (i=0; i < C.pool[hY1Y1Y1Y1]->nd; i++) d[3][i+3] += y[C.pool[hY1Y1Y1Y1]->d_idx + i] / 24.0;
...
Err ord_sum(SOrdSeries *s, SOrdSeries *s1)
{
	...
	if (s1->min < s->min) s->min = s1->min;
	if (s1->max > s->max) s->max = s1->max;

	for (i=s->min; i <= s->max; i++) p[i] += p1[i];
	return NULL;
}
...
Err ord_product(SOrdSeries *s, SOrdSeries *s1)
{
	int			i, j;
	SOrdSeries	tmp;
	double		*p = s->val + Expansion_ZEROPOS, *p1 = s1->val + Expansion_ZEROPOS, *pt = tmp.val + Expansion_ZEROPOS;
	...
	if (tmp.min + Expansion_ZEROPOS < 0 || tmp.max + Expansion_ZEROPOS >= Expansion_MAXORDER)
				 ...
				 double eps, double *yk, int nyk, double **vols)
{
	...
	if (!f) { err = serror("Memory failure");  goto FREE_RETURN; }
	memset(f, 0, (n-1) * sizeof(SOrdSeries *));

	for (i=0; i < n-1; i++)
	{
		f[i] = (SOrdSeries *) calloc(nyk, sizeof(SOrdSeries));
		if (!f[i]) { err = serror("Memory failure");  goto FREE_RETURN; }
	...
	for (i=0; i < ORDER; i++)
	{
		...
		if (!D[i]) { err = serror("Memory failure");  goto FREE_RETURN; }
	...
	for (i=n-1; i >= 1; i--)
	{
		if (i < n-1) for (j=0; j < ORDER; j++) memcpy(&D[j][i-1], &D[j][i], sizeof(SOrdSeries));

		for (j=0; j < m; j++)
		{
			if (bplus[j] >= ORDER) { err = serror("Invalid bplus");  goto FREE_RETURN; }

			if (i - uplus[j] >= 0)
			{
				ord = i-uplus[j]+splus[j];
				D[bplus[j]][i-1].val[Expansion_ZEROPOS + ord] += d[j][i-uplus[j]];
				if (D[bplus[j]][i-1].min > ord) D[bplus[j]][i-1].min = ord;
				if (D[bplus[j]][i-1].max < ord) D[bplus[j]][i-1].max = ord;
			}
		}
	}
	...
	for (j=0; j < nyk; j++)
	{
		...
		for (i=1; i < n-1; i++)
		{
			...
			if ( ( err = ord_mult_const(&ftmp, -yk[j]/(eps*eps)) ) ||
				( err = ord_shift(&ftmp, -1) ) ||
				( err = ord_sum(&f[i][j], &ftmp) ) )
				goto FREE_RETURN;
		...
		{
			vols[k][j] = 0.0;
			for (i=s[k].min; i <= ORDER-k; i++) vols[k][j] += s[k].val[Expansion_ZEROPOS + i];
		}
	}

FREE_RETURN:
	if (f) for (i=0; i < n-1; i++) free(f[i]);
	free(f);
	free(D[0]);
	free(D[1]);

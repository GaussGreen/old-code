...
Err OldInitControl(
    OldSControl* ctrl, int nfac, int nbit, int nbr, int nstp, int maxmono, int d_size);
...
static Err AddProdMonoPoly(
    OldSControl* ctrl, OldSPolynomial* poly, double c, long mono_idx, OldSPolynomial* p)
{
    Err  err;
    int  j, k;
    long imono, hmono;
...
static Err AddProdMonoPolyPoly(
    OldSControl*    ctrl,
    OldSPolynomial* poly,
    double          c,
    double*         pc,
...
static Err OldFillStructZ(OldSControl* ctrl, long iZ)
{
    ...
    if ((pZ = ctrl->pool[hZ]) == NULL)
    {
        ...
        {
            err = AddProdMonoPoly(
                ctrl, &pZ->Z[0], facpow[i], iZ - ifac[i], &ctrl->pool[hfac[i]]->Z[0]);
            if (err)
                return err;
...
OldEvalDerivatives(Integer neq, double t, double y[], double yp[], Nag_User* comm)
{
...
static Err OldIntegrateD(OldSControl* ctrl, double* times, double* y)
{
    ...
    for (i = 0, t = 0.0; i < ctrl->nstp; t = times[i], i++)
    {
        ...
        nag_ode_ivp_rk_range(
            neq, OldEvalDerivatives, times[i], &tgot, y, yp, ymax, &opt, &comm_Nag, &fail);
        if (fail.code != NE_NOERROR)
        {
            err = serror(fail.message);
            goto FREE_RETURN;
...
void OldFreeControl(OldSControl* ctrl)
{
            ...
            if (ctrl->pool[i])
            {
                for (j = 0; j < 2; j++)
                {
                    ...
                    if (ctrl->pool[i]->Z[j].coef)
                        free_dmatrix(
                            ctrl->pool[i]->Z[j].coef, 0, ctrl->maxmono - 1, 0, ctrl->nstp - 1);
                }
                for (j = 0; j < ctrl->nbr; j++)
                {
                    ...
                    if (ctrl->pool[i]->volZ[j].coef)
                        free_dmatrix(
                            ctrl->pool[i]->volZ[j].coef, 0, ctrl->maxmono - 1, 0, ctrl->nstp - 1);
                }
                free(ctrl->pool[i]);
            }
    free(ctrl->pool);
    ...
    if (ctrl->rho)
        free_f3tensor(ctrl->rho, 0, ctrl->nbr - 1, 0, ctrl->nbr - 1, 0, ctrl->nstp - 1);
...
Err sabr_test_get_d(double sigma, double alpha, double rho, double t, double** d, int nd)
{
    ...
    for (i = 0; i < C.nfac; i++)
    {
        ...
        for (ki = 0; ki < C.nbr; ki++)
        {
            ...
            {
                err = AddProdMonoPolyPoly(
                    &C, &pZ->Z[1], 1.0, C.rho[ki][kj], 0, &pZ->volZ[ki], &C.pool[hY0]->volZ[kj]);
                if (err)
                    goto FREE_RETURN;
            }
        }
...
Err sabr_test_get_all_d(double sigma, double alpha, double rho, double t, double **d, int nd)
{
        ...
        double			y[MAXNEQ];
        int				i, ki, kj;
        long			hY0, hX1, hX2, hY1V, hY1D, hY2V, hY2D, hY1VY1V, hY1VY1D, hY1DY1D,
hZ;
    ...
    double sigma, double gamma, double alpha, double rho, double t, double** d, int nd)
{
    ...
    if ((err = OldHash(&C, Y0, &hY0)) || (err = OldHash(&C, X1, &hX1)) ||
        ...
        (err = OldHash(&C, Y2D, &hY2D)) || (err = OldHash(&C, Y1VY1V, &hY1VY1V)) ||
        (err = OldHash(&C, Y1VY1D, &hY1VY1D)) || (err = OldHash(&C, Y1DY1D, &hY1DY1D)))
        goto FREE_RETURN;
...
Err sabr_test4_get_d(double sigma, double alpha, double rho, double t, double** d, int nd)
{
    ...
    if ((err = OldHash(&C, Y0, &hY0)) || (err = OldHash(&C, X1, &hX1)) ||
        ...
        (err = OldHash(&C, Y1Y2, &hY1Y2)) || (err = OldHash(&C, Y1Y1Y1, &hY1Y1Y1)) ||
        (err = OldHash(&C, X4, &hX4)) || (err = OldHash(&C, Y4, &hY4)) ||
        (err = OldHash(&C, Y2Y2, &hY2Y2)) || (err = OldHash(&C, Y1Y3, &hY1Y3)) ||
        (err = OldHash(&C, Y1Y1Y2, &hY1Y1Y2)) || (err = OldHash(&C, Y1Y1Y1Y1, &hY1Y1Y1Y1)))
        goto FREE_RETURN;
    ...
    double** vols)
{
    ...
    for (j = 0; j < nyk; j++)
    {
        ...
        if ((err = ord_mult_const(&s[0], 1.0 / eps)) || (err = ord_shift(&s[0], -1)))
            goto FREE_RETURN;
        ...
        memcpy(&ftmp, &s1s1, sizeof(SOrdSeries));
        if ((err = ord_mult_const(&ftmp, eps * eps / 8.0)) || (err = ord_shift(&ftmp, 2)) ||
            (err = ord_sum(&s[1], &ftmp)))
            goto FREE_RETURN;

        if ((err = ord_mult_const(&s[1], 1.0 / eps)) || (err = ord_shift(&s[1], -1)))
            goto FREE_RETURN;

...
{ 
	...
	y1=(log((barrier*barrier*fwd_price)/(spot*spot*strike))+vol*vol/2*mat)/(vol*sqrt(mat));
	y2=y1 - vol*sqrt(mat);
	ndy1 = norm(y1);
	ndy2 = norm(y2);
	...
	ndxx2 = norm(xx2);
        ...
        yy1=(log((barrier*fwd_price)/(spot*spot))+vol*vol/2*mat)/(vol*sqrt(mat));
...
{                                  /*   DELTA  FWD UP*/
	...
	answer =(srt_f_optonetou(	fwd_price+shift,
								spot,
		         				strike,
								barrier,
								vol,
...
{                                  /*   DELTA  SPOT DOWN*/
	shift = spot / 1000;
	answer = ( premium
			- srt_f_optonetou(	fwd_price-shift,
								spot-shift,
								strike,
								barrier,
								vol,
...
{                                  /*   VEGA    */
	shift = GVOPT.vol_add;
	answer = ( srt_f_optonetou(	fwd_price,
								spot,
								strike,
								barrier,
								vol + shift,

    ...
    SrtGreekType   greek)
{
    ...
    else
    {
        ...
        y1 = (log((barrier * barrier * fwd_price) / (spot * spot * strike)) + vol * vol / 2 * mat) /
             (vol * sqrt(mat));
        y2   = y1 - vol * sqrt(mat);
        ndy1 = norm(y1);
        ndy2 = norm(y2);
        ...
        if (call_put == SRT_CALL) /*  if call */
        {
            if (spot <= barrier && strike <= barrier)
            {
                /*     up & out     */
                premium =
                    fwd_price * (ndx1 - ndxx1) - strike * (ndx2 - ndxx2) -
                    fwd_price * pow(spot / barrier, -2 * chi) * (ndy1 - ndyy1) +
                    strike * pow(spot / barrier, -2 * chi + 2) * (ndy2 - ndyy2) /******MODIFIED***/
                    +
                    (barrier - strike) * (ndxx2 + pow(spot / barrier, -2 * chi + 2) * (1 - ndyy2));
                premium *= disc;
            }
        ...
        else if (call_put == SRT_PUT) /*  if put  */
        {
            if (spot >= barrier && strike >= barrier)
            {
                /*     down  & out     */
                premium =
                    -fwd_price * (1 - ndx1) + strike * (1 - ndx2) + fwd_price * (1 - ndxx1) -
                    strike * (1 - ndxx2) -
                    fwd_price * pow(spot / barrier, -2 * chi) * (ndy1 - ndyy1) +
                    strike * pow(spot / barrier, -2 * chi + 2) * (ndy2 - ndyy2) +
                    (strike - barrier) * (1 - ndxx2 + pow(spot / barrier, -2 * chi + 2) * ndyy2);
                premium *= disc;
            }
            else if (spot < barrier && strike >= barrier)
    ...
    { /*   DELTA  FWD UP*/
        ...
        answer = (srt_f_optonetou(
                      fwd_price + shift, spot, strike, barrier, vol, mat, disc, call_put, PREMIUM) -
                  premium) /
                 shift;
    }
    else if (greek == DELTA)
    { /*   DELTA  SPOT DOWN*/
        shift  = spot / 1000;
        answer = (premium - srt_f_optonetou(
                                fwd_price - shift,
                                spot - shift,
                                strike,
                                barrier,
    ...
    { /*   VEGA    */
        ...
        answer =
            (srt_f_optonetou(
                 fwd_price, spot, strike, barrier, vol + shift, mat, disc, call_put, PREMIUM) -
             premium);
    }
    else if (greek == THETA)
    { /*   THETA   */

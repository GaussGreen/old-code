/*-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+--+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-
        ...
        Description	: implementation of the Closed Form Solution of the European
Amortizing/Accreting Swaption.
    ...
    double* pdT_CashFlow_Begin)
{
    ...
    const double* pdCashFlow_End = pdCashFlow_Begin + nSize_FixPay + nSize_FltPay + nSize_FltStart +
                                   nSize_FixFee + nSize_ExFee;
    ...
    // fixed leg
    {
        for (; plFixPay_Begin < plFixPay_End; ++pdCashFlow_Begin,
                                              ++pdT_CashFlow_Begin,
                                              ++plFixPay_Begin,
                                              ++pdFixCoverage_Begin,
                                              ++pdFixNotional_Begin)
    ...
    double* pdG_Begin)
{
    ...
    if (pdLamT_End - pdLamT_Begin == 1)
    {
        ...
        export_lgmsetupG(*pdLam_Begin, pdGT_End - pdGT_Begin, (double*)pdGT_Begin, pdG, 0, 0, 0);
    ...
    double* pdG2_Begin)
{
    ...
    if (pdLamT_End - pdLamT_Begin == 1)
    {
        ...
        export_lgmsetupG2(
            *pdLam_Begin, dGamma, pdGT_End - pdGT_Begin, (double*)pdGT_Begin, pdG2, 0, 0, 0);

...
//
//	NB:	local vol is a function of time only;sgnature required for the PDE engine
//
//	Returns : as indicated
//-------------------------------------------------------------------------------------------------------
double __stdcall _SIGMA1_Sqrd_(const double *pdX1,const double *pdX3,const double *pdT, void* pArg)
{
	// cast 
...
//	Returns : as indicated
//-------------------------------------------------------------------------------------------------------
double __stdcall _SIGMA3_Sqrd_(const double *pdX1,const double *pdX3,const double *pdT, void* pArg)
{
...
//	Returns : as indicated
//-------------------------------------------------------------------------------------------------------
double __stdcall _MU1_BACKWARD_(const double *pdX1,const double *pdX3,const double *pdT,void* pArg)
{
	const _Model *pModel = (const _Model *)pArg;
	const int n = pdT-pModel->m_pdLocalT_Begin;
	_ASSERTE(n>=0 && n<(pModel->m_pdLocalT_End-pModel->m_pdLocalT_Begin));
...
//	Returns : as indicated
//-------------------------------------------------------------------------------------------------------
double __stdcall _MU1_FORWARD_(const double *pdX1,const double *pdX3,const double *pdT, void* pArg)
{
...
//	Returns : as indicated
//-------------------------------------------------------------------------------------------------------
double __stdcall _MU3_BACKWARD_(const double * pdX3,const double * pdX1,const double * pdT, void *pArg)
{
	// cast 
...
//	Returns : as indicated
//-------------------------------------------------------------------------------------------------------
double __stdcall _MU3_FORWARD_(const double *pdX3,const double *pdX1,const double *pdT, void *pArg)
{
...
//						for the LGM drift and vol functors
//
//	Returns : as indicated
//-------------------------------------------------------------------------------------------------------
static void _Prepare_CommPDE(
	const double *pdT_Begin,
	const double *pdT_End,
	const double *pdLocalLam_Begin,
	const double *pdLocalSig_Begin,
	void *pComm_PDE
	)
{
	...
	*(const double**)(&pComm->m_pdLocalLam_Begin)=pdLocalLam_Begin;//_alloca(nNumT * sizeof(double));
	*(const double**)(&pComm->m_pdLocalSig_Begin)=pdLocalSig_Begin;//_alloca(nNumT * sizeof(double));
	
	_bmatch_ex(
		pComm->m_pdLamT_Begin,
		sizeof(*pComm->m_pdLamT_Begin),
		pComm->m_pdLam_Begin,
		sizeof(*pComm->m_pdLam_Begin),
		pComm->m_pdLamT_End-pComm->m_pdLamT_Begin,
		pComm->m_pdLocalT_Begin,
		nNumT,
		(double*)pComm->m_pdLocalLam_Begin
		);

	_bmatch_ex(
		pComm->m_pdSigT_Begin,
		sizeof(*pComm->m_pdSigT_Begin),
		pComm->m_pdSig_Begin,
		sizeof(*pComm->m_pdSig_Begin),
...
//						U solves U(t,x1,x3)=E[Integrand(T,X(T))|X1(t)=x,X3(t)=x3]
//
//						NB: U traverses from *pdT_Begin to *(pdT_End-1)
//						To solve for the backward equation, pdT should be decreasing and for the forward equation, increasing.
//						
//
//	Returns : U(0,0)
//-------------------------------------------------------------------------------------------------------
const char *Price_Contract(
		/// time direction, n = pdTKnot_End- pdTKnot_Begin-1 intervals in total, i.e.
		/// pdTKnot_Begin[0]-pdTKnot_Begin[1], ... pdTKnot_Begin[n-1]-pdTKnot_Begin[n]
		const double *pdTKnot_Begin,
		const double *pdTKnot_End,
		// Grid 
		_Grid *pGrid,
		...
		)
{
	...
	if(nSize_T>=1)
	{
		...
		Max_Grid(pnSizeX1_Begin,pnSizeX1_Begin+nSize_T,pnSizeX3_Begin,pGrid,&nX1_Begin,&nX1_End,&nX3_Begin,&nX3_End);
	
		// alloca mem for the Grid
		if(szErr=_alloc_dmatrix(&ppdU,nX1_Begin,nX1_End,nX3_Begin,nX3_End)) return szErr;
		...
		if(store.m_pdata==0||store.m_pm==0) 
		{
			...
			// free matrix
			free_dmatrix(ppdU,nX1_Begin,nX1_End-1,nX3_Begin,nX3_End-1); // temporarily
		...
		for(;pdTKnot_Begin<(pdTKnot_End-1);++pdTKnot_Begin,++pnSizeX1_Begin,++pnSizeX3_Begin)
		{
			...
			_Prepare_CommPDE(pdLocalT_Begin,pdLocalT_End,pdLocalLam_Begin,pdLocalSig_Begin,pComm_PDE);
		...
		_ASSERTE( pGrid->m_nX3_Zero == 
			(pGrid->m_pdX3_Begin-pGrid->m_pdX3_Begin_+ (pGrid->m_pdX3_End-pGrid->m_pdX3_Begin)/2));
		_ASSERTE( pGrid->m_nX1_Zero == 
			(pGrid->m_pdX1_Begin-pGrid->m_pdX1_Begin_+ (pGrid->m_pdX1_End-pGrid->m_pdX1_Begin)/2));

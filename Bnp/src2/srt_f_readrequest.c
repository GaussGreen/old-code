	...
	for (j=0; j<nr; j++)
	{
		...
		/* -------------------- Vega request -------------------- */
	    	...
	    	if ( strcmp(requests[j],"VEGA") == 0 )   
	    	{
	    		...
	    		/* we create a first default request - since no shift value has been specified by the user */
	    		/* default shift = +0.01 */
		...
		/* ------------------------ RATE request ------------------- */
	    	...
	    	if ( strcmp(requests[j],"PARALLEL") == 0 )
	    	{
	    		...
	    		if (err = srt_f_IOstructsetrshift(  iolist,
					    	"PARALLEL",
					     	0.01,
					     	SH_ABSOLUTE, 
						SRT_NO, 
...
Err srt_f_sig_ts_dates(	int bucket_index,
					Date * date1,
					Date * date2,
					TermStruct ts)
{
...
*****/
    	...
    	for(ls = get_next_sigma_date(ts.head);
     		ls!=NULL;
...
Err srt_f_tau_ts_dates(	int bucket_index,
					Date * date1,
					Date * date2,
					TermStruct ts)
{
...
*****/
    	...
    	for(ls = get_next_tau_date(ts.head);
     		ls!=NULL;

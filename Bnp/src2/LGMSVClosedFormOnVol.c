...
void LGMSV_prod_comp(double	re1,
					  double	im1,
					  double	re2,
					  double	im2,
					  double	*re3,
...
void LGMSV_div_comp( double	re1,
					  double	im1,
					  double	re2,
					  double	im2,
					  double	*re3,
...
void LGMSV_sqr_comp(	double	re1,
						double	im1,
						double	*re3,
						double	*im3)
{
...
void LGMSVMomentInitOnVol(
					...
					LGMSVSolFunc	*FuncPsi,
					LGMSVSolFunc	*FuncPsi2,
					LGMSVSolFunc	*FuncV2,
...
int		dir;
	...
	{
		beta[iNumDff] = (1.0 - exp(-dLambdaX * (CouponTime[iNumDff] - dTStar))) / dLambdaX;
	}
	...
	for (iNumPsi=1; iNumPsi<=iNbPsi; iNumPsi++)
	{
		...
		for (iNumDrift=1; iNumDrift<=iNbDrift; iNumDrift++)
		{
			...
			*/
			...
			for (iNumDff=0;iNumDff<iNbCoupon;iNumDff++)
			{
				Payoff[iNumPsi][iNumDrift] += Coupon[iNumDff] * exp(-beta[iNumDff] * (Drift + 0.5 * beta[iNumDff] * Psi * rho2))
...
void LGMSVOptionPricePrecalcOnVol(	int		iNbPhi,
									int		iNbft,								
...
void LGMSVClosedFormOnVol(	
						...
						double	iNbSigmaPsiGridLeft,
						double	iNbSigmaPsiGridRight,
						double	iNbSigmaftLeft,
						double	iNbSigmaftRight,

						double	iRatioPsi,
						double	iRatioFt,
						int		iPriorityFreqPsi,
						int		iPriorityFreqFt,
						double	iMaxTime,
						int		SaveFile,
...
clock_t			time1, time2;
	...
	LGMSVMomentInitOnVol(0.0, dAlpha, dLambdaEps, dRho, FuncPsi, FuncPsi2, FuncV2, FuncPsiV);
	ExpectPsi = 0.0;
	ExpectPsi2 = 0.0;
	ExpevtV2 = 1.0;
	ExpectPsiV = 0.0;
	...
	for (i=0; i<=endi; i++)
	{
		...
		LGMSVMomentCalculationOnVol(sig1 * sig1, t2 - t1, ExpectPsi, ExpectPsi2, ExpevtV2, ExpectPsiV,
			FuncPsi, FuncPsi2, FuncV2, FuncPsiV, lambdaArray, &ExpectPsi, &ExpectPsi2, &ExpevtV2, &ExpectPsiV);
	...
	/* Find Frequence */		
	LGMSVFindFreqNew(iNbPsi, iNbDrift, AlphaEq, 0.0, endi, dt,
					Coef1Re, Coef2Re, Coef2ImT, Coef3ReT, Coef3ImT,
					dPsitMean, dPsitStd, sqrt(dPsitMean), iNbSigmaPsiGridLeft, iNbSigmaPsiGridRight, iNbSigmaftLeft, iNbSigmaftRight,
					LimitPsi, Limitft, iPriorityFreqPsi, iPriorityFreqFt,
					&dPsiFreqStep, &dPsiStep, &iIndexPsiMean, &dftFreqStep, &dDriftStep, &iIndexDriftMean);

	/* First pricing */
	LGMSVOptionPricePrecalcOnVol(iNbPsi, iNbDrift, dLambdaX, dRho, dTStar, endi, dt,
							Coef1Re, Coef2Re, Coef2ImT, Coef3ReT, Coef3ImT,
							dPsitMean, SaveFile, Density, Densityspeq, PayOff,
							dExTime, iNbCoupon, CouponTime, Coupon, 
							dPsiFreqStep, dPsiStep, iIndexPsiMean, dftFreqStep, dDriftStep, iIndexDriftMean, Price);
		
	
	/* Save the Payoff and Density */
	if (SaveFile)
	{
		LGMSVSaveDensity(iNbPsi,
						 iNbDrift,
						 iIndexPsiMean,						 
						 iIndexDriftMean,
						 dPsiStep,

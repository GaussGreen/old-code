...
the new LGMautocal directly from MAD and Westminster        , which can expose
...
The option holder can exercise only once        , and can exercise on any of the
dates tEx[0]        , tEx[1]        , ...        , tEx[nEx-1] which are on or
after tfirst.

Suppose the option holder exercises at tEx[j]        , and let tPay[i0] be the
first fixed leg paydate which is strictly after tStart[j]. Then the Receiver
gets the fixed payment stream Payment[i0] - RedFirstPay[j] paid at tPay[i0] ,
...
inter-period starts can be handled by adjusting Strike[j] and/or RedFirstPay[j]
      , depending on the deal        , and adjustments for the basis of the
floating leg can be handled by adjusting Strike[j] and Payment[i] */
...
pointed to by *variable. Specifically        , *LGMPtr        , *nExerBdry ,
*NsigPtr        , and *NtauPtr must point to space that has been allocated for
double        , long        , long        , and long which can receive the
outputs of the program;

*rArrExBdryPtr        , *tArrExBdryPtr        , *sigDateArrPtr        ,
*sigValArrPtr        , *tauDateArrPtr        , and *tauValArrPtr must point to
space the has been allocated for pointers to doubles and Dates; the program will
free any space which is being pointed to and reallocated it. Then *variable will
be set to the address of first element in the array of answers.
...
not been allocated with calloc        , malloc        , srt_calloc        , or
srt_malloc. Specifically        , if the arrays *rArrExBdryPtr        ,
*tArrExBdryPtr        , *sigDateArrPtr , *sigValArrPtr        , *tauDateArrPtr
, and *tauValArrPtr are not NULL        , they need to have been allocated with
calloc or malloc.

If findExerBdry is on        , the array *rArrExBdryPtr[i] gives the swap rate
at the exercise boundary for a swap that has exercise date *tArrExBdryPtr[i] and
ends at the final paydate tPay[nPay-1] of the underlying deal; all other
parameters (spot_lag        , daycountconv        , etc.) come from the currency
code.

If convertTS is on        , the program finds the sigma-tau term structure
equivalent to the calibrated zeta-G term structure        , and outputs it in
the arrays *sigDateArrPtr and *sigValArrPtr (of length *NsigPtr) and
*tauDateArrPtr and *tauValArrPtr (of length *NtauPtr).	*/
...
LGMErr TestLGMAmerCaller(
    ...
    Date tfixStart[], /* [0        ,1        ,...        ,nfix-1]  start dates
                         for fixed coupons */
    Date tfixEnd[],   /* [0        ,1        ,...        ,nfix-1]  end dates for
                         fixed coupons */
    Date tfixPay[],   /* [0        ,1        ,...        ,nfix-1]  pay dates for
                         fixed coupons */
    double fixFullPayment[], /* [0        ,1        ,...        ,nfix-1]  fixed
                          coupons (with notional at end) */
    double Strike[],  /* [0        ,1        ,...        ,nfix-1]  premium for
    ...
    Date tflt[], /* [0        ,1        ,...        ,nfltdates-1] all flting
              dates (first start date & all pay dates) */
    ...
    int calibrationmeth, /* 1 = fixexp        , 2=backboot        , 3=fixed
                            sigma */
    int strikechoice, /* 1 = IRR        , 2 = dIRR         , 3 = EMK1        , 4
                         = EMK2 */
                      /* task list */
    ...
    double *intrinsicValPtr) /* ptr to intrinsic value (NULL => not req'd) */
{
  return TestLGMAmerCallerRR(
      ...
      tfixStart, /* [0        ,1        ,...        ,nfix-1]  start dates for
                    fixed coupons */
      tfixEnd, /* [0        ,1        ,...        ,nfix-1]  end dates for fixed
                  coupons */
      tfixPay, /* [0        ,1        ,...        ,nfix-1]  pay dates for fixed
                  coupons */
      fixFullPayment, /* [0        ,1        ,...        ,nfix-1]  fixed coupons
                   (with notional at end) */
      Strike, /* [0        ,1        ,...        ,nfix-1]  premium for exer. at
               * j (with notional)
               */
      ...
      tflt, /* [0        ,1        ,...        ,nfltdates-1] all flting dates
      ...
      endofdayflag,  /* 1=too late to exercise deals today        , 0=not too
                        late */
      ...
      alpha, gamma, rho, calibrationmeth, /* 1 = fixexp        , 2=backboot ,
                                             3=fixed sigma */
      strikechoice, /* 1 = IRR        , 2 = dIRR         , 3 = EMK1        , 4 =
                       EMK2 */
      skipEval,     /* 1=calibrate only        , 0=calibrate & evaluate deal */
      convertTS,    /* 1=cmpute new sigmas        , taus & store in ts; 0=don't
                       bother */
    ...
    char *szRefRate)         /* optional refrate included for MUNI pricing */
{
  ...
  if ((szRefRate != 0) && (strcmp(szRefRate, "UTFBM3") == 0))
    ...
    error = LGMCcyDefaults(yldcrv,
                           &conv); /* get currency conventions        , lag */
                                   /* end modified code */
  ...
  /* create simple American stucture        , check input data        , & copy
  ...
  *LGMValPtr = LGMVal;
  ...
  /* If sigma-kappa term structure has been found        , copy it into output
   * arrays
   */
    ...
    double *intrinsicValPtr) /* ptr to intrinsic value (NULL => not req'd) */
{
  return TestLGMautocalCallerRR(
      ...
      tEx,    /* notification (exercise) dates        , [0        ,1        ,...
                 ,nEx-1] */
      tStart, /* start dates for each exercise        , [0        ,1        ,...
                 ,nEx-1] */
      Strike, /* total value paid at tStart[j] for fixed leg        , [0 ,1 ,...
                 ,nEx-1] */
      nPay,   /* nPay is number of fixed leg coupons */
      tPay,   /* pay dates for period i        , [0        ,1        , ...
                 ,nPay-1] */
      Payment, /* total fixed leg payment(last includes notional)        , [0
                  ,... ,nPay-1] */
...
LGMErr LGMCallInvFloaterCaller(
    ...
    Date *tStart, /* [0        ,1        ,...        ,nEx-1] settlement dates
                     for each exercise i.e. dates on which the fee is paid */
    double *exerFee, /* [0        ,1        ,...        ,nEx-1] fee paid by
                  option holder to exercise most frequently        , the bond is
                  called at par        , i.e. exerFee = 1.0 */
    char
...
- the exotic side also includes a cap paying max (0        , Libor CASH -
cap_str) * libor_cvg */

    long nCpn,       /* number of coupon periods */
    Date *tCpnStart, /* [0        ,1        ,...        ,nCpn-1] coupon start
                        date */
    Date
        *tCpnPay, /* [0        ,1        ,...        ,nCpn-1] coupon pay date */
    ...
    int LGMOneTwoFactor, int usefixtau, /* 1 = calibrate with fixed tau */
    int usecaps,  /* 1 = use caplets for calibration        , 0=use only
                     swaptions */
    double tau,   /* if fixed tau        , use this value for tau (in years) */
    ...
    int capletvolmethod, /* 1 = Model        , 2 = Sliding        , 3 =
                      Converging        , 4 = Calibrated */
    int strikechoice, /* 1 = Not used        , 2 = MidatStrike/MidatStrike , 3 =
                   MidatStrike/std        , 4= MidatStrike/ATM */
    double maxstd,    /* Maximum number of std between forward and strike */

    /*	Results	*/

    int convertTS,     /* 1 = cmpute new sigmas        , taus & store in ts; 0 =
                    don't     bother */
    ...
    FWD_VOL_STR *fwdVolStr)  /* ptr to fwd vol data (NULL => not req'd) */
{
  ...
  /* Allocate and fill deal pointer        , also check that there is a least
   * one exerceise date left */
...
Coupon leg:
        C[i] = cvg(t[i-1]        ,t[i]) * (margin[i] + lvg*max{rate[i]-amin[i]
,0} - lvg*max{rate[i]-amax[i]        ,0}) received at t[i] for i = 1        ,2
,...        ,n where rate[i] is the true (ie        , cash) floating rate for
the period t[i-1] to t[i]

Funding leg:
        cvg(tau[k-1]        , tau[k]) * rate2[k]	for k = 1        ,2 ,
...        , m where rate2[k] is the true (ie        , cash) floating rate for
the period tau[k-1] to tau[k]


Exercise (receiver):
        At any notification date tEx[j]        , j=0        ,1        , ... ,
nEx-1        , the holder can exercise. If exercised at tEx[j] then the
settlement date is t[i0-1]        , the next coupon date on or after tEx[j]. The
option owner pays any exercise fee f[j] at t[i0-1] receives all remaining
coupons C[i]        , i = i0        , i0+1        , ...        , n receives
notional ($1) at end date t[n] (this simplifies the funding leg) pays the
funding leg starting at t[i0-1]. If the start-upon-exercise date t[i0-1] is a
funding date tau[k-1]        , then the option owner receives the funding
payments at tau[k]        , tau[k+1]        , ...        , tau[m] receives the
notional ($1) at the end date The funding leg thus revalues to $1

If the start-upon-exercise date t[i0-1] is not a funding date        , so it
occurs somewhere in the middle of a funding period        , then there are small
adjustments that depend on whether the Early Flag and the ResetRate flags are
on.
...
interest cvg(tau[k-1]        ,t[i0-1]) * rate2[k] at t[i0-1]        , pays the
entire first funding payment cvg(tau[k-1]        ,tau[k]) * rate2[k] at tau[k]
continues to make the funding payments at tau[k+1]        , tau[k+2]        ,
...tau[m]
...
tau[k+2]        , ...tau[m] If the Reset Rate flag is on        , the rate is
assumed to have been reset for the first (stub) period. Otherwise the code will
use rfund_cur (or guess the rate) as before.

Exercise (payer):
        At any notification date tEx[j]        , j=0        ,1        , ... ,
nEx-1        , the holder can exercise. If exercised        , the option owner
pays an exercise fee f[j] pays the coupon leg (reverse of above) receives the
funding leg (reverse of above)
...
the coupon leg prior to calling the code. The internal model is on a CASH basis
      , with the FRA rates rate for t1 to t2 = [df(t1) - df(t2)]/cvg*df(t2)
being used in place of the floating rates. Before the code is called        ,
all basis spreads must be accounted for by (i) adjusting the funding leg margin
(before moving the margin to the coupon leg) (ii) adjusting the effective floor
value a[i] The start dates t[0] and tau[0] should be the same The end dates t[n]
and tau[m] should be the same There is no provision in the code for handling
overlapping start/end/pay dates on either leg Exercise cannot result in starting
at any date other than one of the coupon dates

Sign of the exercise fee: f[j]>0 means the exerciser pays the fee        ,
regardless of whether its a payer or receiver. THIS IS UNLIKE LGMautocal , where
the floating leg always paid the fee

Code automatically disregards any exercise dates that occur before today */
/* tCpn[i-1]        , tCpn[i]        , and tCpn[i] are the the start        ,
   end        , and pay date for period i        , i=1        ,...        ,nCpn
   amin[0]        , amax[0] margin[0] are undefined: amin[*        ,1        ,2
   ,...        ,nCpn]        , amax[*        ,1        ,2        ,... ,nCpn] ,
   margin[*        ,1 ,2        ,...        ,nCpn] tflt[j-1]        , tflt[j] ,
   tflt[j] are the start        , end        , and pay date for
   floating period j        , j=1        ,...        ,nflt */

LGMErr LGMCallCapFloaterCaller(
    /* definition of coupon leg */
    long nCpn,  /* number of coupon periods */
    Date *tCpn, /* t[0        ,1        ,...        ,nCpn]...cxxpn for t[i-1] to
                   t[i] paid at t[i] , i=1        ,...        ,nCpn	*/
    ...
    long nEx,           /* number of exercise dates */
    Date *tEx, /* [0        ,1        ,...        ,nEx-1] notification dates;
            deal starts on next coupon date */
    double *exerFee, /* [0        ,1        ,...        ,nEx-1] fee paid by
                  option holder to exercise */
    char *PayRecStr, /* RECEIVER or PAYER */
    ...
    SrtLgmTSData *lgmTSData) /* ptr to tau/sigma data (NULL => not req'd) */
{
  ...
  *LGMValPtr = 0.0;
  ...
  /* the next routine checks out the deal        , allocates the Bermudan on a
  inverse floater stucture        , and copies the input data into it */
...
LGMErr LGMFillSimAmer(Date tfirst, int standardLag, SrtSimAmer **AmerPtrPtr,
  ...
  if (tFirstExer >= LastFixing)
    return (
        "deal is a European swaption        , a swap        , or worthless");
...
/* This routine ASSUMES that if the option holder exercises on exer date tEx[j]
      , then he will receive fixed leg payments on all pay dates tPay[i] which
are strictly after the settlement date Start[j]. Except for the first pay date
after settlement        , the amount of the payment is Payment[i] for all i. The
first payment is reduced by RedFirstPay[j]. */
...
LGMErr LGMFillSimMidAt(Date tfirst, SrtSimMidAt **MidAtPtrPtr, long nPay,
  ...
  *MidAtPtrPtr = MidAtPtr; /* output is MidAtPtrPtr */
  ...
  /* for each exercise        , find first payment received if option is
   * exercised */
  for (j = 0; j < nEx; j++) {
    i = 0;
    ...
    int earlyFlag, int resetFlt)               /* early and reset flags */
{
  ...
  *nExleftPtr = nExEff;
  ...
  /* Step 5: Add funding leg to strike        , accounting for early and reset
   * flags
   */
...
vanilla option where the underlying has start date swapstart        , end date
swapend , and fixed rate Rfix. It should be replaced by a routine which records
the volatilities used by the options. Note that LGM calls the vols for reasons
not directly related to pricing        , meaning these vols will have zero vega.
Using sticky vols to record the vols will thus be more inefficient than using
these vols.
*/

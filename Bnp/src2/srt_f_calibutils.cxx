/* ------------------------------------------------------------------------
        FILE NAME: srt_f_calibutils.cxx
...
/* -----------------------------------------------------------------------
        From the input array of vols and taus (with dates)        , sets
   optimisation paremeters in a vector as required by Numerical Recipes in C All
   the arrays should already be initialised. The SigmaValues array should look
   like this (in input): [0]	[1]		[2]		[3] [4]
   [5] Date	Sig1	Beta1	Sig2	Beta2	Rho The TauValues array should
   look like this (in input): [0]	[1] [2] Date	Tau1	Tau2 The initial
   set of calibration parameters should look like this (in output): [ Sigmas ][
   Tau ] [ Beta ]  [ Omega] [ Alpha] [ Gamma ] [ Rho ]
    ----------------------------------------------------------------------- */

/* AS: for one factor        , eCalibType is supposed to be blind; forcing to
   blind in one factor case is done in srt_f_CalibrateAll
 */

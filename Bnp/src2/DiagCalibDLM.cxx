...
void constant_or_linear_interpolation_dlm(double *dates, double *values,
                                          int n_dates, double date_wanted,
                                          double *value_wanted) {
  int i = 0;
  double result;
...
static CPD_DIAG_CALIB_PARAM PARAML_LM, PARAMS_LM;

static int NCPNL_LM, NEXL_LM, *EX_CPNL_LM, *EX_ENDCPNL_LM,

    NCPNS_LM, NEXS_LM, *EX_CPNS_LM, *EX_ENDCPNS_LM,

    ONE2F_LM, NB_MOM_LM, INDEX_MOM[MAX_INST], BREAK_INST, FREQ_SHORT_LM,
    SHIFT_FREQ_LM,
...
static double *CPN_TIMEL_LM, *CPN_DFL_LM, *CPN_CVGL_LM, *EX_TIMEL_LM,
    *EX_STRIKEL_LM, *EX_PRICEL_LM, *EX_VEGAL_LM, CPN_GL_LM[MAX_CPN],
    CPN_G2L_LM[MAX_CPN], EX_GL_LM[MAX_CPN], EX_G2L_LM[MAX_CPN], *EX_ZETAL_LM,

    *CPN_TIMES_LM, *CPN_DFS_LM, *CPN_CVGS_LM, *EX_TIMES_LM, *EX_STRIKES_LM,
    *EX_WEIGHTS_LM, *EX_PRICES_LM, CPN_GS_LM[MAX_CPN], CPN_G2S_LM[MAX_CPN],
    EX_GS_LM[MAX_CPN], EX_G2S_LM[MAX_CPN], *EX_ZETAS_LM,

    ZETA2_LM[MAX_CPN], ZETA12_LM[MAX_CPN],

    *LAM_TIME_LM, *LAM_LM_INIT, LAM_LM[MAX_CPN], ALPHA_LM, GAMMA_LM, RHO_LM,

    PREC_LM,

    PRICE_LM[MAX_CPN], SENSI_LM[MAX_CPN][MAX_CPN];
...
void init_static_lgmprcapgiventauts_dlm(
    ...
    int ex_endcpnl[], double ex_strikel[], /*	Strikes */
    ...
    int ex_endcpns[], double ex_strikes[], /*	Strikes */
    ...
    CPD_DIAG_CALIB_PARAM paraml, CPD_DIAG_CALIB_PARAM params,
...
Err static_lgmprcapgiventauts_momentum_dlm(double index, double lam[],
  ...
  if (ind == 0) {
    ...
    if (ONE2F_LM == 1) {
      ...
      for (j = 0; j < nlam; j++) {
        ...
        static_lgmsetupG_tauts(nlam, LAM_TIME_LM, lambda, NCPNL_LM,
                               CPN_TIMEL_LM, CPN_GL_LM, NEXL_LM, EX_TIMEL_LM,
                               EX_GL_LM);
        static_lgmsetupG_tauts(nlam, LAM_TIME_LM, lambda, NCPNS_LM,
                               CPN_TIMES_LM, CPN_GS_LM, NEXS_LM, EX_TIMES_LM,
                               EX_GS_LM);

        err = lgmcalibzeta1F_tauts2(
            NCPNL_LM, CPN_TIMEL_LM, CPN_DFL_LM, CPN_CVGL_LM, CPN_GL_LM, NEXL_LM,
            EX_TIMEL_LM, EX_CPNL_LM, EX_ENDCPNL_LM, EX_GL_LM, EX_STRIKEL_LM,
            EX_PRICEL_LM, EX_VEGAL_LM, EX_ZETAL_LM, nlam, LAM_TIME_LM, lambda,
            0, PARAML_LM->precision, PARAML_LM->vega_prec, PARAML_LM->min_fact,
            PARAML_LM->max_fact, PARAML_LM->use_jumps);

        if (err) {
          return err;
        }

        static_interpolate_zeta(NEXL_LM, EX_TIMEL_LM, EX_ZETAL_LM, nlam,
                                LAM_TIME_LM, lambda, NEXS_LM, EX_TIMES_LM,
                                EX_ZETAS_LM);

        for (i = 0; i < NEXS_LM; i++) {
          if (!fmod(i + SHIFT_FREQ_LM, FREQ_SHORT_LM)) {
            SENSI_LM[i][j] =
                (lgmsopval1F(EX_ENDCPNS_LM[i] + 1 - EX_CPNS_LM[i],
                             &(CPN_DFS_LM[EX_CPNS_LM[i]]),
                             &(CPN_CVGS_LM[EX_CPNS_LM[i]]),
                             &(CPN_GS_LM[EX_CPNS_LM[i]]), EX_ZETAS_LM[i],
                             EX_GS_LM[i], EX_STRIKES_LM[i]) -
                 EX_PRICES_LM[i]);
          }
        }
      ...
      static_lgmsetupG_tauts(nlam, LAM_TIME_LM, lambda, NCPNL_LM, CPN_TIMEL_LM,
                             CPN_GL_LM, NEXL_LM, EX_TIMEL_LM, EX_GL_LM);
      static_lgmsetupG_tauts(nlam, LAM_TIME_LM, lambda, NCPNS_LM, CPN_TIMES_LM,
                             CPN_GS_LM, NEXS_LM, EX_TIMES_LM, EX_GS_LM);

      err = lgmcalibzeta1F_tauts2(
          NCPNL_LM, CPN_TIMEL_LM, CPN_DFL_LM, CPN_CVGL_LM, CPN_GL_LM, NEXL_LM,
          EX_TIMEL_LM, EX_CPNL_LM, EX_ENDCPNL_LM, EX_GL_LM, EX_STRIKEL_LM,
          EX_PRICEL_LM, EX_VEGAL_LM, EX_ZETAL_LM, nlam, LAM_TIME_LM, lambda, 0,
          PARAML_LM->precision, PARAML_LM->vega_prec, PARAML_LM->min_fact,
          PARAML_LM->max_fact, PARAML_LM->use_jumps);
      ...
      for (j = 0; j < nlam; j++) {
        ...
        static_lgmsetupG_tauts(nlam, LAM_TIME_LM, lambda, NCPNL_LM,
                               CPN_TIMEL_LM, CPN_GL_LM, NEXL_LM, EX_TIMEL_LM,
                               EX_GL_LM);
        static_lgmsetupG2_tauts(nlam, LAM_TIME_LM, lambda, GAMMA_LM, NCPNL_LM,
                                CPN_TIMEL_LM, CPN_G2L_LM, NEXL_LM, EX_TIMEL_LM,
                                EX_G2L_LM);

        static_lgmsetupG_tauts(nlam, LAM_TIME_LM, lambda, NCPNS_LM,
                               CPN_TIMES_LM, CPN_GS_LM, NEXS_LM, EX_TIMES_LM,
                               EX_GS_LM);
        static_lgmsetupG2_tauts(nlam, LAM_TIME_LM, lambda, GAMMA_LM, NCPNS_LM,
                                CPN_TIMES_LM, CPN_G2S_LM, NEXS_LM, EX_TIMES_LM,
                                EX_G2S_LM);

        err = lgmcalibzeta2F_tauts2(
            NCPNL_LM, CPN_TIMEL_LM, CPN_DFL_LM, CPN_CVGL_LM, CPN_GL_LM,
            CPN_G2L_LM, NEXL_LM, EX_TIMEL_LM, EX_CPNL_LM, EX_ENDCPNL_LM,
            EX_GL_LM, EX_G2L_LM, EX_STRIKEL_LM, EX_PRICEL_LM, EX_VEGAL_LM,
            EX_ZETAL_LM, nlam, LAM_TIME_LM, lambda, ALPHA_LM, GAMMA_LM, RHO_LM,
            0, PARAML_LM->precision, PARAML_LM->vega_prec, PARAML_LM->min_fact,
            PARAML_LM->max_fact, PARAML_LM->use_jumps);
        ...
        static_lgmcalczeta2zeta12_tauts(NEXS_LM, EX_TIMES_LM, EX_ZETAS_LM, nlam,
        ...
        for (i = 0; i < NEXS_LM; i++) {
          if (!fmod(i + SHIFT_FREQ_LM, FREQ_SHORT_LM)) {
            SENSI_LM[i][j] =
                (lgmsopval2F(EX_ENDCPNS_LM[i] + 1 - EX_CPNS_LM[i],
                             &(CPN_DFS_LM[EX_CPNS_LM[i]]),
                             &(CPN_CVGS_LM[EX_CPNS_LM[i]]),
                             &(CPN_GS_LM[EX_CPNS_LM[i]]),
                             &(CPN_G2S_LM[EX_CPNS_LM[i]]), EX_ZETAS_LM[i],
                             ZETA2_LM[i], ZETA12_LM[i], EX_GS_LM[i],
                             EX_G2S_LM[i], EX_STRIKES_LM[i]) -
                 EX_PRICES_LM[i]);
          }
        }
      ...
      err = lgmcalibzeta2F_tauts2(
          NCPNL_LM, CPN_TIMEL_LM, CPN_DFL_LM, CPN_CVGL_LM, CPN_GL_LM,
          CPN_G2L_LM, NEXL_LM, EX_TIMEL_LM, EX_CPNL_LM, EX_ENDCPNL_LM, EX_GL_LM,
          EX_G2L_LM, EX_STRIKEL_LM, EX_PRICEL_LM, EX_VEGAL_LM, EX_ZETAL_LM,
          nlam, LAM_TIME_LM, lambda, ALPHA_LM, GAMMA_LM, RHO_LM, 0,
          PARAML_LM->precision, PARAML_LM->vega_prec, PARAML_LM->min_fact,
          PARAML_LM->max_fact, PARAML_LM->use_jumps);
        ...
        if (!fmod(i + SHIFT_FREQ_LM, FREQ_SHORT_LM)) {
          PRICE_LM[i] =
              (lgmsopval2F(
                   ...
                   &(CPN_DFS_LM[EX_CPNS_LM[i]]), &(CPN_CVGS_LM[EX_CPNS_LM[i]]),
                   &(CPN_GS_LM[EX_CPNS_LM[i]]), &(CPN_G2S_LM[EX_CPNS_LM[i]]),
                   EX_ZETAS_LM[i], ZETA2_LM[i], ZETA12_LM[i], EX_GS_LM[i],
                   EX_G2S_LM[i], EX_STRIKES_LM[i]) -
               EX_PRICES_LM[i]);
        }
      }
    ...
    if (BREAK_INST) {
      ...
      for (j = 0; j < nlam; j++) {
        ...
        for (k = 0; k < NB_MOM_LM; k++) {
          ...
          SENSI_LM[NEXS_LM + k][j] =
              (temp - PRICE_LM[NEXS_LM + k]) / BUMP_LAM_LM;
...
Err static_lgmprcapgiventauts_dlm(double index, double lam[], double *price,
  ...
  if (ind - SHIFT_FREQ_LM == 0) {
    ...
    if (ONE2F_LM == 1) {
        ...
        for (i = 0; i < NEXS_LM; i++) {
          if (!fmod(i + SHIFT_FREQ_LM, FREQ_SHORT_LM)) {
            SENSI_LM[i][j] = lgmsopval1F(
                EX_ENDCPNS_LM[i] + 1 - EX_CPNS_LM[i],
                &(CPN_DFS_LM[EX_CPNS_LM[i]]), &(CPN_CVGS_LM[EX_CPNS_LM[i]]),
                &(CPN_GS_LM[EX_CPNS_LM[i]]), EX_ZETAS_LM[i], EX_GS_LM[i],
                EX_STRIKES_LM[i]);
          }
        }
      ...
      for (i = 0; i < NEXS_LM; i++) {
        if (!fmod(i + SHIFT_FREQ_LM, FREQ_SHORT_LM)) {
          PRICE_LM[i] = lgmsopval1F(EX_ENDCPNS_LM[i] + 1 - EX_CPNS_LM[i],
                                    &(CPN_DFS_LM[EX_CPNS_LM[i]]),
                                    &(CPN_CVGS_LM[EX_CPNS_LM[i]]),
                                    &(CPN_GS_LM[EX_CPNS_LM[i]]), EX_ZETAS_LM[i],
                                    EX_GS_LM[i], EX_STRIKES_LM[i]);
...
double solve_for_next_coef_dlm(double **res_iter, int nb_iter,
                               double premium_tgt,
...
static Err lgmprcapgiventauts_simpleNewton_dlm(double *lam, double *price,
                                               double *lam_sens) {
  int i, j, k, l;
  ...
  *lam_sens = 0.0;
  ...
  if (ONE2F_LM == 1) {
    ...
    static_lgmsetupG_tauts(NLAM_LM, LAM_TIME_LM, LAM_LM, NCPNL_LM, CPN_TIMEL_LM,
                           CPN_GL_LM, NEXL_LM, EX_TIMEL_LM, EX_GL_LM);
    static_lgmsetupG_tauts(NLAM_LM, LAM_TIME_LM, LAM_LM, NCPNS_LM, CPN_TIMES_LM,
                           CPN_GS_LM, NEXS_LM, EX_TIMES_LM, EX_GS_LM);

    err = lgmcalibzeta1F_tauts2(
        NCPNL_LM, CPN_TIMEL_LM, CPN_DFL_LM, CPN_CVGL_LM, CPN_GL_LM, NEXL_LM,
        EX_TIMEL_LM, EX_CPNL_LM, EX_ENDCPNL_LM, EX_GL_LM, EX_STRIKEL_LM,
        EX_PRICEL_LM, EX_VEGAL_LM, EX_ZETAL_LM, NLAM_LM, LAM_TIME_LM, LAM_LM, 0,
        PARAML_LM->precision, PARAML_LM->vega_prec, PARAML_LM->min_fact,
        PARAML_LM->max_fact, PARAML_LM->use_jumps);
    ...
    static_interpolate_zeta(NEXL_LM, EX_TIMEL_LM, EX_ZETAL_LM, NLAM_LM,
                            LAM_TIME_LM, LAM_LM, NEXS_LM, EX_TIMES_LM,
                            EX_ZETAS_LM);
    ...
    while (fabs(price1) > PARAMS_LM->precision && k < PARAMS_LM->nb_iter_max) {
      ...
      static_lgmsetupG_tauts(NLAM_LM, LAM_TIME_LM, LAM_LM, NCPNL_LM,
                             CPN_TIMEL_LM, CPN_GL_LM, NEXL_LM, EX_TIMEL_LM,
                             EX_GL_LM);
      static_lgmsetupG_tauts(NLAM_LM, LAM_TIME_LM, LAM_LM, NCPNS_LM,
                             CPN_TIMES_LM, CPN_GS_LM, NEXS_LM, EX_TIMES_LM,
                             EX_GS_LM);
      ...
      err = lgmcalibzeta1F_tauts2(
          NCPNL_LM, CPN_TIMEL_LM, CPN_DFL_LM, CPN_CVGL_LM, CPN_GL_LM, NEXL_LM,
          EX_TIMEL_LM, EX_CPNL_LM, EX_ENDCPNL_LM, EX_GL_LM, EX_STRIKEL_LM,
          EX_PRICEL_LM, EX_VEGAL_LM, EX_ZETAL_LM, NLAM_LM, LAM_TIME_LM, LAM_LM,
          0, PARAML_LM->precision, PARAML_LM->vega_prec, PARAML_LM->min_fact,
          PARAML_LM->max_fact, PARAML_LM->use_jumps);
      ...
      lam2 = solve_for_next_coef_gen(res_iter, k, 0.0, PARAMS_LM->precision, 2);
      ...
      if (lam2 < LIMIT_LAM_LM_DOWN || lam2 > LIMIT_LAM_LM_UP) {
        ...
        if (already_tried) {
          ...
          err = lgmcalibzeta1F_tauts2(
              NCPNL_LM, CPN_TIMEL_LM, CPN_DFL_LM, CPN_CVGL_LM, CPN_GL_LM,
              NEXL_LM, EX_TIMEL_LM, EX_CPNL_LM, EX_ENDCPNL_LM, EX_GL_LM,
              EX_STRIKEL_LM, EX_PRICEL_LM, EX_VEGAL_LM, EX_ZETAL_LM, NLAM_LM,
              LAM_TIME_LM, LAM_LM, 0, PARAML_LM->precision,
              PARAML_LM->vega_prec, PARAML_LM->min_fact, PARAML_LM->max_fact,
              PARAML_LM->use_jumps);
    ...
    static_lgmsetupG_tauts(NLAM_LM, LAM_TIME_LM, LAM_LM, NCPNL_LM, CPN_TIMEL_LM,
                           CPN_GL_LM, NEXL_LM, EX_TIMEL_LM, EX_GL_LM);
    static_lgmsetupG2_tauts(NLAM_LM, LAM_TIME_LM, LAM_LM, GAMMA_LM, NCPNL_LM,
                            CPN_TIMEL_LM, CPN_G2L_LM, NEXL_LM, EX_TIMEL_LM,
                            EX_G2L_LM);

    static_lgmsetupG_tauts(NLAM_LM, LAM_TIME_LM, LAM_LM, NCPNS_LM, CPN_TIMES_LM,
                           CPN_GS_LM, NEXS_LM, EX_TIMES_LM, EX_GS_LM);
    static_lgmsetupG2_tauts(NLAM_LM, LAM_TIME_LM, LAM_LM, GAMMA_LM, NCPNS_LM,
                            CPN_TIMES_LM, CPN_G2S_LM, NEXS_LM, EX_TIMES_LM,
                            EX_G2S_LM);

    err = lgmcalibzeta2F_tauts2(
        NCPNL_LM, CPN_TIMEL_LM, CPN_DFL_LM, CPN_CVGL_LM, CPN_GL_LM, CPN_G2L_LM,
        NEXL_LM, EX_TIMEL_LM, EX_CPNL_LM, EX_ENDCPNL_LM, EX_GL_LM, EX_G2L_LM,
        EX_STRIKEL_LM, EX_PRICEL_LM, EX_VEGAL_LM, EX_ZETAL_LM, NLAM_LM,
        LAM_TIME_LM, LAM_LM, ALPHA_LM, GAMMA_LM, RHO_LM, 0,
        PARAML_LM->precision, PARAML_LM->vega_prec, PARAML_LM->min_fact,
        PARAML_LM->max_fact, PARAML_LM->use_jumps);
    ...
    static_lgmcalczeta2zeta12_tauts(NEXS_LM, EX_TIMES_LM, EX_ZETAS_LM, NLAM_LM,
                                    LAM_TIME_LM, LAM_LM, ALPHA_LM, GAMMA_LM,
                                    RHO_LM, ZETA2_LM, ZETA12_LM);
    ...
    while (fabs(price1) > PARAMS_LM->precision && k < PARAMS_LM->nb_iter_max) {
      ...
      err = lgmcalibzeta2F_tauts2(
          ...
          CPN_G2L_LM, NEXL_LM, EX_TIMEL_LM, EX_CPNL_LM, EX_ENDCPNL_LM, EX_GL_LM,
          EX_G2L_LM, EX_STRIKEL_LM, EX_PRICEL_LM, EX_VEGAL_LM, EX_ZETAL_LM,
          NLAM_LM, LAM_TIME_LM, LAM_LM, ALPHA_LM, GAMMA_LM, RHO_LM, 0,
          PARAML_LM->precision, PARAML_LM->vega_prec, PARAML_LM->min_fact,
          PARAML_LM->max_fact, PARAML_LM->use_jumps);
      ...
      static_lgmcalczeta2zeta12_tauts(NEXS_LM, EX_TIMES_LM, EX_ZETAS_LM,
                                      NLAM_LM, LAM_TIME_LM, LAM_LM, ALPHA_LM,
                                      GAMMA_LM, RHO_LM, ZETA2_LM, ZETA12_LM);
      ...
      if (lam2 < LIMIT_LAM_LM_DOWN || lam2 > LIMIT_LAM_LM_UP) {
        ...
        if (already_tried) {
          ...
          static_lgmsetupG_tauts(NLAM_LM, LAM_TIME_LM, LAM_LM, NCPNL_LM,
                                 CPN_TIMEL_LM, CPN_GL_LM, NEXL_LM, EX_TIMEL_LM,
                                 EX_GL_LM);
          static_lgmsetupG2_tauts(NLAM_LM, LAM_TIME_LM, LAM_LM, GAMMA_LM,
                                  NCPNL_LM, CPN_TIMEL_LM, CPN_G2L_LM, NEXL_LM,
                                  EX_TIMEL_LM, EX_G2L_LM);

          static_lgmsetupG_tauts(NLAM_LM, LAM_TIME_LM, LAM_LM, NCPNS_LM,
                                 CPN_TIMES_LM, CPN_GS_LM, NEXS_LM, EX_TIMES_LM,
                                 EX_GS_LM);
          static_lgmsetupG2_tauts(NLAM_LM, LAM_TIME_LM, LAM_LM, GAMMA_LM,
                                  NCPNS_LM, CPN_TIMES_LM, CPN_G2S_LM, NEXS_LM,
                                  EX_TIMES_LM, EX_G2S_LM);

          err = lgmcalibzeta2F_tauts2(
              NCPNL_LM, CPN_TIMEL_LM, CPN_DFL_LM, CPN_CVGL_LM, CPN_GL_LM,
              CPN_G2L_LM, NEXL_LM, EX_TIMEL_LM, EX_CPNL_LM, EX_ENDCPNL_LM,
              EX_GL_LM, EX_G2L_LM, EX_STRIKEL_LM, EX_PRICEL_LM, EX_VEGAL_LM,
              EX_ZETAL_LM, NLAM_LM, LAM_TIME_LM, LAM_LM, ALPHA_LM, GAMMA_LM,
              RHO_LM, 0, PARAML_LM->precision, PARAML_LM->vega_prec,
              PARAML_LM->min_fact, PARAML_LM->max_fact, PARAML_LM->use_jumps);
  ...
  *price = price2;
  ...
  solve_for_next_sensi(res_iter, k, lam2, 0.0005, lam_sens);

FREE_RETURN:

  if (res_iter)
    free_dmatrix(res_iter, 0, PARAMS_LM->nb_iter_max, 0, 1);
...
Err lgmcalibzetalambda_tauts_dlm(
  ...
  if (fix_lambda) {
    err = lgmprcapgivenlambda_tauts2(
        ncpnl, cpn_timel, cpn_dfl, cpn_cvgl, nexl, ex_timel, ex_cpnl, ex_ncpnl,
        ex_strikel, ex_pricel, ex_vegal, ex_ncpns, ex_strikes, ex_zeta, one2F,
        nlam, lam_time, lam, alpha, gamma, rho, 0, paraml->precision,
        paraml->vega_prec, paraml->min_fact, paraml->max_fact,
        paraml->use_jumps, 0, ex_spriceres);
    ...
    if (lm_params->use_moment) {
      ...
      */
      ...
      init_static_lgmprcapgiventauts_dlm(
          ncpnl, cpn_timel, cpn_dfl, cpn_cvgl, nexl, ex_timel, ex_cpnl,
          ex_ncpnl, ex_strikel, ex_pricel, ex_vegal, ex_zeta, ncpns, cpn_times,
          cpn_dfs, cpn_cvgs, nexs, ex_times, ex_cpns, ex_ncpns, ex_strikes,
          weight, ex_prices, ex_zetas, one2F, nlam, lam_time, lam, alpha, gamma,
          rho, paraml, params, lm_params);
      ...
      if (nlam == 1)
      {
              err = lgmprcapgiventauts_simpleNewton_dlm(lambda        ,
      &fitting_error);
      ...
      */
      ...
      init_static_lgmprcapgiventauts_dlm(
          ...
          ex_ncpnl, ex_strikel, ex_pricel, ex_vegal, ex_zeta, ncpns, cpn_times,
          cpn_dfs, cpn_cvgs, nexs, ex_times, ex_cpns, ex_ncpns, ex_strikes,
          weight, ex_prices, ex_zetas, one2F, nlam, lam_time, lambda, alpha,
          gamma, rho, paraml, params, lm_params);

      err = levenberg_marquardt(
          data, target, weight, nexs / lm_params->freq_short, lambda, nlam,
          lm_params->nb_iter, static_lgmprcapgiventauts_dlm, &fitting_error);
...
Err Construct_MultiSchedule(
    ...
    long *ncpn, long *cpn_date, double *cpn_time, double *cpn_cvg,
    double *cpn_df, long *theo_end_dates, long *act_end_dates) {
  int i;
...
Err Reduct_ExerciseDates(long today,

                         int old_num_ex_date, /*	Exercise dates */
                         long *old_ex_date,   /*	Supposed to be sorted */
                         double *old_strike, double *old_weight,
                         long *old_theo_end_dates, long *old_act_end_dates,
                         int ncpn, long *cpn_date,

                         int *cal_date, CPD_DIAG_CALIB_PARAM param,

                         int *num_ex_date, long **ex_date, double **strike,
                         double **weight, long **theo_end_dates,
                         long **act_end_dates) {
  int i, j, k, l;
  long min_cal_date;
  Err err = NULL;
  ...
  if (old_weight && weight)
    memcpy((*weight), old_weight, old_num_ex_date * sizeof(double));
...
Err shift_the_vol(double dFwd, double dStrike, double dMaturity,
                  double dInputVol, double dInputVolType,

                  double dVolShift, DIAGCALIB_VOLTYPE eShiftVolType,
                  DIAGCALIB_SHIFTTYPE eShiftType, double *dOutputVol,
  ...
  if (fabs(dVolShift) < 1.0E-08 || eShiftVolType == LGM_VOL) {
    ...
    *dOutputVolType = dInputVolType;
    ...
    if (dInputVolType < 0.5 && eShiftVolType == LOGNORMAL_VOL) {
      dPrice = srt_f_optblknrm(dFwd, dStrike, dInputVol, dMaturity, 1.0,
                               SRT_PUT, PREMIUM);

      err = srt_f_optimpvol(dPrice, dFwd, dStrike, dMaturity, 1.0, SRT_PUT,
                            SRT_LOGNORMAL, &dInputVol);

      if (err)
        return err;
    } else if (dInputVolType > 0.5 && eShiftVolType == NORMAL_VOL) {
      dPrice = srt_f_optblksch(dFwd, dStrike, dInputVol, dMaturity, 1.0,
                               SRT_PUT, PREMIUM);

      err = srt_f_optimpvol(dPrice, dFwd, dStrike, dMaturity, 1.0, SRT_PUT,
                            SRT_NORMAL, &dInputVol);
...
Err Calculate_Underlying(
    long today, char *yc_name, /*	Name of the yield curve */
    ...
    char *vol_ref_rate_name, char *instr_freq, char *instr_basis,
    char *ref_rate_name,

    CPD_DIAG_CALIB_PARAM param,

    long ncpn, long *cpn_date, double *cpn_time, double *cpn_cvg,
    double *cpn_df, int num_ex_dates, long *ex_date, long *theo_end_dates,
    long *act_end_dates, double *strike, double *weights, int vega_weight,

    /* shift informations */
    int num_vol_shift, double *vol_shift_time, double *vol_shift,

    /* information in the case of the short instruments */
    int num_ex_primdates, double *ex_primtime, double *ex_primnstd,

    double *ex_time, long *ex_startcpn, long *ex_endcpn, double *ex_lvl,
    double *ex_fwd, double *ex_strike, double *ex_weights, double *ex_price,
    double *ex_atm_price, double *ex_vega, double *ex_vol, double *ex_nstd) {
  int i, j, k, l;
  ...
  for (i = 0; i < num_ex_dates; i++) {
    ...
    if (param->transform_vol)
      err = swp_f_transformvol_func(
          yc_name, vol_curve_name, get_cash_vol, szStdFreq, szStdBasis,
          vol_ref_rate_name,
          (double)add_unit(ex_date[i], 2, SRT_BDAY, MODIFIED_SUCCEEDING),
          (double)theo_end_dates[i], ex_fwd[i], instr_freq, instr_basis, &std,
          &power);
    else
      err = get_cash_vol(vol_curve_name,
                         add_unit(ex_date[i], 2, SRT_BDAY, MODIFIED_SUCCEEDING),
                         theo_end_dates[i], ex_fwd[i], 0, vol_ref_rate_name,
                         &std, &power);
    ...
    if (vol_shift_time && vol_shift) {
      ...
      constant_or_linear_interpolation_dlm(vol_shift_time, vol_shift,
                                           num_vol_shift, ex_time[i], &shift);
    } else {
      shift = 0.0;
    }

    err = shift_the_vol(ex_fwd[i], ex_fwd[i], ex_time[i], std, power,
                        param->vol_shift + shift, param->vol_type,
                        param->shift_type, &std, &power);

    if (err)
      goto FREE_RETURN;

    if (power > 0.5) {
      power = srt_f_optblksch(ex_fwd[i], ex_fwd[i], std, ex_time[i], 1.0,
                              SRT_CALL, PREMIUM);

      err = srt_f_optimpvol(power, ex_fwd[i], ex_fwd[i], ex_time[i], 1.0,
                            SRT_CALL, SRT_NORMAL, &std);
    }

    if (ex_atm_price) {
      ex_atm_price[i] = srt_f_optblknrm(ex_fwd[i], ex_fwd[i], std, ex_time[i],
                                        ex_lvl[i], SRT_PUT, PREMIUM);
    }

    std *= sqrt(ex_time[i]);

    /*	Strike */
    if (param->strike_type == 0 || param->strike_type == 5 ||
        (strike[i] < 1.0e-04 && param->strike_type != 4)) {
    ...
    } else if (param->strike_type == 4) {
      ex_strike[i] =
          ex_fwd[i] +
          ex_primnstd[Get_Index(ex_time[i], ex_primtime, num_ex_primdates)] *
              std;
    }
    ...
    if (param->transform_vol)
      err = swp_f_transformvol_func(
          ...
          vol_ref_rate_name,
          (double)add_unit(ex_date[i], 2, SRT_BDAY, MODIFIED_SUCCEEDING),
          (double)theo_end_dates[i], ex_strike[i], instr_freq, instr_basis,
          &(ex_vol[i]), &power);
    else
      err = get_cash_vol(vol_curve_name,
                         add_unit(ex_date[i], 2, SRT_BDAY, MODIFIED_SUCCEEDING),
                         theo_end_dates[i], ex_strike[i], 0, vol_ref_rate_name,
                         &(ex_vol[i]), &power);
    ...
    err = shift_the_vol(ex_fwd[i], ex_strike[i], ex_time[i], ex_vol[i], power,
                        param->vol_shift + shift, param->vol_type,
                        param->shift_type, &(ex_vol[i]), &power);

    if (err)
      goto FREE_RETURN;

    if (power > 0.5) {
      ex_price[i] = srt_f_optblksch(ex_fwd[i], ex_strike[i], ex_vol[i],
                                    ex_time[i], ex_lvl[i], SRT_PUT, PREMIUM);

      ex_vega[i] = srt_f_optblksch(ex_fwd[i], ex_strike[i], ex_vol[i],
                                   ex_time[i], ex_lvl[i], SRT_PUT, VEGA);
    } else {
      ex_price[i] = srt_f_optblknrm(ex_fwd[i], ex_strike[i], ex_vol[i],
                                    ex_time[i], ex_lvl[i], SRT_PUT, PREMIUM);

      ex_vega[i] = srt_f_optblknrm(ex_fwd[i], ex_strike[i], ex_vol[i],
                                   ex_time[i], ex_lvl[i], SRT_PUT, VEGA);
    }
...
//// 	Now save market instruments prices in the inst_data structure PMc
///17Nov03
////
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~//
Err cpd_calib_diagonal_dlm(
    ...
    /*	Shift Parameters */
    int num_long_vol_shift, double *long_vol_shift_time, double *long_vol_shift,

    int num_short_vol_shift, double *short_vol_shift_time,
        ...
        inst_data) /*	NULL = don't save calibration instrument data */
{
  ...
  double ex_timel[MAX_INST], ex_fwdl[MAX_INST], ex_lvll[MAX_INST],
      ex_strikel[MAX_INST], ex_voll[MAX_INST], ex_pricel[MAX_INST],
      ex_atm_pricel[MAX_INST], ex_vegal[MAX_INST], ex_atm_vegal[MAX_INST],
      ex_nstdl[MAX_INST],

      ex_times[MAX_INST], ex_fwds[MAX_INST], ex_lvls[MAX_INST],
      ex_strikes[MAX_INST], ex_weights[MAX_INST], ex_prices[MAX_INST],
      ex_atm_prices[MAX_INST], ex_vols[MAX_INST], ex_vegas[MAX_INST],
      ex_nstds[MAX_INST],

      ex_zeta[MAX_INST], ex_zeta_temp[MAX_INST], ex_G_temp[MAX_INST];

  int ex_startcpnl[MAX_INST], ex_endcpnl[MAX_INST], ex_startcpns[MAX_INST],
      ex_endcpns[MAX_CPN];

  long cpn_datel[MAX_CPN], cpn_dates[MAX_CPN];

  double cpn_timel[MAX_CPN], cpn_cvgl[MAX_CPN], cpn_dfl[MAX_CPN],
      cpn_times[MAX_CPN], cpn_cvgs[MAX_CPN], cpn_dfs[MAX_CPN],
      cpn_G_temp[MAX_CPN];
  ...
  double *used_strikel, *used_pricel, *used_vegal, *used_strikes, *used_prices;
  ...
  *sig = NULL;
  ...
  err = Construct_MultiSchedule(
      ...
      ex_datel_, end_tenorl, end_datel, &ncpnl, cpn_datel, cpn_timel, cpn_cvgl,
      cpn_dfl, theo_end_datesl, act_end_datesl);

  if (err) {
    goto FREE_RETURN;
  }

  err = Reduct_ExerciseDates(today, num_ex_datesl, ex_datel_, strikel_, NULL,
                             theo_end_datesl, act_end_datesl, ncpnl, cpn_datel,
                             cal_datel, paraml, &num_ex_datesl, &ex_datel,
                             &strikel, NULL, &theo_end_datesl, &act_end_datesl);
  ...
  err = Calculate_Underlying(
      today, yc_name, vol_curve_name, get_cash_vol, vol_ref_rate_name,
      instr_long_freq, instr_long_basis, long_ref_rate_name, paraml, ncpnl,
      cpn_datel, cpn_timel, cpn_cvgl, cpn_dfl, num_ex_datesl, ex_datel,
      theo_end_datesl, act_end_datesl, strikel, NULL, 0,

      num_long_vol_shift, long_vol_shift_time, long_vol_shift,

      num_ex_datesl, ex_timel, ex_nstdl,

      ex_timel, ex_startcpnl, ex_endcpnl, ex_lvll, ex_fwdl, ex_strikel, NULL,
      ex_pricel, ex_atm_pricel, ex_atm_vegal, ex_voll, ex_nstdl);
  ...
  if (!fix_lambda || (one_factor_equi && one2F == 2)) {
    err = Construct_MultiSchedule(
        ...
        ex_dates_, end_tenors, end_dates, &ncpns, cpn_dates, cpn_times,
        cpn_cvgs, cpn_dfs, theo_end_datess, act_end_datess);
    ...
    err = Reduct_ExerciseDates(
        today, num_ex_datess, ex_dates_, strikes_, weights_, theo_end_datess,
        act_end_datess, ncpns, cpn_dates, cal_dates, params, &num_ex_datess,
        &ex_dates, &strikes, &weights, &theo_end_datess, &act_end_datess);
    ...
    err = Calculate_Underlying(
        today, yc_name, vol_curve_name, get_cash_vol, vol_ref_rate_name,
        instr_short_freq, instr_short_basis, short_ref_rate_name, params, ncpns,
        cpn_dates, cpn_times, cpn_cvgs, cpn_dfs, num_ex_datess, ex_dates,
        theo_end_datess, act_end_datess, strikes, weights,
        lm_params->vega_weight,

        num_short_vol_shift, short_vol_shift_time, short_vol_shift,

        num_ex_datesl, ex_timel, ex_nstdl,

        ex_times, ex_startcpns, ex_endcpns, ex_lvls, ex_fwds, ex_strikes,
        ex_weights, ex_prices, ex_atm_prices, ex_vegas, ex_vols, ex_nstds);
  ...
  if (one_factor_equi && one2F == 2 && fix_lambda) {
    ...
    err = lgmcalibzetalambda_tauts_dlm(
        ncpnl, cpn_timel, cpn_dfl, cpn_cvgl, num_ex_datesl, ex_timel,
        ex_startcpnl, ex_endcpnl, ex_strikel, ex_pricel, ex_vegal,

        ncpns, cpn_times, cpn_dfs, cpn_cvgs, num_ex_datess, ex_times,
        ex_startcpns, ex_endcpns, ex_strikes, ex_weights, ex_prices, ex_vegas,

        ex_zeta, fix_lambda, nlam, lam_time, lam, 1, alpha, gamma, rho, paraml,
        params, lm_params, &lam_sens);
    ...
    static_interpolate_zeta(num_ex_datesl, ex_timel, ex_zeta, nlam, lam_time,
                            lam, num_ex_datess, ex_times, ex_zeta_temp);
    static_lgmsetupG_tauts(nlam, lam_time, lam, ncpns, cpn_times, cpn_G_temp,
                           num_ex_datess, ex_times, ex_G_temp);

    for (i = 0; i < num_ex_datess; i++) {
      ex_prices[i] = lgmsopval1F(
          ex_endcpns[i] - ex_startcpns[i] + 1, &(cpn_dfs[ex_startcpns[i]]),
          &(cpn_cvgs[ex_startcpns[i]]), &(cpn_G_temp[ex_startcpns[i]]),
          ex_zeta_temp[i], ex_G_temp[i], ex_strikes[i]);
    }
  ...
  for (i = 0; i < nlam; i++) {
    if ((lam_shift && fabs(lam_shift[i]) > 1.0E-08) ||
        (params && fabs(params->lambda_shift) > 1.0E-08)) {
      shift_lam = 1;
      break;
    }
  }
  ...
  if (!fix_lambda && (params->strike_type == 5 || shift_lam)) {
    ...
    err = lgmcalibzetalambda_tauts_dlm(
        ncpnl, cpn_timel, cpn_dfl, cpn_cvgl, num_ex_datesl, ex_timel,
        ex_startcpnl, ex_endcpnl, used_strikel, used_pricel, used_vegal,

        ncpns, cpn_times, cpn_dfs, cpn_cvgs, num_ex_datess, ex_times,
        ex_startcpns, ex_endcpns, used_strikes, ex_weights, used_prices,
        ex_vegas,

        ex_zeta, fix_lambda, nlam, lam_time, lam, one2F, alpha, gamma, rho,
        paraml, params, lm_params, &lam_sens);
  ...
  *sig = (double *)calloc(num_ex_datesl, sizeof(double));
  ...
  for (i = 1; i < num_ex_datesl; i++) {
    ...
    if (ex_zeta[i] > ex_zeta[i - 1]) {
      exp_fact = static_lgmcalcexpfact_tauts(ex_timel[i - 1], ex_timel[i], nlam,
                                             lam_time, lam);
  ...
  if (inst_data) {
    ...
    for (i = 0; i < num_ex_datesl; i++) {
      inst_data->exer_dates_long[i] =
          (long)(ex_timel[i] * DAYS_IN_YEAR + EPS_MAT_TO_LONG_DATE_CONVERSION) +
          today;
    ...
    if (!fix_lambda) {
      for (i = 0; i < num_ex_datess; i++) {
        inst_data->exer_dates_short[i] =
            (long)(ex_times[i] * DAYS_IN_YEAR +
                   EPS_MAT_TO_LONG_DATE_CONVERSION) +
            today;
...
Err AllocateCalibInst(int iNbStrike, CALIBINSTRUMENTDLM CalibInst) {
  Err err = NULL;
...
void FreeCalibInst(CALIBINSTRUMENTDLM CalibInst) {
  if (CalibInst) {
    if (CalibInst->dStrike)
      free(CalibInst->dStrike);
    CalibInst->dStrike = NULL;

    if (CalibInst->dPrice)
      free(CalibInst->dPrice);
    CalibInst->dPrice = NULL;

    if (CalibInst->dNormalVol)
      free(CalibInst->dNormalVol);
    CalibInst->dNormalVol = NULL;

    if (CalibInst->dVega)
      free(CalibInst->dVega);
    CalibInst->dVega = NULL;

    if (CalibInst->dNbStd)
      free(CalibInst->dNbStd);
    CalibInst->dNbStd = NULL;

    if (CalibInst->dModelPrice)
      free(CalibInst->dModelPrice);
    CalibInst->dModelPrice = NULL;
  }
}

Err AllocateAllCalibInst(int iNbInst, int iNbStrike,
  ...
  for (i = 0; i < iNbInst; i++) {
    err = AllocateCalibInst(iNbStrike, &(AllCalibInst->sCalibInst[i]));
    if (err) {
      return err;
    }
  }
...
void AllocateCalibExeSchedule(long *lExeDates, double *dExeTimes,
                              long *lStartDates, long *lTheoEndDates,
                              long *lActEndDates, double *dStrikes,
                              double *dStrikesS1, double *dStrikesS2,
                              double *dWeights,
...
Err Construct_CalibSchedule(
  ...
  for (i = 0; i < num_ex_dates; i++) {
    err = get_end_date(ex_date[i], end_date, end_tenor[i], 0,
                       &(CalibExeSchedule->lTheoEndDates[i]));
    if (err) {
      goto FREE_RETURN;
    }
    CalibExeSchedule->lActEndDates[i] = bus_date_method(
        CalibExeSchedule->lTheoEndDates[i], MODIFIED_SUCCEEDING);
  ...
  for (i = 0; i < j; i++) {
    if (cal_date && cal_date[i]) {
      if (CalibExeSchedule->lTheoEndDates[i] > theo_date ||
          CalibExeSchedule->lActEndDates[i] > act_date) {
...
Err Reduct_ExeSchedule(CALIBCPNSCHEDULEDLM CalibCpnSchedule,
                       CALIBEXESCHEDULEDLM CalibExeSchedule, int *cal_date,
                       CPD_DIAG_CALIB_PARAM param) {
  int i, j, k, l;
  Err err = NULL;
  ...
  for (i = (CalibExeSchedule->iNExe) - 1; i >= 0; i--) {
    if (cal_date[i] == 0) {
      for (k = i - 1; k >= 0; k--) {
        ...
        (CalibExeSchedule->lTheoEndDates)[k + 1] =
            (CalibExeSchedule->lTheoEndDates)[k];
        (CalibExeSchedule->lActEndDates)[k + 1] =
            (CalibExeSchedule->lActEndDates)[k];
      ...
      if (CalibExeSchedule->dStrikesS1)
        (CalibExeSchedule->dStrikesS1)++;
      if (CalibExeSchedule->dStrikesS2)
        (CalibExeSchedule->dStrikesS2)++;
      if (CalibExeSchedule->dWeights)
        (CalibExeSchedule->dWeights)++;
  ...
  /*	Remove short maturities */
  while (CalibExeSchedule->iNExe > 0 &&
         CalibExeSchedule->dExeTimes[0] < param->min_calib_time) {
  ...
  for (i = (CalibExeSchedule->iNExe) - 1; i >= 0; i--) {
    while (j > 0 &&
           CalibCpnSchedule->lCpnDate[j] > (CalibExeSchedule->lExeDates)[i]) {
      j--;
    }
    if (CalibCpnSchedule->lCpnDate[j] < (CalibExeSchedule->lExeDates)[i] &&
        j < CalibCpnSchedule->iNCpn - 1) {
      j++;
    }
  ...
  for (i = (CalibExeSchedule->iNExe) - 2; i >= param->keep_first; i--) {
    if (((CalibExeSchedule->lExeDates)[j] - (CalibExeSchedule->lExeDates)[i]) *
            YEARS_IN_DAY <
        param->min_time - ONE_MONTH) {
      for (k = i - 1; k >= 0; k--) {
        (CalibExeSchedule->lExeDates)[k + 1] = (CalibExeSchedule->lExeDates)[k];
...
Err Calculate_CalibInst(
    ...
    CALIBCPNSCHEDULEDLM CalibCpnSchedule, CALIBEXESCHEDULEDLM CalibExeSchedule,
  ...
  for (i = 0; i < AllCalibInst->iNbInst; i++) {
    ...
    if (CalibExeSchedule->lStartDate[i] == 0) {
      start_date = add_unit(CalibExeSchedule->lExeDates[i], 2, SRT_BDAY,
                            MODIFIED_SUCCEEDING);
    } else {
    ...
    while (j < CalibCpnSchedule->iNCpn &&
           CalibCpnSchedule->lCpnDate[j] < start_date - 5) {
      j++;
    }
    if (j > 0 &&
        fabs(start_date - CalibCpnSchedule->lCpnDate[j]) >
    ...
    k = j;
    while (k < CalibCpnSchedule->iNCpn &&
           CalibCpnSchedule->lCpnDate[k] <
               CalibExeSchedule->lActEndDates[i] - 5) {
      k++;
    }
    if (k > 0 && fabs(CalibExeSchedule->lActEndDates[i] -
                      CalibCpnSchedule->lCpnDate[k]) >
                     fabs(CalibExeSchedule->lActEndDates[i] -
                          CalibCpnSchedule->lCpnDate[k - 1])) {
      k--;
    }
  ...
  for (i = 0; i < AllCalibInst->iNbInst; i++) {
    ...
    if (is_for_calibration) {
      /*	ATM std */
      err = get_cash_vol(vol_curve_name, CalibCpnSchedule->lCpnDate[j],
                         CalibCpnSchedule->lCpnTheoDate[l], CalibInst->dFwdCash,
                         0, vol_ref_rate_name, &std, &power);

      if (err)
        goto FREE_RETURN;

      err = shift_the_vol(CalibInst->dFwdCash, CalibInst->dFwdCash,
                          CalibInst->dExeTime, std, power, param->vol_shift,
                          param->vol_type, param->shift_type, &std, &power);

      if (err)
        goto FREE_RETURN;

      if (power > 0.5) {
        power = srt_f_optblksch(CalibInst->dFwdCash, CalibInst->dFwdCash, std,
                                CalibInst->dExeTime, 1.0, SRT_PUT, PREMIUM);

        err = srt_f_optimpvol(power, CalibInst->dFwdCash, CalibInst->dFwdCash,
                              CalibInst->dExeTime, 1.0, SRT_PUT, SRT_NORMAL,
                              &std);
      }
      ...
      for (m = 0; m < CalibInst->iNbStrike; m++) {
        ...
        } else if (strike_type == 4) {
          CalibInst->dStrike[m] =
              CalibInst->dFwdCash +
              (AllCalibShortInst->sCalibInst[Get_Index(
                   CalibInst->dExeTime,
                   AllCalibShortInst->sCalibExeSchedule->dExeTimes,
                   AllCalibShortInst->sCalibExeSchedule->iNExe)])
                      .dNbStd[m] *
                  std;
        } else if (strike_type == 5) {
          if (m > 0) {
        ...
        CalibInst->dNbStd[m] =
            (CalibInst->dStrike[m] - CalibInst->dFwdCash) / std;
        ...
        if (m == 0 || strike_type < 3) {
          if (CalibInst->dStrike[m] >
              CalibInst->dFwdCash + param->max_std * std) {
            if (m == 0) {
              CalibInst->dStrike[m] =
                  CalibInst->dFwdCash + param->max_std * std;
            } else {
              ...
              CalibInst->dStrike[m] =
                  CalibInst->dFwdCash + (CalibInst->dStrike[m] - dMainStrike);
            }
          } else if (CalibInst->dStrike[m] <
                     CalibInst->dFwdCash - param->max_std * std) {
            if (m == 0) {
              CalibInst->dStrike[m] =
                  CalibInst->dFwdCash - param->max_std * std;
        ...
        err = get_cash_vol(vol_curve_name, CalibCpnSchedule->lCpnDate[j],
                           CalibCpnSchedule->lCpnTheoDate[l],
                           CalibInst->dStrike[m], 0, vol_ref_rate_name,
                           &(CalibInst->dNormalVol[m]), &power);

        if (err) {
          goto FREE_RETURN;
        }

        err = shift_the_vol(CalibInst->dFwdCash, CalibInst->dStrike[m],
                            CalibInst->dExeTime, CalibInst->dNormalVol[m],
                            power, vol_shift, vol_type, shift_type,
                            &(CalibInst->dNormalVol[m]), &power);
        ...
        if (power > 0.5) {
          CalibInst->dPrice[m] =
              srt_f_optblksch(CalibInst->dFwdCash, CalibInst->dStrike[m],
                              CalibInst->dNormalVol[m], CalibInst->dExeTime,
                              CalibInst->dLevel, SRT_PUT, PREMIUM);

          err = srt_f_optimpvol(CalibInst->dPrice[m], CalibInst->dFwdCash,
                                CalibInst->dStrike[m], CalibInst->dExeTime,
                                CalibInst->dLevel, SRT_PUT, SRT_NORMAL,
                                &(CalibInst->dNormalVol[m]));
        } else {
          CalibInst->dPrice[m] =
              srt_f_optblknrm(CalibInst->dFwdCash, CalibInst->dStrike[m],
                              CalibInst->dNormalVol[m], CalibInst->dExeTime,
                              CalibInst->dLevel, SRT_PUT, PREMIUM);
        }
      ...
      /* Fill the Coupons */
      CalibInst->dCpnValues[0] =
          -CalibCpnSchedule->dCpnDf[CalibInst->iStartCpn];

      for (iLoopCoupon = 1; iLoopCoupon < CalibInst->iNbCoupon; iLoopCoupon++) {
        CalibInst->dCpnValues[iLoopCoupon] =
            CalibInst->dStrike[0] *
            CalibCpnSchedule->dCpnCvg[CalibInst->iStartCpn + iLoopCoupon] *
            CalibCpnSchedule->dCpnDf[CalibInst->iStartCpn + iLoopCoupon];
      }

      CalibInst->dCpnValues[CalibInst->iNbCoupon - 1] +=
          CalibCpnSchedule->dCpnDf[CalibInst->iEndCpn];
    } else {
      /* Calculate the spread */
      if (err = swp_f_ForwardRate(
              CalibCpnSchedule->lCpnDate[j], CalibCpnSchedule->lCpnTheoDate[l],
              instr_freq, instr_basis, yc_name, ref_rate_name, &swp_rte)) {
        goto FREE_RETURN;
      }
...
Err GetCashVolAndConvert(GET_CASH_VOL_FUNC get_cash_vol, char *vol_curve_name,
                         double start_date, double end_date, double mat,
                         double cash_fwd, double cash_strike,
                         char *ref_rate_name, SrtDiffusionType vol_type,
                         double *vol) {
  Err err = NULL;
  double power, opt;
  ...
  if (mat > 1.0E-08) {
    err = get_cash_vol(vol_curve_name, start_date, end_date, cash_strike, 0,
                       ref_rate_name, vol, &power);
    ...
    if (power > 0.5 && vol_type == SRT_NORMAL) {
      ...
      opt = srt_f_optblksch(cash_fwd, cash_strike, *vol, mat, 1.0, SRT_CALL,
                            SRT_PREMIUM);

      if (mat >= YEARS_IN_DAY && opt > 1.0E-08) {
        err = srt_f_optimpvol(opt, cash_fwd, cash_strike, mat, 1.0, SRT_CALL,
                              SRT_NORMAL, vol);
      ...
      opt = srt_f_optblknrm(cash_fwd, cash_strike, *vol, mat, 1.0, SRT_CALL,
                            SRT_PREMIUM);

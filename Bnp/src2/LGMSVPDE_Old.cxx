/* ==========================================================================
   FILE_NAME:	LGMSVPDE_Old.cxx

   PURPOSE:		PDE implementation of the 1 Factor LGM model with
   stochastic vol.
...
/* -----------------------------------------------------------------------------------------------
        ...
        Calculation of expectations for the definition of the grids
                in Xt (constant grid center in XTMean =E(XT / X0) and +-
   iNbSigmaXGrid XTStd) in Epst (constant grid center in EpsTMean =E(EpsT /
   Eps0) and +- iNbSigmaEpsGrid EpsStd) in Phit (variable grid center at each t
   on PhitMean(t)
...
static Err LGMSVPDEExpectations(
    /* Inputs */
    int iNbTime, double *dTime,

    /* For X diffusion */
    double dLambdaX, double dSigma, double dTstar,
    ...
    double *PhitMean, double *PhitStd,

    double *ftMean, double *ftStd,

    double *ZtMean, double *ZtStd) {
  ...
  for (iNumTime = 0; iNumTime < iNbTime; iNumTime++) {
    if ((dTime[iNumTime] > SigTime[iNumSigTime]) &&
  ...
  for (iNumTime = 1; iNumTime < iNbTime; iNumTime++) {
    ...
    FuncDerBeta2.dXt1 =
        dDerBeta2; /*exp(-2*dLambdaX*(dTstar-dTime[iNumTime-1]));*/
    ...
    ZtMean[iNumTime] =
        1 - dAlpha * dRho / Sigt[iNumTime] / dDerBeta * ftMean[iNumTime];

    /* Calculation of Zt Mean */
    ZtStd[iNumTime] =
        sqrt(dVt2Mean - 1 +
             pow(dAlpha * dRho / (Sigt[iNumTime] * dDerBeta), 2) * dVarft -
    ...
    /* Error Check */
    if ((dPhit2Mean - dPhitMean * dPhitMean < 0) || (dVarft < 0) ||
        (dVt2Mean - 1 +
             pow(dAlpha * dRho / (Sigt[iNumTime] * dDerBeta), 2) * dVarft -
             2 * dAlpha * dRho / (Sigt[iNumTime] * dDerBeta) * dVtftMean <
         0)) {
      err = "Err LGMSVPDE : Numerical Pb in Std Calculation        , Tau could "
            "be "
            "too big";
    }
  }
...
/* -----------------------------------------------------------------------------------------------
        ...
        compute a uniform (equally spaced) grid from dMean-iNbSigmaLeft*dStd to
   dMean+iNbSigmaRight*dStd and shift the grid so that 0 is in the grid

   ------------------------------------------------------------------------------------------------
 */
static void LGMSVPDEGridUnif(/* Inputs */
                             int iNb, double dMean, double dStd,
                             double iNbSigmaLeft, double iNbSigmaRight,

                             /* Outputs */
                             double *Grid, int *pIndex0) {
  /* Declaration of locals variables */
  int iNumStep;
  double dStep;
  double dMin;
...
static void LGMSVPDEMakePhiGrid(/* Inputs */
                                int iNbPhi, double dPhiMean, double dPhiStd,
                                double iNbSigmaPhiGrid,

                                /* Parameter */
                                int IsExtremePoints,
                                double iNbSigmaExtremePoints, int IsPhiBoundExp,

                                /* Outputs */
                                double *GridPhi, int *pIndexPhiMean) {
  /* Declaration of locals variables */
        ...
        case LGMSV_TRUE: {
          if (iNbPhi == 3) {
            ...
            *pIndexPhiMean = 1;
            ...
            dPhiStep = dPhiMean *
                       (exp(iNbSigmaPhiGrid * dLogPhiStd) -
                        exp(-iNbSigmaPhiGrid * dLogPhiStd)) /
                       (iNbPhi - 3);
            ...
            *pIndexPhiMean += 1;
            ...
            GridPhi[0] = dPhiMean * exp(-iNbSigmaExtremePoints * dLogPhiStd);
            GridPhi[iNbPhi - 1] =
                dPhiMean * exp(iNbSigmaExtremePoints * dLogPhiStd);
          }
          break;
        }
    ...
    default: {
      ...
      case LGMSV_TRUE: {
        if (iNbPhi == 3) {
          ...
          GridPhi[0] =
              max(dPhiMean - iNbSigmaExtremePoints * dPhiStd, PhiFloor);
          ...
          *pIndexPhiMean = 1;
          ...
          /* Calculation of the step in Phi grid */
          dPhiStep = (dPhiMean + iNbSigmaPhiGrid * dPhiStd -
                      max(dPhiMean - iNbSigmaPhiGrid * dPhiStd, PhiFloor)) /
                     (iNbPhi - 3);
...
static double LGMSVPDEInterpolate(/* Inputs */
                                  double x0, int iNbfx, double *x,
                                  double ****fx, int iNumX, int iNumEps,
                                  int iNumProduct, int InterpolationType,
                                  int IsExtrapolationFlat, int iIndex) {
  /* declaration of locals variables */
  double fx0;
      ...
      if (iNbfx == 1)
        ...
        if (x[0] == x[1])
          ...
          if (iIndex == 0)
            ...
            fx0 = (x0 - x[iIndex]) / (x[iIndex + 1] - x[iIndex]) *
                      (fx[iIndex + 1][iNumX][iNumEps][iNumProduct] -
                       fx[iIndex][iNumX][iNumEps][iNumProduct]) +
                  fx[iIndex][iNumX][iNumEps][iNumProduct];
          else
            fx0 = (x0 - x[iIndex - 1]) / (x[iIndex] - x[iIndex - 1]) *
                      (fx[iIndex][iNumX][iNumEps][iNumProduct] -
                       fx[iIndex - 1][iNumX][iNumEps][iNumProduct]) +
                  fx[iIndex - 1][iNumX][iNumEps][iNumProduct];
        }
    ...
    case LGMSV_QUADRATIC: {
      ...
      if (iNbfx == 1)
        ...
        if (x[0] == x[1])
          ...
          /* Interpolation */
          fx0 = fx[iIndex1][iNumX][iNumEps][iNumProduct] /
                    (x[iIndex1] - x[iIndex2]) / (x[iIndex1] - x[iIndex3]) *
                    (x0 * x0 - (x[iIndex2] + x[iIndex3]) * x0 +
                     x[iIndex2] * x[iIndex3]) +
                fx[iIndex2][iNumX][iNumEps][iNumProduct] /
                    (x[iIndex2] - x[iIndex3]) / (x[iIndex2] - x[iIndex1]) *
                    (x0 * x0 - (x[iIndex3] + x[iIndex1]) * x0 +
                     x[iIndex3] * x[iIndex1]) +
                fx[iIndex3][iNumX][iNumEps][iNumProduct] /
                    (x[iIndex3] - x[iIndex1]) / (x[iIndex3] - x[iIndex2]) *
                    (x0 * x0 - (x[iIndex1] + x[iIndex2]) * x0 +
                     x[iIndex1] * x[iIndex2]);
        }
      }
      break;
...
static void LGMSVPDECalculationValueMatrixfromPhit(
    /* Inputs */
    int iIndexMinX, int iIndexMaxX, int iIndexMinZ, int iIndexMaxZ,

    int iNbPhi, int iProductNb, double *GridftTstar, double *GridZCenter,
    double *GridPhi_t1,
    ...
    double dPhit, double dSigma, double dSigt, double dLambdaX, double dAlpha,
    double dRho,

    double t,  /*time(t) */
    double t1, /*time(t+1) */

    int TypeInterpolationPhi, int IsExtrapolationFlat,
  ...
  /* Calculation of beta(t        ,Tstar)*D/DTstar ( beta(t        ,Tstar)) and
   * D/DTstar ( beta(t        ,Tstar)) */
  ...
  for (iNumX = iIndexMinX; iNumX <= iIndexMaxX; iNumX++) {
    ...
    for (iNumZ = iIndexMinZ; iNumZ <= iIndexMaxZ; iNumZ++) {
      ...
      /* dVt =
       * max((GridZCenter[iNumZ]+dZtMean)+dAlpha*dRho/dSigt/dDerBeta*GridftTstar[iNumX]
       *       ,dVFloor); */
      dVt = max((GridZCenter[iNumZ] + dZtMean) +
                    dAlphaRho_SigtDerBeta * GridftTstar[iNumX],
                dVFloor);
      ...
      /* Find the Index such that GridPhi_t1[index]<dPhi_t1 and
       * GridPhi_t1[index+1]>= dPhi_t1 */
      ...
      if (iIndexPhi == -1)
      ...
      else if (iIndexPhi == iNbPhi - 1)
        /* Extrapolation flat */
        for (iNumProduct = 0; iNumProduct < iProductNb; iNumProduct++)
          ValueGridXtZtProduct[iNumX][iNumZ][iNumProduct] =
              ValueTensor[iNbPhi - 1][iNumX][iNumZ][iNumProduct];
      else if ((iIndexPhi > 1) && (iIndexPhi < iNbPhi - 3) &&
               (TypeInterpolationPhi == LGMSV_MIXQUADRALINEAR)) {
        /* Quadratic Interpolation  */
        if (dPhi_t1 - GridPhi_t1[iIndexPhi] >
            0.5 * (GridPhi_t1[iIndexPhi + 1] - GridPhi_t1[iIndexPhi])) {
          /*
          iIndex1 = iIndexPhi;
          iIndex2 = iIndexPhi+1;
          iIndex3 = iIndexPhi+2;
          ...
          */
          ...
          for (iNumProduct = 0; iNumProduct < iProductNb; iNumProduct++) {
            ValueGridXtZtProduct[iNumX][iNumZ][iNumProduct] =
                ValueTensor[iIndex1][iNumX][iNumZ][iNumProduct] *
                    InterpQuadraCoef[iIndex1][0] *
                    (dPhi_t1 * dPhi_t1 -
                     InterpQuadraCoef[iIndex1][1] * dPhi_t1 +
                     InterpQuadraCoef[iIndex1][2]) +
                ValueTensor[iIndex2][iNumX][iNumZ][iNumProduct] *
                    InterpQuadraCoef[iIndex1][3] *
                    (dPhi_t1 * dPhi_t1 -
                     InterpQuadraCoef[iIndex1][4] * dPhi_t1 +
                     InterpQuadraCoef[iIndex1][5]) +
                ValueTensor[iIndex3][iNumX][iNumZ][iNumProduct] *
                    InterpQuadraCoef[iIndex1][6] *
                    (dPhi_t1 * dPhi_t1 -
                     InterpQuadraCoef[iIndex1][7] * dPhi_t1 +
                     InterpQuadraCoef[iIndex1][8]);
          }
        } else {
          iIndex1 = iIndexPhi - 1;
          iIndex2 = iIndexPhi;
        ...
        for (iNumProduct = 0; iNumProduct < iProductNb; iNumProduct++) {
          ValueGridXtZtProduct[iNumX][iNumZ][iNumProduct] =
              (dPhi_t1 - GridPhi_t1[iIndexPhi]) /
                  (GridPhi_t1[iIndexPhi + 1] - GridPhi_t1[iIndexPhi]) *
                  (ValueTensor[iIndexPhi + 1][iNumX][iNumZ][iNumProduct] -
...
LGMSVPDECalculationRDriftVar(/* Inputs */
                             int iIndexMinX, int iIndexMaxX, int iIndexMinZ,
                             int iIndexMaxZ,

                             double *GridftTstar, double *GridZCenter,

                             double dSigma, double dSigt, double dSig_t1,
                             double dXtMean, double dZtMean, double dPhitMean,
                             double dLambdaX, double dLambdaEps, double dAlpha,
                             double dRho, double dIfrt, double t, /*time(t) */
                             double t1,                           /*time(t+1) */
                             ...
                             /* Outputs */
                             double **DfDrift, double **DZDrift, double **DfVar,
                             double **DZVar, double **r) {
  /* Declaration of locals variables */
  double dDt;
  double dVt, dZt;
  int iNumX, iNumZ;
  ...
  DZDriftCoef =
      dAlphaRho_DerBeta * ((1 / dSig_t1 - 1 / dSigt) - dLambdaX / dSigt * dDt);
  ...
  for (iNumX = iIndexMinX; iNumX <= iIndexMaxX; iNumX++) {
    ...
    for (iNumZ = iIndexMinZ; iNumZ <= iIndexMaxZ; iNumZ++) {
      ...
      /*	DZDrift[iNumX][iNumZ] =
         -dLambdaEps*(dVt-1)*dDt-dAlpha*dRho/dDerBeta*GridftTstar[iNumX]*
                      ((1/dSig_t1-1/dSigt)-dLambdaX/dSigt*dDt);
              DZVar[iNumX][iNumZ] = dAlpha*dAlpha*dVt*(1-dRho*dRho)*dDt;
       */
      DZDrift[iNumX][iNumZ] =
          -dLambdaEps * (dVt - 1) * dDt - DZDriftCoef * GridftTstar[iNumX];
...
void LGMSVSaveTensor(/* Inputs */
                     ...
                     double *GridPhi, double *GridX, double *GridZ,
  ...
  for (iNumPhi = 0; iNumPhi < iNbPhi; iNumPhi++)
    for (iNumX = 0; iNumX < iNbX; iNumX++)
      for (iNumEps = 0; iNumEps < iNbEps; iNumEps++)
        fwrite(&ValueTensor_t1[iNumPhi][iNumX][iNumEps][0], 1, sizeof(double),
               fid);
  fclose(fid);
}
/* -----------------------------------------------------------------------------------------------
        ...
        sigma : time        , spot dependant and stochastic of the following
   form sigma(Xt        ,t        ,omega)  = sig(t)*sigma* Epst
...
Err lgmSV_adi(
  ...
  if (Params.IsNbSigmaXGridRightAuto == LGMSV_TRUE) {
    if (fabs(dLambdaEps) < 1E-06) {
      ...
      Params.iNbSigmaXGridRight =
          7.5 + 25 * max(dRho, 0) * dAlpha / 2.0 *
                    sqrt((1 - exp(-dLambdaEps * dTime[iNbTime - 1])) /
                         (dLambdaEps * dTime[iNbTime - 1]));
    }
  }
  ...
  ValueGridftZtProduct =
      f3tensor(0, iNbX - 1, 0, iNbEps - 1, 0, iNbProduct - 1);

  ValueTensor_t =
      f4tensor(0, iNbPhi - 1, 0, iNbX - 1, 0, iNbEps - 1, 0, iNbProduct - 1);
  ...
  num_f_pde_init_2d_adi(pde, iNbX, iNbEps, iNbProduct);
  ...
  if (!pde || !Sigt || !GridftTstar || !GridZCenter || !GridPhi_t ||
      !GridPhi_t1 || !PhiMean || !PhiStd || !ftMean || !ftStd || !ZtMean ||
      !ZtStd || !DfDrift || !DZDrift || !DfVar || !DZVar || !r ||
  ...
  err = LGMSVPDEExpectations(
      ...
      /* For X diffusion */
      dLambdaXGrid, dSigma, dTstar,

      SigTime, Sig, iNbSigTime,

      /* For Eps diffusion */
      dLambdaEpsGrid, dAlpha, dRho,

      /* Outputs */
      Sigt,

      PhiMean, PhiStd, ftMean, ftStd, ZtMean, ZtStd);
  ...
  LGMSVPDEGridUnif(/* Inputs */
                   iNbEps, 0, ZtStd[iNbTime - 1],

                   Params.iNbSigmaZGridLeft, Params.iNbSigmaZGridRight,

                   /* Outputs */
                   GridZCenter, &iIndexZ0);

  LGMSVPDEGridUnif(/* Inputs */
                   iNbX, 0, ftStd[iNbTime - 1], Params.iNbSigmaXGridLeft,
                   Params.iNbSigmaXGridRight,

                   /* Outputs */
                   GridftTstar, &iIndexf0);

  /* Final Grid of Phi         , Grid of phi at time T */
  LGMSVPDEMakePhiGrid(/* Inputs */
                      iNbPhi, PhiMean[iNbTime - 1], PhiStd[iNbTime - 1],
                      Params.iNbSigmaPhiGrid,

                      /* Parameter */
                      Params.IsExtremePoints, Params.iNbSigmaExtremePoints,
                      Params.IsPhiBoundExp,

                      /* Outputs */
                      GridPhi_t1, &iIndexPhi);

  /* Final Payoff valuation   */
  err = payoff_func(/* Time Information */
                    dDate[iNbTime - 1], dTime[iNbTime - 1],
                    ...
                    /* Grid data Information	*/
                    0, iNbPhi - 1, 0, iNbX - 1, 0, iNbEps - 1,

                    GridPhi_t1, GridftTstar,

                    /* Tensor of results to be updated		*/
                    /* 4 dimensions : Phit        ,Xt        ,Epst ,Product
                     */
                    iNbProduct, ValueTensor_t1);
      ...
      for (iNumX = 0; iNumX < iNbX; iNumX++)
        for (iNumEps = 0; iNumEps < iNbEps; iNumEps++)
          for (iNumProduct = 0; iNumProduct < 1; iNumProduct++)
            ValueTensor_t1[iNumPhi][iNumX][iNumEps][iNumProduct] =
                GridZCenter[iNumEps] * GridZCenter[iNumEps];
  }
  ...
  smessage("Phase 2 -convolution        , NbSigXGridRight: %f ",
           Params.iNbSigmaXGridRight);
  ...
  if (Params.ModeExport != LGMSV_TRUE) {
    ...
    for (iNumTime = iNbTime - 2; iNumTime >= 0; iNumTime--) {
      ...
      iNbPhiGrid_t =
          iNbPhi -
          (Params.GridPhiOptim == LGMSV_TRUE) *
              (int)((1 - iNumTime / ((double)(iNbTime - 1))) * (iNbPhi - 1) +
                    1.0e-08);
      iNbPhiGrid_t = ((int)(iNbPhiGrid_t / 2)) * 2 + 1;

      if (Params.GridXADIOptim == LGMSV_TRUE) {
        iIndexMinX = (int)((-Params.iNbSigmaXGridLeft * ftStd[iNumTime + 1] -
                            GridftTstar[0]) /
                               (GridftTstar[1] - GridftTstar[0]) +
                           1.0e-08);
        iIndexMaxX =
            max((int)((Params.iNbSigmaXGridRight * ftStd[iNumTime + 1] -
                       GridftTstar[0]) /
                          (GridftTstar[1] - GridftTstar[0]) +
                      1.0e-08),
                min(iIndexMinX + 2, iNbX - 1));
      } else {
        iIndexMinX = 0;
        iIndexMaxX = iNbX - 1;
      }
      ...
      if (Params.GridZADIOptim == LGMSV_TRUE) {
        iIndexMinZ = (int)((-Params.iNbSigmaZGridLeft * ZtStd[iNumTime + 1] -
                            GridZCenter[0]) /
                               (GridZCenter[1] - GridZCenter[0]) +
                           1.0e-08);
        iIndexMaxZ =
            max((int)((Params.iNbSigmaZGridRight * ZtStd[iNumTime + 1] -
                       GridZCenter[0]) /
                          (GridZCenter[1] - GridZCenter[0]) +
                      1.0e-08),
                min(iIndexMinZ + 2, iNbEps - 1));
      } else {
        iIndexMinZ = 0;
        iIndexMaxZ = iNbEps - 1;
      }
      ...
      if (Params.SaveTensor == LGMSV_TRUE) {
        ...
        LGMSVSaveTensor(/* Inputs */
                        ...
                        GridPhi_t1, GridftTstar, GridZCenter, ValueTensor_t1);
      }
      ...
      LGMSVPDEMakePhiGrid(/* Inputs */
                          iNbPhiGrid_t, PhiMean[iNumTime], PhiStd[iNumTime],
                          Params.iNbSigmaPhiGrid,
      ...
      LGMSVPDECalculationRDriftVar(/* Inputs */
                                   iIndexMinX, iIndexMaxX, iIndexMinZ,
                                   iIndexMaxZ,

                                   GridftTstar, GridZCenter,

                                   dSigma, Sigt[iNumTime], Sigt[iNumTime + 1],
                                   ftMean[iNumTime], ZtMean[iNumTime],
                                   PhiMean[iNumTime], dLambdaX, dLambdaEps,
                                   dAlpha, dRho, Ifr[iNumTime],
      ...
      for (iNumPhi = 0; iNumPhi < iNbPhiGrid_t; iNumPhi++) {
         ...
         */
        /* Interpolation From the value in the grid (XCenter        ,ZCenter
         * ,Phi) at t+1	 */
        LGMSVPDECalculationValueMatrixfromPhit(
            /* Inputs */
            iIndexMinX, iIndexMaxX, iIndexMinZ, iIndexMaxZ,

            iNbPhiGrid_t1, iNbProduct, GridftTstar, GridZCenter, GridPhi_t1,
            ...
            GridPhi_t[iNumPhi], dSigma, Sigt[iNumTime], dLambdaX, dAlpha, dRho,

            dTime[iNumTime],     /*time(t) */
            dTime[iNumTime + 1], /*time(t+1) */

            Params.TypeInterpolationPhi, Params.IsExtrapolationFlat,
        ...
        num_f_pde_one_step_backward_2f_adi(
            pde, iNbX, GridftTstar, iNbEps, GridZCenter, 0, iNbProduct - 1,
            ValueGridftZtProduct, DfDrift, DZDrift, DfVar, DZVar, r,
            ValueTensor_t[iNumPhi], iIndexMinX, iIndexMaxX, iIndexMinZ,
            iIndexMaxZ);
      ...
      if (EvalEvent[iNumTime]) {
        ...
        err = payoff_func(/* Time Information */
                          ...
                          /* Grid data Information	*/
                          0, iNbPhiGrid_t - 1, iIndexMinX, iIndexMaxX,
                          iIndexMinZ, iIndexMaxZ,

                          GridPhi_t, GridftTstar,

                          /* Tensor of results to be updated		*/
                          /* 4 dimensions : Phit        ,Xt        ,Epst
                             ,Product	*/
                          iNbProduct, ValueTensor_t);
        if (err) {
          goto FREE_RETURN;
        }
      }
    ...
    memcpy(dProductArrayPv, ValueTensor_t1[iIndexPhi][iIndexf0][iIndexZ0],
           iNbProduct * sizeof(double));
...
FREE_RETURN:
  ...
  /* free all the arrays */
  if (pde)
    num_f_pde_free_2d_adi(pde, iNbX, iNbEps, iNbProduct);
  /*if (pde) num_f_pde_free_2d_lod(pde        , iNbX        , iNbEps        ,
   * iNbProduct); */
  ...
  if (ValueGridftZtProduct)
    free_f3tensor(ValueGridftZtProduct, 0, iNbX - 1, 0, iNbEps - 1, 0,
                  iNbProduct - 1);

  if (ValueTensor_t)
    free_f4tensor(ValueTensor_t, 0, iNbPhi - 1, 0, iNbX - 1, 0, iNbEps - 1, 0,
                  iNbProduct - 1);

  if (ValueTensor_t1)
    free_f4tensor(ValueTensor_t1, 0, iNbPhi - 1, 0, iNbX - 1, 0, iNbEps - 1, 0,
                  iNbProduct - 1);
...
static void LGMSVPDEExpectations_QBeta(
  ...
  for (iNumTime = 1; iNumTime < iNbTime; iNumTime++) {
    ...
    FuncXtVtMean.cxx = dAlpha * dRho * Sigt[iNumTime - 1];
    ...
    ZtStd[iNumTime] =
        sqrt(dVt2Mean + pow(dAlpha * dRho / Sigt[iNumTime], 2) * dXt2Mean -
             2 * dAlpha * dRho / Sigt[iNumTime] * dXtVtMean -
             ZtMean[iNumTime] * ZtMean[iNumTime]);
  }
}
...
static void LGMSVPDEMakePhiGrid2_QBeta(/* Inputs */
                                       int iNbPhi, double dPhiMean,
                                       double dPhiStd, double iNbSigmaPhiGrid,
      ...
      if (dPhiMean == 0) {
        ...
        default: {
          ...
          dLogPhiStep = (exp(dLogPhiMean + iNbSigmaPhiGrid * dLogPhiStd) -
                         exp(dLogPhiMean - iNbSigmaPhiGrid * dLogPhiStd)) /
                        (iNbPhi - 1);
...
static void LGMSVPDECalculationValueMatrixfromPhit_QBeta(
    ...
    int iNbPhi, int iProductNb, double *GridXCenter, double *GridZCenter,
    double *GridPhi_t1,
  ...
  for (iNumZ = iIndexMinZ; iNumZ <= iIndexMaxZ; iNumZ++) {
    for (iNumX = iIndexMinX; iNumX <= iIndexMaxX; iNumX++) {
      ...
      dPhi_t1 =
          dPhit +
          (dSigma * dSigt * dSigma * dSigt * dVt - 2 * dLambdaX * dPhit) * dDt;

      /* Find the Index such that GridPhi_t1[index-1]< dPhi_t1 and
       * GridPhi_t1[index]>= dPhi_t1 */
      iIndexPhi = 0;
      ...
      for (iNumProduct = 0; iNumProduct < iProductNb; iNumProduct++) {
        ValueGridXtZtProduct[iNumX][iNumZ][iNumProduct] =
            LGMSVPDEInterpolate_QBeta(/* Inputs */
                                      dPhi_t1, iNbPhi, GridPhi_t1, ValueTensor,
                                      iNumX, iNumZ, iNumProduct,
                                      TypeInterpolationPhi, IsExtrapolationFlat,
                                      iIndexPhi);
      }
    }
  }
...
LGMSVPDECalculationRDriftVar_QBeta(/* Inputs */
                                   int iIndexMinX, int iIndexMaxX,
                                   int iIndexMinZ, int iIndexMaxZ,

                                   double *GridXCenter, double *GridZCenter,
                                   double dPhit, double dSigma, double dSigt,
                                   double dSig_t1, double dXtMean,
                                   double dZtMean, double dPhitMean,
                                   double dLambdaX, double dLambdaEps,
                                   double dAlpha, double dRho, double dIfrt,
                                   double t,  /*time(t) */
                                   double t1, /*time(t+1) */
                                   ...
                                   /* Outputs */
                                   double **DXDrift, double **DZDrift,
                                   double **DXVar, double **DZVar, double **r) {
  /* Declaration of locals variables */
  double dDt;
  double dXt, dVt, dZt;
  int iNumX, iNumZ;
  ...
  for (iNumX = iIndexMinX; iNumX <= iIndexMaxX; iNumX++) {
    ...
    for (iNumZ = iIndexMinZ; iNumZ <= iIndexMaxZ; iNumZ++) {
      ...
      /* Et+1 [Xt+1 - Xt / Ft] and Vt+1 [Xt+1 - Xt / Ft]*/
      DXDrift[iNumX][iNumZ] =
          ((dPhit - dPhitMean) - dLambdaX * (dXt - dXtMean)) * dDt;
      DXVar[iNumX][iNumZ] = dSigt * dSigt * dVt * dDt;

      /* Et+1 [Zt+1 - Zt / Ft] and Vt+1 [Zt+1 - Zt / Ft]*/
      DZDrift[iNumX][iNumZ] =
          -dLambdaEps * (dVt - 1) * dDt -
          dAlpha * dRho *
              ((1 / dSig_t1 - 1 / dSigt) * (dXt - dXtMean) +
               ((dPhit - dPhitMean) - dLambdaX * (dXt - dXtMean)) / dSigt *
                   dDt);
...
Err lgmSV_adi_QBeta(
  ...
  /* Gestion of allocation errors */
  if (!pde || !GridX || !GridXCenter || !Sigt || !GridZCenter || !GridPhi_t ||
      !GridPhi_t1 || !PhiMean || !PhiStd || !XtMean || !XtStd || !ZtMean ||
      !ZtStd || !DXDrift || !DZDrift || !DXVar || !DZVar || !r ||
      !ValueGridXtZtProduct || !ValueTensor_t || !ValueTensor_t1) {
    err = "Memory allocation error (1) in LGMSVPDE";
    goto FREE_RETURN;
  }
  ...
  LGMSVPDEExpectations_QBeta(
      ...
      PhiMean, PhiStd, XtMean, XtStd, ZtMean, ZtStd);
  ...
  LGMSVPDEGridUnif_QBeta(/* Inputs */
                         iNbX, 0, XtStd[iNbTime - 1], Params.iNbSigmaXGridLeft,
                         Params.iNbSigmaXGridRight,

                         /* Outputs */
                         GridXCenter, &iIndexX0);
  ...
  LGMSVPDEMakePhiGrid2_QBeta(/* Inputs */
                             ...
                             Params.iNbSigmaExtremePoints, Params.IsPhiBoundExp,
  ...
  if (Params.ModeExport != LGMSV_TRUE) {
    ...
    for (iNumTime = iNbTime - 2; iNumTime >= 0; iNumTime--) {
      ...
      if (Params.GridXADIOptim == LGMSV_TRUE) {
        iIndexMinX = (int)((-Params.iNbSigmaXGridLeft * XtStd[iNumTime + 1] -
                            GridXCenter[0]) /
                               (GridXCenter[1] - GridXCenter[0]) +
                           1.0e-08);
        iIndexMaxX = (int)((Params.iNbSigmaXGridRight * XtStd[iNumTime + 1] -
                            GridXCenter[0]) /
                               (GridXCenter[1] - GridXCenter[0]) +
                           1.0e-08);
      } else {
      ...
      if (Params.GridZADIOptim == LGMSV_TRUE) {
        iIndexMinZ = (int)((-Params.iNbSigmaZGridLeft * ZtStd[iNumTime + 1] -
                            GridZCenter[0]) /
                               (GridZCenter[1] - GridZCenter[0]) +
                           1.0e-08);
        iIndexMaxZ = (int)((Params.iNbSigmaZGridRight * ZtStd[iNumTime + 1] -
                            GridZCenter[0]) /
                               (GridZCenter[1] - GridZCenter[0]) +
                           1.0e-08);
      } else {
      ...
      if (Params.SaveTensor == LGMSV_TRUE) {
        ...
        LGMSVSaveTensor_QBeta(/* Inputs */
                              ...
                              GridPhi_t1, GridXCenter, GridZCenter,
                              ValueTensor_t1);
      }

      /* Make the grid in Phit at time t */
      LGMSVPDEMakePhiGrid2_QBeta(/* Inputs */
                                 iNbPhiGrid_t, PhiMean[iNumTime],
                                 PhiStd[iNumTime], Params.iNbSigmaPhiGrid,
      ...
      for (iNumPhi = 0; iNumPhi < iNbPhiGrid_t; iNumPhi++) {
        ...
        LGMSVPDECalculationValueMatrixfromPhit_QBeta(
            ...
            iNbPhiGrid_t1, iNbProduct, GridXCenter, GridZCenter, GridPhi_t1,

            Params.VFloor,

            ValueTensor_t1,

            XtMean[iNumTime], ZtMean[iNumTime],
        ...
        LGMSVPDECalculationRDriftVar_QBeta(/* Inputs */
                                           ...
                                           GridXCenter, GridZCenter,
                                           GridPhi_t[iNumPhi], dSigma,
                                           Sigt[iNumTime], Sigt[iNumTime + 1],
                                           XtMean[iNumTime], ZtMean[iNumTime],
                                           PhiMean[iNumTime], dLambdaX,
                                           dLambdaEps, dAlpha, dRho,
                                           Ifr[iNumTime],
                                           dTime[iNumTime],     /*time(t) */
                                           dTime[iNumTime + 1], /*time(t+1) */
                                           ...
                                           /* Outputs */
                                           DXDrift, DZDrift, DXVar, DZVar, r);

        /* Launch The Crank-Nicolson					*/
        /* Calculation of the payoff tensor at time t	*/
        /* for the correspondant phi(t)					*/
        num_f_pde_one_step_backward_2f_adi(
            pde, iNbX, GridXCenter, iNbEps, GridZCenter, 0, iNbProduct - 1,
            ValueGridXtZtProduct, DXDrift, DZDrift, DXVar, DZVar, r,
            ValueTensor_t[iNumPhi], iIndexMinX, iIndexMaxX, iIndexMinZ,
            iIndexMaxZ);
    ...
    /* The grids are made such as
            center on Phi = 0        , X=0        ,  at t=0 */
    memcpy(dProductArrayPv, ValueTensor_t1[iIndexPhi][iIndexX0][iIndexZ0],
           iNbProduct * sizeof(double));
  } else {
    dProductArrayPv[0] = PhiStd[iNbTime - 1];  /*dZTStd; /*PhiStd[iNbTime-1];*/
    dProductArrayPv[1] = PhiMean[iNbTime - 1]; /*dXTStd; */
...
FREE_RETURN:
  ...
  if (ValueGridXtZtProduct)
    free_f3tensor(ValueGridXtZtProduct, 0, iNbX - 1, 0, iNbEps - 1, 0,
                  iNbProduct - 1);

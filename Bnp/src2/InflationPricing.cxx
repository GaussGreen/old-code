/* ===================================================================================
   FILENAME:      InflationPricing.cxx / P.A
...
double Inflation_GetFixing(long lValDate, Inflation_CPIFixing *CPIFixing) {
  ...
  iNum = 0;
  while ((iNum < CPIFixing->lNbDate) &&
         (lValDate < CPIFixing->lFixingDate[iNum]))
    iNum++;
...
*/
double
Inflation_FwdCPI(long lValDate, Inflation_CPIMkt *CPIMkt,
                 const char *NomCurve, /* Nominal Curve name*/
                 WrapInfo *RealCurve /* Real curve either a name if WestWrapper
                                        or  a c++ object ptr */
) {
  double dFwdCPI;
  long lToday;
...
*/
double
Inflation_FwdDRI_givenAdj(/* Inputs */
                          long lFixDate1, long lFixDate2, double dAlpha,
                          double dBeta, double dCvxtyAdj1, double dCvxtyAdj2,
                          Inflation_CPIMkt *CPIMkt, /* CPI Market */
                          const char *NomCurve,     /* Nominal Curve name*/
                          WrapInfo *RealCurve, /* Real curve either a name if
  ...
  if (dAlpha != 0) {
    ...
    dFwdCPI1 =
        Inflation_FwdCPI(lFixDate1, CPIMkt, NomCurve, /* Nominal Curve name*/
  ...
  if (dBeta != 0) {
    ...
    dFwdCPI2 =
        Inflation_FwdCPI(lFixDate2, CPIMkt, NomCurve, /* Nominal Curve name*/
                         RealCurve /* Real curve name */);
...
double Inflation_FwdCouponValue(
  ...
  dFwdDRI = Inflation_FwdDRI_givenAdj(
      /* Inputs */
      Coupon->lFixDateCPI1, Coupon->lFixDateCPI2, Coupon->dAlpha, Coupon->dBeta,
      Coupon->dCvxtyAdj1, Coupon->dCvxtyAdj2, CPIMkt, /* CPI Market */
   ...
   */
  /*  E (under Qtpay) max(DRI(Tfix1        ,Tfix2)/DRIRef + gamma        ,
   * Floor)
   */
  /* = E (under Qtpay) (  DRI(Tfix1        ,Tfix2)/DRIRef + gamma +
   * max((Floor-gamma)
   * - DRI(Tfix1        ,Tfix2)/DRIRef         , 0)*/

  /* Pricing of E (under Qtpay) max((Floor-gamma)*DRIRef - DRI(Tfix1 ,Tfix2) ,
   * 0) */
  if (Coupon->iIsFloored) {
    dTimeToExpiry =
        ((double)(max(Coupon->lFixDateCPI1, Coupon->lFixDateCPI2) - lToday)) /
        DAYSINYEAR;

    if (dTimeToExpiry >= 0) {
      /* Spread Option pricing */
      /* E[max(K - (nx*Xt + ny*Yt)        , 0)] with Xt        , Yt lognormal
       * (using 1D numerical integration) */
      ...
      dFloor = max(
          (Coupon->dFloor - Coupon->dGamma) - dFwdDRI / Coupon->dDRIRef, 0.0);
    }
  }
...
double Inflation_ValueCashFlow(
  ...
  for (iNum = 0; iNum < CashFlows->iNbCoupon; iNum++) {
    ...
    dFwdCouponValue =
        Inflation_FwdCouponValue(Coupon,       /* Coupon */
                                 CPIMkt,       /* CPI Market */
                                 NomCurve,     /* Nominal Curve name*/
										  ...
										  InflationCashFlows		*AssetSwapCashFlows		/* Description of the assetswap cash flows	 */ )
{
  ...
  case IndexedNotional:
    ...
    for (iNum = 0; iNum < AssetSwapCashFlows->iNbCoupon - 1; iNum++) {
      if (BondCashFlows->Coupon[iNum].dCouponValue == 0.0) {
        ...
        AssetSwapCashFlows->Coupon[iNum].dCouponValue =
            dDf_Tsettle *
            (BondCashFlows->Coupon[iNum].dFwdPV -
    ...
    if (BondCashFlows->Coupon[iNum].dCouponValue +
      ...
      AssetSwapCashFlows->Coupon[iNum].dCouponValue =
          dDf_Tsettle *
          (BondCashFlows->Coupon[iNum].dFwdPV +
           BondCashFlows->Coupon[iNum + 1].dFwdPV -
           BondCashFlows->Coupon[iNum].dFwdPVFloor -
  ...
  case FlatDirtyNotional:
    ...
    /* Calculation of the Dirty price if the notional of the asset swap is the
     * dirty price */
    ...
    dSum = 0;
    for (iNum = 0; iNum < AssetSwapCashFlows->iNbCoupon; iNum++) {
      dSum += AssetSwapCashFlows->Coupon[iNum].dGamma *
  ...
  case IndexedDirtyNotional:
    ...
    dBondLegTodayPv =
        BondCashFlows->Coupon[AssetSwapCashFlows->iNbCoupon].dFwdPV *
    ...
    for (iNum = AssetSwapCashFlows->iNbCoupon - 1; iNum >= 0; iNum--) {
      ...
      dBondLegTodayPv += BondCashFlows->Coupon[iNum].dFwdPV *
                         BondCashFlows->Coupon[iNum].dDfPayDate;
      ...
      dDirtyPrice = (dBondLegTodayPv - dAssetSwapLegTodayPV) /
                    (AssetSwapCashFlows->Coupon[iNum].dGamma *
                         BondCashFlows->Coupon[iNum].dDfPayDate +
                     dfTi_m1);
							...
							double				*ptr_DfSettle			/* Disount factor nominal at settlement date */)
{
  ...
  /* Get dTodayCBP */
  dTodayCBP = Inflation_FwdCPI(CPIMkt->lTodayDate, CPIMkt,
                               NomCurve, /* Nominal Curve name*/
                               RealCurve /* Real curve either a name if
                                            WestWrapper or  a c++ object ptr */
  );
  ...
  /* All the intermediate PV seen as of today have been stored in the
   * BondCashFlows */

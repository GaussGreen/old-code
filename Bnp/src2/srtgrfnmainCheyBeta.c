				  ...
				  double	**prod_val)
{
	...
	if (!void_prm || !is_event || !ifr || !date ||  ((!dff || !gam || !gam_sqr) && (max_num_df > 0)))
	{
...
FREE_RETURN:
	...
	if (max_num_df > 0)
	{
		...
		if (gam_sqr) free_dvector(gam_sqr, 0, max_num_df-1);

		dff= NULL; gam = NULL; gam_sqr = NULL;
	}
	...
	is_event = NULL; date = NULL; ifr = NULL; time = NULL;
				  ...
				  double	**prod_val)
{
	...
	if (!void_prm || !is_event || ((!dff || !gam || !gam_sqr) && (max_num_df > 0)))
	{
		err = "Memory allocation failure";
		goto FREE_RETURN;
	}
	...
	else if((method == 0) || (method ==1))
	{
		...
		err = cheybeta_pricing_mc(	
					...
					/*	Discretisation	*/
					numpaths,
					method, /* 0 balantisam, 1 balantisam adjusted */
					gen_method,
					/*	Model data		*/
					und,					
					/*	Product data */
...
FREE_RETURN:
	...
	if (ifr) free (ifr);
	if (time) free (time);
	if (stop_vol) free(stop_vol);
	if (stop_lambda) free(stop_lambda);

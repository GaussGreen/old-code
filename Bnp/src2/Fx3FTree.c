...
Err				err;
	...
	{	
		...
		if (k1 >= 1 && k1 <= nb_nodes_tab[step][idx_rel] - 2)
		{
			...
			maxmin_dual_tab[step][idx_rel][0] = maxmin_dual_tab[step][idx_rel][1] 
		...
		{
			a -= (k1_p - k1);
			bar_idx[step] = lngmatrix (0, nb_nodes_tab[step][idx1]-1, 0, nb_nodes_tab[step][idx2]-1);
			if (!bar_idx[step])
			{
...
Err			err							= NULL;
	...
	{	
		...
		for (l=0; l<3; l++)
		{
			...
			temp_mat1[2][l] = delta_t * (transf_matrix[0][l] - transf_matrix[1][l])
				+ transf_matrix[2][l];
		}
	...
	if (bar_idx)
	{
		for (i=0; i<nstp; i++)
		{
			if (bar_idx[i])
			{
				...
				{
				case 0:
					free_lngmatrix (bar_idx[i], 0, nb_nodes_tab[i][1]-1, 0, nb_nodes_tab[i][2]-1);
					break;
				case 1:
					free_lngmatrix (bar_idx[i], 0, nb_nodes_tab[i][0]-1, 0, nb_nodes_tab[i][2]-1);
					break;
				case 2:
					free_lngmatrix (bar_idx[i], 0, nb_nodes_tab[i][0]-1, 0, nb_nodes_tab[i][1]-1);
					break;
				default:
					break;
				}

    ...
    double* res)
{
    ...
    if (!dom_ifr || !for_ifr || !transf_matrix || !transf_matrix_next || !transf_matrix_inv ||
    ...
    for (step = nstp - 2; step >= 1; step--)
    {
        ...
        c = (for_var[step] - corr_for_fx[step + 1] * vol_fx * sigma_next[1]) * delta_t;
        d = 1.0 - for_lam * delta_t;
        e = (dom_ifr[step] - for_ifr[step] - 0.5 * vol_fx * vol_fx) * delta_t;
        ...
        for (l = 0; l < 3; l++)
        {
            temp_mat1[0][l] = b * transf_matrix[0][l];
            temp_mat1[1][l] = d * transf_matrix[1][l];
            temp_mat1[2][l] =
                delta_t * (transf_matrix[0][l] - transf_matrix[1][l]) + transf_matrix[2][l];
        }
            ...
            for (i = 0; i < n0; i++)
            {
                ...
                for (j = 0; j < n1; j++)
                {
                    ...
                    for (k = 0; k < n2; k++)
                    {
                        ...
                        fwd     = dig_dt * (svijk[0] - svijk[1]) + svijk[2] + e;
    ...
    a = 0.0;
    b = 1.0 - dom_lam * delta_t;
    c = -corr_for_fx[1] * vol_fx * sigma_next[1] * delta_t;
    d = 1.0 - for_lam * delta_t;
    e = (dom_ifr[0] - for_ifr[0] - 0.5 * vol_fx * vol_fx) * delta_t;

    expect_node[0] = a + b * sv[i][j][k][0];
    expect_node[1] = c + d * sv[i][j][k][1];
    expect_node[2] = delta_t * (sv[i][j][k][0] - sv[i][j][k][1]) + sv[i][j][k][2] + e;
    ...
    if (do_dig)
    {
        fwd     = dig_dt * (sv[i][j][k][0] - sv[i][j][k][1]) + sv[i][j][k][2] + e;
        dig_val = norm((fwd - dig_strike) / dig_ivol);

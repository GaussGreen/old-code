...
static void _FixPV(long lToday, const char *szYC, const long *plStart_Begin,
                   ...
                   // results
                   double *pdPV, double *pdLevel, double *pdLevel_Mult_Coupon) {
  ...
  *pdLevel_Mult_Coupon = 0.;
  ...
  for (; plStart_Begin < plStart_End;
    ...
    _ASSERTE(plStart_Begin && plEnd_Begin && pdCoupon_Begin &&
             pdNotional_Begin);
...
double Swap_Rate(long lToday, const char *szYC, const long *plFixStart_Begin,
  ...
  dFltPV = FltLeg(lToday, szYC, plFltStart_Begin, plFltStart_End,
                  plFltEnd_Begin, plFltPay_Begin, pdMargin_Begin,
                  pdSpread_Begin, pdFltNotional_Begin, eFltBasis, &dLevel_Flt,
                  &dLevel_Mult_Margin, &dLevel_Mult_Spread);

  dFixPV =
      FixLeg(lToday, szYC, plFixStart_Begin, plFixStart_End, plFixEnd_Begin,
             plFixPay_Begin,
             memset(_alloca(nSize_Fix * sizeof(double)), 0,
                    nSize_Fix * sizeof(double)),
             pdFixNotional_Begin, eFixBasis, &dLevel_Fix, &dLevel_Mult_Coupon);
...
double FltLeg(long lToday, const char *szYC, const long *plStart_Begin,
  ...
  // margins and spreads
  _FixPV(lToday, szYC, plStart_Begin, plStart_End, plEnd_Begin, plPay_Begin,
         pdMargin_Begin, pdNotional_Begin, eBasis, &dPV_Margin, pdLevel,
         pdLevelMultCoupon_Margin);

  _FixPV(lToday, szYC, plStart_Begin, plStart_End, plEnd_Begin, plPay_Begin,
         pdSpread_Begin, pdNotional_Begin, eBasis, &dPV_Spread, pdLevel,
  ...
  for (; plStart_Begin < plStart_End;
       ++plStart_Begin, ++plPay_Begin, ++pdNotional_Begin) {
    _ASSERTE(*plPay_Begin >= lToday);
...
double FixLeg(long lToday, const char *szYC, const long *plStart_Begin,
  ...
  _FixPV(lToday, szYC, plStart_Begin, plStart_End, plEnd_Begin, plPay_Begin,
         pdCoupon_Begin, pdNotional_Begin, eBasis, &dPV, pdLevel,
         pdLevelMultCoupon);
...
const char *Price_DiagGenEuro(
  ...
  for (k = 0; k < nex; ++k) {
    ...
    if ((plFixStart_Begin[k] - lastcaltime >= lMinInterval) &&
      ...
      // NB: because of the assumption that fixed leg freq is the same as
      // exercise freq
      /// but this exercise fee is in fact fixed fee.
      exer_fee = pdFixFee_Begin[k];

      while ((l < lNFltNot) &&
             (plFltStart_Begin[l] < plFixStart_Begin[k] - 10.0)) {
        ++l;
      }

      NDimCorrel = lNFixNot - k;
      err = Compute_CoInitalSwaps_Correl2(
          SpotDate, (char *)szCorrel, get_correl, StartDate, lTheoEnd, srtFreq,
          srtBasis, NDimCorrel, &Correl);
      if (err)
        return err;

      err = AmortizedSwaptionShiftedLogForMAD(
          (char *)szYC, (char *)szVC, (char *)szRefRate,

          srtFreq, srtBasis,

          exer_fee,

          (long *)plFixStart_Begin + k, (long *)plFixEnd_Begin + k,
          lNFixNot - k, (double *)pdFixNotional_Begin + k,
          (double *)pdFixRate_Begin + k,

          (long *)plFltStart_Begin + l, (long *)plFltEnd_Begin + l,
          lNFltNot - l, (double *)pdFltNotional_Begin + l,
          (double *)pdMargin_Begin + l, (double *)pdSpread_Begin + l,

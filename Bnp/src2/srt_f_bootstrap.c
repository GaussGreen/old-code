...
/* ---------------------------------------------------------------------------
   ...
   In this case  , f is the price of the asset  , and x is the bucket vol
   s_tsv[1][s_lCurrentSigmaIndex]
   ---------------------------------------------------------------------------
 */
...
Err srt_f_bootstrap_calibrate(SrtUndPtr undptr, SrtGrfnParam *grfnparam,
  ...
  for (i = 0; i < (*num_inp); i++) {
    ...
    if (lLastFixing > clcndate) {
      ...
      /*
         ...
         ,&dLevel); dfStart = swp_f_df(clcndate  ,dStart  ,szYieldCurveName);
                              dfEnd = swp_f_df(clcndate  ,dEnd
  ...
  for (i = 0; i < iNumUsedInstruments; i++) {
    ...
    /* If not over the number of iterations: set the vol   , else skip this
     * price */

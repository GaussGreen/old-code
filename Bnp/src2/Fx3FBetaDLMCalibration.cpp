...
int i;
	...
	if (CalibConst)
	{
		...
		if (CalibConst->CalibFunctionsForVol) free (CalibConst->CalibFunctionsForVol);
		CalibConst->CalibFunctionsForVol = NULL;

		if (CalibConst->CalibFunctionsForBeta) free (CalibConst->CalibFunctionsForBeta);
...
Err					err = NULL;
	...
	cCalibConst->cAllInstPrecalc = calloc(iNbOpt, sizeof(FxBetaDLM_InstPrecalc*));

	if (!cCalibConst->cPrecalc || !cCalibConst->lSigIndex || !cCalibConst->cAllInstPrecalc)
	{
	...
	err = FxBetaDLM_Allocation_Precalculations(	iNbOpt,
												cCalibConst->cPrecalc);
	...
	for (i=0; i<iNbOpt; i++)
	{
		...
		cCalibConst->lSigIndex[i] = Get_Index(Inst->dExeTime, cModel->dPWTime, cModel->iNbPWTime);
		...
		err = FxBetaDLM_Fill_InstPrecalc(	Inst,
											cModel,
	...
	err = FxBetaDLM_Fill_ModelPrecalc(	cModel,
										cCalibConst);
	...
	if (!cCalibConst->CalibFunctionsForVol || !cCalibConst->CalibFunctionsForBeta)
	{
...
Err					err = NULL;
long				nb_merge_dates;
double				*null_sigma_fx		= NULL,
					*exercise_opt		= NULL,
					*maturity_opt		= NULL,
					*merge_dates		= NULL,
					*sig_dom			= NULL,
					*sig_for			= NULL,
					*corr_dom_for_3F	= NULL,
					*corr_dom_fx_3F		= NULL,
					*corr_for_fx_3F		= NULL,
					max_time_correl = 0.0,
...
int	i, index;
	...
	if (!str || !null_sigma_fx || !model || !hermite || !maturity_opt || !exercise_opt)
	{
	...
	err = FxBetaDLM_fill_str(	undname_fx,
								undname_dom,
								undname_for,
	...
	/* Merge the rates */
	err = merge_rates_fx_corr_ts(	str->time_dom, 
									str->sig_dom,
									str->nb_dom,
									str->time_for, 
									str->sig_for,
...
*/
	...
	{
		err = FxBetaDLM_GetFirstGuessFromB0(	str->b0,
												str->tstar,
												exercise_opt,
												maturity_opt,
												nb_opt,
...
*/
	...
	err = init_FxBetaDLM_model(	NumParams->dMinTimeSpaceCorrel,
								NumParams->dMaxTimeCorrel,
								str->today,
								str->yc_dom,
								str->tau_dom,
								str->nb_dom,
	...
	err = initialise_FxBetaDLM_Hermite(	NumParams,
										hermite);
	...
	for (i=0; i<model->iNbPWTime; i++)
	{
		...
		{
			model->dSigmaFxUp[i] = model->dSigmaFx[i] * exp ( model->dAlpha * sqrt(exercise_opt[index]));
			model->dSigmaFxDown[i] = model->dSigmaFx[i] * exp (-model->dAlpha * sqrt(exercise_opt[index]));
		}
		else
		{
			model->dSigmaFxUp[i] = model->dSigmaFx[i] * exp ( model->dAlpha * sqrt((1.0 - exp(- 2.0 * model->dLambda * exercise_opt[index])) / (2.0 * model->dLambda)));
			model->dSigmaFxDown[i] = model->dSigmaFx[i] * exp (-model->dAlpha * sqrt((1.0 - exp(- 2.0 * model->dLambda * exercise_opt[index])) / (2.0 * model->dLambda)));
		}
	}

	err = FxBetaDLM_CalibrationModel(	nb_opt,
										opt_settlmt_date,
										opt_fixing_date,
										opt_strikes,
										opt_bs_vols,
	...
	for (i=0; i<nb_opt; i++)
	{
		(*fx_vols)[i] = model->dSigmaFx[Get_Index(exercise_opt[i], model->dPWTime, model->iNbPWTime)];
	}
	...
	if(!dom_fx_DLM_corr || !for_fx_DLM_corr || !dom_fx_DLM_corr_down || !for_fx_DLM_corr_down || !date_DLM_corr)
	{
	...
	*nb_DLM_corr = model->iNbPWTime;
	...
	{
		(*date_DLM_corr)[i] = (long) (model->lToday + model->dPWTime[i] * DAYS_IN_YEAR);
...
FREE_RETURN:
	...
	if (exercise_opt) free (exercise_opt);
	if (maturity_opt) free (maturity_opt);
	if (sig_dom) free (sig_dom);
	if (sig_for) free (sig_for);
	if (corr_dom_for_3F) free (corr_dom_for_3F);
	if (corr_dom_fx_3F) free (corr_dom_fx_3F);
	if (corr_for_fx_3F) free (corr_for_fx_3F);
...
FxBetaDLM_CalibConst	*CalibConst = NULL;
	...
	{
		index_smile = 0;
		while (index_smile < nb_opt && opt_settlmt_date[index_smile] < smile_settlmt_date)
		{
			index_smile++;
		}		

		if (index_smile == nb_opt || fabs(opt_settlmt_date[index_smile] - smile_settlmt_date) > 0.5)
		{
	...
	for (i=0; i<nb_opt; i++)
	{
		...
		err = FxBetaDLM_Allocate_FxOptInst(	i,
											nb_strike,
											AllInst[i]);
		...
		if (calib_smile && i == index_smile)
		{
			...
			err = FxBetaDLM_Setup_FxOptInst(opt_settlmt_date[i],
											opt_settlmt_date[i],
											model->lToday,
											opt_fixing_date[i],
											3,
	...
	err = FxBetaDLM_Fill_CalibConst(nb_opt,
									AllInst,
									calib_smile,
									index_smile,
									model,
	...
	CalibConst->CalibFunctionsForVol->GetLimitAndLastParam = FxBetaDLM_GetLimitAndLastVol;
	CalibConst->CalibFunctionsForVol->GetSecondGuess = FxBetaDLM_GetSecondGuessVol;
	...
	CalibConst->CalibFunctionsForVol->UpdateConstsAfterParam = FxBetaDLM_UpdateConstsAfterVol;
	...
	CalibConst->CalibFunctionsForBeta->GetFirstGuess = FxBetaDLM_GetFirstGuessBeta;
	CalibConst->CalibFunctionsForBeta->GetLimitAndLastParam = FxBetaDLM_GetLimitAndLastBeta;
	CalibConst->CalibFunctionsForBeta->GetSecondGuess = FxBetaDLM_GetSecondGuessBeta;
	...
	CalibConst->CalibFunctionsForBeta->UpdateConstsAfterParam = FxBetaDLM_UpdateConstsAfterBeta;

	if (calib_smile)
	{
		CalibConst->first_guess_for_smile = model->dSigmaFx[CalibConst->lSigIndex[index_smile]];

		CalibConst->ExtrapolAfterSetBeta = FxBetaDLM_ExtrapolVol;
		CalibConst->CalibFunctionsForVol->ExtrapolParam = FxBetaDLM_ExtrapolVol_WhenBetaCalib;
...
double	t1, t2;
	...
	for (i=start_index; i<end_index+1; i++)
	{
		...
		var_rates_dom += sig_dom2 * Phi_Func(2.0 * model->dLambdaDom, model->dTStar, t1, t2);
		var_rates_for += sig_for2 * Phi_Func(2.0 * model->dLambdaFor, model->dTStar, t1, t2);
		covar_rates += sig_domfor * Phi_Func(model->dLambdaDom + model->dLambdaFor, model->dTStar, t1, t2);
		
		if (index < CalibConst->iNbOpt && fabs(t2 - model->dPWTime[CalibConst->lSigIndex[index]]) < 1.0E-10)
		{
...
{
	CalibConst->dBetaDom = (1.0 - exp(-model->dLambdaDom * (Inst->dSettlementTime - model->dTStar))) / model->dLambdaDom;
	CalibConst->dBetaFor = (1.0 - exp(-model->dLambdaFor * (Inst->dSettlementTime - model->dTStar))) / model->dLambdaFor;
...
double	old_integ_down, new_integ_down, old_integ2_down, new_integ2_down;
	...
	start_index = Get_Index(Inst->dFwdStartTime, model->dPWTime, model->iNbPWTime);
	...
	if(fabs(model->dAlpha) < TINY)
	{
		for (j=0; j<start_index+1; j++)
		{
			...
			if (dt > 0.0)
			{
				...
				total_var_ffx += sig_for2 * Psi_Func(for_lam, for_lam, tstar, t1, t2)
		...
		temp_bracket = temp_for * (sig_forfx - sig_for2 * (1.0 - temp_for) / for_lam 
		...
		for (j=start_index; j<end_index+1; j++)
		{
			...
			if (dt > 0.0)
			{
				...
				for (k=0; k<nb_points; k++)
				{
					...
					total_var_ffx += -2.0 * sig_forfx * Etha_Func(for_lam, tstar, old_t, new_t)
							+ 2.0 * sig_domfx * Etha_Func(dom_lam, tstar, old_t, new_t);
					total_var_ffx += sig_for2 * Psi_Func(for_lam, for_lam, tstar, old_t, new_t)
							+ sig_dom2 * Psi_Func(dom_lam, dom_lam, tstar, old_t, new_t)
							- 2.0 * sig_domfor * Psi_Func(dom_lam, for_lam, tstar, old_t, new_t);

					covar_dom_ffx += sig_domfx * Phi_Func(dom_lam, tstar, old_t, new_t)
		...
		InstConst->dVarRatesFor = var_rates_for - 2.0 * model->dC0 * (1.0 + 2.0 * model->dC0 * var_0_fwdstart) * for_fwd * for_fwd;
		InstConst->dCovarRates = covar_dom_for - 2.0 * model->dC0 * covar_dom_for_adjust;
		...
		InstConst->dCovarFFxFor = -(1.0 + 2.0 * model->dC0 * var_0_fwdstart) * for_fwd;
	}
	else  /* Fill the constants with alpha fudge */
	{
		for (j=0; j<start_index+1; j++)
		{
			...
			if (dt > 0.0)
			{
				...
				total_var_ffx_down += -2.0 * sig_forfx_down * Etha_Func(for_lam, tstar, t1, t2)
						+ 2.0 * sig_domfx_down * Etha_Func(dom_lam, tstar, t1, t2);
				total_var_ffx_down += sig_for2 * Psi_Func(for_lam, for_lam, tstar, t1, t2)
						+ sig_dom2 * Psi_Func(dom_lam, dom_lam, tstar, t1, t2)
						- 2.0 * sig_domfor * Psi_Func(dom_lam, for_lam, tstar, t1, t2);
			}
		}
		...
		temp_bracket = temp_for * (sig_forfx - sig_for2 * (1.0 - temp_for) / for_lam 
				+ sig_domfor * (1.0 - temp_dom) / dom_lam);
		temp_bracket_down = temp_for * (sig_forfx_down - sig_for2 * (1.0 - temp_for) / for_lam 
				+ sig_domfor * (1.0 - temp_dom) / dom_lam);
		...
		old_integ_down = temp_bracket_down / (1.0 + 2.0 * model->dC0 * total_var_ffx_down);
		...
		for (j=start_index; j<end_index+1; j++)
		{
			...
			if (dt > 0.0)
			{
				...
				for (k=0; k<nb_points; k++)
				{
					...
					total_var_ffx_down += -2.0 * sig_forfx_down * Etha_Func(for_lam, tstar, old_t, new_t)
								+ 2.0 * sig_domfx_down * Etha_Func(dom_lam, tstar, old_t, new_t);
					total_var_ffx_down += sig_for2 * Psi_Func(for_lam, for_lam, tstar, old_t, new_t)
								+ sig_dom2 * Psi_Func(dom_lam, dom_lam, tstar, old_t, new_t)
								- 2.0 * sig_domfor * Psi_Func(dom_lam, for_lam, tstar, old_t, new_t);

					covar_dom_ffx_down += sig_domfx_down * Phi_Func(dom_lam, tstar, old_t, new_t)
								- sig_domfor * Gamma_Func(for_lam, dom_lam, tstar, old_t, new_t)
								+ sig_dom2 * Gamma_Func(dom_lam, dom_lam, tstar, old_t, new_t);
					...
					new_integ_down = temp_bracket_down / (1.0 + 2.0 * model->dC0 * total_var_ffx_down);
					...
					covar_dom_for_adjust_down += (old_integ2_down + new_integ2_down) * min_dt / 2.0;
				...
				var_ffx_down += -2.0 * sig_forfx_down * Etha_Func(for_lam, tstar, t1, t2)
							+ 2.0 * sig_domfx_down * Etha_Func(dom_lam, tstar, t1, t2);

				/* Rates Down / Rates component */
				var_ffx_down += sig_for2 * Psi_Func(for_lam, for_lam, tstar, t1, t2)
							+ sig_dom2 * Psi_Func(dom_lam, dom_lam, tstar, t1, t2)
							- 2.0 * sig_domfor * Psi_Func(dom_lam, for_lam, tstar, t1, t2);			

				covar_for_ffx_down += sig_forfx_down * Phi_Func(for_lam, tstar, t1, t2)
								- sig_for2 * Gamma_Func(for_lam, for_lam, tstar, t1, t2)
								+ sig_domfor * Gamma_Func(dom_lam, for_lam, tstar, t1, t2);
			}
		}
		...
		/* Save Down */	
		InstConst->dVarRatesFor_down = var_rates_for - 2.0 * model->dC0 * (1.0 + 2.0 * model->dC0 * var_0_fwdstart_down) * for_fwd_down * for_fwd_down;
		InstConst->dCovarRates_down = covar_dom_for - 2.0 * model->dC0 * covar_dom_for_adjust_down;
		...
		InstConst->dCovarFFxFor_down = -(1.0 + 2.0 * model->dC0 * var_0_fwdstart_down) * for_fwd_down;	
	}
	return err;
}	

Err FxBetaDLM_Update_ModelPrecalc(	int							index,
									FxBetaDLM_model				*model,
									FxBetaDLM_CalibConst		*CalibConst)
{
Err		err = NULL;
...
double	sig_fx_down, sig_fx2_down,
		sig_domfx_down, sig_forfx_down,
		var_ffx_down,
		covar_dom_ffx_down,
		covar_for_ffx_down,
		expect_for_down,
...
double	sig_fx_mid, sig_fx2_mid,
		sig_domfx_mid, sig_forfx_mid,
		var_ffx_mid,
		covar_dom_ffx_mid,
		covar_for_ffx_mid,
		expect_for_mid,
	...
	if (fabs(model->dAlpha) < TINY)
	{
		...
		for (j=start_index; j<end_index+1; j++)
		{
			...
			if (dt > 0.0)
			{
				...
				for (k=0; k<nb_points; k++)
				{
					...
					var_ffx += -2.0 * sig_forfx * Etha_Func(for_lam, tstar, old_t, new_t)
							+ 2.0 * sig_domfx * Etha_Func(dom_lam, tstar, old_t, new_t);
					var_ffx += sig_for2 * Psi_Func(for_lam, for_lam, tstar, old_t, new_t)
							+ sig_dom2 * Psi_Func(dom_lam, dom_lam, tstar, old_t, new_t)
							- 2.0 * sig_domfor * Psi_Func(dom_lam, for_lam, tstar, old_t, new_t);
		...
		if (CalibConst->cPrecalc)
		{
			...
			CalibConst->cPrecalc->dCovarFFxDom[CalibConst->cPrecalc->iNbDone] = covar_dom_ffx;
			CalibConst->cPrecalc->dCovarFFxFor[CalibConst->cPrecalc->iNbDone] = -for_fwd;
			CalibConst->cPrecalc->dCovarFFxFor3D[CalibConst->cPrecalc->iNbDone] = covar_for_ffx;
			CalibConst->cPrecalc->dExpectFor[CalibConst->cPrecalc->iNbDone] = expect_for;
			CalibConst->cPrecalc->dIntegral[CalibConst->cPrecalc->iNbDone] = new_integ;
			CalibConst->cPrecalc->dIntegral2[CalibConst->cPrecalc->iNbDone] = new_integ2;
			CalibConst->cPrecalc->dCovarRatesAdjust[CalibConst->cPrecalc->iNbDone] = covar_dom_for_adjust;

			/* Add the adjustments to the FORVAR and RATESCOV*/
			CalibConst->cPrecalc->dVarRatesFor[CalibConst->cPrecalc->iNbDone] = CalibConst->cPrecalc->dVarRatesFor3D[CalibConst->cPrecalc->iNbDone]
				-2.0 * model->dC0 * for_fwd * for_fwd;
			CalibConst->cPrecalc->dCovarRates[CalibConst->cPrecalc->iNbDone] = CalibConst->cPrecalc->dCovarRates3D[CalibConst->cPrecalc->iNbDone]
	...
	else
	{
		...
		else
		{
			...
			covar_dom_for_adjust_down = CalibConst->cPrecalc->dCovarRatesAdjust_down[index-1];
			...
			covar_dom_for_adjust_mid = CalibConst->cPrecalc->dCovarRatesAdjust_mid[index-1];
		}
		...
		for (j=start_index; j<end_index+1; j++)
		{
			...
			if (dt > 0.0)
			{
				...
				for (k=0; k<nb_points; k++)
				{
					...
					var_ffx_down += -2.0 * sig_forfx_down * Etha_Func(for_lam, tstar, old_t, new_t)
							+ 2.0 * sig_domfx_down * Etha_Func(dom_lam, tstar, old_t, new_t);
					var_ffx_down += sig_for2 * Psi_Func(for_lam, for_lam, tstar, old_t, new_t)
							+ sig_dom2 * Psi_Func(dom_lam, dom_lam, tstar, old_t, new_t)
							- 2.0 * sig_domfor * Psi_Func(dom_lam, for_lam, tstar, old_t, new_t);
					...
					new_integ_down = temp_bracket_down / (1.0 + 2.0 * model->dC0 * var_ffx_down);
					new_integ2_down = covar_dom_ffx_down * new_integ_down;
					...
					var_ffx_mid += -2.0 * sig_forfx_mid * Etha_Func(for_lam, tstar, old_t, new_t)
							+ 2.0 * sig_domfx_mid * Etha_Func(dom_lam, tstar, old_t, new_t);
					var_ffx_mid += sig_for2 * Psi_Func(for_lam, for_lam, tstar, old_t, new_t)
							+ sig_dom2 * Psi_Func(dom_lam, dom_lam, tstar, old_t, new_t)
							- 2.0 * sig_domfor * Psi_Func(dom_lam, for_lam, tstar, old_t, new_t);

					covar_dom_ffx_mid += sig_domfx_mid * Phi_Func(dom_lam, tstar, old_t, new_t)
							- sig_domfor * Gamma_Func(for_lam, dom_lam, tstar, old_t, new_t)
							+ sig_dom2 * Gamma_Func(dom_lam, dom_lam, tstar, old_t, new_t);

					temp_bracket_mid = temp_for * (sig_forfx_mid - sig_for2 * (1.0 - temp_for) / for_lam 
							+ sig_domfor * (1.0 - temp_dom) / dom_lam);
					
					new_integ_mid = temp_bracket_mid / (1.0 + 2.0 * model->dC0 * var_ffx_mid);
					...
					covar_dom_for_adjust_mid += (old_integ2_mid + new_integ2_mid) * min_dt / 2.0;
				...
				covar_for_ffx_down += sig_forfx_down * Phi_Func(for_lam, tstar, t1, t2)
									- sig_for2 * Gamma_Func(for_lam, for_lam, tstar, t1, t2)
									+ sig_domfor * Gamma_Func(dom_lam, for_lam, tstar, t1, t2);
				covar_for_ffx_mid += sig_forfx_mid * Phi_Func(for_lam, tstar, t1, t2)
									- sig_for2 * Gamma_Func(for_lam, for_lam, tstar, t1, t2)
									+ sig_domfor * Gamma_Func(dom_lam, for_lam, tstar, t1, t2);
			}
		}
		...
		if (CalibConst->cPrecalc)
		{
			...
			CalibConst->cPrecalc->dVarFFx_down[CalibConst->cPrecalc->iNbDone] = var_ffx_down;
			CalibConst->cPrecalc->dCovarFFxDom_down[CalibConst->cPrecalc->iNbDone] = covar_dom_ffx_down;
			CalibConst->cPrecalc->dCovarFFxFor_down[CalibConst->cPrecalc->iNbDone] = -for_fwd_down;
			CalibConst->cPrecalc->dCovarFFxFor3D_down[CalibConst->cPrecalc->iNbDone] = covar_for_ffx_down;
			CalibConst->cPrecalc->dExpectFor_down[CalibConst->cPrecalc->iNbDone] = expect_for_down;
			CalibConst->cPrecalc->dIntegral_down[CalibConst->cPrecalc->iNbDone] = new_integ_down;
			CalibConst->cPrecalc->dIntegral2_down[CalibConst->cPrecalc->iNbDone] = new_integ2_down;
			CalibConst->cPrecalc->dCovarRatesAdjust_down[CalibConst->cPrecalc->iNbDone] = covar_dom_for_adjust_down;

			CalibConst->cPrecalc->dVarFFx_mid[CalibConst->cPrecalc->iNbDone] = var_ffx_mid;
			CalibConst->cPrecalc->dCovarFFxDom_mid[CalibConst->cPrecalc->iNbDone] = covar_dom_ffx_mid;
			CalibConst->cPrecalc->dCovarFFxFor_mid[CalibConst->cPrecalc->iNbDone] = -for_fwd_mid;
			CalibConst->cPrecalc->dCovarFFxFor3D_mid[CalibConst->cPrecalc->iNbDone] = covar_for_ffx_mid;
			CalibConst->cPrecalc->dExpectFor_mid[CalibConst->cPrecalc->iNbDone] = expect_for_mid;
			CalibConst->cPrecalc->dIntegral_mid[CalibConst->cPrecalc->iNbDone] = new_integ_mid;
			CalibConst->cPrecalc->dIntegral2_mid[CalibConst->cPrecalc->iNbDone] = new_integ2_mid;
			CalibConst->cPrecalc->dCovarRatesAdjust_mid[CalibConst->cPrecalc->iNbDone] = covar_dom_for_adjust_mid;
			...
			CalibConst->cPrecalc->dVarRatesFor_down[CalibConst->cPrecalc->iNbDone] = CalibConst->cPrecalc->dVarRatesFor3D[CalibConst->cPrecalc->iNbDone]
				-2.0 * model->dC0 * for_fwd_down * for_fwd_down;
			CalibConst->cPrecalc->dCovarRates_down[CalibConst->cPrecalc->iNbDone] = CalibConst->cPrecalc->dCovarRates3D[CalibConst->cPrecalc->iNbDone]
				-2.0 * model->dC0 * covar_dom_for_adjust_down;

			CalibConst->cPrecalc->dVarRatesFor_mid[CalibConst->cPrecalc->iNbDone] = CalibConst->cPrecalc->dVarRatesFor3D[CalibConst->cPrecalc->iNbDone]
				-2.0 * model->dC0 * for_fwd_mid * for_fwd_mid;
			CalibConst->cPrecalc->dCovarRates_mid[CalibConst->cPrecalc->iNbDone] = CalibConst->cPrecalc->dCovarRates3D[CalibConst->cPrecalc->iNbDone]
...
double	varY, varZ, covFFxY,  covFFxZ;
	...
	if(NumParams->iMethod == 1)
	{
		...
		k1 = (InstConst->dVarFFx * InstConst->dVarRatesDom - InstConst->dCovarFFxDom * InstConst->dCovarFFxDom) / InstConst->dVarFFx;
		k2 = (InstConst->dVarFFx * InstConst->dVarRatesFor - InstConst->dCovarFFxFor * InstConst->dCovarFFxFor) / InstConst->dVarFFx;
		k3 = (InstConst->dVarFFx * InstConst->dCovarRates - InstConst->dCovarFFxDom * InstConst->dCovarFFxFor) / InstConst->dVarFFx;

		varY = InstConst->dBetaDom * InstConst->dBetaDom * InstConst->dVarRatesDom
		...
		lin_adjustment = (InstConst->dBetaDom * InstConst->dCovarFFxDom 
			- InstConst->dBetaFor * InstConst->dCovarFFxFor) / InstConst->dVarFFx;
		...
		else
		{
			...
			{
				err = "FxBetaDLM_Update_InstPrecalc_FromMoment: Infinite variance process";
				return err;
			}
			InstConst->dFwdQuad2 = InstConst->dVarFFx / InstConst->dFwdQuad2;
			InstConst->dFwdAlpha = covFFxY / InstConst->dVarFFx;
			InstConst->dFwdBeta = exp(0.5 * (varY * InstConst->dVarFFx - covFFxY * covFFxY) / InstConst->dVarFFx) / sqrt(InstConst->dFwdQuad2);
		}
	...
	if (NumParams->iMethod == 2)
	{
		...
		{
			fixed_adjustment = Inst->dLnFwdFx + 0.5 * log(temp_fwd) 
				- 0.5 * (InstConst->dVarFFx * varY - covFFxY * covFFxY) / InstConst->dVarFFx 
				- 0.5 * pow(coef_b + covFFxY/InstConst->dVarFFx , 2) * InstConst->dVarFFx / temp_fwd;
		}
		else
		{
			fixed_adjustment = Inst->dLnFwdFx + 0.5 * log(temp_fwd); 
		...
		else
		{
			...
			InstConst->dFwdQuad2 = InstConst->dVarFFx / InstConst->dFwdQuad2;
			InstConst->dFwdAlpha = covFFxY / InstConst->dVarFFx;
			InstConst->dFwdBeta = 1 / sqrt(InstConst->dFwdQuad2) * exp(0.5 * (varY * InstConst->dVarFFx - covFFxY * covFFxY) / InstConst->dVarFFx);
		}
		...
		else
		{
			...
			InstConst->dQuadVar = (varY * InstConst->dVarFFx - covFFxY * covFFxY) / varY;
			if(InstConst->dQuadVar < 0)
			{
				err = "FxBetaDLM_Update_InstPrecalc_FromMoment: Negative Variance";
			}
		...
		if (InstConst->iIsCPD)
		{
			InstConst->coef_b_const = model->dB0 / temp - 2.0 * coef_c * InstConst->dCovarFFxDom * InstConst->dBetaDom;
			InstConst->coef_b_lin = 2.0 * coef_c;

			InstConst->fixed_adj_const = 0.5 * log(temp_fwd) 
			- 0.5 * (InstConst->dVarFFx * varY - covFFxY * covFFxY) / InstConst->dVarFFx ;
		...
		if(fabs(model->dAlpha) > TINY)
		{
			expectX = NumParams->dX0 - InstConst->dCovarFFxDom_down * InstConst->dBetaDom;
			...
			varY = InstConst->dBetaDom * InstConst->dBetaDom * InstConst->dVarRatesDom
						+ InstConst->dBetaFor * InstConst->dBetaFor * InstConst->dVarRatesFor_down
						- 2 * InstConst->dBetaDom * InstConst->dBetaFor * InstConst->dCovarRates_down;
			...
			{
				fixed_adjustment = Inst->dLnFwdFx + 0.5 * log(temp_fwd) 
					- 0.5 * (InstConst->dVarFFx_down * varY - covFFxY * covFFxY) / InstConst->dVarFFx_down 
					- 0.5 * pow(coef_b + covFFxY / InstConst->dVarFFx_down , 2) * InstConst->dVarFFx_down / temp_fwd;
			}
			else
			{
				fixed_adjustment = Inst->dLnFwdFx + 0.5 * log(temp_fwd);
			...
			else
			{
				...
				InstConst->dFwdQuad2_down = InstConst->dVarFFx_down / InstConst->dFwdQuad2_down;
				InstConst->dFwdAlpha_down = covFFxY / InstConst->dVarFFx_down;
				InstConst->dFwdBeta_down = 1 / sqrt(InstConst->dFwdQuad2_down) * exp(0.5 * (varY * InstConst->dVarFFx_down - covFFxY * covFFxY) / InstConst->dVarFFx_down);
			}
			...
			else
			{
				...
				InstConst->dQuadVar_down = (varY * InstConst->dVarFFx_down - covFFxY * covFFxY) / varY;
			}
			...
			if (InstConst->iIsCPD)
			{
				InstConst->coef_b_const_down = model->dB0 / temp - 2.0 * coef_c * InstConst->dCovarFFxDom_down * InstConst->dBetaDom;
				InstConst->coef_b_lin_down = 2.0 * coef_c;

				InstConst->fixed_adj_const_down = 0.5 * log(temp_fwd) 
				- 0.5 * (InstConst->dVarFFx * varY - covFFxY * covFFxY) / InstConst->dVarFFx_down;

				InstConst->fixed_adj_lin_down = - 0.5 * InstConst->dVarFFx_down / temp_fwd;
				InstConst->fixed_adj_lin_const_down = covFFxY / InstConst->dVarFFx_down;
			}
		}
	}
	else	/* Pricing With a first conditioning on a minimum variance Linear varible*/
	if(NumParams->iMethod == 3)
	{
		...
		{
			fixed_adjustment = Inst->dLnFwdFx + 0.5 * log(temp_fwd) 
				- 0.5 * (InstConst->dVarFFx * varZ - covFFxZ * covFFxZ) / InstConst->dVarFFx 
				- 0.5 * pow(coef_b + covFFxZ / InstConst->dVarFFx , 2) * InstConst->dVarFFx / temp_fwd;
		}
		else
		{
			fixed_adjustment = Inst->dLnFwdFx + 0.5 * log(temp_fwd);
		...
		else
		{
			...
			InstConst->dQuadVar = (varZ * InstConst->dVarFFx - covFFxZ * covFFxZ) / varZ;
			if(InstConst->dQuadVar < 0)
			{
				err = "FxBetaDLM_Update_InstPrecalc_FromMoment: Negative Variance";
			}
	...
	{
		err = "FxBetaDLM_Update_InstPrecalc_FromMoment: Parameter Method is invalid";
		return err;
	}
	return err; 
}
...
double	varY, varZ, covFFxY,  covFFxZ;
	...
	if (NumParams->iMethod == 2)
	{
		...
		InstConst->dQuadConst = Inst->dLnFwdFx + InstConst->fixed_adj_const 
								+ InstConst->fixed_adj_lin * pow(InstConst->dQuadLin + InstConst->fixed_adj_lin_const, 2);
	}
	else	/* Pricing With a first conditioning on a minimum variance Linear varible*/
	if(NumParams->iMethod == 3)
	{
		...
		if(fabs(InstConst->dVarFFx) > 1e-10)
		{
			fixed_adjustment = Inst->dLnFwdFx - 0.5 * log(temp) 
				- 0.5 * (InstConst->dVarFFx * varZ - covFFxZ * covFFxZ) / InstConst->dVarFFx 
				- 0.5 * pow(coef_b + covFFxZ / InstConst->dVarFFx , 2) * InstConst->dVarFFx * temp;
		}
		else
		{
			fixed_adjustment = Inst->dLnFwdFx - 0.5 * log(temp);
...
Err		err = NULL;
	...
	InstConst->dVarRatesDom = CalibConst->cPrecalc->dVarRatesDom[Inst->iInstIndex];
	InstConst->dVarRatesFor = CalibConst->cPrecalc->dVarRatesFor[Inst->iInstIndex];
	...
	InstConst->dCovarFFxDom = CalibConst->cPrecalc->dCovarFFxDom[Inst->iInstIndex];
	InstConst->dCovarFFxFor = CalibConst->cPrecalc->dCovarFFxFor[Inst->iInstIndex];	
	
	if(fabs(model->dAlpha) > TINY)
	{
		InstConst->dVarRatesFor_down = CalibConst->cPrecalc->dVarRatesFor_down[Inst->iInstIndex];
		InstConst->dCovarRates_down = CalibConst->cPrecalc->dCovarRates_down[Inst->iInstIndex];

		InstConst->dVarFFx_down = CalibConst->cPrecalc->dVarFFx_down[Inst->iInstIndex];
		InstConst->dTotalVarFFx_down = InstConst->dVarFFx_down;
		InstConst->dStdFFX_down = sqrt(InstConst->dVarFFx_down);

		InstConst->dCovarFFxDom_down = CalibConst->cPrecalc->dCovarFFxDom_down[Inst->iInstIndex];
		InstConst->dCovarFFxFor_down = CalibConst->cPrecalc->dCovarFFxFor_down[Inst->iInstIndex];			
	}
	
	err = FxBetaDLM_Update_InstPrecalc_FromMoment(	Inst,
													model,
													CalibConst->cNumParams,
													InstConst);
...
FxBetaDLM_CalibConst	*CalibConst = (FxBetaDLM_CalibConst*) (GlobalConst);
	...
	*vol1 = model->dSigmaFx[CalibConst->lSigIndex[index_param]];
	...
	if (index_param > 0)
	{
		*vol1 += model->dSigmaFx[CalibConst->lSigIndex[index_param - 1]] - CalibConst->first_guess;
...
FxBetaDLM_CalibConst	*CalibConst = (FxBetaDLM_CalibConst*) (GlobalConst);
	...
	*limit_down = sqrt(CalibConst->cPrecalc->dVarX[vol_index-1] * CalibConst->dMinFact / t1);
	*limit_up = sqrt(CalibConst->cPrecalc->dVarX[vol_index-1] / CalibConst->dMaxFact / t1);
...
Err err = NULL;
	...
	for (i=last_index; i<=new_index; i++)
	{
		...
		if(fabs(model->dLambda) < TINY)
		{
			model->dSigmaFxUp[i] = vol * exp( model->dAlpha * sqrt(model->dPWTime[new_index]));	
			model->dSigmaFxDown[i] = vol * exp(-model->dAlpha * sqrt(model->dPWTime[new_index]));		
		}
		else
		{
			model->dSigmaFxUp[i] = vol * exp( model->dAlpha * sqrt((1.0 - exp(- 2.0 * model->dLambda * model->dPWTime[new_index]))/(2.0 * model->dLambda)));
			model->dSigmaFxDown[i] = vol * exp(-model->dAlpha * sqrt((1.0 - exp(- 2.0 * model->dLambda * model->dPWTime[new_index]))/(2.0 * model->dLambda)));				
		}
		t2  = model->dPWTime[i];
		...
		if(fabs(model->dAlpha) < TINY)
		{
			...
			{
				err = FxBetaDLM_correl_mapping(	model->dSigmaFx[i],
												model->dB0,
												model->dC0,
												model->dTStar,
												model->dSigmaDom[i],
												...
												model->dLambdaFor,
												model->dCorrDomFor[i],
												model->dCorrDomFxInput[i],
												model->dCorrForFxInput[i],
												!i? 0.0:model->dPWTime[i-1],
												model->dPWTime[i],
												varFFx,
												&(model->dCorrDomFx[i]),
												&(model->dCorrForFx[i]));
			...
			if(CalibConst->cNumParams->iMappingMethod == 2)
			{
				err = FxBetaDLM_correl_mapping_forward(	CalibConst->cNumParams->dFFxCorrelTolerance,
														model->dPWTime[i],
														!i? 0.0:model->dPWTime[i-1],
														model->dB0,
														model->dC0,
			...
			varFFx += -2.0 * model->dCorrForFx[i] * model->dSigmaFor[i] * model->dSigmaFx[i] * Etha_Func(model->dLambdaFor, model->dTStar, t1, t2)
					+ 2.0 * model->dCorrDomFx[i] * model->dSigmaDom[i] * model->dSigmaFx[i] * Etha_Func(model->dLambdaDom, model->dTStar , t1, t2);
			varFFx += model->dSigmaFor[i] * model->dSigmaFor[i] * Psi_Func(model->dLambdaFor, model->dLambdaFor, model->dTStar, t1, t2)
					+ model->dSigmaDom[i] * model->dSigmaDom[i] * Psi_Func(model->dLambdaDom, model->dLambdaDom, model->dTStar, t1, t2)
					- 2.0 * model->dCorrDomFor[i] * model->dSigmaDom[i] * model->dSigmaFor[i] * Psi_Func(model->dLambdaDom, model->dLambdaFor, model->dTStar, t1, t2);
		}
		else
		{
			/* transform correlations */
			if(CalibConst->cNumParams->iMappingMethod == 1)
			{
				if(CalibConst->cNumParams->iMidCorrelation)
				{
					...
					err = FxBetaDLM_correl_mapping(	model->dSigmaFx[i],
													...
													model->dPWTime[i],
													varFFx,
													&(model->dCorrDomFxDown[i]),
													&(model->dCorrForFxDown[i]));

					if (err) goto FREE_RETURN;
				}
				else
				{
					err = FxBetaDLM_correl_mapping(	model->dSigmaFxUp[i],
													model->dB0,
													model->dC0,
													model->dTStar,
													model->dSigmaDom[i],
					...
					err = FxBetaDLM_correl_mapping(	model->dSigmaFxDown[i],
													model->dB0,
													model->dC0,
													model->dTStar,
													model->dSigmaDom[i],
			...
			varFFxUp += -2.0 * model->dCorrForFx[i] * model->dSigmaFor[i] * model->dSigmaFxUp[i] * Etha_Func(model->dLambdaFor, model->dTStar, t1, t2)
					+ 2.0 * model->dCorrDomFx[i] * model->dSigmaDom[i] * model->dSigmaFxUp[i] * Etha_Func(model->dLambdaDom, model->dTStar , t1, t2);
			varFFxUp += model->dSigmaFor[i] * model->dSigmaFor[i] * Psi_Func(model->dLambdaFor, model->dLambdaFor, model->dTStar, t1, t2)
					+ model->dSigmaDom[i] * model->dSigmaDom[i] * Psi_Func(model->dLambdaDom, model->dLambdaDom, model->dTStar, t1, t2)
					- 2.0 * model->dCorrDomFor[i] * model->dSigmaDom[i] * model->dSigmaFor[i] * Psi_Func(model->dLambdaDom, model->dLambdaFor, model->dTStar, t1, t2);		

			/* update var ffx down*/
			varFFxDown += model->dSigmaFxDown[i] * model->dSigmaFxDown[i] * (t2 - t1);
			varFFxDown += -2.0 * model->dCorrForFxDown[i] * model->dSigmaFor[i] * model->dSigmaFxDown[i] * Etha_Func(model->dLambdaFor, model->dTStar, t1, t2)
					+ 2.0 * model->dCorrDomFxDown[i] * model->dSigmaDom[i] * model->dSigmaFxDown[i] * Etha_Func(model->dLambdaDom, model->dTStar , t1, t2);
			varFFxDown += model->dSigmaFor[i] * model->dSigmaFor[i] * Psi_Func(model->dLambdaFor, model->dLambdaFor, model->dTStar, t1, t2)
					+ model->dSigmaDom[i] * model->dSigmaDom[i] * Psi_Func(model->dLambdaDom, model->dLambdaDom, model->dTStar, t1, t2)
					- 2.0 * model->dCorrDomFor[i] * model->dSigmaDom[i] * model->dSigmaFor[i] * Psi_Func(model->dLambdaDom, model->dLambdaFor, model->dTStar, t1, t2);		
		}
...
Err		err = NULL;
	...
	err = FxBetaDLM_Update_ModelPrecalc(vol_index,
										model,
										CalibConst);
...
Err	err = NULL;
	...
	err = FxBetaDLM_Update_ModelPrecalc(Inst->iInstIndex,
										model,
										CalibConst);

	if (err) return err;

	err = FxBetaDLM_Update_InstPrecalc(	Inst,
										model,
										CalibConst,
										InstConst);
...
FxBetaDLM_CalibConst	*CalibConst = (FxBetaDLM_CalibConst*) (GlobalConst);	
	...
	*last_beta = model->dC0;	
	...
	{
		*limit_down = -1.0 / (2.0 * CalibConst->cPrecalc->dVarFFx[CalibConst->iSmileIndex]) * 1.001;
	}
	else
	{
		*limit_down = -1.0 / (2.0 * 0.1 * 0.1 * model->dPWTime[CalibConst->lSigIndex[CalibConst->iSmileIndex]]) * 1.001;
	}
...
Err		err = NULL;
	...
	{
		shift = model->dSigmaFx[CalibConst->lSigIndex[CalibConst->cPrecalc->iNbDone - 1]]
			- CalibConst->first_guess_for_smile;
		
		if (CalibConst->iNbOpt > CalibConst->cPrecalc->iNbDone)
		{
			model->dSigmaFx[CalibConst->lSigIndex[CalibConst->cPrecalc->iNbDone]] += shift;
		}
	}
	...
	*/
	...
	CalibConst->CalibFunctionsForVol->ExtrapolParam = CalibConst->ExtrapolAfterSetBeta;

	err = CalibrateParamTS(	CalibConst->cPrecalc->iNbDone,
							CalibConst->iNbOpt-1,
							CalibConst->cAllInst,
							CalibConst->cAllInstPrecalc,
							CalibConst, 

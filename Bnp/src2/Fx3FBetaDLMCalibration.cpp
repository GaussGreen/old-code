    ...
    FxBetaDLM_CalibConst*     cCalibConst)
{
    ...
    cCalibConst->cPrecalc  = (FxBetaDLM_ModelPrecalc*)calloc(1, sizeof(FxBetaDLM_ModelPrecalc));
    cCalibConst->lSigIndex = (long*)calloc(iNbOpt, sizeof(long));
    cCalibConst->cAllInstPrecalc =
        (FxBetaDLM_InstPrecalc**)calloc(iNbOpt, sizeof(FxBetaDLM_InstPrecalc*));
    ...
    for (i = 0; i < iNbOpt; i++)
    {
        ...
        cCalibConst->cAllInstPrecalc[i] =
            (FxBetaDLM_InstPrecalc*)calloc(1, sizeof(FxBetaDLM_InstPrecalc));
        ...
        err = FxBetaDLM_Fill_InstPrecalc(Inst, cModel, cCalibConst->cAllInstPrecalc[i]);
    ...
    cCalibConst->CalibParams        = (CALIBGEN_Params*)calloc(1, sizeof(CALIBGEN_Params));
    cCalibConst->CalibParamsForBeta = (CALIBGEN_Params*)calloc(1, sizeof(CALIBGEN_Params));
    ...
    err = Initialise_CalibParams(1, precision, 10, 0, 1, 0, 0, 0.0, cCalibConst->CalibParams);

    if (err)
        goto FREE_RETURN;

    err = Initialise_CalibParams(
        1, precision, 10, 1, iCalibSmile, 1, 0, 0.0, cCalibConst->CalibParamsForBeta);

    if (err)
        goto FREE_RETURN;

    cCalibConst->CalibFunctionsForVol  = (CalibFunctions*)calloc(1, sizeof(CalibFunctions));
    cCalibConst->CalibFunctionsForBeta = (CalibFunctions*)calloc(1, sizeof(CalibFunctions));
...

{
    ...
    Err           err = NULL;
    int           nb_merge_dates;
    double *null_sigma_fx = NULL, *exercise_opt = NULL, *maturity_opt = NULL, *merge_dates = NULL,
           *sig_dom = NULL, *sig_for = NULL, *corr_dom_for_3F = NULL, *corr_dom_fx_3F = NULL,
           *corr_for_fx_3F = NULL, max_time_correl = 0.0, min_time_space_correl = 0.0;
    FxBetaDLM_model*   model   = NULL;
    FxBetaDLM_Hermite* hermite = NULL;

    int i, index;

    model   = (FxBetaDLM_model*)calloc(1, sizeof(FxBetaDLM_model));
    hermite = (FxBetaDLM_Hermite*)calloc(1, sizeof(FxBetaDLM_Hermite));

    str           = (FxBetaDLM_str*)calloc(1, sizeof(FxBetaDLM_str));
    null_sigma_fx = (double*)calloc(nb_opt, sizeof(double));
    exercise_opt  = (double*)calloc(nb_opt, sizeof(double));
    maturity_opt  = (double*)calloc(nb_opt, sizeof(double));
    ...
    */
        ...
        else
        {
            model->dSigmaFxUp[i] =
                ...
                exp(model->dAlpha * sqrt(
                                        (1.0 - exp(-2.0 * model->dLambda * exercise_opt[index])) /
                                        (2.0 * model->dLambda)));
            model->dSigmaFxDown[i] =
                model->dSigmaFx[i] *
                exp(-model->dAlpha * sqrt(
    ...
    {
        (*fx_vols)[i] =
            model->dSigmaFx[Get_Index(exercise_opt[i], model->dPWTime, model->iNbPWTime)];
    }
    ...
    if (!dom_fx_DLM_corr || !for_fx_DLM_corr || !dom_fx_DLM_corr_down || !for_fx_DLM_corr_down ||
        !date_DLM_corr)
    {
                               ...
                               FxBetaDLM_Hermite*        hermite)
{
    ...
    AllInst = (void**)calloc(nb_opt, sizeof(void*));
    ...
    for (i = 0; i < nb_opt; i++)
    {
        ...
        err = FxBetaDLM_Allocate_FxOptInst(i, nb_strike, (FxBetaDLM_FxOptInst*)AllInst[i]);
    ...
    CalibConst = (FxBetaDLM_CalibConst*)calloc(1, sizeof(FxBetaDLM_CalibConst));
    ...
    err = FxBetaDLM_Fill_CalibConst(
        nb_opt, AllInst, calib_smile, index_smile, model, NumParams, hermite, CalibConst);
    ...
    if (calib_smile)
    {
        ...
        err = CalibrateParamTS(
            ...
            0,
            &(AllInst[index_smile]),
            (void**)&(CalibConst->cAllInstPrecalc[index_smile]),
            CalibConst,
            model,
    ...
    else
    {
        err = CalibrateParamTS(
            ...
            nb_opt - 1,
            AllInst,
            (void**)CalibConst->cAllInstPrecalc,
            CalibConst,
            model,
...
FREE_RETURN:
    ...
    if (AllInst)
    {
        for (i = 0; i < nb_opt; i++)
        {
            if (AllInst[i])
            {
                FxBetaDLM_Free_FxOptInst((FxBetaDLM_FxOptInst*)AllInst[i]);
                free(AllInst[i]);
                AllInst[i] = NULL;
            }
        }
...
Err FxBetaDLM_Fill_ModelPrecalc(FxBetaDLM_model* model, FxBetaDLM_CalibConst* CalibConst)
{
    Err    err = NULL;
    ...
    for (i = start_index; i < end_index + 1; i++)
    {
        ...
        covar_rates +=
            sig_domfor * Phi_Func(model->dLambdaDom + model->dLambdaFor, model->dTStar, t1, t2);
...
Err FxBetaDLM_Fill_InstPrecalc(
    FxBetaDLM_FxOptInst* Inst, FxBetaDLM_model* model, FxBetaDLM_InstPrecalc* CalibConst)
{
    CalibConst->dBetaDom =
        (1.0 - exp(-model->dLambdaDom * (Inst->dSettlementTime - model->dTStar))) /
        model->dLambdaDom;
    CalibConst->dBetaFor =
        (1.0 - exp(-model->dLambdaFor * (Inst->dSettlementTime - model->dTStar))) /
        model->dLambdaFor;
    ...
    FxBetaDLM_InstPrecalc*    InstConst)
{
    ...
    if (fabs(model->dAlpha) < TINY)
    {
        ...
        InstConst->dVarRatesFor = var_rates_for - 2.0 * model->dC0 *
    ...
    else /* Fill the constants with alpha fudge */
    {
        ...
        InstConst->dVarRatesFor_down =
            var_rates_for - 2.0 * model->dC0 * (1.0 + 2.0 * model->dC0 * var_0_fwdstart_down) *
...
Err FxBetaDLM_Update_ModelPrecalc(
    int index, FxBetaDLM_model* model, FxBetaDLM_CalibConst* CalibConst)
{
    Err    err = NULL;
    int    j, k;
    ...
    double sig_fx_down, sig_fx2_down, sig_domfx_down, sig_forfx_down, var_ffx_down,
        covar_dom_ffx_down, covar_for_ffx_down, expect_for_down, old_integ_down, new_integ_down,
        old_integ2_down, new_integ2_down, covar_dom_for_adjust_down, temp_bracket_down,
        for_fwd_down;

    double sig_fx_mid, sig_fx2_mid, sig_domfx_mid, sig_forfx_mid, var_ffx_mid, covar_dom_ffx_mid,
        covar_for_ffx_mid, expect_for_mid, old_integ_mid, new_integ_mid, old_integ2_mid,
        new_integ2_mid, covar_dom_for_adjust_mid, temp_bracket_mid, for_fwd_mid;
    ...
    else
    {
        ...
        for (j = start_index; j < end_index + 1; j++)
        {
            ...
            if (dt > 0.0)
            {
                ...
                for (k = 0; k < nb_points; k++)
                {
                    ...
                    temp_bracket_mid =
                        temp_for * (sig_forfx_mid - sig_for2 * (1.0 - temp_for) / for_lam +
                                    sig_domfor * (1.0 - temp_dom) / dom_lam);
        ...
        if (CalibConst->cPrecalc)
        {
            ...
            CalibConst->cPrecalc->dCovarRatesAdjust_down[CalibConst->cPrecalc->iNbDone] =
            ...
            CalibConst->cPrecalc->dCovarRatesAdjust_mid[CalibConst->cPrecalc->iNbDone] =
    ...
    FxBetaDLM_InstPrecalc*    InstConst)
{
    ...
    if (NumParams->iMethod == 1)
    {
        ...
        else
        {
            ...
            InstConst->dFwdBeta =
                exp(0.5 * (varY * InstConst->dVarFFx - covFFxY * covFFxY) / InstConst->dVarFFx) /
        ...
        if (NumParams->iMethod == 2)
    {
        ...
        if (InstConst->iIsCPD)
        {
            InstConst->coef_b_const =
                model->dB0 / temp - 2.0 * coef_c * InstConst->dCovarFFxDom * InstConst->dBetaDom;
...
Err FxBetaDLM_Update_InstPrecalc(
    FxBetaDLM_FxOptInst*   Inst,
    FxBetaDLM_model*       model,
    ...
    FxBetaDLM_InstPrecalc* InstConst)
{
    ...
    err = FxBetaDLM_Update_InstPrecalc_FromMoment(Inst, model, CalibConst->cNumParams, InstConst);
...
Err FxBetaDLM_GetTargetVol(
    void* Inst_, void* GlobalConst, void* Model, CALIBGEN_PARAMS CalibConsts, double* target)
{
...
Err FxBetaDLM_GetFirstGuessVol(
    void* Model_, void* GlobalConst, int index_param, double target, double* vol1)
{
...
Err FxBetaDLM_GetSecondGuessVol(
    void*   Model_,
    void*   GlobalConst,
    int     vol_index,
    double  vol1,
...
Err FxBetaDLM_GetLimitAndLastVol(
    void*           Model_,
...
Err FxBetaDLM_BumpVol(void* Model_, void* GlobalConst, int vol_index, double vol)
{
    FxBetaDLM_model*      model      = (FxBetaDLM_model*)(Model_);
    FxBetaDLM_CalibConst* CalibConst = (FxBetaDLM_CalibConst*)(GlobalConst);
    ...
    for (i = last_index; i <= new_index; i++)
    {
        ...
        {
            model->dSigmaFxUp[i] =
                vol * exp(model->dAlpha *
                          sqrt(
                              (1.0 - exp(-2.0 * model->dLambda * model->dPWTime[new_index])) /
                              (2.0 * model->dLambda)));
            model->dSigmaFxDown[i] =
                vol * exp(-model->dAlpha *
                          sqrt(
        ...
        if (fabs(model->dAlpha) < TINY)
        {
            ...
            else if (CalibConst->cNumParams->iMappingMethod == 2)
            {
                err = FxBetaDLM_correl_mapping_forward(
                    CalibConst->cNumParams->dFFxCorrelTolerance,
                    model->dPWTime[i],
                    !i ? 0.0 : model->dPWTime[i - 1],
                    model->dB0,
                    model->dC0,
            ...
            varFFx += -2.0 * model->dCorrForFx[i] * model->dSigmaFor[i] * model->dSigmaFx[i] *
                          Etha_Func(model->dLambdaFor, model->dTStar, t1, t2) +
                      2.0 * model->dCorrDomFx[i] * model->dSigmaDom[i] * model->dSigmaFx[i] *
                          Etha_Func(model->dLambdaDom, model->dTStar, t1, t2);
            varFFx += model->dSigmaFor[i] * model->dSigmaFor[i] *
                          Psi_Func(model->dLambdaFor, model->dLambdaFor, model->dTStar, t1, t2) +
                      model->dSigmaDom[i] * model->dSigmaDom[i] *
                          Psi_Func(model->dLambdaDom, model->dLambdaDom, model->dTStar, t1, t2) -
                      2.0 * model->dCorrDomFor[i] * model->dSigmaDom[i] * model->dSigmaFor[i] *
                          Psi_Func(model->dLambdaDom, model->dLambdaFor, model->dTStar, t1, t2);
        }
        else
        {
            /* transform correlations */
            ...
            varFFxUp += -2.0 * model->dCorrForFx[i] * model->dSigmaFor[i] * model->dSigmaFxUp[i] *
                            Etha_Func(model->dLambdaFor, model->dTStar, t1, t2) +
                        2.0 * model->dCorrDomFx[i] * model->dSigmaDom[i] * model->dSigmaFxUp[i] *
                            Etha_Func(model->dLambdaDom, model->dTStar, t1, t2);
            ...
            varFFxDown +=
                -2.0 * model->dCorrForFxDown[i] * model->dSigmaFor[i] * model->dSigmaFxDown[i] *
                    Etha_Func(model->dLambdaFor, model->dTStar, t1, t2) +
                2.0 * model->dCorrDomFxDown[i] * model->dSigmaDom[i] * model->dSigmaFxDown[i] *
                    Etha_Func(model->dLambdaDom, model->dTStar, t1, t2);
            varFFxDown +=
                model->dSigmaFor[i] * model->dSigmaFor[i] *
                    Psi_Func(model->dLambdaFor, model->dLambdaFor, model->dTStar, t1, t2) +
                model->dSigmaDom[i] * model->dSigmaDom[i] *
                    Psi_Func(model->dLambdaDom, model->dLambdaDom, model->dTStar, t1, t2) -
                2.0 * model->dCorrDomFor[i] * model->dSigmaDom[i] * model->dSigmaFor[i] *
...
Err FxBetaDLM_SetVol(
    void* Model_, CALIBGEN_PARAMS CalibConsts, void* GlobalConst, int vol_index, double vol)
{
    FxBetaDLM_model*      model      = (FxBetaDLM_model*)(Model_);
    FxBetaDLM_CalibConst* CalibConst = (FxBetaDLM_CalibConst*)(GlobalConst);
...
Err FxBetaDLM_ExtrapolVol_WhenBetaCalib(void* Model_, void* GlobalConst, int last_vol_index)
{
    Err err = NULL;
...
Err FxBetaDLM_UpdateConstsAfterVol(
    void* Inst_, void* InstConst_, void* GlobalConst, void* Model_, CALIBGEN_PARAMS CalibConsts)
{
    FxBetaDLM_FxOptInst*   Inst       = (FxBetaDLM_FxOptInst*)(Inst_);
    FxBetaDLM_model*       model      = (FxBetaDLM_model*)(Model_);
    FxBetaDLM_CalibConst*  CalibConst = (FxBetaDLM_CalibConst*)(GlobalConst);
...
Err FxBetaDLM_PriceInstVol(
    void* Inst_, void* InstConst_, void* GlobalConst, void* Model_, double* InstPrice)
{
    FxBetaDLM_FxOptInst*   Inst       = (FxBetaDLM_FxOptInst*)(Inst_);
    FxBetaDLM_model*       model      = (FxBetaDLM_model*)(Model_);
    FxBetaDLM_CalibConst*  CalibConst = (FxBetaDLM_CalibConst*)(GlobalConst);
    ...
    err = FxBetaDLM_Price_FxOptInst(
        Inst, model, InstConst, CalibConst->cHermite, CalibConst->cNumParams, &(Result[0]));
...
Err FxBetaDLM_GetFirstGuessBeta(
    void* Model_, void* GlobalConst, int index_param, double target, double* beta1)
{
    FxBetaDLM_model*      model      = (FxBetaDLM_model*)(Model_);
    FxBetaDLM_CalibConst* CalibConst = (FxBetaDLM_CalibConst*)(GlobalConst);
...
Err FxBetaDLM_GetSecondGuessBeta(
    void*   Model_,
    void*   GlobalConst,
    int     beta_index,
    double  beta1,
...
Err FxBetaDLM_GetLimitAndLastBeta(
    void*           Model_,
    CALIBGEN_PARAMS CalibParams,
    void*           GlobalConst,
    ...
    double* limit_up)
{
    ...
    *last_beta = model->dC0;
    ...
    else
    {
        *limit_down =
            -1.0 /
            (2.0 * 0.1 * 0.1 * model->dPWTime[CalibConst->lSigIndex[CalibConst->iSmileIndex]]) *
            1.001;
    }
...
Err FxBetaDLM_BumpBeta(void* Model_, void* GlobalConst, int beta_index, double beta)
{
    FxBetaDLM_model*      model      = (FxBetaDLM_model*)(Model_);
    FxBetaDLM_CalibConst* CalibConst = (FxBetaDLM_CalibConst*)(GlobalConst);
...
Err FxBetaDLM_SetBeta(
    void* Model_, CALIBGEN_PARAMS CalibConsts, void* GlobalConst, int beta_index, double beta)
{
    FxBetaDLM_model*      model      = (FxBetaDLM_model*)(Model_);
    FxBetaDLM_CalibConst* CalibConst = (FxBetaDLM_CalibConst*)(GlobalConst);
...
Err FxBetaDLM_ExtrapolBeta(void* Model_, void* GlobalConst, int last_beta_index)
{
    return NULL;
}

Err FxBetaDLM_UpdateConstsAfterBeta(
    void* Inst_, void* InstConst_, void* GlobalConst, void* Model_, CALIBGEN_PARAMS CalibConsts)
{
    FxBetaDLM_FxOptInst*   Inst       = (FxBetaDLM_FxOptInst*)(Inst_);
    FxBetaDLM_model*       model      = (FxBetaDLM_model*)(Model_);

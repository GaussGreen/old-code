...
CRIFUtil.cxx
Routines:
CRIF_calcHistory:  from the historical fixings        , calculate what has
happened.
*/
#include "CRIFUtil.h"
...
Err crif_calculate_option(CMSCTS_MARKET sMarket, CRIF_DEAL sDeal,
  ...
  smessage("Launching MC        , Nb Steps: %d        , Nb Paths: %d",
           sMCArg->iNbTimes, sPricingParams->sSimulParams->lNbPaths);
...
/*
        ...
        err = cpd_calib_diagonal_dlm(	sMarket->cYcName        ,
         ...
         , sMarket->GetCashVol        , sMarket->cRefRateName        ,
                                                                        ...
                                                                        cPrimFreq
         , cPrimBasis        , cPrimRef        , lNbPrim        , lPrimExeDates
   , iPrimCalib        , cPrimTenors , sDeal->lTheoEndDate        , dPrimStrike
   , sCalibration->sPrimCalibParams        ,

                                                                        cSecFreq
         , cSecBasis        , cSecRef        , lNbSec        , lSecExeDates ,
   iSecCalib        , cSecTenors       , sDeal->lTheoEndDate , dSecStrike ,
   dSecWeights        , sCalibration->sSecCalibParams        ,

                                                                        sCalibration->sLGMSVCalibParams
   ->fix_lambda        , sCalibration->iNbFactor        , 0        , &dTime0 ,
   &dLambda        , 0        , 2        , sCalibration->dLGMAlpha ,
   ...
   "LGM": "LGM2F")        , iNbSigTime        , 2        , tsv        , 1 , 2 ,
   tst        , 0.0        , sCalibration->dLGMAlpha        ,

...
Err swp_f_CashSettledSwaption_SwapDP(SwapDP *swapdp, double vol, double strike,
  ...
  /* Makes the swap schedule        , and compute the first period fraction
   * (alpha) */
  /*	swap = make_SimpleSwap(swapdp        , 1.0        , 0.0        , 0.0 ,
     today        , SWAP); if (swap.dl.type == BROKEN)
          {
                  alpha = coverage(swap.dl.date[0]        , swap.dl.date[1] ,
     swapdp->basis_code); alpha /= coverage(swap.dl.prev        ,
     swap.dl.date[1]        , swapdp->basis_code); alpha *= (double)
     swapdp->compd;
          }
          else
          {
  ...
  */
  ...
  /* Compute the forward cash settled level        , using the forward swap rate
   * to discount */

    ...
    double* varff)
{
    ...
    for (i = StartIndex; i < EndIndex + 1; i++)
    {
        ...
        phidomfor +=
            sig_curve_dom[i] * sig_curve_for[i] * Phi_Func(lda_dom + lda_for, Tfix, t1, t2);
    }
    ...
    double* price)
{
    ...
    err = OptBlkSch(fx_fwd, strike, fx_vol, fix_time, df, pay_reveceive, "PREMIUM", price);
...

{
    ...
    err = srt_f_display_IRM_OneFac_TermStruct(
        ts, sigma_date, sigma, &beta, &vovol, &rho, &meanvol, sigma_n, tau_date, tau, tau_n);
...

{
    ...
    double *beta = NULL, *vovol = NULL, *rho = NULL, *meanvol = NULL, *TauDates = NULL, *Tau = NULL;
    ...
    err = srt_f_display_IRM_OneFac_TermStruct(
        ts, sigma_time, sigma, &beta, &vovol, &rho, &meanvol, sigma_n, &TauDates, &Tau, &nbTau);
    if (err)
    {
        goto FREE_RETURN;
    }
...

{
    ...
    double *beta1 = NULL, *sigma2 = NULL, *beta2 = NULL, *Rho = NULL, *TauDates = NULL,
           *Tau1 = NULL, *Tau2 = NULL;
    ...
    double*  fixed_rho)
{
    ...
    double *beta1 = NULL, *Sigma2 = NULL, *beta2 = NULL, *Rho = NULL, *Tau2 = NULL;
    ...
    double*  correl_for_fx)
{
    ...
    *sigma_fx       = NULL;
    ...
    domname = get_domname_from_fxund(und);
    err     = Get_LGM_TermStructure(
        domname, sigma_date_dom, sigma_dom, sigma_n_dom, tau_date_dom, tau_dom, tau_n_dom);
    if (err)
    {
        goto FREE_RETURN;
    }
    ...
    forname = get_forname_from_fxund(und);
    err     = Get_LGM_TermStructure(
        forname, sigma_date_for, sigma_for, sigma_n_for, tau_date_for, tau_for, tau_n_for);
    if (err)
    {
        goto FREE_RETURN;
    }
    ...
    double* adjust)
{
    ...
    for (i = StartIndex; i < EndIndex + 1; i++)
    {
        ...
        for (j = StartIndex2; j < EndIndex2 + 1; j++)
        {
            ...
            adjust_partial = correl_dom_fx * sig_dom * Etha_Func(lda_dom, Tpay, t1, t2) * sig_fx -
    ...
    double* expect)
{
    ...
    double *sigma_date_dom = NULL, *sigma_dom = NULL, *tau_date_dom = NULL, *tau_dom = NULL,
           *sigma_date_for = NULL, *sigma_for = NULL, *tau_date_for = NULL, *tau_for = NULL,
           *merge_dates = NULL, *sig_dom = NULL, *sig_for = NULL, *sigma_date_fx = NULL,
           *sigma_fx = NULL;
...
static double Psi3_Func(double x, double y, double t0, double T, double t1, double t2)
{
    ...
    else
    {
        res = 1.0 / y * (exp(-y * (t1 - t0)) - exp(-y * (t2 - t0)));
        res -= 1.0 / (y - x) *
               (exp(-x * T + y * t0 - (y - x) * t1) - exp(-x * T + y * t0 - (y - x) * t2));
        return res / x;
    }
}
    ...
    double* fx_vol)
{
    ...
    else
    {
        ...
        for (j = StartIndex2; j < EndIndex2 + 1; j++)
        {
            ...
            var_partial +=
                (sig0 - sig_inf) * (sig0 - sig_inf) * Phi_Func(-2.0 * lda_vol, t0, t1, t2);
    ...
    double* fx_vol)
{
    ...
    double  t0, sig0;
    double  correl_dom_for, correl_dom_fx, correl_for_fx;
    double *mat_rates_corr = NULL, *new_sig_curve_dom = NULL, *new_sig_curve_for = NULL,
           *new_correl_dom_for_ts = NULL, *new_correl_dom_fx_ts = NULL,
    ...
    nb_merged      = nbMat;
    mat_rates_corr = (double*)calloc(nb_merged, sizeof(double));
    ...
    double*  covar)
{
    ...
    for (i = StartIndex; i < EndIndex + 1; i++)
    {
        ...
        /* Dom_1 with Dom_2 */
        correl_fxfx[4] +=
            sig_1_dom * sig_2_dom * (Psi_Func(lda_dom1, lda_dom2, option_maturity, T1, T2));

        /* Dom_1 with For_2 */
        correl_fxfx[5] +=
            sig_1_dom * sig_2_for * (Psi_Func(lda_dom1, lda_for2, option_maturity, T1, T2));
        ...
        /* For_1 with Dom_2 */
        correl_fxfx[7] +=
            sig_1_for * sig_2_dom * (Psi_Func(lda_for1, lda_dom2, option_maturity, T1, T2));

        /* For_1 with For_2 */
        correl_fxfx[8] +=
            sig_1_for * sig_2_for * (Psi_Func(lda_for1, lda_for2, option_maturity, T1, T2));
    }

    *covar = correlations[2][5] * correl_fxfx[0] + correlations[2][3] * correl_fxfx[1] -
             correlations[2][4] * correl_fxfx[2] + correlations[0][5] * correl_fxfx[3] +
             correlations[0][3] * correl_fxfx[4] - correlations[0][4] * correl_fxfx[5] -
             correlations[1][5] * correl_fxfx[6] - correlations[1][3] * correl_fxfx[7] +
    ...
    double*   covar)
{
    ...
    for (i = StartIndex; i < EndIndex + 1; i++)
    {
        ...
        if (is_fx1 && is_fx2)
        {
            ...
            correl_fxfx[8] +=
                sig_1_for * sig_2_for *
                (Psi2_Func(lda_for1, lda_for2, option_maturity1, option_maturity2, T1, T2)) *
                correlations[1][4][i];
        }
        ...
        if (is_fx1)
        {
            ...
            /* For_1 with For_1 */
            var_fx1[8] +=
                sig_1_for * sig_1_for * (Psi_Func(lda_for1, lda_for1, option_maturity1, T1, T2));
            ...
            if (is_fx2)
            {
                ...
                correl_fxfx[7] +=
                    sig_1_for * sig_2_dom *
                    (Psi2_Func(lda_for1, lda_dom2, option_maturity1, option_maturity2, T1, T2)) *
                    correlations[1][3][i];
            }
            else
            {
                ...
                /* For_1 with Dom_2 */
                // correl_fxfx[7] += sig_1_for * sig_2_dom * (Zeta_Func(lda_dom2, lda_for1,
                // option_maturity, T1, T2))
                //	* correlations[1][3][i];
                correl_fxfx[7] +=
                    sig_1_for * sig_2_dom *
                    (Gamma2_Func(lda_for1, lda_dom2, option_maturity1, option_maturity2, T1, T2)) *
                    correlations[1][3][i];
            }
        }
        ...
        if (is_fx2)
        {
            ...
            /* For_2 with For_2 */
            var_fx2[8] +=
                sig_2_for * sig_2_for * (Psi_Func(lda_for2, lda_for2, option_maturity2, T1, T2));
            ...
            if (is_fx1)
            {
                ...
                correl_fxfx[5] +=
                    sig_1_dom * sig_2_for *
                    (Psi2_Func(lda_dom1, lda_for2, option_maturity1, option_maturity2, T1, T2)) *
                    correlations[0][4][i];
            }
            else
            {
                ...
                /* Dom_1 with For_2 */
                // correl_fxfx[5] += sig_1_dom * sig_2_for * (Zeta_Func(lda_dom1, lda_for2,
                // option_maturity, T1, T2))
                //	* correlations[0][4][i];
                correl_fxfx[5] +=
                    sig_1_dom * sig_2_for *
                    (Gamma2_Func(lda_for2, lda_dom1, option_maturity2, option_maturity1, T1, T2)) *
                    correlations[0][4][i];
            }
        }
        ...
        if (is_fx1 && is_fx2)
        {
            correl_fxfx[4] +=
                sig_1_dom * sig_2_dom *
                (Psi2_Func(lda_dom1, lda_dom2, option_maturity1, option_maturity2, T1, T2)) *
                correlations[0][3][i];
        }
        else if (is_fx1)
        {
            // correl_fxfx[4] += sig_1_dom * sig_2_dom * (Zeta_Func(lda_dom2, lda_dom1,
            // option_maturity, T1, T2))
            //	* correlations[0][3][i];
            correl_fxfx[4] +=
                sig_1_dom * sig_2_dom *
                (Gamma2_Func(lda_dom1, lda_dom2, option_maturity1, option_maturity2, T1, T2)) *
                correlations[0][3][i];
        }
        else if (is_fx2)
        {
            // correl_fxfx[4] += sig_1_dom * sig_2_dom * (Zeta_Func(lda_dom1, lda_dom2,
            // option_maturity, T1, T2))
            //	* correlations[0][3][i];
            correl_fxfx[4] +=
                sig_1_dom * sig_2_dom *
                (Gamma2_Func(lda_dom2, lda_dom1, option_maturity2, option_maturity1, T1, T2)) *
                correlations[0][3][i];
        }
        else
        {
            ...
            correl_fxfx[4] += sig_1_dom * sig_2_dom *
                              (Phi_Func(
                                  lda_dom1 + lda_dom2,
                                  (lda_dom1 * option_maturity1 + lda_dom2 * option_maturity2) /
                                      (lda_dom1 + lda_dom2),
                                  T1,
                                  T2)) *
                              correlations[0][3][i];
        ...
        {
            var_fx1[4] +=
                sig_1_dom * sig_1_dom * (Psi_Func(lda_dom1, lda_dom1, option_maturity1, T1, T2));
        }
        else
        {
            var_fx1[4] +=
                sig_1_dom * sig_1_dom * (Phi_Func(2.0 * lda_dom1, option_maturity1, T1, T2));
        }
        ...
        {
            var_fx2[4] +=
                sig_2_dom * sig_2_dom * (Psi_Func(lda_dom2, lda_dom2, option_maturity2, T1, T2));
        }
        else
        {
            var_fx2[4] +=
                sig_2_dom * sig_2_dom * (Phi_Func(2.0 * lda_dom2, option_maturity2, T1, T2));
        }
    }

    *covar = correl_fxfx[0] + correl_fxfx[1] - correl_fxfx[2] + correl_fxfx[3] + correl_fxfx[4] -
             correl_fxfx[5] - correl_fxfx[6] - correl_fxfx[7] + correl_fxfx[8];

    *var1 = var_fx1[0] + 2.0 * var_fx1[1] - 2.0 * var_fx1[2] + var_fx1[4] - 2.0 * var_fx1[7] +
            var_fx1[8];

    *var2 = var_fx2[0] - 2.0 * var_fx2[2] + 2.0 * var_fx2[3] + var_fx2[4] - 2.0 * var_fx2[5] +
            var_fx2[8];

    return err;
}

Err display_FX1F_TermStruct(char* szFxUndName, long* sigma_n, double** sigma_time, double** sigma)

{
    ...
    *sigma      = (double*)malloc(sizeof(double));
    ...
    {
        (*sigma_n)++;
        *sigma_time = (double*)realloc(*sigma_time, (*sigma_n) * sizeof(double));
        *sigma      = (double*)realloc(*sigma, (*sigma_n) * sizeof(double));
        (*sigma_time)[*sigma_n - 1] =
            ((double)(((FxTermStructVal*)l->element->val.pval)->date) - today) * YEARS_IN_DAY;
    ...
    double* quanto_adj)
{
    ...
    double *domsigtime = NULL, *forsigtime = NULL, *domsig = NULL, *forsig = NULL,
           *mergesigtime = NULL, *mergedomsig = NULL, *mergeforsig = NULL, *mergecorr = NULL,
    ...
    for (i = StartIndex; i < EndIndex + 1; i++)
    {
        ...
        I2 +=
            mergecorr[i] * mergedomsig[i] * mergedomsig[i] *
            (exp(dom_lam * t2) - exp(dom_lam * t1) -
             exp(-dom_lam * pay_time) * (exp(2.0 * dom_lam * t2) - exp(2.0 * dom_lam * t1)) / 2.0);
        I3 += mergedomsig[i] * mergeforsig[i] *
              ((exp(dom_lam * t2) - exp(dom_lam * t1)) / dom_lam -
               exp(-for_lam * pay_time) *
                   ...
                   (dom_lam + for_lam));
        I4 += mergedomsig[i] * mergedomsig[i] * (exp(2.0 * dom_lam * t2) - exp(2.0 * dom_lam * t1));
    }

    I1 *= corr_spot_fx_dom * sigma_s * (exp(-dom_lam * start_time) - exp(-dom_lam * end_time)) /
          dom_lam / dom_lam / (end_time - start_time);
    I2 *= (exp(-dom_lam * start_time) - exp(-dom_lam * end_time)) / dom_lam / dom_lam / dom_lam /
          (end_time - start_time);
    I3 *= corr_dom_for * (exp(-dom_lam * start_time) - exp(-dom_lam * end_time)) / dom_lam /
    ...
    double* quanto_adj)
{
    ...
    double *domsigtime = NULL, *forsigtime = NULL, *domsig = NULL, *forsig = NULL,
           *mergesigtime = NULL, *mergedomsig = NULL, *mergeforsig = NULL, *mergecorr = NULL,
           *mergecorrdomfor = NULL, *mergecorrdomfx = NULL, *mergecorrforfx = NULL,
           *temp_fx_vol_curve = NULL;
    double sigma_s;
    ...
    I1 *= sigma_s * (exp(-dom_lam * start_time) - exp(-dom_lam * end_time)) / dom_lam / dom_lam /
          (end_time - start_time);
    I2 *= (exp(-dom_lam * start_time) - exp(-dom_lam * end_time)) / dom_lam / dom_lam / dom_lam /
          (end_time - start_time);
    I3 *= (exp(-dom_lam * start_time) - exp(-dom_lam * end_time)) / dom_lam / for_lam /
          (end_time - start_time);
    I4 *= ((exp(-dom_lam * start_time) - exp(-dom_lam * end_time)) / dom_lam /
           (end_time - start_time)) *
          ((exp(-dom_lam * start_time) - exp(-dom_lam * end_time)) / dom_lam /
    ...
    long*    correl_n)
{
    ...
    *correl_for_fx  = NULL;
    ...
    err = srt_f_display_FX_TermStruct(
        underlying, sigma_n_fx, sigma_date_fx, sigma_fx, correl_n, correl_date, &corr);
    if (err)
    {
        goto FREE_RETURN;
    }
    ...
    *correl_dom_for = (double*)calloc(*correl_n, sizeof(double));
    *correl_dom_fx  = (double*)calloc(*correl_n, sizeof(double));
    *correl_for_fx  = (double*)calloc(*correl_n, sizeof(double));
    ...
    double* expect)
{
    ...
    double *sigma_date_dom = NULL, *sigma_dom = NULL, *tau_date_dom = NULL, *tau_dom = NULL,
           *sigma_date_for = NULL, *sigma_for = NULL, *tau_date_for = NULL, *tau_for = NULL,
           *correl_date = NULL, *correl_dom_for = NULL, *correl_dom_fx = NULL,
           *correl_for_fx = NULL, *merge_dates = NULL, *sig_dom = NULL, *sig_for = NULL,
    ...
    double* correl)
{
    ...
    double *sigma_date_dom1 = NULL, *sigma_dom1 = NULL, *tau_date_dom1 = NULL, *tau_dom1 = NULL,
           *sigma_date_dom2 = NULL, *sigma_dom2 = NULL, *tau_date_dom2 = NULL, *tau_dom2 = NULL,
           *sigma_date_for1 = NULL, *sigma_for1 = NULL, *tau_date_for1 = NULL, *tau_for1 = NULL,
           *sigma_fx1 = NULL, *sigma_date_for2 = NULL, *sigma_for2 = NULL, *tau_date_for2 = NULL,
           *tau_for2 = NULL, *sigma_fx2 = NULL, *sigma_date_fx1 = NULL, *sigma_date_fx2 = NULL,
           *corr1 = NULL, *corr_date1 = NULL, *corr2 = NULL, *corr_date2 = NULL, *all_dates = NULL;

    double lda_dom1, lda_dom2, lda_for1, lda_for2;
    long sigma_n_dom1, tau_n_dom1, sigma_n_dom2, tau_n_dom2, sigma_n_for1, tau_n_for1, sigma_n_for2,
        tau_n_for2, sigma_n_fx1, sigma_n_fx2, nb_corr_date1, nb_corr_date2;
    int nb_dates;
    ...
    err = srt_f_display_FX_TermStruct(
        fx_und1, &sigma_n_fx1, &sigma_date_fx1, &sigma_fx1, &nb_corr_date1, &corr_date1, &corr1);
    if (err)
    {
        goto FREE_RETURN;
    }
    ...
    err = srt_f_display_FX_TermStruct(
        fx_und2, &sigma_n_fx2, &sigma_date_fx2, &sigma_fx2, &nb_corr_date2, &corr_date2, &corr2);
    if (err)
    {
        goto FREE_RETURN;
    }
    ...
    all_dates = (double*)calloc(sigma_n_dom1, sizeof(double));
    if (!all_dates)
    {
    ...
    sig_curve_dom1 = (double*)calloc(nb_dates, sizeof(double));
    sig_curve_for1 = (double*)calloc(nb_dates, sizeof(double));
    sig_curve_fx1  = (double*)calloc(nb_dates, sizeof(double));
    sig_curve_dom2 = (double*)calloc(nb_dates, sizeof(double));
    sig_curve_for2 = (double*)calloc(nb_dates, sizeof(double));
    sig_curve_fx2  = (double*)calloc(nb_dates, sizeof(double));
    ...
    for (i = 0; i < nb_dates; i++)
    {
        ...
        err = srt_f_get_corr_from_CorrList(
            corr_list, dom_name1, for_name1, all_dates[i], &(correlations[0][1][i]));
        if (err)
        {
            goto FREE_RETURN;
        }
        ...
        err = srt_f_get_corr_from_CorrList(
            corr_list, dom_name1, fx_und1, all_dates[i], &(correlations[0][2][i]));
        if (err)
        {
            goto FREE_RETURN;
        }
        ...
        err = srt_f_get_corr_from_CorrList(
            corr_list, dom_name1, dom_name2, all_dates[i], &(correlations[0][3][i]));
        if (err)
        {
            goto FREE_RETURN;
        }
        ...
        err = srt_f_get_corr_from_CorrList(
            corr_list, dom_name1, for_name2, all_dates[i], &(correlations[0][4][i]));
        if (err)
        {
            goto FREE_RETURN;
        }
        ...
        err = srt_f_get_corr_from_CorrList(
            corr_list, dom_name1, fx_und2, all_dates[i], &(correlations[0][5][i]));
        if (err)
        {
            goto FREE_RETURN;
        }
        ...
        err = srt_f_get_corr_from_CorrList(
            corr_list, for_name1, fx_und1, all_dates[i], &(correlations[1][2][i]));
        if (err)
        {
            goto FREE_RETURN;
        }
        ...
        err = srt_f_get_corr_from_CorrList(
            corr_list, for_name1, dom_name2, all_dates[i], &(correlations[1][3][i]));
        if (err)
        {
            goto FREE_RETURN;
        }
        ...
        err = srt_f_get_corr_from_CorrList(
            corr_list, for_name1, for_name2, all_dates[i], &(correlations[1][4][i]));
        if (err)
        {
            goto FREE_RETURN;
        }
        ...
        err = srt_f_get_corr_from_CorrList(
            corr_list, for_name1, fx_und2, all_dates[i], &(correlations[1][5][i]));
        if (err)
        {
            goto FREE_RETURN;
        }
        ...
        err = srt_f_get_corr_from_CorrList(
            corr_list, fx_und1, dom_name2, all_dates[i], &(correlations[2][3][i]));
        if (err)
        {
            goto FREE_RETURN;
        }
        ...
        err = srt_f_get_corr_from_CorrList(
            corr_list, fx_und1, for_name2, all_dates[i], &(correlations[2][4][i]));
        if (err)
        {
            goto FREE_RETURN;
        }
        ...
        err = srt_f_get_corr_from_CorrList(
            corr_list, dom_name2, for_name2, all_dates[i], &(correlations[3][4][i]));
        if (err)
        {
            goto FREE_RETURN;
        }
        ...
        err = srt_f_get_corr_from_CorrList(
            corr_list, dom_name2, fx_und2, all_dates[i], &(correlations[3][5][i]));
        if (err)
        {
            goto FREE_RETURN;
        }
        ...
        err = srt_f_get_corr_from_CorrList(
            corr_list, fx_und1, fx_und2, all_dates[i], &(correlations[2][5][i]));
        if (err)
        {
            goto FREE_RETURN;
        }
        ...
        err = srt_f_get_corr_from_CorrList(
            corr_list, for_name2, fx_und2, all_dates[i], &(correlations[4][5][i]));
        if (err)
        {
            goto FREE_RETURN;
        }
    ...
    double* expect)
{
    ...
    double *sigma_date_dom = NULL, *sigma_dom = NULL, *tau_date_dom = NULL, *tau_dom = NULL,
           *sigma_date_for = NULL, *sigma_for = NULL, *tau_date_for = NULL, *tau_for = NULL,
           *merge_dates = NULL, *sig_dom = NULL, *sig_for = NULL, *sigma_date_fx = NULL,
           *sigma_fx = NULL, *correl_date = NULL, *correl_dom_for = NULL, *correl_dom_fx = NULL,
           *correl_for_fx = NULL;
    ...
    double* correl)
{
    ...
    double lda_dom1, lda_dom2, lda_for1, lda_for2;
    long   sigma_n_dom1, sigma_n_dom2, sigma_n_for1, sigma_n_for2, sigma_n_fx1, sigma_n_fx2,
    ...
    /* Get all the term structures */
    err =
        Get_LGM_TermStructure2(dom_name1, &sigma_date_dom1, &sigma_dom1, &sigma_n_dom1, &lda_dom1);
    ...
    {
        err = Get_LGM_TermStructure2(
            for_name1, &sigma_date_for1, &sigma_for1, &sigma_n_for1, &lda_for1);
    ...
    err =
        Get_LGM_TermStructure2(dom_name2, &sigma_date_dom2, &sigma_dom2, &sigma_n_dom2, &lda_dom2);
    ...
    {
        err = Get_LGM_TermStructure2(
            for_name2, &sigma_date_for2, &sigma_for2, &sigma_n_for2, &lda_for2);
    ...
    if (!sig_curve_dom1 || !sig_curve_dom2 || (is_fx1 && (!sig_curve_for1 || !sig_curve_fx1)) ||
    ...
    for (i = 0; i < nb_dates; i++)
    {
        ...
        if (is_fx1 && is_fx2)
        {
            ...
            err = srt_f_get_corr_from_CorrList(
                corr_list, for_name1, und2_name, all_dates[i], &(correlations[1][5][i]));
            if (err)
                goto FREE_RETURN;

            err = srt_f_get_corr_from_CorrList(
                corr_list, und1_name, for_name2, all_dates[i], &(correlations[2][4][i]));
            if (err)
                goto FREE_RETURN;

            err = srt_f_get_corr_from_CorrList(
                corr_list, und1_name, und2_name, all_dates[i], &(correlations[2][5][i]));
            if (err)
                goto FREE_RETURN;
        }
        ...
        if (is_fx1)
        {
            ...
            err = srt_f_get_corr_from_CorrList(
                corr_list, dom_name1, und1_name, all_dates[i], &(correlations[0][2][i]));
            if (err)
                goto FREE_RETURN;

            err = srt_f_get_corr_from_CorrList(
                corr_list, for_name1, und1_name, all_dates[i], &(correlations[1][2][i]));
            if (err)
                goto FREE_RETURN;
            ...
            err = srt_f_get_corr_from_CorrList(
                corr_list, und1_name, dom_name2, all_dates[i], &(correlations[2][3][i]));
            if (err)
                goto FREE_RETURN;
        }
        ...
        if (is_fx2)
        {
            ...
            err = srt_f_get_corr_from_CorrList(
                corr_list, dom_name2, und2_name, all_dates[i], &(correlations[3][5][i]));
            if (err)
                goto FREE_RETURN;

            err = srt_f_get_corr_from_CorrList(
                corr_list, for_name2, und2_name, all_dates[i], &(correlations[4][5][i]));
            if (err)
                goto FREE_RETURN;
            ...
            err = srt_f_get_corr_from_CorrList(
                corr_list, dom_name1, und2_name, all_dates[i], &(correlations[0][5][i]));
            if (err)
                goto FREE_RETURN;
        }
    }

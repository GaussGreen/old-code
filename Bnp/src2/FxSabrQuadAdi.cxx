...
Err FxSabrQuad_adi(
  ...
  if (bar_lvl) {
    ...
    // disc_linleft_logright_center(x        , nstepx        , spot_fx        ,
    // flx        , fux        , stdln        , NSTD_FX        , &index_x);
    // disc_linleft_linright_center(x        , nstepx        , spot_fx        ,
    // flx        , fux        , stdln        , NSTD_FX        , &index_x);
    // disc_Quad_center(x
    //       , nstepx        , spot_fx        , flx        , fux        , 0.9999
    //       , 0.00001        , stdln        , NSTD_FX , &index_x);
...

{
  ...
  if (*vol1 < 1.0E-08 || *vol2 < 1.0E-08 || *strike1 < 1.0E-08 ||
    ...
    *vol1 = butterfly + atm_vol - 0.5 * risk_reversal;
    //*strike1 = SolveForKPutPercent(-0.25        , fwd_fx        , *vol1 *
    //sqrt(mat_smile)
    //      , df_dom        , df_for);
    ...
    *vol2_out = vol2;
    ...
    while ((k < nbIterSmile) && (error >= precisionSmile)) {
      ...
      if (fabs(sensrho1) < 10E-06 || fabs(sensrho2) < 10E-06) {
        /*	No more sensitivity to rho        , it means that alpha is too
         * small
         */

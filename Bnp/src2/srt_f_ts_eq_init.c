...
/* -------------------------------------------------------------------------------
   The MAIN function to call when a TS has to be initialised for an EQD underlying
   ------------------------------------------------------------------------------- */

Err srt_f_init_EQ_TermStruct(	
		Date             today,
...
Err err;
	...
	if ((mdl_type == BLACK_SCHOLES) || (mdl_type == NORMAL_BS) || (mdl_type == FX_STOCH_RATES) || (mdl_type == EQ_STOCH_RATES)
		||(mdl_type == EQ_STOCH_RATES_SRVGS))
	{
		err = srt_f_init_EQ_NORMLOG_TermStruct(
					undname,
					ir_or_yc_name,
					mdl_type,
					ts,
...
Err srt_f_display_EQ_TermStruct(
		TermStruct *ts, 
		double **sigma_date, double **sigma, long *sigma_n)
...
/* Loop on all the elements of the Term Sturcture */
	...
	{	
		...
		*sigma = realloc (*sigma, (*sigma_n) * sizeof (double));
		(*sigma_date)[*sigma_n-1] = (double) (((EquityTermStructVal*)l->element->val.pval) -> date);
...
/* Current index for tau and sigma start at a date >= today */
	...
	/* Insert the TermStructAtom in the TS (==linked ts sorted by key = date) */
		srt_f_lstins(
				*ts,
				"BsTsAtom", 
				tsval->date, 
				OBJ_PTR_EQ_TermStruct,
				(void*)tsval,
				&srt_f_EquityTermStructValFree, 
				&ticker);
	}
	...
	if( (mdl_type == EQ_STOCH_RATES) || (mdl_type == EQ_STOCH_RATES_SRVGS) )
	{
		...
		if(ir_mdl_type == LGM)
		{
			...
			/* loops on the interest rate term structure dates to add them to the equity one */
			while(ir_atom != NULL)
			{
				...
				while((atom!=NULL) &&
						( ((EquityTermStructVal*)atom->element->val.pval)->time < ir_tsval->time) ) 
				atom = atom->next;
				if(atom == NULL)	atom = (*ts)->tail;
			...
			while(atom !=NULL)
			{
				...
				err = srt_f_get_corr_from_CorrList( corrlist,ir_und_or_yc_name, undname,tsval->time,&tsval->rho_e_ir);
				if (err)
				{
...
double tau,ir_sig,F;	
	...
	while( lc->next != NULL)
	{
		...
		c->V_ir_e = p->V_ir_e + p->rho_e_ir*p->sig*(ir_sig/F)*(exp(time/tau)-1)*tau;
...
double tau,ir_sig,F,Psi;	
	...
	while( lc->next != NULL)
	{
		...
		c->W_ir_e = p->W_ir_e + p->rho_e_ir*p->sig*ir_sig*(Psi/F)*(exp(time/tau)-1)*tau
							 + p->rho_e_ir*p->sig*ir_sig*tau*( (exp(time/tau)-1)*tau - time);

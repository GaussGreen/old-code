    ...
    TermStruct** ts)
{
    ...
    if ((mdl_type == BLACK_SCHOLES) || (mdl_type == NORMAL_BS) || (mdl_type == FX_STOCH_RATES) ||
...
Err srt_f_display_EQ_TermStruct(TermStruct* ts, double** sigma_date, double** sigma, long* sigma_n)
        ...
        if (ir_mdl_type == LGM)
        {
            ...
            while (atom != NULL)
            {
                ...
                err = srt_f_get_corr_from_CorrList(
                    corrlist, ir_und_or_yc_name, undname, tsval->time, &tsval->rho_e_ir);
                if (err)
                {
...
static Err make_W_ir_e_vector(TermStruct* l)
{
    ...
    while (lc->next != NULL)
    {
        ...
        c->W_ir_e = p->W_ir_e +
                    p->rho_e_ir * p->sig * ir_sig * (Psi / F) * (exp(time / tau) - 1) * tau +

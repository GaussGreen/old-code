/*	RainbowOpt.c
	Author: D. Mayevski
	Purpose: calculate E[min(nx*Xt - kX, ny*Yt - kY, 0)] with Xt, Yt lognormal (using 1D numerical integration)
*/
...
//	Calculates E[min(nx*Xt - kX, ny*Yt - kY, 0)] with Xt, Yt lognormal (using 1D numerical integration)

Err OptRainbow(double fwdx, double nx, double kx, double sigx,
			   double fwdy, double ny, double ky, double sigy,
			   double mat, double rho, double *res)
{
	Err			err = NULL;
	...
	if (fabs(ny) < 1e-8) return serror("nY is too close to zero");
	...
	if (d0 > umin)
	{
		...
		for (i=0; i < NPTS; i++)
		{
			...
			BS = srt_f_optblksch(fwdBS, strikeBS, volBS, 1.0, 1.0, (ny > 0.0 ? SRT_PUT : SRT_CALL), PREMIUM);
...
//	Calculates E[max(nx*Xt - ny*Yt - K, 0)] with Xt, Yt lognormal (using 1D numerical integration)

Err OptSpread(double fwdx, double nx, double sigx,
			  double fwdy, double ny, double sigy,
			  double K, double mat, double rho, double *res)
{
	Err			err = NULL;
	double		w[NPTS], u[NPTS];
	double		sqrtT = sqrt(mat);
...
Err OptSpreadSabrMC(double fwdx, double nx, double sigx, double alphax, double betax, double rhox,
					double fwdy, double ny, double sigy, double alphay, double betay, double rhoy,
					double K, double mat, double rho, long npaths, long nsteps, double *res, double *std)
{
	Err			err = NULL;
	...
	if (!sv || !corr || !chol || !pvs) { err = serror("Memory failure in OptSpreadSabrMC");  goto FREE_RETURN; }
	...
	for (i=0; i < N; i++)
	{
		for (j=0; j < npaths; j++)
		{
			...
			for (k=0; k < 4; k++)
				for (m=0; m <= k; m++) cbr[k] += chol[k][m] * br[m];
	...
	*res = 0.0;
	for (j=0; j < npaths; j++)
	{
		...
		if (pvs[j] < 0.0) pvs[j] = 0.0;
		*res += pvs[j] / npaths;
	}
	// Calculate std
	*std = 0.0;
	for (j=0; j < npaths; j++) *std += (pvs[j] - *res) * (pvs[j] - *res) / (npaths-1);
	*std = sqrt(*std / npaths);

FREE_RETURN:

	ABS_Free(&rg);
	if (sv) free_dmatrix(sv, 0, npaths-1, 0, 3);
	if (corr) free_dmatrix(corr, 0, 3, 0, 3);
	if (chol) free_dmatrix(chol, 0, 3, 0, 3);
	if (pvs) free_dvector(pvs, 0, npaths-1);
	return err;
}
...
/*	----------------------------------------------------------------------------------------------------
	if call 
		calculates E[max((nx*Xt + ny*Yt) - K, 0)] with Xt, Yt lognormal (using 1D numerical integration)
	if put  
		calculates E[max(K - (nx*Xt + ny*Yt), 0)] with Xt, Yt lognormal (using 1D numerical integration) 

	---------------------------------------------------------------------------------------------------- */

#define SwapVariables(Type,A,B) {Type C; C=A; A=B; B=C;}
	...
	else
	{
		...
		{		
			...
			for (i=0;i<NPTS;i++)
			{
				...
				sum += w[i]* fabs(ny)*srt_f_optblksch(fwdBS,
													  strikeBS,
													  volBS,
													  1.0,
													  1.0,
		...
		else 
		{
			...
			else
			{
				...
				{
					strikeBS = K/ny;
					*res = fabs(ny)*srt_f_optblksch(fwdy,
													strikeBS,
													sigy,
													mat,
													1.0,
				...
				else
				{
					/* ny = 0 */
					strikeBS = K/nx;
					*res = fabs(nx)*srt_f_optblksch(fwdx,
													strikeBS,
													sigx,
													mat,
													1.0,

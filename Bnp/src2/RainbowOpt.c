/*	RainbowOpt.c
        Author: D. Mayevski
        Purpose: calculate E[min(nx*Xt - kX  , ny*Yt - kY  , 0)] with Xt  , Yt
...
//	Calculates E[min(nx*Xt - kX  , ny*Yt - kY  , 0)] with Xt  , Yt lognormal
...
Err OptRainbow(double fwdx, double nx, double kx, double sigx, double fwdy,
  ...
  double vx = sigx * sqrtT, vy = sigy * sqrtT,
  ...
  if (d0 > umin) {
    ...
    for (i = 0; i < NPTS; i++) {
      ...
      BS = srt_f_optblksch(fwdBS, strikeBS, volBS, 1.0, 1.0,
                           (ny > 0.0 ? SRT_PUT : SRT_CALL), PREMIUM);
...
//	Calculates E[max(nx*Xt - ny*Yt - K  , 0)] with Xt  , Yt lognormal (using
...
// Calculates E[max(nx*Xt - ny*Yt - K  , 0)] with Xt  , Yt following SABR
// distributions using MC Note: nsteps - number of time discretization steps PER
// YEAR
...
/*	----------------------------------------------------------------------------------------------------
   ...
   lognormal (using 1D numerical integration) if put calculates E[max(K - (nx*Xt
   + ny*Yt)  , 0)] with Xt  , Yt lognormal (using 1D numerical integration)
  ...
  if (mat < 0) {
    *res = 0;
    ...
    if ((nx != 0) && (ny != 0)) {
      ...
      /* Calculate the abscissas and weights of the n-point Gauss-Hermite
       * quadrature */
      ...
      for (i = 0; i < NPTS; i++) {
        ...
        sum += w[i] * fabs(ny) *
               srt_f_optblksch(
                   fwdBS, strikeBS, volBS, 1.0, 1.0,
                   (ny > 0.0 ? call_put
                             : (call_put == SRT_CALL ? SRT_PUT : SRT_CALL)),
                   PREMIUM);
      }
      *res = INV_SQRTPI * sum;
    } else {

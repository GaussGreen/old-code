...
Err MY_LGM2F_Preliminary_Computation_For_Df(long today, long eventDate,
  ...
  for (i = 0; i < mylgm2f->NDates; ++i) {
    ...
    mylgm2f->AdjDf[i] =
        (swp_f_df(today, Dates[i], yc) / swp_f_df(today, eventDate, yc)) *
        exp(-0.5 * sigmaSquare);
  }
...
void MY_LGM2F_Compute_Df_From_Factors_Value(
    double fact1, double fact2,
    MYLGM2F mylgm2f) //(long today        , long eventDate        , double fact1
                     //, double fact2        , MYLGM2F mylgm2f        ,
                     // TermStruct* l        , char *yc)
{
  long i;

  for (i = 0; i < mylgm2f->NDates; ++i) {
    /*		mylgm2f->Df[i] = (mylgm2f->initDf[i] / swp_f_df(today        ,
     * eventDate        , yc)) exp(-0.5 * mylgm2f->sigmaSquare[i])
     * exp(-mylgm2f->lam1[i] * fact1) exp(-mylgm2f->lam2[i] * fact2);
     */
...
Err Fill_SWAP_FOR_LGM2F(long today, long SwapStartDate, long SwapEndDate,
  ...
  err = swp_f_initSwapDP(SwapStartDate, SwapEndDate, SwapFreq, SwapBasis,
                         &swapdp);
  if (err) {
  ...
  err = swp_f_make_FloatLegDatesCoveragesAndSpreads(
      &swapdp, today, RefRate, &float_pay_dates, &float_nb_pay_dates,
      &float_fixing_dates, &(swapForLgm2f->float_start_dates),
      &(swapForLgm2f->float_end_dates), &(swapForLgm2f->float_cvgs),
      &(swapForLgm2f->float_spreads), &float_nb_dates);
  if (err) {
  ...
  swapForLgm2f->float_pay_dates =
      (long *)calloc(float_nb_pay_dates - 1, sizeof(long));
  ...
  err = swp_f_make_FixedLegDatesAndCoverages(
      ...
      &fix_end_dates, &(swapForLgm2f->fix_cvgs), &fix_nb_dates);
  if (err) {
  ...
  swapForLgm2f->fix_pay_dates =
      (long *)calloc(fix_nb_pay_dates - 1, sizeof(long));
  ...
  if (err) {
    //		err = "Error in Fill_SWAP_FOR_LGM2F : Cannot fill fix_pay
    // MY_LGM2";
    goto FREE_RETURN;
  }
  ...
  if (err) {
    //		err = "Error in Fill_SWAP_FOR_LGM2F : Cannot fill float start
    // MY_LGM2";
    goto FREE_RETURN;
  }
  ...
  if (err) {
    //		err = "Error in Fill_SWAP_FOR_LGM2F : Cannot fill float end
    // MY_LGM2";
    goto FREE_RETURN;
  }
  ...
  if (err) {
    //		err = "Error in Fill_SWAP_FOR_LGM2F : Cannot fill float pay
    // MY_LGM2";
    goto FREE_RETURN;
  }
...
Err SWAP_FOR_LGM2F_Preliminary_Computation(long today, long eventDate,
  ...
  err = MY_LGM2F_Preliminary_Computation_For_Df(
      today, eventDate, swapForLgm2f->fix_pay_dates,
      &(swapForLgm2f->fix_pay_lgm2f), l, yc);
  if (err) {
  ...
  err = MY_LGM2F_Preliminary_Computation_For_Df(
      today, eventDate, swapForLgm2f->float_start_dates,
      &(swapForLgm2f->float_start_lgm2f), l, yc);
  if (err) {
  ...
  err = MY_LGM2F_Preliminary_Computation_For_Df(
      today, eventDate, swapForLgm2f->float_end_dates,
      &(swapForLgm2f->float_end_lgm2f), l, yc);
  if (err) {
  ...
  err = MY_LGM2F_Preliminary_Computation_For_Df(
      today, eventDate, swapForLgm2f->float_pay_dates,
      &(swapForLgm2f->float_pay_lgm2f), l, yc);
  if (err) {
...
double SWAP_FOR_LGM2F_SwapRate_From_Factors_Value(
    double fact1, double fact2,
    swap_for_lgm2f
        *swapForLgm2f) //(long today        , long eventDate        , double
                       // fact1        , double fact2        , swap_for_lgm2f
                       //*swapForLgm2f        , TermStruct* l        , char *yc)
{
  long i;
  double FloatPV;
  double LVL;

  MY_LGM2F_Compute_Df_From_Factors_Value(
      fact1, fact2,
      &(swapForLgm2f->fix_pay_lgm2f)); //(today        , eventDate        ,
                                       //fact1        , fact2        ,
  ...
  MY_LGM2F_Compute_Df_From_Factors_Value(
      fact1, fact2,
      &(swapForLgm2f->float_start_lgm2f)); //(today        , eventDate        ,
                                           //fact1        , fact2        ,
  ...
  MY_LGM2F_Compute_Df_From_Factors_Value(
      fact1, fact2,
      &(swapForLgm2f->float_end_lgm2f)); //(today        , eventDate        ,
                                         //fact1        , fact2        ,
                                         //&(swapForLgm2f->float_end_lgm2f) , l
                                         //, yc);

  MY_LGM2F_Compute_Df_From_Factors_Value(
      fact1, fact2,
      &(swapForLgm2f->float_pay_lgm2f)); //(today        , eventDate        ,
                                         //fact1        , fact2        ,
                                         //&(swapForLgm2f->float_pay_lgm2f) , l
                                         //, yc);

  LVL = 0;
  FloatPV = 0;
  for (i = 0; i < max(swapForLgm2f->fix_NDates, swapForLgm2f->float_NDates);
       ++i) {
    ...
    if (i < swapForLgm2f->float_NDates) {
      FloatPV += (swapForLgm2f->float_pay_lgm2f).Df[i] *
                 (swapForLgm2f->float_cvgs)[i] * swapForLgm2f->float_spreads[i];
      FloatPV += (swapForLgm2f->float_pay_lgm2f).Df[i] *
                 ((swapForLgm2f->float_start_lgm2f).Df[i] /
                      (swapForLgm2f->float_end_lgm2f).Df[i] -
                  1);
    }
  }
...
Err Fill_VolBondCoupon(long today, char *first_SwapFreq, char *first_SwapBasis,
                       char *first_SwapRefRate, char *second_SwapFreq,
                       char *second_SwapBasis, char *second_SwapRefRate,
                       double notional, long first_fixingDate,
                       long first_startDate, long first_endDate,
                       long second_fixingDate, long second_startDate,
                       long second_endDate, long payment_Date, double alphaC,
                       double alphaP, double betaC, double betaP,
  ...
  err = Fill_SWAP_FOR_LGM2F(today, second_startDate, second_endDate,
                            second_SwapFreq, second_SwapBasis,
                            second_SwapRefRate, &(volBondCpn->secondSwap));
  if (err) {
...
Err Fill_VolBondStruct(long today, long NbCoupons, double *notional,
                       ...
                       long *first_endDate, long *second_fixingDate,
                       long *second_startDate, long *second_endDate,
                       long *payment_Date, double *alphaC, double *alphaP,
                       double *betaC, double *betaP, double *strikeC,
                       double *strikeP, volBondStruct *volBond) {
  long i;
  Err err;
  ...
  volBond->VolBondCpn = NULL;
  volBond->VolBondCpn =
      (volBondCoupon *)calloc(NbCoupons, sizeof(volBondCoupon));
  if (!volBond->VolBondCpn) {
  ...
  for (i = 0; i < NbCoupons; ++i) {
    err = Fill_VolBondCoupon(
        today, first_SwapFreq[i], first_SwapBasis[i], first_SwapRefRate[i],
        second_SwapFreq[i], second_SwapBasis[i], second_SwapRefRate[i],
        notional[i], first_fixingDate[i], first_startDate[i], first_endDate[i],
        second_fixingDate[i], second_startDate[i], second_endDate[i],
        payment_Date[i], alphaC[i], alphaP[i], betaC[i], betaP[i], strikeC[i],
        strikeP[i], &(volBond->VolBondCpn[i]));
    if (err) {
      goto FREE_RETURN;
    }
  }
...
Err PayOff_VolBondCoupon(long today, double fact1, double fact2,
                         volBondCoupon *volBondCpn, double CMSAdj,
                         double sabrAlpha, double sabrBeta, double sabrBetaVol,
                         double sabrRho, double *Payoff, TermStruct *l,
                         char *yc) {
  Err err = NULL;
  double firstswap;
  double secondswap;

  firstswap = max(0.00000001, SWAP_FOR_LGM2F_SwapRate_From_Factors_Value(
                                  fact1, fact2, &(volBondCpn->firstSwap)));

  secondswap = max(0.00000001, SWAP_FOR_LGM2F_SwapRate_From_Factors_Value(
                                   fact1, fact2, &(volBondCpn->secondSwap)));

  *Payoff =
      volBondCpn->alphaC *
          srt_f_optblkschbetastochquick(
              secondswap * CMSAdj,
              (volBondCpn->betaC / volBondCpn->alphaC) * firstswap +
                  (volBondCpn->strikeC / volBondCpn->alphaC),
              (volBondCpn->second_fixingDate - volBondCpn->first_fixingDate) /
                  365.0,
              sabrBetaVol, sabrAlpha, sabrBeta, sabrRho, 1.0, SRT_BETAVOL,
              SRT_LOGNORMAL, SRT_CALL, PREMIUM)

      + volBondCpn->alphaP *
            srt_f_optblkschbetastochquick(
                secondswap * CMSAdj,
                (volBondCpn->betaP / volBondCpn->alphaP) * firstswap +
                    (volBondCpn->strikeP / volBondCpn->alphaP),
                (volBondCpn->second_fixingDate - volBondCpn->first_fixingDate) /
                    365.0,
                sabrBetaVol, sabrAlpha, sabrBeta, sabrRho, 1.0, SRT_BETAVOL,
                SRT_LOGNORMAL, SRT_PUT, PREMIUM);

...
/*
        ...
        procedure:
        1:	find joint distribution of rt (ie. -log(df)) at strike and spot
   fixing under appropriate probability measure (cms effect) 2:	denoting Y rt at
   strike and X rt at spot        , write the payoff of reset caplet as
   (a*exp(X) - b*exp(Y) - k)+ 3:	price caplets using spread numeraire
   (srt_f_optsprnum) 4:	for full caps: strip dates and call caplet pricing
   function
*/
...
Err srt_f_lgm_phifunc(SrtUndPtr undptr, Ddate fixing, double phi[2][2]) {
  long today_date;
  double time;
  double f, g, temp;
  SrtTFTSVec tf_f;
...
/*	compute moments of spot rate at fixing under Qpay measure
        model: LGM 1F and 2F
        undptr must point to ir underlying of type LGM */
static Err srt_f_lgm_spotratemoments(SrtUndPtr undptr, Ddate fixing, Ddate pay,
                                     double exp[2], double var[2][2]) {
  long today_date;
  ...
  if (mdldim == ONE_FAC) {
    ...
    exp[FIRST] = -(tf_psi_pay[FIRST] - tf_psi_fix[FIRST]) * tf_f_fix[FIRST] *
                     tf_g_fix[FIRST][FIRST] -
                 (tf_psi_pay[SECOND] - tf_psi_fix[SECOND]) * tf_f_fix[FIRST] *
                     tf_g_fix[FIRST][SECOND];
    exp[SECOND] = -(tf_psi_pay[SECOND] - tf_psi_fix[SECOND]) *
                      tf_f_fix[SECOND] * tf_g_fix[SECOND][SECOND] -
                  (tf_psi_pay[FIRST] - tf_psi_fix[FIRST]) * tf_f_fix[SECOND] *
                      tf_g_fix[SECOND][FIRST];
    var[FIRST][FIRST] =
        tf_f_fix[FIRST] * tf_f_fix[FIRST] * tf_g_fix[FIRST][FIRST];
    var[SECOND][SECOND] =
        tf_f_fix[SECOND] * tf_f_fix[SECOND] * tf_g_fix[SECOND][SECOND];
    var[FIRST][SECOND] =
        tf_f_fix[FIRST] * tf_f_fix[SECOND] * tf_g_fix[FIRST][SECOND];
...
/*	compute moments of R.T (zero rate times time period)
        ...
        model: LGM 1F and 2F
        undptr must point to ir underlying of type LGM */
static Err srt_f_lgm_zeroratemoments(SrtUndPtr undptr, Ddate fixing,
                                     Ddate start_end[2], Ddate pay, double *exp,
                                     double *var) {
  YTatt_param YT[2];
  ...
  if (mdldim == ONE_FAC) {
    *exp =
        YT[END].fwd_zero_rate - YT[START].fwd_zero_rate +
        (YT[END].phi_coeff[FIRST][FIRST] - YT[START].phi_coeff[FIRST][FIRST]) *
            phi[FIRST][FIRST] +
        (YT[END].x_coeff[FIRST] - YT[START].x_coeff[FIRST]) * spot_exp[FIRST];
    ...
    *exp =
        ...
        + (YT[END].phi_coeff[SECOND][SECOND] -
           YT[START].phi_coeff[SECOND][SECOND]) *
              phi[SECOND][SECOND]

        + (YT[END].phi_coeff[FIRST][SECOND] -
           YT[START].phi_coeff[FIRST][SECOND]) *
              phi[FIRST][SECOND]

        + (YT[END].phi_coeff[SECOND][FIRST] -
           YT[START].phi_coeff[SECOND][FIRST]) *
              phi[SECOND][FIRST]
...
/*	compute covariance of spot rate(s) at different times
        model: LGM 1F and 2F
        undptr must point to ir underlying of type LGM */
static Err srt_f_lgm_spotratecovar(SrtUndPtr undptr, Ddate fix1, Ddate fix2,
                                   double covar[2][2]) {
  long today_date;
  double time1, time2;
  ...
  if (mdldim == ONE_FAC) {
    ...
    covar[SECOND][SECOND] =
        tf_f1[SECOND] * tf_f2[SECOND] * tf_g1[SECOND][SECOND];
...
/*	compute covariance of two different R.T
        model: LGM 1F and 2F
        undptr must point to ir underlying of type LGM */
static Err srt_f_lgm_zeroratecovar(SrtUndPtr undptr, Ddate fix1, Ddate fix2,
                                   Ddate start_end1[2], Ddate start_end2[2],
                                   double *covar) {
  YTatt_param YT[2][2];
...
Err srt_f_lgm_resetcaplet(SrtUndPtr undptr, Ddate str_fix,
                          Ddate str_start_end[2], double str_cvg,
                          Ddate spot_fix, Ddate spot_start_end[2],
                          double spot_cvg, Ddate pay_date, double pay_level,
                          SrtReceiverType rec_pay, double *premium,
                          double fixed_strike) {
  double str_frt_exp, str_frt_var, spot_frt_exp, spot_frt_var,
      str_spot_frt_covar;
  ...
  /* cross */
  err = srt_f_lgm_zeroratecovar(undptr, str_fix, spot_fix, str_start_end,
                                spot_start_end, &str_spot_frt_covar);
  if (err) {
    return serror(err);
  }
  ...
  /* main call */
  if (sy > EPS)
  /* general case: strike Libor is not deterministic        , call
     srt_f_optsprnum */
  {
    if (rec_pay == SRT_STRADDLE) {
      *premium = srt_f_optsprnum(Fx, Fy, sx, sy, rho, 1.0, a, b, k, 1.0,
                                 SRT_CALL, PREMIUM) +
                 srt_f_optsprnum(Fx, Fy, sx, sy, rho, 1.0, a, b, k, 1.0,
                                 SRT_PUT, PREMIUM);
     ...
     call srt_f_optblksch */
  {
    if (rec_pay == SRT_STRADDLE) {
      *premium =
          srt_f_optblksch(a * Fx, b * Fy + k, sx, 1.0, 1.0, SRT_CALL, PREMIUM) +
          srt_f_optblksch(a * Fx, b * Fy + k, sx, 1.0, 1.0, SRT_PUT, PREMIUM);
    } else if (rec_pay == SRT_RECEIVER) {
      *premium =
          srt_f_optblksch(a * Fx, b * Fy + k, sx, 1.0, 1.0, SRT_PUT, PREMIUM);
    } else if (rec_pay == SRT_PAYER) {
...
double gen_ir_resetcap(SrtUndPtr undptr, long *fixing_date,
  ...
  if (fixing_date[0] >= today_date) {
    ...
    err = srt_f_lgm_resetcaplet(undptr, (Ddate)fixing_date[0], str_start_end,
                                cvg[1], fixing_date[1], spot_start_end, cvg[2],
                                pay, df[1] * cvg[1], rec_pay, &temp,
                                first_fixing);
  ...
  for (i = 1; i < nfp - 2; i++) {
    if (fixing_date[i] >= today_date) {
      ...
      err = srt_f_lgm_resetcaplet(undptr, (Ddate)fixing_date[i], str_start_end,
                                  cvg[i + 1], fixing_date[i + 1],
                                  spot_start_end, cvg[i + 2], pay,
                                  df[i + 1] * cvg[i + 1], rec_pay, &temp, 0.0);
...
/*
        ...
        to be implemented in srt_f_bsresetcap.cxx
*/

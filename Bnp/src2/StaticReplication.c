...
//
//	Creation of a robust algorithm to find the way to spread the tangent
//points if the function 	has a constant convexity.
//
//	11.11.2004 J. Dinh
//================================================================
...
//		by using a constant + list of calls + Put or Call for the
//extrapolation
//
//================================================================
Err Static_payoff_replication(double *tangent_points, int n_tangent_points,
                              double cvx_strike, int is_left_flat,
                              int is_right_flat,
                              Err (*payoff)(double S, double K, double *out),
                              Err (*deriv)(double S, double K, double *out),
                              double *constant, double *strikes,
                              double *notionals, int *is_call) {
  int i;
...
//================================================================
//
//	Get the Strikes for a smooth replication in the case of a monotonic
//derivative
//
//================================================================
Err SR_find_tangents(double start_point, double end_point, int index,
                     int nb_points,
                     Err (*deriv)(double S, double K, double *out),
                     double *final_points, double *middle_point,
                     int *left_points, int *right_points) {
  ...
  *middle_point = 0.5 * (start_point + end_point);
  ...
  if (nb_points > 1) {
    ...
    err = SR_find_tangents(start_point, *middle_point, index, *left_points,
                           deriv, final_points, &temp_middle, &temp_left_points,
                           &temp_right_points);
    if (err)
      goto FREE_RETURN;
    err = SR_find_tangents(*middle_point, end_point, index + *left_points,
                           *right_points, deriv, final_points, &temp_middle,
                           &temp_left_points, &temp_right_points);
    if (err)
      goto FREE_RETURN;
  } else
...
Err Convex_replication( // Market
    long Today, long fx_spot_date, double Spot, double *fx_mkt_vol,
    ...
    int nb_strikes,
    // Output
    double **Replication, double *Price) {
  ...
  err = cpd_fill_smile_vol_market(
      Today, smile_spec_type, num_fx_mkt_vol, NULL, fx_mkt_vol,
  ...
  err = Convex_replication_simple(
      Today, Forward, smile_params->sigma, smile_params->alpha,
      smile_params->beta, smile_params->rho, smile_params->pi,
      smile_params->smile_spec_type, is_reversed_fx, Notional, Fix_date,
      Settle_date, Kmin, Kmax, Convex_strike, nb_strikes, Replication, Price);
...
Err Convex_replication_simple( // Market
  ...
  // Tangents and vols
  double *pStrike = NULL, *pNotional = NULL, *tangent_points = NULL,
         *pVols = NULL;
  int *pIsCall = NULL;
  double dConstant;
  ...
  if (nb_tangents > 2) {
    err = SR_find_tangents(Kmin,
                           ...
                           1, nb_tangents - 2, convex_deriv, tangent_points,
                           &dtemp, &itemp1, &itemp2);
    if (err)
      goto FREE_RETURN;
  }
  ...
  //=========================================
  pIsCall =
      (int *)calloc(nb_strikes + 2,
                    sizeof(int)); //+2 for the interpolation on the left & right
  pStrike = (double *)calloc(
      nb_strikes + 2,
      sizeof(double)); //+2 for the interpolation on the left & right
  ...
  err = Static_payoff_replication(tangent_points, nb_tangents, Convex_strike, 0,
                                  0, convex_payoff, convex_deriv, &dConstant,
                                  pStrike, pNotional, pIsCall);
  ...
  for (i = 0; i < nb_strikes + 2; i++) {
    if (!is_reversed_fx) {
      err = cpd_vol_get_vol(Forward, Fix_time, pStrike[i], smile_params,
                            SRT_LOGNORMAL, &(pVols[i]));
      if (err)
        goto FREE_RETURN;
    } else {
      err = cpd_vol_get_vol(1 / Forward, Fix_time, 1 / pStrike[i], smile_params,
                            SRT_LOGNORMAL, &(pVols[i]));
      if (err)
        goto FREE_RETURN;
    }
  ...
  *Price = dConstant;
  ...
  for (i = 0; i < nb_strikes + 2; i++)
    *Price += pNotional[i] *
              srt_f_optblksch(Forward, pStrike[i], pVols[i], Fix_time, 1.0,
                              pIsCall[i] ? SRT_CALL : SRT_PUT, PREMIUM);

...
//
//================================================================
Err Static_payoff_replication(double	*tangent_points,
...
Err err = NULL;
	...
	*constant = payoff_val[0];
	...
	{
		strikes[i+1] = ((deriv_val[i]*tangent_points[i] - payoff_val[i])-(deriv_val[i+1]*tangent_points[i+1] - payoff_val[i+1]));
	...
	//Handels the left extrapolation
	if(!is_right_flat && payoff_val[n_tangent_points-1]>1.0e-10 && fabs(deriv_val[n_tangent_points-1])>1.0e-10)
	{
		notionals[n_tangent_points] = 0.0;
		
		is_call[n_tangent_points+2] = 1;
		strikes[n_tangent_points+2] = strikes[n_tangent_points] - payoff_val[n_tangent_points-1] / deriv_val[n_tangent_points-1] ;
...
FREE_RETURN:
	if(payoff_val) free(payoff_val);
	if(deriv_val) free(deriv_val);
...
//
//================================================================
Err SR_find_tangents(	double	start_point,
						double	end_point,
						int		index,
						int		nb_points,
  						Err (*deriv)(double S, double K, double *out),
...
Err err = NULL;
	...
	*middle_point = 0.5 * (start_point + end_point);
	...
	if(nb_points > 1)
	{
		err = deriv(start_point, 0.0, &left_deriv); if(err) goto FREE_RETURN;
		err = deriv(*middle_point, 0.0, &middle_deriv); if(err) goto FREE_RETURN;
		err = deriv(end_point, 0.0, &right_deriv); if(err) goto FREE_RETURN;

		coef = (left_deriv - middle_deriv) / (left_deriv - right_deriv);
		if(coef>0.9) coef=0.9;
		...
		err = SR_find_tangents(start_point, *middle_point,index,*left_points,deriv,final_points,&temp_middle,&temp_left_points,&temp_right_points); if(err) goto FREE_RETURN;
		err = SR_find_tangents(*middle_point, end_point,index+*left_points,*right_points,deriv,final_points,&temp_middle,&temp_left_points,&temp_right_points); if(err) goto FREE_RETURN;
	}
	else
...
Err err = NULL;
	...
	if(S==0) { err = "function undifined"; goto FREE_RETURN; }
...
Err err = NULL;
	...
	if(S==0)
	{
		err = "derivative undifined"; goto FREE_RETURN;
	}
...
Err Convex_replication(	//Market
						...
						int			num_fx_mkt_vol,
						int			smile_spec_type,		//	0: lognormal vol + SABR params, 1: sigma-beta + SABR params, 2: lognormal vol + BMM, 3: lognormal vol + BMM
						char		*for_yc,
						char		*dom_yc,
						int			is_reversed_fx,
						//Convex Data
...
Err err = NULL;
	...
	{
		err = "Convex_replication: Memory allocation failure"; if(err) goto FREE_RETURN;
	}
	...
	// memory allocation
	//=========================================
	err = cpd_alloc_smile_vol_market(num_fx_mkt_vol, smile_mkt); if(err) goto FREE_RETURN;
	...
	err = cpd_vol_get_smile_params(0, Fix_date, 0.0, smile_mkt, smile_params); if(err) goto FREE_RETURN;	
	err = cpd_vol_get_linterpVol(0, Fix_date, 0.0, smile_mkt, smile_params); if(err) goto FREE_RETURN;

	err = Convex_replication_simple(Today,Forward,
									smile_params->sigma, smile_params->alpha,
									smile_params->beta, smile_params->rho,smile_params->pi,
									smile_params->smile_spec_type,is_reversed_fx,
									Notional,Fix_date,Settle_date, Kmin, Kmax, Convex_strike, nb_strikes, 
									Replication, Price);
...
Err err = NULL;
	...
	//=========================================
	pIsCall = (int*) calloc(nb_strikes+2, sizeof(int));			//+2 for the interpolation on the left & right
	pStrike = (double*) calloc(nb_strikes+2, sizeof(double));	//+2 for the interpolation on the left & right
	pNotional = (double*) calloc(nb_strikes+2, sizeof(double));	//+2 for the interpolation on the left & right
	pVols = (double*) calloc(nb_strikes+2, sizeof(double));		//+2 for the interpolation on the left & right
	...
	err = Static_payoff_replication(tangent_points, nb_tangents,Convex_strike,0,0,
									convex_payoff,convex_deriv,
									&dConstant,pStrike,pNotional,pIsCall);
	...
	for(i=0; i<nb_strikes+2; i++)
	{
		if(!is_reversed_fx)
		{
			err = cpd_vol_get_vol(Forward, Fix_time, pStrike[i],smile_params,SRT_LOGNORMAL,&(pVols[i])); 
			if(err) goto FREE_RETURN;
		}
		else
		{
			err = cpd_vol_get_vol(1/Forward, Fix_time, 1/pStrike[i],smile_params,SRT_LOGNORMAL,&(pVols[i])); 
			if(err) goto FREE_RETURN;
		}
	}
	...
	*Price = dConstant;
	...
	for(i=0; i<nb_strikes+2; i++)
		*Price += pNotional[i] * srt_f_optblksch(Forward, pStrike[i], pVols[i], Fix_time, 1.0, pIsCall[i]?SRT_CALL:SRT_PUT,PREMIUM);
...
FREE_RETURN:
	if(smile_params) free(smile_params);
	if(tangent_points) free(tangent_points);
	if(pIsCall) free(pIsCall);
	if(pStrike) free(pStrike);
	if(pNotional) free(pNotional);
	if(pVols) free(pVols);

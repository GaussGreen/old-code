...
static Err amotry(double **p, double *y, double *psum, int ndim,
				  Err (*funk)(double *, double *, void *), int ihi, double fac, double *res, void *comm)
...
dimensions, by the downhill simplex method of Nelder and Mead. The matrix p[0..ndim][0..ndim-1] is input.
Its ndim+1 rows are ndim-dimensional vectors which are the vertices of
the starting simplex. Also input is the vector y[0..ndim], whose components must be preinitialized
...
nfunk gives the number of function evaluations taken. comm is a pointer to a communication structure (external
parameters that will be passed to the objective function (3rd parameter)) */
{
	...
	*nfunk=0;
	...
	for (;;)
	{
		...
		if (*nfunk >= NMAX) return serror("NMAX exceeded in d_amoeba");
		*nfunk += 2;

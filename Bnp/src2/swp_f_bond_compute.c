...
double dirty_price_fct(SwapDP p, double coupon, double irr, double redemption, double first_coupon)
{
    double   price, disc1;
    double   dr;
    DateList list;
    ...
    if (disc1 != 1)
    {
        price = (coupon / (double)p.compd) * (pow(disc1, list.len - 2) - 1) / (disc1 - 1) +
...
double clean_price_fct(SwapDP p, double coupon, double irr, double redemption, double first_coupon)
{
    double price;
...
double yield_fct(
    SwapDP p, double coupon, double clean_price, double redemption, double first_coupon)
{
    double dirty_price;
    int    i;

    double guess, delta1 = 10, delta2, df, f, marg1 = 0, marg2 = 1, xl, xh, fl, fh, dx, dxold, temp,
                  delta = 0.0001;
    DateList list;
    ...
    fl = dirty_price_fct(p, coupon, marg1, redemption, first_coupon) - dirty_price;
    fh = dirty_price_fct(p, coupon, marg2, redemption, first_coupon) - dirty_price;
    ...
    f = delta1 = dirty_price_fct(p, coupon, guess, redemption, first_coupon) - dirty_price;
    delta2     = dirty_price_fct(p, coupon, guess + delta, redemption, first_coupon) - dirty_price;
    ...
    double first_coupon)
{
    ...
    {
        price -= first_coupon / swp_f_df((Ddate)(list.date[1]), (Ddate)new_date, swap);
...
/* bond sensibility */
double bond_sens_fct(SwapDP p, double coupon, double irr, double redemption, double first_coupon)
{
    ...
    double first_coupon)
{
    ...
    for (i = 1; i <= nc; i++)
    {
        sum_coup += (c / p.compd) / swp_f_df((Ddate)(list.date[i]), (Ddate)fut, swap);
        c = coupon;
    }
...

{
    ...
    fl = fwd_clean_price_fct(fut, p, coupon, marg1, swap, repo, first_coupon) - fwd_price;
    fh = fwd_clean_price_fct(fut, p, coupon, marg2, swap, repo, first_coupon) - fwd_price;
    ...
    f = delta1 = fwd_clean_price_fct(fut, p, coupon, guess, swap, repo, first_coupon) - fwd_price;
    delta2 =
        fwd_clean_price_fct(fut, p, coupon, guess + delta, swap, repo, first_coupon) - fwd_price;
    df = (delta2 - delta1) / delta;
    ...
    for (i = 0; i < 500; i++)
    {
        ...
        f = delta1 =
            fwd_clean_price_fct(fut, p, coupon, guess, swap, repo, first_coupon) - fwd_price;
        delta2 = fwd_clean_price_fct(fut, p, coupon, guess + delta, swap, repo, first_coupon) -
                 fwd_price;
        df = (delta2 - delta1) / delta;
...
/*additional parameters: SwapDP p,double c, double r, double f*/
{
    ...
    ret = (function(value0 + delta / 2, argptr) - function(value0 - delta / 2, argptr)) / delta;
    va_end(argptr);
    return (ret);
}
...
/*additional parameters: SwapDP p,double c, double r, double f*/
{
    ...
    va_start(argptr, value0);
    ret = 4 *
          (function(value0 + delta / 2, argptr) + function(value0 - delta / 2, argptr) -
           2 * function(value0, argptr)) /
          (delta * delta);
    va_end(argptr);
    return (ret);
}

double fwd_irr_fct_tlr(
    Date   fut,
    SwapDP p,
    double coupon,
    double clean_price,
    ...
    double first_coupon)
{
    ...
    yield_prim2 = prim2(yield_function, fwd_clean_price, pf, coupon, redemption, first_coupon);
    stoch       = fwd_clean_price * fwd_clean_price * (exp(volatility * volatility * T) - 1) *
            yield_prim2 / 2;
                     ...
                     double redemption, double first_coupon*/
{
    ...
    return (
        1 / (volatility * sqrt(2 * SRT_PI * T)) * clean_price * y * exp(x) *
                ...
                double redemption, double first_coupon*/
{
    ...
    n = 1 / (sqrt(2 * SRT_PI * T) * volatility) *
        exp(-pow(log(s / clean_price) - (myu - volatility * volatility / 2) * T, 2) /
            (2 * volatility * volatility * T));
    return (x * d * n / s);
}
/*****************************************************************************/

double fwd_irr_fct_smp(
    Date   fut,
    SwapDP p,
    double coupon,
    double clean_price,

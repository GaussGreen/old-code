...
DateList 	list;
	...
	if(disc1 != 1)
	{
	   	price = (coupon/(double)p.compd)*
               	(pow(disc1,list.len-2)-1)
				/(disc1-1) 
	...
	if(list.type == BROKEN)
	{
		...
		{ 
			dr = day_count_date(list.date[0],list.date[1],
								p.basis_code);           
			dr /= day_count_date(list.prev,list.date[1],
								 p.basis_code);
			dr /= p.compd;           
		}
		else
...
double 	guess,
		delta1=10,
		delta2,
		df,f,marg1=0,marg2=1,xl,xh,fl,fh,dx,dxold,temp, 
		delta=0.0001;
DateList list;
...
DateList list;
	...
	price = dirty_price
			/ swp_f_df((Ddate)(p.start),(Ddate)new_date,repo);
	...
	{
		price -=  first_coupon
			   / swp_f_df( (Ddate)(list.date[1]),(Ddate)new_date, swap);
...
double 	delta;
	...
	price1 	= dirty_price_fct(p, coupon, irr       , redemption, 
								first_coupon);
	price2 	= dirty_price_fct(p, coupon, irr + 0.0001, redemption, 
								first_coupon);
...
SwapDP 	pf;
    ...
    {    
		sum_coup += (c/p.compd)
					/ swp_f_df((Ddate)(list.date[i]),(Ddate)fut,swap);
		c = coupon;        
	}
...
double  guess,
	...
	delta2=fwd_clean_price_fct(fut,p,coupon,guess+delta,swap,repo,first_coupon) - fwd_price;
	df 	= (delta2-delta1)/delta;
...
SwapDP 		pf;
	...
	fwd_clean_price = fwd_clean_price_fct(fut,p,coupon,clean_price,swap,
										repo,first_coupon);
...
/*additional parameters: SwapDP p,double c, double r, double f*/
{
	...
	ret=(function(value0+delta/2,argptr)-function(value0-delta/2,argptr))
		/delta;
	va_end(argptr);
	return(ret);  
}
...
/*additional parameters: SwapDP p,double c, double r, double f*/
{
	...
	double ret;
	va_start(argptr,value0);
	ret=4*(function(value0+delta/2,argptr)+function(value0-delta/2,argptr)
            -2*function(value0,argptr))/ (delta*delta);
	va_end(argptr);
	return(ret);  
}
                         ...
                         double redemption, double first_coupon*/
{
    ...
    return(1/(volatility*sqrt(2*SRT_PI*T))*clean_price*
           y*exp(x)*exp( -pow(x-(myu-volatility*volatility/2)*T,2) 
			...
			double redemption, double first_coupon) 
{
    ...
    fwd_dirty_price=fwd_clean_price_fct(fut,p,coupon,clean_price,swap,repo,
				first_coupon)+ acc_int_fct(pf,coupon);

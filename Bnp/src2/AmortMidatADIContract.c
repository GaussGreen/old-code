...
//-------------------------------------------------------------------------------------------------------
double __stdcall _SIGMA1_Sqrd_(
    const double* pdX1, const double* pdX3, const double* pdT, void* pArg)
{
    // cast
...
//	Returns : as indicated
//-------------------------------------------------------------------------------------------------------
double __stdcall _SIGMA3_Sqrd_(
    const double* pdX1, const double* pdX3, const double* pdT, void* pArg)
{
...
//	Returns : as indicated
//-------------------------------------------------------------------------------------------------------
double __stdcall _MU1_BACKWARD_(
    const double* pdX1, const double* pdX3, const double* pdT, void* pArg)
{
    const _Model* pModel = (const _Model*)pArg;
    const int     n      = pdT - pModel->m_pdLocalT_Begin;
...
//	Returns : as indicated
//-------------------------------------------------------------------------------------------------------
double __stdcall _MU1_FORWARD_(
    const double* pdX1, const double* pdX3, const double* pdT, void* pArg)
{
...
//	Returns : as indicated
//-------------------------------------------------------------------------------------------------------
double __stdcall _MU3_BACKWARD_(
    const double* pdX3, const double* pdX1, const double* pdT, void* pArg)
{
    // cast
...
//	Returns : as indicated
//-------------------------------------------------------------------------------------------------------
double __stdcall _MU3_FORWARD_(
    const double* pdX3, const double* pdX1, const double* pdT, void* pArg)
{
...
//						To solve for the backward equation, pdT should be decreasing
    ...
    double* pdResult)
{
    ...
    if (nSize_T >= 1)
    {
             ...
             ++pdTKnot_Begin, ++pnSizeX1_Begin, ++pnSizeX3_Begin)
        {
            ...
            _Prepare_CommPDE(
                pdLocalT_Begin, pdLocalT_End, pdLocalLam_Begin, pdLocalSig_Begin, pComm_PDE);
        ...
        _ASSERTE(
            pGrid->m_nX3_Zero == (pGrid->m_pdX3_Begin - pGrid->m_pdX3_Begin_ +
                                  (pGrid->m_pdX3_End - pGrid->m_pdX3_Begin) / 2));
        _ASSERTE(
            pGrid->m_nX1_Zero == (pGrid->m_pdX1_Begin - pGrid->m_pdX1_Begin_ +

...
Err CMSCTSAutocal(CMSCTS_MARKET sMarket, CMSCTS_DEAL sDeal,
  ...
  /* Fill the structures */
  err = cmscts_fill_check_all_struct(sDeal, sMarket,
  ...
  for (i = sDeal->sFunding->sAux->iStartIndex;
       i <= sDeal->sFunding->sAux->iEndIndex; i++) {
  ...
  for (i = sDeal->sExotic->sAux->iStartIndex;
       i <= sDeal->sExotic->sAux->iEndIndex; i++) {
  ...
  if (sDeal->sExotic->sAux->iNbUsedNumCpn > 0) {
    /* Allocation */
    sSimulArg = calloc(1, sizeof(cmscts_simularg));
    ...
    err = cmscts_calibrate_model(sMarket, sDeal, sSimulArg, sCalibration,
                                 sOutputs);
    ...
    time1 = clock();
    smessage("CMSCTS Calibration: %.2f",
             (double)(time1 - time2) / CLOCKS_PER_SEC);

    /* Calculates the unadjusted option */
    err = cmscts_calculate_option(sMarket, sDeal, sSimulArg, sPricingParams,
                                  sOutputs);
    ...
    time2 = clock();
    smessage("CMSCTS Option Pricing: %.2f",
    ...
    if (sDeal->sCall->sAux->iNbUsedCall > 1) {
      ...
      time1 = clock();
      smessage("CMSCTS Switch Adjustment: %.2f",
               (double)(time1 - time2) / CLOCKS_PER_SEC);
    } else {
...
Err cmscts_allocate_outputs(CMSCTS_OUTPUTS sOutputs) {
  ...
  sOutputs->sFwdIVInfos = calloc(1, sizeof(cmscts_fwdiv));
  sOutputs->sSwitchInstDatas = calloc(1, sizeof(cpd_calib_inst_data));

  if (!sOutputs->sInstDatas || !sOutputs->sLGMSVModel ||
      !sOutputs->sFwdIVInfos || !sOutputs->sSwitchInstDatas) {
...
Err cmscts_fill_outputs(CMSCTS_DEAL sDeal, CMSCTS_SIMULARG sSimulArg,
                        CMSCTS_OUTPUTS sOutputs) {
  Err err = NULL;
  LGMSV_MODEL sModel;
  CMSCTS_CALL sCall;
  ...
  if (sDeal->sExotic->sAux->iNbUsedNumCpn > 0) {
    ...
    if (sOutputs->sLGMSVModel) {
      err = init_LGMSV_model(
          sOutputs->sLGMSVModel, sModel->lToday, sModel->iOne2F,
          sModel->iNbPWTime, sModel->dLambdaX, sModel->dPWTime, sModel->dSigma,
          sModel->dAlpha, sModel->dLambdaEps, sModel->dLvlEps, sModel->dRho,
          sModel->dTStar, sModel->dInitLGMAlpha, sModel->dLGMGamma,
          sModel->dInitLGMRho, sModel->dRho2);
      ...
      ConvertTS_LGMSV_to_LGM(
          sOutputs->sLGMSVModel->iNbPWTime, sOutputs->sLGMSVModel->dPWTime,
          sOutputs->sLGMSVModel->dSigma, sOutputs->sLGMSVModel->dLambdaX,
    ...
    if (sCallAux->iNbUsedCall > 0 && sFwdIVInfos) {
      ...
      iIndexEvt = 0;
      sPayArg =
          (CMSCTS_PAYARG)(sMCArg
                              ->vPayoffParams[sMCArg->iIndexEvent[iIndexEvt]]);

      while (sPayArg && !sPayArg->iIsCall) {
        iIndexEvt++;
        if (iIndexEvt < sMCArg->iNbEvent) {
          sPayArg = (CMSCTS_PAYARG)(sMCArg->vPayoffParams
                                        [sMCArg->iIndexEvent[iIndexEvt]]);
        } else {
          sPayArg = NULL;
        }
      }
      ...
      for (i = 0; i < sFwdIVInfos->lNbCall; i++) {
        ...
        /* Model IV */
        sFwdIVInfos->dMarketIV[i] =
            sMCArg->sMCEBParams->dMarketFwdIV[iIndexEvt];
        ...
        if (sMCArg->sMCEBParams->dOneTimeCall)
          sFwdIVInfos->dOTCall[i] =
              sMCArg->sMCEBParams->dOneTimeCall[iIndexEvt];
        ...
        while (sPayArg && !sPayArg->iIsCall) {
          sFwdIVInfos->dMarketIV[i] +=
              sMCArg->sMCEBParams->dMarketFwdIV[iIndexEvt];
          sFwdIVInfos->dModelIV[i] +=
              sMCArg->sMCEBParams->dModelFwdIV[iIndexEvt];
          sFwdIVInfos->dFees[i] += sMCArg->sMCEBParams->dFee[iIndexEvt];
          sFwdIVInfos->dNewModelIV[i] += sMCArg->dModelValue[iIndexEvt];
      ...
      if (iIncludePartialPV) {
        for (i = sKOAux->iStartFundIdx[iIndexStart];
             i < sKOAux->iStartFundIdx[iIndexStart + 1]; i++) {
        ...
        if (sDeal->sExotic->sAux
                ->iFloatingType[sKOAux->iStartCpnIdx[iIndexStart]] < 2) {
          sFwdIVInfos->dMarketIV[0] +=
              sDeal->sExotic->sAux->dFixedPV * sDeal->dPayRec;
          sFwdIVInfos->dModelIV[0] +=
              sDeal->sExotic->sAux->dFixedPV * sDeal->dPayRec;
          sFwdIVInfos->dNewModelIV[0] +=
              sDeal->sExotic->sAux->dFixedPV * sDeal->dPayRec;
...
Err cnscts_allocate_fwdiv(CMSCTS_FWDIV sFwdIVInfos, long lNbCall) {
  ...
  if (!sFwdIVInfos->lExeDates || !sFwdIVInfos->dMarketIV ||
      !sFwdIVInfos->dModelIV || !sFwdIVInfos->dNewModelIV ||
      !sFwdIVInfos->dFees || !sFwdIVInfos->dOTCall ||

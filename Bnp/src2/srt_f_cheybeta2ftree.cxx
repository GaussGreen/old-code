/* -------------------------------------------------------------
   ...
   FILENAME: srt_f_cheybeta2dtree.cxx

   PURPOSE: the a new implementation of pink elephant (or Malliavin
 stratification - copyright BFL) for the Cheyette beta  2D in GRFN

                                        First step:  generation of diffusion
 with MC optimized methods

                                        second step: triangulation of random
 mesh O(nlog(n))

                                        third step: computation of probabilities
 solving Lagrangien problem for quadrature formula (exact formulas)

                                        fourth step:   pricing

                                        fifth  step:  splitting up for non
 markovian part
...
#define FREE_CHEYBETA2F_MCTREE_MEMORY                                          \
  {                                                                            \
    ...
    free_dmatrix(next_assets, 0,                                               \
                 grfnparams->num_MCarlo_paths - 1 + NB_BOUND_PTS, 0,           \
                 grfnparams->node_dim - 1);                                    \
    free_dmatrix(cur_assets, 0,                                                \
                 grfnparams->num_MCarlo_paths - 1 + NB_BOUND_PTS, 0,           \
                 grfnparams->node_dim - 1);                                    \
  }
...
/* -------------------------------------------------------------------------------
   ...
   PURPOSE:  The main functions to call when pricing backward with Cheyette Beta
   2f
   -------------------------------------------------------------------------------
 */

Err srt_f_cheybeta2dtree(SrtUndPtr und,            /* underlying  pointer*/
  ...
  double **cur_assets = NULL, **next_assets = NULL;
   ...
   */
  grfnparams->num_MCarlo_paths =
      (long)floor(MAGICAL_NUMBER * num_time_pts * log(num_time_pts));

  /* Always use SPECTRUNC for the MC Scheme (better compact Brownian
   * description) */
  ...
  for (t = 0; t < num_time_pts; t++)
    grid[t] = (MCTreePoint *)calloc(grfnparams->num_MCarlo_paths + NB_BOUND_PTS,
                                    sizeof(MCTreePoint));
  ...
  in.pointattributelist = (double *)malloc(
      in.numberofpoints * in.numberofpointattributes * sizeof(double));
  ...
  next_assets = dmatrix(0, grfnparams->num_MCarlo_paths - 1 + NB_BOUND_PTS, 0,
                        grfnparams->node_dim - 1);
  cur_assets = dmatrix(0, grfnparams->num_MCarlo_paths - 1 + NB_BOUND_PTS, 0,
                       grfnparams->node_dim - 1);
  ...
  for (stp = bot; stp != NULL; stp = stp->prev) {
    if (stp->next) {
      ...
      for (i = 0; i < grfnparams->num_MCarlo_paths; i++) {
        ...
        err = srt_f_cheybeta2ftree_discount_to_node(grid, t, i, stp,
                                                    cur_assets[i], grfnparams);
        if (err)
          return err;
      } /* END loop i on  nodes */
    }
    ...
    for (i = 0; i < grfnparams->num_MCarlo_paths; i++) {
      ...
      err = evalcf((GrfnEvent *)stp->e, &sam, gd, (double *)cur_assets[i],
  ...
  err = srt_f_IOstructsetcolpvs(iolist, SRT_NO, (double *)next_assets[0],
                                grfnparams->node_dim, "");
  if (err) {
    FREE_CHEYBETA2F_MCTREE_MEMORY;
    return err;

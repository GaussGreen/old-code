    ...
    double*          ans)
{
    ...
    err = swp_f_cmsratedp(
        swapdp, today, forward, volatility, pay_date, delta, nswaps, lognrm_nrm, ans);
    ...
    double*          ans)
{
    ...
    /* so delay is number of years */
    err = swp_f_cmsrate(
        forward, nfp, swapdp->compd, volatility, maturity, delay, delta, nswaps, lognrm_nrm, &cms);
    if (err)
        return err;
    ...
    double*          ans)
{
    ...
    dlvl = (nfp / swapdp->compd) * pow(1.0 + forward / swapdp->compd, -nfp - 1) * forward /
               swapdp->compd -
...
/*<%%STA>-----------------------------------------------------------------
  ...
  DESCRIPTION     :Returns the value of a CMS rate using the SABR model through a one dimensional
numerical integration AUTHOR	      :Ezra Nahum LAST MODIFIED   :October 2000
<%%END>---------------------------------------------------------------------*/
double cms_sm_trapzd(
    double (*function)(double, double, double, double, double, double, double, double, double),
    double forward,
    double mat,
    double betavol,
    double alpha,
    ...
    double accum)
{
    ...
    {
        accum = 0.5 * (b - a) *
                (function(a, forward, mat, betavol, alpha, beta, rho, numperiod, coverage) +
                 function(b, forward, mat, betavol, alpha, beta, rho, numperiod, coverage));
    }
    else
    {
        it = 1;
    ...
    double precision)
{
    ...
    sumN = cms_sm_trapzd(
        func, forward, mat, betavol, alpha, beta, rho, numperiod, coverage, a, b, 1, 0.0);
    sum2N = oldsum = -1.0e30;

    for (j = 2; j < JMAX; j++)
    {
        sum2N = cms_sm_trapzd(
            func, forward, mat, betavol, alpha, beta, rho, numperiod, coverage, a, b, j, sumN);
    ...
    double accum)
{
    ...
    if (n == 1)
    {
        accum =
            0.5 * (b - a) *
            (function(a, forward, mat, betavol, alpha, beta, rho, numperiod, coverage, K1, K2, q) +
             function(b, forward, mat, betavol, alpha, beta, rho, numperiod, coverage, K1, K2, q));
    }
    else
    {
        it = 1;
        ...
        {
            sum += function(
                x, forward, mat, betavol, alpha, beta, rho, numperiod, coverage, K1, K2, q);
            x += del;
        }
        accum = 0.5 * (accum + (b - a) * sum / tnm);
    }
...
/*
double ProbaIntegrand(double s, double S0, double mat,double sigb, double a, double b, double r,
double m, double d)
{
double SIGMA;
double RV;
...
double CMSIntegrand(double s, double S0, double mat,double sigb, double a, double b, double r,
double m, double d)
{
double SIGMA;
double RV;
...
double ProbaIntegrand(
    double s, double S0, double mat, double sigb, double a, double b, double r, double m, double d)
{
    double SIGMA;
    double Expo;
    double normalize;
    ...
    double q)
{
    ...
    if ((b == 1.0) && (a == 0.0))
    {
        normalize = 1 / (sqrt(2 * SRT_PI * mat * s * s * s * sigb * sigb / S0));
        Expo      = exp(-(S0 - s) * (S0 - s) / (2 * mat * SIGMA * SIGMA) - mat * sigb * sigb / 8);
        p1 = exp(-(S0 - K1) * (S0 - K1) / (2 * mat * SIGMA1 * SIGMA1) - mat * sigb * sigb / 8) * 1 /
             (sqrt(2 * SRT_PI * mat * K1 * K1 * K1 * sigb * sigb / S0));
        p2 = exp(-(S0 - K2) * (S0 - K2) / (2 * mat * SIGMA2 * SIGMA2) - mat * sigb * sigb / 8) * 1 /
             (sqrt(2 * SRT_PI * mat * K2 * K2 * K2 * sigb * sigb / S0));
    }
    else
    {
        normalize = pow(Csi * Csi - 2 * r * Csi + 1, -0.75) /
    ...
    double precision)
{
    ...
    srt_f_optblkvolATMtobetavolStochVol(
        forward, forward, ATMvol, maturity, alpha, beta, rho, &betavol);

    if (beta == 1)
    {
        K1 = exp(
            log(forward) - 0.5 * betavol * betavol * maturity - nstdev * betavol * sqrt(maturity));
        K2 = exp(
            log(forward) - 0.5 * betavol * betavol * maturity + nstdev * betavol * sqrt(maturity));
    }
    ...
    {
        K1 =
            pow((1 - beta) * (pow(forward, 1 - beta) / (1 - beta) -
                              0.5 * beta * betavol * betavol * pow(forward, beta - 1) * maturity -

...
SrtCurvePtr yldcrv;
	...
	maturity = coverage(get_clcndate_from_yldcrv(yldcrv),fix_date,BASIS_ACT_365);
	...
	err=swp_f_cmsoption(forward, swapdp->nfp, swapdp->compd,
		strike,
		volatility, maturity, pay_or_receive, delay,
...
/*<%%STA>-----------------------------------------------------------------
  FUNCNAME        :swp_f_cmswap
  DESCRIPTION     :Returns the value of a CMS rate, as Call(F) - Put(F) + F
				   computing the forward rate itself
  MODIFIES        :
  CALL            :
<%%END>---------------------------------------------------------------------*/
...
SrtCurvePtr yldcrv;
	...
	err = swp_f_cmsratedp(	swapdp,today,forward,volatility, 
							pay_date, delta, nswaps, lognrm_nrm, ans);
...
Err err;
	...
	/* so delay is number of years */
	err = swp_f_cmsrate(forward, nfp, swapdp->compd,
						volatility, maturity, delay,
						delta, nswaps, lognrm_nrm, &cms);
	if(err)
		return err;
...
/*<%%STA>-----------------------------------------------------------------
  ...
  DESCRIPTION     :Returns the value of a CMS rate from a closed-form approximation
  MODIFIES        :
  CALL            :
<%%END>---------------------------------------------------------------------*/
...
double cms_adj, del_adj, tot_adj;
	...
	/*	LVL and LVL'	*/
	lvl = (1.0 - pow (1.0 + forward / swapdp->compd, - nfp)) / (forward / swapdp->compd);
	dlvl = 
		(nfp / swapdp->compd) * pow (1.0 + forward / swapdp->compd, - nfp - 1) * forward / swapdp->compd
...
Err swp_f_Cms_Rate(double				dFwdSwapRate, /* Forward Swap Rate */
...
Err err;
	...
	err = swp_f_Cms_Option (dFwdSwapRate, 
							dMaturity, 
							dNumPeriods,
							dFwdSwapRate,
							dFrequency, 
...
/* ------------------------------------------------------------------------------- */
/* Returns the value of a CMS rate, as Call(F) - Put(F) + F						   */
Err swp_f_Tec_Rate(double				dFwdSwapRate, /*Forward Swap Rate */
				   double				dMaturity, 
				   double				dNumPeriods,
				   double				dAtmStrike,
				   double				dMargin,				   
...
Err err;
	...
	err = swp_f_Tec_Option (dFwdSwapRate, 
							dMaturity, 
							dNumPeriods,
							dAtmStrike,
							dMargin,				   
	...
	/* Use call-put parity: conv(forward) - forward = call - put */
	dPaymentEquivalent = dTecPayVal - dTecRecVal - ( 1 - pow(( 1 + dAtmStrike),(1/dPaymentPower)));

	/* Gets the Equivalent Forward (Payment = ( 1+ TEC +m) ^1/4 - 1) */
	*dTecRateValue = pow( dPaymentEquivalent + 1.0 , dPaymentPower) - (1.0 + dMargin);
...
/*<%%STA>-----------------------------------------------------------------
  ...
  DESCRIPTION     :Returns the value of a CMS rate using the SABR model through a one dimensional numerical integration
  AUTHOR	      :Ezra Nahum
  LAST MODIFIED   :October 2000
<%%END>---------------------------------------------------------------------*/
double cms_sm_trapzd(	double (*function)(double,double,double,double,double,double, double,double, double),
			double forward,
			double mat,
			double betavol,
			double alpha,
			...
			double accum)
{
	...
	if (n==1) 
	{
		accum = 0.5*(b-a)*( function(a,forward, mat,betavol,alpha,beta,rho,numperiod,coverage) + function(b,forward, mat,betavol,alpha,beta,rho,numperiod,coverage) );
	}
	else
	{
		it = 1;
		...
		for (j=1; j<=it; j++) 
		{
			sum += function(x,forward, mat, betavol,alpha,beta,rho,numperiod,coverage);
			x += del;
		}
...
double 	cms_sm_qsimp	(
			double 	(*func)(double,double,double,double,double,double,double,double,double),
			double forward,
			double mat,
			double betavol,
			double alpha,
...
{       
	...
	sumN  = cms_sm_trapzd(func, forward, mat,betavol,alpha,beta,rho,numperiod,coverage,a, b, 1, 0.0);
	sum2N = oldsum = -1.0e30;
    
	for (j = 2; j < JMAX; j++)
	{
		sum2N  = cms_sm_trapzd(func, forward, mat,betavol,alpha,beta,rho,numperiod,coverage,a, b, j, sumN);
...
double cms_sm_trapzd2(	double (*function)(double,double,double,double,double,double, double,double, double,double,double,double),
			double forward,
			double mat,
			double betavol,
			double alpha,
			...
			double accum)
{
	...
	if (n==1) 
	{
		accum = 0.5*(b-a)*( function(a,forward, mat,betavol,alpha,beta,rho,numperiod,coverage,K1,K2,q) + function(b,forward, mat,betavol,alpha,beta,rho,numperiod,coverage,K1,K2,q) );
	}
	else
	{
		it = 1;
		...
		for (j=1; j<=it; j++) 
		{
			sum += function(x,forward, mat, betavol,alpha,beta,rho,numperiod,coverage,K1,K2,q);
			x += del;
		}
		accum = 0.5 * ( accum + (b-a)*sum/tnm );
	}
...
double 	cms_sm_qsimp2	(
			double 	(*func)(double,double,double,double,double,double,double,double,double,double,double,double),
			double forward,
			double mat,
			double betavol,
			double alpha,
...
{       
	...
	sumN  = cms_sm_trapzd2(func, forward, mat,betavol,alpha,beta,rho,numperiod,coverage,K1,K2,q,a, b, 1, 0.0);
	sum2N = oldsum = -1.0e30;
    
	for (j = 2; j < JMAX; j++)
	{
		sum2N  = cms_sm_trapzd2(func, forward, mat,betavol,alpha,beta,rho,numperiod,coverage,K1,K2,q,a, b, j, sumN);
...
/*
double ProbaIntegrand(double s, double S0, double mat,double sigb, double a, double b, double r, double m, double d)
{
double SIGMA;
double RV;
double integ;
...
dbsdsig = S0*(-log(S0/s)/(SIGMA*SIGMA*sqrt(mat))+sqrt(mat)/2)*exp(-d1*d1/2)/sqrt(2*SRT_PI)+
		  s*(log(S0/s)/(SIGMA*SIGMA*sqrt(mat))+sqrt(mat)/2)*exp(-d2*d2/2)/sqrt(2*SRT_PI);

d2bsd2sig = (2*S0*log(S0/s)/(SIGMA*SIGMA*SIGMA*sqrt(mat))-S0*(-log(S0/s)/(SIGMA*SIGMA*sqrt(mat))+sqrt(mat)/2)*
...
d2bsd2k = S0*(0.5-log(S0/s)/(SIGMA*SIGMA*mat))*exp(-d1*d1/2)/(sqrt(2*SRT_PI)*SIGMA*sqrt(mat)*s*s)+
		  (0.5+log(S0/s)/(SIGMA*SIGMA*mat))*exp(-d2*d2/2)/(sqrt(2*SRT_PI)*SIGMA*sqrt(mat)*s);


d2bsdsigdk  = (S0/(s*SIGMA*SIGMA*sqrt(mat))+S0*(-log(S0/s)/(SIGMA*SIGMA*sqrt(mat))+sqrt(mat)/2)*d1/(SIGMA*s*sqrt(mat)))*exp(-d1*d1/2)/sqrt(2*SRT_PI)
...
double CMSIntegrand(double s, double S0, double mat,double sigb, double a, double b, double r, double m, double d)
{
double SIGMA;
double RV;
double integ;
...
integ = RV*(d2bsd2k+2*d2bsdsigdk*dsigdk+dbsdsig*d2sigd2k+d2bsd2sig*dsigdk*dsigdk);
...
RV = s*s/(1-pow(1+d*s,-m));
if (b==1.0) Csi = a*(log(S0)-log(s))/sigb;
else Csi = a*(pow(S0,1-b)-pow(s,1-b))/(sigb*(1-b));
...
else if (a==0)
{
	...
	else SIGMA  = (sigb*(1-b))*(S0-s)/(pow(S0,1-b)-pow(s,1-b));
}
else
{
	if ((S0<s+0.00001)&&(S0>s-0.00001)) 
	...
	else SIGMA = a*(S0-s)/XofZ;
}

       if ((b==1.0)&&(a == 0.0)) 
{
	normalize = 1/(sqrt(2*SRT_PI*mat*s*s*s*sigb*sigb/S0));
	Expo = exp(-(S0-s)*(S0-s)/(2*mat*SIGMA*SIGMA)-mat*sigb*sigb/8);
}
else
{

	normalize = pow(Csi*Csi-2*r*Csi+1,-0.75)/(sqrt(2*SRT_PI*mat*sigb*sigb*omega*omega));
	Expo = exp(-(S0-s)*(S0-s)/(2*mat*SIGMA*SIGMA));
	 	
/*	Expo = exp(-(S0-s)*(S0-s)/(2*mat*SIGMA*SIGMA))*(1-(((XofZ*(exp(XofZ)+exp(-XofZ))/(exp(XofZ)-exp(-XofZ)))-1)/(XofZ*XofZ)+1)*mat*a*a/8);
*/	
...
double CMSIntegrand(double s, double S0, double mat,double sigb, double a, double b, double r, double m, double d, double K1,double K2,double q)
{
 ...
 if ((b==1.0)&&(a == 0.0)) 
{
	normalize = 1/(sqrt(2*SRT_PI*mat*s*s*s*sigb*sigb/S0));
	Expo = exp(-(S0-s)*(S0-s)/(2*mat*SIGMA*SIGMA)-mat*sigb*sigb/8);
	p1 = exp(-(S0-K1)*(S0-K1)/(2*mat*SIGMA1*SIGMA1)-mat*sigb*sigb/8)*1/(sqrt(2*SRT_PI*mat*K1*K1*K1*sigb*sigb/S0));
	p2 = exp(-(S0-K2)*(S0-K2)/(2*mat*SIGMA2*SIGMA2)-mat*sigb*sigb/8)*1/(sqrt(2*SRT_PI*mat*K2*K2*K2*sigb*sigb/S0));
}
else
{
	...
	normalize = pow(Csi*Csi-2*r*Csi+1,-0.75)/(sqrt(2*SRT_PI*mat*sigb*sigb*omega*omega));
	Expo = exp(-(S0-s)*(S0-s)/(2*mat*SIGMA*SIGMA));
	p1 = exp(-(S0-K1)*(S0-K1)/(2*mat*SIGMA1*SIGMA1))*pow(Csi1*Csi1-2*r*Csi1+1,-0.75)/(sqrt(2*SRT_PI*mat*sigb*sigb*omega1*omega1));
	p2 = exp(-(S0-K2)*(S0-K2)/(2*mat*SIGMA2*SIGMA2))*pow(Csi2*Csi2-2*r*Csi2+1,-0.75)/(sqrt(2*SRT_PI*mat*sigb*sigb*omega2*omega2));
...
if (s <=K1)
{
	integ = ((3*p1/(K1*K1)-6*q/(K1*K1*K1))*s*s+(6*q/(K1*K1)-2*p1/K1)*s)*RV;
}
...
double swp_f_cmsrateNewYork(double maturity,
						 double underlying,
						 double forward,
						 double ATMvol,
						 double alpha,
...
srt_f_optblkvolATMtobetavolStochVol(forward,
										  forward,
										  ATMvol,
										  maturity,
										  alpha,
...
if (beta ==1)
{
K1 = exp(log(forward)-0.5*betavol*betavol*maturity-nstdev*betavol*sqrt(maturity));
K2 = exp(log(forward)-0.5*betavol*betavol*maturity+nstdev*betavol*sqrt(maturity));
}

else
{
K1 = pow((1-beta)*(pow(forward,1-beta)/(1-beta)-0.5*beta*betavol*betavol*pow(forward,beta-1)*maturity-nstdev*betavol*sqrt(maturity)),1/(1-beta));
K2 = pow((1-beta)*(pow(forward,1-beta)/(1-beta)-0.5*beta*betavol*betavol*pow(forward,beta-1)*maturity+nstdev*betavol*sqrt(maturity)),1/(1-beta));
}

mainpart = cms_sm_qsimp	(
			&ProbaIntegrand,
			forward,
			maturity,
			betavol,

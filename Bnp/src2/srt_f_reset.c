 ...
 *				: CMSfixing		- forward fixing rate CMS adjusted for pay
 *date : CMSpay		- forward pay rate CMS adjusted for pay date
...
Err srt_f_sprrf_tok_mc(
    ...
    double* ans)
{
    double   slopevol = sqrt(CMSfvol * CMSfvol + CMSpvol * CMSpvol - 2 * rhofp * CMSfvol * CMSpvol);
    double   slopevar, fwdvar;
    double   CMSslope = CMSpay - CMSfixing;
    ...
    if (slopevar + fwdvar > 0)
    {
        ...
        *ans = 0.0;
        ...
        for (i = 1; i <= NSOBOL; i++)
        {
            ...
            f1 = CMSfixing +
                 CMSfvol * sqrt(YEARS_IN_DAY * (fixing_long - today_long)) * brownians[1][1];

            correl_brownian = rhofp * brownians[1][1] + sqrt(1 - rhofp * rhofp) * brownians[1][2];

            f2 = CMSpay +
                 CMSpvol * sqrt(YEARS_IN_DAY * (fixing_long - today_long)) * correl_brownian;

    ...
    LGMQTO_UND und)
{
    ...
    if (!und->sigma_date || !und->sigma_time || !und->dom_sigma || !und->for_sigma ||
    ...
    for (i = nb_merge_dates - 1; i >= 0; i--)
    {
        ...
        err = srt_f_get_corr_from_CorrList(
            sCorrlist, domname, forname, merge_dates[i], &(und->domfor_rho[i]));
        if (err)
        {
            goto FREE_RETURN;
        }
        err = srt_f_get_corr_from_CorrList(
            sCorrlist, forname, und3dfx, merge_dates[i], &(und->quanto_rho[i]));
        if (err)
        {
            goto FREE_RETURN;
        }

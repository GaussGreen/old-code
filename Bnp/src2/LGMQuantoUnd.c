...
//	Fill the new term structures 
	...
	if (!und->sigma_date || !und->sigma_time || 
		!und->dom_sigma || !und->for_sigma || 
	...
	for (i=nb_merge_dates-1; i>=0; i--)
	{
		...
		err = srt_f_get_corr_from_CorrList(sCorrlist, domname, forname, merge_dates[i], &(und->domfor_rho[i]));
		if(err)
		{
			goto FREE_RETURN;
		}
		err = srt_f_get_corr_from_CorrList(sCorrlist, forname, und3dfx, merge_dates[i], &(und->quanto_rho[i]));
		if(err)
		{
			goto FREE_RETURN;
		}
...
FREE_RETURN:
	if(err)
	{
		if(und) free_lgmQto_und(und);
	}

	return err;
}

void copy_lgmQto_und(
LGMQTO_UND		src,
LGMQTO_UND		dest)
{	
...
LGMQTO_UND		und)
{
	...
	if (und->dom_sigma) free (und->dom_sigma);
	if (und->for_sigma) free (und->for_sigma);
	if (und->fx_sigma) free (und->fx_sigma);
	if (und->domfor_rho) free (und->domfor_rho);
	if (und->quanto_rho) free (und->quanto_rho);
...
//	int i;
	...
	if (adi_arg->dom_ifr) free (adi_arg->dom_ifr);
	if (adi_arg->for_ifr) free (adi_arg->for_ifr);
...
//	}
	...
	if (adi_arg->void_prm) free (adi_arg->void_prm);
						...
						LGMQTO_ADI_ARG		adi_arg)
{
	...
	{
		adi_arg->dom_ifr[i] = swp_f_zr (adi_arg->date[i], adi_arg->date[i+1], adi_arg->dom_yc);
	}
	...
	for (i=0; i<adi_arg->nstp-1; i++)
	{
		adi_arg->for_ifr[i] = swp_f_zr (adi_arg->date[i], adi_arg->date[i+1], adi_arg->for_yc);
	}

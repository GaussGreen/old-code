    ...
    double** optim_res)
{
    ...
    for (i = 0; i < nb_paths; i++)
    {
        ...
        for (j = 1; stop_path == 0 && j < nb_dates; j++)
        {
            ...
            Z += fx_fwd[j] + for_beta[j - 1] * Y - dom_beta[j - 1] * X + matrix[i][3 * (j - 1) + 2];

            /* random X and Y */
            X = X * exp(-dom_lam * (t2 - t1)) + dom_fwd[j] + matrix[i][3 * (j - 1)];
            Y = Y * exp(-for_lam * (t2 - t1)) + for_fwd[j] + matrix[i][3 * (j - 1) + 1];

            df = dom_bond_pay[j] * exp(dom_beta_pay[j] * (-0.5 * dom_beta_pay[j] * dom_phi[j] + X));

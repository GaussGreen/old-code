...
void srt_f_UpdateGlobalHedgeUndFraTermStr(Date *GlobalHedgeUndFraEndDates,
  ...
  for (k = 0; k < NumLocalHedgeUndFra; k++) {
    j = 0;
    while ((j < NumGlobalHedgeUndFra) &&
           (GlobalHedgeUndFraEndDates[j] != LocalHedgeUndFraEndDates[k]))
      j++;
    (*GlobalHedgeUndFraPEQ)[j] += LocalHedgeUndFraPEQ[k];
  }
}

void srt_f_UpDateHedgeTermStr(Date *HedgeTenorDates, long NumHedgeTenors,
                              Date *SwapHedgeTenorDates, long NumSwpHedgeTenors,
                              double *SwapHedgePEQ, long PrevTenorIndex,
                              long NextTenorIndex,
  ...
  j = 0;
  while (HedgeTenorDates[j] != SwapHedgeTenorDates[PrevTenorIndex])
    j++;
  (*HedgePEQ)[j] += SwapHedgePEQ[PrevTenorIndex];

  if (PrevTenorIndex != NextTenorIndex) {
    j = 0;
    while (HedgeTenorDates[j] != SwapHedgeTenorDates[NextTenorIndex])
      j++;
    (*HedgePEQ)[j] += SwapHedgePEQ[NextTenorIndex];
  }
}

void srt_f_UpdateLocHedgeUndFraTermStr(
    Date *UndFraHedgeEndDates, double *tmp_HedgeUndFraPEQ, long PrevTenorIndex,
    long NextTenorIndex, SrtUndFRAType UndFraType,

    /*OUTPUT */
    long *NumLocalHedgeUndFra, double **LocalHedgeUndFraPEQ,
    Date **LocalHedgeUndFraEndDates, SrtUndFRAType **LocalHedgeUndFraTypes) {
  ...
  if (PrevTenorIndex != NextTenorIndex) {
    ...
    (*LocalHedgeUndFraPEQ) = realloc((*LocalHedgeUndFraPEQ),
                                     (*NumLocalHedgeUndFra) * sizeof(double));
    (*LocalHedgeUndFraEndDates) = realloc(
        (*LocalHedgeUndFraEndDates), (*NumLocalHedgeUndFra) * sizeof(Date));
    (*LocalHedgeUndFraTypes) =
        realloc((*LocalHedgeUndFraTypes),
                (*NumLocalHedgeUndFra) * sizeof(SrtUndFRAType));
...
Err UpdateUndFRATermStruct(Date DateToInsert, Date **SetOfRefDates,
  ...
  jmax = (*NumUndFra);
  j = 0;
  while ((jmax > 0) && (DateToInsert != (*SetOfRefDates)[j]) && (j < jmax))
    j++;
  ...
  if (j == jmax) {
    ...
    jmax++;
    (*SetOfRefDates) =
        (Date *)realloc((*SetOfRefDates), (*NumUndFra) * sizeof(Date));
    (*SetOfRefUndFRATypes) = (SrtUndFRAType *)realloc(
        (*SetOfRefUndFRATypes), (*NumUndFra) * sizeof(SrtUndFRAType));
...
Err GetTenorsPEQ(String RefTenors, long NumMarketTenors, String *MarketTenors,
  ...
  i = 0;
  while ((i < NumMarketTenors) && (strcmp(MarketTenors[i], RefTenors) != 0))
    i++;
...
SRT_Boolean ExitsHedgeTenors(Date MarketTenorDate, /*THE SWAP TO HEDGE */
                             long NumHedgeTenors, Date *HedgeTenorDates)

{
  ...
  for (i = 0; i < NumHedgeTenors; i++) {
    if (HedgeTenorDates[i] >= MarketTenorDate)
      ExitsNext = SRT_YES;
    if (HedgeTenorDates[i] <= MarketTenorDate)
      ExitsPrevious = SRT_YES;
  }

  if ((ExitsNext == SRT_YES) && (ExitsPrevious == SRT_YES))
    Exits = SRT_YES;

  return Exits;
}

Err FindHedges(Date MarketTenorDate, long NumHedgeTenors, Date *HedgeTenorDates,
               long *PrevTenorIndex, long *NextTenorIndex) {
  ...
  i = 0;
  while ((i < NumHedgeTenors) && (HedgeTenorDates[i] < MarketTenorDate))
    i++;
  *NextTenorIndex = min(i, NumHedgeTenors - 1);

  /*GET THE PrevSwapIndex */
  while ((i > 0) && (HedgeTenorDates[i] > MarketTenorDate))
    i--;
...
Err srt_f_AggregateFraTenor(
    Date FraEndDate, double FraPEQ, long NumHedgeSwpUndFra,
    long NumHedgeFutUndFra, Date *SwpUndFraHedgeEndDates,
    Date *FutUndFraHedgeEndDates, SrtCompounding FloatFreq,
    SrtBasisCode FloatBasis, String FloatRefRate, String YieldCurveName,
...

{
  ...
  {
    FindHedges(FraEndDate, NumHedgeFutUndFra, FutUndFraHedgeEndDates,
               &PrevTenorIndex, &NextTenorIndex);

    tmp_LocalHedgeUndFraPEQ = dvector(0, NumHedgeFutUndFra - 1);

    err = srt_f_GetHedgePEQ(FraEndDate, "FRA", FutUndFraHedgeEndDates,
                            PrevTenorIndex, NextTenorIndex, 0, 0, FloatFreq,
                            FloatBasis, FloatRefRate, YieldCurveName, FraPEQ,
                            &tmp_LocalHedgeUndFraPEQ);

    srt_f_UpdateLocHedgeUndFraTermStr(
        FutUndFraHedgeEndDates, tmp_LocalHedgeUndFraPEQ, PrevTenorIndex,
        NextTenorIndex, SRT_UNDFRA_FUT,

        NumLocalHedgeUndFra, LocalHedgeUndFraPEQ, LocalHedgeUndFraEndDates,
        LocalHedgeUndFraTypes);
    if (err)
      return err;
  ...
  /*THEN HEDGE WITH THE SWAP UNDERLYING FRA */
  {
    ...
    FindHedges(FraEndDate, NumHedgeSwpUndFra, SwpUndFraHedgeEndDates,
               &PrevTenorIndex, &NextTenorIndex);

    tmp_LocalHedgeUndFraPEQ = dvector(0, NumHedgeSwpUndFra - 1);

    err = srt_f_GetHedgePEQ(FraEndDate, "FRA", SwpUndFraHedgeEndDates,
                            PrevTenorIndex, NextTenorIndex, 0, 0, FloatFreq,
                            FloatBasis, FloatRefRate, YieldCurveName, FraPEQ,
                            &tmp_LocalHedgeUndFraPEQ);
    if (err)
    ...
    srt_f_UpdateLocHedgeUndFraTermStr(
        SwpUndFraHedgeEndDates, tmp_LocalHedgeUndFraPEQ, PrevTenorIndex,
        NextTenorIndex, SRT_UNDFRA_SWAP,
...
Err srt_f_GetHedgePEQ(Date MKTTenorDate,   /*SWAP MAT OR FRA THEO DATE */
                      ...
                      Date *HedgeTenorDates, long PrevTenorIndex,
                      long NextTenorIndex, SrtCompounding FixedFreq,
                      SrtBasisCode FixedBasis, SrtCompounding FloatFreq,
                      SrtBasisCode FloatBasis, String FloatRefRate,
                      String YieldCurveName, double MarketPEQ,
                      double **HedgePEQ)

{
  ...
  if (HedgeType == SRT_SWAP) {
    ...
    err =
        swp_f_LevelPayment(SpotDate, MKTTenorDate, FixedFreqStr, FixedBasisStr,
                           YieldCurveName, FloatRefRate, &MKTSensitivity);
    if (err)
      return err;

    /*IF AGGREGATION ON TWO REFERENCES */
    if (PrevTenorIndex != NextTenorIndex) {
      err = srt_f_SwapConvexity(SpotDate, MKTTenorDate, FixedFreq, FixedBasis,
                                FloatFreq, FloatBasis, FloatRefRate,
                                YieldCurveName, &MKTConvexity);
      if (err)
        return err;
    }

    /*GET THE HEDGE'S SENSTIVITIES  AND (CONVEXITIES) */
    err = swp_f_LevelPayment(SpotDate, HedgeTenorDates[PrevTenorIndex],
                             FixedFreqStr, FixedBasisStr, YieldCurveName,
                             FloatRefRate, &PrevHedgeTenorSensitivity);
    if (err)
      return err;
    ...
    if (PrevTenorIndex != NextTenorIndex) {
      ...
      err = swp_f_LevelPayment(SpotDate, HedgeTenorDates[NextTenorIndex],
                               FixedFreqStr, FixedBasisStr, YieldCurveName,
                               FloatRefRate, &NextHedgeTenorSensitivity);
      if (err)
        return err;

      err = srt_f_SwapConvexity(SpotDate, HedgeTenorDates[PrevTenorIndex],
                                FixedFreq, FixedBasis, FloatFreq, FloatBasis,
                                FloatRefRate, YieldCurveName,
                                &PrevHedgeTenorConvexity);
      if (err)
        return err;

      err = srt_f_SwapConvexity(SpotDate, HedgeTenorDates[NextTenorIndex],
                                FixedFreq, FixedBasis, FloatFreq, FloatBasis,
                                FloatRefRate, YieldCurveName,
    ...
    FraStartDate = add_unit(MKTTenorDate, -12 / (int)FloatFreq, SRT_MONTH,
                            NO_BUSDAY_CONVENTION);

    err = swp_f_LevelPayment(FraStartDate, MKTTenorDate, FloatFreqStr,
                             FloatBasisStr, YieldCurveName, FloatRefRate,
                             &MKTSensitivity);

    if (PrevTenorIndex != NextTenorIndex) {
      err = srt_f_FRAConvexity(FraStartDate, MKTTenorDate, FloatBasis,
                               YieldCurveName, &MKTConvexity);
      if (err)
        return err;
    }
    ...
    FraStartDate =
        add_unit(HedgeTenorDates[PrevTenorIndex], -12 / (int)FloatFreq,
                 SRT_MONTH, NO_BUSDAY_CONVENTION);

    err = swp_f_LevelPayment(FraStartDate, HedgeTenorDates[PrevTenorIndex],
                             FloatFreqStr, FloatBasisStr, YieldCurveName,
                             FloatRefRate, &PrevHedgeTenorSensitivity);

    if (PrevTenorIndex != NextTenorIndex) {
      /* FraStartDate IS COMPUTED ABOVE */
      err = srt_f_FRAConvexity(FraStartDate, HedgeTenorDates[PrevTenorIndex],
                               FloatBasis, YieldCurveName,
                               &PrevHedgeTenorConvexity);
      if (err)
        return err;

      FraStartDate =
          add_unit(HedgeTenorDates[NextTenorIndex], -12 / (int)FloatFreq,
                   SRT_MONTH, NO_BUSDAY_CONVENTION);

      err = swp_f_LevelPayment(FraStartDate, HedgeTenorDates[NextTenorIndex],
                               FloatFreqStr, FloatBasisStr, YieldCurveName,
                               FloatRefRate, &NextHedgeTenorSensitivity);

      err = srt_f_FRAConvexity(FraStartDate, HedgeTenorDates[NextTenorIndex],
                               FloatBasis, YieldCurveName,
                               &NextHedgeTenorConvexity);
      if (err)
        return err;
  ...
  if (PrevTenorIndex != NextTenorIndex) {
    (*HedgePEQ)[PrevTenorIndex] =
        MarketPEQ *
        (NextHedgeTenorConvexity * MKTSensitivity -
         NextHedgeTenorSensitivity * MKTConvexity) /
        (PrevHedgeTenorSensitivity * NextHedgeTenorConvexity -
         PrevHedgeTenorConvexity * NextHedgeTenorSensitivity);

    (*HedgePEQ)[NextTenorIndex] =
        MarketPEQ *
        (PrevHedgeTenorSensitivity * MKTConvexity -
         MKTSensitivity * PrevHedgeTenorConvexity) /
        (PrevHedgeTenorSensitivity * NextHedgeTenorConvexity -
         PrevHedgeTenorConvexity * NextHedgeTenorSensitivity);
    ...
    (*HedgePEQ)[PrevTenorIndex] =
        MarketPEQ * MKTSensitivity / PrevHedgeTenorSensitivity;
  }
...
Err srt_f_GetSwapUndFRAPrincEq(
    Date SwapStartDate, Date SwapEndDate, SrtCompounding FixedFreq,
    SrtBasisCode FixedBasis, SrtCompounding FloatFreq, SrtBasisCode FloatBasis,
    String FloatRefRate, String YieldCurveName, double QSwapPEq,

    /*OUTPUT	*/
    long *NumSwapUndFra, double **SwapUndFraPEq, long **SwapUndFraEndDates)
...
/*QSwapPEq IS THE QUATERLY PRINCIPAL EQUIVALENT */
{
  ...
  err = swp_f_make_FloatLegDatesAndCoverages(
      ...
      &EndDates, &Coverage, NumSwapUndFra);
  ...
  for (i = 0; i < (*NumSwapUndFra); i++) {
    ...
    (*SwapUndFraEndDates)[i] =
        add_unit(DTOL(SwapStartDate), (i + 1) * (12 / (int)FloatFreq),
                 SRT_MONTH, NO_BUSDAY_CONVENTION);
  }
...
Err srt_f_SwapConvexity(Date SwapStartDate, /*SwapStartDate = SpotDate */
                        Date SwapEndDate, SrtCompounding FixedFreq,
                        SrtBasisCode FixedBasis, SrtCompounding FloatFreq,
                        SrtBasisCode FloatBasis, String FloatRefRate,
                        String YieldCurveName, double *SwapConvexity)

{
  ...
  err = srt_FraSwapWeights(SwapStartDate, SwapEndDate, FixedFreq, FixedBasis,
                           FloatFreq, FloatBasis, YieldCurveName,
                           /*OUTPUT*/
                           &FraSwapWeightsMatrix, &FraSwapWeightsSize);
  ...
  Date = add_unit(SwapStartDate, (FraSwapWeightsSize - 1) * 12 / (int)FloatFreq,
                  SRT_MONTH, NO_BUSDAY_CONVENTION);

  DiscountFactor = dvector(0, 1);
  DiscountFactor[0] = swp_f_df((Ddate)today, (Ddate)Date, YieldCurveName);
  DiscountFactor[1] =
      swp_f_df((Ddate)today, (Ddate)SwapEndDate, YieldCurveName);
  Coverage = coverage(Date, SwapEndDate, FloatBasis);

  (*SwapConvexity) =
      -2 * pow(Coverage * DiscountFactor[1] / DiscountFactor[0], 2) *
      FraSwapWeightsMatrix[FraSwapWeightsSize][FraSwapWeightsSize];
  return err;

  if (DiscountFactor)
    free_dvector(DiscountFactor, 0, 1);
}

/*COMPUTATION OF THE CONVEXITY OF THE FRA */

Err srt_f_FRAConvexity(long FraStartDate, long FraEndDate,
  ...
  *FraConvexity = -coverage(FraStartDate, FraEndDate, FloatBasis) *
                  pow(df_End, 2) / df_Start;
...
Err srt_f_GetFraUndSWAPPrincEq(
    Date SpotDate, Date FraEndDate, /*THEO END DATE ? */

    /*SOME SWAP DETAILS */
    SrtCompounding SwpFixedFreq, SrtBasisCode SwpFixedBasis,
    SrtCompounding SwpFloatFreq, SrtBasisCode SwpFloatBasis,
    ...
    /*OUTPUT	*/
    long *NumFraUndSwap, double **FraUndSwapPEq, long **FraUndSWAPEndDates)

{
  ...
  err =
      swp_f_setSwapDP(SpotDate,   /* ALL THE SWAP START AT SPOT DATE */
                      FraEndDate, /*THE LAST SWAP MATURES AT THE FRA MATURITY*/
                      SwpFloatFreq, SwpFloatBasis, SwpFloatLegDP);
  if (err)
    return err;
  ...
  err = swp_f_make_FloatLegDatesAndCoverages(
      SwpFloatLegDP, today, &PayDates, /* THE SWAPS MATURITIES  (PayDates)*/
      &NumPayDates,                    /* THE NUMBER OF SWAPS */
      &FixingDates, &StartDates, &EndDates, &Coverage, NumFraUndSwap);
  ...
  for (i = 0; i < (*NumFraUndSwap); i++)
    (*FraUndSwapPEq)[i] = 0;
  for (i = 0; i < (*NumFraUndSwap) + 1; i++) {
    (*FraUndSWAPEndDates)[i] =
        add_unit(DTOL(SpotDate), i * 12 / (int)SwpFloatFreq, SRT_MONTH,
                 NO_BUSDAY_CONVENTION);
  }
  ...
  err = swp_f_LevelPayment(SpotDate, FraEndDate, SwpFixedFreqStr,
                           SwpFixedBasisStr, YieldCurveName, SwpFloatRefRate,
                           &FixedRefSwapLevel);

  err = swp_f_LevelPayment(SpotDate, FraEndDate, SwpFloatFreqStr,
                           SwpFloatBasisStr, YieldCurveName, SwpFloatRefRate,
                           &FloatRefSwapLevel);

  (*FraUndSwapPEq)[(*NumFraUndSwap) - 1] =
      FraPEQ * FloatRefSwapLevel / FixedRefSwapLevel;

  if ((*NumFraUndSwap) > 1) {
    SwapEndDate = add_unit(DTOL(SpotDate),
                           ((*NumFraUndSwap) - 1) * 12 / (int)SwpFloatFreq,
                           SRT_MONTH, NO_BUSDAY_CONVENTION);

    err = swp_f_LevelPayment(SpotDate, SwapEndDate, SwpFixedFreqStr,
                             SwpFixedBasisStr, YieldCurveName, SwpFloatRefRate,
                             &FixedRefSwapLevel);

    err = swp_f_LevelPayment(SpotDate, SwapEndDate, SwpFloatFreqStr,
                             SwpFloatBasisStr, YieldCurveName, SwpFloatRefRate,
                             &FloatRefSwapLevel);

    (*FraUndSwapPEq)[(*NumFraUndSwap) - 2] =
        -FraPEQ * FloatRefSwapLevel / FixedRefSwapLevel;
  }

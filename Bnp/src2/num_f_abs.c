    ...
    long*     seed)
{
    ...
    for (ibr = first_brownian; ibr <= last_brownian; ibr++)
    {
        ...
        for (k = 1; k < num_step; k++)
        {
            ...
            /* Initialises uni2 as uni1 : uni2 is a set of NPB blocks of NBuck points (dual to
             * paths) */
            ...
            for (j = 0; j < gen_path;)
            {
                /* Treats all the increments v[0..NPB-1] in the curent bucket # b by INCREASING
                 * order */
                for (i = 0; i < NPB / 2; i++)
                {
                    /* Chooses randomly a REMAINING increment to associate to the current path (#i)
                     * in current bucket (#b) */
                    ...
                    /* Copies the last value of the uniform bucket in place of the chosen value to
                     * allow to choose it next time */
                    uni2[temp] = uni2[NBuck - b - 1 + i * NBuck];
                }

                /* Permutates randomly these increments in the bucket (so far, first increment is
                 * lower,...) */

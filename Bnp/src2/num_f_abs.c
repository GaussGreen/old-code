...
Err ABSCube (double ***rand, 
			 long first_path, long last_path, 
			 long first_brownian, long last_brownian, 
...
/* Memory allocation */
	...
	if ((uni1==NULL) || (uni2==NULL) || (vect==NULL) || (spots==NULL) || (index==NULL))
...
/* Loop on the number of Brownians */
	...
	/* Permutates randomly the uniform mesh (for uncorrelated Brownians on first step) */
	...
	/* Loops on all the steps (after the first one) */
		...
		/* Sorts relative to spot value so far */		
			...
			/* Initialises uni2 as uni1 : uni2 is a set of NPB blocks of NBuck points (dual to paths) */
		...
		/* Treatment of all the increments, one by one */
			...
			/* Treats all the increments v[0..NPB-1] in the curent bucket # b by INCREASING order */
				for (i=0; i<NPB/2; i++)
				{			
					/* Chooses randomly a REMAINING increment to associate to the current path (#i) in current bucket (#b) */
				...
				/* Copies the last value of the uniform bucket in place of the chosen value to allow to choose it next time */
					uni2[temp] = uni2[NBuck-b-1+i*NBuck];									
				}
					
			/* Permutates randomly these increments in the bucket (so far, first increment is lower,...) */
				RandomPermutation (vect, NPB, seed);
					
			/* Associates these uniform numbers to their randomly selected path in the bucket */	
				for (i=0; i<NPB; i++)							
				{		
					spots[index[j+i]] += inv_cumnorm_fast(vect[i]);
					if (first_path+index[j+i] <= last_path)
					{
						rand[first_path+index[j+i]][ibr][first_step+k] = inv_cumnorm_fast(vect[i]);			
					}
					else
					{
						if (odd == SRT_NO)

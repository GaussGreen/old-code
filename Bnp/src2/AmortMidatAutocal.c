					   ...
					   )
{
	...
	// margins and spreads
	dResult = _leg_coupon_pv_(lToday,szYC,	plStart_Begin,plStart_End,plEnd_Begin,plPay_Begin,pdMargin_Begin,pdNotional_Begin,eBasis);
	dResult += _leg_coupon_pv_(lToday,szYC,plStart_Begin,plStart_End,plEnd_Begin,plPay_Begin,pdSpread_Begin,pdNotional_Begin,eBasis);
				   ...
				   )
{
	...
	if (dPower<1.)
	{
		szErr = swp_f_ForwardRate(lStart,lEnd,(char*)szFreq,(char*)szBasis,(char*)szYC,(char*)szRefRate,&dFRA);
		if(szErr) return szErr;
...
/*	SrtDiffusionType srt_vol_type; */
	...
	for(i=0;i<lNFixNot;++i)
	{
		...
		while( (j < am->fund_leg->num_cpn - 1 ) && ( am->fund_leg->cpn[j].start_date < am->fix_leg->cpn[i].start_date - 10) )
		{
			++j;
		}
	...
	*pdequinotional = dEquiv_Notional;
	...
	else
	{
		...
		while( (l < am->fund_leg->num_cpn) && (am->fund_leg->cpn[l].start_time < am->fix_leg->cpn[j].start_time - 10.0/365.0) )
		{
			++l;
		}
...
/*		if(model==1)
	...
	for(k=1; k<*nex; ++k)
	{
		...
		else
		{
			...
			while( (l < am->fund_leg->num_cpn) && (am->fund_leg->cpn[l].start_time < am->fix_leg->cpn[j+k].start_time - 10.0/365.0) )
			{
				++l;
			}
...
//	}
	...
	if( (!fixNotional) || (!long_strikes) || (!ex_fee)  || (!fix_start_dates) || (!fix_end_dates) || (!fix_pay_dates) )
	{
	...
	if( (!floatNotional) || (!float_margin) || (!float_spread) || (!float_start_dates) || (!float_end_dates)  || (!float_pay_dates) )
	{
		err = "memory allocation failed in am_calib_und";
		smessage("memory allocation failed in am_calib_und");
		goto FREE_RETURN;
	...
	err = amortMidat_compute_diagonal_prices_new_equistrike(yc, vc, ref, swap_freq, swap_basis, 
						...
						&nex, &firstex, &ex_bool, &diag_prices, &dequinotional, pdlong_strikes,european_model, 
	...
	/// Calibrate term structure again based on the combination of amortizing and equivalent strike swaptions
							...
							double		***prod_val)
{
	...
	for (i=l1; i<=u1; i++)
	{
		...
		{		
			...
			if(nCompExProb)
			{
				...
				if(i>l1&& j>l2 && j<u2)
				{
					...
					// if dExInd is different from the ajacent 4 cells (i-1,j-1), (i-1,j),(i-1,j+1),(i,j-1)
					...
					if( nNumID<=2)
					{
						...
						double dFltLeg = 0.;
						{
							...
							double _dFltInitCoeff_R1,_dFltInitCoeff_R2,_dFltInitBase,_dFltInitPayment;
							double _dFltTermCoeff_R1,_dFltTermCoeff_R2,_dFltTermBase,_dFltTermPayment;
							...
							for (l=0; l<num_fund_cpn; l++)
							{
								...
								_pdFltBase[l] = exp(-eval_const->df_alpha[ndf] - eval_const->df_beta[ndf] * __R1);
							...
							_dFltInitBase = exp(-eval_const->df_alpha[ndf] - eval_const->df_beta[ndf] *__R1);
							...
							_dFltTermBase = exp(-eval_const->df_alpha[ndf] - eval_const->df_beta[ndf] * __R1);
							...
							for (l=0; l<num_fix_cpn; l++)
							{
								...
								_pdFixBase[l] = exp(-eval_const->df_alpha[ndf] - eval_const->df_beta[ndf] * __R1);
			...
			prod_val[i][j][0] = (prod_val[i][j][0] - iv) > (-fee_total) ? (prod_val[i][j][0] - iv) : (-fee_total);
		}
		...
		if(nCompExProb)
		{
			...
			int nFlip = 0;
			for(pdExInd= ppdExIndicator[i]+l2+1;pdExInd<ppdExIndicator[i]+u2+1;++pdExInd)
			{
	...
	if(call_idx == 0)
	{
		fund_leg_pv =  swp_f_df(func_params->lToday,call->set_date,yc) * fund_leg->notional[call->fund_idx];
						...
						double		*phi12)
{
	...
	{	
		...
		fwd3_mid[i-1] = fact1 * (coef21 * exp(-lam2 * t2) * H2 + coef22 * phi2[i] + coef23 * phi12[i])
					 -fact2 * fwd1_mid[i-1];
	...
	else
	{
		for (i=l1; i<=u1; i++)
		{
			for (j=l2; j<=u2; j++)
			{
				...
				prod_val[i][j][0] = ppdExIndicator[i][j] == 1. ? 0. : prod_val[i][j][0] / ppdCondiDF[i][j];	
				/// TEST TEST TEST TEST TEST
					...
					)
{
	...
	//// that once the grid is set up - by the solver - at the termination, it remains fixed  as the solver 
					...
					int *pnEx)
{
	...
	double dInit_Prev = 0.;	
	double dIV_FirstEx; // dIV_FirstEx stores the PV of the swap with no cash flow until the first exercise date
	Err szErr;
	
	/// user-provided payoff function arguments
	genmidat_arg *pAmortArg = 
		memset(_alloca(adi_arg->nstp*sizeof(genmidat_arg)), 0, adi_arg->nstp*sizeof(genmidat_arg));
	ptr_genmidat_arg *ppAmortArg = 
		memset(_alloca(adi_arg->nstp*sizeof(ptr_genmidat_arg)), 0, adi_arg->nstp*sizeof(ptr_genmidat_arg));

	// global exercise indicator
	double*** pppdExIndicator = memset(_alloca(nNumEx * sizeof(double**)),0,nNumEx*sizeof(double**)); 
	
	// global conditional discount factor
	// i.e. discount factors observed at time tnEx and paid at tnEx+1 (>tnEx)
	double*** pppdCondiDF = memset(_alloca(nNumEx * sizeof(double**)),0,nNumEx*sizeof(double**)); 
	...
	int *pnExToTime = memset(_alloca(nNumEx * sizeof(int)),0,nNumEx*sizeof(int)); /// initialize to 0
					...
					int *pnEx)
{
	...
	*pnEx = nNumEx;
	...
	/// compute option value only
	if(!nCompExProb)
		return _price_genmidat_ts_(adi_arg,ppAmortArg,pdLambdaValue,pdLambdaTime,nLambdaSize,pdOptionPV);
	// else compute both option value and exercise probabilities
	else 	
		 szErr = "exercise probability not available with Tau TS yet !" ;  // _price_genmidat_exprob_(adi_arg,ppAmortArg,pnExToTime,pdOptionPV,*ppdExProb);
...
//			fix_leg_pv += fix_fix_cpn[i] 
	...
	if (call_feat == 1)
	{
		...
		// NB: most cases fix rates are the same! but might not be correct for some cases!!! shall come back later ... 
		szErr = _price_genmidat(am,und,adi_arg,today,fix_rate[0],&call, (nOutPutExProb ? ppdExProb : 0),(nOutPutExProb ? ppdAvgSwapRate : 0),pnEx );
...
//			fix_leg_pv += fix_fix_cpn[i] 
	...
	if (call_feat == 1)
	{
		smessage ("Launching adi, time steps requested: %d, actual: %d", req_stp, adi_arg->nstp);
		{
			...
			szErr = _price_genmidat(am,und,adi_arg,today,fix_rate[0],&call, &pdEquiExProb,&pdAvgStrike,pnEx );
...
//			fix_leg_pv += fix_fix_cpn[i] 
	...
	{
		smessage ("Launching adi, time steps requested: %d, actual: %d", req_stp, adi_arg->nstp);
		szErr = _price_genmidat_ts(am, und, adi_arg,today,fix_rate[0],pdLambdaValue,pdLambdaTime,nLambdaSize, &call, (nOutPutExProb ? ppdExProb : 0),(nOutPutExProb ? ppdAvgSwapRate : 0),pnEx);

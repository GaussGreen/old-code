    ...
    double*  VolShift_bskt)
{
    ...
    double **SecondMomentsShifted, ***ThirdMomentsShifted, **SecondMoments, ***ThirdMoments;
    ...
    for (i = 0; i < Num_assets; i++)
        ...
        {
            SecondMomentsShifted[i][j] = (Forwards[i] + Shifts[i]) * (Forwards[j] + Shifts[j]) *
    ...
    for (i = 0; i < Num_assets; i++)
        for (j = i; j < Num_assets; j++)
            ...
            {
                ThirdMomentsShifted[i][j][k] = (Forwards[i] + Shifts[i]) *
                                               (Forwards[j] + Shifts[j]) *
                                               (Forwards[k] + Shifts[k]) *
    ...
    for (i = 0; i < Num_assets; i++)
        for (j = i; j < Num_assets; j++)
        {
            SecondMoments[i][j] = SecondMomentsShifted[i][j] - Shifts[i] * Forwards[j] -
                                  Shifts[j] * Forwards[i] - Shifts[i] * Shifts[j];
    ...
    for (i = 0; i < Num_assets; i++)
        for (j = i; j < Num_assets; j++)
            ...
            {
                ThirdMoments[i][j][k] =
                    ThirdMomentsShifted[i][j][k] - SecondMoments[i][j] * Shifts[k] -
                    SecondMoments[i][k] * Shifts[j] - Forwards[i] * Shifts[j] * Shifts[k] -
                    Shifts[i] * SecondMoments[j][k] - Shifts[i] * Shifts[k] * Forwards[j] -
                    Shifts[i] * Shifts[j] * Forwards[k] - Shifts[i] * Shifts[j] * Shifts[k];
    ...
    err = srt_f_matching_moments(
        &Tempshift_bskt, &varshift_bskt, Temp_Fwd_bskt, SecondMoment_bskt, ThirdMoment_bskt);
...
double srt_f_GammaInvOption(
    double dStrike, double dM1, double dM2, SrtCallPutType call_put, SrtGreekType greek)
{
    return srt_f_GammaInvOpt(
        dStrike,
...
double srt_f_GammaInvOpt(
    double strike, double a, double b, SrtCallPutType call_put, SrtGreekType greek)
{
    double dInvStrike = .0;
    if ((a != 1.0) && (strike > 0) && (greek == PREMIUM))
    {
        ...
        if (call_put == SRT_CALL)
            return gammp(a - 1.0, dInvStrike) / b / (a - 1.0) - strike * gammp(a, dInvStrike);
        else
...
/* Call/Put price using Gamma Inverse function */
double srt_f_GammaOption(
    double dStrike, double dM1, double dM2, SrtCallPutType call_put, SrtGreekType greek)
{
    return srt_f_GammaOpt(
        dStrike, dM1 * dM1 / (dM2 - dM1 * dM1), (dM2 - dM1 * dM1) / dM1, call_put, greek);
}

double srt_f_GammaOpt(
    double strike, double a, double b, SrtCallPutType call_put, SrtGreekType greek)
{
    double dStrike = .0;
    if ((a != 1.0) && (strike > 0) && (greek == PREMIUM))
    {
        ...
        if (call_put == SRT_CALL)
            return a * b * (1.0 - gammp(a + 1.0, dStrike)) - strike * (1.0 - gammp(a, dStrike));
        else
...
{
    Err    err = NULL;
    double dM1 = .0, dM2 = .0, dM21 = .0, dM22 = .0, dM3 = .0, *dNewton, dSub22, dSigma, dForward;
    int    i, j, k;
    ...
    case PREMIUM:
        ...
        for (i = 0; i < iCms; i++)
        {
            ...
            dM1 += dCmsForwards[i];
            dM21 +=
                dCmsForwards[i] * dCmsForwards[i] * exp(dCmsVols[i] * dCmsVols[i] * dStartDates[i]);
            dSub22 = .0;
            for (j = i + 1; j < iCms; j++)
        ...
        else
        {
            ...
            if ((iMethod == 0) || (iMethod == 3))
                *dAnswer =
                    srt_f_optblksch(dForward, dStrike, dSigma, 1.0, 1.0, SrtCallPut, SrtGreek);
        }
        break;

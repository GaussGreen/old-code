...
answer=F*F*F*(exp(3*var)-1)+3*(M2-M1*M1)*(M1-F)+M1*M1*M1;
return answer;
}
...
/* solve the system in (F,d,Sig) knowing (M1,M2,M3) s.t. 
				...
				F^3*(exp(3*sig^2*T)-1)+3*(M2-M1^2)*(M1-F)=M3-M1^3 
Use a dichotomy */
Err srt_f_matching_moments(double *shift,double *varshift,
...
ThirdMomentsShifted=f3tensor(0,Num_assets-1,0,Num_assets-1,0,Num_assets-1);
SecondMoments=dmatrix(0,Num_assets-1,0,Num_assets-1);
ThirdMoments=f3tensor(0,Num_assets-1,0,Num_assets-1,0,Num_assets-1);

/* Calculates the 2 and 3 moments of the (assets+shifts) */
for (i=0;i<Num_assets;i++)
	for (j=i;j<Num_assets;j++)
	{
		SecondMomentsShifted[i][j]=(Forwards[i]+Shifts[i])*(Forwards[j]+Shifts[j])*exp(Correl[i][j]*VolShifts[i]*VolShifts[j]*Maturity);
...
for (i=0;i<Num_assets;i++)
	for (j=i;j<Num_assets;j++)
		...
		{
			ThirdMomentsShifted[i][j][k]=(Forwards[i]+Shifts[i])*(Forwards[j]+Shifts[j])*(Forwards[k]+Shifts[k])*
		exp((Correl[i][j]*VolShifts[i]*VolShifts[j]+Correl[i][k]*VolShifts[i]*VolShifts[k]+Correl[j][k]*VolShifts[j]*VolShifts[k])*Maturity);
...
for (i=0;i<Num_assets;i++)
	for (j=i;j<Num_assets;j++)
	{
		SecondMoments[i][j]=SecondMomentsShifted[i][j]-Shifts[i]*Forwards[j]-Shifts[j]*Forwards[i]-Shifts[i]*Shifts[j];		
...
for (i=0;i<Num_assets;i++)
	for (j=i;j<Num_assets;j++)
		...
		{
			ThirdMoments[i][j][k]=ThirdMomentsShifted[i][j][k]-SecondMoments[i][j]*Shifts[k]-SecondMoments[i][k]*Shifts[j]
	-Forwards[i]*Shifts[j]*Shifts[k]-Shifts[i]*SecondMoments[j][k]-Shifts[i]*Shifts[k]*Forwards[j]-Shifts[i]*Shifts[j]*Forwards[k]
...
for (i=0;i<Num_assets;i++)
	for (j=0;j<Num_assets;j++)
		...
		{
			ThirdMoment_bskt+=Weights[i]*Weights[j]*Weights[k]*ThirdMoments[i][j][k];
		}
...
free_f3tensor(ThirdMomentsShifted,0,Num_assets-1,0,Num_assets-1,0,Num_assets-1);
free_dmatrix(SecondMoments,0,Num_assets-1,0,Num_assets-1);
free_f3tensor(ThirdMoments,0,Num_assets-1,0,Num_assets-1,0,Num_assets-1);
...
double eval_asian( int ndate, double* date, double* fwds, double* vols, 
				   ...
				   int npast_fix, double avg_cur, double disc, SrtCallPutType call_put)
{	
	long		i;
...
{	
	...
	for( i = 0; i < nfixing; i++)
	{
		while ((nf < nforwards) && (forwards_date[nf] < fixing_date[i])) nf++;
		if (nf == 0)  fwds[i] = forwards[0];
		else if (nf == nforwards)  fwds[i] = forwards[nforwards-1];
		else 
		{ 
			x = (fixing_date[i] - forwards_date[nf-1]) / (forwards_date[nf] - forwards_date[nf-1]);
			fwds[i] = x*forwards[nf] + (1.0 - x)*forwards[nf-1];
		}

		while ((nv < nvol) && (vol_date[nv] < fixing_date[i])) nv++;
		if (nv == 0)  vols[i] = volat[0];
		else if (nf == nforwards)  vols[i] = volat[nforwards-1];
		else 
		{ 
	...
	switch( greek )
	{
		case PREMIUM : 
			...
			*answer = eval_asian(nfixing, fixing_date, fwds, vols, spot,
								maturity, strike, npast_fix, avg_cur, disc, call_put);
			break ;
...
double srt_f_GammaInvOption (double 			dStrike,
							 ...
							 SrtGreekType		greek)
{
	return  srt_f_GammaInvOpt(dStrike, 
							  (2.0 * dM2 - dM1 * dM1) / (dM2 - dM1 * dM1), 
							  (dM2 - dM1 * dM1) / dM2 / dM1, 
							  call_put, greek);
}
					  ...
					  SrtGreekType		greek)
{
	...
	if ((a != 1.0) && (strike > 0) && (greek == PREMIUM)) 
	{
		...
		if (call_put == SRT_CALL)
			...
			return strike * gammp(a, dStrike)
				   - a * b * gammp(a + 1.0, dStrike);
	}
	else
		return .0;
}
...
/* Two moments and three moments matching code for Asian option
	In: Cms Start Dates (in the correct basis (Date - today)/365 ), 
		...
		Correl as a vector (since the difference between two dates should be constant
		and Correl[0] = 1 and Correl[i] = <w1,w2>
		iCms the number of Cms,
		...
	...
	switch(SrtGreek)
	{
		case PREMIUM : 
			...
			for (i = 0; i < iCms; i++)
			{
				...
				dSub22 = .0;
				for (j = i + 1; j < iCms; j++)
					dSub22 += dCmsForwards[j] * 
							exp(dCorrels[j - i] * dCmsVols[i] * dCmsVols[j] * dStartDates[i]);
					...
					for (j = 0; j < iCms; j++)
						for (k = 0; k < iCms; k++)
							dM3 += dCmsForwards[i] * dCmsForwards[j] * dCmsForwards[k] *
								exp(dCorrels[abs(j - i)] * dCmsVols[i] * dCmsVols[j] * min(dStartDates[i], dStartDates[j]) 
									+ dCorrels[abs(j - k)] * dCmsVols[k] * dCmsVols[j] * min(dStartDates[k], dStartDates[j])
									+ dCorrels[abs(i - k)] * dCmsVols[i] * dCmsVols[k] * min(dStartDates[i], dStartDates[k]));
			}
			...
			/* The option maturity is equal to 1 since it's include in the matching moments */
			if (SrtVolType == SRT_NORMAL)
				*dAnswer = srt_f_optblknrm(dForward, dStrike, dSigma, 1.0, 1.0, SrtCallPut, SrtGreek);
			else
			{
				if (iMethod == 1)

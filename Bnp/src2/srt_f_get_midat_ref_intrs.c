...
{
    Err    err = NULL;
    double dx, *swp_spread, *cvg, ref_swap_rate, swap_rate, yr_to_exp_from_exer_date,
        yr_to_midat_exer;
    ...
    for (i = 1; i <= nEx; i++)
        midat_exer_dates[i] = add_unit(midat_start_dates[i], -spot_lag, SRT_BDAY, SUCCEEDING);
    ...
    for (i = 1; i <= nEx; i++)
    {
        ...
        err = swp_f_setSwapDP(
            midat_start_dates[i], midat_theo_end_date, fixed_freq, fixed_basis, &ref_swp_dp);
        if (err)
            return err;

        err = swp_f_ForwardRate_SwapDP(&ref_swp_dp, yc_name, ref_rate_code, &ref_swap_rate);
        if (err)
            return err;
        ...
        {
            err = swp_f_setSwapDP(
                midat_start_dates[i + j], midat_theo_end_date, fixed_freq, fixed_basis, &swp_dp);

            if (err)
                return err;

            err = swp_f_ForwardRate_SwapDP(&swp_dp, yc_name, ref_rate_code, &swap_rate);
            if (err)
                return err;
            ...
            yr_to_exp_from_exer_date =
                (double)(midat_exer_dates[j + i] - midat_exer_dates[i]) * YEARS_IN_DAY;
                 ...
                 k++) /* LOOP ON THE NUMBER OF DISCRETISATION POINTS */
            {
                ...
                if (swp_vol_type == SRT_LOGNORMAL)
                {
                    ...
                    prob_dens_func -= 2 * srt_f_optblksch(
                                              ref_swap_rate,
                                              tex_ref_swp_rate[k],
                                              swp_vol,
                                              yr_to_midat_exer,
                    ...
                    swp_premium[i][j] += prob_dens_func * level_cash *
                                         srt_f_optblksch(
                                             k * dx + swp_spread[j],
                                             swp_strikes[i + j],
                                             swp_fwd_vol[i][j],
                ...
                else
                {
                    ...
                    prob_dens_func -= 2 * srt_f_optblknrm(
                                              ref_swap_rate,
                                              tex_ref_swp_rate[k],
                                              swp_vol,
                                              yr_to_midat_exer,

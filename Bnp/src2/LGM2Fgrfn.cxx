...
Err payoff_lgm2f_pde(double evt_date, double evt_time, void *func_parm,

                     /* Market data	*/
                     void *yc,

                     /* Model data	*/
                     double lam1, double lam2, double rho, double phi1,
  ...
  for (i = l1; i <= u1; i++) {
    for (j = l2; j <= u2; j++) {
      for (k = 0; k < total->num_df; k++) {
        local->evt->df[0][k] =
            exp(total->dff[k] + total->gam12[k] - total->gam1[k] * r1[i] +
                -total->gam2[k] * r2[i][j]);
      }

      err = FIRSTEvalEvent(global, local, nprod, 2, NULL, NULL, prod_val[i][j],
                           &temp);
      if (err) {
        return err;
      }
...
Err payoff_lgm2fTau_pde(double evt_date, double evt_time, void *func_parm,
  ...
  static double temp, lam1, lam2, gam1, gam2, gam12, fact1, fact1temp, fact2,
      fact2temp;
  ...
  for (i = l1; i <= u1; i++) {
    for (j = l2; j <= u2; j++) {
      for (k = 0; k < total->num_df; k++) {
        local->evt->df[0][k] =
            total->dff[k] * exp(total->gam12[k] - total->gam1[k] * r1[i] +
                                -total->gam2[k] * r2[i][j]);
      }
...
Err grfn_payoff_lgm2f_mc(
  ...
  if (total->do_dom) {
    for (l = 0; l < total->num_dom_df; l++) {
      local->evt->df[0][l] = total->dom_dff[l] * exp(total->dom_gam12[l] -
                                                     total->dom_gam1[l] * R1D +

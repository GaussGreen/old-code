...
/* -------------------------------------------------------------------------
   ...
   rk=e^{x}*K_{nu}(x) and their derivatives rip and rkp for positive x and for
   xnu >0. The relative accuracy is within one or two significant digits of
   BESSEL_EPS. FPMIN is a number close to the machine smallest floating point
   number.
   -------------------------------------------------------------------------- */

Err bessik(double x, double xnu, double *ri, double *rk, double *rip,
           double *rkp) {
  ...
  if (x < XMIN) {
    ...
    for (i = 1; i <= MAXIT; i++) {
      ...
      del1 = c * (p - i * ff);
      sum1 += del1;
      if (fabs(del) < fabs(sum) * BESSEL_EPS)
        break;
    }
    if (i > MAXIT) {

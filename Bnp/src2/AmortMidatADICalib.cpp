...
//								int nZeta_Begin,// beginning position of Zeta no
//pdTS 								int nG_Begin,// beginning position of G no pdTS
//								// results
//								_GenEuropean* pInst
//							   )
//{
...
//								  const _GenEuropean*
//pGenEuroBlock_Begin, 								  const _GenEuropean* pGenEuroBlock_End
//								  )
//{
...
//									long m, // number of
//equations 									long n, // number of unknowns 									double xVec[], // x 									double fVec[], 									Nag_Comm* pComm
//									)
//{
...
//	{
//		/// locate indeces
//		const long *plFixStart_Begin_ =
//find_min_greater_than(plEx_Begin,plFixStart_Begin,nSize_Fix,sizeof(*plEx_Begin),lless); 		int
//nFixOffSet = plFixStart_Begin_ - plFixStart_Begin;
//
//		const long *plFltStart_Begin_ =
//find_min_greater_than(plEx_Begin,plFltStart_Begin,nSize_Flt,sizeof(*plEx_Begin),lless); 		int
//nFltOffSet = plFltStart_Begin_ - plFltStart_Begin;
//
//		//_ASSERTE(nFixOffSet!=nSize_Fix);
//		//_ASSERTE(nFltOffSet!=nSize_Flt);
//
//		// NB: in case indeces are not located, e.g. the last call in call freq = q and fix
//freq = s 		*pdStrike_Begin = Equi_Strike( 			lToday, 			szYC,
...
//		const long *plFltStart_Begin_ =
//find_min_greater_than(plEx_Begin,plFltStart_Begin,nSize_Flt,sizeof(*plEx_Begin),lless); 		const int
//nFltOffSet = plFltStart_Begin_ - plFltStart_Begin;
//
...
//	const char *szVC,
//	Err (*get_cash_vol)(char*,double,double,double,int,char	*,double*,double*),	//
//functor to get cash vol from the market
//	/// MidAt General Specs
//	long lTheoEnd,
//	const char *szRefRate, // Refrate
//	const char *szFreq, // frequency of the fixed leg -"M","Q","S","A"
...
//		pdLam_Begin,
//		(double*)_memset(&dZero,_alloca(nSize_Lam*sizeof(double)),nSize_Lam,sizeof(double)),
//// lambda shift 		nNumFactor, 		nNumFactor==1?1.e-5:*pdAlpha, 		nNumFactor==1?1.e-5:*pdGamma,
...
//otherwise 	const double *pdAlpha, 	const double *pdGamma, 	const double *pdRho,
...
//diagonal 		int			n1FEqui,//1: 2F lambda will calibrate to the cap priced within
//calibrated 1F	with the given lambda
//		int			nSkipLast,//If 1, the last option is disregardedand the forward
//volatility is flat from option	n-1 		int			nUseJump, 		double
//dMaxVarJump, 		int			nStrikeType, 		int			nEuroModel,
//		// Calibration results
//		double **ppdSigT_Begin,
//		double **ppdSigT_End,
//		double **ppdSig_Begin,
...
//		MyModel.m_dAlpha*sqrt(1-MyModel.m_dRho*MyModel.m_dRho),//const double
//m_dAlpha_Sqrt1mRhoSqrd;
//		MyModel.m_dGamma*MyModel.m_dRho/sqrt(1.-MyModel.m_dRho*MyModel.m_dRho),//const double
...
//
//	// more alias
//	const _Calib_LamTS_DiagEuro_Comm*
//pCalib_LamTS_DiagEuro_Begin=pComm->m_pCalib_LamTS_DiagEuro_Begin; 	const double
//*pdAlpha=pComm->m_pdAlpha; 	const double *pdGamma=pComm->m_pdGamma; 	const double
//*pdRho=pComm->m_pdRho;
//
...
//	const char *szVC,
//	Err (*get_cash_vol)(char*,double,double,double,int,char	*,double*,double*),	//
//functor to get cash vol from the market 	long lToday, 	const char *szFreq, 	const char *szBasis,
//	const char *szRefRate,
//	long lTheoEnd,
//	int nOnefequi,
//	// model
...
//	const _Calib_LamTS_DiagEuro_Comm
//*pCalib_LamTS_DiagEuro_Begin=_alloca(sizeof(_Calib_LamTS_DiagEuro_Comm)*m); 	const
//_Calib_LamTS_DiagEuro_Comm *pCalib_LamTS_DiagEuro_End = pCalib_LamTS_DiagEuro_Begin+m;
//	_Calib_LamTS_DiagEuro_Comm *pCalib_LamTS_DiagEuro =
...
//	// ATM rate
//	const char *szErr =
//_fill_swapcashrate_diag(szYC,plExStart_Begin,plExStart_Begin+m,lTheoEnd,szFreq,szBasis,szRefRate,pdCoupon_Begin);
//	if(szErr)return szErr;
//
//
//	// because we have a co-terminal system, all options share the same terminal measure
//numeraire 	if(szErr =_alloc_Seq(1,&Numeraire,0,0)) return szErr;
//	{
...
//
//		// locate indeces
//		const long *plFixPay_nI_Begin = (const
//long*)find_min_greater_than(plExStart_Begin,plFixPay_Begin,nSize_Fix,sizeof(long),lless); 		const
//long *plFltPay_nI_Begin = (const
//long*)find_min_greater_than(plExStart_Begin,plFltPay_Begin,nSize_Flt,sizeof(long),lless);
//
...
//			(const
//double*)_memset(pdCoupon_Begin,_alloca(sizeof(double)*nSize_Fix_nI),nSize_Fix_nI,sizeof(double)),//pdFixNotional_Begin+nOffSet_Fix,

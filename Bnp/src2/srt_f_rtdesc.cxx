...
void srt_f_rtsetyp(SrtRtFnc rt, void *ptr) { rt->yp = ptr; }
void *srt_f_rtgetyp(SrtRtFnc rt) { return rt->yp; }
void srt_f_rtsettm(SrtRtFnc rt, double *tm) { rt->t = tm; }
double *srt_f_rtgettm(SrtRtFnc rt) { return rt->t; }
void srt_f_rtsetdf(SrtRtFnc rt, double *df) { rt->df = df; }
double *srt_f_rtgetdf(SrtRtFnc rt) { return rt->df; }
void srt_f_rtsetdt(SrtRtFnc rt, Ddate *dt) { rt->date = dt; }
Ddate *srt_f_rtgetdt(SrtRtFnc rt) { return rt->date; }
void srt_f_rtsetcvg(SrtRtFnc rt, Ddate *cvg) { rt->cvg = cvg; }
Ddate *srt_f_rtgetcvg(SrtRtFnc rt) { return rt->cvg; }
int srt_f_rtgetlen(SrtRtFnc rt) { return rt->len; }
...
/*-----------------input validation-----------------------------*/
/*
        Error check the arguments describing a swap rate        , then populate
   a SrtSwapDP
*/
static Err srt_f_sefb_to_SwapDP(Date start, Date end_or_nfp, char *freq,
                                char *basis, SrtSwapDP *sdp) {
  Err err;
...
/*<%%STA>-----------------------------------------------------------------
  ...
  DESCRIPTION     :given input like the swap tools function
    ...
    if end is used and is not a number of full periods        , then it must be
an IMM date. (Or there will be an error). Num periods is interpreted as the
number of periods after the first fixing date >= start. FREQUENCY: if frequency
is less than monthly        , dates must fall on IMM quarterly months        ,
e.g. {m        ,j        ,s        ,d}.
  ...
  *y = atoi(futstr + 3);
  ...
  if (len == 5 || futstr[5] == 'Q') {
    if (*m != SRT_MAR && *m != SRT_JUN && *m != SRT_SEP && *m != SRT_DEC) {
      return serror(
          "Quarterly futs must be in {m        ,j        ,s        ,d}: %s",
          futstr);
    }
    *nm = 3;
  } else if (futstr[5] == 'M')
...
SrtErr srt_f_rtmkimmswp(Date start, char *startfutstr, Date end_or_nfp,
                        char *endfutstr, char *freqstr, char *basisstr,
                        SrtRtFnc *rt) {
  double *cvg, *t;
  Ddate *swpdate;
  ...
  *rt = NULL;
  ...
  if (endfutstr) {
    ...
    if (nm > 0 && nm != nm2) {
      return serror("futs must be same kind %s        , %s for IMM swap",
                    startfutstr, endfutstr);
    }
  ...
  /*
   * check that if freq is less than monthly        , end date falls on {m ,j ,s
   * ,d}
   */
  m2 = month(end);
  y2 = year(end) + 1900;
  if ((int)freq < 12) {
    if (m2 != SRT_MAR && m2 != SRT_JUN && m2 != SRT_SEP && m2 != SRT_DEC) {
      return serror(
          "%s IMM swaps must end on {m        ,j        ,s        ,d}",
          freqstr);
    }
  }
  ...
  if (first_non_stub > end || start >= end || start > first_non_stub ||
      nfp < 0) {
  ...
  *rt = srt_f_rtalloc();
  ...
  for (i = 0; i < nfp; i++) {
    ...
    cvg[flg + i + 1] =
        coverage(DTOL(swpdate[flg + i]), DTOL(swpdate[flg + i + 1]), basis);
  }
  if (flg) {
...
*/
static Err srt_f_rtmkswp(Date start, Date end_or_nfp, char *freq, char *basis,
                         SrtRtFnc *ret) {
  SrtRtFnc rt;
  SrtDateList dl;
  SwapDP sdp;
...
*/
static Err srt_f_rtmkfra(Date start, Date end_or_nfp, char *basis,
                         SrtRtFnc *ret) {
  Err err;
  SrtRtFnc rt;
  ...
  if (end_or_nfp > start) {
    ...
    end_or_nfp =
        add_unit(start, (int)end_or_nfp, SRT_MONTH, MODIFIED_SUCCEEDING);
  } else
    return serror("bad end_or_nfp srt_f_rtdesc");
...
SrtErr srt_f_rtmk(char *rtname, Date start, char *stastr, Date end_or_nfp,
...
#define HANDLE {if(err){printf("Err: %s\n"        ,(char*)err); exit(0);}}
        ...
        s = date(11        ,11        ,1994);
        err = srt_f_rtmk("SWAP"        ,s        ,NULL        ,10        ,NULL
,"SA"        ,"A5"        ,&rt); HANDLE; srt_f_rtprint(NULL        ,rt);
        srt_f_rtfre(rt);

        err = srt_f_rtmk("LVL"        ,s        ,NULL        ,10        ,NULL
,"A"        ,"BB"        ,&rt); HANDLE; srt_f_rtprint(NULL        ,rt);
        srt_f_rtfre(rt);

        err = srt_f_rtmk("FRA"        ,s        ,NULL        ,6        ,NULL
,NULL        ,"MM"        ,&rt); HANDLE; srt_f_rtprint(NULL        ,rt);
        srt_f_rtfre(rt);

        err = srt_f_rtmk("DF"        ,s        ,NULL        ,12        ,NULL
,NULL        ,NULL        ,&rt); HANDLE; srt_f_rtprint(NULL        ,rt);
        srt_f_rtfre(rt);

        err = srt_f_rtmk("IMMSWP"        ,s        ,NULL        ,10        ,NULL
,"SA"        ,"A5"        ,&rt); HANDLE; srt_f_rtprint(NULL        ,rt);
        srt_f_rtfre(rt);

        err = srt_f_rtmk("IMMSWP"        ,0        ,"DEC95Q"        ,10 ,NULL
,"SA"        ,"A5"
      ,&rt); HANDLE; srt_f_rtprint(NULL        ,rt); srt_f_rtfre(rt);
}
*/

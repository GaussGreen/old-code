...
Err monte_carlo_sabr(double **rndm_mat, double forw, double vovol, double beta,
                     double rho, double sigma, double num_paths,
  ...
  *pathnogood = 0;
...

  {
    ...
    case 1:
      ...
      statevar =
          statevar + delta * delta_t + 2 * sqrt(statevar) * g[0] * sqrt_delta_t;
      Forward[0] = pow(statevar * (1 - beta) * (1 - beta) * Eta * Eta,
                       1 / (2 - 2 * beta));
    ...
    case 2:
      ...
      statevar0 = statevar0 + Eta * g[0] * sqrt_delta_t -
                  beta * Eta * Eta * delta_t / (2 * (1 - beta) * statevar0);
      statevar1 = pow(Forward[1], 1 - beta) / (1 - beta);
      statevar1 = statevar1 + sigma * g[0] * sqrt_delta_t -
                  beta * sigma * sigma * delta_t / (2 * (1 - beta) * statevar1);
    ...
    case 3:
      ...
      Forward[0] += pow(Forward[0], beta) * (Eta * g[0] * sqrt_delta_t) +
                    0.5 * Eta * Eta * beta * pow(Forward[0], 2 * beta - 1) *
                        (g[0] * g[0] - 1) * delta_t;
      Forward[1] += pow(Forward[1], beta) * (sigma * g[0] * sqrt_delta_t) +
                    0.5 * sigma * sigma * beta * pow(Forward[1], 2 * beta - 1) *
                        (g[0] * g[0] - 1) * delta_t;
...

{
  ...
  /* Spectrunc*/
  if (sampletype == 3) {
    if (err = SpecTruncCube(cube, times_at_steps, 0.0001, 1, (long)num_paths, 1,
                            2, 1, (long)num_steps, &seed)) {
  ...
  {
    if (err = ABSCube(cube, 1, (long)num_paths, 1, 2, 1, (long)num_steps,
                      &seed)) {
      free_dmatrix(price, 1, 2, 1, num_strikes);
      free_dvector(Forward, 1, 2);
      free_f3tensor(cube, 1, (long)num_paths, 1, 2, 1, (long)num_steps);
  ...
  for (i = 1; i <= num_strikes; i++) {
    blkbeta[i] = srt_f_optblkschbeta(forw, strike[i], sigma, maturity, beta,
                                     1.0, SRT_CALL, SRT_PREMIUM);
  }

  for (cur_path = 1; cur_path <= num_paths; cur_path++) {

    if (err = monte_carlo_sabr(cube[cur_path], forw, vovol, beta, rho, sigma,
  ...
  for (i = 1; i <= num_strikes; i++) {
    ...
    price[1][i] =
        price[1][i] / actnumpaths -
        correlation *
            sqrt(stdev[1][i] - price[1][i] * price[1][i] / actnumpaths) /
            sqrt(stdev[2][i] - price[2][i] * price[2][i] / actnumpaths) *
            (price[2][i] / actnumpaths - blkbeta[i]);

  ...
  *pathnogood = 0;
...

  {
    ...
    case 0:
      ...
      //,beta)*g[0]*sqrt_delta_t; 			Forward[1]+=sigma*pow(Forward[1]
...
Err sabrBVMmontecarlo(
  ...
  /*	if (sampletype == 3)
     ...
     ,(long) num_paths  ,1  ,2  ,1  ,(long) num_steps  ,&seed))
                  {
                          free_dmatrix(price  ,1  ,2  ,1  ,num_strikes);
                          free_f3tensor(cube  ,1  ,(long) num_paths  ,1  ,2  ,1
     ,(long) num_steps); free_dvector(Forward  ,1  ,2); free_dvector(blkbeta  ,1
     ,num_strikes); smessage("error in sobol cube"); return err;
                  }
          }
  */
  /*Sobol*/
  /*	if (sampletype==2)
          {
                  if (err = sobol_init(1  ,(long) num_paths  ,1  ,2  ,1  ,(long)
     num_steps))
                  {
                          free_dmatrix(price  ,1  ,2  ,1  ,num_strikes);
                          free_f3tensor(cube  ,1  ,(long) num_paths  ,1  ,2  ,1
     ...
     ,num_strikes); smessage("error in sobol cube"); return err;
                  ...
                  if (err = sobol_cube(cube  ,1  ,(long) num_paths  ,1  ,2  ,1
     ,(long) num_steps))
                  {
                          free_dmatrix(price  ,1  ,2  ,1  ,num_strikes);
                          free_f3tensor(cube  ,1  ,(long) num_paths  ,1  ,2  ,1
  ...
  */
  ...
  /*	else if (sampletype == 1)
          {
                  if (err = ABSCube(cube  ,1  ,(long) num_paths  ,1  ,2  ,1
     ,(long) num_steps  , &seed))
                  {
                          free_dmatrix(price  ,1  ,2  ,1  ,num_strikes);
...
Err BVMApproxPrice(double Xinit, double XStar, double GStar, double Sigma,
  ...
  *Fwd = 0;
  ...
  //	for(j=0;j<NStrikes;++j)
  //	{
  //		err = srt_f_optimpvol(ImpliedVol[j]  , *Fwd  , Strikes[j]  ,
  //Maturity  , 1  , SRT_CALL  , SRT_LOGNORMAL  , &(impliedvol)); 		ImpliedVol[j]
  //= impliedvol;
  //	}

	...
	switch(iF)
	{
		case 4:
			switch(iSigma)
			{
				case 0:
					...
					o6=pow(o5,2.);
					o7=o4*T;
					res = (0.0249338925250895*K*exp((-0.5*o6)/(o4*T)-0.125*o4*T)*(-8.*o4*o6*(6.+o7)*T+16.*pow(o5,4.)+o3*(-4.+o7)*pow(T,3.))*pow(T,-3./2.)*sqrt(o2))/o3;
				break;
			}
		break;
		...
		case 3:
			...
			case 0:
				...
				o3=pow(sigma,2.);
				o4=log(o2);
				res = -0.199471140200716*K*(2.*o4+3.*o3*T)*exp(-0.125*o3*T-(0.5*pow(o4,2.))/(o3*T))*pow(F,-3.)*pow(sigma,-3.)*pow(T,-3./2.)*sqrt(o2);
			break;
			case 1:
				o1=1/K;
				o2=F*o1;
				...
				o4=log(o2);
				o5=o3*T;
				res = 0.0498677850501791*K*(2.*o4+o5)*(-4.*o4*(o4+o5)+3.*o3*(4.+o5)*T)*exp(-0.125*o3*T-(0.5*pow(o4,2.))/(o3*T))*pow(F,-3.)*pow(sigma,-6.)*pow(T,-5./2.)*sqrt(o2);
			break;
			}
		break;
		...
		case 2:
			...
			case 1:
				...
				o4=log(o2);
				o5=pow(o4,2.);
				res = -0.0997355701003582*K*(-4.*o5+o3*T*(4.+o3*T))*exp((-0.5*o5)/(o3*T)-0.125*o3*T)*pow(F,-2.)*pow(sigma,-4.)*pow(T,-3./2.)*sqrt(o2);
			break;
			case 2:
				o1=1/K;
				o2=F*o1;
				...
				o5=pow(o4,2.);
				o6=o3*T;
				res = 0.0249338925250895*K*exp((-0.5*o5)/(o3*T)-0.125*o3*T)*pow(F,-2.)*pow(sigma,-7.)*(-8.*o3*o5*(10.+o6)*T+16.*pow(o4,4.)+(32.+o3*(4.+o6)*T)*pow(sigma,4.)*pow(T,2.))*pow(T,-5./2.)*sqrt(o2);
			break;
			}
		break;
		...
		case 1:
			...
			case 2:
				...
				o4=log(o2);
				o5=pow(o4,2.);
				res = (-0.04986778505017909*exp((-0.5*o5)/(o3*T)-0.125*o3*T)*pow(sigma,-5.)*(-4.*o3*o5*T-2.*o3*o4*T*(8.+o3*T)+8.*pow(o4,3.)+pow(sigma,6.)*pow(T,3.))*pow(T,-3./2.))/sqrt(o2);
			break;
			case 3:
				o1=1/K;
				o2=F*o1;
				...
				o8=pow(sigma,4.);
				o9=pow(T,2.);
				res = (0.012466946262544772*exp((-0.5*o6)/(o4*T)-0.125*o4*T)*(2.*o5*(-(o8*o9*(96.+o4*(12.+o7)*T))+4.*o5*(-((6.+o7)*o8*o9)+2.*o5*(-2.*o6+o4*o5*T+o4*(14.+o7)*T)))+o3*(-4.+o7)*pow(T,4.))*pow(T,-5./2.))/(o3*sqrt(o2));
			break;
			}
		break;
		...
		case 0:
			...
			case 4:
				...
				o5=pow(o4,2.);
				o6=o3*T;
				res = -0.00623347313127239*K*exp((-0.5*o5)/(o3*T)-0.125*o3*T)*pow(sigma,-9.)*(48.*o3*(12.+o6)*T*pow(o4,4.)-64.*pow(o4,6.)-12.*o5*(64.+o3*(8.+o6)*T)*pow(sigma,4.)*pow(T,2.)+(-12.+o6)*pow(sigma,10.)*pow(T,5.))*pow(T,-5./2.)*sqrt(o2);
			break;
			}
		break;
						...
						double *f2)
{
	...
	if(L_or_N==1)
	{
		...
		*f1 = Vol * Vol * Fwd * Fwd
				*(local_vol(Fwd, gamma, 0, 0, L_or_N) / local_vol(FwdInit, gamma, 0, 0, L_or_N) - 1.0) * Gamma;
...
static double Expectation_In_Fwd(double today,
						double date,
						double Fwd,
						double Vol,
						double Strike,
						...
						double *f2)
{
	...
	*f2 = 0.0;
	for(i=1;i<=NQuadrature;++i)
	{
		...
		res += w[i] * First_Order_Integrand_function(date,
									Fwd,
									Forward,
									Vol,
									Strike,
...
static double First_Order_Integral_In_Time(double today,
						double Fwd,
						double Vol,
						double Strike,
						double Maturity,
						...
						double *f2)
{
	...
	*f2 = 0.0;
	for(i=1;i<=NQuad_Leg;++i)
	{
		if(today < x_leg[i])
		{
			res += w_leg[i] * Expectation_In_Fwd(today, x_leg[i],
								Fwd,Vol,Strike,Maturity,gamma,alpha,rho,
								VolInfinity,Lambda,
								log_or_norm,
								local_vol,
								NQuad_Her,w_her,x_her,
								&tempf1, &tempf2);
						...
						)
{
	...
	f = First_Order_Integral_In_Time(date,
								Fwd, Vol, Strike, Maturity, 
								gamma, alpha, rho, VolInfinity, Lambda,
								L_or_N, local_vol,
								NQuad_Her, w_her, x_her, NQuad_Leg, w_leg, x_leg,
								&f1,
								&f2);

	f_f_up = First_Order_Integral_In_Time(date,
								Fwd + dFwd, Vol, Strike, Maturity, 
								gamma, alpha, rho, VolInfinity, Lambda,
								L_or_N, local_vol,
								NQuad_Her, w_her, x_her, NQuad_Leg, w_leg, x_leg,
								&f1_f_up,
	...
	f_f_down = First_Order_Integral_In_Time(date,
								Fwd - dFwd, Vol, Strike, Maturity, 
								gamma, alpha, rho, VolInfinity, Lambda,
								L_or_N, local_vol,
								NQuad_Her, w_her, x_her, NQuad_Leg, w_leg, x_leg,
	...
	f_vol_up = First_Order_Integral_In_Time(date,
								Fwd, Vol + dVol, Strike, Maturity, 
								gamma, alpha, rho, VolInfinity, Lambda,
								L_or_N, local_vol,
								NQuad_Her, w_her, x_her, NQuad_Leg, w_leg, x_leg,
								&f1_vol_up,
	...
	f_vol_down = First_Order_Integral_In_Time(date,
								Fwd, Vol - dVol, Strike, Maturity, 
								gamma, alpha, rho, VolInfinity, Lambda,
								L_or_N, local_vol,
								NQuad_Her, w_her, x_her, NQuad_Leg, w_leg, x_leg,
	...
	f_f_up_vol_up = First_Order_Integral_In_Time(date,
								Fwd + dFwd, Vol + dVol, Strike, Maturity, 
								gamma, alpha, rho, VolInfinity, Lambda,
								L_or_N, local_vol,
								NQuad_Her, w_her, x_her, NQuad_Leg, w_leg, x_leg,
								&f1_f_up_vol_up,
...
static double Second_Order_Expectation_In_Fwd(double today,
						double date,
						double Fwd,
						double Vol,
						double Strike,
						...
						double *x_leg)
{
	...
	for(i=1;i<=NQuad_Her;++i)
	{
		...
		res += w_her[i] * Second_Order_Integrand_function(date,
									Fwd,
									Forward,
									Vol,
									Strike,
...
static double Second_Order_Integral_In_Time(double today,
						double Fwd,
						double Vol,
						double Strike,
						double Maturity,
						...
						double *x_leg)
{
	...
	for(i=1;i<=NQuad_Leg;++i)
	{
		res += w_leg[i] * Second_Order_Expectation_In_Fwd(today, x_leg[i],
							Fwd,Vol,Strike,Maturity,gamma,alpha,rho,
							VolInfinity,Lambda,
							log_or_norm,
							local_vol,
						...
						int LOG_NORM)
{
	...
	pertvalue = First_Order_Integral_In_Time(today,
						Fwd,
						Vol * locvol,
						Strike,
						Maturity,
...
//	log_or_norm = 1;
	...
	firstpertvalue = First_Order_Integral_In_Time(today,
						Fwd,
						Vol * locvol,
						Strike,
						Maturity,
	...
	secondpertvalue = Second_Order_Integral_In_Time(today,
						Fwd,
						Vol * locvol,
						Strike,
						Maturity,
...
//			Forward[1]+=sigma*pow(Forward[1],beta)*g[0]*sqrt_delta_t;
			...
			/* Mihlstein discretization */		
				...
				Forward[0] += (1-exp(-beta*Forward[0]))*(Eta*g[0]*sqrt_delta_t)+0.5*Eta*Eta*beta*exp(-beta*Forward[0])*(g[0]*g[0]-1)*delta_t;
				Forward[1] += (1-exp(-beta*Forward[1]))*(Eta*g[0]*sqrt_delta_t)+0.5*Eta*Eta*beta*exp(-beta*Forward[0])*(g[0]*g[0]-1)*delta_t;
...
{	
	...
	for (j=0; j<nb_strike; j++)
	{
		...
		err=srt_f_optimpvol(res[j][0],forward,strike[j],maturity,1,SRT_CALL,SRT_LOGNORMAL,&(impliedvol));
		if (err)
		{
			res[j][0] = 0;
	...
	if (matrix) free_dmatrix (matrix, 0, npaths-1, 0, nsteps-1);
...
*/ 
	...
	for (cur_path = 1; cur_path<= num_paths;cur_path++)
	{
			...
			if (err=monte_carlo_sabrbvm(cube[cur_path],forw,vovol,beta,rho,sigma,num_paths,maturity,num_steps,cur_path+1, Forward, modeltype, sampletype,&pathnogood))
			{
				free_dmatrix(price,1,2,1,num_strikes);
				free_f3tensor(cube,1,(long) num_paths,1,2,1,(long) num_steps);
				free_dvector(Forward,1,2);
					...
					)
{
	...
	for (cur_path = 1; cur_path<= num_paths;cur_path++)
	{
			...
			if (err=monte_carlo_sabrbvm3(forw,vovol,beta,rho,sigma,lambda,num_paths,maturity,num_steps,cur_path+1, Forward, modeltype, sampletype,&pathnogood))
			{
				free_dmatrix(price,1,2,1,num_strikes);
				free_dvector(Forward,1,2);
				free_dvector(blkbeta,1,num_strikes);
				 ...
				 double betavol, double vovol, double rho, double eta)
{
	...
	zeta = (vovol / betavol) * (eta * (forward - strike) + log(1-exp(-eta*forward)) - log(1-exp(-eta*strike)) );
	...
	else
	{
		logvol = betavol * zeta_xhat
			* log(forward/strike)
			/ (eta*(forward - strike) + (log(1-exp(-forward*eta)) - log(1-exp(-strike*eta))) );
	}

	logvol = logvol * (1 + ( (2*gamma2 - gamma1*gamma1 + 1/(favg*favg))/24.0 * betavol * betavol * volavg * volavg
				 ...
				 double betavol, double vovol, double rho, double beta)
{
	...
	{
		normvol = betavol * zeta_xhat * (1 - beta)
				* (forward - strike) / (pow(forward, 1 - beta) - pow(strike, 1 - beta));
	}

	normvol = normvol * (1 + ( (2*gamma2 - gamma1*gamma1)/24.0 * betavol * betavol * volavg * volavg
							  + 0.25 * rho * vovol * betavol * gamma1 * volavg
							  + (2 - 3 * rho * rho) * vovol * vovol / 24.0
						   ) * maturity
					  );
...
//	for(j=0;j<NStrikes;++j)
//	{
//		err = srt_f_optimpvol(ImpliedVol[j], *Fwd, Strikes[j], Maturity, 1, SRT_CALL, SRT_LOGNORMAL, &(impliedvol));
//		ImpliedVol[j] = impliedvol;
//	}
...
double g_function(double x, double xStar, double gStar, double lambda, int nderiv)
{
	...
	switch(nderiv)
	{
		...
		case 3:
			res = - exp(x-xStar) * ( exp(lambda * gStar) - 1 ) 
				* ( 1 + exp(x-xStar) - exp(x - xStar + lambda * gStar) )
				/ lambda
				/ (exp(x-xStar) - 1 - exp(x - xStar + lambda * gStar) )
				/ (exp(x-xStar) - 1 - exp(x - xStar + lambda * gStar) )
				/ (exp(x-xStar) - 1 - exp(x - xStar + lambda * gStar) );
		...
		case 5:
			res = -exp(x-xStar) * ( exp(lambda * gStar) - 1 ) 
				* ( 1 
					+ 11 * exp(x-xStar) 
					- 22 * exp(2*(x - xStar) + lambda * gStar)
					- exp(3*(x - xStar + lambda * gStar))
					+ 11 * exp(2*(x - xStar))
					- 3 * exp(3*(x - xStar) + lambda * gStar)
					+ 3 * exp(3*(x - xStar) + 2 * lambda * gStar)
					- 11 * exp(x - xStar + lambda * gStar)
					+ exp(3*(x - xStar))
					  ...
					  double *Price)
{
	...
	err = GaussianIntegral ((H-Xinit)/vol,
					  1000, 
					  0,
					  1,
					  NQuadrature,

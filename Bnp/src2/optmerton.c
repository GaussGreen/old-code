...
#define prec 0.0001    /* Related to the approximation made when considering a finite number 
							of jumps*/
...
for (j=2;j<p+1;j++)
{
    ...
    {
	s=0;
	for (k=1;k<i+1;k++) s+=u[k][j-1];
	u[i][j] =s;
     }
   
}
for (i=1;i<max+2;i++) nbrespuplets[i] = u[i][p];
...
for (i=2;i<=p;i++) if (Poisson[i][1]>intensmax) intensmax = Poisson[i][1];
...
for (i=0;i<=maxloc;i++)
{
    ...
    {
	*n += 1;
	for (j=1;j<=p;j++) enspuplets[*n][j] = pupletloc[j];
    }
    else FLoop(enspuplets, pupletloc,levelloc+1, maxloc-i, p, n);
}
...
Err srt_f_tooljumps(double **Poissons,/*  For i=1..p, Poisson[i]
	 has two parameters: intensity and amplitude*/
		       int p,
...
*nbretotalpuplets = 0;
for(n=1;n<(NSautsMax+2);n++) *nbretotalpuplets += (int) nbrespuplets[n];
...
TermCompens=1;
for (i=1;i<=2*n_periods;i++) TermCompens=TermCompens*exp(-Poissons[i][2]*Poissons[i][1]);
...
for (i=1;i<=nevents;i++)
{
	...
	premium += sauts.probas[i]*srt_f_optblksch(dFwdJump,Strike,sigma_eq,maturity,
		1,SRT_CALL,PREMIUM);
}
...
Err optmertonsmiletimedependent (double dFwd,
					int n_strikes,
					double *strikes,
					int n_periods, /* The 6 following variables are vectors of length nperiods */
					double *param,
...
for (i=1;i<=n_strikes;i++)
{
	for (j=1;j<=nevents;j++)
	{
		...
		premium[i] += sauts.probas[j]*srt_f_optblksch(dFwdJump,strikes[i],sigma_eq,maturity,
		1,SRT_CALL,PREMIUM);
	}
}
...
for (i=1;i<=n_strikes;i++)
{
		...
		srt_f_optimpvol(premium[i],dFwd,strikes[i],maturity,1.0, 
			SRT_CALL,SRT_LOGNORMAL,&(impvols[i]));
}
...
for (n1 = 0;n1<N1max;n1++)
{
	for (n2 = 0;n2<N2max;n2++)
	{
		...
		if(proba1*proba2>probainf)
		{
			...
			premium1 +=proba1*proba2*sigma*T*exp(-d*d/(2*sigma*sigma*T))/sqrt(2*SRT_PI*T);
...
for (n1 = 0;n1<N1max;n1++)
{
	for (n2 = 0;n2<N2max;n2++)
	{
		...
		if((proba1p*proba2p>probainf)||(proba1*proba2>probainf))
		{
			...
			d1 = (log(dFwd/Strike)-U1*lambda1*T-U2*lambda2*T+n1*log(1+U1)+
...
double shift;
	...
	case DELTA:
			...
			*answer = (optmertonpremium(dFwd+shift,Strike,sigma,U1,lambda1,U2,
							lambda2, maturity,Logornorm)-optmertonpremium(dFwd,Strike,sigma,U1,lambda1,U2,
							lambda2, maturity, Logornorm))/shift;
			break;
	case GAMMA:
			shift = dFwd/10000;
	...
	case  VEGA:
			...
			*answer = (optmertonpremium(dFwd,Strike,sigma+shift,U1,lambda1,U2,
							lambda2, maturity, Logornorm)-
							optmertonpremium(dFwd,Strike,sigma,U1,lambda1,U2,
							lambda2, maturity, Logornorm))/shift;
	...
	case  THETA:
			...
			*answer = (optmertonpremium(dFwd,Strike,sigma,U1,lambda1,U2,
							lambda2, maturity-shift, Logornorm)-
							optmertonpremium(dFwd,Strike,sigma,U1,lambda1,U2,
							lambda2, maturity, Logornorm))/shift;
...
for (i=1;i<=n_strikes;i++)
{
	price = optmertonpremium(dFwd,strikes[i],parameters[1],parameters[2],parameters[3],parameters[4],
							parameters[5], maturity, Logornorm);
	
	if (strcmp(Logornorm,"Normal")==0)
	{
		srt_f_optimpvol( price,dFwd,strikes[i],maturity,1.0, 
			SRT_CALL,SRT_NORMAL  ,&(impvols[i]));
	}
	else
	{
		srt_f_optimpvol(price,dFwd,strikes[i],maturity,1.0, 

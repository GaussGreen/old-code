...
double					*ans)
{
	...
	for (i=0; i<nKo; i++)
	{
		for (j=i; j<nKo; j++)
		{			
			err = Fx3DCovar (undName, koMat[i], koMat[i], koMat[j], koMat[j], &(covar[i][j]));
			if (err)
			{
				goto FREE_RETURN;
			}
...
FREE_RETURN:
	...
	if (koMat) free (koMat);
	if (fwdFx) free (fwdFx);
	if (logBar) free (logBar);
	if (covar) free_dmatrix (covar, 0, nKo-1, 0, nKo-1);
...
double					*ans)
{
	...
	for (i=0; i<nKo; i++)
	{
		...
		err = Fx3DFwdQf (undName, koMat[i], koMat[i], fixMat, fixMat, payMat, 0, &(fwdFx[i]));
		if (err)
		{
			goto FREE_RETURN;
		}
	...
	/*	Main calculation */
	*ans = df * paidFwdFx * num_f_multinorm (	nKo,
												fwdFx,
												covar,
												logBar,
												ab,
...
FREE_RETURN:
	...
	if (newKoDates) free (newKoDates);
	if (newKoLvl) free (newKoLvl);
	if (newAb) free (newAb);

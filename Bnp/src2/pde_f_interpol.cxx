...
Err pde_interpol(SrtPdeObject *pde, double M[2][2], double M1[2][2]) {
  ...
  SrtTwoFacPdeStepInfo *cur_pde_info,
      *next_pde_info; /* PdeInfo at current and next step */
  ...
  for (i = 0; i <= mxx + 1; i++) {
    for (j = 0; j <= myy + 1; j++) {
      ...
      /* --------------- */
      /* BASIC DICHOTOMIE TO FIND INDEX OF THE INTERVAL CONTAINING xp        ,
       * yp */
      imin = 0;
      imax = mxx + 1;
      while ((imax - imin) > 1) {
      ...
      if (xp >= grid->vec_x[mxx + 1])
        ip = mxx;
      if (xp <= grid->vec_x[0])
        ip = 0;
      if (yp >= grid->vec_y[mxx + 1])
        jp = myy;
      if (yp <= grid->vec_y[0])
        jp = 0;
      ...
      ux1 = thetax * grid->new_value[0][ip + 1][jp + 1][0][0] +
            (1.0 - thetax) * grid->new_value[0][ip][jp + 1][0][0];
      ux = thetax * grid->new_value[0][ip + 1][jp][0][0] +
           (1.0 - thetax) * grid->new_value[0][ip + 1][jp][0][0];

...
Err allocate_link_und(LINK_UND link) {
  ...
  if (link) {
    ...
    link->covariance =
        f3tensor(0, nb_dates - 1, 0, num_und - 1, 0, num_und - 1);

    if (!link->yc || !link->lambda || !link->spot_fx || !link->phi ||
        !link->fwd || !link->std || !link->beta || !link->dom_bond_pay ||
        !link->dom_beta_pay || !link->covariance
...
Err free_link_und(LINK_UND link) {
  int nb_dates, num_und;
  Err err = NULL;
  ...
  if (link) {
    ...
    if (link->correlations)
      free_f3tensor(link->correlations, 0, num_und - 1, 0, num_und - 1, 0,
                    link->nb_sig_dates - 1);

    if (link->covariance)
      free_f3tensor(link->covariance, 0, nb_dates - 1, 0, num_und - 1, 0,
                    num_und - 1);
      ...
      if (total->do_und[und]) {
        for (l = 0; l < total->num_df[und]; l++) {
          FIRSTSetDFValue(
              local, und, l,
              total->dff[und][l] *
                  exp(-total->gam[und][l] * sv[link->dom_forex[und]] -
                      total->gam2[und][l]));
        }
      }
    } else {
      if (total->do_und[und]) {
        for (l = 0; l < total->num_df[und]; l++) {
          FIRSTSetDFValue(
              local, und, l,
              total->dff[und][l] *
                  exp(-total->gam[und][l] * sv[und] - total->gam2[und][l]));
        }
      }
    }
  }

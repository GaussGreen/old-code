...
static double optvalbeta(double fwd, double strike, double vol, double mat,
                         double beta, double disc_fact, SrtCallPutType call_put,
                         SrtGreekType greek) {

  return srt_f_optblkschbeta(fwd, strike, vol, mat, beta, disc_fact, call_put,
                             greek);
}
...
double srt_f_optimpvolbeta(double premium, double fwd_price, double strike,
  ...
  */
  ...
  prem_new = optval(fwd_price, strike, NULL_VOL, mat, disc, call_put, PREMIUM);
...

  {
    ...
    for (i = 0; i <= MAX_ITER; i++) {
      ...
      prem_new = optvalbeta(fwd_price, strike, vol_guess, mat, beta, disc,
                            call_put, PREMIUM);
      ...
      prem_shift = optvalbeta(fwd_price, strike, vol_guess + vol_shift, mat,
                              beta, disc, call_put, PREMIUM);

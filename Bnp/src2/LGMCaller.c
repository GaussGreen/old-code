...
/****************************************************************************/
/**** Comments ****/
/* NewLGMautocalCaller is a temporizing step, which does not use the flexibility
of the new program. It should be replaced with programs which call the new
LGMautocal directly from MAD and Westminster, which can expose the new
functionality, ASAP */
...
and the value of the floating leg should be in Strike[j]. Accruals for inter-period
starts can be handled by adjusting Strike[j] and/or RedFirstPay[j], depending on the
deal, and adjustments for the basis of the floating leg can be handled by adjusting
Strike[j] and Payment[i] */ 

/* Output
The outputs are all of the form *variable = answer for numbers and **variable = *Array
for arrays. The calling program must allocate space which is being pointed to by
*variable. Specifically,
*LGMPtr, *nExerBdry, *NsigPtr, and *NtauPtr must point to space that has been allocated
for double, long, long, and long which can receive the outputs of the program;

*rArrExBdryPtr, *tArrExBdryPtr, *sigDateArrPtr, *sigValArrPtr, *tauDateArrPtr, and *tauValArrPtr
must point to space the has been allocated for pointers to doubles and Dates; the
program will free any space which is being pointed to and reallocated it. Then
*variable will be set to the address of first element in the array of answers.

All output arrays which are not NULL are freed with srt_free and then reallocated
with srt_calloc. This will result in TERRIBLE errors if they have not been
allocated with calloc, malloc, srt_calloc, or srt_malloc. Specifically, if the arrays
*rArrExBdryPtr, *tArrExBdryPtr, *sigDateArrPtr, *sigValArrPtr, *tauDateArrPtr, and *tauValArrPtr are
not NULL, they need to have been allocated with calloc or malloc. 

If findExerBdry is on, the array *rArrExBdryPtr[i] gives the swap rate at the exercise
boundary for a swap that has exercise date *tArrExBdryPtr[i] and ends
...
If convertTS is on, the program finds the sigma-tau term structure equivalent to the
calibrated zeta-G term structure, and outputs it in the arrays *sigDateArrPtr and 
*sigValArrPtr (of length *NsigPtr) and *tauDateArrPtr and *tauValArrPtr (of length *NtauPtr).	*/
...
ignores this argument if it is zero or negative) as well as the calibration method
arguments at the end. */

LGMErr TestLGMAmerCaller(
/* definition of American deal */
		long nfix,				/* number of fixed periods */
		Date  tfixStart[],		/* [0,1,...,nfix-1]  start dates for fixed coupons */
		Date  tfixEnd[],		/* [0,1,...,nfix-1]  end dates for fixed coupons */
		Date  tfixPay[],		/* [0,1,...,nfix-1]  pay dates for fixed coupons */
		double fixFullPayment[],/* [0,1,...,nfix-1]  fixed coupons (with notional at end) */
		double Strike[],		/* [0,1,...,nfix-1]  premium for exer. at j (with notional) */
		char  *fixBasis,		/* basis for fixed coupons */
		int  fixEarlyFlag,		/* 0=subtract accrual from frst pymnt; 1=add accrual to fee */
		long  nfltdates,		/* number of floating dates */
		Date  tflt[],			/* [0,1,...,nfltdates-1] all flting dates (first start date & all pay dates) */
		double rflt_current,	/* current floating rate (if fixed); ignored if non-positive */
		char *fltBasis,			/* basis for floating coupons */
		int  fltEarlyFlag,		/* 0 = subtrct accrual from frst pymnt; 1 = add accrual to prem */
		...
		BusDayConv convStart,	/* business day convention for start (typically none or suceeding) */
...
/* info about today's marketplace */
    	...
    	LGMErr (*GetVol)(long, long, double, SRT_Boolean, double *), /* volatility function for reference swptns */
		...
		double *intrinsicValPtr)		/* ptr to intrinsic value (NULL => not req'd) */
{
	return TestLGMAmerCallerRR(
		...
		fixFullPayment,/* [0,1,...,nfix-1]  fixed coupons (with notional at end) */
		...
		tflt,			/* [0,1,...,nfltdates-1] all flting dates (first start date & all pay dates) */
		rflt_current,	/* current floating rate (if fixed); ignored if non-positive */
		fltBasis,			/* basis for floating coupons */
		fltEarlyFlag,		/* 0 = subtrct accrual from frst pymnt; 1 = add accrual to prem */
		...
		convStart,	/* business day convention for start (typically none or suceeding) */
...
/* interpret pay-receive and the day count bases for the fixed and floating legs */
...
/* create simple American stucture, check input data, & copy input data into structure */
	error = LGMFillSimAmer(tfirst, standardLag, &AmerPtr,
						   nfix, tfixStart, tfixEnd,						/* fixed leg info */
						   tfixPay, fixFullPayment,
						   fixBasisCode, fixEarlyFlag,
						   Strike,											/* float leg info */
						   nfltdates, tflt,
						   fltBasisCode, fltEarlyFlag,
						   tFirstExer, lagExerStart, cal_or_bus, convStart,	/* exercise info */
						   srt_p_r_flag);
	if (error!=NULL)
...
/* Set calibration method */	
	CalReqPtr = LGMSetCalibMeth(LGMOneTwoFactor,
								usefixtau,
								usecaps,
								tau,
								alpha,
...
/* Call LGMautocal */	
	error = LGMNewAmerican(
				...
				&ExerIntoPtr, &ExerBdryPtr,			/* swaptions representing the underlying & exer boundary */
...
/* free unneeded structures */		/* these are not needed to get output */
	...
	if(LGMOneTwoFactor==1)	LGMFreeLGM_TS(&LGMtsPtr);
	if(LGMOneTwoFactor==2)	LGMFreeLGM2F_TS(&LGMtsPtr);
...
/* If exercise boundary has been found, copy it into tExBdry & rExBdry */
	if (skipEval!=1 && findExerBdry!=0 && ExerBdryPtr!=NULL && ExerBdryPtr->n>0 && lgmExerBdryData!=NULL)
...
/************************************************************************************/
/************************************************************************************/
/* Program to calibrate the term structure and compute value of a MidAtlantic deal.
Program has the identical call as the old LGMautocal. It uses its inputs to
create the call to the LGMnewautocal program.  
This file now calls a new routine with an additional argument */
		...
		char* szRefRate )
{
	...
	/*	1.- Initialise generic data	
		...
		if (LGMOneTwoFactor==2)
		{
			if(usefixtau == 0)
			{
				if ((calibrationmeth == 1) && (HybridShortInstrsIndex != NULL) ) CalReqPtr->calmeth = HybridShortAndDiag;
				else if((calibrationmeth == 2) || (HybridShortInstrsIndex == NULL) ) CalReqPtr->calmeth = FullCapAndDiag;
				else
				{
					return serror("Calibration Method should be 1: HybridShortAndDiag or 2: FullCapAndDiag ");
					/* CalReqPtr->calmeth = TenorAndDiag; */
				}
			}
		...
		if (strikechoice == 1) CalReqPtr->Rmeth = IRR;
		else if (strikechoice == 2) CalReqPtr->Rmeth = dIRR;
		else if ((strikechoice == 3)) CalReqPtr->Rmeth = EMK1;
		else if ((strikechoice == 4)) CalReqPtr->Rmeth = EMK2;
	...
	/*	2.- Initialise specific Midat data	
		...
		error = LGMFillSimMidAt(tfirst, &MidAtPtr, nPay, Payment, tPay, 
								nEx, tEx, tStart, Strike, RedFirstPay, srt_p_r_flag);
	...
	/*	4.- Free and return	
		...
		else if(LGMOneTwoFactor==2)
		{
			...
			else	if(LGMtsPtr) LGMFreeLGM2F_TS(&LGMtsPtr);
		}
		...
		/* Unpack output */
		*LGMvalPtr = LGMVal;
		if (intrinsicValPtr != NULL)	*intrinsicValPtr = intrinsicVal;
...
LGMErr LGMCallInvFloaterCaller(
	...
	double *exerFee,		/* [0,1,...,nEx-1] fee paid by option holder to exercise 
								most frequently, the bond is called at par, i.e. exerFee = 1.0 */
	char *PayRecStr,		/* RECEIVER (call on the bond) or PAYER (put on the bond) */
  ...
  - the exotic side also includes a cap paying max (0, Libor CASH - cap_str) * libor_cvg */
	...
	int capletvolmethod,	/* 1 = Model, 2 = Sliding, 3 = Converging, 4 = Calibrated */
	int strikechoice,		/* 1 = Not used, 2 = MidatStrike/MidatStrike, 
							   3 = MidatStrike/std, 4= MidatStrike/ATM */
	double maxstd,			/* Maximum number of std between forward and strike */
	...
	FWD_VOL_STR *fwdVolStr)	/* ptr to fwd vol data (NULL => not req'd) */
{
	...
	/*	2.- Initialise specific callable inverse floater data	
		...
		/* Allocate and fill deal pointer, also check that there is a least one exerceise date left */
		error = LGMFillCallInvFloater (	&dealPtr,
										&nExLeft, 
										tfirst, 
										ycName, 
										nCpn, 
		...
		if (capletvolmethod == 1) CalReqPtr->CapletVolMeth = MODEL;
		else if (capletvolmethod == 2) CalReqPtr->CapletVolMeth = SLIDING2;	/* Just change the name SLIDING to SLIDING2 to avoid conflict of definitions */
		else if (capletvolmethod == 3) CalReqPtr->CapletVolMeth = CONVERGING;
		else if (capletvolmethod == 4) CalReqPtr->CapletVolMeth = MARKET;
	...
	/*	4.- Free and return	
		...
		*LGMValPtr = LGMVal;
		...
		/* If sigma-kappa term structure has been found, copy it into output arrays */
		if (convertTS!=0 && SigKaptsPtr!=NULL
			&& SigKaptsPtr->numS>0 && SigKaptsPtr->numK>0
			&& SigKaptsPtr->sdate!=NULL && SigKaptsPtr->sig!=NULL
			&& SigKaptsPtr->kdate!=NULL && SigKaptsPtr->kap!=NULL
			&& lgmTSData!=NULL)
...
Funding leg:
	cvg(tau[k-1], tau[k]) * rate2[k]	for k = 1,2, ..., m
		where rate2[k] is the true (ie, cash) floating rate for the period tau[k-1] to tau[k]


Exercise (receiver):
	At any notification date tEx[j], j=0,1, ..., nEx-1, the holder can exercise. If exercised at tEx[j]
then the settlement date is t[i0-1], the next coupon date on or after tEx[j]. The option owner
	pays any exercise fee f[j] at t[i0-1]
	receives all remaining coupons C[i], i = i0, i0+1, ..., n
	receives notional ($1) at end date t[n] (this simplifies the funding leg)
	pays the funding leg starting at t[i0-1].
If the start-upon-exercise date t[i0-1] is a funding date tau[k-1], then the option owner
	receives the funding payments at tau[k], tau[k+1], ..., tau[m]
	receives the notional ($1) at the end date
The funding leg thus revalues to $1

If the start-upon-exercise date t[i0-1] is not a funding date, so it occurs somewhere in the middle
of a funding period, then there are small adjustments that depend on whether the Early Flag and the
ResetRate flags are on.

If the Early Flag is on (this represents an option that is cancelling an existing CapFloater,
the option owner
	receives the accrued (funding leg) interest cvg(tau[k-1],t[i0-1]) * rate2[k] at t[i0-1],
	pays the entire first funding payment cvg(tau[k-1],tau[k]) * rate2[k] at tau[k]
	continues to make the funding payments at tau[k+1], tau[k+2], ...tau[m]

If the start of this funding period occurs before spot-of-today (which happens in re-valuing deals)
then the floating rate for this period has already been set; in this case the code uses the input
rfund_cur (if it is positive); if rfund_cur<=0 then the code will guess the correct rate from the
discount curve.

If the Early Flag is off, then the option owner
	pays remaining part of the first funding payment cvg(t[i0-1],tau[k]) * rate2[k] at tau[k]
	continues to make the funding payments at tau[k+1], tau[k+2], ...tau[m]
If the Reset Rate flag is on, the rate is assumed to have been reset for the first (stub) period.
Otherwise the code will use rfund_cur (or guess the rate) as before.
	
Exercise (payer):
	At any notification date tEx[j], j=0,1, ..., nEx-1, the holder can exercise. If exercised,
the option owner
	pays an exercise fee f[j]
	pays the coupon leg (reverse of above)
	receives the funding leg (reverse of above)

Notes:
	All margins and "basis spreads" must be moved from the funding leg to the coupon leg
prior to calling the code.
	The internal model is on a CASH basis, with the FRA rates
		rate for t1 to t2 = [df(t1) - df(t2)]/cvg*df(t2)
being used in place of the floating rates. Before the code is called, all basis spreads
must be accounted for by
(i) adjusting the funding leg margin (before moving the margin to the coupon leg)
(ii) adjusting the effective floor value a[i]
	The start dates t[0] and tau[0] should be the same
	The end dates t[n] and tau[m] should be the same
	There is no provision in the code for handling overlapping start/end/pay dates on either leg
	Exercise cannot result in starting at any date other than one of the coupon dates

Sign of the exercise fee: f[j]>0 means the exerciser pays the fee, regardless of whether its a payer
or receiver. THIS IS UNLIKE LGMautocal, where the floating leg always paid the fee

Code automatically disregards any exercise dates that occur before today */
/* tCpn[i-1], tCpn[i], and tCpn[i] are the the start, end, and pay date for period i, i=1,...,nCpn
	amin[0], amax[0] margin[0] are undefined: amin[*,1,2,...,nCpn], amax[*,1,2,...,nCpn], margin[*,1,2,...,nCpn]
	tflt[j-1], tflt[j], tflt[j] are the start, end, and pay date for floating period j, j=1,...,nflt */

LGMErr LGMCallCapFloaterCaller(
/* definition of coupon leg */
	long nCpn,				/* number of coupon periods */
	Date  *tCpn,			/* t[0,1,...,nCpn]...cpn for t[i-1] to t[i] paid at t[i], i=1,...,nCpn	*/
	double *amax,			/* cpn[i] is															*/
	double *amin,			/*	cvg(t[i-1],t[i]) * (margin[i] +										*/
	double *margin,			/*		lvg*max{rate[i]-amin[i],0} - lvg*max{rate[i]-amax[i],0})		*/
...
/* definition of floating leg */
	...
	Date *tflt,				/* tau[0,1,...,nflt]...float periofs are tau[j-1] to tau[j], j=1,...,nflt */
...
/* exercise information */
	...
	double *exerFee,		/* [0,1,...,nEx-1] fee paid by option holder to exercise */
	char *PayRecStr,		/* RECEIVER or PAYER */
	int earlyFlag,			/* if 1, deal is a cancelation of an existing inverse floater */
	int resetFlt,			/* float for stub period is reset upon exercise */
/* information about today and today's market place*/
	String  ycName,			/* yield curve name */
	double rfund_cur,		/* current (true) funding rate (if fixed); ignored if non-positive */
	int endofday,			/* 1=too late to notify today, 0=not too late */
    LGMErr (*GetVol)(long, long, double, SRT_Boolean, double *), /* volatility function for reference swptns */
	char *char_vol_type,	/* normal or log normal swaption vols */
/* calibration method to use */
...
/* outputs */
	String outfile,			/* output log file (unused) */
	double *LGMValPtr,		/* answer...value of the option */
	double *intrinValPtr,	/* value if exercise date tNot[j] had to be chosen today */
	SrtLgmExerBdryData *lgmExerBdryData, /* ptr to exercise boundary data structure (NULL => not req'd) */
	SrtLgmRefSwptnData *lgmRefSwptnData, /* ptr to reference swaption data structure (NULL => not req'd) */
/* calibrated term structure */
	SrtLgmTSData *lgmTSData)/* ptr to tau/sigma data (NULL => not req'd) */
...
/* the next routine checks out the deal, allocates the Bermudan on a inverse floater
stucture, and copies the input data into it */
	error = LGMFillCallCapFloater(&nExLeft, tfirst, ycName, rfund_cur, &dealPtr,
								nCpn, tCpn, amax, amin, margin, lvg, cpnBasis, aBasis,
								nflt, tflt, fltBasis,
								nEx, tEx, exerFee, srt_p_r_flag,
								earlyFlag, resetFlt);
...
/* This routine creates a simple American deal and copies the input data into the
simple American structure */
LGMErr LGMFillSimAmer(Date tfirst, int standardLag,
					  SrtSimAmer** AmerPtrPtr,
					  long nfix, Date *tfixStart, Date *tfixEnd,
					  Date *tfixPay, double *fixFullPayment,
					  SrtBasisCode fixBasisCode, int fixEarlyFlag,
					  double *Strike,
					  long nfltdates, Date *tflt,
					  SrtBasisCode fltBasisCode, int fltEarlyFlag,
					  Date tFirstExer, int lagExerStart,
					  int calbus, BusDayConv convStart,
...
/* check out deal */
	...
	LastFixing = add_unit(tflt[nflt-1], -standardLag, SRT_BDAY, NO_BUSDAY_CONVENTION);
	...
	if (tfirstStart >= tfixEnd[nfix-1] ||
		tfirstStart >= tflt[nfltdates-1])
		return("no underlying");

	for (i=0; i<nfix-1
			&& tfixStart[i] < tfixEnd[i]
			&& tfixStart[i] < tfixStart[i+1]
			&& tfixEnd[i] < tfixEnd[i+1]
			&& tfixPay[i] <= tfixPay[i+1]
			&& tfixStart[i] < tfixPay[i]; i++)
        ;
...
/* Fill in structure */
  /* fill in fixed leg */
	...
	{	AmerPtr->tfixStart[i] = tfixStart[i];
		...
		AmerPtr->ExtraPrem[i] = Strike[i] - 1;	/* remove notional from floating leg */
	}
	AmerPtr->fixCoupon[nfix-1] = AmerPtr->fixCoupon[nfix-1] - 1;	/* remove notional from fixed leg*/

  /* fill in floating leg */
	AmerPtr->nflt = nflt;
	AmerPtr->fltBasis = fltBasisCode;
	AmerPtr->EarlyFlagFlt = fltEarlyFlag;
	AmerPtr->ResetFlt = 0;						/* do not reset floating rate for mid period exercise */
	
	for (i=0; i<nflt; i++)
	{	AmerPtr->tfltFixing[i] = add_unit(tflt[i], -standardLag, SRT_BDAY, NO_BUSDAY_CONVENTION);
...
/*********************************************************************/
/* This routine creates a simple midatlantic deal and copies the input data into the
simple MidAtlantic structure */
/* This routine ASSUMES that if the option holder exercises on exer date tEx[j], then
he will receive fixed leg payments on all pay dates tPay[i] which are strictly after
the settlement date Start[j]. Except for the first pay date after settlement, the amount
of the payment is Payment[i] for all i. The first payment is reduced by RedFirstPay[j]. */

LGMErr LGMFillSimMidAt(Date tfirst, SrtSimMidAt **MidAtPtrPtr,
					   long nPay, double *Payment, Date *tPay, 
					   long nEx, Date *tEx,
...
/* for each exercise, find first payment received if option is exercised */
    for (j=0; j<nEx; j++)
    {	i=0;
		while (i<nPay && tPay[i] <= tStart[j]) i++;
...
/* find first exercise date on or after tfirst */
	...
	{	for (j=0; j<nEx && tEx[j]<tfirst; j++)	/* first exer date after today+endofday */
			;
...
LGMErr LGMFillCallInvFloater(
		SrtCallInvFlt** dealPtrPtr,								/* output: the deal */
		...
		double *a, double *gear, double *cap_str, double *cvg, double *lcvg, /* more coupon leg */
...
/* Step 1: Check out deal */
	...
	*nExleftPtr = nExEff;
	...
	/* Step 3: Allocate Bermudan inverse floater with nExEff exer dates and n-i cpn periods */
...
/***************************************************************************************/    
/* Routines that help make the conversion from old LGM call to new LGM call */
/* This routine verifies the input data, creates a Bermudan cap floater structure and
copies the input data into the simple cap structure */
LGMErr LGMFillCallCapFloater(
		...
		double lvg, SrtBasisCode cpnBasis, SrtBasisCode	aBasis,		/* more coupon leg */
...
/* Step 1: Ensure the deal makes sense */
	if (cpnBasis<0 || cpnBasis>=LASTBASISCODE ||
		  aBasis<0 ||   aBasis>=LASTBASISCODE ||
		fltBasis<0 || fltBasis>=LASTBASISCODE)
...
/* Coupon leg dates in increasing order and coupons between -100% and 100% */
	for (i=1; i<=n && t[i-1]<t[i]
/*					&& amax[i]>0.0 && amax[i]<1.0
					&& amin[i]>0.0 && amin[i]<1.0
*/
					&& marg[i]>-1.0 && marg[i]<1.0; i++)
		;
...
/* Exercise dates in increasing order and fees between -100% and 100% */
	for (j=1; j<nEx && tEx[j-1]<tEx[j]
					&& exFee[j]>-1.0 && exFee[j]<1.0; j++)
		;
/* Check there was no error before and gearing between 0 and 100 */
	if (i<=n || k<=m || j<nEx || exFee[0]<=-1.0 ||
		exFee[0]>=1.0 || lvg<=0.0 || lvg>=100.0)
...
/* Step 3: Create Bermudan inverse floater with nExEff exer dates and n-i cpn periods */
...
/* get rflt for period */
		...
		if (df1==SRT_DF_ERROR || df2==SRT_DF_ERROR || dfSet==SRT_DF_ERROR || cvg<=0.0)
		{	LGMFreeCallCapFlt(&ptr);
...
/* compute strike - equivalent to the PV of the funding leg at the settlement date */
		if (earlyFlag!=0)
...
a swaption or caplet volatility in pricing. The vol used is the vol for a vanilla
option where the underlying has start date swapstart, end date swapend,
...
directly related to pricing, meaning these vols will have zero vega. Using sticky
vols to record the vols will thus be more inefficient than using these vols.
*/
...
LGMCalParmPtr LGMSetCalibMeth(int   iLGMOneTwoFactor,
							  int    FixedTauFlag,
							  int    CapsFlag,
							  double Tau,
							  double Alpha,
...
LGM_TSPtr *atcTSData)					/* ptr to zeta/G data (NULL => not req'd) */
{
	...
	{
		if ((*atcTSData)->zdate) srt_free((*atcTSData)->zdate);
		if ((*atcTSData)->zeta) srt_free((*atcTSData)->zeta);
		if ((*atcTSData)->Gdate) srt_free((*atcTSData)->Gdate);
		if ((*atcTSData)->G) srt_free((*atcTSData)->G);

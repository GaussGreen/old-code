...
static char* copyExerBdry(LGMSwptns* ExerBdryPtr, SrtLgmExerBdryData* lgmExerBdryData);
static char* copyTSData(SigKapTS* SKtsPtr, SrtLgmTSData* TSData);
...
char* TestLGMAmerCaller(
    ...
    char* (*GetVol)(
        long, long, double, SRT_Boolean, double*), /* volatility function for reference swptns */
    ...
    char*   szRefRate)         /* optional refrate included for MUNI pricing */
{
    ...
    char* (*GetBeta)(Date, Date, double*);
    char* (*RecVol)(Date, Date, double);
    LGMMarkConv conv;
...
char* TestLGMautocalCaller(
    ...
    char* (*GetVol)(Date, Date, double, SRT_Boolean, double*), /* function to get swaption vols */
...
- the exotic side also includes a cap paying max (0, Libor CASH - cap_str) * libor_cvg */
    ...
    char* (*GetVol)(long, long, double, SRT_Boolean, double*),
    ...
    FWD_VOL_STR*  fwdVolStr)  /* ptr to fwd vol data (NULL => not req'd) */
{
    ...
    if (capletvolmethod == 1)
        ...
        CalReqPtr->CapletVolMeth = SLIDING2; /* Just change the name SLIDING to SLIDING2 to avoid
...
Funding leg:
        cvg(tau[k-1], tau[k]) * rate2[k]	for k = 1,2, ..., m
                where rate2[k] is the true (ie, cash) floating rate for the period tau[k-1] to
tau[k]


Exercise (receiver):
        At any notification date tEx[j], j=0,1, ..., nEx-1, the holder can exercise. If exercised at
tEx[j] then the settlement date is t[i0-1], the next coupon date on or after tEx[j]. The option
owner pays any exercise fee f[j] at t[i0-1] receives all remaining coupons C[i], i = i0, i0+1, ...,
n receives notional ($1) at end date t[n] (this simplifies the funding leg) pays the funding leg
starting at t[i0-1]. If the start-upon-exercise date t[i0-1] is a funding date tau[k-1], then the
option owner receives the funding payments at tau[k], tau[k+1], ..., tau[m] receives the notional
($1) at the end date The funding leg thus revalues to $1
...
/* tCpn[i-1], tCpn[i], and tCpn[i] are the the start, end, and pay date for period i, i=1,...,nCpn
        amin[0], amax[0] margin[0] are undefined: amin[*,1,2,...,nCpn], amax[*,1,2,...,nCpn],
   margin[*,1,2,...,nCpn]
        tflt[j-1], tflt[j], tflt[j] are the start, end, and pay date for floating period j,
   j=1,...,nflt */

char* LGMCallCapFloaterCaller(
...
char* LGMFillSimAmer(
    ...
    Date*           tfixEnd,
    Date*           tfixPay,
    double*         fixFullPayment,
    SrtBasisCode    fixBasisCode,
    int             fixEarlyFlag,
    double*         Strike,
    long            nfltdates,
    Date*           tflt,
    SrtBasisCode    fltBasisCode,
    int             fltEarlyFlag,
    Date            tFirstExer,
    int             lagExerStart,
    int             calbus,
    BusDayConv      convStart,
    SrtReceiverType payrec)
{
    ...
    for (i = 0;
         i < nfix - 1 && tfixStart[i] < tfixEnd[i] && tfixStart[i] < tfixStart[i + 1] &&
         tfixEnd[i] < tfixEnd[i + 1] && tfixPay[i] <= tfixPay[i + 1] && tfixStart[i] < tfixPay[i];
         i++)
        ;
    if (tfixStart[nfix - 1] >= tfixEnd[nfix - 1] || tfixStart[nfix - 1] >= tfixPay[nfix - 1])
        i = nfix - 2;
...
char* LGMFillSimMidAt(
    Date            tfirst,
    SrtSimMidAt**   MidAtPtrPtr,
    long            nPay,
    double*         Payment,
    Date*           tPay,
    long            nEx,
...
char* LGMFillCallCapFloater(
    long*           nExleftPtr, /* effective number of exer */
    Date            tfirst,
    char*           ycName,
    double          rfund_cur,  /* info about today */
    SrtCallCapFlt** dealPtrPtr, /* output: the deal */
    long            n,
    Date*           t,
    double*         amax,
    double*         amin,
    double*         marg, /* coupon leg */
    double          lvg,
    SrtBasisCode    cpnBasis,
    SrtBasisCode    aBasis, /* more coupon leg */
    long            m,
    Date*           tflt,
    SrtBasisCode    fltBasis, /* floating leg */
    long            nEx,
    Date*           tEx,
    double*         exFee,  /* exercise info */
    SrtReceiverType payrec, /* PAYER or RECEIVER */
    int             earlyFlag,
    int             resetFlt) /* early and reset flags */
{
    char*          error = NULL;
    LGMMarkConv    conventions; /* standard swaption conventions */
    Date           tNow, tSpot;
    ...
    *dealPtrPtr = NULL;
    ...
    if (cpnBasis < 0 || cpnBasis >= LASTBASISCODE || aBasis < 0 || aBasis >= LASTBASISCODE ||
    ...
    for (j = 1; j < nEx && tEx[j - 1] < tEx[j] && exFee[j] > -1.0 && exFee[j] < 1.0; j++)
        ;
    /* Check there was no error before and gearing between 0 and 100 */
    if (i <= n || k <= m || j < nEx || exFee[0] <= -1.0 || exFee[0] >= 1.0 || lvg <= 0.0 ||
        lvg >= 100.0)
...
char* LGMReturnExp1(Date swapstart, Date swapend, double* beta)
{
    *beta = 1.0;
    return (NULL);
}
...
/*********************************************************************/
char* LGMReturnExp0(Date swapstart, Date swapend, double* beta)
{
    *beta = 0.0;
    return (NULL);
}
...
*/
char* LGMRecVolDummy(Date swapstart, Date swapend, double Rfix)
{
    return (NULL);
}

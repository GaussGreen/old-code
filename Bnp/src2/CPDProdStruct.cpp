...
PD_FUND_LEG	fund_leg)
{
	...
	i = 0;
	while (i < fund_ncpn && fund_fix[i] < today + eod_flag) i++;
...
FREE_RETURN:
	...
	if (err) cpd_free_fund_leg (fund_leg);
	return err;
}

/*	Check dates consistency */
Err cpd_check_fund_leg(
PD_FUND_LEG	fund_leg)
{
	int				i;
...
PD_EXO_LEG	exo_leg)
{
	...
	/*	Skip coupons fixed before today */
	i = 0;
	while (i < pd_ncpn && pd_fix[i] < today + eod_flag) i++;
	...
	while (i < pd_ncpn)
	{
		...
		else
		{
			...
			if (pd_num_strikes[i] > 0)
			{
				...
				for (k=0; k < pd_num_strikes[i]; k++) exo_leg->cpn[j].weights[k] = cvg * pd_weights[i][k];
			}
		}
		i++;
		j++;
...
DO_NOTIONAL_REF:
	...
	// Interp redemption:
	{
		...
		if (pd_not_num_strikes > 0)
		{
			...
			for (k=0; k < pd_not_num_strikes; k++) exo_leg->not_ref.weights[k] = pd_not * pd_not_weights[k];
		}
	}
...
FREE_RETURN:
	...
	if (err) cpd_free_exo_leg (exo_leg);
	return err;
}

/*	Check dates consistency */
Err cpd_check_exo_leg(
PD_EXO_LEG	exo_leg)
{
	int				i;
	...
	if (exo_leg->num_cpn > 0)
	{
		...
		if (exo_leg->not_ref.fx_val_date < exo_leg->cpn[exo_leg->num_cpn-1].fx_val_date)
			return "Notional refund value date should be after last coupon value date in exotic leg";
	}
...
Err cpd_fill_exo_and_fund_leg_TARN(long		today,
								   CPD_STR	cpd)
{
int				j;
...
Err err = NULL;
	...
	for(j=0; j<cpd->pd_leg->num_cpn; j++)
	{
		...
		if(cpd->pd_leg->cpn[j].use_opt_str)
		{
			...
			{
				pd_leg->cpn[j].strikes = calloc(pd_leg->cpn[j].nstrikes, sizeof(double));
				pd_leg->cpn[j].weights = calloc(pd_leg->cpn[j].nstrikes, sizeof(double));
				if (!pd_leg->cpn[j].strikes || !pd_leg->cpn[j].weights)
				{
					err = serror("Memory failure in cpd_fill_exo_leg");
					goto FREE_RETURN;
				}
				memcpy(pd_leg->cpn[j].strikes, cpd->pd_leg->cpn[j].strikes, pd_leg->cpn[j].nstrikes * sizeof(double));
				memcpy(pd_leg->cpn[j].weights, cpd->pd_leg->cpn[j].weights, pd_leg->cpn[j].nstrikes * sizeof(double));
			}
		}
	}
	
	/*	Dates */
	pd_leg->cpn[cpd->pd_leg->num_cpn].start_date = cpd->pd_leg->cpn[cpd->pd_leg->num_cpn - 1].pay_date;
	pd_leg->cpn[cpd->pd_leg->num_cpn].pay_date = cpd->pd_leg->cpn[cpd->pd_leg->num_cpn - 1].pay_date;
	pd_leg->cpn[cpd->pd_leg->num_cpn].fx_fix_date = cpd->pd_leg->cpn[cpd->pd_leg->num_cpn - 1].pay_date;
	pd_leg->cpn[cpd->pd_leg->num_cpn].fx_val_date = add_unit(cpd->pd_leg->cpn[cpd->pd_leg->num_cpn - 1].pay_date, 2, SRT_BDAY, MODIFIED_SUCCEEDING);

	/*	Times */
	pd_leg->cpn[cpd->pd_leg->num_cpn].start_time = cpd->pd_leg->cpn[cpd->pd_leg->num_cpn - 1].pay_time;
	pd_leg->cpn[cpd->pd_leg->num_cpn].pay_time = cpd->pd_leg->cpn[cpd->pd_leg->num_cpn - 1].pay_time;
	pd_leg->cpn[cpd->pd_leg->num_cpn].fx_fix_time = cpd->pd_leg->cpn[cpd->pd_leg->num_cpn - 1].pay_time;
	pd_leg->cpn[cpd->pd_leg->num_cpn].fx_val_time = (pd_leg->cpn[cpd->pd_leg->num_cpn - 1].fx_val_date - today) * YEARS_IN_DAY;
	...
	if (cpd->pd_leg->not_ref.use_opt_str)
	{
		...
		{
			pd_leg->not_ref.strikes = calloc(pd_leg->not_ref.nstrikes, sizeof(double));
			pd_leg->not_ref.weights = calloc(pd_leg->not_ref.nstrikes, sizeof(double));
			if (!pd_leg->not_ref.strikes || !pd_leg->not_ref.weights)
			{
				err = serror("Memory failure in cpd_not_fill_exo_leg");
				goto FREE_RETURN;
			}
			memcpy(pd_leg->not_ref.strikes, cpd->pd_leg->not_ref.strikes, pd_leg->not_ref.nstrikes * sizeof(double));
			memcpy(pd_leg->not_ref.weights, cpd->pd_leg->not_ref.weights, pd_leg->not_ref.nstrikes * sizeof(double));
		}
	}
	...
	/*	Dates */
	fund_leg->cpn[fund_leg->num_cpn].start_date = cpd->fund_leg->cpn[fund_leg->num_cpn - 1].pay_date;
	fund_leg->cpn[fund_leg->num_cpn].pay_date = cpd->fund_leg->cpn[fund_leg->num_cpn - 1].pay_date;
	
	/*	Times */
	fund_leg->cpn[fund_leg->num_cpn].start_time = cpd->fund_leg->cpn[fund_leg->num_cpn - 1].pay_time;
...
FREE_RETURN:
	if(err) cpd_free_exo_leg (pd_leg);
	if(err) cpd_free_fund_leg (fund_leg);
return err;
}
...
CPD_STR		cpd)
{
	...
	i = 0;
	while (i < ncall && ex_date[i] < today + eod_flag) i++;
	...
	while (i < ncall)
	{
		...
		k = 0;
		while (k < cpd->fund_leg->num_cpn && cpd->fund_leg->cpn[k].start_date < cpd->call[j].ex_date) k++;
		...
		/*	k = index of the first coupon to be called on exotic leg,
				i.e. first coupon with a start date >= ex date */
		k = 0;
		while (k < cpd->pd_leg->num_cpn && cpd->pd_leg->cpn[k].start_date < cpd->call[j].ex_date) k++;
		...
		if (use_not_opt_str)
		{
			...
			cpd->call[j].fx_val_date = add_unit(cpd->call[j].fx_fix_date, 2, SRT_BDAY, MODIFIED_SUCCEEDING);
			...
			if (pd_not_num_strikes[i] > 0)
			{
				...
				memcpy(cpd->call[j].strikes, pd_not_strikes[i], pd_not_num_strikes[i] * sizeof(double));
				for (k=0; k < pd_not_num_strikes[i]; k++) cpd->call[j].weights[k] = pd_not[i] * pd_not_weights[i][k];
			}
		}
		i++;
		j++;
	...
	if (use_bound) cpd->type = 1;

	err = cpd_check_calls (cpd);

FREE_RETURN:

	if (err) cpd_free_calls (cpd);
...
double				fx_vol_shift)
{
	...
	if (cpd_dlm_params && cpd_dlm_params->use_beta_dlm)
	{
		...
		err = FxBetaDLM_Get_Model(	fx3dund,
									und->model);
		...
		err = initialise_FxBetaDLM_Hermite(	und->num_params,
											und->hermite);
		...
		err = merge_rates_ts(	und->model->str->time_dom,
		...
		memcpy(und->corr_times, und->model->str->time_3F_corr, und->model->str->nb_3F_corr* sizeof(double));
		memcpy(und->correl_dom_for, und->model->str->dom_for_3F_corr, und->model->str->nb_3F_corr * sizeof(double));
		memcpy(und->correl_dom_fx, und->model->str->dom_fx_3F_corr, und->model->str->nb_3F_corr * sizeof(double));
		memcpy(und->correl_for_fx, und->model->str->for_fx_3F_corr, und->model->str->nb_3F_corr * sizeof(double));
	...
	else
	{
		...
		for (i=0; i<und->sigma_n_rates; i++)
		{
			und->sigma_date_rates[i] = und->today + und->sigma_time_rates[i] * DAYS_IN_YEAR + 1.0e-08;
		}
...
double		fx_vol_shift)
{
	...
	double			*temp_sig_time1		= NULL,
					*temp_sig_time2		= NULL,
					*temp_sig1			= NULL,
					*temp_sig2			= NULL;

	double			*temp_tau_time		= NULL,
					*temp_tau			= NULL;
	...
	if (dom_lam < 1.0e-08) dom_lam = 1.0e-08;
	if (for_lam < 1.0e-08) for_lam = 1.0e-08;
	...
	und->spot_fx = fx_spot * swp_f_df (today, fx_spot_date, dom_yc) / swp_f_df (today, fx_spot_date, for_yc);
	...
	if (cpd->pd_leg->num_cpn > 0) last = cpd->pd_leg->cpn[cpd->pd_leg->num_cpn-1].pay_date;
	else last = 0;

	if (last < today + 740) last = today + 740;

	if (dom_calib || for_calib)
	{
		/* check that we don't calibrate too short options */
		i0 = 0;
		while (i0 < cpd->num_calls && (cpd->call[i0].ex_date - today) < CPD_MINCALDAYS) i0++;

		if (cpd->num_calls - i0 > 0 && !(cpd->num_calls - i0 == 1 && cpd->call[i0].ex_date <= today + eod_flag))
		{
			...
			for (i=0; i<nex; i++) lex[i] = cpd->call[i+i0].ex_date;
		}
		else
		{
			...
			for (i=0; i<nex; i++) lex[i] = last - (nex-i)*365;
		}
	}
	...
	else
	{
		...
		for (i=0; i<temp_sig_n1; i++) temp_sig1[i] += dom_vol_shift;
	}
	...
	else
	{
		...
		for (i=0; i<temp_sig_n2; i++) temp_sig2[i] += for_vol_shift;
	}
	...
	i = num_fx_mkt_vol - 1;
	while (i > 0 && fx_mkt_vol_date[i] > last) i--;

	if (i < num_fx_mkt_vol - 1 && fx_mkt_vol_date[i] < last) i++;
	num_fx_mkt_vol = i + 1;

	/*	Remove unused correl dates */
	i = und->corr_n_times - 1;
	while (i > 0 && und->corr_times[i] > (last - today) * YEARS_IN_DAY) i--;

	if (i < und->corr_n_times - 1 && und->corr_times[i] < (last - today) * YEARS_IN_DAY) i++;
	...
	if (cpd_dlm_params->use_beta_dlm)
	{
		...
		err = init_FxBetaDLM_model(	1.0,									//MinTimeCorrel
		...
		for (i=0; i<num_fx_mkt_vol; i++) opt_strikes[i] = 0.0;

		err = FxBetaDLM_CalibrationModel(	num_fx_mkt_vol,
											fx_mkt_vol_date,
											fx_mkt_fix_date,
											opt_strikes,
											fx_mkt_vol,
...
FREE_RETURN:
	...
	if (fx_mkt_vol_time) free (fx_mkt_vol_time);

	if (lex) free (lex);
	if (temp_sig_time1) free (temp_sig_time1);
	if (temp_sig1) free (temp_sig1);
	if (temp_sig_time2) free (temp_sig_time2);
	if (temp_sig2) free (temp_sig2);
	if (temp_tau_time) free (temp_tau_time);
	if (temp_tau) free (temp_tau);

	if (opt_strikes) free (opt_strikes);
	if (fx_mkt_fix_date) free (fx_mkt_fix_date);
...
Err	err = NULL;
	...
	if (src->use_beta_dlm)
	{
		...
		err = initialise_FxBetaDLM_Hermite(	src->num_params,
											dest->hermite);
	}
	else
	{
...
Err cpd_AlphaFudgeUpdateUnd (	CPD_UND		und,
								int			UpOrDown) /* Up = 0 and Down = 1 */
{
int i;
Err err = NULL;
...
CPD_UND		und)
{
	if (und->sigma_date_rates) free (und->sigma_date_rates);
	if (und->sigma_time_rates) free (und->sigma_time_rates);
	if (und->sigma_dom) free (und->sigma_dom);
	if (und->sigma_for) free (und->sigma_for);
	if (und->sigma_date_fx) free (und->sigma_date_fx);
	if (und->sigma_time_fx) free (und->sigma_time_fx);
	if (und->sigma_fx) free (und->sigma_fx);
	if (und->corr_times) free (und->corr_times);
	if (und->correl_dom_for) free (und->correl_dom_for);
	if (und->correl_dom_fx) free (und->correl_dom_fx);
	if (und->correl_for_fx) free (und->correl_for_fx);
	...
	if (und->fees) free (und->fees);
	und->fees = NULL;
	if (und->fees_dates) free (und->fees_dates);
	und->fees_dates = NULL;
	und->nb_fees = 0;
...
CPD_EVAL_CONST	eval_const)
{
	...
	eval_const->spot_fx_dom_gam = 
		(1.0 - exp (- dom_lam * (eval_const->fx_spot_time - evt_time))) 
		/ dom_lam;
			
	eval_const->spot_fx_for_gam = 
		(1.0 - exp (- for_lam * (eval_const->fx_spot_time - evt_time))) 
		/ for_lam;
	...
	eval_const->start_dff = swp_f_df (evt_date, cpd->fund_leg->cpn[call->fund_idx].start_date, fund_yc);
	...
	if (call->num_pd_cpn > 0)
	{
		...
		for (i=call->pd_idx; i<cpd->pd_leg->num_cpn; i++)
		{
			...
			eval_const->pd_fwd_dom_gam[i-call->pd_idx] = 
				(1.0 - exp (- dom_lam * (cpn->fx_val_time - evt_time))) 
				/ dom_lam;
			
			eval_const->pd_fwd_for_gam[i-call->pd_idx] = 
				(1.0 - exp (- for_lam * (cpn->fx_val_time - evt_time))) 
				/ for_lam;
	...
	eval_const->in_not_fund_gam2 = 0.5 * eval_const->in_not_fund_gam * eval_const->in_not_fund_gam * fund_phi;
	...
	eval_const->in_not_pd_gam2 = 0.5 * eval_const->in_not_pd_gam * eval_const->in_not_pd_gam * dom_phi;
	...
	if (call->use_opt_str)
	{
		...
		eval_const->in_not_pd_fwd_dom_gam = (1.0 - exp (- dom_lam * (call->fx_val_time - evt_time))) 
			/ dom_lam;
		
		eval_const->in_not_pd_fwd_for_gam = (1.0 - exp (- for_lam * (call->fx_val_time - evt_time))) 
			/ for_lam;
	...
	if (call_idx < cpd->num_calls - 1)
	{
		...
		eval_const->next_fund_gam = (1.0 - exp (- fund_lam * (next_call->set_time - evt_time))) / fund_lam;
		eval_const->next_fund_gam2 = 0.5 * eval_const->next_fund_gam * eval_const->next_fund_gam * fund_phi;
		
		eval_const->next_pd_dff = swp_f_df (evt_date, next_call->set_date, dom_yc);
		eval_const->next_pd_gam = (1.0 - exp (- dom_lam * (next_call->set_time - evt_time))) / dom_lam;
		eval_const->next_pd_gam2 = 0.5 * eval_const->next_pd_gam * eval_const->next_pd_gam * dom_phi;
		...
		eval_const->next_start_fund_gam = (1.0 - exp (- fund_lam * (temp_time - evt_time))) / fund_lam;
	...
	eval_const->fin_not_disc_gam = 
		(1.0 - exp (- dom_lam * (cpn->pay_time - evt_time))) 
		/ dom_lam;
	eval_const->fin_not_disc_gam2 = 
		0.5 * eval_const->fin_not_disc_gam 
	...
	eval_const->fin_not_fwd_dom_gam = (1.0 - exp (- dom_lam * (cpn->fx_val_time - evt_time))) 
		/ dom_lam;
	
	eval_const->fin_not_fwd_for_gam = (1.0 - exp (- for_lam * (cpn->fx_val_time - evt_time))) 
		/ for_lam;
...
Err	cpd_fill_FxInstrument_dlm(	CPD_UND			und,
								CPD_STR			cpd,
								PD_CALL			call,
								int				index_cpn,
								PD_EXO_CPN		cpn,
...
double	coef, log_coef;
	...
	err = FxBetaDLM_Allocate_FxOptInst(	0,
										nb_strikes,
										Inst);
	...
	if (und->num_params->iPrecalcSmile)
	{
		...
		fwd_strike[0] = und->model->dCashFx * swp_f_df(und->model->lToday, cpn->fx_val_date, und->model->cYcFor) 
		...
		err = FxBetaDLM_Setup_FxOptInst(cpn->fx_val_date,
										cpn->pay_date,
										und->model->lToday,
										call->ex_date,
										1,
		...
		err = FxBetaDLM_Calculate_AllConst(	PrecalcInstForwardATM,
											und->model,
											und->num_params,
											InstConstForwardATM);
		...
		err = FxBetaDLM_Price_FxOptInst(PrecalcInstForwardATM,
										und->model,
										InstConstForwardATM,
										und->hermite,
										und->num_params,
		...
		err = srt_f_optimpvol(	fwd_premium,
								fwd_strike[0],
								fwd_strike[0],
		...
		*/
		...
		{
			step_Fwd = (norm(- std_Fwd_adj / std_Fwd + und->num_params->dNbStdFwd) - norm(- std_Fwd_adj / std_Fwd - und->num_params->dNbStdFwd)) 
	...
	/* Update the precalculations */
	err = FxBetaDLM_Calculate_AllConst(	Inst,
										und->model,
										und->num_params,
										InstConst);
	...
	err = FxBetaDLM_Update_InstPrecalc_FromMoment(	Inst,
													und->model,
													und->num_params,
													InstConst);
	...
	if (und->num_params->iPrecalcSmile)
	{
		...
		for (i=0; i<und->num_params->iNbX; i++)
		{
			...
			err = FxBetaDLM_Update_InstPrecalc_FromX0(	X0,
														PrecalcInst,
														und->model,
														und->num_params,
														InstConst);
			...
			err = FxBetaDLM_Price_FxOptInst(PrecalcInst,
											und->model,
											InstConst,
											und->hermite,
											und->num_params,
			...
			for (j=0; j<PrecalcInst->iNbStrike; j++)
			{
				...
				if(j>0)
				{
					alpha[j] = (premium[j] - premium[j-1]) / (InstConst->dPrecalcFwdFloor[j] - InstConst->dPrecalcFwdFloor[j-1]);
				}
			}
		}
		...
		if (nb_strikes > 2)
		{
			...
			for (i=0; i<und->num_params->iNbX; i++)
			{
				...
				for (j=0; j<PrecalcInst->iNbStrike; j++)
				{
					...
					if(j>0)
					{
						alpha[j] = (premium[j] - premium[j-1]) / (InstConst->dPrecalcFwdCap[j] - InstConst->dPrecalcFwdCap[j-1]);
					}
				}
			}
		}
...
FREE_RETURN:	
	...
	if (InstConstForwardATM) free(InstConstForwardATM);
	return err;
}
...
CPD_EVAL_CONST	eval_const)
{
	...
	{
		eval_const->pd_inst = calloc(eval_const->nb_pd_inst, sizeof(FxBetaDLM_FxOptInst));
		eval_const->pd_beta_precalc = calloc(eval_const->nb_pd_inst, sizeof(FxBetaDLM_InstPrecalc));
	...
	if (is_mc)
	{
		beta_temp = (1.0 - exp(-dom_lam * (eval_const->fx_spot_time - und->model->dTStar))) / dom_lam;
		eval_const->spot_fx_dom_gam = beta_temp - dom_beta;
		eval_const->spot_fx_cvx = 0.5 * (beta_temp * beta_temp - dom_beta * dom_beta) * dom_phi;

		beta_temp = (1.0 - exp(-for_lam * (eval_const->fx_spot_time - und->model->dTStar))) / for_lam;
	...
	if (call->num_fund_cpn > 0)
	{
		...
		for (i=call->fund_idx; i<=last_used_fund; i++)
		{
			...
			if (is_mc)
			{				
				beta_temp = (1.0 - exp(-fund_lam * (cpd->fund_leg->cpn[i].pay_time - und->model->dTStar))) / fund_lam;
				eval_const->fund_gam[i-call->fund_idx] = beta_temp - dom_beta;
				eval_const->fund_gam2[i-call->fund_idx] = 0.5 * (beta_temp * beta_temp - dom_beta * dom_beta) * fund_phi;
			}
			else
			{
				eval_const->fund_gam[i-call->fund_idx] = 
	...
	if (is_mc)
	{				
		beta_temp = (1.0 - exp(-fund_lam * (cpd->fund_leg->cpn[call->fund_idx].start_time - und->model->dTStar))) / fund_lam;
	...
	if (call->num_pd_cpn > 0)
	{
		...
		for (i=call->pd_idx; i<=last_used_pd; i++)
		{
			...
			if (is_mc)
			{				
				beta_temp = (1.0 - exp(-dom_lam * (cpd->pd_leg->cpn[i].pay_time - und->model->dTStar))) / dom_lam;
				eval_const->pd_disc_gam[i-call->pd_idx] = beta_temp - dom_beta;
				eval_const->pd_disc_gam2[i-call->pd_idx] = 0.5 * (beta_temp * beta_temp - dom_beta * dom_beta) * dom_phi;
			}
			else
			{
				eval_const->pd_disc_gam[i-call->pd_idx] = 
	...
	if (is_mc)
	{
		...
		eval_const->pd_S_const += -0.5 * (und->model->dB0 * und->model->dB0 * eval_const->varFFX / eval_const->pd_S_lin + log(eval_const->pd_S_lin));
		eval_const->pd_S_const += 0.5 * (dom_beta * dom_beta * dom_phi
										- for_beta * for_beta * for_phi);
	...
	else
	{
		...
		eval_const->pd_S_quad = und->model->dC0 * eval_const->pd_S_lin;
		eval_const->pd_S_const = -0.5 * (eval_const->varFFX * eval_const->pd_S_lin * und->model->dB0 * und->model->dB0 - log(eval_const->pd_S_lin));
		eval_const->pd_S_lin *= und->model->dB0;

		eval_const->pd_X_const = 0.5 * (eval_const->varFFX + eval_const->beta_dom * eval_const->beta_dom * eval_const->phi_dom  
		...
		eval_const->pd_S_const += log(fwd_fx);
		eval_const->pd_S_const += -0.5 * (eval_const->beta_dom * eval_const->beta_dom * eval_const->phi_dom 
					- eval_const->beta_for * eval_const->beta_for * eval_const->phi_for);
	...
	if (call->num_pd_cpn > 0)
	{
		...
		for (i=call->pd_idx; i<=last_used_pd; i++)
		{
			...
			if (eval_const->pd_abs_beta[i-call->pd_idx] > 1.0E-16 ||
				eval_const->pd_floor_type[i-call->pd_idx] == 1 || eval_const->pd_floor_type[i-call->pd_idx] == 2 ||
				eval_const->pd_floor_type[i-call->pd_idx] == 4 || eval_const->pd_cap_type[i-call->pd_idx] > 0)
			{
			...
			if (nb_strikes > 0)
			{
				err = cpd_fill_FxInstrument_dlm(und,
												cpd,
												call,
												i-call->pd_idx,
												cpn,
			...
			if (is_mc)
			{
				...
				eval_const->pd_fwd_cvx[i-call->pd_idx] = 0.5 * (beta_temp * beta_temp - dom_beta * dom_beta) * dom_phi;
				...
				eval_const->pd_fwd_cvx[i-call->pd_idx] -= 0.5 * (beta_temp * beta_temp - for_beta * for_beta) * for_phi;						
			}
			else
			{
				eval_const->pd_fwd_dom_gam[i-call->pd_idx] = 
	...
	{				
		...
		eval_const->in_not_fund_gam2 = 0.5 * (beta_temp * beta_temp - dom_beta * dom_beta) * fund_phi;				
	}
	else
	{
		eval_const->in_not_fund_gam = (1.0 - exp (- fund_lam * (call->set_time - evt_time))) / fund_lam;
	...
	if (!eval_redemption)
	{
		...
		if (is_mc)
		{				
			beta_temp = (1.0 - exp(-fund_lam * (next_call->set_time - und->model->dTStar))) / fund_lam;
			eval_const->next_fund_gam = beta_temp - dom_beta;
			eval_const->next_fund_gam2 = 0.5 * (beta_temp * beta_temp - dom_beta * dom_beta) * fund_phi;
		}
		else
		{
			eval_const->next_fund_gam = (1.0 - exp (- fund_lam * (next_call->set_time - evt_time))) / fund_lam;
		...
		if (is_mc)
		{				
			beta_temp = (1.0 - exp(-dom_lam * (next_call->set_time - und->model->dTStar))) / dom_lam;
			eval_const->next_pd_gam = beta_temp - dom_beta;
			eval_const->next_pd_gam2 = 0.5 * (beta_temp * beta_temp - dom_beta * dom_beta) * dom_phi;				
		}
		else
		{
			eval_const->next_pd_gam = (1.0 - exp (- dom_lam * (next_call->set_time - evt_time))) / dom_lam;
		...
		{				
			...
			eval_const->next_start_fund_gam2 = 0.5 * (beta_temp * beta_temp - dom_beta * dom_beta) * fund_phi;
		}
		else
		{
			eval_const->next_start_fund_gam = (1.0 - exp (- fund_lam * (temp_time - evt_time))) / fund_lam;
	...
	else
	{
		...
		{				
			...
			eval_const->fin_not_disc_gam2 = 0.5 * (beta_temp * beta_temp - dom_beta * dom_beta) * dom_phi;				
		}
		else
		{
			eval_const->fin_not_disc_gam = 
		...
		if (is_mc)
		{
			...
			eval_const->fin_not_fwd_cvx = 0.5 * (beta_temp * beta_temp - dom_beta * dom_beta) * dom_phi;

			beta_temp = (1.0 - exp(-for_lam * (cpn->fx_val_time - und->model->dTStar))) / for_lam;
			eval_const->fin_not_fwd_for_gam = beta_temp - for_beta;
			eval_const->fin_not_fwd_cvx -= 0.5 * (beta_temp * beta_temp - for_beta * for_beta) * for_phi;						
		}
		else
		{	
			eval_const->fin_not_fwd_dom_gam = (1.0 - exp (- dom_lam * (cpn->fx_val_time - evt_time))) 
		...
		if (eval_const->fin_not_abs_beta > 1.0E-16 ||
				eval_const->fin_not_floor_type == 1 || eval_const->fin_not_floor_type == 2 ||
				eval_const->fin_not_floor_type == 4 || eval_const->fin_not_cap_type > 0)
		{
			/* calculation of the fwd */
			strikes[nb_strikes] = fwd_fx / 10000.0;
...
CPD_TREE_ARG	tree_arg)
{
	...
	if (und->use_beta_dlm)
	{
		...
		if (!tree_arg->vol_change || !tree_arg->sig_dom || !tree_arg->sig_for || !tree_arg->sig_fx ||
			!tree_arg->corr_dom_for || !tree_arg->corr_dom_fx || !tree_arg->corr_for_fx ||
			!tree_arg->dr_const_dom || !tree_arg->dr_coef_dom || !tree_arg->dr_const_for || !tree_arg->dr_coef1_for || !tree_arg->dr_coef2_for || 
			!tree_arg->dr_coef3_for || !tree_arg->dr_const_fx || !tree_arg->dr_coef1_fx || !tree_arg->dr_coef2_fx || !tree_arg->dr_coef3_fx || 
			!tree_arg->dom_ifr || !tree_arg->dom_fwd || !tree_arg->dom_var || !dom_phi || !dom_beta || 
			!tree_arg->for_ifr || !tree_arg->for_fwd || !tree_arg->for_var || !for_phi || !for_beta ||
			!tree_arg->fx_fwd || !tree_arg->fx_var || !var_ffx ||
			!tree_arg->glob_corr_dom_for || !tree_arg->glob_corr_dom_fx || !tree_arg->glob_corr_for_fx)
		{
		...
		Fx3DBetaDLM_PrecalcGRFNTreeQBeta(	tree_arg->nstp,
											tree_arg->time,
											und->model,
											tree_arg->dom_fwd,
											tree_arg->dom_var,
		...
		Fx3DBetaDLM_DriftVolAndCorr(	tree_arg->nstp,
										tree_arg->time,
										und->model,
										dom_phi,
										dom_beta,
		...
		for (i=0; i<tree_arg->nstp; i++)
		{
			...
			{
				tree_arg->dom_ifr[i] = swp_f_zr (tree_arg->date[i], tree_arg->date[i+1], tree_arg->dom_yc);
			}
			else
			{
				tree_arg->dom_ifr[i] = swp_f_zr (tree_arg->date[i], tree_arg->date[i]+1, tree_arg->dom_yc);
	...
	else
	{
		...
		for (i=tree_arg->nstp-2; i>=0; i--)
		{
			...
			if (fabs (tree_arg->sig_dom[i] - tree_arg->sig_dom[i+1]) 
				...
				+ fabs (tree_arg->corr_for_fx[i] - tree_arg->corr_for_fx[i+1]) > EPS)
			{
		...
		fill_fwd_var_corr (	tree_arg->nstp,
							tree_arg->time,
							tree_arg->date,
							tree_arg->sig_dom,		
							tree_arg->sig_for, 
		...
		for (i=0; i<tree_arg->nstp; i++)
		{
			tree_arg->fx_fwd[i] = log (
				swp_f_df (und->today, tree_arg->date[i], und->for_yc)
				/ swp_f_df (und->today, tree_arg->date[i], und->dom_yc))
				- 0.5 * tree_arg->fx_var[i];
			
			err = Fx3DtsFwdBetaAdjustment_corr(0.0, tree_arg->time[i], tree_arg->time[i], tree_arg->time[i],
									und->sigma_time_rates,  und->sigma_n_rates,
									und->sigma_dom, und->lda_dom,
									und->sigma_for, und->lda_for,   
									und->sigma_date_fx,  und->sigma_fx,  und->sigma_n_fx, 
									und->corr_times, und->correl_dom_for,  und->correl_dom_fx,  und->correl_for_fx, und->corr_n_times, &temp);
	...
	if (!tree_arg->is_event || !tree_arg->void_prm 
		|| !tree_arg->is_bar || !tree_arg->bar_lvl || !tree_arg->bar_cl)
	{
...
FREE_RETURN:
	...
	if (dom_beta) free (dom_beta);
	if (for_beta) free (for_beta);
	if (var_ffx) free (var_ffx);

	return err;
}

Err cpd_free_tree_arg(
CPD_TREE_ARG	tree_arg)
{
	int					i;
	CPD_PAY_ARG			cpd_prm;

	if (tree_arg->time) free (tree_arg->time);
	if (tree_arg->date) free (tree_arg->date);
	if (tree_arg->vol_change) free (tree_arg->vol_change);
	if (tree_arg->sig_dom) free (tree_arg->sig_dom);
	if (tree_arg->sig_for) free (tree_arg->sig_for);
	if (tree_arg->sig_fx) free (tree_arg->sig_fx);
	if (tree_arg->corr_dom_for) free (tree_arg->corr_dom_for);
	if (tree_arg->corr_dom_fx) free (tree_arg->corr_dom_fx);
	if (tree_arg->corr_for_fx) free (tree_arg->corr_for_fx);
	if (tree_arg->dom_ifr) free (tree_arg->dom_ifr);
	if (tree_arg->dom_fwd) free (tree_arg->dom_fwd);
	if (tree_arg->dom_var) free (tree_arg->dom_var);
	if (tree_arg->for_ifr) free (tree_arg->for_ifr);
	if (tree_arg->for_fwd) free (tree_arg->for_fwd);
	if (tree_arg->for_var) free (tree_arg->for_var);
	if (tree_arg->fx_fwd) free (tree_arg->fx_fwd);
	if (tree_arg->fx_var) free (tree_arg->fx_var);
	if (tree_arg->is_event) free (tree_arg->is_event);
	if (tree_arg->bar_lvl) free (tree_arg->bar_lvl);
	if (tree_arg->bar_cl) free (tree_arg->bar_cl);
	if (tree_arg->is_bar) free (tree_arg->is_bar);

	if (tree_arg->dr_const_dom) free (tree_arg->dr_const_dom);
	if (tree_arg->dr_coef_dom) free (tree_arg->dr_coef_dom);
	if (tree_arg->dr_const_for) free (tree_arg->dr_const_for);
	if (tree_arg->dr_coef1_for) free (tree_arg->dr_coef1_for);
	if (tree_arg->dr_coef2_for) free (tree_arg->dr_coef2_for);
	if (tree_arg->dr_coef3_for) free (tree_arg->dr_coef3_for);
	if (tree_arg->dr_const_fx) free (tree_arg->dr_const_fx);
	if (tree_arg->dr_coef1_fx) free (tree_arg->dr_coef1_fx);
	if (tree_arg->dr_coef2_fx) free (tree_arg->dr_coef2_fx);
	if (tree_arg->dr_coef3_fx) free (tree_arg->dr_coef3_fx);

	if (tree_arg->glob_corr_dom_for) free (tree_arg->glob_corr_dom_for);
	if (tree_arg->glob_corr_dom_fx) free (tree_arg->glob_corr_dom_fx);
	if (tree_arg->glob_corr_for_fx) free (tree_arg->glob_corr_for_fx);
	...
	if (tree_arg->void_prm)
	{
		for (i=0; i<tree_arg->nstp; i++)
		{
			...
			if (cpd_prm)
			{
				cpd_free_eval_const(cpd_prm->und,
									cpd_prm->cpd,
									&(cpd_prm->eval_const));
...
CPD_MC_ARG		mc_arg)
{
	...
	if (und->use_beta_dlm)
	{
		...
		if (!mc_arg->dom_fwd || !mc_arg->dom_std || !mc_arg->dom_phi || !mc_arg->dom_beta
			|| !mc_arg->dom_bond_pay || !mc_arg->dom_beta_pay
			|| !mc_arg->for_fwd_const || !mc_arg->for_fwd_lin || !mc_arg->for_std || !mc_arg->for_phi || !mc_arg->for_beta
			|| !mc_arg->fx_fwd || !mc_arg->fx_std || !mc_arg->ffx_var
			|| !mc_arg->dom_for_cov || !mc_arg->dom_fx_cov || !mc_arg->for_fx_cov)
		{
			err = "Memory allocation error (4) in cpd_fill_mc_arg";
			goto FREE_RETURN;
		}

		Fx3DBetaDLM_ExpectAndVarGrfn(	mc_arg->nb_dates,
										mc_arg->time,
										und->model,
										mc_arg->dom_fwd,
										mc_arg->dom_std,
	...
	else
	{
		...
		if (!mc_arg->dom_ifr || !mc_arg->dom_fwd || !mc_arg->dom_std || !mc_arg->dom_phi || !mc_arg->dom_beta
			|| !mc_arg->dom_bond_pay || !mc_arg->dom_beta_pay
			|| !mc_arg->for_ifr || !mc_arg->for_fwd || !mc_arg->for_std || !mc_arg->for_phi || !mc_arg->for_beta
			|| !mc_arg->fx_fwd || !mc_arg->fx_std 
			|| !mc_arg->dom_for_cov || !mc_arg->dom_fx_cov || !mc_arg->for_fx_cov)
		{
			err = "Memory allocation error (4) in cpd_fill_mc_arg";
			goto FREE_RETURN;
	...
	for (i=mc_arg->nb_dates-1; i>=0; i--)
	{
		if (j >= 0 && fabs (mc_arg->date[i] - cpd->call[j].ex_date) < 1.0e-04)
		{
			...
			if (und->use_beta_dlm)
			{
				...
				mc_arg->dom_bond_pay[i] = log(swp_f_df(cpd->call[j].ex_date, und->model->lTStarDate, und->dom_yc));
				mc_arg->dom_bond_pay[i] += 0.5 * cpd_prm->eval_const.beta_dom * cpd_prm->eval_const.beta_dom * cpd_prm->eval_const.phi_dom;
...
CPD_MC_ARG	mc_arg)
{
	...
	if (mc_arg->time) free (mc_arg->time);
	if (mc_arg->date) free (mc_arg->date);
	if (mc_arg->dom_ifr) free (mc_arg->dom_ifr);
	if (mc_arg->dom_fwd) free (mc_arg->dom_fwd);
	if (mc_arg->dom_std) free (mc_arg->dom_std);
	if (mc_arg->dom_phi) free (mc_arg->dom_phi);
	if (mc_arg->dom_beta) free (mc_arg->dom_beta);
	if (mc_arg->dom_bond_pay) free (mc_arg->dom_bond_pay);
	if (mc_arg->dom_beta_pay) free (mc_arg->dom_beta_pay);
	if (mc_arg->for_ifr) free (mc_arg->for_ifr);
	if (mc_arg->for_fwd) free (mc_arg->for_fwd);
	if (mc_arg->for_std) free (mc_arg->for_std);
	if (mc_arg->for_phi) free (mc_arg->for_phi);
	if (mc_arg->for_beta) free (mc_arg->for_beta);
	if (mc_arg->fx_fwd) free (mc_arg->fx_fwd);
	if (mc_arg->fx_std) free (mc_arg->fx_std);
	if (mc_arg->dom_for_cov) free (mc_arg->dom_for_cov);
	if (mc_arg->dom_fx_cov) free (mc_arg->dom_fx_cov);
	if (mc_arg->for_fx_cov) free (mc_arg->for_fx_cov);

	if (mc_arg->for_fwd_const) free (mc_arg->for_fwd_const);
	if (mc_arg->for_fwd_lin) free (mc_arg->for_fwd_lin);
	if (mc_arg->ffx_var) free (mc_arg->ffx_var);
...
int			skip_fill)
{
	...
	if(skip_fill == 0)
	{
		...
		{
			err = cpd_fill_und (fx3dund, und, cpd_dlm_params, dom_vol_shift, for_vol_shift, fx_vol_shift);
		}
	...
	if (cpd->num_calls > 0 && cpd->call)
	{
		...
		if (cpd->type == 1 && forcetree) cpd->type = -1;
	...
	(TYPE) == 0?																					\
		0.0:(																						\
			...
			POS_VAL ((FWD) - (STRIKE)):(															\
				...
				POS_VAL ((STRIKE) - (FWD)):(														\
					(TYPE) == 3?																	\
						CALL_VAL ((FWD), (STRIKE), ((LOGFWD)-(LOGSTRIKE))/(STD)-(HALF_STD), (STD)):	\
							PUT_VAL ((FWD), (STRIKE), ((LOGFWD)-(LOGSTRIKE))/(STD)-(HALF_STD), (STD))))))	
...
double		bar_lvl)
{
	...
	if (n1>0) fx_DI = exp (sv[1][0][0][2] - sv[0][0][0][2]);
	else fx_DI = 1.0;
	
	if (n2>0) fx_DJ = exp (sv[0][1][0][2] - sv[0][0][0][2]);
	else fx_DJ = 1.0;

	if (n3>0) fx_DK = exp (sv[0][0][1][2] - sv[0][0][0][2]);
	else fx_DK = 1.0;
	...
	if (n1>0) start_DI = exp (- eval_const->start_gam * (sv[1][0][0][eval_const->fund_var] - sv[0][0][0][eval_const->fund_var]));
	else start_DI = 1.0;
	
	if (n2>0) start_DJ = exp (- eval_const->start_gam * (sv[0][1][0][eval_const->fund_var] - sv[0][0][0][eval_const->fund_var]));
	else start_DJ = 1.0;

	if (n3>0) start_DK = exp (- eval_const->start_gam  * (sv[0][0][1][eval_const->fund_var] - sv[0][0][0][eval_const->fund_var]));
	else start_DK = 1.0;

	/* For initial notional */
	in_not_fund_I =  eval_const->in_not_fund_dff 
				* exp (- eval_const->in_not_fund_gam * sv[0][0][0][eval_const->fund_var] - eval_const->in_not_fund_gam2)
				* call->fund_not_amt;

	in_not_fund_J = in_not_fund_I;
	in_not_fund_K = in_not_fund_I;

	if (n1>0) in_not_fund_DI = exp (- eval_const->in_not_fund_gam * (sv[1][0][0][eval_const->fund_var] - sv[0][0][0][eval_const->fund_var]));
	else in_not_fund_DI = 1.0;
	
	if (n2>0) in_not_fund_DJ = exp (- eval_const->in_not_fund_gam * (sv[0][1][0][eval_const->fund_var] - sv[0][0][0][eval_const->fund_var]));
	else in_not_fund_DJ = 1.0;

	if (n3>0) in_not_fund_DK = exp (- eval_const->in_not_fund_gam  * (sv[0][0][1][eval_const->fund_var] - sv[0][0][0][eval_const->fund_var]));
	else in_not_fund_DK = 1.0;
	...
	if (n1>0) in_not_pd_DI = exp (- eval_const->in_not_pd_gam * (sv[1][0][0][0] - sv[0][0][0][0]));
	else in_not_pd_DI = 1.0;
	
	if (n2>0) in_not_pd_DJ = exp (- eval_const->in_not_pd_gam * (sv[0][1][0][0] - sv[0][0][0][0]));
	else in_not_pd_DJ = 1.0;

	if (n3>0) in_not_pd_DK = exp (- eval_const->in_not_pd_gam  * (sv[0][0][1][0] - sv[0][0][0][0]));
	else in_not_pd_DK = 1.0;
	...
	if (call->use_opt_str)
	{
		...
		if (n1>0) in_not_pd_fwd_DI = exp (- eval_const->in_not_pd_fwd_for_gam * (sv[1][0][0][1] - sv[0][0][0][1])
									    + eval_const->in_not_pd_fwd_dom_gam * (sv[1][0][0][0] - sv[0][0][0][0]));
		else in_not_pd_fwd_DI = 1.0;
		
		if (n2>0) in_not_pd_fwd_DJ = exp (- eval_const->in_not_pd_fwd_for_gam * (sv[0][1][0][1] - sv[0][0][0][1])
										+ eval_const->in_not_pd_fwd_dom_gam * (sv[0][1][0][0] - sv[0][0][0][0]));
		else in_not_pd_fwd_DJ = 1.0;

		if (n3>0) in_not_pd_fwd_DK = exp (- eval_const->in_not_pd_fwd_for_gam * (sv[0][0][1][1] - sv[0][0][0][1])
										+ eval_const->in_not_pd_fwd_dom_gam * (sv[0][0][1][0] - sv[0][0][0][0]));
		else in_not_pd_fwd_DK = 1.0;
	}
	...
	if (!eval_redemption)
	{
		next_start_fund_I =  eval_const->next_start_fund_dff
						* exp (- eval_const->next_start_fund_gam * sv[0][0][0][eval_const->fund_var] - eval_const->next_start_fund_gam2)
						* cpd->fund_leg->notional;

		next_start_fund_J = next_start_fund_I;
		next_start_fund_K = next_start_fund_I;

		if (n1>0) next_start_fund_DI = exp (- eval_const->next_start_fund_gam * (sv[1][0][0][eval_const->fund_var] - sv[0][0][0][eval_const->fund_var]));
		else next_start_fund_DI = 1.0;
		
		if (n2>0) next_start_fund_DJ = exp (- eval_const->next_start_fund_gam * (sv[0][1][0][eval_const->fund_var] - sv[0][0][0][eval_const->fund_var]));
		else next_start_fund_DJ = 1.0;

		if (n3>0) next_start_fund_DK = exp (- eval_const->next_start_fund_gam  * (sv[0][0][1][eval_const->fund_var] - sv[0][0][0][eval_const->fund_var]));
		else next_start_fund_DK = 1.0;


		next_fund_I = eval_const->next_fund_dff 
					* exp (- eval_const->next_fund_gam * sv[0][0][0][eval_const->fund_var]- eval_const->next_fund_gam2)
					* next_call->fund_not_amt;

		next_fund_J = next_fund_I;
		next_fund_K = next_fund_I;

		if (n1>0) next_fund_DI = exp (- eval_const->next_fund_gam * (sv[1][0][0][eval_const->fund_var] - sv[0][0][0][eval_const->fund_var]));
		else next_fund_DI = 1.0;
		
		if (n2>0) next_fund_DJ = exp (- eval_const->next_fund_gam * (sv[0][1][0][eval_const->fund_var] - sv[0][0][0][eval_const->fund_var]));
		else next_fund_DJ = 1.0;

		if (n3>0) next_fund_DK = exp (- eval_const->next_fund_gam  * (sv[0][0][1][eval_const->fund_var] - sv[0][0][0][eval_const->fund_var]));
		else next_fund_DK = 1.0;
		...
		if (n1>0) next_pd_DI = exp (- eval_const->next_pd_gam * (sv[1][0][0][0] - sv[0][0][0][0]));
		else next_pd_DI = 1.0;
		
		if (n2>0) next_pd_DJ = exp (- eval_const->next_pd_gam * (sv[0][1][0][0] - sv[0][0][0][0]));
		else next_pd_DJ = 1.0;

		if (n3>0) next_pd_DK = exp (- eval_const->next_pd_gam  * (sv[0][0][1][0] - sv[0][0][0][0]));
		else next_pd_DK = 1.0;
	}
	else
	/*	Final notional */
	{
		...
		if (n1>0) fin_not_disc_DI = exp (- eval_const->fin_not_disc_gam * (sv[1][0][0][0] - sv[0][0][0][0]));
		else fin_not_disc_DI = 1.0;
		
		if (n2>0) fin_not_disc_DJ = exp (- eval_const->fin_not_disc_gam * (sv[0][1][0][0] - sv[0][0][0][0]));
		else fin_not_disc_DJ = 1.0;

		if (n3>0) fin_not_disc_DK = exp (- eval_const->fin_not_disc_gam  * (sv[0][0][1][0] - sv[0][0][0][0]));
		else fin_not_disc_DK = 1.0;
		...
		if (n1>0) fin_not_fwd_DI = exp (- eval_const->fin_not_fwd_for_gam * (sv[1][0][0][1] - sv[0][0][0][1])
									    + eval_const->fin_not_fwd_dom_gam * (sv[1][0][0][0] - sv[0][0][0][0]));
		else fin_not_fwd_DI = 1.0;
		
		if (n2>0) fin_not_fwd_DJ = exp (- eval_const->fin_not_fwd_for_gam * (sv[0][1][0][1] - sv[0][0][0][1])
										+ eval_const->fin_not_fwd_dom_gam * (sv[0][1][0][0] - sv[0][0][0][0]));
		else fin_not_fwd_DJ = 1.0;

		if (n3>0) fin_not_fwd_DK = exp (- eval_const->fin_not_fwd_for_gam * (sv[0][0][1][1] - sv[0][0][0][1])
										+ eval_const->fin_not_fwd_dom_gam * (sv[0][0][1][0] - sv[0][0][0][0]));
		else fin_not_fwd_DK = 1.0;
	}
	...
	{		
		...
		eval_const->fund_I[l] = eval_const->fund_dff[l] *
			exp (- eval_const->fund_gam[l] * sv[0][0][0][eval_const->fund_var] - eval_const->fund_gam2[l])
		...
		if (n1>0) eval_const->fund_DI[l] = exp (- eval_const->fund_gam[l] * (sv[1][0][0][eval_const->fund_var] - sv[0][0][0][eval_const->fund_var]));
		else eval_const->fund_DI[l] = 1.0;
		
		if (n2>0) eval_const->fund_DJ[l] = exp (- eval_const->fund_gam[l] * (sv[0][1][0][eval_const->fund_var] - sv[0][0][0][eval_const->fund_var]));
		else eval_const->fund_DJ[l] = 1.0;

		if (n3>0) eval_const->fund_DK[l] = exp (- eval_const->fund_gam[l] * (sv[0][0][1][eval_const->fund_var] - sv[0][0][0][eval_const->fund_var]));
		else eval_const->fund_DK[l] = 1.0;
	}
	
	for (l=0; l<num_pd_cpn; l++)
	{
		/*	Coupons */
		eval_const->pd_disc_I[l] = eval_const->pd_disc_dff[l] 
		...
		if (n1>0) eval_const->pd_disc_DI[l] = exp (- eval_const->pd_disc_gam[l] * (sv[1][0][0][0] - sv[0][0][0][0]));
		else eval_const->pd_disc_DI[l] = 1.0;
		
		if (n2>0) eval_const->pd_disc_DJ[l] = exp (- eval_const->pd_disc_gam[l] * (sv[0][1][0][0] - sv[0][0][0][0]));
		else eval_const->pd_disc_DJ[l] = 1.0;

		if (n3>0) eval_const->pd_disc_DK[l] = exp (- eval_const->pd_disc_gam[l] * (sv[0][0][1][0] - sv[0][0][0][0]));
		else eval_const->pd_disc_DK[l] = 1.0;

		
		/* for the fwd */
		
		eval_const->pd_fwd_I[l] = eval_const->pd_fwd_dff_ratio[l]
		...
		if (n1>0)
			eval_const->pd_fwd_DI[l] = exp (- eval_const->pd_fwd_for_gam[l] * (sv[1][0][0][1] - sv[0][0][0][1])
											+ eval_const->pd_fwd_dom_gam[l] * (sv[1][0][0][0] - sv[0][0][0][0]));
		else eval_const->pd_fwd_DI[l] = 1.0;
		
		if (n2>0)
			eval_const->pd_fwd_DJ[l] = exp (- eval_const->pd_fwd_for_gam[l] * (sv[0][1][0][1] - sv[0][0][0][1])
											+ eval_const->pd_fwd_dom_gam[l] * (sv[0][1][0][0] - sv[0][0][0][0]));
		else eval_const->pd_fwd_DJ[l] = 1.0;

		if (n3>0)
			eval_const->pd_fwd_DK[l] = exp (- eval_const->pd_fwd_for_gam[l] * (sv[0][0][1][1] - sv[0][0][0][1])
											+ eval_const->pd_fwd_dom_gam[l] * (sv[0][0][1][0] - sv[0][0][0][0]));
		else eval_const->pd_fwd_DK[l] = 1.0;
	}
		...
		{			
			...
			for (k=0; k<n3; k++)
			{
				...
				/*fund_leg += eval_const->start_dff
					* exp (- eval_const->start_gam * svijk[eval_const->fund_var] - eval_const->start_gam2)
					* cpd->fund_leg->notional; */
			...
			* exp (- eval_const->in_not_fund_gam * svijk[eval_const->fund_var] - eval_const->in_not_fund_gam2)
					* call->fund_not_amt; */
				...
				if (!eval_redemption)
				{
					/* fund_leg -= eval_const->next_start_fund_dff
						* exp (- eval_const->next_start_fund_gam * svijk[eval_const->fund_var] - eval_const->next_start_fund_gam2)
						* cpd->fund_leg->notional; */
					fund_leg -= next_start_fund_K;

					/*fund_leg += eval_const->next_fund_dff 
						* exp (- eval_const->next_fund_gam * svijk[eval_const->fund_var] - eval_const->next_fund_gam2)
						* next_call->fund_not_amt; */
					...
					*/
					...
					if (cpn->use_opt_str)
					{
						...
						for (str_idx = 0; str_idx < cpn->nstrikes; str_idx++)
						{
							...
							{
								if (eval_const->pd_std[l] > 1e-16 && cpn->strikes[str_idx] > 1e-16) opt_type = 3;
								else opt_type = 1;
					...
					else
					{
						...
						pd_leg += df * (cpn->alpha + cpn->beta * fwd + eval_const->pd_abs_beta[l] * (floor - cap));
					}
				}
				/*	Final notional */
				...
				if (eval_redemption)
				{
					...
					if (cpn->use_opt_str)
					{
						...
						for (str_idx = 0; str_idx < cpn->nstrikes; str_idx++)
						{
							if (fabs(cpn->weights[str_idx]) > 1e-16)
							{
								if (eval_const->fin_not_std > 1e-16 && cpn->strikes[str_idx] > 1e-16) opt_type = 3;
								else opt_type = 1;
				...
				if (call->use_opt_str)
				{
					...
					for (str_idx = 0; str_idx < call->nstrikes; str_idx++)
					{
						...
						{
							if (eval_const->in_not_pd_std > 1e-16 && call->strikes[str_idx] > 1e-16) opt_type = 3;
							else opt_type = 1;
				...
				for (i=0; i<n1; i++)
				{
					...
					for (j=0; j<n2; j++)
					{
						...
						for (k=0; k<n3; k++)
						{
							...
							if ((call->bar_type == 0 && svijk[2] > bar_lvl) || (call->bar_type == 1 && svijk[2] < bar_lvl))
							{
...
double		*pd_leg)
{
	...
	*fund_leg += eval_const->start_dff
	...
	for (l=0; l<num_fund_cpn; l++)
	{
		*fund_leg += 
			eval_const->fund_dff[l] 
			* exp (- eval_const->fund_gam[l] * Xdomfor[eval_const->fund_var] - eval_const->fund_gam2[l])
	...
	*fund_leg -= eval_const->in_not_fund_dff 
		* exp (- eval_const->in_not_fund_gam * Xdomfor[eval_const->fund_var] - eval_const->in_not_fund_gam2)
		* fund_not_amt;
...
Err	cpd_price_FxInstrument_dlm(	double			Fwd,
								double			LogFwd,
								double			X0,
								CPD_UND			und,
								CPD_STR			cpd,
...
double	*dPrecalcPriceStdAlphaInX;
	...
	if (und->num_params->iPrecalcSmile)
	{
		...
		if(und->num_params->iPriceVol == 0) // We do an interpolation of the prices
		{
			...
			price1 = dPrecalcPriceStdAlphaInX[index_Fwd1] * (Fwd - InstConst->dPrecalcFwdFloor[index_Fwd2]) 
			...
			price2 = dPrecalcPriceStdAlphaInX[index_Fwd1] * (Fwd - InstConst->dPrecalcFwdFloor[index_Fwd2]) 
					+ dPrecalcPriceStdInX[index_Fwd2];
			...
			if(price2 <= 0.0)
			{
				prices[0] = price1 / (InstConst->dPrecalcX[index_X1] - InstConst->dPrecalcX[index_X2])
					* (X0 - InstConst->dPrecalcX[index_X2]);
			}
			else
			if(price1 <= 0.0)
			{
				prices[0] = - price2 / (InstConst->dPrecalcX[index_X1] - InstConst->dPrecalcX[index_X2])
					* (X0 - InstConst->dPrecalcX[index_X2]) + price2;
			}
			else
			{
				prices[0] = (price1 - price2) / (InstConst->dPrecalcX[index_X1] - InstConst->dPrecalcX[index_X2])
					* (X0 - InstConst->dPrecalcX[index_X2]) + price2;
			}
			...
			if (Inst->iNbStrike > 2)
			{
				...
				price1 = dPrecalcPriceStdAlphaInX[index_Fwd1] * (Fwd - InstConst->dPrecalcFwdCap[index_Fwd2]) 
						+ dPrecalcPriceStdInX[index_Fwd2];
				...
				price2 = dPrecalcPriceStdAlphaInX[index_Fwd1] * (Fwd - InstConst->dPrecalcFwdCap[index_Fwd2]) 
						+ dPrecalcPriceStdInX[index_Fwd2];
				...
				if(price2 <= 0.0)
				{
					prices[1] = price1 / (InstConst->dPrecalcX[index_X1] - InstConst->dPrecalcX[index_X2])
						* (X0 - InstConst->dPrecalcX[index_X2]);
				}
				else
				if(price1 <= 0.0)
				{
					prices[1] = - price2 / (InstConst->dPrecalcX[index_X1] - InstConst->dPrecalcX[index_X2])
						* (X0 - InstConst->dPrecalcX[index_X2]) + price2;
				}
				else
				{
					prices[1] = (price1 - price2) / (InstConst->dPrecalcX[index_X1] - InstConst->dPrecalcX[index_X2])
						* (X0 - InstConst->dPrecalcX[index_X2]) + price2;
				}
				prices[2] = Fwd;
			}
		...
		else  // We do an interpolation of the volatilities
		{
			...
			if (Fwd > InstConst->dPrecalcFwdFloor[und->num_params->iNbFwd-1])
			{
			...
			else
			{
				...
				vol1 = dPrecalcPriceStdAlphaInX[index_Fwd1] * (Fwd - InstConst->dPrecalcFwdFloor[index_Fwd2]) 
					+ dPrecalcPriceStdInX[index_Fwd2];

				//vol1 = (dPrecalcPriceStdInX[index_Fwd1] - dPrecalcPriceStdInX[index_Fwd2])
				//	/ (InstConst->dPrecalcFwdFloor[index_Fwd1] - InstConst->dPrecalcFwdFloor[index_Fwd2])
				//	* (Fwd - InstConst->dPrecalcFwdFloor[index_Fwd2]) + dPrecalcPriceStdInX[index_Fwd2];
			}
			...
			if (X0 > InstConst->dPrecalcX[und->num_params->iNbX-1])
			{
				vol = vol1;
			}
			else
			{
				...
				else
				{
					...
					vol2 = dPrecalcPriceStdAlphaInX[index_Fwd1] * (Fwd - InstConst->dPrecalcFwdFloor[index_Fwd2]) 
						+ dPrecalcPriceStdInX[index_Fwd2];
				...
				vol = (vol1 - vol2) / (InstConst->dPrecalcX[index_X1] - InstConst->dPrecalcX[index_X2])
					* (X0 - InstConst->dPrecalcX[index_X2]) + vol2;
			}
			...
			*/
			if (Inst->iNbStrike > 2)
			{
				...
				if (Fwd > InstConst->dPrecalcFwdCap[und->num_params->iNbFwd-1])
				{
					vol1 = dPrecalcPriceStdInX[und->num_params->iNbFwd-1];
				}
				else
				{
					...
					vol1 = dPrecalcPriceStdAlphaInX[index_Fwd1] * (Fwd - InstConst->dPrecalcFwdCap[index_Fwd2]) 
						+ dPrecalcPriceStdInX[index_Fwd2];

					//*vol1 = (dPrecalcPriceStdInX[index_Fwd1] - dPrecalcPriceStdInX[index_Fwd2])
					//	/ (InstConst->dPrecalcFwdCap[index_Fwd1] - InstConst->dPrecalcFwdCap[index_Fwd2])
					//	* (Fwd - InstConst->dPrecalcFwdCap[index_Fwd2]) + dPrecalcPriceStdInX[index_Fwd2];
				}
				...
				else
				{
					...
					else
					{
						...
						vol2 = dPrecalcPriceStdAlphaInX[index_Fwd1] * (Fwd - InstConst->dPrecalcFwdCap[index_Fwd2]) 
							+ dPrecalcPriceStdInX[index_Fwd2];
	...
	else
	{
		...
		err = FxBetaDLM_Price_FxOptInst(Inst,
										und->model,
										InstConst,
										und->hermite,
										und->num_params,
...
double		*pd_leg)
{
	...
	*pd_leg = 0.0;
	...
	for (l=0; l<num_pd_cpn; l++)
	{
		...
		{
			err = cpd_price_FxInstrument_dlm(	fwd,
												log(fwd),
												X,
												und,
												cpd,
		...
		{
			fwd_adj = prices[eval_const->index_fwd[l]] + eval_const->pd_inst[l].dStrike[eval_const->index_fwd[l]];
		}
		else
		{
			fwd_adj = fwd;
...
double		*fee)
{
	...
	if (calc_coupon)
	{
		...
		*fund_leg -= eval_const->in_not_fund_dff 
		...
		{
			*fund_leg -= eval_const->next_start_fund_dff
				* exp (- eval_const->next_start_fund_gam * Xdomfor[eval_const->fund_var] - eval_const->next_start_fund_gam2)
				* fund_not_amt;

			*fund_leg += eval_const->next_fund_dff 
				* exp (- eval_const->next_fund_gam * Xdomfor[eval_const->fund_var] - eval_const->next_fund_gam2)
				* next_fund_not_amt;
		}
...
int			*stop_path)
{
	...
	*stop_path = 0;
	...
	fx = exp (eval_const->pd_S_const + Zfx * (eval_const->pd_S_lin + Zfx * eval_const->pd_S_quad)
				+ eval_const->pd_S_dom * Xdom + eval_const->pd_S_for * Yfor);

	fx_now = log(fx / und->model->dCashFx) 
		+ eval_const->spot_fx_dff_log_ratio
		-	eval_const->spot_fx_for_gam * Yfor 
		+	eval_const->spot_fx_dom_gam * Xdom
		+	eval_const->spot_fx_cvx;
...
int			*stop_path)
{
	...
	*stop_path = 0;
	...
	err = cpd_current_pd_cpn_val(	Xdomfor,
									Zfx,
									spot_fx,
									cpd,
									call_idx,
	...
	if (call->bar_type == 0)
	{
		if (iv > 0.0)
		{
			...
			if (CPNPAID >= call->orig_barrier + SMO)
			{
				*res = iv;
				*stop_path = 1;
				return NULL;
...
double		***optim_bar)
{
	...
	*optim_bar = (double **) dmatrix(0, mc_arg->nb_dates-1, 0, 
		do_infos * (3 + 2 * mc_arg->nb_dates) + (1-do_infos) * 1);
...
double		bar_lvl)
{
	...
	/* For X */
	X_I = eval_const->pd_X_const + eval_const->pd_X_dom * sv[0][0][0][0] + eval_const->pd_X_for * sv[0][0][0][1] + sv[0][0][0][2];
	X_J = X_I;
	X_K = X_I;
	if (n1>0)
	{
		X_DI = eval_const->pd_X_dom * (sv[1][0][0][0]-sv[0][0][0][0]) + eval_const->pd_X_for * (sv[1][0][0][1]-sv[0][0][0][1]) + (sv[1][0][0][2]-sv[0][0][0][2]);
	}
	else
	{
		X_DI = 0.0;
	...
	if (n2>0)
	{
		X_DJ = eval_const->pd_X_dom * (sv[0][1][0][0]-sv[0][0][0][0]) + eval_const->pd_X_for * (sv[0][1][0][1]-sv[0][0][0][1]) + (sv[0][1][0][2]-sv[0][0][0][2]);
	}
	else
	{
		X_DJ = 0.0;
	...
	if (n3>0)
	{
		X_DK = eval_const->pd_X_dom * (sv[0][0][1][0]-sv[0][0][0][0]) + eval_const->pd_X_for * (sv[0][0][1][1]-sv[0][0][0][1]) + (sv[0][0][1][2]-sv[0][0][0][2]);
	}
	else
	{
		X_DK = 0.0;
	...
	if (n1>0)
	{
		fx_DI_Const = X_DI * (eval_const->pd_S_lin + X_DI * eval_const->pd_S_quad)
			+ eval_const->pd_S_dom * (sv[1][0][0][0]-sv[0][0][0][0]) + eval_const->pd_S_for * (sv[1][0][0][1]-sv[0][0][0][1]);
	...
	if (n2>0)
	{
		fx_DJ_Const = X_DJ * (eval_const->pd_S_lin + X_DJ * eval_const->pd_S_quad)
			+ eval_const->pd_S_dom * (sv[0][1][0][0]-sv[0][0][0][0]) + eval_const->pd_S_for * (sv[0][1][0][1]-sv[0][0][0][1]);
	...
	if (n3>0)
	{
		fx_DK_Const = X_DK * (eval_const->pd_S_lin + X_DK * eval_const->pd_S_quad)
			+ eval_const->pd_S_dom * (sv[0][0][1][0]-sv[0][0][0][0]) + eval_const->pd_S_for * (sv[0][0][1][1]-sv[0][0][0][1]);
	...
	if (n1>0)
	{
		start_DI = exp (- eval_const->start_gam * (sv[1][0][0][eval_const->fund_var] - sv[0][0][0][eval_const->fund_var]));
	}
	else
	{
		start_DI = 1.0;
	...
	if (n2>0)
	{
		start_DJ = exp (- eval_const->start_gam * (sv[0][1][0][eval_const->fund_var] - sv[0][0][0][eval_const->fund_var]));
	}
	else
	{
		start_DJ = 1.0;
	...
	if (n3>0)
	{
		start_DK = exp (- eval_const->start_gam  * (sv[0][0][1][eval_const->fund_var] - sv[0][0][0][eval_const->fund_var]));
	}
	else
	{
		start_DK = 1.0;
	...
	if (n1>0)
	{
		in_not_fund_DI = exp (- eval_const->in_not_fund_gam * (sv[1][0][0][eval_const->fund_var] - sv[0][0][0][eval_const->fund_var]));
	}
	else
	{
		in_not_fund_DI = 1.0;
	...
	if (n2>0)
	{
		in_not_fund_DJ = exp (- eval_const->in_not_fund_gam * (sv[0][1][0][eval_const->fund_var] - sv[0][0][0][eval_const->fund_var]));
	}
	else
	{
		in_not_fund_DJ = 1.0;
	...
	if (n3>0)
	{
		in_not_fund_DK = exp (- eval_const->in_not_fund_gam  * (sv[0][0][1][eval_const->fund_var] - sv[0][0][0][eval_const->fund_var]));
	}
	else
	{
		in_not_fund_DK = 1.0;
	...
	if (!eval_redemption)
	{
		...
		if (n1>0)
		{
			next_start_fund_DI = exp (- eval_const->next_start_fund_gam * (sv[1][0][0][eval_const->fund_var] - sv[0][0][0][eval_const->fund_var]));
		}
		else
		{
		...
		if (n2>0)
		{
			next_start_fund_DJ = exp (- eval_const->next_start_fund_gam * (sv[0][1][0][eval_const->fund_var] - sv[0][0][0][eval_const->fund_var]));
		}
		else
		{
		...
		if (n3>0)
		{
			next_start_fund_DK = exp (- eval_const->next_start_fund_gam  * (sv[0][0][1][eval_const->fund_var] - sv[0][0][0][eval_const->fund_var]));
		}
		else
		{
		...
		if (n1>0)
		{
			next_fund_DI = exp (- eval_const->next_fund_gam * (sv[1][0][0][eval_const->fund_var] - sv[0][0][0][eval_const->fund_var]));
		}
		else
		{
			next_fund_DI = 1.0;
		...
		if (n2>0)
		{
			next_fund_DJ = exp (- eval_const->next_fund_gam * (sv[0][1][0][eval_const->fund_var] - sv[0][0][0][eval_const->fund_var]));
		}
		else
		{
			next_fund_DJ = 1.0;
		...
		if (n3>0)
		{
			next_fund_DK = exp (- eval_const->next_fund_gam  * (sv[0][0][1][eval_const->fund_var] - sv[0][0][0][eval_const->fund_var]));
		}
		else
		{
			next_fund_DK = 1.0;
	...
	/*	Final notional */
	{
		...
		if (n1>0)
		{
			fin_not_disc_DI = exp (- eval_const->fin_not_disc_gam * (sv[1][0][0][0] - sv[0][0][0][0]));
		}
		else
		{
			fin_not_disc_DI = 1.0;
		...
		if (n2>0)
		{
			fin_not_disc_DJ = exp (- eval_const->fin_not_disc_gam * (sv[0][1][0][0] - sv[0][0][0][0]));
		}
		else
		{
			fin_not_disc_DJ = 1.0;
		...
		if (n3>0)
		{
			fin_not_disc_DK = exp (- eval_const->fin_not_disc_gam  * (sv[0][0][1][0] - sv[0][0][0][0]));
		}
		else
		{
			fin_not_disc_DK = 1.0;
		...
		if (n1>0)
		{
			fin_not_log_fwd_DI = -	eval_const->fin_not_fwd_for_gam * (sv[1][0][0][1] - sv[0][0][0][1])
							  + eval_const->fin_not_fwd_dom_gam * (sv[1][0][0][0] - sv[0][0][0][0]);
		}
		else
		{
		...
		if (n2>0)
		{
			fin_not_log_fwd_DJ = -	eval_const->fin_not_fwd_for_gam * (sv[0][1][0][1] - sv[0][0][0][1])
							  + eval_const->fin_not_fwd_dom_gam * (sv[0][1][0][0] - sv[0][0][0][0]);
		}
		else
		{
		...
		if (n3>0)
		{
			fin_not_log_fwd_DK = -	eval_const->fin_not_fwd_for_gam * (sv[0][0][1][1] - sv[0][0][0][1])
							  +	eval_const->fin_not_fwd_dom_gam * (sv[0][0][1][0] - sv[0][0][0][0]);
		}
		else
		{
		...
		if (n1>0)
		{
			fin_not_fwd_DI = exp (-	eval_const->fin_not_fwd_for_gam * (sv[1][0][0][1] - sv[0][0][0][1])
								  + eval_const->fin_not_fwd_dom_gam * (sv[1][0][0][0] - sv[0][0][0][0]));
		}
		else
		{
		...
		if (n2>0)
		{
			fin_not_fwd_DJ = exp (-	eval_const->fin_not_fwd_for_gam * (sv[0][1][0][1] - sv[0][0][0][1])
								  + eval_const->fin_not_fwd_dom_gam * (sv[0][1][0][0] - sv[0][0][0][0]));
		}
		else
		{
		...
		if (n3>0)
		{
			fin_not_fwd_DK = exp (-	eval_const->fin_not_fwd_for_gam * (sv[0][0][1][1] - sv[0][0][0][1])
								  +	eval_const->fin_not_fwd_dom_gam * (sv[0][0][1][0] - sv[0][0][0][0]));
		}
		else
		{
	...
	{		
		...
		if (n1>0)
		{
			eval_const->fund_DI[l] = exp (- eval_const->fund_gam[l] * (sv[1][0][0][eval_const->fund_var] - sv[0][0][0][eval_const->fund_var]));
		}
		else
		{
		...
		if (n2>0)
		{
			eval_const->fund_DJ[l] = exp (- eval_const->fund_gam[l] * (sv[0][1][0][eval_const->fund_var] - sv[0][0][0][eval_const->fund_var]));
		}
		else
		{
		...
		if (n3>0)
		{
			eval_const->fund_DK[l] = exp (- eval_const->fund_gam[l] * (sv[0][0][1][eval_const->fund_var] - sv[0][0][0][eval_const->fund_var]));
		}
		else
		{
	...
	for (l=0; l<num_pd_cpn; l++)
	{
		...
		if (n1>0)
		{
			eval_const->pd_disc_DI[l] = exp (- eval_const->pd_disc_gam[l] * (sv[1][0][0][0] - sv[0][0][0][0]));
		}
		else
		{
		...
		if (n2>0)
		{
			eval_const->pd_disc_DJ[l] = exp (- eval_const->pd_disc_gam[l] * (sv[0][1][0][0] - sv[0][0][0][0]));
		}
		else
		{
		...
		if (n3>0)
		{
			eval_const->pd_disc_DK[l] = exp (- eval_const->pd_disc_gam[l] * (sv[0][0][1][0] - sv[0][0][0][0]));
		}
		else
		{
		...
		/* for the log fwd */		
		eval_const->pd_log_fwd_I[l] = log(eval_const->pd_fwd_dff_ratio[l])
		...
		if (n1>0)
		{
			eval_const->pd_log_fwd_DI[l] = - eval_const->pd_fwd_for_gam[l] * (sv[1][0][0][1] - sv[0][0][0][1])
										+ eval_const->pd_fwd_dom_gam[l] * (sv[1][0][0][0] - sv[0][0][0][0]);
		}
		else
		{
		...
		if (n2>0)
		{
			eval_const->pd_log_fwd_DJ[l] = - eval_const->pd_fwd_for_gam[l] * (sv[0][1][0][1] - sv[0][0][0][1])
										+ eval_const->pd_fwd_dom_gam[l] * (sv[0][1][0][0] - sv[0][0][0][0]);
		}
		else
		{
		...
		if (n3>0)
		{
			eval_const->pd_log_fwd_DK[l] = - eval_const->pd_fwd_for_gam[l] * (sv[0][0][1][1] - sv[0][0][0][1])
										+ eval_const->pd_fwd_dom_gam[l] * (sv[0][0][1][0] - sv[0][0][0][0]);
		}
		else
		{
	...
	for (i=0; i<n1; i++)
	{
		...
		{			
			...
			for (k=0; k<n3; k++)
			{
				...
				//X = eval_const->pd_X_const + eval_const->pd_X_dom * svijk[0] + eval_const->pd_X_for * svijk[1] + svijk[2];
				X = X_K;

				/*fx = exp (eval_const->pd_S_const + X * (eval_const->pd_S_lin + X * eval_const->pd_S_quad)

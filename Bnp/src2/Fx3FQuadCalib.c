/* ==========================================================================
   ...
   LGM 1 factor on the domestic and foreign market and a lognormal model on the
   Spot Fx
...
Err FxQuad_log_approx(long today, double *maturity, long nb_mat,
  ...
  for (i = 0; i < last_index; i++) {
    ...
    F = logSpot +
        (swp_f_zr(today, (long)(today + T * 365.0 + 1.0E-08), dom_yc) -
  ...
  for (i = 0; i < nb_time_fx; i++) {
    ...
    if (t > 0) {
      ...
      fx_vol[i] =
          alpha * stdev * y + (1.0 - y) * (beta + gamma / stdev * (1.0 - y));
    } else {
    }
  }
...

{
  ...
  err = FxQuad_log_approx(today, maturity, nbMat, sig_curve_dom, sig_curve_for,
                          mat_fx, nb_mat_fx, sig_curve_fx, alpha, beta, gamma,
                          sig0, lda_dom, lda_for, corr_dom_for, corr_dom_fx,
                          corr_for_fx, spot_fx, dom_yc, for_yc, fx_time,
                          nb_fx_time2, fx_fwd, fx_vol_curve, fx_fwd, disc_dt);
  if (err) {
    goto FREE_RETURN;
  }
...
Err Fx3DQuadImpliedVol(char *fx_underlying, double beta, double gamma,
  ...
  err = Fx3DQuadtsImpliedVol(
      ...
      alpha2, beta2, gamma2, sig0, spot_fx, correl_dom_for, correl_dom_fx,
      correl_for_fx, dom_yc, for_yc, vol, disc_dt, fx_dt);
...
Err Fx3DQuadtsCalibration(
    ...
    double *param, double spot_fx, double corr_dom_for, double corr_dom_fx,
    double corr_for_fx, char *dom_yc, char *for_yc, double **fx_vol_curve,
...

{
  ...
  err = Fx3DLocaltsCalibration(
      ...
      sig_curve_dom, lda_dom, sig_curve_for, lda_for, vol_ln_func, param,
      spot_fx, corr_dom_for, corr_dom_fx, corr_for_fx, dom_yc, for_yc,
  ...
  for (i = nbrShort; i < nbrOpt; i++) {
    ...
    err = OptBlkSch(Fwd, Fwd, vol_opt[i], exercise_opt[i], df, "CALL",
                    "PREMIUM", &price_tgt);
    ...
    while (j < nbNewton && (fabs(error) / price_tgt > MAX_ERROR2)) {
      err = Fx3DQuadtsTreeFxOptions(
          ...
          param[0], param[1], param[2], vol_opt[i], spot_fx, corr_dom_for,
          corr_dom_fx, corr_for_fx, dom_yc, for_yc, vol_opt, exercise_opt,
          nbrOpt, &(price1[0]), nbSteps);
      ...
      if (fabs(error) / price_tgt > MAX_ERROR2) {
        ...
        err = Fx3DQuadtsTreeFxOptions(
            today, exercise_date, strikes, 1, maturity, nbrMat, sig_curve_dom,
            lda_dom, sig_curve_for, lda_for, exercise_opt, nbrOpt,
            *fx_vol_curve, param[0], param[1], param[2], vol_opt[i], spot_fx,
            corr_dom_for, corr_dom_fx, corr_for_fx, dom_yc, for_yc, vol_opt,
            exercise_opt, nbrOpt, &(price2[0]), nbSteps);
...

{
  ...
  spot_fx = spot_fx / swp_f_df(today, spot_date, for_yc) *
  ...
  err = Fx3DQuadtsCalibration(
      ...
      nb_merge_dates, sigma_dom, lda_dom, sigma_for, lda_for,
      vol_lnATM_func_Quad, &(Param[0]), spot_fx, correl_dom_for, correl_dom_fx,
      correl_for_fx, dom_yc, for_yc, fx_vol_curve, nbSteps, nbNewton, disc_dt,
      fx_dt, nbIterMax);
...
Err Fx3DQuadtsTreeFxOptions(
  ...
  double *dom_ifr = NULL, *dom_fwd = NULL, *dom_var = NULL, *for_ifr = NULL,
         *for_fwd = NULL, *for_var = NULL, *fx_fwd = NULL, *fx_fwd0 = NULL,
         *fx_var = NULL;

  double *sig_fx_approx = NULL, *fx_fwd_approx = NULL;
  ...
  if (!vol_change || !dom_vol || !for_vol || !fx_vol || !is_bar || !bar_lvl ||
      !bar_cl) {
  ...
  fx_fwd0[0] = spot_fx;
  for (i = 1; i < nstp; i++) {
    fx_fwd0[i] = spot_fx / swp_f_df(today, date[i], dom_yc) *
  ...
  err = fill_local_model(model_func, model_param, nstp, time, alpha, beta,
                         gamma, sig0, fx_fwd0);
  if (err) {
    goto FREE_RETURN;
  }
  ...
  err = FxLocal_log_approx(today, time, nstp, dom_vol, for_vol, time, nstp,
                           fx_vol, vol_lnATM_func_Quad, param, dom_lam, for_lam,
                           corr_dom_for, corr_dom_fx, corr_for_fx, spot_fx,
                           dom_yc, for_yc, time, nstp, fx_fwd_approx,
                           sig_fx_approx, fx_fwd0, MAX_TIME);
  ...
  err = treeQuad_main_3dfx(
      nstp, time, date, vol_change, dom_vol, for_vol, fx_vol, dom_ifr, dom_fwd,
      dom_var, for_ifr, for_fwd, for_var, fx_fwd, fx_fwd0, fx_var, func_parm,
      has_evt, bar_lvl, bar_cl, is_bar, dom_lam, for_lam, corr_dom_for,
      corr_dom_fx, corr_for_fx, spot_fx, dom_yc, for_yc, model_func,
      model_param, FxCall_payoff_4_3dfxQuad_tree, nbrOpt, 1, option_prices);

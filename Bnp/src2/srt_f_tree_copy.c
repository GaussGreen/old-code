...
{
Err err = 0;
 if ( tminf == NULL ) { (*tminf_dup) = NULL; return err; }
...
{
Err err = 0;
 if ( trinf == NULL ) { (*trinf_dup) = NULL; return err; }
...
{
SrtStpPtr new_stp; 
 if (input_source == NULL ) return input_dest;
 else
	{
	...
	if ( input_dest != NULL ) input_dest->next = new_stp;

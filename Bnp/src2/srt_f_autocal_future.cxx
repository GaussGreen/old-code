...
Err srt_f_price_future_lgm(long today, long *fut_dates, long num_fut_dates,
                           char *yc_name, char *ref_rate_code,
                           char *calibrated_underlying, int result_type,
                           double *fut_price) {
  Err err = NULL;
  int i;
  ...
  err = swp_f_get_ref_rate_details(ref_rate_code, &float_basis,
                                   &float_compounding);
  if (err)
    goto e1;
  ...
  for (i = 0; i < num_fut_dates; i++) {
    err = srt_f_lgmcashfuture(fut_dates[i], end_futdates[i], float_basis,
                              und_name, &fut_price[i]);
    if (err)
      goto e1;
    ...
    if (result_type != 0) {
      ...
      fwd = (df_start / df_end - 1.0) /
            coverage(fut_dates[i], end_futdates[i], float_basis);
      fwd += spread;
...
Err srt_f_autocal_future_input_instr(
    long *fut_dates, long num_fut_dates, long ninstruments, char **type_str,
    long *start_dates, long *end_dates, char **compd_str, char **basis_str,
    char **recpay_str, char **refratecode, char *mdl_name, char *yc_name,
    char *volcurvename, char *ref_rate_code, double tau, double alpha,
    double gamma, double rho, double *fut_price, double *instr_price,
    int bumpinst) {
  Err err = NULL;
  int i;
  Date *end_futdates = NULL;
  ...
  for (i = 0; i < ninstruments; i++) {
    if (strcmp(type_str[i], "SWAPTION") == 0) {
      err = swp_f_ForwardRate(start_dates[i], end_dates[i], compd_str[i],
                              basis_str[i], yc_name, refratecode[i], &fwd);

      err = swp_f_vol(volcurvename, start_dates[i], end_dates[i], fwd, &vol,
                      &lognorm);
      ...
      err =
          swp_f_Swaption(start_dates[i], end_dates[i], compd_str[i],
                         basis_str[i], vol, fwd, recpay_str[i], refratecode[i],
                         yc_name, "PREMIUM", "LOGNORMAL", &(price[i]));

    } else {
      err = srt_BGM_FwdRate(start_dates[i], end_dates[i], refratecode[i],
                            yc_name, 1, &fwd);
      ...
      fixingdate =
          add_unit(start_dates[i], -spotlag, SRT_BDAY, MODIFIED_SUCCEEDING);

      mat = coverage(today, fixingdate, BASIS_ACT_365);

      if (strcmp(recpay_str[i], "PAY") == 0) {
        price[i] =
            coverage(start_dates[i], end_dates[i], basis[i]) *
            swp_f_df(today, end_dates[i], yc_name) *
            srt_f_optblksch(fwd, fwd, vol, mat, 1, SRT_CALL, SRT_PREMIUM);
      } else {
        price[i] = coverage(start_dates[i], end_dates[i], basis[i]) *
                   swp_f_df(today, end_dates[i], yc_name) *
                   srt_f_optblksch(fwd, fwd, vol, mat, 1, SRT_PUT, SRT_PREMIUM);
      }
  ...
  err = SrtInitIRUnd(und_name, yc_name, mdl_name, volCrvRows, volCrvCols,
                     volCrvVals, tauCrvRows, tauCrvCols, tauCrvVals, 0.0, alpha,
                     gamma, rho, 0.0, 0.0, 0.0, 0, 0, NULL);
  ...
  err =
      SrtBootstrap(numgrfnparams, grfnparams_str, grfnvalue_str,
                   (int *)&ninstruments, start_dates, end_dates, compd_str,
                   basis_str, strike, bondstrike, type_str, recpay_str,
                   refratecode, price, price, tau, alpha, gamma, rho, und_name);
  if (err) {
    goto e1;
  }

  /* -------------------------------------------------------------------------------------
     The Pricing of futures according to the model (closed form for LGM        ,
     tableau for Chey)
     -------------------------------------------------------------------------------------
   */
  if (mdl_typ == LGM) {
    for (i = 0; i < num_fut_dates; i++) {
      if (err = srt_f_lgmcashfuture(fut_dates[i], end_futdates[i], basis[i],
                                    und_name, &fut_price[i])) {
        goto e1;
      }
      spread = swp_f_spread(fut_dates[i], end_futdates[i], ref_rate_code);
    ...
    for (i = 0; i < num_fut_dates; i++) {
      if (err = srt_f_cheycashfuture(fut_dates[i], end_futdates[i], "MM",
                                     und_name, numpaths, &fut_price[i])) {
        goto e1;
      }
      spread = swp_f_spread(fut_dates[i], end_futdates[i], ref_rate_code);
      fut_price[i] -= spread;
...
e1:
  if (grfnparams_str)
    free_svector_size(grfnparams_str, 0, numgrfnparams - 1, 32);
  if (grfnvalue_str)
    free_svector_size(grfnvalue_str, 0, numgrfnparams - 1, 32);
  if (end_futdates)
    free(end_futdates);
  if (basis)
    free(basis);
  if (price)
    free_dvector(price, 0, ninstruments - 1);
  if (strike)
    free_dvector(strike, 0, ninstruments - 1);
  if (bondstrike)
    free_dvector(bondstrike, 0, ninstruments - 1);
...
Err srt_f_autocal_future(
    long *fut_dates, long num_fut_dates,
    Err (*GetVol)(double dStart, double dEnd, double dStrike, double dForward,
                  double dSpread, double *pdBsVol),
    /* function that returns the volatility of the reference swptns */
    String logNormStr, char *mdl_name, char *yc_name, char *ref_rate_code,
    double tau, double alpha, double gamma, double rho, long numpaths,
    double *fut_price) {
  Err err = NULL;
  int i;
  Date *end_dates = NULL;
  ...
  if ((!end_dates) || (!compd_str) || (!basis_str) || (!basis) ||
      (!compounding) || (!strike) || (!bondstrike) || (!type_str) ||
      (!recpay_str) || (!price) || (!refratecode)) {
    err = "Memory Allocation Failure";
    goto e1;
  }
  ...
  for (i = 0; i < num_fut_dates; i++) {
    ...
    err = swp_f_CapFloor(fut_dates[i], end_dates[i], strike[i], GetVol,
                         recpay_str[i], refratecode[i], yc_name, "PREMIUM",
                         logNormStr, &price[i]);
    if (err)
      goto e1;
  ...
  /* Initialise underlying */
  err =
      SrtInitIRUnd(und_name, yc_name, mdl_name, volCrvRows, volCrvCols,
                   volCrvVals, tauCrvRows, tauCrvCols, tauCrvVals, 0.0, alpha,
                   gamma, rho, 0.0, 0.0, 0.0, 0, 0, NULL); /* vasicek params */
  ...
  /* Call to the calibration routine: bootstrap */
  err = SrtBootstrap(
      numgrfnparams, grfnparams_str, grfnvalue_str, (int *)&num_fut_dates,
      fut_dates, end_dates, compd_str, basis_str, strike, bondstrike, type_str,
      recpay_str, refratecode, price, price, tau, alpha, gamma, rho, und_name);
  if (err) {
    goto e1;
  }
  ...
  if (mdl_typ == LGM) {
    for (i = 0; i < num_fut_dates; i++) {
      if (err = srt_f_lgmcashfuture(fut_dates[i], end_dates[i], basis[i],
                                    und_name, &fut_price[i])) {
        goto e1;
      }
    ...
    for (i = 0; i < num_fut_dates; i++) {
      if (err = srt_f_cheycashfuture(fut_dates[i], end_dates[i], basis_str[i],
                                     und_name, numpaths, &fut_price[i])) {
        goto e1;
      }
      spread = swp_f_spread(fut_dates[i], end_dates[i], ref_rate_code);
...
e1:
  ...
  if (end_dates)
    free(end_dates);
  if (compd_str)
    free_svector_size(compd_str, 0, num_fut_dates - 1, 3);
  if (basis_str)
    free_svector_size(basis_str, 0, num_fut_dates - 1, 8);
  if (basis)
    free(basis);
  if (strike)
    free_dvector(strike, 0, num_fut_dates - 1);
  if (bondstrike)
    free_dvector(bondstrike, 0, num_fut_dates - 1);
  if (type_str)
    free_svector_size(type_str, 0, num_fut_dates - 1, 9);
  if (recpay_str)
    free_svector_size(recpay_str, 0, num_fut_dates - 1, 4);
  if (price)
    free_dvector(price, 0, num_fut_dates - 1);
...
/* in a LGM model */
Err srt_f_lgmcashfuture(Ddate start_date, Ddate end_date, SrtBasisCode basis,
                        char *und_name, double *future_price) {
  SrtErr err;
  SrtUndPtr und;
  TermStruct *ts;
  SrtMdlType mdltype;
  ...
  fixing_date =
      (Ddate)add_unit((Date)start_date, -spot_lag, SRT_BDAY, SUCCEEDING);
  ...
  if (mdldim == ONE_FAC) {
    ...
    zc_exp = (YT[1].phi_coeff[0][0] - YT[0].phi_coeff[0][0]) * phi[0][0] +
             (YT[1].x_coeff[0] - YT[0].x_coeff[0]) * H;

    zc_var = phi[0][0] * (YT[1].x_coeff[0] - YT[0].x_coeff[0]) *
             (YT[1].x_coeff[0] - YT[0].x_coeff[0]);
  } else if (mdldim == TWO_FAC) {
    err = get_2f_H_funcs(fixing_time, ts, &Two_fac_H);

    zc_exp = (YT[1].phi_coeff[0][0] - YT[0].phi_coeff[0][0]) * phi[0][0] +
             (YT[1].phi_coeff[1][1] - YT[0].phi_coeff[1][1]) * phi[1][1] +
             (YT[1].phi_coeff[0][1] - YT[0].phi_coeff[0][1]) * phi[0][1] +
             (YT[1].phi_coeff[1][0] - YT[0].phi_coeff[1][0]) * phi[1][0] +
             (YT[1].x_coeff[0] - YT[0].x_coeff[0]) *
    ...
    zc_var = phi[0][0] * (YT[1].x_coeff[0] - YT[0].x_coeff[0]) *
                 (YT[1].x_coeff[0] - YT[0].x_coeff[0]) +
             phi[1][1] * (YT[1].x_coeff[1] - YT[0].x_coeff[1]) *
                 (YT[1].x_coeff[1] - YT[0].x_coeff[1]) +
             phi[0][1] * (YT[1].x_coeff[0] - YT[0].x_coeff[0]) *
                 (YT[1].x_coeff[1] - YT[0].x_coeff[1]) +
             phi[1][0] * (YT[1].x_coeff[1] - YT[0].x_coeff[1]) *
                 (YT[1].x_coeff[0] - YT[0].x_coeff[0]);
  }
...
SrtErr srt_f_cheycashfuture(Ddate start_date, Ddate end_date, char *basis,
                            char *und_name, long numpaths,
                            double *future_price) {
  int i;
  SrtErr err = NULL;
  SrtUndPtr und;
  ...
  sprintf(tableaustrings[0][0],
          "FRA(%.0f        ,%.0f        ,\"%s\")|hist(\"%s\")", start_date,
          end_date, basis, histo_name);
  tableaumask[0][0] = GRFNSCELL;

  /* Call to GRFN */
  err = SrtGrfnMain(und_name, numgrfnparams, grfnparams_str, grfnvalue_str,
                    numeventdates, eventdates, tableaurows, tableaucols,
                    tableaustrings, tableaumask, 0, NULL, NULL, &price, &stdev,
                    NULL, NULL);
  ...
  err = srt_f_makehistogram((char *)histo_name, 0, "", 0, &histo_x, &x_size,
                            &histo_y, &y_size, &histo_values);
  if (err)
    goto e4;
...
e3:
  if (tableaustrings)
    free_smatrix_size(tableaustrings, 0, 0, 0, 0, GRFN_DEF_ARGBUFSZ);
  if (tableaumask)
    free_imatrix(tableaumask, 0, 0, 0, 0);

e2:
  if (eventdates)
    free_lngvector(eventdates, 0, numeventdates - 1);

...
/*	The Approximation: 
	X(T) = dConst + dCoef1_1 * U + dCoef2_1 * V + dCoef2_2 * V^2 + dCross * U * V
	U and V follow independant normal distributions */
...
double	dSqTime;
	...
	/* Outputs */
	*dConst = -0.5 * dVar0 * dSigma * dSigma * dTime
			+ 0.25 * dSigma * dSigma * (dMeanLevel - dLambda * dVar0) * dTime * dTime
			- 0.25 * dRho * dAlpha * dSigma * dTime;

	*dCoef1_1 = dSigma * (dSigma0 + 
				1.0 / dSigma0 * ((dMeanLevel - dLambda * dVar0) / 4.0 - dAlpha * dAlpha / 32.0) * dTime
				) * dSqTime;

	*dCoef2_1 = -dSigma * dAlpha * (
				dSigma * dSigma0 / 4.0 + dRho * dAlpha / dSigma0 / 32.0) * dTime * dSqTime;	
...
double	*dInitGauss	= NULL,
	...
	err	= copula_sv_get_polynomial_coefs(dTime,
									dSigma,									
									dAlpha,
									dRho,
									dVar0,
...
Err	copula_sv_from_sabr_to_heston(	double				dMaturity,
									double				dFwd,
									double				dSigma,
									double				dAlpha,
									double				dBeta,
...
double	copula_sv_get_SABR_get_cumul(	double	dMaturity,
										double	dForward,
										double	dStrike,
										double	dSigmaBeta,
										double	dAlpha,
...
double	price1, price2, res;
	...
	price1 = srt_f_optblkschbetastochquick( dForward,
											dStrike - dShift,
											dMaturity,
											dSigmaBeta,
											dAlpha,
	...
	price2 = srt_f_optblkschbetastochquick( dForward,
											dStrike + dShift,
											dMaturity,
											dSigmaBeta,
											dAlpha,
...
Err	copula_sv_get_SABR_cumulative(	double	dMaturity,
									double	dForward,
									double	dSigmaBeta,
									double	dAlpha,
									double	dBeta,
...
double	*dFirstGuessPoints	= NULL,
		*dFirstGuessCumul	= NULL;
...
int		i;
	...
	for (i=1; i<lFirstGuessNbPoints; i++)
	{
		...
		dFirstGuessCumul[i] = copula_sv_get_SABR_get_cumul(	dMaturity,
															dForward,
															dFirstGuessPoints[i],
															dSigmaBeta,
															dAlpha,
	...
	if (i >= 0)
	{
		...
		dNextCumul  = copula_sv_get_SABR_get_cumul(	dMaturity,
													dForward,
													dFirstGuessPoints[i+1],
													dSigmaBeta,
													0.0,
													dBeta,
													0.0,
		...
		for (i=i; i>=0; i--)
		{
			...
			dCurrentCumul  = copula_sv_get_SABR_get_cumul(	dMaturity,
														dForward,
														dFirstGuessPoints[i],
														dSigmaBeta,
														0.0,
			...
			dFirstGuessCumul[i] = dFirstGuessCumul[i+1] * dCurrentCumul / dNextCumul;
	...
	if (iAdaptGrid && !iUsePoints)
	{
		...
		dProbaStep = (dFirstGuessCumul[lFirstGuessNbPoints-1] - dFirstGuessCumul[0]) / (lNumPoints - 1.0);

		for (i=0; i<lNumPoints; i++)
		{
			dPoints[i] = interp(dFirstGuessCumul,
								dFirstGuessPoints,
								lFirstGuessNbPoints,
								dProba,
								0,
		...
		/* recall */
		err = copula_sv_get_SABR_cumulative(dMaturity,
											dForward,
											dSigmaBeta,
											dAlpha,
											dBeta,
...
FREE_RETURN:
	...
	if (iAdaptGrid)
	{
		if (dFirstGuessPoints) free(dFirstGuessPoints);
		if (dFirstGuessCumul) free(dFirstGuessCumul);
	}
...
Err copula_sv_basket_SABR (	int					iNbFwd,	
							double				*dFwds,
							double				*dWeights,
							int					iNbStrikes,
							double				*dStrike,
...
FILE	*stream;	
	...
	for (j=0; j<iNbFwd; j++)
	{
		...
		if (sParams->iForceLogBeta)
		{
			...
			err = transform_sabr_beta(	dMaturity,
										dFwds[j],
										dSigmaBeta,
										dAlpha[j],
										dBeta[j],
		...
		if (sParams->iUseSABR)
		{
			err = copula_gaussian_get_SABR_cumulative(	dMaturity,
														dFwds[j],
														dSigmaBeta,
														dNewAlpha,
														dNewBeta,
		...
		else
		{
			...
			err = BMMCalibOnSabrStates(	dFwds[j],
										dMaturity,
										dSigmaLog,
										dNewBeta,
										dNewAlpha,
			...
			err = copula_gaussian_get_BMM_linterp_cumulative(dMaturity,
															dFwds[j],
															dForward1,
															dSigmaBeta1,
															dForward2,
	...
	for (i=0; i<iNbFwd; i++)
	{				
		err = copula_sv_from_sabr_to_heston(	dMaturity,
												dFwds[i],
												dVols[i],
												dAlpha[i],
												dBeta[i],
	...
	if (sParams->iCalcImpliedRho && iNbStrikes >= 5)
	{
		...
		{	
			...
			dPremium[1] += dResult[i][0] * dResult[i][0] / (i - sParams->iIntegSkipFirst + 1.0);
			dPremium[2] += dResult[i][1] / (i - sParams->iIntegSkipFirst + 1.0);
			dPremium[3] += dResult[i][1] * dResult[i][1] / (i - sParams->iIntegSkipFirst + 1.0);
			dPremium[4] += dResult[i][0] * dResult[i][1] / (i - sParams->iIntegSkipFirst + 1.0);
		}
	}
	else
	{		
		for (i=sParams->iIntegSkipFirst; i<sParams->lNumPaths; i++)
		{
			...
			for (j=0; j<iNbStrikes; j++)
			{
				...
				dPremium[j] += (dCashFlow - dPremium[j]) / (i - sParams->iIntegSkipFirst + 1.0);
			}
		}
	}
...
FREE_RETURN:
	...
	if (dResult) free_dmatrix(dResult, 0, sParams->lNumPaths - 1, 0, iNbFwd - 1);
	...
	if (dCumulative)
	{
		for (i=0; i<iNbFwd; i++)
		{
			if (dCumulative[i]) free(dCumulative[i]);
		}
		
		free(dCumulative);
	}

	if (lNbPoints) free(lNbPoints);

	if (dShiftSV) free(dShiftSV);
	if (dSigmaSV) free(dSigmaSV);
	if (dAlphaSV) free(dAlphaSV);
	if (dRhoSV) free(dRhoSV);
	if (dVar0SV) free(dVar0SV);
...
Err		err = NULL;
	...
	if (!GaussSample || !XtSample || !dSqCorrMatrix || !dBrownian || !dSortMatrix
		|| !dConst || !dCoef1_1 || !dCoef2_1 || !dCross)
	{
	...
	switch(sParams->eMCType) 
	{
		...
		{		
			...
			for (j=0; j<iNbFwd; j++)
			{
				XtSample[j][i] = dConst[j] + dCoef1_1[j] * dBrownian[j] + 
		...
		for (j=0; j<iNbFwd; j++) 
		{
			...
			num_f_sort_matrix(	sParams->lNumPaths,
								2,
								1,
								dSortMatrix);
			...
			for (i=0; i<sParams->lNumPaths; i++)
			{																					   	
				dResult[(long) (dSortMatrix[0][i])][j] = interp_columns(	ya, 
																			xa, 
																			lNbPoints[j], 
																			IntermRes, 
																			0, 
	...
	{
		err = copula_sv_get_implied_linear_correl(	iNbFwd,
													0,
													1,
													dCoef1_1,
													dCoef2_1,
...
FREE_RETURN:
	...
	if (GaussSample) free_dmatrix(GaussSample, 0, sParams->lNumPaths, 0, eff_d-1);
	if (XtSample) free_dmatrix(XtSample, 0, iNbFwd-1, 0, sParams->lNumPaths);
	if (dSqCorrMatrix) free_dmatrix(dSqCorrMatrix, 0, eff_d-1, 0, eff_d-1);
	if (dBrownian) free(dBrownian);
	if (dSortMatrix) free_dmatrix(dSortMatrix, 0, 1, 0, sParams->lNumPaths);

	if (dConst) free(dConst);
	if (dCoef1_1) free(dCoef1_1);
	if (dCoef2_1) free(dCoef2_1);
	if (dCross) free(dCross);	
...
Err		err = NULL;
	...
	switch(sParams->eMCType) 
	{
		...
		{		
			...
			for (j=0; j<iNbFwd; j++)
			{
				XtSample[j][i] = dCoef1_1[j] * dBrownian[j] + 
								dBrownian[j+iNbFwd] * (dCoef2_1[j] + dCross[j] * dBrownian[j]);
			}
		}
...
Err copula_sv_GearedOption_SABR (	int					nFwds,	
									double				*dFwds,
									double				*dWeights,
									double				dMargin,
									int					iNbStrikes,
									...
									double				*dPremium)
{
	...
	double  dApproxStdev, dCoeffAdjustStrike, dCoeff, dAdjStrike, dTemp1, dTemp2, dGearFwd, dBasket;
	...
	if (!pdFwdsAtMaturity || !dAvg || !dShiftSV || !dSigmaSV || !dAlphaSV || 
		!dRhoSV || !dVar0SV || !dLambdaSV || !ppdPoints || !ppdPoints || !ppdCumProbaValue || !dCorrCube)
	{
	...
	for (i=0; i<nFwds; i++)
	{
		...
		if (sParams->iUseOldGridMethod)
		{
			...
			n_StdDev = 5.0 * dSqMat;
			ppdPoints[i][0] = dFwds[i] * exp(-0.5 * dApproxStdev * dApproxStdev - n_StdDev * dApproxStdev);
			ppdPoints[i][2*nNumPoints] = dFwds[i] * exp(- 0.5 * dApproxStdev * dApproxStdev + n_StdDev * dApproxStdev);

			ppdCumProbaValue[i][0] = 0.0;

			for (j=1; j<=2*nNumPoints; j++)
			{
				ppdPoints[i][j] = dFwds[i] * 
					exp(- 0.5 * dApproxStdev * dApproxStdev + (j-nNumPoints) * 4.0 / nNumPoints  * dApproxStdev);
				
				// Compute the Cum Proba associated to this points P(S < pdPoints) 			
				dAdjStrike = ppdPoints[i][j] * dCoeffAdjustStrike;

				dTemp1 = srt_f_optblkschbetastochquick( dFwds[i],
														dAdjStrike,
														dMaturity,
														dSigmaBeta,
														dAlpha[i],
				...
				dTemp2 = srt_f_optblkschbetastochquick( dFwds[i],
														dAdjStrike,
														dMaturity,
														dSigmaBeta,
														dAlpha[i],
				...
				ppdCumProbaValue[i][j] = 1.0 + 
					(dTemp1 - dTemp2) * dCoeff / ppdPoints[i][j];
			}		
		...
		//// then use a lower vovol
			...
			switch (kfail)
			{
				...
				for (k=1; k<=kfail+1; k++)
				{
					...
					ppdCumProbaValue[i][k] = 1.0 + 
						(dTemp1 - dTemp2) * dCoeff / ppdPoints[i][k];
				}
...
FREE_RETURN:
	...
	if (pdFwdsAtMaturity) free(pdFwdsAtMaturity);
	if (dAvg) free(dAvg);
	...
	if (ppdPoints) free_dmatrix(ppdPoints, 0, nFwds-1, 0, 2*nNumPoints);
	if (ppdCumProbaValue) free_dmatrix(ppdCumProbaValue, 0, nFwds-1, 0, 2*nNumPoints);	
	if (dCorrCube) free_dmatrix(dCorrCube, 0, sParams->lNumPaths, 0, nFwds-1);

	return err;
}

/* Monte Carlo to check results */
Err copula_sv_basket_SABR_MC (	int					nFwds,	
								double				*dFwds,
								double				*dWeights,
								int					iNbStrikes,
								double				*dStrike,
								...
								double				*dPremium)
{
	...
	if (!dSqCorrMatrix || !dXtSample || !dIndBrownian || !dCorBrownian || !dCoefVol || !dStartVol || !dCoefVoVol)
	{
	...
	for (i=0; i<sParams->lNumPaths; i++)
	{
		...
		for (j=0; j<lNumSteps; j++)
		{
			...
			for (k=0; k<nFwds; k++)
			{
				dXtSample[k] = max(dXtSample[k] + dXtSample[nFwds+k] * exp(dBeta[k] * log(dXtSample[k])) * sqdt * dCorBrownian[k], 0.0001);
				dXtSample[nFwds+k] = max(dXtSample[nFwds+k] * (1.0 + dCoefVoVol[k] * dCorBrownian[nFwds+k]), 0.0001);
			}
		}
		...
		{
			dPremium[k] += DMAX(cp * (dBasket - dStrike[k]), 0.0) / sParams->lNumPaths;
		}
	} 
	
FREE_RETURN:

	if (dCorrMatrix) free_dmatrix(dCorrMatrix, 0, eff_d-1, 0, eff_d-1);
	if (dSqCorrMatrix) free_dmatrix(dSqCorrMatrix, 0, eff_d-1, 0, eff_d-1);
	if (dXtSample) free(dXtSample);
	if (dIndBrownian) free(dIndBrownian);
	if (dCorBrownian) free(dCorBrownian);
	if (dCoefVol) free(dCoefVol);
	if (dStartVol) free(dStartVol);
	if (dCoefVoVol) free(dCoefVoVol);
	
	return err;
}

Err copula_sv_quadrant_dependence_SABR (int					nFwds,	
										double				*dFwds,
										double				*dWeights,
										int					iNbFract,
										double				*dFract,
...
Err copula_student_quadrant_dependence(	int					iNbFwd,
										double				*dWeights,
										int					iNbFract,
										double				*dFract,
										double				**dCorrMatrix,
...
FREE_RETURN:
	...
	if (GaussSample) free_dmatrix(GaussSample, 0, sParams->lNumPaths, 0, total_d-1);
	if (XtSample) free_dmatrix(XtSample, 0, eff_d-1, 0, sParams->lNumPaths);
	if (dSqCorrMatrix) free_dmatrix(dSqCorrMatrix, 0, eff_d-1, 0, eff_d-1);
	if (dBrownian) free(dBrownian);
...
Err copula_sv_get_implied_linear_correl(int		iNbFwd,
										int		iIndex1,
										int		iIndex2,
										double	*dCoef1,
										double	*dCoef2,
...
Err	err = NULL;
	...
	dVar1 = pow(dCoef1[iIndex1] * dCholesky[3][0] + dCoef2[iIndex1] * dCholesky[1][0], 2.0)
		  + pow(dCoef1[iIndex1] * dCholesky[3][1] + dCoef2[iIndex1] * dCholesky[1][1], 2.0)
	...
	dVar2 = pow(dCoef1[iIndex2] * dCholesky[2][0] + dCoef2[iIndex2] * dCholesky[0][0], 2.0)
	...
	dCovar = (dCoef1[iIndex1] * dCholesky[3][0] + dCoef2[iIndex1] * dCholesky[1][0])
		   * (dCoef1[iIndex2] * dCholesky[2][0] + dCoef2[iIndex2] * dCholesky[0][0])
		   + (dCoef1[iIndex1] * dCholesky[3][1] + dCoef2[iIndex1] * dCholesky[1][1])
		   * (dCoef1[iIndex2] * dCholesky[2][1])
		   + (dCoef1[iIndex1] * dCholesky[3][2]) * (dCoef1[iIndex2] * dCholesky[2][2])
		   + 3.0 * dCross[iIndex1] * dCholesky[3][0] * dCholesky[1][0]
		   * dCross[iIndex2] * dCholesky[2][0] * dCholesky[0][0];

	if (dVar1 < 0.0 || dVar2 < 0.0)
	{
		err = "Correlation Matrix not definite positive in copula_sv_get_implied_linear_correl";
		goto FREE_RETURN;
	}
...
FREE_RETURN:
	...
	if (dNewCorrel) free_dmatrix(dNewCorrel, 0, 3, 0, 3);
	if (dCholesky) free_dmatrix(dCholesky, 0, 3, 0, 3);

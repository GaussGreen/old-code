            ...
            if (iim > 1)
                for (;;)
                {
                    ...
                    for (i = 1; i <= p->n; i++)
                    {
                        for (k = 0; k <= p->n; k++)
                            yp[m_idx + k] +=
                                coef * y[mjj_idx + i] * y[cmjj_idx + i] * p->D[i - 1][i - 1][k];
    ...
    SPolynomial* poly)
{
    ...
    const double RKtol = 1e-5, thres_min = 1e-8;  //, thres_coef = 1e-7;		//
                                                  //adjustable
    ...
    */
    ...
    for (iim = 1; iim <= affi.M; iim++)  // order of the moment
    {
        ...
        for (;;)
        {
            ...
            coef = y[m_idx] +
                   y[m_idx +
                     2];  // Heston specific	(later: force all factors initial values = 0)
            for (k = 0; k < affi.n; k++)
                coef /= fact[ii[k]];
    ...
    for (ii[0] = 1; ii[0] <= order; ii[0]++)
    {
        ...
        if (poly.pool[hii] >= 0)
        {
            err = Poly_AddConstMono(
                &pp,
                (ii[0] % 4 >= 2 ? -1.0 : 1.0) * poly.coef[poly.pool[hii]][0],
                NULL,
    ...
    double* stddev)
{
    ...
    for (ii[0] = 1; ii[0] <= order; ii[0]++)
    {
        ...
        err = Poly_AddConstMonoPoly(
            &a[0], (ii[0] % 2 ? -1.0 : 1.0) * dd.coef[dd.pool[hii]][0], NULL, NULL, &herm[hii1]);
        if (err)
            goto FREE_RETURN;
    }

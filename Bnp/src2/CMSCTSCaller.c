...
clock_t	time1, time2;
	...
	/* Fill the structures */
	err = cmscts_fill_check_all_struct(	sDeal,
										sMarket,
	...
	err = cmscts_calc_mkt_value(sDeal,
								sMarket,
								sPricingParams,
								sCalibration);
	...
	if (sDeal->sExotic->sAux->iNbUsedNumCpn > 0)
	{
		...
		err = cmscts_calibrate_model(	sMarket,
										sDeal,
										sSimulArg,
										sCalibration,
										sOutputs);
		...
		err = cmscts_calculate_option(	sMarket,
										sDeal,
										sSimulArg,
										sPricingParams,
										sOutputs);
		...
		{
			err = cmscts_calculate_switch_adjust(	sMarket,
													sDeal,
													sSimulArg,
													sCalibration,
													sPricingParams,
...
Err	cmscts_allocate_outputs(CMSCTS_OUTPUTS	sOutputs)
{
	...
	if (!sOutputs->sInstDatas || !sOutputs->sLGMSVModel || !sOutputs->sFwdIVInfos || !sOutputs->sSwitchInstDatas)
	{
...
int				iIncludePartialPV;	
	...
	if (sDeal->sExotic->sAux->iNbUsedNumCpn > 0)
	{
		...
		if (sOutputs->sLGMSVModel)
		{
			err = init_LGMSV_model(	sOutputs->sLGMSVModel,
									sModel->lToday,
									sModel->iOne2F,
									sModel->iNbPWTime,
									sModel->dLambdaX,
			...
			ConvertTS_LGMSV_to_LGM(	sOutputs->sLGMSVModel->iNbPWTime,
		...
		if (sCallAux->iNbUsedCall > 0 && sFwdIVInfos)
		{
			...
			err = cnscts_allocate_fwdiv(sFwdIVInfos,
										sCallAux->iNbUsedCall);
			...
			for (i=0; i<sFwdIVInfos->lNbCall; i++)
			{
				...
				if (sMCArg->sMCEBParams->dOneTimeCall) sFwdIVInfos->dOTCall[i] = sMCArg->sMCEBParams->dOneTimeCall[iIndexEvt];
				if (sMCArg->sMCEBParams->dOneTimePartial) sFwdIVInfos->dOTCallPartial[i] = sMCArg->sMCEBParams->dOneTimePartial[iIndexEvt];
		...
		else if (sKOAux->iNbUsedKO > 0 && sFwdIVInfos)
		{
			...
			{
				for (i=sKOAux->iStartFundIdx[iIndexStart]; i<sKOAux->iStartFundIdx[iIndexStart+1]; i++)
				{
					sFwdIVInfos->dMarketIV[0] -= sDeal->sFunding->sAux->dMarketValue[i] * sDeal->dPayRec;
					sFwdIVInfos->dModelIV[0] -= sDeal->sFunding->sAux->dMarketValue[i] * sDeal->dPayRec;
					sFwdIVInfos->dNewModelIV[0] -= sDeal->sFunding->sAux->dMarketValue[i] * sDeal->dPayRec;
				}
...
Err	cnscts_allocate_fwdiv(	CMSCTS_FWDIV	sFwdIVInfos,
							long			lNbCall)
{
	...
	if (!sFwdIVInfos->lExeDates || !sFwdIVInfos->dMarketIV || !sFwdIVInfos->dModelIV  || !sFwdIVInfos->dNewModelIV || !sFwdIVInfos->dFees  || 
...
{
	if (sFwdIVInfos->lExeDates) free(sFwdIVInfos->lExeDates);
	if (sFwdIVInfos->dMarketIV) free(sFwdIVInfos->dMarketIV);
	if (sFwdIVInfos->dModelIV) free(sFwdIVInfos->dModelIV);
	if (sFwdIVInfos->dNewModelIV) free(sFwdIVInfos->dNewModelIV);
	if (sFwdIVInfos->dFees) free(sFwdIVInfos->dFees);
	if (sFwdIVInfos->dOTCall) free(sFwdIVInfos->dOTCall);
	if (sFwdIVInfos->dOTCallPartial) free(sFwdIVInfos->dOTCallPartial);

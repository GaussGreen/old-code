 ...
 * ******************************************************************/
/*
 *       Exponential decay function: ExpDecay (a        ,t) = (1 - exp (at)) /
 * (a*t)
 */
...
{
  double *SwapYield, /* Array of swap yields at 6mo interval        , both
                  benchmark and interpolated ones */
  ...
  //	term_data->ValueDate = Nxtbusday (      term_data->Today        , /*
  //Value date is today shifted by (Spotdays) business days */
  // term_data->Spotdays        , term_data->Holidays        ,
  ...
  spotrate(term_data->Zero +
           ...
           SwapYield, /* = Zero[4] which is the 6m zero        , the first used
                   for bootstrapping.        */
...
void yldconv(double *Zero,    /* output: zeros at money market points */
             ...
             double *MMYield, /* Money Market rates (O/N        , 1        ,2
                                 ,3        ,6        ,12 m) */
             ...
             int MMB)         /* Money Market basis (360 or 365) */
{
  ...
  for (i = 1; i < 6; i++) /* 1        ,2        ,3        ,6        ,12 month
                             Money Market rates */
  {
          ...
          TREE_DATA *tree_data) /* Output: Structure of tree data */
{
  ...
  NbDate = NbExer + NbFwd + NbCoupon +
           1; /* The dates are: the value date        , the
  ...
  if ((EoA == 'A') &&
      (NbExer > 1)) /* For American option        , we interpolate the
...
void Tree_Alloc(TREE_DATA *tree_data, int NbNodes) {
  ...
  tree_data->FwdRate =
      mbs_dvector(0, NbNodes + 1); /* forward and        , consequently        ,
                                      an extra zero coupon price. */
                 ...
                 PREPAY_DATA *prepay_data) /* Structure of prepayment data */
{
  ...
  if (err = Forward_Vol(tree_data->FwdVol, /* Calculation of forward rate
                                        volatility at each node in the tree */
                        &(tree_data->Jump), tree_data->FwdRate,
                        tree_data->Length, term_data->Zero, term_data->Period,
                        term_data->SwaptionVol, term_data->SwaptionExp,
                        term_data->Beta, term_data->IndexMat,
    ...
    int NbNodes)         /* Total number of nodes (or time steps) */
{
  ...
  *Jump = MaxSpotVol *
          sqrt(JUMPCOEFF /
               (double)Ppy); /* The jump size depends on maximum
    ...
    int NbNodes)        /* Total number of nodes (or time steps) */
{
  ...
  do {
    ...
    Bottom = (int)max(0., NbPer - *NodeMax); /* If NbPer>NodeMax the tree will
                                          be cut at the top and the bottom */
         ...
         i += Top - Bottom) /* 2 cases: Top and Bottom of the tree */
    {
       ...
       * Top+1        , Top) and the bottom */
      /* node branches up (i.e. to bottom+1        , bottom+2        ,
       * bottom+3). */
      ...
      if (Top == 0) /* For NbPer=0        , Top=Bottom=0 i.e. there is only one
                       node. */
        break;
    } /* for i */
  ...
  } while (NbPer <= NbNodes - 1); /* We need Drift[NbNodes-1] => We need
                                     FwdRate[NbNodes] , FwdVol[NbNodes-1] and */
  /* Discount[NbNodes+1].                        */
  Drift[NbPer] = 0.;
    ...
    int NbNodes)       /* Total number of nodes (or time steps) */
{
  ...
  *NodeMax2 = 10000; /* 10000 = Big! */
  ...
  while (
      (!CutFlag) &&
      (NbPer < NbNodes)) /* bottom of the tree are not too big or too small. */
  {
    Vol =
...
void Print_Term(TERM_DATA *term_data, TREE_DATA *tree_data) {
  ...
  for (i = 0, days = 0., discount = 1., ZeroRate = 0.; i <= tree_data->NbNodes;
    ...
    Forward =
        ...
        (pow(1. +
                 tree_data->FwdRate
                     [i], /* Conversion: 1.+FwdRate[i]=(1.+Forward/Freq)^(Length[i]*Freq/365.)
                           */
    ...
    TREE_DATA *tree_data) /* Structure of tree data */
{
  ...
  Bottom =
      (int)max(0., NbPer - tree_data->NodeMax); /* If NbPer>NodeMax the tree is
...
void Zero_Calc_new(
    ...
    // Total number of zeros needed to calculate the par yield index */
    // int Reset        , /* Reset the zero price to 1 if TRUE */
    int NbPer, /* Current time period */
    //			int             TotPer        ,                                 /*
    //Total number of period in the rate tree including the N additional periods
    // */
               ...
               TREE_DATA *tree_data) /* Structure of tree data */
{
  ...
  if (Reset) /* If Reset = TRUE        , we permute the zeros. */
  {
    ...
    { /* TotNbZero + 1)        , then we need to keep an extra zero: the one
         which address */

...
void _transform_vector(
    ...
    unsigned    nSize_Out,  // size of element (in bytes) returned by the unary functor
...
//-------------------------------------------------------------------------------------------------------
//	Description: transform the matrix pOut by calling the binary function pfnBI
//
//	Returns : if(n1R)
//						pOut[n_RBegin+i][nC_Begin+j]=pfnBI(pIn1[i],pIn2[j],pComm)
//					else
...
void _transform_matrix(
    ...
    unsigned    nSize_Out,  // size of elements (in bytes) returned by the binary functor
...
void __stdcall _MULTD_PFN_BI(
    const double* pIn1, const double* pIn2, const void* pComm, double* pProduct)
{
    *pProduct = (*pIn1) * (*pIn2);
    // not referenced
    pComm;
...
const void* lower_bound(
    const void*        pvKey,      // pointer to key
...
const void* find_min_greater_than(
    const void*        pvKey,    // pointer to key
    const void*        pvBegin,  // pointer to base element
    unsigned           nElems,   // # elements
    unsigned           nSize,    // size of elements (in bytes)
    PFN_COMPARE_BINARY pComp)
{
    ...
    if (nElems > 0 && nSize == sizeof(double))
    {
        ...
        _ASSERTE(
            pvResult == (char*)pvBegin + nElems * nSize
                ? *(double*)((char*)pvBegin + (nElems - 1) * nSize) <= *(double*)pvKey
                : 1);
...
)
{
    ...
    unsigned n =
        ((char*)find_min_greater_than(pc, pcBegin, nElems, ncSize, dless_v) - (char*)pcBegin) /
        ncSize;
...
)
{
    ...
    const char *pcin = (const char*)pcInBegin, *pc = (const char*)pcBegin,
               *pv = (const char*)pvBegin;
    char* pvout    = (char*)pvOut;
    ...
    for (; pcin < pcInEnd;
         pcin += ncSize, pc += n * ncSize, pv += n * nvSize, pvout += nvSize, nElems -= n)
...
const void* _reverse(
    void*    pvBegin,  // pointer to base element
    unsigned nElems,   // # elements
    unsigned nSize     // size of elements (in bytes)
)
{
    ...
    /* for (; (char*)pvBegin <= (char*)pvEnd - nSize; (char*)pvBegin += nSize, (char*)pvEnd -=
       nSize) memswap(pvBegin, pvEnd, nSize);*/
    return preturn;
}

#ifdef _DEBUG
int _non_descending_(
    const void* pvBegin,  // pointer to base element
    unsigned    nElems,   // # elements
    unsigned    nSize     // size of elements (in bytes)
)
{
    ...
    for (; pv < (const char*)pvEnd; pv += nSize)
    {
        ...
        const double dR =
            nSize == sizeof(double) ? *((double*)(pv - nSize)) : *((int*)(pv - nSize));
        if (dL < dR)
            return 0;
    }
    return 1;
...
int __stdcall dless_v(const void* pdLeft, const void* pdRight)
{
    return dless((const double*)pdLeft, (const double*)pdRight);
}
...
int __stdcall lless_v(const void* pdLeft, const void* pdRight)
{
    return lless((const long*)pdLeft, (const long*)pdRight);
}
...
void** _alloca_matrix(
    void** pm, void* pdata, int nR_Begin, int nR_End, int nC_Begin, int nC_End, unsigned nSize)
{
    const int   nSize_R = nR_End - nR_Begin;
    const int   nSize_C = nC_End - nC_Begin;
    void **     p = pm, **pm_end = pm + nSize_R;
    ...
    void*       pvOut)
{
    ...
    memcpy(pvOut, pv1Begin, n1Elems * nvSize);
    memcpy((char*)pvOut + nvSize * n1Elems, pv2Begin, n2Elems * nvSize);
    memcpy((char*)pvOut + nvSize * n1Elems + nvSize * n2Elems, pv3Begin, n3Elems * nvSize);
...
void _bmatch_ex(
    ...
    void*       pInvOut)
{
    const char *pInc = (const char*)pcInBegin, *pIncEnd = (char*)pcInBegin + nInElems * ncSize;
    char*       pInv = (char*)pInvOut;
    ...
    {
        const char* p = (const char*)bsearch(pInc, pcBegin, nElems, ncSize, _dcompare);
...
void _bmatch(
    ...
    void*       psubvOut)
{
    const char *psubc = (const char*)psubcBegin, *psubcEnd = (char*)psubcBegin + nsubElems * ncSize;
    const char* pc    = (const char*)pcBegin;
    const char* pv    = (const char*)pvBegin;
    char*       psubv = (char*)psubvOut;
    int         n     = 0;
    ...
    for (; psubc < psubcEnd;
         psubc += ncSize, psubv += nvSize, pc += n * ncSize, pv += n * nvSize, nElems -= n)
    {
    ...
    int     nPivot_Begin)
{
    ...
    for (nI = 0; nI < nMatrixSize - 1; ++nI)  // NB: pdSubDiagL is one element less than nSize
    {
...
const char* _fill_swapcashrate_diag(
    const char* szYC,
    ...
    double*     pdSwapCashRate_Begin)
{
    ...
    for (; plStart_Begin < plStart_End; ++pdSwapCashRate_Begin, ++plStart_Begin)
    {
        ...
        *pdSwapCashRate_Begin = swp_f_swapcashrate(
            *plStart_Begin, lEndDate, eBasis, eFreq, (char*)szYC, (char*)szRefRate);
    }
...
const long* _fill_date(
    long          lBase,
    const double* pdTenor_Begin,
    ...
    long* plDate_Begin)
{
...
#ifdef _DEBUG
    {
        ...
        double*   pdTenorBack = (double*)malloc(nSize * sizeof(double));
...
const double* _multiply_vector2(
    const double* pdIn_Begin, const double* pdIn_End, double dConstant, double* pdOut_Begin)
{
...
const double* _multiply_vector(
    const double* pdIn1_Begin,
    const double* pdIn1_End,
    const double* pdIn2_Begin,
    double*       pdOut_Begin)
{
    int       nI    = 0;
...
const char* _alloc_dmatrix(
    const double*** pppd_Begin, int nR_Begin, int nR_End, int nC_Begin, int nC_End)
{
    *pppd_Begin = 0;
    *pppd_Begin = (const double**)dmatrix(nR_Begin, nR_End - 1, nC_Begin, nC_End - 1);
...
double _NormDensity(double dOne_Over_Sqr2Pi, double dX, double dMu, double dVar)
{
    if (dVar == 0.)
        return 1.;
    else
...
double _Sum_dMatrix(const double** ppdMatrix, int nR_Begin, int nR_End, int nC_Begin, int nC_End)
{
    double dResult = 0.;
    int    nR, nC;
...
void _print_matrix(const double** ppdMatrix, int nR_Begin, int nR_End, int nC_Begin, int nC_End)
{
    int nR, nC;
    _RPT0(_CRT_WARN, "\n Matrix \n");
    for (nR = nR_Begin; nR < nR_End; ++nR)
...
const char* Decode_RefRate(const char* szRefRate, char** pszFeq, char** pszBasis)
{
    const char*    szErr = 0;

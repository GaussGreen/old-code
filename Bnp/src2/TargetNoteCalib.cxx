 ...
 */
/*
        Calibrate to caplets.  If the lambda value is not input        ,
   calibrate to diagonal swaptions.  The LGM2F parameters must be input.
*/
/* --------------------------------------------------------------------------------
 */
...
void TARN_calib_setInst(
    ...
    int iStrikeType, /* 0: ATM        ,  1: CASH        ,  2: SWAP        ,  3:
                        STD        ,  4:  KO */
...
char *TARN_calib_LGM2F(
  ...
  if (!strcmp(szLGM2FUnd, "CAL")) {
    ...
    if (err = cpd_calib_diagonal_dlm(
            ...
            tarn->deal.lvFltrFixingDates, calib_aux->Primary.cxxal_date,
            ...
            tarn->deal.lvFltrFixingDates, calib_aux->Secondary.cxxal_date,
...
char *TARN_calib_LGMSV(
  ...
  if (!strcmp(szLGMSVUnd, "CAL")) {
    ...
    if (err = cpd_calib_diagonal_LGMSV_new_dlm(
            ...
            &cal.num_sig, &cal.sig_time, &cal.sig, &cal.cxxalalpha,
            &cal.cxxallambdaeps, &cal.cxxalrho, &cal.cxxalrho2,
    ...
    if (err = irm_sv_get_term_struct_date(
            ...
            &cal.rho, &cal.num_smile, &cal.smile_dates, &cal.cxxalalpha,
            &cal.cxxallambdaeps, &cal.cxxalrho, &cal.cxxalrho2, &cal.tstar))
      goto FREE_RETURN;
  }
...
char *TARN_calc_KO_prob_2F(TARN_Struct *tarn, TARN_AUX *aux,
  ...
  /* Variable declarations */
  char *err = 0;
  //	TARN_CALIB_AUX CALIB_AUX_TEMP        , *calib_aux_temp =
  //&CALIB_AUX_TEMP;
  ...
  /* set up the calibraton parameters */
  //	if ( err = TARN_set_calib_param( tarn        , aux        ,
  //calib_aux_temp )  ) 		goto FREE_RETURN;
  ...
  for (i = 0; i < tarn->deal.nCouponDates; i++) {
    ...
    calib_aux->Secondary.cxxal_date[i] = 0;
  ...
  for (i = tarn->deal.nCouponDates - 1; i >= 0 && dCumulProb < 0.9; i--) {
    ...
    calib_aux->Secondary.cxxal_date[lvIndex[i]] = 1;
  }

...
static void _df_constant_(double dLAM1_0_t, double dLAM2_0_t, double dLAM1_0_T,
                          double dLAM2_0_T, double dEILam1_t, double dEILam2_t,
                          double dPHI1_t, double dPHI2_t, double dPHICross_t,
                          double dExp_R1, double dExp_R2, double dRho,
                          // results
                          double *pdConstant, double *pdCoeff1,
                          double *pdCoeff2) {
  /// LAMBDA
  ...
  *pdConstant = -dHalfBondVar - *pdCoeff1 * dExp_R1 - *pdCoeff2 * dExp_R2;
  ...
  _ASSERTE(
      ...
      (dPHI1_t == 0. && dPHI2_t == 0. &&
       dPHICross_t == 0.) || // initial point at today
      (dPHI1_t > 0. && dPHI2_t == 0. &&
...
void _df_constant(double dDF_0_t, double dDF_0_T, double dLAM1_0_t,
  ...
  if (dDF_0_t == dDF_0_T) {
    ...
    *pdCoeff2 = 0.;
    ...
    _df_constant_(dLAM1_0_t, dLAM2_0_t, dLAM1_0_T, dLAM2_0_T, dEILam1_t,
                  dEILam2_t, dPHI1_t, dPHI2_t, dPHICross_t, dExp_R1, dExp_R2,
                  dRho, pdConstant, pdCoeff1, pdCoeff2);
  }
}

static double __DF_nojump_(double dX1, double dX3, double dDF_0_t,
                           double dDF_0_T, double dLAM1_0_t, double dLAM2_0_t,
                           double dLAM1_0_T, double dLAM2_0_T, double dEILam1_t,
                           double dEILam2_t, double dPHI1_t, double dPHI2_t,
                           double dPHICross_t, const _PCQ_Seq *pNumeraire,
                           double dRho, double dAlphaRho,
                           double dAlpha_Sqrt1mRhoSqrd) {
  /// LAMBDA
  const double dLAM1_t_Nume =
      (*pNumeraire->m_pdLAMZero1_Begin - dLAM1_0_t) * dEILam1_t;
  const double dLAM2_t_Nume =
      (*pNumeraire->m_pdLAMZero2_Begin - dLAM2_0_t) * dEILam2_t;
  const double dLAM1_t_T = (dLAM1_0_T - dLAM1_0_t) * dEILam1_t;
  const double dLAM2_t_T = (dLAM2_0_T - dLAM2_0_t) * dEILam2_t;
  ...
  // Exponent
  const double dX2 = dX3 * dAlpha_Sqrt1mRhoSqrd +
                     dAlphaRho * dX1; // back out X2 from X1 and X3
...
static double __DF_(double dX1, double dX3, double dDF_0_t, double dDF_0_T,
  ...
  _ASSERTE(dLAM1_t_T > 0 && dLAM2_t_T > 0.);
  _ASSERTE((dPHI1_t > 0. && dPHI2_t > 0. && dPHICross_t > 0.) ||
...
double _DF_nojump_(double dX1, double dX3, double dDF_0_t, double dDF_0_T,
  ...
  if (dDF_0_t == dDF_0_T)
    return 1.;
  return __DF_nojump_(dX1, dX3, dDF_0_t, dDF_0_T, dLAM1_0_t, dLAM2_0_t,
                      dLAM1_0_T, dLAM2_0_T, dEILam1_t, dEILam2_t, dPHI1_t,
                      dPHI2_t, dPHICross_t, pNumeraire, dRho, dAlphaRho,
...
double _DF_(double dX1, double dX3, double dDF_0_t, double dDF_0_T,
  ...
  if (dDF_0_t == dDF_0_T)
    return 1.;
  return __DF_(dX1, dX3, dDF_0_t, dDF_0_T, dLAM1_0_t, dLAM2_0_t, dLAM1_0_T,
               dLAM2_0_T, dEILam1_t, dEILam2_t, dPHI1_t, dPHI2_t, dPHICross_t,
               dExp_R1, dExp_R2, dRho, dAlphaRho, dAlpha_Sqrt1mRhoSqrd);
}

//-------------------------------------------------------------------------------------------------------
//	Description: Local helper for _fill_ExpR
//
//  Returns:  the expected value of R(t) under the measure induced by ZeroBond(t
//  ,T)
//-------------------------------------------------------------------------------------------------------
static double _ExpR(double dLAM_0_t, double dLAM_0_T, double dAuxLAM_0_t,
                    double dAuxLAM_0_T, double dEILam1_t, double dEIAuxLam1_t,
                    double dPHI_t, double dPHICross_t, double dRho) {
...
//	Description:
//
//  Returns:  the expected value of R1  ,2(t) under the measure induced by
//  ZeroBond(t  ,T)
//-------------------------------------------------------------------------------------------------------
void _fill_ExpR(const double *pdLAM1_0_T_Begin, const double *pdLAM1_0_T_End,
  ...
  for (; pdLAM1_0_T_Begin < pdLAM1_0_T_End;
       ++pdLAM1_0_T_Begin, ++pdLAM2_0_T_Begin, ++pdEILam1_T_Begin,
       ++pdEILam2_T_Begin, ++pdPHI1_T_Begin, ++pdPHI2_T_Begin,
       ++pdPHICross_T_Begin, ++pdExp_R1_Begin, ++pdExp_R2_Begin) {
    ...
    const double dExp_R1Tm1 =
        _ExpR(dLAM1_0_Tm1, dLAM1_0_NumMAt, dLAM2_0_Tm1, dLAM2_0_NumMAt,
              dEILam1_Tm1, dEILam2_Tm1, dPHI1_Tm1, dPHICross_Tm1, dRho);

    /// expected R2(T-1) under the measure induced by ZeroBond(t  ,T)
    /// or used-passed in numeraire
    const double dExp_R2Tm1 =
        _ExpR(dLAM2_0_Tm1, dLAM2_0_NumMAt, dLAM1_0_Tm1, dLAM1_0_NumMAt,
              dEILam2_Tm1, dEILam1_Tm1, dPHI2_Tm1, dPHICross_Tm1, dRho);

    /// expected R1(T) under the measure induced by ZeroBond(t  ,T)
    /// or used-passed in numeraire
    const double dExp_R1T =
        _ExpR(*pdLAM1_0_T_Begin, dLAM1_0_NumMAt, *pdLAM2_0_T_Begin,
              dLAM2_0_NumMAt, *pdEILam1_T_Begin, *pdEILam2_T_Begin,
              *pdPHI1_T_Begin, *pdPHICross_T_Begin, dRho);

    /// expected R2(T) under the measure induced by ZeroBond(t  ,T)
    /// or used-passed in numeraire
    const double dExp_R2T =
        _ExpR(*pdLAM2_0_T_Begin, dLAM2_0_NumMAt, *pdLAM1_0_T_Begin,
              dLAM1_0_NumMAt, *pdEILam2_T_Begin, *pdEILam1_T_Begin,
              *pdPHI2_T_Begin, *pdPHICross_T_Begin, dRho);
...
void _df_singleslice(
  ...
  const double dDeltaX1 =
      (pdX1_End - pdX1_Begin == 1) ? 0. : pdX1_Begin[1] - pdX1_Begin[0];
  const double dDeltaX3 =
      (pdX3_End - pdX3_Begin == 1) ? 0. : pdX3_Begin[1] - pdX3_Begin[0];
  ...
  const double dX2_Base =
      dX3_Base * dAlpha_Sqrt1mRhoSqrd + *pdX1_Begin * dAlphaRho;
  ...
  _df_constant(dDF_0_t, dDF_0_T, dLAM1_0_t, dLAM2_0_t, dLAM1_0_T, dLAM2_0_T,
               dExpIntglam1_t, dExpIntglam2_t, dPHI1_t, dPHI2_t, dPHICross_t,
               dExp_R1, dExp_R2, dRho, &dConstant, &dCoeff1, &dCoeff2);

  {
    const double dExpX1 =
        exp(-dDeltaX1 * dCoeff1 - dDeltaX1_AlphaRho * dCoeff2);
    const double dExpX3 = exp(-dDeltaX3_Alpha_Sqrt1mRhoSqrd * dCoeff2);
    double dDF_Base =
        exp(dConstant - (*pdX1_Begin) * dCoeff1 - dX2_Base * dCoeff2) *
        dBondRatio;

    for (nX1 = nC_Begin; nX1 < nC_End;
         ++nX1, dDF_Base *= dExpX1) /// X1 - cols !
    {

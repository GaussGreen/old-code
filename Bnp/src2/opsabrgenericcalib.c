...
Err		err = NULL;
	...
	if (Forward	< 0.0)					{err = "Fatal: Forward should be positive";			goto FREE_RETURN;}
	if (Sigma < 0.0)					{err = "Fatal: SigmaBeta should be positive";		goto FREE_RETURN;}
	if (Alpha	< 0.0)					{err = "Fatal: Alpha should be positive";			goto FREE_RETURN;}
	...
	// range check
	if (vol_parameters_check_from_SrtDiffusionType( a,  b,  c, 30,  volLoc) == 0){err = "Error: local volatility parameters out of range";goto FREE_RETURN;}
	...
	if (input != SRT_BETAVOL)	// From LOG to SigmaBeta
		SigmaBeta = op_sabrgen_calib(Forward, Forward, Maturity, Sigma, Alpha, a, b, c, Rho, vol_loc);
...
double					*Rho_sens)
{
	...
	if (err = op_sabrgen_pricing (	forward,
									strike,
									maturity,
									disc,
									call_put,
...
double					*sens)
{
	...
	*sens = fac * vega1;
	...
	if (vega_volga == 1)
	{
		if (err = op_sabrgen_vega_volga (	forward,
											strike,
											maturity,
											disc,
											call_put,
					  ...
					  int				nbr_paramSABR)   //useless ????????????????????
{
	...
	{
		err=op_sabrgen_pricing(	forward,
								strike,
								maturity,
								1,
								SRT_CALL,
		...
		err=op_sabrgen_model_sens(	forward,
									strike,
									maturity,
									1,
									SRT_CALL,
		...
		err=op_sabrgen_vega_volga(	forward,
									strike,
									maturity,
									1,
									SRT_CALL,
					...
					double				*fitting_error)
{
	...
	e04jbc (nb_param, objfun, bound, lbounds, ubounds, InitGuess, &objf,g, &  options, &nagcomm, &fail);

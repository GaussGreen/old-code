    ...
    SrtGreekType   greek)
{
    ...
    {
        answer = srt_f_optblknrm(fwd_price, strike, vol, mat, disc, SRT_CALL, greek) +
    ...
    else
    {
        ...
        case THETA: /*** THETA ***/
            answer = -(1 / 365.00) * (0.5 * disc * vol / sqrt(mat) * gd1 + disc * log(disc) / mat);
            break;
    ...
    double* result)
{
    ...
    *result = srt_f_optblknrm(fwd_price, strike, vol, mat, disc, call_put_type, greek_type);
    ...
    SrtGreekType   greek)
{
    ...
    if (call_put == SRT_STRADDLE)
    {
        answer = srt_f_optblknrm_accurate(fwd_price, strike, vol, mat, disc, SRT_CALL, greek) +
                 srt_f_optblknrm(fwd_price, strike, vol, mat, disc, SRT_PUT, greek);
        return answer;
    }

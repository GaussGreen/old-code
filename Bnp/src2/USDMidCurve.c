    ...
    double*          out_dVol)
{
    ...
    *out_dPrice = -1.0;
    ...
    for (n = 0; n <= lIterNo; n++)
    {
        ...
        bs = srt_f_optblksch(
            dFwd_n, dCashStrike, dVol_n, dOptionExpiry, dOptionExpiryDf, enumCallPut, enumGreek);

        dPremium +=
            1 / n_f * exp(-dJumpInt * dOptionExpiry) * pow(dJumpInt * dOptionExpiry, n) * bs;
    }
...
char* PL_convertInput(struct PL_input* input, struct PL_parameters* out_p)
{
    ...
    /*	if ( err = levenberg_marquardt(
                                                                                    data,   // From
       [1] to [ndata] target, // From [1] to [ndata] weight, // From [1] to [ndata] 3, param,  //
       From [1] to [nparam] long nparam, long niter, Err (*funcs)(double, double[], double*,
       double[], int), chisq)	) return err;
    */

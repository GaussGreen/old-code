...
double compute_sum_sigma(double t, double *sigma, double *sigma_date,
  ...
  while ((sigma_date[i] < t) & (i < nb_sigma)) {
    ...
    res += sigma[i] * sigma[i] * (exp(2 * lambda * t2) - exp(2 * lambda * t1)) /
           (2 * lambda);
    i++;
  }
  t1 = t2;
  ...
  res += sigma_temp * sigma_temp *
         (exp(2 * lambda * t2) - exp(2 * lambda * t1)) / (2 * lambda);
  return res;
}
...
double compute_A(double t, double _T, double sum_sigma_1, double sum_sigma_2,
  ...
  if ((_T == t) | (t == 0)) {
    ...
    while ((sigma_date[i] < t) & (i < nb_sigma)) {
      ...
      temp1 += sigma_1[i] * sigma_1[i] *
               (exp(-2 * lambda_1 * (Tpay - t2)) -
                exp(-2 * lambda_1 * (Tpay - t1))) /
               (2 * lambda_1);
      temp2 += sigma_2[i] * sigma_2[i] *
               (exp(-2 * lambda_2 * (Tpay - t2)) -
                exp(-2 * lambda_2 * (Tpay - t1))) /
               (2 * lambda_2);
    ...
    coef =
        (exp(lambda_2 * (Tpay - _T)) - exp(lambda_2 * (Tpay - t))) / lambda_2;
    coef *=
        lambda_1 / (exp(lambda_1 * (Tpay - _T)) - exp(lambda_1 * (Tpay - t)));
    temp2 *= coef * coef * rho * rho;
    temp3 *= 2 * coef * rho;
    res = temp1 + temp2 + temp3;
    res = sqrt(res);
    res *=
        (exp(lambda_1 * (Tpay - _T)) - exp(lambda_1 * (Tpay - t))) / lambda_1;
    return res;
  }
}
...
double compute_CMS_GaussLeg(
    double Tf, double Tpay, double *DF_fix, double *fix_coupon_date,
    ...
    int nb_float_coupon, double *spread, double *sigma_1, double *sigma_2,
    double *sigma_date, int nb_sigma, double lambda_1, double lambda_2,
    double rho, int nb_integration,
    double bound, // must be positive
    SrtReceiverType ProductType, double strike) {
  ...
  // compute B(t        ,T)=B(0        ,T)/B(0        ,t)*exp(A(t ,T)+c1(t
  // ,T)*z1+c2(t        ,T)*z2) under QTp
  ...
  sum_diagterm = compute_sum_diagterm(Tf, sigma_1, sigma_2, sigma_date,
                                      nb_sigma, lambda_1, lambda_2);

  for (i = 0; i < nb_float_coupon; i++) {
    Afloat[i] = compute_A(Tf, float_coupon_date[i], sum_sigma_1, sum_sigma_2,
                          sum_diagterm, rho, lambda_1, lambda_2, Tpay);
    c1float[i] = compute_c1(Tf, float_coupon_date[i], Tpay, sigma_1, sigma_2,
                            sigma_date, nb_sigma, rho, lambda_1, lambda_2);
    c2float[i] = compute_c2(Tf, float_coupon_date[i], Tpay, sigma_2, sigma_date,
                            nb_sigma, rho, lambda_2);
  }
  for (i = 0; i < nb_fix_coupon; i++) {
    Afix[i] = compute_A(Tf, fix_coupon_date[i], sum_sigma_1, sum_sigma_2,
                        sum_diagterm, rho, lambda_1, lambda_2, Tpay);
    c1fix[i] = compute_c1(Tf, fix_coupon_date[i], Tpay, sigma_1, sigma_2,
                          sigma_date, nb_sigma, rho, lambda_1, lambda_2);
    c2fix[i] = compute_c2(Tf, fix_coupon_date[i], Tpay, sigma_2, sigma_date,
                          nb_sigma, rho, lambda_2);
  }
  ...
  for (l = 0; l < nb_integration; l++) {
    ...
    for (k = 0; k < nb_integration; k++) {
      ...
      swap_num = 0;
      swap_den = 0;
      ZC_float[0] = DF_float[0] / DF_TF *
                    exp(Afloat[0] + c1float[0] * y1 + c2float[0] * y2);
      for (i = 0; i < nb_float_coupon - 1; i++) {
        ZC_float[i + 1] =
            DF_float[i + 1] / DF_TF *
            exp(Afloat[i + 1] + c1float[i + 1] * y1 + c2float[i + 1] * y2);
        coupon_float[i] = ((double)(1) / float_cvg[i + 1] *
                               (ZC_float[i] / ZC_float[i + 1] - 1) +
                           spread[i]) *
                          float_cvg[i + 1];
      ...
      while ((i < nb_float_coupon - 1) & (j < nb_fix_coupon)) {
        if (float_coupon_date[i] > fix_coupon_date[j]) {
          ZC_fix[j] =
              DF_fix[j] / DF_TF * exp(Afix[j] + c1fix[j] * y1 + c2fix[j] * y2);
          j++;
...
double compute_CMScov_GL(
    ...
    int nb_fix_coupon1, double *DF_float1, double DF_TF1,
    double *float_coupon_date1,
    double *float_cvg1, // the same size as float_coupon_date
    int nb_float_coupon1, double *spread1, double Tf2, double Tpay2,
    double *DF_fix2, double *fix_coupon_date2,
    double *fix_cvg2, // cvg[0]=0        , the same size as fix_coupon_date
    int nb_fix_coupon2, double *DF_float2, double DF_TF2,
    double *float_coupon_date2,
    double *float_cvg2, // the same size as float_coupon_date
    int nb_float_coupon2, double *spread2, double *sigma_11, double *sigma_21,
    double *sigma_date1, int nb_sigma1, double lambda_11, double lambda_21,
    double rho1, double *sigma_12, double *sigma_22, double *sigma_date2,
    int nb_sigma2, double lambda_12, double lambda_22, double rho2,
    int nb_integration,
    double bound // must be positive
) {
  double y1, y1r, y1m, dy1, y2, y2r, y2m, dy2, s1, s2;
  ...
  // ,T)*z1+c2(t        ,T)*z2) under QTp
  sum_sigma_11 =
      compute_sum_sigma(Tf1, sigma_11, sigma_date1, nb_sigma1, lambda_11);
  sum_sigma_21 =
      compute_sum_sigma(Tf1, sigma_21, sigma_date1, nb_sigma1, lambda_21);
  sum_diagterm1 = compute_sum_diagterm(Tf1, sigma_11, sigma_21, sigma_date1,
                                       nb_sigma1, lambda_11, lambda_21);

  sum_sigma_12 =
      compute_sum_sigma(Tf2, sigma_12, sigma_date2, nb_sigma2, lambda_12);
  sum_sigma_22 =
      compute_sum_sigma(Tf2, sigma_22, sigma_date2, nb_sigma2, lambda_22);
  sum_diagterm2 = compute_sum_diagterm(Tf2, sigma_12, sigma_22, sigma_date2,
                                       nb_sigma2, lambda_12, lambda_22);

  for (i = 0; i < nb_float_coupon1; i++) {
    Afloat1[i] =
        compute_A(Tf1, float_coupon_date1[i], sum_sigma_11, sum_sigma_21,
                  sum_diagterm1, rho1, lambda_11, lambda_21, Tpay1);
    c1float1[i] =
        compute_c1(Tf1, float_coupon_date1[i], Tpay1, sigma_11, sigma_21,
                   sigma_date1, nb_sigma1, rho1, lambda_11, lambda_21);
    c2float1[i] = compute_c2(Tf1, float_coupon_date1[i], Tpay1, sigma_21,
                             sigma_date1, nb_sigma1, rho1, lambda_21);
  }
  for (i = 0; i < nb_fix_coupon1; i++) {
    Afix1[i] = compute_A(Tf1, fix_coupon_date1[i], sum_sigma_11, sum_sigma_21,
                         sum_diagterm1, rho1, lambda_11, lambda_21, Tpay1);
    c1fix1[i] = compute_c1(Tf1, fix_coupon_date1[i], Tpay1, sigma_11, sigma_21,
                           sigma_date1, nb_sigma1, rho1, lambda_11, lambda_21);
    c2fix1[i] = compute_c2(Tf1, fix_coupon_date1[i], Tpay1, sigma_21,
                           sigma_date1, nb_sigma1, rho1, lambda_21);
  }

  for (i = 0; i < nb_float_coupon2; i++) {
    Afloat2[i] =
        compute_A(Tf2, float_coupon_date2[i], sum_sigma_12, sum_sigma_22,
                  sum_diagterm2, rho2, lambda_12, lambda_22, Tpay2);
    c1float2[i] =
        compute_c1(Tf2, float_coupon_date2[i], Tpay2, sigma_12, sigma_22,
                   sigma_date2, nb_sigma2, rho2, lambda_12, lambda_22);
    c2float2[i] = compute_c2(Tf2, float_coupon_date2[i], Tpay2, sigma_22,
                             sigma_date2, nb_sigma2, rho2, lambda_22);
  }
  for (i = 0; i < nb_fix_coupon2; i++) {
    Afix2[i] = compute_A(Tf2, fix_coupon_date2[i], sum_sigma_12, sum_sigma_22,
                         sum_diagterm2, rho2, lambda_12, lambda_22, Tpay2);
    c1fix2[i] = compute_c1(Tf2, fix_coupon_date2[i], Tpay2, sigma_12, sigma_22,
                           sigma_date2, nb_sigma2, rho2, lambda_12, lambda_22);
    c2fix2[i] = compute_c2(Tf2, fix_coupon_date2[i], Tpay2, sigma_22,
                           sigma_date2, nb_sigma2, rho2, lambda_22);
  }
  ...
  for (l = 0; l < nb_integration; l++) {
    ...
    for (k = 0; k < nb_integration; k++) {
      ...
      swap_num2 = 0;
      swap_den2 = 0;
      ZC_float1[0] = DF_float1[0] / DF_TF1 *
                     exp(Afloat1[0] + c1float1[0] * y1 + c2float1[0] * y2);
      ZC_float2[0] = DF_float2[0] / DF_TF2 *
                     exp(Afloat2[0] + c1float2[0] * y1 + c2float2[0] * y2);

      for (i = 0; i < nb_float_coupon1 - 1; i++) {
        ZC_float1[i + 1] =
            DF_float1[i + 1] / DF_TF1 *
            exp(Afloat1[i + 1] + c1float1[i + 1] * y1 + c2float1[i + 1] * y2);
        coupon_float1[i] = ((double)(1) / float_cvg1[i + 1] *
                                (ZC_float1[i] / ZC_float1[i + 1] - 1) +
                            spread1[i]) *
                           float_cvg1[i + 1];
      ...
      for (i = 0; i < nb_float_coupon2 - 1; i++) {
        ...
        coupon_float2[i] = ((double)(1) / float_cvg2[i + 1] *
                                (ZC_float2[i] / ZC_float2[i + 1] - 1) +
                            spread2[i]) *
                           float_cvg2[i + 1];
        swap_num2 += coupon_float2[i] * ZC_float2[i + 1];
      }
      for (j = 0; j < nb_fix_coupon1; j++) {
        ZC_fix1[j] = DF_fix1[j] / DF_TF1 *
                     exp(Afix1[j] + c1fix1[j] * y1 + c2fix1[j] * y2);
        swap_den1 += fix_cvg1[j] * ZC_fix1[j];
      }
      for (j = 0; j < nb_fix_coupon2; j++) {
        ZC_fix2[j] = DF_fix2[j] / DF_TF2 *
                     exp(Afix2[j] + c1fix2[j] * y1 + c2fix2[j] * y2);
...
Err compute_CMScorrel_GL(
  ...
  (*output)[0] = (cov - cms1 * cms2) /
                 (sqrt(varcms1 - cms1 * cms1) * sqrt(varcms2 - cms2 * cms2));
...
Err LGM2FCMS(char *yc_name, char *vol_curve_name, SrtUndPtr und, double alpha,
  ...
  if (do_calib == 1) {
    ...
    err = cpd_calib_diagonal_dlm(
        //	Market
        yc_name, vol_curve_name, GetCpdAutocalCashVol,
        // Get vol ref
        refRateName,
        // Long Instruments //
        fixFreq, fixBasis, refRateName, 1, primary_ex_date1, nb_UNO,
        end_tenor_primary1, (long)(lEndDate), primary_strike1, &paraml1,
        // Short Instruments //
        secundary_freq, secundary_basis, refRateName, 1, secundary_ex_date,
        nb_UNO, end_tenor_secundary, (long)(lTpay), secundary_strike, NULL,
        &params1,
        //	Model
        fix_lambda, one_f_equi, nlam, lam_time1, lam1, lam_shift1, 2, alpha,
        gamma, rho,
        //	Shift Parameters
        0, 0, 0, 0, 0, 0,
        //	Output //
        &sigma_n, &sigma_date, &sigma,
        //	Parameters //
        &lm_params1,
  ...
  *price = compute_CMS_GaussLeg(
      Tf, Tp, DF_fix, fix_coupon_date,
      fix_cvg, // cvg[0]=0        , the same size as fix_coupon_date
      nb_fix_coupon, DF_float, DF_Tf, float_coupon_date,
      float_cvg, // the same size as float_coupon_date
      nb_float_coupon, float_spreads, sigma_1, sigma_2, sigma_date, sigma_n,
      lambda_1, lambda_2, rho, nb_integration, bound, ProductType, strike);
...
// 1-CMS(Tp)-CMS(Ts) / 2-CMS(Tp) / 3-CMS(Ts)
{
  ...
  err = LGM2FCMS(yc_name, vol_curve_name, und, alpha, gamma, rho, do_calib,
                 lTfixing, lStartDate, lEndDate, fixFreq, fixBasis, refRateName,
                 lTpay, lToday, nb_integration, bound, ProductType_loc, strike,
                 GetCpdAutocalCashVol, &price);
  (*output)[1] = price;
  err = LGM2FCMS(yc_name, vol_curve_name, und, alpha, gamma, rho, do_calib,
                 lTfixing, lStartDate, lEndDate, fixFreq, fixBasis, refRateName,
                 lStartDate, // instead of lTpay
                 lToday, nb_integration, bound, ProductType_loc, strike,
                 GetCpdAutocalCashVol, &price);
  (*output)[2] = price;
...
Err LGM2FCMSCorrel(
    char *yc_name, char *vol_curve_name, SrtUndPtr und, double alpha,
    double gamma, double rho, int do_calib, long lTfixing, long lStartDate,
    long lEndDate1, char *fixFreq1, char *fixBasis1, char *refRateName,
    long lTpay, long lToday, long lEndDate2, char *fixFreq2, char *fixBasis2,
    int nb_integration, double bound,
    char *(GetCpdAutocalCashVol)(char *lpszVolCurveName, double dStart,
                                 double dEnd, double dStrike, int bIsACap,
                                 char *lpszRefRateCode, double *pdVol,
                                 double *pdPower),
    double **output) // dimension=7: OUTPUT:1-Correl / 2-CMS1 / 3-CMS2 /
                     // 4-VAR(CMS1) / 5-VAR(CMS2) / 6-sigma1 / 7-lambda1
{
  Err err = NULL;
  int sigma_n;
  double *sigma_date = NULL;
  ...
  if (do_calib == 1) {
    ...
    err = cpd_calib_diagonal_dlm(
        ...
        refRateName,
        // Long Instruments //
        fixFreq1, fixBasis1, refRateName, 1, primary_ex_date1, nb_UNO,
        end_tenor_primary1, (long)(lEndDate1), primary_strike1, &paraml1,
        // Short Instruments //
        fixFreq2, fixBasis2, refRateName, 1, secundary_ex_date, nb_UNO,
        end_tenor_secundary, (long)(lEndDate2), secundary_strike, NULL,
        &params1,
        //	Model
        fix_lambda, one_f_equi, nlam, lam_time1, lam1, lam_shift1, 2, alpha,
        gamma, rho,
  ...
  err = LGM2FCMSCorrelBuild(yc_name, und_name, lTfixing, lStartDate, lEndDate1,
                            lEndDate2, lTpay, lToday, fixFreq1, fixBasis1,
                            refRateName, fixFreq2, fixBasis2, sigma_1, sigma_2,
                            sigma_date, sigma_n, lam1[0], lam2[0], alpha, gamma,
                            rho, nb_integration, bound, &output_tmp);
...
Err LGM2FCMSCorrelBuild(char *yc_name, char *und, long lTfixing,
  ...
  long *float_fixing_dates1 = NULL, *float_start_dates1 = NULL,
       *float_end_dates1 = NULL, *float_pay_dates1 = NULL;
  ...
  long *float_fixing_dates2 = NULL, *float_start_dates2 = NULL,
       *float_end_dates2 = NULL, *float_pay_dates2 = NULL;
  ...
  err = swp_f_make_FloatLegDatesCoveragesAndSpreads(
      &swapdp1, lToday, refRateName, &float_pay_dates1, &float_nb_pay_dates1,
      &float_fixing_dates1, &float_start_dates1, &float_end_dates1,
      &float_cvgs1, &float_spreads1, &float_nb_dates1);
  if (err) {
    goto FREE_RETURN;
  }
  err = swp_f_make_FloatLegDatesCoveragesAndSpreads(
      &swapdp2, lToday, refRateName, &float_pay_dates2, &float_nb_pay_dates2,
      &float_fixing_dates2, &float_start_dates2, &float_end_dates2,
      &float_cvgs2, &float_spreads2, &float_nb_dates2);
  if (err) {
    goto FREE_RETURN;
  }
  ...
  err = swp_f_make_FixedLegDatesAndCoverages(
      &swapdp1, lToday, &fix_pay_dates1, &fix_nb_pay_dates1, &fix_start_dates1,
      &fix_end_dates1, &fix_cvgs1, &fix_nb_dates1);
  if (err) {
    goto FREE_RETURN;
  }
  err = swp_f_make_FixedLegDatesAndCoverages(
      &swapdp2, lToday, &fix_pay_dates2, &fix_nb_pay_dates2, &fix_start_dates2,
      &fix_end_dates2, &fix_cvgs2, &fix_nb_dates2);
  if (err) {
    goto FREE_RETURN;
  }
  ...
  output_tmp = (double *)calloc(5, sizeof(double));
  err = compute_CMScorrel_GL(
      Tf, Tp, DF_fix1, fix_coupon_date1, fix_cvg1, nb_fix_coupon1, DF_float1,
      DF_Tf1, float_coupon_date1, float_cvg1, nb_float_coupon1, spread1, Tf, Tp,
      DF_fix2, fix_coupon_date2, fix_cvg2, nb_fix_coupon2, DF_float2, DF_Tf2,
      float_coupon_date2, float_cvg2, nb_float_coupon2, spread2, sigma_1,
      sigma_2, sigma_date, sigma_n, lambda1, lambda2, rho, sigma_1, sigma_2,
      sigma_date, sigma_n, lambda1, lambda2, rho, nb_integration, bound,
      &output_tmp);
  (*output)[0] = output_tmp[0];
  (*output)[1] = output_tmp[1];
  (*output)[2] = output_tmp[2];

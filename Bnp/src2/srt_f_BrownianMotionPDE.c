...
static Err set_pde_bound(long	pde_dim,
						 double	*mesh_bound,
						 Err    (*pde_boundary_func)(),
						 double	**pde_bound)
{
	Err		err = NULL;
	long	i;
...
static Err srt_f_amer_option_payoff(Date eval_date,Date ExerDate,double dwt, double *payoff);

Err srt_f_brownain_motion_eur_pde(Date					eval_date, /* EVALUATION DATE */
								...
								double					*option_pv)
{
	...
	Err			err = NULL;
	double		*cur_premium_ptr = NULL,*prev_premium_ptr = NULL,*payoff = NULL,*X = NULL, prob[3],yr_to_exp_date,
				time_step = MAX_TIME,dX;
	...
	if(pde_solving_scheme == EXPLICIT)
	{
		...
		for( j = -max_index; j <= max_index; j++) X[j] = (j*dX);
		...
		{
			if(pde_boundary_cond == DIRICHLET) /*CASE OF OF DIRICHLET BOUNDARIES CONDITIONS */
			{
				for(j = (- max_index + 1); j <= (max_index-1); j++)/* LOOP ON THE SPACE POINTS */
				{
				...
				for(j = - max_index; j <= max_index; j++) prev_premium_ptr[j] =  cur_premium_ptr[j];
	...
	else if(pde_solving_scheme == THETASCHEME_CENTRED)
	{
		...
		for(i = 1; i < num_time_step; i++) /* LOOP ON THE TIME STEP */
		{	
			for(j = 1; j < (max_index+max_index); j++)/* LOOP ON THE SPACE POINTS */
			{	
				if( j == 1) /* BOUNDARY CONDITION TO APPLY */
				{
					vect_R[j] = pde_params[1]*prev_premium_ptr[j+1-max_index]
							-( 1.0 + 2.0*pde_params[1])*prev_premium_ptr[j-max_index]
							+ pde_params[1]*pde_bound[0]
							- pde_params[0]*pde_bound[0]

							+(1-PDE_THETA)*div_mat[i-1][j]
							+PDE_THETA*div_mat[i][j];
				} 
				else if( j == (max_index+max_index-1)) /* BOUNDARY CONDITION TO APPLY */
				{
					vect_R[j] = pde_params[1]*pde_bound[1]
								-pde_params[0]*pde_bound[1]
								-( 1.0 + 2.0*pde_params[1])*prev_premium_ptr[j-max_index]
			...
			err = tridag(diag_A,diag_B,diag_C,vect_R,vect_U,(max_index+max_index-1));
			if(err) return err;

			for(j = (- max_index +1); j <= (max_index-1); j++) prev_premium_ptr[j] =  vect_U[j+max_index];
		...
		if(diag_A) free_dvector(diag_A,1, (max_index+max_index-1));diag_A = NULL;
		if(diag_B) free_dvector(diag_B,1, (max_index+max_index-1));diag_B = NULL;
		if(diag_C) free_dvector(diag_C,1, (max_index+max_index-1));diag_C = NULL;
		if(vect_R) free_dvector(vect_R,1, (max_index+max_index-1));vect_R = NULL;
		if(vect_U) free_dvector(vect_U,1, (max_index+max_index-1));vect_U = NULL;
		if(prev_premium_ptr) free_dvector(prev_premium_ptr,-max_index,max_index);prev_premium_ptr = NULL;
		if(payoff) free_dvector(payoff,-max_index,max_index);payoff = NULL;
...
Err srt_f_brownain_motion_amer_pde(Date					eval_date, /* EVALUATION DATE */
	...
	for (i = 0; i < nEx; i++)
	{
		...
		num_interval = ((long) ((end_date - start_date)*YEARS_IN_DAY / time_step)) + 1;
	...
	num_time_step = ((long) (yr_to_exp_date/ time_step)) + 1;/** end_date OF THE FIRST STEP */

	/* GET THE pde_bound AT EACH EXERCISE DATE */
	
	if(pde_solving_scheme == EXPLICIT) dX = TREE_MESH_SPACING*sqrt(time_step);
	else if(pde_solving_scheme == THETASCHEME_CENTRED) dX = LIM_IN_STDEV*sqrt(yr_to_exp_date)/num_mesh;
	...
	while( (k < n) )
	{
		...
		{
			while(tDeal[k] > tEx[index]) index++;
			Weight = ((double)(tDeal[k]-tEx[index-1]))/(tEx[index]-tEx[index-1]);
			pde_boundary_deal[0][k] = pde_bound[0][index-1] + Weight*(pde_bound[0][index]-pde_bound[0][index-1]);
			pde_boundary_deal[1][k] = pde_bound[1][index-1] + Weight*(pde_bound[1][index]-pde_bound[1][index-1]);
		}
		...
		for( i = (num_time_step-1); i > 0; i--) /* LOOP ON THE TIME STEP */
		{
			if(pde_boundary_cond == DIRICHLET) /*CASE OF OF DIRICHLET BOUNDARIES CONDITIONS */
			{
				...
				if (tDeal[i] == tEx[index_exercise])
				{
					for(j = - max_index; j <= max_index; j++)
					{
						err = (*amer_payoff_func)(eval_date,tEx[index_exercise],X[j],&payoff[j]);
						if(err) return err;
	...
	{
		if(pde_boundary_cond == DIRICHLET)	N = (max_index+max_index-1);
		else if(pde_boundary_cond == NEUMANN) N = (max_index+max_index+1);
		...
		{	
			...
			{	
				if( j == 1) /* BOUNDARY CONDITION TO APPLY */
				{
					...
					else if(pde_boundary_cond == NEUMANN)
					{
						vect_R[j] = pde_params[1]*prev_premium_ptr[j+1-max_index]
								...
								+ pde_params[1]*(prev_premium_ptr[j-max_index]-pde_boundary_deal[0][i]*dX)
				...
				else if( j == N) /* BOUNDARY CONDITION TO APPLY */
				{
					if(pde_boundary_cond == DIRICHLET)
					{
						vect_R[j] = pde_params[1]*pde_boundary_deal[1][i]
					...
					{
						vect_R[j] = pde_params[1]*(pde_boundary_deal[1][i]*dX + prev_premium_ptr[j-max_index])
			...
			else 
			{
				...
				{
					for(j = -max_index; j <= max_index; j++) prev_premium_ptr[j] =  vect_U[j+max_index];
				}
		...
		if(diag_A) free_dvector(diag_A,1,N);diag_A = NULL;
		if(diag_B) free_dvector(diag_B,1,N);diag_B = NULL;
		if(diag_C) free_dvector(diag_C,1,N);diag_C = NULL;
		if(vect_R) free_dvector(vect_R,1,N);vect_R = NULL;
		if(vect_U) free_dvector(vect_U,1,N);vect_U = NULL;
		if(prev_premium_ptr) free_dvector(prev_premium_ptr,-max_index,max_index);prev_premium_ptr = NULL;
		if(payoff) free_dvector(payoff,-max_index,max_index);payoff = NULL;
		if(pde_boundary_deal) free_dmatrix(pde_boundary_deal,0,pde_dim,0,num_time_step);pde_boundary_deal = NULL;
...
Err srt_f_pde_eur_option(Date		eval_date,
						 Date		exp_date,
						 double		forward,
						 double		equity_strike,
						 double		equity_vol,
						 char		*rec_pay_str,
						 char		*pde_solving_scheme_str,
						 long		min_node,
						 long		num_mesh,
	...
	
{
	...
	if(!strcmp(pde_solving_scheme_str,"EXPLICIT"))	pde_solving_scheme = EXPLICIT;
	else if(!strcmp(pde_solving_scheme_str,"THETASCHEME_CENTRED"))	pde_solving_scheme = THETASCHEME_CENTRED;
	...
	err = set_pde_bound(pde_dim,mesh_bound,&srt_f_eur_option_payoff,&pde_bound);
	if(err) return err;
	...
	err = srt_f_brownain_motion_eur_pde(eval_date,
									   exp_date,
									   &srt_f_eur_option_payoff,
									   min_node,
									   num_mesh,
									   pde_solving_scheme,
	...
	if(pde_bound) free_dvector(pde_bound,0,pde_dim); pde_bound = NULL;
	if(mesh_bound) free_dvector(mesh_bound,0,pde_dim); mesh_bound = NULL;
	if(div_mat) free_dmatrix(div_mat,0,num_time_step-1,0,max_index+max_index-1); div_mat = NULL;

	return err;

}

Err srt_f_pde_amer_option(Date		eval_date,
							 Date       *tEx,
							 long		nEx,
							 double		forward,
							 double		equity_strike,
	...
	
{
	...
	if(!strcmp(pde_boundary_cond_str,"NEUMANN"))	pde_boundary_cond = NEUMANN;
	else pde_boundary_cond = DIRICHLET;
	...
	err = srt_f_brownain_motion_amer_pde(eval_date,
									   tEx,
									   nEx,
									   min_node,
									   num_mesh,
									   &srt_f_amer_option_payoff,
									   pde_solving_scheme,
									   pde_boundary_cond,
									   pde_bound,
	...
	if(pde_bound)  free_dmatrix(pde_bound,0,pde_dim,0,nEx-1); pde_bound = NULL;
...
static Err srt_f_eur_option_payoff(double dwt, double *payoff)
{
	...
	if(pay_rec == SRT_RECEIVER) (*payoff) = max(0,equity_strike-forward*exp(equity_vol*dwt-0.5*sq_equity_vol*yr_to_exp));
	else if(pay_rec == SRT_RECEIVER) (*payoff) = max(0,forward*exp(equity_vol*dwt-0.5*sq_equity_vol*yr_to_exp)-equity_strike);
...
static Err srt_f_amer_option_payoff(Date eval_date,Date ExerDate,double dwt, double *payoff)
{
	...
	if(pay_rec == SRT_RECEIVER) Eps = -1.0;
	else Eps = +1.0;

	(*payoff) = max(0,Eps*(forward*exp(equity_vol*dwt-0.5*sq_equity_vol*time)-equity_strike));

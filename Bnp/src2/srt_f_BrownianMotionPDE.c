...
static Err set_pde_bound(
    long pde_dim, double* mesh_bound, Err (*pde_boundary_func)(), double** pde_bound)
{
    Err  err = NULL;
    long i;
...
{
    Err     err             = NULL;
    double *cur_premium_ptr = NULL, *prev_premium_ptr = NULL, *payoff = NULL, *X = NULL, prob[3],
           yr_to_exp_date, time_step = MAX_TIME, dX;
    ...
    else if (pde_solving_scheme == THETASCHEME_CENTRED)
    {
        ...
        for (i = 1; i < num_time_step; i++) /* LOOP ON THE TIME STEP */
        {
            for (j = 1; j < (max_index + max_index); j++) /* LOOP ON THE SPACE POINTS */
            {
                ...
                {
                    vect_R[j] = pde_params[1] * pde_bound[1] - pde_params[0] * pde_bound[1] -
                                (1.0 + 2.0 * pde_params[1]) * prev_premium_ptr[j - max_index] +
    ...
    double* option_pv)
{
    Err     err             = NULL;
    double *cur_premium_ptr = NULL, *prev_premium_ptr = NULL, *payoff = NULL, *X = NULL, prob[3],
           yr_to_exp_date, time_step = MAX_TIME, dX, Weight;
    ...
    while ((k < n))
    {
        ...
        else
        {
            ...
            pde_boundary_deal[0][k] =
                pde_bound[0][index - 1] + Weight * (pde_bound[0][index] - pde_bound[0][index - 1]);
            pde_boundary_deal[1][k] =
                pde_bound[1][index - 1] + Weight * (pde_bound[1][index] - pde_bound[1][index - 1]);
        }
        ...
        for (i = (num_time_step - 1); i > 0; i--) /* LOOP ON THE TIME STEP */
        {
            for (j = 1; j <= N; j++) /* LOOP ON THE SPACE POINTS */
            {
                ...
                else if (j == N) /* BOUNDARY CONDITION TO APPLY */
                {
                    ...
                    {
                        vect_R[j] = pde_params[1] * (pde_boundary_deal[1][i] * dX +
...
static Err srt_f_eur_option_payoff(double dwt, double* payoff)
{
    ...
    if (pay_rec == SRT_RECEIVER)
        (*payoff) = max(
            0, equity_strike - forward * exp(equity_vol * dwt - 0.5 * sq_equity_vol * yr_to_exp));
    else if (pay_rec == SRT_RECEIVER)
        (*payoff) = max(
            0, forward * exp(equity_vol * dwt - 0.5 * sq_equity_vol * yr_to_exp) - equity_strike);
...
static Err srt_f_amer_option_payoff(Date eval_date, Date ExerDate, double dwt, double* payoff)
{
    ...
    (*payoff) = max(
        0, Eps * (forward * exp(equity_vol * dwt - 0.5 * sq_equity_vol * time) - equity_strike));

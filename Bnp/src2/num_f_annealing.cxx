/* ------------------------------------------------------------------------
        ...
        PURPOSE:	provide a full function that minimises an error
                                criteria using the Simulated Annealing
   algorithm. This function is intended as a generic wrapper to the amebsa
   function as described in Numerical Recipes in C. INPUTS: p: initial simplex
                        y:            values of funk at the simplex points
                        ndim:         dimension of the space (== number of
   parameters ) ftol:         fractionnal convergence tolerance niter: maximum
   number of iterations funcs():      the f(x) function to minimise init_temp:
   inital temperature in the system decr_fact:    parameter to specify the speed
   of "cooling" niter_per_t:  number of moves for each temperature method:
   method used for the annealation scheme restart:      restart the search after
   a big drop (0 or 1)
...
Err interp_annealing_method(String sMethod, AnnealMethod *pMethod) {
  ...
  if ((!strcmp(sMethod, "ALPHA")) || (!strcmp(sMethod, "POWER")) ||
    ...
    *pMethod = POWER_ALPHA;
  } else if ((!strcmp(sMethod, "EPSILON")) ||
             (!strcmp(sMethod, "PROPORTIONAL_EPSILON")) ||
...
Err simulated_annealing(double **p, /* From [1] to [ndim+1]  * [1] to [ndim] */
                        ...
                        double min_temp, double dec_fact)

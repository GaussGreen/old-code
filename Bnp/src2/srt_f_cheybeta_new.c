...
double CheyBeta_Vol(double x, double phi, double sig, double beta, double gamT, double fr,
					double xcut_u, double xcut_d)
{
	double vol, xx;

	xx = x - phi * gamT;
	if (xx > xcut_u) xx = xcut_u;
	if (xx < xcut_d) xx = xcut_d;

	vol = sig * (1.0 + beta * xx / fr);
	if (vol < 0.0001) vol = 0.0001;
	if (vol > 0.03) vol = 0.03;
...
Err CheyBeta_Init(SCheyBeta *pmdl, char *und_name)
{
	...
	if (!und) return serror("Underlying not found");
	...
	if (pmdl->nsig == 0) return serror("No sigma dates found");
	...
	if (!pmdl->sig || !pmdl->beta || !pmdl->sigtms) return serror("Memory failure");
	...
	if (!ls) return serror("No tau dates found");

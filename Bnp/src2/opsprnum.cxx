...
static double int_in(double var) {
  ...
  if (NI_b > 0) {
    bs = kprime * norm(d1) -
         NI_b * exp(NI_muY + NI_sigZ * NI_sigZ / 2.0) * norm(d2);
  } else {
    bs = kprime * norm(-d1) -
         NI_b * exp(NI_muY + NI_sigZ * NI_sigZ / 2.0) * norm(-d2);
  }
 ...
 * PURPOSE      	: ??
 *
 * DESCRIPTION  	: called by int_doub_addin (in option_tools.cxx) <--
 * 			@spread_numeric(...)
 *
 * CALLS		: int_in(...)
 *		: sm_qsimp(...)
...
double srt_f_optsprnum(double fwdx, double fwdy, double sigx, double sigy,
                       double rho, double disc, double a, double b, double k,
                       double t, SrtCallPutType call_put, SrtGreekType greek) {
  double result;
  double answer;
  double price;
  ...
  glimitlow = (fabs(NI_limitX) > INTEG_LIMIT ? -INTEG_LIMIT - fabs(NI_limitX)
                                             : -INTEG_LIMIT);
  glimithigh = (fabs(NI_limitX) > INTEG_LIMIT ? +INTEG_LIMIT + fabs(NI_limitX)
                                              : +INTEG_LIMIT);
  ...
  case DELTAX:
    shift = fwdx / 10000;
    answer = (srt_f_optsprnum(fwdx + shift, fwdy, sigx, sigy, rho, disc, a, b,
                              k, t, call_put, PREMIUM) -
              result) /
             shift;
    return (answer);
    break;
  ...
  case DELTAY:
    shift = fwdy / 10000;
    answer = (srt_f_optsprnum(fwdx, fwdy + shift, sigx, sigy, rho, disc, a, b,
                              k, t, call_put, PREMIUM) -
              result) /
             shift;
    return (answer);
  ...
  case GAMMAX:
    shift = fwdx / 1000;
    answer = srt_f_optsprnum(fwdx + shift, fwdy, sigx, sigy, rho, disc, a, b, k,
                             t, call_put, PREMIUM);
    answer += srt_f_optsprnum(fwdx - shift, fwdy, sigx, sigy, rho, disc, a, b,
                              k, t, call_put, PREMIUM);
    answer -= 2 * result;
  ...
  case GAMMAY:
    shift = fwdy / 1000;
    answer = srt_f_optsprnum(fwdx, fwdy + shift, sigx, sigy, rho, disc, a, b, k,
                             t, call_put, PREMIUM);
    answer += srt_f_optsprnum(fwdx, fwdy - shift, sigx, sigy, rho, disc, a, b,
                              k, t, call_put, PREMIUM);
    answer -= 2 * result;
  ...
  case GAMMAXY:
    shiftx = fwdx / 1000;
    shifty = fwdy / 1000;
    answer = srt_f_optsprnum(fwdx + shiftx, fwdy + shifty, sigx, sigy, rho,
                             disc, a, b, k, t, call_put, PREMIUM);
    answer += srt_f_optsprnum(fwdx - shiftx, fwdy - shifty, sigx, sigy, rho,
                              disc, a, b, k, t, call_put, PREMIUM);
    answer -= srt_f_optsprnum(fwdx + shiftx, fwdy - shifty, sigx, sigy, rho,
                              disc, a, b, k, t, call_put, PREMIUM);
  ...
  case VEGAX:
    shift = GVOPT.vol_add;
    answer = (srt_f_optsprnum(fwdx, fwdy, sigx + shift, sigy, rho, disc, a, b,
                              k, t, call_put, PREMIUM) -
              result) /
             shift;
    return (answer);
  ...
  case VEGAY:
    shift = GVOPT.vol_add;
    answer = (srt_f_optsprnum(fwdx, fwdy, sigx, sigy + shift, rho, disc, a, b,
                              k, t, call_put, PREMIUM) -
              result) /
             shift;
    return (answer);
  ...
  case THETA:
    shift = YEARS_IN_DAY;
    answer = srt_f_optsprnum(fwdx, fwdy, sigx, sigy, rho,
                             disc * exp(-shift * log(disc) / t), a, b, k,
                             t - shift, call_put, PREMIUM) -
             result;
    return (answer);
    break;

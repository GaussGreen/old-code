...
double result;
	...
	result = 1.0 / x * (Phi_Func(y, Ty, s, t) - exp (-x * (Tx - Ty)) * Phi_Func(x + y, Ty, s, t));
...
static Err Coefs_Partial_Var(double T, double T1, double T2,
						   double sig_dom, double lda_dom,
						   double sig_for, double lda_for,		
						   ...
						   double *a, double *b, double *c)
{
	...
	(*c) = sig_for*sig_for*Psi_Func(lda_for, lda_for, T, T1, T2)
		   ...
		   - 2*correl_dom_for*sig_dom*sig_for*Psi_Func(lda_for, lda_dom, T, T1, T2);

	return NULL;
}

static Err Partial_Var(double T, double T1, double T2,
								   double sig_dom, double lda_dom,
								   double sig_for, double lda_for,
								   double sig_fx,
								   double correl_dom_for, double correl_dom_fx, double correl_for_fx, double *var)
{
double a;
double b;
double c;
Err err = NULL;
	...
	/* Get Coefs */
	err = Coefs_Partial_Var(T, T1, T2, sig_dom, lda_dom, sig_for,
							lda_for, correl_dom_for, correl_dom_fx, correl_for_fx, &a, &b, &c);
...
Err get_unique_lambda		(	double	*tau,
								int		ntau,
								double	*lda)
{
	double the_tau;
...
Err merge_rates_ts		(	double	*sig_dom_mat,
							double	*sig_dom,
							int		sig_n_dom,
							double	*sig_for_mat,
							double	*sig_for,
							...
							int		*sig_merge_n)
{
	...
	*sig_merge_for = (double*) calloc (*sig_merge_n, sizeof (double));
	...
	{
		(*sig_merge_dom)[i] = sig_dom[Get_Index ((*sig_merge_mat)[i], sig_dom_mat, sig_n_dom)];
		(*sig_merge_for)[i] = sig_for[Get_Index ((*sig_merge_mat)[i], sig_for_mat, sig_n_for)];
	}
...
FREE_RETURN:
	...
	if (err)
	{
		if (*sig_merge_mat) free (*sig_merge_mat);
		if (*sig_merge_dom) free (*sig_merge_dom);
		if (*sig_merge_for) free (*sig_merge_for);
	}
...
Err merge_rates_fx_corr_ts(	double	*sig_dom_mat,
							double	*sig_dom,
							int		sig_n_dom,
							double	*sig_for_mat,
							double	*sig_for,
							...
							int		*sig_merge_n)
{
	...
	*corr_merge_for_fx = (double*) calloc (*sig_merge_n, sizeof (double));
	...
	if (!(*sig_merge_dom) || !(*sig_merge_for) || (sig_fx_mat && !(*sig_merge_fx))
		|| !(*corr_merge_dom_for) || !(*corr_merge_dom_fx) || !(*corr_merge_for_fx))
	{
...
FREE_RETURN:
	...
	if (err)
	{
		if (*sig_merge_mat) free (*sig_merge_mat);
		if (*sig_merge_dom) free (*sig_merge_dom);
		if (*sig_merge_fx) free (*sig_merge_fx);
		if (*corr_merge_dom_for) free (*corr_merge_dom_for);
		if (*corr_merge_dom_fx) free (*corr_merge_dom_fx);
		if (*corr_merge_for_fx) free (*corr_merge_for_fx);
	}
						...
						double *varff)
{
	...
	for (i = StartIndex; i < EndIndex + 1; i++)
	{
		...
		phidom += sig_curve_dom[i] * sig_curve_dom[i] * Phi_Func (2 * lda_dom, Tfix, t1, t2);
		phifor += sig_curve_for[i] * sig_curve_for[i] * Phi_Func (2 * lda_for, Tfix, t1, t2);
	...
	*varff = phidom * lamdom * lamdom + phifor * lamfor * lamfor - 2 * phidomfor * lamdom * lamfor;

	return err;
}

/*	IR Phi */
Err Fx3DtsPhi		(	double maturity,
						double *maturity_sig, long nbSig,
						double *sig_curve, double lda,
						double *phi)
{
double	sig;
double	T1, T2;
...
Err err = NULL;
	...
	for (idxopt = 0 ; idxopt <= nbrOpt-1 ; idxopt++)
	{
		...
		for (i=StartIndex; i<EndIndex + 1; i++)
		{
			...
			err = Coefs_Partial_Var( val_opt,  t1,  t2,
									sig_dom,  lda_dom,
									sig_for,  lda_for,
		...
		if ((delta < 0) || ((c > 0) && (b > 0)))
		{
			...
			err = serror ("Cannot find solution to match the option %d (its vol should be > %.2f %%)", 
				idxopt+1, 
				VolMin);
...
Err			err = NULL;
	...
	err = Fx3DtsImpliedVol(	val_time, start_time, end_time,
							merge_dates, nb_merge_dates,
							sig_dom,  lda_dom,
							sig_for,  lda_for,
							sigma_date_fx,  sigma_fx,  sigma_n_fx, 
...
Err			err = NULL;
	...
	err = Fx3DtsCalibration(
							exercise_opt, maturity_opt, vol_opt, nbropt,
							merge_dates,  nb_merge_dates,
							sig_dom, lda_dom,
							sig_for, lda_for,   
							correl_dom_for,  correl_dom_fx,  correl_for_fx,
...
Err			err = NULL;
	...
	err = Fx3DFwdTpay_corr (fx_underlying, fix_time, val_time, pay_time, 1, &fx_fwd);
	if (err)
	{
		return err;
	}
...
Err Get_LGM_TermStructure(char *underlying, double **sigma_date, double **sigma, long *sigma_n,
...

{
	...
	if (get_mdltype_from_irund (und) != LGM || get_mdldim_from_irund (und) != ONE_FAC)
	{
	...
	err = srt_f_display_IRM_OneFac_TermStruct (ts, sigma_date, sigma, &beta, &vovol, &rho, &meanvol, sigma_n, tau_date, tau, tau_n);
...

{
	...
	err = srt_f_display_IRM_OneFac_TermStruct (	ts,
												sigma_time,
												sigma, 
												&beta,
												&vovol,
...

{
	...
	if (get_mdltype_from_irund (und) != LGM || get_mdldim_from_irund (und) != TWO_FAC)
	{
	...
	err = srt_f_display_IRM_TwoFac_TermStruct (	ts,
												sigma_time,
												sigma, 
												&beta1,
												&sigma2, 
	...
	/* Now transform dates into maturities and check alpha, gamma, rho and tau	*/
							...
							double	*fixed_rho)
{
	...
	/* Now transform dates into maturities and check alpha, gamma, and rho */
	...
	for (i=0; i<*nb_lambda; i++)
	{
		...
		while (i+j < *nb_lambda && fabs((*lambda)[i+j] - 1.0 / (*lambda)[i]) < 1.0E-10)
		{
			j++;
		}
		j--;
...
/*	IR Phi */
Err LGM2FDtsPhi		(	double	maturity,
						double	*maturity_sig,
						long	nbSig,
						double	*sig_curve,
						double	*maturity_lam,
...
Err Get_FX_StochRate_TermStructures(char *underlying,
									double **sigma_date_dom, double **sigma_dom, long *sigma_n_dom,
									double **tau_date_dom, double **tau_dom, long *tau_n_dom,
...
Err			err = NULL;
	...
	*sigma_fx = NULL;
	...
	domname = get_domname_from_fxund (und);
	err = Get_LGM_TermStructure (domname, sigma_date_dom, sigma_dom, sigma_n_dom, tau_date_dom, tau_dom, tau_n_dom);
	if (err)
	{
		goto FREE_RETURN;
	}
	...
	forname = get_forname_from_fxund (und);
	err = Get_LGM_TermStructure (forname, sigma_date_for, sigma_for, sigma_n_for, tau_date_for, tau_for, tau_n_for);
	if (err)
	{
		goto FREE_RETURN;
	}
	
	err = srt_f_display_FX_TermStruct (underlying, sigma_n_fx, sigma_date_fx, sigma_fx, &corr_date_n, &corr_date, &corr);
	if (err)
	{
		goto FREE_RETURN;
	}
...
/*	Calculate adjustment between Q-Tpay and Q-beta, such that
	Q-beta expect log ( S ( Tfix ) / S ( 0 ) ) = Q-Tpay expect log ( S ( Tfix ) / S ( 0 ) ) + adj */
	...
	for (i = StartIndex; i < EndIndex + 1; i++)
	{
		...
		for (j = StartIndex2; j < EndIndex2 + 1; j++)
		{
			...
			adjust_partial = 
				...
				- correl_dom_for * sig_dom * sig_for * Psi2_Func (lda_for, lda_dom, Tval, Tpay, t1, t2)
...
/*	Calculate adjustment between Q-Tpay1 and Q-Tpay2, such that
	Q-Tpay2 expect log ( S ( Tfix ) / S ( 0 ) ) = Q-Tpay1 expect log ( S ( Tfix ) / S ( 0 ) ) + adj */
		...
		for (j = StartIndex2; j < EndIndex2 + 1; j++)
		{
			...
			covar_partial = 
				...
				(	
						...
						(
							Etha_Func (lda_dom, Tval1, t1, t2) + Etha_Func (lda_dom, Tval2, t1, t2)
						)
						
						- correl_for_fx * sig_for * 
						(
							Etha_Func (lda_for, Tval1, t1, t2) + Etha_Func (lda_for, Tval2, t1, t2)
						)
				)
				+	sig_dom * sig_dom * Psi2_Func (lda_dom, lda_dom, Tval1, Tval2, t1, t2)
				+	sig_for * sig_for * Psi2_Func (lda_for, lda_for, Tval1, Tval2, t1, t2)
...
Err		err = NULL;
	...
	*expect = log (fx_fwd / spot_fx) - 0.5 * fx_vol * fx_vol * Tfix;
	...
	err = Fx3DtsFwdBetaAdjustment(0.0, Tval, Tval, Tfix,
							merge_dates,  nb_merge_dates,
							sig_dom, lda_dom,
							sig_for, lda_for,   
							sigma_date_fx,  sigma_fx,  sigma_n_fx, 
...
Err		err = NULL;
	...
	*expect = log (fx_fwd / spot_fx) - 0.5 * fx_vol * fx_vol * Tfix;
	...
	err = Fx3DtsFwdPayAdjustment(0.0, Tval, Tval, Tpay, Tfix,
							merge_dates,  nb_merge_dates,
							sig_dom, lda_dom,
							sig_for, lda_for,   
							sigma_date_fx,  sigma_fx,  sigma_n_fx, 
...
Err		err = NULL;
	...
	*expect = log (fx_fwd / spot_fx) - 0.5 * fx_vol * fx_vol * Tbarfix;
	...
	err = Fx3DtsFwdPayAdjustment(0.0, Tbarval, Tbarval, Tpay, Tbarfix,
							merge_dates,  nb_merge_dates,
							sig_dom, lda_dom,
							sig_for, lda_for,   
							sigma_date_fx,  sigma_fx,  sigma_n_fx, 
	...
	*expect += adjustment;
	...
	err = Fx3DtsFwdCumCovar (0.0, Tval, Tbarval, Tbarfix,
							merge_dates,  nb_merge_dates,
							sig_dom, lda_dom,
							sig_for, lda_for,   
							sigma_date_fx,  sigma_fx,  sigma_n_fx, 
...
Err			err = NULL;
	...
	err = Fx3DtsImpliedVol(	Tval1, 0, Tfix1,
							merge_dates,  nb_merge_dates,	
							sig_dom,  lda_dom,
							sig_for,  lda_for,
							sigma_date_fx,  sigma_fx,  sigma_n_fx, 
							correl_dom_for,  correl_dom_fx,  correl_for_fx,  &vol01);

	var01 = vol01 * vol01 * Tfix1;

	err = Fx3DtsImpliedVol(	Tval2, 0, Tfix2,
							merge_dates,  nb_merge_dates,	
							sig_dom,  lda_dom,
							sig_for,  lda_for,
							sigma_date_fx,  sigma_fx,  sigma_n_fx, 
							correl_dom_for,  correl_dom_fx,  correl_for_fx,  &vol02);
	
	var02 = vol02 * vol02 * Tfix2;

	err = Fx3DtsImpliedVol(	Tval2, Tfix1, Tfix2,
							merge_dates,  nb_merge_dates,	
							sig_dom,  lda_dom,
							sig_for,  lda_for,
							sigma_date_fx,  sigma_fx,  sigma_n_fx, 
							correl_dom_for,  correl_dom_fx,  correl_for_fx,  &vol12);

	var12 = vol12 * vol12 * (Tfix2 - Tfix1);
	
	err = Fx3DtsFwdFactVar (0.0, Tval1, Tval2, Tfix1,
							merge_dates,  nb_merge_dates,	
							sig_dom,  lda_dom,
							sig_for,  lda_for,
							correl_dom_for, &varff);
...
static double Psi3_Func(double x, double y, double t0, double T, double t1, double t2)
{
double res;
	...
	{
		res = 1.0 / y * (exp(-y * (t1 - t0)) - exp(-y * (t2 - t0)));
		res -= 1.0 / (y - x) * (exp(- x * T + y * t0 - (y - x) * t1)
							   -exp(- x * T + y * t0 - (y - x) * t2));
		return res / x;
	}
}

Err Fx3DtsImpliedVolExtrapol(	double opt_maturity, double start_date, double end_date,
								double *maturity_rates, long nbMat,
								double *sig_curve_dom, double lda_dom,
								double *sig_curve_for, double lda_for,
								double *maturity_fx, double *sig_curve_fx, long nbrMat_fx, 
								double	sig_inf, double lda_vol,
								double correl_dom_for, double correl_dom_fx, double correl_for_fx,
								double *fx_vol)
{
double	sig_dom, sig_for, sig_fx;
...
Err		err = NULL;
	...
	else
	{
		...
		for (j = StartIndex2; j < EndIndex2 + 1; j++)
		{
			...
			var_partial += 2.0 * (sig0 - sig_inf) * 
							(correl_dom_fx * sig_dom * Psi3_Func(lda_dom, lda_vol, t0, opt_maturity, t1, t2)
						   - correl_for_fx * sig_for * Psi3_Func(lda_for, lda_vol, t0, opt_maturity, t1, t2));

			var_partial += 2.0 * sig_inf * (sig0 - sig_inf) * Phi_Func(-lda_vol, t0, t1, t2);
			var_partial += (sig0 - sig_inf) * (sig0 - sig_inf) * Phi_Func(- 2.0 * lda_vol, t0, t1, t2);
...
Err Fx3DtsImpliedVolExtrapol_corr(	double opt_maturity, double start_date, double end_date,
									double *maturity_rates, long nbMat,
									double *sig_curve_dom, double lda_dom,
									double *sig_curve_for, double lda_for,
									double *maturity_fx, double *sig_curve_fx, long nbrMat_fx, 
									double	sig_inf, double lda_vol,
									double *correl_times, double *correl_dom_for_ts, double *correl_dom_fx_ts, double *correl_for_fx_ts, long nb_correl,
									double *fx_vol)
{
double	sig_dom, sig_for, sig_fx;
double	T1, T2, t1, t2;
...
double	t0, sig0;
double	correl_dom_for, correl_dom_fx, correl_for_fx;
double	*mat_rates_corr			= NULL,
		*new_sig_curve_dom		= NULL,
		*new_sig_curve_for		= NULL,
		*new_correl_dom_for_ts	= NULL,
		*new_correl_dom_fx_ts	= NULL,
		*new_correl_for_fx_ts	= NULL;
long	nb_merged;
...
Err		err = NULL;
	...
	if (!new_sig_curve_dom || !new_sig_curve_for || !new_correl_dom_for_ts || !new_correl_dom_fx_ts || !new_correl_for_fx_ts)
	{
...
FREE_RETURN:
	...
	if (mat_rates_corr) free (mat_rates_corr);
	if (new_sig_curve_dom) free_dvector(new_sig_curve_dom, 0, nb_merged-1);
	if (new_sig_curve_for) free_dvector(new_sig_curve_for, 0, nb_merged-1);
	if (new_correl_dom_for_ts) free_dvector(new_correl_dom_for_ts, 0, nb_merged-1);
	if (new_correl_dom_fx_ts) free_dvector(new_correl_dom_fx_ts, 0, nb_merged-1);
	if (new_correl_for_fx_ts) free_dvector(new_correl_for_fx_ts, 0, nb_merged-1);
...
Err		err = NULL;
	...
	for (i = StartIndex; i < EndIndex + 1; i++)
	{
		...
		/* Z_1 with Dom_2 */
		correl_fxfx[1] += sig_1 * sig_2_dom * (Etha_Func(lda_dom2, option_maturity, T1, T2));

		/* Z_1 with For_2 */
		correl_fxfx[2] += sig_1 * sig_2_for * (Etha_Func(lda_for2, option_maturity, T1, T2));

		/* Dom_1 with Z_2 */
		correl_fxfx[3] += sig_1_dom * sig_2 * (Etha_Func(lda_dom1, option_maturity, T1, T2));

		/* Dom_1 with Dom_2 */
		correl_fxfx[4] += sig_1_dom * sig_2_dom * (Psi_Func(lda_dom1, lda_dom2, option_maturity, T1, T2));

		/* Dom_1 with For_2 */
		correl_fxfx[5] += sig_1_dom * sig_2_for * (Psi_Func(lda_dom1, lda_for2, option_maturity, T1, T2));

		/* For_1 with Z_2 */
		correl_fxfx[6] += sig_1_for * sig_2 * (Etha_Func(lda_for1, option_maturity, T1, T2));

		/* For_1 with Dom_2 */
		correl_fxfx[7] += sig_1_for * sig_2_dom * (Psi_Func(lda_for1, lda_dom2, option_maturity, T1, T2));

		/* For_1 with For_2 */
		correl_fxfx[8] += sig_1_for * sig_2_for * (Psi_Func(lda_for1, lda_for2, option_maturity, T1, T2));
	}

	*covar = 
				  correlations[2][5] * correl_fxfx[0]
...
Err Fx3DtsFxFwdCov_corr(double	option_maturity1,
...
Err		err = NULL;
	...
	for (i = StartIndex; i < EndIndex + 1; i++)
	{
		...
		if (is_fx1 && is_fx2)
		{
			...
			/* Z_1 with For_2 */
			correl_fxfx[2] += sig_1 * sig_2_for * (Etha_Func(lda_for2, option_maturity2, T1, T2))
				* correlations[2][4][i];

			/* For_1 with Z_2 */
			correl_fxfx[6] += sig_1_for * sig_2 * (Etha_Func(lda_for1, option_maturity1, T1, T2))
				* correlations[1][5][i];

			/* For_1 with For_2 */
			correl_fxfx[8] += sig_1_for * sig_2_for * (Psi2_Func(lda_for1, lda_for2, option_maturity1, option_maturity2, T1, T2))
				* correlations[1][4][i];						
		}
		...
		if (is_fx1)
		{
			...
			/* Z_1 with Dom_1 */
			var_fx1[1] += sig_1 * sig_1_dom * (Etha_Func(lda_dom1, option_maturity1, T1, T2))
				* correlations[0][2][i];

			/* Z_1 with For_1 */
			var_fx1[2] += sig_1 * sig_1_for * (Etha_Func(lda_for1, option_maturity1, T1, T2))
				* correlations[1][2][i];

			/* For_1 with Dom_1 */
			var_fx1[7] += sig_1_for * sig_1_dom * (Psi_Func(lda_for1, lda_dom1, option_maturity1, T1, T2))
				* correlations[0][1][i];
									
			/* For_1 with For_1 */
			var_fx1[8] += sig_1_for * sig_1_for * (Psi_Func(lda_for1, lda_for1, option_maturity1, T1, T2));
			...
			{				
				/* Z_1 with Dom_2 */
				correl_fxfx[1] += sig_1 * sig_2_dom * (Etha_Func(lda_dom2, option_maturity2, T1, T2))
					* correlations[2][3][i];

				/* For_1 with Dom_2 */
				correl_fxfx[7] += sig_1_for * sig_2_dom * (Psi2_Func(lda_for1, lda_dom2, option_maturity1, option_maturity2, T1, T2))
					* correlations[1][3][i];
			...
			{
				/* Z_1 with Dom_2 */
				correl_fxfx[1] += sig_1 * sig_2_dom * (Phi_Func(lda_dom2, option_maturity2, T1, T2))
					* correlations[2][3][i];

				/* For_1 with Dom_2 */
				//correl_fxfx[7] += sig_1_for * sig_2_dom * (Zeta_Func(lda_dom2, lda_for1, option_maturity, T1, T2))
				//	* correlations[1][3][i];				
				correl_fxfx[7] += sig_1_for * sig_2_dom * (Gamma2_Func(lda_for1, lda_dom2, option_maturity1, option_maturity2, T1, T2))
					* correlations[1][3][i];				
			}
		...
		{	
			...
			/* Z_2 with For_2 */
			var_fx2[2] += sig_2 * sig_2_for * (Etha_Func(lda_for2, option_maturity2, T1, T2))
				* correlations[4][5][i];

			/* Z_2 with Dom_2 */
			var_fx2[3] += sig_2 * sig_2_dom * (Etha_Func(lda_dom2, option_maturity2, T1, T2))
				* correlations[3][5][i];
						
			/* Dom_2 with For_2 */
			var_fx2[5] += sig_2_dom * sig_2_for * (Psi_Func(lda_dom2, lda_for2, option_maturity2, T1, T2))
				* correlations[3][4][i];						

			/* For_2 with For_2 */
			var_fx2[8] += sig_2_for * sig_2_for * (Psi_Func(lda_for2, lda_for2, option_maturity2, T1, T2));
			...
			{
				/* Dom_1 with Z_2 */
				correl_fxfx[3] += sig_1_dom * sig_2 * (Etha_Func(lda_dom1, option_maturity1, T1, T2))
					* correlations[0][5][i];

				/* Dom_1 with For_2 */
				correl_fxfx[5] += sig_1_dom * sig_2_for * (Psi2_Func(lda_dom1, lda_for2, option_maturity1, option_maturity2, T1, T2))
					* correlations[0][4][i];
			}
			else
			{
				/* Dom_1 with Z_2 */
				correl_fxfx[3] += sig_1_dom * sig_2 * (Phi_Func(lda_dom1, option_maturity1, T1, T2))
					* correlations[0][5][i];

				/* Dom_1 with For_2 */
				//correl_fxfx[5] += sig_1_dom * sig_2_for * (Zeta_Func(lda_dom1, lda_for2, option_maturity, T1, T2))
				//	* correlations[0][4][i];
				correl_fxfx[5] += sig_1_dom * sig_2_for * (Gamma2_Func(lda_for2, lda_dom1, option_maturity2, option_maturity1, T1, T2))
					* correlations[0][4][i];
			}						
		}
		...
		if (is_fx1 && is_fx2)
		{
			correl_fxfx[4] += sig_1_dom * sig_2_dom * (Psi2_Func(lda_dom1, lda_dom2, option_maturity1, option_maturity2, T1, T2))
				* correlations[0][3][i];
		}
		else if (is_fx1)
		{
			//correl_fxfx[4] += sig_1_dom * sig_2_dom * (Zeta_Func(lda_dom2, lda_dom1, option_maturity, T1, T2))
			//	* correlations[0][3][i];
			correl_fxfx[4] += sig_1_dom * sig_2_dom * (Gamma2_Func(lda_dom1, lda_dom2, option_maturity1, option_maturity2, T1, T2))
				* correlations[0][3][i];
		}
		else if (is_fx2)
		{
			//correl_fxfx[4] += sig_1_dom * sig_2_dom * (Zeta_Func(lda_dom1, lda_dom2, option_maturity, T1, T2))
			//	* correlations[0][3][i];
			correl_fxfx[4] += sig_1_dom * sig_2_dom * (Gamma2_Func(lda_dom2, lda_dom1, option_maturity2, option_maturity1, T1, T2))
				* correlations[0][3][i];
		}
		else
		{
			//correl_fxfx[4] += sig_1_dom * sig_2_dom * (Phi_Func(lda_dom1 + lda_dom2, option_maturity, T1, T2))
			//	* correlations[0][3][i];
			correl_fxfx[4] += sig_1_dom 
							* sig_2_dom 
							* (Phi_Func (	lda_dom1 + lda_dom2, 
											(lda_dom1 * option_maturity1 + lda_dom2 * option_maturity2) / (lda_dom1 + lda_dom2), 
											T1, 
											T2))
							* correlations[0][3][i];
		}
		...
		if (is_fx1)
		{			
			var_fx1[4] += sig_1_dom * sig_1_dom * (Psi_Func(lda_dom1, lda_dom1, option_maturity1, T1, T2));
		}
		else
		{		
			var_fx1[4] += sig_1_dom * sig_1_dom * (Phi_Func(2.0 * lda_dom1, option_maturity1, T1, T2));
		}
		...
		if (is_fx2)
		{			
			var_fx2[4] += sig_2_dom * sig_2_dom * (Psi_Func(lda_dom2, lda_dom2, option_maturity2, T1, T2));
		}
		else
		{
			var_fx2[4] += sig_2_dom * sig_2_dom * (Phi_Func(2.0 * lda_dom2, option_maturity2, T1, T2));
		}
	}
...
Err display_FX1F_TermStruct(	char			*szFxUndName,								
								long			*sigma_n,
								double			**sigma_time,
								double			**sigma
								)
...
/*-------------- Get the local volatility term structure---------------------------*/
...
/* Loop on all the elements of the Term Sturcture */
	...
	{	
		...
		*sigma = realloc (*sigma, (*sigma_n) * sizeof (double));
		(*sigma_time)[*sigma_n-1] = ((double) (((FxTermStructVal*)l->element->val.pval) -> date) - today) * YEARS_IN_DAY;
...
double	*quanto_adj)
{
	...
	if (pay_date < fix_date || start_date < fix_date || end_date <= start_date || fwd_start_date > fix_date)
	{
	...
	for (i = StartIndex; i < EndIndex + 1; i++)
	{
		...
		I3 += mergedomsig[i] * mergeforsig[i]
			...
			* (exp ((dom_lam + for_lam) * t2) - exp ((dom_lam + for_lam) * t1)) / (dom_lam + for_lam));
	...
	I1 *= corr_spot_fx_dom 
		* sigma_s
		* (exp (-dom_lam * start_time) - exp (-dom_lam * end_time))
		/ dom_lam / dom_lam / (end_time - start_time);
	I2 *= (exp (-dom_lam * start_time) - exp (-dom_lam * end_time))
		/ dom_lam / dom_lam / dom_lam / (end_time - start_time);
	I3 *= corr_dom_for * (exp (-dom_lam * start_time) - exp (-dom_lam * end_time))
		/ dom_lam / for_lam / (end_time - start_time);
	I4 *= ((exp (-dom_lam * start_time) - exp (-dom_lam * end_time)) / dom_lam / (end_time - start_time))
		* ((exp (-dom_lam * start_time) - exp (-dom_lam * end_time)) / dom_lam / (end_time - start_time))
		/ 2.0 / dom_lam;

	/*	Implied vol of Fx */
	err = Fx3DtsImpliedVol (pay_time, fwd_start_time, fix_time,
		mergesigtime, nmergesig, mergedomsig, dom_lam, mergeforsig, for_lam,
		&fx_fix_time, &sigma_s, 1,
		corr_dom_for, corr_spot_fx_dom, corr_spot_fx_for,
		&denomivol);
	if (err)
	{
		goto FREE_RETURN;
...
FREE_RETURN:
	...
	if (mergesigtime) free (mergesigtime);
	if (mergedomsig) free (mergedomsig);
	if (mergeforsig) free (mergeforsig);
	if (mergecorr) free (mergecorr);
	if (temp_fx_vol_curve) free (temp_fx_vol_curve);
					...
					double	*quanto_adj)
{
	...
	for (i = StartIndex; i < EndIndex + 1; i++)
	{
		...
		I1 += mergecorrdomfx[i] * mergedomsig[i] * (exp (dom_lam * t2) - exp (dom_lam * t1));
		I2 += mergecorr[i] * mergedomsig[i] * mergedomsig[i]
			* (exp (dom_lam * t2) - exp (dom_lam * t1)
			- exp (-dom_lam * pay_time) 
			* (exp (2.0 * dom_lam * t2) - exp (2.0 * dom_lam * t1))/ 2.0);
	...
	I2 *= (exp (-dom_lam * start_time) - exp (-dom_lam * end_time))
		/ dom_lam / dom_lam / dom_lam / (end_time - start_time);
	I3 *= (exp (-dom_lam * start_time) - exp (-dom_lam * end_time))
		/ dom_lam / for_lam / (end_time - start_time);
	I4 *= ((exp (-dom_lam * start_time) - exp (-dom_lam * end_time)) / dom_lam / (end_time - start_time))
		* ((exp (-dom_lam * start_time) - exp (-dom_lam * end_time)) / dom_lam / (end_time - start_time))
		/ 2.0 / dom_lam;
	...
	/*	Implied vol of Fx */
	err = Fx3DtsImpliedVol_corr (pay_time, fwd_start_time, fix_time,
		mergesigtime, nmergesig, mergedomsig, dom_lam, mergeforsig, for_lam,
		&fx_fix_time, &sigma_s, 1,
		corr_times, corr_dom_for_ts, corr_dom_fx_ts, corr_for_fx_ts, corr_n_times,
		&denomivol);
	if (err)
	{
		goto FREE_RETURN;
...
FREE_RETURN:
	...
	if (mergeforsig) free (mergeforsig);
	if (mergecorr) free (mergecorr);
	if (mergecorrdomfor) free (mergecorrdomfor);
	if (mergecorrdomfx) free (mergecorrdomfx);
	if (mergecorrforfx) free (mergecorrforfx);
	if (temp_fx_vol_curve) free (temp_fx_vol_curve);
...
Err Get_FX_StochRate_TermStructures_corr(char	*underlying,
...
Err			err = NULL;
	...
	*correl_for_fx = NULL;
	...
	err = srt_f_display_FX_TermStruct (underlying, sigma_n_fx, sigma_date_fx, sigma_fx, correl_n, correl_date, &corr);
	if (err)
	{
		goto FREE_RETURN;
	}
...
FREE_RETURN:
	...
	if (err)
	{
		...
		*sigma_fx = NULL;
		...
		if (*correl_date) free (*correl_date);
		*correl_date = NULL;
...
/*	Fx Implied volatility */
/*  The rates maturity dates have to be merged ! */
Err Fx3DtsImpliedVol_corr(	double	opt_maturity,
							double	start_date,
							double	end_date,
							double	*maturity_rates,
							long	nbMat,
	...
	for (idxopt = 0 ; idxopt <= nbrOpt-1 ; idxopt++)
	{
		...
		for (i=StartIndex; i<EndIndex + 1; i++)
		{
			...
			for (j=StartIndex2; j<EndIndex2 + 1; j++)
			{
				...
				err = Coefs_Partial_Var( val_opt,  ta,  tb,
										sig_dom,  lda_dom,
										sig_for,  lda_for,
										correl_dom_for[j],  correl_dom_fx[j],  correl_for_fx[j],
										&a_part, &b_part, &c_part);
...
{
long		sigma_n_dom, tau_n_dom, sigma_n_for, tau_n_for, sigma_n_fx, nb_merge_dates, nb_corr;
double		*sigma_date_dom	= NULL, 
			*sigma_dom		= NULL;
double		*tau_date_dom	= NULL, 
			*tau_dom		= NULL;
...
Err			err = NULL;
	...
	err = Get_FX_StochRate_TermStructures_corr(	fx_underlying,
												&sigma_date_dom,
												&sigma_dom,
												&sigma_n_dom,
												&tau_date_dom,
	...
	err = Fx3DtsImpliedVol_corr(val_time,
								start_time,
								end_time,
								merge_dates,
								nb_merge_dates,
...
Err			err = NULL;
	...
	err = Fx3DtsCalibration_corr(exercise_opt,
								maturity_opt,
								vol_opt,
								nbropt,
								merge_dates,
	...
	for (i = StartIndex; i < EndIndex + 1; i++)
	{
		...
		for (j = StartIndex2; j < EndIndex2 + 1; j++)
		{
			...
			for (k = StartIndex3; k < EndIndex3 + 1; k++)
			{
				...
				adjust_partial = 
				correl_dom_fx[k] * sig_dom * Etha_Func (lda_dom, Tpay, ta, tb) * sig_fx 
				- correl_dom_for[k] * sig_dom * sig_for * Psi2_Func (lda_for, lda_dom, Tval, Tpay, ta, tb)
...
Err		err = NULL;
	...
	*expect = log (fx_fwd / spot_fx) - 0.5 * fx_vol * fx_vol * Tfix;
	...
	err = Fx3DtsFwdBetaAdjustment_corr(0.0, Tval, Tval, Tfix,
							merge_dates,  nb_merge_dates,
							sig_dom, lda_dom,
							sig_for, lda_for,   
							sigma_date_fx,  sigma_fx,  sigma_n_fx, 
...
Err Fx3DtsFwdPayAdjustment_corr(	
								...
								double	lda_for,
								double	*maturity_fx, double *sig_curve_fx, long nbrMat_fx, 
								double	*maturity_corr, double *correl_dom_for, double *correl_dom_fx, double *correl_for_fx, long nbrMat_corr, 
								/*	Result */
								double *adjust)
{
	double adj1, adj2;
...
Err		err = NULL;
	...
	err = Fx3DtsImpliedVol_corr( T,  0,  T,
							 maturity, nbMat,
							 sig_curve_dom1,  lda_dom1,
							 sig_curve_for1,  lda_for1,
							 maturity,  sig_curve_fx1, nbMat, 
							 maturity, correlations[0][1], correlations[0][2], correlations[1][2], nbMat,
							 &vol1);
	...
	err = Fx3DtsImpliedVol_corr( T,  0,  T,
							 maturity, nbMat,
							 sig_curve_dom2,  lda_dom2,
							 sig_curve_for2,  lda_for2,
							 maturity,  sig_curve_fx2, nbMat, 
							 maturity, correlations[3][4], correlations[3][5], correlations[4][5], nbMat,
							 &vol2);

	if (err)
		return err;

	err = Fx3DtsFxFwdCov_corr(	T, T,  0,  T,
						 maturity, nbMat,
						 sig_curve_dom1, lda_dom1,
						 sig_curve_for1, lda_for1,
						 sig_curve_fx1,
...
double	correl_dom_for, correl_dom_fx, correl_for_fx;
double	correl12,
		correl13_1, correl13_2, correl13_3,
		correl23_1, correl23_2, correl23_3;		
int		i;
long	StartIndex, EndIndex;
Err		err = NULL;
	...
	/*	Implied Fx Vol */
	err = Fx3DtsImpliedVol_corr(	
						val_mat, start_mat, end_mat,
						sig_dates, nb_sig_dates,
						sig_curve_dom, lda_dom,
						sig_curve_for, lda_for,
						sig_dates, sig_curve_fx, nb_sig_dates,
						sig_dates, correl_dom_for_ts,  correl_dom_fx_ts,  correl_for_fx_ts, nb_sig_dates, fx_vol);	
	...
	for (i = StartIndex; i < EndIndex + 1; i++)
	{
		...
		correl13_2 += correl_dom_for * sig_domfor * (x_dom - exp(-lda_for * val_mat) * x_domfor);
		correl13_3 += correl_dom_fx * sig_dom * sig_fx * x_dom;

		/* foreign fx */
		correl23_1 += sig_for2 * (x_for - exp(-lda_for * val_mat) * y_for);
		correl23_2 += correl_dom_for * sig_domfor * (x_for - exp(-lda_dom * val_mat) * x_domfor);
	...
	*dom_for_cor = correl12 * exp(-(lda_dom + lda_for) * val_mat) / ((*dom_vol) * (*for_vol) * mat);

	*dom_fx_cor = exp(-lda_dom * val_mat) * 
												(correl13_1 / lda_dom
												- correl13_2 / lda_for
												+ correl13_3) / ((*dom_vol) * (*fx_vol) * mat);
					  
	*for_fx_cor = exp(-lda_for * val_mat) * 
												(-correl23_1 / lda_for
												+ correl23_2 / lda_dom
												+ correl23_3) / ((*for_vol) * (*fx_vol) * mat);
...
Err srt_f_optrainbow3F(			double	T,
								double	K,
								double	nX,
								double	nY,
								char	*fx_und1,
...
double	*sigma_date_dom1= NULL,
		*sigma_dom1		= NULL,
		*tau_date_dom1	= NULL,
		*tau_dom1		= NULL,
		*sigma_date_dom2= NULL,
		*sigma_dom2		= NULL,
		*tau_date_dom2	= NULL,
		*tau_dom2		= NULL,
		*sigma_date_for1= NULL,
		*sigma_for1		= NULL,
		*tau_date_for1	= NULL,
		*tau_for1		= NULL,
		*sigma_fx1		= NULL,
		*sigma_date_for2= NULL,
		*sigma_for2		= NULL,
		*tau_date_for2	= NULL,
		*tau_for2		= NULL,
		*sigma_fx2		= NULL,
...
long	sigma_n_dom1, tau_n_dom1,
		sigma_n_dom2, tau_n_dom2,
		sigma_n_for1, tau_n_for1,
		sigma_n_for2, tau_n_for2,
		sigma_n_fx1, sigma_n_fx2,
		nb_corr_date1, nb_corr_date2,
		nb_dates;
...
Err					err = NULL;
	...
	{
		smessage("Be carreful, Fx underlyings must have the same domestic underlying");		
	}
	...
	err = Get_LGM_TermStructure (dom_name1, &sigma_date_dom1, &sigma_dom1, &sigma_n_dom1, &tau_date_dom1, &tau_dom1, &tau_n_dom1);
	if (err)
	{
		goto FREE_RETURN;
	}
	
	err = Get_LGM_TermStructure (for_name1, &sigma_date_for1, &sigma_for1, &sigma_n_for1, &tau_date_for1, &tau_for1, &tau_n_for1);	
	if (err)
	{
		goto FREE_RETURN;
	}
	...
	err = srt_f_display_FX_TermStruct (	fx_und1,
										&sigma_n_fx1,
										&sigma_date_fx1,
										&sigma_fx1,
										&nb_corr_date1,
	...
	for (i=0; i < nb_corr_date1; i++) corr_date1[i] = (corr_date1[i] - today) / 365.0;
	...
	err = Get_LGM_TermStructure (dom_name2, &sigma_date_dom2, &sigma_dom2, &sigma_n_dom2, &tau_date_dom2, &tau_dom2, &tau_n_dom2);
	if (err)
	{
		goto FREE_RETURN;
	}

	err = Get_LGM_TermStructure (for_name2, &sigma_date_for2, &sigma_for2, &sigma_n_for2, &tau_date_for2, &tau_for2, &tau_n_for2);	
	if (err)
	{
		goto FREE_RETURN;
	}
	...
	err = srt_f_display_FX_TermStruct (	fx_und2,
										&sigma_n_fx2,
										&sigma_date_fx2,
										&sigma_fx2,
										&nb_corr_date2,
	...
	for (i=0; i < nb_corr_date2; i++) corr_date2[i] = (corr_date2[i] - today) / 365.0;
	...
	if (!sig_curve_dom1 || !sig_curve_for1 || !sig_curve_fx1 ||
		!sig_curve_dom2 || !sig_curve_for2 || !sig_curve_fx2 ||
		!correlations)
	{
	...
	{
		sig_curve_dom1[i] = sigma_dom1[Get_Index(all_dates[i], sigma_date_dom1, sigma_n_dom1)];
		sig_curve_for1[i] = sigma_for1[Get_Index(all_dates[i], sigma_date_for1, sigma_n_for1)];
		sig_curve_fx1[i] = sigma_fx1[Get_Index(all_dates[i], sigma_date_fx1, sigma_n_fx1)];
		sig_curve_dom2[i] = sigma_dom2[Get_Index(all_dates[i], sigma_date_dom2, sigma_n_dom2)];
		sig_curve_for2[i] = sigma_for2[Get_Index(all_dates[i], sigma_date_for2, sigma_n_for2)];
		sig_curve_fx2[i] = sigma_fx2[Get_Index(all_dates[i], sigma_date_fx2, sigma_n_fx2)];

		/* get the correlations */

		err = srt_f_get_corr_from_CorrList(corr_list, dom_name1, for_name1, all_dates[i], &(correlations[0][1][i]));
		if (err)
		{
			goto FREE_RETURN;
		}
		
		err = srt_f_get_corr_from_CorrList(corr_list, dom_name1, fx_und1, all_dates[i], &(correlations[0][2][i]));
		if (err)
		{
			goto FREE_RETURN;
		}

		err = srt_f_get_corr_from_CorrList(corr_list, dom_name1, dom_name2, all_dates[i], &(correlations[0][3][i]));
		if (err)
		{
			goto FREE_RETURN;
		}	

		err = srt_f_get_corr_from_CorrList(corr_list, dom_name1, for_name2, all_dates[i], &(correlations[0][4][i]));
		if (err)
		{
			goto FREE_RETURN;
		}

		err = srt_f_get_corr_from_CorrList(corr_list, dom_name1, fx_und2, all_dates[i], &(correlations[0][5][i]));
		if (err)
		{
			goto FREE_RETURN;
		}

		err = srt_f_get_corr_from_CorrList(corr_list, for_name1, fx_und1, all_dates[i], &(correlations[1][2][i]));
		if (err)
		{
			goto FREE_RETURN;
		}

		err = srt_f_get_corr_from_CorrList(corr_list, for_name1, dom_name2, all_dates[i], &(correlations[1][3][i]));
		if (err)
		{
			goto FREE_RETURN;
		}	

		err = srt_f_get_corr_from_CorrList(corr_list, for_name1, for_name2, all_dates[i], &(correlations[1][4][i]));
		if (err)
		{
			goto FREE_RETURN;
		}

		err = srt_f_get_corr_from_CorrList(corr_list, for_name1, fx_und2, all_dates[i], &(correlations[1][5][i]));
		if (err)
		{
			goto FREE_RETURN;
		}

		err = srt_f_get_corr_from_CorrList(corr_list, fx_und1, dom_name2, all_dates[i], &(correlations[2][3][i]));
		if (err)
		{
			goto FREE_RETURN;
		}	

		err = srt_f_get_corr_from_CorrList(corr_list, fx_und1, for_name2, all_dates[i], &(correlations[2][4][i]));
		if (err)
		{
			goto FREE_RETURN;
		}

		err = srt_f_get_corr_from_CorrList(corr_list, dom_name2, for_name2, all_dates[i], &(correlations[3][4][i]));
		if (err)
		{
			goto FREE_RETURN;
		}

		err = srt_f_get_corr_from_CorrList(corr_list, dom_name2, fx_und2, all_dates[i], &(correlations[3][5][i]));
		if (err)
		{
			goto FREE_RETURN;
		}

		err = srt_f_get_corr_from_CorrList(corr_list, fx_und1, fx_und2, all_dates[i], &(correlations[2][5][i]));
		if (err)
		{
			goto FREE_RETURN;
		}
		...
		err = srt_f_get_corr_from_CorrList(corr_list, for_name2, fx_und2, all_dates[i], &(correlations[4][5][i]));
		if (err)
		{
			goto FREE_RETURN;
		}
...
Err		err = NULL;
	...
	*expect = log (fx_fwd / spot_fx) - 0.5 * fx_vol * fx_vol * Tfix;
	...
	err = Fx3DtsFwdPayAdjustment_corr(0.0, Tval, Tval, Tpay, Tfix,
							merge_dates,  nb_merge_dates,
							sig_dom, lda_dom,
							sig_for, lda_for,   
							sigma_date_fx,  sigma_fx,  sigma_n_fx, 
							correl_date, correl_dom_for,  correl_dom_fx,  correl_for_fx,  nb_corr, &adjustment);
...
Err Fx3DGenCorrelation(	char	*und1_name,
						char	*und2_name,
						double	val_time,
						double	start_time,
						double	end_time,
...
Err					err = NULL;
	...
	switch (get_underlying_type (und1))
	{
		...
		{
			if (get_mdltype_from_irund(und1) != LGM && get_mdldim_from_irund(und1) != ONE_FAC)
			{
		...
		default:
		{
			err = ("Underlying named %s must be of type LGM1F of FX_STOCH_RATES", und1_name);
			goto FREE_RETURN;
			break;
		}
	}
	...
	switch (get_underlying_type (und2))
	{
		case INTEREST_RATE_UND:
		{
			if (get_mdltype_from_irund(und2) != LGM && get_mdldim_from_irund(und2) != ONE_FAC)
			{
		...
		default:
		{
			err = ("Underlying named %s must be of type LGM1F of FX_STOCH_RATES", und2_name);
			goto FREE_RETURN;
			break;
		}
	}
	...
	/* Get all the term structures */
	err = Get_LGM_TermStructure2 (dom_name1, &sigma_date_dom1, &sigma_dom1, &sigma_n_dom1, &lda_dom1);	
	...
	if (is_fx1)
	{				
		err = Get_LGM_TermStructure2 (for_name1, &sigma_date_for1, &sigma_for1, &sigma_n_for1, &lda_for1);
		
		if (err) goto FREE_RETURN;

		lda_for1 = 1.0 / lda_for1;
		
		err = srt_f_display_FX_TermStruct (	und1_name,
											&sigma_n_fx1,
											&sigma_date_fx1,
											&sigma_fx1,
											&nb_corr_date1,
		...
		for (i=0; i<nb_corr_date1; i++) corr_date1[i] = (corr_date1[i] - today) *YEARS_IN_DAY;
		for (i=0; i<sigma_n_fx1; i++) sigma_date_fx1[i] = (sigma_date_fx1[i] - today) * YEARS_IN_DAY;		
	}

	err = Get_LGM_TermStructure2 (dom_name2, &sigma_date_dom2, &sigma_dom2, &sigma_n_dom2, &lda_dom2);
	...
	if (is_fx2)
	{				
		err = Get_LGM_TermStructure2 (for_name2, &sigma_date_for2, &sigma_for2, &sigma_n_for2, &lda_for2);
		
		if (err) goto FREE_RETURN;

		lda_for2 = 1.0 / lda_for2;
		
		err = srt_f_display_FX_TermStruct (	und2_name,
											&sigma_n_fx2,
											&sigma_date_fx2,
											&sigma_fx2,
											&nb_corr_date2,
		...
		for (i=0; i<nb_corr_date2; i++) corr_date2[i] = (corr_date2[i] - today) *YEARS_IN_DAY;
		for (i=0; i<sigma_n_fx2; i++) sigma_date_fx2[i] = (sigma_date_fx2[i] - today) * YEARS_IN_DAY;
	}		
	...
	if	(!sig_curve_dom1 || !sig_curve_dom2 ||
		...
		(is_fx2 && (!sig_curve_for2 || !sig_curve_fx2)) ||
		!correlations)
	{
		err = "Memory allocation failure in srt_f_optrainbow3F (2)";
		goto FREE_RETURN;
	...
	for (i=0; i<nb_dates; i++)
	{
		...
		{			
			...
			err = srt_f_get_corr_from_CorrList(corr_list, for_name1, und2_name, all_dates[i], &(correlations[1][5][i]));
			if (err) goto FREE_RETURN;

			err = srt_f_get_corr_from_CorrList(corr_list, und1_name, for_name2, all_dates[i], &(correlations[2][4][i]));
			if (err) goto FREE_RETURN;

			err = srt_f_get_corr_from_CorrList(corr_list, und1_name, und2_name, all_dates[i], &(correlations[2][5][i]));
			if (err) goto FREE_RETURN;
		...
		if (is_fx1)
		{
			...
			err = srt_f_get_corr_from_CorrList(corr_list, dom_name1, und1_name, all_dates[i], &(correlations[0][2][i]));
			if (err) goto FREE_RETURN;

			err = srt_f_get_corr_from_CorrList(corr_list, for_name1, und1_name, all_dates[i], &(correlations[1][2][i]));
			if (err) goto FREE_RETURN;

			err = srt_f_get_corr_from_CorrList(corr_list, for_name1, dom_name2, all_dates[i], &(correlations[1][3][i]));
			if (err) goto FREE_RETURN;

			err = srt_f_get_corr_from_CorrList(corr_list, und1_name, dom_name2, all_dates[i], &(correlations[2][3][i]));
			if (err) goto FREE_RETURN;
		...
		if (is_fx2)
		{
			...
			err = srt_f_get_corr_from_CorrList(corr_list, dom_name2, und2_name, all_dates[i], &(correlations[3][5][i]));
			if (err) goto FREE_RETURN;

			err = srt_f_get_corr_from_CorrList(corr_list, for_name2, und2_name, all_dates[i], &(correlations[4][5][i]));
			if (err) goto FREE_RETURN;

			err = srt_f_get_corr_from_CorrList(corr_list, dom_name1, for_name2, all_dates[i], &(correlations[0][4][i]));
			if (err) goto FREE_RETURN;
			
			err = srt_f_get_corr_from_CorrList(corr_list, dom_name1, und2_name, all_dates[i], &(correlations[0][5][i]));
			if (err) goto FREE_RETURN;
		}		
	}

	err = Fx3DtsFxFwdCov_corr(	val_time,
								val_time,
								start_time,
								end_time,
								all_dates,

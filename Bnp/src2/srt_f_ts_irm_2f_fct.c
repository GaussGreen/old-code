...
Err find_2f_tau(double time, TermStruct* l, double* tau1, double* tau2)
{
    ...
    ls = l->head;
    while ((ls != NULL) && (((TwoFacIrmTermStructVal*)ls->element->val.pval)->time < time))
    {
        ls = ls->next;
    }
    ...
    /* Take last */
    {
        *tau1 = ((TwoFacIrmTermStructVal*)l->tail->element->val.pval)->exp_ts[0].tau;
        *tau2 = ((TwoFacIrmTermStructVal*)l->tail->element->val.pval)->exp_ts[1].tau;
    }
    return NULL;
} /* END Err find_2f_tau(...) */
...
Err find_2f_sig(double time, TermStruct* l, double* sig1, double* sig2)
{
    ...
    {
        *sig1 = ((TwoFacIrmTermStructVal*)l->tail->element->val.pval)->exp_ts[0].sig;
        *sig2 = ((TwoFacIrmTermStructVal*)l->tail->element->val.pval)->exp_ts[1].sig;
    }
...
Err get_2f_sig2_rho_interp(
    double time1, double time2, TermStruct* l, double* sig1_s, double* sig2_s, double* rho)
{
    SrtLst *                ls1, *ls2;
    ...
    while ((ls1 != NULL) && (((TwoFacIrmTermStructVal*)ls1->element->val.pval)->time < time1))
    {
        ls1 = ls1->next;
    }
    ...
    ls2 = ls1;
    ind = 0;
    while ((ls2 != NULL) && (((TwoFacIrmTermStructVal*)ls2->element->val.pval)->time < time2))
    {
        ls2 = ls2->next;
        ind++;
    }
...
Err find_tf_beta(double time, TermStruct* l, double* beta1, double* beta2)
{
    ...
    {
        *beta1 = ((TwoFacIrmTermStructVal*)l->tail->element->val.pval)->exp_ts[0].beta;
        *beta2 = ((TwoFacIrmTermStructVal*)l->tail->element->val.pval)->exp_ts[1].beta;
    }
...
Err get_2f_G_funcs(double time, TermStruct* l, SrtTFTSMat* G)
{
    ...
    for (i = 0; i <= 1; i++)
    {
        (*G)[i][i] = tsval->reb_ts[i][i].G + temp2[i] * var[i] / pow(tsval->exp_ts[i].F, 2);
    }
    (*G)[0][1] = (*G)[1][0] =
        tsval->reb_ts[0][1].G + tempc1 * covar / tsval->exp_ts[0].F / tsval->exp_ts[1].F;
...
Err get_2f_H_funcs(double time, TermStruct* l, SrtTFTSMat* H)
{
    ...
    for (i = 0; i <= 1; i++)
    {
        for (j = 0; j <= 1; j++)
        {
            ...
            {
                tempc2[i][j] = (exp(-(lambda[i] + lambda[j]) * dt) - 1.0) / (lambda[i] + lambda[j]);
            }
            else
            {
                tempc2[i][j] = -dt;
    ...
    for (i = 0; i <= 1; i++)
    {
        for (j = 0; j <= 1; j++)
        {
            (*H)[i][j] = temp0[i] * tsval->reb_ts[i][j].H +
...
Err get_2f_J_funcs(double time, TermStruct* l, SrtTFTSVec* J)
{
    ...
    *J[0] = tsval->exp_ts[0].J + tsval->exp_ts[0].sig * temp2 / tsval->exp_ts[0].F;
    ...
    *J[1] = tsval->exp_ts[1].J + tsval->exp_ts[1].sig * temp2 / tsval->exp_ts[1].F;

...

{
  ...
  case COMPUTE_IRR:
    ...
    pv = value_SimpleSwap(&s, crv) / ldf;
    if (err = irr_range(s.dl.date, s.cpn.d, s.cpn.len, s.dl.type, s.dl.prev, pv,
                        s.sdp.basis_code, s.sdp.compd, &irr))
      return err;
    free_inSimpleSwap(&s);
    *answer = irr;
    break;
  case COMPUTE_DURATION:
    ...
    if (err = irr_range(s.dl.date, s.cpn.d, s.cpn.len, s.dl.type, s.dl.prev, pv,
      ...
      return err;
    if (err =
            duration_range(s.dl.date, s.cpn.d, s.cpn.len, s.dl.type, s.dl.prev,
                           s.sdp.basis_code, s.sdp.compd, irr, &duration))
      return err;
    free_inSimpleSwap(&s);
    *answer = duration;
    break;
  case COMPUTE_CONVEXITY:
    ...
    if (err = irr_range(s.dl.date, s.cpn.d, s.cpn.len, s.dl.type, s.dl.prev, pv,
      ...
      return err;
    if (err =
            convexity_range(s.dl.date, s.cpn.d, s.cpn.len, s.dl.type, s.dl.prev,
                            s.sdp.basis_code, s.sdp.compd, irr, &convexity))
      return err;
    free_inSimpleSwap(&s);
    *answer = convexity;
    break;
  case COMPUTE_THIRDMOMENT:
    ...
    if (err = irr_range(s.dl.date, s.cpn.d, s.cpn.len, s.dl.type, s.dl.prev, pv,
                        s.sdp.basis_code, s.sdp.compd, &irr))
      return err;
    if (err = thirdmoment_range(s.dl.date, s.cpn.d, s.cpn.len, s.dl.type,
                                s.dl.prev, s.sdp.basis_code, s.sdp.compd, irr,
                                &thirdmoment))
      return err;
    free_inSimpleSwap(&s);
    *answer = thirdmoment;
  ...
  case COMPUTE_MODIFIED_DURATION:
    ...
    if (err = irr_range(s.dl.date, s.cpn.d, s.cpn.len, s.dl.type, s.dl.prev, pv,
                        s.sdp.basis_code, s.sdp.compd, &irr))
      return err;
    if (err = modified_duration_range(s.dl.date, s.cpn.d, s.cpn.len, s.dl.type,
                                      s.dl.prev, s.sdp.basis_code, s.sdp.compd,
                                      irr, &duration))
      return err;
    free_inSimpleSwap(&s);
    *answer = duration;
  ...
  case COMPUTE_MODIFIED_CONVEXITY:
    ...
    if (err = irr_range(s.dl.date, s.cpn.d, s.cpn.len, s.dl.type, s.dl.prev, pv,
                        s.sdp.basis_code, s.sdp.compd, &irr))
      return err;
    if (err = modified_convexity_range(s.dl.date, s.cpn.d, s.cpn.len, s.dl.type,
                                       s.dl.prev, s.sdp.basis_code, s.sdp.compd,
                                       irr, &convexity))
      return err;
    free_inSimpleSwap(&s);
...
/* ------------------------------------------------------------------------------
   ...
   cashflows ( a bond  , for instance) (we assume no initial exchange  , and 1.0
   as the final one) PV is the pv of the swap at the swap start date All the
   calculations are done on the CLEAN price (which is not bond math)
   ------------------------------------------------------------------------------
 */

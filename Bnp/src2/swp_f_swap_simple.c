...
Err zcsens(String tenor, SrtCurvePtr yldcrv, double* answer)
{
    ...
    if (err = string_to_YCInstr(
            tenor,
            &instr,
            NULL,
            ccy_param,
            add_unit(today, ccy_param->spot_lag, SRT_BDAY, MODIFIED_SUCCEEDING)))
    {
        return err;
    }
    ...
    case SWAPINSTR:
        ...
        *answer = sens;
        break;
    default:
        dt = coverage(instr.dp.futdates.start, instr.dp.futdates.end, instr.dp.futdates.basis_code);
    ...
    SwapDP* sdp, double strike, double ini, double fin, Date today, StructType t)
{
    ...
    err = cpn_list(
        sdp->basis_code, sdp->compd, start_date, end_date, cfs.dl, strike, ini, fin, t, &(cfs.cpn));
...
/* ------------------------------------------------------------------------------
   ...
   ------------------------------------------------------------------------------ */
Err bond_modified_duration(SwapDP* sdp, double coupon, double pv, double* answer)
{
    SimpleSwap s;
    double     irr;
    double     duration;
...
Err bond_modified_convexity(SwapDP* sdp, double coupon, double pv, double* answer)
{
    SimpleSwap s;
    double     irr;
    double     convexity;

...
static void copy_grfn_cell(GrfnCell* to, GrfnCell* from)
{
    ...
    if (from->type != GRFNBCELL && from->sval != NULL && from->str_alloced == SRT_YES)
    {
...
static void insert_blank_evt(
    long date, int insert_before, long* tab_rw, long tab_cl, long** evt_dts, GrfnCell*** tab)
{
    int        i, j;
    GrfnCell** new_tab;
    long*      new_evt_dts;
...
static void calc_det_df(GrfnEvent* event, SrtSample* sample, SrtUndInfo* und_info)
{
    SrtUndPtr und;
    long      ref;
    char*     yc_name;
    ...
    GRFNCOMMSTRUCT comm)
{
    ...
    if (num_evt_dts != tab_rw)
    {
        ...
        return serror(
            "Number of event dates (%d) is inconsistent "
    ...
    if (err = grfn_check_tableau_and_dates(&loc_tab_rw, loc_tab_cl, tab, loc_evt_dts))
    {
        free(gd);
        free(und_info);
        free(loc_evt_dts);
    ...
    {
        /*	Insert back	*/
        insert_blank_evt(first_stop, loc_tab_rw, &loc_tab_rw, loc_tab_cl, &loc_evt_dts, &tab);
        first_stop_idx = loc_tab_rw - 1;
    }
    else if (loc_evt_dts[loc_tab_rw - 1] == first_stop)
    {
        /*	Insert blank event after today */
        insert_blank_evt(first_stop + 1, loc_tab_rw, &loc_tab_rw, loc_tab_cl, &loc_evt_dts, &tab);
    ...
    if (err = grfn_copy_to_GrfnDeal(
            gd, loc_tab_rw, loc_tab_cl, tab, loc_evt_dts, 0, NULL, aux_width, aux_len, aux))
    {
        free(gd);
        free(und_info);
        free(loc_evt_dts);
    ...
    for (dte_idx = 0; dte_idx < first_stop_idx; dte_idx++)
    {
        ...
        if (err = grfn_create_historical_event(
                gd, &(event[dte_idx]), loc_evt_dts[dte_idx], loc_evt_dts[dte_idx + 1]))
        {
            free(und_info);
            free(loc_evt_dts);
            grfn_free_GrfnCellmatrix(tab, loc_tab_rw, loc_tab_cl);
    ...
    *back_or_for = (prm->imp_type == IMPMONTECARLO ? 1 : -1);
    ...
    if (err = grfn_attach_past_info_to_future(gd, &(event[first_stop_idx - front_evt])))
    {
        free(und_info);
        free(loc_evt_dts);
        grfn_free_GrfnCellmatrix(tab, loc_tab_rw, loc_tab_cl);
    ...
    for (dte_idx = 0, dte_idx2 = first_stop_idx; dte_idx2 < loc_tab_rw; dte_idx++, dte_idx2++)
    {
        ...
        comm->tms[dte_idx + front_evt] = (double)(loc_evt_dts[dte_idx2] - ref) / DAYS_IN_YEAR;
    }
...
Err FIRSTFreeEventInfoFromDeal(
    int           num_evt,
    int           num_und,
    int           max_num_df,
    FIRSTMktAtT** evt,
    ...
    double****    df_mat_tms)
{
    ...
    if (*df_mat_dts)
        free_l3tensor(*df_mat_dts, 0, num_evt - 1, 0, num_und - 1, 0, max_num_df - 1);
    if (*df_mat_tms)
        free_f3tensor(*df_mat_tms, 0, num_evt - 1, 0, num_und - 1, 0, max_num_df - 1);
...
Err SrtInitDetermUnd(char* undName, char* ycName)
{
    ...
    err = srt_f_lstins(
        und_list, undName, 0.0, OBJ_PTR_UND, und, SrtFreeDetermUnd, &(und->underl_ticker));
    ...
    double* price)
{
    ...
    /*	Get actual event dates (as opposed to initial ones, today is added
            and empty events are removed)	*/
    ...
    if (err = FIRSTGetEventInfoFromDeal(
            &comm, num_evt, num_und, max_num_df, &evt, &am, &num_df_mat, &df_mat_dts, &df_mat_tms))
    {
        FIRSTFreeMktStruct(&comm);
        FIRSTFreeUndFromDeal(num_und, &und_ptr);
    ...
    /*	Main loop	*/
    for (i = (back_or_for == 1 ? 0 : num_evt - 1); (back_or_for == 1 ? (i < num_evt) : (i > -1));
         i += back_or_for)
    {
        ...
        for (j = 0; j < num_und; j++)
        {
            for (k = 0; k < num_df_mat[i][j]; k++)
            {
                if (err = FIRSTSetDFValue(
                        evt + i, j, k, swp_f_df(evt_dts[i], df_mat_dts[i][j][k], yc_name[j])))
                {
                    FIRSTFreeMktStruct(&comm);
                    FIRSTFreeUndFromDeal(num_und, &und_ptr);
                    FIRSTFreeEvtDatesFromDeal(num_evt, &evt_dts, &evt_tms);
                    FIRSTFreeEventInfoFromDeal(
                        num_evt,
                        num_und,
                        max_num_df,
                        &evt,
        ...
        /*	Value event	*/
        if (err = FIRSTEvalEvent(&comm, evt + i, num_col, 2, NULL, NULL, pvs, &cf))
        {
            FIRSTFreeMktStruct(&comm);
            FIRSTFreeUndFromDeal(num_und, &und_ptr);
            FIRSTFreeEvtDatesFromDeal(num_evt, &evt_dts, &evt_tms);
            FIRSTFreeEventInfoFromDeal(
                num_evt, num_und, max_num_df, &evt, &am, &num_df_mat, &df_mat_dts, &df_mat_tms);
            free(yc_name);
            free(pvs);
            return err;
        }
    ...
    if (back_or_for == -1)
    {
        *price = pv_of_past + pvs[num_col - 1] * swp_f_df(ref, evt_dts[0], yc_name[0]);
    }

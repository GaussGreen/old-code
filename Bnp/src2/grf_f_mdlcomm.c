...
GRFNCOMMSTRUCT			comm)
{
	...
	if (err = srt_f_set_GrfnCell (	loc_tab_rw,
									loc_tab_cl,
									tab_str,
									mask,
									&tab))
	...
	if (err = grfn_check_tableau_and_dates (	&loc_tab_rw,
												loc_tab_cl, 
												tab,
												loc_evt_dts))
	{
...
*/
	...
	if (err = grfn_copy_to_GrfnDeal (	gd, 
										loc_tab_rw, 
										loc_tab_cl, 
										tab, 
										loc_evt_dts,
    ...
    for (dte_idx=0; dte_idx<first_stop_idx; dte_idx++)
	{
		...
		if (err = grfn_create_historical_event (	gd, 
													&(event[dte_idx]),
													loc_evt_dts[dte_idx],
													loc_evt_dts[dte_idx+1]))
		{
			...
			grfn_free_inGrfnDeal (gd);	
			free (gd);
			for (i=0; i<loc_tab_rw; i++) grfn_free_GrfnEvent (event[i]);
			free (event);
			return err ;
		}
    } 
	...
	{
		/*	Create Grfn event */
		if (err = grfn_create_future_event (		gd,
													&(event[dte_idx]),
													loc_evt_dts[dte_idx],
	...
	*back_or_for = (prm->imp_type == IMPMONTECARLO? 1: -1);
	...
	if (		first_stop_idx > 0 
			&&	loc_evt_dts[first_stop_idx] > first_stop
			&&	GrfnIsStatus (	gd->rowstatus[first_stop_idx-1], 
								GRFNCSAMERICAN))
	{
		front_evt = 1;
	}
	...
	if (err = grfn_attach_past_info_to_future (	
				gd, 
				&(event[first_stop_idx-front_evt])))
	{
		free (und_info);
		free (loc_evt_dts);
		grfn_free_GrfnCellmatrix (tab, loc_tab_rw, loc_tab_cl);
	...
	for (	dte_idx=0, dte_idx2=first_stop_idx; 
			dte_idx2<loc_tab_rw; 
			dte_idx++, dte_idx2++)
	{
		comm->evt[dte_idx+front_evt].evt = event[dte_idx2];
		comm->am[dte_idx+front_evt] = (GrfnIsStatus (	
			gd->rowstatus[dte_idx2], 
			GRFNCSAMERICAN)? 
					1: 
					0);
					 
		comm->dts[dte_idx+front_evt] = loc_evt_dts[dte_idx2];
		comm->tms[dte_idx+front_evt] = (double) (loc_evt_dts[dte_idx2] - ref) 
									/ DAYS_IN_YEAR;
	}
	...
	free (loc_evt_dts);
	grfn_free_GrfnCellmatrix (tab, loc_tab_rw, loc_tab_cl);
	for (i=0; i<first_stop_idx-front_evt; i++) grfn_free_GrfnEvent (event[i]);
	free (event);
...
Err FIRSTFreeUndFromDeal(
int						num_und,
SrtUndPtr**				und_ptr)
{
	if (*und_ptr) free (*und_ptr);
	return NULL;
}
...
Err FIRSTFreeEvtDatesFromDeal(
int						num_evt,
long**					evt_dts,
double**				evt_tms)
{
	if (*evt_dts) free (*evt_dts);
	if (*evt_tms) free (*evt_tms);
...
double****				df_mat_tms)
{
	if (*num_df_mat) free_imatrix (*num_df_mat, 0, num_evt-1, 0, num_und-1);
	if (*df_mat_dts) free_l3tensor (*df_mat_dts, 0, num_evt-1, 0, num_und-1, 0, max_num_df-1);
	if (*df_mat_tms) free_f3tensor (*df_mat_tms, 0, num_evt-1, 0, num_und-1, 0, max_num_df-1);
...

{
	if (evt && evt->evt)
	{
		return grfn_eval_event_ammc (	type_eval,
										evt->evt,
										&(evt->smp),
										comm->gd,
										col_pvs,
...

{
	if (evt && evt->evt)
	{
		return grfn_eval_event_credit (	type_eval,
										evt->evt,
										&(evt->smp),
										comm->gd,
										col_pvNR,
...
double					*price)
{
	...
	if (err = FIRSTInitMktStruct (	init_num_evt_dts,
									init_evt_dts,
									tab_rw,
									tab_cl,
									tab_str,
	...
	if (err = FIRSTGetUndFromDeal (	&comm,
									&num_und,
									&und_ptr))
	{
	...
	if (err = FIRSTGetEventInfoFromDeal (	&comm, 
											num_evt, 
											num_und,
											max_num_df,
											&evt,
		...
		for (j=0; j<num_und; j++)
		{
			for (k=0; k<num_df_mat[i][j]; k++)
			{
														...
														yc_name[j])))
				{
					...
					FIRSTFreeUndFromDeal (num_und, &und_ptr);
					FIRSTFreeEvtDatesFromDeal (num_evt, &evt_dts, &evt_tms);
					FIRSTFreeEventInfoFromDeal (	num_evt,
													num_und,
													max_num_df,
													&evt,
													&am,
	...
	if (back_or_for == -1)
	{
		*price = pv_of_past + pvs[num_col-1] * swp_f_df (	ref,
															evt_dts[0],
															yc_name[0]);
	}

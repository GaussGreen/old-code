/* ==========================================================
	...
	PURPOSE:			Calculates One Time Callables using a smile on the Fx by a copula based method
	...
	(TYPE) == 0?																					\
		0.0:(																						\
			...
			POS_VAL ((FWD) - (STRIKE)):(															\
				...
				POS_VAL ((STRIKE) - (FWD)):(														\
					(TYPE) == 3?																	\
						CALL_VAL ((FWD), (STRIKE), log((FWD)/(STRIKE))/(STD)-(HALF_STD), (STD)):	\
							PUT_VAL ((FWD), (STRIKE), log((FWD)/(STRIKE))/(STD)-(HALF_STD), (STD))))))	

/* =========================================================================
OTC STRUCTURE
==========================================================================*/
void cpd_init_otc_params(	otcpd_params *otc_params,
							CPD_STR		cpd,
							int			ncall,
							int			nSimul, 
							int			Do_pecs, 
...
int i;
	...
	otc_params->OTCcalculate = (int*) calloc(otc_params->Options_number, sizeof(int));
	...
	else
	{
		...
		{
			if(i < keepFirstN || (i >= keepFirstN && (double)(i - keepFirstN + 1) / oneOutOfN == (i - keepFirstN + 1) / oneOutOfN))
				otc_params->OTCcalculate[i] = 1;
		}
	}
}
...
Err	err = NULL;
	...
	if(otc_params->COPULAgauss)
		free_dmatrix( otc_params->COPULAgauss, 0, otc_params->COPULAnSimul - 1, 0, 3 - 1);

	if(otc_params->CUMUL)
		free_dmatrix( otc_params->CUMUL, 0, 2 - 1, 0, otc_params->CUMULnPoints - 1);

	if(otc_params->COPULAmatrix)
		free_dmatrix( otc_params->COPULAmatrix, 0, otc_params->COPULAnSimul - 1, 0, 3 - 1 + otc_params->OTCsmileAndFlat);

	if(otc_params->KO_FxGauss)
		free_dmatrix( otc_params->KO_FxGauss, 0, otc_params->COPULAnSimul - 1, 0, otc_params->Options_number - 1);

	if(otc_params->KO_FxMatrix)
		free_dmatrix( otc_params->KO_FxMatrix, 0, otc_params->COPULAnSimul - 1, 0, otc_params->Options_number - 1);

	if(otc_params->KO_RatesGauss)
		free_dmatrix( otc_params->KO_RatesGauss, 0, otc_params->COPULAnSimul - 1, 0, 2 - 1);

	if(otc_params->KO_RatesMatrix)
		free_dmatrix( otc_params->KO_RatesMatrix, 0, otc_params->COPULAnSimul - 1, 0, 2 - 1);
...
void cpd_free_otc_precalc(	otcpd_precalc	*otc_precalc,
							int				nCall,
							int				nCoupons_pd,
							int				nCoupons_fund)
{
	if(otc_precalc->df_const_dom_fx_val) free_dmatrix(otc_precalc->df_const_dom_fx_val, 0, nCall - 1, 0, nCoupons_pd + 2);
	if(otc_precalc->df_lin_dom_fx_val) free_dmatrix(otc_precalc->df_lin_dom_fx_val, 0, nCall - 1, 0, nCoupons_pd + 2);
	if(otc_precalc->df_const_for_fx_val) free_dmatrix(otc_precalc->df_const_for_fx_val, 0, nCall - 1, 0, nCoupons_pd + 2);
	if(otc_precalc->df_lin_for_fx_val) free_dmatrix(otc_precalc->df_lin_for_fx_val, 0, nCall - 1, 0, nCoupons_pd + 2);
	if(otc_precalc->df_const_pd_pay) free_dmatrix(otc_precalc->df_const_pd_pay, 0, nCall - 1, 0, nCoupons_pd + 2);
	if(otc_precalc->df_lin_pd_pay) free_dmatrix(otc_precalc->df_lin_pd_pay, 0, nCall - 1, 0, nCoupons_pd + 2);
	if(otc_precalc->df_const_fund_pay) free_dmatrix(otc_precalc->df_const_fund_pay, 0, nCall - 1, 0, nCoupons_fund + 2);
	if(otc_precalc->df_lin_fund_pay) free_dmatrix(otc_precalc->df_lin_fund_pay, 0, nCall - 1, 0, nCoupons_fund + 2);
	if(otc_precalc->df_const_fund_start) free(otc_precalc->df_const_fund_start);
	if(otc_precalc->df_lin_fund_start) free(otc_precalc->df_lin_fund_start);

	if(otc_precalc->pd_fwd3F_vol) free_dmatrix(otc_precalc->pd_fwd3F_vol,0, nCall, 0, nCoupons_pd +2);
	if(otc_precalc->pd_fwdsmile_alpha) free_dmatrix(otc_precalc->pd_fwdsmile_alpha,0, nCall, 0, nCoupons_pd +2);
	if(otc_precalc->pd_fwdsmile_beta) free_dmatrix(otc_precalc->pd_fwdsmile_beta,0, nCall, 0, nCoupons_pd +2);
	if(otc_precalc->pd_fwdsmile_rho) free_dmatrix(otc_precalc->pd_fwdsmile_rho,0, nCall, 0, nCoupons_pd +2);
	if(otc_precalc->pd_fwdsmile_pi) free_dmatrix(otc_precalc->pd_fwdsmile_pi,0, nCall, 0, nCoupons_pd +2);
	if(otc_precalc->pd_fwdsmile_vol) free_dmatrix(otc_precalc->pd_fwdsmile_vol,0, nCall, 0, nCoupons_pd +2);

	if(otc_precalc->FxAdj) free_dmatrix(otc_precalc->FxAdj,0, nCall - 1, 0, nCoupons_pd - 1);
	if(otc_precalc->FeeFxAdj) free(otc_precalc->FeeFxAdj);

	if(otc_precalc->NotFxAdj) free_dmatrix(otc_precalc->NotFxAdj,0, nCall - 1, 0, 2);
	if(otc_precalc->NotFeeFxAdj) free(otc_precalc->NotFeeFxAdj);

	if(otc_precalc->KO_barrier) free_dmatrix(otc_precalc->KO_barrier,0, nCall - 1, 0, nCoupons_pd - 1);
}

Err cpd_alloc_otc_precalc(	otcpd_precalc	*otc_precalc,
							int				nCall,
							int				nCoupons_pd,
							int				nCoupons_fund,
							int				nMktDates)
...
Err err = NULL;
	...
	otc_precalc->NotFeeFxAdj = calloc(nCall + 1, sizeof(double)); /* +1 for the last notional */

	otc_precalc->KO_barrier = dmatrix(0, nCall - 1, 0, nCoupons_pd - 1);

	if(	!otc_precalc->pd_fwd3F_vol || !otc_precalc->pd_fwdsmile_vol || 
		!otc_precalc->pd_fwdsmile_alpha || !otc_precalc->pd_fwdsmile_beta || !otc_precalc->pd_fwdsmile_rho || 
		!otc_precalc->pd_fwdsmile_pi || !otc_precalc->FxAdj || !otc_precalc->FeeFxAdj || !otc_precalc->KO_barrier)
	{
...
/* =========================================================================
 Precalculations
========================================================================= */
Err cpd_otc_precalc(	otcpd_precalc		*otc_precalc,
...
Err		err = NULL;
	...
	err  = cpd_alloc_otc_precalc(otc_precalc, cpd->num_calls, cpd->pd_leg->num_cpn, cpd->fund_leg->num_cpn, smile_mkt->num_vols);
	...
	if(cpd->fund_leg->dom_for && OTC_params->SpeedFunding)
	{
		...
		{
			if(cpd->fund_leg->cpn[cpd->call[k].fund_idx].pay_date > cpd->pd_leg->cpn[cpd->call[k].pd_idx].fx_val_date)
	...
	for(i = 0; i < cpd->num_calls; i++)
	{
		...
		beta = (1.0 - exp( - fund_lda * (cpd->fund_leg->cpn[cpd->call[i].fund_idx].start_time - cpd->call[i].ex_time))) / fund_lda;
		...
		{
			beta = (1.0 - exp( - und->lda_dom * (cpd->pd_leg->cpn[j].fx_val_time - cpd->call[i].ex_time))) / und->lda_dom;
			...
			{
				beta = (1.0 - exp( - und->lda_dom * (cpd->pd_leg->cpn[j].pay_time - cpd->call[i].ex_time))) / und->lda_dom;
			...
			else
			{
				...
				otc_precalc->df_const_pd_pay[i][j] = otc_precalc->df_const_dom_fx_val[i][j];
			}

			beta = (1.0 - exp( - und->lda_for * (cpd->pd_leg->cpn[j].fx_val_time - cpd->call[i].ex_time))) / und->lda_for;
		...
		{
			beta = (1.0 - exp( - fund_lda * (cpd->fund_leg->cpn[j].pay_time - cpd->call[i].ex_time))) / fund_lda;
			otc_precalc->df_lin_fund_pay[i][j] = - beta;
			otc_precalc->df_const_fund_pay[i][j] = - 0.5 * beta * beta * fund_phi;
		}

		beta = (1.0 - exp( - und->lda_dom * (cpd->pd_leg->not_ref.fx_val_time - cpd->call[i].ex_time))) / und->lda_dom;
		otc_precalc->df_lin_dom_fx_val[i][cpd->pd_leg->num_cpn] = - beta;
		otc_precalc->df_const_dom_fx_val[i][cpd->pd_leg->num_cpn] = - 0.5 * beta * beta * phiDom;

		if(cpd->pd_leg->not_ref.fx_val_time != cpd->pd_leg->not_ref.pay_time)
		{
			beta = (1.0 - exp( - und->lda_dom * (cpd->pd_leg->not_ref.pay_time - cpd->call[i].ex_time))) / und->lda_dom;
			otc_precalc->df_lin_pd_pay[i][cpd->pd_leg->num_cpn] = - beta;
			otc_precalc->df_const_pd_pay[i][cpd->pd_leg->num_cpn] = - 0.5 * beta * beta * phiDom;
		}
		else
		{
			otc_precalc->df_lin_pd_pay[i][cpd->pd_leg->num_cpn] = otc_precalc->df_lin_dom_fx_val[i][cpd->pd_leg->num_cpn];
			otc_precalc->df_const_pd_pay[i][cpd->pd_leg->num_cpn] = otc_precalc->df_const_dom_fx_val[i][cpd->pd_leg->num_cpn];
		}

		beta = (1.0 - exp( - und->lda_for * (cpd->pd_leg->not_ref.fx_val_time - cpd->call[i].ex_time))) / und->lda_for;
		otc_precalc->df_lin_for_fx_val[i][cpd->pd_leg->num_cpn] = - beta;
		otc_precalc->df_const_for_fx_val[i][cpd->pd_leg->num_cpn] = - 0.5 * beta * beta * phiFor;
		...
		for(j = 0; j < 2; j++)
		{
			...
			{
				beta = (1.0 - exp( - und->lda_dom * (cpd->call[i+j].fx_val_time - cpd->call[i].ex_time))) / und->lda_dom;
				otc_precalc->df_lin_dom_fx_val[i][cpd->pd_leg->num_cpn + j + 1] = - beta;
				otc_precalc->df_const_dom_fx_val[i][cpd->pd_leg->num_cpn + j + 1] = - 0.5 * beta * beta * phiDom;

				if(cpd->call[i+j].fx_val_time != cpd->call[i+j].set_time)
				{
					beta = (1.0 - exp( - und->lda_dom * (cpd->call[i+j].set_time - cpd->call[i].ex_time))) / und->lda_dom;
					otc_precalc->df_lin_pd_pay[i][cpd->pd_leg->num_cpn + j + 1] = - beta;
					otc_precalc->df_const_pd_pay[i][cpd->pd_leg->num_cpn + j + 1] = - 0.5 * beta * beta * phiDom;
				}
				else
				{
					otc_precalc->df_lin_pd_pay[i][cpd->pd_leg->num_cpn + j + 1] = otc_precalc->df_lin_dom_fx_val[i][cpd->pd_leg->num_cpn + j + 1];
					otc_precalc->df_const_pd_pay[i][cpd->pd_leg->num_cpn + j + 1] = otc_precalc->df_const_dom_fx_val[i][cpd->pd_leg->num_cpn + j + 1];
				}

				beta = (1.0 - exp( - und->lda_for * (cpd->call[i+j].fx_val_time - cpd->call[i].ex_time))) / und->lda_for;
				otc_precalc->df_lin_for_fx_val[i][cpd->pd_leg->num_cpn + j + 1] = - beta;
				otc_precalc->df_const_for_fx_val[i][cpd->pd_leg->num_cpn + j + 1] = - 0.5 * beta * beta * phiFor;

				beta = (1.0 - exp( - fund_lda * (cpd->call[i+j].set_time - cpd->call[i].ex_time))) / fund_lda;
				otc_precalc->df_lin_fund_pay[i][cpd->fund_leg->num_cpn + j + 1] = - beta;
				otc_precalc->df_const_fund_pay[i][cpd->fund_leg->num_cpn + j + 1] = - 0.5 * beta * beta * fund_phi;
	...
	for(i = 0; i < cpd->num_calls; i++)
	{
		for(j = 0; j < cpd->pd_leg->num_cpn; j++)
		{
			...
			{
				err = Fx3DtsFwdPayAdjustment_corr(	cpd->call[i].ex_time,
													cpd->pd_leg->cpn[j].fx_val_time,
													cpd->pd_leg->cpn[j].fx_val_time,
													cpd->pd_leg->cpn[j].pay_time,
													cpd->pd_leg->cpn[j].fx_fix_time,
													und->sigma_time_rates,
													und->sigma_n_rates,
													und->sigma_dom,
													und->lda_dom,
	...
	for (j = 0; j < cpd->pd_leg->num_cpn; j++)
	{
		err = Fx3DtsFwdPayAdjustment_corr(	0.0,
											cpd->pd_leg->cpn[j].fx_val_time,
											cpd->pd_leg->cpn[j].fx_val_time,
											cpd->pd_leg->cpn[j].pay_time,
											cpd->pd_leg->cpn[j].fx_fix_time,
	...
	for(i = 0; i < cpd->num_calls; i++)
	{
		for(j = 0; j < 2; j++)
		{
			if(i+j < cpd->num_calls && cpd->call->use_opt_str)
			{
				...
				{
					err = Fx3DtsFwdPayAdjustment_corr(	cpd->call[i].ex_time,
														cpd->call[i+j].fx_val_time,
														cpd->call[i+j].fx_val_time,
														cpd->call[i+j].set_time,
														cpd->call[i+j].fx_fix_time,
														und->sigma_time_rates,
														und->sigma_n_rates,
														und->sigma_dom,
														und->lda_dom,
		...
		err = Fx3DtsFwdPayAdjustment_corr(	cpd->call[i].ex_time,
											cpd->pd_leg->not_ref.fx_val_time,
											cpd->pd_leg->not_ref.fx_val_time,
											cpd->pd_leg->not_ref.pay_time,
											cpd->pd_leg->not_ref.fx_fix_time,
											und->sigma_time_rates,
											und->sigma_n_rates,
											und->sigma_dom,
											und->lda_dom,
	...
	if(cpd->call->use_opt_str)
	{
		...
		{
			err = Fx3DtsFwdPayAdjustment_corr(	0.0,
												cpd->call[j].fx_val_time,
												cpd->call[j].fx_val_time,
												cpd->call[j].set_time,
												cpd->call[j].fx_fix_time,
												und->sigma_time_rates,
												und->sigma_n_rates,
												und->sigma_dom,
												und->lda_dom,
	...
	otc_precalc->NotFeeFxAdj[cpd->num_calls] = exp(otc_precalc->NotFeeFxAdj[cpd->num_calls]);
	...
	if(cpd->type)
	{
		for(i = 0; i < cpd->num_calls; i++)
		{
			for(j = 0; j < cpd->num_calls; j++)
			{
				...
				{
					err = Fx3DtsFwdPayAdjustment_corr(	0.0,
														cpd->call[j].ex_time2bd,
														cpd->call[j].ex_time2bd,
														cpd->call[i].ex_time2bd,
														cpd->call[j].ex_time,
														und->sigma_time_rates,
														und->sigma_n_rates,
														und->sigma_dom,
	...
	for(i = 0; i < cpd->pd_leg->num_cpn; i++)
		forwardVec[i] = und->spot_fx * swp_f_df( und->today, cpd->pd_leg->cpn[i].fx_val_date, und->for_yc)
					/ swp_f_df (und->today, cpd->pd_leg->cpn[i].fx_val_date, und->dom_yc);;

	forwardVec[cpd->pd_leg->num_cpn] = und->spot_fx 
	...
	for(j = 0; j < cpd->num_calls; j++)
	{
		for(i = 0; i < 2; i++)
		{
			...
			{
				forwardVec[cpd->pd_leg->num_cpn + 1 + i] = und->spot_fx 
		...
		if(OTC_params->OTCpayoff == 6)
		{
			...
			err = OTCgetFwdSABR(	forwardVec,
									cpd,
									und,
									smile_mkt,
									j,
									otc_precalc->pd_fwd3F_vol[j],
									otc_precalc->pd_fwdsmile_alpha[j],
									otc_precalc->pd_fwdsmile_beta[j],
									otc_precalc->pd_fwdsmile_rho[j],
									otc_precalc->pd_fwdsmile_pi[j],
									cpd->pd_leg->num_cpn + 1,
									mergeTimes,
									mergeNtimes,
									sigDom,
									und->lda_dom,
									...
									OTC_params->FSMbeta,
									OTC_params->FSMrho,
									4); /* 0=Sliding 3F vol; 1=Converging 3F vol; 2=Sliding Cvg Sbeta; 3=Cvg Cvg Sbeta; 4=3F*/
		...
		err = OTCgetFwdSABR(	forwardVec,
								...
								OTC_params->FSMbeta,
								OTC_params->FSMrho,
								OTC_params->FwdVolMethod); /* 0=Sliding 3F vol; 1=Converging 3F vol; 2=Sliding Cvg Sbeta; 3=Cvg Cvg Sbeta; 4=3F*/

		if(err) goto FREE_RETURN;
	}	

FREE_RETURN:
	if(forwardVec) free(forwardVec);
return err;
}

/* =========================================================================
 Calibration of the sum of shifted lognormal parameters to SABR to obtain a copula based estimation of OTC
========================================================================= */
...
Err		err = NULL;
	...
	dMarketprices[2] = srt_f_optblksch(fwd, fwd, sigma, t, 1.0, SRT_CALL, PREMIUM);
	dMarketprices[3] = srt_f_optblksch(fwd, k2, v2, t, 1.0, SRT_CALL, PREMIUM);


/* Finds a first guess */
	err = find_shifted_log_params(t,fwd,k1,v1,k2,v2,Fwd_Shift,&Vol_SL,&Beta_SL);
...
/* Adjust the ATM vol to match the market */
	...
	oldVol = Vol_SL;
	newVol = Vol_SL * 0.95;
	err = SSLprice(fwd, t, fwd, oldVol * expUp, oldVol * expDown, dParams[4], 1, &oldPrice);
	if(err) goto FREE_RETURN;
	err = SSLprice(fwd, t, fwd, newVol * expUp, newVol * expDown, dParams[4], 1, &price);
	if(err) goto FREE_RETURN;

	do
	{
		vol = newVol - (price - dMarketprices[2]) / ((price - oldPrice)/(newVol - oldVol));
		oldVol = newVol;
		newVol = vol;
		oldPrice = price;
		err = SSLprice(fwd, t, fwd, vol * expUp, vol * expDown, dParams[4], 1, &price);
		if(err) goto FREE_RETURN;
		volIter++;		
...
/* Call Levenberg-Marcquardt */
	...
	err  = levenberg_marquardt_select(	dStrikes,     
										dMarketprices, 
										dWeights,  
										nProducts,
										dParams,
...
FREE_RETURN:
	...
	if(dStrikes)		free_dvector(dStrikes,1,nProducts);
	if(dMarketprices)	free_dvector(dMarketprices,1,nProducts);
	if(dWeights)		free_dvector(dWeights,1,nProducts);
	if(dParams)			free_dvector(dParams,1,nParams);
	if(lUseparams)		free_lngvector(lUseparams,1,nParams);	
	return err;
}

Err SSLGradient(	double	strike,
					double	*ParamSSL,
					double	*price,
					double	*gradient,
					int		nbParamsSSL)
...
/* Gradient calculations */
	...
	if(err) goto FREE_RETURN;
	err = SSLprice(forward, maturity, strike, volUp, volDown, shift + bumpSL, 1, &shiftedpriceshift);
	if(err) goto FREE_RETURN;
	err = SSLprice(forward, maturity, strike, volUp * exp(bumpVolup), volDown, shift, 1, &shiftedpricevolup);
	if(err) goto FREE_RETURN;
	err = SSLprice(forward, maturity, strike, volUp, volDown * exp(bumpVoldown), shift, 1, &shiftedpricevoldown);
	if(err) goto FREE_RETURN;
...
========================================================================= */
Err SSLCumulative(	double	Forward,
					double	Maturity,
					double	VolUp,
					double	VolDown,
					double	FwdShift,
...
double	temp;
	...
	if(FwdShift < 0.0 && fabs(FwdShift) > Forward)
		firstGuessGrid[0] = - FwdShift + (Forward + FwdShift) * exp( Nstd * volATM * sqrt(Maturity));
	else
		firstGuessGrid[0] = (Forward + FwdShift) * exp( -Nstd * volATM * sqrt(Maturity)) - FwdShift;
	...
	{
		if(FwdShift < 0.0 && fabs(FwdShift) > Forward)
			firstGuessGrid[i] =  - FwdShift + (Forward + FwdShift) * exp(-Nstd * (2.0 * (double)i / Npoints - 1.0) * volATM * sqrt(Maturity));
		else
			firstGuessGrid[i] = (Forward + FwdShift) * exp( Nstd * (2.0 * (double)i / Npoints - 1.0) * volATM * sqrt(Maturity)) - FwdShift;

		err = SSLcumul(Forward, Maturity, firstGuessGrid[i], VolUp, VolDown, FwdShift, &(firstGuessCumul[i]));
		if(err) goto FREE_RETURN;
	}
	...
	{	
		Cumulative[0][i] = firstGuessCumul[0] 
			+ (double)i / (Npoints - 1) * (firstGuessCumul[Npoints-1] - firstGuessCumul[0]);
		Cumulative[0][i] = interp(firstGuessCumul,firstGuessGrid,Npoints,Cumulative[0][i],0,&temp);
	}

	for(i = 0; i < Npoints; i++)
	{
		err = SSLcumul(Forward, Maturity, Cumulative[0][i], VolUp, VolDown, FwdShift, &(Cumulative[1][i]));
		if(err) goto FREE_RETURN;
	}

FREE_RETURN:
	if(firstGuessGrid) free(firstGuessGrid);
	if(firstGuessCumul) free(firstGuessCumul);

return err;
}

Err BMCumulative(	double			Forward,
					double			Maturity,
					double			BMbeta,
					double			BMfwd1,
					double			BMfwd2,
...
double	expSigSqrtT;
	...
	/* Calculates the grid */
	gridMin1 = BMfwd1 * exp( - nStd * BMsig1 * pow(Forward,BMbeta - 1.0) * sqrt(Maturity));
	gridMin2 = BMfwd2 * exp( - nStd * BMsig2 * pow(Forward,BMbeta - 1.0) * sqrt(Maturity));
	gridMax1 = BMfwd1 * exp( + nStd * BMsig1 * pow(Forward,BMbeta - 1.0) * sqrt(Maturity));
	gridMax2 = BMfwd2 * exp( + nStd * BMsig2 * pow(Forward,BMbeta - 1.0) * sqrt(Maturity));
	...
	err = BMM_Option_Price_From_States(	Forward, Maturity, BMbeta,
										BMfwd1, BMfwd2,
										BMsig1, BMsig2, 
										BMpi, SRT_PUT, &price);
	...
	err = srt_f_optimpvol( price, Forward, Forward, Maturity, 1.0, SRT_PUT, SRT_LOGNORMAL, &ATMvol);
...
int		cp;
	...
	d2Up = (log((Forward + Shift) / (Strike + Shift)) - 0.5 * VolUp * VolUp * Maturity) / stdevUp;
	d2Down = (log((Forward + Shift) / (Strike + Shift)) - 0.5 * VolDown * VolDown * Maturity) / stdevDown;
...
Err err = NULL;
	...
	{
		err = BMM_Option_Price_From_States(	strike - strikeBump,
											Maturity,
											BMbeta,
											BMfwd1,
											BMfwd2,
...
Err err = NULL;
	...
	if(Shift<0.0 && fabs(Shift)>Forward)
	{
		*Price = srt_f_optblksch(-Forward - Shift, -Strike - Shift, fabs(VolUp), Maturity, 1.0, IsCall? SRT_PUT:SRT_CALL, PREMIUM);
		*Price += srt_f_optblksch(-Forward - Shift, -Strike - Shift, fabs(VolDown), Maturity, 1.0, IsCall? SRT_PUT:SRT_CALL, PREMIUM);
	}
	else
	{
		*Price = srt_f_optblksch(Forward + Shift, Strike + Shift, fabs(VolUp), Maturity, 1.0, IsCall? SRT_CALL:SRT_PUT, PREMIUM);
		*Price += srt_f_optblksch(Forward + Shift, Strike + Shift, fabs(VolDown), Maturity, 1.0, IsCall? SRT_CALL:SRT_PUT, PREMIUM);
	}
	*Price *= 0.5;
return err;
}

Err GenerateRealisations(	double	Mean1,
							double	Mean2,
							double	**cumulative,
							int		Nsimul,
							int		Npoints,
...
int		i, index1, index2, index3, method;
	...
	for(i=0; i<Nsimul; i++)
	{
		...
		if(GetGaussOrMarginal)
		{
			...
			err = OTCutilsGetIndex(	cumulative[1],
									Npoints,
									matrix[i][FxIndex],
									isLinear,
									&index1,
			...
			err = OTCutilsInterpolate(	cumulative[0][index1],
										cumulative[0][index2],
										cumulative[0][index3],
										cumulative[1][index1],
										cumulative[1][index2],
			...
			else
			{
				if(test1 > cumulative[0][index1] && test1 < cumulative[0][index3])
					...
					matrix[i][FxIndex] = cumulative[0][Npoints-1]; // min between the two
...
/* ===============================================
	Discount factor
=============================================== */
Err OTCdfBis(	double	realisation,
			double	lambda,
			double	Tstart,
			double	Tend,
...
Err err = NULL;
	...
	else
	{
		...
		for(i=1; i<endIndex; i++)
			Phi += vol[i] * vol[i] * Phi_Func(2*lambda, Tstart, volTimes[i-1], volTimes[i]);

		Phi += vol[endIndex] * vol[endIndex] * Phi_Func(2*lambda, Tstart, volTimes[i-1], Tstart);
	}
...
Err OTCgetMoments(	double		endTime,
					double		ldaDom,
					double		ldaFor,
					double		TStar,
					double		*mergeTimes,
...
double	sig_dom, sig_for, sig_fx, sig_dom2, sig_for2, sig_fx2,
		sig_domfor, sig_domfx, sig_forfx;
...
double	sig_dom, sig_for, sig_fx, sig_dom2,
		sig_domfor, sig_domfx;
...
Err		err = NULL;
	...
	if(fabs(Tval -  Tpay) > 1.0e-10)
	{
		...
		for (j=start_index; j<end_index+1; j++)
		{
			...
			covar_dom_ffx += sig_domfx * Phi_Func(ldaDom, 0.0, t1, t2)
...
Err OTCcorrelateVariables(	double	domStd,
							double	forStd,
							double	corDF,
							double	corDFx,
							double	corFFx,
...
Err OTCgetFwdSABR(	double				*forward,
					CPD_STR				cpd,
					CPD_UND				und,
					SMILE_VOL_MARKET	smile_mkt,
					int					otc,
					...
					char				*CPDrho,
					int					method) 
/* 0=Sliding 3F vol; 1=Converging 3F vol; 2=Sliding Cvg Sbeta; 3=Cvg Cvg Sbeta; 4=3F; 5=FwdSmile */
{
int		i, j;
double	power;
long	FxNotValDate[2];
...
Err err = NULL;
	...
	for(i = 0; i < 2; i++)
	{
		...
		{
			FxNotValDate[i] = add_unit (cpd->call[otc + i].ex_date, 2, SRT_BDAY, MODIFIED_SUCCEEDING);
	...
	if(method == 5)
	{
		...
		for(i = 0; i < noptions; i++)
		{
			for(j = 0; j < 4; j++)
			{
				err = swp_f_vol(	CPDvol[j],
									cpd->call[otc].ex_date,
									(i == (noptions - 1)) ? cpd->pd_leg->not_ref.fx_fix_date : cpd->pd_leg->cpn[i].fx_fix_date,
									(i == (noptions - 1)) ? cpd->pd_leg->not_ref.fx_val_date : cpd->pd_leg->cpn[i].fx_val_date,
									&value,
									&power);
			...
			err = srt_f_optsarbvol(	forward[i],							
									forward[i], 
									(i == (noptions - 1)) ? cpd->pd_leg->not_ref.fx_fix_time : cpd->pd_leg->cpn[i].fx_fix_time,
									optionsSigma[i],  
									optionsAlpha[i],
									optionsBeta[i],
									optionsRho[i],
		...
		for(i = 0; i < 2; i++)
		{
			if(otc + i < cpd->num_calls)
			{
				...
				err = srt_f_optsarbvol(	forward[noptions + i],							
										forward[noptions + i], 
										cpd->call[otc + i].ex_time,
										optionsSigma[noptions + i],  
										optionsAlpha[noptions + i],
										optionsBeta[noptions + i],
										optionsRho[noptions + i],
										SRT_LOGNORMAL,
										SRT_BETAVOL,
										&(optionsSigma[noptions + i]));
	...
	else
	{
		...
		for(i = 0; i < noptions; i++)
		{
			err = OTCfwdSABR(	forward? forward[i]:0.0,
								cpd->call[otc].ex_time,
								(i == (noptions - 1)) ? cpd->pd_leg->not_ref.fx_fix_time : cpd->pd_leg->cpn[i].fx_fix_time,
								(i == (noptions - 1)) ? cpd->pd_leg->not_ref.fx_val_time : cpd->pd_leg->cpn[i].fx_val_time,
								smile_mkt,
								smile_params,
								mergeTimes,
								mergeNtimes,
...
FREE_RETURN:
	...
	if(smile_params) free(smile_params);

return err;
}

Err OTCfwdSABR(	double				forward,
				double				fwdStartTime,
				double				optionsTimeFix,
				double				optionsTimeSettle,
				SMILE_VOL_MARKET	smile_mkt,
...
Err err = NULL;
	...
	if(optionsTimeFix > fwdStartTime)
	{
		...
		{
			err = cpd_vol_get_smile_params(1,0,timeToMaturity,smile_mkt,smile_params);
			if(err) goto FREE_RETURN;
		}		

		if(method < 2)
		{
			err = Fx3DtsImpliedVol_corr(	optionsTimeSettle,
											fwdStartTime,
											optionsTimeFix,
											mergeTimes,
											(long) mergeNtimes,
			...
			err = cpd_vol_get_vol(forward, optionsTimeFix, forward,smile_params, SRT_BETAVOL, &(smile_params->sigmabeta));
			if(err) goto FREE_RETURN;
...
Err OTCgetSMILEparams(	CPD_STR				cpd,
						CPD_UND				und,
						double				Tfix,
						double				Tval,
						long				DateVal,
...
Err err = NULL;
	...
	================================== */
	err = Fx3DtsImpliedVol_corr(	Tval,
									0.0,
									Tfix,
									und->sigma_time_rates,
									und->sigma_n_rates,
...
Err OTCgetSABRparams(	CPD_STR		cpd,
						CPD_UND		und,
						double		Tfix,
						double		Tval,
						long		DateVal,
...
Err err = NULL;
	...
	if(smileFee)
	{
		...
		else
		{
			...
			*smileAlpha = interpCoef * fx_mkt_smile_alpha[j+1] + (1.0-interpCoef) * fx_mkt_smile_alpha[j];
			*smileBeta = interpCoef * fx_mkt_smile_beta[j+1] + (1.0-interpCoef) * fx_mkt_smile_beta[j];
			*smileRho = interpCoef * fx_mkt_smile_rho[j+1] + (1.0-interpCoef) * fx_mkt_smile_rho[j];
		}
	}
	else
	{
...
Err OTCcalibSmileModel(	double				Tex,			/* Maturity in years */
						...
						double				*error)
{
double StrikeUp, StrikeDown, VolUp, VolDown, alpha, rho, CalibError, SigmaBeta;
Err err = NULL;
	...
	else if(OTC_params->smileModel == 1 || OTC_params->smileModel == 2)
	{
		if(smile_params->smile_spec_type > 1)
		{
			err = srt_f_optbmmvol(	forward, forward, Tex, smile_params->sigma,
									...
									smile_params->rho,smile_params->pi,
									SRT_LOGNORMAL,SRT_BETAVOL,&SigmaBeta);

			if(err) goto FREE_RETURN;

			err = BMMGetStates(	forward, Tex, 
								SigmaBeta, smile_params->alpha, 
								smile_params->beta, smile_params->rho, smile_params->pi,
								&(OTC_params->BMsig1), &(OTC_params->BMsig2),
								&(OTC_params->BMfwd1), &(OTC_params->BMfwd2));
		...
		else
		{
			if(OTC_params->OTCsmile && OTC_params->smileModel == 1)
			{
				...
				StrikeDown = forward * exp(- OTC_params->SSLstd * smile_params->sigma * sqrt(Tex));
				StrikeUp = forward * exp(+ OTC_params->SSLstd * smile_params->sigma * sqrt(Tex));
				
				err = srt_f_optsarbvol(	forward, StrikeDown, Tex, 
										smile_params->sigma,smile_params->alpha,
										smile_params->beta, smile_params->rho,
										SRT_LOGNORMAL,SRT_LOGNORMAL,&VolDown);

				if(err) goto FREE_RETURN;

				err = srt_f_optsarbvol(	forward, StrikeUp, Tex, 
										smile_params->sigma,smile_params->alpha,
										smile_params->beta, smile_params->rho,
										SRT_LOGNORMAL,SRT_LOGNORMAL,&VolUp);

				if(err) goto FREE_RETURN;

				err = calib_sabr_rr_bt_given_beta(	forward, Tex, smile_params->sigma,
													StrikeDown, VolDown,
													StrikeUp, VolUp,
													&SigmaBeta,
													&alpha, 1.0, &rho,
			...
			err = BMMCalibOnSabrStates(	forward,
										Tex,
										smile_params->sigma,
										OTC_params->OTCsmile? smile_params->beta:1.0,
										OTC_params->OTCsmile? smile_params->alpha:0.0,
...
Err OTCgetCumulative(	double			forward,
						double			Tex,
						otcpd_params	*OTC_params)
{
...
Err err = NULL;
	...
	{
		err = SSLCumulative(	forward,
								Tex,
								OTC_params->SSLvolUp,
								OTC_params->SSLvolDown,
								OTC_params->SSLshift,
								OTC_params->CUMULnPoints,
								OTC_params->CUMULnStd,
								OTC_params->CUMUL);
	...
	else if(OTC_params->smileModel == 1 || OTC_params->smileModel == 2)
	{
		...
		err = copula_gaussian_get_BMM_linterp_cumulative(	Tex,
															forward,
															OTC_params->BMfwd1,
															OTC_params->BMsig1,
															OTC_params->BMfwd2,
		...
		err = OTCmatrixRealloc(	&(OTC_params->CUMUL),
								0, 2-1, 0, OTC_params->CUMULnPoints-1,
								0, 2-1, 0, lNbPoints-1);
...
FREE_RETURN:
	if(dX) free(dX);
	if(dCumulative) free(dCumulative);
return err;
}

Err OTCmatrixRealloc(	double	*** mat,
						long	nrl,
						long	nrh,
						long	ncl,
						long	nch,
...
Err OTCgetChangedPayoff(	CPD_STR		cpd,
							CPD_UND		und,
							double		*NewCoupons)
{
...
Err err = NULL;
	...
	if(cpd->fund_leg->dom_for == 0)
	{
		for(k = 0; k < cpd->num_calls; k++)
		{
			...
			{
				NewCoupons[pdIndex] += cpd->fund_leg->cpn[i].cpn 
					* swp_f_df (und->today, cpd->fund_leg->cpn[i].pay_date, und->dom_yc);
			}

			NewCoupons[pdIndex] /= swp_f_df (und->today, cpd->pd_leg->cpn[pdIndex].pay_date, und->dom_yc);
		}
	}
	else
	{
		for(k = 0; k < cpd->num_calls; k++)
		{
			...
			NewCoupons[pdIndex] /= swp_f_df (und->today, cpd->pd_leg->cpn[pdIndex].fx_val_date, und->for_yc);
		}
	}
return err;
}

Err OTCgetChangedPayoff2(	CPD_STR		cpd,
							CPD_UND		und,
							double		*NewCoupons)
{
...
Err err = NULL;
	...
	for(i = 0; i < cpd->fund_leg->num_cpn; i++)
	{
		...
		else
		{
			if(!k)
			{
				...
				else
				{
					...
					else
					{
						...
						midDate = (long) ((cpd->pd_leg->cpn[j - 1].pay_date + cpd->pd_leg->cpn[j].pay_date) / 2.0);
			...
			NewCoupons[couponIndex] += cpd->fund_leg->cpn[i].cpn 
				* swp_f_df (und->today, cpd->fund_leg->cpn[i].pay_date, und->dom_yc)
				/ swp_f_df (und->today, cpd->pd_leg->cpn[couponIndex].pay_date, und->dom_yc);
		}
	}
...
=================================================================================================
================================================================================================= */
Err	OTCfwdSABRCalib(	CPD_STR		cpd,
						CPD_UND		und,
...
Err err = NULL;
	...
	{
		forward[i] = FxAdj[i] * und->spot_fx 
					* swp_f_df( und->today, cpd->pd_leg->cpn[i].fx_val_date, und->for_yc)
					/ swp_f_df (und->today, cpd->pd_leg->cpn[i].fx_val_date, und->dom_yc);

		err = Fx3DtsImpliedVol_corr(	cpd->pd_leg->cpn[i].fx_val_time,
										0.0,
		...
		Results[i][sUp] = forward[i] * exp( fwdSmileVisuNstd * vol * sqrt(cpd->pd_leg->cpn[i].fx_val_time));
		Results[i][sATM] = forward[i];
		Results[i][sDown] = forward[i] * exp( - fwdSmileVisuNstd * vol * sqrt(cpd->pd_leg->cpn[i].fx_val_time));

		PdDfDom[i] = swp_f_df( und->today, cpd->pd_leg->cpn[i].pay_date, und->dom_yc) / dfDom;
		FwdDfDom[i] = swp_f_df( und->today, cpd->pd_leg->cpn[i].fx_val_date, und->dom_yc) / dfDom;
		FwdDfFor[i] = swp_f_df( und->today, cpd->pd_leg->cpn[i].fx_val_date, und->for_yc) / dfFor;
	}
	...
	for(realisation = 0; realisation<nSimul; realisation++)
	{
		for (i=pd_otc_start; i<cpd->pd_leg->num_cpn; i++)
		{
			...
			Forward @ Tval
			================================== */
			err = OTCdf(	matrix[realisation][0],
							und->lda_dom,
							cpd->call[otc].ex_time,
							cpn->fx_val_time,
							tstarTime,
			...
			err = OTCdf(	matrix[realisation][1],
							und->lda_for,
							cpd->call[otc].ex_time,
							cpn->fx_val_time,
							tstarTime,
			...
			UP
			================================== */
			err = srt_f_optsarbvol(	localForward,
									Results[i][sUp],
			...
			{
				Results[i][vUp] += dfDom * OPT_VAL_MACRO(	type - 2, 
																localForward, 
																Results[i][sUp], 
																smile_std,
																smile_half_std);
			...
			{
				Results[i][vUp] += dfDom * OPT_VAL_MACRO(	type, 
																localForward, 
																Results[i][sUp], 
																smile_std,
																smile_half_std);
			...
			if (localForward < 0.0 && type >= 3)
			{
				Results[i][vATM] += dfDom * OPT_VAL_MACRO(	type - 2, 
																localForward, 
																Results[i][sATM], 
																smile_std,
																smile_half_std);
			...
			{
				Results[i][vATM] += dfDom * OPT_VAL_MACRO(	type, 
																localForward, 
																Results[i][sATM], 
																smile_std,
																smile_half_std);
			...
			if (localForward < 0.0 && type >= 3)
			{
				Results[i][vDown] += dfDom * OPT_VAL_MACRO(	type - 2, 
																localForward, 
																Results[i][sDown], 
																smile_std,
																smile_half_std);
			...
			{
				Results[i][vDown] += dfDom * OPT_VAL_MACRO(	type, 
																localForward, 
																Results[i][sDown], 
																smile_std,
																smile_half_std);
	...
	for(i=pd_otc_start; i<cpd->pd_leg->num_cpn; i++)
	{
		...
		{	
			...
			err =  srt_f_optimpvol(	Results[i][2 * j + 1],
									forward[i],
									Results[i][2 * j],
...
FREE_RETURN:
	...
	if(PdDfDom)		free(PdDfDom);
	if(FwdDfDom)	free(FwdDfDom);
	if(FwdDfFor)	free(FwdDfFor);
	if(forward)		free(forward);

	return err;
}

Err OTCPDpayoffFeeCombinedSmileAndFlat(	CPD_STR				cpd,
										CPD_UND				und,
										otcpd_params		*OTC_params,
...
double			discount, df, dfDom, dfFor, dfFund, dfFirstFunding, 
				dfNotExchgePd, dfNotExchgeShortPd,
				dfNotExchgeFund, dfNotExchgeShortFund;
...
int				pd_otc_start, pd_otc_short_end;			/*	index of the first coupon to be called */
...
Err err = NULL;
	...
	if(!fund_leg || !pd_leg || !fund_legShort || !pd_legShort || 
		!fundDf || !PdDfDom || !FwdDfDom || !FwdDfFor || !strikeCap || !strikeFloor ||
	...
	dfFirstFunding = swp_f_df( und->today, cpd->fund_leg->cpn[fund_otc_start].start_date, fund_yc) / dfFund;
	dfNotExchgePd = swp_f_df( und->today, cpd->call[OTC_params->OTC].set_date, und->dom_yc) / dfDom;
	dfNotExchgeFund = swp_f_df( und->today, cpd->call[OTC_params->OTC].set_date, fund_yc) / dfFund;
	
	if(OTC_params->OTC < cpd->num_calls - 1)
	{
		dfNotExchgeShortPd = swp_f_df( und->today, cpd->call[OTC_params->OTC + 1].set_date, und->dom_yc) / dfDom;
		dfNotExchgeShortFund = swp_f_df( und->today, cpd->call[OTC_params->OTC + 1].set_date, fund_yc) / dfFund;
	}

	for(i = fund_otc_start; i < cpd->fund_leg->num_cpn; i++)
		fundDf[i] =  swp_f_df( und->today, cpd->fund_leg->cpn[i].pay_date, fund_yc) / dfFund;
	...
	for(i = pd_otc_start; i < cpd->pd_leg->num_cpn; i++)
	{
		...
		{
			strikeCap[i] = (cpd->pd_leg->cpn[i].cap - cpd->pd_leg->cpn[i].alpha) / cpd->pd_leg->cpn[i].beta;
			strikeFloor[i] = (cpd->pd_leg->cpn[i].floor - cpd->pd_leg->cpn[i].alpha) / cpd->pd_leg->cpn[i].beta;
		}
	}

	PdDfDom[cpd->pd_leg->num_cpn] = swp_f_df( und->today, cpd->pd_leg->not_ref.pay_date, und->dom_yc) / dfDom;
	FwdDfDom[cpd->pd_leg->num_cpn] = swp_f_df( und->today, cpd->pd_leg->not_ref.fx_val_date, und->dom_yc) / dfDom;
	FwdDfFor[cpd->pd_leg->num_cpn] = swp_f_df( und->today, cpd->pd_leg->not_ref.fx_val_date, und->for_yc) / dfFor;
	if(fabs(cpd->pd_leg->not_ref.beta) > 1.0e-16)
	{
		strikeCap[cpd->pd_leg->num_cpn] = (cpd->pd_leg->not_ref.cap - cpd->pd_leg->not_ref.alpha) / cpd->pd_leg->not_ref.beta;
		strikeFloor[cpd->pd_leg->num_cpn] = (cpd->pd_leg->not_ref.floor - cpd->pd_leg->not_ref.alpha) / cpd->pd_leg->not_ref.beta;
	}
	...
	for(i = 0; i < 2; i++)
	{
		if(OTC_params->OTC + i < cpd->num_calls)
		{
			...
			{
				dfNotEx[i] = (swp_f_df( und->today, cpd->call[OTC_params->OTC + i].fx_val_date, und->for_yc) / dfFor)
							/ (swp_f_df( und->today, cpd->call[OTC_params->OTC + i].fx_val_date, und->dom_yc) / dfDom);
			}
		}
	}
	...
	fund_fee -= swp_f_df( und->today, cpd->call[OTC_params->OTC].set_date, fund_yc) * cpd->fund_leg->notional;
	if(OTC_params->OTC < cpd->num_calls - 1)
	{
		fund_feeShort = fund_fee;

		/*	Final Notional For Short option*/
		fund_feeShort += swp_f_df( und->today, cpd->call[OTC_params->OTC + 1].set_date, fund_yc) * cpd->fund_leg->notional;
	}
	...
	for(i = 0; i < 2; i++)
	{
		if(OTC_params->OTC + i < cpd->num_calls)
		{
			dfDom = swp_f_df( und->today, cpd->call[OTC_params->OTC + i].set_date, und->dom_yc);
			...
			if(cpd->call[OTC_params->OTC + i].use_opt_str)			
			{
				...
				SABR parameters @ Tval
				================================== */
				err = OTCgetSMILEparams(cpd, und,cpd->call[OTC_params->OTC + i].fx_fix_time, 
											cpd->call[OTC_params->OTC + i].fx_val_time, 
											cpd->call[OTC_params->OTC + i].fx_val_date,
											smile_mkt, smile_params, Calc_flat?1:OTC_params->OTCsmileFee);

				if(err) goto FREE_RETURN;
				
				if(Calc_flat) // As sigma is log, only the ABR params have to be set to the log case.
				{
					err = OTCgetSMILEparams(cpd, und,cpd->call[OTC_params->OTC + i].fx_fix_time, 
											cpd->call[OTC_params->OTC + i].fx_val_time, 
											cpd->call[OTC_params->OTC + i].fx_val_date,
											smile_mkt, smile_params_flat, Calc_flat?1:OTC_params->OTCsmileFee);
				...
				Forward @ Tval
				================================== */
				forward = OTC_precalc->NotFeeFxAdj[OTC_params->OTC + i] * und->spot_fx
					 * swp_f_df( und->today, cpd->call[OTC_params->OTC + i].fx_val_date, und->for_yc)
					/ swp_f_df( und->today, cpd->call[OTC_params->OTC + i].fx_val_date, und->dom_yc);
				...
				for (str_idx = 0; str_idx < cpd->call[OTC_params->OTC + i].nstrikes; str_idx++)
				{
					if (fabs(cpd->call[OTC_params->OTC + i].weights[str_idx]) > 1e-16 * pd_not)
					{
						if (smile_params->sigma * sqrt (cpd->call[OTC_params->OTC + i].fx_fix_time)> 1e-16 && cpd->call[OTC_params->OTC + i].strikes[str_idx] > 1e-16) type = 3;
						else type = 1;

						err = cpd_vol_get_price(type, forward,
												cpd->call[OTC_params->OTC + i].fx_fix_time,
												cpd->call[OTC_params->OTC + i].strikes[str_idx],
												smile_params,&OptionValue);
						if(err) goto FREE_RETURN;

						opt_string += cpd->call[OTC_params->OTC + i].weights[str_idx] * OptionValue;
						...
						if(Calc_flat)
						{
							...
							opt_string_flat += cpd->call[OTC_params->OTC + i].weights[str_idx] * OptionValue_flat;
						}
					}
				}
				...
				if(!i)
				{
					pd_fee -= dfDom * (	cpd->call[OTC_params->OTC + i].wcst + 
					...
					if(Calc_flat)
					{
						pd_fee_flat -= dfDom * (	cpd->call[OTC_params->OTC + i].wcst + 
											cpd->call[OTC_params->OTC + i].wspot * forward + 
				...
				{
					pd_feeShort += dfDom * (	cpd->call[OTC_params->OTC + i].wcst + 
										cpd->call[OTC_params->OTC + i].wspot * forward + 
										opt_string + cpd->call[OTC_params->OTC + i].orig_fee);

					if(Calc_flat)
					{
						pd_feeShort_flat += dfDom * (	cpd->call[OTC_params->OTC + i].wcst + 
											cpd->call[OTC_params->OTC + i].wspot * forward + 
											opt_string_flat + cpd->call[OTC_params->OTC + i].orig_fee);
					}
				}
			...
			else
			{
				if(!i)
				{
					...
					pd_feeShort = pd_fee;
					if(Calc_flat) pd_fee_flat = pd_fee;
					if(Calc_flat) pd_feeShort_flat = pd_fee;
				}
				else
				{
					pd_feeShort += dfDom * (pd_not + cpd->call[OTC_params->OTC + i].orig_fee);
					if(Calc_flat) pd_feeShort_flat += dfDom * (pd_not + cpd->call[OTC_params->OTC + i].orig_fee);
				}
			}
		}
	}
	...
	SABR parameters @ Tval
	================================== */
	err = OTCgetSMILEparams(cpd, und,cpn->fx_fix_time, cpn->fx_val_time, cpn->fx_val_date,
							smile_mkt, smile_params, Calc_flat?1:OTC_params->OTCsmileFee);
	...
	if(!cpn->use_opt_str)
	{
		if (fabs (cpn->beta) > 1.0e-16)
		{
			...
			Floor
			================================== */
			err  = OTCgetOptValue(	cpn->floored,
										0,
										cpn->floor,
		...
		/*	Coupon pv */		
		pd_fee += dfDom * (cpn->alpha + cpn->beta * forward + fabs (cpn->beta) * (floor - cap));
		if(Calc_flat) pd_fee_flat += dfDom * (cpn->alpha + cpn->beta * forward + fabs (cpn->beta) * (floor_flat - cap_flat));
	}
	else
	{
		opt_string = 0.0;
		...
		for (str_idx = 0; str_idx < cpn->nstrikes; str_idx++)
		{
			...
			{
				if (smile_params->sigma * sqrt (cpn->fx_fix_time) > 1e-16 && cpn->strikes[str_idx] > 1e-16) type = 3;
				else type = 1;

				err = cpd_vol_get_price(type, forward, cpn->fx_fix_time, cpn->strikes[str_idx],smile_params,&OptionValue);
				if(err) goto FREE_RETURN;
				...
				if(Calc_flat)
				{
					err = cpd_vol_get_price(type, forward, cpn->fx_fix_time, cpn->strikes[str_idx],smile_params_flat,&OptionValue_flat);
					if(err) goto FREE_RETURN;
		...
		if(Calc_flat) pd_fee_flat += dfDom * (cpn->wcst + cpn->wspot * forward + opt_string_flat);	
	}	
	...
	fund_fee += swp_f_df( und->today, cpd->fund_leg->cpn[fund_otc_start].start_date, fund_yc) * cpd->fund_leg->notional;

	if(OTC_params->OTC < cpd->num_calls - 1)
		fund_feeShort += swp_f_df( und->today, cpd->fund_leg->cpn[fund_otc_start].start_date, fund_yc) * cpd->fund_leg->notional;
	...
	for (i = pd_otc_start; i < cpd->pd_leg->num_cpn; i++)
	{
		...
		================================== */
		forward = OTC_precalc->FeeFxAdj[i] * und->spot_fx * swp_f_df( und->today, cpn->fx_val_date, und->for_yc)
			/ swp_f_df( und->today, cpn->fx_val_date, und->dom_yc);
		...
		if(!cpn->use_opt_str)
		{
			...
			if(OTC_params->OTC < cpd->num_calls - 1)
			{
				...
				{
					pd_feeShort += dfDom * (cpn->alpha + cpn->beta * forward + fabs(cpn->beta) * (floor - cap));
					if(Calc_flat) pd_feeShort_flat += dfDom * (cpn->alpha + cpn->beta * forward + fabs(cpn->beta) * (floor_flat - cap_flat));
				}
			}
		}
		else
		{
			...
			if(OTC_params->OTC < cpd->num_calls - 1)
			{
				if(i >= pd_otc_start && i < pd_otc_short_end)
				{
					pd_feeShort += dfDom * (cpn->wcst + cpn->wspot * forward + opt_string);
					if(Calc_flat) pd_feeShort_flat += dfDom * (cpn->wcst + cpn->wspot * forward + opt_string_flat);
				}
			}
		}
	}
	...
	if(OTC_params->OTCcalculate[OTC_params->OTC])
	{
		...
		for(realisation = 0; realisation<OTC_params->COPULAnSimul; realisation++)
		{
			/* ==================================
			...
			for (i = pd_otc_start; i < cpd->pd_leg->num_cpn; i++)
			{
				...
				df = exp(OTC_precalc->df_const_pd_pay[OTC_params->OTC][i] + OTC_precalc->df_lin_pd_pay[OTC_params->OTC][i] * matrix[realisation][0]);

				PDdfReal[i] = PdDfDom[i] * df;

				if(cpn->pay_date != cpn->fx_val_date)
				{
					df = exp(OTC_precalc->df_const_dom_fx_val[OTC_params->OTC][i] + OTC_precalc->df_lin_dom_fx_val[OTC_params->OTC][i] * matrix[realisation][0]);
				}

				PDFxDomdfReal[i] = FwdDfDom[i] * df;

				df = exp(OTC_precalc->df_const_for_fx_val[OTC_params->OTC][i] + OTC_precalc->df_lin_for_fx_val[OTC_params->OTC][i] * matrix[realisation][1]);
			...
			for(i = 0; i < 2; i++)
			{
				if(OTC_params->OTC + i < cpd->num_calls)
				{
					df = exp(OTC_precalc->df_const_pd_pay[OTC_params->OTC][cpd->pd_leg->num_cpn + i + 1] + OTC_precalc->df_lin_pd_pay[OTC_params->OTC][cpd->pd_leg->num_cpn + i + 1] * matrix[realisation][0]);
					...
					err = cpd_vol_get_smile_params_from_otc(smile_mkt->smile_spec_type+1,OTC_params->OTC, cpd->pd_leg->num_cpn+i+1,OTC_precalc,smile_params);
					if(err) goto FREE_RETURN;

					smile_params_flat->sigma = OTC_precalc->pd_fwd3F_vol[OTC_params->OTC][cpd->pd_leg->num_cpn+i+1];  
					...
					if(cpd->call[OTC_params->OTC + i].use_opt_str)
					{
						...
						Forward @ Tval
						================================== */
						forward = OTC_precalc->NotFxAdj[OTC_params->OTC][i] * matrix[realisation][2] * dfNotEx[i];
						if(Calc_flat) forward_flat = OTC_precalc->NotFxAdj[OTC_params->OTC][i] * matrix[realisation][3] * dfNotEx[i];

						df = exp(OTC_precalc->df_const_dom_fx_val[OTC_params->OTC][cpd->pd_leg->num_cpn + i + 1] + OTC_precalc->df_lin_dom_fx_val[OTC_params->OTC][cpd->pd_leg->num_cpn + i + 1] * matrix[realisation][0]);

						forward /= df;
						if(Calc_flat) forward_flat /= df;

						df = exp(OTC_precalc->df_const_for_fx_val[OTC_params->OTC][cpd->pd_leg->num_cpn + i + 1] + OTC_precalc->df_lin_for_fx_val[OTC_params->OTC][cpd->pd_leg->num_cpn + i + 1] * matrix[realisation][1]);
						...
						for (str_idx = 0; str_idx < cpd->call[OTC_params->OTC + i].nstrikes; str_idx++)
						{
							if (fabs(cpd->call[OTC_params->OTC + i].weights[str_idx]) > 1e-16 * pd_not)
							{
								if (OTC_precalc->pd_fwdsmile_vol[OTC_params->OTC][cpd->pd_leg->num_cpn + i + 1] * sqrt (cpd->call[OTC_params->OTC + i].fx_fix_time - cpd->call[OTC_params->OTC].ex_time)> 1e-16 && cpd->call[OTC_params->OTC + i].strikes[str_idx] > 1e-16) type = 3;
								else type = 1;

								err = cpd_vol_get_price(type, forward,
														cpd->call[OTC_params->OTC + i].fx_fix_time - cpd->call[OTC_params->OTC].ex_time,
														cpd->call[OTC_params->OTC + i].strikes[str_idx],
														smile_params,&OptionValue);
								if(err) goto FREE_RETURN;
						...
						if(!i)
						{
							pd_leg[realisation] -= dfDom * (cpd->call[OTC_params->OTC + i].wcst + 
															cpd->call[OTC_params->OTC + i].wspot * forward + 
															opt_string + cpd->call[OTC_params->OTC + i].orig_fee);
							pd_legShort[realisation] -= dfDom * (	cpd->call[OTC_params->OTC + i].wcst + 
																	cpd->call[OTC_params->OTC + i].wspot * forward + 
																	opt_string + cpd->call[OTC_params->OTC + i].orig_fee);

							if(Calc_flat)
							{
								pd_leg_flat[realisation] -= dfDom * (cpd->call[OTC_params->OTC + i].wcst + 
																cpd->call[OTC_params->OTC + i].wspot * forward_flat + 
																opt_string_flat + cpd->call[OTC_params->OTC + i].orig_fee);
								pd_legShort_flat[realisation] -= dfDom * (	cpd->call[OTC_params->OTC + i].wcst + 
																cpd->call[OTC_params->OTC + i].wspot * forward_flat + 
																opt_string_flat + cpd->call[OTC_params->OTC + i].orig_fee);
							}
						}
					...
					else
					{
						if(!i)
						{
							pd_leg[realisation] -= dfDom * (pd_not + cpd->call[OTC_params->OTC + i].orig_fee);
							pd_legShort[realisation] = pd_leg[realisation];
							if(Calc_flat) 
							{
								pd_leg_flat[realisation] -= dfDom * (pd_not + cpd->call[OTC_params->OTC + i].orig_fee);
								pd_legShort_flat[realisation] = pd_leg_flat[realisation];
							}
						}
						else
						{
							pd_legShort[realisation] += dfDom * (pd_not + cpd->call[OTC_params->OTC + i].orig_fee);
							if(Calc_flat) pd_legShort_flat[realisation] += dfDom * (pd_not + cpd->call[OTC_params->OTC + i].orig_fee);
						}
					}
				}
			}
			...
			/*	Discount */
			df = exp(OTC_precalc->df_const_pd_pay[OTC_params->OTC][cpd->pd_leg->num_cpn] + OTC_precalc->df_lin_pd_pay[OTC_params->OTC][cpd->pd_leg->num_cpn] * matrix[realisation][0]);

			discount = PdDfDom[cpd->pd_leg->num_cpn] * df;
		
			/*	Fwd fx */

			if ((fabs (cpn->beta) > 1.0e-16 && !cpn->use_opt_str) || cpn->use_opt_str)
			{
				/* ==================================
				Forward @ Tval
				================================== */
				df = exp(OTC_precalc->df_const_dom_fx_val[OTC_params->OTC][cpd->pd_leg->num_cpn] + OTC_precalc->df_lin_dom_fx_val[OTC_params->OTC][cpd->pd_leg->num_cpn] * matrix[realisation][0]);

				dfDom = FwdDfDom[cpd->pd_leg->num_cpn] * df;

				df = exp(OTC_precalc->df_const_for_fx_val[OTC_params->OTC][cpd->pd_leg->num_cpn] + OTC_precalc->df_lin_for_fx_val[OTC_params->OTC][cpd->pd_leg->num_cpn] * matrix[realisation][1]);

				dfFor = FwdDfFor[cpd->pd_leg->num_cpn] * df;

				forward = OTC_precalc->NotFxAdj[OTC_params->OTC][2] * matrix[realisation][2] * dfFor / dfDom;
				if(Calc_flat) forward_flat = OTC_precalc->NotFxAdj[OTC_params->OTC][2] * matrix[realisation][3] * dfFor / dfDom;
			}

			err = cpd_vol_get_smile_params_from_otc(smile_mkt->smile_spec_type+1,OTC_params->OTC, cpd->pd_leg->num_cpn,
													OTC_precalc,smile_params);
			if(err) goto FREE_RETURN;

			smile_params_flat->sigma = OTC_precalc->pd_fwd3F_vol[OTC_params->OTC][cpd->pd_leg->num_cpn];
			...
			if(!cpn->use_opt_str)
			{
				...
				/*	Coupon pv */		
				pd_leg[realisation] += discount * (cpn->alpha + cpn->beta * forward + fabs (cpn->beta) * (floor - cap));
				if(Calc_flat) pd_leg_flat[realisation] += discount * (cpn->alpha + cpn->beta * forward_flat + fabs (cpn->beta) * (floor_flat - cap_flat));
			}
			else
			{
				opt_string = 0.0;
				...
				for (str_idx = 0; str_idx < cpn->nstrikes; str_idx++)
				{
					if (fabs(cpn->weights[str_idx]) > 1e-16 * pd_not)
					{
						if (OTC_precalc->pd_fwdsmile_vol[OTC_params->OTC][cpd->pd_leg->num_cpn] * sqrt (cpn->fx_fix_time - cpd->call[OTC_params->OTC].ex_time) > 1e-16 && cpn->strikes[str_idx] > 1e-16) type = 3;
						else type = 1;

						err = cpd_vol_get_price(type, forward, cpn->fx_fix_time - cpd->call[OTC_params->OTC].ex_time, 
						...
						if(Calc_flat)
						{
							err = cpd_vol_get_price(type, forward_flat, cpn->fx_fix_time - cpd->call[OTC_params->OTC].ex_time, 
				...
				/*	Coupon pv */		
				pd_leg[realisation] += discount * (cpn->wcst + cpn->wspot * forward + opt_string);
				if(Calc_flat) pd_leg_flat[realisation] += discount * (cpn->wcst + cpn->wspot * forward_flat + opt_string_flat);
			...
			if(!OTC_params->SpeedFunding)
			{
				err = OTCfundingValuation(	matrix[realisation][0],
											...
											dfFirstFunding,
											fundDf,
											&(fund_leg[realisation]),
											&(fund_legShort[realisation]));	
			}
			else
			{
				err = OTCNewfundingValuation(	matrix[realisation][0],
												matrix[realisation][1],
												matrix[realisation][2],
												cpd,
												und,
			...
			if(Calc_flat)
			{
				...
				else
				{
					...
					{
						fund_leg_flat[realisation] = fund_leg[realisation] * matrix[realisation][3] / matrix[realisation][2];
						fund_legShort_flat[realisation] = fund_legShort[realisation] * matrix[realisation][3] / matrix[realisation][2];
					}
					else
					{
			...
			for (i = pd_otc_start; i < cpd->pd_leg->num_cpn; i++)
			{
				...
				================================== */
				forward = OTC_precalc->FxAdj[OTC_params->OTC][i] * matrix[realisation][2] * PDFxFordfReal[i] / PDFxDomdfReal[i];
				if(Calc_flat) forward_flat = OTC_precalc->FxAdj[OTC_params->OTC][i] * matrix[realisation][3] * PDFxFordfReal[i] / PDFxDomdfReal[i];

				/* ==================================
				domestic DF @ Tpay
				================================== */
				//the forward vols are of beta type and vol mkt of log type
				err = cpd_vol_get_smile_params_from_otc(smile_mkt->smile_spec_type+1,OTC_params->OTC, i,
														OTC_precalc,smile_params);
				if(err) goto FREE_RETURN;

				smile_params_flat->sigma = OTC_precalc->pd_fwd3F_vol[OTC_params->OTC][i];
				...
				if(!cpn->use_opt_str)
				{
					...
					pd_leg[realisation] += PDdfReal[i] * (cpn->alpha + cpn->beta * forward + fabs(cpn->beta) * (floor - cap));
					if(Calc_flat) pd_leg_flat[realisation] += PDdfReal[i] * (cpn->alpha + cpn->beta * forward_flat + fabs(cpn->beta) * (floor_flat - cap_flat));
					if(OTC_params->OTC < cpd->num_calls - 1)
					{
						if(i >= pd_otc_start && i < pd_otc_short_end)
						{
							pd_legShort[realisation] += PDdfReal[i] * (cpn->alpha + cpn->beta * forward + fabs(cpn->beta) * (floor - cap));
							if(Calc_flat) pd_legShort_flat[realisation] += PDdfReal[i] * (cpn->alpha + cpn->beta * forward_flat + fabs(cpn->beta) * (floor_flat - cap_flat));
						}
					}
				}
				else
				{
					...
					for (str_idx = 0; str_idx < cpn->nstrikes; str_idx++)
					{
						if (fabs(cpn->weights[str_idx]) > 1e-16 * pd_not)
						{
							...
							if (OTC_precalc->pd_fwdsmile_vol[OTC_params->OTC][i] * sqrt (cpn->fx_fix_time - cpd->call[OTC_params->OTC].ex_time)> 1e-16 
								&& cpn->strikes[str_idx] > 1e-16) type = 3;
							else type = 1;
					...
					/*	Coupon pv */		
					pd_leg[realisation] += PDdfReal[i] * (cpn->wcst + cpn->wspot * forward + opt_string);
					if(Calc_flat) pd_leg_flat[realisation] += PDdfReal[i] * (cpn->wcst + cpn->wspot * forward_flat + opt_string_flat);
					if(OTC_params->OTC < cpd->num_calls - 1)
					{
						if(i >= pd_otc_start && i < pd_otc_short_end)
						{
							pd_legShort[realisation] += PDdfReal[i] * (cpn->wcst + cpn->wspot * forward + opt_string);
							if(Calc_flat) pd_legShort_flat[realisation] += PDdfReal[i] * (cpn->wcst + cpn->wspot * forward_flat + opt_string_flat);
						}
					}
				}
			}
		...
		if(OTC_params->OTC == cpd->num_calls - 1)
		{
			fund_feeShort = fund_fee;
			pd_feeShort = pd_fee;
			if(Calc_flat) pd_feeShort_flat = pd_fee_flat;
		}


		df = swp_f_df (und->today, tstarDate, und->dom_yc);
		
		if(Calc_flat)	Results[OTC_params->OTC + 1][0] = tstarTime;
		...
		if(Calc_flat)
		{
			...
			Results[OTC_params->OTC + 1][5] = Pay_Rec_Mult * (pd_fee_flat - fund_fee_flat);
		}
		else
		{
		...
		{
			/* Smile */
			callRealisation = Pay_Rec_Mult * ((pd_leg[realisation] + pd_fee) - (fund_leg[realisation] + fund_fee));
			...
			callRealisation = callRealisation - Pay_Rec_Mult * (((pd_legShort[realisation] + pd_feeShort) - (fund_legShort[realisation] + fund_feeShort)));
			if(callRealisation > 0.0)	
			{
				if(Calc_flat)
			...
			callRealisation = Pay_Rec_Mult * ((pd_legShort[realisation] + pd_feeShort) - (fund_legShort[realisation] + fund_feeShort));
			if(callRealisation > 0.0)	
			{
				if(Calc_flat)
			...
			{
				/* 3F Smile */
				callRealisation = Pay_Rec_Mult * ((pd_leg_flat[realisation] + pd_fee_flat) - (fund_leg_flat[realisation] + fund_fee_flat));
				if(callRealisation > 0.0)	
					Results[OTC_params->OTC + 1][6] += callRealisation;

				callRealisation = callRealisation - Pay_Rec_Mult * (((pd_legShort_flat[realisation] + pd_feeShort_flat) - (fund_legShort_flat[realisation] + fund_feeShort_flat)));
				if(callRealisation > 0.0)	
					Results[OTC_params->OTC + 1][8] += callRealisation;

				callRealisation = Pay_Rec_Mult * ((pd_legShort_flat[realisation] + pd_feeShort_flat) - (fund_legShort_flat[realisation] + fund_feeShort_flat));
				if(callRealisation > 0.0)	
		...
		else
		{
			...
			Results[3][0] *= df / OTC_params->COPULAnSimul / sqrt(OTC_params->COPULAnSimul);

			Results[0][1] *= df;
			Results[1][1] *= df;
			Results[2][1] *= df / OTC_params->COPULAnSimul;
			Results[3][1] *= df / OTC_params->COPULAnSimul / sqrt(OTC_params->COPULAnSimul);

			Results[0][2] = Results[0][0] - Results[0][1];
			Results[1][2] = Results[1][0] - Results[1][1];
			Results[2][2] *= df / OTC_params->COPULAnSimul;
			Results[3][2] *= df / OTC_params->COPULAnSimul / sqrt(OTC_params->COPULAnSimul);
		}
	}
	else
	{	
	...
	if(!Calc_flat)
	{
		if(Results[2][0] < Pay_Rec_Mult * (Results[1][0] - Results[0][0]))
			Results[2][0] = Pay_Rec_Mult * (Results[1][0] - Results[0][0]) * (1.0 + 1.0e-10);

		if(Results[2][1] < Pay_Rec_Mult * (Results[1][1] - Results[0][1]))
			Results[2][1] = Pay_Rec_Mult * (Results[1][1] - Results[0][1]) * (1.0 + 1.0e-10);

		if(Results[2][2] < Pay_Rec_Mult * (Results[1][2] - Results[0][2]))
			Results[2][2] = Pay_Rec_Mult * (Results[1][2] - Results[0][2]) * (1.0 + 1.0e-10);
	}

FREE_RETURN:
	if(smile_params_flat) free(smile_params_flat);
	
	if(fund_leg)	free(fund_leg);
	if(pd_leg)		free(pd_leg);
	if(fund_legShort)	free(fund_legShort);
	if(pd_legShort)		free(pd_legShort);

	if(fund_leg_flat)	free(fund_leg_flat);
	if(pd_leg_flat)		free(pd_leg_flat);
	if(fund_legShort_flat)	free(fund_legShort_flat);
	if(pd_legShort_flat)		free(pd_legShort_flat);

	if(fundDf)	free(fundDf);
	if(PdDfDom)		free(PdDfDom);
	if(FwdDfDom)	free(FwdDfDom);
	if(FwdDfFor)	free(FwdDfFor);

	if(strikeCap)	free(strikeCap);
	if(strikeFloor)	free(strikeFloor);

	if(PDdfReal)	free(PDdfReal);
	if(PDFxDomdfReal)	free(PDFxDomdfReal);
	if(PDFxFordfReal)	free(PDFxFordfReal);

	if(NewCoupons)	free(NewCoupons);
	
	return err;
}

Err OTCfundingValuation(	double	domReal,
							double	forReal,
							double	fxReal,
							CPD_STR cpd,
							CPD_UND	und,
							...
							int		otc,
							double	dfNotExchge,
							double	dfNotExchgeShort,
							double	dfFirstFunding,
							double	*fundDf,
							double	*fund_leg,
...
Err err = NULL;
	...
	================================== */
	/* Initial Notional */
	df = exp(OTC_precalc->df_const_fund_pay[otc][cpd->fund_leg->num_cpn + 1] + OTC_precalc->df_lin_fund_pay[otc][cpd->fund_leg->num_cpn + 1] * real);
	...
	*fund_leg = - dfNotExchge * df * cpd->fund_leg->notional;
	...
	if(otc < cpd->num_calls - 1)
	{
		*fund_legShort = *fund_leg;
		...
		/*	Final Notional For Short option*/
		df = exp(OTC_precalc->df_const_fund_pay[otc][cpd->fund_leg->num_cpn + 2] + OTC_precalc->df_lin_fund_pay[otc][cpd->fund_leg->num_cpn + 2] * real);
	...
	================================== */
	/* B(t,Ts) */
	df = exp(OTC_precalc->df_const_fund_start[otc] + OTC_precalc->df_lin_fund_start[otc] * real);
	...
	*fund_leg += dfFirstFunding * df * cpd->fund_leg->notional;
	...
	for (i = fund_otc_start; i < cpd->fund_leg->num_cpn; i++)
	{
		...
		coupon: spread + margin 
		================================== */
		df = exp(OTC_precalc->df_const_fund_pay[otc][i] + OTC_precalc->df_lin_fund_pay[otc][i] * real);
...
Err OTCNewfundingValuation(	double	domReal,
							double	forReal,
							double	fxReal,
							CPD_STR cpd,
							CPD_UND	und,
...
double			*fundDf = NULL, *PdDfDom = NULL, *FwdDfDom = NULL, *FwdDfFor = NULL;
double			discount, df, dfDom, dfFor, dfFirstFunding, dfNotExchge, dfFinalNotExchge, df0T, df0Tstar;

/* For the correl */
double			*fund_leg = NULL, *pd_leg = NULL;
double			*fund_leg1 = NULL, *pd_leg1 = NULL;
int				pd_otc_start, pd_otc_end;			/*	index of the first coupon to be called */
...
Err err = NULL;
	...
	if(!fund_leg || !pd_leg || !fundDf || !PdDfDom || !FwdDfDom || 
		!FwdDfFor || !strikeCap || !strikeFloor) 
	{
		err = "OTCcaller: memory allocation error";
		goto FREE_RETURN;
	}
	...
	for(ii = 0; ii < 2; ii++)
	{
		...
		dfFirstFunding = swp_f_df( und->today, cpd->fund_leg->cpn[fund_otc_start].start_date, und->dom_yc) / dfDom;
		dfNotExchge = swp_f_df( und->today, cpd->call[Index].set_date, und->dom_yc) / dfDom;
		
		if(Index < cpd->num_calls - 1)
			dfFinalNotExchge = swp_f_df( und->today, cpd->call[Index + 1].set_date, und->dom_yc) / dfDom;
		else
			dfFinalNotExchge = swp_f_df( und->today, cpd->pd_leg->cpn[pd_otc_end - 1].pay_date, und->dom_yc) / dfDom;
		
		for(i = fund_otc_start; i < fund_otc_end; i++)
			fundDf[i] =  swp_f_df( und->today, cpd->fund_leg->cpn[i].pay_date, und->dom_yc) / dfDom;
		...
		for(realisation = 0; realisation<nSimul; realisation++)
		{
			...
			if(pd_otc_end == cpd->pd_leg->num_cpn)
			{
				...
				if (fabs (cpn->beta) > 1.0e-16)
				{
					...
					forward = FxAdj[cpd->pd_leg->num_cpn] * matrix[realisation][2] * dfFor / dfDom;
			...
			else
			{
				...
				err = OTCdf(	matrix[realisation][0],
								und->lda_dom,
								cpd->call[otc].ex_time,
								cpd->pd_leg->cpn[pd_otc_end - 1].pay_time,
								tstarTime,
								phiDom,
								&df);
			...
			err = OTCdf(	matrix[realisation][0],
							und->lda_dom,
							cpd->call[otc].ex_time,
							cpd->fund_leg->cpn[fund_otc_start].start_time,
							tstarTime,
							phiDom,
							&df);
			...
			for (i = pd_otc_start; i < pd_otc_end; i++)
			{
				...
				pd_leg[realisation] += dfDom * (cpn->alpha + cpn->beta * forward + fabs(cpn->beta) * (floor - cap));
			}
...
FREE_RETURN:
	...
	if(fund_leg1)	free(fund_leg1);
	if(pd_leg1)		free(pd_leg1);

	if(dfTstarVec)	free(dfTstarVec);
...
Err	fwdSABRpayoff(	long		today,
					...
					double		forward,
					double		fwdsmile_vol,
					double		fwdsmile_alpha,
					double		fwdsmile_beta,
					double		fwdsmile_rho,
					double		**matrix,
					int			nSimul,
					double		tstarTime,
...
Err err = NULL;
	...
	for(realisation = 0; realisation<nSimul; realisation++)
	{
		...
		for(j = 0; j < nStrikes; j++)
		{
			...
			if (localForward < 0.0 && type >= 3)
			{
				Results[j] += dfDom * OPT_VAL_MACRO(	type - 2, 
														localForward, 
														Strikes[j],
														smile_std,
														smile_half_std);
			...
			{
				Results[j] += dfDom * OPT_VAL_MACRO(	type, 
														localForward, 
														Strikes[j], 
														smile_std,
														smile_half_std);
	...
	if(OutputVol)
	{
		for(j = 0; j < nStrikes; j++)
		{	
			err =  srt_f_optimpvol(	Results[j],
									forward,
									Strikes[j],
									fixTime,
									dfDom,
									isCall? SRT_CALL:SRT_PUT,
									SRT_LOGNORMAL,
									&vol);	
...
Err	fwdSABRpayoffAlphaFudge(	long		today,
								long		forwardFixDate,
								double		forwardFixTime,
								long		valDate,
								double		fixTime,
	...
	for(realisation = 0; realisation<nSimul; realisation++)
	{
		...
		for(j = 0; j < nStrikes; j++)
		{
			...
			err = srt_f_optsarbvol(	localForward, Strikes[j], fixTime - forwardFixTime, 
									betaVolUp, fwdsmile_alpha, fwdsmile_beta, fwdsmile_rho,
			...
			if (localForward < 0.0 && type >= 3)
			{
				Results[j] += p * dfDom * OPT_VAL_MACRO(	type - 2, 
																localForward, 
																Strikes[j],
																smile_std,
																smile_half_std);
			...
			{
				Results[j] += p * dfDom * OPT_VAL_MACRO(	type, 
																localForward, 
																Strikes[j], 
																smile_std,
																smile_half_std);
			...
			err = srt_f_optsarbvol(	localForward, Strikes[j], fixTime - forwardFixTime, 
									betaVolDown, fwdsmile_alpha, fwdsmile_beta, fwdsmile_rho,
									SRT_BETAVOL, SRT_LOGNORMAL, &smile_std);
			...
			if (localForward < 0.0 && type >= 3)
			{
				Results[j] += (1.0 - p) * dfDom * OPT_VAL_MACRO(	type - 2, 
																	localForward, 
																	Strikes[j],
																	smile_std,
																	smile_half_std);
			...
			{
				Results[j] += (1.0 - p) * dfDom * OPT_VAL_MACRO(	type, 
																	localForward, 
																	Strikes[j], 
																	smile_std,
																	smile_half_std);
...
====================================================
==================================================== */
Err KO_get_coupon_paid(	int			Coupon_idx,
						CPD_STR		cpd,
						double		fx,
						double		*Coupon_value)
{
...
Err				err = NULL;
	...
	if(!cpn->use_opt_str)
	{
		...
		*Coupon_value = cpn->alpha + cpn->beta * fx + fabs (cpn->beta) * (floor - cap);
	}
	else
	{
		opt_string = 0.0;
		for (str_idx = 0; str_idx < cpn->nstrikes; str_idx++)
		{
			if (fabs(cpn->weights[str_idx]) > 1e-16 * cpd->call[Coupon_idx].pd_not_amt)
			{
...
double			discount, df, dfDom, dfFor, dfFund, dfFirstFunding, 
				dfNotExchgePd, dfNotExchgeShortPd,
				dfNotExchgeFund, dfNotExchgeShortFund,
				dfStartNotExchgePd, dfStartNotExchgeFund,
				dfNotExchgeEndPd, dfNotExchgeEndFund;
double			start_time;
...
Err err = NULL;
	...
	/* =============================
	Populates a vector with 0-1 => 0 : Pay Coupons; 1 : Exchange Notionals; 2 : allready KO in the past
	============================= */
	...
	if(!cpd->call[0].TARN_Do)
	{
		...
		for(j = 0; j < OTC_params->OTC; j++)
			Barrier[j] = cpd->call[j].orig_barrier / OTC_precalc->KO_barrier[OTC_params->OTC][j];
		...
		for(realisation = 0; realisation < OTC_params->COPULAnSimul; realisation++)
		{
			...
			{
				if(OTC_params->KO_FxMatrix[realisation][j] > Barrier[j]) //change the barrier /* To do */
				{
					if(!OTC_params->KO_do_optim || (OTC_params->KO_do_optim && cpd->call[j].call_type))
					{
			...
			{
				if(OTC_params->KO_FxMatrix[realisation][OTC_params->OTC] > Barrier[OTC_params->OTC])
				{
					if(!OTC_params->KO_do_optim || (OTC_params->KO_do_optim && cpd->call[OTC_params->OTC].call_type))
					{
				...
				{
					//Ncoupons --;			/* To calculate the proba of an exchange of notional */
				}
			}
		}
	}
	else
	{
		...
		for(realisation = 0; realisation < OTC_params->COPULAnSimul; realisation++)
		{
			...
			for(j = 0; j < OTC_params->OTC && !KnockedOut[realisation]; j++)
			{
				fx = OTC_params->KO_FxMatrix[realisation][j] * OTC_precalc->KO_barrier[OTC_params->OTC][j];
			...
			if(!KnockedOut[realisation])
			{
				...
				{
					KnockedOut[realisation] = 1;
					Ncoupons --;			/* To calculate the proba not to have hitten the TARN Level */
				}
				else
				{
				}
	...
	if(OTC_params->OTC < cpd->num_calls - 1)
	{
		dfNotExchgeEndPd = swp_f_df( und->today, cpd->call[OTC_params->OTC+1].set_date, und->dom_yc) / dfDom;
		dfNotExchgeEndFund = swp_f_df( und->today, cpd->call[OTC_params->OTC+1].set_date, fund_yc) / dfFund;
	}
	...
	if(start_date > cpd->call[OTC_params->OTC].ex_date)
		fund_fee -= swp_f_df( und->today, start_date, fund_yc) * cpd->fund_leg->notional;	
	...
	for(i = 0; i < 2; i++)
	{
		...
		{
			if(cpd->call[OTC_params->OTC+i].ex_date > start_date)//&& OTC_params->OTC == 0)
			{
				exchange_fund = fund_mult * swp_f_df( und->today, cpd->call[OTC_params->OTC+i].set_date, fund_yc) * cpd->fund_leg->notional;		

				if(!OTC_params->OTC && !i)
					ko_fund_fee = exchange_fund;		

				if(OTC_params->KO_do_optim || (!OTC_params->KO_do_optim && !(!OTC_params->OTC && !i)))
				{
					if(!i)
			...
			else if(cpd->call[OTC_params->OTC+i].ex_date <= start_date && OTC_params->OTC == 0)
				ko_fund_fee = 0.0;		
			...
			if(cpd->call[OTC_params->OTC + i].use_opt_str)			
			{
				...
				SABR parameters @ Tval
				================================== */
				err = OTCgetSMILEparams(cpd, und,cpd->call[OTC_params->OTC + i].ex_time, 
										cpd->call[OTC_params->OTC+i].fx_val_time, cpd->call[OTC_params->OTC+i].fx_val_date,
										smile_mkt, smile_params, OTC_params->OTCsmileFee);
				...
				/*	Coupon pv */		
				exchange_pd = dfDom * (cpd->call[OTC_params->OTC+i].wcst + cpd->call[OTC_params->OTC+i].wspot * forward + opt_string + cpd->call[OTC_params->OTC + i].orig_fee);
				...
				if(cpd->call[OTC_params->OTC+i].ex_date > start_date)//&& OTC_params->OTC == 0)
				{
					...
					ko_pd_fee = exchange_pd - dfDom * cpd->call[OTC_params->OTC+i].pd_not_amt;
			...
			else
			{
				if(cpd->call[OTC_params->OTC+i].ex_date > start_date)//&& OTC_params->OTC == 0)
				{
					exchange_pd = dfDom * (pd_not + cpd->call[OTC_params->OTC+i].orig_fee);
	...
	//Coupon evaluation
	smile_params->Use_BetaQuick_in_MC = 1;
	for(realisation = 0; realisation<OTC_params->COPULAnSimul && (OTC_params->KO_do_optim || fmc_current_std > OTC_params->KO_FMC_precision); realisation++)
	{
		coupon_pd = 0.0;
		coupon_fund = 0.0;
		ko_coupon_pd = 0.0;
		...
		if(start_date > cpd->call[OTC_params->OTC].ex_date)
		{
			...
			err = OTCdf(	OTC_params->KO_RatesMatrix[realisation][0],
							und->lda_dom,
							cpd->call[OTC_params->OTC].ex_time,
							start_time,
							tstarTime,
		...
		for (i = pd_otc_start; i < pd_otc_short_end; i++)
		{
			...
			Forward @ Tval with Convexity adjustment
			================================== */
			df = exp(OTC_precalc->df_const_for_fx_val[OTC_params->OTC][i] + OTC_precalc->df_lin_for_fx_val[OTC_params->OTC][i] * OTC_params->KO_RatesMatrix[realisation][1]);

			dfFor = FwdDfFor[i] * df;

			df = exp(OTC_precalc->df_const_dom_fx_val[OTC_params->OTC][i] + OTC_precalc->df_lin_dom_fx_val[OTC_params->OTC][i] * OTC_params->KO_RatesMatrix[realisation][0]);

			dfDom = FwdDfDom[i] * df;

			forward = OTC_precalc->FxAdj[OTC_params->OTC][i] * OTC_params->KO_FxMatrix[realisation][OTC_params->OTC] * dfFor / dfDom;
			...
			if(cpn->pay_date != cpn->fx_val_date)
			{
				df = exp(OTC_precalc->df_const_pd_pay[OTC_params->OTC][i] + OTC_precalc->df_lin_pd_pay[OTC_params->OTC][i] * OTC_params->KO_RatesMatrix[realisation][0]);
			}
			...
			if(!cpn->use_opt_str)
			{
				...
				coupon_pd += dfDom * (cpn->alpha + cpn->beta * forward + fabs(cpn->beta) * (floor - cap));
			}
			else
			{
				opt_string = 0.0;
				...
				for (str_idx = 0; str_idx < cpn->nstrikes; str_idx++)
				{
					if (fabs(cpn->weights[str_idx]) > 1e-16 * pd_not)
					{
						if (OTC_precalc->pd_fwdsmile_vol[OTC_params->OTC][i] * sqrt (cpn->fx_fix_time) > 1e-16 && cpn->strikes[str_idx] > 1e-16) type = 3;
						else type = 1;
		...
		if(OTC_params->OTC == cpd->num_calls - 1)
		{
			...
			/*	Discount */
			df = exp(OTC_precalc->df_const_pd_pay[OTC_params->OTC][cpd->pd_leg->num_cpn] + OTC_precalc->df_lin_pd_pay[OTC_params->OTC][cpd->pd_leg->num_cpn] * OTC_params->KO_RatesMatrix[realisation][0]);
			...
			df = exp(OTC_precalc->df_const_dom_fx_val[OTC_params->OTC][cpd->pd_leg->num_cpn] + OTC_precalc->df_lin_dom_fx_val[OTC_params->OTC][cpd->pd_leg->num_cpn] * OTC_params->KO_RatesMatrix[realisation][0]);

			dfDom = FwdDfDom[cpd->pd_leg->num_cpn] * df;

			df = exp(OTC_precalc->df_const_for_fx_val[OTC_params->OTC][cpd->pd_leg->num_cpn] + OTC_precalc->df_lin_for_fx_val[OTC_params->OTC][cpd->pd_leg->num_cpn] * OTC_params->KO_RatesMatrix[realisation][1]);

			dfFor = FwdDfFor[cpd->pd_leg->num_cpn] * df;

			forward = OTC_precalc->NotFxAdj[OTC_params->OTC][2] * OTC_params->KO_FxMatrix[realisation][OTC_params->OTC] * dfFor / dfDom;
			...
			if(!cpn->use_opt_str)
			{
				...
				/*	Coupon pv */		
				coupon_pd += discount * (cpn->alpha + cpn->beta * forward + fabs (cpn->beta) * (floor - cap));
		...
		if(start_date > cpd->call[OTC_params->OTC].ex_date)
		{
			...
			err = OTCdf(	cpd->fund_leg->dom_for? OTC_params->KO_RatesMatrix[realisation][1]:OTC_params->KO_RatesMatrix[realisation][0],
							cpd->fund_leg->dom_for? und->lda_for:und->lda_dom,
							cpd->call[OTC_params->OTC].ex_time,
							start_time,
							tstarTime,
							cpd->fund_leg->dom_for? phiFor:phiDom,
							&df);
		...
		if(cpd->fund_leg->dom_for)
			df = exp(OTC_precalc->df_const_fund_start[OTC_params->OTC] + OTC_precalc->df_lin_fund_start[OTC_params->OTC] * OTC_params->KO_RatesMatrix[realisation][1]);
		else
			df = exp(OTC_precalc->df_const_fund_start[OTC_params->OTC] + OTC_precalc->df_lin_fund_start[OTC_params->OTC] * OTC_params->KO_RatesMatrix[realisation][0]);
		...
		for (i = fund_otc_start; i < fund_otc_short_end; i++)
		{
			...
			================================== */
			if(cpd->fund_leg->dom_for)
				df = exp(OTC_precalc->df_const_fund_pay[OTC_params->OTC][i] + OTC_precalc->df_lin_fund_pay[OTC_params->OTC][i] * OTC_params->KO_RatesMatrix[realisation][1]);
			else
				df = exp(OTC_precalc->df_const_fund_pay[OTC_params->OTC][i] + OTC_precalc->df_lin_fund_pay[OTC_params->OTC][i] * OTC_params->KO_RatesMatrix[realisation][0]);
		...
		for(i = 0; i < 2; i++)
		{
			if(OTC_params->OTC < cpd->num_calls - 1 || !i)
			{
				...
				if(cpd->call[OTC_params->OTC+i].ex_date > start_date)
				{
					...
					================================== */
					if(cpd->fund_leg->dom_for)
						df = exp(OTC_precalc->df_const_fund_pay[OTC_params->OTC][cpd->fund_leg->num_cpn + 1 + i] + OTC_precalc->df_lin_fund_pay[OTC_params->OTC][cpd->fund_leg->num_cpn + 1 + i] * OTC_params->KO_RatesMatrix[realisation][1]);
					else
						df = exp(OTC_precalc->df_const_fund_pay[OTC_params->OTC][cpd->fund_leg->num_cpn + 1 + i] + OTC_precalc->df_lin_fund_pay[OTC_params->OTC][cpd->fund_leg->num_cpn + 1 + i] * OTC_params->KO_RatesMatrix[realisation][0]);
					...
					if(cpd->fund_leg->dom_for)
						exchange_fund *= OTC_params->KO_FxMatrix[realisation][OTC_params->OTC];
				...
				else if(cpd->call[OTC_params->OTC].ex_date <= start_date && OTC_params->OTC == 0)
					ko_coupon_fund = 0.0;
				
					
					
				//Pd
				df = exp(OTC_precalc->df_const_pd_pay[OTC_params->OTC][cpd->pd_leg->num_cpn + 1 + i] + OTC_precalc->df_lin_pd_pay[OTC_params->OTC][cpd->pd_leg->num_cpn + 1 + i] * OTC_params->KO_RatesMatrix[realisation][0]);
				...
				if(cpd->call[OTC_params->OTC + i].use_opt_str)			
				{
					...
					Forward @ Tval
					================================== */
					forward = OTC_precalc->NotFxAdj[OTC_params->OTC][i] * OTC_params->KO_FxMatrix[realisation][OTC_params->OTC] * dfNotEx[i];

					df = exp(OTC_precalc->df_const_dom_fx_val[OTC_params->OTC][cpd->pd_leg->num_cpn + i + 1] + OTC_precalc->df_lin_dom_fx_val[OTC_params->OTC][cpd->pd_leg->num_cpn + i + 1] * OTC_params->KO_RatesMatrix[realisation][0]);

					forward /= df;

					df = exp(OTC_precalc->df_const_for_fx_val[OTC_params->OTC][cpd->pd_leg->num_cpn + i + 1] + OTC_precalc->df_lin_for_fx_val[OTC_params->OTC][cpd->pd_leg->num_cpn + i + 1] * OTC_params->KO_RatesMatrix[realisation][1]);
					...
					exchange_pd = dfDom * (	cpd->call[OTC_params->OTC + i].wcst + 
											cpd->call[OTC_params->OTC + i].wspot * forward + 
											opt_string + cpd->call[OTC_params->OTC + i].orig_fee);					
		...
		if(KnockedOut[realisation] == 2)
		{
			...
			if(OTC_params->KO_FMC && (realisation+1)%500 == 0 && realisation > OTC_params->KO_FMC_min_paths)
			{
				fmc_current_std = fmc_current_var - fmc_current_mean * fmc_current_mean / (realisation + 1);
		...
		else if(KnockedOut[realisation] == 0)
		{
			...
			{
				fmc_current_var += (coupon_pd - coupon_fund) * (coupon_pd - coupon_fund);
				fmc_current_mean += coupon_pd - coupon_fund;
				
				if((realisation+1)%500 == 0 && realisation > OTC_params->KO_FMC_min_paths)
				{
					fmc_current_std = fmc_current_var - fmc_current_mean * fmc_current_mean / (realisation + 1);
					fmc_current_std /= (realisation + 1);
					fmc_current_std = sqrt(fmc_current_std / (realisation + 1));
		...
		else if(KnockedOut[realisation] == 1)
		{
			...
			if(OTC_params->KO_FMC)
			{
				fmc_current_var += (ko_coupon_pd - ko_coupon_fund) * (ko_coupon_pd - ko_coupon_fund);
	...
	if(!OTC_params->KO_do_optim && OTC_params->KO_FMC)
	{
		...
		smessage ("Phase 4 -Payoff evaluation Finished with %.2d simulations", fmc_nb_realisations);
	}
	else
	{
	...
	if(!OTC_params->KO_do_optim)
	{
	PriceStd = (Payoff_pd[0] - Payoff_fund[0]) * (Payoff_pd[0] - Payoff_fund[0]);
	...
	for(realisation = 1; realisation<fmc_nb_realisations; realisation++)
	{
		...
		PriceStd += (Payoff_pd[realisation] - Payoff_fund[realisation]) * (Payoff_pd[realisation] - Payoff_fund[realisation]); 
	}

	PriceStd -= (Payoff_fund[0] - Payoff_pd[0]) * (Payoff_fund[0] - Payoff_pd[0]) / fmc_nb_realisations;
	PriceStd /= fmc_nb_realisations;
	PriceStd = sqrt(PriceStd / fmc_nb_realisations);

	if(OTC_params->OTCsmileFee > -1)
	{
		Payoff_pd[0] -= (pd_fee_mc / fmc_nb_realisations - pd_fee / df) * fmc_nb_coupons;
		Payoff_fund[0] -= (fund_fee_mc / fmc_nb_realisations - fund_fee / df) * fmc_nb_coupons;
	}
	...
	Results[2+OTC_params->OTC][1] = (double) Ncoupons / OTC_params->COPULAnSimul;
	...
	if(OTC_params->OTCsmileFee > -1)
		Results[2+OTC_params->OTC][4] = (fund_fee_mc / fmc_nb_realisations - fund_fee / df) - (pd_fee_mc / fmc_nb_realisations - pd_fee / df);
	...
	if(!OTC_params->OTC)
	{
		...
		ko_fund_fee_mc = (ko_fund_fee_mc / OTC_params->COPULAnSimul - ko_fund_fee / df);
		for(realisation = 0; realisation<OTC_params->COPULAnSimul; realisation++)
		{
	...
	for(realisation = 0; realisation<OTC_params->COPULAnSimul; realisation++)
	{
		save_values[OTC_params->OTC+1][0][realisation] = OTC_params->KO_FxMatrix[realisation][OTC_params->OTC];
		
		if(!KnockedOut[realisation])
		{
			save_values[OTC_params->OTC+1][1][realisation] = Payoff_fund[realisation] - fund_fee;
			save_values[OTC_params->OTC+1][1][realisation] -= Payoff_pd[realisation] - pd_fee;
...
FREE_RETURN:
	...
	if(KnockedOut)		free(KnockedOut);
	if(Payoff_fund)		free(Payoff_fund);
	if(Payoff_pd)		free(Payoff_pd);
...
UTILS
==================================================== */
Err OTCgetOptValue(	int					IsOption,
					int					IsCap,
					double				cap,
					double				Strike,
					double				Notional,
...
Err		err = NULL;
	...
	if(IsOption)
	{
		...
		if (type >= 3)
		{
			err = cpd_vol_get_vol(Forward,Maturity,Strike, smile_params, SRT_LOGNORMAL, &smile_std);
			if(err) goto FREE_RETURN;
		...
		*Value = OPT_VAL_MACRO(	type, 
		...
		err = cpd_vol_get_price(type, Forward, Maturity, Strike, smile_params, Value);
		if(err) goto FREE_RETURN;
...
Err OTCgetCopula(	double	forward,
					double	forwardFixTime,
					double	tstarTime,
					double	*mergeTimes, 
					int		mergeNtimes,
					...
					double **matrix)
{
double			stdDom, cumulDomForCor, 
				stdFor, cumulForFxCor, meanFor, 
				cumulDomFxCor;
...
Err err = NULL;
	...
	err = balsam_generation (	nSimul, 
								3,
								gauss);
	
	if (err) goto FREE_RETURN;

	adjust_emp_covar (	gauss,
						nSimul,
						3);
	...
	================================== */
	err = OTCgetMoments(	forwardFixTime,
							lda_dom,
							lda_for,
							tstarTime,
							mergeTimes,
	...
	err = OTCcorrelateVariables(	stdDom,
									stdFor,
									cumulDomForCor,
									cumulDomFxCor,
									cumulForFxCor,
	...
	else if(smileModel == 1)
	{
		...
		err = copula_gaussian_get_BMM_linterp_cumulative(	forwardFixTime,
															forward,
															BMfwd1,
															BMsig1,
															BMfwd2,
...
FREE_RETURN:
	if(gauss)		free_dmatrix (gauss, 0, nSimul - 1, 0, 3 - 1);
	if(cumulative)	free_dmatrix (cumulative, 0, 2 - 1, 0, nPoints - 1);
	if(dX)			free(dX);
	if(dCumulative)	free(dCumulative);
...
Err OTCutilsInterpolate(	double	X,
							double	XX,
							double	XXX,
							double	F1,
							double	F2,
...
Err err = NULL;
	...
	if(method == 2) 
	{
		...
		else if(fabs(X - XX) < 1.0e-15 || fabs(XX - XXX) < 1.0e-15) //lin interp X XXX
		{
			Islin = 1;
			WhichSeg = 2;
		}
...
Err OTCutilsGetIndex(	double	*probas,
						int		Nprobas,
						double	prob,
						int		isLinear,
						int		*index1,
...
Err err = NULL;
	...
	else
	{
		...
		{
			//step = 0.3 * probas[(int)(Nprobas / 2.0)] - probas[(int)(Nprobas / 2.0) - 1];
			*index1 = (int) (prob * (double)(Nprobas - 1));
			...
			do
			{
				*index1 += dir;
				test = probas[*index1];
				if((dir == 1 && test > prob) || (dir == -1 && test < prob) || *index1 <= 0 || *index1 >= Nprobas - 1)
					stop = 1;
...
int		i, index1, index2, index3, method;
	...
	{
		otc_params->KO_FxMatrix[i][otc_params->OTC] = norm(otc_params->KO_FxGauss[i][otc_params->OTC]);
		
		err = OTCutilsGetIndex(	otc_params->CUMUL[1],
								otc_params->CUMULnPoints,
								otc_params->KO_FxMatrix[i][otc_params->OTC],
								otc_params->CUMULlinear,
								&index1,
								&index2,
								&index3,
								&method);
		...
		err = OTCutilsInterpolate(	otc_params->CUMUL[0][index1],
									otc_params->CUMUL[0][index2],
									otc_params->CUMUL[0][index3],
									otc_params->CUMUL[1][index1],
									otc_params->CUMUL[1][index2],
									otc_params->CUMUL[1][index3],
									otc_params->KO_FxMatrix[i][otc_params->OTC],
									method,
									&test1,
									&test2);
		...
		{
			if(test1 > otc_params->CUMUL[0][index1] && test1 < otc_params->CUMUL[0][index3])
				otc_params->KO_FxMatrix[i][otc_params->OTC] = test1;
			else if (test2 > otc_params->CUMUL[0][index1] && test2 < otc_params->CUMUL[0][index3])
				otc_params->KO_FxMatrix[i][otc_params->OTC] = test2;
			else if (index1 == 0 && fabs(otc_params->KO_FxMatrix[i][otc_params->OTC] - test1)<1.0e-10)
				otc_params->KO_FxMatrix[i][otc_params->OTC] = test1;
			else if (otc_params->KO_FxMatrix[i][otc_params->OTC] > otc_params->CUMUL[1][otc_params->CUMULnPoints - 1])
				otc_params->KO_FxMatrix[i][otc_params->OTC] = otc_params->CUMUL[0][otc_params->CUMULnPoints - 1];
			else if (otc_params->KO_FxMatrix[i][otc_params->OTC] < otc_params->CUMUL[1][0])
...
Err OTCutilsCorrelateFxAndRates(	otcpd_params	*OTC_params,
									double			meanDom,
									double			stdDom,
									double			meanFor,
									double			stdFor,
...
double	b, c, d, e, f, x, y, z;
	...
	for (i=0; i<OTC_params->COPULAnSimul; i++)
	{
		...
		OTC_params->KO_RatesMatrix[i][1] = meanFor + stdFor * (d * x + e * y + f * z);
	}
...
================================== */
Err alloc_calib_sl_params(	int				iNbStrikes,
							CALIBSL_PARAMS	sParams)
{
Err	err = NULL;
...
void free_calib_sl_params(CALIBSL_PARAMS	sParams)
{
	if (sParams)
	{
		if (sParams->dStrikes) free (sParams->dStrikes);
		if (sParams->dTargetPrices) free (sParams->dTargetPrices);
		if (sParams->dTargetVols) free (sParams->dTargetVols);
	}
}

static void NAG_CALL MixedSL_Calib_Fonctionnal(Integer n, double x[], double *objf, double g[], Nag_Comm *comm)
{
...
Err				err = NULL;
	...
	{
		/* Pricing */
		dPremium = srt_f_optblksch(	dConv * (sParams->dForward + x[0]),
									dConv * (sParams->dStrikes[i] + x[0]),
									x[1] + x[2],
									sParams->dMaturity,
									1.0,
									SRT_CALL,
		...
		if (comm->flag == 2)
		{
			...
			d1 = log((sParams->dForward + x[0]) / (sParams->dStrikes[i] + x[0])) / stdev + 0.5 * stdev;
			d2 = d1 - stdev;

			dDelta1 = norm(d1) - norm(d2);
			dVega1 = dConv * (sParams->dForward + x[0]) * gauss(d1) * sParams->dSqMaturity;
			...
			dVega2 = dConv * (sParams->dForward + x[0]) * gauss(d1) * sParams->dSqMaturity;

			g[0] += dConv * (sParams->dProba1 * dDelta1 + sParams->dProba2 * dDelta2) * dErrorTemp;
			g[1] += (sParams->dProba1 * dVega1 + sParams->dProba2 * dVega2) * dErrorTemp;
			g[2] += (sParams->dProba1 * dVega1 - sParams->dProba2 * dVega2) * dErrorTemp;
		}
	}
...
static void NAG_CALL MixedSL_Calib_LeastSquare(	Integer		m,
												Integer		n,
												double		x[],
												double		fvec[],
												double		fjac[],
...
Err				err = NULL;
	...
	for (i=0; i<sParams->iNbStrikes; i++)
	{
		...
		if (!comm->flag == 0)
		{
			...
			fjac[tdj*i] = dConv * (sParams->dProba1 * dDelta1 + sParams->dProba2 * dDelta2);
...
Err	Calib_MixedSL_ToSmile(	CALIBSL_PARAMS	sParams,
							double			*dShift,
							double			*dVolDown,
							double			*dVolUp,
...
static	NagError		fail;
	...
	*/
	...
	if (0)
	{
		nag_opt_bounds_deriv (	3, 
								MixedSL_Calib_Fonctionnal,
								bound,
								dLBound,
								dHBound,
								dInputs,
...
void OTCutilsSaveMatrix(	int		iNRows,
							int		iNCols,
							double	**Matrix)
{	
int  i, j;
...
void OTCutilsSaveDouble(	char* fileName, 
							double	toSave)
{	
FILE *fid;
char tempString[256];
...
Err Srt_smooth_matrix(int		method,
					  int		num_adj, 
					  int		NRow, 
					  int		NCol, 
					  double	**in_matrix, 
					  double	**out_matrix)
{
int		i, j, ii, jj, posLeft, posRight, posAbove, posBelow, num_points, num_max_points;
double	coef;
Err err = NULL;
	...
	for(i=0; i<NRow; i++)
	{
		for(j=0; j<NCol; j++)
		{
			...
			if(posRight>NCol-1) posRight = NCol-1;
			if(posAbove<0) posAbove = 0;
			if(posBelow>NRow-1) posBelow = NRow-1;
			...
			for(ii=posAbove; ii<=posBelow; ii++)
			{
				for(jj=posLeft; jj<=posRight; jj++)
				{
					...
					{
						if(ii==i && jj==j)
							coef = (double)(1+num_max_points-num_points)/(double)(num_max_points);
						else

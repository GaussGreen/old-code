/* ==========================================================
        ...
        PURPOSE:			Calculates One Time Callables using a smile on the Fx by a
   copula based method
...
void cpd_init_otc_params(
    ...
    /* otc */
    int fwdVolMethod, /*	0=Sliding 3F vol; 1=Converging 3F vol; 2=Sliding Cvg Sbeta; 3=Cvg
                         Cvg Sbeta */
    ...
    int LongShort)
{
    ...
    else
    {
        ...
        {
            if (i < keepFirstN || (i >= keepFirstN && (double)(i - keepFirstN + 1) / oneOutOfN ==
...
void cpd_free_otc_precalc(otcpd_precalc* otc_precalc, int nCall, int nCoupons_pd, int nCoupons_fund)
{
    if (otc_precalc->df_const_dom_fx_val)
        free_dmatrix(otc_precalc->df_const_dom_fx_val, 0, nCall - 1, 0, nCoupons_pd + 2);
    if (otc_precalc->df_lin_dom_fx_val)
        free_dmatrix(otc_precalc->df_lin_dom_fx_val, 0, nCall - 1, 0, nCoupons_pd + 2);
    if (otc_precalc->df_const_for_fx_val)
        free_dmatrix(otc_precalc->df_const_for_fx_val, 0, nCall - 1, 0, nCoupons_pd + 2);
    if (otc_precalc->df_lin_for_fx_val)
        free_dmatrix(otc_precalc->df_lin_for_fx_val, 0, nCall - 1, 0, nCoupons_pd + 2);
    if (otc_precalc->df_const_pd_pay)
        free_dmatrix(otc_precalc->df_const_pd_pay, 0, nCall - 1, 0, nCoupons_pd + 2);
    ...
    if (otc_precalc->df_const_fund_pay)
        free_dmatrix(otc_precalc->df_const_fund_pay, 0, nCall - 1, 0, nCoupons_fund + 2);
    if (otc_precalc->df_lin_fund_pay)
        free_dmatrix(otc_precalc->df_lin_fund_pay, 0, nCall - 1, 0, nCoupons_fund + 2);
...
Err cpd_alloc_otc_precalc(
    otcpd_precalc* otc_precalc, int nCall, int nCoupons_pd, int nCoupons_fund, int nMktDates)
{
    Err err                          = NULL;
    ...
    if (!otc_precalc->pd_fwd3F_vol || !otc_precalc->pd_fwdsmile_vol ||
        ...
        !otc_precalc->pd_fwdsmile_rho || !otc_precalc->pd_fwdsmile_pi || !otc_precalc->FxAdj ||
        !otc_precalc->FeeFxAdj || !otc_precalc->KO_barrier)
    {
    ...
    double*          corFFx)
{
    ...
    for (i = 0; i < cpd->num_calls; i++)
    {
        ...
        beta = (1.0 - exp(-fund_lda * (cpd->fund_leg->cpn[cpd->call[i].fund_idx].start_time -
                                       cpd->call[i].ex_time))) /
               fund_lda;
        ...
        {
            beta = (1.0 -
                    exp(-und->lda_dom * (cpd->pd_leg->cpn[j].fx_val_time - cpd->call[i].ex_time))) /
                   und->lda_dom;
            ...
            {
                beta = (1.0 - exp(-und->lda_dom *
                                  (cpd->pd_leg->cpn[j].pay_time - cpd->call[i].ex_time))) /
                       und->lda_dom;
            ...
            beta = (1.0 -
                    exp(-und->lda_for * (cpd->pd_leg->cpn[j].fx_val_time - cpd->call[i].ex_time))) /
                   und->lda_for;
        ...
        {
            beta =
                (1.0 - exp(-fund_lda * (cpd->fund_leg->cpn[j].pay_time - cpd->call[i].ex_time))) /
                fund_lda;
        ...
        beta =
            (1.0 - exp(-und->lda_dom * (cpd->pd_leg->not_ref.fx_val_time - cpd->call[i].ex_time))) /
            und->lda_dom;
        ...
        {
            beta = (1.0 -
                    exp(-und->lda_dom * (cpd->pd_leg->not_ref.pay_time - cpd->call[i].ex_time))) /
                   und->lda_dom;
        ...
        beta =
            (1.0 - exp(-und->lda_for * (cpd->pd_leg->not_ref.fx_val_time - cpd->call[i].ex_time))) /
            und->lda_for;
        ...
        for (j = 0; j < 2; j++)
        {
            ...
            {
                beta = (1.0 - exp(-und->lda_dom *
                                  (cpd->call[i + j].fx_val_time - cpd->call[i].ex_time))) /
                       und->lda_dom;
                ...
                beta = (1.0 - exp(-fund_lda * (cpd->call[i + j].set_time - cpd->call[i].ex_time))) /
                       fund_lda;
    ...
    forwardVec[cpd->pd_leg->num_cpn] =
        und->spot_fx * swp_f_df(und->today, cpd->pd_leg->not_ref.fx_val_date, und->for_yc) /
    ...
    for (j = 0; j < cpd->num_calls; j++)
    {
        for (i = 0; i < 2; i++)
        {
            if (j + i < cpd->num_calls)
            {
                forwardVec[cpd->pd_leg->num_cpn + 1 + i] =
                    und->spot_fx * swp_f_df(und->today, cpd->call[j + i].fx_val_date, und->for_yc) /
        ...
        if (OTC_params->OTCpayoff == 6)
        {
            ...
            err = OTCgetFwdSABR(
                ...
                OTC_params->FSMbeta,
                OTC_params->FSMrho,
                4); /* 0=Sliding 3F vol; 1=Converging 3F vol; 2=Sliding Cvg Sbeta; 3=Cvg Cvg Sbeta;
                       4=3F*/
        ...
        err = OTCgetFwdSABR(
            ...
            OTC_params->FSMbeta,
            OTC_params->FSMrho,
            OTC_params->FwdVolMethod); /* 0=Sliding 3F vol; 1=Converging 3F vol; 2=Sliding Cvg
                                          Sbeta; 3=Cvg Cvg Sbeta; 4=3F*/
...
/* =========================================================================
 Calibration of the sum of shifted lognormal parameters to SABR to obtain a copula based estimation
of OTC
========================================================================= */
Err CalibSSLtoSABR(
    ...
    int nIter)
{
    ...
    double *dStrikes = NULL, *dWeights = NULL, *dParams = NULL, *dMarketprices = NULL;
    ...
    srt_f_optsarbvol(fwd, k1, t, sigma, alpha, beta, rho, SRT_LOGNORMAL, SRT_LOGNORMAL, &v1);

    srt_f_optsarbvol(fwd, k2, t, sigma, alpha, beta, rho, SRT_LOGNORMAL, SRT_LOGNORMAL, &v2);
    dStrikes[1] = k1;
    dStrikes[2] = fwd;
    dStrikes[3] = k2;
...
Err SSLGradient(double strike, double* ParamSSL, double* price, double* gradient, int nbParamsSSL)
{
    Err    err = NULL;
    ...
    if (err)
        goto FREE_RETURN;
    err =
        SSLprice(forward, maturity, strike, volUp, volDown, shift + bumpSL, 1, &shiftedpriceshift);
    if (err)
        goto FREE_RETURN;
    err = SSLprice(
        forward, maturity, strike, volUp * exp(bumpVolup), volDown, shift, 1, &shiftedpricevolup);
    if (err)
        goto FREE_RETURN;
    err = SSLprice(
        forward,
    ...
    double** Cumulative)
{
    ...
    for (i = 1; i < Npoints; i++)
    {
        if (FwdShift < 0.0 && fabs(FwdShift) > Forward)
            firstGuessGrid[i] =
                -FwdShift + (Forward + FwdShift) * exp(-Nstd * (2.0 * (double)i / Npoints - 1.0) *
                                                       volATM * sqrt(Maturity));
        else
            firstGuessGrid[i] =
                (Forward + FwdShift) *
                    exp(Nstd * (2.0 * (double)i / Npoints - 1.0) * volATM * sqrt(Maturity)) -
                FwdShift;

        err = SSLcumul(
            Forward, Maturity, firstGuessGrid[i], VolUp, VolDown, FwdShift, &(firstGuessCumul[i]));
        if (err)
            goto FREE_RETURN;
    }
    ...
    for (i = 0; i < Npoints; i++)
    {
        ...
        Cumulative[0][i] =
            interp(firstGuessCumul, firstGuessGrid, Npoints, Cumulative[0][i], 0, &temp);
    }

    for (i = 0; i < Npoints; i++)
    {
        err = SSLcumul(
            Forward, Maturity, Cumulative[0][i], VolUp, VolDown, FwdShift, &(Cumulative[1][i]));
        if (err)
            goto FREE_RETURN;
    }
    ...
    double** Cumulative)
{
    ...
    err = BMM_Option_Price_From_States(
        Forward, Maturity, BMbeta, BMfwd1, BMfwd2, BMsig1, BMsig2, BMpi, SRT_PUT, &price);
    ...
    double* Cumul)
{
    ...
    d2Down = (log((Forward + Shift) / (Strike + Shift)) - 0.5 * VolDown * VolDown * Maturity) /
             stdevDown;
    ...
    double* Price)
{
    ...
    if (Shift < 0.0 && fabs(Shift) > Forward)
    {
        *Price = srt_f_optblksch(
            -Forward - Shift,
            -Strike - Shift,
            fabs(VolUp),
            Maturity,
            1.0,
            IsCall ? SRT_PUT : SRT_CALL,
            PREMIUM);
        *Price += srt_f_optblksch(
            -Forward - Shift,
            -Strike - Shift,
...
Err OTC_balsam_generation(
    long     nbPaths, /* Must be odd */
    long     nbSteps,
    double** matrix,
    long*    seed)
{
    ...
    double* df)
{
    ...
    else
    {
        ...
        Phi +=
            vol[endIndex] * vol[endIndex] * Phi_Func(2 * lambda, Tstart, volTimes[i - 1], Tstart);
    }
...
Err OTCdfPhi(
    double lambda, double Tstart, double* volTimes, double* vol, int nVolTimes, double* Phi)
{
    int i, endIndex;
    Err err = NULL;
...
/* 0=Sliding 3F vol; 1=Converging 3F vol; 2=Sliding Cvg Sbeta; 3=Cvg Cvg Sbeta; 4=3F; 5=FwdSmile */
{
    ...
    for (i = 0; i < 2; i++)
    {
        ...
        {
            FxNotValDate[i] =
                add_unit(cpd->call[otc + i].ex_date, 2, SRT_BDAY, MODIFIED_SUCCEEDING);
    ...
    else
    {
        ...
        for (i = 0; i < 2; i++)
        {
            ...
            {
                err = OTCfwdSABR(
                    forward ? forward[noptions + i] : 0.0,
                    cpd->call[otc].ex_time,
                    cpd->call[otc + i].ex_time,
                    FxNotValTime[i],
                    smile_mkt,
    ...
    int     smileFee)
{
    ...
    if (smileFee)
    {
        ...
        else
        {
            ...
            *smileAlpha =
                interpCoef * fx_mkt_smile_alpha[j + 1] + (1.0 - interpCoef) * fx_mkt_smile_alpha[j];
            *smileBeta =
                interpCoef * fx_mkt_smile_beta[j + 1] + (1.0 - interpCoef) * fx_mkt_smile_beta[j];
            *smileRho =
                interpCoef * fx_mkt_smile_rho[j + 1] + (1.0 - interpCoef) * fx_mkt_smile_rho[j];
        }
    }
    else
    {
...
*/
Err OTCcalibSmileModel(
    double           Tex,     /* Maturity in years */
...
Err OTCgetCumulative(double forward, double Tex, otcpd_params* OTC_params)
{
    ...
    else if (OTC_params->smileModel == 1 || OTC_params->smileModel == 2)
    {
        ...
        err = OTCmatrixRealloc(
            &(OTC_params->CUMUL),
            0,
            2 - 1,
            0,
            OTC_params->CUMULnPoints - 1,
            0,
            2 - 1,
            0,
            lNbPoints - 1);
...
Err OTCgetChangedPayoff(CPD_STR cpd, CPD_UND und, double* NewCoupons)
{
    ...
    else
    {
        for (k = 0; k < cpd->num_calls; k++)
        {
            ...
            NewCoupons[pdIndex] /=
                swp_f_df(und->today, cpd->pd_leg->cpn[pdIndex].fx_val_date, und->for_yc);
        }
    }
    return err;
}
            ...
            if (!k)
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        midDate =
                            (long)((cpd->pd_leg->cpn[j - 1].pay_date + cpd->pd_leg->cpn[j].pay_date) / 2.0);
    ...
    double** Results)
{
    ...
    for (i = pd_otc_start; i < cpd->pd_leg->num_cpn; i++)
    {
        ...
        Results[i][sUp] =
            forward[i] * exp(fwdSmileVisuNstd * vol * sqrt(cpd->pd_leg->cpn[i].fx_val_time));
    ...
    for (realisation = 0; realisation < nSimul; realisation++)
    {
        for (i = pd_otc_start; i < cpd->pd_leg->num_cpn; i++)
        {
            ...
            if (localForward < 0.0 && type >= 3)
            {
                Results[i][vUp] +=
                    dfDom * OPT_VAL_MACRO(
                                type - 2, localForward, Results[i][sUp], smile_std, smile_half_std);
            }
            else
            {
                Results[i][vUp] +=
                    dfDom *
                    OPT_VAL_MACRO(type, localForward, Results[i][sUp], smile_std, smile_half_std);
            }
            ...
            if (localForward < 0.0 && type >= 3)
            {
                Results[i][vATM] +=
                    dfDom *
                    OPT_VAL_MACRO(
                        type - 2, localForward, Results[i][sATM], smile_std, smile_half_std);
            }
            else
            {
                Results[i][vATM] +=
                    dfDom *
                    OPT_VAL_MACRO(type, localForward, Results[i][sATM], smile_std, smile_half_std);
            }
            ...
            if (localForward < 0.0 && type >= 3)
            {
                Results[i][vDown] +=
                    dfDom *
                    OPT_VAL_MACRO(
                        type - 2, localForward, Results[i][sDown], smile_std, smile_half_std);
            }
            else
            {
                Results[i][vDown] +=
                    dfDom *
                    OPT_VAL_MACRO(type, localForward, Results[i][sDown], smile_std, smile_half_std);
            }
        }
    }
    ...
    double**         Results)
{
    ...
    double discount, df, dfDom, dfFor, dfFund, dfFirstFunding, dfNotExchgePd, dfNotExchgeShortPd,
    ...
    if (!fund_leg || !pd_leg || !fund_legShort || !pd_legShort || !fundDf || !PdDfDom ||
        !FwdDfDom || !FwdDfFor || !strikeCap || !strikeFloor || !PDdfReal || !PDFxDomdfReal ||
        !PDFxFordfReal)
    {
        err = "OTCcaller: memory allocation error";
        goto FREE_RETURN;
    ...
    dfFirstFunding =
        swp_f_df(und->today, cpd->fund_leg->cpn[fund_otc_start].start_date, fund_yc) / dfFund;
    ...
    {
        dfNotExchgeShortPd =
            swp_f_df(und->today, cpd->call[OTC_params->OTC + 1].set_date, und->dom_yc) / dfDom;
        dfNotExchgeShortFund =
            swp_f_df(und->today, cpd->call[OTC_params->OTC + 1].set_date, fund_yc) / dfFund;
    }
    ...
    for (i = pd_otc_start; i < cpd->pd_leg->num_cpn; i++)
    {
        ...
        {
            strikeCap[i] =
                (cpd->pd_leg->cpn[i].cap - cpd->pd_leg->cpn[i].alpha) / cpd->pd_leg->cpn[i].beta;
            strikeFloor[i] =
                (cpd->pd_leg->cpn[i].floor - cpd->pd_leg->cpn[i].alpha) / cpd->pd_leg->cpn[i].beta;
        }
    }
    ...
    FwdDfDom[cpd->pd_leg->num_cpn] =
        swp_f_df(und->today, cpd->pd_leg->not_ref.fx_val_date, und->dom_yc) / dfDom;
    FwdDfFor[cpd->pd_leg->num_cpn] =
        swp_f_df(und->today, cpd->pd_leg->not_ref.fx_val_date, und->for_yc) / dfFor;
    if (fabs(cpd->pd_leg->not_ref.beta) > 1.0e-16)
    {
        strikeCap[cpd->pd_leg->num_cpn] =
            (cpd->pd_leg->not_ref.cap - cpd->pd_leg->not_ref.alpha) / cpd->pd_leg->not_ref.beta;
        strikeFloor[cpd->pd_leg->num_cpn] =
            (cpd->pd_leg->not_ref.floor - cpd->pd_leg->not_ref.alpha) / cpd->pd_leg->not_ref.beta;
    }
    ...
    fund_fee -= swp_f_df(und->today, cpd->call[OTC_params->OTC].set_date, fund_yc) *
    ...
    if (OTC_params->OTC < cpd->num_calls - 1)
    {
        ...
        fund_feeShort += swp_f_df(und->today, cpd->call[OTC_params->OTC + 1].set_date, fund_yc) *
                         cpd->fund_leg->notional;
    }
    ...
    for (i = 0; i < 2; i++)
    {
        if (OTC_params->OTC + i < cpd->num_calls)
        {
            ...
            if (cpd->call[OTC_params->OTC + i].use_opt_str)
            {
                ...
                if (Calc_flat)  // As sigma is log, only the ABR params have to be set to the log
                                // case.
                {
                    err = OTCgetSMILEparams(
                        cpd,
                ...
                for (str_idx = 0; str_idx < cpd->call[OTC_params->OTC + i].nstrikes; str_idx++)
                {
                    ...
                    {
                        if (smile_params->sigma * sqrt(cpd->call[OTC_params->OTC + i].fx_fix_time) >
                                1e-16 &&
                        ...
                        if (Calc_flat)
                        {
                            ...
                            opt_string_flat +=
                                cpd->call[OTC_params->OTC + i].weights[str_idx] * OptionValue_flat;
                        }
                    }
                }
    ...
    else
    {
        ...
        if (Calc_flat)
            pd_fee_flat += dfDom * (cpn->wcst + cpn->wspot * forward + opt_string_flat);
    }
    ...
    fund_fee += swp_f_df(und->today, cpd->fund_leg->cpn[fund_otc_start].start_date, fund_yc) *
                cpd->fund_leg->notional;

    if (OTC_params->OTC < cpd->num_calls - 1)
        fund_feeShort +=
            swp_f_df(und->today, cpd->fund_leg->cpn[fund_otc_start].start_date, fund_yc) *
            cpd->fund_leg->notional;
        ...
        if (!cpn->use_opt_str)
        {
            ...
            if (OTC_params->OTC < cpd->num_calls - 1)
            {
                if (i >= pd_otc_start && i < pd_otc_short_end)
                {
                    ...
                    if (Calc_flat)
                        pd_feeShort_flat += dfDom * (cpn->alpha + cpn->beta * forward +
                                                     fabs(cpn->beta) * (floor_flat - cap_flat));
                }
            }
        }
    ...
    if (OTC_params->OTCcalculate[OTC_params->OTC])
    {
        ...
        for (realisation = 0; realisation < OTC_params->COPULAnSimul; realisation++)
        {
            ...
            for (i = pd_otc_start; i < cpd->pd_leg->num_cpn; i++)
            {
                ...
                df =
                    exp(OTC_precalc->df_const_pd_pay[OTC_params->OTC][i] +
                        OTC_precalc->df_lin_pd_pay[OTC_params->OTC][i] * matrix[realisation][0]);
                ...
                {
                    df =
                        exp(OTC_precalc->df_const_dom_fx_val[OTC_params->OTC][i] +
                ...
                df = exp(
                    OTC_precalc->df_const_for_fx_val[OTC_params->OTC][i] +
                    OTC_precalc->df_lin_for_fx_val[OTC_params->OTC][i] * matrix[realisation][1]);
            ...
            for (i = 0; i < 2; i++)
            {
                if (OTC_params->OTC + i < cpd->num_calls)
                {
                    df = exp(
                        OTC_precalc
                            ->df_const_pd_pay[OTC_params->OTC][cpd->pd_leg->num_cpn + i + 1] +
                        OTC_precalc->df_lin_pd_pay[OTC_params->OTC][cpd->pd_leg->num_cpn + i + 1] *
                            matrix[realisation][0]);
                    ...
                    smile_params_flat->sigma =
                        OTC_precalc->pd_fwd3F_vol[OTC_params->OTC][cpd->pd_leg->num_cpn + i + 1];
                    ...
                    if (cpd->call[OTC_params->OTC + i].use_opt_str)
                    {
                        ...
                        df =
                            exp(OTC_precalc->df_const_dom_fx_val[OTC_params->OTC]
                                                                [cpd->pd_leg->num_cpn + i + 1] +
                        ...
                        df =
                            exp(OTC_precalc->df_const_for_fx_val[OTC_params->OTC]
                                                                [cpd->pd_leg->num_cpn + i + 1] +
            ...
            /*	Discount */
            df =
                exp(OTC_precalc->df_const_pd_pay[OTC_params->OTC][cpd->pd_leg->num_cpn] +
            ...
            if ((fabs(cpn->beta) > 1.0e-16 && !cpn->use_opt_str) || cpn->use_opt_str)
            {
                ...
                ================================== */
                df =
                    exp(OTC_precalc->df_const_dom_fx_val[OTC_params->OTC][cpd->pd_leg->num_cpn] +
                        OTC_precalc->df_lin_dom_fx_val[OTC_params->OTC][cpd->pd_leg->num_cpn] *
                            matrix[realisation][0]);

                dfDom = FwdDfDom[cpd->pd_leg->num_cpn] * df;

                df =
                    exp(OTC_precalc->df_const_for_fx_val[OTC_params->OTC][cpd->pd_leg->num_cpn] +
                        OTC_precalc->df_lin_for_fx_val[OTC_params->OTC][cpd->pd_leg->num_cpn] *
                            matrix[realisation][1]);

                dfFor = FwdDfFor[cpd->pd_leg->num_cpn] * df;

                forward = OTC_precalc->NotFxAdj[OTC_params->OTC][2] * matrix[realisation][2] *
                          dfFor / dfDom;
                if (Calc_flat)
            ...
            if (!cpn->use_opt_str)
            {
                ...
                /*	Coupon pv */
                pd_leg[realisation] +=
                    discount * (cpn->alpha + cpn->beta * forward + fabs(cpn->beta) * (floor - cap));
                if (Calc_flat)
                    pd_leg_flat[realisation] +=
            ...
            else
            {
                ...
                for (str_idx = 0; str_idx < cpn->nstrikes; str_idx++)
                {
                    if (fabs(cpn->weights[str_idx]) > 1e-16 * pd_not)
                    {
                        if (OTC_precalc->pd_fwdsmile_vol[OTC_params->OTC][cpd->pd_leg->num_cpn] *
                                    sqrt(cpn->fx_fix_time - cpd->call[OTC_params->OTC].ex_time) >
                                1e-16 &&
                            cpn->strikes[str_idx] > 1e-16)
                            type = 3;
                        else
                ...
                if (Calc_flat)
                    pd_leg_flat[realisation] +=
                        discount * (cpn->wcst + cpn->wspot * forward_flat + opt_string_flat);
            }
            ...
            if (Calc_flat)
            {
                ...
                else
                {
                    ...
                    {
                        fund_leg_flat[realisation] =
                            fund_leg[realisation] * matrix[realisation][3] / matrix[realisation][2];
                        fund_legShort_flat[realisation] = fund_legShort[realisation] *
            ...
            for (i = pd_otc_start; i < cpd->pd_leg->num_cpn; i++)
            {
                ...
                ================================== */
                forward = OTC_precalc->FxAdj[OTC_params->OTC][i] * matrix[realisation][2] *
                          PDFxFordfReal[i] / PDFxDomdfReal[i];
                if (Calc_flat)
                    forward_flat = OTC_precalc->FxAdj[OTC_params->OTC][i] * matrix[realisation][3] *
                                   PDFxFordfReal[i] / PDFxDomdfReal[i];
                ...
                err = cpd_vol_get_smile_params_from_otc(
                    smile_mkt->smile_spec_type + 1, OTC_params->OTC, i, OTC_precalc, smile_params);
                if (err)
                    goto FREE_RETURN;
                ...
                if (!cpn->use_opt_str)
                {
                    ...
                    pd_leg[realisation] += PDdfReal[i] * (cpn->alpha + cpn->beta * forward +
                                                          fabs(cpn->beta) * (floor - cap));
                    if (Calc_flat)
                        pd_leg_flat[realisation] +=
                ...
                else
                {
                    ...
                    if (Calc_flat)
                        pd_leg_flat[realisation] +=
                            PDdfReal[i] * (cpn->wcst + cpn->wspot * forward_flat + opt_string_flat);
                    if (OTC_params->OTC < cpd->num_calls - 1)
                    {
                        if (i >= pd_otc_start && i < pd_otc_short_end)
                        {
        ...
        {
            /* Smile */
            callRealisation = Pay_Rec_Mult *
                              ((pd_leg[realisation] + pd_fee) - (fund_leg[realisation] + fund_fee));
            ...
            callRealisation =
                callRealisation - Pay_Rec_Mult * (((pd_legShort[realisation] + pd_feeShort) -
            ...
            {
                /* 3F Smile */
                callRealisation = Pay_Rec_Mult * ((pd_leg_flat[realisation] + pd_fee_flat) -
                ...
                callRealisation =
                    callRealisation -
                    Pay_Rec_Mult * (((pd_legShort_flat[realisation] + pd_feeShort_flat) -
    ...
    double*        fund_legShort)
{
    ...
    *fund_leg = -dfNotExchge * df * cpd->fund_leg->notional;
    ...
    if (otc < cpd->num_calls - 1)
    {
        *fund_legShort = *fund_leg;
        ...
        /*	Final Notional For Short option*/
        df =
            exp(OTC_precalc->df_const_fund_pay[otc][cpd->fund_leg->num_cpn + 2] +
                OTC_precalc->df_lin_fund_pay[otc][cpd->fund_leg->num_cpn + 2] * real);
    ...
    *fund_leg += dfFirstFunding * df * cpd->fund_leg->notional;
    ...
    for (i = fund_otc_start; i < cpd->fund_leg->num_cpn; i++)
    {
        ...
        ================================== */
        df = exp(
            OTC_precalc->df_const_fund_pay[otc][i] + OTC_precalc->df_lin_fund_pay[otc][i] * real);
    ...
    double** Results)
{
    ...
    double *       fundDf = NULL, *PdDfDom = NULL, *FwdDfDom = NULL, *FwdDfFor = NULL;
    double         discount, df, dfDom, dfFor, dfFirstFunding, dfNotExchge, dfFinalNotExchge, df0T,
        df0Tstar;
    ...
    if (!fund_leg || !pd_leg || !fundDf || !PdDfDom || !FwdDfDom || !FwdDfFor || !strikeCap ||
        !strikeFloor)
    {
        err = "OTCcaller: memory allocation error";
        goto FREE_RETURN;
    ...
    ============================= */
    /* Tests */
    if (firstIndex < otc || secondIndex < otc || firstIndex > cpd->num_calls - 1 ||
        secondIndex > cpd->num_calls - 1)
    {
    ...
    for (ii = 0; ii < 2; ii++)
    {
        ...
        dfFirstFunding =
            swp_f_df(und->today, cpd->fund_leg->cpn[fund_otc_start].start_date, und->dom_yc) /
            dfDom;
        dfNotExchge = swp_f_df(und->today, cpd->call[Index].set_date, und->dom_yc) / dfDom;

        if (Index < cpd->num_calls - 1)
            dfFinalNotExchge =
                swp_f_df(und->today, cpd->call[Index + 1].set_date, und->dom_yc) / dfDom;
        else
            dfFinalNotExchge =
                swp_f_df(und->today, cpd->pd_leg->cpn[pd_otc_end - 1].pay_date, und->dom_yc) /
                dfDom;
        ...
        for (i = pd_otc_start; i < pd_otc_end; i++)
        {
            PdDfDom[i] = swp_f_df(und->today, cpd->pd_leg->cpn[i].pay_date, und->dom_yc) / dfDom;
            FwdDfDom[i] =
                swp_f_df(und->today, cpd->pd_leg->cpn[i].fx_val_date, und->dom_yc) / dfDom;
            FwdDfFor[i] =
                swp_f_df(und->today, cpd->pd_leg->cpn[i].fx_val_date, und->for_yc) / dfFor;
            if (fabs(cpd->pd_leg->cpn[i].beta) > 1.0e-16)
            {
        ...
        for (realisation = 0; realisation < nSimul; realisation++)
        {
            ...
            if (pd_otc_end == cpd->pd_leg->num_cpn)
            {
                ...
                if (fabs(cpn->beta) > 1.0e-16)
                {
                    ...
                    cap = OPT_VAL_MACRO(
                        type, forward, strikeCap[cpd->pd_leg->num_cpn], smile_std, smile_half_std);
                }
                else
                {
            ...
            for (i = pd_otc_start; i < pd_otc_end; i++)
            {
                ...
                floor = OPT_VAL_MACRO(type, forward, strikeFloor[i], smile_std, smile_half_std);
                /* ==================================
                Cap
                ================================== */
                if (cpn->capped && fabs(cpn->beta) > 1.0e-16 && cpn->cap > -1.0e-16)
                ...
                cap = OPT_VAL_MACRO(type, forward, strikeCap[i], smile_std, smile_half_std);

                pd_leg[realisation] +=
                    dfDom * (cpn->alpha + cpn->beta * forward + fabs(cpn->beta) * (floor - cap));
            }
    ...
    for (realisation = 0; realisation < nSimul; realisation++)
    {
        ...
        Vol1 += (fund_leg1[realisation] - pd_leg1[realisation]) *
                (fund_leg1[realisation] - pd_leg1[realisation]) / dfTstarVec[realisation];
        Vol2 += (fund_leg[realisation] - pd_leg[realisation]) *
                (fund_leg[realisation] - pd_leg[realisation]) / dfTstarVec[realisation];
    ...
    double*  Results)
{
    ...
    Value Fx Options for the 3 strikes
    ================================== */
    err = OTCdfPhi(lda_dom, forwardFixTime, sigma_time_rates, sigma_dom, sigma_n_rates, &phiDom);

    if (err)
        goto FREE_RETURN;

    err = OTCdfPhi(lda_for, forwardFixTime, sigma_time_rates, sigma_for, sigma_n_rates, &phiFor);
    ...
    for (realisation = 0; realisation < nSimul; realisation++)
    {
        ...
        ================================== */
        err =
            OTCdf(matrix[realisation][0], lda_dom, forwardFixTime, valTime, tstarTime, phiDom, &df);
        ...
        err =
            OTCdf(matrix[realisation][1], lda_for, forwardFixTime, valTime, tstarTime, phiFor, &df);
        ...
        for (j = 0; j < nStrikes; j++)
        {
            ...
            if (localForward < 0.0 && type >= 3)
            {
                Results[j] +=
                    dfDom *
                    OPT_VAL_MACRO(type - 2, localForward, Strikes[j], smile_std, smile_half_std);
            }
            else
            {
                Results[j] +=
                    dfDom *
                    OPT_VAL_MACRO(type, localForward, Strikes[j], smile_std, smile_half_std);
            }
        }
    }
...
====================================================
==================================================== */
Err KO_get_coupon_paid(int Coupon_idx, CPD_STR cpd, double fx, double* Coupon_value)
{
    int        str_idx;
    ...
    double***        save_values)
{
    ...
    double *   fundDf = NULL, *PdDfDom = NULL, *FwdDfDom = NULL, *FwdDfFor = NULL;
    double discount, df, dfDom, dfFor, dfFund, dfFirstFunding, dfNotExchgePd, dfNotExchgeShortPd,
        dfNotExchgeFund, dfNotExchgeShortFund, dfStartNotExchgePd, dfStartNotExchgeFund,
    ...
    /* =============================
    Populates a vector with 0-1 => 0 : Pay Coupons; 1 : Exchange Notionals; 2 : allready KO in the
    past
    ============================= */
    ...
    if (!cpd->call[0].TARN_Do)
    {
        ...
        for (realisation = 0; realisation < OTC_params->COPULAnSimul; realisation++)
        {
            ...
            if (!KnockedOut[realisation])
            {
                ...
                {
                    if (!OTC_params->KO_do_optim ||
                        (OTC_params->KO_do_optim && cpd->call[OTC_params->OTC].call_type))
                    {
    ...
    if (!fund_leg || !pd_leg || !fund_legShort || !pd_legShort || !fundDf || !PdDfDom ||
        !FwdDfDom || !FwdDfFor || !strikeCap || !strikeFloor)
    {
    ...
    for (i = 0; i < 2; i++)
    {
        if (OTC_params->OTC < cpd->num_calls - 1 || !i)
        {
            if (cpd->call[OTC_params->OTC + i].ex_date > start_date)  //&& OTC_params->OTC == 0)
            {
                exchange_fund =
                    fund_mult *
                    swp_f_df(und->today, cpd->call[OTC_params->OTC + i].set_date, fund_yc) *
                    cpd->fund_leg->notional;
            ...
            if (cpd->call[OTC_params->OTC + i].use_opt_str)
            {
                ...
                err = OTCgetSMILEparams(
                    cpd,
                    und,
                    cpd->call[OTC_params->OTC + i].ex_time,
                    cpd->call[OTC_params->OTC + i].fx_val_time,
                    cpd->call[OTC_params->OTC + i].fx_val_date,
                    smile_mkt,
                    smile_params,
                ...
                if (cpd->call[OTC_params->OTC + i].ex_date > start_date)  //&& OTC_params->OTC == 0)
                {
                    ...
                    cpd->call[OTC_params->OTC + i].ex_date <= start_date && OTC_params->OTC == 0)
    ...
    smile_params->Use_BetaQuick_in_MC = 1;
    for (realisation = 0;
         realisation < OTC_params->COPULAnSimul &&
         (OTC_params->KO_do_optim || fmc_current_std > OTC_params->KO_FMC_precision);
         realisation++)
    {
        coupon_pd      = 0.0;
        coupon_fund    = 0.0;
        ...
        for (i = pd_otc_start; i < pd_otc_short_end; i++)
        {
            ...
            ================================== */
            df =
                exp(OTC_precalc->df_const_for_fx_val[OTC_params->OTC][i] +
            ...
            forward = OTC_precalc->FxAdj[OTC_params->OTC][i] *
                      OTC_params->KO_FxMatrix[realisation][OTC_params->OTC] * dfFor / dfDom;
        ...
        if (start_date > cpd->call[OTC_params->OTC].ex_date)
        {
            ...
            err = OTCdf(
                cpd->fund_leg->dom_for ? OTC_params->KO_RatesMatrix[realisation][1]
                                       : OTC_params->KO_RatesMatrix[realisation][0],
        ...
        if (cpd->fund_leg->dom_for)
            df =
                exp(OTC_precalc->df_const_fund_start[OTC_params->OTC] +
        ...
        for (i = fund_otc_start; i < fund_otc_short_end; i++)
        {
            ...
            if (cpd->fund_leg->dom_for)
                df =
                    exp(OTC_precalc->df_const_fund_pay[OTC_params->OTC][i] +
        ...
        for (i = 0; i < 2; i++)
        {
            if (OTC_params->OTC < cpd->num_calls - 1 || !i)
            {
                ...
                if (cpd->call[OTC_params->OTC + i].ex_date > start_date)
                {
                    ...
                    if (cpd->fund_leg->dom_for)
                        df =
                            exp(OTC_precalc->df_const_fund_pay[OTC_params->OTC]
                ...
                // Pd
                df = exp(
                    OTC_precalc->df_const_pd_pay[OTC_params->OTC][cpd->pd_leg->num_cpn + 1 + i] +
                    OTC_precalc->df_lin_pd_pay[OTC_params->OTC][cpd->pd_leg->num_cpn + 1 + i] *
                        OTC_params->KO_RatesMatrix[realisation][0]);
                ...
                if (cpd->call[OTC_params->OTC + i].use_opt_str)
                {
                    ...
                    ================================== */
                    forward = OTC_precalc->NotFxAdj[OTC_params->OTC][i] *
                              OTC_params->KO_FxMatrix[realisation][OTC_params->OTC] * dfNotEx[i];

                    df = exp(
                        OTC_precalc
                            ->df_const_dom_fx_val[OTC_params->OTC][cpd->pd_leg->num_cpn + i + 1] +
                        OTC_precalc
                                ->df_lin_dom_fx_val[OTC_params->OTC][cpd->pd_leg->num_cpn + i + 1] *
                            OTC_params->KO_RatesMatrix[realisation][0]);
                    ...
                    df = exp(
                        OTC_precalc
                            ->df_const_for_fx_val[OTC_params->OTC][cpd->pd_leg->num_cpn + i + 1] +
                        OTC_precalc
                                ->df_lin_for_fx_val[OTC_params->OTC][cpd->pd_leg->num_cpn + i + 1] *
                            OTC_params->KO_RatesMatrix[realisation][1]);
                    ...
                    if (cpd->call[OTC_params->OTC + i].ex_date > start_date)
                    {
                        ...
                        cpd->call[OTC_params->OTC].ex_date <= start_date && OTC_params->OTC == 0)
                        ko_coupon_pd += exchange_pd;
                }
                else
                {
        ...
        if (!OTC_params->KO_do_optim)
        {
            if (KnockedOut[realisation] == 2)
            {
                ...
                {
                    fmc_current_std =
                        fmc_current_var - fmc_current_mean * fmc_current_mean / (realisation + 1);
            ...
            else if (KnockedOut[realisation] == 0)
            {
                ...
                if (OTC_params->KO_FMC)
                {
                    ...
                    {
                        fmc_current_std = fmc_current_var -
                                          fmc_current_mean * fmc_current_mean / (realisation + 1);
                        fmc_current_std /= (realisation + 1);
                        fmc_current_std = sqrt(fmc_current_std / (realisation + 1));
                        fmc_current_std /= pd_not;
                    }
    ...
    if (!OTC_params->KO_do_optim)
    {
        ...
        PriceStd -=
            (Payoff_fund[0] - Payoff_pd[0]) * (Payoff_fund[0] - Payoff_pd[0]) / fmc_nb_realisations;
        ...
        if (OTC_params->OTCsmileFee > -1)
            Results[2 + OTC_params->OTC][4] = (fund_fee_mc / fmc_nb_realisations - fund_fee / df) -
    ...
    double*          Value)
{
    ...
    if (IsOption)
    {
        ...
        if (type >= 3)
        {
                err = cpd_vol_get_vol(Forward,Maturity,Strike, smile_params, SRT_LOGNORMAL,
        &smile_std); if(err) goto FREE_RETURN;
    ...
    double** matrix)
{
    ...
    err = OTCcorrelateVariables(
        stdDom, stdFor, cumulDomForCor, cumulDomFxCor, cumulForFxCor, nSimul, gauss, matrix);
    ...
    {
        err = CalibSSLtoSABR(
            forwardFixTime, /* Maturity in years */
            forward,        /* Forward */
        ...
        ==================================*/
        err = SSLCumulative(
            forward, forwardFixTime, volUp, volDown, shift, nPoints, nStd, cumulative);
    ...
    err = GenerateRealisations(0.0, meanFor, cumulative, nSimul, nPoints, 1, 2, 0, matrix);
        ...
        else
        {
            ...
            *index1 = (int)(prob * (double)(Nprobas - 1));
            ...
            do
            {
                *index1 += dir;
                test = probas[*index1];
                if ((dir == 1 && test > prob) || (dir == -1 && test < prob) || *index1 <= 0 ||
                    *index1 >= Nprobas - 1)
                    stop = 1;
    ...
    for (i = 0; i < otc_params->COPULAnSimul; i++)
    {
        ...
        if (otc_params->CUMULlinear == 0)
        {
            if (test1 > otc_params->CUMUL[0][index1] && test1 < otc_params->CUMUL[0][index3])
                ...
                index1 == 0 && fabs(otc_params->KO_FxMatrix[i][otc_params->OTC] - test1) < 1.0e-10)
                otc_params->KO_FxMatrix[i][otc_params->OTC] = test1;
            else if (
                otc_params->KO_FxMatrix[i][otc_params->OTC] >
...
Err OTCutilsCorrelateFxAndRates(
    otcpd_params* OTC_params,
    double        meanDom,
    double        stdDom,
    double        meanFor,
...
MixedSL_Calib_Fonctionnal(Integer n, double x[], double* objf, double g[], Nag_Comm* comm)
{
    ...
    {
        /* Pricing */
        dPremium = srt_f_optblksch(
            dConv * (sParams->dForward + x[0]),
        ...
        if (comm->flag == 2)
        {
            ...
            d1    = log((sParams->dForward + x[0]) / (sParams->dStrikes[i] + x[0])) / stdev +
                 0.5 * stdev;
            d2 = d1 - stdev;
...
static void NAG_CALL MixedSL_Calib_LeastSquare(
    Integer m, Integer n, double x[], double fvec[], double fjac[], Integer tdj, Nag_Comm* comm)
{
    CALIBSL_PARAMS sParams = (CALIBSL_PARAMS)(comm->p);
    int            i;
...
Err Calib_MixedSL_ToSmile(
    CALIBSL_PARAMS sParams, double* dShift, double* dVolDown, double* dVolUp, double* dFittingError)
{
    Err err = NULL;

    double dInputs[3], dLBound[3], dHBound[3], dGradient[3], dFuncValue[3], dJacobi[9];
...
Err Srt_smooth_matrix(
    int method, int num_adj, int NRow, int NCol, double** in_matrix, double** out_matrix)
{

...
/*
  ...
  2 senn at some observation date through a grfn tableau.
...
Err swap_correl(Date obs_date, Date start1, Date end1, String cmp1,
  ...
  if (*n_ln == 'N') {
    sprintf(swap1, "SWAP(%d  ,%d  ,\"%s\"  ,\"%s\")", (int)start1, (int)end1,
            cmp1, basis1);
    sprintf(swap2, "SWAP(%d  ,%d  ,\"%s\"  ,\"%s\")", (int)start2, (int)end2,
            cmp2, basis2);
  } else if (*n_ln == 'L') {
    sprintf(swap1, "LOG(SWAP(%d  ,%d  ,\"%s\"  ,\"%s\"))", (int)start1,
            (int)end1, cmp1, basis1);
    sprintf(swap2, "LOG(SWAP(%d  ,%d  ,\"%s\"  ,\"%s\"))", (int)start2,
            (int)end2, cmp2, basis2);
  } else

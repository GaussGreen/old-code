...
/* -----------------------------------------------------------------------
   ...
   ----------------------------------------------------------------------- */
double model_correl(double mat1, double mat2, double alpha, double beta, double rho)
{
    double normvar1;
    double normvar2;
    double normcovar;
    ...
    normvar1  = 1 + 2 * rho * alpha * exp(-beta * mat1) + alpha * alpha * exp(-2 * beta * mat1);
    normvar2  = 1 + 2 * rho * alpha * exp(-beta * mat2) + alpha * alpha * exp(-2 * beta * mat2);
...
Err model_corr_matrix(
    double** model_corr, double* tenor_mat, long num_tenor, double alpha, double beta, double rho)
{
    long i, j;
    ...
    for (i = 0; i < num_tenor; i++)
    {
        for (j = 0; j < i; j++)
        {
            model_corr[i][j] = model_correl(tenor_mat[i], tenor_mat[j], alpha, beta, rho);
    ...
    double pdConfidenceLevel[2])
{
    ...
    if (err =
            model_corr_matrix(CorrelMatrix, pdSetOfDates, iNumberOfDates - 1, dAlpha, dGamma, dRho))
        return err;
    ...
    if (err = jacobi_diagonalisation(CorrelMatrix, iNumberOfDates, EigenValues, EigenVectors, &i))
        return err;
...
Err srt_f_twofac_ifr_correl(String tenor1, String tenor2, String und_name, double* correl)
...
Err swap_correl(
    ...
    double* correl)
{
    char ***tableau = smatrix_size(0, 0, 0, 1, GRFN_DEF_ARGBUFSZ), swap1[GRFN_DEF_ARGBUFSZ],
    ...
    if (*n_ln == 'N')
    {
        sprintf(swap1, "SWAP(%d,%d,\"%s\",\"%s\")", (int)start1, (int)end1, cmp1, basis1);
        sprintf(swap2, "SWAP(%d,%d,\"%s\",\"%s\")", (int)start2, (int)end2, cmp2, basis2);
    }
    else if (*n_ln == 'L')
    {
        sprintf(swap1, "LOG(SWAP(%d,%d,\"%s\",\"%s\"))", (int)start1, (int)end1, cmp1, basis1);
        sprintf(swap2, "LOG(SWAP(%d,%d,\"%s\",\"%s\"))", (int)start2, (int)end2, cmp2, basis2);
    }
    else

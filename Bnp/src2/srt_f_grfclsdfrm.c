...
static void grfn_make_bondoption_string(String bndopt_str, SwapDP *sdp,
				double strike, double bk, double acc_int,
                                Date pay_date,SrtReceiverType rec_pay);
					
static	void 	
grfn_mk_resetcap_spr(Date				*eventdates,
					 GrfnCell			**sprdsht,
					 SwapDP 			*sdp,
					 double 			strike,
...
static	void 	
grfn_mk_bdt_bnd_opt_sht( GrfnCell ***sprdshtptr, SwapDP *sdp, double strike,
			double bk,double acc_int,SrtReceiverType rec_pay,
			Date *eventdatesptr, double *cvg, int numeventdates,
			int i_start);
static	void 	
grfn_mk_bdt_cap_flr_sht( GrfnCell ***sprdshtptr, SwapDP *sdp,double strike,
			double bk,double acc_int,SrtReceiverType rec_pay,
			Date *eventdatesptr,double *cvg,int numeventdates,
			int numcaplets, int istart);
...
///// added by Albert Wang 08/25/03 - end 
	...
	/* There is no closed form available: have to build a full grfn tableau */
		if (mdl_type == BDT)
		{
			err = grfn_SwapDP_to_BDT_GrfnCells(
       			&numeventdates,&eventdates,&nrows,&ncols,&sprdsht,
         		und,sdp,strike,bk,rec_pay,type);
			if(err)
				return err;
		}
		...
		else
		{
			...
			err = grfn_SwapDP_to_GrfnCells(
       				&numeventdates,&eventdates,&nrows,&ncols,&sprdsht,
         			today, sdp, strike, bk, rec_pay, type, und_name, ref_rate_code);
			if(err)
				return err;
			if (nrows == 0)
	...
	/* Call Grfn with this tableau */
		if (!err)
			err = srt_f_grfn(und,grfnparam,numeventdates,&eventdates,&nrows,&ncols,&sprdsht,
...
Err			 err;
	...
	{
		return serror ("Error in srt_f_grfn_clsdfrm_vega: ts vega works only with LGM and CHEY underlyings ");
	}
	if (mdl_dim != ONE_FAC)
	{
	...
	err = srt_f_display_IRM_OneFac_TermStruct (initial_ts, &(sigma[0]), &(sigma[1]), &(sigma[2]), &(sigma[3]), &(sigma[4]), &(sigma[5]), num_sig, 
							&(tau[0]), &(tau[1]), num_tau);		
	if (err)
	{
		return err;
...
///// added by Albert Wang 08/25/03 - begin
		case SIMPLEMIDAT:
			...
			{
				fixing_dates[i] = add_unit(start_dates[i], -sdp->spot_lag, SRT_BDAY, MODIFIED_SUCCEEDING);
			}
...
///// added by Albert Wang 08/25/03 - end 
		...
		case RESETCAPFLOOR:
			...
			/* DON'T INCLUDE RATES SET BEFORE TODAY 
			 ...
			 grfn_mk_resetcap_spr	(fixing_dates,
								sprdsht, 
								sdp, 
								strike, 
								rec_pay, 
...
*/
		"max(swap(%d,%d,\"%s\",\"%s\",\"%s\",\"\",\"%s\") - %.10lf , 0.0) *  lvl(%d,%d,\"%s\",\"%s\")",
			 s, e_nfp, compd_str, basis_str, und_name, ref_rate, strike, s, e_nfp, compd_str, basis_str);
	}
...
String	basis_str;
	...
	{
 	  	sprintf( caplet_str, 
		"max( fra(%d, %d, \"%s\", \"%s\", \"%s\") - %.10lf , 0.0) *  cvg(%d, %d, \"%s\") * df(now, %d)",
			 s, e_nfp, basis_str, und_name, ref_rate, strike, s, e_nfp, basis_str, e_nfp);
	}
...
String	basis_str;
	...
	if ( sdp->direction == FWD )
	{
	...
	else
   	{
		...
		{
         		sprintf( bndopt_str,
				 "max(%.10lf*df(now,%d)-df(now,%d) -%.10lf*df(now,%d)-%.10lf*lvl(%d,%d,\"%s\",\"%s\"),0.0)",
				 bk+acc_int, s, e_nfp, acc_int, pay_date, strike, s, e_nfp, compd_str, basis_str);
     		}
		...
		switch (rec_pay)
		{
			case SRT_PAYER:
				sprintf (sprdsht[i][1].sval, 
						"max(c[0,i]-c[0,i-1],0)*df(d[i],%d)*%g",
						dfind[i], cvg[i]);
				break;
			case SRT_RECEIVER:
				sprintf (sprdsht[i][1].sval, 
...
double    *cvg;
   ...
   switch(t)
   {
		...
		/* If spot_lag = 0: fixing= start => no need to generate two different dates*/
            ...
            grfn_mk_bdt_bnd_opt_sht( &sprdsht, sdp, strike, bk, accrue_int, 
				rec_pay, eventdates , cvg, 
				numeventdates, istart);
...
/* As many rows as fixings + payment dates */
	    ...
	    eventdates[numeventdates-1] = fixing_dates[num_pay_dates-1];
            ...
            grfn_mk_bdt_cap_flr_sht( &sprdsht, sdp, strike, bk, accrue_int, 
				rec_pay, eventdates , cvg, 
				numeventdates, num_pay_dates-1, istart);
...
static	void 	
grfn_mk_bdt_bnd_opt_sht(   	GrfnCell  	***sprdshtptr, 
		       		SwapDP 		*sdp,
		   		double 		strike,
				double 	bk,
				double 		acc_int,
...
static	void 	
grfn_mk_bdt_cap_flr_sht(   	GrfnCell  	***sprdshtptr, 
		       		SwapDP 		*sdp,
		   		double 		strike,
				double 	bk,
				double 		acc_int,
...
/* IF LGM or EtaBeta, there is a REAL closed form: go to it */
		...
		|| (grfnparam->closed_form_type == QUICK_CLSDFRM) )
	{
		...
		{
			err = srt_f_closed_form(und, mdl_type, mdl_dim, &(sdp[i]), strike[i], bk[i], rec_pay[i], 
				type[i], ref_rate_code[i], &(answer[i]));
	...
	/* There is no closed form available: have to build a full grfn tableau */
		...
		else
		{
			...
			err = grfn_SwapDParray_to_GrfnCells(
       				&numeventdates,&eventdates,&nrows,&ncols,&sprdsht,
         			today, lNumInstr, sdp, strike, bk, rec_pay, type, und_name, ref_rate_code);
			if(err)
				return err;
			if (nrows == 0)
				return serror("Product has expired in srt_f_grfn_clsdfrm");
...
{   	
	...
	for(i = 0;i < lNumInstr; i++)
	{
		...
		else
		{
			...
			FixingDatesForSort[i] = add_unit(start_dates[0], -sdp[i].spot_lag, SRT_BDAY, MODIFIED_SUCCEEDING);
			srt_free(start_dates);
			srt_free(end_dates);
			srt_free(pay_dates);
			srt_free(cvg);
...
{   	
	...
	for (i = 0; i < lNumInstr; i++)
	{
		...
		else 		/* SWAPTION */
		{
			...
			FixingDatesForSort[i][0] = add_unit(start_dates[0], -sdp[i].spot_lag, SRT_BDAY, MODIFIED_SUCCEEDING);
			FixingDatesNum[i] = 1;	
		}
	...
	for (i = 0; i < lNumInstr ; i++) index[i] = 0;

	for(j = 0; j < SprdshtRows; j++)
	{
		for(i = 0; i < lNumInstr ; i++)
		{
			if ((index[i] < FixingDatesNum[i]) && (FixingDatesForSort[i][index[i]] == EventDates[j]))
			{						
				switch(type[i])
				{
					case SWAPTION:
	...
	*eventdatesptr = EventDates;
	...
	for(i = 0; i < lNumInstr ; i++)
	{
		if(FixingDatesForSort[i]) srt_free(FixingDatesForSort[i]);
		if (type[i] == CAPFLOOR)
		{
			if(startDatesForSort[i]) srt_free(startDatesForSort[i]);
			if(endDatesForSort[i]) srt_free(endDatesForSort[i]);
		}
	}
	if(FixingDatesForSort) srt_free(FixingDatesForSort);
	if (type[i] == CAPFLOOR)
	{
		if(startDatesForSort) srt_free(startDatesForSort);
		if(endDatesForSort) srt_free(endDatesForSort);
	}

	if(FixingDatesNum) srt_free(FixingDatesNum);
	if(index) srt_free(index);
...
String	basis_str;
	...
	"swap(%d,%d,\"%s\",\"%s\",\"%s\",\"\",\"%s\") *  lvl(%d,%d,\"%s\",\"%s\",\"%s\")",
		 start, end, compd_str, basis_str, und_name, ref_rate, start, end, compd_str, basis_str,und_name);
}

...
SrtLst* ls;
	...
	while ( (ls!=NULL) &&	  
		( ((EquityTermStructVal*)ls->element->val.pval)->time < time) ) ls = ls->next;
...
/* general case */
  ...
  {
    while ( (ls!=NULL) && 
      ( ((EquityTermStructVal*)ls->element->val.pval)->time < time) ) 
      ls = ls->next ;
...
double W_ir_e_func(double time,TermStruct *l)
{
	...
	res = tsval->W_ir_e + tsval->rho_e_ir*tsval->sig*ir_sig*(Psi/F)*(exp(time/tau)-1)*tau
...
Err srt_f_zc_bond_vol(double expiry,
					  char	 *und_name,
...
TermStruct	*ts;
	...
	if(err) return serror("can not get term structure");
...
Err one_expiry_calib_func(char			*model, 
						  double		spot,
						  char			*und_name,
						  char			*disc_und_name,
						  char			*ccy,
						  ...
						  double		*return_value)
{
	...
	und = lookup_und(und_name);
	if(!und) return serror("can not find undelying");
	...
	if(err) return serror("can not get term struct");

	disc_und = lookup_und(disc_und_name);
	if(!disc_und) return serror("can find underlying");

	err = get_underlying_ts(disc_und,&disc_und_ts);
	if(err) return serror("can not get term struct");

	(*return_value) = eq_cum_vol_func(expiry,SRT_YES,ts)-2*(W_ir_e_func(expiry,ts)-Psi_func(expiry,disc_und_ts)*V_ir_e_func(expiry,ts));
...
Err srt_f_eq_forward(Date		date,
					SrtUndPtr	und,
...
Err srt_f_eq_implied_vol(double	expiry,
					  char		*und_name,
					  double    *black_vol)
{
	Err				err = NULL;
	...
	err = get_underlying_ts(und,&ts);
	if(err) return serror("can not get the equity term structure ");
	...
	if((model_type != EQ_STOCH_RATES) && (model_type != EQ_STOCH_RATES_SRVGS) )
	{
		...
		else *black_vol = find_eq_sig(0.0, ts);
	...
	if(!ir_und) return serror(" can not get underlying from name");

	err = get_underlying_ts(ir_und,&disc_und_ts);
	if(err) return serror("can not get interest rate term structure");
	...
	zc_bond_vol = 2*(Psi_func(expiry,disc_und_ts)*L_func(expiry,disc_und_ts)-O_func(expiry,disc_und_ts))/expiry;
				   ...
				   char		**und_name)
{
	...
	disc_und = lookup_und(disc_und_name);
	if(!disc_und) return serror("can not get disc underlying");
	yc_name = get_discname_from_underlying(disc_und);

	yc_crv = lookup_curve(yc_name);
	if(yc_crv == NULL) return serror("fatal: can not get yield curve");
	...
	for( l = 1; l <= n_ex; l++)
	{
		err = srt_f_zc_bond_vol((dates[l-1]-date_today)*YEARS_IN_DAY,
								disc_und_name,
								&zc_bond_vol);
	...
	for(l = 1; l <= n_ex; l++)
	{
		...
		err = one_expiry_calib_func("EQ_STOCH_RATES",
								    spot,
									*und_name,
									disc_und_name,
									ccy,
	...
	if(sig_matrix) free_dmatrix(sig_matrix,0,1,0,n_ex-1); sig_matrix = NULL;
	if(dvd_matrix) free_dmatrix(dvd_matrix,0,1,0,n_dvd-1); dvd_matrix = NULL;
	if(repo_matrix) free_dmatrix(repo_matrix,0,1,0,n_repo-1); repo_matrix = NULL;
	if(newton_target) free_dvector(newton_target,1,n_ex); newton_target = NULL;

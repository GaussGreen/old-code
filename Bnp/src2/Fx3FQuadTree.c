...
/* ==========================================================================
   ...
   PURPOSE:		Modelling of a 3 Facors Quadratic Model:
                                ...
                                - Third one is BS with a quadratic volatility function for the Spot
   Fx.
...
static Err calculate_boundaries(
    long step,
    double (*S_to_Z)(long i, double S, double F, LOCAL_MODEL_PARAM* model_param),
    ...
    double*            eigen_val)
{
    ...
    for (i = 0; i < 8; i++)
    {
        Z = S_to_Z(step, exp(logSpot + maxmin_dual_global[2][i]), fx_fwd, model_param);
    ...
    double*            eigen_val)
{
    ...
    for (step = nstp - 1; step >= 1; step--)
    {
        ...
        nb_nodes_tab[step][i] =
            (long)((maxmin_dual_tab[step][i][0] - maxmin_dual_tab[step][i][1]) / MESH1) + 1;
        ...
        {
            nb_nodes_tab[step][i] =
                (long)((maxmin_dual_tab[step][i][0] - maxmin_dual_tab[step][i][1]) / MESH2) + 1;
        ...
        if (k1 >= 1 && k1 <= nb_nodes_tab[step][idx_rel] - 2)
        {
            ...
            maxmin_dual_tab[step][idx_rel][0] =
                maxmin_dual_tab[step][idx_rel][1] + (nb_nodes_tab[step][idx_rel] - 1) * MESH1;
    ...
    double* res)
{
        ...
        for (i = 0; i < n0; i++)
        {
            ...
            for (j = 0; j < n1; j++)
            {
                ...
                for (k = 0; k < n2; k++)
                {
                    ...
                    prod_matrix_vector_constant(
                        transf_matrix, coos_dual, sqt_prev, svijk, gj, gk, gsum);

                    Z2 = model_func->U_to_Z(step, svijk[2], model_param);

                    /*
                    if ((Z2 > model_param -> Zmax[step]) || (Z2 < model_param -> Zmin[step]))
                    {
                    ...
                    */
                    ...
                    expect_node[2] = drift + e * Z2 * (svijk[0] - svijk[1]) / volZ +
    ...
    expect_node[2] = sv[0][0][0][2] + f * model_func->vol_func_deriv(0, Z2, model_param);

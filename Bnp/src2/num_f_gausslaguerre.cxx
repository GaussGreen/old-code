...
/* Given alf        , the parameter of the Laguerre polynomials        , this
   routine returns arrays x[1..n]*/
/* and w[1..n] containing the abscissas and weights of the n-point
   Gauss-Laguerre quadrature*/
/* formula. The smallest abscissa is returned in x[1]        , the largest in
   x[n].*/

{
  ...
  int i, its, j;
  double ai;
  double p1, p2, p3, pp, z,
      z1; /* High precision is a good idea for this routine.*/
  for (i = 1; i <= n; i++) { /* Loop over the desired roots.*/
    ...
    for (its = 1; its <= MAXIT; its++) { /*Reinement by Newton's method.*/
      p1 = 1.0;
      p2 = 0.0;
       ...
       * its derivative        ,*/
      /*by a standard relation involving also p2        , the polynomial of one
       * lower order.*/
      pp = (n * p1 - (n + alf) * p2) / z;
      z1 = z;

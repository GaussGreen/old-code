	  ...
	  
{
	...
	find_closest_strictly_below (sam_get(node->drift_sam,0,SHORT_RATE),prev_r,trinf->min_r_index,
				  trinf->max_r_index,&node->mid_son_index);  
...

{
	...
	if( node->p[2]>1 || node->p[2]<0 || node->p[1]>1 || node->p[1]<0 || node->p[0]>1 || node->p[0]<0) 
	{  
		if(bndflg)
		{
        ...
        )
{
	...
	for (j=0; j<3; j++)
	{
		find_closest_strictly_below (sam_get (node -> drift_sam, 0 ,PHI), prev_phi[r_i-1+j],
					  trinf -> min_phi_index, trinf -> max_phi_index, &phiind[j]);
		...
		)
{
	...
	else
	{
		...
		/* r at top sam */
		sam_get (top_sam, 0, SHORT_RATE) = exp (trinf -> logrmin + logu  
												* (trinf -> max_r_index - trinf -> min_r_index + 1));

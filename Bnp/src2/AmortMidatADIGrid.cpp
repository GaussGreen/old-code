...
//-------------------------------------------------------------------------------------------------------
//	Description: computes # of intervals between dHB and dLB according to
//dDelta_Base
//
//	Returns :  indicated  , rounded up
//
//-------------------------------------------------------------------------------------------------------
int NumInterval(double dB1, // bound 1
                double dB2, // bound 2
                double dDelta_Base) {
  // establish the base
  const double dNumInterval = fabs(dB1 - dB2) / dDelta_Base;
  const int nNumInterval = (int)(dNumInterval + 0.5) > 1
                               ? (int)(dNumInterval + 0.5)
                               : 1; /// truncate
...
//-------------------------------------------------------------------------------------------------------
//	Description: divides up [dFront  ,dBack] in nNumInterval idential
//intevals 						and fills pdBegin with the result of the discretization
//
...
void Max_Grid(const int *pnSizeX1_Begin, const int *pnSizeX1_End,
  ...
  const int nMaxSize_X3 = *_max_nEle(
      pnSizeX3_Begin, pnSizeX3_Begin + (pnSizeX1_End - pnSizeX1_Begin));
...
//-------------------------------------------------------------------------------------------------------
//	Description: shifts *ppdX_Begin and *ppdX_End equal distance to reach
//size nSize_After
//
//	Returns : as indicated
//-------------------------------------------------------------------------------------------------------
static int __stdcall _Shift_dVec_(int nSize_After, const double **ppdX_Begin,
...
void __stdcall _Shift_X1_(int nSize_After, _Grid *pGrid) {
  ...
  _ASSERTE(pGrid->m_pdX1_End <=
           pGrid->m_pdX1_Begin_ + 2 * pGrid->m_nX1_Zero + 1);
}
...
void __stdcall _Shift_X3_(int nSize_After, _Grid *pGrid) {
  ...
  _ASSERTE(pGrid->m_pdX3_End <=
           pGrid->m_pdX3_Begin_ + 2 * pGrid->m_nX3_Zero + 1);
}
...
static void _Shift_dVec(
    ...
    int *
        pnBegin, // on entry  , current begin index; output: updated begin index
...
//-------------------------------------------------------------------------------------------------------
void __stdcall _Shift_X1(int nSize_After, _Grid *pGrid,
                         int *pnX1_Begin, // result
                         int *pnX1_End    // result
) {
  *pnX1_Begin = pGrid->m_pdX1_Begin - pGrid->m_pdX1_Begin_;
  *pnX1_End = pGrid->m_pdX1_End - pGrid->m_pdX1_Begin_;
  _Shift_dVec(nSize_After, &pGrid->m_pdX1_Begin, &pGrid->m_pdX1_End, pnX1_Begin,
              pnX1_End);
}
...
//-------------------------------------------------------------------------------------------------------
void __stdcall _Shift_X3(int nSize_After, _Grid *pGrid, int *pnX3_Begin,
                         int *pnX3_End) {
  *pnX3_Begin = pGrid->m_pdX3_Begin - pGrid->m_pdX3_Begin_;
  *pnX3_End = pGrid->m_pdX3_End - pGrid->m_pdX3_Begin_;
  _Shift_dVec(nSize_After, &pGrid->m_pdX3_Begin, &pGrid->m_pdX3_End, pnX3_Begin,
              pnX3_End);
}
...
//	Returns : as indicated
//-------------------------------------------------------------------------------------------------------
static double _Variance_X3(double dPHI1, double dPHI2, double dPHI12,
                           double dRho, double dAlpha) {
  // variance of r1
...
static void _preprocess_grid_(const double *pdVarX_Begin,
                              const double *pdVarX_End, int nBase_NumX,
                              int *pnSizeX_Begin, double *pdDeltaX) {
  const double dMaxVarX = *_max_dEle(pdVarX_Begin, pdVarX_End);

  int *pnSizeX = pnSizeX_Begin,
      *pnSizeX_End = pnSizeX_Begin + (pdVarX_End - pdVarX_Begin);
...
void Variance_X3(const double *pdPHI1_Begin, const double *pdPHI1_End,
  ...
  for (; pdPHI1_Begin < pdPHI1_End;
    ...
    *pdVarX3_Begin = _Variance_X3(*pdPHI1_Begin, *pdPHI2_Begin, *pdPHI12_Begin,
                                  dRho, dAlpha);
  }
}
...
const char *_init_amort_grid(
  ...
  _preprocess_grid_(pdVarX1_Begin, pdVarX1_Begin + nNumEx, nBase_NumX1,
                    pnSizeX1_Begin, &dDeltaX1);
  _preprocess_grid_(pdVarX3_Begin, pdVarX3_Begin + nNumEx, nBase_NumX3,
                    pnSizeX3_Begin, &dDeltaX3);

  if (szErr = _init_Grid(*_max_nEle(pnSizeX1_Begin, pnSizeX1_Begin + nNumEx),
                         dDeltaX1,
                         *_max_nEle(pnSizeX3_Begin, pnSizeX3_Begin + nNumEx),
                         dDeltaX3, nNumT, dEx_Back, pGrid)) {
    return szErr;
  }
...
int _validate_gridpoints(int nInitPoint) {
  return (nInitPoint = (nInitPoint % 2 ? nInitPoint : nInitPoint + 1)) > 1
             ? nInitPoint
             : 1;
}
...
const char *_init_Grid(int nInitNumPointX1, double dDeltaX1,
  ...
  *(double *)(&pGrid->m_dDeltaT) = dT / nNumPointT;
  ...
  /// space direction
  if (szErr = _space_discretize_(nNumPointX1, dDeltaX1,
                                 (double **)(&pGrid->m_pdX1_Begin),
                                 (int *)(&pGrid->m_nX1_Zero)))
    return szErr;

  if (szErr = _space_discretize_(nNumPointX3, dDeltaX3,
                                 (double **)(&pGrid->m_pdX3_Begin),
                                 (int *)(&pGrid->m_nX3_Zero)))
    return szErr;
...
static const char *_space_discretize_(
    /// inputs
    int nNumPoint, double dDeltaX,
    /// outputs
    double **ppd_Begin, int *pn_Zero) {
  int nI;
  const char *szErr = 0;
  ...
  *pn_Zero = (nNumPoint - 1) / 2;
  ...
  if (szErr = _alloc_dvector(ppd_Begin, nNumPoint))
    return szErr;
  pd_Begin = (*ppd_Begin);

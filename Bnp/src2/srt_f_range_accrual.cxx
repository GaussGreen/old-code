...
static Err SrtRA_RequestDfDates(SrtProduct *product, long date, int *pn_unds,
  ...
  *payoff = 0.0;
  ...
  if (date != RA->last_date) {
    ...
    if (!RA->quanto) {
      ...
      RA->idx =
          (long *)realloc(RA->idx, (RA->ndfs_d + RA->ndfs_f) * sizeof(long));
    ...
    for (j = 0; j < 4; j++) {
      if (stp->K[j] < K_TINY)
        ...
        err = srt_f_optsarbvol(fwd, stp->K[j], mat, betavol, stp->alpha,
                               stp->beta, stp->rho, SRT_BETAVOL, SRT_LOGNORMAL,
                               &vol);
        if (err)
          return err;
        bs[j] = srt_f_optblksch(fwd, stp->K[j], vol, mat, df, SRT_PUT, PREMIUM);
        /*				bs[j] = srt_f_optblksch( fwd        ,
           stp->K[j]        ,
           stp->vol_atK[j]        , mat        , df        , SRT_PUT        ,
           PREMIUM );*/
      }
    }
...
SrtProduct *srt_f_init_range_accrual(
    char *name, char *yc_d, char *yc_f, char *volc_d, char *volc_f,
    char *refrate_d, char *refrate_f, int n_fxvol_dates, long *fxvol_dates,
    double *fxvol, double *qtocorrel, int n_periods,
    long *dates, /* n_periods + 1 */
    double *cpns, char *basis, double *upper_barr, double *lower_barr,
    char *recpay, double c_spread, int obs_freq, double rho_df) {
  Err err = NULL;
  ...
  for (i = 0, stp = RA->obs_steps[0]; stp->next; stp = stp->next) {
    ...
    /*		for (k=0; k < 4; k++) if (stp->K[k] >= K_TINY)
                    {
                            err = swp_f_SABRvol( volc_f        , stp->date , d ,
       stp->K[k]        , &stp->vol_atK[k]        , &power        , SABR_LOGVOL
       ); if (err) goto RETURN_ERR;
                    }*/
    /* Get SABR parameters for the step */
    err = swp_f_SABRvol(volc_f, stp->date, d, 0.0, &stp->betavol, &power,
                        SABR_BETAVOL);
    if (err)
      goto RETURN_ERR;
    err = swp_f_SABRvol(volc_f, stp->date, d, 0.0, &stp->alpha, &power,
                        SABR_ALPHA);
    if (err)
      goto RETURN_ERR;
    err =
        swp_f_SABRvol(volc_f, stp->date, d, 0.0, &stp->beta, &power, SABR_BETA);
    if (err)
      goto RETURN_ERR;
    err = swp_f_SABRvol(volc_f, stp->date, d, 0.0, &stp->rho, &power, SABR_RHO);
    if (err)
      goto RETURN_ERR;
  }

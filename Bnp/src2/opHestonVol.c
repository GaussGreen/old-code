...
*/
			...
			BSPremium = srt_f_optblknrm(Forward,
											Forward,
											Vol,
											Maturity, 
											1.0, 
			...
			err = srt_f_optimpvol(BSPremium, Forward, Forward, Maturity, 1.0, 
											  SRT_CALL, SRT_LOGNORMAL, &ImplVol);
				...
				double *OutputVol)
{
	...
	*OutputVol = OutPutVolVect[1];
	...
	if(StrikeVect) free_dvector(StrikeVect,1,1);
	if(OutPutVolVect) free_dvector(OutPutVolVect,1,1);
				...
				double *OutputVols)
{
	...
	else if(input == SRT_NORMAL)
	{
		...
		err = srt_f_optimpvol(	price, Forward, Forward, Maturity, 1.0, SRT_CALL, SRT_LOGNORMAL, &ATMLogVol);
		err = HestonCalibrateSigmaInit(
							 			Forward,
							 			Maturity,
							 			ATMLogVol,

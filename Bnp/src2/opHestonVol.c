/*-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-
        ...
        Description	:	implementation of the Heston model: Volatility conversions
...
/*-----------------------------------------------------------------------------------------------
        ...
        of the corresponding calls/puts. It is the equivalent of the add-in SABRVol
                A set of increasing strikes *Strikes must be provided as input, and the equivalent
                volatilities are returned as *NewVols
/*-----------------------------------------------------------------------------------------------*/
...
//{
//    Err err = NULL;
//    int j;  //,IntegrType=3; /* sets up the integration type to the optimised one for a single  */
...
//        */
//
//        BSPremium = srt_f_optblknrm(Forward, Forward, Vol, Maturity, 1.0, SRT_CALL, PREMIUM);
//
//        err = srt_f_optimpvol(
//            BSPremium, Forward, Forward, Maturity, 1.0, SRT_CALL, SRT_LOGNORMAL, &ImplVol);
//
//        err = HestonCalibrateSigmaInit(
//            Forward,
//            Maturity,
    ...
    double*          OutputVols)
{
    ...
    {
        price = srt_f_optblksch(Forward, Forward, Sigma, Maturity, 1.0, SRT_CALL, PREMIUM);
        err   = srt_f_optimpvol(
            price, Forward, Forward, Maturity, 1.0, SRT_CALL, SRT_LOGNORMAL, &ATMLogVol);
        err = HestonCalibrateSigmaInit(
            Forward,
            Maturity,
            ATMLogVol,
...
/*-----------------------------------------------------------------------------------------------
  This function returns the equivalent BS volatility for a given set of Heston parameters and
  a given strike. This overloads the function HestonVol for this particular case
-----------------------------------------------------------------------------------------------*/
//
//Err HestonATMVol(
//    double      Forward,

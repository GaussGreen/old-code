                                ...
                                )
{
	...
	prob -= exp(2*mu*b/(sig*sig)) 
		* norm( du * (b + mu*mat)/sig_sqrt );
...
double 	mu, mu_1, mu_2, k, b;
	...
	/* Usual case */
	{
		...
		premium = fwd * proba_joint_spot_min_max(	mu_2,
													vol,
													mat,
													k,
													b,
		...
		premium -=  strike * proba_joint_spot_min_max(	mu_1,
														vol,
														mat,
														k,
														b,
	...
	switch (greek)
	{
		...
		case DELTA_FWD : 	/*** DELTA FWD ***/  
			shift = fwd / 10000;
			answer = ( srt_f_optexting(	fwd + shift,
										spot,
										strike,
										barrier,
										vol,
		...
		case DELTA :	/*** DELTA SPOT + FWD ***/  
			shift = spot / 10000;
			answer = ( srt_f_optexting(	fwd * (1 + shift/spot),
										spot + shift,
										strike,
										barrier,
										vol,

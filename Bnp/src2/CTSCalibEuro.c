...
static double stat_call, stat_euro[MAX_EURO];

#define OPT_EPS 1.0e-15
#define VOL_MULT 5.0
#define VOL_MULT_ITER 5

Err static_cts_price_european(double index, double vol[], double* price, double* gradient, int nvol)
{
    static int    i, j, ind;
    static double sum2;
    Err           err = NULL;
    ...
    if (ind == 0)
    {
        ...
        for (j = 0; j < nvol; j++)
        {
           ...
           /* err =
                cts_launch_adi(stat_model_cts, stat_und, stat_adi_arg, &stat_call, &(stat_euro[0]));*/

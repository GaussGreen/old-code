...
Err FxSabrSL_adi(
  ...
  double *expect_z = NULL, *drift_z = NULL, *x = NULL, *z = NULL,
         ...
         *varxinit = NULL, *sigBeta = NULL, **r = NULL;
  ...
  if (bar_lvl) {
    disc_SL_center_barrier(&x, &nstepx, spot_fx, flx, fux, a, b, std1 / a,
                           NSTD_FX, bar_lvl, nstp, &index_x);
  } else {
    disc_SL_center(x, nstepx, spot_fx, flx, fux, a, b, std1 / a, NSTD_FX,
                   &index_x);
  }
...
Err FxSabrSLCalibration(
  ...
  if (start_opt == 0) {
    ...
    err = srt_f_optimpvol(premium_tgt * coefa, shift_spot, shift_strike,
                          exercise[0], df, SRT_CALL, SRT_LOGNORMAL, &sig0);

    sig0 /= coefa;
    vol2 = sig0;

    sig0 = op_sabrSLcalib(spot_fx, strike, exercise[0], volatility[0], alpha,
                          beta, rho, lambda);
    ...
    err = FxSabrSL_adi(nstpt, time, date, nstpfx, nstpvol, sig0, drift_adi,
                       alpha, beta, rho, lambda, floormu, func_parm, eval_evt,
                       NULL, NULL, NULL, spot_fx, dom_yc, for_yc,
                       payoff_fx_sabr_adi_ATM_opt, 1, &premium, 0, NULL, 1, 0,
                       premium_tgt, &newSig0, &premium2);
    ...
    while ((k < nbIter) && (errorv >= precision)) {
      err = FxSabrSL_adi(nstpt, time, date, nstpfx, nstpvol, newSig0, drift_adi,
                         alpha, beta, rho, lambda, floormu, func_parm, eval_evt,
                         NULL, NULL, NULL, spot_fx, dom_yc, for_yc,
                         payoff_fx_sabr_adi_ATM_opt, 1, &premium2, 0, NULL, 0,
                         0, 0, NULL, NULL);
      if (err) {
        goto FREE_RETURN;
      }
  ...
  if (start_opt > 0) {
    ...
    err = srt_f_optimpvol(premium_tgt * coefa, shift_spot, shift_strike,
                          exercise[start_opt - 1], df, SRT_CALL, SRT_LOGNORMAL,
                          &vol2);

    vol2 /= coefa;

    volSL2 =
        op_sabrSLcalib(spot_fx, strike, exercise[start_opt - 1],
                       volatility[start_opt - 1], alpha, beta, rho, lambda);
  }
  ...
  for (i = max(start_opt, 1); i < nbOpt; i++) {
    ...
    err = srt_f_optimpvol(premium_tgt * coefa, shift_spot, shift_strike,
                          exercise[i], df, SRT_CALL, SRT_LOGNORMAL, &vol2);

    vol2 /= coefa;

    volSL2 = op_sabrSLcalib(spot_fx, strike, exercise[i], volatility[i], alpha,
                            beta, rho, lambda);

    var_diff =
        (volSL2 * volSL2 * exercise[i] - volSL1 * volSL1 * exercise[i - 1]);
...
static double SolveForKPutPercent(double DeltaP, double F, double std,
                                  double DfDom, double DfFor);
static double SolveForKCallOutPercent(double DeltaP, double F, double std,
                                      double DfDom, double DfFor);

static Err FxSabrSLCalibSmilePricer(
    /* Model params */
    double today, double spot_fx, double fwd_fx, double alpha, double beta,
...

{
  ...
  err = FxSabrSL_adi(
      nstp, time, date, nstpfx, nstpvol, sigma_fx[0], drift, alpha, beta, rho,
      lambda, floormu, func_parm, is_event, NULL, NULL, NULL, spot_fx, dom_yc,
      for_yc, payoff_fx_sabr_adi_opt, 2, res, 0, NULL, 0, 0, 0, NULL, NULL);
...

{
  ...
  if (*vol1 < 1.0E-08 || *vol2 < 1.0E-08 || *strike1 < 1.0E-08 ||
    ...
    *strike2 = SolveForKCallOutPercent(0.25, fwd_fx, *vol2 * sqrt(mat_smile),
                                       df_dom, df_for);
  } else {
    if (*strike1 > *strike2) {
      temp = *strike1;
...
Err FxSabrSLCalibSmile(
  ...
  if (calib_smile) {
    ...
    err = GetStrikesVol(fwd_fx, mat_smile, df, df_for, atm_vol, risk_reversal,
                        ...
                        &vol2, &alpha1, beta, &rho1, lambda, is_straddle,
    ...
    *vol2_out = vol2;
    ...
    /* calibration */
    err = FxSabrSLCalibration(
        dom_yc, for_yc, today, spot_fx, alpha1, beta, rho1, lambda, floormu,
    ...
    /* pricer */
    err = FxSabrSLCalibSmilePricer(
        today, spot_fx_cash, fwd_fx, alpha1, beta, rho1, lambda, floormu,
    ...
    if (error > precisionSmile) {
      ...
      if ((use_input || (!use_input && fabs(fitting_error) > 0.025)) &&
        ...
        /* calibration */
        err = FxSabrSLCalibration(dom_yc, for_yc, today, spot_fx, alpha1, beta,
                                  rho1, lambda, floormu, exercise_cal,
    ...
    while ((k < nbIterSmile) && (error >= precisionSmile)) {
      ...
      /* calibration */
      err = FxSabrSLCalibration(dom_yc, for_yc, today, spot_fx, alpha2, beta,
                                rho1, lambda, floormu, exercise_cal,
                                maturity_cal, volatility_cal, nb_calib, 0,
                                is_straddle, nstpt_tgt, nstpfx, nstpvol,
      ...
      /* pricer */
      err = FxSabrSLCalibSmilePricer(
          today, spot_fx_cash, fwd_fx, alpha2, beta, rho1, lambda, floormu,
          dom_yc, for_yc, exercise_cal, sigma, nb_calib, time, date, drift,
      ...
      /* calibration */
      err = FxSabrSLCalibration(dom_yc, for_yc, today, spot_fx, alpha1, beta,
                                rho2, lambda, floormu, exercise_cal,
                                maturity_cal, volatility_cal, nb_calib, 0,
                                is_straddle, nstpt_tgt, nstpfx, nstpvol,
      ...
      /* pricer */
      err = FxSabrSLCalibSmilePricer(
          today, spot_fx_cash, fwd_fx, alpha1, beta, rho2, lambda, floormu,
          dom_yc, for_yc, exercise_cal, sigma, nb_calib, time, date, drift,
          nstp, func_parm, is_event, nstpt_tgt, nstpfx, nstpvol, df, mat_smile,
          strike1, strike2, res, &impvol12, &impvol22);
      ...
      /* calibration */
      err = FxSabrSLCalibration(
          dom_yc, for_yc, today, spot_fx, alpha3, beta, rho3, lambda, floormu,
          exercise_cal, maturity_cal, volatility_cal, nb_calib, 0, is_straddle,
      ...
      /* pricer */
      err = FxSabrSLCalibSmilePricer(
          today, spot_fx_cash, fwd_fx, alpha3, beta, rho3, lambda, floormu,
          dom_yc, for_yc, exercise_cal, sigma, nb_calib, time, date, drift,
          nstp, func_parm, is_event, nstpt_tgt, nstpfx, nstpvol, df, mat_smile,
...
void disc_SL_center(double *x, long nstp, double fwd, double xmin, double xmax,
  ...
  for (i = 1; i < (*index); i++) {
    x[i] = (fwd_sl * exp(-0.5 * std_sl * std_sl +
                         std_sl * inv_cumnorm_fast(dmin + i * meshx)) -
            b) /
           a;
  }
  ...
  for (i = (*index) + 1; i < nstp; i++) {
    x[i] = (fwd_sl *
                exp(-0.5 * std_sl * std_sl +
                    std_sl * inv_cumnorm_fast(dfwd + (i - (*index)) * meshx)) -
            b) /
           a;
  }
}
...
void disc_SL_center_barrier(double **x, long *nstp, double fwd, double xmin,
                            ...
                            double nstd, double *barrier, long nb_bar,
                            long *index) {
  double meshx;
  ...
  for (i = 0; i < nb_bar; i++) {
    temp = a * barrier[i] + b;
    if (barrier[i] > 1.0E-10 && temp > 1.0E-10 &&
        fabs(barrier[i] - fwd) > 1.0E-10) {
  ...
  while (dtemp < dfwd * 0.9999999) {
    res[i] = (fwd_sl * exp(-0.5 * std_sl * std_sl +
                           std_sl * inv_cumnorm_fast(dtemp)) -
              b) /
             a;
    i++;
    ...
    while (j < nb_bar2 && dbar[j] < dtemp * 0.9999999 &&
           dbar[j] < dfwd * 0.9999999) {
      res[i] = bar[j];

      temp = 2.0 * res[i] - res[i - 1];

      if (temp < fwd * 0.9999999 &&
          ((j < nb_bar2 - 1 && temp < bar[j + 1] * 0.9999999) ||
           (j == nb_bar2 - 1))) {
        i++;
        res[i] = temp;
        dtemp = norm((log((a * temp + b) / fwd_sl) + 0.5 * std_sl * std_sl) /
                     std_sl) +
                meshx;
      } else {
        dtemp = dbar[j] + meshx;
  ...
  *index = i;
  ...
  while (dtemp < dmax * 0.9999999) {
    ...
    while (j < nb_bar2 && dbar[j] < dtemp * 0.9999999 &&
           dbar[j] < dmax * 0.9999999) {
      res[i] = bar[j];

      temp = 2.0 * res[i] - res[i - 1];

      if (temp < xmax * 0.9999999 &&
          ((j < nb_bar2 - 1 && temp < bar[j + 1] * 0.9999999) ||
           (j == nb_bar2 - 1))) {
        i++;
        res[i] = temp;
...
void disc_SL_strike(double *x, long nstp, double fwd, double xmin, double xmax,
  ...
  if (!is_center) {
    ...
    (*index) = (int)(nstp / (1 + (dmax - dfwd) / (dmaxt - dfwd) *
                                     (dfwd - dmin) / (dfwd - dmint)));
  ...
  if (strike < fwd * 0.9999 && strike >= xmin) {
    ...
    for (i = index_s + 1; i < (*index); i++) {
      x[i] =
          (fwd_sl *
               exp(-0.5 * std_sl * std_sl +
                   std_sl * inv_cumnorm_fast(dstrike + (i - index_s) * meshx)) -
           b) /
          a;
    }

    ...
    double* res_at_point)
{
    ...
    err = FxSabrSLPrecalculations(
        nstp, time, sig0, drift, alpha, a, b, rho, lambda, spot_fx, expect_z, drift_z, &std1);
    ...
    if (bar_lvl)
    {
        disc_SL_center_barrier(
            &x, &nstepx, spot_fx, flx, fux, a, b, std1 / a, NSTD_FX, bar_lvl, nstp, &index_x);
    }
    else
    {
    ...
    double* sigma)
{
    ...
    if (start_opt == 0)
    {
        ...
        sig0 =
            op_sabrSLcalib(spot_fx, strike, exercise[0], volatility[0], alpha, beta, rho, lambda);
    ...
    for (i = max(start_opt, 1); i < nbOpt; i++)
    {
        ...
        volSL2 =
            op_sabrSLcalib(spot_fx, strike, exercise[i], volatility[i], alpha, beta, rho, lambda);
...
static double SolveForKCallOutPercent(
    double DeltaP, double F, double std, double DfDom, double DfFor);
    ...
    long*   index)
{
    ...
    for (i = 1; i < (*index); i++)
    {
        x[i] = (fwd_sl * exp(-0.5 * std_sl * std_sl + std_sl * inv_cumnorm_fast(dmin + i * meshx)) -
                b) /
               a;
    }
...
void disc_SL_center_linleft_logright(
    double* x,
    long    nstp,
    double  fwd,
    double  xmin,
    ...
    while (dtemp < dfwd * 0.9999999)
    {
        ...
        while (j < nb_bar2 && dbar[j] < dtemp * 0.9999999 && dbar[j] < dfwd * 0.9999999)
        {
            ...
            if (temp < fwd * 0.9999999 &&
                ((j < nb_bar2 - 1 && temp < bar[j + 1] * 0.9999999) || (j == nb_bar2 - 1)))
            {
                i++;
                res[i] = temp;
                dtemp =
                    norm((log((a * temp + b) / fwd_sl) + 0.5 * std_sl * std_sl) / std_sl) + meshx;
            }
            else
            {
                dtemp = dbar[j] + meshx;
    ...
    long*   index)
{
    ...
    if (!is_center)
    {
        ...
        (*index) =
            (int)(nstp / (1 + (dmax - dfwd) / (dmaxt - dfwd) * (dfwd - dmin) / (dfwd - dmint)));
    ...
    if (strike < fwd * 0.9999 && strike >= xmin)
    {
        ...
        {
            x[i] = (fwd_sl *
                        exp(-0.5 * std_sl * std_sl + std_sl * inv_cumnorm_fast(dmin + i * meshx)) -
                    b) /
                   a;
        }

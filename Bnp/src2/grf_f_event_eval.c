...
the value of the entire event  , which will be the cashflow in the rightmost of
the tableau.

                        Result = f(sample  , fwd_sample  , gd  , [tree
information])
...
return NULL (no error).
                        ...
                        payment is too small  , it will approximate the swap
rate with the short rate.
  ...
  while (comll != NULL) {
    ...
    if ((type_eval == 2) ||
      ...
      case COMLL_F_PAYFROMPAST:
        ...
        /* Keep track of this payment: store cashflow  , pay date and event date
         * in the GrfnDeal */
        gd->numpay += 1;
      ...
      case COMLL_F_CMS:
        ...
        /*					error = swp_f_cmsratedp(
                                                                (SwapDP
           *)comll->ptr  , event->d  , x  , vol  , DTOL(delay)  ,
                                                                DEFAULT_CMS_DELTA
           , MAX_CMS_SWAPS  , difftype  , &x);	*/
      ...
      case COMLL_A_BSSABR: /* Black-Scholes with SABR */
        ...
        y = POP(); /* time difference = mat */
        difftype =
            (SrtDiffusionType)POP(); /* 0 = Log  , 1 = Norm  , 2 = beta */
        call_put = (int)POP();

        /*					if (error =
           srt_f_optbetastochvoltoblkvol( f  , k  , betavol  , alpha  , rho  ,
                                                        y  ,
                                                        beta  ,
                                                        &x)) return error;
        */
      ...
      case COMLL_X_PVINTERP:
         ...
         * input */
        //					grf_f_evlcllfct_interp(  , gd->cells  ,
        //r1  , c1  , c2  , x  , comll->type  , &y);
        PUSH(y);
        comll = comll->next;
        break;

/******************************************************************************\
    ...
    RETURNS         :   An error
                                                Affect in the cash-flow input the value of the
entire event, which will be the cashflow in the rightmost of the tableau.
    ...
    while (comll != NULL)
    {
            ...
            ((type_eval == 1) && (comll->evalstatus == BWDCELL)))
        {
            ...
            case COMLL_F_PAYFROMPAST:
                ...
                /* Keep track of this payment: store cashflow, pay date and event date in the
                 * GrfnDeal */
                gd->numpay += 1;
            ...
            case COMLL_F_PAYOFF:
                ...
                if (comll->next && comll->next->type == COMLL_F_PAYOFF && comll->next->ptr == NULL)
                {
                    comll = comll->next;
                    ix    = comll->ivec[0];
                    for (i = 0; i < comll->dfindlen; i++)
            ...
            case COMLL_F_CMS:
                ...
                error = swp_f_cmsratedp_clsdfrm(
                    (SwapDP*)comll->ptr, event->d, x, vol, DTOL(delay), difftype, &x);
            ...
            case COMLL_A_BSSABR: /* Black-Scholes with SABR */
                ...
                */
                ...
                if (error = srt_f_optsarbvol(
                        f, k, y, betavol, alpha, beta, rho, difftype, SRT_LOGNORMAL, &x))
                    return error;
                if (x <= 0.0)
                {
                    if (call_put) /* put */
            ...
            case COMLL_X_INTERP:
                ...
                x  = POP();
                y  = x;
                interp(gd->aux[r1], gd->aux[r2], IMIN(gd->auxlen[r1], gd->auxlen[r2]), y, 0, &x);
                PUSH(x);
                comll = comll->next;
                break;
            ...
            case COMLL_X_PVINTERP:
                ...
                //					grf_f_evlcllfct_interp(, gd->cells, r1, c1, c2, x,
                //comll->type, &y);
                PUSH(y);
                comll = comll->next;
                break;
...

{
    ...
    return (grfn_eval_event_ammc(
        2, event, sample, gd, pv_vect, (double*)NULL, (double*)NULL, evaldf, und_info, cashflow));
}
...
#define PVSOURCESTATUSSET(c1, v)   \
    {                              \
        pvSource_status[c1] = (v); \
    } /* Set a new PVSource status*/

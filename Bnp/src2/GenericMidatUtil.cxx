...
Err genmidat_alloc_model(int iNbExe, int iNbFactor, GENMIDAT_MODEL sModel) {
  ...
  if (!sModel->dTimes || !sModel->dTimes || !sModel->dSigma || !sModel->dBeta ||
      !sModel->dLambda) {
  ...
  if (iNbFactor == 2) {
    ...
    if (!sModel->dSigma2 || !sModel->dBeta2 || !sModel->dLambda2 ||
        !sModel->dCorrel) {
...
Err genmidat_init_model(double dAlpha, double dGamma, double dRho,
                        double *dTimes, double *dForward, double *dSigma,
                        double *dBeta, double *dSigma2, double *dCorrel,
                        double dStartBeta2, double dNumeraire,
...
Err genmidat_copy_model(GENMIDAT_MODEL sSourceModel,
  ...
  err = genmidat_alloc_model(sSourceModel->iNbVols, sSourceModel->iNbFactor,
                             sDestModel);
  ...
  err = genmidat_init_model(
      sSourceModel->dAlpha, sSourceModel->dGamma, sSourceModel->dRho,
      sSourceModel->dTimes, sSourceModel->dForward, sSourceModel->dSigma,
      sSourceModel->dBeta, sSourceModel->dSigma2, sSourceModel->dCorrel,
      sSourceModel->dStartBeta2, sSourceModel->dNumeraire, sDestModel);
  ...
  if (0) {
    if (0) {
      for (i = 0; i < sModel->iNbVols; i++) {
        if (i > 0) {
          sModel->dLambda[i] = -log(sModel->dBeta[i] / sModel->dBeta[i - 1]) /
        ...
        if (sModel->iNbFactor == 2) {
          ...
          sModel->dBeta2[i] =
              sModel->dBeta[i] * exp(-sModel->dGamma * sModel->dTimes[i]);
        }
      }
    } else {
      ...
      for (i = 0; i < sModel->iNbVols; i++) {
        if (i > 0) {
          if (sModel->dBeta[i - 1] - sModel->dBeta[i] > 1.0E-08) {
            sModel->dLambda[i] =
                (-log(sModel->dBeta[i - 1] - sModel->dBeta[i]) - dSumLam) /
                (sModel->dTimes[i] - sModel->dTimes[i - 1]);
          } else {
          ...
          dSumLam +=
              sModel->dLambda[i] * (sModel->dTimes[i] - sModel->dTimes[i - 1]);
        } else {
          if (sModel->dBeta[i] >= 0.9999999999) {
            sModel->dLambda[i] = 0.0;
          } else {
            sModel->dLambda[i] =
                -log(1.0 - sModel->dBeta[i]) / sModel->dTimes[i];
          }
...
Err genmidat_fill_2factor(GENMIDAT_MODEL sModel) {
  ...
  sModel->dBeta2[0] =
      sModel->dStartBeta2 + (sModel->dBeta[0] - sModel->dStartBeta) *
                                exp(-sModel->dGamma * sModel->dTimes[0]);
  sModel->dLambda2[0] = 0.0;

  for (i = 1; i < sModel->iNbVols; i++) {
    exp_fact = exp(sModel->dGamma * sModel->dTimes[i]);
    sModel->dBeta2[i] = sModel->dBeta2[i - 1] +
                        (sModel->dBeta[i] - sModel->dBeta[i - 1]) / exp_fact;
    sModel->dLambda2[i] = sModel->dLambda[i] + sModel->dGamma;
  }
...
double genmidat_Swap_correlation(double dTime1, double dTime2, double dAlpha,
...
Err SrtInitGenericMidatUnd(char *undName, /* und name */
                           int iNbFactor, double dNumeraire, double dAlpha,
                           double dGamma, double dRho, int iNbVols,
                           double *dTimes, double *dForward, double *dSigma,
                           double *dBeta, double *dSigma2, double *dCorrel,
                           double dStartBeta2) {
  ...
  /* Fill the model */
  err = genmidat_alloc_model(iNbVols, iNbFactor, sModel);

  if (err)
    goto FREE_RETURN;

  err =
      genmidat_init_model(dAlpha, dGamma, dRho, dTimes, dForward, dSigma, dBeta,
                          dSigma2, dCorrel, dStartBeta2, dNumeraire, sModel);
  ...
  err = srt_f_lstins(und_list, pUndPtr->underl_name, 0.0, OBJ_PTR_UND,
                     (void *)pUndPtr, &genmidat_free_und_struct,
...
Err genmidat_get_model(char *cUndName, GENMIDAT_MODEL sModel) {
  ...
  err = genmidat_copy_model(sSavedModel, sModel);
...
Err genmidat_implied_volatility_and_price_model(
    int iIntegStartIndex, int iIntegEndIndex, int iUndIndex, int iNbPeriod,
    GENMIDAT_MODEL sModel, double *dVolatility, double *dPrice) {
  Err err = NULL;
  int i;
  ...
  if (iIntegStartIndex < 0) {
    ...
    if (sModel->iNbFactor == 2) {
      ...
      covar += sModel->dCorrel[0] * sModel->dSigma[0] * sModel->dSigma2[0] *
               sModel->dTimes[0];
    }
  ...
  for (i = iIntegStartIndex; i < iIntegEndIndex; i++) {
    varX += sModel->dSigma[i + 1] * sModel->dSigma[i + 1] *
            (sModel->dTimes[i + 1] - sModel->dTimes[i]);

    if (sModel->iNbFactor == 2) {
      varY += sModel->dSigma2[i + 1] * sModel->dSigma2[i + 1] *
              (sModel->dTimes[i + 1] - sModel->dTimes[i]);
      covar += sModel->dCorrel[i + 1] * sModel->dSigma[i + 1] *
               sModel->dSigma2[i + 1] *
               (sModel->dTimes[i + 1] - sModel->dTimes[i]);
    }
  }
  ...
  dVol = dBeta1 * dBeta1 * varX + dBeta2 * dBeta2 * varY +
         2.0 * dBeta1 * dBeta2 * covar;
  ...
  *dPrice = srt_f_optblknrm(dForward, 0.0, *dVolatility, dEndTime - dStartTime,
                            sModel->dNumeraire, SRT_CALL, PREMIUM);

  return err;
}

Err genmidat_implied_volatility_and_price_und(
    char *cUndName, int iIntegStartIndex, int iIntegEndIndex, int iUndIndex,
    int iNbPeriod, double *dVolatility, double *dPrice) {
  GENMIDAT_MODEL sModel;
  Err err = NULL;
  ...
  /* Get the Model */
  err = genmidat_get_model(cUndName, sModel);
  ...
  err = genmidat_implied_volatility_and_price_model(
      iIntegStartIndex, iIntegEndIndex, iUndIndex, iNbPeriod, sModel,
      dVolatility, dPrice);
...
Err genmidat_implied_correlation_model(int iIntegStartIndex, int iIntegEndIndex,
                                       int iUndIndex1, int iNbPeriod1,
                                       int iUndIndex2, int iNbPeriod2,
                                       GENMIDAT_MODEL sModel,
                                       double *dCorrelation) {
  Err err = NULL;
  int i;
  ...
  /* Checks */
  if (iIntegStartIndex > sModel->iNbVols || iIntegEndIndex > sModel->iNbVols ||
      iIntegStartIndex >= iIntegEndIndex || iUndIndex1 > sModel->iNbVols - 1 ||
      iUndIndex1 < iIntegEndIndex || iUndIndex2 > sModel->iNbVols - 1 ||
      iUndIndex2 < iIntegEndIndex) {
  ...
  dVol1 = dBetaX1 * dBetaX1 * varX + dBetaY1 * dBetaY1 * varY +
          2.0 * dBetaX1 * dBetaY1 * covar;
  dVol2 = dBetaX2 * dBetaX2 * varX + dBetaY2 * dBetaY2 * varY +
          2.0 * dBetaX2 * dBetaY2 * covar;
...
Err genmidat_implied_correlation_und(char *cUndName, int iIntegStartIndex,
                                     int iIntegEndIndex, int iUndIndex1,
                                     int iNbPeriod1, int iUndIndex2,
                                     int iNbPeriod2, double *dCorrelation) {
  GENMIDAT_MODEL sModel;
  Err err = NULL;
  ...
  err = genmidat_implied_correlation_model(iIntegStartIndex, iIntegEndIndex,
                                           iUndIndex1, iNbPeriod1, iUndIndex2,
                                           iNbPeriod2, sModel, dCorrelation);

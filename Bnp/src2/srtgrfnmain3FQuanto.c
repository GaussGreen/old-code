 ...
 *            in the case of the 3Factor QUANTO model				  *	
						  ...
						  double**	prod_val)
{
	...
	double	*dom_sigma_dates	=	NULL,
			...
			*dom_sigma_merge_values = NULL,
			*for_sigma_merge_values = NULL;
	...
	if (xStr.num_und > 3)
	{
		err = "Fx3FQuantoGrfn can only accept 3 underlyings, please check the GRFN tableau or use the MultiGRFN functions";
		goto FREE_RETURN;
	}
	...
	// Takes the time vector containing the event dates and slices it into a bigger
	// time vector using the number of steps required in the tree. Don't know why this
	// function needs bar_times and vol_times since they are not used....
	...
	// The KEY point is that we will project all the term structures (vols, correls)
	// on the single "time" schedule which contains the GRFN event dates and intermediary
	// dates which are only calculated from the number of steps we want. (no vols or corr dates)
	// We MAY therefore be WRONG on the vols/corrs since the vols/corr dates will not always match
	// the "time" schedule but we do not care since increasing the number of steps will lead to convergence.
	...
	if (!vol_change || !r1_merge_sigma || !r2_merge_sigma || !r3_merge_sigma || !fx_merge_sigma
		|| !corr_merge_r1_r2 || !corr_merge_r1_r3 || !corr_merge_r1_fx
		|| !corr_merge_r2_r3 || !corr_merge_r2_fx || !corr_merge_r3_fx)
	{
	...
	vol_change[nstp-1] = 1;
	r1_merge_sigma[nstp-1] = r1_sigma[Get_Index(time[nstp-1], r1_sigma_dates, n_r1_sigma_dates)];
	r2_merge_sigma[nstp-1] = alpha * r1_merge_sigma[nstp-1];
	r3_merge_sigma[nstp-1] = r3_sigma[Get_Index(time[nstp-1], r3_sigma_dates, n_r3_sigma_dates)];
	fx_merge_sigma[nstp-1] = fx_sigma_values[Get_Index(time[nstp-1], fx_sigma_dates, n_fx_sigma_dates)];

	correl_index = Get_Index(time[nstp-1], correl_dates, n_correl_dates);

	corr_merge_r1_r2[nstp-1] = rho;
	corr_merge_r1_r3[nstp-1] = correl_matrix_4x4_ts[correl_index][LGM2F_W1][LGM1F_W3];
	corr_merge_r2_r3[nstp-1] = correl_matrix_4x4_ts[correl_index][LGM2F_W2][LGM1F_W3];

	if (lgm2F_is_dom_for == 0)
	{
		corr_merge_r1_fx[nstp-1] = 0.0;
		corr_merge_r2_fx[nstp-1] = 0.0;
		corr_merge_r3_fx[nstp-1] = correl_matrix_4x4_ts[correl_index][LGM1F_W3][FX_3F_QUANTO];
	}
	else
	{
		corr_merge_r1_fx[nstp-1] = correl_matrix_4x4_ts[correl_index][LGM2F_W1][FX_3F_QUANTO];
		corr_merge_r2_fx[nstp-1] = correl_matrix_4x4_ts[correl_index][LGM2F_W2][FX_3F_QUANTO];
	...
	for (i=nstp-2; i>=0; i--)
	{
		r1_merge_sigma[i] = r1_sigma[Get_Index(time[i], r1_sigma_dates, n_r1_sigma_dates)];
		r2_merge_sigma[i] = alpha * r1_merge_sigma[i];
		r3_merge_sigma[i] = r3_sigma[Get_Index(time[i], r3_sigma_dates, n_r3_sigma_dates)];
		fx_merge_sigma[i] = fx_sigma_values[Get_Index(time[i], fx_sigma_dates, n_fx_sigma_dates)];

		correl_index = Get_Index(time[i], correl_dates, n_correl_dates);

		corr_merge_r1_r2[i] = rho;
		corr_merge_r1_r3[i] = correl_matrix_4x4_ts[correl_index][LGM2F_W1][LGM1F_W3];
		corr_merge_r2_r3[i] = correl_matrix_4x4_ts[correl_index][LGM2F_W2][LGM1F_W3];

		if (lgm2F_is_dom_for == 0)
		{
			corr_merge_r1_fx[i] = 0.0;
			corr_merge_r2_fx[i] = 0.0;
			corr_merge_r3_fx[i] = correl_matrix_4x4_ts[correl_index][LGM1F_W3][FX_3F_QUANTO];
		}
		else
		{
			corr_merge_r1_fx[i] = correl_matrix_4x4_ts[correl_index][LGM2F_W1][FX_3F_QUANTO];
			corr_merge_r2_fx[i] = correl_matrix_4x4_ts[correl_index][LGM2F_W2][FX_3F_QUANTO];
		...
		if (  fabs (r1_merge_sigma[i] - r1_merge_sigma[i+1])
			...
			+ fabs (corr_merge_r3_fx[i] - corr_merge_r3_fx[i+1]) > EPS)
		{
			vol_change[i] = 1;
		}	
		else
	...
	if (!lgm2F_ifr || !r1_fwd || !r1_var || !r2_fwd || !r2_var
		|| !r1_r2_covar || !lgm1F_ifr || !r3_fwd || !r3_var)
	{
	...
	for (i=nstp-1; i>=0; i--)
	{
		...
		if (j >= 0 && fabs (date[i] - xStr.dts[j]) < 1.0e-04)
		{
			...
			// Detects if any DF without underlying is requested at the GRFN date (FX DF = DOM DF)
		...
		{
			// Filling when i is a discretization date which is not a GRFN date (most cases)
			is_event[i] = 0;
			void_prm[i] = NULL;
		}
	}
...
FREE_RETURN:
	...
	if (dom_sigma_merge_values) free(dom_sigma_merge_values);
	if (for_sigma_merge_values) free(for_sigma_merge_values);
	
	// Free the merged term structures
	if (vol_change) free (vol_change);
	if (r1_merge_sigma) free(r1_merge_sigma);
	if (r2_merge_sigma) free(r2_merge_sigma);
	if (r3_merge_sigma) free(r3_merge_sigma);
	if (fx_merge_sigma) free(fx_merge_sigma);
	if (corr_merge_r1_r2) free(corr_merge_r1_r2);
	if (corr_merge_r1_r3) free(corr_merge_r1_r3);
	if (corr_merge_r1_fx) free(corr_merge_r1_fx);
	if (corr_merge_r2_r3) free(corr_merge_r2_r3);
	if (corr_merge_r2_fx) free(corr_merge_r2_fx);
	if (corr_merge_r3_fx) free(corr_merge_r3_fx);

	// Free the ifr, expectations and variance
	if (lgm2F_ifr) free (lgm2F_ifr);
	if (r1_fwd) free (r1_fwd);
	if (r1_var) free (r1_var);
	if (r2_fwd) free (r2_fwd);
	if (r2_var) free (r2_var);
	if (r1_r2_covar) free (r1_r2_covar);
	if (lgm1F_ifr) free (lgm1F_ifr);
	if (r3_fwd) free (r3_fwd);
	if (r3_var) free (r3_var);

...
5.4500332736,0.0000000888,
4.8326046132,0.0000020596,
4.2320211100,0.0000305598,
3.6447812499,0.0003025570,
3.0681351690,0.0020620511,
2.4998404152,0.0099034612,
1.9380049059,0.0341098475,
1.3809801993,0.0853448082,
0.8272849038,0.1565389938,
0.2755464192,0.2117055699,
-0.2755464192,0.2117055699,
-0.8272849038,0.1565389938,
-1.3809801993,0.0853448082,
-1.9380049059,0.0341098475,
    ...
    double cur_time, SrtIRMTmInf *tminf)
{
  ...
  sam_get(tminf->fwd_sam,0,PHI) = tminf->rf.onef.G 
...
static double lgmcms_integrand(double dr)
{
  ...
  sam_get(NI_sam[NI_index+1],0,STATEVAR) =
    sam_get(NI_sam[NI_index],0,STATEVAR) * NI_tminf[NI_index+1].ev.onef.F/NI_tminf[NI_index].ev.onef.F 
  ...
  sam_get(NI_sam[NI_index+1],0,SHORT_RATE) = short_rate = 
    sam_get(NI_sam[NI_index+1],0,STATEVAR) + sam_get(NI_tminf[NI_index+1].fwd_sam,0,F_0_t); 
...
/* find limits of integration; note that answers here are returned
 in terms of STATEVAR, so we have to transform to get variance 1 */
  ...
  if(err)global_err = err;
  err = srt_f_lgmfindr(CMS_RTF,&NI_sam[NI_index+1], CMS_STRIKE2, &lower);
  if(err)global_err = err;

  mu = sam_get(NI_sam[NI_index],0,STATEVAR) * NI_tminf[NI_index+1].ev.onef.F/NI_tminf[NI_index].ev.onef.F 
           + NI_tminf[NI_index+1].ev.onef.F*NI_tminf[NI_index].rf.onef.G
          * (NI_tminf[NI_index+1].ev.onef.Psi - NI_tminf[NI_index].ev.onef.Psi);
  ...
  if(fabs(upper - lower) < 1.0e-6)payoff = 0.0;
  else
  {
    dx = 1.0;
    f = lgmcms_integrand;
...
SrtErr srt_f_lgmrfltr(Date start1,Date enfp1, char *freq, char *bas,  
  Date start2, Date enfp2, Date fix1, Date fix2, Date paydt, double strike,
  double strike2, double margin, SrtUndPtr und, double rate1, double rate2,
  double *answer)
{
SrtCurvePtr  crv;
...
/* find limits of integration; note that answers here are returned
 in terms of STATEVAR, so we have to transform to get variance 1 */
    ...
    if(fabs(upper - lower) < 1.0e-6)price = 0.0;
    else
    {
      f = lgmcms_integrand;
 ...
 */
    ...
    for(i=0;i<GAUHER_MAX;i++)
    {
      ...
      if(global_err) return global_err;
    }

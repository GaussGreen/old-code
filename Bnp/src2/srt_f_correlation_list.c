/* -------------------------------------------------------------------------
 	...
 	FILE NAME	: srt_f_correlation_list.c
...
/*  ----------------------------------------------------------------------
	...
	---------------------------------------------------------------------- */
static	Err sort_underlyings( int ndates, int ncorr, double ***correl, 
			int n_und, String **und_names, String **sort_und_names);
...
/* ----------------------------------------------------------------------- 
   ...
   data taken from spreadsheet...
		...
		correl is a full matrix [0..ndates-1][0..ncorr-1], which is supposed
 		to store a full term structure of correlation matrixes
   ----------------------------------------------------------------------- */
...
/* DO NOT DO THIS FOR FX STOCH RATES : corr is needed before UND initilisation 
	...
	for (i=0; i<n_und;i++)
	{
		...
		if (!und)
			return serror("Underlying %s not found in init_Corr_TermStruct !",
								sort_und_name[i]);
	}
*/
...
/* Gets today */
	...
	if (today < 0 && ndates > 1)
	{
		...
		return serror("Cannot initialise the Corr Term Structure when no underlying has been initialised yet");
	}
...
/* Loop on all dates after today */
	...
	/* Copy underlying names */
		for (i=0; i<n_und; i++)
		{
			...
			strncpy(corrval->und_names[i], sort_und_name[i],strlen(sort_und_name[i]));
	...
	/* Checks whether the correlation matrix is a real correlation matrix */
    	if (1)
		{
			err = compute_eigen_from_correl( corrval->correl, n_und,
												corrval->coeff); 
												
		}
		else
		{
			err = compute_coeff_from_correl( corrval->correl, n_und,
												corrval->coeff);
	...
	/* The pval value of the object is set to point to corrval */
		srt_f_lstins(*the_corr_list,"CorrAtom", corrval->date, OBJ_PTR_CorrelStruct,
				(void*)corrval, &srt_f_corrvalfree,&ticker);
...
/* Checks that the correlation list is properly created */
   if (!big_corr_list)
	   return serror("Global correlation list not properly initialised in make_deal_corrlist");
...
/* Checks that the main list is properly filled in */
	...
	if (!ls->element)
		return serror("Global correlation list empty in make_deal_corrlist: cannot compute correlation");
	corrval = (SrtCorrLstVal *)ls->element->val.pval;

/* Memory allocation for indexes that will be used to refer to the underlyings*/
	...
	for (i=0;i<n_und;i++)
	{
		...
		while (strcmp(corrval->und_names[j], und_names[i]) != 0)
		{
			...
			if (j >= corrval->nund)
			{
				...
				return serror("Could not find und %s in local deal corr", 
														und_names[i] );
			}
		}
		und_index[i] = j;
...
/* Duplicates the correlation information needed, passing through the list*/
	while ( ls != NULL ) 	 	
    {
		corrval = (SrtCorrLstVal *)ls->element->val.pval;
	...
	/*	memcpy(corrval1->und_names, und_names, n_und*sizeof(char*));*/
		...
		for (i=0; i<n_und; i++)
		{
			...
			{
				corrval1->correl[i][j]=
								corrval->correl[und_index[i]][und_index[j]];
	...
	/* Computes the linear coefficients used to correlate the numbers */
 		if (1)
		{
			err = compute_eigen_from_correl( corrval1->correl, n_und,
												corrval1->coeff);
		}
		else
		{
			err = compute_coeff_from_correl( corrval1->correl, n_und,
												corrval1->coeff);
		}
		if (err)
		{
	...
	/* The pval value of the object is set to point to corrval1 */
		srt_f_lstins(*deal_corr_list,"CorrAtom",corrval1->date, OBJ_PTR_CorrelStruct,
				corrval1,&srt_f_corrvalfree, &ticker);
...
/* ----------------------------------------------------------------------- 
   ...
   from tdy), according to the initialisation of the SrtCorrLstPtr correlation list 
...
/* Finds the underlying number in the corrval->und_names[] for first und */
 	...
 	while ( strcmp(und_name1, corrval->und_names[i]) != 0 )
	{
		i++;
		if ( i>= corrval->nund)
			return serror("Could not find und in correlation matrix: %s",
										und_name1);
	} 

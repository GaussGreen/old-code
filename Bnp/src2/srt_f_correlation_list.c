...
/*  ----------------------------------------------------------------------
        ...
        ---------------------------------------------------------------------- */
static Err sort_underlyings(
    int       ndates,
    int       ncorr,
    double*** correl,
    int       n_und,
    String**  und_names,
    String**  sort_und_names);
    ...
    SrtCorrLstPtr* the_corr_list)
{
    ...
    if (err = sort_underlyings(ndates, ncorr, &correl, n_und, und_names, &sort_und_name))
        return err;
    ...
    */
    ...
    if (today < 0 && ndates > 1)
    {
        ...
        return serror(
            "Cannot initialise the Corr Term Structure when no underlying has been initialised "
            "yet");
    }
    ...
    for (date_index = cur_date_ind; date_index < ndates; date_index++)
    {
        ...
        for (i = 0; i < n_und; i++)
        {
            corrval->und_names[i] = (char*)malloc((strlen(sort_und_name[i]) + 1) * sizeof(char));
    ...
    SrtCorrLstPtr* deal_corr_list)
{
    ...
    if (!ls->element)
        return serror(
            "Global correlation list empty in make_deal_corrlist: cannot compute correlation");
    ...
    while (ls != NULL)
    {
        ...
        for (i = 0; i < n_und; i++)
        {
            corrval1->und_names[i] = (char*)malloc((strlen(und_names[i]) + 1) * sizeof(char));
            if (!corrval->und_names[i])
            {
                free_ivector(und_index, 0, n_und);
                return serror("Allocation failure (5) in srt_f_init_Corr_TermStruct");
...
Err srt_f_get_corr_from_CorrList(
    SrtCorrLstPtr cls, String und_name1, String und_name2, double time, double* value)
{
    SrtLst*        ls;
    Err            err = NULL;
    int            i, j;
    ...
    ls = cls->head;
    while ((ls != NULL) && (((SrtCorrLstVal*)ls->element->val.pval)->time < time))
        ls = ls->next;
    if (ls == NULL)
        ls = cls->tail;
...
static Err sort_underlyings(
    int ndates, int ncorr, double*** correl, int n_und, String** und_names, String** sort_und_names)
{
    Err    err = NULL;
    int    i, j, k;
    double rubbish;
    ...
    for (i = 0; i < ncorr; i++)
    {
        ...
        if (strcmp(und_names[i][0], und_names[i][1]) == 0)
            return serror("Duplicate underlying name in correlation matrix: %s", und_names[i][0]);
    }
        ...
        for (j = i + 1; j < ncorr; j++)
        {
                ...
                if (strcmp(und_names[i][0], und_names[j][0]) == 0)
            {
                if (strcmp(und_names[i][1], und_names[j][1]) > 0)
                {
                    ...
                    return serror(
                        "Duplicate correlation couple in matrix :%s-%s",
                        und_names[i][0],
                        und_names[i][1]);

    ...
    CF_EXO_LEG exo_leg)
{
    ...
    while (i < cf_ncpn)
    {
        ...
        else if (fabs(cpn->alphabeta[0]) < 1.0e-08 && fabs(cpn->alphabeta[1]) < 1.0e-08)
        {
            cpn->type         = 0;
            cpn->ncms         = 0;
            cpn->alphabeta[0] = cpn->alphabeta[1] = 0.0;
        ...
        for (l = 0; l < cpn->ncms; l++)
        {
            ...
            err = interp_compounding(cmsfreq, (SrtCompounding*)&cmscpnd);
            ...
            cms->slbeta =
                interp(spread_vol_time, spread_slbeta, spread_vol_n, cms->start_time, 0, &temp);

            cms->slshift = 0;
            cms->slvol   = 0;

            err = add_tenor(cms->start_date, cmstenor, NO_BUSDAY_CONVENTION, &(cms->end_date));
            if (err)
            {
                goto FREE_RETURN;
            }

            err = swp_f_initSwapDP(cms->start_date, cms->end_date, cmsfreq, cmsbasis, &Swap);
            if (err)
            {
                goto FREE_RETURN;
            }
            ...
            cms->cpnd        = cmscpnd;
            cms->nfp =
                ((int)((cms->end_date - cms->start_date) * YEARS_IN_DAY * cms->cpnd + 0.1)) * 1.0;
            ...
            err = swp_f_ForwardRate(
                cms->start_date, cms->end_date, cmsfreq, cmsbasis, yc, "CASH", &(cms->fwd));
            ...
            if (cpn->cms_fix_date > today)
            {
                if (cms_smile && cms_adj)
                {
                    ...
                    if (use_SL)
                    {
                        if (!calib_SL)
                        {
                            /* Calibration of the shifted log parameter from the slbeta and the atm
                             * norm vol */
                            if (cms_vol_adj)
                            {
            ...
            /* Fill the option normal variance with the ATM normal vol by default for string of
             * options */
            if (use_cfoptions)
            {
                    ...
                    for (iNumOpt = 0; iNumOpt < cpn->nopt; iNumOpt++)
                    {
                        ...
                        if (cms_smile && cms_adj && cms_vol_adj)
                        {
                            ...
                            err = swp_f_Cms_Option(
                                ...
                                strike + cms->swap_spread,
                                cms->cpnd,
                                (SrtReceiverType)cpn->typeopt[iNumOpt],
                                cms->delay,
                                rconv,
                                vol_type,
                                0.0,
                            ...
                            err = srt_f_optimpvol(
                                ...
                                cms->fix_time,
                                1.0,
                                (SrtCallPutType)cpn->typeopt[iNumOpt],
                                SRT_NORMAL,
                                &(cms->optvar[iNumOpt]));
                        }
                        else
    ...
    for (i = 0; i < ccf->num_calls; i++)
    {
        ...
        if (temp)
        {
            if (call->ex_time > 1.0E-10)
            {
                ...
                {
                    temp = srt_f_optblksch(fwd, fwd, vol, call->ex_time, 1.0, SRT_CALL, PREMIUM);
                    err  = srt_f_optimpvol(
                        temp, fwd, fwd, call->ex_time, 1.0, SRT_CALL, SRT_NORMAL, &vol);
                }
            }
            else
            {
    ...
    int     is_corr)
{
    ...
    if (lam_ts)
    {
        ...
        copy_tslam = (double*)calloc(tsnlam, sizeof(double));
                        ...
                        if (und->is_corr)
                        {
                            eval_const->cf_spr_floor_std[i] = sqrt(
                                ...
                                2.0 * spread_vol * exo_cpn->alphabeta[0] * exo_cpn->alphabeta[1] *
                                    sqrt(exo_cpn->cms[0].floorvar * exo_cpn->cms[1].floorvar));
                        }
                        else
                        {
    ...
    CCF_ADI_ARG adi_arg)
{
    ...
    for (i = adi_arg->nstp - 1; i >= 0; i--)
    {
        ...
        {
            ccf_prm      = (ccf_pay_arg*)malloc(sizeof(ccf_pay_arg));
            ccf_prm->und = und;
            ccf_prm->ccf = ccf;
    ...
    CCF_MC_ARG mc_arg)
{
    ...
    if (!mc_arg->dom_fwd1 || !mc_arg->dom_fwd2 || !mc_arg->dom_phi1 || !mc_arg->dom_phi2 ||
        !mc_arg->dom_phi12 || !mc_arg->dom_exp1 || !mc_arg->dom_exp2 || !mc_arg->dom_gam1_fwd ||
        !mc_arg->dom_gam2_fwd || !mc_arg->dom_bond_pay || !mc_arg->dom_gam1_pay ||
        !mc_arg->dom_gam2_pay || !mc_arg->covar)
    {
    ...
    for (i = mc_arg->nb_dates - 1; i >= flag; i--)
    {
        ...
        err = ccf_fill_eval_const(und, ccf, ccf_prm->call_idx, &(ccf_prm->eval_const));
    ...
    CCF_ADI_ARG adi_arg)
{
    ...
    {
        err = ccf_fill_calls(und->today, eod_ex_flag, ncall, pay_rec, ex_date, set_date, fee, ccf);
        if (err)
        {
            goto FREE_RETURN;
        }
    ...
    {
        mc_arg = (ccf_mc_arg*)calloc(1, sizeof(ccf_mc_arg));
        ...
        probas    = (double*)calloc(ccf->num_calls, sizeof(double));
        exe_bound = (double*)calloc(ccf->num_calls, sizeof(double));
...
/*	Free all structures */
Err ccf_free_all_struct(CCF_STR ccf, CCF_UND und, int call_feat, CCF_ADI_ARG adi_arg)
{
    ccf_free_und(und);
    ccf_free_calls(ccf);
...
{
    int     i, j, k, index;
    double *time = NULL, *x = NULL, *w = NULL, *phi1 = NULL, *phi2 = NULL, *phi12 = NULL,
           *r1 = NULL, **r2 = NULL, *r3 = NULL, ***pv = NULL;
...
static double cms_rate(double delay, double nfp, double compd, double forward, double nvar)
{
    ...
    if (fabs(forward) > 1.0E-08)
    {
        ...
        dlvl = (nfp / compd) * pow(1.0 + forward / compd, -nfp - 1) * forward / compd -
    ...
    double*** prod_val)
{
    ...
    double fund_leg_pv, df, df_start, df_end, level, forward, cmsrate[2], spread, coupon, floor,
        cap, exo_leg_pv, iv;
    ...
    for (i = l1; i <= u1; i++)
    {
        ...
        for (j = l2; j <= u2; j++)
        {
            ...
            /*	Notional */
            fund_leg_pv -=
                eval_const->df[eval_const->fund_idx[num_fund_cpn - 1]] * fund_leg->notional;
            ...
            for (l = 0; l < num_cf_cpn; l++)
            {
                ...
                case 3:
                    /*	CMS Spread */
                    spread =
                        exo_cpn->alphabeta[0] * cmsrate[0] + exo_cpn->alphabeta[1] * cmsrate[1];
                    coupon = spread + exo_cpn->gamma;
                    floor  = OPT_VAL_MACRO_N(
                        eval_const->cf_spr_floor_type[l],
                        spread,
                        ...
                        for (iNumOpt = 0; iNumOpt < exo_cpn->nopt; iNumOpt++)
                        {
                            if (exo_cpn->notopt[iNumOpt] != 0)
                            {
                                err = OptSpreadNew(
                                    ...
                                    mat,                                                 /* mat,  */
                                    eval_const->cf_spr_slcorr[l],                        /* rho,  */
                                    (SrtCallPutType)exo_cpn->typeopt[iNumOpt],
                                    &optionvalue);
    ...
    for (l = 0; l < num_cf_cpn; l++)
    {
        ...
        for (m = 0; m < exo_cpn->ncms; m++)
        {
            ...
            if (eval_const->cf_cms_vol[m][l] > 1.0E-08)
            {
                ...
                case 1:
                    cmsrate[m] = cms_rate(
                        cms->delay, cms->nfp, cms->cpnd, forward, eval_const->cf_cms_vol[m][l]);
                    break;
                ...
                case 2:
                    ...
                    {
                        cmsrate[m] =
                            forward + cms->lambda * pow(fabs(forward), 2.0 * cms->beta) * mat;
                    }
                    else
                    {
                        cmsrate[m] = forward + cms->lambda * mat;
        ...
        else
        {
            ...
            if (!exo_cpn->use_SL)
            {
                for (iNumOpt = 0; iNumOpt < exo_cpn->nopt; iNumOpt++)
                {
                    if (exo_cpn->notopt[iNumOpt] != 0)
                    {
                        floor += exo_cpn->notopt[iNumOpt] *
                                 OPT_VAL_MACRO_N(
                                     (exo_cpn->typeopt[iNumOpt] == SRT_CALL
                                          ? (eval_const->cf_spr_opt_std[l][iNumOpt] == 0 ? 1 : 3)
                                          : (eval_const->cf_spr_opt_std[l][iNumOpt] == 0 ? 2 : 4)),
                                     spread,
                                     exo_cpn->strikeopt[iNumOpt],
                                     eval_const->cf_spr_opt_std[l][iNumOpt]);
                    }
    ...
    double* exe_bound)
{
    ...
    optimise = (int*)calloc(mc_arg->nb_dates, sizeof(int));

...
/*	Needed to calculate ATM (converging) normal volatility associated to CMS coupons */
char		*yc,							/*	yc */
char		*vc,							/*	vc */
char		*ref,							/*	ref rate */
...
CF_EXO_LEG	exo_leg)
{
	...
	while (i < cf_ncpn)
	{
		...
		libor = (1.0 / swp_f_df (max (cpn->start_date, today), cpn->pay_date, yc) - 1.0) / cpn->cvg;
		...
		if (fabs (cpn->alphabeta[0]) < 1.0e-08 && fabs (cpn->alphabeta[1]) < 1.0e-08)
		{
			cpn->type = 0;
			cpn->ncms = 0;
			cpn->alphabeta[0] = cpn->alphabeta[1] = 0.0;
		...
		for (l = 0; l < cpn->ncms; l++)
		{
			...
			cms->lambda = 0.0;
			cms->beta = cmsbeta;
			cms->start_date = add_unit (cpn->cms_fix_date, 2, SRT_BDAY, MODIFIED_SUCCEEDING);
			cms->start_time = (cms->start_date - today) * YEARS_IN_DAY;

			/* Default value for shifted log model */
			cms->slbeta = interp(spread_vol_time,
								spread_slbeta,
								spread_vol_n,
								cms->start_time,
								0,
			...
			for (k=0; k<cms->num_cpn; k++)
			{
				...
				cms->cvg[k] = coverage (SwapDates.date[k], SwapDates.date[k+1], cmsbas);
			}
			...
			cms->cpnd = cmscpnd;
			cms->nfp = ((int) ((cms->end_date - cms->start_date) * YEARS_IN_DAY * cms->cpnd + 0.1)) * 1.0;
			...
			if (cpn->cms_fix_date > today)
			{
				if (cms_smile && cms_adj)
				{
					...
					else
					{
						...
						if (temp)
						{
							if (cms->fwd < 1e-10) cms->atmvar = 1e-6;
							else
							{
					...
					if (use_SL)
					{
						 if (!calib_SL)
						 {
							 /* Calibration of the shifted log parameter from the slbeta and the atm norm vol */
							 if (cms_vol_adj)
							 {
							...
							/* Calibration of the shifted log model  */ 
							 ...
							 if (cms_vol_adj)
							 {
								/* Calibration to cmsoptions */
								 ...
								 dStrike1 = cms->fwd_cms - NbStdcalib_SL * cms->atmvar * sqrt(cms->fix_time);
								 dStrike2 = cms->fwd_cms + NbStdcalib_SL * cms->atmvar * sqrt(cms->fix_time);
								...
								/* Find the shifted log params */
								 err = find_shifted_log_params(	cms->fix_time,
												cms->fwd_cms,
												dStrike1,
												dLogVol1,
												dStrike2,
						 ...
						 else
						 {
								/* Calibration to swaptions */
								 ...
								 /* calibration of the slope */
								 dStrike1 = forward - NbStdcalib_SL * cms->atmvar * sqrt(cms->fix_time);
								 dStrike2 = forward + NbStdcalib_SL * cms->atmvar * sqrt(cms->fix_time);
						...
						cms->lambda = (cms->fwd_cms - forward) 
								/ (forward + cms->slshift) / (forward + cms->slshift) / cpn->cms_fix_time;
					...
					else
					{
						...
						{
							cms->lambda = (cms->fwd_cms - forward) 
								/ pow (fabs (forward), 2.0 * cms->beta) / cpn->cms_fix_time;
						}
						else
						{
			...
			/* Fill the option normal variance with the ATM normal vol by default for string of options */
			if (use_cfoptions)
			{
		...
		{
			cms = &(cpn->cms[0]);
			if (fabs (cms->nfp - 1.0) < 1.0e-08 && cms->end_time < cpn->pay_time + ONE_MONTH)
			{
				cpn->type = 1;
				cms->adj_type = 0;
			}
			...
			if (cpn->cms_fix_date > today)
			{
				...
				{
					strike = (cpn->floor - cpn->gamma) / cpn->alphabeta[0] - cms->swap_spread;
					...
					{						
						...
						err = swp_f_Cms_Option (cms->fwd + cms->swap_spread,
												cpn->cms_fix_time,
												cms->nfp,
												strike + cms->swap_spread,
												cms->cpnd, 
												SRT_RECEIVER, 
												cms->delay,	
												rconv, 
					...
					else
					{
						...
						if (temp)
						{
							if (cms->fwd < 1e-10) cms->floorvar = 1e-6;
							else
							{
								temp = srt_f_optblksch(
									cms->fwd, 
				...
				{
					strike = (cpn->cap - cpn->gamma) / cpn->alphabeta[0] - cms->swap_spread;
					...
					else
					{
						...
						if (temp)
						{
							if (cms->fwd < 1e-10) cms->capvar = 1e-6;
							else
							{
								temp = srt_f_optblksch(
									cms->fwd, 
				...
				if (use_cfoptions)
				{
					...
					{
						/* Cash strike */
						strike = cpn->strikeopt[iNumOpt] / cpn->alphabeta[0] - cms->swap_spread;
						...
						else
						{
							...
							if (temp)
							{
								if (cms->fwd < 1e-10) cms->optvar[iNumOpt] = 1e-6;
								else
								{
									temp = srt_f_optblksch(
										cms->fwd, 
...
CF_EXO_LEG	exo_leg)
{
	...
	for (i=0; i<exo_leg->num_cpn; i++)
	{
		...
		for (k=0; k<cpn->ncms; k++)
		{
			...
			if(
				cms->end_date < cms->cpn_pay_date[cms->num_cpn-1]
			||	cms->start_date > cms->cpn_pay_date[0])
			{
				return "Pay dates should be between start and end dates in CMS reference swaps";
			}
		}
	}
					...
					CCF_STR		ccf)
{
	...
	for (i=0; i<ccf->num_calls; i++)
	{
		...
		if (	ccf->fund_leg->cpn[ccf->call[i].fund_idx].start_date < ccf->call[i].ex_date
			||	ccf->fund_leg->cpn[ccf->call[i].fund_idx].pay_date < ccf->call[i].ex_date)
		{
			return "A funding coupon starts before its exercise date";
		}
	
		if (	ccf->cf_leg->cpn[ccf->call[i].cf_idx].start_date < ccf->call[i].ex_date
			||	ccf->cf_leg->cpn[ccf->call[i].cf_idx].pay_date < ccf->call[i].ex_date
			||	ccf->cf_leg->cpn[ccf->call[i].cf_idx].cms_fix_date < ccf->call[i].ex_date)
		{
...
Err				err					= NULL;
	...
	if (spread_vol_n > 0)
	{
		...
		if (!und->spread_vol_time || !und->spread_vol_floor || !und->spread_vol_cap)
		{
			err = "Allocation error (1) in ccf_fill_und";
			goto FREE_RETURN;
		}

		memcpy (und->spread_vol_time, spread_vol_time, spread_vol_n * sizeof (double));
		memcpy (und->spread_vol_floor, spread_vol_floor, spread_vol_n * sizeof (double));
	...
	/*	Get term structures */
	err = Get_LGM2F_TermStructure2(	lgm2dund,
									&(und -> sigma),
									&(und -> sigma_time),									
									&(und -> sigma_n),
									&(und -> lambda),
	...
	{
		und->sigma_date[i] = und->today + und->sigma_time[i] * DAYS_IN_YEAR + 1.0e-08;
	}

	for (i=0; i<und->lambda_n; i++)
	{
		und->lambda_date[i] = und->today + und->lambda_time[i] * DAYS_IN_YEAR + 1.0e-08;
	}
...
int			*strike_type)
{
	...
	for (i=0; i<ccf->num_calls; i++)
	{
		...
		for (j=0; j<call->num_fund_cpn; j++)
		{
			...
			pvk2 += fundki[k] * fund_cpn->cvg * swp_f_df (today, fund_cpn->pay_date, yc);
		...
		for (j=0; j<call->num_cf_cpn; j++)
		{
			...
			pvlib += bi[k] 
				* (swp_f_df (today, exo_cpn->start_date, yc) - swp_f_df (today, exo_cpn->pay_date, yc));
		...
		err = swp_f_ForwardRate(
				add_unit (call->ex_date, 2, SRT_BDAY, MODIFIED_SUCCEEDING),
				end_date,
				swap_freq,
				swap_basis,
				yc,
...
Err				err					= NULL;
	...
	if (lam_ts)
	{
		for (i=0; i<tsnlam; i++)
		{
			if (tslamtime[i] <= 0.0)
			{
				err = "Past lambda times are forbidden - please check your tau term struct";
				goto FREE_RETURN;
			}
		}
	...
	if (ccf->num_calls > 0 && !(ccf->num_calls == 1 && ccf->call[0].ex_date <= today + eod_flag))
	{
	...
	{
		und->sigma_n = 0;
		if (und->sigma_time) free(und->sigma_time);
		und->sigma_time = NULL;
		if (und->sigma) free(und->sigma);
		und->sigma = NULL;
	...
	if (lam_ts)
	{
		for (i=0; i<und->lambda_n; i++)
		{
			...
			und->lambda_date[i] = today + und->lambda_time[i] * DAYS_IN_YEAR + 1.0e-08;
...
{	
	...
	if (src->lambda_date)
	{
		...
		memcpy (dest->lambda_date, src->lambda_date, dest->lambda_n * sizeof (double));
	}
	else
	{
	...
	if (src->lambda_time)
	{
		...
		memcpy (dest->lambda_time, src->lambda_time, dest->lambda_n * sizeof (double));
	}
	else
	{
	...
	{
		dest->spread_vol_time = (double*) calloc (dest->spread_vol_n, sizeof (double));
		memcpy (dest->spread_vol_time, src->spread_vol_time, dest->spread_vol_n * sizeof (double));
	}
	else
	{
	...
	{
		dest->spread_vol_floor = (double*) calloc (dest->spread_vol_n, sizeof (double));
		memcpy (dest->spread_vol_floor, src->spread_vol_floor, dest->spread_vol_n * sizeof (double));
	}
	else
	{
	...
	if (src->spread_vol_cap)
	{
		...
		memcpy (dest->spread_vol_cap, src->spread_vol_cap, dest->spread_vol_n * sizeof (double));
	}
	else
	{
	...
	if (src->has_fwd_iv)
	{
		...
		memcpy (dest->exercise_date, src->exercise_date, dest->nb_fwdiv * sizeof (double));

		dest->market_fwdiv = (double *) calloc(dest->nb_fwdiv, sizeof(double));
		memcpy (dest->market_fwdiv, src->market_fwdiv, dest->nb_fwdiv * sizeof (double));
		
		dest->model_fwdiv = (double *) calloc(dest->nb_fwdiv, sizeof(double));
		memcpy (dest->model_fwdiv, src->model_fwdiv, dest->nb_fwdiv * sizeof (double));
...
CCF_UND		und)
{
	if (und->sigma_date) free (und->sigma_date);
	if (und->sigma_time) free (und->sigma_time);
	if (und->sigma) free (und->sigma);
	if (und->lambda_date) free (und->lambda_date);
	if (und->lambda_time) free (und->lambda_time);
	if (und->lambda) free (und->lambda);
	if (und->spread_vol_time) free (und->spread_vol_time);
	if (und->spread_vol_floor) free (und->spread_vol_floor);
	if (und->spread_vol_cap) free (und->spread_vol_cap);
	...
	if (und->has_fwd_iv)
	{
		if (und->market_fwdiv) free (und->market_fwdiv);
		if (und->model_fwdiv) free (und->model_fwdiv);
		if (und->extra_fees) free (und->extra_fees);
		if (und->exercise_date) free (und->exercise_date);
						...
						CCF_EVAL_CONST	eval_const)
{
	...
	eval_const->df_mat[eval_const->num_df] = fund_leg->cpn[call->fund_idx].start_time - evt_time;
	(eval_const->num_df)++;
	...
	if (call->num_cf_cpn > 0)
	{
		...
		for (i=0; i<call->num_cf_cpn; i++)
		{
			...
			for (l=0; l<exo_cpn->ncms; l++)
			{
				...
				for (k=0; k<cms->num_cpn; k++)
				{
					eval_const->df_mat[eval_const->num_df] = cms->cpn_pay_time[k] - evt_time;
					(eval_const->num_df)++;
				}
			}
		}
	...
	for (i=0; i<eval_const->num_df; i++)
	{
		...
		if (i > 0)
		{
			...
			eval_const->df_gamma[i] = eval_const->df_gamma[i-1] + exp(-fact2) * gam2;
		}
		else
		{
	...
	while (eval_const->df_mat[tmpidx] < temp_time - 1.0e-08) tmpidx++;
		...
		for (i=0; i<call->num_cf_cpn; i++)
		{
			...
			for (l=0; l<exo_cpn->ncms; l++)
			{
				...
				for (k=0; k<cms->num_cpn; k++)
				{
					...
					while (eval_const->df_mat[tmpidx2] < temp_time - 1.0e-08) tmpidx2++;
	...
	if (eval_const->do_cf_disc && eval_const->do_cf_fwd)
	{
			...
			if (!exo_cpn->use_cfoptions && !exo_cpn->use_SL)
			{
				...
				case 3:
					...
					if (temp_time > 1.0E-08)
					{
						...
						if (und->is_corr)
						{
							eval_const->cf_spr_floor_std[i] = sqrt(
		...
		+ 2.0 * spread_vol * exo_cpn->alphabeta[0] * exo_cpn->alphabeta[1] * sqrt (exo_cpn->cms[0].floorvar * exo_cpn->cms[1].floorvar));
						}
						else
						{
		...
		+ 2.0 * spread_vol * exo_cpn->alphabeta[0] * exo_cpn->alphabeta[1] * sqrt (exo_cpn->cms[0].capvar * exo_cpn->cms[1].capvar));
						}
						else
						{
					...
					else
					{
						eval_const->cf_cms_vol[0][i] = eval_const->cf_cms_vol[1][i]
								= eval_const->cf_spr_floor_std[i] = eval_const->cf_spr_cap_std[i] = 0.0;
					}
			...
			else
			{
				if (exo_cpn->use_SL)
				{
					if (!exo_cpn->use_cfoptions)
					{
						...
						if (temp_time > 1.0E-08)
						{
							...
							eval_const->cf_spr_slcorr[i] = interp(
																	und->spread_vol_time,
																	und->spread_vol_floor,
																	und->spread_vol_n,
																	sv_time,
				...
				else
				{
					...
					if (temp_time > 1.0E-08)
					{
						...
						{
							eval_const->cf_spr_opt_std[i][iNumOpt] = sqrt( temp_time *
								...
								+ 2.0 * spread_vol * exo_cpn->alphabeta[0] * exo_cpn->alphabeta[1] * 
						...
						CCF_ADI_ARG		adi_arg)
{
	...
	{
		adi_arg->ifr[i] = swp_f_zr (adi_arg->date[i], adi_arg->date[i+1], adi_arg->yc);
	}
						...
						CCF_MC_ARG		mc_arg)
{
	...
	if (!mc_arg->dom_fwd1 || !mc_arg->dom_fwd2 || !mc_arg->dom_phi1 || !mc_arg->dom_phi2 || !mc_arg->dom_phi12 || !mc_arg->dom_exp1 || !mc_arg->dom_exp2
		|| !mc_arg->dom_gam1_fwd || !mc_arg->dom_gam2_fwd || !mc_arg->dom_bond_pay || !mc_arg->dom_gam1_pay ||!mc_arg->dom_gam2_pay || !mc_arg->covar)
	{
...
FREE_RETURN:
	...
	if (err)
	{
		...
		// Free new structures
	if (new_time) free(new_time);
	if (new_lambda) free(new_lambda);
	if (new_lambda2) free(new_lambda2);
	if (new_sigma) free(new_sigma);
					...
					CCF_MC_ARG	mc_arg)
{
	...
	if (mc_arg)
	{
		...
		if (mc_arg->dom_fwd1) free (mc_arg->dom_fwd1);
		if (mc_arg->dom_fwd2) free (mc_arg->dom_fwd2);
		if (mc_arg->dom_exp1) free (mc_arg->dom_exp1);
		if (mc_arg->dom_exp2) free (mc_arg->dom_exp2);
		if (mc_arg->dom_phi1) free (mc_arg->dom_phi1);
		if (mc_arg->dom_phi2) free (mc_arg->dom_phi2);
		if (mc_arg->dom_phi12) free (mc_arg->dom_phi12);
		if (mc_arg->dom_gam1_fwd) free (mc_arg->dom_gam1_fwd);
		if (mc_arg->dom_gam2_fwd) free (mc_arg->dom_gam2_fwd);
		if (mc_arg->dom_bond_pay) free (mc_arg->dom_bond_pay);
		if (mc_arg->dom_gam1_pay) free (mc_arg->dom_gam1_pay);
		if (mc_arg->dom_gam2_pay) free (mc_arg->dom_gam2_pay);	

		if (mc_arg->covar) free_f3tensor(mc_arg->covar, 0, mc_arg->nb_dates -1, 0, 1, 0, 1);
...
CCF_ADI_ARG	adi_arg)								
{
	...
	if (!fix_lambda && proba_weight && ccf->num_calls > 0 && ccf->call && use_calib)
	{
...
FREE_RETURN:
	...
	if (probas) free (probas);
	if (exe_bound) free (exe_bound);
...
Err		err = NULL;
	...
	if (!time || !x || !w || !phi1 || !phi2 || !phi12 || !r1 || !r2 || !r3 || !pv)
	{
	...
	/* Phi Calculation */
	err = LGM2FCalcPhi(	ccf->num_calls+1, 
						time,
						und->sigma,
						und->sigma_time,
						und->sigma_n,
	...
	for (i=0; i<ccf->num_calls; i++)
	{
		...
		/* r3 = r2 + coef * r1 */
		std3 = sqrt(phi2[i+1] + coef * coef * phi1[i+1] + 2.0 * und->rho * coef * std1 * std2);
		...
		{
			err = ccf_payoff_4_3dfx_adi_sl(call->ex_date,
									call->ex_time,
		...
		{
			err = ccf_payoff_4_3dfx_adi(call->ex_date,
										call->ex_time,
										(adi_arg->void_prm)[index],
										und->yc,
										und->lambda,
...
FREE_RETURN:
	...
	if (x) free_dvector (x, 1, num_hermite);
	if (w) free_dvector (w, 1, num_hermite);

	if (phi1) free (phi1);
	if (phi2) free (phi2);
	if (phi12) free (phi12);

	if (r1) free_dvector(r1, 0, num_hermite - 1);
	if (r2) free_dmatrix(r2, 0, num_hermite - 1, 0, num_hermite - 1);
	if (r3) free_dvector(r3, 0, num_hermite - 1);

	if (pv) free_f3tensor(pv, 0, num_hermite - 1, 0, num_hermite - 1, 0, 0);
							...
							double		***prod_val)
{
	...
	{	
		...
		{		
			...
			{
				eval_const->df[l] = exp(
					-eval_const->df_alpha[l] - eval_const->df_beta[l] * R1 - eval_const->df_gamma[l] * R2);
			}
			...
			/*	Notional */
			fund_leg_pv -= eval_const->df[eval_const->fund_idx[num_fund_cpn-1]] * fund_leg->notional;
			...
			for (l=0; l<num_cf_cpn; l++)
			{
				...
				case 2:
					...
					floor = OPT_VAL_MACRO_N(
						eval_const->cf_spr_floor_type[l], 
						spread, 
						eval_const->cf_spr_floor_str[l], 
						eval_const->cf_spr_floor_std[l]);
					cap = OPT_VAL_MACRO_N(
						eval_const->cf_spr_cap_type[l], 
						spread, 
				...
				case 3:
					/*	CMS Spread */
					spread = exo_cpn->alphabeta[0] * cmsrate[0] + exo_cpn->alphabeta[1] * cmsrate[1];
					coupon = spread + exo_cpn->gamma;
					floor = OPT_VAL_MACRO_N(
						eval_const->cf_spr_floor_type[l], 
						spread, 
					...
					if (eval_const->cf_cms_vol[m][l] > 1.0E-08)
					{
						switch (cms->adj_type)
						{
							...
							case 2:
								...
								{
									cmsrate[m] = forward + 
										cms->lambda * (forward + cms->slshift) * (forward + cms->slshift) * mat;
								}
								else
								{
									/* Beta adjustement */
				...
				{	
					...
					else
					{
						err = OptSpreadNew(
								...
								(eval_const->cf_spr_floor_type[l]>2 ? exo_cpn->cms[0].slvol : 0.0),		/* sigx, */
								cmsrate[1] + exo_cpn->cms[1].slshift,									/* fwdy, */
								exo_cpn->alphabeta[1],													/* ny,   */
								(eval_const->cf_spr_floor_type[l]>2 ? exo_cpn->cms[1].slvol : 0.0),														/* sigy, */
								eval_const->cf_spr_floor_str[l] 
									+ exo_cpn->cms[0].slshift * exo_cpn->alphabeta[0] 
									+ exo_cpn->cms[1].slshift * exo_cpn->alphabeta[1],					/* K,	 */
								mat,																	/* mat,  */
					...
					else
					{
						err = OptSpreadNew(
								cmsrate[0] + exo_cpn->cms[0].slshift,										/* fwd x */
								exo_cpn->alphabeta[0],														/*  nx,  */
								(eval_const->cf_spr_cap_type[l]>2 ? exo_cpn->cms[0].slvol : 0.0),			/* sigx, */
								cmsrate[1] + exo_cpn->cms[1].slshift,										/* fwdy, */
								exo_cpn->alphabeta[1],														/* ny,   */
								(eval_const->cf_spr_cap_type[l]>2 ? exo_cpn->cms[1].slvol : 0.0),			/* sigy, */
								eval_const->cf_spr_cap_str[l] 
									+ exo_cpn->cms[0].slshift * exo_cpn->alphabeta[0] 
									+ exo_cpn->cms[1].slshift * exo_cpn->alphabeta[1],						/* K,	 */
						...
						for (iNumOpt=0; iNumOpt<exo_cpn->nopt; iNumOpt++)
						{
							if (exo_cpn->notopt[iNumOpt] != 0)
							{
								floor += exo_cpn->notopt[iNumOpt]  * OPT_VAL_MACRO_N(
																				(exo_cpn->typeopt[iNumOpt] == SRT_CALL ? (eval_const->cf_spr_opt_std[l][iNumOpt] == 0 ? 1 : 3) : (eval_const->cf_spr_opt_std[l][iNumOpt] == 0 ? 2 : 4)),
																				spread,
...
FREE_RETURN:
	...
	if (optimise) free(optimise);
	if (prod_val) free_dmatrix (prod_val, 0, nb_row - 1, 0, 3);

	...
	for (i=0; i<n1; i++)
	{
		...
		for (j=0; j<n2; j++)
		{
			...
			for (k=0; k<n3; k++)
			{
				...
				local->smp.und[total->fx_idx].sv[SPOT] = exp (coef_A + temp_X * (coef_B + temp_X * coef_C)
...
FREE_RETURN:
	...
	if (dom_const) free_dvector (dom_const, 0, total->num_dom_df-1);
	if (dom_lin) free_dvector (dom_lin, 0, total->num_dom_df-1);
	if (fx_const) free_dvector (fx_const, 0, total->num_fx_df-1);
	if (fx_lin) free_dvector (fx_lin, 0, total->num_fx_df-1);
	if (for_const) free_dvector (for_const, 0, total->num_for_df-1);
	if (for_lin) free_dvector (for_lin, 0, total->num_for_df-1);

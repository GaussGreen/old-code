...
Err free_resetableperiod(double **paramperiod, double *periodmat,
                         ...
                         double nfixdays, double *driftint) {
  if (paramperiod)
    free_dmatrix(paramperiod, 1, (long)nfixdays, 1, 5);
  paramperiod = NULL;
  ...
  if (driftint)
    free_dvector(driftint, 1, (long)nfixdays);
  isfriday = NULL;
...
Err get_fra(long Fradate, int spotlag, char *cRefRateCode,
  ...
  err = swp_f_ForwardRate_SwapDP(&sdpFra, szYieldCurveName, cRefRateCode, dFra);
  if (err)
    return err;
...
Err calibtosmile(long today, long dDate, char *cMarketId, char *szVolCurveName,
  ...
  err = optcalibmerton(dFra[0], NMAXSTRIKES, strikes, vols, param, mat,
...

{
  ...
  err = calibtosmile(today, (long)resetdates[n_resetdates], cMarketId,
                     szVolCurveName, cRefRateCode, szYieldCurveName,
                     paramatrix[n_resetdates], &dFra, freeze);
  ...
  for (j = n_resetdates - 1; j >= 1; j--) {
    ...
    paramatrix[j][3] = paramatrix[j + 1][3] / (1 + paramatrix[j + 1][2] * cvg *
                                                       dFra / (1 + cvg * dFra));
    paramatrix[j][4] = paraminitguess[4];
    paramatrix[j][5] = paramatrix[j + 1][5] / (1 + paramatrix[j + 1][4] * cvg *
                                                       dFra / (1 + cvg * dFra));

    err = calibtosmile(today, (long)resetdates[j], cMarketId, szVolCurveName,
                       cRefRateCode, szYieldCurveName, paramatrix[j], &dFra,
                       freeze);
  }
...
Err get_DRSts(long today, long Paydate, int nfixdays, double *periodmat,
  ...
  for (index = 1; index <= nfixdays; index++) {
    ...
    FraSpot = add_unit((long)periodmat[index], spotlag, SRT_BDAY,
                       MODIFIED_SUCCEEDING);
    ...
    err = get_fra((long)periodmat[index], spotlag, cRefRateCode,
                  szYieldCurveName, &dFra);
...
Err srt_f_resetable(int n_resetdates, double *resetdates, double *initparam,
  ...
  /* gets the weights and points form a Hermite Polynomial for the numerical
   * integration made further down*/
  err = HermiteStandard(points, weights, 2 * nherm);
  if (err) {
    free_resetablegeneral(weights, points, wn1n2, paramatrix, nherm,
                          n_resetdates);
    return err;
  }
  ...
  for (perindex = 1; perindex < n_resetdates; perindex++) {
    ...
    cvg = coverage((long)resetdates[perindex + 1],
                   add_unit((long)resetdates[perindex + 1],
                            ...
                            MODIFIED_SUCCEEDING),
                   float_basis);
    /* here        , we get the parameters corresponding to each fixing day
     within the period and put them in paramperiod which is a matrix with
     nfixdays rows and 5 columns (for the 5 parameters)*/
    ...
    N1max = (int)(paramperiod[1][3] * resetmat +
                  10 * sqrt(paramperiod[1][3] * resetmat));
    N2max = (int)(paramperiod[1][5] * resetmat +
                  10 * sqrt(paramperiod[1][5] * resetmat));

    for (n1 = 0; n1 <= N1max; n1++) {
      for (n2 = 0; n2 <= N2max; n2++) {

        probajumps =
            exp(-paramperiod[1][3] * resetmat - paramperiod[1][5] * resetmat +
                n1 * log(paramperiod[1][3] * resetmat) +
                n2 * log(paramperiod[1][5] * resetmat)) /
            (fact(n1) * fact(n2));
        ...
        if (probajumps > probainf) {
          ...
          for (n = 1; n <= 2 * nherm; n++) {
            ...
            err = DRSCalc(wn1n2, (int)nfixdays, resetmat, paramperiod,
                          driftinterp, DRSts);
            ...
            err = NewtonJumps((int)nfixdays, DRSts, periodmat, isfriday,
                              paramperiod, width, callspread, &eps, &prix);
            ...
            if (strcmp(FloatCoupon, "YES") == 0) {
              pv[perindex] += weights[n] * probajumps * (DRSts[1] + margin) *
                              (-prix) / ntotaldays;
            }

            else {
              pv[perindex] +=
                  weights[n] * probajumps * (margin) * (-prix) / ntotaldays;
            }
          }
        }
      }
    ...
    /*
    discfact[perindex] = swp_f_df(today        ,resetdates[perindex+1]
          ,szYieldCurveName);
...
Err get_atm_vols(int nfixings, double *DRSts, double *Spotdates,
                 double resetmat, char *cRefRateCode, char *szVolCurveName,
                 char *szYieldCurveName, double *atmvols) {
  Err err = NULL;
  int i;
  long EndDate;
  double lognorm = 1.0;
  ...
  /*Get details for the index Fra*/
  err = swp_f_get_ref_rate_details(cRefRateCode, &float_basis,
                                   &float_compounding);
  if (err) {
    return err;
  }
  ...
  for (i = 1; i <= nfixings; i++) {
    EndDate = add_unit((long)Spotdates[i], 12 / float_compounding, SRT_MONTH,
                       MODIFIED_SUCCEEDING);
    err = swp_f_vol(szVolCurveName, (double)Spotdates[i], (double)EndDate,
                    DRSts[i], &(atmvols[i]), &lognorm);
  }

  return err;
}

Err get_atm_normal_vols(int nfixings, double *DRSts, double *Spotdates,
                        char *cRefRateCode, char *szVolCurveName,
                        double *atmvols) {
  Err err = NULL;
  int i;
  long EndDate;
...
Err SimulateDRS(double w, int nfixings, double resetmat, double *vols,
                double *DRSts) {
  Err err = NULL;
  int i;

  for (i = 1; i <= nfixings; i++) {
    DRSts[i] = DRSts[i] * exp(vols[i] * w * sqrt(resetmat) -
                              vols[i] * vols[i] * resetmat / 2);
  }

  return err;
}

double Compute_Price(int nfixings, double *DRSts, double width,
                     double lowerbarrier, double callspread, long spotdate,
                     double *periodmat, double *isfriday, char *FloatCoupon,
                     double margin, SrtCompounding frequency,
                     SrtCompounding ResetFrequency, char *szVolCurveName,
                     char *szYieldCurveName, char *cRefRateCode, char *VolType,
                     double *VolsInputs, double VolShift) {
  int i, j;
  double *corridorvols;
  double lognorm;
  long EndDate;
  ...
  for (i = 1; i < frequencyratio + 1; i++) {
    dates[i + 1] =
        add_unit(dates[i], 12 / frequency, SRT_MONTH, MODIFIED_SUCCEEDING);
  ...
  for (i = 1; i <= nfixings; i++) {
    ...
    EndDate =
        add_unit(periodspot, 12 / frequency, SRT_MONTH, MODIFIED_SUCCEEDING);

    if (strcmp(VolType, "Flat") == 0) {

      swp_f_vol(szVolCurveName, periodspot, EndDate, lowerbarrier - callspread,
                &(corridorvols[1]), &lognorm);

      swp_f_vol(szVolCurveName, periodspot, EndDate, lowerbarrier,
                &(corridorvols[2]), &lognorm);

      swp_f_vol(szVolCurveName, periodspot, EndDate, lowerbarrier + width,
                &(corridorvols[3]), &lognorm);

      swp_f_vol(szVolCurveName, periodspot, EndDate,
                lowerbarrier + width + callspread, &(corridorvols[4]),
                &lognorm);
    }
...

    {
      ...
      enddate = add_unit((long)(spotdate + periodmat[i] - periodmat[1]),
                         12 / frequency, SRT_MONTH, MODIFIED_SUCCEEDING);

      get_fra(add_unit(startdate, -spotlag, SRT_BDAY, MODIFIED_SUCCEEDING),
              spotlag, cRefRateCode, szYieldCurveName, &Fwd);

      swp_f_vol(szVolCurveName, startdate, enddate, Fwd, &(ATMVol), &lognorm);

      stdevoriginal =
          ATMVol * Fwd *
          sqrt((add_unit(startdate, -spotlag, SRT_BDAY, MODIFIED_SUCCEEDING) -
                today) /
               365.25);
      stdevactual =
          ATMVol * DRSts[i] *
      ...
      swp_f_vol(szVolCurveName, startdate, enddate,
                ((lowerbarrier - callspread) - DRSts[i]) * stdevoriginal /
                        stdevactual +
                    Fwd,
                &(corridorvols[1]), &lognorm);

      swp_f_vol(szVolCurveName, startdate, enddate,
                (lowerbarrier - DRSts[i]) * stdevoriginal / stdevactual + Fwd,
                &(corridorvols[2]), &lognorm);

      swp_f_vol(szVolCurveName, startdate, enddate,
                ((lowerbarrier + width) - DRSts[i]) * stdevoriginal /
                        stdevactual +
                    Fwd,
                &(corridorvols[3]), &lognorm);

      swp_f_vol(szVolCurveName, startdate, enddate,
                ((lowerbarrier + width + callspread) - DRSts[i]) *
                        stdevoriginal / stdevactual +
                    Fwd,
                &(corridorvols[4]), &lognorm);
    }
    ...
    else /* NormSliding*/
    {
      ...
      swp_f_vol(szVolCurveName, startdate, enddate,
                ((lowerbarrier - callspread) - DRSts[i]) + Fwd,
                &(corridorvols[1]), &lognorm);

      corridorvols[1] = NormalVol * corridorvols[1] / (DRSts[i] * ATMVol);

      swp_f_vol(szVolCurveName, startdate, enddate,
                (lowerbarrier - DRSts[i]) + Fwd, &(corridorvols[2]), &lognorm);

      corridorvols[2] = NormalVol * corridorvols[2] / (DRSts[i] * ATMVol);

      swp_f_vol(szVolCurveName, startdate, enddate,
                ((lowerbarrier + width) - DRSts[i]) + Fwd, &(corridorvols[3]),
                &lognorm);

      corridorvols[3] = NormalVol * corridorvols[3] / (DRSts[i] * ATMVol);

      swp_f_vol(szVolCurveName, startdate, enddate,
                ((lowerbarrier + width + callspread) - DRSts[i]) + Fwd,
                &(corridorvols[4]), &lognorm);
    ...
    call_lower_eps = srt_f_optblksch(
        DRSts[i], lowerbarrier - callspread, corridorvols[1],
        (periodmat[i] - periodmat[1]) / 365.25, 1.0, SRT_CALL, PREMIUM);

    call_lower = srt_f_optblksch(DRSts[i], lowerbarrier, corridorvols[2],
                                 (periodmat[i] - periodmat[1]) / 365.25, 1.0,
                                 SRT_CALL, PREMIUM);

    call_upper = srt_f_optblksch(
        DRSts[i], lowerbarrier + width, corridorvols[3],
        (periodmat[i] - periodmat[1]) / 365.25, 1.0, SRT_CALL, PREMIUM);

    call_upper_eps = srt_f_optblksch(
        DRSts[i], lowerbarrier + width + callspread, corridorvols[4],
        (periodmat[i] - periodmat[1]) / 365.25, 1.0, SRT_CALL, PREMIUM);
    ...
    if (strcmp(FloatCoupon, "YES") == 0) {
      ...
      price += frequencyratio * cvgs[paymentindex] *
               (DRSts[drsindex] + margin) * (1 + 2 * isfriday[i]) *
               ((call_lower_eps - call_lower) / callspread -
                (call_upper - call_upper_eps) / callspread);
    } else {
      price += frequencyratio * cvgs[paymentindex] * margin *
               (1 + 2 * isfriday[i]) *
               ((call_lower_eps - call_lower) / callspread -
                (call_upper - call_upper_eps) / callspread);
    }
...
double Optimise_Time_Swap_Price(
    int nfixings, double *DRSts, double width, double lowermin, double lowermax,
    double *lowerbarrier, double callspread, long spodate, double *periodmat,
    double *isfriday, char *FloatCoupon, double margin,
    SrtCompounding frequency, SrtCompounding ResetFrequency,
    char *szVolCurveName, char *szYieldCurveName, char *cRefRateCode,
    char *VolType, double *VolsInputs, double VolShift) {
  int iter = 0;
  ...
  precision = 1.0;
  pleft = Compute_Price(nfixings, DRSts, width, xleft, callspread, spodate,
                        periodmat, isfriday, FloatCoupon, margin, frequency,
                        ResetFrequency, szVolCurveName, szYieldCurveName,
                        cRefRateCode, VolType, VolsInputs, VolShift);
  pright = Compute_Price(nfixings, DRSts, width, xright, callspread, spodate,
                         periodmat, isfriday, FloatCoupon, margin, frequency,
                         ResetFrequency, szVolCurveName, szYieldCurveName,
                         cRefRateCode, VolType, VolsInputs, VolShift);
  ...
  while ((iter < maxiter) && (precision >= 0.0001)) {
    ...
    pmid = Compute_Price(nfixings, DRSts, width, xmid, callspread, spodate,
                         periodmat, isfriday, FloatCoupon, margin, frequency,
                         ResetFrequency, szVolCurveName, szYieldCurveName,
                         cRefRateCode, VolType, VolsInputs, VolShift);
    ...
    pmidleft =
        Compute_Price(nfixings, DRSts, width, xmidleft, callspread, spodate,
                      periodmat, isfriday, FloatCoupon, margin, frequency,
                      ResetFrequency, szVolCurveName, szYieldCurveName,
                      cRefRateCode, VolType, VolsInputs, VolShift);
    pmidright =
        Compute_Price(nfixings, DRSts, width, xmidright, callspread, spodate,
                      periodmat, isfriday, FloatCoupon, margin, frequency,
                      ResetFrequency, szVolCurveName, szYieldCurveName,
                      cRefRateCode, VolType, VolsInputs, VolShift);
...
double Price_Time_Swap(
    int nfixings, double *DRSts, double width, double callspread, long spotdate,
    double *periodmat, double *isfriday, char *FloatCoupon, double margin,
    SrtCompounding float_frequency, SrtCompounding ResetFrequency,
    char *szVolCurveName, char *szYieldCurveName, char *cRefRateCode,
    char *ResetType, char *VolType, double *VolsInputs, double VolShift,
    double lowerbarrier) /*returns the optimal barrier if OPTIMISED or contains
                            the spread to lower barrier if input*/
{
  double price;
  ...
  if (strcmp(ResetType, "NORESET") == 0) {
    price = Compute_Price(nfixings, DRSts, width, lowerbarrier, callspread,
                          spotdate, periodmat, isfriday, FloatCoupon, margin,
                          float_frequency, ResetFrequency, szVolCurveName,
                          szYieldCurveName, cRefRateCode, VolType, VolsInputs,
                          VolShift);
  }

  else if (strcmp(ResetType, "CENTERED") == 0) {
    price = Compute_Price(nfixings, DRSts, width, DRSts[1] - width / 2,
                          callspread, spotdate, periodmat, isfriday,
                          FloatCoupon, margin, float_frequency, ResetFrequency,
                          szVolCurveName, szYieldCurveName, cRefRateCode,
                          VolType, VolsInputs, VolShift);
  }

  else {
    price = Optimise_Time_Swap_Price(
        nfixings, DRSts, width, lowermin, lowermax, &eps, callspread, spotdate,
        periodmat, isfriday, FloatCoupon, margin, float_frequency,
        ResetFrequency, szVolCurveName, szYieldCurveName, cRefRateCode, VolType,
        VolsInputs, VolShift);
  }
...
Err srt_f_simpleresetable(
    ...
    char *VolType,     /*Flat or LogSliding or Input or NormSliding (that's the
                    default)*/
  ...
  /* gets the weights and points from a Hermite Polynomial for the numerical
   * integration made further down*/
  err = HermiteStandard(points, weights, 2 * nherm);
  ...
  *pv = 0.0;
  ...
  /*Gets the dates of fixing within the period and whether or not the date is a
   * friday or not*/
  err = get_periodmatandisfriday(0, dates, nfixdays, periodmat, isfriday);

  err = get_DRSts(today, (long)EndDate, (int)nfixdays, periodmat, cRefRateCode,
                  szYieldCurveName, DRStsStart);

  resetmat = (periodmat[1] - today) / 365.25;

  err = get_atm_vols((int)nfixdays, DRStsStart, periodmat, resetmat,
                     cRefRateCode, szVolCurveName, szYieldCurveName, atmvols);
  ...
  for (n = 1; n <= 2 * nherm; n++) {
    ...
    price = Price_Time_Swap((int)nfixdays, DRSts, width, callspread, spotdate,
                            periodmat, isfriday, FloatCoupon, margin,
                            float_compounding, ResetFrequency, szVolCurveName,
                            szYieldCurveName, cRefRateCode, ResetType, VolType,
                            VolsInput, VolShift, lowerbarrier);
...
double Price_Caplet(long StrikeDate, long StartDate, long EndDate, double dFra,
                    double dStrike, char *szVolCurveName,
                    char *szYieldCurveName, char *cRefRateCode, char *VolType,
                    double VolInput, SrtCallPutType CallPut) {
  Err err = NULL;
  double price;
  long today, spotdate;
  int spotlag;
  ...
  if (strcmp(VolType, "Input") == 0) {
    if (CallPut == SRT_STRADDLE)
      price =
          srt_f_optblksch(dFra, dStrike, VolInput, mat, 1.0, SRT_CALL,
                          PREMIUM) +
          srt_f_optblksch(dFra, dStrike, VolInput, mat, 1.0, SRT_PUT, PREMIUM);
    else {
      price =
          srt_f_optblksch(dFra, dStrike, VolInput, mat, 1.0, CallPut, PREMIUM);
    }
  }

  else if (strcmp(VolType, "Flat") == 0) {
    err = swp_f_vol(szVolCurveName, StartDate, EndDate, dStrike, &flatvol,
                    &lognorm);

    if (CallPut == SRT_STRADDLE)
      price =
          srt_f_optblksch(dFra, dStrike, flatvol, mat, 1.0, SRT_CALL, PREMIUM) +
          srt_f_optblksch(dFra, dStrike, flatvol, mat, 1.0, SRT_PUT, PREMIUM);
    else
      price =
          srt_f_optblksch(dFra, dStrike, flatvol, mat, 1.0, CallPut, PREMIUM);
  } else if (strcmp(VolType, "LogSliding") == 0) {
    start = add_unit(spotdate, 12 / float_compounding, SRT_MONTH,
                     MODIFIED_SUCCEEDING);
    end =
        add_unit(start, 12 / float_compounding, SRT_MONTH, MODIFIED_SUCCEEDING);
    ...
    err = swp_f_vol(szVolCurveName, start, end,
                    dfirstfra + (dStrike - dFra) * stdevoriginal / stdevactual,
                    &forwardvol, &lognorm);

    if (CallPut == SRT_STRADDLE)
      price = srt_f_optblksch(dFra, dStrike, forwardvol, mat, 1.0, SRT_CALL,
                              PREMIUM) +
              srt_f_optblksch(dFra, dStrike, forwardvol, mat, 1.0, SRT_PUT,
                              PREMIUM);
    else
      price = srt_f_optblksch(dFra, dStrike, forwardvol, mat, 1.0, CallPut,
                              PREMIUM);
    ...
    err = swp_f_vol(szVolCurveName, start, end, dfirstfra + (dStrike - dFra),
                    &forwardvol, &lognorm);
...
Err srt_f_simpleresetcap(
  ...
  *pv = 0.0;
  ...
  err = get_fra(StrikeFixing, spotlag, cRefRateCode, szYieldCurveName,
                &StrikeForward);

  resetmat = (StrikeFixing - today) / 365.25;

  err = swp_f_vol(szVolCurveName, StrikeDate, StartDate, StrikeForward,
                  &atmvolstrike, &lognorm);
  ...
  err = swp_f_vol(szVolCurveName, StartDate, EndDate, StrikeForward,
                  &atmvolforward, &lognorm);

  for (n = 1; n <= 2 * nherm; n++) {
    w = points[n];
    Strike = StrikeForward * exp(atmvolstrike * w * sqrt(resetmat) -
                                 atmvolstrike * atmvolstrike * resetmat / 2);
    dFwd = dForward + Strike - StrikeForward;
    price = Price_Caplet(StrikeDate, StartDate, EndDate, dFwd, Strike,
                         szVolCurveName, szYieldCurveName, cRefRateCode,
                         VolType, VolInput, CallPut);

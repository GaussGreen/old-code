																				...
																				at initial exchange date) */
{
	...
	/*	Compute the domestic cash flow at start date equivalent to final exchange */
	if (*fund_start_date >= today + eod_pay_flag)
	{
		ffx = fx_fund_dom * swp_f_df (fx_fund_dom_spot_date, *fund_start_date, fund_yc)
	...
	/*	Compute the domestic cash flow at start date equivalent to initial exchange */
...
Err transform_interp_coupon(int npts, double *pts, double *cpn, int linxtr_l, int linxtr_r,
								   double *cst, double *wspot, int *first_pt, int *last_pt, double *weights)
{
	int			i;
	double		tangent = 0.0;

	if ( (linxtr_l || linxtr_r) && npts < 2 ) return serror("At least 2 points needed for extrapolation");
...
void transform_oldspec_into_newspec(double alpha, double beta, int floored, double floor, int capped, double cap,
										   int *nstrikes, double *wcst, double *wspot, double *strikes, double *weights)

{
	*nstrikes = floored + capped;
	...
	else if ( fabs(beta) < 1e-16 )
	{
		...
		*wcst = alpha;
		*wspot = 0.0;
		if (floored && *wcst < floor) *wcst = floor;
		if (capped && *wcst > cap) *wcst = cap;
	}
	else if (*nstrikes == 1)
	{
...
int			use_3F_interp,			/*	0: interpolates the ATM market vols linearly, 1: uses the 3F for the interpolation */
...
int			no_prune_years,			/*	Number of years from the next call to the first prune */
int			prune_factor,			/*	Ex: 2 -> one call out of two */

/* Disable call dates */
int			disable_calls_method,	/* Name of the disable call method used : 0 None , 1 All in range 
											2 list of call ( Parameter1 and Parameter 2 not used */
...
int			erasing_call_done,		// always to 0, switch to 1 after haveng disabled calls
int			*erased_call_list,		// list of 0 & 1 used only if erasing_call_done = 1 and if use_GMA = TRUE

/* Alpha Beta Smile Model */
int			use_smile,				/* Flag: 0 no use alphabeta model, 1 use of the alphabeta model */
double		alpha,					/* Alpha, only used if use_smile =1 */
double		beta,					/* Beta,  only used if use_smile =1 */
/*For the smile impact*/
int			use_GMA,
int			use_3f_optim_barrier,	// Optimize the boundary with the 3F and then price the smile adjustment
double		SSLstd,
int			SSLniter,
int			CopNsimul,
int			CopDoPecs,
...
double		***optim_bar,			/*	Contains the value of the optimal KO for corresponding calls */
...
CPD_UND		und_exp)				/*	TS to be exported */
{
	...
	double	forward_at_smile, vol_at_smile, alpha_at_smile, beta_at_smile, rho_at_smile;
	...
	double		*pd_wcst = NULL, *pd_wspot = NULL, **pd_strikes = NULL, **pd_weights = NULL;
	double		*pd_not_wcst = NULL, *pd_not_wspot = NULL, **pd_not_strikes = NULL, **pd_not_weights = NULL;
	int			use_cpn_opt_str = (pd_nfxpts != NULL), use_not_opt_str = (pd_not_ref_nfxpts != NULL);
	...
	/*	Results */
	double		**Results = NULL;
	double		*mergeTimes = NULL, *sigDom = NULL, *sigFor = NULL, *sigFx = NULL, *corDF = NULL, *corDFx = NULL, *corFFx = NULL;
	int			mergeNtimes;
	int			Height = ncall + 4;
	int			Width = 12;
	...
	if ( (use_cpn_opt_str || use_not_opt_str) && (cpd_dlm_params->use_beta_dlm || use_smile) )
	{
		err = serror("Interpolated coupon is temporarily not supported with BetaDLM and AlphaBeta models");
		goto FREE_RETURN;
	}
/*
	if (ncall > 0 && ex_date[0] < start_date)
	{
		err = serror("First exercise date is before start date: not allowed");
		goto FREE_RETURN;
	}
*/
	if(use_GMA && use_sabr == 2)
	{
		err = serror("Smile impact with 3F fees and GMA should not be calculated simultaneously");
		goto FREE_RETURN;
	}

	// Transform smile specification if not default:
	if (smile_spec_type == 1 || smile_spec_type == 3) // SABR Beta -> SABR Log || BMM Beta -> BMM Log
	{
		for (i=0; i < num_fx_mkt_vol; i++)
		{
			...
			temp = (add_unit(fx_mkt_vol_date[i], -2, SRT_BDAY, MODIFIED_SUCCEEDING) - today) * YEARS_IN_DAY;
			...
			{
				err = srt_f_optbmmvol(	fwd, fwd, temp, 
										fx_mkt_vol[i],
										fx_mkt_smile_alpha[i],
										fx_mkt_smile_beta[i],
										fx_mkt_smile_rho[i],
	...
	if (use_cpn_opt_str) for (i=0; i < pd_ncpn; i++)
		if (pd_nfxpts[i] > max_interp_pts) max_interp_pts = pd_nfxpts[i];
	if (use_not_opt_str) for (i=0; i <= ncall; i++)
		if (pd_not_ref_nfxpts[i] > max_interp_pts) max_interp_pts = pd_not_ref_nfxpts[i];
	...
	if ( !new_call_type || !new_ex_date || !new_set_date || !new_barrier || !new_bar_type || !new_fees ||
		!new_pd_not_ref_fix || !new_pd_not_ref_fix_fx ||
		 (use_not_opt_str && (!new_pd_not_ref_nfxpts || !new_pd_not_ref_fxpts || !new_pd_not_ref_cpn_at_pts ||
	...
	/*	Automatic pruning */
	if (prune_calls && ncall > 0 && ex_date[ncall-1] > today + eod_ex_flag && prune_factor > 0)
	{
		//	Check prune factor
		if (prune_factor < 1) prune_factor = 1;

		/*	Calc next call date */
		i = 0;
		while (ex_date[i] < today + eod_ex_flag) i++;
		next_call_date = ex_date[i];

		/*	Check for pure KO feature */
		is_ko = 0;
		for (j=i; j<ncall; j++) if (call_type[j] != 0) is_ko = 1;
		
		/*	Do not prune if structure is a full ko */
		if (!is_ko)
		{
			/*	Calc prune date */
			prune_date = add_unit (next_call_date, no_prune_years, SRT_YEAR, MODIFIED_SUCCEEDING);
			...
			if (ex_date[ncall-1] > prune_date)
			{
				...
				while (ex_date[i] <= prune_date)
				{
					...
					if (use_not_opt_str)
					{
						...
						memcpy(new_pd_not_ref_fxpts[i], pd_not_ref_fxpts[i], pd_not_ref_nfxpts[i] * sizeof(double));
						memcpy(new_pd_not_ref_cpn_at_pts[i], pd_not_ref_cpn_at_pts[i], pd_not_ref_nfxpts[i] * sizeof(double));
				...
				while (j < ncall)
				{
					...
					if (use_not_opt_str)
					{
						...
						memcpy(new_pd_not_ref_fxpts[i], pd_not_ref_fxpts[j], pd_not_ref_nfxpts[j] * sizeof(double));
						memcpy(new_pd_not_ref_cpn_at_pts[i], pd_not_ref_cpn_at_pts[j], pd_not_ref_nfxpts[j] * sizeof(double));
				...
				if (use_not_opt_str & (new_ncall != ncall))		// copy final redemption information:
				{
					new_pd_not_ref_nfxpts[new_ncall] = pd_not_ref_nfxpts[ncall];
					memcpy(new_pd_not_ref_fxpts[new_ncall], pd_not_ref_fxpts[ncall], pd_not_ref_nfxpts[ncall] * sizeof(double));
					memcpy(new_pd_not_ref_cpn_at_pts[new_ncall], pd_not_ref_cpn_at_pts[ncall], pd_not_ref_nfxpts[ncall] * sizeof(double));
				...
				err = cpd_caller(
					...
					pd_not_ref_capped,
					pd_not_ref_cap,
					new_pd_not_ref_nfxpts,
					new_pd_not_ref_fxpts,
					new_pd_not_ref_cpn_at_pts,
					new_pd_not_ref_lin_xtrpl_l,
	...
	/*	Manual pruning */
	if (prune_calls && ncall > 0 && ex_date[ncall-1] > today + eod_ex_flag && prune_factor == -1)
	{				
		j = 0;
		for (i = 0; i<ncall; i++)
		{
			if (call_type[i] != -1)
			{
				...
				if (use_not_opt_str)
				{
					...
					memcpy(new_pd_not_ref_fxpts[j], pd_not_ref_fxpts[i], pd_not_ref_nfxpts[i] * sizeof(double));
					memcpy(new_pd_not_ref_cpn_at_pts[j], pd_not_ref_cpn_at_pts[i], pd_not_ref_nfxpts[i] * sizeof(double));
	...
	if (disable_calls_method && ncall > 0 && ex_date[ncall-1] > today + eod_ex_flag)
	{				
		j = 0;
		...
		case 1 : 
			...
			/* All in Range Method */
			/* Disable any Call or any KO which index is outside [first_call_index, last_call_index] */
			for (i = 0; i<ncall; i++)
			{
				/* Copy the arrays */
				new_call_type[i] = call_type[i];
	...
	if (!pd_num_strikes || !pd_wcst || !pd_wspot || !pd_strikes || !pd_weights ||
		!pd_not_num_strikes || !pd_not_wcst || !pd_not_wspot || !pd_not_strikes || !pd_not_weights)
	{
		err = serror("Memory failure in cpd_autocal");
		goto FREE_RETURN;
	}

	// Now convert all types of coupon specifications into const + fwd + sum of calls:
	...
	if (use_cpn_opt_str)
	{
		for (i=0; i < pd_ncpn; i++)
		{
			...
			memcpy(pd_strikes[i], pd_fxpts[i] + ifst_pt, pd_num_strikes[i] * sizeof(double));
			if (ifst_pt > 0) memmove(pd_weights[i], pd_weights[i] + ifst_pt, pd_num_strikes[i] * sizeof(double));
		}
	}
	else		// in this case transform the old alpha-beta specification into a new one:
	{
		for (i=0; i < pd_ncpn; i++)
		{
			transform_oldspec_into_newspec(
				pd_alpha[i], pd_beta[i], pd_floored[i], pd_floor[i], pd_capped[i], pd_cap[i],
				&pd_num_strikes[i], &pd_wcst[i], &pd_wspot[i], pd_strikes[i], pd_weights[i]);
		}
	}
	...
	if (use_not_opt_str)
	{
		for (i=0; i <= ncall; i++)
		{
			err = transform_interp_coupon(
				pd_not_ref_nfxpts[i],
				pd_not_ref_fxpts[i],
				pd_not_ref_cpn_at_pts[i],
				pd_not_ref_lin_xtrpl_l[i],
				pd_not_ref_lin_xtrpl_r[i],
				&pd_not_wcst[i],
				&pd_not_wspot[i],
				&ifst_pt,
				&ilst_pt,
			...
			memcpy(pd_not_strikes[i], pd_not_ref_fxpts[i] + ifst_pt, pd_not_num_strikes[i] * sizeof(double));
			if (ifst_pt > 0) memmove(pd_not_weights[i], pd_not_weights[i] + ifst_pt,
				pd_not_num_strikes[i] * sizeof(double));
		}
	}
	else
	{
		for (i=0; i < ncall; i++)		// Only domestic redemption for exercised deal in the old case
		{
		...
		transform_oldspec_into_newspec(
			pd_not_ref_alpha, pd_not_ref_beta, pd_not_ref_floored, pd_not_ref_floor, pd_not_ref_capped, pd_not_ref_cap,
			&pd_not_num_strikes[ncall], &pd_not_wcst[ncall], &pd_not_wspot[ncall],
			pd_not_strikes[ncall], pd_not_weights[ncall] );
	}

	// Prepare beta DLM smile calib:

	if(cpd_dlm_params->use_beta_dlm && cpd_dlm_params->calib_smile && use_calib)
	{
		cpd_dlm_params->smile_settlmt_date = (long) (add_unit(fx_spot_date, (int)(12 * cpd_dlm_params->calib_smile_maturity + 0.5), SRT_MONTH, NO_BUSDAY_CONVENTION));
		
		i = 0;
		while(cpd_dlm_params->smile_settlmt_date > fx_mkt_vol_date[i] && i < (num_fx_mkt_vol - 1))
			i++;
		
		if(!i || i == num_fx_mkt_vol - 1)
		{
			if(fabs(cpd_dlm_params->smile_settlmt_date - fx_mkt_vol_date[i]) < 5) /* 5 days */
				cpd_dlm_params->smile_settlmt_date = fx_mkt_vol_date[i];
			else
			{
				err = "CPDcaller : smile calibration not included in the term structure";
				goto FREE_RETURN;
			}
		}
		else
		{
			if(fabs(cpd_dlm_params->smile_settlmt_date - fx_mkt_vol_date[i]) < fabs(cpd_dlm_params->smile_settlmt_date - fx_mkt_vol_date[i-1]))
			{
				if(fabs(cpd_dlm_params->smile_settlmt_date - fx_mkt_vol_date[i]) < 5) /* 5 days */
					cpd_dlm_params->smile_settlmt_date = fx_mkt_vol_date[i];
				else
			...
			{
				if(fabs(cpd_dlm_params->smile_settlmt_date - fx_mkt_vol_date[i-1]) < 5) /* 5 days */
				{
		...
		cpd_dlm_params->calib_smile_maturity = YEARS_IN_DAY * ( add_unit(cpd_dlm_params->smile_settlmt_date, -2, SRT_BDAY, MODIFIED_SUCCEEDING) - today);
		...
		forward_at_smile = fx_spot * swp_f_df (today, cpd_dlm_params->smile_settlmt_date, for_yc) 
		...
		cpd_dlm_params->smile_strikes[0] = forward_at_smile 
					* exp( cpd_dlm_params->calib_smile_std_up * vol_at_smile * sqrt(cpd_dlm_params->calib_smile_maturity));
		cpd_dlm_params->smile_strikes[1] = forward_at_smile 
					* exp( cpd_dlm_params->calib_smile_std_down * vol_at_smile * sqrt(cpd_dlm_params->calib_smile_maturity));

		srt_f_optsarbvol(	forward_at_smile,							
							...
							SRT_LOGNORMAL,
							SRT_LOGNORMAL,
							&(cpd_dlm_params->smile_bs_vols[0]));
		...
		srt_f_optsarbvol(	forward_at_smile,							
							...
							SRT_LOGNORMAL,
							SRT_LOGNORMAL,
							&(cpd_dlm_params->smile_bs_vols[1]));
	}
	...
	for(i=0; i<num_fx_mkt_vol; i++)
		fwd_at_vol_dates[i] = fx_spot * swp_f_df(fx_spot_date, fx_mkt_vol_date[i], for_yc) /
										swp_f_df(fx_spot_date, fx_mkt_vol_date[i], dom_yc);
	
	// fill the mkt structure
	err = cpd_fill_smile_vol_market(	today,
										smile_spec_type, 
										num_fx_mkt_vol, 
										fwd_at_vol_dates, 
										fx_mkt_vol, 
	...
	if(fwd_at_vol_dates) free(fwd_at_vol_dates);
	...
	if (exercised)
	{
		...
		for (i=0; i < cpd->pd_leg->num_cpn && cpd->pd_leg->cpn[i].start_date < ex_date_ex; i++);
		pd_fut_ncpn = i;

		for (i=0; i < cpd->fund_leg->num_cpn && cpd->fund_leg->cpn[i].start_date < ex_date_ex; i++);
		fund_fut_ncpn = i;
		...
		cpn->fx_val_date = add_unit(cpn->fx_fix_date, 2, SRT_BDAY, MODIFIED_SUCCEEDING);
		...
		else
		{
			...
			{
				if (cpn->strikes) free(cpn->strikes);
				...
				memcpy(cpn->strikes, pd_not_strikes[ex_idx], pd_not_num_strikes[ex_idx] * sizeof(double));
				for (i=0; i < pd_not_num_strikes[ex_idx]; i++) cpn->weights[i] = pd_not * pd_not_weights[ex_idx][i];
			}
		}
	}	
	...
	if (use_smile)
	{
		...
		err = call_GRFN_alphabetaModel(
								...
								fx3dund,
								dom_calib,				/*	Calibrate domestic underlying */
	...
	if (fund_fut_ncpn > 0)
	{
		if (for_fund)
		{
			fund_leg += swp_f_df (und->today, cpd->fund_leg->cpn[0].start_date, Copyfund_ccy_yc) *
								thirdCcyfundNot * fx_fund_dom * swp_f_df (und->today, fx_fund_dom_spot_date, dom_yc) 
								/ swp_f_df (und->today, fx_fund_dom_spot_date, Copyfund_ccy_yc);

			// Exerciced Case
			// Force the Notional to be paid at ex_settle_date
			if (exercised && cpd->fund_leg->cpn[fund_fut_ncpn-1].pay_date != ex_date_set)
			{
				// Third currency case:
				if (cpd->fund_leg->cpn[fund_fut_ncpn-1].pay_date >= today + eod_pay_flag)
				{
					fund_leg -= swp_f_df (und->today, cpd->fund_leg->cpn[fund_fut_ncpn-1].pay_date, Copyfund_ccy_yc) *
								thirdCcyfundNot * fx_fund_dom * swp_f_df (und->today, fx_fund_dom_spot_date, dom_yc) 
								/ swp_f_df (und->today, fx_fund_dom_spot_date, Copyfund_ccy_yc);
				}
				...
				{	
					fund_leg += swp_f_df (und->today, ex_date_set, Copyfund_ccy_yc) *
								thirdCcyfundNot * fx_fund_dom * swp_f_df (und->today, fx_fund_dom_spot_date, dom_yc) 
								/ swp_f_df (und->today, fx_fund_dom_spot_date, Copyfund_ccy_yc);
				}
			}
		...
		{
			fund_leg += swp_f_df (und->today, cpd->fund_leg->cpn[0].start_date, fund_yc)
			...
			if (exercised && cpd->fund_leg->cpn[fund_fut_ncpn-1].pay_date != ex_date_set)
			{
				if (cpd->fund_leg->cpn[fund_fut_ncpn-1].pay_date >= today + eod_pay_flag)
				{
					fund_leg -= swp_f_df (und->today, cpd->fund_leg->cpn[fund_fut_ncpn-1].pay_date, fund_yc)
						* cpd->fund_leg->notional;
				}
				
				if (ex_date_set >= today + eod_pay_flag)
				{
					fund_leg += swp_f_df (und->today, ex_date_set, fund_yc) * cpd->fund_leg->notional;
				}
			}
		}
		...
		if (partial_pv)
		{
			partial_fund_leg += swp_f_df (und->today, cpd->fund_leg->cpn[partial_fund_idx].start_date, fund_yc)
				* cpd->fund_leg->notional;
		}
	}
	else
	{		
		...
		if (fin_not_date >= today + eod_pay_flag)
		{
			...
			if (for_fund)
			{
				fund_leg += swp_f_df (und->today, fin_not_date, Copyfund_ccy_yc) * 
								thirdCcyfundNot * fx_fund_dom * swp_f_df (und->today, fx_fund_dom_spot_date, dom_yc) 
								/ swp_f_df (und->today, fx_fund_dom_spot_date, Copyfund_ccy_yc);
			...
			{
				fund_leg += swp_f_df (und->today, fin_not_date, fund_yc) * cpd->fund_leg->notional;
			}
		}
	}
	...
	if (partial_pv)
	{
		partial_fund_leg -= swp_f_df (und->today, cpd->call[0].set_date, fund_yc) * cpd->call[0].fund_not_amt;
	}
	...
	{
		if (fund_pay[i] >= und->today + eod_pay_flag && (!exercised || fund_start[i] < ex_date_ex))
		{
	...
	for (i=0; i < pd_fut_ncpn; i++)
	{
		...
		if (!use_3F_interp)
		{
			...
			else
			{
				...
				std = interp_coef * fx_mkt_vol[j+1] + (1.0-interp_coef) * fx_mkt_vol[j];
			}
		}
		else
		{
		...
		if (use_sabr)
			err = cpd_vol_get_smile_params( 0,cpn->fx_val_date, 0.0, smile_mkt, smile_params);
		...
		if (use_cpn_opt_str)
		{
			...
			for (str_idx = 0; str_idx < cpn->nstrikes; str_idx++)
			{
				...
				{
					if (std > 1e-16 && cpn->strikes[str_idx] > 1e-16) type = 3;
					else type = 1;
					...
					if (use_sabr)
					{
						...
						smile_opt_string += cpn->weights[str_idx] * OPT_VAL_MACRO(
														type,
														fwd,
														cpn->strikes[str_idx],
														smile_std,
			...
			if (use_sabr) adj_fees[i] = df * (smile_opt_string - opt_string);

			/*	Coupon pv */		
			temp = df * (cpn->wcst + cpn->wspot * fwd + opt_string);
			pd_leg += temp;
			if (use_sabr) pd_leg += adj_fees[i];
		...
		else
		{
			...
			if (use_sabr)
			{
				if (cpd_dlm_params->use_beta_dlm)
				{				
					adj_fees[i] = df * fabs (cpn->beta) * (smile_floor - dlm_floor - smile_cap + dlm_cap);
				}
				else
				{
					adj_fees[i] = df * fabs (cpn->beta) * (smile_floor - floor - smile_cap + cap);
				}
			}
			...
			if (cpd_dlm_params->use_beta_dlm)
			{
				dlm_temp = df * (cpn->alpha + cpn->beta * dlm_fwd + fabs (cpn->beta) * (dlm_floor - dlm_cap));
				dlm_pd_leg += dlm_temp;

				if (use_sabr) dlm_pd_leg += adj_fees[i];
	...
	if (cpn->pay_date >= und->today + eod_pay_flag)
	{
		...
		if (cpn->fx_fix_date >= und->today + eod_fix_flag)
		{
			...
			if (use_not_opt_str)
			{
				...
				if (use_sabr) adj_fee_not = df * (smile_opt_string - opt_string);

				/*	Coupon pv */		
				temp = df * (cpn->wcst + cpn->wspot * fwd + opt_string);
				pd_leg += temp;
				if (use_sabr) pd_leg += adj_fee_not;
			...
			else
			{
				...
				if (use_sabr)
				{
					if (cpd_dlm_params->use_beta_dlm)
					{					
						adj_fee_not = df * fabs (cpn->beta) * (smile_floor - dlm_floor - smile_cap + dlm_cap);
					}
					else
					{
						adj_fee_not = df * fabs (cpn->beta) * (smile_floor - floor - smile_cap + cap);
					}
				}

				/*	Coupon pv */		
				temp = df * (cpn->alpha + cpn->beta * fwd + fabs (cpn->beta) * (floor - cap));
				...
				if (cpd_dlm_params->use_beta_dlm)
				{
					...
					if (use_sabr) dlm_pd_leg += adj_fee_not;
		...
		else	// if final notional is already fixed but not yet paid
		{
			if (use_not_opt_str)
			{
				...
				for (str_idx = 0; str_idx < cpn->nstrikes; str_idx++)
				{
					if ( fabs(cpn->weights[str_idx]) > 1e-16 && pd_not_ref_fix_fx[ex_idx] - cpn->strikes[str_idx] > 0.0 )
						opt_string += cpn->weights[str_idx] * (pd_not_ref_fix_fx[ex_idx] - cpn->strikes[str_idx]);
				}

				temp = (cpn->wcst + cpn->wspot * pd_not_ref_fix_fx[ex_idx] + opt_string) * df;
			}
			else
			{
	...
	if (partial_pv)
	{
		partial_pd_leg -= swp_f_df (und->today, cpd->call[0].set_date, und->dom_yc) * cpd->call[0].pd_not_amt;
	}
	...
	{
		if (pd_pay[i] >= und->today + eod_pay_flag && (!exercised || pd_start[i] < ex_date_ex))
		{
			...
			if (use_cpn_opt_str)
			{
				...
				{
					if ( fabs(pd_weights[i][str_idx]) > 1e-16 && pd_fix_fx[i] - pd_strikes[i][str_idx] > 0.0 )
						opt_string += pd_weights[i][str_idx] * (pd_fix_fx[i] - pd_strikes[i][str_idx]);
				}
					
				pd_leg += (pd_wcst[i] + pd_wspot[i] * pd_fix_fx[i] + opt_string)
					* coverage (pd_start[i], pd_pay[i], bas)
					* pd_not
			...
			else
			{
				...
				if (pd_floored[i] && fabs (pd_beta[i]) > 1.0e-16 && pd_floor[i] > -1.0e-16)
				{
				...
				if (pd_capped[i] && fabs (pd_beta[i]) > 1.0e-16 && pd_cap[i] > -1.0e-16)
				{
	...
	if (call_feat > 0 && use_sabr >= 1)		/* First update fees in all preinitialized structures */
	{
		/* Update cpd->call */
		for (i=0; i < cpd->num_calls; i++)
		{
			...
			cum_adj_fee = 0.0;
			for (j = cpd->call[i].pd_idx; j < cpd->pd_leg->num_cpn; j++) cum_adj_fee += adj_fees[j];
			cum_adj_fee += adj_fee_not;

			cum_adj_fee /= swp_f_df(und->today, cpd->call[i].set_date, und->dom_yc);

			cpd->call[i].extra_fee = (cpd->call[i].pay_rec ? cum_adj_fee : -cum_adj_fee);
		...
		if (!und->fees) und->fees = calloc(und->nb_fees, sizeof(double));
		if (!und->fees_dates) und->fees_dates = calloc(und->nb_fees, sizeof(double));
	...
	if (export_ts)		cpd_copy_und (und, und_exp);
	...
	/*	3)	If there is at least one call after today, value call feature */
	if(cpd_dlm_params->use_beta_dlm && fabs(und->model->dAlpha) > TINY)
	{
		...
		{
			err = cpd_AlphaFudgeUpdateUnd(	und,
											UpOrDown);		 /* Up = 0 and Down = 1 */

			if (err) goto FREE_RETURN;

			/*	Free and then re-Initialise structures */	
			if (UpOrDown)	cpd_free_tree_arg(tree_arg);
			if (UpOrDown)	cpd_free_mc_arg(mc_arg);
			...
			{
				if ( (call_feat == 1 && cpd->type == -1 && do_optim) || call_feat == 2 )	/* MC or tree */
				{
					/* Update mc_arg */
					for (i=0; i < mc_arg->nb_dates; i++)
					{
						...
						if (cpd_prm)
						{
							cpd_prm->eval_const.extra_fee = cpd->call[cpd_prm->call_idx].extra_fee;
							...
							cpd_prm->eval_const.fee = cpd_prm->eval_const.orig_fee + cpd_prm->eval_const.extra_fee;
						}
					}
				}
				else
				...
				switch (cpd->type)
				{
					case -1:
						if (do_optim)
						{
							...
							err = cpd_launch_opti_mc (cpd, und, mc_arg, &call, &call_std, do_infos, optim_bar);
						}
						else
						{
							smessage ("Launching tree, time steps requested: %d, actual: %d", req_stp, tree_arg->nstp);
		...
		*call_stdev = sqrt(0.5 * (*call_stdev * *call_stdev + call_val_alpha_std * call_val_alpha_std));
	}
	else
	{	
		if (call_feat > 0 && use_sabr == 2)
			...
			switch (cpd->type)
			{
				case -1:
					...
					else
					{
						...
						{
							err = serror("KO in a tree temporarily not supported with option string coupon or redemption");
							goto FREE_RETURN;
						}
				...
				case 0:
					...
					smessage ("Launching tree, time steps requested: %d, actual: %d", req_stp, tree_arg->nstp);
					err = cpd_launch_tree (cpd, und, tree_arg, &call, &temp); /* Calculates the IV in the tree */
				break;
			}
	...
	if(!exercised && use_GMA && cpd->num_calls > 0)
	{
		...
		{
			if(cpd->call[0].ex_date > cpd->pd_leg->cpn[cpd->call[0].pd_idx].fx_val_date)
			{
				FundingSpeedUp = 0;
			}
		}
		...
		cpd_init_otc_params(	OTC_params,
								cpd,
								ncall,
								CopNsimul, 
								CopDoPecs, 
								...
								fwdSmileVisuNstd,
								/* otc */
								fwdVolMethod,			/*	0=Sliding 3F vol; 1=Converging 3F vol; 2=Sliding Cvg Sbeta; 3=Cvg Cvg Sbeta */
		...
		Merge the TS
		================================== */
		err = merge_rates_fx_corr_ts(	und->sigma_time_rates, und->sigma_dom, und->sigma_n_rates,
										und->sigma_time_rates, und->sigma_for, und->sigma_n_rates,
										und->sigma_time_fx, und->sigma_fx, und->sigma_n_fx,
		...
		if(!cpd->type)
		{
			err = cpd_otc_precalc(	OTC_precalc,
									OTC_params,
									cpd,
									und,
									smile_mkt,
		...
		{
			if(OTC_params->KO_do_optim && use_3f_optim_barrier && (cpd->type == -1 && do_optim))
			{
				OTC_params->KO_do_optim = 0;
				for(i=0 ; i<cpd->num_calls; i++)
				{			
					if(1 - cpd->call[i].call_type)
					{
			...
			// the CALLABLE/KO has been priced in the tree and we want to be conservative
			// on the smile adj. (use the same barrier)
			...
			*smile_adjustment = - (Results[0][0] - Results[1][0]);
			...
			if(OTC_params)	cpd_free_otc_params(OTC_params);
			if(OTC_precalc)	cpd_free_otc_precalc(OTC_precalc, cpd->num_calls,cpd->pd_leg->num_cpn, cpd->fund_leg->num_cpn);
			Results[0][0] = 0.0;
			Results[1][0] = 0.0;

			// second pricing with smile
			if(OTC_params->KO_do_optim && use_3f_optim_barrier && (cpd->type == -1 && !do_optim))
				OTC_params->KO_do_optim = 0; // the barrier has allready been updated in the cpd args
...
FREE_RETURN:
	...
	free(pd_wcst);
	free(pd_wspot);
	if (pd_strikes) free_dmatrix(pd_strikes, 0, alloc_pd_ncpn-1, 0, max_interp_pts-1);
	if (pd_weights) free_dmatrix(pd_weights, 0, alloc_pd_ncpn-1, 0, max_interp_pts-1);
	free(pd_not_num_strikes);
	free(pd_not_wcst);
	free(pd_not_wspot);
	if (pd_not_strikes) free_dmatrix(pd_not_strikes, 0, alloc_ncall, 0, max_interp_pts-1);
	if (pd_not_weights) free_dmatrix(pd_not_weights, 0, alloc_ncall, 0, max_interp_pts-1);

	if(OTC_params)	cpd_free_otc_params(OTC_params);
	if(OTC_precalc)	cpd_free_otc_precalc(OTC_precalc, cpd->num_calls,cpd->pd_leg->num_cpn, cpd->fund_leg->num_cpn);
	
	if (free_struct) cpd_free_all_struct (cpd, und, tree_arg, mc_arg);
	free (new_call_type);
	free (new_ex_date);
	free (new_set_date);
	free (new_barrier);
	...
	if (new_pd_not_ref_fxpts) free_dmatrix(new_pd_not_ref_fxpts, 0, alloc_ncall, 0, max_interp_pts-1);
	if (new_pd_not_ref_cpn_at_pts) free_dmatrix(new_pd_not_ref_cpn_at_pts, 0, alloc_ncall, 0, max_interp_pts-1);
	...
	if (InstConst) free (InstConst);

	if(Results) free_dmatrix(Results, 0, Height - 1, 0, Width - 1);
	if(mergeTimes) free (mergeTimes);
	if(sigDom) free(sigDom);
	if(sigFor) free(sigFor); 
	if(sigFx) free(sigFx); 
	if(corDF) free(corDF); 
	if(corDFx) free(corDFx); 
	if(corFFx) free(corFFx); 

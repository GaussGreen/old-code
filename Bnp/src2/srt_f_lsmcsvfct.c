...
static Err cheybetastochvol_evolve_in_Grid(
    double** rndm_mat, SrtStpPtr step, MCTreePointCheyBetaStochVol** grid, long n)
{
    Err          err = NULL;
    SrtStpPtr    top;
    long         t;
    SrtIRMTmInf* tminf1;
    double dt, sqdt, c1, c2, dw1, dw2, sig, eta, rho, lam, r, x, phi, beta, vol, vovol, sqrvovol;
    ...
    while (step->next != NULL)
    {
        ...
        r = grid[t][n].short_rate = sam_get(tminf1->fwd_sam, 0, F_0_t) + grid[t][n].x;
...
/* --------------------------------------------------------------------------------------------- */
static double phi1(double x, double phi, double sigma)
{
    return (1.0);
}

static double phi2(double x, double phi, double sigma)
{
    return (x);
}

static double phi3(double x, double phi, double sigma)
{
    return (x * x);
}

static double phi4(double x, double phi, double sigma)
{
    return (x * x * x);
}
...
static double phi8(double x, double phi, double sigma)
{
    return (sigma);
}
...
static double phi10(double x, double phi, double sigma)
{
    return (phi);
}

...
static Err cheybetastochvol_evolve_in_Grid( double **rndm_mat, SrtStpPtr step, 
								MCTreePointCheyBetaStochVol **grid, long n)					     
{
	Err			 err = NULL;
	SrtStpPtr 	 top;
	long 		 t; 
	SrtIRMTmInf   *tminf1;
	double       dt, sqdt, c1, c2, dw1, dw2, sig, eta, rho, lam,  
				 r, x, phi, beta, vol, vovol, sqrvovol;
								  ...
								  long             nt)
{
	...
	err = SrtMCRanSrc_start(&mcrs, 0, grfnparam->num_MCarlo_paths - 1, 1, nt, 
		                    und_info->no_of_brownians, grfnparam->sample_type, grfnparam->rand_seed, top);
	if (err) return err;
							...
							double s22)
{
	...
	for (k = 1; k <= nreg; k++)
	{
	   for (l = 1; l <= nreg; l++)	cov[k][l] = 0.0; 
	   for (l = 1; l <= node_dim; l++)  vec[k][l] = 0.0; 		 
	}

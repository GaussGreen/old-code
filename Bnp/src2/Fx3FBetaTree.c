...
static  double Bounds[3][8];
	...
	for (i=0; i<8; i++)
	{
		maxmin_dual_global[2][i] = (exp((1.0 - beta) * (logSpot + 	maxmin_dual_global[2][i])) - 1.0) / (1.0 - beta);
	}
...
Err				err;
	...
	{	
		...
		if (failed)
		{			
			a = ((exp ((1.0 - beta[step]) * (fx_fwd[step] + logSpot)) - 1.0) / (1.0 - beta[step]) * sqt_prev_inv 
					- transf_fx[0] * maxmin_dual_tab[step][0][1]
					- transf_fx[1] * maxmin_dual_tab[step][1][1]
					- transf_fx[2] * maxmin_dual_tab[step][2][1]
					) / (transf_fx[idx_rel] * MESH);

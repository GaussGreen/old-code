/* -------------------------------------------------------------------------
   ...
   FILE NAME	: swp_f_vol.cxx

   PURPOSE		: compute volatilities via a pointer to a function
   VolFunc passed to the SrtExternalFunctions static ( see
   srt_f_external_fct.cxx
   )
   ------------------------------------------------------------------------- */
...
/* ---------------------------------------------------------------------------
        Calculate volatility         , using the VolFunc that has been attached
...
/* ---------------------------------------------------------------------------
        Calculate volatility         , using the SABRVolFunc that has been
  attached to the library (see srt_f_external_fct.cxx)
  ---------------------------------------------------------------------------
        case 0:"ATMLOGNORMAL";
        case 1:"ATMNORMAL";
        case 2:"ATMSIGMABETA";
...
Err swp_f_SABRvol(char *vol_id, Ddate start, Ddate end, double strike,
  ...
  /*	if (component == 9)
          {
                  err = sabrvol_func(vol_id        , start        , end        ,
     strike        , &dtemp , power        , (int)8); if (err) return err;

...
#define ALLOC_BIGMEM_ASSETS                                                    \
  {                                                                            \
    *assets = f4tensor(0, num_time_pts - 1, -maxtrinf.max_index[0],            \
                       maxtrinf.max_index[0], -maxtrinf.max_index[1],          \
                       maxtrinf.max_index[1], 0, nbi);                         \
    *prob = f4tensor(0, num_time_pts - 1, -maxtrinf.max_index[0],              \
                     maxtrinf.max_index[0], -maxtrinf.max_index[1],            \
                     maxtrinf.max_index[1], 0, 4);                             \
    *df = f3tensor(0, num_time_pts - 1, -maxtrinf.max_index[0],                \
                   maxtrinf.max_index[0], -maxtrinf.max_index[1],              \
                   maxtrinf.max_index[1]);                                     \
    *dom = f3tensor(0, num_time_pts - 1, -maxtrinf.max_index[0],               \
                    maxtrinf.max_index[0], -maxtrinf.max_index[1],             \
                    maxtrinf.max_index[1]);                                    \
  }
...
#define FREE_BIGMEM_ASSETS                                                     \
  {                                                                            \
    free_f4tensor(assets, 0, num_time_pts - 1, -maxx, maxx, -maxy, maxy, 0,    \
                  nbi);                                                        \
...
Err srt_f_lgm2dtree_proj(
  ...
  /* call to tree lgm2d */
  err = srt_f_lgm2dtree_forminim(und, grfnparam, stp, gd, evalcf, iolist,
                                 und_info, &assets, &prob, &df, &dom, &maxx,
                                 &maxy);
  if (err) {
    FREE_BIGMEM_ASSETS
    return err;
  ...
  /* err = simul_and_regress_pdestatic(grfnparam        , stp        , assets ,
     prob        , vec        , cov        , nbi        , &residu        , maxx
           , maxy        , num_time_pts); */
  err = simul_and_regress_mcstatic(grfnparam, stp, assets, prob, df, dom, vec,
                                   cov, nbi, &residu);
  if (err) {
    FREE_BIGMEM_ASSETS
    return err;
...
Err simul_and_regress_mcstatic(SrtGrfnParam *grfnparam, SrtStpPtr step,
                               double ****assets, double ****prob, double ***df,
                               double ***dom, double **vec, double **cov,
                               int nbi, double *residu) {
  Err err = NULL;
  ...
  for (n = 0; n < grfnparam->num_MCarlo_paths; n++) {
    ...
    while ((step != NULL) && (time <= grfnparam->minmaxtime)) {
      ...
      for (k = 1; k <= nbi; k++) {
        ...
        for (l = 1; l <= nbi; l++)
          cov[k][l] +=
              (dfcur * assets[t][i][j][k - 1] * assets[t][i][j][l - 1]);
      }
      ...
      if (step->next != NULL) {
        ...
        if (rand < prob[t][i][j][1])
          ...
          j++;
                 ...
                 (prob[t][i][j][1] + prob[t][i][j][2] + prob[t][i][j][3]))
          i--;
        else if (rand < (prob[t][i][j][1] + prob[t][i][j][2] +
                         prob[t][i][j][3] + prob[t][i][j][4]))
          j--;
      }
  ...
  for (n = 0; n < grfnparam->num_MCarlo_paths; n++) {
    ...
    while ((step != NULL) && (time <= grfnparam->minmaxtime)) {
      ...
      res += (dfcur * (port - assets[t][i][j][nbi]) * (port -
      assets[t][i][j][nbi]) * dt);
      ...
      */
      ...
      for (k = 1; k <= nbi; k++)
        port += (vec[k][1] * assets[t][i][j][k - 1]);
      res += (dfcur * (port - assets[t][i][j][nbi]) *
              (port - assets[t][i][j][nbi]) * dt);
...
#define ALLOC_MEM_ASSETS                                                       \
  {                                                                            \
    cur_assets = f3tensor(-maxtrinf.max_index[0], maxtrinf.max_index[0],       \
                          -maxtrinf.max_index[1], maxtrinf.max_index[1], 0,    \
                          node_dim - 1);                                       \
...
Err srt_f_lgm2dtree_forminim(
    ...
    SrtIOStruct *iolist,     /* Requests */
    SrtUndInfo *und_info,    /* Underlying info (used in evalcf) */
    double *****assets, double *****prob, double ****df, double ****dom,
    long *maxx, long *maxy) {
  Err err = NULL;
  int i, j;
  double cashflow;
  ...
  *maxy = maxtrinf.max_index[1];
  ...
  for (nt = num_time_pts - 1, stp = bot; stp != NULL; stp = stp->prev, nt--) {
    ...
    sam_get(node.cxxur_sam, 0, PHI1) = sam_get(tminf->fwd_sam, 0, PHI1);
    sam_get(node.cxxur_sam, 0, PHI2) = sam_get(tminf->fwd_sam, 0, PHI2);
    sam_get(node.cxxur_sam, 0, CROSSPHI) = sam_get(tminf->fwd_sam, 0, CROSSPHI);
    ...
    for (i = -trinf->max_index[0]; i <= trinf->max_index[0]; i++)
      for (j = -trinf->max_index[1]; j <= trinf->max_index[1]; j++) {
        ...
        sam_get(node.cxxur_sam, 0, X1) =
            trinf->dual_basis[0][0] * y1 + trinf->dual_basis[0][1] * y2;
        sam_get(node.cxxur_sam, 0, X2) =
            trinf->dual_basis[1][0] * y1 + trinf->dual_basis[1][1] * y2;

        /* If it is not the last step        , compute forward        ,
         * connections and probas */
        if (stp->next)
          populate_lgm2f_tree_node(tminf, &node, stp->delta_t,
                                   stp->next->trinf);

        /* r1 = X1 + H11 + H12        , r2 = X2 + H22 + H21        ,  r = F(0
         * ,t) + r1 + r2
         */
        sam_get(node.cxxur_sam, 0, X1) +=
            tminf->rf.twof[0][0].H + tminf->rf.twof[0][1].H;
        sam_get(node.cxxur_sam, 0, X2) +=
            tminf->rf.twof[1][0].H + tminf->rf.twof[1][1].H;
        sam_get(node.cxxur_sam, 0, SHORT_RATE) =
            sam_get(tminf->fwd_sam, 0, F_0_t) + sam_get(node.cxxur_sam, 0, X1) +
            sam_get(node.cxxur_sam, 0, X2);
        sam_get(node.cxxur_sam, 0, STATEVAR) = sam_get(node.cxxur_sam, 0, X1);
        ...
        if (stp->next) {
          ...
          lgm2f_tree_expectation(&node, next_assets, cur_assets[i][j],
                                 node_dim);
        }

        /* Evaluate cash flows in Grfn tableau */
        if (err = evalcf((GrfnEvent *)stp->e, &node.cxxur_sam, gd,
...
Err simul_and_regress_pdestatic(SrtGrfnParam *grfnparam, SrtStpPtr step,
                                double ****assets, double ****prob,
                                double ***df, double **vec, double **cov,
                                int nbi, double *residu, long maxx, long maxy,
                                long maxt) {
  Err err = NULL;
  ...
  for (t = maxt - 1, step = gotobot(step)->prev; step != NULL;
       step = step->prev, t--) {
    trinf = (SrtDiagTwoFacTreeInfo *)(step->trinf);
    dt = step->delta_t;
    /* Loop on XiYj */
    for (i = -trinf->max_index[0]; i <= trinf->max_index[0]; i++)
      for (j = -trinf->max_index[1]; j <= trinf->max_index[1]; j++) {
        ...
        for (k = 1; k <= nbi; k++)
          port += (vec[k][1] * assets[t][i][j][k - 1]);
        res = ((port - assets[t][i][j][nbi]) * (port - assets[t][i][j][nbi]) *
               dt);
        ...
        Rt_cur[i][j] = res + prob[t][i][j][0] * Rt_next[i][j] +
                       prob[t][i][j][1] * Rt_next[ip][j] +
                       prob[t][i][j][2] * Rt_next[i][jp] +
                       prob[t][i][j][3] * Rt_next[im][j] +

    ...
    SrtGreekType   greek)
{
    ...
    else
    {
        ...
        d1x = (log(fwdx / strike) + mux * mat2 + sigx * sigx * mat2) / (sigx * sqrt(mat2));
        d2x = d1x - sigx * sqrt(mat2);

        d1y = (log(spoty / barrier) + muy * mat1 + rho * sigy * sigx * mat1) / (sigy * sqrt(mat1));
        ...
        premium -= fwdx * a * exp(alpha) *
                   bivar(
                       cp * (d1x + (alpha / (sigx * sqrt(mat1)))),
                       du * (d1y + (beta / (sigy * sqrt(mat1)))),
                       cp * du * rho);
        premium -= strike * bivar(cp * d2x, du * d2y, cp * du * rho);
        premium += strike * a *
                   bivar(
                       cp * (d2x + (alpha / (sigx * sqrt(mat1)))),
                       du * (d2y + (beta / (sigy * sqrt(mat1)))),
                       cp * du * rho);
        premium *= disc * cp;
    }

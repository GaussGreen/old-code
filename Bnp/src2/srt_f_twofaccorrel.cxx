/* =======================================================================
        ...
        FILENAME:   srt_f_twofaccorrel.cxx
...
double model_correl(double mat1, double mat2, double alpha, double beta,
  ...
  normvar1 = 1 + 2 * rho * alpha * exp(-beta * mat1) +
             alpha * alpha * exp(-2 * beta * mat1);
  normvar2 = 1 + 2 * rho * alpha * exp(-beta * mat2) +
             alpha * alpha * exp(-2 * beta * mat2);
  normcovar = 1 + rho * alpha * (exp(-beta * mat1) + exp(-beta * mat2)) +
...
Err model_corr_matrix(double **model_corr, double *tenor_mat, long num_tenor,
  ...
  for (i = 0; i < num_tenor; i++) {
    for (j = 0; j < i; j++) {
      model_corr[i][j] =
          model_correl(tenor_mat[i], tenor_mat[j], alpha, beta, rho);
...
/* -----------------------------------------------------------------------
        Given model parameters (alpha        , beta        , rho) and a set of
   dates (in Y) return the confidence level explain by the two eigen vectors
   ----------------------------------------------------------------------- */

Err TwoFactorEigenConf(double *pdSetOfDates, int iNumberOfDates, double dAlpha,
                       double dGamma, double dRho,
                       /* return a vector of two elements */
                       double pdConfidenceLevel[2]) {
  double **CorrelMatrix = NULL, *EigenValues = NULL, **EigenVectors = NULL;
  int i;
  Err err;

  CorrelMatrix = dmatrix(0, iNumberOfDates - 1, 0, iNumberOfDates - 1);
  EigenVectors = dmatrix(0, iNumberOfDates - 1, 0, iNumberOfDates - 1);

  EigenValues = (double *)malloc(sizeof(double) * (iNumberOfDates - 1));

  if (err = model_corr_matrix(CorrelMatrix, pdSetOfDates, iNumberOfDates - 1,
                              dAlpha, dGamma, dRho))
    return err;
  ...
  if (err = jacobi_diagonalisation(CorrelMatrix, iNumberOfDates, EigenValues,
                                   EigenVectors, &i))
    return err;
...
Err chisq_correl(double **hist_corr, double *tenor_mat, long num_tenor,
  ...
  err = model_corr_matrix(model_corr, tenor_mat, num_tenor, alpha, beta, rho);
  if (err) {
...
/* ------------------------------------------------------------------------
        Returns in stdev (sqrt of average of errors squared )        , the
   difference between model and historical correlation matrix        , for given
        model parameters (alpha        , beta        , rho) and ifr tenors
   ------------------------------------------------------------------------ */
...
Err stdev_correl(double **hist_corr, double *tenor_mat, long num_tenor,
  ...
  err = chisq_correl(hist_corr, tenor_mat, num_tenor, alpha, beta, rho, &chisq);
  if (err)
    return err;
...

{
  ...
  alpha =
      ((TwoFacIrmTermStructVal *)(ts->head->element->val.pval))->exp_ts[1].sig /
      ((TwoFacIrmTermStructVal *)(ts->head->element->val.pval))->exp_ts[0].sig;

  beta = (1.0 / ((TwoFacIrmTermStructVal *)(ts->head->element->val.pval))
                    ->exp_ts[1]
                    .tau) -
         (1.0 / ((TwoFacIrmTermStructVal *)(ts->head->element->val.pval))
                    ->exp_ts[0]
                    .tau);
...
/*
  ...
  Given any model        , returns the normal correl between swap rate 1 and
  swap rate 2 senn at some observation date through a grfn tableau.
...
Err swap_correl(Date obs_date, Date start1, Date end1, String cmp1,
  ...
  char ***tableau = smatrix_size(0, 0, 0, 1, GRFN_DEF_ARGBUFSZ),
       swap1[GRFN_DEF_ARGBUFSZ], swap2[GRFN_DEF_ARGBUFSZ];
  double **ss = dmatrix(0, 0, 0, 1), temp_price, temp_stdev;
  long i;

  double e1 = 0.0, e2 = 0.0, var1 = 0.0, var2 = 0.0, covar = 0.0;
  ...
  if (*n_ln == 'N') {
    ...
    return serror("parameter %s should be normal or lognormal", n_ln);
  ...
  if (err = SrtGrfnMain(und, mdl_rows, paramStrings, valueStrings, 1, &obs_date,
                        1, 2, tableau, mask, 0, 0, NULL, &temp_price,
                        &temp_stdev, ss, NULL)) {
    return err;
  }
...
Err generic_correl(Date obs_date, String instr1, String instr2, String und,

  ...
  *num_evt_dtes = ncpn = nexo + today_flag + first_ex_flag;
  ...
  for (i = 0; i < nexo; i++) {
    ...
    sprintf(evt[today_flag + first_ex_flag + i][1],
            "MAX(A[6  ,I]  ,MIN(A[7  ,I]  ,A[4  ,I]+A[5  ,I]*C[0  "
            ",I]))*DF(D[I]  ,A[3  ,I]  ,\"%s\")",
            dom_name);
  }
  ...
  /*	Redemption */
  sprintf(tmpstr,
          "+MAX(A[14  ,0]  ,MIN(A[15  ,0]  ,A[12  ,0]+A[13  ,0]*C[0  "
          ",I]))*DF(D[I]  ,A[3  ,I]  ,\"%s\")",
          dom_name);
  ...
  if (ncall > 0) {
    ...
    for (i = 0; i < ncall; i++) {
      ...
      */
      ...
      {
        sprintf(evt[j][3], "(PV[1]-A[11  ,I]*DF(D[I]  ,A[10  ,I]  ,\"%s\")",
                dom_name);
      }
      ...
      /*	Barrier */
      {
        if (call[i].bar_type == 0) {
          sprintf(evt[j][4], "IF(S(\"%s\")>A[8  ,I]  ,C[3  ,I]  ,PV[J])",
                  fx_name);
        } else {
          sprintf(evt[j][4], "IF(S(\"%s\")<A[8  ,I]  ,C[3  ,I]  ,PV[J])",
                  fx_name);
...
/* Function  call_GRFN_alphabetaModel
        ...
        Price the funding  , power dual leg and the call of a PD in the alpha
   beta model by initialising the dom  , for  , and fx underlying if necessary
                making the grfn tableau from the cpd_str structure of the power
   dual calling the alphabeta grfn tree to price
   ...
   the alpha beta model  , the call_GRFN_alphabetaModel calculates the ts and
   filled the cpd_und with it.
    ...
    if (pd_pay[iNum] >= cpd_und->today + eod_pay_flag) {
      ...
      /* Old Method
        ...
        pd_cf[pd_ncf] = pd_past_fix[i]
              * coverage ((long) (pd_start[i] + 1.0e-08)  , (long) (pd_pay[i]
        + 1.0e-08)  , bas)
              * pd_not;
      */

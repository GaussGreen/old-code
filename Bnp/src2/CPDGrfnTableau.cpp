    ...
    double** aux)
{
    ...
    int         i, j, k;
    int ncpn, nexo = exo_leg->num_cpn, nfund = fund_leg->num_cpn, ncall = cpd->num_calls, ncols = 5;
    ...
    *num_evt_dtes = ncpn = nexo + today_flag + first_ex_flag;
    ...
    for (i = 0; i < nexo; i++)
    {
        ...
        aux[today_flag + first_ex_flag + i][7] =
            (exo_leg->cpn[i].capped ? exo_leg->cpn[i].cap : NO_CAP);
    ...
    sprintf(tmpstr, "+PVRNG(0,1,D[I],\"%s\"))", fund_name);
    ...
    if (ncall > 0)
    {
        ...
        for (i = 0; i < ncall; i++)
        {
            ...
            */
            if (j > 0 &&
                fabs(call[i].ex_date - evt_dtes[j - 1]) < fabs(evt_dtes[j] - call[i].ex_date))
            {
            ...
            {
                sprintf(evt[j][3], "(PV[1]+C[1,I]-A[11,I]*DF(D[I],A[10,I],\"%s\")", dom_name);
            }

            if (k == j + 1)
            /*	The next coupon is called */
            {
                sprintf(evt[j][3], "(PV[1]-A[11,I]*DF(D[I],A[10,I],\"%s\")", dom_name);
            }
            ...
            /*	PV of funding leg */
            sprintf(tmpstr, "-PVRNG(0,1,%d,\"%s\")", fund_leg->cpn[k].pay_date - 1, fund_name);
            ...
            /*	Barrier */
            {
                ...
                {
                    sprintf(evt[j][4], "IF(S(\"%s\")>A[8,I],C[3,I],PV[J])", fx_name);
                }
                else
                {
                    sprintf(evt[j][4], "IF(S(\"%s\")<A[8,I],C[3,I],PV[J])", fx_name);
    ...
    double** aux)
{
    ...
    *num_evt_dtes = nexo + (nexo - 1) * credit_freq + today_flag + first_ex_flag;
    ...
    {
        step =
            (long)((evt_dtes[today_flag + first_ex_flag + (i + 1) * (1 + credit_freq)] - evt_dtes[today_flag + first_ex_flag + i * (1 + credit_freq)]) / (credit_freq + 1) + 0.5);

        for (j = 1; j <= credit_freq; j++)
        {
            evt_dtes[today_flag + first_ex_flag + i * (1 + credit_freq) + j] =
                evt_dtes[today_flag + first_ex_flag + i * (1 + credit_freq) + j - 1] + step;
        }
    }
    ...
    for (i = 0; i < nexo; i++)
    {
        aux[today_flag + first_ex_flag + i * (1 + credit_freq)][2] = add_unit(
    ...
    double        fx_vol_shift)
{
    ...
    long    temp_tau_n, temp_sig_n1, temp_sig_n2;
    CPD_UND basic_und;
    int     num_cal;
    Err     err = NULL;
...
/* Function  call_GRFN_alphabetaModel
   ...
   cpd_fill_check_all_struct function
                ...
                the fx ts corresponds either to the fx und passed to the function (which in that
   case is assumed to be a alpha beta calibrated fx ts) or the calibrated one by the
   cpd_fill_check_all_struct that is to say a Lognormal Model calibrated fx ts

                In the case where the Fx underlying needs to be calibrated with the alpha beta
   model, the call_GRFN_alphabetaModel calculates the ts and filled the cpd_und with it.
    ...
    double* call_stdev /*	Standard deviation of the call if applicable */)
{
    ...
    if ((!DateSigmaTab) || (!und_names) || (!correl) || (!correl_dates) || (!auxLen) ||
        (!evt_dtes) || (!tableauStrings) || (!aux) || (!aux_transpose) || (!mask) ||
        (!tab_barrier) || (!bar_col) || (!maturity_opt) || (!fund_cfdtes) || (!fund_cf) ||
        (!pd_cfdtes) || (!pd_cf))
    {
    ...
    if (!use_calib)
    {
        ...
        /* It is supposed to be a alpha beta fxund with the same alpha and beta passed to the
         * function */
    ...
    else
    {
        ...
        err = SrtInitCorrelationMatrix(cpd_und->corr_n_times, 3, correl, correl_dates, und_names);
    ...
    while (iNum < fund_ncpn && fund_fix[iNum] < cpd_und->today + eod_fix_flag)
    {
        if (fund_pay[iNum] >= cpd_und->today + eod_pay_flag)
        {
            ...
            fund_cf[fund_ncf] =
                (fund_fix_cpn[iNum] + fund_mrg[iNum]) *
                coverage(
                    (long)(fund_start[iNum] + 1.0e-08), (long)(fund_pay[iNum] + 1.0e-08), bas) *
                fund_not;
            fund_ncf++;
        }
...
FREE_RETURN:
    ...
    if (auxLen)
    {
        free_lngvector(auxLen, 0, MAX_COL - 1);
    }

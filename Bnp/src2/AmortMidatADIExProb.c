    ...
    void*         pAuxArg)
{
    ...
    const int nEx_I =
        (const double*)bsearch(pdEx, pdEx_Begin, nNumEx, sizeof(*pdEx), _dcompare) - pdEx_Begin;
    ...
    void*         pAuxArg)
{
    ...
    *pdExProb = 0.;
    ...
    _ASSERTE(
        pComm->m_pnExIndicator_RBegin[nI] >= nX1_Begin &&
        nX1_End >= pComm->m_pnExIndicator_REnd[nI]);
    _ASSERTE(
        pComm->m_pnExIndicator_CBegin[nI] >= nX3_Begin &&
    ...
    for (nX1 = nX1_Begin; nX1 < nX1_End; ++nX1)
    {
        for (nX3 = nX3_Begin; nX3 < nX3_End; ++nX3)
        {
            ...
            _ASSERTE(
                nX1 < pComm->m_pnExIndicator_RBegin[nI] || nX1 >= pComm->m_pnExIndicator_REnd[nI]
                    ? dIndiator == 0.
                    : 1);
            _ASSERTE(
                nX3 < pComm->m_pnExIndicator_CBegin[nI] || nX3 >= pComm->m_pnExIndicator_CEnd[nI]
                    ? dIndiator == 0.
                    : 1);
    ...
    double* pdExProb_Begin)
{
    ...
    double*   pdTKnot_Begin =
        memcpy(_alloca((nSize + 1) * sizeof(double)), pdT_Begin, nSize * sizeof(double));
    int* pnSizeX1_Begin_ =
        memcpy(_alloca((nSize + 1) * sizeof(int)), pnSizeX1_Begin, nSize * sizeof(int));
    int* pnSizeX3_Begin_ =
        memcpy(_alloca((nSize + 1) * sizeof(int)), pnSizeX3_Begin, nSize * sizeof(int));
    double *    pdTotalProb = _alloca(nSize * sizeof(double)), *pdExProb = pdExProb_Begin;
    const char* szErr = 0;

    _GenMidAt_ExProb_Comm Comm = {
        0,
        pdT_Begin,
        pdT_End,
        pdVarX1_Begin,
    ...
    double* pdExProb)
{
    ...
    const int nNumEx = pdEx_End - pdEx_Begin;
    double*   pdT_Begin =
        memset(_alloca((nNumEx + 1) * sizeof(double)), 0, (nNumEx + 1) * sizeof(double));
    const char* szErr = 0;

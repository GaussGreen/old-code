...
Err		err = NULL;
	...
	else
	{
		...
		if (output == SRT_LOGNORMAL)
		{
			...
			{
				SigmaBeta = op_sabrgen_calib(Forward,
											Forward,						
											Maturity,
											Sigma,
											Alpha,
							...
							double *price)
{
	...
	err = srt_f_optsarbvol(forward, strike, maturity, volinput, Alpha, Beta, Rho, input, output, &volK);
	priceK = srt_f_optblksch(forward, strike, volK, maturity, 1.0, SRT_PUT, SRT_PREMIUM);

	err = srt_f_optsarbvol(forward, strike + putSpreadWidth, maturity, volinput, Alpha, Beta, Rho, input, output, &volKdK);
	priceKdK = srt_f_optblksch(forward, strike + putSpreadWidth, volKdK, maturity, 1.0, SRT_PUT, SRT_PREMIUM);
...
Err HardLimitStructurePriceGen(double	forward,
							/*	Option specs	*/
							double	strike,
							double	maturity,
							double	putSpreadWidth,
					...
					double *AlphaHardLimit)
{
	...
	Alpha_max = 1.0;
	err = HardLimitStructurePrice(forward,
									strike,
									maturity,
									putSpreadWidth,
									volinput,
...
Err SABRGenHardLimitDichoGen(double	forward,
					/*	Option specs	*/
					double	strike,
					double	maturity,
					double	putSpreadWidth,
...
//		Params2[HardLimType] = vol_local(0.01,Alpha,a,b,c,Rho,20+HardLimType);
	...
	err = HardLimitStructurePriceGen(forward,
									strike,
									maturity,
									putSpreadWidth,
									volinput,
...
Err alphaHardLimitDichoGen(double	forward,
					/*	Option specs	*/
					double	strike,
					double	maturity,
					double	putSpreadWidth,
...
Err delayHardLimitDichoGen(double	forward,
					/*	Option specs	*/
					double	strike,
					double	maturity,
					double	putSpreadWidth,
...
double					*Rho_sens)
{
	...
	if (err = op_bvmsabr_pricing (	forward,
								strike,
								maturity,
								disc,
								call_put,
...
double					*sens)
{
	...
	*sens = fac * vega1;
	...
	if (vega_volga == 1)
	{
		if (err = op_bvmsabr_vega_volga (	forward,
										strike,
										maturity,
										disc,
										call_put,
					  ...
					  int nbr_paramSABR)
{
	...
	Betavol = op_sabrgen_calib(	forward,
								strike,
								maturity,
								ATMvol,
								Alpha,
	...
	{
		err=op_bvmsabr_pricing(	forward,
								strike,
								maturity,
								1,
								SRT_CALL,
		...
		err=op_bvmsabr_model_sens(	forward,
									strike,
									maturity,
									1,
									SRT_CALL,
...
//		err=op_sabr_vega_volga(
		err=op_bvmsabr_vega_volga(	forward,
									strike,
									maturity,
									1,
									SRT_CALL,
					...
					double *fitting_error)
{
	...
	{
		market_prices[i] = srt_f_optblksch (	
								forward, 
								strikes[i], 
								market_vols[i], 
								maturity, 

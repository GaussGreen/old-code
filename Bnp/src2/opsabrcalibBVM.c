    ...
    double*          vol)
{
    ...
    {
        res = op_sabrgen(Forward, Strike, Maturity, Sigma, Alpha, Delay, 0, 0, Rho, vol_bvm);
    ...
    else
    {
        if (input == SRT_NORMAL)
        {
            ...
            err = srt_f_optsarbvol(
                Forward, Strike, Maturity, Sigma, 0, 0, 0, SRT_BETAVOL, SRT_LOGNORMAL, &Sigma);
            if (err)
            {
                goto FREE_RETURN;
            }
        ...
        if (output == SRT_LOGNORMAL)
        {
            ...
            {
                SigmaBeta = op_sabrgen_calib(
                    Forward, Forward, Maturity, Sigma, Alpha, Delay, 0, 0, Rho, vol_bvm);

                res = op_sabrgen(
                    Forward, Strike, Maturity, SigmaBeta, Alpha, Delay, 0, 0, Rho, vol_bvm);
            }
        }
        else
        {
            res = op_sabrgen_calib(
                Forward, Strike, Maturity, Sigma, Alpha, Delay, 0, 0, Rho, vol_bvm);
        }
    }
    ...
    double*          vol)
{
    ...
    {
        modAlpha = Alpha * sqrt((1 - exp(-2 * Lambda * Maturity)) / (2 * Lambda * Maturity));
    }

    err = srt_f_optbvmsabrvol(
        Forward, Strike, Maturity, Sigma, modAlpha, Delay, Rho, input, output, vol);

    return err;
}

Err HardLimitStructurePrice(
    double forward,
    /*	Option specs	*/
    double strike,
    double maturity,
    ...
    double* price)
{
    ...
    err = srt_f_optsarbvol(
        forward, strike, maturity, volinput, Alpha, Beta, Rho, input, output, &volK);
    priceK = srt_f_optblksch(forward, strike, volK, maturity, 1.0, SRT_PUT, SRT_PREMIUM);

    err = srt_f_optsarbvol(
        forward,
        strike + putSpreadWidth,
        maturity,
        volinput,
        Alpha,
        Beta,
        ...
        output,
        &volKdK);
    priceKdK = srt_f_optblksch(
        forward, strike + putSpreadWidth, volKdK, maturity, 1.0, SRT_PUT, SRT_PREMIUM);
    ...
    double* price)
{
    ...
    volK   = op_sabrgen(forward, strike, maturity, volinput, Alpha, a, b, c, Rho, vol_local);
    priceK = srt_f_optblksch(forward, strike, volK, maturity, 1.0, SRT_PUT, SRT_PREMIUM);

    volKdK = op_sabrgen(
        forward, strike + putSpreadWidth, maturity, volinput, Alpha, a, b, c, Rho, vol_local);
    priceKdK = srt_f_optblksch(
        forward, strike + putSpreadWidth, volKdK, maturity, 1.0, SRT_PUT, SRT_PREMIUM);
    ...
    double* AlphaHardLimit)
{
    ...
    Alpha1 = DMIN(Alpha_max, Alpha1 - price1 * (Alpha2 - Alpha1) / (price2 - price1));
    if (Alpha1 < 0)
    {
    ...
    double*          HardLimit)
{
    ...
    //		Params1[HardLimType] = vol_local(0.01,Alpha,a,b,c,Rho,10+HardLimType);
    //		Params2[HardLimType] = vol_local(0.01,Alpha,a,b,c,Rho,20+HardLimType);
    ...
    else
    {
        ...
        *HardLimit             = ParamsOpt[HardLimType];
        ...
        while ((compt < 20) && (fabs(price) > 1e-8))
        {
            ParamsOpt[HardLimType] = 0.5 * (Params1[HardLimType] + Params2[HardLimType]);
    ...
    double* prem)
{
    ...
    Betavol =
        op_sabrgen_calib(forward, forward, maturity, atmvol, Alpha, Delay, 0, 0, Rho, vol_bvm);

    *prem = op_sabrgen(forward, strike, maturity, Betavol, Alpha, Delay, 0, 0, Rho, vol_bvm);
    ...
    double* Rho_sens)
{
    ...
    if (err = op_bvmsabr_pricing(
            forward, strike, maturity, disc, call_put, Alpha, delay, Rho, frozen_vol, &prem))
    {
        return err;
    }
    ...
    double* sens)
{
    ...
    /*	Calculate premium before shift	*/
    if (err = op_bvmsabr_pricing(
            forward, strike, maturity, disc, call_put, Alpha, Delay, Rho, bump_vol, &prem))
    {
        return err;
    }
    ...
    double strike, double paramSABR[], double* price, double* gradient, int nbr_paramSABR)
{
    ...
    Betavol = op_sabrgen_calib(forward, strike, maturity, ATMvol, Alpha, Delay, 0, 0, Rho, vol_bvm);
    ...
    {
        err = op_bvmsabr_pricing(
            forward, strike, maturity, 1, SRT_CALL, Alpha, Delay, Rho, ATMvol, price);

...
{
    sDeal->sFunding = (cmscts_funding*)calloc(1, sizeof(cmscts_funding));
    sDeal->sExotic  = (crif_exotic*)calloc(1, sizeof(crif_exotic));
    sDeal->sCall    = (crif_call*)calloc(1, sizeof(crif_call));
...
Err crif_fill_funding(CRIF_DEAL sDeal, CMSCTS_MARKET sMarket)
{
    ...
    while (iStartIndex < sFunding->iNbCpn &&
           sFunding->lPayDate[iStartIndex] < sMarket->lToday + sMarket->iEODPayFlag)
    {
        iStartIndex++;
    }
    ...
    for (i = iStartIndex; i < sFunding->iNbCpn; i++)
    {
        ...
        {
            sAux->dInitSpread[i] =
                swp_f_spread(sFunding->lStartDate[i], sFunding->lPayDate[i], sFunding->cRefRate);
            sAux->dNewSpread[i] =
                swp_f_spread(sFunding->lStartDate[i], sFunding->lPayDate[i], sFunding->cRefRate);
        }
    }
    ...
    sAux->dCashFx = sMarket->dFundSpotFx *
    ...
    for (i = iStartIndex; i < sFunding->iNbCpn; i++)
    {
        sAux->dCouponPlusEx[i] =
            (sFunding->dCoverage[i] * (sAux->dMargin[i] + sAux->dNewSpread[i]) - 1.0) *
            sAux->dNot[i];
...
Err crif_exotic_allocate_aux(CRIF_EXOTIC sExotic)
{
    ...
    sExotic->sAux = (crif_exotic_aux*)calloc(1, sizeof(crif_exotic_aux));
    sAux          = sExotic->sAux;
    if (!sAux)
        return "Memory allocation faillure in crif_exotic_allocate_aux";

    sAux->iFloatingType = (int*)calloc(sExotic->iNbCpn, sizeof(int));
    sAux->iHasFloorCap  = (int*)calloc(sExotic->iNbCpn, sizeof(int));
    sAux->dMarketValue  = (double*)calloc(sExotic->iNbCpn, sizeof(double));
    sAux->dCVStrike     = (double*)calloc(sExotic->iNbCpn, sizeof(double));
...
Err crif_fill_exotic(CRIF_DEAL sDeal, CMSCTS_MARKET sMarket, CRIF_CPN_NUMPARAMS sNumerParams)
{
    Err             err = NULL;
    CRIF_EXOTIC     sExotic;
    CRIF_EXOTIC_AUX sAux;
    ...
    while (iStartIndex < sExotic->iNbCpn &&
           sExotic->lPayDate[iStartIndex] < sMarket->lToday + sMarket->iEODPayFlag)
    {
        iStartIndex++;
    }
...
Err crif_call_allocate_aux(CRIF_CALL sCall)
{
    ...
    sCall->sAux = (crif_call_aux*)calloc(1, sizeof(crif_call_aux));
    sAux        = sCall->sAux;
    if (!sAux)
        return "Memory allocation faillure in crif_call";

    sAux->iStartCpnIdx  = (int*)calloc(sCall->iNbCall, sizeof(int));
    sAux->iStartFundIdx = (int*)calloc(sCall->iNbCall, sizeof(int));
...
Err crif_fill_call(CRIF_DEAL sDeal, CMSCTS_MARKET sMarket)
{
    ...
    while (iStartIndex < sCall->iNbCall &&
           sCall->lExeDate[iStartIndex] < sMarket->lToday + sMarket->iEODExeFlag)
    {
        iStartIndex++;
    }
    ...
    CRIF_DEAL sDeal, CMSCTS_MARKET sMarket, CRIF_CPN_NUMPARAMS sNumerParams)
{
    ...
    if (sDeal->sCall->iNbCall > 0 && sDeal->sCall->iIsExercised)
    {
        sFunding->sAux->iEndIndex =
            sDeal->sCall->sAux->iStartFundIdx[sDeal->sCall->sAux->iIndexExercised] - 1;
    }
    ...
    if (sDeal->sCall->iNbCall > 0 && sDeal->sCall->iIsExercised)
    {
        sExotic->sAux->iEndIndex =
            sDeal->sCall->sAux->iStartCpnIdx[sDeal->sCall->sAux->iIndexExercised] - 1;
    }
    ...
    {
        // no closed form for the underlying so need to evaluate numerically from the the coupon
        // fixing aftertoday
    ...
    if (sExotic->sAux->iNumStartIndex < sExotic->iNbCpn)
    {
        ...
        {
            err = serror(
                "Exotic coupon number %d does not have a funding equivalent",
                sExotic->sAux->iNumStartIndex);
            goto FREE_RETURN;
        }
    }
...
{
    sPricingParams->sCouponPricingParams =
        (crif_cpn_numparams*)calloc(1, sizeof(crif_cpn_numparams));
    sPricingParams->sSimulParams = (cmscts_simul_params*)calloc(1, sizeof(cmscts_simul_params));
    sPricingParams->sReserveParams =
        (cmscts_reserve_params*)calloc(1, sizeof(cmscts_reserve_params));
    ...
    sPricingParams->sSimulParams->sLGMSVParams = (LGMSVParam*)calloc(1, sizeof(LGMSVParam));

// Expansion.cxx : Automatic expansion pricing for stoch vol models
...
static Err OldFillStructZ(OldSControl *ctrl, long iZ) {
  ...
  if ((pZ = ctrl->pool[hZ]) == NULL) {
    ...
    for (i = 0; i < 2; i++) {
      ...
      pZ->Z[i].cxxoef = dmatrix(0, ctrl->maxmono - 1, 0, ctrl->nstp - 1);

      if (!pZ->Z[i].mono || !pZ->Z[i].cxxoef)
        return serror("Memory failure");

      memset(&pZ->Z[i].cxxoef[0][0], 0,
      ...
      for (i = 0; i < nfac; i++) {
        if (ctrl->pool[hfac[i]]->volZ[ki].n_mono > 0) {
          if (!pZ->volZ[ki].mono) {
            ...
            if (!pZ->volZ[ki].mono || !pZ->volZ[ki].cxxoef)
              return serror("Memory failure");

            memset(&pZ->volZ[ki].cxxoef[0][0], 0,
                   ctrl->maxmono * ctrl->nstp * sizeof(double));
            memset(ctrl->local_pool, -1, ctrl->pool_size * sizeof(int));
          }
       ...
       di++) // allocate memory for equations for each power of u
  {
    ...
    dZ[di].cxxoef = dmatrix(0, ctrl->maxmono - 1, 0, ctrl->nstp - 1);

    if (!dZ[di].mono || !dZ[di].cxxoef)
      return serror("Memory failure");
  }
  ...
  for (i = 0; i < 2; i++) {
    for (j = 0; j < pZ->Z[i].n_mono; j++) {
      ...
      if (imono == 0) {
        ...
        for (k = 0; k < ctrl->nstp; k++)
          dZ[i].cxxoef[dZ[i].n_mono][k] = pZ->Z[i].cxxoef[j][k];
        dZ[i].n_mono++;
        ...
        for (di = 0; di < ctrl->pool[hmono]->nd; di++) {
          ...
          for (k = 0; k < ctrl->nstp; k++)
            dZ[i + di].cxxoef[dZ[i + di].n_mono][k] = pZ->Z[i].cxxoef[j][k];
          dZ[i + di].n_mono++;
...
Err OldInitControl(OldSControl *ctrl, int nfac, int nbit, int nbr, int nstp,
  ...
  for (i = 0; i < nfac; i++) {
    ...
    for (j = 0; j < 2; j++) {
      ...
      pZ->Z[j].cxxoef = dmatrix(0, maxmono - 1, 0, nstp - 1);
      if (!pZ->Z[j].mono || !pZ->Z[j].cxxoef)
        return serror("Memory failure");

      memset(&pZ->Z[j].cxxoef[0][0], 0, maxmono * nstp * sizeof(double));
    }

    for (j = 0; j < nbr; j++) {
      pZ->volZ[j].mono = (long *)calloc(maxmono, sizeof(long));
      pZ->volZ[j].cxxoef = dmatrix(0, maxmono - 1, 0, nstp - 1);
      if (!pZ->volZ[j].mono || !pZ->volZ[j].cxxoef)
        return serror("Memory failure");

      memset(&pZ->volZ[j].cxxoef[0][0], 0, maxmono * nstp * sizeof(double));
    }
  }
...
void OldFreeControl(OldSControl *ctrl) {
    ...
    for (i = 0; i < ctrl->pool_pos; i++)
      if (ctrl->pool[i]) {
        for (j = 0; j < 2; j++) {
          ...
          if (ctrl->pool[i]->Z[j].cxxoef)
            free_dmatrix(ctrl->pool[i]->Z[j].cxxoef, 0, ctrl->maxmono - 1, 0,
                         ctrl->nstp - 1);
        }
        for (j = 0; j < ctrl->nbr; j++) {
          free(ctrl->pool[i]->volZ[j].mono);
          if (ctrl->pool[i]->volZ[j].cxxoef)
            free_dmatrix(ctrl->pool[i]->volZ[j].cxxoef, 0, ctrl->maxmono - 1, 0,
                         ctrl->nstp - 1);
        }
        free(ctrl->pool[i]);
      }
  ...
  if (ctrl->d_ODE)
    for (i = 0; i < ctrl->d_pos; i++) {
      ...
      if (ctrl->d_ODE[i].cxxoef)
        free_dmatrix(ctrl->d_ODE[i].cxxoef, 0, ctrl->maxmono - 1, 0,
                     ctrl->nstp - 1);
    }
  free(ctrl->d_ODE);
...
Err sabr_test_get_d(double sigma, double alpha, double rho, double t,
  ...
  pZ->Z[0].cxxoef[0][0] = -0.5 * sigma * sigma;

  pZ->volZ[0].n_mono = 1;
  pZ->volZ[0].mono[0] = 0; // const
  pZ->volZ[0].cxxoef[0][0] = sigma;
...
Err heston_test_get_d(double sigma, double gamma, double alpha, double rho,
  ...
  pZ->volZ[1].n_mono = 1;
  pZ->volZ[1].mono[0] = X1;
  pZ->volZ[1].cxxoef[0][0] = 0.5 * alpha;
  ...
  pZ->volZ[0].n_mono = 1;
  pZ->volZ[0].mono[0] = X1;
  pZ->volZ[0].cxxoef[0][0] = 0.5 * sigma;
  ...
  pZ->volZ[0].cxxoef[1][0] = -sigma / 8.0;
...
/*
Err sabr_test_get_all_d(double sigma        , double alpha        , double rho
, double t , double **d        , int nd)
{
        Err				err = NULL;
        OldSControl		C;
        ...
        double			y[MAXNEQ];
        int				i        , ki        , kj;
        long			hY0        , hX1        , hX2        , hY1V ,
hY1D        , hY2V        , hY2D        , hY1VY1V        , hY1VY1D        ,
hY1DY1D        , hZ;

        memset(&C        , 0        , sizeof(OldSControl));

        err = OldInitControl(&C        , 7        , 2        , 2        , 1 ,
100        , MAXNEQ); if (err) goto FREE_RETURN;
        ...
        for (i=0; i < C.nfac; i++)
        {
                ...
                pZ = C.pool[hZ];
                memset(C.local_pool        , -1        , C.pool_size *
sizeof(int));
...
, 1.0        , C.rho[ki][kj]        , 0        , &pZ->volZ[ki]        ,
&C.pool[hY0]->volZ[kj] ); if (err) goto FREE_RETURN;
                        }
                }
        }
...
Err sabr_test4_get_d(double sigma, double alpha, double rho, double t,
  ...
  pZ->Z[0].cxxoef[1][0] = -sigma * sigma;
  pZ->Z[0].mono[2] = X2X2;
  pZ->Z[0].cxxoef[2][0] = -0.5 * sigma * sigma;

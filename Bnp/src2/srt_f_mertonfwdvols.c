...
Err free_resetableperiodfwdvols(
    double** paramperiod,
    double*  periodmat,
    double*  DRStsStart,
    double*  DRSts,
    ...
    long*   ia)
{
    ...
    while (s <= Strikesup)
    {
        ...
        if (err = swp_f_vol(
                szVolCurveName, (double)dDatespot, (double)dDatespot3, s, &(vols[i]), &lognorm))
        {
            smessage("Error in swp_f_vol");
        }
    ...
    err = optcalibmerton(
        dFra, NMAXSTRIKES, strikes, vols, param, mat, "Lognormal", &chisq, ia, calibvols);
...
Err resetablecalibfwdvols_timedep(
    long     today,
    int      n_resetdates,
    double*  resetdates,
    char*    cMarketId,
...

{
    ...
    /* First we calibrate the parameters to the caplet smile whose maturity corresponds to the end
    date of the resetable to get the "best" intensities*/
...
Err get_paramfwdvols(double Fradate, double* dates, double** paramatrix, double* paramfra)
{
    Err    err = NULL;
    double datespread;
    ...
    paramfra[1] = (paramatrix[2][1] - paramatrix[1][1]) * (Fradate - dates[1]) / datespread +
                  paramatrix[1][1];

    paramfra[2] = paramatrix[1][2];

    paramfra[3] = (paramatrix[2][3] - paramatrix[1][3]) * (Fradate - dates[1]) / datespread +
                  paramatrix[1][3];

    paramfra[4] = paramatrix[1][4];

    paramfra[5] = (paramatrix[2][5] - paramatrix[1][5]) * (Fradate - dates[1]) / datespread +
                  paramatrix[1][5];

    return err;
}

Err get_param_fra_before_resetfwdvols(
    double Fradate, double* dates, double** paramatrix, double* paramfra)
{
    Err    err = NULL;
    double datespread;
...
Err get_param_fra_after_resetfwdvols(
    double   Fradate,
    double   today,
    double   fra,
    double   frafwd,
    ...
    double*  paramfra)
{
    ...
    paramfra[3] =
        ((dates[3] - today) * paramatrix[2][3] / datespread -
         (dates[1] - today) * (paramatrix[1][3] / (1 + paramfra[2] * cvg * fra / (1 + cvg * fra))) /
             datespread) *
    ...
    paramfra[5] =
        ((dates[3] - today) * paramatrix[2][5] / datespread -
         (dates[1] - today) * (paramatrix[1][5] / (1 + paramfra[4] * cvg * fra / (1 + cvg * fra))) /
             datespread) *
...
Err DRSCalcfwdvols(
    double* wn1n2, double resetmat, double** periodparam, double* drift, double* DRSts)
{
    Err err = NULL;
    ...
    double*    res)
{
    ...
    if (err)
    {
        free_resetableperiodfwdvols(
            paramperiod, dates, DRStsStart, DRSts, drift_before_reset, drift_after_reset);
        free_resetablegeneralfwdvols(wn1n2, paramatrix);
        return err;
    }
    ...
    cvg = coverage(
        (long)dates[3],
        add_unit((long)dates[3], (int)(12 / float_compounding), SRT_MONTH, MODIFIED_SUCCEEDING),
        float_basis);
    /* here, we get the parameters corresponding to each fixing day within the period and put them
     in paramperiod which is a matrix with nfixdays rows and 5 columns (for the 5 parameters)*/
    ...
    else
    {
        ...
        drift =
            paramatrix[1][1] * cvg * dFradrift / (1 + cvg * dFradrift) + paramatrix[1][3] +
            paramatrix[1][5] -
            paramatrix[1][3] / (1 + paramatrix[1][2] * cvg * dFradrift / (1 + cvg * dFradrift)) -
            paramatrix[1][5] / (1 + paramatrix[1][4] * cvg * dFradrift / (1 + cvg * dFradrift));

        for (j = 1; j <= 3; j++)
        {
            err = get_param_fra_before_resetfwdvols(dates[j], dates, paramatrix, paramperiod[j]);
    ...
    /* maximum values for the number of positive and negative jumps are computed for the resetdate
    (start date) of the period in question */
...

    {
        ...
        {
            err = get_param_fra_after_resetfwdvols(
                dates[j], today, dFradrift, dFraFwddrift, cvg, dates, paramatrix, paramperiod[j]);

            drift_after_reset[j] =
                paramperiod[j][3] *
                (1 - 1 / (1 + paramperiod[j][2] * cvg * dFraFwddrift / (1 + cvg * dFraFwddrift)));
            drift_after_reset[j] +=
                paramperiod[j][5] *
                (1 - 1 / (1 + paramperiod[j][4] * cvg * dFraFwddrift / (1 + cvg * dFraFwddrift)));

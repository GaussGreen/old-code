...
Err free_resetableperiodfwdvols(double **paramperiod,double *periodmat, double *DRStsStart,
			   double *DRSts,double *driftbefore, double *driftafter)

{
	if (paramperiod) free_dmatrix (paramperiod, 1,3, 1, 5);
	paramperiod = NULL;
	
	if (periodmat) free_dvector (periodmat, 1, 3);
	periodmat = NULL;
	
	if (DRSts) free_dvector (DRSts, 1, 3);
	DRSts = NULL;

	if (DRStsStart) free_dvector (DRSts, 1, 3);
	DRStsStart = NULL;

	
	if (driftbefore) free_dvector (driftbefore, 1, 3);
	driftbefore = NULL;

	if (driftafter) free_dvector (driftafter, 1, 3);
	driftafter = NULL;

	return NULL;
}	

Err free_resetablegeneralfwdvols(double *wn1n2, 
						  double **paramatrix)
{
	...
	if (paramatrix) free_dmatrix (paramatrix, 1,2,1,5);
	paramatrix = NULL;
...
Err calibtosmilefwdvols(  long today,
				   long dDate,
				   char *cMarketId,
				   char *szVolCurveName,
				   char *cRefRateCode,
...
if (err = swp_f_vol(szVolCurveName,(double) dDatespot, (double) dDatespot3,s,&(vols[i]),&lognorm))
{
	smessage("Error in swp_f_vol");
}
...
err = optcalibmerton(dFra,NMAXSTRIKES,strikes,vols,param,mat,"Lognormal",&chisq,ia,calibvols);
...
Err resetablecalibfwdvols(long today, int n_resetdates, 
				   double *resetdates,
				   char *cMarketId,
				   char *szVolCurveName,
				   char *cRefRateCode,
...
err = calibtosmilefwdvols(today,(long) resetdates[n_resetdates],
				   cMarketId,
				   szVolCurveName,
				   cRefRateCode,
				   szYieldCurveName,
...
/* First we calibrate the parameters to the caplet smile whose maturity corresponds to the end date
of the resetable to get the "best" intensities*/
...
Err get_paramfwdvols(double Fradate, double *dates, 
...
*(Fradate-dates[1])/datespread+paramatrix[1][1];

paramfra[2] = paramatrix[1][2];

paramfra[3] = (paramatrix[2][3]-paramatrix[1][3])
*(Fradate-dates[1])/datespread+paramatrix[1][3];



paramfra[4] = paramatrix[1][4];

paramfra[5] = (paramatrix[2][5]-paramatrix[1][5])
*(Fradate-dates[1])/datespread+paramatrix[1][5];
...
Err get_param_fra_after_resetfwdvols(double Fradate, double today, double fra, double frafwd, double cvg, double *dates, 
			  double **paramatrix, double *paramfra)
{
Err err=NULL;
double datespread;
...
paramfra[3] = ((dates[3]-today)*paramatrix[2][3]/datespread-(dates[1]-today)*
			   (paramatrix[1][3]/(1+paramfra[2]*cvg*fra/(1+cvg*fra)))/datespread)*(1+paramfra[2]*cvg*frafwd/(1+cvg*frafwd));


paramfra[4] = paramatrix[1][4];

paramfra[5] = ((dates[3]-today)*paramatrix[2][5]/datespread-(dates[1]-today)*
			   (paramatrix[1][5]/(1+paramfra[4]*cvg*fra/(1+cvg*fra)))/datespread)*(1+paramfra[4]*cvg*frafwd/(1+cvg*frafwd));
...
Err get_DRStsfwdvols(long today, 
					 long Paydate, 
					 double *mat, 
					 char	*cRefRateCode, 
					 char	*szYieldCurveName,
					 ...
					 Err (*GetVol)(double dStart, double dEnd, double dStrike, 
									SRT_Boolean bAdjForSpread, double dForward, double dSpread,double *pdBsVol),
					 elemforCms passCms,
					 char   *UseDRS,
					 double *DRSts)
{
...
FraSpot = add_unit((long) mat[index],spotlag,SRT_BDAY,MODIFIED_SUCCEEDING);
...
enddate = add_unit(  FraSpot, (int) num_of_months, SRT_MONTH, MODIFIED_SUCCEEDING);
...
/* compute the fra */
		...
		err = get_fra((long) mat[index], spotlag, cRefRateCode, szYieldCurveName, &dFra);
	...
	/* Computes the CMS rate */
		...
		if (err=swp_f_Cms_Rate(dFra, maturity, numperiods, (double) float_compounding, 
							delay, 365.0/360.0, SRT_LOGNORMAL, 0.0, 1 /*Full Smile Approx*/,
							FraSpot, (long) theoenddate, passCms.AdjForSpread,
							passCms.AdjForSpread*dspread, szVolCurveName, passCms.NumStrikesInVol, passCms.Strikes,
							&(DRSts[index])))
		{
...
Err DRSCalcfwdvols(	  double *wn1n2,
				  double resetmat,
				  double **periodparam,
				  double *drift,
				  double *DRSts
...
err = resetablecalibfwdvols(today,n_resetdates, 
						resetdates,
						cMarketId,
						szVolCurveName,
						cRefRateCode,
...
impvols = dvector(1,n_strikes);
	...
	err =  get_DRStsfwdvols(today, (long) dates[3], dates, cRefRateCode, szYieldCurveName,
						szVolCurveName,	GetVol, passCms,UseDRS,DRStsStart);																									
	if (err)
	{
		free_resetableperiodfwdvols(paramperiod,dates,DRStsStart,DRSts, drift_before_reset, drift_after_reset);
		free_resetablegeneralfwdvols(wn1n2, paramatrix);
		return err;
	}


cvg = coverage((long) dates[3], add_unit((long) dates[3], (int) (12/float_compounding), SRT_MONTH, MODIFIED_SUCCEEDING),float_basis);
/* here, we get the parameters corresponding to each fixing day within the period and put them in
 paramperiod which is a matrix with nfixdays rows and 5 columns (for the 5 parameters)*/
	...
	if (strcmp(UseDRS,"YES")==0)
	{
		...
		for (j=1;j<=3;j++)
		{
			err=get_paramfwdvols(dates[j], dates, 
	...
	{
			err=get_fra((long) resetdates[2], spotlag, cRefRateCode, szYieldCurveName, &dFradrift);

		drift = paramatrix[1][1]*cvg*dFradrift/(1+cvg*dFradrift)+
			paramatrix[1][3]+paramatrix[1][5]-paramatrix[1][3]/(1+paramatrix[1][2]*cvg*dFradrift/(1+cvg*dFradrift))
			-paramatrix[1][5]/(1+paramatrix[1][4]*cvg*dFradrift/(1+cvg*dFradrift));
	
		for (j=1;j<=3;j++)
		{
			err=get_param_fra_before_resetfwdvols(dates[j], dates, 
				  paramatrix, paramperiod[j]);

			drift_before_reset[j] = drift*(dates[j]-dates[1])/(dates[3]-dates[1]);
...
of the period in question */
				...
				for (j=1;j<=3;j++)
				{
					err=get_param_fra_after_resetfwdvols(dates[j], today, dFradrift, dFraFwddrift, cvg, dates, 
					paramatrix, paramperiod[j]);

					drift_after_reset[j] = paramperiod[j][3]*(1-1/(1+paramperiod[j][2]*cvg*dFraFwddrift/(1+cvg*dFraFwddrift)));
					drift_after_reset[j] += paramperiod[j][5]*(1-1/(1+paramperiod[j][4]*cvg*dFraFwddrift/(1+cvg*dFraFwddrift)));
					...
					optmertonsmile(DRSts[2],n_strikes, strikes,paramperiod[2], (dates[2]-dates[1])/365.0, "Lognormal",impvols);

...
static double
_multiply_level_coupon(long lToday, const char *szYC, const long *plStart_Begin,
                       const long *plStart_End, const long *plEnd_Begin,
                       const long *plPay_Begin, SrtBasisCode eBasis,
...
static double
_leg_coupon_pv_(long lToday, const char *szYC, const long *plStart_Begin,
                const long *plStart_End, const long *plEnd_Begin,
...
static double _flt_leg(long lToday, const char *szYC, const long *plStart_Begin,
                       const long *plStart_End, const long *plEnd_Begin,
  ...
  // margins and spreads
  dResult =
      _leg_coupon_pv_(lToday, szYC, plStart_Begin, plStart_End, plEnd_Begin,
                      plPay_Begin, pdMargin_Begin, pdNotional_Begin, eBasis);
  dResult +=
      _leg_coupon_pv_(lToday, szYC, plStart_Begin, plStart_End, plEnd_Begin,
                      plPay_Begin, pdSpread_Begin, pdNotional_Begin, eBasis);
...
static double _fix_leg(long lToday, const char *szYC, const long *plStart_Begin,
                       const long *plStart_End, const long *plEnd_Begin,
                       const long *plPay_Begin, const double *pdCoupon_Begin,
                       const double *pdNotional_Begin, SrtBasisCode eBasis) {
  return _leg_coupon_pv_(lToday, szYC, plStart_Begin, plStart_End, plEnd_Begin,
                         plPay_Begin, pdCoupon_Begin, pdNotional_Begin, eBasis);
}

static double
_equi_strike(long lToday, const char *szYC, const long *plFixStart_Begin,
  ...
  const double dFltPV =
      _flt_leg(lToday, szYC, plFltStart_Begin, plFltStart_End, plFltEnd_Begin,
               plFltPay_Begin, pdMargin_Begin, pdSpread_Begin,
               pdFltNotional_Begin, eFltBasis);

  const double dFixPV = _fix_leg(
      lToday, szYC, plFixStart_Begin, plFixStart_End, plFixEnd_Begin,
  ...
  const double dMarginLevel = _multiply_level_coupon(
      ...
      plFltPay_Begin, eFltBasis, pdMargin_Begin);

  const double dSpreadLevel = _multiply_level_coupon(
      lToday, szYC, plFltStart_Begin, plFltStart_End, plFltEnd_Begin,
      plFltPay_Begin, eFltBasis, pdSpread_Begin);

  const double dFixLevel =
      _multiply_level_coupon(lToday, szYC, plFixStart_Begin, plFixStart_End,
                             plFixEnd_Begin, plFixPay_Begin, eFixBasis, 0);
...
static double _equi_notional(long lToday, const char *szYC,
                             const long *plStart_Begin, const long *plStart_End,
                             const long *plEnd_Begin, const long *plPay_Begin,
  ...
  const double dFltPV = _flt_leg(lToday, szYC, plStart_Begin, plStart_End,
                                 plEnd_Begin, plPay_Begin, pdMargin_Begin,
                                 pdSpread_Begin, pdNotional_Begin, eBasis);

  const double dLevelMargin =
      _multiply_level_coupon(lToday, szYC, plStart_Begin, plStart_End,
                             plEnd_Begin, plPay_Begin, eBasis, pdMargin_Begin);

  const double dLevelSpread =
      _multiply_level_coupon(lToday, szYC, plStart_Begin, plStart_End,
                             plEnd_Begin, plPay_Begin, eBasis, pdSpread_Begin);
...
static Err _LognormalSwapVol(long lToday, const char *szYC, const char *szVC,
                             const char *szFreq, const char *szBasis,
                             const char *szRefRate, long lEx, long lStart,
                             long lEnd, double dStrike, double *pdVol) {
  ...
  if (dPower < 1.) {
    szErr = swp_f_ForwardRate(lStart, lEnd, (char *)szFreq, (char *)szBasis,
                              (char *)szYC, (char *)szRefRate, &dFRA);
    if (szErr)
      return szErr;
    dPrice =
        srt_f_optblknrm(dFRA, dStrike, *pdVol, dEx, 1., SRT_CALL, SRT_PREMIUM);
    szErr = srt_f_optimpvol(dPrice, dFRA, dStrike, dEx, 1, SRT_CALL,
                            SRT_LOGNORMAL, pdVol);
    if (szErr)
      return szErr;
  }
...
Err amortMidat_compute_diagonal_prices_new_equistrike(
    char *yc, char *vc, char *ref, char *cFreq, char *cBasis, long today,
    int eod_flag, double mintime, double mininterval, int UseVol, int *nex,
    int *firstex, int **ex_bool, double **diag_prices, double *pdequinotional,
    double *pdlong_strikes, int model,
    Err (*get_correl)(char *vol_curve_name, double start_date, double end_date,
                      double strike, double *vol),
    char *CorrelName,
    Err (*get_cash_vol)(char *vol_curve_name, double start_date,
  ...
  for (i = 0; i < lNFixNot; ++i) {
    ...
    dCoverages[i + 1] = am->fix_leg->cpn[i].cxxvg;
  ...
  *diag_prices = (double *)calloc(*nex, sizeof(double));
  ...
  dEquiv_Notional = _equi_notional(
      today, yc, lFloatStartDates, lFloatStartDates + lNFloatNot,
      lFloatEndDates, lFloatEndDates, dMargins, dSpreads, dFloatNotionals,
  ...
  *pdequinotional = dEquiv_Notional;
  ...
  if (am->fix_leg->cpn[j].start_time < DMAX(mintime, am->call[0].ex_time)) {
    ...
    dEqui_Strike = _equi_strike(
        today, yc,
        //// fixed leg
        lFixStartDates + j, lFixStartDates + lNFixNot, lFixEndDates + j,
        lFixEndDates + j, dFixRates + j, dFixNotionals + j,
        srtBasis, // eFixBasis        ,
        /// floating leg
        lFloatStartDates + l, lFloatStartDates + lNFloatNot, lFloatEndDates + l,
        lFloatEndDates + l, dMargins + l, dSpreads + l, dFloatNotionals + l,
        srtBasis, // eFltBasis; wrong !!!
        dEquiv_Notional);
    ...
    /// get vol
    err = _LognormalSwapVol(today, yc, vc, cFreq, cBasis, ref,
                            *(lFloatStartDates + l), *(lFloatStartDates + l),
                            *(lFloatEndDates + lNFloatNot - 1), dEqui_Strike,
                            &dVol);

    /// get pricer
    err =
        swp_f_Swaption(*(lFloatStartDates + l),
                       *(lFloatEndDates + lNFloatNot - 1), cFreq, cBasis, dVol,
                       dEqui_Strike, //// wrong ! must adjust for exercise fee
                       cRec,         // szPayRec        ,
                       ref, yc, "PREMIUM", "LOGNORMAL", &dPrice);
    ...
    /*		if(model==1)
                    {
                            ...
                            if(err)
                            {
                                    smessage("Pb in pricing European with
       Shifted Log SMM"); goto FREE_RETURN;
                            }

                            if(Correl)
                            {
                                    free_dmatrix(Correl        , 0        ,
       NDimCorrel-1        ,0 , NDimCorrel-1); Correl = NULL;
                            }
                    }
                    else
                    {
                            ExDate = add_unit (StartDate        , -2        ,
       SRT_BDAY        , MODIFIED_SUCCEEDING); err =
       EuropeanAmortizingSwaption2(today        , ExDate       , EndDate ,
                                                                    lNFixNot-j ,
                                                                    lStartDates+j
             , lEndDates+j        , lPayDates+j        , dCoverages+j        ,
       dCoverages+j        , dFixRates[0] , dFixNots+j        , yc        , vc
       , cFreq        , cBasis        , cRec        , ref        , get_cash_vol
             ...
             , dvReplicatingNotionals        , &dPrice        , &dFixedPV ,
       &dFloatPV        , &dSwapRate       , &lgmSigKapTSPtrPtr , &dLGMVol ,
       dvReplicatingSwaptions        ,

                                                                    //Inputs
                                                                    pdMargins+j
             , 0        ,//dFixRates+j        , 0        , exer_fee); if(err)
                            {
  ...
  for (k = 1; k < *nex; ++k) {
    if ((am->fix_leg->cpn[j + k].start_time - lastcaltime < mininterval) ||
      ...
      /// currently        , equivalent strike
      dEqui_Strike =
          _equi_strike(today, yc,
                       //// fixed leg
                       lFixStartDates + k + j, lFixStartDates + lNFixNot,
                       lFixEndDates + k + j, lFixEndDates + k + j,
                       dFixRates + k + j, dFixNotionals + k + j,
                       srtBasis, // eFixBasis        ,
                       /// floating leg
                       lFloatStartDates + l, lFloatStartDates + lNFloatNot,
                       lFloatEndDates + l, lFloatEndDates + l, dMargins + l,
                       dSpreads + l, dFloatNotionals + l,
                       srtBasis, // eFltBasis; wrong !!!
                       dEquiv_Notional);
      ...
      /// get pricer
      err = swp_f_Swaption(
          *(lFloatStartDates + l), *(lFloatEndDates + lNFloatNot - 1), cFreq,
          cBasis, dVol,
          dEqui_Strike, //// wrong ! must adjust for exercise fee
          cRec,         // szPayRec        ,
          ref, yc, "PREMIUM", "LOGNORMAL", &dPrice);
      ...
      /*if(model==1)
      {
              ...
              err = Compute_CoInitalSwaps_Correl2(SpotDate        ,
                                                                              CorrelName
            , get_correl        , StartDate        , EndDate        , srtFreq ,
      srtBasis        , NDimCorrel , &Correl); if(err)
              {
                      goto FREE_RETURN;
              }
              ...
              if(Correl)
              {
                      free_dmatrix(Correl        , 0        , NDimCorrel-1 ,0 ,
      NDimCorrel-1); Correl = NULL;
              }
      }
      else
      {
              ExDate = add_unit (StartDate        , -2        , SRT_BDAY ,
      MODIFIED_SUCCEEDING); err = EuropeanAmortizingSwaption2(today        ,
      ExDate        , EndDate        , lNFixNot-(k+j)        , lStartDates+(k+j)
...
Err am_calib_und_new_reduc(
  ...
  /// Previously err = amortMidat_compute_diagonal_prices_new(yc        , vc ,
  /// ref        , swap_freq        , swap_basis        ,
  /// 1. Compute equivalent notional and strikes for the european amortizing
  /// underlyings and
  ...
  err = amortMidat_compute_diagonal_prices_new_equistrike(
      ...
      UseVol, &nex, &firstex, &ex_bool, &diag_prices, &dequinotional,
      pdlong_strikes, european_model, get_correl, CorrelName, get_cash_vol, am);

  /// overwrite the notionals
  _memset(&dequinotional, pdlong_strikes, am->fix_leg->num_cpn,
          sizeof(*pdlong_strikes));
  ...
  /// Calibrate term structure again based on the combination of amortizing and
  /// equivalent strike swaptions
  ...
  err = amortMidat_cpd_calib_diagonal_new(
      ...
      float_pay_dates, float_margin, float_spread,
...
Err am_fill_check_all_struct_AmortMidatAutocal(
  ...
  if (!am->fund_leg) {
    szErr =
        "Memory allocation error (1) in am_fill_check_all_AmortMidatAutocal";
    goto FREE_RETURN;
  }

  szErr = am_fill_fund_leg(und->today, eod_fix_flag, fund_not, fund_ncpn,
  ...
  if (!am->fix_leg) {
    szErr = "Memory allocation error (2) in "
            "am_fill_check_all_struct_AmortMidatAutocal";
    goto FREE_RETURN;
  }

  szErr = am_fill_fix_leg(und->today, eod_fix_flag, fix_not, fix_ncpn,
                          //			fix_fix        ,
  ...
  if (ncall > 0 && ex_date[ncall - 1] >= und->today + eod_ex_flag) {
    szErr = am_fill_calls(und->today, eod_ex_flag, ncall, pay_rec, ex_date,
                          set_date, fee, am);
    if (szErr) {
      goto FREE_RETURN;
    }
...
Err am_fill_check_all_struct_ts_AmortMidatAutocal(
  ...
  if (!am->fix_leg) {
    szErr =
        "Memory allocation error (2) in am_fill_check_all_AmortMidatAutocal";
    goto FREE_RETURN;
  }
  ...
  if (am->num_calls > 0 && am->call) {
    szErr = am_fill_adi_arg_ts(nlambda, pdlambda, pdlambda_time, und, am,
...
Err _genmidat_nocall_payoff_(
  ...
  //// NB: Optimzation in the time direction ; compute DF only between 2
  /// exercise dates
  /// total number of funding leg coupons
  const int num_fund_cpn =
      call->num_fund_cpn - (*func_params->pnum_fund_cpn_done);
  ...
  for (l = 0; l < num_fund_cpn; l++) {
    ...
    pdFltBase[l] =
        exp(-eval_const->df_alpha[ndf] - eval_const->df_beta[ndf] * _R1);
  ...
  for (i = l1; i <= u1; i++) {
    ...
    for (j = l2; j <= u2; j++) {
      ...
      //	Intrinsic value
      iv = call->pay_rec ? (fund_leg_pv - fix_leg_pv)
                         : (fix_leg_pv - fund_leg_pv);

      fee *= dCallFeeCoeff_R2; /// call fee
      fee_total =
          fee + (dFixFee *= dFltInitCoeff_R2); // factor fixed fee into call fee

      // update indicator function - exercise 1. and not exercise 0.
      if (nCompExProb) {
        ppdExIndicator[i][j] =
            (prod_val[i][j][0] - iv) > (-fee_total) ? 0. : 1.;
          ...
          if (nNumID <= 2) {
            ...
            double dFltLeg = 0.;
            {
              ...
              for (l = 0; l < num_fund_cpn; l++) {
                ...
                _pdFltBase[l] = exp(-eval_const->df_alpha[ndf] -
                                    eval_const->df_beta[ndf] * __R1);
              ...
              _dFltInitBase = exp(-eval_const->df_alpha[ndf] -
                                  eval_const->df_beta[ndf] * __R1);
              ...
              _dFltTermBase = exp(-eval_const->df_alpha[ndf] -
                                  eval_const->df_beta[ndf] * __R1);
              ...
              for (l = 0; l < num_fix_cpn; l++) {
                ...
                _pdFixBase[l] = exp(-eval_const->df_alpha[ndf] -
                                    eval_const->df_beta[ndf] * __R1);
      ...
      prod_val[i][j][0] = (prod_val[i][j][0] - iv) > (-fee_total)
                              ? (prod_val[i][j][0] - iv)
                              : (-fee_total);
    }
    ...
    if (nCompExProb) {
      ...
      int nFlip = 0;
      for (pdExInd = ppdExIndicator[i] + l2 + 1;
           pdExInd < ppdExIndicator[i] + u2 + 1; ++pdExInd) {
  ...
  if (nCompExProb) {
    ...
    _ASSERTE(nNum_Boundary <= (u1 - l1 - 1) > (u2 - l2 - 2) ? (u1 - l1 - 1)
                                                            : (u2 - l2 - 2));
  ...
  if (call_idx == 0) {
    fund_leg_pv = swp_f_df(func_params->lToday, call->set_date, yc) *
    ...
    for (l = 0; l < call->num_fund_cpn; l++) {
      fund_cpn = am->fund_leg->cpn + call->fund_idx + l;
      fund_leg_pv +=
          swp_f_df(func_params->lToday, fund_cpn->pay_date, yc) * fund_cpn->cpn;
    }
    ...
    for (l = 0; l < call->num_fix_cpn; l++) {
      fix_cpn = fix_leg->cpn + call->fix_idx + l;
      fix_leg_pv +=
          swp_f_df(func_params->lToday, fix_cpn->pay_date, yc) * fix_cpn->cpn;
    }
  ...
  if (nEx == call_idx) {
    ...
    for (i = l1; i <= u1; i++) {
      for (j = l2; j <= u2; j++) {
        ...
        prod_val[i][j][0] = ppdExIndicator[i][j] == 1.
                                ? 0.
                                : prod_val[i][j][0] / ppdCondiDF[i][j];
        /// TEST TEST TEST TEST TEST
...
static Err _lgm2f_adi_shrinkgrid(
    const int *plx, const int *pux, const int *plz, const int *puz,
    /*	Time data		*/
  ...
  double **r1_dim2 = NULL, **r2 = NULL, ***values = NULL, ***values_p1 = NULL,
         ...
         **var_r3 = NULL, **r = NULL, **r_init = NULL;
  ...
  */
  ...
  // volatility 	is the same        , the mesh has to be the same        , but
  // the number of steps has to be adjusted
  ...
  //	Eval payoff
  szErr = payoff_func(date[nstept - 1], time[nstept - 1],
                      func_parm_tab[nstept - 1], yc, &lam1, date, 1, gamma, rho,
                      phi1[nstept - 1], phi2[nstept - 1], phi12[nstept - 1],
                      plx[nstept - 1], pux[nstept - 1], plz[nstept - 1],
                      puz[nstept - 1], r1_dim1, r2, nprod, values_p1);
  ...
  for (step = nstept - 2; step >= 0; step--) {
    ...
    //	convolve
    num_f_pde_one_step_backward_2f_adi(pde, nstepx, r1_bar, nstepz, r3_bar, 0,
                                       nprod - 1, values_p1, mu_r1, mu_r3,
                                       var_r1, var_r3, r, values, plx[step],
                                       pux[step], plz[step], puz[step]);
    ...
    if (eval_evt[step]) {
      ...
      szErr = payoff_func(date[step], time[step], func_parm_tab[step], yc,
                          &lam1, date, 1, gamma, rho, phi1[step], phi2[step],
                          phi12[step], plx[step], pux[step], plz[step],
                          puz[step], r1_dim1, r2, nprod, values);
      if (szErr) {
        goto FREE_RETURN;
      }
    }
...
static Err _lgm2f_adi_fixgrid(
    ...
    Err (*payoff_func)(/* Event */
                       ...
                       int nprod, double ***prod_val),
    /*	Result */
    int nprod, double *res, int *plx, int *pux, int *plz, int *puz) {
  ...
  /*	Calculation of the number of steps in each direction: since local
   * volatility			*/
  /*	is the same        , the mesh has to be the same        , but the number
   * of steps has to be adjusted	*/
  ...
  smessage("Phase 2 -convolution        , stept: %d stepx: %d stepz: %d",
           nstept, nstepx, nstepz);
  ...
  *puz = uz;
  ...
  for (step = nstept - 2; step >= 0; step--) {
    ...
    /// do not reduce the number of points when computing the exercise
    /// probablities
#if 0
...
static void
_init_genmidat_arg(genmidat_arg *pAmortArg, ptr_genmidat_arg *ppAmortArg,
                   int *pdUB_D1, int *pdLB_D1, int *pdUB_D2, int *pdLB_D2,
                   int *pnum_fix_cpn_done, int *pnum_fund_cpn_done, int *pnEx,
                   double *pdInit_Prev, double *pdIV, const double *pdStrike,
                   double ***pppdExIndicator, double ***pppdCondiDF,
                   double *pdAvgSwapRate, int *pnExToTime, AM_ADI_ARG adi_arg,
                   long lToday, int nCompExProb) {
  static int _accumulate_(const int *piBegin, const int *piEnd);
  const int nNumEx =
      _accumulate_(adi_arg->is_event, (adi_arg->is_event) + adi_arg->nstp);
  int nI, nJ;
...
static Err
_compute_exercise_prob_(ptr_genmidat_arg *ppAmortArg, AM_ADI_ARG adi_arg,
                        int lx, int ux, int lz, int uz, const int *pnExToTime,
  ...
  const int nNumEx =
      _accumulate_(adi_arg->is_event, (adi_arg->is_event) + nNumStep);
  ...
  //// computing exercise probablities; bounds of the 2D grid are stored in plx
  ///      ,pux        ,plz        ,puz
  if (szErr = _lgm2f_adi_shrinkgrid(
          plx, pux, plz, puz, adi_arg->nstp, adi_arg->time, adi_arg->date,
          adi_arg->nstpx, adi_arg->lam, adi_arg->sig_time, adi_arg->sig1,
          adi_arg->nb_sig, adi_arg->alpha, adi_arg->gamma, adi_arg->rho,
          (void **)ppAmortArg, // adi_arg->void_prm        ,//
          adi_arg->is_event, adi_arg->ifr, adi_arg->yc,
          am_CondiDF_4_lgm2f_adi_fixgrid, 1, &dDummy)) {
    return szErr;
  }
  ...
  *pnEx = nJ;
  if (szErr = _lgm2f_adi_shrinkgrid(
          ...
          adi_arg->time, adi_arg->date, adi_arg->nstpx, adi_arg->lam,
          adi_arg->sig_time, adi_arg->sig1, adi_arg->nb_sig, adi_arg->alpha,
          adi_arg->gamma, adi_arg->rho,
          (void **)ppAmortArg, // adi_arg->void_prm        ,//
          adi_arg->is_event, adi_arg->ifr, adi_arg->yc,
          am_notexerprob_4_lgm2f_adi_fixgrid, 1, &pdExProb[nNumEx])) {
    return szErr;
  }
  ...
  for (nJ = nNumEx - 1; nJ >= 0; --nJ) {
    *pnEx = nJ;
    ...
    if (szErr = _lgm2f_adi_shrinkgrid(
            ...
            (void **)ppAmortArg, // adi_arg->void_prm        ,//
            adi_arg->is_event, adi_arg->ifr, adi_arg->yc,
            am_exerprob_4_lgm2f_adi_fixgrid, 1, &pdExProb[nJ])) {
      return szErr;
    }
...
static Err _price_genmidat_(AM_ADI_ARG adi_arg, // structure
                            ptr_genmidat_arg *ppAmortArg, double *pdOptionPV) {
  double _pdOptionPV[2] = {0};
  Err szErr = 0;

  /// delegate ....
  szErr = lgm2f_adi(
      adi_arg->nstp, adi_arg->time, adi_arg->date, adi_arg->nstpx, adi_arg->lam,
      adi_arg->sig_time, adi_arg->sig1, adi_arg->nb_sig, adi_arg->alpha,
      adi_arg->gamma, adi_arg->rho, (void **)ppAmortArg, adi_arg->is_event,
      adi_arg->ifr, adi_arg->yc, _genmidat_nocall_payoff_, 1, _pdOptionPV);
...
static Err _price_genmidat_ts_(
    ...
    ptr_genmidat_arg *ppAmortArg, const double *pdLambdaValue,
    const double
        *pdLambdaTime, //// NB: calendar time        , not year fraction !!!!!!!
    int nLambdaSize, double *pdOptionPV) {
  double _pdOptionPV[2] = {0};
...
static void _free_genmidat_arg(ptr_genmidat_arg *ppAmortArg,
                               const int *pnExToTime, int nNumEx, int lx,
                               int ux, int lz, int uz) {
  int nJ, nExToTime;
  double **pdmatrix = 0;
...
static Err _price_genmidat_exprob_(AM_ADI_ARG adi_arg, // structure
                                   ptr_genmidat_arg *ppAmortArg,
                                   const int *pnExToTime, double *pdOptionPV,
                                   double *pdExProb) {
  const int nNumEx =
      _accumulate_(adi_arg->is_event, (adi_arg->is_event) + adi_arg->nstp);
  double _pdOptionPV[2] = {0};
  ...
  //// that once the grid is set up - by the solver - at the termination , it
  /// remains fixed  as the solver / advances backwards in time; this variation
  /// is needed for / computing exercise probablities; bounds of the 2D grid are
  /// stored in plx        ,pux        ,plz        ,puz
  if (szErr = _lgm2f_adi_fixgrid(
          adi_arg->nstp, adi_arg->time, adi_arg->date, adi_arg->nstpx,
          adi_arg->lam, adi_arg->sig_time, adi_arg->sig1, adi_arg->nb_sig,
          adi_arg->alpha, adi_arg->gamma, adi_arg->rho, (void **)ppAmortArg,
          adi_arg->is_event, adi_arg->ifr, adi_arg->yc,
          _genmidat_nocall_payoff_, 1, _pdOptionPV, &lx, &ux, &lz, &uz)) {
  ...
  *pdOptionPV = _pdOptionPV[0];
  ...
  szErr = _compute_exercise_prob_(ppAmortArg, adi_arg, lx, ux, lz, uz,
                                  pnExToTime, pdExProb);
...
static Err _price_genmidat(AM_STR am,          // structure
                           AM_UND und,         // structure
                           AM_ADI_ARG adi_arg, // structure
                           long lToday, double dStrike, double *pdOptionPV,
                           double **ppdExProb, double **ppdAvgStrike,
                           int *pnEx) {
  // total number of exercise ;
  // NB: is_event contains {0        ,1 sequence}        , indicating occurrence
  // of an event/exercise
  const int nNumEx =
      _accumulate_(adi_arg->is_event, (adi_arg->is_event) + adi_arg->nstp);
  ...
  double dInit_Prev = 0.;
  double dIV_FirstEx; // dIV_FirstEx stores the PV of the swap with no cash flow
                      // until the first exercise date
  Err szErr;

  /// user-provided payoff function arguments
  genmidat_arg *pAmortArg =
      memset(_alloca(adi_arg->nstp * sizeof(genmidat_arg)), 0,
             adi_arg->nstp * sizeof(genmidat_arg));
  ptr_genmidat_arg *ppAmortArg =
      memset(_alloca(adi_arg->nstp * sizeof(ptr_genmidat_arg)), 0,
             adi_arg->nstp * sizeof(ptr_genmidat_arg));

  // global exercise indicator
  double ***pppdExIndicator = memset(_alloca(nNumEx * sizeof(double **)), 0,
                                     nNumEx * sizeof(double **));

  // global conditional discount factor
  // i.e. discount factors observed at time tnEx and paid at tnEx+1 (>tnEx)
  double ***pppdCondiDF = memset(_alloca(nNumEx * sizeof(double **)), 0,
                                 nNumEx * sizeof(double **));

  // global average strike
  // double* pdAvgStrike = memset(_alloca(nNumEx * sizeof(double))        ,0
  //       ,nNumEx*sizeof(double));
  ...
  int *pnExToTime = memset(_alloca(nNumEx * sizeof(int)), 0,
                           nNumEx * sizeof(int)); /// initialize to 0
  ...
  *pnEx = nNumEx;
  ...
  _init_genmidat_arg(pAmortArg, ppAmortArg, pdUB_D1, pdLB_D1, pdUB_D2, pdLB_D2,
                     &num_fix_cpn_done, &num_fund_cpn_done, &nEx, &dInit_Prev,
                     &dIV_FirstEx, &dStrike, pppdExIndicator, pppdCondiDF,
                     ppdAvgStrike ? *ppdAvgStrike : 0, pnExToTime, adi_arg,
                     lToday, nCompExProb);
  ...
  // else compute both option value and exercise probabilities
    ...
    szErr = _price_genmidat_exprob_(adi_arg, ppAmortArg, pnExToTime, pdOptionPV,
                                    *ppdExProb);
...
static Err _price_genmidat_ts(
    ...
    AM_UND und,         // structure
    AM_ADI_ARG adi_arg, // structure
    long lToday, double dStrike, const double *pdLambdaValue,
    const double
        *pdLambdaTime, //// NB: calendar time        , not year fraction !!!!!!!
    int nLambdaSize, double *pdOptionPV, double **ppdExProb,
  ...
  *pnEx = nNumEx;
  ...
  _init_genmidat_arg(pAmortArg, ppAmortArg, pdUB_D1, pdLB_D1, pdUB_D2, pdLB_D2,
                     &num_fix_cpn_done, &num_fund_cpn_done, &nEx, &dInit_Prev,
                     &dIV_FirstEx, &dStrike, pppdExIndicator, pppdCondiDF,
                     ppdAvgSwapRate ? *ppdAvgSwapRate : 0, pnExToTime, adi_arg,
                     lToday, nCompExProb);

  /// compute option value only
  if (!nCompExProb)
    return _price_genmidat_ts_(adi_arg, ppAmortArg, pdLambdaValue, pdLambdaTime,
                               nLambdaSize, pdOptionPV);
  // else compute both option value and exercise probabilities
  else
    szErr =
        "exercise probability not available with Tau TS yet !"; // _price_genmidat_exprob_(adi_arg
                                                                //       ,ppAmortArg
                                                                //       ,pnExToTime
                                                                //       ,pdOptionPV
                                                                //       ,*ppdExProb);
...
Err am_caller_AmortMidatAutocal(
    ...
    double **ppdExProb, double **ppdAvgSwapRate, int *pnEx,
  ...
  if (exercised) {
    ...
    szErr = am_caller_AmortMidatAutocal(
        today, use_calib, yc, vc, ref, swap_freq, swap_basis, lambda, alpha,
        gamma, rho, get_cash_vol, lgm2dund, start_date, theoEndDate, fund_ref,
        fund_ccy, fund_not, fund_ccy_yc, fx_fund_dom, fx_fund_dom_spot_date,

        fund_ncpn, fund_fix, fund_start, fund_end, fund_pay, fund_basis,
        fund_spr, fund_mrg, fund_fix_cpn,

        fix_not, fix_ncpn, fix_start, fix_end, fix_pay, fix_basis, fix_rate,
        fix_fee,
        ...
        mintime, mininterval, notperiod, one2F, use_jump, max_var_jump,
        strike_type, european_model,

        get_correl, CorrelName,

        max_std_short, fix_lambda, one_f_equi, skip_last,

        eod_fix_flag, eod_pay_flag, eod_ex_flag,

        exercised, ex_date_ex, ex_date_set, ex_fee,

        nReduCalibPoints, nOutPutExProb,

        fund_val, fix_val, call_val, ppdExProb, ppdAvgSwapRate, &nEx, export_ts,
        und_exp);
  ...
  if (fund_ccy == 1) {
    ...
    szErr = convert_funding_to_domestic_amort(
        ...
        fx_fund_dom_spot_date, yc, fix_ncpn, fix_not, fund_ncpn, fund_fix,
        fund_start, fund_pay, fund_basis, fund_ccy_yc, fund_not, fund_spr,
        fund_mrg, fund_fix_cpn, &fund_start_date, &eq_final_ex, &eq_init_ex);
    if (szErr) {
      return szErr;
    }
  } else {
  ...
  szErr = am_fill_check_all_struct_AmortMidatAutocal(
      ...
      use_calib, yc, vc, ref, swap_freq, swap_basis, lambda, alpha, gamma, rho,

      lgm2dund,

      fund_ref, fund_not, fund_ncpn, fund_fix, fund_start, fund_end, fund_pay,
      fund_basis, fund_spr, fund_mrg,
  ...
  if (fund_leg->num_cpn > 0) {
    if (for_fund) {
      ...
      if (fund_leg->cpn[fund_leg->num_cpn - 1].pay_date >=
          today + eod_pay_flag) {
        temp =
            swp_f_df(today, fund_leg->cpn[fund_leg->num_cpn - 1].pay_date, yc) *
  ...
  while (i < fix_ncpn && fix_start[i] < today + eod_fix_flag) {
    if (fix_pay[i] >= today + eod_pay_flag) {
      ...
      fix_leg_pv += fix_rate[i] * coverage(fix_start[i], fix_pay[i], bas) *
                    fix_not[i] * swp_f_df(today, fix_pay[i], yc);
    }
  ...
  if (for_fund) {
    ...
    if (fix_leg->num_cpn > 0) {
      for (j = i; j < fix_leg->num_cpn - 1; ++j) {
        temp = swp_f_df(today, fix_leg->cpn[j].pay_date, yc) *
               (fix_not[j] - fix_not[j + 1]);
        fix_leg_pv += temp;
      }
      ...
      if (fin_not_date >= today + eod_pay_flag) {
        temp =
            swp_f_df(today, fin_not_date, yc) * fix_not[fund_leg->num_cpn - 1];
        fix_leg_pv += temp;
      }
    }
  ...
  if (call_feat == 1) {
    ...
    // NB: most cases fix rates are the same! but might not be correct for some
    // cases!!! shall come back later ...
    szErr = _price_genmidat(am, und, adi_arg, today, fix_rate[0], &call,
                            (nOutPutExProb ? ppdExProb : 0),
                            (nOutPutExProb ? ppdAvgSwapRate : 0), pnEx);
...
Err am_caller_AmortMidatAutocal_reduc(
  ...
  if (call_feat == 1) {
    smessage(
        ...
        req_stp, adi_arg->nstp);
    {
      /// 4. Compute exercise probabilities based on the "equivalent" term
      /// structure
      double *pdEquiExProb = _alloca((*pnEx + 1) * sizeof(double));
      double *pdAvgStrike = _alloca((*pnEx) * sizeof(double));
      szErr = _price_genmidat(am, und, adi_arg, today, fix_rate[0], &call,
                              &pdEquiExProb, &pdAvgStrike, pnEx);
      /// 5. Determine the "most probable" equivalent strike swaptions

      /// 6. Get Market price for the corresponding "most probable" amortizing
      /// swaptions        , while keeping the rest equivalent strikes swaptions
      /// untouched amortMidat_compute_diagonal_prices_new_equistrike
...
Err am_caller_ts_AmortMidatAutocal(
    ...
    char *CorrelName,
    /*
                            Err (*GetVolForBadr)( Date        , Date        ,
       double        , SRT_Boolean        , double *)        , char *cVolType ,
    */
    double max_std_short, int fix_lambda, /*	0: calib lambda to cap        ,
                                 1: fix lambda calib to diagonal */
    int one_f_equi,                       /*	1F equivalent flag:
    ...
    int *pnEx, int export_ts, /*	1: Export TS        , 0: don't */
    AM_UND und_exp) {
  am_str *am = NULL;
  am_und *und = NULL;
  am_adi_arg *adi_arg = NULL;
  ...
  if (exercised) {
    ...
    szErr = am_caller_ts_AmortMidatAutocal(
        today, use_calib, yc, vc, ref, swap_freq, swap_basis, pdLambdaValue,
        pdLambdaTime, nLambdaSize,

        alpha, gamma, rho, get_cash_vol, lgm2dund, start_date, theoEndDate,
        fund_ref, fund_ccy, fund_not, fund_ccy_yc, fx_fund_dom,
        ...
        exercised, ex_date_ex, ex_date_set, ex_fee, nReduCalibPoints,
        nOutPutExProb,
  ...
  szErr = am_fill_check_all_struct_ts_AmortMidatAutocal(
      ...
      use_calib, yc, vc, ref, swap_freq, swap_basis,

      nLambdaSize, pdLambdaTime, pdLambdaValue,
  ...
  if (call_feat == 1) {
    smessage(
        "Launching adi        , time steps requested: %d        , actual: %d",
        req_stp, adi_arg->nstp);
    szErr = _price_genmidat_ts(am, und, adi_arg, today, fix_rate[0],
                               pdLambdaValue, pdLambdaTime, nLambdaSize, &call,
                               (nOutPutExProb ? ppdExProb : 0),
                               (nOutPutExProb ? ppdAvgSwapRate : 0), pnEx);

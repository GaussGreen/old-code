...
Err FxSVFullFromUnd(SFxSVUndFull* o, SrtUndPtr und, long last_date)
{
    ...
    if (!o->pmdl || strcmp(und->underl_lbl, "FXSV_UND"))
        return serror(
            "Underlying %s improperly initialised or of incorrect type", und->underl_name);
...
static void NAG_CALL EvalDerivatives(Integer neq, double t, double y[], double yp[], Nag_User* comm)
{
    ...
    if (p->is_cur_idx && p->want[1])
    {
        ...
        ypg[1][4] +=
            c6 * (p->u_re * y[5] + p->u_im * y[4]) - 4.0 * p->alpha * (y[4] * y[4] - y[5] * y[5]);
...
static Err FxSVCalcPhi(SFxSVComm_InvFT* q, double u_re, double u_im, double* h_re, double* h_im)
{
    ...
    for (i = o->ntimes - 2; i >= 0; i--)
    {
        ...
        nag_ode_ivp_rk_range(
            neq, EvalDerivatives, o->times[i], &tgot, y, yp, ymax, &opt, &comm_Nag, &fail);
        if (fail.code != NE_NOERROR)
        {
            err = serror(fail.message);
            goto FREE_RETURN;
...
EvalDerivatives2(Integer neq, double t, double y[], double yp[], Nag_User* comm)
{
    SFxSVComm_ODE* p = (SFxSVComm_ODE*)comm->p;
    double         Gamma_d, Gamma_f;
...
static Err FxSVCalcMoments(SFxSVComm_InvFT* q, double* mean, double* std)
{
    ...
    for (i = o->ntimes - 2; i >= 0; i--)
    {
        ...
        nag_ode_ivp_rk_range(
            6, EvalDerivatives2, o->times[i], &tgot, y, yp, ymax, &opt, &comm_Nag, &fail);
        if (fail.code != NE_NOERROR)
        {
            err = serror(fail.message);
            goto FREE_RETURN;
    ...
    for (i = 0; i < nK; i++)
    {
        ...
        for (j = 0; j < 5; j++)
            if (j == 0 || want[j - 1])
            {
                ...
                //			res[i][j] *= swp_f_df(und_full.pmdl->today, pay_date, yc_dom);
                //affine
            }
    }
...
static Err CalibrateBeta(SFxSVComm_Calib* r)
{
    ...
    for (i = r->idx_from; i <= r->idx_to; i = j + 1)
    {
        ...
        t2 = clock();
        smessage(
            "Volatility calibration at step %d, time in sec: %.2f",
            j + 1,
...
Err FxSVCalibrate(
    SrtUndPtr und,    // Starting points and params not calibrated must be initialized
    ...
    double**  vol)     // Vols per exercise date per strike
{
    ...
    for (i = 0; i < pmdl->ntimes; i++)
    {
        ...
        for (j = 0; j < nK[i]; j++)
        {
            ...
            comm_Calib.mkt_val[i][j] = srt_f_optblksch(
                ffx0,
                K[i][j],
                vol[i][j],
                pmdl->times[i],
                1.0,
                (K[i][j] < ffx0 ? SRT_PUT : SRT_CALL),
                PREMIUM);
    ...
    for (i = 0; i < pmdl->ntimes; i = j + 1)
    {
        ...
        else  // step j is a smile calibration date
        {
            ...
            {
                err = serror(
                    "Error: Smile calibration date %d is not a volatility calibration date", j + 1);
                goto FREE_RETURN;
            }
            ...
            if (n > 0)  // if there's smth to calibrate
            {
                ...
                nag_opt_lsq_no_deriv(
                    nK[j] - 1, n, LsqFun, x, &fsumsq, fvec, &fjac[0][0], 3, &options, &comm, &fail);
            ...
            t2 = clock();
            smessage(
                "Smile calibration at step %d, time in sec: %.2f",
                j + 1,
                (double)(t2 - t1) / CLOCKS_PER_SEC);

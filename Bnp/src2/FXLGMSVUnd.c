    ...
    irm_sv* irmsv)
{
    ...
    if ((!irmsv->sigma_dates) || (!irmsv->sigma_times) || (!irmsv->tau_dates) ||
        (!irmsv->tau_times) || (!irmsv->smile_dates) || (!irmsv->smile_times) || (!irmsv->sigma) ||
        (!irmsv->tau) || (!irmsv->alphaSV) || (!irmsv->lamSV) || (!irmsv->rhoSV) ||
        (!irmsv->rho2SV))
    {
...
Err fxlgmsv_free_und(fxlgmsv_str* fxlgmsv)
{
    if (fxlgmsv)
    {
        ...
        if (fxlgmsv->correlation)
            free_f3tensor(fxlgmsv->correlation, 0, fxlgmsv->num_dates - 1, 0, 6, 0, 6);
    ...
    fxlgmsv_str* fxlgmsv)
{
    ...
    if ((!fxlgmsv->dates) || (!fxlgmsv->times) || (!fxlgmsv->dates) || (!fxlgmsv->fx_sigma) ||
        (!fxlgmsv->correlation))
    {
    ...
    double**** merge_correlation)
{
    ...
    *merge_correlation = f3tensor(0, *merge_n - 1, 0, 6, 0, 6);
    ...
    if (!(*merge_dom_sigma) || !(*merge_dom_tau) || !(*merge_dom_alphaSV) || !(*merge_dom_lamSV) ||
        !(*merge_dom_rhoSV) || !(*merge_dom_rho2SV) || !(*merge_for_sigma) || !(*merge_for_tau) ||
        !(*merge_for_alphaSV) || !(*merge_for_lamSV) || !(*merge_for_rhoSV) ||
        !(*merge_for_rho2SV) || !(*merge_fx_sigma) || !(*merge_correlation))
    {
        err = "Memory allocation error in FXLGMSV_merge_fx_ir_ir_ts";
        goto FREE_RETURN;
    }
...
Err fxlgmsv_free_model(fxlgmsv_model* fxlgmsvmodel)
{
    if (fxlgmsvmodel)
    {
        ...
        if (fxlgmsvmodel->correlation)
            free_f3tensor(fxlgmsvmodel->correlation, 0, fxlgmsvmodel->num_times - 1, 0, 6, 0, 6);
...
Err qtolgmsv_check_und(char* und, int* for_one2F)
{
    ...
    if ((qtolgmsv->dom_irmsv->alpha > 1.0e-5) || (qtolgmsv->dom_irmsv->alphaSV[0] > 1.0e-5))
    {
    ...
    double**** correlation)
{
    ...
    *correlation = f3tensor(0, fxlgmsv->num_dates - 1, 0, 6, 0, 6);
    ...
    if ((!(*dates)) || (!(*times)) || (!(*dates)) || (!(*dom_sigma)) || (!(*dom_tau)) ||
        (!(*dom_alphaSV)) || (!(*dom_lamSV)) || (!(*dom_rhoSV)) || (!(*dom_rho2SV)) ||
        (!(*for_sigma)) || (!(*for_tau)) || (!(*for_alphaSV)) || (!(*for_lamSV)) ||
        (!(*for_rhoSV)) || (!(*for_rho2SV)) || (!(*fx_sigma)) || (!(*correlation)))
    {
...
void HestonConstructLegendreGrid(
    int iNbInt, double* dBreakPoints, int* iNbPoints, double* dX, double* dW)
{
    int    i;
    int    iStartIndex;
    double dStart, dEnd;
...
void HestonComputeIntegralLegendre(
    int     iNbX,
    double* X,
    double* W,
    double* IntRe,
    ...
    double* Price)
{
    ...
    double *dt = NULL, *CoefIntT = NULL, *Coef1ReT = NULL, *Coef2ReT = NULL, *Coef2ImT = NULL,
           *Coef3ReT = NULL, *Coef3ImT = NULL;
    ...
    if (!dt || !CoefIntT || !Coef1ReT || !Coef2ReT || !Coef2ImT || !Coef3ReT || !Coef3ImT || !X ||
        !IntRe || !IntIm)
    {
    ...
    HestonComputeIntegralLegendre(iNbX, X, W, IntRe, IntIm, 1, &LogStrike, dIntegParam, Price);
    ...
    double* dPWCSigma)
{
    ...
    ZCVar1 =
        (dSigma[0] / dLambda) * (dSigma[0] / dLambda) *
        (0.5 * (exp(2 * dLambda * dTStar) - exp(2 * dLambda * (dTStar - dPWTime[0]))) / dLambda -
         2.0 * exp(dLambda * (dTStar - dBondMat)) *
             (exp(dLambda * dTStar) - exp(dLambda * (dTStar - dPWTime[0]))) / dLambda +
         exp(2 * dLambda * (dTStar - dBondMat)) * dPWTime[0]);

    ZCVar2 =
        (dAlphaLGM[0] * dSigma[0] / dLambda2) * (dAlphaLGM[0] * dSigma[0] / dLambda2) *
        (0.5 * (exp(2 * dLambda2 * dTStar) - exp(2 * dLambda2 * (dTStar - dPWTime[0]))) / dLambda2 -
         2.0 * exp(dLambda2 * (dTStar - dBondMat)) *
             (exp(dLambda2 * dTStar) - exp(dLambda2 * (dTStar - dPWTime[0]))) / dLambda2 +
         exp(2 * dLambda2 * (dTStar - dBondMat)) * dPWTime[0]);

    ZCCoVar =
        dAlphaLGM[0] * dRhoLGM[0] * (dSigma[0] / dLambda) * (dSigma[0] / dLambda2) *
        ((exp((dLambda + dLambda2) * dTStar) - exp((dLambda + dLambda2) * (dTStar - dPWTime[0]))) /
             (dLambda + dLambda2) -
    ...
    for (i = 1; i < FixTimeIndex; ++i)
    {
        ZCVar1 =
            ...
            (0.5 *
                 ...
                 dLambda -
             2.0 * exp(dLambda * (dTStar - dBondMat)) *
                 (exp(dLambda * (dTStar - dPWTime[i - 1])) - exp(dLambda * (dTStar - dPWTime[i]))) /
                 dLambda +
        ...
        ZCCoVar =
            dAlphaLGM[i] * dRhoLGM[i] * (dSigma[i] / dLambda) * (dSigma[i] / dLambda2) *
    ...
    ZCVar1 = (dSigma[i] / dLambda) * (dSigma[i] / dLambda) *
             (0.5 *
                  ...
                  dLambda -
              2.0 * exp(dLambda * (dTStar - dBondMat)) *
                  (exp(dLambda * (dTStar - dPWTime[i - 1])) - exp(dLambda * (dTStar - dFixTime))) /
                  dLambda +
              exp(2 * dLambda * (dTStar - dBondMat)) * (dFixTime - dPWTime[i - 1]));

    ZCVar2 =
        (dAlphaLGM[i] * dSigma[i] / dLambda2) * (dAlphaLGM[i] * dSigma[i] / dLambda2) *
        (0.5 *
             (exp(2 * dLambda2 * (dTStar - dPWTime[i - 1])) -
              exp(2 * dLambda2 * (dTStar - dFixTime))) /
             dLambda2 -
         2.0 * exp(dLambda2 * (dTStar - dBondMat)) *
             (exp(dLambda2 * (dTStar - dPWTime[i - 1])) - exp(dLambda2 * (dTStar - dFixTime))) /
             dLambda2 +
    ...
    double* dLambdaEpsNew)
{
    ...
    {
        dLambdaEpsNew[i] =
            dLambdaEps[i] + dAlpha[i] * dSigma[i] * (Coeff1 * dRho[i] + Coeff2 * dRho2[i]);
    }
}
    ...
    double* dforLambdaEpsNew)
{
    ...
    for (i = 0; i < iNbPWTime; ++i)
    {
        adjustment = 0.5 * dforAlpha[i] * ddomSigma[i] *
    ...
    double* dforLambdaEpsNew)
{
    ...
    for (i = 0; i < iNbPWTime; ++i)
    {
        ...
        dforLambdaEpsNew[i] = dforLambdaEps[i] + dforAlpha[i] * forZCVol1 * dCorrelation[i][5][3] +
    ...
    double* dPWCRho)
{
    ...
    ZCVol1 = (exp(dLambda * (dTStar - dPWTime[0])) - exp(dLambda * (dTStar - dBondMat))) *
             dSigma[0] / dLambda;
    ZCVol1 += (exp(dLambda * dTStar) - exp(dLambda * (dTStar - dBondMat))) * dSigma[0] / dLambda;
    ZCVol1 = 0.5 * ZCVol1;

    ZCVol2 = (exp(dLambda2 * (dTStar - dPWTime[0])) - exp(dLambda2 * (dTStar - dBondMat))) *
             dAlphaLGM[0] * dSigma[0] / dLambda2;
    ZCVol2 += (exp(dLambda2 * dTStar) - exp(dLambda2 * (dTStar - dBondMat))) * dAlphaLGM[0] *
              dSigma[0] / dLambda2;
    ZCVol2 = 0.5 * ZCVol2;
    ...
    for (i = 1; i < FixTimeIndex; ++i)
    {
        ...
        ZCVol1 = (exp(dLambda * (dTStar - dPWTime[i])) - exp(dLambda * (dTStar - dBondMat))) *
                 dSigma[i] / dLambda;
        ZCVol1 += (exp(dLambda * (dTStar - dPWTime[i - 1])) - exp(dLambda * (dTStar - dBondMat))) *
                  dSigma[i] / dLambda;
        ZCVol1 = 0.5 * ZCVol1;

        ZCVol2 = (exp(dLambda2 * (dTStar - dPWTime[i])) - exp(dLambda2 * (dTStar - dBondMat))) *
                 dAlphaLGM[i] * dSigma[i] / dLambda2;
        ZCVol2 +=
            (exp(dLambda2 * (dTStar - dPWTime[i - 1])) - exp(dLambda2 * (dTStar - dBondMat))) *
            dAlphaLGM[i] * dSigma[i] / dLambda2;
        ZCVol2 = 0.5 * ZCVol2;
    ...
    ZCVol1 = (exp(dLambda * (dTStar - dFixTime)) - exp(dLambda * (dTStar - dBondMat))) * dSigma[i] /
             dLambda;
    ZCVol1 += (exp(dLambda * (dTStar - dPWTime[i - 1])) - exp(dLambda * (dTStar - dBondMat))) *
              dSigma[i] / dLambda;
    ZCVol1 = 0.5 * ZCVol1;

    ZCVol2 = (exp(dLambda2 * (dTStar - dFixTime)) - exp(dLambda2 * (dTStar - dBondMat))) *
             dAlphaLGM[i] * dSigma[i] / dLambda2;
    ZCVol2 += (exp(dLambda2 * (dTStar - dPWTime[i - 1])) - exp(dLambda2 * (dTStar - dBondMat))) *
              dAlphaLGM[i] * dSigma[i] / dLambda2;
    ZCVol2 = 0.5 * ZCVol2;
    ...
    double* ddomforCov)
{
    ...
    domZCVar1 =
        (ddomSigma[0] / ddomLambda) * (ddomSigma[0] / ddomLambda) *
        (0.5 * (exp(2 * ddomLambda * ddomTStar) - exp(2 * ddomLambda * (ddomTStar - dPWTime[0]))) /
             ddomLambda -
         2.0 * exp(ddomLambda * (ddomTStar - dBondMat)) *
             (exp(ddomLambda * ddomTStar) - exp(ddomLambda * (ddomTStar - dPWTime[0]))) /
             ddomLambda +
    ...
    domZCVar2 =
        ...
        (ddomAlphaLGM[0] * ddomSigma[0] / ddomLambda2) *
        (0.5 *
             (exp(2 * ddomLambda2 * ddomTStar) - exp(2 * ddomLambda2 * (ddomTStar - dPWTime[0]))) /
             ddomLambda2 -
         2.0 * exp(ddomLambda2 * (ddomTStar - dBondMat)) *
             (exp(ddomLambda2 * ddomTStar) - exp(ddomLambda2 * (ddomTStar - dPWTime[0]))) /
             ddomLambda2 +
         exp(2 * ddomLambda2 * (ddomTStar - dBondMat)) * dPWTime[0]);

    domZCCoVar = ddomAlphaLGM[0] * ddomRhoLGM[0] * (ddomSigma[0] / ddomLambda) *
    ...
    forZCVar1 =
        (dforSigma[0] / dforLambda) * (dforSigma[0] / dforLambda) *
        (0.5 * (exp(2 * dforLambda * dforTStar) - exp(2 * dforLambda * (dforTStar - dPWTime[0]))) /
             dforLambda -
         2.0 * exp(dforLambda * (dforTStar - dBondMat)) *
             (exp(dforLambda * dforTStar) - exp(dforLambda * (dforTStar - dPWTime[0]))) /
             dforLambda +
    ...
    forZCVar2 =
        ...
        (dforAlphaLGM[0] * dforSigma[0] / dforLambda2) *
        (0.5 *
             (exp(2 * dforLambda2 * dforTStar) - exp(2 * dforLambda2 * (dforTStar - dPWTime[0]))) /
             dforLambda2 -
         2.0 * exp(dforLambda2 * (dforTStar - dBondMat)) *
             (exp(dforLambda2 * dforTStar) - exp(dforLambda2 * (dforTStar - dPWTime[0]))) /
             dforLambda2 +
         exp(2 * dforLambda2 * (dforTStar - dBondMat)) * dPWTime[0]);

    forZCCoVar = dforAlphaLGM[0] * dforRhoLGM[0] * (dforSigma[0] / dforLambda) *
    ...
    // FX-Dom Covariance
    dfxdomCoVar1 =
        -dCorrelation[0][0][4] * dfxSigma[0] * ddomSigma[0] / ddomLambda *
        ((exp(ddomLambda * ddomTStar) - exp(ddomLambda * (ddomTStar - dPWTime[0]))) / ddomLambda -
         exp(ddomLambda * (ddomTStar - dBondMat)) * dPWTime[0]);

    dfxdomCoVar2 = -dCorrelation[0][1][4] * dfxSigma[0] * ddomAlphaLGM[0] * ddomSigma[0] /
                   ddomLambda2 *
                   ((exp(ddomLambda2 * ddomTStar) - exp(ddomLambda2 * (ddomTStar - dPWTime[0]))) /
                        ddomLambda2 -
    ...
    // FX-For Covariance
    dfxforCoVar1 =
        -dCorrelation[0][2][4] * dfxSigma[0] * dforSigma[0] / dforLambda *
        ((exp(dforLambda * dforTStar) - exp(dforLambda * (dforTStar - dPWTime[0]))) / dforLambda -
         exp(dforLambda * (dforTStar - dBondMat)) * dPWTime[0]);

    dfxforCoVar2 = -dCorrelation[0][3][4] * dfxSigma[0] * dforAlphaLGM[0] * dforSigma[0] /
                   dforLambda2 *
                   ((exp(dforLambda2 * dforTStar) - exp(dforLambda2 * (dforTStar - dPWTime[0]))) /
                        dforLambda2 -
    ...
    // Dom-For Covariance
    ddomforCoVar11 =
        dCorrelation[0][0][2] * ddomSigma[0] * dforSigma[0] / (dforLambda * ddomLambda) *
    ...
    ddomforCoVar22 =
        dCorrelation[0][1][3] * ddomSigma[0] * ddomAlphaLGM[0] * dforSigma[0] * dforAlphaLGM[0] /
        (dforLambda2 * ddomLambda2) *
    ...
    for (i = 1; i < FixTimeIndex; ++i)
    {
        ...
        domZCCoVar = ddomAlphaLGM[i] * ddomRhoLGM[i] * (ddomSigma[i] / ddomLambda) *
        ...
        forZCCoVar = dforAlphaLGM[i] * dforRhoLGM[i] * (dforSigma[i] / dforLambda) *
        ...
        dfxdomCoVar2 = -dCorrelation[i][1][4] * dfxSigma[i] * ddomAlphaLGM[i] * ddomSigma[i] /
                       ddomLambda2 *
                       ((exp(ddomLambda2 * (ddomTStar - dPWTime[i - 1])) -
                         exp(ddomLambda2 * (ddomTStar - dPWTime[i]))) /
                            ddomLambda2 -
        ...
        dfxforCoVar2 = -dCorrelation[i][3][4] * dfxSigma[i] * dforAlphaLGM[i] * dforSigma[i] /
                       dforLambda2 *
                       ((exp(dforLambda2 * (dforTStar - dPWTime[i - 1])) -
                         exp(dforLambda2 * (dforTStar - dPWTime[i]))) /
                            dforLambda2 -
        ...
        // Dom-For Covariance
        ddomforCoVar11 = dCorrelation[i][0][2] * ddomSigma[i] * dforSigma[i] /
        ...
        ddomforCoVar12 = dCorrelation[i][0][3] * ddomSigma[i] * dforSigma[i] * dforAlphaLGM[i] /
                         (dforLambda2 * ddomLambda) *
        ...
        ddomforCoVar21 = dCorrelation[i][1][2] * ddomSigma[i] * ddomAlphaLGM[i] * dforSigma[i] /
                         (dforLambda * ddomLambda2) *
        ...
        ddomforCoVar22 = dCorrelation[i][1][3] * ddomSigma[i] * ddomAlphaLGM[i] * dforSigma[i] *
    ...
    // Dom-For Covariance
    ddomforCoVar11 =
        dCorrelation[i][0][2] * ddomSigma[i] * dforSigma[i] / (dforLambda * ddomLambda) *
    ...
    ddomforCoVar22 =
        dCorrelation[i][1][3] * ddomSigma[i] * ddomAlphaLGM[i] * dforSigma[i] * dforAlphaLGM[i] /
        (dforLambda2 * ddomLambda2) *
    ...
    double* dAlphaforCov)
{
    ...
    dAlphadomCoVar1 =
        -dCorrelation[0][0][5] * dAlpha[0] * ddomSigma[0] / ddomLambda *
        ((exp(ddomLambda * ddomTStar) - exp(ddomLambda * (ddomTStar - dPWTime[0]))) / ddomLambda -
         exp(ddomLambda * (ddomTStar - dBondMat)) * dPWTime[0]);

    dAlphadomCoVar2 =
        -dCorrelation[0][1][5] * dAlpha[0] * ddomAlphaLGM[0] * ddomSigma[0] / ddomLambda2 *
        ((exp(ddomLambda2 * ddomTStar) - exp(ddomLambda2 * (ddomTStar - dPWTime[0]))) /
             ddomLambda2 -
         exp(ddomLambda2 * (ddomTStar - dBondMat)) * dPWTime[0]);
    ...
    dAlphaforCoVar1 =
        -dCorrelation[0][0][5] * dAlpha[0] * dforSigma[0] / dforLambda *
        ((exp(dforLambda * dforTStar) - exp(dforLambda * (dforTStar - dPWTime[0]))) / dforLambda -
         exp(dforLambda * (dforTStar - dBondMat)) * dPWTime[0]);

    dAlphaforCoVar2 =
        -dCorrelation[0][1][5] * dAlpha[0] * dforAlphaLGM[0] * dforSigma[0] / dforLambda2 *
        ((exp(dforLambda2 * dforTStar) - exp(dforLambda2 * (dforTStar - dPWTime[0]))) /
             dforLambda2 -
         exp(dforLambda2 * (dforTStar - dBondMat)) * dPWTime[0]);
    ...
    for (i = 1; i < FixTimeIndex; ++i)
    {
        ...
        dAlphadomCoVar2 =
            -dCorrelation[i][1][5] * dAlpha[i] * ddomAlphaLGM[i] * ddomSigma[i] / ddomLambda2 *
            ((exp(ddomLambda2 * (ddomTStar - dPWTime[i - 1])) -
              exp(ddomLambda2 * (ddomTStar - dPWTime[i]))) /
                 ddomLambda2 -
             exp(ddomLambda2 * (ddomTStar - dBondMat)) * (dPWTime[i] - dPWTime[i - 1]));
        ...
        dAlphaforCoVar2 =
            -dCorrelation[i][1][5] * dAlpha[i] * dforAlphaLGM[i] * dforSigma[i] / dforLambda2 *
            ((exp(dforLambda2 * (dforTStar - dPWTime[i - 1])) -
              exp(dforLambda2 * (dforTStar - dPWTime[i]))) /
                 dforLambda2 -
             exp(dforLambda2 * (dforTStar - dBondMat)) * (dPWTime[i] - dPWTime[i - 1]));
        ...
        // FX-Alpha Covariance
        dAlphafxCoVar =
            dfxSigma[i] * dAlpha[i] * dCorrelation[i][4][5] * (dPWTime[i] - dPWTime[i - 1]);
    ...
    dAlphadomCoVar2 = -dCorrelation[i][1][5] * dAlpha[i] * ddomAlphaLGM[i] * ddomSigma[i] /
                      ddomLambda2 *
                      ((exp(ddomLambda2 * (ddomTStar - dPWTime[i - 1])) -
                        exp(ddomLambda2 * (ddomTStar - dFixTime))) /
                           ddomLambda2 -
    ...
    dAlphaforCoVar2 = -dCorrelation[i][1][5] * dAlpha[i] * dforAlphaLGM[i] * dforSigma[i] /
                      dforLambda2 *
                      ((exp(dforLambda2 * (dforTStar - dPWTime[i - 1])) -
                        exp(dforLambda2 * (dforTStar - dFixTime))) /
                           dforLambda2 -
                         ...
                         // 3 : h4
{
    ...
    if ((!ddomVar) || (!dforVar) || (!dfxVar) || (!dAlphaVariance) || (!ddomfxCov) ||
        (!dforfxCov) || (!ddomforCov) || (!dAlphafxCov) || (!dAlphadomCov) || (!dAlphaforCov))
    {
    ...
    double* dVol)
{
    ...
    *dVol = sqrt(
        (ddomVar + dforVar + dfxVar + 2 * ddomfxCov + 2 * dforfxCov + 2 * ddomfxCov) / dFixTime);
}
    ...
    double* FXImpliedVol)
{
    ...
    *FXImpliedVol = sqrt(
        (dfxVar + ddomBondATMVol * ddomBondATMVol * dFixTime +
         ...
         2 * (ddomfxCov / sqrt(ddomVar * dfxVar)) * sqrt(dfxVar / dFixTime) * ddomBondATMVol *
             dFixTime +
         2 * (dforfxCov / sqrt(dforVar * dfxVar)) * sqrt(dfxVar / dFixTime) * dforBondATMVol *
             dFixTime -
    ...
    double**** merge_correlation)
{
    ...
    *merge_correlation = f3tensor(0, *merge_n - 1, 0, 6, 0, 6);
    ...
    if (!(*merge_dom_sigma) || !(*merge_dom_alphaLGM) || !(*merge_dom_rhoLGM) ||
        ...
        !(*merge_for_rhoSV) || !(*merge_for_rho2SV) || !(*merge_fx_sigma) || !(*merge_correlation))
    {
        err = "Memory allocation error in FXLGMSV_merge_fx_ir_ir_model_ts";
        goto FREE_RETURN;
    }
    ...
    double**** dFullCorrelation)
{
    ...
    fxlgmsv_get_fx_and_correl_ts(
        UndName, &fx_spot, &fx_n, &fx_time, &fx_vol, &rho_n, &rho_time, &CorrMatrix);
    ...
    double* dd2vol)
{
    ...
    var += (h1[i] * h1[i] + h2[i] * h2[i] + h3[i] * h3[i]
            ...
            - 2 * h1[i] * h2[i] * correlation[i][0][1] + 2 * h1[i] * h3[i] * correlation[i][0][2]

            - 2 * h2[i] * h3[i] * correlation[i][1][2]) *
           (dPWTime[indexMat1] - Mat1);

    d1Integral += (h1[i] * h3[i] * correlation[i][0][2] - h2[i] * h3[i] * correlation[i][1][2]) *
                  (exp(-kappa * Mat1) - exp(-kappa * dPWTime[indexMat1])) / kappa;
    ;
    d1Integral +=
        2 * h3[i] * h3[i] * (exp(-kappa * Mat1) - exp(-kappa * dPWTime[indexMat1])) / kappa;

    d2Integral += (h1[i] * h3[i] * correlation[i][0][2] - h2[i] * h3[i] * correlation[i][1][2]) *
                  (exp(-2 * kappa * Mat1) - exp(-2 * kappa * dPWTime[indexMat1])) / (2 * kappa);
    ...
    for (i = indexMat1 + 1; i < indexMat2; ++i)
    {
        ...
        d1Integral +=
            (h1[i] * h3[i] * correlation[i][0][2] - h2[i] * h3[i] * correlation[i][1][2]) *
            (exp(-kappa * dPWTime[i - 1]) - exp(-kappa * dPWTime[i])) / kappa;
        ;
        d1Integral +=
            2 * h3[i] * h3[i] * (exp(-kappa * dPWTime[i - 1]) - exp(-kappa * dPWTime[i])) / kappa;

        d2Integral +=
            (h1[i] * h3[i] * correlation[i][0][2] - h2[i] * h3[i] * correlation[i][1][2]) *
            (exp(-2 * kappa * dPWTime[i - 1]) - exp(-2 * kappa * dPWTime[i])) / (2 * kappa);
    }
    ...
    d1Integral += (h1[i] * h3[i] * correlation[i][0][2] - h2[i] * h3[i] * correlation[i][1][2]) *
                  (exp(-kappa * dPWTime[max(0, i - 1)]) - exp(-kappa * Mat2)) / kappa;
    ;
    d1Integral +=
        2 * h3[i] * h3[i] * (exp(-kappa * dPWTime[max(0, i - 1)]) - exp(-kappa * Mat2)) / kappa;

    d2Integral += (h1[i] * h3[i] * correlation[i][0][2] - h2[i] * h3[i] * correlation[i][1][2]) *
                  (exp(-2 * kappa * dPWTime[max(0, i - 1)]) - exp(-2 * kappa * Mat2)) / (2 * kappa);

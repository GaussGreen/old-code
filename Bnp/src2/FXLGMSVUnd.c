...
Err fxlgmsv_free_und(fxlgmsv_str	*fxlgmsv)
{
	if(fxlgmsv)
	{
		if(fxlgmsv->dates) free(fxlgmsv->dates);
		if(fxlgmsv->times) free(fxlgmsv->times);

		if(fxlgmsv->fx_sigma) free(fxlgmsv->fx_sigma);

		if(fxlgmsv->correlation) free_f3tensor(fxlgmsv->correlation, 0, fxlgmsv->num_dates-1, 0, 6, 0, 6);
					...
					fxlgmsv_str	*fxlgmsv)
{
	...
	err = init_LGMSV_str(today, num_dates, dates, times,
							dom_one2F, dom_sigma, dom_tau,
							dom_alpha, dom_gamma, dom_rho,
							dom_alphaSV, dom_lamSV, dom_rhoSV, dom_rho2SV,
							dom_tstar,
							fxlgmsv->dom_irmsv);
	if(err)
	{
	...
	err = init_LGMSV_str(today, num_dates, dates, times,
							for_one2F, for_sigma, for_tau,
							for_alpha, for_gamma, for_rho,
							for_alphaSV, for_lamSV, for_rhoSV, for_rho2SV,
							for_tstar,
							fxlgmsv->for_irmsv);
	if(err)
	{
...
Err FXLGMSV_merge_fx_ir_ir_ts(	
							...
							int		*merge_n,
							 ...
							 /* Domestic TS */
							double	**merge_dom_sigma,
							double	**merge_dom_tau,

							double	**merge_dom_alphaSV,
							double	**merge_dom_lamSV,
							double	**merge_dom_rhoSV,
							double	**merge_dom_rho2SV,

							 /* Foreign TS */
							double	**merge_for_sigma,
							double	**merge_for_tau,

							double	**merge_for_alphaSV,
							double	**merge_for_lamSV,
							double	**merge_for_rhoSV,
							...
							double	****merge_correlation)
{
	...
	*merge_correlation = f3tensor(0, *merge_n - 1, 0, 6, 0, 6);
	...
	{
		/* Domestic TS */
		(*merge_dom_sigma)[i] = dom_sigma[Get_Index ((*merge_time)[i], dom_sigma_time, dom_n_sigma)];
		(*merge_dom_tau)[i] = dom_tau[Get_Index ((*merge_time)[i], dom_tau_time, dom_n_tau)];
		...
		/* Foreign TS */
		(*merge_for_sigma)[i] = for_sigma[Get_Index ((*merge_time)[i], for_sigma_time, for_n_sigma)];
		(*merge_for_tau)[i] = for_tau[Get_Index ((*merge_time)[i], for_tau_time, for_n_tau)];
		...
		/* FX TS */
		(*merge_fx_sigma)[i] = fx_sigma[Get_Index ((*merge_time)[i], fx_sigma_time, fx_n_sigma)];
					  ...
					  double		***correlation)
{
	...
	err = irm_sv_get_term_struct(	dom_undName,
									&dom_one2F,
									&dom_n_sigma,
									&dom_sigma_time,
									&dom_sigma,
	...
	err = irm_sv_get_term_struct(	for_undName,
									&for_one2F,
									&for_n_sigma,
									&for_sigma_time,
									&for_sigma,
...
Err fxlgmsv_free_model(fxlgmsv_model	*fxlgmsvmodel)
{
	if(fxlgmsvmodel)
	{
		if(fxlgmsvmodel->times) free(fxlgmsvmodel->times);

		if(fxlgmsvmodel->fx_sigma) free(fxlgmsvmodel->fx_sigma);

		if(fxlgmsvmodel->correlation) free_f3tensor(fxlgmsvmodel->correlation, 0, fxlgmsvmodel->num_times-1, 0, 6, 0, 6);
...
Err fxlgmsv_get_fx_and_correl_ts(char *und, double *fx_spot, int *num_fx, double **fx_time, double **fx_vol, int *num_rho, double **rho_time, double ****correlation)
{
	Err err = NULL;
	fxlgmsv_str *fxlgmsv;
	int i, j, k;
					...
					double	****correlation)
{
	...
	*correlation = f3tensor(0, fxlgmsv->num_dates-1, 0, 6, 0, 6);
	...
	memcpy (*dom_sigma, fxlgmsv->dom_irmsv->sigma, fxlgmsv->num_dates * sizeof (double));
	memcpy (*dom_tau, fxlgmsv->dom_irmsv->tau, fxlgmsv->num_dates * sizeof (double));
	memcpy (*dom_alphaSV, fxlgmsv->dom_irmsv->alphaSV, fxlgmsv->num_dates * sizeof (double));
	memcpy (*dom_lamSV, fxlgmsv->dom_irmsv->lamSV, fxlgmsv->num_dates * sizeof (double));
	memcpy (*dom_rhoSV, fxlgmsv->dom_irmsv->rhoSV, fxlgmsv->num_dates * sizeof (double));
	memcpy (*dom_rho2SV, fxlgmsv->dom_irmsv->rho2SV, fxlgmsv->num_dates * sizeof (double));
	
	memcpy (*for_sigma, fxlgmsv->for_irmsv->sigma, fxlgmsv->num_dates * sizeof (double));
	memcpy (*for_tau, fxlgmsv->for_irmsv->tau, fxlgmsv->num_dates * sizeof (double));
	memcpy (*for_alphaSV, fxlgmsv->for_irmsv->alphaSV, fxlgmsv->num_dates * sizeof (double));
	memcpy (*for_lamSV, fxlgmsv->for_irmsv->lamSV, fxlgmsv->num_dates * sizeof (double));
	memcpy (*for_rhoSV, fxlgmsv->for_irmsv->rhoSV, fxlgmsv->num_dates * sizeof (double));
	memcpy (*for_rho2SV, fxlgmsv->for_irmsv->rho2SV, fxlgmsv->num_dates * sizeof (double));
...
void HestonConstructLegendreGrid(int		iNbInt,
								double	*dBreakPoints,
								int		*iNbPoints,
								double	*dX,
								double	*dW)
...
double	dStart, dEnd;
	...
	{
		dEnd = dBreakPoints[i];
		gauleg(dStart, dEnd, &(dX[iStartIndex-1]), &(dW[iStartIndex-1]), iNbPoints[i]);
		dStart = dEnd;
...
void HestonComputeIntegralLegendre(	int		iNbX,
									double	*X,
									double	*W,									
									double	*IntRe,
									double	*IntIm,
...
int				iBreakNbX[4];
	...
	if (!dt || !CoefIntT || !Coef1ReT || !Coef2ReT || !Coef2ImT || !Coef3ReT || !Coef3ImT
		|| !X || !IntRe || !IntIm) 
	{
	...
	HestonComputeIntegralLegendre(	iNbX,
										X,
										W,
										IntRe,
										IntIm,
...
FREE_RETURN:
	...
	if (dt) free_dvector(dt, 0, endi);
	if (CoefIntT) free_dvector(CoefIntT, 0, endi);
	...
	if (X) free_dvector(X, 0, iNbX - 1);
	if (W) free_dvector(W, 0, iNbX - 1);
	if (IntRe) free_dvector(IntRe, 0, iNbX - 1);
	if (IntIm) free_dvector(IntIm, 0, iNbX - 1);
						...
						double	*dPWCSigma)
{
	...
	ZCVar1 = (dSigma[0] / dLambda) * (dSigma[0] / dLambda) * (
			0.5 * ( exp(2 * dLambda * dTStar) - exp(2 * dLambda * (dTStar - dPWTime[0])) ) / dLambda
			- 2.0 * exp(dLambda * (dTStar - dBondMat)) * ( exp(dLambda * dTStar) - exp(dLambda * (dTStar - dPWTime[0])) ) / dLambda
			+ exp(2 * dLambda * (dTStar - dBondMat)) * dPWTime[0] );

	ZCVar2 = (dAlphaLGM[0] * dSigma[0] / dLambda2) * (dAlphaLGM[0] * dSigma[0] / dLambda2) * (
			0.5 * ( exp(2 * dLambda2 * dTStar) - exp(2 * dLambda2 * (dTStar - dPWTime[0])) ) / dLambda2
			- 2.0 * exp(dLambda2 * (dTStar - dBondMat)) * ( exp(dLambda2 * dTStar) - exp(dLambda2 * (dTStar - dPWTime[0])) ) / dLambda2
			+ exp(2 * dLambda2 * (dTStar - dBondMat)) * dPWTime[0] );

	ZCCoVar = dAlphaLGM[0] * dRhoLGM[0] * (dSigma[0] / dLambda) * (dSigma[0] / dLambda2) * (
			( exp( (dLambda + dLambda2) * dTStar) - exp( (dLambda + dLambda2) * (dTStar - dPWTime[0])) ) / (dLambda + dLambda2)
			- exp(dLambda * (dTStar - dBondMat)) * ( exp(dLambda2 * dTStar) - exp(dLambda2 * (dTStar - dPWTime[0])) ) / dLambda2
			- exp(dLambda2 * (dTStar - dBondMat)) * ( exp(dLambda * dTStar) - exp(dLambda * (dTStar - dPWTime[0])) ) / dLambda
	...
	for(i=1;i<FixTimeIndex;++i)
	{
		ZCVar1 = (dSigma[i] / dLambda) * (dSigma[i] / dLambda) * (
				0.5 * ( exp(2 * dLambda * (dTStar - dPWTime[i-1]) ) - exp(2 * dLambda * (dTStar - dPWTime[i])) ) / dLambda
				- 2.0 * exp(dLambda * (dTStar - dBondMat)) * ( exp(dLambda * (dTStar - dPWTime[i-1])) - exp(dLambda * (dTStar - dPWTime[i])) ) / dLambda
				+ exp(2 * dLambda * (dTStar - dBondMat)) * (dPWTime[i] - dPWTime[i-1]) );

		ZCVar2 = (dAlphaLGM[i] * dSigma[i] / dLambda2) * (dAlphaLGM[i] * dSigma[i] / dLambda2) * (
				0.5 * ( exp(2 * dLambda2 * (dTStar - dPWTime[i-1])) - exp(2 * dLambda2 * (dTStar - dPWTime[i])) ) / dLambda2
				- 2.0 * exp(dLambda2 * (dTStar - dBondMat)) * ( exp(dLambda2 * (dTStar - dPWTime[i-1])) - exp(dLambda2 * (dTStar - dPWTime[i])) ) / dLambda2
				+ exp(2 * dLambda2 * (dTStar - dBondMat)) * (dPWTime[i] - dPWTime[i-1]) );

		ZCCoVar = dAlphaLGM[i] * dRhoLGM[i] * (dSigma[i] / dLambda) * (dSigma[i] / dLambda2) * (
				( exp( (dLambda + dLambda2) * (dTStar - dPWTime[i-1])) - exp( (dLambda + dLambda2) * (dTStar - dPWTime[i])) ) / (dLambda + dLambda2)
				- exp(dLambda * (dTStar - dBondMat)) * ( exp(dLambda2 * (dTStar - dPWTime[i-1])) - exp(dLambda2 * (dTStar - dPWTime[i])) ) / dLambda2
				- exp(dLambda2 * (dTStar - dBondMat)) * ( exp(dLambda * (dTStar - dPWTime[i-1])) - exp(dLambda * (dTStar - dPWTime[i])) ) / dLambda
				+ exp( (dLambda + dLambda2) * (dTStar - dBondMat)) * (dPWTime[i] - dPWTime[i-1]) );

		dPWCSigma[i] = sqrt( (ZCVar1 + ZCVar2 + 2 * ZCCoVar) / (dPWTime[i] - dPWTime[i-1]) );
	}

	ZCVar1 = (dSigma[i] / dLambda) * (dSigma[i] / dLambda) * (
			0.5 * ( exp(2 * dLambda * (dTStar - dPWTime[i-1]) ) - exp(2 * dLambda * (dTStar - dFixTime)) ) / dLambda
			- 2.0 * exp(dLambda * (dTStar - dBondMat)) * ( exp(dLambda * (dTStar - dPWTime[i-1])) - exp(dLambda * (dTStar - dFixTime)) ) / dLambda
			+ exp(2 * dLambda * (dTStar - dBondMat)) * (dFixTime - dPWTime[i-1]) );

	ZCVar2 = (dAlphaLGM[i] * dSigma[i] / dLambda2) * (dAlphaLGM[i] * dSigma[i] / dLambda2) * (
			0.5 * ( exp(2 * dLambda2 * (dTStar - dPWTime[i-1])) - exp(2 * dLambda2 * (dTStar - dFixTime)) ) / dLambda2
			- 2.0 * exp(dLambda2 * (dTStar - dBondMat)) * ( exp(dLambda2 * (dTStar - dPWTime[i-1])) - exp(dLambda2 * (dTStar - dFixTime)) ) / dLambda2
			+ exp(2 * dLambda2 * (dTStar - dBondMat)) * (dFixTime - dPWTime[i-1]) );

	ZCCoVar = dAlphaLGM[i] * dRhoLGM[i] * (dSigma[i] / dLambda) * (dSigma[i] / dLambda2) * (
			( exp( (dLambda + dLambda2) * (dTStar - dPWTime[i-1])) - exp( (dLambda + dLambda2) * (dTStar - dFixTime)) ) / (dLambda + dLambda2)
			- exp(dLambda * (dTStar - dBondMat)) * ( exp(dLambda2 * (dTStar - dPWTime[i-1])) - exp(dLambda2 * (dTStar - dFixTime)) ) / dLambda2
			- exp(dLambda2 * (dTStar - dBondMat)) * ( exp(dLambda * (dTStar - dPWTime[i-1])) - exp(dLambda * (dTStar - dFixTime)) ) / dLambda
			+ exp( (dLambda + dLambda2) * (dTStar - dBondMat)) * (dFixTime - dPWTime[i-1]) );

	dPWCSigma[i] = sqrt( (ZCVar1 + ZCVar2 + 2 * ZCCoVar) / (dFixTime - dPWTime[i-1]) );

}


void FwdAdjustLambdaOnDom(
					int		iNbPWTime,
					double	*dPWTime,
					double	*dSigma,
					double	*dAlpha,
					...
					double	*dLambdaEpsNew)
{
	...
	{
		dLambdaEpsNew[i] = dLambdaEps[i] + dAlpha[i] * dSigma[i] * (Coeff1 * dRho[i] + Coeff2 * dRho2[i]);
	}
}


void FwdAdjustLevelAndLambdaOnFor(
					int		iNbPWTime,
					double	*dPWTime,
					double	*dforAlpha,
					double	*dforLevelEps,
					...
					double	*dforLambdaEpsNew)
{
	...
	for(i=0;i<iNbPWTime;++i)
	{
		adjustment = 0.5 * dforAlpha[i] * ddomSigma[i] * (Coeff1 * dCorrelation[i][5][0] + Coeff2 * dCorrelation[i][5][1] );
					...
					double	*dforLambdaEpsNew)
{
	...
	for(i=0;i<iNbPWTime;++i)
	{
		domZCVol1 = - ddomSigma[i]*(exp(ddomLambda*(dTStar-dPWTime[i]))-1.0) / ddomLambda;
		domZCVol2 = - ddomAlphaLGM[i]*ddomSigma[i]*(exp(ddomLambda2*(dTStar-dPWTime[i]))-1.0) / ddomLambda2;

		dforLevelEpsNew[i] = dforLevelEps[i] 
		...
		forZCVol1 = - dforSigma[i]*(exp(dforLambda*(dTStar-dPWTime[i]))-1.0) / dforLambda;
		forZCVol2 = - dforAlphaLGM[i]*dforSigma[i]*(exp(dforLambda2*(dTStar-dPWTime[i]))-1.0) / dforLambda2;

		dforLambdaEpsNew[i] = dforLambdaEps[i] 
				+ dforAlpha[i] * forZCVol1 * dCorrelation[i][5][3]
						...
						double	*dPWCRho)
{
	...
	ZCVol1 = ( exp(dLambda * (dTStar - dPWTime[0])) - exp(dLambda * (dTStar - dBondMat)) ) * dSigma[0] / dLambda;
	ZCVol1 += ( exp(dLambda * dTStar) - exp(dLambda * (dTStar - dBondMat)) ) * dSigma[0] / dLambda;
	ZCVol1 = 0.5 * ZCVol1;

	ZCVol2 = ( exp(dLambda2 * (dTStar - dPWTime[0])) - exp(dLambda2 * (dTStar - dBondMat)) ) * dAlphaLGM[0] * dSigma[0] / dLambda2;
	ZCVol2 += ( exp(dLambda2 * dTStar) - exp(dLambda2 * (dTStar - dBondMat)) ) * dAlphaLGM[0] * dSigma[0] / dLambda2;
	ZCVol2 = 0.5 * ZCVol2;

	ZCVol = sqrt(ZCVol1 * ZCVol1 + ZCVol2 * ZCVol2 + 2 * dRhoLGM[0] * ZCVol1 * ZCVol2);
	...
	for(i=1;i<FixTimeIndex;++i)
	{
		...
		ZCVol1 = ( exp(dLambda * (dTStar - dPWTime[i])) - exp(dLambda * (dTStar - dBondMat)) ) * dSigma[i] / dLambda;
		ZCVol1 += ( exp(dLambda * (dTStar - dPWTime[i-1])) - exp(dLambda * (dTStar - dBondMat)) ) * dSigma[i] / dLambda;
		ZCVol1 = 0.5 * ZCVol1;

		ZCVol2 = ( exp(dLambda2 * (dTStar - dPWTime[i])) - exp(dLambda2 * (dTStar - dBondMat)) ) * dAlphaLGM[i] * dSigma[i] / dLambda2;
		ZCVol2 += ( exp(dLambda2 * (dTStar - dPWTime[i-1])) - exp(dLambda2 * (dTStar - dBondMat)) ) * dAlphaLGM[i] * dSigma[i] / dLambda2;
		ZCVol2 = 0.5 * ZCVol2;

		ZCVol = sqrt(ZCVol1 * ZCVol1 + ZCVol2 * ZCVol2 + 2 * dRhoLGM[i] * ZCVol1 * ZCVol2);
	...
	ZCVol1 = ( exp(dLambda * (dTStar - dFixTime)) - exp(dLambda * (dTStar - dBondMat)) ) * dSigma[i] / dLambda;
	ZCVol1 += ( exp(dLambda * (dTStar - dPWTime[i-1])) - exp(dLambda * (dTStar - dBondMat)) ) * dSigma[i] / dLambda;
	ZCVol1 = 0.5 * ZCVol1;

	ZCVol2 = ( exp(dLambda2 * (dTStar - dFixTime)) - exp(dLambda2 * (dTStar - dBondMat)) ) * dAlphaLGM[i] * dSigma[i] / dLambda2;
	ZCVol2 += ( exp(dLambda2 * (dTStar - dPWTime[i-1])) - exp(dLambda2 * (dTStar - dBondMat)) ) * dAlphaLGM[i] * dSigma[i] / dLambda2;
	ZCVol2 = 0.5 * ZCVol2;
						...
						double	*dBondATMVol)
{
	...
	ComputePWCBondVolAndCorrel(iNbPWTime,
					  dPWTime,
					  dSigma,
					  dAlpha,
					  dRho,
	...
	HestonClosedFormApprox(iNbPWTime,
						   dPWTime,
						   dPWCSigma,
						   dAlpha,
						   dLevelEps,
	...
	err = srt_f_optimpvol(dFwdPrice, 1, 1, dFixTime, 1, SRT_CALL, SRT_LOGNORMAL, dBondATMVol);
	if(err)
	{
		return err;
	}
	
	if(dPWCSigma) free(dPWCSigma);
	if(dPWCRho) free(dPWCRho);
						...
						double	*ddomforCov)
{
	...
	double ddomforCoVar, ddomforCoVar11, ddomforCoVar12, ddomforCoVar21, ddomforCoVar22;
	...
	domZCVar1 = (ddomSigma[0] / ddomLambda) * (ddomSigma[0] / ddomLambda) * (
			0.5 * ( exp(2 * ddomLambda * ddomTStar) - exp(2 * ddomLambda * (ddomTStar - dPWTime[0])) ) / ddomLambda
			- 2.0 * exp(ddomLambda * (ddomTStar - dBondMat)) * ( exp(ddomLambda * ddomTStar) - exp(ddomLambda * (ddomTStar - dPWTime[0])) ) / ddomLambda
			+ exp(2 * ddomLambda * (ddomTStar - dBondMat)) * dPWTime[0] );

	domZCVar2 = (ddomAlphaLGM[0] * ddomSigma[0] / ddomLambda2) * (ddomAlphaLGM[0] * ddomSigma[0] / ddomLambda2) * (
			0.5 * ( exp(2 * ddomLambda2 * ddomTStar) - exp(2 * ddomLambda2 * (ddomTStar - dPWTime[0])) ) / ddomLambda2
			- 2.0 * exp(ddomLambda2 * (ddomTStar - dBondMat)) * ( exp(ddomLambda2 * ddomTStar) - exp(ddomLambda2 * (ddomTStar - dPWTime[0])) ) / ddomLambda2
			+ exp(2 * ddomLambda2 * (ddomTStar - dBondMat)) * dPWTime[0] );

	domZCCoVar = ddomAlphaLGM[0] * ddomRhoLGM[0] * (ddomSigma[0] / ddomLambda) * (ddomSigma[0] / ddomLambda2) * (
			( exp( (ddomLambda + ddomLambda2) * ddomTStar) - exp( (ddomLambda + ddomLambda2) * (ddomTStar - dPWTime[0])) ) / (ddomLambda + ddomLambda2)
			- exp(ddomLambda * (ddomTStar - dBondMat)) * ( exp(ddomLambda2 * ddomTStar) - exp(ddomLambda2 * (ddomTStar - dPWTime[0])) ) / ddomLambda2
			- exp(ddomLambda2 * (ddomTStar - dBondMat)) * ( exp(ddomLambda * ddomTStar) - exp(ddomLambda * (ddomTStar - dPWTime[0])) ) / ddomLambda
	...
	forZCVar1 = (dforSigma[0] / dforLambda) * (dforSigma[0] / dforLambda) * (
			0.5 * ( exp(2 * dforLambda * dforTStar) - exp(2 * dforLambda * (dforTStar - dPWTime[0])) ) / dforLambda
			- 2.0 * exp(dforLambda * (dforTStar - dBondMat)) * ( exp(dforLambda * dforTStar) - exp(dforLambda * (dforTStar - dPWTime[0])) ) / dforLambda
			+ exp(2 * dforLambda * (dforTStar - dBondMat)) * dPWTime[0] );

	forZCVar2 = (dforAlphaLGM[0] * dforSigma[0] / dforLambda2) * (dforAlphaLGM[0] * dforSigma[0] / dforLambda2) * (
			0.5 * ( exp(2 * dforLambda2 * dforTStar) - exp(2 * dforLambda2 * (dforTStar - dPWTime[0])) ) / dforLambda2
			- 2.0 * exp(dforLambda2 * (dforTStar - dBondMat)) * ( exp(dforLambda2 * dforTStar) - exp(dforLambda2 * (dforTStar - dPWTime[0])) ) / dforLambda2
			+ exp(2 * dforLambda2 * (dforTStar - dBondMat)) * dPWTime[0] );

	forZCCoVar = dforAlphaLGM[0] * dforRhoLGM[0] * (dforSigma[0] / dforLambda) * (dforSigma[0] / dforLambda2) * (
			( exp( (dforLambda + dforLambda2) * dforTStar) - exp( (dforLambda + dforLambda2) * (dforTStar - dPWTime[0])) ) / (dforLambda + dforLambda2)
			- exp(dforLambda * (dforTStar - dBondMat)) * ( exp(dforLambda2 * dforTStar) - exp(dforLambda2 * (dforTStar - dPWTime[0])) ) / dforLambda2
			- exp(dforLambda2 * (dforTStar - dBondMat)) * ( exp(dforLambda * dforTStar) - exp(dforLambda * (dforTStar - dPWTime[0])) ) / dforLambda
	...
	//FX-Dom Covariance
	dfxdomCoVar1 = - dCorrelation[0][0][4] * dfxSigma[0] * ddomSigma[0] / ddomLambda * (
					( exp(ddomLambda * ddomTStar ) - exp(ddomLambda * (ddomTStar - dPWTime[0]) ) ) /ddomLambda
					- exp(ddomLambda * (ddomTStar - dBondMat) ) * dPWTime[0] );

	dfxdomCoVar2 = - dCorrelation[0][1][4] * dfxSigma[0] * ddomAlphaLGM[0] * ddomSigma[0] / ddomLambda2 * (
					( exp(ddomLambda2 * ddomTStar ) - exp(ddomLambda2 * (ddomTStar - dPWTime[0]) ) ) /ddomLambda2
					- exp(ddomLambda2 * (ddomTStar - dBondMat) ) * dPWTime[0] );
	
	dfxdomCoVar = dfxdomCoVar1 + dfxdomCoVar2;

	//FX-For Covariance
	dfxforCoVar1 = - dCorrelation[0][2][4] * dfxSigma[0] * dforSigma[0] / dforLambda * (
					( exp(dforLambda * dforTStar ) - exp(dforLambda * (dforTStar - dPWTime[0]) ) ) /dforLambda
					- exp(dforLambda * (dforTStar - dBondMat) ) * dPWTime[0] );

	dfxforCoVar2 = - dCorrelation[0][3][4] * dfxSigma[0] * dforAlphaLGM[0] * dforSigma[0] / dforLambda2 * (
					( exp(dforLambda2 * dforTStar ) - exp(dforLambda2 * (dforTStar - dPWTime[0]) ) ) /dforLambda2
					- exp(dforLambda2 * (dforTStar - dBondMat) ) * dPWTime[0] );
	
	dfxforCoVar = dfxforCoVar1 + dfxforCoVar2;

	//Dom-For Covariance
	ddomforCoVar11 = dCorrelation[0][0][2] * ddomSigma[0] * dforSigma[0] / (dforLambda * ddomLambda) * (
					- ( exp( (ddomLambda + dforLambda) * (ddomTStar - dPWTime[0]) ) - exp( (ddomLambda + dforLambda) * ddomTStar ) ) / (ddomLambda + dforLambda)
					- exp( ddomLambda * (ddomTStar - dBondMat) ) * ( exp( dforLambda * dforTStar ) - exp( dforLambda * (dforTStar - dPWTime[0]) ) ) / dforLambda
					- exp( dforLambda * (dforTStar - dBondMat) ) * ( exp( ddomLambda * ddomTStar ) - exp( ddomLambda * (ddomTStar - dPWTime[0]) ) ) / ddomLambda
					+ exp( (ddomLambda + dforLambda) * (ddomTStar - dBondMat) ) * dPWTime[0] );

	ddomforCoVar12 = dCorrelation[0][0][3] * ddomSigma[0] * dforSigma[0] * dforAlphaLGM[0] / (dforLambda2 * ddomLambda) * (
					- ( exp( (ddomLambda + dforLambda2) * (ddomTStar - dPWTime[0]) ) - exp( (ddomLambda + dforLambda2) * ddomTStar ) ) / (ddomLambda + dforLambda2)
					- exp( ddomLambda * (ddomTStar - dBondMat) ) * ( exp( dforLambda2 * dforTStar ) - exp( dforLambda2 * (dforTStar - dPWTime[0]) ) ) / dforLambda2
					- exp( dforLambda2 * (dforTStar - dBondMat) ) * ( exp( ddomLambda * ddomTStar ) - exp( ddomLambda * (ddomTStar - dPWTime[0]) ) ) / ddomLambda
					+ exp( (ddomLambda + dforLambda2) * (ddomTStar - dBondMat) ) * dPWTime[0] );
	
	ddomforCoVar21 = dCorrelation[0][1][2] * ddomSigma[0] * ddomAlphaLGM[0] * dforSigma[0] / (dforLambda * ddomLambda2) * (
					- ( exp( (ddomLambda2 + dforLambda) * (ddomTStar - dPWTime[0]) ) - exp( (ddomLambda2 + dforLambda) * ddomTStar ) ) / (ddomLambda2 + dforLambda)
					- exp( ddomLambda2 * (ddomTStar - dBondMat) ) * ( exp( dforLambda * dforTStar ) - exp( dforLambda * (dforTStar - dPWTime[0]) ) ) / dforLambda
					- exp( dforLambda * (dforTStar - dBondMat) ) * ( exp( ddomLambda2 * ddomTStar ) - exp( ddomLambda2 * (ddomTStar - dPWTime[0]) ) ) / ddomLambda2
					+ exp( (ddomLambda2 + dforLambda) * (ddomTStar - dBondMat) ) * dPWTime[0] );

	ddomforCoVar22 = dCorrelation[0][1][3] * ddomSigma[0] * ddomAlphaLGM[0] * dforSigma[0] * dforAlphaLGM[0] / (dforLambda2 * ddomLambda2) * (
					- ( exp( (ddomLambda2 + dforLambda2) * (ddomTStar - dPWTime[0]) ) - exp( (ddomLambda2 + dforLambda2) * ddomTStar ) ) / (ddomLambda2 + dforLambda2)
					- exp( ddomLambda2 * (ddomTStar - dBondMat) ) * ( exp( dforLambda2 * dforTStar ) - exp( dforLambda2 * (dforTStar - dPWTime[0]) ) ) / dforLambda2
					- exp( dforLambda2 * (dforTStar - dBondMat) ) * ( exp( ddomLambda2 * ddomTStar ) - exp( ddomLambda2 * (ddomTStar - dPWTime[0]) ) ) / ddomLambda2
					+ exp( (ddomLambda2 + dforLambda2) * (ddomTStar - dBondMat) ) * dPWTime[0] );

	ddomforCoVar = ddomforCoVar11 + ddomforCoVar12 + ddomforCoVar21 + ddomforCoVar22;

	for(i=1;i<FixTimeIndex;++i)
	{
		//	Domestic Bond Variance
		domZCVar1 = (ddomSigma[i] / ddomLambda) * (ddomSigma[i] / ddomLambda) * (
				0.5 * ( exp(2 * ddomLambda * (ddomTStar - dPWTime[i-1]) ) - exp(2 * ddomLambda * (ddomTStar - dPWTime[i])) ) / ddomLambda
				- 2.0 * exp(ddomLambda * (ddomTStar - dBondMat)) * ( exp(ddomLambda * (ddomTStar - dPWTime[i-1])) - exp(ddomLambda * (ddomTStar - dPWTime[i])) ) / ddomLambda
				+ exp(2 * ddomLambda * (ddomTStar - dBondMat)) * (dPWTime[i] - dPWTime[i-1]) );

		domZCVar2 = (ddomAlphaLGM[i] * ddomSigma[i] / ddomLambda2) * (ddomAlphaLGM[i] * ddomSigma[i] / ddomLambda2) * (
				0.5 * ( exp(2 * ddomLambda2 * (ddomTStar - dPWTime[i-1])) - exp(2 * ddomLambda2 * (ddomTStar - dPWTime[i])) ) / ddomLambda2
				- 2.0 * exp(ddomLambda2 * (ddomTStar - dBondMat)) * ( exp(ddomLambda2 * (ddomTStar - dPWTime[i-1])) - exp(ddomLambda2 * (ddomTStar - dPWTime[i])) ) / ddomLambda2
				+ exp(2 * ddomLambda2 * (ddomTStar - dBondMat)) * (dPWTime[i] - dPWTime[i-1]) );

		domZCCoVar = ddomAlphaLGM[i] * ddomRhoLGM[i] * (ddomSigma[i] / ddomLambda) * (ddomSigma[i] / ddomLambda2) * (
				( exp( (ddomLambda + ddomLambda2) * (ddomTStar - dPWTime[i-1])) - exp( (ddomLambda + ddomLambda2) * (ddomTStar - dPWTime[i])) ) / (ddomLambda + ddomLambda2)
				- exp(ddomLambda * (ddomTStar - dBondMat)) * ( exp(ddomLambda2 * (ddomTStar - dPWTime[i-1])) - exp(ddomLambda2 * (ddomTStar - dPWTime[i])) ) / ddomLambda2
				- exp(ddomLambda2 * (ddomTStar - dBondMat)) * ( exp(ddomLambda * (ddomTStar - dPWTime[i-1])) - exp(ddomLambda * (ddomTStar - dPWTime[i])) ) / ddomLambda
				+ exp( (ddomLambda + ddomLambda2) * (ddomTStar - dBondMat)) * (dPWTime[i] - dPWTime[i-1]) );

		domZCVar += domZCVar1 + domZCVar2 + 2 * domZCCoVar;

		//	Foreign Bond Variance
		forZCVar1 = (dforSigma[i] / dforLambda) * (dforSigma[i] / dforLambda) * (
				0.5 * ( exp(2 * dforLambda * (dforTStar - dPWTime[i-1]) ) - exp(2 * dforLambda * (dforTStar - dPWTime[i])) ) / dforLambda
				- 2.0 * exp(dforLambda * (dforTStar - dBondMat)) * ( exp(dforLambda * (dforTStar - dPWTime[i-1])) - exp(dforLambda * (dforTStar - dPWTime[i])) ) / dforLambda
				+ exp(2 * dforLambda * (dforTStar - dBondMat)) * (dPWTime[i] - dPWTime[i-1]) );

		forZCVar2 = (dforAlphaLGM[i] * dforSigma[i] / dforLambda2) * (dforAlphaLGM[i] * dforSigma[i] / dforLambda2) * (
				0.5 * ( exp(2 * dforLambda2 * (dforTStar - dPWTime[i-1])) - exp(2 * dforLambda2 * (dforTStar - dPWTime[i])) ) / dforLambda2
				- 2.0 * exp(dforLambda2 * (dforTStar - dBondMat)) * ( exp(dforLambda2 * (dforTStar - dPWTime[i-1])) - exp(dforLambda2 * (dforTStar - dPWTime[i])) ) / dforLambda2
				+ exp(2 * dforLambda2 * (dforTStar - dBondMat)) * (dPWTime[i] - dPWTime[i-1]) );

		forZCCoVar = dforAlphaLGM[i] * dforRhoLGM[i] * (dforSigma[i] / dforLambda) * (dforSigma[i] / dforLambda2) * (
				( exp( (dforLambda + dforLambda2) * (dforTStar - dPWTime[i-1])) - exp( (dforLambda + dforLambda2) * (dforTStar - dPWTime[i])) ) / (dforLambda + dforLambda2)
				- exp(dforLambda * (dforTStar - dBondMat)) * ( exp(dforLambda2 * (dforTStar - dPWTime[i-1])) - exp(dforLambda2 * (dforTStar - dPWTime[i])) ) / dforLambda2
				- exp(dforLambda2 * (dforTStar - dBondMat)) * ( exp(dforLambda * (dforTStar - dPWTime[i-1])) - exp(dforLambda * (dforTStar - dPWTime[i])) ) / dforLambda
				+ exp( (dforLambda + dforLambda2) * (dforTStar - dBondMat)) * (dPWTime[i] - dPWTime[i-1]) );
		...
		//FX-Dom Covariance
		dfxdomCoVar1 = - dCorrelation[i][0][4] * dfxSigma[i] * ddomSigma[i] / ddomLambda * (
						( exp(ddomLambda * (ddomTStar - dPWTime[i-1]) ) - exp(ddomLambda * (ddomTStar - dPWTime[i]) ) ) /ddomLambda
						- exp(ddomLambda * (ddomTStar - dBondMat) ) * (dPWTime[i] - dPWTime[i-1]) );

		dfxdomCoVar2 = - dCorrelation[i][1][4] * dfxSigma[i] * ddomAlphaLGM[i] * ddomSigma[i] / ddomLambda2 * (
						( exp(ddomLambda2 * (ddomTStar - dPWTime[i-1]) ) - exp(ddomLambda2 * (ddomTStar - dPWTime[i]) ) ) /ddomLambda2
						- exp(ddomLambda2 * (ddomTStar - dBondMat) ) * (dPWTime[i] - dPWTime[i-1]) );
		
		dfxdomCoVar += dfxdomCoVar1 + dfxdomCoVar2;

		//FX-For Covariance
		dfxforCoVar1 = - dCorrelation[i][2][4] * dfxSigma[i] * dforSigma[i] / dforLambda * (
						( exp(dforLambda * (dforTStar - dPWTime[i-1]) ) - exp(dforLambda * (dforTStar - dPWTime[i]) ) ) /dforLambda
						- exp(dforLambda * (dforTStar - dBondMat) ) * (dPWTime[i] - dPWTime[i-1]) );

		dfxforCoVar2 = - dCorrelation[i][3][4] * dfxSigma[i] * dforAlphaLGM[i] * dforSigma[i] / dforLambda2 * (
						( exp(dforLambda2 * (dforTStar - dPWTime[i-1]) ) - exp(dforLambda2 * (dforTStar - dPWTime[i]) ) ) /dforLambda2
						- exp(dforLambda2 * (dforTStar - dBondMat) ) * (dPWTime[i] - dPWTime[i-1]) );
		
		dfxforCoVar += dfxforCoVar1 + dfxforCoVar2;

		//Dom-For Covariance
		ddomforCoVar11 = dCorrelation[i][0][2] * ddomSigma[i] * dforSigma[i] / (dforLambda * ddomLambda) * (
						- ( exp( (ddomLambda + dforLambda) * (ddomTStar - dPWTime[i]) ) - exp( (ddomLambda + dforLambda) * (ddomTStar - dPWTime[i-1]) ) ) / (ddomLambda + dforLambda)
						- exp( ddomLambda * (ddomTStar - dBondMat) ) * ( exp( dforLambda * (dforTStar - dPWTime[i-1]) ) - exp( dforLambda * (dforTStar - dPWTime[i]) ) ) / dforLambda
						- exp( dforLambda * (dforTStar - dBondMat) ) * ( exp( ddomLambda * (ddomTStar - dPWTime[i-1]) ) - exp( ddomLambda * (ddomTStar - dPWTime[i]) ) ) / ddomLambda
						+ exp( (ddomLambda + dforLambda) * (ddomTStar - dBondMat) ) * (dPWTime[i] - dPWTime[i-1]) );

		ddomforCoVar12 = dCorrelation[i][0][3] * ddomSigma[i] * dforSigma[i] * dforAlphaLGM[i] / (dforLambda2 * ddomLambda) * (
						- ( exp( (ddomLambda + dforLambda2) * (ddomTStar - dPWTime[i]) ) - exp( (ddomLambda + dforLambda2) * (ddomTStar - dPWTime[i-1]) ) ) / (ddomLambda + dforLambda2)
						- exp( ddomLambda * (ddomTStar - dBondMat) ) * ( exp( dforLambda2 * (dforTStar - dPWTime[i-1]) ) - exp( dforLambda2 * (dforTStar - dPWTime[i]) ) ) / dforLambda2
						- exp( dforLambda2 * (dforTStar - dBondMat) ) * ( exp( ddomLambda * (ddomTStar - dPWTime[i-1]) ) - exp( ddomLambda * (ddomTStar - dPWTime[i]) ) ) / ddomLambda
						+ exp( (ddomLambda + dforLambda2) * (ddomTStar - dBondMat) ) * (dPWTime[i] - dPWTime[i-1]) );
		
		ddomforCoVar21 = dCorrelation[i][1][2] * ddomSigma[i] * ddomAlphaLGM[i] * dforSigma[i] / (dforLambda * ddomLambda2) * (
						- ( exp( (ddomLambda2 + dforLambda) * (ddomTStar - dPWTime[i]) ) - exp( (ddomLambda2 + dforLambda) * (ddomTStar - dPWTime[i-1]) ) ) / (ddomLambda2 + dforLambda)
						- exp( ddomLambda2 * (ddomTStar - dBondMat) ) * ( exp( dforLambda * (dforTStar - dPWTime[i-1]) ) - exp( dforLambda * (dforTStar - dPWTime[i]) ) ) / dforLambda
						- exp( dforLambda * (dforTStar - dBondMat) ) * ( exp( ddomLambda2 * (ddomTStar - dPWTime[i-1]) ) - exp( ddomLambda2 * (ddomTStar - dPWTime[i]) ) ) / ddomLambda2
						+ exp( (ddomLambda2 + dforLambda) * (ddomTStar - dBondMat) ) * (dPWTime[i] - dPWTime[i-1]) );

		ddomforCoVar22 = dCorrelation[i][1][3] * ddomSigma[i] * ddomAlphaLGM[i] * dforSigma[i] * dforAlphaLGM[i] / (dforLambda2 * ddomLambda2) * (
						- ( exp( (ddomLambda2 + dforLambda2) * (ddomTStar - dPWTime[i]) ) - exp( (ddomLambda2 + dforLambda2) * (ddomTStar - dPWTime[i-1]) ) ) / (ddomLambda2 + dforLambda2)
						- exp( ddomLambda2 * (ddomTStar - dBondMat) ) * ( exp( dforLambda2 * (dforTStar - dPWTime[i-1]) ) - exp( dforLambda2 * (dforTStar - dPWTime[i]) ) ) / dforLambda2
						- exp( dforLambda2 * (dforTStar - dBondMat) ) * ( exp( ddomLambda2 * (ddomTStar - dPWTime[i-1]) ) - exp( ddomLambda2 * (ddomTStar - dPWTime[i]) ) ) / ddomLambda2
						+ exp( (ddomLambda2 + dforLambda2) * (ddomTStar - dBondMat) ) * (dPWTime[i] - dPWTime[i-1]) );
	...
	//	Domestic Bond Variance
	domZCVar1 = (ddomSigma[i] / ddomLambda) * (ddomSigma[i] / ddomLambda) * (
				0.5 * ( exp(2 * ddomLambda * (ddomTStar - dPWTime[i-1]) ) - exp(2 * ddomLambda * (ddomTStar - dFixTime)) ) / ddomLambda
				- 2.0 * exp(ddomLambda * (ddomTStar - dBondMat)) * ( exp(ddomLambda * (ddomTStar - dPWTime[i-1])) - exp(ddomLambda * (ddomTStar - dFixTime)) ) / ddomLambda
				+ exp(2 * ddomLambda * (ddomTStar - dBondMat)) * (dFixTime - dPWTime[i-1]) );

	domZCVar2 = (ddomAlphaLGM[i] * ddomSigma[i] / ddomLambda2) * (ddomAlphaLGM[i] * ddomSigma[i] / ddomLambda2) * (
				0.5 * ( exp(2 * ddomLambda2 * (ddomTStar - dPWTime[i-1])) - exp(2 * ddomLambda2 * (ddomTStar - dFixTime)) ) / ddomLambda2
				- 2.0 * exp(ddomLambda2 * (ddomTStar - dBondMat)) * ( exp(ddomLambda2 * (ddomTStar - dPWTime[i-1])) - exp(ddomLambda2 * (ddomTStar - dFixTime)) ) / ddomLambda2
				+ exp(2 * ddomLambda2 * (ddomTStar - dBondMat)) * (dFixTime - dPWTime[i-1]) );

	domZCCoVar = ddomAlphaLGM[i] * ddomRhoLGM[i] * (ddomSigma[i] / ddomLambda) * (ddomSigma[i] / ddomLambda2) * (
				( exp( (ddomLambda + ddomLambda2) * (ddomTStar - dPWTime[i-1])) - exp( (ddomLambda + ddomLambda2) * (ddomTStar - dFixTime)) ) / (ddomLambda + ddomLambda2)
				- exp(ddomLambda * (ddomTStar - dBondMat)) * ( exp(ddomLambda2 * (ddomTStar - dPWTime[i-1])) - exp(ddomLambda2 * (ddomTStar - dFixTime)) ) / ddomLambda2
				- exp(ddomLambda2 * (ddomTStar - dBondMat)) * ( exp(ddomLambda * (ddomTStar - dPWTime[i-1])) - exp(ddomLambda * (ddomTStar - dFixTime)) ) / ddomLambda
				+ exp( (ddomLambda + ddomLambda2) * (ddomTStar - dBondMat)) * (dFixTime - dPWTime[i-1]) );

	domZCVar += domZCVar1 + domZCVar2 + 2 * domZCCoVar;

	//	Foreign Bond Variance
	forZCVar1 = (dforSigma[i] / dforLambda) * (dforSigma[i] / dforLambda) * (
				0.5 * ( exp(2 * dforLambda * (dforTStar - dPWTime[i-1]) ) - exp(2 * dforLambda * (dforTStar - dFixTime)) ) / dforLambda
				- 2.0 * exp(dforLambda * (dforTStar - dBondMat)) * ( exp(dforLambda * (dforTStar - dPWTime[i-1])) - exp(dforLambda * (dforTStar - dFixTime)) ) / dforLambda
				+ exp(2 * dforLambda * (dforTStar - dBondMat)) * (dFixTime - dPWTime[i-1]) );

	forZCVar2 = (dforAlphaLGM[i] * dforSigma[i] / dforLambda2) * (dforAlphaLGM[i] * dforSigma[i] / dforLambda2) * (
				0.5 * ( exp(2 * dforLambda2 * (dforTStar - dPWTime[i-1])) - exp(2 * dforLambda2 * (dforTStar - dFixTime)) ) / dforLambda2
				- 2.0 * exp(dforLambda2 * (dforTStar - dBondMat)) * ( exp(dforLambda2 * (dforTStar - dPWTime[i-1])) - exp(dforLambda2 * (dforTStar - dFixTime)) ) / dforLambda2
				+ exp(2 * dforLambda2 * (dforTStar - dBondMat)) * (dFixTime - dPWTime[i-1]) );

	forZCCoVar = dforAlphaLGM[i] * dforRhoLGM[i] * (dforSigma[i] / dforLambda) * (dforSigma[i] / dforLambda2) * (
				( exp( (dforLambda + dforLambda2) * (dforTStar - dPWTime[i-1])) - exp( (dforLambda + dforLambda2) * (dforTStar - dFixTime)) ) / (dforLambda + dforLambda2)
				- exp(dforLambda * (dforTStar - dBondMat)) * ( exp(dforLambda2 * (dforTStar - dPWTime[i-1])) - exp(dforLambda2 * (dforTStar - dFixTime)) ) / dforLambda2
				- exp(dforLambda2 * (dforTStar - dBondMat)) * ( exp(dforLambda * (dforTStar - dPWTime[i-1])) - exp(dforLambda * (dforTStar - dFixTime)) ) / dforLambda
				+ exp( (dforLambda + dforLambda2) * (dforTStar - dBondMat)) * (dFixTime - dPWTime[i-1]) );
	...
	//FX-Dom Covariance
	dfxdomCoVar1 = - dCorrelation[i][0][4] * dfxSigma[i] * ddomSigma[i] / ddomLambda * (
						( exp(ddomLambda * (ddomTStar - dPWTime[i-1]) ) - exp(ddomLambda * (ddomTStar - dFixTime) ) ) /ddomLambda
						- exp(ddomLambda * (ddomTStar - dBondMat) ) * (dFixTime - dPWTime[i-1]) );

	dfxdomCoVar2 = - dCorrelation[i][1][4] * dfxSigma[i] * ddomAlphaLGM[i] * ddomSigma[i] / ddomLambda2 * (
						( exp(ddomLambda2 * (ddomTStar - dPWTime[i-1]) ) - exp(ddomLambda2 * (ddomTStar - dFixTime) ) ) /ddomLambda2
	...
	//FX-For Covariance
	dfxforCoVar1 = - dCorrelation[i][2][4] * dfxSigma[i] * dforSigma[i] / dforLambda * (
						( exp(dforLambda * (dforTStar - dPWTime[i-1]) ) - exp(dforLambda * (dforTStar - dFixTime) ) ) /dforLambda
						- exp(dforLambda * (dforTStar - dBondMat) ) * (dFixTime - dPWTime[i-1]) );

	dfxforCoVar2 = - dCorrelation[i][3][4] * dfxSigma[i] * dforAlphaLGM[i] * dforSigma[i] / dforLambda2 * (
						( exp(dforLambda2 * (dforTStar - dPWTime[i-1]) ) - exp(dforLambda2 * (dforTStar - dFixTime) ) ) /dforLambda2
	...
	//Dom-For Covariance
	ddomforCoVar11 =  dCorrelation[i][0][2] * ddomSigma[i] * dforSigma[i] / (dforLambda * ddomLambda) * (
						-( exp( (ddomLambda + dforLambda) * (ddomTStar - dFixTime) ) - exp( (ddomLambda + dforLambda) * (ddomTStar - dPWTime[i-1]) ) ) / (ddomLambda + dforLambda)
						- exp( ddomLambda * (ddomTStar - dBondMat) ) * ( exp( dforLambda * (dforTStar - dPWTime[i-1]) ) - exp( dforLambda * (dforTStar - dFixTime) ) ) / dforLambda
						- exp( dforLambda * (dforTStar - dBondMat) ) * ( exp( ddomLambda * (ddomTStar - dPWTime[i-1]) ) - exp( ddomLambda * (ddomTStar - dFixTime) ) ) / ddomLambda
						+ exp( (ddomLambda + dforLambda) * (ddomTStar - dBondMat) ) * (dFixTime - dPWTime[i-1]) );

	ddomforCoVar12 =  dCorrelation[i][0][3] * ddomSigma[i] * dforSigma[i] * dforAlphaLGM[i] / (dforLambda2 * ddomLambda) * (
						-( exp( (ddomLambda + dforLambda2) * (ddomTStar - dFixTime) ) - exp( (ddomLambda + dforLambda2) * (ddomTStar - dPWTime[i-1]) ) ) / (ddomLambda + dforLambda2)
						- exp( ddomLambda * (ddomTStar - dBondMat) ) * ( exp( dforLambda2 * (dforTStar - dPWTime[i-1]) ) - exp( dforLambda2 * (dforTStar - dFixTime) ) ) / dforLambda2
						- exp( dforLambda2 * (dforTStar - dBondMat) ) * ( exp( ddomLambda * (ddomTStar - dPWTime[i-1]) ) - exp( ddomLambda * (ddomTStar - dFixTime) ) ) / ddomLambda
						+ exp( (ddomLambda + dforLambda2) * (ddomTStar - dBondMat) ) * (dFixTime - dPWTime[i-1]) );
		
	ddomforCoVar21 =  dCorrelation[i][1][2] * ddomSigma[i] * ddomAlphaLGM[i] * dforSigma[i] / (dforLambda * ddomLambda2) * (
						-( exp( (ddomLambda2 + dforLambda) * (ddomTStar - dFixTime) ) - exp( (ddomLambda2 + dforLambda) * (ddomTStar - dPWTime[i-1]) ) ) / (ddomLambda2 + dforLambda)
						- exp( ddomLambda2 * (ddomTStar - dBondMat) ) * ( exp( dforLambda * (dforTStar - dPWTime[i-1]) ) - exp( dforLambda * (dforTStar - dFixTime) ) ) / dforLambda
						- exp( dforLambda * (dforTStar - dBondMat) ) * ( exp( ddomLambda2 * (ddomTStar - dPWTime[i-1]) ) - exp( ddomLambda2 * (ddomTStar - dFixTime) ) ) / ddomLambda2
						+ exp( (ddomLambda2 + dforLambda) * (ddomTStar - dBondMat) ) * (dFixTime - dPWTime[i-1]) );

	ddomforCoVar22 =  dCorrelation[i][1][3] * ddomSigma[i] * ddomAlphaLGM[i] * dforSigma[i] * dforAlphaLGM[i] / (dforLambda2 * ddomLambda2) * (
						-( exp( (ddomLambda2 + dforLambda2) * (ddomTStar - dFixTime) ) - exp( (ddomLambda2 + dforLambda2) * (ddomTStar - dPWTime[i-1]) ) ) / (ddomLambda2 + dforLambda2)
						- exp( ddomLambda2 * (ddomTStar - dBondMat) ) * ( exp( dforLambda2 * (dforTStar - dPWTime[i-1]) ) - exp( dforLambda2 * (dforTStar - dFixTime) ) ) / dforLambda2
						- exp( dforLambda2 * (dforTStar - dBondMat) ) * ( exp( ddomLambda2 * (ddomTStar - dPWTime[i-1]) ) - exp( ddomLambda2 * (ddomTStar - dFixTime) ) ) / ddomLambda2
						+ exp( (ddomLambda2 + dforLambda2) * (ddomTStar - dBondMat) ) * (dFixTime - dPWTime[i-1]) );
						...
						double	*dAlphaforCov)
{
	...
	dAlphadomCoVar1 = - dCorrelation[0][0][5] * dAlpha[0] * ddomSigma[0] / ddomLambda * (
					( exp(ddomLambda * ddomTStar ) - exp(ddomLambda * (ddomTStar - dPWTime[0]) ) ) /ddomLambda
					- exp(ddomLambda * (ddomTStar - dBondMat) ) * dPWTime[0] );

	dAlphadomCoVar2 = - dCorrelation[0][1][5] * dAlpha[0] * ddomAlphaLGM[0] * ddomSigma[0] / ddomLambda2 * (
					( exp(ddomLambda2 * ddomTStar ) - exp(ddomLambda2 * (ddomTStar - dPWTime[0]) ) ) /ddomLambda2
					- exp(ddomLambda2 * (ddomTStar - dBondMat) ) * dPWTime[0] );
	...
	dAlphaforCoVar1 = - dCorrelation[0][0][5] * dAlpha[0] * dforSigma[0] / dforLambda * (
					( exp(dforLambda * dforTStar ) - exp(dforLambda * (dforTStar - dPWTime[0]) ) ) /dforLambda
					- exp(dforLambda * (dforTStar - dBondMat) ) * dPWTime[0] );

	dAlphaforCoVar2 = - dCorrelation[0][1][5] * dAlpha[0] * dforAlphaLGM[0] * dforSigma[0] / dforLambda2 * (
					( exp(dforLambda2 * dforTStar ) - exp(dforLambda2 * (dforTStar - dPWTime[0]) ) ) /dforLambda2
					- exp(dforLambda2 * (dforTStar - dBondMat) ) * dPWTime[0] );
	...
	for(i=1;i<FixTimeIndex;++i)
	{
		...
		//Alpha-Dom Covariance
		dAlphadomCoVar1 = - dCorrelation[i][0][5] * dAlpha[i] * ddomSigma[i] / ddomLambda * (
						( exp(ddomLambda * (ddomTStar - dPWTime[i-1]) ) - exp(ddomLambda * (ddomTStar - dPWTime[i]) ) ) /ddomLambda
						- exp(ddomLambda * (ddomTStar - dBondMat) ) * (dPWTime[i] - dPWTime[i-1]) );

		dAlphadomCoVar2 = - dCorrelation[i][1][5] * dAlpha[i] * ddomAlphaLGM[i] * ddomSigma[i] / ddomLambda2 * (
						( exp(ddomLambda2 * (ddomTStar - dPWTime[i-1]) ) - exp(ddomLambda2 * (ddomTStar - dPWTime[i]) ) ) /ddomLambda2
						- exp(ddomLambda2 * (ddomTStar - dBondMat) ) * (dPWTime[i] - dPWTime[i-1]) );
		...
		//Alpha-for Covariance
		dAlphaforCoVar1 = - dCorrelation[i][0][5] * dAlpha[i] * dforSigma[i] / dforLambda * (
						( exp(dforLambda * (dforTStar - dPWTime[i-1]) ) - exp(dforLambda * (dforTStar - dPWTime[i]) ) ) /dforLambda
						- exp(dforLambda * (dforTStar - dBondMat) ) * (dPWTime[i] - dPWTime[i-1]) );

		dAlphaforCoVar2 = - dCorrelation[i][1][5] * dAlpha[i] * dforAlphaLGM[i] * dforSigma[i] / dforLambda2 * (
						( exp(dforLambda2 * (dforTStar - dPWTime[i-1]) ) - exp(dforLambda2 * (dforTStar - dPWTime[i]) ) ) /dforLambda2
						- exp(dforLambda2 * (dforTStar - dBondMat) ) * (dPWTime[i] - dPWTime[i-1]) );
		...
		//FX-Alpha Covariance
		dAlphafxCoVar = dfxSigma[i] * dAlpha[i] * dCorrelation[i][4][5] * (dPWTime[i] - dPWTime[i-1]);
	...
	//FX-Alpha Covariance
	dAlphafxCoVar = dfxSigma[i] * dAlpha[i] * dCorrelation[i][4][5] * (dFixTime - dPWTime[i-1]);
	dAlphafxCov[i] = dAlphafxCoVar;
						...
						double	***dCorrelation)
{
	...
	dCorrelation[0][1][3] = dAlphadomCov[0] / sqrt(dAlphaVariance[0] * ddomVar[0]);

	dCorrelation[0][2][3] = dAlphaforCov[0] / sqrt(dAlphaVariance[0] * dforVar[0]);
	...
	for(i=1;i<FixTimeIndex;++i)
	{
		...
		dCorrelation[i][1][3] = dAlphadomCov[i] / sqrt(dAlphaVariance[i] * ddomVar[i]);

		dCorrelation[i][2][3] = dAlphaforCov[i] / sqrt(dAlphaVariance[i] * dforVar[i]);
													...
													// 3 : h4
{
	...
	if(	(!ddomVar) || (!dforVar) || (!dfxVar) || (!dAlphaVariance)
		|| (!ddomfxCov) || (!dforfxCov) || (!ddomforCov) 
		|| (!dAlphafxCov) || (!dAlphadomCov) || (!dAlphaforCov) )
	{
						...
						double	*dVol)
{
	...
	*dVol = sqrt( (ddomVar + dforVar + dfxVar + 2 * ddomfxCov + 2 * dforfxCov + 2 * ddomfxCov) / dFixTime);
...
Err FXLGMSV_FXImpliedVolApprox_FromTS(
						...
						//Correlation
						double	***d5FCorrelation,
						double	***dFullCorrelation,
						...
						double	*FXImpliedVol)
{
	...
	*FXImpliedVol = sqrt( 
					( dfxVar + ddomBondATMVol * ddomBondATMVol * dFixTime
					+ dforBondATMVol * dforBondATMVol * dFixTime
					- 2 * (ddomfxCov / sqrt(ddomVar * dfxVar)) * sqrt(dfxVar/dFixTime) * ddomBondATMVol * dFixTime
					+ 2 * (dforfxCov / sqrt(dforVar * dfxVar)) * sqrt(dfxVar/dFixTime) * dforBondATMVol * dFixTime
					- 2 * (ddomforCov / sqrt(ddomVar * dforVar)) * ddomBondATMVol * dforBondATMVol * dFixTime
					) / dFixTime );
...
Err FXLGMSV_merge_fx_ir_ir_model_ts(	
							...
							int		*merge_n,
							 ...
							 /* Domestic TS */
							double	**merge_dom_sigma,
							double	**merge_dom_alphaLGM,
							double	**merge_dom_rhoLGM,
							double	**merge_dom_alphaSV,
							double	**merge_dom_lamSV,
							double	**merge_dom_rhoSV,
							double	**merge_dom_rho2SV,
							 ...
							 /* Foreign TS */
							double	**merge_for_sigma,
							double	**merge_for_alphaLGM,
							double	**merge_for_rhoLGM,
							double	**merge_for_alphaSV,
							double	**merge_for_lamSV,
							double	**merge_for_rhoSV,
							double	**merge_for_rho2SV,
						...
						double	****dFullCorrelation)
{
	...
	fxlgmsv_get_fx_and_correl_ts(UndName, &fx_spot, &fx_n, &fx_time, &fx_vol, &rho_n, &rho_time, &CorrMatrix);
...
//	d2vol = *dd2vol;
	...
	var += (h1[i]*h1[i] + h2[i]*h2[i] + h3[i]*h3[i]
			...
			- 2*h2[i]*h3[i]*correlation[i][1][2]) * (dPWTime[indexMat1] - Mat1);

	d1Integral += (h1[i]*h3[i]*correlation[i][0][2]
				  - h2[i]*h3[i]*correlation[i][1][2]) * (exp(-kappa*Mat1) - exp(-kappa*dPWTime[indexMat1])) / kappa;
					;
	d1Integral += 2*h3[i]*h3[i]*(exp(-kappa*Mat1) - exp(-kappa*dPWTime[indexMat1])) / kappa;

	d2Integral += (h1[i]*h3[i]*correlation[i][0][2]
				  - h2[i]*h3[i]*correlation[i][1][2]) * (exp(-2*kappa*Mat1) - exp(-2*kappa*dPWTime[indexMat1])) / (2*kappa);
	...
	for(i=indexMat1+1;i<indexMat2;++i)
	{
		var += (h1[i]*h1[i] + h2[i]*h2[i] + h3[i]*h3[i]
				...
				- 2*h2[i]*h3[i]*correlation[i][1][2]) * (dPWTime[i] - dPWTime[i-1]);

		d1Integral += (h1[i]*h3[i]*correlation[i][0][2]
					  - h2[i]*h3[i]*correlation[i][1][2]) * (exp(-kappa*dPWTime[i-1]) - exp(-kappa*dPWTime[i])) / kappa;
						;
		d1Integral += 2*h3[i]*h3[i]*(exp(-kappa*dPWTime[i-1]) - exp(-kappa*dPWTime[i])) / kappa;

		d2Integral += (h1[i]*h3[i]*correlation[i][0][2]
					  - h2[i]*h3[i]*correlation[i][1][2]) * (exp(-2*kappa*dPWTime[i-1]) - exp(-2*kappa*dPWTime[i])) / (2*kappa);
	}
	...
	var += (h1[i]*h1[i] + h2[i]*h2[i] + h3[i]*h3[i]
			...
			- 2*h2[i]*h3[i]*correlation[i][1][2]) * (Mat2 - dPWTime[max(0,i-1)]);

	d1Integral += (h1[i]*h3[i]*correlation[i][0][2]
				  - h2[i]*h3[i]*correlation[i][1][2]) * (exp(-kappa*dPWTime[max(0,i-1)]) - exp(-kappa*Mat2)) / kappa;
					;
	d1Integral += 2*h3[i]*h3[i]*(exp(-kappa*dPWTime[max(0,i-1)]) - exp(-kappa*Mat2)) / kappa;

	d2Integral += (h1[i]*h3[i]*correlation[i][0][2]
					  - h2[i]*h3[i]*correlation[i][1][2]) * (exp(-2*kappa*dPWTime[max(0,i-1)]) - exp(-2*kappa*Mat2)) / (2*kappa);

	*dvol = sqrt(var / (Mat2-Mat1));
	*dd1vol = 0.25 * d1Integral / ((*dvol) * (Mat2-Mat1));
	*dd2vol = -( (*dd1vol) * (*dd1vol) + 0.125 * d2Integral / (Mat2-Mat1) ) / (*dvol);
						...
						double	*dVol)
{
	...
	double Mat1, Mat2;
	double price, Fwd, BSVega, BSVolga, BSVanna, coeffVanna, coeffVolga, Volga, Vanna;
	...
	dd1vol = 0.0;
	dd2vol = 0.0;
	compute_vol_derivatives(iNbPWTime, dPWTime,
								h1, h2, h3, h4,
								kappa, correlation,
								0, Maturity,
								&dvol, &dd1vol, &dd2vol);
	...
	for(i=1;i<=NLegendreQuad;++i)
	{
		...
		for(j=1;j<=NHermiteQuad;++j)
		{
			...
			vol = sqrt((dvol_full*dvol_full*Maturity - dvol*dvol*Mat2)/(Maturity-Mat2));
			BSVega = srt_f_optblksch(Fwd, 1.0, vol, Maturity-Mat2, 1.0, SRT_CALL, VEGA);
			BSVolga = srt_f_optblksch(Fwd, 1.0, vol, Maturity-Mat2, 1.0, SRT_CALL, VOLGA);
			BSVanna = srt_f_optblksch(Fwd, 1.0, vol, Maturity-Mat2, 1.0, SRT_CALL, VANNA);
	...
	err = srt_f_optimpvol(price, 1, 1, Maturity, 1, SRT_CALL, SRT_LOGNORMAL, dVol);

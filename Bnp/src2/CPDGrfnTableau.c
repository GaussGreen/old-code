...
double						**aux)
{
	...
	int						i, j, k;
	int						ncpn,
							nexo = exo_leg->num_cpn,
							nfund = fund_leg->num_cpn,
							ncall = cpd->num_calls,
							ncols = 5;
	...
	if (ncall > 0 && nexo > 0 && call[0].ex_date > today  && call[0].ex_date < exo_leg->cpn[0].fx_fix_date - 60)
	{
		first_ex_flag = 1;
	}
	else
	...
	*num_evt_dtes = ncpn = nexo + today_flag + first_ex_flag; 
	evt_dtes[0] = today;
	if (first_ex_flag) evt_dtes[1] = call[0].ex_date;
	for (i=0; i<nexo; i++)
	{
	...
	for (i=0; i<ncpn; i++)
	{
		...
		sprintf(
			evt[i][0],
			"S(\"%s\")*DF(D[I],A[2,I],\"%s\")/DF(D[I],A[2,I],\"%s\")",
			fx_name,
			for_name,
			dom_name);
	}
	...
	for (i=0; i<nexo; i++)
	{
		...
		aux[today_flag+first_ex_flag+i][6] = (exo_leg->cpn[i].floored? exo_leg->cpn[i].floor: NO_FLOOR);
		aux[today_flag+first_ex_flag+i][7] = (exo_leg->cpn[i].capped? exo_leg->cpn[i].cap: NO_CAP);

		sprintf(
			evt[today_flag+first_ex_flag+i][1],
			"MAX(A[6,I],MIN(A[7,I],A[4,I]+A[5,I]*C[0,I]))*DF(D[I],A[3,I],\"%s\")",
			dom_name);		
	}
	...
	sprintf(
		tmpstr,
		"+MAX(A[14,0],MIN(A[15,0],A[12,0]+A[13,0]*C[0,I]))*DF(D[I],A[3,I],\"%s\")",
		dom_name);		
	...
	aux[0][15] = (exo_leg->not_ref.capped? exo_leg->not_ref.cap: NO_CAP);
	...
	if (nfund>0)
	{
		pvcf = fund_leg->notional * swp_f_df (today, fund_leg->cpn[0].start_date, fund_yc);
	}
	...
	if (ncall > 0)
	{
		...
		for (i=0; i<ncall; i++)
		{
			...
			/* Correction of
			if (j > 0 && call[i].ex_date - evt_dtes[j-1] < evt_dtes[j] - call[i].ex_date)
			{
				j--;
			}
			Pb when
			...
			*/
			if (j > 0 && fabs(call[i].ex_date - evt_dtes[j-1]) < fabs(evt_dtes[j] - call[i].ex_date))
			{
				if (((evt_dtes[j-1] == call[i].ex_date) && (call[i].ex_date < evt_dtes[j] - 60)) || (j>1) || (first_ex_flag ==1) || (today_flag == 0))
				{
					j--;
				}
			}
			...
			/*	The current coupon is called */
			{
				sprintf(
					evt[j][3],
					"(PV[1]+C[1,I]-A[11,I]*DF(D[I],A[10,I],\"%s\")",
					dom_name);
			}
		...
		aux[j][10] = (call[i].set_date >= evt_dtes[j]? call[i].set_date: evt_dtes[j]);
...
double						**aux)
{
	...
	if (ncall > 0 && call[0].ex_date > today + 15 && call[0].ex_date < exo_leg->cpn[0].fx_fix_date - 60)
	{
		first_ex_flag = 1;
	}
	else
	...
	*num_evt_dtes = nexo + (nexo - 1) * credit_freq  + today_flag + first_ex_flag;
	...
	evt_dtes[0] = today;
	if (first_ex_flag) evt_dtes[1 + credit_freq] = call[0].ex_date;
	for (i=0; i<nexo; i++)
	{
		evt_dtes[today_flag + first_ex_flag + i * (1 + credit_freq)] = exo_leg->cpn[i].fx_fix_date;
	}

	/* fill the intermediary dates */
	for (i=0; i<nexo-1; i++)
	{
		step = (long) ((evt_dtes[today_flag + first_ex_flag + (i + 1) * (1 + credit_freq)]
			- evt_dtes[today_flag + first_ex_flag + i * (1 + credit_freq)]) / (credit_freq + 1) + 0.5);

		for (j=1; j<=credit_freq; j++)
		{
			evt_dtes[today_flag + first_ex_flag + i * (1 + credit_freq) + j] = evt_dtes[today_flag + first_ex_flag + i * (1 + credit_freq) + j - 1] + step;
		}
	}
	...
	for (i=0; i<nexo; i++)
	{
		aux[today_flag + first_ex_flag + i * (1 + credit_freq)][2] = add_unit (evt_dtes[today_flag + first_ex_flag + i * (1 + credit_freq)], 2, SRT_BDAY, MODIFIED_SUCCEEDING);

		sprintf(
			evt[today_flag + first_ex_flag + i * (1 + credit_freq)][0],
			"S(\"%s\")*DF(D[I],A[2,I],\"%s\")/DF(D[I],A[2,I],\"%s\")",
			fx_name,
			for_name,
			dom_name);
	...
	{		
		for (j=1; j<=credit_freq; j++)
		{
			sprintf(evt[today_flag + first_ex_flag + i * (1 + credit_freq) + j][0], "pv[j]");
		}
	}
	...
	/*	Coupons */
	aux_rows[3] = aux_rows[4] = aux_rows[5] = aux_rows[6] = aux_rows[7] = *num_evt_dtes;

	for (i=0; i<nexo; i++)
	{
		aux[today_flag+first_ex_flag + i * (1 + credit_freq)][3] = exo_leg->cpn[i].pay_date;
		aux[today_flag+first_ex_flag + i * (1 + credit_freq)][4] = exo_leg->cpn[i].alpha;
		aux[today_flag+first_ex_flag + i * (1 + credit_freq)][5] = exo_leg->cpn[i].beta;
		aux[today_flag+first_ex_flag + i * (1 + credit_freq)][6] = (exo_leg->cpn[i].floored? exo_leg->cpn[i].floor: NO_FLOOR);
		aux[today_flag+first_ex_flag + i * (1 + credit_freq)][7] = (exo_leg->cpn[i].capped? exo_leg->cpn[i].cap: NO_CAP);

		sprintf(
			evt[today_flag+first_ex_flag + i * (1 + credit_freq)][1],
			"MAX(A[6,I],MIN(A[7,I],A[4,I]+A[5,I]*C[0,I]))*DF(D[I],A[3,I],\"%s\")",
			dom_name);		
	}
	...
	for (i=0; i<nexo; i++)
	{
		for (j=1; j<=credit_freq; j++)
		{
			...
			sprintf(evt[today_flag+first_ex_flag + i * (1 + credit_freq) + j][1], "pv[j]");
		}
	}
...
double		fx_vol_shift)
{
	...
	err = cpd_fill_und (fx3dund, &(und->und), NULL, dom_vol_shift, for_vol_shift, fx_vol_shift);
	if (err)
	{
		return err;
	}
...
double		fx_vol_shift)
{
	...
	basic_und->spot_fx = fx_spot * swp_f_df (today, fx_spot_date, dom_yc) / swp_f_df (today, fx_spot_date, for_yc);
	...
	{
		if (cpd->num_calls > 0 && !(cpd->num_calls == 1 && cpd->call[0].ex_date <= today + eod_flag))
		{
	...
	basic_und->sigma_date_rates = (double*) calloc (basic_und->sigma_n_rates, sizeof (double));
	...
	{
		basic_und->sigma_date_rates[i] = today + basic_und->sigma_time_rates[i] * DAYS_IN_YEAR + 1.0e-16;
	}
	...
	if (!fx_mkt_vol_time || !basic_und->sigma_date_fx || !basic_und->sigma_time_fx)
	{
		err = "Allocation error (2) in cpd_calib_smile_und";
		goto FREE_RETURN;
	}
	...
	for (i=0; i<num_fx_mkt_vol; i++)
	{
		...
		basic_und->sigma_date_fx[i] = add_unit (fx_mkt_vol_date[i], -2, SRT_BDAY, MODIFIED_SUCCEEDING);
		basic_und->sigma_time_fx[i] = (basic_und->sigma_date_fx[i] - today) * YEARS_IN_DAY;
	...
	err = Fx3DAlphaBetatsCalibration2_corr(
		...
		und->beta,
		und->fx2bdfwd,
		basic_und->corr_times,
		basic_und->correl_dom_for,
		basic_und->correl_dom_fx,
		basic_und->correl_for_fx,
		basic_und->corr_n_times,
		basic_und->dom_yc,
		basic_und->for_yc,
		&(basic_und->sigma_fx),
		50,
...
Err cpd_free_smile_und(
CPD_SMILE_UND und)
{
	return cpd_free_und (&und->und);
}
...
double		*call)
{
	...
	correl = dmatrix (0, 2, 0, und->und.corr_n_times -1);
	corr_date = dvector(0, und->und.corr_n_times -1);
	...
	for (i=0; i<und->und.corr_n_times; i++)
	{
		correl[0][i] = und->und.correl_dom_for[0];
		correl[1][i] = und->und.correl_dom_fx[0];
		correl[2][i] = und->und.correl_for_fx[0];
		corr_date[i] = und->und.today + DAYS_IN_YEAR * und->und.corr_times[i];
	}	
...
FREE_RETURN:
	...
	if (correl)
	{
		free_dmatrix (correl, 0, 2, und->und.corr_n_times -1);
	}

	if (corr_date)
	{
		free_dvector (corr_date, 0, und->und.corr_n_times -1);
	}
...
/* Function  call_GRFN_alphabetaModel 
	...
	Price the funding, power dual leg and the call of a PD in the alpha beta model by 
		initialising the dom, for, and fx underlying if necessary
		making the grfn tableau from the cpd_str structure of the power dual
		calling the alphabeta grfn tree to price 
	  
	Notice that
		the cpd_und underlying and cpd_str structure are filled by the cpd_fill_check_all_struct function
		
		In the cpd_und, the dom and for instantaneous ts are available.
		the fx ts corresponds either to the fx und passed to the function (which in that case is assumed to be a alpha beta calibrated fx ts)
			or the calibrated one by the cpd_fill_check_all_struct that is to say a Lognormal Model calibrated fx ts

		In the case where the Fx underlying needs to be calibrated with the alpha beta model, 
			the call_GRFN_alphabetaModel calculates the ts and filled the cpd_und with it.
								...
								double		*call_stdev				/*	Standard deviation of the call if applicable */)
{
	...
	DateSigmaTab = dmatrix(0,1, 0,max(cpd_und->sigma_n_fx,cpd_und->sigma_n_rates));
	...
	tableauStrings = smatrix_size (0, MAX_NEVT-1, 0, MAX_COL-1, GRFN_DEF_ARGBUFSZ);
	...
	if ((!DateSigmaTab) || (!und_names) || (!correl) || (!correl_dates) || (!auxLen) || (!evt_dtes) ||
		(!tableauStrings) || (!aux) || (!aux_transpose) || (!mask) || (!tab_barrier) || (!bar_col) || (!maturity_opt) ||
		(!fund_cfdtes) || (!fund_cf) || (!pd_cfdtes) || (!pd_cf) )
	{
	...
	{
		if (cpd_str->call[iNum].call_type>0)
		{
			/* KO Case */
	...
	if (!use_calib)
	{
		...
		/* It is supposed to be a alpha beta fxund with the same alpha and beta passed to the function */
	...
	else
	{
		...
		if (dom_calib)
		{
			...
			err = SrtInitIRUnd(dom_name,
								dom_yc,
								"LGM",
								cpd_und->sigma_n_rates,
								2,
								DateSigmaTab,
								1,
								1,
		...
		for (iNum=0; iNum<cpd_und->corr_n_times; iNum++)
		{
			...
			correl_dates[iNum] = cpd_und->today + DAYS_IN_YEAR * cpd_und->corr_times[iNum];
		}	

		err = SrtInitCorrelationMatrix(	cpd_und->corr_n_times,
										3,
										correl,
										correl_dates,
										und_names);
		...
		{
			maturity_opt[iNum] = (fx_mkt_vol_date[iNum] - cpd_und->today) * YEARS_IN_DAY;
		...
		err = Fx3DAlphaBetatsCalibration2_corr(	
							cpd_und->today,
							cpd_und->sigma_time_fx,
							maturity_opt,
							fx_mkt_vol,
							cpd_und->sigma_n_fx,
							num_cal,
							cpd_und->sigma_time_rates,
							cpd_und->sigma_n_rates,
							cpd_und->sigma_dom,
							cpd_und->lda_dom,
							cpd_und->sigma_for,
							...
							fx_spot,
							cpd_und->corr_times,
							cpd_und->correl_dom_for,
							cpd_und->correl_dom_fx,
							cpd_und->correl_for_fx,
							cpd_und->corr_n_times,
							cpd_und->dom_yc,
							cpd_und->for_yc,												
							&(cpd_und->sigma_fx),
							50,
	...
	while (iNum < fund_ncpn && fund_fix[iNum] < cpd_und->today + eod_fix_flag)
	{
		if (fund_pay[iNum] >= cpd_und->today + eod_pay_flag)
		{
			...
			fund_cf[fund_ncf] = (fund_fix_cpn[iNum] + fund_mrg[iNum])
				* coverage ((long) (fund_start[iNum] + 1.0e-08), (long) (fund_pay[iNum] + 1.0e-08), bas)
				* fund_not;
			fund_ncf++;
		}
		...
		if (pd_pay[iNum] >= cpd_und->today + eod_pay_flag)
		{
			...
			/* Old Method 
			  ...
			  pd_cf[pd_ncf] = pd_past_fix[i]
				* coverage ((long) (pd_start[i] + 1.0e-08), (long) (pd_pay[i] + 1.0e-08), bas)
				* pd_not; 
			*/
			...
			pd_cf[pd_ncf] = PdFixingRate
				* coverage ((long) (pd_start[iNum] + 1.0e-08), (long) (pd_pay[iNum] + 1.0e-08), bas)
				* pd_not; 
			/* */					
			pd_ncf++;
		}
...
FREE_RETURN:
	...
	if (DateSigmaTab)
	{
		free_dmatrix(DateSigmaTab,0,1, 0,max(cpd_und->sigma_n_fx,cpd_und->sigma_n_rates));
	}
	...
	if (tableauStrings)
	{
		free_smatrix_size (tableauStrings, 0, MAX_NEVT-1, 0, MAX_COL-1 , GRFN_DEF_ARGBUFSZ);
	}

...
double CMSAdjustementWithDelay(
    double         fact1,
    double         fact2,
    long           today,
    long           eventDate,
    ...
    char*          yc)
{
    ...
    compSwapValue =
        max(0.000001, SWAP_FOR_LGM2F_SwapRate_From_Factors_Value(fact1, fact2, &compSwap));

    fwdSwapValue =
        max(0.000001, SWAP_FOR_LGM2F_SwapRate_From_Factors_Value(fact1, fact2, &fwdSwap));
...
double CMSAdjustementWithoutDelay(
    double         fact1,
    double         fact2,
    long           today,
    long           eventDate,
    ...
    double*        VolBondCouponPrice)
{
    ...
    if (Test > 2)
    {
        err = Fill_SWAP_FOR_LGM2F(
            today,
            volBondCpn->second_fixingDate,
            volBondCpn->second_startDate,
            volBondCpn->first_SwapFreq,
            volBondCpn->first_SwapBasis,
    ...
    for (i = 0; i < NGaussQuadrature; ++i)
    {
        for (j = 0; j < NGaussQuadrature; ++j)
        {
            ...
            err = PayOff_VolBondCoupon(
                ...
                fact2,
                volBondCpn,
                exp(CMSAdj * (volBondCpn->second_fixingDate - volBondCpn->first_fixingDate) /
                    365.25),
                sabrAlpha,
                sabrBeta,
                sabrBetaVol,

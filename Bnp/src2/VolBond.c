...
double CMSAdjustementWithDelay(double fact1, double fact2, long today, long eventDate, 
					  swap_for_lgm2f compSwap, 
					  double comp_sabrBetaVol,
					  double comp_sabrBeta,
					  swap_for_lgm2f fwdSwap,
					  double fwd_sabrBetaVol,
					  double fwd_sabrBeta,
					  double swapCorrel,
					  TermStruct* l, char *yc)
{
	...
	compSwapValue = max(0.000001, SWAP_FOR_LGM2F_SwapRate_From_Factors_Value(fact1, fact2, &compSwap));

	fwdSwapValue = max(0.000001, SWAP_FOR_LGM2F_SwapRate_From_Factors_Value(fact1, fact2, &fwdSwap));

	temp = fwd_sabrBetaVol*pow(fwdSwapValue,fwd_sabrBeta-1);

	temp =	((fwdSwap.SwapStartDate - compSwap.SwapStartDate)/365.25) * comp_sabrBetaVol
				* pow(compSwapValue, comp_sabrBeta) * swapCorrel
				/ (1 + ((fwdSwap.SwapStartDate - compSwap.SwapStartDate)/365.25) * compSwapValue);

	temp = fwd_sabrBetaVol*pow(fwdSwapValue,fwd_sabrBeta-1)
					*( 
						fwdSwap.fix_cvgs[0] * fwdSwap.fix_NDates * fwdSwapValue
						* pow(1+fwdSwap.fix_cvgs[0]*fwdSwapValue,-fwdSwap.fix_NDates-1)
						/(1 - pow(1+fwdSwap.fix_cvgs[0]*fwdSwapValue,-fwdSwap.fix_NDates))
	...
	return fwd_sabrBetaVol*pow(fwdSwapValue,fwd_sabrBeta-1)
			* (
				((fwdSwap.SwapStartDate - compSwap.SwapStartDate)/365.25) * comp_sabrBetaVol
				* pow(compSwapValue, comp_sabrBeta) * swapCorrel
				/ (1 + ((fwdSwap.SwapStartDate - compSwap.SwapStartDate)/365.25) * compSwapValue)
...
double CMSAdjustementWithoutDelay(double fact1, double fact2, long today, long eventDate, 
					  swap_for_lgm2f fwdSwap,
					  double fwd_sabrBetaVol,
					  double fwd_sabrBeta,
					  TermStruct* l, char *yc)
			...
			volBondCoupon *volBondCpn, double *VolBondCouponPrice)
{
	...
	if(Test>2)
	{
		err = Fill_SWAP_FOR_LGM2F(today, volBondCpn->second_fixingDate, 
						volBondCpn->second_startDate,
						volBondCpn->first_SwapFreq, volBondCpn->first_SwapBasis,
						volBondCpn->first_SwapRefRate,
						&(swapForCMSAdj));
		if(err)
		{
			goto FREE_RETURN;
	...
	err = MY_LGM2F_Compute_Mean_Variance_Covariance(today, volBondCpn->first_fixingDate,
													volBondCpn->payment_Date,
													&(lgm2fForMeanVarCov),
													l, yc);
	if(err)
	{
	...
	{
		GQRho = lgm2fForMeanVarCov.covar/sqrt(lgm2fForMeanVarCov.var1*lgm2fForMeanVarCov.var2);
	}
	else
	{
		GQRho = 0.0;
	...
	err = SWAP_FOR_LGM2F_Preliminary_Computation(today,
										volBondCpn->first_fixingDate,
										&(volBondCpn->firstSwap), l, yc);
	if(err)
	{
		goto FREE_RETURN;
	}
...
//--------------------------------------------------------------------------
	...
	*VolBondCouponPrice = 0;
	for(i=0;i<NGaussQuadrature;++i)
	{
		...
		{
			fact1 = lgm2fForMeanVarCov.esp1 + sqrt(lgm2fForMeanVarCov.var1) * x[i+1];
			fact2 = lgm2fForMeanVarCov.esp2 + sqrt(lgm2fForMeanVarCov.var2) * (GQRho * x[i+1] + sqrt(1-GQRho*GQRho) * x[j+1] );

			if(Test>2)
			{
				CMSAdj = CMSAdjustementWithDelay(fact1, fact2, today, 
									volBondCpn->first_fixingDate, 
									swapForCMSAdj, 
									CompSabrBetaVol, CompSabrBeta,
									volBondCpn->secondSwap, 
									sabrBetaVol, sabrBeta, swapRho,
									l, yc);
			}
			else
			{
				CMSAdj = CMSAdjustementWithoutDelay(fact1, fact2, today, 
									volBondCpn->first_fixingDate, 
									volBondCpn->secondSwap, 
									sabrBetaVol, sabrBeta,
									l, yc);
			}

			err = PayOff_VolBondCoupon(today, fact1, fact2, volBondCpn, 
								exp(CMSAdj*(volBondCpn->second_fixingDate - volBondCpn->first_fixingDate ) / 365.25), 
								sabrAlpha, sabrBeta, 
								sabrBetaVol, sabrRho, &Payoff, l, yc);
			if(err)
			{
				goto FREE_RETURN;
			}
			...
			double *VolBondCpnPrices, double *VolBondPrice)
{
	...
	{
		err = Price_VolBondCoupon(today, yc, l, 
								 sabrBetaVol[i], sabrAlpha[i], sabrBeta[i], sabrRho[i], 
								 swapRho[i], CompSabrBeta[i], CompSabrBetaVol[i], 
								 NGaussQuadrature, x, w,
...
Err Price_VolBondAutoCal(long today,
			...
			char *refrate,
			double alpha, double gamma, double rho, //LGM2F parameters
			//	SABR Parameters	
			double		*sabrBetaVol,
			double		*sabrAlpha,
			double		*sabrBeta,
			...
			double *lambdaVec,
			double *sigmaVec,
			volBondStruct *volBond, double *cpnPrices, double *VolBondPrice)
{
	long i;
	double price;
	double temp;
	...
	for(i=0;i<volBond->NbOfCoupons;++i)
	{
		err = VolBondCalibrationOld(yc,
								 ...
								 (volBond->VolBondCpn[i]).first_SwapBasis,
								 alpha,
								 gamma,
								 rho,
								 &lambda,
		...
		err = srt_f_init_IRM_TwoFac_TermStruct(&ts,
												today,
												sig_data,
												sig_cols,
												num_sigs,
		...
		err = Price_VolBondCoupon(today, yc, ts, 
								 sabrBetaVol[i], sabrAlpha[i], sabrBeta[i], sabrRho[i], 
								 swapRho[i], CompSabrBeta[i], CompSabrBetaVol[i], 
								 NGaussQuadrature, x, w,
								 &(volBond->VolBondCpn[i]), &temp);
...
Err VolBond_Price(long today, 
				  char *yc, 
				  TermStruct* l,
				  //VolBond Parameters//
				  long NbCoupons, double *notional,
				  char **first_SwapFreq, char **first_SwapBasis, char **first_SwapRefRate,
				  char **second_SwapFreq, char **second_SwapBasis, char **second_SwapRefRate,
				  ...
				  double *priceValue)
{
	...
	err = Fill_VolBondStruct(today, NbCoupons, notional,
							 first_SwapFreq, first_SwapBasis, first_SwapRefRate,
							 second_SwapFreq, second_SwapBasis, second_SwapRefRate,
							 first_fixingDate, first_startDate, 
							 first_endDate,
							 second_fixingDate, second_startDate, 
							 second_endDate,
							 payment_Date, 
							 alphaC, alphaP,
							 betaC, betaP, strikeC, strikeP, &volBond);
	if(err)
	{
		goto FREE_RETURN;
	}

	err = Price_VolBond(today,yc, l, 
						sabrBetaVol, sabrAlpha, sabrBeta, sabrRho,
						swapRho, CompSabrBeta, CompSabrBetaVol,
						NGaussQuadrature,
...
Err VolBond_PriceAutoCal(long today, 
				  char *yc, char *vc, char *refrate,
				  double defaultLambda,
				  ...
				  double *priceValue)
{
	...
	err = Price_VolBondAutoCal(today, yc, vc, defaultLambda, refrate, lgm2f_alpha, lgm2f_gamma, lgm2f_rho,
						sabrBetaVol, sabrAlpha, sabrBeta, sabrRho,
						swapRho, CompSabrBeta, CompSabrBetaVol,
						NGaussQuadrature,
						lambdaVec, sigmaVec,

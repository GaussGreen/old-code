    ...
    unsigned long Max_Steps)  // The maximum number of iterations allowed in the minimization loop
{
    ...
    // Compute the historical linear correlation (set the last two arguments to 2 to calculate the
    // correlation of the data series), the historical rank correlation and the the maximum
    // likelihood for the first 10 degrees.
    *linear_corr             = srt_f_corr(changes_1, changes_2, n - 1, 2, 2);
    double        rank_corr = spear_rho(changes_1, changes_2, n - 1);
    unsigned long MLE =
        Student_degree_MLE(*linear_corr, changes_1, changes_2, n - 1, Max_Stud_Degree);

    // Calculate the linear correlation compatible with the MLE value of m and the historical rank
    // correlation. Use it to re-estimate the MLE.  Continue iterating until the MLE value for m
    // stops changing or we have looped too many times
    ...
    do
    {
        ...
        *linear_corr = RankToLinCorr(rank_corr, MLE, RankToLin_acc);
        new_MLE = Student_degree_MLE(*linear_corr, changes_1, changes_2, n - 1, Max_Stud_Degree);
...
// any real number) 		   m		degree of the distribution (0 is Gaussian, otherwise can
// be any
// positive integer)
...
double log_Student_copula_dist(double x1, double x2, unsigned long m, double rho)
{
    if (m == 0)
        ...
        return gammln(0.5 * (m + 2.0)) + gammln(0.5 * m) - 2.0 * gammln(0.5 * (m + 1.0)) -
               0.5 * log(1 - rho * rho) -
               0.5 * (m + 1.0) *
                   log(1 + (x1 * x1 - 2.0 * rho * x1 * x2 + x2 * x2) / (1 - rho * rho) / m) +

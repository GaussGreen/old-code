...
//
/**********************************************************************************************************************************/
void Stud_Deg_Fit(			 double *data1,						// input historical data series
							 ...
							 unsigned long RankToLin_acc, 		// Accuracy parameter for calculating the linear correlation from rank 
...
// Compute the historical linear correlation (set the last two arguments to 2 to calculate the correlation of the data series),
// the historical rank correlation and the the maximum likelihood for the first 10 degrees.
	linear_corr = srt_f_corr( changes_1, changes_2, n-1, 2, 2 );
	double rank_corr = spear_rho( changes_1, changes_2, n-1);
	unsigned long MLE = Student_degree_MLE( linear_corr, changes_1, changes_2, n-1, Max_Stud_Degree );

// Calculate the linear correlation compatible with the MLE value of m and the historical rank correlation.  
// Use it to re-estimate the MLE.  Continue iterating until the MLE value for m stops changing or we have looped too many times
...
// For a given value of the `linear correlation' parameter, calculate the value of m between 0 (Gaussian) and Max_Stud_Degree
// that maximizes the log-likelihood function
unsigned long Student_degree_MLE( double linear_corr, double *changes_1, double *changes_2, unsigned long len, 
								  unsigned long Max_Stud_Degree )
{
	double max_likelihood = log_Stud_ML( 0, linear_corr, changes_1, changes_2, len ); 
	unsigned long MLE = 0;
...
double log_Stud_ML( unsigned long degree, double linear_corr, double *x1, double *x2, unsigned long NumData )
{
	...
	for ( unsigned long i=0; i<NumData; i++ )
		sum += log_Student_copula_dist( x1[i], x2[i], degree, linear_corr ) + log_Student_dist( x1[i], degree ) 
					+ log_Student_dist( x2[i], degree );
	return sum;
}
...
//
// Inputs: x1		value at which the probability density is to be evaluated (can be any real number)
//		   x2		value at which the probability density is to be evaluated (can be any real number)
...
double log_Student_copula_dist( double x1, double x2, unsigned long m, double rho )
{
	if ( m==0 )
		...
		return gammln( 0.5*(m+2.0) ) + gammln( 0.5*m ) - 2.0* gammln( 0.5*(m+1.0) ) - 0.5*log( 1 - rho*rho ) - 
			0.5*(m+1.0)*log( 1+(x1*x1-2.0*rho*x1*x2+x2*x2)/(1-rho*rho)/m ) + 0.5*(m+1.0)*log( (1+x1*x1/m)*(1+x1*x1/m) );
}

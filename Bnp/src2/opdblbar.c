...
/*******************************************************************************
        ...
        and maturity, for either risk neutral (measure_flag = 0) or measure
        associated with the underlying (measure_flag = 1) .
...
static double dbarrier_dens(
    double bound, double x1_n, double x2_n, double mu, double vol, double mat)
{
...
/* ==========================================================================
   ...
   normal Brownian motion:
                ...
                if (call_put == SRT_PUT):
                                Prob[ X(T) < k; min X(t) > b_do ; max X(t) < b_up ]
                                                                        ==
                                Prob[ -X(T) > -k; max -X(t) < -b_do ; min -X(t) > -b_up ]
   ========================================================================== */
    ...
    SrtGreekType   greek)
{
    ...
    else /* maturity = 0 case */
    {
        ...
        case DELTA:
        case DELTA_FWD:
            /* the delta when spot = strike is assumed to be 0, instead of infinity */
            if (premium == 0)
                answer = 0.0;
            else
                answer = 1.0;
        ...
        case GAMMA:
            /* again, ignore discontinuous delta at spot = strike and assume gamma is
               0 throughout */
        case VEGA:
        case THETA:
            answer = 0.0;

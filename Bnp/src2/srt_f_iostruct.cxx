...
Err srt_f_IOencodename(char id_name[32], int type, int bucket_no,
  ...
  sprintf(name, "%s-%.1d-%.5lf-%d-%s",
          name_type, /* PREMIUM        , TAU        , ... */
          bucket_no, shift_value, shift_type, id_name);
...
Err srt_f_IOvalshow(SrtIOVal *ioval, FILE *file_out) {
  ...
  fprintf(file_out, "Function use for computation : %s\n",
          ((strcmp(ioval->computation_origin, "") == 0)
               ? "None"
               : ioval->computation_origin));
...
Err srt_f_IOstructset(SrtIOStruct *l, char id_name[32], int type, int bucket_no,
                      ...
                      long lval, double dval, void *pval) {
  char name[32];
  SrtIOVal *ioval;
  Err err = 0;
  long ticker;
  ...
  /* Builds the name of the object to put into the IO list (using the
   * ioval->type) */
  srt_f_IOencodename(id_name, ioval->type, bucket_no, ioval->shift_value,
                     ioval->shift_type, name);

  /* Inserts the object (with its name) in the IO list */
  srt_f_lstins(l, name, ioval->shift_value, OBJ_PTR_IO, (void *)ioval,
               &srt_f_IOvalfree, &ticker);
...
Err srt_f_IOstructsetpremium(SrtIOStruct *l, SRT_Boolean done, double result,
  ...
  err = srt_f_IOstructset(l, "", IO_PREMIUM, 0, 0, 0, 0, 0, done, comp_origin,
                          0, result, NULL);
  return err;
}
...
Err srt_f_IOstructsetstdev(SrtIOStruct *l, SRT_Boolean done, double result,
  ...
  err = srt_f_IOstructset(l, "", IO_STDEV, 0, 0, 0, 0, 0, done, comp_origin, 0,
                          result, NULL);
  return err;
}
...
Err srt_f_IOstructsetcolpvs(SrtIOStruct *l, SRT_Boolean done, double *pv_cols,
                            long num_pv, char comp_origin[32]) {
  Err err = 0;
  double *copy;
  ...
  /* Here        , uses the lval field to store the dimension of the tableau to
   * store
   */
  err = srt_f_IOstructset(l, "", IO_COLPVS, 0, 0, 0, 0, 0, done, comp_origin,
                          num_pv, 0, (void *)copy);
...
Err srt_f_IOstructgetrshift(SrtIOStruct l, char id_name[32], double shift_value,
  ...
  if (err = srt_f_IOstructget(l, "", IO_RATE_SHIFT, 0, shift_value, shift_type,
                              ioval_pp)) {
    return err;
  }
...
Err srt_f_IOstructgetrshiftval(SrtIOStruct l, char id_name[32],
  ...
  if (err = srt_f_IOstructgetrshift(l, id_name, shift_value, shift_type,
                                    &ioval_p)) {
    return err;
  }
...
Err srt_f_IOstructsetrshift(SrtIOStruct *l, char id_name[32],
  ...
  if (err = srt_f_IOstructset(l, id_name, IO_RATE_SHIFT, 0, 0, 0, shift_value,
                              shift_type, done, comp_origin, 0, value, ptr)) {
    return err;
  }
...
Err srt_f_IOstructgetsigmashift(SrtIOStruct l, int bucket_no,
  ...
  if (err = srt_f_IOstructget(l, "", IO_SIGMA_SHIFT, bucket_no, shift_value,
                              shift_type, ioval_pp)) {
    return err;
  }
...
Err srt_f_IOstructgetsigmashiftval(SrtIOStruct l, int bucket_no,
  ...
  if (err = srt_f_IOstructgetsigmashift(l, bucket_no, shift_value, shift_type,
                                        &ioval_p)) {
    return err;
  }
...
Err srt_f_IOstructsetsigmashift(SrtIOStruct *l, int bucket_no,
  ...
  if (err = srt_f_IOstructset(l, "", IO_SIGMA_SHIFT, bucket_no, bucket_start,
                              bucket_end, shift_value, shift_type, done,
                              comp_origin, 0, value, NULL)) {
    return err;
  }
...
Err srt_f_IOstructgettaushift(SrtIOStruct l, int bucket_no, double shift_value,
  ...
  if (err = srt_f_IOstructget(l, "", IO_TAU_SHIFT, bucket_no, shift_value,
                              shift_type, ioval_pp)) {
    return err;
  }
...
Err srt_f_IOstructgettaushiftval(SrtIOStruct l, int bucket_no,
  ...
  err = srt_f_IOstructgettaushift(l, bucket_no, shift_value, shift_type,
                                  &ioval_p);
  if (err)
    *value = 0.0;
  else
...
Err srt_f_IOstructsettaushift(SrtIOStruct *l, int bucket_no, Date bucket_start,
                              ...
                              char comp_origin[32]) {
  Err err = 0;
  err = srt_f_IOstructset(l, "", IO_TAU_SHIFT, bucket_no, bucket_start,
                          bucket_end, shift_value, shift_type, done,
                          comp_origin, 0, value, NULL);
  return err;
}

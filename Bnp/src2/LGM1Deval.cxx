...
static LGMErr
GenEurpayoff(double **payoff, long nx, double *x, double *reduction,
             LGMDealType dealtype, void *dealPtr, LGMCalParm *CalReq, Date tNow,
...
static LGMErr
MidAtpayoff(double **payoff, long nx, double *x, double *reduction,
            LGMDealType dealtype, void *dealPtr, LGMCalParm *CalReq, Date tNow,
            String ycName, LGM_TS *tsPtr,
            LGMErr (*GetVol)(Date, Date, double, SRT_Boolean,
                             double *), /* swaption/cap vols */
...
static LGMErr
GenMidAtpayoff(double **payoff, long nx, double *x, double *reduction,
...
static LGMErr BerInvFltpayoff(
    double **payoff, long nx, double *x, double *reduction,
    LGMDealType dealtype, void *dealPtr, LGMCalParm *CalReq, Date tNow,
    String ycName, LGM_TS *tsPtr,
    LGMErr (*GetVol)(Date, Date, double, SRT_Boolean,
                     double *), /* swaption/cap vols */
...
LGMErr NewMidAtEval(
    ...
    int endofday, /* 1 = cannot exercise today        , 0 = can exercise today
                   */
    String ycName, LGMCalParm *CalReq,
    ...
    double **xExBdry) /* array of exercise points (in x) */
{
  ...
  double **Zeta1Scenari, **Zeta2Scenari, **Zeta12Scenari, **H1Scenari,
      **H2Scenari;
  ...
  if (endofday == 1)
    tfirst =
        add_unit(tNow, 1, SRT_BDAY,
                 SUCCEEDING /*        , conv.cxxcy*/); /* tfirst is the first
                                                  possible exercise */
  ...
  case SimpleMidAt:
    ...
    if (CalReq->LGMOneTwoFactor == 2) {
      ...
      error = LGMAutoCal2DTree(nEx, tExArr, Zeta1, Zeta12, Zeta2, H1, H2, gamma,
                               tNow, ycName, GetVol, GetBeta, (void *)dealPtr,
                               Midat2DPayoff, &LGMval, xExBdry);

      lEndTime = time(NULL);
      smessage("Midatlantic Pricing achieved in %.2f s",
               difftime(lEndTime, lStartTime));

      LGMvalVega = (double *)srt_calloc(2 * tsPtr->numZ, sizeof(double));

      /*
      error = LGMAutoCal2DVega(2*tsPtr->numZ        , nEx        , tExArr ,
      Zeta1Scenari        , Zeta12Scenari        , Zeta2Scenari        ,
      H1Scenari        , H2Scenari        , gamma        , tNow , ycName ,
      GetVol        , GetBeta        , (void*) dealPtr        , Midat2DPayoff ,
      LGMvalVega        , xExBdry);
      */
      srt_free(LGMvalVega);
      ...
      *LGMvalue = LGMval;
      ...
      error = Convolver(nEx, zeta, parms, tNow, ycName, GetVol, GetBeta, tsPtr,
                        dealtype, (void *)dealPtr, MidAtpayoff, CalReq, &LGMval,
                        xExBdry);
  ...
  case GenMidAt:
    ...
    GenExOnce->FirstExer =
        j; /* Ensure that evaluation begins at right exercise */
    ...
    /* evaluate deal */
    error = Convolver(nEx, zeta, parms, tNow, ycName, GetVol, GetBeta, tsPtr,
                      dealtype, (void *)dealPtr, GenMidAtpayoff, CalReq,
                      &LGMval, xExBdry);
    *LGMvalue = LGMval;
    break;
  ...
  case CallInvFlt:
    ...
    /* evaluate deal */
    error = Convolver(nEx, zeta, parms, tNow, ycName, GetVol, GetBeta, tsPtr,
                      dealtype, (void *)dealPtr, BerInvFltpayoff, CalReq,
                      &LGMval, xExBdry);
    *LGMvalue = LGMval;
    break;
  ...
  case CallCapFlt:
    ...
    /* evaluate deal */
    error = Convolver(nEx, zeta, parms, tNow, ycName, GetVol, GetBeta, tsPtr,
                      dealtype, (void *)dealPtr, BerCapFltpayoff, CalReq,
                      &LGMval, xExBdry);
    *LGMvalue = LGMval;
    break;
  ...
  case CallTimeSwap:
    ...
    /* evaluate deal */
    /*		error = ConvolverTimeSwap(nEx        , zeta        , parms        ,
       tNow        , ycName        , GetVol        , GetBeta        , tsPtr ,
       dealtype        , (void*)dealPtr        , BerTimeSwapPayoff        ,
       CalReq        , &LGMval        , xExBdry);	*/
    error = ComputeBermudanAndEuropeanTimeSwap(
        nEx, zeta, parms, tNow, ycName, GetVol, GetBeta, tsPtr, dealtype,
        (void *)dealPtr, BerTimeSwapPayoff, CalReq, &LGMval, xExBdry);
    *LGMvalue = LGMval;
    break;
...
/* Outputs:
        *answer = value of the deal        ,
        xExBdry[0        ,1        ,...        ,nEx-1] are the x values of the
   exercise boundary at each exercise if multiple exercise points        , gives
...
/* Convolver requires three sets of arguments
        ...
        2a) information about the market (including today        , a yield curve
, and a calibrated LGM term structure 2b) information about caplet/swaption vols
(for deals whose payoffs include options) 3) information about the deal        ,
and a function to calculate the payoff for all the exercise dates The arguments
of the payoff function are:
        (*payofffunc)(double payoff[][]        , long nx        , double x[] ,
double reduction[] LGMDealType dealtype        , void *dealPtr        , Date
EvalDate        , String ycName        , LGM_TS tsPtr)
 */
...
startagain:
  ...
  */
  ...
  if (x == NULL || Qarr == NULL || Qplus == NULL || differ == NULL ||
  ...
  error = (*payofffunc)(payofftable, nx, x, reduction, /* output and grid */
                        dealtype, dealPtr, CalReq,     /* deal */
                        EvalDate, ycName, tsPtr, GetVol,
    ...
    LGMErr (*GetBeta)(Date, Date, double *)) /* swaption/cap exponents (beta) */
{
  ...
  for (i = 0; i < nPay; i++) /* For each payment received on this exercise */
  {
    ...
    for (k = 0; k <= nx; k++) /* for each gridpoint */
      payoff[0][k] =
          payoff[0][k] + aD * exp(Gpay * x[k] * reduction[0]); /* add payment */
    ...
    LGMErr (*GetBeta)(Date, Date, double *)) /* swaption/cap exponents (beta) */
{
  ...
  nEx = MidAt->nEx -
        j0; /* Number of exercises (excluding ones before today+eod) */
  ...
  zetaex = (double *)srt_calloc(
      nEx, sizeof(double)); /* zeta(tEx_j)        , j=0        ,... ,nEx-1 */
  StrikeD = (double *)srt_calloc(
      nEx,
      sizeof(
          double)); /* Strike[j]*D(tset_j)        , j=0        ,... ,nEx-1 */
  aredD = (double *)srt_calloc(
      nEx,
      sizeof(
          double)); /* Ared[j]*D(tfirst_j)        , j=0        ,... ,nEx-1 */
  Gset = (double *)srt_calloc(
      nEx,
      sizeof(double)); /* G(tset_j)        , j=0        ,...        ,nEx-1 */
  Gpay = (double *)srt_calloc(
      nPay, sizeof(double)); /* G(tPay_i)        , i=0        ,... ,nPay-1 */
  aD = (double *)srt_calloc(
      nPay, sizeof(
                double)); /* a[i]D(tPay_i)        , i=0        ,... ,nPay-1 */
  prefac =
      dmatrix(0, nx, 0, nPay - 1); /* exp(Gpay[i]*x[k])        , k=0        ,..
                                      ,nx        , i=0        ,... ,nPay-1 */
  postfac = dmatrix(
      0, nEx - 1, 0, nPay - 1); /* exp(-.5*zetaex[j]*Gpay[i]^2)        , j=0
                       ,...        ,nEx-1        , i=0        ,... ,nPay-1 */

  if (zetaex == NULL || StrikeD == NULL || aredD == NULL || Gset == NULL ||
      Gpay == NULL || aD == NULL || prefac == NULL || postfac == NULL) {
    srt_free(zetaex);
    srt_free(StrikeD);
    srt_free(aredD);
    srt_free(Gset);
  ...
  for (i = 0; i < nPay; i++) {
    ipay = i0 + i; /* Note: payments are offset by -i0 from payments in MidAt
                      structure*/
  ...
  /* compute zeta at exercise dates        , G and StrikeD at settlement dates
   * , and aredD at first paydates */
  for (j = 0; j < nEx; j++) {
    jex = j0 +
          j; /* Note: exer's are offset by -j0 from exer's in MidAt structure*/
    tEx = MidAt->tEx[jex];
  ...
  for (j = 0; j < nEx; j++) {
    ...
    if (j == 0 ||
        reduction[j] >
            1.1 * reduction[j - 1]) /* recompute prefactors if needed */
    {
    ...
    for (k = 0; k <= nx; k++) {
      ...
      payoff[j][k] =
          -StrikeD[j] * exp(Gset[j] * (reduction[j] * x[k] -
                                       0.5 * Gset[j] * zetaex[j])); /* Strike */
      payoff[j][k] = payoff[j][k] + (aD[i] - aredD[j]) * prefac[k][i] *
                                        postfac[j][i]; /* first payment */
      i++;
      for (; i < nPay; i++)
        payoff[j][k] = payoff[j][k] + aD[i] * prefac[k][i] *
                                          postfac[j][i]; /* other payments */
    }
  }
    ...
    LGMErr (*GetBeta)(Date, Date, double *)) /* swaption/cap exponents (beta) */
{
  ...
  for (j = 0; j < nEx; j++) /* For each exercise */
  {
    ...
    for (i = 0; i < nPay; i++) /* For each payment received on this exercise */
    {
      ...
      for (k = 0; k <= nx; k++) /* for each gridpoint */
        payoff[j][k] = payoff[j][k] +
                       aD * exp(Gpay * x[k] * reduction[j]); /* add payment */
...
static LGMErr BerInvFltpayoff(
  ...
  if (zEx == NULL || dSet == NULL || Gset == NULL || tFix == NULL ||
      dStart == NULL || dPay == NULL || Gstart == NULL || Gpay == NULL ||
      payCvg == NULL || refCvg == NULL) {
  ...
  for (j = 0; j < nEx; j++) {
    ...
    for (i = ifirst; i < nCpn; i++) {
      ...
      YrtoExpFromtEx =
          DMAX(1.0e-04, (tFix[i] - ptr->tEx[j + j0]) * YEARS_IN_DAY);
      ...
      /* If caplet expiry is less than 25 days        , we price it with a vol
       * of 0 */
      /*	Also        , if caplet strike is irrelevant        , we price
       * it with a vol of 0 */
      if (tFix[i] - ptr->tEx[j + j0] > 25 && caplet_strike < 1 &&
          caplet_strike > 1.0e-08) {
        /* Forward vol */
        switch (CalReq->CapletVolMeth) {
        case MODEL:

          caplet_vol =
              (Gstart[i] - Gpay[i]) * sqrt((zFix[i] - zEx[j]) / YrtoExpFromtEx);
          break;
        ...
        case SLIDING2:
          ...
          caplet_vol = GetNormalVol(
              (tFix[i] - ptr->tEx[j + j0]) * YEARS_IN_DAY, tStart, tEnd,
              (swp_f_df(tNow, tStart, ycName) / swp_f_df(tNow, tEnd, ycName) -
               1.0) /
                  refCvg[i],
              caplet_strike, SRT_TRUE, GetVol, GetBeta);
          break;

        case CONVERGING:

          caplet_vol = GetNormalVol(
              (tFix[i] - tNow) * YEARS_IN_DAY, ptr->tCpnStart[i + i0],
              ptr->tCpnPay[i + i0], (dStart[i] / dPay[i] - 1.0) / refCvg[i],
              caplet_strike, SRT_TRUE, GetVol, GetBeta);
          break;
        ...
        case MARKET:
          ...
          total_vol = GetNormalVol((tFix[i] - tNow) * YEARS_IN_DAY,
                                   ptr->tCpnStart[i + i0], ptr->tCpnPay[i + i0],
                                   (dStart[i] / dPay[i] - 1.0) / refCvg[i],
                                   caplet_strike, SRT_TRUE, GetVol, GetBeta);

          /* Get today's PV */
          caplet_pv_today = srt_f_optblknrm(
              (dStart[i] / dPay[i] - 1.0) / refCvg[i], caplet_strike, total_vol,
              YrtoExpFromtNow, dPay[i] * refCvg[i], SRT_CALL, SRT_PREMIUM);

          /* Get the total lognormal vol of 1 + cvg * Libor */
          error =
              srt_f_optimpvol(caplet_pv_today, dStart[i] / dPay[i],
                              1.0 + refCvg[i] * caplet_strike, YrtoExpFromtNow,
                              dPay[i], SRT_CALL, SRT_LOGNORMAL, &total_vol);
          if (error)
            return error;
      ...
      if (switch_fwd == 1) {
        ptr->fwdVolMat[i + 1][j + 1] =
            caplet_vol * dStart[i] / dPay[i] / refCvg[i];
      } else {
      ...
      for (k = 0; k <= nx; k++) {
        ...
        /* Caplet */
        if (switch_fwd == 1) {
          if (1.0 + refCvg[i] * fwd_libor > 0 &&
              1.0 + refCvg[i] * caplet_strike > 0) {
            caplet_pv = srt_f_optblksch(
                1.0 + refCvg[i] * fwd_libor, 1.0 + refCvg[i] * caplet_strike,
                caplet_vol, YrtoExpFromtEx, 1.0, SRT_CALL, SRT_PREMIUM);
          } else {
          ...
          if (fwd_libor > 0 && caplet_strike > 0) {
            caplet_pv =
                srt_f_optblknrm(fwd_libor, caplet_strike, caplet_vol,
                                YrtoExpFromtEx, 1.0, SRT_CALL, SRT_PREMIUM);
          } else {
        ...
        discount =
            dPay[i] *
            exp(Gpay[i] * (x[k] * reduc[j + j0] - 0.5 * Gpay[i] * zEx[j]));
    ...
    LGMErr (*GetBeta)(Date, Date, double *)) /* swaption/cap exponents (beta) */
{
  ...
  long j0, i0, nEx, nCpn, i, j, k;
  double *zEx =
      NULL; /* zeta(tEx[j])        , j = j0        , ...        , nEx-1 */
  double *dPay =
      NULL; /* d(tCpn[i])        , i = i0        ,...        , nCpn */
  double *beta =
      NULL; /* beta for cpn i        , i = i0+1        ,...        , nCpn */
  double *cvgCpn =
      NULL; /* cvg for cpn i        , i = i0+1        ,...        , nCpn */
  double *cvgR = NULL; /* flting rate cvg for in cpn i        , i = i0+1 ,...
                          , nCpn */
  double *Gpay =
      NULL; /* G(tCpn[i])        , i = i0        , ...        , nCpn */
  Date *tFix =
      NULL; /* fixing date tFix[i]        , i = i0+1        ,...        ,nCpn */
  double **postf = NULL; /* exp(-0.5*Gpay[i]*Gpay[i]*zEx[j])        , [j][i] ,
                            i=iSet[j]        ,...        ,nCpn */
  double **pref =
      NULL; /* D[i]*exp(G[i]*x[k])        , [i][k]        , i = iSet[j] ,...
                     ,nCpn        , k=0        ,...        , nx */
  ...
  dPay = (double *)srt_calloc(
      nCpn + 1,
      sizeof(double)); /* dPay[i0+i]        , i = 0        ,...        , nCpn */
  beta = (double *)srt_calloc(nCpn + 1,
                              sizeof(double)); /* beta[i0+i]        , i = * ,1
                                                  ,...        , nCpn */
  cvgCpn = (double *)srt_calloc(
      nCpn + 1, sizeof(double)); /* cvg[i0+i]        , i = *        ,1 ,...
                                    , nCpn */
  cvgR = (double *)srt_calloc(nCpn + 1,
                              sizeof(double)); /* cvg[i0+i]        , i = * ,1
                                                  ,...        , nCpn */
  Gpay = (double *)srt_calloc(
      nCpn + 1,
      sizeof(double)); /* Gpay[i0+i]        , i = 0        ,...        , nCpn */
  tFix = (Date *)srt_calloc(nCpn + 1,
                            sizeof(Date)); /* tFix[i0+i]        , i = * ,1
                                              ,...        , nCpn */

  postf =
      dmatrix(0, nEx - 1, 0, nCpn); /* [j][i] = [0        ,1        ,...
                                       ,nEx-1][0        ,...        ,nCpn] */
  pref = dmatrix(0, nCpn, 0, nx);   /* [i][k] = [0        ,1        ,...
                                       ,nCpn][0        ,...        ,nx] */

  if (zEx == NULL || dPay == NULL || beta == NULL || cvgCpn == NULL ||
      cvgR == NULL || Gpay == NULL || tFix == NULL || postf == NULL ||
      pref == NULL) {
  ...
  for (j = 0; j < nEx; j++) {
    for (k = 0; k <= nx; k++)
      payoff[j][k] =
          0.0; /*PAYOFF VALUE AT THE J THE EXERCISE DATE AND NODE K */
  }            /*END OF PAYOFF INITIALISATION */

  for (j = 0; j < nEx; j++) {
    if (j == 0 ||
        reduc[j] > SRT_MIN_REDUC_FACT * reduc[j - 1]) /* X IS RE-SCALED */
    {
    ...
    for (i = ifirst + 1; i <= nCpn; i++) {
      ...
      for (k = 0; k <= nx; k++) {
        rate = (pref[i - 1][k] * postf[j][i - 1] / (pref[i][k] * postf[j][i]) -
                1.0) /
               cvgR[i];
        ...
        CAPLETMINPV =
            srt_f_optblksch((1 + cvgCpn[i] * rate), (1 + cvgCpn[i] * amini),
                            ATMVOL, CAPLETMAT, 1.0, /*DISCOUNT FACTOR */
                            SRT_CALL, SRT_PREMIUM);

        CAPLETMAXPV =
            srt_f_optblksch((1 + cvgCpn[i] * rate), (1 + cvgCpn[i] * amaxi),
                            ATMVOL, CAPLETMAT, 1.0, /*DISCOUNT FACTOR */
                            SRT_CALL, SRT_PREMIUM);

        cpn =
            cvgCpn[i] * ptr->marg[i + i0] + gear * (CAPLETMINPV - CAPLETMAXPV);
    ...
    for (k = 0; k <= nx; k++) {
      ...
      payoff[j][k] = payoff[j][k] -
                     ptr->strike[j + j0] * pref[ifirst][k] * postf[j][ifirst];
    }
  }

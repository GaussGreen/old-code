...
/* ==========================================================================
   ...
   PURPOSE:		Modelling of the spot FX vol by taking in consideration:
                                ...
                                - A term structure of correlations (4x4 correlation tensor)

                                The pricing is done through a trinomial tree.
    ...
    double*   eigen_val)
{
    ...
    for (step = nstp - 1; step >= 1; step--)
    {
        ...
        for (i = 0; i < 3; i++)
        {
            ...
            if (nb_nodes_tab[step][i] > MAXNODE3FQUANTO)
            {
                err = serror(
                    "MAXNODE3FQUANTO exceeded in tree_main_3fquanto: %d > %d !!!",
                    nb_nodes_tab[step][i],
                    MAXNODE3FQUANTO);
                goto FREE_RETURN_BOUNDARIES;
            }
...
/*--------------------------------------------------------------------------------------------------------*/
/*                               Main function for 3F Quanto Tree */
/*--------------------------------------------------------------------------------------------------------*/
    ...
    double* res)
{
    ...
    if (!lgm2F_ifr || !lgm1F_ifr || !transf_matrix || !transf_matrix_next || !transf_matrix_inv ||
        !transf_matrix_inv_next || !maxmin_dual_tab || !sigma || !sigma_next || !nb_nodes_tab ||
        !nb_nodes_max || !correl || !covar_matrix || !eigen_vector || !eigen_vector_transp ||
        !eigen_val || !maxmin_orig || !expect_node || !expect_dual_node || !proba || !coos_dual ||
        !temp_vec1 || !temp_vec2 || !temp_mat1 || !temp_mat2 || !temp_mat3)
    {
        err = "Memory allocation error (1) in tree_main_3fquanto";
        goto FREE_RETURN;
    }
    ...
    for (i = 0; i < nstp; i++)
    {
        // The important correlations for the boundaries are the correlations between r1, r2, r3
        // {0: r1, 1: r2, 2: r3}
        correl[i][0] = corr_r1_r2[i];
        correl[i][1] = corr_r1_r3[i];
        correl[i][2] = corr_r2_r3[i];
    }
    ...
    {
        err = "Memory allocation (3) error in tree_main_3fquanto";
        goto FREE_RETURN;
    }
    ...
    sigma_next[0] = r1_sig[nstp - 1];
    sigma_next[1] = r2_sig[nstp - 1];
    sigma_next[2] = r3_sig[nstp - 1];
    vol_fx        = fx_sig[nstp - 1];
    ...
    if (!eval_evt[nstp - 1])
    {
        err = "No event at last step in tree_main_3fquanto";
        goto FREE_RETURN;
    }
    ...
    for (step = nstp - 2; step >= 1; step--)
    {
        ...
        /*	Precalculate quantities used to compute original state variables and dual
           expectations E[R_t+dt_in_base_t+dt] = transf_matrix_inv_t+dt * A
                                                           + transf_matrix_inv_t+dt * B *
           transf_matrix_t * R_t_in_base_t*/

        // A = vector (a1, a2, a3) and calculate transf_matrix_inv_t+dt * A

        a1 = r1_var[step] + corr_r1_r2[step] * r1_r2_covar[step];
        a2 = r2_var[step] + corr_r1_r2[step] * r1_r2_covar[step];
        a3 = r3_var[step];

        if (lgm2F_is_dom_for == 1)
        {
            a1 -= corr_r1_fx[step + 1] * vol_fx * sigma_next[0];
            a2 -= corr_r2_fx[step + 1] * vol_fx * sigma_next[1];
        }
        else
        {
            a3 -= corr_r3_fx[step + 1] * vol_fx * sigma_next[2];
        }
        ...
        temp_vec1[1] = a2;
        temp_vec1[2] = a3;
        prod_matrix_vector(transf_matrix_inv_next, temp_vec1, temp_vec2, i, j, gsum);

        // B = diag(b1, b2, b3) and calculate transf_matrix_inv_t+dt * B * transf_matrix_t

        b1 = 1.0 - r1_lam * delta_t;
        b2 = 1.0 - r2_lam * delta_t;
        b3 = 1.0 - r3_lam * delta_t;

        for (l = 0; l < 3; l++)
        {
            temp_mat1[0][l] = b1 * transf_matrix[0][l];
            temp_mat1[1][l] = b2 * transf_matrix[1][l];
            temp_mat1[2][l] = b3 * transf_matrix[2][l];
        }
        prod_matrix(transf_matrix_inv_next, temp_mat1, temp_mat2, i, j, k, gsum);

        // Calculations finished and results in temp_vec2 and temp_mat2
        ...
        for (i = 0; i < n0; i++)
        {
            ...
            for (j = 0; j < n1; j++)
            {
                ...
                for (k = 0; k < n2; k++)
                {
                    ...
                    {
                        rd = 0.5 * (svijk[0] + svijk[1] + lgm2F_ifr[step]) * delta_t;
                    }
                    else
                    {
                        rd = 0.5 * (svijk[2] + lgm1F_ifr[step]) * delta_t;
                    }
                    rd = (1.0 - rd) / (1.0 + rd);
    ...
    if (lgm2F_is_dom_for == 0)
    {
        rd = 0.5 * lgm2F_ifr[0] * delta_t;
    }
    else
    {
        rd = 0.5 * lgm1F_ifr[0] * delta_t;
    }
    rd = (1.0 - rd) / (1.0 + rd);

    /*	Precalculate quantities used to compute original state variables and dual expectations
            E[R_t1_in_base_t1] = transf_matrix_inv_t1 * (A + B * transf_matrix_t0 * R_t0_in_base_t0)
     */
    ...
    if (lgm2F_is_dom_for == 0)
    {
        a1 = 0.0;
        a2 = 0.0;
        a3 = -corr_r3_fx[1] * vol_fx * sigma_next[2] * delta_t;
    }
    else
    {
        a1 = -corr_r1_fx[1] * vol_fx * sigma_next[0] * delta_t;
        a2 = -corr_r2_fx[1] * vol_fx * sigma_next[1] * delta_t;
        a3 = 0.0;
    }
    ...
    expect_node[0] = a1 + b1 * sv[i][j][k][0];
    expect_node[1] = a2 + b2 * sv[i][j][k][1];
    expect_node[2] = a3 + b3 * sv[i][j][k][2];
...
FREE_RETURN:
    ...
    if (nb_nodes_max)
    {
        free_lvector((unsigned long*)nb_nodes_max, 0, 2);
    }

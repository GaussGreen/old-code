...
double	dDomNotional, dFundNotional;
	...
	memcpy(sAux->dFixCoupon, sFunding->dFixCoupon, sFunding->iNbCpn * sizeof(double));	
	strcpy(sAux->cFundYcName, sMarket->cFundYcName);

	/*	Skip coupons fixed before today */
	iStartIndex = 0;
	while (iStartIndex < sFunding->iNbCpn && sFunding->lPayDate[iStartIndex] < sMarket->lToday + sMarket->iEODPayFlag)
	{
		iStartIndex++;
	}
	...
	{	
		...
		{
			sAux->dInitSpread[i] = swp_f_spread(sFunding->lStartDate[i], sFunding->lPayDate[i], sFunding->cRefRate);
			sAux->dNewSpread[i] = swp_f_spread(sFunding->lStartDate[i], sFunding->lPayDate[i], sFunding->cRefRate);
		}
	}
	...
	if (sFunding->iCCY)
	{
		...
		err = convert_funding_to_domestic(	sMarket->lToday,
											sDeal->lStartDate,
											sMarket->iEODFixFlag,
											sMarket->iEODPayFlag,
											sMarket->dFundSpotFx,
											sMarket->lFundFxSpotDate,
											dDomNotional,
											sMarket->cYcName,
											sFunding->iNbCpn,
											sFunding->lFixDate,
	...
	sAux->dCashFx = sMarket->dFundSpotFx * swp_f_df(sMarket->lToday, sMarket->lFundFxSpotDate, sMarket->cYcName) / swp_f_df(sMarket->lToday, sMarket->lFundFxSpotDate, sMarket->cFundYcName);

	/* Fill the Coupon Exchanges */
	for (i=iStartIndex; i<sFunding->iNbCpn; i++)
	{				
		sAux->dCouponPlusEx[i] = (sFunding->dCoverage[i] * (sAux->dMargin[i] + sAux->dNewSpread[i]) - 1.0) * sAux->dNot[i];
								...
								CRIF_COUPON_TYPE	*eCouponType)
{
	...
	if (!cCouponType) return "Empty string in crif_interp_coupon_type";
	...
	if(!strcmp(str,"REGULAR")) 	{*eCouponType = REGULAR_CRIF; 		return 0;}
...
Err crif_exotic_allocate_aux(CRIF_EXOTIC	sExotic)
{
	...
	sAux = sExotic->sAux;
	if (!sAux) return "Memory allocation faillure in crif_exotic_allocate_aux";	
	...
	if (!sAux->dCVStrike || !sAux->iFloatingType || !sAux->iHasFloorCap || !sAux->dMarketValue)
	{
...
void crif_exotic_free_aux(CRIF_EXOTIC	sExotic)
{
	...
	if (sAux->iFloatingType) free(sAux->iFloatingType);
	if (sAux->iHasFloorCap) free(sAux->iHasFloorCap);
	if (sAux->dMarketValue) free(sAux->dMarketValue);
}
...
void crif_free_exotic(CRIF_EXOTIC sExotic)
{
	...
	if (sExotic->lStartDate) free(sExotic->lStartDate);
	if (sExotic->lEndDate) free(sExotic->lEndDate);
	if (sExotic->lPayDate) free(sExotic->lPayDate);
	if (sExotic->dCoverage) free(sExotic->dCoverage);
	if (sExotic->dNot) free(sExotic->dNot);
	if (sExotic->eCouponType) free(sExotic->eCouponType);
	
	if (sExotic->lPaidFixingDate) free(sExotic->lPaidFixingDate);
	if (sExotic->dPaidMargin) free(sExotic->dPaidMargin);
	
	if (sExotic->dFloor) free(sExotic->dFloor);
	if (sExotic->dCap) free(sExotic->dCap);		
	
	if (sExotic->dPrvCpngearing) free(sExotic->dPrvCpngearing);
	if (sExotic->dPaidGearing) free(sExotic->dPaidGearing);
	if (sExotic->cPaidTenor) free_svector_size(sExotic->cPaidTenor, 0, 0, 5);
/*	if (sExotic->cPaidBasis) free_svector_size(sExotic->cPaidBasis, 0, 1-1, 5);
	if (sExotic->cPaidFreq) free_svector_size(sExotic->cPaidFreq, 0, 1-1, 5);
	if (sExotic->cPaidRef) free_svector_size(sExotic->cPaidRef, 0, 1-1, 10);	*/
	
	if (sExotic->dPastPaidCpn) free(sExotic->dPastPaidCpn);
}
...
Err	crif_fill_exotic(	CRIF_DEAL			sDeal,
					 ...
					 CRIF_CPN_NUMPARAMS	sNumerParams)
{
	...
	/*	Skip coupons fixed before today */
	iStartIndex = 0;
	while (iStartIndex < sExotic->iNbCpn && sExotic->lPayDate[iStartIndex] < sMarket->lToday + sMarket->iEODPayFlag)
	{		
		iStartIndex++;
	}
...
Err	 crif_call_allocate_aux(CRIF_CALL sCall)
{
	...
	sAux = sCall->sAux;
	if (!sAux) return "Memory allocation faillure in crif_call";	
...
void crif_call_free_aux(CRIF_CALL sCall)
{
	...
	if (sAux->iStartFundIdx) free(sAux->iStartFundIdx);
	if (sAux->iStartCpnIdx) free(sAux->iStartCpnIdx);	
}
...
void crif_free_call(CRIF_CALL	sCall)
{
	...
	if (sCall->lExeDate) free(sCall->lExeDate);
	if (sCall->lSettlDate) free(sCall->lSettlDate);
	if (sCall->dFee) free(sCall->dFee);
}


Err crif_check_call(CRIF_CALL	sCall,
					CRIF_EXOTIC	sExotic)
{
	int i;
	CRIF_CALL_AUX sAux;
	...
	for (i=sAux->iStartIndex; i<sCall->iNbCall; i++)
	{
		if (i > sAux->iStartIndex)
		{
			...
			{
				return serror ("Calls %d and %d -indexed after today- are redundant", i, i+1);
			}
		...
		if (i > sCall->sAux->iStartIndex)
		{
			if (sExotic->sAux->iFloatingType[sCall->sAux->iStartCpnIdx[i-1]] > 0 &&
				sCall->lExeDate[i] <= sExotic->lPaidFixingDate[sCall->sAux->iStartCpnIdx[i-1]])
			{
				return "Exercise date cannot be smaller or equal to the floating fixing date";
			}
		}				
	}	
...
Err	crif_fill_call(	CRIF_DEAL	sDeal,
					CMSCTS_MARKET	sMarket)
{
	CMSCTS_FUNDING	sFunding;
	CRIF_EXOTIC	sExotic;
	...
	/*	Skip coupons fixed before today */
	iStartIndex = 0;
	while (iStartIndex < sCall->iNbCall && sCall->lExeDate[iStartIndex] < sMarket->lToday + sMarket->iEODExeFlag)
	{
		iStartIndex++;
	}
	...
	for (i=0; i<sCall->iNbCall; i++)
	{
		...
		while (iFundIdx < sFunding->iNbCpn && sFunding->lStartDate[iFundIdx] < sCall->lExeDate[i])
		{
			iFundIdx++;
		}
		if (iFundIdx == sFunding->iNbCpn)
		{
			err = serror ("Call number %d does not control any coupon in funding leg", i+1);
			goto FREE_RETURN;
		}
		...
		while (iCpnIdx < sExotic->iNbCpn && sExotic->lStartDate[iCpnIdx] < sCall->lExeDate[i])
		{
			iCpnIdx++;
		}
		if (iCpnIdx == sExotic->iNbCpn)
		{
			err = serror ("Call number %d does not control any coupon in exotic leg", i+1);
			goto FREE_RETURN;
		}
	...
	if (sCall->iIsExercised)
	{
		...
		while (i < sCall->iNbCall && fabs(sCall->lExeDate[i] - sCall->lExercisedDate) > 0.5)
		{
			i++;
		}
							   ...
							   CRIF_CPN_NUMPARAMS	sNumerParams)
{
	...
	{
		sFunding->sAux->iEndIndex = sDeal->sCall->sAux->iStartFundIdx[sDeal->sCall->sAux->iIndexExercised]-1;
	}
	
	sFunding->sAux->iNbUsedCpn = sFunding->sAux->iEndIndex - sFunding->sAux->iStartIndex + 1;
	...
	if (sDeal->sCall->iNbCall > 0 && sDeal->sCall->iIsExercised)
	{
		sExotic->sAux->iEndIndex = sDeal->sCall->sAux->iStartCpnIdx[sDeal->sCall->sAux->iIndexExercised]-1;
	}
	
	sExotic->sAux->iNbUsedCpn = sExotic->sAux->iEndIndex - sExotic->sAux->iStartIndex + 1;
	
	/* find the start index exotic for numeric */
	if (sDeal->sCall->iIsExercised == 0)
	{
		// no closed form for the underlying so need to evaluate numerically from the the coupon fixing aftertoday
		sExotic->sAux->iNumStartIndex = sExotic->sAux->iStartIndex; 
		while(sExotic->sAux->iNumStartIndex <sExotic->iNbCpn && sExotic->lPaidFixingDate[sExotic->sAux->iNumStartIndex] < sMarket->lToday + sMarket->iEODPayFlag)
		{
	...
	sExotic->sAux->iNbUsedNumCpn = sExotic->sAux->iEndIndex - sExotic->sAux->iNumStartIndex + 1;
	...
	if (sExotic->sAux->iNumStartIndex < sExotic->iNbCpn)
	{
		...
		while (sDeal->sFunding->sAux->iNumStartIndex < sDeal->sFunding->iNbCpn &&
			sDeal->sFunding->lStartDate[sDeal->sFunding->sAux->iNumStartIndex] < sExotic->lStartDate[sExotic->sAux->iNumStartIndex] - 10)
		{
		...
		{
			err = serror("Exotic coupon number %d does not have a funding equivalent", sExotic->sAux->iNumStartIndex);
			goto FREE_RETURN;
		}
	}
	else
...
{   
	...
	if (!constStr) return "Empty string in crif_interp_calib_strat";
	...
	if(!strcmp(str,"DIAGCMS")) 	{*val = CMSCTS_DIAGCMS1; 		return 0;}
	if(!strcmp(str,"CMSDIAG")) 	{*val = CMSCTS_CMS1DIAG; 		return 0;}
...
Err	crif_allocate_pricing_params(CRIF_PRICING_PARAMS sPricingParams)
{
	...
	if (!sPricingParams->sCouponPricingParams || !sPricingParams->sSimulParams || !sPricingParams->sReserveParams)
	{

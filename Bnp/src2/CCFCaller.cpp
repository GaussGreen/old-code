...
Err ccf_caller(
  ...
  if (exercised) {
    ...
    {
            err = "Exercised settlement should be > than payment date of the
    last not called cf coupon "; goto FREE_RETURN;
    }

    if ((fund_ncpn > 0) && (ex_date_set<fund_pay[fund_ncpn-1]))
    {
            err = "Exercised settlement should be > than payment date of the
    last not called funding coupon "; goto FREE_RETURN;
    }
    */
  ...
  if (!exercised && cms_adj && cms_for_iv_only) {
    ...
    err = ccf_caller(
        today, use_calib, yc, vc, ref, swap_freq, swap_basis, lam_ts, lambda,
        tsnlam, tslamtime, tslam, alpha, gamma, rho, get_cash_vol, lgm2dund,
        start_date, fund_ccy, fund_not, fund_ccy_yc, fx_fund_dom,
        fx_fund_dom_spot_date, fund_ncpn, fund_fix, fund_start, fund_pay,
        fund_basis, fund_spr, fund_mrg, fund_fix_cpn, cf_not, cf_ncpn, cf_fix,
        cf_start, cf_pay, cf_basis, cf_cms_tenor1, cf_cms_freq1, cf_cms_basis1,
        cf_cms_spread1, cf_cms_tenor2, cf_cms_freq2, cf_cms_basis2,
        cf_cms_spread2, spread_vol_n, spread_vol_time, spread_vol_floor,
        spread_vol_cap,

        spread_slbeta1, spread_slbeta2,

        cf_alpha, cf_beta, cf_gamma, cf_floored, cf_floor, cf_capped, cf_cap,
        cf_fix_cpn,

        cf_nopt, cf_notopt, cf_strikeopt, cf_typeopt,

        0, pay_rec, ex_date, set_date, fee, req_stp, req_stpx, req_paths,
        force_atm, max_std_long, max_std_short, fix_lambda, cal_vol_shift_type,
        cal_vol_shift, cal_lambda_shift, one_f_equi, skip_last, long_prec,
        short_prec, min_fact, max_fact, use_jumps, proba_weight, calc_fwdiv,
        adjust_fee, eod_fix_flag, eod_pay_flag, eod_ex_flag, 1, 0,
        use_cms_smile, cms_vol_adj, cms_beta1, cms_beta2, num_strikes_in_vol,
        strikes_in_vol, vol_type, cash_vol, cvg_sv, is_corr,

        use_SL, calib_SL, NbStdcalib_SL, calib_correl_SL, use_cfoptions,

        exercised, ex_date_ex, ex_date_set, ex_fee, fund_val, cf_val, call_val,
        export_ts, und_exp);
    if (err) {
      goto FREE_RETURN;
    }
    ...
    err = ccf_caller(
        ...
        ncall, pay_rec, ex_date, set_date, fee, req_stp, req_stpx, req_paths,
        force_atm, max_std_long, max_std_short, fix_lambda, cal_vol_shift_type,
        cal_vol_shift, cal_lambda_shift, one_f_equi, skip_last, long_prec,
        short_prec, min_fact, max_fact, use_jumps, proba_weight, calc_fwdiv,
        adjust_fee, eod_fix_flag, eod_pay_flag, eod_ex_flag, 0, 0,
        use_cms_smile, cms_vol_adj, cms_beta1, cms_beta2, num_strikes_in_vol,
        strikes_in_vol, vol_type, cash_vol, cvg_sv, is_corr,

        use_SL, calib_SL, NbStdcalib_SL, calib_correl_SL, use_cfoptions,

        exercised, ex_date_ex, ex_date_set, ex_fee, &tmp1, &tmp2, call_val,
        export_ts, und_exp);
  ...
  if (fund_ccy == 1) {
    ...
    err = convert_funding_to_domestic(
        ...
        fx_fund_dom_spot_date, cf_not, yc, fund_ncpn, fund_fix, fund_start,
  ...
  free_struct = 0;
  err =
      ccf_fill_check_all_struct(
          today, use_calib, yc, vc, ref, swap_freq, swap_basis, lam_ts, lambda,
          tsnlam, tslamtime, tslam, alpha, gamma, rho, get_cash_vol, lgm2dund,
          fund_not, fund_ncpn, fund_fix, fund_start, fund_pay, fund_basis,
          fund_spr, fund_mrg, cf_not, cf_ncpn, cf_fix, cf_start, cf_pay,
          cf_basis, cf_cms_tenor1, cf_cms_freq1, cf_cms_basis1, cf_cms_spread1,
          cf_cms_tenor2, cf_cms_freq2, cf_cms_basis2, cf_cms_spread2,
          spread_vol_n, spread_vol_time, spread_vol_floor, spread_vol_cap,

          spread_slbeta1, /* Shifted log beta on the CMS1 */
          spread_slbeta2, /* Shifted log beta on the CMS2 */

          cvg_sv, is_corr, cf_alpha, cf_beta, cf_gamma, cf_floored, cf_floor,
          cf_capped, cf_cap,
          ...
          calib_correl_SL, /*	1: Calibrate the correlation between the two SL
                              to get the same ATM normal spread vol 0: use the
                              normal spread correl for the sl correl */

          use_cfoptions, /*  1: Use the spread options and don't take into
                            account the floor and cap in the cf coupons
                                         0: take into account the floor and cap
                            in the cf coupons */

          cms_adj, use_cms_smile, cms_vol_adj, cms_beta1, cms_beta2,
          num_strikes_in_vol, strikes_in_vol, vol_type, cash_vol, ncall,
          pay_rec, ex_date, set_date, fee, req_stp, req_stpx, req_paths,
          force_atm, max_std_long, max_std_short, fix_lambda,
          cal_vol_shift_type, cal_vol_shift, cal_lambda_shift, one_f_equi,
          skip_last, long_prec, short_prec, min_fact, max_fact, use_jumps,
          proba_weight, eod_fix_flag, eod_ex_flag, ccf, und, &call_feat,
          adi_arg);
  ...
  if (calc_fwdiv && ccf->num_calls > 0) {
    ...
    err = ccf_calc_mdl_iv_fwd(ccf, und, adi_arg, NUM_HERMITE, und->model_fwdiv);
  ...
  if (fund_leg->num_cpn > 0) {
    if (for_fund) {
      fund_leg_pv +=
          swp_f_df(today, fund_leg->cpn[0].start_date, yc) * eq_final_ex;
      ...
      if (calc_fwdiv) {
        ...
        for (i = 0; i < ccf->num_calls; i++) {
          ...
          und->market_fwdiv[i] =
              fact *
              swp_f_df(today,
                       (ccf->fund_leg->cpn + callt->fund_idx)->start_date, yc) *
              eq_final_ex;
        }
      }
    } else {
      fund_leg_pv +=
          swp_f_df(today, fund_leg->cpn[0].start_date, yc) * fund_leg->notional;
    ...
    if (fin_not_date >= today + eod_pay_flag) {
      if (for_fund) {
        ...
        /*fund_leg_pv += swp_f_df (today  , fin_not_date  , yc) *
         * fund_leg->notional;*/
      }
    }
  }
  ...
  if (for_fund) {
    ...
    if (calc_fwdiv) {
      for (i = 0; i < ccf->num_calls; i++) {
        ...
        und->market_fwdiv[i] -=
            fact *
            swp_f_df(today, (ccf->fund_leg->cpn + callt->fund_idx)->start_date,
                     yc) *
            eq_final_ex;
      }
    }
  ...
  if ((exercised) && (fund_pay[fund_ncpn - 1] != ex_date_set)) {
    ...
    if (for_fund) {
      ...
      fund_leg_pv +=
          ...
          swp_f_df(und->today, fx_fund_dom_spot_date, yc) /
  ...
  for (i = 0; i < exo_leg->num_cpn; i++) {
    ...
    for (j = 0; j < exo_cpn->ncms; j++) {
      ...
      if (cms_adj && vol > 1.0E-08) {
        if (use_cms_smile) {
          ...
          cmsrate[j] = cms_rate(cms->delay, cms->nfp, cms->cpnd, forward, vol);
        }
      } else {
        cmsrate[j] = forward;
      }
    ...
    if (!use_cfoptions) {
      if (!use_SL) {
        ...
        case 2:
          ...
          if (exo_cpn->cms_fix_time > 1.0E-08) {
            stdfloor = fabs(exo_cpn->alphabeta[0]) *
                       sqrt(exo_cpn->cms[0].floorvar * exo_cpn->cms_fix_time);
            stdcap = fabs(exo_cpn->alphabeta[0]) *
                     sqrt(exo_cpn->cms[0].capvar * exo_cpn->cms_fix_time);
          } else {
            stdfloor = stdcap = 0.0;
          }
          break;
        ...
        case 3:
          if (exo_cpn->cms_fix_time > 1.0E-08) {
            stdfloor = interp(spread_vol_time, spread_vol_floor, spread_vol_n,
                              exo_cpn->cms_fix_time, 0, &temp);
            stdcap = interp(spread_vol_time, spread_vol_cap, spread_vol_n,
                            exo_cpn->cms_fix_time, 0, &temp);

            if (is_corr) {
              stdfloor = sqrt(exo_cpn->alphabeta[0] * exo_cpn->alphabeta[0] *
                                  exo_cpn->cms[0].floorvar +
                              exo_cpn->alphabeta[1] * exo_cpn->alphabeta[1] *
                                  exo_cpn->cms[1].floorvar +
                              2.0 * stdfloor * exo_cpn->alphabeta[0] *
                                  exo_cpn->alphabeta[1] *
              ...
              stdcap = sqrt(
                  ...
                  2.0 * stdcap * exo_cpn->alphabeta[0] * exo_cpn->alphabeta[1] *
        ...
        floor = OPT_VAL_MACRO_N(floortype, spread,
                                exo_cpn->floor - exo_cpn->gamma, stdfloor);

        cap = OPT_VAL_MACRO_N(captype, spread, exo_cpn->cap - exo_cpn->gamma,
                              stdcap);
        ...
        if (exo_cpn->cms_fix_time > 1.0E-08) {
          ...
          spread_slcorr = interp(spread_vol_time, spread_vol_floor,
                                 spread_vol_n, exo_cpn->cms_fix_time, 0, &temp);
      ...
      if (!use_SL) {
        ...
        if (exo_cpn->cms_fix_time > 1.0E-08) {
          /* Extract the correlation */
          spread_corr = interp(spread_vol_time, spread_vol_floor, spread_vol_n,
                               exo_cpn->cms_fix_time, 0, &temp);
            ...
            if (exo_cpn->notopt[iNumOpt] != 0) {
              stdfloor = sqrt(exo_cpn->cms_fix_time *
                              (exo_cpn->alphabeta[0] * exo_cpn->alphabeta[0] *
                               ...
                               2.0 * spread_corr * exo_cpn->alphabeta[0] *
                                   exo_cpn->alphabeta[1] *
                                   sqrt(exo_cpn->cms[0].optvar[iNumOpt] *
                                        exo_cpn->cms[1].optvar[iNumOpt])));

              floor += exo_cpn->notopt[iNumOpt] *
                       OPT_VAL_MACRO_N((exo_cpn->typeopt[iNumOpt] == SRT_CALL
                                            ? (stdfloor == 0 ? 1 : 3)
                                            : (stdfloor == 0 ? 2 : 4)),
  ...
  while (i < cf_ncpn && cf_fix[i] < today + eod_fix_flag) {
    if (cf_pay[i] >= today + eod_pay_flag) {
      ...
      exo_leg_pv += cf_fix_cpn[i] * coverage(cf_start[i], cf_pay[i], bas) *
                    cf_not * swp_f_df(today, cf_pay[i], yc);
    }
  ...
  if (for_fund) {
    ...
    if (calc_fwdiv) {
      for (i = 0; i < ccf->num_calls; i++) {
        ...
        und->market_fwdiv[i] -=
            -fact *
            swp_f_df(today, (ccf->cf_leg->cpn + callt->cf_idx)->start_date,
                     yc) *
            cf_not;
      }
    }
  ...
  if (calc_fwdiv) {
    for (i = 0; i < ccf->num_calls; i++) {
      ...
      und->extra_fees[i] = -(und->market_fwdiv[i] - und->model_fwdiv[i]) /
                           swp_f_df(today, callt->set_date, yc);
  ...
  if ((exercised) && (fund_pay[fund_ncpn - 1] != ex_date_set)) {
    ...
    if (for_fund) {
      exo_leg_pv += (swp_f_df(und->today, ex_date_set, yc) -
                     swp_f_df(und->today, fund_pay[fund_ncpn - 1], yc)) *
                    cf_not;
    }
...
Err ccf_caller_LambdaShift(
  ...
  if (!exercised && cms_adj && cms_for_iv_only) {
    ...
    err = ccf_caller(
        ...
        use_SL, calib_SL, 0.5, calib_correl_SL, use_cfoptions,
  ...
  err =
      ccf_fill_check_all_struct(
          ...
          calib_SL, /*  1: Calibrate the Shifted Log models on CMS1 and on CMS2
                       * 0: use the Shifted Log beta given */
          0.5, calib_correl_SL, /*	1: Calibrate the correlation between the
                                   two SL to get the same ATM normal spread vol
                                                        0: use the normal spread
                                   correl for the sl correl */

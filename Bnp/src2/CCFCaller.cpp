    ...
    CCF_UND und_exp)
{
    ...
    /* Memory Allocation */
    ccf     = (ccf_str*)calloc(1, sizeof(ccf_str));
    und     = (ccf_und*)calloc(1, sizeof(ccf_und));
    adi_arg = (ccf_adi_arg*)calloc(1, sizeof(ccf_adi_arg));
    ...
    if (exercised)
    {
        ...
        {
                err = "Exercised settlement should be > than payment date of the last not called cf
        coupon "; goto FREE_RETURN;
        }

        if ((fund_ncpn > 0) && (ex_date_set<fund_pay[fund_ncpn-1]))
        {
                err = "Exercised settlement should be > than payment date of the last not called
        funding coupon "; goto FREE_RETURN;
        }
        */
        ...
        if ((ex_date_set < today + eod_pay_flag) || ((cf_ncpn == 0) && (fund_ncpn == 0)))
        {
            ...
            *fund_val = *cf_val = *call_val = 0.0;
            ...
            if (start_date >= today + eod_pay_flag)
            {
                if (fund_ccy)
                {
                    *fund_val -= swp_f_df(today, start_date, fund_ccy_yc) * fund_not * fx_fund_dom *
    ...
    err         = ccf_fill_check_all_struct(
        ...
        use_cfoptions, /*  1: Use the spread options and don't take into account
                                                                                       the floor and
                          cap in the cf coupons 0: take into account the floor and cap in the cf
                          coupons */
    ...
    if (fund_leg->num_cpn > 0)
    {
        if (for_fund)
        {
            fund_leg_pv += swp_f_df(today, fund_leg->cpn[0].start_date, yc) * eq_final_ex;
            ...
            if (calc_fwdiv)
            {
                ...
                for (i = 0; i < ccf->num_calls; i++)
                {
                    ...
                    und->market_fwdiv[i] =
                        fact *
                        swp_f_df(today, (ccf->fund_leg->cpn + callt->fund_idx)->start_date, yc) *
                        eq_final_ex;
                }
            }
        }
        ...
        {
            fund_leg_pv += swp_f_df(today, fund_leg->cpn[0].start_date, yc) * fund_leg->notional;
    ...
    if (for_fund)
    {
        ...
        if (calc_fwdiv)
        {
            for (i = 0; i < ccf->num_calls; i++)
            {
                callt = ccf->call + i;
                und->market_fwdiv[i] -=
                    fact * swp_f_df(today, (ccf->fund_leg->cpn + callt->fund_idx)->start_date, yc) *
                    eq_final_ex;
            }
        }
    }
    ...
    for (i = 0; i < exo_leg->num_cpn; i++)
    {
        ...
        if (!use_cfoptions)
        {
            if (!use_SL)
            {
                    ...
                    if (exo_cpn->cms_fix_time > 1.0E-08)
                    {
                        ...
                        if (is_corr)
                        {
                            stdfloor = sqrt(
                                ...
                                2.0 * stdfloor * exo_cpn->alphabeta[0] * exo_cpn->alphabeta[1] *
                ...
                floor =
                    OPT_VAL_MACRO_N(floortype, spread, exo_cpn->floor - exo_cpn->gamma, stdfloor);

                cap = OPT_VAL_MACRO_N(captype, spread, exo_cpn->cap - exo_cpn->gamma, stdcap);
    ...
    while (i < cf_ncpn && cf_fix[i] < today + eod_fix_flag)
    {
        if (cf_pay[i] >= today + eod_pay_flag)
        {
            ...
            exo_leg_pv += cf_fix_cpn[i] * coverage(cf_start[i], cf_pay[i], bas) * cf_not *
    ...
    if (for_fund)
    {
        ...
        {
            {
                temp = swp_f_df(today, fund_leg->cpn[fund_leg->num_cpn - 1].pay_date, yc) * cf_not;
                exo_leg_pv += temp;
        ...
        if (calc_fwdiv)
        {
            for (i = 0; i < ccf->num_calls; i++)
            {
                callt = ccf->call + i;
                und->market_fwdiv[i] -=
                    -fact * swp_f_df(today, (ccf->cf_leg->cpn + callt->cf_idx)->start_date, yc) *
                    cf_not;
            }
        }
    }

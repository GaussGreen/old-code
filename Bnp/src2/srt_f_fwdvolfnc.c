	...
	double				**cal_weights)
{
	...
	*cal_weights = dvector (0, *cal_numinst-1);
	...
	{
		(*cal_start)[i] = add_unit ((*cal_start)[i], spot_lag, SRT_BDAY, MODIFIED_SUCCEEDING);
	
		add_tenor(DTOL ((*cal_start)[i]),
							und_tenor,
		...
		err = swp_f_initSwapDP((*cal_start)[i],
								(*cal_end)[i],
								(*cal_freq)[i],
								(*cal_basis)[i],
								&swapdp);
		...
		err = swp_f_Swaption_SwapDP(&swapdp,
								   ATMVol,
								   (*cal_str)[i],
								   SRT_RECEIVER,
								   (*cal_refrate)[i],
	...
	{
		(*cal_start)[i] = add_unit (short_swap_start_dates[i-num_long_swaptions], spot_lag, SRT_BDAY, MODIFIED_SUCCEEDING);
	...
	if(short_swap_start_dates) free_lngvector(short_swap_start_dates,0,num_period-1);
...
/*	No memory error assumed */
	...
	*cal_weights = dvector (0, *cal_numinst-1);
	...
	for (i=0;i<*cal_numinst;i++)
	{
		...
		(*cal_end)[i] = add_unit (	DTOL ((*cal_start)[i]), 
	...
	double				*premium)
{
	...
	{	err = swp_f_LevelPayment (DTOL (start_act), 
								  DTOL (end_th), 
								  compdStr, 
								  basisStr, 
								  yc_id, 
	...
	/* CHEY case: make grfn tableau */
	{
		...
		sprintf (sprdsht[0][0].sval, "fra(%d,%d,\"%s\",\"%s\",\"%s\")", (long)start_act, (long)end_th, basisStr,und_name,ref_rate);
		sprdsht[0][0].type = GRFNSCELL;
	
		sprintf (sprdsht[1][0].sval, "fra(%d,%d,\"%s\",\"%s\",\"%s\")", (long)start_act, (long)end_th, basisStr,und_name,ref_rate);
		...
		sprintf (sprdsht[2][1].sval, "max(c[0,1]-c[0,0],0)*%.12f", cvg);
...
/*	Forward Reset CMS Option is the following profile:
	At T3, pay: MAX (CMS(T2,T3,T4) - CMS(T1,T3,T4), 0) 
	Where	T1 = vol start date = strike_fix
			T2 = vol end date = CMS_fix
			T3 = CMS start date = start_act
			T4 = CMS end date = end_theo */

Err srt_f_fwd_resetcms(SrtUndPtr		irundptr,
					   SrtGrfnParam		*grfnparam,
					   Date				strike_fix,
					   Date				cms_fix,
					   Date				start_act,
...
double			SwpSpread;
	...
	sprintf(sprdsht[0][0].sval,"CMS(%d,%d,\"%s\",\"%s\",%lf,%d,\"%s\",\"%s\",\"%s\")",start_act,end_theo,compStr,basisStr,cmsvol,start_act,bs_vol_type,ir_und_name,ref_rate);
	sprdsht[0][0].type = GRFNSCELL;

	sprintf(sprdsht[1][0].sval,"CMS(%d,%d,\"%s\",\"%s\",%lf,%d,\"%s\",\"%s\",\"%s\")",start_act,end_theo,compStr,basisStr,cmsvol,start_act,bs_vol_type,ir_und_name,ref_rate);
	sprdsht[1][0].type = GRFNSCELL;
...
Err err;
	...
	*answer = 0.0;
	...
	{
		opt = srt_f_optblknrm (fwd_cms, strike, vol, maturity, 1.0, call_put, PREMIUM);
	}
	else
	{
		opt = srt_f_optblksch (fwd_cms, strike, vol, maturity, 1.0, call_put, PREMIUM);
	}

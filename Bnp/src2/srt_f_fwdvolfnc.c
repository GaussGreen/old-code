    ...
    double** cal_weights)
{
    ...
    *cal_weights = dvector(0, *cal_numinst - 1);
    ...
    for (i = 0; i < num_long_swaptions; i++)
    {
        ...
        add_tenor(DTOL((*cal_start)[i]), und_tenor, NO_BUSDAY_CONVENTION, &(*cal_end)[i]);
        ...
        err = swp_f_initSwapDP(
            (*cal_start)[i], (*cal_end)[i], (*cal_freq)[i], (*cal_basis)[i], &swapdp);
    ...
    {
        (*cal_start)[i] = add_unit(
            short_swap_start_dates[i - num_long_swaptions],
            spot_lag,
            SRT_BDAY,
            MODIFIED_SUCCEEDING);
...

{
    ...
    *cal_weights = dvector(0, *cal_numinst - 1);
    ...
    for (i = 0; i < *cal_numinst; i++)
    {
        ...
        (*cal_end)[i] =
            add_unit(DTOL((*cal_start)[i]), 12 / (int)compd, SRT_MONTH, NO_BUSDAY_CONVENTION);
    ...
    double*         premium)
{
    ...
    /* LGM case: call srt_f_lgm_resetcaplet */
    {
        err = swp_f_LevelPayment(
            DTOL(start_act), DTOL(end_th), compdStr, basisStr, yc_id, ref_rate, &level);
        if (err)
        {
            return err;
        }
...

{
    ...
    err = swp_f_ForwardRate(start_act, end_theo, compStr, basisStr, yc_id, ref_rate, &swap);

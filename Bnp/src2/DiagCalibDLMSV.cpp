...
static int NB_MOM_LM, INDEX_MOM[MAX_INST], BREAK_INST, FREQ_SHORT_LM,
    SHIFT_FREQ_LM, NEXS_LM, MAX_ITER_NEWTON;

static double *EX_PRICES_LM;

static double PRICE_LM[MAX_CPN], SENSI_LM[MAX_CPN][MAX_CPN], PRECISION_LM;

static Err (*CALIBRATION_FUNCTION)(void *AllPrimaryInst, void *Model,
                                   void *Params),

    (*PRICING_FUNCTION)(void *AllSecondaryInst, void *Model, void *Params,
                        int iIndex, double *dPrice),

    (*BUMPINGMODEL_FUNCTION)(void *Model, int iIndex, double NewValue);

void *ALLPRIMINST, *ALLSECINST, *MODEL_LM, *PARAMS;

void init_static_moment_dlm(
    ...
    int nb_exe_dates, double *target_prices,
...
Err static_pricer_momentum_dlm(double index, double lam[], double *price,
  ...
  if (ind == 0) {
    ...
    for (j = 0; j < nlam; j++) {
      ...
      err = BUMPINGMODEL_FUNCTION(MODEL_LM, j, lambda[j]);

      if (err) {
        return err;
      }

      err = CALIBRATION_FUNCTION(ALLPRIMINST, MODEL_LM, PARAMS);
      ...
      for (i = 0; i < NEXS_LM; i++) {
        if (!fmod(i + SHIFT_FREQ_LM, FREQ_SHORT_LM)) {
          err = PRICING_FUNCTION(ALLSECINST, MODEL_LM, PARAMS, i,
                                 &SENSI_LM[i][j]);
...
static Err calib_lambda_simpleNewton_dlm(double *lam, double *price) {
  int i, j, k, l;
  ...
  /* First guess */
  err = BUMPINGMODEL_FUNCTION(MODEL_LM, 0, lam1);
  ...
  while (fabs(price1) > PRECISION_LM && k < MAX_ITER_NEWTON) {
    ...
    err = BUMPINGMODEL_FUNCTION(MODEL_LM, 0, lam2);
    ...
    lam2 = solve_for_next_coef_dlm(res_iter, k, 0.0, 2);

    if (lam2 < LIMIT_LAM_SV) {
      smessage("DiagCalibSVDLM: Limit lambda has been reached  , calibration "
               "stopped");
      lam2 = LIMIT_LAM_SV;
      break;
    }
...
FREE_RETURN:
  ...
  if (res_iter)
    free_dmatrix(res_iter, 0, MAX_ITER_NEWTON, 0, 1);
...
Err LGMSVcalibration_dlm(
  ...
  if (fix_lambda) {
    ...
    lm_params->shift_freq =
        (int)(fmod(lm_params->shift_freq, nb_exe_dates) + 0.5);
    ...
    if (lm_params->use_moment) {
      ...
      init_static_moment_dlm(AllPrimaryInst, AllSecondaryInst, Model, Params,
                             CalibrationFunction, PricingFunction,
                             BumpingModelFunction, nb_exe_dates, target_prices,
                             lm_params);
      ...
      if (nlam == 1) {
        ...
        err = levenberg_marquardt(data, target, weight, lm_params->nb_moment,
                                  ...
                                  static_pricer_momentum_dlm, &fitting_error);

...
static void FillAffineCoefFxSV(SAffinCoef* p, double t)
{
    ...
    // Drift:
    p->B[0][0] =
        -(Gamma_d * Gamma_d / 2.0 + Gamma_f * Gamma_f / 2.0 -
          pmdl->rho[idx_x][0][1] * Gamma_d * Gamma_f);
    p->B[0][1] = 0.0;
    p->B[0][2] =
        -pmdl->beta[idx_x] * (pmdl->rho[idx_x][1][2] * Gamma_f - pmdl->rho[idx_x][0][2] * Gamma_d);
...
static void NAG_CALL EvalDerivatives(Integer neq, double t, double y[], double yp[], Nag_User* comm)
{
    ...
    for (i = 1; i <= p->n; i++)
    {
        ...
        {
            yp[2 * k] -=
                0.5 * (y[2 * i] * y[2 * i] - y[2 * i + 1] * y[2 * i + 1]) * p->D[i - 1][i - 1][k];
        ...
        for (j = i + 1; j <= p->n; j++)
        {
            ...
            {
                yp[2 * k] -=
                    (y[2 * i] * y[2 * j] - y[2 * i + 1] * y[2 * j + 1]) * p->D[i - 1][j - 1][k];
                yp[2 * k + 1] -=
                    (y[2 * i] * y[2 * j + 1] + y[2 * i + 1] * y[2 * j]) * p->D[i - 1][j - 1][k];
            }
        }
    }
}
...
Err AffineCalcMoments(SFxSVComm_InvFT* q, double* mean, double* std)
{
    ...
    for (i = o->ntimes - 2; i >= 0; i--)
    {
        ...
        nag_ode_ivp_rk_range(
            NEQ, EvalDerivativesU, o->times[i], &tgot, y, yp, ymax, &opt, &comm_Nag, &fail);
        if (fail.code != NE_NOERROR)
        {
            err = serror(fail.message);
            goto FREE_RETURN;

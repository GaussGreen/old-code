...
static void LGMSV_solve_poly2_comp(
    double  Are,
    double  Aim,
    double  Bre,
    double  Bim,
    ...
    double* AIm)
{
    ...
    if (fabs(DeltaRe) + fabs(DeltaIm) > 1.0E-10)
    {
        if (fabs(*DRe - x2Re) + fabs(*DIm - x2Im) > 1.0E-10)
        {
            ...
            LGMSV_div_comp(*DRe - x1Re, *DIm - x1Im, *DRe - x2Re, *DIm - x2Im, &RatRe, &RatIm);
            LGMSV_prod_comp(RatRe, RatIm, -ExpRe, -ExpIm, &FactRe, &FactIm);

            if (coefBRe != 0.0)
            {
                LGMSV_div_comp(1.0 - RatRe, -RatIm, 1.0 + FactRe, FactIm, &ExpRe, &ExpIm);
            ...
            LGMSV_div_comp(x1Re + (*DRe), x1Im + (*DIm), 1.0 + FactRe, FactIm, DRe, DIm);
        }
        else
        {
            *ARe += *DRe * coefInt * dt;
...
void LGMSVCalculateDensityPrecalcNew(
    int    iNbPhi,
    int    iNbft,
    double dPhiFreqStep,
    double dftFreqStep,
...
void LGMSVCalculateDensityTPPointNew(
    double dPhifreq,
    double dftFreq,
    ...
    double* LogTFIm)
{
    ...
    for (i = endi; i >= 0; i--)
    {
        ...
        LGMSVUpdateADClosedForm(
            dt[i], Coef2Re, Coef1Re, Coef2Re, Coef2Im, Coef3Re, Coef3Im, &DRe, &DIm, &ARe, &AIm);
    }
    ...
    int*    iIndexft0)
{
    ...
    *dPhiStep      = (dPhitMean - dPhiMin) / (*iIndexPhiMean - 1);
    ...
    if (iPriorityFreqPhi)
    {
        ...
        LGMSVCalculateDensityTPPointNew(
            dPhiFreqMin,
            0.0,
            endi,
            dt,
    ...
    double* sigma)
{
    ...
    double *dt = NULL, *Coef2ImT = NULL, *Coef3ReT = NULL, *Coef3ImT = NULL, **res_iter = NULL;
    ...
    /* Gestion of allocation errors */
    if (!Density || !Densityspeq || !dt || !Coef2ImT || !Coef3ReT || !Coef3ImT || !res_iter)
    {
        err = "Memory allocation error (1) in LGMSVClosedForm2New";
        goto FREE_RETURN;
    }
    ...
    for (i = 0; i < nbEx; i++)
    {
        ...
        dPhitStd  = sqrt(ExpectPhi2Temp - ExpectPhiTemp * ExpectPhiTemp) * Coef3ImInit;
    ...
    double* pSwaptionPrice)
{
    ...
    swp_cash = (swp_f_df(today, cpn_date[0], yc_name) - swp_f_df(today, cpn_date[ncpn-1], yc_name))
    / level; dStrike -= swp_rte - swp_cash;
    */
    ...
    for (i = 1; i < ncpn; i++)
    {
        ...
        cpn_cvg[i] *=
            -pay_rec * (dStrike - (swp_rte - swp_cash)) * swp_f_df(today, cpn_date[i], yc_name);
    }
    ...
    for (i = 0; i < nex; i++)
    {
        ...
        for (j = 1; j < iNbCoupon[i]; j++)
        {
            if (param->strike_type == 2)
            {
                ...
                swp_cash =
                    (swp_f_df(today, cpn_date[j + ex_cpn[i] - 1], yc_name) -
                     swp_f_df(today, cpn_date[j + ex_cpn[i]], yc_name)) /
                    (cpn_cvg[j + ex_cpn[i]] * swp_f_df(today, cpn_date[j + ex_cpn[i]], yc_name));

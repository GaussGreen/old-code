...
static void LGMSV_solve_poly2_comp(	double	Are,
									double	Aim,
									double	Bre,
									double	Bim,
									double	Cre,
...
void LGMSVCalculateDensityPrecalcNew(int	iNbPhi,
									int		iNbft,							
									double	dPhiFreqStep,
									double	dftFreqStep,
	...
	{					
		...
		for (iNumftFreq=1; iNumftFreq<=nbFtHalf; iNumftFreq++)
		{
			...
			{					
				...
				LGMSVUpdateADClosedForm(dt[i], Coef2Re, Coef1Re, Coef2Re, Coef2Im, Coef3Re, Coef3Im, &DRe, &DIm, &ARe, &AIm);
			}
...
void LGMSVCalculateDensityTPPointNew(double	dPhifreq,
									double	dftFreq,
...
void LGMSVOptionPricePrecalcNew(int		iNbPhi,
								int		iNbft,								
...
clock_t	time1, time2, time3;
	...
	LGMSVCalculateDensityPrecalcNew(iNbPhi, iNbft, dPhiFreqStep, dftFreqStep, endi,
									dt, Coef1Re, Coef2Re, Coef2ImT, Coef3ReT, Coef3ImT,
...
void LGMSVFindFreqNew(int	iNbPhi,
					int		iNbft,
...
int		i, NbMinMean;
	...
	*dPhiStep = (dPhitMean - dPhiMin) / (*iIndexPhiMean - 1);
	...
	if (iPriorityFreqPhi)
	{
		...
		LGMSVCalculateDensityTPPointNew(dPhiFreqMin, 0.0, endi, dt, Coef1Re, Coef2Re, Coef2ImT, Coef3ReT, Coef3ImT,
	...
	*dftStep =  -dftMin / (*iIndexft0 - 1);
	...
	if (iPriorityFreqFt)
	{
		...
		LGMSVCalculateDensityTPPointNew(0.0, dFtFreqMin, endi, dt, Coef1Re, Coef2Re, Coef2ImT, Coef3ReT, Coef3ImT,
									dPhitMean, &LogTFRe, &LogTFIm);
...
clock_t			time1, time2;
	...
	/* First pricing */
	LGMSVOptionPricePrecalcNew(iNbPhi, iNbft, dLambdaX, dTStar, endi, dt,
							Coef1Re, Coef2Re, Coef2ImT, Coef3ReT, Coef3ImT,
...
Err LGMSVOptionGRFNPrice(	int		iNbPhi,
							int		iNbft,								
...
clock_t	time1, time2, time3;
	...
	for (iNumPhi=1; iNumPhi<=iNbPhi; iNumPhi++)
	{
		for (iNumft=1; iNumft<=iNbft; iNumft++)
		{
			...
			{			
				Price[iNumProd] += Density[1][iNumPhi][iNumft] * PayOff[iNumPhi][iNumft][iNumProd];
			}
		}
	}	
...
clock_t			time1, time2;
	...
	/* Gestion of allocation errors */
	if (!Density || !Densityspeq ||
		!dt || !Coef2ImT || !Coef3ReT || !Coef3ImT || !res_iter)
	{
		err = "Memory allocation error (1) in LGMSVClosedForm2New";
		goto FREE_RETURN;
	}
	...
	for (i=0; i<nbEx; i++)
	{
		...
		/* Find Frequence */		
		LGMSVFindFreqNew(iNbPhi, iNbft, AlphaEq, dRho, i, dt,
						Coef1Re, Coef2Re, Coef2ImT, Coef3ReT, Coef3ImT,
						dPhitMean, dPhitStd, dftStd, iNbSigmaPhiGridLeft, iNbSigmaPhiGridRight, iNbSigmaftLeft, iNbSigmaftRight,
						LimitPhi, Limitft, iPriorityFreqPhi, iPriorityFreqFt,
						&dPhiFreqStep, &dPhiStep, &iIndexPhiMean, &dftFreqStep, &dftStep, &iIndexft0);
		...
		/* First pricing */
		LGMSVOptionPricePrecalcNew(iNbPhi, iNbft, dLambdaX, dTStar, i, dt,
								Coef1Re, Coef2Re, Coef2ImT, Coef3ReT, Coef3ImT,
...
FREE_RETURN:
	...
	if (dt) free_dvector(dt, 0, nbEx-1);
				...
				double			*pSwaptionPrice)
{
	...
	*/
	...
	for (i=1; i<ncpn; i++)
	{
		...
		cpn_cvg[i] *= -pay_rec * (dStrike - (swp_rte - swp_cash)) * swp_f_df(today, cpn_date[i], yc_name);
	}
	...
	for (i=0; i<nex; i++)
	{
		...
		for (j=1; j<iNbCoupon[i]; j++)
		{
			if (param->strike_type == 2)
			{
				...
				swp_cash = (swp_f_df(today, cpn_date[j+ex_cpn[i]-1], yc_name) - swp_f_df(today, cpn_date[j+ex_cpn[i]], yc_name)) / (cpn_cvg[j+ex_cpn[i]] * swp_f_df(today, cpn_date[j+ex_cpn[i]], yc_name));

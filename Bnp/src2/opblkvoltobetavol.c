...
static Err rtsafe_func(double x, double* fx, double* dfx)
{
    ...
    err = srt_f_optbetavoltoblkvol(
        static_forward, static_strike, x, static_mat, static_beta, &vol_tmp);

    if (err)
        return err;

    err = srt_f_optbetavoltoblkvol(
        static_forward, static_strike, x + dx, static_mat, static_beta, &vol_tmp2);
    ...
    double forward, double strike, double bsvol, double mat, double beta, double* betavol)
{
    ...
    err = srt_f_optbetavoltoblkvol(forward, strike, guess_vol, mat, beta, &vol_log);
    ...
    while ((setup == 0) && (niter <= 5))
    {
        ...
        err = num_f_optblkvoltobetavol(
            forward, strike, bsvol + direc * vol_bump, mat, beta, &guess_vol_down);
        if (err)
            return err;

        err = srt_f_optbetavoltoblkvol(forward, strike, guess_vol_down, mat, beta, &vol_log_down);
    ...
    double* betavol)
{
    ...
    err =
        srt_f_optbetastochvoltoblkvol(forward, strike, guess_vol, alpha, rho, mat, beta, &vol_log);
    ...
    while ((setup == 0) && (niter <= 10))
    {
        ...
        err = srt_f_optbetastochvoltoblkvol(
            forward, strike, guess_vol_down, alpha, rho, mat, beta, &vol_log_down);

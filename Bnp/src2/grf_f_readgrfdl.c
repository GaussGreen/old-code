...
#define GETLNG(LINE,T){\
	...
	if(!TOK)return serror("failed to read long");\
	else{\
		T = strtol(TOK,&TOK2,10);\
		if(strlen(TOK2)!=0)return serror("failed to read long");\
	}}

#define GETDBL(LINE,T){\
	String TOK2,TOK=strtok(LINE,GRFSEPSTR);\
	if(!TOK)return serror("failed to read double");\
	else{\
		T = strtod(TOK,&TOK2);\
		if(strlen(TOK2)!=0)return serror("failed to read double");\
	}}

#define GETSTR(LINE,T){\
	String TOK=strtok(LINE,GRFSEPSTR);\
	if(!TOK)return serror("failed to read str");\
	else{\
		T = TOK;\
	}}
...
{
	if(fgets(line,buflen,in)==NULL)return 0;
	return strlen(line);
}
...
static long getgoodline(String line, long buflen, FILE *in, String commentstr)
{
	...
	while(1)
	{
		if(!getline(line,buflen,in))return 0;
		strupper(line);
		strip_white_space(line);
		len=strlen(line);
		if(len>0 && strpbrk(line,commentstr)!=line)break;
	}
	return len;
}
...
static Err read_event_row(String line,long rowlen)
{
	...
	tok=strtok(line,c);
	if(!tok)return serror("failed to read event row");
	d = (Date)strtol(tok,&tok2,10);
	if(strlen(tok2)!=0)return serror("evdate bad format");
...
/* declare date to mad */
	...
	for(i=0;i<rowlen;i++){
		tok=strtok(NULL,c);
		if(!tok)return serror("failed on event %d in row",i);
		if(strlen(tok)==1 && tok[0]==GRFBLNCHAR)
...
#define GETGRFCELL(T){\
	...
	if(!TOK)return serror("failed to read str");\
	if(strlen(TOK)==1 && TOK[0]==GRFBLNCHAR){T.type=GRFNBCELL;}\
	else{\
		...
		)
{
	...
	if(!len)return serror("premature EOF");
			...
			)
{
	...
	*auxwidthptr=0;
    ...
    len=getgoodline(inputbuf,BUFLEN,in,GRFCOMSTR);
    if(!len)return serror("premature EOF");
	if(!strcmp(inputbuf,GRFENDAUX))return NULL;

	GETLNG(inputbuf,auxwidth);
	if(auxwidth < 1 || auxwidth > 100)return serror("bad auxdimensions");
	...
	*auxwidthptr = auxwidth;
	...
	if(auxlen[i]<1 || auxlen[i] > 500)return serror("bad auxdimensions");
	toalloc+=auxlen[i];
	...
	for(i=0;i<nrow;i++){
       len=getgoodline(inputbuf,BUFLEN,in,GRFCOMSTR);
	   if(!len)return serror("premature EOF reading aux");
	   buf=inputbuf;
...
Err grf_f_readgrfdl(FILE *in, GrfnDeal *gd)
{
   ...
   grfn_copy_to_GrfnDeal(gd,nrow,ncol,sprdsht,eventdates,
		numgrng,grng,auxwidth,auxlen,aux);
   
   if(aux[0])srt_free(aux[0]);
   if(aux)srt_free(aux);
   if(auxlen)srt_free(auxlen);
   if(sprdsht)
   {
	   grfn_free_GrfnCellmatrix(sprdsht,nrow,ncol);
   }
   if(eventdates)srt_free(eventdates);
   if(grng)srt_free(grng);/*FIX*/

...
char *SrtGrfnFXLGMSVMC(char *underlying, int numeventdates, long *eventdates,
                       ...
                       int nstept, int numpaths, int *nb_prod,
                       double ***prod_val) {
  int free_str = 0;
  FIRSTAllMkts xStr;
  ...
  double *evt_tms = NULL, *time = NULL, *date = NULL,

         *dom_sigma = NULL, *dom_alpha = NULL, *dom_rho = NULL,
         *dom_rho2 = NULL, *dom_lameps = NULL, *dom_lvleps = NULL,
         *dom_newalphaLGM = NULL, *dom_newrhoLGM = NULL,

         *dom_zcvol1_star = NULL, *dom_zcvol2_star = NULL,

         *dom_dff_star = NULL, *dom_gam1_star = NULL, *dom_gam2_star = NULL,
         *dom_gam1_2_star = NULL, *dom_gam2_2_star = NULL,
         *dom_gam12_star = NULL,

         *for_sigma = NULL, *for_alpha = NULL, *for_rho = NULL,
         *for_rho2 = NULL, *for_lameps = NULL, *for_lvleps = NULL,
         *for_newalphaLGM = NULL, *for_newrhoLGM = NULL,

         *for_zcvol1_star = NULL, *for_zcvol2_star = NULL,

         *for_dff_star = NULL, *for_gam1_star = NULL, *for_gam2_star = NULL,
         *for_gam1_2_star = NULL, *for_gam2_2_star = NULL,
         *for_gam12_star = NULL;
  ...
  fxlgmsv_get_fx_and_correl_ts(underlying, &fx_spot, &num_fx_vol, &fx_time,
                               &fx_vol, &num_rho, &rho_time, &CorrMatrix);
  ...
  if (!void_prm || !is_event || !date || !dom_sigma || !dom_alpha || !dom_rho ||
      !dom_rho2 || !dom_lameps || !dom_lvleps || !dom_newalphaLGM ||
      !dom_newrhoLGM || !fx_sigma || !correlation || !dom_zcvol1_star ||
      !dom_zcvol2_star || !for_zcvol1_star || !for_zcvol2_star ||
      !dom_dff_star || !dom_gam1_star || !dom_gam2_star || !dom_gam1_2_star ||
      !dom_gam2_2_star || !dom_gam12_star || !for_sigma || !for_alpha ||
      !for_rho || !for_rho2 || !for_lameps || !for_lvleps || !for_newalphaLGM ||
      !for_newrhoLGM || !for_dff_star || !for_gam1_star || !for_gam2_star ||
      !for_gam1_2_star || !for_gam2_2_star || !for_gam12_star) {
  ...
  for (i = nstp - 1; i >= 0; i--) {
    ...
    if (i < nstp - 1) {
      ...
      // J.M.L 10 Mar 2004
      // dom_zcvol1_star represents the term (1 - exp(lbda1 * ( Tstar - t )) /
      // lbda1 which appears in the volatility of the bond B(t        ,Tstar).
      // We can either code it directly:

      // dom_zcvol1_star[i+1] = (1.0 - exp(dom_model->dLambdaX *
      // (dom_model->dTStar - time[i]))) / dom_model->dLambdaX;

      // but a better plan is to integrate the square of this expression which
      // has the dimension of a volatility and find an equivalent volatility for
      // the bond which is valid between ti and ti+1

      dom_zcvol1_star[i + 1] =
          -sqrt(
              (time[i + 1] - time[i] +
               2 *
                   (exp(dom_model->dLambdaX *
                        (dom_model->dTStar - time[i + 1])) -
                    exp(dom_model->dLambdaX * (dom_model->dTStar - time[i]))) /
                   dom_model->dLambdaX +
               0.5 *
                   (exp(2 * dom_model->dLambdaX *
                        (dom_model->dTStar - time[i])) -
                    exp(2 * dom_model->dLambdaX *
                        (dom_model->dTStar - time[i + 1]))) /
                   dom_model->dLambdaX) /
              (time[i + 1] - time[i])) /
      ...
      // dom_zcvol2_star[i+1] = (1.0 - exp(dom_model->dLambdaX2 *
      // (dom_model->dTStar - time[i]))) / dom_model->dLambdaX2;

      dom_zcvol2_star[i + 1] =
          -sqrt(
              (time[i + 1] - time[i] +
               2 *
                   (exp(dom_model->dLambdaX2 *
                        (dom_model->dTStar - time[i + 1])) -
                    exp(dom_model->dLambdaX2 * (dom_model->dTStar - time[i]))) /
                   dom_model->dLambdaX2 +
               0.5 *
                   (exp(2 * dom_model->dLambdaX2 *
                        (dom_model->dTStar - time[i])) -
                    exp(2 * dom_model->dLambdaX2 *
                        (dom_model->dTStar - time[i + 1]))) /
                   dom_model->dLambdaX2) /
      ...
      //	for_zcvol1_star[i+1] = (1.0 - exp(for_model->dLambdaX *
      //(for_model->dTStar - time[i]))) / for_model->dLambdaX;
      //	for_zcvol2_star[i+1] = (1.0 - exp(for_model->dLambdaX2 *
      //(for_model->dTStar - time[i]))) / for_model->dLambdaX2;

      for_zcvol1_star[i + 1] =
          -sqrt(
              (time[i + 1] - time[i] +
               2 *
                   (exp(for_model->dLambdaX *
                        (for_model->dTStar - time[i + 1])) -
                    exp(for_model->dLambdaX * (for_model->dTStar - time[i]))) /
                   for_model->dLambdaX +
               0.5 *
                   (exp(2 * for_model->dLambdaX *
                        (for_model->dTStar - time[i])) -
                    exp(2 * for_model->dLambdaX *
                        (for_model->dTStar - time[i + 1]))) /
                   for_model->dLambdaX) /
              (time[i + 1] - time[i])) /
      ...
      for_zcvol2_star[i + 1] =
          -sqrt(
              (time[i + 1] - time[i] +
               2 *
                   (exp(for_model->dLambdaX2 *
                        (for_model->dTStar - time[i + 1])) -
                    exp(for_model->dLambdaX2 * (for_model->dTStar - time[i]))) /
                   for_model->dLambdaX2 +
               0.5 *
                   (exp(2 * for_model->dLambdaX2 *
                        (for_model->dTStar - time[i])) -
                    exp(2 * for_model->dLambdaX2 *
                        (for_model->dTStar - time[i + 1]))) /
                   for_model->dLambdaX2) /
    ...
    if (j >= 0 && fabs(time[i] - evt_tms[j]) < 1.0E-08) {
      ...
      dom_gam1_star[j] =
          (1.0 - exp(dom_model->dLambdaX * (dom_model->dTStar - evt_tms[j]))) /
          dom_model->dLambdaX;
      dom_gam2_star[j] =
          (1.0 - exp(dom_model->dLambdaX2 * (dom_model->dTStar - evt_tms[j]))) /
          dom_model->dLambdaX2;
      ...
      if (grfn_prm->num_dom_df > 0 && grfn_prm->dom_idx != -1) {
        ...
        if (!grfn_prm->dom_dff || !grfn_prm->dom_gam1 ||
            !grfn_prm->dom_gam1_2 || !grfn_prm->dom_gam2 ||
            !grfn_prm->dom_gam2_2 || !grfn_prm->dom_gam12) {
        ...
        for (k = 0; k < grfn_prm->num_dom_df; k++) {
          grfn_prm->dom_dff[k] =
              swp_f_df(xStr.dts[j], grfn_prm->dom_df_dts[k], dom_yc);
          grfn_prm->dom_dff[k] = log(grfn_prm->dom_dff[k]);
          temp1 =
              (1.0 - exp(dom_model->dLambdaX * (dom_model->dTStar - evt_tms[j] -
                                                grfn_prm->dom_df_tms[k]))) /
              dom_model->dLambdaX;
          temp2 = (1.0 -
                   exp(dom_model->dLambdaX2 * (dom_model->dTStar - evt_tms[j] -
                                               grfn_prm->dom_df_tms[k]))) /
                  dom_model->dLambdaX2;
      ...
      if (grfn_prm->num_fx_df > 0 && grfn_prm->fx_idx != -1) {
        ...
        if (!grfn_prm->fx_dff || !grfn_prm->fx_gam1 || !grfn_prm->fx_gam1_2 ||
            !grfn_prm->fx_gam2 || !grfn_prm->fx_gam2_2 || !grfn_prm->fx_gam12) {
        ...
        for (k = 0; k < grfn_prm->num_fx_df; k++) {
          grfn_prm->fx_dff[k] =
              swp_f_df(xStr.dts[j], grfn_prm->fx_df_dts[k], dom_yc);
          grfn_prm->fx_dff[k] = log(grfn_prm->fx_dff[k]);
          temp1 =
              (1.0 - exp(dom_model->dLambdaX * (dom_model->dTStar - evt_tms[j] -
                                                grfn_prm->fx_df_tms[k]))) /
              dom_model->dLambdaX;
          temp2 = (1.0 -
                   exp(dom_model->dLambdaX2 * (dom_model->dTStar - evt_tms[j] -
                                               grfn_prm->fx_df_tms[k]))) /
      ...
      for_gam1_star[j] =
          (1.0 - exp(for_model->dLambdaX * (for_model->dTStar - evt_tms[j]))) /
          for_model->dLambdaX;
      for_gam2_star[j] =
          (1.0 - exp(for_model->dLambdaX2 * (for_model->dTStar - evt_tms[j]))) /
          for_model->dLambdaX2;
      ...
      if (grfn_prm->num_for_df > 0 && grfn_prm->for_idx != -1) {
        ...
        if (!grfn_prm->for_dff || !grfn_prm->for_gam1 ||
            !grfn_prm->for_gam1_2 || !grfn_prm->for_gam2 ||
            !grfn_prm->for_gam2_2 || !grfn_prm->for_gam12) {
          err = "Memory allocation error in SrtGrfnMainFXLGMSV";
          goto FREE_RETURN;
        }
        ...
        for (k = 0; k < grfn_prm->num_for_df; k++) {
          grfn_prm->for_dff[k] =
              swp_f_df(xStr.dts[j], grfn_prm->for_df_dts[k], for_yc);
          grfn_prm->for_dff[k] = log(grfn_prm->for_dff[k]);
          temp1 =
              (1.0 - exp(for_model->dLambdaX * (for_model->dTStar - evt_tms[j] -
                                                grfn_prm->for_df_tms[k]))) /
              for_model->dLambdaX;
          temp2 = (1.0 -
                   exp(for_model->dLambdaX2 * (for_model->dTStar - evt_tms[j] -
                                               grfn_prm->for_df_tms[k]))) /
                  for_model->dLambdaX2;
  ...
  *resRows = max(num_col + 1, num_evt);
  ...
  if (do_optimisation) {
    ...
    err = mceb_allocate_params(params, num_evt);
  ...
  err = fxlgmsv_mc_balsam(nstp, num_evt, time, date,

                          numpaths,

                          dom_model->dLambdaX, dom_model->dLambdaX2,

                          dom_sigma, dom_newalphaLGM, dom_newrhoLGM, dom_alpha,
                          dom_lameps, dom_lvleps, dom_rho, dom_rho2,

                          dom_zcvol1_star, dom_zcvol2_star,

                          dom_dff_star, dom_gam1_star, dom_gam2_star,
                          dom_gam1_2_star, dom_gam2_2_star, dom_gam12_star,

                          for_model->dLambdaX, for_model->dLambdaX2,

                          for_sigma, for_newalphaLGM, for_newrhoLGM, for_alpha,
                          for_lameps, for_lvleps, for_rho, for_rho2,

                          for_zcvol1_star, for_zcvol2_star,

                          for_dff_star, for_gam1_star, for_gam2_star,
                          for_gam1_2_star, for_gam2_2_star, for_gam12_star,

                          fx_spot *
                              swp_f_df(today, for_model->lTStarDate, for_yc) /
                              swp_f_df(today, dom_model->lTStarDate, dom_yc),
                          fx_sigma,
                          ...
                          do_optimisation, optimise, params,
...
FREE_RETURN:
  ...
  if (fx_sigma)
    free(fx_sigma);
  if (correlation)
    free_f3tensor(correlation, 0, nstp - 1, 0, 6, 0, 6);

  if (dom_zcvol1_star)
    free(dom_zcvol1_star);
  if (dom_zcvol2_star)
    free(dom_zcvol2_star);
  if (for_zcvol1_star)
    free(for_zcvol1_star);
  if (for_zcvol2_star)
    free(for_zcvol2_star);
  ...
  if (dom_gam2_star)
    free(dom_gam2_star);
  if (dom_gam1_2_star)
    free(dom_gam1_2_star);
  if (dom_gam2_2_star)
    free(dom_gam2_2_star);
  if (dom_gam12_star)
    free(dom_gam12_star);
  ...
  if (for_gam2_star)
    free(for_gam2_star);
  if (for_gam1_2_star)
    free(for_gam1_2_star);
  if (for_gam2_2_star)
    free(for_gam2_2_star);
  if (for_gam12_star)
    free(for_gam12_star);
...
char *SrtGrfnQTOLGMSVMC(char *underlying, int numeventdates, long *eventdates,
                        long tableauRows, long *tableauCols,
  ...
  if (for_one2F == 1) {
    err = SrtGrfnQTOLGMSV1FMC(
        underlying, numeventdates, eventdates, tableauRows, tableauCols,
        tableauStrings, tableauMask, auxWidth, auxLen, aux, do_optimisation,
        optimise, params, resRows, nstept, numpaths, nb_prod, prod_val);
  } else {
    err = SrtGrfnQTOLGMSV2FMC(
        underlying, numeventdates, eventdates, tableauRows, tableauCols,
        tableauStrings, tableauMask, auxWidth, auxLen, aux, do_optimisation,
        optimise, params, resRows, nstept, numpaths, nb_prod, prod_val);
  }
...
char *SrtGrfnQTOLGMSV2FMC(char *underlying, int numeventdates, long *eventdates,
                          long tableauRows, long *tableauCols,
                          char ***tableauStrings, int **tableauMask,
                          long auxWidth, long *auxLen, double **aux,
  ...
  double *evt_tms = NULL, *time = NULL, *date = NULL,
         ...
         *dom_sigma = NULL, *dom_zcvol_star = NULL,

         *dom_dff_star = NULL, *dom_gam1_star = NULL, *dom_gam1_2_star = NULL,
  ...
  // TEST FXLGMSV
  // correlation = f3tensor(0        , nstp-1        , 0        , 6        , 0
  // , 6);

  if (!void_prm || !is_event || !date || !dom_sigma || !fx_sigma ||
      !correlation || !dom_zcvol_star || !for_zcvol1_star || !for_zcvol2_star ||
      !dom_dff_star || !dom_gam1_star || !dom_gam1_2_star || !for_sigma ||
      !for_alpha || !for_rho || !for_rho2 || !for_lameps || !for_lvleps ||
      !for_newalphaLGM || !for_newrhoLGM || !for_dff_star || !for_gam1_star ||
      !for_gam2_star || !for_gam1_2_star || !for_gam2_2_star ||
      !for_gam12_star) {
  ...
  for (i = nstp - 1; i >= 0; i--) {
    ...
    if (i < nstp - 1) {
      ...
      // J.M.L 10 Mar 2004
      // dom_zcvol_star represents the term (1 - exp(lbda1 * ( Tstar - t )) /
      // lbda1 which appears in the volatility of the bond B(t        ,Tstar).
      // We can either code it directly:

      // Previous code:
      // dom_zcvol_star[i+1] = (1.0 - exp(dom_model->dLambdaX *
      // (dom_model->dTStar - time[i]))) / dom_model->dLambdaX;
  ...
  *resRows = max(num_col + 1, num_evt);
  ...
  err =
      qtolgmsv2f_mc_balsam(nstp, num_evt, time, date,

                           numpaths,

                           dom_model->dLambdaX,

                           dom_sigma, dom_zcvol_star,
...
FREE_RETURN:
  ...
  // TEST FXLGMSV
  // if (correlation) free_f3tensor(correlation        , 0        , nstp-1 , 0
  // , 6        , 0        , 6);

  // Original Call
  if (correlation)
    free_f3tensor(correlation, 0, nstp - 1, 0, 4, 0, 4);
...
char *SrtGrfnQTOLGMSV1FMC(char *underlying, int numeventdates, long *eventdates,
                          long tableauRows, long *tableauCols,
                          char ***tableauStrings, int **tableauMask,
                          long auxWidth, long *auxLen, double **aux,
  ...
  double *evt_tms = NULL, *time = NULL, *date = NULL,
         ...
         *for_sigma = NULL, *for_alpha = NULL, *for_rho = NULL,
         *for_lameps = NULL, *for_lvleps = NULL, *for_newalphaLGM = NULL,
         *for_newrhoLGM = NULL,

         *for_zcvol_star = NULL,

         *for_dff_star = NULL, *for_gam1_star = NULL, *for_gam1_2_star = NULL;
  ...
  if (!void_prm || !is_event || !date || !dom_sigma || !fx_sigma ||
      !correlation || !dom_zcvol_star || !for_zcvol_star || !dom_dff_star ||
      !dom_gam1_star || !dom_gam1_2_star || !for_sigma || !for_alpha ||
      !for_rho || !for_lameps || !for_lvleps || !for_newalphaLGM ||
      !for_newrhoLGM || !for_dff_star || !for_gam1_star || !for_gam1_2_star) {
  ...
  for (i = nstp - 1; i >= 0; i--) {
    ...
    if (i < nstp - 1) {
      ...
      // Previous code:
      // for_zcvol_star[i+1] = (1.0 - exp(for_model->dLambdaX *
      // (for_model->dTStar - time[i]))) / for_model->dLambdaX;
  ...
  *resRows = max(num_col + 1, num_evt);
  ...
  err = qtolgmsv1f_mc_balsam(nstp, num_evt, time, date,
                             ...
                             for_sigma, for_alpha, for_lameps, for_lvleps,
                             for_rho,
...
FREE_RETURN:
  ...
  if (fx_sigma)
    free(fx_sigma);
  if (correlation)
    free_f3tensor(correlation, 0, nstp - 1, 0, 3, 0, 3);

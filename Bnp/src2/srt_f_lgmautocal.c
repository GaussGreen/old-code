...
For midatlantics:                      
 1. Find the LGM parameters by matching swaptions to market (BS) price     
    ...
    B. Swaptions fitted:                                       
    "Long" swaptions: Vanilla swaptions which start every six months in semiannual markets (USD)
    and every year in annual markets (Europe), beginning with the initial start
    date of the mid-atlantic, All these "long" reference swaptions end at the approximate end
    date of mid-atlantic                
		"Short" swaptions: Vanilla swaptions of one year duration starting every six months in
     semiannual markets (USD) and every year in annual markets (Europe), beginning with the
    initial start date of the mid-atlantic.
		Caplets fitted:	Caplets that start every three or six months depending on the market
	***If fix_tau_flag!=1, then the code fits the long swaption and, whenever possible, the short
	swaptions. It is very rare that a deal cannot have all short swaptions fitted exactly (4 out of
    150) as well as all the long swaptions.
	***If fix_tau_flag==1 and usecaps!=1, then only the long swaptions are fitted
	***If fix_tau_flag==1 and usecaps==1, then only the caplets are fitted
       Strikes of the long reference swaptions:
    The strikes are chosen so that the swaption has the same ratio of PV(fixed leg)/PV(floating leg)
    as the swap that would be recieved if the mid-atlantic was exercised at the starting date of 
    the reference swaption.
       Strikes for the short reference swaptions:
    The strike is the "effective interest rate" of the mid-atlantic for that
    year, including adjustments for exercise premiums, etc.
		Strikes for the caplets:
	The strikes are chosen like the short reference swaption strikes


    C. Numerical method used: Various global & local Newton routines for fitting. Whenever fit,
    swaption prices are matched to relative error ofless than 1.e-8.    

 2. Uses the fitted parameters and evaluates the midatlantic by convolution method. Numerical accuracy
    of the convolution is better than a relative error of 1.E-5. The midatlantic value is *LGMvalue.
 
 3. If the update_ts_flag is on, the code will determine the sigma and tau values which generate the
    integrals (zeta[j] and g[j] used by the code and put them in the market denoted by SrtCurvePtr.
    IMPORTANT NOTE:
    Any method using LGM calibration should NOT interpolate on sigmas and tau values. 
    This is an unstable procedure, and may lead to inaccurate prices (it hasn't yet, however.)
...
Payment rules implemented:
 For mid-atlantics (LGMautocal), the payment rule is assumed to be:
    ...
    If the deal is exercised at It_ex[j], which has a settlement ("as of") date It_start[j],
    then the first fixed leg payment is fixpymnt[i]-Iredpay[j] paid on tpay[i], and the
    other fixed leg payments are fixpymnt[i+1] on tpay[i+1], ..., fixpymnt[npay-1] on tpay[npay-1].
    Here coupon i is the first coupon whose pay date tpay[i] is strictly after the settlement date
	It_start[j].

    Note that the final payment should include the notional.

    The floating payments, fees, etc. are assumed to be equivalent to a payment of
    Iprem[j] paid on the settlement date It_start[j]. Note that this includes the notional,
    and should be equal to the notional apart from fees and adjustments.

    Any alterations in the payment schedule due to e.g. intraperiod starts should be accounted for
    in Iredpay[j] (reduction in first fixed coupon payment upon exercise j) and by adjusting
    the premium Iprem[j] before the code is called. Any alterations in the floating side due to,
    e.g. the floating pay date being several days after the end date of the floating period need
    to be accounted for in Iprem[j] before the code is called.

 For American swaptions THE NOTIONAL IS ASSUMED TO BE 1. The payment rules are:

    First possible exercise date is the latter of tfirst_ex and either today (calculation date) if
    end_of_day_flg is off or the next business day if end_day_flg is on.

    If the deal is exercised at T, the theoretical start (settlement or "as of") date S is
    lag_exer calendar or business days after T. The actual start date is obtained from the
    theoretical start date using the Business Day Rule conv_start. 

    Let i be the first fixed coupon whose end date tfix_end[i] is strictly after S. If early_flag_fix
    is off, it is assumed that the first fixed coupon will be reduced by the interest accrued
    between the coupon's start date tfix_start[i] and the settlement date S (so that the first
    payment corresponds to the interest for the period between S and tfix_end[i]). If
    early_fix_flag is on, it is assumed that the interest accrued between tfix_start[i] and S
    is added to the floating leg at S, effectively increasing the value of the floating leg, 
    and the entire fixed coupon is paid at i.

	The last fixed rate payment full_pay_fix[npay-1] should include the notional (which should be 1).

    On the floating side, suppose that t_flt[k] is the first floating pay date after the settlement
	date S, and let coupon i be the first fixed coupon whose pay date is strictly after S. The code
	assumes that all floating leg payments that are made AFTER t_flt[k] plus any exercise premiums are
	equivalent to a payment of Inprem[i] made at t_flt[k]. So like the mid-ats, Inprem[i] should include
	the notional plus (for receivers) or minus (for payers) any exercise premium.

    Unless the deal is exercised exactly at the end of a floating period, the code will adjust the value
	of the floating leg for the accrued intereston the floating leg. If the early_flag_flt is OFF, then
	the code assumes that the interest, rflt*CVG(S,t_flt[k]) that accrues between settlement and the next
	floating pay date t_flt[k] is paid on t_flt[k] in addition to Inprem[i]. If the early_flag_flt is ON
	then the code assumes that rflt*CVG(t_flt[k-1],S) the interest that accrues between t_flt[k-1] and S
	is paid at settlement (goes to reduce the floating leg's value) and that the full floating coupon 
	rflt*CVG(t_flt[k-1],t_flt[k]) is paid at t_flt[k] in addition to Inprem[i].

    In the above, rflt is the floating rate for the period from t_flt[k-1] to t_flt[k]. If 
	t_flt[k-1] <= tnow < t_flt[k], so that today is in the interval, then the code will use the input
	argument rflt_current for this already-fixed interest rate. If rflt_current is zero or negative, or
	if tnow does not fall in the interval, then the code will calculate rflt from the current discount
	curve.
	*/

/******** WARNINGS ***********
1) The payment rules must be repected when entering deals with weird frequencies, long payment or start
   lags, etc.
2) If the BS vols are bumped one at a time for vega calculations, best results are obtained if the vols
   are bumped by a twentieth to a fiftieth of a per cent or less. If the BS vols are all bumped
   simultaneously, bumping by a per cent is easily OK.
3) The code hasn't been tested as rigorously for "European style" markets in which swaps have annual
   coupons instead of semi-annual
*******************************/
...
static String mid_atl_fit_market(String (*GetVol)(), SrtDiffusionType srt_vol_type, long n, 
		long nlast, int icmpnd, SrtCompounding compounding, SrtBasisCode basis, 
		int spot_lag, SrtLgmRefSwptnData *lgmRefSwptnData) ;

static String mid_atl_fit_market_fix_tau(String (*GetVol)(), SrtDiffusionType srt_vol_type, long n, 
		long nlast, int icmpnd, SrtCompounding compounding, SrtBasisCode basis,
		int usecaps, int spot_lag, SrtLgmRefSwptnData *lgmRefSwptnData) ;
...
static String pvcashflow(Date tnow, String ycName, long nex, Date t_ex[], Date t_start[],
              long npay, Date tfix_start[], Date tfix_end[], Date tpay[], double fixpymnt[],  
...
static String Olivier_lg(long n, long nlast, int icmpnd, long nex, long npay,
              long ifirst[], Date t_start[], double pvpay[], double redpay[]) ;

static String Olivier_sh(long n, long nlast, long nex, long npay, long ifirst[],
              Date t_start[], double pvpay[], double redpay[], int icmpnd, int fix_tau_flag) ;
...
static String Findexerbdry(Date tnow, long nex, Date t_ex[], Date tend,
                           ...
                           int natural_spot_lag, SrtLgmExerBdryData *lgmExerBdryData) ;
...
static double Vbs_jn(double strike, double vol, SrtDiffusionType srt_vol_type, int spot_lag) ;
...
static String R_restrictions(String (*GetVol)(), SrtDiffusionType srt_vol_type, long n, 
							 long nlast, int icmpnd, SrtCompounding compounding,
							 SrtBasisCode basis, int fix_tau_flag, int usecaps) ;
...
static String mid_atl_eval(Date tnow, long n, SrtCurvePtr crv, long nex,       
                           Date t_ex[], Date t_start[], long npay,      
                           Date tpay[], double pvpay[], double redpay[],
                           long ifirst[], double *LGMvalue,
                           double phi_cr[]) ;   

static double iterp_integrand(double exk, double rik, long ik, double Q1[], long nphi,
                              double v_cr, double exk_cr, double rik_cr, long di) ;

static void interpolate_par(Date tnow, long n,
            long nex, Date t_ex[], double zeta_ex[],
            Date t_start[], double G_start[], 
...
static double EXPAY(double phi, long npay, long i0, double pvpay[],
                    double redpayj, double strike, double G_pay[],
                    double Gst, double zetaj) ;

static void init_point(double *x, int *check, double target, long i0,
                       long npay, double pvpay[], double redpayj,
                       double G_pay[], double Gst, double zetaj) ;

static void ScNewt(double *x, int *check, double target,
                   double (*func1)(), double (*deriv1)()) ;

static void ScSlowNewt(double *x, double xmin, double xmax,
                       int *check, double target, int gzfind,
                       double (*func1)(),
                       double (*deriv1)() ) ; 

static void FreeMost(void*, void*, void*, void*, void*,
                     void*) ;

static void FreeMore(void*, void*, void*, void*, void*,
                     void*, void*, void*, void*, void* ) ;

static void FreeLots(void*, void*, void*, void*, void*,
                     void*, void*, void*, void*) ;

static void FreeRest(void*, void*) ;

static Err get_ccy_defaults(SrtCurvePtr crvptr, int fix_tau_flag, int usecaps,
							BasisCode *basis, SrtCompounding *compnd, BusDayConv *conv,
							int *spot_lag) ;
                                                                   /* S. R. may-07-96 */
/* MACROS */

#define ERROR_CHECK   if (error != NULL)                          \
                      {   FreeMost(t_ex, t_start, ifirst, pvpay,  \
                           redpay, phi_cr) ;                      \
                          return(error) ;                         \
                      }                                          

#define ERROR_CHECK1  if (error != NULL)                    \
                      {   FreeMore(zeta_ex, G_start, G_pay, \
                              wgt, wgt2, Q0, Q1,            \
                              eff_pay, postfac, effred) ;   \
                         return(error) ;                    \
                      }                                     

#define ERROR_CHECK2  if (error != NULL)                      \
                      {   FreeLots(tnatst, paydate, zeta_ex,  \
                               G_pay, G_st, B_st,             \
                               aB_pay, aB_first, ifirstpay);  \
...
#define MAX_NUM_DATES    500    /* allow only midatlantics with fewer
                                   than 241 exer. and pay dates       */

#define MIN_EX_INT       10     /* ignore exercise dates closer than
                                    10 days apart                     */

#define MAX_REF_DATES    500    /* allow only midatlantics shorter than
                                   20 years from first exercise to end
                                   date                               */

#define FAR_OUT          2.5    /* strikes of swaptions used to match 
                                   market limited to FAR_OUT std dev.s
                                   from par, regardless of mid-atlantic
                                   strike                             */

#define WIDTH         3        /* integration domain runs from - WIDTH
                                   to + WIDTH std deviations in
                                   mid-atlantic evaluator             */

#define NDIV        360         /* number of points used to discretize   
                                   mid-atlantic. Should be divisible by 20 */

#define NQMAX        48         /* number of points used in Gaussian
                                   kernel                             */

#define STENCIL       4        /* kernel runs from -STENCIL to +STENCIL
                                   std deviations                     */
...
static double    sy[MAX_REF_DATES]; /* time from tnow (in years) to dates of ref. swptns  */
static double    B[MAX_REF_DATES];       /* discount factors from tnow to s[j]            */
static double    aB[MAX_REF_DATES];      /* products of discount factors and coverages    */
static double    Ract_sh[MAX_REF_DATES]; /* fixed rates used for short ref. swaptions     */
static double    Ract_lg[MAX_REF_DATES]; /* fixed rates used for long ref. swaptions      */
...
static double    prem[MAX_NUM_DATES];     /* premiums [j] paid if mid_atl exercised at sj */
static double    origprem[MAX_NUM_DATES]; /* premiums [j] paid if mid_atl exercised at sj */
static int       pr;                     /* 1 for payer, 0 for reciever                   */
static double    v_bs_sh[MAX_REF_DATES]; /* BS prices for short reference swaptions       */
static double    v_bs_lg[MAX_REF_DATES]; /* BS prices for long reference swaptions        */

/* model parameters */
static double   g[MAX_REF_DATES];    /* g[j] is integral of exp(-lambda*t) from tj to tn   */
static double   zeta[MAX_REF_DATES]; /* zeta[j] is integral from zero to tj of sigma stuff */
...
/* Bug Fix A (BFA): Slightly changes the value of the floating leg if we are currently in a
floating period in which the interest rate has already been set. The current rate is a new
input parameter rflt_current; the program ignores this input if it is zero or negative.
To implement, need to uncomment and delete some lines */

/* Bug Fix B (BFB): Correctly accounts for the accrual of the floating rate branch. Implemented */
...
/* initialize */
    ...
    error = NULL;
    full_pay_fix[nfix-1] = full_pay_fix[nfix-1] - 1; /* rem. notional from fixed leg */
...
/* get earliest exercise date t0_ex and earliest start (as of) date t0_start */
    ...
    t0_ex=tnow;
    if (end_of_day==1)
        t0_ex = add_unit(tnow,1,SRT_DAY,SUCCEEDING);	   /* PH 2/28/98 trivial bug fix */
...
/* check inputs */
	...
	if ( (nfix<1) || (nfix>MAX_NUM_DATES) || (nflt<1) || (nflt>MAX_NUM_DATES) )
    ...
    for (i=0; i<(nfix-1)                      \
         && tfix_start[i] < tfix_end[i]      \
         && tfix_start[i] < tfix_start[i+1]  \
         && tfix_end[i] < tfix_end[i+1]      \
         && tfix_pay[i] < tfix_pay[i+1]; i++)
        ;
...
/* subdivide fixed pay periods into mdiv subintervals */
        ...
        {   t0 = tfix_start[i];
            ...
            for (k=0; k<mdiv; k++)
            {   j = i*mdiv+k;
                It_start[j] = (Date)(t0 + ((double)(t1-t0))
                                      *((double)k)/((double)mdiv) + 0.5);
                if (cal_or_bus == 0)
                    It_ex[j] = add_unit(It_start[j]-lag_exer_start+1, -1, SRT_DAY, conv_start);
                else
                    It_ex[j] = add_unit(It_start[j], -lag_exer_start, SRT_BDAY, conv_start);
...
/* exercise dates are now It_ex[j0,j0+1,...,In_ex-1]
   ...
   compute premiums and reduction in first fixed coupon payment */
        ...
        i=0;
        for (j=j0; j<In_ex; j++)
        {   while (i<nfix && tfix_pay[i]<=It_start[j]) i++;                   
                                                 /* "as of" date for exer. j occurs in period i */
            Iprem[j] = Inprem[i];                /* floating branch + fee                       */
            if (tfix_start[i] < It_start[j])
            {   accrual = full_pay_fix[i]*coverage(tfix_start[i], It_start[j], srt_fix_basis)
                                              /coverage(tfix_start[i], tfix_end[i], srt_fix_basis);
				accrual = ((accrual<full_pay_fix[i])?accrual:full_pay_fix[i]); }    /* PH 2/28/98 potential bug fix */
            else
                accrual = 0.;
            if (early_flag_fix == 0)
                Iredpay[j] = accrual;  /* reduce first payment by accrued fixed interest */
            else
...
/* BFB: correct the floating leg to properly account for accruals */
  /* Get floating rate */			  
			if (t_flt[i-1]>=tnow)	  /* rate has not been fixed; estimate from df's */
...
/* Adjust floating leg */
			...
			if (early_flag_flt == 0)
			    Iprem[j] = Iprem[j] - 1. + (1.+ rflt*coverage(t0,t_flt[i],srt_float_basis))
												*swp_f_df(It_start[j],t_flt[i],yc_name);
			else
				/* Premium = (Premium minus the complete floating leg) - accrued interest + *
				/* (aggregation of following floating legs + first floating leg) */
				/* We do premium minus the complete floating leg because we insert a more */
				/* accurate floating leg */
				Iprem[j] = (Iprem[j]-1.) - rflt*coverage(t_flt[i-1],t0,srt_float_basis)
...
/* feed autocal */
        ...
        i = 0;
        while (i<nfix && tfix_pay[i] <= It_start[j0])		/* PH 2/18/98 potential bug fix */
...
/* evaluate mid-atlantic */
        ...
        error = LGMautocal(
			...
			lgmExerBdryData,
			/*
			Not interested in LGM reference swaption data, tau/sigma data or
			intrinsic value
			*/
			NULL,
			NULL,
...
/* extrapolate to get value of the american */ 
    if (countmax==1)
        ...
        *LGMamervalue = (delta_tex[1]*delta_tex[1]*LGMvalue[0] - delta_tex[0]*delta_tex[0]*LGMvalue[1])
...
String LGMautocal(
		...
		SrtLgmExerBdryData *lgmExerBdryData, /* ptr to exercise boundary data structure (NULL => not req'd) */
		SrtLgmRefSwptnData *lgmRefSwptnData, /* ptr to reference swaption data structure (NULL => not req'd) */
		SrtLgmTSData   *lgmTSData,          /* ptr to tau/sigma data (NULL => not req'd) */
		double   *intrinsicValue,     /* ptr to intrinsic value (NULL => not req'd) */
		int       skip_deal_evaluation)  /* 0 for a noraml valuation, 1 for a simple volatility points extraction */
{
/* Declarations */
		...
		long	num_months;		/* delta_t for term structure. Depends on calibration instruments:
								/* 6 for semiannual swaptions, 12 for annual swaptions */ 
		...
		double	smoothing;		/* strike smoothing parameter; 0.0=no smoothing; 1.0=full smoothing */
...
/* sets natural_compnd to SRT_SEMIANNUAL or SRT_ANNUAL, sets natural_basis;
   sets natural_conv, and sets natural_spot_lag  */
	...
	error_str = get_ccy_defaults(               /* S.R. may-07-96 */
	...
	if ((usecaps!=1) || (fix_tau_flag!=1))			  /* PH 2/18/98 ModC */
	{
		...
		{	icmpnd=1;		   /*PH 2/18/98 */
			num_months=6;}	   /* parameter values when calibrating to semiannual swaptions */
		else
			;
	}
 	...
 	{
		icmpnd=0;			   /*PH 2/18/98 */
		num_months=6;		   /* parameter values when calibrating to semiannual caplets */

		if (natural_compnd == SRT_QUARTERLY)
			num_months=3;	   /* parameter values when calibrating to quarterly caplets */
		else
			;
	}
...
/* set default tau */		  
	...
	smoothing = 1.0;     /* strike smoothing parameter: 0.0 = no smoothing, 1.0 = full smoothing */
...
/* Check end of day: if true, move to next business day */
    if (end_of_day == 1)
		tfirst = add_unit(tnow,1,SRT_DAY,SUCCEEDING);	   /* PH 2/28/98 trivial bug fix */
...
/* Increment i until finds dates in the wrong order: error returned below */
    for (i=0; i<(npay-1) && tpay[i]<=tpay[i+1]   
         && tfix_start[i]<tfix_start[i+1]       
         && tfix_end[i]<tfix_end[i+1]           
         && tfix_start[i]<tfix_end[i]           
         && tfix_start[i]<tpay[i] ; i++) ;
...
/* Removes all fixed coupons (+ associated dates) before tfirst(today+eofday)*/
...
/* -------------------------*/    
    t_ex=NULL; t_start=NULL; ifirst=NULL;
    pvpay=NULL; redpay=NULL; phi_cr=NULL;

    t_ex = (Date*)    srt_calloc(nex+1, sizeof(Date));      /* t_ex[0,1,...,nex]   */
    t_start = (Date*) srt_calloc(nex+1, sizeof(Date));      /* t_start[0,..,nex]   */
    ifirst = (long*) srt_calloc(nex+1, sizeof(long));       /* ifirst[0,..,nex]    */
    pvpay = (double*) srt_calloc(npay, sizeof(double));     /* pvpay[0,...,npay-1] */
    redpay =  (double*) srt_calloc(nex+1, sizeof(double));  /* redpay[0,...,nex]   */       
    phi_cr = (double*) srt_calloc(nex+1, sizeof(double));   /* phi_cr[0,...,nex]   */

    if (t_ex==NULL || t_start==NULL || ifirst==NULL
       || pvpay==NULL || redpay==NULL || phi_cr==NULL)
...
/* ------------------------------- */    
/* ... omit exercise dates closer than MIN_EX_INT days apart, and omit exercise
       dates before today ... */
    nex = -1;
    nextdate = tfirst;
    for (j=0; j<In_ex; j++)
...
/* eliminate any exercise dates that have no underlying left */
    ...
    for (;nex>=0 && t_start[nex]>(tpay[npay-1]-10); nex--) ; /* PH 2/28/98 trivial bug fix */
    if (nex < 0)
...
/* This is the deal that will be evaluated:
	...
	if exercised at t_ex[j], then fixed leg will receive the payments
		...
		fixpymnt[i] paid on tpay[i] for i=ifirst[j]+1, ifirst[j]+2, ..., npay-1
	the floating leg will receive a single payment of origprem[j] paid on  t_start[j]
...
/* ------------------------------------- */    
/* STEP FOUR: Choose reference swaptions */	   /* This next part has been simplified */
/* ------------------------------------- */    
/* choose dates s[0, ..., n] for reference swaptions */

    choose_dates(tnow, t_start[0], &n, npay, tpay, nex, t_ex, icmpnd, num_months, natural_conv);
	nlast = ((icmpnd==1) ? n-2:n-1);
        
    for (j=0; j<=n; j++)                
    {   sy[j] = coverage(tnow,s[j],BASIS_ACT_365);   /* sy[0,..,n] years from tnow    */
        B[j] = swp_f_df(tnow,s[j],yc_name); }       /* B[0,..,n] today's dis.factors */
    aB[0] = 0.;
    for (j=1; j<=n; j++)
        aB[j] = B[j]*coverage(s[j-1], s[j], natural_basis);   /* discounted coverages */
...
/* Reference instruments:
	If fix_tau_flag != 1, code uses swaptions with start dates are s[j], j=0,1,...,nlast.
   End dates for short swaptions are s[j+1] (annual markets) or s[j+2] (semiannual markets).
   End dates for long swaptions are s[n].
   Pay dates are s[i], i=j+1,...,n. Exercise dates precede s[j] by natural_spot_lag.
	If fix_tau_flag == 1, and usecaps !=1 code uses long swaptions but not the short ones 
	If fix_tau_flag == 1 and usecaps ==1, code uses caps with start dates s[j] and end dates
			s[j+1], j=0,1,...,nlast*/
...
/* Find the minimum & maximum strikes allowed
     ...
     if ((fix_tau_flag!=1) || (usecaps!=1))
		error = Olivier_lg(n, nlast, icmpnd, nex, npay, ifirst, t_start, pvpay, redpay);
     ERROR_CHECK;
     
     if ((fix_tau_flag!=1) || (usecaps==1))
		error = Olivier_sh(n, nlast, nex, npay, ifirst, t_start, pvpay, redpay, icmpnd, fix_tau_flag);
     ERROR_CHECK;

/* To simplify the coding, when we don't need both long and short rates, we copy the needed
		  one into the unneeded one */
	 if ((fix_tau_flag==1) && (usecaps!=1))
	 {	for (j=0; j<=nlast; j++)
...
/*   Fit zeta[t] and g[t] so that the LGM value of the reference 
   swaptions or caplets match market value */
	 if (fix_tau_flag!=1)
		error = mid_atl_fit_market(GetVol, srt_vol_type, n, nlast,
									icmpnd, natural_compnd, natural_basis,
									natural_spot_lag, lgmRefSwptnData);
     else
		error = mid_atl_fit_market_fix_tau(GetVol, srt_vol_type, n, nlast,
									 icmpnd, natural_compnd, natural_basis,
									 usecaps, natural_spot_lag, lgmRefSwptnData);
	 ERROR_CHECK;
...
/* ------------ */
    ...
    {   error = Findexerbdry(tnow, nex, t_ex, tpay[npay-1], phi_cr, n, yc_name, icmpnd,
                             natural_basis, natural_conv, natural_spot_lag,
							 lgmExerBdryData);
        if (error != NULL)
...
/* fit g[j] and zeta[j] by matching LGM values of vanilla swaptions to
   market (BS) values. Swaptions used:
      ...
      start s[j], end one year later, strike Ract_sh[j] (fit whenever possible)        
              for j = 0,1, ...,nlast
   Uses global Newton routine (semiannual or annual) for fit                */

static String mid_atl_fit_market(String (*GetVol)(), SrtDiffusionType srt_vol_type,
								 long n, long nlast,
                                 int icmpnd, SrtCompounding compounding,
								 SrtBasisCode basis,
								 int spot_lag, SrtLgmRefSwptnData *lgmRefSwptnData)
{
   ...
   error = NULL;
	...
	if (lgmRefSwptnData)
	{
		...
		if (!lgmRefSwptnData->refShortSwptnArr)
			lgmRefSwptnData->refShortSwptnArr = srt_calloc(nlast+1, sizeof(SrtLgmRefSwptn));
		if (!lgmRefSwptnData->refLongSwptnArr)
			lgmRefSwptnData->refLongSwptnArr = srt_calloc(nlast+1, sizeof(SrtLgmRefSwptn));
	}
...
/* Set g[nlast], ..., g[n] to default values  */
    phistsh = 0.; phistlg = 0.;
    g[n]=0.;
    g[n-1] = overall_tau*(exp(-sy[n-1]/overall_tau) - exp(-sy[n]/overall_tau) );  /* wolog ! */
    if (icmpnd != 0)
        g[n-2] = overall_tau*(exp(-sy[n-2]/overall_tau) - exp(-sy[n]/overall_tau) );

/* Find zeta[nlast] by matching market price */       
    sqzmax = 2.*sig*sqrt(sy[nlast])/g[nlast];         /* fit LGM value computed by Vjn  */
    sqznew = sqzmax*(1.-B[n]/B[nlast])/2.;            /* to BS value in v_bs_sh[nlast]  */
    sqzmin = .2*sqznew;                      
    Ract = Ract_sh[nlast];
    jst = nlast; nst = n;
    phiact = phistsh;
...
/* set maximum zeta[j] and minimum g[j] values */
        g[j] = g[j+1] + 0.05*(g[j+1]-g[n])*(sy[j+1]-sy[j])/(sy[n]-sy[j+1]);
        zeta[j] = zeta[j+1];
...
/* fit long swaption by changing either zeta or g, keeping the other at its extreme value */ 
       ...
       {   gmin = g[j];           
           gnew = g[j+1] + (g[j+1]-g[n])*(sy[j+1]-sy[j])/(sy[n]-sy[j+1]);
           gmax = 2.0*gnew;
...
/* if the LGM value of the short swaption is less  than the BS value, then 
   a unique pair zeta[j] and  g[j] exist which fit LGM values exactly ... else
   we cannot do better than the bdry values we already have */ 
...
/* fit zeta[j] by matching LGM values of vanilla long swaptions (if usecaps!=1)
	...
	Reference swaptions:
      start s[j], end s[n], strike Ract_lg[j]	 for j = 0,1, ...,nlast
	Reference caplets
      start s[j], end s[j+1], strike Ract_sh[j]  for j = 0,1, ...,nlast        
   Uses global Newton routine (semiannual or annual) for fit                */

static String mid_atl_fit_market_fix_tau(
								String (*GetVol)(), SrtDiffusionType srt_vol_type,
								long n, long nlast, int icmpnd,
								SrtCompounding compounding, SrtBasisCode basis,
								int usecaps, int spot_lag,
...
/* Define g[0,1,...,n] from tau  */
    g[n]=0.;
    for (j=0;j<n;j++)
		g[j] = overall_tau*(exp(-sy[j]/overall_tau) - exp(-sy[n]/overall_tau));
...
/* Find zeta[nlast] by matching market price */       
    ...
    phiact = 0.;
    ScSlowNewt(&sqznew, sqzmin, sqzmax,			/* Find zeta to match last swaption or cap */
...
/* the array zeta[j] has actually been fitted at the exercise dates of the reference swaptions
   instead of s[j] ... use linear interpolation to correct zeta[j] for spot_lag */
...
/* set extreme values & initial guess for g[j] */
    ...
    if (gnew < gmin)
        gnew = gmin;
    g[j] = g[j+1] + (g[j+1]-g[n])*(sy[j+1]-sy[j])/(sy[n]-sy[j+1]);
    if (g[j] > gnew)
        gnew = g[j];
    g[j] = gnew;
    gsave = gnew;
...
/* new Newton step */
        ...
        {   sqznew = sqzold - v2*v21*sqzold*sqzold
...
/***********************************************************************/ 
static String pvcashflow(Date tnow, String ycName,                              
    long nex, Date t_ex[], Date t_start[],   /* exercise info */
    long npay, Date tfix_start[], Date tfix_end[], Date tpay[], 
	double fixpymnt[], 	/* payment info  */
    long ifirst[], double pvpay[], double redpay[])
                                /* output        */
...
static void choose_dates(Date tnow, Date tstart, long *nptr,
                              long npay, Date tpay[],
							  long nex, Date t_ex[],
							  int icmpnd, long num_months, BusDayConv natural_conv)
{
...
/*	num_fudge = ((num_months>6)	? 2:1);
    ...
    {   for (j=0; j<=nmin; j++)
            s[j] = add_unit(tnow+20, ((int) (num_months*j)),SRT_MONTH, natural_conv);
        n = nmin;
    }
    else
    {
...
/*        tmaybe = add_unit(tnow, num_months, SRT_MONTH, natural_conv) + 14;   
        if (tmin < tmaybe)	*/	  /* Avoids calib. on instrmnts with less than 14 days to exer */
/*           tmin = tmaybe;
        ...
        while (tpaymin >= tmin)
        {   n++;
            tpaymin = add_unit(tpay[npay-1], -((int)(num_months*n)), SRT_MONTH, natural_conv);
...
/*     
        ...
        for (j=0; j<=nmax; j++)
            s[j] = add_unit(tpay[npay-1], -((int)(num_months*(n-j))), SRT_MONTH, natural_conv);
        n = nmax; 
*/
	i=0;
	n = 0;
	...
	while (((s[n]<tpay[npay-1]) || (n < nmin)) && (n < MAX_REF_DATES))
	{
		...
		while (i<(npay-1) && tpay[i]<=s[n]-num_fudge_minus)		/* PH 2/20/98 trivial change */
			i++;
		if ((tpay[i]>=s[n]-num_fudge_minus) && (tpay[i]<=s[n]+num_fudge_plus))
			s[n]=tpay[i];
	}
...
/*    if (n<nmin)
    {   for (j=n+1; j<=nmin && j < MAX_REF_DATES; j++)
            s[j] = add_unit(t_ex[start_ex], num_months*n, SRT_MONTH, natural_conv);   
        n=nmin;
    }
*/
...
static void smooth_strikes(long nlast, double smth)
{
	...
	if ((smth<0) || (smth>1)) smth = 1.; /* smth=1 is full smoothing; smth=0 is no smoothing */
...
/* PH 2/18/98 If nlast < 1 (one period), there is no smoothing to be done */
    ...
    {   rlast_lg = (.75*R_theor_lg[1] + .25*R_theor_lg[0])*smth + (1.-smth)*R_theor_lg[1];    
        rlast_sh = (.75*R_theor_sh[1] + .25*R_theor_sh[0])*smth + (1.-smth)*R_theor_sh[1];
		R_theor_lg[0] = (.75*R_theor_lg[0] + .25*R_theor_lg[1])*smth + (1.-smth)*R_theor_lg[0];
		R_theor_sh[0] = (.75*R_theor_sh[0] + .25*R_theor_sh[1])*smth + (1.-smth)*R_theor_sh[0];
		...
		;
/* Main case: nlast>1 */
    rlast_lg = (.75*R_theor_lg[nlast] + .50*R_theor_lg[nlast-1] - .25*R_theor_lg[nlast-2])*smth
					+ (1.-smth)*R_theor_lg[nlast];
    rlast_sh = (.75*R_theor_sh[nlast] + .50*R_theor_sh[nlast-1] - .25*R_theor_sh[nlast-2])*smth
    ...
    for (j=0; j<=(nlast-1); j++)
    {   rtemp = R_theor_sh[j];
        R_theor_sh[j] = smth*0.25*(rtempsh0+2.0*R_theor_sh[j]+R_theor_sh[j+1]) + (1.-smth)*R_theor_sh[j];
        rtempsh0 = rtemp;   

        rtemp = R_theor_lg[j];
        R_theor_lg[j] = smth*0.25*(rtemplg0+2.0*R_theor_lg[j]+R_theor_lg[j+1]) + (1.-smth)*R_theor_lg[j];
        rtemplg0 = rtemp;
    }
   ...
   interest rate of the long swaption so that it has the same ratio  */
    ...
    {   sum = sum + aB[j+1];
        ...
        if (i==nex)
            ...
            rat = (ratio[i]*((double)(t_start[i+1]-s[j])) + ratio[i+1]*((double)(s[j]-t_start[i]))) 
...
/* eliminates slight arbitrage for in the money deals 
	when the paydates extend more than a couple years past the exercise dates */
...
/* interpolate on ratio[j] to find its value at s[j] and s[j+delay], and choose the
   interest rate of the short swaption so that it is equivalent to buying a long swaption at
   s[j] with the first ratio and selling one at s[j+delay] at the second ratio  */
    ...
    {   while (i1>0 && t_start[i1]>s[j])
        ...
        if (i1==nex)
        {   rat1 = ratio[nex];
            prem1 = origprem[nex]; }                    /* interpolation */
        ...
        if (i2==nex)
        {   rat2 = ratio[nex];
            prem2 = origprem[nex]; }                            /* interpolation */
...
/* compute strike of swaption */
        ...
        for (i=(j+1); i<=jend; i++)
            sum = sum + aB[i];
        R_theor_sh[j] = (rat1*B[j] - B[jlast]*(1.0 + prem2*(rat2-1.0)/prem1))/sum ;
...
/*	PH 2/18/98 this doesn't seem to be necessary */
	...
	if (fix_tau_flag != 1)
	{
		...
		{   rat= ((s[delay]-t_start[nex])*R_theor_lg[0]   /* feather shorts if more exer. dates */
				+ (t_start[nex]-s[0])*R_theor_sh[0])/(s[delay]-s[0]);  
		}
	}
	else
	{
...
static double Vjn1_der(double par, int whichpar)
{
    ...
    sum = (B[nst]+Ract*aB[nst])*(g[jst+1]-g[nst])*exp(-tmp*tmp/2.);
    for (i=jst+1; i<nst; i++) 
    {   tmp = phiact/sqz-g[i]*sqz;
...
static double Rpar_jn(double ph)
{
    ...
    ans = (B[jst]*exp(g[jst]*(ph-.5*g[jst]*zeta[jst]))     \
         - B[nst]*exp(g[nst]*(ph-.5*g[nst]*zeta[jst])))/sum;
...
static double Vbs_jn(double r, double sig, SrtDiffusionType srt_vol_type, int spot_lag)
{
...
/* Lognormal Black-Scholes */
    if (srt_vol_type == SRT_LOGNORMAL)
	{
    	...
    	ans = (B[jst]-B[nst])*((r/rswp)*norm_accurate(chi) - norm_accurate(chi-var));
    }
	else
/* Normal Black-Scholes */
	...
	{
		chi = ( r - rswp ) /var ;
    	ans = (B[jst]-B[nst]) / rswp * var * (chi*norm_accurate(chi) + gauss(chi));
	}
	if (pr==1)
...
static double pay_payer(double ph)
{
    ...
    for (i=jst+1; i<nst; i++)
        sum = sum +     
              aB[i]*exp(-(g[jst]-g[i])*(ph-.5*(g[i]+g[jst])*zeta[jst]));

    ans = sum*Ract - B[jst] + (B[nst]+Ract*aB[nst])*    
...
static double pay_payer_der(double ph)
{
    ...
    sum = 0.;
    for (i=jst+1; i<nst; i++)
        sum = sum - aB[i]*(g[jst]-g[i])    
                * exp(-(g[jst]-g[i])*(ph-.5*(g[i]+g[jst])*zeta[jst]));

    ans = sum*Ract - (B[nst]+Ract*aB[nst])*(g[jst]-g[nst])    
                * exp(-(g[jst]-g[nst])*(ph-.5*(g[jst]+g[nst])*zeta[jst]));
							 ...
							 int fix_tau_flag, int usecaps)
{
    ...
    for (j=0; j<=nlast; j++)
    {	
		if ((fix_tau_flag!=1) || (usecaps==1))  /*get par swap rate for short swaption */
		{	jst=j; nst=j+dj;               
			rbs=Rbs_par_jn();
	/* PH: 2/18/98 Will this work for caplets? If so, uncomment 
			if (err = srt_f_setSwapDP(s[j], s[j+dj], compounding, basis, &sdp))
				return err;
		
			err = swp_f_ForwardRate_SwapDP( &sdp, s_yldcrv, "CASH", &fwd);
			if (err)	
				return err;
	*/
     /*   error = (*GetVol)(s[j],s[j+dj]-s[j],fwd,&sig); */  /* Get BS volatility */
...
/* extreme strikes for short swaptions */
			if (srt_vol_type == SRT_LOGNORMAL)
			{
				Rmin_sh[j] = rbs*exp(-sig*sqrt(sy[j])*FAR_OUT-.5*sig*sig*sy[j]);
				Rmax_sh[j] = rbs*exp(+sig*sqrt(sy[j])*FAR_OUT-.5*sig*sig*sy[j]);
			}
			else
			{
		...
		if ((fix_tau_flag!=1) || (usecaps!=1))
        {	jst=j; nst=n;               /*get par swap rate for long swaption */
			rbs=Rbs_par_jn();
...
/* extreme strikes for long swaptions */		
			if (srt_vol_type == SRT_LOGNORMAL)
			{
				Rmin_lg[j] = rbs*exp(-sig*sqrt(sy[j])*FAR_OUT-.5*sig*sig*sy[j]);
				Rmax_lg[j] = rbs*exp(+sig*sqrt(sy[j])*FAR_OUT-.5*sig*sig*sy[j]);
			}
			else
			{
...
/* exercise info */
         long     nex,       
         Date     t_ex[],     /* exercise dates t_ex[0,...,nex]   */
...
/*Initialize */
    error = NULL;
    nq = NQMAX; nphi = NDIV;
    nq = (nq+3)/4; nq2 = 2*nq; nq = 4*nq;
    nphi = (nphi+3)/4; nphi = 4*nphi;
	Vs1 = -33.3333e+257;
	Vs2 = +33.3333e+257;


/*STEP ONE: Allocate space */
    wgt=NULL; wgt2 =NULL; Q0=NULL; Q1=NULL;
    zeta_ex=NULL; G_start=NULL; G_pay=NULL;
    eff_pay=NULL; postfac=NULL; effred=NULL;

    zeta_ex = (double*) srt_calloc(nex+1,sizeof(double));  /* zeta_ex[0,...,nex]   */
    G_start = (double*) srt_calloc(nex+1,sizeof(double));  /* G_start[0,...,nex]   */
    G_pay = (double*) srt_calloc(npay, sizeof(double));    /* G_pay[0,...,npay-1]  */
    wgt = (double*) srt_calloc(nq+1, sizeof(double));      /* wgt[0,...,nq]        */
    wgt2 = (double*) srt_calloc(nq2+1, sizeof(double));    /* wgt[0,...,nq2]       */
    Q0 = (double*) srt_calloc(nphi+1, sizeof(double));     /* Q0[0,...,nphi]       */
    Q1 = (double*) srt_calloc(nphi+1, sizeof(double));     /* Q1[0,...,nphi]       */
...
/* Interpolate G and zeta values at needed dates */
    ...
    interpolate_par(tnow, n, nex, t_ex, zeta_ex, t_start,
                              G_start, npay, tpay, G_pay);
...
/* Find initial strike to center grid on */
    j=nex;
    init_point(&phi, &check, prem[j], ifirst[j], npay, pvpay, 
               redpay[j], G_pay, G_start[j], zeta_ex[j]);
    if (check != 0)
...
/* Setup payoff matrix (trade space for time!) */
    ...
    {   
        ...
        for (j=0; j<=nex; j++)
        {   i = ifirst[j];
            effred[k0+j] = redpay[j]*exp(G_pay[i]*(phi-.5*G_pay[i]*zeta_ex[j]));
		}
    }
...
/* compute payoff */
			...
			vact = vact - prem[j]*exp(G_start[j]*(phi-.5*G_start[j]*zeta_ex[j]));
            if (pr!=0)
                vact = -vact;
...
/* find exercise boundary */                                                    
        ...
        {   if (j<nex) 
            ...
            exk_cr = ((double)klast) + (Q0[klast]-Vs1)/(Q0[klast+1]-Vs2+Q0[klast]-Vs1);  
        else
            exk_cr = ((double)klast) - (Q0[klast]-Vs1)/(Q0[klast-1]-Vs2+Q0[klast]-Vs1);  
...
/* Find value of payoff and k where the break occurs */
        ...
        v_cr = EXPAY(phi_cr[j], npay, ifirst[j], pvpay, redpay[j],
                     prem[j], G_pay, G_start[j], zeta_ex[j]);
        ik_cr = DTOL(exk_cr);
        rik_cr = ik_cr;
...
/* convolve with discretization 2dq and 2dphi */
		...
		fine_pv = 0.0;
            ...
            {   
                ...
                ik =2*(DTOL(exk/2.));
                rik = ik;
                alfa = iterp_integrand(exk, rik, ik, Q1, nphi, v_cr, exk_cr, rik2_cr, 2) ;

				coarse_pv = coarse_pv + wgt2[i]*alfa;
                /* Q0[k] = Q0[k] -  wgt2[i]*alfa/3.0; */  /* sum is -1/3 of the integral evaluated */
                                                    /*   on the coarse grid                  */

                if (i==ii_cr)                       /* correction for break in slope */
					coarse_pv = coarse_pv + beta*2.*dq*(v_cr-alfa)*(1. + ri_cr - exi_cr);
				/*    Q0[k] = Q0[k] - beta*2.*dq*(v_cr-alfa)*(1. + ri_cr - exi_cr)/3.; */
                if (i==(ii_cr+1))
					coarse_pv = coarse_pv + beta*2.*dq*(v_cr-alfa)*(exi_cr - ri_cr);
...
/* convolve with discretization dq and dphi */
            ...
            for (i=0; i<=nq; i++)
            {
				...
				/*Q0[k] = Q0[k] + 4.0* wgt[i]*alfa/3.0 ;* /* adds 4/3 the integral evaluated on  */
                                                       /* the fine grid                       */

                if (i==ii_cr)                          /* correction for break in slope */
					fine_pv  = fine_pv + beta*dq*(v_cr-alfa)*(1. + ri_cr - exi_cr);
				/*   Q0[k] = Q0[k] + 4.0*beta*dq*(v_cr-alfa)*(1. + ri_cr - exi_cr)/3.; */               if (i==(ii_cr+1))
...
/* Find value at tnow (phi=0) */
    ...
    *LGMvalue = (1. + rik - exk)*Q0[ik] + (exk - rik)*Q0[ik+1];
    ik = 2*(DTOL(exk/2.));
    rik = ik;
    *LGMvalue = 4./3.*(*LGMvalue) - (2. + rik - exk)*Q0[ik]/6. - (exk - rik)*Q0[ik+2]/6.;
/* Free everything and return */
    FreeMore(zeta_ex, G_start, G_pay, wgt, wgt2, Q0, Q1, 
             eff_pay, postfac, effred) ;
    return(error);
}
...
/* OVE fix: if interpolation gives negative prices, set to 0 */
	...
	if (exk < exk_cr)
    {
		alfa = (v_cr*(exk - rik +.01) + Q1[ik]*(exk_cr - exk +.01))/(exk_cr - rik + .02);
    }
	else
    {
		alfa = (v_cr*(edi+.01 + rik - exk) + Q1[ik+di]*(exk-exk_cr +.01))/(rik+edi - exk_cr + .02); 
	}
            ...
            long npay, Date tpay[], double G_pay[])
{
    ...
    j = 0;
    for (i=0; i<=npay-1; i++)
    {   while (s[j]<tpay[i] && j<n) j++;
        if (s[j] < tpay[i])
           G_pay[i] = g[n] + (g[n]-g[n-1])*(tpay[i]-s[n]) 
                                          /((double)(s[n]-s[n-1]));
        else if (j==0)
           G_pay[i] = g[0] + (g[1]-g[0])*(tpay[i]-s[0])   
                                          /((double)(s[1]-s[0]));
        else
     ...
     {  while (s[j]<t_ex[i] && j<n) j++;
        if (s[j]<t_ex[i])
          ...
          zeta_ex[i] = zeta[0]*(t_ex[i]-tnow)/((double)(s[0]-tnow));
           ...
           zeta_ex[i] = (zeta[j]*(t_ex[i]-s[j-1]) + zeta[j-1]*(s[j]-t_ex[i])) 
                                                            /((double)(s[j]-s[j-1]));  
     }

     j=0;
     for (i=0; i<=nex; i++)
     {  while (s[j]<t_start[i] && j<n) j++;
        if (s[j]<t_start[i])
           G_start[i] = g[n] + (g[n]-g[n-1])*(t_start[i]-s[n]) \
                                             /((double)(s[n]-s[n-1]));
        else if (j==0)
           G_start[i] = g[0] + (g[1]-g[0])*(t_start[i]-s[0])   \
                                             /((double)(s[1]-s[0]));
        else                                                          
           G_start[i] = (g[j]*(t_start[i]-s[j-1])               \
                                     + g[j-1]*(s[j]-t_start[i]))  \
                                             /((double)(s[j]-s[j-1]));
     }
    return;
...
static void genweights(long nq, double wid, double wgt[])
{
    ...
    {   qi = qmin + dq*((double) i);
        ...
        wgt[i] = wgt[i] + (1./sq2pi)*(exp(-(qi+dq)*(qi+dq)/2.)  \
                       - 2.*exp(-qi*qi/2.) + exp(-(qi-dq)*(qi-dq)/2.));
    }

    wgt[0]  = wgt[nq] = -qmin*norm_accurate(qmin) + (qmin+dq)*norm_accurate(qmin+dq) \
           + (1./sq2pi)*(exp(-(qmin+dq)*(qmin+dq)/2.)-exp(-qmin*qmin/2.));
...
static double EXPAY(double phi, long npay, long i0, double pvpay[],
                    double redj, double strike, double G_pay[],
                    double Gst, double zetaj)
{   double sum, ans;
    long i;
    ...
    sum = 0.;
    for  (i=i0; i<=npay-1; i++)
    {   sum = sum + (pvpay[i]-redj)*exp(G_pay[i]
                                      *(phi-.5*G_pay[i]*zetaj));
        redj = 0.;}
...
/* Correct the maturities by the spot_lag for the proper sigma computation */
	for (j=0; j<=n; j++)
    {   
		sy[j] -= (s[j] - add_unit(s[j], spot_lag, SRT_BDAY, MODIFIED_SUCCEEDING))/365.0;
	}
...
/* construct sigma at rest of the dates */
    ...
    {   
		...
		sig_val[k] = glc*sqrt(2./tau_val[k]*(zeta[k]-zeta[k-1])/(exp(2.*th)-1.));
...
/* find number of coupon dates */
    ...
    {   ncoup++;
        coupondate = add_unit(tend, -((int) (mult*ncoup) ), SRT_MONTH, natural_conv); }

/* allocate space */                         
    tnatst = (Date*) srt_calloc(nex+1, sizeof(Date));        /* tnatst[0, 1, ..., nex]     */
	exerBdryArr = srt_calloc(nex+1, sizeof(SrtLgmExerBdry)); 
    zeta_ex = (double*) srt_calloc(nex+1, sizeof(double));   /* zeta_ex[0, 1, ..., nex]    */
    G_st = (double*) srt_calloc(nex+1, sizeof(double));      /* G_st[0, 1, ..., nex]       */
    B_st = (double*) srt_calloc(nex+1, sizeof(double));      /* B_st[0, 1, ..., nex]       */
    aB_first = (double*) srt_calloc(nex+1, sizeof(double));  /* aB_first[0, 1, ..., nex]   */
    ifirstpay = (long*) srt_calloc(nex+1, sizeof(long));     /* ifirstpay[0, 1, ..., nex]  */

    paydate = (Date*) srt_calloc(ncoup+1, sizeof(Date));     /* paydate[0, 1, ..., ncoup]  */
    G_pay = (double*) srt_calloc(ncoup+1, sizeof(double));   /* G_pay[0, 1, ..., ncoup]    */
    aB_pay = (double*) srt_calloc(ncoup+1, sizeof(double));  /* aB_pay[0, 1, ..., ncoup]   */

    if (tnatst==NULL || exerBdryArr==NULL || paydate==NULL ||
        zeta_ex==NULL || G_pay==NULL || G_st==NULL ||
        G_st==NULL || aB_pay==NULL || aB_first==NULL ||
        ifirstpay==NULL)
...
/* generate coupon dates (end & pay dates) */
    for (i=1; i<=ncoup; i++)
        paydate[i] = add_unit(tend, -((int) (mult*(ncoup-i))), SRT_MONTH, natural_conv);
    paydate[0] = tfirstst;
...
/* interpolate to get zeta and G at the needed dates */
    ...
    interpolate_par(tnow, n, nex, t_ex, zeta_ex, tnatst, G_st, ncoup+1, paydate, G_pay);
...
/* compute par swap rate at the exercise boundary */
    ...
    {   
        ...
        exerBdryArr[j].parRate = (B_st[j]*exp(G_st[j]*(phi_cr[j]-.5*G_st[j]*zeta_ex[j]))
                  - Bend*exp(G_pay[ncoup]*(phi_cr[j]-.5*G_pay[ncoup]*zeta_ex[j])))
                  /sum;
...
/* free unneeded arrays & return */
    FreeLots(tnatst, paydate, zeta_ex, G_pay, G_st,
             B_st, aB_pay, aB_first, ifirstpay) ;
    return(error) ;
}
...
static void init_point(double *x, int *check, double target, long i0,
                       long npay, double pvpay[], double redj, double G_pay[],
                       double Gst, double zetaj)
{
    ...
    *x = 0.;
    ...
    {   sum = sum1 = 0.;
        ...
        {   if (i==i0)
                ...
                paymnt = pvpay[i];
            temp = paymnt*exp(-(Gst-G_pay[i])*(*x-(Gst+G_pay[i])*zetaj/2.));
            sum = sum + temp;
...
/* OVE it has to be >= 0.0 otherwise blows up on out of the money (go the wrong way) */
...
/* OVE: this will work as long as SrtInitYc is called, that sets default ccy_param */
...
/***************************************************************************************/    
static void FreeMost(void *a1, void *a2, void *a3, void *a4,
                     void *a5, void *a6)
{
     srt_free(a1);
     srt_free(a2);
...
static void FreeMore(void *a1, void *a2, void *a3, void *a4,
...
static void FreeLots(void *a1, void *a2, void *a3, void *a4,
                     void *a5, void *a6, void *a7, void *a8,
                     void *a9)
{
     srt_free(a1); 

...
For midatlantics:
 1. Find the LGM parameters by matching swaptions to market (BS) price
    ...
    date of mid-atlantic
                "Short" swaptions: Vanilla swaptions of one year duration starting every six months
in semiannual markets (USD) and every year in annual markets (Europe), beginning with the initial
start date of the mid-atlantic. Caplets fitted:	Caplets that start every three or six months
depending on the market
        ***If fix_tau_flag!=1, then the code fits the long swaption and, whenever possible, the short
        swaptions. It is very rare that a deal cannot have all short swaptions fitted exactly (4 out
of 150) as well as all the long swaptions.
 ...
 2. Uses the fitted parameters and evaluates the midatlantic by convolution method. Numerical
accuracy of the convolution is better than a relative error of 1.E-5. The midatlantic value is
*LGMvalue.
...
Payment rules implemented:
 ...
 For American swaptions THE NOTIONAL IS ASSUMED TO BE 1. The payment rules are:
    ...
    Let i be the first fixed coupon whose end date tfix_end[i] is strictly after S. If
early_flag_fix is off, it is assumed that the first fixed coupon will be reduced by the interest
accrued between the coupon's start date tfix_start[i] and the settlement date S (so that the first
    ...
    and the entire fixed coupon is paid at i.
        ...
        The last fixed rate payment full_pay_fix[npay-1] should include the notional (which should
be 1).

    On the floating side, suppose that t_flt[k] is the first floating pay date after the settlement
        date S, and let coupon i be the first fixed coupon whose pay date is strictly after S. The
code assumes that all floating leg payments that are made AFTER t_flt[k] plus any exercise premiums
are equivalent to a payment of Inprem[i] made at t_flt[k]. So like the mid-ats, Inprem[i] should
include the notional plus (for receivers) or minus (for payers) any exercise premium.

    Unless the deal is exercised exactly at the end of a floating period, the code will adjust the
value of the floating leg for the accrued intereston the floating leg. If the early_flag_flt is OFF,
then the code assumes that the interest, rflt*CVG(S,t_flt[k]) that accrues between settlement and
the next floating pay date t_flt[k] is paid on t_flt[k] in addition to Inprem[i]. If the
early_flag_flt is ON then the code assumes that rflt*CVG(t_flt[k-1],S) the interest that accrues
between t_flt[k-1] and S is paid at settlement (goes to reduce the floating leg's value) and that
the full floating coupon rflt*CVG(t_flt[k-1],t_flt[k]) is paid at t_flt[k] in addition to Inprem[i].

    In the above, rflt is the floating rate for the period from t_flt[k-1] to t_flt[k]. If
        t_flt[k-1] <= tnow < t_flt[k], so that today is in the interval, then the code will use the
input argument rflt_current for this already-fixed interest rate. If rflt_current is zero or
negative, or if tnow does not fall in the interval, then the code will calculate rflt from the
current discount curve.
        */

/******** WARNINGS ***********
1) The payment rules must be repected when entering deals with weird frequencies, long payment or
start lags, etc. 2) If the BS vols are bumped one at a time for vega calculations, best results are
obtained if the vols are bumped by a twentieth to a fiftieth of a per cent or less. If the BS vols
are all bumped simultaneously, bumping by a per cent is easily OK. 3) The code hasn't been tested as
rigorously for "European style" markets in which swaps have annual coupons instead of semi-annual
*******************************/
...
static String mid_atl_fit_market(
    String (*GetVol)(),
...
static double comp_intrin_val(long nex, long ifirst[], double redpay[], long npay, double pvpay[]);

static void choose_dates(
    Date       tnow,
    Date       tstart,
    long*      n,
    long       npay,
...
static void ScNewt(double* x, int* check, double target, double (*func1)(), double (*deriv1)());

static void ScSlowNewt(
    double* x,
    double  xmin,
    double  xmax,
    int*    check,
...
static void FreeMore(void*, void*, void*, void*, void*, void*, void*, void*, void*, void*);

static void FreeLots(void*, void*, void*, void*, void*, void*, void*, void*, void*);

static void FreeRest(void*, void*);

static Err get_ccy_defaults(
    SrtCurvePtr     crvptr,
    int             fix_tau_flag,
    int             usecaps,
    BasisCode*      basis,
    SrtCompounding* compnd,
    BusDayConv*     conv,
    int*            spot_lag);
/* S. R. may-07-96 */
/* MACROS */
...
#define ERROR_CHECK1                                                                    \
    if (error != NULL)                                                                  \
    {                                                                                   \
        FreeMore(zeta_ex, G_start, G_pay, wgt, wgt2, Q0, Q1, eff_pay, postfac, effred); \
        return (error);                                                                 \
    }
...
#define ERROR_CHECK2                                                                        \
    if (error != NULL)                                                                      \
    {                                                                                       \
        FreeLots(tnatst, paydate, zeta_ex, G_pay, G_st, B_st, aB_pay, aB_first, ifirstpay); \
    ...
    SrtLgmExerBdryData* lgmExerBdryData) /* ptr to exercise boundary data structure         */
{
    ...
    for (i = 0;
         i < (nfix - 1) && tfix_start[i] < tfix_end[i] && tfix_start[i] < tfix_start[i + 1] &&
    ...
    for (count = 1; count <= countmax; count++)
    {
        ...
        for (i = 0; i < nfix; i++)
        {
            ...
            {
                j           = i * mdiv + k;
                It_start[j] = (Date)(t0 + ((double)(t1 - t0)) * ((double)k) / ((double)mdiv) + 0.5);
                if (cal_or_bus == 0)
        ...
        for (j = j0; j < In_ex; j++)
        {
            ...
            else /* rate has been fixed, but is unknown; est. from df's */
            {
                ...
                rflt =
                    (1.0 / swp_f_df(tnow, t0, yc_name) - 1.0) / coverage(tnow, t0, srt_float_basis);
            }
            /* Adjust floating leg */
                                     ...
                                     extraction */
{
    ...
    for (i = 0;
         i < (npay - 1) && tpay[i] <= tpay[i + 1] && tfix_start[i] < tfix_start[i + 1] &&
         tfix_end[i] < tfix_end[i + 1] && tfix_start[i] < tfix_end[i] && tfix_start[i] < tpay[i];
         i++)
        ;

    if (tfix_start[npay - 1] >= tfix_end[npay - 1] || tfix_start[npay - 1] >= tpay[npay - 1])
    ...
    for (j = 0; j < (In_ex - 1) && It_ex[j] < It_ex[j + 1] && It_start[j] >= It_ex[j] &&
                It_start[j] <= It_start[j + 1];
         j++)
        ;
    ...
    if (t_ex == NULL || t_start == NULL || ifirst == NULL || pvpay == NULL || redpay == NULL ||
        phi_cr == NULL)
    {
    ...
    if ((fix_tau_flag != 1) || (usecaps == 1))
        error =
            Olivier_sh(n, nlast, nex, npay, ifirst, t_start, pvpay, redpay, icmpnd, fix_tau_flag);
    ERROR_CHECK;
    ...
    if (fix_tau_flag != 1)
        ...
        error = mid_atl_fit_market_fix_tau(
            GetVol,
            srt_vol_type,
            n,
            nlast,
    ...
    SrtLgmRefSwptnData* lgmRefSwptnData)
{
    ...
    */
    ...
    g[n]    = 0.;
    g[n - 1] =
        overall_tau * (exp(-sy[n - 1] / overall_tau) - exp(-sy[n] / overall_tau)); /* wolog ! */
    if (icmpnd != 0)
...
static void smooth_strikes(long nlast, double smth)
{
    ...
    else if (nlast == 1)
    {
        ...
        R_theor_lg[0] =
            (.75 * R_theor_lg[0] + .25 * R_theor_lg[1]) * smth + (1. - smth) * R_theor_lg[0];
        R_theor_sh[0] =
            (.75 * R_theor_sh[0] + .25 * R_theor_sh[1]) * smth + (1. - smth) * R_theor_sh[0];
    ...
    /* Main case: nlast>1 */
    rlast_lg =
        (.75 * R_theor_lg[nlast] + .50 * R_theor_lg[nlast - 1] - .25 * R_theor_lg[nlast - 2]) *
            smth +
        (1. - smth) * R_theor_lg[nlast];
    rlast_sh =
        (.75 * R_theor_sh[nlast] + .50 * R_theor_sh[nlast - 1] - .25 * R_theor_sh[nlast - 2]) *
            smth +
    ...
    {
        rtemp         = R_theor_sh[j];
        R_theor_sh[j] = smth * 0.25 * (rtempsh0 + 2.0 * R_theor_sh[j] + R_theor_sh[j + 1]) +
                        (1. - smth) * R_theor_sh[j];
        rtempsh0 = rtemp;

        rtemp         = R_theor_lg[j];
        R_theor_lg[j] = smth * 0.25 * (rtemplg0 + 2.0 * R_theor_lg[j] + R_theor_lg[j + 1]) +
                        (1. - smth) * R_theor_lg[j];
        rtemplg0 = rtemp;
    }
   ...
   the delta irr  of the underlying swaps */
{
    ...
    if (fix_tau_flag != 1)
    {
        ...
        {
            rat =
                ((s[delay] - t_start[nex]) * R_theor_lg[0] /* feather shorts if more exer. dates */
...
static double Vjn(double par, int whichpar)
{
    ...
    ans = sum * Ract + (B[nst] + aB[nst] * Ract) * norm_accurate(phi / sqz - g[nst] * sqz) -
...
static double pay_payer(double ph)
{
    ...
    sum = 0.;
    for (i = jst + 1; i < nst; i++)
        sum = sum + aB[i] * exp(-(g[jst] - g[i]) * (ph - .5 * (g[i] + g[jst]) * zeta[jst]));
...

{
    ...
    if (wgt == NULL || wgt2 == NULL || Q0 == NULL || Q1 == NULL || zeta_ex == NULL ||
        G_start == NULL || G_pay == NULL || eff_pay == NULL || postfac == NULL || effred == NULL)
    {
    ...
    interpolate_par(tnow, n, nex, t_ex, zeta_ex, t_start, G_start, npay, tpay, G_pay);
    ...
    j = nex;
    init_point(
        &phi, &check, prem[j], ifirst[j], npay, pvpay, redpay[j], G_pay, G_start[j], zeta_ex[j]);
    if (check != 0)
    {
    ...
    for (j = nex; j >= 0; j--)
    {
        ...
        v_cr = EXPAY(
            phi_cr[j], npay, ifirst[j], pvpay, redpay[j], prem[j], G_pay, G_start[j], zeta_ex[j]);
        ik_cr   = DTOL(exk_cr);
        rik_cr  = ik_cr;
    ...
    *LGMvalue = (1. + rik - exk) * Q0[ik] + (exk - rik) * Q0[ik + 1];
    ...
    rik       = ik;
    *LGMvalue =
        4. / 3. * (*LGMvalue) - (2. + rik - exk) * Q0[ik] / 6. - (exk - rik) * Q0[ik + 2] / 6.;
    /* Free everything and return */
    FreeMore(zeta_ex, G_start, G_pay, wgt, wgt2, Q0, Q1, eff_pay, postfac, effred);
    return (error);
}
    ...
    long   di)
{
    ...
    {
        alfa = (v_cr * (edi + .01 + rik - exk) + Q1[ik + di] * (exk - exk_cr + .01)) /
               (rik + edi - exk_cr + .02);
    }
    ...
    double G_pay[])
{
    ...
    for (i = 0; i <= npay - 1; i++)
    {
        while (s[j] < tpay[i] && j < n)
            j++;
        if (s[j] < tpay[i])
            G_pay[i] = g[n] + (g[n] - g[n - 1]) * (tpay[i] - s[n]) / ((double)(s[n] - s[n - 1]));
        else if (j == 0)
            G_pay[i] = g[0] + (g[1] - g[0]) * (tpay[i] - s[0]) / ((double)(s[1] - s[0]));
        else
    ...
    for (i = 0; i <= nex; i++)
    {
        ...
        if (s[j] < t_ex[i])
            zeta_ex[i] =
                zeta[n] + (zeta[n] - zeta[n - 1]) * (t_ex[i] - s[n]) / ((double)(s[n] - s[n - 1]));
        else if (j == 0)
    ...
    for (i = 0; i <= nex; i++)
    {
        ...
        if (s[j] < t_start[i])
            G_start[i] =
                g[n] + (g[n] - g[n - 1]) * (t_start[i] - s[n]) / ((double)(s[n] - s[n - 1]));
        else if (j == 0)
            G_start[i] = g[0] + (g[1] - g[0]) * (t_start[i] - s[0]) / ((double)(s[1] - s[0]));
        else
            G_start[i] = (g[j] * (t_start[i] - s[j - 1]) + g[j - 1] * (s[j] - t_start[i])) /
                         ((double)(s[j] - s[j - 1]));
    }
    return;
}
...
static void genweights(long nq, double wid, double wgt[])
{
    ...
    for (i = 1; i < nq; i++)
    {
        ...
        wgt[i] =
            wgt[i] + (1. / sq2pi) * (exp(-(qi + dq) * (qi + dq) / 2.) - 2. * exp(-qi * qi / 2.) +
                                     exp(-(qi - dq) * (qi - dq) / 2.));
    }
    ...
    double zetaj)
{
    ...
    for (i = i0; i <= npay - 1; i++)
    {
        sum  = sum + (pvpay[i] - redj) * exp(G_pay[i] * (phi - .5 * G_pay[i] * zetaj));
        redj = 0.;
    }
    ...
    SrtLgmExerBdryData* lgmExerBdryData)
{
    ...
    if (tnatst == NULL || exerBdryArr == NULL || paydate == NULL || zeta_ex == NULL ||
        G_pay == NULL || G_st == NULL || G_st == NULL || aB_pay == NULL || aB_first == NULL ||
        ifirstpay == NULL)
    {
...
/***************************************************************************************/
static void FreeMost(void* a1, void* a2, void* a3, void* a4, void* a5, void* a6)
{
    srt_free(a1);
    srt_free(a2);
    srt_free(a3);

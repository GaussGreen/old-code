...
static void NAG_CALL EvalDerivatives(Integer neq, double t, double y[], double yp[], Nag_User *comm)
{
	...
	int				i, j, k, iim, ii[10], iisum[10], m_idx, jj[10], jsum, jjcoef, mjj_idx, cjj[10], cmjj_idx;
	double			coef, coef2;
	...
	{
		memset(ii, 0, p->n * sizeof(int));		// order of differentiation on each variable
		...
		for (;;)
		{
			...
			if (iim > 1) for (;;)
			{
				...
				for (k=0; k < p->n; k++) cjj[k] = ii[k] - jj[k];
				...
				{
					for (k=0; k <= p->n; k++)
						yp[m_idx + k] += coef * y[mjj_idx + i] * y[cmjj_idx + i] * p->D[i-1][i-1][k];

					for (j=i+1; j <= p->n; j++)
						for (k=0; k <= p->n; k++)
							yp[m_idx + k] += coef2 * y[mjj_idx + i] * y[cmjj_idx + j] * p->D[i-1][j-1][k];
				}

				// increment jj:

				for (k=p->n-1; k > 0 && jj[k] == ii[k]; k--);
				if (k == 0 && jj[0] >= ii[0]-1) break;
...
static void FillAffiCoefHeston(SAffiCoef *p, double sigma, double lambda, double alpha, double rho)
{
	// Drift:
	p->B[0][0] = 0.0;
	p->B[0][1] = 0.0;
...
static Err HestonDMoments(double sigma, double lambda, double alpha, double rho, double mat,
						  int max_order, SPolynomial *poly)
{
	Err					err = NULL;
	int					k, neq;
	SAffiCoef			affi;
	...
	if (poly->hash->pos > 1) return serror("Second hand hash - oops !");
	...
	if (!affi.B || !affi.D) { err = serror("Memory failure");  goto FREE_RETURN; }
	...
	{
		memset(ii, 0, affi.n * sizeof(int));		// order of differentiation on each variable
	...
	for (k=0; k < neq; k++) RKthres[k] = thres_min;

	// Calculate solution at time 0

	nag_ode_ivp_rk_setup(neq, mat, y, 0.0, RKtol, RKthres,
...
*/
	...
	fact[0] = fact[1] = 1;
	for (k=2; k <= max_order; k++) fact[k] = fact[k-1] * k;
	...
	for (iim=1; iim <= affi.M; iim++)		// order of the moment
	{
		...
		for (;;)
		{
			...
			coef = y[m_idx] + y[m_idx + 2];		// Heston specific	(later: force all factors initial values = 0)
			for (k=0; k < affi.n; k++) coef /= fact[ii[k]];
...
FREE_RETURN:
	...
	if (affi.B) free_dmatrix(affi.B, 0, affi.n - 1, 0, affi.n);
	if (affi.D) free_f3tensor(affi.D, 0, affi.n - 1, 0, affi.n - 1, 0, affi.n);
							  ...
							  int order, int nu, double *u_re, double *u_im, double *h_re, double *h_im)
{
	...
	for (ii[0] = 1; ii[0] <= order; ii[0]++)
	{
		...
		if (poly.pool[hii] >= 0)
		{
			err = Poly_AddConstMono(&pp, (ii[0] % 4 >= 2 ? -1.0 : 1.0) * poly.coef[poly.pool[hii]][0], NULL,
				poly.mono[poly.pool[hii]]);
			if (err) goto FREE_RETURN;
		}
	}
...
static Err calc_g_deriv(int *iuv, SHash *uvHash, SPolynomial *g_pool, SPolynomial *g)
{
	Err				err = NULL;
	...
	if (g_pool[huv].mono) return NULL;
	...
	err = Poly_AddConstMonoPolyPoly(&g_pool[huv], 1.0, NULL, NULL, &g_pool[h1], &g_pool[hi]);
	if (err) return err;
				  ...
				  long npaths, double fwd, int nK, double *K, double *pv, double *stddev)
{
	Err					err = NULL;
	SHash				hash;
	SPolynomial			poly, pp, dd, g_pool[MAXNEQ], phi, herm[MAXNEQ], a[10], corr, corr_cut, tmp[2];
	...
	// init fact:
	fact[0] = fact[1] = 1;
	for (i=2; i <= order; i++) fact[i] = fact[i-1] * i;
	...
	for (ii[0] = 1; ii[0] <= order; ii[0]++)
	{
		...
		err = Poly_AddConstMono(&dd, g_pool[hii].coef[0][0] / fact[ii[0]], NULL, ii);
		if (err) goto FREE_RETURN;
	}
	...
	for (ii[0] = 1; ii[0] <= order; ii[0]++)
	{
		...
		err = Poly_AddConstMonoPoly(&a[0], (ii[0] % 2 ? -1.0 : 1.0) * dd.coef[dd.pool[hii]][0], NULL, NULL, &herm[hii1]);
		if (err) goto FREE_RETURN;
	}
	...
	for (r=0, d = 1.0; r < cf_ord; r++)
	{
		...
		for (i=0; i < r; i++)
		{
			...
			err = Poly_AddConstMonoPolyPoly(&tmp[1], r-i, NULL, NULL, &herm[hii1], &tmp[0]);
			if (err) goto FREE_RETURN;
	...
	for (i=0; i < corr.n_mono; i++)
	{
		if (corr.mono[i][0] <= order)
		{
			...
			{ err = serror("Max mono exceeded");  goto FREE_RETURN; }

			memcpy(corr_cut.mono[corr_cut.n_mono], corr.mono[i], hash.nfac * sizeof(int));
	...
	if (!YY || !reject_path || !pvs) { err = serror("Memory failure");  goto FREE_RETURN; }
	...
	for (r=0; r < npaths; r++)
	{
		...
		if (!reject_path[r])
		{
			...
			{
				d = 1.0;
				for (j=0; j < corr_cut.mono[i][0]; j++) d *= WW;
	...
	for (i=0; i < nK; i++)
	{
		for (j=0; j < npaths; j++) if (!reject_path[j])
		{
			pvs[j][i] = fwd * exp(YY[j]) - K[i];
			if (pvs[j][i] < 0.0) pvs[j][i] = 0.0;

			pv[i] += pvs[j][i] / npaths_used;
		}
		for (j=0; j < npaths; j++) if (!reject_path[j])
			stddev[i] += (pvs[j][i] - pv[i]) * (pvs[j][i] - pv[i]) / (npaths_used-1);
...
FREE_RETURN:
	...
	for (i=0; i < cf_ord; i++) Poly_Free(&a[i]);
	for (i=0; i < 2; i++) Poly_Free(&tmp[i]);
	FreeHashTree(hash.hash_tree);

	ABS_Free(&rg);
	free(YY);
	free(reject_path);
	if (pvs) free_dmatrix(pvs, 0, npaths-1, 0, nK-1);

...
void MomentsComputing(int iDimension, double dMaturity, double *dForwards,
                      double *dVols, double **dCorrels, double *dMoments) {
  ...
  for (i = 0; i < iDimension; i++) {
    ...
    for (j = 0; j < iDimension; j++) {
      dMoment2Container =
          dMoment2Container +
          dForwards[j] * exp(dVols[i] * dVols[j] * dCorrels[i][j] * dMaturity);
    }
    ...
    for (j = 0; j < iDimension; j++) {
      ...
      for (k = 0; k < iDimension; k++) {
        dMoment3Container1 =
            dMoment3Container1 +
            dForwards[k] * exp((dVols[i] * dVols[j] * dCorrels[i][j] +
                                dVols[i] * dVols[k] * dCorrels[i][k] +
                                dVols[k] * dVols[j] * dCorrels[k][j]) *
                               dMaturity);
      }
      dMoment3Container2 =
          dMoment3Container2 + dForwards[j] * dMoment3Container1;
    }
  ...
  dMoments[2] =
      dMoment3 - 3. * dMoment1 * dMoment2 + 2. * dMoment1 * dMoment1 * dMoment1;
}
...
void LogNormalParametersComputing(double *dMoments, double *dParameters) {
  ...
  dBigX =
      (dMoments[2] + sqrt(pow(dMoments[2], 2.) + 4. * pow(dMoments[1], 3.))) /
      2.;
  dU = pow(dBigX, 1. / 3.);
  dZ = dU - dMoments[1] / dU;
  dX = dMoments[1] / dZ;
...
Err srt_f_BasketCapPrice(int iNbRates, double dStartDate, double dStrike,
  ...
  case PREMIUM:
    ...
    MomentsComputing(iNbRates, dStartDate, dForwards, dVols, dCorrels,
                     dMoments);
...
static double inflation_multiput_ll(
    double T, int nidx, double *ai, double *fyi, double *ki, double *si,
    double fx, double kx, double sx,
    double **rho,          /*	index 0..nidx-1: xi        , index nidx: x */
    int npth, double ***g) /*	Sobol cube */
{
  int i, j, k;
    ...
    int npth, double ***g) /*	Sobol cube */
{
  ...
  result = inflation_multiput_ll(T, nidx, ai, fyi_, ki_, voli, fx, kx, sx, rho,
                                 npth, g);
...
static void atm_beta_2_sl(double fwd, double atm, double beta, double *shift,
                          double *vol) {
  *shift = (1.0 - beta) / beta * fwd;
  *vol = beta * atm;
}

static void sl_2_atm_beta(double fwd, double shift, double vol, double *atm,
                          double *beta) {
  *beta = fwd / (fwd + shift);
  *atm = vol / (*beta);
}
...
double inflation_multiput(
  ...
  result =
      inflation_multiput_ll(T, nidx, ai, fyi, ki, si, fx, kx, sx, rho, npth, g);
...
double inflation_multiput_sl(
  ...
  result = inflation_multiput_sl_ll(T, nidx, ai, fyi, ki, shifti, voli, fx, kx,
                                    sx, rho, npth, g);
...
static double price_given_vol_and_shift(double t,     /*	Maturity in years */
                                        double fwd,   /*	Fwd */
                                        double k,     /*	Strike */
                                        double vol,   /*	Vol of L */
                                        double shift) /*	Shift */
{
  if (vol > 0.0) {
    /*	Supernormal case */
    return srt_f_optblksch(fwd + shift, k + shift, vol, t, 1.0, SRT_CALL,
                           PREMIUM);
  } else {
    /*	Subnormal case */
    return srt_f_optblksch(-fwd - shift, -k - shift, -vol, t, 1.0, SRT_PUT,
                           PREMIUM);
  }
}

static double
price_given_shift(double t,     /*	Maturity in years */
                  double fwd,   /*	Fwd */
                  double k1,    /*	Strike 1 */
                  ...
                  double shift) /*	Shift */
{
  if (*vol > 0.0) {
    /*	Supernormal case */
    srt_f_optimpvol(p1, fwd + shift, k1 + shift, t, 1.0, SRT_CALL,
                    SRT_LOGNORMAL, vol);
  } else {
    /*	Subnormal case */
    srt_f_optimpvol(p1, -fwd - shift, -k1 - shift, t, 1.0, SRT_PUT,
                    SRT_LOGNORMAL, vol);
    *vol *= -1;
  }
...
static void implement_shift_limits(double fwd, double k1, double k2, double p1,
                                   double p2, int shift_sign, int vol_sign,
...
Err find_shifted_log_params(double t,   /*	Maturity in years */
  ...
  implement_shift_limits(fwd, k1, k2, p1, p2, shift_sign, vol_sign, 0.0, 0.0,
                         &s);

  s1 = 0.8 * s;
  s2 = 2 * s - s1;
  implement_shift_limits(fwd, k1, k2, p1, p2, shift_sign, vol_sign, 0.0, s,
                         &s1);
  ...
  while (fabs(x - p2) > 1.0e-08 && it < 15) {
    ...
    s = s1 + (p2 - x1) / (x2 - x1) * (s2 - s1);
    implement_shift_limits(fwd, k1, k2, p1, p2, shift_sign, vol_sign, s1, s2,
                           &s);
...
char *i_stellar_cpn(
  ...
  for (i = 0; i < nidx; i++) {
    ...
    err = find_shifted_log_params(fix_mat_years, idx_fwd[i], str1, vol1, str2,
                                  vol2, shift + i, vol + i, beta + i);
    ...
    if (idx_qto[i]) {
      idx_fwd[i] = (idx_fwd[i] + shift[i]) *
                       exp(fx_correl[i] * fx_vol[i] * vol[i] * fix_mat_years) -
                   shift[i];
    }
  ...
  *pv = inflation_multiput_sl(
      fix_mat_years, nidx, idx_weights, idx_fwd, idx_str, shift, vol, cpi_fwd,
      global_str, cpi_vol, rho, (int)(pow(2.0, (double)integ_points) + 1.0e-08),
      atmf_bump_type, beta_bump_type, atmf_bump, beta_bump);
...
/*	Low level lognormal */
static double
cmt_stellar_ll(double T, int nidx, double *ai, double *fyi, double *ki,
    ...
    int npth, double ***g) /*	Sobol cube */
{
  ...
  result = cmt_stellar_ll(T, nidx, ai, fyi_, ki_, voli, ax, fx, kx, sx, rho,
                          npth, g);
...
double
cmt_stellar(double T, int nidx, double *ai, double *fyi, double *ki, double *si,
            double ax, double fx, double kx, double sx,
            double **rho, /* index 0..nidx-1: xi        , index nidx: x */
            int npth) {
  double ***g = NULL;
  double result;
  ...
  result =
      cmt_stellar_ll(T, nidx, ai, fyi, ki, si, ax, fx, kx, sx, rho, npth, g);
...
/*	Price max ( ax * X         , sum [ai * min (Yi         , Ki)] - Kx ) in
 * a shifted-log model */
double cmt_stellar_sl(
  ...
  result = cmt_stellar_sl_ll(T, nidx, ai, fyi, ki, shifti, voli, ax, fx, kx, sx,
                             rho, npth, g);
...
char *cmt_stellar_cpn(
  ...
  /*	Value coupon */
  *pv = cmt_stellar_sl(fix_mat_years, nidx, idx_weights, idx_fwd, idx_str,
                       shift, vol, cmt_weight, cmt_fwd, global_str, cmt_vol,
                       rho, (int)(pow(2.0, (double)integ_points) + 1.0e-08),
                       atmf_bump_type, beta_bump_type, atmf_bump, beta_bump);

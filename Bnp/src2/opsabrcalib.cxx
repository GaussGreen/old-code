...
Err SabrPriceGradient(double strike, double paramSABR[], double *price,
  ...
  *vol=SabrLognormalVol(
  ...
  if (out_range == 1) {
    *price = -1000000;
    ...
    err = op_sabr_pricing(forward, strike, maturity, 1, SRT_CALL, alpha, beta,
                          rho, ATMvol, SABR_ATM_LOG, price);

    err = op_sabr_model_sens(forward, strike, maturity, 1, SRT_CALL, alpha,
                             beta, rho, ATMvol, SABR_ATM_LOG, 1e-4, 1e-4, 1e-4,
                             SABR_ATM_LOG, 1, 1, 1, &(gradient[1]),
    ...
    gradient[5] = 0; /* No need to calculate it */
                     /*	gradient[6]= SabrDsigmaDvolatm(
                                                                             ...
                                                                             strike        ,
                                                                             maturity); */
    err = op_sabr_vega_volga(forward, strike, maturity, 1, SRT_CALL, alpha,
                             beta, rho, ATMvol, SABR_ATM_LOG, 1e-5, 1e-4, 1, 1,
                             SABR_ATM_LOG, 0, 1, &(gradient[6]));
  }

  return err;
}

Err compute_weights(double *strikes, int nbr_strikes, double forward,
                    double ATMVol, double maturity,
...
Err compute_ATMVol(double forward, double *strikes, double *market_vols,
                   int nbr_strikes, int *freeze_ATMVol, double *ATMVol) {
  Err err = NULL;
  int i;
  i = 1;
  ...
  if (strikes[1] >= forward) {
    ...
    if (i == nbr_strikes) {
      ...
      return err;
    } else if (strikes[i] == forward) /* if the ATMVol is given        , we
                                         don't calibrate it */
    {
    ...
    /* Interpolation of the ATMVol. For the moment        , linear interpolation
       To be changed */
    {
      *ATMVol = (forward - strikes[i - 1]) / (strikes[i] - strikes[i - 1]) *
                    market_vols[i] +
                (strikes[i] - forward) / (strikes[i] - strikes[i - 1]) *
                    market_vols[i - 1];
...
Err init_SABR_parameters(double forward, double *strikes, double *market_vols,
                         int nbr_strikes, double ATMVol, double *alpha,
                         int freeze_alpha, double *beta, int freeze_beta,
                         double *rho, int freeze_rho) {
  Err err = NULL;

  return err;
}

Err opsabrcalib(double forward, double maturity, int nbr_strikes,
  ...
  /* Memory allocation */
  nbr_paramSABR = 6; /* alpha        ,beta        ,rho        ,Forward
                        ,Maturity and ATMVol */
  ...
  err = init_SABR_parameters(forward, strikes, market_vols, nbr_strikes,
                             *ATMVol, alpha, freeze_alpha, beta, freeze_beta,
                             rho, freeze_rho);
  ...
  err = levenberg_marquardt_select(
      ...
      nbr_paramSABR, nbr_iter, SabrPriceGradient, fitting_error);

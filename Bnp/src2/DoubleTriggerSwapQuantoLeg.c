						   ...
						   double*	result)
{
	...
	e_x_2 = 1 / (sigma_x * sqrt(T_x)) * (log(B / x0) - correl * sigma_x * sigma_y * min(T_x,T_y) + 0.5 * sigma_x * sigma_x * T_x);
	e_y_2 = 1 / (sigma_y * sqrt(T_y)) * (log(K / y0) - 0.5 * sigma_y * sigma_y * T_y);
	e_x_3 = 1 / (sigma_x * sqrt(T_x)) * (log(B / x0) - 0.5 * sigma_x * sigma_x * T_x);
	e_y_3 = 1 / (sigma_y * sqrt(T_y)) * (log(K / y0) - correl * sigma_x * sigma_y * min(T_x,T_y) + 0.5 * sigma_y * sigma_y * T_y);
	e_x_4 = 1 / (sigma_x * sqrt(T_x)) * (log(B / x0) - correl * sigma_x * sigma_y * min(T_x,T_y) - 0.5 * sigma_x * sigma_x * T_x);
	e_y_4 = 1 / (sigma_y * sqrt(T_y)) * (log(K / y0) - correl * sigma_x * sigma_y * min(T_x,T_y) - 0.5 * sigma_y * sigma_y * T_y);
...
Err   double_inf_barrier_lognormal(double	fwdX,
								   double	B1,
								   double	volX_B1,
								   double	B2,
								   double	volX_B2,
...
Err   double_trigger_swap_quanto_cms_rate_quanto(double fwd,
												 ...
												 double* result_cms_rate_quanto)
{
	...
	if (use_CMSRate_CMSTEC == 0)
	{
		//Call the old functions to calculate the CMS Quanto
		 digital_quanto_float_leg_cms_rate_quanto(fwd,
												  cms_fixing_time,
												  cms_start_time,
												  cms_nfp,
									 ...
									 double*			DoubleTriggerSwapQuantoLeg)
{
	...
	double cms1_ats_log_vol, cms1_B1_log_vol, cms1_B2_log_vol, cms2_ats_log_vol, cms2_K1_log_vol, cms2_K2_log_vol;
	...
	double exotic_coupon1_PV = 0.0, exotic_coupon2_PV = 0.0, exotic_coupon3_PV = 0.0, exotic_coupon4_PV = 0.0;
	...
	frequency_and_basis_to_compounding_and_dRateConv(cms1_frequency,
													 cms1_basis,
													 &cms1_compounding,
													 &cms1_dRateConv);

	frequency_and_basis_to_compounding_and_dRateConv(cms2_frequency,
													 cms2_basis,
													 &cms2_compounding,
													 &cms2_dRateConv);

	//Parsing the tenor num and tenor unit of the CMS assets
	cms_get_tenor_num_and_unit_from_tenor(cms1_tenor,
										  &cms1_tenor_num,
										  &cms1_tenor_unit);

	cms_get_tenor_num_and_unit_from_tenor(cms2_tenor,
										  &cms2_tenor_num,
										  &cms2_tenor_unit);
	...
	while (exo_pay_dates[i] < today + eod_pay_flag) i++;
	...
	{
		//Both fixed
		if ((exo_cms1_fixing_dates[i] < today + eod_fix_flag) && (exo_cms2_fixing_dates[i] < today + eod_fix_flag))
		{
			if ((exo_cms1_past_fixings[i] < exo_cms1_barriers[i]) && (exo_cms2_past_fixings[i] < exo_cms2_barriers[i]))
			{
				// If CMS1<B && CMS2<K then m12
				exotic_coupon1_PV = exo_m12[i];
			}
			else if ((exo_cms1_past_fixings[i] >= exo_cms1_barriers[i]) && (exo_cms2_past_fixings[i] < exo_cms2_barriers[i]))
			{
			...
			else if ((exo_cms1_past_fixings[i] >= exo_cms1_barriers[i]) && (exo_cms2_past_fixings[i] >= exo_cms2_barriers[i]))
			{
				...
				exotic_coupon4_PV = 0.5 * (exo_g4_X[i] * exo_cms1_past_fixings[i] + exo_g4_Y[i] * exo_cms2_past_fixings[i] + exo_m4[i]);
			}
		}
		else
		{
			err = "Handling of past fixings where one of the CMS is fixed in advance and the other one in arrears is not yet available";
			goto FREE_RETURN;
		}

	/*
		//One asset fixed in the case of a Digital Quanto Float TO BE REWRITTEN if DoubleTipTop with fixing in advance and in arrears
		else
		{
			//CMS1 has fixed
			if (exo_cms1_fixing_dates[i] < today + eod_fix_flag)
			{
				...
				if (((exo_cms1_past_fixings[i] > exo_barriers[i]) && (above_below == 0)) ||
					((exo_cms1_past_fixings[i] < exo_barriers[i])  && (above_below == 1)) ||
					((exo_cms1_past_fixings[i] == exo_barriers[i]) && (accrue_on_barrier == 1)))
				{
					//Calculates the start and theo end date of the CMS2 underlying
					cms2_start_date = add_unit(exo_cms2_fixing_dates[i],cms2_spot_lag, SRT_BDAY, MODIFIED_SUCCEEDING);
					if(cms2_tenor_unit == 'M')
					{
						cms2_theo_end_date = add_unit(cms2_start_date, cms2_tenor_num, SRT_MONTH, NO_BUSDAY_CONVENTION);
					}
					else
					{
						cms2_theo_end_date = add_unit(cms2_start_date, cms2_tenor_num, SRT_YEAR, NO_BUSDAY_CONVENTION);
					}
					...
					cms2_nfp = ((int) ((cms2_theo_end_date - cms2_start_date) * YEARS_IN_DAY * cms2_compounding + 0.1)) * 1.0;
			...
			else
			{
				...
				cms1_start_date = add_unit(exo_cms1_fixing_dates[i],cms1_spot_lag, SRT_BDAY, MODIFIED_SUCCEEDING);
				if(cms1_tenor_unit == 'M')
				{
					cms1_theo_end_date = add_unit(cms1_start_date, cms1_tenor_num, SRT_MONTH, NO_BUSDAY_CONVENTION);
				}
				else
				{
					cms1_theo_end_date = add_unit(cms1_start_date, cms1_tenor_num, SRT_YEAR, NO_BUSDAY_CONVENTION);
				}
				...
				cms1_nfp = ((int) ((cms1_theo_end_date - cms1_start_date) * YEARS_IN_DAY * cms1_compounding + 0.1)) * 1.0;
				...
				exotic_coupon_PV = exo_gearings[i] * exo_cms2_past_fixings[i] + exo_margins[i];
				if(above_below == 1) //CMS1<B
				{
					if(pay_rec == 0) //PAY
						exotic_coupon_PV *= - (put_margin_B_shift -  put_margin_B) / call_spread;
					else //REC
						exotic_coupon_PV *= (put_margin_B - put_margin_B_shift) / call_spread;
				}
				else //CMS1>B
				{
					if(pay_rec == 0) //PAY
						exotic_coupon_PV *= - 1 + (put_B + put_margin_B - put_B_shift - put_margin_B_shift) / call_spread;
					else //REC
						exotic_coupon_PV *= 1 - (put_B_shift + put_margin_B_shift - put_B - put_margin_B) / call_spread;
				}
			}
		}*/
	...
	while (i<n_exo_coupons)
	{
		...
		err = digital_quanto_float_leg_get_lognormal_vol(cms2_vc_name,
														 fwd2,
														 fwd_spread2,
														 cms2_fixing_time,
														 cms2_start_date,
		...
		// Correlation
		err = constant_or_linear_interpolation(cms1_cms2_corr_times,
		...
		// For the numeraire adjustments, the vols are taken at exo_cms1_barriers and exo_cms2_barriers
		err = digital_quanto_float_leg_get_lognormal_vol(cms1_vc_name,
														 fwd1,
														 fwd_spread1,
														 cms1_fixing_time,
		...
		cms1_numeraire_adjustment = cms1 * exp(cms1_ats_log_vol * cms1_ats_log_vol * cms1_fixing_time);
		cms2_numeraire_adjustment = cms2 * exp(cms1_cms2_correlation * cms1_ats_log_vol * cms2_ats_log_vol * min(cms1_fixing_time, cms2_fixing_time));

		put2 = srt_f_optblksch(cms2_numeraire_adjustment,
							   K2,
							   cms2_K2_log_vol,
							   cms2_fixing_time,
							   1.0,
		...
		put1 = srt_f_optblksch(cms2_numeraire_adjustment,
							   K1,
							   cms2_K1_log_vol,
							   cms2_fixing_time,
							   1.0,
		...
		cms1_numeraire_adjustment = cms1 * exp(cms1_cms2_correlation * cms1_ats_log_vol * cms2_ats_log_vol * min(cms1_fixing_time, cms2_fixing_time));
		cms2_numeraire_adjustment = cms2 * exp(cms2_ats_log_vol * cms2_ats_log_vol * cms2_fixing_time);

		put2 = srt_f_optblksch(cms1_numeraire_adjustment,
							   B2,
							   cms1_B2_log_vol,
							   cms1_fixing_time,
							   1.0,
		...
		put1 = srt_f_optblksch(cms1_numeraire_adjustment,
							   B1,
							   cms1_B1_log_vol,
							   cms1_fixing_time,
							   1.0,

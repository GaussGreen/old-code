...
double dirty_price_fct(SwapDP p, double coupon, double irr, double redemption,
  ...
  disc1 = 1 / (1 + irr / (double)p.cxxompd);

  /* we price the bond from the second coupon (the first 'regular' coupon) */
  /* to maturity */
  if (disc1 != 1) {
    price = (coupon / (double)p.cxxompd) * (pow(disc1, list.len - 2) - 1) /
                (disc1 - 1) +
            redemption * pow(disc1, list.len - 3);
  } else {
    price = (coupon / (double)p.cxxompd) * (list.len - 2) + redemption;
  }

  /* First coupon */
  price = price * disc1 + first_coupon / (double)p.cxxompd;
...
double fwd_clean_price_fct(Date fut, SwapDP p, double coupon,
  ...
  for (i = 1; i <= nc; i++) {
    sum_coup +=
        (c / p.cxxompd) / swp_f_df((Ddate)(list.date[i]), (Ddate)fut, swap);
    c = coupon;
  }
...
/*additional parameters: SwapDP p        ,double c        , double r        ,
   double f*/
{
  va_list argptr;
  double delta = 0.0001;
...
/*SwapDP pf        ,double coupon        , double clean_price        ,double
   volatility        , double myu        , int T        , double redemption ,
   double first_coupon*/
{
  SwapDP p;
...
/*SwapDP pf        ,double coupon        , double clean_price        ,double
   volatility        , double myu        , double T        , double redemption
   , double first_coupon*/
{
  SwapDP pf;
  double coupon, redemption, first_coupon, myu, clean_price, volatility;
...
double fwd_irr_fct_smp(Date fut, SwapDP p, double coupon, double clean_price,
  ...
  /*x0=log(clean_price_fct(p        ,coupon        ,-0.2        ,redemption
     ,first_coupon) /clean_price);*/

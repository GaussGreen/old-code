...
char * history_reader( char * data_dir,
					  HISTORY * history )
{
	    int   i, rcode;
...
char * HISTORY_Check( HISTORY * history)
{
	...
	{
		if( history->hrates[i] < 0.0 ) return(nrerror("Some historical rates < 0"));
		...
		if( curr != Nxtmth(prev, 1, 1))
		{
			...
			return(nrerror(strcat("History dates are not consecutive at, ", string)));
		}
		prev = curr;
	}
	return(0);
...
char * density_reader( char * data_dir, MBSDENSITY *density)
{
		...
		i = 0;
		while  ( (rcode=fscanf(stream,"%lf %lf\n",&(density->IncentKnots[i]), &(density->iweights[i]))) != EOF )
		{
			if(rcode != 2)
			{
				sprintf(string, "Can't read density.IncentKnots[%d] or iweights[%d]", i, i );
				return(nrerror(string));
			}
...
char * MBSDENSITY_Check( MBSDENSITY * density)
{
	...
	if(!density) return(0);
	if(density->nirates < 1 ) return(nrerror( "Invalid density.nirates"));
	if(density->Nbweights < 1 ) return(nrerror("Invalid Nbweights"));
	for(i=0; i < density->nirates; ++i)
	{
		if( i > 0 && density->irates[i] <= prev ) return(nrerror("density.irates must be increasing"));
		prev = density->irates[i];
		if(density->fastweights[i] < 0.0 || density->sloweights[i] < 0.0) return(nrerror("Fast/Slow weights >=0!"));
	}
	return(0);
}//MBSDENSITY_Check

void adjust_density( MBSDENSITY * density, double M, double gwac, double refiRate, double Rate10Y, long Nbweights  )
{
		int i;
		...
		{
			density->sloweights[i] = density->fastweights[i];
			density->irates[i] = transformIncentive( density->IncentKnots[i], gwac, refiRate, Rate10Y );//convert ratio into absolute 10Y rate
		}
		density->Nbweights = Nbweights;
		neweights(density->urates, density->ufweights, density->IncentKnots, density->fastweights, density->Nbweights);	/* linear interpolation of density */
		neweights(density->urates, density->usweights, density->IncentKnots, density->sloweights, density->Nbweights);
		for(i=0; i < density->Nbweights; ++i)
					density->urates[i] = transformIncentive( density->urates[i], gwac, refiRate, Rate10Y);//transform urates from mtgRate space to 10YRate space
}//adjust_density

void adjust_history( HISTORY * history, int term,  double refiRate, double Rate10Y  )
{
	int i;
	double agency_adj = 0.0 ;
//FIX: this has to be reconciled with speedup adjustment in prepay_engine_setparams
	if(term==180)
		agency_adj = 0.5;

		for(i=0; i < history->Number; ++i)
		{	
			history->hrates[i] = transformIncentive( history->hrates[i], 1.0, refiRate, Rate10Y);//convert to 10Y rate
...
//
void spdclass(   double *fastweights, 
	    double M,
            int     NUMWTS)	
{
                int   i;
		...
		if ( M != 1.0) {
			if( M < 1E-14 ) M = 1E-14;
			l[0] = fastweights[0];
			for (i=1; i<NUMWTS; i++) l[i] = l[i-1]+fastweights[i];	/* Fractions "left" (not triggered yet). */
			fract[0] = 1.0;
			for (i=1; i<NUMWTS; i++) fract[i] = fastweights[i]/l[i];	 /* Fraction of remaining at this trigger. */
			for (i=0; i<NUMWTS; i++) fract[i] = 1.-(pow(1.-fract[i],M));	         /* Speed up these triggers by a factor of M. */

			l[NUMWTS-1] = 1.0;   /* We will now keep track of updated fractions "left" after the speed-up. Same value of 1.0 here. */
			for (i=NUMWTS-1; i>=0; i--) {
				fastweights[i] = fract[i] * l[i];               /* New weight at this trigger. */
				if (i>0) l[i-1] = l[i] - fastweights[i];	 /* New fraction "left" for next trigger. */
		...
		int	 N)
{
		...
		double sum = 0.;	  
		for (i=0; i<N; i++) {
			rates[i] = 0.92 * (double)i/((double)N-1.);			/* generate uniformly distributed trigger rates */

			ldensity(inrates, inweights, &rates[i], &weights[i]);		/* linear interpolation */
		sum += weights[i];				   	
		}
		for (i=0; i<N; i++) {
...
{   
	...
	if (*rate >= irates[16])
		...
		*weight = iweights[i-1] + (*rate - irates[i-1]) *
			(iweights[i-1] - iweights[i])/(irates[i-1]-irates[i]);			/* linear interpolation */
	}
}//ldensity
...
//where 
//1st payment <-> payment_nb=1; calc the scheduled principal paydown during the payment_nb-th payment, per $1 principal remaining before the payment
...
{
	return( 1.0 -
				(1.0 - SCHEDULED_AMORT_RATE( wam, gwac ))
				*( 1.0 - smm/100.0)
			);
}//TOTAL_AMORT_RATE
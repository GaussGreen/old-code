...
void adjust_density(
    MBSDENSITY* density, double M, double gwac, double refiRate, double Rate10Y, long Nbweights)
{
    int i;
    ...
    {
        density->sloweights[i] = density->fastweights[i];
        density->irates[i]     = transformIncentive(
...
void adjust_history(HISTORY* history, int term, double refiRate, double Rate10Y)
{
    ...
    {
        history->hrates[i] =
            transformIncentive(history->hrates[i], 1.0, refiRate, Rate10Y);  // convert to 10Y rate
...
void spdclass(double* fastweights, double M, int NUMWTS)
{
    ...
    if (M != 1.0)
    {
        ...
        l[0] = fastweights[0];
        for (i = 1; i < NUMWTS; i++)
            l[i] = l[i - 1] + fastweights[i]; /* Fractions "left" (not triggered yet). */
        fract[0] = 1.0;
        for (i = 1; i < NUMWTS; i++)
            fract[i] = fastweights[i] / l[i]; /* Fraction of remaining at this trigger. */
        for (i = 0; i < NUMWTS; i++)
            fract[i] = 1. - (pow(1. - fract[i], M)); /* Speed up these triggers by a factor of M. */

        l[NUMWTS - 1] = 1.0; /* We will now keep track of updated fractions "left" after the
                                speed-up. Same value of 1.0 here. */
        for (i = NUMWTS - 1; i >= 0; i--)
        {
            fastweights[i] = fract[i] * l[i]; /* New weight at this trigger. */
            if (i > 0)
                l[i - 1] = l[i] - fastweights[i]; /* New fraction "left" for next trigger. */
        }
    }
}  // spdclass

void neweights(double* rates, double* weights, double* inrates, double* inweights, int N)
{
    int    i;
    double sum = 0.;
    for (i = 0; i < N; i++)
    {
        rates[i] =
            0.92 * (double)i / ((double)N - 1.); /* generate uniformly distributed trigger rates */
...

{
    ...
    else
    {
        ...
        *weight = iweights[i - 1] + (*rate - irates[i - 1]) * (iweights[i - 1] - iweights[i]) /
...
// where
// 1st payment <-> payment_nb=1; calc the scheduled principal paydown during the payment_nb-th
// payment, per $1 principal remaining before the payment

...
Err cheybeta_pricing_pde(
  ...
  err = payoff_func(date[nstept - 1], time[nstept - 1],
                    ...
                    /* Gride data	*/
                    0, grid.nx - 1, 0, grid.nphi[nstept - 1] - 1, grid.x,
                    grid.phi[nstept - 1],
                    /* Vector of results to be updated */
                    nprod, grid.vt2);
  if (err) {
  ...
  for (t = nstept - 2; t >= 0; t--) {
    ...
    srt_f_chey_beta_bck_pde_one_step(&tmppde, &tmp, &mdl, &grid, t, t + 1);

    /*	Eval payoff */
    if (eval_evt[t]) {

      err =
          payoff_func(date[t], time[t], func_parm_tab[t], yc, ifr[t],
                      /* Model data	*/
                      ((SrtIrDesc *)(und->spec_desc))->ts,
                      /* Gride data	*/
...
Err cheybeta_pricing_mc(
    ...
    /*	Market data */
    double *ifr, char *yc, int *stop_vol, int *stop_lambda,
      ...
      if (total->num_df > 0) {
        for (i = 0; i < total->num_df; i++) {
          if (total->df_tms[i] >= YEARS_IN_DAY) {
            ...
            total->gam[i] = Lambda_func(time[t], time[t] + total->df_tms[i],
                                        ((SrtIrDesc *)(und->spec_desc))->ts);
  ...
  err = srt_f_New_gen_random(cube, 0, numpaths - 1, 0, nstept - 2, 1,
                             gen_method, &seed, time);
  if (err)
    goto FREE_RETURN;

  if (method && ((gen_method == BAL_ANTI) || (gen_method == RANDOM_GAUSS) ||
                 (gen_method == ANTITHETIC))) {
    /* indeed        , it's ugly ! */
  ...
  for (n = 0; n < numpaths; n++) {
    ...
    if (eval_evt[0]) {
      ...
      err = payoff_func(date[0], time[0], func_parm_tab[0], yc,
                        /* Model data	*/
                        ifr[0], x, phi,
                        /* Vector of results to be updated */
                        nprod, currentvalues);
    ...
    for (t = 1; t <= nstept - 1; t++) {
      ...
      x += (phi - x * tau) * dt + stdv * cube[n][0][t - 1];
      ...
      /*	Eval payoff */
      if (eval_evt[t]) {
        err = payoff_func(date[t], time[t], func_parm_tab[t], yc,
                          /* Model data	*/
                          x + ifr[t], x, phi,
                          /* Vector of results to be updated */
                          nprod, temppayoffs);
  ...
  for (i = 0; i < nprod; i++) {
    ...
    res[1][i] = sqrt(
        (sum_payoff2[i] * numpaths - sum_payoff[i] * sum_payoff[i]) / numpaths);
  }

FREE_RETURN:

  if (sum_payoff)
    free_dvector(sum_payoff, 0, nprod);
  if (sum_payoff2)
    free_dvector(sum_payoff2, 0, nprod);
  if (currentvalues)
    free_dvector(currentvalues, 0, nprod);
  if (temppayoffs)
    free_dvector(temppayoffs, 0, nprod);
  if (cube) {
    for (i = 0; i < numpaths; i++) {
      free(cube[i][0]);
      free(cube[i]);

...
Err interp_sabr_vol_type(char* str, SABR_VOL_TYPE* val)
{
    ...
    if (!strcmp(str, "NORMAL") || !strcmp(str, "NORM") || !strcmp(str, "N") || !strcmp(str, "1") ||
        !strcmp(str, "ATMNORM"))
    {
        *val = SABR_ATM_NORM;
        return NULL;
    ...
    if (!strcmp(str, "LOGNORMALATSTRIKE") || !strcmp(str, "BLACKSCHOLES") || !strcmp(str, "BS") ||
        !strcmp(str, "STRLOG"))
    {
        *val = SABR_STR_LOG;
        return NULL;
    ...
    double rho)
{
    ...
    if (err = vol_type_to_diff_type_atm(input_vol_type, &input_diff_type, &input_vol_atm))
    {
        return err;
    }

    /*	Get output diffusion type and strike	*/
    if (err = vol_type_to_diff_type_atm(output_vol_type, &output_diff_type, &output_vol_atm))
    {
        return err;
    }
    ...
    if (err = srt_f_optbetavoltoblkvol(forward, strike, input_vol, maturity, 0.0, &tmp_vol))
    {
        return err;
    }
    ...
    double* prem)
{
    ...
    *prem = srt_f_optblksch(forward, strike, str_log_vol, maturity, disc, call_put, PREMIUM);
    ...
    double* thetacarry)
{
    ...
    *thetavanna = -rho * pow(sigma_beta, 2.0) * pow(forward, beta) * alpha * vanna;

...
{   
	...
	if (	!strcmp (str,"LOGNORMALATSTRIKE")
		||	!strcmp (str,"BLACKSCHOLES")
		||	!strcmp (str,"BS")
		||	!strcmp (str,"STRLOG"))
	{
		*val = SABR_STR_LOG;
	...
	if (	!strcmp (str,"NORMALATSTRIKE")
		||	!strcmp (str,"BLACKSCHOLESNORMAL")
		||	!strcmp (str,"BSN")
		||	!strcmp (str,"STRNORM"))
	{
		*val = SABR_STR_NORM;
...
SABR_RISK_PARAM			*param)
{
	...
	for (i=0;i<num_param;i++)
	{
		...
		if (!strcmp (param_str[i], "GAMMASHIFT"))
		{
			sscanf (value_str[i], "%lf", &d);
			param->gamma_shift = d;
		}
		else
		if (!strcmp (param_str[i], "VEGASHIFT"))
		{
			sscanf (value_str[i], "%lf", &d);
			param->vega_shift = d;
		}
		else
		if (!strcmp (param_str[i], "VOLGASHIFT"))
		{
			sscanf (value_str[i], "%lf", &d);
			param->volga_shift = d;
		}
		else
		if (!strcmp (param_str[i], "VANNASHIFT"))
		{
			sscanf (value_str[i], "%lf", &d);
			param->vanna_shift = d;
		}
		else
		if (!strcmp (param_str[i], "THETASHIFT"))
		{
			sscanf (value_str[i], "%lf", &d);
			param->theta_shift = d;
		}
		else
		if (!strcmp (param_str[i], "ALPHASHIFT"))
		{
			sscanf (value_str[i], "%lf", &d);
			param->alpha_shift = d;
		}
		else
		if (!strcmp (param_str[i], "BETASHIFT"))
		{
			sscanf (value_str[i], "%lf", &d);
			param->beta_shift = d;
		}
		else
		if (!strcmp (param_str[i], "RHOSHIFT"))
		{
			sscanf (value_str[i], "%lf", &d);
			param->rho_shift = d;
		}
		else
		if (!strcmp (param_str[i], "DELTAFAC"))
		{
			sscanf (value_str[i], "%lf", &d);
			param->delta_fac = d;
		}
		else
		if (!strcmp (param_str[i], "GAMMAFAC"))
		{
			sscanf (value_str[i], "%lf", &d);
			param->gamma_fac = d;
		}
		else
		if (!strcmp (param_str[i], "VEGAFAC"))
		{
			sscanf (value_str[i], "%lf", &d);
			param->vega_fac = d;
		}
		else
		if (!strcmp (param_str[i], "VANNAFAC"))
		{
			sscanf (value_str[i], "%lf", &d);
			param->vanna_fac = d;
		}
		else
		if (!strcmp (param_str[i], "VOLGAFAC"))
		{
			sscanf (value_str[i], "%lf", &d);
			param->volga_fac = d;
		}
		else
		if (!strcmp (param_str[i], "THETAFAC"))
		{
			sscanf (value_str[i], "%lf", &d);
			param->theta_fac = d;
		}
		else
		if (!strcmp (param_str[i], "ALPHAFAC"))
		{
			sscanf (value_str[i], "%lf", &d);
			param->alpha_fac = d;
		}
		else
		if (!strcmp (param_str[i], "BETAFAC"))
		{
			sscanf (value_str[i], "%lf", &d);
			param->beta_fac = d;
		}
		else
		if (!strcmp (param_str[i], "RHOFAC"))
		{
			sscanf (value_str[i], "%lf", &d);
			param->rho_fac = d;
		}
		else
		if (!strcmp (param_str[i], "VEGAMULT"))
		{
		...
		if (!strcmp (param_str[i], "VOLGAMULT"))
		{
			if (!strcmp (value_str[i], "YES"))
			{
				param->volga_mult = 1;
		...
		if (!strcmp (param_str[i], "DELTAFREEZEVOL"))
		{
		...
		if (!strcmp (param_str[i], "THETAFREEZEVOL"))
		{
			if (err = interp_sabr_vol_type (value_str[i], &vt))
			{
				return err;
		...
		if (!strcmp (param_str[i], "SMILEFREEZEVOL"))
		{
			if (err = interp_sabr_vol_type (value_str[i], &vt))
			{
				return err;
		...
		if (!strcmp (param_str[i], "VEGABUMPVOL"))
		{
			if (err = interp_sabr_vol_type (value_str[i], &vt))
			{
				return err;
...
static Err vol_type_to_diff_type_atm(
SABR_VOL_TYPE			vol_type,
...
double					rho)
{
	...
	if (err = vol_type_to_diff_type_atm (	input_vol_type, 
											&input_diff_type, 
											&input_vol_atm))
	{
		return err;
	...
	if (err = vol_type_to_diff_type_atm (	output_vol_type, 
											&output_diff_type, 
											&output_vol_atm))
	{
		return err;
	...
	if (input_vol_atm)
	{
		...
		if (err = srt_f_optsarbvol (	forward,							
											strike, 
											maturity, 
											input_vol,  
											alpha,
	...
	if (input_diff_type == SRT_LOGNORMAL)
	{
		...
		if (err = srt_f_optblkvolATMtobetavolStochVol (	forward,							
														strike, 
														input_vol,
														maturity,
														alpha,
	...
	if (err = srt_f_optbetavoltoblkvol (	forward,							
											strike, 
											input_vol, 
											maturity,  
											0.0,
...
double					*sens)
{
	...
	if (err = op_sabr_pricing (	forward,
								strike,
								maturity,
								disc,
								call_put,
	...
	if (err = op_sabr_pricing (	forward + delta_shift,
								strike,
								maturity,
								disc,
								call_put,
	...
	*sens = fac * delta1;
	...
	if (delta_gamma == 1)
	{
		if (err = op_sabr_delta_gamma (	forward + gamma_shift,
										strike,
										maturity,
										disc,
										call_put,
...
double					*sens)
{
	...
	*sens = fac * vega1;
	...
	if (vega_volga == 1)
	{
		if (err = op_sabr_vega_volga (	forward,
										strike,
										maturity,
										disc,
										call_put,
...
double					*vanna)
{
	...
	/*	Calculate delta before shift	*/
	if (err = op_sabr_delta_gamma (	forward,
									strike,
									maturity,
									disc,
									call_put,

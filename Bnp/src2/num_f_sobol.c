...
static long PolInit (long dimension)
{
	...
	{
		if (polcount>nb_prim_pol_deg_smaller_than_poldeg+number_of_prim_pols_of_degree[poldeg-1]) {
			nb_prim_pol_deg_smaller_than_poldeg+=number_of_prim_pols_of_degree[poldeg-1];
			poldeg++;
		}
		otpol[polcount] = PrimitivePolynomials[poldeg-1][polcount-1-nb_prim_pol_deg_smaller_than_poldeg];
		deg[polcount]=poldeg; 
		polcount++;
	}   
...
void select_all_init_sequences(long* init_sequence, long dimension, long maxdeg);
...
/*
	...
	for (k=0;k<maxdeg;k++) {
		for (j=0;j<dimension;j++) {
			sprintf(tmp,"%d ", rinit[j+k*dimension]);
			strcat(init_seq_string, tmp);
		}
 ...
 *
 */
void sobol_next(long dimension, double *otp)
{
	SobNext(dimension,otp);
}
...
/* Collect and transfer the full path */
    for (i=0; i<SOBOL_BRW_NUM; i++)
		for (j=0; j<SOBOL_STP_NUM; j++) 
    		v[rl+i][sl+j] = inv_cumnorm_fast (SOBOL_RAN_NUM[i+SOBOL_BRW_NUM*j]);
...
/* Loops on all paths to generate */
	...
	/* Collect and transfer the full path */
		for (i=0; i<SOBOL_BRW_NUM; i++)
			for (j=0; j<SOBOL_STP_NUM; j++) 
    			v[pl+k][rl+i][sl+j] = inv_cumnorm_fast (SOBOL_RAN_NUM[i+SOBOL_BRW_NUM*j]);

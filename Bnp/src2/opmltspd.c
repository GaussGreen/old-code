...
static void local_Uniform_With_Antithetic_Noise(
			double **m, 
			long pathindexstart, long pathindexend,  
			long stepindexstart, long stepindexend, 
			long *seed);
                        ...
                        )
{
	...
	for (i=1;i<=path_numb;i++)
	{
	   ...
	   strike_xyz = strike 
			  - b_y * fwdy * exp(y*sqrt_vary - 0.5*vary)
	   ...
	   if (strike_xyz > 0.0)
		...
		d_xyz = log(fwd_xyz/strike_xyz) / (alpha*sqrt_varx);
	   	bs_xyz = cp * (  fwd_xyz  
				* norm(cp * (d_xyz + 0.5 * alpha * sqrt_varx) )
				- strike_xyz  
				* norm(cp * (d_xyz - 0.5 * alpha * sqrt_varx) )
			      );	
           }

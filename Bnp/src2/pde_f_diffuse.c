    ...
    for (m = 0 ; m < grid->nb_payoffs ; m++)
    {
        ...
        for (l = 0 ; l < nbcoeff ; l++)  
        {
			...
			for (i = 1 ; i <= mxx; i++)
			{
				for (j = 1 ; j <= myy ; j++)
				{
					grid->new_value[m][i][j][0][l] =  grid->AX1[i][j-1][0]*grid->old_value[m][i][j-1][0][l]
			...
			for (i = 0 ; i <= mxx + 1 ; i++)
            {
			   ...
			   grid->temp_value[m][i][0][0][l] = grid->old_value[m][i][0][0][l]
              ...
              for (j = 1 ; j <= myy ; j++)
              {
    	          if ((i == 0) || (i == mxx + 1))
                  {
                     grid->temp_value[m][i][j][0][l] = grid->old_value[m][i][j][0][l] 
              ...
              grid->temp_value[m][i][myy+1][0][l] = grid->old_value[m][i][myy+1][0][l]
			            - pde->bound_cond_y * grid->old_value[m][i][myy][0][l]
                        - grid->L2[i][myy+1][0] * grid->temp_value[m][i][myy][0][l];
    
              /*--------------*/

              grid->new_value[m][i][myy+1][0][l] = grid->temp_value[m][i][myy+1][0][l] 
                                                  / grid->D2[i][myy+1][0];

              for (j = myy ; j >= 0 ; j--)
			  {
                  grid->new_value[m][i][j][0][l] = (grid->temp_value[m][i][j][0][l] 
	...
	for (m = 0 ; m < grid->nb_payoffs ; m++)
    {
        ...
        for (l = 0 ; l < nbcoeff ; l++)  
        {
			...
			for (j = 1 ; j <= myy; j++)
			{
				for (i = 1 ; i <= mxx ; i++)
				{
					grid->new_value[m][i][j][0][l] =  grid->AY1[i-1][j][0]*grid->old_value[m][i-1][j][0][l]
			...
			for (j = 0 ; j <= myy + 1 ; j++)
            {
			  ...
			  grid->temp_value[m][0][j][0][l] = grid->old_value[m][0][j][0][l]
            ...
            grid->temp_value[m][mxx+1][j][0][l] = grid->old_value[m][mxx+1][j][0][l]
                                                   - pde->bound_cond_x * grid->old_value[m][mxx][j][0][l]
                                                   - grid->L1[mxx+1][j][0] * grid->temp_value[m][mxx][j][0][l];

            /*--------------*/

            grid->new_value[m][mxx+1][j][0][l] = grid->temp_value[m][mxx+1][j][0][l] 
                                                              / grid->D1[mxx+1][j][0];

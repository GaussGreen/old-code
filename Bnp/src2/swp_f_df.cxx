/* ---------------------------------------------------------------------------
   FILENAME:     swp_f_df.cxx

   PURPOSE		: compute a discount factor via a pointer to a function
   DiscFunc passed to the SrtExternalFunctions static ( see
   srt_f_external_fct.cxx ) Also available        , a function to compute a
   CashFwd
   ---------------------------------------------------------------------------
 */
#include "math.h"
...
double swp_f_disc_or_zrate(Ddate dstart, Ddate dend, char *crv_name,
                           int rate_or_disc) {
  Err err = NULL;
  double zr;
  double df;
...
/*-----------------------------------------------------------------------------------
This is a proper cash rate taking into account the fixing date        , the
start and end dates
 ----------------------------------------------------------------------- */
double swp_f_fwdcashrate(Ddate fixing, Ddate start, Ddate end, BasisCode b,
                         char *crvname) {
  double df1, df2;
  double fra;
  double cvg;
...
This is a swap cash rate: It uses a Theo End date.
 ----------------------------------------------------------------------- */
double swp_f_swapcashrate(long start, long theoend, SrtBasisCode Basis,
                          SrtCompounding Comp, char *ycName,
                          char *refRateCode) {
  double df1, df2;
  long today;
  String basisStr;
  ...
  /* Computes the Level */
  swp_f_LevelPayment(start, theoend, compStr, basisStr, ycName, refRateCode,
                     &level);

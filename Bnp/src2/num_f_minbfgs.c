...
free_matrix(hessin,1,n,1,n);free_dvector(hdg,1,n);free_dvector(g,1,n); \
free_dvector(dg,1,n);

Err dfpmin(DFPMinCriteriumType returnCriterium, double p[], int n, double func_gtol, double dfunc_tol,
		  int *iter, double *fret, 
		  Err(*func)(double [], double*), Err (*dfunc)(double [], double []))
{
	Err err = NULL;
	Err lnsrch1(int n, double xold[], double fold, double g[], double p[], double x[],
		 double *f, double stpmax, int *check, Err (*func)(double [], double*));
	...
	for (its=1;its<=ITMAX;its++) {
		*iter=its;
		...
		if (returnCriterium == FUNCCRITERIUM)
			if (fabs(*fret) < func_gtol) return err;
...
static Err lnsrch1(int n, double xold[], double fold, double g[], double p[], double x[],
	double *f, double stpmax, int *check, Err (*func)(double [], double*))
{
	Err err = NULL;
	int i;
			...
			*check=1;
			...
			if (alam == 1.0)
				...
				if (a == 0.0) tmplam = -slope/(2.0*b);
					...
					if (disc<0.0) smessage("Roundoff problem in lnsrch.");
...
#define FREEALLFRPMN free_dvector(xi,1,n);free_dvector(h,1,n);free_dvector(g,1,n);
...
static void linmin(double p[], double xi[], int n, double *fret, Err (*func)(double [], double *))
{
...
static double brent(double ax, double bx, double cx, double (*f)(double), double tol,
	double *xmin)
{
	int iter;

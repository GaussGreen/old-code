...
********************************************************************************
    ...
    DESCRIPTION     :   For functions of the form f(a        ,b        ,c ,...)
we have already evaluated the arguments and joined them together. Hence
COMLL_PTR argument to these functions is a list: "c"-->"b"-->"a".
                        ...
                        the arguments        , replacing what was there before
with a single "f(a        ,b        ,c)". Back in YACC        , the pattern
matching code contains a pointer to the TOP of this list        , which we can
NOT give up. Therefore in the second case we will free this part of the list:
                                            -->"b"-->"a"
                        and replace "c" with "f(a        ,b        ,c)".

                        Other important modules called from this one include:

                        o grf_f_lngsymtab.cxx (GRFN language symbol table)

                        o grf_f_lang_finance.cxx (Calls to financial functions)

                        o grf_f_lang_pvrng.cxx (grfn function pvrng())
    ...
    PROBLEMS        :   Error checking may not respond well to character
                        strings used in the wrong place        , e.g. "annual" +
2 should return an error BUT it will not!
                        ...
                        2) Need a single function to check the arguments to
                           any function        , taking as inputs a prototype
and a COMLL representing the arguments.
...
static Err check_string_arg(COMLL_PTR args, COMLLType type) {
  ...
  if ((type == COMLL_A_BLKSCHLS) || (type == COMLL_A_BLKSCHLSNRM) ||
      (type == COMLL_A_BSSABR) || (type == COMLL_A_SPREADNUM)) {
...
/******************************************************************************\
    ...
    DESCRIPTION     :   Check cell co-ordinates col        , row1        , row2
specified in cell functions. All cells in the implied range must be already
computed either on the same row and to the left of        , or above        ,
the current cell.

\******************************************************************************/

static Err check_cell_coords(GrfnDeal *gd, int *detargs, String name) {
  long c, r1, r2;
  long r, c1, c2, ind;

  if ((strcmp(name, "ROWSORT") == 0) || (strcmp(name, "ROWINTERP") == 0)) {
    ...
    if (r < 0 || r > gd->sslength - 1 || c1 < 0 || c1 > gd->sswidth - 1 ||
      ...
      return serror("%s: %s[%d        ,%d        ,%d        ,%d]",
                    GRERR_REF_OUT_OF_RANGE, name, r, c1, c2, ind);

    /* If the second column reference is before the first one */
    if (c2 < c1)
      return serror("%s: %s[%d        ,%d        ,%d        ,%d]",
                    GRERR_COL_OUT_OF_RANGE, name, r, c1, c2, ind);

    /* If the index required is not within the number of columns on which to
     * sort        , mention it */
    if ((ind < 0) || (ind > (c2 - c1)))
      return serror("%s: %s[%d        ,%d        ,%d        ,%d]",
                    GRERR_INDEX_OUT_OF_RANGE, name, r, c1, c2, ind);
  } else if ((strcmp(name, "ROWMIN") == 0) || (strcmp(name, "ROWMAX") == 0) ||
             (strcmp(name, "ROWSUM") == 0) || (strcmp(name, "ROWAVG") == 0) ||
    ...
    /* If there is a reference to the future        , mention it */
    if ((r > gd->I) || ((r == gd->I) && (c2 >= gd->J)))
      return serror("%s: %s[%d        ,%d        ,%d]", GRERR_ILLEGAL_REF, name,
                    r, c1, c2);

    /* If the arguments are out of the tableau range        , mention it */
    if (r < 0 || r > gd->sslength - 1 || c1 < 0 || c1 > gd->sswidth - 1 ||
        c2 < 0 || c2 > gd->sswidth - 1)
      return serror("%s: %s[%d        ,%d        ,%d]", GRERR_REF_OUT_OF_RANGE,
                    name, r, c1, c2);

    /* If the second column reference is before the first one */
    if (c2 < c1)
      return serror("%s: %s[%d        ,%d        ,%d]", GRERR_COL_OUT_OF_RANGE,
                    name, r, c1, c2);

  } else /* perform check on COLXXX functions */
  {
    /* Gets the parameters of the COL... function: Column        , RowStart ,
     * RowEnd
     */
    c = detargs[0];
    r1 = detargs[1];
    ...
    if (r2 < r1)
      return serror("%s: %s[%d        ,%d        ,%d        ,%d]",
                    GRERR_ROW_OUT_OF_RANGE, name, c, r1, r2);

    /* If there is an illegel reference to the future        , mention it */
    if ((r2 > gd->I) || ((c == gd->I) && (r2 >= gd->J)))
      return serror("%s: %s[%d        ,%d        ,%d]", GRERR_ILLEGAL_REF, name,
                    c, r1, r2);
  }
  return NULL;
...
/******************************************************************************\
    ...
    DESCRIPTION     :   Check a cell function.
                        ...
                        which take deterministic arguments
                                                ...
                                                Called by the LEX/YACC parser
when encountering a NAME[...] type of input string

                                                Arguments are passed BACKWARDS
by the parser (see the 'Args' section in Grfn_Yacc.y)
\******************************************************************************/
...

{
  ...
  if (args->nargs != gs.ndargs + gs.nrargs)
    return serror("%s %s %d        , requires %d args", name,
                  GRERR_WRONG_NUM_ARGS, args->nargs, gs.ndargs + gs.nrargs);
  ...
  case CLLNCellFunc:
    ...
    /* Makes sure all the arguments are REAL and collects them by function order
     * (back of COMLL) */
    ...
    /* C is the column index         , except for ROWSORT/ROWINTERP where it is
     * the row index */
    c = detargs[0];

    /* R... is the row index        , except for ROWSORT/ROWINTERP where it is
     * the column index */
    r1 = detargs[1];
    r2 = detargs[2];

    /* The final argument of ROWSORT        , COLSORT or ROWINTERP: the index
     * wanted
     */
    if (gs.type == COMLL_C_COLSORT || gs.type == COMLL_C_ROWSORT ||
        gs.type == COMLL_C_ROWINTERP)
      ind = detargs[3];
    else
      ind = 0;
    ...
    if (gs.type == COMLL_C_ROWINTERP) {
      /* Checks the auxiliary range has the right length for interpolation (and
       * that ind is not outside of range) */
      CheckAuxRange1(ind, gd);
      if (gd->auxlen[ind] != (r2 - r1))
        serror("%s: lengths %d        ,%d", GRERR_INTERP_SAME_SIZE,
               gd->auxlen[ind], (r2 - r1));

      /* For ROWINTERP        , the value to interpolate (x) is not
       * deterministic : need a TWO nodes COMLL */
      ...
      /* Populate the COMLL node with the indexes: aux        , row        ,
       * col_s        , col_e
       */
      memcpy(args->next->ivec, detargs, 3 * sizeof(int));

      /* The (complex) argument at the top of the list is x (args points on the
       * last argument of "x") */

      return NULL;

    } else {
      /* We need only one element in this function COMLL (storing everything in
       * ivec) */
      args->next->prev = NULL;
  ...
  case CLLNCellRef:
    ...
    if (cindex < 0 || cindex > gd->sswidth - 1 || rindex < 0 ||
        rindex > gd->sslength - 1)
      return serror("%s C[%d        ,%d]", GRERR_REF_OUT_OF_RANGE, cindex,
                    rindex);
    ...
    if (rindex > gd->I)
      return serror("%s: C[%d        ,%d]", GRERR_ILLEGAL_REF, cindex, rindex);
  ...
  case CLLNAuxRef:
    ...
    cindex = DTOL(lastarg->dval);
    rindex = DTOL(lastarg->prev->dval);
    if (cindex < 0 || rindex < 0 || cindex > gd->auxwidth - 1 ||
        rindex > gd->auxlen[cindex] - 1)
      return serror("%s A[%d        ,%d]", GRERR_REF_OUT_OF_RANGE, cindex,
                    rindex);
    lastarg = lastarg->prev;
    lastarg->next->prev = NULL;
    comll_free_list(lastarg->next);
  ...
  case CLLNDateRef:
    ...
    if (dateindex < 0 || dateindex >= gd->sslength)
      return serror("%s: D[%d]", GRERR_BAD_DATE_REF, dateindex);
  ...
  case CLLNPvRef:
    ...
    if (cindex < 0 || cindex > gd->sswidth - 1)
      return serror("%s : PV[%d]", GRERR_ILLEGAL_REF, cindex);
  ...
  case CLLNPvRRef:
    ...
    cindex = DTOL(lastarg->dval);
    if (cindex < 0 || cindex > gd->sswidth - 1)
      return serror("%s : PVR[%d]", GRERR_ILLEGAL_REF, cindex);
    lastarg->ivec[0] = cindex;
...
*   FUNCTION        :   grfn_interp_func
    ...
    DESCRIPTION     :   Interprets f(...) where f is a reserved GRFN function
                                                Only the name of the function is
passed        , as well as the COMLL that corresponds only to the ARGUMENTS of
the function No specific COMLL element is yet built for the function name

                                                The COMLL_PTR args returned
ALWAYS points to the top of the COMLL corresponding to this function (not to
break the full COMLL construction in the YACC parsing) This top correponds to
the LAST argument input to the function (see the Grfn_Yacc.y file: reverse
building)


                                                It also performs error checking
on its arguments        , and modifies its arguments  appropriately.  If the
result of the function can be computed now        , this is done.


                        Example: If name is "max"        , args would have to
describe two  expressions        , "a"-->"b".  For "a" and "b" both real numbers
, we replace this with one node of type COMLL_REAL and value max(a ,b).
Otherwise        , we would place another node at the end of the function of
type COMLL_A_MAX: "a"-->"b"-->"max".


    PROBLEM         :   In order to pre-compute results when arguments are
                        known        , there is duplication within this function
of evaluation of all arithmetic functions with names        , like max        ,
exp        , etc. This is confusing in that other arithmetic functions are
precomputed directly by YACC. ( e.g. +        , -        , * ...). Also        ,
if a new arithmetic function with a name were added (sin(x) say)        , 3
modules rather than 2 would need modification. (grf_f_eval_event.cxx and
grf_f_lang.cxx to say how to evaluate it        , and grf_f_lnfsymtab.cxx to
declare it).
...
parser when encountering a NAME(...) type of input
...

{
  ...
  /* Point to the FIRST argument of this function ( the last one put into the
   * COMLL ) */
  lastarg = comll_gotobot(args);
  ...
  case CLLNAuxFunc:
    ...
    if (gs.type == COMLL_X_INTERP) { /* Interpolate Function */

      /* NB. Since we have interp(r1        ,r2        ,x)        , then r1 and
         r2 must be deterministic        , but x may not be. Hence r1 and r2
         will be the last two nodes of args; there may be any number of nodes
         above representing x */
    ...
    } else if (gs.type == COMLL_X_PVINTERP) { /* Interpolate Function */

      /* PVInterp(AuxIndex        ,StartCol        ,ColSkip        ,x).  The
         first three arguments are deterministic        , but x may not be.
         Hence the deterministic arguments will be in the last be the last three
         nodes of args; there may be any number of nodes above representing x */
  ...
  case CLLNArithFunc:
    ...
    if (((gs.type == COMLL_A_BLKSCHLS) || (gs.type == COMLL_A_BLKSCHLSNRM) ||
         (gs.type == COMLL_A_BSSABR) || (gs.type == COMLL_A_SPREADNUM)) &&
    ...
    if (numenddetargs == gs.ndargs + gs.nrargs) {
      ...
      case COMLL_A_SPREADNUM:
        ...
        error = OptSpreadNew(fwdx, a, sigx, fwdy, -b, sigy, k, t, rho, call_put,
                             &x);
...
/******************************************************************************\
    ...
    DESCRIPTION     :   Interprets a string like "CPN" or "I" or "SEMI".
                        In this case        , c points to a blank COMLL_PTR
allocated from within the yacc code.
...

{
  ...
  case CLLNNxt:
    ...
    c->type = COMLL_REAL;
    if (gd->I > gd->sslength - 2)
      return serror("Invalid NXT keyword");
  ...
  case CLLNPrv:
    ...
    if (gd->I <= 0)
      return serror("Invalid PRV keyword");
    c->type = COMLL_REAL;
...
/* --------------------------------------------------------------------------------------------
 */
/* Aggregates into a full status (row        , tableau...) a single status (for
 * the cell) */
void GrfnAggregateStatus(GrfnCellStatus full_status,
                         GrfnCellStatus cell_status) {
  int i;

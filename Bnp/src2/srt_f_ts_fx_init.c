...
/* Loop on all the elements of the Term Sturcture */
	...
	{	
		...
		*sigma = realloc (*sigma, (*sigma_n) * sizeof (double));
		(*sigma_date)[*sigma_n-1] = (double) (((FxTermStructVal*)l->element->val.pval) -> date);
		(*sigma)[*sigma_n-1] = ((FxTermStructVal*)l->element->val.pval) -> sigx;
	
		l = l->next;
	}

	/*------------- Get the correlation term structure-----------------------------*/
	...
	*corr  = (double*) malloc(sizeof(double));
	...
	while(psCorr_lc != NULL)
	{
		...
		if(psCorrval->ncorrel < 3)
			return serror("Fatal error in srt_f_display_FX_TermStruct: Wrong number of correlation"); 
		...
		*/
		...
		err = srt_f_get_corr_from_CorrList(sCorrlist,		
					...
					szForUndName,									 
					time,
					&(*corr)[corr_start_index]);
		
		if(err) return serror("Fatal Error in srt_f_get_corr_from_CorrList");

		err = srt_f_get_corr_from_CorrList(sCorrlist,		
					szDomUndName,									    
					szFxUndName,									 
					time,
					&(*corr)[corr_start_index+1]);
...
/* Current index for tau and sigma start at a date >= today */
	while ( (cur_sig < num_sig) &&
		 ((sig_data[0][cur_sig]-today) <= EPSILON) ) cur_sig++;
	...
	/* Insert the TermStructAtom in the TS (==linked ts sorted by key = date) */
		srt_f_lstins(
				*ts,
				"BsTsAtom", 
				tsval->date, 
				OBJ_PTR_FX_TermStruct,
				(void*)tsval,
				&srt_f_FxTermStructValFree,
				&ticker);
	}
...
/* If not lgm or vasicek / lgm or vasicek : nothing else to be done */
	if( (dom_mdl_type !=LGM && dom_mdl_type !=VASICEK) || (for_mdl_type !=LGM && for_mdl_type !=VASICEK) )
		return NULL;
...
/* Gets the Domestic Underlying */
	...
	if (err){ 
	return serror("Can not get the domestic term struct in srt_f_init_FX_STOCH_RATES_TermStruct.");
	}
...
/* Loops on the Domestic Term Struct dates to add them to the FX one */
	...
	{		
		...
		/* Moves on to the FX date just after (or on) this date */
			while((fx_atom!=NULL) &&
				( ((FxTermStructVal*)fx_atom->element->val.pval)->time < dom_tsval1F->time) ) 
	...
	else
	{
		...
		/* Moves on to the FX date just after (or on) this date */
			while((fx_atom!=NULL) &&
				( ((FxTermStructVal*)fx_atom->element->val.pval)->time < dom_tsval2F->time) ) 
				fx_atom = fx_atom->next;
			if(fx_atom ==NULL)
				fx_atom = (*fx_ts)->tail;
...
/* Loops on the Foreign Term Struct dates to add them to the FX one */
	...
	if (get_mdldim_from_irund(for_und) != TWO_FAC)
	{
		...
		/* Moves on to the FX date just after (or on) this date */
			while((fx_atom!=NULL) &&
				( ((FxTermStructVal*)fx_atom->element->val.pval)->time < for_tsval1F->time) ) 
				fx_atom = fx_atom->next;
			if(fx_atom ==NULL)
				fx_atom = (*fx_ts)->tail;
	...
	else
	{
		...
		/* Moves on to the FX date just after (or on) this date */
			while((fx_atom!=NULL) &&
				( ((FxTermStructVal*)fx_atom->element->val.pval)->time < for_tsval2F->time) ) 
				fx_atom = fx_atom->next;
			if(fx_atom ==NULL)
				fx_atom = (*fx_ts)->tail;
...
/* ---------  Add Domestic and Foreign specific information in the FX Term Struct ------- */
...
/* Loops on all the Dates of the Fx Term Struct to add ... */
	...
	/* Gets all the Correlations for this date */
		err = srt_f_get_corr_from_CorrList( corrlist,dom_und_name, for_und_name,fx_tsval->time,&fx_tsval->rhofd);
		err = srt_f_get_corr_from_CorrList( corrlist,undName, for_und_name,fx_tsval->time,&fx_tsval->rhofx);
		err = srt_f_get_corr_from_CorrList( corrlist,dom_und_name, undName,fx_tsval->time,&fx_tsval->rhodx);
		if (err)
		{
	...
	/* Stores the information in the TermStructVal */
		for(i=0;i<2;i++)
		{
			...
			fx_tsval->StochRates_Ts[i].lambda = (1.0/fx_tsval->StochRates_Ts[i].tau);
			fx_tsval->StochRates_Ts[i].F	 = F_func(time,subls);
			G_H_func(time,subls,&fx_tsval->StochRates_Ts[i].G,&fx_tsval->StochRates_Ts[i].H);
...
/* Builds the Fx Cumulative Volatilities (not used in this case, but why not) */
...
/* Builds the Nice Integrals we need for the Fx Stoch Rates with Jumping (LGM/LGM) */
...
Err err=0;
	...
	while ( lc->next != NULL ) 
	{
		...
		c->M_fd = p->M_fd
				 +p->rhofd*(temp/(domF*forF))*domPsi*forPsi*temp3
...
Err err=0;
	...
	while ( lc->next != NULL ) 
	{
		lc = lc->next;
		c = (FxTermStructVal*) lc->element->val.pval; 
		c->H_fd = c->StochRates_Ts[1].F*(c->R_fd-c->StochRates_Ts[0].Psi*c->O_fd);
...
Err err=0;
	...
	while ( lc->next != NULL ) 
	{
		...
		c->Q_fd	=	p->Q_fd
					-forF*p->O_fd*domPsi*temp
					-domF*forF*p->O_fd*domtau*(temp-temp1)
					-p->rhofd*domsig*forsig*domPsi/(domF*lambda)*(temp2-temp)
					-p->rhofd*domsig*forsig*domtau/lambda*(temp2-temp-time+temp1)
					+forF*p->P_fd*temp
					+p->rhofd*domPsi*domsig*forsig/(domF*lambda)*(temp2-temp)
					+p->rhofd*domsig*forsig*(temp2/lambda-temp/lambda-time*fortau+temp*fortau)*domtau;
...
SrtLst *lp, *lc;	
double time;
double domtau,fortau,lambda,domsig,forsig,domF,forF,domPsi,temp,temp1,temp2;
Err err=0;
	lc = l->head;	
	c = (FxTermStructVal*)lc->element->val.pval;
	c->U_fd = 0.0;
	...
	while ( lc->next != NULL ) 
	{
		...
		temp2  = (1-exp(-lambda*time))/lambda;
	 	...
	 	c->U_fd = p->U_fd
				+forF*domPsi*p->O_fd*temp
				+p->O_fd*domF*forF*domtau*(temp-temp2)
				+domPsi*(p->rhofd/lambda)*(domsig*forsig/(domF))*(temp1-temp)
				+p->rhofd*(domsig*forsig*domtau/lambda)*(temp1-temp-time+temp2);
...
Err err=0;
	...
	while ( lc->next != NULL ) 
	{
		...
		c->W_fd = p->W_fd+p->rhofd*(dDomSig*dForSig/dDomF)*dDomPsi*dTemp
...
Err err=0;
	...
	while ( lc->next != NULL ) 
	{
		...
		c->X_dx = p->X_dx
				  +p->rhodx*p->sigx*(domsig/sqdomF)*temp;
...
Err err=0;
	...
	while ( lc->next != NULL ) 
	{
		...
		c->V_dx = p->V_dx
				  +p->rhodx*p->sigx*(domsig/domF)*temp;
...
Err err=0;
	...
	while ( lc->next != NULL ) 
	{
		...
		c->W_dx = p->W_dx
				  +p->rhodx*p->sigx*domsig*(domPsi/domF)*temp
...
Err err=0;
	...
	while ( lc->next != NULL ) 
	{
		...
		c->V_fx = p->V_fx
				  +p->rhofx*p->sigx*(forsig/forF)*temp;
...
Err err=0;
	...
	while ( lc->next != NULL ) 
	{
		...
		c->W_fx = p->W_fx
				  +p->rhofx*p->sigx*forsig*(forPsi/forF)*temp

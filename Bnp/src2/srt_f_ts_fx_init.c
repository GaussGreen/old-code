    ...
    TermStruct** ts)
{
    ...
    if (mdl_type == FX_STOCH_RATES)
    {
        ...
        err = srt_f_init_FX_STOCH_RATES_TermStruct(
            undName, dom_irund, for_irund, ts, today, sig, sig_cols, nsig);
    ...
    *corr      = (double*)malloc(sizeof(double));
    ...
    while (psCorr_lc != NULL)
    {
        ...
        if (psCorrval->ncorrel < 3)
            return serror(
                "Fatal error in srt_f_display_FX_TermStruct: Wrong number of correlation");
        ...
        */
        ...
        err = srt_f_get_corr_from_CorrList(
            sCorrlist, szDomUndName, szForUndName, time, &(*corr)[corr_start_index]);

        if (err)
            return serror("Fatal Error in srt_f_get_corr_from_CorrList");

        err = srt_f_get_corr_from_CorrList(
            sCorrlist, szDomUndName, szFxUndName, time, &(*corr)[corr_start_index + 1]);

        if (err)
            return serror("Fatal Error in srt_f_get_corr_from_CorrList");

        err = srt_f_get_corr_from_CorrList(
            sCorrlist, szForUndName, szFxUndName, time, &(*corr)[corr_start_index + 2]);
...

{
    ...
    err = srt_f_init_FX_NORMLOG_TermStruct(fx_ts, today, sig_data, sig_cols, num_sig);
    if (err)
    {
        return err;
    }
    ...
    {
        return serror(
            "Can not get the domestic term struct in srt_f_init_FX_STOCH_RATES_TermStruct.");
    }
    ...
    while (fx_atom != NULL)
    {
        ...
        err = srt_f_get_corr_from_CorrList(
            corrlist, dom_und_name, for_und_name, fx_tsval->time, &fx_tsval->rhofd);
        err = srt_f_get_corr_from_CorrList(
            corrlist, undName, for_und_name, fx_tsval->time, &fx_tsval->rhofx);
        err = srt_f_get_corr_from_CorrList(
            corrlist, dom_und_name, undName, fx_tsval->time, &fx_tsval->rhodx);
        if (err)
        {
...
static Err make_N_fx_vector(TermStruct* l)
{
    ...
    while (lc->next != NULL)
    {
        ...
        c->N_fx =
            p->N_fx + p->M_fx * forF * temp + p->rhofx * p->sigx * forsig * fortau * (time - temp);
    }
    return err;
}
...
static Err make_M_fd_vector(TermStruct* l)
{
    ...
    while (lc->next != NULL)
    {
        ...
        c->M_fd = p->M_fd + p->rhofd * (temp / (domF * forF)) * domPsi * forPsi * temp3 +
...
static Err make_Q_fd_vector(TermStruct* l)
{
    ...
    while (lc->next != NULL)
    {
        ...
        c->Q_fd = p->Q_fd - forF * p->O_fd * domPsi * temp -
                  ...
                  p->rhofd * domsig * forsig *
                      (temp2 / lambda - temp / lambda - time * fortau + temp * fortau) * domtau;
    }
    return err;
}
...
static Err make_Y_dx_vector(TermStruct* l)
{
    ...
    while (lc->next != NULL)
    {
        ...
        c->Y_dx = p->Y_dx + p->rhodx * p->sigx * domsig * (domPsi / sqdomF) * temp1 +

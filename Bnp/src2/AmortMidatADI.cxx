...
//-------------------------------------------------------------------------------------------------------
//	Description: common function parameters needed by the binary function
// PFN_BI
//-------------------------------------------------------------------------------------------------------
typedef struct _COMM_PFN_BI_ {
  const PFN_COEFF_3DPDE m_FUNC_; // tertiary function PFN_COEFF_3DPDE(double x
                                 //       ,double aux        ,double t ,void*)
...
static void __stdcall _Update_Mu(
    /// State X
    const double *pdX_Begin, const double *pdX_End,
    // Auxiliary State X
    const double *pdAuxX_Begin, const double *pdAuxX_End,
    // Time
    const double *pdT,
    /// Common parameters used in drift functor
    void *pComm,
    // functor to update the drift : _MU_FUNC_(State X        , Aux X        , T
    // , Comm) NB: first argument must be State X        , not Auxiliary X
    PFN_COEFF_3DPDE _MU_FUNC_,
    /// Results
    /// Row Indices = [nAuxX_Begin        ,...)         , Col Indices =
    /// [nX_Begin        , ...)
    double **ppdHalf_Mu_InvDeltaX, // based
    int nX_Begin, int nAuxX_Begin) {
  ...
  if (pdX_End - pdX_Begin == 1)
    ...
    _transform_matrix(pdX_Begin, pdX_End - pdX_Begin, sizeof(*pdX_Begin),
                      pdAuxX_Begin, pdAuxX_End - pdAuxX_Begin,
                      sizeof(*pdAuxX_Begin), sizeof(**ppdHalf_Mu_InvDeltaX),
                      _Half_Mu_InvDeltaX_PFN_BI, &Comm_BI, ppdHalf_Mu_InvDeltaX,
                      nAuxX_Begin, nX_Begin, 0);
  }
}
...
//				 pdHalf_SigXSqrd_InvDeltaXSqrd = 0.5 *
// Sig(Time)/DeltaX^2
//-------------------------------------------------------------------------------------------------------
static void __stdcall _Update_Slice(
    ...
    void *pComm,
    /// functors to update local vol : _SIGMA_Sqrd_FUNC_(State X        , Aux
    /// State X
    ///       , Time)
    // NB: first argument must be State X        , not Auxiliary X
    PFN_COEFF_3DPDE _SIGMA_Sqrd_FUNC_,
    /// functors to update drift : _MU_FUNC_(State X        , Aux State X ,
    /// Time)
    // NB: first argument must be State X        , not Auxiliary X
    PFN_COEFF_3DPDE _MU_FUNC_,
    /// Results
  ...
  if (pdX_End - pdX_Begin == 1)
    ...
    // if there is no diffusion in the X direction        , then no updating is
    // needed!
    if (dDelta_X == 0.)
      return;

    // update volatility-related info
    *pdHalf_SigSqrd_InvDeltaXSqrd =
        0.5 * _SIGMA_Sqrd_FUNC_(0, 0, pdT, pComm) / dDeltaSqrd_X;

    // update drift-related info
    _Update_Mu(pdX_Begin, pdX_End, pdAuxX_Begin, pdAuxX_End, pdT, pComm,
               _MU_FUNC_, ppdHalf_Mu_InvDeltaX, nX_Begin, nAuxX_Begin);
  }
}
...
static void __stdcall _lu_decompose_tridiag_(
    ...
    double dInvHalfDeltaT, double dHalf_InvDeltaXSqrd_SigSqrd,
    const double *pdHalf_InvDeltaX_Mu_Begin,
    const double *pdHalf_InvDeltaX_Mu_End,
    // results
    double *pdDInv_Begin, double *pdL_Begin, double *pdU_Begin) {
  ...
  *pdDInv_Begin = 1. / dC;
  ...
  if (pdHalf_InvDeltaX_Mu_End - pdHalf_InvDeltaX_Mu_Begin >= 3) {
    ...
    double *pdU = pdU_Begin + 1, *pdDInv = pdDInv_Begin + 1,
           *pdL = pdL_Begin + 1;
    ...
    *pdU_Begin = dOffDiag_Imp_front; // NB : boundary
    ...
    for (pdMuQ = pdHalf_InvDeltaX_Mu_Begin + 1;
         pdMuQ < pdHalf_InvDeltaX_Mu_End - 2; ++pdMuQ, ++pdL, ++pdDInv) {
    ...
    // boundary
    {
      ...
      *(pdDInv + 1) =
          dDiag_Imp_back + (*pdL) * (dHalf_InvDeltaXSqrd_SigSqrd + *pdMuQ);
      _ASSERTE((*(pdDInv + 1)) != 0.);
      *(pdDInv + 1) = 1. / (*(pdDInv + 1));
    }
  }
...
//-------------------------------------------------------------------------------------------------------
//	Description: explicit scheme in the State X        ,traversing from
//ppd_From
// to ppd_To 						at the boundary        , the second
// derivative w.r.t to State X is 0
//
//	Returns : ppd_To
//-------------------------------------------------------------------------------------------------------
static void __stdcall _ADI_SingleSlice_Explicit(
    ...
    // "From matrix" - rows represent State X and cols represent Auxiliary State
    // X
    ...
    // drift at time slice from	=  0.5 *  Mu (State X        , Aux State X ,
    // T)/DeltaX
    // NB: same size of the TRANSPOSE of ppd_From
    // Row Indices = [nAuxX_Begin        ,...nAuxX_End)         , Col Indices =
    // [nX_Begin        ,
    // ...nX_End)
    const double **ppdHalf_Mu_InvDeltaX_from,

    // local vol at time slice from        , states - independent = 0.5 *
    // Sig(T)/Delta_X^2
    double dHalf_SigSqrd_InvDeltaXSqrd_from,

    /// Result
    /// same size of the TRANSPOSE ppd_From
    /// Rows = [nAuxX_Begin        , ... nAuxX_End) ; Cols =  [nX_Begin        ,
    /// ... nX_End)
    double **ppd_To) {
  const double dC = dInvHalfDeltaT - 2. * dHalf_SigSqrd_InvDeltaXSqrd_from;
  const double *pdCenter_from = ppd_From[nX_Begin], *pdUp_from = 0,
               *pdDown_from = 0;
  int nX, nAuxX;
    ...
    /// handle the boundary - nRow_Begin
    {
      ...
      for (nAuxX = nAuxX_Begin; nAuxX < nAuxX_End; ++nAuxX) {
        ...
        ppd_To[nAuxX][nX_Begin] =
            (dInvHalfDeltaT - dMuQ) * pdCenter_from[nAuxX] +
            dMuQ * pdUp_from[nAuxX];
      }
    }
...
//-------------------------------------------------------------------------------------------------------
//	Description: implict scheme in the State X        ,traversing from
//ppd_From to
// ppd_To 						at the boundary        , the second
// derivative w.r.t to State X is 0
//
...
static void __stdcall _ADI_SingleSlice_Implicit(
    ...
    double dInvHalfDeltaT,
    // "From matrix" - rows represent State X and cols represent Auxiliary State
    // X On input : From Matrix On output : To Matrix
    double **ppd_From,
    int nX_Begin,    // starting row index of pdd_from
    int nX_End,      // end row index of pdd_from        , not inclusive
    int nAuxX_Begin, // starting col index of pdd_from
    int nAuxX_End,   // end row index of pdd_from        , not inclusive
    // drift at time slice from	=  0.5 *  Mu (State X        , Aux State X , T)
    // /DeltaX
    // NB: same size of the TRANSPOSE of ppd_From
    // Row Indices = [nAuxX_Begin        ,...nAuxX_End)         , Col Indices =
    // [nX_Begin        ,
    ...
    /// Rows = [nAuxX_Begin        , ... nAuxX_End) ; Cols =  [nX_Begin        ,
    /// ... nX_End)
    double **ppdStorage, double **ppd_DInv, double **ppd_LSub,
    double **ppd_USup) {
  ...
  //// decompose LU
  for (nAuxX = nAuxX_Begin; nAuxX < nAuxX_End; ++nAuxX) {
    const double dInvDeltaXR_MuR_front =
        2. * ppdHalf_MuX_InvDeltaX_to[nAuxX][nX_Begin];
    const double dInvDeltaXR_MuR_back =
        2. * ppdHalf_MuX_InvDeltaX_to[nAuxX][nX_End - 1];
    ...
    _lu_decompose_tridiag_(
        dDiag_front, dOffDiag_front, dDiag_back, dOffDiag_back, dInvHalfDeltaT,
        dHalf_SigXSqrd_InvDeltaXSqrd_to,
        ppdHalf_MuX_InvDeltaX_to[nAuxX] + nX_Begin,
        ppdHalf_MuX_InvDeltaX_to[nAuxX] + nX_End, ppd_DInv[nAuxX] + nX_Begin,
        ppd_LSub[nAuxX] + nX_Begin, ppd_USup[nAuxX] + nX_Begin);
  }
  ...
  // Lower Ly = b
  {
    ...
    for (nAuxX = nAuxX_Begin; nAuxX < nAuxX_End; ++nAuxX) {
      ...
      for (nX = nX_Begin + 1; nX < nX_End; ++nX)
        pdCenter_to[nX] =
            ppd_From[nX][nAuxX] - pdLSub[nX - 1] * pdCenter_to[nX - 1];
    }
  }
  ...
  // Lower Ux = y
  {
    ...
    for (nAuxX = nAuxX_Begin; nAuxX < nAuxX_End; ++nAuxX)
      pdCenter_to[nAuxX] =
          ppd_From[nAuxX][nX_End - 1] * ppd_DInv[nAuxX][nX_End - 1];
    ...
    for (nAuxX = nAuxX_Begin; nAuxX < nAuxX_End; ++nAuxX) {
      ...
      for (nX = nX_End - 2; nX >= nX_Begin; --nX)
        ppdStorage[nX][nAuxX] =
            (pdCenter_from[nX] - pUSup[nX] * ppdStorage[nX + 1][nAuxX]) *
            pd_DInv[nX];
    }
  }
}
...
static void __stdcall _ADI_SingleSlice(
    ...
    /// On output : "To Matrix"
    /// Row Indices = [nX_Begin        ,...)         , Col Indices =
    /// [nAuxX_Begin        , ...)
  ...
  _ADI_SingleSlice_Explicit(dInvHalfDeltaT, ppdU, nX_Begin, nX_End, nAuxX_Begin,
                            nAuxX_End, ppdHalf_Mu_InvDeltaX_from,
                            dHalf_SigSqrd_InvDeltaXSqrd_from, ppdStorage);

  /// implicit in secondary factor
  _ADI_SingleSlice_Implicit(dInvHalfDeltaT, ppdStorage, nAuxX_Begin, nAuxX_End,
                            nX_Begin, nX_End, ppdHalf_Mu_InvDeltaAuxX_to,
                            dHalf_SigSqrd_InvDeltaAuxXSqrd_to, ppdU,
                            ppd_XDInvAuxX, ppd_XLAuxX, ppd_XUAuxX);

  // explict in secondary factor
  _ADI_SingleSlice_Explicit(dInvHalfDeltaT, ppdStorage, nAuxX_Begin, nAuxX_End,
                            nX_Begin, nX_End, ppdHalf_Mu_InvDeltaAuxX_from,
                            dHalf_SigSqrd_InvDeltaAuxXSqrd_from, ppdU);

  /// implicit in dominant factor
  _ADI_SingleSlice_Implicit(dInvHalfDeltaT, ppdU, nX_Begin, nX_End, nAuxX_Begin,
                            nAuxX_End, ppdHalf_Mu_InvDeltaX_to,
                            dHalf_SigSqrd_InvDeltaXSqrd_to, ppdStorage,
                            ppd_AuxXDInvX, ppd_AuxXLX, ppd_AuxXUX);
}

//-------------------------------------------------------------------------------------------------------
//	Description:  initializes the storage needed by the call to
//_ADI_MultipleSlices
//
//	Returns : storage
//-------------------------------------------------------------------------------------------------------
...
const char *ADI_MultipleSlices(
    ...
    const double *pdT_Begin, const double *pdT_End,
    // orthogonal states
  ...
  const char *szErr =
      Init_ADIStorage(&store, pdX_End - pdX_Begin, pdAuxX_End - pdAuxX_Begin);
  ...
  _ADI_MultipleSlices(pdT_Begin, pdT_End, pdX_Begin, pdX_End, pdAuxX_Begin,
                      pdAuxX_End, pFuncArg, _SIGMAX_Sqrd_FUNC_, _MUX_FUNC_,
                      _SIGMAAuxX_Sqrd_FUNC_, _MUAuxX_FUNC_, ppdU, nX_Begin,
                      nAuxX_Begin, &store);
...
void _ADI_MultipleSlices(
  ...
  double **ppdHalf_Mu1_InvDeltaX1_from = (double **)_alloca_matrix(
      pm += nSize_AuxX, pdata += nSize_Grid, nAuxX_Begin, nAuxX_End, nX_Begin,
      nX_End, sizeof(double));
  ...
  /// Row Indices = [nX_Begin        ,...)         , Col Indices = [nAuxX_Begin
  /// , ...)
  double **ppdHalf_Mu2_InvDeltaX2_from =
      (double **)_alloca_matrix(pm += nSize_AuxX, pdata += nSize_Grid, nX_Begin,
                                nX_End, nAuxX_Begin, nAuxX_End, sizeof(double));
  double **ppdHalf_Mu2_InvDeltaX2_to =
      (double **)_alloca_matrix(pm += nSize_X, pdata += nSize_Grid, nX_Begin,
                                nX_End, nAuxX_Begin, nAuxX_End, sizeof(double));
  double **ppd_XLAuxX =
      (double **)_alloca_matrix(pm += nSize_X, pdata += nSize_Grid, nX_Begin,
                                nX_End, nAuxX_Begin, nAuxX_End, sizeof(double));
  ...
  _Update_Slice(pdT, pdX_Begin, pdX_End, pdAuxX_Begin, pdAuxX_End, pFuncArg,
                _SIGMAX_Sqrd_FUNC_, _MUX_FUNC_, pSigXQ_from, ppMuXQ_from,
                nX_Begin, nAuxX_Begin);

  _Update_Slice(pdT, pdAuxX_Begin, pdAuxX_End, pdX_Begin, pdX_End, pFuncArg,
                _SIGMAAuxX_Sqrd_FUNC_, _MUAuxX_FUNC_, pSigAuxXQ_from,
                ppMuAuxXQ_from, nAuxX_Begin, nX_Begin);
  ...
  for (pdT = pdT_Begin + 1; pdT < pdT_End; ++pdT) {
    ...
    if (dDeltaT != 0.) {
      ...
      // Update state info for X1 and X2 at slice from
      _Update_Slice(pdT, pdX_Begin, pdX_End, pdAuxX_Begin, pdAuxX_End, pFuncArg,
                    _SIGMAX_Sqrd_FUNC_, _MUX_FUNC_, pSigXQ_to, ppMuXQ_to,
                    nX_Begin, nAuxX_Begin);

      _Update_Slice(pdT, pdAuxX_Begin, pdAuxX_End, pdX_Begin, pdX_End, pFuncArg,
                    _SIGMAAuxX_Sqrd_FUNC_, _MUAuxX_FUNC_, pSigAuxXQ_to,
                    ppMuAuxXQ_to, nAuxX_Begin, nX_Begin);

      _ADI_SingleSlice(pdX_Begin, pdX_End, pdAuxX_Begin, pdAuxX_End, ppdU,
                       nX_Begin, nAuxX_Begin, dInvHalfDeltaT, *pSigXQ_from,
                       *pSigAuxXQ_from, ppMuXQ_from, ppMuAuxXQ_from, *pSigXQ_to,
                       *pSigAuxXQ_to, ppMuXQ_to, ppMuAuxXQ_to, ppd_Storage,
                       ppd_AuxXLX, ppd_AuxXDInvX, ppd_AuxXUX, ppd_XLAuxX,
                       ppd_XDInvAuxX, ppd_XUAuxX);
...
static double __stdcall _SIGMA1_Sqrd_TEST_(const double *pdX1,
                                           const double *pdX2,
                                           const double *pdT, void *pAux) {
  pdT, pAux;
...
static double _LAMBDA1_TEST_() { return _lambda_; }

static double _ALPHA_TEST_() { return _alpha_; }

static double _GAMMA_TEST_() { return _gamma_; }

static double _RHO_TEST_() { return _rho_; }

/// drift can be a function of states and time
static double __stdcall _MU1_TEST_(const double *pdX1, const double *pdX3,
                                   const double *pdT, void *pAux) {
...
// Sigma is a function of time only
static double _SIGMA2_Sqrd_TEST_(const double *pdX1, const double *pdX2,
                                 const double *pdT, void *pAux) {
...
// Sigma is a function of time only
static double __stdcall _SIGMA3_Sqrd_TEST_(const double *pdX1,
                                           const double *pdX2,
                                           const double *pdT, void *pAux) {
...
/// drift can be a function of states and time
static double __stdcall _MU3_TEST_(const double *pdX3, const double *pdX1,
                                   const double *pdT, void *pAux) {
...
static const char *__adi_tester_(int nR_Begin, int nR_End, int nC_Begin,
                                 int nC_End, int nFlat) {
  const char *szErr = 0;
  int nr, nc;
  ...
  const double dTerminalVarX1 =
      (dLambda1 == 0.) ? dT * _SIGMA1_Sqrd_TEST_(0, 0, 0, 0)
                       : _SIGMA1_Sqrd_TEST_(0, 0, 0, 0) *
                             (1 - exp(2 * dLambda1 * dT)) / (-2 * dLambda1);

  const double dTerminalVarX2 =
      (dLambda2 == 0.) ? dT * _SIGMA2_Sqrd_TEST_(0, 0, 0, 0)
                       : _SIGMA2_Sqrd_TEST_(0, 0, 0, 0) *
                             (1 - exp(2 * dLambda2 * dT)) / (-2 * dLambda2);

  const double dTerminalCovar12 =
      (dLambda1 + dLambda2) == 0.
          ? dT *
                sqrt(_SIGMA1_Sqrd_TEST_(0, 0, 0, 0) *
                     _SIGMA2_Sqrd_TEST_(0, 0, 0, 0)) *
                dRho
          : sqrt(_SIGMA1_Sqrd_TEST_(0, 0, 0, 0) *
                 _SIGMA2_Sqrd_TEST_(0, 0, 0, 0)) *
                dRho * (1 - exp(dLambda1 * dT + dLambda2 * dT)) /
                (-dLambda1 - dLambda2);
  ...
  const double dDeltaX1 =
      (2. * 7. * sqrt(dTerminalVarX1)) / (double)(nNumX1 - 1);
  const double dDeltaX3 =
      (2. * 7. * sqrt(dTerminalVarX3)) / (double)(nNumX2 - 1);
  const double dDeltaT = (dTL - dTH) / ((double)nNumT);
  double *pdT_Begin = _alloca((nNumT + 1) * sizeof(double)), *pdT = 0;
  const double *pdT_End = pdT_Begin + nNumT + 1;

  int *pnSize_X1 = _alloca((nNumT + 1) * sizeof(int)), *pnX1;
  int *pnSize_X3 = _alloca((nNumT + 1) * sizeof(int)), *pnX3;
  ...
  for (pdT = pdT_Begin, pnX1 = pnSize_X1, pnX3 = pnSize_X3;
       pnX1 < pnSize_X1 + nNumT + 1; ++pdT, ++pnX1, ++pnX3) {
    const double dT = *pdT;
    const double dTerminalVarX1 =
        (dLambda1 == 0.) ? dT * _SIGMA1_Sqrd_TEST_(0, 0, 0, 0)
                         : _SIGMA1_Sqrd_TEST_(0, 0, 0, 0) *
  ...
  szErr = ADI_MultipleSlices(pdT_Begin, // backwards
                             pdT_End, grid.m_pdX1_Begin, grid.m_pdX1_End,
                             grid.m_pdX3_Begin, grid.m_pdX3_End,
                             0, // FuncArg
                             _SIGMA1_Sqrd_TEST_, _MU1_TEST_, _SIGMA3_Sqrd_TEST_,
                             _MU3_TEST_, ppdU, nC_Begin, nR_Begin);

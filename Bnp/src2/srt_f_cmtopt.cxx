...
typedef struct {
  ...
  double yr_to_exp, cms_rate, cmt_rate, atm_sigma_beta, sabr_alpha, sabr_beta,
      sabr_rho, spot_spread, fwd_spread, *cmt_calibration_strikes;
...
static Err set_static_params_for_calib(
    String swp_yc_name, SrtDiffusionType swp_vol_type, double yr_to_exp,
    double cms_rate, double cmt_rate, double atm_sigma_beta, double sabr_alpha,
    double sabr_beta, double sabr_rho, double spot_spread, double fwd_spread,
    double *cmt_calibration_strikes, long num_strike)

{
  ...
  static_params.cxxms_rate = cms_rate;
  static_params.cxxmt_rate = cmt_rate;
  ...
  static_params.cxxmt_calibration_strikes = cmt_calibration_strikes;
...
static Err from_optparam_to_CMT_VOL_parameters(double *pdOptimParams,
                                               double *spread_mean_reversion,
...
static Err levenberg_CMT_VOL_funcs(double instr_index, double *pdOptimParams,
  ...
  double atm_sigma_beta = static_params.atm_sigma_beta,
         sabr_alpha = static_params.sabr_alpha,
         sabr_beta = static_params.sabr_beta, sabr_rho = static_params.sabr_rho,
         spot_spread = static_params.spot_spread,
         *cmt_calibration_strikes = static_params.cxxmt_calibration_strikes,
         cms_rate = static_params.cxxms_rate,
         cmt_rate = static_params.cxxmt_rate;
  ...
  if (are_calib_parameters_within_band(
          pdOptimParams, lNumParams, static_params.ppdParamBounds) == SRT_NO) {
    *value = SRT_BIG;
    return NULL;
  }
  ...
  err = srt_f_get_cmt_vol(swp_yc_name, swp_vol_type, yr_to_exp, cms_rate,
                          cmt_rate, cmt_calibration_strikes[(long)instr_index],

                          atm_sigma_beta, sabr_alpha, sabr_beta, sabr_rho,

                          spot_spread, spread_tau, spread_fwd_level,
                          spread_loc_vol, rho, SRT_TRUE, &cmt_vol);
...
static Err levenberg_calib_funcs(double instr_index, double pdOptParams[],
  ...
  for (i = 1; i <= lNumParams; i++) {
    ...
    err = levenberg_CMT_VOL_funcs(instr_index, pdOptParams, &(deriv[i]),
                                  lNumParams);
    if (err)
      return err;
...
Err srt_f_calibrate_CMT_CMS_spread_parameters(
    double yr_to_exp, String swp_yc_name, String swp_vol_type_str,
    double atm_sigma_beta, double sabr_alpha, double sabr_beta, double sabr_rho,

    double cmt_rate, double spot_spread, double fwd_spread,

    double spread_mean_reversion_guess, double spread_fwd_level_guess,
    double spread_loc_vol_guess, double rho_guess,

    long num_strike, double *cmt_calibration_strikes, double *cmt_vols,
  ...
  err = set_static_params_for_calib(
      swp_yc_name, swp_vol_type, yr_to_exp, cms_rate, cmt_rate, atm_sigma_beta,
      sabr_alpha, sabr_beta, sabr_rho, spot_spread, fwd_spread,
      cmt_calibration_strikes, num_strike); /* NBR OF PARAMETER */
  if (err)
    return err;
  ...
  err = levenberg_marquardt(data, target, weight, ndata, opt_params, 3, 100,
  ...
  (*param)[2] =
      (fwd_spread - spot_spread * trunc_exp(-opt_params[1] * yr_to_exp)) /
...
/* COMPUTE THE NORMAL CMS VOLATILTY FROM THE CMS SABR PARAMATERS AND ADJUST IT
 * BY THE CONVEXITY OF THE SPREAD OVER CMT */
Err srt_f_get_cmt_vol(String swp_yc_name, SrtDiffusionType swp_vol_type,

                      double yr_to_exp, double cms_rate, double cmt_rate,
                      double cmt_strike,

                      /* SABR CMS PARAMETERS */
                      double atm_sigma_beta, double sabr_alpha,
                      double sabr_beta, double sabr_rho,
  ...
  if (use_mkt_fwd_spread == SRT_FALSE)
    fwd_spread = spread_fwd_level + (spot_spread - spread_fwd_level) *
                                        trunc_exp(-yr_to_exp / spread_tau);
  else
  ...
  err = srt_f_optsarbvol(cms_rate, cms_adjusted_strike, yr_to_exp,
                         atm_sigma_beta, sabr_alpha, sabr_beta, sabr_rho,
                         swp_vol_type, swp_vol_type, &cms_market_vol);
  if (err)
    return err;

  /* CONVERT THE CMS VOLATILITY  INTO NORMAL */
  err = srt_f_optsarbvol(cms_rate, cms_adjusted_strike, yr_to_exp,
                         cms_market_vol, sabr_alpha, sabr_beta, sabr_rho,
                         swp_vol_type, SRT_NORMAL, &cms_norm_vol);
  if (err)
    return err;

  /* COMPUTE THE NORMAL VOLATILITY OF THE CMT */
  spread_vol =
      spread_loc_vol * sqrt(0.5 * (1 - trunc_exp(-2 * yr_to_exp / spread_tau)) *
                            (spread_tau / yr_to_exp));
  cmt_norm_vol =
      sqrt(cms_norm_vol * cms_norm_vol - 2 * rho * cms_norm_vol * spread_vol +

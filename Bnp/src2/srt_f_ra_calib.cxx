...
static Err calib_bump(SrtProduct *product, CalibRA_ExData *ex_data, int nx) {
  Err err = NULL;
  TryBumpData data;
  ...
  gauleg(ex_data->R_ffx - 4.0 * data.std, ex_data->R_ffx + 4.0 * data.std,
         x - 1, w - 1, nx);
  ...
  /* Bracket the root */
  err = num_f_zbrac((quanto ? try_bump_quanto : try_bump), &x1, &x2, &f1, &f2,
                    &data);
  if (err)
    return err;

  /* Calibrate */
  err = num_f_zbrent((quanto ? try_bump_quanto : try_bump), x1, x2, f1, f2, tol,
                     &data, &result);
  if (err)
    return err;
...
Err srt_f_calib_range_accrual(char *yc_d, char *yc_f, SrtProduct *product,
                              int nex, long *ex_dates, double *sig_d,
                              double *sig_f, double *sig_fx, double lam_d,
                              double lam_f, double rho_df, double rho_ffx,
                              int nx) {
  Err err = NULL;
  int i, j, k;
  ...
  if (!yc_ptr)
    return serror("Yield curve %s not found", yc_d);
  ...
  for (i = 0, t1 = 0.0; i < nex; i++) {
    ...
    if (yc_f) {
      ...
      a_SdSf_Lf =
          a_SdSf_Lf * exp_Lf + sig_d[i] * sig_f[i] * (1.0 - exp_Lf) / lam_f;

      a_2Sf_Lf =
          a_2Sf_Lf * exp_Lf + sig_f[i] * sig_f[i] * (1.0 - exp_Lf) / lam_f;

      a_SfSx_Lf =
          a_SfSx_Lf * exp_Lf + sig_f[i] * sig_fx[i] * (1.0 - exp_Lf) / lam_f;
    ...
    err =
        product->RequestDfDates(product, ex_dates[i], &n_unds, &n_dfs, &dates);
    if (err)
      goto FREE_RETURN;
    if (yc_f && n_unds != 2) {
      err = serror("Product is not quanto");
      goto FREE_RETURN;
    }
    if (!yc_f && n_unds != 1) {
    ...
    /* Calculate target */
    err =
        product->Payoff(product, today, ex_dates[i], dfs, &(ex_data[i].target));
    if (err)
      goto FREE_RETURN;
    ...
    for (j = 0; j < n_unds; j++)
      for (k = 0; k < n_dfs[j]; k++) {
        ...
        gam[j][k] =
            (1.0 - exp(-lam[j] * (dates[j][k] - ex_dates[i]) * YEARS_IN_DAY)) /
            lam[j];
      }

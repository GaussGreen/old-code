...
/* Picks coef so the LGM price of a long swaption matches the market price when
        Gst = Gst + coef*dGst
   (G at start date)
        Gpay[i] = Gpay[i] + coef*dGpay[i]  , i=1  , ...  ,n	(G at pay
   dates) */
...
/* Picks coef so the LGM price of a long swaption matches the market price when
        Gst = Gst + coef*dGst
(G at start date) Gpay[i] = Gpay[i] + coef*dGpay[i]  , i=1  , ...  ,n	(G at
pay dates) and sqrt(zeta)*Gst is held constant at product */
static double FitOneSwapCap(long n, double *a, double DStart,
                            SrtReceiverType recpay,      /* the swaption */
...
swaptions match their market price when
        Gst = Gst + coef*dGst
(G at start date) Gpay[i] = Gpay[i] + coef*dGpay[i]  , i=1  , ...  ,n	(G at
pay dates) zeta(tEx) = sqzeta*sqzeta */
static double
                                            ...
                                            structure (NULL => not req'd) */
{
  ...
  *LGMtsPtrPtr = NULL;
  ...
  if (CalReq->LGMOneTwoFactor == 1) {
    ...
    /* Use constant sigma to construct zeta(t)  , then get G(t) by calibrating
    ...
    /* Calibrate on the "1 into k" swaptions to get G(t)  , then get zeta(t) by
    calibrating on the long "k into n-k" swaptions or the caplets  , depending
    on usecaps */
    case FixExp:
      error =
...
from the incoming fixed kappa: If usestarts != 0  , it will construct G(t) on
each start date of the caplets or swaptions  , and on the end date of the last
caplet (if usecaps=1) or tPay[n] (if usecaps!=1). If usestarts = 0  , it will
construct G(t) on the start date of the first caplet or swaption and on the end
date of the last caplet or tPay[n]   , and on the dates Gdates[j]  , j=1  ,...
, which fall between the first start date and the last end date. Note that
Gdate[0] is ignored.  In the call from LGMCalibration  , Gdate[] are the pay
dates of the fixed leg of the longest swaption used to calibrate It then finds
zeta(t) at tNow  , and at each exercise date of the set of caplets or swaptions
by calibrating to the set of caplets  (if usecaps=1) or long swaptions (if
usecaps!=1) in the calibration set CSPtr. */
...
j tStart[j] for j=1  ,...  ,nEx		start date for caplet j DStart[j] for
j=1  ,...  ,nEx		dis factor from tNow to the start date tEnd[j] for j=1
,...  ,nEx			end date for caplet j Dcap[j] for j=1  ,... ,nEx
dis factor to the end date of caplet j
cvgcap[j] for j=1  ,...  ,nEx		coverage from start date to end date for
caplet j Rfcap[j] for j=1  ,...  ,nEx		fixed rate for caplet j Vcap[j]
for j=1  ,...  ,nEx			market value at tNow for caplet j Note
the index j=0 is ignored in the above arrays. */
...
ifirst [j] for j=1  ,...  ,nEx		fixed leg pay dates are i = ifirst[j] to
nlong[j] nlong [j] for j=1  ,...  ,nEx
cvgfirst t[j] for j=1  ,...  ,nEx	first period coverage (from tStart[j] to
tPay[ifirst[j]]) Rlong [j] for j=1  ,...  ,nEx		fixed rate for swaption
j Vlong [j] for j=1  ,...  ,nEx		market price of swaption j Note the
             ...
             LGMCalSet *CSPtr)     /* Reference caplets */
{
  ...
  *LGMtsPtrPtr = NULL;
  ...
  /* Create term structure  , fill in  G dates  , calculate and fill in G values
   * from kappa */
...
sigma*sigma*(tEx[j]-tNow) on the exercise dates tEx[j]  , j=1  ,...  ,nEx. The
particular value of sigma used is irrelevent.
/* If usecaps ==1  , the following information must be set in CSPtr:
tNow						evaluation date
...
cvgcap[j] for j=1  ,...  ,nEx	coverage from start date to end date for caplet
j Rfcap[j] for j=1  ,...  ,nEx	fixed rate for caplet j Vcap[j]	for j=1 ,...
,nEx		market value at tNow for caplet j
...
we need to calibrate to tPay[i] for i=1  ,...  ,nPay	fixed leg pay dates
cvgpay[i] for i=1  ,...  ,nPay	coverage from tPay[i-1] to tPay[i]
Dpay[i] for i=1  ,...  ,nPay	dis factor from tNow to tPay[i]
ifirst [j] for j=1  ,...  ,nEx	fixed leg pay dates are i = ifirst[j] to
nlong[j] nlong [j] for j=1  ,...  ,nEx cvgfirst t[j] for j=1  ,...  ,nEx  first
period coverage (from tStart[j] to tPay[ifirst[j]]) Rlong [j] for j=1  ,... ,nEx
fixed rate for swaption j Vlong [j] for j=1  ,...  ,nEx	market price of swaption
j Note the indices j=0 and i=0 are ignored in the above arrays */
LGMErr LGMCalFixSig(
    ...
    LGMCalSet *CSPtr) /* Reference instruments */
{
  ...
  *LGMtsPtrPtr = NULL;
  ...
  /* Create term structure  , fill in  zeta dates  , calculate and fill in zeta
   * values */
...
this routine finds the G values by calibrating to the caplets  , and then
...
this routine finds the zeta values by calibrating to the caplets  , and then
fills them into the term structure */
static LGMErr
FitAllZetaCaps(LGM_TS *tsPtr,    /* Return: zeta's in LGM term structure ts */
               LGMCalSet *CSPtr) /* Reference caplets */
...
/********************************************************************************/
/* Fits value of zeta at exercise date for a swaption */
/* The fixed leg has the disc payments a[i] = payment[i]*df(tPay[i])  , i=1 ,...
,n. The strike is worth DStart = df(tStart)  , and  Gpay[i] = G(t) at tPay[i]  ,
i=1  ,...  ,n and Gst = G(tStart) */
static double FitOneZetaSwap(long n, double *a, double DStart,
                             SrtReceiverType recpay, double *Gpay, double Gst,
                             double price) {
  SrtCallPutType callput;
...
all 1 into k swaptions
        DfixStart							dis
factor from tNow to common start date fstt tPay[i] for i=ffirst  ,...  ,nlast
common set of pay dates for 1 into k swaptions
        ffirst							pay dates of
...
start date to first pay date cvgpay[i]  , i=ffirst+1  ,...  ,nlast	coverage
from tPay[i-1] to tPay[i] Dpay[i]  , i=ffirst  , ...  , nlast	dis factor from
tNow to tPay[i] Rfix[j] for j=nfirst  ,...  ,nlast	fixed rate for caplet j
        Vfix[j]	for j=nfirst  ,...  ,nlast	market value at tNow for caplet
j
*/
/* If usecaps == 1  , the caplets must also be defined:
        nEx>1						number of reference
caplets tEx[j] for j=1  ,...  ,nEx		exercise date for caplet j
        tStart[j] for j=1  ,...  ,nEx		start date for caplet j
        DStart[j] for j=1  ,...  ,nEx		dis factor from tNow to the
start date tEnd[j] for j=1  ,...  ,nEx		end date for caplet j Dcap[j]
for j=1  ,...  ,nEx		dis factor to the end date of caplet j cvgcap[j]
for j=1  ,...  ,nEx	coverage from start date to end date for caplet j
        Rfcap[j] for j=1  ,...  ,nEx	fixed rate for caplet j
        Vcap[j]	for j=1  ,...  ,nEx		market value at tNow for caplet
j Note the indices i=0 and j=0 is ignored in the above arrays
*/
/* If usecaps != 1  , the long "k into n-k" swaptions must also be defined:
        nEx>1						number of reference
   swaptions tEx[j] for j=1  ,...  ,nEx		exercise date for swaption j
        tStart[j] for j=1  ,...  ,nEx		start date for swaption j
        DStart[j] for j=1  ,...  ,nEx		dis factor from tNow to the
   start date
        nPay						total number of fixed
   leg pay dates tPay[i] for i=1  ,...  ,nPay	fixed leg pay dates cvgpay[i]
   for i=1  ,...  ,nPay	coverage from tPay[i-1] to tPay[i] Dpay[i] for i=1  ,...
   ,nPay	dis factor from tNow to tPay[i]

        ifirst [j] for j=1  ,...  ,nEx	fixed leg pay dates are i = ifirst[j] to
   nlong[j] nlong [j] for j=1  ,...  ,nEx cvgfirst t[j] for j=1  ,...  ,nEx
   first period coverage (from tStart[j] to tPay[ifirst[j]]) Rlong [j] for j=1
   ,...  ,nEx	fixed rate for swaption j Vlong [j] for j=1  ,...  ,nEx market
   price of swaption j Note the indices j=0 and i=0 are ignored in the above
   arrays
*/
LGMErr LGMCalFixExp(
    LGM_TS **LGMtsPtrPtr, /* Return: Calibrated term structure */
...
caplet j cvgcap[j] for j=1  ,...  ,nEx	coverage from start date to end date for
caplet j Rfcap[j] for j=1  ,...  ,nEx	fixed rate for caplet j Vcap[j] for
j=1  ,...  ,nEx		market value at tNow for caplet j */
...
static LGMErr FitAllGPairs(long npair, long *jpair, long usecaps, LGM_TS *tsPtr,
  ...
  /* For k = npair-1  , npair-2  , ...  , 1  , for each j=jpair[k] find
  G(tStart[j]) so that the LGM value of long swaption j and either the LGM value
  ...
  for (k = npair - 1; k >= 1; k--) {
    ...
    if (usecaps == 1) {
      ...
      ind = ifirst - 1;
      zeta = FitOneSwapCap(nsw, dispaylong, CSPtr->DStart[j],
                           recpay,        /* the swaption */
...
matches the market price when
        Gst = Gst + C*dGst
(G at start date) Gpay[i] = Gpay[i] + C*dGpay[i]  , i=1  , ...  ,n	(G at
pay dates)
...
/* The fixed leg has discounted payments a[i] = payment[i]*df(tPay[i])  , i=1
                  ...
                  double price)               /* market price of swaption */
{
  ...
  /* check for intrinsic */
  /*	if (fabs (LGMRecVal(&ystar  , n  , a  , DStart  , 1.0e-15  , Gpay  ,
     Gst) - price) < 1.0e-08)
          {
                  return 1.0e-15;
          }*/
...
chosen  , this keeps the caplet with fixing date tEx  , start date tStart and
end date calibrated to its market value

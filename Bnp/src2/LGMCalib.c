...
static LGMErr FitAllGCaps(LGM_TS *tsPtr,			/* Return: calibrated LGM term structure tsPtr */
					  LGMCalSet *CSPtr);			/* Reference caplets */
...
static LGMErr FitAllZetaSwaps(LGM_TS *tsPtr,		/* Return: zeta's in LGM term structure ts */
							  LGMCalSet *CSPtr);	/* Reference caplets */
...
static double FitOneLongShort(
		long n1, double *a1,						/* swaption 1 */
		long n2, double *a2,						/* swaption 2 */
		double DStart, SrtReceiverType recpay,		/* PV of strike for both swaptions */
		double* Gstptr, double *Gpay,				/* the existing term structure */
		double dGst, double *dGpay,					/* direction of change */
...
static double RecGDer(double ystar, long n, double *a, double DStart,		/* receiver swaption */ 
					double sqzeta, double *Gpay, double Gst,				/* term structure */
...
static LGM_TSPtr CreateTSwithSigma(
			LGMCalSet *CSPtr);		/* Reference instruments */
...
tStart[j] for j=1,...,nEx		start date for swaption j
DStart[j] for j=1,...,nEx		dis factor from tNow to the start date
n							tPay[n] is the last date we need to calibrate to
tPay[i] for i=1,...,nPay	fixed leg pay dates
cvgpay[i] for i=1,...,nPay	coverage from tPay[i-1] to tPay[i]
Dpay[i] for i=1,...,nPay	dis factor from tNow to tPay[i]
ifirst [j] for j=1,...,nEx	fixed leg pay dates are i = ifirst[j] to nlong[j]
...
static LGMErr FitAllGCaps(LGM_TS *tsPtr,			/* Return: calibrated LGM term structure ts */
					  LGMCalSet *CSPtr)				/* Reference caplets */
{
	LGMErr error;
	SrtReceiverType recpay;
...
/* initialize */
	...
	if (tsPtr->numG < nEx+1)
	{	srt_free(tsPtr->G); srt_free(tsPtr->Gdate);
...
/* fill in Gdates */
	...
	{	stddev = FitOneCap(CSPtr->Vcap[j], CSPtr->cvgcap[j], CSPtr->Rfcap[j],
		...
		{	for (k=j; tsPtr->Gdate[k] < CSPtr->tEnd[j] && k < tsPtr->numG; k++)	
				;
			dt1 = (double)(tsPtr->Gdate[k] - tsPtr->Gdate[k-1]);	/* tEnd[j] between Gdate[k-1] and Gdate[k] */
...
long swaption j to the market price Vlong[j] */
	...
	{	ifirst = CSPtr->ifirst[j];								/* discounted payments of fixed leg of swaption j */			
		...
		ind = ifirst-1;
		zeta = FitOneZetaSwap(n, dispayment, CSPtr->DStart[j],
							recpay, &(Gpay[ind]), Gst, CSPtr->Vlong[j]);
		if (zeta<0)
...
/* initialize */
	...
	{	LGMFreeLGM_TS(&tsPtr);
		srt_free(dispayment); srt_free(Gpay); srt_free(dGpay);
...
/* update up Gpay and dGpay arrays */
		...
		ind = ifirst-1;
		zeta = FitOneSwap(n, dispayment, CSPtr->DStart[j], recpay,	/* the swaption */
							zeta, 									/* zeta(tEx) */
...
Definition of the 1 into k swaptions:
	ftEx							exercise date for all 1 into k swaptions
	ftStart							start date for all 1 into k swaptions
	DfixStart							dis factor from tNow to common start date fstt
	tPay[i] for i=ffirst,...,nlast	common set of pay dates for 1 into k swaptions
	ffirst							pay dates of swaption j are tPay[i] for
	nfirst							i = ffirst, ..., j
	nlast									for j=nfirst, ..., nlast
	cvgfix							coverage from start date to first pay date
	cvgpay[i], i=ffirst+1,...,nlast	coverage from tPay[i-1] to tPay[i]
	Dpay[i], i=ffirst, ..., nlast	dis factor from tNow to tPay[i]
	Rfix[j] for j=nfirst,...,nlast	fixed rate for caplet j
	Vfix[j]	for j=nfirst,...,nlast	market value at tNow for caplet j
*/
...
term structure */
	...
	else
	{
		...
		else										/* insert at j0 */
		{
			...
			zmax = min(tsPtr->zeta[j0-1] + 2.5*slope*dt, tsPtr->zeta[j0] - 0.4*slope*(dint-dt));
			zmin = max(tsPtr->zeta[j0-1] + 0.4*slope*dt, tsPtr->zeta[j0] - 2.5*slope*(dint-dt));
			if (zetafix>zmax) zetafix = zmax;
			if (zetafix<zmin) zetafix = zmin;
...
agrees with the market price Vlong[j] */
	...
	{
		dispayment[1] = CSPtr->Rfix[j]*CSPtr->cvgfix*CSPtr->Dpay[ifirst];		/* compute discounted payments */
		...
		dummy = FitOneSwap(n, dispayment, CSPtr->DfixStart, recpay,	/* the swaption */
						zeta, 										/* zeta(tEx) */
						&Gst, &(Gpay[ind]), 						/* the term structure */
						dGst, &(dGpay[ind]),						/* direction G */
...
/***************************************************************/
static LGMErr FitAllGPairs(long npair, long *jpair, long usecaps,
						   LGM_TS *tsPtr, LGMCalSet *CSPtr)
{
	LGMErr error;
	SrtReceiverType recpay;
...
/* initialize */
	...
	{	srt_free(dispaylong); srt_free(dispayshort);
		srt_free(Gpay);	srt_free(dGpay);
...
/* finished calibrating G[k] ... re-scale G */
	scale = (tsPtr->G[0] - tsPtr->G[numG-1])*365.0/((double)(tsPtr->Gdate[numG-1] - tsPtr->Gdate[0]));
	for (j=0; j<numG; j++)
...
/* Use global Newton on coef to match LGM price to market price */
	...
	{	coef = coef + isign*dcoef;									/* update term structure */
		...
		if (fabs(newvalue)/(timevalue+.001) < 1.e-5 || fabs(dcoef)<1.e-6)		/* check convergence */
			success++;
		...
		{	value = newvalue;										/* step successful */
			isign = 1.0;											/* take new step */
			der = RecGDer(ystar, n, a, DStart, sqzeta, Gpay, Gst, dGst, dGpay);
			der = der-(product/(coef*coef*dGst))*RecSqZetaDer(ystar, n, a, DStart, sqzeta, Gpay, Gst);
			dcoef = -value/der;										/* Newton step */
		}
		else
		{	isign = -1.0;											/* step unsuccessful */
...
/* calculate f1, f2, and their derivatives */
		...
		{	arg = (Gst-Gpay[i])*stddev2*(z+dz) + (dGst-dGpay[i])*stddev2*(cz+dcz);
			...
			if (fabs(arg*theta1)<1.e-5)
				f1 = f1 + a1[i]*arg*(1.0 - 0.5*arg*theta1 + arg*arg*theta1*theta1/6.0);
			else
		...
		{	arg = (Gst-Gpay[i])*stddev1*(z+dz) + (dGst-dGpay[i])*stddev1*(cz+dcz);
			...
			if (fabs(arg*theta2)<1.e-5)
				f2 = f2 + a2[i]*arg*(1.0 - 0.5*arg*theta2 + arg*arg*theta2*theta2/6.0);
			else
...
double LGMRecVal(double *ystar, long n, double *a, double DStart,	/* receiver swaption */ 
				double sqzeta, double *Gpay, double Gst)			/* term structure */
{
...
double LGMFindystar(long n, double *a, double DStart,
						double zeta, double *Gpay, double Gst)
{

...
/* -----------------------------------------------------------------------------------------------------------------------------
        MODEL Free : Vol functions
                ...
                for the moment Inflation_VolTs contains a 3F VolTs but should be a generic
   VolTsModel inside all the following functions we should cast a void ptr to the right model
...
/* ------------------------------------------------------------------------------------------------
        ...
        Calculation of
        E(under QTp) of (FFX(Tf,Tf) knowing Ft) = E(under QTf) of (FFX(Tf,Tf) knowing Ft) *
  Cvxty_adj Cvxty_adj = exp (int(t->Tf) (<dFFX(t,Tf)/FFX(t,Tf),(G(t,Tp)-G(t,Tf))dW>
                              ...
                              double* ptr_dVol2)
{
    ...
    {
        *ptr_dVol1 =
            sqrt(dVar1 / (min(dTend, max(dTval1, dTval2)) - min(min(dTstart, dTval1), dTval2)));
        *ptr_dVol2 =
            sqrt(dVar2 / (min(dTend, max(dTval1, dTval2)) - min(min(dTstart, dTval1), dTval2)));
    }
    else
    {
        *ptr_dVol1 = 0.0;
    ...
    double dRhoYZ)
{
    ...
    dCovar = 2.0 - exp(dRhoXZ * dVolX * dVolZ * dT) - exp(dRhoXY * dVolX * dVolY * dT) -
             exp(dRhoYZ * dVolY * dVolZ * dT) +
                           ...
                           double* dSLVol /* Calibrated SL Vol */)
{
    ...
    m2 = dAlpha * dAlpha * dAlpha *
             CovarXYZ(dT, dFwdX, dFwdX, dFwdX, dVolX, dVolX, dVolX, 1.0, 1.0, 1.0) +
         3.0 * dAlpha * dAlpha * dBeta *
             CovarXYZ(dT, dFwdX, dFwdX, dFwdY, dVolX, dVolX, dVolY, 1.0, dRhoXY, dRhoXY) +
         3.0 * dAlpha * dBeta * dBeta *
             CovarXYZ(dT, dFwdX, dFwdY, dFwdY, dVolX, dVolY, dVolY, dRhoXY, dRhoXY, 1.0) +
         dBeta * dBeta * dBeta *

/* ===================================================================================
   FILENAME:      InflationUtils.cpp / P.A
...
{   
	...
	if(!strcmp(str,"FLAT_NOTIONAL")) 	{*val = FlatNotional; 	return 0;}
	if(!strcmp(str,"FLAT_INDEXED_NOTIONAL")) 	{*val = FlatIndexedNotional; 	return 0;}
	if(!strcmp(str,"INDEXED_NOTIONAL")) 	{*val = IndexedNotional; 	return 0;}
	if(!strcmp(str,"FLAT_DIRTY_NOTIONAL")) 	{*val = FlatDirtyNotional; 	return 0;}
	if(!strcmp(str,"INDEXED_DIRTY_NOTIONAL")) 	{*val = IndexedDirtyNotional; 	return 0;}
...
double InflationTry(long				lToday,
					long				lValDate,
					WrapInfo			*RealCurve		)	
{
...
/* -----------------------------------------------------------------------------------------------------------------------------
	MODEL Free : Vol functions
		...
		for the moment Inflation_VolTs contains a 3F VolTs but should be a generic VolTsModel 
		inside all the following functions we should cast a void ptr to the right model
...
/* ------------------------------------------------------------------------------------------------
	...
	Calculation of 
	E(under QTp) of (FFX(Tf,Tf) knowing Ft) = E(under QTf) of (FFX(Tf,Tf) knowing Ft) * Cvxty_adj
	Cvxty_adj = exp (int(t->Tf) (<dFFX(t,Tf)/FFX(t,Tf),(G(t,Tp)-G(t,Tf))dW>	
...
Inflation_3FVolTs *VolTs = NULL;
	...
	case Infl3F :
		...
		err = Fx3DtsImpliedVol_corr(	dTval,
										dTstart,
										dTend,
										VolTs->dRateSigTime,
										VolTs->lRateNbDate,
...
/* ------------------------------------------------------------------------------------------------
	...
	calculate the cumulated vol and corr from Tstart to Tend of FFX(t,Tval1) and FFX(t,Tval2)
  ------------------------------------------------------------------------------------------------- */
...
Inflation_3FVolTs	*VolTs = NULL;
	...
	case Infl3F :			
		...
		err = Fx3DtsImpliedVol_corr(	dTval1,
										min(dTstart,dTval1),
										min(dTend,dTval1),
										VolTs->dRateSigTime,
										VolTs->lRateNbDate,
		...
		err = Fx3DtsImpliedVol_corr(	dTval2,
										min(dTstart,dTval2),
										min(dTend,dTval2),
										VolTs->dRateSigTime,
										VolTs->lRateNbDate,
		...
		err = Fx3DtsFwdCumCovar_corr(	min(min(dTstart,dTval1),dTval2),			/*	Forward start date */
	...
	{
		*ptr_dVol1 = sqrt (dVar1 / (min(dTend,max(dTval1,dTval2)) - min(min(dTstart,dTval1),dTval2)));
		*ptr_dVol2 = sqrt (dVar2 / (min(dTend,max(dTval1,dTval2)) - min(min(dTstart,dTval1),dTval2)));
	}
	else
	{
		*ptr_dVol1 = 0.0;
...
/* ------------------------------------------------------------------------------------------------
	...
	Calculate cumulative covariance of F ( mat1 ) and F ( mat2 ) between T0 and EndDate
  ------------------------------------------------------------------------------------------------- */
		...
		for (j = StartIndex2; j < EndIndex2 + 1; j++)
		{
			...
			for (k = StartIndex3; k < EndIndex3 + 1; k++)
			{
				...
				covar_partial = 
								...
								+	sig_fx *
								(	
										correl_dom_fx[k] * sig_dom * 
										(
											Etha_Func (lda_dom, Tval1, U1, U2) + Etha_Func (lda_dom, Tval2, U1, U2)
										)
										
										- correl_for_fx[k] * sig_for * 
										(
											Etha_Func (lda_for, Tval1, U1, U2) + Etha_Func (lda_for, Tval2, U1, U2)
										)
								)
								+	sig_dom * sig_dom * Psi2_Func (lda_dom, lda_dom, Tval1, Tval2, U1, U2)
								+	sig_for * sig_for * Psi2_Func (lda_for, lda_for, Tval1, Tval2, U1, U2)
...
double dCovar;
	...
	dCovar = 2.0 - exp(dRhoXZ*dVolX*dVolZ*dT)- exp(dRhoXY*dVolX*dVolY*dT)- exp(dRhoYZ*dVolY*dVolZ*dT) 
				 + exp((dRhoXZ*dVolX*dVolZ+dRhoXY*dVolX*dVolY+dRhoYZ*dVolY*dVolZ)*dT);
...
Err		err = NULL;
	...
	m2 = dAlpha*dAlpha*dAlpha*CovarXYZ (dT,dFwdX,dFwdX,dFwdX,dVolX,dVolX,dVolX,1.0,1.0,1.0)
		+3.0*dAlpha*dAlpha*dBeta*CovarXYZ (dT,dFwdX,dFwdX,dFwdY,dVolX,dVolX,dVolY,1.0,dRhoXY,dRhoXY)
		+3.0*dAlpha*dBeta*dBeta*CovarXYZ (dT,dFwdX,dFwdY,dFwdY,dVolX,dVolY,dVolY,dRhoXY,dRhoXY,1.0)
		+dBeta*dBeta*dBeta*CovarXYZ (dT,dFwdY,dFwdY,dFwdY,dVolY,dVolY,dVolY,1.0,1.0,1.0);

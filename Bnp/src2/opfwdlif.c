...
**      DESCRIPTION:    Remy: missing discount factor, failing in extremal situations.
**
********************************************************************************
**
**      AMENDED BY:     Julia Matsumoto
...
static double fct_global(double x, va_list argptr)
{
    ...
    d   = vol * sqrt(mat1);
    ret = sm_qsimp_list(
        fct_life, 0.000001, mat2 - mat1 /*life option itself duration*/, m, vol, lambda1);
    ...
    SrtGreekType   greek)
{
    ...
    {
        premium = srt_f_optlifopt(fwd2, spot, barrier, vol2, mat2, disc2, down_up, PREMIUM);
    }
    ...
    else
    {
        ...
        intgrl = sm_qsimp_list(
            fct_global,
            (du ? 0 : log(barrier)),
            (du ? log(barrier) : log(barrier) * 3),
            spot,
            barrier,
            mat1,
            mat2,
        ...
        f11 = bivar(
            du * (log(barrier / fwd1) + vol1 * vol1 / 2 * mat1) / d1,
            du * (-(log(barrier / fwd2) + vol2 * vol2 / 2 * mat2) / d2),
            -vol1 / vol2 * sqrt(mat1 / mat2));

        f12 = bivar(
            du * (log(barrier / spot) + (myud + lambda1) * mat1) / d1,
        ...
        f2 = (mat2 - mat1) * norm(du * (-log(barrier / spot) + lambda2 * mat1) / d1);
    ...
    case THETA:
        ...
        answer = srt_f_optfwdlife(
                     ...
                     mat1 - shift,
                     mat2 - shift,
                     disc1 * exp(-shift * log(disc1) / mat1),
                     disc2 * exp(-shift * log(disc2) / mat2),
                     down_up,
                     PREMIUM) -
                 premium;
        break;

...
Err fill_mc_multi_init_corr(long pay_date, double pay_time, long *date,
                            double *time, long nb_dates, LINK_UND link) {
  ...
  double *correl = NULL, *correl_1 = NULL, *correl_2 = NULL, *correl_3 = NULL,
         **correl_fxfx = NULL;

  Err err = NULL;

  do_covar =
      f3tensor(0, nb_dates - 1, 0, link->num_und - 1, 0, link->num_und - 1);
  ...
  if (!do_covar || !correl || !correl_1 || !correl_2 || !correl_3 ||
      !correl_fxfx) {
    ...
    if (link->type[und] < 2) {
      ...
      for (k = 0; k < nb_dates - 1; k++) {
        ...
        if (und == link->dom_index) {
          ...
          link->dom_beta_pay[k] =
              -1.0 / lda_dom * (1 - exp(-lda_dom * pay_mat));
        }
        for (i = StartIndex; i < EndIndex + 1; i++) {
          ...
          if (link->type[und] == 0) {
            ...
            adj_pay += sig_und * link->sig_curve[dom_index][i] *
                       (x_und - exp(-lda_dom * mat_pay) *
                                    Z_Func(lda_dom + lda_und, T1, T2));
          } else /* link -> type[und] == 1 */
          {
            ...
            adj_quanto += sig_und * link->sig_curve[link->fx_index[und]][i] *
                          x_und *
            ...
            if (!do_covar[k][und][l] && und != l) {
              ...
              case 2:
                ...
                correl_1[l] +=
                    sig_und * sig_l_dom *
                    (x_und -
                     exp(-link->lambda[link->dom_forex[l]] * end_mat) *
                         Z_Func(lda_und + link->lambda[link->dom_forex[l]], T1,
                                T2)) *
                ...
                correl_2[l] +=
                    sig_und * sig_l_for *
                    (x_und -
                     exp(-link->lambda[link->for_forex[l]] * end_mat) *
                         Z_Func(lda_und + link->lambda[link->for_forex[l]], T1,
                                T2)) *
        ...
        if (link->type[und] == 0) {
          link->fwd[und][k + 1] =
              1.0 / lda_und * (expect - exp(-lda_und * end_mat) * adj_pay) +
          ...
          link->fwd[und][k + 1] =
              1.0 / lda_und * expect +
              exp(-lda_und * end_mat) *
                  (-1.0 / lda_dom * adj_pay - 1.0 * adj_quanto) +
              link->phi[und][k] * exp(-lda_und * mat) *
                  Phi_Func(-lda_und, start_mat, start_mat, end_mat);
        }
        ...
        link->covariance[k + 1][und][und] =
            link->std[und][k + 1] * link->std[und][k + 1];
    ...
    if (link->type[und] == 2) {
      ...
      for (k = 0; k < nb_dates - 1; k++) {
        ...
        QTexpect = -mat * (zc_for - zc_dom) -
                   0.5 * (link->beta[index_for][k] * link->beta[index_for][k] *
                              link->phi[index_for][k] -
                          link->beta[index_dom][k] * link->beta[index_dom][k] *
                              link->phi[index_dom][k]);

        /*	Implied Fx Vol */
        err = Fx3DtsImpliedVol_corr(
            end_mat, start_mat, end_mat, link->sig_dates, link->nb_sig_dates,
            ...
            link->sig_dates, link->correlations[index_dom][index_for],
            link->correlations[index_dom][und],
            link->correlations[und][index_for], link->nb_sig_dates, &fx_vol);
        if (err)
          goto FREE_RETURN;
        ...
        for (i = StartIndex; i < EndIndex + 1; i++) {
          ...
          adj_pay +=
              link->correlations[dom_index][und][i] * sig_dom * sig_und *
                  (Etha_Func(lda_dom, end_mat, T1, T2) -
                   Etha_Func(lda_dom, mat_pay, T1, T2)) -
              link->correlations[dom_index][index_for][i] * sig_dom *
                  link->sig_curve[index_for][i] *
                  (Psi_Func(link->lambda[index_for], lda_dom, end_mat, T1, T2) -
                   Psi2_Func(link->lambda[index_for], lda_dom, end_mat, mat_pay,
                             T1, T2)) +
              link->correlations[dom_index][index_dom][i] * sig_dom *
                  link->sig_curve[index_dom][i] *
                  (Psi_Func(link->lambda[index_dom], lda_dom, end_mat, T1, T2) -
                   Psi2_Func(link->lambda[index_dom], lda_dom, end_mat, mat_pay,
                             T1, T2));
          ...
          if (dom_index != index_dom) {
            /* NOT VALID */
            /*
            adj_quanto += sig_und * link -> sig_curve[link ->
            fx_index[index_dom]][i] * (T2 - T1);
            */
          }
          ...
          for (l = 0; l < link->num_und; l++) {
            if (!do_covar[k][und][l] && und != l) {
              ...
              case 2:
                ...
                /* Z_und with Z_l */
                correl_fxfx[l][0] +=
                    sig_und * sig_l * (T2 - T1) * link->correlations[und][l][i];

                /* Z_und with Dom_l */
                correl_fxfx[l][1] +=
                    sig_und * sig_l_dom *
                    (Etha_Func(link->lambda[link->dom_forex[l]], end_mat, T1,
                               T2)) *
                    link->correlations[und][link->dom_forex[l]][i];
                ...
                correl_fxfx[l][2] +=
                    sig_und * sig_l_for *
                    (Etha_Func(link->lambda[link->for_forex[l]], end_mat, T1,
                               T2)) *
                    link->correlations[und][link->for_forex[l]][i];
                ...
                correl_fxfx[l][3] +=
                    sig_und_dom * sig_l *
                    (Etha_Func(link->lambda[link->dom_forex[und]], end_mat, T1,
                               T2)) *
                    link->correlations[link->dom_forex[und]][l][i];

                /* Dom_und with Dom_l */
                correl_fxfx[l][4] +=
                    sig_und_dom * sig_l_dom *
                    (Psi_Func(link->lambda[link->dom_forex[und]],
                              link->lambda[link->dom_forex[l]], end_mat, T1,
                              T2)) *
                    link->correlations[link->dom_forex[und]][link->dom_forex[l]]
                                      [i];

                /* Dom_und with For_l */
                correl_fxfx[l][5] +=
                    sig_und_dom * sig_l_for *
                    (Psi_Func(link->lambda[link->dom_forex[und]],
                              link->lambda[link->for_forex[l]], end_mat, T1,
                              T2)) *
                    link->correlations[link->dom_forex[und]][link->for_forex[l]]
                                      [i];

                /* For_und with Z_l */
                correl_fxfx[l][6] +=
                    sig_und_for * sig_l *
                    (Etha_Func(link->lambda[link->for_forex[und]], end_mat, T1,
                               T2)) *
                    link->correlations[link->for_forex[und]][l][i];

                /* For_und with Dom_l */
                correl_fxfx[l][7] +=
                    sig_und_for * sig_l_dom *
                    (Psi_Func(link->lambda[link->for_forex[und]],
                              link->lambda[link->dom_forex[l]], end_mat, T1,
                              T2)) *
                    link->correlations[link->for_forex[und]][link->dom_forex[l]]
                                      [i];

                /* For_und with For_l */
                correl_fxfx[l][8] +=
                    sig_und_for * sig_l_for *
                    (Psi_Func(link->lambda[link->for_forex[und]],
                              link->lambda[link->for_forex[l]], end_mat, T1,
                              T2)) *
                    link->correlations[link->for_forex[und]][link->for_forex[l]]
                                      [i];
        ...
        for (l = 0; l < link->num_und; l++) {
          if (!do_covar[k][und][l] && und != l) {
            ...
            case 2:
              ...
              link->covariance[k + 1][und][l] =
                  1.0 * correl_fxfx[l][0] + 1.0 * correl_fxfx[l][1] -
                  1.0 * correl_fxfx[l][2] + 1.0 * correl_fxfx[l][3] +
                  1.0 * correl_fxfx[l][4] - 1.0 * correl_fxfx[l][5] -
                  1.0 * correl_fxfx[l][6] - 1.0 * correl_fxfx[l][7] +
                  1.0 * correl_fxfx[l][8];
...
FREE_RETURN:
  ...
  if (do_covar)
    free_f3tensor(do_covar, 0, nb_dates - 1, 0, link->num_und - 1, 0,
                  link->num_und - 1);
  if (correl)

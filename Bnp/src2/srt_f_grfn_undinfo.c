...
/* Set up Underlying specific information */
	...
	/* See if stochastic process is required for underlying */
		...
		/* Check for stochastic volatility */
			...
			{
			/* Stochastic volatility model not implemented in Multi-Underlying Exept in EQ_STOCH_RATES_SRVGS (fudge) */
	...
	/* EQ_STOCH_VOL model no jumping numeraire */
		if ( (mdl_type == EQ_STOCH_RATES) ||  (mdl_type == EQ_STOCH_RATES_SRVGS) )	und_info->jumping = SRT_NO;
...
SRT_Boolean				found = SRT_NO;
	...
	*/
	...
	for (i = 0 ; i < und_info->no_of_underlyings ; i++)
	{
		...
		if (mdl_type == FX_BETADLM || !strcmp(und->underl_lbl,"BLACK_UND") || !strcmp(und->underl_lbl,"SABR_UND"))
		{
			und_info->use_corr_ts &= SRT_NO;
		}
		else
...
/* Loop on the different underlying to see if we need to add some new ones */
	...
	/* Check for different currencies (QUANTO) : need the FX to be defined */
		if ((  (und_type == FOREX_UND) || (und_type == INTEREST_RATE_UND)
				|| (und_type == EQUITY_UND)) && und_info->use_corr_ts == SRT_YES)
		{
			...
			/* Go through the correlation list to find an FX und with both currencies */
				...
				/* If it is an FX underlying : this might be of interest */
					...
					/* Look for the FX with the same or opposite convention */
						if (
							( (strcmp(tempdomccy, dom_ccy_str) == 0) && (strcmp(tempforccy, und_ccy) == 0) )
							||
							( (strcmp(tempforccy, dom_ccy_str) == 0) && (strcmp(tempdomccy, und_ccy) == 0) )
							)
						{
							found = SRT_YES;
						...
						/* Check if this underlying belongs to the Grfn underlying list */
							for (j = 0 ; j < und_info->no_of_underlyings ; j++)
							{
								if (strcmp(und_info->und_data[j].und_name, corrval->und_names[k]) == 0)
									break;
							}
						
	/* OVE: the FX underlying should not be added to the list of underlyings to be diffused if it is not there
							/* It is not in the list, we add it ( it will have to be diffused...)
							if (j >= und_info->no_of_underlyings)
							{
								strcpy(und_info->und_data[und_info->no_of_underlyings].und_name, corrval->und_names[k]);
								und_info->no_of_underlyings++;
							} 
	*/
			...
			/* We have not found a forex underlying for the quanto adjustment */
				if (found == SRT_NO)
				{
					return serror("Need a FOREX underlying [%s/%s]"
								  , dom_ccy_str, und_ccy);
				}

...
double kprime, d1, d2, bs, integrd;
	...
	kprime = NI_a * exp((NI_sigX - NI_rho * NI_sigY) * var + NI_muX) - NI_k * exp(-NI_rho * NI_sigY * var); 
	...
	integrd =  exp(-0.5*var*var) * exp(NI_rho*NI_sigY*var) * bs;  /* Here, we forget the sqrt(2*PI)*/
...
/* sets integration limits */
	glimitlow		= (fabs(NI_limitX) > INTEG_LIMIT ? \
						- INTEG_LIMIT - fabs(NI_limitX) : - INTEG_LIMIT);
	glimithigh		= (fabs(NI_limitX) > INTEG_LIMIT ? \
						+ INTEG_LIMIT + fabs(NI_limitX) : + INTEG_LIMIT);
...
/* Different cases depending on a,b, and k */
	...
	switch ( greek )
	{
		...
		case DELTAX :
			shift  = fwdx / 10000;
	 		answer = ( srt_f_optsprnum(	fwdx + shift,
									 	fwdy, 
										sigx,
										sigy,
										rho,
		...
		case GAMMAXY:
			shiftx  = fwdx / 1000;
			shifty  = fwdy / 1000;
			answer  = srt_f_optsprnum(	fwdx + shiftx,
										fwdy + shifty,
										sigx,
										sigy,
										rho,

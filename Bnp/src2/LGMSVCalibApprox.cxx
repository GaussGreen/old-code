...
void LGMSV_Copy_CalibParams(LGMSV_CALIBPARAMS source,
                            LGMSV_CALIBPARAMS target) {
...
Err LGMSV_Get_Rho_From_RhoTarget(double rho_target, double rho_mat1,
                                 double rho_mat2, double LGM_alpha,
                                 double LGM_gamma, double LGM_rho, double *rho1,
                                 double *rho2) {
  Err err = NULL;
...
Err LGMSV_Get_RhoTarget_From_Rho(double rho1, double rho2, double rho_mat1,
                                 double rho_mat2, double LGM_alpha,
                                 double LGM_gamma, double LGM_rho,
                                 double *rho_target) {
  Err err = NULL;
  ...
  rho_target1 =
      rho1 * (exp(LGM_gamma * rho_mat1) - exp(LGM_gamma * rho_mat2)) /
      (exp(LGM_gamma * rho_mat1) * s1 - exp(LGM_gamma * rho_mat2) * s2);
  rho_target2 = rho2 * LGM_alpha *
                (exp(-LGM_gamma * rho_mat1) - exp(-LGM_gamma * rho_mat2)) /
                (s1 - s2);
...
Err LGMSV_Get_RhoTargetSensitivity_From_Rho(double rho1, double rho2,
                                            double rho_mat1, double rho_mat2,
                                            double LGM_alpha, double LGM_gamma,
                                            double LGM_rho, double *sensi1,
                                            double *sensi2) {
  Err err = NULL;
...
Err LGMSVCalibApprox(
    ...
    double ex_time[], double ex_lfwd[], double ex_llvl[],
    double ex_lstrike[], /*	Strikes */
    double ex_lprice[],  /*	Market prices */
    double shift[], double coef_vol[], double coef_meanrev[],
  ...
  for (i = 0; i < nex; i++) {
    ...
    /* first pricing */
    LGMSVClosedFormApprox(dLambdaX, iNbPWTime, dPWTime, dSigmaTS, dAlphaTS,
                          dLambdaEpsTS, dRhoTS, dTStar, dShiftFwd, 1,
                          &dShiftStrike, ex_time[i], coef_vol[i], 1000,
                          coef_meanrev[i], 0, 0, 0, NumerParams, &price1);
    ...
    if (fabs(ex_lprice[i] - price1) > Precision) {
      ...
      sig2 =
          sqrt((ex_lprice[i] * ex_lprice[i] / price1 / price1 * cum_var_temp -
                cum_var) /
               (ex_time[i] - last_time));
      ...
      while (fabs(ex_lprice[i] - price1) > Precision && k < NbIterMax) {
        ...
        LGMSVClosedFormApprox(dLambdaX, iNbPWTime, dPWTime, dSigmaTS, dAlphaTS,
                              dLambdaEpsTS, dRhoTS, dTStar, dShiftFwd, 1,
                              &dShiftStrike, ex_time[i], coef_vol[i], 1000,
                              coef_meanrev[i], 0, 0, 0, NumerParams, &price2);
...
Err cpd_calibSV_approx(
    ...
    double Precision, int NbIterMax,
    /*	Output */
    int *numres, double **restime, double ***result,
    /*	Parameters */
        ...
        inst_data) /*	NULL = don't save calibration instrument data */
{
  ...
  double *sig = NULL, *sig_time = NULL, *dPWTimeNew = NULL, *dSigmaTS = NULL,
         *dAlphaTSNew = NULL, *dRhoTSNew = NULL, *dLambdaEpsTSNew = NULL;
  ...
  if (!dSigmaTS || !dAlphaTSNew || !dLambdaEpsTSNew || !dRhoTSNew ||
      !lSigIndex) {
  ...
  if (power > 0.5) {
    for (i = 0; i < nex; i++) {
      for (j = last_index + 1; j <= lSigIndex[i]; j++) {
        if (j > 0) {
          sumAlpha2 += dAlphaTSNew[j] * dAlphaTSNew[j] *
                       (dPWTimeNew[j] - dPWTimeNew[j - 1]);
      ...
      if (LambdaEq > 1.0E-16) {
        AlphaEq *= sqrt((1.0 - exp(-2.0 * LambdaEq * ex_time[i])) /
                        (2.0 * LambdaEq * ex_time[i]));
      }

      vol_conv(ex_lvol[i], SABR_STR_LOG, &atm_vol, SABR_ATM_LOG, ex_lfwd[i],
               ex_lstrike[i], ex_time[i], AlphaEq, 0.0, RhoEq);
    ...
    for (i = 0; i < nex; i++) {
      ...
      vol_conv(ex_lvol[i], SABR_STR_NORM, &atm_vol, SABR_ATM_NORM, ex_lfwd[i],
               ex_lstrike[i], ex_time[i], AlphaEq, 0.001, RhoEq);
  ...
  for (i = 0; i < nex; i++) {
    ...
    err = Calculate_HestonEquivalent_LGMSV(
        l - j + 1, &(cpn_date[j]), &(cpn_time[j]), &(cpn_cvg[j]), &(cpn_df[j]),
        0, 0, today, yc_name, 1.0 / dLambdaX, dTStar, &swp_rte, &lvl,
        &(shift[i]), &(coef_vol[i]), &(coef_meanrev[i]), &temp);
  ...
  err = LGMSVCalibApprox(nex, ex_time, ex_lswp, ex_llvl, ex_lstrike, ex_lprice,
                         shift, coef_vol, coef_meanrev, sig, dLambdaX,
                         iNbPWTimeNew, dPWTimeNew, dAlphaTSNew, dLambdaEpsTSNew,
                         dRhoTSNew, dTStar, lSigIndex, NumerParams, Precision,
                         NbIterMax, dSigmaTS);
...
void Fill_Equivalent_ATM_Prices(LGMSV_MODEL model,
                                ALLCALININSTRUMENTSDLM AllCalibInst,
                                long *lSigIndex, int iIndexStrike,
  ...
  for (i = 0; i < CalibExeSchedule->iNExe; i++) {
    ...
    for (j = last_index + 1; j <= lSigIndex[i]; j++) {
      if (j > 0) {
        ...
        sumLambda +=
            model->dLambdaEps[j] * (model->dPWTime[j] - model->dPWTime[j - 1]);
    ...
    if (LambdaEq > 1.0E-16) {
      AlphaEq *= sqrt((1.0 - exp(-2.0 * LambdaEq * CalibInst->dExeTime)) /
                      (2.0 * LambdaEq * CalibInst->dExeTime));
    }

    if (fabs(CalibInst->dFwdCash - CalibInst->dStrike[iIndexStrike]) >
        1.0E-08) {
      vol_conv(CalibInst->dNormalVol[iIndexStrike], SABR_STR_NORM, &atm_vol,
               SABR_ATM_NORM, CalibInst->dFwdCash,
               CalibInst->dStrike[iIndexStrike], CalibInst->dExeTime, AlphaEq,
               0.001, RhoEq);
    } else {
    ...
    CalibExeSchedule->dATMPrice[i] = srt_f_optblknrm(
        CalibInst->dFwdCash, CalibInst->dFwdCash, atm_vol, CalibInst->dExeTime,
        CalibInst->dLevel, SRT_PUT, PREMIUM);

    CalibExeSchedule->dATMVega[i] =
        srt_f_optblknrm(CalibInst->dFwdCash, CalibInst->dFwdCash, atm_vol,
                        CalibInst->dExeTime, CalibInst->dLevel, SRT_PUT, VEGA);
  }
}

Err LGMSV_calib_const_alpha_and_rho_from_SABR(
    AllCalibInstrumentsDLM *Instruments, CTS_MKT mkt, double *alpha,
    double *lambda, double *rho, LGMSV_CALIBPARAMS sParams,
    CPD_CALIB_INST_DATA inst_data) {
  double *mkt_alpha = NULL, *mkt_rho = NULL, *exercise_times = NULL,
         *weights = NULL;
  ...
  if (!mkt_alpha || !mkt_rho || !exercise_times || !weights) {
    err = "Memory allocation faillure (1) in "
          "LGMSV_calib_const_alpha_and_rho_from_SABR";
    goto FREE_RETURN;
  }
  ...
  /* old method
          err = swp_f_SABRvol(mkt->vc        ,
           ...
           , Instruments->sCalibExeSchedule->lTheoEndDates[0]        , 0.05 ,
     &temp        , &power        , SABR_ISMARKETSABR);
                                                  */
  ...
  if (err || (fabs(temp) < 1e-8)) {
    ...
    err = swp_f_SABRvol(mkt->vc,
                        Instruments->sCalibCpnSchedule
                            ->lCpnDate[Instruments->sCalibInst[0].iStartCpn],
                        Instruments->sCalibExeSchedule->lTheoEndDates[0], 0.05,
                        &temp, &temp, SABR_ZETA);
    if (err) {
      iIsSABRMarket = 1;
  ...
  while (iIndexStart < Instruments->iNbInst - 1 &&
         Instruments->sCalibExeSchedule->dExeTimes[iIndexStart] <
             sParams->sabr_calib_min_time) {
    iIndexStart++;
  }
  ...
  for (i = iIndexStart; i < Instruments->iNbInst; i++) {
    ...
    if (iIsSABRMarket) {
      ...
      err = swp_f_SABRvol(mkt->vc,
                          Instruments->sCalibCpnSchedule
                              ->lCpnDate[Instruments->sCalibInst[i].iStartCpn],
                          Instruments->sCalibExeSchedule->lTheoEndDates[i],
                          0.05, &dATMLog, &temp, SABR_ATMLOG);
    ...
    err = cts_get_mkt_sabr_beta_param(
        mkt, Instruments->sCalibExeSchedule->lExeDates[i],
        Instruments->sCalibCpnSchedule
            ->lCpnDate[Instruments->sCalibInst[i].iStartCpn],
        Instruments->sCalibExeSchedule->lTheoEndDates[i], LGMSV_SABRBETA,
        sParams->use_sabr_levenberg, &mkt_alpha[i], &mkt_rho[i],
        &fitting_error);
  ...
  if (Instruments->iNbInst - iIndexStart > 1) {
    ...
    *rho = temp / Instruments->iNbInst;
    ...
    err = levenberg_marquardt_select(
        exercise_times - 1 + iIndexStart, mkt_alpha - 1 + iIndexStart,
        weights - 1 + iIndexStart, Instruments->iNbInst - iIndexStart, param,
        use_param, 2, LGMSV_ALPHA_MAXITER, cts_implied_alpha_approx,
        &fitting_error);
    ...
    *lambda = DMIN(*lambda, sParams->sabr_calib_max_lambda);
    ...
    if (Instruments->iNbInst - iIndexStart == 1) {
      ...
      *alpha =
          sqrt(mkt_alpha[iIndexStart] * 2.0 * (*lambda) *
               exercise_times[iIndexStart] /
  ...
  if (inst_data && Instruments->iNbInst) {
    ...
    if (!inst_data->start_dates_smile || !inst_data->end_dates_smile ||
      ...
      err = "Memory allocation faillure in "
            "LGMSV_calib_const_alpha_and_rho_from_SABR";
      goto FREE_RETURN;
    }
...
Err cpd_calib_diagonal_LGMSV_new_dlm(
    ...
    double *strikeS1l_, double *strikeS2l_,
    ...
    int iOne2F, double *dLambdaX,
    int iNbPWTime, /* Piece Wise Term Structures  */
    double *dPWTime, double *dAlphaTS, double *dLambdaEpsTS, double *dRhoTS,
    double dTStar, double dLGMAlpha, double dLGMGamma, double dLGMRho,
    double *dRho2TS,
    ...
    /*	Output */
    int *num_sig, /*	Answer */
    double **sig_time, double **sig, double **alpha, double **lambdaeps,
        ...
        inst_data) /*	NULL = don't save calibration instrument data */
{
  ...
  double strikel[MAX_INST], strikeS1l[MAX_INST], strikeS2l[MAX_INST],
      strikeS1s[MAX_INST], strikeS2s[MAX_INST], ex_timel[MAX_INST],
      strikes[MAX_INST], weightl[MAX_INST], weights[MAX_INST],
      ex_times[MAX_INST];
  ...
  int iNbPWTimeNew;
  double *dPWTimeNew = NULL, *dSigmaTS = NULL, *dAlphaTSNew = NULL,
         *dRhoTSNew = NULL, *dRho2TSNew = NULL, *dLambdaEpsTSNew = NULL,
         *dLvlEpsTSNew = NULL;
  ...
  *rho2 = NULL;
  ...
  if (!CalibCpnLongSchedule || !CalibCpnShortSchedule ||
      ...
      !AllCalibShortInst || !AllParams || !model || !AllCalibShortInstOne) {
  ...
  if (local_calib_params->calib_rho2 &&
      local_calib_params->use_sabr_calib == 0) {
    local_calib_params->onefac_rho = 0;
  }

  if (local_calib_params->fix_lambda &&
      !local_calib_params->calib_smile_on_prim) {
  ...
  AllocateCalibExeSchedule(&(ex_datel[0]), &(ex_timel[0]), &(start_datel[0]),
                           ...
                           &(strikel[0]), &(strikeS1l[0]), &(strikeS2l[0]),
                           &(weightl[0]), CalibExeLongSchedule);

  err = Construct_CalibSchedule(
      yc_name, today, instr_long_freq, instr_long_basis, num_ex_datesl,
      ex_datel_, cal_datel, end_tenorl, end_datel, strikel_, strikeS1l_,
      strikeS2l_, NULL, CalibCpnLongSchedule, CalibExeLongSchedule);
  ...
  err = AllocateAllCalibInst(CalibExeLongSchedule->iNExe, nb_long_smile,
                             AllCalibLongInst);
  ...
  if (!local_calib_params->fix_lambda) {
    ...
    AllocateCalibExeSchedule(&(ex_dates[0]), &(ex_times[0]), &(start_dates[0]),
                             ...
                             &(strikes[0]), &(strikeS1s[0]), &(strikeS2s[0]),
                             &(weights[0]), CalibExeShortSchedule);

    err = Construct_CalibSchedule(
        yc_name, today, instr_short_freq, instr_short_basis, num_ex_datess,
        ex_dates_, cal_dates, end_tenors, end_dates, strikes_, strikeS1s_,
        strikeS2s_, weights_, CalibCpnShortSchedule, CalibExeShortSchedule);
    ...
    if (!local_calib_params->calib_smile_on_prim) {
      if (local_calib_params->calib_alpha || local_calib_params->calib_rho) {
        if (local_calib_params->calib_rr_bt) {
          ...
          nb_short_smile = 1 + local_calib_params->calib_alpha +
                           local_calib_params->calib_rho;
        }
      } else {
        nb_short_smile = 1;
      }
    ...
    err = AllocateAllCalibInst(CalibExeShortSchedule->iNExe, nb_short_smile,
                               AllCalibShortInst);
  ...
  if ((local_calib_params->use_sabr_calib &&
    ...
    err = cts_init_mkt(today, yc_name, vol_curve_name, vol_ref_rate_name,
                       instr_long_freq, instr_long_basis, instr_short_freq,
                       instr_short_basis, get_cash_vol, GetCashVolAndConvert, 0,
                       0, 1, &null_strikes_in_vol, mkt);
    ...
    if (local_calib_params->calib_smile_on_prim) {
      err = LGMSV_calib_const_alpha_and_rho_from_SABR(
          AllCalibLongInst, mkt, &flat_alpha, &flat_lambda, &flat_rho,
          local_calib_params, inst_data);
    } else {
      err = LGMSV_calib_const_alpha_and_rho_from_SABR(
          AllCalibShortInst, mkt, &flat_alpha, &flat_lambda, &flat_rho,
          local_calib_params, inst_data);
    }
    ...
    if (local_calib_params->onefac_rho) {
      ...
      for (i = 0; i < iNbPWTime; i++) {
        err = LGMSV_Get_Rho_From_RhoTarget(
            dRhoTS[i], local_calib_params->rho_mat1,
            local_calib_params->rho_mat2, dLGMAlpha, dLGMGamma, dLGMRho,
            &(dRhoTS[i]), &(dRho2TS[i]));
      ...
      for (i = 0; i < iNbPWTime; i++) {
        err = LGMSV_Get_RhoTarget_From_Rho(
            dRhoTS[i], dRho2TS[i], local_calib_params->rho_mat1,
            local_calib_params->rho_mat2, dLGMAlpha, dLGMGamma, dLGMRho, &temp);
  ...
  memcpy(dPWTimeNew, CalibExeLongSchedule->dExeTimes,
  ...
  if (!local_calib_params->fix_lambda) {
    ...
    num_f_concat_vector(&iNbPWTimeNew, &dPWTimeNew,
  ...
  if (!dSigmaTS || !dAlphaTSNew || !dLambdaEpsTSNew || !dLvlEpsTSNew ||
      !dRhoTSNew || !dRho2TSNew || !lSigLongIndex ||
      (!lSigShortIndex && !local_calib_params->fix_lambda)) {
    err = "Memory allocation faillure (2) in cpd_calibSV_approx";
    goto FREE_RETURN;
  }
  ...
  for (i = 0; i < CalibExeLongSchedule->iNExe; i++) {
    lSigLongIndex[i] =
        Get_Index(CalibExeLongSchedule->dExeTimes[i], dPWTimeNew, iNbPWTimeNew);
  }

  if (!local_calib_params->fix_lambda) {
    for (i = 0; i < CalibExeShortSchedule->iNExe; i++) {
      lSigShortIndex[i] = Get_Index(CalibExeShortSchedule->dExeTimes[i],
                                    dPWTimeNew, iNbPWTimeNew);
    }
  }

  err = init_LGMSV_model(model, today, iOne2F, iNbPWTimeNew, *dLambdaX,
                         dPWTimeNew, dSigmaTS, dAlphaTSNew, dLambdaEpsTSNew,
                         dLvlEpsTSNew, dRhoTSNew, dTStar, dLGMAlpha, dLGMGamma,
                         dLGMRho, dRho2TSNew);

  if (err)
    goto FREE_RETURN;

  Fill_Equivalent_ATM_Prices(model, AllCalibLongInst, lSigLongIndex, 0,
                             CalibExeLongSchedule);

  if (!local_calib_params->fix_lambda) {
    Fill_Equivalent_ATM_Prices(model, AllCalibShortInst, lSigShortIndex, 0,
  ...
  err = lgmcalibzetalambda_tauts_dlm(
      ...
      CalibCpnLongSchedule->iNCpn, CalibCpnLongSchedule->dCpnTime,
      CalibCpnLongSchedule->dCpnDf, CalibCpnLongSchedule->dCpnCvg,

      CalibExeLongSchedule->iNExe, CalibExeLongSchedule->dExeTimes,
      CalibExeLongSchedule->iStartCpn, CalibExeLongSchedule->iEndCpn,
      CalibExeLongSchedule->dATMStrike, CalibExeLongSchedule->dATMPrice,
      CalibExeLongSchedule->dATMVega,

      CalibCpnShortSchedule->iNCpn, CalibCpnShortSchedule->dCpnTime,
      CalibCpnShortSchedule->dCpnDf, CalibCpnShortSchedule->dCpnCvg,

      CalibExeShortSchedule->iNExe, CalibExeShortSchedule->dExeTimes,
      CalibExeShortSchedule->iStartCpn, CalibExeShortSchedule->iEndCpn,
      CalibExeShortSchedule->dATMStrike, CalibExeShortSchedule->dWeights,
      CalibExeShortSchedule->dATMPrice, CalibExeShortSchedule->dATMPrice,

      ex_zeta, local_calib_params->fix_lambda, 1, &time0, &model->dLambdaX,
      iOne2F, dLGMAlpha, dLGMGamma, dLGMRho, paraml, params, lm_params,
      &lam_sens);
  ...
  if (model->iOne2F == 2) {
    ...
    ConvertAlphaRho_LGM_to_LGMSV(
        model->iNbPWTime, model->dPWTime, model->dInitTStar, model->dLambdaX,
        model->dInitLGMAlpha, model->dLGMGamma, model->dInitLGMRho,
  ...
  for (i = 1; i < CalibExeLongSchedule->iNExe; i++) {
    if (ex_zeta[i] > ex_zeta[i - 1]) {
      temp = sqrt((ex_zeta[i] - ex_zeta[i - 1]) /
  ...
  err = Initialise_AllParams(
      model, lSigLongIndex, CalibCpnLongSchedule, AllCalibLongInst, paraml,
      lSigShortIndex, CalibCpnShortSchedule, AllCalibShortInst, params,
      local_calib_params, NumerParams, model->iNbPWTime, AllParams);
  ...
  AllParams->LongInstParams =
      calloc(AllCalibLongInst->iNbInst, sizeof(INSTPARAMS));
  ...
  for (i = 0; i < AllCalibLongInst->iNbInst - 1; i++) {
    AllCalibLongInst->sCalibInst[i].NextInst =
        &(AllCalibLongInst->sCalibInst[i + 1]);
    AllParams->LongInstParams[i]->NextInstParams =
        AllParams->LongInstParams[i + 1];
  }
  ...
  if (!local_calib_params->fix_lambda) {
    AllParams->ShortInstParams =
        calloc(AllCalibShortInst->iNbInst, sizeof(INSTPARAMS));
    ...
    for (i = 0; i < AllCalibShortInst->iNbInst; i++) {
      ...
      AllParams->ShortInstParams[i]->HestonInst =
          &(AllParams->HestonShortInst[i]);
      AllParams->ShortInstParams[i]->NumerInst =
    ...
    for (i = 0; i < AllCalibShortInst->iNbInst - 1; i++) {
      AllCalibShortInst->sCalibInst[i].NextInst =
          &(AllCalibShortInst->sCalibInst[i + 1]);
      AllParams->ShortInstParams[i]->NextInstParams =
          AllParams->ShortInstParams[i + 1];
    }
    ...
    for (i = 0; i < AllCalibShortInst->iNbInst; i++) {
      AllCalibShortInst->sCalibInst[i].dWeight =
          CalibExeShortSchedule->dWeights[i];
    }
  ...
  AllCalibShortInstOne->sCalibCpnSchedule =
      AllCalibShortInst->sCalibCpnSchedule;
  AllCalibShortInstOne->sCalibExeSchedule =
      AllCalibShortInst->sCalibExeSchedule;
  ...
  AllParams->CalibFunctionsForVol.GetLimitAndLastParam =
      GetLimitAndLastVol_LGMSV;
  ...
  if (local_calib_params->calib_flat_smile) {
    ...
    AllParams->CalibFunctionsForRho.GetLimitAndLastParam =
        GetLimitAndLastFlatRho_LGMSV;
    AllParams->CalibFunctionsForRho.GetSecondGuess =
        GetSecondGuessFlatRho_LGMSV;
    AllParams->CalibFunctionsForRho.PriceInst = PriceInstFlatRho_LGMSV;
    AllParams->CalibFunctionsForRho.SetParam = SetFlatRho_LGMSV;
    AllParams->CalibFunctionsForRho.UpdateConstsAfterParam =
        UpdateParamsAfterFlatRho_LGMSV;
    ...
    AllParams->CalibFunctionsForAlpha.GetFirstGuess =
        GetFirstGuessFlatAlpha_LGMSV;
    AllParams->CalibFunctionsForAlpha.GetLimitAndLastParam =
        GetLimitAndLastFlatAlpha_LGMSV;
    AllParams->CalibFunctionsForAlpha.GetSecondGuess =
        GetSecondGuessFlatAlpha_LGMSV;
    ...
    AllParams->CalibFunctionsForAlpha.UpdateConstsAfterParam =
        UpdateParamsAfterFlatAlpha_LGMSV;
  ...
  AllParams->CalibFunctionsForLambda.GetLimitAndLastParam =
      GetLimitAndLastLambda_LGMSV;
  AllParams->CalibFunctionsForLambda.GetSecondGuess =
      GetSecondGuessLambda_LGMSV;
  ...
  AllParams->CalibFunctionsForRho2.GetLimitAndLastParam =
      GetLimitAndLastRho2_LGMSV;
  ...
  AllParams->CalibFunctionsForRho2.UpdateConstsAfterParam =
      UpdateParamsAfterRho2_LGMSV;

  if (local_calib_params->fix_lambda) {
    err = CalibrateParamTS(0, AllCalibLongInst->iNbInst - 1, AllLongInstruments,
                           AllParams->LongInstParams, AllParams, model,
    ...
    if (iOne2F == 1) {
      err = CalibrateParamTS(0, 0, AllShortInstruments,
                             AllParams->ShortInstParams, AllParams, model,
  ...
  smessage("Total Iterations: %d        , Rho2: %d        , Lambda: %d        "
           ", Rho: %d        , Alpha: %d",
  ...
  if (AllParams->CalibParams->do_calib == 0 || shift_lam ||
      ...
      (model->iOne2F == 2 &&
       fabs(local_calib_params->rho2_sv_shift) > 1.0E-08)) {
    /* Update Lambda */
    if (shift_lam) {
      model->dLambdaX += params->lambda_shift;
      ...
      if (model->iOne2F == 2) {
        ...
        ConvertAlphaRho_LGM_to_LGMSV(
            model->iNbPWTime, model->dPWTime, model->dInitTStar,
            model->dLambdaX, model->dInitLGMAlpha, model->dLGMGamma,
      ...
      for (i = 0; i < AllParams->iNbLongInst; i++) {
        ...
        err = Calculate_HestonEquivalent_LGMSV_FromLGM_struct(
            NextInst, AllParams->CalibCpnLongSchedule, model,
    ...
    if (local_calib_params->onefac_rho && model->iOne2F == 2 &&
      ...
      for (i = 0; i < model->iNbPWTime; i++) {
        err = LGMSV_Get_RhoTarget_From_Rho(
            model->dRho[i], model->dRho2[i], local_calib_params->rho_mat1,
            local_calib_params->rho_mat2, model->dInitLGMAlpha,
            model->dLGMGamma, model->dInitLGMRho, &model->dFlatOneFactorRho);
        ...
        err = LGMSV_Get_Rho_From_RhoTarget(
            model->dFlatOneFactorRho, local_calib_params->rho_mat1,
            local_calib_params->rho_mat2, model->dInitLGMAlpha,
            model->dLGMGamma, model->dInitLGMRho, &(model->dRho[i]),
            &(model->dRho2[i]));
    ...
    err = CalibrateParamTS(0, AllParams->iNbLongInst - 1,
                           AllParams->AllLongInst, AllParams->LongInstParams,
                           AllParams, model, AllParams->CalibParams,
  ...
  if (inst_data) {
    ...
    for (i = 0; i < num_ex_datesl; i++) {
      inst_data->exer_dates_long[i] =
          (long)(CalibExeLongSchedule->dExeTimes[i] * DAYS_IN_YEAR +
    ...
    if (!calib_params->fix_lambda) {
      for (i = 0; i < num_ex_datess; i++) {
        inst_data->exer_dates_short[i] =
            (long)(CalibExeShortSchedule->dExeTimes[i] * DAYS_IN_YEAR +
                   EPS_MAT_TO_LONG_DATE_CONVERSION) +
            today;
...
Err Initialise_PricingConst(LGMSV_MODEL model, LGMSV_NUMERPARAMS NumerParams,
  ...
  if (!PricingConst->CoefIntT || !PricingConst->Coef1ReT ||
      !PricingConst->Coef2ReT || !PricingConst->Coef2ImT ||
      !PricingConst->Coef3ReT || !PricingConst->Coef3ImT || !PricingConst->dt ||
      !PricingConst->X || !PricingConst->W || !PricingConst->IntRe ||
      !PricingConst->IntIm) {
  ...
  if (NumerParams->iIntegMethod == 3 || NumerParams->iIntegMethod == 4) {
    ...
    LGMSVConstructLegendreGrid(4, dBreakPoints, iBreakNbX, PricingConst->XX,
                               PricingConst->WW);
  }
...
Err Initialise_NumerInst(LGMSV_MODEL model, CALIBINSTRUMENTDLM CalibInst,
  ...
  NumerInst->iEndIndex =
      Get_Index(CalibInst->dExeTime, model->dPWTime, model->iNbPWTime);
  ...
  if (CalibInst->iIsLiborOption) {
    ...
    if (fabs(CalibInst->dLiborFixTime -
             model->dPWTime[NumerInst->iSwitchIndex]) > 1.0E-08 &&
...
void Update_NumerInst(CALIBINSTRUMENTDLM CalibInst,
  ...
  for (i = 0; i < CalibInst->iNbStrike; i++) {
    NumerInst->dLogShiftStrike[i] =
        log(fabs(CalibInst->dStrike[i] + HestonParam->dShift));
  }
...
Free_EquiLGM(LGMSV_EQUILGM equi_lgm) {
  if (equi_lgm) {
...
Err Calculate_SV_Sensi(LGMSV_MODEL model, ALLCALININSTRUMENTSDLM AllCalibInst,
                       CPD_DIAG_CALIB_PARAM CPDParams,
                       LGMSV_CALIBPARAMS CalibParams, double *sensi_alpha,
                       double *sensi_rho) {
  int i, j, k;
    ...
    for (k = 0; k < 3; k++) {
      ...
      case 0: {
        ...
        alpha = 0.5 * model->dAlpha[0] *
                sqrt((1.0 - exp(-model->dLambdaEps[0] * Inst->dExeTime)) /
      ...
      err = srt_f_optsarbvol(Inst->dFwdCash, Inst->dFwdCash, Inst->dExeTime,
                             Inst->dATMVol, alpha, 0.0, rho, SRT_NORMAL,
                             SRT_BETAVOL, &beta_vol);
      ...
      for (j = 0; j < Inst->iNbStrike; j++) {
        err = srt_f_optsarbvol(Inst->dFwdCash, Inst->dStrike[j], Inst->dExeTime,
                               beta_vol, alpha, 0.0, rho, SRT_BETAVOL,
                               SRT_NORMAL, &price);
        ...
        price =
            srt_f_optblknrm(Inst->dFwdCash, Inst->dStrike[j], price,
                            Inst->dExeTime, Inst->dLevel, SRT_PUT, SRT_PREMIUM);
  ...
  *sensi_rho = 0.0;
  ...
  if (CalibParams->calib_rr_bt) {
    ...
    if (CalibParams->calib_rho || CalibParams->calib_rho2) {
      *sensi_rho =
          ((rho_price[1] - rho_price[2]) - (init_price[1] - init_price[2])) /
          rho_shift;
    }
  } else {
    if (CalibParams->calib_alpha) {
...
Err Initialise_AllParams(
  ...
  AllParams->HestonLongInst =
      calloc(AllCalibLongInst->iNbInst, sizeof(LGMSV_HestonInst));
  AllParams->NumerLongInst =
      calloc(AllCalibLongInst->iNbInst, sizeof(LGMSV_NumerInst));
  AllParams->lSigLongIndex = calloc(AllCalibLongInst->iNbInst, sizeof(long));

  if (!CalibParams->fix_lambda) {
    AllParams->HestonShortInst =
        calloc(AllCalibShortInst->iNbInst, sizeof(LGMSV_HestonInst));
    AllParams->NumerShortInst =
        calloc(AllCalibShortInst->iNbInst, sizeof(LGMSV_NumerInst));
    AllParams->lSigShortIndex =
        calloc(AllCalibShortInst->iNbInst, sizeof(long));
  }
  ...
  if (!AllParams->HestonLongInst || !AllParams->NumerLongInst ||
      !AllParams->lSigLongIndex ||
      (!AllParams->HestonShortInst && !CalibParams->fix_lambda) ||
      (!AllParams->NumerShortInst && !CalibParams->fix_lambda) ||
      (!AllParams->lSigShortIndex && !CalibParams->fix_lambda) ||
      !AllParams->PricingConst || !AllParams->CalibParams ||
      !AllParams->EquiLGM || !AllParams->CalibParamsLambda ||
      !AllParams->CalibParamsRho || !AllParams->CalibParamsAlpha ||
      !AllParams->CalibParamsRho2) {
  ...
  if (!CalibParams->fix_lambda || CalibParams->calib_alpha ||
      CalibParams->calib_lameps || CalibParams->calib_rho ||
  ...
  err = Initialise_CalibParams(paraml->use_jumps, paraml->precision,
                               paraml->nb_iter_max, 0, calib_vol, 0, 0, 0.0,
  ...
  err = Initialise_CalibParams(
      params->use_jumps, params->precision, params->nb_iter_max,
      CalibParams->recalib_at_end_lambda, !CalibParams->fix_lambda, 0, 1, 0.001,
      AllParams->CalibParamsLambda);
  ...
  err = Initialise_CalibParams(
      paraml->smile_use_jumps, paraml->smile_precision,
      paraml->smile_nb_iter_max, CalibParams->recalib_at_end_alpha,
      CalibParams->calib_alpha, 0, 1, 0.005, AllParams->CalibParamsAlpha);
  ...
  err = Initialise_CalibParams(
      paraml->smile_use_jumps, paraml->smile_precision,
      paraml->smile_nb_iter_max, CalibParams->recalib_at_end_lameps,
      CalibParams->calib_lameps, 0, 1, 0.001, AllParams->CalibParamsLamEps);
  ...
  err = Initialise_CalibParams(
      paraml->smile_use_jumps, paraml->smile_precision,
      paraml->smile_nb_iter_max, CalibParams->recalib_at_end_rho,
      CalibParams->calib_rho, 0, 1, 0.005, AllParams->CalibParamsRho);

  if (err)
    return err;

  err = Initialise_CalibParams(
      params->smile_use_jumps, params->smile_precision,
      params->smile_nb_iter_max, CalibParams->recalib_at_end_rho,
      CalibParams->calib_rho2, 0, 1, 0.005, AllParams->CalibParamsRho2);

  if (err)
    return err;

  memcpy(AllParams->lSigLongIndex, lSigLongIndex,
         AllCalibLongInst->iNbInst * sizeof(long));
  AllParams->iNbLongInst = AllCalibLongInst->iNbInst;

  for (i = 0; i < AllCalibLongInst->iNbInst; i++) {
    Calculate_HestonEquivalent_LGMSV_FromLGM_struct(
        &(AllCalibLongInst->sCalibInst[i]), CalibLongCpnSchedule, model,
    ...
    err = Initialise_NumerInst(model, &(AllCalibLongInst->sCalibInst[i]),
                               &(AllParams->HestonLongInst[i]),
  ...
  if (!CalibParams->fix_lambda) {
    memcpy(AllParams->lSigShortIndex, lSigShortIndex,
           AllCalibShortInst->iNbInst * sizeof(long));
    AllParams->iNbShortInst = AllCalibShortInst->iNbInst;

    for (i = 0; i < AllCalibShortInst->iNbInst; i++) {
      Calculate_HestonEquivalent_LGMSV_FromLGM_struct(
          &(AllCalibShortInst->sCalibInst[i]), CalibShortCpnSchedule, model,
          &(AllParams->HestonShortInst[i]));
      ...
      err = Initialise_NumerInst(model, &(AllCalibShortInst->sCalibInst[i]),
                                 &(AllParams->HestonShortInst[i]),
                                 &(AllParams->NumerShortInst[i]));
  ...
  /* Sensitivities */
  if (CalibParams->calib_alpha || CalibParams->calib_rho ||
      CalibParams->calib_rho2) {
    err = Calculate_SV_Sensi(model, AllCalibLongInst, paraml, CalibParams,
    ...
    if (CalibParams->onefac_rho) {
      ...
      if (model->iOne2F == 2) {
        err = LGMSV_Get_RhoTargetSensitivity_From_Rho(
            model->dRho[0], model->dRho2[0], CalibParams->rho_mat1,
            CalibParams->rho_mat2, model->dInitLGMAlpha, model->dLGMGamma,
            model->dInitLGMRho, &sensi1, &sensi2);
      ...
      /*
      AllParams->CalibParamsRho2->sensi = AllParams->CalibParamsRho->sensi *
      sensi2; AllParams->CalibParamsRho->sensi *= sensi1;
      */
...
Err LGMSVBumpLambdaModel(void *model_, int iIndex, double dNewValue) {
  ...
  if (model->iOne2F == 2) {
    ...
    /* adjust correlations */
    ConvertAlphaRho_LGM_to_LGMSV(
        model->iNbPWTime, model->dPWTime, model->dTStar, model->dLambdaX,
        model->dInitLGMAlpha, model->dLGMGamma, model->dInitLGMRho,
        model->dLGMAlpha, model->dLGMRho);
  }
...
Err LGMSVPricingShortInstruments(/* Instrument informations	*/
  ...
  Calculate_HestonEquivalent_LGMSV_FromLGM_struct(
      &(AllCalibInst->sCalibInst[iIndex]), AllCalibInst->sCalibCpnSchedule,
      model, &(AllParams->HestonShortInst[iIndex]));
  ...
  err = Calculate_ShiftAndVol_LGMSV_FromLGM_struct(
      CalibInst, AllCalibInst->sCalibCpnSchedule, model,
  ...
  LGMSVClosedFormApprox_struct(
      model, CalibInst, &(AllParams->HestonShortInst[iIndex]),
      AllParams->PricingConst, &(AllParams->NumerShortInst[iIndex]), dPrice);

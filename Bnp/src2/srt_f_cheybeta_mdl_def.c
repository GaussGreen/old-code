...
static SrtLst* get_next_sigma_date(
SrtLst*							ts)
{
	SrtLst* ls;
	ls = ts;
...
static SrtLst* get_next_tau_date(
SrtLst*							ts)
{
	SrtLst *ls;
...
void chey_beta_mdl_build_from_und(
CHEYBETA_MDL					*mdl,
SrtUndPtr						und)
{
	...
	for (	i = 0, ls = get_next_sigma_date (ts->head); 
			i < mdl->num_sigma; 
	...
	for (	i = 0, ls = get_next_tau_date (ts->head); 
			i < mdl->num_lambda; 
...
double							minvar)
{
	...
	double var = param->sigma 
			* param->sigma 
			* pow (fabs (f + x), 2 * param->beta);

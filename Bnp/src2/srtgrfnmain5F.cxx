...
char *SrtGrfn5DFXMc(char *und3dfx, double **correl, int numeventdates,
                    long *eventdates, long tableauRows, long *tableauCols,
                    char ***tableauStrings, int **tableauMask, long auxWidth,
                    long *auxLen, double **aux, long num_paths, int do_pecs,
                    double ***prod_val) {
  int free_str = 0;
  FIRSTAllMkts xStr;
  ...
  double *dom_ifr = NULL, *dom_fwd1 = NULL, *dom_fwd2 = NULL, *dom_exp1 = NULL,
         *dom_exp2 = NULL, *dom_phi1 = NULL, *dom_phi2 = NULL,
         *dom_phi12 = NULL, *dom_gam1_fwd = NULL, *dom_gam2_fwd = NULL,
         *dom_bond_pay = NULL, *dom_gam1_pay = NULL, *dom_gam2_pay = NULL,
         *for_ifr = NULL, *for_fwd1 = NULL, *for_fwd2 = NULL, *for_exp1 = NULL,
         *for_exp2 = NULL, *for_phi1 = NULL, *for_phi2 = NULL,
         *for_phi12 = NULL, *for_gam1_fwd = NULL, *for_gam2_fwd = NULL,
         *fx_fwd = NULL, ***covar = NULL;
  ...
  double lam_dom, lam_dom2, tau_dom, alpha_dom, gamma_dom, rho_dom, lam_for,
      lam_for2, tau_for, alpha_for, gamma_for, rho_for, correl_dom_for,
      correl_dom_fx, correl_for_fx;
  double spot_fx;
  char *dom_yc, *for_yc;
  int fx_idx, dom_idx, for_idx;

  double *sigma_date_dom = NULL, *sigma_dom = NULL, *sigma_date_for = NULL,
         *sigma_for = NULL, *sigma_date_fx = NULL, *sigma_fx = NULL,
         *merge_dates = NULL;
  ...
  err = Get_FX_StochRate_TermStructures5F(
      und3dfx, &sigma_date_dom, &sigma_dom, &sigma_n_dom, &tau_dom, &alpha_dom,
      &gamma_dom, &rho_dom, &sigma_date_for, &sigma_for, &sigma_n_for, &tau_for,
      &alpha_for, &gamma_for, &rho_for, &sigma_date_fx, &sigma_fx, &sigma_n_fx,
  ...
  if (!dom_ifr || !dom_fwd1 || !dom_fwd2 || !dom_phi1 || !dom_phi2 ||
      !dom_phi12 || !dom_exp1 || !dom_exp2 || !dom_gam1_fwd || !dom_gam2_fwd ||
      !dom_bond_pay || !dom_gam1_pay || !dom_gam2_pay || !for_ifr ||
      !for_fwd1 || !for_fwd2 || !for_phi1 || !for_phi2 || !for_phi12 ||
      !for_exp1 || !for_exp2 || !for_gam1_fwd || !for_gam2_fwd || !fx_fwd ||
      !covar) {
    err = "Memory allocation error (3) in SrtGrfn5DFXMc";
    goto FREE_RETURN;
  }

  fill_mc_init5F(pay_date, pay_time, date, time, nstp, merge_dates,
                 nb_merge_dates, sig_dom, lam_dom, alpha_dom, gamma_dom,
                 sig_for, lam_for, alpha_for, gamma_for, sig_fx, correl, dom_yc,
                 for_yc, dom_ifr, dom_fwd1, dom_fwd2, dom_exp1, dom_exp2,
                 dom_phi1, dom_phi2, dom_phi12, dom_gam1_fwd, dom_gam2_fwd,
                 dom_bond_pay, dom_gam1_pay, dom_gam2_pay, for_ifr, for_fwd1,
                 for_fwd2, for_exp1, for_exp2, for_phi1, for_phi2, for_phi12,
                 for_gam1_fwd, for_gam2_fwd, fx_fwd, covar);
    ...
    if (xStr.evt[i].evt) {
      ...
      if (grfn_prm->num_fx_df > 0 && grfn_prm->fx_idx != -1) {
        ...
        for (k = 0; k < grfn_prm->num_fx_df; k++) {
          ...
          grfn_prm->fx_gam1[k] =
              (1.0 - exp(-lam_dom * grfn_prm->fx_df_tms[k])) / lam_dom;
          grfn_prm->fx_gam2[k] =
              (1.0 - exp(-lam_dom2 * grfn_prm->fx_df_tms[k])) / lam_dom2;
          grfn_prm->fx_gam12[k] =
              -0.5 * (grfn_prm->fx_gam1[k] * grfn_prm->fx_gam1[k] *
                          dom_phi1[i + flag] +
                      grfn_prm->fx_gam2[k] * grfn_prm->fx_gam2[k] *
                          dom_phi2[i + flag]) -
      ...
      if (grfn_prm->num_dom_df > 0 && grfn_prm->dom_idx != -1) {
        ...
        for (k = 0; k < grfn_prm->num_dom_df; k++) {
          ...
          grfn_prm->dom_gam12[k] =
              -0.5 * (grfn_prm->dom_gam1[k] * grfn_prm->dom_gam1[k] *
                          dom_phi1[i + flag] +
                      grfn_prm->dom_gam2[k] * grfn_prm->dom_gam2[k] *
                          dom_phi2[i + flag]) -
              grfn_prm->dom_gam1[k] * grfn_prm->dom_gam2[k] *
                  dom_phi12[i + flag];
        }
      ...
      if (grfn_prm->num_for_df > 0 && grfn_prm->for_idx != -1) {
        ...
        for (k = 0; k < grfn_prm->num_for_df; k++) {
          ...
          grfn_prm->for_gam12[k] =
              -0.5 * (grfn_prm->for_gam1[k] * grfn_prm->for_gam1[k] *
                          for_phi1[i + flag] +
                      grfn_prm->for_gam2[k] * grfn_prm->for_gam2[k] *
                          for_phi2[i + flag]) -
              grfn_prm->for_gam1[k] * grfn_prm->for_gam2[k] *
                  for_phi12[i + flag];
        }
  ...
  *prod_val = dmatrix(0, num_col - 1, 0, 1);
  ...
  err = mc_main_5dfx(
      /*	Time data */
      num_paths, num_col, time, date, nstp, dom_ifr, dom_fwd1, dom_fwd2,
      dom_exp1, dom_exp2, dom_phi1, dom_phi2, dom_phi12, dom_gam1_fwd,
      dom_gam2_fwd, dom_bond_pay, dom_gam1_pay, dom_gam2_pay, for_ifr, for_fwd1,
      for_fwd2, for_exp1, for_exp2, for_phi1, for_phi2, for_phi12, for_gam1_fwd,
      for_gam2_fwd, fx_fwd, covar, void_prm,
      /*	Model data */
      lam_dom, alpha_dom, gamma_dom, lam_for, alpha_for, gamma_for, correl,
      /*	Market data */
...
char *SrtGrfn5DFXMcEB(char *und3dfx, double **correl, int numeventdates,
                      long *eventdates, int *nUsedEventDates, int *optimise,
                      double *fwd_iv, MCEBPARAMS params, long *resRows,
  ...
  *prod_val = dmatrix(0, *resRows - 1, 0, 2 + params->iNbIndex);
  ...
  err = mc_main_5dfx(
      ...
      spot_fx, dom_yc, for_yc,
      /* Do PECS adjustment */
      do_pecs, 1, ivOptimise, params, NULL,
      /*	Payoff function */
      grfn_payoff_4_5dfx_mc, /*	Result */
      *prod_val);
...
char *SrtGrfn5DFXMcTest(char *und3dfx, double **correl, int numeventdates,
                        long *eventdates, long tableauRows, long *tableauCols,
                        char ***tableauStrings, int **tableauMask,
                        long auxWidth, long *auxLen, double **aux,
                        long num_dates, long num_paths, int do_pecs,
                        double ***prod_val) {
  int free_str = 0;
  FIRSTAllMkts xStr;
  SrtGrfnParam defParm;
  ...
  double *dom_ifr = NULL, *dom_fwd = NULL, *dom_std = NULL, *dom_phi1 = NULL,
         *dom_phi2 = NULL, *dom_phi12 = NULL, *dom_beta = NULL,
         *dom_bond_pay = NULL, *dom_beta_pay = NULL,

         *for_ifr = NULL, *for_fwd = NULL, *for_std = NULL, *for_phi1 = NULL,
         *for_phi2 = NULL, *for_phi12 = NULL, *for_beta = NULL,
  ...
  if (!dom_ifr || !dom_fwd || !dom_std || !dom_phi1 || !dom_phi2 ||
      !dom_phi12 || !dom_beta || !dom_bond_pay || !dom_beta_pay || !for_ifr ||
      !for_fwd || !for_std || !for_phi1 || !for_phi2 || !for_phi12 ||
      !for_beta || !fx_fwd || !fx_std) {
    err = "Memory allocation error (5) in SrtGrfn5DFXMc";
    goto FREE_RETURN;
  }

  LGM2FExpectations(nstp, time, lam_dom, merge_dates, sig_dom, nb_merge_dates,
                    alpha_dom, gamma_dom, rho_dom, dom_phi1, dom_phi2,
                    dom_phi12);

  LGM2FExpectations(nstp, time, lam_for, merge_dates, sig_for, nb_merge_dates,
                    alpha_for, gamma_for, rho_for, for_phi1, for_phi2,
                    for_phi12);

  for (i = 1; i < nstp; i++) {
    sqdt = sqrt(time[i] - time[i - 1]);
    dom_std[i] =
        sigma_dom[Get_Index(time[i], sigma_date_dom, sigma_n_dom)] * sqdt;
    for_std[i] =
        sigma_for[Get_Index(time[i], sigma_date_for, sigma_n_for)] * sqdt;
    ...
    if (j >= 0 && fabs(date[i] - xStr.dts[j]) < 1.0e-04) {
      ...
      if (grfn_prm->num_fx_df > 0 && grfn_prm->fx_idx != -1) {
        ...
        for (k = 0; k < grfn_prm->num_fx_df; k++) {
          ...
          grfn_prm->fx_gam12[k] =
              -0.5 *
                  (grfn_prm->fx_gam1[k] * grfn_prm->fx_gam1[k] * dom_phi1[i] +
      ...
      if (grfn_prm->num_dom_df > 0 && grfn_prm->dom_idx != -1) {
        ...
        for (k = 0; k < grfn_prm->num_dom_df; k++) {
          ...
          grfn_prm->dom_gam12[k] =
              -0.5 *
                  (grfn_prm->dom_gam1[k] * grfn_prm->dom_gam1[k] * dom_phi1[i] +
                   grfn_prm->dom_gam2[k] * grfn_prm->dom_gam2[k] *
                       dom_phi2[i]) -
      ...
      if (grfn_prm->num_for_df > 0 && grfn_prm->for_idx != -1) {
        ...
        for (k = 0; k < grfn_prm->num_for_df; k++) {
          ...
          grfn_prm->for_gam12[k] =
              -0.5 *
                  (grfn_prm->for_gam1[k] * grfn_prm->for_gam1[k] * for_phi1[i] +
                   grfn_prm->for_gam2[k] * grfn_prm->for_gam2[k] *
                       for_phi2[i]) -
  ...
  *prod_val = dmatrix(0, num_col - 1, 0, 1);
  ...
  err = mc_main_5dfx_test(
      ...
      dom_fwd, dom_std, dom_phi1, dom_phi2, dom_phi12, dom_beta, dom_bond_pay,
      dom_beta_pay, for_ifr, for_fwd, for_std, for_phi1, for_phi2, for_phi12,
      for_beta, fx_fwd, fx_std, dom_for_cov, dom_fx_cov, for_fx_cov, void_prm,
      /*	Model data */
      lam_dom, alpha_dom, gamma_dom, rho_dom, lam_for, alpha_for, gamma_for,
      rho_for, correl,
      /*	Market data */
      spot_fx, dom_yc, for_yc,
      /* Do PECS adjustment */
...
static double Psi2_Func(double x, double y, double Tx, double Ty, double s,
                        double t) {
  double result;
...
Err fill_mc_init5Fsimple(
    long pay_date, double pay_time, double *date, double *time, long nb_dates,
    double *sig_dates, long nb_sig_dates, double *sig_curve_dom, double lda_dom,
    double *sig_curve_for, double lda_for, double *sig_curve_fx,
    double correl_dom_for, double correl_dom_fx, double correl_for_fx,
    char *dom_yc, char *for_yc, double *dom_ifr, double *dom_fwd,
    double *dom_std, double *dom_phi, double *dom_beta, double *dom_bond_pay,
    double *dom_beta_pay, double *for_ifr, double *for_fwd, double *for_std,
    double *for_phi, double *for_beta, double *fx_fwd, double *fx_std,
  ...
  for (k = 0; k < nb_dates - 1; k++) {
    ...
    QTexpect = -mat * (zc_for - zc_dom) -
               0.5 * (for_beta[k] * for_beta[k] * for_phi[k] -
    ...
    err = Fx3DtsImpliedVol(
        end_mat, start_mat, end_mat, sig_dates, nb_sig_dates, sig_curve_dom,
        lda_dom, sig_curve_for, lda_for, sig_dates, sig_curve_fx, nb_sig_dates,
        correl_dom_for, correl_dom_fx, correl_for_fx, &fx_vol);
    ...
    for (i = StartIndex; i < EndIndex + 1; i++) {
      ...
      adj_fx_pay +=
          correl_dom_fx * sig_dom * sig_fx *
              (Etha_Func(lda_dom, end_mat, T1, T2) -
    ...
    dom_fwd[k + 1] =
        ...
        dom_phi[k] * exp(-lda_dom * mat) *
            Phi_Func(-lda_dom, start_mat, start_mat, end_mat);

    dom_std[k + 1] = exp(-lda_dom * end_mat) * sqrt(var_dom);
    for_fwd[k + 1] =
        1.0 / lda_for * expect_for +
        exp(-lda_for * end_mat) * (-correl_dom_for * adj_pay_for / lda_dom -
                                   correl_for_fx * adj_quanto) +
        for_phi[k] * exp(-lda_for * mat) *
            Phi_Func(-lda_for, start_mat, start_mat, end_mat);
    ...
    /* Covariance*/
    dom_for_cov[k + 1] =
        correl_dom_for * correl12 * exp(-(lda_dom + lda_for) * end_mat);
...
Err fill_mc_init5F(long pay_date, double pay_time, double *date, double *time,
                   long nb_dates, double *sig_dates, long nb_sig_dates,
                   double *sig_curve_dom, double lda_dom, double alpha_dom,
                   double gamma_dom, double *sig_curve_for, double lda_for,
                   double alpha_for, double gamma_for, double *sig_curve_fx,
                   double **correl, char *dom_yc, char *for_yc, double *dom_ifr,
                   double *dom_fwd1, double *dom_fwd2, double *dom_exp1,
                   double *dom_exp2, double *dom_phi1, double *dom_phi2,
                   double *dom_phi12, double *dom_gam1_fwd,
                   double *dom_gam2_fwd, double *dom_bond_pay,
                   double *dom_gam1_pay, double *dom_gam2_pay, double *for_ifr,
                   double *for_fwd1, double *for_fwd2, double *for_exp1,
                   double *for_exp2, double *for_phi1, double *for_phi2,
                   double *for_phi12, double *for_gam1_fwd,
                   double *for_gam2_fwd, double *fx_fwd, double ***covar) {
  ...
  double psif_domfor12_Tend, psif_domfor12_Tpay, psif_domfor21_Tend,
      psif_domfor21_Tpay, psif_domfor11_Tend, psif_domfor11_Tpay,
      psif_domfor22_Tend, psif_domfor22_Tpay, psif_domdom11_Tend,
      psif_domdom11_Tpay, psif_domdom12_Tend, psif_domdom12_Tpay,
      psif_domdom21_Tend, psif_domdom21_Tpay, psif_domdom22_Tend,
      psif_domdom22_Tpay;

  double adj_pay_fx1, adj_pay_fx2, adj_pay_fx3, adj_pay_fx4, adj_pay_fx5,
      adj_pay_fx6, adj_pay_fx7, adj_pay_fx8, adj_pay_fx9, adj_pay_fx10;

  double covar_dom1_fx1, covar_dom1_fx2, covar_dom1_fx3, covar_dom1_fx4,
      covar_dom1_fx5, covar_dom2_fx1, covar_dom2_fx2, covar_dom2_fx3,
      covar_dom2_fx4, covar_dom2_fx5, covar_for1_fx1, covar_for1_fx2,
      covar_for1_fx3, covar_for1_fx4, covar_for1_fx5, covar_for2_fx1,
      covar_for2_fx2, covar_for2_fx3, covar_for2_fx4, covar_for2_fx5;

  double mat_pay, mat, pay_mat, pay_mat2;
  double zc_for, zc_dom, zc_pay;
  double FxQTexpect, expect_fx, fx_vol;
  double exp_dom_mat, exp_dom_mat2, exp_dom_pay, exp_dom_pay2, exp_for_mat,
      exp_for_mat2, exp_for_pay, exp_for_pay2;
  int i, k;
  long StartIndex, EndIndex;
  ...
  for (k = 0; k < nb_dates - 1; k++) {
    ...
    err = Fx5DtsImpliedVol(
        end_mat, start_mat, end_mat, sig_dates, nb_sig_dates, sig_curve_dom,
        lda_dom, alpha_dom, gamma_dom, sig_curve_for, lda_for, alpha_for,
        gamma_for, sig_dates, sig_curve_fx, nb_sig_dates, correl, &fx_vol);
    ...
    covar_dom1_fx1 = covar_dom1_fx2 = covar_dom1_fx3 = covar_dom1_fx4 =
        covar_dom1_fx5 = 0.0;
    covar_dom2_fx1 = covar_dom2_fx2 = covar_dom2_fx3 = covar_dom2_fx4 =
        covar_dom2_fx5 = 0.0;
    covar_for1_fx1 = covar_for1_fx2 = covar_for1_fx3 = covar_for1_fx4 =
        covar_for1_fx5 = 0.0;
    covar_for2_fx1 = covar_for2_fx2 = covar_for2_fx3 = covar_for2_fx4 =
        covar_for2_fx5 = 0.0;
    ...
    for (i = StartIndex; i < EndIndex + 1; i++) {
      ...
      psif_domfor11_Tend =
          Psi2_Func(lda_dom, lda_for, end_mat, end_mat, T1, T2);
      psif_domfor11_Tpay =
          Psi2_Func(lda_dom, lda_for, mat_pay, end_mat, T1, T2);

      psif_domfor21_Tend =
          Psi2_Func(lda_dom2, lda_for, end_mat, end_mat, T1, T2);
      psif_domfor21_Tpay =
          Psi2_Func(lda_dom2, lda_for, mat_pay, end_mat, T1, T2);

      psif_domfor12_Tend =
          Psi2_Func(lda_dom, lda_for2, end_mat, end_mat, T1, T2);
      psif_domfor12_Tpay =
          Psi2_Func(lda_dom, lda_for2, mat_pay, end_mat, T1, T2);

      psif_domfor22_Tend =
          Psi2_Func(lda_dom2, lda_for2, end_mat, end_mat, T1, T2);
      psif_domfor22_Tpay =
          Psi2_Func(lda_dom2, lda_for2, mat_pay, end_mat, T1, T2);

      psif_domdom11_Tend =
          Psi2_Func(lda_dom, lda_dom, end_mat, end_mat, T1, T2);
      psif_domdom11_Tpay =
          Psi2_Func(lda_dom, lda_dom, mat_pay, end_mat, T1, T2);

      psif_domdom21_Tend =
          Psi2_Func(lda_dom2, lda_dom, end_mat, end_mat, T1, T2);
      psif_domdom21_Tpay =
          Psi2_Func(lda_dom2, lda_dom, mat_pay, end_mat, T1, T2);

      psif_domdom12_Tend =
          Psi2_Func(lda_dom, lda_dom2, end_mat, end_mat, T1, T2);
      psif_domdom12_Tpay =
          Psi2_Func(lda_dom, lda_dom2, mat_pay, end_mat, T1, T2);

      psif_domdom22_Tend =
          Psi2_Func(lda_dom2, lda_dom2, end_mat, end_mat, T1, T2);
      psif_domdom22_Tpay =
          Psi2_Func(lda_dom2, lda_dom2, mat_pay, end_mat, T1, T2);
    ...
    dom_phi2[k + 1] =
        dom_phi2[k] * exp_dom_mat2 * exp_dom_mat2 + alpha_dom2 * phi_dom2;
    dom_phi12[k + 1] = dom_phi12[k] * exp_dom_mat * exp_dom_mat2 +
                       correl[0][1] * alpha_dom * phi_dom12;

    dom_fwd1[k + 1] = dom_phi1[k] * exp_dom_mat *
                          Phi_Func(-lda_dom, start_mat, start_mat, end_mat) +
                      expect_dom1 / lda_dom +
                      dom_phi12[k] * exp_dom_mat *
                          Phi_Func(-lda_dom2, start_mat, start_mat, end_mat) +
                      correl[0][1] * alpha_dom * expect_dom12 / lda_dom2 -
                      adj_pay_dom1 / lda_dom -
                      correl[0][1] * alpha_dom * adj_pay_dom12 / lda_dom2;

    dom_fwd2[k + 1] = dom_phi2[k] * exp_dom_mat2 *
                          Phi_Func(-lda_dom2, start_mat, start_mat, end_mat) +
                      alpha_dom2 * expect_dom2 / lda_dom2 +
                      dom_phi12[k] * exp_dom_mat2 *
                          Phi_Func(-lda_dom, start_mat, start_mat, end_mat) +
    ...
    for_phi2[k + 1] =
        for_phi2[k] * exp_for_mat2 * exp_for_mat2 + alpha_for2 * phi_for2;
    for_phi12[k + 1] = for_phi12[k] * exp_for_mat * exp_for_mat2 +
                       correl[2][3] * alpha_for * phi_for12;

    for_fwd1[k + 1] = for_phi1[k] * exp_for_mat *
                          Phi_Func(-lda_for, start_mat, start_mat, end_mat) +
                      expect_for1 / lda_for +
                      for_phi12[k] * exp_for_mat *
                          Phi_Func(-lda_for2, start_mat, start_mat, end_mat) +
                      correl[2][3] * alpha_for * expect_for12 / lda_for2 -
                      correl[0][2] * adj_pay_for1 / lda_dom -
                      correl[1][2] * alpha_dom * adj_pay_for12 / lda_dom2 -
                      correl[2][4] * adj_quanto_for1;

    for_fwd2[k + 1] =
        for_phi2[k] * exp_for_mat2 *
            Phi_Func(-lda_for2, start_mat, start_mat, end_mat) +
        alpha_for2 * expect_for2 / lda_for2 +
        for_phi12[k] * exp_for_mat2 *
            Phi_Func(-lda_for, start_mat, start_mat, end_mat) +
    ...
    fx_fwd[k + 1] =
        expect_fx + correl[0][4] * adj_pay_fx1 +
        correl[1][4] * alpha_dom * adj_pay_fx2 - correl[0][2] * adj_pay_fx3 -
        correl[1][2] * alpha_dom * adj_pay_fx4 -
        correl[0][3] * alpha_for * adj_pay_fx5 -
        correl[1][3] * alpha_domfor * adj_pay_fx6 + adj_pay_fx7 +
        correl[0][1] * alpha_dom * adj_pay_fx8 +
    ...
    cov[0][4] = cov[4][0] =
        correl[0][4] * covar_dom1_fx1 -
        correl[0][2] * covar_dom1_fx2 / lda_for -
        correl[0][3] * alpha_for * covar_dom1_fx3 / lda_for2 +
        covar_dom1_fx4 / lda_dom +
    ...
    cov[1][4] = cov[4][1] =
        correl[1][4] * alpha_dom * covar_dom2_fx1 -
    ...
    cov[2][4] = cov[4][2] =
        correl[2][4] * covar_for1_fx1 - covar_for1_fx2 / lda_for -
    ...
    cov[3][4] = cov[4][3] =
        correl[3][4] * alpha_for * covar_for2_fx1 -

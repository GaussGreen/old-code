...
void MomentsComputing(int      iDimension,
   				      double   dMaturity,
				      double*  dForwards,
				      double*  dVols,
				      double** dCorrels,
				      double*  dMoments)
{
	...
	for(i=0;i<iDimension;i++)
	{
		...
		{
			dMoment2Container = dMoment2Container + dForwards[j]*exp(dVols[i]*dVols[j]*dCorrels[i][j]*dMaturity);
		}
		...
		for(j=0;j<iDimension;j++)
		{
			...
			{
				dMoment3Container1 = dMoment3Container1 + dForwards[k]*exp((dVols[i]*dVols[j]*dCorrels[i][j] +
																		    dVols[i]*dVols[k]*dCorrels[i][k] +
																		    dVols[k]*dVols[j]*dCorrels[k][j])*dMaturity);
			}
...
void LogNormalParametersComputing(double*  dMoments,
								  double*  dParameters)
{				
...
Err srt_f_BasketCapPrice(int				iNbRates,
						 double				dStartDate,
						 double				dStrike,
						 double             *dForwards,
						 double             *dVols,
...
double*             dParameters;	
	...
	switch(SrtGreek)
	{
		case PREMIUM : 
			...
			LogNormalParametersComputing(dMoments,
										 dParameters);
...
FREE_RETURN:
	...
	if (ex) free_dvector (ex, 0, nidx-1);
	if (cov) free_dmatrix (cov, 0, nidx-1, 0, nidx-1);
	if (h) free_dmatrix (h, 0, npth-1, 0, nidx-1);
	if (chol) free_dmatrix (chol, 0, nidx-1, 0, nidx-1);
	if (yti) free_dvector (yti, 0, nidx-1);	
...
FREE_RETURN:
	...
	if (fyi_) free_dvector (fyi_, 0, nidx - 1);
	if (ki_) free_dvector (ki_, 0, nidx - 1);

	return result;
}

static void atm_beta_2_sl(
double		fwd,
double		atm,
double		beta,
double		*shift,
...
FREE_RETURN:
	...
	if (g) free_f3tensor (g, 0, npth-1, 0, 0, 0, nidx);
	sobol_free ();
	return result;
}
...
FREE_RETURN:
	...
	if (g) free_f3tensor (g, 0, npth-1, 0, 0, 0, nidx-1);
	sobol_free ();
	return result;
}
...
double		*beta)
{
	...
	if (fabs (s2 - s1) < 1.0e-04) s2 += 1.0e-04;
...
double		*idx_ref,						/*	The base values for indices 
													...
													or interpolated from forward curve if Ref date > Today */
...
double		*fx_vol,						/*	For each eqd index, the volatility of the corresponding Fx
													interpolated from the implied BS volatility curve of Fx
													for the fixing date */
...
i_stellar_info	*info)
{
	...
	for (i=0; i<nidx; i++)
	{
		...
		err = find_shifted_log_params (fix_mat_years, idx_fwd[i], 
			str1, vol1,str2, vol2,
			shift+i, vol+i, beta+i);
	...
	/*	For each eqd index, adjust for quanto */
		...
		{
			idx_fwd[i] 
				= (idx_fwd[i] + shift[i]) * exp (fx_correl[i] * fx_vol[i] * vol[i] * fix_mat_years) - shift[i];
		}
	...
	*pv *= pay_df;
	...
	/*	Free memory */
FREE_RETURN:
	if (shift) free_dvector (shift, 0, nidx - 1);
	if (vol) free_dvector (vol, 0, nidx - 1);
	if (beta) free_dvector (beta, 0, nidx - 1);
...
FREE_RETURN:
	...
	if (ex) free_dvector (ex, 0, nidx);
	if (cov) free_dmatrix (cov, 0, nidx, 0, nidx);
	if (h) free_dmatrix (h, 0, npth-1, 0, nidx);
	if (chol) free_dmatrix (chol, 0, nidx, 0, nidx);
	if (yti) free_dvector (yti, 0, nidx-1);	

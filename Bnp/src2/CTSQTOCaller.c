...
Err ctsqto_caller(
					...
					/*		calls */
					int			ncall,
					int			pay_rec,				/*	0: rec exo, 1: pay exo */
					long		*ex_date,
					long		*set_date,
					double		*fee,
					/*	Numerical params */
...
//		NUM_HERMITE = req_stpx;
		err = ctsqto_calc_mdl_iv_fwd(	ctsqto,
							und,
							adi_arg,
							NUM_HERMITE,
							und->model_fwdiv);
	...
	if (fund_leg->num_cpn > 0)
	{
		if (for_fund)
		{
			...
			if (calc_fwdiv)
			{
					...
					{
						callt = ctsqto->call + i;
						und->market_fwdiv[i] = fact * swp_f_df (today, (ctsqto->fund_leg->cpn + callt->fund_idx)->start_date, dom_yc)
						* eq_final_ex;
					}
				}
			}
...
//					smessage("while spread");
	...
	else
	{
		if (fund_leg->num_cpn > 0)
		{
			temp = swp_f_df (today, fund_leg->cpn[fund_leg->num_cpn-1].pay_date, dom_yc) 
			* fund_leg->notional;
			fund_leg_pv -= temp;
			if (calc_fwdiv)
			{
...
//						SRT_BDAY, MODIFIED_SUCCEEDING ) < today + eod_fix_flag)
...
//	ra_leg = ctsqto->ra_leg;
	...
	smessage("RA_FwdPV");
	err = RA_FwdPV(dom_yc, for_yc, ra_leg, today, nbFwdPVDates, FwdPVDates, fwdPV);
	if(err)
	{
	...
	while (i < ra_ncpn && ra_fixingdates[i][0] < today + eod_fix_flag)
	{
		if (ra_pay[i] >= today + eod_pay_flag)
		{
			...
			weight_fixing = coverage (ra_start[i], ra_pay[i], bas) 
				/ ((ra_pay[i] - add_unit(ra_fixingdates[i][0], spotlag_f, SRT_BDAY, MODIFIED_SUCCEEDING )) * 1.0)
				* ra_not
				* swp_f_df (today, ra_pay[i], dom_yc);

			while (j < ra_nfixings[i] && ra_fixingdates[i][j] < today + eod_fix_flag)
			{
				...
				if (ra_cpn_type != 0)
				{
					...
					else
					{
						...
						d_cpn = add_unit(ra_float_startl[i], tenor_float_cpn, SRT_MONTH, MODIFIED_SUCCEEDING);
						...
						fixing_cpn_float = add_unit(ra_float_startl[i], -spotlag, SRT_BDAY, MODIFIED_SUCCEEDING);
						...
						ra_cpn_float = swp_f_fra(ra_float_startl[i], d_cpn, basis_float_cpn, yc_cpn_float, ra_float_refrate);
						...
						if (ra_float_refrate_is_dom_for == 1)
						{
							...
							for (; q < n_fxvol_dates-1 && fxvol_dates[q+1] < ra_float_startl[i]; q++);
							...
							else
							{
								...
								vol_fx_cpn = coef_cpn * fxvol[q+1] + (1.0-coef_cpn) * fxvol[q];
								rho_ffx_cpn = coef_cpn * qtocorrel[q+1] + (1.0-coef_cpn) * qtocorrel[q];
							}
						...
						if (ra_float_startl[i] == ra_start[i] && d_cpn == ra_start[i+1]) DRS_corr_cpn = 0;
						else
						{
							...
							err = swp_f_SABRvol(dom_vc, d1_cpn, d2_cpn, fra_d_cpn, &vol_d_cpn, &power, SABR_LOGVOL);
							if (err) return err;
									
							DRS_corr_cpn = rho_df * vol_d_cpn * vol_fra_cpn * cvg_d_cpn * fra_d_cpn / (1.0 + cvg_d_cpn * fra_d_cpn);
							if (ra_start[i+1] > d_cpn) DRS_corr_cpn = -DRS_corr_cpn;
						}

						// DRS floating coupon is...
						ra_cpn_float *= ra_float_gearings[i] * exp((DRS_corr_cpn + quanto_corr_cpn) * (fixing_cpn_float - today) / 365.0);
					}
				}
				else
				{
				...
				if((ra_fixings[i][j] > lower_barr[i]) && (ra_fixings[i][j] < upper_barr[i]) )
				// For testing: if( (ra_fixings[i][j] > -0.1) && (ra_fixings[i][j] > lower_barr[i]) && (ra_fixings[i][j] < upper_barr[i]) )
				{
					if (j < ra_nfixings[i]-1)
					{
						ra_leg_pv -= (add_unit(ra_fixingdates[i][j+1], spotlag_f, SRT_BDAY, MODIFIED_SUCCEEDING )
							- add_unit(ra_fixingdates[i][j], spotlag_f, SRT_BDAY, MODIFIED_SUCCEEDING ))							
						* weight_fixing
					...
					{
						ra_leg_pv -=  (ra_pay[i]
							- add_unit(ra_fixingdates[i][j], spotlag_f, SRT_BDAY, MODIFIED_SUCCEEDING )) * (ra_cpns[i] + ra_cpn_float)
						* weight_fixing
						* (ra_cpns[i] + ra_cpn_float);
					}
				}
			...
			if (j < ra_nfixings[i])
			{
				...
				ra_leg_fixing = (RangeAccrualStruct *) calloc(1, sizeof(RangeAccrualStruct));
				if(!ra_leg_fixing)
				{
					err = "Memory Allocation Failed ra_leg";
					goto FREE_RETURN;
				...
				dates_fixing[0] = add_unit( ra_fixingdates[i][j], spotlag_f, SRT_BDAY, MODIFIED_SUCCEEDING );
				...
				err = RA_FwdPV(dom_yc, for_yc, ra_leg_fixing, today, 1, FwdPVDates, &ra_leg_pv_fixing);

				/* coverage adjustment */
				ra_leg_pv_fixing *= coverage (ra_start[i], ra_pay[i], bas) / coverage (dates_fixing[0], ra_pay[i], bas);

				/* fixing adjustment */
				ra_leg_pv_fixing *= (dates_fixing[1] - dates_fixing[0])
					/ ((ra_pay[i] - add_unit(ra_fixingdates[i][0], spotlag_f, SRT_BDAY, MODIFIED_SUCCEEDING )) * 1.0);
	...
	if (for_fund)
	{
		...
		if (calc_fwdiv)
		{
			...
			{
				callt = ctsqto->call + i;
				und->market_fwdiv[i] -= -fact * swp_f_df (today, ctsqto->ra_leg->dates[callt->ra_idx], dom_yc)
				* ra_not;
			}
		}
	}
	...
	if (calc_fwdiv)
	{
		for (i=0; i<ctsqto->num_calls; i++)
		{
			...
			// Was - (und-> replaced by +) to be consistent with convention for fee (fee is PAID)
			und->extra_fees[i] = +(und->market_fwdiv[i] + und->model_fwdiv[i]) / swp_f_df (today, callt->set_date, dom_yc);
...
FREE_RETURN:
	...
	ra_free( ra_leg);
	if(ra_leg) free(ra_leg);
	
	ra_free( ra_leg_fixing);
	if(ra_leg_fixing) free(ra_leg_fixing);

	smessage("Free CTSQTOCaller 1");
	if (rafreqchar) free(rafreqchar);

	smessage("Free CTSQTOCaller 2");
	if (dates) free_lvector(dates, 0,ra_ncpn);

	smessage("Free CTSQTOCaller 3");
	if (fwdPV) free_dvector(fwdPV, 0,ctsqto->num_calls);

	smessage("Free CTSQTOCaller 4");
	if (FwdPVDates) free(FwdPVDates);
	...
	if (fund_mrg2) free (fund_mrg2);
	if (fund_spr2) free (fund_spr2);
	if (fund_fix_cpn2) free (fund_fix_cpn2);

...
void Zero_6m(TERM_DATA* term_data) /* Structure of term structure data */
{
    ...
    int IndexSwapYield[] = {4, 6, 8, 10, 14, 20, 40, 60}, /* Swap benchmark points in 1/2 years */
        i;

    SwapYield = mbs_dvector(0, 2 * MAXMAT);
    YieldS    = mbs_dvector(0, 2 * MAXMAT);

    //	term_data->ValueDate = Nxtbusday (      term_data->Today,               /* Value date is
    //today shifted by (Spotdays) business days */ 						term_data->Spotdays, term_data->Holidays,
    //term_data->numHolidays);

    for (i = 0; i < 8; i++)
    { /* We fill the swap yield array with the benchmark yields */
        SwapYield[IndexSwapYield[i]] = term_data->SwapYield[i];
        YieldS[IndexSwapYield[i]]    = term_data->YieldS[i];
    }

    yldconv(
        term_data->Zero,  /* Conversion of the money market rates. */
    ...
    spotrate(
        term_data->Zero + 3, /* Bootstapping of swap curve to get zeros at 6mo intervals */
        term_data->ZeroS + 3,
        term_data->Period +
            3,     /* We pass the pointers Zero and Period starting at 3 so that (Zero + 3)[1] */
    ...
    int     MMB)           /* Money Market basis (360 or 365) */
{
    ...
    for (i = 1; i < 6; i++) /* 1,2,3,6,12 month Money Market rates */
    {
        ...
        Zero[i] =
            pow(1. + MMYield[i] / 100. * Period[i] / (double)MMB, 365. / (double)Period[i]) - 1.;
    ...
    char*   YBase)       /* Year basis for the benchmark swaps ("ACT" or "365") */
{
        ...
        for (j = 2; j <= MAXMAT; j++)
        {
            ...
            {                    /* receive is the quoted one x number of days in  */
                temp1 = (SwapYield[2 * j] * Period[2] /
                        ...
                        / pow(1. + Zero[2],
                              (double)(Period[2] / 365.)); /* a difference only for leap years. */
                temp3 = (YieldS[2 * j] * Period[2] /
                         365.) /* the coupon period / 365. For annual it makes   */
                        / pow(1. + ZeroS[2],
                              (double)(Period[2] / 365.)); /* a difference only for leap years. */
                for (i = 2; i < j; i++)
                { /* Note that we use Period[2] instead of          */
                    temp1 += (SwapYield[2 * j] * (Period[2 * i] - Period[2 * (i - 1)]) /
                              365.) /* Period[2*i] - Period[2*(i-1)] when i=1 because */
                             / pow(1. + Zero[2 * i],
                                   (double)(Period[2 * i] / 365.)); /* we need Period[0]=0 which is
                                                                       not the case here */
                    temp3 += (YieldS[2 * j] * (Period[2 * i] - Period[2 * (i - 1)]) /
                              365.) /* Period[2*i] - Period[2*(i-1)] when i=1 because */
                             / pow(1. + ZeroS[2 * i],
                                   (double)(Period[2 * i] / 365.)); /* we need Period[0]=0 which is
                                                                       not the case here */
            ...
            else
            {
                ...
                temp3 = 0.;
                for (i = 1; i < j; i++)
                {                             /* In the "ACT" convention the coupon is always   */
            ...
            newdate = Nxtmth(
                ValueDate, /* Maturity date of the current zero */
                (long)j * 12,
                (long)1);
            Period[2 * j] = Daysact(
                ValueDate, /* Maturity of the current zero in days */
                newdate);

            if (YBase[0] == '3')
            {
                Zero[2 * j] =
                    pow((100. + SwapYield[2 * j] *
                                    (Period[2 * j] -
            ...
            {
                Zero[2 * j] =
                    pow((100. + SwapYield[2 * j]) / (100. - temp1), 365. / (double)Period[2 * j]) -
                    1.;
                ZeroS[2 * j] =
                    pow((100. + YieldS[2 * j]) / (100. - temp3), 365. / (double)Period[2 * j]) - 1.;
            }

            newdate = Nxtmth(
                ValueDate, /* Maturity date of the zero maturing 6m before the current one */
                (long)(2 * j - 1) * 6,
                (long)1);
            Period[2 * j - 1] = Daysact(
                ValueDate, /* Maturity date of the zero maturing 6m before the current one, in days
                            */
                newdate);
            linterp(
                (double)Period[2 * j - 1], /* Interpolation of the zero maturing 6m before the
                                              current one */
                &temp,
                (double)Period[2 * j - 2],
                (double)Period[2 * j],
    ...
    else
    {
        ...
        for (j = 3; j <= 2 * MAXMAT; j++)
        {
            ...
            {
                temp1 = 0.; /* temp1 is the price of an annuity paying a coupon of SwapYield[j]
                               every 6m    */
                temp3 = 0.;
                for (i = 1; i < j; i++)
                {
                    temp1 +=
                        (SwapYield[j] / 2.) / pow(1. + Zero[i] / 2., (double)(Period[i] / 182.5));
                    temp3 +=
                        (YieldS[j] / 2.) / pow(1. + ZeroS[i] / 2., (double)(Period[i] / 182.5));
                }
            } /* if then else */
            ...
            if (YBase[0] == '3')
            {
                Zero[j] = 2. * (pow((100. + SwapYield[j] * (Period[j] - Period[j - 1]) / 365.) /
                                        (100. - temp1),
                                    (182.5 / (double)Period[j])) -
                                1.);
                ZeroS[j] =
                    2. *
                    (pow((100. + YieldS[j] * (Period[j] - Period[j - 1]) / 365.) / (100. - temp3),
                         (182.5 / (double)Period[j])) -
                     1.);
            }
            else
    ...
    TREE_DATA* tree_data) /* Output: Structure of tree data */
{
    ...
    {
        Date = Nxtmth(
            Maturity, /* Calculate the current coupon date by going backward from maturity */
            (long)(-12 * NbCoupon++ / F),
            (long)1);
        Time = Daysact(ValueDate, Date);
    } while (Time > 0);

    NbDate = NbExer + NbFwd + NbCoupon +
             1; /* The dates are: the value date, the exercise dates, the coupon dates */
    ...
    for (i = 0; i < NbExer; i++) /* Exercise dates */
    {
        CDate[DateIndex] =
            Daysact(
                ValueDate, /* Time from value date to current exercise date (ACT/365) */
                Exer[i]) /
            365.;
    ...
    for (i = 0; i < NbFwd; i++) /* Forward maturities */
    {
        CDate[DateIndex] =
            Daysact(
                ValueDate, /* Time from value date to current forward maturity (ACT/365) */
                FwdMat[i]) /
            365.;
    ...
    for (i = 0; i < NbCoupon;
         i++) /* Coupon dates. The last CDate is only used for accrued interest calculation. */
    {         /* It correspond to the coupon falling before the value date                    */
        Date = Nxtmth(
            Maturity, /* Current coupon date */
            (long)(-12 * i / F),
            (long)1);
        CDate[DateIndex] =
            Daysact(
                ValueDate, /* Time from value date to current coupon date (ACT/365) */
                Date) /
            365.;
    ...
    pksort3(
        NbDate, /* Sort the CDate in ascending order */
        CDate,
        ExerNode,
        FwdNode);

    for (i = 0, NbNodes = 0, ExerIndex = 0, FwdIndex = 0; i < NbDate - 1;
         i++) /* Total number of periods and number of periods between critical dates */
    {
        x = Ppy * (CDate[i + 1] - CDate[i]); /* The number of periods between two critical dates is
                                                the smallest integer */
        NodePerPeriod[i] =
            (int)ceil(x); /* that gives a period length smaller than 1/Ppy (in years) */
    ...
    Tree_Alloc(
        tree_data, /* Allocate memory for the arrays constituting the tree. We could not do it */
        NbNodes);  /* before because we did not know the total number of nodes NbNodes.        */

    for (i = 0, k = 0; i < NbDate - 1;
         i++) /* Calculate the length of each time step (period) which is store in the global */
    {         /* structure tree_data->Length                                                  */
        if (!NodePerPeriod[i]) /* If two consecutive dates were equal, NodePerPeriod[i] = 0. We
                                  ignore it      */
            continue;

        x = (CDate[i + 1] - CDate[i]) / NodePerPeriod[i]; /* All periods between CDate[i] and
                                                             CDate[i+1] have the same length x */
    ...
    tree_data->Length[k] =
        x; /* We add one more time step which will be used for probability calculation */

    for (i = 0, k = NbNodes; i < NbCoupon - 1;
         i++) /* Calculation of accrued interest at each node */
    {
        x = Coupon[i] - Coupon[i + 1]; /* Time between current coupon and preceding one, going
                                          backward in time */
    ...
    if ((EoA == 'A') &&
        (NbExer >
         1)) /* For American option, we interpolate the strike linearly between two dates */
...
{
    tree_data->ZeroCoupon = mbs_dvector(
        0, NbNodes + 1); /* As we have to calculate probabilities up to NbNodes, we need an extra */
    tree_data->FwdRate =
        mbs_dvector(0, NbNodes + 1); /* forward and, consequently, an extra zero coupon price. */
    ...
    PREPAY_DATA* prepay_data) /* Structure of prepayment data */
{
    ...
    Forward(
        tree_data->ZeroCoupon, /* Calculation of zero coupon price at each node in the tree */
    ...
    //**NOT USED**
    Forward(
        tree_data->ZeroCouponS, /* Calculation of zero coupon price at each node in the tree */
    ...
    //**NOT USED**
    Forward_Prepay(
        tree_data->FwdPrepay, /* Calculation of forward prepayment at each node in the tree */
        prepay_data->Spot,
        tree_data->NbNodes);

    if (err = Forward_Vol(
            tree_data->FwdVol, /* Calculation of forward rate volatility at each node in the tree */
            &(tree_data->Jump),
            tree_data->FwdRate,
            tree_data->Length,
            term_data->Zero,
    ...
    //**NOT USED**
    Prepay_Vol(
        tree_data->PrepayVol, /* Calculation of instantaneous volatility of forward prepayment */
        prepay_data->SpotVol,
        tree_data->NbNodes);

    //**NOT USED**
    Conv_Adj(
        tree_data->FwdRate, /* Calculate convexity adjustment for discounting rates */
        tree_data->FwdRateS,
        tree_data->FwdVol,
        tree_data->Length,
        tree_data->Delta,
    ...
    if (err = Drift(
            tree_data->Drift, /* Calculation of the 1 period rate drift in the tree */
            &(tree_data->NodeMax),
            tree_data->Jump,
    ...
    //**NOT USED**
    NodeMax2(
        &(tree_data->Jump2), /* Calculate the jump in the prepayment tree and the maximum node ever
                                attained */
        &(tree_data->NodeMax2),
        prepay_data->SpotVol,
        tree_data->PrepayVol,
    ...
    char    AoS)           /* Yield curve type (annual or semi-annual) */
{
    ...
    for (i = 0, k = 1, Days = 0.; i <= NbNodes;
         i++) /* Interpolation of zero rates at each node (time step). Note that because we need */
    {         /* to calculate probabilities up to NbNodes we need one extra forward and          */
        ...
        linterp(Days, &zero, (double)Period[k - 1], (double)Period[k], Zero[k - 1], Zero[k]);

        FwdRate[i + 1] =
            zero; /* For the time being FwdRate[i] is not the forward rate but the zero rate */
         ...
         i++) /* Calculation of forward rates from interpolated zero rates */
    {
        ...
        ZeroCoupon[i] = 1. / Discount1; /* We store the prices of zero coupon maturing at each node.
 ...
 */
char* Forward_Vol(
    double* FwdVol,      /* Output: forward rate volatility at each node in the tree */
    ...
    int     NbNodes)         /* Total number of nodes (or time steps) */
{
    double BaseVol[NBSWAPTION + 1], /* Base volatility (squared) at expiration of each of the
                                       benchmark swaptions */
        ...
        *FwdZeroYield, /* Yield of the zero going from expiration of the swaption to one of the
                          coupons */
    ...
    NbCoupon =
        IndexMat * IndexF; /* Each of the CMT swaption is going to have IndexMat*IndexF coupons */
    ...
    for (i = 0; i < NBSWAPTION; i++)
    {
                ...
                pow(1. + Zero[ZeroIndex] / Freq,
                    Freq * Period[ZeroIndex] /
                        365.); /* forward swap. Note the 3 offset due to money   */
                               /* market instruments.                            */
            ZeroIndex =
                2 * SwaptionExp[i] + 3 + j; /* Index of the zero maturing at coupon j of the */
            FwdZero /=
                pow(1. + Zero[ZeroIndex] / Freq,
                    Freq * Period[ZeroIndex] /
        ...
        FwdYield =
            IndexF * (1. - FwdZero) /
            Annuity; /* calculated above i.e. the one maturing at maturity of the forward swap. */
             ...
             j++) /* Calculation of the intermediate doubles x & y */
        {
            ...
            y += FwdYield * pow(1. + FwdZeroYield[j] / IndexF, -j - 1.) * j * FwdZeroYield[j] *
                 ExpDecay(Beta, T2 - T1);
        ...
        BaseVol[i + 1] =
            pow(SwaptionVol[i] / 100. * x / y,
                2.); /* use the relation between the base vol, swaption vol, x & y */
         ...
         i++) /* The i index is now running from 1 to NBSWAPTION */
    {
        ...
        SpotVol[i] = (T2 * BaseVol[i] - T1 * BaseVol[i - 1] * exp(-2. * Beta * (T2 - T1))) /
    ...
    {
        FwdVol[j] =
            SpotVol[i]; /* The spot volatility is constant between the benchmark swaption points */

        Time += Length[j++];

        if (Time > Period[2 * SwaptionExp[i - 1] + 3] / 365. -
                       MBS_ERROR) /* If Time is greater than the swaption expiration we switch to
                                     the next swaption */
            i++;
    } /* while */
    ...
    *Jump = MaxSpotVol *
            sqrt(
                JUMPCOEFF /
                (double)Ppy); /* The jump size depends on maximum volatility and time step, 1/ppy */
 ...
 */
void Prepay_Vol(
    double* PrepayVol, /* Output: instantaneous volatility of forward prepayment */
    ...
    double  Beta)
{
    ...
    {
        Delta[i] =
            Delta[i - 1] * (1.0 - Beta * Length[i]); /* multiplicative correction due to mean rev */
    ...
    int     NbNodes)        /* Total number of nodes (or time steps) */
{
    ...
    double *Discount, /* 1 period discount factor at each node (i.e. 1/(1+FwdRate[i]) */
    ...
    {
        Vol = FwdVol[NbPer] *
              sqrt(Length[NbPer]); /* Volatility over the period = sigma * sqrt(time step) */

        Bottom = (int)max(
            0.,
            NbPer - *NodeMax); /* If NbPer>NodeMax the tree will be cut at the top and the bottom */
        Top = (int)min(2 * NbPer, NbPer + *NodeMax);

        for (i = Bottom; i <= Top + 2; i++) /* We don't need all the nodes if the tree is cut */
        {
            FwdRate1 = FwdRate[NbPer + 1] *
                       exp(Jump * (i - (NbPer + 1))); /* Value of the short term rate at node i */
            Discount1[i] =
                1. / (1. + FwdRate1); /* 1 period discount factor in one period at node i */
        }                             /* for i */

        x1 = x2 = x3 = y1 = y2 = y3 = z1 = z2 = z3 =
            0.; /* Intermediate calculations for the drift */
        for (i = Bottom + 1; i <= Top - 1;
             i++) /* We don't include the top and bottom of the tree: it is dealt with afterwards */
        {
        ...
        {
            k = (NbPer + 1 > *NodeMax) *
                ((i == Bottom) - (i == Top)); /* If the number of nodes in the next period is
            ...
            c += StatePr[i] * Discount[i] *
                 (.5 * Discount1[i + 2 + k] *
                  ...
                  Discount1[i + 1 + k] * (k * k - 1. + pow(Vol / Jump, 2.) +
        ...
        for (i = Bottom; i <= Top; i++) /* Calculate state prices in one period */
        {
            ...
            pu = .5 * (k * (k - 1.) + pow(Vol / Jump, 2.) +
                       (1. - 2. * k) * (Drift[NbPer] - Beta * Length[NbPer] * (i - NbPer)) +
            ...
            StatePr1[i + 2 + k] +=
                pu * StatePr[i] * Discount[i]; /* There are 3 branches originating from node i and
        ...
        for (i = Bottom; i <= Top + 4; i++) /* Permute the values so that they can be used for the
        ...
        { /* bottom of the tree are not too big or too small.                            */
            pu = .5 * (pow(Vol / Jump, 2.) + Drift[NbPer] + Beta * Length[NbPer] * NbPer +
                       pow(Drift[NbPer] + Beta * Length[NbPer] * NbPer, 2.));
            pd = .5 * (pow(Vol / Jump, 2.) - Drift[NbPer] - Beta * Length[NbPer] * NbPer +
                       pow(Drift[NbPer] + Beta * Length[NbPer] * NbPer, 2.));
            p0 = 1. - pu - pd;

            if ((pu > .999) || (p0 < .001) ||
                (pd <
                 .001)) /* Bottom of the tree (i=0): pu might be close to 1 and pd close to 0 */
            {
                CutFlag  = TRUE;
            ...
            if ((pu < .001) || (p0 < .001) ||
                (pd >
                 .999)) /* Top of the tree (i=2*NbPer): pd might be close to 1 and pu close to 0 */
            {
                CutFlag  = TRUE;
                *NodeMax = NbPer + 1;
            } /* if */
        ...
        NbPer <=
        NbNodes -
            1); /* We need Drift[NbNodes-1] => We need FwdRate[NbNodes], FwdVol[NbNodes-1] and */
                /* Discount[NbNodes+1].                                                        */
    Drift[NbPer] = 0.;
    ...
    int     NbNodes)       /* Total number of nodes (or time steps) */
{
    ...
    *Jump2 =
        SpotVol / 100. * sqrt(JUMPCOEFF / JUMPCOEFF2); /* The jump in the second dimension is not
    ...
    *NodeMax2 = 10000; /* 10000 = Big! */
    ...
    while ((!CutFlag) && (NbPer < NbNodes)) /* bottom of the tree are not too big or too small. */
    {
        ...
        if ((pu > .999) || (p0 < .001) ||
            (pd < .001)) /* Bottom of the tree (i=0): pu might be close to 1 and pd close to 0 */
        {
            CutFlag   = TRUE;
        ...
        if ((pu < .001) || (p0 < .001) ||
            (pd > .999)) /* Top of the tree (i=2*NbPer): pd might be close to 1 and pu close to 0 */
        {
            CutFlag   = TRUE;
            *NodeMax2 = NbPer + 1;
        } /* if */
...
void Print_Term(TERM_DATA* term_data, TREE_DATA* tree_data)
{
    ...
    {
        date = Nxtmth(
            term_data->ValueDate, /* Maturity date of the current zero */
            (long)((i - 3) * 6),
            (long)1);
        discount =
            1. / pow(1. + term_data->Zero[i] / Freq, /* Discount factor up to the current date */
...
void Lattice_new(
    double*     Discount,  /* Output: one period discount factor at each node at period NbPer */
    ...
    TREE_DATA* tree_data) /* Structure of tree data */
{
    ...
    Bottom = (int)max(
        0.,
        NbPer -
            tree_data->NodeMax); /* If NbPer>NodeMax the tree is cut at the top and the bottom */
    ...
    for (i = Bottom; i <= Top; i++)
    {
        ...
        if (term_data->TStree != term_data->TSdiscount) /*include  convexity adjustments */
            FwdYield = FwdYield *
                       Multiplier; /* if discounting is different from tree-building, we redo it */

        //#ifdef USECONSISTENTOAS
        //		DiscountS[i] = 1.0/(1.0 + FwdYield)/(1.0 + term_data->OAS * Length /
        //10000.0); #else
        FwdYield +=
            term_data->OAS * Length /
            10000.; /* We add the OAS to the current short term rate to calculate DiscountS */
...
void Zero_Calc_new(
    ...
    //			int             *NbZero,                                /* Current number of zeros being
    //priced */ 			int             TotNbZero,                              /* Total number of zeros
    //needed to calculate the par yield index */ 			int             Reset, /* Reset the zero price to 1
    //if TRUE */
    int NbPer, /* Current time period */
    //			int             TotPer,                                 /* Total number of period in the
    //rate tree including the N additional periods */
    int         useOAS,
    MBS_BKTree* tree)
{
    double** tmp;
...
void Zero_Calc(
    ...
    TREE_DATA* tree_data) /* Structure of tree data */
{
    double* TempPointer; /* Temporary pointer to doubles used in the permutation of the zeros */
    int     i;
    ...
    {
        TempPointer = Zero[*NbZero - 1]; /* Store the address of the zero with the longest maturity
                                            ("rightmost" zero) */

        for (i = *NbZero - 1; i > 0; i--) /* Shift all the zeros to the "right" by one. */
            Zero[i] = Zero[i - 1];

        if (*NbZero <= TotNbZero) /* If we don't have all the zeros we need to calculate the par
                                     yield (i.e.   */
        { /* TotNbZero + 1), then we need to keep an extra zero: the one which address */
            Zero[0] = Zero[*NbZero];     /* is stored in TempPointer. Then we give a new address to
                                            Zero[0] and       */
        ...
        Zero_Price(
            Zero[0], /* Zero[0] is the zero coupon maturing now. We reset it to 1. */
            1,
            Discount,
            pu,
            pd,
 ...
 */
void Dev(
    double*    Price,     /* Array of prices that has to be discounted */
    double*    Discount,  /* One period discount factor at each node at period NbPer */
    double*    pu,        /* Array of probabilities in the up state in the interest rate tree */
    double*    pd,        /* Array of probabilities in the down state in the interest rate tree */
    double*    p0,        /* Array of probabilities in the flat state in the interest rate tree */
    ...
    TREE_DATA* tree_data) /* Structure of tree data */
{
    ...
    k = -(NbPer + 1 > tree_data->NodeMax); /* in the previous loop and we need the old value (Temp)
                                              to calculate Price[Top] */
 ...
 */
void Par_Yield(
    double*  ParYield,  /* Output: par yield at every node at the current period */
    double** Zero,      /* Set of zeros maturing on each coupon date */
    int      TotNbZero, /* Total number of zeros needed to calculate the par yield index */
    ...
    int      NbPer)          /* Current time period */
{
    ...
    Bottom = (int)max(
        0., NbPer - NodeMax); /* If NbPer>NodeMax the tree is cut at the top and the bottom */
    Top = (int)min(2 * NbPer, NbPer + NodeMax);

    for (i = Bottom; i <= Top; i++)
    {
        for (j = F / IndexF; j <= TotNbZero;
             j += F / IndexF)      /* Because the SFS and the index might have different maturity we
                                      don't use all  */
            Annuity += Zero[j][i]; /* the zeros only zeros at F/IndexF intervals. Note also that the
                                      index stops at */
...
char* copyTermStruct(MBS_BKTree* tree, MBSPT_DealStruct* deal_struct, TREE_DATA* tree_data)
{
    int                   i, j, sz;
    ...
    tree_data->NbNodes =
        tree->NbTimeSlices - 2;  // without 0 <-> 0.0 AND tree_data->Length is [0, NbNodes]
    ...
    tree_data->Accrued[0] =
        cashflow_struct->accrualFactor / (cashflow_struct->accrualFactor + tree->tree_times[1]);
    j = 1;
    ...
    for (i = j; i <= tree_data->NbNodes; ++i)
        tree_data->Accrued[i] = 1.0;  // these are needed because it affects how zeros are computed

/* ===================================================================================
   ...
   PURPOSE:       Computes the correlation between two fwd swap rates
				  ...
				  Inputs are spot swaps correlation matrix and tenor dates associated
   =================================================================================== */
...
Err Compute_CoInitalSwaps_Correl2(long lToday,
								 char *CorrelCubeId,
								 Err	(*get_correl)(
								 ...
								 double ***dCorrelMatrix)
{
	...
	for(i=0;i<iNDates;++i)
	{
		...
		{
			start = add_unit (lToday, i * 12 / srtFreq, SRT_MONTH, NO_BUSDAY_CONVENTION);
			end = add_unit ((long)(start), (long)(fabs(i-j) * 12 / srtFreq), SRT_MONTH, NO_BUSDAY_CONVENTION);
...
Err Compute_CoInitalSwaps_Correl(long lToday,
								 char *CorrelCubeId,
								 Err	(*get_correl)(
									char	*correl_cube_name,	
									double	start_date, 
...
//			err = swp_f_vol( CorrelCubeId, start, end, strike, &rho, &pow);
			err = get_correl(CorrelCubeId, start, end, strike, &rho);
			(*dCorrelMatrix)[i][j] = DMIN(0.99999, DMAX(-0.99999, rho));
			(*dCorrelMatrix)[j][i] = (*dCorrelMatrix)[i][j];
		}

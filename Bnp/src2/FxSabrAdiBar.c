    ...
    double* res)
{
    ...
    double *exp_h = NULL, *expect_z = NULL, *expect_z2 = NULL, *drift_z = NULL, *x = NULL,
           *x_bar = NULL, *z = NULL, ***values = NULL, ***values_p1 = NULL, ***values_temp = NULL,
           **mux = NULL, **muz = NULL, **muzinit = NULL, **sigBeta = NULL, **varx = NULL,
           **varz = NULL, **varxinit = NULL, **varzinit = NULL, **r = NULL;
    ...
    CNPDE_TEMP_2D_ADI pdestr1, *pde1 = NULL, pdestr2, *pde2 = NULL, pdestr3, *pde3 = NULL, pdestr4,
                               *pde4 = NULL;
    ...
    for (i = 0; i < nstp - 1; i++)
    {
        ...
        drift_z[i] =
            (const1 * sig * exp(alpha2 * (time[i] + dt)) + const2 / sig * drift[i]) * 2.0 * dt;
    ...
    */
    ...
    if (!values || !values_p1 || !mux || !muz || !muzinit || !sigBeta || !varx || !varxinit ||
        !varz || !varzinit || !r)
    {
            ...
            if (is_up[step])
            {
                ...
                if (bar_lvl[step] < bar_lvl[step + 1])
                {
                    ...
                    if (ux == 0 || bar_lvl[step] >= x[ux - 1])
                    {
                        ...
                        Interpolate_Payoff(
                            values, x, ux, nstepx, lz, uz, nprod, bar_col, bar_lvl[step], 0);
                    ...
                    else
                    {
                        ...
                        if (ux == index_x || lx == index_x)
                        {
                            ...
                            Interpolate_Payoff(
                                values, x, adj_index, nstepx, lz, uz, nprod, bar_col, spot_fx, 1);
            ...
            else
            {
                ...
                if (bar_lvl[step] < bar_lvl[step + 1])
                {
                    ...
                    if (lx == 0 || bar_lvl[step] >= x[lx - 1])
                    {
                        /* Interpolate the payoff for the new point */
                        Interpolate_Payoff(
                            values, x, lx, nstepx, lz, uz, nprod, bar_col, bar_lvl[step], 0);
    ...
    if (fabs(x[index_x] - spot_fx) > 1.0E-08)
    {
        ...
        Interpolate_Payoff(
            values, x, index_x, nstepx, index_z, index_z, nprod, bar_col, spot_fx, 1);
    }
...
static void Interpolate_Payoff(
    double*** values,
    double*   x,
    int       ux,
    int       nstepx,
    ...
    double* res)
{
    ...
    double *exp_h = NULL, *expect_z = NULL, *expect_z2 = NULL, *drift_z = NULL, *x = NULL,
           ...
           **muz = NULL, **muzinit = NULL, **sigBeta = NULL, **varx = NULL, **varz = NULL,
    ...
    if (is_up)
    {
        ...
        if (strike2 > 1.0E-08)
        {
            ...
            if (is_cvx)
            {
                disc_linleft_logright_strike(
                    x, nstepx, spot_fx, flx, bar_ex, std1, NSTD_FX, 1.0 / strike2, &index_x);
            }
            else
            {
                disc_linleft_logright_strike(
                    x, nstepx, spot_fx, flx, bar_ex, std1, NSTD_FX, strike2, &index_x);
            }
        }
        else
        {
    ...
    else
    {
        ...
        if (strike2 > 1.0E-08)
        {
            ...
            {
                disc_linleft_logright_strike(
                    x, nstepx, spot_fx, bar_ex, fux, std1, NSTD_FX, 1.0 / strike2, &index_x);
            }
            else
            {
                disc_linleft_logright_strike(
                    x, nstepx, spot_fx, bar_ex, fux, std1, NSTD_FX, strike2, &index_x);
            }
        }
        else
        {

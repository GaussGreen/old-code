...
CNPDE_TEMP_2D_ADI	pdestr1, *pde1 = NULL,
					pdestr2, *pde2 = NULL,
					pdestr3, *pde3 = NULL,
					pdestr4, *pde4 = NULL;

int					ms1, me1, ms2, me2, ms3, me3, ms4, me4, n1, n2, n3, n4, adj_index, change_lim;
...
double				logx, logmax, logmin, save1, save2, save3;
	...
	for (i=0; i<nstp-1; i++)
	{
		...
		exp_z += const1 * sig * exp(alpha2 * time[i]) / (drift[i] + alpha2) * (exp((drift[i] + alpha2) * dt) - 1.0)
		...
		drift_z[i] =  (const1 * sig * exp(alpha2 * (time[i] + dt)) + const2 / sig * drift[i]) * 2.0 * dt;

		exp_z += const1 * sig * exp(alpha2 * (time[i] + dt)) / (drift[i] + alpha2) * (exp((drift[i] + alpha2) * dt) - 1.0)
				- const2 / sig * (exp(-drift[i] * dt) - 1.0);
	...
	*/
	...
	if (!values || !values_p1 || !mux || !muz || !muzinit || !sigBeta
		|| !varx || !varxinit || !varz || !varzinit  || !r
		)
	{
	...
	for (step=nstp-2; step>=0; step--)
	{
		...
		{			
			...
			const_muz2 = -sigBeta[i][0] / exp_h[step] * drift[step] * dt - drift_z[step];
		...
		if (is_bar[step+1])
		{
			...
			if (n1)
			{
				num_f_pde_one_step_backward_2f_adi(	pde1,
													nstepx,
													x,
													nstepz,
													z,
			...
			num_f_pde_one_step_backward_2f_adi_bar(	pde2,
													nstepx,
													x,
													nstepz,
													z,
			...
			if (n3)
			{
				num_f_pde_one_step_backward_2f_adi(	pde3,
													nstepx,
													x,
													nstepz,
													z,
		...
		else
		{
			...
			num_f_pde_one_step_backward_2f_adi(	pde4,
												nstepx,
												x,
												nstepz,
												z,
...
FREE_RETURN:
	...
	if (pde1) num_f_pde_free_2d_adi (pde1, nstepx, nstepz, n1);
	if (pde2) num_f_pde_free_2d_adi (pde2, nstepx, nstepz, n2);
	if (pde3) num_f_pde_free_2d_adi (pde3, nstepx, nstepz, n3);
	if (pde4) num_f_pde_free_2d_adi (pde4, nstepx, nstepz, n4);
	...
	if (muzinit) free_dmatrix (muzinit, 0, nstepx-1, 0, nstepz-1);
	if (sigBeta) free_dmatrix (sigBeta, 0, nstepx-1, 0, nstepz-1);
	if (varx) free_dmatrix (varx, 0, nstepx-1, 0, nstepz-1);
	if (varxinit) free_dmatrix (varxinit, 0, nstepx-1, 0, nstepz-1);
	if (varz) free_dmatrix (varz, 0, nstepx-1, 0, nstepz-1);
	if (varzinit) free_dmatrix (varzinit, 0, nstepx-1, 0, nstepz-1);
	if (r) free_dmatrix (r, 0, nstepx-1, 0, nstepz-1);
			
	return err;
}

static void Interpolate_Payoff(	double	***values,
								double	*x,
								int		ux,
								int		nstepx,
								int		lz,
...
double				logx, logmax, logmin, save1, save2, save3, coef;
	...
	mat_date = add_unit ((long) (date[nstp-1] + 1.0E-08), 2, SRT_BDAY, MODIFIED_SUCCEEDING);
	
	df_ratio = swp_f_df (today, mat_date, for_yc) / swp_f_df (today, mat_date, dom_yc);
	if (is_cvx)
	{
	...
	if (is_up)
	{
		...
		if (strike2 > 1.0E-08)
		{
			...
			if (is_cvx)
			{
				disc_linleft_logright_strike(x, nstepx, spot_fx, flx, bar_ex, std1, NSTD_FX, 1.0 / strike2, &index_x);
			}
			else
			{
				disc_linleft_logright_strike(x, nstepx, spot_fx, flx, bar_ex, std1, NSTD_FX, strike2, &index_x);
			}
		}
		else
		{
			disc_linleft_logright(x, nstepx, spot_fx, flx, bar_ex, std1, NSTD_FX, &index_x);
		}
	...
	else
	{
		...
		if (strike2 > 1.0E-08)
		{
			...
			if (is_cvx)
			{
				disc_linleft_logright_strike(x, nstepx, spot_fx, bar_ex, fux, std1, NSTD_FX, 1.0 / strike2, &index_x);
			}
			else
			{
				disc_linleft_logright_strike(x, nstepx, spot_fx, bar_ex, fux, std1, NSTD_FX, strike2, &index_x);
			}
		}
		else
		{
			disc_linleft_logright(x, nstepx, spot_fx, bar_ex, fux, std1, NSTD_FX, &index_x);
		}
	...
	for (step=nstp-2; step>=0; step--)
	{
		...
		if (do_bar)
		{
			num_f_pde_one_step_backward_2f_adi_bar(	pde,
													nstepx,
													x,
													nstepz,
													z,
				...
				if (bar_lvl[step] < bar_lvl[step+1])
				{
					...
					else
					{
						...
						if (ux == index_x)
						{
							...
							coef *= (spot_fx * (spot_fx - (x[ux] + x[ux+1])) + x[ux] * x[ux+1]);
				...
				if (bar_lvl[step] < bar_lvl[step+1])
				{
					...
					{	
						...
						if (lx == index_x)
						{
							...
							coef *= (spot_fx * (spot_fx - (x[lx] + x[lx-1])) + x[lx] * x[lx-1]);

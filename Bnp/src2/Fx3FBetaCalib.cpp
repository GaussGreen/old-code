...
Err		err = NULL;
	...
	{		
		...
		F = logSpot + 
			(swp_f_zr (today, today + T * 365.0, dom_yc) - swp_f_zr (today, today + T * 365.0, for_yc)) * T;
...
Err			err = NULL;
	...
	for (i=nb_merge_dates-1; i>=0; i--)
	{
		...
		//beta2[i] = beta[Get_Index(merge_dates[i], sigma_date_fx, sigma_n_fx)];
		beta2[i] = beta;
	}
...
Err		err = NULL;
	...
	err = Fx3DtsCalibration(exercise_opt,
							maturity_opt,
							vol_opt,
							nbrOpt,
							maturity,
	...
	for (i=0; i<nbrOpt; i++)
	{
		...
		mat = maturity_opt[i];
		T = exercise_opt[i];
		Fwd = spot_fx * swp_f_df (today, today + T * 365.0, for_yc) / swp_f_df (today, today + T * 365.0, dom_yc);
		bet = beta[i] - 1.0;
		vol_tgt = vol_opt[i];
...
Err		err = NULL;
	...
	if (!vol_change || !dom_vol || !for_vol || !fx_vol || !beta_tab || !is_bar || !bar_lvl || !bar_cl)
	{
	...
	fx_vol[nstp-1] = sig_curve_fx[Get_Index(time[nstp-1], maturity_fx, nbrMat_fx)] * exp (alpha * sqrt (time[nstp-1]) - 0.5 * alpha * alpha * time[nstp-1]);;
	...
	for (i=nstp-2; i>=0; i--)
	{
		...
		fx_vol[i] = sig_curve_fx[Get_Index(time[i], maturity_fx, nbrMat_fx)] * exp (alpha * sqrt (time[i]) - 0.5 * alpha * alpha * time[i]);
...
FREE_RETURN:
	...
	if (corr_dom_for_ts) free (corr_dom_for_ts);
	if (corr_dom_fx_ts) free (corr_dom_fx_ts);
	if (corr_for_fx_ts) free (corr_for_fx_ts);
...
Err		err = NULL;
	...
	{
		return Fx3DtsCalibration(exercise_opt,
								maturity_opt,
								vol_opt,
								nbrOpt,
								maturity,
	...
	for (i=nbrShort; i<nbrOpt; i++)
	{
		...
		Fwd = spot_fx * swp_f_df(today, exercise_date, for_yc) / swp_f_df(today, exercise_date, dom_yc);		
		...
		while (j < nbNewton && (error > MAX_ERROR_VOL))
		{
			...
			err = srt_f_optimpvol(price1[0],Fwd, Fwd, exercise_opt[i], df, 0, 0, &vol_imp);
			...
			else
			{
				...
				{					
					if (i > 0)
					{
						delta = (*fx_vol_curve)[i] * (*fx_vol_curve)[i] * exp(2*(beta2[i]-1) * log(Fwd))
								- (vol_imp * vol_imp - vol_opt[i] * vol_opt[i]) * exercise_opt[i] / (exercise_opt[i] - exercise_opt[i-1]);
					}
					else
					{
						delta = (*fx_vol_curve)[i] * (*fx_vol_curve)[i] * exp(2*(beta2[i]-1) * log(Fwd))
					...
					if (delta > 0)
					{
						shift = -(*fx_vol_curve)[i] + sqrt(delta) * exp((1.0 - beta2[i]) * log(Fwd));
						/*
						for (k=i; k<nbrOpt; k++)
						{
...
double		*correl_date	= NULL,
			...
			*correl_dom_fx	= NULL,
			*correl_for_fx	= NULL,
			*correl_dom_for_ts	= NULL,
			*correl_dom_fx_ts	= NULL,
			*correl_for_fx_ts	= NULL;
double		lda_dom, lda_for;
double		*sig_dom		= NULL,
			*sig_for		= NULL,
			*sig_fx			= NULL,
...
Err		err = NULL;
	...
	if (fabs(beta - 1.0) < 1.0E-04)
	{
		return Fx3DtsCalibration_corr(exercise_opt,
								maturity_opt,
								vol_opt,
								nbrOpt,
								maturity,
...
Err		err = NULL;
	...
	for_vol[nstp-1] = sig_curve_for[Get_Index(time[nstp-1], maturity, nbrMat)];
	fx_vol[nstp-1] = sig_curve_fx[Get_Index(time[nstp-1], maturity_fx, nbrMat_fx)] * exp (alpha * sqrt (time[nstp-1]) - 0.5 * alpha * alpha * time[nstp-1]);;
	corr_dom_for[nstp-1] = corr_dom_for_ts[Get_Index(time[nstp-1], corr_mat, nb_corr_mat)];
	corr_dom_fx[nstp-1] = corr_dom_fx_ts[Get_Index(time[nstp-1], corr_mat, nb_corr_mat)];
	corr_for_fx[nstp-1] = corr_for_fx_ts[Get_Index(time[nstp-1], corr_mat, nb_corr_mat)];
	...
	for (i=nstp-2; i>=0; i--)
	{
		...
		corr_dom_for[i] = corr_dom_for_ts[Get_Index(time[i], corr_mat, nb_corr_mat)];

    ...
    double  max_time)
{
    ...
    time2 = (double*)calloc(nb_time, sizeof(double));
    if (!time2)
    {
...

{
    ...
    fx_time = (double*)calloc(nb_fx_time, sizeof(double));
    ...
    fx_vol_curve = (double*)calloc(nb_fx_time2, sizeof(double));
    fx_fwd       = (double*)calloc(nb_fx_time2, sizeof(double));
    ...
    long    num_stp)
{
    ...
    int*    vol_change = NULL;
    double *dom_vol = NULL, *for_vol = NULL, *fx_vol = NULL, *corr_dom_for_ts = NULL,
           *corr_dom_fx_ts = NULL, *corr_for_fx_ts = NULL;

    double *dom_ifr = NULL, *dom_fwd = NULL, *dom_var = NULL, *for_ifr = NULL, *for_fwd = NULL,
           *for_var = NULL, *fx_fwd = NULL, *fx_var = NULL;
    ...
    if (!vol_change || !dom_vol || !for_vol || !fx_vol || !beta_tab || !is_bar || !bar_lvl ||
        !bar_cl)
    {
    ...
    if (!dom_ifr || !dom_fwd || !dom_var || !for_ifr || !for_fwd || !for_var || !corr_dom_for_ts ||
        !corr_dom_fx_ts || !corr_for_fx_ts || !fx_fwd || !fx_var || !sig_fx_approx ||
        !fx_fwd_approx)
    {
        err = "Memory allocation error (3) in Fx3DBetatsTreeFxOptions";
        goto FREE_RETURN;
                ...
                if (error > MAX_ERROR_VOL)
                {
                    if (i > 0)
                    {
                        delta = (*fx_vol_curve)[i] * (*fx_vol_curve)[i] *
                                ...
                                (vol_imp * vol_imp - vol_opt[i] * vol_opt[i]) * exercise_opt[i] /
                    ...
                    {
                        shift =
                            -(*fx_vol_curve)[i] + sqrt(delta) * exp((1.0 - beta2[i]) * log(Fwd));
                        /*
                        for (k=i; k<nbrOpt; k++)
                        {
    ...
    double* vol)
{
    ...
    double *correl_date = NULL, *correl_dom_for = NULL, *correl_dom_fx = NULL,
           *correl_for_fx = NULL, *correl_dom_for_ts = NULL, *correl_dom_fx_ts = NULL,
    ...
    long    num_stp)
{
    ...
    if (!vol_change || !dom_vol || !for_vol || !fx_vol || !beta_tab || !is_bar || !bar_lvl ||
        !bar_cl || !corr_dom_for || !corr_dom_fx || !corr_for_fx)
    {

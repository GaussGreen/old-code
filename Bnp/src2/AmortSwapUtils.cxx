          ...
          for(i=1;i<iNFixPayDates-1;++i)
          {
                  ...
                  for(k=(i-1)*FixFloatMult+1;k<i*FixFloatMult+1;++k)
                  {
                          (*dCpns)[i] += - (dFloatCoverages[k-1]
                                                          * dFloatSpreads[k-1])
                                                          * (swp_f_df(lToday ,
     lFloatPayDates[k]        , cYCname) /swp_f_df(lToday        ,
...
Err ComputeBondIRR(char *cYCname, char *cVCname, long lToday, char *cFreq,
  ...
  R = 0.05;
  //	PV = PVBOND_IRR(cYCname        , lToday        , nPayDates        ,
  //lCpnDates        , dCpns        , R);
  ...
  while ((fabs(PV) > 1e-12) && (Compt < 15)) {
    R2 = R + 0.0001;
    //		PV2 = PVBOND_IRR(cYCname        , lToday        , nPayDates        ,
    //lCpnDates        , dCpns
    //      , R2);
    err = PVBOND_IRR(cYCname, cVCname, lToday, cFreq, cBasis, cRefRname,
...
Err ComputeSwapRateFromIRR(char *cYCname, char *cVCname, long lToday,
  ...
  */
  ...
  //	PV_Float = PVBOND_IRR(cYCname        , lToday        , iNFixPayDates ,
  //lFixPayDates
  //      , dCpns_Float        , IRR);
  err = PVBOND_IRR(cYCname, cVCname, lToday, cFreq, cBasis, cRefRname,
  ...
  //	PV_Fix = PVBOND_IRR(cYCname        , lToday        , iNFixPayDates ,
  //lFixPayDates
  //      , dCpns_Fix        , IRR);
  err =
  ...
  // / PVBOND_IRR(cYCname        , lToday        , iNFixPayDates        ,
  // lFixPayDates        , dCpns_Fix
  //      , IRR);

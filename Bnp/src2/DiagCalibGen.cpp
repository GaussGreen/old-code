...
void Free_CalibParams(CALIBGEN_PARAMS CalibParams)
{
    if (CalibParams)
    {
        if (CalibParams->res_iter)
...
double solve_for_next_coef_gen(
    double** res_iter,
    int      nb_iter,
    double   premium_tgt,
    double   precision,
    int      method) /* 0: linear 1: quadratic */
{
    ...
    else if (nb_iter > 2)
    {
        ...
        {
            /* we are linear */
            coef = solve_for_next_coef_gen(res_iter, nb_iter, premium_tgt, precision, 0);
            return coef;
        }
...
void solve_for_next_sensi(
    double** res_iter, int nb_computed, double value, double delta, double* sensi)
{
    int    i, index;
    ...
    /* Set the Sensitivity */
    if (nb_computed > 1 && (res_iter[nb_computed - 1][0] - res_iter[0][0]) > delta)
    {
        for (index = 0; (index < nb_computed) && (res_iter[index][0] < value - 1.0E-10); index++)
            ;
        ...
        if (index > 0 && fabs(value - res_iter[index - 1][0]) < fabs(value - res_iter[index][0]))
        {
            index--;
        }
        ...
        if (i < nb_computed)
        {
            sens_up = (res_iter[i][1] - res_iter[index][1]) / (res_iter[i][0] - res_iter[index][0]);
            has_up  = 1.0;
        }
        else
        {
        ...
        {
            sens_down =
                (res_iter[i][1] - res_iter[index][1]) / (res_iter[i][0] - res_iter[index][0]);
            has_down = 1.0;
        }
        else
        {
    ...
    int*    success)
{
    ...
    if (CalibConsts->do_calib)
    {
        ...
        *success = 1;
        ...
        /* First Pricing */
        err = AllFunctions->GetFirstGuess(Model, GlobalConst, index_param, target, &param1);
        ...
        err =
            AllFunctions->UpdateConstsAfterParam(Inst, InstConst, GlobalConst, Model, CalibConsts);
        ...
        if (fabs(target - price1) > CalibConsts->Precision)
        {
            ...
            {
                err = AllFunctions->GetSecondGuess(
                    Model, GlobalConst, index_param, param1, price1, target, &param2);
            ...
            if (fabs(param1 - param2) < 1.0E-10)
            {
                ...
                smessage("Calibration: failed to calibrate instrument nb %d", index_param);
                *success = 0;
            }
        }
        else
        ...
        while (fabs(target - price1) > CalibConsts->Precision && iter < CalibConsts->NbIterMax &&
               *success)
        {
            iter++;
            ...
            err = AllFunctions->UpdateConstsAfterParam(
                Inst, InstConst, GlobalConst, Model, CalibConsts);
            ...
            {
                err = AllFunctions->PriceInst(Inst, InstConst, GlobalConst, Model, &price2);
                ...
                param2 = solve_for_next_coef_gen(
                    res_iter, nb_computed, target, CalibConsts->Precision, 2);
            }
            ...
            if ((res_iter[0][0] < limit_down + 1.0E-12 && param2 < limit_down + 1.0E-12) ||
                (res_iter[iter - 1][0] > limit_up - 1.0E-12 && param2 > limit_up - 1.0E-12))
            {
        ...
        if (fabs(target - price1) > CalibConsts->Precision && iter >= CalibConsts->NbIterMax)
        {
            smessage(
                "Calibration: failed to calibrate instrument nb %d , Max iteration reached",
                index_param);
            *success = 0;
        }
    ...
    CALIBFUNCTIONS AllFunctions)
{
    ...
    {
        err = AllFunctions->GetLimitAndLastParam(
            Model, CalibConsts, GlobalConst, i, &last_param, &limit_down, &limit_up);
        ...
        {
            solve_for_next_sensi(
                CalibConsts->res_iter,
                CalibConsts->nb_computed,
                new_param,
                CalibConsts->delta,
                &(CalibConsts->sensi));
        }

...
)
{
    ...
    part1 = bivar(
        du * (-b + mu1 * mat1) / sig_sqrt1, cp * (-k + mu2 * mat2) / sig_sqrt2, du * cp * corr);

    part2 = bivar(
        du * (b + mu1 * mat1) / sig_sqrt1,
...
{ /* BEGIN srt_f_optprtext(11) */
    ...
    if ((spot - barrier) * du < 0)
        ...
        if (mat1 >= mat2)
    {
        premium = srt_f_optexting(
            fwd2, spot, strike, barrier, vol2, mat2, disc, call_put, down_up, PREMIUM);
    }
    ...
    else
    {
        ...
        premium = fwd2 *
                  proba_joint_spot_part_min_max(mu_1p, mu_2p, vol1, vol2, mat1, mat2, k, b, cp, du);

        premium -= strike * proba_joint_spot_part_min_max(
                                mu_1m, mu_2m, vol1, vol2, mat1, mat2, k, b, cp, du);
        premium *= cp * disc;
    }

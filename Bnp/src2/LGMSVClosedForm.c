...
static void prod_comp(double re1, double im1, double re2, double im2, double* re3, double* im3)
{
    *re3 = re1 * re2 - im1 * im2;
    *im3 = re1 * im2 + im1 * re2;
}

static void div_comp(double re1, double im1, double re2, double im2, double* re3, double* im3)
{
    static double div;
    ...
    for (iNumPhi = 1; iNumPhi <= iNbPhi; iNumPhi++)
    {
        for (iNumft = 1; iNumft < iNbft; iNumft = iNumft + 2)
        {
            fprintf(
                ...
                " %g	",
                sqrt(
                    Density[1][iNumPhi][iNumft] * Density[1][iNumPhi][iNumft] +
                    Density[1][iNumPhi][iNumft + 1] * Density[1][iNumPhi][iNumft + 1]));
        }
...
void free_LGMSVSolFunc3tensor(
    LGMSVSolFunc1*** t, long nrl, long nrh, long ncl, long nch, long ndl, long ndh)
    ...
    for (iNumOrder = 1; iNumOrder <= iNbCumulant; iNumOrder++)
    {
        ...
        for (i = 0; i <= iNumOrder; i++)
        {
            MomentValue[iNumOrder - i][i][0] =
                dCoef * LGMSVFuncValue1(&MomentFunc[iNumOrder - i][i][0], dDt, lambdaArray, 0);
            dCoef *= dInvDerBeta;
        }
    }
                           ...
                           double**  Densityspeq)
{
    ...
    for (iNumOrder = 1; iNumOrder <= iNbCumulant; iNumOrder++)
    {
        ...
        for (j = 0; j <= iNumOrder; j++)
        {
            for (iNumPhiFreq = 1; iNumPhiFreq <= iNbPhi; iNumPhiFreq++)
            {
                dPhifreq = (iNumPhiFreq - 1 - iNbPhi * (int)((iNumPhiFreq - 1) / (iNbPhi / 2.0))) *
                           dPhiFreqStep;
                for (iNumftFreq = 1; iNumftFreq <= (iNbft / 2.0); iNumftFreq++)
                {
                    DensityTF[1][iNumPhiFreq][2 * (iNumftFreq - 1) + 1 + iIsComplex] +=
                        d2pik_kFact * dSign * Combination[j] *
                        pow(dftFreqStep * (iNumftFreq - 1), j) * pow(dPhifreq, iNumOrder - j) *
    ...
    for (iNumOrder = 1; iNumOrder <= iNbCumulant; iNumOrder++)
    {
        ...
        for (j = 0; j <= iNumOrder; j++)
        {
            if (iIsComplex == LGMSV_TRUE)
                XImagPart[iNumOrder - j][j] =
                    d2pik_kFact * dSign * Combination[j] * MomentValue[iNumOrder - j][j][0];
            else
                XRealPart[iNumOrder - j][j] =
                    d2pik_kFact * dSign * Combination[j] * MomentValue[iNumOrder - j][j][0];
        }
    }
        ...
        for (j = 0; j <= iNumOrder; j++)
        {
            for (iNumPhiFreq = 1; iNumPhiFreq <= iNbPhi; iNumPhiFreq++)
            {
                ...
                for (iNumftFreq = 1; iNumftFreq <= (iNbft / 2.0); iNumftFreq++)
                {
                    ...
                    /* Imaginary Part */
                    DensityTF[1][iNumPhiFreq][2 * (iNumftFreq - 1) + 2] = fmod(
                     ...
                     double** PayOff)
{
    ...
    for (iNumPhi = 1; iNumPhi <= iNbPhi; iNumPhi++)
    {
        ...
        dPhiTex =
            dPhitMean +
            (iIndexPhi - 1 - ((iIndexPhi - 1) > (iNbPhi - iIndexPhiMean)) * iNbPhi) * dPhiStep;
...
void LGMSVClosedForm(                 /* Inputs */
                     int iNbCumulant, /* Number of cumulant for the approximation of the (f,Phi)
                                         density */
                     ...
                     double* Price)
{
    ...
    /* MomentFunc(i,j,k) = E(PhiBar^i*Y^j*V^k) where PhiBar = Phi - E(Phi) a nd
     * Y=exp(lx*(Tstar-t))*(f(t,Tstar)-f(0,Tstar)) */
    MomentFunc = LGMSVSolFunc3tensor(0, iNbCumulant, 0, iNbCumulant, 0, iNbCumulant);
    ...
    if (!MomentFunc || !lambdaArray || !MomentValue || !IsUsefullMoment || !DFF_div_BTTstar ||
        !Combination || !Density || !Densityspeq || !PayOff || !CumulantRealPart ||
        !CumulantImagPart || !XRealPart || !XImagPart || !TempRealPart || !TempImagPart)
    {
    ...
    */
    ...
    dPhiMin =
        max(dPhitMean - iNbPhi * iNbSigmaPhiGridLeft /
                            (iNbSigmaPhiGridLeft + iNbSigmaPhiGridRight) * dPhiStep,
            0.0);
    ...
    double* pSwaptionPrice)
{
    ...
    if (iNbSigmaXGridRight == 0)
        ...
        else
        {
            iNbSigmaXGridRight = 7.5 + 25 * max(dRho, 0) * dAlpha *
                                           sqrt(
                                               (1 - exp(-2 * dLambdaEps * cpn_time[ncpn - 1])) /
    ...
    LGMSVClosedForm(             /* Inputs */
                    ...
                    (lExDate - today) *
                        YEARS_IN_DAY, /* Exercice of the swaption in years from today */
...
void LGMSVCalculateReClosedForm(
    double dPhifreq,
    double dftFreq,
    ...
    double**  DensitySpeq)
{
    ...
    for (iNumPhiFreq = 1; iNumPhiFreq <= iNbPhi; iNumPhiFreq++)
    {
        dPhifreq = PI2 * (iNumPhiFreq - 1 - iNbPhi * (int)((iNumPhiFreq - 1) / (iNbPhi / 2.0))) *
                   dPhiFreqStep;

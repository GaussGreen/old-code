...
void LGMSVCalculateDensityCumulant(/* Input */
  ...
  /*			DensityTF[1][iNumPhiFreq][2*(iNumftFreq-1)+2] =
     ...
     ,j)*pow(dPhifreq  ,iNumOrder-j)*CumulantImagPart[iNumOrder-j][j]  ,d2pi);
...
void LGMSVFillPayoff(/* Input */
  ...
  /* Calculation of beta(Tex  ,Tstar)*D/DTstar ( beta(Tex  ,Tstar)) and D/DTstar
   * ( beta(Tex  ,Tstar)) */
  ...
  /*																												*/
  /* Payoff(Tex)/B(Tex  ,Tstar)*B(Today  ,Tstar) = Sum of Ci*DF(Tex  ,Ti)/DF(Tex
   * ,Tstar)*DF(Today  ,Tstar)					*/
  /*										= Sum of Ci*DFF(Tex  ,Ti)/DFF(Tex
   * ,Tstar)*DF(Today  ,Tstar)*					*/
   ...
   */
  /*	f(Ti  ,Tstar)-f(0  ,Tstar)-(D/DTstar(Beta(Ti  ,Tstar))*Beta(Ti
   * ,Tstar))(Ti  ,Tstar)
   */
...
void LGMSVClosedForm(                 /* Inputs */
  ...
  /* MomentFunc(i  ,j  ,k) = E(PhiBar^i*Y^j*V^k) where PhiBar = Phi - E(Phi) a
   * nd Y=exp(lx*(Tstar-t))*(f(t  ,Tstar)-f(0  ,Tstar)) */
  MomentFunc =
  ...
  RealPart = 0.0;
  /*
  LGMSVCalculateReClosedForm(dPhifreq  , 0  , Sig[0]  , dAlpha  , dRho  ,
  dExTime  , &RealPart);
  */
  ...
  if (dLambdaEps > 1.0E-12) {
    ...
    */
    ...
    LGMSVCalculateDensityClosedForm(iNbCumulant  ,
                                                                    ...
                                                                    dPhiFreqStep
    , dftFreqStep  , Sig[0]  , dAlpha  , dRho  , dExTime  , Density  ,
                                                                    Densityspeq);
    */
  }

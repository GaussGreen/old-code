...
*   FUNCTIONS USED  :
*
*   EXT FUNCTIONS   :   coverage (srt_f_utldates.cxx)
*
*
********************************************************************************
...
/*****************************************************************************
    ...
    DESCRIPTION :   Allocates dates according to sdp stored in dfind        ,
                    cvg        , dfindlen which will find their way into a
comll. These are the dates of a swap        , and may be used to compute a swap
rate        , level payment        , or cap.
...
static Err grfn_make_swapdates(SwapDP *swapdp, Date today, StructType type,
                               Date **df_date, double **cvg, int *num_dates) {
  Err err = NULL;
  int num_start_dates;
  Date *start_dates;
  Date *end_dates;
   ...
   */
  err = swp_f_make_FixedLegDatesAndCoverages(swapdp, today, df_date, num_dates,
                                             &start_dates, &end_dates, cvg,
                                             &num_start_dates);
...
/* Function to compute right away all the pay dates         , the spreads (with
   their coverage) on the floating leg  */
static Err grfn_make_floating_leg(SwapDP *float_sdp, Date today,
                                  StructType type, Date **float_df_date,
                                  double **float_cvg, double **spread,
                                  int *float_dfindlen, String ref_rate_code) {
  Err err = NULL;
  int num_start_dates;
  Date *fixing_dates;
  Date *start_dates;
  ...
  /* If dealing with CASH        , there is no need to do this ( float leg =
   * df(f) - df(i) )*/
  ...
  err = swp_f_make_FloatLegDatesCoveragesAndSpreads(
      float_sdp, today, ref_rate_code, float_df_date, float_dfindlen,
      &fixing_dates, &start_dates, &end_dates, float_cvg, spread,
      &num_start_dates);
...
/* -----------------------------------------------------------------------------
    ...
    DESCRIPTION     :   This function checks the arguments of a GRFN financial
                        ...
                        of 4 args: (s)tart        , (e)nd        , (c)ompounding
   , (b)asis.

                        If there is a "start" and an "end"        , direction is
   set.

   -----------------------------------------------------------------------------
 */

Err check_financial_args(String name, COMLLType type, COMLL_PTR lastarg,
                         GrfnDeal *gd, Date *s, long *e_nfp, BasisCode *b,
                         SrtCompounding *c, long *aux, String short_ref_rate,
                         String long_ref_rate, double **dstore, long *dstorelen,
                         comllchar **sstore, long *sstorelen) {
  ...
  /* s = start        , e = end        , c = compounding        , b = basis , d
   * = double        , t = string */
  switch (type) {
  case COMLL_F_PAYOFF:
    code = "dt";
  ...
  for (i = 0; i < code_len; i++) {
    ...
    case 't': /* sTring */
      ...
      (*sstore) =
          (comllchar *)realloc((*sstore), (*sstorelen) * sizeof(comllchar));
      if ((*sstore) == NULL)
        return serror("%s %s", GRERR_MEMORY_ALLOC, name);
    ...
    case 'b': /* Basis */
      ...
      if ((lastarg->type != COMLL_STRING) ||
          (err = interp_basis(lastarg->sval, b))) {
    ...
    case 'c': /* Compounding */
      ...
      if ((lastarg->type != COMLL_STRING) ||
          (err = interp_compounding(lastarg->sval, c)))
    ...
    case 'e': /* End date */
      ...
      *e_nfp = (Date)DTOL(lastarg->dval);
      ...
      switch (type) {
        ...
        /* If end is an nfp (3 for instance) it is in month for CVG        , FRA
         * , DF
         */
      case COMLL_F_CVG:
      case COMLL_F_FRA:
...
/* -----------------------------------------------------------------------------
    ...
    DESCRIPTION     :   This function checks the arguments of a GRFN financial
                        function against the contents of a coded string
                        comprising of special characters defined in the
                        GRFN symbol table.  It does not overwrite any
   information        , unlike check_  financial_args
...
Err parse_financial_args(String name, COMLLType type, COMLL_PTR lastarg,
                         GrfnDeal *gd, Date *s, long *e_nfp, BasisCode *b,
                         SrtCompounding *c, long *aux, String short_ref_rate,
...
/* ------------------------------------------------------------------------------
    ...
    DESCRIPTION :   Pay requires a special treatment also because it
                                        ...
                                        In case of simulations        , the
   report does not have to be done (the report should only happen if in the
   past)

                                        The comll should look like this when it
   is passed in:

                                           (top)                   (bottom)
                                            Date          -->  [Amount
   Expression] REAL                  REAL/COMLL

                                        (only the arguments within the PAY
   function are past) lastargs points to the last argument passed to PAY: the
   payment date

                                        If we are in the past and DATE is in the
   past        , don't do anything.

                                        If we are in the past and DATE in in the
   future: adds TODAY as the start date of the discounting: AMOUNT   -->
   PAYFROMPAST(TODAY        ,DATE) (top)               (bottom) that will
   correspond to the following operations AMOUNT * DF(TODAY        ,paydate) and
   will allow for proper computations once the COMLL is evaluated without
   failing on the use of DF

                    If we are in the future        , use the COMLL_PAY type ,
                                        setting the discretisation date as the
   start date of the discounting: AMOUNT --> PAY..(NOW        ,DATE)        ,
   that will correspond to the following operations AMOUNT * DF(NOW ,paydate)
...
static Err pay_function(String name, GrfnSymbol *gs, GrfnDeal *gd,
                        COMLL_PTR lastarg) {
  ...
  /* Makes sure that pay is not used anywhere else but in the last column of the
   * tableau */
  if (gd->J != gd->sswidth - 1) {
    return serror("%s", GRERR_PAY_LAST_COL);
  }

  /* Make sure the pay date (top element) is a real; if not return an error
   * message */
  ...
  /* Adds the "Reference to Pay" status to the cell: the cell should only
   * contain PAY */
  ...
  if (gd->is_history) {
    ...
    if ((long)top->dval < (gd->today + gd->end_of_day_payment)) {
      /* Adds an extra element in the COMLL with a PAYINPAST type ( to keep
       * track of payment) */
    ...
    /* Pay is used in the past        , with (complex) payment in the future :
       make it a [ AMOUNT * PAYFROMPAST ] COMLL */
    else {
  ...
  /* Pay is used in the FUTURE        , with a simple or complex payment
     expression: make it a [ AMOUNT * PAY ] COMLL */

  {
    ...
    /* Set NOW as the date to which the cash flow will be discounted (date in
     * the future) */
    df_start = gd->nowdt;
  ...
  if (require_df == SRT_YES) {
    ...
    /* Stores the discount factor dates (df_start and pay date) in the bottom
     * element (as for a DF type ) */
    bot->dfindlen = 2;
  ...
  /* Removes the top element of the COMLL        , where the payment date was
   * stored (keeping address of top the same) */
  comll = top->next;
...
static Err payn_function(String name, GrfnSymbol *gs, GrfnDeal *gd,
                         COMLL_PTR lastarg) {
  double pv_of_cashflow = 0.0;
  Err err = NULL;
  COMLL_PTR top, bot, comll;
...
/******************************************************************************\
    ...
    DESCRIPTION     :   Interpret financial function
                                                Function called by the YACC
parser (through the grfn_interp_func function) when encountering a NAME(...)
type of input string that correspond to a financial name

                                                The COMLL lastarg points to the
last argument of the function        , which is the top of the COMLL (parsed
backwards)
    ...
    * date)        , then we can have a maximum of 3 "optional" arguments
specified *
    *                                                                         *
    ...
    *                                                                         *
    * When a Reference Rate Code is specified        , then we must have a
function *
    ...
    *                                                                         *
    * We assume that for all functions containing optional arguments        ,
the 1st
*
...
Err grfn_interp_fin_func(String name, GrfnSymbol *gs, GrfnDeal *gd,
                         COMLL_PTR lastarg)

{
  ...
  /* Set up and update the und_index (on first parse        , make the list of
   * names)
   */
  und_index = 0;
  ...
  /* From now on        , the underlying name has been stripped out:any optional
     argument left is a short ref rate        , a long one or both (opt_args has
     not been changed) */
  ...
  if (gd->is_history == SRT_YES && grfn_lang_cur_amcell == SRT_NO) {
    ...
    if (gs->type == COMLL_F_CVG) {
      err = check_financial_args(name, gs->type, lastarg, gd, &start, &end_nfp,
                                 &basis, &comp, aux, short_ref_rate,
                                 long_ref_rate, &dstore, &dstorelen, &sstore,
                                 &sstorelen);
    ...
    if (gs->type == COMLL_F_DF || gs->type == COMLL_F_PVRANGE ||
        gs->type == COMLL_F_CMS || gs->type == COMLL_F_LEVEL ||
        gs->type == COMLL_F_PAYOFF) {
      return serror("%s: %s", name, " Historical use of function is illegal!");
    }

    /* Check the number of optional arguments input : there should be at least
     * one ref rate */
    ...
    /* If the LAST argument is a string and we have at least 2 optional
       arguments then we MUST have a valid (non-NULL) fixing function */
    if ((top->type == COMLL_STRING) && (gs->no_opt_args >= 2)) {

      /* get the info */
      if (err = check_financial_args(name, gs->type, lastarg, gd, &start,
                                     &end_nfp, &basis, &comp, aux,
                                     short_ref_rate, long_ref_rate, &dstore,
                                     &dstorelen, &sstore, &sstorelen)) {
        return err;
      }
      /* Make sure the end date is properly defined and set (not only nfp) */
      err = swp_f_setSwapDP(start, end_nfp, comp, basis, &sdp);
      ...
      fixing =
          add_unit(sdp.start, -sdp.spot_lag, SRT_BDAY, NO_BUSDAY_CONVENTION);
      if (fixing < gd->today + gd->end_of_day_fixing) {
        ...
        /* Get the historical fixing for the first ref rate code ( the long one)
         * on that date */
        ...
        top->dval = swp_f_fra(sdp.start, sdp.end, sdp.basis_code, szYieldCurve,
                              comll->sval);
      }
  ...
  /* ----------------------------------------------------------------------------
     ...
     ----------------------------------------------------------------------------
   */
  if ((gd->nowdt == gd->today) &&
      ((gs->type == COMLL_F_FRA) || (gs->type == COMLL_F_SWAP))) {

    /* Check the number of optional arguments */
    if (opt_args == gs->no_opt_args) {

      /* If the LAST argument is a string and we have at least 2 optional
         arguments then we must have a valid (non-NULL) function */
      if ((top->type == COMLL_STRING) && (gs->no_opt_args >= 2)) {
        /* get the info */
        comll_temp = lastarg;
        if (err = parse_financial_args(name, gs->type, comll_temp, gd, &start,
                                       &end_nfp, &basis, &comp, aux,
                                       short_ref_rate, long_ref_rate)) {
          return err;
        }
        /* Have we got a function to get the reference rates fixings */
        if (gd->fixing_fct != NULL) {
          /* Check to see if the fixing date of the FRA is today        , or in
           * the future */
          /* Make sure the end date is properly defined and set (not only nfp)
           */
          err = swp_f_setSwapDP(start, end_nfp, comp, basis, &sdp);
          ...
          /* calculate the fixing date */
          fixing = add_unit(sdp.start, -sdp.spot_lag, SRT_BDAY,
          ...
          if (fixing < gd->today + gd->end_of_day_fixing) {
            ...
            /* If error in getting fixing and end of day fixing flag is on :
               error */
            ...
            /* get the market name from the underlying */
            SrtGetUnderlyingYieldCurveName(gd->domestic_und, szYieldCurve);
            top->dval = swp_f_fra(sdp.start, sdp.end, sdp.basis_code,
                                  szYieldCurve, comll->sval);
            /* We have a REAL value returned        , so use it (set type) */
            top->type = COMLL_REAL;
  ...
  /* --------------------------------------------------------------------------------
                  Now we are looking at an event date in "future" (i.e. on or
     after the calculation date so discard the optional arguments (if any)) .
     --------------------------------------------------------------------------------
   */

  /* --------------------------------------------------------------------------
     comll point now to the first optional argument that is not the und name
     Since we need to be able to collect the short reference rate name        ,
     we only get rid of the first optionnal arguments for a COMLL_F_SWAP. The
     und name has already been stored in the first parsing of the tableau , and
     removed from the COMLL at the second parsing. If there is no optionnal
     reference rate code        , the function check_financial_arg() will set a
     default string
     --------------------------------------------------------------------------
   */

  /* Check arguments of financial functions and sets the sdp & ref rates
   * accordingly */
  if (err = check_financial_args(name, gs->type, lastarg, gd, &start, &end_nfp,
                                 &basis, &comp, aux, short_ref_rate,
                                 long_ref_rate, &dstore, &dstorelen, &sstore,
                                 &sstorelen)) {
    return err;
  ...
  /* ----------------------------------------------------------------------------------------
     Prepares the arguments (dates        , space for df        , cvg...) that
     will be required for the computation of the financial function        , and
     stack them into top
     ----------------------------------------------------------------------------------------
   */
  ...
  case COMLL_F_PAYOFF:
    ...
    if (product->RequestDfDates) {
      err = product->RequestDfDates(product, (long)top->dval, &n_unds, &n_dfs,
                                    &dates);
      if (err)
        return err;
  ...
  case COMLL_F_CMS: /* should certainly be merged with COMLL_F_SWAP */
    ...
    /* Store und_index in ivec        , swaps dates        , cvg        , sprd
     * ... in top */
    if (und_index >= 0) {
      top->ivec[0] = und_index;
      ...
      err = grfn_make_swapdates(&sdp, gd->today, SWAP, &top->dfind, &top->cvg,
                                &top->dfindlen);
      if (err)
        return err;

      /* Create and populate the floating leg (backward from same end as fixed
       * leg) */
      sdp.direction = BKWD;
      err = grfn_make_floating_leg(&sdp, gd->today, SWAP, &top->dfindfloat,
                                   &top->cvgfloat, &top->spread,
                                   &top->dfindfloatlen, short_ref_rate);
      if (err)
        return err;
  ...
  case COMLL_F_SWAP:
    ...
    if (und_index >= 0) {
      ...
      /* If the long ref rate is mentionned and the short ref rate == CASH ,
       * get long spread */
      if (!(strcmp(short_ref_rate, "CASH")) &&
          (strcmp(long_ref_rate, "") != 0)) {
        top->long_spread = swp_f_spread(sdp.start, sdp.end, long_ref_rate);
        if (top->long_spread == SRT_SPREAD_ERROR)
          return serror("Cannot compute long spread for %s(%d        ,%d)",
                        long_ref_rate, sdp.start, sdp.end);
      } else
  ...
  case COMLL_F_LEVEL:
    ...
    /* Store und_index in ivec        , swaps dates        , cvg        ,  ...
     * in top */
    if (und_index >= 0) {
      top->ivec[0] = und_index;
  ...
  case COMLL_F_FRA:
    ...
    if (und_index >= 0) {
      ...
      top->dfindfloatlen = 0;
      top->spread[0] =
          swp_f_spread(top->dfind[0], top->dfind[1], short_ref_rate);
      if (top->spread[0] == SRT_SPREAD_ERROR)
        return serror("Error in getting %s(%d        ,%d) spread",
                      short_ref_rate, top->dfind[0], top->dfind[1]);
    }
  ...
  case COMLL_F_ACCINT:
    ...
    if (i == gd->auxlen[aux[0]]) {
      ...
      top->dval =
          coverage(DTOL(floor(gd->aux[aux[0]][i])), sdp.start, sdp.basis_code) *
          gd->aux[aux[1]][i];
    }

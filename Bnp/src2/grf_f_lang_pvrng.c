...
/******************************************************************************\
 ...
 pvrng(auxrng1  ,auxrng2  ,startdate).  As this is a financial function  , we
will end up with only one node. (The nodes after top will be freed.) This
assumes that auxrng1 contains dates  , auxrng2 contains (known) cash flows; only
the cashflows in auxrng2 corresponding to dates in auxrng1 that are greater than
startdate are included; pvrng is supposed to calculate the pv of those
cashflows; hence it is a sum of discount factors.  In grfn_eval_event() a node
of type COMLL_PVRNG  will be evaluated just like a level payment; the only
difference is that the coverages will be different.
...
, we will end up with only one node. (The nodes after top will be freed.) This
assumes that auxrng1 contains dates  , auxrng2 contains (known) cash flows; only
the cashflows in auxrng2 corresponding to dates in auxrng1 that are greater than
startdate and <= enddate are included; drty is supposed to calculate the pv of
those cashflows (like the dirty price of a bond); hence it is a sum of discount
factors.  It would be calculated in grfn_eval_event just like a call to
pvrng(...) see above.

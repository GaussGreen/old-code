 ...
 *
 * DESCRIPTION  	: this option can be decomposed in a sum of extinguish
 *and lightable options with barrier and stirke being equal to each rung defined
 *and the associated strike CALLS		: srt_f_optblksch() :
 *srt_f_optprtext() : srt_f_optprtlgt()
 *
...
double srt_f_optratcht(double fwd1, double fwd2, double spot, double hist_ext,
  ...
  ***/
  ...
  } else if (num_rungs == 2) { /* if only one rung                 */
    if ((rungs[2] - spot) > 0)
  ...
  if (mat1 <= 0) {
    ...
    premium = srt_f_optblksch(fwd2, strikes[first_rung], vol2, mat2, disc,
                              call_put, PREMIUM);
  } else if (first_rung > num_rungs) {
    premium = srt_f_optblksch(fwd2, strikes[num_strikes], vol2, mat2, disc,
                              call_put, PREMIUM);
  } else {
    premium = srt_f_optprtext(fwd2, fwd1, spot, strikes[first_rung - 1],
                              rungs[first_rung], vol1, vol2, mat1, mat2, disc,
                              call_put, down_up, PREMIUM);

    for (i = first_rung; i <= num_rungs; i++) {
      premium +=
          srt_f_optprtlgt(fwd2, fwd1, spot, strikes[i], rungs[i], vol1, vol2,
                          mat1, mat2, disc, call_put, down_up, PREMIUM);
      if (i <= num_rungs - 1)
        premium -=
            srt_f_optprtlgt(fwd2, fwd1, spot, strikes[i], rungs[i + 1], vol1,
                            vol2, mat1, mat2, disc, call_put, down_up, PREMIUM);
    }
  }
  ...
  case DELTA_FWD1: /*** DELTA FWD 2***/
    shift = fwd1 / 10000;
    answer =
        (srt_f_optratcht(fwd1 + shift, fwd2, spot, hist_ext, vol1, vol2, mat1,
                         mat2, disc, call_put, PREMIUM, rungs, strikes) -
         premium) /
        shift;
    break;
  ...
  case DELTA_FWD2: /*** DELTA FWD 2***/
    shift = fwd2 / 10000;
    answer =
        (srt_f_optratcht(fwd1, fwd2 + shift, spot, hist_ext, vol1, vol2, mat1,
                         mat2, disc, call_put, PREMIUM, rungs, strikes) -
         premium) /
        shift;
    break;
  ...
  case DELTA: /*** DELTA SPOT + FWD1 + FWD 2 ***/
    shift = spot / 10000;
    answer =
        (srt_f_optratcht(fwd1 * (1 + shift / spot), fwd2 * (1 + shift / spot),
                         spot + shift, hist_ext, vol1, vol2, mat1, mat2, disc,
                         call_put, PREMIUM, rungs, strikes) -
         premium) /
        shift;
    break;
  ...
  case VEGA1: /*** VEGA1 ***/
    shift = GVOPT.vol_add;
    answer =
        (srt_f_optratcht(fwd1, fwd2, spot, hist_ext, vol1 + shift, vol2, mat1,
                         mat2, disc, call_put, PREMIUM, rungs, strikes) -
         premium) /
        shift;
    break;
  ...
  case VEGA2: /*** VEGA2 ***/
    shift = GVOPT.vol_add;
    answer =
        (srt_f_optratcht(fwd1, fwd2, spot, hist_ext, vol1, vol2 + shift, mat1,
                         mat2, disc, call_put, PREMIUM, rungs, strikes) -
         premium) /
        shift;
    break;
  ...
  case THETA: /*** THETA  ***/
    shift = YEARS_IN_DAY;
    answer =
        srt_f_optratcht(fwd1, fwd2, spot, hist_ext, vol1, vol2, mat1 - shift,

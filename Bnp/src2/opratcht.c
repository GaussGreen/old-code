...
***/
	...
	for (i = 1; strikes[i] > 0 && i <= MAX_RUNG; i++);   		/* set no. of strikes */
		num_strikes = i - 1;
	for (i = 2; rungs[i]  > 0 && i <= MAX_RUNG; i++);        	/* set no. of rungs   */
	...
	switch (greek)
	{
		...
		case DELTA_FWD1 : 	/*** DELTA FWD 2***/  
			shift = fwd1 / 10000;
			answer = ( srt_f_optratcht(	fwd1 + shift, 
										fwd2, 
										spot, 
										hist_ext, 
										vol1,
		...
		case DELTA :	/*** DELTA SPOT + FWD1 + FWD 2 ***/  
			shift = spot / 10000;
			answer = ( srt_f_optratcht(	fwd1 * (1 + shift/spot), 
										fwd2 * (1 + shift/spot),
										spot + shift, 
										hist_ext, 
										vol1,

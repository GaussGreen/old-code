...
Err FundLeg_RequestDfDates(funding_leg* fundleg, long date, int* n_dates, long** pdates)
{
    ...
    for (i = 0;
         ...
         date - 10 >
             add_unit(
                 (fundleg->cpn[i]).start_date, -fundleg->spot_lag, SRT_BDAY, MODIFIED_SUCCEEDING);
         i++)
        ;
...
Err FundLeg_Payoff(funding_leg* fundleg, long today, long date, double* dfs, double* payoff)
{
    Err err = NULL;
    int i, k;

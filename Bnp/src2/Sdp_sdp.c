    ...
    int       max_iterations)
{
    ...
    while (go_on == 0)
    {
        ...
        phi = compute_infeas(
            const_mat, const_val, num_const, xmat, zmat, cmat, dim, yvec, tau, &bufpinf, &bufdinf);
        ...
        if (iter == 1)
            first_mu = mu;
        /*  Test for success. */
        if (global_error(const_mat, const_val, num_const, xmat, zmat, cmat, dim, yvec, tau, kappa) <
            tolerance)
            go_on = 1;
        ...
        if ((status_tracker[iter - 1][2] <= status_tracker[iter][2]) && (feasibility_step == 0) &&
            (iter >= 10))
            feasibility_step = 0;
        else
            feasibility_step = 0;
        ...
        {
            sigma = compute_sigma(
                xmat, dpxmat, zmat, dpzmat, dim, alphap, betap, tau, dptau, kappa, dpkappa, expon);
            eta = 1 - sigma;
            /*  Update gamma */
            ...
            right_vector(
                ...
                vecbuf1,
                default_num_improve);
            /*  System matrix is the same as for the predictor (stored in systmat), matbuf1 is the

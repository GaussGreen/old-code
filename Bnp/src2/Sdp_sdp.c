...
Err srt_sdp(double ***const_mat, double *const_val, double **cmat, int dim, int num_const, double **xmat, double *obj_val, double *error, int *return_error, double toler, int printlevel, int max_iterations)
{
	...
	num_calls++;
	if (printlevel>=2) smessage("SDP: Initializing... Call number: %d \n",num_calls);
	...
	while (go_on==0)
	{
		...
		phi=compute_infeas(const_mat,const_val,num_const,xmat,zmat,cmat,dim,yvec,tau,&bufpinf,&bufdinf);
		/*  Status display and tracking. In debug send the status_tracker to the monitor. */
		status(iter,const_mat,const_val,num_const,systmat,xmat,zmat,cmat,dim,yvec,tau,kappa,sigma,eta,alphac,betac,status_tracker,printlevel,max_iter,numer_error);
		/*  Send matrix to monitor if in debug. **** DEBUG ****** */
		visu_mat(status_tracker,max_iter,11,1);
		/*  ---------------------------- */
		if (iter==1) first_mu=mu;
		/*  Test for success. */
		if (global_error(const_mat,const_val,num_const,xmat,zmat,cmat,dim,yvec,tau,kappa)<tolerance) go_on=1;
		/*  Or for infeasibility... */
		if ((fabs(mu/first_mu)<tolerance_infeasibility) && ((tau/kappa)/(first_tau/first_kappa)<tolerance_infeasibility)) go_on=2;
		/*  Then test for increasing primal infeasibility. */
		if ((status_tracker[iter-1][2]<=status_tracker[iter][2]) && (feasibility_step==0) && (iter>=10)) feasibility_step=0; else feasibility_step=0;
		/*  *************************** FEASIBILITY STEP ******************* */
		if (feasibility_step==1)
		{
			take_feasibility_step(const_mat,const_val,num_const,xmat,dcxmat,zmat,dczmat,dim,yvec,dcyvec,&dctau, tau, &dckappa,inverse_gram);
			alphac=step_length(xmat,dcxmat,dim,tau,dctau,kappa,dckappa,gamma);
			betac=step_length(zmat,dczmat,dim,tau,dctau,kappa,dckappa,gamma);
			set_steps(const_mat,const_val,num_const,yvec,dcyvec,xmat,dcxmat,zmat,dczmat,cmat,dim,kappa,dckappa,tau,dctau,eta,&alphac,&betac,line_reduc,"Corrector");
		}
		/*  *************************** PREDICTOR STEP ********************* */
		if ((go_on==0) && (feasibility_step==0))
		{
			...
			sigma=0; eta=1;
			/*  Solve for (dy,dtau). */
			err=system_matrix(const_mat,num_const,xmat,zmat,cmat,dim,"AHO",systmat,big_inv_E,&numer_error,default_num_improve);
			/*  Test for singular system operator; */
			if (err!=NULL) go_on=7;
			left_matrix(systmat,num_const,const_val,kappa,tau,systmat);
			right_vector(const_mat,num_const,big_inv_E,xmat,zmat,cmat,dpxmat,dpzmat,"Predictor",dim,yvec,const_val,eta,tau,sigma,mu,kappa,dptau,dpkappa,vecbuf1,default_num_improve);
			/*  Solve the main system for (dy.dtau): */
			/*  Test the condition number, if to bad, stops. */
			if (condition_mat(systmat,num_const+1)>condition_bound) go_on=6;
			err=lsolve(systmat,vecbuf1,num_const+1,vecbuf2,matbuf1,0,(10e-2)*tolerance,default_num_improve);
			/*  -------- */
			...
			/*  Get dx,dz,dkappa. */
			delta_z(const_mat,num_const,zmat,cmat,dim,yvec,dpyvec,tau,eta,dptau,dpzmat);
			delta_x(big_inv_E,xmat,dpxmat,zmat,dpzmat,dczmat,dim,sigma,mu,"Predictor",dpxmat,default_num_improve);
			dpkappa=(rc_val(sigma,mu,tau,kappa)-kappa*dptau)/tau;
			/*  Define the primal and dual step length. */
			alphap=step_length(xmat,dpxmat,dim,tau,dptau,kappa,dpkappa,gamma);
			betap=step_length(zmat,dpzmat,dim,tau,dptau,kappa,dpkappa,gamma);
			set_steps(const_mat,const_val,num_const,yvec,dpyvec,xmat,dpxmat,zmat,dpzmat,cmat,dim,kappa,dpkappa,tau,dptau,eta,&alphap,&betap,line_reduc,"Predictor");
			if ((alphap==0)||(betap==0)) go_on=3;
		}
		/*  *************************** CORRECTOR STEP ********************** */
		if ((go_on==0) && (feasibility_step==0))
		{
			sigma=compute_sigma(xmat,dpxmat,zmat,dpzmat,dim,alphap,betap,tau,dptau,kappa,dpkappa,expon);
			eta=1-sigma;
			/*  Update gamma */
			gamma=frac_gamma_reduction+0.1*frac_gamma_reduction*(-d_max(-alphap,-betap));
			/*  Then apply corrector step as for predictor with a few add-ins. */
			/*  Solve for (dy,dtau). */
			right_vector(const_mat,num_const,big_inv_E,xmat,zmat,cmat,dpxmat,dpzmat,"Corrector",dim,yvec,const_val,eta,tau,sigma,mu,kappa,dptau,dpkappa,vecbuf1,default_num_improve);
			/*  System matrix is the same as for the predictor (stored in systmat), matbuf1 is the inverse for further usage. */
			/*  Solves the main system for (dy,dtau) */
			/*  If error small enough, system is dimension num_cont instead of num_const+1 */
			/*  for path-following. */
			err=lsolve(systmat,vecbuf1,num_const+1,vecbuf2,matbuf1,0,(10e-2)*tolerance,default_num_improve);
			/*  ---------- */
			...
			/*  Get dx,dz,dkappa. */
			delta_z(const_mat,num_const,zmat,cmat,dim,yvec,dcyvec,tau,eta,dctau,dczmat);
			delta_x(big_inv_E,xmat,dpxmat,zmat,dpzmat,dczmat,dim,sigma,mu,"Corrector",dcxmat,default_num_improve);
			/*  For match with mathematica ******* DEBUG ********** */
			/* dckappa=(rc_val(sigma,mu,tau,kappa)-dptau*dpkappa-kappa*dctau)/tau; */
			dckappa=(rc_val(sigma,mu,tau,kappa)-kappa*dctau)/tau;
			/*  Define the primal and dual step length. */
			alphac=step_length(xmat,dcxmat,dim,tau,dctau,kappa,dckappa,gamma);
			betac=step_length(zmat,dczmat,dim,tau,dctau,kappa,dckappa,gamma);
			set_steps(const_mat,const_val,num_const,yvec,dcyvec,xmat,dcxmat,zmat,dczmat,cmat,dim,kappa,dckappa,tau,dctau,eta,&alphac,&betac,line_reduc,"Corrector");
			if ((alphac==0)||(betac==0)) go_on=4;
		}
		/*  ********* STEP ******** */
		if (go_on==0)
		{
			...
			add_mat(tau/primal_tau,xmat,(tau/primal_tau)*alphac,dcxmat,dim,xmat,"No");
			add_mat(tau/dual_tau,zmat,(tau/dual_tau)*betac,dczmat,dim,zmat,"No");
			add_vec(tau/dual_tau,yvec,(tau/dual_tau)*betac,dcyvec,num_const,yvec);
		}
		/*  For DEBUG ****************************************************************** */
			visu_mat(xmat,dim,dim,2);visu_mat(dcxmat,dim,dim,4);
			visu_mat(zmat,dim,dim,5);visu_mat(dczmat,dim,dim,7);
			/*  **************************************************************************** */
		/*  Test for max iteration. */
		if (iter>=max_iter) go_on=5;
		/*  ***************************** End of Main loop ******************** */
	}
	...
	if (printlevel>=1) 
	{
		...
		{
			/*  Status display and tracking. In debug send the status_tracker to the monitor. */
			status(iter,const_mat,const_val,num_const,systmat,xmat,zmat,cmat,dim,yvec,tau,kappa,sigma,eta,alphac,betac,status_tracker,4,max_iter,numer_error);
		}
		smessage("SDP:  %s",err);
		if (printlevel>=2) 
		{
	...
	*return_error=go_on;
	*error=global_error(const_mat,const_val,num_const,xmat,zmat,cmat,dim,yvec,tau,kappa);
	/*  Free */
...
Err srt_bgm_sdp(double ***const_mat, double *const_val, double **cmat, int dim, int num_const, double **xmat, double *obj_val, double *error, int *return_error, double toler, int printlevel, int max_iterations)
{
	/*  General parameters for the algorithm. */
	int max_iter=max_iterations;
	double tolerance=toler;
	...
	*return_error=go_on;
	*error=global_error(const_mat,const_val,num_const,xmat,zmat,cmat,dim,yvec,tau,kappa);
	/*****************************Added line from original version***********************/

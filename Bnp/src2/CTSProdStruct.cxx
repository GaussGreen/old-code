...
Err cts_init_mkt(long today, char *yc, char *vc, char *ref, char *swap_freq,
  ...
  mkt->strikes_in_vol =
      (double *)calloc(mkt->num_strikes_in_vol, sizeof(double));
  if (!mkt->strikes_in_vol)
    return "Allocation error in cts_init_mkt";
  memcpy(mkt->strikes_in_vol, strikes_in_vol,
         mkt->num_strikes_in_vol * sizeof(double));
...
Err cts_copy_mkt(CTS_MKT src, CTS_MKT dest) {
  ...
  dest->strikes_in_vol =
      (double *)calloc(dest->num_strikes_in_vol, sizeof(double));
  if (!dest->strikes_in_vol)
    return "Allocation error in cts_copy_mkt";
  memcpy(dest->strikes_in_vol, src->strikes_in_vol,
         dest->num_strikes_in_vol * sizeof(double));
...
Err cts_fill_fund_leg(
  ...
  fund_leg->cpn =
      (cts_fund_cpn *)calloc(fund_leg->num_cpn, sizeof(cts_fund_cpn));
  if (!fund_leg->cpn) {
...
static Err Construct_Schedule(long lTodayDate, char *cYcName, long lStartDate,
                              long lTheoEndDate, SrtCompounding iFreq,
                              SrtBasisCode iBasis, int *iNbCoupon,
                              long *lCouponDate, double *dCouponTime,
                              double *dCouponCvg, double *dCouponDf,
                              double *dSwpCash, double *dSumDf,
                              double *dLevel) {
...
Err cts_init_fixing(long today, char *yc, int ref_fix_lag, long ref_start_date,
  ...
  fixing->ref_fix_date =
      add_unit(ref_start_date, -ref_fix_lag, SRT_BDAY, MODIFIED_SUCCEEDING);
  ...
  if (fixing->ref_fix_date >= today) {
    ...
    err = Construct_Schedule(
        today, yc, ref_start_date, ref_theo_end_date, srt_freq, srt_basis,
        &fixing->schedule.iNCpn, &(fixing->schedule.lCpnDate[0]),
        &(fixing->schedule.dCpnTime[0]), &(fixing->schedule.dCpnCvg[0]),
        &(fixing->schedule.dCpnDf[0]), &(fixing->ref_swp_cash),
        &(fixing->ref_sum_df), &(fixing->ref_level));

    if (err)
      goto FREE_RETURN;

    fixing->ref_cpnd =
        365.0 / (fixing->schedule.lCpnDate[1] - fixing->schedule.lCpnDate[0]);
    fixing->ref_cpnd = (fixing->ref_cpnd - floor(fixing->ref_cpnd) < 0.50
...
Err cts_check_fixing(CTS_FIX fixing) {
  if (fixing->schedule.iNCpn > 0) {
    if (fixing->schedule.lCpnDate[0] >=
        fixing->schedule.lCpnDate[fixing->schedule.iNCpn - 1])
...
Err cts_calc_shifted_range_pv(
    ...
    double payoff,  /*	Payoff in range */
    double shift_strike, double call_spread, double numer_spread, CTS_MKT mkt,
    CTS_CPN cpn, double *alpha, double *beta, int *nstr, double *str,
    double *nbopt, double *price) {
  Err err = NULL;
  ...
  for (i = 0; i < cpn->used_nfix; i++) {
    ...
    for (j = 0; j < *nstr; j++) {
      err = mkt->get_cash_vol(mkt->vc, fix->schedule.lCpnDate[0],
                              fix->ref_theo_end_date, str[j], 0, mkt->ref, &vol,
                              &power);
      ...
      if (power > 0.5) {
        opt = srt_f_optblksch(ref_swp_fwd, str[j], vol, fix->ref_fix_time, 1.0,
                              SRT_CALL, SRT_PREMIUM);
      } else {
        opt = srt_f_optblknrm(ref_swp_fwd, str[j], vol, fix->ref_fix_time, 1.0,
                              SRT_CALL, SRT_PREMIUM);
      }
...
Err cts_adjust_call_spread(
  ...
  err = cts_calc_shifted_range_pv(buy_sell, value_zero, min_barrier,
                                  max_barrier, lb, ub, payoff, 0, call_spread,
                                  call_spread, mkt, cpn, &alpha, &beta, &nstr,
                                  str, nbopt, &price_tgt);
  ...
  err = cts_calc_shifted_range_pv(buy_sell, value_zero, min_barrier,
                                  max_barrier, lb, ub, payoff, shift1,
                                  call_spread, numer_spread, mkt, cpn, &alpha,
                                  &beta, &nstr, str, nbopt, &price1);
  ...
  while (fabs(price_tgt - price1) > precision && i < maxiter) {
    ...
    err = cts_calc_shifted_range_pv(buy_sell, value_zero, min_barrier,
                                    max_barrier, lb, ub, payoff, shift2,
                                    call_spread, numer_spread, mkt, cpn, &alpha,
                                    &beta, &nstr, str, nbopt, &price2);
...
Err cts_init_cpn(
  ...
  /*	Floating coupon */
  {
    ...
    if (cpn->type == 3 || cpn->type == 4) {
      ...
      cpn->pay_theo_end_date = add_unit(cpn->pay_start_date, pay_months,
                                        SRT_MONTH, NO_BUSDAY_CONVENTION);
    }

    cpn->pay_end_date =
        bus_date_method(cpn->pay_theo_end_date, MODIFIED_SUCCEEDING);
  ...
  for (i = 0; i < cpn->used_nfix; i++) {
    ...
    err = cts_init_fixing(today, mkt->yc, fix_lag_bd, start, end, srt_used_freq,
                          srt_used_basis, ref_fwd_spreads[cpn->used_fixidx[i]],
                          &(cpn->fix[i]));
  ...
  /*	Range */
  {
    if (cpn->type < 3) {
      /* classical Time Swap */
      err = cts_init_cpn_payoff_for_range(buy_sell, value_zero, CTS_MINBARRIER,
                                          CTS_MAXBARRIER, lb, ub, payoff,
                                          call_spread, cpn);
      ...
      if (numer_spread - call_spread > 1.0E-08) {
        ...
        err = cts_adjust_call_spread(
            buy_sell, value_zero, CTS_MINBARRIER, CTS_MAXBARRIER, lb, ub,
            payoff, cpn, mkt, call_spread, numer_spread,
            CTS_CALLSPREADPRECISION, CTS_CALLSPREADMAXITER);
      ...
      /* CIF type */
      err = cts_init_cpn_payoff_for_cif(value_zero, 1.0E-08, CTS_MAXBARRIER,
                                        pay_gearing, cpn_coupon, lb, ub, cpn);
    }
  }
  ...
  /*	Calc PV */
  if (calc_mkt_iv) {
    err = cts_calc_cpn_mkt_value(cpn, mkt, use_cmsopt, correl_start, correl_end,
                                 float_adjust_type);
    if (err)
      goto FREE_RETURN;
  }
...
Err cts_trim_fixings(
  ...
  if (trim_type == 0) {
    ...
    max_fix =
        (int)((cpn->cpn_pay_time - cpn->cpn_start_time) / min_fix_time + 0.5) -
        1;
  ...
  if (tstype == 1) {
    if (cpn_type < 3) {
      ...
      case 2:
        ...
        /*
        err = add_tenor (cpn->pay_start_date        , fix_tenor        ,
        NO_BUSDAY_CONVENTION        , &cpn->pay_end_date);
        */
        ...
        if (pay_fix_date < mkt->today + eod_fix_flag) {
          ...
          pay_end_date = add_unit(pay_start_date, pay_months, SRT_MONTH,
                                  MODIFIED_SUCCEEDING);
        ...
        cpn_pv *=
            (pay_gearing * libor + cpn_coupon) * cpn_cvg * cpn_not * df_pay;
...
Err cts_calc_cpn_mkt_value(
    CTS_CPN cpn, CTS_MKT mkt, int use_cmsopt,
    double correl_start, /*	Correl between libor fixing and libor paid at
                      start */
    double
        correl_end, /*	Correl between libor fixing and libor paid at start */
                                   ...
                                   0: ATM vol        , 1: Strike Vol */
{
  ...
  double ref_swp_cash, ref_swp_fwd, ref_swp_drs, ref_delay, pay_adjlogvol,
      ref_adjlogvol, ref_atmlogvol, ref_atmlogstd, ref_strikelogvol, ref_adj;
  double ref_strikestd, pay_new_strike;
  double pay_swp_cash, pay_swp_fwd, pay_swp_drs, pay_delay, pay_atmlogvol,
      pay_atmlogstd;
  ...
  if (cpn->type == 1 || cpn->type == 2) {
    ...
    err = swp_f_Cms_Rate(
        pay_swp_fwd, cpn->pay_fix_time, 1, cpn->pay_cpnd, pay_delay,
        cpn->pay_conv, mkt->vol_type, 0.0, 1, cpn->pay_start_date,
        cpn->pay_theo_end_date, mkt->cash_vol, cpn->pay_fwd_spread, mkt->vc,
        mkt->num_strikes_in_vol, mkt->strikes_in_vol, &pay_swp_drs);
    ...
    if (cpn->pay_fix_time > 0.0) {
      ...
      err = mkt->get_cash_vol_convert(
          mkt->get_cash_vol, mkt->vc, cpn->pay_start_date,
          cpn->pay_theo_end_date, cpn->pay_fix_time, pay_swp_cash, pay_swp_cash,
          mkt->ref, SRT_LOGNORMAL, &pay_atmlogvol);
  ...
  for (i = 0; i < cpn->used_nfix; i++) {
    ...
    if (cpn->beta || cpn->type == 1 || cpn->type == 2 ||
      ...
      err = swp_f_Cms_Rate(
          ref_swp_fwd, fix->ref_fix_time, fix->schedule.iNCpn - 1,
          fix->ref_cpnd, ref_delay, fix->ref_conv, mkt->vol_type, 0.0, 1,
          fix->schedule.lCpnDate[0], fix->ref_theo_end_date, mkt->cash_vol,
          fix->ref_fwd_spread, mkt->vc, mkt->num_strikes_in_vol,
          mkt->strikes_in_vol, &ref_swp_drs);
      ...
      for (j = 0; j < cpn->nstr; j++) {
        if (use_cmsopt) {
          ...
          err = swp_f_Cms_Option(
              ref_swp_fwd, fix->ref_fix_time, fix->schedule.iNCpn - 1,
              cpn->str[j], fix->ref_cpnd, SRT_PAYER, ref_delay, fix->ref_conv,
              mkt->vol_type, 0.0, 1, fix->schedule.lCpnDate[0],
              fix->ref_theo_end_date, mkt->cash_vol, fix->ref_fwd_spread,
              mkt->vc, mkt->num_strikes_in_vol, mkt->strikes_in_vol, &opt);
          ...
          if (cpn->str[j] - fix->ref_fwd_spread > 1.0E-08) {
            ...
            err = mkt->get_cash_vol_convert(
                mkt->get_cash_vol, mkt->vc, fix->schedule.lCpnDate[0],
                fix->ref_theo_end_date, fix->ref_fix_time, ref_swp_cash,
                cpn->str[j] - fix->ref_fwd_spread, mkt->ref, SRT_LOGNORMAL,
                &ref_strikelogvol);
            ...
            opt = srt_f_optblksch(ref_swp_drs - fix->ref_fwd_spread,
                                  ...
                                  ref_strikelogvol, fix->ref_fix_time, 1.0,
                                  SRT_CALL, SRT_PREMIUM);
          } else {
        ...
        if (cpn->type == 0 || cpn->type == 3 || cpn->type == 4) {
          if (cpn->type == 3) {
            ...
            if (use_cmsopt) {
              ...
              err = swp_f_Cms_Option(
                  ref_swp_fwd, fix->ref_fix_time, fix->schedule.iNCpn - 1,
                  cpn->str[j] * 1.01, fix->ref_cpnd, SRT_PAYER, ref_delay,
                  fix->ref_conv, mkt->vol_type, 0.0, 1,
                  fix->schedule.lCpnDate[0], fix->ref_theo_end_date,
                  mkt->cash_vol, fix->ref_fwd_spread, mkt->vc,
                  mkt->num_strikes_in_vol, mkt->strikes_in_vol, &delta);
              ...
              if (cpn->str[j] - fix->ref_fwd_spread > 1.0E-08) {
                ...
                delta = srt_f_optblksch(ref_swp_drs - fix->ref_fwd_spread,
                                        cpn->str[j] - fix->ref_fwd_spread,
                                        ref_strikelogvol, fix->ref_fix_time,
                                        1.0, SRT_CALL, DELTA);
              } else {
                delta = 1.0;
              }
    ...
    if (cpn->type == 1 || cpn->type == 2) {
      ...
      if (float_adjust_type == 0 || float_adjust_type == 2) {
        ...
        err = mkt->get_cash_vol_convert(
            mkt->get_cash_vol, mkt->vc, fix->schedule.lCpnDate[0],
            fix->ref_theo_end_date, fix->ref_fix_time, ref_swp_cash,
            ref_swp_cash, mkt->ref, SRT_LOGNORMAL, &ref_atmlogvol);
      ...
      for (j = 0; j < cpn->nstr; j++) {
        ...
        if (j % 2 == 0) {
          ...
          case 2: {
            ...
            err = mkt->get_cash_vol_convert(
                mkt->get_cash_vol, mkt->vc, cpn->pay_start_date,
                cpn->pay_theo_end_date, cpn->pay_fix_time, pay_swp_cash,
                pay_new_strike - cpn->pay_fwd_spread, mkt->ref, SRT_LOGNORMAL,
                &pay_adjlogvol);
          ...
          case 3: {
            ...
            err = mkt->get_cash_vol_convert(
                mkt->get_cash_vol, mkt->vc, cpn->pay_start_date,
                cpn->pay_theo_end_date, cpn->pay_fix_time, pay_swp_cash,
                cpn->str[j] - cpn->pay_fwd_spread, mkt->ref, SRT_LOGNORMAL,
                &pay_adjlogvol);
          ...
          if (cpn->type == 1) {
            ref_adj = exp(ref_correl * ref_adjlogvol * pay_adjlogvol *
                          cpn->pay_fix_time);
          } else {
            ref_adj = exp(ref_correl * ref_adjlogvol * pay_adjlogvol *
                          fix->ref_fix_time);
...
Err cts_calc_equivalent_strikes(CTS cts_iv, CTS cts_call, CTS_MKT mkt) {
  ...
  last_df = swp_f_df(mkt->today, cts_iv->fund_leg->cpn[last_fund - 1].pay_date,
                     mkt->yc);
  end_exo = cts_iv->exo_leg->cpn[last_exo - 1].cxxpn_pay_date;
  ...
  for (i = cts_call->num_calls - 1; i >= 00; i--) {
    ...
    swap_pv = exo_pv / cts_iv->exo_leg->cpn[last_exo].cxxpn_not -
              fund_pv / cts_iv->fund_leg->cpn[last_fund].not ;

    err = swp_f_LevelPayment(cts_iv->exo_leg->cpn[last_exo].cxxpn_start_date,
                             end_exo, mkt->swap_freq, mkt->swap_basis, mkt->yc,
                             mkt->ref, &level_exo);
...
Err cts_fill_cpn_leg(
  ...
  for (i = 0; i < exo_leg->num_cpn; i++) {
    err = cts_init_cpn(
        mkt, cpn_start_date[j + i], cpn_end_date[j + i], cpn_pay_date[j + i],
        cpn_coupon[j + i], cpn_basis[j + i], cpn_not[j + i], fix_lag_bd,
        cpn_type[j + i], pay_fix_date[j + i], pay_months, pay_freq, pay_basis,
        pay_fwd_spread[j + i], pay_gearing[j + i], nfix[j + i], weights[j + i],
        ref_fix_dates[j + i], fix_tenor[j + i], fix_freq, fix_basis,
        ref_fwd_spreads[j + i], tstype, alpha[j + i], beta[j + i], nstr[j + i],
        str[j + i], nbopt[j + i], buy_sell, value_zero, lb[j + i], ub[j + i],
        payoff[j + i], call_spread, numer_spread, trim_type, max_fix,
        min_fix_time, calc_mkt_iv, use_cmsopt, correl_start, correl_end,
        float_adjust_type, &(exo_leg->cpn[i]));
...
Err cts_fill_calls(
  ...
  while (i < ncall) {
    ...
    if (j > 0) {
      (call - 1)->num_partial_fund_cpn =
          (call - 1)->num_fund_cpn - call->num_fund_cpn;
    }

    /*	Call on exotic leg */
    /*	k = index of the first coupon to be called on exotic leg        ,
                    i.e. first coupon with a start date >= ex date */
    while (l < exo_leg->num_cpn &&
           exo_leg->cpn[l].cxxpn_start_date < call->ex_date) {
      l++;
    }
    ...
    if (j > 0) {
      (call - 1)->num_partial_exo_cpn =
          (call - 1)->num_exo_cpn - call->num_exo_cpn;
    }
...
Err cts_check_calls(CTS cts) {
  ...
  for (i = 1; i < cts->num_calls; i++) {
    ...
    /* It returns an error for now by safety        , later it will be handled
     * properly */
    if (cts->call[i].exo_idx > 0) {
      if (cts->exo_leg->cpn[cts->call[i].exo_idx - 1].used_nfix > 0 &&
          cts->exo_leg->cpn[cts->call[i].exo_idx - 1]
                  .fix[cts->exo_leg->cpn[cts->call[i].exo_idx - 1].used_nfix -
                       1]
                  .ref_fix_date > cts->call[i].ex_date) {
        return serror(
            "Either too many fixings requested        , or Floating Rate "
            "fixe after call date");
      }
    }
  }
  ...
  for (i = 0; i < cts->num_calls; i++) {
    ...
    if (cts->fund_leg->cpn[cts->call[i].fund_idx].start_date <
            cts->call[i].ex_date ||
        cts->fund_leg->cpn[cts->call[i].fund_idx].pay_date <
            cts->call[i].ex_date) {
      return "A funding coupon starts before its exercise date";
    }

    if (cts->exo_leg->cpn[cts->call[i].exo_idx].cxxpn_start_date <
            cts->call[i].ex_date ||
        cts->exo_leg->cpn[cts->call[i].exo_idx].cxxpn_pay_date <
            cts->call[i].ex_date ||
        cts->exo_leg->cpn[cts->call[i].exo_idx].fix[0].ref_fix_date <
            cts->call[i].ex_date) {
...
Err cts_get_mkt_sabr_beta_param(CTS_MKT mkt, long exe_date, long start_date,
  ...
  while (act_date >= start_date - 5) {
    ...
    theo_date =
        add_unit(theo_date, -nb_months, SRT_MONTH, NO_BUSDAY_CONVENTION);
    act_date = bus_date_method(theo_date, MODIFIED_SUCCEEDING);
  ...
  err = mkt->get_cash_vol_convert(mkt->get_cash_vol, mkt->vc, start_date,
                                  theo_end_date, maturity, swap_cash, swap_cash,
                                  mkt->ref, SRT_LOGNORMAL, &atm_log_vol);
  ...
  err = mkt->get_cash_vol_convert(mkt->get_cash_vol, mkt->vc, start_date,
                                  theo_end_date, maturity, swap_cash, swap_cash,
                                  mkt->ref, SRT_NORMAL, &atm_nor_std);
  ...
  for (i = 1; i < 6; i++) {
    err = mkt->get_cash_vol_convert(
        mkt->get_cash_vol, mkt->vc, start_date, theo_end_date, maturity,
        swap_cash, Strikes[i], mkt->ref, SRT_LOGNORMAL, &Vols[i]);
  ...
  if (use_levenberg) {
    err = opsabrcalib(swap_cash, maturity, 5, Strikes, Vols, &atm_log_vol,
                      alpha, 1, &fixed_beta, 0, rho, 1, fitting_error);
  } else {
    err = calib_sabr_rr_bt_given_beta(
        swap_cash, maturity, atm_log_vol, Strikes[1], Vols[1], Strikes[5],
        Vols[5], &sigma_beta, alpha, fixed_beta, rho, SRT_LOGNORMAL, 1, 0.00001,
        7, fitting_error);
  }
...
/* Implied Alpha^2 = Alpha0^2 * (1.0 - exp(-2.0 * LamEps * T)) / (2.0 * LamEps *
 * T) */
...
Err cts_calib_const_alpha_and_rho(
  ...
  if (calib_months == 0) {
    ...
    while (i0 < cts->exo_leg->num_cpn &&
           cts->exo_leg->cpn[cts->call[0].exo_idx + i0].cxxpn_start_time <
               MIN_CALIB_TIME) {
      i0++;
    }
    ...
    for (i = 0; i < num_ex_date; i++) {
      exercise_dates_[i] = cts->exo_leg->cpn[i0 + i].cxxpn_start_date;
      exercise_dates_[i] =
          add_unit(exercise_dates_[i], -2, SRT_BDAY, MODIFIED_SUCCEEDING);
    }
  }
  ...
  for (i = num_ex_date - 2; i >= 0; i--) {
    if ((exercise_dates[j] - exercise_dates[i]) * YEARS_IN_DAY <
        min_time - ONE_MONTH) {
      for (k = i - 1; k >= 0; k--) {
  ...
  if (!mkt_alpha || !mkt_rho || !exercise_times || !weights || !start_dates ||
      !end_dates) {
  ...
  for (i = 0; i < num_ex_date; i++) {
    ...
    if (calib_months == 0) {
      ...
      theo_end_date =
          add_unit(start_date, calib_months, SRT_MONTH, NO_BUSDAY_CONVENTION);
    }
    ...
    err = cts_get_mkt_sabr_beta_param(
        mkt, exercise_dates[i], start_date, theo_end_date, fixed_beta, 1,
        &mkt_alpha[i], &mkt_rho[i], &fitting_error);
  ...
  if (num_ex_date > 1) {
    ...
    *rho = temp / num_ex_date;
    ...
    err = levenberg_marquardt_select(exercise_times - 1, mkt_alpha - 1,
                                     weights - 1, num_ex_date, param, use_param,
                                     2, LGMSV_ALPHA_MAXITER,
                                     cts_implied_alpha_approx, &fitting_error);
  ...
  if (save_inst_data) {
    ...
    if (num_ex_date > 0) {
      inst_data->start_dates_smile =
          calloc(inst_data->num_inst_smile, sizeof(long));
      inst_data->end_dates_smile =
          calloc(inst_data->num_inst_smile, sizeof(long));
...
Err cts_calib_und(
    ...
    int calib_strategy, /*	0: autocal        , 1: swaptions / cap        ,
                     2: cap / swaptions */
  ...
  int used_nsmilepar;
  double *used_smilepartime = NULL, *used_alpha = NULL, *used_rho = NULL,
         *used_ldaeps = NULL, *used_rho2 = NULL;
  ...
  char *prim_ten, *prim_ref, *prim_freq, *prim_basis, *sec_ten, *sec_ref,
      *sec_freq, *sec_basis;
  ...
  if (!prim_param || !sec_param || !lm_params || !calib_params ||
      !used_smilepartime || !used_alpha || !used_rho || !used_ldaeps ||
      !used_rho2) {
  ...
  if (cts->num_calls == 0 ||
      (cts->num_calls == 1 && cts->call[0].ex_date <= today + eod_flag)) {
  ...
  skip_dates = 0;
  while (skip_dates < cts->num_calls &&
         cts->call[skip_dates].ex_time < MIN_CALIB_TIME) {
    skip_dates++;
  }
  ...
  if (!cal_dates || !ex_dates || !prim_strikes || !sec_strikes || !prim_tenor ||
      !sec_tenor || !prim_strikesS1 || !prim_strikesS2 || !sec_strikesS1 ||
      !sec_strikesS2) {
    err = "Memory allocation faillure in cts_calib_und";
    goto FREE_RETURN;
  }
  ...
  case 2: {
    ...
    if (smile_calib_months < 0) {
      smile_calib_months =
          (int)((temp_date - mkt->today) * YEARS_IN_DAY * 12.0 + 0.5);
    }
  ...
  if (short_strike_flag == 1 && force_atm == 0) {
    for (i = 0; i < nb_cal_dates; i++) {
      sec_strikes[i] =
          cts->exo_leg->cpn[cts->call[i + skip_dates].exo_idx].cxxaplet_strike;
    }
  }

  if (cts->exo_leg->num_cpn > 0) {
    end_date = cts->exo_leg->cpn[cts->exo_leg->num_cpn - 1].cxxpn_pay_date;
  } else {
    end_date = 0;
  }
  ...
  if (!fix_smile && calib_params->use_sabr_calib) {
    err = cts_calib_const_alpha_and_rho(
        mkt, cts, smile_calib_months, LGMSV_SABRBETA, min_time, &cal_alpha,
        &cal_rho, &cal_lameps, save_inst_data, inst_data);
  ...
  err = cpd_calib_diagonal_LGMSV_new_dlm(
      mkt->yc, mkt->vc, mkt->get_cash_vol, mkt->ref,

      prim_freq, prim_basis, prim_ref, nb_cal_dates, ex_dates, cal_dates,
      prim_tenor, end_date, prim_strikes, prim_strikesS1, prim_strikesS2,
      prim_param,

      sec_freq, sec_basis, sec_ref, nb_cal_dates, ex_dates, cal_dates,
      sec_tenor, end_date, sec_strikes, sec_strikesS1, sec_strikesS2, NULL,
      sec_param,

      calib_params,

      nb_factor, &lambda, used_nsmilepar, used_smilepartime, used_alpha,
      used_ldaeps, used_rho, tstar, lgm_alpha, lgm_gamma, lgm_rho, used_rho2,

      NumerParams,

      &(und->model.iNbPWTime), &(und->model.dPWTime), &(und->model.dSigma),
      &(und->model.dAlpha), &(und->model.dLambdaEps), &(und->model.dRho),
      &(und->model.dRho2),
  ...
  und->model.lTStarDate =
      (long)(und->model.lToday + und->model.dTStar * DAYS_IN_YEAR + 1.0E-08);
  ...
  und->model.dSigma = (double *)realloc(und->model.dSigma,
                                        und->model.iNbPWTime * sizeof(double));
  und->model.dAlpha = (double *)realloc(und->model.dAlpha,
                                        und->model.iNbPWTime * sizeof(double));
  und->model.dRho =
      (double *)realloc(und->model.dRho, und->model.iNbPWTime * sizeof(double));
  und->model.dLambdaEps = (double *)realloc(
      und->model.dLambdaEps, und->model.iNbPWTime * sizeof(double));
  ...
  if (und->model.iOne2F == 2) {
    und->model.dRho2 = (double *)realloc(und->model.dRho2,
                                         und->model.iNbPWTime * sizeof(double));
    und->model.dLGMAlpha =
  ...
  memcpy(und->model.dLvlEps, und->model.dLambdaEps,
         und->model.iNbPWTime * sizeof(double));

  ConvertTS_LGM_to_LGMSV(
      und->model.iNbPWTime, und->model.dPWTime, und->model.dSigma,
      und->model.dLambdaX, und->model.dTStar, und->model.iOne2F,
      und->model.dInitLGMAlpha, und->model.dLGMGamma, und->model.dInitLGMRho,
      und->model.dLGMAlpha, und->model.dLGMRho);
...
Err cts_copy_und(CTS_UND src, CTS_UND dest) {
  ...
  if (dest && src) {
    ...
    err = init_LGMSV_model(
        &(dest->model), src->model.lToday, src->model.iOne2F,
        src->model.iNbPWTime, src->model.dLambdaX, src->model.dPWTime,
        src->model.dSigma, src->model.dAlpha, src->model.dLambdaEps,
        src->model.dLvlEps, src->model.dRho, src->model.dTStar,
        src->model.dInitLGMAlpha, src->model.dLGMGamma, src->model.dInitLGMRho,
        src->model.dRho2);
...
void cts_free_extra_infos(CTS_EXTRA_INFOS extra_infos) {
  if (extra_infos) {
    if (extra_infos->extra_infos && extra_infos->nrow > 0 &&
        extra_infos->ncol > 0) {
      free_dmatrix(extra_infos->extra_infos, 0, extra_infos->nrow - 1, 0,
                   extra_infos->ncol - 1);
...
Err cts_calc_model_fwd_iv(CTS_UND und, CTS cts, int calc_fwd_iv, int adj_fee,
  ...
  double InstPrices_[10], InstPrices2_[10], InstPrices_temp_[10],
      InstStrikes_[10], InstStds_[10],
      *InstPrices = &(InstPrices_[0]), *InstPrices2 = &(InstPrices2_[0]),
      *InstPrices_temp = &(InstPrices_temp_[0]),
      *InstStrikes = &(InstStrikes_[0]), *InstStds = &(InstStds_[0]);
  ...
  if (!sPricingConst || !sNumerParams || !cCalibInst || !cHestonInst ||
      !cCpnSchedule) {
  ...
  if (calc_fwd_iv) {
    ...
    for (i = first_coupon; i < cts->exo_leg->num_cpn; i++) {
      ...
      if (exo_cpn->type == 0 || exo_cpn->type == 3 || exo_cpn->type == 4) {
        ...
        cCpnSchedule->dCpnTime[0] =
            (cCpnSchedule->lCpnDate[0] - today) * YEARS_IN_DAY;
        cCpnSchedule->dCpnTime[1] =
            (cCpnSchedule->lCpnDate[1] - today) * YEARS_IN_DAY;
        cCpnSchedule->dCpnCvg[1] = exo_cpn->pay_cvg;
        cCpnSchedule->dCpnDf[0] =
            swp_f_df(today, cCpnSchedule->lCpnDate[0], mkt->yc);
        cCpnSchedule->dCpnDf[1] =
            swp_f_df(today, cCpnSchedule->lCpnDate[1], mkt->yc);
        ...
        cCalibInst->dFwdCash =
            (cCpnSchedule->dCpnDf[0] - cCpnSchedule->dCpnDf[1]) /
            cCalibInst->dLevel;
        ...
        err = Initialise_NumerInst(&(und->model), cCalibInst, cHestonInst,
                                   sNumerParams);
        ...
        LGMSVFwdClosedFormApprox_struct(&(und->model), cCalibInst, cHestonInst,
                                        sPricingConst, sNumerParams,
                                        &dLiborFwd);
      ...
      for (j = 0; j < exo_cpn->used_nfix; j++) {
        ...
        if (cCalibInst->iIsLiborOption) {
          ...
          sNumerParams->iSwitchIndex =
              Get_Index(sNumerParams->dNewSwitchTime, und->model.dPWTime,
                        und->model.iNbPWTime);
          ...
          if (cCalibInst->dExeTime < sNumerParams->dNewSwitchTime) {
            ...
            if (fabs(sNumerParams->dNewSwitchTime -
                     und->model.dPWTime[sNumerParams->iSwitchIndex]) >
                    1.0E-08 ||
                sNumerParams->iSwitchIndex == sNumerParams->iEndIndex) {
              /* we add the switch */
        ...
        if (cCalibInst->iNbStrike > 0) {
          LGMSVClosedFormApprox_struct(&(und->model), cCalibInst, cHestonInst,
                                       sPricingConst, sNumerParams,
                                       InstPrices_temp);
        }
        ...
        if (do_min_strike) {
          err = srt_f_optimpvol(InstPrices_temp[0], dLiborRefFwd + 100,
                                cCalibInst->dStrike[0] + 100,
                                cCalibInst->dExeTime, df_pay, SRT_CALL,
                                SRT_NORMAL, &min_vol);
          ...
          for (k = 0; k < first_strike; k++) {
            InstPrices[k] = srt_f_optblknrm(dLiborRefFwd, exo_cpn->str[k],
                                            min_vol, cCalibInst->dExeTime,
                                            df_pay, SRT_CALL, SRT_PREMIUM);
          }
        }
        ...
        if (do_max_strike) {
          err = srt_f_optimpvol(
              InstPrices_temp[nb_strike - 1], dLiborRefFwd + 100,
              cCalibInst->dStrike[nb_strike - 1] + 100, cCalibInst->dExeTime,
              df_pay, SRT_CALL, SRT_NORMAL, &max_vol);
        ...
        if (cCalibInst->iIsLiborOption) {
          ...
          if (do_min_strike) {
            ...
            for (k = 0; k < first_strike; k++) {
              InstPrices2[k] = srt_f_optblknrm(dLiborRefFwd, exo_cpn->str[k],
                                               min_vol, cCalibInst->dExeTime,
                                               df_pay, SRT_CALL, SRT_PREMIUM);
            }
          }
          ...
          fixing_iv += exo_cpn->alpha * dLiborFwd +
                       exo_cpn->beta * InstPrices[exo_cpn->nstr];
...
Err cts_adjust_model_fwd_iv(CTS_UND und, CTS market_cts, CTS model_cts,
  ...
  if (calc_fwd_iv) {
    ...
    if (save_fwdiv) {
      ...
      fwd_iv_info->call_date =
          (long *)calloc(model_cts->num_calls, sizeof(long));
      fwd_iv_info->market_fwdiv =
          (double *)calloc(model_cts->num_calls, sizeof(double));
      fwd_iv_info->model_fwdiv =
          (double *)calloc(model_cts->num_calls, sizeof(double));
      fwd_iv_info->extra_fee =
          (double *)calloc(model_cts->num_calls, sizeof(double));
    ...
    for (i = 0; i < model_cts->num_calls; i++) {
      ...
      for (j = model_cts->call[i].exo_idx; j < model_cts->exo_leg->num_cpn;
           j++) {
      ...
      for (j = model_cts->call[i].fund_idx; j < model_cts->fund_leg->num_cpn;
           j++) {
      ...
      if (save_fwdiv) {
        ...
        fwd_iv_info->market_fwdiv[i] =
            model_cts->call[i].pay_rec * (mkt_exo_iv - fund_iv);
        fwd_iv_info->model_fwdiv[i] =
            model_cts->call[i].pay_rec * (mdl_exo_iv - fund_iv);
    ...
    if (pde_or_mc == 1) {
      ...
      for (i = 0; i < model_cts->num_calls - 1; i++) {
        model_cts->call[i].total_fee -=
            ...
            swp_f_df(model_cts->call[i].set_date,
                     model_cts->call[i + 1].set_date, und->mkt->yc);
      }
    }
  } else {
    for (i = 0; i < model_cts->num_calls; i++) {
...
Err cts_fill_eval_const_fix(CTS_UND und, CTS cts,
  ...
  eval_const_fix->tpay_tstar_alpha =
      swp_f_df(today, cpn->cpn_pay_date, und->mkt->yc) /
  ...
  eval_const_fix->tpay_tstar_beta =
      (1.0 -
       exp(-und->model.dLambdaX * (cpn->cpn_pay_time - und->model.dTStar))) /
      und->model.dLambdaX;
  ...
  eval_const_fix->ts_te_alpha =
      swp_f_df(today, fix->schedule.lCpnDate[0], und->mkt->yc) /
      swp_f_df(today, fix->schedule.lCpnDate[1], und->mkt->yc);
  eval_const_fix->ts_te_alpha = -log(eval_const_fix->ts_te_alpha);

  beta1 = (1.0 - exp(-und->model.dLambdaX *
                     (fix->schedule.dCpnTime[0] - und->model.dTStar))) /
          und->model.dLambdaX;
  beta2 = (1.0 - exp(-und->model.dLambdaX *
                     (fix->schedule.dCpnTime[1] - und->model.dTStar))) /
          und->model.dLambdaX;

  eval_const_fix->ts_te_beta = beta1 - beta2;
  eval_const_fix->ts_te_gamma =
      0.5 * eval_const_fix->ts_te_beta * (beta1 + beta2);

  if (und->model.iOne2F == 2) {
    /* Fill the Extras */
    eval_const_fix->tpay_tstar_beta2 =
        (1.0 -
         exp(-und->model.dLambdaX2 * (cpn->cpn_pay_time - und->model.dTStar))) /
        und->model.dLambdaX2;
    eval_const_fix->tpay_tstar_gamma2 = 0.5 * eval_const_fix->tpay_tstar_beta2 *
                                        eval_const_fix->tpay_tstar_beta2;
    eval_const_fix->tpay_tstar_gamma12 =
        eval_const_fix->tpay_tstar_beta * eval_const_fix->tpay_tstar_beta2;

    beta1_2 = (1.0 - exp(-und->model.dLambdaX2 *
                         (fix->schedule.dCpnTime[0] - und->model.dTStar))) /
              und->model.dLambdaX2;
    beta2_2 = (1.0 - exp(-und->model.dLambdaX2 *
                         (fix->schedule.dCpnTime[1] - und->model.dTStar))) /
              und->model.dLambdaX2;

    eval_const_fix->ts_te_beta2 = beta1_2 - beta2_2;
    eval_const_fix->ts_te_gamma2 =
        0.5 * eval_const_fix->ts_te_beta2 * (beta1_2 + beta2_2);
  ...
  /*	Precalculations */
  eval_const_fix->a = cpn->alpha + cpn->beta * (fix->ref_fwd_spread -
                                                1.0 / fix->schedule.dCpnCvg[1]);
  eval_const_fix->b = cpn->beta / fix->schedule.dCpnCvg[1];

  if (cpn->type == 0) {
    /* we include the coupon in the fixings */
    temp = cpn->used_fixweights[fix_idx] * cpn->cpn_coupon * cpn->cpn_cvg *
           cpn->cpn_not;
  } else {
  ...
  for (i = 0; i < cpn->nstr; i++) {
    eval_const_fix->s[i] =
        (cpn->str[i] - fix->ref_fwd_spread) * fix->schedule.dCpnCvg[1] + 1.0;
...
Err cts_fill_eval_const_fix_cms(CTS_UND und, CTS cts,
  ...
  eval_const_fix_cms->tpay_tstar_alpha =
      -log(eval_const_fix_cms->tpay_tstar_alpha);
  eval_const_fix_cms->tpay_tstar_beta =
      (1.0 -
       exp(-und->model.dLambdaX * (cpn->cpn_pay_time - und->model.dTStar))) /
      und->model.dLambdaX;
  eval_const_fix_cms->tpay_tstar_gamma = 0.5 *
                                         eval_const_fix_cms->tpay_tstar_beta *
                                         eval_const_fix_cms->tpay_tstar_beta;
  ...
  for (i = 1; i < fix->schedule.iNCpn - 1; i++) {
    eval_const_fix_cms->ti_tstar_alpha[i] =
        fix->schedule.dCpnCvg[i] *
        swp_f_df(today, fix->schedule.lCpnDate[i], und->mkt->yc) / df_tstar;
    eval_const_fix_cms->ti_tstar_alpha[i] =
        -log(eval_const_fix_cms->ti_tstar_alpha[i]);
    eval_const_fix_cms->ti_tstar_beta[i] =
        (1.0 - exp(-und->model.dLambdaX *
                   (fix->schedule.dCpnTime[i] - und->model.dTStar))) /
        und->model.dLambdaX;
    eval_const_fix_cms->ti_tstar_gamma[i] =
        0.5 * eval_const_fix_cms->ti_tstar_beta[i] *
        eval_const_fix_cms->ti_tstar_beta[i];
  }
  ...
  if (fix->schedule.iNCpn > 0) {
    ...
    eval_const_fix_cms->ti_tstar_beta[fix->schedule.iNCpn - 1] =
        (1.0 - exp(-und->model.dLambdaX *
                   (fix->schedule.dCpnTime[fix->schedule.iNCpn - 1] -
                    und->model.dTStar))) /
        und->model.dLambdaX;
  ...
  if (und->model.iOne2F == 2) {
    ...
    eval_const_fix_cms->tpay_tstar_gamma2 =
        0.5 * eval_const_fix_cms->tpay_tstar_beta2 *
        eval_const_fix_cms->tpay_tstar_beta2;
    ...
    for (i = 1; i < fix->schedule.iNCpn - 1; i++) {
      ...
      eval_const_fix_cms->ti_tstar_gamma2[i] =
          0.5 * eval_const_fix_cms->ti_tstar_beta2[i] *
          eval_const_fix_cms->ti_tstar_beta2[i];
    ...
    if (fix->schedule.iNCpn > 0) {
      ...
      eval_const_fix_cms->ti_tstar_gamma2[0] =
          0.5 * eval_const_fix_cms->ti_tstar_beta2[0] *
          eval_const_fix_cms->ti_tstar_beta2[0];
...
Err cts_fill_eval_const_cpn(CTS_UND und, CTS cts,
  ...
  /* reconstruction of DF(t        ,Tpay) / DF(t        ,Tstar) */
  eval_const_cpn->tpay_tstar_alpha =
      swp_f_df(today, cpn->cpn_pay_date, und->mkt->yc) /
      swp_f_df(today, und->model.lTStarDate, und->mkt->yc);
  ...
  /* reconstruction of DF(t        ,Ts) / DF(t        ,Te) */
  eval_const_cpn->ts_te_alpha =
      swp_f_df(today, cpn->pay_start_date, und->mkt->yc) /
      swp_f_df(today, cpn->pay_end_date, und->mkt->yc);
  eval_const_cpn->ts_te_alpha = -log(eval_const_cpn->ts_te_alpha);

  beta1 = (1.0 - exp(-und->model.dLambdaX *
                     (cpn->pay_start_time - und->model.dTStar))) /
          und->model.dLambdaX;
  beta2 = (1.0 - exp(-und->model.dLambdaX *
                     (cpn->pay_end_time - und->model.dTStar))) /
          und->model.dLambdaX;

  eval_const_cpn->ts_te_beta = beta1 - beta2;
  eval_const_cpn->ts_te_gamma =
      0.5 * eval_const_cpn->ts_te_beta * (beta1 + beta2);
  ...
  if (und->model.iOne2F == 2) {
    ...
    eval_const_cpn->tpay_tstar_beta2 =
        (1.0 -
         exp(-und->model.dLambdaX2 * (cpn->cpn_pay_time - und->model.dTStar))) /
        und->model.dLambdaX2;
    eval_const_cpn->tpay_tstar_gamma2 = 0.5 * eval_const_cpn->tpay_tstar_beta2 *
                                        eval_const_cpn->tpay_tstar_beta2;
    ...
    eval_const_cpn->ts_te_gamma2 =
        0.5 * eval_const_cpn->ts_te_beta2 * (beta1_2 + beta2_2);
  ...
  eval_const_cpn->shift =
      cpn->pay_gearing * (-1.0 / cpn->pay_cvg + cpn->pay_fwd_spread) +
      cpn->cpn_coupon;
...
Err cts_fill_eval_const_call(CTS_UND und, CTS cts,
  ...
  for (i = 0; i < call->num_fund_cpn; i++) {
    ...
    eval_const_call->tpay_tstar_alpha[i] =
        swp_f_df(today, cpn->pay_date, und->mkt->yc) /
        swp_f_df(today, und->model.lTStarDate, und->mkt->yc);
    eval_const_call->tpay_tstar_alpha[i] =
        -log(eval_const_call->tpay_tstar_alpha[i]);
    eval_const_call->tpay_tstar_beta[i] =
        (1.0 -
         exp(-und->model.dLambdaX * (cpn->pay_time - und->model.dTStar))) /
        und->model.dLambdaX;
    eval_const_call->tpay_tstar_gamma[i] = 0.5 *
                                           eval_const_call->tpay_tstar_beta[i] *
                                           eval_const_call->tpay_tstar_beta[i];
  }
  ...
  i = call->num_fund_cpn;
  eval_const_call->tpay_tstar_alpha[i] =
      swp_f_df(today, cpn->start_date, und->mkt->yc) /
      swp_f_df(today, und->model.lTStarDate, und->mkt->yc);
  eval_const_call->tpay_tstar_alpha[i] =
      -log(eval_const_call->tpay_tstar_alpha[i]);
  eval_const_call->tpay_tstar_beta[i] =
      (1.0 -
       exp(-und->model.dLambdaX * (cpn->start_time - und->model.dTStar))) /
      und->model.dLambdaX;
  eval_const_call->tpay_tstar_gamma[i] = 0.5 *
                                         eval_const_call->tpay_tstar_beta[i] *
                                         eval_const_call->tpay_tstar_beta[i];
  ...
  eval_const_call->tset_tstar_alpha =
      swp_f_df(today, call->set_date, und->mkt->yc) /
      swp_f_df(today, und->model.lTStarDate, und->mkt->yc);
  ...
  if (call->is_midat) {
    for (i = 0; i < call->num_exo_cpn; i++) {
      ...
      eval_const_call->tpay_tstar_alpha2[i] =
          -log(eval_const_call->tpay_tstar_alpha2[i]);
      eval_const_call->tpay_tstar_beta2[i] =
          (1.0 - exp(-und->model.dLambdaX *
                     (exo_cpn->cpn_pay_time - und->model.dTStar))) /
          und->model.dLambdaX;
      eval_const_call->tpay_tstar_gamma2[i] =
          0.5 * eval_const_call->tpay_tstar_beta2[i] *
          eval_const_call->tpay_tstar_beta2[i];
    }
  }
  ...
  if (und->model.iOne2F == 2) {
    ...
    for (i = 0; i < call->num_fund_cpn; i++) {
      ...
      eval_const_call->tpay_tstar_beta_2[i] =
          (1.0 -
           exp(-und->model.dLambdaX2 * (cpn->pay_time - und->model.dTStar))) /
          und->model.dLambdaX2;
      eval_const_call->tpay_tstar_gamma_2[i] =
          0.5 * eval_const_call->tpay_tstar_beta_2[i] *
          eval_const_call->tpay_tstar_beta_2[i];
    ...
    eval_const_call->tpay_tstar_beta_2[i] =
        (1.0 -
         exp(-und->model.dLambdaX2 * (cpn->start_time - und->model.dTStar))) /
        und->model.dLambdaX2;
    eval_const_call->tpay_tstar_gamma_2[i] =
        0.5 * eval_const_call->tpay_tstar_beta_2[i] *
        eval_const_call->tpay_tstar_beta_2[i];
    ...
    eval_const_call->tset_tstar_beta2 =
        (1.0 -
         exp(-und->model.dLambdaX2 * (call->set_time - und->model.dTStar))) /
        und->model.dLambdaX2;
    eval_const_call->tset_tstar_gamma2 = 0.5 *
                                         eval_const_call->tset_tstar_beta2 *
                                         eval_const_call->tset_tstar_beta2;
      ...
      for (i = 0; i < call->num_exo_cpn; i++) {
        ...
        eval_const_call->tpay_tstar_beta2_2[i] =
            (1.0 - exp(-und->model.dLambdaX2 *
                       (exo_cpn->cpn_pay_time - und->model.dTStar))) /
            und->model.dLambdaX2;
        eval_const_call->tpay_tstar_gamma2_2[i] =
            0.5 * eval_const_call->tpay_tstar_beta2_2[i] *
            eval_const_call->tpay_tstar_beta2_2[i];
...
Err cts_find_one_time_index(CTS_UND und, CTS cts, int *one_time_index) {
  ...
  if (*one_time_index == 0) {
    exe_mat = cts->exo_leg->cpn[cts->exo_leg->num_cpn - 1].cxxpn_pay_time / 4.0;
...
Err cts_fill_algo_arg(
  ...
  if (pde_or_mc == 0) {
    ...
    for (i = 0; i < adi_arg->nstp - 1; i++) {
      ...
      nstp +=
          max((int)((times[i + 1] - times[i]) / adi_arg->mc_mintime + 0.5), 1);
    }
    ...
    if (!adi_arg->time || !adi_arg->optimise || !adi_arg->mcebparams ||
        ...
        !adi_arg->dLvlEps || !adi_arg->dRho || !adi_arg->dRho2 ||
        !adi_arg->dLGMAlpha || !adi_arg->dLGMRho) {
    ...
    for (i = 0; i < adi_arg->nstp; i++) {
      index =
          Get_Index(adi_arg->time[i], und->model.dPWTime, und->model.iNbPWTime);
  ...
  for (i = 0; i < adi_arg->nstp - 1; i++) {
    adi_arg->ifr[i] =
        swp_f_zr(adi_arg->date[i], adi_arg->date[i + 1], und->mkt->yc);
  }
  ...
  for (i = 0; i < adi_arg->nstp; i++) {
    ...
    if (index_call < cts->num_calls &&
        fabs(adi_arg->date[i] - call->ex_date) < 1.0e-08) {
      /* this is a call date */
      cts_prm->und = und;
      cts_prm->cts = cts;
      cts_prm->call = call;
      ...
      err = cts_fill_eval_const_call(und, cts, index_call,
    ...
    if (index_cpn < cts->exo_leg->num_cpn &&
        fabs(adi_arg->date[i] - cts->exo_leg->cpn[index_cpn].pay_fix_date) <
            1.0e-08 &&
        is_midat == 0) {
      /* this is a cpn date */
      ...
      if (cpn->type == 1 || cpn->type == 2) {
        ...
        err = cts_fill_eval_const_cpn(und, cts, index_cpn,
    ...
    if (index_cpn_for_fix < cts->exo_leg->num_cpn &&
        ...
        fabs(adi_arg->date[i] - cpn_for_fix->fix[index_fix].ref_fix_date) <
            1.0e-08 &&
        is_midat == 0) {
      ...
      if (cts_prm->fix->schedule.iNCpn == 2) {
        ...
        err = cts_fill_eval_const_fix(und, cts, index_cpn_for_fix, index_fix,
                                      &(cts_prm->eval_const_fix));
      } else {
        cts_prm->is_fixing_cms = 1;

        err =
            cts_fill_eval_const_fix_cms(und, cts, index_cpn_for_fix, index_fix,
  ...
  if (index_call < cts->num_calls ||
      (index_cpn_for_fix < cts->exo_leg->num_cpn && is_midat == 0)) {
...
void cts_get_numer_tstar(long today, CTS cts, double *numer_tstar) {
  ...
  if (fabs(*numer_tstar) < 1.0E-08) {
    temp_numer_tstar =
        0.5 * (cts->exo_leg->cpn[cts->call[0].exo_idx].cxxpn_start_time +
               cts->exo_leg->cpn[cts->call[cts->num_calls - 1].exo_idx]
                   .cxxpn_pay_time);
...
Err cts_fill_check_all_struct(
    ...
    double vol_shift_short, DIAGCALIB_VOLTYPE vol_type_short,
    DIAGCALIB_SHIFTTYPE vol_shift_type_short, double lambda_shift,
    int calib_strategy, /*	-1: autocal        , 0: swaptions / cap        ,
                     1: cap / swaptions */
  ...
  if (!just_recalib) {
    ...
    err = cts_fill_fund_leg(mkt, eod_fix_flag, fund_ncpn, fund_not, fund_fix,
                            fund_start, fund_end, fund_pay, fund_basis,
                            fund_spr, fund_mrg, cts->fund_leg);
    ...
    err = cts_fill_cpn_leg(
        mkt, eod_fix_flag, fix_lag_bd, ncpn, cpn_type, cpn_start_date,
        cpn_end_date, cpn_pay_date, cpn_coupon, cpn_basis, cpn_not,
        pay_fix_date, pay_months, pay_freq, pay_basis, pay_fwd_spread,
        pay_gearing, nfix, weights, fix_dates, fix_tenor, fix_freq, fix_basis,
        fix_fwd_spreads, tstype, alpha, beta, nstr, str, nbopt, buy_sell,
        value_zero, lb, ub, payoff, call_spread, numer_spread, trim_type,
        max_fix, min_fix_time, calc_mkt_iv, use_cmsopt, correl_start,
        correl_end, float_adjust_type, cts->exo_leg);
    ...
    if (ncall > 0 && ex_date[ncall - 1] >= mkt->today + eod_ex_flag) {
      err = cts_fill_calls(mkt->today, eod_ex_flag, ncall, pay_rec, ex_date,
                           set_date, fee, cts);
  ...
  if (cts->num_calls > 0 && cts->call) {
    if (use_calib) {
      err = cts_calib_und(
          mkt, eod_ex_flag, lambda, nb_factor, lgm_alpha, lgm_gamma, lgm_rho,
          nsmilepar, smilepartime, alphaeps, rhoeps, ldaeps, rho2eps, tstar,
          NumerParams, cal_tenor, cal_ref, cal_freq, cal_basis, force_atm,
          max_std_long, max_std_short, vol_shift_long, vol_type_long,
          ...
          max_fact, use_jumps, prec, maxiter, keep_first, long_strike_flag,
          short_strike_flag, cts, und, save_inst_data, inst_data);
    } else {
  ...
  if (cts->num_calls > 0 && cts->call) {
    ...
    err = cts_fill_algo_arg(und, cts, pde_or_mc, req_stp, req_stppsi, req_stpx,
                            req_stpz, req_mintime, req_paths, integ_mintime,
                            do_one_time, one_time_index, adi_arg);
...
/*	Free all structures */
void cts_free_all_struct(CTS cts, CTS_UND und, int call_feat,
                         CTS_ADI_ARG adi_arg) {
  cts_free_und(und);
  cts_free_calls(cts);
...
Err cts_payoff_4_lgmsv_adi(
  ...
  double DFR_start, DFR_end, DFR_fix, DFR_pay, DFR_set, coupon, libor, funding,
      exotic, iv, fee;
  double cms;
    ...
    if (call->is_midat) {
      for (i = lz; i <= uz; i++) {
        ...
        for (j = lx; j <= ux; j++) {
          ...
          for (k = lpsi; k <= upsi; k++) {
            DFR_set = exp(-eval_const_call->tset_tstar_alpha -
            ...
            for (l = 0; l < call->num_fund_cpn; l++) {
              DFR_pay = exp(-eval_const_call->tpay_tstar_alpha[l] -
            ...
            for (l = 0; l < call->num_exo_cpn; l++) {
              DFR_pay = exp(-eval_const_call->tpay_tstar_alpha2[l] -
            ...
            prod_val[i][j][k][0] =
                max(prod_val[i][j][k][0], call->pay_rec * iv - fee);

            /* Value the one time call if required */
            if (eval_one_time) {
              prod_val[i][j][k][one_time_col] =
                  max(0.0, call->pay_rec * iv - fee);
            }
          }
        }
      }
      ...
      for (i = lz; i <= uz; i++) {
        ...
        for (j = lx; j <= ux; j++) {
          ...
          for (k = lpsi; k <= upsi; k++) {
            ...
            /* Value the one time call if required */
            if (eval_one_time) {
              prod_val[i][j][k][one_time_col] = max(
                  0.0, call->pay_rec * (prod_val[i][j][k][1] - funding) - fee);
            }
          }
        }
      }
  ...
  if (cts_arg->is_fixing) {
    ...
    for (i = lz; i <= uz; i++) {
      ...
      for (j = lx; j <= ux; j++) {
        ...
        for (k = lpsi; k <= upsi; k++) {
          DFR_fix = exp(-eval_const_fix->ts_te_alpha -
                        eval_const_fix->ts_te_beta * tmpf -
                        eval_const_fix->ts_te_gamma * psi[k]);
          DFR_pay = exp(-eval_const_fix->tpay_tstar_alpha -
                        eval_const_fix->tpay_tstar_beta * tmpf -
      ...
      for (j = lx; j <= ux; j++) {
        ...
        for (k = lpsi; k <= upsi; k++) {
          ...
          if (schedule->iNCpn - 1 > 0) {
            DFR_start = exp(-eval_const_fix_cms->ti_tstar_alpha[0] -
            ...
            for (l = 1; l < schedule->iNCpn - 1; l++) {
              DFR_fix = exp(-eval_const_fix_cms->ti_tstar_alpha[l] -
            ...
            /* Last DF */
            DFR_end = exp(
                -eval_const_fix_cms->ti_tstar_alpha[schedule->iNCpn - 1] -
                eval_const_fix_cms->ti_tstar_beta[schedule->iNCpn - 1] * tmpf -
                eval_const_fix_cms->ti_tstar_gamma[schedule->iNCpn - 1] *
                    psi[k]);
            ...
            while (l < exo_cpn->nstr && cms > eval_const_fix_cms->s[l]) {
              coupon +=
                  eval_const_fix_cms->n[l] * (cms - eval_const_fix_cms->s[l]);
              l++;
            }
          } else {
          ...
          DFR_pay = exp(-eval_const_fix_cms->tpay_tstar_alpha -
  ...
  if (cts_arg->is_cpn) {
    ...
    for (i = lz; i <= uz; i++) {
      ...
      for (j = lx; j <= ux; j++) {
        ...
        for (k = lpsi; k <= upsi; k++) {
          DFR_fix = exp(-eval_const_cpn->ts_te_alpha -
                        eval_const_cpn->ts_te_beta * tmpf -
          ...
          case 1:
            ...
            break;
          case 2:
            DFR_pay = exp(-eval_const_cpn->tpay_tstar_alpha -
...
Err cts_payoff_4_lgmsv_mc(
  ...
  int l;
  double DFR_start, DFR_end, DFR_fix, DFR_pay, DFR_set, coupon, libor,
      partial_funding, funding, exotic, iv, fee;
  double cms;
  ...
  if (cts_arg->is_call) {
    ...
    if (call->is_midat) {
      DFR_set = exp(-eval_const_call->tset_tstar_alpha -
                    eval_const_call->tset_tstar_beta * ft -
  ...
  if (cts_arg->is_cpn) {
    ...
    case 2:
      DFR_pay = exp(-eval_const_cpn->tpay_tstar_alpha -
                    eval_const_cpn->tpay_tstar_beta * ft -
                    eval_const_cpn->tpay_tstar_gamma * psi);
      prod_val[0] += -cts->call->pay_rec * libor * DFR_pay *
                     cts_arg->dPathInfos[path_index];
  ...
  if (cts_arg->is_fixing) {
    ...
    DFR_pay = exp(-eval_const_fix->tpay_tstar_alpha -
                  eval_const_fix->tpay_tstar_beta * ft -
  ...
  if (cts_arg->is_fixing_cms) {
    ...
    if (schedule->iNCpn - 1 > 0) {
      DFR_start = exp(-eval_const_fix_cms->ti_tstar_alpha[0] -
                      eval_const_fix_cms->ti_tstar_beta[0] * ft -
    ...
    DFR_pay = exp(-eval_const_fix_cms->tpay_tstar_alpha -
                  eval_const_fix_cms->tpay_tstar_beta * ft -
...
Err cts_payoff_4_lgmsv2F_mc(
  ...
  if (cts_arg->is_call) {
    ...
    if (call->is_midat) {
      DFR_set = exp(-eval_const_call->tset_tstar_alpha -
                    eval_const_call->tset_tstar_beta * ft1 -
                    eval_const_call->tset_tstar_beta2 * ft2 -
                    eval_const_call->tset_tstar_gamma * psi1 -
  ...
  if (cts_arg->is_cpn) {
    ...
    DFR_fix = exp(
        -eval_const_cpn->ts_te_alpha - eval_const_cpn->ts_te_beta * ft1 -
        eval_const_cpn->ts_te_beta2 * ft2 - eval_const_cpn->ts_te_gamma * psi1 -
        eval_const_cpn->ts_te_gamma2 * psi2 -
        eval_const_cpn->ts_te_gamma12 * psi12);
    ...
    case 2:
      DFR_pay = exp(-eval_const_cpn->tpay_tstar_alpha -
                    eval_const_cpn->tpay_tstar_beta * ft1 -
                    eval_const_cpn->tpay_tstar_beta2 * ft2 -
                    eval_const_cpn->tpay_tstar_gamma * psi1 -
  ...
  if (cts_arg->is_fixing) {
    ...
    DFR_fix = exp(
        -eval_const_fix->ts_te_alpha - eval_const_fix->ts_te_beta * ft1 -
        eval_const_fix->ts_te_beta2 * ft2 - eval_const_fix->ts_te_gamma * psi1 -
        eval_const_fix->ts_te_gamma2 * psi2 -
        eval_const_fix->ts_te_gamma12 * psi12);
    DFR_pay = exp(-eval_const_fix->tpay_tstar_alpha -
                  eval_const_fix->tpay_tstar_beta * ft1 -
                  eval_const_fix->tpay_tstar_beta2 * ft2 -
                  eval_const_fix->tpay_tstar_gamma * psi1 -
                  eval_const_fix->tpay_tstar_gamma2 * psi2 -
                  eval_const_fix->tpay_tstar_gamma12 * psi12);
  ...
  if (cts_arg->is_fixing_cms) {
    ...
    DFR_pay = exp(-eval_const_fix_cms->tpay_tstar_alpha -
                  eval_const_fix_cms->tpay_tstar_beta * ft1 -
...
Err cts_launch_algo(CTS cts, CTS_UND und, CTS_ADI_ARG adi_arg,
  ...
  if (adi_arg->pde_or_mc == 0) {
    smessage(
        ...
        adi_arg->initnstp, adi_arg->nstp);
  } else {
    smessage("Launching MC        , Nb Steps: %d        , Nb Paths: %d",
             adi_arg->nstp, adi_arg->npaths);
  }
  ...
  if (adi_arg->pde_or_mc == 0) {
    ...
    err = lgmSV_adi_UtPieceWise(
        adi_arg->nstp, adi_arg->time, adi_arg->date, adi_arg->nstppsi,
        adi_arg->nstpx, adi_arg->nstpz, und->model.dLambdaX, 0.0,
        und->model.dPWTime, und->model.dSigma, und->model.iNbPWTime,
        und->model.dLambdaEps, und->model.dLvlEps, und->model.dAlpha,
        und->model.dRho, adi_arg->params, adi_arg->void_prm, adi_arg->is_event,
        adi_arg->ifr, und->mkt->yc, cts_payoff_4_lgmsv_adi, ncol_init, ncol_tot,
        &(temp_val[0]));
    ...
    *multi_pv = temp_val[0];
    ...
    temp_val_mc = dmatrix(0, adi_arg->nb_event + 4, 0,
                          2 + adi_arg->mcebparams->iNbIndex +
                              adi_arg->mcebparams->iCalcOneTime);
    ...
    if (und->model.iOne2F == 1) {
      err = lgmSV_mc_balsam_rev(
          adi_arg->nstp, adi_arg->nb_event, adi_arg->time, adi_arg->date,
          adi_arg->npaths, und->model.dLambdaX, adi_arg->dSigma,
          adi_arg->dAlpha, adi_arg->dLambdaEps, adi_arg->dLvlEps, adi_arg->dRho,
          NULL, NULL, NULL, adi_arg->params, adi_arg->void_prm,
          adi_arg->is_event, 1, adi_arg->optimise, adi_arg->mcebparams, NULL,
          cts_payoff_4_lgmsv_mc, NULL, 3, temp_val_mc);
      ...
      *multi_pv = (temp_val_mc[3][0] - temp_val_mc[0][0]) * dfstar;
      ...
      err = lgmSV2F_mc_balsam_rev(
          adi_arg->nstp, adi_arg->nb_event, adi_arg->time, adi_arg->date,
          adi_arg->npaths, und->model.dLambdaX, und->model.dLambdaX2,
          adi_arg->dSigma, adi_arg->dLGMAlpha, adi_arg->dLGMRho,
          adi_arg->dAlpha, adi_arg->dLambdaEps, adi_arg->dLvlEps, adi_arg->dRho,
          adi_arg->dRho2, NULL, NULL, NULL, NULL, NULL, NULL, adi_arg->params,
          adi_arg->void_prm, adi_arg->is_event, 1, adi_arg->optimise,
          adi_arg->mcebparams, NULL, cts_payoff_4_lgmsv2F_mc, NULL, 4,
          temp_val_mc);
...
FREE_RETURN:
  ...
  if (temp_val)
    free(temp_val);
  if (temp_val_mc)
    free_dmatrix(temp_val_mc, 0, adi_arg->nb_event + 4, 0,
                 2 + adi_arg->mcebparams->iNbIndex +
                     adi_arg->mcebparams->iCalcOneTime);

...
static Err make_pde_operators_allocations(
...
static Err make_pde_operators_desallocations(
    SrtBasicPdeInf pde_inf,
    double**       payoff,
    double****     diffusion_mat,
    double****     convection_mat,
    ...
    double**       option_pv_vect)
{
    ...
    if ((*diffusion_mat))
        free_f3tensor(
            (*diffusion_mat), 0, pde_inf.num_pde - 1, 0, pde_inf.num_step, 1, pde_inf.num_mesh);
    (*diffusion_mat) = NULL;
    if ((*convection_mat))
        free_f3tensor(
            (*convection_mat), 0, pde_inf.num_pde - 1, 0, pde_inf.num_step, 1, pde_inf.num_mesh);
    (*convection_mat) = NULL;
    if ((*dividend_tensor))
        free_f3tensor(
            (*dividend_tensor), 0, pde_inf.num_pde - 1, 0, pde_inf.num_step, 1, pde_inf.num_mesh);
...
static Err free_pde_inf_ptr(SrtBasicPdeInf* pde_inf)
{
    ...
    free_dvector((*pde_inf).correlation_und_vol, 0, num_time_step - 1);
    (*pde_inf).correlation_und_vol = NULL;
    free_dvector((*pde_inf).correlation_und_rate, 0, num_time_step - 1);
    (*pde_inf).correlation_und_rate = NULL;
...
static Err fill_pde_payoff(
    SrtBasicPdeInf pde_inf, double strike, SrtReceiverType rec_pay, long pde_index, double** payoff)
{
    long i;
    Err  err = NULL;
...

{
    ...
    err = und_pde_lim(event_date, und, dom_und, max_time, min_node, min_num_mesh, &pde_inf);

    if (err)
        return err;

    err = make_pde_operators_allocations(
        pde_inf, &payoff, &diffusion_mat, &convection_mat, &dividend_tensor, &option_pv_vect);
    if (err)
        return err;
    ...
    err = srt_f_eq_implied_vol((event_date - date_today) * YEARS_IN_DAY, und_name, &black_vol);

    if (err)
        return err;

    (*greeks) = srt_f_optblksch(
        forward,
        strike,
        black_vol,
    ...
    for (k = 0; k < pde_inf.num_pde; k++)
    {
        ...
        {
            (*greeks) +=
                (option_pv_vect[pde_inf.spot_index + 1] - option_pv_vect[pde_inf.spot_index]) /
                pde_inf.h;
        }
        else if (greek_type == GAMMA)
        {
            (*greeks) +=
                (option_pv_vect[pde_inf.spot_index + 1] - 2 * option_pv_vect[pde_inf.spot_index] +
    ...
    err = make_pde_operators_desallocations(
        pde_inf, &payoff, &diffusion_mat, &convection_mat, &dividend_tensor, &option_pv_vect);
    if (err)
        return err;

...
static Err int_range(double *a,double *b, double pay_time, 
					 TermStruct *ts, double (*g_x)(double));
/* -------------------------------------------------------------------------
...
double			beta, eta;
	...
	{/* loop over cash flows after the initial reverse flow */
		...
		intrinsic += STATIC_rp * STATIC_coupon[i] * STATIC_df[i]*exp(-lambda_t*xb_yb - A_t + M_txT);
	...
	/* value cash flow over numeraire */
	intrinsic -= STATIC_rp * STATIC_bond_strike * STATIC_df[0]*exp(-lambda_t*xb_yb - A_t + M_txT);
...
double			beta, eta;	
	...
	intrinsic = 0.0;
		...
		intrinsic = STATIC_rp * STATIC_period_end_df 
...
ans = ((y_bar/STATIC_deltau)*F - F_xb * dxb_yb) *
	  pow(y_bar/y,1-nu) *  
	  i_nu *
...
double F;
	...
	ans = F * exp(-(y_bar-y+.5*STATIC_beta*STATIC_deltau)*(y_bar-y+.5*STATIC_beta*STATIC_deltau)/(2*STATIC_deltau))/

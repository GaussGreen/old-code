/* ===================================================================================
   ...
   PURPOSE:       Compute swaption, bond option,caps/floors prices in betaeta, via a
...
static Err int_range(double* a, double* b, double pay_time, TermStruct* ts, double (*g_x)(double));
/* -------------------------------------------------------------------------
   FUNCTION: srt_f_betaetaclsdfrm

   PURPOSE:  betaeta model --- closed form prices for swaptions, bond options,
                 caps and floors.
                         When dealing with a swaption, a cap or a floor, the bond strike
                         has no impact.
             Please note that if it is an option on a bond, the bond strike
                         is assumed to be a clean strike. The coupons are calculated
                         with the coverage when generating the fixed leg: it is a clean
             bond generated
   ------------------------------------------------------------------------- */
    ...
    SrtMdlDim       mdl_dim)
{
    ...
    /* Finds the critical x* that corresponds to the exercise ( intrinsic(x*) = strike ) */
...
static Err int_range(double* a, double* b, double pay_time, TermStruct* ts, double (*g_x)(double))
{
    ...
    */
    ...
    if (eta != 1.)
    { /* mean is the same except for power = 1 */
        mean = 1. / (beta * (1. - eta));
    }
    else
        mean = .5 * beta * zeta;
...
static double swaption_cfs(double y_bar)
{
    ...
    intrinsic = 0.0;
    for (i = 1; i <= STATIC_n; i++)
    { /* loop over cash flows after the initial reverse flow */
        ...
        intrinsic +=
            STATIC_rp * STATIC_coupon[i] * STATIC_df[i] * exp(-lambda_t * xb_yb - A_t + M_txT);
    ...
    /* value cash flow over numeraire */
    intrinsic -=
        STATIC_rp * STATIC_bond_strike * STATIC_df[0] * exp(-lambda_t * xb_yb - A_t + M_txT);
...
/* ----------------------------------------------------------------------------
        For the pricing of a cap/floor in betaeta (with spreads already in the coupon)
   ---------------------------------------------------------------------------- */
...
/* ------------------------------------------------------------------------------ */
/* Price an option on a discount bond (zero coupon), using one factor beta-eta model */
...
static double capfl_cfs(double y_bar)
{
    ...
    intrinsic = STATIC_rp * STATIC_period_end_df * exp(-lambda_T * xb_yb - A_T + M_txT);
   ...
   is no longer needed */
{
    ...
    xb_yb  = (pow(fabs(STATIC_beta * (1 - STATIC_eta) * y_bar), 2 * nu) - 1.) / STATIC_beta;
    dxb_yb = 2 * nu * pow((1. - STATIC_eta) * STATIC_beta * y_bar, 2 * nu - 1);
    xt_yb  = (-pow(fabs(STATIC_beta * (1 - STATIC_eta) * y_bar), 2 * nu) - 1.) / STATIC_beta;
    ...
    ans = (/* 1a */
           pow(y / y_bar, nu) * (y_bar / STATIC_deltau) * (.5 * i_nu + .5 * i_minus_nu) *
           /* exp(-y*y_bar/deltau)*  taken out due to e^{-x} from N.R. I_nu */
           exp(-((y_bar - y) * (y_bar - y)) / (2 * STATIC_deltau))) +
          (/* 1b */
           pow(y / y_bar, nu) * (y_bar / STATIC_deltau) * (sin(SRT_PI * nu) / SRT_PI) * k_nu *
           exp(-(y_bar + y) * (y_bar + y) / (2 * STATIC_deltau)));
...
static double density51(double y_bar)
{
    ...
    ans = ((y_bar / STATIC_deltau) * F - F_xb * dxb_yb) * pow(y_bar / y, 1 - nu) * i_nu *
...
static double density1(double y_bar)
{
    ...
    ans = F *
          exp(-(y_bar - y + .5 * STATIC_beta * STATIC_deltau) *
              (y_bar - y + .5 * STATIC_beta * STATIC_deltau) / (2 * STATIC_deltau)) /

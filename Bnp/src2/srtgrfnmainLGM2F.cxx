...
char *SrtGrfnLGM2Fpde(char *underlying, int numeventdates, long *eventdates,
                      ...
                      int is_end_of_day_payment, int nstept, int nstepx,
                      int *nb_prod, double **prod_val) {
  int free_str = 0;
  FIRSTAllMkts xStr;
  ...
  double *evt_tms = NULL, *time = NULL, *sig_time = NULL, *sigma = NULL,
         *ifr = NULL, *date = NULL;
  ...
  err = Get_LGM2F_TermStructure(domestic_name, &sig_time, &sigma, &nbSig, &tau,
                                &alpha, &gamma, &rho);
  if (err) {
    goto FREE_RETURN;
  }
  ...
  *prod_val = dvector(0, num_col - 1);
  ...
  if (eventdates[numeventdates - 1] >= today) {
    err = lgm2f_adi(nstp, time, date, nstepx, 1.0 / tau, sig_time, sigma, nbSig,
                    alpha, gamma, rho, void_prm, is_event, ifr, yc,
                    payoff_lgm2fTau_pde, num_col, *prod_val);
  }
  if (err) {
    goto FREE_RETURN;
  }
...
char *SrtGrfnLGM2FTaupde(char *underlying, int numeventdates, long *eventdates,
                         long tableauRows, long tableauCols,
                         ...
                         int nstept, int nstepx, int *nb_prod,
                         double **prod_val) {
  int free_str = 0;
  FIRSTAllMkts xStr;
  SrtGrfnParam defParm;
  ...
  double *evt_tms = NULL, *time = NULL, *sigma_time = NULL, *lambda_time = NULL,
         *sigma = NULL, *lambda = NULL, *ifr = NULL, *date = NULL;
  ...
  /* Get the term structure */
  err = Get_LGM2F_TermStructure2(domestic_name, &sigma, &sigma_time, &nb_sigma,
                                 &lambda, &lambda_time, &nb_lambda, &alpha,
                                 &gamma, &rho);
  if (err) {
    goto FREE_RETURN;
  }
  ...
  *prod_val = dvector(0, num_col - 1);
  ...
  err =
      lgm2fTau_adi(nstp, time, date, nstepx, 0, sigma, sigma_time, nb_sigma,
                   lambda, lambda_time, nb_lambda, alpha, gamma, rho, void_prm,
                   is_event, ifr, yc, payoff_lgm2fTau_pde, num_col, *prod_val);
...
char *SrtGrfnLGM2FTaupde2(char *underlying, int numeventdates, long *eventdates,
                          long tableauRows, long tableauCols,
                          char ***tableauStrings, int **tableauMask,
                          long auxWidth, long *auxLen, double **aux,
                          int is_end_of_day_fixing, int is_end_of_day_payment,
  ...
  *prod_val = dvector(0, num_col - 1);
  ...
  err =
      lgm2fTau2_adi(nstp, time, date, nstepx, 0, sigma, sigma_time, nb_sigma,
                    lambda, lambda_time, nb_lambda, alpha, gamma, rho, void_prm,
                    is_event, ifr, yc, payoff_lgm2fTau_pde, num_col, *prod_val);
...
Err fill_mc_init_lgm2f(
  ...
  for (k = 0; k < nb_dates - 1; k++) {
    ...
    dom_fwd1[k + 1] = dom_phi1[k] * exp_dom_mat *
                      ...
                      rho_dom * alpha_dom * expect_dom12 / lda_dom2 -
    ...
    dom_fwd2[k + 1] = dom_phi2[k] * exp_dom_mat2 *
                      ...
                      dom_phi12[k] * exp_dom_mat2 *
                          Phi_Func(-lda_dom, start_mat, start_mat, end_mat) +
                      rho_dom * alpha_dom * expect_dom21 / lda_dom -
...
char *SrtGrfnLGM2FMC(char *lgmund, int numeventdates, long *eventdates,
                     long tableauRows, long *tableauCols,
                     char ***tableauStrings, int **tableauMask, long auxWidth,
                     long *auxLen, double **aux, int is_end_of_day_fixing,
                     int is_end_of_day_payment, long num_paths, int do_pecs,
                     int do_jump, double ***prod_val) {
  int free_str = 0;
  FIRSTAllMkts xStr;
  SrtGrfnParam defParm;
  int forback;
  ...
  double *dom_fwd1 = NULL, *dom_fwd2 = NULL, *dom_exp1 = NULL, *dom_exp2 = NULL,
         *dom_phi1 = NULL, *dom_phi2 = NULL, *dom_phi12 = NULL,
         *dom_gam1_fwd = NULL, *dom_gam2_fwd = NULL, *dom_bond_pay = NULL,
         *dom_gam1_pay = NULL, *dom_gam2_pay = NULL, ***covar = NULL;
  ...
  /* Get the term structure */
  err = Get_LGM2F_TermStructure(domestic_name, &sigma_date_dom, &sigma_dom,
                                &sigma_n_dom, &tau_dom, &alpha_dom, &gamma_dom,
                                &rho_dom);
  ...
  if (!dom_fwd1 || !dom_fwd2 || !dom_phi1 || !dom_phi2 || !dom_phi12 ||
      !dom_exp1 || !dom_exp2 || !dom_gam1_fwd || !dom_gam2_fwd ||
      !dom_bond_pay || !dom_gam1_pay || !dom_gam2_pay || !covar) {
    err = "Memory allocation error (3) in SrtGrfn5DFXMc";
    goto FREE_RETURN;
  }

  fill_mc_init_lgm2f(do_jump, pay_date, pay_time, date, time, nstp,
                     sigma_date_dom, sigma_n_dom, sigma_dom, lam_dom, alpha_dom,
                     gamma_dom, rho_dom, dom_yc, dom_fwd1, dom_fwd2, dom_exp1,
                     dom_exp2, dom_phi1, dom_phi2, dom_phi12, dom_gam1_fwd,
                     dom_gam2_fwd, dom_bond_pay, dom_gam1_pay, dom_gam2_pay,
                     covar);
  ...
  *prod_val = dmatrix(0, num_col - 1, 0, 1);
  ...
  err = mc_main_lgm2f(
      /*	Time data */
      num_paths, num_col, time, date, nstp, do_jump, dom_fwd1, dom_fwd2,
      dom_exp1, dom_exp2, dom_phi1, dom_phi2, dom_phi12, dom_gam1_fwd,
      dom_gam2_fwd, dom_bond_pay, dom_gam1_pay, dom_gam2_pay, covar, void_prm,
...
char *SrtGrfnLGM2FMCEB(char *lgmund, int numeventdates, long *eventdates,
...
// Calculates exp_lambda = exp(-integral(s        ,T        ,lambda(w) ,w)) when
// lambda(w) is piecewise constant
...
Err calculate_Bk_rhoCk(double Tk, double Tk1, long k, double Tn1,
  ...
  for (i = 0; i < k + 1; i++) {
    ...
    sumBk1 += sigma_dom2 / (2 * lda_dom[i]) * a1 * a1 *
              (1 - exp(-2 * lda_dom[i] * (T2i - T1i)));
  ...
  // For fwd1
  Bk1 = sigma_dom_k * sigma_dom_k / (2 * lda_dom[k]) *
        (phi_lda_dom - phi_minus_lda_dom);
...
Err calculate_Dk_rhoEk(double Tk, double Tk1, long k, double Tpay, long iTpay,
  ...
  double phi_lda_dom, phi_2_lda_dom, phi_lda_dom_lda_dom2, phi_lda_dom2,
      phi_2_lda_dom2;
...
Err fill_mc_init_lgm2f_lambda(
  ...
  double aux_exp_lambda1, aux_exp_lambda2, var1_tn1_tn, var2_tn1_tn,
      var12_tn1_tn;

  double Bk_rhoCk_fwd1, Dk_rhoEk_fwd1, Bn_rhoCn_fwd1, Dn_rhoEn_fwd1,
      Bk_rhoCk_fwd2, Dk_rhoEk_fwd2, Bn_rhoCn_fwd2, Dn_rhoEn_fwd2;
  ...
  for (n = 0; n < nb_dates - 1; n++) {
    ...
    err = gamma_lambda(start_mat, end_mat, sig_dates, lda_dom, nb_sig_dates,
                       &dom_gam1_fwd[n]);
    err = gamma_lambda(start_mat, end_mat, sig_dates, lda_dom2, nb_sig_dates,
                       &dom_gam2_fwd[n]);

    // For Df to pay date
    zc_pay = swp_f_zr(start_date, pay_date, dom_yc);

    err = gamma_lambda(start_mat, pay_time, sig_dates, lda_dom, nb_sig_dates,
                       &dom_gam1_pay[n]);
    err = gamma_lambda(start_mat, pay_time, sig_dates, lda_dom2, nb_sig_dates,
                       &dom_gam2_pay[n]);
    ...
    //	Variables initialisation
    err = exp_lambda(start_mat, end_mat, sig_dates, lda_dom, nb_sig_dates,
                     &exp_dom_mat);
    err = exp_lambda(start_mat, end_mat, sig_dates, lda_dom2, nb_sig_dates,
                     &exp_dom_mat2);
    err = exp_lambda(end_mat, pay_time, sig_dates, lda_dom, nb_sig_dates,
                     &exp_dom_pay);
    err = exp_lambda(end_mat, pay_time, sig_dates, lda_dom2, nb_sig_dates,
                     &exp_dom_pay2);
    ...
    // For all [Tk        ,Tk+1] between both event dates Tn and Tn+1 where all
    ...
    for (k = StartIndex; k < EndIndex + 1; k++) {
      ...
      // Conditional variance var1_tn1_tn        , var2_tn1_tn        ,
      // var12_tn1_tn for phi1        , phi2        , phi12
      err = exp_lambda(T2, end_mat, sig_dates, lda_dom, nb_sig_dates,
                       &aux_exp_lambda1);
      var1_tn1_tn += sig_dom2 * aux_exp_lambda1 * aux_exp_lambda1 *
                     Phi_Func(2 * lda_dom[k], T2, T1, T2);

      err = exp_lambda(T2, end_mat, sig_dates, lda_dom2, nb_sig_dates,
                       &aux_exp_lambda2);
      ...
      err = calculate_Bk_rhoCk(T1, T2, k, end_mat, sig_dates, nb_sig_dates,
                               sig_curve_dom, lda_dom, lda_dom2, alpha_dom,
                               gamma_dom, rho_dom, &Bk_rhoCk_fwd1,
                               &Bk_rhoCk_fwd2);

      Bn_rhoCn_fwd1 += Bk_rhoCk_fwd1;
      Bn_rhoCn_fwd2 += Bk_rhoCk_fwd2;

      err = calculate_Dk_rhoEk(T1, T2, k, pay_time, pay_time_Index, end_mat,
                               sig_dates, nb_sig_dates, sig_curve_dom, lda_dom,
                               lda_dom2, alpha_dom, gamma_dom, rho_dom,
                               &Dk_rhoEk_fwd1, &Dk_rhoEk_fwd2);
    ...
    dom_phi2[n + 1] =
        dom_phi2[n] * exp_dom_mat2 * exp_dom_mat2 + alpha_dom2 * var2_tn1_tn;
    dom_phi12[n + 1] = dom_phi12[n] * exp_dom_mat * exp_dom_mat2 +
                       rho_dom * alpha_dom * var12_tn1_tn;

    // Expectations of the state variables under the QTpay_time probability
    // measure
...
char *SrtGrfnLGM2FMClambda(char *lgmund, int numeventdates, long *eventdates,
                           long tableauRows, long *tableauCols,
                           char ***tableauStrings, int **tableauMask,
                           long auxWidth, long *auxLen, double **aux,
                           int is_end_of_day_fixing, int is_end_of_day_payment,
                           long num_paths, int do_pecs, int do_jump,
                           double ***prod_val) {
  int free_str = 0;
  FIRSTAllMkts xStr;
  SrtGrfnParam defParm;
  ...
  err = Get_LGM2F_TermStructure2(domestic_name, &sigma_dom, &sigma_date_dom,
                                 &n_sigma_dom, &lam_dom, &lam_dom_time,
                                 &n_lam_dom, &alpha_dom, &gamma_dom, &rho_dom);
  ...
  err = merge_lambda_sigma_ts(sigma_dom, sigma_date_dom, n_sigma_dom, lam_dom,
                              lam_dom_time, n_lam_dom, &new_time, &new_lambda,
                              &new_sigma, &n_new_time);
  if (err) {
    goto FREE_RETURN;
  }
  ...
  // Checks that ALL new_lambda[i]        , new_lambda2[i]        ,
  // new_lambda[i] + new_lambda2[i] <>0
  for (i = 0; i < n_new_time; i++) {
    if (new_lambda[i] == 0.0) {
  ...
  fill_mc_init_lgm2f_lambda(do_jump, pay_date, pay_time, date, time, nstp,
                            new_time, n_new_time, new_sigma, new_lambda,
                            new_lambda2, alpha_dom, gamma_dom, rho_dom, dom_yc,
                            dom_fwd1, dom_fwd2, dom_exp1, dom_exp2, dom_phi1,
                            dom_phi2, dom_phi12, dom_gam1_fwd, dom_gam2_fwd,
                            dom_bond_pay, dom_gam1_pay, dom_gam2_pay, covar);
  ...
  for (i = xStr.num_evt - 1; i >= 0; i--) {
    if (xStr.evt[i].evt) {
      ...
      if (grfn_prm->num_dom_df > 0) {
        ...
        for (k = 0; k < grfn_prm->num_dom_df; k++) {
          // For this event date i        , reconstruct all future zero coupons
          // needed at time k with k>i
          grfn_prm->dom_dff[k] =
              swp_f_df(xStr.dts[i], grfn_prm->dom_df_dts[k], (char *)dom_yc);
          err = gamma_lambda(xStr.tms[i], xStr.tms[i] + grfn_prm->dom_df_tms[k],
                             new_time, new_lambda, n_new_time,
                             &grfn_prm->dom_gam1[k]);
          err = gamma_lambda(xStr.tms[i], xStr.tms[i] + grfn_prm->dom_df_tms[k],
                             new_time, new_lambda2, n_new_time,
                             &grfn_prm->dom_gam2[k]);
          grfn_prm->dom_gam12[k] =
              -0.5 * (grfn_prm->dom_gam1[k] * grfn_prm->dom_gam1[k] *
                          dom_phi1[i + flag] +
...
char *SrtGrfnLGM2FMCEBlambda(char *lgmund, int numeventdates, long *eventdates,
                             int *nUsedEventDates, int *optimise,
                             double *fwd_iv, MCEBPARAMS params, long *resRows,
                             long tableauRows, long *tableauCols,
                             char ***tableauStrings, int **tableauMask,
                             long auxWidth, long *auxLen, double **aux,
                             int is_end_of_day_fixing,
                             int is_end_of_day_payment, long num_paths,
                             int do_pecs, int do_jump, double ***prod_val) {
  int free_str = 0;
  FIRSTAllMkts xStr;
  SrtGrfnParam defParm;
  int forback;

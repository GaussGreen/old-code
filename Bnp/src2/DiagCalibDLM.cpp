...
double x_dlm[NUM_HERMITE + 1], w_dlm[NUM_HERMITE + 1];

// Interpolate linearly or piecewise constant
void constant_or_linear_interpolation_dlm(
    double* dates, double* values, int n_dates, double date_wanted, double* value_wanted)
{
    int    i = 0;
    double result;
    ...
    else
    {
        ...
        if (i < n_dates - 1)
        {
            ...
            result = values[i] + (values[i + 1] - values[i]) / (dates[i + 1] - dates[i]) *
                                     (date_wanted - dates[i]);
        }
        else
        {
...
static int NCPNL_LM, NEXL_LM, *EX_CPNL_LM, *EX_ENDCPNL_LM,
    ...
    ONE2F_LM, NB_MOM_LM, INDEX_MOM[MAX_INST], BREAK_INST, FREQ_SHORT_LM, SHIFT_FREQ_LM,

    NLAM_LM,

    USE_JUMPS;

static double *CPN_TIMEL_LM, *CPN_DFL_LM, *CPN_CVGL_LM, *EX_TIMEL_LM, *EX_STRIKEL_LM, *EX_PRICEL_LM,
    *EX_VEGAL_LM, CPN_GL_LM[MAX_CPN], CPN_G2L_LM[MAX_CPN], EX_GL_LM[MAX_CPN], EX_G2L_LM[MAX_CPN],
    *EX_ZETAL_LM,

    *CPN_TIMES_LM, *CPN_DFS_LM, *CPN_CVGS_LM, *EX_TIMES_LM, *EX_STRIKES_LM, *EX_WEIGHTS_LM,
    *EX_PRICES_LM, CPN_GS_LM[MAX_CPN], CPN_G2S_LM[MAX_CPN], EX_GS_LM[MAX_CPN], EX_G2S_LM[MAX_CPN],
    *EX_ZETAS_LM,
...
Err static_lgmprcapgiventauts_momentum_dlm(
    double index, double lam[], double* price, double* gradient, int nlam)
{
        ...
        else
        {
            ...
            for (j = 0; j < nlam; j++)
            {
                for (k = 0; k < NB_MOM_LM; k++)
                {
                    ...
                    SENSI_LM[NEXS_LM + k][j] = (temp / fabs(temp) * exp(log(fabs(temp)) / (k + 1)) -
                                                PRICE_LM[NEXS_LM + k]) /
                                               BUMP_LAM_LM;
                }
            }
...
Err static_lgmprcapgiventauts_dlm(
    double index, double lam[], double* price, double* gradient, int nlam)
{
    static int i, j, ind;
    Err        err;
...
double solve_for_next_coef_dlm(
    double** res_iter, int nb_iter, double premium_tgt, int method) /* 0: linear 1: quadratic */
{
    ...
    else if (nb_iter > 2)
    {
        ...
        b = res_iter[i - 1][1] / (res_iter[i - 1][0] - res_iter[i][0]) /
                (res_iter[i - 1][0] - res_iter[i + 1][0]) * (res_iter[i][0] + res_iter[i + 1][0]) +
            res_iter[i][1] / (res_iter[i][0] - res_iter[i - 1][0]) /
                (res_iter[i][0] - res_iter[i + 1][0]) * (res_iter[i - 1][0] + res_iter[i + 1][0]) +
            res_iter[i + 1][1] / (res_iter[i + 1][0] - res_iter[i - 1][0]) /
                (res_iter[i + 1][0] - res_iter[i][0]) * (res_iter[i][0] + res_iter[i - 1][0]);
        b = -b;
        ...
        c = res_iter[i - 1][1] / (res_iter[i - 1][0] - res_iter[i][0]) /
            ...
            res_iter[i][1] / (res_iter[i][0] - res_iter[i - 1][0]) /
                (res_iter[i][0] - res_iter[i + 1][0]) * res_iter[i - 1][0] * res_iter[i + 1][0] +
            res_iter[i + 1][1] / (res_iter[i + 1][0] - res_iter[i - 1][0]) /
                (res_iter[i + 1][0] - res_iter[i][0]) * res_iter[i - 1][0] * res_iter[i][0] -
            premium_tgt;
...
static Err lgmprcapgiventauts_simpleNewton_dlm(double* lam, double* price, double* lam_sens)
{
    int      i, j, k, l;
    ...
    double*              lam_sens)
{
    ...
    Err    err = NULL;
    double data[MAX_CPN], weight[MAX_CPN], target[MAX_CPN], lambda[MAX_CPN], ex_zetas[MAX_INST];
    ...
    double*             dOutputVolType)
{
    ...
    else
    {
        if (dInputVolType < 0.5 && eShiftVolType == LOGNORMAL_VOL)
        {
            ...
            err = srt_f_optimpvol(
                dPrice, dFwd, dStrike, dMaturity, 1.0, SRT_PUT, SRT_LOGNORMAL, &dInputVol);
        ...
        else if (dInputVolType > 0.5 && eShiftVolType == NORMAL_VOL)
        {
            ...
            err = srt_f_optimpvol(
                dPrice, dFwd, dStrike, dMaturity, 1.0, SRT_PUT, SRT_NORMAL, &dInputVol);
    ...
    double* ex_nstd)
{
    ...
    for (i = 0; i < num_ex_dates; i++)
    {
        ...
        if (vol_shift_time && vol_shift)
        {
            ...
            constant_or_linear_interpolation_dlm(
                vol_shift_time, vol_shift, num_vol_shift, ex_time[i], &shift);
        }
        else
        {
            shift = 0.0;
        ...
        if (power > 0.5)
        {
            power = srt_f_optblksch(ex_fwd[i], ex_fwd[i], std, ex_time[i], 1.0, SRT_CALL, PREMIUM);

            err = srt_f_optimpvol(
                power, ex_fwd[i], ex_fwd[i], ex_time[i], 1.0, SRT_CALL, SRT_NORMAL, &std);
        }

        if (ex_atm_price)
        {
            ex_atm_price[i] =
                srt_f_optblknrm(ex_fwd[i], ex_fwd[i], std, ex_time[i], ex_lvl[i], SRT_PUT, PREMIUM);
        }
        ...
        {
            ex_strike[i] =
                ex_fwd[i] + ex_primnstd[Get_Index(ex_time[i], ex_primtime, num_ex_primdates)] * std;
        }
        ...
        if (power > 0.5)
        {
            ex_price[i] = srt_f_optblksch(
                ex_fwd[i], ex_strike[i], ex_vol[i], ex_time[i], ex_lvl[i], SRT_PUT, PREMIUM);

            ex_vega[i] = srt_f_optblksch(
                ex_fwd[i], ex_strike[i], ex_vol[i], ex_time[i], ex_lvl[i], SRT_PUT, VEGA);
        }
        else
        {
    ...
    CPD_CALIB_INST_DATA inst_data) /*	NULL = don't save calibration instrument data */
{
    ...
    double ex_timel[MAX_INST], ex_fwdl[MAX_INST], ex_lvll[MAX_INST], ex_strikel[MAX_INST],
        ex_voll[MAX_INST], ex_pricel[MAX_INST], ex_atm_pricel[MAX_INST], ex_vegal[MAX_INST],
        ex_atm_vegal[MAX_INST], ex_nstdl[MAX_INST],

        ex_times[MAX_INST], ex_fwds[MAX_INST], ex_lvls[MAX_INST], ex_strikes[MAX_INST],
        ex_weights[MAX_INST], ex_prices[MAX_INST], ex_atm_prices[MAX_INST], ex_vols[MAX_INST],
        ex_vegas[MAX_INST], ex_nstds[MAX_INST],

        ex_zeta[MAX_INST], ex_zeta_temp[MAX_INST], ex_G_temp[MAX_INST];

    int ex_startcpnl[MAX_INST], ex_endcpnl[MAX_INST], ex_startcpns[MAX_INST], ex_endcpns[MAX_CPN];

    long cpn_datel[MAX_CPN], cpn_dates[MAX_CPN];

    double cpn_timel[MAX_CPN], cpn_cvgl[MAX_CPN], cpn_dfl[MAX_CPN], cpn_times[MAX_CPN],
        cpn_cvgs[MAX_CPN], cpn_dfs[MAX_CPN], cpn_G_temp[MAX_CPN];
    ...
    *sig      = NULL;
    ...
    if (one2F == 2)
    {
        HermiteStandard(x_dlm, w_dlm, NUM_HERMITE);
    }
    ...
    if (one_factor_equi && one2F == 2 && fix_lambda)
    {
        ...
        static_interpolate_zeta(
            ...
            ex_zeta_temp);
        static_lgmsetupG_tauts(
            nlam, lam_time, lam, ncpns, cpn_times, cpn_G_temp, num_ex_datess, ex_times, ex_G_temp);

        for (i = 0; i < num_ex_datess; i++)
        {
            ex_prices[i] = lgmsopval1F(
                ex_endcpns[i] - ex_startcpns[i] + 1,
    ...
    *sig      = (double*)calloc(num_ex_datesl, sizeof(double));
    ...
    for (i = 1; i < num_ex_datesl; i++)
    {
        ...
        {
            exp_fact =
                static_lgmcalcexpfact_tauts(ex_timel[i - 1], ex_timel[i], nlam, lam_time, lam);
    ...
    if (inst_data)
    {
        ...
        if (!fix_lambda)
        {
            ...
            if (!inst_data->exer_dates_long || !inst_data->start_dates || !inst_data->end_dates ||
                ...
                !inst_data->end_datess || !inst_data->short_strikes || !inst_data->short_weights ||
        ...
        for (i = 0; i < num_ex_datesl; i++)
        {
            inst_data->exer_dates_long[i] =
                (long)(ex_timel[i] * DAYS_IN_YEAR + EPS_MAT_TO_LONG_DATE_CONVERSION) + today;
        ...
        if (!fix_lambda)
        {
            for (i = 0; i < num_ex_datess; i++)
            {
                inst_data->exer_dates_short[i] =
                    (long)(ex_times[i] * DAYS_IN_YEAR + EPS_MAT_TO_LONG_DATE_CONVERSION) + today;
...
Err AllocateCalibInst(int iNbStrike, CALIBINSTRUMENTDLM CalibInst)
{
    ...
    CalibInst->dStrike = (double*)calloc(iNbStrike, sizeof(double));

    CalibInst->dPrice = NULL;
    CalibInst->dPrice = (double*)calloc(iNbStrike, sizeof(double));

    CalibInst->dNormalVol = NULL;
    CalibInst->dNormalVol = (double*)calloc(iNbStrike, sizeof(double));

    CalibInst->dVega = NULL;
    CalibInst->dVega = (double*)calloc(iNbStrike, sizeof(double));

    CalibInst->dNbStd = NULL;
    CalibInst->dNbStd = (double*)calloc(iNbStrike, sizeof(double));

    CalibInst->dModelPrice = NULL;
    CalibInst->dModelPrice = (double*)calloc(iNbStrike, sizeof(double));

    if (!CalibInst->dStrike || !CalibInst->dPrice || !CalibInst->dNormalVol || !CalibInst->dVega ||
        !CalibInst->dNbStd || !CalibInst->dModelPrice)
    {
...
Err AllocateAllCalibInst(int iNbInst, int iNbStrike, ALLCALININSTRUMENTSDLM AllCalibInst)
{
    int i;
    Err err = NULL;

    AllCalibInst->sCalibInst = (CalibInstrumentDLM*)calloc(iNbInst, sizeof(CalibInstrumentDLM));
    ...
    CALIBEXESCHEDULEDLM CalibExeSchedule)
{
    ...
    {
        err = get_end_date(
            ex_date[i], end_date, end_tenor[i], 0, &(CalibExeSchedule->lTheoEndDates[i]));
        if (err)
        {
            goto FREE_RETURN;
        }
        CalibExeSchedule->lActEndDates[i] =
            bus_date_method(CalibExeSchedule->lTheoEndDates[i], MODIFIED_SUCCEEDING);
    ...
    CPD_DIAG_CALIB_PARAM param)
{
    ...
    {
        if (((CalibExeSchedule->lExeDates)[j] - (CalibExeSchedule->lExeDates)[i]) * YEARS_IN_DAY <
    ...
    ALLCALININSTRUMENTSDLM AllCalibShortInst)
{
    ...
    for (i = 0; i < AllCalibInst->iNbInst; i++)
    {
        ...
        while (k < CalibCpnSchedule->iNCpn &&
               CalibCpnSchedule->lCpnDate[k] < CalibExeSchedule->lActEndDates[i] - 5)
        {
            k++;
        }
        if (k > 0 &&
            fabs(CalibExeSchedule->lActEndDates[i] - CalibCpnSchedule->lCpnDate[k]) >
    ...
    for (i = 0; i < AllCalibInst->iNbInst; i++)
    {
        ...
        if (is_for_calibration)
        {
            ...
            for (m = 0; m < CalibInst->iNbStrike; m++)
            {
                ...
                {
                    CalibInst->dStrike[m] =
                        CalibInst->dFwdCash + (AllCalibShortInst->sCalibInst[Get_Index(
                                                   CalibInst->dExeTime,
    ...
    double*           vol)
{
    ...
    {
        err = get_cash_vol(
            vol_curve_name, start_date, end_date, cash_strike, 0, ref_rate_name, vol, &power);
        ...
        if (power > 0.5 && vol_type == SRT_NORMAL)
        {
            ...
            opt = srt_f_optblksch(cash_fwd, cash_strike, *vol, mat, 1.0, SRT_CALL, (SrtGreekType)SRT_PREMIUM);

            if (mat >= YEARS_IN_DAY && opt > 1.0E-08)
            {
                err = srt_f_optimpvol(
                    opt, cash_fwd, cash_strike, mat, 1.0, SRT_CALL, SRT_NORMAL, vol);
        ...
        else if (power < 0.5 && vol_type == SRT_LOGNORMAL)
        {
            ...
            opt = srt_f_optblknrm(
                cash_fwd, cash_strike, *vol, mat, 1.0, SRT_CALL, (SrtGreekType)SRT_PREMIUM);

            if (mat >= YEARS_IN_DAY && opt > 1.0E-08)
            {
                err = srt_f_optimpvol(
                    opt, cash_fwd, cash_strike, mat, 1.0, SRT_CALL, SRT_LOGNORMAL, vol);

...
double s, q, d1, d2, bs, integrd;
	...
	if ( (s/q) <= NI_nlimitlow )		/* d1 and d2 are - infinity */
	{
		integrd = 0.;
		return (integrd);
	}
...
/* sets integration limits */
	...
	glimithigh		= (NI_qcoef > NI_scoef) ? \
						(GAUSS_LIMIT + NI_qcoef) : (GAUSS_LIMIT + NI_scoef);
	glimitlow		= (NI_scoef > 0.) ? \
						(-GAUSS_LIMIT) : (-GAUSS_LIMIT + NI_scoef);
	...
	switch ( greek )
	{
		...
		case DELTAX :
			shift  = fwdx / 10000;
	 		answer = ( srt_f_optextspr(	fwdx + shift,
									 	fwdy, 
										sigx,
										sigy,
										rho,
		...
		case GAMMAXY:
			shiftx  = fwdx / 1000;
			shifty  = fwdy / 1000;
			answer  = srt_f_optextspr(	fwdx + shiftx,
										fwdy + shifty,
										sigx,
										sigy,
										rho,

...
static double level_cash(double x, int n, double *cvg)
{
	...
	{
		tmp = 1.0 + cvg[i] * x;
		for (j=1, tmpi = tmp; j < i+1; j++) tmpi *= tmp;
		lvlc += cvg[i] / tmpi;
	}
	return lvlc;
}
...
static Err gf_calc_deriv(double x, smm_swap *swp, double shift, double *g, double *f)
{
	...
	for (i=0; i < swp->nfix; i++)
	{
		...
		lvlc2 += swp->fix_cvg[i] * swp->fix_cvg[i] * swp->fix_cvg[i] * (i+1) * (i+2) / (tmpi * tmp * tmp);
	}
	g[0] = 1.0 / lvlc0;
	g[1] = g[0] * g[0] * lvlc1;
	...
	err = BMM_Option_Price_From_States(x, swp->fxg_time,
		swp->beta, swp->fwd1, swp->fwd2, swp->sigbeta1, swp->sigbeta2, swp->pi, SRT_CALL, &f[0]);
	if (err) return err;

	err = BMM_Option_Price_From_States(x - shift, swp->fxg_time,
		swp->beta, swp->fwd1, swp->fwd2, swp->sigbeta1, swp->sigbeta2, swp->pi, SRT_CALL, &p[0]);
	if (err) return err;

	err = BMM_Option_Price_From_States(x + shift, swp->fxg_time,
		swp->beta, swp->fwd1, swp->fwd2, swp->sigbeta1, swp->sigbeta2, swp->pi, SRT_CALL, &p[1]);
	if (err) return err;
								...
								double	**dCumulative)
{
	...
	// First Guess
	srt_f_optbmmvolfromstates(	swp->cms_fwd,//		 swp->swap0,
								swp->fxg_time,
								swp->beta,
								swp->fwd1,
								swp->fwd2,
	...
	do
	{
		...
		err = BMM_Option_Price_From_States(dMaxPoint, swp->fxg_time,
			swp->beta, swp->fwd1, swp->fwd2, swp->sigbeta1, swp->sigbeta2, swp->pi, SRT_CALL, &dNewF[0]);
		if (err) return err;
	...
	(*dCumulative)[0] = ( dNewG[0] * dNewF[1] - dLastG[0] * dLastF[1] - dNewG[1] * dNewF[0] + dLastG[1] * dLastF[0] +
		0.5 * (dNewG[2] * dNewF[0] + dLastG[2] * dLastF[0]) * (*dPoints)[0] ) * swp->lvlc0;
	...
	while (dNewPoint < dMaxPoint)
	{
		...
		dNewCumul = dLastCumul +
			( dNewG[0] * dNewF[1] - dLastG[0] * dLastF[1] - dNewG[1] * dNewF[0] + dLastG[1] * dLastF[0] +
			0.5 * (dNewG[2] * dNewF[0] + dLastG[2] * dLastF[0]) * (dNewPoint - dLastPoint) ) * swp->lvlc0;
		...
		while (dError > dMaxError)
		{
			...
			dRealCumul = dLastCumul +
				( dRealG[0] * dRealF[1] - dLastG[0] * dLastF[1] - dRealG[1] * dRealF[0] + dLastG[1] * dLastF[0] +
				0.5 * (dRealG[2] * dRealF[0] + dLastG[2] * dLastF[0]) * (dInterpPoint - dLastPoint) ) * swp->lvlc0;

			dInterpCumul = dLastCumul + (dNewCumul - dLastCumul) / (dNewPoint - dLastPoint) * (dInterpPoint - dLastPoint);
...
static Err smm_swap_density(smm_swap *swp, double x, double shift, double *density)
{
	...
	err = BMM_Option_Price_From_States(x + shift, swp->fxg_time,
		swp->beta, swp->fwd1, swp->fwd2, swp->sigbeta1, swp->sigbeta2, swp->pi, SRT_CALL, &p[2]);
	if (err) return err;
								...
								double	**dCumulative)
{
	...
	double	dNbStdDevLeft, dNbStdDevRight, dLogStd, dLogCvx;
	double	dLastPoint, dNewPoint, dInterpPoint,
			dLastCumul, dNewCumul, dInterpCumul, dRealCumul,
			dLastDensity, dNewDensity, dRealDensity;
	long	i, lAllocTotPoints;
	double	dError;

	// First Guess
	srt_f_optbmmvolfromstates(	swp->swap0,
								swp->fxg_time,
								swp->beta,
								swp->fwd1,
								swp->fwd2,
	...
	while (dNewPoint < dMaxPoint)
	{
		...
		while (dError > dMaxError)
		{
			...
			dRealCumul = dLastCumul + 0.5 * (dLastDensity + dRealDensity) * (dInterpPoint - dLastPoint);
...
static Err smm_simulate(int nswaps, smm_swap *swaps, double **corr_matrix, smm_params *params, double **samples)
{
	Err				err = NULL;
	SRandomGen		rg;
	...
	if (!chol || !gg) { err = serror("Memory failure");  goto FREE_RETURN; }
	...
	for (i=0; i < params->npaths; i++)
	{
		...
		{
			tmp = 0.0;
			for (k=0; k < nswaps; k++) tmp += chol[j][k] * gg[i][k];

			samples[i][j] = swaps[j].cms_fwd /*swaps[j].swap0 */*
				exp(swaps[j].sigbeta1 * sqrtt * tmp - 0.5 * swaps[j].sigbeta1 * swaps[j].sigbeta1 * t);
		}
	}

FREE_RETURN:
	ABS_Free(&rg);
	if (chol) free_dmatrix(chol, 0, nswaps-1, 0, nswaps-1);
	if (gg) free_dmatrix(gg, 0, params->npaths-1, 0, nswaps-1);

	return err;
}

static Err smm_simulate2(int nswaps, smm_swap *swaps, double **corr_matrix, smm_params *params, double **samples)
{
	Err				err = NULL;
	int				i;
	long			*npts = NULL;
	...
	for (i=0; i < nswaps; i++)
	{
		err = smm_BMM_linterp_cumulative(&swaps[i], params->maxerror, params->nstd, params->nstd,
			&npts[i], &pts[i], &cumul[i]);
		if (err) goto FREE_RETURN;
	}
...
FREE_RETURN:
	...
	if (pts) for (i=0; i < nswaps; i++) free(pts[i]);
	free(pts);
	if (cumul) for (i=0; i < nswaps; i++) free(cumul[i]);
	free(cumul);
	free(npts);
...
static Err smm_setup_swaps(char *yc, char *vc, smm_params *params, char *freq_str, char *bas_str, char *ref_rate,
					long ex_date, long last_date, smm_swap **swaps, int *nswaps)
{
	Err				err = NULL;
	...
	if (!crv) return serror("Yield curve not found");
	...
	*swaps = (smm_swap *) calloc(*nswaps, sizeof(smm_swap));
	if (!*swaps) return serror("Memory failure");
	...
	for (i=0; i < *nswaps; i++)
	{
		...
		if (!swp->fix_d || !swp->fix_t || !swp->fix_cvg || !swp->fix_df ||
			!swp->flt_d || !swp->flt_t || !swp->flt_cvg || !swp->flt_df || !swp->flt_spr)
		...
		for (j=0; j < swp->nfix; j++)
		{
			...
			swp->fix_cvg[j] = coverage( (j==0 ? swp->start_date : swp->fix_d[j-1]), swp->fix_d[j], fix_basis );
		...
		for (j=0; j < swp->nflt; j++)
		{
			...
			swp->flt_cvg[j] = coverage( (j==0 ? swp->start_date : swp->flt_d[j-1]), swp->flt_d[j], flt_basis );
			...
			for (k = flt_freq / fix_freq - 1; k >= 0; k--)
			{
				...
				temp_date = (k==0 ? swp->flt_d[j] : bus_date_method (theo_date, MODIFIED_SUCCEEDING));
		...
		for (j=0; j < swp->nfix; j++) swp->lvl0 += swp->fix_cvg[j] * swp->fix_df[j];
		for (j=0; j < swp->nflt; j++) flt_leg += swp->flt_spr[j] * swp->flt_cvg[j] * swp->flt_df[j];
		...
		err = swp_f_SABRvol(vc, swp->start_date, swp->end_date, 0.0, &sabr_sigbeta, &power, SABR_BETAVOL);
		if (err) return err;

		err = swp_f_SABRvol(vc, swp->start_date, swp->end_date, 0.0, &sabr_alpha, &power, SABR_ALPHA);
		if (err) return err;
		...
		err = srt_f_optsarbvol(swp->swap0, swp->swap0, swp->fxg_time, sabr_sigbeta, sabr_alpha, sabr_beta, sabr_rho,
			SRT_BETAVOL, SRT_LOGNORMAL, &imp_vols[0]);
		if (err) return err;
		...
		err = srt_f_optsarbvol(swp->swap0, swp->swpn_strikes[1], swp->fxg_time, imp_vols[0],
			sabr_alpha, sabr_beta, sabr_rho, SRT_LOGNORMAL, SRT_LOGNORMAL, &imp_vols[1]);
		if (err) return err;

		err = srt_f_optsarbvol(swp->swap0, swp->swpn_strikes[2], swp->fxg_time, imp_vols[0],
			sabr_alpha, sabr_beta, sabr_rho, SRT_LOGNORMAL, SRT_LOGNORMAL, &imp_vols[2]);
		if (err) return err;

		for (j=0; j < 3; j++) swp->swpn_values[j] =
			srt_f_optblksch(swp->swap0, swp->swpn_strikes[j], imp_vols[j], swp->fxg_time, swp->lvl0, SRT_CALL, PREMIUM);
		...
		err = srt_f_optsarbvol(swp->swap0, swp->swap0, swp->fxg_time, imp_vols[0],
			sabr_alpha, sabr_beta, sabr_rho, SRT_LOGNORMAL, SRT_BETAVOL, &cms_vol.vol);
		if (err) return err;
...
//		err = swp_f_cmsrate(swp->swap0, swp->nfix, fix_freq, sabr_sigbeta, swp->fxg_time,
//			0.0, 0.0, 0, SRT_LOGNORMAL, &swp->cms_fwd);
		...
		err = srt_f_optimpvol(cms_opt_atm, swp->cms_fwd, swp->cms_fwd, swp->fxg_time, 1.0, SRT_PUT, SRT_LOGNORMAL,
			&imp_vols[0]);
		if (err) return err;

		err = srt_f_optsarbvol(swp->cms_fwd, swp->cms_fwd, swp->fxg_time, imp_vols[0],
			sabr_alpha, sabr_beta, sabr_rho, SRT_LOGNORMAL, SRT_BETAVOL, &sabr_sigbeta);
		if (err) return err;
...
//		swp->cms_fwd *= exp(-0.0010 * (swp->end_time - swp->start_time));
		...
		if (err) return err;
		if (calib_pres > 1e-5) return serror("BMM calibration failed");
...
static void smm_strip(int nswaps, smm_swap *swaps, double *orig_samples,
					  double *swp_times, double *swp_dfs, double *zc_rate)
{
...
PASS_START:
	...
	for (i=0; i < nswaps; i++)
	{
		...
		swp_dfs[i+1] = (swp_dfs[0] - samples[i] * fix_leg + flt_leg) / (samples[i] * fix_tmp + 1.0 - flt_tmp);
	//		(swp_dfs[0] - samples[i] * level_cash(samples[i], swaps[i].nfix, swaps[i].fix_cvg) + flt_leg) /
	//		(1.0 - flt_tmp);

		if (swp_dfs[i+1] > 0)
			zc_rate[i+1] = -log(swp_dfs[i+1]) / (swp_times[i+1] - swaps[0].fxg_time);
		else zc_rate[i+1] = 999.99;
		fix_leg += fix_tmp * swp_dfs[i+1];
		flt_leg += flt_tmp * swp_dfs[i+1];

		if (ipass < NPASS) samples[i] = orig_samples[i] * swaps[i].lvl0 / swaps[i].df_start / fix_leg;
	}

	if (ipass++ < NPASS) goto PASS_START;
}

static void smm_interp_dfs(int swp_ndfs, double *swp_times, double *swp_dfs, double *zc_rate, double mat,
						   int ndfs, double *times, double *dfs)
{
	int			i, idx = 0;
	double		zcr, coef;
	...
	for (i=0; i < ndfs; i++)
	{
		if (times[i] <= swp_times[0]) dfs[i] = swp_dfs[0];
		else if (times[i] >= swp_times[swp_ndfs-1]) dfs[i] = swp_dfs[swp_ndfs-1];
		else
		{
			while (times[i] >= swp_times[idx+1]) idx++;
			...
			else dfs[i] = coef * swp_dfs[idx+1] + (1.0 - coef) * swp_dfs[idx];
		}
	}
}

static Err smm_match(int nswaps, smm_swap *swaps, smm_params *params, double **swp_dfs, double *weights)
{
	Err				err = NULL;
	...
	if (!A || !AA || !b || !sv || !e) { err = serror("Memory failure");  goto FREE_RETURN; }
	...
	for (k=0; k < nswaps; k++)
	{
		...
		for (j=0; j < N; j++)
		{
			...
			for (n=0; n < 3; n++)
			{
				...
				if (A[i+n+1][j] < 0.0) A[i+n+1][j] = 0.0;										// Swaptions
				sum_s[n] += A[i+n+1][j] * weights[j];
			}
		}
		b[i++] = sum - swaps[k].cms_fwd;
		for (n=0; n < 3; n++) b[i++] = sum_s[n] - swaps[k].swpn_values[n] / swaps[k].df_start;
	}
	...
	for (i=0; i < M; i++)
	{
		for (k=i; k < M; k++)
		{
			for (j=0, sum = 0.0; j < N; j++) sum += A[i][j] * A[k][j];
			AA[i][k] = AA[k][i] = sum;
		}
	}
	...
	nag_real_svd(M, M, &AA[0][0], M, 1, b, 1, 0, NULL, 0, sv, 1, NULL, 0, &iter, e, &failinfo, &fail);
	...
	for (i=0; i < M; i++)
	{
		for (k=0, sum = 0.0; k < M; k++) sum += AA[k][i] * b[k];
		e[i] = sum;
	}
	...
	for (j=0; j < N; j++)
	{
		for (i=0, sum = 0.0; i < M; i++) sum += A[i][j] * e[i];
		weights[j] -= sum;
	}

FREE_RETURN:
	if (A) free_dmatrix(A, 0, M-1, 0, N-1);
	if (AA) free_dmatrix(AA, 0, M-1, 0, M-1);
	free(b);
	free(sv);
	free(e);
	...
	double			*stddev )
{
	...
	double			sabr_sigbeta[MAXNDATES], sabr_alpha[MAXNDATES], sabr_beta[MAXNDATES], sabr_rho[MAXNDATES];
	...
	if (!iv || !fee) { err = serror("Memory failure");  goto FREE_RETURN; }

	if (g->nvol) for (i=0; i < g->nvol[0][iex]; i++)
	{
		err = swp_f_SABRvol(vc, g->vol_start[0][iex][i], g->vol_end[0][iex][i], 0.0,
			&sabr_sigbeta[i], &power, SABR_BETAVOL);
		if (err) goto FREE_RETURN;

		err = swp_f_SABRvol(vc, g->vol_start[0][iex][i], g->vol_end[0][iex][i], 0.0,
			&sabr_alpha[i], &power, SABR_ALPHA);
		if (err) goto FREE_RETURN;

		err = swp_f_SABRvol(vc, g->vol_start[0][iex][i], g->vol_end[0][iex][i], 0.0,
			&sabr_beta[i], &power, SABR_BETA);
		if (err) goto FREE_RETURN;

		err = swp_f_SABRvol(vc, g->vol_start[0][iex][i], g->vol_end[0][iex][i], 0.0,
			&sabr_rho[i], &power, SABR_RHO);
		if (err) goto FREE_RETURN;
	}
	...
	if (g->nvol && (params->vol_hyp & 1))		// sigma-beta sliding
	{
		...
		{
			err = swp_f_SABRvol(vc, g->vol_start[0][iex][i] - slide, g->vol_end[0][iex][i] - slide, 0.0,
				&sabr_sigbeta[i], &power, SABR_BETAVOL);
			if (err) goto FREE_RETURN;
		}
	}
	...
	err = smm_setup_swaps(yc, vc, params, freq_str, bas_str, ref_rate,
		g->ex_d[iex], g->mat_d[0][iex][g->nmat[0][iex] - 1], &swaps, &nswaps);
	if (err) goto FREE_RETURN;

	// Fill corr matrix:

	corr_matrix = dmatrix(0, nswaps-1, 0, nswaps-1);
	if (!corr_matrix) { err = serror("Memory failure");  goto FREE_RETURN; }
	...
	if (!samples) { err = serror("Memory failure");  goto FREE_RETURN; }
	...
	if (!swp_times || !swp_dfs || !zc_rate) { err = serror("Memory failure");  goto FREE_RETURN; }
	...
	if (!weights) { err = serror("Memory failure");  goto FREE_RETURN; }
	...
	if (!pvs) { err = serror("Memory failure");  goto FREE_RETURN; }
	...
	for (ipath=0; ipath < params->npaths; ipath++)
	{
		smm_interp_dfs(swp_ndfs, swp_times, swp_dfs[ipath], zc_rate[ipath], swaps[0].fxg_time,
			g->nmat[0][iex], g->mat[0][iex], dfs);
	...
	// Calculate fees:
	for (i=0; i < g->ninst; i++) fee[i] = (params->use_fee ? (iv[i] - pv[i]) / swaps[0].df_start : 0.0);
	...
	for (ipath=0; ipath < params->npaths; ipath++)
	{
		...
		{
			pvs[ipath][i] += fee[i];
			if (params->otc_type == 1) pvs[ipath][i] = max(pvs[ipath][i], 0.0);		// call
			if (params->otc_type == 2) pvs[ipath][i] = max(-pvs[ipath][i], 0.0);	// put
			pv[i] += pvs[ipath][i] * weights[ipath] * swaps[0].df_start;
		}
	}
...
FREE_RETURN:
	if (pvs) free_dmatrix(pvs, 0, params->npaths-1, 0, g->ninst-1);

	free(swp_times);
	if (swp_dfs) free_dmatrix(swp_dfs, 0, params->npaths-1, 0, swp_ndfs-1);
	if (zc_rate) free_dmatrix(zc_rate, 0, params->npaths-1, 0, swp_ndfs-1);
	free(weights);

	if (samples) free_dmatrix(samples, 0, params->npaths-1, 0, nswaps-1);
	if (corr_matrix) free_dmatrix(corr_matrix, 0, nswaps-1, 0, nswaps-1);
	if (swaps) for (i=0; i < nswaps; i++) smm_free_swap(&swaps[i]);
	free(swaps);
...
static Err IndexDates(int ndates, long *dates, int *new_ndates, long *new_dates, long *index)
{
	...
	if (ndates > 10 * MAXNDATES) return serror("Too many dates in IndexDates");
	...
	{
		if (dates[idx[i]] != lastd) new_dates[j++] = lastd = dates[idx[i]];
		index[idx[i]] = j-1;
	}
	*new_ndates = j;
	return NULL;
...
)
{
	...
	if (!g->ex || !g->ex_d || !g->nmat || !g->nvol || !g->spec_desc) return serror("Memory failure");

	memcpy(g->ex_d, ex_date, ncall * sizeof(long));
	for (i=0; i < ncall; i++) g->ex[i] = (g->ex_d[i] - today) * YEARS_IN_DAY;
	...
	if (!cif->fund_cpn) return serror("Memory failure");
	...
	double			**K )
{
	...
	g->nex = g->nccy = 1;
	g->ninst = 0;
	for (i=0; i < nends; i++) g->ninst += nK[i];
	...
	if (!g->ex || !g->ex_d || !g->nmat || !g->spec_desc) return serror("Memory failure");
	...
	if (!swp_desc->cf_idx || !swp_desc->ncf) return serror("Memory failure");
	...
	for (i=0; i < nends; i++)
	{
		for (j=0; j < nK[i]; j++)
		{
			...
			while (act_date < ends[i])
			{
				...
				if (act_date > ends[i]) act_date = ends[i];
				all_dates[n_all_dates] = act_date;
				all_cash_flows[n_all_dates] =
					swp_f_spread(temp_date, act_date, ref_rate) *
			...
			while (act_date < ends[i])
			{
				...
				all_cash_flows[n_all_dates] =
					- K[i][j] *
					coverage(temp_date, act_date, fix_basis);
				n_all_dates ++;
				swp_desc->ncf[inst_idx] ++;
				temp_date = act_date;
			}
	...
	if (!swp_desc->cf || !swp_desc->dfs_idx) return serror("Memory failure");
	...
	if (!g->mat || !g->mat_d) return serror("Memory failure");

	memcpy(g->mat_d[0][0], new_dates, g->nmat[0][0] * sizeof(long));

	for (i=0; i < g->nmat[0][0]; i++) g->mat[0][0][i] = (g->mat_d[0][0][i] - today) * YEARS_IN_DAY;
...
void smm_swaptions_free_product(SProductDesc *g)
{
	...
	if (g->mat) free_f3tensor(g->mat, 0, 0, 0, 0, 0, g->nmat[0][0] - 1);
	if (g->mat_d) free_l3tensor(g->mat_d, 0, 0, 0, 0, 0, g->nmat[0][0] - 1);
	...
	free(g->ex);
	free(g->ex_d);
	if (g->nmat) free_imatrix(g->nmat, 0, 0, 0, 0);
}
	...
	double			*pv	)		// ninst sized vector with PVs (output)
{
	...
	for (i=0; i < g->ninst; i++) pv[i] = dfs[i];
...
Err smm_dfs_init_product(SProductDesc *g, char *yc, long ex_date, int ndates, long *dates)
{
	...
	for (i=0; i < ndates; i++) g->mat[0][0][i] = (g->mat_d[0][0][i] - today) * YEARS_IN_DAY;
...
{
	if (g->mat_d) free_l3tensor(g->mat_d, 0, 0, 0, 0, 0, g->ninst-1);
	if (g->mat) free_f3tensor(g->mat, 0, 0, 0, 0, 0, g->ninst-1);
	if (g->nmat) free_imatrix(g->nmat, 0, 0, 0, 0);
	free(g->ex_d);
	free(g->ex);
}

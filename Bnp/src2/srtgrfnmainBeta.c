						  ...
						  double	**prod_val)
{
	...
	{
		return SrtGrfn3DFXTree(		und3dfx,
						  			numeventdates, 
						  			eventdates,
						  			tableauRows,
						  			tableauCols,
	...
	if (!vol_change || !dom_vol || !for_vol || !fx_vol || !corr_dom_for || !corr_dom_fx || !corr_for_fx || !beta_tab)
	{
	...
	fx_vol[nstp-1] = find_fx_sig (time[nstp-1], fx_ts) * exp (alpha * sqrt (time[nstp-1]) - 0.5 * alpha * alpha * time[nstp-1]);
	beta_tab[nstp-1] = beta;

	err = srt_f_get_corr_from_CorrList(sCorrlist, domname, forname, time[nstp-1], &(corr_dom_for[nstp-1]));
	if (err)
	{
		goto FREE_RETURN;
	}
	err = srt_f_get_corr_from_CorrList(sCorrlist, domname, und3dfx, time[nstp-1], &(corr_dom_fx[nstp-1]));
	if (err)
	{
		goto FREE_RETURN;
	}
	err = srt_f_get_corr_from_CorrList(sCorrlist, und3dfx, forname, time[nstp-1], &(corr_for_fx[nstp-1]));
	if (err)
	{
		goto FREE_RETURN;
	}
	...
	for (i=nstp-2; i>=0; i--)
	{
		...
		fx_vol[i] = find_fx_sig (time[i], fx_ts) * exp (alpha * sqrt (time[i]) - 0.5 * alpha * alpha * time[i]);

		err = srt_f_get_corr_from_CorrList(sCorrlist, domname, forname, time[i], &(corr_dom_for[i]));
		if (err) goto FREE_RETURN;
		err = srt_f_get_corr_from_CorrList(sCorrlist, domname, und3dfx, time[i], &(corr_dom_fx[i]));
		if (err) goto FREE_RETURN;
		err = srt_f_get_corr_from_CorrList(sCorrlist, und3dfx, forname, time[i], &(corr_for_fx[i]));
		if (err) goto FREE_RETURN;
	...
	for (i=0; i<nstp; i++)
	{
		err = Fx3DFwdBeta (und3dfx, time[i], time[i], 0, &(fx_fwd[i]));
		if (err)
		{
			goto FREE_RETURN;
		}
	...
	*/
	...
	for (i=nstp-1; i>=0; i--)
	{
		...
		{
			next_bar =
				(next_bar + (for_ifr[i-1] + for_fwd[i-1] - dom_ifr[i-1] - dom_fwd[i-1]) * (time[i] - time[i-1]))
				 / (1.0 - beta2 * (for_ifr[i-1] + for_fwd[i-1] - dom_ifr[i-1] - dom_fwd[i-1])* (time[i] - time[i-1]));
		}		
	}
						  ...
						  double	***prod_val)
{
	...
	*prod_val = dmatrix(0, *num_prod+1-1, 0, 1);
	...
	beta_tab[0] = beta;
	err = srt_f_get_corr_from_CorrList(sCorrlist, domname, forname, 0.0, &(corr_dom_for[0]));
	if (err) goto FREE_RETURN;
	err = srt_f_get_corr_from_CorrList(sCorrlist, domname, und3dfx, 0.0, &(corr_dom_fx[0]));
	if (err) goto FREE_RETURN;
	err = srt_f_get_corr_from_CorrList(sCorrlist, und3dfx, forname, 0.0, &(corr_for_fx[0]));
	if (err) goto FREE_RETURN;
	...
	for (i=0; i<*num_prod; i++)
	{		
		(*prod_val)[i][1] = sqrt (((*prod_val)[i][1] - (*prod_val)[i][0] * (*prod_val)[i][0]) / nbPaths);
	}
	...
	for (i=nstp-1; i>=0; i--)
	{
		if (j >= 0 && fabs (date[i] - xStr.dts[j]) < 1.0e-04)
		{
			...
			if (grfn_prm->num_fx_df > 0 && grfn_prm->fx_idx != -1)
			{
				...
				for (k=0; k<grfn_prm->num_fx_df; k++)
				{
					...
					grfn_prm->fx_gam[k] = (1.0 - exp ( - dom_lam * grfn_prm->fx_df_tms[k] )) / dom_lam;
					grfn_prm->fx_gam2[k] = 0.5 * grfn_prm->fx_gam[k] * grfn_prm->fx_gam[k] * dom_phi[i];
				}
			...
			if (grfn_prm->num_dom_df > 0 && grfn_prm->dom_idx != -1)
			{
				...
				for (k=0; k<grfn_prm->num_dom_df; k++)
				{
					...
					grfn_prm->dom_gam[k] = (1.0 - exp ( - dom_lam * grfn_prm->dom_df_tms[k] )) / dom_lam;
					grfn_prm->dom_gam2[k] = 0.5 * grfn_prm->dom_gam[k] * grfn_prm->dom_gam[k] * dom_phi[i];
				}
			...
			if (grfn_prm->num_for_df > 0 && grfn_prm->for_idx != -1)
			{
				...
				for (k=0; k<grfn_prm->num_for_df; k++)
				{
					...
					grfn_prm->for_gam[k] = (1.0 - exp ( - for_lam * grfn_prm->for_df_tms[k] )) / for_lam;
					grfn_prm->for_gam2[k] = 0.5 * grfn_prm->for_gam[k] * grfn_prm->for_gam[k] * for_phi[i];
				}
		...
		{				
			...
			if (grfn_prm->num_fx_df > 0 && grfn_prm->fx_idx != -1)
			{
				...
				for (k=0; k<grfn_prm->num_fx_df; k++)
				{
					grfn_prm->fx_dff[k] = swp_f_df (date[i], grfn_prm->fx_df_dts[k], (char*) dom_yc);
					grfn_prm->fx_gam[k] = (1.0 - exp ( - dom_lam * (grfn_prm->fx_df_dts[k] - date[i]) * YEARS_IN_DAY )) / dom_lam;
					grfn_prm->fx_gam2[k] = 0.5 * grfn_prm->fx_gam[k] * grfn_prm->fx_gam[k] * dom_phi[i];
				}
			...
			if (grfn_prm->num_dom_df > 0 && grfn_prm->dom_idx != -1)
			{
				...
				for (k=0; k<grfn_prm->num_dom_df; k++)
				{
					grfn_prm->dom_dff[k] = swp_f_df (date[i], grfn_prm->dom_df_dts[k], (char*) dom_yc);
					grfn_prm->dom_gam[k] = (1.0 - exp ( - dom_lam * (grfn_prm->dom_df_dts[k] - date[i]) * YEARS_IN_DAY)) / dom_lam;
					grfn_prm->dom_gam2[k] = 0.5 * grfn_prm->dom_gam[k] * grfn_prm->dom_gam[k] * dom_phi[i];
				}
			...
			if (grfn_prm->num_for_df > 0 && grfn_prm->for_idx != -1)
			{
				...
				for (k=0; k<grfn_prm->num_for_df; k++)
				{
					grfn_prm->for_dff[k] = swp_f_df (date[i], grfn_prm->for_df_dts[k], (char*) for_yc);
					grfn_prm->for_gam[k] = (1.0 - exp ( - for_lam * (grfn_prm->for_df_dts[k] - date[i]) * YEARS_IN_DAY)) / for_lam;
					grfn_prm->for_gam2[k] = 0.5 * grfn_prm->for_gam[k] * grfn_prm->for_gam[k] * for_phi[i];
				}

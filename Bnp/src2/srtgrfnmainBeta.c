    ...
    double** prod_val)
{
    ...
    int*    vol_change = NULL;
    double *dom_vol = NULL, *for_vol = NULL, *fx_vol = NULL, *sig_fx_approx = NULL,
           *fx_fwd_approx = NULL;

    double *dom_ifr = NULL, *dom_fwd = NULL, *dom_var = NULL, *for_ifr = NULL, *for_fwd = NULL,
           *for_var = NULL, *fx_fwd = NULL, *fx_var = NULL, *corr_dom_for = NULL,
    ...
    err =
        fill_time_vector(&time, &nstp, num_bar_times, bar_times, num_vol_times, vol_times, num_stp);
    ...
    if (!vol_change || !dom_vol || !for_vol || !fx_vol || !corr_dom_for || !corr_dom_fx ||
        !corr_for_fx || !beta_tab)
    {
    ...
    err = srt_f_get_corr_from_CorrList(
        sCorrlist, domname, forname, time[nstp - 1], &(corr_dom_for[nstp - 1]));
    if (err)
    {
        goto FREE_RETURN;
    }
    err = srt_f_get_corr_from_CorrList(
        sCorrlist, domname, und3dfx, time[nstp - 1], &(corr_dom_fx[nstp - 1]));
    if (err)
    {
        goto FREE_RETURN;
    }
    err = srt_f_get_corr_from_CorrList(
        sCorrlist, und3dfx, forname, time[nstp - 1], &(corr_for_fx[nstp - 1]));
    if (err)
    {
        goto FREE_RETURN;
    }
    ...
    for (i = nstp - 2; i >= 0; i--)
    {
        ...
        err =
            srt_f_get_corr_from_CorrList(sCorrlist, domname, forname, time[i], &(corr_dom_for[i]));
        if (err)
            goto FREE_RETURN;
        ...
        if (fabs(dom_vol[i] - dom_vol[i + 1]) + fabs(for_vol[i] - for_vol[i + 1]) +
                fabs(fx_vol[i] - fx_vol[i + 1]) + fabs(corr_dom_for[i] - corr_dom_for[i + 1]) +
    ...
    if (!dom_ifr || !dom_fwd || !dom_var || !for_ifr || !for_fwd || !for_var || !fx_fwd ||
        ...
        if ((i > 0) && (fabs(next_bar) > 1.0e-08))
        {
            next_bar =
                (next_bar + (for_ifr[i - 1] + for_fwd[i - 1] - dom_ifr[i - 1] - dom_fwd[i - 1]) *
                                (time[i] - time[i - 1])) /
                (1.0 - beta2 * (for_ifr[i - 1] + for_fwd[i - 1] - dom_ifr[i - 1] - dom_fwd[i - 1]) *
                           (time[i] - time[i - 1]));
        }
    }
    ...
    double*** prod_val)
{
    ...
    *prod_val    = dmatrix(0, *num_prod + 1 - 1, 0, 1);
    ...
    {
        (*prod_val)[i][1] =
            sqrt(((*prod_val)[i][1] - (*prod_val)[i][0] * (*prod_val)[i][0]) / nbPaths);
    }
    ...
    double*** prod_val)
{
    ...
    double *sigma_date_dom = NULL, *sigma_dom = NULL, *tau_date_dom = NULL, *tau_dom = NULL,
           ...
           *for_fx_cov = NULL, *correl_mat = NULL, *correl_dom_for = NULL, *correl_dom_fx = NULL,
           *correl_for_fx = NULL;
    ...
    if (!sig_dom || !sig_for || !sig_fx || !dom_vol || !for_vol || !fx_vol || !corr_dom_for ||
        !corr_dom_fx || !corr_for_fx || !dom_ifr || !dom_phi || !for_ifr || !for_phi)
    {
        ...
        else if (j >= 0 && xStr.am[j])
        {
            ...
            if (grfn_prm->num_fx_df > 0 && grfn_prm->fx_idx != -1)
            {
                ...
                for (k = 0; k < grfn_prm->num_fx_df; k++)
                {
                    ...
                    grfn_prm->fx_gam[k] =
                        (1.0 - exp(-dom_lam * (grfn_prm->fx_df_dts[k] - date[i]) * YEARS_IN_DAY)) /
                        dom_lam;
                    grfn_prm->fx_gam2[k] =
                        0.5 * grfn_prm->fx_gam[k] * grfn_prm->fx_gam[k] * dom_phi[i];
                }
            ...
            if (grfn_prm->num_dom_df > 0 && grfn_prm->dom_idx != -1)
            {
                ...
                for (k = 0; k < grfn_prm->num_dom_df; k++)
                {
                    ...
                    grfn_prm->dom_gam[k] =
                        (1.0 - exp(-dom_lam * (grfn_prm->dom_df_dts[k] - date[i]) * YEARS_IN_DAY)) /
                        dom_lam;
                    grfn_prm->dom_gam2[k] =
                        0.5 * grfn_prm->dom_gam[k] * grfn_prm->dom_gam[k] * dom_phi[i];
                }
            ...
            if (grfn_prm->num_for_df > 0 && grfn_prm->for_idx != -1)
            {
                ...
                for (k = 0; k < grfn_prm->num_for_df; k++)
                {
                    ...
                    grfn_prm->for_gam[k] =
                        (1.0 - exp(-for_lam * (grfn_prm->for_df_dts[k] - date[i]) * YEARS_IN_DAY)) /
                        for_lam;
                    grfn_prm->for_gam2[k] =
                        0.5 * grfn_prm->for_gam[k] * grfn_prm->for_gam[k] * for_phi[i];
                }

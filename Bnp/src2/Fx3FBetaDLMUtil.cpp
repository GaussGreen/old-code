/*	==========================================================================
        ...
        PURPOSE:	Modelling of the FFX(t  ,T*) as a log-quadratic function
   of a gaussian variable:

                                FFX(t  , T*) = FFX(0  , T*) * Exp[A(t) + B(t)*Xt
   + C(T)*Xt^2]
  ...
  do {
    ...
    *RhoFX = RhoFX1;
    ...
    if (fabs(RhoDS) > 1.0E-08) {
      ...
      if (delta < 0.0) {
        err = serror(
            "No solution for correlation mapping  , delta < 0 for t = %f", t);
        return err;
      }
  ...
  do {
    ...
    *RhoFX = RhoFX1;
    ...
    if (fabs(RhoDS) > 1.0E-08) {
      ...
      if (fabs(RhoDX1) > vol_X * 0.999) {
        err = "No solution for correlation mapping  , correlation dom X not in "
              "[-1;1]";
        return err;
      }
      ...
      if (fabs(RhoFX1) > vol_X * 0.999) {
        err = "No solution for correlation mapping  , correlation For|X not in "
              "[-1;1]";
        return err;
      }
    }
...
Err FxBetaDLM_correl_mapping3(int index, double VarFFx, double volX,
  ...
  for (step = 0; step < 2; step++) {
    ...
    if (fabs(RhoDS) > 1.0E-08) {
      ...
      if (fabs(RhoDX1) > vol_X * 0.999) {
        err = serror("No solution for correlation mapping  , correlation dom_X "
                     "not in [-1;1] for t = %f",
                     t);
        return err;
      }
      ...
      if (fabs(RhoFX1) > vol_X * 0.999) {
        err = serror("No solution for correlation mapping  , correlation for_X "
                     "not in [-1;1] for t = %f",
                     t);
        return err;
      }

	...
	else
	{
		if(strike < forward)
		{
			...
			else
			{
				...
				err = vol_conv(*vol, SABR_STR_LOG, vol, SABR_ATM_BETA, forward, modstrike, mat, 0.0, lowbetacut, 0.0);
				err = srt_f_optsarbvol(forward, strike, mat, *vol, 0.0, lowbetacut, 0.0, SRT_BETAVOL, output, vol);
		...
		else if(strike > forward)
		{
			...
			else
			{
				...
				err = vol_conv(*vol, SABR_STR_LOG, vol, SABR_ATM_BETA, forward, modstrike, mat, 0.0, highbetacut, 0.0);
				err = srt_f_optsarbvol(forward, strike, mat, *vol, 0.0, highbetacut, 0.0, SRT_BETAVOL, output, vol);
			}
        ...
        )
{
    ...
    *alpha_adj = 0.0;
    ...
    t = (double *) malloc( n * sizeof(double) ); if( t == NULL ) { err = serror(memErrMsg); goto free_return; }
    t[n-1] = maturity;
    t[0] = t_init;
    for( i = n-2; i != 0; --i )
    {
    ...
    g = (double *) malloc( n * sizeof(double) ); if( g == NULL ) { err = serror(memErrMsg); goto free_return; }
    for( i = n-1; i != -1; --i )
    {
    ...
    h = (double *) malloc( n * sizeof(double) ); if( h == NULL ) { err = serror(memErrMsg); goto free_return; }
    h[n-1] = 0.0;
    for( i = n-2; i != -1; --i )
    {
    ...
    // Quanto adjusted term structures of sigma (the forward's vol) and rho (correlation between the forward and its vol)
    sigma = (double *) malloc( n * sizeof(double) ); if( sigma == NULL ) { err = serror(memErrMsg); goto free_return; }
    rho   = (double *) malloc( n * sizeof(double) ); if( rho == NULL ) { err = serror(memErrMsg); goto free_return; }
    for( i = n-1; i != -1; --i )
    {
        c0 = alpha*h[i];
    ...
    y1 = (double *) malloc( n * sizeof(double) ); if( y1 == NULL ) { err = serror(memErrMsg); goto free_return; }
    y2 = (double *) malloc( n * sizeof(double) ); if( y2 == NULL ) { err = serror(memErrMsg); goto free_return; }
    y3 = (double *) malloc( n * sizeof(double) ); if( y3 == NULL ) { err = serror(memErrMsg); goto free_return; }
    y4 = (double *) malloc( n * sizeof(double) ); if( y4 == NULL ) { err = serror(memErrMsg); goto free_return; }
    y5 = (double *) malloc( n * sizeof(double) ); if( y5 == NULL ) { err = serror(memErrMsg); goto free_return; }
    y6 = (double *) malloc( n * sizeof(double) ); if( y6 == NULL ) { err = serror(memErrMsg); goto free_return; }
    y7 = (double *) malloc( n * sizeof(double) ); if( y7 == NULL ) { err = serror(memErrMsg); goto free_return; }
    ...
    for( i = n-2; i != -1; --i )
    {
        ...
        y5[i] = y5[i+1] + dt * (alpha * alpha * y1[i+1] * y1[i+1] + 6.0 * alpha * rho[i+1] * sigma[i+1] * y2[i+1]);
        y6[i] = y6[i+1] + dt * (2.0 * sigma[i+1] * sigma[i+1] * y2[i+1] + alpha * rho[i+1] * sigma[i+1] * y4[i+1]);
        y7[i] = y7[i+1] + dt * sigma[i+1] * sigma[i+1] * y4[i+1];
    }

    // And finally...
    eta_hat = 2.0 * y2[0] / (y1[0] * y1[0]);
    upsilon_hat = 3.0 * (y5[0] - 3.0*eta_hat*(y6[0] - eta_hat*y7[0])) / (y1[0] * y1[0] * y1[0]);
    ...
    err	= srt_f_optsarbvol(F_init, strike, maturity, sigma_hat, alpha_hat, beta, rho_hat, SRT_BETAVOL, output, &implied_vol);

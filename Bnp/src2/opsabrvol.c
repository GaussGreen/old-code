...
*
*
* PURPOSE      	: A Quick vol transformation to go from a BS vol to a BS beta vol and vice versa
*
        ...
        if (strike < forward)
        {
            if (lowbetacut == 1)
            {
                ...
                else if (output == SRT_NORMAL)
                {
                    ...
                    price = srt_f_optblksch(forward, strike, *vol, mat, 1.0, SRT_CALL, PREMIUM);

                    err = srt_f_optimpvol(
                        price, forward, strike, mat, 1.0, SRT_CALL, SRT_NORMAL, vol);
                }
            ...
            else if (lowbetacut == 0)
            {
                ...
                else if (output == SRT_LOGNORMAL)
                {
                    ...
                    price = srt_f_optblknrm(forward, strike, *vol, mat, 1.0, SRT_CALL, PREMIUM);

                    err = srt_f_optimpvol(
                        price, forward, strike, mat, 1.0, SRT_CALL, SRT_LOGNORMAL, vol);
                }
                else if (output == SRT_BETAVOL)
                {
                    err = bsvol(
            ...
            else
            {
                ...
                err = srt_f_optsarbvol(
                    forward, strike, mat, *vol, 0.0, lowbetacut, 0.0, SRT_BETAVOL, output, vol);
            }
        }
            ...
            else
            {
                ...
                err = vol_conv(
                    *vol,
                    ...
                    0.0);
                err = srt_f_optsarbvol(
                    forward, strike, mat, *vol, 0.0, highbetacut, 0.0, SRT_BETAVOL, output, vol);
            }
        }
        else
        {
    ...
    double*          vol)
{
    ...
    else if (alpha == 0.0 && beta < 1.0E-08)
    {
        ...
        else if (input == SRT_LOGNORMAL)
        {
            ...
            price = srt_f_optblksch(forward, forward, volinput, mat, 1.0, SRT_CALL, PREMIUM);

            err =
                srt_f_optimpvol(price, forward, forward, mat, 1.0, SRT_CALL, SRT_NORMAL, &tempvol);
        }
        else
        {
        ...
        {
            price = srt_f_optblknrm(forward, strike, tempvol, mat, 1.0, SRT_CALL, PREMIUM);

            err = srt_f_optimpvol(price, forward, strike, mat, 1.0, SRT_CALL, SRT_LOGNORMAL, vol);
        }
        else
        {
    ...
    else if (alpha == 0)
    {
        ...
        {
            err = srt_f_optbetavoltoblkvol(forward, strike, volinput, mat, beta, &tempvol);
            ...
            if (output == SRT_NORMAL)
            {
                /* 	err = srt_f_optblkvoltobetavol(
                                                                                ...
                                                                                0.0,
                                                                                vol); */
                price = srt_f_optblksch(forward, strike, tempvol, mat, 1.0, SRT_CALL, PREMIUM);

                err = srt_f_optimpvol(price, forward, strike, mat, 1.0, SRT_CALL, SRT_NORMAL, vol);
            }
            else
                *vol = tempvol;
        }
        ...
        {
            err = srt_f_optblkvoltobetavol(forward, forward, volinput, mat, beta, &tempvol);
            ...
            {
                err = srt_f_optbetavoltoblkvol(forward, strike, tempvol, mat, beta, &tempvol2);
                if (output == SRT_NORMAL)
                {
                    err = srt_f_optblkvoltobetavol(forward, strike, tempvol2, mat, 0.0, vol);
                }
                else
                    *vol = tempvol2;
            }
        ...
        else if (input == SRT_NORMAL)
        {
            /* err = srt_f_optbetavoltoblkvol(
                                                                            ...
                                                                            0.0,
                                                                            &tempvol); */
            price = srt_f_optblknrm(forward, forward, volinput, mat, 1.0, SRT_CALL, PREMIUM);

            err = srt_f_optimpvol(
                price, forward, forward, mat, 1.0, SRT_CALL, SRT_LOGNORMAL, &tempvol);

            err = srt_f_optblkvoltobetavol(forward, forward, tempvol, mat, beta, &tempvol2);
            ...
            {
                err = srt_f_optbetavoltoblkvol(forward, strike, tempvol2, mat, beta, &tempvol);
    ...
    else
    {
        if (input == SRT_BETAVOL)
        {
            err = srt_f_optbetastochvoltoblkvol(
                forward, strike, volinput, alpha, rho, mat, beta, &tempvol);
        ...
        else if ((input == SRT_LOGNORMAL))
        {
            err = srt_f_optblkvolATMtobetavolStochVol(
                forward, forward, volinput, mat, alpha, beta, rho, &tempvol);
            ...
            {
                err = srt_f_optbetastochvoltoblkvol(
                    forward, strike, tempvol, alpha, rho, mat, beta, &tempvol2);
                if (output == SRT_NORMAL)
                {
                    /*	err = srt_f_optblkvoltobetavol(
                                                                            forward,
                                                                            ...
                                                                            0.0,
                                                                            vol); */
                    price = srt_f_optblksch(forward, strike, tempvol2, mat, 1.0, SRT_CALL, PREMIUM);
        ...
        else if ((input == SRT_NORMAL))
        {
            ...
            err = srt_f_optblkvolATMtobetavolStochVol(
                forward, forward, tempvol, mat, alpha, beta, rho, &tempvol2);

            if ((output == SRT_LOGNORMAL) || (output == SRT_NORMAL))
            {
                err = srt_f_optbetastochvoltoblkvol(
                    forward, strike, tempvol2, alpha, rho, mat, beta, &tempvol);
                if (output == SRT_NORMAL)
                {
                    /*	err = srt_f_optblkvoltobetavol(
                                                                                            forward,
    ...
    double*          vol)
{
    ...
    err = srt_f_optsarbvol(forward, strike, mat, volinput, modalpha, beta, rho, input, output, vol);
...
Err srt_f_optsabrvolq(
    ...
    double           volFwd,      // sigma
    double           alpha,       // alpha
    double           beta,        // beta TO DO: note that this is converted to shifted-log params
...
)
{
    ...
    Err    err       = NULL;
    char*  memErrMsg = "FATAL ERROR: Insufficient memory to create array. In srt_f_optsabrvolq()";
    double t_init    = 0.0;
    double y_init    = 1.0;
    double Y_init;
    double F_init;
    ...
    *alpha_adj   = 0.0;
    ...
    // We need to do some numerical integration, so let's define a uniform grid
    // of points between t_init and maturity
    ...
    // Quanto adjusted term structures of sigma (the forward's vol) and rho (correlation between the
    // forward and its vol)
    ...
    y1[n - 1] = y2[n - 1] = y3[n - 1] = y4[n - 1] = y5[n - 1] = y6[n - 1] = y7[n - 1] = 0.0;
    for (i = n - 2; i != -1; --i)
    {
        dt    = t[i + 1] - t[i];
        ...
        y5[i] = y5[i + 1] + dt * (alpha * alpha * y1[i + 1] * y1[i + 1] +
    ...
    upsilon_hat =
        3.0 * (y5[0] - 3.0 * eta_hat * (y6[0] - eta_hat * y7[0])) / (y1[0] * y1[0] * y1[0]);
    sigma_hat   = (y1[0] + y3[0] - 0.5 * upsilon_hat * y1[0] * y1[0]) / (maturity - t_init);
    upsilon_hat = sqrt(upsilon_hat);
    sigma_hat   = sqrt(sigma_hat);
    alpha_hat   = upsilon_hat * sigma_hat;
    rho_hat     = eta_hat / upsilon_hat;

    // Now we can look up the implied vol at the strike using the above SABR parameters
    err = srt_f_optsarbvol(
        F_init,
        strike,
        maturity,

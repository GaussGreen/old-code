...
char *SrtInitIRUnd(char *undName, char *ycName, char *model, int volCrvRows,
  ...
  if (!PRL_CONTEXT.bLocalComp)
    SendInitIRUndData(undName, ycName, model, volCrvRows, volCrvCols,
                      volCrvVals, tauCrvRows, tauCrvCols, tauCrvVals, beta,
                      alpha, gamma, rho, vovol, etaOrBeta2);
#endif /*PVMPI*/
  /* Get the Yield Curve */
  ...
  err = srt_f_init_IRM_TermStruct(
      today, volCrvVals, volCrvCols, volCrvRows, tauCrvVals, tauCrvCols,
      tauCrvRows, modelType, modelDim, beta, alpha, gamma, rho, vovol,
      etaOrBeta2, vasicek_init_cond, vasicek_mean_rev_level_rows,
      vasicek_mean_rev_level_cols, vasicek_mean_rev_level_vals, &ts);
  ...
  err = srt_f_addundtolist(und_list, undName, "IR_UND", ccy, model, ycName,
                           NULL, NULL, ts, 0.0);
  if (err)

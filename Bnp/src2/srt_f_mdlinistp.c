...
/* We need the Yield Curve for the Y_t_at_T call */
	...
	/* Get the time info */
		...
		{
			err = srt_f_vasicek_discount_factor(stp->date,
												und,
												&tminf->df);
			if(err) return err;
	...
	/* Sets the instantaneous forward rate for that date */
		...
		{
			if(mdl_type != VASICEK) 
				sam_get(tminf->fwd_sam,und_index,F_0_t) = swp_f_zr(stp->ddate,stp->next->ddate,yc_name);
			else
			{
				err = srt_f_vasicek_cont_fwd_zr(stp->ddate,
												stp->next->ddate,
												und,
		...
		{
			if(mdl_type != VASICEK) 
				sam_get(tminf->fwd_sam,und_index,F_0_t) = swp_f_zr(stp->ddate,stp->ddate + 14.0 ,yc_name);
			else 
			{
				err = srt_f_vasicek_cont_fwd_zr(stp->ddate,
												stp->ddate + 14.0,
		...
		/* Two factor models: put the vols, mean reversion,... */
			case TWO_FAC:
				...
				err = find_2f_tau(stp->time,ts,
						&tminf->ev.twof[0].tau,&tminf->ev.twof[1].tau); 
...
*/
		...
		sam_get(tminf->fwd_sam,und_index,PHI) = tminf->rf.onef.G 
				* tminf->ev.onef.F  * tminf->ev.onef.F ;

		/* compute the integral of PHI for stochastic rate & volatility gamma smile model */
		if(cur->prev)	dt = (cur->time-cur->prev->time);
		else dt = cur->time;

      	if(prvtminf)
			tminf->rf.onef.stdev_x = tminf->ev.onef.F*sqrt(tminf->rf.onef.G - prvtminf->rf.onef.G);
	   ...
	   case TWO_FAC:
		...
		sam_get(tminf->fwd_sam,und_index,CROSSPHI) = G[1][0] * F[1] * F[0]; 
      		...
      		if(prvtminf)
		{
		   tminf->rf.twof[0][0].stdev_x = F[0] * 
				sqrt( G[0][0] - prvtminf->rf.twof[0][0].G);
		   tminf->rf.twof[1][1].stdev_x = F[1]*
				sqrt(G[1][1] - prvtminf->rf.twof[1][1].G);
...
/* Sets the volatility */
	while(stp->next)
	{
		...
		if (und_type == EQUITY_UND)
		{
			tminf->int_sig_dt = eq_cum_vol_func(stp->next->time,SRT_NO,ts) - eq_cum_vol_func(stp->time,SRT_NO,ts) ;
			tminf->int_sig2_dt = eq_cum_vol_func(stp->next->time,SRT_YES,ts) - eq_cum_vol_func(stp->time,SRT_YES,ts);
		...
		if (und_type == FOREX_UND)
		{
			...
			else tminf->inv_exp_int_spread_dt = 1.0;
...
/* Sets the forward (as of today) using the second growth curve */    
	...
	while(stp)
	{
		...
		if (obj_type2 == DVD_CURVE)
		{
			tminf->init_fwd_val = spot * srt_f_forward_from_fwdcrv(stp->time,dvdcrv,repocrv)/ tminf->df;
		...
		/* For FX, the forward is computed using the foreign yield curve input and the discount one */	
...
/* Check the FX underlying is defined */
	...
	/* If not found: Build the second possible FX name: "DOM/FOR" */
		...
		fx_und = lookup_und(FX_name);
		if (fx_und == NULL)
			return serror("Can't find the FX for %s/%s or even %s/%S ",und_ccy,numeraire_ccy,numeraire_ccy,und_ccy);
	}
...
/* Gets the Global correlation list (not only the deal one: the FX could be defined but not used) */
...
/* Populate all the time steps of the dicretisation */
	...
	/* If LGM and Jumping: special quanto adjustment */
		...
		{
		/* Get the correlation between the FX underlying and the foreign one (from the Global Correlation Curve) */
			err = srt_f_get_corr_from_CorrList( cls,		
											FX_name,									    
											und_name,									 
											time,
											&correlation);	
...
==============================================================================================*/
 ...
 Err srt_f_fx_initstp(SrtStpPtr	stp,
					   SrtUndInfo	*und_info,
					   SrtUndPtr	und,
					   SrtUndPtr	dom_und,
					   SrtUndPtr	for_und,
...
Ddate				today;
double				dom_Phi,dom_Psi,dom_F,dom_G,dom_H,dom_lambda_dt,next_dom_G,next_dom_H,dom_G_dt,next_dom_Psi,next_dom_F;
double				for_Psi,for_F,for_G,for_H,next_for_G,next_for_H,for_G_dt,next_for_Psi,next_for_F,for_lambda_dt;
...
/* Allocate space for a new correlation matrix with values at each time step (now we look at the Fx Forward) */
	num_time_points = 0;
	...
	while(stp->next)
	{
		...
		mean_int_dom_sr_dt = -log(nexttminf->StochRatesVal[0].df) + log(tminf->StochRatesVal[0].df)
		...
		cov_int_dom_for = next_dom_Psi*next_for_Psi*Ofd_dt - next_for_Psi*Rfd_dt - next_dom_Psi*Pfd_dt + Mfd_dt;

		mean_int_for_sr_dt = -log(nexttminf->StochRatesVal[1].df) + log(tminf->StochRatesVal[1].df)
		...
		var_ln_fx =  var_int_dom_sr + var_int_for_sr  + var_int_fx -2*cov_int_dom_for + 2*cov_int_dom_fx - 2*cov_int_for_fx;
...
/* Replaces the current correlation matrix by the new one (with coeffcients done inside) in UndInfo */
	...
	err= srt_f_make_deal_corrlist( 	used_und_names, 

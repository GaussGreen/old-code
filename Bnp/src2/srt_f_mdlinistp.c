...
/* ------------------------------------------------------------------------------
   ...
   (und_index corresponds to the underlying und_index  , needed to extract the
  correct tminf from the stp)
  -----------------------------------------------------------------------------
*/
  ...
  while (stp) {
    ...
    if (obj_type2 == DVD_CURVE) {
      ...
      tminf->ajust_init_fwd_val =
        ...
        /* For Equity  , the forward is computed using ONLY the second yield
           curve input */
        if (und_type == EQUITY_UND) {
      ...
      tminf->ajust_init_fwd_val =
        ...
        /* For FX  , the forward is computed using the foreign yield curve input
           and the discount one */

...
/* ------------------------------------------------------------------------------
   ...
   (und_index corresponds to the underlying und_index, needed to extract the correct
   tminf from the stp)
  ----------------------------------------------------------------------------- */
    ...
    SrtUndInfo* und_info)
{
    ...
    while (stp)
    {
        ...
        if (stp->next)
        {
            ...
            else
            {
                err = srt_f_vasicek_cont_fwd_zr(
                    stp->ddate, stp->next->ddate, und, &sam_get(tminf->fwd_sam, und_index, F_0_t));
                if (err)
                    return err;
            }
        }
        else
        {
            ...
            {
                err = srt_f_vasicek_cont_fwd_zr(
                    stp->ddate, stp->ddate + 14.0, und, &sam_get(tminf->fwd_sam, und_index, F_0_t));
                if (err)
                    return err;
            }
        }
        ...
        case ONE_FAC:
            if (stp->next)
                ...
                tminf->ev.onef.sig2 = find_sig2_interp(stp->time, stp->time + 14.0 / 365.0, ts);
        ...
        case TWO_FAC:
            ...
            err = find_2f_tau(stp->time, ts, &tminf->ev.twof[0].tau, &tminf->ev.twof[1].tau);
            tminf->ev.twof[0].lambda = 1.0 / tminf->ev.twof[0].tau;
            tminf->ev.twof[1].lambda = 1.0 / tminf->ev.twof[1].tau;

            find_tf_beta(stp->time, ts, &(tminf->ev.twof[0].beta), &(tminf->ev.twof[1].beta));
        ...
        if (prvtminf && mdl_dim == ONE_FAC && ((mdl_type == CHEY) || (mdl_type == CHEY_BETA)))
        {
            srt_f_CheyBeta_drift_at_sam(
                mdl_type, stp->prev, &(prvtminf->fwd_sam), &(tminf->fwd_sam), und_index);
        }
        else
        {
            switch (mdl_dim)
    ...
    err = srt_f_steps_set_quanto_adjustment(top, und_info, und_index, und, numeraire_und);
    if (err)
        return err;
...
/* ------------------------------------------------------------------------
   ...
   DESCRIPTION     :puts phi (which is deterministic),F,G,Psi on nodes
     on step for LGM model; these values are computed from the ts on mdl;
     really the ts should be located on a SrtUndPtr.  Note that we regard the
LGM as a subset of the Cheyette model (with beta = 0), justifying the use of
Cheyette structures; this function should be called after srt_f_cheinistp
    ...
    SrtUndInfo* und_info)
{
    ...
    /* init. of the integrale of PHI stochastic rate & volatility gamma smile model  */
    ...
    while (cur)
    {
        ...
        case ONE_FAC:
            ...
            */
            ...
            /* compute the integral of PHI for stochastic rate & volatility gamma smile model */
            if (cur->prev)
        ...
        case TWO_FAC:
            ...
            if (prvtminf)
            {
                tminf->rf.twof[0][0].stdev_x = F[0] * sqrt(G[0][0] - prvtminf->rf.twof[0][0].G);
                tminf->rf.twof[1][1].stdev_x = F[1] * sqrt(G[1][1] - prvtminf->rf.twof[1][1].G);
                /* Here, [1][0].stedev_r is the correlation, no the covariance...*/
                tminf->rf.twof[1][0].stdev_x = (G[0][1] - prvtminf->rf.twof[0][1].G) /
                                               sqrt(
                                                   (G[0][0] - prvtminf->rf.twof[0][0].G) *
...
Err srt_f_betaetainistp(SrtStpPtr stp, TermStruct* ts, int und_index, SrtMdlDim mdl_dim)
{
    SrtStpPtr    cur, first;
    SrtIRMTmInf *tminf, *prvtminf;
    Err          err = NULL;
...
Err srt_f_loginistp(
    SrtStpPtr stp, SrtUndPtr und, int und_index, SrtUndPtr numeraire_und, SrtUndInfo* und_info)
{
    SrtStpPtr         top;
    Err               err;
    ...
    else if ((mdl_type == EQ_STOCH_RATES) || (mdl_type == EQ_STOCH_RATES_SRVGS))
    {
        /* Get the domestic underlying name and pointer to */
    ...
    while (stp)
    {
        ...
        {
            tminf->init_fwd_val =
                spot * srt_f_forward_from_fwdcrv(stp->time, dvdcrv, repocrv) / tminf->df;

            tminf->vol2_cum           = eq_cum_vol_func(stp->time, SRT_YES, ts);
            tminf->ajust_init_fwd_val = tminf->init_fwd_val * exp(-0.5 * tminf->vol2_cum);
        }
        else

            /* For Equity, the forward is computed using ONLY the second yield curve input */
            if (und_type == EQUITY_UND)
        {
            ...
            /* For FX, the forward is computed using the foreign yield curve input and the discount
               one */
            if ((und_type == FOREX_UND) && !(mdl_type == FX_STOCH_RATES))
        {
            tminf->init_fwd_val       = spot * swp_f_df(today, stp->ddate, crv2) / tminf->df;
    ...
    while (stp->next)
    {
        ...
        tminf->drift = log(nxttminf->init_fwd_val / tminf->init_fwd_val) / stp->delta_t;
        stp          = stp->next;
    }
...
/* -------------------------------------------------------------------------
   ...
   The Name of the FX underlying should be FOR/DOM
   The Qunato Adjustement is stored in the TmInf attached to the SrtStp
   ------------------------------------------------------------------------- */

Err srt_f_steps_set_quanto_adjustment(
    SrtStpPtr stp, SrtUndInfo* und_info, int und_index, SrtUndPtr und, SrtUndPtr numeraire_und)
{
    TermStruct*       ts;     /* Term structure for for_und*/
    TermStruct*       fx_ts;  /* Term structure for FX_und*/
    TermStruct*       dom_ts; /* Term structure for dom_und (need it in Jumping Numeraire)*/
    Err               err;
    double            correlation;
    ...
    /* Gets the Currency of this underlying (not necessarily different from domestic...) */
    ...
    /* For a normal quanto, the adjustemnt is - rho(fx,for) * fx_vol */
    ...
    if (fx_und == NULL)
    {
        ...
        if (fx_und == NULL)
            return serror(
                "Can't find the FX for %s/%s or even %s/%S ",
                und_ccy,
                numeraire_ccy,
                numeraire_ccy,
                und_ccy);
    ...
    /* Gets the Global correlation list (not only the deal one: the FX could be defined but not
     * used) */
    cls = srt_f_GetTheCorrelationList();

    /* Extends the Undelying Term Structure if LGM */
    if (((eModelType == LGM) && (eModelDim == ONE_FAC)) && (und_info->jumping == SRT_YES))
    {
    ...
    while (stp->next)
    {
        ...
        if ((und_type == INTEREST_RATE_UND) && (eModelType == LGM) && (eModelDim == ONE_FAC) &&
        ...
        {
            /* Get the correlation between the FX underlying and the foreign one (from the Global
             * Correlation Curve) */
            err = srt_f_get_corr_from_CorrList(cls, FX_name, und_name, time, &correlation);
            if (err)
                return (err);
...
/* ===========================================================================================
                                                        ...
                                                        FX STOCHASTIC RATES WITH JUMPING NUMERAIRE
...

{
    ...
    Ddate             today;
    double dom_Phi, dom_Psi, dom_F, dom_G, dom_H, dom_lambda_dt, next_dom_G, next_dom_H, dom_G_dt,
        next_dom_Psi, next_dom_F;
    double for_Psi, for_F, for_G, for_H, next_for_G, next_for_H, for_G_dt, next_for_Psi, next_for_F,
        for_lambda_dt;
    ...
    double   var_dom_sr, var_for_sr, stdev_dom_sr, stdev_for_sr, cov_ln_fx_dom_sr, cov_ln_fx_for_sr;
    String** ppszUndNames;
    ...
    while (stp->next)
    {
        tminf = stp->tminf[und_index];
        tminf->int_sigx_dt =
            fx_cum_vol_func(stp->next->time, SRT_NO, ts) - fx_cum_vol_func(stp->time, SRT_NO, ts);
        tminf->int_sigx2_dt =
            fx_cum_vol_func(stp->next->time, SRT_YES, ts) - fx_cum_vol_func(stp->time, SRT_YES, ts);
    ...
    while (stp)
    {
        ...
        for (i = 0; i < 2; i++)
        {
            ...
            G_H_func(stp->time, gen_ts, &tminf->StochRatesVal[i].G, &tminf->StochRatesVal[i].H);
    ...
    /* Allocate space for a new correlation matrix with values at each time step (now we look at the
     * Fx Forward) */
    num_time_points = 0;
    ...
    while (stp->next)
    {
        ...
        dom_Psi_dt    = Psi_func(stp->next->time, dom_ts) - Psi_func(stp->time, dom_ts);
        ...
        for_Psi_dt    = Psi_func(stp->next->time, for_ts) - Psi_func(stp->time, for_ts);
        ...
        mean_int_dom_sr_dt = -log(nexttminf->StochRatesVal[0].df) +
                             log(tminf->StochRatesVal[0].df) - next_dom_Psi * dom_L_dt + dom_O_dt +
                             pow(dom_lambda_dt, 2) * dom_Phi;

        cov_int_dom_fx  = next_dom_Psi * Vdx_dt - Wdx_dt;
        cov_int_for_fx  = next_for_Psi * Vfx_dt - Wfx_dt;
        cov_int_dom_for = next_dom_Psi * next_for_Psi * Ofd_dt - next_for_Psi * Rfd_dt -
                          next_dom_Psi * Pfd_dt + Mfd_dt;

        mean_int_for_sr_dt = -log(nexttminf->StochRatesVal[1].df) +
                             log(tminf->StochRatesVal[1].df) + next_for_Psi * for_L_dt - for_O_dt -
                             cov_int_for_fx - cov_int_dom_for;
        ...
        var_int_dom_sr =
            ...
            2 * next_dom_Psi * (next_dom_Psi * next_dom_G - dom_Psi * dom_G - dom_L_dt) +
            next_dom_G * next_dom_Psi * next_dom_Psi - dom_G * dom_Psi * dom_Psi - 2 * dom_O_dt;

        var_int_for_sr =
            next_for_Psi * next_for_Psi * for_G_dt -
            2 * next_for_Psi * (next_for_Psi * next_for_G - for_Psi * for_G - for_L_dt) +
            next_for_G * next_for_Psi * next_for_Psi - for_G * for_Psi * for_Psi - 2 * for_O_dt;
        ...
        /* computation of the new correlation : ( 0: FX/DOM | 1 : FX/FOR | 2 : DOM/FOR ) */
        ...
        cov_ln_fx_dom_sr = next_dom_F * (next_dom_Psi * dom_G_dt - dom_S_dt) +
                           next_dom_F * (Pfd_dt - next_for_Psi * Ofd_dt) + next_dom_F * Vdx_dt;

        /* Covariance between rf and Z */
        cov_ln_fx_for_sr = -next_for_F * (next_for_Psi * for_G_dt - for_S_dt) -
                           next_for_F * (Rfd_dt - next_dom_Psi * Ofd_dt) + next_for_F * Vfx_dt;

        cov_dom_for_sr = next_dom_F * next_for_F * Ofd_dt;

        new_corr_matrix[0][step_index] = cov_ln_fx_dom_sr / (stdev_dom_sr * std_ln_fx);

        new_corr_matrix[1][step_index] = cov_ln_fx_for_sr / (stdev_for_sr * std_ln_fx);

        new_corr_matrix[2][step_index] = cov_dom_for_sr / (stdev_for_sr * stdev_dom_sr);
    ...
    /* Replaces the current correlation matrix by the new one (with coeffcients done inside) in
     * UndInfo */
    err = srt_f_init_Corr_TermStruct(
        step_index, 3, new_corr_matrix, pdDates, ppszUndNames, &und_info->corr_ts);
    ...
    /* Creates and fills the local SrtCorrLst with relevent underlying correlation matrix*/
    err = srt_f_make_deal_corrlist(
        used_und_names, und_info->no_of_underlyings, "Local deal correlation list", corrlist, &cls);

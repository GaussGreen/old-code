    ...
    double **xExBdry) /* array of exercise points (in x) */
{
  ...
  tfirst = tNow + 1 - 1;
  if (endofday == 1)
    tfirst = add_unit(tNow, 1, SRT_BDAY,
                      SUCCEEDING /*  , conv.ccy*/); /* tfirst is the first
                                                       possible exercise */
  ...
  case SimpleMidAt:
    ...
    if (CalReq->LGMOneTwoFactor == 2) {
      ...
      /*
      error = LGMAutoCal2DVega(2*tsPtr->numZ  , nEx  , tExArr  , Zeta1Scenari  ,
      Zeta12Scenari  , Zeta2Scenari  , H1Scenari  , H2Scenari  , gamma  , tNow
      , ycName  , GetVol  , GetBeta  , (void*) dealPtr  , Midat2DPayoff  ,
      LGMvalVega  , xExBdry);
      */
      srt_free(LGMvalVega);
  ...
  case CallTimeSwap:
    ...
    /* evaluate deal */
    /*		error = ConvolverTimeSwap(nEx  , zeta  , parms  , tNow  , ycName  ,
       GetVol  , GetBeta  , tsPtr  , dealtype  , (void*)dealPtr  ,
...
/* Outputs:
        *answer = value of the deal  ,
        xExBdry[0  ,1  ,...  ,nEx-1] are the x values of the exercise boundary
   at each exercise if multiple exercise points  , gives the nearest to the
   money
*/
...
(for deals whose payoffs include options) 3) information about the deal  , and a
function to calculate the payoff for all the exercise dates The arguments of the
payoff function are:
        (*payofffunc)(double payoff[][]  , long nx  , double x[]  , double
reduction[] LGMDealType dealtype  , void *dealPtr  , Date EvalDate  , String
ycName  , LGM_TS tsPtr)
 */
    ...
    LGMErr (*GetBeta)(Date, Date, double *)) /* swaption/cap exponents (beta) */
{
  ...
  StrikeD = (double *)srt_calloc(
      nEx, sizeof(double)); /* Strike[j]*D(tset_j)  , j=0  ,...  ,nEx-1 */
  aredD = (double *)srt_calloc(
      nEx, sizeof(double)); /* Ared[j]*D(tfirst_j)  , j=0  ,...  ,nEx-1 */
  Gset = (double *)srt_calloc(
      nEx, sizeof(double)); /* G(tset_j)  , j=0  ,...  ,nEx-1 */
  Gpay = (double *)srt_calloc(
      nPay, sizeof(double)); /* G(tPay_i)  , i=0  ,...  ,nPay-1 */
  aD = (double *)srt_calloc(
      nPay, sizeof(double)); /* a[i]D(tPay_i)  , i=0  ,...  ,nPay-1 */
  prefac = dmatrix(
      0, nx, 0,
      nPay - 1); /* exp(Gpay[i]*x[k])  , k=0  ,..  ,nx  , i=0  ,...  ,nPay-1 */
  postfac =
      dmatrix(0, nEx - 1, 0, nPay - 1); /* exp(-.5*zetaex[j]*Gpay[i]^2)  , j=0
                                           ,...  ,nEx-1  , i=0  ,...  ,nPay-1 */
  ...
  /* compute zeta at exercise dates  , G and StrikeD at settlement dates  , and
   * aredD at first paydates */
  for (j = 0; j < nEx; j++) {
    jex = j0 +
...
static LGMErr BerInvFltpayoff(
  ...
  for (j = 0; j < nEx; j++) {
    ...
    for (i = ifirst; i < nCpn; i++) {
      ...
      /*	Also  , if caplet strike is irrelevant  , we price it with a vol
       * of 0 */
    ...
    LGMErr (*GetBeta)(Date, Date, double *)) /* swaption/cap exponents (beta) */
{
  ...
  double *cvgR =
      NULL; /* flting rate cvg for in cpn i  , i = i0+1  ,...  , nCpn */
  ...
  double **postf = NULL; /* exp(-0.5*Gpay[i]*Gpay[i]*zEx[j])  , [j][i]  ,
                            i=iSet[j]  ,...  ,nCpn */
  double **pref = NULL;  /* D[i]*exp(G[i]*x[k])  , [i][k]  , i = iSet[j]  ,...
                            ,nCpn  , k=0  ,...  , nx */
  ...
  dPay = (double *)srt_calloc(
      nCpn + 1, sizeof(double)); /* dPay[i0+i]  , i = 0  ,...  , nCpn */
  beta = (double *)srt_calloc(
      nCpn + 1, sizeof(double)); /* beta[i0+i]  , i = *  ,1  ,...  , nCpn */
  cvgCpn = (double *)srt_calloc(
      nCpn + 1, sizeof(double)); /* cvg[i0+i]  , i = *  ,1  ,...  , nCpn */
  cvgR = (double *)srt_calloc(
      nCpn + 1, sizeof(double)); /* cvg[i0+i]  , i = *  ,1  ,...  , nCpn */
  Gpay = (double *)srt_calloc(
      nCpn + 1, sizeof(double)); /* Gpay[i0+i]  , i = 0  ,...  , nCpn */
  tFix = (Date *)srt_calloc(
      nCpn + 1, sizeof(Date)); /* tFix[i0+i]  , i = *  ,1  ,...  , nCpn */

  postf = dmatrix(0, nEx - 1, 0,
                  nCpn); /* [j][i] = [0  ,1  ,...  ,nEx-1][0  ,...  ,nCpn] */
  pref =
      dmatrix(0, nCpn, 0, nx); /* [i][k] = [0  ,1  ,...  ,nCpn][0  ,...  ,nx] */

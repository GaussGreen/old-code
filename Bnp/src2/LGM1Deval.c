...
static LGMErr GenEurpayoff(double **payoff, long nx, double *x, double *reduction,
...
static LGMErr BerCapFltpayoff(
				double **payoff, long nx, double *x, double *reduc, 
				LGMDealType dealtype, void *dealPtr, LGMCalParm	*CalReq,
				Date tNow, String ycName, LGM_TS *tsPtr,
				LGMErr	(*GetVol)(Date, Date, double, SRT_Boolean, double*),	/* swaption/cap vols */
				LGMErr	(*GetBeta)(Date, Date, double*));		/* swaption/cap exponents (beta) */
...
/* Get number of exercise and zeta values at each valid exercise date */
	tfirst = tNow+1-1;
	if (endofday==1)
		tfirst = add_unit(tNow, 1, SRT_BDAY, SUCCEEDING/*, conv.ccy*/);	/* tfirst is the first possible exercise */
		...
		/* allocate zeta array */
			...
			if(CalReq->LGMOneTwoFactor==2)
			{
				...
				error = LGMAutoCal2DVega(2*tsPtr->numZ, nEx, tExArr, Zeta1Scenari, Zeta12Scenari, 
										Zeta2Scenari, H1Scenari, H2Scenari, gamma,
					    					tNow, ycName, GetVol, GetBeta, (void*) dealPtr, 
										Midat2DPayoff, LGMvalVega, xExBdry);
				*/
				srt_free(LGMvalVega);   
			...
			else if(CalReq->LGMOneTwoFactor==1)
			{
				...
				tExArr = (Date*) srt_calloc(nEx, sizeof(Date));
				if (zeta==NULL || tExArr==NULL) return ("allocation failed in LGMEval");
			...
			/* find today's zetas at the exercise dates */
				for (k=0; k<nEx; k++)
				{
				 ...
				 error = Convolver(nEx, zeta, parms, tNow, ycName, GetVol, GetBeta, 
								tsPtr, dealtype, (void*)dealPtr, MidAtpayoff, CalReq, 
								&LGMval, xExBdry);
...
/* General Exercise once (out of many) option */
	...
	/* evaluate deal */
		error = Convolver(nEx, zeta, parms, tNow, ycName, GetVol, GetBeta,
						tsPtr, dealtype, (void*)dealPtr, GenMidAtpayoff, CalReq,
						&LGMval, xExBdry);
		*LGMvalue = LGMval;		
		break;
...
/* evaluate deal */
		error = Convolver(nEx, zeta, parms, tNow, ycName, GetVol, GetBeta,
						tsPtr, dealtype, (void*)dealPtr, BerCapFltpayoff, CalReq,
						&LGMval, xExBdry);
		*LGMvalue = LGMval;	
		break;
...
/*		error = ConvolverTimeSwap(nEx, zeta, parms, tNow, ycName, GetVol, GetBeta,
						tsPtr, dealtype, (void*)dealPtr, BerTimeSwapPayoff, CalReq,
						&LGMval, xExBdry);	*/	
		error = ComputeBermudanAndEuropeanTimeSwap(nEx, zeta, parms, tNow, ycName, GetVol, GetBeta,
						tsPtr, dealtype, (void*)dealPtr, BerTimeSwapPayoff, CalReq,
						&LGMval, xExBdry);	
		*LGMvalue = LGMval;	
		break;
...
/* Allocate arrays */
	...
	{	srt_free(varr); srt_free(vmax); srt_free(reduction);
		srt_free(x); srt_free(Qarr); srt_free(Qplus);
		srt_free(differ);
...
/* call payoff function to set up calculation */
	error = (*payofffunc)(payofftable, nx, x, reduction,				/* output and grid */
							...
							EvalDate, ycName, tsPtr, GetVol, GetBeta);	/* today's market & tsPtr */
...
/* free up space allocated */
	...
	srt_free(reduction);
	srt_free(differ); srt_free(x); 
	srt_free(Qarr); srt_free(Qplus);
	free_dmatrix(payofftable,0, nEx-1, 0, nx);
	srt_free(weights);
...
/* Allocate space */
    ...
    zetaex=NULL; StrikeD=NULL; aredD=NULL; Gset=NULL;
    aD=NULL; Gpay=NULL; 
	prefac=NULL; postfac=NULL;
	...
	postfac = dmatrix(0, nEx-1, 0, nPay-1);					/* exp(-.5*zetaex[j]*Gpay[i]^2), j=0,...,nEx-1, i=0,...,nPay-1 */

    if (zetaex==NULL || StrikeD==NULL || aredD==NULL
       || Gset==NULL || Gpay==NULL || aD==NULL ||
	    prefac==NULL || postfac==NULL)
	{	srt_free(zetaex); srt_free(StrikeD);
		srt_free(aredD); srt_free(Gset);
		srt_free(aD); srt_free(Gpay); 
...
/* compute discounted cash flows and G at the pay dates */
 ...
 /* compute zeta at exercise dates, G and StrikeD at settlement dates, and aredD at first paydates */
	for (j=0; j<nEx; j++)
...
/* compute payoff table */
	...
	{	jex = j0+j;
		...
		{	i = MidAt->FirstPay[jex]-i0;
			payoff[j][k] = -StrikeD[j]*exp(Gset[j]*(reduction[j]*x[k]-0.5*Gset[j]*zetaex[j]));			/* Strike */
			payoff[j][k] = payoff[j][k] + (aD[i]-aredD[j])*prefac[k][i]*postfac[j][i];		/* first payment */
			i++;
			for ( ; i<nPay;	i++)
				payoff[j][k] = payoff[j][k] + aD[i]*prefac[k][i]*postfac[j][i];				/* other payments */	
		}
	}
				...
				LGMErr	(*GetBeta)(Date, Date, double*))		/* swaption/cap exponents (beta) */
{
	...
	{	jex = j0+j;
		...
		{	tPay = deal->tPay[jex][i];
			...
			for (k=0; k<=nx; k++)					/* for each gridpoint */
				payoff[j][k] = payoff[j][k] + aD*exp(Gpay*x[k]*reduction[j]);	/* add payment */
...
LGMErr			(*GetBeta) (Date, Date, double*))
{
	...
	if (zEx==NULL || dSet==NULL || Gset==NULL
		|| tFix==NULL || dStart==NULL || dPay==NULL 
		|| Gstart==NULL || Gpay==NULL 
	...
	for (j=0; j<nEx; j++) 
	{
		...
		{	
				...
				&& caplet_strike > 1.0e-08)
			{
				...
				case SLIDING2:
					...
					caplet_vol = GetNormalVol(
						...
						tStart, 
						tEnd, 
						(swp_f_df (tNow, tStart, ycName) / swp_f_df (tNow, tEnd, ycName) - 1.0) / refCvg[i],
						caplet_strike, 
						SRT_TRUE,
						GetVol, 
						GetBeta);
			...
			{	
				...
				fwd_libor *=exp 
						(
								(Gstart[i] - Gpay[i]) * x[k] * reduc[j+j0]
		...
		{	
			/*	Subtract strike */
			payoff[j][k] -= ptr->strike[j+j0] * dSet[j] * exp (Gset[j] * (x[k] * reduc[j+j0] - 0.5 * Gset[j] * zEx[j]));
		}
	}
...
/* Allocate space */
	...
	{	
		...
		{	
			...
			for (k=0; k<=nx; k++)
			{	
				rate = (pref[i-1][k]*postf[j][i-1]/(pref[i][k]*postf[j][i]) - 1.0)/cvgR[i];
				...
				CAPLETMINPV = srt_f_optblksch((1+cvgCpn[i]*rate),
										(1+cvgCpn[i]*amini),
										ATMVOL,
										CAPLETMAT,
										1.0, /*DISCOUNT FACTOR */
				...
				CAPLETMAXPV = srt_f_optblksch((1+cvgCpn[i]*rate),
										(1+cvgCpn[i]*amaxi),
										ATMVOL,
										CAPLETMAT,
										1.0, /*DISCOUNT FACTOR */
...
/* Free and return */
cleanup:
    srt_free(zEx); srt_free(dPay);
	srt_free(beta); srt_free(cvgCpn);
    srt_free(cvgR); srt_free(Gpay);
	srt_free(tFix);

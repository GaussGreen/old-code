...
Err err = NULL;	
	...
	if ( !strcmp(messageStr,"FWD_PV") )
	{ 
		*output = COMPUTE_FWD_PV;
	}
	else
	if ( !strcmp(messageStr,"FWD_RATE") )
	{ 
		*output = COMPUTE_FWD_RATE;
	}
	else
	if ( !strcmp(messageStr,"LEVEL") )
	{ 
		*output = COMPUTE_LEVEL;
	}
	else
	if ( !strcmp(messageStr,"MARGIN") )
	{ 
		*output = COMPUTE_MARGIN;
	}		 
	else
	if ( !strcmp(messageStr,"IRR") )
	{ 
		*output = COMPUTE_IRR;
	}		 
	else
	if ( !strcmp(messageStr,"FWD_IRR") )
	{ 
		*output = COMPUTE_FWD_IRR;
	}		 
	else
	if ( !strcmp(messageStr,"DURATION") )
	{ 
		*output = COMPUTE_DURATION;
	}
	else
	if ( !strcmp(messageStr,"CONVEXITY") )
	{ 
		*output = COMPUTE_CONVEXITY;
	}
	else
	if ( !strcmp(messageStr,"MODIFIED_DURATION") )
	{ 
		*output = COMPUTE_MODIFIED_DURATION;
	}
	else
	if ( !strcmp(messageStr,"MODIFIED_CONVEXITY") )
	{ 
		*output = COMPUTE_MODIFIED_CONVEXITY;
	}
	else
	if ( (!strcmp(messageStr,"MATCHING_RATIO") ) || (!strcmp(messageStr, "CONVEXITY_DURATION")))
	{ 
		*output = COMPUTE_MODIFIED_CONVEXITY;
	}
	else
			...
			)
{
	...
	if (!fix_leg_flag) Arg_set(arg, ARG_STRIKE, 0.0) ;
...
/*	  add end-of-day funtionality  */
	  ...
	  if( (coupon_today==1 ) && (eod==1) )
	    ...
	    prev_date = next_date ;
	    next_date = (Date) Leg_field_get(leg,LEG_DATE,j) ;
	    if( value_date==today ){ value_date += 1 ; }
	    }
	  ...
	  index_start = Leg_Iget(leg, LEG_INDEX_START) ;
	    ...
	    else
	    {
	      ...
	      ans += -strike * factor *
		     ( cover + coverage(today,next_date,basis) ) ;
	    }
...
double black_scholes_tmp(double fwd_price,double strike,double vol,double mat, 
		double disc, SrtReceiverType rec_or_pay, struct greek_struct *greeks)
{		
	double d1,d2,prem;
	double nd1, nd2;
    ...
    if(rec_or_pay == SRT_PAYER) 	     /** if call **/
	{
		...
		greeks->theta = 0;
	     	...
	     	greeks->theta = -1/365.25*(0.5*fwd_price*disc*vol/sqrt(mat)
                                  *gauss(d1)-log(disc)*disc*strike*nd2/mat); 
	}
    if(rec_or_pay == SRT_RECEIVER) 	    /** if put **/
	{
		...
		greeks->theta = 0;
                ...
                greeks->theta = -1/365.25*(0.5*fwd_price*disc*vol/sqrt(mat)
				*gauss(d1)-strike*disc*log(disc)/mat*(nd2-1));
		nd1 = 1 - nd1;
		nd2 = 1 - nd2;     
	} 

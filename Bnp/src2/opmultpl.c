...
)
{
    ...
    prob = bivar(
        cp_x * (-k_x + mu_x) / sigx_sqrt, cp_y * (-k_y + mu_y) / sigy_sqrt, cp_x * cp_y * corr);
 ...
 *
 * DESCRIPTION  	: At maturity, the holder receives the price difference between
 *		  some asset S1 and a strike K1, times the price difference
 *		  between a 2nd asset S2 and another strike K2, times a
 *		  gearing.
 *
 ...
 *              	: corr          - correlation between 1st and 2nd underlyings
 *              	: mat           - option maturity, in years
 *              	: disc          - discount factor
 *              	: gearing       - gearing
 *              	: call_put1    	- option on 1st underlying (0: call, 1: put)
 *              	: call_put2    	- option on 2nd underlying (0: call, 1: put)
 *
    ...
    SrtGreekType   greek)
{
    ...
    {
        premium = srt_f_optblksch(fwd2, strike2, vol2, mat, disc, call_put2, PREMIUM) *
                  srt_f_optblksch(fwd1, strike1, vol1, mat, disc, call_put1, PREMIUM) / disc;
    }
    else
    {
        ...
        n_x_y = proba_joint_x_y(
            mu_x_x + cov, mu_y_y + cov, vol1, vol2, mat, k_x, k_y, corr, cp_x, cp_y);

        n_x = proba_joint_x_y(mu_x_x, mu_y + cov, vol1, vol2, mat, k_x, k_y, corr, cp_x, cp_y);

        n_y = proba_joint_x_y(mu_x + cov, mu_y_y, vol1, vol2, mat, k_x, k_y, corr, cp_x, cp_y);

        n = proba_joint_x_y(mu_x, mu_y, vol1, vol2, mat, k_x, k_y, corr, cp_x, cp_y);

        premium = disc * cp_x * cp_y *
                  (exp(cov) * fwd1 * fwd2 * n_x_y - fwd1 * strike2 * n_x - fwd2 * strike1 * n_y +
                   strike1 * strike2 * n);
    }

...
double answer;
	...
	else
	{
		...
		n_x_y = proba_joint_x_y( mu_x_x + cov , mu_y_y + cov, vol1, vol2, 
								 mat, k_x, k_y, corr, cp_x, cp_y );

		n_x = proba_joint_x_y( 	mu_x_x, mu_y + cov, vol1, vol2,
								mat, k_x, k_y, corr, cp_x,  cp_y );

		n_y = proba_joint_x_y( 	mu_x + cov ,mu_y_y, vol1, vol2,
								mat, k_x, k_y, corr, cp_x, cp_y );

		n = proba_joint_x_y( mu_x, mu_y, vol1, vol2, 
							 mat, k_x, k_y, corr, cp_x,  cp_y );

		premium = 	disc * cp_x * cp_y *
					( exp(cov) * fwd1 * fwd2 * n_x_y  
				   	- fwd1    * strike2 * n_x    
				   	- fwd2    * strike1 * n_y       
				   	+ strike1 * strike2 * n      );
	} 
	...
	switch (greek)
	{
		...
		case DELTAX : 	/*** DELTA FWD 1***/  
			shift = fwd1 / 10000;
			answer = ( srt_f_optmultpl(	fwd1 + shift, 
		 								strike1, 
										fwd2, 
			       						strike2, 
			       						vol1, 
		...
		case GAMMAXY :	/*** CROSS GAMMA ***/  
			shift1 = fwd1 / 10000;
			shift2 = fwd2 / 10000;
			answer = srt_f_optmultpl(	fwd1 + shift1, 
		 								strike1, 
										fwd2 + shift2, 
			    			   			strike2, 
						       			vol1, 

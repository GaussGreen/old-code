...
/* Sobol sampling of the initial space of parameters */
	...
	for(i=1; i<=tot_pts; i++)
	{
		...
		err = error_function (data, target, weight, ndata, pts[i], min_param, max_param, 
			nparam, funcs, &(values[i]));
		if (err)
		{
			free_everything;
			return err;
...
/* Sort the points */
	...
	for (i=1; i<=bst_pts; i++)
	{
		...
		err = error_function (data, target, weight, ndata, bpts[i], min_param, max_param,
			nparam, funcs, &(bvalues[i]));
		if (err)
		{
			free_everything;
			return err;
	...
	for (i=1; i<=bst_pts; i++)
	{
		...
		memcpy (&(x[kernel_of_point[i]][npts_in_clust[kernel_of_point[i]]][1]), &(bpts[i][1]),
	...
	for (i=1; i<=nclust; i++)
	{
		...
		err = error_function (data, target, weight, ndata, min[i], min_param, max_param,
			nparam, funcs, &(minval[i]));
		if (err)
		{
			free_everything;
			return err;
	...
	for (i=1; i<=nclust; i++)
	{
		...
		err = levenberg_marquardt (data, target, weight, ndata, min[i], nparam, niter, dfuncs, 
								   &(minval[i]));	
		if (err)
		{
			free_everything;

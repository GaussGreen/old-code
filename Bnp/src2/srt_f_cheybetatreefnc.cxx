/* ============================================================================
   ...
   FILENAME:			srt_f_cheybetatreefnc.cxx
...
/*	Function:		srt_f_locvol_init_trinf
    Object:			initialise the tree information in step ( geomtry
   and limits ) allocate and populate
*/
                                                     ...
                                                     (for allocation) */
{
  ...
  while (step) {
    ...
    /*		convexified_forward = sam_get(tminf->fwd_sam        , 0        ,
       SHORT_RATE)
  ...
  /* Set the r spacing delta_r to sqrt of minimum of (average_stdev        ,
   * min_stdev
   * * 2) */
  ...
  trinf->delta_phi = 0.0;
  trinf->max_state_var_index = trinf->min_state_var_index =
      maxtrinf->min_state_var_index = maxtrinf->max_state_var_index = 0;
  ...
  while (step) {
    ...
    trinf->delta_phi = (trinf->max_phi - trinf->min_phi) /
                       (trinf->max_phi_index - trinf->min_phi_index);
...
Err srt_f_CheyBetaTree_make_state_var_grid(SrtLocTreInf maxtrinf,
  ...
  for (i = maxtrinf.min_state_var_index; i <= maxtrinf.max_state_var_index;
       i++) {
...
Err srt_f_CheyBetaTree_make_phi_grid(SrtLocTreInf *trinf,
                                     double *cur_phi_grid) {
...
Err srt_f_CheyBetaTree_recombine_in_r(SrtTrinTreNdInf *node, double *r_grid,
  ...
  find_closest(target, r_grid, nxttrinf->min_state_var_index + 1,
               nxttrinf->max_state_var_index - 1, &(node->son_index[TREE_MID]));
  ...
  find_closest(target, r_grid, nxttrinf->min_state_var_index,
               node->son_index[TREE_MID] - 1, &(node->son_index[TREE_DOWN]));
  ...
  find_closest(target, r_grid, node->son_index[TREE_MID] + 1,
               nxttrinf->max_state_var_index, &(node->son_index[TREE_UP]));
...
Err srt_f_CheyBetaTree_calc_prob(SrtTrinTreNdInf *node) {
  ...
  Su = node->son_state_var_value[TREE_UP],
  Sm = node->son_state_var_value[TREE_MID],
  Sd = node->son_state_var_value[TREE_DOWN], E = node->state_var_expectation,
  V = node->state_var_variance;

  node->p[TREE_UP] = 1.0 + (-E + Su) / (-Su + Sm) +
                     (-((-E + Su) * (-Su * Su + Sm * Sm)) +
                      (-Su + Sm) * (-E * E + Su * Su - V)) /
                         ((Sd * Sd - Su * Su) * (-Su + Sm) -
                          (Sd - Su) * (-Su * Su + Sm * Sm)) -
                     ((Sd - Su) * (-((-E + Su) * (-Su * Su + Sm * Sm)) +
                                   (-Su + Sm) * (-E * E + Su * Su - V))) /
                         ((-Su + Sm) * ((Sd * Sd - Su * Su) * (-Su + Sm) -
                                        (Sd - Su) * (-Su * Su + Sm * Sm)));

  node->p[TREE_MID] = -((-E + Su) / (-Su + Sm)) +
                      ((Sd - Su) * (-((-E + Su) * (-Su * Su + Sm * Sm)) +
                                    (-Su + Sm) * (-E * E + Su * Su - V))) /
                          ((-Su + Sm) * ((Sd * Sd - Su * Su) * (-Su + Sm) -
                                         (Sd - Su) * (-Su * Su + Sm * Sm)));
  ...
  if (node->p[TREE_UP] < 0.0 || node->p[TREE_MID] < 0.0 ||
      node->p[TREE_DOWN] < 0.0) {
...
/*	Function:		srt_f_locvol_discount_assets
        Object:			Discount asset prices (as many assets as Grfn
   cols ) */
...
Err srt_f_CheyBetaTree_discount_assets(double *cur_assets,
   ...
   */
  find_closest_strictly_below(next_phi, next_phi_grid, nxttrinf->min_phi_index,
                              nxttrinf->max_phi_index - 1, &next_phi_index_low);
  ...
  for (i = 0; i < nassets; i++) {
    ...
    for (j = 0; j < 3; j++) {
      ...
      asset_at_phi_high =
          next_assets[node->son_index[j]][next_phi_index_high][i];

      asset_here_and_now +=
          node->p[j] *
          interp_linear(next_phi, next_phi_grid[next_phi_index_low],
                        asset_at_phi_low, next_phi_grid[next_phi_index_high],
                        asset_at_phi_high);
    }

...
Err mrqcof(double x[], double y[], double sig[], int ndata, double a[], int ia[],
		   ...
		   Err (*funcs)(double, double [], double *, double [], int))
{
	...
	*chisq=0.0;
	...
	if (PRL_CONTEXT.bAggregate) {
		...
		CheckRcvCalibData(0,NULL,&rcvd,(double **)NULL,(double ***)NULL,(int**)NULL,&max_time,1);
		...
		while(!strcmp(still_data_to_get,"true")){ 
			...
			strcpy(still_data_to_get, CheckRcvCalibData(ma,&nb_local_data,&rcvd,
				&y_vect,&dyda_mtrx,&local_indexes,&max_time,0));
			if (strcmp(still_data_to_get,"true") && strcmp(still_data_to_get,"false")){
				/*this is an error*/
			...
			if (nb_local_data==0) continue;
			/* Compute alpha, beta, chi square */
			for (i=0;i<nb_local_data;i++) {
				ymod = y_vect[i];
				dyda = dyda_mtrx[i]-1; /* NR algorithms are for indexes starting at 1... */
		...
		free(y_vect);
		for(i=0;i<nb_local_data;i++) free(dyda_mtrx[i]);
		free(dyda_mtrx);
	}  /* end of if (Version...) */
#endif
	for (j=2;j<=mfit;j++)

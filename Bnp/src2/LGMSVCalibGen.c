...
Err GetTargetVol_LGMSV(void *Inst_, void *GlobalParam, void *Model,
...
Err GetSecondGuessVol_LGMSV(void *Model, void *GlobalParam, int vol_index,
  ...
  if (vol_index == 0) {
    ...
    cum_var_sv =
        AllParams->EquiLGM->cum_var_sv[AllParams->lSigLongIndex[vol_index - 1]];
  }
...
Err BumpVol_LGMSV(void *Model, void *GlobalParam, int vol_index, double var) {
  Err err = NULL;
...
Err SetVol_LGMSV(void *Model, CALIBGEN_PARAMS CalibConsts, void *GlobalParam,
  ...
  fact_exp = sqrt((AllParams->EquiLGM->exp_fact[new_index + 1] -
                   AllParams->EquiLGM->exp_fact[last_index + 1]) /
...
Err GetLimitAndLastVol_LGMSV(void *Model, CALIBGEN_PARAMS CalibParams,
  ...
  sig_limit_down_lgm = sqrt(AllParams->EquiLGM->cum_var_lgm[last_index] *
                            AllParams->MinFact / t1);
  sig_limit_up_lgm = sqrt(AllParams->EquiLGM->cum_var_lgm[last_index] /
                          AllParams->MaxFact / t1);
...
Err ExtrapolVol_LGMSV(void *Model, void *GlobalParam, int last_vol_index) {
  ...
  for (i = last_index + 1; i < model->iNbPWTime; i++) {
    ...
    fact_exp = sqrt((AllParams->EquiLGM->exp_fact[i + 1] -
                     AllParams->EquiLGM->exp_fact[i]) /
                    (2.0 * model->dLambdaX) / dt);
...
Err UpdateParamsAfterVol_LGMSV(void *Inst_, void *InstParam, void *GlobalParam,
  ...
  err = Calculate_ShiftAndVol_LGMSV_FromLGM_struct(
      Inst, HestonParam->sCalibCpnSchedule, model, HestonParam->HestonInst);
...
Err PriceInstVol_LGMSV(void *Inst_, void *InstParam, void *GlobalParam,
  ...
  LGMSVClosedFormApprox_struct(model, Inst, HestonParam->HestonInst,
                               AllParams->PricingConst, HestonParam->NumerInst,
                               Inst->dModelPrice);
...
Err GetTargetRho_LGMSV(void *Inst_, void *GlobalConst, void *Model,
  ...
  if (AllParams->calib_params->calib_alpha ||
      AllParams->calib_params->calib_rr_bt) {
...
Err GetSecondGuessRho_LGMSV(void *Model, void *GlobalParam, int index_smile,
...
Err GetLimitAndLastRho_LGMSV(void *Model, CALIBGEN_PARAMS CalibConsts,
                             void *GlobalParam, int index_smile,
...
Err BumpRho_LGMSV(void *Model, void *GlobalParam, int smile_param, double rho) {
  ...
  if (model->iOne2F == 2 && AllParams->calib_params->onefac_rho) {
    err = LGMSV_Get_Rho_From_RhoTarget(rho, AllParams->calib_params->rho_mat1,
                                       AllParams->calib_params->rho_mat2,
                                       model->dInitLGMAlpha, model->dLGMGamma,
                                       model->dInitLGMRho, &rho1, &rho2);
...
Err UpdateParamsAfterRho_LGMSV(void *Inst_, void *InstParam, void *GlobalParam,
  ...
  if (AllParams->calib_params->calib_alpha) {
    err = AllParams->CalibFunctionsForAlpha.GetLimitAndLastParam(
        Model, AllParams->CalibParamsAlpha, AllParams, AllParams->index_vol,
        &last_alpha, &limit_down, &limit_up);

    if (err)
      goto FREE_RETURN;

    err = CalibrateNextParam(
        Inst, HestonParam, AllParams, Model, AllParams->CalibParamsAlpha,
        AllParams->index_vol, last_alpha, limit_down, limit_up,
        &(AllParams->CalibFunctionsForAlpha), &alpha, &success);

    if (err)
      goto FREE_RETURN;

    err = AllParams->CalibFunctionsForAlpha.SetParam(
        Model, AllParams->CalibParamsAlpha, AllParams, AllParams->index_vol,
        alpha);
    ...
    err = AllParams->CalibFunctionsForVol.GetLimitAndLastParam(
        Model, AllParams->CalibParams, AllParams, AllParams->index_vol,
        &last_var, &limit_down, &limit_up);

    if (err)
      goto FREE_RETURN;

    err = CalibrateNextParam(
        Inst, HestonParam, AllParams, Model, AllParams->CalibParams,
        AllParams->index_vol, last_var, limit_down, limit_up,
        &(AllParams->CalibFunctionsForVol), &var, &success);

    if (err)
      goto FREE_RETURN;

    err = AllParams->CalibFunctionsForVol.SetParam(
        Model, AllParams->CalibParams, AllParams, AllParams->index_vol, var);
...
Err GetTargetAlpha_LGMSV(void *Inst_, void *GlobalConst, void *Model,
  ...
  if (AllParams->calib_params->calib_rho ||
      AllParams->calib_params->calib_rr_bt) {
...
Err GetSecondGuessAlpha_LGMSV(void *Model, void *GlobalParam, int index_smile,
...
Err UpdateParamsAfterAlpha_LGMSV(void *Inst_, void *InstParam,
  ...
  err = AllParams->CalibFunctionsForVol.GetLimitAndLastParam(
      Model, AllParams->CalibParams, AllParams, AllParams->index_vol, &last_var,
      &limit_down, &limit_up);

  if (err)
    goto FREE_RETURN;

  err = CalibrateNextParam(Inst, HestonParam, AllParams, Model,
                           AllParams->CalibParams, AllParams->index_vol,
                           last_var, limit_down, limit_up,
                           &(AllParams->CalibFunctionsForVol), &var, &success);
...
Err PriceInstAlpha_LGMSV(void *Inst_, void *InstParam, void *GlobalParam,
  ...
  if (AllParams->calib_params->calib_rho ||
      AllParams->calib_params->calib_rr_bt) {
    *InstPrice = Inst->dModelPrice[2] + Inst->dModelPrice[1] -
                 2.0 * Inst->dModelPrice[0];
  } else {
    *InstPrice = Inst->dModelPrice[1];
  }
...
Err GetSecondGuessLambda_LGMSV(void *Model, void *GlobalParam, int index_lambda,
...
Err PriceInstLambda_LGMSV(void *Inst_, void *InstParam, void *GlobalParam,
  ...
  while (NextInst != NULL) {
    LGMSVClosedFormApprox_struct(
        model, NextInst, NextHestonParam->HestonInst, AllParams->PricingConst,
        NextHestonParam->NumerInst, NextInst->dModelPrice);
...
Err UpdateParamsAfterLambda_LGMSV(void *Inst_, void *InstParam,
  ...
  if (AllParams->calib_params->use_lgm_first_guess) {
    err = lgmcalibzetalambda_tauts_dlm(
        ...
        AllParams->CalibExeShortSchedule->dATMStrike, NULL,
        AllParams->CalibExeShortSchedule->dATMPrice,
        AllParams->CalibExeShortSchedule->dATMPrice,

        ex_zeta, 1, 1, &time0, &model->dLambdaX, model->iOne2F,
        model->dInitLGMAlpha, model->dLGMGamma, model->dInitLGMRho,
        AllParams->long_param, AllParams->short_param, NULL, &temp);

    fact = exp(model->dLambdaX * model->dInitTStar);
    temp =
        sqrt(ex_zeta[0] / AllParams->CalibExeLongSchedule->dExeTimes[0]) / fact;
    ...
    for (i = 1; i < AllParams->CalibExeLongSchedule->iNExe; i++) {
      if (ex_zeta[i] > ex_zeta[i - 1]) {
        temp = sqrt((ex_zeta[i] - ex_zeta[i - 1]) /
                    (AllParams->CalibExeLongSchedule->dExeTimes[i] -
                     AllParams->CalibExeLongSchedule->dExeTimes[i - 1])) /
               fact;
  ...
  if (CalibConsts->do_calib) {
    ...
    while (NextInst != NULL) {
      err = Calculate_HestonEquivalent_LGMSV_FromLGM_struct(
          NextInst, AllParams->CalibCpnShortSchedule, model,
  ...
  if (AllParams->calib_params->calib_smile_on_prim || nb_inst > 1) {
    err = CalibrateParamTS(0, nb_inst - 1, AllParams->AllLongInst,
                           AllParams->LongInstParams, AllParams, model,
                           AllParams->CalibParamsRho,
                           &(AllParams->CalibFunctionsForRho));
  } else {
    err = CalibrateParamTS(0, nb_inst - 1, AllParams->AllShortInst,
  ...
  while (NextInst != NULL) {
    err = Calculate_ShiftAndVol_LGMSV_FromLGM_struct(
        NextInst, NextHestonParam->sCalibCpnSchedule, model,
        NextHestonParam->HestonInst);
...
Err GetSecondGuessRho2_LGMSV(void *Model, void *GlobalParam, int index_rho2,
...
Err GetLimitAndLastRho2_LGMSV(void *Model, CALIBGEN_PARAMS CalibConsts,
                              void *GlobalParam, int index_rho2,
...
Err BumpRho2_LGMSV(void *Model, void *GlobalParam, int index_rho2,
                   double rho2) {
  Err err = NULL;
...
Err UpdateParamsAfterRho2_LGMSV(void *Inst_, void *InstParam, void *GlobalParam,
  ...
  err = CalibrateParamTS(0, 0, AllParams->AllShortInst, &InstParam, AllParams,
                         model, AllParams->CalibParamsLambda,
...
Err GetTargetFlatAlpha_LGMSV(void *Inst_, void *GlobalConst, void *Model,
  ...
  while (NextInst != NULL) {
    if (AllParams->calib_params->calib_rho ||
        AllParams->calib_params->calib_rr_bt) {
      total +=
          NextInst->dPrice[2] + NextInst->dPrice[1] - 2.0 * NextInst->dPrice[0];
    } else {
      total += NextInst->dPrice[1];
    }
...
Err GetSecondGuessFlatAlpha_LGMSV(void *Model, void *GlobalParam,
...
Err PriceInstFlatAlpha_LGMSV(void *Inst_, void *InstParam, void *GlobalParam,
  ...
  while (NextInst != NULL) {
    ...
    if (AllParams->calib_params->calib_rho ||
        AllParams->calib_params->calib_rr_bt) {
      total += NextInst->dModelPrice[2] + NextInst->dModelPrice[1] -
               2.0 * NextInst->dModelPrice[0];
    } else {
      total += NextInst->dModelPrice[1];
    }
...
Err UpdateParamsAfterFlatAlpha_LGMSV(void *Inst_, void *InstParam,
  ...
  if (AllParams->CalibParams->do_calib) {
    err = CalibrateParamTS(0, AllParams->iNbLongInst - 1,
                           ...
                           GlobalParam, Model, AllParams->CalibParams,
...
Err GetSecondGuessFlatRho_LGMSV(void *Model, void *GlobalParam,
...
Err GetLimitAndLastFlatRho_LGMSV(void *Model, CALIBGEN_PARAMS CalibConsts,
                                 void *GlobalParam, int index_flat_rho,
...
Err BumpFlatRho_LGMSV(void *Model, void *GlobalParam, int index_flat_rho,
  ...
  if (model->iOne2F == 2 && AllParams->calib_params->onefac_rho) {
    ...
    err = LGMSV_Get_Rho_From_RhoTarget(
        flat_rho, AllParams->calib_params->rho_mat1,
        AllParams->calib_params->rho_mat2, model->dInitLGMAlpha,
        model->dLGMGamma, model->dInitLGMRho, &rho1, &rho2);
...
Err PriceInstFlatRho_LGMSV(void *Inst_, void *InstParam, void *GlobalParam,
  ...
  while (NextInst != NULL) {
    if (!AllParams->calib_params->pricerho_on_alpha ||
        !AllParams->calib_params->calib_alpha) {
      if (!AllParams->calib_params->calib_alpha &&
          !AllParams->calib_params->calib_smile_on_prim) {
        /* First Recalculate the Shifted Log params */
        err = Calculate_ShiftAndVol_LGMSV_FromLGM_struct(
            NextInst, NextHestonParam->sCalibCpnSchedule, model,
            NextHestonParam->HestonInst);
...
Err ExtrapolFlatRho_LGMSV(void *Model, void *GlobalParam,
                          int last_smile_index) {
  return NULL;
}
...
Err UpdateParamsAfterFlatRho_LGMSV(void *Inst_, void *InstParam,
  ...
  if (AllParams->calib_params->calib_smile_on_prim) {
    err = CalibrateParamTS(0, 0, AllParams->AllLongInst, &InstParam,
                           GlobalParam, Model, AllParams->CalibParamsAlpha,
                           &(AllParams->CalibFunctionsForAlpha));
  } else {
    err = CalibrateParamTS(0, 0, AllParams->AllShortInst, &InstParam,
                           GlobalParam, Model, AllParams->CalibParamsAlpha,
                           &(AllParams->CalibFunctionsForAlpha));
  }

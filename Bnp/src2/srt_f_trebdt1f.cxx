...
static Err bdttrelim(SrtStpPtr stp, SrtTreStpInf *maxtrinf) {
  Err err;
  SrtStpPtr cur;
  SrtTreStpInf *trinf;
  SrtIRMTmInf *tminf;
  ...
  while (cur->next) {
    ...
    trinf->xmin =
        -cur->index * delta_x + log(sam_get(tminf->fwd_sam, 0, F_0_t));
    cur = cur->next;
  }
...
static void bdtdrfatsam(SrtStpPtr stp, SrtSample *cur_sam, SrtSample *drift_sam,
                        double theta) {
  double mu;
  SrtIRMTmInf *tminf;
  ...
  samptr_get(drift_sam, 0, STATEVAR) =
      samptr_get(cur_sam, 0, STATEVAR) + mu * stp->delta_t;
  return;
}
...
static void gen_x(SrtStpPtr stp, /* ptr to current step in lst */
...
static double calc_adj(SrtIRMTmInf *tminf, SrtTreStpInf *trinf, double dt1,
                       double dt2, double *x, double *Q, double df_at_2dt) {
...
static void generate_prob(SrtStpPtr stp, SrtTrinTreNdInf *node, double varatsam,
                          double adj, double cur_x, double *next_x) {
  double dt, r;
...
static void calc_prob_and_Q(SrtStpPtr stp, double *cur_x, double *cur_Q,
                            double adj, double *next_x, double *next_Q) {
...
Err srt_f_trebdt1f(SrtUndPtr und, /* the underlying with its parameters */
  ...
  /* attach one more step at the end of step list        , for use in forward
     induction because the discounting factor at that step is required */
  ...
  next_assets = dmatrix(maxtrinf.min_x_index, maxtrinf.max_x_index, 0,
                        grfnparam->node_dim - 1);
  ...
  tminf = stp->tminf[0];
  for (i = trinf->min_x_index; i <= trinf->max_x_index; i++) {
    sam_get(node.cxxur_sam, 0, STATEVAR) = cur_x[i];
    sam_get(node.cxxur_sam, 0, SHORT_RATE) = exp(cur_x[i]);
    err = evalcf(
        (GrfnEvent *)stp->e, &node.cxxur_sam, gd, (double *)cur_assets[i],
  ...
  for (stp = stp->prev, stp_num--; stp != NULL; stp = stp->prev, stp_num--) {
    ...
    gen_x(stp, cur_x);
    /* In BK model        , variance of X is only a function of t and not X ,
       hence it needs to be calculated only once per step */
  ...
  strncpy(io_request_val_p->computation_origin, "Tre_BDT_1F",
          strlen("Tre_BDT_1F"));
  ...
  free_dmatrix(cur_assets, maxtrinf.min_x_index, maxtrinf.max_x_index, 0,
               grfnparam->node_dim - 1);
  free_dmatrix(next_assets, maxtrinf.min_x_index, maxtrinf.max_x_index, 0,
               grfnparam->node_dim - 1);

    ...
    SrtGreekType   greek)
{
    ...
    ***/
    ...
    {
        premium =
            disc * cp * (rungs[num_rungs] - strike) +
            gb * srt_f_optblksch(fwd_price, rungs[num_rungs], vol, mat, disc, call_put, PREMIUM);
    }
    else
    {
        if (first_rung == 1)
        {
            premium = gb * srt_f_optonetou(
                               fwd_price,
                               spot,
                               strike,
                               rungs[first_rung],
            ...
            premium += gb * srt_f_optblksch(
                                fwd_price, rungs[num_rungs], vol, mat, disc, call_put, PREMIUM);
        }
        else
        {

...
***/
	...
	if (call_put == SRT_PUT  && (spot<hist_ext))
		hist_ext = spot;
          
	for(i=1;rungs[i]>0 && i<=MAX_RUNG;i++);  	/* set number of rungs */
		num_rungs = i-1;                   

	if (rungs[1]<=strike && call_put == SRT_CALL)
		return(0.0);
	else
	if (rungs[1]>=strike && call_put == SRT_PUT)
		return(0.0);
...
{	
	...
	if (  ((down_up == SRT_DOWN) && (call_put == SRT_CALL))	/*  if decreasing, then put*/  
	   || ((down_up == SRT_UP) && (call_put == SRT_PUT)) )	/*  if increasing, then call*/ 
		return(0.0);     
}
...
else 
{
	if (first_rung==1) 
	{ 
		premium = gb * srt_f_optonetou(fwd_price,
										spot,
										strike,
										rungs[first_rung],
										vol,
		...
		premium += gb*srt_f_optblksch(	fwd_price,
										rungs[num_rungs],
										vol,
										mat,
										disc,
...
{                                  /* DELTA FWD */
	...
	answer = ( srt_f_optladder(	fwd_price+shift,
								spot,
								strike,
								hist_ext,
								rungs,
...
{                                  /* DELTA */
	shift = spot / 1000;
	answer = ( srt_f_optladder(	fwd_price* ( 1 + shift/spot),
								spot+shift,
								strike,
								hist_ext,
								rungs,
...
{                                  /*   GAMMA FWD  */	
	...
	answer =  ( srt_f_optladder(fwd_price+shift,
								...
								grad_best,
								PREMIUM) 
			+ srt_f_optladder(	fwd_price-shift,
								spot,
								strike,
								hist_ext,
								rungs,
...
{                                  /*   VEGA    */
	shift = GVOPT.vol_add;
	answer = ( srt_f_optladder(	fwd_price,
								spot,
								strike,
								hist_ext,
								rungs,

...
#define GRFN_UND_LIST_SIZE                                                     \
  64 /* Maximum number of underlyings allowed in the list */
...
/*******************************************************************************
    ...
    DESCRIPTION :   Given a string s        , will return the positional index
of s in the list of underlying names if found        , -1 otherwise.
...
/*******************************************************************************
    ...
    DESCRIPTION :   Given an integer i        , returns the string in the static
array _grfn_und_list[i] (if valid).
...
/*******************************************************************************
    ...
    DESCRIPTION :   Saves the name of the underlying curve found in the
                    GRFN tableau in the static string list _grfn_und_list.
                                        Names are stored in the same order as
that found in the tableau        , except domestic that is stored as
_grfn_und_list[0]
...
/*******************************************************************************
    ...
    DESCRIPTION     :   This function (indirectly) creates a (static) list of
                        ...
                        or not a list needs to be created        , i.e. if
gd->pass = 1 then we are just parsing and extracting information from the
tableau and NOT creating events.
...

{
  ...
  /*We start getting the underlying names at index 1 NOT 0 ( the domestic one is
   * in position 0)*/
  i = 1;
  ...
  while (1) {
    ...
    if (s == NULL) {
      ...
      /* We have reached the end of the list of names : make it a blank one (not
       * to loose address) */
      strcpy(list[i], "");
      break;
    } else {

...
static double Psi_Func(double x, double y, double T, double s, double t) {
  ...
  if ((fabs(x) > 1.0e-8) && (fabs(y) > 1.0e-8)) {
    ...
    if (fabs(x) > 1.0e-8) {
      ...
      result =
          1 / x * (T * (t - s - Phi_Func(x, T, s, t)) + B_Func(x, T, s, t));
    } else {
      if (fabs(y) > 1.0e-8) {
        /* y different from 0  , x equal to 0 */
        result =
            1 / y * (T * (t - s - Phi_Func(y, T, s, t)) + B_Func(y, T, s, t));
      } else {
        /* both equal to 0 */
...
void fill_fwd_var(long nstp, double *time, double *date, double *sig_dom,
                  double *sig_for, double *sig_fx, double dom_lam,
                  ...
                  double *dom_ifr, double *dom_fwd, double *dom_var,
                  double *for_ifr, double *for_fwd, double *for_var,
  ...
  for (i = 1; i < nstp; i++) {
    ...
    for (j = 0; j < i; j++) {
      ...
      c = vol_for * vol_for * Psi_Func(for_lam, for_lam, t, t1, t2) +
          ...
          2.0 * corr_dom_for * vol_dom * vol_for *
              Psi_Func(for_lam, dom_lam, t, t1, t2);
      ...
      L = 1.0 / (dom_lam * dom_lam) *
          (aux2 * (1.0 - 0.5 * aux2) - aux1 * (1.0 - 0.5 * aux1));
    ...
    fx_var[i] = var_fx;
    fx_fwd[i] = fx_fwd[i - 1] +
                (prev_rd - prev_rf - vol_fx * vol_fx / 2.0) * (t - prev_t);
...
void fill_fwd_var_corr(
    long nstp, double *time, double *date, double *sig_dom, double *sig_for,
    double *sig_fx, double dom_lam, double for_lam, double *correl_dom_for,
    double *correl_dom_fx, double *correl_for_fx, char *dom_yc, char *for_yc,
    /*	To be allocated by caller  , filled by the function */
    double *dom_ifr, double *dom_fwd, double *dom_var, double *for_ifr,

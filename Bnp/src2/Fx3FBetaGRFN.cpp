...
double		bar_lvl)
{
			...
			for (j=0; j<n2; j++)
			{
				for (k=0; k<n3; k++)
				{
					...
					if (i >= 0)
					{
						...
						local->smp.und[total->fx_idx].sv[SPOT] = exp((log((1.0 - beta) * bar_lvl + 1.0)) / (1.0 - beta)) - 1.0e-04;
	...
	for (i=0; i<n1; i++)
	{
		for (j=0; j<n2; j++)
		{
			for (k=0; k<n3; k++)
			{
				local->smp.und[total->fx_idx].sv[SPOT] = exp((log((1.0 - beta) * sv[i][j][k][2] + 1.0)) / (1.0 - beta)) + 1.0E-08;

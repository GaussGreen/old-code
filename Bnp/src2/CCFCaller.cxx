...
Err ccf_caller(
  ...
  err = ccf_fill_check_all_struct(
      today, use_calib, yc, vc, ref, swap_freq, swap_basis, lam_ts, lambda,
      tsnlam, tslamtime, tslam, alpha, gamma, rho, get_cash_vol, lgm2dund,
      fund_not, fund_ncpn, fund_fix, fund_start, fund_pay, fund_basis, fund_spr,
      fund_mrg, cf_not, cf_ncpn, cf_fix, cf_start, cf_pay, cf_basis,
      cf_cms_tenor1, cf_cms_freq1, cf_cms_basis1, cf_cms_spread1, cf_cms_tenor2,
      cf_cms_freq2, cf_cms_basis2, cf_cms_spread2, spread_vol_n,
      ...
      num_strikes_in_vol, strikes_in_vol, vol_type, cash_vol, ncall, pay_rec,
      ex_date, set_date, fee, req_stp, req_stpx, req_paths, force_atm,
      max_std_long, max_std_short, fix_lambda, cal_vol_shift_type,
      cal_vol_shift, cal_lambda_shift, one_f_equi, skip_last, long_prec,
      short_prec, min_fact, max_fact, use_jumps, proba_weight, eod_fix_flag,
      eod_ex_flag, ccf, und, &call_feat, adi_arg);
        ...
        case 2:
          ...
          if (exo_cpn->cms_fix_time > 1.0E-08) {
            ...
            stdcap = fabs(exo_cpn->alphabeta[0]) *
                     sqrt(exo_cpn->cms[0].cxxapvar * exo_cpn->cms_fix_time);
          } else {
            stdfloor = stdcap = 0.0;
          }
          break;
        ...
        case 3:
          if (exo_cpn->cms_fix_time > 1.0E-08) {
            ...
            if (is_corr) {
              ...
              stdcap = sqrt(exo_cpn->alphabeta[0] * exo_cpn->alphabeta[0] *
                                exo_cpn->cms[0].cxxapvar +
                            exo_cpn->alphabeta[1] * exo_cpn->alphabeta[1] *
                                exo_cpn->cms[1].cxxapvar +
                            2.0 * stdcap * exo_cpn->alphabeta[0] *
                                exo_cpn->alphabeta[1] *
                                sqrt(exo_cpn->cms[0].cxxapvar *
                                     exo_cpn->cms[1].cxxapvar));
            }

...
static double _StochShift_h(double x, double rho, double f, double l,
                            double alpha, double sigma, double t, double K) {
  return rho * x +
         ((f + l) * exp(-.5 * sigma * sigma * t + sigma * sqrt(t) * x) - l -
          K) /
             alpha / sqrt(t);
}

char *_StochShift_Exact(double in_dF, double in_dL, double in_dalpha,
  ...
  *out_dPrice = -1.0;
  ...
  dSum = 0.0;
  for (i = 1; i < N; i++) {
    h = _StochShift_h(x[i], in_drho, in_dF, in_dL, in_dalpha, in_dsigma,
                      in_dTau, in_dK);
...
char *_StochShift_Approx(double in_dF, double in_dL, double in_dalpha,
                         double in_dsigma, double in_drho, double in_dK,
                         double in_dTau, struct StochShift_Param *in_Param,
                         double *out_dPrice) {
  /* Variable Declaration */
  double normStd, normStd2, normStd4, h0, Nh0, nh0, fpl, fpl2, intrin, intrin2,
      logStd, logStd2;
  ...
  dTerm4 = 0.125 * logStd2 * logStd2 / normStd4 / normStd * fpl2 * nh0 *
           (fpl2 * intrin2 - 4.0 * fpl * intrin * normStd2 - fpl2 * normStd2 +
            2 * normStd4);
...
char *StochShift_PriceCall(double in_dF, double in_dL, double in_dalpha,
  ...
  case SS_EXACT:
    return _StochShift_Exact(in_dF, in_dL, in_dalpha, in_dsigma, in_drho, in_dK,
                             in_dTau, in_Param, out_dPrice);
  case SS_APPROX:
    return _StochShift_Approx(in_dF, in_dL, in_dalpha, in_dsigma, in_drho,
                              in_dK, in_dTau, in_Param, out_dPrice);
  }

...
static double _StochShift_h(	double x,
								double rho,
								double f,
								double l,
								double alpha,
								...
								double K )
{
	return rho*x + ( (f+l)*exp(-.5*sigma*sigma*t+sigma*sqrt(t)*x ) - l - K ) / alpha / sqrt(t);
}



char* _StochShift_Exact(	double in_dF, 
							double in_dL, 
							double in_dalpha,
							double in_dsigma,
							double in_drho,
...
char* _StochShift_Approx(double in_dF, 
							double in_dL, 
							double in_dalpha,
							double in_dsigma,
							double in_drho,
...
/* triage */
char* StochShift_PriceCall( double in_dF, 
							double in_dL, 
							double in_dalpha,
							double in_dsigma,
							double in_drho,
...
/* Call the correct model */
	...
	{
		case SS_EXACT:
			return _StochShift_Exact(	in_dF, 
											in_dL, 
											in_dalpha,
											in_dsigma,
											in_drho,
											...
											out_dPrice );
		case SS_APPROX:
			return _StochShift_Approx(	in_dF, 
										in_dL, 
										in_dalpha,
										in_dsigma,
										in_drho,

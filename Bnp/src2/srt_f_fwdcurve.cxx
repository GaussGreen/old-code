...
double srt_f_forward_from_fwdcrv(double start_time, SrtCurvePtr dvdcrv,
                                 SrtCurvePtr repocrv) {
...
Err srt_f_init_DividendCurve(Date today, char *ccy, double **dividend_curve,
                             int ncols, int nrows, String dvd_name) {
  SrtDvdObj *dvd_obj;
  ...
  err = srt_f_dvdobj_init(today, dividend_curve, ncols, nrows, dvd_name,
                          &dvd_obj);
  if (err)
    return err;

  /*  Puts the SrtDvdObj as Curve in the Curve List with fwd_name as the
   * reference */
  err = swp_f_addcurvetolist(curve_list, dvd_name, "DVD_CURVE", NULL, ccy,
                             today, 0, NULL, (void *)(dvd_obj));
  if (err) {

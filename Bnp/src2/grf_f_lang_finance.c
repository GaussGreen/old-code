...
/* Function to compute right away all the pay dates   , the spreads (with their
   coverage) on the floating leg  */
static Err grfn_make_floating_leg(SwapDP *float_sdp, Date today,
  ...
  /* If dealing with CASH  , there is no need to do this ( float leg = df(f) -
   * df(i) )*/
...
Err check_financial_args(String name, COMLLType type, COMLL_PTR lastarg,
  ...
  /* s = start  , e = end  , c = compounding  , b = basis  , d = double  , t =
   * string */
  switch (type) {
  case COMLL_F_PAYOFF:
    code = "dt";
...
/* ------------------------------------------------------------------------------
    ...
    DESCRIPTION :   Pay requires a special treatment also because it
                                        ...
                                        In case of simulations  , the report
   does not have to be done (the report should only happen if in the past)
   ...
   PAYFROMPAST(TODAY  ,DATE) (top)               (bottom) that will correspond
   to the following operations AMOUNT * DF(TODAY  ,paydate) and will allow for
   proper computations once the COMLL is evaluated without failing on the use of
   DF

                    If we are in the future  , use the COMLL_PAY type  ,
                                        setting the discretisation date as the
   start date of the discounting: AMOUNT --> PAY..(NOW  ,DATE)  , that will
   correspond to the following operations AMOUNT * DF(NOW  ,paydate) once the
   COMLL is evaluated
...
static Err pay_function(String name, GrfnSymbol *gs, GrfnDeal *gd,
  ...
  if (gd->is_history) {
    ...
    /* Pay is used in the past  , with (complex) payment in the future : make it
       a [ AMOUNT * PAYFROMPAST ] COMLL */
    else {
  ...
  /* Pay is used in the FUTURE  , with a simple or complex payment expression:
     make it a [ AMOUNT * PAY ] COMLL */
  ...
  /* Removes the top element of the COMLL  , where the payment date was stored
   * (keeping address of top the same) */
  comll = top->next;
...

{
  ...
  /* From now on  , the underlying name has been stripped out:any optional
     argument left is a short ref rate  , a long one or both (opt_args has not
     been changed) */
  ...
  if ((gd->nowdt == gd->today) &&
    ...
    if (opt_args == gs->no_opt_args) {
      ...
      if ((top->type == COMLL_STRING) && (gs->no_opt_args >= 2)) {
        ...
        /* Have we got a function to get the reference rates fixings */
        if (gd->fixing_fct != NULL) {
          /* Check to see if the fixing date of the FRA is today  , or in the
           * future */
          /* Make sure the end date is properly defined and set (not only nfp)
           */
          err = swp_f_setSwapDP(start, end_nfp, comp, basis, &sdp);
  ...
  /* --------------------------------------------------------------------------
     ...
     Since we need to be able to collect the short reference rate name  , we
     only get rid of the first optionnal arguments for a COMLL_F_SWAP. The und
     name has already been stored in the first parsing of the tableau  , and
  ...
  /* ----------------------------------------------------------------------------------------
     Prepares the arguments (dates  , space for df  , cvg...) that will be
     required for the computation of the financial function  , and stack them
     into top
     ----------------------------------------------------------------------------------------
   */
  ...
  case COMLL_F_FRA:
    ...
    if (und_index >= 0) {
      ...
      if (top->spread[0] == SRT_SPREAD_ERROR)
        return serror("Error in getting %s(%d  ,%d) spread", short_ref_rate,
                      top->dfind[0], top->dfind[1]);
    }

...
/* Make the floating leg: dates, times, cvg, spreads ...*/
	...
	if (start_dates) srt_free(start_dates);
	if (end_dates) srt_free(end_dates);
	if (fixing_dates) srt_free(fixing_dates);
...
static Err pay_function (   String      name,
                            GrfnSymbol  *gs,
                            GrfnDeal    *gd,
                            COMLL_PTR   lastarg   )
{
...
/* Pay is used in the PAST */
	...
	/* Pay is used in the past, with (complex) payment in the future : make it a [ AMOUNT * PAYFROMPAST ] COMLL */
		else
		{			
...
/* Pay is used in the FUTURE, with a simple or complex payment expression: make it a [ AMOUNT * PAY ] COMLL */
...
/* If necessary, make initialisation for df calculations */
	...
	/* Stores the discount factor dates (df_start and pay date) in the bottom element (as for a DF type ) */
		bot->dfindlen = 2;
...
/* Removes the top element of the COMLL, where the payment date was stored (keeping address of top the same) */
	comll = top->next;
...
static Err payn_function (   String      name,
                            GrfnSymbol  *gs,
                            GrfnDeal    *gd,
                            COMLL_PTR   lastarg   )
{
...
Err grfn_interp_fin_func    (   String      name,
                                GrfnSymbol  *gs,
                                GrfnDeal    *gd,
                                COMLL_PTR   lastarg     )
...
/* -------------------------------------------------------------------------
   ...
   ------------------------------------------------------------------------- */
	...
	/* Return an error message for DF and PVRNG in the PAST */
        ...
        if (   gs->type == COMLL_F_DF
			|| gs->type == COMLL_F_PVRANGE
			|| gs->type == COMLL_F_CMS
			|| gs->type == COMLL_F_LEVEL
...
/* calculate the fixing date */
			...
			/* Get the historical fixing for the first ref rate code ( the long one) on that date */
...
/* get the market name from the underlying */
				...
				top->dval = swp_f_fra( sdp.start, sdp.end, sdp.basis_code, szYieldCurve, comll->sval );
			}
...
/* ----------------------------------------------------------------------------------------
   ...
   ---------------------------------------------------------------------------------------- */
	...
	if (	gs->type == COMLL_F_YIELD 
		||	gs->type == COMLL_F_CLEAN)
	{
		top = top->next->next;
	}
	top->type = gs->type;
	...
	switch (gs->type) 
	{
		...
		/* Request DFs dates if needed */
			...
			{
				err = product->RequestDfDates(product,
					(long)top->dval, &n_unds, &n_dfs, &dates);
				if (err) return err;
		...
		/* Store und_index in ivec, swaps dates, cvg, sprd ... in top */
			...
			/* Create and populate the fixed leg (dates + cvg...): end is properly set */
				...
				err = grfn_make_swapdates(&sdp,gd->today, SWAP, &top->dfind,
								&top->cvg, &top->dfindlen);
				if (err)
					return err;

			/* Create and populate the floating leg (backward from same end as fixed leg) */
				sdp.direction = BKWD;
				err = grfn_make_floating_leg    ( &sdp, gd->today,
								SWAP,
								&top->dfindfloat,
								&top->cvgfloat, 
								&top->spread,
			...
			/* If the long ref rate is mentionned and the short ref rate == CASH, get long spread */
				if ( !(strcmp(short_ref_rate,"CASH")) && (strcmp(long_ref_rate, "")!= 0))
				{
					...
					if (top->long_spread == SRT_SPREAD_ERROR)
						return serror("Cannot compute long spread for %s(%d,%d)",long_ref_rate,sdp.start,sdp.end);
				}
				else
		...
		case COMLL_F_FRA:
			...
			/*Spread */	
				...
				if (top->spread[0] == SRT_SPREAD_ERROR)
					return serror("Error in getting %s(%d,%d) spread",short_ref_rate, top->dfind[0],top->dfind[1]);
			 }
		...
		case COMLL_F_ACCINT:
			...
			{
				top->dval = coverage(DTOL(floor(gd->aux[aux[0]][i])), sdp.start,
							sdp.basis_code) * gd->aux[aux[1]][i];
			}

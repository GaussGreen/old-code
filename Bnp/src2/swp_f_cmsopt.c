/* =======================================================================================
   ...
   DESCRIPTION     :Contains all versions of 
					value of cms (rate and option) through integration with swaptions
...
Err swp_f_cmsoption(double            forward, 
					double            num_periods, 
...
Err               err;
	...
	err  = swp_f_Cms_Option(forward, maturity, num_periods, strike, 
							(double) frequency, pay_or_receive, delay,	
							1, lognrm_nrm, volatility, 0, (Date) 0, (Date) 0, 
							SRT_FALSE, 0,NULL, 0, NULL, ans);
	return err;
}

Err swp_f_Cms_Option(double				dFwdSwapRate, /*Forward Swap Rate */
...
{
Err					err = NULL;
double				*pdCoeffApprox = NULL,
					*pdVolVector = NULL;
double				dFactor_n = 1.0, 
					dVol = 0.0,
					dVal = 0.0,
					dAdjForwardRate,
					dDeltaStrike,
...
int                 OldType;
	...
	if(dMaturity <= 0.0)
	{
		...
		if(PayRec == SRT_RECEIVER) dVal*= -1.0;
		/* max (...,0) */
	...
	else
	{
		...
		if(PayRec == SRT_RECEIVER) dSignRecPay *= -1;

		/* If Error in the vol matrix then switch to constant vol*/
		if  (iMethod == 1 && lNumStrikesInVol == 0) iMethod = 0;

		/* Set a flat vol just in case 
			otherwise get the vol for the strike */
		if (!iMethod) dVol = dFlatVol;
		else if(err = swp_f_truncvol(szVolCurveName, dStart, dEnd, dStrike, &dVol, &power)) return err;
		
		if (CMS_SPACE_TYPE == SRT_LOGNORMAL)
			dDeltaStrike = exp(log(
				( - (dSignRecPay - 1.0 ) *1.0e-10 + (dSignRecPay + 1.0 ) * BOUNDARYINT / 2.0 ) / dStrike) 
				/ CMS_SPACE_NUMSTEP);
		else
			dDeltaStrike = (- (dSignRecPay - 1.0 ) *1.0e-10 + (dSignRecPay + 1.0 ) * BOUNDARYINT / 2.0 - dStrike) 
				/ CMS_SPACE_NUMSTEP;
		...
		if (iMethod == 1)
		{
			...
			if (pdCoeffApprox == NULL)	return "Cms allocation Error";

			pdVolVector = dvector(0, lNumStrikesInVol);
			if (pdVolVector == NULL)	return "Cms allocation Error";
			...
			if(err = swp_f_truncvol(szVolCurveName, dStart, dEnd, pdStrikesVol[iLower-1], &pdVolVector[iLower-1], &power)) return err;
			
			for(i = iLower; i <= iHigher; i++)
			{
				if(err = swp_f_truncvol(szVolCurveName, dStart, dEnd, pdStrikesVol[i], &pdVolVector[i], &power)) return err;
				pdCoeffApprox[i-1] = (pdVolVector[i] - pdVolVector[i-1])/(pdStrikesVol[i]-pdStrikesVol[i-1]);
			}
		...
		/* If the Model is Lognormal, we have to restrict to positive strikes */
			if (CMS_SPACE_TYPE == SRT_LOGNORMAL)
			{
				...
				/* If there is not enough space for two swaptions, just do the one and return */
				if( (dStrike*(1.0 - dDeltaStrike*dDeltaStrike)) > (dStrike - CMS_EPSILON) )
				{	
					if (VolType == SRT_NORMAL)
						*dCMSOptionValue = srt_f_optblknrm(dAdjForwardRate, dStrike, dVol, dMaturity, 1.0, CallPut, PREMIUM);
					else
						*dCMSOptionValue = srt_f_optblksch(dAdjForwardRate, dStrike, dVol, dMaturity, 1.0, CallPut, PREMIUM);
		...
		/* ... now this is a payer (call on the rate) */
			...
			if( dStrike < CMS_EPSILON) dStrike = CMS_EPSILON;

		/* First term: swaption, centered on strike */
		if (VolType == SRT_NORMAL)
			dOpt = srt_f_optblknrm(dAdjForwardRate, dStrike, dVol, dMaturity, 1.0, CallPut, PREMIUM);
		else
			dOpt = srt_f_optblksch(dAdjForwardRate, dStrike, dVol, dMaturity, 1.0, CallPut, PREMIUM);
		...
		if (iMethod) if(err = swp_f_truncvol(szVolCurveName, dStart, dEnd, r * dFrequency, &dVol, &power)) return err;
		...
		if (VolType == SRT_NORMAL)
			dOpt = srt_f_optblknrm(dAdjForwardRate, r * dFrequency, dVol, dMaturity, 1.0, CallPut, PREMIUM);
		else
			dOpt = srt_f_optblksch(dAdjForwardRate, r * dFrequency, dVol, dMaturity, 1.0, CallPut, PREMIUM);
		...
		/* Get the Vol at strike K + (i - 1) * delta_k */
			if (iMethod == 2)
				if (err = swp_f_truncvol(szVolCurveName, dStart, dEnd, r * dFrequency, &dVol, &power)) return err;

			if (iMethod == 1)
			{
				if (dSignRecPay*(r * dFrequency - pdStrikesVol[min(m, lNumStrikesInVol - 1)]) >= 0.0) 
					m+=dSignRecPay;
				m = min(lNumStrikesInVol - 1 + iLeftRightPos, 
						max(m, iLeftRightPos));
				dVol = pdVolVector[m-iLeftRightPos] + pdCoeffApprox[m-iLeftRightPos] 
			...
			/* two ways of convergence :: the integral has converged or the boundary is reached */
...
Err swp_f_Cms_OptionNY(double			dFwdSwapRate, /*Forward Swap Rate */
					 double				dMaturity, /* CMS Maturity as double */
					 double				dNumPeriods,
					 double				dStrike, /* Option Strike */
					 double				dFrequency, /* CMS Frequency */
...
double				dSens1Sliding;
	...
	err = swp_f_initSwapDP(dStartSliding,dEndSliding,"SEMIANNUAL","BB",&swapdp);
	if (err)
		return err;
...
Err swp_f_Tec_Option(double				dFwdSwapRate, /*Forward Swap Rate */
...
double				dVol = .0,
					...
					dSens1;
double              power = 0.0;
int					i, j, m = 0, iNumSwaps, dSignRecPay = 1, iLeftRightPos = 0,
					iLower, iHigher;
SRT_Boolean				bConverged = SRT_FALSE;
SrtCallPutType		CallPut;
double		        *dTECw = NULL;
	...
	else
	{
		...
		/* Set a flat vol just in case 
			otherwise get the vol for the strike */
		if (!iMethod) dVol = dFlatVol;
		else if(err = swp_f_truncvol(szVolCurveName, dStart, dEnd, dStrike - dMargin, &dVol, &power)) return err;
		
		/* Let's be sure that we won't use strikes < 0 	
		dDeltaStrike = DEFAULT_CMS_DELTA_STRIKE;
		iNumSwaps = MAX_CMS_SWAPS;
		if (PayRec == SRT_RECEIVER)
			iNumSwaps = min((int)((dStrike - dMargin - CMS_EPSILON) / dDeltaStrike), 
							iNumSwaps);

		 reverse the sign if needed 
		dDeltaStrike *= dSignRecPay; */
	
		dDeltaStrike = (- (dSignRecPay - 1.0 ) *1.0e-10 + (dSignRecPay + 1.0 ) * BOUNDARYINT / 2.0 - dStrike + dMargin) 
				/ CMS_SPACE_NUMSTEP;
		...
		{
			pdCoeffApprox = dvector(0, lNumStrikesInVol);
			if (pdCoeffApprox == NULL)	return "Tec allocation Error";

			pdVolVector = dvector(0, lNumStrikesInVol);
			if (pdVolVector == NULL)	return "Tec allocation Error";
			...
			if(err = err = swp_f_truncvol(szVolCurveName, dStart, dEnd, pdStrikesVol[iLower-1], &pdVolVector[iLower-1], &power)) return err;
		...
		/* If the Model is Lognormal, we have to restrict to positive strikes */
			...
			{				
				...
				{
					if (VolType == SRT_NORMAL)
						dVal = srt_f_optblknrm(dAdjForwardRate, dStrike - dMargin, dVol, dMaturity, 1.0, CallPut, PREMIUM);
					else
						dVal = srt_f_optblksch(dAdjForwardRate, dStrike - dMargin, dVol, dMaturity, 1.0, CallPut, PREMIUM);

					r = (dDeltaStrike + dStrike - dMargin) / dFrequency;
					dSens = pow(1.0 + (dAdjForwardRate / dFrequency), dNumPeriods);
					dSens1 = pow(1.0 + r , dNumPeriods);
					dSens = pow(1.0 + (dAdjForwardRate / dFrequency), dDelay) * (1.0 - (1.0 / dSens)) / dAdjForwardRate;
					dSens1 = pow(1.0 + r , dDelay) * (1.0 - (1.0 / dSens1)) / r;
					dTECw[0]= dFrequency * dSens1 ;
					dVal*= (1/dPaymentPower)*(pow((1 + dStrike),(1/dPaymentPower)-1) 
						+ (1/dPaymentPower-1) * dDeltaStrike * pow( (1 + dStrike),(1/dPaymentPower)-2) ) * dTECw[0] * dSens;
		...
		/* ... now this is a payer (call on the rate) */
			...
			if( dStrike < CMS_EPSILON + dMargin)
		...
		if (VolType == SRT_NORMAL)
			dOpt = srt_f_optblknrm(dAdjForwardRate, dStrike - dMargin, dVol, dMaturity, 1.0, CallPut, PREMIUM);
		else
			dOpt = srt_f_optblksch(dAdjForwardRate, dStrike - dMargin, dVol, dMaturity, 1.0, CallPut, PREMIUM);
		...
		if(dPaymentPower != 1)
			dAmt= (1/dPaymentPower)*(pow((1+dStrike),(1/dPaymentPower-1))
				+(1/dPaymentPower-1) * dDeltaStrike*pow((1+dStrike),(1/dPaymentPower-2)))*dTECw[0];
		else
			dAmt = dTECw[0];

		dVal= dAmt * dOpt;

		/* Second term: Swaption centered on option strike - m + delta_k ( pos. or neg.)*/
		if (iMethod) if(err = swp_f_truncvol(szVolCurveName, dStart, dEnd, r * dFrequency, &dVol, &power)) return err;
		...
		{
			for( j=0 ; j < 2 ; j++)
				dAmt +=pow((1 + dStrike + (1-j) * dDeltaStrike),(1/dPaymentPower-2)) * dTECw[j];
			dAmt =(1/dPaymentPower)*pow((1+dStrike),(1/dPaymentPower-1)) * dTECw[1] + 
		...
		/* Coefficient from the series (recursive in 2 steps) 
			...
			dTECw[i-1]= (i * dInvLvl) - (2.0 * (i - 1) * dInvLvlm1) + ((i - 2) * dInvLvlm2);
			...
			{
				for(j = 0; j < i; j++)
					dAmt += pow((1 + dStrike + ((i - 1) - j) * dDeltaStrike),(1/dPaymentPower-2))*dTECw[j];

				dAmt = (1/dPaymentPower) * pow((1 + dStrike),(1 / dPaymentPower-1))*dTECw[i-1] 
					+ (1/dPaymentPower) * (1/dPaymentPower-1) * dDeltaStrike * dAmt;
			}
			else
				dAmt = dTECw[i-1];
		...
		if (dTECw) free_dvector(dTECw,0,(long) CMS_SPACE_NUMSTEP);
		dTECw = NULL;
...
/*	--------------------------------------------------------------------
			Alan: the following code is used in the direct integration
	-------------------------------------------------------------------- */


/* Level cash Numeraire used for direct integration */
double CMSNumeraire(double		dForwardRate, 
					double		dDelay,
					double		dFrequency,
					double		dNumPeriods)
{
...
double CMSdNumeraire(double		dForwardRate, 
					 double		dDelay,
					 double		dFrequency,
					 double		dNumPeriods)
{
...
int i;
	...
	/* Init the weights */
	w[0] = w[2 * n] = INV_SQRT_TWO_PI * 11 / 24  
					  * exp(- x[0] * x[0] / 2.0 ) * dx;

	w[1] = w[2 * n - 1] = INV_SQRT_TWO_PI 
						  * exp( - x[1] * x[1] / 2.0 ) * dx;

	w[2] = w[2 * n- 2] = INV_SQRT_TWO_PI * 25 / 24 
						 * exp( - x[2] * x[2] / 2.0 ) * dx;
...
double TecCmsGenericPayoffDensity(
	double				S, 
	double				dStrike, 
	...
	double				dTecPower)
{
	switch(PayRec) 
	{
		/* Cms Cap Case */
		case SRT_PAYER:
			if (S >= dStrike)	return (S - dStrike);			
			break;
		/* Cms Floor Case */
		case SRT_RECEIVER:	
			if (S < dStrike)	return (dStrike - S);
			break;
		/* Cms forward Case */
...
Err swp_f_FlatTecCmsOption (double				dFwdSwapRate, 
							Date				StartDate, 
							Date				TheoEndDate,
							double				dMaturity, 
							double				dNumPeriods,
	...
	*dGenericValue = 0.0;
	for(i = 0; i <= 2 * NumHermitePoints; i++)
	{
		...
		if (VolType == SRT_NORMAL)
			...
			S = dAdjForwardRate * 
				exp(-dStdDev * dStdDev / 2.0 + dStdDev * pdx[i]);
		...
		*dGenericValue += pdWeight[i] 
				* TecCmsGenericPayoffDensity(S, dStrike, PayRec, 
										  dMargin, dPower) / Numeraire;
	}
...
double 	srt_f_OptGeneric(double 			Fwd, 
						 double 			Strike,
						 double 			Vol, 
						 SrtDiffusionType	VolType, /* Vol type Lognormal or Normal */
						 double 			Mat, 
...
{ /* See Cms Bnp Approximation for the poor notation of the variable */
double	dL = 0.0, dM = 0.0, dTemp = 0.0, dStdDev, dK, Y, dRate;
	...
	{
		dK = dNumPeriods / dFrequency / Y  / (pow(Y,dNumPeriods) -1) - 1.0 / dFwdSwapRate;
	}
	if (iMethod == 4)
	{
		dTemp = dCmsSpread + 
						 ...
						 double				*dCmsSpread)
{
	...
	*dCmsSpread = dCmsFullSmile - dFwdSwapRate + dK * dStdDev -
		*dlambda * dNumPeriods * dStdDev * dVol * dVol * dMaturity / dFrequency;
}
				  ...
				  double* volatility, double *power )
{
	...
	if (strike > CAPSTRIKE)
		adjstrike = CAPSTRIKE;
	else adjstrike = strike;

	err = swp_f_vol(vol_id,start, end, adjstrike,  
				  volatility, power);
	return err;
}
...
{
	if (Volstruct->szVolCurveName) free(Volstruct->szVolCurveName);
	Volstruct->szVolCurveName = NULL;
}

Err CMSTECsetupvol(CMSVol	 *Volstruct,
...
int				isSABR =0, isSABRAF =0;
	...
	if (Volstruct->input_method == 4)
	{
		...
		if (Volstruct->atm == 1)
			strikeused = swaprate;
		else strikeused = strike;

		err = swp_f_vol(Volstruct->szVolCurveName, Volstruct->start,Volstruct->end,strikeused, &(Volstruct->vol),&power);
		if (err) return err;

		Volstruct->comp_method = 0;
	}
	else if (Volstruct->input_method == 2 || Volstruct->input_method == 3 || Volstruct->input_method == 250) //3 for BMM in test 
	{
		// Detects if the market is SABR by retrieving the sigmabetavol.
		/* old method
		err = swp_f_SABRvol(Volstruct->szVolCurveName, Volstruct->start, Volstruct->end, swaprate, &yesno,  &power, SABR_BETAVOL);
		*/
		...
		{
			err = swp_f_SABRvol(Volstruct->szVolCurveName, Volstruct->start, Volstruct->end, swaprate, &yesno,  &power, SABR_ZETA) ;
			if (err)
			{
				isSABR = 1;
				isSABRAF = 0;
		...
		if (isSABR || isSABRAF)
		{
			...
			if(err = swp_f_SABRvol(Volstruct->szVolCurveName, Volstruct->start, Volstruct->end, swaprate,  
					  &(Volstruct->vol),  &power, index)) return err;

			if(err = swp_f_SABRvol(Volstruct->szVolCurveName, Volstruct->start, Volstruct->end, swaprate,  
					  &(Volstruct->beta),  &power, 6)) return err;

			if(err = swp_f_SABRvol(Volstruct->szVolCurveName, Volstruct->start, Volstruct->end, swaprate,  
					  &(Volstruct->alpha),  &power, 5)) return err;

			if(err = swp_f_SABRvol(Volstruct->szVolCurveName, Volstruct->start, Volstruct->end, swaprate,  
					  &(Volstruct->rho),  &power, 7)) return err;
			...
			{
				if(err = swp_f_SABRvol(Volstruct->szVolCurveName, Volstruct->start, Volstruct->end, swaprate,  
						  &(Volstruct->zeta),  &power, 9)) return err;
				Volstruct->comp_method = 0;
				Volstruct->input_SABR = 0;
	...
	if (Volstruct->input_SABR == 1)
	{
		...
		err = vol_conv(Volstruct->vol,sabrtype,&tempvol,SABR_ATM_BETA, swaprate,swaprate,
			maturity,Volstruct->alpha,Volstruct->beta, Volstruct->rho);
		if (err) return err;

		if (Volstruct->voltype_input == SRT_BMMVOL) 
			Volstruct->voltype_used = SRT_BMMVOL;
		else
		if (Volstruct->voltype_input == SRT_BMM2VOL)
	...
	else if (Volstruct->input_SABRAF == 1)
	{
		...
		/*	Calculate equivalent vol of the type to be frozen	only ATM types or BETA*/
		err = srt_f_optbmm2vol(	swaprate, 
								swaprate, 
								maturity, 
								Volstruct->vol, 
								Volstruct->alpha, 
...
void CMS_get_discount(double x,
					  double cvg,
					  double spd,
					  double *ZC,
					  double *d1ZC,
...
double CMS_get_reduced_payoff2(double x,
							double nfp,
							double invf,
							double K, /* real strike */
							double invp,
...
double CMS_get_controlvariatefactor(double strikeadj,
									double nfp,
									double invr,
									double m,
									double invp,
...
void    CMS_get_lastpart(double x,
						double forward,
						double vol,
						double maturity,
						double cp,
...
void CMS_get_pdX(double strikeadj,
				 long   Nx,
				 double IntUD,
				 double cp,
				 int    Mesh,
...
void swp_f_FinalCms_Weights(/* underlying */
					 ...
					 long               nx,
					 double             dnfp, /* number of full period of the default swap rate */
					 double				dfreq, /* frequency of the default swap rate 1:A, 2:S, 4:Q */
...
long   n;
	...
	for (n=0; n<nx; n++)
		dCMSWeights[n] = CMS_get_reduced_payoff2(x[n],dnfp,invf,dRealStrike,invp,invr,dMargin,dspdZC,cp,dCvgZC);
}
...
SrtCallPutType 	call_put;
	...
	/* for speed reasons and in order to be ready for the Heston vol, we sort by method */
	if (Volstruct->comp_method == 0)
	{
		...
		{
			for (n=0;n<nx; n++)
				dCMSOpts[n] = srt_f_optblksch(dDefaultSwap,x[n],volused,
				dMaturity,1.0,call_put, SRT_PREMIUM);
		}
		else if (Volstruct->voltype_used == SRT_NORMAL)
		{
			for (n=0;n<nx; n++)
				dCMSOpts[n] = srt_f_optblknrm(dDefaultSwap,x[n],volused,
				dMaturity,1.0,call_put, SRT_PREMIUM);
		}
		else if (Volstruct->voltype_used == SRT_BETAVOL) 
		{
			...
			err = srt_f_optbetastochvoltoblkvol(dDefaultSwap,VolUD, betavol,alpha,rho,dMaturity,beta,&volused);
			if (err) return err;
			...
			for (n=0;n<nx; n++)
			{
				if((cp*(VolUD -  x[n]) >= -10e-17)) 
				{
					err = srt_f_optbetastochvoltoblkvol(
						dDefaultSwap,x[n], betavol,alpha,rho,dMaturity,beta,&volused);
					if (err) return err;
				}
				dCMSOpts[n] = srt_f_optblksch(dDefaultSwap,x[n],volused,
						dMaturity,1.0,call_put, SRT_PREMIUM);
		...
		else if (Volstruct->voltype_used == SRT_BMM2VOL)
		{
			...
			{
				zeta = Volstruct->zeta;
				BMM2GetStates(dDefaultSwap, dMaturity,betavol,alpha,beta, rho,zeta, &Sig1, &Sig2, &Fwd1, &Fwd2,&Pi);
			}
			else
			{
				BMM2CalibOnSabrStates(dDefaultSwap,dMaturity,betavol,beta,alpha,rho,Volstruct->Pi ,Volstruct->NstD ,&Fwd1,&Fwd2,&Sig1,&Sig2,&Pi,SRT_BETAVOL,&res);
				if (res>1e-3) return "SABRAF could'nt calibrate";
			}

			srt_f_optbmmvolfromstates(VolUD,dMaturity,beta,Fwd1,Fwd2,Sig1,Sig2,Pi,SRT_LOGNORMAL,&volused);
			if (err) return err;

			for (n=0;n<nx; n++)
			{
				if((cp*(VolUD -  x[n]) >= -10e-17)) 
					srt_f_optbmmvolfromstates(x[n],dMaturity,beta,Fwd1,Fwd2,Sig1,Sig2,Pi,SRT_LOGNORMAL,&volused);
				dCMSOpts[n] = srt_f_optblksch(dDefaultSwap,x[n],volused,dMaturity,1.0,call_put, SRT_PREMIUM);
			}
		}
		else if (Volstruct->voltype_used == SRT_BMMVOL)
		{
			...
			BMMCalibOnSabrStates(dDefaultSwap,dMaturity,betavol,beta,alpha,rho,Volstruct->Pi ,Volstruct->NstD ,&Fwd1,&Fwd2,&Sig1,&Sig2,&Pi,SRT_BETAVOL,&res);
			if (res>1e-3) return "BMM could'nt calibrate";

			srt_f_optbmmvolfromstates(VolUD,dMaturity,beta,Fwd1,Fwd2,Sig1,Sig2,Volstruct->Pi,SRT_LOGNORMAL,&volused);
			if (err) return err;

			for (n=0;n<nx; n++)
			{
				if((cp*(VolUD -  x[n]) >= -10e-17)) 
					srt_f_optbmmvolfromstates(x[n],dMaturity,beta,Fwd1,Fwd2,Sig1,Sig2,Volstruct->Pi,SRT_LOGNORMAL,&volused);
				dCMSOpts[n] = srt_f_optblksch(dDefaultSwap,x[n],volused,dMaturity,1.0,call_put, SRT_PREMIUM);
			}
		}
		else
	...
	if (Volstruct->comp_method == 2)
	{
		err = swp_f_vol(Volstruct->szVolCurveName,Volstruct->start,Volstruct->end,VolUD,&volused, &power);
		if (err) return err;
		...
		for (n=0;n<nx; n++)
		{
			...
			if((cp*(VolUD -  x[n]) >= -10e-17)) 
			{
				err = swp_f_vol(Volstruct->szVolCurveName,Volstruct->start,Volstruct->end,x[n],&volused, &power);
				if (err) return err;
...
double Fwd1, Fwd2, Sig1, Sig2, Pi,res;
	...
	err = CMSTECsetupvol(Volstruct, Greekstruct, strikeadj,dDefaultSwap,dMaturity,PayRec);
	if (err) return err;
	...
	CMS_get_pdX(left, params->Nx, right, cp, Mesh, pdX+params->Nx, pdDX+params->Nx);
	...
	err = swp_f_FinalCms_Opts(pdX,2*params->Nx,dDefaultSwap,
			strikeadj,cp,dMaturity,Volstruct,VolUD,
			 /* Output */
			pdOpts);
	if (err) goto FREE_CMS_END;
	...
	factor = CMS_get_controlvariatefactor(strikeadj,dnfp,invr,
		dMargin,invp,invf,cvgZC,spdZC);
		...
		else if (Volstruct->voltype_used == SRT_BMM2VOL)
		{
			if (Volstruct->input_SABRAF == 1)
			{
				BMM2GetStates(dDefaultSwap, dMaturity,Volstruct->vol,Volstruct->alpha,
					Volstruct->beta, Volstruct->rho,Volstruct->zeta, &Sig1, &Sig2, &Fwd1, &Fwd2,&Pi);
			}
			else
			{
				BMM2CalibOnSabrStates(dDefaultSwap,dMaturity,Volstruct->vol,Volstruct->beta,Volstruct->alpha,Volstruct->rho,Volstruct->Pi ,Volstruct->NstD ,&Fwd1,&Fwd2,&Sig1,&Sig2,&Pi,SRT_BETAVOL,&res);
				if (res>1e-3) return "SABRAF could'nt calibrate";
			}

			srt_f_optbmmvolfromstates(strikeadj,dMaturity,Volstruct->beta,Fwd1,Fwd2,Sig1,Sig2,Pi,SRT_LOGNORMAL,&vol);
			if (err) return err;
		}
		else if (Volstruct->voltype_used == SRT_BMMVOL)
		{
			BMMCalibOnSabrStates(dDefaultSwap,dMaturity,Volstruct->vol,Volstruct->beta,Volstruct->alpha,Volstruct->rho,Volstruct->Pi ,Volstruct->NstD ,&Fwd1,&Fwd2,&Sig1,&Sig2,&Pi,SRT_BETAVOL,&res);
			if (res>1e-3) return " BMM could'nt calibrate";

			srt_f_optbmmvolfromstates(strikeadj,dMaturity,Volstruct->beta,Fwd1,Fwd2,Sig1,Sig2,Volstruct->Pi,SRT_LOGNORMAL,&vol);
			if (err) return err;
		}
		else
		{
			err = srt_f_optbetastochvoltoblkvol(dDefaultSwap,strikeadj, Volstruct->vol,Volstruct->alpha
	...
	if (Volstruct->voltype_used == SRT_NORMAL) 
		dTV = srt_f_optblknrm(dDefaultSwap,strikeadj,vol,
					dMaturity,1.0,((cp > 0.0) ? SRT_CALL : SRT_PUT), SRT_PREMIUM);
	else 
		dTV = srt_f_optblksch(dDefaultSwap,strikeadj,vol,
					dMaturity,1.0, ((cp > 0.0) ? SRT_CALL : SRT_PUT), SRT_PREMIUM);
	...
	if (cvgZC > 1.0e-3) dIntegral /= dFwdZC;
...
FREE_CMS_END:
	...
	if (pdDX) free(pdDX);
	if (pdW) free(pdW);
	if (pdOpts) free(pdOpts);
	pdX = NULL; pdDX = NULL; pdW = NULL; pdOpts = NULL;
...
double  put;
	...
	err = swp_f_CMSTEC_Option(dFwdSwapRate,dnfp,dfreq,dRateConv,dMaturity,
						dFwdSwapRate,SRT_PAYER,dPower,dMargin,dCvgZC,dFwdZC,
						Volstruct,Greekstruct,params,&call);
	if (err) return err;

	err = swp_f_CMSTEC_Option(dFwdSwapRate,dnfp,dfreq,dRateConv,dMaturity,
						dFwdSwapRate,SRT_RECEIVER,dPower,dMargin,dCvgZC,dFwdZC,
						Volstruct,Greekstruct,params,&put);
	if (err) return err;

	/* return the rate inside the decompouding formula */
		*dCMSTECrate = pow(call - put + pow( 1.0 + dFwdSwapRate , 1 / dPower) , dPower) - 1.0 - dMargin; 
...
Err err = NULL;
	...
	{
		err = swp_f_CMSTEC_rate(dFwdSwapRate,
					 dnfp,dfreq,dRateConv,dMaturity,dPower,dMargin,
					 dCvgZC,dFwdZC,Volstruct,Greekstruct,params,dCMSTEC);
		return err;
	}
	else
	{
...
Err err = NULL;
	...
	if (PayRec == SRT_FORWARD)
	{
		err = swp_f_CMSTEC_rate(dFwdSwapRate,
					 dnfp,dfreq,dRateConv,dMaturity,dPower,dMargin,
					 dCvgZC,dFwdZC,Volstruct,NULL,params,dCMSTEC);
		return err;
	}
	else
	{
...
Err swp_f_CMSTEC_Weights(/* underlying */
					 ...
					 /* delay parameters */
					 double				dCvgZC,	/* cvg of the delay */
					 double             dspdZC, /* spread to retreive B(0,Tpay) / B(0,Tstart) */
					 /* numerical param */
					 CMSParams			*params,
					 /* Output */
					 double				*pdW,
...
int    id    = 0;
	...
	CMS_get_pdX(left, params->Nx, right, cp, Mesh, pdX+params->Nx+shift, pdDX+params->Nx+shift);
	
	/* compute the weights */
	swp_f_FinalCms_Weights(pdX+shift, 2*params->Nx,dnfp,dfreq,dRateConv,dPower,
		dMargin,dStrike,cp,cvgZC,spdZC,pdW+shift);
	...
	pdW[id] = CMS_get_controlvariatefactor(strikeadj,dnfp,invr,
		dMargin,invp,invf,cvgZC,spdZC);
	pdX[id] = strikeadj;

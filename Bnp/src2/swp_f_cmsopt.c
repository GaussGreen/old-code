/* =======================================================================================
   ...
   DESCRIPTION     :Contains all versions of
                                        value of cms (rate and option) through integration with
   swaptions
    ...
    double*          dCMSOptionValue)
{
    ...
    double dFactor_n = 1.0, dVol = 0.0, dVal = 0.0, dAdjForwardRate, dDeltaStrike, dOpt, r, dInvLvl,
           dAmt, dInvLvlm1, dInvLvlm2, dSens, dSens1;
    ...
    else
    {
        ...
        if (!iMethod)
            dVol = dFlatVol;
        else if (err = swp_f_truncvol(szVolCurveName, dStart, dEnd, dStrike, &dVol, &power))
            return err;

        if (CMS_SPACE_TYPE == SRT_LOGNORMAL)
            dDeltaStrike =
                exp(log((-(dSignRecPay - 1.0) * 1.0e-10 + (dSignRecPay + 1.0) * BOUNDARYINT / 2.0) /
                        dStrike) /
                    CMS_SPACE_NUMSTEP);
        else
        ...
        if (iMethod == 1)
        {
            ...
            if (err = swp_f_truncvol(
                    szVolCurveName,
                    dStart,
                    dEnd,
                    pdStrikesVol[iLower - 1],
                    &pdVolVector[iLower - 1],
                    &power))
                return err;
            ...
            {
                if (err = swp_f_truncvol(
                        szVolCurveName, dStart, dEnd, pdStrikesVol[i], &pdVolVector[i], &power))
                    return err;
                pdCoeffApprox[i - 1] =
                    (pdVolVector[i] - pdVolVector[i - 1]) / (pdStrikesVol[i] - pdStrikesVol[i - 1]);
            }
        ...
        if (PayRec == SRT_RECEIVER)
        {
            ...
            if (CMS_SPACE_TYPE == SRT_LOGNORMAL)
            {
                ...
                {
                    if (VolType == SRT_NORMAL)
                        *dCMSOptionValue = srt_f_optblknrm(
                            dAdjForwardRate, dStrike, dVol, dMaturity, 1.0, CallPut, PREMIUM);
                    else
                        *dCMSOptionValue = srt_f_optblksch(
                            dAdjForwardRate, dStrike, dVol, dMaturity, 1.0, CallPut, PREMIUM);
        ...
        if (VolType == SRT_NORMAL)
            dOpt =
                srt_f_optblknrm(dAdjForwardRate, dStrike, dVol, dMaturity, 1.0, CallPut, PREMIUM);
        else
            dOpt =
                srt_f_optblksch(dAdjForwardRate, dStrike, dVol, dMaturity, 1.0, CallPut, PREMIUM);
        ...
        if (iMethod)
            if (err = swp_f_truncvol(szVolCurveName, dStart, dEnd, r * dFrequency, &dVol, &power))
                return err;
        ...
        if (VolType == SRT_NORMAL)
            dOpt = srt_f_optblknrm(
                dAdjForwardRate, r * dFrequency, dVol, dMaturity, 1.0, CallPut, PREMIUM);
        else
            dOpt = srt_f_optblksch(
                dAdjForwardRate, r * dFrequency, dVol, dMaturity, 1.0, CallPut, PREMIUM);
        ...
        while ((bConverged == SRT_FALSE) && (i <= CMS_SPACE_NUMSTEP))
        {
            ...
            if (iMethod == 2)
                if (err =
                        swp_f_truncvol(szVolCurveName, dStart, dEnd, r * dFrequency, &dVol, &power))
                    return err;

            if (iMethod == 1)
            {
                if (dSignRecPay * (r * dFrequency - pdStrikesVol[min(m, lNumStrikesInVol - 1)]) >=
                    0.0)
                    m += dSignRecPay;
    ...
    double*          dTECOptionValue)
{
    ...
    double *pdCoeffApprox = NULL, *pdVolVector = NULL;
    double  dVol = .0, dVal = .0, dDeltaStrike, dAdjForwardRate, dOpt = .0, r = .0, dInvLvl,
           dAmt          = .0, dLvl, dInvLvlm1, dInvLvlm2, dSens, dSens1;
    double         power = 0.0;
    int            i, j, m = 0, iNumSwaps, dSignRecPay = 1, iLeftRightPos = 0, iLower, iHigher;
    SRT_Boolean    bConverged = SRT_FALSE;
    SrtCallPutType CallPut;
    double*        dTECw = NULL;
    ...
    else
    {
        ...
        if (!iMethod)
            ...
            err = swp_f_truncvol(szVolCurveName, dStart, dEnd, dStrike - dMargin, &dVol, &power))
            return err;
        ...
        dDeltaStrike = (-(dSignRecPay - 1.0) * 1.0e-10 + (dSignRecPay + 1.0) * BOUNDARYINT / 2.0 -
                        dStrike + dMargin) /
                       CMS_SPACE_NUMSTEP;
        ...
        if (VolType == SRT_NORMAL)
            dOpt = srt_f_optblknrm(
                dAdjForwardRate, dStrike - dMargin, dVol, dMaturity, 1.0, CallPut, PREMIUM);
        else
            dOpt = srt_f_optblksch(
                dAdjForwardRate, dStrike - dMargin, dVol, dMaturity, 1.0, CallPut, PREMIUM);
        ...
        if (dPaymentPower != 1)
        {
            for (j = 0; j < 2; j++)
                dAmt +=
                    pow((1 + dStrike + (1 - j) * dDeltaStrike), (1 / dPaymentPower - 2)) * dTECw[j];
        ...
        while ((bConverged == SRT_FALSE) && (i <= iNumSwaps))
        {
            ...
            if (dPaymentPower != 1)
            {
                for (j = 0; j < i; j++)
                    dAmt +=
                        pow((1 + dStrike + ((i - 1) - j) * dDeltaStrike), (1 / dPaymentPower - 2)) *
                        dTECw[j];
...
double CMSNumeraire(double dForwardRate, double dDelay, double dFrequency, double dNumPeriods)
{
    double Y, IL, IB;
...
double CMSdNumeraire(double dForwardRate, double dDelay, double dFrequency, double dNumPeriods)
{
    double Y, IL, IB;
...
Err PatExtendedHermite(double* x, double* w, double xMax, int n)
{
    ...
    w[2] = w[2 * n - 2] = INV_SQRT_TWO_PI * 25 / 24 * exp(-x[2] * x[2] / 2.0) * dx;
...
double TecCmsGenericPayoffDensity(
    double S, double dStrike, SrtReceiverType PayRec, double dMargin, double dTecPower)
{
    switch (PayRec)
    {
    /* Cms Cap Case */
    ...
    double*          dGenericValue)
{
    ...
    Err                err = NULL;
    double             S, Numeraire, dxMax = 6.0, dAdjForwardRate, dAdjNumeraire = 0.0, dStdDev;
...
{ /* See Cms Bnp Approximation for the poor notation of the variable */
    ...
    if (iMethod == 4)
    {
        dTemp = dCmsSpread + dlambda * dNumPeriods * dStdDev * dVol * dVol * dMaturity / dFrequency;
    }
...
Err swp_f_truncvol(
    char* vol_id, Ddate start, Ddate end, double strike, double* volatility, double* power)
{
    Err    err = NULL;
    double adjstrike;
    ...
    SrtReceiverType PayRec)
{
    ...
    if (Volstruct->input_method == 4)
    {
        ...
        Volstruct->input_method == 2 || Volstruct->input_method == 3 ||
        Volstruct->input_method == 250)  // 3 for BMM in test
    {
        // Detects if the market is SABR by retrieving the sigmabetavol.
        /* old method
        err = swp_f_SABRvol(Volstruct->szVolCurveName, Volstruct->start, Volstruct->end, swaprate,
        &yesno,  &power, SABR_BETAVOL);
        */
...
void CMS_get_discount(double x, double cvg, double spd, double* ZC, double* d1ZC, double* d2ZC)
{
    double df;
...
double CMS_get_reduced_payoff2(
    double x,
    double nfp,
    double invf,
    double K, /* real strike */
...
double CMS_get_controlvariatefactor(
    double strikeadj,
    double nfp,
    double invr,
    double m,
...
void CMS_get_pdX(
    double strikeadj, long Nx, double IntUD, double cp, int Mesh, double* pX, double* pdX)
{
    long   i;
    double spacing;
    double prev;
                        ...
                        double* dCMSOpts)
{
    ...
    if (Volstruct->comp_method == 0)
    {
        ...
        {
            for (n = 0; n < nx; n++)
                dCMSOpts[n] = srt_f_optblksch(
                    dDefaultSwap, x[n], volused, dMaturity, 1.0, call_put, SRT_PREMIUM);
        }
        else if (Volstruct->voltype_used == SRT_NORMAL)
        {
            for (n = 0; n < nx; n++)
                dCMSOpts[n] = srt_f_optblknrm(
                    dDefaultSwap, x[n], volused, dMaturity, 1.0, call_put, SRT_PREMIUM);
        }
        else if (Volstruct->voltype_used == SRT_BETAVOL)
        {
            ...
            err = srt_f_optbetastochvoltoblkvol(
                dDefaultSwap, VolUD, betavol, alpha, rho, dMaturity, beta, &volused);
            if (err)
                return err;
        ...
        else if (Volstruct->voltype_used == SRT_BMM2VOL)
        {
            ...
            srt_f_optbmmvolfromstates(
                VolUD, dMaturity, beta, Fwd1, Fwd2, Sig1, Sig2, Pi, SRT_LOGNORMAL, &volused);
            if (err)
                return err;
            ...
            for (n = 0; n < nx; n++)
            {
                if ((cp * (VolUD - x[n]) >= -10e-17))
                    srt_f_optbmmvolfromstates(
                        x[n], dMaturity, beta, Fwd1, Fwd2, Sig1, Sig2, Pi, SRT_LOGNORMAL, &volused);
                dCMSOpts[n] = srt_f_optblksch(
                    dDefaultSwap, x[n], volused, dMaturity, 1.0, call_put, SRT_PREMIUM);
            }
        }
    ...
    {
        err = swp_f_vol(
            Volstruct->szVolCurveName, Volstruct->start, Volstruct->end, VolUD, &volused, &power);
        if (err)
            return err;
                        ...
                        double* dCMSOptionValue)
{
    ...
    factor = CMS_get_controlvariatefactor(strikeadj, dnfp, invr, dMargin, invp, invf, cvgZC, spdZC);

    /* Get the vol */
    if (Volstruct->comp_method == 0)
    {
        if ((Volstruct->voltype_used == SRT_NORMAL) || (Volstruct->voltype_used == SRT_LOGNORMAL))
            vol = Volstruct->vol;
        else if (Volstruct->voltype_used == SRT_BMM2VOL)
        {
            if (Volstruct->input_SABRAF == 1)
    ...
    {
        err = swp_f_vol(
            Volstruct->szVolCurveName, Volstruct->start, Volstruct->end, strikeadj, &vol, &power);
        if (err)
            return err;
                         ...
                         double* pdX)
{
    ...
    CMS_get_pdX(
        left, params->Nx, right, cp, Mesh, pdX + params->Nx + shift, pdDX + params->Nx + shift);
    ...
    pdW[id] =
        CMS_get_controlvariatefactor(strikeadj, dnfp, invr, dMargin, invp, invf, cvgZC, spdZC);
    pdX[id] = strikeadj;

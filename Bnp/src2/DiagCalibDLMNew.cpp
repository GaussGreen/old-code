...
Err Initialise_Factors_LGM(
    CALIBCPNSCHEDULEDLM CalibCpnSchedule,
    CALIBEXESCHEDULEDLM sCalibExeSchedule,
    LGM_MODEL           model,
    LGM_MODELFACTORS    sModelFactors)
{
    sModelFactors->dCpn_G1   = (double*)calloc(CalibCpnSchedule->iNCpn, sizeof(double));
    sModelFactors->dEx_G1    = (double*)calloc(sCalibExeSchedule->iNExe, sizeof(double));
    sModelFactors->dExpFact1 = (double*)(sCalibExeSchedule->iNExe, sizeof(double));
    sModelFactors->dZeta1    = (double*)calloc(CalibCpnSchedule->iNCpn, sizeof(double));

    if (!sModelFactors->dCpn_G1 || !sModelFactors->dExpFact1 || !sModelFactors->dEx_G1 ||
        !sModelFactors->dZeta1)
    {
    ...
    {
        sModelFactors->dCpn_G2    = (double*)calloc(CalibCpnSchedule->iNCpn, sizeof(double));
        sModelFactors->dEx_G2     = (double*)calloc(sCalibExeSchedule->iNExe, sizeof(double));
        sModelFactors->dExpFact2  = (double*)calloc(sCalibExeSchedule->iNExe, sizeof(double));
        sModelFactors->dExpFact12 = (double*)calloc(sCalibExeSchedule->iNExe, sizeof(double));
        sModelFactors->dZeta2     = (double*)calloc(sCalibExeSchedule->iNExe, sizeof(double));
        sModelFactors->dZeta12    = (double*)calloc(sCalibExeSchedule->iNExe, sizeof(double));

        if (!sModelFactors->dCpn_G2 || !sModelFactors->dExpFact2 || !sModelFactors->dExpFact12 ||
            !sModelFactors->dEx_G2 || !sModelFactors->dZeta2 || !sModelFactors->dZeta12)
        {
            return "Memory allocation faillure in Initialise_Factors_LGM";
        }
    }
    ...
    LGM_ALLPARAMS AllParams)
{
    ...
    AllParams->CalibParams       = (CALIBGEN_Params*)calloc(1, sizeof(CALIBGEN_Params));
    AllParams->CalibParamsLambda = (CALIBGEN_Params*)calloc(1, sizeof(CALIBGEN_Params));
    AllParams->sLongFactors      = (LGM_ModelFactors*)calloc(1, sizeof(LGM_ModelFactors));
    AllParams->sShortFactors     = (LGM_ModelFactors*)calloc(1, sizeof(LGM_ModelFactors));

    if (!AllParams->CalibParams || !AllParams->CalibParamsLambda || !AllParams->sLongFactors ||
    ...
    err = Initialise_Factors_LGM(
        CalibLongCpnSchedule, AllCalibLongInst->sCalibExeSchedule, model, AllParams->sLongFactors);
...
void Calculate_Lambda_Factors(
    CALIBCPNSCHEDULEDLM sSchedule,
    CALIBEXESCHEDULEDLM sCalibExeSchedule,
    LGM_MODEL           model,
    LGM_MODELFACTORS    sModelFactors)
{
    ...
    static_lgmsetupG_tauts(
        ...
        sSchedule->iNCpn,
        sSchedule->dCpnTime,
        sModelFactors->dCpn_G1,
        sCalibExeSchedule->iNExe,
        sCalibExeSchedule->dExeTimes,
        sModelFactors->dEx_G1);
    ...
    if (model->iNbFactor == 2)
    {
        static_lgmsetupG_tauts(
            ...
            sSchedule->iNCpn,
            sSchedule->dCpnTime,
            sModelFactors->dCpn_G2,
            sCalibExeSchedule->iNExe,
            sCalibExeSchedule->dExeTimes,
            sModelFactors->dEx_G2);
    }
    ...
    for (i = 0; i < sCalibExeSchedule->iNExe; i++)
    {
        ...
        if (model->iNbFactor == 2)
        {
            sModelFactors->dExpFact2[i] =
                export_lgmcalcexpfact_tauts(
                    t1, t2, model->iNbLam, model->dLambdaTime, model->dLambda, model->dGamma) /
                sModelFactors->dExpFact1[i];

            sModelFactors->dExpFact12[i] = export_lgmcalcexpfact_tauts(
                                               t1,
                                               t2,
                                               model->iNbLam,
                                               model->dLambdaTime,
    ...
    CPD_CALIB_INST_DATA inst_data) /*	NULL = don't save calibration instrument data */
{
    ...
    double strikel[MAX_INST], ex_timel[MAX_INST], strikes[MAX_INST], weightl[MAX_INST],
    ...
    *sig      = NULL;
    ...
    /* Initialisations */
    CalibCpnLongSchedule  = (CalibCpnScheduleDLM*)calloc(1, sizeof(CalibCpnScheduleDLM));
    CalibCpnShortSchedule = (CalibCpnScheduleDLM*)calloc(1, sizeof(CalibCpnScheduleDLM));
    CalibExeLongSchedule  = (CalibExeScheduleDLM*)calloc(1, sizeof(CalibExeScheduleDLM));
    CalibExeShortSchedule = (CalibExeScheduleDLM*)calloc(1, sizeof(CalibExeScheduleDLM));
    AllCalibLongInst      = (AllCalibInstrumentsDLM*)calloc(1, sizeof(AllCalibInstrumentsDLM));
    AllCalibShortInst     = (AllCalibInstrumentsDLM*)calloc(1, sizeof(AllCalibInstrumentsDLM));
    AllCalibShortInstOne  = (AllCalibInstrumentsDLM*)calloc(1, sizeof(AllCalibInstrumentsDLM));
    AllParams             = (LGM_AllParams*)calloc(1, sizeof(LGM_AllParams));
    model                 = (LGM_model*)calloc(1, sizeof(LGM_model));

    if (!CalibCpnLongSchedule || !CalibCpnShortSchedule || !CalibExeLongSchedule ||
        !CalibExeShortSchedule || !AllCalibLongInst || !AllCalibShortInst || !AllParams ||
        !AllCalibShortInstOne || !model)
    {
    ...
    if (!fix_lambda)
    {
        ...
        err = AllocateAllCalibInst(CalibExeShortSchedule->iNExe, 1, AllCalibShortInst);
    ...
    model->dPWTime     = (double*)calloc(model->iNbPWTime, sizeof(double));
    model->iNbLam      = nlam;
    model->dLambdaTime = (double*)calloc(model->iNbLam, sizeof(double));
    model->dInitLambda = (double*)calloc(model->iNbLam, sizeof(double));
    model->dNewLambda  = (double*)calloc(model->iNbLam, sizeof(double));
    model->dNewLambda2 = (double*)calloc(model->iNbLam, sizeof(double));

    if (!model->dPWTime || !model->dLambdaTime || !model->dInitLambda || !model->dNewLambda ||
        !model->dNewLambda2)
    {
    ...
    model->dSigma  = (double*)calloc(model->iNbPWTime, sizeof(double));
    model->dLambda = (double*)calloc(model->iNbPWTime, sizeof(double));

    lSigLongIndex = lngvector(0, CalibExeLongSchedule->iNExe - 1);

    if (!fix_lambda)
    {
        lSigShortIndex = lngvector(0, CalibExeShortSchedule->iNExe - 1);
    }

    if (!model->dSigma || !model->dLambda || !lSigLongIndex || (!lSigShortIndex && !fix_lambda))
    {
    ...
    {
        lSigLongIndex[i] =
            Get_Index(CalibExeLongSchedule->dExeTimes[i], model->dPWTime, model->iNbPWTime);
    }
    ...
    if (!fix_lambda)
    {
        ...
        {
            lSigShortIndex[i] =
                Get_Index(CalibExeShortSchedule->dExeTimes[i], model->dPWTime, model->iNbPWTime);
        }
    }
    ...
    AllParams->CalibFunctionsForVol.UpdateConstsAfterParam = UpdateParamsAfterVol_LGM;
    ...
    AllParams->CalibFunctionsForLambda.GetLimitAndLastParam   = GetLimitAndLastLambda_LGM;
    ...
    AllParams->CalibFunctionsForLambda.UpdateConstsAfterParam = UpdateParamsAfterLambda_LGM;
    ...
    if (inst_data)
    {
        ...
        for (i = 0; i < num_ex_datesl; i++)
        {
            inst_data->exer_dates_long[i] =
                (long)(CalibExeLongSchedule->dExeTimes[i] * DAYS_IN_YEAR + 0.5) + today;
        ...
        if (!fix_lambda)
        {
            for (i = 0; i < num_ex_datess; i++)
            {
                inst_data->exer_dates_short[i] =
                    (long)(CalibExeShortSchedule->dExeTimes[i] * DAYS_IN_YEAR + 0.5) + today;
...
FREE_RETURN:
    ...
    if (lSigLongIndex)
        free_lngvector(lSigLongIndex, 0, CalibExeLongSchedule->iNExe - 1);
    if (lSigShortIndex)
        free_lngvector(lSigShortIndex, 0, CalibExeShortSchedule->iNExe - 1);
...
Err GetTargetVol_LGM(
    void* Inst_, void* GlobalParam, void* Model, CALIBGEN_PARAMS CalibConsts, double* target)
{
...
Err GetFirstGuessVol_LGM(void* Model, void* GlobalParam, int vol_index, double target, double* vol1)
{
    LGM_MODEL     model     = (LGM_MODEL)(Model);
    LGM_ALLPARAMS AllParams = (LGM_ALLPARAMS)(GlobalParam);
...
Err SetVol_LGM(
    void* Model, CALIBGEN_PARAMS CalibConsts, void* GlobalParam, int vol_index, double var)
{
    Err err = NULL;
    ...
    double* limit_up)
{
    ...
    sig_limit_down_lgm = sqrt(AllParams->cum_var_lgm * AllParams->long_param->min_fact / t1);
    sig_limit_up_lgm   = sqrt(AllParams->cum_var_lgm / AllParams->long_param->max_fact / t1);
...
Err UpdateParamsAfterVol_LGM(
    void* Inst_, void* InstParam, void* GlobalParam, void* Model, CALIBGEN_PARAMS CalibConsts)
{
    Err err = NULL;
    ...
    AllParams->sLongFactors->dZeta1[AllParams->index_vol] =
        last_zeta1 + var * AllParams->sLongFactors->dExpFact1[AllParams->index_vol];

    if (model->iNbFactor == 2)
    {
        AllParams->sLongFactors->dZeta2[AllParams->index_vol] =
            last_zeta2 + var * AllParams->sLongFactors->dExpFact2[AllParams->index_vol];
        AllParams->sLongFactors->dZeta12[AllParams->index_vol] =
            last_zeta12 + var * AllParams->sLongFactors->dExpFact12[AllParams->index_vol];
    }

    return err;
}

Err PriceInstVol_LGM(
    void* Inst_, void* InstParam, void* GlobalParam, void* Model, double* InstPrice)
{
    Err err = NULL;
...
Err GetFirstGuessLambda_LGM(
    void* Model, void* GlobalConst, int index_lambda, double target, double* lambda1)
{
    LGM_MODEL     model     = (LGM_MODEL)(Model);
    LGM_ALLPARAMS AllParams = (LGM_ALLPARAMS)(GlobalConst);
    ...
    double*         limit_up)
{
    ...
    *limit_up   = -999;
    ...
    for (i = 0; i < model->iNbLam; i++)
    {
        *limit_down = DMIN(AllParams->short_param->lambda_min - model->dNewLambda[i], *limit_down);
        *limit_up   = DMAX(AllParams->short_param->lambda_max - model->dNewLambda[i], *limit_up);
    }

    return NULL;
}

Err BumpLambda_LGM(void* Model, void* GlobalParam, int index_lambda, double lambda)
{
    Err err = NULL;
...
Err SetLambda_LGM(
    void* Model, CALIBGEN_PARAMS CalibConsts, void* GlobalParam, int index_lambda, double lambda)
{
    Err err = NULL;
...
Err UpdateParamsAfterLambda_LGM(
    void* Inst_, void* InstParam, void* GlobalParam, void* Model, CALIBGEN_PARAMS CalibConsts)
{
    Err    err = NULL;
    ...
    Calculate_Lambda_Factors(
        AllParams->CalibCpnLongSchedule,
        AllParams->CalibExeLongSchedule,
        model,
    ...
    if (CalibConsts->do_calib)
    {
        ...
        Calculate_Lambda_Factors(
            ...
            AllParams->CalibExeShortSchedule,
            model,
            AllParams->sShortFactors);
    }
    ...
    if (CalibConsts->do_calib)
    {
        ...
        if (model->iNbFactor == 2)
        {
            static_lgmcalczeta2zeta12_tauts(
                ...
                model->dGamma,
                model->dRho,
                AllParams->sShortFactors->dZeta2,
                AllParams->sShortFactors->dZeta12);
        }
    }

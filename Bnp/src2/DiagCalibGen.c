...
Free_CalibParams(CALIBGEN_PARAMS		CalibParams)
{
	if (CalibParams)
	{
		if (CalibParams->res_iter) free_dmatrix(CalibParams->res_iter, 0, CalibParams->NbIterMax - 1, 0, 1);
		CalibParams->res_iter = NULL;
...
static int		i, j, k, type;
	...
	if (nb_iter == 2 || method == 0)
	{
		...
		for (j=0; j<nb_iter; j++)
		{
			if (fabs(res_iter[j][0] - coef) < 1.0E-10 && fabs(res_iter[j][1] - premium_tgt) > precision)
			{
				/* We have a problem */
				if (j > i + 1)
				{
	...
	else if (nb_iter > 2)
	{
		...
		{
			coef = solve_for_next_coef_gen(	res_iter,
											nb_iter,
											premium_tgt,
											precision,
											0);
	...
	if (method > 0)
	{
		for (i=0; i<nb_iter; i++)
		{
			if (fabs(res_iter[i][0] - coef) < 1.0E-10 && fabs(res_iter[i][1] - premium_tgt) > precision)
			{
...
/* Set the Sensitivity */
	...
	(res_iter[nb_computed-1][0] - res_iter[0][0]) > delta)
	{
		...
		if (i >= 0)
		{
			sens_down = (res_iter[i][1] - res_iter[index][1]) / (res_iter[i][0] - res_iter[index][0]);
			has_down = 1.0;
		}
		else
		{
...
double	**res_iter;
	...
	if (CalibConsts->do_calib)
	{
		...
		*success = 1;
		...
		/* First Pricing */
		err = AllFunctions->GetFirstGuess(	Model,
											GlobalConst,
											index_param,
											target,
											&param1);
		...
		err = AllFunctions->BumpParam(	Model,
										GlobalConst,
										index_param,
										param1);
		...
		err = AllFunctions->UpdateConstsAfterParam(	Inst,
													InstConst,
													GlobalConst,
													Model,
													CalibConsts);
		...
		err = AllFunctions->PriceInst(	Inst,
										InstConst,
										GlobalConst,
										Model,
										&price1);
		...
		{						
			...
			{
				err = AllFunctions->GetSecondGuess(	Model,
													GlobalConst,
													index_param,
													param1,
													price1,
		...
		while (fabs(target - price1) > CalibConsts->Precision && iter < CalibConsts->NbIterMax && *success)
		{
			iter++;
			...
			else
			{
				...
				param2 = solve_for_next_coef_gen(	res_iter,
													nb_computed,
													target,
													CalibConsts->Precision,
													2);
			}
		...
		{
			smessage("Calibration: failed to calibrate instrument nb %d , Max iteration reached", index_param);
			*success = 0;		
		}

		if ((*success == 0 && !CalibConsts->UseJumps) || (CalibConsts->RecalibAtEnd && nb_computed > 1))
		{
			err = AllFunctions->BumpParam(	Model,
											GlobalConst,
											index_param,
...
double	limit_down, limit_up, last_param, new_param;
	...
	{
		err = AllFunctions->GetLimitAndLastParam(	Model,
													CalibConsts,
													GlobalConst,
													i,
													&last_param,
		...
		err = CalibrateNextParam(	AllInst[i],
									InstConst[i],
									GlobalConst,
									Model,					
									CalibConsts,
		...
		err = AllFunctions->SetParam(	Model,
										CalibConsts,
										GlobalConst,
										i,
										new_param);
		...
		{
			solve_for_next_sensi(	CalibConsts->res_iter,
	...
	AllFunctions->ExtrapolParam(Model,
								GlobalConst,
								EndIndexInst);

...
/***
        interpolate x        , y = f(x) at xt        , linearly        , or
linear on xt*f(xt) if method==1        , geometric if method == 2        ,
geometric on rt if method==3 on r*r*t if method=5(by Julia); x is assumed to be
in increasing order
***/
...
double interp(double *x, double *y, int l, double xt, int method, double *q) {
  ...
  /**	geometric interpol i.e. linear interpol on the log of y **/
    ...
    yt = log(y[i - 1]) +
         (xt - x[i - 1]) * (log(y[i]) - log(y[i - 1])) / (x[i] - x[i - 1]);
    yt = exp(yt);
  }
  /**	linear interpol on (log(y)/x) **/
  else if (method == 3) {
    yt = log(y[i - 1]) / x[i - 1] +
         (xt - x[i - 1]) * (log(y[i]) / x[i] - log(y[i - 1]) / x[i - 1]) /
             (x[i] - x[i - 1]);
    yt = exp(yt * xt);
  }
  /** 	quadratic interpol  on rate*rate times time **/
    ...
    /*
if(i>2)
yt=interp_quadratic(xt        ,x[i-2]        ,y[i-2]        ,x[i-1] ,y[i-1]
,x[i]        ,y[i]); else yt = y[i-1] + (xt - x[i-1]) * (y[i] -
y[i-1])/(x[i]-x[i-1]);
*/
...
/////////////////////////////////////////////////////////////////////////////////////////////////
////////////  same function as before in which one can interpolate the columns
/// of matrices
/////////////////////////////////////////////////////////////////////////////////////////////////
...
double interp_columns(double **x, double **y, int l, double xt, int method,
                      double *q, const int n) {
  int i;
  double yt;
  ...
  */
  ...
  if (method == 0) {
    yt = y[n][i - 1] +
         (xt - x[n][i - 1]) * (y[n][i] - y[n][i - 1]) / (x[n][i] - x[n][i - 1]);
  }
  /** 	interpol on rate times time **/
  else if (method == 1) {
    yt = y[n][i - 1] * x[n][i - 1] +
         (xt - x[n][i - 1]) * (y[n][i] * x[n][i] - y[n][i - 1] * x[n][i - 1]) /
             (x[n][i] - x[n][i - 1]);
    yt /= xt;
  }
  ...
  /**	geometric interpol i.e. linear interpol on the log of y **/
    ...
    yt = log(y[n][i - 1]) + (xt - x[n][i - 1]) *
                                (log(y[n][i]) - log(y[n][i - 1])) /
                                (x[n][i] - x[n][i - 1]);
    yt = exp(yt);
  }
  /**	linear interpol on (log(y)/x) **/
    ...
    yt = log(y[n][i - 1]) / x[n][i - 1] +
         (xt - x[n][i - 1]) *
             (log(y[n][i]) / x[n][i] - log(y[n][i - 1]) / x[n][i - 1]) /
             (x[n][i] - x[n][i - 1]);
    yt = exp(yt * xt);
  }
  /** 	quadratic interpol  on rate*rate times time **/
...
double lin_interp_2d(double x, double y, double *xa, double *ya, double **za,
                     long num_xa, long num_ya)

{
  ...
  a = (xa[start1] != xa[end1])
          ? (za[end1][start2] - za[start1][start2]) / (xa[end1] - xa[start1])
          : 0.0;
  int1 = za[start1][start2] + a * (x - xa[start1]);

  a = (xa[start1] != xa[end1])
          ? (za[end1][end2] - za[start1][end2]) / (xa[end1] - xa[start1])
          : 0.0;
...
double InterpLagrangeColumns1D(double **Xdata, double **Ydata, unsigned int n,
                               double x, int m)

...
char *SrtGrfnLGMSVpde(char *underlying, int numeventdates, long *eventdates,
                      ...
                      int nstept, int nstepx, int nstepeps, int nstepphi,
  ...
  double *dff = NULL, *gam = NULL, *gam2 = NULL;
  ...
  /* Get the model */
  err = Get_LGMSV_model(underlying, model);
  ...
  /* change of TStar */
  new_tstar =
      (double)((long)(0.5 * (evt_dts[num_evt - 1] - today)) * YEARS_IN_DAY);
  ...
  if (eventdates[numeventdates - 1] >= today) {
    err = lgmSV_adi_UtPieceWise(
        nstp, time, date, nstepphi, nstepx, nstepeps, model->dLambdaX, 1.0,
        model->dPWTime, model->dSigma, model->iNbPWTime, model->dLambdaEps,
        model->dLvlEps, model->dAlpha, model->dRho, &Params, void_prm, is_event,
        ifr, yc, payoff_lgmsv_pde_UtPieceWise, num_col, num_col, *prod_val);
  }
  if (err) {
    goto FREE_RETURN;
  }
...
char *SrtGrfnLGMSVMC(char *underlying, int numeventdates, long *eventdates,
                     int *nUsedEventDates, long tableauRows, long *tableauCols,
                     char ***tableauStrings, int **tableauMask, long auxWidth,
                     long *auxLen, double **aux,
                     ...
                     int do_optimisation, int *optimise, double *fwd_iv,
  ...
  if (mdl_dim == ONE_FAC) {
    err = SrtGrfnLGMSVMC_1F(
        underlying, numeventdates, eventdates, nUsedEventDates, tableauRows,
        tableauCols, tableauStrings, tableauMask, auxWidth, auxLen, aux,
        is_end_of_day_fixing, is_end_of_day_payment, Params, do_optimisation,
        optimise, fwd_iv, params, resRows, nstept, numpaths, nb_prod, prod_val);
  } else if (mdl_dim == TWO_FAC) {
    err = SrtGrfnLGMSVMC_2F(
        underlying, numeventdates, eventdates, nUsedEventDates, tableauRows,
        tableauCols, tableauStrings, tableauMask, auxWidth, auxLen, aux,
        is_end_of_day_fixing, is_end_of_day_payment, Params, do_optimisation,
        optimise, fwd_iv, params, resRows, nstept, numpaths, nb_prod, prod_val);
...
char *SrtGrfnLGMSVMC_1F(char *underlying, int numeventdates, long *eventdates,
                        int *nUsedEventDates, long tableauRows,
  ...
  double *evt_tms = NULL, *time = NULL, *date = NULL, *sigma = NULL,
         *alpha = NULL, *rho = NULL, *lameps = NULL, *lvleps = NULL,

         *dff_star = NULL, *gam_star = NULL, *gam2_star = NULL;
  ...
  if (!void_prm || !is_event || !date || !sigma || !alpha || !rho || !lameps ||
      !lvleps || !dff_star || !gam_star || !gam2_star)
  ...
  for (i = nstp - 1; i >= 0; i--) {
    ...
    if (j >= 0 && fabs(time[i] - evt_tms[j]) < 1.0E-08) {
      ...
      gam_star[j] =
          (1.0 - exp(model->dLambdaX * (model->dTStar - evt_tms[j]))) /
          model->dLambdaX;
      ...
      if (grfn_prm->num_df > 0) {
        ...
        for (k = 0; k < grfn_prm->num_df; k++) {
          ...
          temp = (1.0 - exp(model->dLambdaX * (model->dTStar - evt_tms[j] -
                                               grfn_prm->df_tms[k]))) /
                 model->dLambdaX;
  ...
  *nUsedEventDates = num_evt;
  ...
  if (do_optimisation) {
    ...
    mceb_allocate_params(params, num_evt);
  ...
  case 0: {
    err = lgmSV_mc_balsam(nstp, num_evt, time, date, numpaths, model->dLambdaX,
                          sigma, alpha, lameps, lvleps, rho, dff_star, gam_star,
                          gam2_star, Params, void_prm, is_event,
                          do_optimisation, optimise, params, NULL,
                          payoff_lgmsv_mc, num_col, *prod_val);
    break;
  }

  case 1: {
    err = lgmSV_mc_balsam_rev(
        nstp, num_evt, time, date, numpaths, model->dLambdaX, sigma, alpha,
        lameps, lvleps, rho, dff_star, gam_star, gam2_star, &Params, void_prm,
        is_event, do_optimisation, optimise, params, NULL, payoff_lgmsv_mc,
        NULL, num_col, *prod_val);
  }
...
char *SrtGrfnLGMSVMC_2F(char *underlying, int numeventdates, long *eventdates,
                        int *nUsedEventDates, long tableauRows,
                        long *tableauCols, char ***tableauStrings,
                        int **tableauMask, long auxWidth, long *auxLen,
                        double **aux,
  ...
  double *evt_tms = NULL, *time = NULL, *date = NULL,
         ...
         *sigma = NULL, *alpha = NULL, *rho = NULL, *rho2 = NULL,
         *lameps = NULL, *lvleps = NULL, *newalphaLGM = NULL, *newrhoLGM = NULL,

         *dff_star = NULL, *gam1_star = NULL, *gam2_star = NULL,
         *gam1_2_star = NULL, *gam2_2_star = NULL, *gam12_star = NULL;
  ...
  if (!void_prm || !is_event || !date || !sigma || !alpha || !rho || !rho2 ||
      !lameps || !lvleps || !newalphaLGM || !newrhoLGM || !dff_star ||
      !gam1_star || !gam2_star || !gam1_2_star || !gam2_2_star || !gam12_star) {
  ...
  for (i = nstp - 1; i >= 0; i--) {
    ...
    if (j >= 0 && fabs(time[i] - evt_tms[j]) < 1.0E-08) {
      ...
      gam1_star[j] =
          ...
          model->dLambdaX;
      gam2_star[j] =
          (1.0 - exp(model->dLambdaX2 * (model->dTStar - evt_tms[j]))) /
          model->dLambdaX2;
      ...
      if (grfn_prm->num_df > 0) {
        ...
        if (!grfn_prm->dff || !grfn_prm->gam1 || !grfn_prm->gam1_2 ||
            !grfn_prm->gam2 || !grfn_prm->gam2_2 || !grfn_prm->gam12) {
        ...
        for (k = 0; k < grfn_prm->num_df; k++) {
          grfn_prm->dff[k] = swp_f_df(xStr.dts[j], grfn_prm->df_dts[k], yc);
          grfn_prm->dff[k] = log(grfn_prm->dff[k]);
          temp1 = (1.0 - exp(model->dLambdaX * (model->dTStar - evt_tms[j] -
                                                grfn_prm->df_tms[k]))) /
                  model->dLambdaX;
          temp2 = (1.0 - exp(model->dLambdaX2 * (model->dTStar - evt_tms[j] -
                                                 grfn_prm->df_tms[k]))) /
                  model->dLambdaX2;
  ...
  *nUsedEventDates = num_evt;
  ...
  case 0: {
    err = lgmSV2F_mc_balsam(
        nstp, num_evt, time, date, numpaths, model->dLambdaX, model->dLambdaX2,
        sigma, newalphaLGM, newrhoLGM, alpha, lameps, lvleps, rho, rho2,
        dff_star, gam1_star, gam2_star, gam1_2_star, gam2_2_star, gam12_star,
        Params, void_prm, is_event, do_optimisation,
        &optimise[numeventdates - xStr.num_evt], params, NULL,
        payoff_lgmsv2F_mc, num_col, *prod_val);
  ...
  case 1: {
    err = lgmSV2F_mc_balsam_rev(
        ...
        &Params, void_prm, is_event, do_optimisation,
        &optimise[numeventdates - xStr.num_evt], params, NULL,
        payoff_lgmsv2F_mc, NULL, num_col, *prod_val);
...
char *SrtGrfnLGMSVFFT(
  ...
  double *evt_tms = NULL, *time = NULL, *SigTime = NULL, *Sig = NULL,
         *alphaTS = NULL, *rhoTS = NULL, *lamepsTS = NULL, *ifr = NULL,
         *date = NULL;
  ...
  err = Get_LGMSV_TermStructure(domestic_name, &SigTime, &Sig, &alphaTS, &rhoTS,
                                &lamepsTS, &dTStar, &iNbSigTime, &tau, &one2F,
                                NULL, NULL, NULL);
  ...
  dAlpha = alphaTS[0];
  dRho = rhoTS[0], dLambdaEps = lamepsTS[0];

  for (i = 1; i < iNbSigTime; i++) {
    if ((fabs(alphaTS[i] - dAlpha) > 1.0E-08) ||
        (fabs(rhoTS[i] - dRho) > 1.0E-08) ||
        (fabs(lamepsTS[i] - dLambdaEps) > 1.0E-08)) {
  ...
  if (!dt || !Coef2ImT || !Coef3ReT || !Coef3ImT ||
      ((!dff || !gam || !gam2) && (max_num_df > 0))) {
    err = "Memory allocation failure";
    goto FREE_RETURN;
  }
  ...
  *prod_val = dvector(0, num_col - 1);
  ...
  err = LGMSVOptionGRFNPrice(iNbPhi, iNbft, dLambdaX, dTStar, endi, dt, Coef1Re,
                             Coef2Re, Coef2ImT, Coef3ReT, Coef3ImT, dPhitMean,
                             0, dExTime, dExDate, dPhiFreqStep, dPhiStep,
                             iIndexPhiMean, dftFreqStep, dftStep, iIndexft0, yc,
                             void_prm, payoff_lgmsv_FFT, num_col, *prod_val);
...
FREE_RETURN:
  ...
  if (free_str) {
    ...
    FIRSTFreeEvtDatesFromDeal(numeventdates, &evt_dts, &evt_tms);

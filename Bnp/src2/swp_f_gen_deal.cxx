...
double value_swap(SrtCurvePtr crv, Swap_Obj *swap) {
  ...
  if ((mess == CMS_MARGIN_FLOATING_SWAP) || (mess == ONE_LEG_CMS_MARGIN) ||
      ...
      (mess == CMS_FOR_TEC_MARGIN_FLOATING_SWAP) ||
      (mess == ONE_LEG_CMS_FOR_TEC_MARGIN) ||
  ...
  for (i = 0; i < num_leg; i++) {
    ...
    if ((mess == CMS_MARGIN_LEG) || (mess == CMT_MARGIN_LEG) ||
        (mess == TEC_MARGIN_LEG) || (mess = CMS_FOR_TEC_MARGIN_LEG)) {
...
static void leg_eval_cm_rate(Leg_Obj *leg, SrtCurvePtr cmt_crv) {
  ...
  for (i = index_start; i < index_end; i++) {
    ...
    fixing_date = add_unit((long)period_start, -cmt_param->spot_lag, SRT_BDAY,
                           SUCCEEDING);
    ...
    if (err = swp_f_cmsrate(fwd, nfp, cm_compounding, vol, maturity, delay,
                            DEFAULT_CMS_DELTA, MAX_CMS_SWAPS, SRT_LOGNORMAL,
                            &cm_rate))
      cm_rate = -1;
...
Leg_Obj *generate_leg(Arg_Obj *arg, LegType mess, int rec_pay) {
  ...
  if (Arg_Iget(arg, ARG_DATE_DIR) == FWD) {
    ...
    toalloc = (year(Arg_Dateget(arg, ARG_END)) -
               year(Arg_Dateget(arg, ARG_START)) + 1) *
...
void leg_eval_payment(Leg_Obj *leg, String curvename) {
  ...
  case FLOATING_LEG:
    for (i = index_start + 1; i <= index_end; i++) {
      ...
      coupon = swp_f_fwdcash((double)start, (double)end, (BasisCode)basis_code,
                             curvename);
      cvg = Leg_field_get(leg, LEG_COVERAGE, i);
  ...
  case CMS_FOR_TEC_MARGIN_LEG:
    ...
    for (i = index_start + 1; i <= index_end; i++) {
      coupon = Leg_field_get(leg, LEG_CM_RATE, i);
      payment = (exp(1.0 / (double)compd * log(1 + coupon + spread)) - 1.0) *
                notional;
      ;
      Leg_field_set(leg, LEG_PAYMENT, i, payment);
    }
...
static void generate_leg_dates(Arg_Obj *arg, Leg_Obj *leg) {
  ...
  swap.cxxompd = Arg_Iget(arg, ARG_COMPD);
  ...
  for (i = k; i < list.len; i++) {
    ...
    Leg_field_Dateset(leg, LEG_TIME_IN_YEARS, i + 1 - k,
                      (list.date[i] - today) / 365.0);
  }

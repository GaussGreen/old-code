    ...
    double*** prod_val)
{
    ...
    double *evt_tms = NULL, *time = NULL, *date = NULL, *sigma = NULL, *alpha = NULL, *rho = NULL,
           *lameps = NULL, *lvleps = NULL,
    ...
    if (!void_prm || !is_event || !date || !sigma || !alpha || !rho || !lameps || !lvleps ||
    ...
    for (i = nstp - 1; i >= 0; i--)
    {
        ...
        if (j >= 0 && fabs(time[i] - evt_tms[j]) < 1.0E-08)
        {
            ...
            gam_star[j] =
                (1.0 - exp(model->dLambdaX * (model->dTStar - evt_tms[j]))) / model->dLambdaX;
            ...
            if (grfn_prm->num_df > 0)
            {
                ...
                for (k = 0; k < grfn_prm->num_df; k++)
                {
                    ...
                    temp             = (1.0 - exp(model->dLambdaX *
                                      (model->dTStar - evt_tms[j] - grfn_prm->df_tms[k]))) /
                           model->dLambdaX;
    ...
    double*** prod_val)
{
    ...
    double *evt_tms = NULL, *time = NULL, *date = NULL,
           ...
           *sigma = NULL, *alpha = NULL, *rho = NULL, *rho2 = NULL, *lameps = NULL, *lvleps = NULL,
           *newalphaLGM = NULL, *newrhoLGM = NULL,

           *dff_star = NULL, *gam1_star = NULL, *gam2_star = NULL, *gam1_2_star = NULL,
           *gam2_2_star = NULL, *gam12_star = NULL;
    ...
    if (!void_prm || !is_event || !date || !sigma || !alpha || !rho || !rho2 || !lameps ||
        !lvleps || !newalphaLGM || !newrhoLGM || !dff_star || !gam1_star || !gam2_star ||
        !gam1_2_star || !gam2_2_star || !gam12_star)
    {
    ...
    for (i = nstp - 1; i >= 0; i--)
    {
        ...
        if (j >= 0 && fabs(time[i] - evt_tms[j]) < 1.0E-08)
        {
            ...
            gam1_star[j] =
                (1.0 - exp(model->dLambdaX * (model->dTStar - evt_tms[j]))) / model->dLambdaX;
            gam2_star[j] =
                (1.0 - exp(model->dLambdaX2 * (model->dTStar - evt_tms[j]))) / model->dLambdaX2;
            ...
            if (grfn_prm->num_df > 0)
            {
                ...
                if (!grfn_prm->dff || !grfn_prm->gam1 || !grfn_prm->gam1_2 || !grfn_prm->gam2 ||
                    !grfn_prm->gam2_2 || !grfn_prm->gam12)
                {
                ...
                for (k = 0; k < grfn_prm->num_df; k++)
                {
                    grfn_prm->dff[k] = swp_f_df(xStr.dts[j], grfn_prm->df_dts[k], yc);
                    grfn_prm->dff[k] = log(grfn_prm->dff[k]);
                    temp1            = (1.0 - exp(model->dLambdaX *
                                       (model->dTStar - evt_tms[j] - grfn_prm->df_tms[k]))) /
                            model->dLambdaX;
                    temp2 = (1.0 - exp(model->dLambdaX2 *
                                       (model->dTStar - evt_tms[j] - grfn_prm->df_tms[k]))) /
                            model->dLambdaX2;
    ...
    double** prod_val)
{
    ...
    double *evt_tms = NULL, *time = NULL, *SigTime = NULL, *Sig = NULL, *alphaTS = NULL,
           *rhoTS = NULL, *lamepsTS = NULL, *ifr = NULL, *date = NULL;
    ...
    {
        if ((fabs(alphaTS[i] - dAlpha) > 1.0E-08) || (fabs(rhoTS[i] - dRho) > 1.0E-08) ||

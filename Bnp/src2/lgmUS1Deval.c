...
startagain:
    ...
    */
    ...
    if (x == NULL || payofftable == NULL || weights == NULL || x_exerarr == NULL ||
        payofftable_payers == NULL || payofftable_receivers == NULL || receiver_european == NULL ||
        payer_european == NULL)
    {
        srt_free(varr);
        srt_free(vmax);
    ...
    for (index_european = 0; index_european < nEx; index_european++)
    {
        ...
        NewConvolverTimeSwap(
            ...
            m,
            killkinks,
            &(reduction[index_european]),
            weights,
            &stdev_european,
            payofftable_receivers[index_european],
            &price);
    ...
    for (index_european = 0; index_european < nEx; index_european++)
    {
        ...
        NewConvolverTimeSwap(
            ...
            weights,
            &stdev_european,
            payofftable_payers[index_european],
            &price);
    ...
    NewConvolverTimeSwap(
        nEx, nx, n0, nz, dx, h, m, killkinks, reduction, weights, varr, payofftable, answer);
    ...
    double*            result)                /* output:  volatility */
{
    ...
    if (deal->timeswapvolmethod == 2) /* Converging:  tkaes vol from today's market assuming that
                                         the market in the future is shifted. */
    {
        startfudge = start - (value_date - tNow);
        endfudge   = end - (value_date - tNow);
    }
    else if (deal->timeswapvolmethod == 3) /* Sliding: takes vol from today's market independent of
                                              where we are evaluating it. */
    {
        startfudge = start;
        endfudge   = end;
    }
    ...
    /*	if ( deal->dMaxStd > 0.0 )
            {
                    error =
       (*(deal->GetSABRParam))(startfudge,endfudge,deal->vol_id,SABR_ATM_LOG,&sigma); if (error)
                            return error;
    ...
    */
    ...
    if (deal->shiftvol)
        volstrike +=
            deal->shiftvol *
            (1 - exp(-(value_date - tNow + 0.0) / (deal->tCpnPay[0] - deal->tCpnStart[0] + 0.0)));
    ...
    LGMErr (*GetBeta)(Date, Date, double*))                     /* swaption/cap exponents (beta) */
{
    ...
    double           proba_between, proba_between_adj, proba_below, proba_below_adj, proba_above,
        proba_above_adj;
    ...
    */
    ...
    tvols_discret       = f3tensor(
        0,
        num_subdiscret_obsfreq - 1,
        0,
         ...
         current_index_obsfreq++)
    {
        index_discret_obsfreq[current_index_obsfreq] =
            (int)(current_index_obsfreq * (observation_freq - 1.0) / (num_subdiscret_obsfreq - 1.0));
    }
    ...
    for (j = 0; j < nEx; j++) /* Loop on exo date */
    {
        ...
        for (i = ifirst; i < nCpn; i++) /*Loop on coupons */
        {
                    ...
                    if (tfras_subperiod[l][k] > 0)
                    {
                        tcms_subperiod[l][k] =
                            ...
                            exp(vol_for_adj_cms * vol_for_adj_cms * fra_for_adj_cms *
                                ...
                                (ptr->observationdays[i + i0][l][1] - ptr->tCpnPay[i + i0] + 0.0) /
                                365.0);
                    }
                }
            }
            ...
            for (current_index_obsfreq = 0; current_index_obsfreq < num_subdiscret_obsfreq;
                 current_index_obsfreq++)
            {
                for (current_index_statvar = 0; current_index_statvar < num_subdiscret_statvar;
                     current_index_statvar++)
                {
                    ...
                    error = GetVol_SABRValues(
                        tNow, ptr->tEx[j + j0], Start, End, ptr, &alpha, &beta, &sigma, &rho);
                    if (error)
                        return error;
                    ...
                    { /* gets vollow and vollowshift */
                        ...
                        error = GetVol_FudgeTimeSwap_new(
                            ...
                            tfras_subperiod[l][k],
                            ptr,
                            &(tvols_discret[current_index_obsfreq][current_index_statvar][0]),
                            alpha,
                            beta,
                            sigma,
                            rho);
                        ...
                        error = GetVol_FudgeTimeSwap_new(
                            ...
                            tfras_subperiod[l][k],
                            ptr,
                            &(tvols_discret[current_index_obsfreq][current_index_statvar][2]),
                            alpha,
                            beta,
                            sigma,
                            rho);
            ...
            for (l = 0; l <= observation_freq - 1; l++) /* Loop on the observation days */
            {
                ...
                for (k = tindex_firstpositive[l]; k <= nx; k++) /* Loop on the state variable */
                {
                    if (tfras_subperiod[l][k] >
                        tfras_subperiod[l][index_discret_statvar[l][current_index_statvar + 1]])
                    {
                        current_index_statvar += 1;
                    }

                    yy = (tfras_subperiod[l][k] -
                          tfras_subperiod[l][index_discret_statvar[l][current_index_statvar]]) /
                         (tfras_subperiod[l][index_discret_statvar[l][current_index_statvar + 1]] -
                          tfras_subperiod[l][index_discret_statvar[l][current_index_statvar]]);
                    ...
                    for (m = 0; m <= 3; m++)
                    {
                        tvols_subperiod[l][k][m] =
                            w1 * tvols_discret[current_index_obsfreq][current_index_statvar][m] +
                            w2 *
                                tvols_discret[current_index_obsfreq + 1][current_index_statvar][m] +
                            w3 *
                                tvols_discret[current_index_obsfreq][current_index_statvar + 1][m] +
                            w4 * tvols_discret[current_index_obsfreq + 1][current_index_statvar + 1]
                                              [m];
            ...
            if (!(0 == ptr->gear[i + i0])) /* Floating digital computation */
            {
                ...
                end_accrued =
                    add_unit(start_accrued, ptr->undAccrued, SRT_MONTH, MODIFIED_SUCCEEDING);
            ...
            for (k = 0; k <= nx; k++) /* Loop on the state variable */
            {
                ...
                for (l = 0; l < observation_freq; l++) /* Loop on the number of days */
                {
                    ...
                    else
                    {
                        ...
                        else /* Floating digital */
                        {
                            ...
                            callup_adj = srt_f_optblksch(
                                tcms_subperiod[l][k] * adjustmentup,
                                strikeup,
                                tvols_subperiod[l][k][1],
                                tmaturities_subperiod[l],
                                1,
                                SRT_CALL,
                                PREMIUM);
                            ...
                            else
                            {
                                ...
                                putlow_adj = srt_f_optblksch(
                                    tcms_subperiod[l][k] * adjustmentlow,
                                    strikelow,
                                    tvols_subperiod[l][k][0],
                                    tmaturities_subperiod[l],
                                    1,
                                    SRT_PUT,
                                    PREMIUM);
                    ...
                    //					expected_accrued_ratio_high += (1.0 - proba_above) *
                    //(ptr->ratiodays_for_subperiod)[i+i0][l];
    ...
    for (j = 0; j < nEx; j++)
    {
        ...
        for (i = ifirst; i < nCpn; i++)
        {
            ...
            for (k = 0; k <= nx; k++)
            {
                ...
                /* Fix Coupon */
                payoff[j][k] +=
                    dfPay * ptr->tCvgCpn[i + i0] * ptr->tCpn[i + i0] * expected_accrued_ratio;
...
cleanup:
    ...
    free_f3tensor(
        tvols_discret, 0, num_subdiscret_obsfreq - 1, 0, num_subdiscret_statvar - 1, 0, 3);

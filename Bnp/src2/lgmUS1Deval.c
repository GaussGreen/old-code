...
startagain:
  ...
  /* x[k] = (k-n0)*dx*reduction[j]  , k=0  ,1  ,...  , nx  , j=0  ,1  ,2  ,...
   * ,nEx-1 */
  dx = 2.0 * totalstddev * gridwidth / ((double)nx);

  /* z[i] = (i-m)*h  , i = 0  , 1  , ...  , 2m=nz */
  /* integration intervals from z[i] to z[i+1]  , for i = 0  , 1  , ...  , 2m-1
   * = nz-1 */
  h = 0.9233333 * dx /
...
/* Interpolator for convolver
Function: Interpolates value at k+dk from an array of values
TheArray[k]  , k=0  , ...  , nk. For time swap  , extrapolates flat; this solves
a problem for very small maturities  */
static double InterpfromArrayTS(long k, double dk, long nk, double *TheArray) {
  double interpval, x;
    ...
    double *result)          /* output:  volatility */
{
  ...
  /*	if ( deal->dMaxStd > 0.0 )
     ...
     ,deal->vol_id  ,SABR_ATM_LOG  ,&sigma); if (error) return error; dStdDev =
     fra * exp( sigma*sqrt(maturity) ); if ( strike > fra * dStdDev ) strike =
     fra* dStdDev; else if ( strike < fra / dStdDev ) strike = fra / dStdDev;
          }
  */
    ...
    LGMErr (*GetBeta)(Date, Date, double *)) /* swaption/cap exponents (beta) */
{
  ...
  /* for debug
  clock_t counter1  ,counter2  ,counter3  ,counter4  ,counter_observationdf1
  ,counter_observationdf2;
  */
  ...
  zEx = (double *)srt_calloc(
      nEx, sizeof(double)); /* zeta[j0+j]  , j=0  ,...  ,nEx-1 */
  ...
  for (j = 0; j < nEx; j++) /* Loop on exo date */
  {
    ...
    for (i = ifirst; i < nCpn; i++) /*Loop on coupons */
    {
      ...
      /* computes probabilities to be between barriers  , and corresponding
       * ratios */

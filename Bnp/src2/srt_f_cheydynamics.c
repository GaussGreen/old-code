...

{
    ...
    mu = -tminf->ev.onef.lambda * samptr_get(cur_sam, 0, STATEVAR) + samptr_get(cur_sam, 0, PHI);
    mu *= stp->delta_t;

    dphi = samptr_get(cur_sam, 0, SHORT_RATE) * tminf->ev.onef.sig;
    dphi = dphi * dphi - 2.0 * tminf->ev.onef.lambda * samptr_get(cur_sam, 0, PHI);
    dphi *= stp->delta_t;
...
/* -------------------------------------------------------------------------------
   ...
   DESCRIPTION     :calculates variance at stp, cur_sam, that is sig^2*r^2*dt
(stp should correspond to cur_sam)
...
void srt_f_chevaratsam(SrtStpPtr stp, SrtSample* cur_sam, double* var_at_sam, int index)
{
    ...
    /* see comment in srt_f_chedrfatsam concerning the index in the following macro
       call */

...
/* returns the first index whose correspinding maturity is greater or equal to
 * the maturity T */
...
static long Get_Index_Und(SrtUndPtr *und_ptr, int num_und, char *und_name) {
  static int i;
...
static long Get_Index_FxUnd(SrtUndPtr *und_ptr, int num_und, char *domname,
                            char *forname) {
  static int i;
  static char *temp_domname, *temp_forname;
...
static Err CompleteUndPtrList(SrtUndPtr **und_ptr, int *num_und,
                              char *domestic_name, char *domestic_ccy,
                              int **type, int **dom_forex, int **for_forex,
                              int **fx_index, int *dom_index)

{
  ...
  for (i = 0; i < num_und_temp; i++) {
    switch
      get_mdltype_from_fxund(((*und_ptr)[i])) {
      case FX_STOCH_RATES:
        ...
        */
        ...
        if (index < 0) {
          ...
          num_und_temp += 1;
          *und_ptr =
              (SrtUndPtr *)realloc(*und_ptr, num_und_temp * sizeof(SrtUndPtr));
          ...
          *fx_index = (int *)realloc(*fx_index, num_und_temp * sizeof(int));
          ...
          err = srt_f_getundinlist(all_und, domname,
                                   &((*und_ptr)[num_und_temp - 1]));
        ...
        if (index < 0) {
          ...
          *fx_index = (int *)realloc(*fx_index, num_und_temp * sizeof(int));
          ...
          err = srt_f_getundinlist(all_und, forname,
                                   &((*und_ptr)[num_und_temp - 1]));
      ...
      case LGM:
        ...
        /*	it is a foreign LGM */
        /*	we have to check that the corresponding fxd rate is in the
         * structure */

        (*type)[i] = 1;
        index = Get_Index_FxUnd(*und_ptr, num_und_temp, domestic_name,
                                ((*und_ptr)[i])->underl_name);
        if (index < 0) {
          /* we have to add this underlying */
          num_und_temp += 1;
          *und_ptr =
          ...
          *fx_index = (int *)realloc(*fx_index, num_und_temp * sizeof(int));
          ...
          (*und_ptr)[num_und_temp - 1] =
              srt_f_lookup_fxund(domestic_ccy, ((*und_ptr)[i])->underl_ccy);
...
char *SrtMultiGrfn3DFXMc(char *underlying, int numeventdates, long *eventdates,
  ...
  double *evt_tms = NULL, *sig_dates = NULL, **und_sig_dates = NULL,
         **und_sig_curve = NULL, **sig_curves = NULL, pay_time;

  int *type = NULL, *dom_forex = NULL, *for_forex = NULL, *fx_index = NULL,
      dom_index;

  double *corr_date = NULL, *tau_date = NULL, *tau = NULL, tau_temp, df;

  long nb_tau, *nb_und_sig = NULL, nb_corr_date, nb_sig_dates, pay_date;

  double *corr = NULL;

  long *evt_dts = NULL, today, spot_date;

  int num_und, num_col, max_num_df, num_evt;

  char *domestic_name, *domestic_ccy;

  SrtUndPtr *und_ptr = NULL, dom_und, und;
  ...
  err =
      CompleteUndPtrList(&und_ptr, &num_und, domestic_name, domestic_ccy, &type,
                         &dom_forex, &for_forex, &fx_index, &dom_index);
  ...
  err = allocate_link_und(und_link);
  if (err) {
    goto FREE_RETURN;
  }
  ...
  for (i = 0; i < num_und; i++) {
    ...
    if (und_link->type[i] < 2) {
      ...
      if (!dom_und) {
        err = serror("Couldn't find underlying named %s",
                     und_ptr[i]->underl_name);
        goto FREE_RETURN;
      }
      und_link->yc[i] = (char *)get_ycname_from_irund(dom_und);
      err = Get_LGM_TermStructure(und_ptr[i]->underl_name, &(und_sig_dates[i]),
                                  &(und_sig_curve[i]), &(nb_und_sig[i]),
                                  &tau_date, &tau, &nb_tau);
      if (err) {
        goto FREE_RETURN;
      }
      ...
      if (!sig_dates_alloc) {
        ...
        num_f_concat_vector(&nb_sig_dates, &sig_dates, nb_und_sig[i],
                            und_sig_dates[i]);
      }
  ...
  for (i = 0; i < num_und; i++) {
    if (und_link->type[i] == 2) {
      err = srt_f_display_FX_TermStruct(
          und_ptr[i]->underl_name, &(nb_und_sig[i]), &(und_sig_dates[i]),
          &(und_sig_curve[i]), &nb_corr_date, &corr_date, &corr);
      if (err) {
        goto FREE_RETURN;
      }
      ...
      for (l = 0; l < nb_corr_date; l++)
        corr_date[l] = (corr_date[l] - today) / 365.0;
  ...
  und_link->correlations =
      f3tensor(0, num_und - 1, 0, num_und - 1, 0, nb_sig_dates - 1);
    ...
    for (i = 0; i < num_und; i++) {
      ...
      for (j = i + 1; j < num_und; j++) {
        err = srt_f_get_corr_from_CorrList(
            corr_list, und_ptr[i]->underl_name, und_ptr[j]->underl_name,
            sig_dates[l], &(und_link->correlations[i][j][l]));
        if (err) {
          goto FREE_RETURN;
        }
  ...
  for (i = 0; i < num_und; i++) {
    for (j = 0; j < nb_sig_dates; j++) {
      sig_curves[i][j] = und_sig_curve[i][Get_Index(
          sig_dates[j], und_sig_dates[i], nb_und_sig[i])];
    }
    if (und_sig_dates[i]) {
  ...
  /* now calculate all the fwd        , std        , covariances of the
   * underlyings */
  err = fill_mc_multi_init_corr(pay_date, pay_time, evt_dts, evt_tms, nstp,
                                und_link);
        ...
        if (grfn_prm->num_df[j] > 0) {
          ...
          if (und_link->type[j] < 2) {
            for (k = 0; k < grfn_prm->num_df[j]; k++) {
              grfn_prm->dff[j][k] = swp_f_df(
                  evt_dts[i + flag], grfn_prm->df_dts[j][k], und_link->yc[j]);
              grfn_prm->gam[j][k] =
                  (1.0 - exp(-und_link->lambda[j] * grfn_prm->df_tms[j][k])) /
                  und_link->lambda[j];
              grfn_prm->gam2[j][k] = 0.5 * grfn_prm->gam[j][k] *
                                     grfn_prm->gam[j][k] *
                                     und_link->phi[j][i + flag];
            }
          } else {
            ...
            for (k = 0; k < grfn_prm->num_df[j]; k++) {
              grfn_prm->dff[j][k] = swp_f_df(
                  evt_dts[i + flag], grfn_prm->df_dts[j][k], und_link->yc[j]);
              grfn_prm->gam[j][k] =
                  (1.0 - exp(-und_link->lambda[und_link->dom_forex[j]] *
                             grfn_prm->df_tms[j][k])) /
                  und_link->lambda[und_link->dom_forex[j]];
              grfn_prm->gam2[j][k] =
                  0.5 * grfn_prm->gam[und_link->dom_forex[j]][k] *
                  grfn_prm->gam[und_link->dom_forex[j]][k] *
                  und_link->phi[und_link->dom_forex[j]][i + flag];
            }
          }
  ...
  *prod_val = dmatrix(0, num_col - 1, 0, 1);
  ...
  err = mc_main_multi_3dfx(
      /*	Time data */
      num_paths, num_col, evt_tms, evt_dts, nstp, do_pecs, 0, NULL, NULL,
      und_link, void_prm, grfn_payoff_multi_3dfx_mc, *prod_val);
  ...
  if (void_prm) {
    for (i = 0; i < num_evt; i++) {
      if (void_prm[i + flag]) {
        ...
        for (l = 0; l < num_und; l++) {
          if (grfn_prm->do_und[l] && grfn_prm->num_df[l] > 0)

          {
            if (grfn_prm->dff[l])
              free_dvector(grfn_prm->dff[l], 0, grfn_prm->num_df[l] - 1);
            if (grfn_prm->gam[l])
              free_dvector(grfn_prm->gam[l], 0, grfn_prm->num_df[l] - 1);
            if (grfn_prm->gam2[l])
              free_dvector(grfn_prm->gam2[l], 0, grfn_prm->num_df[l] - 1);
          }
        }
        if (grfn_prm->dff)
          free(grfn_prm->dff);
...
char *SrtMultiGrfn3DFXMcEB(char *underlying, int numeventdates,
  ...
  *prod_val = dmatrix(0, *resRows - 1, 0, 2 + params->iNbIndex);
  ...
  err = mc_main_multi_3dfx(
      /*	Time data */
      num_paths, num_col, evt_tms, evt_dts, nstp, do_pecs, 1, optimise, params,
      und_link, void_prm, grfn_payoff_multi_3dfx_mc, *prod_val);

// HestonD.cxx : New Heston implementation
...
typedef struct _SHestonComm_ODE {
  ...
  double u_re, u_im;
  int want[4]; // Flags "want derivatives" : { sigma        , alpha        , rho
               // , lam }
} SHestonComm_ODE;
...
static void NAG_CALL EvalDerivatives(Integer neq, double t, double y[],
  ...
  for (i = 0; i < 4; i++)
    if (p->want[i]) {
      ypg[i][2] = -c1 * (yg[i][2] * y[2] - yg[i][3] * y[3]) +
                  p->lam * yg[i][2] +
                  c2 * (p->u_re * yg[i][3] + p->u_im * yg[i][2]);

      ypg[i][3] = -c1 * (yg[i][2] * y[3] + yg[i][3] * y[2]) +
                  p->lam * yg[i][3] -
  ...
  if (p->want[1]) {
    ...
    ypg[1][3] += -2.0 * p->alpha * y[2] * y[3] -
                 p->rho * p->sigma * (p->u_re * y[2] - p->u_im * y[3]);
  }
  if (p->want[2]) {
...
static Err HestonCalcPhi(SHestonComm_InvFT *q, double u_re, double u_im,
  ...
  nag_ode_ivp_rk_setup(neq, q->mat, y, 0.0, RKtol, RKthres, Nag_RK_4_5,
...
static Err HestonCalcPhiClosedForm(SHestonComm_InvFT *q, double u_re,
  ...
  LGMSVUpdateADClosedForm(
      q->mat, q->lam, -0.5 * q->alpha * q->alpha, q->lam + c1 * u_im,
      -c1 * u_re, c2 * (u_re * u_re - u_im * u_im - u_im),
...
static double NAG_CALL Z_func(double v, Nag_User *comm) {
  ...
  double phi_re[5], phi_im[5], zeta_re[5],
      zeta_im[5]; //        , phi_re_test[1]        , phi_im_test[1];
  double dd1, dd2;
  int i, j;
...
static Err HestonDoIntegration(SHestonComm_InvFT *q, int idx_k, int idx_fn,
                               double *res) {
  Err err = NULL;
...
static Err HestonCalcMoments(SHestonComm_InvFT *q, double *mean, double *std) {
  ...
  nag_ode_ivp_rk_setup(4, q->mat, y, 0.0, RKtol, RKthres, Nag_RK_4_5,
                       Nag_RK_range, Nag_ErrorAssess_off, 0.0, &opt, &fail);
  if (fail.cxxode != NE_NOERROR) {
    err = serror(fail.message);
    goto FREE_RETURN;
  ...
  nag_ode_ivp_rk_range(4, EvalDerivativesU, 0.0, &tgot, y, yp, ymax, &opt,
                       &comm_Nag, &fail);
  if (fail.cxxode != NE_NOERROR) {
    err = serror(fail.message);
    goto FREE_RETURN;
...
static Err EvalATMdiff(double x, double *diff, void *comm) {
  ...
  if (Calib->alpha / Calib->lam < 1e-3)
    z = srt_f_optblksch(Calib->fwd + Calib->shift, Calib->fwd + Calib->shift,
                        Calib->sigma, Calib->mat, 1.0, SRT_CALL, PREMIUM);
  else {
  ...
  err = srt_f_optimpvol(z, Calib->fwd, Calib->fwd, Calib->mat, 1.0, SRT_CALL,
                        SRT_LOGNORMAL, &vol);
  if (err)
    goto FREE_RETURN;
...
static Err Get_RR_BF(SHestonComm_Calib *Calib, double *RR, double *BF) {
  ...
  for (i = 0; i < 2; i++) {
    ...
    if (Calib->alpha / Calib->lam < 1e-3)
      z = srt_f_optblksch(Calib->fwd + Calib->shift, Calib->K[i] + Calib->shift,
                          Calib->sigma, Calib->mat, 1.0,
...
static Err EvalBFdiff(double x, double *diff, void *comm) {
  ...
  err = NewtonD(Calib->sigma, 1e-8, 2.0 / sqrt(Calib->mat), EvalATMdiff, 1e-4,
                7, comm);
  if (err)
    return err;
...
static Err EvalRRdiff(double x, double *diff, void *comm) {
  ...
  err = NewtonD(Calib->alpha * Calib->alpha, a_min * a_min, a_max * a_max,
...
Err HestonDCalibrate(double f0, double volf0, double K1, double vol1, double K2,
  ...
  if (!calib_smile)
    ...
    err = NewtonD(Calib.rho * (Calib.alpha + 1e-5), ra_min, ra_max, EvalRRdiff,
                  1e-4, 7, &Calib);
...
Err HestonDDensityFT(double sigma, double alpha, double rho, double lam,
  ...
  LGMSVUpdateADClosedForm(mat, lam, -0.5 * alpha * alpha, lam + c1 * u_im,
                          -c1 * u_re, c2 * (u_re * u_re - u_im * u_im - u_im),
                          c2 * (u_re + 2.0 * u_re * u_im), &C_re, &C_im, &A_re,
                          &A_im);

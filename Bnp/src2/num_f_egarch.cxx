...
static double LL_computation(double *x) {
  ...
  if (garcharray[1].s1 == 0)
    garcharray[1].s1 = NULL_S1;
  if (global_s2_array[1] == 0)
    global_s2_array[1] = NULL_S1;
  global_zeta_array[1] = log(garcharray[1].rate / garcharray[0].rate) /
                         garcharray[1].s1 / global_s2_array[1];
  for (i = 2; i <= global_count; i++) {
    global_s2_array[i] =
        global_s2_array[i - 1] *
        exp(global_beta_0 + global_beta_1 * global_zeta_array[i - 1] +
            global_beta_2 *
                (fabs(global_zeta_array[i - 1]) - sqrt(2.0) / SRT_PI));
    if (garcharray[i].s1 == 0)
      garcharray[i].s1 = NULL_S1;
    if (global_s2_array[i] == 0)
      global_s2_array[i] = NULL_S1;
  ...
  /* we want a maximisation and simulated annealing do a minimisation so we
   * consider -LL */
...
Err srt_f_egarchmethod(
  ...
  Err err = NULL;
  int i, temp;
  int num_params = 0; /* number of parameters not fixed s2_init        , beta_0
                         , beta_1        , beta_2 */
  double **p = NULL;
  double *y = NULL;
  ...
  qsort((void *)garcharray, (size_t)num_data, sizeof(GarchData),
        compare_garchdata);
  ...
  if ((!p) || (!y)) {
    if (p)
      free_dmatrix(p, 1, num_params + 1, 1, num_params);
  ...
  for (i = 1; i <= num_params + 1; i++) {
    temp = 1;
    if (fixed[1] == NOT_FIXED) {
      p[i][temp] = *s2_init;
      temp++;
    }
    if (fixed[2] == NOT_FIXED) {
      p[i][temp] = *beta_0;
      temp++;
    }
    if (fixed[3] == NOT_FIXED) {
      p[i][temp] = *beta_1;
      temp++;
    }
    if (fixed[4] == NOT_FIXED) {
      p[i][temp] = *beta_2;
    }
  }
  ...
  /* Algorithme */
  err = simulated_annealing(p, y, num_params, FTOL, num_iter, LL_computation,
                            MAX_TEMP, MIN_TEMP, DEC_FAC);

  if (err) {
    if (y)
      free_dvector(y, 1, num_params + 1);
    if (p)
      free_dmatrix(p, 1, num_params + 1, 1, num_params);
    srt_free(global_zeta_array);
    srt_free(global_s2_array);

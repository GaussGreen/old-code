    ...
    double* ImpliedRhoTimesStdVt)
{
    ...
    double dVarft, dNewVarft, dVt2Mean, dNewVt2Mean, dPsitMean, dNewPsitMean, dVtPsitMean,
        dNewVtPsitMean, dPsit2Mean, dNewPsit2Mean, dVtftMean, dNewVtftMean;
    ...
    if (!lambda || !FuncVarft || !FuncVt2Mean || !FuncVtPsitMean || !FuncPsit2Mean || !FuncVtftMean)
    {
    ...
    double* ImpliedRhoTimesStdVt)
{
    ...
    {
        if ((dTime[iNumTime] > SigTime[iNumSigTime] + 1.0E-09) && (iNumSigTime < iNbSigTime - 1))
        {
            iNumSigTime++;
        }
        ...
        *pIndex0 = *pIndex0 + iNbLeft;
        ...
        else if (iNbRight > 0)
        {
            ...
            */
            ...
            dCumulStep = ((dMax - Grid[iNb1 + iNbLeft - 1]) - iNbRight * dStep) * 2.0 /
                         (iNbRight * (iNbRight + 1.0));
            ...
            else
            {
                for (iNumStep = iNb1 + iNbLeft; iNumStep < iNb; iNumStep++)
                    Grid[iNumStep] =
                        Grid[iNumStep - 1] + dStep + (iNumStep - (iNb1 + iNbLeft) + 1) * dCumulStep;
            }
        }
    }
    /* Force to 0 */
        ...
        case LGMSV_TRUE:
        {
            ...
            else
            {
                ...
                default:
                {
                    ...
                    dPsiStep =
                        dPsiMean *
                        (exp(iNbSigmaPsiGrid * dLogPsiStd) - exp(-iNbSigmaPsiGrid * dLogPsiStd)) /
                        (iNbPsi - 1);
    ...
    for (iNumZ = iIndexMinZ; iNumZ <= iIndexMaxZ; iNumZ++)
    {
        ...
        for (iNumX = iIndexMinX; iNumX <= iIndexMaxX; iNumX++)
        {
            ...
            for (iNumPsi = 0; iNumPsi < iNbPsi_t; iNumPsi++)
            {
                ...
                /* Find the Index such that GridPsi_t1[index]<dPsi_t1 and GridPsi_t1[index+1]>=
                 * dPsi_t1 */
                ...
                if (IsExtrapolationFlat == LGMSV_TRUE)
                {
                    ...
                    else if (iIndexPsi == -1)
                    {
                        ...
                        for (iNumProduct = 0; iNumProduct < iProductNb; iNumProduct++)
                        {
                            ValueTensorNewZX[iNumPsi][iNumProduct] = ValueTensorZX[0][iNumProduct];
                        }
                    }
            ...
            if (IsExtrapolationFlat == LGMSV_TRUE)
            {
                ...
                if (iIndexXplus == iIndexMinX - 1)
                {
                    ...
                    for (iNumPsi = 0; iNumPsi < iNbPsi; iNumPsi++)
                        for (iNumProduct = iIndexMinProduct; iNumProduct <= iIndexMaxProduct;
                             iNumProduct++)
                ...
                else if (iIndexXplus == iIndexMaxX)
                {
                    ...
                    (iIndexXplus > 1 + iIndexMinX) && (iIndexXplus < iIndexMaxX - 2) &&
                             ...
                             iNumProduct++)
                        {
                            ValueGridZtXtPsitProduct_t[iNumZ][iNumX][iNumPsi][iNumProduct] =
                                (dX_t1 - GridX[iIndexXplus]) /
                                    ...
                                    (ValueGridZtXtPsitProduct_t1[iNumZ][iIndexXplus + 1][iNumPsi]
                                                                [iNumProduct] -
...
{
    /* Declaration of locals variables */
    double *Sigt = NULL, *dLambdaVt = NULL, *dLvlVt = NULL, *dAlphaVt = NULL, *dRhot = NULL;
    ...
    ValueGridXtZtProduct = f3tensor(0, iNbZ - 1, 0, iNbX - 1, 0, iMaxNbProduct - 1);

    ValueTensor_t = f4tensor(0, iNbZ - 1, 0, iNbX - 1, 0, iNbPsi - 1, 0, iMaxNbProduct - 1);

    ValueTensor_t1 = f4tensor(0, iNbZ - 1, 0, iNbX - 1, 0, iNbPsi - 1, 0, iMaxNbProduct - 1);
    ...
    if (!pde || !Sigt || !dLambdaVt || !dLvlVt || !dAlphaVt || !dRhot || !GridX || !GridZ ||
        !GridPsi_t || !GridPsi_t1 || !PsiMean || !PsiStd || !XtMean || !XtStd || !ZtMean ||
        !ZtStd || !DXDrift || !DZDrift || !DXVar || !DZVar || !r || !ValueGridXtZtProduct ||
        !ValueTensor_t || !ValueTensor_t1 || !InterpQuadraCoef || !InterpQuadraCoefGridX ||
                        ...
                        for (iNumProduct = 0; iNumProduct < 1; iNumProduct++)
                            ValueTensor_t1[iNumZ][iNumX][iNumPsi][iNumProduct] *=
                                GridX[iNumX] + dRhot[iNbTime - 1] * Sigt[iNbTime - 1] /
                                                   dAlphaVt[iNbTime - 1] * (GridZ[iNumZ] - 1.0);
        }
    ...
    if (Params->ModeExport != LGMSV_TRUE)
    {
        ...
        for (iNumTime = iNbTime - 2; iNumTime >= 0; iNumTime--)
        {
            ...
            iNbPsiGrid_t =
                ...
                (Params->GridPhiOptim == LGMSV_TRUE) *
                    (int)((1 - iNumTime / ((double)(iNbTime - 1))) * (iNbPsi - 1) + 1.0e-08);
        ...
        memcpy(
            dProductArrayPv,
            ValueTensor_t1[iIndexZ0][iIndexX0][iIndexPsi],

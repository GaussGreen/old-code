  ...
  while (iNumTime < iNbTime) {
    ...
    for (iPeriodIndex = 0; iPeriodIndex < iNbPeriod; iPeriodIndex++) {
      ...
      while (iNumTime < iNbTime &&
        ...
        /* Mean of f(t  ,Tstar)-f(0  ,Tstar)- u(t)*dRho/dAlpha*(Vt-1) at t
         * knowing t-1 */
        XtMean[iNumTime] = 0;

        /* Std of Yt = f(t  ,Tstar)-f(0  ,Tstar) - u(t)*dRho/dAlpha*(Vt-1) at t
         * knowing t-1 */
...
static Err LGMSVPDEExpectations_UtPieceWise_OldVersion(
  ...
  for (iNumTime = 1; iNumTime < iNbTime; iNumTime++) {
    ...
    /* Mean of f(t  ,Tstar)-f(0  ,Tstar)- u(t)*dRho/dAlpha*(Vt-1) at t knowing
     * t-1 */
    XtMean[iNumTime] = 0;

...
static Err srt_f_logtrelim(
    SrtStpPtr stp, SrtTreStpInf* maxtrinf, double* old_u /* previous value of state var spacing u*/
)
{
    Err           err;
    SrtStpPtr     cur;
    ...
    if (*old_u == -1)
    { /* assume here that there is only one underlying : und_ind = 0 */
    ...
    SrtUndInfo*   und_info)
{
    ...
    prev_assets = dmatrix(maxtrinf.min_x_index, maxtrinf.max_x_index, 0, grfnparam->node_dim - 1);
    ...
    for (stp = bot; stp != NULL; stp = stp->prev)
    {
        ...
        for (i = trinf->min_x_index; i <= trinf->max_x_index; i++)
        {
            ...
            if (stp->next)
            {
                ...
                /*CHECK*/ srt_f_trintredcntvec(
                    stp, cur_assets[i], &node, prev_assets, grfnparam->node_dim);
            }
    ...
    for (io_request = iolist->head; io_request != NULL; io_request = io_request->next)
    {
        ...
        strncpy(io_request_val_p->computation_origin, "Tree_Log", strlen("Tree_Log"));
    ...
    err =
        srt_f_IOstructsetcolpvs((iolist), SRT_NO, (double*)prev_assets[0], grfnparam->node_dim, "");

    free_dmatrix(
        prev_assets, maxtrinf.min_x_index, maxtrinf.max_x_index, 0, grfnparam->node_dim - 1);
    free_dmatrix(

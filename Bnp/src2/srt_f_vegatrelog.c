...
/* Memory Allocation -------------------------------------*/
	...
	sz = (maxtrinf.max_x_index-maxtrinf.min_x_index+1)*
			grfnparam->node_dim;
	prev_assets = dmatrix(maxtrinf.min_x_index,maxtrinf.max_x_index,
...
/** create vector of state variables **/
    		...
    		for(i=trinf->min_x_index+1;i<=trinf->max_x_index;i++)
      		...
      		if(stp->next)
        	{
			node.df = ((SrtLogTmInf *)(stp->next->tminf[0]))->df
						/tminf->df;
		}
/** For each value of the state variable == at each node of the tree step **/
    		for(i= trinf->min_x_index; i <= trinf->max_x_index; i++) 
    		{
        		...
        		tminf = (SrtLogTmInf *)(stp->tminf[0]);
      			...
      			sam_get(node.cur_sam,0,SPOT) = exp(cur_st_var[i])
						* tminf->init_fwd_val;
...
/** calculate x index to center on in previous level **/
        			...
        			srt_f_trintrexindex(stp,prev_st_var,
...
/* Stores all the columns PV in the I/O list */
	err = srt_f_IOstructsetcolpvs(	
				(iolist),
				SRT_NO,
				(double*) prev_assets[0],
				grfnparam->node_dim,
				""	);

	free_dmatrix(prev_assets,maxtrinf.min_x_index,maxtrinf.max_x_index,
                				0,grfnparam->node_dim-1);

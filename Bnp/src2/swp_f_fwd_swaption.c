...

{
    ...
    double *dCoveragesShort = NULL, *dCoveragesLong = NULL, **dCopulaCube = NULL, *dMean_v = NULL,
           *dAvg   = NULL;
    ...
    double *dPayoff = NULL, *dPayoffSwnShort = NULL, *dPayoffSwnLong = NULL, dFwdLong, dFwdShort,
           dLvlLong, dLvlShort, *dCoveragesAux                       = NULL;
    ...
    if (flag != 1)
    {
        err = swp_f_ForwardRate(
            lPayDatesLong[0],
            lPayDatesLong[iNPayDatesLong - 1],
            cFreq,
            cBasis,
            cYCname,
            cRefRname,
        ...
        err = swp_f_ForwardRate(
            lPayDatesShort[0],
            lPayDatesLong[iNPayDatesShort - 1],
            cFreq,
            cBasis,
            cYCname,
            cRefRname,
...

{
    ...
    for (i = 1; i < iNPayDatesLong; i++)
    {
        ...
        err = swp_f_ForwardRate(
            lPayDatesLong[0], lPayDatesLong[i], cFreq, cBasis, cYCname, cRefRname, &dFwd);
        err = swp_f_LevelPayment(
            lPayDatesLong[0], lPayDatesLong[i], cFreq, cBasis, cYCname, cRefRname, &dLevel);
    ...
    //      computes the Fwd swap rate
                    ...
                    err =
    GetDensity_Lvl_Measure(nClass,dMat,dFwd,SigmaHestonShort[i-1],AlphaHestonShort[i-1],1.0,LambdaHestonShort[i-1],
    ...
    int     flag)
{
    ...
    */
    ...
    for (i = 0; i < nStrikes; i++)
    {
        ...
        dPayoff[i] = DMAX(
            0.0,
            flag * (dLevelLong * (dCopulaCube[iMC][iNDatesLong - 1] - xlStrike[i]) -
        ...
        dPayoffSwnShort[i] =
            DMAX(0.0, flag * (dLevelShort * (dCopulaCube[iMC][iNDatesShort - 1] - xlStrike[i])));

        dPayoffSwnLong[i] =
            DMAX(0.0, flag * (dLevelLong * (dCopulaCube[iMC][iNDatesLong - 1] - xlStrike[i])));
...
Err GetSimulatedLevel(
    double* DfSet, int iNDates, Date* lEndDates, double* dCoverages, double* dLevel)
{
    Err err = NULL;
    int i;
    ...
    double** dCPDy)
{
    ...
    dPPDx[nTen][0] =
        -dShiftH + dFwd * exp(-0.5 * dEqBSVol * dEqBSVol * dMat - dStdDev * dEqBSVol * sqrt(dMat));
    //	dPPDx[nTen][2*nClass]=-dShiftH + dFwd * exp( - 0.5*dEqBSVol*dEqBSVol*dMat +
...
Err GetSpanningSetDf(
    double** dCopulaCube, int iNDates, Date* lEndDates, double* dCoverages, int iMC, double* DfSet)
{
    Err    err = NULL;
    double dProd, dSum;
    int    i, k, l;
    ...
    double*  ChiError)
{
    ...
    */
    ...
    {
        err = srt_f_optimpvol(
            dPrice[i], dFwd, Strikes[i], Maturity, 1.0, SRT_CALL, SRT_LOGNORMAL, &ImplVol);

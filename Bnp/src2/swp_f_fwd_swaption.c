/* ===================================================================================
   ...
   PURPOSE:       Computes price of a fwd swaption, with maturity T, to enter into a Fwd swap 
                  between T-1 > T and T_2 > T. 
				  To model the fwd vol, the function uses a method 
                  based on CMS replication and Copula simulation in a Swap market model 
				  context
   =================================================================================== */
...
Err		PriceFwdSwaption(
						 ...
						 double *SigmaHestonShort,
						 double *AlphaHestonShort,
						 double *LambdaHestonShort,
						 double *BetaHestonShort,
						 double *RhoHestonShort,
						 double **CorrelLong,
						 double *dPrice,
						 double *dPriceSwnShort,
...

{
	...
	double *dCoveragesShort=NULL,*dCoveragesLong=NULL,**dCopulaCube=NULL,*dMean_v=NULL,*dAvg=NULL;
	double **dPPDx=NULL,**dPPDy=NULL,**dCPDy=NULL,**dPPD_Shortx=NULL,**dPPD_Shorty=NULL,**dCPD_Shorty=NULL;
	int i,j,k,iNDatesShort, iNDatesLong, iNPayDatesShort, iNPayDatesLong,nClass=1<<xlnClasses,flag;
	double *dPayoff=NULL,*dPayoffSwnShort=NULL,*dPayoffSwnLong=NULL,dFwdLong,dFwdShort,dLvlLong,dLvlShort,
		   *dCoveragesAux=NULL;
    ...
    err = swp_f_setSwapDP(xlMaturityDate, xlStartDate, srtFreq, srtBasis, &SwapShort);
    err = swp_f_setSwapDP(xlStartDate, xlEndDate, srtFreq, srtBasis, &SwapLong);
														
	err = swp_f_make_FixedLegDatesAndCoverages(&SwapShort,lToday,&lPayDatesShort,&iNPayDatesShort,
								  			   &lStartDatesShort,&lEndDatesShort,
								  			   &dCoveragesShort,&iNDatesShort);
	
	err = swp_f_make_FixedLegDatesAndCoverages(&SwapLong,lToday,&lPayDatesAux,&iNPayDatesAux,
								  			   &lStartDatesAux,&lEndDatesAux,
								  			   &dCoveragesAux,&iNDatesAux);	
	...
	///////////////////////////////////////////////////////////////
		...
		GetStudentCplDev (xlStudDegree,dMean_v,CorrelLong,xlNumSim,xlStudDegree+iNDatesLong,dPPDx,
						  dCPDy,2*nClass+1,0,SOBOL,dCopulaCube);
 ...
 //   swp_f_ForwardRate(lPayDatesLong[0],lPayDatesLong[1],cFreq,cBasis,cYCname,cRefRname,&dFwd);
	...
	for (i=3; i<xlNumSim; i++){
		...
		err = GetFwdSwaptionPayoff(	dCopulaCube,
									iNDatesLong,
									lEndDatesLong,
									dCoveragesLong,
	...
	if (flag != 1) {
		...
		err = swp_f_ForwardRate(lPayDatesLong[0],lPayDatesLong[iNPayDatesLong-1],cFreq,cBasis,cYCname,cRefRname,&dFwdLong);
		err = swp_f_ForwardRate(lPayDatesShort[0],lPayDatesLong[iNPayDatesShort-1],cFreq,cBasis,cYCname,cRefRname,&dFwdShort);

		err = swp_f_LevelPayment(lPayDatesLong[0],lPayDatesLong[iNPayDatesLong-1],cFreq,cBasis,cYCname,cRefRname,&dLvlLong);
		err = swp_f_LevelPayment(lPayDatesShort[0],lPayDatesLong[iNPayDatesShort-1],cFreq,cBasis,cYCname,cRefRname,&dLvlShort);

		for (k=0; k< nStrikes; k++) {
		 dPrice[k]-=dLvlLong*(dFwdLong-Strikes[k])-dLvlShort*(dFwdShort-Strikes[k]);
		}
	}
...
Err       Fwd_Swaption_Get_Cum_Density(Date lToday,
								 char *cYCname,
						         char *cRefRname,
								 SrtCompounding srtFreq,
								 SrtBasisCode srtBasis,
...
//		shiftHeston = BetaHeston[i-1];
		...
		err = GetDensity_Lvl_Measure(nClass,dMat,dFwd,SigmaHeston[i-1],AlphaHeston[i-1],1.0,LambdaHeston[i-1],
...
//      computes the Fwd swap rate
		...
		swp_f_ForwardRate(lPayDatesShort[0],lPayDatesShort[i],cFreq,cBasis,cYCname,cRefRname,&dFwd);
		err = GetDensity_Lvl_Measure(nClass,dMat,dFwd,SigmaHestonShort[i-1],AlphaHestonShort[i-1],1.0,LambdaHestonShort[i-1],
			     ShiftHestonShort[i-1],RhoHestonShort[i-1],dDisc,dUpBound,iNPts,i-1,dCov,dPPD_Shortx,dPPD_Shorty,dCPD_Shorty);					
	}
...
Err		GetFwdSwaptionPayoff(double  **dCopulaCube,
			                 int     iNDatesLong,
							 Date    *lEndDatesLong,
							 double  *dCoveragesLong,
							 ...
							 int     iMC,
							 double  *dPayoff,
							 double  *dPayoffSwnShort,
							 double  *dPayoffSwnLong,
							 int flag)
{
	...
	err = GetSpanningSetDf(dCopulaCube,iNDatesLong,lEndDatesLong,dCoveragesLong,iMC,DfSet);
...
*/
	...
	err = GetSimulatedLevel(DfSet,iNDatesLong,lEndDatesLong,dCoveragesLong,&dLevelLong);
	err = GetSimulatedLevel(DfSet,iNDatesShort,lEndDatesShort,dCoveragesShort,&dLevelShort);

	for (i=0; i< nStrikes; i++) {

		//	(*dPayoff)= DfSet[iNDatesLong-1];	

			dPayoff[i]=DMAX(0.0, flag*(dLevelLong*(dCopulaCube[iMC][iNDatesLong-1]-xlStrike[i]) 
		...
		//	(*dPayoff)=DMAX(0.0, dCopulaCube[iMC][iNDatesLong-1]-xlStrike);
			...
			dPayoffSwnShort[i]=DMAX(0.0, flag*(dLevelShort*(dCopulaCube[iMC][iNDatesShort-1]-xlStrike[i]) ) );
	
			dPayoffSwnLong[i]=DMAX(0.0, flag*(dLevelLong*(dCopulaCube[iMC][iNDatesLong-1]-xlStrike[i]) ) ); 
...
Err		GetSimulatedLevel(double  *DfSet,
			              int     iNDates,
						  Date    *lEndDates,
						  double  *dCoverages,
						  double  *dLevel)
...
/*----------------------------------------------------------------------------------- */
/*        Function that returns a set of n probability densities for the Heston model */
/*----------------------------------------------------------------------------------- */
...
//	dShiftH = HESTON_CONST*(1.-dBetaH)/dBetaH;
	...
	dPPDx[nTen][0]=-dShiftH + dFwd * exp( - 0.5*dEqBSVol*dEqBSVol*dMat - dStdDev*dEqBSVol*sqrt(dMat));
//	dPPDx[nTen][2*nClass]=-dShiftH + dFwd * exp( - 0.5*dEqBSVol*dEqBSVol*dMat + dStdDev*dEqBSVol*sqrt(dMat));
	...
	for (i=1; i<= 15; i++) {
		...
		dX[1]+=0.04;
 		HestonPrice(dFwd,dX,1,dMat,dSigmaH,dAlphaH,dSigmaIftyH,dMeanRevH,dBetaH,dRhoH,
                    dDisc,dUpBound,SRT_CALL,DENSITY,SRT_TRUE,2,nSteps,dY);
		if (dY[1] < dThreshold) {
	...
	/// computes the density of the heston model on the whole domain;
		...
		HestonPrice(dFwd,dPPDx[nTen]-1,2*nClass+1,dMat,dSigmaH,dAlphaH,dSigmaIftyH,dMeanRevH,dBetaH,dRhoH,
                                   dDisc,dUpBound,SRT_CALL,DENSITY,SRT_TRUE,2,nSteps,dPPDy[nTen]-1);		
...
/*--------------------------------------------------------------------------------------- */
/*        Function that returns a set n Discount factors compatible with the set of swaps */
/*--------------------------------------------------------------------------------------- */


Err GetSpanningSetDf( double  **dCopulaCube,
			          int     iNDates,
					  Date    *lEndDates,
					  double  *dCoverages,
					  int     iMC,
					  double  *DfSet)
{
	...
	for (i=0; i<iNDates; i++){
		...
		for (k=0; k<i; k++) {
			...
			dProd=1.0;
			for (l=k; l<=i; l++) dProd*=(1.+dCoverages[l]*dCopulaCube[iMC][l]);

			dSum+=dCoverages[k]/dProd;

		}
		DfSet[i]=-dSum*dCopulaCube[iMC][i]+1./(1+dCoverages[i]*dCopulaCube[iMC][i]);
	}
...

{
	...
	///////////////////////////////////////////////////////////////
		...
		for (i=0; i<nDates; i++) dMean_v[i]=0.0;  
	...
	GetStudentCplDev (xlStudDegree,dMean_v,CorrelLong,xlNumSim,xlStudDegree+nDates,dPPDx,     
					  dCPDy,2*nClass+1,0,SOBOL,dCopulaCube);
	...
	for (i=3; i<xlNumSim; i++){
		...
		err = GetCapletPayoff  (	dCopulaCube,
									nDates,
									lDates,
									dCoverages,
					 ...
					 )
{
	...
	err = EvalChi2Error(a,Maturity,iNDates,lEndDates,dCoverages,dCoverageShort,nStrikes,Strikes,BSVols,dFwd,dCopulaCube,
		                dPPDx,dCPDy,xlStudDegree,dMean_v,
					    nClass,nSim,SigmaHestonShort,AlphaHestonShort,LambdaHestonShort,BetaHestonShort,
						RhoHestonShort,dCorrel,&fa);

	err = EvalChi2Error(b,Maturity,iNDates,lEndDates,dCoverages,dCoverageShort,nStrikes,Strikes,BSVols,dFwd,dCopulaCube,
		                dPPDx,dCPDy,xlStudDegree,dMean_v,
					    nClass,nSim,SigmaHestonShort,AlphaHestonShort,LambdaHestonShort,BetaHestonShort,
						RhoHestonShort,dCorrel,&fb);
                  ...
                  double *ChiError)
{
	...
	GetStudentCplDev (xlStudDegree,dMean_v,Correl,nSim,xlStudDegree+iNDates,dPPDx,     
					  dCPDy,2*nClass+1,0,SOBOL,dCopulaCube);
...
*/
	...
	for (i=0;i<nStrikes; i++) {
		...
		err = srt_f_optimpvol(dPrice[i], dFwd, Strikes[i], Maturity, 1.0, 
							 ...
							 )
{
	...
	err = GetSpanningSetDf(dCopulaCube,iNDates,lEndDates,dCoverages,iMC,DfSet);

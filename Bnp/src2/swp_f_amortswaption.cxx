/* ===================================================================================
   FILENAME:      swp_f_amortswaption.cxx
...
Err ShiftedLogCopulaSimul(double **corr_mtx, long NPaths, long nFactorDim,
                          int *pCutDim, double **res) {
  int i, j, k;
  ...
  //////////  Step1: performs Chol decomposition of the correlation matrix
  /// corr_mtx
  ...
  //////////  Step3: Generates the matrix of r.v. compatible with the Copula and
  /// the marginals
  for (i = 0; i < NPaths; i++) {
...
void ShiftedLogCopulaSimul_NotOpt(double **corr_mtx, long NPaths, long Dim,
                                  double *Fwd, double *Shifts, double *Vols,
                                  double **res) {
  long i, idum = -8935807;
  int j, k;
  double ***GaussSample = NULL, **UnifSample = NULL, **sqrt_corr_mtx = NULL;
  double sign;
  ...
  //////////  Step1: performs Chol decomposition of the correlation matrix
  /// corr_mtx
  CholDec(&CholAlg, corr_mtx, sqrt_corr_mtx, Dim);
  ...
  for (i = 0; i < NPaths; i++) {
    ...
    for (j = 0; j < Dim; j++) {
      ...
      //				res[i][j] = sign * ( (sign * Fwd[j] +
      // Shifts[j]) * exp(-0.5 * Vols[j] * Vols[j] + Vols[j] * res[i][j]) -
      // Shifts[j] );
      res[i][j] = ((Fwd[j] + Shifts[j]) *
                   exp(-0.5 * Vols[j] * Vols[j] + Vols[j] * res[i][j])) -
                  Shifts[j];
    }
  }
...
Err GetAmortizedSwaptionPayoff_ForMAD(
    double **dCopulaCube, SrtCompounding srtFreq, SrtBasisCode srtBasis,
    int iNCoInitSwaps, double *dLvls, double *dStrikes, double *dNotionals,
    int iMC, double *dPayoff, double *controlvar, int flag) {
  Err err = NULL;
  int i;

  *dPayoff = 0;
  for (i = 0; i < iNCoInitSwaps; ++i) {
    *dPayoff += dNotionals[i] * dLvls[i] * (dCopulaCube[iMC][i] - dStrikes[i]) +
                controlvar[i];
  }
...
//--------------------------------------------------------------------------------------
//------------------Utilities Functions for Shifted Log
// Model---------------------------
//--------------------------------------------------------------------------------------

Err GetShiftedLogDensity_Lvl_Measure_ForMAD(
    long today, char *cYCname, char *cVCname, char *cRefRname,
    SrtCompounding srtFreq, SrtBasisCode srtBasis, long start, long end,
    double dStrike,
    int iCalibshift, // 0 => Calibrate the Shift parameter        ,
                     // 1 => Use the input Shift
    double *dShift, double *dVol) {
  ...
  err = swp_f_ForwardRate(start, end, cFreq, cBasis, cYCname, cRefRname,
                          &dFwdSwap);
  dFwdCash =
      swp_f_swapcashrate(start, end, srtBasis, srtFreq, cYCname, cRefRname);
  err =
      swp_f_LevelPayment(start, end, cFreq, cBasis, cYCname, cRefRname, &dLvl);
  ex_time = (add_unit(start, -2, SRT_BDAY, MODIFIED_SUCCEEDING) - today) *
            YEARS_IN_DAY;
  ...
  /*	err = swp_f_SABRvol( cVCname        , start        , end        , 0.05 ,
          ...
          err = swp_f_SABRvol( cVCname        , start        , end        , 0.05
     , &alpha        , &power        , SABR_ALPHA ); if (err) return err; err =
     swp_f_SABRvol( cVCname        , start        , end        , 0.05        ,
                          &beta        , &power        , SABR_BETA );
          if (err) return err;
          err = swp_f_SABRvol( cVCname        , start        , end        , 0.05
     , &rho        , &power        , SABR_RHO ); if (err) return err;
  ...
  */
  ...
  k1 = dFwdSwap;
  k2 = DMIN(dFwdSwap * (1 + 4 * sqrt(ex_time) * atmvol),
            DMAX(dFwdSwap * exp(-4 * sqrt(ex_time) * atmvol), dStrike));
  if (fabs(k2 - k1) < 0.001) {
    k1 = k2 + 0.001;
  }
  ...
  if (power < 1) {
    price1 =
        srt_f_optblknrm(dFwdSwap, k1, vol1, ex_time, 1, SRT_CALL, SRT_PREMIUM);
    price2 =
        srt_f_optblknrm(dFwdSwap, k2, vol2, ex_time, 1, SRT_CALL, SRT_PREMIUM);

    err = srt_f_optimpvol(price1, dFwdSwap, k1, ex_time, 1, SRT_CALL,
                          SRT_LOGNORMAL, &vol1);
    err = srt_f_optimpvol(price2, dFwdSwap, k2, ex_time, 1, SRT_CALL,
                          SRT_LOGNORMAL, &vol2);
    if (err)
      return err;
  }
  ...
  */
  ...
  if (iCalibshift == 0) {
    err = find_shifted_log_params(ex_time, dFwdSwap, k1, vol1, k2, vol2, &shift,
                                  &shifted_vol, &equi_beta);
    *dShift = shift;
  } else {
    price2 =
        srt_f_optblksch(dFwdSwap, k2, vol2, ex_time, 1.0, SRT_CALL, PREMIUM);

    err = srt_f_optimpvol(price2, *dShift + dFwdSwap, *dShift + k2, ex_time,
                          1.0, SRT_CALL, SRT_LOGNORMAL, &shifted_vol);
  }
...
Err Fwd_Swaption_Get_Cum_ShiftedLogDensity_ForMAD(
    Date lToday, char *cYCname, char *cVCname, char *cRefRname,
    SrtCompounding srtFreq, SrtBasisCode srtBasis,

    int iNDates, Date *lStartDates, Date *lEndDates,

    double *dStrikes,

    int iCalibShift, double Shift, double *dShifts, double *dVols) {
  Err err = NULL;
  int i;
  double dMat, dCov;
  char *cBasis, *cFreq;
  ...
  for (i = 0; i < iNDates; i++) {
    shift = Shift;
    err = GetShiftedLogDensity_Lvl_Measure_ForMAD(
        lToday, cYCname, cVCname, cRefRname, srtFreq, srtBasis, lStartDates[0],
        lEndDates[i], dStrikes[i], iCalibShift, &shift, &vol);
    dShifts[i] = shift;
    dVols[i] = vol;
  }
...
//--------------------------------------------------------------------------------------
//----------------End Of Utilities Functions for Shifted Log
// Model----------------------
//--------------------------------------------------------------------------------------

//--------------------------------------------------------------------------------------
//---------------------Shifted Log Model : Function for
// MAD-----------------------------
//--------------------------------------------------------------------------------------
Err AmortizedSwaptionShiftedLogForMAD(
  ...
  const int nFactorDim =
      nForwardDim > nFactorMax ? nFactorMax : nForwardDim; // # of factors
  ...
  double *pdA = memset(_alloca(sizeof(double) * nFactorDim), 0,
                       sizeof(double) * nFactorDim);
  double *pdB = memset(_alloca(sizeof(double) * nFactorDim), 0,
                       sizeof(double) * nFactorDim);
  double *pdC = memset(_alloca(sizeof(double) * nFactorDim), 0,
                       sizeof(double) * nFactorDim);

  if (err = translate_basis(&cBasis, srtBasis))
    return err;
  if (err = err = translate_compounding(&cFreq, srtFreq))
    return err;
  if (err = err =
          swp_f_get_ref_rate_details(cRefRname, &srtFloatBasis, &srtFloatFreq))
    return err;
  ...
  for (i = 0; i < lNFloatDates; ++i)
    dFloatCoverages[i] =
        coverage(lFloatStartDates[i], lFloatEndDates[i], srtFloatBasis);
  ...
  //---------------------------------------------------------------------------------
  err = Fwd_Swaption_Get_Cum_ShiftedLogDensity_ForMAD(
      lToday, cYCname, cVCname, cRefRname, srtFreq, srtBasis,

      lNFixDates, lFixStartDates, lFixEndDates,

      dStrikes,

      iCalibShift, Shift, dShifts, dVols);
  if (err) {
    return err;
  }
  ...
  err =
      ShiftedLogCopulaSimul(Correl, xlNumSim, nFactorDim, pCutDim, dCopulaCube);
  if (err) {
  ...
  *dPrice = 0.;
  ...
  for (i = 0, j = 0; j < nFactorDim; ++j) {
    for (; i <= pCutDim[j]; ++i) {
      ...
      const double dWLFS =
          dCoInitNotionals[i] * dLvls[i] * (dFwdSwapRates[i] + dShifts[i]);
...
//--------------------------------------------------------------------------------------
//----------------Shifted Log Model : End Of Function For
// MAD---------------------------
//--------------------------------------------------------------------------------------
...
Err AmortizedSwaptionShiftedLogForMAD_NotOpt(
  ...
  double **dCopulaCube = NULL, **dCopulaCubeShiftedLog = NULL, *dMean_v = NULL,
         *dAvg = NULL;
  ...
  */
  ...
  for (i = 0; i < lNFixDates; ++i) {
    //		dFixCoverages[i] = coverage(lFixStartDatesOut[i]        ,
    //lFixEndDatesOut[i]
    //      , srtBasis);
    dFixCoverages[i] = coverage(lFixStartDates[i], lFixEndDates[i], srtBasis);
  }
  ...
  for (i = 0; i < lNFloatDates; ++i) {
    //		dFloatCoverages[i] = coverage(lFloatStartDatesOut[i]        ,
    // lFloatEndDatesOut[i]        , srtFloatBasis);
    dFloatCoverages[i] =
        coverage(lFloatStartDates[i], lFloatEndDates[i], srtFloatBasis);
  }
  ...
  //---------------------------------------------------------------------------------
  //--------Convert AMort Swap in Coinit Swap
  // Portfolio------------------------------
  //---------------------------------------------------------------------------------
  ...
  err = ConvertAmortSwapWithMarginsInCoInitSwapPortfolioForMAD2(
      ...
      //      , 								 lFixEndDatesOut
      //      ,
      lFixStartDates, lFixEndDates, dFixCoverages, dFixNotionals, dFixRates,

      lNFloatDates,
      //								 lFloatStartDatesOut
      //      , 								 lFloatEndDatesOut
      //      ,
      lFloatStartDates, lFloatEndDates, dFloatCoverages, dFloatNotionals,
      dFloatSpreads, dFloatMargins,
  ...
  //	if( add_unit (lToday        , 2        , SRT_BDAY        ,
  //MODIFIED_SUCCEEDING) >= lFixStartDatesOut[0])
  if (add_unit(lToday, 2, SRT_BDAY, MODIFIED_SUCCEEDING) >= lFixStartDates[0]) {
  ...
  //---------------------------------------------------------------------------------
  //--------Copula Retrieves the Matrix of Joint
  // distributions-----------------------
  //---------------------------------------------------------------------------------
  ...
  ShiftedLogCopulaSimul_NotOpt(Correl, xlNumSim, lNFixDates, dFwdSwapRates,
                               dShifts, dVols, dCopulaCube);

  for (i = 3; i < xlNumSim; i++) {
    for (j = 0; j < lNFixDates; ++j) {
      mean[j] +=
          dCoInitNotionals[j] * dLvls[j] * (dCopulaCube[i][j] - dStrikes[j]);
    }
  }
  ...
  for (j = 0; j < lNFixDates; ++j) {
    ...
    dFwdLvl = dLvls[j];
    mean[j] = dCoInitNotionals[j] * dFwdLvl * (dFwdSwap - dStrikes[j]) -
              mean[j] / (xlNumSim - 2);
  }
  ...
  *dPrice = 0.0;
  ...
  for (i = 3; i < xlNumSim; i++) {
    err = GetAmortizedSwaptionPayoff_ForMAD(
        dCopulaCube, srtFreq, srtBasis, lNFixDates, dLvls, dStrikes,
        dCoInitNotionals, i, &dPayoff, mean, flag);
...
FREE_RETURN:
  ...
  if (dFixCoverages)
    free_dvector(dFixCoverages, 0, lNFixDates - 1);
  if (dFloatCoverages)
    free_dvector(dFloatCoverages, 0, lNFloatDates - 1);

  if (dStrikes)
    free_dvector(dStrikes, 0, lNFixDates - 1);

  if (dShifts)
    free_dvector(dShifts, 0, lNFixDates - 1);
  if (dVols)
    free_dvector(dVols, 0, lNFixDates - 1);

  if (dMean_v)
    free_dvector(dMean_v, 0, lNFixDates - 1);
  if (dAvg)
    free_dvector(dAvg, 0, lNFixDates - 1);
  if (dCopulaCube)
    free_dmatrix(dCopulaCube, 0, xlNumSim, 0, lNFixDates - 1);

  if (dCoInitStrikes)
    free(dCoInitStrikes);
  if (dCoInitNotionals)
    free_dvector(dCoInitNotionals, 0, lNFixDates - 1);

  if (mean)
    free_dvector(mean, 0, lNFixDates - 1);
  if (dLvls)
    free_dvector(dLvls, 0, lNFixDates - 1);
  if (dFwdSwapRates)
    free_dvector(dFwdSwapRates, 0, lNFixDates - 1);

  //	if(lFixStartDatesOut) free_lngvector(lFixStartDatesOut        , 0
  //      ,lNFixDates); 	if(lFixEndDatesOut) free_lngvector(lFixEndDatesOut
  //      , 0 ,lNFixDates);

  //	if(lFloatStartDatesOut) free_lngvector(lFloatStartDatesOut        , 0
  //      ,lNFloatDates); 	if(lFloatEndDatesOut)
  //      free_lngvector(lFloatEndDatesOut        , 0 ,lNFloatDates);

  return err;
}

//--------------------------------------------------------------------------------------
//---------------------Shifted Log Model : Main
// Function--------------------------------
//--------------------------------------------------------------------------------------
Err AmortizedSwaptionShiftedLog(char *cYCname, char *cVCname, char *cRefRname,
  ...
  err = ConvertAmortSwapWithMarginsInCoInitSwapPortfolio2(
      cYCname, cVCname, cRefRname, srtCallPut, xlStartDate, xlEndDate, srtFreq,
      srtBasis, exer_fee, lNFixNot, dFixNotionals, dFixRates, lNFloatNot,
      dFloatNotionals, dMargins, dStrikes, dCoInitNotionals, dLvls,
      dFwdSwapRate, UseVol);
  ...
  shortstub = 0;
  if ((iNDates > 1) &&
      ((int)(12 * (lEndDates[0] - lStartDates[0]) / 365.0 + 0.5) !=
       (int)(12 * (lEndDates[1] - lStartDates[1]) / 365.0 + 0.5))) {
    shortstub = 1;
  }
  ...
  err = Fwd_Swaption_Get_Cum_ShiftedLogDensity_ForMAD(
      ...
      iNPayDates - 1, lStartDates, lEndDates,
  ...
  ShiftedLogCopulaSimul_NotOpt(Correl, xlNumSim, iNDates, dFwdSwapRate, dShifts,
                               dVols, dCopulaCube);
  ...
  *dPrice = 0.0;
  ...
  for (i = 3; i < xlNumSim; i++) {
    err = GetAmortizedSwaptionPayoff_ForMAD(
        dCopulaCube, srtFreq, srtBasis, lNFixNot, dLvls, dStrikes,
        dCoInitNotionals, i, &dPayoff, mean, flag);
...
FREE_RETURN:
  if (dStrikes)
    free_dvector(dStrikes, 0, iNDates - 1);

  if (dShifts)
    free_dvector(dShifts, 0, iNDates - 1);
  if (dVols)
    free_dvector(dVols, 0, iNDates - 1);

  if (dMean_v)
    free_dvector(dMean_v, 0, iNDates - 1);
  if (dAvg)
    free_dvector(dAvg, 0, iNDates - 1);
  if (dCopulaCube)
    free_dmatrix(dCopulaCube, 0, xlNumSim, 0, iNDates - 1);

  if (dCoInitStrikes)
    free(dCoInitStrikes);
  if (dCoInitNotionals)
    free_dvector(dCoInitNotionals, 0, iNDates - 1);

  if (mean)
    free_dvector(mean, 0, iNDates - 1);
  if (dLvls)
    free_dvector(dLvls, 0, iNDates - 1);
  if (dFwdSwapRate)
    free_dvector(dFwdSwapRate, 0, iNDates - 1);

  return err;
}
//--------------------------------------------------------------------------------------
//----------------Shifted Log Model : End Of Main
// Function------------------------------
//--------------------------------------------------------------------------------------
...
Err PriceAmortizedSwaptionShiftedLog(
  ...
  err = "Function PriceAmortizedSwaptionShiftedLog no more supported        , "
        "please "
        "use AmortizedSwaptionShiftedLogForMAD";
...
Err PriceAmortizedSwaptionSABR(char *cYCname, char *cVCname, char *cRefRname,
  ...
  err = "Function PriceAmortizedSwaptionSABR no more supported        , please "
        "use "
        "AmortizedSwaptionShiftedLogForMAD";
                                           ...
                                           )
{
        ...
        double	***GaussSample=NULL        ,**UnifSample=NULL        ,
                ...
                //////////  Memory allocation
                GaussSample=dcube(0        ,p        ,0        ,d-1        ,0
,0); sqrt_corr_mtx=dmatrix(0        ,d-1        ,0        ,d-1);

                PositiveMatrix(corr_mtx        , d);

                //////////  Step1: performs Chol decomposition of the
correlation matrix corr_mtx CholDec	(&CholAlg        , corr_mtx        ,
sqrt_corr_mtx       , d);
                ...
                //////////  Step3: Generates the matrix of r.v. compatible with
the Copula and the marginals for (i=0;i<p;i++)
                {
                        for (j=0; j<d; j++)
                        {
                                GaussSample[i][j][0]=inv_cumnorm_fast(UnifSample[i][j]);
...
/*
double amortSwap_PV(long lToday        , char *cYCname        , int iNCpns ,
long *lCpnDates        , double *dCpns)
{
        int i;
        double PV;
        ...
        for(i=0;i<iNCpns;++i)
        {
                PV = PV + dCpns[i] * swp_f_df(lToday        , lCpnDates[i] ,
cYCname);
        }
...
Err		GetAmortizedSwaptionPayoff5(double  **dCopulaCube        ,
      ...
      , SrtBasisCode srtBasis        , double	 *dCoverages        , int
iNCoInitSwaps        , double  *dStrikes        , double  *dNotionals        ,
                                                         int     iMC        ,
                                                         double  *dPayoff , int
flag)
{
...
//		*dPayoff += dNotionals[i] * dLevelCash
*(dCopulaCube[iMC][i]-dStrikes[i]);
//	}
        ...
        *dPayoff = 0;
        for(i=0;i<iNCoInitSwaps;++i)
        {
                dLevelCash =(1.-1./pow(1+dCopulaCube[iMC][i]*dCov
      ,i+1))/dCopulaCube[iMC][i]; *dPayoff += dNotionals[i] * dLevelCash
*(dCopulaCube[iMC][i]-dStrikes[i]);
        }
...
Err		GetAmortizedSwaptionPayoff5_c(double  **dCopulaCube        ,
                                                         SrtCompounding srtFreq
      , SrtBasisCode srtBasis        , double	 *dCoverages        , int
iNCoInitSwaps        , double  *dLvls        , double  *dStrikes        , double
*dNotionals        , int     iMC        , double  *dPayoff        , double
*controlvar        , int flag)
{
        Err err = NULL;
        double dLevelCash;
        ...
        *dPayoff = 0;
        ...
        {
                dLevelCash =(1.-1./pow(1+dCopulaCube[iMC][i]*dCov
      ,i+1))/dCopulaCube[iMC][i]; dLevelCash =dLvls[i]; *dPayoff +=
dNotionals[i] * dLevelCash *(dCopulaCube[iMC][i]-dStrikes[i]) + controlvar[i];
        }
                                                ...
                                                )
{
        ...
        double *dVec = NULL        , *dX=NULL        ,*dY=NULL        ,
dThreshold=1.e-3; int i; double deriv1        , deriv2; Err err = NULL; double
epsilon = 1.0e-4; double temp        , temp_plus        , temp_moins; double dK;
        double dFwdSwap        , dFwdCash        , dSpread        , dLvl ,
ex_time; double sigma        , alpha        , beta        , rho; char *cBasis ,
*cFreq; double power        , atmvol; double shift        , shifted_vol        ,
equi_beta; double k1        , k2        , vol1        , vol2; SrtCallPutType
callput; double sgn;

        err = translate_basis(&cBasis        , srtBasis);
        err = translate_compounding(&cFreq        , srtFreq);

        err = swp_f_ForwardRate(start        , end        , cFreq        ,
cBasis        , cYCname        , cRefRname        , &dFwdSwap); dFwdCash =
swp_f_swapcashrate( start        , end        , srtBasis , srtFreq        ,
cYCname        , cRefRname); err = swp_f_LevelPayment(start        , end , cFreq
, cBasis        , cYCname        , cRefRname        , &dLvl); ex_time =
(add_unit (start        ,
- 2        , SRT_BDAY        , MODIFIED_SUCCEEDING) - today) * YEARS_IN_DAY;
...
//	{
//		shift = 10000;
//		price2 = srt_f_optblksch(dFwdSwap        , k2        , vol2 ,
ex_time        , 1 , SRT_CALL        , SRT_PREMIUM);
//		err = srt_f_optimpvol(price2        , shift+dFwdSwap        ,
shift+k2        , ex_time        , 1        , SRT_CALL        , SRT_LOGNORMAL ,
&shifted_vol);
//	}
        ...
        *dShift = shift;
        ...
        temp = srt_f_optblksch(shift + sgn * dFwdSwap        ,
                                                                shift + sgn *
dPPDx[nTen][0]        , shifted_vol        , ex_time        , 1.0        ,
callput        , PREMIUM);

        temp_plus = srt_f_optblksch(shift + sgn * dFwdSwap        ,
                                                                shift + sgn *
dPPDx[nTen][1]        , shifted_vol        , ex_time        , 1.0        ,
callput        , PREMIUM);
        ...
        dPPDy[nTen][0]=DMAX(0        ,(deriv1 -
deriv2)/dK)/Df_x;//*LvlCFwd/LvlC_x; dCPDy[nTen][0]=0.0;
        ...
        for (i=1;i<2*nClass;i++)
        {
                ...
                temp_plus = srt_f_optblksch(shift + sgn * dFwdSwap        ,
                                                                shift + sgn *
dPPDx[nTen][i+1]        , shifted_vol        , ex_time        , 1.0        ,
callput        , PREMIUM);

                LvlC_x = (1.-1./pow(1+dPPDx[nTen][i]*dCov
      ,nTen+1))/dPPDx[nTen][i]; Df_x = 1;//./(1+dPPDx[nTen][i]*dCov);

                deriv1 = DMIN(1.0        ,(temp_moins - temp)/dK);
                deriv2 = DMIN(deriv1        ,(temp - temp_plus)/dK);

                dPPDy[nTen][i]=DMAX(0        ,(deriv1 -
deriv2)/dK)/Df_x;//*LvlCFwd/LvlC_x;
        ...
        LvlC_x = (1.-1./pow(1+dPPDx[nTen][2*nClass]*dCov
      ,nTen+1))/dPPDx[nTen][2*nClass]; Df_x =
1;//./(1+dPPDx[nTen][2*nClass]*dCov);

        deriv1 = (temp_moins -
temp)/(dPPDx[nTen][2*nClass]-dPPDx[nTen][2*nClass-1]); deriv2 = 0.0;

        dPPDy[nTen][2*nClass]=DMAX(0        ,(deriv1 -
deriv2)/dK)/Df_x;//*LvlCFwd/LvlC_x;
...
Err       Fwd_Swaption_Get_Cum_ShiftedLogDensity(Date lToday        ,
                                                         ...
                                                         char *cRefRname ,
                                                                 SrtCompounding
srtFreq        , SrtBasisCode srtBasis        ,

                                                                 int
iNPayDatesLong        , Date *lPayDatesLong        , double *dCoveragesLong ,
...
//		dDisc=swp_f_df(lToday        , lPayDatesLong[0]        ,
cYCname);
                                    ...
                                    )
{
        ...
        double **dCopulaCube=NULL        ,**dCopulaCubeShiftedLog=NULL
,*dMean_v=NULL
      ,*dAvg=NULL;
...
control-------------------------------------------------- double *mean=NULL;
        ...
        err = translate_compounding(&cFreq        , srtFreq);
        /////////////////////////////////////////////////////////////////////////
        ///// defines the pointer to the curve and retrieves today and spot
dates
        /////////////////////////////////////////////////////////////////////////

        pCurve = lookup_curve (cYCname);
        ///// if (!pCurve) throw XSort("Fatal: (FwdSwaption) Yield Curve Object
not found"); lToday = (Date) get_today_from_curve (pCurve); lSpotLag = (Date)
get_spotlag_from_curve(pCurve);

        ///////////////////////////////////////////////////////////////
        ///// defines the relevant swap objects
        ///////////////////////////////////////////////////////////////
    err = swp_f_setSwapDP(xlStartDate        , xlEndDate        , srtFreq ,
srtBasis        , &Swap); if (err)
        {
                goto FREE_RETURN;
        }

        err = swp_f_make_FixedLegDatesAndCoverages(&Swap        ,lToday
,&lPayDates
      ,&iNPayDates        , &lStartDates        ,&lEndDates        , &dCoverages
,&iNDates); if (err)
        {
                goto FREE_RETURN;
        }
...
//---------------------------------------------------------------------------------
        ...
        err = ConvertAmortSwapWithMarginsInCoInitSwapPortfolio2(
                                                                 ...
                                                                 lNFloatNot ,
                                                                 dFloatNotionals
      , dMargins        , dStrikes        , dCoInitNotionals        , dLvls ,
dFwdSwapRate        , UseVol
                                                                 );
    ...
    if (srtCallPut == SRT_CALL) flag=1;
        ...
        if( (iNDates>1)
                        &&
        ( (int)(12*(lEndDates[0]-lStartDates[0])/365.0 +
0.5)!=(int)(12*(lEndDates[1]-lStartDates[1])/365.0+0.5) ) )
        {
                shortstub = 1;
        }
        ...
        {
                mean[j] = 0.0;
                iv += dCoInitNotionals[j] * dLvls[j] * ( dFwdSwapRate[j] -
dStrikes[j]);
        }

        if( add_unit (lToday        , 2        , SRT_BDAY        ,
MODIFIED_SUCCEEDING) >= xlStartDate)
        {
                *dPrice = DMAX(0.0        , flag * iv);
                goto FREE_RETURN;
        }
...
//---------------------------------------------------------------------------------
//--------Conversion into shifted Log before
Copula--------------------------------
//---------------------------------------------------------------------------------
...
//---------------------------------------------------------------------------------
        ...
        dCopulaCube = dmatrix(0        ,xlNumSim        ,0        ,iNDates-1);
        dCopulaCubeShiftedLog = dmatrix(0        ,xlNumSim        ,0
,iNDates-1);

//	GetStudentCplDev (xlStudDegree        , dMean_v        , Correl ,
xlNumSim        , xlStudDegree+iNDates        ,dPPDShiftedLogx        ,
//						  dCPDy        ,2*nClass+1 ,0
      ,RANDOM_GAUSS        ,dCopulaCubeShiftedLog);

        CopulaSimul(Correl        ,xlNumSim        ,iNDates ,dPPDShiftedLogx
,dCPDy ,2*nClass+1        ,dCopulaCubeShiftedLog);

        //--------Get the swap rates back from their shifted
log--------------------------- for(i=0;i<iNDates;++i)
        {
                for(j=0;j<xlNumSim+1;++j)
                {
                        dCopulaCube[j][i] = exp(dCopulaCubeShiftedLog[j][i]) -
dShifts[i];
                }
        }
        ...
        for (i=3; i<xlNumSim; i++)
        {
                for(j=0;j<lNFixNot;++j)
                {
                        mean[j] += dCoInitNotionals[j] * dLvls[j]
*(dCopulaCube[i][j]-dStrikes[j]);
                }
        }

        for(j=0;j<lNFixNot;++j)
        {
//		err = swp_f_ForwardRate(lStartDates[0]        , lEndDates[j] ,
cFreq , cBasis        , cYCname        , cRefRname        , &dFwdSwap); dFwdSwap
= dFwdSwapRate[j]; dFwdLvl = dLvls[j]; mean[j] = dCoInitNotionals[j] * dFwdLvl *
( dFwdSwap - dStrikes[j]) - mean[j] / (xlNumSim-2);
        }
...
FREE_RETURN:
        ...
        if(dPPDx) free_dmatrix(dPPDx        ,0        ,iNDates-1        ,0
,2*nClass); if(dPPDy) free_dmatrix(dPPDy        ,0        ,iNDates-1        ,0
,2*nClass); if(dCPDy) free_dmatrix(dCPDy        ,0        ,iNDates-1        ,0
,2*nClass);

        if(dPPDShiftedLogx) free_dmatrix(dPPDShiftedLogx        ,0 ,iNDates-1 ,0
      ,2*nClass);

        if(dMean_v) free_dvector(dMean_v        ,0        ,iNDates-1);
        if(dAvg) free_dvector(dAvg        ,0        ,iNDates-1);
        if(dCopulaCube) free_dmatrix(dCopulaCube        ,0        ,xlNumSim ,0
,iNDates-1); if(dCopulaCubeShiftedLog) free_dmatrix(dCopulaCubeShiftedLog ,0
      ,xlNumSim        ,0        ,iNDates-1);
        ...
        if(dCoInitNotionals) free_dvector(dCoInitNotionals        , 0        ,
iNDates-1);
...
SrtBasisCode srtBasis        , long	lNFixNot        , double *dFixNotionals
, double *dFixRates       , double *dStrikes        , long	*lNCoInitSwaps
, double  **dAmounts)
{
        Err err = NULL;
        int i;
        ...
        long *lFixPayDates=NULL        , *lFixStartDates=NULL        ,
*lFixEndDates=NULL; double *dFixCoverages=NULL;

        pCurve = lookup_curve (cYCname);
        lToday = (Date) get_today_from_curve (pCurve);

    err = swp_f_setSwapDP(StartDate        , EndDate        , srtFreq        ,
srtBasis        , &Swap); if(err) goto FREE_RETURN;

        err = swp_f_make_FixedLegDatesAndCoverages(&Swap        ,lToday
,&lFixPayDates
      ,&iNFixPayDates        , &lFixStartDates        ,&lFixEndDates        ,
&dFixCoverages
      ,&iNFixDates); if(err) goto FREE_RETURN;
...
(1+dFixCoverages[iNFixDates-1]*dStrikes[iNFixDates-1]);
        ...
        {
                (*dAmounts)[iNFixDates-i-1] = ( dFixNotionals[iNFixDates-i-1] -
dFixNotionals[iNFixDates-i]
...
//--------------------------------------------------------------------------------------
//------------End Of Convert Amortized Swap in CoInitial Swaps Portfolio
(2)------------
//--------------------------------------------------------------------------------------
*/

/*
double PV_IRR(char *cYCname        , long lToday        , int iIndexStart , int
iNPayDates , long *lPayDates        , double *dCpns        , double R)
{
        int i;
        double PV;
        ...
        {
                PV+= dCpns[i]
                        * (swp_f_df(lToday        , lPayDates[i]        ,
cYCname) / swp_f_df(lToday        , lPayDates[0]        , cYCname) )
...
*lFloatStartDates=NULL        , *lFloatEndDates=NULL; double
*dFloatCoverages=NULL; double *dFloatSpreads=NULL; int i        , FixFloatMult;
        double *dCpns=NULL;
...
&dFixCoverages
      ,&iNFixDates); if(err) goto FREE_RETURN;
        ...
        err = swp_f_make_FloatLegDatesCoveragesAndSpreads(&Swap        , lToday
, cRefRname        , &lFloatPayDates        , &iNFloatPayDates        ,
&lFloatFixingDates        , &lFloatStartDates        , &lFloatEndDates        ,
                                                                                                &dFloatCoverages        ,
                                                                                                &dFloatSpreads        ,
                                                                                                &iNFloatDates);
        ...
        for(i=1;i<iNFloatPayDates-1;++i)
        {
                ...
                {
                        dCpns[i] = dCpns[i] +
dFixCoverages[(int)(i/FixFloatMult)-1]*dFixRates[(int)(i/FixFloatMult)-1] *
dFixNotionals[(int)(i/FixFloatMult)-1];

                        dCpns_Fix[(int)(i/FixFloatMult)] =
dFixCoverages[(int)(i/FixFloatMult)-1];
                }
        }
...
dFixRates[iNFixPayDates-2]
        ...
        R = 0.05;
        PV = PV_IRR(cYCname        , lToday        , 0        , iNFloatPayDates
, lFloatPayDates , dCpns        , R);

        Compt = 0;
        while((fabs(PV)>1e-12)&&(Compt<15))
        {
                R2 = R + 0.0001;
                PV2 = PV_IRR(cYCname        , lToday        , 0        ,
iNFloatPayDates        , lFloatPayDates        , dCpns        , R2);

                R = R - PV * (R2 - R) / (PV2 - PV);
                PV = PV_IRR(cYCname        , lToday        , 0        ,
iNFloatPayDates        , lFloatPayDates        , dCpns        , R);
        ...
        for(i=1;i<iNFixPayDates;++i)
        {
                ...
                PV_Float = PV_IRR(cYCname        , lToday        , 0        ,
i*FixFloatMult+1        , lFloatPayDates        , dCpns_Float        , R);
PV_Fix = PV_IRR(cYCname        , lToday        , 0        , i+1        ,
lFixPayDates        , dCpns_Fix        , R);

                dStrikes[i-1] = -PV_IRR(cYCname        , lToday        , 0 ,
i*FixFloatMult+1        , lFloatPayDates        , dCpns_Float        , R);
dStrikes[i-1] = dStrikes[i-1] / PV_IRR(cYCname        , lToday        , 0 , i+1
, lFixPayDates        , dCpns_Fix        , R);
...
, srtFreq        ,srtBasis        , lNFixNot        ,dFixNotionals        ,
                                                 lNFloatNot ,dFloatNotionals
      ,dFixRates        , dStrikes); if (err)
        {
                goto FREE_RETURN;
        }
//-----------------------------------------------------------------------
...
---------------------------------------- dShifts = dvector(0        ,iNDates-1);
        ...
        //--------Conversion into shifted Log befor
Copula--------------------------------- dPPDShiftedLogx=dmatrix(0 ,iNDates-1 ,0
      ,2*nClass); for(i=0;i<iNDates;++i)
        {
                for(j=0;j<2*nClass+1;++j)
                {
                        dPPDShiftedLogx[i][j] = log(dShifts[i] + dPPDx[i][j]);
        ...
        //--------Copula Retrieves the Matrix of Joint
distributions----------------------- dMean_v=dvector(0        ,iNDates-1);
        dAvg=dvector(0        ,iNDates-1);
        for (i=0; i<iNDates; i++) {
                dAvg[i]=0.0;
                dMean_v[i]=0.0;
...
//	GetStudentCplDev (xlStudDegree        ,dMean_v        ,Correl ,xlNumSim
      ,xlStudDegree+iNDates        ,dPPDShiftedLogx        ,
//						  dCPDy        ,2*nClass+1 ,0
      ,RANDOM_GAUSS        ,dCopulaCubeShiftedLog);
...
//							&dCoInitStrikes        ,
        ...
        *dPrice=0.0;
        ...
        {
                mean[j] = 0.0;
                err = swp_f_LevelPayment(lStartDates[0]        , lEndDates[j] ,
cFreq , cBasis        , cYCname        , cRefRname        , &(dLvls[j]));
        }
        ...
        for (i=3; i<xlNumSim; i++)
        {
                for(j=0;j<iNCoInitSwaps;++j)
                {
//			err = swp_f_LevelPayment(lStartDates[0]        ,
lEndDates[j] , cFreq        , cBasis        , cYCname        , cRefRname ,
&dFwdLvl);
//			dLevelCash =(1.-1./pow(1+dCopulaCube[i][j]*dCov
      ,j+1))/dCopulaCube[i][j]; mean[j] += dCoInitNotionals[j] * dLvls[j]
*(dCopulaCube[i][j]-dStrikes[j]);
                }
        }
        ...
        {
                err = swp_f_ForwardRate(lStartDates[0]        , lEndDates[j] ,
cFreq , cBasis        , cYCname        , cRefRname        , &dFwdSwap);
//		dFwdLvl = (1.-1./pow(1+dFwdSwap*dCov        ,j+1))/dFwdSwap;
                dFwdLvl = dLvls[j];
//		err = swp_f_LevelPayment(lStartDates[0]        , lEndDates[j] ,
cFreq , cBasis        , cYCname        , cRefRname        , &dFwdLvl); mean[j] =
dCoInitNotionals[j] * dFwdLvl * ( dFwdSwap - dStrikes[j]) - mean[j] /
(xlNumSim-2);
        }


//	dSwapPV = amortSwap_PV(lToday        , cYCname        , iNCpns ,
lCpnDates        , dCpns); df_start = swp_f_df(lToday        , xlStartDate ,
cYCname); flag = -1; if (srtCallPut == SRT_CALL) flag=1; for (i=3; i<xlNumSim;
i++)
        {
//		if(iPayoffMethod==5)
//		{
                                                        ...
                                                        )
{
        ...
        double *dVec = NULL        , *dX=NULL        ,*dY=NULL        ,
dThreshold=1.e-3; int i; double dShiftH; double deriv1        , deriv2; Err err
= NULL; double epsilon = 1.0e-4; double *strike=NULL        , *strike_plus=NULL
, *strike_moins=NULL; double *temp=NULL        , *temp_plus=NULL        ,
*temp_moins=NULL; double dK; double price_moins; double price; double
price_plus; double *strikeVect=NULL; double *priceVect=NULL;
...
//	dShiftH = HESTON_CONST*(1.-dBetaH)/dBetaH;
        ...
        dPPDx[nTen][2*nClass] = 1.0;//sqrt(dMat);//dFwd +
dStdDev*dEqBSVol*sqrt(dMat);
        ...
        HestonPrice(dFwd        ,strikeVect        ,2*nClass+1        ,dMat ,
                dSigmaH        ,dAlphaH        ,dSigmaIftyH        ,dMeanRevH
,dBetaH        ,dRhoH        , 1.0        ,dUpBound        ,SRT_CALL ,PREMIUM
,SRT_TRUE        ,2        ,nSteps ,priceVect);
      ...
      ,nTen+1))/dPPDx[nTen][i];
                ...
                dPPDy[nTen][i]=LvlCFwd * DMAX(0        ,(deriv1 -
deriv2))/dK;//(dK*LvlC_x));
...
iNPayDatesLong        , Date *lPayDatesLong        , double *dCoveragesLong ,
                                                                 ...
                                                                 double
*SigmaHeston        , double *AlphaHeston        , double *LambdaHeston        ,
...
//      computes the Fwd swap rate
                ...
                err = swp_f_ForwardRate(lPayDatesLong[0] ,lPayDatesLong[i]
      ,cFreq        ,cBasis        ,cYCname        ,cRefRname        ,&dFwd);
err = swp_f_LevelPayment(lPayDatesLong[0]        ,lPayDatesLong[i]        ,cFreq
,cBasis       ,cYCname ,cRefRname        ,&dLevel);

                err = GetDensity_Lvl_Measure9_b(nClass        ,dMat        ,dFwd
      ,SigmaHeston[i-1]        ,AlphaHeston[i-1]        ,1.0 ,LambdaHeston[i-1]
, BetaHeston[i-1]        ,RhoHeston[i-1] ,dLevel        ,dUpBound        ,iNPts
,i-1        ,dCov        ,dPPDx        ,dPPDy        ,dCPDy);
        }
...
//--------------------------------------------------------------------------------------
//-------------------Utilities Functions for Heston
Model-------------------------------
//--------------------------------------------------------------------------------------
Err  GetHestonParameters(long lToday        ,
                                                 char	*cYCname        ,
                                                 char	*cVCname        ,
                                                 long lStartDate        , long
lEndDate        , SrtCompounding srtFreq        , SrtBasisCode srtBasis        ,
                                                 char	*cRefRname        ,
                                                 double UpperBound        ,
                                                 int nSteps        ,
                                                 int iIntegerType        ,
                                                 ...
                                                 double **HestonLambda)
{
        //---Swap
Details------------------------------------------------------------------ SwapDP
Swap; char *cBasis        , *cFreq; Date *lPayDates=NULL ,*lStartDates=NULL
      ,*lEndDates=NULL; double *dCoverages=NULL; int iNDates        ,
iNPayDates;

        int i;
        double SABRsigma        , SABRalpha        , SABRbeta        , SABRrho;
        double HESTONsigma        , HESTONalpha        , HESTONbeta        ,
HESTONrho; double HESTONsigmainfinity        , HESTONlambda;

        long start        , end;

        double dFwdSwap        , dFwdCash        , dSpread        , dLvl ,
ex_time; double power;
//	double shift;

        Err err=NULL;

        //----------Convert Basis and Freq in
SRTcode-------------------------------------- err = translate_basis(&cBasis ,
srtBasis); err = translate_compounding(&cFreq        , srtFreq);


        //-----Init and Fill
SWAPDP-------------------------------------------------------- err =
swp_f_setSwapDP(lStartDate        , lEndDate        , srtFreq        , srtBasis
, &Swap); if(err)
        {
                goto FREE_RETURN;
        }
...
cYCname        , cRefRname); err = swp_f_LevelPayment(start , end        , cFreq
, cBasis        , cYCname        , cRefRname        , &dLvl); ex_time =
(add_unit (start        , - 2        , SRT_BDAY        , MODIFIED_SUCCEEDING) -
lToday) * YEARS_IN_DAY;

                dSpread = dFwdSwap - dFwdCash;

                err = swp_f_SABRvol( cVCname        , start        , end , 0.05
, &SABRsigma        , &power        , SABR_BETAVOL ); if (err) return err; err =
swp_f_SABRvol( cVCname        , start        , end        , 0.05        ,
                                &SABRalpha        , &power        , SABR_ALPHA
); if (err) return err; err = swp_f_SABRvol( cVCname        , start        , end
, 0.05        , &SABRbeta        , &power        , SABR_BETA ); if (err) return
err; err = swp_f_SABRvol( cVCname        , start        , end        , 0.05 ,
                ...
                err = CalibrateHestonToSABR(dFwdSwap        ,
                                                                        ex_time
      , SABRsigma        , SABRalpha        , SABRbeta        , SABRrho        ,
&HESTONsigma        , &HESTONalpha , &HESTONsigmainfinity        , &HESTONlambda
, &HESTONbeta        , &HESTONrho        , UpperBound , nSteps        ,
iIntegerType        , nStdDev        , SRT_TRUE        , CHI2_MIN);
...
//--------------------------------------------------------------------------------------
//------------End Of Utilities Functions for Heston
// Model-------------------------------
//--------------------------------------------------------------------------------------
...
lToday        ,lSpotLag;
        ...
        //---Swap
Details------------------------------------------------------------------ char
*cBasis        , *cFreq; Date *lPayDates=NULL        ,*lStartDates=NULL
,*lEndDates=NULL; double *dCoverages=NULL; int iNDates        , iNPayDates;

        //---Monte Carlo
Paths------------------------------------------------------------- double
**dCopulaCube=NULL        ,**dCopulaCubeShiftedLog=NULL        ,*dMean_v=NULL
,*dAvg=NULL;

        //---Distribution
Discretization--------------------------------------------------- double
**dPPDx=NULL        ,**dPPDy=NULL        ,**dCPDy=NULL; double
**dPPDShiftedLogx=NULL;

        //---Miscelaneous
Integers--------------------------------------------------------- int i        ,
j; int nClass=1<<xlnClasses; int flag;
...
//	int iNCpns;
        ...
        //---Amortized Swap as a Coinit Swaps
Portfolio------------------------------------ int iNCoInitSwaps; Date
*lCoInitSwapsEndDates=NULL; double *dCoInitStrikes=NULL; double
*dCoInitNotionals=NULL;
...
shifts------------------------------------------------- double shift;
        ...
        //---Heston
Parameters------------------------------------------------------------- double
*SigmaHeston=NULL; double *AlphaHeston=NULL; double *LambdaHeston=NULL; double
*BetaHeston=NULL; double *RhoHeston=NULL;
...
srtBasis); err = translate_compounding(&cFreq        , srtFreq);
        ...
        //-------defines the pointer to the curve and retrieves today and spot
dates------- pCurve = lookup_curve (cYCname); lToday = (Date)
get_today_from_curve (pCurve); lSpotLag = (Date) get_spotlag_from_curve(pCurve);

        //-----Init and Fill
SWAPDP-------------------------------------------------------- err =
swp_f_setSwapDP(xlStartDate        , xlEndDate        , srtFreq        ,
srtBasis        , &Swap); if(err)
        {
                goto FREE_RETURN;
        }
...
,&iNDates); if(err)
        ...
        //--------Retrieves the
densities-------------------------------------------------- dPPDx=dmatrix(0
      ,iNDates-1        ,0        ,2*nClass); dPPDy=dmatrix(0        ,iNDates-1
,0        ,2*nClass); dCPDy=dmatrix(0        ,iNDates-1        ,0 ,2*nClass);
...
Copula--------------------------------- dPPDShiftedLogx=dmatrix(0 ,iNDates-1 ,0
      ,2*nClass); for(i=0;i<iNDates;++i)
        {
                err = swp_f_ForwardRate(lStartDates[0]        , lEndDates[i] ,
cFreq , cBasis        , cYCname        , cRefRname        , &dFwdSwap);
for(j=0;j<2*nClass+1;++j)
                {
...
shift;
        ...
        //--------Convert AMort Swap in Coinit Swap
Portfolio------------------------------ dStrikes = dvector(0        ,iNDates-1);
err = ComputeIRRStrikes(cYCname        , lToday        , cRefRname        ,
xlStartDate        , xlEndDate        , srtFreq        ,srtBasis        ,
                                                 lNFixNot        ,dFixNotionals
, lNFloatNot        ,dFloatNotionals ,dFixRates        , dStrikes); if (err)
        {
                goto FREE_RETURN;
        }
        err=ConvertAmortSwapInCoInitalSwapsPortfolio2(
...
dPayoff;
        ...
        //---Amortized Swap as a
Bond------------------------------------------------------ Date *lCpnDates=NULL;
        double *dCpns=NULL;

        //---Amortized Swap as a Coinit Swaps
Portfolio------------------------------------ Date *lCoInitSwapsEndDates=NULL;
        double *dCoInitStrikes=NULL;
        double *dCoInitNotionals=NULL;
...
Portfolio------------------------------ dStrikes = dvector(0        ,iNDates-1);
        ...
        err = ConvertAmortSwapWithMarginsInCoInitSwapPortfolio(
                                                                 ...
                                                                 lNFloatNot ,
                                                                 dFloatNotionals
      , dMargins        , dStrikes        , dCoInitNotionals        , UseVol);
if (err)
        {
                goto FREE_RETURN;
        }
...
- 2        , SRT_BDAY        , MODIFIED_SUCCEEDING) - today) * YEARS_IN_DAY;
        ...
        LvlC_x = dDf * (1.-1./pow(1+dPPDx[nTen][0]*dCov
      ,nTen+1))/dPPDx[nTen][0];
        ...
        for (i=1;i<2*nClass;i++)
        {
                ...
                temp_plus = srt_f_optblkschbetastochquick(
                                                                dFwdSwap ,
                                                                dPPDx[nTen][i+1]
      , ex_time        , sigma        , alpha        , beta        , rho , 1.0
, SRT_BETAVOL        , SRT_LOGNORMAL        , SRT_CALL        , PREMIUM);

                LvlC_x = dDf * (1.-1./pow(1+dPPDx[nTen][i]*dCov
      ,nTen+1))/dPPDx[nTen][i];
        ...
        LvlC_x = dDf * (1.-1./pow(1+dPPDx[nTen][2*nClass]*dCov
      ,nTen+1))/dPPDx[nTen][2*nClass];
...
epsilon = 1.0e-4; double temp        , temp_plus        , temp_moins; double dK;
        double dFwdSwap        , dFwdCash        , dSpread        , dLvl ,
ex_time; double sigma        , alpha        , beta        , rho; char *cBasis ,
*cFreq; double power; long kfail; double PrMin        , PrAdj;
...
, SRT_BETAVOL        , SRT_LOGNORMAL        , SRT_CALL        , PREMIUM);
      ...
      ,nTen+1))/dPPDx[nTen][i];
                ...
                dCPDy[nTen][i-1]=DMAX(0
      ,dCPDy[nTen][i]-(dPPDy[nTen][i]+dPPDy[nTen][i-1])/2.*dK);
...
, SRT_BETAVOL        , SRT_LOGNORMAL        , SRT_CALL        , PREMIUM);
                ...
                LvlC_x = dDf * (1.-1./pow(1+dPPDx[nTen][kfail]*dCov
      ,nTen+1))/dPPDx[nTen][kfail];
...
, SRT_BETAVOL        , SRT_LOGNORMAL        , SRT_CALL        , PREMIUM);
      ...
      ,nTen+1))/dPPDx[nTen][i];
                        ...
                        dCPDy[nTen][i-1]=DMAX(0
      ,dCPDy[nTen][i]-(dPPDy[nTen][i]+dPPDy[nTen][i-1])/2.*dK/PrAdj);
                        if(dCPDy[nTen][i-1]>dCPDy[nTen][i])
                        {
                                kfail = i;
                                PrMin = dCPDy[nTen][i];
...
epsilon = 1.0e-4; double temp        , temp_plus        , temp_moins; double dK;
        double dFwdSwap        , dFwdCash        , dSpread        , dLvl ,
ex_time; double sigma        , alpha        , beta        , rho; char *cBasis ,
*cFreq; double power; long kfail; double PrMin; double x_PrMin        ,
discriminent        , shifted_vol        , shift;
...
//			shift = HESTON_CONST*(1.-beta)/beta;
                        shift = 0.0;
                        discriminent = x_PrMin * x_PrMin * ex_time - 2 * ex_time
* log((shift+dPPDx[nTen][i])/(shift+dFwdSwap)); shifted_vol =
(x_PrMin*sqrt(ex_time) + sqrt(discriminent))/ex_time; i=0;
                }
        }
...
//-----------------------------------------------------------------------------
      ...
      , shifted_vol        , ex_time        , 1.0        , SRT_CALL        ,
PREMIUM);
                                          ...
                                          double *sigmabeta)
{
        ...
        while(fabs(atm_model - atm_premium)>1e-7)
        {
                ...
                atm_model2 = srt_f_optblkschbetastochquick(
                                                                ...
                                                                ex_time        ,
                                                                betavol2 , alpha
, beta        , rho        , disc        , SRT_BETAVOL        , SRT_LOGNORMAL ,
                                                                SRT_CALL ,
                                                                PREMIUM);
...
epsilon = 1.0e-4; double temp        , temp_plus        , temp_moins; double dK;
        double dFwdSwap        , dFwdCash        , dSpread        , dLvl ,
ex_time; double sigma        , alpha        , beta        , rho; char *cBasis ,
*cFreq; double power; double shift        , shifted_vol        , atm_premium;
...
//	shift = HESTON_CONST*(1.-beta)/beta;
        ...
        *dShifts = shift;
        ...
        temp_moins = srt_f_optblkschbetastochquick(
                                                                shift+dFwdSwap ,
                                                                DMAX(0.0001 ,
shift+dPPDx[nTen][0] - dK)        , ex_time        , shifted_vol        , alpha
, 1.0        , rho        , 1.0        , SRT_BETAVOL        , SRT_LOGNORMAL ,
                                                                SRT_CALL ,
                                                                PREMIUM);
        ...
        temp = srt_f_optblkschbetastochquick(
                                                                shift+dFwdSwap ,
                                                                shift+dPPDx[nTen][0]
      , ex_time        , shifted_vol        , alpha        , 1.0        , rho
, 1.0        , SRT_BETAVOL        , SRT_LOGNORMAL        , SRT_CALL        ,
                                                                PREMIUM);
...
/*
//--------------------------------------------------------------------------------------
//-----------Function that transforms smile from the swap measure into Fwd
measure------
//--------------------------------------------------------------------------------------
...
//--------------------------------------------------------------------------------------
//-------End Of Function that transforms smile from the swap measure into Fwd
measure---
//--------------------------------------------------------------------------------------
*/
...
//	int NSpreads;
        ...
        //------defines the pointer to the curve and retrieves today and spot
dates------------ pCurve = lookup_curve (cYCname);
        ///// if (!pCurve) throw XSort("Fatal: (FwdSwaption) Yield Curve Object
not found");
        //------Get Today and
SpotLag---------------------------------------------------------- lToday =
(Date) get_today_from_curve (pCurve); lSpotLag = (Date)
get_spotlag_from_curve(pCurve);
...
//		{
                        err = GetAmortizedSwaptionPayoff5(dCopulaCube        ,
                                                                                        ...
                                                                                        iNCoInitSwaps        ,
//
dCoInitStrikes        , dStrikes        , dCoInitNotionals        , i        ,
&dPayoff        , flag); if(err)
                        {
                                goto FREE_RETURN;
                        }
...
//		*dPrice += df_start * DMAX(0        , flag * dPayoff) - flag *
(dSwapPV + df_start * dPayoff);

                *dPrice += (DMAX(0        ,flag * dPayoff) - *dPrice)/(i-2);
//		*dPrice += df_start * (flag * dPayoff - *dPrice)/(i-2);
//		*dPrice += df_start * flag * dPayoff + (df_start * dSimuSwapPV
- dSwapPV);
        }

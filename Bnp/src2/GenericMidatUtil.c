...
Err genmidat_copy_model(GENMIDAT_MODEL sSourceModel, GENMIDAT_MODEL sDestModel)
{
    ...
    err = genmidat_alloc_model(sSourceModel->iNbVols, sSourceModel->iNbFactor, sDestModel);
...
Err genmidat_fill_2factor(GENMIDAT_MODEL sModel)
{
    ...
    sModel->dBeta2[0] = sModel->dStartBeta2 + (sModel->dBeta[0] - sModel->dStartBeta) *
    ...
    for (i = 1; i < sModel->iNbVols; i++)
    {
        ...
        sModel->dBeta2[i] =
            sModel->dBeta2[i - 1] + (sModel->dBeta[i] - sModel->dBeta[i - 1]) / exp_fact;
        sModel->dLambda2[i] = sModel->dLambda[i] + sModel->dGamma;
    }
    ...
    double*        dPrice)
{
    ...
    if (iIntegStartIndex < 0)
    {
        ...
        if (sModel->iNbFactor == 2)
        {
            ...
            covar +=
                sModel->dCorrel[0] * sModel->dSigma[0] * sModel->dSigma2[0] * sModel->dTimes[0];
        }
    ...
    for (i = iIntegStartIndex; i < iIntegEndIndex; i++)
    {
        ...
        if (sModel->iNbFactor == 2)
        {
            varY += sModel->dSigma2[i + 1] * sModel->dSigma2[i + 1] *
                    (sModel->dTimes[i + 1] - sModel->dTimes[i]);
            covar += sModel->dCorrel[i + 1] * sModel->dSigma[i + 1] * sModel->dSigma2[i + 1] *
                     (sModel->dTimes[i + 1] - sModel->dTimes[i]);
        }
    }
    ...
    *dPrice = srt_f_optblknrm(
        dForward, 0.0, *dVolatility, dEndTime - dStartTime, sModel->dNumeraire, SRT_CALL, PREMIUM);
    ...
    double* dPrice)
{
    ...
    err = genmidat_implied_volatility_and_price_model(
        iIntegStartIndex, iIntegEndIndex, iUndIndex, iNbPeriod, sModel, dVolatility, dPrice);
    ...
    double*        dCorrelation)
{
    ...
    dCovar = dBetaX1 * dBetaX2 * varX + (dBetaX1 * dBetaY2 + dBetaY1 * dBetaX2) * covar +

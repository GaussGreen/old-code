...
Err cts_set_default_params(
  ...
  *numer_tstar = 0;
  ...
  lgmsv_app_set_default_params(iNbX, iNbSigmaXLeft, iNbSigmaXRight, dIntegParam,
                               iIntegMethod, dVolLimit, iCalibLGM, dMinStd,
                               dMaxStd);
...
Err cts_caller(
    ...
    SrtDiffusionType vol_type, /*	Type of vol in matrix        ,
                                  SRT_NORMAL or SRT_LOGNORMAL */
  ...
  double *fund_mrg2 = NULL, *fund_spr2 = NULL, *fund_not_ts2 = NULL,
         *fund_fix_cpn2 = NULL;

  /* All Structure initialisations */
  cts_pricing = calloc(1, sizeof(cts));
  cts_on_iv = calloc(1, sizeof(cts));
  cts_iv_adj = calloc(1, sizeof(cts));

  und = calloc(1, sizeof(cts_und));
  und_iv = calloc(1, sizeof(cts_und));
  und_iv_adj = calloc(1, sizeof(cts_und));

  adi_arg = calloc(1, sizeof(cts_adi_arg));

  fwd_iv_secu = calloc(1, sizeof(cts_iv));
  fwd_iv_reserve = calloc(1, sizeof(cts_iv));

  cpn2 = calloc(1, sizeof(cts_cpn));
  mkt = calloc(1, sizeof(cts_mkt));

  NumerParams = calloc(1, sizeof(LGMSV_NumerParams));

  if (!cts_pricing || !cts_on_iv || !cts_iv_adj || !und || !und_iv ||
      !und_iv_adj || !adi_arg || !fwd_iv_secu || !fwd_iv_reserve || !cpn2 ||
      !mkt || !NumerParams) {
  ...
  lgmsv_init_numer_params(NumerParams, iNbX, iNbSigmaXGridLeft,
                          iNbSigmaXGridRight, dIntegParam, dVolLimit, iCalibLGM,
                          iIntegMethod, dMinStd, dMaxStd);
  ...
  err = cts_init_mkt(today, yc, vc, ref, swap_freq, swap_basis, lib_freq,
                     lib_basis, get_cash_vol, GetCashVolAndConvert, vol_type,
                     cash_vol, num_strikes_in_vol, strikes_in_vol, mkt);
  ...
  if (exercised) {
    ...
    if ((ncpn > 0) && (ex_date_set < cpn_pay_date[ncpn - 1])) {
      err = "Exercised settlement should be > than payment date of the last "
            "not called cf coupon ";
      goto FREE_RETURN;
    }

    if ((fund_ncpn > 0) && (ex_date_set < fund_pay[fund_ncpn - 1])) {
      err = "Exercised settlement should be > than payment date of the last "
            "not called funding coupon ";
      goto FREE_RETURN;
    }
    ...
    if ((ex_date_set < today + eod_pay_flag) ||
        ((ncpn == 0) && (fund_ncpn == 0))) {
      /* Fill the output */
      *fund_val = *cpn_val = *call_val = *mostexp_reserve = *switch_reserve =
          0.0;

      /* Initial exchange of notional */
      if (start_date >= today + eod_pay_flag) {
        if (fund_ccy) {
          *fund_val -= swp_f_df(today, start_date, fund_ccy_yc) *
                       fund_not_ts[0] * fx_fund_dom *
      ...
      if (ex_date_set >= today + eod_pay_flag) {
        if (fund_ccy) {
          /* Final exchange of notional */
          *fund_val += swp_f_df(today, ex_date_set, fund_ccy_yc) *
                       fund_not_ts[0] * fx_fund_dom *
                       swp_f_df(today, fx_fund_dom_spot_date, yc) /
                       swp_f_df(today, fx_fund_dom_spot_date, fund_ccy_yc);
    ...
    if ((ncpn > 0) && (fund_ncpn == 0)) {
      err = "All the funding coupons are called but not all the cf coupons : "
            "Not allowed ";
      goto FREE_RETURN;
    }
    if ((ncpn == 0) && (fund_ncpn > 0))
    ...
    {
      err = "All the cf coupons are called but not all the funding coupons : "
            "Not allowed ";
      goto FREE_RETURN;
    }
  ...
  fund_not_ts2 = calloc(fund_ncpn, sizeof(double));
  ...
  if (fund_ccy == 1) {
    ...
    err = convert_funding_to_domestic(
        ...
        fx_fund_dom_spot_date, cpn_not, yc, fund_ncpn, fund_fix, fund_start,
  ...
  err = cts_fill_check_all_struct(
      mkt, tstar, use_calib, lambda, nb_factor, lgm_alpha, lgm_gamma, lgm_rho,
      nsmilepar, smilepartime, alphaeps, rhoeps, ldaeps, rho2eps, lgmsvund,
      fund_ncpn, fund_not_ts2, fund_fix, fund_start, fund_end, fund_pay,
      fund_basis, fund_spr2, fund_mrg2, fix_lag_bd, ncpn, cpn_type,
      cpn_start_date, cpn_end_date, cpn_pay_date, cpn_coupon, cpn_basis,
      cpn_not_ts, pay_fix_date, pay_months, pay_freq, pay_basis, pay_fwd_spread,
      pay_gearing, nfix, weights, fix_dates, fix_tenor, fix_freq, fix_basis,
      fix_fwd_spreads, tstype, alpha, beta, nstr, str, nbopt, iv_buy_sell,
      value_zero, lb, ub, payoff, iv_call_spread, iv_call_spread, iv_trim_type,
      iv_max_fix, iv_min_fix_time, use_cmsopt, correl_start, correl_end,
      float_adjust_type, 0, pay_rec, ex_date, set_date, fee, adj_fee,
      do_one_time, one_time_index, 0, NumerParams, pde_or_mc, req_stp,
      req_stppsi, req_stpx, req_stpz, req_mintime, req_paths, integ_mintime,
      cal_tenor, cal_ref, cal_freq, cal_basis, force_atm, max_std_long,
      max_std_short, vol_shift_long, vol_type_long, vol_shift_type_long,
      vol_shift_short, vol_type_short, vol_shift_type_short, lambda_shift,
      calib_strategy, fix_lambda, short_tenor, short_refrate, short_freq,
      short_basis, fix_smile, smile_calib_months, lgmsv_calib_params, min_time,
      skip_last, min_fact, max_fact, use_jumps, numer_tstar, prec, maxiter,
      keep_first, long_strike_flag, short_strike_flag, NULL, 1, calc_fwd_iv,
      eod_fix_flag, eod_ex_flag, cts_on_iv, und_iv, &call_feat_iv, adi_arg,
      save_inst_data, inst_data, save_fwdiv, fwd_iv_info);
  ...
  if (cts_on_iv->fund_leg->num_cpn > 0) {
    if (for_fund) {
      fund_leg_pv +=
          swp_f_df(today, cts_on_iv->fund_leg->cpn[0].start_date, yc) *
          eq_final_ex;
      fund_leg_pv +=
          swp_f_df(today,
                   cts_on_iv->fund_leg->cpn[cts_on_iv->fund_leg->num_cpn - 1]
                       .pay_date,
                   mkt->yc) *
  ...
  while (i < fund_ncpn && fund_fix[i] < today + eod_fix_flag) {
    if (fund_pay[i] >= today + eod_pay_flag) {
      ...
      fund_leg_pv += (fund_fix_cpn2[i] + fund_mrg2[i]) *
                     coverage(fund_start[i], fund_end[i], bas) *
                     fund_not_ts2[i] * swp_f_df(today, fund_pay[i], yc);
    }
  ...
  if ((exercised) && (fund_pay[fund_ncpn - 1] != ex_date_set)) {
    /* Substract the notional received at fund_pay[fund_ncpn-1] and add notional
     * paid at ex_settle_date */
    if (for_fund) {
      /* Other currency case */
      fund_leg_pv +=
          (swp_f_df(today, ex_date_set, Copyfund_ccy_yc) -
  ...
  if (!compatibility_flag) {
    ...
    while (i < ncpn && fix_dates[i][0] < today + eod_fix_flag) {
      if (cpn_pay_date[i] >= today + eod_pay_flag) {
        ...
        err = cts_calc_fixed_cpn_mkt_value(
            mkt, cpn_start_date[i], cpn_end_date[i], cpn_pay_date[i],
            cpn_coupon[i], cpn_basis[i], cpn_not_ts[i], fix_lag_bd, cpn_type[i],
            pay_fix_date[i], pay_months, pay_freq, pay_basis, pay_fwd_spread[i],
            pay_gearing[i], fixed_pay[i], j0, weights[i], fixed_ref[i], tstype,
            alpha[i], beta[i], nstr[i], str[i], nbopt[i], lb[i], ub[i],
            payoff[i], accrue_on_barrier, eod_fix_flag, eod_pay_flag,
            eod_ex_flag, &cpn_pv);
        ...
        if (j0 < nfix[i]) {
          ...
          err = cts_init_cpn(
              mkt, cpn_start_date[i], cpn_end_date[i], cpn_pay_date[i],
              cpn_coupon_temp, cpn_basis[i], cpn_not_ts[i], fix_lag_bd,
              cpntype_temp, pay_fix_date[i], pay_months, pay_freq, pay_basis,
              pay_fwd_spread[i], pay_gearing[i], nfix[i] - j0,
              &(weights[i][j0]), &(fix_dates[i][j0]), fix_tenor[i], fix_freq,
              fix_basis, &(fix_fwd_spreads[i][j0]), tstype, alpha[i], beta[i],
              nstr[i], str[i], nbopt[i], iv_buy_sell, value_zero, lb[i], ub[i],
              payoff[i], iv_call_spread, iv_call_spread, iv_trim_type,
              iv_max_fix, iv_min_fix_time, 1, use_cmsopt, correl_start,
              correl_end, float_adjust_type, cpn2);
    ...
    if (for_fund) {
      ...
      if (cts_on_iv->exo_leg->num_cpn > 0) {
        exo_leg_pv +=
            swp_f_df(today,
                     cts_on_iv->exo_leg->cpn[cts_on_iv->exo_leg->num_cpn - 1]
                         .cxxpn_pay_date,
                     yc) *
            cts_on_iv->exo_leg->cpn[cts_on_iv->exo_leg->num_cpn - 1].cxxpn_not;
      } else {
        fin_not_date = fund_pay[fund_ncpn - 1];
      ...
      /*	Initial */
      if (start_date >= today + eod_pay_flag) {
        exo_leg_pv -= swp_f_df(today, start_date, yc) *
                      cts_on_iv->exo_leg->cpn[0].cxxpn_not;
      }
    }
  } else {
    ...
    if (i < ncall) {
      ...
      while (j < cts_on_iv->exo_leg->num_cpn &&
             cts_on_iv->exo_leg->cpn[j].cxxpn_start_date < ex_date[i]) {
        j++;
      }
  ...
  if (ncall > 0) {
    if (iv_buy_sell != call_buy_sell && calc_fwd_iv) {
      err = cts_fill_check_all_struct(
          mkt, tstar, use_calib, lambda, nb_factor, lgm_alpha, lgm_gamma,
          lgm_rho, nsmilepar, smilepartime, alphaeps, rhoeps, ldaeps, rho2eps,
          lgmsvund, fund_ncpn, fund_not_ts2, fund_fix, fund_start, fund_end,
          fund_pay, fund_basis, fund_spr2, fund_mrg2, fix_lag_bd, ncpn,
          cpn_type, cpn_start_date, cpn_end_date, cpn_pay_date, cpn_coupon,
          cpn_basis, cpn_not_ts, pay_fix_date, pay_months, pay_freq, pay_basis,
          pay_fwd_spread, pay_gearing, nfix, weights, fix_dates, fix_tenor,
          fix_freq, fix_basis, fix_fwd_spreads, tstype, alpha, beta, nstr, str,
          nbopt, call_buy_sell, value_zero, lb, ub, payoff, iv_call_spread,
          iv_call_spread, iv_trim_type, iv_max_fix, iv_min_fix_time, use_cmsopt,
          correl_start, correl_end, float_adjust_type, 0, pay_rec, ex_date,
          set_date, fee, adj_fee, do_one_time, one_time_index, 0, NumerParams,
          pde_or_mc, req_stp, req_stppsi, req_stpx, req_stpz, req_mintime,
          req_paths, integ_mintime, cal_tenor, cal_ref, cal_freq, cal_basis,
          ...
          max_fact, use_jumps, numer_tstar, prec, maxiter, keep_first,
          long_strike_flag, short_strike_flag, NULL, 1, calc_fwd_iv,
          eod_fix_flag, eod_ex_flag, cts_iv_adj, und_iv_adj, &call_feat_iv_adj,
          adi_arg, save_inst_data, inst_data, save_fwdiv, fwd_iv_info);
    ...
    err = cts_fill_check_all_struct(
        ...
        fund_basis, fund_spr2, fund_mrg2, fix_lag_bd, ncpn, cpn_type,
        cpn_start_date, cpn_end_date, cpn_pay_date, cpn_coupon, cpn_basis,
        cpn_not_ts, pay_fix_date, pay_months, pay_freq, pay_basis,
        pay_fwd_spread, pay_gearing, nfix, weights, fix_dates, fix_tenor,
        fix_freq, fix_basis, fix_fwd_spreads, tstype, alpha, beta, nstr, str,
        nbopt, call_buy_sell, value_zero, lb, ub, payoff, call_call_spread,
        numer_call_spread, call_trim_type, call_max_fix, call_min_fix_time,
        use_cmsopt, correl_start, correl_end, float_adjust_type, ncall, pay_rec,
        ex_date, set_date, fee, adj_fee, do_one_time, one_time_index, 0,
        NumerParams, pde_or_mc, req_stp, req_stppsi, req_stpx, req_stpz,
        req_mintime, req_paths, integ_mintime, cal_tenor, cal_ref, cal_freq,
        cal_basis, force_atm, max_std_long, max_std_short, vol_shift_long,
        vol_type_long, vol_shift_type_long, vol_shift_short, vol_type_short,
        vol_shift_type_short, lambda_shift, calib_strategy, fix_lambda,
        short_tenor, short_refrate, short_freq, short_basis, fix_smile,
        smile_calib_months, lgmsv_calib_params, min_time, skip_last, min_fact,
        max_fact, use_jumps, numer_tstar, prec, maxiter, keep_first,
        long_strike_flag, short_strike_flag, cts_for_iv, 0, calc_fwd_iv,
        eod_fix_flag, eod_ex_flag, cts_pricing, und, &call_feat, adi_arg,
        save_inst_data, inst_data, save_fwdiv, fwd_iv_info);
    ...
    err = cts_adjust_model_fwd_iv(und, cts_for_iv, cts_pricing, for_fund,
                                  calc_fwd_iv, adj_fee, pde_or_mc, save_fwdiv,
                                  fwd_iv_info);
  ...
  if (call_feat == 1) {
    err = cts_launch_algo(cts_pricing, und, adi_arg, &call, &(onetime[0]),
                          &fwd_iv);
  ...
  if ((exercised) && (fund_pay[fund_ncpn - 1] != ex_date_set)) {
     ...
     * paid at ex_settle_date */
    if (for_fund) {
      exo_leg_pv += (swp_f_df(today, ex_date_set, yc) -
                     swp_f_df(today, fund_pay[fund_ncpn - 1], yc)) *
                    cpn_not_ts[ncpn - 1];
    }
  ...
  *call_val = call;
  ...
  if (export_und && call_feat == 1) {
    ...
    ConvertTS_LGMSV_to_LGM(und_exp->model.iNbPWTime, und_exp->model.dPWTime,
                           und_exp->model.dSigma, und_exp->model.dLambdaX,
  ...
  if (compute_reserve && call_feat == 1) {
    err = cts_calc_reserve(
        cts_for_iv, cts_pricing, und, adi_arg, for_fund, call, &(onetime[0]),
        ...
        short_strike_flag, calc_fwd_iv, adj_fee, lambda_reserve, lgm_alpha,
        lgm_gamma, lgm_rho, one_time_vega, vol_shift_type_long, reserve_method,
        lgm_reserve, midat_reserve, recalib_european, euro_nb_iter,
        one_time_index, recalc_one_factor, lgm_nstpt, lgm_nstpx, req_stppsi,
        req_stpz, integ_mintime, switch_reserve, mostexp_reserve, fwd_iv_info,
        save_extra_infos, extra_infos);

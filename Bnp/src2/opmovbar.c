...
*                                                                           
* DESCRIPTION  	: calculate the price for an option with the spot staying above 
*				  (or below) bar1 from T=0 to mat1 then above (or below) bar2 from mat1 to mat2.                                                        
*                                                                           
* CALLS			: SELF
*                                                                           
...
double answer;
	...
	answer =	pow(exp(1),(-mu01 + pow(sigma01,2))*mat1)
				*(bar2*norm((2*mu12*mat1 - pow(sigma12,2)*mat1 - 
				2*mu12*mat2 + pow(sigma12,2)*mat2 + 2*log(bar2) - 
				2*log(x))/(2*sigma12*sqrt(-mat1 + mat2))) - 
				pow(bar2/x,2*mu12/pow(sigma12,2))*x*
				norm((2*mu12*mat1 - pow(sigma12,2)*mat1 - 
				2*mu12*mat2 + pow(sigma12,2)*mat2 - 2*log(bar2) + 2*log(x))
				/(2*sigma12*sqrt(-mat1 + mat2))))*
				(-(pow(bar1,2*mu01/pow(sigma01,2))
				*pow(spot,3)*gauss((2*mu01*mat1 - 
				3*pow(sigma01,2)*mat1 + 
				4*log(bar1) - 2*log(spot) - 2*log(x))/
				(2*sigma01*sqrt(mat1)))) 
				+ pow(bar1,3)*pow(spot,2*mu01/pow(sigma01,2))*
				gauss((2*mu01*mat1 - 
				3*pow(sigma01,2)*mat1 + 2*log(spot) 
				- 2*log(x))/(2*sigma01*sqrt(mat1))))/
...
double answer;
	...
	answer =	pow(exp(1),(-mu01 + pow(sigma01,2))*mat1)
				*(pow(bar2*x,2*mu12/pow(sigma12,2))/x*
				...
				gauss((2*mu01*mat1 - 3*pow(sigma01,2)*mat1 + 
				4*log(bar1) - 2*log(spot) + 2*log(x))/
				(2*sigma01*sqrt(mat1)))) +
				pow(bar1,3)*pow(spot,2*mu01/pow(sigma01,2))*
				gauss((2*mu01*mat1 - 3*pow(sigma01,2)*mat1 
				+ 2*log(spot) + 2*log(x))/(2*sigma01*sqrt(mat1))))/
				(pow(bar1,3)*bar2*sigma01*
				pow(spot,(2*mu01 + pow(sigma01,2))/pow(sigma01,2))*sqrt(mat1));
...
/* PRICING function */
double srt_f_optmovbar (double spot, double fwd1, double fwd2, double bar1, double bar2,
						double sigma1, double sigma2, double mat1, double mat2, 
						double disc_fact, SrtBarrierType below_above, SrtGreekType greek)
{
...
SrtMinmaxType min_max;
	...
	else
	{
		...
		sigma01 = sigma1;
		sigma12 = sqrt((sigma2*sigma2*mat2 - sigma1*sigma1*mat1)/(mat2-mat1));

		if (below_above == SRT_DOWN)
		{
			premium = sm_qsimp_list(&prob_inf_bar2, EPS, bar1-EPS,
									spot, mat1, mat2, bar1, bar2, mu01, mu12, sigma01, sigma12);
		}
		else if (below_above == SRT_UP)
		{
			premium = sm_qsimp_list(&prob_sup_bar2, EPS, 1/(bar1+EPS),
									spot, mat1, mat2, bar1, bar2, mu01, mu12, sigma01, sigma12);
		}

 ...
 *				  (or below) bar1 from T=0 to mat1 then above (or below) bar2 from mat1 to
 *mat2.
 *
 * CALLS			: SELF
 *
...
static double prob_inf_bar2(double x, va_list argptr)
{
    ...
    answer = pow(exp(1), (-mu01 + pow(sigma01, 2)) * mat1) *
             (bar2 * norm(
                         (2 * mu12 * mat1 - pow(sigma12, 2) * mat1 - 2 * mu12 * mat2 +
             ...
             (-(pow(bar1, 2 * mu01 / pow(sigma01, 2)) * pow(spot, 3) *
                gauss(
                    (2 * mu01 * mat1 - 3 * pow(sigma01, 2) * mat1 + 4 * log(bar1) - 2 * log(spot) -
                     2 * log(x)) /
                    (2 * sigma01 * sqrt(mat1)))) +
              pow(bar1, 3) * pow(spot, 2 * mu01 / pow(sigma01, 2)) *
                  gauss(
                      (2 * mu01 * mat1 - 3 * pow(sigma01, 2) * mat1 + 2 * log(spot) - 2 * log(x)) /
                      (2 * sigma01 * sqrt(mat1)))) /
    ...
    SrtGreekType   greek)
{
    ...
    else if (mat1 == 0)
    {
        ...
        premium =
            srt_f_optamedig(fwd2, spot, bar2, sigma2, mat2, disc_fact, below_above, min_max, greek);
    }
    else
    {

	...
	for (i = -n;i<=n;i++)
 	{
   	...
   	price = price1 - 
    		exp(log(up/spot)*(2*mu1/vol/vol)) *
		exp(2*mu1*t1*(vol0*vol0/vol/vol*mu1-mu0)/vol/vol)*
    		price2;
...
double fct_gold_stay_inside_fwd(double x, va_list argptr)
{
	...
	result = - stay_inside_fwd_fct( spot, fwd1,fwd2, x+gap, x, t1,t2,
			vol0,vol, n, measure_flag);
...
-------------------------------------------------------------------------------------------------------------------------------- */
double prob_ExpectedDays_BrownianBridge( double x1, double x0, double dBup, double dBdown, double dVol, double dStartTime, double dLength,
								int nStep );
double prob_ExpectedDays_NoSmile( double x1, double x0, double dBup, double dBdown, double dVol, double dStartTime, double dLength, 
								int nStep, int Flag );
double prob_ExpectedDays_LinearSmile( double dDrift, double dSpot, double* dBarrier, double dATMVol, double* dSlope, 
...
{
	if ( BBflag==0 )
		return prob_ExpectedDays_BrownianBridge(x1, x0, dBarrier[0], dBarrier[1], dVol, dStartTime, dLength, nStep );
	else if ( BBflag==1 )
		return prob_ExpectedDays_NoSmile(x1, x0, dBarrier[0], dBarrier[1], dVol, dStartTime, dLength, nStep, BBflag );
	else if ( BBflag==2 )
	{	
		return prob_ExpectedDays_LinearSmile( x1, x0, dBarrier, dVol, dParameters, dStartTime, dLength, nStep );
	}
	else
		return -1.0;
...
/* -------------------------------------------------------------------------------------------------------------------------------- 
	...
	Calculates the expected number of days within a corridor using a Brownian Bridge
-------------------------------------------------------------------------------------------------------------------------------- */
double prob_ExpectedDays_BrownianBridge( double x1, double x0, double dBup, double dBdown, double dVol, double dStartTime, double dLength,
								int nStep )
{

	double dLogBup, dLogBdn, dLogF, dLogS, dStepSize, dExpectDays, dTime, dTimeRatio, dTotalTime;
	int i;
	...
	for ( ; i < nStep; i++ )
	{
		...
		dExpectDays += norm( (dLogBup - (1.0-dTimeRatio)*dLogS - dTimeRatio*dLogF)/dVol/sqrt(dTime*(1-dTimeRatio)) ) -  
							norm( (dLogBdn - (1.0-dTimeRatio)*dLogS - dTimeRatio*dLogF)/dVol/sqrt(dTime*(1-dTimeRatio)) );
	}
...
/* -------------------------------------------------------------------------------------------------------------------------------- 
	...
	Calculates the expected number of days within a corridor using call spreads
-------------------------------------------------------------------------------------------------------------------------------- */
...
/* -------------------------------------------------------------------------------------------------------------------------------- 
	...
	Calculates the expected number of days within a corridor using call spreads with a simple linear smile model
	The 0 value is the up and the 1 value is down
-------------------------------------------------------------------------------------------------------------------------------- */
							   ...
							   double dStartTime, double dLength, int nStep )
{
	...
	double dStepSize, dExpectDays, dTime, dStdDev, dLog[2], dCallSpread[2], dVol[2], dHplusTerm[2], dVegaTerm[2];
	...
	for ( ; i < nStep; i++ )
	{
		...
		for ( j=0; j<2; j++ )
		{
			...
			dCallSpread[j] = -norm( dHminus ) + dVegaTerm[j] * exp( dDriftTerm - 0.5*dHplus*dHplus );
		}

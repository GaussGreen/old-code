...
Err cpd_fill_fund_leg(
  ...
  while (i < fund_ncpn) {
    ...
    /*	Times */
    fund_leg->cpn[j].start_time =
        (fund_leg->cpn[j].start_date - today) * YEARS_IN_DAY;
    fund_leg->cpn[j].pay_time =
        (fund_leg->cpn[j].pay_date - today) * YEARS_IN_DAY;
    ...
    fund_leg->cpn[j].cxxpn = fund_not * cvg * (fund_spr[i] + fund_mrg[i]);
...
Err cpd_fill_exo_leg(
    ...
    //		pd interp coupon:
    int use_cpn_opt_str, int *pd_num_strikes, double *pd_wcst, double *pd_wspot,
    double **pd_strikes, double **pd_weights,
    ...
    //		pd interp notional:
    int use_not_opt_str, int pd_not_num_strikes, double pd_not_wcst,
  ...
  while (i < pd_ncpn) {
    ...
    /*	Times */
    exo_leg->cpn[j].start_time =
        (exo_leg->cpn[j].start_date - today) * YEARS_IN_DAY;
    exo_leg->cpn[j].pay_time =
        (exo_leg->cpn[j].pay_date - today) * YEARS_IN_DAY;
    exo_leg->cpn[j].fx_fix_time =
        (exo_leg->cpn[j].fx_fix_date - today) * YEARS_IN_DAY;
    exo_leg->cpn[j].fx_val_time =
        (exo_leg->cpn[j].fx_val_date - today) * YEARS_IN_DAY;
    ...
    if (!use_cpn_opt_str) {
      ...
      exo_leg->cpn[j].cxxapped = pd_capped[i];

      exo_leg->cpn[j].floor = cvg * pd_floor[i];
      exo_leg->cpn[j].cxxap = cvg * pd_cap[i];
    } else {
      // Interp coupon:
      ...
      if (pd_num_strikes[i] > 0) {
        exo_leg->cpn[j].strikes = calloc(pd_num_strikes[i], sizeof(double));
        exo_leg->cpn[j].weights = calloc(pd_num_strikes[i], sizeof(double));
        if (!exo_leg->cpn[j].strikes || !exo_leg->cpn[j].weights) {
          err = serror("Memory failure in cpd_fill_exo_leg");
          goto FREE_RETURN;
        }
        memcpy(exo_leg->cpn[j].strikes, pd_strikes[i],
...
DO_NOTIONAL_REF:
  ...
  /*	Times */
  exo_leg->not_ref.start_time =
      (exo_leg->not_ref.start_date - today) * YEARS_IN_DAY;
  exo_leg->not_ref.pay_time =
      (exo_leg->not_ref.pay_date - today) * YEARS_IN_DAY;
  exo_leg->not_ref.fx_fix_time =
      (exo_leg->not_ref.fx_fix_date - today) * YEARS_IN_DAY;
  exo_leg->not_ref.fx_val_time =
      (exo_leg->not_ref.fx_val_date - today) * YEARS_IN_DAY;
  ...
  if (!use_not_opt_str) {
    ...
    exo_leg->not_ref.cxxapped = pd_not_ref_capped;

    exo_leg->not_ref.floor = pd_not * pd_not_ref_floor;
    exo_leg->not_ref.cxxap = pd_not * pd_not_ref_cap;
  } else
  // Interp redemption:
  {
    ...
    if (pd_not_num_strikes > 0) {
      exo_leg->not_ref.strikes = calloc(pd_not_num_strikes, sizeof(double));
      exo_leg->not_ref.weights = calloc(pd_not_num_strikes, sizeof(double));
      if (!exo_leg->not_ref.strikes || !exo_leg->not_ref.weights) {
        err = serror("Memory failure in cpd_not_fill_exo_leg");
        goto FREE_RETURN;
      }
      memcpy(exo_leg->not_ref.strikes, pd_not_strikes,
             pd_not_num_strikes * sizeof(double));
...
Err cpd_check_exo_leg(PD_EXO_LEG exo_leg) {
  ...
  for (i = 0; i < exo_leg->num_cpn; i++) {
    if (!exo_leg->cpn[i].use_opt_str) {
      if (exo_leg->cpn[i].floored && exo_leg->cpn[i].cxxapped &&
          exo_leg->cpn[i].floor > exo_leg->cpn[i].cxxap)
  ...
  if (exo_leg->not_ref.pay_date < exo_leg->not_ref.start_date)
    return "Pay dates should be after start dates in exotic leg notional "
           "refund";

  if (exo_leg->not_ref.pay_date < exo_leg->not_ref.fx_fix_date)
    return "Pay dates should be after fixing dates in exotic leg notional "
           "refund";

  if (exo_leg->not_ref.fx_val_date < exo_leg->not_ref.fx_fix_date)
    return "Value dates should be after fixing dates in exotic leg notional "
           "refund";

  if (!exo_leg->not_ref.use_opt_str) {
    if (exo_leg->not_ref.floored && exo_leg->not_ref.cxxapped &&
        exo_leg->not_ref.floor > exo_leg->not_ref.cxxap)
      return "Floor should be lower than cap";
  }

  /*	Check that notional refund starts        , fixes and pays after the last
   * coupon */
  if (exo_leg->num_cpn > 0) {
    if (exo_leg->not_ref.start_date <
        exo_leg->cpn[exo_leg->num_cpn - 1].start_date)
    ...
    if (exo_leg->not_ref.fx_fix_date <
        exo_leg->cpn[exo_leg->num_cpn - 1].fx_fix_date)
      return "Notional refund should fix after last coupon in exotic leg";

    if (exo_leg->not_ref.fx_val_date <
        exo_leg->cpn[exo_leg->num_cpn - 1].fx_val_date)
      return "Notional refund value date should be after last coupon value "
             "date in exotic leg";
  }
...
Err cpd_fill_exo_and_fund_leg_TARN(long today, CPD_STR cpd) {
  ...
  pd_leg->cpn =
      (pd_exo_cpn *)calloc(cpd->pd_leg->num_cpn + 1, sizeof(pd_exo_cpn));
    ...
    if (cpd->pd_leg->cpn[j].use_opt_str) {
      ...
      if (pd_leg->cpn[j].nstrikes > 0) {
        pd_leg->cpn[j].strikes =
            calloc(pd_leg->cpn[j].nstrikes, sizeof(double));
        ...
        memcpy(pd_leg->cpn[j].strikes, cpd->pd_leg->cpn[j].strikes,
               pd_leg->cpn[j].nstrikes * sizeof(double));
        memcpy(pd_leg->cpn[j].weights, cpd->pd_leg->cpn[j].weights,
               pd_leg->cpn[j].nstrikes * sizeof(double));
      }
    }
  }
  ...
  pd_leg->cpn[cpd->pd_leg->num_cpn].fx_fix_date =
      cpd->pd_leg->cpn[cpd->pd_leg->num_cpn - 1].pay_date;
  pd_leg->cpn[cpd->pd_leg->num_cpn].fx_val_date =
      add_unit(cpd->pd_leg->cpn[cpd->pd_leg->num_cpn - 1].pay_date, 2, SRT_BDAY,
               MODIFIED_SUCCEEDING);
  ...
  pd_leg->cpn[cpd->pd_leg->num_cpn].fx_val_time =
      (pd_leg->cpn[cpd->pd_leg->num_cpn - 1].fx_val_date - today) *
      YEARS_IN_DAY;
  ...
  if (!cpd->pd_leg->cpn[cpd->pd_leg->num_cpn - 1].use_opt_str) {
    ...
    pd_leg->cpn[cpd->pd_leg->num_cpn].cxxapped = 0;

    pd_leg->cpn[cpd->pd_leg->num_cpn].floor = 0.0;
    pd_leg->cpn[cpd->pd_leg->num_cpn].cxxap = 0.0;
  } else {
  ...
  if (cpd->pd_leg->not_ref.use_opt_str) {
    ...
    if (pd_leg->not_ref.nstrikes > 0) {
      pd_leg->not_ref.strikes =
          calloc(pd_leg->not_ref.nstrikes, sizeof(double));
      ...
      memcpy(pd_leg->not_ref.strikes, cpd->pd_leg->not_ref.strikes,
             pd_leg->not_ref.nstrikes * sizeof(double));
      memcpy(pd_leg->not_ref.weights, cpd->pd_leg->not_ref.weights,
             pd_leg->not_ref.nstrikes * sizeof(double));
    }
  }
  ...
  fund_leg->cpn =
      (pd_fund_cpn *)calloc(cpd->fund_leg->num_cpn + 1, sizeof(pd_fund_cpn));
  ...
  fund_leg->cpn[fund_leg->num_cpn].pay_date =
      cpd->fund_leg->cpn[fund_leg->num_cpn - 1].pay_date;

  /*	Times */
  fund_leg->cpn[fund_leg->num_cpn].start_time =
      cpd->fund_leg->cpn[fund_leg->num_cpn - 1].pay_time;
  fund_leg->cpn[fund_leg->num_cpn].pay_time =
      cpd->fund_leg->cpn[fund_leg->num_cpn - 1].pay_time;

  /*	Coupon */
  fund_leg->cpn[fund_leg->num_cpn].cxxpn = 0.0;
...
Err cpd_fill_calls(
    ...
    int TARN_Do, double smooth, /*	Smoothing factor */
    int fx_bound, int use_bound, int force_optim, CPD_STR cpd) {
  int i, j, k;
  Err err = NULL;
  ...
  while (i < ncall) {
    ...
    /*	k = index of the first coupon to be called on exotic leg        ,
                    i.e. first coupon with a start date >= ex date */
    k = 0;
    while (k < cpd->pd_leg->num_cpn &&
           cpd->pd_leg->cpn[k].start_date < cpd->call[j].ex_date)
      k++;
    ...
    /*	KO */
    {
      ...
      if (use_bound) {
         ...
         */
        cpd->call[j].cxxall_type = 1; /* turn to call */
        cpd->call[j].bar_type = 0;    /* turn to UO */
        cpd->call[j].barrier = barrier[i];
        cpd->call[j].orig_barrier = barrier[i];
      } else {
        cpd->call[j].cxxall_type = 0;
      }
    }
    ...
    if (use_not_opt_str) {
      ...
      if (pd_not_num_strikes[i] > 0) {
        cpd->call[j].strikes = calloc(pd_not_num_strikes[i], sizeof(double));
        cpd->call[j].weights = calloc(pd_not_num_strikes[i], sizeof(double));
        if (!cpd->call[j].strikes || !cpd->call[j].weights) {
          err = serror("Memory failure in cpd_fill_calls");
          goto FREE_RETURN;
        }
        memcpy(cpd->call[j].strikes, pd_not_strikes[i],
...
Err cpd_check_calls(CPD_STR cpd) {
  ...
  /*	Check that set dates are after ex dates
                  Also check that the call date is before the start        , end
     and fixing dates of the coupons it controls */
  for (i = 0; i < cpd->num_calls; i++) {
    if (cpd->call[i].set_date < cpd->call[i].ex_date)
      return "Settlement dates should be after exercise dates";

    if (cpd->call[i].use_opt_str &&
        cpd->call[i].fx_fix_date < cpd->call[i].ex_date)
      return serror(
          "Fx fixing date cannot be before the corresponding exercise date");

    if (cpd->fund_leg->cpn[cpd->call[i].fund_idx].start_date <
            cpd->call[i].ex_date ||
        cpd->fund_leg->cpn[cpd->call[i].fund_idx].pay_date <
            cpd->call[i].ex_date)
      return "A funding coupon starts before its exercise date";

    if (cpd->pd_leg->cpn[cpd->call[i].pd_idx].start_date <
            cpd->call[i].ex_date ||
        cpd->pd_leg->cpn[cpd->call[i].pd_idx].pay_date < cpd->call[i].ex_date ||
        cpd->pd_leg->cpn[cpd->call[i].pd_idx].fx_fix_date <
            cpd->call[i].ex_date)
  ...
  if (cpd->num_calls > 0) {
    if (cpd->call[cpd->num_calls - 1].ex_date >
            cpd->pd_leg->not_ref.start_date ||
        cpd->call[cpd->num_calls - 1].ex_date > cpd->pd_leg->not_ref.pay_date ||
        cpd->call[cpd->num_calls - 1].ex_date >
            cpd->pd_leg->not_ref.fx_fix_date)
      return serror(
          "Last call date should be before the exotic notional refund date");
  }
  /*	OK */
  return NULL;
}
...
Err cpd_fill_und(char *fx3dund, CPD_UND und, CPDBETADLMPARAMS cpd_dlm_params,
                 double dom_vol_shift, double for_vol_shift,
                 double fx_vol_shift) {
  double *sig_time_dom = NULL, *sig_dom = NULL, *tau_time_dom = NULL,
         *tau_dom = NULL, *sig_time_for = NULL, *sig_for = NULL,
         *tau_time_for = NULL, *tau_for = NULL;
  ...
  if (cpd_dlm_params && cpd_dlm_params->use_beta_dlm) {
    ...
    und->model = calloc(1, sizeof(FxBetaDLM_model));
    und->hermite = calloc(1, sizeof(FxBetaDLM_Hermite));
    ...
    und->sigma_time_fx = calloc(und->sigma_n_fx, sizeof(double));
    ...
    memcpy(und->sigma_time_fx, und->model->str->time_fx,
           und->sigma_n_fx * sizeof(double));

    err = FxBetaDLM_GetEqui3FactorTS(und->model, und->num_params, und->hermite,
                                     und->sigma_n_fx, und->sigma_time_fx,
                                     &(und->sigma_fx));

    if (err)
      goto FREE_RETURN;

    und->model->dSigmaFx3F = calloc(und->model->iNbPWTime, sizeof(double));
    for (i = 0; i < und->model->iNbPWTime; i++) {
      index = Get_Index(und->model->dPWTime[i], und->sigma_time_fx,
                        und->sigma_n_fx);
      und->model->dSigmaFx3F[i] = und->sigma_time_fx[index];
    }

    err = merge_rates_ts(und->model->str->time_dom, und->model->str->sig_dom,
                         und->model->str->nb_dom, und->model->str->time_for,
                         und->model->str->sig_for, und->model->str->nb_for,
                         &(und->sigma_time_rates), &(und->sigma_dom),
                         &(und->sigma_for), &(und->sigma_n_rates));

    if (err)
      goto FREE_RETURN;

    und->sigma_date_rates = calloc(und->sigma_n_rates, sizeof(double));
    und->sigma_date_fx = calloc(und->sigma_n_fx, sizeof(double));

    und->corr_n_times = und->model->str->nb_3F_corr;
    und->corr_times = calloc(und->model->str->nb_3F_corr, sizeof(double));
    und->correl_dom_for = calloc(und->model->str->nb_3F_corr, sizeof(double));
    und->correl_dom_fx = calloc(und->model->str->nb_3F_corr, sizeof(double));
    und->correl_for_fx = calloc(und->model->str->nb_3F_corr, sizeof(double));
    ...
    memcpy(und->corr_times, und->model->str->time_3F_corr,
           und->model->str->nb_3F_corr * sizeof(double));
    memcpy(und->correl_dom_for, und->model->str->dom_for_3F_corr,
           und->model->str->nb_3F_corr * sizeof(double));
    memcpy(und->correl_dom_fx, und->model->str->dom_fx_3F_corr,
           und->model->str->nb_3F_corr * sizeof(double));
    memcpy(und->correl_for_fx, und->model->str->for_fx_3F_corr,
           und->model->str->nb_3F_corr * sizeof(double));

    for (i = 0; i < und->sigma_n_rates; i++) {
      und->sigma_date_rates[i] =
          und->today + und->sigma_time_rates[i] * DAYS_IN_YEAR;
    }
    ...
    err = Get_FX_StochRate_TermStructures_corr(
        fx3dund, &sig_time_dom, &sig_dom, &sig_n_dom, &tau_time_dom, &tau_dom,
        &tau_n_dom, &sig_time_for, &sig_for, &sig_n_for, &tau_time_for,
        &tau_for, &tau_n_for, &(und->sigma_time_fx), &(und->sigma_fx),
    ...
    err =
        merge_rates_ts(sig_time_dom, sig_dom, sig_n_dom, sig_time_for, sig_for,
                       sig_n_for, &(und->sigma_time_rates), &(und->sigma_dom),
                       &(und->sigma_for), &(und->sigma_n_rates));
    ...
    und->sigma_date_rates =
        (double *)calloc(und->sigma_n_rates, sizeof(double));
    ...
    for (i = 0; i < und->sigma_n_fx; i++) {
      und->sigma_date_fx[i] =
          und->today + und->sigma_time_fx[i] * DAYS_IN_YEAR + 1.0e-08;
    }
...
Err cpd_calib_und(
    ...
    CPDBETADLMPARAMS cpd_dlm_params, CPD_STR cpd, /*	Structure */
    ...
    long *fx_mkt_vol_date, double *fx_mkt_vol, int num_fx_mkt_vol, CPD_UND und,
  ...
  und->spot_fx = fx_spot * swp_f_df(today, fx_spot_date, dom_yc) /
                 swp_f_df(today, fx_spot_date, for_yc);

  und->corr_n_times = corr_n_times;
  und->corr_times = calloc(corr_n_times, sizeof(double));
  und->correl_dom_for = calloc(corr_n_times, sizeof(double));
  und->correl_dom_fx = calloc(corr_n_times, sizeof(double));
  und->correl_for_fx = calloc(corr_n_times, sizeof(double));

  if (!und->corr_times || !und->correl_dom_for || !und->correl_dom_fx ||
      !und->correl_for_fx) {
  ...
  if (dom_calib || for_calib) {
    ...
    i0 = 0;
    while (i0 < cpd->num_calls &&
           (cpd->call[i0].ex_date - today) < CPD_MINCALDAYS)
      i0++;

    if (cpd->num_calls - i0 > 0 &&
        !(cpd->num_calls - i0 == 1 &&
          cpd->call[i0].ex_date <= today + eod_flag)) {
  ...
  if (dom_calib) {
    ...
    err = cpd_calib_diagonal(
        ...
        CALPRES, CALPRES, min_fact, max_fact, use_jumps, 0, NULL, &dom_lam, 1,
        0.0, 0.0, 0.0, &temp_sig_n1, &temp_sig_time1, &temp_sig1, NULL);

    if (err &&
        cpd->pd_leg->cpn[cpd->pd_leg->num_cpn - 1].pay_date < today + 740) {
      err = cpd_calib_diagonal(
          dom_yc, dom_vc, dom_ref, get_ir_cash_vol, dom_vol_shift, 0, nex, lex,
          last, NULL, NULL, 0, 1.0, 1.0, "Q", dom_swap_basis, 1, 0, 1, CALPRES,
          CALPRES, min_fact, max_fact, use_jumps, 0, NULL, &dom_lam, 1, 0.0,
          0.0, 0.0, &temp_sig_n1, &temp_sig_time1, &temp_sig1, NULL);

      if (err &&
          strcmp(err, "All exercise dates are past in cpd_calib_diagonal")) {
        temp_sig_n1 = 1;
  ...
  if (for_calib) {
    ...
    err = cpd_calib_diagonal(
        ...
        CALPRES, CALPRES, min_fact, max_fact, use_jumps, 0, NULL, &for_lam, 1,
        0.0, 0.0, 0.0, &temp_sig_n2, &temp_sig_time2, &temp_sig2, NULL);

    if (err &&
        cpd->pd_leg->cpn[cpd->pd_leg->num_cpn - 1].pay_date < today + 740) {
      err = cpd_calib_diagonal(
          for_yc, for_vc, for_ref, get_ir_cash_vol, for_vol_shift, 0, nex, lex,
          last, NULL, NULL, 0, 1.0, 1.0, "Q", for_swap_basis, 1, 0, 1, CALPRES,
          CALPRES, min_fact, max_fact, use_jumps, 0, NULL, &for_lam, 1, 0.0,
          0.0, 0.0, &temp_sig_n2, &temp_sig_time2, &temp_sig2, NULL);
  ...
  err = merge_rates_ts(temp_sig_time1, temp_sig1, temp_sig_n1, temp_sig_time2,
                       temp_sig2, temp_sig_n2, &(und->sigma_time_rates),
                       &(und->sigma_dom), &(und->sigma_for),
                       &(und->sigma_n_rates));
  ...
  for (i = 0; i < und->sigma_n_rates; i++)
    und->sigma_date_rates[i] =
        today + und->sigma_time_rates[i] * DAYS_IN_YEAR + 1.0e-16;
  ...
  if (i < und->corr_n_times - 1 &&
      und->corr_times[i] < (last - today) * YEARS_IN_DAY)
    i++;
  ...
  if (!fx_mkt_vol_time || !fx_mkt_fix_date || !und->sigma_date_fx ||
      !und->sigma_time_fx) {
    err = "Allocation error (2) in cpd_calib_und";
    goto FREE_RETURN;
  }
  ...
  err = Fx3DtsCalibration_corr(
      und->sigma_time_fx, fx_mkt_vol_time, fx_mkt_vol, num_fx_mkt_vol,
      und->sigma_time_rates, und->sigma_n_rates, und->sigma_dom, und->lda_dom,
      und->sigma_for, und->lda_for, und->corr_times, und->correl_dom_for,
      und->correl_dom_fx, und->correl_for_fx, und->corr_n_times,
      &(und->sigma_fx));
  ...
  if (cpd_dlm_params->use_beta_dlm) {
    ...
    und->model = calloc(1, sizeof(FxBetaDLM_model));
    und->hermite = calloc(1, sizeof(FxBetaDLM_Hermite));
    opt_strikes = calloc(num_fx_mkt_vol, sizeof(double));
    ...
    err = init_FxBetaDLM_model(
        ...
        today, dom_yc, 1.0 / und->lda_dom, und->sigma_n_rates,
        und->sigma_time_rates, und->sigma_dom, for_yc, 1.0 / und->lda_for,
        und->sigma_n_rates, und->sigma_time_rates, und->sigma_for, fx_spot,
        cpd_dlm_params->tstar, cpd_dlm_params->B0, cpd_dlm_params->C0,
        cpd_dlm_params->alpha, cpd_dlm_params->lambda, num_fx_mkt_vol,
        und->sigma_time_fx, und->sigma_fx, und->sigma_fx, und->corr_n_times,
        und->corr_times, und->correl_dom_for, und->correl_dom_fx,
        und->correl_for_fx, und->model);
    ...
    err = FxBetaDLM_CalibrationModel(
        num_fx_mkt_vol, fx_mkt_vol_date, fx_mkt_fix_date, opt_strikes,
        fx_mkt_vol, cpd_dlm_params->calib_smile,
        cpd_dlm_params->smile_settlmt_date, cpd_dlm_params->smile_strikes,
        cpd_dlm_params->smile_bs_vols, und->model, und->num_params,
        und->hermite);
...
void cpd_copy_und(CPD_UND src, CPD_UND dest) {
  ...
  dest->sigma_date_rates =
      (double *)calloc(dest->sigma_n_rates, sizeof(double));
  memcpy(dest->sigma_date_rates, src->sigma_date_rates,
         dest->sigma_n_rates * sizeof(double));
  dest->sigma_time_rates =
      (double *)calloc(dest->sigma_n_rates, sizeof(double));
  memcpy(dest->sigma_time_rates, src->sigma_time_rates,
         dest->sigma_n_rates * sizeof(double));
  ...
  memcpy(dest->sigma_date_fx, src->sigma_date_fx,
         dest->sigma_n_fx * sizeof(double));
  dest->sigma_time_fx = (double *)calloc(dest->sigma_n_fx, sizeof(double));
  memcpy(dest->sigma_time_fx, src->sigma_time_fx,
         dest->sigma_n_fx * sizeof(double));
  ...
  memcpy(dest->corr_times, src->corr_times,
         dest->corr_n_times * sizeof(double));
  dest->correl_dom_for = (double *)calloc(dest->corr_n_times, sizeof(double));
  memcpy(dest->correl_dom_for, src->correl_dom_for,
         dest->corr_n_times * sizeof(double));
  dest->correl_dom_fx = (double *)calloc(dest->corr_n_times, sizeof(double));
  memcpy(dest->correl_dom_fx, src->correl_dom_fx,
         dest->corr_n_times * sizeof(double));
  dest->correl_for_fx = (double *)calloc(dest->corr_n_times, sizeof(double));
  memcpy(dest->correl_for_fx, src->correl_for_fx,
         dest->corr_n_times * sizeof(double));

  if (src->use_beta_dlm) {
    dest->use_beta_dlm = 1;
    dest->model = calloc(1, sizeof(FxBetaDLM_model));
    dest->hermite = calloc(1, sizeof(FxBetaDLM_Hermite));
  ...
  if (src->nb_fees > 0 && src->fees && src->fees_dates) {
    ...
    dest->fees = calloc(dest->nb_fees, sizeof(double));
    dest->fees_dates = calloc(dest->nb_fees, sizeof(double));
...
Err cpd_AlphaFudgeUpdateUnd(CPD_UND und, int UpOrDown) /* Up = 0 and Down = 1 */
{
  ...
  err = FxBetaDLM_GetEqui3FactorTS(	und->model        ,
                                                                          und->num_params
        , und->hermite        , und->sigma_n_fx        , und->sigma_time_fx ,
                                                                          &(und->sigma_fx));
  */
...
Err cpd_fill_eval_const(CPD_UND und, CPD_STR cpd,
                        /*	Index of the current call/KO */
                        int call_idx, CPD_EVAL_CONST eval_const) {
  ...
  dom_lam = und->lda_dom;
  for_lam = und->lda_for;
  err = Fx3DtsPhi(evt_time, und->sigma_time_rates, und->sigma_n_rates,
                  und->sigma_dom, und->lda_dom, &dom_phi);
  if (err) {
    goto FREE_RETURN;
  }
  err = Fx3DtsPhi(evt_time, und->sigma_time_rates, und->sigma_n_rates,
                  und->sigma_for, und->lda_for, &for_phi);
  if (err) {
    goto FREE_RETURN;
  }
  ...
  eval_const->fx_spot_date =
      add_unit(evt_date, 2, SRT_BDAY, MODIFIED_SUCCEEDING);
  ...
  eval_const->spot_fx_cvx =
      -0.5 * eval_const->spot_fx_for_gam * eval_const->spot_fx_for_gam *
          for_phi +
  ...
  if (call->num_fund_cpn > 0) {
    ...
    for (i = call->fund_idx; i < cpd->fund_leg->num_cpn; i++) {
      ...
      eval_const->fund_gam[i - call->fund_idx] =
          (1.0 - exp(-fund_lam * (cpd->fund_leg->cpn[i].pay_time - evt_time))) /
          fund_lam;
  ...
  eval_const->start_dff = swp_f_df(
      evt_date, cpd->fund_leg->cpn[call->fund_idx].start_date, fund_yc);
  eval_const->start_gam =
      (1.0 - exp(-fund_lam *
                 (cpd->fund_leg->cpn[call->fund_idx].start_time - evt_time))) /
      fund_lam;
  eval_const->start_gam2 =
      0.5 * eval_const->start_gam * eval_const->start_gam * fund_phi;
  ...
  if (call->num_pd_cpn > 0) {
    ...
    for (i = call->pd_idx; i < cpd->pd_leg->num_cpn; i++) {
      ...
      eval_const->pd_disc_gam[i - call->pd_idx] =
          (1.0 - exp(-dom_lam * (cpd->pd_leg->cpn[i].pay_time - evt_time))) /
          dom_lam;
  ...
  if (call->num_pd_cpn > 0) {
    ...
    for (i = call->pd_idx; i < cpd->pd_leg->num_cpn; i++) {
      ...
      err = Fx3DtsFwdPayAdjustment_corr(
          evt_time, cpn->fx_val_time, cpn->fx_val_time, cpn->pay_time,
      ...
      err = Fx3DtsImpliedVol_corr(
          cpn->fx_val_time, evt_time, cpn->fx_fix_time, und->sigma_time_rates,
      ...
      eval_const->pd_half_std[i - call->pd_idx] =
          0.5 * eval_const->pd_std[i - call->pd_idx];
      ...
      if (!cpn->use_opt_str) {
        ...
        if (cpn->capped && fabs(cpn->beta) > 1.0e-16 && cpn->cap > -1.0e-16) {
          eval_const->pd_cap_str[i - call->pd_idx] =
              (cpn->cap - cpn->alpha) / cpn->beta;
  ...
  eval_const->in_not_fund_gam2 = 0.5 * eval_const->in_not_fund_gam *
                                 eval_const->in_not_fund_gam * fund_phi;

  eval_const->in_not_pd_dff = swp_f_df(evt_date, call->set_date, dom_yc);
  eval_const->in_not_pd_gam =
      (1.0 - exp(-dom_lam * (call->set_time - evt_time))) / dom_lam;
  ...
  if (call->use_opt_str) {
    eval_const->in_not_pd_fwd_dff_ratio =
        swp_f_df(evt_date, call->fx_val_date, for_yc) /
    ...
    eval_const->in_not_pd_fwd_cvx =
        -0.5 * eval_const->in_not_pd_fwd_for_gam *
            eval_const->in_not_pd_fwd_for_gam * for_phi +
        0.5 * eval_const->in_not_pd_fwd_dom_gam *
            eval_const->in_not_pd_fwd_dom_gam * dom_phi;

    err = Fx3DtsFwdPayAdjustment_corr(
        evt_time, call->fx_val_time, call->fx_val_time, call->set_time,
        call->fx_fix_time, und->sigma_time_rates, und->sigma_n_rates,
        und->sigma_dom, und->lda_dom, und->sigma_for, und->lda_for,
        und->sigma_time_fx, und->sigma_fx, und->sigma_n_fx, und->corr_times,
        und->correl_dom_for, und->correl_dom_fx, und->correl_for_fx,
        und->corr_n_times, &temp);
    ...
    err = Fx3DtsImpliedVol_corr(
        call->fx_val_time, evt_time, call->fx_fix_time, und->sigma_time_rates,
        und->sigma_n_rates, und->sigma_dom, und->lda_dom, und->sigma_for,
        und->lda_for, und->sigma_time_fx, und->sigma_fx, und->sigma_n_fx,
        und->corr_times, und->correl_dom_for, und->correl_dom_fx,
        und->correl_for_fx, und->corr_n_times, &(eval_const->in_not_pd_std));
  ...
  if (call_idx < cpd->num_calls - 1) {
    eval_const->next_fund_dff =
        swp_f_df(evt_date, next_call->set_date, fund_yc);
    ...
    eval_const->next_start_fund_gam2 = 0.5 * eval_const->next_start_fund_gam *
                                       eval_const->next_start_fund_gam *
                                       fund_phi;
  }
  ...
  eval_const->fin_not_disc_gam2 = 0.5 * eval_const->fin_not_disc_gam *
                                  eval_const->fin_not_disc_gam * dom_phi;
  ...
  eval_const->fin_not_fwd_cvx = -0.5 * eval_const->fin_not_fwd_for_gam *
                                    eval_const->fin_not_fwd_for_gam * for_phi +
                                0.5 * eval_const->fin_not_fwd_dom_gam *
                                    eval_const->fin_not_fwd_dom_gam * dom_phi;
  ...
  err = Fx3DtsImpliedVol_corr(
      ...
      und->lda_for, und->sigma_time_fx, und->sigma_fx, und->sigma_n_fx,
      und->corr_times, und->correl_dom_for, und->correl_dom_fx,
      und->correl_for_fx, und->corr_n_times, &(eval_const->fin_not_std));
  ...
  if (!cpn->use_opt_str) {
    ...
    if (cpn->floored && fabs(cpn->beta) > 1.0e-16 && cpn->floor > -1.0e-16) {
      ...
      if (cpn->beta > 0.0) {
        if (eval_const->fin_not_std > 1.0e-16 &&
            eval_const->fin_not_floor_str > 1.0e-16)
    ...
    if (cpn->capped && fabs(cpn->beta) > 1.0e-16 && cpn->cap > -1.0e-16) {
      ...
      if (cpn->beta > 0.0) {
        if (eval_const->fin_not_std > 1.0e-16 &&
            eval_const->fin_not_cap_str > 1.0e-16)
...
Err cpd_fill_FxInstrument_dlm(CPD_UND und, CPD_STR cpd, PD_CALL call,
                              int index_cpn, PD_EXO_CPN cpn,
                              CPD_EVAL_CONST eval_const, int nb_strikes,
                              double *strikes) {
  ...
  if (und->num_params->iPrecalcSmile) {
    ...
    fwd_strike = calloc(1, sizeof(double));
    PrecalcInstForwardATM = calloc(1, sizeof(FxBetaDLM_FxOptInst));
    InstConstForwardATM = calloc(1, sizeof(FxBetaDLM_InstPrecalc));
    ...
    err = FxBetaDLM_Setup_FxOptInst(
        cpn->fx_val_date, cpn->pay_date, und->model->lToday, call->ex_date, 1,
        fwd_strike, NULL, SRT_CALL, und->model, PrecalcInstForwardATM);

    if (err)
      goto FREE_RETURN;

    /* Update the precalculations */
    err = FxBetaDLM_Calculate_AllConst(PrecalcInstForwardATM, und->model,
                                       und->num_params, InstConstForwardATM);

    if (err)
      goto FREE_RETURN;

    err = FxBetaDLM_Update_InstPrecalc_FromMoment(PrecalcInstForwardATM,
                                                  und->model, und->num_params,
                                                  InstConstForwardATM);

    if (err)
      goto FREE_RETURN;

    err = FxBetaDLM_Price_FxOptInst(PrecalcInstForwardATM, und->model,
                                    InstConstForwardATM, und->hermite,
                                    und->num_params, &(fwd_premium));

    err = srt_f_optimpvol(fwd_premium, fwd_strike[0], fwd_strike[0],
                          PrecalcInstForwardATM->dVolTime,
                          PrecalcInstForwardATM->dDfPayDom, SRT_CALL,
                          SRT_LOGNORMAL, &std_Fwd);
    ...
    und->sigma_for        , und->lda_for        , und->sigma_time_fx       ,
                                                            ...
                                                            und->corr_times ,
                                                            und->correl_dom_for
          , und->correl_dom_fx        , und->correl_for_fx        ,
    und->corr_n_times        , &std_Fwd);
    */
    ...
    if (und->num_params->iEquiSpacedFwd) {
      step_Fwd = 2.0 * und->num_params->dNbStdFwd * std_Fwd /
      ...
      for (i = 0; i < und->num_params->iNbFwd; i++) {
        used_strikes[i] =
            strikes[0] * exp(std_Fwd * inv_cumnorm_fast(coef_Fwd));
        coef_Fwd -= step_Fwd;
      }
    }
    ...
    /* Setup the X */
    step_X =
        2.0 * und->num_params->dNbStdX * std_X / (und->num_params->iNbX - 1.0);
  ...
  err = FxBetaDLM_Setup_FxOptInst(cpn->fx_val_date, cpn->pay_date,
                                  call->ex_date, cpn->fx_fix_date, nb_strikes,
                                  strikes, NULL, SRT_CALL, und->model, Inst);
  ...
  if (und->num_params->iPrecalcSmile) {
    ...
    PrecalcInst = calloc(1, sizeof(FxBetaDLM_FxOptInst));
    ...
    err = FxBetaDLM_Setup_FxOptInst(cpn->fx_val_date, cpn->pay_date,
                                    call->ex_date, cpn->fx_fix_date,
                                    und->num_params->iNbFwd, used_strikes, NULL,
                                    SRT_CALL, und->model, PrecalcInst);
    ...
    for (i = 0; i < PrecalcInst->iNbStrike; i++) {
      InstConst->dPrecalcFwdFloor[i] =
          fwd * strikes[0] / PrecalcInst->dStrike[i];
    }

    for (i = 0; i < und->num_params->iNbX; i++) {
      X0 = InstConst->dPrecalcX[i];

      err = FxBetaDLM_Update_InstPrecalc_FromX0(X0, PrecalcInst, und->model,
                                                und->num_params, InstConst);
      ...
      err = FxBetaDLM_Price_FxOptInst(PrecalcInst, und->model, InstConst,
                                      und->hermite, und->num_params, premium);
      ...
      for (j = 0; j < PrecalcInst->iNbStrike; j++) {
        ...
        if (und->num_params->iPriceVol) {
          ...
          err = srt_f_optimpvol(premium[j], InstConst->dPrecalcFwdFloor[j],
                                strikes[0], PrecalcInst->dVolTime, 1.0,
    ...
    if (nb_strikes > 2) {
      ...
      for (i = 0; i < PrecalcInst->iNbStrike; i++) {
        InstConst->dPrecalcFwdCap[i] =
            fwd * strikes[1] / PrecalcInst->dStrike[i];
      }
      ...
      for (i = 0; i < und->num_params->iNbX; i++) {
        ...
        for (j = 0; j < PrecalcInst->iNbStrike; j++) {
          ...
          if (und->num_params->iPriceVol) {
            /* Transform price into std */
            err = srt_f_optimpvol(premium[j], InstConst->dPrecalcFwdCap[j],
                                  strikes[1], PrecalcInst->dVolTime, 1.0,
                                  SRT_CALL, SRT_LOGNORMAL, &vol);
...
Err cpd_fill_eval_const_dlm(CPD_UND und, CPD_STR cpd,
                            /*	Index of the current call/KO */
                            int call_idx, int is_mc,
  ...
  if (eval_const->nb_pd_inst > 0) {
    eval_const->pd_inst =
        calloc(eval_const->nb_pd_inst, sizeof(FxBetaDLM_FxOptInst));
    eval_const->pd_beta_precalc =
        calloc(eval_const->nb_pd_inst, sizeof(FxBetaDLM_InstPrecalc));
  ...
  if (is_mc) {
    ...
    eval_const->spot_fx_cvx = -0.5 * eval_const->spot_fx_for_gam *
                                  eval_const->spot_fx_for_gam * for_phi +
                              0.5 * eval_const->spot_fx_dom_gam *
                                  eval_const->spot_fx_dom_gam * dom_phi;
  }
  ...
  if (call->num_fund_cpn > 0) {
    ...
    for (i = call->fund_idx; i <= last_used_fund; i++) {
      ...
      if (is_mc) {
        beta_temp = (1.0 - exp(-fund_lam * (cpd->fund_leg->cpn[i].pay_time -
                                            und->model->dTStar))) /
                    fund_lam;
        ...
        eval_const->fund_gam[i - call->fund_idx] =
            (1.0 -
             exp(-fund_lam * (cpd->fund_leg->cpn[i].pay_time - evt_time))) /
            fund_lam;
        eval_const->fund_gam2[i - call->fund_idx] =
            0.5 * eval_const->fund_gam[i - call->fund_idx] *
            eval_const->fund_gam[i - call->fund_idx] * fund_phi;
  ...
  if (is_mc) {
    beta_temp =
        (1.0 - exp(-fund_lam * (cpd->fund_leg->cpn[call->fund_idx].start_time -
                                und->model->dTStar))) /
        fund_lam;
  ...
  if (call->num_pd_cpn > 0) {
    ...
    for (i = call->pd_idx; i <= last_used_pd; i++) {
      ...
      if (is_mc) {
        beta_temp = (1.0 - exp(-dom_lam * (cpd->pd_leg->cpn[i].pay_time -
                                           und->model->dTStar))) /
                    dom_lam;
  ...
  fwd_fx = und->model->dCashFx * swp_f_df(today, evt_date, for_yc) /
           swp_f_df(today, evt_date, dom_yc);
  ...
  if (is_mc) {
    ...
    eval_const->pd_S_const +=
        -0.5 * (und->model->dB0 * und->model->dB0 * eval_const->varFFX /
                    eval_const->pd_S_lin +
    ...
    eval_const->pd_S_lin =
        1.0 / (1.0 + 2.0 * und->model->dC0 * eval_const->varFFX);
    eval_const->pd_S_quad = und->model->dC0 * eval_const->pd_S_lin;
    eval_const->pd_S_const = -0.5 * (eval_const->varFFX * eval_const->pd_S_lin *
                                         und->model->dB0 * und->model->dB0 -
                                     log(eval_const->pd_S_lin));
  ...
  if (call->num_pd_cpn > 0) {
    ...
    for (i = call->pd_idx; i <= last_used_pd; i++) {
      ...
      if (nb_strikes > 0) {
        err = cpd_fill_FxInstrument_dlm(und, cpd, call, i - call->pd_idx, cpn,
                                        eval_const, nb_strikes, strikes);
        if (err)
          goto FREE_RETURN;
      } else {
      ...
      if (is_mc) {
        beta_temp =
            (1.0 - exp(-dom_lam * (cpn->fx_val_time - und->model->dTStar))) /
            dom_lam;
        ...
        beta_temp =
            (1.0 - exp(-for_lam * (cpn->fx_val_time - und->model->dTStar))) /
            for_lam;
  ...
  if (is_mc) {
    beta_temp = (1.0 - exp(-fund_lam * (call->set_time - und->model->dTStar))) /
                fund_lam;
  ...
  if (is_mc) {
    beta_temp =
        (1.0 - exp(-dom_lam * (call->set_time - und->model->dTStar))) / dom_lam;
  ...
  if (!eval_redemption) {
    ...
    if (is_mc) {
      beta_temp =
          (1.0 - exp(-fund_lam * (next_call->set_time - und->model->dTStar))) /
          fund_lam;
      ...
      eval_const->next_fund_gam =
          (1.0 - exp(-fund_lam * (next_call->set_time - evt_time))) / fund_lam;
      eval_const->next_fund_gam2 = 0.5 * eval_const->next_fund_gam *
                                   eval_const->next_fund_gam * fund_phi;
    }
    ...
    if (is_mc) {
      beta_temp =
          (1.0 - exp(-dom_lam * (next_call->set_time - und->model->dTStar))) /
          dom_lam;
    ...
    if (is_mc) {
      beta_temp =
          (1.0 - exp(-fund_lam * (temp_time - und->model->dTStar))) / fund_lam;
    ...
    if (is_mc) {
      beta_temp = (1.0 - exp(-dom_lam * (cpn->pay_time - und->model->dTStar))) /
                  dom_lam;
    ...
    if (is_mc) {
      ...
      eval_const->fin_not_fwd_cvx =
          -0.5 * eval_const->fin_not_fwd_for_gam *
              eval_const->fin_not_fwd_for_gam * for_phi +
          0.5 * eval_const->fin_not_fwd_dom_gam *
              eval_const->fin_not_fwd_dom_gam * dom_phi;
    }
    ...
    if (eval_const->fin_not_abs_beta > 1.0E-16 ||
        eval_const->fin_not_floor_type == 1 ||
        eval_const->fin_not_floor_type == 2 ||
        eval_const->fin_not_floor_type == 4 ||
    ...
    /* Setup the instrument */
    if (nb_strikes > 0) {
      err = cpd_fill_FxInstrument_dlm(und, cpd, call, -1, cpn, eval_const,
                                      nb_strikes, strikes);
      if (err)
        goto FREE_RETURN;
    } else {
...
cpd_free_eval_const(CPD_UND und, CPD_STR cpd, CPD_EVAL_CONST eval_const) {
  int i;
...
static void static_find_sig(CPD_UND und, double time, double *sig_dom,
                            double *sig_for, double *sig_fx,
                            double *corr_dom_for, double *corr_dom_fx,
                            double *corr_for_fx) {
  int i = 0, j = 0, k = 0;

  while (i < und->sigma_n_rates - 1 &&
         und->sigma_time_rates[i] < time - 1.0e-08) {
    i++;
  }
...
Err cpd_fill_tree_arg(CPD_UND und, CPD_STR cpd,
                      /*	Required number of time steps */
                      long req_stp, CPD_TREE_ARG tree_arg) {
  Err err = NULL;
  CPD_PAY_ARG cpd_prm;
  double temp, next_bar;
  int i, j;

  double *dom_phi = NULL, *for_phi = NULL, *dom_beta = NULL, *for_beta = NULL,
         *var_ffx = NULL;
  ...
  if (tree_arg->time[0] > EPS) {
    num_f_add_number(&(tree_arg->nstp), &(tree_arg->time), 0.0);
    num_f_sort_vector(tree_arg->nstp, tree_arg->time);
    num_f_unique_vector(&(tree_arg->nstp), tree_arg->time);
  }

  /*	If only one event today        , add empty event */
  if (tree_arg->nstp == 1) {
    num_f_add_number(&(tree_arg->nstp), (&tree_arg->time), 1.0);
  }

  /*	Fill the vector */
  num_f_fill_vector_newalgo(&(tree_arg->nstp), &(tree_arg->time), req_stp);
  ...
  if (und->use_beta_dlm) {
    ...
    tree_arg->glob_corr_dom_for =
        (double *)calloc(tree_arg->nstp, sizeof(double));
    ...
    if (!tree_arg->vol_change || !tree_arg->sig_dom || !tree_arg->sig_for ||
        !tree_arg->sig_fx || !tree_arg->corr_dom_for ||
        !tree_arg->corr_dom_fx || !tree_arg->corr_for_fx ||
        !tree_arg->dr_const_dom || !tree_arg->dr_coef_dom ||
        !tree_arg->dr_const_for || !tree_arg->dr_coef1_for ||
        !tree_arg->dr_coef2_for || !tree_arg->dr_coef3_for ||
        !tree_arg->dr_const_fx || !tree_arg->dr_coef1_fx ||
        !tree_arg->dr_coef2_fx || !tree_arg->dr_coef3_fx ||
        !tree_arg->dom_ifr || !tree_arg->dom_fwd || !tree_arg->dom_var ||
        !dom_phi || !dom_beta || !tree_arg->for_ifr || !tree_arg->for_fwd ||
        !tree_arg->for_var || !for_phi || !for_beta || !tree_arg->fx_fwd ||
        !tree_arg->fx_var || !var_ffx || !tree_arg->glob_corr_dom_for ||
        !tree_arg->glob_corr_dom_fx || !tree_arg->glob_corr_for_fx) {
    ...
    Fx3DBetaDLM_PrecalcGRFNTreeQBeta(
        tree_arg->nstp, tree_arg->time, und->model, tree_arg->dom_fwd,
        tree_arg->dom_var, dom_phi, dom_beta, tree_arg->for_fwd,
        tree_arg->for_var, for_phi, for_beta, tree_arg->fx_fwd,
        tree_arg->fx_var, var_ffx, tree_arg->glob_corr_dom_for,
        tree_arg->glob_corr_dom_fx, tree_arg->glob_corr_for_fx,
    ...
    Fx3DBetaDLM_DriftVolAndCorr(
        tree_arg->nstp, tree_arg->time, und->model, dom_phi, dom_beta, for_phi,
        for_beta, var_ffx,

        tree_arg->dr_const_dom, tree_arg->dr_coef_dom, tree_arg->dr_const_for,
        tree_arg->dr_coef1_for, tree_arg->dr_coef2_for, tree_arg->dr_coef3_for,
        tree_arg->dr_const_fx, tree_arg->dr_coef1_fx, tree_arg->dr_coef2_fx,
        tree_arg->dr_coef3_fx,

        tree_arg->vol_change, tree_arg->sig_dom, tree_arg->sig_for,
    ...
    for (i = 0; i < tree_arg->nstp; i++) {
      if (i < tree_arg->nstp - 1) {
        tree_arg->dom_ifr[i] = swp_f_zr(
            tree_arg->date[i], tree_arg->date[i + 1], tree_arg->dom_yc);
      } else {
        tree_arg->dom_ifr[i] = swp_f_zr(
            tree_arg->date[i], tree_arg->date[i] + 1, tree_arg->dom_yc);
      }
    ...
    static_find_sig(und, tree_arg->time[tree_arg->nstp - 1],
    ...
    for (i = tree_arg->nstp - 2; i >= 0; i--) {
      static_find_sig(und, tree_arg->time[i], &(tree_arg->sig_dom[i]),
                      &(tree_arg->sig_for[i]), &(tree_arg->sig_fx[i]),
                      &(tree_arg->corr_dom_for[i]), &(tree_arg->corr_dom_fx[i]),
    ...
    if (!tree_arg->dom_ifr || !tree_arg->dom_fwd || !tree_arg->dom_var ||
        !tree_arg->for_ifr || !tree_arg->for_fwd || !tree_arg->for_var ||
        !tree_arg->fx_fwd || !tree_arg->fx_var) {
      err = "Memory allocation error (4) in cpd_fill_tree_arg";
      goto FREE_RETURN;
    }

    fill_fwd_var_corr(tree_arg->nstp, tree_arg->time, tree_arg->date,
                      tree_arg->sig_dom, tree_arg->sig_for, tree_arg->sig_fx,
                      tree_arg->dom_lam, tree_arg->for_lam,
                      tree_arg->corr_dom_for, tree_arg->corr_dom_fx,
                      tree_arg->corr_for_fx, tree_arg->dom_yc, tree_arg->for_yc,
                      tree_arg->dom_ifr, tree_arg->dom_fwd, tree_arg->dom_var,
                      tree_arg->for_ifr, tree_arg->for_fwd, tree_arg->for_var,
                      tree_arg->fx_fwd, tree_arg->fx_var);
    ...
    for (i = 0; i < tree_arg->nstp; i++) {
      tree_arg->fx_fwd[i] =
          log(swp_f_df(und->today, tree_arg->date[i], und->for_yc) /
              swp_f_df(und->today, tree_arg->date[i], und->dom_yc)) -
          0.5 * tree_arg->fx_var[i];

      err = Fx3DtsFwdBetaAdjustment_corr(
          0.0, tree_arg->time[i], tree_arg->time[i], tree_arg->time[i],
          und->sigma_time_rates, und->sigma_n_rates, und->sigma_dom,
          und->lda_dom, und->sigma_for, und->lda_for, und->sigma_date_fx,
          und->sigma_fx, und->sigma_n_fx, und->corr_times, und->correl_dom_for,
          und->correl_dom_fx, und->correl_for_fx, und->corr_n_times, &temp);
  ...
  if (!tree_arg->is_event || !tree_arg->void_prm || !tree_arg->is_bar ||
      !tree_arg->bar_lvl || !tree_arg->bar_cl) {
  ...
  for (i = tree_arg->nstp - 1; i >= 0; i--) {
    if (j >= 0 && fabs(tree_arg->date[i] - cpd->call[j].ex_date) < 1.0e-04) {
      cpd_prm = calloc(1, sizeof(cpd_pay_arg));
      cpd_prm->und = und;
      cpd_prm->cpd = cpd;
      ...
      if (cpd->call[j].cxxall_type == 1) {
        /*	Knock Out in the Tree	*/
        next_bar = cpd->call[j].barrier *
                   swp_f_df(cpd->call[j].ex_date,
                            add_unit(cpd->call[j].ex_date, 2, SRT_BDAY,
                                     MODIFIED_SUCCEEDING),
                            tree_arg->dom_yc)
...
Err cpd_fill_mc_arg(CPD_UND und, CPD_STR cpd,
                    /*	Required number of paths */
                    long req_pth,
                    /*	Do PECS */
                    int do_pecs, CPD_MC_ARG mc_arg) {
  Err err = NULL;
  CPD_PAY_ARG cpd_prm;
  ...
  if (mc_arg->nb_dates == 1) {
    num_f_add_number(&(mc_arg->nb_dates), (&mc_arg->time), 1.0);
  }
  ...
  if (und->use_beta_dlm) {
    ...
    if (!mc_arg->dom_fwd || !mc_arg->dom_std || !mc_arg->dom_phi ||
        !mc_arg->dom_beta || !mc_arg->dom_bond_pay || !mc_arg->dom_beta_pay ||
        !mc_arg->for_fwd_const || !mc_arg->for_fwd_lin || !mc_arg->for_std ||
        !mc_arg->for_phi || !mc_arg->for_beta || !mc_arg->fx_fwd ||
        !mc_arg->fx_std || !mc_arg->ffx_var || !mc_arg->dom_for_cov ||
    ...
    Fx3DBetaDLM_ExpectAndVarGrfn(
        mc_arg->nb_dates, mc_arg->time, und->model, mc_arg->dom_fwd,
        mc_arg->dom_std, mc_arg->dom_phi, mc_arg->dom_beta,
        mc_arg->for_fwd_const, mc_arg->for_fwd_lin, mc_arg->for_std,
        mc_arg->for_phi, mc_arg->for_beta, mc_arg->fx_fwd, mc_arg->fx_std,
        mc_arg->ffx_var, 0, mc_arg->dom_for_cov, mc_arg->dom_fx_cov,
        mc_arg->for_fx_cov, und->grfn_params->min_time);
  } else {
    ...
    if (!mc_arg->dom_ifr || !mc_arg->dom_fwd || !mc_arg->dom_std ||
        !mc_arg->dom_phi || !mc_arg->dom_beta || !mc_arg->dom_bond_pay ||
        !mc_arg->dom_beta_pay || !mc_arg->for_ifr || !mc_arg->for_fwd ||
        !mc_arg->for_std || !mc_arg->for_phi || !mc_arg->for_beta ||
        !mc_arg->fx_fwd || !mc_arg->fx_std || !mc_arg->dom_for_cov ||
        !mc_arg->dom_fx_cov || !mc_arg->for_fx_cov) {
      err = "Memory allocation error (4) in cpd_fill_mc_arg";
      goto FREE_RETURN;
    }
    ...
    memcpy(merge_times, und->sigma_time_rates,
           und->sigma_n_rates * sizeof(double));
    nb_merge_times = und->sigma_n_rates;
    num_f_concat_vector(&nb_merge_times, &merge_times, und->sigma_n_fx,
                        und->sigma_time_fx);
    num_f_concat_vector(&nb_merge_times, &merge_times, und->corr_n_times,
                        und->corr_times);
    ...
    for (i = nb_merge_times - 1; i >= 0; i--) {
      static_find_sig(und, merge_times[i], &(sig_dom[i]), &(sig_for[i]),
                      &(sig_fx[i]), &(corr_dom_for[i]), &(corr_dom_fx[i]),
                      &(corr_for_fx[i]));
    }

    err = fill_mc_init_corr(
        (long)(mc_arg->date[mc_arg->nb_dates - 1] + 1.0e-08),
        mc_arg->time[mc_arg->nb_dates - 1], mc_arg->date, mc_arg->time,
        mc_arg->nb_dates, merge_times, nb_merge_times, sig_dom, und->lda_dom,
        sig_for, und->lda_for, sig_fx, corr_dom_for, corr_dom_fx, corr_for_fx,
        und->dom_yc, und->for_yc, mc_arg->dom_ifr, mc_arg->dom_fwd,
        mc_arg->dom_std, mc_arg->dom_phi, mc_arg->dom_beta,
        mc_arg->dom_bond_pay, mc_arg->dom_beta_pay, mc_arg->for_ifr,
        mc_arg->for_fwd, mc_arg->for_std, mc_arg->for_phi, mc_arg->for_beta,
        mc_arg->fx_fwd, mc_arg->fx_std, mc_arg->dom_for_cov, mc_arg->dom_fx_cov,
        mc_arg->for_fx_cov);
  ...
  for (i = mc_arg->nb_dates - 1; i >= 0; i--) {
    if (j >= 0 && fabs(mc_arg->date[i] - cpd->call[j].ex_date) < 1.0e-04) {
      ...
      if (und->use_beta_dlm) {
        ...
        mc_arg->dom_bond_pay[i] = log(swp_f_df(
            cpd->call[j].ex_date, und->model->lTStarDate, und->dom_yc));
...
Err cpd_fill_check_all_struct(
    ...
    int TARN_Do,
    /*	Numerical params */
    long req_stp, long req_pth, int do_pecs, int forcetree,
    ...
    int *call_feat, /*	0: No callable feature to be valued
                 ...
                 tree 2: Callable feature (KO) to be valued through MC */
    CPD_TREE_ARG tree_arg, CPD_MC_ARG mc_arg, double dom_vol_shift,
    double for_vol_shift, double fx_vol_shift, int skip_fill) {
  Err err = NULL;
  ...
  if (skip_fill == 0) {
    ...
    err = cpd_fill_fund_leg(und->today, eod_fix_flag, fund_not, fund_ccy,
                            fund_ncpn, fund_fix, fund_start, fund_pay,
                            fund_basis, fund_spr, fund_mrg, cpd->fund_leg);
    if (err) {
      goto FREE_RETURN;
    }
    ...
    err = cpd_fill_exo_leg(
        und->today, eod_fix_flag, pd_not, pd_ncpn, pd_fix, pd_start, pd_pay,
        pd_basis, pd_alpha, pd_beta, pd_floored, pd_floor, pd_capped, pd_cap,
        use_cpn_opt_str, pd_num_strikes, pd_wcst, pd_wspot, pd_strikes,
        pd_weights, pd_not_ref_fix[ncall], pd_not_ref_alpha, pd_not_ref_beta,
        pd_not_ref_floored, pd_not_ref_floor, pd_not_ref_capped, pd_not_ref_cap,
        use_not_opt_str, use_not_opt_str ? pd_not_num_strikes[ncall] : 0,
        use_not_opt_str ? pd_not_wcst[ncall] : 0.0,
        use_not_opt_str ? pd_not_wspot[ncall] : 0.0,
        use_not_opt_str ? pd_not_strikes[ncall] : NULL,
        use_not_opt_str ? pd_not_weights[ncall] : NULL, cpd->pd_leg);
    ...
    if (ncall > 0 && ex_date[ncall - 1] >= und->today + eod_ex_flag) {
      ...
      err = cpd_fill_calls(und->today, eod_ex_flag, ncall, call_type, pay_rec,
                           ex_date, set_date, pd_not_ref_fix, fund_not_s,
                           pd_not_s, use_not_opt_str, pd_not_num_strikes,
                           pd_not_wcst, pd_not_wspot, pd_not_strikes,
                           pd_not_weights, barrier, bar_type, fees, TARN_Do,
                           smooth, fx_bound, use_bound, force_optim, cpd);
    ...
    if (use_calib) {
      err = cpd_calib_und(
          today, eod_ex_flag, fx_spot, fx_spot_date, dom_calib, dom_und, dom_yc,
          ...
          correl_dom_fx, correl_for_fx, corr_n_times, cpd_dlm_params, cpd,
          get_ir_cash_vol, fx_mkt_vol_date, fx_mkt_vol, num_fx_mkt_vol, und,
          dom_vol_shift, for_vol_shift, fx_vol_shift);
    } else {
      err = cpd_fill_und(fx3dund, und, cpd_dlm_params, dom_vol_shift,
                         for_vol_shift, fx_vol_shift);
    }
...
/*	Free all structures */
Err cpd_free_all_struct(CPD_STR cpd, CPD_UND und, CPD_TREE_ARG tree_arg,
                        CPD_MC_ARG mc_arg) {
  cpd_free_und(und);
  cpd_free_calls(cpd);
...
Err cpd_payoff_4_3dfx_tree(
  ...
  double in_not_fund_I, in_not_fund_J, in_not_fund_K, in_not_fund_DI,
      in_not_fund_DJ, in_not_fund_DK;

  double in_not_pd_I, in_not_pd_J, in_not_pd_K, in_not_pd_DI, in_not_pd_DJ,
      in_not_pd_DK;

  double in_not_pd_fwd_I, in_not_pd_fwd_J, in_not_pd_fwd_K, in_not_pd_fwd_DI,
      in_not_pd_fwd_DJ, in_not_pd_fwd_DK;

  double next_start_fund_I, next_start_fund_J, next_start_fund_K,
      next_start_fund_DI, next_start_fund_DJ, next_start_fund_DK;

  double next_fund_I, next_fund_J, next_fund_K, next_fund_DI, next_fund_DJ,
      next_fund_DK;

  double next_pd_I, next_pd_J, next_pd_K, next_pd_DI, next_pd_DJ, next_pd_DK;

  double fin_not_disc_I, fin_not_disc_J, fin_not_disc_K, fin_not_disc_DI,
      fin_not_disc_DJ, fin_not_disc_DK;

  double fin_not_fwd_I, fin_not_fwd_J, fin_not_fwd_K, fin_not_fwd_DI,
      fin_not_fwd_DJ, fin_not_fwd_DK;
  ...
  /* for funding start */
  start_I = eval_const->start_dff *
            exp(-eval_const->start_gam * sv[0][0][0][eval_const->fund_var] -
                eval_const->start_gam2) *
  ...
  if (n1 > 0)
    start_DI =
        exp(-eval_const->start_gam * (sv[1][0][0][eval_const->fund_var] -
                                      sv[0][0][0][eval_const->fund_var]));
  else
    start_DI = 1.0;

  if (n2 > 0)
    start_DJ =
        exp(-eval_const->start_gam * (sv[0][1][0][eval_const->fund_var] -
                                      sv[0][0][0][eval_const->fund_var]));
  else
    start_DJ = 1.0;
  ...
  if (n3 > 0)
    start_DK =
        exp(-eval_const->start_gam * (sv[0][0][1][eval_const->fund_var] -
                                      sv[0][0][0][eval_const->fund_var]));
  else
    start_DK = 1.0;
  ...
  if (n1 > 0)
    in_not_fund_DI =
        exp(-eval_const->in_not_fund_gam * (sv[1][0][0][eval_const->fund_var] -
                                            sv[0][0][0][eval_const->fund_var]));
  else
    in_not_fund_DI = 1.0;
  ...
  if (n2 > 0)
    in_not_fund_DJ =
        exp(-eval_const->in_not_fund_gam * (sv[0][1][0][eval_const->fund_var] -
                                            sv[0][0][0][eval_const->fund_var]));
  else
    in_not_fund_DJ = 1.0;
  ...
  if (n3 > 0)
    in_not_fund_DK =
        exp(-eval_const->in_not_fund_gam * (sv[0][0][1][eval_const->fund_var] -
                                            sv[0][0][0][eval_const->fund_var]));
  else
    in_not_fund_DK = 1.0;
  ...
  in_not_pd_I = eval_const->in_not_pd_dff *
                exp(-eval_const->in_not_pd_gam * sv[0][0][0][0] -
                    eval_const->in_not_pd_gam2);
  ...
  if (n1 > 0)
    in_not_pd_DI =
        exp(-eval_const->in_not_pd_gam * (sv[1][0][0][0] - sv[0][0][0][0]));
  else
    in_not_pd_DI = 1.0;

  if (n2 > 0)
    in_not_pd_DJ =
        exp(-eval_const->in_not_pd_gam * (sv[0][1][0][0] - sv[0][0][0][0]));
  else
    in_not_pd_DJ = 1.0;

  if (n3 > 0)
    in_not_pd_DK =
        exp(-eval_const->in_not_pd_gam * (sv[0][0][1][0] - sv[0][0][0][0]));
  else
    in_not_pd_DK = 1.0;
  ...
  if (call->use_opt_str) {
    /*	For Fwd fx */
    in_not_pd_fwd_I = eval_const->in_not_pd_fwd_dff_ratio *
                      exp(-eval_const->in_not_pd_fwd_for_gam * sv[0][0][0][1] +
                          eval_const->in_not_pd_fwd_dom_gam * sv[0][0][0][0] +
                          eval_const->in_not_pd_fwd_cvx);
    ...
    if (n1 > 0)
      in_not_pd_fwd_DI = exp(-eval_const->in_not_pd_fwd_for_gam *
                                 (sv[1][0][0][1] - sv[0][0][0][1]) +
                             eval_const->in_not_pd_fwd_dom_gam *
                                 (sv[1][0][0][0] - sv[0][0][0][0]));
    else
      in_not_pd_fwd_DI = 1.0;

    if (n2 > 0)
      in_not_pd_fwd_DJ = exp(-eval_const->in_not_pd_fwd_for_gam *
                                 (sv[0][1][0][1] - sv[0][0][0][1]) +
                             eval_const->in_not_pd_fwd_dom_gam *
                                 (sv[0][1][0][0] - sv[0][0][0][0]));
    else
      in_not_pd_fwd_DJ = 1.0;

    if (n3 > 0)
      in_not_pd_fwd_DK = exp(-eval_const->in_not_pd_fwd_for_gam *
                                 (sv[0][0][1][1] - sv[0][0][0][1]) +
                             eval_const->in_not_pd_fwd_dom_gam *
                                 (sv[0][0][1][0] - sv[0][0][0][0]));
    else
  ...
  if (!eval_redemption) {
    next_start_fund_I = eval_const->next_start_fund_dff *
    ...
    if (n1 > 0)
      next_start_fund_DI = exp(-eval_const->next_start_fund_gam *
                               (sv[1][0][0][eval_const->fund_var] -
                                sv[0][0][0][eval_const->fund_var]));
    else
      next_start_fund_DI = 1.0;

    if (n2 > 0)
      next_start_fund_DJ = exp(-eval_const->next_start_fund_gam *
                               (sv[0][1][0][eval_const->fund_var] -
                                sv[0][0][0][eval_const->fund_var]));
    else
      next_start_fund_DJ = 1.0;

    if (n3 > 0)
      next_start_fund_DK = exp(-eval_const->next_start_fund_gam *
                               (sv[0][0][1][eval_const->fund_var] -
                                sv[0][0][0][eval_const->fund_var]));
    else
    ...
    if (n1 > 0)
      next_fund_DI =
          exp(-eval_const->next_fund_gam * (sv[1][0][0][eval_const->fund_var] -
                                            sv[0][0][0][eval_const->fund_var]));
    else
      next_fund_DI = 1.0;
    ...
    if (n2 > 0)
      next_fund_DJ =
          exp(-eval_const->next_fund_gam * (sv[0][1][0][eval_const->fund_var] -
                                            sv[0][0][0][eval_const->fund_var]));
    else
      next_fund_DJ = 1.0;
    ...
    if (n3 > 0)
      next_fund_DK =
          exp(-eval_const->next_fund_gam * (sv[0][0][1][eval_const->fund_var] -
                                            sv[0][0][0][eval_const->fund_var]));
    else
      next_fund_DK = 1.0;

    next_pd_I = eval_const->next_pd_dff *
                exp(-eval_const->next_pd_gam * sv[0][0][0][0] -
                    eval_const->next_pd_gam2);
    ...
    if (n1 > 0)
      next_pd_DI =
          exp(-eval_const->next_pd_gam * (sv[1][0][0][0] - sv[0][0][0][0]));
    else
      next_pd_DI = 1.0;

    if (n2 > 0)
      next_pd_DJ =
          exp(-eval_const->next_pd_gam * (sv[0][1][0][0] - sv[0][0][0][0]));
    else
      next_pd_DJ = 1.0;

    if (n3 > 0)
      next_pd_DK =
          exp(-eval_const->next_pd_gam * (sv[0][0][1][0] - sv[0][0][0][0]));
    else
      next_pd_DK = 1.0;
  } else
  /*	Final notional */
  {
    /* For df */
    fin_not_disc_I = eval_const->fin_not_disc_dff *
                     exp(-eval_const->fin_not_disc_gam * sv[0][0][0][0] -
                         eval_const->fin_not_disc_gam2);
    ...
    if (n1 > 0)
      fin_not_disc_DI = exp(-eval_const->fin_not_disc_gam *
                            (sv[1][0][0][0] - sv[0][0][0][0]));
    else
      fin_not_disc_DI = 1.0;

    if (n2 > 0)
      fin_not_disc_DJ = exp(-eval_const->fin_not_disc_gam *
                            (sv[0][1][0][0] - sv[0][0][0][0]));
    else
      fin_not_disc_DJ = 1.0;

    if (n3 > 0)
      fin_not_disc_DK = exp(-eval_const->fin_not_disc_gam *
                            (sv[0][0][1][0] - sv[0][0][0][0]));
    else
      fin_not_disc_DK = 1.0;

    /*	For Fwd fx */
    fin_not_fwd_I = eval_const->fin_not_fwd_dff_ratio *
  ...
  for (l = 0; l < num_fund_cpn; l++) {
    ...
    eval_const->fund_I[l] =
        ...
        cpd->fund_leg->cpn[call->fund_idx + l].cxxpn;
    ...
    if (n1 > 0)
      eval_const->fund_DI[l] =
          exp(-eval_const->fund_gam[l] * (sv[1][0][0][eval_const->fund_var] -
                                          sv[0][0][0][eval_const->fund_var]));
    else
    ...
    if (n2 > 0)
      eval_const->fund_DJ[l] =
          exp(-eval_const->fund_gam[l] * (sv[0][1][0][eval_const->fund_var] -
                                          sv[0][0][0][eval_const->fund_var]));
    else
    ...
    if (n3 > 0)
      eval_const->fund_DK[l] =
          exp(-eval_const->fund_gam[l] * (sv[0][0][1][eval_const->fund_var] -
                                          sv[0][0][0][eval_const->fund_var]));
    else
  ...
  for (l = 0; l < num_pd_cpn; l++) {
    ...
    eval_const->pd_disc_I[l] =
        ...
        exp(-eval_const->pd_disc_gam[l] * sv[0][0][0][0] -
    ...
    eval_const->pd_fwd_I[l] =
        ...
        exp(-eval_const->pd_fwd_for_gam[l] * sv[0][0][0][1] +
            eval_const->pd_fwd_dom_gam[l] * sv[0][0][0][0] +
  ...
  * exp (- eval_const->fund_gam[l] * svijk[eval_const->fund_var] -
  eval_const->fund_gam2[l])
        ...
        if (!eval_redemption) {
           ...
           * exp (- eval_const->next_start_fund_gam *
           * svijk[eval_const->fund_var] - eval_const->next_start_fund_gam2)
           * cpd->fund_leg->notional; */
          ...
          if (cpn->use_opt_str) {
            ...
            for (str_idx = 0; str_idx < cpn->nstrikes; str_idx++) {
              if (fabs(cpn->weights[str_idx]) > 1e-16) {
                ...
                opt_string +=
                    cpn->weights[str_idx] *
                    OPT_VAL_MACRO(opt_type, fwd, cpn->strikes[str_idx],
                                  eval_const->pd_std[l],
            ...
            /*	Floor */
            floor = OPT_VAL_MACRO(
                eval_const->pd_floor_type[l], fwd, eval_const->pd_floor_str[l],
                eval_const->pd_std[l], eval_const->pd_half_std[l]);

            /*	Cap */
            cap = OPT_VAL_MACRO(
                eval_const->pd_cap_type[l], fwd, eval_const->pd_cap_str[l],
                eval_const->pd_std[l], eval_const->pd_half_std[l]);
        ...
        if (eval_redemption) {
          ...
          /* fwd = fx
                  * eval_const->fin_not_fwd_dff_ratio
                  * exp (	-	eval_const->fin_not_fwd_for_gam *
             svijk[1] +	eval_const->fin_not_fwd_dom_gam * svijk[0] +
             eval_const->fin_not_fwd_cvx); */
          ...
          if (cpn->use_opt_str) {
            ...
            /*	Floor */
            floor = OPT_VAL_MACRO(eval_const->fin_not_floor_type, fwd,
            ...
            /*	Cap */
            cap = OPT_VAL_MACRO(
                eval_const->fin_not_cap_type, fwd, eval_const->fin_not_cap_str,
                eval_const->fin_not_std, eval_const->fin_not_half_std);
           ...
           * eval_const->next_pd_gam2) next_call->pd_not_amt; */
        ...
        if (call->use_opt_str) {
          ...
          for (str_idx = 0; str_idx < call->nstrikes; str_idx++) {
            if (fabs(call->weights[str_idx]) > 1e-16) {
              if (eval_const->in_not_pd_std > 1e-16 &&
                  call->strikes[str_idx] > 1e-16)
                opt_type = 3;
              else
                opt_type = 1;

              opt_string += call->weights[str_idx] *
                            OPT_VAL_MACRO(opt_type, fwd, call->strikes[str_idx],
...
Err cpd_payoff_4_3dfx_tree_ko(
  ...
  if (cpd->call[call_idx].cxxall_type == 0) {
    /*	Callable Case	*/
    for (i = 0; i < n1; i++) {
      svi = sv[i];
      for (j = 0; j < n2; j++) {
            ...
            for (k = 0; k < n3; k++) {
              ...
              if (i >= 0) {
                ...
                for (l = 0; l < num_pd_cpn; l++) {
                  ...
                  /*	Floor */
                  floor = OPT_VAL_MACRO(eval_const->pd_floor_type[l], fwd,
                                        eval_const->pd_floor_str[l],
                                        eval_const->pd_std[l],
                                        eval_const->pd_half_std[l]);

                  /*	Cap */
                  cap = OPT_VAL_MACRO(eval_const->pd_cap_type[l], fwd,
                ...
                if (eval_redemption) {
                  ...
                  /*	Cap */
                  cap = OPT_VAL_MACRO(eval_const->fin_not_cap_type, fwd,
    ...
    } else if ((call->bar_type == 0 && is_bar == -1) ||
               (call->bar_type == 1 && is_bar == -2)) {
      for (i = 0; i < n1; i++) {
        for (j = 0; j < n2; j++) {
          for (k = 0; k < n3; k++) {
            prod_val[i][j][k][0] = 0.0;
...
/*	Launch the tree */
Err cpd_launch_tree(CPD_STR cpd, CPD_UND und, CPD_TREE_ARG tree_arg,
                    /*	Result */
                    double *prem, double *IV) {
  double *temp_val;
  int nprod;
  Err err = NULL;

  nprod = (int)(*IV + 1e-8);
  temp_val = calloc(nprod + 1, sizeof(double));
  if (!temp_val) {
  ...
  if (und->use_beta_dlm) {
    err = tree_main_3dBetaDLM_QBeta(
        tree_arg->nstp, tree_arg->time, tree_arg->date, tree_arg->vol_change,
        tree_arg->dom_lam, tree_arg->for_lam, tree_arg->sig_dom,
        tree_arg->sig_for, tree_arg->sig_fx,

        tree_arg->corr_dom_for, tree_arg->corr_dom_fx, tree_arg->corr_for_fx,
        ...
        tree_arg->dom_fwd, tree_arg->dom_var, tree_arg->for_fwd,
        tree_arg->for_var, tree_arg->fx_fwd, tree_arg->fx_var,

        tree_arg->glob_corr_dom_for, tree_arg->glob_corr_dom_fx,
        tree_arg->glob_corr_for_fx,

        tree_arg->spot_fx, tree_arg->dom_yc, tree_arg->for_yc,
        tree_arg->dom_ifr,

        tree_arg->void_prm, tree_arg->is_event, tree_arg->bar_lvl,
        tree_arg->bar_cl, tree_arg->is_bar,

        cpd_payoff_4_3dfx_BetaDLM_tree, nprod, 1, (double *)temp_val);
  } else {
    err = tree_main_3dfx(
        tree_arg->nstp, tree_arg->time, tree_arg->date, tree_arg->vol_change,
        tree_arg->sig_dom, tree_arg->sig_for, tree_arg->sig_fx,
        tree_arg->dom_ifr, tree_arg->dom_fwd, tree_arg->dom_var,
        tree_arg->for_ifr, tree_arg->for_fwd, tree_arg->for_var,
        tree_arg->fx_fwd, tree_arg->fx_var, tree_arg->void_prm,
        tree_arg->is_event, tree_arg->bar_lvl, tree_arg->bar_cl,
        tree_arg->is_bar, tree_arg->dom_lam, tree_arg->for_lam,
        tree_arg->corr_dom_for, tree_arg->corr_dom_fx, tree_arg->corr_for_fx,
        tree_arg->spot_fx, tree_arg->dom_yc, tree_arg->for_yc,
        cpd_payoff_4_3dfx_tree, nprod, 1, (double *)temp_val);
  }
...
Err cpd_launch_tree_ko(CPD_STR cpd, CPD_UND und, CPD_TREE_ARG tree_arg,
                       /*	Result */
                       double *prem) {
  double temp_val[2];
  Err err = NULL;
  ...
  err = tree_main_3dfx(
      tree_arg->nstp, tree_arg->time, tree_arg->date, tree_arg->vol_change,
      tree_arg->sig_dom, tree_arg->sig_for, tree_arg->sig_fx, tree_arg->dom_ifr,
      tree_arg->dom_fwd, tree_arg->dom_var, tree_arg->for_ifr,
      tree_arg->for_fwd, tree_arg->for_var, tree_arg->fx_fwd, tree_arg->fx_var,
      tree_arg->void_prm, tree_arg->is_event, tree_arg->bar_lvl,
      tree_arg->bar_cl, tree_arg->is_bar, tree_arg->dom_lam, tree_arg->for_lam,
      tree_arg->corr_dom_for, tree_arg->corr_dom_fx, tree_arg->corr_for_fx,
      tree_arg->spot_fx, tree_arg->dom_yc, tree_arg->for_yc,
      cpd_payoff_4_3dfx_tree_ko, 1, 1, (double *)temp_val);
...
Err cpd_fut_intr_val(
    ...
    CPD_STR cpd,
    /*	Call */
    int fund_idx, int num_fund_cpn, double fund_not_amt, int pd_idx,
    int num_pd_cpn, double pd_not_amt,
    /* Precalculated constants */
    CPD_EVAL_CONST eval_const,
    /* Results */
    double *fund_leg, double *pd_leg) {
  ...
  *fund_leg += eval_const->start_dff *
               exp(-eval_const->start_gam * Xdomfor[eval_const->fund_var] -
                   eval_const->start_gam2) *
               cpd->fund_leg->notional;
  ...
  for (l = 0; l < num_fund_cpn; l++) {
    *fund_leg += eval_const->fund_dff[l] *
                 exp(-eval_const->fund_gam[l] * Xdomfor[eval_const->fund_var] -
                     eval_const->fund_gam2[l]) *
                 cpd->fund_leg->cpn[fund_idx + l].cxxpn;
  }
  ...
  *pd_leg = 0.0;
  ...
  for (l = 0; l < num_pd_cpn; l++) {
    ...
    /*	Discount */
    df = eval_const->pd_disc_dff[l] *
         exp(-eval_const->pd_disc_gam[l] * Xdomfor[0] -
             eval_const->pd_disc_gam2[l]);

    /*	Fwd fx */
    fwd = fx * eval_const->pd_fwd_dff_ratio[l] *
          exp(-eval_const->pd_fwd_for_gam[l] * Xdomfor[1] +
              eval_const->pd_fwd_dom_gam[l] * Xdomfor[0] +
              eval_const->pd_fwd_cvx[l]);
    ...
    if (cpn->use_opt_str) {
      ...
      *pd_leg += df * (cpn->wcst + cpn->wspot * fwd + opt_string);
      ...
      /*	Floor */
      floor = OPT_VAL_MACRO(eval_const->pd_floor_type[l], fwd,
                            eval_const->pd_floor_str[l], eval_const->pd_std[l],
                            eval_const->pd_half_std[l]);

      /*	Cap */
      cap = OPT_VAL_MACRO(eval_const->pd_cap_type[l], fwd,
                          eval_const->pd_cap_str[l], eval_const->pd_std[l],
                          eval_const->pd_half_std[l]);
  ...
  /*	Discount */
  df = eval_const->fin_not_disc_dff *
       exp(-eval_const->fin_not_disc_gam * Xdomfor[0] -
           eval_const->fin_not_disc_gam2);

  /*	Fwd fx */
  fwd = fx * eval_const->fin_not_fwd_dff_ratio *
        exp(-eval_const->fin_not_fwd_for_gam * Xdomfor[1] +
            eval_const->fin_not_fwd_dom_gam * Xdomfor[0] +
            eval_const->fin_not_fwd_cvx);
  ...
  if (cpn->use_opt_str) {
    ...
    *pd_leg += df * (cpn->wcst + cpn->wspot * fwd + opt_string);
    ...
    /*	Floor */
    floor = OPT_VAL_MACRO(
        eval_const->fin_not_floor_type, fwd, eval_const->fin_not_floor_str,
        eval_const->fin_not_std, eval_const->fin_not_half_std);

    /*	Cap */
    cap = OPT_VAL_MACRO(eval_const->fin_not_cap_type, fwd,
                        eval_const->fin_not_cap_str, eval_const->fin_not_std,
                        eval_const->fin_not_half_std);
  ...
  df = eval_const->in_not_pd_dff * exp(-eval_const->in_not_pd_gam * Xdomfor[0] -
                                       eval_const->in_not_pd_gam2);

  if (call->use_opt_str) {
    fwd = fx * eval_const->in_not_pd_fwd_dff_ratio *
          exp(-eval_const->in_not_pd_fwd_for_gam * Xdomfor[1] +
              eval_const->in_not_pd_fwd_dom_gam * Xdomfor[0] +
              eval_const->in_not_pd_fwd_cvx);
    ...
    *pd_leg -=
        df * (call->wcst + call->wspot * fwd + opt_string + eval_const->fee);
  } else {
...
Err cpd_current_pd_cpn_val(
    ...
    CPD_STR cpd,
    /*	Call */
    int pd_idx, int num_pd_cpn, double pd_not_amt,
    /* Precalculated constants */
    CPD_EVAL_CONST eval_const,
    /* Results */
    double *pd_leg) {
  ...
  *pd_leg = 0.0;
  ...
  if (cpn->use_opt_str) {
    ...
    *pd_leg +=
        df * (cpn->alpha + cpn->beta * fx + fabs(cpn->beta) * (floor - cap));
  }

  return NULL;
}

Err cpd_price_FxInstrument_dlm(double Fwd, double LogFwd, double X0,
                               CPD_UND und, CPD_STR cpd, int index_cpn,
                               PD_EXO_CPN cpn, CPD_EVAL_CONST eval_const,
                               double *prices) {
  FxBetaDLM_FxOptInst *Inst;
  FxBetaDLM_InstPrecalc *InstConst;
  Err err = NULL;
  ...
  if (und->num_params->iPrecalcSmile) {
    ...
    index_Fwd1 =
        Get_Index(Fwd, InstConst->dPrecalcFwdFloor, und->num_params->iNbFwd);
    ...
    if (und->num_params->iPriceVol == 0) // We do an interpolation of the prices
    {
      ...
      dPrecalcPriceStdAlphaInX =
          InstConst->dPrecalcFloorPriceStdAlpha[index_X1];
      ...
      dPrecalcPriceStdAlphaInX =
          InstConst->dPrecalcFloorPriceStdAlpha[index_X2];
      ...
      if ((price1 <= 0.0) && (price2 <= 0.0)) {
        ...
        prices[0] = -price2 /
                        (InstConst->dPrecalcX[index_X1] -
                         InstConst->dPrecalcX[index_X2]) *
                        (X0 - InstConst->dPrecalcX[index_X2]) +
                    price2;
      } else {
      ...
      if (Inst->iNbStrike > 2) {
        index_Fwd1 =
            Get_Index(Fwd, InstConst->dPrecalcFwdCap, und->num_params->iNbFwd);
        if (index_Fwd1 == 0) {
          index_Fwd1 = 1;
        }
        index_Fwd2 = index_Fwd1 - 1;
        ...
        dPrecalcPriceStdAlphaInX =
            InstConst->dPrecalcCapPriceStdAlpha[index_X1];
        price1 = dPrecalcPriceStdAlphaInX[index_Fwd1] *
        ...
        dPrecalcPriceStdAlphaInX =
            InstConst->dPrecalcCapPriceStdAlpha[index_X2];
        price2 = dPrecalcPriceStdAlphaInX[index_Fwd1] *
                     (Fwd - InstConst->dPrecalcFwdCap[index_Fwd2]) +
                 dPrecalcPriceStdInX[index_Fwd2];
    ...
    } else // We do an interpolation of the volatilities
    {
      ...
      if (Fwd < InstConst->dPrecalcFwdFloor[0]) {
        vol1 = dPrecalcPriceStdInX[0];
                 ...
                 InstConst->dPrecalcFwdFloor[und->num_params->iNbFwd - 1]) {
        ...
        // vol1 = (dPrecalcPriceStdInX[index_Fwd1] -
        // dPrecalcPriceStdInX[index_Fwd2]) 	/
      ...
      prices[0] = OPT_LOGVAL_MACRO(3, Fwd, LogFwd, Inst->dStrike[0],
                                   Inst->dLogStrike[0], vol, 0.5 * vol);
      ...
      if (Inst->iNbStrike > 2) {
        ...
        if (Fwd < InstConst->dPrecalcFwdCap[0]) {
          vol1 = dPrecalcPriceStdInX[0];
                   ...
                   InstConst->dPrecalcFwdCap[und->num_params->iNbFwd - 1]) {
          vol1 = dPrecalcPriceStdInX[und->num_params->iNbFwd - 1];
        } else {
          /* interpolation */
          index_Fwd2 = index_Fwd1 - 1;
        ...
        prices[1] = OPT_LOGVAL_MACRO(3, Fwd, LogFwd, Inst->dStrike[1],
                                     Inst->dLogStrike[1], vol, 0.5 * vol);
    ...
    err = FxBetaDLM_Update_InstPrecalc_FromX0(X0, Inst, und->model,
                                              und->num_params, InstConst);
...
Err cpd_fut_intr_val_dlm(
  ...
  *pd_leg = 0.0;
  ...
  for (l = 0; l < num_pd_cpn; l++) {
    ...
    if (eval_const->pd_inst[l].iNbStrike > 0) {
      err = cpd_price_FxInstrument_dlm(fwd, log(fwd), X, und, cpd, l, cpn,
                                       eval_const, prices);
    ...
    case 1:
    case 2:
      floor = OPT_VAL_MACRO(eval_const->pd_floor_type[l], fwd_adj,
                            eval_const->pd_floor_str[l], 1.0, 1.0);
      break;
    case 3:
      floor = prices[0];
      break;
    ...
    case 1:
    case 2:
      cap = OPT_VAL_MACRO(eval_const->pd_cap_type[l], fwd_adj,
                          eval_const->pd_cap_str[l], 1.0, 1.0);
      break;
    case 3:
      cap = prices[eval_const->index_cap[l]];
      break;
    case 4:
      cap = prices[eval_const->index_cap[l]] + eval_const->pd_cap_str[l] -
            fwd_adj;
      break;
    }
  ...
  if (eval_const->fin_not_inst.iNbStrike > 0) {
    err = cpd_price_FxInstrument_dlm(fwd, log(fwd), X, und, cpd, -1, cpn,
                                     eval_const, prices);
  ...
  case 1:
  case 2:
    floor = OPT_VAL_MACRO(eval_const->fin_not_floor_type, fwd_adj,
                          eval_const->fin_not_floor_str, 1.0, 1.0);
    break;
  case 3:
    floor = prices[0];
    break;
  ...
  case 1:
  case 2:
    cap = OPT_VAL_MACRO(eval_const->fin_not_cap_type, fwd_adj,
                        eval_const->fin_not_cap_str, 1.0, 1.0);
    break;
  case 3:
  ...
  *pd_leg += df * (cpn->alpha + cpn->beta * fwd +
  ...
  /*	Initial notional */
  *pd_leg -= eval_const->in_not_pd_dff *
             exp(-eval_const->in_not_pd_gam * Xdomfor[0] -
                 eval_const->in_not_pd_gam2) *
...
Err cpd_fut_total_val_dlm(
    ...
    CPD_STR cpd, CPD_UND und,
    /*	Call */
    int fund_idx, int num_fund_cpn, double fund_not_amt,
    double next_fund_not_amt, int pd_idx, int num_pd_cpn, double pd_not_amt,
    double next_pd_not_amt, int eval_redemption,
    /* Precalculated constants */
    CPD_EVAL_CONST eval_const,
    /* Results */
    int calc_coupon, double *fund_leg, double *pd_leg, double *fee) {
  double df, fwd, fwd_adj, cap, floor;
  PD_EXO_CPN cpn;
  int l;
  double prices[3];
  ...
  if (calc_coupon) {
    ...
    *pd_leg = 0.0;
    ...
    if (eval_redemption) {
      ...
      case 4:
        cap = prices[eval_const->fin_not_index_cap] +
              eval_const->fin_not_cap_str - fwd_adj;
        break;
      }
      ...
      *pd_leg += eval_const->next_pd_dff *
                 exp(-eval_const->next_pd_gam * Xdomfor[0] -
                     eval_const->next_pd_gam2) *
                 next_pd_not_amt;
    }
  }
...
Err cpd_payoff_4_3dfx_mc_dlm(
    /* Event */
    double evt_date, double evt_time, void *func_parm, double Xdom, double Yfor,
    double Zfx,
    /* Results */
  ...
  *stop_path = 0;
  ...
  err = cpd_fut_intr_val_dlm(Xdomfor, Zfx, fx, cpd, und, call->fund_idx,
                             call->num_fund_cpn, call->fund_not_amt,
                             call->pd_idx, call->num_pd_cpn, call->pd_not_amt,
                             eval_const, &fund_leg, &pd_leg);
...
Err cpd_payoff_total_4_3dfx_mc_dlm(
  ...
  *stop_path = 0;
  ...
  err = cpd_fut_total_val_dlm(
      Xdomfor, Zfx, fx, cpd, und, call->fund_idx, num_fund_cpn,
      call->fund_not_amt, next_call->fund_not_amt, call->pd_idx, num_pd_cpn,
      call->pd_not_amt, next_call->pd_not_amt, eval_redemption, eval_const,
      calc_coupon, &fund_leg, &pd_leg, &fee);
...
Err cpd_payoff_4_3dfx_tarn(
  ...
  *stop_path = 0;
  ...
  err = cpd_current_pd_cpn_val(Xdomfor, Zfx, spot_fx, cpd, call_idx, 1,
                               call->pd_not_amt, eval_const, &partial_pd_leg);

  if (err)
    return err;

  CPNPAID +=
      REMNO * partial_pd_leg; // we add the current coupon to the coupons paid

  if (call->bar_type == 0 &&
      CPNPAID < call->orig_barrier - SMO) // bar_type 0: up and in
    return NULL;

  err = cpd_fut_intr_val(Xdomfor, Zfx, spot_fx, cpd, call->fund_idx,
                         call->num_fund_cpn, call->fund_not_amt, call->pd_idx,
                         call->num_pd_cpn, call->pd_not_amt, eval_const,
                         &fund_leg, &pd_leg);
...
Err cpd_payoff_4_3dfx_mc(
  ...
  *stop_path = 0;
  ...
  fx_now = Zfx + eval_const->spot_fx_dff_log_ratio -
           eval_const->spot_fx_for_gam * Yfor +
           eval_const->spot_fx_dom_gam * Xdom + eval_const->spot_fx_cvx;
...
Err cpd_payoff_4_3dfx_optiFx_mc(
  ...
  *stop_path = 0;
  ...
  if (cpd->call[call_idx].cxxall_type == 1 &&
      ((call->bar_type == 0 && fx_now < call->barrier - SMO) ||
       (call->bar_type == 1 && fx_now > call->barrier + SMO))) {
    return NULL;
  }
...
/*	Launch the mc */
Err cpd_launch_mc(CPD_STR cpd, CPD_UND und, CPD_MC_ARG mc_arg,
                  /*	Result */
                  double *prem, double *std) {
  ...
  if (!cpd->call[0].TARN_Do) {
    if (und->use_beta_dlm) {
      err = mc_main_3dBetaDLMfx(
          mc_arg->npaths, mc_arg->num_col, mc_arg->time, mc_arg->date,
          mc_arg->nb_dates, mc_arg->dom_fwd, mc_arg->dom_std,
          mc_arg->for_fwd_const, mc_arg->for_fwd_lin, mc_arg->for_std,
          mc_arg->fx_fwd, mc_arg->fx_std, mc_arg->dom_bond_pay,
          mc_arg->dom_beta_pay, mc_arg->dom_for_cov, mc_arg->dom_fx_cov,
          mc_arg->for_fx_cov, mc_arg->void_prm, mc_arg->do_pecs, 0, NULL, NULL,
          cpd_init_4_3dfx_mc, cpd_payoff_total_4_3dfx_mc_dlm, temp_val);

      df = swp_f_df(und->today, und->model->lTStarDate, und->dom_yc);
    } else {
      err = mc_main_3dfx(
          mc_arg->npaths, mc_arg->num_col, mc_arg->time, mc_arg->date,
          mc_arg->nb_dates, mc_arg->dom_ifr, mc_arg->dom_fwd, mc_arg->dom_std,
          mc_arg->dom_phi, mc_arg->dom_beta, mc_arg->dom_bond_pay,
          mc_arg->dom_beta_pay, mc_arg->for_ifr, mc_arg->for_fwd,
          mc_arg->for_std, mc_arg->for_phi, mc_arg->for_beta, mc_arg->fx_fwd,
          mc_arg->fx_std, mc_arg->dom_for_cov, mc_arg->dom_fx_cov,
          mc_arg->for_fx_cov, mc_arg->void_prm, mc_arg->dom_lam,
          mc_arg->for_lam, mc_arg->corr_dom_for[0], mc_arg->corr_dom_fx[0],
          mc_arg->corr_for_fx[0], mc_arg->spot_fx, mc_arg->dom_yc,
          mc_arg->for_yc, mc_arg->do_pecs, 0, NULL, NULL, cpd_init_4_3dfx_mc,
          cpd_payoff_4_3dfx_mc, temp_val, NULL);

      df = swp_f_df(und->today, mc_arg->date[mc_arg->nb_dates - 1] + 1.0e-16,
                    und->dom_yc);
    }
  } else {
    if (und->use_beta_dlm) {
      ...
      err = mc_main_3dfx(
          ...
          mc_arg->corr_for_fx[0], mc_arg->spot_fx, mc_arg->dom_yc,
          mc_arg->for_yc, mc_arg->do_pecs, 0, NULL, NULL, cpd_init_4_3dfx_mc,
          cpd_payoff_4_3dfx_tarn, temp_val, NULL);
...
/*	Launch the mc */
Err cpd_launch_opti_mc(CPD_STR cpd, CPD_UND und, CPD_MC_ARG mc_arg,
                       /*	Result */
                       double *prem, double *std, int do_infos,
                       double ***optim_bar) {
  ...
  if (cpd->call[0].ex_time < 1.0E-10) {
    for (i = 0; i < mc_arg->nb_dates; i++) {
      optimise[i] = 1 - cpd->call[i].cxxall_type;
    }
  } else {
    optimise[0] = 0;
    for (i = 1; i < mc_arg->nb_dates; i++) {
      optimise[i] = 1 - cpd->call[i - 1].cxxall_type;
    }
  }
  ...
  *optim_bar = (double **)dmatrix(0, mc_arg->nb_dates - 1, 0,
                                  do_infos * (3 + 2 * mc_arg->nb_dates) +
                                      (1 - do_infos) * 1);
  ...
  err = mc_main_3dfx(
      ...
      mc_arg->dom_ifr, mc_arg->dom_fwd, mc_arg->dom_std, mc_arg->dom_phi,
      mc_arg->dom_beta, mc_arg->dom_bond_pay, mc_arg->dom_beta_pay,
      ...
      mc_arg->dom_fx_cov, mc_arg->for_fx_cov, mc_arg->void_prm, mc_arg->dom_lam,
      mc_arg->for_lam, mc_arg->corr_dom_for[0], mc_arg->corr_dom_fx[0],
      mc_arg->corr_for_fx[0], mc_arg->spot_fx, mc_arg->dom_yc, mc_arg->for_yc,
      mc_arg->do_pecs, 1, optimise, params, cpd_init_4_3dfx_mc,
      cpd_payoff_4_3dfx_optiFx_mc, temp_val, *optim_bar);
...
Err cpd_payoff_4_3dfx_BetaDLM_tree(
  ...
  double fx_I, fx_J, fx_K, fx_DI_Const, fx_DJ_Const, fx_DK_Const, fx_DI_Lin,
      fx_DJ_Lin, fx_DK_Lin;
  ...
  double fin_not_log_fwd_I, fin_not_log_fwd_J, fin_not_log_fwd_K,
      fin_not_log_fwd_DI, fin_not_log_fwd_DJ, fin_not_log_fwd_DK;
  ...
  /* For Fx */
  fx_I = eval_const->pd_S_const +
         X_I * (eval_const->pd_S_lin + X_I * eval_const->pd_S_quad) +
         eval_const->pd_S_dom * sv[0][0][0][0] +
         eval_const->pd_S_for * sv[0][0][0][1];
  fx_J = fx_I;
  fx_K = fx_I;
  ...
  for (l = 0; l < num_pd_cpn; l++) {
    ...
    eval_const->pd_log_fwd_I[l] =
        log(eval_const->pd_fwd_dff_ratio[l]) -
        eval_const->pd_fwd_for_gam[l] * sv[0][0][0][1] +
        eval_const->pd_fwd_dom_gam[l] * sv[0][0][0][0] +
        eval_const->pd_fwd_cvx[l];
      ...
      for (k = 0; k < n3; k++) {
        ...
        /*fx = exp (eval_const->pd_S_const + X * (eval_const->pd_S_lin + X *
           eval_const->pd_S_quad)
                + eval_const->pd_S_dom * svijk[0] + eval_const->pd_S_for *
           svijk[1]);*/
        fx = exp(fx_K);
  ...
  eval_const->fund_gam2[l])
        ...
        for (l = 0; l < num_pd_cpn; l++) {
          ...
          */
          ...
          if (eval_const->pd_inst[l].iNbStrike > 0) {
            err = cpd_price_FxInstrument_dlm(fwd, log_fwd, X, und, cpd, l, cpn,
                                             eval_const, prices);
        ...
        if (eval_redemption) {
          ...
          if (eval_const->fin_not_inst.iNbStrike > 0) {
            err = cpd_price_FxInstrument_dlm(fwd, log_fwd, X, und, cpd, -1, cpn,
                                             eval_const, prices);

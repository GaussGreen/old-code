...
/* 1) If the argument "greek" is set to "PREMIUM",
   ...
   this function returns a pointer *result that contains the values of the probability cumulative
   density associated to the input parameters in correspondence to a string of increasing strikes to
   be input in *Strikes.
...
//    double  Int, Int1, NodeLeft, NodeRight, NodeLeft1, NodeRight1, dScaling = 1., dGear, Gamma,
//                                                                  UpperBound_opt;
//    Err    err = NULL;
//    double HSigma, HAlpha, Hb, HBeta, HSigmaIfty, HRho;
//
//    err = ConvertSABRParamInHestonParam(
//        Forward, Maturity, Sigma, Alpha, b, Beta, Rho, &HSigma, &HAlpha, &Hb, &HBeta, &HRho);
//
//    HSigmaIfty  = HSigma;
//    isVolInfFix = 1;
//
...
//
//            ImAppr2 =
//                -(log(Forward) + (esp2 - 1) * HSigma * HSigma / 2. / (Hb - HAlpha * HRho) +
//                  Maturity * a / 2. / (Hb - HAlpha * HRho) +
//                  a * (esp2 - 1.) / 2 / (Hb - HAlpha * HRho) / (Hb - HAlpha * HRho));
//            ImAppr1 =
//                -(log(Forward) + HSigma * HSigma / Hb * (1. - esp) - Maturity * a / 2. / Hb +
//                  a / 2. / Hb / Hb * (1. - esp));
//
//            for (j = 1; j <= nStrikes; j++)
//            {
...
//                //		Strike_used[j] = DMAX(Strike_used[j], DMAX(0.0015,Forward -
//                //nStdDev*dStdDev));
//

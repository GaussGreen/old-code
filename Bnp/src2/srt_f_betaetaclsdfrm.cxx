/* ===================================================================================
   FILENAME:      srt_f_betaetaclsdfrm.cxx

   PURPOSE:       Compute swaption        , bond option        ,caps/floors
   prices in betaeta        , via a closed form analytical solution.
   ===================================================================================
 */
...
/* Evaluates cash flows for a swaption as sum(CF(t        ,x)/N(t
 * ,x))*density(x) */
static double swaption_cfs(double y_bar);

/* Evaluates cash flows for a cap/floor as sum(CF(t        ,x)/N(t
 * ,x))*density(x) */
...
/* -------------------------------------------------------------------------
   ...
   PURPOSE:  betaeta model --- closed form prices for swaptions        , bond
   options , caps and floors. When dealing with a swaption        , a cap or a
   floor        , the bond strike has no impact. Please note that if it is an
   option on a bond        , the bond strike is assumed to be a clean strike.

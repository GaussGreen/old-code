...
/* ==========================================================================
   ...
   PURPOSE:		Functions for the GRFN payoff evaluation in a 3FQuanto model
								...
								double		****prod_val)
{
	...
	if (total->num_dom_df > 0 && total->dom_idx != -1)
	{
		...
		for (i=0; i<total->num_dom_df; i++)
		{
			if (lgm2F_is_dom_for == 0)
			{
				...
				dom_1f_gam[i] = (1.0 - exp ( - r1_lambda * total->dom_df_tms[i] )) / r1_lambda;
				dom_1f_gam2[i] = 0.5 * dom_1f_gam[i] * dom_1f_gam[i] * r1_phi;
				dom_2f_gam[i] = (1.0 - exp ( - r2_lambda * total->dom_df_tms[i] )) / r2_lambda;
			...
			else
			{
				...
				dom_1f_gam[i] = (1.0 - exp ( - r3_lambda * total->dom_df_tms[i] )) / r3_lambda;
	...
	if (total->num_fx_df > 0 && total->fx_idx != -1)
	{
		...
		for (i=0; i<total->num_fx_df; i++)
		{
			...
			if (lgm2F_is_dom_for == 0)
			{
				...
				fx_1f_gam[i] = (1.0 - exp ( - r1_lambda * total->fx_df_tms[i] )) / r1_lambda;
				fx_1f_gam2[i] = 0.5 * fx_1f_gam[i] * fx_1f_gam[i] * r1_phi;
				fx_2f_gam[i] = (1.0 - exp ( - r2_lambda * total->fx_df_tms[i] )) / r2_lambda;
			...
			else
			{
				...
				fx_1f_gam[i] = (1.0 - exp ( - r3_lambda * total->fx_df_tms[i] )) / r3_lambda;
	...
	if (total->num_for_df > 0 && total->for_idx != -1)
	{
		...
		for (i=0; i<total->num_for_df; i++)
		{
			if (lgm2F_is_dom_for == 1)
			{
				...
				for_1f_gam[i] = (1.0 - exp ( - r1_lambda * total->for_df_tms[i] )) / r1_lambda;
				for_1f_gam2[i] = 0.5 * for_1f_gam[i] * for_1f_gam[i] * r1_phi;
				for_2f_gam[i] = (1.0 - exp ( - r2_lambda * total->for_df_tms[i] )) / r2_lambda;
			...
			else
			{
				...
				for_1f_gam[i] = (1.0 - exp ( - r3_lambda * total->for_df_tms[i] )) / r3_lambda;
	...
	for (i=0; i<n1; i++)
	{
		for (j=0; j<n2; j++)
		{
			for (k=0; k<n3; k++)
			{
				// local->smp.und[total->fx_idx].sv[SPOT] = spot_fx * exp (sv[i][j][k][2]) + 1.0e-12;
					...
					for (l=0; l<total->num_dom_df; l++)
					{
						if (lgm2F_is_dom_for == 0)
						{
							local->evt->df[total->dom_idx][l]
								= dom_dff[l] * exp(- dom_1f_gam[l] * sv[i][j][k][0] - dom_1f_gam2[l] 
						...
						{
							local->evt->df[total->dom_idx][l]
								= dom_dff[l] * exp(- dom_1f_gam[l] * sv[i][j][k][2] - dom_1f_gam2[l]);
						}
					}
				}
					...
					for (l=0; l<total->num_fx_df; l++)
					{
						if (lgm2F_is_dom_for == 0)
						{
							local->evt->df[total->fx_idx][l]
								= fx_dff[l] * exp(- fx_1f_gam[l] * sv[i][j][k][0] - fx_1f_gam2[l] 
						...
						{
							local->evt->df[total->fx_idx][l]
								= fx_dff[l] * exp(- fx_1f_gam[l] * sv[i][j][k][2] - fx_1f_gam2[l]);
						}
					}
				}
					...
					for (l=0; l<total->num_for_df; l++)
					{
						if (lgm2F_is_dom_for == 0)
						{
							local->evt->df[total->for_idx][l]
								= for_dff[l] * exp(- for_1f_gam[l] * sv[i][j][k][2] - for_1f_gam2[l]);

						}
						else
						{
							local->evt->df[total->for_idx][l]
								= for_dff[l] * exp(- for_1f_gam[l] * sv[i][j][k][0] - for_1f_gam2[l] 
...
FREE_RETURN:
	...
	if (dom_dff) free_dvector (dom_dff, 0, total->num_dom_df-1);
	if (dom_1f_gam)	free_dvector (dom_1f_gam, 0, total->num_dom_df-1);
	if (dom_1f_gam2) free_dvector (dom_1f_gam2, 0, total->num_dom_df-1);
	if (dom_2f_gam)	free_dvector (dom_2f_gam, 0, total->num_dom_df-1);
	if (dom_2f_gam2) free_dvector (dom_2f_gam2, 0, total->num_dom_df-1);
	if (dom_cross_gam) free_dvector (dom_cross_gam, 0, total->num_dom_df-1);

	if (fx_dff) free_dvector (fx_dff, 0, total->num_fx_df-1);
	if (fx_1f_gam) free_dvector (fx_1f_gam, 0, total->num_fx_df-1);
	if (fx_1f_gam2) free_dvector (fx_1f_gam2, 0, total->num_fx_df-1);
	if (fx_2f_gam) free_dvector (fx_2f_gam, 0, total->num_fx_df-1);
	if (fx_2f_gam2) free_dvector (fx_2f_gam2, 0, total->num_fx_df-1);
	if (fx_cross_gam) free_dvector (fx_cross_gam, 0, total->num_fx_df-1);

	if (for_dff) free_dvector (for_dff, 0, total->num_for_df-1);
	if (for_1f_gam) free_dvector (for_1f_gam, 0, total->num_for_df-1);
	if (for_1f_gam2) free_dvector (for_1f_gam2, 0, total->num_for_df-1);
	if (for_2f_gam) free_dvector (for_2f_gam, 0, total->num_for_df-1);
	if (for_2f_gam2) free_dvector (for_2f_gam2, 0, total->num_for_df-1);
	if (for_cross_gam) free_dvector (for_cross_gam, 0, total->num_for_df-1);

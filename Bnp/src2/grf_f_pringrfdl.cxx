...
Err grf_f_princells(FILE *out, Date *eventdates, GrfnCell **gcells, long nr,
                    long nc) {
  long i, j;
...
Err grf_f_pringrfdl(FILE *out, GrfnDeal *gd) {
  ...
  err = grf_f_pringrfrng(out, gd->num_grng, gd->grng);
  if (err)
    return err;

  err = grf_f_prinauxrng(out, gd->auxwidth, gd->auxlen, gd->aux);
  if (err)
    return err;

  err = grf_f_princells(out, gd->event_dates, gd->gcells, gd->sslength,
                        gd->sswidth);
  if (err)
    return err;
...
Err grf_f_prgrdl(String filename, int numeventdates, Date *eventdates,
  ...
  grfn_copy_to_GrfnDeal(&gd, nrows, ncols, sprdsht, eventdates, numgrng, grng,
                        auxwidth, auxlen, aux);

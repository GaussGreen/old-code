...
Err MY_LGM2F_Preliminary_Computation_For_Df(
    long today, long eventDate, long* Dates, MYLGM2F mylgm2f, TermStruct* l, char* yc)
{
    Err        err = NULL;
...
Err MY_LGM2F_Compute_Mean_Variance_Covariance(
    long today, long eventDate, long probaDate, MYLGM2F mylgm2f, TermStruct* l, char* yc)
{
    SrtTFTSVec FVec, PsiVec1, PsiVec2;
    SrtTFTSMat GVec;
    double     lam1, lam2;
...
void MY_LGM2F_Compute_Df_From_Factors_Value(
    double  fact1,
    double  fact2,
    MYLGM2F mylgm2f)  //(long today, long eventDate, double fact1, double fact2, MYLGM2F mylgm2f,
                      //TermStruct* l, char *yc)
{
    ...
    for (i = 0; i < mylgm2f->NDates; ++i)
    {
         ...
         */
        mylgm2f->Df[i] =
            mylgm2f->AdjDf[i] * exp(-mylgm2f->lam1[i] * fact1) * exp(-mylgm2f->lam2[i] * fact2);
    }
}
...
Err SWAP_FOR_LGM2F_Preliminary_Computation(
    long today, long eventDate, swap_for_lgm2f* swapForLgm2f, TermStruct* l, char* yc)
{
    Err err = NULL;

    err = MY_LGM2F_Preliminary_Computation_For_Df(
        today, eventDate, swapForLgm2f->fix_pay_dates, &(swapForLgm2f->fix_pay_lgm2f), l, yc);
    if (err)
    {
    ...
    err = MY_LGM2F_Preliminary_Computation_For_Df(
        today, eventDate, swapForLgm2f->float_end_dates, &(swapForLgm2f->float_end_lgm2f), l, yc);
    if (err)
    {
    ...
    err = MY_LGM2F_Preliminary_Computation_For_Df(
        today, eventDate, swapForLgm2f->float_pay_dates, &(swapForLgm2f->float_pay_lgm2f), l, yc);
    if (err)
    {
...
double SWAP_FOR_LGM2F_SwapRate_From_Factors_Value(
    double          fact1,
    double          fact2,
    swap_for_lgm2f* swapForLgm2f)  //(long today, long eventDate, double fact1, double fact2,
                                   //swap_for_lgm2f *swapForLgm2f, TermStruct* l, char *yc)
{
    long   i;
    double FloatPV;
    double LVL;
    ...
    MY_LGM2F_Compute_Df_From_Factors_Value(
        fact1,
        fact2,
        &(swapForLgm2f->fix_pay_lgm2f));  //(today, eventDate, fact1, fact2,
                                          //&(swapForLgm2f->fix_pay_lgm2f), l, yc);

    MY_LGM2F_Compute_Df_From_Factors_Value(
        fact1,
        fact2,
        &(swapForLgm2f->float_start_lgm2f));  //(today, eventDate, fact1, fact2,
                                              //&(swapForLgm2f->float_start_lgm2f), l, yc);

    MY_LGM2F_Compute_Df_From_Factors_Value(
        fact1,
        fact2,
        &(swapForLgm2f->float_end_lgm2f));  //(today, eventDate, fact1, fact2,
                                            //&(swapForLgm2f->float_end_lgm2f), l, yc);

    MY_LGM2F_Compute_Df_From_Factors_Value(
        fact1,
        fact2,
        &(swapForLgm2f->float_pay_lgm2f));  //(today, eventDate, fact1, fact2,
    ...
    for (i = 0; i < max(swapForLgm2f->fix_NDates, swapForLgm2f->float_NDates); ++i)
    {
        ...
        {
            FloatPV += (swapForLgm2f->float_pay_lgm2f).Df[i] * (swapForLgm2f->float_cvgs)[i] *
                       swapForLgm2f->float_spreads[i];
            FloatPV +=
                (swapForLgm2f->float_pay_lgm2f).Df[i] *
                ((swapForLgm2f->float_start_lgm2f).Df[i] / (swapForLgm2f->float_end_lgm2f).Df[i] -
                 1);
        }
    }
...
Err Fill_VolBondCoupon(
    ...
    double         notional,
    long           first_fixingDate,
    long           first_startDate,
    long           first_endDate,
    long           second_fixingDate,
    long           second_startDate,
    long           second_endDate,
    long           payment_Date,
    double         alphaC,
    double         alphaP,
    ...
    char*          yc)
{
    ...
    firstswap =
        max(0.00000001,
            SWAP_FOR_LGM2F_SwapRate_From_Factors_Value(fact1, fact2, &(volBondCpn->firstSwap)));

    secondswap =
        max(0.00000001,
            SWAP_FOR_LGM2F_SwapRate_From_Factors_Value(fact1, fact2, &(volBondCpn->secondSwap)));

    *Payoff = volBondCpn->alphaC *

...
void MY_LGM2F_Compute_Df_From_Factors_Value(
    ...
    MYLGM2F mylgm2f) //(long today  , long eventDate  , double fact1  , double
                     //fact2  , MYLGM2F mylgm2f  , TermStruct* l  , char *yc)
{
  long i;

  for (i = 0; i < mylgm2f->NDates; ++i) {
    /*		mylgm2f->Df[i] = (mylgm2f->initDf[i] / swp_f_df(today  , eventDate  ,
     * yc)) exp(-0.5 * mylgm2f->sigmaSquare[i]) exp(-mylgm2f->lam1[i] * fact1)
...
double SWAP_FOR_LGM2F_SwapRate_From_Factors_Value(
    double fact1, double fact2,
    swap_for_lgm2f *swapForLgm2f) //(long today  , long eventDate  , double
                                  ...
                                  //*swapForLgm2f  , TermStruct* l  , char *yc)
{
  ...
  MY_LGM2F_Compute_Df_From_Factors_Value(
      fact1, fact2,
      &(swapForLgm2f
            ->fix_pay_lgm2f)); //(today  , eventDate  , fact1  , fact2  ,
                               //&(swapForLgm2f->fix_pay_lgm2f)  , l  , yc);

  MY_LGM2F_Compute_Df_From_Factors_Value(
      fact1, fact2,
      &(swapForLgm2f
            ->float_start_lgm2f)); //(today  , eventDate  , fact1  , fact2  ,
                                   //&(swapForLgm2f->float_start_lgm2f)  , l  ,
                                   //yc);

  MY_LGM2F_Compute_Df_From_Factors_Value(
      fact1, fact2,
      &(swapForLgm2f
            ->float_end_lgm2f)); //(today  , eventDate  , fact1  , fact2  ,
                                 //&(swapForLgm2f->float_end_lgm2f)  , l  , yc);

  MY_LGM2F_Compute_Df_From_Factors_Value(
      fact1, fact2,
      &(swapForLgm2f
            ->float_pay_lgm2f)); //(today  , eventDate  , fact1  , fact2  ,
                                 //&(swapForLgm2f->float_pay_lgm2f)  , l  , yc);

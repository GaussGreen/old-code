...
Err srt_f_cpi_forward(char* price_index_und_name, Date fixing_date, Date pay_date, double* fwd_cpi)
{
    Err         err = NULL;
    ...
    err = srt_f_vasicek_discount_factor(settlt_date, real_rate_und, &df_real_settdt);
    if (err)
        return err;

    err = srt_f_vasicek_discount_factor(fixing_date, real_rate_und, &df_real_fixingdt);
    if (err)
        return err;
    ...
    double*   model_daily_ref_index)
{
    ...
    double    df_nom_ipc_date_hm, df_nom_ipc_date_lm, df_real_settlt, df_real_ipc_date_lm,
        df_real_ipc_date_hm;
    ...
    {
        return serror(
            "can not find vasicek underlying in "
    ...
    {
        return serror(
            "can not find price index und in "
            "srt_f_vasicek_mean_rev_lvl_and_initial_cond_calib_func");
    }
    ...
    /* get the forward real rate discount factor */
    err = srt_f_vasicek_discount_factor((long)settlt_date, real_rate_und, &df_real_settlt);
    if (err)
        return err;

    err = srt_f_vasicek_discount_factor(ipc_date_lm, real_rate_und, &df_real_ipc_date_lm);
    if (err)
        return err;

    err = srt_f_vasicek_discount_factor(ipc_date_hm, real_rate_und, &df_real_ipc_date_hm);
    if (err)
        return err;
    ...
    (*model_daily_ref_index) =
        get_spot_from_fxund(price_index_und) * (ipc_lm + interp_coeff * (ipc_hm - ipc_lm));
    ...
    double*   calibrated_vasicek_init_cond)
{
    ...
    if (!und)
        return serror(
            "can not get underlying in srt_f_vasicek_calibrated_mean_rev_lvl_and_initial_cond");
    ...
    err = srt_f_get_vasicek_init_cond(
        (settlt_date - date_today) * YEARS_IN_DAY, ts, &vasicek_init_cond);

...
static Err srt_f_lgm_spotratemoments(
    SrtUndPtr undptr, Ddate fixing, Ddate pay, double exp[2], double var[2][2])
{
    long       today_date;
    ...
    else if (mdldim == TWO_FAC)
    {
        ...
        exp[FIRST] =
            -(tf_psi_pay[FIRST] - tf_psi_fix[FIRST]) * tf_f_fix[FIRST] * tf_g_fix[FIRST][FIRST] -
            (tf_psi_pay[SECOND] - tf_psi_fix[SECOND]) * tf_f_fix[FIRST] * tf_g_fix[FIRST][SECOND];
        exp[SECOND] =
            -(tf_psi_pay[SECOND] - tf_psi_fix[SECOND]) * tf_f_fix[SECOND] *
                tf_g_fix[SECOND][SECOND] -
            (tf_psi_pay[FIRST] - tf_psi_fix[FIRST]) * tf_f_fix[SECOND] * tf_g_fix[SECOND][FIRST];
        var[FIRST][FIRST]   = tf_f_fix[FIRST] * tf_f_fix[FIRST] * tf_g_fix[FIRST][FIRST];
        var[SECOND][SECOND] = tf_f_fix[SECOND] * tf_f_fix[SECOND] * tf_g_fix[SECOND][SECOND];
        var[FIRST][SECOND]  = tf_f_fix[FIRST] * tf_f_fix[SECOND] * tf_g_fix[FIRST][SECOND];
...
static Err srt_f_lgm_zeroratemoments(
    SrtUndPtr undptr, Ddate fixing, Ddate start_end[2], Ddate pay, double* exp, double* var)
{
    YTatt_param YT[2];
    ...
    else if (mdldim == TWO_FAC)
    {
        *exp = YT[END].fwd_zero_rate - YT[START].fwd_zero_rate
               ...
               + (YT[END].x_coeff[SECOND] - YT[START].x_coeff[SECOND]) * spot_exp[SECOND];

        *var = spot_var[FIRST][FIRST] * (YT[END].x_coeff[FIRST] - YT[START].x_coeff[FIRST]) *
                   (YT[END].x_coeff[FIRST] - YT[START].x_coeff[FIRST])

               + spot_var[SECOND][SECOND] * (YT[END].x_coeff[SECOND] - YT[START].x_coeff[SECOND]) *
                     (YT[END].x_coeff[SECOND] - YT[START].x_coeff[SECOND])

               + spot_var[FIRST][SECOND] * (YT[END].x_coeff[FIRST] - YT[START].x_coeff[FIRST]) *
                     (YT[END].x_coeff[SECOND] - YT[START].x_coeff[SECOND])

               + spot_var[SECOND][FIRST] * (YT[END].x_coeff[SECOND] - YT[START].x_coeff[SECOND]) *
                     (YT[END].x_coeff[FIRST] - YT[START].x_coeff[FIRST]);
    }
...
/*	compute covariance of spot rate(s) at different times
        model: LGM 1F and 2F
        undptr must point to ir underlying of type LGM */
static Err srt_f_lgm_spotratecovar(SrtUndPtr undptr, Ddate fix1, Ddate fix2, double covar[2][2])
{
    long       today_date;
    double     time1, time2;
    ...
    double          fixed_strike)
{
    ...
    /* strike */
    err = srt_f_lgm_zeroratemoments(
        undptr, str_fix, str_start_end, pay_date, &str_frt_exp, &str_frt_var);
    if (err)
    {
        return serror(err);
    }
    /* spot */
    err = srt_f_lgm_zeroratemoments(
        undptr, spot_fix, spot_start_end, pay_date, &spot_frt_exp, &spot_frt_var);
    if (err)
    {
        return serror(err);
    }
    /* cross */
    err = srt_f_lgm_zeroratecovar(
        undptr, str_fix, spot_fix, str_start_end, spot_start_end, &str_spot_frt_covar);
    if (err)
    {
        return serror(err);
    }
    ...
    /* general case: strike Libor is not deterministic, call srt_f_optsprnum */
    {
        ...
        {
            *premium = srt_f_optsprnum(Fx, Fy, sx, sy, rho, 1.0, a, b, k, 1.0, SRT_CALL, PREMIUM) +
                       srt_f_optsprnum(Fx, Fy, sx, sy, rho, 1.0, a, b, k, 1.0, SRT_PUT, PREMIUM);
        }
        else if (rec_pay == SRT_RECEIVER)
        {
            *premium = srt_f_optsprnum(Fx, Fy, sx, sy, rho, 1.0, a, b, k, 1.0, SRT_PUT, PREMIUM);
        }
       ...
       call srt_f_optblksch */
    {
        if (rec_pay == SRT_STRADDLE)
        {
            *premium = srt_f_optblksch(a * Fx, b * Fy + k, sx, 1.0, 1.0, SRT_CALL, PREMIUM) +
                       srt_f_optblksch(a * Fx, b * Fy + k, sx, 1.0, 1.0, SRT_PUT, PREMIUM);
        }
        else if (rec_pay == SRT_RECEIVER)
        {
            *premium = srt_f_optblksch(a * Fx, b * Fy + k, sx, 1.0, 1.0, SRT_PUT, PREMIUM);
        }
        else if (rec_pay == SRT_PAYER)
        {
            *premium = srt_f_optblksch(a * Fx, b * Fy + k, sx, 1.0, 1.0, SRT_CALL, PREMIUM);

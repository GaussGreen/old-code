...
static Err american_bondoption_payofffunc(double InOutMoneyness, double Stdev,
                                          double *payoff) {
  double PV;
  Err err = NULL;
...
Err srt_f_find_american_bond_option_exercise_frontier_index(
    double *InOutMoneyness, double Stdev, long max_index, double *PrevExPrePtr,
    double *Payoff,

    long *imin, double *iEx) {
  Err err = NULL;
  ...
  (*iEx) =
      (*imin) - 1 + Array[(*imin) - 1] / (Array[(*imin) - 1] + Array[(*imin)]);
...
Err srt_f_american_bondoption_exercise_premuim(
    ...
    /*OUTPUTS */
    long *MaxIndexOutput, double **xExBdryOutput, double **EarlyExPr) {

  Err err = NULL;
  double *xExBdry, *CurExPrePtr, *PrevExPrePtr, prob[3];
  long MinNode = 350, MaxNode = 1000, MaxIndex, NumTimeStep, i, j, imin;
  double TimeStep = 1.0 / 24, dx, Stdev, *InOutMoneyness, *Payoff, iEx,
         YrtoAmExpDate;
  ...
  dx = TREE_MESH_SPACING *
       sqrt(TimeStep); /* TO VERIFY THE STABILITY CONDITION */
...
Err XLAmericanBondOptionExPremuim(long EvalDate, long AmExpDate,

                                  long *MaxIndex, double **xExBdryOutput,
                                  double **EarlyExPr) {
  Err err = NULL;

  err = srt_f_american_bondoption_exercise_premuim(
      EvalDate, AmExpDate, &american_bondoption_payofffunc,

      MaxIndex, xExBdryOutput, EarlyExPr);
  if (err)
    return err;

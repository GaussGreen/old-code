...
{
	(*PV) = InOutMoneyness*norm_accurate(InOutMoneyness/Stdev) + Stdev*norm(InOutMoneyness/Stdev);

	return NULL;
}
 
static Err american_bondoption_payofffunc(double			InOutMoneyness,
										  double			Stdev,
										  double			*payoff)
{
	double	PV;
...
Err srt_f_find_american_bond_option_exercise_frontier_index(double	*InOutMoneyness,
													  double	Stdev,
													  long		max_index,
													  double	*PrevExPrePtr,
													  double	*Payoff,
													  ...
													  double	*iEx)
{
	...
	for(j = IndexMin; j <= IndexMax; j++) Array[j] = (PrevExPrePtr[j]-Payoff[j]);

	(*imin) = IndexMin;
	while ((imax - (*imin)) > 1)
	{
		k = (imax + (*imin)) >> 1;
		if (RefVal > Array[k]) (*imin) = k;
		else if (RefVal < Array[k]) imax = k;
		else (*imin) = k;
	}

	(*iEx) = (*imin)-1 + Array[(*imin)-1]/(Array[(*imin)-1] + Array[(*imin)]);

	if(Array) free_dvector(Array,IndexMin,IndexMax); Array = NULL;
...
Err srt_f_american_bondoption_exercise_premuim(Date		EvalDate, /* EVALUATION DATE */
											   ...
											   double	**EarlyExPr)
{
	...
	long		MinNode = 350, MaxNode = 1000, MaxIndex,
				NumTimeStep,i,j,imin;
	...
	for(j = - MaxIndex; j <= MaxIndex; j++) InOutMoneyness[j] = j*dx; 
	...
	for(i = 1; i <= NumTimeStep; i++) /* LOOP ON THE TIME STEP */
	{
		...
		{
			CurExPrePtr[j] = prob[0]*PrevExPrePtr[j-1] 
							+ prob[1]*PrevExPrePtr[j]
		...
		if(i > 1)
		{
			err = srt_f_find_american_bond_option_exercise_frontier_index(InOutMoneyness,
																	Stdev,
																	MaxIndex,
																	PrevExPrePtr,
																	Payoff,
	...
	if(PrevExPrePtr) free_dvector(PrevExPrePtr,-MaxIndex,MaxIndex);PrevExPrePtr = NULL;
	if(Payoff) free_dvector(Payoff,-MaxIndex,MaxIndex);Payoff = NULL;
	if(InOutMoneyness) free_dvector(InOutMoneyness,-MaxIndex,MaxIndex);InOutMoneyness = NULL;
...
Err XLAmericanBondOptionExPremuim(long		EvalDate,
								  long		AmExpDate,
								  ...
								  double	**EarlyExPr)
{
	...
	err = srt_f_american_bondoption_exercise_premuim(EvalDate,
													AmExpDate,

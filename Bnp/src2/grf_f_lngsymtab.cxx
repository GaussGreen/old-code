...
********************************************************************************
    ...
    DESCRIPTION     :   This module conatins a static variable which contains
                        ...
                        and varibales including their data type        , number
...
...
     ...
     NOTE           :   Strings like "ACT/365"        , are not represented
explicitly in the symbol table. They are checked for before searching the symbol
table. In interp_comll_name() these strings        , are translated into
enumerated types and passed around in the "ival" field of a COMLL. We know they
have this particular type (and are not integers) by the type of the comll (e.g.
comll_basis)
...
static GrfnSymbol grfn_symtab[] = {
    ...
    0,
    0,
    "IF(expr1        , expr2        , expr3): if expr1 > .5 then expr2 else "
    "expr3",
    ...
    0,
    0,
    "Black_Scholes Sabr : BSSABR(f        ,k        ,betavol        ,alpha     "
    "   ,beta        ,rho        ,mat  "
    ...
    0,
    0,
    "Number of days in a corridor : NUMDAYS(f        ,spot        ,bup        "
    ",bdown        ,vol  "
    "      ,delay        ,mat        ,nstep        ,0->Brwn Brdg        , "
    "1->No Smile        , 2->Linear Smile        , "
    "Par0        , Par1)",
    "BSN",
    CLLNArithFunc,
    COMLL_A_BLKSCHLSNRM,
    ...
    0,
    0,
    "Black_Scholes Normal : BSN(f        ,k        ,vol        ,mat        "
    ",c/p)",
    ...
    0,
    0,
    "Spread_numer  : SPREADNUMER(fwdx        ,fwdy        ,sigx        ,sigy   "
    "     ,rho        ,a        ,b        ,k  "
    "      ,t        ,c/p)",
    ...
    0,
    0,
    "Col Sort       : COLSORT[column        , start_row        , end_row       "
    " , index]",
    "ROWMAX",
    CLLNCellFunc,
    COMLL_C_ROWMAX,
    ...
    0,
    0,
    "Row Sort       : ROWSORT[row        , start_col        , end_col        , "
    "index]: Index th "
    ...
    0,
    0,
    "ROWINTERP[row        , start_col        , end_col        , x_range "
    "(a[...])        , x] : "
    ...
    0,
    0,
    "SOURCE(opt        , expr) : Assign expr as the new PVSource for this "
    "column; "
    ...
    0,
    0,
    "CUM[j] : cumulative capit. value of column j        , included current "
    "row",
    ...
    0,
    0,
    "INTERP(xrange (a[...])        , yrange (a[...])        , x) : Approximate "
    "x by "
    ...
    0,
    0,
    "PVINTERP(AuxIndex        ,FirstCol        ,ColSkip        ,x) : Interp "
    "y(x) from A[AuxIndex "
    ...
    0,
    0,
    "Bond : yield(start        ,end/nfp        ,compd        ,basis        "
    ",cpn        ,clean)",
    ...
    0,
    0,
    "Bond : clean(start        ,end/nfp        ,compd        ,basis        "
    ",cpn        ,yield)",
    ...
    /*  "ACCINT"        ,   CLLNFinFunc        ,    COMLL_F_ACCINT        , 4 ,0
       ,0        ,0        , "ACCINT(daterng        , cfrng        , accdate
       , basiscode)"        ,  */
    ...
    3,
    0,
    "SWAP(start        , end/nfp        , comp        , basis        , [und]   "
    "     , [ratecode]        , "
    "[floatcode]) : Swap Rate",
    "LVL",
    CLLNFinFunc,
    ...
    1,
    0,
    "LVL(start        ,end/nfp        , comp        , basis        ,[und]) : "
    "Level Payment",
    "FRA",
    CLLNFinFunc,
    COMLL_F_FRA,
    ...
    2,
    0,
    "FRA(start        , end/nummonth        , basis        , [und]        , "
    "[floatcode]) : Moneymarket "
    "Short Rate",
    "DF",
    CLLNFinFunc,
    COMLL_F_DF,
    ...
    1,
    0,
    "PVRNG(daterange        , cfrange        , startdate <after this>        "
    ",[und])",
    ...
    2,
    0,
    "CMS(start        , end/nfp        , comp        , basis        , vol      "
    "  , delay/date        ,logn/nrm        , "
    ...
    1,
    0,
    "PAYOFF(date        , product        , [und]) : payoff of a "
    "pre-initialized product",

    /*  "DRTY"        ,     CLLNFinFunc        ,    COMLL_F_PVRANGE        ,4 ,0
       ,0        ,0        , "DRTY(daterange        , cfrange        ,
       startdate(>)        , enddate(<=))"        , */

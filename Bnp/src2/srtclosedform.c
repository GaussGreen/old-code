    ...
    if (greek == PREMIUM)
    {
        ...
        *num_tau = 0;
        ...
        err = srt_f_grfn_clsdfrm(
            sUndPtr, &grfnparam, &sdp, strike, bondStrike, rec_pay, type, refRateCodeStr, price);
    }
    else if (greek == VEGA)
    {
    ...
    double* pdFuturePrice)
{
    ...
    if (err = srt_f_set_GrfnParams(iNumParams, pszParamStrings, pszValueStrings, &grfnparam))
    {
        return err;
    }
    ...
    double* pdFutureVolatility)
{
    ...
    err = swp_f_ForwardRate(
        lStartDate, lNfpOrEnd, szComp, szBasis, szYcName, szRefRateCode, &dForward);
    if (err)
    {
    ...
    err = swp_f_LevelPayment(
        lStartDate, lNfpOrEnd, szComp, szBasis, szYcName, szRefRateCode, &dLevel);
    if (err)
    {

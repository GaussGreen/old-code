...
Err EuropeanAmortizingSwaption2(
  ...
  // fill dvPayDates        , i.e.        , today's zerobond values to all pay
  // dates
  ...
  */
  ...
  // 2 zeta        , NumPeriods G(Ti)        , i = 0        ,1        ,...
  // NumPeriods not sure why LGMCreateLGM_TS disallow 1 zeta
...
// temporarily commented out
  ...
  *lgmSigKapTSPtrPtr = LGMConvertZGtoSigKap(AsOfDate, pTermStruct);
  ...
  //					* sqrt( dLambda / (exp(2.0*dLambda*dTex)
  //- 1.0)
...
void Init_CalibInstruments(long NumPeriods, long AsOfDate, long FixDate,
  ...
  pLGMCalSet->ffirst =
      1; /* pay dates of swap. j are tPay[ffirst        , ...        ,j] */
  pLGMCalSet->nfirst = 1; /* the swaptions are j = nfirst        , nfirst+1 ,
                             ...        , nlast */
  ...
  for (ntime = 1; ntime <= NumPeriods; ++ntime) {
    ...
    if(ErrMessage = swp_f_Swaption(	pLGMCalSet->tStart[ntime]        ,
                                                                    pLGMCalSet->tPay[ntime]
          , pszFreq        , pszBasis        , vol        ,
    pLGMCalSet->Rfix[ntime]        , pszPayRec        , pszRefRate       ,
                                                                    pszYieldCurveName
          , "PREMIUM"        , pszVolType        ,
...
static char *EuropeanAmortizingSwaption_(
  ...
  // case 2: at least one zero solution
    ...
    while ((averageGDiffSqr > termstructtol) && (counter++ < maxcounter)) {
      ...
      // Set new strikes to calibration instruments        , and update vol and
      // prices
      ...
      if (LGMFindStateStar(NumPeriods, AsOfDate, plAcrlStartDates[1],
        ...
        // previously
        // free_all_vectors(NumPeriods        ,pdA        ,ptmpG        ,pdEta
        // ,pdCoupons
        //       ,pdInitCoupons        ,pdESOs); return "Numerical solver error
        //       in
                        ...
                        char *pszYieldCurveName)
{
      ...
      ,pszYieldCurveName)/ swp_f_df(AsOfDate        ,plPayDates[ntime]
,pszYieldCurveName);
        }
}*/
...
void ComputeESOWeights(long NumPeriods, double *pdEta,
                       double *pdA,       // vector(0        ,NumPeriods)
                       double *pdCoupons, // vector(1        ,NumPeriods) ,
                                          // strikes of replicating ESOs
                       double *pdCoverages) // vector(1        ,NumPeriods)
...
// it is assumed that Amorting Swaps covers period [T0        , T1        , ...
// , Tn]        , where T0 is first (Acrl) StartDate and Tn last (Acrl) End Date
...
Err EuropeanAmortizingSwaption(
    ...
    double Coupon,
    double *dvNotionals, // 0        ,..        ,NumPeriod  (
                         // dvNotionals[NumPeriod]=0 )
  ...
  // Check to see if it is a constant notional swap        , and if so        ,
  // return the standard swaption value
  ...
  for (i = 0; i < NumPeriod; i++) {
    //		/*K =*/ tPay[i] = add_unit(TheoEndDate        , - (int) (( NumPeriod -
    //i-1 )
    //* 12 / (frequency+0.0))         , SRT_MONTH        ,
    //NO_BUSDAY_CONVENTION); 		if (i<NumPeriod-1) tStartDate[i+1] =
    //add_unit(tPay[i]        , 0         , SRT_DAY        , Conv);
    // tEndDate[i] = add_unit(tPay[i]        , 0         , SRT_DAY        ,
    // Conv); 		tPay[i] = add_unit(tPay[i]        , 0         , SRT_BDAY
    // , MODIFIED_SUCCEEDING);
    ...
    // Calculate the spread.  For the moment        , we assume that the fixed
    // and floating frequencies are the same.  Use it to adjust the fixed
    // coverage
  ...
  /* if the expiry < 1M and the strike is more than 5 std dev away        ,
   * simply return the intrinsic value */
  SwapRate = floatinglegPV / fixedlegPV;
  *dSwapRate = SwapRate;
  //	GetVol( StartDate        , TheoEndDate        , coupon        ,
  //SRT_FALSE        , &dVol );
  ...
  *dFloatPV = floatinglegPV;
  ...
  for (j = 1; j < 10 && iNotFinished; j++) {
    ...
    if (error = FitAllGFwdSwap(tsPtr, CSPtr, &zdatefix, &zetafix)) {
      // Check to see if the error is caused by short maturity        , and if
      // so        , return the intrinsic
    ...
    // value (if no previous value throw an error)
      ...
      // get the strikes        , ystar value.  If it is -infinity        , then
      // use the previous strike values
  ...
  for (j = 1; j < NumPeriod + 1;
       j++) /* once we get the strikes and notionals        , we compute the
               vanilla swaptions prices*/
  {
    ...
    //	vecLGMPrice[j] = LGMRecVal(&vecYstar[j]        , j        , dvDiscPay ,
    //DStart        ,
    ///* receiver swaption */ 			sqrt(zeta)        , coupon_G        , ex_G);
    ///dvDiscPay[j] -=
    // tDFnow[j-1];
...
/*double NAG_CALL _ObjectiveFunction_ZC_Price_(double dX        ,Nag_User
*pComm)
{
        int tmpind;
        double result = 1.;
  ...
  *pdFltAdj = 0.;
  ...
  for (indfix = 0; indfix < NumFixPeriods; ++indfix) {
    ...
    for (indflt = indfix * tmpinc; indflt < (indfix + 1) * tmpinc; ++indflt) {
      // looks though plFltDates might be out of bound        , but this is not
      // true
    ...
    // swp_f_df(plFixedDates[indfix]        ,plFixedDates[indfix+1]        ,
    // pcYCName));
    fixedleglevel +=
...
SPP::XSchedule::Create(static_cast<long>(start_date)
      ,static_cast<long>(end_date)        ,freqFixed        , basisFixed ,
ptrCurrency        , false
); SPP::TPtr_Schedule ptrFloatingSchedule =
SPP::XSchedule::Create(static_cast<long>(start_date)
      ,static_cast<long>(end_date)        ,freqFloating        , basisFloating
, ptrCurrency        , false );
...
coverage(lvFixedStartDates[tmpind]        , lvFixedEndDates[tmpind]        ,

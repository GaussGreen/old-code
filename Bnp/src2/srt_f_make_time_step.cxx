...
<%%END
   ...
   Author     	: E Auld
   Date		: July 13        , 1994
   Description	: ins_vol_dates still an option        , though in the long run
will probably not be used; also changed insert vol dates not to insert last vol
point (since vol is the same on either side of the last point        , there is
no information lost by not inserting it)
   ...
   Author       : K L Chau
   Date         : August 16        , 1994
   Description  : restructure program        , so that the main routine is
separated into various subroutines        , to enhance flexibility. Some
comments: 1) insert dates according to "sigma^2 * time". 2) code concerning the
fields "center" and "dcenter" are commented out in grfn_mdl_init_step.  If any
problems arise they should be put back in.
   ...
   Description	:removed adding of grfn events (to grfn_attach_events);
                 renamed file to srt_f_mdl_init_step.cxx        , removed
references to grfn throughout file.
...
static int insert_ir_und_type_vol_dates(TermStruct *ts, SrtStpPtr step,
                                        double today);

/* --------------------------------------------------------------- */
/* PUT NODES ON FOR DATES OF GRFN DEAL */

static Err insert_eventdates(SrtStpPtr step, Date *eventdates,
                             long numeventdates, double today) {
  Err err = NULL;
  long i;
  ...
  /* Insert all the event dates BEFORE and AFTER today (no need to reinsert
   * today) */
...
static Err insert_vol_dates(SrtUndInfo *und_info, SrtStpPtr step,
                            double today) {
  ...
  while (i < und_info->no_of_underlyings) {
    ...
    if (und_type == INTEREST_RATE_UND)
      ...
      while (ls) {
        term_struct_date =
            (Date)((EquityTermStructVal *)ls->element->val.pval)->date;

        while ((step->date < term_struct_date) && (step->next))
          step = step->next;
...
static int insert_ir_und_type_vol_dates(TermStruct *ts, SrtStpPtr step,
  ...
  while ((ls != NULL) &&
         (bot->date >
          (tsval = (IrmTermStructVal *)(ls->element->val.pval))->date)) {
    while (step->date < tsval->date)
      step = step->next;
...
static Err insert_constrained_dates(SrtGrfnParam *grfnparam, SrtStpPtr step,
                                    double today, SrtUndInfo *und_info) {
  Err err = NULL;
  long i;
  ...
  i = 0;
  while ((i < und_info->no_of_underlyings) &&
         (grfnparam->insert_vol_dates == SRT_NO)) {
  ...
  if ((!und_info->jumping || grfnparam->imp_type != IMPMONTECARLO) &&
 ...
 * ) */
static int insert_extra_pts_in_future(SrtGrfnParam *grfnparam, SrtStpPtr step,
                                      double today, TermStruct *ts) {
...
Err srt_f_make_time_step(long numeventdates, Date *eventdates, SrtUndPtr und,
                         SrtStpPtr *stpptr, SrtGrfnParam *grfnparam,
                         SrtUndInfo *und_info) {
  Err err;
  ...
  err = insert_eventdates(step, eventdates, numeventdates, today);
  if (err)
    return err;

  /* Insert constrained dates (vol dates etc. if needed) */
  err = insert_constrained_dates(grfnparam, step, today, und_info);

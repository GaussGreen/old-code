 ...
 *
 * PURPOSE      	: Premium of a European call option for a single risk
 *factor using dF=a*F^{beta}dW
 *
 * DESCRIPTION  	: Black-Scholes: calculates the option premium in which
 *the underlying can vary between normal to lognormal by varying 0<=beta<1
 *
 * CALLS		: I_nu(...) in opblkschbeta.cxx
 *			  integrand(...) in opblkschbeta.cxx
 *			  safe_exp(...) in opblkschbeta.cxx
 *
 * PARAMETERS
...
static double integranda(double y, va_list argptr) { /* exponent < .5 */
...
static double integrandb(double y, va_list argptr) { /* exponent >= .5 */
...
double srt_f_optblkschbeta(double fwd_price, double strike, double vol,
  ...
  if ((mat <= 0) || (vol <= 0) ||
      (pow(fwd_price, beta - 1) * vol * sqrt(mat) <= 1e-4)) {
    if (call_put == SRT_PUT)
    ...
    if (fabs(beta - 1.0) <= 1E-2) {
      premium =
          srt_f_optblksch(fwd_price, strike, vol, mat, disc, call_put, greek);
    } else if (beta < 0) {
      premium =
          srt_f_optblknrm(fwd_price, strike, vol, mat, disc, call_put, greek);
    } else {
      /*** Change of variables ***/
      /* level 1 */
      b = 1. / (1. - beta);
  ...
  case DELTA:
    ...
    ans = (srt_f_optblkschbeta(fwd_price + shift, strike, vol, mat, beta, disc,
                               call_put, PREMIUM) -
           premium) /
          shift;
    break;
  ...
  case VEGA:
    shift = GVOPT.vol_add;
    ans = (srt_f_optblkschbeta(fwd_price, strike, vol + shift, mat, beta, disc,
                               call_put, PREMIUM) -
           premium) /
          shift;
    break;
  ...
  case THETA:
    shift = YEARS_IN_DAY;
    ans = srt_f_optblkschbeta(fwd_price, strike, vol, mat - shift, beta,

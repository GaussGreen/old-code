 ...
 *
 * PURPOSE      	: Calculates the premium and the greeks for a scud
 *options with multiple barriers
 *
 * DESCRIPTION  	: based on the nono-tree srt_f_nono_tree developed in
 *SORT equity by R. Carson 15/05/95 - originally in the file srt_f_nono_bar.cxx
 *
 * PARAMETERS
...
/************************************************************************************************/
/*																								*/
/* IDEA		: Calculates the 1st 2 absorbed moments of a brownian motion
 * (Barriers)				*/
/*																								*/
/************************************************************************************************/

static void absorbed_moment(double spot, double barrier, double dt, double vol,
                            double rate, double absorb_moments[2])

{
  ...
  absorb_moment[0] =
      barrier * (q2(-up_down, m1, ft) - fe * q2(-up_down, m2, ft));
  absorb_moment[0] /= absorb_prob;

  absorb_moments[0] =
      absorb_moment[0] * absorb_prob + barrier * (1 - absorb_prob);
  absorb_moments[0] /= spot;

  absorb_moment[1] =
      barrier * barrier *
      (q2(-2 * up_down, m1, ft) - fe * q2(-2 * up_down, m2, ft)) / absorb_prob;

  absorb_moments[1] =
      absorb_moment[1] * absorb_prob + barrier * barrier * (1 - absorb_prob);
  absorb_moments[1] /= spot * spot;

  if (absorb_moments[0] * absorb_moments[0] >
      absorb_moments[1]) /* Send all paths to barrier        , no vol.*/
  {
    absorb_moments[0] = barrier / spot;
    absorb_moments[1] = absorb_moments[0] * absorb_moments[0];
  }
...
/* + + + + + + + + + + + + + + + + + + + + + + + + + + + */
static void quick_live_val(double S[], double R[], MOMENTS *moments,
                           double *val_u, double *val_m, double *val_d,
                           double *new_val, double rho) {
  ...
  fS = (vals[1][0] - f0 - fSS / 2.0 * (S[0] - S[1]) * (S[0] - S[1])) * sd;
  fr = (vals[0][1] - f0 - frr / 2.0 * (R[0] - R[1]) * (R[0] - R[1])) * rd;

  if ((S[2] != S[0]) && (R[2] != R[0])) {
    f_uu = (vals[2][2] - vals[2][1] - vals[1][2] + vals[1][1]) * ru * su;
    f_du = (vals[0][2] - vals[0][1] - vals[1][2] + vals[1][1]) * rd * su;
    f_dd = (vals[0][0] - vals[0][1] - vals[1][0] + vals[1][1]) * rd * sd;
    f_ud = (vals[2][0] - vals[2][1] - vals[1][0] + vals[1][1]) * ru * sd;
    fSr = 0.25 * ((1 - rho) * (f_du + f_ud) + (1 + rho) * (f_uu + f_dd));
  }
...
/* + + + + + + + + + + + + + + + + + + + + + + + + + + + */
static void moments_stock_stock(double fwd_S, double nearest_S, double fwd_S2,
                                double nearest_S2, double vol_s, double vol_s2,
                                double rho, double dt, MOMENTS *moments) {
  moments->E_1 = 1;

  moments->E_S = fwd_S - nearest_S;

  moments->E_SS = fwd_S * fwd_S * exp(vol_s * vol_s * dt) -
                  2 * fwd_S * nearest_S + nearest_S * nearest_S;

  moments->E_R = fwd_S2 - nearest_S2;

  moments->E_RR = fwd_S2 * fwd_S2 * exp(vol_s2 * vol_s2 * dt) -
                  2 * fwd_S2 * nearest_S2 + nearest_S2 * nearest_S2;

  moments->E_RS = fwd_S * fwd_S2 * exp(vol_s * vol_s2 * rho * dt) -
                  fwd_S * nearest_S2 - fwd_S2 * nearest_S +
                  nearest_S * nearest_S2;
}

/* + + + + + + + + + + + + + + + + + + + + + + + + + + + */

static Err init_nono_tree(double **R, double **S, double **new_R,
                          double **new_S, double **fwd_S, double **mid_R,
                          double **gamma, double **gamma_prime, double **df_cve,
                          double **stoch_rate, double **fwd_rate, double ***val,
                          double ***new_val, long n_steps) {
  ...
  *new_val = dmatrix(-n_steps - 1, n_steps + 1, -n_steps - 1, n_steps + 1);
  ...
  if ((*mid_R == NULL) || (*gamma == NULL) || (*gamma_prime == NULL) ||
      (*fwd_S == NULL) || (*df_cve == NULL) || (*stoch_rate == NULL) ||
      (*fwd_rate == NULL) || (*R == NULL) || (*S == NULL) || (*new_R == NULL) ||
      (*new_S == NULL) || (*val == NULL) || (*new_val == NULL)) {
...
static void free_all(double **R, double **S, double **new_R, double **new_S,
                     double **fwd_S, double **mid_R, double **gamma,
                     double **gamma_prime, double **df_cve, double **stoch_rate,
                     double **fwd_rate, double ***val, double ***new_val,
                     long n_steps) {
...
double srt_f_nono_bar(double spot1, double spot2, double fwd1, double fwd2,
                      ...
                      double rho, double barrier[], double disc_fac,
                      long n_steps, SrtCallPutType call_put, SrtGreekType greek)

{
  ...
  /* Memory Allocation */
  if (init_nono_tree(&R, &S, &new_R, &new_S, &fwd_S, &mid_R, &gamma,
                     &gamma_prime, &df_cve, &stoch_rate, &fwd_rate, &val,
                     &new_val, n_steps) != NULL) {
    free_all(&R, &S, &new_R, &new_S, &fwd_S, &mid_R, &gamma, &gamma_prime,
             &df_cve, &stoch_rate, &fwd_rate, &val, &new_val, n_steps);
  ...
  for (time_step = n_steps - 1; time_step >= 0; time_step--) {
    ...
    for (idx_R = -(time_step + 1); idx_R <= (time_step + 1); idx_R++) {
      ...
      if ((barr21) && (new_R[idx_R] < barrier[2])) {
        ...
        smooth_R = 0;
      } else if ((barr22) && (R[idx_R] > barrier[3])) {
        new_val[idx_R][idx_S] = 0.0;
        smooth_R = 0;
      } else if ((barr21) && (R[idx_R - 1] < barrier[2])) {
        absorbed_moment(new_R[idx_R], barrier[2], dt, vol2, rate2,
                        absorb_moments);

        fwd_r = new_R[idx_R] * absorb_moments[0];
        vol_R = sqrt(
            log(absorb_moments[1] / absorb_moments[0] / absorb_moments[0]) /
            dt);
        ...
        smooth_R = 1;
      } else if ((barr22) && (R[idx_R + 1] > barrier[3])) {
        absorbed_moment(new_R[idx_R], barrier[3], dt, vol2, rate2,
                        absorb_moments);
      ...
      if (smooth_R) {
        for (idx_S = -(time_step + 1); idx_S <= (time_step + 1); idx_S++) {
          ...
          if ((barr11) && (new_S[idx_S] < barrier[0])) {
            new_val[idx_R][idx_S] = 0.0;
            smooth_S = 0;
          } else if ((barr12) && (S[idx_S] > barrier[1])) {
            new_val[idx_R][idx_S] = 0.0;
            smooth_S = 0;
          } else if ((barr11) && (S[idx_S - 1] < barrier[0])) {
            absorbed_moment(new_S[idx_S], barrier[0], dt, vol1, rate1,
                            absorb_moments);
            ...
            smooth_S = 1;
          } else if ((barr12) && (S[idx_S + 1] > barrier[1])) {
            absorbed_moment(new_S[idx_S], barrier[1], dt, vol1, rate1,
                            absorb_moments);
          ...
          if (smooth_S) {
            moments_stock_stock(fwd_s, S[idx_S], fwd_r, R[idx_R], vol_S, vol_R,
                                rho, dt, &moments);

            quick_live_val(&temp_S[0], &temp_R[0], &moments,
                           &val[idx_R + 1][idx_S - 1], &val[idx_R][idx_S - 1],
                           &val[idx_R - 1][idx_S - 1], &new_val[idx_R][idx_S],
                           rho);
  ...
  delta_up = (val[0][1] - val[0][0]) / (dlt_lat_up * spot1);

  delta_dn = (val[0][0] - val[0][-1]) / (dlt_lat_dn * spot1);

  dlt_lat_up1 = u_r - 1.0;
  dlt_lat_dn1 = 1.0 - 1.0 / u_r;

  delta_up1 = (val[1][0] - val[0][0]) / (dlt_lat_up1 * spot2);
  delta_dn1 = (val[0][0] - val[0][-1]) / (dlt_lat_dn1 * spot1);

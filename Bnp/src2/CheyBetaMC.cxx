...
static void CopyToGD(long path, GrfnDeal *gd, double ***cells,
                     GrfnSSParam *vars) {
  memcpy(&gd->cells[0][0], &cells[path][0][0],
         gd->sslength * gd->sswidth * sizeof(double));
  memcpy(&gd->ssparam, &vars[path], sizeof(GrfnSSParam));
}

static void CopyFromGD(long path, GrfnDeal *gd, double ***cells,
                       GrfnSSParam *vars) {
  memcpy(&cells[path][0][0], &gd->cells[0][0],
         gd->sslength * gd->sswidth * sizeof(double));
...
Err CheyBetaMC_Price(SProductDesc *g, int nt, long npaths, double cutcoef,
  ...
  SRandomGen rg;
  // Containers to save cells and vars from GRFN deal to keep track of
  // path-dependence:
  double ***cells = NULL;
  ...
  if (cutcoef > 50.0)
    cutcoef = 50.0;
  if (g->nccy > 1)
    return serror("Error (CheyBeta MC): Only one underlying allowed");
  ...
  while (times[ntimes - 1] > g->ex[g->nex - 1] + 1e-5)
    ntimes--;
  ...
  if (!x || !phi || !pvs) {
    err = serror("Memory failure in CheyBetaMC_Price");
    goto FREE_RETURN;
  }
  ...
  if (rgmode == 0)
    err = RandomGen_Init(&rg, seed);
  else if (rgmode == 1)
    err = ABS_Init(&rg, seed, npaths, 1, 0);
  else if (rgmode == 2)
    err = ABS_Init(&rg, seed, npaths, 1, 1);
  if (err)
    goto FREE_RETURN;
  ...
  for (i = 0; i < ntimes - 1; i++) {
    ...
    sqrtdt = sqrt(dt);
    while (isig < pmdl->nsig - 1 && pmdl->sigtms[isig] < times[i] + 1e-5)
      isig++;
    sig = pmdl->sig[isig];
    beta = pmdl->beta[isig];
    ...
    while (iex < g->nex - 1 && g->ex[iex + 1] < times[i + 1] + 1e-5)
      iex++;

    // Evaluate events if any
    if (iex >= 0 &&
        ((fabs(g->ex[iex] - times[i + 1]) < 1e-5) || (g->am && g->am[iex]))) {
...
FREE_RETURN:
  ...
  free(phi);
  if (pvs)
    free_dmatrix(pvs, 0, npaths - 1, 0, g->ninst - 1);
  if (cells)
    free_f3tensor(cells, 0, npaths - 1, 0, gd->sslength - 1, 0,
                  gd->sswidth - 1);
  free(vars);
  if (rgmode > 0)
    ABS_Free(&rg);

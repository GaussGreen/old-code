...
{
	const PFN_COEFF_3DPDE m_FUNC_; // tertiary function PFN_COEFF_3DPDE(double x,double aux,double t,void*)
	...
	if(pdHalf_InvDeltaX_Mu_End-pdHalf_InvDeltaX_Mu_Begin>=3)
	{
		...
		*pdU_Begin = dOffDiag_Imp_front; // NB : boundary
		...
		// boundary
		{
			...
			*pdL_Begin = (*(pdHalf_InvDeltaX_Mu_Begin+1) - dHalf_InvDeltaXSqrd_SigSqrd)/dDiag_Imp_front;
			_ASSERTE(( dC - *pdL_Begin * dOffDiag_Imp_front)!=0.);
			*(pdDInv_Begin+1) = 1./ ( dC - *pdL_Begin * dOffDiag_Imp_front);
		}
		
		for(
			pdMuQ=pdHalf_InvDeltaX_Mu_Begin+1;
			pdMuQ<pdHalf_InvDeltaX_Mu_End-2;
			++pdMuQ,++pdL,++pdDInv
			)
		{
...
//						at the boundary, the second derivative w.r.t to State X is 0 
//
//	Returns : ppd_To
//-------------------------------------------------------------------------------------------------------
	...
	else 
	{
		...
		/// handle the boundary - nRow_Begin
		{
			...
			for(nAuxX = nAuxX_Begin; nAuxX <nAuxX_End; ++nAuxX)
			{
				...
				ppd_To[nAuxX][nX_Begin] = (dInvHalfDeltaT -dMuQ) * pdCenter_from[nAuxX]+dMuQ* pdUp_from[nAuxX];
			}
		}
		...
		/// handle the boundary - nRow_End-1
		{
			...
			for(nAuxX = nAuxX_Begin; nAuxX <nAuxX_End; ++nAuxX)
			{
				...
				ppd_To[nAuxX][nX_End-1] = (dInvHalfDeltaT +dMuQ) * pdCenter_from[nAuxX]-dMuQ * pdDown_from[nAuxX];
			}
		}
	}
}
	...
	// Lower Ux = y
	{
		...
		for(nAuxX = nAuxX_Begin; nAuxX < nAuxX_End; ++nAuxX)
		{
			...
			for(nX = nX_End-2;nX >=nX_Begin; --nX)
				ppdStorage[nX][nAuxX] = (pdCenter_from[nX] - pUSup[nX] * ppdStorage[nX+1][nAuxX]) * pd_DInv[nX] ;
		}
	}
}
...
const char* Init_ADIStorage(_ADI_Storage *pstore,int nSize_X, int nSize_AuxX)
{
	...
	if(!pstore->m_pm || !pstore->m_pdata) return "Init_ADIStorage(..): mem allocation failed!";
	return 0;
}
...
{
	if(pStorage->m_pm) free(pStorage->m_pm);
	if(pStorage->m_pdata) free(pStorage->m_pdata);
}
		...
		)
{
	...
	/// Row Indices = [nAuxX_Begin,...) , Col Indices = [nX_Begin, ...)
	double **ppd_Storage
		= (double**)_alloca_matrix(pm,pdata,nAuxX_Begin,nAuxX_End,nX_Begin,nX_End,sizeof(double));
	
	/// Row Indices = [nAuxX_Begin,...) , Col Indices = [nX_Begin, ...)
	double **ppdHalf_Mu1_InvDeltaX1_from
		=(double**)_alloca_matrix(pm+=nSize_AuxX,pdata+=nSize_Grid,nAuxX_Begin,nAuxX_End,nX_Begin,nX_End,sizeof(double));
	double **ppdHalf_Mu1_InvDeltaX1_to
		=(double**)_alloca_matrix(pm+=nSize_AuxX,pdata+=nSize_Grid,nAuxX_Begin,nAuxX_End,nX_Begin,nX_End,sizeof(double));
	double **ppd_AuxXLX
		=(double**)_alloca_matrix(pm+=nSize_AuxX,pdata+=nSize_Grid,nAuxX_Begin,nAuxX_End,nX_Begin,nX_End,sizeof(double));
	double **ppd_AuxXDInvX
	...
	double **ppdHalf_Mu2_InvDeltaX2_from
		=(double**)_alloca_matrix(pm+=nSize_AuxX,pdata+=nSize_Grid,nX_Begin,nX_End,nAuxX_Begin,nAuxX_End,sizeof(double));
	double **ppdHalf_Mu2_InvDeltaX2_to 
		=(double**)_alloca_matrix(pm+=nSize_X,pdata+=nSize_Grid,nX_Begin,nX_End,nAuxX_Begin,nAuxX_End,sizeof(double));
	double **ppd_XLAuxX
		=(double**)_alloca_matrix(pm+=nSize_X,pdata+=nSize_Grid,nX_Begin,nX_End,nAuxX_Begin,nAuxX_End,sizeof(double));
	double **ppd_XDInvAuxX
		=(double**)_alloca_matrix(pm+=nSize_X,pdata+=nSize_Grid,nX_Begin,nX_End,nAuxX_Begin,nAuxX_End,sizeof(double));
...
static double __stdcall _SIGMA1_Sqrd_TEST_(const double *pdX1, const double *pdX2,const double* pdT, void* pAux)
{
	pdT,pAux;
...
static double __stdcall _MU1_TEST_(const double* pdX1,const double* pdX3,const double* pdT, void* pAux)
{
...
// Sigma is a function of time only
static double _SIGMA2_Sqrd_TEST_(const double *pdX1,const double *pdX2,const double* pdT, void* pAux)
{
...
// Sigma is a function of time only
static double __stdcall _SIGMA3_Sqrd_TEST_(const double *pdX1,const double *pdX2,const double* pdT,void* pAux)
{
...
/// drift can be a function of states and time
static double __stdcall _MU3_TEST_(const double* pdX3,const double* pdX1,const double* pdT, void* pAux)
{
...
static const char* __adi_tester_(
						  int nR_Begin,
						  int nR_End,
						  int nC_Begin,
						  int nC_End,
						  int nFlat) 
{
	...
	const double dTerminalVarX1 =  (dLambda1 == 0.) ?
	dT * _SIGMA1_Sqrd_TEST_(0,0,0,0) : 
	_SIGMA1_Sqrd_TEST_(0,0,0,0)*(1 - exp(2*dLambda1*dT))/(-2*dLambda1); 
	
	const double dTerminalVarX2 = (dLambda2 == 0.) ?
	dT * _SIGMA2_Sqrd_TEST_(0,0,0,0) : 
	_SIGMA2_Sqrd_TEST_(0,0,0,0)*(1 - exp(2*dLambda2*dT))/(-2*dLambda2); 

	const double dTerminalCovar12 = (dLambda1+dLambda2) ==0. ?
		dT*sqrt(_SIGMA1_Sqrd_TEST_(0,0,0,0) *_SIGMA2_Sqrd_TEST_(0,0,0,0))*dRho:
		sqrt(_SIGMA1_Sqrd_TEST_(0,0,0,0) *_SIGMA2_Sqrd_TEST_(0,0,0,0)) *dRho * (1 - exp(dLambda1*dT+dLambda2*dT))/(-dLambda1-dLambda2); 
	
	const double dTerminalVarX3 = 
		(dTerminalVarX2  + dRho*dRho*dAlpha*dAlpha* dTerminalVarX1 - 2. * dRho*dAlpha * dTerminalCovar12)

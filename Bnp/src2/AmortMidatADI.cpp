...
//						drift is a function of X  , AuxX and
//Time
//
...
static void __stdcall _Update_Slice(
    ...
    void *pComm,
    /// functors to update local vol : _SIGMA_Sqrd_FUNC_(State X  , Aux State X
    /// , Time)
    // NB: first argument must be State X  , not Auxiliary X
...
//-------------------------------------------------------------------------------------------------------
//	Description: explicit scheme in the State X  ,traversing from ppd_From
//to ppd_To 						at the boundary  , the second derivative w.r.t to State X is 0
//
//	Returns : ppd_To
//-------------------------------------------------------------------------------------------------------
static void __stdcall _ADI_SingleSlice_Explicit(
    ...
    // Row Indices = [nAuxX_Begin  ,...nAuxX_End)   , Col Indices = [nX_Begin  ,
    // ...nX_End)
...
//-------------------------------------------------------------------------------------------------------
//	Description: implict scheme in the State X  ,traversing from ppd_From to
//ppd_To 						at the boundary  , the second derivative w.r.t to State X is 0
//
...
static void __stdcall _ADI_SingleSlice_Implicit(
    ...
    // local vol at time slice from  , states - independent = 0.5 *  Sig^2(T)
    // /Delta_X^2

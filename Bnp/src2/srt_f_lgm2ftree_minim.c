  ...
  for (nt = num_time_pts - 1, stp = bot; stp != NULL; stp = stp->prev, nt--) {
    ...
    for (i = -trinf->max_index[0]; i <= trinf->max_index[0]; i++)
      for (j = -trinf->max_index[1]; j <= trinf->max_index[1]; j++) {
        ...
        /* If it is not the last step  , compute forward  , connections and
         * probas */
        if (stp->next)
        ...
        sam_get(node.cur_sam, 0, SHORT_RATE) =
            sam_get(tminf->fwd_sam, 0, F_0_t) + sam_get(node.cur_sam, 0, X1) +
            sam_get(node.cur_sam, 0, X2);
        ...
        if (stp->next) {
          ...
          Y_T_at_t_compute(1, &node.cur_sam, &tminf->yp, &zc_yield, 0, TWO_FAC,
                           LGM);
        ...
        if (err = evalcf((GrfnEvent *)stp->e, &node.cur_sam, gd,

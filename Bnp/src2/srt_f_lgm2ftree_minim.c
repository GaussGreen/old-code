...
#define ALLOC_BIGMEM_ASSETS   {\
    *assets = f4tensor (0, num_time_pts - 1, - maxtrinf.max_index[0], maxtrinf.max_index[0],\
                       - maxtrinf.max_index[1], maxtrinf.max_index[1], 0,  nbi); \
	*prob = f4tensor(0, num_time_pts - 1, - maxtrinf.max_index[0], maxtrinf.max_index[0],\
                       - maxtrinf.max_index[1], maxtrinf.max_index[1], 0, 4); \
	*df = f3tensor(0, num_time_pts - 1, - maxtrinf.max_index[0], maxtrinf.max_index[0],\
                                    - maxtrinf.max_index[1], maxtrinf.max_index[1]); \
	*dom = f3tensor(0, num_time_pts - 1, - maxtrinf.max_index[0], maxtrinf.max_index[0],\
                                    - maxtrinf.max_index[1], maxtrinf.max_index[1]); }
					...
					)
{
	...
	/* call to tree lgm2d */
	err = srt_f_lgm2dtree_forminim(und,grfnparam,stp,gd,evalcf,iolist,und_info, 
						&assets, &prob, &df, &dom, &maxx, &maxy);
	if (err) 
	{ 
		FREE_BIGMEM_ASSETS
		return err;
	...
	err = simul_and_regress_mcstatic(grfnparam, stp, assets, prob, df, dom,
					vec, cov, nbi, &residu);
	if (err) 
	{ 
		FREE_BIGMEM_ASSETS
...
Err  simul_and_regress_mcstatic(SrtGrfnParam     *grfnparam,
					   SrtStpPtr 		step,
					   double           ****assets, 
					   double           ****prob,
					   double           ***df,
	...
	for (n = 0; n < grfnparam->num_MCarlo_paths; n++)
  	{ 
		...
		{ 		
			...
			if (dom[t][i][j] > 0.0) break; // exit to free boundary
			...
			if (step->next != NULL)
			{
				...
				if (rand < prob[t][i][j][1])  
				    ...
				    i--;
				else if (rand < (prob[t][i][j][1] + prob[t][i][j][2] + prob[t][i][j][3] + prob[t][i][j][4]))
				    j--;
			}
					...
					long             *maxy)		
{
	...
	if (grfnparam->colmincible == 0) grfnparam->colmincible = grfnparam->node_dim - 1;
...
Err  simul_and_regress_pdestatic(SrtGrfnParam     *grfnparam,
					   SrtStpPtr 		step,
					   double           ****assets, 
					   double           ****prob,
					   double           ***df,
	...
	for (n = 0; n < grfnparam->num_MCarlo_paths; n++)
  	{ 	
		...
		{ 		
			...
			if (rand < prob[t][i][j][1])
			{
				if (i < trinf->max_index[0])   i++;
			}
			else if (rand < (prob[t][i][j][1] + prob[t][i][j][2]))
			{
				if (j < trinf->max_index[1])   j++;
			}
			else if (rand < (prob[t][i][j][1] + prob[t][i][j][2] + prob[t][i][j][3]))
			{
				if (i > - trinf->max_index[0])   i--;
	...
	for (t = maxt - 1, step = gotobot(step)->prev; step != NULL; step = step->prev, t--)
	{
			...
			for (j = - trinf->max_index[1]; j <= trinf->max_index[1]; j++) 	 
		{
            ...
            Rt_cur[i][j] =  res + prob[t][i][j][0] * Rt_next[i][j] 
			  + prob[t][i][j][1] * Rt_next[ip][j] + prob[t][i][j][2] * Rt_next[i][jp] 
			  + prob[t][i][j][3] * Rt_next[im][j] + prob[t][i][j][4] * Rt_next[i][jm];
		} /* End of loop on i, j */

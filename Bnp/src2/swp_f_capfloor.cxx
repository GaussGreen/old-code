/* =============================================================================
   ...
   FILENAME:    swp_f_capfloor.cxx

  PURPOSE:     Calculation for caps/floors: premium        , implied
  volatilities...
...
Err swp_f_CapFloor(long start, long end_nfp, double strike,
                   Err (*GetBSVol)(double start, double end, double strike,
                                   ...
                                   double *vol),
                   String capFloorStr, String szRefRateCode, String ycName,
  ...
  err = swp_f_get_ref_rate_details(szRefRateCode, &swapdp.basis_code,
                                   &swapdp.cxxompd);
  if (err)
    return err;

  /* Set the SwapDP for the cap/floor */
  err = swp_f_setSwapDP(start, end_nfp, swapdp.cxxompd, swapdp.basis_code,
                        &swapdp);
  if (err)
    return err;
  ...
  err = swp_f_CapFloor_SwapDP(&swapdp, strike, GetBSVol, capFloor,
                              szRefRateCode, ycName, greek, logNorm, result);
  if (err)
    return err;
...
Err swp_f_CapFloor_SwapDP(SwapDP *swapdp, double strike,
                          ...
                          SrtReceiverType capFloor, String szRefRateCode,
  ...
  err = swp_f_make_FloatingLeg(swapdp->start, swapdp->end, today, szRefRateCode,
                               yccrv, &floatleg);
  if (err)
    return err;
  ...
  *result = 0.0;
  for (i = 0; i < floatleg.leg_length - 1; i++) {
    ...
    err = GetBSVol(floatleg.start_date.date[i], floatleg.end_date.date[i],
                   strike, forward, floatleg.spread.d[i], &vol);

    if (err)
      return err;

    /* Get coverage and discount factor */
    cvg = floatleg.cxxvg.d[i];
    df = floatleg.df.d[i + 1];

    /* Call to the BlackScholes type formulas */
    err = swp_f_Caplet_Struct(forward, strike, vol, mat, capFloor, greek,
                              logNorm, &caplet);
    if (err)
      return err;
...
Err swp_f_CapFloorImpliedVol(double premium, long start, long end_nfp,
  ...
  /* Get the compounding and the basis of the cap from the reference rate */
  err = swp_f_get_ref_rate_details(refRateCode, &swapdp.basis_code,
                                   &swapdp.cxxompd);
  if (err)
    return err;
  ...
  err = swp_f_CapFloorImpVol_SwapDP(premium, &swapdp, strike, capFloor,
...
/* This function cheets        , and returns a null vol in *vol         , no
   matter the start date        , the end date and the strike */
static Err NullVol(double start, double end, double strike, double dForward,
                   double dSpread, double *vol) {
  *vol = 0.0;
  return NULL;
}
...
/* This function cheets        , and returns the same vol in *vol         , no
   matter the start date        , the end date and the strike ( the vol stored
   in _cafloorconstantvol )*/
static Err ConstantVol(double start, double end, double strike, double dForward,
                       double dSpread, double *vol) {
...
Err swp_f_CapFloorImpVol_SwapDP(double premium, SwapDP *swapdp, double strike,
   ...
   */
  err = swp_f_CapFloor_SwapDP(swapdp, strike, NullVol, capFloor, refRateCode,
  ...
  for (i = 0; i <= 20; i++) {
    ...
    err =
        swp_f_CapFloor_SwapDP(swapdp, strike, ConstantVol, capFloor,
                              refRateCode, ycName, PREMIUM, logNorm, &prem_new);

    /* Checks if relative difference in premium is lower than 1.0e-06: if it is
     *       , stop */
    ...
    err = swp_f_CapFloor_SwapDP(swapdp, strike, ConstantVol, capFloor,
                                refRateCode, ycName, PREMIUM, logNorm,
                                &prem_shift);

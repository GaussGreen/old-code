			...
			SrtRepoObj	**repo_obj)
{
	...
	if(err) return serror("error in creating the %s SrtRepoObject",repo_crv_name);

	/* insert a fake point cotaining ony today in the SrtList = SrtRepoObj */
	repo_today = 0.0;
	err = srt_f_lstins(*repo_obj,"_TODAY",(double)today,OBJ_DOUBLE,(void*)(&repo_today), NULL, &ticker);
	if(err) return err;

	if (err) return serror("Error in inserting today in the %s SrtRepoObj", repo_crv_name);
...
Err srt_f_repo_obj_insert_point(SrtRepoObj	*repo_obj,Ddate date,double repo)
{
	...
	err = srt_f_lstins(repo_obj,"REPO",(double)date,OBJ_DOUBLE,(void*)(&repo),NULL,&ticker);
	if(err) return err;
			...
			SrtRepoObj	**repo_obj)
{
	...
	if(num_cols != 2) return serror("need two columns for the repo curve");

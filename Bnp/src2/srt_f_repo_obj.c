...
Err srt_f_repo_obj_create(Date today, char* repo_crv_name, SrtRepoObj** repo_obj)
{
    Err    err = NULL;
    ...
    repo_today = 0.0;
    err        = srt_f_lstins(
        *repo_obj, "_TODAY", (double)today, OBJ_DOUBLE, (void*)(&repo_today), NULL, &ticker);
    if (err)
        return err;

    if (err)
        return serror("Error in inserting today in the %s SrtRepoObj", repo_crv_name);

...
// tree is assumed to have basic information        , such as RefDate and PPY
// ready alloc memory for tree_times and assume times        , always
// tree_times[0] = 0.0
// <-> RefDate add last tree_time = penutilmate_tree_time + penutimate dt return
// the index idx such that tree->tree_times[idx] <-> last DuePeriodEndDates
// Assume tree times coincide with mtg reset dates        , with RefDate
// (whether settle or spot date does not matter as long as they differ by a
// small amount) and some dates at the end; this means that PPY == 11 Assume
// security already seasoned (i.e.. wala >=0) on settle date
int MBS_BKTree_create_times(MBS_BKTree *tree, MBSPT_DealStruct *deal_struct,
  ...
  // actually        , given the way we structure dates        , refisDuePrdEnd
  // is always 0!!

  numDuePrdTimes = cashflow_struct->numCashFlows +
                   1; // - refIsDuePrdEnd;//add 0.0 at beginning
  // add those needed for for index calculation
  numTimeSlices =
      numDuePrdTimes + prepay_engine->EndIndexMat *
                           12; // strictly        , need 1 less than this
  ...
  if (cashflow_struct->numCashFlows > 0) {
    last_date =
        cashflow_struct->duePeriodEndDates[cashflow_struct->numCashFlows - 1];
  } else {
  ...
  tree->NbTimeSlices =
      totalNumSubIntervals + 1 + 1; // include 0.0 AND one more at end
...
// smm[0..prepay_engine->SFSNb-1][0        , tree->NbSpaceNodes-1]        , mem
// to be alloc by user 1% <-> 0.01
char *MBS_PricesGetSMM(double **smm, int NbPer,
                       MBSPT_CashFlowStruct *cashflowstruct,
                       MBS_Prepay_Engine *prepay_engine, MBS_BKTree *tree) {
  double *indices;
  double *smm_holder;
  char *err = 0;
  int i, j;
  ...
  if ((NbPer > prepay_engine->NbFixedPrepay - prepay_engine->Delay) &&
    ...
    if (err = MBS_BKTree_Yield_Approx(indices, prepay_engine->IndexFreq,
                                      prepay_engine->IndexMat, NbPer, tree)) {
      goto CLEANUP;
    }
    Dsplit(cashflowstruct->duePeriodEndDates[NbPer - 1], &yy, &mm,
           &dd); // only mm is needed
    mm--; // rate on reset date 20030801        , e.g.        , signifies prepay
          // of 200307!
    if (mm <= 0)
      mm += 12;
    for (i = start; i <= end; ++i) {
      MBS_Prepay_Engine_SMM(smm_holder, prepay_engine, o_wala + NbPer,
                            indices[i], mm, NbPer);
...
char *MBS_PricesBackwardInduct(double *io, double *po, int NbPer,
  ...
  // behavior is always such that smm denotes (possible) cash adjustment due to
  // PrepayDelay sch_amort_rate = foreknown amortization rate        , without
  // any delay there is no smm for NbPer in [1..NbFixedPrepay-PrepayDelay] and
  // [last-PrepayDelay..last] sch_amort_rate has to incorporate in the
  // FixedPrepay for [1..NbFixedPrepay]
  ...
  for (i = 0; i < NbZeros; ++i) {
    ...
    pos = interp_search(NbZerosCached, tree->tree_times + NbPer,
                        t); // position relative to NbPer
  ...
  if (err = MBSPT_AddCashFlowPV(io, po, start, end, coupon / 100.0,
                                sch_amort_rate, smm, NbZeros, zeros,
                                NbZerosToSkip)) {
...
// filled with zeros
// then start with backward induct at the last slice        , i.e.        , at
// NbPer == cashflowstruct->numCashFlows user alloc mem
...
// which means:
// set scheduled and unscheduled principal payments = 0 for up to NbFixedPrepay
// - Prepay-Delay and the coresponding io and po prices the following make the
// necessary adjustments due to user provided SMM the result are the dirty
// prices
...

{
  ...
  if (err = get_initial_principal_payments(
          NbCashFlow, bals + 1, wam, prepay_engine->gwac,
          prepay_engine->NbFixedPrepay, prepay_engine->FixedPrepay)) {
    goto CLEANUP;
  }

  if (NbCashFlow ==
      cashflowstruct->numCashFlows) // if it is the case that after NbCashFlow
                                    // exhaust all principals
  {
  ...
  //////
  for (i = cashflowstruct->NbInitialCashFlowToSkip;
       (i < NbCashFlow) && (i < wam); ++i) {
...
char *MBSPT_IOPO_Price(double *io, double *po, MBSPT_DealStruct *deal_struct,
                       MBS_Prepay_Engine *prepay_engine, MBS_BKTree *tree) {
  char *err = 0;
  double **smm, *smm_at1;
  ...
  io_prices =
      mbspt_dmatrix(0, prepay_engine->SFSNb - 1, 0, tree->NbSpaceNodes - 1);
  po_prices =
      mbspt_dmatrix(0, prepay_engine->SFSNb - 1, 0, tree->NbSpaceNodes - 1);
  ...
  for (i = cashflowstruct->numCashFlows; i >= 0; --i) {
    ...
    //
    for (j = 0; j < prepay_engine->SFSNb; ++j) {
      if (err = MBS_PricesBackwardInduct(io_prices[j], po_prices[j], i, coupon,
                                         smm[j], cashflowstruct, prepay_engine,
                                         tree))
        goto CLEANUP;
    }
  }
  // do weigted average
  (*io) = (*po) = 0.0;
  if (err = get_Trigger_Distribution(prepay_engine, deal_struct->TradeDate, wam,
                                     weights))
    goto CLEANUP;
  if (cashflowstruct->NbInitialCashFlowToSkip > 0 &&
      prepay_engine->Delay == 0) {
    sum_of_weights = 0.0;
    for (j = 0; j < prepay_engine->SFSNb; ++j) {
  ...
  if (err = MBS_IOPO_Price_Adjust(io, po, deal_struct, prepay_engine, tree))
    goto CLEANUP;
  //
  df = MBSDF(&(tree->df_data), deal_struct->cashflow_struct.SettleDate,
             deal_struct->SettleDate);
  df *= exp(-tree->OAS / 10000.0 *
            (double)day_count_date(deal_struct->cashflow_struct.SettleDate,
                                   deal_struct->SettleDate, BASIS_ACT_USD) /
            365.0);
  (*io) /= df;
  (*po) /= df;
  // accrued interest:
...
// cleanup
CLEANUP:
  mbspt_free_dmatrix(smm, 0, prepay_engine->SFSNb - 1, 0,
                     tree->NbSpaceNodes - 1);
  mbspt_free_dvector(smm_at1, 0, prepay_engine->SFSNb - 1);
  mbspt_free_dmatrix(io_prices, 0, prepay_engine->SFSNb - 1, 0,
                     tree->NbSpaceNodes - 1);
  mbspt_free_dmatrix(po_prices, 0, prepay_engine->SFSNb - 1, 0,
                     tree->NbSpaceNodes - 1);
  mbspt_free_dvector(weights, 0, prepay_engine->SFSNb - 1);
  return (err);
} // MBSPT_IOPO_Price

char *MBSPT_MBSDF_Prepare_Helper(MBSDF_Data *df_data, int NbRates,
                                 double *rates, int NbMMTerms,
                                 DATE_UNIT *mm_date_units, int *MMTerms,
                                 DATE_UNIT *yield_date_units, int *yield_terms,
                                 // mm:
                                 YBASIS mm_ybase, DIFFBASIS mm_diffBase,
                                 int mm_delay,
                                 // swap
                                 YBASIS yield_ybase, DIFFBASIS yield_diffBase,
                                 int yield_freq, int yield_delay,
                                 // zero spec
                                 int zero_freq, int interp_freq, long RefDate) {
  char *err = 0;
  ...
  for (i = 0; i < NbMMTerms; ++i)
    MMYield[i] = rates[i];
  if (err = MM_Data_Construct(&mm_data, mm_ybase, mm_diffBase, mm_delay,
                              NbMMTerms, mm_date_units, MMTerms, MMYield,
                              ValueDate))
    goto CLEANUPMM;
  ...
  if (err = Yield_Data_Construct(&yield_data, yield_ybase, yield_diffBase,
                                 yield_delay, yield_freq, NbRates - NbMMTerms,
                                 yield_date_units, yield_terms, yields,
                                 ValueDate))
    goto CLEANUPSWAP;
  // strip:
  zero_freq = yield_freq;
  if (err = MBS_Strip((&zero_data), &mm_data, &yield_data, interp_freq,
                      zero_freq, zero_ybase, ValueDate))
    goto CLEANUPSWAP;
...
char *MBSPT_MBSDF_Prepare_FromRatesAndFile(
    MBSDF_Data *df_data, char *data_dir, int NbRates, double *rates,
    int NbMMTerms, DATE_UNIT *mm_date_units, int *MMTerms,
    DATE_UNIT *yield_date_units, int *yield_terms, long RefDate) {
  char *err = 0;
  ...
  if (err = MBSPT_MBSDF_Prepare_Helper(
          df_data, NbRates, rates, NbMMTerms, mm_date_units, MMTerms,
          yield_date_units, yield_terms, mm_ybasis, mm_diffbasis, mm_delay,
          yield_ybasis, yield_diffbasis, yield_freq, yield_delay, zero_freq,
          interp_freq, RefDate))
    goto CLEANUP;
...
// for the MBS_BKTree used in the first generation mortgage pricer (as of
// September 2003)
char *MBSPT_MBSDF_Prepare_FromRatesAndFile_Special(MBSDF_Data *df_data,
...
#ifdef MBSPTCORRECTDAYCT
  int NbYields = 9;
  DATE_UNIT yield_date_units[] = {YEAR, YEAR, YEAR, YEAR, YEAR,
                                  YEAR, YEAR, YEAR, YEAR};
  int yield_terms[] = {1, 2, 3, 4, 5, 7, 10, 20, 30};
#else
  int NbYields = 8;
  DATE_UNIT yield_date_units[] = {YEAR, YEAR, YEAR, YEAR,
                                  YEAR, YEAR, YEAR, YEAR};
  ...
  return (MBSPT_MBSDF_Prepare_FromRatesAndFile(
      df_data, data_dir, NbMMTerms + NbYields, rates, NbMMTerms, mm_date_units,
      MMTerms, yield_date_units, yield_terms, CrvDate));
} // MBSPT_MBSDF_Prepare_FromRatesAndFile_Special

char *MBSPT_Structures_Prepare_FromFile(
    MBS_BKTree *tree, MBSPT_DealStruct *deal_struct,
    MBS_Prepay_Engine *prepay_engine, char *data_dir, long valuation,
    long FwdSettle, MBSDF_Data *df_data, double refisprd,
    double Rate10Y, // needed in MBS_Prepay_Engine_SetParams for converting
                    // ratio form of refi-incent to diff form
    MTGPTACYPROG AgcyProg, int NbSwaptions, double *volcurve, long *volexp,
    double meanrev, long swapmat, long *dealmat, double *coupon, double inoas,
    long paydelays, double *pptweaks) {
  int i;
  ...
  TradeDate = valuation;
  //	SpotSettleDate = Date_Add( valuation        , DAY        , sptdays , 1
  //, 0);
  ...
  if (err = MBS_BKTree_Construct_FromFile(
          tree, data_dir, df_data, NbSwaptions, swapmat, swaptions_exp,
          volcurve, meanrev, inoas, Rate10Y, refisprd, TreeRefDate))
    goto CLEANUP;
  ...
  if (err = MBS_Prepay_Engine_Params_Reader(data_dir, prepay_engine, AgcyProg,
                                            dealmat[1], coupon[1], pptweaks[0],
                                            pptweaks[1], (int)swapmat))
    goto CLEANUP;
  if (err = MBS_Prepay_Engine_SetParams(prepay_engine, tree->RefiRate,
                                        tree->RefiBaseIndex))
    goto CLEANUP;
  ...
  if (err = BKTreeCachStruct_Reset(
          (tree->cach),
          1 + prepay_engine->IndexMat *
                  deal_struct->cpnFreq, // prepay_engine->IndexFreq        ,
...
char *MBSPT_Pricer(double outputs[3], MBS_BKTree *tree,
                   MBSPT_DealStruct *deal_struct,
                   MBS_Prepay_Engine *prepay_engine) {
  double io, po;
  char *err = 0;
...
char *MBSPT_OAS_Solver(
    double *out, double mktPrice, char *data_dir, long valuation,
    long FwdSettle, char *mkt, double refisprd,
    double Rate10Y, // needed in MBS_Prepay_Engine_SetParams for converting
                    // ratio form of refi-incent to diff form
    MTGPTACYPROG AgcyProg, int NbSwaptions, double *volcurve, long *volexp,
    double meanrev, long swapmat, long *dealmat, double *coupon,
    double guessed_oas, long paydelays, double *pptweaks, double tolerance,
    int max_trial,
  ...
  while (it <= max_trial) {
    ...
    if (err = MBSPT_Structures_Prepare_FromFile(
            &tree, &deal_struct, &prepay_engine, data_dir, valuation, FwdSettle,
            &df_data, refisprd, Rate10Y, AgcyProg, NbSwaptions, volcurve,
            volexp, meanrev, swapmat, dealmat, coupon, curr_oas, paydelays,
            pptweaks)) {
      MBSPT_Structures_Cleanup(&df_data, &tree, &deal_struct, &prepay_engine);
      return (err);
    }
    ...
    if (err = MBSPT_Structures_Prepare_FromFile(
            &tree, &deal_struct, &prepay_engine, data_dir, valuation, FwdSettle,
            &df_data, refisprd, Rate10Y, AgcyProg, NbSwaptions, volcurve,
            volexp, meanrev, swapmat, dealmat, coupon, curr_oas + oas_inc,
            paydelays, pptweaks)) {
      MBSPT_Structures_Cleanup(&df_data, &tree, &deal_struct, &prepay_engine);
      return (err);
    }
...
char *MBS_Prepay_GetIndexValue(double **indices, int endIndex, MBS_BKTree *tree,
                               MBS_Prepay_Engine *prepay_engine) {
  char *err = 0;
  int i;
  // alloc mem
  ...
  for (i = endIndex; i >= 0; --i) {
    if (err = MBS_BKTree_Yield_Approx(indices[i], prepay_engine->IndexFreq,
                                      prepay_engine->IndexMat, i, tree))
      return (err);
  }
  return (err);
} // MBS_Prepay_GetIndexValue

void MBSPT_Structures_Cleanup(MBSDF_Data *df_data, MBS_BKTree *tree,
                              MBSPT_DealStruct *deal_struct,
                              MBS_Prepay_Engine *prepay_engine) {
...
char *MBS_Prepay_Projection_ParamsFromFile(
    char *data_dir, MTGPTACYPROG AgencyProg, long oterm, double gwac,
    double Speedup, double rational, double RefiRate, double Rate10Y,
    long TradeYYYYMMDD, long spotYYYYMMDD, long wam, int subgroup,
    int num_future_rates, double *futureRates, int useFixedPrepay,
    int *numProjections, double rates[MAXOTERM], double cpr[MAXOTERM],
    double principal_payments[MAXOTERM], int IndexMat) {
  MBS_Prepay_Engine prepay_engine = {0};
  char *err;

  if (err = MBS_Prepay_Engine_Params_Reader(data_dir, &prepay_engine,
                                            AgencyProg, oterm, gwac, Speedup,
                                            rational, IndexMat)) {
    MBS_Prepay_Engine_Cleanup(&prepay_engine);
    return (err);
  }
  ...
  err = MBS_Prepay_Path(&prepay_engine, TradeYYYYMMDD, spotYYYYMMDD, wam,
                        subgroup, num_future_rates, futureRates, useFixedPrepay,
                        numProjections, rates, cpr, principal_payments);
  MBS_Prepay_Engine_Cleanup(&prepay_engine);
  return (err);
...
char *MBS_Prepay_Projection_ParamsExplicit(
    char *data_dir, MTGPTACYPROG AgencyProg, long oterm, double gwac,
    double Speedup, double rational, double FebruaryEffect, double *IndexLevel,
    double *AmortLevel, double *SeasoningLevel, double *SeasoiningMat,
    int SFSNb, double *Amorts, double Seasonality[12], int Delay,
    int NbFixedPrepay, double *FixedPrepay, int nuniform, double RefiRateSpread,
    double Rate10Y, long TradeYYYYMMDD, long spotYYYYMMDD, long wam,
    int subgroup, int num_future_rates, double *futureRates, int useFixedPrepay,
    int *numProjections, double rates[MAXOTERM], double cpr[MAXOTERM],
    double principal_payments[MAXOTERM], int IndexMat, int EndIndexMat,
    int IndexFreq) {
  MBS_Prepay_Engine prepay_engine = {0};
  char *err;
  ...
  if (err = MBS_Prepay_Engine_Params_ReaderExplicit(
          ...
          FebruaryEffect, IndexLevel, AmortLevel, SeasoningLevel, SeasoiningMat,
          SFSNb, Amorts, Seasonality, Delay, NbFixedPrepay, FixedPrepay,
          nuniform, IndexMat, EndIndexMat, IndexFreq)) {
    MBS_Prepay_Engine_Cleanup(&prepay_engine);
    return (err);
  }
  if (err = MBS_Prepay_Engine_SetParams(&prepay_engine, RefiRateSpread,
                                        Rate10Y)) {
    MBS_Prepay_Engine_Cleanup(&prepay_engine);
    return (err);
  }

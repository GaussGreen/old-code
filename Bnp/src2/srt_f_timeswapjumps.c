...
Err free_resetableperiod(double **paramperiod,double *periodmat, double *DRStsStart,
			   double *DRSts, double *isfriday, double nfixdays, double *driftinterp);
...
Err get_param_first_leg(double Fradate, double *resetdates,
			  double **paramatrix, double *paramfra)
{
Err err=NULL;
double datespread;
...
*(Fradate-resetdates[1])/datespread+paramatrix[1][1];

paramfra[2] = (paramatrix[2][2]-paramatrix[1][2])
*(Fradate-resetdates[1])/datespread+paramatrix[1][2];
...
*(Fradate-resetdates[1])/datespread+paramatrix[1][4];
...
Err get_DRSts(long today, long Paydate, int nfixdays, double *periodmat, char *cRefRateCode, char *szYieldCurveName,
			  double *DRSts);
...
of the period in question */
				...
				prix = PrixTimeSwap(eps,DRSts,(int) nfixdays, maturities,isfriday,paramperiod,width,callspread);

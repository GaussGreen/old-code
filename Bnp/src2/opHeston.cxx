...
Err HestonPrice(double Forward, double *Strike, int nStrikes, double Maturity,
                ...
                double Gamma, double Rho, double Disc, double UpperBound,
  ...
  double Int, Int1, NodeLeft, NodeRight, NodeLeft1, NodeRight1, dScaling = 1.,
                                                                dGear;
  Err err = NULL;
  ...
  case PREMIUM:
    ...
    case 1:
      ...
      ImAppr1 = -(log(Forward) + Sigma * Sigma / b * (1. - esp) -
                  Maturity * a / 2. / b + a / 2. / b / b * (1. - esp));
      ...
      for (j = 1; j <= nStrikes; j++) {
        ...
        do {
          ...
          for (k = 1; k <= nStp; k++) {
            ...
            err =
                PsiFunction(isVolInfFix, Maturity, 1., -x[k], a, b, Alpha, Rho,
                            log(Forward), Sigma * Sigma, greek, &RePsi1,
            ...
            err =
                PsiFunction(isVolInfFix, Maturity, 0., -x1[k], a, b, Alpha, Rho,
                            log(Forward), Sigma * Sigma, greek, &RePsi2,
      ...
      err = PsiFunction(isVolInfFix, Maturity, 1., 0., a, b, Alpha, Rho,
                        log(Forward), Sigma * Sigma, greek, &RePsi1, &ImPsi1,
                        &DRePsi1, &DImPsi1, &DDRePsi1, &DDImPsi1);
      G11 = exp(RePsi1);

      err = PsiFunction(isVolInfFix, Maturity, 0., 0., a, b, Alpha, Rho,
                        log(Forward), Sigma * Sigma, greek, &RePsi2, &ImPsi2,
    ...
    case 0:
      ...
      /*
               ...
               ,-UpperBound        ,a        ,b        ,Alpha        ,Rho
         ,log(Forward)        ,Sigma*Sigma        ,greek
               ,&ReUpBound        ,&ImUpBound        ,&DRePsi1        ,&DImPsi1
         ,&DDRePsi1       ,&DDImPsi1); PsiFunction (isVolInfFix        ,Maturity
         ,1.        ,0.0        ,a ,b        ,Alpha        ,Rho ,log(Forward)
         ,Sigma*Sigma        ,greek        ,&ReLowBound
               ,&ImLowBound        ,&DRePsi1        ,&DImPsi1        ,&DDRePsi1
         ,&DDImPsi1);
         ...
         ,log(Forward)        ,Sigma*Sigma        ,greek
               ,&ReUpBound        ,&ImUpBound        ,&DRePsi1        ,&DImPsi1
         ,&DDRePsi1       ,&DDImPsi1); PsiFunction (isVolInfFix        ,Maturity
         ,0.        ,0.0        ,a ,b        ,Alpha        ,Rho ,log(Forward)
         ,Sigma*Sigma        ,greek        ,&ReLowBound
               ,&ImLowBound        ,&DRePsi1        ,&DImPsi1        ,&DDRePsi1
         ,&DDImPsi1);
      ...
      */
      for (i = 1; i <= nSteps; i++) {
        ...
        err = PsiFunction(isVolInfFix, Maturity, 1., -x[i], a, b, Alpha, Rho,
                          log(Forward), Sigma * Sigma, greek, &RePsi1, &ImPsi1,
                          &DRePsi1, &DImPsi1, &DDRePsi1, &DDImPsi1);
        err = PsiFunction(isVolInfFix, Maturity, 0., -x[i], a, b, Alpha, Rho,
                          log(Forward), Sigma * Sigma, greek, &RePsi2, &ImPsi2,
                          &DRePsi2, &DImPsi2, &DDRePsi2, &DDImPsi2);
    ...
    case 3:
      ...
      for (j = 1; j <= nStrikes; j++) {
        ...
        //       , the precision is forced to increse        , as there is
        //       little time value

        // Here it branches following the rule: if Strike is larger than nStdDev
        // from the forward or smaller than -nStdDev        , then it simply
        // evaluates the price at +- nStdDev
    ...
    default:
      ...
      /* second part        , now I use the parallel integrator for strikes > 30
       * bps
       */
...
Err HestonATMVol(double Forward, double Maturity, double Sigma, double Alpha,
                 double SigmaIfty, double b, double Gamma, double Rho,
  ...
  err = HestonPrice(Forward, Strike, nStrikes, Maturity, Sigma, Alpha,
                    SigmaIfty, b, Gamma, Rho, Disc, UpperBound, SRT_CALL,
...
Err HestonCalibrateSigmaInit(double Forward, double Maturity, double ATMVol,
  ...
  err = HestonATMVol(Forward, Maturity, SigmaGuess, AlphaHeston,
                     SigmaIftyHeston, LambdaHeston, ShiftHeston, RhoHeston, 1,
  ...
  while (current_iter <= n_iter_max) {
    ...
    err = HestonATMVol(Forward, Maturity, Sigma2, AlphaHeston, SigmaIftyHeston,
                       LambdaHeston, ShiftHeston, RhoHeston, 1, UpperBound,
                       nSteps, isVolInfFix, &Vol2);
...
Err CalibrateHestonToSABR(double Forward, double Maturity, double SigmaBeta,
                          ...
                          double *ShiftHeston, double *RhoHeston,
                          double UpperBound, int nSteps, int nStdDev,
  ...
  */
  ...
  case MATCH_CONV:
    ...
    while ((alphamax - alphamin > precalpha) && (current_iter < n_iter_max)) {
      ...
      err = HestonCalibrateSigmaInit(
          ...
          *ShiftHeston, *RhoHeston, UpperBound, nSteps, isVolInfFix, &sigma0);

      if (err)
        return err;

      err = HestonPrice(Forward, Strikes, 2, Maturity, sigma0, alphamid,
                        *SigmaIftyHeston, *LambdaHeston, *ShiftHeston,
  ...
  case CHI2_MIN:
    ...
    err = HestonCalibrateSigmaInit(
        ...
        *ShiftHeston, *RhoHeston, UpperBound, nSteps, isVolInfFix,
    ...
    do {
      ...
      err = HestonATMVol(Forward, Maturity, sigma0, x1, *SigmaIftyHeston,
                         *LambdaHeston, *ShiftHeston, *RhoHeston, 1.0,
      ...
      f1 = SmileChi2(Forward, Maturity, sigma0, x1, *SigmaIftyHeston,
                     *LambdaHeston, *ShiftHeston, *RhoHeston, UpperBound,
                     nSteps, nK, StrikeVec, SigmaBeta, AlphaSABR, BetaSABR,
                     RhoSABR, gap, isVolInfFix);
...
double SmileChi2(double Forward, double Maturity, double Sigma0,
                 double AlphaHeston, double SigmaIftyHeston,
                 double LambdaHeston, double ShiftHeston, double RhoHeston,
                 double UpperBound, int nSteps, int nK, double *StrikeVec,
                 double SigmaBeta, double AlphaSABR, double BetaSABR,
                 double RhoSABR, double gap, SRT_Boolean isVolInfFix) {
  ...
  err = HestonVol(Forward, StrikeVec, nK, Maturity, Sigma0, SigmaIftyHeston,
                  AlphaHeston, ShiftHeston, RhoHeston, LambdaHeston,
                  HESTON_TO_LOG, UpperBound, nSteps, isVolInfFix, BS_HESTONVol);
...
/*-----------------------------------------------------------------------------------------------*/
/*            This function returns the shift on the Fwd. lognormal dynamics
 * that                */
...
/*-----------------------------------------------------------------------------------------------*/
/*        This function converts among different types of volatilities by
 * matching the premiums  */
/*        of the corresponding calls/puts. It is the equivalent of the add-in
 * SABRVol adapted    */
/*        for the Heston case */
/*-----------------------------------------------------------------------------------------------*/
...
Err HestonVol(double Forward, double *Strikes, int nStrikes, double Maturity,
              double Vol, double VolInfty, double Alpha, double Gamma,
              double Rho, double MeanRev, SrtVolConversion TypeConversion,
              double UpperBound, int nSteps, SRT_Boolean isVolInfFix,
              double *NewVols) {

  Err err = NULL;
  int i, j,
      IntegrType = 3; /* sets up the integration type to the "mixed" one */
  ...
  case LOG_TO_HESTON:
    ...
    err = HestonPrice(Forward, Strikes, 1, Maturity, x1, Alpha, VolInfty,
                      MeanRev, Gamma, Rho, 1.0, UpperBound, SRT_CALL, PREMIUM,
                      isVolInfFix, IntegrType, nSteps, result_low);

    /* computes premium at the right boundary */
    err = HestonPrice(Forward, Strikes, 1, Maturity, x2, Alpha, VolInfty,
                      MeanRev, Gamma, Rho, 1.0, UpperBound, SRT_CALL, PREMIUM,
                      isVolInfFix, IntegrType, nSteps, result_high);

    for (i = 1; i <= nStrikes; i++) {

      BSPremium = srt_f_optblksch(Forward, Strikes[i], Vol, Maturity, 1.0,
                                  SRT_CALL, PREMIUM);
      ...
      err = HestonPrice(Forward, Strike, 1, Maturity, rts, Alpha, VolInfty,
                        MeanRev, Gamma, Rho, 1.0, UpperBound, SRT_CALL, PREMIUM,
                        isVolInfFix, IntegrType, nSteps, result);

      f = result[1] - BSPremium;

      err = HestonPrice(Forward, Strike, 1, Maturity, rts, Alpha, VolInfty,
                        MeanRev, Gamma, Rho, 1.0, UpperBound, SRT_CALL, VEGA,
                        isVolInfFix, IntegrType, nSteps, result);
      df = result[1];
      ...
      for (j = 1; j <= MAXIT; j++) {
        ...
        err = HestonPrice(Forward, Strike, 1, Maturity, rts, Alpha, VolInfty,
                          MeanRev, Gamma, Rho, 1.0, UpperBound, SRT_CALL,
                          PREMIUM, isVolInfFix, IntegrType, nSteps, result);
  ...
  case HESTON_TO_LOG:
    ...
    err = HestonPrice(Forward, Strikes, nStrikes, Maturity, Vol, Alpha,
                      VolInfty, MeanRev, Gamma, Rho, 1.0, UpperBound, SRT_CALL,
                      PREMIUM, isVolInfFix, IntegrType, nSteps, result);
  ...
  case NORMAL_TO_HESTON:
    ...
    for (i = 1; i <= nStrikes; i++) {
      ...
      BSPremium = srt_f_optblknrm(Forward, Strikes[i], Vol, Maturity, 1.0,
                                  SRT_CALL, PREMIUM);

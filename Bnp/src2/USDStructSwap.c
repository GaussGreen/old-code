...
char *getCashVolwithType(
  ...
  // Check that the volatility is consistent with the model  , otherwise convert
  // it as required
  if (dPower == 0.0)
...
//calculated using lag 	dStrike				strike of the option
//	lFutureStart		start date for the Libor underlying the midcurve
//option 	dFuturePrice		market price of the future 	iFutureTenorInMonths
//length of underlying Libor in months (generally 1 or 3) 	iConvexityModel
//0:	midcurve strike adjusted by forward-future difference 	iVolatilityModel
//0:  stationary normal volatility 	enumCallPut			Call = long
...
//option 	dFuturePrice		market price of the future 	iFutureTenorInMonths
//length of underlying Libor in months (generally 1 or 3) 	iConvexityModel
//0:  future = 100 * (1 - forward)  , strike is yield strike 						1:	midcurve strike
//adjusted by forward-future difference
//
//	iVolatilityModel	0:  stationary normal volatility
//	enumCallPut			Call/Put on price ( i.e.  , opposite on
//yield ) 	enumGreek			PREMIUM  , DELTA  , etc 	out_dVol
//vol used to price.  Will depend on the volatility model
//
...
char *CorrelFromStationaryCaps(
    long lToday,
    /*								char* szVolCurve
       , char* getCashVol_NYK( char *szVolCurve  , double dStartDate  , double
       dEndDate  , double dCashStrike  , int iZero  , char *szRefRate  , double
       *dVol  , double *dPower )  ,
    */
  ...
  /*	for ( i=0; i<nDates; i++ )
     ...
     lStartDates[i]  , (double) lEndDates[i]  , dvFwds[i]  , 0  , szRefRate  ,

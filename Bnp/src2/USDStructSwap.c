    ...
    double* out_dVega)
{
    ...
    srt_f_optsarbvol(
        dLibor, dEffStrike, dExpiry1, dVol1, 0.0, dPower, 0.0, srtVol, srtPricing, &dVol1);
    srt_f_optsarbvol(
        dLibor, dEffStrike, dExpiry2, dVol2, 0.0, dPower, 0.0, srtVol, srtPricing, &dVol2);
    ...
    if (bUseNormalModel == 1)
        *out_dPrice =
            srt_f_optblknrm(dLibor, dEffStrike, dEffVol, dExpiry2, dLevel, srtCallPut, PREMIUM);
    else
        *out_dPrice =
            srt_f_optblksch(dLibor, dEffStrike, dEffVol, dExpiry2, dLevel, srtCallPut, PREMIUM);
    ...
    double*        out_dVol)
{
    ...
    *out_dVol   = 0.0;
    ...
    if (dShortCashVol < 0.0)
    {
        ...
        lShortStart =
            add_unit(lShortExpiry, pCcyParam->spot_lag, SRT_BDAY, pCcyParam->swap_bus_day_conv);
        lShortEnd =
            add_unit(lShortStart, 12 / compounding, SRT_MONTH, pCcyParam->swap_bus_day_conv);
        ...
        // Calculate the std deviations that the option strike is from the long cash fwd, and hence
        // the strike
        ...
        {
            dStdDev =
                (log(dCashStrike / dLongCashFwd) + 0.5 * dATMLongVol * dATMLongVol * dLongExpiry) /
                dATMLongVol / sqrt(dLongExpiry);
            dShortCashStrike =
                dShortCashFwd * exp(-0.5 * dATMShortVol * dATMShortVol * dShortExpiry +
...
//						1:	midcurve strike adjusted by forward-future
//difference
//
//	iVolatilityModel	0:  stationary normal volatility
    ...
    double*        out_dVol)
{
    ...
    *out_dVol = sqrt((dLongVar - dShortVar) / dOptionExpiry);
    ...
    // Calculate the price
    if (iVolatilityModel == 0)
        *out_dPrice = srt_f_optblknrm(
            dLongCashFwd, dUndStrike, *out_dVol, dOptionExpiry, 1.0, enumCallPut, enumGreek);
...
static char* StationaryAutocorrel(
    int     iLow,
    int     iHigh,
    int     iTime,
    int     nDates,
...
static char* StationaryMidCurve(
    int iCap, int iTime, int nDates, double* dvVol, double* dvDeltaT, double* out_dMidCurve)
{
    /* Variable declaration */
    int    i;
    double dSum;
    ...
    double*  out_dSum)
{
    ...
    for (i = iLow + 1; i <= iHigh; i++)
    {
        ...
        if (err = sumInner(i, iHigh, iTime, dmLvl, nDates, dvVol, dvDeltaT, dmCorrel, &dSumInner))
            return err;
...
char* CorrelFromStationaryCaps(
    long lToday,
    /*								char* szVolCurve,
                                                                    char* getCashVol_NYK( char
       *szVolCurve, double dStartDate, double dEndDate, double dCashStrike, int iZero, char
       *szRefRate, double *dVol, double *dPower ),
    */
    int      nDates,
    long*    lStartDates,
    long*    lEndDates,
    ...
    /*	for ( i=0; i<nDates; i++ )
                    for ( j=i; j<nDates; j++ )
                    {
                            if( err = getCashVol_NYK( szVolCurve, (double) lStartDates[i],
                                                    (double) lEndDates[i], dvFwds[i], 0, szRefRate,
       &dVol, &dPower )  ) return err; dmVar[i][j] = dVol * dVol * dvTime[i];
                    }
    */
    ...
    for (iDiag = 0; iDiag < nDates; iDiag++)
    {
        for (iCol = iDiag; iCol < nDates - iDiag; iCol++)
        {
            iRow = iDiag + iCol;
            if (err = sumOuter(
                    iRow, iCol, iRow, dmLvl, nDates, dvSigma, dvDeltaT, *out_dmCorrel, &dSum))
                return err;

            if (err = StationaryAutocorrel(iRow + 1, iCol, iRow, nDates, dvSigma, dvDeltaT, &dAC))
                return err;

            (*out_dmCorrel)[iRow + 1][iCol] =
                0.5 *

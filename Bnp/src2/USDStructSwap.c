...
Err BMA_Option( 
				...
				char* szVolCurveName,
				char* szRefRate,
				Err (*GetVol)(char*, double, double, double, int, char*, double*, double*),
				double *out_dPrice,
				double *out_dVega )
{
...
/* Calculate the expiry times */
	if ( err = getExerciseDate( (long) dStartDate1, szRefRate, enumBusDayConv, &lExercise )  )
		return err;
	dExpiry1 = ( lExercise - dToday ) / 365.0;
	dExpiry1 = dExpiry1 < 0.0 ? 0.0 : dExpiry1;

	if ( err = getExerciseDate( (long) dStartDate2, szRefRate, enumBusDayConv, &lExercise )  )
		return err;
...
/* Get the volatilities */
	...
	{
		if ( err = (*GetVol)( szVolCurveName, dStartDate1, dEndDate1, dEffStrike, bIsACap, szRefRate, &dVol1, &dPower ) )
			return err;
	} 
	else
		dVol1 = 0.0;
	...
	{
		if ( err = (*GetVol)( szVolCurveName, dStartDate2, dEndDate2, dEffStrike, bIsACap, szRefRate, &dVol2, &dPower ) )
			return err;
	} 
	else
		dVol2 = 0.0;
...
/* Convert the vols to the requested model type */
	srt_f_optsarbvol( dLibor, dEffStrike, dExpiry1, dVol1, 0.0, dPower, 0.0, srtVol, srtPricing, &dVol1 );
	srt_f_optsarbvol( dLibor, dEffStrike, dExpiry2, dVol2, 0.0, dPower, 0.0, srtVol, srtPricing, &dVol2 );

/* Calculate the effective vol*/
	dEffVol = sqrt( 2.0 * dVol1 * dVol1 * dExpiry1 / 3.0 / dExpiry2 + dVol2 * dVol2 / 3.0 );

/* calculate the price */
	if ( bUseNormalModel == 1 )
		*out_dPrice = srt_f_optblknrm( dLibor, dEffStrike, dEffVol, dExpiry2, dLevel, srtCallPut, PREMIUM );
	else
		*out_dPrice = srt_f_optblksch( dLibor, dEffStrike, dEffVol, dExpiry2, dLevel, srtCallPut, PREMIUM );
...
/* Bump the vols, recalculate the price and hence obtain the vega */
	if ( bUseNormalModel == 1 )
	{
		...
		dEffVol = sqrt( 2.0 * dVol1 * dVol1 * dExpiry1 / 3.0 / dExpiry2 + dVol2 * dVol2 / 3.0 );
		*out_dVega = srt_f_optblknrm( dLibor, dEffStrike, dEffVol, dExpiry2, dLevel, srtCallPut, PREMIUM );
	} 
	else
	{
		dVol1 = dVol1;
		dVol2 = dVol2;
		dEffVol = sqrt( 2.0 * dVol1 * dVol1 * dExpiry1 / 3.0 / dExpiry2 + dVol2 * dVol2 / 3.0 );
		*out_dVega = srt_f_optblksch( dLibor, dEffStrike, dEffVol, dExpiry2, dLevel, srtCallPut, PREMIUM );
	}
...
char* getCashVolwithType( char* szVolCurve,
			long lToday, 
			long lExpiry,
			long lStart,
			double lEnd, 
...
// Get the vol
	if ( err = getCashVol( szVolCurve, (double) lStart, (double) lEnd, dCashStrike, 0, szRefRate, out_dCashVol, &dPower ) )
		return err;

// Check that the volatility is consistent with the model, otherwise convert it as required
	if ( dPower == 0.0 )
	...
	if ( enumInVol != enumOutVol )
	{
		...
		if ( err = srt_f_optsarbvol( dCashFwd, dCashStrike, dExpiry, *out_dCashVol, 
												0.0, dPower, 0.0, enumInVol, enumOutVol, out_dCashVol ) )
			return err;
...
// Get the long forward if it is not passed in
	...
	{
		double dRatioDF, dCvg;
		if ( err = (*getDF)( szYieldCurveName, (double) lFutureStart, (double) lFutureEnd, &dRatioDF ) )
			return err;
...
// Get the long vol if it is not passed in
	if ( dLongCashVol < 0.0 )
	{
		if ( err = getCashVolwithType( szVolCurve, lToday, lFutureExpiry, lFutureStart, lFutureEnd, dLongCashFwd, 
					dCashStrike, szRefRate, iVolatilityModel, getCashVol, &dLongCashVol )  )
			return err;
	}
...
// Get the short vol if it is not passed in
	...
	{
		double dATMLongVol, dATMShortVol, dStdDev, dLongExpiry, dShortCashStrike, dShortExpiry;
		long lShortExpiry, lShortStart, lShortEnd;
		lShortExpiry = lFutureExpiry - lOptionExpiry + lToday;
		lShortStart = add_unit( lShortExpiry, pCcyParam->spot_lag, SRT_BDAY, pCcyParam->swap_bus_day_conv );
		lShortEnd = add_unit( lShortStart, 12/compounding, SRT_MONTH, pCcyParam->swap_bus_day_conv );

// Get the short forward if it is not passed in
		if ( dShortCashFwd < 0.0 )
		{
			double dRatioDF, dCvg;
			if ( err = (*getDF)( szYieldCurveName, (double) lShortStart, (double) lShortEnd, &dRatioDF ) )
				return err;
...
// Get the ATM vols
		if ( err = getCashVolwithType( szVolCurve, lToday, lFutureExpiry, lFutureStart, lFutureEnd, dLongCashFwd, 
					dLongCashFwd, szRefRate, iVolatilityModel, getCashVol, &dATMLongVol )  )
			return err;
		
		if ( err = getCashVolwithType( szVolCurve, lToday, lShortExpiry, lShortStart, lShortEnd, dShortCashFwd, 
					dShortCashFwd, szRefRate, iVolatilityModel, getCashVol, &dATMShortVol )  )
			return err;

// Calculate the std deviations that the option strike is from the long cash fwd, and hence the strike
		...
		if ( iVolatilityModel == 0 )
		{
			...
			dShortCashStrike = dShortCashFwd + dStdDev * dATMShortVol * sqrt( dShortExpiry );
		}
		else
		{
			dStdDev = (  log( dCashStrike / dLongCashFwd ) + 0.5 * dATMLongVol * dATMLongVol * dLongExpiry ) 
							/ dATMLongVol / sqrt(dLongExpiry);
			dShortCashStrike = dShortCashFwd * exp( -0.5 * dATMShortVol * dATMShortVol * dShortExpiry
												+ dStdDev * dATMShortVol * sqrt( dShortExpiry )  );
		}
// Calculate the vols
		if ( err = getCashVolwithType( szVolCurve, lToday, lShortExpiry, lShortStart, lShortEnd, dShortCashFwd, 
					dShortCashStrike, szRefRate, iVolatilityModel, getCashVol, &dShortCashVol )  )
			return err;
	}

// Calculate the price
	return MidCurveOption( 
						lToday,
						lOptionExpiry,
						dCashStrike,
						lFutureExpiry,
...
//	dFuturePrice		market price of the future
//	iFutureTenorInMonths	length of underlying Libor in months (generally 1 or 3)
//	iConvexityModel		0:  future = 100 * (1 - forward), strike is yield strike
//						1:	midcurve strike adjusted by forward-future difference
//						
//	iVolatilityModel	0:  stationary normal volatility
//	enumCallPut			Call/Put on price ( i.e., opposite on yield )
//	enumGreek			PREMIUM, DELTA, etc
//	out_dVol			vol used to price.  Will depend on the volatility model
//
char* MidCurveOption( 
...
// Reverse the call put so that it is on the yield and calculate the intrinsic value
...
// Calculate the price
	if ( iVolatilityModel == 0 )
		*out_dPrice = srt_f_optblknrm( dLongCashFwd, dUndStrike, *out_dVol, dOptionExpiry, 1.0, 
...
static char* StationaryAutocorrel( int iLow, int iHigh, int iTime, int nDates, double* dvVol, 
								double* dvDeltaT, double* out_dAutoCorrel )
{
/* Variable declaration */
	int i;
	double dSum;
...
char* sumInner( int iLow, int iHigh, int iTime, double** dmLvl, int nDates, double* dvVol, 
			double* dvDeltaT, double** dmCorrel, double* out_dSum )
{
/* Variable declaration */
	int i;
...
/* Loop over the relevant indices */
	...
	{
		if ( err = StationaryAutocorrel( iLow, i, iTime, nDates, dvVol, dvDeltaT, &dAC ) )
			return err;
...
char* sumOuter( int iLow, int iHigh, int iTime, double** dmLvl, int nDates, double* dvVol, 
			double* dvDeltaT, double** dmCorrel, double* out_dSum )
{
/* Variable declaration */
	int i;
...
/* Loop over the relevant indices */
	for ( i=iLow + 1; i<=iHigh; i++ )
	{
		...
		if ( err = sumInner( i, iHigh, iTime, dmLvl, nDates, dvVol, dvDeltaT, dmCorrel, 
			&dSumInner )  ) return err;
...
char* CorrelFromStationaryCaps(	long lToday,
/*								char* szVolCurve,
								char* getCashVol_NYK( char *szVolCurve, 
														double dStartDate, 
														double dEndDate, 
														double dCashStrike, 
														int iZero, 
...
/*	for ( i=0; i<nDates; i++ )
		for ( j=i; j<nDates; j++ )
		{
			if( err = getCashVol_NYK( szVolCurve, (double) lStartDates[i], 
						(double) lEndDates[i], dvFwds[i], 0, szRefRate, &dVol,
						&dPower )  )
				return err;
			dmVar[i][j] = dVol * dVol * dvTime[i];
		}
*/
...
/* Bootstrap the correlation matrix */
	for ( iDiag=0; iDiag<nDates; iDiag++ )
	{
		...
		{
			iRow = iDiag + iCol;
			if ( err = sumOuter( iRow, iCol, iRow, dmLvl, nDates, dvSigma, dvDeltaT, *out_dmCorrel,
					&dSum )  ) return err;

			if ( err = StationaryAutocorrel( iRow+1, iCol, iRow, nDates, dvSigma, dvDeltaT, &dAC ) )
				return err;

			(*out_dmCorrel)[iRow+1][iCol] = 0.5 * ( 
				dmVol[iRow][iCol]*dmVol[iRow][iCol]*dvTime[iRow]*dmLvl[iRow][iCol] - dSum ) 
				/ dmLvl[iRow+1][iRow+1] * dmLvl[iCol][iCol] / dAC;
		}
	}

...
static double prob_tri_scud(double y, double L, double U, double A, double mux,
                            double muy, double sigx, double sigy, double rho,
                            double mat, int du, int cp, int nterms);

static double triscud_price(double fwdy, double fwdx, double spoty,
                            double spotx, double strikey, double barriery_up,
                            double barriery_dwn, double barrierx, double sigy,
                            double sigx, double rho, double mat, double disc,
                            SrtCallPutType call_put, int scud_type, int nterms);
...
double srt_f_opttriscud(double fwdy, double fwdx, double spoty, double spotx,
  ...
  {
    f0 = triscud_price(fwdy, fwdx, spoty, spotx, strikey, barriery_up,
                       barriery_dwn, barrierx, sigy, sigx, rho, mat, disc,
                       call_put, scud_type, nterms);
  }
  ...
  if (greek != PREMIUM) {
    ...
    f1 = triscud_price(fwdy, fwdx, spoty, spotx, strikey, barriery_up,
                       barriery_dwn, barrierx, sigy, sigx, rho, mat, disc,
                       call_put, scud_type, nterms);
    ...
    f2 = triscud_price(fwdy, fwdx, spoty, spotx, strikey, barriery_up,
                       barriery_dwn, barrierx, sigy, sigx, rho, mat, disc,
                       call_put, scud_type, nterms);
  }
  ...
  if (greek == PREMIUM) {
    ...
    f3 = triscud_price(fwdy, fwdx, spoty, spotx, strikey, barriery_up,
                       barriery_dwn, barrierx, sigy, sigx, rho, mat, disc,
                       call_put, scud_type, nterms);
...
static double triscud_price(double fwdy, double fwdx, double spoty,
  ...
  premium = fwdy * prob_tri_scud(l_k, l_yd, l_yu, l_x, mux + rho * sigx * sigy,
                                 muy + sigy * sigy, sigx, sigy, rho, mat, du,
                                 cp, nterms);

  premium -= strikey * prob_tri_scud(l_k, l_yd, l_yu, l_x, mux, muy, sigx, sigy,
                                     rho, mat, du, cp, nterms);
...

{
  ...
  double sigxrt, sigyrt, sigx_2, sigy_2;
  int n;
  ...
  for (n = -nterms; n <= nterms; n++) {
    ...
    term1 = (bivar(du * (-A + rho * sigx * bn / sigy + mux * mat) / sigxrt,
                   cp * (-y + bn + muy * mat) / sigyrt, cp * du * rho) -
             bivar(du * (-A + rho * sigx * bn / sigy + mux * mat) / sigxrt,
                   cp * (-B + bn + muy * mat) / sigyrt, cp * du * rho));

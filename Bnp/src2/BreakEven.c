    ...
    double** BEMatrix)
{
    ...
    for (j = 0; j < NUnderlying; ++j)
    {
        ...
        err =
            swp_f_ForwardRate(start1, end1, swap_freq, swap_basis, yc_name, ref_rate_name, &swap1);
        if (err)
        {
            return err;
        }
    ...
    for (i = 1; i < NMaturity; ++i)
    {
        for (j = 0; j < NUnderlying; ++j)
        {
            err = add_tenor(spot_date, maturity_tenor[i], MODIFIED_SUCCEEDING, &start1);
            if (err)
            {
                return err;
            }
            ...
            err = swp_f_ForwardRate(
                start1, end1, swap_freq, swap_basis, yc_name, ref_rate_name, &swap1);
            if (err)
            {
                return err;
            }
            ...
            err = add_tenor(spot_date, maturity_tenor[i - 1], MODIFIED_SUCCEEDING, &start2);
            if (err)
            {
                return err;
            }
            ...
            err = swp_f_ForwardRate(
                start2, end2, swap_freq, swap_basis, yc_name, ref_rate_name, &swap2);
            if (err)
            {
                return err;
            }
            ...
            BE2 = 1 +
                  (start1_lag - today) * (log(vol1 * vol1) - log(vol2 * vol2)) / (start1 - start2);
            ...
            {
                smessage("Negative Break Even for maturity %d and underlying %d ", i + 1, j + 1);
                BEMatrix[i][j] = 0;
            }
        }
    }

...
Err BreakEvenFromMkt(	char *yc_name, char *vc_name, long spot_date, int spot_lag,
				...
				int NMaturity, char **maturity_tenor, 
				int NUnderlying, char **underlying_tenor,
				int NbOfBDayPerYear,
				double **BEMatrix)
{
	Err err = NULL;
	...
	for(i=1;i<NMaturity;++i)
	{
		for(j=0;j<NUnderlying;++j)
		{
			...
			BE2 = 1 + (start1_lag - today)
					* (log(vol1*vol1)-log(vol2*vol2))/(start1 - start2);
...
Err BreakEvenFromVol(	long spot_date, int spot_lag,
				int NMaturity, char **maturity_tenor, 
				int NUnderlying, char **underlying_tenor,
				int NbOfBDayPerYear,
				double **VolMatrix,

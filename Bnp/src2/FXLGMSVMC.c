                      ...
                      double** res)
{
    ...
    double *sum_payoff = NULL, *sum_payoff2 = NULL, *path_payoff = NULL, *res_evt = NULL,
           ...
           *dom_sigf1 = NULL, *dom_sigf2 = NULL, *dom_sigpsi1 = NULL, *dom_sigpsi2 = NULL,
           *dom_sigpsi12 = NULL, *dom_muv1 = NULL, *dom_muv2 = NULL, *dom_sigv = NULL,

           *for_sigf1 = NULL, *for_sigf2 = NULL, *for_sigpsi1 = NULL, *for_sigpsi2 = NULL,
           *for_sigpsi12 = NULL, *for_muv1 = NULL, *for_muv2 = NULL, *for_sigv = NULL,
    ...
    double  dt, sqdt;
    double *dom_ft1 = NULL, *dom_ft2 = NULL, *dom_v = NULL, dom_sqv, *dom_psi1 = NULL,
           *dom_psi2 = NULL, *dom_psi12 = NULL;
    double *for_ft1 = NULL, *for_ft2 = NULL, *for_v = NULL, for_sqv, *for_psi1 = NULL,
           *for_psi2 = NULL, *for_psi12 = NULL;
    double* fwdfx = NULL;
    double* brow  = NULL;
    ...
    if (!matrix || !path_payoff || !sum_payoff || !sum_payoff2 || !res_evt || !dom_sigf1 ||
        !dom_sigf2 || !dom_sigpsi1 || !dom_sigpsi2 || !dom_sigpsi12 || !dom_muv1 || !dom_muv2 ||
        !dom_sigv || !for_sigf1 || !for_sigf2 || !for_sigpsi1 || !for_sigpsi2 || !for_sigpsi12 ||
        !for_muv1 || !for_muv2 || !for_sigv || !fx_sig || !dom_ft1 || !dom_ft2 || !dom_v ||
        !dom_psi1 || !dom_psi2 || !dom_psi12 || !fwdfx || !for_ft1 || !for_ft2 || !for_v ||
        !for_psi1 || !for_psi2 || !for_psi12 || !cholmat)
    {
    ...
    */
    ...
    for (j = 1; j < iNbTime; j++)
    {
        for (i = 0; i < iNumPaths; i++)
        {
            ...
            dom_v[i] = max(
                dom_v[i] + dom_muv2[j] - dom_muv1[j] * dom_v[i] + dom_sigv[j] * dom_sqv * brow[2],
                0.0);
            ...
            fwdfx[i] +=
                -0.5 * (fx_sig[j] * fx_sig[j] + domzcvol1 * domzcvol1 + domzcvol2 * domzcvol2 +
                        forzcvol1 * forzcvol1 + forzcvol2 * forzcvol2);

            fwdfx[i] +=
                -(-fx_sig[j] * domzcvol1 * CorrMatrix[j][6][0] -
    ...
    for (i = 0; i < iNumPaths; i++)
    {
        ...
        if (func_parm_tab[0] && EvalEvent[0])
        {
            ...
            if (do_optimisation)
            {
                mceb_fill_savevalues_from_GRFN(save_values[0], res_evt, i, dom_df, params);
            }
        ...
        for (j = 1; stop_path == 0 && j < iNbTime; j++)
        {
            ...
            if (EvalEvent[j])
            {
                ...
                if (do_optimisation)
                {
                    mceb_fill_savevalues_from_GRFN(
                        save_values[evtindex], res_evt, i, dom_df, params);
                }
    ...
    double** res)
{
    ...
    if (!gauss || !matrix || !path_payoff || !sum_payoff || !sum_payoff2 || !res_evt ||
        !dom_sigf1 || !dom_sigf2 || !dom_sigpsi1 || !dom_sigpsi2 || !dom_sigpsi12 || !dom_muv1 ||
        !dom_muv2 || !dom_sigv || !for_sigf1 || !for_sigf2 || !for_sigpsi1 || !for_sigpsi2 ||
        !for_sigpsi12 || !for_muv1 || !for_muv2 || !for_sigv || !fx_sig || !cholmat)
    {
        err = "Memory allocation failure in fxlgmsv_mc_balsam";
        goto FREE_RETURN;
    }
    ...
    for (i = 0; i < iNumPaths; i++)
    {
        ...
        for (j = 1; stop_path == 0 && j < iNbTime; j++)
        {
            ...
            dom_v = max(
                dom_v + dom_muv2[j] - dom_muv1[j] * dom_v + dom_sigv[j] * dom_sqv * brow[2], 0.0);
            ...
            fwdfx += -0.5 * (fx_sig[j] * fx_sig[j] + domzcvol1 * domzcvol1 + domzcvol2 * domzcvol2 +
                             forzcvol1 * forzcvol1 + forzcvol2 * forzcvol2);
            ...
            {
                dom_df =
                    exp(ddomff_star[evtindex] + domgam1_star[evtindex] * dom_ft1 +
                        domgam2_star[evtindex] * dom_ft2 + domgam1_2_star[evtindex] * dom_psi1 +
                        domgam2_2_star[evtindex] * dom_psi2 + domgam12_star[evtindex] * dom_psi12);

                for_df =
                    exp(dforff_star[evtindex] + forgam1_star[evtindex] * for_ft1 +
                        forgam2_star[evtindex] * for_ft2 + forgam1_2_star[evtindex] * for_psi1 +
                        forgam2_2_star[evtindex] * for_psi2 + forgam12_star[evtindex] * for_psi12);
    ...
    double** res)
{
    ...
    for (i = 0; i < iNumPaths; i++)
    {
        ...
        if (func_parm_tab[0] && EvalEvent[0])
        {
            ...
            if (do_optimisation)
            {
                mceb_fill_savevalues_from_GRFN(save_values[evtindex], res_evt, i, dom_df, params);
            }
    ...
    double** res)
{
    ...
    if (!matrix || !path_payoff || !sum_payoff || !sum_payoff2 || !res_evt || !dom_sigf ||
        !dom_sigpsi || !for_sigf1 || !for_sigf2 || !for_sigpsi1 || !for_sigpsi2 || !for_sigpsi12 ||
        !for_muv1 || !for_muv2 || !for_sigv || !fx_sig || !cholmat)
    {
    ...
    for (i = 0; i < iNumPaths; i++)
    {
        ...
        for (j = 1; stop_path == 0 && j < iNbTime; j++)
        {
            ...
            {
                dom_df =
                    exp(ddomff_star[evtindex] + domgam1_star[evtindex] * dom_ft +
    ...
    double** res)
{
    ...
    double *sum_payoff = NULL, *sum_payoff2 = NULL, *path_payoff = NULL, *res_evt = NULL,
           ...
           *for_sigf = NULL, *for_sigpsi = NULL, *for_muv1 = NULL, *for_muv2 = NULL,
           *for_sigv = NULL,
    ...
    if (!matrix || !path_payoff || !sum_payoff || !sum_payoff2 || !res_evt || !dom_sigf ||
        !dom_sigpsi || !for_sigf || !for_sigpsi || !for_muv1 || !for_muv2 || !for_sigv || !fx_sig ||
        !cholmat)
    {
    ...
    for (i = 0; i < iNumPaths; i++)
    {
        ...
        for (j = 1; stop_path == 0 && j < iNbTime; j++)
        {
            ...
            if (EvalEvent[j])
            {
                ...
                for_df =
                    exp(dforff_star[evtindex] + forgam1_star[evtindex] * for_ft +
...
long	seed = -123456789;
        ...
        if (!matrix || !path_payoff || !sum_payoff || !sum_payoff2 || !res_evt ||
                ...
                !for_sigf1 || !for_sigf2 || !for_sigpsi1 || !for_sigpsi2 || !for_sigpsi12 ||
!for_muv1 || !for_muv2 || !for_sigv || !fx_sig || !cholmat)
        {
                err = "Memory allocation failure in fxlgmsv_mc_balsam";
                goto FREE_RETURN;
        ...
        time2 = clock();
        smessage ("Phase 1 -BalSam generation, time in sec: %.2f", (double) (time2 - time1) /
CLOCKS_PER_SEC);
        ...
        time2 = clock();
        smessage ("Phase 1 -preprocessing, time in sec: %.2f", (double) (time2 - time1) /
CLOCKS_PER_SEC);
...
res_evt[params->iColPay];
                ...
                for (j=1; stop_path == 0 && j<iNbTime; j++)
                {
                        ...
                        for_ft1 += for_sigf1[j] * for_sqv * brow[1];
                        for_ft1 += -for_sigf1[j] * for_sqv * CorrMatrix[j][1][4] * fx_sig[j];
                        for_ft1 += -for_sigf1[j] * for_sqv * ( forzcvol1  + CorrMatrix[j][1][2] *
forzcvol2 ); for_ft1 += +for_sigf1[j] * for_sqv * ( CorrMatrix[j][0][1] * domzcvol1);

                        for_ft2 += for_sigf2[j] * for_sqv * brow[2];
                        for_ft2 += -for_sigf2[j] * for_sqv * CorrMatrix[j][2][4] * fx_sig[j];
                        for_ft2 += -for_sigf2[j] * for_sqv * ( CorrMatrix[j][1][2] * forzcvol1  +
forzcvol2 ); for_ft2 += +for_sigf2[j] * for_sqv * ( CorrMatrix[j][2][0] * domzcvol1);

                        for_psi1 += for_sigpsi1[j] * for_v;
                        for_psi2 += for_sigpsi2[j] * for_v;
                        for_psi12 += for_sigpsi12[j] * for_v;
                        for_v += for_muv2[j] - for_muv1[j] * for_v + for_sigv[j] * for_sqv *
brow[3]; for_v += -for_sigv[j] * for_sqv * CorrMatrix[j][3][4] * fx_sig[j]; for_v += -for_sigv[j] *
for_sqv * ( CorrMatrix[j][1][3] * forzcvol1  + CorrMatrix[j][2][3] * forzcvol2 ); for_v +=
+for_sigv[j] * for_sqv * ( CorrMatrix[j][3][0] * domzcvol1); for_v = max(0.0, for_v);

                        if (EvalEvent[j])
                        {
                                dom_df = exp (ddomff_star[evtindex] + domgam1_star[evtindex] *
dom_ft1
                                                + domgam1_2_star[evtindex] * dom_psi1);

                                // USELESS SINCE NO FX DIFFUSION
                                //for_df = exp (dforff_star[evtindex] + forgam1_star[evtindex] *
for_ft1 + forgam2_star[evtindex] * for_ft2
...
res_evt[params->iColPay];
                ...
                if (do_optimisation && params->iKnockInCol)
                {
                        ...
                        for (j=0; j<iNbEvent; j++)
                        {
                                if (optimise[j])
                                {
                                        save_values[j][params->iNbIndex][i] = path_payoff[(int)
(save_values[j][params->iNbIndex][i] + 0.5)];
                                }
                        }
                }
        }
        ...
        time1 = clock();
        smessage ("Phase 2 -convolution, time in sec: %.2f", (double) (time1 - time2) /
CLOCKS_PER_SEC);
        ...
        if (do_optimisation)
        {
                ...
                time2 = clock();
                smessage ("Phase 3 -optimisation, time in sec: %.2f", (double) (time2 - time1) /
CLOCKS_PER_SEC);
        }
...
res_evt[params->iColPay];
                ...
                for (j=1; stop_path == 0 && j<iNbTime; j++)
                {
                        ...
                        for_ft1 += for_sigf1[j] * for_sqv * brow[3];
                        for_ft1 += -for_sigf1[j] * for_sqv * CorrMatrix[j][3][6] * fx_sig[j];
                        for_ft1 += -for_sigf1[j] * for_sqv * ( forzcvol1  + CorrMatrix[j][3][4] *
forzcvol2 ); for_ft1 += +for_sigf1[j] * for_sqv * ( CorrMatrix[j][3][0] * domzcvol1);

                        for_ft2 += for_sigf2[j] * for_sqv * brow[4];
                        for_ft2 += -for_sigf2[j] * for_sqv * CorrMatrix[j][4][6] * fx_sig[j];
                        for_ft2 += -for_sigf2[j] * for_sqv * ( CorrMatrix[j][4][3] * forzcvol1  +
forzcvol2 ); for_ft2 += +for_sigf2[j] * for_sqv * ( CorrMatrix[j][4][0] * domzcvol1);

                        for_psi1 += for_sigpsi1[j] * for_v;
                        for_psi2 += for_sigpsi2[j] * for_v;
                        for_psi12 += for_sigpsi12[j] * for_v;
                        for_v += for_muv2[j] - for_muv1[j] * for_v + for_sigv[j] * for_sqv *
brow[5]; for_v += -for_sigv[j] * for_sqv * CorrMatrix[j][5][6] * fx_sig[j]; for_v += -for_sigv[j] *
for_sqv * ( CorrMatrix[j][5][3] * forzcvol1  + CorrMatrix[j][5][4] * forzcvol2 ); for_v +=
+for_sigv[j] * for_sqv * ( CorrMatrix[j][5][0] * domzcvol1); for_v = max(0.0, for_v);

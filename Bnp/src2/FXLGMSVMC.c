/* ==========================================================================
   ...
   PURPOSE:		Monte Carlo FX Black-Scholes / IR Dom LGMSV2F / IR for LGMSV2F
...
double	dt, sqdt;
double	*dom_ft1 = NULL, *dom_ft2 = NULL, *dom_v = NULL, dom_sqv, *dom_psi1 = NULL, *dom_psi2 = NULL, *dom_psi12 = NULL;
double	*for_ft1 = NULL, *for_ft2 = NULL, *for_v = NULL, for_sqv, *for_psi1 = NULL, *for_psi2 = NULL, *for_psi12 = NULL;
double	*fwdfx = NULL;
double	*brow = NULL;
...
SRandomGen		rg;
	...
	if (!matrix || !path_payoff || !sum_payoff || !sum_payoff2 || !res_evt ||
		!dom_sigf1 || !dom_sigf2 || !dom_sigpsi1 || !dom_sigpsi2 || !dom_sigpsi12 || !dom_muv1 || !dom_muv2 || !dom_sigv ||
		!for_sigf1 || !for_sigf2 || !for_sigpsi1 || !for_sigpsi2 || !for_sigpsi12 || !for_muv1 || !for_muv2 || !for_sigv ||
		!fx_sig || !dom_ft1 || !dom_ft2 || !dom_v || !dom_psi1 || !dom_psi2 || !dom_psi12 ||
		!fwdfx || !for_ft1 || !for_ft2 || !for_v || !for_psi1 || !for_psi2 || !for_psi12 ||
		!cholmat)
	{
	...
	*/
	...
	for (j=1; j < iNbTime; j++)
	{
		for (i=0; i<iNumPaths; i++)
		{
			...
			dom_v[i] = max(dom_v[i] + dom_muv2[j] - dom_muv1[j] * dom_v[i] + dom_sigv[j] * dom_sqv * brow[2], 0.0);
			
			for_ft1[i] += for_sigf1[j] * for_sqv * brow[3];
			for_ft1[i] += -for_sigf1[j] * for_sqv * CorrMatrix[j][3][6] * fx_sig[j];
			for_ft1[i] += -for_sigf1[j] * for_sqv * ( forzcvol1  + CorrMatrix[j][3][4] * forzcvol2 );
			for_ft1[i] += +for_sigf1[j] * for_sqv * ( CorrMatrix[j][3][0] * domzcvol1  + CorrMatrix[j][3][1] * domzcvol2 );
			
			for_ft2[i] += for_sigf2[j] * for_sqv * brow[4];
			for_ft2[i] += -for_sigf2[j] * for_sqv * CorrMatrix[j][4][6] * fx_sig[j];
			for_ft2[i] += -for_sigf2[j] * for_sqv * ( CorrMatrix[j][4][3] * forzcvol1  + forzcvol2 );
			for_ft2[i] += +for_sigf2[j] * for_sqv * ( CorrMatrix[j][4][0] * domzcvol1  + CorrMatrix[j][4][1] * domzcvol2 );
			
			for_psi1[i] += for_sigpsi1[j] * for_v[i];
			for_psi2[i] += for_sigpsi2[j] * for_v[i];
			for_psi12[i] += for_sigpsi12[j] * for_v[i];
			for_v[i] += for_muv2[j] - for_muv1[j] * for_v[i] + for_sigv[j] * for_sqv * brow[5];
			for_v[i] += -for_sigv[j] * for_sqv * CorrMatrix[j][5][6] * fx_sig[j];
			for_v[i] += -for_sigv[j] * for_sqv * ( CorrMatrix[j][5][3] * forzcvol1  + CorrMatrix[j][5][4] * forzcvol2 );
			for_v[i] += +for_sigv[j] * for_sqv * ( CorrMatrix[j][5][0] * domzcvol1  + CorrMatrix[j][5][1] * domzcvol2 );
			for_v[i] = max(0.0, for_v[i]);

			fwdfx[i] += (fx_sig[j] * brow[6]
						- domzcvol1 * brow[0] - domzcvol2 * brow[1]
						+ forzcvol1 * brow[3] + forzcvol2 * brow[4] );

			fwdfx[i] += -0.5 * (fx_sig[j] * fx_sig[j] 
							+ domzcvol1 * domzcvol1 + domzcvol2 * domzcvol2
							+ forzcvol1 * forzcvol1 + forzcvol2 * forzcvol2 );
	...
	for (i=0; i<iNumPaths; i++)
	{
		...
		{			
			...
			err = payoff_func(	0,
								...
								exp(matrix[i][evtindex * NbInfoSave + 12]) * dom_df / for_df,								
		...
		{																
			...
			if (EvalEvent[j])
			{
				dom_df = exp (ddomff_star[evtindex] + domgam1_star[evtindex] * matrix[i][evtindex * NbInfoSave + 0] + domgam2_star[evtindex] * matrix[i][evtindex * NbInfoSave + 1]
						+ domgam1_2_star[evtindex] * matrix[i][evtindex * NbInfoSave + 2] + domgam2_2_star[evtindex] * matrix[i][evtindex * NbInfoSave + 3] + domgam12_star[evtindex] * matrix[i][evtindex * NbInfoSave + 4]);

				for_df = exp (dforff_star[evtindex] + forgam1_star[evtindex] * matrix[i][evtindex * NbInfoSave + 6] + forgam2_star[evtindex] * matrix[i][evtindex * NbInfoSave + 7]
						+ forgam1_2_star[evtindex] * matrix[i][evtindex * NbInfoSave + 8] + forgam2_2_star[evtindex] * matrix[i][evtindex * NbInfoSave + 9] + forgam12_star[evtindex] * matrix[i][evtindex * NbInfoSave + 10]);
	...
	if (do_optimisation)
	{
		...
		if (matrix) free_dmatrix (matrix, 0, iNumPaths - 1, 0, NbInfoSave * iNbEvent - 1);
		matrix = NULL;
...
FREE_RETURN:
	...
	if (dom_sigf2) free_dvector (dom_sigf2, 0, iNbTime - 1);
	if (dom_sigpsi1) free_dvector (dom_sigpsi1, 0, iNbTime - 1);
	if (dom_sigpsi2) free_dvector (dom_sigpsi2, 0, iNbTime - 1);
	if (dom_sigpsi12) free_dvector (dom_sigpsi12, 0, iNbTime - 1);
	if (dom_muv1) free_dvector (dom_muv1, 0, iNbTime - 1);
	if (dom_muv2) free_dvector (dom_muv2, 0, iNbTime - 1);
	if (dom_sigv) free_dvector (dom_sigv, 0, iNbTime - 1);
	...
	if (cholmat) free_f3tensor(cholmat, 0, iNbTime - 1, 0, 6, 0, 6);

	if (CorrMatrix_loc) free_dmatrix(CorrMatrix_loc, 0, 6, 0, 6);

	mceb_free_savevalues_for_GRFN(save_values, iNumPaths, iNbEvent, params);
	
	if (dom_ft1) free_dvector(dom_ft1, 0, iNumPaths - 1); 
	if (dom_ft2) free_dvector(dom_ft2, 0, iNumPaths - 1); 
	if (dom_v) free_dvector(dom_v, 0, iNumPaths - 1); 
	if (dom_psi1) free_dvector(dom_psi1, 0, iNumPaths - 1); 
	if (dom_psi2) free_dvector(dom_psi2, 0, iNumPaths - 1); 
	if (dom_psi12) free_dvector(dom_psi12, 0, iNumPaths - 1);

	if (for_ft1) free_dvector(for_ft1, 0, iNumPaths - 1); 
	if (for_ft2) free_dvector(for_ft2, 0, iNumPaths - 1); 
	if (for_v) free_dvector(for_v, 0, iNumPaths - 1); 
	if (for_psi1) free_dvector(for_psi1, 0, iNumPaths - 1); 
	if (for_psi2) free_dvector(for_psi2, 0, iNumPaths - 1); 
	if (for_psi12) free_dvector(for_psi12, 0, iNumPaths - 1);

	if (fwdfx) free_dvector(fwdfx, 0, iNumPaths - 1);

	if (gauss) free_dvector(gauss, 0, NbFactor);
...
double	*gauss = NULL;
	...
	if (!gauss || !matrix || !path_payoff || !sum_payoff || !sum_payoff2 || !res_evt ||
		!dom_sigf1 || !dom_sigf2 || !dom_sigpsi1 || !dom_sigpsi2 || !dom_sigpsi12 || !dom_muv1 || !dom_muv2 || !dom_sigv ||
		!for_sigf1 || !for_sigf2 || !for_sigpsi1 || !for_sigpsi2 || !for_sigpsi12 || !for_muv1 || !for_muv2 || !for_sigv ||
		!fx_sig ||
		!cholmat)
	...
	for (i=0; i<iNumPaths; i++)
	{
		...
		{																
			...
			dom_v = max(dom_v + dom_muv2[j] - dom_muv1[j] * dom_v + dom_sigv[j] * dom_sqv * brow[2], 0.0);
			
			for_ft1 += for_sigf1[j] * for_sqv * brow[3];
			for_ft1 += -for_sigf1[j] * for_sqv * CorrMatrix[j][3][6] * fx_sig[j];
			for_ft1 += -for_sigf1[j] * for_sqv * ( forzcvol1  + CorrMatrix[j][3][4] * forzcvol2 );
			for_ft1 += +for_sigf1[j] * for_sqv * ( CorrMatrix[j][3][0] * domzcvol1  + CorrMatrix[j][3][1] * domzcvol2 );
			
			for_ft2 += for_sigf2[j] * for_sqv * brow[4];
			for_ft2 += -for_sigf2[j] * for_sqv * CorrMatrix[j][4][6] * fx_sig[j];
			for_ft2 += -for_sigf2[j] * for_sqv * ( CorrMatrix[j][4][3] * forzcvol1  + forzcvol2 );
			for_ft2 += +for_sigf2[j] * for_sqv * ( CorrMatrix[j][4][0] * domzcvol1  + CorrMatrix[j][4][1] * domzcvol2 );
			
			for_psi1 += for_sigpsi1[j] * for_v;
			for_psi2 += for_sigpsi2[j] * for_v;
			for_psi12 += for_sigpsi12[j] * for_v;
			for_v += for_muv2[j] - for_muv1[j] * for_v + for_sigv[j] * for_sqv * brow[5];
			for_v += -for_sigv[j] * for_sqv * CorrMatrix[j][5][6] * fx_sig[j];
			for_v += -for_sigv[j] * for_sqv * ( CorrMatrix[j][5][3] * forzcvol1  + CorrMatrix[j][5][4] * forzcvol2 );
			for_v += +for_sigv[j] * for_sqv * ( CorrMatrix[j][5][0] * domzcvol1  + CorrMatrix[j][5][1] * domzcvol2 );
			for_v = max(0.0, for_v);

			fwdfx += (fx_sig[j] * brow[6]
						- domzcvol1 * brow[0] - domzcvol2 * brow[1]
						+ forzcvol1 * brow[3] + forzcvol2 * brow[4] );

			fwdfx += -0.5 * (fx_sig[j] * fx_sig[j] 
							+ domzcvol1 * domzcvol1 + domzcvol2 * domzcvol2
							+ forzcvol1 * forzcvol1 + forzcvol2 * forzcvol2 );
			...
			if (EvalEvent[j])
			{
				dom_df = exp (ddomff_star[evtindex] + domgam1_star[evtindex] * dom_ft1 + domgam2_star[evtindex] * dom_ft2
						+ domgam1_2_star[evtindex] * dom_psi1 + domgam2_2_star[evtindex] * dom_psi2 + domgam12_star[evtindex] * dom_psi12);

				for_df = exp (dforff_star[evtindex] + forgam1_star[evtindex] * for_ft1 + forgam2_star[evtindex] * for_ft2
						+ forgam1_2_star[evtindex] * for_psi1 + forgam2_2_star[evtindex] * for_psi2 + forgam12_star[evtindex] * for_psi12);
	...
	{
		/* Free the big matrix of memory first */
		if (matrix) free_dmatrix (matrix, 0, iNumPaths - 1, 0, NbFactor * (iNbTime - 1) - 1);
		matrix = NULL;
...
long	seed = -123456789;
	...
	for (i=0; i<iNumPaths; i++)
	{
		...
		{																
			...
			for_ft1 += -for_sigf1[j] * for_sqv * ( forzcvol1  + CorrMatrix[j][1][2] * forzcvol2 );
			...
			// Previous code:
			// for_ft2 += -for_sigf2[j] * for_sqv * ( CorrMatrix[j][2][3] * forzcvol1  + forzcvol2 );
			for_ft2 += -for_sigf2[j] * for_sqv * ( CorrMatrix[j][2][1] * forzcvol1  + forzcvol2 );
			for_ft2 += +for_sigf2[j] * for_sqv * ( CorrMatrix[j][2][0] * domzcvol );

			for_psi1 += for_sigpsi1[j] * for_v;
			for_psi2 += for_sigpsi2[j] * for_v;
			for_psi12 += for_sigpsi12[j] * for_v;
			for_v += for_muv2[j] - for_muv1[j] * for_v + for_sigv[j] * for_sqv * brow[3];
			for_v += -for_sigv[j] * for_sqv * CorrMatrix[j][3][4] * fx_sig[j];
			for_v += -for_sigv[j] * for_sqv * ( CorrMatrix[j][3][1] * forzcvol1  + CorrMatrix[j][3][2] * forzcvol2 );
	...
	{
		// Free the big matrix of memory first
		if (matrix) free_dmatrix (matrix, 0, iNumPaths - 1, 0, 4 * (iNbTime - 1) - 1);
		matrix = NULL;
...
FREE_RETURN:
	...
	if (dom_sigf) free_dvector (dom_sigf, 0, iNbTime - 1);
	if (dom_sigpsi) free_dvector (dom_sigpsi, 0, iNbTime - 1);
	...
	if (cholmat) free_f3tensor(cholmat, 0, iNbTime - 1, 0, 3, 0, 3);

	if (CorrMatrix_loc) free_dmatrix(CorrMatrix_loc, 0, 4, 0, 4);
...
long	seed = -123456789;
	...
	if (!matrix || !path_payoff || !sum_payoff || !sum_payoff2 || !res_evt ||
		!dom_sigf || !dom_sigpsi ||
		!for_sigf || !for_sigpsi || !for_muv1 || !for_muv2 || !for_sigv ||
		!fx_sig ||
		!cholmat)
	{
	...
	for (i=0; i<iNumPaths; i++)
	{
		...
		{																
			...
			for_v += for_muv2[j] - for_muv1[j] * for_v + for_sigv[j] * for_sqv * brow[2];
...
FREE_RETURN:
	...
	if (for_sigf) free_dvector (for_sigf, 0, iNbTime - 1);
	if (for_sigpsi) free_dvector (for_sigpsi, 0, iNbTime - 1);
	if (for_muv1) free_dvector (for_muv1, 0, iNbTime - 1);
	if (for_muv2) free_dvector (for_muv2, 0, iNbTime - 1);
	if (for_sigv) free_dvector (for_sigv, 0, iNbTime - 1);
	...
	if (brow) free_dvector (brow, 0, 2);

	if (cholmat) free_f3tensor(cholmat, 0, iNbTime - 1, 0, 2, 0, 2);
...
long	seed = -123456789;
	...
	if (do_optimisation)
	{
		save_values = f3tensor(0, iNbEvent - 1, 0, params->iNbIndex, 0, iNumPaths - 1);
		opt_bar = dvector(0, iNbEvent - 1);
		coef_lin = dmatrix(0, iNbEvent - 1, 0, params->iNbIndex);

		if (!save_values || !opt_bar || !coef_lin)
		{
			err = "Memory allocation (2) failure in fxlgmsv_mc_balsam";
			goto FREE_RETURN;
		}
	}
	...
	for (i=0; i<iNumPaths; i++)
	{
		...
		{			
			...
			if (do_optimisation)
			{
				for (i=0; i<iNumPaths-1; i++)
				{
					...
					{
						save_values[0][k][i] = res_evt[params->iColBound+k];
					}

					if (!params->iKnockInCol)
					{
						save_values[0][params->iNbIndex][i] = res_evt[params->iColPay] / dom_df;
					}
					else
					{
						save_values[0][params->iNbIndex][i] = res_evt[params->iColPay];
					}
				}
			}
		...
		{																
			...
			for_ft1 += -for_sigf1[j] * for_sqv * CorrMatrix[j][1][4] * fx_sig[j];
			for_ft1 += -for_sigf1[j] * for_sqv * ( forzcvol1  + CorrMatrix[j][1][2] * forzcvol2 );
			for_ft1 += +for_sigf1[j] * for_sqv * ( CorrMatrix[j][0][1] * domzcvol1);
			
			for_ft2 += for_sigf2[j] * for_sqv * brow[2];
			for_ft2 += -for_sigf2[j] * for_sqv * CorrMatrix[j][2][4] * fx_sig[j];
			for_ft2 += -for_sigf2[j] * for_sqv * ( CorrMatrix[j][1][2] * forzcvol1  + forzcvol2 );
			for_ft2 += +for_sigf2[j] * for_sqv * ( CorrMatrix[j][2][0] * domzcvol1);
			...
			for_v += for_muv2[j] - for_muv1[j] * for_v + for_sigv[j] * for_sqv * brow[3];
			for_v += -for_sigv[j] * for_sqv * CorrMatrix[j][3][4] * fx_sig[j];
			for_v += -for_sigv[j] * for_sqv * ( CorrMatrix[j][1][3] * forzcvol1  + CorrMatrix[j][2][3] * forzcvol2 );
			for_v += +for_sigv[j] * for_sqv * ( CorrMatrix[j][3][0] * domzcvol1);
			for_v = max(0.0, for_v);
			...
			if (EvalEvent[j])
			{
				...
				if (do_optimisation)
				{
					for (k=0; k<params->iNbIndex; k++)
					{
						save_values[evtindex][k][i] = res_evt[params->iColBound+k];
					}

					if (!params->iKnockInCol)
					{
						save_values[evtindex][params->iNbIndex][i] = res_evt[params->iColPay] / dom_df;
					}
					else
					{
						save_values[evtindex][params->iNbIndex][i] = res_evt[params->iColPay];
					}
				}
...
FREE_RETURN:
	...
	if (save_values) free_f3tensor(save_values, 0, iNbEvent - 1, 0, 2, 0, iNumPaths - 1);	
	if (opt_bar) free_dvector(opt_bar, 0, iNbEvent - 1);
	if (coef_lin) free_dmatrix(coef_lin, 0, iNbEvent - 1, 0, params->iNbIndex);
...
/*
// qtolgmsv2f_mc_balsam using the fxlgmsv scheme in order to check the accuracy of the
...
long	seed = -123456789;
	...
	for (i=0; i<iNumPaths; i++)
	{
		...
		{																
			...
			for_ft1 += -for_sigf1[j] * for_sqv * CorrMatrix[j][3][6] * fx_sig[j];
			for_ft1 += -for_sigf1[j] * for_sqv * ( forzcvol1  + CorrMatrix[j][3][4] * forzcvol2 );
			for_ft1 += +for_sigf1[j] * for_sqv * ( CorrMatrix[j][3][0] * domzcvol1);
			
			for_ft2 += for_sigf2[j] * for_sqv * brow[4];
			for_ft2 += -for_sigf2[j] * for_sqv * CorrMatrix[j][4][6] * fx_sig[j];
			for_ft2 += -for_sigf2[j] * for_sqv * ( CorrMatrix[j][4][3] * forzcvol1  + forzcvol2 );
			for_ft2 += +for_sigf2[j] * for_sqv * ( CorrMatrix[j][4][0] * domzcvol1);
			...
			for_v += -for_sigv[j] * for_sqv * CorrMatrix[j][5][6] * fx_sig[j];
			for_v += -for_sigv[j] * for_sqv * ( CorrMatrix[j][5][3] * forzcvol1  + CorrMatrix[j][5][4] * forzcvol2 );
			for_v += +for_sigv[j] * for_sqv * ( CorrMatrix[j][5][0] * domzcvol1);
			for_v = max(0.0, for_v);
	...
	if (do_optimisation)
	{
		...
		if (matrix) free_dmatrix (matrix, 0, iNumPaths - 1, 0, 7 * (iNbTime - 1) - 1);
		matrix = NULL;

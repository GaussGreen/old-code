    ...
    double** AlphaCcyB)
{
    ...
    *AlphaCcyB = (double*)srt_malloc(NumZCcyB * sizeof(double));
    ...
    for (i = 1; i < NumZCcyA; i++)
        (*AlphaCcyA)[i] = sqrt(
            max((tsPtrA->zeta[i] - tsPtrA->zeta[i - 1]) /
                    ((double)(tsPtrA->zdate[i] - tsPtrA->zdate[i - 1])),
                0));

    for (i = 1; i < NumZCcyB; i++)
        (*AlphaCcyB)[i] = sqrt(
            max((tsPtrB->zeta[i] - tsPtrB->zeta[i - 1]) /
...
double LGMIntAlpha(long lNum, Date* dates, double* dAlpha, Date lStartDate, Date lEndDate)
{
    long   i;
    double dIntegral = .0;
...
double LGMIntAlphaUAlphaV(
    long lNum, Date* dates, double* dAlphaU, double* dAlphaV, Date lStartDate, Date lEndDate)
{
    long   i;
    double dIntegral = .0;
    ...
    double** dgamma)
{
    ...
    double  dInt2Fx = .0, dIntAFx = .0, dIntBFx = .0, dIntAB = .0, dIntA = .0, dIntB = .0;
    ...
    *dgamma = (double*)srt_malloc(lNumFx * sizeof(double));
    ...
    if (error = LGMExtractAlphaFromTS(tsPtrA, tsPtrB, &AlphaA, &AlphaB))
        return error;
    if (error = LGMExpandAlphaUAlphaV(
            tsPtrA->numZ,
            tsPtrA->zdate,
            AlphaA,
            tsPtrB->numZ,
    ...
    for (i = 1; i < lNumFx; i++)
    {
        ...
        dCi = dInt2Fx + 2.0 * dCorrelBFx * dHBi * dIntBFx - 2.0 * dCorrelAFx * dHAi * dIntAFx +
              dHAi * dHAi * dksiA + dHBi * dHBi * dksiB - 2.0 * dCorrelAB * dHAi * dHBi * dIntAB -
        ...
        dIntA = LGMIntAlpha(tsPtrA->numZ, tsPtrA->zdate, AlphaA, datesFx[i - 1], datesFx[i]);

        dIntB = LGMIntAlpha(tsPtrB->numZ, tsPtrB->zdate, AlphaB, datesFx[i - 1], datesFx[i]);

        dIntAFx += (*dgamma)[i] * dIntA;
        dIntBFx += (*dgamma)[i] * dIntB;
        dIntAB +=
            LGMIntAlphaUAlphaV(lNumAB, dDatesAB, AlphaCcyA, AlphaCcyB, datesFx[i - 1], datesFx[i]);
    }
...
LGMErr LGMReducAndMatchMoments(
    LGM_TS*  tsPtrA,
    LGM_TS*  tsPtrB,
    long     lNumEx,
    Date*    datesEx,
    ...
    double** dSki22)
{
    ...
    double  dIntGB = .0, dIntB = .0, dIntA = .0, dIntGA = .0, dIntG2 = .0, dIntAB = .0, m = 1.0,
           dDeltaLambda, dksiB;
    LGMErr error;
    ...
    for (i = 2; i < lNumEx; i++)
    {
        ...
        dIntGA += LGMIntAlphaUAlphaV(
            lNumAFx, dDatesAFx, dGammaFxA, dAlphaAFx, datesEx[i - 1], datesEx[i]);

        dIntGB += LGMIntAlphaUAlphaV(
            lNumBFx, dDatesBFx, dGammaFxB, dAlphaBFx, datesEx[i - 1], datesEx[i]);

        dIntG2 += LGMIntAlpha(lNumFx, datesFx, dGamma, datesEx[i - 1], datesEx[i]);

        dksiB = LGMZetaFromTS(datesEx[i], 0, tsPtrB);

        m *= (1 + dDeltaLambda * dCorrelBFx * dIntGB + dLambda[i] * dDeltaLambda * dksiB) /
             (dIntG2 + 2 * dLambda[i] * dCorrelBFx * dIntGB + dLambda[i] * dLambda[i] * dksiB);

        (*dSki12)[i] = (dCorrelAFx * dIntGA + dLambda[i] * dCorrelAB * dIntAB) / m;

        (*dSki22)[i] =
            (dIntG2 + 2.0 * dLambda[i] * dCorrelBFx * dIntGB + dLambda[i] * dLambda[i] * dksiB) /
            m / m;
    }
    return NULL;
}

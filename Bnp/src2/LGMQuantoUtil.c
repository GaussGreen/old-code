...
LGMErr LGMExtractAlphaFromTS(LGM_TS *tsPtrA, LGM_TS *tsPtrB,
  ...
  *AlphaCcyB = (double *)srt_malloc(NumZCcyB * sizeof(double));
  ...
  for (i = 1; i < NumZCcyA; i++)
    (*AlphaCcyA)[i] =
        sqrt(max((tsPtrA->zeta[i] - tsPtrA->zeta[i - 1]) /
                     ((double)(tsPtrA->zdate[i] - tsPtrA->zdate[i - 1])),
                 0));

  for (i = 1; i < NumZCcyB; i++)
    (*AlphaCcyB)[i] =
        sqrt(max((tsPtrB->zeta[i] - tsPtrB->zeta[i - 1]) /
                     ((double)(tsPtrB->zdate[i] - tsPtrB->zdate[i - 1])),
                 0));
...
LGMErr LGMExpandAlphaUAlphaV(long lNumU, Date *datesU,
                             ...
                             long *lNum, Date **dates, double **dNewAlphaU,
                             double **dNewAlphaV) {
  long lNumMax, i, j, k;
  ...
  *dates = (Date *)srt_malloc(lNumMax * sizeof(Date));
  ...
  for (k = 0; k < lNumMax || i < lNumU || j < lNumV; k++) {
    ...
    if (datesU[i] > datesV[j])
      (*dates)[k] = datesV[j++];
    else {
      if (datesU[i] < datesV[j])
        (*dates)[k] = datesU[i++];
      else if (datesU[i++] == datesV[j])
        (*dates)[k] = datesV[j++];
    }
  }
...
double LGMIntAlpha(long lNum, Date *dates, double *dAlpha, Date lStartDate,
                   Date lEndDate) {
  long i;
  double dIntegral = .0;
...
double LGMIntAlphaUAlphaV(long lNum, Date *dates, double *dAlphaU,
                          double *dAlphaV, Date lStartDate, Date lEndDate) {
  long i;
  double dIntegral = .0;
...
LGMErr LGMCalibGammaFromTSnFx(LGM_TS *tsPtrA, LGM_TS *tsPtrB, long lNumFx,
                              Date *datesFx, double *VolFx, double dCorrelAB,
                              double dCorrelAFx, double dCorrelBFx,
                              /* outputs linked to lNumFx and datesFx*/
                              double **dgamma) {
  double *AlphaA = NULL, *AlphaB = NULL, *AlphaCcyA = NULL, *AlphaCcyB = NULL;
  double dInt2Fx = .0, dIntAFx = .0, dIntBFx = .0, dIntAB = .0, dIntA = .0,
         dIntB = .0;
  ...
  *dgamma = (double *)srt_malloc(lNumFx * sizeof(double));
  if (*dgamma == NULL)
    return "Error in CalibGammaFromTSnFx";
  ...
  for (i = 1; i < lNumFx; i++) {
    ...
    dCi = dInt2Fx + 2.0 * dCorrelBFx * dHBi * dIntBFx -
          2.0 * dCorrelAFx * dHAi * dIntAFx + dHAi * dHAi * dksiA +
          dHBi * dHBi * dksiB - 2.0 * dCorrelAB * dHAi * dHBi * dIntAB -
    ...
    if (dBi * dBi >= 4.0 * dDeltatFx * dCi && dCi <= .0)
      (*dgamma)[i] =
          (sqrt(dBi * dBi - 4.0 * dDeltatFx * dCi) - dBi) / 2.0 / dDeltatFx;
    else
      (*dgamma)[i] = .0;

    dInt2Fx += (*dgamma)[i] * (*dgamma)[i] * dDeltatFx;

    dIntA = LGMIntAlpha(tsPtrA->numZ, tsPtrA->zdate, AlphaA, datesFx[i - 1],
                        datesFx[i]);

    dIntB = LGMIntAlpha(tsPtrB->numZ, tsPtrB->zdate, AlphaB, datesFx[i - 1],
                        datesFx[i]);

    dIntAFx += (*dgamma)[i] * dIntA;
    dIntBFx += (*dgamma)[i] * dIntB;
    dIntAB += LGMIntAlphaUAlphaV(lNumAB, dDatesAB, AlphaCcyA, AlphaCcyB,
...
/* ------------ Step 6 of the quanto autocal: Matching moments  ,
                                from 3 gaussian variables to 2 gaussian
   variables --------- */
/****************************************************************************/
/*
...
LGMErr LGMComputeLambdaForMatchM(LGM_TS		*tsPtrB  ,
                                                                 long
lNumEx  , Date		*datesEx  , long		lNumCashFlow  , Date
*PayDatesB  , Date		*StartDatesB  , Date		tNow  , char
*cYCName  , double		*db  , double		*dbReduce  , double
*dStrikes  , double		**dLambda)
{
        long	i  , k;
        ...
        *dLambda = (double *) srt_malloc(lNumEx * sizeof(double));
        ...
        for (i = 1; i < lNumEx; i++)
        {
                ...
                for(k = 1; k < lNumCashFlow; k++)
                {
                        ...
                        {
                                dCashFlow = swp_f_df(tNow  , PayDatesB[k]  ,
cYCName)*db[k]; dSum1 += dCashFlow; dSum2 += dCashFlow*LGMGFromTS(PayDatesB[k]
, tsPtrB);
                        }
                }
        }
...
LGMErr LGMReducAndMatchMoments(LGM_TS *tsPtrA, LGM_TS *tsPtrB, long lNumEx,
                               Date *datesEx, double *dLambda, long lNumFx,
                               Date *datesFx, double *dGamma, double dCorrelAB,
                               double dCorrelAFx, double dCorrelBFx,
                               double **dSki12, double **dSki22) {
  double *dGammaFxA, *dAlphaAFx, *dGammaFxB, *dAlphaBFx, *AlphaA, *AlphaB;
  Date *dDatesAFx, *dDatesBFx;
  long lNumAFx, lNumBFx, i;
  double dIntGB = .0, dIntB = .0, dIntA = .0, dIntGA = .0, dIntG2 = .0,
         dIntAB = .0, m = 1.0, dDeltaLambda, dksiB;
  LGMErr error;

  if (error = LGMExtractAlphaFromTS(tsPtrA, tsPtrB, &AlphaA, &AlphaB))
    return error;

  if (error = LGMExpandAlphaUAlphaV(tsPtrA->numZ, tsPtrA->zdate, AlphaA, lNumFx,
                                    datesFx, dGamma, &lNumAFx, &dDatesAFx,
                                    &dAlphaAFx, &dGammaFxA))
    return error;

  if (error = LGMExpandAlphaUAlphaV(tsPtrB->numZ, tsPtrB->zdate, AlphaB, lNumFx,
                                    datesFx, dGamma, &lNumBFx, &dDatesBFx,
                                    &dAlphaBFx, &dGammaFxB))
    return error;
  ...
  for (i = 2; i < lNumEx; i++) {
    ...
    dIntG2 += LGMIntAlpha(lNumFx, datesFx, dGamma, datesEx[i - 1], datesEx[i]);

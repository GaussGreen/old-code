...
long       dStart,dEnd;
   ...
   else
   {
	...
	for (i=0; i<*nInputs; i++)
	{
		err = swp_f_initSwapDP( startDates[i],
								endDates[i],
								cpdStr[i],
								basisStr[i],
								&sdpArray[i]);
	   ...
	   /* Extracts the relevant fixing date for the deal */
		...
		if ((derivType[i] == SWAPTION) || (derivType[i] == BOND_OPTION))
		{
		...
		{
		/* Compute the forward rate and the spread for the forward period underlying the instrument */
		   ...
		   if (derivType[i] == CAPFLOOR)
			{
			   err = swp_f_CapFloor_SwapDP(&(sdpArray[i]), dForwardRate, pfGetVol,rec_pay[i], refRateStr[i],
					szYieldCurveName, PREMIUM, eLogOrNorm, &ATMprice[i] );
			   if(err)
			   {
			...
			if ((derivType[i] == SWAPTION) || (derivType[i] == BOND_OPTION))
			{
				...
				err= swp_f_Swaption_SwapDP( &(sdpArray[i]), dATMVol, dForwardRate, rec_pay[i], refRateStr[i],
						szYieldCurveName, PREMIUM, eLogOrNorm, &ATMprice[i]);
				if(err)
				{	
					srt_free(date_list.date);

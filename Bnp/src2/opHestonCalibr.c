...
/*-----------------------------------------------------------------------------------------------								
           This is the generic add-in to calibrate the shifted-Heston model. Depending on the 
		   type of calibration that is specified as input, it performs three different algorithms

		   1) If CalibrType = "MATCH_3STRIKES", it calibrates on the BS volatilties associated to 
		   an increasing set of (three) strikes that are inpout through the pointer *CalibrDblArgs. 
		   In all cases, the middle
		   strike HAS TO BE at-the-money for the routine to work. First, the local vol is fitted to 
		   match the ATM volatility, then it is freezed and the correlation is fitted to the volatility
		   skew associated to the two expreme strikes. Finally, the first two parameters are frozem
		   and the volvol (alpha) is fitted to the smile convexity. Then the loop is repeated until 
		   convergence.

  `		   2) If CalibrType = "MATCH_CONV", it minimizes the distance between the Heston-generated
           smile and a set of volatility provided by the user in input. Here, 
		   a global measure of convexity is used to determine the best fit. In this case none of the 
		   input volatilities is exactly matched.
		   
		   3) If CalibrType = "CHI2_MIN", it performs the same algorithm as MATCH_CONV. The only 
		   difference is that the distance between the two sets of BS volatilities is measured in terms
		   of a standard CHI2 distance. In this case none of the 
		   input volatilities is exactly matched.
-----------------------------------------------------------------------------------------------*/ 
...
Err         CalibrateHestonModel(  const double	dForward, 
								   const double dBeta, 
								   const double dTau, 
								   const double dExpiry,
								   const double dUpperBound,
								   const int iNumSteps,
								   const int iIntegerType,
								   const SRT_Boolean isVolInfFix,
								   const SrtCalibrationType CalibrType,
								   const double  *CalibrDblArgs,
								   const double  *CalibrInitArgs,
								   double *CalibratedArgs
								)

{
	...
	double       /* ShiftHeston=HESTON_CONST*(1.-dBeta)/dBeta, */
				  ...
				  LambdaHeston=1./dTau,
				  SigmaHeston=CalibrInitArgs[0],
				  AlphaHeston=CalibrInitArgs[1],
				  RhoHeston=CalibrInitArgs[2],BetaHeston,
				  Sigma_new=SigmaHeston,Sigma_old,Alpha_new=AlphaHeston,Alpha_old;
	...
	case (MATCH_3STRIKES) :
		...
		} while ((fabs(Sigma_new-Sigma_old) > 1.e-7  || fabs(Alpha_new-Alpha_old) > 1.e-3 ) 
			      && n_iter <= ITMAX);
...
/*-----------------------------------------------------------------------------------------------								
            Given the set of SABR parameters, this function returns the equivalent                
            Heston-shift parameter such that the slope of the two smiles ATM are matched               
-----------------------------------------------------------------------------------------------*/
					...
					double		*Beta)
{
	...
	/* Calculates the shift by matching the smile slope ATM between SABR and Heston*/
...
//	(*BetaHeston)=Forward/(ShiftHeston+Forward);
	...
	precalpha = 0.0000000001;
		...
		do {
			...
			err = HestonATMVol(Forward,Maturity,sigma0,x1,*SigmaIftyHeston,*LambdaHeston,*BetaHeston,
					            *RhoHeston,1.0,UpperBound,nSteps,isVolInfFix,&atm_BSHESTON_vol);

			gap = atm_BSSABR_vol-atm_BSHESTON_vol;

			f1=SmileChi2(Forward,Maturity,sigma0,x1,*SigmaIftyHeston,*LambdaHeston,*BetaHeston,*RhoHeston,
				         UpperBound,nSteps,iIntegerType,nK,StrikeVec,SigmaBeta,AlphaSABR,BetaSABR,RhoSABR,gap,isVolInfFix);


			/**********************  computes the Chi2 at x1 **********************/

			err = HestonATMVol(Forward,Maturity,sigma0,x2,*SigmaIftyHeston,*LambdaHeston,*BetaHeston,
					            *RhoHeston,1.0,UpperBound,nSteps,isVolInfFix,&atm_BSHESTON_vol);

			gap = atm_BSSABR_vol-atm_BSHESTON_vol;

			f2=SmileChi2(Forward,Maturity,sigma0,x2,*SigmaIftyHeston,*LambdaHeston,*BetaHeston,*RhoHeston,
				         UpperBound,nSteps,iIntegerType,nK,StrikeVec,SigmaBeta,AlphaSABR,BetaSABR,RhoSABR,gap,isVolInfFix);
...
/*-----------------------------------------------------------------------------------------------	
  Assuming AlphaHeston (the VolVol) and RhoHeston (Correlation) constant, this function
  determines the Heston local (or initial) volatility to match the ATM BS volatility 
-----------------------------------------------------------------------------------------------*/		
...
/*-----------------------------------------------------------------------------------------------	
  Assuming SigmaHeston (the Local Vol) and AlphaHeston (the VolVol) constant, this function
  determines the Heston correlation (RhoHeston) to match the skew at two different points.
-----------------------------------------------------------------------------------------------*/	

Err  HestonCalibrRho           (
					 			const double dForward,
					 			const double dExpiry,
					 			const double *CalibrDblArgs,
					 			const double AlphaHeston,
								const double SigmaIftyHeston,
					 			const double LambdaHeston,
								const double dBeta,
					 			const double SigmaHeston,
					 			const double dUpperBound,
					 			const int iNumSteps,
					 			const int iIntegerType,
								const SRT_Boolean isVolInfFix,
								double 	*RhoHeston)
{
	...
	for (i=1;i<=3;i++) Strikes[i]=CalibrDblArgs[i+1];

	// finds the value of the objective function at the two interval extremes first
...
/*-----------------------------------------------------------------------------------------------	
  Assuming SigmaHeston (the VolVol) and RhoHeston (Correlation) constant, this function
  determines the Heston local (or initial) volatility to match the ATM BS volatility 
-----------------------------------------------------------------------------------------------*/	
...
/*-----------------------------------------------------------------------------------------------								
		...
		This, in turn,  is used in the calibration routine under the specifics "CHI2_MIN"
		where the algorithm is in fact based on the minimization of the Chi2 distance
-----------------------------------------------------------------------------------------------*/

...
/*-----------------------------------------------------------------------------------------------
           This is the generic add-in to calibrate the shifted-Heston model. Depending on the
                   type of calibration that is specified as input, it performs three different
algorithms

                   1) If CalibrType = "MATCH_3STRIKES", it calibrates on the BS volatilties
associated to an increasing set of (three) strikes that are inpout through the pointer
*CalibrDblArgs. In all cases, the middle strike HAS TO BE at-the-money for the routine to work.
First, the local vol is fitted to match the ATM volatility, then it is freezed and the correlation
is fitted to the volatility skew associated to the two expreme strikes. Finally, the first two
parameters are frozem and the volvol (alpha) is fitted to the smile convexity. Then the loop is
repeated until convergence.

  `		   2) If CalibrType = "MATCH_CONV", it minimizes the distance between the
Heston-generated smile and a set of volatility provided by the user in input. Here, a global measure
of convexity is used to determine the best fit. In this case none of the input volatilities is
exactly matched.

                   3) If CalibrType = "CHI2_MIN", it performs the same algorithm as MATCH_CONV. The
only difference is that the distance between the two sets of BS volatilities is measured in terms of
a standard CHI2 distance. In this case none of the input volatilities is exactly matched.
-----------------------------------------------------------------------------------------------*/
...

{
    ...
    double /* ShiftHeston=HESTON_CONST*(1.-dBeta)/dBeta, */
        ...
        LambdaHeston = 1. / dTau, SigmaHeston = CalibrInitArgs[0], AlphaHeston = CalibrInitArgs[1],
        RhoHeston = CalibrInitArgs[2], BetaHeston, Sigma_new = SigmaHeston, Sigma_old,
        Alpha_new       = AlphaHeston, Alpha_old;

...

{
  ...
  **/
  ...
  if (mat2 == mat1) {
    if ((fwd_price1 == fwd_price2) && (vol1 == vol2)) {
      premium = srt_f_optlokbak(fwd_price1, spot, hist_extr, vol1, mat1, disc,
                                call_put, PREMIUM);
    } else
      premium = 0.0;
  } else
    ...
    /** CALL  S - min S(s)  **/
    if (mat1 == 0.0) {
      premium = srt_f_optblksch(fwd_price2, hist_extr, vol2, mat2, disc,
                                call_put, PREMIUM);
    } else if (vol1 == 0.0) {
      ...
      if (vol2 != 0.0) {
        ...
        premium = srt_f_optblksch(fwd_price2, hist_extr, vol, mat2 - mat1, disc,
                                  call_put, PREMIUM);
      } else {
      ...
      premium1 -= hist_extr *
                  bivar(d_ext01 - sqrt(var01), d_ext02 - sqrt(var02), corr12);
      ...
      if (mu01 != 0.0) {
        ...
        premium3 =
            bivar(d_ext01 - sqrt(var01) + 2 * l_ext / sqrt(var01),
                  d_ext02 - sqrt(var02) + 2 * l_ext / sqrt(var02), corr12);
        premium3 *= 1 / nu * spot * exp(l_ext * nu);

        premium4 = bivar(-(d_12 - sqrt(var12)) + nu * sqrt(var12),
                         d_ext01 * sqrt(var01 / var02) -
                             (d_12 - sqrt(var12)) * sqrt(var12 / var02) +
                             nu * var12 / sqrt(var02),
                         sqrt(var12 / var02));
        ...
        premium3 = bivar(
            -d_ext01, d_ext02 - sqrt(var02) + 2 * l_ext / sqrt(var02), corr12);
        premium3 *= -spot - fwd_price1 * (d_ext01 * sqrt(var01));

        premium4 = norm(-(d_12 - sqrt(var12)));
        premium4 -= bivar(-(d_12 - sqrt(var12)),
                          d_ext01 * sqrt(var01 / var02) -
                              (d_12 - sqrt(var12)) * sqrt(var12 / var02),
                          sqrt(var12 / var02));
        ...
        premium5 +=
            ...
            gauss(
                ...
                sqrt(var02)) *
            norm(((-d_12 + sqrt(var12)) * sqrt(var01) - d_ext01 * sqrt(var12)) /
                 sqrt(var02));
        premium5 *= fwd_price1;
      }
    ...
    if (mat1 == 0.0) {
      ...
      if (mu01 != 0.0) {
        ...
        premium5 +=
            sqrt(var02) *
            gauss((d_ext01 * sqrt(var01) - (d_12 - sqrt(var12)) * sqrt(var12)) /
                  sqrt(var02)) *
            norm(((d_12 - sqrt(var12)) * sqrt(var01) + d_ext01 * sqrt(var12)) /
                 sqrt(var02));
        premium5 *= fwd_price1;
  ...
  if (greek == PREMIUM) {
    answer = premium;
  } else if (greek == DELTA_FWD1) { /* DELTA FWD1 UP */
    shift = fwd_price1 / 10000;
    answer = (srt_f_optpaloba(fwd_price1 + shift, fwd_price2, spot, hist_extr,
                              vol1, vol2, mat1, mat2, disc, call_put, PREMIUM) -
              premium) /
             shift;
  } else if (greek == DELTA_FWD2) { /* DELTA FWD2 UP */
    shift = fwd_price2 / 10000;
    answer = (srt_f_optpaloba(fwd_price1, fwd_price2 + shift, spot, hist_extr,
                              vol1, vol2, mat1, mat2, disc, call_put, PREMIUM) -
              premium) /
             shift;
  } else if (greek == DELTA) { /* DELTA */
    shift = spot / 10000;
    answer = (srt_f_optpaloba(fwd_price1 + shift, fwd_price2 + shift,
                              spot + shift, hist_extr, vol1, vol2, mat1, mat2,
  ...
  /*
          else if ( greek == GAMMA_FWD1 )
          {
                  ...
                  answer = ( srt_f_optpaloba(	fwd_price1+shift        ,
                                                                          fwd_price2
           , spot        , hist_extr        , vol1        , vol2        , mat1
     , mat2        , disc        , call_put        , PREMIUM)
  ...
  */
    ...
    shift = spot / 10000;
    answer = (srt_f_optpaloba(fwd_price1 * (1 + shift / spot),
                              fwd_price2 * (1 + shift / spot), spot + shift,
                              hist_extr, vol1, vol2, mat1, mat2, disc, call_put,
                              PREMIUM) +
              srt_f_optpaloba(fwd_price1 * (1 - shift / spot),
                              fwd_price2 * (1 - shift / spot), spot - shift,
                              hist_extr, vol1, vol2, mat1, mat2, disc, call_put,
                              PREMIUM) -
              2 * premium) /
             (shift * shift);
  } else if (greek == VEGA1) { /*   VEGA 1   */
    shift = GVOPT.vol_add;
    answer =
        (srt_f_optpaloba(fwd_price1, fwd_price2, spot, hist_extr, vol1 + shift,
                         vol2, mat1, mat2, disc, call_put, PREMIUM) -
         premium);
  } else if (greek == VEGA2) { /*   VEGA 2   */
    shift = GVOPT.vol_add;
    answer =
        (srt_f_optpaloba(fwd_price1, fwd_price2, spot, hist_extr, vol1,
                         vol2 + shift, mat1, mat2, disc, call_put, PREMIUM) -
         premium);
  } else if (greek == THETA) { /*   THETA   */
    shift = YEARS_IN_DAY;
    answer = (srt_f_optpaloba(fwd_price1, fwd_price2, spot, hist_extr, vol1,
                              vol2, mat1 - shift, mat2 - shift,

...
Err fill_mc_init_lgm2f_lambda(
  ...
  for (n = 0; n < nb_dates - 1; n++) {
    ...
    // For all [Tk  ,Tk+1] between both event dates Tn and Tn+1 where all params
    // are constants
    for (k = StartIndex; k < EndIndex + 1; k++) {
      ...
      // Conditional variance var1_tn1_tn  , var2_tn1_tn  , var12_tn1_tn for
      // phi1  , phi2  , phi12
...
char *SrtGrfnLGM2FMClambda(char *lgmund, int numeventdates, long *eventdates,
  ...
  // Checks that ALL new_lambda[i]  , new_lambda2[i]  , new_lambda[i] +
  // new_lambda2[i] <>0
  for (i = 0; i < n_new_time; i++) {
    if (new_lambda[i] == 0.0) {
  ...
  for (i = xStr.num_evt - 1; i >= 0; i--) {
    if (xStr.evt[i].evt) {
      ...
      if (grfn_prm->num_dom_df > 0) {
        ...
        for (k = 0; k < grfn_prm->num_dom_df; k++) {
          // For this event date i  , reconstruct all future zero coupons needed
          // at time k with k>i
          grfn_prm->dom_dff[k] =
              swp_f_df(xStr.dts[i], grfn_prm->dom_df_dts[k], (char *)dom_yc);

				  ...
				  double	**prod_val)
{
	...
	err = Get_LGM2F_TermStructure(	domestic_name,
									&sig_time,
									&sigma,
									&nbSig,
									&tau,
	...
	if (!void_prm || !is_event || !ifr || !date ||  ((!dff || !gam1 || !gam2 || !gam12) && (max_num_df > 0)))
	{
...
FREE_RETURN:
	...
	if (max_num_df > 0)
	{
		...
		if (gam1) free_dvector(gam1, 0, max_num_df-1);
		if (gam2) free_dvector(gam2, 0, max_num_df-1);
		if (gam12) free_dvector(gam12, 0, max_num_df-1);		
	}
				  ...
				  double	**prod_val)
{
	...
	/* Get the term structure */
	err = Get_LGM2F_TermStructure2(	domestic_name,
									&sigma,
									&sigma_time,
									&nb_sigma,
									&lambda,
...
Err		err = NULL;	
	...
	for (k=0; k<nb_dates-1; k++)
	{
		...
		dom_phi12[k+1] = dom_phi12[k] * exp_dom_mat * exp_dom_mat2 + rho_dom * alpha_dom * phi_dom12;
...
FREE_RETURN:	
	...
	if (covar) free_f3tensor(covar, 0, nstp-1, 0, 1, 0, 1);
...
FREE_RETURN:	
	...
	if (sigma_date_dom)	free (sigma_date_dom);
	if (sigma_dom)	free (sigma_dom);	
	if ( ivOptimise ) free_ivector(ivOptimise, 0, nstp-1 );
					   ...
					   double	*Bk_rhoCk_fwd2)
{
	...
	for (i = 0; i < k + 1 ; i++)
	{
		...
		sumBk1+= sigma_dom2/(2*lda_dom[i])*a1*a1*(1 - exp(-2*lda_dom[i]*(T2i-T1i)));
		sumCk1+= alpha_dom * sigma_dom2 /(lda_dom[i]+lda_dom2[i])*a1*a2*(1-exp(-(lda_dom[i]+lda_dom2[i])*(T2i-T1i)));

		//To be used for Bk2 (the sum for Ck2 is identical to Ck1)
		sumBk2+= alpha_dom2 * sigma_dom2/(2*lda_dom2[i])*a2*a2*(1 - exp(-2*lda_dom2[i]*(T2i-T1i)));
	}
	...
	Ck1 = alpha_dom * sigma_dom_k * sigma_dom_k / (lda_dom[k]+lda_dom2[k]) * (phi_lda_dom - phi_minus_lda_dom2);
	Ck1+= phi_minus_lda_dom2 * sumCk1;
	Ck1*= a1;

	//For fwd2
	Bk2 = alpha_dom2 * sigma_dom_k * sigma_dom_k / (2*lda_dom2[k]) * (phi_lda_dom2 - phi_minus_lda_dom2);
	Bk2+= phi_minus_lda_dom2 * sumBk2;
	Bk2*= a2;

	Ck2 = alpha_dom * sigma_dom_k * sigma_dom_k / (lda_dom[k]+lda_dom2[k]) * (phi_lda_dom2 - phi_minus_lda_dom);
...
Err		err = NULL;	
	...
	for (n=0; n<nb_dates-1; n++)
	{
		...
		err = gamma_lambda(start_mat, pay_time, sig_dates, lda_dom2, nb_sig_dates, &dom_gam2_pay[n]);

		dom_bond_pay[n] = exp(-zc_pay * pay_mat 
		...
		for (k = StartIndex; k < EndIndex + 1; k++)
		{
			...
			var1_tn1_tn += sig_dom2 * aux_exp_lambda1 * aux_exp_lambda1 * Phi_Func(2*lda_dom[k], T2, T1, T2);

			err = exp_lambda(T2, end_mat, sig_dates, lda_dom2, nb_sig_dates, &aux_exp_lambda2);
			var2_tn1_tn += sig_dom2 * aux_exp_lambda2 * aux_exp_lambda2 * Phi_Func(2*lda_dom2[k], T2, T1, T2);

			var12_tn1_tn += sig_dom2 * aux_exp_lambda1 * aux_exp_lambda2 * Phi_Func(lda_dom[k] + lda_dom2[k], T2, T1, T2);
		...
		dom_phi12[n+1] = dom_phi12[n] * exp_dom_mat * exp_dom_mat2 + rho_dom * alpha_dom * var12_tn1_tn;
	...
	for (i=xStr.num_evt-1; i>=0; i--)
	{
		...
		{			
			...
			if (grfn_prm->num_dom_df > 0)
			{
				...
				for (k=0; k<grfn_prm->num_dom_df; k++)
				{
					//For this event date i, reconstruct all future zero coupons needed at time k with k>i
					grfn_prm->dom_dff[k] = swp_f_df (xStr.dts[i], grfn_prm->dom_df_dts[k], (char*) dom_yc);
					err = gamma_lambda(xStr.tms[i], xStr.tms[i] + grfn_prm->dom_df_tms[k], new_time, new_lambda, n_new_time, &grfn_prm->dom_gam1[k]);
					err = gamma_lambda(xStr.tms[i], xStr.tms[i] + grfn_prm->dom_df_tms[k], new_time, new_lambda2, n_new_time, &grfn_prm->dom_gam2[k]);
					grfn_prm->dom_gam12[k] = -0.5 * (grfn_prm->dom_gam1[k] * grfn_prm->dom_gam1[k] * dom_phi1[i+flag] +
													 grfn_prm->dom_gam2[k] * grfn_prm->dom_gam2[k] * dom_phi2[i+flag])
												- grfn_prm->dom_gam1[k] * grfn_prm->dom_gam2[k] * dom_phi12[i+flag];
				}

/* =================================================================================
   ...
   FILENAME: srt_f_cheybeta2fdynamics.cxx
...
Err srt_f_CheyBeta2f_shortrate_drifts(SrtIRMTmInf *tminf, SrtSample *cur_sample,
                                      long und_index, double *drift_x1,
                                      double *drift_x2) {
  Err err = NULL;
  ...
  *drift_x2 = samptr_get(cur_sample, 0, PHI2) +
              samptr_get(cur_sample, 0, CROSSPHI) -
...
/* The Expected values of the short rate components ( E(Xt+dt|Xt) = Xt + drift *
 * dt )  */

Err srt_f_CheyBeta2f_shortrate_forwards(SrtIRMTmInf *tminf,
                                        SrtSample *cur_sample, long und_index,
                                        double delta_t, double *forward_x1,
                                        double *forward_x2) {
  Err err = NULL;

  *forward_x1 =
      samptr_get(cur_sample, und_index, X1) +
      (samptr_get(cur_sample, und_index, PHI1) +
       samptr_get(cur_sample, und_index, CROSSPHI) -
       tminf->ev.twof[0].lambda * samptr_get(cur_sample, und_index, X1)) *
          delta_t;
  ...
  *forward_x2 =
      ...
      (samptr_get(cur_sample, und_index, PHI2) +
       samptr_get(cur_sample, und_index, CROSSPHI) -
       tminf->ev.twof[1].lambda * samptr_get(cur_sample, und_index, X2)) *
          delta_t;
...
Err srt_f_CheyBeta2f_shortrate_localvols(SrtIRMTmInf *tminf,
...
/* The Variances/Covariance of the short rate components ( Var[Xt+dt] = vol *
 * vol * dt) */

Err srt_f_CheyBeta2f_shortrate_variances(SrtIRMTmInf *tminf,
                                         SrtSample *cur_sample, int und_index,
                                         double delta_t, double *variance_x1,
                                         double *variance_x2,
...
Err srt_f_CheyBeta2f_phi_drifts(SrtIRMTmInf *tminf, SrtSample *cur_sample,
                                long und_index, double *drift_phi1,
                                double *drift_phi2, double *drift_phi12) {
  double localvol_x1;
  double localvol_x2;
  double correl_x1x2;
  Err err = NULL;
...
Err srt_f_CheyBeta2f_phi_forwards(SrtIRMTmInf *tminf, SrtSample *cur_sample,
                                  long und_index, double delta_t,
  ...
  err = srt_f_CheyBeta2f_shortrate_variances(tminf, cur_sample, und_index,
                                             delta_t, &variance_x1,
                                             &variance_x2, &covariance_x1x2);

  /* Compute the forwards for the straight Phi's */
  *forward_phi1 = samptr_get(cur_sample, und_index, PHI1) *
                      (1.0 - 2.0 * tminf->ev.twof[0].lambda * delta_t) +
                  variance_x1;

  *forward_phi2 = samptr_get(cur_sample, und_index, PHI2) *
                      (1.0 - 2.0 * tminf->ev.twof[1].lambda * delta_t) +
                  variance_x2;
  ...
  *forward_phi12 =
      samptr_get(cur_sample, und_index, CROSSPHI) *
          (1.0 -
           (tminf->ev.twof[0].lambda + tminf->ev.twof[1].lambda) * delta_t) +
      covariance_x1x2;
...
Err srt_f_CheyBeta2f_forward_sample(SrtStpPtr cur_step, SrtSample *cur_sample,
                                    SrtSample *fwd_sample, long und_index) {
  ...
  err = srt_f_CheyBeta2f_shortrate_forwards(cur_tminf, cur_sample, und_index,
                                            cur_step->delta_t, &forward_x1,
                                            &forward_x2);
  if (err)
    return err;

  /* Compute the forwards of the Phi varaibles */
  err = srt_f_CheyBeta2f_phi_forwards(cur_tminf, cur_sample, und_index,
                                      cur_step->delta_t, &forward_phi1,
                                      &forward_phi2, &forward_phi12);
  ...
  /* Reconstruct the Short Rate from F(0        ,t) stored in the forward sam of
   * next step */
  samptr_get(fwd_sample, und_index, SHORT_RATE) =
      sam_get(next_tminf->fwd_sam, und_index, F_0_t) +
...
Err srt_f_CheyBeta2f_evolve_sample(SrtStpPtr cur_step, SrtSample *cur_sample,
                                   long und_index, double rand1, double rand2,
  ...
  /* Compute the local volatilities of the short rate components and their
   * correlation */
  err = srt_f_CheyBeta2f_shortrate_localvols(cur_tminf, cur_sample, und_index,
                                             &vol_x1, &vol_x2, &corr_x1x2);
  if (err)
    return err;

  /* Compute the forwards of the Phi variables */
  err = srt_f_CheyBeta2f_phi_forwards(cur_tminf, cur_sample, und_index,
                                      cur_step->delta_t, &fwd_phi1, &fwd_phi2,
                                      &fwd_phi12);
  ...
  samptr_get(next_sample, und_index, X1) =
      fwd_x1 + g1 * vol_x1 * cur_step->sqrt_delta_t;
  samptr_get(next_sample, und_index, X2) =
      fwd_x2 + g2 * vol_x2 * cur_step->sqrt_delta_t;
...
Err srt_f_GenericCheyBeta2f_shortrate_localvols(
    SrtIRMTmInf *tminf, SrtSample *cur_sample, int und_index,
    SrtMdlType mdl_type, double *localvol_x1, double *localvol_x2) {
  double sigma1;
  double sigma2;
  double short_rate;
  double beta1;

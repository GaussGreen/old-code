...
static double d_function_onefac( int n, double d, double *coupon,
                double *df, double *s_i, double bond_strike);

static double d_function_twofac(int n,double d, double g, double *coupon,
                double *df, double *s_i_squared, double *S_i_3, double *S_i_4, 
                double bond_strike);
...
static Err twofac_exer_probs_under_QTi_for_g( double *prob,double g,
		double *s_i_3, double *s_i_4, double *s_i_squared,
		double *coupon, double bond_strike, double *df,
		int n,SrtReceiverType rec_pay );
...
/* Check if option has expired */
	...
	if (mdl_dim == ONE_FAC)
	{
		err = onefac_exer_probs_under_QTi( s_i, coupon, 
			bond_strike, df, n, pay_rec, prob);
	}	
	else  
	if (mdl_dim == TWO_FAC)
	{ 
		...
		/* Version 2 : 25 pts Hermite interpolation
							Generic code with Hermite points generated by NR gauss_hermite */
		#if  Version == 2
		...
		{
			err = twofac_exer_probs_under_QTi_for_g(cond_prob, 
													gi[i] * SQRT_TWO,
													s_i_3, 
													s_i_4, 
													s_i_squared,
		...
		double wi[6] = {3.216643615128420E-01, 1.469670480453520E-01, 2.911668791236190E-02,
						2.203380687533160E-03, 4.837184922590710E-05, 1.499927167637000E-07};
		for( i = 0; i <= 5; i++)
		{
			err = twofac_exer_probs_under_QTi_for_g(cond_prob, 
													gi[i],
													...
													n, 
													pay_rec );
			err = twofac_exer_probs_under_QTi_for_g(Reflected_cond_prob, 
													-gi[i],
													s_i_3, 
													s_i_4, 
													s_i_squared,
		...
		{
			err = twofac_exer_probs_under_QTi_for_g( cond_prob, g,
						s_i_3, s_i_4, s_i_squared,
						coupon, bond_strike, df,
						n, pay_rec );
			for (j=0; j<=n; j++)
...
Err      err;
	...
	if (mdl_dim == TWO_FAC)
	{
		err = make_2f_lgm_cum_vols(ts, fixing_time, period_start_time, period_end_time,
								&cv3,&cv4,&s);
    }
	else
	if (mdl_dim == ONE_FAC)
...
/** initial guesses: **/    	
	...
	nstop = 0.0;
	a[0] = 1;
	b[0] = d_function_twofac(n, a[0], g, coupon, df, s_i_squared, 
				s_i_3, s_i_4, bond_strike);
	a[1] = 1.01;
	b[1] = d_function_twofac(n, a[1], g, coupon, df, s_i_squared, 
				s_i_3, s_i_4, bond_strike);
	a[2] = 1.02;
	count = 0;
...
/** iterations **/
	...
	while(nstop < 1.0 && count < MAXITER)
	{
		b[2] = d_function_twofac(n, a[2], g, coupon, df, s_i_squared, 
				s_i_3, s_i_4, bond_strike);

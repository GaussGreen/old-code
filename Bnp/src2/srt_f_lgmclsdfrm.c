...
#define MAXITER 20
#define Version 1
/* if Version == 2 then we use Hermite interpolation with n = 25 (may be generic)
          Version == 1 then we use Hermite interpolation with n = 12 (hardcoded)
          Version == 0 then we use the old version
*/
...
static double lgm_discount_bond_option(
    ...
    double          period_start_df,
    double          period_end_df,
    double          bond_strike,
    TermStruct*     ts,
    SrtReceiverType pay_rec,
    SrtMdlDim       mdl_dim);

static double d_function_onefac(
    int n, double d, double* coupon, double* df, double* s_i, double bond_strike);

static double d_function_twofac(
    int     n,
    double  d,
    double  g,
    double* coupon,
...
static Err onefac_exer_probs_under_QTi(
    double*         s_i,
    double*         coupon,
    double          bond_strike,
    double*         df,
...
static Err twofac_exer_probs_under_QTi_for_g(
    double*         prob,
    double          g,
    double*         s_i_3,
    double*         s_i_4,
...
/* ----------------------------------------------------------------------------
        For the pricing of a cap/floor in LGM (with spreads already in the coupon)
   ---------------------------------------------------------------------------- */
...
{
    int     i, j;
    double *s_i, *s_i_squared, *s_i_3, *s_i_4, *cond_prob, *Reflected_cond_prob, *prob;
    double  price;
    double  intrinsic;
    int     rp;
    Err     err;
    ...
    for (i = 1; i <= n; i++)
    {
        if (mdl_dim == ONE_FAC)
        {
            s_i[i] = sqrt(srt_f_lgm_cum_vol(ts, fixing_time, pay_time[0], pay_time[i], NULL));
        }
        else if (mdl_dim == TWO_FAC)
        {
            err = make_2f_lgm_cum_vols(
                ts, fixing_time, pay_time[0], pay_time[i], &s_i_3[i], &s_i_4[i], &s_i_squared[i]);
        }
    }

    if (mdl_dim == ONE_FAC)
    {
        err = onefac_exer_probs_under_QTi(s_i, coupon, bond_strike, df, n, pay_rec, prob);
    }
    else if (mdl_dim == TWO_FAC)
    {
/* Modif by Alan : Use Hermit point instead of The previous horrible linear integration */
/* Version 2 : 25 pts Hermite interpolation
                                        Generic code with Hermite points generated by NR
   gauss_hermite */
#if Version == 2
        ...
        {
            err = twofac_exer_probs_under_QTi_for_g(
                cond_prob,
                gi[i] * SQRT_TWO,
                s_i_3,
                s_i_4,
                ...
                pay_rec);
            for (j = 0; j <= n; j++)
                prob[j] += INV_SQRT_PI * wi[i] * exp(-0.5 * s_i_3[j] * s_i_3[j]) * cond_prob[j] *
                           exp(-s_i_3[j] * gi[i] * SQRT_TWO);
        }
        free(gi);
        free(wi);
        ...
        double gi[6] = {
            ...
            5.500901704467750};
        double wi[6] = {
            3.216643615128420E-01,
            1.469670480453520E-01,
            2.911668791236190E-02,
            2.203380687533160E-03,
            4.837184922590710E-05,
            1.499927167637000E-07};
        for (i = 0; i <= 5; i++)
        {
            err = twofac_exer_probs_under_QTi_for_g(
                cond_prob, gi[i], s_i_3, s_i_4, s_i_squared, coupon, bond_strike, df, n, pay_rec);
            err = twofac_exer_probs_under_QTi_for_g(
                Reflected_cond_prob,
                -gi[i],
                s_i_3,
...
/* Version 0 : default linear interpolation  form -4; +4 and 50 pts */
        ...
        {
            err = twofac_exer_probs_under_QTi_for_g(
                cond_prob, g, s_i_3, s_i_4, s_i_squared, coupon, bond_strike, df, n, pay_rec);
            for (j = 0; j <= n; j++)
...
/* ------------------------------------------------------------------------------ */
/* Price an option on a discount bond (zero coupon), using one factor gauss
   markov model */
    ...
    SrtMdlDim       mdl_dim)
{
    ...
    {
        err = make_2f_lgm_cum_vols(
            ts, fixing_time, period_start_time, period_end_time, &cv3, &cv4, &s);
    }
    else if (mdl_dim == ONE_FAC)
    {
        s = srt_f_lgm_cum_vol(ts, fixing_time, period_start_time, period_end_time, NULL);
    }
    s = sqrt(s);
    ...
    price =
        rp * period_end_df * norm(rp * (d + s)) - rp * period_start_df * norm(rp * d) * bond_strike;
    ...
    double  bond_strike)
{
    ...
    for (i = 1; i <= n; i++)
    {
        sum += coupon[i] * df[i] * exp(-0.5 * s_i_squared[i] - S_i_3[i] * g - S_i_4[i] * d);
    }
    sum -= bond_strike * df[0];
...
/* =========================================================================
   ...
   unique solution of:
        ...
        sum(1->n){ c_i B(t,Ti)exp(- .5 * s_i_squared - s_i_3*g - s_i_4*d )}
                                        = bond_strike * B(t,T)
   (this is computed using  d_function() with Newton-Rhapson style iteration
    ...
    SrtReceiverType rec_pay)
{
    ...
    nstop = 0.0;
    a[0]  = 1;
    b[0]  = d_function_twofac(n, a[0], g, coupon, df, s_i_squared, s_i_3, s_i_4, bond_strike);
    a[1]  = 1.01;
    b[1]  = d_function_twofac(n, a[1], g, coupon, df, s_i_squared, s_i_3, s_i_4, bond_strike);
    a[2]  = 1.02;
    count = 0;
    ...
    while (nstop < 1.0 && count < MAXITER)
    {
        b[2] = d_function_twofac(n, a[2], g, coupon, df, s_i_squared, s_i_3, s_i_4, bond_strike);

/* ==================================================================
   ...
   MAIN FUNCTION:  Err sobenberg:
        ...
        Err funcs (double data        , double param[]        , double
   *return_value        , int nparam) Err dfuncs (double data        , double
   param[]        , double *return_value        , int nparam]
   ================================================================== */
...
static Err error_function(double *data, double *target, double *weight,
                          long ndata, double *param_0_1, double *min_param,
                          double *max_param, long nparam,
  ...
  *chisq = 0;
  for (i = 1; i <= ndata; i++) {
    ...
    *chisq += ((funcs_value - target[i]) / weight[i]) *
              ((funcs_value - target[i]) / weight[i]);
  }
...
Err sobenberg(double *data, double *target, double *weight, long ndata,
  ...
  for (i = 1; i <= tot_pts; i++) {
    ...
    err = error_function(data, target, weight, ndata, pts[i], min_param,
                         max_param, nparam, funcs, &(values[i]));
    if (err) {
      free_everything;
      return err;
    }
  ...
  for (i = 1; i <= bst_pts; i++) {
    ...
    err = error_function(data, target, weight, ndata, bpts[i], min_param,
                         max_param, nparam, funcs, &(bvalues[i]));
    if (err) {
      free_everything;
      return err;
    }
  ...
  for (i = 1; i <= bst_pts; i++) {
    ...
    memcpy(&(x[kernel_of_point[i]][npts_in_clust[kernel_of_point[i]]][1]),
           &(bpts[i][1]), nparam * sizeof(double));
  ...
  for (i = 1; i <= nclust; i++) {
    ...
    if (!strcmp(rsc_mth, "BESTFIT")) {
      err = quadr_best_fit(x[i], y[i], npts_in_clust[i], nparam, &a, grad, hess,
                           min[i]);
      if (err) {
        free_everything;
        return err;
    ...
    err = error_function(data, target, weight, ndata, min[i], min_param,
                         max_param, nparam, funcs, &(minval[i]));
    if (err) {
      free_everything;
      return err;
    }
  ...
  for (i = 1; i <= nclust; i++) {
    ...
    err = levenberg_marquardt(data, target, weight, ndata, min[i], nparam,
                              niter, dfuncs, &(minval[i]));
    if (err) {
      free_everything;
      return err;
    }

...
/* ==============================================================================
   ...
   FILE NAME:      srt_f_cheybeta_fwd_pricing.cxx
...
static double Local_Lambda_func(double pt, double GT, CHEYBETA_MDL *pmdl) {
...
static Err insert_expiry_inst(double expiry_time, int swapt_idx,
                              SWAPTIONS_DATA *g) {
  int i, j;
  ...
  for (j = g->nex; j > i; j--) {
    ...
    memcpy(&(g->data_e[j][0]), &(g->data_e[j - 1][0]),
           g->ndata_e[j - 1] * sizeof(int));
  }
...
static Err fill_swaption_data(SWAPTION_DATA *data,
                              /*	Underlying info	*/
                              CHEYBETA_MDL *pmdl, char *yc_name, long today,
                              int spot_lag,
                              /*	Swaption info	*/
                              long expiry, double expiry_time, long start,
                              long end, char *freq, char *basis, double strike,
                              double bond_strike, char *pay_rec_str,
                              char *refrate) {
  SwapDP fix_sdp;
  ...
  if (err = swp_f_make_FixedAndNotionalsLeg(&fix_sdp, strike, 1.0, 1.0, today,
                                            &fix_leg)) {
    return err;
  }
  ...
  for (i = 0; i < data->ncpn; i++) {
    data->pay_gamma[i] =
        Local_Lambda_func(expiry_time, data->pay_time[i], pmdl);
  }

  data->pay_0_50_gamma_sqr = calloc(data->ncpn, sizeof(double));
  for (i = 0; i < data->ncpn; i++) {
    data->pay_0_50_gamma_sqr[i] =
        0.50 * data->pay_gamma[i] * data->pay_gamma[i];
  }
...
Err add_new_swaption(
    /*	Underlyin info	*/
    CHEYBETA_MDL *pmdl, char *yc_name, long today, int spot_lag,
    /*	Index of the instrument the option belongs to	*/
    int inst_idx,
    /*	Option info	*/
    long expiry, long start, long end, char *freq, char *basis, double strike,
    double bond_strike, char *pay_rec, char *refrate, SWAPTIONS_DATA *g) {
  ...
  /*	Fill data	*/
  if (err = fill_swaption_data(g->data + g->nswapt - 1, pmdl, yc_name, today,
                               spot_lag, expiry, expiry_time, start, end, freq,
                               basis, strike, bond_strike, pay_rec, refrate)) {
    return err;
  }
...
Err chey_beta_initmem_swaption_data(int ninst, SWAPTIONS_DATA *g) {
  g->ninst = ninst;
  ...
  if (!g->data || !g->ndata_i || !g->data_i || !g->ex || !g->ndata_e ||
      !g->data_e)
    return serror("Memory failure in chey_beta_initmem_swaption_data");
  return NULL;
}

Err chey_beta_fwd_init_swaption_data(CHEYBETA_MDL *pmdl, int ninst, long *start,
                                     long *end, char **freq_str,
                                     char **basis_str, double *strike,
                                     double *bond_strike, char **type_str,
                                     char **pay_rec_str, char **refrate_str,
                                     SWAPTIONS_DATA *g) {
  ...
  for (i = 0; i < ninst; i++) {
    ...
    if (type == SWAPTION) {
      ...
      if (err = add_new_swaption(pmdl, yc_name, today, spot_lag, i, expiry,
                                 start[i], end[i], freq_str[i], basis_str[i],
                                 strike[i], bond_strike[i], pay_rec_str[i],
                                 refrate_str[i], g)) {
        chey_beta_fwd_free_swaption_data(g);
        return err;
      }
      ...
      if (err = swp_f_get_ref_rate_details(refrate_str[i], &rate_basis,
                                           &rate_freq)) {
        chey_beta_fwd_free_swaption_data(g);
        return err;
      }
      ...
      if (err = swp_f_make_SpreadLeg(start[i], end[i], today, refrate_str[i],
                                     &float_leg)) {
        chey_beta_fwd_free_swaption_data(g);
        return err;
      }
      ...
      for (j = 0; j < float_leg.leg_length - 1; j++) {
        expiry = add_unit(float_leg.start_date.date[j], -spot_lag, SRT_BDAY,
                          MODIFIED_SUCCEEDING);
        if (err = add_new_swaption(pmdl, yc_name, today, spot_lag, i, expiry,
                                   float_leg.start_date.date[j],
                                   float_leg.end_date.date[j], rate_freq_str,
                                   rate_basis_str, strike[i], bond_strike[i],
                                   pay_rec_str[i], refrate_str[i], g)) {
          chey_beta_fwd_free_swaption_data(g);
          return err;
        }
      }
...
Err chey_beta_fwd_update_swaption_data_for_model(
    /*	new model	*/
    CHEYBETA_MDL *pmdl, SWAPTIONS_DATA *g) {
  int i, j;
  ...
  for (i = 0; i < g->nswapt; i++) {
    ...
    for (j = 0; j < g->data[i].ncpn; j++) {
      g->data[i].pay_gamma[j] =
          Local_Lambda_func(g->data[i].exp_time, g->data[i].pay_time[j], pmdl);

      g->data[i].pay_0_50_gamma_sqr[j] =
          0.50 * g->data[i].pay_gamma[j] * g->data[i].pay_gamma[j];
    }
  }
...
Err chey_beta_fwd_free_swaption_data(SWAPTIONS_DATA *g) {
  int i;
  ...
  free(g->ndata_i);
  if (g->data_i)
    free_imatrix(g->data_i, 0, g->ninst, 0, MAX_CAPLET - 1);
  free(g->ex);
  free(g->ndata_e);
  if (g->data_e)
    free_imatrix(g->data_e, 0, g->ninst, 0, MAX_OPT_PER_EXP - 1);
  ...
  if (g->ex[exp_idx] < 1.0e-08) {
    ...
    for (i = 0; i < g->ndata_e[exp_idx]; i++) {
      ...
      for (l = 0; l < g->data[g->data_e[exp_idx][i]].ncpn; l++) {
        temp_df = g->data[g->data_e[exp_idx][i]].pay_dff[l];

        iv += temp_df * g->data[g->data_e[exp_idx][i]].pay_cpn[l];
      }
          ...
          for (l = 0; l < g->data[g->data_e[exp_idx][i]].ncpn; l++) {
            temp_df = g->data[g->data_e[exp_idx][i]].pay_dff[l] *
                      exp(-g->data[g->data_e[exp_idx][i]].pay_gamma[l] * x[j] -
                          g->data[g->data_e[exp_idx][i]].pay_0_50_gamma_sqr[l] *
                              phi[k]);

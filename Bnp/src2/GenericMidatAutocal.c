...
void genmidat_free_autocalinfos(GENMIDAT_AUTOCALINFO sInfos) {
  if (sInfos) {
    if (sInfos->dExeBound)
      free(sInfos->dExeBound);
    if (sInfos->dCoefLin)
      free_dmatrix(sInfos->dCoefLin, 0, sInfos->iNbExe - 1, 0, 1);
  }
}
...
Err GenericMidat_OneFactorPayoff_PDE(/* Event */
                                     ...
                                     /* Gride data	*/
                                     long lStartIndex, long lEndIndex,
                                     double *dXt, GENMIDAT_RECONS sReconsParams,
...
Err GenericMidat_TwoFactorPayoff_PDE(/* Event */
                                     ...
                                     /* Gride data	*/
                                     long lStartX, long lEndX, long lStartZ,
                                     long lEndZ,
  ...
  coeftotX = sPayoffParam->dBeta1 * sReconsParams->dCoefX1 +
             sPayoffParam->dBeta2 * sReconsParams->dCoefY1;
  coeftotZ = sPayoffParam->dBeta1 * sReconsParams->dCoefX2 +
             sPayoffParam->dBeta2 * sReconsParams->dCoefY2;
...
Err GenericMidat_TwoFactorPayoff_MC(/* Event */
                                    ...
                                    /* Gride data	*/
                                    long iNumPaths, double *dXt, double *dYt,
                                    GENMIDAT_RECONS sReconsParams,
...
Err GenericMidatAutocal(int iNbExe, double *dLongOptions, double *dShortOptions,
  ...
  Err err = NULL;
  double *dTimes = NULL, **dMCProdVal = NULL;
  ...
  err = generic_midat_calib(iNbExe, dLongOptions, dShortOptions, sModel,
                            sCalibParams, sInfos->sCalibInfos);
  ...
  if (sPDEParams->iPricingMethod == 0) {
    /* PDE */
    err = fill_time_vector(&dTimes, &iNbStep, 0, NULL, 0, NULL,
                           sPDEParams->lNbTime);
  ...
  for (i = iNbStep - 1; i >= 0; i--) {
    if (iIndexEvent >= 0 &&
        fabs(dTimes[i] - sModel->dTimes[iIndexEvent]) < 1.0e-04) {
      if ((sAutocalParams->iOneTimeCallIndex == 0 ||
           iIndexEvent == sAutocalParams->iOneTimeCallIndex - 1) &&
        ...
        if (iIndexEvent < sModel->iNbVols - 1) {
          sPayoffParams[i]->dForward2 =
              sModel->dForward[iIndexEvent] - sModel->dForward[iIndexEvent + 1];
        } else {
        ...
        if (sAutocalParams->iOneTimeCallIndex > 0 &&
            sAutocalParams->iOneTimeShortLong < 0.5 &&
            iIndexEvent < iNbExe - 1) {
  ...
  case 0: {
    ...
    if (sModel->iNbFactor == 1) {
      err = GenericMidat_OneFactor_PDE(
          iNbStep, dTimes, sModel, sPayoffParams, iEvalEvent,
          GenericMidat_OneFactorPayoff_PDE, sPDEParams, 1, MultiCall);
    } else if (sModel->iNbFactor == 2) {
      err = GenericMidat_TwoFactor_PDE(
          iNbStep, dTimes, sModel, sPayoffParams, iEvalEvent,
          GenericMidat_TwoFactorPayoff_PDE, sPDEParams, 1, MultiCall);
    }
  ...
  case 1: {
    ...
    dMCProdVal =
        dmatrix(0, sModel->iNbFactor + iNbStep, 0, 2 + sModel->iNbFactor);
    ...
    if (sAutocalParams->iOneTimeCallIndex == 0 ||
        sAutocalParams->iOneTimeShortLong || sModel->iNbFactor == 1) {
    ...
    if (!sAutocalParams->iCalcExeBound) {
      ...
      err = genmidat_allocate_autocalinfos(iNbStep, sAutocalParams, sInfos);
    ...
    /* MC */
    if (sModel->iNbFactor == 1) {
      err = GenericMidat_OneFactor_MC(iNbStep, dTimes, sModel, sPayoffParams,
                                      GenericMidat_OneFactorPayoff_MC,
                                      sPDEParams, 1, iOptimise, sMCEBParams,
                                      sModel->iNbFactor, dMCProdVal);
    } else if (sModel->iNbFactor == 2) {
      err = GenericMidat_TwoFactor_MC(iNbStep, dTimes, sModel, sPayoffParams,
...
FREE_RETURN:
  ...
  if (dMCProdVal)
    free_dmatrix(dMCProdVal, 0, sModel->iNbFactor + iNbStep, 0,
                 2 + sModel->iNbFactor);

...
Err genmidat_allocate_autocalinfos(
    int iNbExe, GENMIDAT_AUTOCALPARAMS sParams, GENMIDAT_AUTOCALINFO sInfos)
{
    Err err = NULL;
    ...
    GENMIDAT_AUTOCALINFO sInfos)
{
    ...
    err = generic_midat_calib(
        iNbExe, dLongOptions, dShortOptions, sModel, sCalibParams, sInfos->sCalibInfos);
    ...
    {
        /* PDE */
        err = fill_time_vector(&dTimes, &iNbStep, 0, NULL, 0, NULL, sPDEParams->lNbTime);
    ...
    for (i = iNbStep - 1; i >= 0; i--)
    {
        if (iIndexEvent >= 0 && fabs(dTimes[i] - sModel->dTimes[iIndexEvent]) < 1.0e-04)
        {
                ...
                (!iIsCallDate || iIsCallDate[iIndexEvent]))
            {
                ...
                if (sAutocalParams->iOneTimeCallIndex > 0 &&
                    sAutocalParams->iOneTimeShortLong < 0.5 && iIndexEvent < iNbExe - 1)
                {
    ...
    case 1:
    {
        ...
        if (sAutocalParams->iOneTimeCallIndex == 0 || sAutocalParams->iOneTimeShortLong ||
            sModel->iNbFactor == 1)
        {
...
FREE_RETURN:
    ...
    if (dMCProdVal)
        free_dmatrix(dMCProdVal, 0, sModel->iNbFactor + iNbStep, 0, 2 + sModel->iNbFactor);

/* ==========================================================================
   FILE_NAME:	LGMSVClosedForm2.cxx
...
void LGMSVSolveODELogTSEvalFunc(double Coef1Re, double Coef2Re, double Coef2Im,
                                double Coef3Re, double Coef3Im, double expt,
                                double DRe, double DIm, double *ResRe,
                                double *ResIm) {
  prod_comp(Coef1Re * DRe + Coef2Re, Coef1Re * DIm + Coef2Im * expt, DRe, DIm,
            ResRe, ResIm);
  expt *= expt;
  *ResRe += Coef3Re * expt;
  *ResIm += Coef3Im * expt;
...
void LGMSVOptionPricePrecalc(
  ...
  LGMSVCalculateDensityPrecalc(iNbPhi, iNbft, dPhiFreqStep, dftFreqStep, endi,
                               expt2init, Coef1ReT, Coef2ReT, Coef2ImT,
                               Coef3ReT, Coef3ImT, CoefAT, CoefexpT, nbpT,
...
void LGMSVFindFreq(int iNbPhi, int iNbft,
  ...
  *dPhiStep = (dPhitMean - dPhiMin) / (*iIndexPhiMean - 1);
  ...
  if (iPriorityFreqPhi) {
    ...
    LGMSVCalculateDensityTPPoint(dPhiFreqMin, 0.0, endi, expt2init, Coef1ReT,
                                 Coef2ReT, Coef2ImT, Coef3ReT, Coef3ImT, CoefAT,
    ...
    while (LogTFRe > iLimitPhi && i++ < 200) {
      dPhiFreqMin *= 1.2;
      LGMSVCalculateDensityTPPoint(dPhiFreqMin, 0.0, endi, expt2init, Coef1ReT,
                                   Coef2ReT, Coef2ImT, Coef3ReT, Coef3ImT,
                                   CoefAT, CoefexpT, nbpT, dPhitMean, &LogTFRe,
                                   &LogTFIm);
    }
  ...
  *dftStep = -dftMin / (*iIndexft0 - 1);
  ...
  if (iPriorityFreqFt) {
    ...
    LGMSVCalculateDensityTPPoint(0.0, dFtFreqMin, endi, expt2init, Coef1ReT,
                                 Coef2ReT, Coef2ImT, Coef3ReT, Coef3ImT, CoefAT,
                                 CoefexpT, nbpT, dPhitMean, &LogTFRe, &LogTFIm);
...
void LGMSVCalculateDensityTPPoint(double dPhifreq, double dftFreq,
  ...
  for (i = endi; i >= 0; i--) {
    ...
    for (j = 0; j < nbp; j++) {
      ...
      expt2 *= coefexp;
      LGMSVSolveODELogTSEvalFunc(Coef1Re, Coef2Re, Coef2Im, Coef3Re, Coef3Im,
                                 expt2, DRe + 0.5 * K1Re, DIm + 0.5 * K1Im,
                                 &K2Re, &K2Im);
      LGMSVSolveODELogTSEvalFunc(Coef1Re, Coef2Re, Coef2Im, Coef3Re, Coef3Im,
                                 expt2, DRe + 0.5 * K2Re, DIm + 0.5 * K2Im,
                                 &K3Re, &K3Im);
      expt2 *= coefexp;
      LGMSVSolveODELogTSEvalFunc(Coef1Re, Coef2Re, Coef2Im, Coef3Re, Coef3Im,
    ...
    for (iNumftFreq = 1; iNumftFreq <= nbFtHalf; iNumftFreq++) {
      ...
      for (i = endi; i >= 0; i--) {
        ...
        for (j = 0; j < nbp; j++) {
          ...
          LGMSVSolveODELogTSEvalFunc(Coef1Re, Coef2Re, Coef2Im, Coef3Re,
                                     Coef3Im, expt2, DRe, DIm, &K1Re, &K1Im);
          expt2 *= coefexp;
          LGMSVSolveODELogTSEvalFunc(Coef1Re, Coef2Re, Coef2Im, Coef3Re,
                                     Coef3Im, expt2, DRe + 0.5 * K1Re,
                                     DIm + 0.5 * K1Im, &K2Re, &K2Im);
          LGMSVSolveODELogTSEvalFunc(Coef1Re, Coef2Re, Coef2Im, Coef3Re,
                                     Coef3Im, expt2, DRe + 0.5 * K2Re,
                                     DIm + 0.5 * K2Im, &K3Re, &K3Im);
          expt2 *= coefexp;
          LGMSVSolveODELogTSEvalFunc(Coef1Re, Coef2Re, Coef2Im, Coef3Re,
                                     Coef3Im, expt2, DRe + K3Re, DIm + K3Im,
                                     &K4Re, &K4Im);
...
Err LGMSVCalculateTFDensity(
    int iNbPhi, int iNbft, double dPhiFreqStep, double dftFreqStep,

    double dExTime,

    double dLambdaX, int NbSigTime,              /* Term Structure of g(t) */
    double *SigTime, double *Sig, double dTStar, /* Tstar in years from today */
    double dAlpha, double dLambdaEps, double dRho, double PhiMean,

    double MaxTime, int SaveFile,
  ...
  double CoefA;
  double *Coef1ReT = NULL, *Coef2ReT = NULL, *Coef2ImT = NULL, *Coef3ReT = NULL,
         *Coef3ImT = NULL, *CoefAT = NULL, *CoefexpT = NULL;
  ...
  if (!Coef1ReT || !Coef2ReT || !Coef2ImT || !Coef3ReT || !Coef3ImT ||
      !CoefAT || !CoefexpT || !nbpT) {
...
FREE_RETURN:
  ...
  if (CoefAT)
    free_dvector(CoefAT, 0, endi);
  if (CoefexpT)
    free_dvector(CoefexpT, 0, endi);
  if (nbpT)
...
void LGMSVSolveODELogTSRungeKutta(double dPhifreq, double dFtFreq, double T,
...
void LGMSVClosedForm2(
  ...
  if (!Density || !Densityspeq || !Coef1ReT || !Coef2ReT || !Coef2ImT ||
      !Coef3ReT || !Coef3ImT || !CoefAT || !CoefexpT || !nbpT) {
  ...
  LGMSVMomentInit2(dLambdaX, dAlpha, dLambdaEps, dRho, FuncPhi, FuncPhi2,
                   FuncV2, FuncPhiV);
  ExpectPhi = 0.0;
  ExpectPhi2 = 0.0;
  ExpevtV2 = 1.0;
  ...
  /* Find Frequence */
  LGMSVFindFreq(iNbPhi, iNbft, AlphaEq, dRho, endi, expt2init, Coef1ReT,
                Coef2ReT, Coef2ImT, Coef3ReT, Coef3ImT, CoefAT, CoefexpT, nbpT,
                dPhitMean, dPhitStd, dftStd, iNbSigmaPhiGridLeft,
                iNbSigmaPhiGridRight, iNbSigmaftLeft, iNbSigmaftRight, LimitPhi,
                Limitft, iPriorityFreqPhi, iPriorityFreqFt, &dPhiFreqStep,
                &dPhiStep, &iIndexPhiMean, &dftFreqStep, &dftStep, &iIndexft0);

  /* First pricing */
  LGMSVOptionPricePrecalc(iNbPhi, iNbft, dLambdaX, dTStar, endi, expt2init,
                          Coef1ReT, Coef2ReT, Coef2ImT, Coef3ReT, Coef3ImT,
                          CoefAT, CoefexpT, nbpT, dPhitMean, SaveFile, Density,
                          Densityspeq, PayOff, dExTime, iNbCoupon, CouponTime,
                          Coupon, dPhiFreqStep, dPhiStep, iIndexPhiMean,
                          dftFreqStep, dftStep, iIndexft0, Price);
...
Err LGMSVOption(
  ...
  err = swp_f_ForwardRate(cpn_date[0], cpn_date[ncpn - 1], swaption_freq,
  ...
  swp_cash = (swp_f_df(today, cpn_date[0], yc_name) -
              swp_f_df(today, cpn_date[ncpn - 1], yc_name)) /
             level;
  ...
  LGMSVClosedForm2(1.0 / dTau, dNbSig,     /* Term Structure of g(t) */
...
void LGMSVCalib2(
  ...
  double *Coef1ReT = NULL, *Coef2ReT = NULL, *Coef2ImT = NULL, *Coef3ReT = NULL,
         *Coef3ImT = NULL, *CoefAT = NULL, *CoefexpT = NULL, **res_iter = NULL;
  ...
  /* Gestion of allocation errors */
  if (!Density || !Densityspeq || !Coef1ReT || !Coef2ReT || !Coef2ImT ||
      !Coef3ReT || !Coef3ImT || !CoefAT || !CoefexpT || !nbpT || !res_iter) {
    err = "Memory allocation error (1) in LGMSVClosedForm2";
    goto FREE_RETURN;
  }
  ...
  for (i = 0; i < nbEx; i++) {
    ...
    /* Find Frequence */
    LGMSVFindFreq(iNbPhi, iNbft, AlphaEq, dRho, i, expt2init, Coef1ReT,
                  Coef2ReT, Coef2ImT, Coef3ReT, Coef3ImT, CoefAT, CoefexpT,
                  nbpT, dPhitMean, dPhitStd, dftStd, iNbSigmaPhiGridLeft,
                  iNbSigmaPhiGridRight, iNbSigmaftLeft, iNbSigmaftRight,
                  LimitPhi, Limitft, iPriorityFreqPhi, iPriorityFreqFt,
                  &dPhiFreqStep, &dPhiStep, &iIndexPhiMean, &dftFreqStep,
                  &dftStep, &iIndexft0);

    /* First pricing */
    LGMSVOptionPricePrecalc(
        iNbPhi, iNbft, dLambdaX, dTStar, i, expt2init, Coef1ReT, Coef2ReT,
        Coef2ImT, Coef3ReT, Coef3ImT, CoefAT, CoefexpT, nbpT, dPhitMean, 0,
        Density, Densityspeq, PayOff, dExTime[i], iNbCoupon[i], CouponTime[i],
        Coupon[i], dPhiFreqStep, dPhiStep, iIndexPhiMean, dftFreqStep, dftStep,
        iIndexft0, &price1);

    if (price1 < 0.0 || price1 > 10000) {
      smessage(
          "Numerical problem at option %d        , try to decrease MaxTime or "
          "number of Phi/ft",
          i + 1);
      for (j = i; j < nbEx; j++) {
        sigma[j] = 0.0;
      ...
      */
      ...
      while (fabs(TargetPrice[i] - price1) > Precision && k < NbIterMax) {
        ...
        LGMSVOptionPricePrecalc(
            iNbPhi, iNbft, dLambdaX, dTStar, i, expt2init, Coef1ReT, Coef2ReT,
            Coef2ImT, Coef3ReT, Coef3ImT, CoefAT, CoefexpT, nbpT, dPhitMean, 0,
            Density, Densityspeq, PayOff, dExTime[i], iNbCoupon[i],
            CouponTime[i], Coupon[i], dPhiFreqStep, dPhiStep, iIndexPhiMean,
            dftFreqStep, dftStep, iIndexft0, &price2);
        ...
        if (vega < 0.0 || price2 < 0.0 || price2 > 10000) {
          smessage(
              "Numerical problem at option %d        , try to increase MaxTime "
              "or number of Phi/ft",
              i + 1);
          for (j = i; j < nbEx; j++) {
            sigma[j] = 0.0;
...
FREE_RETURN:
  ...
  if (Coef1ReT)
    free_dvector(Coef1ReT, 0, nbEx - 1);
  if (Coef2ReT)
    free_dvector(Coef2ReT, 0, nbEx - 1);
  if (Coef2ImT)
  ...
  if (CoefAT)
    free_dvector(CoefAT, 0, nbEx - 1);
  if (CoefexpT)
    free_dvector(CoefexpT, 0, nbEx - 1);
  if (nbpT)
    free_lvector(nbpT, 0, nbEx - 1);
        ...
        inst_data) /*	NULL = don't save calibration instrument data */
{
  ...
  */
  ...
  for (i = 0; i < nex; i++) {
    ...
    /*
    dDFFTexTstar = dDFTodayTstar / swp_f_df(lExDate        , today +
    DAYS_IN_YEAR * ex_time[i]        , yc_name);
    */
  ...
  *sig = (double *)calloc(nex, sizeof(double));
  ...
  (*sig)[0] = sqrt(ex_zeta[0] * 2 * (dLambdaX) /
                   (exp(2 * (dLambdaX)*ex_time[0]) - 1.0));

  for (i = 1; i < nex; i++) {
    (*sig_time)[i] = ex_time[i];
    if (ex_zeta[i] > ex_zeta[i - 1]) {
      (*sig)[i] = sqrt((ex_zeta[i] - ex_zeta[i - 1]) * 2 * (dLambdaX) /
                       (exp(2 * (dLambdaX)*ex_time[i]) -
                        exp(2 * (dLambdaX)*ex_time[i - 1])));
    } else {
  ...
  LGMSVCalib2(dLambdaX, dTStar, dAlpha, dLambdaEps, dRho, yc_name, ex_time, nex,
              iNbCoupon, CouponTime, Coupon, ex_lprice, ex_lvol, ex_lfwd,
              ex_lstrike, ex_llvl, iNbPhi, iNbft, iNbSigmaPhiGridLeft,
              iNbSigmaPhiGridRight, iNbSigmaftLeft, iNbSigmaftRight, iRatioPhi,
              iRatioFt, iPriorityFreqPhi, iPriorityFreqFt, iMaxTime, Precision,
              NbIterMax, *sig);

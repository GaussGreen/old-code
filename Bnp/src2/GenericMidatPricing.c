    ...
    double** dNewValues)
{
    ...
    if (sParams->iJumpQuadra == 0)
    {
        ...
        for (i = lStart; i <= lEnd; i++)
        {
            ...
            {
                dNewValues[i][j] =
                    (1.0 - dProba) * dOldValues[oldI][j] + dProba * dOldValues[oldI + 1][j];
            }
        }
    }
    else
                                           ...
                                           double*** dNewValues)
{
    ...
    dCoefX =
        1.0 / sqrt(1.0 - sModel->dCorrel[iIndex] * sModel->dCorrel[iIndex]) *
    ...
    if (sParams->iJumpQuadra == 0)
    {
        ...
        for (i = lNewStartX; i <= lNewEndX; i++)
        {
            for (j = lNewStartZ; j <= lNewEndZ; j++)
            {
                ...
                oldZ =
                    max(min(lOldStartZ + (int)((dOldZ - dFirstZ) / dz), lOldEndZ - 1), lOldStartZ);
    ...
    else
    {
        ...
        for (i = lNewStartX; i <= lNewEndX; i++)
        {
            dOldX = dRatio1 * dNewGridX[i];
            oldI =
                max(min(lOldStartX + (int)((dOldX - dFirstX) / dx + 0.5), lOldEndX - 1),
                    lOldStartX + 1);
        ...
        for (i = lNewStartX; i <= lNewEndX; i++)
        {
            for (j = lNewStartZ; j <= lNewEndZ; j++)
            {
                dOldZ = dCoefX * dNewGridX[i] + dCoefZ * dNewGridZ[j];
                oldZ =
                    max(min(lOldStartZ + (int)((dOldZ - dFirstZ) / dz + 0.5), lOldEndZ - 1),
                        lOldStartZ + 1);

                dProbaDown = (dOldZ * (dOldZ - dOldGridZ[oldZ] - dOldGridZ[oldZ + 1]) +
                              dOldGridZ[oldZ] * dOldGridZ[oldZ + 1]) /
                             (2.0 * dz2);
                dProbaMid = (dOldZ * (dOldZ - dOldGridZ[oldZ - 1] - dOldGridZ[oldZ + 1]) +
...
Err GenericMidat_ConvertModelLinear(
    int     iNbVols,
    double* dTimes,
    double* dSigmaTarget,
    double  dSigma0,
...
static Err GenericMidat_VarModelLinear(
    int     iNbVols,
    double* dTimes,
    double* dSigmaTarget,
    double  dSigma0,
    ...
    double* dResult)
{
    ...
    err = GenericMidat_ConvertModelLinear(iNbVols, dTimes, dSigmaTarget, dSigma0, dCoefs, dSigma);
...
Err GenericMidat_OptimalConvertModelLinear(
    int     iNbVols,
    double* dTimes,
    double* dSigmaTarget,
    double* dSigma0,
    ...
    double* dSigma)
{
    ...
    err = GenericMidat_VarModelLinear(iNbVols, dTimes, dSigmaTarget, sig2, dCoefs, dSigma, &var2);
    ...
    while (err && i < 4)
    {
        ...
        err =
            GenericMidat_VarModelLinear(iNbVols, dTimes, dSigmaTarget, sig2, dCoefs, dSigma, &var2);
    ...
    if (err)
    {
        ...
        while (err && i < 4)
        {
            ...
            err = GenericMidat_VarModelLinear(
                iNbVols, dTimes, dSigmaTarget, sig2, dCoefs, dSigma, &var2);
...
static Err solve_integ_exponential(double t1, double t2, double target, double* lam)
{
    Err    err = NULL;
    double lam1, lam2;
...
Err GenericMidat_ConvertModelLambda(
    int     iNbVols,
    double* dTimes,
    double* dSigmaTarget,
    double* dLambda,
    ...
    double* dSigma)
{
    ...
    *dSigma0 = sqrt(dVar / dTimes[iNbVols - 1]);
    ...
    err = GenericMidat_OptimalConvertModelLinear(
        iNbVols, dTimes, dSigmaTarget, dSigma0, dLambda, dSigma);
    ...
    {
        dTarget =
            dSigmaTarget[i] * dSigmaTarget[i] * (dTimes[i] - dLastTime) / dLastSigma / dLastSigma;

        err = solve_integ_exponential(0, dTimes[i] - dLastTime, dTarget, &(dLambda[i]));
                               ...
                               double* dProdVal)
{
    ...
    double *dXtGrid = NULL, *dNextXtGrid = NULL, *dXtGridTemp, *dXtDrift = NULL, *dXtVar = NULL,
           *dXtVarGrid = NULL, *dShortR = NULL, *dXtTotStd = NULL, *dBeta = NULL, *dLambda = NULL,
           *dSigma = NULL;
    ...
    if (sParams->iDiffMethod == 2)
    {
        ...
        err = GenericMidat_ConvertModelLambda(
            sModel->iNbVols, sModel->dTimes, sModel->dSigma, dLambda, &dSigma0, dSigma);
    ...
    if (iEvalEvt[lNbTime - 1])
    {
        ...
        case 2:
        {
            sReconsParams->dCoefX1 =
                dSigma[Get_Index(dTimes[lNbTime - 1], sModel->dTimes, sModel->iNbVols)];
            break;
        }
        ...
        else
        {
            ...
            for (i = lStartX; i <= lEndX; i++)
            {
                ...
                for (j = lStartZ; j <= lEndZ; j++)
                {
                    ...
                    newJ = max(
                        min(lStartZ + (int)((dNewZ - dFirstZ) / dz + 0.5), lEndZ - 1), lStartZ + 1);

                    dProbaDown = (dNewZ * (dNewZ - dNextGridZ[newJ] - dNextGridZ[newJ + 1]) +
                                  dNextGridZ[newJ] * dNextGridZ[newJ + 1]) /
                                 (2.0 * dz2);
                    dProbaMid = (dNewZ * (dNewZ - dNextGridZ[newJ - 1] - dNextGridZ[newJ + 1]) +
                                 dNextGridZ[newJ - 1] * dNextGridZ[newJ + 1]) /
                                (-dz2);
                    dProbaUp = (dNewZ * (dNewZ - dNextGridZ[newJ - 1] - dNextGridZ[newJ]) +
    ...
    else
    {
        ...
        else
        {
            ...
            for (i = lStartX; i <= lEndX; i++)
            {
                dNewX = dCoefX * dGridX[i];
                newI =
                    max(min(lStartX + (int)((dNewX - dFirstX) / dx + 0.5), lEndX - 1), lStartX + 1);

                dProbaDown = (dNewX * (dNewX - dNextGridX[newI] - dNextGridX[newI + 1]) +
                              dNextGridX[newI] * dNextGridX[newI + 1]) /
                             (2.0 * dx2);
                dProbaMid = (dNewX * (dNewX - dNextGridX[newI - 1] - dNextGridX[newI + 1]) +
                               ...
                               double* dProdVal)
{
    ...
    double *dXtGrid = NULL, *dNextXtGrid = NULL, *dXtGridTemp, *dZtGrid = NULL, *dNextZtGrid = NULL,
           *dZtGridTemp, *dXtVar = NULL, *dZtVar = NULL, *dCorrel = NULL, **dXtDrift = NULL,
           **dZtDrift = NULL, **dXtVarGrid = NULL, **dZtVarGrid = NULL, **dShortR = NULL,
           *dXtTotStd = NULL, *dZtTotStd = NULL, *dLambda1 = NULL, *dSigma1 = NULL,
           *dLambda2 = NULL, *dSigma2 = NULL;

    double dSigma01, dSigma02;

    double *dBeta1 = NULL, *dBeta2 = NULL;

    double ***dNextValues = NULL, ***dValues = NULL, ***dTempValues = NULL, ***dValuesTemp;
    ...
    if (!dXtVar || !dZtVar || !dCorrel || !dXtTotStd || !dZtTotStd || !ValueJump || !dBeta1 ||
        !dBeta2 || !sReconsParams)
    {
    ...
    if (sParams->iDiffMethod == 2)
    {
        ...
        /* We need to convert the model */
        err = GenericMidat_ConvertModelLambda(
            sModel->iNbVols, sModel->dTimes, sModel->dSigma, dLambda1, &dSigma01, dSigma1);

        if (err)
            goto FREE_RETURN;

        err = GenericMidat_ConvertModelLambda(
            sModel->iNbVols, sModel->dTimes, sModel->dSigma2, dLambda2, &dSigma02, dSigma2);
    ...
    if (!dXtGrid || !dNextXtGrid || !dZtGrid || !dNextZtGrid || !dXtDrift || !dZtDrift ||
        !dXtVarGrid || !dZtVarGrid || !dShortR || !dNextValues || !dValues || !dTempValues)
    {
    ...
    if (iEvalEvt[lNbTime - 1])
    {
        ...
        case 0:
        {
            ...
            sReconsParams->dCoefX1 = 1.0;
            sReconsParams->dCoefX2 = 0.0;
            sReconsParams->dCoefY1 =
                sModel->dCorrel[index] * sModel->dSigma2[index] / sModel->dSigma[index];
        ...
        case 2:
        {
            ...
            sReconsParams->dCoefY2 =
                sqrt(1.0 - sModel->dCorrel[index] * sModel->dCorrel[index]) * dSigma2[index];
        ...
        if (ValueJump[step] > 0 && (sParams->iDiffMethod == 0 || sParams->iDiffMethod == 2))
        {
            ...
            case 0:
            {
                ...
                sReconsParams->dLastCoefY1 = sModel->dCorrel[index - 1] *
                                             sModel->dSigma2[index - 1] / sModel->dSigma[index - 1];
                              ...
                              double** dProdVal)
{
    ...
    double *dXt = NULL, *dInitGauss = NULL, *dBrownian = NULL, **dPayoff = NULL, **dResEvt = NULL,
           ***dSaveValues = NULL;
    ...
    for (j = 0; j < iNbEvent; j++)
    {
        ...
        if (sFuncParmTab[j])
        {
            ...
            err = payoff_func(
                dTimes[j], sFuncParmTab[j], iNumPaths, dXt, sReconsParams, iNbProd, dResEvt);
...
FREE_RETURN:
    ...
    if (dSaveValues)
        free_f3tensor(dSaveValues, 0, iNbEvent - 1, 0, sMCEBParams->iNbIndex, 0, iNumPaths - 1);
                              ...
                              double** dProdVal)
{
    ...
    double *dXt = NULL, *dYt = NULL, *dInitGauss = NULL, **dBrownian = NULL, **dPayoff = NULL,
    ...
    for (j = 0; j < iNbEvent; j++)
    {
        ...
        {
            err = payoff_func(
                dTimes[j], sFuncParmTab[j], iNumPaths, dXt, dYt, sReconsParams, iNbProd, dResEvt);

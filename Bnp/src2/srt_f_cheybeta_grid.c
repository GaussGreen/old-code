    ...
    int nphi)
{
    ...
    for (t1 = 0, t2 = 1; t2 < grid->nt; t1++, t2++)
    {
        ...
        v_mid = chey_beta_mdl_norm_var_at_t(&p, x_mid, phi_mid, grid->fwdrt[t1], DBL_MAX, 0.0);

        grid->maxvar[t1] =
            chey_beta_mdl_norm_var_at_t(&p, x_max, phi_max, grid->fwdrt[t1], DBL_MAX, 0.0);
        ...
        {
            grid->nphi[t2] = (int)(3 + (nphi - 3) * grid->t[t2] / grid->t[grid->nt - 1]);

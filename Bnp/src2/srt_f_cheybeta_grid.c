/* ==============================================================================
   ...
   FILE NAME:      srt_f_cheybeta_grid.c 
...
int								nphi)
{
	...
	for (t1=0, t2=1; t2<grid->nt; t1++, t2++)
	{
		...
		grid->fwdrt[t1] = swp_f_zr	(	
							mdl->today_date + DAYS_IN_YEAR * grid->t[t1],
							mdl->today_date + DAYS_IN_YEAR * grid->t[t2],
							mdl->yc_name	);

		/*	Update expectation and variances	*/

		v_mid = chey_beta_mdl_norm_var_at_t (	&p, 
												x_mid, 
												phi_mid, 
												grid->fwdrt[t1],
												DBL_MAX,
		...
		grid->maxvar[t1] = chey_beta_mdl_norm_var_at_t (	&p, 
															x_max, 
															phi_max, 
															grid->fwdrt[t1],
															DBL_MAX,
		...
		v_mid = chey_beta_mdl_var (	mdl,
									&p,
									x_mid,
									phi_mid,
									v_mid );

		v_max = chey_beta_mdl_var (	mdl,
									&p,
									x_max,
									phi_max,
									grid->maxvar[t1]	);

		v_min = chey_beta_mdl_var (	mdl,
									&p,
									x_min,
									phi_min,
									grid->minvar[t1]	);
...
double							**prod_val)
{
	...
	if (phi >= grid->phi[ti][grid->nphi[ti]-1])
	{
		j = grid->nphi[ti]-1;
	}
	else

/* =======================================================================================
   ...
   FILENAME :       srt_f_fwdobj.c
	 
   PURPOSE:         structures and functions to store a forward curve for use in Grfn
                    A ForwardCurve is stored as a SrtDvdObj which is a SrtListHdr
...
long     ticker;
	...
	dvd_today = 0.0;
	err = srt_f_lstins(*dvdobj,"_TODAY",(double)today,OBJ_DOUBLE,(void*)(&dvd_today), NULL, &ticker);
	if (err)
...
/* Insert a forward point == object in the SrtList = SrtDvdObj */
	err = srt_f_lstins(dvdobj,"DVD",(double)date,OBJ_DOUBLE,(void*)(&dvd), NULL, &ticker);
	if (err)
		return err;
...
static SrtListAtom *previous_element_with_name(SrtListAtom *lst, char *name,int skip)
{
/* Check that the element exists else nothing to do */
	if(!lst)
		return NULL;
...
/* Go to the First element in the list that is labelled "_TODAY" (not "DVD"...) */
...
/* Go to the First element in the list that is labelled "DVD" (not "_TODAY"...) */
	top = next_element_with_name(dvdobj->head,"DVD",0);
	if(!top) return serror("%s: dvd empty",dvdobj->name);

	/* case where the start date is before the first dividend date */
	if(top->element->key >= (double) (start_date) )
	{
		if(top->element->key == date_today) time = 0.0;
		else time = (start_date-date_today)*YEARS_IN_DAY;
	...
	while(top && (top->element->key <= (double)start_date))
	{
		...
		else time = (top->element->key-date_today)*YEARS_IN_DAY;
...
/* Computes the implied df (inverse of forward ratio) from start_date to end_date */

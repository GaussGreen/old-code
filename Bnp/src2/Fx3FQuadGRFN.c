...
Err allocate_local_model(
    long nbSteps, LOCAL_MODEL_FUNC** model_func, LOCAL_MODEL_PARAM** model_param)
{
    Err err = NULL;
    ...
    double*            fwd_fx)
{
    ...
    for (i = 1; i < nbSteps; i++)
    {
        ...
        model_param->ZU_D_param1[i] =
            model_func->vol_func(i, model_param->Zmin[i], model_param) / model_param->Zmin[i];
        ...
        /* case Floor
        model_param -> ZU_D_param1[i] = model_func -> vol_func(i, model_param -> Zmin[i],
        model_param);
    ...
    double bar_lvl)
{
    ...
    FIRSTMktAtT*   local;
    double *dom_dff = NULL, *dom_gam = NULL, *dom_gam2 = NULL, *fx_dff = NULL, *fx_gam = NULL,
           *fx_gam2 = NULL, *for_dff = NULL, *for_gam = NULL, *for_gam2 = NULL;
    int            i, j, k, l;
    double         temp;
                ...
                for (k = 0; k < n3; k++)
                {
                    ...
                    if (i >= 0)
                    {
                        ...
                        local->smp.und[total->fx_idx].sv[SPOT] =
                            model_func->Z_to_S(
                                step,
                                model_func->U_to_Z(step, bar_lvl, model_param),
                                fwd_fx,
                                model_param) -
                            1.0e-04;
                        memcpy(temp1, prod_val[i][j][k], nprod * sizeof(double));
                        err = FIRSTEvalEvent(global, local, nprod, 2, NULL, NULL, temp1, &temp);
                        ...
                        err = FIRSTEvalEvent(global, local, nprod, 2, NULL, NULL, temp2, &temp);
        ...
        for (j = 0; j < n2; j++)
        {
            for (k = 0; k < n3; k++)
            {
                local->smp.und[total->fx_idx].sv[SPOT] =
                    model_func->Z_to_S(
                        step,
                        model_func->U_to_Z(step, sv[i][j][k][2], model_param),
                        fwd_fx,
                        model_param) +
                    1.0E-08;

							...
							)
{
	...
	{
		return	log((Z - model_param -> ZU_M_param1) / (Z - model_param -> ZU_M_param2)) 
...
static double x;
	...
	else
	{
		...
		//return (U - model_param -> ZU_D_param2[i]) * model_param -> ZU_D_param1[i];
	}
}

/* if delta is negative */

double	Z_to_U_deltaNeg_Quad(
							long				i,
							double				Z,
							LOCAL_MODEL_PARAM	*model_param
							)
...
{	
	...
	else if (U >= model_param -> Umin[i])
	{
		return	tan((U + model_param -> ZU_M_param4) / model_param -> ZU_M_param1)
				/ model_param -> ZU_M_param2 - model_param -> ZU_M_param3;		
	}
	else
	{
		/* Floor on U */
...
double  Z_to_S_Quad		(long i, double Z, double Fwd, LOCAL_MODEL_PARAM	*model_param)
{
	return Z * Fwd;
}

double	S_to_Z_Quad		(long i, double S, double Fwd, LOCAL_MODEL_PARAM	*model_param)
{
	return S / Fwd;
}

double	vol_func_Quad (
						long				i,
						double				Z,
						LOCAL_MODEL_PARAM	*model_param
						)
...
Err	err = NULL;
	...
	if (!(*model_param) -> Zmin || !(*model_param) -> Zmax ||
		!(*model_param) -> Umin || !(*model_param) -> Umax ||
...
long	i;
	...
	for (i=1; i<nbSteps; i++)
	{
		...
		//model_param -> Zmax[i]	 = 1.0 + CAP_STDEV * sig0 * sqrt(time[nbSteps-1]);
		//model_param -> Zmin[i]	 = 1.0 - FLOOR_STDEV * sig0 * sqrt(time[nbSteps-1]);
		...
		model_param -> Umax[i] = model_func -> Z_to_U(i, model_param -> Zmax[i], model_param);
		model_param -> Umin[i] = model_func -> Z_to_U(i, model_param -> Zmin[i], model_param);

		/* Param vol Up */
		model_param -> ZU_U_param1[i] = model_func -> vol_func(i, model_param -> Zmax[i], model_param);
		
		model_param -> ZU_U_param2[i] = model_func -> Z_to_U(i, model_param -> Zmax[i], model_param)
		...
		/* Case Ln */
		model_param -> ZU_D_param1[i] = model_func -> vol_func(i, model_param -> Zmin[i], model_param)
										/ model_param -> Zmin[i];

		model_param -> ZU_D_param2[i] = model_func -> Z_to_U(i, model_param -> Zmin[i], model_param)
...
void free_local_model_param(LOCAL_MODEL_FUNC *model_func,  LOCAL_MODEL_PARAM *model_param)
{

	if (model_param)
	{
		if (model_param -> Zmin) free (model_param -> Zmin);
		if (model_param -> Zmax) free (model_param -> Zmax);
		if (model_param -> Umin) free (model_param -> Umin);
		if (model_param -> Umax) free (model_param -> Umax);

		if (model_param -> volD_param1) free (model_param -> volD_param1);
		if (model_param -> volderivD_param1) free (model_param -> volderivD_param1);
		if (model_param -> ZU_D_param1) free (model_param -> ZU_D_param1);
		if (model_param -> ZU_D_param2) free (model_param -> ZU_D_param2);

		if (model_param -> volU_param1) free (model_param -> volU_param1);
		if (model_param -> volderivU_param1) free (model_param -> volderivU_param1);
		if (model_param -> ZU_U_param1) free (model_param -> ZU_U_param1);
		if (model_param -> ZU_U_param2) free (model_param -> ZU_U_param2);
								...
								double		bar_lvl)
{
			...
			for (j=0; j<n2; j++)
			{
				for (k=0; k<n3; k++)
				{
					...
					if (i >= 0)
					{
						...
						local->smp.und[total->fx_idx].sv[SPOT] = model_func -> Z_to_S(step, model_func -> U_to_Z(step, bar_lvl, model_param), fwd_fx, model_param) - 1.0e-04;
	...
	for (i=0; i<n1; i++)
	{
		for (j=0; j<n2; j++)
		{
			for (k=0; k<n3; k++)
			{				
				local->smp.und[total->fx_idx].sv[SPOT] = model_func -> Z_to_S(step, model_func -> U_to_Z(step, sv[i][j][k][2], model_param), fwd_fx, model_param) + 1.0E-08;
						...
						int		*stop_path)
{
	...
	if (total->do_dom)
	{
		...
		{
			local->evt->df[total->dom_idx][l] 
				= total->dom_dff[l] * exp (-total->dom_gam[l] * Xdom - total->dom_gam2[l]);
		}
	}
	if (total->do_fx)
	{
	...
	if (total->do_for)
	{
		...
		{
			local->evt->df[total->for_idx][l] 
				= total->for_dff[l] * exp (-total->for_gam[l] * Yfor - total->for_gam2[l]);
		}
	}
...
double	*strikes;
	...
	for (i=0; i<n1; i++)
	{
		for (j=0; j<n2; j++)
		{
			for (k=0; k<n3; k++)
			{
				Fx = model_func -> Z_to_S(step, model_func -> U_to_Z(step, sv[i][j][k][2], model_param), fwd_fx, model_param);

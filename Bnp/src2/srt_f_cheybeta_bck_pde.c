...
static void lininter_quick_coef(
    double* x, int n, double x0, double* cup, double* cdown, long* indexup, long* indexdown)
{
    int i;
...
void srt_f_chey_beta_bck_pde_alloc_temp(CHEYBETA_BCK_PDE_TEMP* tmp, int nx, int nphi, int num_prod)
{
    ...
    int t2)
{
    ...
    for (xi = 0; xi < grid->nx; xi++)
    {
        ...
        nvar = chey_beta_mdl_norm_var_at_t(
            &p, x, 0.0, grid->fwdrt[t1], grid->maxvar[t1], grid->minvar[t1]);
    ...
    for (phi_slice = 0; phi_slice < grid->nphi[t1]; phi_slice++)
    {
        ...
        for (xi = 0; xi < grid->nx; xi++)
        {
            ...
            lininter_quick_coef(
                grid->phi[t2],
                grid->nphi[t2],
                chey_beta_mdl_phi(mdl, &p, x, grid->phi[t1][phi_slice], tmpcb->var[xi]),
                &cup,
                &cdown,
                &indexup,
                &indexdown);
            ...
            for (k = 0; k < grid->num_prod; k++)
            {
                ...
                tmpcb->val[xi][k] +=
                    cup * grid->vt2[indexup][xi][k] + cdown * grid->vt2[indexdown][xi][k];
            }
        }
    ...
    for (phi_slice = 0; phi_slice < grid->nphi[t1]; phi_slice++)
    {
        ...
        for (xi = 0; xi < grid->nx; xi++)
        {
            ...
            lininter_quick_coef(
                grid->phi[t1],
                grid->nphi[t1],
                chey_beta_mdl_phi_back(mdl, &p, x, grid->phi[t2][phi_slice], tmpcb->var[xi]),
                &cup,
                &cdown,
                &indexup,
                &indexdown);
    ...
    int t2)
{
    ...
    for (phi_slice = 0; phi_slice < grid->nphi[t1]; phi_slice++)
    {
        ...
        for (xi = 0; xi < grid->nx; xi++)
        {
            ...
            tmpcb->var[xi] = chey_beta_mdl_var(mdl, &p, x, grid->phi[t1][phi_slice], nvar);

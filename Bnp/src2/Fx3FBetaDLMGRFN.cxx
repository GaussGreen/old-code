...
Err grfn_payoff_4_3dfxBetaDLM_mc(
  ...
  local->smp.und[total->fx_idx].sv[SPOT] =
      exp(total->fx_coef_const +
          (total->fx_coef_lin + total->fx_coef_quad * Zfx) * Zfx +
...
Err grfn_payoff_4_3dfxBetaDLMQBeta_Tree(
  ...
  GRFNCOMMSTRUCT global;
  FIRSTMktAtT *local;
  double *dom_const = NULL, *dom_lin = NULL, *fx_const = NULL, *fx_lin = NULL,
         *for_const = NULL, *for_lin = NULL;
  ...
  if (total->num_dom_df > 0 && total->dom_idx != -1) {
    ...
    for (i = 0; i < total->num_dom_df; i++) {
      dom_const[i] =
          log(swp_f_df(evt_date, total->dom_df_dts[i], (char *)dom_yc));
  ...
  if (total->num_fx_df > 0 && total->fx_idx != -1) {
    ...
    for (i = 0; i < total->num_fx_df; i++) {
      fx_const[i] =
          log(swp_f_df(evt_date, total->fx_df_dts[i], (char *)dom_yc));
  ...
  if (total->num_for_df > 0 && total->for_idx != -1) {
    ...
    for (i = 0; i < total->num_for_df; i++) {
      for_const[i] =
          log(swp_f_df(evt_date, total->for_df_dts[i], (char *)for_yc));
  ...
  coef_A =
      -0.5 * (total->varFFX * coef_B * total->B0 * total->B0 - log(coef_B));
  coef_B *= total->B0;

  const_X = 0.5 * (total->varFFX +
                   total->beta_dom * total->beta_dom * total->phi_dom -
  ...
  for (i = 0; i < n1; i++) {
    ...
    for (j = 0; j < n2; j++) {
      ...
      for (k = 0; k < n3; k++) {
        ...
        local->smp.und[total->fx_idx].sv[SPOT] =
            exp(coef_A + temp_X * (coef_B + temp_X * coef_C) +
                coef1_S * svijk[0] + coef2_S * svijk[1]);
        ...
        if (do_fx) {
          for (l = 0; l < total->num_fx_df; l++) {
            local->evt->df[total->fx_idx][l] =
                exp(fx_const[l] + fx_lin[l] * svijk[0]);
          }
        }
        if (do_for) {
          for (l = 0; l < total->num_for_df; l++) {
  ...
  for (i = 0; i < n1; i++) {
    ...
    for (j = 0; j < n2; j++) {
      ...
      for (k = 0; k < n3; k++) {
        ...
        if (total->do_dom) {
          for (l = 0; l < total->num_dom_df; l++) {
            local->evt->df[total->dom_idx][l] = exp(
                total->df_dom_coef_const[l] + total->df_dom_coef_lin[l] * Xdom);
          }
        }
        if (total->do_fx) {
          for (l = 0; l < total->num_fx_df; l++) {
            local->evt->df[total->fx_idx][l] = exp(
                total->df_fx_coef_const[l] + total->df_fx_coef_lin[l] * Xdom);
          }
        }
        if (total->do_for) {
          for (l = 0; l < total->num_for_df; l++) {
            local->evt->df[total->for_idx][l] = exp(
                total->df_for_coef_const[l] + total->df_for_coef_lin[l] * Yfor);
          }
        }
        ...
        err = FIRSTEvalEvent(global, local, nprod, 2, NULL, NULL, prod_valijk,
                             &temp);

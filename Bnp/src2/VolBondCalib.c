...
static double static_lgmsafenorm(double z) {
  if (z < -10) {
    z = -10.0;
  } else if (z > 10) {
    z = 10.0;
...
static double static_lgmystar1F(
    int ncpn,     /*	Num coupons */
    double cpn[], /*	Discounted coupons */
    /*	G at coupon dates and zeta and G at exercise date  , in order to define
       a and b */
...
static void static_lgmcalczeta1(int nsig,       /*	Number of Sigmas */
...
/*	Calculate zeta2 and zeta12 from zeta1  , lambda  , alpha  , gamma and
 * rho in the 2F model */
static void
static_lgmcalczeta2zeta12(int n,          /*	Number of dates */
                          double t[],     /*	Times */
                          double zeta1[], /*	Zeta1 */
                          /*	Lambda  , Alpha  , Beta  , Rho */
...
double lgmopval2FVolBond(int ncpn,         /*	Number of cash-flows */
...
Err lgm2FcalibzetaTo1SwaptionGivenG(
    ...
    double lambda, double alpha, double gamma, double rho, double volInit) {
  ...
  exp_fact12 =
      ((exp((l1 + l2) * ex_time[0]) - exp((l1 + l2) * t)) / (l1 + l2)) /
      ((exp(2 * l1 * ex_time[0]) - exp(2 * l1 * t)) / 2 / l1);
  ...
  pr1 = lgmopval2FVolBond(ncpn, cpn, cpn_G1, cpn_G2, z1, z2, z12, ex_G1[0],
                          ex_G2[0]);
  ...
  pr2 = lgmopval2FVolBond(ncpn, cpn, cpn_G1, cpn_G2, q1, q2, q12, ex_G1[0],
                          ex_G2[0]);
...
Err lgm2FPriceShortSwaptionAndCalibrateLongSwaptionGivenLambda(
    ...
    //	Alpha  , Gamma  , Rho
    double alpha, double gamma, double rho, double volInit,
    double *ShortPrice) //	Short Swaption price as output
{
  ...
  static_lgmsetupG2(lambda, gamma, Longncpn, cpn_time, cpn_G2, nex, ex_time,
                    ex_G2);

  err = lgm2FcalibzetaTo1SwaptionGivenG(
      Longncpn, cpn_time, longcpn, cpn_G, cpn_G2, ex_time, ex_G, ex_G2,
      ex_lstrike, ex_lprice, ex_zeta, lambda, alpha, gamma, rho, volInit);
  ...
  *ShortPrice = lgmopval2FVolBond(Shortncpn, //	Number of cash-flow dates  ,
                                             //including 	start and end date
                                  shortcpn, //
                                  cpn_G, cpn_G2, ex_zeta[0], zeta2[0],
                                  zeta12[0], ex_G[0], ex_G2[0]);
...
Err VolBondCalibration(
  ...
  double ex_time[MAX_CPN], ex_lfwd[MAX_CPN], ex_llvl[MAX_CPN], ex_zeta[MAX_CPN];
  int ex_cpn[MAX_CPN];
  long cpn_date[MAX_CPN];
  double cpn_time[MAX_CPN], shortcpn[MAX_CPN], longcpn[MAX_CPN];
  long today;
  double dfi, dff;
  double power;
  double longMktPrice;
  ...
  err = swp_f_initSwapDP(startDate, shortEndDate, swaption_freq, swaption_basis,
                         &shortSwapDP);
  if (err) {
    goto FREE_RETURN;
  }

  err = swp_f_initSwapDP(startDate, longEndDate, swaption_freq, swaption_basis,
                         &longSwapDP);
  if (err) {
    goto FREE_RETURN;
  }
  ...
  err = swp_f_make_FloatLegDatesCoveragesAndSpreads(
      &shortSwapDP, today, ref_rate_name, &shortfloat_pay_dates,
      &shortfloat_nb_pay_dates, &shortfloat_fixing_dates,
      &shortfloat_start_dates, &shortfloat_end_dates, &shortfloat_cvgs,
      &shortfloat_spreads, &shortfloat_nb_dates);
  if (err) {
    goto FREE_RETURN;
  }

  err = swp_f_make_FloatLegDatesCoveragesAndSpreads(
      &longSwapDP, today, ref_rate_name, &longfloat_pay_dates,
      &longfloat_nb_pay_dates, &longfloat_fixing_dates, &longfloat_start_dates,
      &longfloat_end_dates, &longfloat_cvgs, &longfloat_spreads,
      &longfloat_nb_dates);
  if (err) {
    goto FREE_RETURN;
  }
  ...
  err = swp_f_make_FixedLegDatesAndCoverages(
      &shortSwapDP, today, &shortfix_pay_dates, &shortfix_nb_pay_dates,
      &shortfix_start_dates, &shortfix_end_dates, &shortfix_cvgs,
      &shortfix_nb_dates);
  if (err) {
    goto FREE_RETURN;
  }
  ...
  err = swp_f_make_FixedLegDatesAndCoverages(
      &longSwapDP, today, &longfix_pay_dates, &longfix_nb_pay_dates,
      &longfix_start_dates, &longfix_end_dates, &longfix_cvgs,
      &longfix_nb_dates);
  if (err) {
    goto FREE_RETURN;
  }
  ...
  for (i = 1; i < shortfix_nb_pay_dates; ++i) {
    shortLVL +=
        shortfix_cvgs[i - 1] * swp_f_df(today, shortfix_pay_dates[i], yc_name);
  }
  longLVL = 0;
  for (i = 1; i < longfix_nb_pay_dates; ++i) {
    longLVL +=
        longfix_cvgs[i - 1] * swp_f_df(today, longfix_pay_dates[i], yc_name);
  }
  ...
  for (i = 0; i < shortfloat_nb_dates; ++i) {
    shortFloatPV += swp_f_df(today, shortfloat_pay_dates[i + 1], yc_name) *
                    (shortfloat_cvgs[i] * shortfloat_spreads[i] +
                     swp_f_df(today, shortfloat_start_dates[i], yc_name) /
                         swp_f_df(today, shortfloat_end_dates[i], yc_name) -
                     1);
    shortFloatPVWithoutSpread +=
        swp_f_df(today, shortfloat_pay_dates[i + 1], yc_name) *
        (swp_f_df(today, shortfloat_start_dates[i], yc_name) /
             swp_f_df(today, shortfloat_end_dates[i], yc_name) -
         1);
  }
  ...
  for (i = 0; i < longfloat_nb_dates; ++i) {
    longFloatPV += swp_f_df(today, longfloat_pay_dates[i + 1], yc_name) *
                   (longfloat_cvgs[i] * longfloat_spreads[i] +
                    swp_f_df(today, longfloat_start_dates[i], yc_name) /
                        swp_f_df(today, longfloat_end_dates[i], yc_name) -
                    1);
    longFloatPVWithoutSpread +=
        swp_f_df(today, longfloat_pay_dates[i + 1], yc_name) *
        (swp_f_df(today, longfloat_start_dates[i], yc_name) /
             swp_f_df(today, longfloat_end_dates[i], yc_name) -
         1);
  }
  ...
  for (i = 1; i < longfix_nb_pay_dates; ++i) {
    ...
    longcpn[i] = longStrike * longfix_cvgs[i - 1] *
                 swp_f_df(today, longfix_pay_dates[i], yc_name);
  }
  longNcpn = longfix_nb_pay_dates;
  longcpn[longfix_nb_pay_dates - 1] =
      longcpn[longfix_nb_pay_dates - 1] +
      swp_f_df(today, longfix_pay_dates[longfix_nb_pay_dates - 1], yc_name);

  shortcpn[0] = -swp_f_df(today, shortfloat_pay_dates[0], yc_name);
  for (i = 1; i < shortfix_nb_pay_dates; ++i) {
    shortcpn[i] = shortStrike * shortfix_cvgs[i - 1] *
                  swp_f_df(today, shortfix_pay_dates[i], yc_name);
  }
  shortNcpn = shortfix_nb_pay_dates;
  shortcpn[shortNcpn - 1] =
      shortcpn[shortNcpn - 1] +
      swp_f_df(today, shortfix_pay_dates[shortfix_nb_pay_dates - 1], yc_name);
  ...
  err = swp_f_vol(vol_curve_name, startDate, longEndDate, ex_lfwd[0], &vol,
                  &power);
  if (err) {
    goto FREE_RETURN;
  }
  if (power > 0.5) {
    longMktPrice = srt_f_optblksch(longFwdSwapRate, longFwdSwapRate, vol,
                                   ex_time[0], ex_llvl[0], SRT_PUT, PREMIUM);
  } else {
    longMktPrice = srt_f_optblknrm(longFwdSwapRate, longFwdSwapRate, vol,
                                   ex_time[0], ex_llvl[0], SRT_PUT, PREMIUM);
  }
  ...
  err = swp_f_vol(vol_curve_name, startDate, shortEndDate, shortfwd, &vol,
                  &power);
  if (err) {
    goto FREE_RETURN;
  }
  if (power > 0.5) {
    shortMktPrice = srt_f_optblksch(shortfwd, shortfwd, vol, ex_time[0],
                                    shortLVL, SRT_PUT, PREMIUM);
  } else {
    shortMktPrice = srt_f_optblknrm(shortfwd, shortfwd, vol, ex_time[0],
                                    shortLVL, SRT_PUT, PREMIUM);
  }

  err = swp_f_ForwardRate(startDate, shortEndDate, swaption_freq,
  ...
  if ((shortEndDate - startDate) / 365.0 < 0.2) {
    err = lgm2FPriceShortSwaptionAndCalibrateLongSwaptionGivenLambda(
        longNcpn, shortNcpn, cpn_time, shortcpn, longcpn, ex_time, longstrike,
        longMktPrice, shortstrike, ex_zeta, *lambda, alpha, gamma, rho, *sig,
        &ShortModelPrice);
  } else {
    while ((AbsoluteError > CALPRES) && (compt < 20)) {
      ...
      err = lgm2FPriceShortSwaptionAndCalibrateLongSwaptionGivenLambda(
          longNcpn, shortNcpn, cpn_time, shortcpn, longcpn, ex_time, longstrike,
          longMktPrice, shortstrike, ex_zeta, *lambda + deltaLambda, alpha,
          gamma, rho, *sig, &ShortModelPriceDeltaLambda);
      if (fabs(ShortModelPriceDeltaLambda - shortMktPrice) <= CALPRES) {
        *lambda = *lambda + deltaLambda;
        break;
      }

      ShortModelPriceDerivative =
          (ShortModelPriceDeltaLambda - ShortModelPrice) / deltaLambda;

      *lambda = max(0.00001, *lambda + (shortMktPrice - ShortModelPrice) /
                                           ShortModelPriceDerivative);
...
Err VolBondCalibrationOld(
  ...
  while (i + j < longfix_nb_pay_dates + longfloat_nb_pay_dates) {
    if (longfix_pay_dates[i] < longfloat_pay_dates[j]) {
      ...
      longcpn[k] = longStrike * longfix_cvgs[i - 1] *
                   swp_f_df(today, longfix_pay_dates[i], yc_name);
      i++;
    } else {
      if (longfix_pay_dates[i] > longfloat_pay_dates[j]) {
        cpn_time[k] = (longfloat_pay_dates[j] - today) * YEARS_IN_DAY;
        longcpn[k] = -longfloat_spreads[j - 1] * longfloat_cvgs[j - 1] *
                     swp_f_df(today, longfloat_pay_dates[j], yc_name);
        j++;
      } else {
        cpn_time[k] = (longfix_pay_dates[i] - today) * YEARS_IN_DAY;
        longcpn[k] = (longStrike * longfix_cvgs[i - 1] -
                      longfloat_spreads[j - 1] * longfloat_cvgs[j - 1]) *
                     swp_f_df(today, longfix_pay_dates[i], yc_name);
        i++;
        j++;
      }
  ...
  while (i + j < shortfix_nb_pay_dates + shortfloat_nb_pay_dates) {
    if (shortfix_pay_dates[i] < shortfloat_pay_dates[j]) {
      shortcpn[k] = shortStrike * shortfix_cvgs[i - 1] *
                    swp_f_df(today, shortfix_pay_dates[i], yc_name);
      i++;
    } else {
      if (shortfix_pay_dates[i] > shortfloat_pay_dates[j]) {
        shortcpn[k] = -shortfloat_spreads[j - 1] * shortfloat_cvgs[j - 1] *
                      swp_f_df(today, shortfloat_pay_dates[j], yc_name);
        j++;
      } else {
        shortcpn[k] = (shortStrike * shortfix_cvgs[i - 1] -
                       shortfloat_spreads[j - 1] * shortfloat_cvgs[j - 1]) *
                      swp_f_df(today, longfix_pay_dates[i], yc_name);
        i++;
        j++;
      }
  ...
  if ((shortEndDate - startDate) / 365.0 < 0.2) {
    ...
    while ((AbsoluteError > CALPRES) && (compt < 20)) {
      ...
      *lambda = max(0.001, *lambda + (shortMktPrice - ShortModelPrice) /
                                         ShortModelPriceDerivative);

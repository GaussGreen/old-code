    ...
    double volInit)
{
    ...
    pr1 = lgmopval2FVolBond(ncpn, cpn, cpn_G1, cpn_G2, z1, z2, z12, ex_G1[0], ex_G2[0]);
    ...
    pr2 = lgmopval2FVolBond(ncpn, cpn, cpn_G1, cpn_G2, q1, q2, q12, ex_G1[0], ex_G2[0]);
    ...
    double* ShortPrice)  //	Short Swaption price as output
{
    ...
    *ShortPrice = lgmopval2FVolBond(
        Shortncpn,  //	Number of cash-flow dates, including
                    //	start and end date
        shortcpn,  //
        cpn_G,
        cpn_G2,
    ...
    double* sig)
{
    ...
    for (i = 1; i < longfix_nb_pay_dates; ++i)
    {
        ...
        longcpn[i] =
            longStrike * longfix_cvgs[i - 1] * swp_f_df(today, longfix_pay_dates[i], yc_name);
    }
    ...
    {
        shortcpn[i] =
            shortStrike * shortfix_cvgs[i - 1] * swp_f_df(today, shortfix_pay_dates[i], yc_name);
    }
    ...
    if (power > 0.5)
    {
        longMktPrice = srt_f_optblksch(
            longFwdSwapRate, longFwdSwapRate, vol, ex_time[0], ex_llvl[0], SRT_PUT, PREMIUM);
    }
    else
    {
    ...
    {
        shortMktPrice =
            srt_f_optblksch(shortfwd, shortfwd, vol, ex_time[0], shortLVL, SRT_PUT, PREMIUM);
    }
    else
    {
        shortMktPrice =
            srt_f_optblknrm(shortfwd, shortfwd, vol, ex_time[0], shortLVL, SRT_PUT, PREMIUM);
    }

    err = swp_f_ForwardRate(
        startDate, shortEndDate, swaption_freq, swaption_basis, yc_name, ref_rate_name, &swp_rte);
    if (err)
    {
        goto FREE_RETURN;
    }
    ...
    else
    {
        while ((AbsoluteError > CALPRES) && (compt < 20))
        {
            ...
            *lambda = max(
                0.00001, *lambda + (shortMktPrice - ShortModelPrice) / ShortModelPriceDerivative);
    ...
    double* sig)
{
    ...
    longNcpn              = k;
    longcpn[longNcpn - 1] = longcpn[longNcpn - 1] +
                            swp_f_df(today, longfix_pay_dates[longfix_nb_pay_dates - 1], yc_name);
    ...
    else
    {
        while ((AbsoluteError > CALPRES) && (compt < 20))
        {
            ...
            *lambda =
                max(0.001, *lambda + (shortMktPrice - ShortModelPrice) / ShortModelPriceDerivative);

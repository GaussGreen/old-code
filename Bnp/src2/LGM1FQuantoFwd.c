								...
								double forfxrho)
{
	...
	if(mattime <= sigtime[0])
	{
		...
		I2_b = I2_b * exp(-forlam*starttime) * exp(-forlam*endtime) * (1-exp(-forlam*coverage))/(forlam*forlam);
		I2 = I2_a - I2_b;
		
		I3_a = I3_a * exp(-forlam*starttime) * (1-exp(-forlam*coverage))/(forlam*domlam);
		I3_b = I3_b * exp(-forlam*starttime) * exp(-domlam*endtime) * (1-exp(-forlam*coverage))/(forlam*domlam);
		I3 = - I3_a + I3_b;
	...
	else
	{
		...
		{
			I1 += forfxrho*fxsig[i+1]*forsig[i+1]*Ji0_func(forlam, sigtime[i], sigtime[i+1]);
		
			I2_a += forsig[i+1]*forsig[i+1]*Ji0_func(forlam, sigtime[i], sigtime[i+1]);
			I2_b += forsig[i+1]*forsig[i+1]*Ji0_func(2*forlam, sigtime[i], sigtime[i+1]);

			I3_a += domforrho*forsig[i+1]*domsig[i+1]*Ji0_func(forlam, sigtime[i], sigtime[i+1]);
			I3_b += domforrho*forsig[i+1]*domsig[i+1]*Ji0_func(domlam+forlam, sigtime[i], sigtime[i+1]);
		...
		if(i==0)
		{
			...
			I3_b += domforrho*forsig[1]*domsig[1]*Ji0_func(domlam+forlam, sigtime[0], mattime);
		...
		else
		{
			...
			I3_a += domforrho*forsig[i+1]*domsig[i+1]*Ji0_func(forlam, sigtime[i], mattime);
			I3_b += domforrho*forsig[i+1]*domsig[i+1]*Ji0_func(domlam+forlam, sigtime[i], mattime);
...
Err LGM1FQuantoFwd(char *und3dfx,
				   long start,
				   long end,
				   char *freq,
				   char *basis,
...
//---------------------------------------------------------------------------
	...
	for(i=0;i<float_nb_dates;++i)
	{
		...
		(*QuantoFwd)[i] = (QAdj * swp_f_df(today, float_start_dates[i], for_yc) / swp_f_df(today, float_end_dates[i], for_yc) - 1.0)/float_cvgs[i] +  float_spreads[i];

...
Err fwd_treas_rate(Ddate start, SrtCurvePtr tc_crv, double *fwd_T_rate) {
  ...
  if (use_spread_interpolation == SRT_YES) {
    ...
    err = swp_f_ForwardRate_SwapDP(&swapdp, yc_name, cmt->swap_ref_rate,
                                   &swap_rate);
    if (err)
      return err;
  }
  ...
  if (num_rate == 1) {
    ...
    i = 0;
    while ((DTOL(FwdT_field_Dget(tc_obj, FwdT_DATE, i)) < DTOL(start)) &&
           (i < num_rate))
      i++;
    ...
    if (i == num_rate) {
      ...
      time_low =
          (FwdT_field_Dget(tc_obj, FwdT_DATE, i1) - today) * YEARS_IN_DAY;
      time_high =
          (FwdT_field_Dget(tc_obj, FwdT_DATE, i2) - today) * YEARS_IN_DAY;
      ...
      if (interp_method == LIN_R) {
        if (use_spread_interpolation == SRT_YES) {
          spread = spread_low + (spread_high - spread_low) *
                                    (time_start - time_low) /
                                    (time_high - time_low);
        ...
        if (use_spread_interpolation == SRT_YES) {
          spread = spread_low * time_low +
                   ((spread_high * time_high - spread_low * time_low) *

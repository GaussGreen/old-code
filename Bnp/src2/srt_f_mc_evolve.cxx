...
Err monte_CHE_1f_Euler_evolve(double *rndm_vec, SrtStpPtr top, SrtSample *sam,
                              int und_index) {
  long i;
  double drift;
  double variance;
  ...
  if (step != NULL) {
    ...
    sam_get(sam[0], und_index, SHORT_RATE) =
        sam_get(tminf->fwd_sam, und_index, SHORT_RATE);
    ...
    while (step->next != NULL) {
      ...
      sam_get(sam[i + 1], und_index, PHI) =
          sam_get(sam[i], und_index, PHI) *
              (1 - 2 * tminf->ev.onef.lambda * step->delta_t) +
          sam_get(sam[i], und_index, SHORT_RATE) *
              sam_get(sam[i], und_index, SHORT_RATE) * tminf->ev.onef.sig2 *
              step->delta_t;
      ...
      if (und_index ==
          sam[i + 1].numeraire_index) /*i.e. if Domestic Underlying */
      {
        sam[i + 1].numeraire =
            exp(sam_get(sam[i], und_index, STATEVAR) * step->delta_t) *
...
/* function " monte_CHE_1f_Milshtein_evolve " describes the temporal evolution
   of the short rate "sam[i].short_rate" using STD. GAUSSIAN random numbers
   ...
   *rndm_vec : pointer to VECTOR containing sample path of
                Monte Carlo random vars        , each row corresponds to one
   path. *step : pointer to linked list of structures which contains all
   preprocessed step (which is path-independent) utilized in the evolution
   equations. *sam : pointer to VECTOR of structure which the function fills
   with sample path of the short_rate        , v_square(called phi)        , and
   the money_market account corresponding to THIS path of the short_rate.


*/

Err monte_CHE_1f_Milshtein_evolve(double *rndm_vec, SrtStpPtr top,
                                  SrtSample *sam, int und_index) {
  long i;
  double drift;
  double variance;
  SrtStpPtr step;
    ...
    while (step->next != NULL) {
      ...
      drift = (sam_get(sam[i], und_index, PHI) -
               ...
               tminf->quanto_adjustment * tminf->ev.onef.sig *
                   sam_get(sam[i], und_index, SHORT_RATE)) *
                  step->delta_t +
              0.5 * tminf->ev.onef.sig2 *
                  sam_get(sam[i], und_index, SHORT_RATE) * step->delta_t *
                  (rndm_vec[i + 1] * rndm_vec[i + 1] - 1);
      ...
      /* The step to get 1/rolling money-market account        , product in
       * (A.31)*/
...
Err monte_CHE_2f_Euler_evolve(double **rndm_mat, SrtStpPtr top, SrtSample *sam,
                              int und_index) {
  long i;
  ...
  if (step != NULL) {
    ...
    /* Initialization */
    /* Please note that all along this function        , r(t) = f(0        ,t) +
     * X1 + X2 */
    ...
    while (step->next != NULL) {
      ...
      sam_get(sam[i + 1], und_index, PHI1) =
          ...
          sam_get(sam[i], und_index, SHORT_RATE) *
              sam_get(sam[i], und_index, SHORT_RATE) * tminf->ev.twof[0].sig2 *
              step->delta_t;
      ...
      sam_get(sam[i + 1], und_index, PHI2) =
          ...
          sam_get(sam[i], und_index, SHORT_RATE) *
              sam_get(sam[i], und_index, SHORT_RATE) * tminf->ev.twof[1].sig2 *
              step->delta_t;

      sam_get(sam[i + 1], und_index, CROSSPHI) =
          sam_get(sam[i], und_index, CROSSPHI) *
              (1 - (tminf->ev.twof[0].lambda + tminf->ev.twof[1].lambda) *
                       step->delta_t) +
          sam_get(sam[i], und_index, SHORT_RATE) *
              sam_get(sam[i], und_index, SHORT_RATE) * tminf->ev.twof[0].sig *
              tminf->ev.twof[1].sig * tminf->correl_x * step->delta_t;
          ...
          sam[i + 1].numeraire_index) /*i.e. if Domestic Underlying */
      {
        sam[i + 1].numeraire =
            sam[i].numeraire *
            exp(step->delta_t * (sam_get(sam[i], und_index, X1) +
                                 sam_get(sam[i], und_index, X2)));
        /* pv_money_mkt */
      }
      i++;
...
Err monte_LGM_1f_Jumping_evolve(double *rndm_vec, SrtStpPtr top, SrtSample *sam,
                                int und_index) {
  long i;
  double yld;
  SrtStpPtr step;
  ...
  if (step != NULL) {
    ...
    while (step->next != NULL) {
      ...
      {
        sam_get(sam[i + 1], und_index, STATEVAR) =
            sam_get(sam[i], und_index, STATEVAR) * nxttminf->ev.onef.F /
                tminf->ev.onef.F +
      ...
      if (und_index == sam[i + 1].numeraire_index) {
        ...
        sam[i + 1].numeraire =
            sam[i].numeraire *
            exp(yld -
                sam_get(tminf->fwd_sam, und_index, F_0_t) * step->delta_t);
      }
...
Err monte_vasicek_1f_euler_evolve(double *rndm_vec, SrtStpPtr top,
                                  SrtSample *sam, int und_index) {
  SrtStpPtr step;
  SrtIRMTmInf *tminf, *nxttminf;
  long i;
  ...
  if (step != NULL) {
    ...
    while (step->next != NULL) {
      ...
      sam_get(sam[i + 1], und_index, SHORT_RATE) =
          sam_get(sam[i], und_index, SHORT_RATE) -
          tminf->ev.onef.lambda * sam_get(sam[i], und_index, SHORT_RATE) *
              step->delta_t +
...
Err monte_LGM_1f_Euler_evolve(double *rndm_vec, SrtStpPtr top, SrtSample *sam,
                              int und_index) {
  long i;
  SrtStpPtr step;
  SrtIRMTmInf *tminf, *nxttminf;
  ...
  if (step != NULL) {
    ...
    while (step->next != NULL) {
      ...
      sam_get(sam[i + 1], und_index, STATEVAR) =
          sam_get(sam[i], und_index, STATEVAR) + nxttminf->rf.onef.H -
          tminf->rf.onef.H -
      ...
      {
        sam[i + 1].numeraire =
            sam[i].numeraire * exp(sam_get(sam[i], und_index, STATEVAR) *
...
Err monte_NEWLGM_1f_Euler_evolve(double *rndm_vec, SrtStpPtr top,
                                 SrtSample *sam, int und_index) {
  long i;
  SrtStpPtr step;
  SrtIRMTmInf *tminf, *nxttminf;
  ...
  if (step != NULL) {
    ...
    sam_get(sam[0], und_index, STATEVAR) = 0.0;
    sam_get(sam[0], und_index, SHORT_RATE) =
        sam_get(sam[0], und_index, STATEVAR);
    sam_get(sam[0], und_index, PHI) = 0.0;
    ...
    while (step->next != NULL) {
      ...
      sam_get(sam[i + 1], und_index, STATEVAR) =
          ...
          tminf->quanto_adjustment * vol_sqdt + vol_sqdt * rndm_vec[i + 1];
      ...
      product in (A.31)  (using renormalisation techniques)*/
      {
        sam[i + 1].numeraire = exp(
            -sam_get(sam[i + 1], und_index, STATEVAR) * nxttminf->rf.onef.H +
...
Err monte_NEWCHEYBETA_1f_Euler_evolve(double *rndm_vec, SrtStpPtr top,
                                      SrtSample *sam, int und_index) {
  long i;
  SrtStpPtr step;
  SrtIRMTmInf *tminf, *nxttminf;
  ...
  if (step != NULL) {
    ...
    while (step->next != NULL) {
      ...
      vol_sqdt = sqrt(nxttminf->rf.onef.G - tminf->rf.onef.G) *
                 pow(fabs(sam_get(sam[i], und_index, SHORT_RATE)),
                     tminf->ev.onef.beta);

      /*	* pow(fabs(sam_get(sam[i]        ,und_index        ,STATEVAR))
       *       ,tminf->ev.onef.beta);*/

      sam_get(sam[i + 1], und_index, STATEVAR) =
          temp_var + vol_sqdt * rndm_vec[i + 1];
      ...
      sam_get(sam[i + 1], und_index, SHORT_RATE) =
          ...
          nxttminf->ev.onef.F *
              (temp_var -
               nxttminf->rf.onef.H * sam_get(sam[i + 1], und_index, PHI));
...
Err monte_LGM_2f_evolve(double **rndm_mat, SrtStpPtr top, SrtSample *sam,
                        int und_index) {
  long i;
  double alpha, beta;
  double g[2]; /*Correlated Random numbers */
  ...
  if (step != NULL) {
    ...
    while (step->next != NULL) {
      ...
      sam_get(sam[i + 1], und_index, X1) =
          sam_get(sam[i], und_index, X1) * nxttminf->ev.twof[0].F /
              tminf->ev.twof[0].F +
      ...
      sam_get(sam[i + 1], und_index, X2) =
          sam_get(sam[i], und_index, X2) * nxttminf->ev.twof[1].F /
              tminf->ev.twof[1].F +
      ...
      */
      ...
      /* Here        , the Numeraire is the Jumping Numeraire : multiply current
       * by B(Ti        ,Ti+1) | F(Ti) */
      ...
      if (und_index ==
          sam[i + 1].numeraire_index) /* i.e. if domestic underlying */
      {
        /* Here again        , do the Numeraire calculation taking the
         * Renormalisation into account */
        sam[i + 1].numeraire =
            exp(yld -
                sam_get(tminf->fwd_sam, und_index, F_0_t) * step->delta_t) *
            sam[i].numeraire; /* pv_money_mkt */
...
Err monte_BETAETA_1f_evolve(double *rndm_vec, SrtStpPtr top, SrtSample *sam,
                            int und_index) {
  long i;
  SrtStpPtr step;
  double stddev;
  ...
  if (step != NULL) {
    ...
    while (step->next != NULL) {
      ...
      stddev = tminf->ev.onef.sig *
               exp(log(fabs(1.0 + tminf->the_beta *
                                      sam_get(sam[i], und_index, STATEVAR))) *
                   tminf->eta) *
               step->sqrt_delta_t;

      /*
                              if   (1.0 + tminf->beta * sam_get(sam[i]
               ,und_index        ,STATEVAR) < 0.) sprintf(msg        ,"1+beta*x
         has gone too small %f"        , (1.0 + tminf->beta * sam_get(sam[i]
         ,und_index        ,STATEVAR)));
               ...
               ,und_index        ,STATEVAR) >= 0.) sprintf(msg        ,"1+beta*x
         is ok %f"        , (1.0
         + tminf->beta * sam_get(sam[i]        ,und_index        ,STATEVAR)));
      ...
      */
          ...
          sam[i + 1].numeraire_index) /* i.e. if Domestic Underlying */
      {
        ...
        /*	h_t_x = x_power_fct( sam_get(sam[i+1]        ,und_index
           ,STATEVAR)* nxttminf->h_t_power        , nxttminf->x_power );
        ...
        */
        ...
        /*	sam[i+1].numeraire = exp ( nxttminf->A_t +
                        nxttminf->lambda_t * sam_get(sam[i+1]        ,und_index
                 ,STATEVAR)) ; */
...
Err monte_LGM_1f_STOCHVOL_evolve(double **rndm_mat, SrtStpPtr top,
                                 SrtSample *sam, double *Pt_by_Zt,
                                 int und_index) {
  long i;
  SrtStpPtr step;
  SrtIRMTmInf *tminf, *nxttminf;
  ...
  if (step != NULL) {
    ...
    while (step->next != NULL) {
      ...
      sam_get(sam[i + 1], und_index, PHI) =
          sam_get(sam[i], und_index, PHI) *
              (1 - 2 * tminf->ev.onef.lambda * step->delta_t) +
          sam_get(sam[i], und_index, SIGMA) *
              sam_get(sam[i], und_index, SIGMA) * step->delta_t;

      /* The finite difference step to get the next vol */
      sam_get(sam[i + 1], und_index, SIGMA) =
          sam_get(sam[i], und_index, SIGMA) * nxttminf->ev.onef.sig /
          tminf->ev.onef.sig *
      ...
      sam_get(sam[i + 1], und_index, SIGMA) =
          DMIN(sam_get(sam[i + 1], und_index, SIGMA),
...
Err monte_CHE_1f_STOCHVOL_evolve(double **rndm_mat, SrtStpPtr top,
                                 SrtSample *sam, double *Pt_by_Zt,
                                 int und_index) {
  long i;
  SrtStpPtr step;
  ...
  if (step != NULL) {
    ...
    while (step->next != NULL) {
      ...
      variance = sam_get(sam[i], und_index, SIGMA) *
                 sam_get(sam[i], und_index, SHORT_RATE) * step->sqrt_delta_t *
                 g[0];
...
/*----------------------------------------------------------------------------*/
Err monte_CHEYBETA_1f_STOCHVOL_evolve(double **rndm_mat, SrtStpPtr top,
                                      SrtSample *sam, double *Pt_by_Zt,
                                      int und_index) {
  long i;
  SrtStpPtr step;
  ...
  if (step != NULL) {
    ...
    while (step->next != NULL) {
      ...
      phi = phi + volsqdt * volsqdt -
            2 * tminf->ev.onef.lambda * phi * step->delta_t;
      ...
      volvar *= (nxttminf->ev.onef.sig / tminf->ev.onef.sig) *
                exp(-tminf->vovol_sqr * step->delta_t +
                    tminf->vovol * g[1] * step->sqrt_delta_t);

      /* AA - Volatility is bound in order to avoid getting explosive rates*/
      /*	sam_get(sam[i+1]        ,und_index        ,SIGMA) =
                      DMIN(sam_get(sam[i+1]        ,und_index        ,SIGMA)
               ,nxttminf->ev.onef.sig*
...
Err monte_BLACKSCHOLES_evolve(double *rndm_vec, SrtStpPtr top, SrtSample *sam,
                              int und_index) {
  long i;
  SrtStpPtr step;
  ...
  if (step != NULL) {
    ...
    while (step->next != NULL) {
      ...
      sam_get(sam[i + 1], und_index, SPOT) =
          sam_get(sam[i], und_index, SPOT) * nxttminf->init_fwd_val /
          tminf->init_fwd_val *
          exp((-tminf->int_sig2_dt / 2) +
              (rndm_vec[i + 1] + tminf->quanto_adjustment) *
                  tminf->sqrt_int_sig2_dt);
...
Err monte_NORMALBS_evolve(double *rndm_vec, SrtStpPtr top, SrtSample *sam,
                          int und_index) {
  long i;
  SrtStpPtr step;
  SrtLogTmInf *tminf, *nxttminf;
...
/* ----------------------------------------------------------------------------
 */
Err monte_EQ_STOCH_RATES_evolve(double *rndm_vec, SrtStpPtr top, SrtSample *sam,
                                int und_index, int dom_index) {
  long i;
  SrtStpPtr step;
  SrtLogTmInf *tminf, *nxttminf;
    ...
    while (step->next != NULL) {
      ...
      sam_get(sam[i + 1], und_index, SPOT) =
          ...
          exp(+(sam_get(sam[i], dom_index, STATEVAR)) * step->delta_t -
              (0.5 * tminf->int_sig2_dt) +
              (rndm_vec[i + 1]) * tminf->sqrt_int_sig2_dt);
      /* Sets pv_money_mkt if underlying is domestic */
      if (und_index == sam[i + 1].numeraire_index) {
        sam[i + 1].numeraire = 1.0;
      }
...
Err monte_EQ_STOCH_RATES_SRVGS(double *rndm_vec_spot, double *rndm_vec_vol,
                               SrtStpPtr top, SrtSample *sam, int und_index,
                               int dom_index) {
  long i;
  SrtStpPtr step;
  SrtLogTmInf *tminf, *nxttminf;
  ...
  if (step != NULL) {
    ...
    while (step->next != NULL) {
      ...
      stdev = log(sam_get(sam[i], und_index, SPOT) / spot) /
              (basevol * sqrt(nxt_time));
      stdev = omega * tanh(stdev / omega);

      /* compute the new SIGMA */
      sam_get(sam[i + 1], und_index, SIGMA) =
          sam_get(sam[i], und_index, SIGMA) +
          (nxttminf->basevol - tminf->basevol) + voldrift * step->delta_t +
          vovol * g[1] * step->sqrt_delta_t;

      sqrt_int_sig2_dt = sam_get(sam[i + 1], und_index, SIGMA) *
                         (1 + beta * stdev + gamma * stdev * stdev) *
                         sqrt(step->delta_t);
      ...
      sam_get(sam[i + 1], und_index, SPOT) =
          ...
          tminf->init_fwd_val *
          exp(+(sam_get(sam[i], dom_index, STATEVAR)) * step->delta_t -
              0.5 * int_sig2_dt + g[0] * sqrt_int_sig2_dt);
...
Err monte_FX_STOCH_RATES_evolve(double *rndm_vec, SrtStpPtr top, SrtSample *sam,
                                int und_index, int dom_index, int for_index) {
  long i;
  SrtStpPtr step;
  SrtLogTmInf *tminf;
...
Err monte_FX_STOCH_RATES_Jumping_evolve(double *rndm_vect, SrtStpPtr top,
                                        SrtSample *sam, int und_index,
                                        int dom_index, int for_index)

{
  ...
  //	fp0 = fopen("D:\\WORKAREA\\DEBUG\\fx_stoch_rates_debug_rdm_vect.txt" ,
  //"w"); 	fp1 =
  // fopen("D:\\WORKAREA\\DEBUG\\fx_stoch_rates_debug_dom_state_var.txt" , "w");
  //	fp2 =
  // fopen("D:\\WORKAREA\\DEBUG\\fx_stoch_rates_debug_for_state_var.txt" , "w");
  ...
  if (step != NULL) {
    ...
    while (step->next != NULL) {
      ...
      //			fprintf(fp0        ,"%.16f\n" ,rndm_vect[i+1]);
      //			fprintf(fp1        ,"%.16f\n" ,sam_get(sam[i+1]
      //      ,dom_index        ,STATEVAR)); 			fprintf(fp2        ,"%.16f\n"
...
/* The two following functions simulate General Cheyette evolutions        , in
a one-factor world (function 1) and two-factor world (function). They use the
local volatility functions defined in srt_f_ts_init.cxx and srt_f_ts_init_2f.cxx
, which return the local volatility as a function of t and r        , according
to the model. */

Err monte_CHEYBETA_1f_Euler_evolve(double *rndm_vec, SrtStpPtr top,
                                   SrtSample *sam, int und_index) {
  long i;
  SrtStpPtr step;
  SrtIRMTmInf *tminf, *nxttminf;
  ...
  if (step != NULL) {
    ...
    while (step->next != NULL) {
      ...
      phi = phi + volsqdt * volsqdt -
            2 * phi * tminf->ev.onef.lambda * step->delta_t;
...
Err monte_CHEYBETA_2f_Euler_evolve(double **rndm_mat, SrtStpPtr top,
                                   SrtSample *sam, int und_index) {
  long i;
  SrtStpPtr step;
  SrtIRMTmInf *tminf;
  ...
  if (step != NULL) {
    ...
    while (step->next != NULL) {
      ...
      err = srt_f_CheyBeta2f_evolve_sample(step, &sam[i], und_index,
                                           rndm_mat[0][i + 1],
                                           rndm_mat[1][i + 1], &sam[i + 1]);

      /* Update the Numeraire (the Bank account unit)(minus the forward rate...)
       */
      if (und_index == sam[i + 1].numeraire_index) {
        sam[i + 1].numeraire = exp((sam_get(sam[i], und_index, X1) +
                                    sam_get(sam[i], und_index, X2)) *
                                   step->delta_t) *
                               sam[i].numeraire;
      }
...
Err monte_MIXEDBETA_2f_Euler_evolve(double **rndm_mat, SrtStpPtr top,
                                    SrtSample *sam, int und_index) {
  long i;
  SrtStpPtr step;
  SrtIRMTmInf *tminf;

/* ==============================================================================
   ...
   FILE NAME:      srt_f_cheybeta_fwd_pde.cxx
...
void srt_f_chey_beta_fwd_pde_alloc_temp(CHEYBETA_FWD_PDE_TEMP *tmp, int nx,
                                        int nphi) {
...
void srt_f_chey_beta_fwd_pde_first_step(
  ...
  nvar = chey_beta_mdl_norm_var_at_t(&p, 0.0, 0.0, grid->fwdrt[0], DBL_MAX,
                                     1.0e-16);

  std = sqrt(chey_beta_mdl_var(mdl, &p, 0.0, 0.0, nvar));
...
/*	Go forward one step	*/
void srt_f_chey_beta_fwd_pde_one_step(CNPDE_TEMP *tmppde,
  ...
  for (phi_slice = 0; phi_slice < grid->nphi[t1]; phi_slice++) {
    ...
    num_f_pde_one_step_forward(tmppde, grid->nx, grid->x, grid->adt1[phi_slice],
                               tmpcb->mu, tmpcb->var, tmpcb->r, 0.55,
                               tmpcb->ad);
...
/*	Go forward n steps	*/
void srt_f_chey_beta_fwd_pde_n_steps(CHEYBETA_FWD_PDE_TEMP *tmpcb,
                                     /*	Model	*/
                                     CHEYBETA_MDL *mdl,
                                     /*	Grid	*/
                                     CHEYBETA_GRID *grid,
  ...
  for (i = 0; i < n; i++) {
    srt_f_chey_beta_fwd_pde_one_step(&tmppde, tmpcb, mdl, grid, *ti + i,
                                     *ti + i + 1);
  }

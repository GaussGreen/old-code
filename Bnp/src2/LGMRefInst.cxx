...
LGMErr LGMMakeRefDeals(
    ...
    Date *TauArr,   /* [*        ,1        ,2        ,...        ,nEx] ARRAY OF
                     * EXERCISE DATES AFTER TODAY
                     */
    double *RatArr, /* [*        ,1        ,2        ,...        ,nEx] RATIO OF
                 PV OF FIXED LEG TO PV OF STRIKE */
    ...
    LGMCalSet **RefDealsPtrPtr)  /* REFERENCE INSTRUMENTS (OUTPUT)*/
{
  ...
  *RefDealsPtrPtr = NULL;
  ...
  nEx = lasti - firsti + 1;
  tExArr = &(TauArr[firsti - 1]); /* tExArr[*        ,1        ,...        ,nEx]
                                     are relevent dates */
  FVArr = &(RatArr[firsti - 1]);  /* FVArr[*        ,1        ,...        ,nEx]
                                     are relevent fwd values */

  /* Create reference instruments */
  /* Set tNow        , n        , nPay        , the long swaption pay dates tPay
     , the coverages cvgpay        , and the dis factors Dpay from tNow to the
     pay dates. Set nEx        , and common exercise dates (tEx)        , start
     dates (tStart)        , and df to start date (DStart) */
...
/* First construct the long fixed leg tPay[]        , which runs from the last
paydate tLast = tPay[n] to the spot of the first exercise date        , tPay[0].
For simplicity        , also extend the pay dates to either tPay[n+1] (for
annual compounding) or tPay[n+2] (for semiannual compounding) to ensure we can
fit in a minimum length swap whose start date is before tPay[n]. Construct the
fixed leg pay dates tPay[i]        , i = 1        ,...        ,n. the discount
factor Dpay[i] from tNow to the pay date the coverage cvgpay[i] between
tPay[i-1] and tPay[i]. If ResExer = 1        , then set the common exercise
dates to be tEx[i]        , and set the start dates tStart[i] to be spot of
tEx[i]. If use starts=0        , then let the start dates be tPay[i]        ,
i=1 to n-1        , and set tEx[i] to be lag business days earlier. In either
case set DStart[i] to be the discount factor from tNow to tStart.
*/
static LGMErr
RefDealsCommon(Date tNow,
               long nEx, /* evaluation date        , number of exercises */
...
cvgfirst[j] like the long swaption        , and the coverages for the period
between tPay[i-1] and tPay[i] is cvgpay[i] for i>ifirst[j]. The only new
variables we need are nshort[j]. */
/* Note: RefDealslong must be called first to set n        , nPay        ,
 * tPay[]        , ifirst[] */
...
the same exercise date        , where the ratios are stored in FVarr for dates
TauArr for n=1        ,...        ,nArr. By definition        , the ratio is 1
at date tLast (last pay date of the original deal) Use GetVol and GetBeta to
ensure that the strike is within two std deviations of par */
static LGMErr FindFixRateRefInst(
                                            ...
                                            structure (NULL => not req'd) */
{
  ...
  if (CSPtr->shortflag != 0) {
    ...
    for (j = 1; j <= nEx; j++) {
      ...
      CSPtr->Vshort[j] = LGMCEVSwaptionPrice(
          ...
          CSPtr->cvgfirst[j],
          Dpay[i], /* stub cvg        , dis fac to stub paydate */
  ...
  if (CSPtr->capflag != 0) {
    ...
    for (j = 1; j <= nEx; j++) {
      ...
      CSPtr->Vcap[j] = LGMCEVCapletGreek(recpay        ,SwapRate
      ,CSPtr->Rfcap[j] ,SwapLevel        ,Yrtoexp        ,CEVvol        ,beta
      ,PREMIUM); if (CSPtr->Vcap[j] < 0.) return("can't get cap price");
      CSPtr->VegaCap[j] = LGMCEVCapletGreek(recpay        ,SwapRate
      ,CSPtr->Rfcap[j]        ,SwapLevel       ,Yrtoexp ,CEVvol        ,beta
      ,VEGA);
...
double LGMCEVSwaptionPrice(
    ...
    double
        *Dpay, /* [0        ,...        ,n-1] dis factor to rest of pay dates */

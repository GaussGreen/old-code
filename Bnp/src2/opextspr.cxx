...
/*************************************************************
        ...
        integration of aX+a' - min(bY+b'        ,K) with aX+a' - min(bY+b' ,K) >
0 where X and Y are correlated log normal assets and K is strike

Essential functionality: For any a        ,b > 0        , K > 0        , and any
a'        ,b':

Writing x=rho*y+ksi*z        , and integrating a first time on z        , we
get:
...
static double int_in(double var) {
  ...
  } else /* d1 and d2 are order 1    */
  {
    ...
    integrd =
        exp(-0.5 * var * var) * bs; /* Here        , we forget the sqrt(2*PI)*/
  }
...
double srt_f_optextspr(double fwdx, double fwdy, double sigx, double sigy,
  ...
  glimithigh = (NI_qcoef > NI_scoef) ? (GAUSS_LIMIT + NI_qcoef)
                                     : (GAUSS_LIMIT + NI_scoef);
  ...
  case DELTAX:
    shift = fwdx / 10000;
    answer = (srt_f_optextspr(fwdx + shift, fwdy, sigx, sigy, rho, disc, a, ap,
                              b, bp, k, t, min_max, call_put, PREMIUM) -
              result) /
             shift;
    return (answer);
    break;
  ...
  case DELTAY:
    shift = fwdy / 10000;
    answer = (srt_f_optextspr(fwdx, fwdy + shift, sigx, sigy, rho, disc, a, ap,
                              b, bp, k, t, min_max, call_put, PREMIUM) -
              result) /
             shift;
    return (answer);
  ...
  case GAMMAX:
    shift = fwdx / 1000;
    answer = srt_f_optextspr(fwdx + shift, fwdx, sigx, sigy, rho, disc, a, ap,
                             b, bp, k, t, min_max, call_put, PREMIUM);
    answer += srt_f_optextspr(fwdx - shift, fwdy, sigx, sigy, rho, disc, a, ap,
                              b, bp, k, t, min_max, call_put, PREMIUM);
    answer -= 2 * result;
  ...
  case GAMMAXY:
    ...
    shifty = fwdy / 1000;
    answer =
        srt_f_optextspr(fwdx + shiftx, fwdy + shifty, sigx, sigy, rho, disc, a,
                        ap, b, bp, k, t, min_max, call_put, PREMIUM);
    answer +=
        srt_f_optextspr(fwdx - shiftx, fwdy - shifty, sigx, sigy, rho, disc, a,
                        ap, b, bp, k, t, min_max, call_put, PREMIUM);
    answer -=
  ...
  case VEGAX:
    shift = GVOPT.vol_add;
    answer = (srt_f_optextspr(fwdx, fwdy, sigx + shift, sigy, rho, disc, a, ap,
                              b, bp, k, t, min_max, call_put, PREMIUM) -
              result) /
             shift;
    return (answer);
  ...
  case VEGAY:
    shift = GVOPT.vol_add;
    answer = (srt_f_optextspr(fwdx, fwdy, sigx, sigy + shift, rho, disc, a, ap,
                              b, bp, k, t, min_max, call_put, PREMIUM) -
              result) /
             shift;
    return (answer);
  ...
  case THETA:
    shift = YEARS_IN_DAY;
    answer = srt_f_optextspr(fwdx, fwdy, sigx, sigy, rho,
                             disc * exp(-shift * log(disc) / t), a, ap, b, bp,
                             k, t - shift, min_max, call_put, PREMIUM) -
             result;
    return (answer);
    break;

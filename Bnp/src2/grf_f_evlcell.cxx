...
void grf_f_evlcllfct(double **cell, int c, int r1, int r2,
                     int ind, /* used for ROWSORT        , COLSORT  */
                     COMLLType fun_type, double *answer) {
  double x;
  int i;
  double *tmp_row;
  double *tmp_col;
  ...
  case COMLL_C_ROWMAXIDX: /* added by C. Godart for Regis Benichou */
    tmp_row =
        (double *)malloc(sizeof(double)); /* it's there        , so use it */
    *tmp_row = CELL(r1, c);
    x = c;
...
/*  Approximate y = f(x) by linear interpolation on a row ( only for
 * COMLL_C_ROWINTERP) */
void grf_f_evlcllfct_interp(double *x_values, double **cell, int r, int c1,
                            int c2, double x, COMLLType type, double *answer) {
  int i;
  double *tmp_row;
  double y;
  int n;

...
static void prod_comp(double re1, double im1, double re2, double im2,
                      double *re3, double *im3) {
  *re3 = re1 * re2 - im1 * im2;
  *im3 = re1 * im2 + im1 * re2;
}

static void div_comp(double re1, double im1, double re2, double im2,
                     double *re3, double *im3) {
  static double div;
...
static void sqr_comp(double re1, double im1, double *re3, double *im3) {
  static double norm;
...
void LGMSVSaveTFDensity(/* Inputs */
  ...
  for (iNumPhi = 1; iNumPhi <= iNbPhi; iNumPhi++) {
    for (iNumft = 1; iNumft < iNbft; iNumft = iNumft + 2) {
      fprintf(fid, " %g	",
              sqrt(Density[1][iNumPhi][iNumft] * Density[1][iNumPhi][iNumft] +
                   Density[1][iNumPhi][iNumft + 1] *
                       Density[1][iNumPhi][iNumft + 1]));
    }
...
/* allocate a double 3tensor with range t[nrl..nrh][ncl..nch][ndl..ndh] */
{
  ...
  if (!t)
    nrerror("allocation failure 1 in LGMSVSolFunc3tensor()");
  t += NR_END;
  t -= nrl;

  /* allocate pointers to rows and set pointers to them */
  t[nrl] = (LGMSVSolFunc1 **)srt_calloc(nrow * ncol + NR_END,
                                        sizeof(LGMSVSolFunc1 *));
  if (!t[nrl]) {
  ...
  t[nrl][ncl] = (LGMSVSolFunc1 *)srt_calloc(nrow * ncol * ndep + NR_END,
                                            sizeof(LGMSVSolFunc1));
  if (!t[nrl][ncl]) {
...
void LGMSVUpdateMomentValue(/* Input */
                            LGMSVSolFunc1 ***MomentFunc, double dDt,
                            int iNbCumulant, long ***IsUsefullMoment,
    ...
    for (j = 0; j <= iNbCumulant; j++)
      for (k = 0; k <= iNbCumulant; k++)
        if (IsUsefullMoment[i][j][k] = LGMSV_TRUE)
          MomentValue[i][j][k] =
              LGMSVFuncValue1(&MomentFunc[i][j][k], dDt, lambdaArray, 0);
}
...
void LGMSVFillDescription(/* Input */
                          ...
                          double dLambdaX, double dAlpha, double dLambdaEps,
                          double dRho,

                          long ***IsUsefullMoment, double ***MomentValue,
                          double dSigt,
    ...
    for (j = 0; j <= iNbCumulant; j++)
      for (k = 0; k <= iNbCumulant; k++) {
        if (IsUsefullMoment[i][j][k] = LGMSV_TRUE) {
          ...
          MomentFunc[i][j][k].dLambda =
              (2.0 * i + j) * dLambdaX + k * dLambdaEps;
          ...
          if (k > 0) {
            ...
            MomentFunc[i][j][k].a[2] =
                k * (dLambdaEps + (k - 1.0) / 2.0 * dAlpha2);
...
void LGMSVMomentCalculation(
  ...
  iNumTime = 0;
  while ((iNumTime < iNbSigTime - 1) &&
         (SigTime[iNumTime + 1] < (dTExercice + 1.0E-10)))
    iNumTime++;
  ...
  for (iNumTime = 0; iNumTime <= iIndExercice; iNumTime++) {
    ...
    if (iNumTime > 0) {
      /* Update of the value of all the useful moment at time
       * SigTime[iNumTime]*/
      LGMSVUpdateMomentValue(/* Input */
                             MomentFunc, dDt, iNbCumulant, IsUsefullMoment,
    ...
    LGMSVFillDescription(/* Input */
                         ...
                         dLambdaX, dAlpha, dLambdaEps, dRho,

                         IsUsefullMoment, MomentValue, Sig[iNumTime],
  ...
  /* Evaluation of all the moment E(PhiBar^i*f^j*V^0) =
   * E(PhiBar^i*Y^j*V^0)*exp(-j*lx*(Tstar-t))         */
  /* 		MomentFunc(i        ,j        ,k) = E(PhiBar^i*Y^j*V^k)
   */
  /*			where PhiBar = Phi - E(Phi) and Y=exp(lx*(Tstar-t))*(f(t
   *       ,Tstar)-f(0        ,Tstar))				   */
  ...
  for (iNumOrder = 1; iNumOrder <= iNbCumulant; iNumOrder++) {
    dCoef = 1.0;
    for (i = 0; i <= iNumOrder; i++) {
      MomentValue[iNumOrder - i][i][0] =
          dCoef * LGMSVFuncValue1(&MomentFunc[iNumOrder - i][i][0], dDt,
                                  lambdaArray, 0);
      dCoef *= dInvDerBeta;
    }
  }
  ...
  /*if (IsUsefullMoment[0][0][2]==LGMSV_TRUE)
          MomentValue[0][0][2] = LGMSVFuncValue1(MomentFunc[0][0][2]        ,dDt
           ,lambdaArray        ,0); */
...
void LGMSVCalculateDensity(/* Input */
  ...
  for (iNumOrder = 1; iNumOrder <= iNbCumulant; iNumOrder++) {
    ...
    for (j = 0; j <= iNumOrder; j++) {
      for (iNumPhiFreq = 1; iNumPhiFreq <= iNbPhi; iNumPhiFreq++) {
        dPhifreq = (iNumPhiFreq - 1 -
                    iNbPhi * (int)((iNumPhiFreq - 1) / (iNbPhi / 2.0))) *
                   dPhiFreqStep;
        for (iNumftFreq = 1; iNumftFreq <= (iNbft / 2.0); iNumftFreq++) {
          DensityTF[1][iNumPhiFreq][2 * (iNumftFreq - 1) + 1 + iIsComplex] +=
              d2pik_kFact * dSign * Combination[j] *
              pow(dftFreqStep * (iNumftFreq - 1), j) *
              pow(dPhifreq, iNumOrder - j) * MomentValue[iNumOrder - j][j][0];
          /*DensityTF[1][iNumPhiFreq][2*(iNumftFreq-1)+1] =
          exp(-2*dpi*dpi*dPhifreq*dPhifreq)
        ...
        if (iIsComplex == LGMSV_FALSE) {
          DensityTFspeq[1][2 * (iNumPhiFreq - 1) + 1 + iIsComplex] +=
              d2pik_kFact * dSign * Combination[j] *
              pow(dftFreqStep * iNbft / 2.0, j) * pow(dPhifreq, iNumOrder - j) *
              MomentValue[iNumOrder - j][j][0];
        }
        /*DensityTFspeq[1][2*(iNumPhiFreq-1)+1] =
        exp(-2*dpi*dpi*dPhifreq*dPhifreq)*exp(-2*dpi*dpi*dftFreqStep*(iNbft/2.0)*dftFreqStep*(iNbft/2.0));
        DensityTFspeq[1][2*(iNumPhiFreq-1)+2] = 0;*/
      }
    }
  }

  /* Normalisation such that rlft3 gives us the true ifft of TF(P(f_phi
   *       ,f_ft))) */
  ...
  /*		for (iNumftFreq=1;iNumftFreq<=(iNbft/2.0);iNumftFreq++)
                  {
                          /* Real Part */
  /*			DensityTF[1][iNumPhiFreq][2*(iNumftFreq-1)+1]
     =max(DensityTF[1][iNumPhiFreq][2*(iNumftFreq-1)+1]        ,0);
                  }
                  /* frequency (iNbft/2.0) *Dfreq */

  /* Real Part */
  /*		DensityTFspeq[1][2*(iNumPhiFreq-1)+1] =
     max(DensityTFspeq[1][2*(iNumPhiFreq-1)+1]        ,0);;
          }
...
void LGMSVCalculateDensityCumulant(/* Input */
                                   ...
                                   double **XRealPart, double **XImagPart,
                                   double **TempRealPart, double **TempImagPart,
  ...
  for (iNumOrder = 1; iNumOrder <= iNbCumulant; iNumOrder++) {
    ...
    for (j = 0; j <= iNumOrder; j++) {
      if (iIsComplex == LGMSV_TRUE)
        XImagPart[iNumOrder - j][j] = d2pik_kFact * dSign * Combination[j] *
                                      MomentValue[iNumOrder - j][j][0];
      else
        XRealPart[iNumOrder - j][j] = d2pik_kFact * dSign * Combination[j] *
                                      MomentValue[iNumOrder - j][j][0];
    }
  }
  ...
  for (n = iNbCumulant; n >= 1; n--) {
    ...
    for (iNumOrder = 0; iNumOrder <= iNbCumulant; iNumOrder++) {
      for (j = 0; j <= iNumOrder; j++) {
        ...
        for (iNumOrder1 = 0; iNumOrder1 <= iNbCumulant - iNumOrder;
             iNumOrder1++) {
          for (j1 = 0; j1 <= iNumOrder1; j1++) {
            dCumulantReal = CumulantRealPart[iNumOrder1 - j1][j1];
            dCumulantImag = CumulantImagPart[iNumOrder1 - j1][j1];

            TempRealPart[iNumOrder + iNumOrder1 - j - j1][j + j1] +=
                dXReal * dCumulantReal - dXImag * dCumulantImag;
            TempImagPart[iNumOrder + iNumOrder1 - j - j1][j + j1] +=
                dXReal * dCumulantImag + dXImag * dCumulantReal;
          }
        }
      }
    }
    ...
    for (j = 0; j <= iNumOrder; j++) {
      for (iNumPhiFreq = 1; iNumPhiFreq <= iNbPhi; iNumPhiFreq++) {
        ...
        for (iNumftFreq = 1; iNumftFreq <= (iNbft / 2.0); iNumftFreq++) {
          ...
          /* Real Part */
          DensityTF[1][iNumPhiFreq][2 * (iNumftFreq - 1) + 1] +=
              pow(dftFreq, j) * pow(dPhifreq, iNumOrder - j) *
              CumulantRealPart[iNumOrder - j][j];

          /* Imaginary Part */
          DensityTF[1][iNumPhiFreq][2 * (iNumftFreq - 1) + 2] =
              fmod(DensityTF[1][iNumPhiFreq][2 * (iNumftFreq - 1) + 2] +
                       pow(dftFreq, j) * pow(dPhifreq, iNumOrder - j) *
                           CumulantImagPart[iNumOrder - j][j],
                   d2pi);
        }
  ...
  /*
          ...
          {
                  dPhifreq =
     (iNumPhiFreq-1-iNbPhi*(int)((iNumPhiFreq-1)/(iNbPhi/2.0)))*dPhiFreqStep;
                  ...
                  for (iNumftFreq=1;iNumftFreq<=(iNbft/2.0);iNumftFreq++)
                  {
                          ...
                          /* Real Part */
  /*			DensityTF[1][iNumPhiFreq][2*(iNumftFreq-1)+1]
     =CumulantRealPart[i][j]+ pow(dftFreq        ,j)* pow(dPhifreq
           ,iNumOrder-j)*CumulantRealPart[iNumOrder-j][j];

                          /* Imaginary Part */
  /*			DensityTF[1][iNumPhiFreq][2*(iNumftFreq-1)+2] =
     fmod(DensityTF[1][iNumPhiFreq][2*(iNumftFreq-1)+2]+ pow(dftFreq
           ,j)*pow(dPhifreq ,iNumOrder-j)*CumulantImagPart[iNumOrder-j][j]
     ,d2pi);
  ...
  */
  ...
  for (iNumPhiFreq = 1; iNumPhiFreq <= iNbPhi; iNumPhiFreq++) {
    ...
    DensityTFspeq[1][2 * (iNumPhiFreq - 1) + 1] =
        exp(DensityTFspeq[1][2 * (iNumPhiFreq - 1) + 1]);
  }
...
void LGMSVFillPayoff(/* Input */
                     ...
                     int iIndexPhiMean, double dPhiStep,

                     int iIndexft0, double dftStep,

                     double dExTime, /* Exercice of the swaption in years from
                                  today */
  ...
  /* Calculation of beta(Tex        ,Tstar)*D/DTstar ( beta(Tex        ,Tstar))
   * and D/DTstar ( beta(Tex        ,Tstar)) */
  /* Use for constructing X(Tex) from Phi(Tex) and f(Tex        ,Tstar)-f(0
   * ,Tstar)
   */
  ...
  /*	Payoff(Today) = B(Today        ,Tstar)*Expectation under QTstar
   * [Payoff(Tex)/B(Tex        ,Tstar) |FToday] */
  /*																												*/
  /* Payoff(Tex)/B(Tex        ,Tstar)*B(Today        ,Tstar) = Sum of Ci*DF(Tex
   * ,Ti)/DF(Tex ,Tstar)*DF(Today        ,Tstar)
   */
  /*										= Sum of Ci*DFF(Tex
   * ,Ti)/DFF(Tex ,Tstar)*DF(Today        ,Tstar)*
   */
  /*				exp(-(beta(Tex        ,Ti)^2-beta(Tex
   *       ,Tstar)^2)*Phi(Ti)-(beta(Tex        ,Ti)-beta(Tex ,Tstar))*X(Ti))
   */
  /* Where (D/DTstar(Beta(Ti        ,Tstar)))*X(Ti) =
   */
  /*	f(Ti        ,Tstar)-f(0        ,Tstar)-(D/DTstar(Beta(Ti
   * ,Tstar))*Beta(Ti ,Tstar))(Ti        ,Tstar)
   */
  ...
  for (iNumDff = 0; iNumDff < iNbCoupon; iNumDff++) {
    dBetaTexTi =
        (1.0 - exp(-dLambdaX * (CouponTime[iNumDff] - dExTime))) / dLambdaX;
    dBeta2TexTi = dBetaTexTi * dBetaTexTi;

    CoefF = -(dBetaTexTi - dBetaTexTstar) / dDerBetaTexTstar;
    CoefPhi =
        -0.5 * (dBeta2TexTi - dBeta2TexTstar) - CoefF * dBetaDerBetaTexTstar;
...
void LGMSVSavePayOff(/* Inputs */
  ...
  for (iNumft = 1; iNumft <= iNbft; iNumft++) {
    iIndexft = iNumft - (iNumft >= iIndexft0) * iNbft + iNbft - (iIndexft0 - 1);
    dfTexTstar =
        (iIndexft - 1 - ((iIndexft - 1) > (iNbft - iIndexft0)) * iNbft) *
        dftStep;
    fprintf(fid, " %g	", dfTexTstar);
  }
  fprintf(fid, "\n");
  for (iNumPhi = 1; iNumPhi <= iNbPhi; iNumPhi++) {
    iIndexPhi = iNumPhi - (iNumPhi >= iIndexPhiMean) * iNbPhi + iNbPhi -
                (iIndexPhiMean - 1);
    dPhiTex =
        dPhitMean + (iIndexPhi - 1 -
                     ((iIndexPhi - 1) > (iNbPhi - iIndexPhiMean)) * iNbPhi) *
                        dPhiStep;

    fprintf(fid, " %g		", dPhiTex);
    for (iNumft = 1; iNumft <= iNbft; iNumft++) {
      iIndexft =
          iNumft - (iNumft >= iIndexft0) * iNbft + iNbft - (iIndexft0 - 1);
...
void LGMSVClosedForm(                 /* Inputs */
                     int iNbCumulant, /* Number of cumulant for the
                                   approximation of the (f        ,Phi) density
                                 */
  ...
  /* MomentFunc(i        ,j        ,k) = E(PhiBar^i*Y^j*V^k) where PhiBar = Phi
   * - E(Phi) a nd Y=exp(lx*(Tstar-t))*(f(t        ,Tstar)-f(0        ,Tstar))
   */
  MomentFunc =
      LGMSVSolFunc3tensor(0, iNbCumulant, 0, iNbCumulant, 0, iNbCumulant);

  MomentValue = f3tensor(0, iNbCumulant, 0, iNbCumulant, 0, iNbCumulant);

  IsUsefullMoment = l3tensor(0, iNbCumulant, 0, iNbCumulant, 0, iNbCumulant);
  ...
  if (!MomentFunc || !lambdaArray || !MomentValue || !IsUsefullMoment ||
      !DFF_div_BTTstar || !Combination || !Density || !Densityspeq || !PayOff ||
      !CumulantRealPart || !CumulantImagPart || !XRealPart || !XImagPart ||
      !TempRealPart || !TempImagPart) {
  ...
  LGMSVMomentCalculation(
      /* Inputs */
      iNbCumulant, /* Number of cumulant for the approximation of the (f ,Phi)
                density */
      ...
      /* Outputs */
      IsUsefullMoment, MomentFunc, MomentValue, &dPhitMean);

  /* Save Moments */
  LGMSVSaveMoment(/* Inputs */
                  iNbCumulant, MomentValue);
  ...
  /*	dPhiStep =
   * dPhitMean*(exp(iNbSigmaPhiGrid*dLogPhiStd)-exp(-iNbSigmaPhiGrid*dLogPhiStd))/(iNbPhi-1);/*((iNbSigmaPhiGrid+iNbSigmaPhiGrid)*dPhitStd)/(iNbPhi-1);/*dPhitMean*(exp(iNbSigmaPhiGrid*dLogPhiStd)-exp(-iNbSigmaPhiGrid*dLogPhiStd))/(iNbPhi-1);
   */

  /*	dPhiMin = dPhitMean*exp(-iNbSigmaPhiGrid*dLogPhiStd);
   * /*dPhitMean-iNbSigmaPhiGrid*dPhitStd;/*dPhitMean*exp(-iNbSigmaPhiGrid*dLogPhiStd);*/
  ...
  dPhiStep =
      (iNbSigmaPhiGridLeft + iNbSigmaPhiGridRight) * dPhitStd / (iNbPhi - 1);
  dPhifreq = 1.0 / (dPhiStep * iNbPhi) * 3.1416;
  RealPart = 0.0;
  /*
  LGMSVCalculateReClosedForm(dPhifreq        , 0        , Sig[0]        , dAlpha
  , dRho        , dExTime        , &RealPart);
  */

  while (RealPart > Limit) {
    dPhifreq *= 1.2;
    /*
    LGMSVCalculateReClosedForm(dPhifreq        , 0        , Sig[0]        ,
    dAlpha        , dRho        , dExTime        , &RealPart);
    */
  }
  ...
  */
  ...
  dPhiMin = max(dPhitMean - iNbPhi * iNbSigmaPhiGridLeft /
                                (iNbSigmaPhiGridLeft + iNbSigmaPhiGridRight) *
                                dPhiStep,
                0.0);
  NbPhiMin = (iNbSigmaPhiGridLeft + iNbSigmaPhiGridRight) * dPhitStd / dPhiStep;
  NbPhiMin =
      (int)(exp(((int)(log(NbPhiMin) / log(2.0)) + 1) * log(2)) + 1.0E-16);
  ...
  if (dLambdaEps > 1.0E-12) {
    ...
    */
    ...
    LGMSVCalculateDensityClosedForm(iNbCumulant        ,
                                                                    ...
                                                                    dPhiFreqStep
          , dftFreqStep        , Sig[0]        , dAlpha        , dRho        ,
    dExTime        , Density        , Densityspeq);
    */
  }

  LGMSVSaveDensity(/* Inputs */
                   iNbPhi, iNbft, iIndexPhiMean, iIndexft0, dPhiStep, dftStep,
                   dPhitMean, dPhitStd, Density);

  /* Fill The Payoff of the swaption */
  LGMSVFillPayoff(/* Input */
                  iNbPhi, iNbft, dLambdaX, dPhitMean, iIndexPhiMean, dPhiStep,
                  iIndexft0, dftStep,
                  dExTime,   /* Exercice of the swaption in years from today */
                  dTStar,    /* Tstar */
  ...
  LGMSVSavePayOff(/* Inputs */
                  ...
                  iIndexPhiMean, iIndexft0,

                  dPhiStep, dftStep, dPhitMean,
...
FREE_RETURN:
  ...
  /* free memory */
  if (MomentFunc)
    free_LGMSVSolFunc3tensor(MomentFunc, 0, iNbCumulant, 0, iNbCumulant, 0,
                             iNbCumulant);
  ...
  if (IsUsefullMoment)
    free_l3tensor(IsUsefullMoment, 0, iNbCumulant, 0, iNbCumulant, 0,
                  iNbCumulant);
...
Err LGMSVCalib(
    ...
    long lExDate, long lEndDate, double dStrike,
  ...
  if (iNbSigmaXGridRight == 0)
    if (fabs(dLambdaEps) < 1E-06) {
      ...
      iNbSigmaXGridRight =
          7.5 + 25 * max(dRho, 0) * dAlpha *
                    sqrt((1 - exp(-2 * dLambdaEps * cpn_time[ncpn - 1])) /
                         (2 * dLambdaEps * cpn_time[ncpn - 1]));
    }
  ...
  LGMSVClosedForm(             /* Inputs */
                  iNbCumulant, /* Number of cumulant for the approximation of
                            the (f        ,Phi) density */
                  ...
                  iNbSigmaXGridLeft, iNbSigmaXGridRight,

                  lExDate, /* Exercice date of the swaption  */
                  (lExDate - today) * YEARS_IN_DAY, /* Exercice of the swaption
                                                 in years from today */
...
static void solve_poly2_comp(double Are, double Aim, double Bre, double Bim,
                             double Cre, double Cim, double *x1re, double *x1im,
                             double *x2re, double *x2im) {
...
static void LGMSVCalculateReClosedFormPDE(
    /* Parameters */
    double lambdaX, double sig0, double alpha, double lambdaEps, double rho,
    double TStar, /* Tstar in years from today */

    double T, double dPhifreq, double dftFreq,

    int NbPoint,

    double *ResARe, double *ResAIm,

    double *ResDRe, double *ResDIm) {
  double Are, Aim, Bre, Bim, Cre, Cim, DRe, DIm, ARe, AIm, DReTemp, CreTemp,
      CimTemp, BimTemp;
  ...
  Cre =
      0.5 * sig0 * sig0 * dftFreq * dftFreq * exp(-2.0 * lambdaX * TStar) * dt;
...
void LGMSVCalculateReClosedForm(double dPhifreq, double dftFreq,
  ...
  if (fabs(DeltaRe) + fabs(DeltaIm) > 1.0E-12) {
    ...
    div_comp(x1Re * T, x1Im * T, x1Re * T + 2.0 / alpha2, x1Im, &TempRe,
             &TempIm);
...
void LGMSVCalculateDensityClosedForm(
  ...
  double x1Re, x1Im, x2Re, x2Im, DeltaRe, DeltaIm, TempRe, TempIm;
  ...
  for (iNumPhiFreq = 1; iNumPhiFreq <= iNbPhi; iNumPhiFreq++) {
    dPhifreq =
        PI2 *
        (iNumPhiFreq - 1 - iNbPhi * (int)((iNumPhiFreq - 1) / (iNbPhi / 2.0))) *
        dPhiFreqStep;
    ...
    for (iNumftFreq = 1; iNumftFreq <= nbFtHalf; iNumftFreq++) {
      ...
      if (fabs(DeltaRe) + fabs(DeltaIm) > 0.0) {
        ...
        div_comp(x1Re, x1Im, x2Re, x2Im, &TempRe, &TempIm);
        prod_comp(TempRe, TempIm, DeltaRe, DeltaIm, &TempRe, &TempIm);
        div_comp(1.0 - DeltaRe, -DeltaIm, 1.0 - TempRe, -TempIm, &TempRe,
                 &TempIm);
        prod_comp(x1Re, x1Im, TempRe, TempIm, &TempRe, &TempIm);
      } else {
        div_comp(x1Re * T, x1Im * T, x1Re * T + 2.0 / alpha2, x1Im, &TempRe,
      ...
      LGMSVCalculateReClosedForm(dPhifreq, dftFreq, sig0, alpha, rho, T,
                                 &DeltaRe);
...
void LGMSVCalculateDensityPDE(
  ...
  double ARe, AIm, DRe, DIm, TempRe, TempIm;
  double PhiMean;
  double constPhi;
  ...
  for (iNumPhiFreq = 1; iNumPhiFreq <= iNbPhi; iNumPhiFreq++) {
    ...
    for (iNumftFreq = 1; iNumftFreq <= nbFtHalf; iNumftFreq++) {
      ...
      LGMSVCalculateReClosedFormPDE(lambdaX, sig0, alpha, lambdaEps, rho, TStar,
                                    T, dPhifreq, dftFreq, NbPoint, &ARe, &AIm,
                                    &DRe, &DIm);

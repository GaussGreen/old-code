...
static SRT_Boolean is_within(Ddate today, double left, double date_or_time, double right);
...
static Err srt_f_vegashiftlgm1dtrelim(SrtStpPtr stp, SrtTreStpInf* maxtrinf, double* old_u);
    ...
    SrtUndInfo*   und_info)
{
    ...
    for (io_request = iolist->head; io_request != NULL; io_request = io_request->next)
    {
        ...
        else
        {
            ...
            For this structure, the parameters have been computed with the shift corresponding to
            the request */
    ...
    sz = xtra_calc_num * (maxtrinf.max_x_index - maxtrinf.min_x_index + 1) * grfnparam->node_dim;

    prev_assets = f3tensor(
        0,
        xtra_calc_num - 1,
        maxtrinf.min_x_index,
        maxtrinf.max_x_index,
        0,
        ...
        for (i = trinf->min_x_index; i <= trinf->max_x_index; i++)
        {
            ...
            */
            sam_get(node.cur_sam, 0, SHORT_RATE) =
                sam_get(node.cur_sam, 0, STATEVAR) + sam_get(tminf->fwd_sam, 0, F_0_t);
            ...
            for (io_request = iolist->head; io_request != NULL; io_request = io_request->next)
            {
                ...
                if (stp->next)
                {
                    ...
                    Y_T_at_t_compute(1, &node.cur_sam, &tminf->yp, &zc_yield, 0, ONE_FAC, LGM);
    ...
    err = srt_f_IOstructsetcolpvs(
        (iolist), SRT_NO, (double*)prev_assets[0][0], grfnparam->node_dim, "");
...
static Err srt_f_vegashiftlgm1dtrelim(
    SrtStpPtr stp, SrtTreStpInf* maxtrinf, double* old_u /* previous value of rate spacing u */
)
{
    Err           err;
    SrtStpPtr     cur;
    ...
    int         tau_shift_type)
{
    ...
    for (ls = ts->head; ls != NULL; ls = ls->next)
    {
        ...
        case SH_NONE:
            break;
        case SH_ABSOLUTE:
            if (is_within(-1, sigma_bucket_start, tsval->date, sigma_bucket_end) == SRT_YES)
        ...
        case SH_NONE:
            break;
        case SH_ABSOLUTE:
            if (is_within(-1, tau_bucket_start, tsval->date, tau_bucket_end) == SRT_YES)
                tsval->tau += tau_shift;
            break;
        case SH_RELATIVE:
            if (is_within(-1, tau_bucket_start, tsval->date, tau_bucket_end) == SRT_YES)

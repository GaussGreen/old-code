...
Err srt_f_vegashifttrelgm1d(SrtUndPtr und, SrtGrfnParam *grfnparam,
  ...
  for (io_request = iolist->head; io_request != NULL;
    ...
    if (srt_f_vegatrelgm1d_requestOK(io_request_val_p->type) == SRT_NO) {
      ...
      structure. For this structure  , the parameters have been computed with
      the shift corresponding to the request */
  ...
  for (stp = bot; stp != NULL; stp = stp->prev) {
    ...
    for (i = trinf->min_x_index; i <= trinf->max_x_index; i++) {
      ...
      sam_get(node.cur_sam, 0, STATEVAR) = cur_x[i] + tminf->rf.onef.H;
      ...
      */
      ...
      for (io_request = iolist->head; io_request != NULL;
        ...
        if (stp->next) {
          ...
          Y_T_at_t_compute(1, &node.cur_sam, &tminf->yp, &zc_yield, 0, ONE_FAC,
                           LGM);
          ...
          */
          /* Modified node.mid_son_index  - Please note that this function uses
             ...
             previous assumption that the tree is build on X  , not really on
             r(t)-f(0  ,t)*/
          ...
          /*
                                  srt_f_chetreprob(my_pointer  ,prev_x  ,&node
             ,mdl);
          */
        ...
        err = evalcf((GrfnEvent *)my_pointer->e, &node.cur_sam, gd,

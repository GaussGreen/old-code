...
Err ctsqto_calib_und(
  ...
  //		corr_n_times  , corr_times  , correl_dom_for  , correl_dom_fx  ,
  //correl_for_fx  ,
  //		&(und->sigma_n)  , &(und->sigma_time)  , &(und->dom_sigma)  ,
  //&(und->for_sigma)  ,
...
LGMQTOPhi(int nstept, double *time, double domlam, double forlam,
  ...
  for (i = 1; i < nstept; i++) {
    ...
    //(1-domforrho[j]*domforrho[j]) 							+domforrho[j]*domforrho[j]*domphi[i] /

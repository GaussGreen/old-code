...
Err merge_all_term_structure(int num_sig, double *sig_time, double *dom_sig, double *for_sig,
		int num_fx_vol, double *fx_vol_time, double *fx_vol,
		int corr_n_times, double *corr_times, double *correl_dom_for, double *correl_dom_fx, double *correl_for_fx,
		int *sigma_n, double **sigma_time, double **dom_sigma, double **for_sigma,
		double **fx_sigma, double **domfor_rho, double **quanto_rho)
{
	int i, j;
...
//	cpd_init_calib_inst_data (&(und->for_inst_data));
	...
	if (ctsqto->num_calls > 0 && !(ctsqto->num_calls == 1 && ctsqto->call[0].ex_date <= today + eod_flag))
	{
		...
		for (i=0; i<nex; i++)
		{
			...
			{
//				numMonth = (int)((end_swap - ctsqto->call[i].ex_date)/365.0 * 12);
				numMonth = 12;
...
//		&(und->fx_sigma), &(und->domfor_rho), &(und->quanto_rho));
	...
	err = merge_rates_fx_corr_ts(sig_time, dom_sig, num_sig, 
								sig_time, for_sig, num_sig,
								fx_vol_time, fx_vol, num_fx_vol,
								corr_times, 
								correl_dom_for, correl_dom_fx, correl_for_fx, 
								corr_n_times,
								&(und->sigma_time), 
								&(und->dom_sigma), &(und->for_sigma), &(und->fx_sigma), 
								&(und->domfor_rho), &domfx_rho, &(und->quanto_rho),
								&(und->sigma_n));
...
FREE_RETURN:
	...
	for (i=0; i<nex; i++)
	{
		if(dom_end_tenor[i]) free(dom_end_tenor[i]);
		if(for_end_tenor[i]) free(for_end_tenor[i]);
	}
	if(dom_end_tenor) free(dom_end_tenor);
	if(for_end_tenor) free(for_end_tenor);

	smessage ("Free in ctsqto_calib_und 2");
	if (err) free_lgmQto_und (und);

	if(dom_swap_freq_loc) free(dom_swap_freq_loc);
	if(for_swap_freq_loc) free(for_swap_freq_loc);

	smessage ("Free in ctsqto_calib_und 3");
	if (lex) free (lex);
	if (uselex) free (uselex);
	if (dom_strikes) free (dom_strikes);
	if (for_strikes) free (for_strikes);

	smessage ("Free in ctsqto_calib_und 4");
	if (sig_time) free (sig_time);
	if (dom_sig) free (dom_sig);
	if (for_sig) free (for_sig);
	if (fx_vol_time) free (fx_vol_time);
	if (fx_vol) free (fx_vol);
	if (domfx_rho) free (domfx_rho);
		...
		for (j=l2; j<u2; j++)
		{
			...
			if (RA->cpn_type !=0)
			{
				if (RA->float_cpn_is_dom_for == 0)
				{
					...
					{
						ra_dfs[2][l] = cpn_float_dff[l] * exp (-cpn_float_gam[l] * r1[i] - cpn_float_gam2[l]);
					}
				}
				else
				{
					for (l=0; l<2*pn_dfs[2]; l++)
					{
						ra_dfs[2][l] = cpn_float_dff[l] * exp (-cpn_float_gam[l] * r2[i][j] - cpn_float_gam2[l]);
					}
				}
			}
...
FREE_RETURN:
	...
	if(ra_pdates[0]) free(ra_pdates[0]);
	if(ra_pdates[1]) free(ra_pdates[1]);
	if (RA->cpn_type !=0)
	{
		if(ra_pdates[2]) free(ra_pdates[2]);	
	}
	if(ra_pdates) free(ra_pdates);

	if (dom_dff) free_dvector (dom_dff, 0, pn_dfs[0]-1);
	if (dom_gam) free_dvector (dom_gam, 0, pn_dfs[0]-1);
	if (dom_gam2) free_dvector (dom_gam2, 0, pn_dfs[0]-1);

	if (for_dff) free_dvector (for_dff, 0, pn_dfs[1]-1);
	if (for_gam) free_dvector (for_gam, 0, pn_dfs[1]-1);
	if (for_gam2) free_dvector (for_gam2, 0, pn_dfs[1]-1);

	if (cpn_float_dff) free_dvector(cpn_float_dff, 0, 2*pn_dfs[2] - 1);
	if (cpn_float_gam) free_dvector(cpn_float_gam, 0, 2*pn_dfs[2] - 1);
	if (cpn_float_gam2) free_dvector(cpn_float_gam2, 0, 2*pn_dfs[2] - 1);

	if (fund_dom_dff) free_dvector (fund_dom_dff, 0, fund_ndfs-1);
	if (fund_dom_gam) free_dvector (fund_dom_gam, 0, fund_ndfs-1);
	if (fund_dom_gam2) free_dvector (fund_dom_gam2, 0, fund_ndfs-1);
	
	if(pn_dfs) free(pn_dfs);

	if(fund_dfs) free(fund_dfs);
	if(fund_pdates) free(fund_pdates);
...
FREE_RETURN:
	...
	if(calibparam) free(calibparam);
	
	if(event_time) free_dvector(event_time, 0,ctsqto->num_calls-1);
...
//		std1 = 0;
		...
		fwd2[i] = (exp(-forlam*t2) * forI1 - exp(-2*forlam*t2) * forI2)/forlam
...
//		std3[i-1] = sqrt(
//							(domsig[j]/forsig[j])*(domsig[j]/forsig[j]) * forphi[i] / (1-domforrho[j]*domforrho[j])
//							+domforrho[j]*domforrho[j]*domphi[i] / (1-domforrho[j]*domforrho[j])
//							-2*domforrho[j]*(domsig[j]/forsig[j])*domforphi[i] / (1-domforrho[j]*domforrho[j])
//							);
...
Err		err = NULL;
	...
	if (!time || !x || !w || !domphi || !forphi || !domforphi 
		|| !sigmadom || !sigmafor || !quantocorrel || !domforcorrel 
		|| !fwd1 || !fwd2 || !std1vec || !std2vec 
		|| !corrvec || !r1 || !r2 || !pv)
	{
	...
	/* Phi Calculation */
	err = LGMQTOPhi(ctsqto->num_calls+1, 
					time,
					und->dom_lambda,
					und->for_lambda,
					und->sigma_time,
	...
	for (i=0; i<ctsqto->num_calls; i++)
	{
		...
		for (j=0; j<num_hermite; j++)
		{
			...
			{
				r2[j][k] = fwd2[i+1] + std2 * ( corr * x[j+1] + sqrt(1-corr*corr) * x[k+1] );
			}
		}
		...
		err = ctsqto_payoff_adi2(call->ex_date,
								call->ex_time,
		...
		for (j=0; j<num_hermite; j++)
		{
			...
			for (k=0; k<num_hermite; k++)
			{
				...
				{
					test_for_gam = (1.0 - exp ( - und->for_lambda * (ctsqto->call[i+1].ex_time - ctsqto->call[i].ex_time) ) ) / und->for_lambda;
					test_for_gam2 = 0.5 * test_for_gam * test_for_gam * forphi[i+1];
					test += w[j+1] * swp_f_df (call->ex_date, ctsqto->call[i+1].ex_date, und->for_yc)
...
FREE_RETURN:
	...
	if (domphi) free (domphi);
	if (forphi) free (forphi);
	if (domforphi) free (domforphi);

	if (sigmadom) free (sigmadom);
	if (sigmafor) free (sigmafor);
	if (quantocorrel) free (quantocorrel);
	if (domforcorrel) free (domforcorrel);
	if (fwd1) free (fwd1);
	if (fwd2) free (fwd2);
	if (std1vec) free (std1vec);
	if (std2vec) free (std2vec);
	if (corrvec) free (corrvec);

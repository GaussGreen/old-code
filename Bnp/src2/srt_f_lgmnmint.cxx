 ...
 * sm_qsimp        , you need lots of global variables.  This method isn't
 *actually used (it is too slow); but that's why the code is structured that way
 *
 *
 *------------------------------------------------------------------------*/
...
/* this was 32 points but ten have weight < 0.0000000001        , these have
   been removed */
static double gauher_out[] = {
    6.0889643091,  0.0000000023,  5.4500332736,  0.0000000888,  4.8326046132,
    0.0000020596,  4.2320211100,  0.0000305598,  3.6447812499,  0.0003025570,
    3.0681351690,  0.0020620511,  2.4998404152,  0.0099034612,  1.9380049059,
    0.0341098475,  1.3809801993,  0.0853448082,  0.8272849038,  0.1565389938,
    0.2755464192,  0.2117055699,  -0.2755464192, 0.2117055699,  -0.8272849038,
    0.1565389938,  -1.3809801993, 0.0853448082,  -1.9380049059, 0.0341098475,
    -2.4998404152, 0.0099034612,  -3.0681351690, 0.0020620511,  -3.6447812499,
    0.0003025570,  -4.2320211100, 0.0000305598,  -4.8326046132, 0.0000020596,
    -5.4500332736, 0.0000000888,  -6.0889643091, 0.0000000023};
...
static SrtErr srt_f_lgmfindr(SrtRtFnc rt, SrtSample *sam, double val,
                             double *answer) {
  double tmp = 0.0;
  SrtErr err;
...
static void lgm_populate_tminf(TermStruct *ts, double prev_time,
                               double cur_time, SrtIRMTmInf *tminf) {
...
static double lgmcms_integrand(double dr) {
  ...
  sam_get(NI_sam[NI_index + 1], 0, STATEVAR) =
      sam_get(NI_sam[NI_index], 0, STATEVAR) *
          NI_tminf[NI_index + 1].ev.onef.F / NI_tminf[NI_index].ev.onef.F +
      NI_tminf[NI_index + 1].ev.onef.F * NI_tminf[NI_index].rf.onef.G *
          (NI_tminf[NI_index + 1].ev.onef.Psi -
           NI_tminf[NI_index].ev.onef.Psi) +
  ...
  /* sad fix */
  sam_get(NI_sam[NI_index + 1], 0, SHORT_RATE) =
      sam_get(NI_sam[NI_index + 1], 0, STATEVAR);
  ...
  payoff = df * gauss(dr) * (cms + CMS_MARGIN) *
           ((cms < CMS_STRIKE && cms > CMS_STRIKE2) ? 1.0 : 0.0);
...
/*
         err = srt_f_lgmrfltr(start        ,end        ,freq        ,bas ,start2
   ,end2        ,fix1 ,fix2        , paydt        ,strike        ,strike2
   ,margin        ,und        ,&answer);
*/

SrtErr srt_f_lgmrfltr(Date start1, Date enfp1, char *freq, char *bas,
                      Date start2, Date enfp2, Date fix1, Date fix2, Date paydt,
                      double strike, double strike2, double margin,
                      SrtUndPtr und, double rate1, double rate2,
                      double *answer) {
  SrtCurvePtr crv;
  ...
  if (fix1 <= today) {
    ...
    sam_get(NI_tminf[1].fwd_sam, 0, F_0_t) =
        swp_f_zr(fix2, fix2 + 30.0, ycname);
    ...
    mu = sam_get(NI_sam[NI_index], 0, STATEVAR) *
             NI_tminf[NI_index + 1].ev.onef.F / NI_tminf[NI_index].ev.onef.F +
         NI_tminf[NI_index + 1].ev.onef.F * NI_tminf[NI_index].rf.onef.G *
             (NI_tminf[NI_index + 1].ev.onef.Psi -
              NI_tminf[NI_index].ev.onef.Psi);
    ...
    sam_get(NI_tminf[2].fwd_sam, 0, F_0_t) =
        swp_f_zr(fix2, 2 * fix2 - fix1, ycname);
    sam_get(NI_sam[0], 0, PHI) = sam_get(NI_tminf[0].fwd_sam, 0, PHI);
    sam_get(NI_sam[1], 0, PHI) = sam_get(NI_tminf[1].fwd_sam, 0, PHI);

...
/****************************************************************************/
/* Gets sigma(t) = sigma[j] for t[j]<t<t[j+1] for j=0        ,...        ,
ntau-1 from zeta[j] = zeta(t) at t=tau[j] for j=0        ,...        , numS
and kap[i] = kappa(t) for tkap[i]<t<t[i+1] */
static void GetSigFromZeta(double g0, long ntau, double *sig, Date *tau,
                           double *zeta, long numK, Date *t, double *kap);
...
static double GetDeltaLogb(long n, double *Arr, Date *tArr, double avg,
                           long iw);
...
void LGMFreeRefSwptnData(SrtLgmRefSwptnData *ptr) {
  ...
  if (ptr->refShortSwptnArr != NULL)
    srt_free(ptr->refShortSwptnArr);
  if (ptr->refLongSwptnArr != NULL)
    srt_free(ptr->refLongSwptnArr);
  if (ptr->refCapSwptnArr != NULL)
    srt_free(ptr->refCapSwptnArr);
  if (ptr->refFixSwptnArr != NULL)
    srt_free(ptr->refFixSwptnArr);
...
SrtSimMidAtPtr LGMCreateSimMidAt(long nEx, long nPay) {
  ...
  if (ptr->Payment == NULL || ptr->tPay == NULL || ptr->tEx == NULL ||
      ptr->tStart == NULL || ptr->Strike == NULL || ptr->FirstPay == NULL ||
...
void LGMFreeSimMidAt(SrtSimMidAt **ptr) {
  ...
  if ((*ptr)->Strike != NULL)
    srt_free((*ptr)->Strike);
  if ((*ptr)->FirstPay != NULL)
    srt_free((*ptr)->FirstPay);
  if ((*ptr)->RedFirstPay != NULL)
    srt_free((*ptr)->RedFirstPay);
...
LGMErr LGMValidSimMidAt(SrtSimMidAt *ptr, Date tfirst, long *nExleft) {
  ...
  for (j = 0; j < nEx; j++) {
    ...
    if (ptr->tEx[j] > ptr->tStart[j] || ptr->tStart[j] > ptr->tPay[i])
...
SrtGenMidAtPtr LGMCreateGenMidAt(long nEx, long *nPayArr) {
  ...
  if (ptr->tEx == NULL || ptr->nPay == NULL || ptr->Payment == NULL ||
      ptr->tPay == NULL) {
    srt_free(ptr->tEx);
    srt_free(ptr->nPay);
    srt_free(ptr->Payment);
...
SrtSimAmerPtr LGMCreateSimAmer(long nfix, long nflt) {
  ...
  if (ptr->tfixStart == NULL || ptr->tfixEnd == NULL || ptr->tfixPay == NULL ||
      ptr->fixCoupon == NULL || ptr->ExtraPrem == NULL ||
      ptr->tfltFixing == NULL || ptr->tfltPay == NULL) {
    LGMFreeSimAmer(&ptr);
    return (NULL);
  }
  return (ptr);
...
void LGMFreeSimAmer(SrtSimAmer **ptr) {
  ...
  if ((*ptr)->tfixStart)
    srt_free((*ptr)->tfixStart);
  if ((*ptr)->tfixEnd)
    srt_free((*ptr)->tfixEnd);
  if ((*ptr)->tfixPay)
    srt_free((*ptr)->tfixPay);
  if ((*ptr)->fixCoupon)
    srt_free((*ptr)->fixCoupon);
  if ((*ptr)->ExtraPrem)
    srt_free((*ptr)->ExtraPrem);
  if ((*ptr)->tfltFixing)
    srt_free((*ptr)->tfltFixing);
  if ((*ptr)->tfltPay)
    srt_free((*ptr)->tfltPay);
...
LGMErr LGMValidSimAmer(SrtSimAmer *ptr, Date tfirst) {
  ...
  if (ptr->CalBusLag == 1)
    tfirstStart =
        add_unit(tfirstExer, ptr->lagExerSettle, SRT_BDAY, ptr->convSettle);
  else
    tfirstStart =
        add_unit(tfirstExer, ptr->lagExerSettle, SRT_DAY, ptr->convSettle);
  if (tfirstStart >= ptr->tfixEnd[nfix - 1] ||
      tfirstStart >= ptr->tfltPay[nflt - 1])
    return ("no underlying left");

  for (i = 0; i < (nfix - 1) && ptr->tfixStart[i] < ptr->tfixEnd[i] &&
              ptr->tfixStart[i] < ptr->tfixStart[i + 1] &&
              ptr->tfixEnd[i] < ptr->tfixEnd[i + 1] &&
              ptr->tfixPay[i] <= ptr->tfixPay[i + 1] &&
              ptr->tfixStart[i] < ptr->tfixPay[i];
       i++)
    ;
  ...
  for (j = 0; j < (nflt - 1) && ptr->tfltPay[j] < ptr->tfltPay[j + 1] &&
              ptr->tfltFixing[j] < ptr->tfltFixing[j + 1] &&
              ptr->tfltFixing[j] < ptr->tfltPay[j];
       j++)
    ;
...
/*******************************************/
/* Allocate a Bermudan inverse floater with nEx exercise dates and n coupon
 * periods */
/* Returns pointer to structure; on failure returns NULL */
SrtCallInvFltPtr LGMCreateCallInvFlt(long nEx, long n) {
  ...
  if (!ptr->tEx || !ptr->iSet || !ptr->strike || !ptr->tCpnStart ||
      !ptr->tCpnPay || !ptr->a || !ptr->gear || !ptr->cvg || !ptr->lcvg ||
      !ptr->cap_str) {
...
void LGMFreeCallInvFlt(SrtCallInvFlt **ptr) {
  ...
  if ((*ptr)->strike != NULL)
    srt_free((*ptr)->strike);
  if ((*ptr)->tCpnStart != NULL)
    srt_free((*ptr)->tCpnStart);
  if ((*ptr)->tCpnPay != NULL)
    srt_free((*ptr)->tCpnPay);
  if ((*ptr)->a != NULL)
    srt_free((*ptr)->a);
  if ((*ptr)->gear != NULL)
    srt_free((*ptr)->gear);
  ...
  if ((*ptr)->lcvg != NULL)
    srt_free((*ptr)->lcvg);
  if ((*ptr)->cap_str != NULL)
    srt_free((*ptr)->cap_str);
  if ((*ptr)->fwdVolMat != NULL)
    free_dmatrix((*ptr)->fwdVolMat, 0, (*ptr)->fwdVolCpn, 0, (*ptr)->fwdVolEx);
...
LGMErr LGMValidCallInvFlt(SrtCallInvFlt *ptr, Date tfirst, long *EffnEx) {
  ...
  /* Check coupons */
  for (i = 1; i < n; i++) {
    if (ptr->tCpnStart[i] <= ptr->tCpnStart[i - 1] ||
        ptr->tCpnPay[i] <= ptr->tCpnPay[i - 1])
      return ("Coupon dates out of order");
    if (ptr->a[i] <= 0 || ptr->a[i - 1] <= 0 || ptr->gear[i] < 0 ||
        ptr->gear[i - 1] < 0)
...
SrtCallTimeSwapPtr LGMCreateCallableTimeSwap(long nEx, long n,
  ...
  for (i = 0; i < nAlloc; i++) {
    (ptr->observationdays)[i] =
        (Date **)srt_calloc(observation_freq, sizeof(Date *));
  ...
  */
  ...
  ptr->ratiodays_for_subperiod =
      (double **)srt_calloc(nAlloc, sizeof(double *));
  for (i = 0; i < nAlloc; i++)
    (ptr->ratiodays_for_subperiod)[i] =
...
void LGMFreeCallableTimeSwap(SrtCallTimeSwap **ptr) {
  ...
  if ((*ptr)->gear != NULL)
    srt_free((*ptr)->gear);
  if ((*ptr)->tCvgCpn != NULL)
    srt_free((*ptr)->tCvgCpn);
  if ((*ptr)->tCpn != NULL)
    srt_free((*ptr)->tCpn);
  if ((*ptr)->tFundingPayment != NULL)
    srt_free((*ptr)->tFundingPayment);
  for (i = 0; i < (*ptr)->nCpn; i++) {
    if (((*ptr)->barriers)[i] != NULL)
      srt_free(((*ptr)->barriers)[i]);
  }
  if ((*ptr)->barriers != NULL)
    srt_free((*ptr)->barriers);
  for (i = 0; i <= 1; i++) {
    if (((*ptr)->tForwardTS)[i] != NULL)
      srt_free(((*ptr)->tForwardTS)[i]);
  }
  if ((*ptr)->tForwardTS != NULL)
    srt_free((*ptr)->tForwardTS);
  for (i = 0; i < (*ptr)->nCpn; i++) {
    for (j = 0; j < (*ptr)->observation_freq; j++) {
      if (((*ptr)->observationdays)[i][j] != NULL)
        srt_free(((*ptr)->observationdays)[i][j]);
    }
    if (((*ptr)->observationdays)[i] != NULL)
      srt_free(((*ptr)->observationdays)[i]);
  }
  if ((*ptr)->observationdays != NULL)
    srt_free((*ptr)->observationdays);
  for (i = 0; i < (*ptr)->nCpn; i++) {
    if (((*ptr)->ratiodays_for_subperiod)[i] != NULL)
      srt_free(((*ptr)->ratiodays_for_subperiod)[i]);
  }
  if ((*ptr)->ratiodays_for_subperiod != NULL)
    srt_free((*ptr)->ratiodays_for_subperiod);
  if ((*ptr)->fwdVolMat != NULL)
    free_dmatrix((*ptr)->fwdVolMat, 0, (*ptr)->fwdVolCpn, 0, (*ptr)->fwdVolEx);
  /* Just for debug
  if ((*ptr)->reval_times != NULL) free_dvector((*ptr)->reval_times        ,0
  ,19);
  */
...
SrtCallCapFltPtr LGMCreateCallCapFlt(long nEx, long n) {
  ...
  if (ptr->tCpn == NULL || ptr->amax == NULL || ptr->amin == NULL ||
      ptr->marg == NULL || ptr->tEx == NULL || ptr->iSet == NULL ||
      ptr->strike == NULL) {
...
LGMErr LGMValidCallCapFlt(SrtCallCapFlt *ptr, Date tfirst, long *EffnEx) {
  ...
  if (ptr->cpnBasis < 0 || ptr->cpnBasis >= LASTBASISCODE || ptr->aBasis < 0 ||
      ptr->aBasis >= LASTBASISCODE)
...
LGMErr LGMVerifyLGM_TS_For_DiagSwpts(LGMCalSet *CSPtr, LGM_TS *tsPtr) {
  ...
  for (i = 2; i <= n_swpts; i++) {
    if (CSPtr->Vlong[i] > CSPtr->Vlong[most_exp_index])
      most_exp_index = i;
  }
...
LGM_TSPtr LGMCreateLGM_TS(long numZ, long NumG) {
  ...
  if (ptr->zdate == NULL || ptr->zeta == NULL || ptr->Gdate == NULL ||
      ptr->G == NULL)
...
LGM_TSPtr LGMCreateLGM2F_TS(long numZ, long numG) {
  ...
  if (ptr->zdate == NULL || ptr->Gdate == NULL || ptr->H1 == NULL ||
      ptr->H2 == NULL || ptr->Zeta1Scenari == NULL ||
      ptr->Zeta2Scenari == NULL || ptr->Zeta12Scenari == NULL ||
...
void LGMFreeLGM2F_TS(LGM_TS **ptr) {
  ...
  srt_free((*ptr)->Zeta1Scenari);
  (*ptr)->Zeta1Scenari = NULL;
  srt_free((*ptr)->Zeta2Scenari);
  (*ptr)->Zeta2Scenari = NULL;
  srt_free((*ptr)->Zeta12Scenari);
  (*ptr)->Zeta12Scenari = NULL;
  srt_free((*ptr)->H1Scenari);
  (*ptr)->H1Scenari = NULL;
  srt_free((*ptr)->H2Scenari);
  (*ptr)->H2Scenari = NULL;
...
SigKapTSPtr LGMCreateSigKapTS(long NumS, long NumK) {
  ...
  if (ptr->sdate == NULL || ptr->sig == NULL || ptr->kdate == NULL ||
      ptr->kap == NULL) {
    LGMFreeSigKapTS(&ptr);
  }
...
LGM_TSPtr LGMConvertSigKaptoZG(Date tNow, SigKapTS *SKtsPtr) {
  ...
  for (j = 1; j <= numZ; j++) {
    ...
    LGMtsPtr->zeta[j] = LGMtsPtr->zeta[j - 1] + SKtsPtr->sig[j - j1 - 1] *
                                                    SKtsPtr->sig[j - j1 - 1] *
                                                    cumul;
  }
...
/*	Modified AS -still doesn't work in all cases        , so we keep track
   of the original Autocal TS        , in order to do tests directly on it
 */
...
static double find_alpha(double dt12, double dt23, double dg12, double dg23,
...
static double find_sigma(double dt, double dz, double lambda, double alpha,
                         double beta) {
...
SigKapTSPtr LGMConvertZGtoSigKap(Date tNow, LGM_TS *input) {
  ...
  /*	Case 1: there is only two Gs        , thus a single mean-reversion that
     can be chosen arbitrarily        , so we choose 0	*/
  ...
  /*	Case 2: there is more than two Gs	*/
  {
    ...
    alpha = find_alpha((Gdate[numG - 2] - Gdate[numG - 3]) * YEARS_IN_DAY,
                       ...
                       G[numG - 3] - G[numG - 2], G[numG - 2] - G[numG - 1],
                       &lambda, &beta);
    ...
    for (i = numG - 1; i >= 1; i--) {
      ...
      output->kap[i - 1] = find_lambda((Gdate[i] - Gdate[i - 1]) * YEARS_IN_DAY,
                                       G[i - 1] - G[i], alpha, beta);
      beta *=
          exp(output->kap[i - 1] * (Gdate[i] - Gdate[i - 1]) * YEARS_IN_DAY);
    }
  }

  /*	GET THE EQUIVALENT SIGMA TERM STRUCTURE	*/

  lambda = interpolate_lambda(input->zdate[input->numZ - 1], Gdate[numG - 1],
                              output->kdate, output->kap, output->numK);

  beta = exp(lambda * (Gdate[numG - 1] - input->zdate[input->numZ - 1]) *
             YEARS_IN_DAY);

  for (i = input->numZ - 1; i >= 1; i--) {
    lambda = interpolate_lambda(input->zdate[i], input->zdate[i - 1],
                                output->kdate, output->kap, output->numK);

    output->sdate[i - 1] = input->zdate[i];
    output->sig[i - 1] =
        find_sigma((input->zdate[i] - input->zdate[i - 1]) * YEARS_IN_DAY,
                   input->zeta[i] - input->zeta[i - 1], lambda, alpha, beta);

    beta *=
        exp(lambda * (input->zdate[i] - input->zdate[i - 1]) * YEARS_IN_DAY);
  }
...
/******************************************************************/
/* Gets sigma(t) = sigma[j] for tau[j]<t<tau[j+1] for j=0        ,...        ,
 * ntau-1 */
...
void LGMFreeCalSet(LGMCalSet **ptr) {
  ...
  if ((*ptr)->nlong)
    srt_free((*ptr)->nlong);
  if ((*ptr)->cvgfirst)
    srt_free((*ptr)->cvgfirst);
  if ((*ptr)->Rflong)
    srt_free((*ptr)->Rflong);
  if ((*ptr)->Vlong)
    srt_free((*ptr)->Vlong);
  if ((*ptr)->VegaLong)
    srt_free((*ptr)->VegaLong);
  if ((*ptr)->FrLong)
    srt_free((*ptr)->FrLong);
  if ((*ptr)->BetaLong)
    srt_free((*ptr)->BetaLong);
  if ((*ptr)->CEVLong)
    srt_free((*ptr)->CEVLong);
  if ((*ptr)->StDLong)
    srt_free((*ptr)->StDLong);
  if ((*ptr)->nshort)
    srt_free((*ptr)->nshort);
  if ((*ptr)->Rfshort)
    srt_free((*ptr)->Rfshort);
  if ((*ptr)->Vshort)
    srt_free((*ptr)->Vshort);
  if ((*ptr)->tEnd)
    srt_free((*ptr)->tEnd);
...
date tEx and last (theor) pay date tEnd        , using market conventions in
*conv. If extraperiods>0        , add extraperiod standard periods after tEnd */
/* Returns an array with tPay[0        ,1        ,...        ,nPay] where nPay =
*nPayPtr        , where tPay[0] is the start date        , and where tPay[i] is
the i-th pay date On error        , returns NULL */
Date *LGMFixLegSched(Date tEx, Date tEnd, long extraperiods, LGMMarkConv *conv,
  ...
  n = 0;
  while ((n <= LGMUTIL_MAXNB_PERIOD) &&
         (tStart < add_unit(tEnd, -n * swmonth, SRT_MONTH, conv->sbdconv)))
    n++;
  ...
  tPay[0] = tStart;
  for (i = 1; i <= n; i++)
    tPay[i] = add_unit(tEnd, -(n - i) * swmonth, SRT_MONTH,
                       conv->sbdconv /*        , ccy */);

  /* add extra periods if needed */
  for (i = 1; i <= extraperiods; i++)
    tPay[n + i] = add_unit(tEnd, i * swmonth, SRT_MONTH,
                           conv->sbdconv /*        , ccy */);
...
start date tPay[0] and paydates tPay[i]        , i=1        ,...        ,nPay
   Returns an array with cvg[0        ,1        ,2        ,...        ,nPay]
where cvg[0] is UNDEFINED and cvg[i] is the cvg from tPay[i-1] to tPay[i] On
error        , returns NULL */
...
start date tPay[0] and paydates tPay[i]        , i=1        ,...        ,nPay
/* Returns an array with DPay[0        ,1        ,2        ,...        ,nPay]
where DPay[0] is the discount factor to the start date        , and DPay[i] is
the discount factor to tPay[i] On error        , returns NULL */
...
start date tPay[0] and paydates tPay[i]        , i=1        ,...        ,nPay
/* Returns an array with GPay[0        ,1        ,2        ,...        ,nPay]
where GPay[0] is G(t) at the start date tPay[0]        , and GPay[i] is G(t) at
the pay date tPay[i] On error        , returns NULL */
...
LGMErr
LGMGetRfixFromX(Date tEx, Date tEnd, double x, /* definition of swaption */
...
LGMErr LGMGetFloatRateFromX(Date tStart, Date tPay, double x, String ycName,
  ...
  dDfStartPay = (dDfPay / dDfStart) *
                exp((dGPay - dGStart) * x -
                    0.5 * dZetaStart * (dGPay * dGPay - dGStart * dGStart));
...
/****** Math Utilities ******************************************/
/****************************************************************/
/* calculates the Gaussian probability density        , protected against
 * underflows
 */
...
/****************************************************************/
/* calculates the cumulative normal distribution        , protected against
 * underflows
 */
    ...
    double beta)   /* exponenet in CEV model */
{
  ...
  if (beta > 0.98)
    beta = 1.0; /* input is log normal vol */
  if (beta < 0.005)
    return (CEVvol); /* input is normal vol */
  ...
  /* general case */
    ...
    fac = (pow(fwd, 1.0 - beta) - pow(strike, 1.0 - beta)) /
          ((1.0 - beta) * (fwd - strike));

  /* get the normal volatility */
  normvol = CEVvol / fac;
  normvol = normvol / (1.0 + beta * (2.0 - beta) * normvol * normvol * mat /
                                 (24.0 * fav * fav));
  /* SEE 4.21 A FORMULA  */
  return (normvol);
}
...
double LGMNormImpVol(double price, double fwd, double strike, double mat,
  ...
  x = fabs(fwd - strike);
  stddev = (price + 0.5 * x) /
           LGMsafeGauss(0); /* guaranteed to converge from this initial guess */
  arg1 = -x / stddev;
  ...
  for (iter = 1; iter < itermax && success < 0; iter++) {
    ...
    /* check for convergence ... take two more steps after satisfactory
     * convergence */
    if (fabs(dPrice / price) < 1.0e-7 || fabs(dPrice) < 1.0e-11 ||
        fabs(dstddev / stddev) < 1.0e-7)
      success++;
...
double LGMBlackOptPrice(double fwd, double strike, double BlackVol, double mat,
  ...
  if ((fabs(arg) > 10.0 * stddev) ||
      stddev <= 1.0e-12) /* nearly no time value */
  {
    theta = -10;
    price = 0.5 * stddev * (fwd + strike) *
            (LGMsafeGauss(theta) + theta * LGMsafeNorm(theta));
    if (callput == SRT_CALL && fwd > strike)
      price = price + fwd - strike;
...
/* COMPUTE THE DIAG SWAPTIONS EQ. STRIKES FOR EACH EXERCISE DATES FOR EMKi
   METHODS
...
Err update_autocal_midat_struct(SrtSimMidAt *MidAt, char *YcName,
  ...
  double *CvgFirst, *CvgPay, *Ks, *StDLong, DtPay, DtStart, DtEnd, Payt,
      SwapLevel, SwapRate, YrtoExp, CEVVol, Beta, NORMVol;
  SrtCurvePtr YldCrv;
  String ccy_str;
  SrtCcyParam *ccy_param = NULL;
  SrtBasisCode FixedBasis;
  long spot_lag, nPay = MidAt->nPay, nEx = MidAt->nEx;
  ...
  for (i = 1; i < nPay; i++)
    CvgPay[i - 1] = coverage(tPay[i - 1], tPay[i], FixedBasis);
  ...
  for (i = 1; i <= nEx; i++) {
    ...
    if (DtEnd == SRT_DF_ERROR)
      return (err);

    Ks[i - 1] =
        Payt / (MidAt->Strike[i - 1] * SwapLevel) +
        DtEnd * (1 - MidAt->Strike[i - 1]) / (MidAt->Strike[i - 1] * SwapLevel);

    SwapRate = (DtStart - DtEnd) / SwapLevel; /* CASH SWAP RATE */

    err = GetVol(tStart[i], tPay[FirstPayIndex + nCpn], SwapRate, SRT_FALSE,
                 &CEVVol);
    if (err)
      return err;
    ...
    err = srt_f_optsarbvol(SwapRate, SwapRate, YrtoExp, CEVVol, 0.0, 0.0, 0.0,
                           input_vol, SRT_NORMAL, &NORMVol);
    if (err)
      return err;
  ...
  if (tStart)
    free_lngvector(tStart, 1, nEx);
  tStart = NULL;
  if (tEx)
    free_lngvector(tEx, 1, nEx);
  tEx = NULL;

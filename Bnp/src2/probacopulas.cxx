...
/**********************************************************************************
   ...
   In order to estimate the empirical Copula we compute
                                     ...
                                     number of pairs (x        ,y) in the sample
such as x<=x(i) and y<=y(i) Cn =
---------------------------------------------------------------
                                                                                                n

***********************************************************************************/

double EmpiricalCopula(double **InitialDatas, double SortedData1,
                       double SortedData2, long index1, long index2,
                       long nDates) {
  long i;
  double result;
  ...
  result = 0.0;
  for (i = 0; i < nDates; i++) {
    if ((InitialDatas[index1][i] <= SortedData1) &&
        (InitialDatas[index2][i] <= SortedData2))
      result += 1.0;
  }
...
/**********************************************************************************
   ...
   In order to estimate the empirical Copula frequency we compute
                                     ...
                                     1/n if (x(i)        ,y(i)) is an element of
the sample cn = 0 otherwise

  Remark : One could notice that Cn(i/n        ,i/j) = sum{p        ,q} cn(p/n
,q/n)

***********************************************************************************/

double EmpiricalCopulaFrequency(double **InitialDatas, double SortedData1,
                                double SortedData2, long index1, long index2,
                                long nDates) {
  long i;
  double result;
...
double SpearmansRho(double **InitialDatas, double **SortedDatas, long index1,
                    long index2, long nDates) {
  long i, j;
  double result;
  double EmpCopul;
  ...
  for (i = 0; i < nDates; i++) {
    for (j = 0; j < nDates; j++) {
      EmpCopul =
          EmpiricalCopula(InitialDatas, SortedDatas[index1][i],
                          SortedDatas[index2][j], index1, index2, nDates);
                                   ...
                                   assumed that it is [0..nDim][0..nDim] */
{
  ...
  for (i = 0; i < nDim; i++) {
    ...
    for (j = i + 1; j < nDim; j++) {
      dTempCovar = SpearmansRho(InitialDataAndIndex, SortedDataAndIndex, i, j,
                                nNumDates);

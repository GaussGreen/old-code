...
static Err check_string_arg(COMLL_PTR args, COMLLType type)
{
    ...
    {
        if ((args->type != COMLL_STRING) || (error = interp_call_put(args->sval, &callputtype)))
        {
...
static Err check_cell_coords(GrfnDeal* gd, int* detargs, String name)
{
    ...
    if ((strcmp(name, "ROWSORT") == 0) || (strcmp(name, "ROWINTERP") == 0))
    {
        ...
        if (0 <= r && r <= gd->I && c1 >= 0 && c2 >= c1 && c2 < gd->J && ind <= (c2 - c1) &&
            ind >= 0)
            return NULL;

        /* If there is a reference to the future, mention it */
        if ((r > gd->I) || ((r == gd->I) && (c2 >= gd->J)))
            return serror("%s: %s[%d,%d,%d,%d]", GRERR_ILLEGAL_REF, name, r, c1, c2, ind);

        /* If the arguments are out of the tableau range, mention it */
        if (r < 0 || r > gd->sslength - 1 || c1 < 0 || c1 > gd->sswidth - 1 || c2 < 0 ||
            c2 > gd->sswidth - 1)
            return serror("%s: %s[%d,%d,%d,%d]", GRERR_REF_OUT_OF_RANGE, name, r, c1, c2, ind);

        /* If the second column reference is before the first one */
        if (c2 < c1)
            return serror("%s: %s[%d,%d,%d,%d]", GRERR_COL_OUT_OF_RANGE, name, r, c1, c2, ind);

        /* If the index required is not within the number of columns on which to sort, mention it */
        if ((ind < 0) || (ind > (c2 - c1)))
            return serror("%s: %s[%d,%d,%d,%d]", GRERR_INDEX_OUT_OF_RANGE, name, r, c1, c2, ind);
    }
    else if (
        (strcmp(name, "ROWMIN") == 0) || (strcmp(name, "ROWMAX") == 0) ||
        (strcmp(name, "ROWSUM") == 0) || (strcmp(name, "ROWAVG") == 0) ||
        (strcmp(name, "ROWMAXIDX") == 0) || (strcmp(name, "ROWMINIDX") == 0))
    {
        /* Gets the parameters of the ROWSORT function */
        r  = detargs[0];
        c1 = detargs[1];
        ...
        if (r < 0 || r > gd->sslength - 1 || c1 < 0 || c1 > gd->sswidth - 1 || c2 < 0 ||
            c2 > gd->sswidth - 1)
            return serror("%s: %s[%d,%d,%d]", GRERR_REF_OUT_OF_RANGE, name, r, c1, c2);

        /* If the second column reference is before the first one */
        if (c2 < c1)
            return serror("%s: %s[%d,%d,%d]", GRERR_COL_OUT_OF_RANGE, name, r, c1, c2);
    }
    else /* perform check on COLXXX functions */
    {
        ...
        if (c < 0 || c > gd->sswidth - 1 || r1 < 0 || r1 > gd->sslength - 1 || r2 < 0 ||
            r2 > gd->sslength - 1)
            return serror("%s: %s[%d,%d,%d]", GRERR_REF_OUT_OF_RANGE, name, c, r1, r2);

        /* If the second row reference is before the first one */
        if (r2 < r1)
            return serror("%s: %s[%d,%d,%d,%d]", GRERR_ROW_OUT_OF_RANGE, name, c, r1, r2);
...

{
    ...
    if (args->nargs != gs.ndargs + gs.nrargs)
        return serror(
            "%s %s %d, requires %d args",
            name,
            GRERR_WRONG_NUM_ARGS,
            args->nargs,
            gs.ndargs + gs.nrargs);
    ...
    case CLLNCellFunc:
        ...
        {
            /* Checks the auxiliary range has the right length for interpolation (and that ind is
             * not outside of range) */
            CheckAuxRange1(ind, gd);
            if (gd->auxlen[ind] != (r2 - r1))
                serror("%s: lengths %d,%d", GRERR_INTERP_SAME_SIZE, gd->auxlen[ind], (r2 - r1));

            /* For ROWINTERP, the value to interpolate (x) is not deterministic : need a TWO nodes
             * COMLL */
            ...
            /* The (complex) argument at the top of the list is x (args points on the last argument
             * of "x") */
    ...
    case CLLNCellRef:
        ...
        if (cindex < 0 || cindex > gd->sswidth - 1 || rindex < 0 || rindex > gd->sslength - 1)
...
*   FUNCTION        :   grfn_interp_func
    ...
    DESCRIPTION     :   Interprets f(...) where f is a reserved GRFN function
                                                Only the name of the function is passed, as well as
the COMLL that corresponds only to the ARGUMENTS of the function No specific COMLL element is yet
built for the function name

                                                The COMLL_PTR args returned ALWAYS points to the top
of the COMLL corresponding to this function (not to break the full COMLL construction in the YACC
parsing) This top correponds to the LAST argument input to the function (see the Grfn_Yacc.y file:
reverse building)


                                                It also performs error checking on its arguments,
and modifies its arguments  appropriately.  If the result of the function can be computed now, this
is done.
    ...
    PROBLEM         :   In order to pre-compute results when arguments are
                        ...
                        evaluation of the "same" function in different places.
                                                ...
                                                Function called by the YACC parser when encountering
a NAME(...) type of input
    ...
    case CLLNAuxFunc:
        ...
        { /* Interpolate Function */
            ...
            /* PVInterp(AuxIndex,StartCol,ColSkip,x).  The first three arguments are
               deterministic, but x may not be. Hence the deterministic arguments will be in the
               last be the last three nodes of args; there may be any number of nodes above
               representing x */
    ...
    case CLLNArithFunc:
        ...
        if (numenddetargs == gs.ndargs + gs.nrargs)
        {
            ...
            case COMLL_A_SPREADNUM:
                ...
                error = OptSpreadNew(fwdx, a, sigx, fwdy, -b, sigy, k, t, rho, call_put, &x);

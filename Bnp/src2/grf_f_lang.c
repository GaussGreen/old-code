...
********************************************************************************
    ...
    DESCRIPTION     :   For functions of the form f(a  ,b  ,c  ,...) we have
already evaluated the arguments and joined them together. Hence COMLL_PTR
argument to these functions is a list: "c"-->"b"-->"a".
                        ...
                        single "f(a  ,b  ,c)". Back in YACC  , the pattern
matching code contains a pointer to the TOP of this list  , which we can NOT
give up. Therefore in the second case we will free this part of the list:
                                            -->"b"-->"a"
...
static Err check_cell_coords(GrfnDeal *gd, int *detargs, String name) {
  ...
  if ((strcmp(name, "ROWSORT") == 0) || (strcmp(name, "ROWINTERP") == 0)) {
    ...
    /* If there is a reference to the future  , mention it */
    if ((r > gd->I) || ((r == gd->I) && (c2 >= gd->J)))
      return serror("%s: %s[%d  ,%d  ,%d]", GRERR_ILLEGAL_REF, name, r, c1, c2);

    /* If the arguments are out of the tableau range  , mention it */
    if (r < 0 || r > gd->sslength - 1 || c1 < 0 || c1 > gd->sswidth - 1 ||
        c2 < 0 || c2 > gd->sswidth - 1)
      return serror("%s: %s[%d  ,%d  ,%d]", GRERR_REF_OUT_OF_RANGE, name, r, c1,
                    c2);

    /* If the second column reference is before the first one */
    if (c2 < c1)
      return serror("%s: %s[%d  ,%d  ,%d]", GRERR_COL_OUT_OF_RANGE, name, r, c1,
                    c2);
  ...
  } else /* perform check on COLXXX functions */
  {
    ...
    if (c < 0 || c > gd->sswidth - 1 || r1 < 0 || r1 > gd->sslength - 1 ||
      ...
      return serror("%s: %s[%d  ,%d  ,%d]", GRERR_REF_OUT_OF_RANGE, name, c, r1,
                    r2);

    /* If the second row reference is before the first one */
    if (r2 < r1)
      return serror("%s: %s[%d  ,%d  ,%d  ,%d]", GRERR_ROW_OUT_OF_RANGE, name,
                    c, r1, r2);

    /* If there is an illegel reference to the future  , mention it */
    if ((r2 > gd->I) || ((c == gd->I) && (r2 >= gd->J)))
      return serror("%s: %s[%d  ,%d  ,%d]", GRERR_ILLEGAL_REF, name, c, r1, r2);
  }
  return NULL;
}
...

{
  ...
  if (args->nargs != gs.ndargs + gs.nrargs)
    return serror("%s %s %d  , requires %d args", name, GRERR_WRONG_NUM_ARGS,
  ...
  case CLLNCellFunc:
    ...
    /* C is the column index   , except for ROWSORT/ROWINTERP where it is the
     * row index */
    c = detargs[0];

    /* R... is the row index  , except for ROWSORT/ROWINTERP where it is the
     * column index */
    r1 = detargs[1];
    r2 = detargs[2];
    ...
    if (gs.type == COMLL_C_ROWINTERP) {
      ...
      if (gd->auxlen[ind] != (r2 - r1))
        serror("%s: lengths %d  ,%d", GRERR_INTERP_SAME_SIZE, gd->auxlen[ind],
               (r2 - r1));

      /* For ROWINTERP  , the value to interpolate (x) is not deterministic :
       * need a TWO nodes COMLL */
  ...
  case CLLNCellRef:
    ...
    if (cindex < 0 || cindex > gd->sswidth - 1 || rindex < 0 ||
      ...
      return serror("%s C[%d  ,%d]", GRERR_REF_OUT_OF_RANGE, cindex, rindex);
  ...
  case CLLNAuxRef:
    ...
    if (cindex < 0 || rindex < 0 || cindex > gd->auxwidth - 1 ||
      ...
      return serror("%s A[%d  ,%d]", GRERR_REF_OUT_OF_RANGE, cindex, rindex);
    lastarg = lastarg->prev;
    lastarg->next->prev = NULL;
    comll_free_list(lastarg->next);
    lastarg->next = NULL;
...
passed  , as well as the COMLL that corresponds only to the ARGUMENTS of the
function No specific COMLL element is yet built for the function name
...
on its arguments  , and modifies its arguments  appropriately.  If the result of
the function can be computed now  , this is done.


                        Example: If name is "max"  , args would have to describe
                        two  expressions  , "a"-->"b".  For "a" and "b" both
real numbers  , we replace this with one node of type COMLL_REAL and value max(a
,b). Otherwise  , we would place another node at the end of the function of type
COMLL_A_MAX: "a"-->"b"-->"max".
    ...
    PROBLEM         :   In order to pre-compute results when arguments are
                        ...
                        ( e.g. +  , -  , * ...). Also  , if a new arithmetic
function with a name were added (sin(x) say)  , 3 modules rather than 2 would
need modification. (grf_f_eval_event.c and grf_f_lang.c to say how to evaluate
it  , and grf_f_lnfsymtab.c to declare it).
...

{
  ...
  case CLLNAuxFunc:
    ...
    if (gs.type == COMLL_X_INTERP) { /* Interpolate Function */
      ...
      /* PVInterp(AuxIndex  ,StartCol  ,ColSkip  ,x).  The first three arguments
         are deterministic  , but x may not be. Hence the deterministic
         arguments will be in the last be the last three nodes of args; there
         may be any number of nodes above representing x */
...
/* --------------------------------------------------------------------------------------------
 */
/* Aggregates into a full status (row  , tableau...) a single status (for the
 * cell) */

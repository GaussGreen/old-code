	...
	for (k=0; k<nb_dates-1; k++)
	{
		...
		for (i = StartIndex; i < EndIndex + 1; i++)
		{
			...
			adj_fx_pay += correl_dom_fx * sig_dom * sig_fx * (Etha_Func (lda_dom, end_mat, T1, T2)
					   ...
					   - correl_dom_for * sig_domfor * (Psi_Func (lda_for, lda_dom, end_mat, T1, T2) 
...
Err		err = NULL;
	...
	for (k=0; k<nb_dates-1; k++)
	{
		...
		for (i = StartIndex; i < EndIndex + 1; i++)
		{
			...
			adj_pay_for += correl_dom_for * sig_domfor * (x_for - exp(-lda_dom * mat_pay) * x_domfor);
			...
			/* domestic / fx */
			correl13_1 += sig_dom2 * (x_dom - exp(-lda_dom * end_mat) * y_dom);
			correl13_2 += correl_dom_for * sig_domfor * (x_dom - exp(-lda_for * end_mat) * x_domfor);
			correl13_3 += correl_dom_fx * sig_dom * sig_fx * x_dom;

			/* foreign fx */
			correl23_1 += sig_for2 * (x_for - exp(-lda_for * end_mat) * y_for);
			correl23_2 += correl_dom_for * sig_domfor * (x_for - exp(-lda_dom * end_mat) * x_domfor);
		...
		for_fwd[k+1] = 1.0 / lda_for * expect_for +
					   exp(-lda_for * end_mat) * 
					   (-adj_pay_for / lda_dom -adj_quanto)
					   + for_phi[k]  * exp(-lda_for * mat) 
					    * Phi_Func(-lda_for, start_mat, start_mat, end_mat);

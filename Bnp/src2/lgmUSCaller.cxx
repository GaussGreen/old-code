...
LGMErr GetSABRParam(long dstart, long dend, char *vol_id,
                    SABR_VOL_TYPE component, double *result) {
  LGMErr err = NULL;
  int icomponent;
  double strike = 0.05;
  double power;
...
static struct {
  LGMErr (*GetVol)(long, long, double, SRT_Boolean,
                   double *); /* A cash volatility function */
  char *YieldCurveName;
  long lSpotLag;
...
LGMErr SetUpStaticMarketIfo(LGMErr (*GetVol)(long, long, double, SRT_Boolean,
                                             double *),
                            char *YCName, double dMaxStrikeStd,
                            char *char_vol_type) {
  ...
  StaticMarketInfo.CalcDate =
      get_clcndate_from_yldcrv(StaticMarketInfo.YieldCurve);
  ...
  return LGMCcyDefaults(StaticMarketInfo.YieldCurve,
                        &StaticMarketInfo.Conventions);
}

/* Wrapper to the input volatility function that cuts the volatility at +/-
 * StdDev above and below the ATM */
/* Only affects the LGM calibration */
LGMErr GetTruncVol(long lStart, long lEnd, double dStrike, SRT_Boolean bUnknown,
                   double *pdVol) {
  /* local variables */
  SwapDP swap_dp;
  ...
  /* Set up the swap DP */
  if (err = swp_f_setSwapDP(lStart, lEnd, StaticMarketInfo.Conventions.sfreq,
                            StaticMarketInfo.Conventions.sbasis, &swap_dp))
    return err;
  swap_dp.spot_lag = StaticMarketInfo.Conventions.lag;

  /* Calculate the CASH forward swap rate */
  if (err = swp_f_ForwardRate_SwapDP(&swap_dp, StaticMarketInfo.YieldCurveName,
                                     "CASH", &dForward))
    return err;
  ...
  if (!StaticMarketInfo.IsLognormal)
    if (err = srt_f_optbetavoltoblkvol(dForward, dForward, dATMvol, dExpiry,
                                       0.0, &dATMvol))
      return err;
...
*/
    ...
    double *tgear_floatdigital, /*[0        ,1        ,...        ,nCpn-1]
                             gearings for floating digitals */
    ...
    double *tFundingPayment, /* [0        ,1        ,...        ,nCpn-1] payment
                          of the floating leg */
    ...
    int calibrationmethod, /* 1 = Swaptions ATM        , 2 = Swaptions Strike eq
                        , 3 = Caplets ATM  */
    int timeswapvolmethod, /* 1 = Model        , 2 = Sliding        , 3 =
                              Converging  */
    int atmvolmethod,      /* 1 = Lognormal        , 2 = Normal        , 3 =
                              SigmaBeta */
  ...
  int convertTS = 1; /* 1 = cmpute new sigmas        , taus & store in ts; 0 =
                        don't bother */
  int findExerBdry =
      1; /* 1 = find swap rates at exercise boundary; 0 = don't bother */
  ...
  *FixedLeg = 0.0;
  ...
  // Check to see if the option is exercised        , at which point we can
  ...
  if (isExercised) {
    ...
    while (iLastStart < nCpn && tCpnStart[iLastStart] < tNow)
      iLastStart++;
    // If the deal is exercised before the first start        , then there is
    // nothing to do
    if (iLastStart == 0) {
      return 0;
    }
    ...
    return calcRangeAccrual(
        lPayDate,
        cpn[iLastStart - 1] +
            tgear_floatdigital[iLastStart - 1] * dFloatCouponFixing,
        eod_fix_flag, iAccrueOnBarrier, nDaysInFirstCoupon,
        lvFirstCouponFixingDates, dvFirstCouponAccrualWeights,
        dvFirstCouponFixingOrSpread, barriers[iLastStart - 1][1],
        barriers[iLastStart - 1][0], call_spread_up, call_spread_low,
        szIndexRefRate, cvgCpn[iLastStart - 1], FixedLeg);
  }
  ...
  if (nDaysInFirstCoupon > 0) {
    ...
    if (iLastStart == 0)
      *FixedLeg = 0.0;
    else if (error = calcRangeAccrual(
                 lPayDate,
                 cpn[iLastStart - 1] +
                     tgear_floatdigital[iLastStart - 1] * dFloatCouponFixing,
                 eod_fix_flag, iAccrueOnBarrier, nDaysInFirstCoupon,
                 ...
                 dvFirstCouponFixingOrSpread, barriers[iLastStart - 1][1],
                 barriers[iLastStart - 1][0], call_spread_up, call_spread_low,
                 szIndexRefRate, cvgCpn[iLastStart - 1], &dStubPV))
      return error;
  ...
  //		tNow = get_clcndate_from_yldcrv (yldcrv);    /* get calculation
  //date
  //*/

  /* Get the exponent beta to use to interpret market vols. Useful only to cope
   * with the generica autocal code */
  ...
  CalReqPtr = LGMSetCalibMeth(1, usefixtau, usecaps, tau, 0, 0, 0, maxstd, NULL,
                              NULL, NULL, NULL, NULL);
  ...
  error = LGMFillCallableTimeSwap(
      ...
      &nExLeft, tfirst, ycName, nCpn, tCpnStart, tCpnPay, cpn,
      tgear_floatdigital, cvgCpn, tFundingPayment, nEx, tEx, tStart, exerFee,
      call_spread_up, call_spread_low, observation_freq, barriers, freqIndex,
      freqAccrued, CorrelStart, CorrelEnd, srt_p_r_flag, timeswapvolmethod,
      atmvolmethod, calibrationmethod, num_subdiscret_obsfreq, shiftvol, nx,
      nofunding, vcName, GetSABRParam, 1, eod_pay_flag, eod_fix_flag);
  ...
  if (!(error = LGMnewautocal(
            ...
            tNow, eod_ex_flag, ycName,           /* info about today's market */
            GetTruncVol, GetBeta, RecVol, /* swaption & caplet price info */
            dealtype, (void *)dealPtr,    /* the deal */
            CalReqPtr, EvalParamsPtr,     /* calib & eval methods */
            &LGMVal, &intrinsic_value,    /* value & intrinsic value of deal */
            &ExerIntoPtr, &ExerBdryPtr, /* swaptions representing the underlying
                               & exer boundary */
    ...
    *FixedLeg = pv_fixedleg;
    ...
    /* to check pricing of forward time swap and the strikes used for the
     * calibration*/
    for (j = 0; j < nEx; j++) {
...
//
/* Create two structures        , one for the option to call        , and one
 * for the time swap itself  */
  ...
  *nExleftPtr = nExEff; /* Number of exercises after today */
  ...
  if (iFirstTS < iFirst) /* then we create a time swap deal */
  {
    ...
    for (i = iFirstTS; i < iFirst; i++) {
      ...
      numdays_in_period =
          tPayCpn[i] - tStartCpn[i]; /* Assumes paydate = enddate */
      for (j = 1; j <= observation_freq; j++) {
        /* for each subperiod ... */
        ptrTS->observationdays[i - iFirstTS][j - 1][0] =
            (int)(tStartCpn[i] + (j - 1) * (numdays_in_period + 0.0) /
                                     (observation_freq - 1.0)); /* Start Date */
        ptrTS->observationdays[i - iFirstTS][j - 1][1] =
            add_unit(ptrTS->observationdays[i - iFirstTS][j - 1][0],
                     ptrTS->undIndex, SRT_MONTH, NO_BUSDAY_CONVENTION);
        ptrTS->observationdays[i - iFirstTS][j - 1][1] =
            add_unit(ptrTS->observationdays[i - iFirstTS][j - 1][1], 0,
        ...
        for (j = 1; j <= observation_freq; j++) {
          if (j < observation_freq) {
            ptrTS->ratiodays_for_subperiod[i - iFirstTS][j - 1] =
                (double)(ptrTS->observationdays[i - iFirstTS][j][0] -
                         ptrTS->observationdays[i - iFirstTS][j - 1][0] + 0.0) /
                (numdays_in_period + 0.0);
          } else {
            ptrTS->ratiodays_for_subperiod[i - iFirstTS][observation_freq - 1] =
                (double)(tPayCpn[i] -
                         (ptrTS->observationdays)[i - iFirstTS]
                                                 [observation_freq - 1][0] +
                         0.0) /
                (numdays_in_period + 0.0);
          }
        }
  ...
  /* Step 3: Allocate Callable Time Swap with nExEff exer dates and n-i cpn
   * periods */
  ptr = LGMCreateCallableTimeSwap(nExEff, n - iFirst, observation_freq);
  /*	ptrTS = LGMCreateCallableTimeSwap(1        ,n-iFirstTS
   * ,observation_freq); */
  ...
  for (i = iFirst; i < n; i++) {
    ...
    for (j = 1; j <= observation_freq; j++) {
      ...
      ptr->observationdays[i - iFirst][j - 1][0] =
          (int)(first_future_fixing +
                (j - 1) * (remaining_numdays_in_period + 0.0) /
                    (observation_freq + 0.0)); /* Start Date */
      ptr->observationdays[i - iFirst][j - 1][1] =
          add_unit(ptr->observationdays[i - iFirst][j - 1][0], ptr->undIndex,
                   SRT_MONTH, NO_BUSDAY_CONVENTION);
      ptr->observationdays[i - iFirst][j - 1][1] =
          add_unit(ptr->observationdays[i - iFirst][j - 1][1], 0, SRT_BDAY,
                   MODIFIED_SUCCEEDING);
    }
    if (observation_freq == 2) /* HARD CODE FOR THE PERIODIC CAP */
    {
      ...
      for (j = 1; j <= observation_freq; j++) {
        if (j < observation_freq) {
          ptr->ratiodays_for_subperiod[i - iFirst][j - 1] =
              (double)(ptr->observationdays[i - iFirst][j][0] -
                       ptr->observationdays[i - iFirst][j - 1][0] + 0.0) /
              (numdays_in_period + 0.0);
        } else {
          ptr->ratiodays_for_subperiod[i - iFirst][observation_freq - 1] =
              (double)(tPayCpn[i] -
                       (ptr->observationdays)[i - iFirst][observation_freq - 1]
                                             [0] +
                       0.0) /
              (numdays_in_period + 0.0);
        }
...
static int IsAccrued(int iAccrueOnBarrier, double dFixing, double dUpBarrier,
...
static char *BS(long lFixing, double dStrike, char *szRefRate, double dSpread,
                SrtCallPutType srtCallPut, double *out_pdPrice) {
  ...
  lStart = add_unit(lFixing, StaticMarketInfo.Conventions.lag, SRT_BDAY,
                    StaticMarketInfo.Conventions.cxxbdconv);
  lEnd = add_unit(lStart, (int)(12 / Comp), SRT_MONTH,
                  StaticMarketInfo.Conventions.cxxbdconv);

  // Get the forward
  dFwd = swp_f_swapcashrate(lStart, lEnd, Basis, Comp,
                            StaticMarketInfo.YieldCurveName, szRefRate);

  // If the expiry is 0        , return the intrinsic value
  if (dExpiry == 0.0) {
    *out_pdPrice = srt_f_optblksch(dFwd, dStrike - dSpread, 0.0, dExpiry, 1.0,
                                   srtCallPut, PREMIUM);
    return 0;
  }
  ...
  // Price the option
  if (StaticMarketInfo.IsLognormal)
    *out_pdPrice = srt_f_optblksch(dFwd, dStrike - dSpread, dVol, dExpiry, 1.0,
                                   srtCallPut, PREMIUM);
  else
    *out_pdPrice = srt_f_optblknrm(dFwd, dStrike - dSpread, dVol, dExpiry, 1.0,
                                   srtCallPut, PREMIUM);
...
static char *putSpread(long lFixing, double dBarrier, double dSpread,
                       char *szRefRate, double dPutSpread, double *out_pdProb) {
  ...
  if (err = BS(lFixing, dBarrier + dPutSpread, szRefRate, dSpread, SRT_PUT,
               &dUpPV))
    return err;
...
static char *getSinglePV(long lFixing, double dUpperBarrier,
                         double dLowerBarrier, double dUpperSpread,
                         double dLowerSpread, char *szRefRate, double dSpread,
                         double *out_pdProb) {
  // Variable declarations
  ...
  if (err = putSpread(lFixing, dUpperBarrier, dSpread, szRefRate, dUpperSpread,
                      &dUpperProb))
    return err;

  if (err = putSpread(lFixing, dLowerBarrier, dSpread, szRefRate, dLowerSpread,
                      &dLowerProb))
    return err;
...
LGMErr calcRangeAccrual(long lPayDate, double dCoupon, int iEODFixFlag,
                        ...
                        double *dvFixingOrSpread, double dUpperBarrier,
                        double dLowerBarrier, double dUpperSpread,
  ...
  while (i < nDaysInCoupon && lvFixingDates[i] < lToday) {
    if (IsAccrued(iAccrueOnBarrier, dvFixingOrSpread[i], dUpperBarrier,
                  dLowerBarrier))
  ...
  while (i < nDaysInCoupon) {
    if (err = getSinglePV(lvFixingDates[i], dUpperBarrier, dLowerBarrier,
                          dUpperSpread, dLowerSpread, szRefRate,
                          dvFixingOrSpread[i], &dProb))
      return err;
  ...
  *out_pdFixed = dCvg * dAccrual * dCoupon *
                 swp_f_df(lToday, lPayDate, StaticMarketInfo.YieldCurveName) /
                 dTotalDays;
...
static LGMErr (*STATIC_ptrFixingGetVol)(Date, Date, Date, double, SRT_Boolean,
                                        double *);
static int STATIC_lag;
static SrtBusDayConv STATIC_BusDayConv;

/* Conversion routine between 2 date and 3 date vol function */
static LGMErr STATIC_GetVol(Date lStartDate, Date lEndDate, double dStrike,
                            SRT_Boolean b, double *Vol) {
  /* Calculate the fixing date */
  Date lFixingDate;
  lFixingDate = add_unit(lStartDate, -STATIC_lag, SRT_BDAY, STATIC_BusDayConv);
  return (*STATIC_ptrFixingGetVol)(lFixingDate, lStartDate, lEndDate, dStrike,
                                   b, Vol);
}

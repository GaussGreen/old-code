...
#include "utdates.h"
//**********************//
// All rates and yields        , as input output        , will be quoted in % ,
// i.e.        , 1%
// <-> 1
//*********************//

double VolFactor(double a, double t) {
  double x;
...
// assume completely unitialized data        , in particular        , pointers
// are all NULL units have to be days first        , then months        , in
// each case the terms must be increasing if NbMMTerms > 0 but date_units ,
// MMTerms are NULL        , still alloc memory but don't copy
char *MM_Data_Construct(MM_Data *mm_data, YBASIS YearBase, DIFFBASIS diffBase,
                        int delay, int NbMMTerms, DATE_UNIT *date_units,
                        int *MMTerms, double *MMYields, long Ref_Date) {
  int i;
  int hasFoundMo = 0;
  int prev = 0;
  // check: have to be days first        , then months        , in each the
  // terms must be increasing
  if (YearBase == _YACT)
    return ("Can't handle ACT for MM");
  for (i = 0; i < NbMMTerms; ++i) {
    if (date_units[i] != DAY && date_units[i] != MONTH)
      return ("Wrong MM date units");
    if (hasFoundMo) {
      if (date_units[i] == DAY)
        return ("MM units has to be DAY followed by MONTH");
      if (MMTerms[i] <= prev)
        return ("MMTerms have to be increasing");
    } else {
  ...
  if (date_units) {
    for (i = 0; i < NbMMTerms; ++i)
      mm_data->date_units[i] = date_units[i];
  }
  if (MMTerms) {
    for (i = 0; i < NbMMTerms; ++i)
      mm_data->MMTerms[i] = MMTerms[i];
  }
  if (MMYields) {
    for (i = 0; i < NbMMTerms; ++i)
      mm_data->MMYields[i] = MMYields[i];
  }
...
char *MM_Data_Copy(MM_Data *new_data, MM_Data *old_data) {
  return (MM_Data_Construct(new_data, old_data->YearBase, old_data->diffBase,
                            old_data->delay, old_data->NbMMTerms,
                            old_data->date_units, old_data->MMTerms,
                            old_data->MMYields, old_data->Ref_Date));
};
...
// assume completely unitialized data        , in particular        , pointers
// are all NULL if NbMMTerms > 0 but date_units        , MMTerms are NULL ,
// still alloc memory but don't copy
char *Zero_Data_Construct(Zero_Data *zero_data, int frequency,
                          YBASIS YBase, /*CBASIS CvgBase        ,*/
                          int NbTimePoints, long *dates, double *DayPoints,
                          double *Rates, long Ref_Date) {
  int i;
  double prev = 0.0;
  // check
  if (YBase == _YACT)
    return ("Can't handle ACT for zero rates");
  if (dates) {
    for (i = 0; i < NbTimePoints; ++i) {
      if (DayPoints[i] <= prev)
        return ("Zero_Data timepoints must be increasing");
      prev = DayPoints[i];
    }
  }
  // fill:
...
char *Zero_Data_Copy(Zero_Data *new_data, Zero_Data *old_data) {
  return (Zero_Data_Construct(new_data, old_data->Frequency, old_data->YearBase,
                              old_data->NbTimePoints, old_data->dates,
                              old_data->DayPoints, old_data->Rates,
                              old_data->Ref_Date));
} // Zero_Data_Copy

char *MBSDF_Data_Construct_FromZeroData(MBSDF_Data *df_data,
                                        Zero_Data *zero_data) {
  char *err = 0;
  if (zero_data == 0)
    return ("Invalid zero_data");
  MBSDF_Data_Destruct(df_data);
  df_data->market[0] = '\0';
  df_data->use_zero_data = 1;
  if (err = Zero_Data_Copy(&(df_data->zero_data), zero_data))
    return (err);
  return (err);
...
char *MBSDF_Data_Construct_FromName(MBSDF_Data *df_data, char *mkt) {
  ...
  if (strlen(mkt) >= MAXSTRBUFSZ)
    return ("market name too long");
...
char *MBSDF_Data_Copy(MBSDF_Data *new_struct, MBSDF_Data *old_struct) {
  ...
  if (old_struct->use_zero_data) {
    if (err = MBSDF_Data_Construct_FromZeroData(new_struct,
                                                &(old_struct->zero_data)))
      return (err);
  } else {
...
char *Yield_Data_Construct(Yield_Data *yield_data, YBASIS YearBase,
                           DIFFBASIS diffBase, int Delay, int Frequency,
                           int NbTimePoints, DATE_UNIT *date_units,
                           int *yield_terms, double *Yields, long Ref_Date) {
  int i;
  int prev = 0;
  // convert and check
  if (date_units && yield_terms) {
    for (i = 0; i < NbTimePoints; ++i) {
      if (date_units[i] != MONTH && date_units[i] != YEAR)
        return ("Ilegal yield date units");
      if (date_units[i] == YEAR) {
        date_units[i] = MONTH;
        yield_terms[i] *= 12;
      }
      if (yield_terms[i] <= prev)
        return ("Yield terms must be increasing");
      prev = yield_terms[i];
    }
  }
  // fill
  ...
  if (date_units) {
    for (i = 0; i < NbTimePoints; ++i)
      yield_data->date_units[i] = date_units[i];
  }
  if (yield_terms) {
    for (i = 0; i < NbTimePoints; ++i)
      yield_data->yield_terms[i] = yield_terms[i];
  }
  if (Yields) {
    for (i = 0; i < NbTimePoints; ++i)
      yield_data->Yields[i] = Yields[i];
  }
  return (0);
...
void Yield_Data_Destruct(Yield_Data *yield_data) {
  if (yield_data->NbTimePoints > 0) {
    mbspt_free_DateUnitVector(yield_data->date_units, 0,
                              yield_data->NbTimePoints - 1);
    mbspt_free_ivector(yield_data->yield_terms, 0,
                       yield_data->NbTimePoints - 1);
...
char *Yield_Data_Copy(Yield_Data *new_data, Yield_Data *old_data) {
  return (Yield_Data_Construct(
      new_data, old_data->YearBase, old_data->diffBase, old_data->Delay,
      old_data->Frequency, old_data->NbTimePoints, old_data->date_units,
      old_data->yield_terms, old_data->Yields, old_data->Ref_Date));
} // Yield_Data_Copy

// using basic info from Yield_Data such as basis and frequency        , and
// knowing the time to maturity generate the sets of start dates        , end
// dates        , and coverages starts        , ends        , and cgs are
// memory-alloc here!!
char *yield_date_gen(int *nb, long **starts, long **ends, double **cvgs,
  ...
  // check
  if (date_unit != MONTH && date_unit != YEAR)
    return ("Illegal date_unit");
  //
...
#ifdef MBSPTCORRECTDAYCT
    if (yield_data->YearBase == _YACT)
      (*cvgs)[i] = 1.0 / (double)yield_data->Frequency;
    else
      (*cvgs)[i] =
          ((double)DateDiff((*starts)[i], (*ends)[i], yield_data->diffBase)) /
          (double)yield_data->YearBase;
#else
    (*cvgs)[i] = 1.0 / (double)yield_data->Frequency;
#endif // MBSPTCORRECTDAYCT
  }
...
// use df implicit in zero_data and basis/frequency spec in yield_data to calc
// YTM yield 1% <-> 0.01
double calc_yield(Zero_Data *zero_data, Yield_Data *yield_data,
                  DATE_UNIT date_unit, int term) {
...
// using basic info such as basis to calc the start and maturity dates of MM
// rate        , also cvg memory to be alloc by user        , start is a single
// date
char *mm_yield_date_gen(long *start, long **ends, double **cvgs,
                        MM_Data *mm_data) {
  int i;
  ...
  *start = Date_Add(mm_data->Ref_Date, DAY, mm_data->delay, 1, 0);
  for (i = 0; i < mm_data->NbMMTerms; ++i) {
    ...
    if (mm_data->date_units[i] == DAY)
      (*ends)[i] =
          Date_Add(*start, mm_data->date_units[i], mm_data->MMTerms[i], 1, 0);
    else {
      (*ends)[i] =
          Date_Add(*start, mm_data->date_units[i], mm_data->MMTerms[i], 0, 1);
      if (!isBusDay((*ends)[i])) {
        Dsplit((*ends)[i], &y, &m, &d);
    ...
    (*cvgs)[i] = DateDiff(*start, (*ends)[i], mm_data->diffBase) /
                 (double)mm_data->YearBase;
  }
  return (0);
} // mm_yield_date_gen
...
char *MMYields_To_ZeroRates(Zero_Data *zero_data, MM_Data *mm_data,
  ...
  if (err = Zero_Data_Construct(zero_data, frequency, YearBase,
                                mm_data->NbMMTerms, ends, day_points, 0,
                                mm_data->Ref_Date))
    return (err);
  // dfs:
  df0 = 1.0 / (1.0 + mm_data->MMYields[0] / 100.0 *
                         ((double)DateDiff(zero_data->Ref_Date, start,
                                           mm_data->diffBase)) /
                         ((double)mm_data->YearBase));
  ...
  for (i = 0; i < zero_data->NbTimePoints; ++i) {
    ...
    zero_data->Rates[i] =
        DFToZR(df * df0, zero_data->TimePoints[i], zero_data->Frequency);
  }
...
double zero_rate_from_t(Zero_Data *zero_data, double t, int extrapolate) {
  return (linear_interp(zero_data->NbTimePoints, zero_data->TimePoints,
                        zero_data->Rates, t, extrapolate));
} // zero_rate_from_t

double zero_rate(Zero_Data *zero_data, long date, int extrapolate) {
  return (
      zero_rate_from_t(zero_data,
                       (double)(DateDiff(zero_data->Ref_Date, date, _DACT)) /
                           (double)zero_data->YearBase,
                       extrapolate));
...
double DFToZR(double df, double t, int freq) {
  if (freq == 0)
    ...
    return ((100.0 * (double)freq) *
            (pow(df, -1.0 / t / ((double)freq)) - 1.0));
} // DFToZR
...
double DF(Zero_Data *zero_data, long date, int extrapolate) {
  double t = (double)(DateDiff(zero_data->Ref_Date, date, _DACT)) /
             (double)zero_data->YearBase;
...
double MBSDF(MBSDF_Data *df_data, long start, long end) {
  if (df_data->use_zero_data) {
    return DF(&(df_data->zero_data), end, 1) /
           DF(&(df_data->zero_data), start, 1);
  }
...
double MBSDF_FromT(MBSDF_Data *df_data, long start, double t) {
  ...
  return LogInterp((t), (((double)d1) / 365.0), (((double)d2) / 365.0), (df1),
                   (df2));
} // DF_MBSBK_FromT
...
// starting at Ref_Date + Delay + yield_data->yield_terms[0] (months)        ,
// at intervals of interp_interval        , and ending at the last yield_term if
// yeilds are provided        , just use it        , else        , use linterp
...
char *Yield_Data_Interpolate(Yield_Data *yield_data, int interp_frequency) {
  ...
  if (12 != interp_interval * interp_frequency)
    return ("Illegal interp_frequency");
  // copy old data
  ...
  for (i = 0; i < oldNb; ++i) {
    ...
    if (yield_data->date_units[i] == YEAR)
      terms[i] *= 12.0;
    yields[i] = yield_data->Yields[i];
  }
  // check
  n = yield_data->yield_terms[oldNb - 1];
  if (yield_data->date_units[oldNb - 1] == YEAR)
    n *= 12;
  if (yield_data->date_units[0] == YEAR)
    n -= yield_data->yield_terms[0] * 12;
  else
    n -= yield_data->yield_terms[0];
  if (n != (n / interp_interval) * interp_interval)
    return ("First and Last Dates do not fit with interp_frequency");
  // prepare new data
  newNb = 1 + (round(terms[oldNb - 1]) - round(terms[0])) / interp_interval;
  Yield_Data_Destruct(yield_data);
  Yield_Data_Construct(yield_data, yield_data->YearBase, yield_data->diffBase,
                       yield_data->Delay, yield_data->Frequency, newNb, 0, 0, 0,
                       yield_data->Ref_Date);
  // interpolate
  ...
  for (i = 1; i < newNb; ++i) {
    ...
    yield_data->yield_terms[i] =
        yield_data->yield_terms[i - 1] + interp_interval;
    yield_data->Yields[i] =
        linterpFlat(oldNb, terms, yields, (double)yield_data->yield_terms[i]);
  }
  // clean up
...
// k = index of zrs (zero rate) and day_points (number of days from
// yield_data->Ref_Date) to be filled in annuity        , as input        , is
// the value of an annuity at yield_data->Ref_Date        , with frequency etc.
// , as encoded in
// yield_data        , 					with last payment on
// day_points[k-1] (from Ref_Date) yield = YTM with maturity day_points[k] =
// Ref_Date + period * 12/frequency (months) zrs[k] = the implied zero-rate
// annuity = value of the annuity with last payment on day_points[k] assume k >=
// 1

void MBS_ZR_BootStrap(int k, int period, double *zrs, double *annuity,
                      double *day_points, double yield, Zero_Data *zero_data,
  ...
  double df0;
  // update day_points
  yield_date_gen(&nb, (&starts), (&ends), (&cvgs), yield_data, MONTH,
                 period * 12 / yield_data->Frequency);
...
// zero_data will be internally initialized        , including memory alloc , in
// this routine mm_data and yield_data are inputs assume that yield_data starts
// with terms  1Y        , mm_data ends with  < 1Y also assume that mm_data has
// values at terms equal to all the coupon dates of a yield of 1Y maturity
// interp_freq is the frequency of interpolation in zero-rate-space
char *MBS_Strip(Zero_Data *zero_data, MM_Data *mm_data, Yield_Data *yield_data,
                int interp_freq, int zero_cpd_freq, YBASIS zero_ybase,
                long zero_Ref_Date) {
...
// offset is used because we woudl like to switch between the (wrong) case of
// treating 1Y as MMYield AND as swap
  ...
  if (yield_data->date_units[yield_data->NbTimePoints - 1] == MONTH)
    zero_NbTimePoints /= 12;
  ...
  if (err = Zero_Data_Construct(zero_data, zero_cpd_freq, zero_ybase,
                                zero_NbTimePoints, 0, 0, 0, zero_Ref_Date))
    return (err);

  // get zero rates up to 1Y
  MMYields_To_ZeroRates(&initial_zeros, mm_data, zero_data->Frequency,
                        zero_data->YearBase);

  if (err = Yield_Data_Construct(
          &local_yield_data, yield_data->YearBase, yield_data->diffBase,
          yield_data->Delay, yield_data->Frequency,
          yield_data->NbTimePoints + 1 - offset, 0, 0, 0, yield_data->Ref_Date))
    return (err);

  if (offset == 0) // if 1Y is considered MM        , then we have to back out
                   // 1Y Swap Rate
  {
    local_yield_data.Yields[0] =
        calc_yield(&initial_zeros, &local_yield_data, MONTH, 12);
  ...
  // interpolate yields
  if (err = Yield_Data_Interpolate(&local_yield_data,
                                   local_yield_data.Frequency)) {
  ...
  if (local_yield_data.date_units[local_yield_data.NbTimePoints - 1] == YEAR)
    max_step *= 12;
  max_step /= 12;
  ...
  zrs = mbspt_dvector(
      0, max_step + offset); // allow one more for initial point when time = 0
  day_points = mbspt_dvector(0, max_step + offset);
  annuity = 0.0;
  day_points[0] = 0;

  for (i = 1; i <= local_yield_data.Frequency - offset; ++i) {
    yield_date_gen(&nb, &starts, &ends, &cvgs, &local_yield_data, MONTH,
                   (i * 12) / local_yield_data.Frequency);

    date = ends[nb - 1];
    day_points[i] = (double)DateDiff(zero_data->Ref_Date, date, _DACT);
    zrs[i] = zero_rate(&initial_zeros, date, 0);
    cvg = cvgs[nb - 1];
    t = ((double)DateDiff(zero_data->Ref_Date, date, _DACT)) /
        ((double)zero_data->YearBase);
  ...
  for (i = local_yield_data.Frequency - offset + 1; i <= max_step; ++i) {
    MBS_ZR_BootStrap(i, i, zrs, &annuity, day_points,
                     local_yield_data.Yields[i - local_yield_data.Frequency],
                     zero_data, &local_yield_data);
  }
  zrs[0] = zrs[1];
...
#ifndef MBSPTCORRECTDAYCT
    date = Date_Add(zero_data->Ref_Date, MONTH,
                    12 + (i + 1 - offset) * 12 / interp_freq, 0, 1);
#else
    date = Date_Add(
        zero_data->Ref_Date, DAY,
        (int)day_points[local_yield_data.Frequency - offset + 1 + i], 0, 0);
#endif

    zero_data->dates[i + initial_zeros.NbTimePoints] = date;
    zero_data->DayPoints[i + initial_zeros.NbTimePoints] =
        DateDiff(zero_data->Ref_Date, date, _DACT);
    zero_data->TimePoints[i + initial_zeros.NbTimePoints] =
        zero_data->DayPoints[i + initial_zeros.NbTimePoints] /
        ((double)zero_data->YearBase);
    zero_data->Rates[i + initial_zeros.NbTimePoints] =
        linterpFlat(max_step + 1, day_points, zrs,
                    zero_data->DayPoints[i + initial_zeros.NbTimePoints]);
  }
  // clean up
  Zero_Data_Destruct(&initial_zeros);
...
// does memory alloc
char *Vol_Surface_Construct(Vol_Surface *vol_surface, int NbSwaptions,
                            double *expiries, double *maturities, double *vols,
                            int Rate_Freq, VOL_TYPE vol_type, long RefDate) {
  int i;
  // check:
  if (Dateok(RefDate))
    return ("Bad RefDate for vol_surface");
  if (Rate_Freq <= 0)
    return ("Bad freq for vol_surface construction");
  if (NbSwaptions <= 0)
    return ("Empty vol_surface in construction");
...
void Vol_Surface_Destruct(Vol_Surface *vol_surface) {
  if (vol_surface->NbSwaptions > 0) {
    ...
    mbspt_free_dvector(vol_surface->maturities, 0,
                       vol_surface->NbSwaptions - 1);
...
char *Vol_Surface_Copy(Vol_Surface *new_data, Vol_Surface *old_data) {
  return (Vol_Surface_Construct(new_data, old_data->NbSwaptions,
                                old_data->expiries, old_data->maturities,
                                old_data->vols, old_data->Rate_Freq,
                                old_data->vol_type, old_data->RefDate));
} // Vol_Surface_Copy

// alloc memory: NbCol        , resets        , starts        , ends
// assume NbCol        , resets        , starts        , ends all NULL as
// input!! NbRow <-> single number = number of swaptions        , maxNbCol <->
// single number NnCol <-> an array = number of dates involved for each swaption
// each row of the matrix are a series of times        , measured from
// vol_surface->RefDate        , using ACT/365        , of each swptn
char *vol_surface_date_gen(int *NbRow, int *maxNbCol, int **NbCol,
  ...
  // check
  if (vol_surface->NbSwaptions == 0)
    return ("Vol Surface is empty!");
  if ((*NbCol) || (*resets) || (*starts) || (*ends))
    return ("Some output pointers pass NOT NULL!");
  //
  ...
  *maxNbCol = maxReset;
  ...
  for (i = 0; i < (*NbRow); ++i) {
    ...
    for (j = 0; j < (*NbCol)[i]; ++j) {
      start =
          Date_Add(vol_surface->RefDate, MONTH, expMon + j * cpnInterval, 0, 1);
      end = Date_Add(vol_surface->RefDate, MONTH,
                     expMon + (j + 1) * cpnInterval, 0, 1);
...
char *MBS_BKTree_Construct_FromFile(MBS_BKTree *tree, char *data_dir,
  ...
  MtgRate = RefiSpread;
  if (RefiSpread < 0.0)
    MtgRate = BaseIndexValue - RefiSpread;
  ////
  if (err = MBS_BKTree_Construct(tree, df_data, // zero_data        ,
                                 NbSwaptions, SwaptionsExp, swaptions_mat, vols,
                                 vol_index_freq, vol_type, RefDate, PPY,
                                 DX_COEFF, beta, OAS, XLimit_Coeff, NSigma,
...
//==1 if we only cach the (trivial) pv (==1.0)
char *BKTreeCachStruct_Reset(BKTreeCachStruct *cach, int NbZerosCached,
                             int NbZerosWithOASCached) {
  int lastIdx;
  MBS_BKTree *tree;
  if (!cach)
    return (0);
  if (NbZerosWithOASCached > NbZerosCached)
    return ("Cannot cach more zeros with OAS han those without");
  /////
  tree = cach->tree;
  ...
  if ((cach->NbFwdZerosWithOAS > 0) && (cach->NbXNodes > 0)) {
    cach->zeros_OAS =
        mbspt_dmatrix(0, cach->NbFwdZerosWithOAS - 1, 0, cach->NbXNodes - 1);
  }
  ///
  cach->currIdx_zeros = cach->currIdx_zeros_OAS = cach->currIdx_DF =
      cach->currIdx_DF_OAS = cach->currIdx_probs =
...
void BKTreeCachStruct_Destruct(BKTreeCachStruct *cach) {
  if (cach) {
    ...
    if ((cach->NbTNodes > 0) && (cach->NbXNodes > 0)) {
      mbspt_free_dmatrix(cach->zeros, 0, cach->NbTNodes - 1, 0,
                         cach->NbXNodes - 1);
    }
    if ((cach->NbFwdZerosWithOAS > 0) && (cach->NbXNodes > 0)) {
      mbspt_free_dmatrix(cach->zeros_OAS, 0, cach->NbFwdZerosWithOAS - 1, 0,
                         cach->NbXNodes - 1);
    }
    //
    cach->DF = 0;
...
char *MBS_BKTree_ConstructHelper(MBS_BKTree *tree, long RefDate, int PPY,
                                 double DX_COEFF, double beta, double OAS,
                                 double XLimit_Coeff, double NSigma,
                                 double RefiBaseIndex, double RefiRate) {
  // checks
  if (Dateok(RefDate))
    return ("Bad RefDate for BKTRee construction");
  if (PPY <= 0)
    return ("Bad PPY");
  if (DX_COEFF <= 0.0)
    return ("Bad DX_COEFF");
  if (beta < 0.0)
    return ("Bad beta");
  if (XLimit_Coeff < 0.0)
    return ("Bad XLimit_Coeff");
  if (NSigma < 0.0)
    return ("Bad NSigma");
  if (RefiBaseIndex < 0.0)
    return ("Bad RefiBaseIndex");
  ...
  //////
  if (tree->RefiRate < 0.0)
    tree->RefiRate = tree->RefiBaseIndex - tree->RefiRate;
  tree->NodeMax =
      ((int)sqrt(XLimit_Coeff * ((double)tree->PPY))) * ((int)NSigma);
  if (tree->NodeMax < 10)
    tree->NodeMax = 10000; // 10000 = Big! 10 = Small!
...
char *MBS_BKTree_Construct(MBS_BKTree *tree,
                           ...
                           double *maturities, double *vols, int Rate_Freq,
                           VOL_TYPE vol_type,
                           // others
                           long RefDate, int PPY, double DX_COEFF, double beta,
                           double OAS, double XLimit_Coeff, double NSigma,
                           double RefiBaseIndex, double RefiRate) {
  char *err = 0;
  tree->cach = 0;
  ...
  if (err = MBS_BKTree_ConstructHelper(tree, RefDate, PPY, DX_COEFF, beta, OAS,
                                       XLimit_Coeff, NSigma, RefiBaseIndex,
                                       RefiRate))
    return (err);
  if (err = MBSDF_Data_Copy(&(tree->df_data), df_data))
    return (err);
  if (err =
          Vol_Surface_Construct(&(tree->vol_surface), NbSwaptions, expiries,
                                maturities, vols, Rate_Freq, vol_type, RefDate))
    return (err);
  return (err);
...
char *MBS_BKTree_FitCrv(MBS_BKTree *tree) {
  ...
  // check
  if ((df_data->use_zero_data) && tree->RefDate != df_data->zero_data.Ref_Date)
    return ("Crv and Tree have different RefDates!");
  // alloc memory
  if (tree->NbTimeSlices > 0) {
    if (!tree->DFS) // if no memory is alloc
      tree->DFS = mbspt_dvector(0, tree->NbTimeSlices - 1);
    //		if(!tree->FwdZCpn)//if no memory is alloc
    //			tree->FwdZCpn = mbspt_dvector(0        ,
    // tree->NbTimeSlices-1);
  }
  // DFS
  for (i = 0; i < tree->NbTimeSlices; ++i)
    tree->DFS[i] = MBSDF_FromT(df_data, tree->RefDate, tree->tree_times[i]);
  // FwdZCpn
  return (0);
...
char *MBS_BKTree_FitVol(MBS_BKTree *tree) {
  ...
  if (tree->RefDate != vol_surface->RefDate)
    return ("Tree and Vol dates differ!");
  if (num == 0)
    return ("Vol_Surface is empty!");
  mat = vol_surface->maturities[0];
  prev = vol_surface->expiries[0];
  for (i = 1; i < num; ++i) {
    if (vol_surface->maturities[i] != mat)
      return ("Can only fit a vol curve!!");
    if (prev >= vol_surface->expiries[i])
      return ("Vol expiries must be increasing!");
  ...
  err = vol_surface_date_gen(&NbRow, &NbCol, &colNb, &resets, &starts, &ends,
                             vol_surface); // num == NbRow!!
  if (err)
    goto CLEANUP1;
  ...
  for (i = 0; i < num; ++i) {
    ...
    ann = 0.0;
    for (j = 0; j < NbCpn; ++j) {
      //			df1 = DFFromT(zero_data        , ends[i][j] ,
      //0);
      df1 = MBSDF_FromT(df_data, tree->RefDate, ends[i][j]);
      ann += df1;
      FwdZeroYield[j] =
          DFToZR(df1 / df0, ((double)j + 1) / ((double)vol_surface->Rate_Freq),
                 vol_surface->Rate_Freq) /
          100.0;
    }
    ...
    for (j = 0; j < NbCpn; ++j) {
      t2 = ends[i][j];
      x += atm_strike *
           pow(1.0 + atm_strike / ((double)vol_surface->Rate_Freq),
               (double)-(j + 2)) *
           (j + 1) * atm_strike;
      y += atm_strike *
           pow(1.0 + FwdZeroYield[j] / ((double)vol_surface->Rate_Freq),
               (double)-(j + 2)) *
           (j + 1) * FwdZeroYield[j] * VolFactor(tree->beta, t2 - t1);
    }
    x += pow(1.0 + atm_strike / (double)vol_surface->Rate_Freq,
             (double)-(NbCpn + 1)) *
         (double)NbCpn * atm_strike;
    y += pow(1.0 + FwdZeroYield[NbCpn - 1] / (double)vol_surface->Rate_Freq,
             (double)-(NbCpn + 1)) *
         (double)NbCpn * FwdZeroYield[NbCpn - 1] *
  ...
  for (i = 1; i <= num; i++) {
    ...
    SigmaValues[i] = (t2 * IntVol[i] -
                     ...
                     (t2 - t1) / VolFactor(2.0 * tree->beta, t2 - t1);
    ...
    if (maxSigma < SigmaValues[i])
      maxSigma = SigmaValues[i];
    t1 = t2;
  }
  // copy sigmas
  ...
  // adjust sigmas as what we have is NOT instantaneous FwdRate but of finite
  // maturity
  if (tree->beta > MBS_SMALL) {
    for (i = 0; i < tree->NbTimeSlices - 1; ++i) {
      ...
      tree->sigmas[i] *= DF * log(DF) / (DF - 1.0) *
                         (1.0 - exp(-tree->beta * DT)) / (tree->beta * DT);
    }
  } else {
    for (i = 0; i < tree->NbTimeSlices - 1; ++i) {
      DT = tree->tree_times[i + 1] - tree->tree_times[i];
...
// int  MBS_BKTree_CalcBranching( int i        , int NbPer        , MBS_BKTree *
// tree )
//{
...
char *MBS_BKTree_CalcVolAndDT(double *vol, double *DT, int NbPer,
                              MBS_BKTree *tree) {
...
char *MBS_BKTree_GetProbs(double **pu, double **p0, double **pd,
                          MBS_BKTree *tree, int NbPer) {
  double probs[3];
  if (!(tree->cach->cachOn))
    return ("Cach not on for probs!");
  if (!(tree->cach->currIdx_probs ==
        NbPer)) // if we have not cached        , calc
  {
    MBS_BKTree_GetProb(probs, 0, tree, NbPer,
                       tree->Top[NbPer]); // to trigger cache
  }
  (*pu) = tree->cach->pu;
  (*p0) = tree->cach->p0;
  (*pd) = tree->cach->pd;
...
char *MBS_BKTree_GetProb(double *probs, double *VolAndDT, MBS_BKTree *tree,
                         int NbPer, int i) {
         ...
         NbPer))) // if we have not cached        , calc
  {
    ...
    for (j = start; j <= end; ++j) {
      if (tree->isZV) {
        ...
        kappa = (double)k;
        alpha = tree->Drift[NbPer] - tree->beta * DT * ((double)(j - NbPer)) -
                kappa;

        pu[j] = 0.5 * (pow(Vol / tree->DX, 2.0) + alpha * (alpha + 1.0));
        pd[j] = 0.5 * (pow(Vol / tree->DX, 2.0) + alpha * (alpha - 1.0));
      }
      p0[j] = 1.0 - pu[j] - pd[j];
    }
  } else {
...
// geometry of the lattice: as NbPer increases        , (bottom        , top)
// increases as (0        , 2 * NbPer)        , with center the middle
// until NbPer = some predefined
// NodeMax        , which may be lowered in the course of program
// thereaftet (bottom        , top) = (NbPer - NodeMax        , NbPer + NodeMax)
// , again the middle being the center node Before the critical point NbPer =
// NodeMax is reached        , always
// normal bramching        , which means: 		(NbPer        , i) -->
// (NbPer+1        ,i)        , (NbPer+1        ,
// i+1)        , (NbPer+1        , i+2)        , [k being the time
// After the critical point        , special branching at top and bottom
// Top:   i --> i-1        , i        , i+1 			Botom: i --> i +
// 1        , i+2        , i+3 At each point before the critical point is reached
// , we
// test for the up probability for top        , and down for bottom
// whenever either of
// them is close to 1.0        , we will decrease NodeMax so that the critical
// point is immediately reached
///////////////////////////////////////////////
char *MBS_BKTree_SolveDrift(MBS_BKTree *tree) {
  ...
  double *currDiscount, // at NbPer it is 1-Perd DF between (tree_times[NbPer]
                        //       , tree_times[NbPer+1])
      *currStatePr,     // State Prices (i.e. Discounted Expected Value of $1
                        // received at this node)
      *NextDiscount, // at NbPer it is 1-Perd DF between (tree_times[NbPer+1] ,
                     // tree_times[NbPer])
  ...
  tree->Top = mbspt_ivector(
      0, NbNodes - 1); // add 2 more in case we want to price at 2 time slices
                       // from now to make derivative calc
  tree->Bottom = mbspt_ivector(0, NbNodes - 1);
  tree->Drift = mbspt_dvector(0, NbNodes - 1);
  ////
  NbSpaceNodes = tree->NbSpaceNodes;
  currDiscount = mbspt_dvector(
      0, NbSpaceNodes - 1); // we need drift for up to NbPer=NbNodes-3        ,
                            // so need space for up to NbPer=NbNodes-1
  ...
  // we can calc Drift Only up to the Period (times[NbNodes - 3]        ,
  // times[NbNodes - 2])        , i.e.        , Drift[NbNodes-3]; so the last 2
  // are bogus The last drift will let us calc transition prob from
  // times[NbNodes - 3] to times[NbNodes - 2]; this in turn        , allows
  // prices on times[NbNodes - 2] to be backward inducted        , but remember
  // times[NbNodes-1] is only an artificial device to allow transitional prob t
  // be calc.        , s we have what we need
  for (NbPer = 0; NbPer < NbNodes - 2; ++NbPer) {
    ...
    MBS_BKTree_CalcVolAndDT(
        &Vol, &DT, NbPer,
        tree); // Volatility over the period = sigma * sqrt(time step)

    // get next 1-Prd DF between [NbPer+1        , NbPer+2]
    for (i = Bottom; i <= Top + 2;
         i++) // if next time slice stays pre-critical it is [Bottom        ,
              // Top+2]; if post-critical [Bottom+1        , Top+1]
    {
    ...
    x1 = x2 = x3 = y1 = y2 = y3 = z1 = z2 = z3 =
        0.0; // Intermediate calculations for the drift
    a = b = c = 0.0;

    // if the model is r = exp(x)        , dx = (a - b*x) dt + sigma dW
    // then drift means a(t) * DT/DX
    // certain combination of currStatePrc        , prevStatePrc        , and
    // curr probabilities  equal to DF but the probabilities are expressible in
    // terms of drift
    ...
    for (i = Bottom; i <= Top; i++) {
      ...
      // (i.e. to Top+1        , Top        , Top-1 instead of Top+2        ,
      // Top+1        , Top) and the bottom node branches up (i.e. to bottom+1
      // , bottom+2        , bottom+3). The shift in branching is given by k.

      x1 = currStatePr[i] * currDiscount[i] * NextDiscount[i + 2 + k];
      x2 = currStatePr[i] * currDiscount[i] * NextDiscount[i + 2 + k] *
           ((double)(i - NbPer));
      x3 = currStatePr[i] * currDiscount[i] * NextDiscount[i + 2 + k] *
           ((double)((i - NbPer) * (i - NbPer)));
      y1 = currStatePr[i] * currDiscount[i] * NextDiscount[i + k];
      y2 = currStatePr[i] * currDiscount[i] * NextDiscount[i + k] *
           ((double)(i - NbPer));
      y3 = currStatePr[i] * currDiscount[i] * NextDiscount[i + k] *
           ((double)((i - NbPer) * (i - NbPer)));
      z1 = currStatePr[i] * currDiscount[i] * NextDiscount[i + 1 + k];
      z2 = currStatePr[i] * currDiscount[i] * NextDiscount[i + 1 + k] *
           ((double)(i - NbPer));
      z3 = currStatePr[i] * currDiscount[i] * NextDiscount[i + 1 + k] *
           ((double)((i - NbPer) * (i - NbPer)));

      c += 0.5 * (pow(Vol / tree->DX, 2.) * (x1 + y1 - 2.0 * z1) +
                  tree->beta * DT * (y2 - x2) +
                  pow(tree->beta * DT, 2.0) * (x3 + y3 - 2.0 * z3) +
                  kappa * ((kappa - 1.0) * x1 + (kappa + 1.0) * y1 -
                           (2.0 * kappa) * z1) +
                  2.0 * kappa * tree->beta * DT * (x2 + y2 - 2.0 * z2)) +
           z1;

      b += 0.5 * (x1 - y1) + tree->beta * DT * (2.0 * z2 - y2 - x2) +
    ...
    {
      k = MBS_BKTree_CalcBranching(
          i, NbPer,
          tree); //(NbPer + 1 > NodeMax) * ((i == Bottom) - (i == Top));
      MBS_BKTree_GetProb(probs, VolAndDT, tree, NbPer, i);
      nxtStatePr[i + 2 + k] +=
          probs[2] * currStatePr[i] *
          currDiscount[i]; // There are 3 branches originating from node i and
                           // they contribute to 3 state
      nxtStatePr[i + 1 + k] +=
          probs[1] * currStatePr[i] * currDiscount[i]; // prices in one period.
    ...
    for (i = Bottom; i <= Top + 4;
         i++) // Permute the values so that they can be used for the next
              ...
              // up to Top+4 because we want to be sure that that nxtStatePrc ,
              // etc        , 2 steps from now        , are all set to zero.
    ...
    {             // bottom of the tree are not too big or too small
      ...
      tree->NodeMax =
          NbPer + 1; // to force prob calculation using normal branching

      MBS_BKTree_GetProb(probs, VolAndDT, tree, NbPer, 0);
      if ((probs[2] > 0.999) || (probs[1] < .001) ||
          (probs[0] < .001)) // Bottom of the tree (i=0): pu might be close to 1
                             // and pd close to 0
      {
      ...
      if ((probs[2] < .001) || (probs[1] < .001) ||
          (probs[0] > .999)) // Top of the tree (i=2*NbPer): pd might be close
                             // to 1 and pu close to 0
      {
        TriggerCutFlag = 1;
      } // if
  ...
  }   // We need Drift[NbNodes-2] => We need FwdVol[NbNodes-2] and
      // Discount[NbNodes].
////
CLEANUP:
...
// calcs the 1-Prd Disc Factor        , with or without OAS for time slice NbPer
// to NbPer + 1 returned in the array staring with address *values and size *sz
// , which may be bigger than needed use tree->Bottom and Top to know which are
// not junk values always shallow copy!!
char *MBS_BKTree_Calc1PrdDF(int *sz, double **values, int useOAS, int NbPer,
                            MBS_BKTree *tree) {
  double rateDT, multiplier, spreadDT;
  int i;
  // handle only cach on case
  if (!((tree->cach)->cachOn))
    return ("Cach is not on!");
  // set sz        , *values
  ...
  *sz = (tree->cach)->NbXNodes;
  if (useOAS) {
    ...
    if ((tree->cach)->currIdx_DF_OAS == NbPer)
      return (0);
  } else {
    (*values) = (tree->cach)->DF;
    if ((tree->cach)->currIdx_DF == NbPer)
      return (0);
  }
  ...
  spreadDT = tree->OAS / 10000.0 *
             (tree->tree_times[NbPer + 1] - tree->tree_times[NbPer]);
  //#else
  //	spreadDT = tree->OAS / 10000.0 * (tree->tree_times[NbPer+1] -
  // tree->tree_times[NbPer]); #endif//MBSPTCONSISTENTOAS

  //
  rateDT = (tree->DFS[NbPer] / tree->DFS[NbPer + 1] - 1.0) *
           exp(tree->DX * ((double)(tree->Bottom[NbPer] - NbPer)));
...
char *MBS_BKTree_BackwardInduct(double *values, // both in and out
                                int useOAS, int outTimeIdx, MBS_BKTree *tree) {
  int i, k, sz;
  double *dfs = 0, tmp, *pu, *p0, *pd;
  char *err = 0;

  //	MBS_BKTree_CalcVolAndDT( VolAndDT        , VolAndDT + 1        ,
  //outTimeIdx        , tree
  //);
  if (err = MBS_BKTree_Calc1PrdDF(&sz, &dfs, useOAS, outTimeIdx, tree))
    return (err);
  ...
  for (i = tree->Bottom[outTimeIdx]; i < tree->Top[outTimeIdx]; ++i) {
    ...
    values[i] =
        dfs[i] *
        (values[i + 2 + k] * pu[i] + values[i + 1 + k] * p0[i] +
         values[i + k] * pd[i]); // that the middle branch of normal branching
                                 // is 1 higher than the current node means
                                 // there is no trouncing of values
  }
  // special handling for top        , for if there is special branching , one
  // node
  //       , i.e.        , Top-1        , has been trounced upon! i =
  //       tree->Top[outTimeIdx];
  k = MBS_BKTree_CalcBranching(i, outTimeIdx, tree);
  if (k == 0)
    tmp = values[i + k];
  values[i] = dfs[i] * (values[i + 2 + k] * pu[i] + values[i + 1 + k] * p0[i] +
                        tmp * pd[i]);
  ///
  return (err);
...
// if useOAS:
//(*zero_prices)[i][j] means the price at time <-> NbPer        , with maturity
// times[NbPer + i]        , at space node j i=0        , ... *lastFwdIndex else
// same but
// zero_prices have OAS
char *MBS_BKTree_ZeroPrices(int *lastFwdIndex, double ***zero_prices,
                            int useOAS, int NbPer, MBS_BKTree *tree) {
  char *err;
  double **zeros = 0;
  ...
  int i, j;
  //
  if (err = MBS_BKTree_ZeroPricesHelper(&lastIndex, &zeros, NbPer, tree))
    return (err);
  local_idx = lastIndex;
  if (local_idx > tree->cach->NbFwdZerosWithOAS - 1)
    local_idx = tree->cach->NbFwdZerosWithOAS - 1;
  //
  if (useOAS && (tree->cach->currIdx_zeros_OAS != NbPer)) // if not yet cached
  {
    ...
    for (i = 0; i <= local_idx; ++i) {
      ...
      if (i < local_idx) {
        OAS_Multiple = OAS_scaled * (tree->tree_times[NbPer + i + 1] -
...
// zero_prices is shallow copied
//(*zero_prices)[i][j] means the price at time <-> NbPer        , with maturity
// times[NbPer + i]        , at space node j i=0        , ... *lastFwdIndex get
// zeros without
// OAS
char *MBS_BKTree_ZeroPricesHelper(int *lastFwdIndex, double ***zero_prices,
  ...
  if (!tree->cach->cachOn)
    return ("Cach not on for zero_prices");
  if (NbPer >= tree->EndTimeSlices)
    return ("NbPer exceeds bounds");
  if (NbPer < tree->EndTimeSlices - 1 &&
      tree->cach->currIdx_zeros != NbPer + 1 &&
  ...
  *lastFwdIndex = tree->EndTimeSlices - 1 - NbPer;
  ...
  if (*lastFwdIndex > tree->cach->NbTNodes - 1)
    *lastFwdIndex = tree->cach->NbTNodes - 1;
  if (prev_NbZeros > tree->cach->NbTNodes)
    prev_NbZeros = tree->cach->NbTNodes;
  ...
  // last time slice
  if (prev_NbZeros ==
      0) // tree->EndTimeSlices - 1 + tree->cach->NbFwdZerosWithOAS - 1)
  {
  ...
  if (prev_NbZeros < tree->cach->NbTNodes) {
    tree->cach->zeros[0] =
        tree->cach->zeros[prev_NbZeros]; // assign an unoccupied array        ,
                                         // to be filled with 1
...
// if term exceeds the number of cached zeros available        , automatically ,
// shrink term to whatever is available
char *MBS_BKTree_Yield_Approx(double *yields, int freq, double term, int NbPer,
                              MBS_BKTree *tree) {
  double **zeros = 0;
  int lastFwdIndex = 0;
  char *err;
  ...
  ////
  if (tree->PPY != 11)
    return ("Can approx yields only for monthly lattice!");
  ////
  ...
  if (err = MBS_BKTree_ZeroPrices(&lastFwdIndex, &zeros, 0, NbPer, tree))
    return (err);
...
char *MBS_BKTree_Calibrate(MBS_BKTree *tree) {
  char *err = 0;
  ////fit curve

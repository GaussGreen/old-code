...
Err LGMFwdSwaptionLGM(long exp, long start, long nfp_or_end, char *cpdStr,
  ...
  if (sUndPtrName) {
    ...
    return serror(
        "Heinous error: No sUndPtrerlying passed to LGMFwdSwaptionLGM");
  }
  ...
  if (sUndPtr) {
    if (!ISUNDTYPE(sUndPtr, INTEREST_RATE_UND) ||
      ...
      return serror(
          "sUndPtrerlying must be of type Interest Rate  , Model must be LGM");
    }
  } else {
  ...
  if (err = swp_f_make_FixedAndNotionalsLeg(&sdp, strike, 1.0, 1.0, today,
                                            &fixedleg)) {
    return err;
  }
  ...
  if (err = swp_f_make_SpreadLeg(sdp.start, sdp.end, today, refRateCodeStr,
                                 &spreadleg)) {
  ...
  *price = srt_f_lgm_coupon_bond_option(
      bigleg.leg_length - 1, -bigleg.payment.d[0], ts,
      (exp - today) * YEARS_IN_DAY, bigleg.payment.d, bigleg.time.d,
      bigleg.df.d, rec_pay, mdl_dim);
...
Err LGMFwdSwaptionATC(long exp, long start, long nfp_or_end, char *cpdStr,
  ...
  /*	Make the fixed leg (coupon of strike) with initial and final */
  if (err = swp_f_make_FixedAndNotionalsLeg(&sdp, strike, 0.0, 0.0, today,
                                            &fixedleg)) {
    return err;
  }

...
static double local_price_fct(double irr, int i)
{
    ...
    {
        price =
            (price + NI_cpn[i] / (double)glob_sdp[i].compd) * pow(disc1, glob_sdp[i].compd * dr);
    }
...
static double bond_opt_integrand(double logy)
{
    ...
    {
        dens -=
            INV_SQRT_TWO_PI * exp(NI_ext_adj2 - (logy - NI_ext_adj1) * (logy - NI_ext_adj1) / 2.0);
    }
    ...
    double*          answer)
{
    ...
    sdp->end =
        add_unit(sdp->start, 12 / (int)sdp->compd * sdp->nfp, SRT_MONTH, NO_BUSDAY_CONVENTION);
    ...
    while (niter < OPTBNDYLDMAXITER && fabs(y1) > OPTBNDYLDTOLERANCE)
    {
        ...
        fprintf(
            fptr,
            "OPTBNDCMS fpr %lf niter %d x0 %lf y0 %lf x1 %lf y1 %lf\n",
            fwd_pr,
            niter,
            x0,
            y0,
    ...
    double*          answer)
{
    ...
    if (ext_flg != 0)
    {
        if (lognrm_nrm == SRT_LOGNORMAL)
        {
            ...
            NI_ext_adj2 =
                (2.0 * (NI_logy0 - log(spot_yld)) / (NI_sig * NI_sig) *
        ...
        else
        {
            ...
            NI_ext_adj2 =
                (2.0 * (NI_logy0 - spot_yld) / (NI_sig * NI_sig) * (ext_barrier - spot_yld));
            NI_ext_flg = ext_flg;
        }
    }
    else

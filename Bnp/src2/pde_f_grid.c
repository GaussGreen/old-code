				...
				long                       num_points[3])
{
   ...
   grid->vec_x  = (double *) malloc(taillealloc); MEMSET(grid->vec_x, 0, taillealloc);   
   grid->inc_x  = (double *) malloc(taillealloc); MEMSET(grid->inc_x, 0, taillealloc);
   ...
   if ( grid->dimension >= 2 )
   {
      ...
      grid->vec_y  = (double *) malloc(taillealloc); MEMSET(grid->vec_y, 0, taillealloc);   
      grid->inc_y  = (double *) malloc(taillealloc); MEMSET(grid->inc_y, 0, taillealloc);   
   ...
   if ( grid->dimension == 3 )
   {
      ...
      grid->vec_z  = (double *) malloc(taillealloc); MEMSET(grid->vec_z, 0, taillealloc);   
      grid->inc_z  = (double *) malloc(taillealloc); MEMSET(grid->inc_z, 0, taillealloc);   
   ...
   while ( i <= mxx )
   {
      ...
      while ( j <= myy )
      {
         ...
         grid->AX[i][j] = (double *) malloc(taillealloc); MEMSET(grid->AX[i][j], 0, taillealloc);   
         grid->BX[i][j] = (double *) malloc(taillealloc); MEMSET(grid->BX[i][j], 0, taillealloc);   
         grid->CX[i][j] = (double *) malloc(taillealloc); MEMSET(grid->CX[i][j], 0, taillealloc);   
         grid->AX1[i][j] = (double *) malloc(taillealloc); MEMSET(grid->AX1[i][j], 0, taillealloc);   
         grid->BX1[i][j] = (double *) malloc(taillealloc); MEMSET(grid->BX1[i][j], 0, taillealloc);   
         grid->CX1[i][j] = (double *) malloc(taillealloc); MEMSET(grid->CX1[i][j], 0, taillealloc);   

         grid->L1[i][j] = (double *) malloc(taillealloc); MEMSET(grid->L1[i][j], 0, taillealloc);
         grid->D1[i][j] = (double *) malloc(taillealloc); MEMSET(grid->D1[i][j], 0, taillealloc);
   
         if ( grid->dimension >= 2 )
         {        
            taillealloc = nzz * sizeof(double);

            grid->AY[i][j] = (double *) malloc(taillealloc); MEMSET(grid->AY[i][j], 0, taillealloc);  
            grid->BY[i][j] = (double *) malloc(taillealloc); MEMSET(grid->BY[i][j], 0, taillealloc);
            grid->CY[i][j] = (double *) malloc(taillealloc); MEMSET(grid->CY[i][j], 0, taillealloc);   
            grid->AY1[i][j] = (double *) malloc(taillealloc); MEMSET(grid->AY1[i][j], 0, taillealloc);  
            grid->BY1[i][j] = (double *) malloc(taillealloc); MEMSET(grid->BY1[i][j], 0, taillealloc);
            grid->CY1[i][j] = (double *) malloc(taillealloc); MEMSET(grid->CY1[i][j], 0, taillealloc);   
            grid->EXY[i][j] = (double *) malloc(taillealloc); MEMSET(grid->EXY[i][j], 0, taillealloc);   

            grid->L2[i][j] = (double *) malloc(taillealloc); MEMSET(grid->L2[i][j], 0, taillealloc);   
            grid->D2[i][j] = (double *) malloc(taillealloc); MEMSET(grid->D2[i][j], 0, taillealloc);   
         }
         ...
         { 
            ...
            grid->AZ[i][j] = (double *) malloc(taillealloc); MEMSET(grid->AZ[i][j], 0, taillealloc);   
            grid->BZ[i][j] = (double *) malloc(taillealloc); MEMSET(grid->BZ[i][j], 0, taillealloc);   
            grid->CZ[i][j] = (double *) malloc(taillealloc); MEMSET(grid->CZ[i][j], 0, taillealloc);   
            grid->AZ1[i][j] = (double *) malloc(taillealloc); MEMSET(grid->AZ1[i][j], 0, taillealloc);   
            grid->BZ1[i][j] = (double *) malloc(taillealloc); MEMSET(grid->BZ1[i][j], 0, taillealloc);   
            grid->CZ1[i][j] = (double *) malloc(taillealloc); MEMSET(grid->CZ1[i][j], 0, taillealloc);   
            grid->EXZ[i][j] = (double *) malloc(taillealloc); MEMSET(grid->EXZ[i][j], 0, taillealloc);   
            grid->EYZ[i][j] = (double *) malloc(taillealloc); MEMSET(grid->EYZ[i][j], 0, taillealloc);   

            grid->L3[i][j] = (double *) malloc(taillealloc); MEMSET(grid->L3[i][j], 0, taillealloc);   
            grid->D3[i][j] = (double *) malloc(taillealloc); MEMSET(grid->D3[i][j], 0, taillealloc);   
         }

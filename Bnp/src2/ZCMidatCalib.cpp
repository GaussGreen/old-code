...
double
ZCMidat_lgmsopval1F(int ncpn,        /*	Number of cash-flow dates  , including
                                                             start and end date */
                    double cpn[],    /*	Notional */
...
double
ZCMidat_lgmsopval2F(int ncpn,         /*	Number of cash-flow dates  , including
                                                              start and end date */
                    double cpn[],     /*	Notional */
                    double df[],      /*	Df to cash flow dates */
                    double cvg[],     /*	cvg from i-1 to i */
...
Err ZCMidat_cpd_calib_diagonal(
    ...
    char *swaption_freq, /*	Frequency  , basis and ref. rate of underlying
                            swaptions */
  ...
  *sig = NULL;
  ...
  for (k = 1; k < nbcoupon; ++k) {
    ...
    if ((coupon_dates[k] == fix_pay_times[fix_index]) &&
      ...
      //fix_cvgs[fix_index-1] * swp_f_df (today  , fix_pay_dates[fix_index]  ,
      //yc_name);

    ...
    CPD_CALIB_INST_DATA inst_data) /*	NULL = don't save calibration instrument data */
{
    ...
    SrtBasisCode   ibasis;
    double         ex_lfwd[MAX_CPN], ex_llvl[MAX_CPN], ex_lstrike[MAX_CPN], ex_lvol[MAX_CPN],
        ex_lprice[MAX_CPN], ex_sfwd[MAX_CPN], ex_slvl[MAX_CPN], ex_sstrike[MAX_CPN],
        ex_svol[MAX_CPN], ex_sweight[MAX_CPN], ex_sprice[MAX_CPN], ex_zeta[MAX_CPN], ex_G[MAX_CPN];
    ...
    *sig      = NULL;
    ...
    for (k = 1; k < nbcoupon; ++k)
    {
            ...
            (coupon_dates[k] != float_pay_times[float_index]))
        {
            ...
            //			coupon[k] += long_strike[fix_index-1] * fix_cvgs[fix_index-1] * swp_f_df (today,
            ...
            (coupon_dates[k] == float_pay_times[float_index]))
        {
            ...
            coupon[k] += -(float_spreads[float_index - 1] * float_cvgs[float_index - 1]) *
        ...
        else
        {
            ...
            else
            {
                ...
                exer_date[fix_index - 1] = fix_pay_dates[fix_index - 1];
                fix_index                = fix_index + 1;
                coupon[k] += -(-1 + float_spreads[float_index - 1] * float_cvgs[float_index - 1]) *
                             swp_f_df(today, float_pay_dates[float_index], yc_name);
                float_index = float_index + 1;
            }
        }
    ...
    for (i = 0; i < nbexercise; i++)
    {
        ...
        {
            power =
                srt_f_optblksch(ex_lfwd[i], ex_lfwd[i], std, exer_time[i], 1.0, SRT_CALL, PREMIUM);
            err = srt_f_optimpvol(
                power, ex_lfwd[i], ex_lfwd[i], exer_time[i], 1.0, SRT_CALL, SRT_NORMAL, &std);
        }
        ...
        if (power > 0.5)
        {
            ex_lprice[i] = srt_f_optblksch(
                ex_lfwd[i], ex_lstrike[i], ex_lvol[i], exer_time[i], ex_llvl[i], SRT_PUT, PREMIUM);
        }
        else
        {

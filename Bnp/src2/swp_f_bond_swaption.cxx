/* ------------------------------------------------------------------------------
   FILENAME:      swp_f_bond_swaption.cxx
...
/* We return the swaption maturity        , strike and volatility        , as
well as vega ratio and hedge ratio. If there is a problem to compute the
volatility or the hedge ratio or the vega ratio        , we return just the
swaption maturity and strike and put -1 for the volatility and 1 for the hedge
ratio and vega ratio */

Err swp_f_match_bond_swaption(
    long option_expiry, long settle_bond_date, long maturity_date,
    String strBondComp, String strBondBasis, double coupon, /* as 0.05 for 5% */
    double fwd_clean_price, /* as 1.00 for 100 */
    double clean_strike, String strCallPut, double bond_option_value,
    String yc_name, String refRateCode, String strSwapComp, String strSwapBasis,
    String strLogorNormal,

    long *swap_settle, long *swap_maturity, double *swap_coupon,
    double *implied_vol, double *vega_ratio, double *hedge_ratio) {
  SwapDP swap_dp;
  SwapDP bond_dp;
  ...
  *swap_settle = settle_swap_date;
  ...
  err = swp_f_initSwapDP(settle_bond_date, maturity_date, strBondComp,
                         strBondBasis, &bond_dp);
  if (err)
    return err;
  ...
  err = bond_modified_convexity(&bond_dp, coupon, fwd_dirty_price,
                                &bond_convexity);
  if (err)
    return err;
  err =
      bond_modified_duration(&bond_dp, coupon, fwd_dirty_price, &bond_duration);
  if (err)
    return err;
  ...
  /* Loops on days until ratios are matched (Newton would be useless: days are
   * integers) */
  *swap_maturity = maturity_date;
  while (stop_flag != SRT_YES) {

    /* Initialises a SwapDP for the swap with same maturity date as the bond */
    err = swp_f_initSwapDP(settle_swap_date, *swap_maturity, strSwapComp,
                           strSwapBasis, &swap_dp);
    if (err)
      return err;
    ...
    *swap_coupon =
        (fwd_clean_price / clean_strike - forward_df) / swap_forward_level;

    /* Compute the forward ratio convexity/duration for the forward swap */
    err = zcswap(&swap_dp, *swap_coupon, 0.0, 1.0,
                 COMPUTE_MODIFIED_MATCHING_RATIO, yc_name, settle_swap_date,
                 &swap_ratio);
    if (err)
      return err;

    /* Determines which way to go: if ratio too high        , increase maturity
     * , else decrease it*/
    if (swap_ratio < bond_ratio)
      new_direction = FORWARD;
    else if (swap_ratio > bond_ratio)
    ...
    if (old_direction * new_direction == -1) {
      ...
      if (new_direction == FORWARD)
        *swap_maturity =
            add_unit(*swap_maturity, 1, SRT_DAY, NO_BUSDAY_CONVENTION);
      else if (new_direction == BACKWARD)
        *swap_maturity =
            add_unit(*swap_maturity, -1, SRT_DAY, NO_BUSDAY_CONVENTION);
  ...
  err = srt_f_optimpvol(target, fwd_clean_price, clean_strike, option_maturity,
                        1.0, call_put, SRT_LOGNORMAL, &bond_price_vol);
  if (err)
    return err;
  ...
  swaption_value =
      settle_swap_df * srt_f_optblksch(fwd_clean_price / clean_strike, 1.0,
                                       swap_price_vol, option_maturity, 1.0,
                                       call_put, PREMIUM);

  /* Gets the forwards with and without spreads (for strike correction) */
  err = swp_f_ForwardRate_SwapDP(&swap_dp, yc_name, refRateCode,
                                 &real_swap_forward);
  if (err)
    return err;
  ...
  *swap_coupon = swaption_strike;
  ...
  err = swp_f_SwaptionImpliedVol_SwapDP(swaption_value, &swap_dp,
                                        swaption_strike, rec_pay, refRateCode,
                                        yc_name, log_or_norm, implied_vol);
  if (err)
    return err;

  /* Computes the coefficient of hedging */
  db = bond_price_vol * bond_price_vol * option_maturity;
  db = (log(fwd_clean_price / clean_strike) + 0.5 * db) / sqrt(db);

  ds = bond_price_vol * bond_price_vol * option_maturity /
       (price_vol_ratio * price_vol_ratio);
  ds = (log(fwd_clean_price / clean_strike) + 0.5 * ds) / sqrt(ds);

  *hedge_ratio =
      bond_duration / swap_duration * clean_strike * norm(db) / norm(ds);
  ...
  err = swp_f_SwaptionImpliedVol_SwapDP(swaption_value, &swap_dp, *swap_coupon,
                                        rec_pay, refRateCode, yc_name,
                                        log_or_norm, vega_ratio);

 ...
 *				: paths			- number of MC paths used for
 *integration : greek         - greek wanted (only premium)
 *				: Points		- Num of points for the approx of
 *density of each underlying
 *
 * RETURNS       : ??            - ??
 *
 *******************************************************************************/

Err Free_memory_at_end(double ****pppdSobolNoise, double **pdFwdsAtMaturity,
                       double ***ppdCorrelCoef, double ***ppdPoints,
                       double ***ppdCumProbaValue, double ***ppdSpreadSquare,
                       long paths, long nFwds, long nNumPoints) {
  Err err;
...
Err srt_f_EuroBasketSmile(int nFwds, double *Fwds, int nWeights,
  ...
  /* Compute the Coeff matrix from the Correlation matrix used later to
   * correlate the noise */
  ...
  /* In order to be the more accurate the gonna fill a Sobol cube for one time
   * step */
  ...
  if (err = sobol_init(1, paths, 0, nFwds - 1, 1, 1)) {
    err = Free_memory_at_end(&pppdSobolNoise, &pdFwdsAtMaturity, &ppdCorrelCoef,
                             &ppdPoints, &ppdCumProbaValue, &ppdSpreadSquare,
                             paths, nFwds, nNumPoints);
  ...
  for (i = 0; i < nFwds; i++) {
    ...
    for (j = 1; j < 2 * nNumPoints; j++) {
      ppdPoints[i][j] =
          Fwds[i] * exp(-0.5 * dApproxStdev * dApproxStdev +
      ...
      err = op_sabr_pricing(Fwds[i], dAdjStrike, mat, 1, SRT_CALL, Alpha[i],
                            Beta[i], Rho[i], Vols[i], SABR_ATM_BETA, &dTemp1);

      dAdjStrike /= dCoeffAdjustStrike * dCoeffAdjustStrike;
      err = op_sabr_pricing(Fwds[i], dAdjStrike, mat, 1, SRT_CALL, Alpha[i],
                            Beta[i], Rho[i], Vols[i], SABR_ATM_BETA, &dTemp2);

      if (err) {
        err = Free_memory_at_end(&pppdSobolNoise, &pdFwdsAtMaturity,
                                 &ppdCorrelCoef, &ppdPoints, &ppdCumProbaValue,
                                 &ppdSpreadSquare, paths, nFwds, nNumPoints);
      ...
      /*
      dAdjStrike = (log(Fwds[i] / ppdPoints[i][j] ) - 0.5 * Vols[i] * Vols[i] *
      mat) / (Vols[i] * sqrt(mat)); ppdCumProbaValue[i][j] = 1.0 -
      norm(dAdjStrike);
      */
    }
  }
  ...
  for (i = 1; i <= paths; i++) {
    ...
    for (j = 0; j < nFwds; j++) {
      ...
      pdFwdsAtMaturity[j] = interp(ppdCumProbaValue[j], ppdPoints[j],
                                   2 * nNumPoints + 1, dCumProba, 1, &dTemp);
  ...
  if (ppdSpreadVol) {
    ...
    for (i = 0; i < nFwds; i++) {
      ...
      for (j = 0; j < i; j++) {
        dCumVariance = ppdSpreadSquare[i][j] / paths -
                       (Fwds[i] - Fwds[j]) * (Fwds[i] - Fwds[j]);
...
/*--------------------------------------------------------------------------------------------------------
This is the same function for pricing a basket under the SABR model. The
difference with the preceding one is that  , for each Forward (in total we have
nFdws) it draws a random variable which is compatible with the observed smile  ,
that is a variable distributed accrding to the implied marginal cumulative
distributions. In addition  , the copula function define a joint density for the
n forwards. The parameter to input is StudDegree that corresponds to the degree
of the Student Copula. In general for StudDegree->Infinity we recover the
gaussian case but in this implementation  , the gaussian copula is given by
assigning StudDegree=0
----------------------------------------------------------------------------------------------------------
*/
...
Err srt_f_EuroBasketSmileCpl(
  ...
  for (i = 0; i < nFwds; i++) {
    ...
    for (j = 1; j <= 2 * nNumPoints; j++) {
      ...
      dTemp1 = srt_f_optblkschbetastochquick(
          Fwds[i], dAdjStrike, mat, Vols[i], Alpha[i], Beta[i], Rho[i], 1.0,
          TypeInput, log_norm, CallPut, greek);
    ...
    default:
      ...
      for (k = 1; k <= kfail + 1; k++) {
        ...
        dTemp1 = srt_f_optblkschbetastochquick(
            Fwds[i], dAdjStrike, mat, Vols[i], 0.0, Beta[i], Rho[i], 1.0,
            TypeInput, log_norm, CallPut, greek);
  ...
  /*
          ...
          {
                  // set the Approximation for the Stdev
                  dApproxStdev = Vols[i] / pow(Fwds[i]  , 1.0 - Beta[i]) *
  sqrt(mat);

                  // For the shift of the Strike
                  dCoeffAdjustStrike = exp( dApproxStdev / nNumPoints);
                  dCoeff = dCoeffAdjustStrike /
  (dCoeffAdjustStrike*dCoeffAdjustStrike - 1.0);

                  // set the value at the boundary 10 stdev
                  n_StdDev=6.;

  //		ppdPoints[i][0] = Fwds[i] * exp( - 0.5 * dApproxStdev *
  dApproxStdev - n_StdDev * dApproxStdev);
  //		ppdPoints[i][2*nNumPoints] = Fwds[i] * exp( - 0.5 * dApproxStdev
  * dApproxStdev + n_StdDev * dApproxStdev);

                  ppdPoints[i][0]=DMAX(0.00001
  ,Fwds[i]-5.*sqrt(Fwds[i]*Fwds[i]*(exp(dApproxStdev*dApproxStdev)-1.)));
  ...
  //		ppdCumProbaValue[i][2*nNumPoints] = 1.0;
                  ...
                  ppdPoints[i][j]=ppdPoints[i][0]+dx*j;
                          ...
                          // Compute the Cum Proba associated to this points P(S
  < pdPoints)
  ...
  */
  ...
  // loads the cube of correlated variables compatible with the Student Copula
  // for n
  // degrees of freedom
  ...
  GetStudentCplDev(StudDegree, mean_v, Correlation, nPaths, StudDegree + nFwds,
                   ppdPoints, ppdCumProbaValue, 2 * nNumPoints + 1, 0, MCType,
                   CorrCube);
  ...
  /*
          ...
          fp2= fopen ("C:\\WORKAREA\\Devl\\Cpl_Spread\\StudentTDis_new.txt"  ,
     "w+"); for (i=1;i<=paths-2;i++) { fprintf(fp2  ,"{ %f  ,  %f}  , \n"  ,
     output[i]  ,1.*i/(paths-2));
          }

          for (i=0;i<2*nNumPoints+1;i++) {
                  fprintf(fp2  ,"{ %f  ,  %f}  , \n"  ,ppdPoints[1][i]  ,
     ppdCumProbaValue[1][i]);
          }
          ...
          if (ppdSpreadVol)
          {
                  ...
                  for (i=0; i< nFwds; i++)
                  {
                          ...
                          for (j=0; j<i; j++) {
                                  ...
                                  dCumVariance =
     ppdSpreadSquare[i][j]-(Fwds[i]-Fwds[j])*(Fwds[i]-Fwds[j]);
                                  ppdSpreadVol[i][j] = ppdSpreadVol[j][i]= sqrt(
     dCumVariance / mat ) ;
...
/*--------------------------------------------------------------------------------------------------------
This is the same function for pricing a standard option on the weighted
geometrical average under the SABR model. The copula function define a joint
density for the n forwards. The parameter to input is StudDegree that
corresponds to the degree of the Student Copula. In general for
StudDegree->Infinity we recover the gaussian case but in this implementation  ,
the gaussian copula is given by assigning StudDegree=0
----------------------------------------------------------------------------------------------------------
*/
Err srt_f_WeightedPdtSmileCpl(
  ...
  for (i = 0; i < nFwds; i++) {
    ...
    /* Sigma Beta */
    srt_f_optsarbvol(Fwds[i], Fwds[i], mat, Vols[i], Alpha[i], Beta[i], Rho[i],
                     TypeInput, SRT_BETAVOL, &SigmaBeta);
    ...
    for (j = 1; j <= 2 * nNumPoints; j++) {
      ...
      dTemp1 = srt_f_optblkschbetastochquick(
          Fwds[i], dAdjStrike, mat, SigmaBeta, Alpha[i], Beta[i], Rho[i], 1.0,
          SRT_BETAVOL, log_norm, CallPut, greek);
    ...
    default:
      ...
      for (k = 1; k <= kfail + 1; k++) {
        ...
        dTemp1 = srt_f_optblkschbetastochquick(
            Fwds[i], dAdjStrike, mat, SigmaBeta, 0.0, Beta[i], Rho[i], 1.0,
            SRT_BETAVOL, log_norm, CallPut, greek);
  ...
  for (i = 3; i < nPaths; i++) {
    ...
    for (j = 0; j < nFwds; j++) {
      ...
      avg[j] += (pdFwdsAtMaturity[j] - avg[j]) / (i - 2);
      dGeomAvg =
          dGeomAvg * pow(pdFwdsAtMaturity[j] * Fwds[j] / avg[j], Weights[j]);
    }
...
The parameter to input is StudDegree that corresponds to the degree of the
Student Copula. In general for StudDegree->Infinity we recover the gaussian case
but in this implementation  , the gaussian copula is given by assigning
StudDegree=0
----------------------------------------------------------------------------------------------------------
*/
...
Err srt_f_BestOfDigitalSmileCpl(
  ...
  for (i = 3; i < nPaths; i++) {
    ...
    for (j = 0; j < nFwds; j++) {
      ...
      avg[j] += (pdFwdsAtMaturity[j] - avg[j]) / (i - 2);
      dBestOf =
          max(dBestOf, pdFwdsAtMaturity[j] * Fwds[j] / avg[j] + Weights[j]);
    }
...
Err srt_f_BestOfSmileCpl(
  ...
  for (i = 3; i < nPaths; i++) {
    ...
    for (j = 0; j < nFwds; j++) {
      ...
      dBestOf =
          max(dBestOf,
              max(pdFwdsAtMaturity[j] * Fwds[j] / avg[j] - Weights[j], 0.0));
    }
...
Err srt_f_CallableBasketCpl(
  ...
  double *pdFwdsAtMaturity = NULL, **ppdPoints = NULL,
         **ppdCumProbaValue = NULL;
  double **CorrCube = NULL, *mean_v = NULL, *Fwds = NULL, *Vols = NULL,
         **Correlation;
  double *Alpha = NULL, *Beta = NULL, *Rho = NULL;

  double dApproxStdev, dCoeffAdjustStrike, dCoeff, dAdjStrike, dTemp1, dTemp2,
      dSum, dBasket0, dBasket1;
  ...
  Correlation =
      dmatrix(0, nFwdsRows * nFwdsCols - 1, 0, nFwdsRows * nFwdsCols - 1);
  ...
  */
  ...
  GetStudentCplDev(StudDegree, mean_v, Correlation, (paths + 1) * (n_conv + 1),
                   StudDegree + nFwds, ppdPoints, ppdCumProbaValue,
                   2 * nNumPoints + 1, 0, MCType, CorrCube);
  ...
  for (i = 3; i < 2 * paths; i++) {
    ...
    for (k = 0; k < nFwdsCols; k++) {
      ...
      for (l = 1; l <= n_conv + 1; l++) {
        pdFwdsAtMaturity[nFwdsRows - 1] +=
            CorrCube[i + (l - 1) * (paths + 1)][m];
      }

      dCashFlow +=
          (dBasket0 + DMAX(cp * (dBasket1 - InStrikes[1]), 0.0) -
  ...
  free_dmatrix(Correlation, 0, nFwdsRows * nFwdsCols - 1, 0,
               nFwdsRows * nFwdsCols - 1);
...
Err srt_f_CompoundBasketCpl(
  ...
  ConvolveMarginals(ppdPoints, ppdCumProbaValue, 2 * nNumPoints + 1, nFwds,
                    n_conv, -1);
  ...
  for (i = 3; i < paths; i++) {
    /*
                    CorrMatr = GetStudentCplDev_Rand (
                                                                    ...
                                                                    mean_v  ,
                                                                    Correlation
       , paths+1  , StudDegree+nFwds  , ppdPoints  , ppdCumProbaValue  ,
                                                                    2*nNumPoints+1
       , n_conv  , &idum
                                                      );
    */
  ...
  GetStudentCplDev(StudDegree, mean_v, Correlation, (paths + 1) * (n_conv + 1),
                   StudDegree + nFwds, ppdPoints, ppdCumProbaValue,
                   2 * nNumPoints + 1, n_conv, MCType, CorrCube);
  ...
  for (i = 3; i < 2 * paths; i++) {
    ...
    for (l = 1; l <= n_conv + 1; l++) {
      pdFwdsAtMaturity[nFwds - 1] +=
          CorrCube[i + (l - 1) * (paths + 1)][nFwds - 1];
    }
...
Err srt_f_Basket_MixedSL_Copula(
  ...
  double *pdFwdsAtMaturity = NULL, **dPoints = NULL, *dDriftDown = NULL,
         *dDriftUp = NULL, **dCumProbaValue = NULL;
  ...
  if (!pdFwdsAtMaturity || !dDriftDown || !dDriftUp || !dPoints ||
      !dCumProbaValue || !dMeanV || !dCorrCube) {
  ...
  for (i = 0; i < iNbFwd; i++) {
    ...
    dApproxShiftedFwd =
        dProba[i] * dShiftedFwdDown[i] + (1.0 - dProba[i]) * dShiftedFwdUp[i];
    ...
    for (j = 1; j <= lNbPoints2; j++) {
      ...
      dCumProbaValue[i][j] =
          dProba[i] * norm((dPoints[i][j] - dDriftDown[i]) / dVolDown[i]);
      dCumProbaValue[i][j] +=
  ...
  GetStudentCplDev(iStudDegree, dMeanV, dCorrelation, lNbPaths,
                   iStudDegree + iNbFwd, dPoints, dCumProbaValue,
                   lNbPoints2 + 1, 0, MCType, dCorrCube);

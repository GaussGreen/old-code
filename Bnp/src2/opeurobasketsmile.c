...
*				: Points		- Num of points for the approx of density of each underlying
*
* RETURNS       : ??            - ??
*
*******************************************************************************/

Err Free_memory_at_end(double ****pppdSobolNoise,
						...
						double  **ppdSpreadVol)
{
	...
	if (ppdSpreadVol) ppdSpreadSquare = dmatrix(0, nFwds - 1,0,nFwds-1);
	...
	for (i=0; i< nFwds; i++)
	{
		...
		ppdPoints[i][2*nNumPoints] = Fwds[i] * exp( - 0.5 * dApproxStdev * dApproxStdev + 10.0 * dApproxStdev);
		ppdCumProbaValue[i][2*nNumPoints] = 1.0;

		for (j=1; j<2*nNumPoints; j++)
		{
			ppdPoints[i][j] = Fwds[i] * 
			...
			err = op_sabr_pricing( Fwds[i],dAdjStrike,mat,1,SRT_CALL, 
							Alpha[i],Beta[i],Rho[i],Vols[i],SABR_ATM_BETA,&dTemp1);

			dAdjStrike /= dCoeffAdjustStrike * dCoeffAdjustStrike;
			err = op_sabr_pricing( Fwds[i],dAdjStrike,mat,1,SRT_CALL, 
							Alpha[i],Beta[i],Rho[i],Vols[i],SABR_ATM_BETA,&dTemp2);
			...
			/*
			dAdjStrike = (log(Fwds[i] / ppdPoints[i][j] ) - 0.5 * Vols[i] * Vols[i] * mat) / (Vols[i] * sqrt(mat));  
			ppdCumProbaValue[i][j] = 1.0 - norm(dAdjStrike); 
			*/
		}
	}
	...
	for (i=1;i<=paths;i++)
	{
		...
		for (j=0; j< nFwds; j++)
		{
			...
			{
				for (k=0; k<j; k++)
					ppdSpreadSquare[j][k] += (pdFwdsAtMaturity[j] - pdFwdsAtMaturity[k]) * (pdFwdsAtMaturity[j] - pdFwdsAtMaturity[k]);
			}
		}

		dCashFlow = cp*(dBasket - strike);
		if (dCashFlow > 0) dSum += dCashFlow;
	...
	if (ppdSpreadVol) 
	{
		...
		for (i=0; i< nFwds; i++)
		{
			...
			for (j=0; j<i; j++)
			{
				dCumVariance = ppdSpreadSquare[i][j] / paths - (Fwds[i] - Fwds[j]) * (Fwds[i] - Fwds[j]);
...
cumulative distributions. In addition, the copula function define a joint density for the n forwards.
The parameter to input is StudDegree that corresponds to the degree of the Student Copula. In general
for StudDegree->Infinity we recover the gaussian case but in this implementation, the gaussian copula is 
given by assigning StudDegree=0
----------------------------------------------------------------------------------------------------------  */
								...
								)
{
	...
	double	**ppdSpreadSquare = NULL,*pdFwdsAtMaturity = NULL,**ppdPoints = NULL,**ppdCumProbaValue = NULL;
...
//	double Gap,dx,PrMin;
	...
	for (i=0; i< nFwds; i++)
	{
		...
		ppdPoints[i][0] = Fwds[i] * exp( - 0.5 * dApproxStdev * dApproxStdev - n_StdDev * dApproxStdev); // modifier....
		ppdPoints[i][2*nNumPoints] = Fwds[i] * exp( - 0.5 * dApproxStdev * dApproxStdev + n_StdDev * dApproxStdev);
//		ppdCumProbaValue[i][0] = 0.0;
//		ppdCumProbaValue[i][2*nNumPoints] = 1.0;
...
//		ppdPoints[i][0] = Fwds[i] * exp( - 0.5 * dApproxStdev * dApproxStdev - n_StdDev * dApproxStdev); 
//		ppdPoints[i][2*nNumPoints] = Fwds[i] * exp( - 0.5 * dApproxStdev * dApproxStdev + n_StdDev * dApproxStdev);
...
//		ppdCumProbaValue[i][2*nNumPoints] = 1.0;
		...
		ppdCumProbaValue[i][0] = ppdCumProbaValue[i][1]-fabs(ppdCumProbaValue[i][1]-ppdCumProbaValue[i][2])
...
/*
				if (ppdSpreadVol)
				{
					for (k=0; k<j; k++) {
						...
						sprd_ran=pdFwdsAtMaturity[j] - pdFwdsAtMaturity[k];
						ppdSpreadSquare[j][k] += ((sprd_ran)*(sprd_ran)-ppdSpreadSquare[j][k])/(i-2); 
	...
	if (ppdSpreadVol) 
	{
		...
		for (i=0; i< nFwds; i++)
		{
			...
			for (j=0; j<i; j++) {
				...
				dCumVariance = ppdSpreadSquare[i][j]-(Fwds[i]-Fwds[j])*(Fwds[i]-Fwds[j]);
				ppdSpreadVol[i][j] = ppdSpreadVol[j][i]= sqrt( dCumVariance / mat ) ;
								...
								)
{
	...
	double  dApproxStdev,dCoeffAdjustStrike,dCoeff,dAdjStrike,dTemp1,dTemp2,dSum,dBasket0,dBasket1;
...
//		output=dvector(0,2*paths);
		...
		for (i=3;i<2*paths;i++)
		{
			...
			dBasket1=0.0;
				...
				dCashFlow += (dBasket0+DMAX(cp*(dBasket1-InStrikes[1]),0.0)-DMAX(cp*(dBasket1-InStrikes[2]),0.0)-
								InWeights[nFwdsRows-1] * pdFwdsAtMaturity[nFwdsRows-1])*InLvl[k];
...
//		output=dvector(0,2*paths);
		...
		for (i=3;i<2*paths;i++)
		{
			...
			dCashFlow = dBasket0+DMAX(cp*(dBasket1-Strikes[1]),0.0)-DMAX(cp*(dBasket1-Strikes[2]),0.0)-
								...
								double			*dPremium)
{
	...
	if (!pdFwdsAtMaturity || !dDriftDown || !dDriftUp || !dPoints || !dCumProbaValue
		|| !dMeanV || !dCorrCube)
	{
	...
	for (i=0; i<iNbFwd; i++)
	{
		...
		dApproxDrift = -0.5 * (dProba[i] * dVolDown[i] * dVolDown[i] + (1.0 - dProba[i]) * dVolUp[i] * dVolUp[i]);
		...
		for (j=1; j<=lNbPoints2; j++)
		{
			...
			dCumProbaValue[i][j] += (1.0 - dProba[i]) * norm((dPoints[i][j] - dDriftUp[i]) / dVolUp[i]);
	...
	/* loads the cube of correlated variables compatible with the Student Copula for n degrees of freedom */
	for (i=0; i<iNbFwd; i++)
	{
		dMeanV[i] = 0.0;  // resets the means of the gaussian variables to 0		
	}
				 
	 GetStudentCplDev (	iStudDegree,
	  					dMeanV,
						dCorrelation,   
						lNbPaths,
						iStudDegree+iNbFwd, 
...
FREE_RETURN:
	...
	if (dDriftDown) free(dDriftDown);
	if (dDriftUp) free(dDriftUp);
	if (dMeanV) free(dMeanV);
	
	if (dPoints) free_dmatrix(dPoints, 0, iNbFwd-1, 0, lNbPoints2);
	if (dCumProbaValue) free_dmatrix(dCumProbaValue, 0, iNbFwd-1, 0, lNbPoints2);
	if (dCorrCube) free_dmatrix(dCorrCube, 0, lNbPaths, 0, iNbFwd-1);

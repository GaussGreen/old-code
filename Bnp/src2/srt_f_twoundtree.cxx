...
#define ALLOC_MEM_ASSETS                                                       \
  cur_assets = f3tensor(-maxtrinf.max_index[0], maxtrinf.max_index[0],         \
                        ...
                        node_dim - 1);                                         \
  next_assets =                                                                \
      f3tensor(-maxtrinf.max_index[0], maxtrinf.max_index[0],                  \
               -maxtrinf.max_index[1], maxtrinf.max_index[1], 0, node_dim - 1)
...
Err srt_f_twoundtree(SrtGrfnParam *grfnparam, SrtStpPtr stp, GrfnDeal *gd,
                     EvalEventFct evalcf, SrtIOStruct *iolist,
                     SrtUndInfo *und_info) {
  /* ----- declare stuff ---- */
  Err err = NULL;
  SrtUndPtr und1, und2;
  ...
  /* information needed for the model at a particular time for cases LGM-LGM /
   * LGM-BS / BS-BS */
  ...
  /* Gets the models from the FIRST (== DOMESTIC) underlying and initialise the
   * Time Info in the steps accordingly */
  ...
  /* Gets the models from the SECOND underlying and initialise the Time Info in
  ...
  for (stp = bot; stp != NULL; stp = stp->prev) {
    ...
    if (mdl_type1 == LGM) {
      ...
      sam_get(node.cxxur_sam, 0, PHI) = sam_get(tminf_irm1->fwd_sam, 0, PHI);
    } else if ((mdl_type1 == BLACK_SCHOLES) || (mdl_type1 == EQ_STOCH_RATES)) {
    ...
    if (mdl_type2 == LGM) {
      ...
      sam_get(node.cxxur_sam, 1, PHI) = sam_get(tminf_irm2->fwd_sam, 1, PHI);

      /* Quanto correction (including the Delta t) */
      node.quanto_fwd = tminf_irm2->quanto_adjustment *
                        tminf_irm2->ev.onef.sig * stp->delta_t;
    } else if ((mdl_type2 == BLACK_SCHOLES) || (mdl_type2 == EQ_STOCH_RATES)) {
    ...
    for (i = -trinf->max_index[0]; i <= trinf->max_index[0]; i++) {
      ...
      for (j = -trinf->max_index[1]; j <= trinf->max_index[1]; j++) {
        ...
        /* Sets the value of the FIRST state variable and rebuilds the
         * underlying process value */
        if (mdl_type1 == LGM) {
          sam_get(node.cxxur_sam, 0, STATEVAR) = x1;
          sam_get(node.cxxur_sam, 0, SHORT_RATE) =
              sam_get(node.cxxur_sam, 0, STATEVAR) + tminf_irm1->rf.onef.H +
              sam_get(tminf_irm1->fwd_sam, 0, F_0_t);
        } else if ((mdl_type1 == BLACK_SCHOLES) ||
                   (mdl_type1 == EQ_STOCH_RATES)) {
          sam_get(node.cxxur_sam, 0, STATEVAR) = x1;
          sam_get(node.cxxur_sam, 0, SPOT) =
              tminf_log1->ajust_init_fwd_val * exp(x1);
        }

        /* Sets the value of the SECOND state variable and rebuilds the
         * underlying process value */
        if (mdl_type2 == LGM) {
          sam_get(node.cxxur_sam, 1, STATEVAR) = x2;
          sam_get(node.cxxur_sam, 1, SHORT_RATE) =
              sam_get(node.cxxur_sam, 1, STATEVAR) + tminf_irm2->rf.onef.H +
              sam_get(tminf_irm2->fwd_sam, 1, F_0_t);
        } else if ((mdl_type2 == BLACK_SCHOLES) ||
                   (mdl_type2 == EQ_STOCH_RATES)) {
          sam_get(node.cxxur_sam, 1, STATEVAR) = x2;
          sam_get(node.cxxur_sam, 1, SPOT) =
              tminf_log2->ajust_init_fwd_val * exp(x2);
        }

        /* If it is not the last step        , populates the node and computes
         * expectation */
        if (stp->next) {
          /* Compute forward        , connections and probas */
          populate_twound_tree_node(mdl_type1, mdl_type2, stp, &node,
                                    stp->delta_t, stp->next->trinf);

          /* Sets the discount factor in the node if an IRM (if not        , set
           * above) */
          if (mdl_type1 == LGM) {
            /* Adds and Removes the H function to the state var for the cal to
             * Y_t_at_T... */
            node.df =
                exp(-sam_get(node.cxxur_sam, 0, SHORT_RATE) * stp->delta_t);
          }

          /* Compute discounted asset prices at current node */
          twound_tree_expectation(&node, next_assets, cur_assets[i][j],
                                  node_dim);
        }
        if (mdl_type1 == LGM) {
          sam_get(node.cxxur_sam, 0, STATEVAR) += tminf_irm1->rf.onef.H;
  ...
  err = srt_f_IOstructsetpremium((iolist), SRT_NO,
                                 next_assets[0][0][grfnparam->node_dim - 1],
                                 "Two Und Tree");

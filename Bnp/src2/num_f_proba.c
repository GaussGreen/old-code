  ...
  if (p>0.0 && p<1.0)
    {
	  ...
	  if (x>0) s = 1;
          ...
          if (fabs(x)<=0.42)
            {
               ...
               */
              x = fabs(x) - 0.3;
              y = t2t1 + x *(t2t2 + x *(t2t3 + x *(t2t4 + x * 
                                                   (t2t5 + x * t2t6))));
              yd = t2b1 + x *(t2b2 + x *(t2b3 + x *(t2b4 + x * 
                                                    (t2b5 + x * t2b6)))); 
              y /= yd;
              return s*y;
            }
	      ...
	      if (p>0.5) p = 1.0 - p;
              ...
              y = t3t1 + x *(t3t2 + x *(t3t3 + x *(t3t4 + x * 
                                                   (t3t5 + x *(t3t6 + x * t3t7)))));
              yd = t3b1 + x *(t3b2 + x *(t3b3 + x *(t3b4 + x * 
                                                    (t3b5 + x *(t3b6 + x * t3b7)))));
              y = y / yd + x;
              return s*y;
            }
...
{  
  ...
  double c1, c2, cos1, cos2, p, power1, power2, r, r1, root,
  rpower, rs2, rthalf, s1, s1c1, s2, s2c2, sine1, sine2,
  theta1, theta2, twopi;
  double tni[16];
  double result;
 ...
 * term series. Ref : Divgi, d.r., (1979), Calculation of
 * univariate and bivariate normal probability functions.
 * The Annals of Statistics, 7,4,903-910
 *
 * Derived from: NAG Fortran Library G01HAF (Last Revision Mark 14).
 *
 * C version: July 1990, Bode Meduoye, NAG Ltd, Oxford, U.K.
 *
 * Mark 1, 1990
*/
      ...
      if (rho==zero)
      ...
      else
        {
          ...
          if (root==zero || rs2<=LOWFLOOR) /*if rs2 is too small*/
            {                              /*it returns a result for rho=+-1*/
              if (rho<zero)
                ...
                { if (x>y)
                   {result = norm(y);}    /*result=norm(MIN(x,y))*/
                  else
                   {result = norm(x);}   
		}
            }
          else
            {
              ...
              cos1 = fabs(cos1);
              cos2 = fabs(cos2);
              if (cos1>one) cos1 = one;
              if (cos2>one) cos2 = one;
              theta1 = acos(cos1);
              theta2 = acos(cos2);
	       ...
	       */
              ...
              p = p*exp(rs2)/twopi;
              if (c1<zero) p = p + s1*norm(-fabs(x));
              if (c2<zero) p = p + s2*norm(-fabs(y));
              if (s1<zero && s2<zero) p = p + one;
              result = p;
            }
        }
...
double 	 srt_f_intnrm_l(double a,
						double b,
						double ap,
						double bp)

{
	...
	return 	(1.0/bs)*gauss((a-ap)/bs)*norm((bp*a/b+b*ap/bp)/bs);

}



 double srt_f_intnrm_m1	(double a,
						 double b,
						 double ap,
						 double bp)

 {
	 ...
	 return   bp*norm_accurate(a/b)*gauss(ap/bp)+(bp*bp)*srt_f_intnrm_l(-a,b,ap,bp)+ap*srt_f_intnrm_i1(a,b,ap,bp);
 }


 double srt_f_intnrm_m2	(double a,
						 double b,
						 double ap,
						 double bp)
 ...
 {	
	 ...
	 return   bp*norm_accurate(a/b)*gauss(ap/bp)-(bp*bp)*srt_f_intnrm_l(a,b,ap,bp)+ap*srt_f_intnrm_i2(a,b,ap,bp);
 }
...
double trivar( double a1, double a2, double a3, double r12, double r23, double r13 )
{
   double  ro, result;
...
/* ========================================================================
   FUNC: EvalStudDis
   DESC: Computes the  cumulative Student distribution with n degrees of freedom at
		 a given point

		n : n. of degrees of freedom (must be <= 10). 0 corresponds to the Gaussian
			Cumulative distribution
		x:  abscissa where we want to evaluate the Cumulative distribution
...

{
	...
	case 0:   // this is the gaussian case (i.e. it corresponds effectively to Student degree -> Infinity)
		Value=norm(x);
		break;
	...
	case 4:
		Value=0.5+x*(6.+x*x)/2/sqrt((4+x*x)*(4+x*x)*(4+x*x));
		break;
	...
	case 6:
		Value=0.5+x*(135.+30.*x*x+2.*x*x*x*x)/4./sqrt((6.+x*x)*(6.+x*x)*(6.+x*x)*(6.+x*x)*(6.+x*x));
		break;
	...
	case 8:
		Value=0.5+x*sqrt(2+x*x/4.)*(1120+280.*x*x+28.*x*x*x*x+x*x*x*x*x*x)/(8.+x*x)/(8.+x*x)/(8.+x*x)/(8.+x*x);
		break;
	
	case 9:
		alfa=3*x*(67797.+13797.*x*x+1155*x*x*x*x+35*x*x*x*x*x*x);
		beta=35*(9.+x*x)*(9.+x*x)*(9.+x*x)*(9.+x*x);
		Value=0.5+(alfa+beta*atan(x/sqrt(n)))/SRT_PI/beta;
		break;

	case 10:
		Value=0.5+x*(196875.+52500.*x*x+6300.*x*x*x*x+360.*x*x*x*x*x*x+8.*x*x*x*x*x*x*x*x)/16./
			sqrt((10.+x*x)*(10.+x*x)*(10.+x*x)*(10.+x*x)*(10.+x*x)*(10.+x*x)*(10.+x*x)*(10.+x*x)*(10.+x*x));
		break;
...
double Student_Distribution( const double x, const double m, const double sigma )
{
	...
	double A = sgn*exp( gammln(0.5*(m+1.0)) + gammln(0.5*m) )/sqrt(SRT_PI*m);
...
// Put the data for the function in an array
	...
	err = rtsafe_with_par(&Inverse_Student_Diff, LowGuess, HighGuess, acc, 100, x, data);
...
// 
// Inputs:
//		t				value of the Student cumulative distribution
//		m				degree of the student distribution
...
/* ========================================================================
   ...
   DESC: Computes the inverse of the cumulative Student distribution
		 ...
		 degree: N. of degrees of freedom of the Student distribution (must be <=10 )
		         0 correspond to the gaussian Cumulative Inverse distribution
...

{
	...
	case 1:
		...
		break;
	case 2:
			return THETA(tVal,0.5)*sqrt((4*tVal-4*tVal*tVal-1)/(2*tVal*tVal-2*tVal))-
				       THETA(0.5,tVal)*sqrt((4*tVal-4*tVal*tVal-1)/(2*tVal*tVal-2*tVal));
		break;
	...
	if ((fl > 0.0 && fh > 0.0) || (fl < 0.0 && fh < 0.0)) goto end;
...

{
	...
	case 3:
		alfa=sqrt(3.)*x;
		beta=(3+x*x);							
		*fVal=x/sqrt(1.*degree)-tan((SRT_PI*beta*(tVal-0.5)-alfa)/beta);
		*f1Val=6.*sqrt(3.)/SRT_PI/(3.+x*x)/(3.+x*x);
		break;

	case 4:
		alfa=(4.+x*x);
		*fVal=(tVal-0.5)*(tVal-0.5)-x*x*(6.+x*x)*(6.+x*x)/4./alfa/alfa/alfa;
	...
	case 5:
		...
		beta=3.*(5.+x*x)*(5.+x*x);
		*fVal=x/sqrt(1.*degree)-tan((SRT_PI*beta*(tVal-0.5)-alfa)/beta);
		*f1Val=200.*sqrt(5.)/SRT_PI/(5.+x*x)/(5.+x*x)/(5.+x*x);
		break;

	case 6:
		alfa=(6.+x*x);
		*fVal=(tVal-0.5)*(tVal-0.5)-x*x*(135.+30*x*x+2.*x*x*x*x)*(135.+30*x*x+2.*x*x*x*x)/16./
	...
	case 8:
		alfa=(8.+x*x);
		*fVal=(tVal-0.5)*alfa*alfa*alfa*alfa-x*sqrt(2.+x*x/4.)*(1120+280.*x*x+28.*x*x*x*x+x*x*x*x*x*x);
		*f1Val=4480./sqrt(alfa*alfa*alfa*alfa*alfa*alfa*alfa*alfa*alfa);
		break;
	...
	case 10:
		alfa=10.+x*x;
		*fVal=(tVal-0.5)*16.*sqrt(alfa*alfa*alfa*alfa*alfa*alfa*alfa*alfa*alfa)-
				x*(196875.+52500.*x*x+6300.*x*x*x*x+360.*x*x*x*x*x*x+8.*x*x*x*x*x*x*x*x);
		*f1Val=984375./8/sqrt(alfa*alfa*alfa*alfa*alfa*alfa*alfa*alfa*alfa*alfa*alfa);
		break;

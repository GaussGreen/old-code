...
static double Boundaries_Matrix[3][8] = {
    {NSTDBETA, NSTDBETA, NSTDBETA, NSTDBETA, -NSTDBETA, -NSTDBETA, -NSTDBETA, -NSTDBETA},
    {NSTDBETA, NSTDBETA, -NSTDBETA, -NSTDBETA, NSTDBETA, NSTDBETA, -NSTDBETA, -NSTDBETA},
    {-NSTDBETA, NSTDBETA, -NSTDBETA, NSTDBETA, -NSTDBETA, NSTDBETA, -NSTDBETA, NSTDBETA}};
 ...
 */
/*	We apply NSTDBETA standard deviation in this base (vol in this base in equal to 1)
 */
    ...
    double*  eigen_val)
{
    ...
    for (i = 0; i < 8; i++)
    {
        maxmin_dual_global[2][i] =
            (exp((1.0 - beta) * (logSpot + maxmin_dual_global[2][i])) - 1.0) / (1.0 - beta);
    }
    ...
    double*   eigen_val)
{
    ...
    for (step = nstp - 1; step >= 1; step--)
    {
        ...
        if (failed)
        {
            a = ((exp((1.0 - beta[step]) * (fx_fwd[step] + logSpot)) - 1.0) / (1.0 - beta[step]) *
                     sqt_prev_inv -
    ...
    double* res)
{
    ...
    smessage("Phase 4 -discounting, nb max nodes: %d", nb_nodes_tab[nstp - 1][0]);
        ...
        for (i = 0; i < n0; i++)
        {
            ...
            for (j = 0; j < n1; j++)
            {
                ...
                for (k = 0; k < n2; k++)
                {
                    ...
                    expect_node[2] =
                        svijk[2] + e * U2 * (svijk[0] + dom_ifr[step] - svijk[1] - for_ifr[step]) +
                        f / U2;

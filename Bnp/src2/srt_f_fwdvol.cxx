...
Err srt_f_fwdvol_autocal(String model,   /* Model (LGM        , CHEY        ,
                                            CHEY_BETA        , 1F or 2F) */
                         ...
                         String fwd_vol_type)  /* Lognormal or normal */
{
  ...
  yldcrv = lookup_curve(yc_id);
  if (!yldcrv) {
    return serror("Error in srt_f_fwdvol_autocal        , YC not found: %s",
                  yc_id);
  }
  ...
  if (option_type == RESETCAPFLOOR) {
    ...
    /*	Forward Reset Caplet is the following profile:
            At T4        , pay: MAX (F(T2        ,T3        ,T4) - F(T1 ,T3 ,T4)
       , 0) * cvg (T3
             , T4) Where	T1 = vol start date T2 = vol end date T3 = fra
       start date T4 = fra end date */
 ...
 *				: rate_tenor		- maturity of the
 *desired rate : fwd_cms_start fwd_cms_end 		- forward rates at for
 *start and end dates CMS adjusted : level_slope_correl : slope_vol
 *- correl level/slope and vol of the slope used for adj : GetVolTenor	    -
 *function that enables to retrieve implied vols : vol_id     		- ivol
 *curve identification : vol_type			- "normal" or
 *"lognormal"
 *

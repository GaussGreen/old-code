...
Err merge_all_term_structure(int num_sig, double *sig_time, double *dom_sig,
                             double *for_sig, int num_fx_vol,
                             double *fx_vol_time, double *fx_vol,
                             int corr_n_times, double *corr_times,
                             double *correl_dom_for, double *correl_dom_fx,
                             double *correl_for_fx, int *sigma_n,
                             double **sigma_time, double **dom_sigma,
                             double **for_sigma, double **fx_sigma,
                             double **domfor_rho, double **quanto_rho) {
  int i, j;
  ...
  *quanto_rho = (double *)calloc(nb_merge_time, sizeof(double));
  if ((!dom_sigma) || (!for_sigma) || (!fx_sigma) || (!domfor_rho) ||
      (!quanto_rho)) {
    smessage("memory allocation failed in merge_all_term_structure");
    err = "Memory Allocation Failed in merge_all_term_structure";
    goto FREE_RETURN;
...
Err ctsqto_calib_und(
  ...
  if (ctsqto->num_calls > 0 && !(ctsqto->num_calls == 1 &&
                                 ctsqto->call[0].ex_date <= today + eod_flag)) {
  ...
  /*	Calibrate all */
  err = cpd_calib_all(
      today, get_cash_vol, dom_yc, dom_vc, dom_ref, dom_swap_freq_loc,
      dom_swap_basis, dom_lambda, for_yc, for_vc, for_ref, for_swap_freq,
      for_swap_basis, for_lambda, fx_mkt_vol_date, fx_mkt_vol, num_fx_mkt_vol,
      corr_times, correl_dom_for, correl_dom_fx, correl_for_fx, corr_n_times,
      nex, lex, uselex, dom_end_tenor, for_end_tenor, end_swap, dom_strikes,
      for_strikes, &num_sig, &sig_time, &dom_sig, &for_sig, &num_fx_vol,
      &fx_vol_time, &fx_vol, calibparam,
  ...
  //		corr_n_times        , corr_times        , correl_dom_for        ,
  //correl_dom_fx        , correl_for_fx        ,
  //		&(und->sigma_n)        , &(und->sigma_time)        ,
  //&(und->dom_sigma)        ,
  //&(und->for_sigma)        ,
  //		&(und->fx_sigma)        , &(und->domfor_rho)        ,
  //&(und->quanto_rho));

  err = merge_rates_fx_corr_ts(
      sig_time, dom_sig, num_sig, sig_time, for_sig, num_sig, fx_vol_time,
      fx_vol, num_fx_vol, corr_times, correl_dom_for, correl_dom_fx,
      correl_for_fx, corr_n_times, &(und->sigma_time), &(und->dom_sigma),
      &(und->for_sigma), &(und->fx_sigma), &(und->domfor_rho), &domfx_rho,
      &(und->quanto_rho), &(und->sigma_n));
...
Err ctsqto_payoff_adi(double evt_date, double evt_time, void *func_parm,

                      /* Market data	*/
                      void *dom_yc, void *for_yc,

                      /* Model data	*/
                      double domlam, double forlam, double domphi,
                      double forphi,
  ...
  double *dom_dff = NULL, *dom_gam = NULL, *dom_gam2 = NULL, *for_dff = NULL,
         *for_gam = NULL, *for_gam2 = NULL, *cpn_float_dff = NULL,
         *cpn_float_gam = NULL, *cpn_float_gam2 = NULL;
  ...
  while ((evt_date > (ctsqtostr->call[i]).ex_date) &&
         (i < ctsqtostr->num_calls)) {
    ++i;
  }
  call_idx = i;
  ...
  err = FundLeg_RequestDfDates(fund_leg, (long)(evt_date), &fund_ndfs,
                               &fund_pdates);
  ...
  fee_dom_dff =
      swp_f_df(evt_date, (ctsqtostr->call[call_idx]).ex_date, (char *)dom_yc);
  ...
  for (i = l1; i < u1; i++) {
    for (j = l2; j < u2; j++) {
      ...
      for (l = 0; l < fund_ndfs; l++) {
        fund_dfs[l] =
            fund_dom_dff[l] * exp(-fund_dom_gam[l] * r1[i] - fund_dom_gam2[l]);
      }

      FundLeg_Payoff(fund_leg, (long)(evt_date), (long)(evt_date), fund_dfs,
                     &fund_payoff);
        ...
        if (RA->float_cpn_is_dom_for == 0) {
          for (l = 0; l < 2 * pn_dfs[2]; l++) {
            ra_dfs[2][l] = cpn_float_dff[l] *
                           exp(-cpn_float_gam[l] * r1[i] - cpn_float_gam2[l]);
          }
        } else {
          for (l = 0; l < 2 * pn_dfs[2]; l++) {
            ra_dfs[2][l] = cpn_float_dff[l] * exp(-cpn_float_gam[l] * r2[i][j] -
                                                  cpn_float_gam2[l]);
          }
        }
      }
...
Err ctsqto_payoff_adi2(double evt_date, double evt_time, void *func_parm,
...
Err ctsqto_launch_adi(CTSQTO_STR cstqto, LGMQTO_ADI_ARG adi_arg,
  ...
  err = doublelgm1fQuanto_adi_correl2(
      ...
      adi_arg->domlambda, adi_arg->forlambda, adi_arg->sig_time,
      adi_arg->domsig, adi_arg->forsig, adi_arg->fxsig, adi_arg->nb_sig,
      adi_arg->quantorho, adi_arg->domforrho, adi_arg->void_prm,
      adi_arg->is_event, adi_arg->dom_ifr, adi_arg->for_ifr, adi_arg->dom_yc,
      adi_arg->for_yc, ctsqto_payoff_adi, 1, (double *)temp_val);
...
Err ctsqto_fill_check_all_struct(
  ...
  err = fill_funding_leg(und->today, eod_fix_flag, fund_spot_lag, fund_not,
                         fund_ncpn, fund_fix, fund_start, fund_pay, fund_basis,
                         fund_spr, fund_mrg, ctsqto->fund_leg);
  if (err) {
  ...
  err = ra_init_struct(
      today, dom_yc, for_yc, dom_vc, for_vc, dom_ref, for_ref, ra_not,
      ra_cpn_type, n_fxvol_dates, fxvol_dates, fxvol, qtocorrel, typeVol,
      n_periods, dates, cpns, basis, ra_nfixings, ra_fixingdates, ra_fixings,

      // RA float coupons
      ra_float_refrate_is_dom_for, ra_float_refrate, ra_float_startl,
      ra_float_past_fixings, ra_float_gearings,

      upper_barr, lower_barr, recpay, ra_refrate, c_spread, obs_freq, rho_df,

      // Params for the floating coupon
      correl_start, correl_end, float_adj_strike,

      eod_fix_flag, ctsqto->ra_leg);
  if (err) {
  ...
  if (ncall > 0 && ex_date[ncall - 1] >= und->today + eod_ex_flag) {
    err = ctsqto_fill_calls(und->today, eod_ex_flag, ncall, pay_rec, ex_date,
                            set_date, fee, ctsqto);
    if (err) {
  ...
  if (ctsqto->num_calls > 0 && ctsqto->call) {
    if (use_calib) {
      err = ctsqto_calib_und(
          today, eod_ex_flag, dom_yc, dom_vc, dom_ref, dom_swap_freq,
          dom_swap_basis, for_yc, for_vc, for_ref, for_swap_freq,
          for_swap_basis, forcalib, dom_lambda, for_lambda, dom_force_atm,
          for_force_atm, max_std_long, max_std_short, fix_lambda, one_f_equi,
          skip_last, fx_mkt_vol_date, fx_mkt_vol, num_fx_mkt_vol, corr_times,
          correl_dom_for, correl_dom_fx, correl_for_fx, corr_n_times,
          calibparam, ctsqto, get_cash_vol, und);
      if (err) {
        smessage("Error in calibration");
        goto FREE_RETURN;
      }
    } else {
      err = fill_lgmQto_und(fx3dund, dom_vc, for_vc, dom_ref, dom_swap_freq,
                            dom_swap_basis, for_ref, for_swap_freq,
                            for_swap_basis, und);
      if (err) {
        goto FREE_RETURN;
      }
  ...
  if (ctsqto->num_calls > 0 && ctsqto->call) {
    ...
    err =
        lgmQto_fill_adi_arg(und, ctsqto->num_calls, event_time, (void *)ctsqto,
...
static Err
LGMQTOPhi(int nstept, double *time, double domlam, double forlam,
          ...
          double *domforrho, double *fwd1, double *fwd2, double *std1,
          double *std2, double *corr, double *domphi, double *forphi,
          double *domforphi, double *sigmadom, double *sigmafor,
          double *quantocorrel, double *domforcorrel) {
  Err err = NULL;
  double t1, t2, ta, tb;
  ...
  for (i = 1; i < nstept; i++) {
    ...
    fwd2[i] =
        ...
        (exp(-forlam * t2) * domforI1 -
         exp(-(domlam + forlam) * t2) * domforI2) /
            domlam -
        exp(-forlam * t2) * QAdj;
    ...
    //		std3[i-1] = sqrt(
    //							(domsig[j]/forsig[j])*(domsig[j]/forsig[j]) * forphi[i]
    /// (1-domforrho[j]*domforrho[j])
    //+domforrho[j]*domforrho[j]*domphi[i] / (1-domforrho[j]*domforrho[j])
    //							-2*domforrho[j]*(domsig[j]/forsig[j])*domforphi[i]
    /// (1-domforrho[j]*domforrho[j])
    //							);
...
Err ctsqto_calc_mdl_iv_fwd(CTSQTO_STR ctsqto, LGMQTO_UND und,
  ...
  int i, j, k, index;
  double *time = NULL, *x = NULL, *w = NULL, *domphi = NULL, *forphi = NULL,
         *domforphi = NULL, *r1 = NULL, **r2 = NULL, ***pv = NULL,
         *std1vec = NULL, *std2vec = NULL, *corrvec = NULL, *fwd1 = NULL,
         *fwd2 = NULL;
  ...
  if (!time || !x || !w || !domphi || !forphi || !domforphi || !sigmadom ||
      !sigmafor || !quantocorrel || !domforcorrel || !fwd1 || !fwd2 ||
      !std1vec || !std2vec || !corrvec || !r1 || !r2 || !pv) {
  ...
  /* Phi Calculation */
  err = LGMQTOPhi(ctsqto->num_calls + 1, time, und->dom_lambda, und->for_lambda,
                  und->sigma_time, und->dom_sigma, und->for_sigma,
                  und->fx_sigma, und->sigma_n, und->quanto_rho, und->domfor_rho,
                  fwd1, fwd2, std1vec, std2vec, corrvec, domphi, forphi,
                  domforphi, sigmadom, sigmafor, quantocorrel, domforcorrel);
  ...
  for (i = 0; i < ctsqto->num_calls; i++) {
    ...
    for (j = 0; j < num_hermite; j++) {
      ...
      for (k = 0; k < num_hermite; k++) {
        r2[j][k] = fwd2[i + 1] +
                   std2 * (corr * x[j + 1] + sqrt(1 - corr * corr) * x[k + 1]);
      }
    }
    ...
    err = ctsqto_payoff_adi2(
        call->ex_date, call->ex_time, (adi_arg->void_prm)[index], und->dom_yc,
        und->for_yc, und->dom_lambda, und->for_lambda, domphi[i + 1],
        forphi[i + 1], 0, num_hermite - 1, 0, num_hermite - 1, r1, r2, 1, pv);
    ...
    for (j = 0; j < num_hermite; j++) {
      ...
      for (k = 0; k < num_hermite; k++) {
        ...
        if (i < ctsqto->num_calls - 1) {
          test_for_gam =
              (1.0 - exp(-und->for_lambda * (ctsqto->call[i + 1].ex_time -
                                             ctsqto->call[i].ex_time))) /
              und->for_lambda;
          test_for_gam2 = 0.5 * test_for_gam * test_for_gam * forphi[i + 1];
          test += w[j + 1] *
                  swp_f_df(call->ex_date, ctsqto->call[i + 1].ex_date,
                           und->for_yc) *

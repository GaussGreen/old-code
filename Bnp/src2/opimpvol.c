    ...
    SrtDiffusionType log_or_norm)
{
    ...
    if (log_or_norm == SRT_LOGNORMAL)
    {
        premium = srt_f_optblksch_accurate(fwd, strike, vol, mat, disc_fact, call_put, greek);
    }
    else
    {
        premium = srt_f_optblknrm_accurate(fwd, strike, vol, mat, disc_fact, call_put, greek);
    }
    ...
    double*          implied_vol)
{
    ...
    *implied_vol = 0.0;
    ...
    /* Compute option intrinsic value */
    intrinsic = optval_accurate(
        fwd_price, strike, NULL_VOL, mat, disc, call_put, PREMIUM, lognormal_normal);
    ...
    prem_up =
        optval_accurate(fwd_price, strike, vol_up, mat, disc, call_put, PREMIUM, lognormal_normal);

    prem_down = optval_accurate(
        fwd_price, strike, vol_down, mat, disc, call_put, PREMIUM, lognormal_normal);
    ...
    prem_new = optval_accurate(
        fwd_price, strike, vol_middle, mat, disc, call_put, PREMIUM, lognormal_normal);

    deriv = (optval_accurate(
                 fwd_price,
                 strike,
                 vol_middle + vol_shift,
                 mat,

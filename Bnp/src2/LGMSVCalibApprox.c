...
Err	LGMSV_Get_Rho_From_RhoTarget(double	rho_target,
								 double	rho_mat1,
								 double	rho_mat2,
								 double	LGM_alpha,
								 double	LGM_gamma,
...
double	s1, s2;
	...
	*rho2 = (s1 - s2) * rho_target / LGM_alpha / (exp(-LGM_gamma * rho_mat1) - exp(-LGM_gamma * rho_mat2));
	*rho1 = s2 * rho_target - LGM_alpha * (*rho2) * exp(-LGM_gamma * rho_mat2);
	
	return err;
}

Err	LGMSV_Get_RhoTarget_From_Rho(double	rho1,
								 double	rho2,
								 double	rho_mat1,
								 double	rho_mat2,
								 double	LGM_alpha,
...
double	s1, s2;
	...
	rho_target1 = rho1 * (exp(LGM_gamma * rho_mat1) - exp(LGM_gamma * rho_mat2)) / (exp(LGM_gamma * rho_mat1) * s1 - exp(LGM_gamma * rho_mat2) * s2);	
	rho_target2 = rho2 * LGM_alpha * (exp(-LGM_gamma * rho_mat1) - exp(-LGM_gamma * rho_mat2)) / (s1 - s2);	
...
Err	LGMSV_Get_RhoTargetSensitivity_From_Rho( double	rho1,
											 double	rho2,
											 double	rho_mat1,
											 double	rho_mat2,
											 double	LGM_alpha,
...
double	s1, s2;
	...
	*sensi1 = (exp(LGM_gamma * rho_mat1) - exp(LGM_gamma * rho_mat2)) / (exp(LGM_gamma * rho_mat1) * s1 - exp(LGM_gamma * rho_mat2) * s2);
...
Err		err = NULL;
	...
	for (i=0; i<nex; i++)
	{
		...
		/* first pricing */
		LGMSVClosedFormApprox(	dLambdaX, iNbPWTime, dPWTime, dSigmaTS, dAlphaTS, dLambdaEpsTS, dRhoTS, dTStar,
								dShiftFwd, 1, &dShiftStrike, ex_time[i], coef_vol[i], 1000, coef_meanrev[i], 0, 0, 0,
								NumerParams, &price1);
		...
		if (fabs(ex_lprice[i] - price1) > Precision)
		{
			...
			sig2 =sqrt( (ex_lprice[i] * ex_lprice[i] / price1 / price1 * cum_var_temp - cum_var) / (ex_time[i] - last_time));
...
double			coef_vol[MAX_CPN],
				coef_meanrev[MAX_CPN],
				shift[MAX_CPN];
...
Err				err				= NULL;
	...
	if (power > 0.5)
	{
		...
		{	
			for (j=last_index+1; j<=lSigIndex[i]; j++)
			{
				if (j > 0)
				{
					sumAlpha2 += dAlphaTSNew[j] * dAlphaTSNew[j] * (dPWTimeNew[j] - dPWTimeNew[j-1]);
			...
			if (LambdaEq > 1.0E-16)
			{
				AlphaEq *= sqrt((1.0 - exp(-2.0 * LambdaEq * ex_time[i])) / (2.0 * LambdaEq * ex_time[i]));
			}
	...
	for (i=0; i<nex; i++)
	{
		...
		err = Calculate_HestonEquivalent_LGMSV(	l - j + 1,
												&(cpn_date[j]),
												&(cpn_time[j]),
												&(cpn_cvg[j]),
												&(cpn_df[j]),
	...
	ConvertTS_LGMSV_to_LGM(	iNbPWTimeNew,
							dPWTimeNew,
							dSigmaTS,
							dLambdaX,
							dTStar);
...
FREE_RETURN:
	...
	if (dPWTimeNew) free (dPWTimeNew);

	if (sig) free (sig);
	if (sig_time) free (sig_time);

	if (dSigmaTS) free_dvector(dSigmaTS, 0, iNbPWTimeNew - 1);
	if (dAlphaTSNew) free_dvector(dAlphaTSNew, 0, iNbPWTimeNew - 1);
	if (dLambdaEpsTSNew) free_dvector(dLambdaEpsTSNew, 0, iNbPWTimeNew - 1);
	if (dRhoTSNew) free_dvector(dRhoTSNew, 0, iNbPWTimeNew - 1);
	if (lSigIndex) free_lvector(lSigIndex, 0, num_ex_dates - 1);
...
CALIBINSTRUMENTDLM	CalibInst;
	...
	for (i=0; i<CalibExeSchedule->iNExe; i++)
	{
		...
		for (j=last_index+1; j<=lSigIndex[i]; j++)
		{
			if (j > 0)
			{
				sumAlpha2 += model->dAlpha[j] * model->dAlpha[j] * (model->dPWTime[j] - model->dPWTime[j-1]);
		...
		if (LambdaEq > 1.0E-16)
		{
			AlphaEq *= sqrt((1.0 - exp(-2.0 * LambdaEq * CalibInst->dExeTime)) / (2.0 * LambdaEq * CalibInst->dExeTime));
		}

		if (fabs(CalibInst->dFwdCash - CalibInst->dStrike[iIndexStrike]) > 1.0E-08)
		{
			vol_conv(CalibInst->dNormalVol[iIndexStrike],
					 SABR_STR_NORM,
					 &atm_vol,
					 SABR_ATM_NORM,
					 CalibInst->dFwdCash,
		...
		CalibExeSchedule->dATMPrice[i] = srt_f_optblknrm(	CalibInst->dFwdCash,
										CalibInst->dFwdCash,
										atm_vol,
										CalibInst->dExeTime,
										CalibInst->dLevel,
		...
		CalibExeSchedule->dATMVega[i] = srt_f_optblknrm(	CalibInst->dFwdCash,
										CalibInst->dFwdCash,
										atm_vol,
										CalibInst->dExeTime,
										CalibInst->dLevel,
...
Err LGMSV_calib_const_alpha_and_rho_from_SABR(	AllCalibInstrumentsDLM	*Instruments,
												CTS_MKT					mkt,
												double					*alpha,
												double					*lambda,
												double					*rho,
...
/* old method
	...
	while (iIndexStart < Instruments->iNbInst-1 && Instruments->sCalibExeSchedule->dExeTimes[iIndexStart] < sParams->sabr_calib_min_time)
	{
		iIndexStart++;
	}	
	...
	if (Instruments->iNbInst - iIndexStart> 1)
	{
		...
		*rho = temp / Instruments->iNbInst;
		...
		err = levenberg_marquardt_select(	exercise_times - 1 + iIndexStart,
	...
	else
	{
		...
		{			
			...
			*alpha = sqrt(mkt_alpha[iIndexStart] * 2.0 * (*lambda) * exercise_times[iIndexStart] / (1.0 - exp(-2.0 * (*lambda) * exercise_times[iIndexStart])));
	...
	if (inst_data && Instruments->iNbInst)
	{
		...
		for (i=iIndexStart; i<Instruments->iNbInst; i++)
		{
			inst_data->start_dates_smile[i-iIndexStart] = Instruments->sCalibCpnSchedule->lCpnDate[Instruments->sCalibInst[i].iStartCpn];
			inst_data->end_dates_smile[i-iIndexStart] = Instruments->sCalibExeSchedule->lTheoEndDates [i];
...
long			ex_datel[MAX_INST],
				start_datel[MAX_INST],
				theo_end_datel[MAX_INST],
				act_end_datel[MAX_INST],
				ex_dates[MAX_INST],
				start_dates[MAX_INST],
				theo_end_dates[MAX_INST],
				act_end_dates[MAX_INST];

double			strikel[MAX_INST],
				strikeS1l[MAX_INST],
				strikeS2l[MAX_INST],
				strikeS1s[MAX_INST],
				strikeS2s[MAX_INST],					
...
AllCalibInstrumentsDLM	*AllCalibLongInst		= NULL,
						*AllCalibShortInst		= NULL,
						*AllCalibShortInstOne	= NULL;


void					**AllLongInstruments	= NULL,
						**AllShortInstruments	= NULL;
...
clock_t			time1, time2;
	...
	*rho2 = NULL;
	...
	if (!CalibCpnLongSchedule || !CalibCpnShortSchedule || !CalibExeLongSchedule || !CalibExeShortSchedule ||
		!AllCalibLongInst || !AllCalibShortInst || !AllParams || !model || !AllCalibShortInstOne)
	{
	...
	AllocateCalibExeSchedule(	&(ex_datel[0]),
								&(ex_timel[0]),
								&(start_datel[0]),
								&(theo_end_datel[0]),
								&(act_end_datel[0]),
	...
	err = Reduct_ExeSchedule(	CalibCpnLongSchedule,
								CalibExeLongSchedule,
								cal_datel,
								paraml);
	...
	err = AllocateAllCalibInst(	CalibExeLongSchedule->iNExe,
								nb_long_smile,
								AllCalibLongInst);
	...
	{		
		/* allocation */
		AllocateCalibExeSchedule(	&(ex_dates[0]),
									&(ex_times[0]),
									&(start_dates[0]),
									&(theo_end_dates[0]),
									&(act_end_dates[0]),
		...
		err = Reduct_ExeSchedule(	CalibCpnShortSchedule,
									CalibExeShortSchedule,
									cal_dates,
									params);
		...
		if (!local_calib_params->calib_smile_on_prim)
		{
			if (local_calib_params->calib_alpha || local_calib_params->calib_rho)
			{
				...
				{
					nb_short_smile = 1 + local_calib_params->calib_alpha + local_calib_params->calib_rho;
				}
			}
			else
			{
		...
		err = AllocateAllCalibInst(	CalibExeShortSchedule->iNExe,
									nb_short_smile,
									AllCalibShortInst);
	...
	if ((local_calib_params->use_sabr_calib && (local_calib_params->calib_alpha || local_calib_params->calib_rho || local_calib_params->calib_lameps)) || local_calib_params->calib_lameps)
	{		
		...
		if (local_calib_params->calib_smile_on_prim)
		{
			err = LGMSV_calib_const_alpha_and_rho_from_SABR(	AllCalibLongInst,
																mkt,
																&flat_alpha,
																&flat_lambda,
																&flat_rho,
		...
		{
			err = LGMSV_calib_const_alpha_and_rho_from_SABR(	AllCalibShortInst,
																mkt,
																&flat_alpha,
																&flat_lambda,
																&flat_rho,
	...
	if (iOne2F == 2)
	{
		if (local_calib_params->onefac_rho)
		{
			...
			for (i=0; i<iNbPWTime; i++)
			{
				err = LGMSV_Get_Rho_From_RhoTarget(	dRhoTS[i],
		...
		else
		{
			...
			for (i=0; i<iNbPWTime; i++)
			{
				err = LGMSV_Get_RhoTarget_From_Rho(	dRhoTS[i],
													dRho2TS[i],
													local_calib_params->rho_mat1,
													local_calib_params->rho_mat2,
													dLGMAlpha,
	...
	memcpy(dPWTimeNew, CalibExeLongSchedule->dExeTimes, CalibExeLongSchedule->iNExe * sizeof(double));
	...
	if (!local_calib_params->fix_lambda)
	{
		...
		num_f_concat_vector(&iNbPWTimeNew, &dPWTimeNew, CalibExeShortSchedule->iNExe, CalibExeShortSchedule->dExeTimes);
	}
	...
	if (!dSigmaTS || !dAlphaTSNew || !dLambdaEpsTSNew || !dLvlEpsTSNew || !dRhoTSNew || !dRho2TSNew 
		|| !lSigLongIndex || (!lSigShortIndex && !local_calib_params->fix_lambda))
	{
		err = "Memory allocation faillure (2) in cpd_calibSV_approx";
		goto FREE_RETURN;
	}
	...
	for (i=0; i<iNbPWTimeNew; i++)
	{
		...
		dRhoTSNew[i] = dRhoTS[j];
		if (iOne2F == 2) dRho2TSNew[i] = dRho2TS[j];
	}		
	...
	if (!local_calib_params->fix_lambda)
	{
		for (i=0; i<CalibExeShortSchedule->iNExe; i++)
		{
			lSigShortIndex[i] = Get_Index(CalibExeShortSchedule->dExeTimes[i], dPWTimeNew, iNbPWTimeNew);
		}
	}

	err = init_LGMSV_model(	model,
							today,
							iOne2F,
							iNbPWTimeNew,
							*dLambdaX,
	...
	Fill_Equivalent_ATM_Prices(	model,
								AllCalibLongInst,
								lSigLongIndex,
								0,
								CalibExeLongSchedule);
	...
	{		
		if (ex_zeta[i] > ex_zeta[i-1])
		{
			temp = sqrt ((ex_zeta[i] - ex_zeta[i-1]) / (CalibExeLongSchedule->dExeTimes[i] - CalibExeLongSchedule->dExeTimes[i-1])) / fact;
		...
		{
			smessage ("Diagonal calibration failed at exercise year %.2f - Calibration stopped", CalibExeLongSchedule->dExeTimes[i]);
	...
	err = Initialise_AllParams(	model,
								lSigLongIndex,
								CalibCpnLongSchedule,
								AllCalibLongInst,
								paraml,								
	...
	{	
		...
		for (i=0; i<AllCalibLongInst->iNbInst; i++)
		{
			total_und_long += CalibCpnLongSchedule->lCpnDate[AllCalibLongInst->sCalibInst[i].iEndCpn]
		...
		for (i=0; i<AllCalibShortInst->iNbInst; i++)
		{
			total_und_short += CalibCpnShortSchedule->lCpnDate[AllCalibShortInst->sCalibInst[i].iEndCpn]
	...
	smessage("Total Iterations: %d, Rho2: %d, Lambda: %d, Rho: %d, Alpha: %d",
	...
	if (AllParams->CalibParams->do_calib == 0 ||
		shift_lam ||
		...
		(model->iOne2F == 2 && fabs(local_calib_params->rho2_sv_shift) > 1.0E-08))
	{
		...
		for (i=0; i<model->iNbPWTime; i++)
		{
			...
			if (!local_calib_params->onefac_rho || model->iOne2F == 1)
			{
				...
				if (model->iOne2F == 2) model->dRho2[i] += local_calib_params->rho2_sv_shift;
			}
		...
		if (local_calib_params->onefac_rho && model->iOne2F == 2 &&  fabs(local_calib_params->rho_sv_shift) > 1.0E-08)
		{
			for (i=0; i<model->iNbPWTime; i++)
			{
				err = LGMSV_Get_RhoTarget_From_Rho(	model->dRho[i],
													model->dRho2[i],
													local_calib_params->rho_mat1,
													local_calib_params->rho_mat2,
													model->dInitLGMAlpha,
				...
				err = LGMSV_Get_Rho_From_RhoTarget(	model->dFlatOneFactorRho,
													local_calib_params->rho_mat1,
													local_calib_params->rho_mat2,
													model->dInitLGMAlpha,
													model->dLGMGamma,
	...
	ConvertTS_LGMSV_to_LGM(	model->iNbPWTime,
							model->dPWTime,
							model->dSigma,
							model->dLambdaX,
							model->dTStar);
	...
	*rho = calloc(*num_sig, sizeof(double));	
	if (iOne2F == 2) *rho2 = calloc(*num_sig, sizeof(double));

	if (!(*sig_time) || !(*sig) || !(*alpha) || !(*lambdaeps) || !(*rho) || (iOne2F == 2 && !(*rho2)))
	{
		err = "Memory allocation faillure in LGMSVCalibApprox";
		goto FREE_RETURN;
	}
	...
	if (inst_data)
	{
		...
		{
			inst_data->exer_dates_long[i] = (long) (CalibExeLongSchedule->dExeTimes[i]*DAYS_IN_YEAR 
																			+ EPS_MAT_TO_LONG_DATE_CONVERSION) + today;
			inst_data->start_dates[i] = CalibCpnLongSchedule->lCpnDate[CalibExeLongSchedule->iStartCpn[i]];
			inst_data->end_dates[i] = CalibCpnLongSchedule->lCpnDate[CalibExeLongSchedule->iEndCpn[i]];
		...
		if (!calib_params->fix_lambda)
		{
			...
			{
				inst_data->exer_dates_short[i] = (long) (CalibExeShortSchedule->dExeTimes[i]*DAYS_IN_YEAR 
																			+ EPS_MAT_TO_LONG_DATE_CONVERSION) + today;
				inst_data->start_datess[i] = CalibCpnShortSchedule->lCpnDate[CalibExeShortSchedule->iStartCpn[i]];
				inst_data->end_datess[i] = CalibCpnShortSchedule->lCpnDate[CalibExeShortSchedule->iEndCpn[i]];
...
FREE_RETURN:
	...
	if (dPWTimeNew) free (dPWTimeNew);
	if (dSigmaTS) free (dSigmaTS);
	if (dAlphaTSNew) free (dAlphaTSNew);
	if (dLambdaEpsTSNew) free (dLambdaEpsTSNew);
	if (dLvlEpsTSNew) free (dLvlEpsTSNew);
	if (dRhoTSNew) free (dRhoTSNew);
	if (dRho2TSNew) free (dRho2TSNew);
	if (lSigLongIndex) free_lvector(lSigLongIndex, 0, CalibExeLongSchedule->iNExe - 1);
	if (lSigShortIndex) free_lvector(lSigShortIndex, 0, CalibExeShortSchedule->iNExe - 1);
	...
	if (local_calib_params) free(local_calib_params);
...
Err		err = NULL;
	...
	if (!PricingConst->CoefIntT || !PricingConst->Coef1ReT || !PricingConst->Coef2ReT ||
		...
		!PricingConst->dt || !PricingConst->X || !PricingConst->W ||
		!PricingConst->IntRe || !PricingConst->IntIm)
	{
	...
	if (NumerParams->iIntegMethod == 3 || NumerParams->iIntegMethod == 4)
	{
		...
		{
			if (PricingConst->X) free_dvector(PricingConst->X, 0, PricingConst->iNbX - 1);
			PricingConst->X = NULL;

			if (PricingConst->W) free_dvector(PricingConst->W, 0, PricingConst->iNbX - 1);
			PricingConst->W = NULL;
		}
	}
	else
...
void Free_PricingConst(LGMSV_PRICINGCONST	PricingConst)
{
	...
	if (PricingConst)
	{
		if (PricingConst->CoefIntT) free_dvector(PricingConst->CoefIntT, 0, PricingConst->iNbCoef - 1);
		PricingConst->CoefIntT = NULL;

		if (PricingConst->Coef1ReT) free_dvector(PricingConst->Coef1ReT, 0, PricingConst->iNbCoef - 1);
		PricingConst->Coef1ReT = NULL;

		if (PricingConst->Coef2ReT) free_dvector(PricingConst->Coef2ReT, 0, PricingConst->iNbCoef - 1);
		PricingConst->Coef2ReT = NULL;

		if (PricingConst->Coef2ImT) free_dvector(PricingConst->Coef2ImT, 0, PricingConst->iNbCoef - 1);
		PricingConst->Coef2ImT = NULL;

		if (PricingConst->Coef3ReT) free_dvector(PricingConst->Coef3ReT, 0, PricingConst->iNbCoef - 1);
		PricingConst->Coef3ReT = NULL;

		if (PricingConst->Coef3ImT) free_dvector(PricingConst->Coef3ImT, 0, PricingConst->iNbCoef - 1);
		PricingConst->Coef3ImT = NULL;

		if (PricingConst->dt) free_dvector(PricingConst->dt, 0, PricingConst->iNbCoef - 1);	
		...
		if (PricingConst->XX) free_dvector(PricingConst->XX, 0, PricingConst->iNbX - 1);
		PricingConst->XX = NULL;

		if (PricingConst->WW) free_dvector(PricingConst->WW, 0, PricingConst->iNbX - 1);
		PricingConst->WW = NULL;

		if (PricingConst->IntRe) free_dvector(PricingConst->IntRe, 0, PricingConst->iNbX - 1);
		PricingConst->IntRe = NULL;

		if (PricingConst->IntIm) free_dvector(PricingConst->IntIm, 0, PricingConst->iNbX - 1);
...
Err err = NULL;
	...
	{
		NumerInst->iLiborIndex = Get_Index(CalibInst->dLiborFixTime, model->dPWTime, model->iNbPWTime);
		NumerInst->iSwitchIndex = NumerInst->iLiborIndex;

		if (fabs(CalibInst->dLiborFixTime - model->dPWTime[NumerInst->iSwitchIndex]) > 1.0E-08 && CalibInst->dLiborFixTime < CalibInst->dExeTime)
		{
...
int	i;
	...
	NumerInst->dLogShiftFwd = log(fabs(CalibInst->dFwdCash + CalibInst->dSpread + HestonParam->dShift));
...
Free_NumerInst(LGMSV_NUMERINST	NumerInst)
{
	if (NumerInst)
	{
		if (NumerInst->dLogShiftStrike) free(NumerInst->dLogShiftStrike);
		NumerInst->dLogShiftStrike = NULL;
	}
}

Err Initialise_EquiLGM(	LGMSV_MODEL		model,
						LGMSV_EQUILGM	equi_lgm)
{
int	nb_lgm_vol;
Err	err = NULL;
	...
	if (!equi_lgm->cum_var_sv || !equi_lgm->cum_var_lgm || !equi_lgm->exp_fact || !equi_lgm->lgm_vol)
	{
...
Err	Update_EquiLGM(		LGMSV_MODEL		model,
						LGMSV_EQUILGM	equi_lgm)
{
int		i, nb_lgm_vol;
	...
	{
		equi_lgm->exp_fact[i + 1] = exp(-2.0 * model->dLambdaX * (model->dInitTStar - model->dPWTime[i]));
	}
...
Free_EquiLGM(LGMSV_EQUILGM	equi_lgm)
{
	if (equi_lgm)
	{
		if (equi_lgm->cum_var_sv) free (equi_lgm->cum_var_sv);
		equi_lgm->cum_var_sv = NULL;

		if (equi_lgm->cum_var_lgm) free (equi_lgm->cum_var_lgm);
		equi_lgm->cum_var_lgm = NULL;

		if (equi_lgm->exp_fact) free (equi_lgm->exp_fact);
		equi_lgm->exp_fact = NULL;
		
		if (equi_lgm->lgm_vol) free (equi_lgm->lgm_vol);
		equi_lgm->lgm_vol = NULL;
	}
}

Err	Calculate_SV_Sensi(	LGMSV_MODEL				model,
						ALLCALININSTRUMENTSDLM	AllCalibInst,
	...
	for (i=0; i<AllCalibInst->iNbInst; i++)
	{
		...
		for (k=0; k<3; k++)
		{
			switch (k)
			{
				...
				{
					/* Equi Alpha and Rho */
					alpha = 0.5 * model->dAlpha[0] * sqrt((1.0 - exp(-model->dLambdaEps[0] * Inst->dExeTime)) 
				...
				{
					/* Equi Alpha and Rho */
					alpha = 0.5 * (model->dAlpha[0] + alpha_shift) * sqrt((1.0 - exp(-model->dLambdaEps[0] * Inst->dExeTime)) 
								/ (model->dLambdaEps[0] * Inst->dExeTime));
					break;
				}
				case 2:
			...
			err = srt_f_optsarbvol(	Inst->dFwdCash,
									Inst->dFwdCash,
									Inst->dExeTime,
									Inst->dATMVol,
									alpha,
			...
			for (j=0; j<Inst->iNbStrike; j++)
			{
				...
				price = srt_f_optblknrm(Inst->dFwdCash,
										Inst->dStrike[j],									
										price,
										Inst->dExeTime,
										Inst->dLevel,
	...
	if (CalibParams->calib_rr_bt)
	{
		if (CalibParams->calib_alpha)
		{
			*sensi_alpha = ((alpha_price[1] + alpha_price[2] - 2.0 * alpha_price[0])
							- (init_price[1] + init_price[2] - 2.0 * init_price[0])) / alpha_shift;
		}

		if (CalibParams->calib_rho || CalibParams->calib_rho2)
		{
			*sensi_rho = ((rho_price[1] - rho_price[2])
							- (init_price[1] - init_price[2])) / rho_shift;
		}
	}
	else
	{
...
Err Initialise_AllParams(	LGMSV_MODEL				model,
...
Err	err = NULL;
	...
	if (!AllParams->HestonLongInst || !AllParams->NumerLongInst || !AllParams->lSigLongIndex ||
		(!AllParams->HestonShortInst && !CalibParams->fix_lambda) || (!AllParams->NumerShortInst && !CalibParams->fix_lambda) || (!AllParams->lSigShortIndex && !CalibParams->fix_lambda) ||
		!AllParams->PricingConst || !AllParams->CalibParams || !AllParams->EquiLGM ||
		!AllParams->CalibParamsLambda || !AllParams->CalibParamsRho || !AllParams->CalibParamsAlpha || !AllParams->CalibParamsRho2)
	{
	...
	if (!CalibParams->fix_lambda || CalibParams->calib_alpha || CalibParams->calib_lameps || CalibParams->calib_rho || CalibParams->calib_rho2)
	{
	...
	err = Initialise_CalibParams(	paraml->use_jumps,
									paraml->precision,
									paraml->nb_iter_max,
									0,
									calib_vol,
	...
	err = Initialise_CalibParams(	params->use_jumps,
									params->precision,
									params->nb_iter_max,
	...
	err = Initialise_CalibParams(	paraml->smile_use_jumps,
	...
	err = Initialise_CalibParams(	params->smile_use_jumps,
	...
	for (i=0; i<AllCalibLongInst->iNbInst; i++)
	{		
		Calculate_HestonEquivalent_LGMSV_FromLGM_struct(&(AllCalibLongInst->sCalibInst[i]),
														CalibLongCpnSchedule,
														model,
														&(AllParams->HestonLongInst[i]));

		Calculate_ShiftAndVol_LGMSV_FromLGM_struct(	&(AllCalibLongInst->sCalibInst[i]),
													CalibLongCpnSchedule,
													model,
													&(AllParams->HestonLongInst[i]));
	...
	if (!CalibParams->fix_lambda)
	{
		memcpy(AllParams->lSigShortIndex, lSigShortIndex, AllCalibShortInst->iNbInst * sizeof(long));
		AllParams->iNbShortInst = AllCalibShortInst->iNbInst;

		for (i=0; i<AllCalibShortInst->iNbInst; i++)
		{			
			Calculate_HestonEquivalent_LGMSV_FromLGM_struct(&(AllCalibShortInst->sCalibInst[i]),
															CalibShortCpnSchedule,
															model,
															&(AllParams->HestonShortInst[i]));

			Calculate_ShiftAndVol_LGMSV_FromLGM_struct(	&(AllCalibShortInst->sCalibInst[i]),
														CalibShortCpnSchedule,
														model,
														&(AllParams->HestonShortInst[i]));
	...
	if (CalibParams->calib_alpha || CalibParams->calib_rho || CalibParams->calib_rho2)
	{
		...
		else
		{
			if (model->iOne2F == 2)
			{
				err = LGMSV_Get_RhoTargetSensitivity_From_Rho(	model->dRho[0],
																model->dRho2[0],
...
Err	err = NULL;
	...
	if (AllParams)
	{
		if (AllParams->lSigLongIndex) free (AllParams->lSigLongIndex);
		...
		if (AllParams->LongInstParams)
		{
			for (i=0; i<AllParams->iNbLongInst; i++)
			{
				if (AllParams->LongInstParams[i]) free (AllParams->LongInstParams[i]);
			}
		...
		if (AllParams->HestonLongInst) free (AllParams->HestonLongInst);
		AllParams->HestonLongInst = NULL;

		if (AllParams->NumerLongInst) free (AllParams->NumerLongInst);
		AllParams->NumerLongInst = NULL;			

		if (AllParams-> CalibParams && !AllParams->calib_params->fix_lambda)
		{
			if (AllParams->lSigShortIndex) free (AllParams->lSigShortIndex);
			...
			if (AllParams->ShortInstParams)
			{
				for (i=0; i<AllParams->iNbShortInst; i++)
				{
					if (AllParams->ShortInstParams[i]) free (AllParams->ShortInstParams[i]);
				}
			...
			if (AllParams->HestonShortInst) free (AllParams->HestonShortInst);
			AllParams->HestonShortInst = NULL;

			if (AllParams->NumerShortInst) free (AllParams->NumerShortInst);
			AllParams->NumerShortInst = NULL;
		}

		Free_PricingConst(AllParams->PricingConst);
		if (AllParams->PricingConst) free (AllParams->PricingConst);
...
Err LGMSVBumpLambdaModel(	void		*model_,
							int			iIndex,
							double		dNewValue)
{
...
Err err = NULL;
	...
	Calculate_HestonEquivalent_LGMSV_FromLGM_struct(&(AllCalibInst->sCalibInst[iIndex]),
	...
	err = Calculate_ShiftAndVol_LGMSV_FromLGM_struct(	CalibInst,
														AllCalibInst->sCalibCpnSchedule,
														model,
														&(AllParams->HestonShortInst[iIndex]));

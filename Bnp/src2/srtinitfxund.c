/* ==========================================================================
   ...
   PURPOSE:   Initialise an FX underlying and stores it in the underlying list 
              The Underlying name is optional: if none is passed, the default
			  name will come from the currencies, as "FOR/DOM".
...
/* Get the underlyings/curves currencies and check for Curve/Model consistency */
	...
	/* The first name corresponds to a yield curve */
		...
		if (!crv)
			return serror("With FX Black-Scholes, the first curve should be a YC");
	...
	/* The second name corresponds to a yield curve */
		...
		if (!crv)
			return serror("With FX Black-Scholes, the second curve should be a YC");
		if (!ISCURVETYPE(crv, YIELD_CURVE))
			return serror("With FX Black-Scholes, the second curve should be a YC");
	/* Get its currency */
	...
	if (modelType == FX_STOCH_RATES)
	{
	/* The first name corresponds to an Interest Rate Underlying */
		dom_und = lookup_und(domDiscName);
		if (!dom_und)
			return serror("With Fx_Stoch_Rates, the first curve should be an UNDERLYING");
	...
	/* The second name corresponds to an Interest Rate Underlying */
		...
		if (!for_und)
			return serror("With Fx_Stoch_Rates, the second curve should be an UNDERLYING");
...
/* Initialise the Underlying Term Structure of Volatilities */
	if (err  = srt_f_init_FX_TermStruct(
					today,

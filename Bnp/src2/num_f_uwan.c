...
static long ipow(long i, long j)
{
    return DTOL(pow(i, j));
}
...
static long   new_path, dim, num_segment, num_square, square_num, *axis_index = NULL;
...
static long UwanNext(double* x, long* seed)
{
    ...
    {
        err = new_path ? UniformVector(random_vector, dim, seed) : Symetrise(random_vector, dim);
...
/* Initialisation function: path, brownian, step indexes (low and high values) */
...
Err uwan_cube(double*** v, long pl, long ph, long rl, long rh, long sl, long sh, long* seed)
{
    long i, j, k;
    ...
    for (k = 0; k < UWAN_PTH_NUM; k++)
    {
        ...
        for (i = 0; i < UWAN_BRW_NUM; i++)
            for (j = 0; j < UWAN_STP_NUM; j++)
                v[rl + k][rl + i][sl + j] = inv_cumnorm_fast(UWAN_RAN_NUM[i + UWAN_BRW_NUM * j]);
    }

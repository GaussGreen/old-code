...
static GrfnSymbol grfn_symtab[] = {
    ...
    0,
    0,
    "Number of days in a corridor : NUMDAYS(f,spot,bup,bdown,vol,delay,mat,nstep,0->Brwn Brdg, "
    "1->No Smile, 2->Linear Smile, Par0, Par1)",
    "BSN",
    CLLNArithFunc,
    COMLL_A_BLKSCHLSNRM,
    0,
    ...
    0,
    0,
    "Row Sort       : ROWSORT[row, start_col, end_col, index]: Index th largest value (index>=0) "
    "of a row",
    "ROWINTERP",
    CLLNCellFunc,
    COMLL_C_ROWINTERP,
    ...
    0,
    0,
    "ROWINTERP[row, start_col, end_col, x_range (a[...]), x] : Approximate y=f(x) by interpolation "
    "on a row ",
    ...
    0,
    0,
    "SOURCE(opt, expr) : Assign expr as the new PVSource for this column; opt=0(cst),1(disc) "
    "(GRFNCR only)",
    ...
    0,
    0,
    "INTERP(xrange (a[...]), yrange (a[...]), x) : Approximate x by interpolation using Auxiliary "
    "Ranges",
    ...
    0,
    0,
    "PVINTERP(AuxIndex,FirstCol,ColSkip,x) : Interp y(x) from A[AuxIndex,i] and "
    "PV[FirsCol+j*ColSkip]",

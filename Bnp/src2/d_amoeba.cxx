...
static Err amotry(double **p, double *y, double *psum, int ndim,
                  Err (*funk)(double *, double *, void *), int ihi, double fac,
                  double *res, void *comm)
// Extrapolates by a factor fac through the face of the simplex across from the
// high point        , tries it        , and replaces the high point if the new
// point is better.
{
  Err err = NULL;
  int j;
...
Mead. The matrix p[0..ndim][0..ndim-1] is input. Its ndim+1 rows are
ndim-dimensional vectors which are the vertices of the starting simplex. Also
input is the vector y[0..ndim]        , whose components must be preinitialized
...
evaluations taken. comm is a pointer to a communication structure (external
parameters that will be passed to the objective function (3rd parameter)) */
{
  Err err = NULL;
  ...
  *nfunk = 0;
  ...
  for (;;) {
    ...
    // next-highest        , and lowest (best)        , by looping over the
    ...
    // satisfactory.
    if (rtol <
        ftol) { // If returning        , put best point and value in slot 0.
      SWAP(y[0], y[ilo])
      for (i = 0; i < ndim; i++)
        SWAP(p[0][i], p[ilo][i])
      break;
    ...
    *nfunk += 2;
    ...
    // of the simplex across from the high point        , i.e.        , reflect
    ...
    // extrapolation by a factor 2.
    {
      ...
      // for an intermediate lower point        , i.e.        , do a

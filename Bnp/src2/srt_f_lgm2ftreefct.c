/* ----------------------------------------------------------------------------------
   ...
   PURPOSE:  utility functions for the implementation the 2 factor lgm tree in Grfn
...
#define TREE_MESH_SPACING	1.73205080756888 	   /* theoretically > 4  (1.414213562373) */
...
/*	Populates trinf and maxtrinf
	...
	-	The grid is constructed in dual basis with spacing NUMSTDEVINSPACING (in srt_h_trestruct.h)
	-	The grid is trimmed at CUTTREEATSTDEV (srt_h_trestruct.h) 
...
/* Next step, Loop on steps */
	...
	/* Compute Global and Local covariances in original basis */
		...
		/* Local */		
		local_var[0] = prvtminf->ev.twof[0].sig2 * ( (fabs(prvtminf->ev.twof[0].lambda) > EPS) 
				  ?	(1.0 - exp (-2 * prvtminf->ev.twof[0].lambda * cur->prev->delta_t)) / (2 * prvtminf->ev.twof[0].lambda)
				  : cur->prev->delta_t);
		local_var[1] = prvtminf->ev.twof[1].sig2 * ( (fabs(prvtminf->ev.twof[1].lambda) > EPS) 
				  ?	(1.0 - exp (-2 * prvtminf->ev.twof[1].lambda * cur->prev->delta_t)) / (2 * prvtminf->ev.twof[1].lambda)
				  : cur->prev->delta_t);
		local_cov =	 prvtminf->correl_x * prvtminf->ev.twof[0].sig * prvtminf->ev.twof[1].sig 
			  * ( (fabs(prvtminf->ev.twof[0].lambda + prvtminf->ev.twof[1].lambda) > EPS)
				  ?	(1.0 - exp (-(prvtminf->ev.twof[0].lambda + prvtminf->ev.twof[1].lambda) * cur->prev->delta_t))
			...
			SrtDiagTwoFacTreeInfo		*nxttrinf)   /* Info about tree structure at next time step */
{
	...
	/* Connects mid to the closest node to forward and rebuilds the statevar accordingly */
		...
		node->son_statevar[i][NODE_MID] = node->son_index[i][NODE_MID] * nxttrinf->spacing[i];
		node->son_index[i][NODE_MID] = DMAX(min(node->son_index[i][NODE_MID], nxttrinf->max_index[i]), -nxttrinf->max_index[i]); 

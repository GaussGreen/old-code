...
/*	Populates trinf and maxtrinf
    At each time step:
        ...
        -	The grid is constructed in dual basis with spacing NUMSTDEVINSPACING (in
   srt_h_trestruct.h) -	The grid is trimmed at CUTTREEATSTDEV (srt_h_trestruct.h) GLOBAL standard
...
Err lgm2f_trelim(SrtStpPtr stp, SrtDiagTwoFacTreeInfo* maxtrinf)
{
    ...
    double global_var[2],              /* Cov matrix seen from today for current step */
        global_cov, global_var_dir[2], /* Global variance in direction of eigenvectors */
    ...
    while (cur)
    {
        ...
        local_var[0] = prvtminf->ev.twof[0].sig2 *
                       ((fabs(prvtminf->ev.twof[0].lambda) > EPS)
                            ? (1.0 - exp(-2 * prvtminf->ev.twof[0].lambda * cur->prev->delta_t)) /
                                  (2 * prvtminf->ev.twof[0].lambda)
                            : cur->prev->delta_t);
        local_var[1] = prvtminf->ev.twof[1].sig2 *
                       ((fabs(prvtminf->ev.twof[1].lambda) > EPS)
                            ? (1.0 - exp(-2 * prvtminf->ev.twof[1].lambda * cur->prev->delta_t)) /
        ...
        local_cov = prvtminf->correl_x * prvtminf->ev.twof[0].sig * prvtminf->ev.twof[1].sig *
                    ((fabs(prvtminf->ev.twof[0].lambda + prvtminf->ev.twof[1].lambda) > EPS)
                         ? (1.0 - exp(-(prvtminf->ev.twof[0].lambda + prvtminf->ev.twof[1].lambda) *
                                      cur->prev->delta_t)) /
    ...
    SrtDiagTwoFacTreeInfo* nxttrinf) /* Info about tree structure at next time step */
{
    ...
    for (i = 0; i <= 1; i++)
    {
        ...
        node->son_index[i][NODE_MID]    = DMAX(
            min(node->son_index[i][NODE_MID], nxttrinf->max_index[i]), -nxttrinf->max_index[i]);

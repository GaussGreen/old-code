    ...
    long*        lNumTaus)
{
    ...
    {
        last_fixing =
            add_unit(sdparray[i].end, -sdparray[i].spot_lag, SRT_BDAY, MODIFIED_SUCCEEDING);
        ...
        {
            date_list         = SwapDP_to_DateList(&(sdparray[i]), ccy_param->cash_bus_day_conv);
            last_period_start = date_list.date[date_list.len - 2];
            last_fixing =
                add_unit(last_period_start, -sdparray[i].spot_lag, SRT_BDAY, MODIFIED_SUCCEEDING);
        ...
        {
            last_fixing =
                add_unit(sdparray[i].start, -sdparray[i].spot_lag, SRT_BDAY, MODIFIED_SUCCEEDING);
            temp_sig_dates[i] = last_fixing;
        }
    ...
    double* criteria)
{
    ...
    for (i = 1; i <= npath; i++)
    {
        ...
        {
            temp_param[j] = param_bound[1][j] + (param_bound[2][j] - param_bound[1][j]) * sobol[j];
        }
...
)
{
    ...
    if (eModelDim == TWO_FAC)
    {
        ...
        {
            /* Alpha ( = Sig2/Sig1 ) */
            pdOptParams[lNumSigmas + lNumUsedTaus + lNumUsedBetas + lNumUsedOmegas + 1] =
                ppdSigmaValues[3][0] / ppdSigmaValues[1][0];

            /* Gamma  ( = Lam2 - Lam1) */
            pdOptParams[lNumSigmas + lNumUsedTaus + lNumUsedBetas + lNumUsedOmegas + 2] =
                1.00 / ppdTauValues[2][0] - 1.00 / ppdTauValues[1][0];

            /* Rho */
            pdOptParams[lNumSigmas + lNumUsedTaus + lNumUsedBetas + lNumUsedOmegas + 3] =
                ppdSigmaValues[5][0];
        }
    }
    ...
    double       dFixedRho)
{
    ...
    if (eModelDim == TWO_FAC)
    {
        /* The Beta 2 ( = Beta 1 + Omega) term structure is stored in the fourth set of calibration
         * parameters */
...
SRT_Boolean are_calib_parameters_within_band(
    double* pdOptimParams, long lNumParams, double** ppdParamBounds)
{
    long i;

    for (i = 1; i <= lNumParams; i++)
    {
        if ((pdOptimParams[i] < ppdParamBounds[1][i]) || (pdOptimParams[i] > ppdParamBounds[2][i]))
            return SRT_NO;
    }
    ...
    SrtMdlDim      eModelDim)
{
    ...
    if (eModelDim == TWO_FAC)
    {
        ...
        {
            /* Alpha */
            ppdParamBounds[1][lNumSigmas + lNumUsedTaus + lNumUsedBetas + lNumUsedOmegas + 1] =
                psCalibParams->dAlphaMin;
            ppdParamBounds[2][lNumSigmas + lNumUsedTaus + lNumUsedBetas + lNumUsedOmegas + 1] =
                psCalibParams->dAlphaMax;

            /* Beta */
            ppdParamBounds[1][lNumSigmas + lNumUsedTaus + lNumUsedBetas + lNumUsedOmegas + 2] =
                psCalibParams->dBetaMin;
            ppdParamBounds[2][lNumSigmas + lNumUsedTaus + lNumUsedBetas + lNumUsedOmegas + 2] =
                psCalibParams->dBetaMax;

            /* Rho */
            ppdParamBounds[1][lNumSigmas + lNumUsedTaus + lNumUsedBetas + lNumUsedOmegas + 3] =
                psCalibParams->dRhoMin;
            ppdParamBounds[2][lNumSigmas + lNumUsedTaus + lNumUsedBetas + lNumUsedOmegas + 3] =
...
Err srt_f_HellingerDist(
    double** HistSquare, int flag1, double** ModelCorr, int flag2, long dim, double* result)
{
    Err      err = NULL;
    double   temp;
    long     i, j;

...
#define SWAP(a,b)  {double tempr; tempr = (a); (a) = (b); (b) = tempr;} 
...
/* Extract the relevant fixing date for each deal */
	...
	{  
		last_fixing = add_unit(sdparray[i].end, - sdparray[i].spot_lag, 
	...
	/* A cap: gets the start date for the last period : one before last date */
		...
		{
			date_list = SwapDP_to_DateList(&(sdparray[i]),
											ccy_param->cash_bus_day_conv);
			last_period_start = date_list.date[date_list.len-2];
			last_fixing = add_unit(last_period_start, - sdparray[i].spot_lag, 
					SRT_BDAY, MODIFIED_SUCCEEDING);
	...
	/* A swaption : extract the fixing date of the underlying swap */
		{
			last_fixing = add_unit(sdparray[i].start, - sdparray[i].spot_lag, 
					SRT_BDAY, MODIFIED_SUCCEEDING);
			temp_sig_dates[i] = last_fixing;
		}
...
/* Removes sig dates that are the same */
	...
	{
		while ((temp_sig_dates[i] == temp_sig_dates[i+1]) && (i < *lNumSigmas - 1))
		{
...
/* Removes tau dates that are the same */
	...
	{
		while ((temp_tau_dates[i] == temp_tau_dates[i+1]) && (i < TempNumTaus - 1))
		{
...
/* If there is not at least two instruments with the same sig dates we don't need to find tau */
	if ( TempNumTaus + (*lNumSigmas) > numInstruments )
		(*lNumTaus) = numInstruments - (*lNumSigmas);
	else *lNumTaus = TempNumTaus;
...
/* Free whatever has to be freed */
	if (temp_sig_dates) free_dvector(temp_sig_dates, 0, numInstruments-1);		 		
	temp_sig_dates = NULL;
	if (temp_tau_dates) free_dvector(temp_tau_dates, 0, numInstruments-1);		 		
	temp_tau_dates = NULL;
...
Err find_taudates_from_deals(SwapDP *sdparray,  
...
double *temp_param;
	...
	if (!temp_param)
	{
		if (sobol) free_dvector(sobol, 1, numParams);
...
/* Initialise the Sobol sequence */
	...
	if (err)
	{
		if (temp_param) free_dvector(temp_param, 1, numParams);
		if (sobol) free_dvector(sobol, 1, numParams);		
...
/* -----------------------------------------------------------------------
	...
	The SigmaValues array should look like this (in input):
			  [0]	[1]		[2]		[3]		[4]		[5]
			  Date	Sig1	Beta1	Sig2	Beta2	Rho
	The TauValues array should look like this (in input):
			  [0]	[1]		[2]	 	
			  Date	Tau1	Tau2
	The initial set of calibration parameters should look like this (in output):
	[ Sigmas ][ Tau ] [ Beta ]  [ Omega] [ Alpha] [ Gamma ] [ Rho ]
    ----------------------------------------------------------------------- */
...
/* The third set of calibration parameters is given by BEta (power) */
	...
	{
		pdOptParams[lNumSigmas+lNumUsedTaus+i+1] = 1.00 / ppdSigmaValues[2][i];
	}

/* For the Two factor model, more parameters might be calibrated */
	if (eModelDim == TWO_FAC)
	{
	/* The fourth set of calibration parameters is given by Omega (diff in Betas) */
		for (i=0; i<lNumUsedOmegas; i++)
		{
			pdOptParams[lNumSigmas+lNumUsedTaus+lNumUsedBetas+i+1] = ppdSigmaValues[4][i] - ppdSigmaValues[2][i];
		}
	...
	/* For a Global calibration, the correlation has to be calibrated */
		...
		{
		/* Alpha ( = Sig2/Sig1 ) */
			pdOptParams[lNumSigmas+lNumUsedTaus+lNumUsedBetas+lNumUsedOmegas+1] = ppdSigmaValues[3][0] 
				/ ppdSigmaValues[1][0];
		
		/* Gamma  ( = Lam2 - Lam1) */
			pdOptParams[lNumSigmas+lNumUsedTaus+lNumUsedBetas+lNumUsedOmegas+2] = 1.00 / ppdTauValues[2][0] 
				-1.00 / ppdTauValues[1][0];
		
		/* Rho */
			pdOptParams[lNumSigmas+lNumUsedTaus+lNumUsedBetas+lNumUsedOmegas+3] = ppdSigmaValues[5][0];
		}
	}
...
/* -----------------------------------------------------------------------
	...
	The initial set of calibration parameters should look like this (in input):
	[ Sigmas ][ Tau ] [ Beta ]  [ Omega] [ Alpha] [ Gamma ] [ Rho ]
    The SigmaValues array should look like this (in output):
			  [0]	[1]		[2]		[3]		[4]		[5]
			  Date	Sig1	Beta1	Sig2	Beta2	Rho
	The TauValues array should look like this (in output):
			  [0]	 [1]    [2]	 	
			  Date	 Tau1	Tau2
	----------------------------------------------------------------------- */
...
/* The Sigma 1 term structure is stored in the first set of calibration parameters */
	for (i = 0; i < lNumSigmas; i++)
	{
		ppdSigmaValues[1][i] = pdOptParams[i+1];
	}
	
/* The Tau (=1/Lambda) Term structure is stored in the second set of calibration parameters */
...
/* The Beta 1 term structure is stored in the third set of calibration parameters */
...
/* For Two factor models: correlation parameters */
	if (eModelDim == TWO_FAC)
	{
	/* The Beta 2 ( = Beta 1 + Omega) term structure is stored in the fourth set of calibration parameters */
		...
		if (bOneOmega == SRT_YES)
		{
		/* Only a Single Omega has been optimised */
			lNumUsedOmegas = 1;
			for (i = 0; i < lNumSigmas; i++)
			{
				ppdSigmaValues[4][i] = ppdSigmaValues[2][i] + pdOptParams[lNumSigmas+lNumUsedTaus+lNumUsedBetas+1];;
			}
		}
		else
		if (bOneOmega == SRT_NO)
		...
		/* The Entire Omega Term Strucutre has been optimised */
			...
			for (i = 0; i < lNumSigmas; i++)
			{
				ppdSigmaValues[4][i] = ppdSigmaValues[2][i] + pdOptParams[lNumSigmas+lNumUsedTaus+lNumUsedBetas+i+1];;
			}
		}
	...
	/* The Correlation Parameters (ALpha, Gamma and Rho) */
		...
		/* The parameters have been calibrated */
			dAlpha = pdOptParams[lNumSigmas+lNumUsedTaus+lNumUsedBetas+lNumUsedOmegas+1];
			dGamma = pdOptParams[lNumSigmas+lNumUsedTaus+lNumUsedBetas+lNumUsedOmegas+2];
			dRho   = pdOptParams[lNumSigmas+lNumUsedTaus+lNumUsedBetas+lNumUsedOmegas+3];
		...
		if (eCalibType == FIXED_CALIB)
		{
		/* Rebuilds the Sigma 2 and Rho Term Structures using the Fixed Alpha and Rho*/
			for (i=0; i<lNumSigmas; i++)
			{
		...
		/* Rebuilds the Tau 2 (Lam2 = Lam 1 + Gamma) Term Struct using the Fixed Gamma */
			for (i=0; i<lNumTaus; i++)
			{
...
/* For two factor model only */
	...
	/* Bounds for Omega */
		...
		{
			ppdParamBounds[1][lNumSigmas+lNumUsedTaus+lNumUsedBetas+i] = psCalibParams->dBetaMin;
			ppdParamBounds[2][lNumSigmas+lNumUsedTaus+lNumUsedBetas+i] = psCalibParams->dBetaMax;
		}
	...
	/* The correlation parameters have to be calibrated too when GLOBAL */
		...
		{
		/* Alpha */
			ppdParamBounds[1][lNumSigmas+lNumUsedTaus+lNumUsedBetas+lNumUsedOmegas+1] = psCalibParams->dAlphaMin;
			ppdParamBounds[2][lNumSigmas+lNumUsedTaus+lNumUsedBetas+lNumUsedOmegas+1] = psCalibParams->dAlphaMax;

		/* Beta */
			ppdParamBounds[1][lNumSigmas+lNumUsedTaus+lNumUsedBetas+lNumUsedOmegas+2] = psCalibParams->dBetaMin;
			ppdParamBounds[2][lNumSigmas+lNumUsedTaus+lNumUsedBetas+lNumUsedOmegas+2] = psCalibParams->dBetaMax;

		/* Rho */
			ppdParamBounds[1][lNumSigmas+lNumUsedTaus+lNumUsedBetas+lNumUsedOmegas+3] = psCalibParams->dRhoMin;
			ppdParamBounds[2][lNumSigmas+lNumUsedTaus+lNumUsedBetas+lNumUsedOmegas+3] = psCalibParams->dRhoMax;
...
/* -----------------------------------------------------------------------
	...
	For Option prices, pdMarketWeights = pdMktVega * sqrt( n * dOptionsWeight ).
...
/* Market instruments weights */
	for (i=1; i <= lNumInstruments; i++)
	{
		...
		{
			pdMarketWeights[i] = pdMktVega[i-1] * sqrt((double) lNumInstruments / dOptionsWeight);
		}
		else
		{
...
/* Correlation weights */
	...
	for (i=1; i <= lNumCorrel; i++)
	{
		...
		{
			pdMarketWeights[lNumInstruments+i] = sqrt((double) lNumCorrel / dCorrelWeight);
		}
		else
		{
...
/* Smooth Sigma */
	if (bSmoothSigma == SRT_YES)
	{
		...
		{
			pdMarketWeights[lNumInstruments+lNumCorrel+1] = 1.0 / sqrt (bSmoothSigmaWeight);
		}
		else
		{
		...
		double         *pdMarketTargets)           /* From [1] to [lNumData] */
{
	...
	for (i=0; i<lNumTenors; i++)
	{
		...
		{
			pdMarketTargets[lNumInstruments+i*lNumTenors+j+1] = ppdCorrelationMatrix[i][j];
		}
	}
	/* smooth sig */
	if (bSmoothSigma == SRT_YES)
...
/* Rebuilds a fake LGM TS by a quick Normal approximation of the Volatility */
	...
	/* Computes the FRA at the volatility date */
		dFra = swp_f_zr ( 
					(Ddate) (ppdModelSigmaCurve[0][i]),
					(Ddate) (ppdModelSigmaCurve[0][i]) + 14.0,
					szYieldCurveName);
...
/* Rebuilds a Model TS by a the inverse of the quick Normal approximation of the Volatility */
	for (i=0; i<lNumSigmas; i++)
	{
	/* Computes the FRA at the volatility date */
		dFra = swp_f_zr ( 
...
Err srt_f_SquareRootSymMatrix(double **A, long dim, double **result)
{
	...
	for (i=0;i<dim;i++) for (j=0;j<dim;j++) result[i][j] += eigen_vec[i][j]*eigen_val[j];

	
	if (eigen_val) free_dvector(eigen_val,0,dim);
	eigen_val = NULL;
	if (eigen_vec) free_dmatrix(eigen_vec,0,dim,0,dim);
	eigen_vec = NULL;
...
/* Flag == 0 means the input is already a Square root Matrix of a Sym Matrix */
	...
	{
		for (i=0;i<dim;i++) for(j=0;j<dim;j++) pdSquareRoot1[i][j] = HistSquare[i][j];
	}
	...
	{
		for (i=0;i<dim;i++) for(j=0;j<dim;j++) pdSquareRoot2[i][j] = ModelCorr[i][j];
	}

/* Compute the diff of the two matrix and put the result into pdSquareRoot1 */
	for (i=0;i<dim;i++) for(j=0;j<dim;j++) pdSquareRoot1[i][j] -= pdSquareRoot2[i][j];
	
/* Compute the Trace of the Square of pdSquareRoot1 */
	temp = 0.0;
	for (i=0;i<dim;i++) for (j=0;j<dim;j++) temp += pdSquareRoot1[i][j] * pdSquareRoot1[j][i];
	
	(*result) = temp;

	if (pdSquareRoot1) free_dmatrix(pdSquareRoot1,0,dim,0,dim);
	pdSquareRoot1 = NULL;
	if (pdSquareRoot2) free_dmatrix(pdSquareRoot2,0,dim,0,dim);
	pdSquareRoot2 = NULL;

...
Err SmileLinearRegression(double dFwd, int length, double *strikes,
                          double *volats, double **abc) {
  Err err = NULL;
  int i, j;
  double **xtx;
  double **xty;
...
Err ComputeabcfromMerton(double dFwd, double T, double *param, double *abc) {
  ...
  Wk = 0;
  srt_f_optimpvol(
      optmertonpremium(dFwd, dFwd, sigma, U1, l1, U2, l2, T, "Lognormal"), dFwd,
      dFwd, T, 1.0, SRT_CALL, SRT_LOGNORMAL, &(abc[1]));
  ...
  for (n1 = 0; n1 < N1max; n1++) {
    for (n2 = 0; n2 < N2max; n2++) {
      ...
      probajumps = exp(-(l1 + l2) * T) * pow(l1 * T, n1) * pow(l2 * T, n2) /
                   (fact(n1) * fact(n2));

      if (probajumps > probainf) {
        Sk += probajumps *
              norm((n1 * log(1 + U1) + n2 * log(1 + U2) -
                    (l1 * U1 + l2 * U2) * T - sigma * sigma * T / 2) /
                   (sigma * sqrt(T)));

        Wk += probajumps *
              exp(-(n1 * log(1 + U1) + n2 * log(1 + U2) -
                    (l1 * U1 + l2 * U2) * T - sigma * sigma * T / 2) *
                  (n1 * log(1 + U1) + n2 * log(1 + U2) -
                   (l1 * U1 + l2 * U2) * T - sigma * sigma * T / 2) /
                  (2 * sigma * sigma * T)) /
              sqrt(2 * SRT_PI);
      }
...
Err ComputeVolsfromabc(double dFwd, int length, double *strikes, double *abc,
                       double *volats) {
  Err err = NULL;
  int j;

  for (j = 1; j <= length; j++) {
    volats[j] = abc[1] + abc[2] * log(dFwd / strikes[j]) +
                abc[3] * log(dFwd / strikes[j]) * log(dFwd / strikes[j]);
  }
...
Err priceforcalibnew(double str, double param[], double *value, double *Deriv,
                     int n_param) {
  ...
  if (str == strikes[1]) {
    ...
    if ((param[1] < 0) || (param[1] > 0.5) || (param[2] < 0) ||
        (param[2] > 1) || (param[3] < 0) || (param[3] > 5) || (param[4] > 0) ||
        (param[4] < -1) || (param[5] < 0) || (param[5] > 5))
      out_range = 1;
...
/* Computes the gradient of the price */
Err priceforcalib(double str, double param[], double *value, double *Deriv,
                  int n_param) {
  ...
  if (str == strikes[1]) {
    ...
    if (strcmp(Lognorm, "Normal") == 0) {
      if ((param[1] < 0) || (param[1] > 0.1) || (param[2] < 0) ||
          (param[2] > 0.02) || (param[3] < 0) || (param[3] > 10) ||
          (param[4] > 0) || (param[4] < -0.02) || (param[5] < 0) ||
          (param[5] > 10))
        out_range = 1;
      ...
      if ((param[1] < 0) || (param[1] > 0.5) || (param[2] < 0) ||
          (param[2] > 1) || (param[3] < 0) || (param[3] > 5) ||
          (param[4] > 0) || (param[4] < -1) || (param[5] < 0) || (param[5] > 5))
        out_range = 1;
    ...
    /* Computes the implied vols for all strikes */
    err = optmertonsmile(dFwd, n_strikes, strikes, param, mat, Lognorm,
                         vols /*implied vols*/
    );
    if (err) {
    ...
    for (j = 1; j < (n_param + 1); ++j) {
      ...
      err = optmertonsmile(dFwd, n_strikes, strikes, param_shiftes, mat,
                           Lognorm, value_shiftee /*resulting implied vols*/
      );
      if (err) {
        smessage("Error in optmertonsmile");
        free_dvector(param_shiftes, 1, n_param);
...
Err optcalibmerton(double dFwd_loc, int n_strikes_loc, double *strikes_loc,
                   ...
                   char *Logornorm, double *chisq, long *ia,
                   double *calibvols) {
  double *sig;
  ...
  /* Assignment of weights to the different strikes. More weight is given to
  strikes closer to the ATM. Also  , basically no weight is given to strikes
  further away than 2 sd's from the money.
  */
  i = 1;
  ...
  err = levenberg_marquardt_select(strikes, dvols, sig, n_strikes_loc, param,
                                   ia, nparam, 25, priceforcalib, chisq);
  if (err) {
    smessage("Error in levenberg_marquardt_select");
  } else {
    err = optmertonsmile(dFwd, n_strikes, strikes, param, mat, Logornorm,
                         calibvols);
    if (err)
      smessage("Error in optmertonsmile");
  }
...
/* Functions which finds the parameters which fit the market vols*/
Err optcalibmertonnew(double dFwd_loc, int n_strikes_loc, double *strikes_loc,
                      double *market_vols, double *param, double maturity,
                      char *Logornorm, double *chisq, long *ia,
                      double *calibvols) {
  double *sig;

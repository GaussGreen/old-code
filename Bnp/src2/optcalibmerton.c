...
Err ComputeabcfromMerton(double dFwd, double T, double* param, double* abc)
{
    ...
    Wk       = 0;
    srt_f_optimpvol(
        optmertonpremium(dFwd, dFwd, sigma, U1, l1, U2, l2, T, "Lognormal"),
        dFwd,
        dFwd,
        T,
        1.0,
    ...
    for (n1 = 0; n1 < N1max; n1++)
    {
        ...
        {
            probajumps =
                exp(-(l1 + l2) * T) * pow(l1 * T, n1) * pow(l2 * T, n2) / (fact(n1) * fact(n2));
            ...
            if (probajumps > probainf)
            {
                ...
                Wk += probajumps *
                      exp(-(n1 * log(1 + U1) + n2 * log(1 + U2) - (l1 * U1 + l2 * U2) * T -
                            sigma * sigma * T / 2) *
                          (n1 * log(1 + U1) + n2 * log(1 + U2) - (l1 * U1 + l2 * U2) * T -
                           sigma * sigma * T / 2) /
                          (2 * sigma * sigma * T)) /
                      sqrt(2 * SRT_PI);
            }
...
Err priceforcalibnew(double str, double param[], double* value, double* Deriv, int n_param)
{
    int     i, j, k;
...
/* Computes the gradient of the price */
Err priceforcalib(double str, double param[], double* value, double* Deriv, int n_param)
{
    int     i, j, k;
    double* value_shiftee;
    double* param_shiftes;
    ...
    if (str == strikes[1])
    {
        ...
        /* Computes the implied vols for all strikes */
        err = optmertonsmile(
            dFwd, n_strikes, strikes, param, mat, Lognorm, vols /*implied vols*/
        );
        if (err)
        {
    ...
    double* calibvols)
{
    ...
    */
    ...
    err = levenberg_marquardt_select(
        strikes, dvols, sig, n_strikes_loc, param, ia, nparam, 25, priceforcalib, chisq);
    if (err)
    {
    ...
    {
        err = optmertonsmile(dFwd, n_strikes, strikes, param, mat, Logornorm, calibvols);
        if (err)
            smessage("Error in optmertonsmile");
    }

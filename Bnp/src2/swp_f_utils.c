		  ...
		  int			*len )
{
	...
	if (elements & SE_SPREADS)
	{
		...
		while (act_date > start_date)
		{
			...
			if (tmp == SRT_SPREAD_ERROR) return serror("Error: swp_f_spread");
	...
	{
		dates[k] = start_date;  cpns[k] = -sign;  k++;
		dates[k] = end_date;  cpns[k] = sign;  k++;
	}
...
Err CashFlows_Init(SCashFlows *g, int nex)
{
	...
	if (!g->ex || !g->nmat) return serror("Memory failure");
	g->mat = (double **) calloc(nex, sizeof(double *));
	if (g->mat) memset(g->mat, 0, nex * sizeof(double *));
	else return serror("Memory failure");
	g->cpn = (double **) calloc(nex, sizeof(double *));
	if (g->cpn) memset(g->cpn, 0, nex * sizeof(double *));
	else return serror("Memory failure");
	return NULL;
}
...
Err CashFlows_Free(SCashFlows *g)
{
	...
	if (g->mat) for (i=0; i < g->nex; i++) free(g->mat[i]);
	if (g->cpn) for (i=0; i < g->nex; i++) free(g->cpn[i]);
	free(g->mat);
	free(g->cpn);
	free(g->nmat);
	free(g->ex);

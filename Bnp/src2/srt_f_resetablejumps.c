    ...
    long*   ia)
{
    ...
    if (err = swp_f_vol(
            szVolCurveName, (double)dDatespot, (double)dDatespot3, dFra[0], &atmvol, &lognorm))
    {
    ...
    err = optcalibmerton(
        dFra[0], NMAXSTRIKES, strikes, vols, param, mat, "Lognormal", &chisq, ia, calibvols);
...

{
    ...
    /* First we calibrate the parameters to the caplet smile whose maturity corresponds to the end
    date of the resetable*/

    err = calibtosmile(
        today,
        (long)resetdates[n_resetdates],
        cMarketId,
        szVolCurveName,
        cRefRateCode,
        szYieldCurveName,
    ...
    for (j = n_resetdates - 1; j >= 1; j--)
    {
        ...
        paramatrix[j][3] =
            paramatrix[j + 1][3] / (1 + paramatrix[j + 1][2] * cvg * dFra / (1 + cvg * dFra));
        paramatrix[j][4] = paraminitguess[4];
        paramatrix[j][5] =
            paramatrix[j + 1][5] / (1 + paramatrix[j + 1][4] * cvg * dFra / (1 + cvg * dFra));
...
Err get_param(
    double Fradate, int indexStart, double* resetdates, double** paramatrix, double* paramfra)
{
    Err    err = NULL;
    double datespread;
...
Err get_periodmatandisfriday(
    int indexStart, double* resetdates, double nfixdays, double* periodmat, double* isfriday)
{
    Err    err = NULL;
    double d;
    int    j, index;
    ...
    double* pv)
{
    ...
    /* gets the weights and points form a Hermite Polynomial for the numerical integration made
     * further down*/
    ...
    for (perindex = 1; perindex < n_resetdates; perindex++)
    {
        ...
        if (err)
        {
            free_resetableperiod(
                paramperiod, periodmat, DRStsStart, DRSts, isfriday, nfixdays, driftinterp);
            free_resetablegeneral(weights, points, wn1n2, paramatrix, nherm, n_resetdates);
            return err;
        }
        ...
        cvg = coverage(
            ...
            add_unit(
                (long)resetdates[perindex + 1],
                (int)(12 / float_compounding),
                SRT_MONTH,
                MODIFIED_SUCCEEDING),
            float_basis);
        /* here, we get the parameters corresponding to each fixing day within the period and put
         them in paramperiod which is a matrix with nfixdays rows and 5 columns (for the 5
         parameters)*/

        err = get_fra(
            (long)resetdates[perindex + 1], spotlag, cRefRateCode, szYieldCurveName, &dFradrift);

        drift = paramatrix[perindex][1] * cvg * dFradrift / (1 + cvg * dFradrift);

        for (j = 1; j <= nfixdays; j++)
        {
            err = get_param(periodmat[j], perindex, resetdates, paramatrix, paramperiod[j]);
        ...
        /* maximum values for the number of positive and negative jumps are computed for the
        resetdate (start date) of the period in question */
            ...
            for (n2 = 0; n2 <= N2max; n2++)
            {
                ...
                if (probajumps > probainf)
                {
                    for (n = 1; n <= 2 * nherm; n++)
                    {
                        ...
                        err = DRSCalc(
                            wn1n2, (int)nfixdays, resetmat, paramperiod, driftinterp, DRSts);
                        ...
                        if (err)
                        {
                            free_resetableperiod(
                                ...
                                nfixdays,
                                driftinterp);
                            free_resetablegeneral(
                                weights, points, wn1n2, paramatrix, nherm, n_resetdates);
                            return err;
                        }
    ...
    double* atmvols)
{
    ...
    {
        EndDate =
            add_unit((long)Spotdates[i], 12 / float_compounding, SRT_MONTH, MODIFIED_SUCCEEDING);
        err = swp_f_vol(
            szVolCurveName,
            (double)Spotdates[i],
            (double)EndDate,
...
Err SimulateDRS(double w, int nfixings, double resetmat, double* vols, double* DRSts)
{
    Err err = NULL;
    int i;
    ...
    for (i = 1; i <= nfixings; i++)
    {
        ...
        if (strcmp(VolType, "Flat") == 0)
        {
            swp_f_vol(
                ...
                periodspot,
                EndDate,
                lowerbarrier - callspread,
                &(corridorvols[1]),
                &lognorm);

            swp_f_vol(
                szVolCurveName, periodspot, EndDate, lowerbarrier, &(corridorvols[2]), &lognorm);
...

        {
            ...
            enddate   = add_unit(
                (long)(spotdate + periodmat[i] - periodmat[1]),
                12 / frequency,
                SRT_MONTH,
                MODIFIED_SUCCEEDING);
            ...
            stdevoriginal =
                ATMVol * Fwd *
                sqrt(
                    (add_unit(startdate, -spotlag, SRT_BDAY, MODIFIED_SUCCEEDING) - today) /
                    365.25);
            stdevactual =
                ATMVol * DRSts[i] *
                sqrt(
            ...
            swp_f_vol(
                ...
                startdate,
                enddate,
                ((lowerbarrier - callspread) - DRSts[i]) * stdevoriginal / stdevactual + Fwd,
                &(corridorvols[1]),
                &lognorm);
            ...
            swp_f_vol(
                ...
                startdate,
                enddate,
                ((lowerbarrier + width) - DRSts[i]) * stdevoriginal / stdevactual + Fwd,
                &(corridorvols[3]),
                &lognorm);
            ...
            swp_f_vol(
                ...
                startdate,
                enddate,
                ((lowerbarrier + width + callspread) - DRSts[i]) * stdevoriginal / stdevactual +
                    Fwd,
                &(corridorvols[4]),
                &lognorm);
        }
        ...
        else /* NormSliding*/
        {
            ...
            swp_f_vol(
                ...
                startdate,
                enddate,
                ((lowerbarrier + width) - DRSts[i]) + Fwd,
                &(corridorvols[3]),
                &lognorm);
        ...
        call_lower_eps = srt_f_optblksch(
            ...
            lowerbarrier - callspread,
            corridorvols[1],
            (periodmat[i] - periodmat[1]) / 365.25,
            1.0,
            SRT_CALL,
            PREMIUM);

        call_lower = srt_f_optblksch(
            DRSts[i],
            lowerbarrier,
            corridorvols[2],
            (periodmat[i] - periodmat[1]) / 365.25,
        ...
        if (strcmp(FloatCoupon, "YES") == 0)
        {
            ...
            price += frequencyratio * cvgs[paymentindex] * (DRSts[drsindex] + margin) *
                     (1 + 2 * isfriday[i]) *
        ...
        {
            price += frequencyratio * cvgs[paymentindex] * margin * (1 + 2 * isfriday[i]) *
                     ((call_lower_eps - call_lower) / callspread -
                      (call_upper - call_upper_eps) / callspread);
        }
    }
...
double Price_Time_Swap(
    ...
    double*        VolsInputs,
    double         VolShift,
    double lowerbarrier) /*returns the optimal barrier if OPTIMISED or contains the spread to lower
                            barrier if input*/
{
    double price;
...
Err srt_f_simpleresetable(
    ...
    double* VolsInput, /*If above is "Input", this contains 4 vols:lowercallspread, lower, upper,
                          uppercallspread*/
    char*          FloatCoupon,
    SrtCompounding ResetFrequency,
    double         margin,
    ...
    double*        pv)
{
    ...
    /* gets the weights and points from a Hermite Polynomial for the numerical integration made
     * further down*/
    err = HermiteStandard(points, weights, 2 * nherm);
    ...
    *pv        = 0.0;
    ...
    err = get_DRSts(
        today, (long)EndDate, (int)nfixdays, periodmat, cRefRateCode, szYieldCurveName, DRStsStart);
    ...
    SrtCallPutType CallPut)
{
    ...
    else if (strcmp(VolType, "Flat") == 0)
    {
        ...
        if (CallPut == SRT_STRADDLE)
            ...
            price = srt_f_optblksch(dFra, dStrike, flatvol, mat, 1.0, CallPut, PREMIUM);
    }
    else if (strcmp(VolType, "LogSliding") == 0)
    {
        ...
        if (CallPut == SRT_STRADDLE)
            ...
            price = srt_f_optblksch(dFra, dStrike, forwardvol, mat, 1.0, CallPut, PREMIUM);
    }
    else
    {
        start  = add_unit(spotdate, 12 / float_compounding, SRT_MONTH, MODIFIED_SUCCEEDING);
        ...
        err = swp_f_vol(
            szVolCurveName, start, end, dfirstfra + (dStrike - dFra), &forwardvol, &lognorm);

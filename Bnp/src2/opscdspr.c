    ...
    double rho)
{
    ...
    result = bivar(
        cpx * (-k + mux * matx) / sigx_sqrt,
        duy * (-l + muy * maty) / sigy_sqrt,
        cpx * duy * rho * sqrt(maty / matx));
    result -= bivar(
                  cpx * (-k + mux * matx + 2 * rho * sigx / sigy * l) / sigx_sqrt,
                  duy * (l + muy * maty) / sigy_sqrt,
    ...
    SrtGreekType   greek)
{
    ...
    if (matyz < 0)
    {
        premium = srt_f_optblksch(fwdx, strike, sigx, matx, disc, call_put, PREMIUM);
            ...
            ((scud_type == 1) && (ext_lit == 1)))   /* Up and in */
        {
            ...
            ((scud_type == 0) && (ext_lit == 1)) || /* Down and in*/
    ...
    else
    {
        ...
        premium =
            fwdx * probascudout(mu2x, sigx, matx, k, mu2yz, volyz, matyz, l, cpx, duyz, rhoxyz);

        premium -=
            strike * probascudout(mu1x, sigx, matx, k, mu1yz, volyz, matyz, l, cpx, duyz, rhoxyz);

			...
			)
{
	...
	result -= bivar(cpx * (-k + mux*matx + 2*rho*sigx/sigy*l ) / sigx_sqrt, 
			duy * ( l + muy * maty) / sigy_sqrt,
...
if (matyz < 0)
{
	...
	if (  ((scud_type == 0) && (ext_lit == 1)) ||   /* Down and in*/
...
premium  = fwdx * probascudout(mu2x, sigx, matx, k,
				mu2yz, volyz, matyz, l, 
					cpx, duyz, rhoxyz); 
				 

premium -= strike * probascudout(mu1x, sigx, matx, k,
				mu1yz, volyz, matyz, l, 
					cpx, duyz, rhoxyz); 
...
/****************************     DELTAS 	******************************/
	...
	case DELTAX :
		shift  = fwdx / 10000;
		deltax = (srt_f_optscdspr(	fwdx + shift, 
						fwdy, 
						fwdz, 
						spoty, 
						spotz, 
...
/****************************     GAMMAS 	******************************/
	case GAMMAX :
		shift   = fwdx / 1000;
		gammax = srt_f_optscdspr(	fwdx + shift, 
						fwdy, 
						fwdz, 
						spoty , 
						spotz, 
	...
	case GAMMAZ :
		shift   = spotz / 1000; 
		gammaz = srt_f_optscdspr(	fwdx * (1 + shift/spotz), 
						fwdy , 
						fwdz, 
						spoty , 
						spotz + shift, 
...
/****************************     CROSS GAMMAS 	******************************/
	...
	case GAMMAXY :
		shifty   = spoty / 1000; 
		shiftx   = fwdx / 1000;
		gammaxy  = srt_f_optscdspr(	fwdx + shiftx, 
						fwdy * (1 + shifty/spoty), 
						fwdz, 
						spoty + shifty, 
						spotz , 
	...
	case GAMMAXZ :
		shiftz   = spotz / 1000; 
		shiftx   = fwdx / 1000;
		gammaxz  = srt_f_optscdspr(	fwdx + shiftx, 
						fwdy , 
						fwdz* (1 + shiftz/spotz), 
						spoty , 
						spotz + shiftz, 

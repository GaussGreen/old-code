...
/* -----------------------------------------------------------------------------------------------
        ...
        Calculation of expectations for the definition of the grids
                in f(t        ,Tstar)-f(0        ,Tstar) -
   u(t)*dRho/dAlpha*(Vt-1) in Zt = Vt in Psit=Phit*exp(-2*dLambdaX*(Tstar-t))
        ...
        Orthogonalisation
                ...
                Xt = f(t        ,Tstar)-f(0        ,Tstar) -
   u(t)*dRho/dAlpha*(Vt-1) is such that <dXt        ,dW2(t)>=0
...
static Err LGMSVPDEExpectations_UtPieceWise(
    ...
    double *dLambdaV, double *dLvlV, double *dAlphaV, double *dRho,
    ...
    double *PsitMean, double *PsitStd,

    double *XtMean, double *XtStd,

    double *ZtMean, double *ZtStd, double *ImpliedRhoTimesStdVt) {
  ...
  LGMSVSolFunc1 *FuncVarft = NULL, *FuncVt2Mean = NULL, *FuncVtPsitMean = NULL,
                *FuncPsit2Mean = NULL, *FuncVtftMean = NULL;

  double dVarft, dNewVarft, dVt2Mean, dNewVt2Mean, dPsitMean, dNewPsitMean,
      dVtPsitMean, dNewVtPsitMean, dPsit2Mean, dNewPsit2Mean, dVtftMean,
      dNewVtftMean;
  ...
  if (!lambda || !FuncVarft || !FuncVt2Mean || !FuncVtPsitMean ||
      !FuncPsit2Mean || !FuncVtftMean) {
  ...
  while (iNumTime < iNbTime) {
    ...
    while (dTime[iLocalIndexTime] < dNewTime - 1.0E-12 &&
           dLocalMinTime > dMinTime) {
      iLocalIndexTime++;

      dLocalMinTime = min(dLocalMinTime,
                          dTime[iLocalIndexTime] - dTime[iLocalIndexTime - 1]);
    }
    ...
    FuncVt2Mean->a[0] =
        dAlphaV[iVolIndex] * dAlphaV[iVolIndex] + 2.0 * dLvlV[iVolIndex];
    ...
    for (iPeriodIndex = 0; iPeriodIndex < iNbPeriod; iPeriodIndex++) {
      ...
      while (iNumTime < iNbTime &&
             dTime[iNumTime] < dLastPeriodTime + 1.0E-10) {
        ...
        dTemp = (dNewPsit2Mean - dPsit2Mean) * dDtRatio + dPsit2Mean -
                PsitMean[iNumTime] * PsitMean[iNumTime];
        ...
        /* Mean of f(t        ,Tstar)-f(0        ,Tstar)-
         * u(t)*dRho/dAlpha*(Vt-1) at t knowing t-1 */
        XtMean[iNumTime] = 0;

        /* Std of Yt = f(t        ,Tstar)-f(0        ,Tstar) -
         * u(t)*dRho/dAlpha*(Vt-1) at t knowing t-1 */
        ...
        ImpliedRhoTimesStdVt[iNumTime] =
            dTempVtFtMean / sqrt(PsitMean[iNumTime]);
...
static Err LGMSVPDEExpectations_UtPieceWise_OldVersion(
  ...
  for (iNumTime = 1; iNumTime < iNbTime; iNumTime++) {
    ...
    FuncVt2Mean->a[0] =
        dAlphaVt[iNumTime] * dAlphaVt[iNumTime] + 2.0 * dLvlVt[iNumTime];
    ...
    /* Mean of f(t        ,Tstar)-f(0        ,Tstar)- u(t)*dRho/dAlpha*(Vt-1) at
     * t knowing t-1 */
    XtMean[iNumTime] = 0;
    ...
    /* => YtStd(t) should increase */
    dTemp =
        dVarft + pow(dRatio, 2) * (dVt2Mean - 1.0) - 2.0 * dRatio * dVtftMean;
...
static void LGMSVPDEGridX_UtPieceWise(/* Inputs */
                                      ...
                                      double iNbSigmaMaxRight, double dPercent,
  ...
  if (iNb < 2) {
    ...
    *pIndex0 = *pIndex0 + iNbLeft;
    ...
    if (iNbRight == 1) {
      ...
      /* iNbRight is > than 1 */
      /*dStepRight =
      (iNbSigmaMaxRight*dStd-Grid[iNb1+iNbLeft-1])/(iNbRight-1.0); for
      (iNumStep=iNb1+iNbLeft; iNumStep<iNb; iNumStep++ )
              Grid[iNumStep]=Grid[iNb1+iNbLeft-1]+(iNumStep-(iNb1+iNbLeft)+1)*dStepRight;
      */

      dMax = iNbSigmaMaxRight * dStd;

      dCumulStep = ((dMax - Grid[iNb1 + iNbLeft - 1]) - iNbRight * dStep) *
                   2.0 / (iNbRight * (iNbRight + 1.0));
      if ((dCumulStep < 0) || (IsEqSpacedExtremePoint == LGMSV_TRUE)) {
        ...
        for (iNumStep = iNb1 + iNbLeft; iNumStep < iNb; iNumStep++)
          Grid[iNumStep] = Grid[iNumStep - 1] + dStep +
                           (iNumStep - (iNb1 + iNbLeft) + 1) * dCumulStep;
      }
    }
  }
  /* Force to 0 */
...
static void LGMSVPDEGridZ_UtPieceWise(/* Inputs */
  ...
  if (iNb < 2) {
    ...
    *pIndex0 = 0;
    ...
    if (iNb2 > 0) {
      dCumulStep = ((dMax - Grid[iNb1 - 1]) - iNb2 * dStep) * 2.0 /
                   (iNb2 * (iNb2 + 1.0));
      if ((dCumulStep < 0) || (IsEqSpacedExtremePoint == LGMSV_TRUE)) {
        ...
        for (iNumStep = iNb1; iNumStep < iNb; iNumStep++)
          Grid[iNumStep] =
              Grid[iNumStep - 1] + dStep + (iNumStep - iNb1 + 1) * dCumulStep;
      }
    }
  }
}
...
static void LGMSVPDEGridZ_UtPieceWise_New(/* Inputs */
  ...
  if (iNb < 2) {
    ...
    *pIndex0 = 0;
    ...
    iNb1_g = (int)(iNb1 * (dNbRightSym + iNbSigmaLeft) /
                   (iNbSigmaLeft + iNbSigmaRight));
    /* Force to odd Nb */
...
LGMSVPDEMakePsiGrid_UtPieceWise(/* Inputs */
                                int iNbPsi, double dPsiMean, double dPsiStd,
                                double iNbSigmaPsiGrid,

                                /* Parameter */
                                int IsExtremePoints,
                                double iNbSigmaExtremePoints, int IsPsiBoundExp,

                                /* Outputs */
                                double *GridPsi, int *pIndexPsiMean) {
  /* Declaration of locals variables */
        ...
        case LGMSV_TRUE: {
          if (iNbPsi == 3) {
            ...
            *pIndexPsiMean = 1;
            ...
            dPsiStep = dPsiMean *
                       (exp(iNbSigmaPsiGrid * dLogPsiStd) -
                        exp(-iNbSigmaPsiGrid * dLogPsiStd)) /
                       (iNbPsi - 3);
            ...
            *pIndexPsiMean += 1;
            ...
            GridPsi[0] = dPsiMean * exp(-iNbSigmaExtremePoints * dLogPsiStd);
            GridPsi[iNbPsi - 1] =
                dPsiMean * exp(iNbSigmaExtremePoints * dLogPsiStd);
          }
          break;
        }
    ...
    default: {
      ...
      case LGMSV_TRUE: {
        if (iNbPsi == 3) {
          ...
          GridPsi[0] =
              max(dPsiMean - iNbSigmaExtremePoints * dPsiStd, PsiFloor);
          ...
          *pIndexPsiMean = 1;
          ...
          /* Calculation of the step in Psi grid */
          dPsiStep = (dPsiMean + iNbSigmaPsiGrid * dPsiStd -
                      max(dPsiMean - iNbSigmaPsiGrid * dPsiStd, PsiFloor)) /
                     (iNbPsi - 3);
...
static void LGMSVPDECalculationValueMatrixfromPsit_UtPieceWise(
    /* Inputs */
    int iIndexMinZ, int iIndexMaxZ, int iIndexMinX, int iIndexMaxX,

    int iNbPsi_t, int iNbPsi_t1, int iProductNb,

    double *GridftTstar, double *GridZCenter,

    double *GridPsi_t, double *GridPsi_t1,
    ...
    double dSig_t1, double dLambdaX, double dAlphaV, double dRho,
  ...
  double ***ValueTensorZ, **ValueTensorZX, *ValueTensorZX1, *ValueTensorZX2,
      *ValueTensorZX3;
  ...
  for (iNumZ = iIndexMinZ; iNumZ <= iIndexMaxZ; iNumZ++) {
    ...
    /* dVt =
     * max((GridZCenter[iNumZ]+dZtMean)+dAlpha*dRho/dSigt*GridftTstar[iNumX]
     *       ,dVFloor); */
    ...
    for (iNumX = iIndexMinX; iNumX <= iIndexMaxX; iNumX++) {
      ...
      for (iNumPsi = 0; iNumPsi < iNbPsi_t; iNumPsi++) {
        ...
        /* the applied vol is u(t+dt)        , the Vt used is also the Vt with
         * u(t+dt)
         */
        dPsi_t1 = GridPsi_t[iNumPsi] + dVt;
        dPsi_t1_sq = dPsi_t1 * dPsi_t1;

        /* Find the Index such that GridPsi_t1[index]<dPsi_t1 and
         * GridPsi_t1[index+1]>= dPsi_t1 */
        ...
        if (IsExtrapolationFlat == LGMSV_TRUE) {
          /* Interpolate value */
          if ((iIndexPsi > 1) && (iIndexPsi < iNbPsi_t1 - 3) &&
              (TypeInterpolationPhi == LGMSV_MIXQUADRALINEAR)) {
            /* Quadratic Interpolation  */
            if (dPsi_t1 - GridPsi_t1[iIndexPsi] >
                0.5 * (GridPsi_t1[iIndexPsi + 1] - GridPsi_t1[iIndexPsi])) {
              iIndex1 = iIndexPsi;
            } else {
              iIndex1 = iIndexPsi - 1;
            }
            ...
            for (iNumProduct = 0; iNumProduct < iProductNb; iNumProduct++) {
              ValueTensorNewZX[iNumPsi][iNumProduct] =
                  (dPsi_t1 - GridPsi_t1[iIndexPsi]) /
                      (GridPsi_t1[iIndexPsi + 1] - GridPsi_t1[iIndexPsi]) *
                      (ValueTensorZX[iIndexPsi + 1][iNumProduct] -
          ...
          if ((iIndexPsi > 1) && (iIndexPsi < iNbPsi_t1 - 3) &&
            ...
            for (iNumProduct = 0; iNumProduct < iProductNb; iNumProduct++) {
              ValueTensorNew[iNumZ][iNumX][iNumPsi][iNumProduct] =
                  ValueTensor[iNumZ][iNumX][iIndex1][iNumProduct] *
                      InterpQuadraCoef[iIndex1][0] *
                      (dPsi_t1_sq - InterpQuadraCoef[iIndex1][1] * dPsi_t1 +
                       InterpQuadraCoef[iIndex1][2]) +
                  ValueTensor[iNumZ][iNumX][iIndex2][iNumProduct] *
                      InterpQuadraCoef[iIndex1][3] *
                      (dPsi_t1_sq - InterpQuadraCoef[iIndex1][4] * dPsi_t1 +
                       InterpQuadraCoef[iIndex1][5]) +
                  ValueTensor[iNumZ][iNumX][iIndex3][iNumProduct] *
            ...
            if (iNbPsi_t1 == 1) {
              ...
              for (iNumProduct = 0; iNumProduct < iProductNb; iNumProduct++) {
                ValueTensorNew[iNumZ][iNumX][iNumPsi][iNumProduct] =
                    (dPsi_t1 - GridPsi_t1[iIndexPsi]) /
                        (GridPsi_t1[iIndexPsi + 1] - GridPsi_t1[iIndexPsi]) *
                        (ValueTensor[iNumZ][iNumX][iIndexPsi + 1][iNumProduct] -
...
LGMSVPDECalculationRDriftVar_UtPieceWise(/* Inputs */
                                         ...
                                         double *GridX, double *GridZ,

                                         double dSig_t1,

                                         double dLambdaV, double dLvlV,
                                         double dAlphaV, double dRho,
...
LGMSVPDEJumpUtTreatment(/* Inputs */
                        ...
                        int iIndexMaxZ, int iIndexMinProduct,
                        int iIndexMaxProduct, int iNbPsi,

                        double *GridX, double *GridZ,

                        double dSigt, double dSig_t1, double dAlphaVt,
                        double dAlphaV_t1, double dRhot, double dRho_t1,

                        int TypeInterpolation, int IsExtrapolationFlat,
  ...
  for (iNumZ = iIndexMinZ; iNumZ <= iIndexMaxZ; iNumZ++) {
    ...
    dX = (GridZ[iNumZ] - 1) *
         (dRhot / dAlphaVt * dSigt - dRho_t1 / dAlphaV_t1 * dSig_t1);
      ...
      if (IsExtrapolationFlat == LGMSV_TRUE) {
        ...
        if (iIndexXplus == iIndexMinX - 1) {
          ...
          for (iNumPsi = 0; iNumPsi < iNbPsi; iNumPsi++)
            for (iNumProduct = iIndexMinProduct;
                 iNumProduct <= iIndexMaxProduct; iNumProduct++)
              ValueGridZtXtPsitProduct_t[iNumZ][iNumX][iNumPsi][iNumProduct] =
                  ValueGridZtXtPsitProduct_t1[iNumZ][iIndexMinX][iNumPsi]
                                             [iNumProduct];
        } else if (iIndexXplus == iIndexMaxX) {
          /* Extrapolation flat */
          for (iNumPsi = 0; iNumPsi < iNbPsi; iNumPsi++)
            for (iNumProduct = iIndexMinProduct;
                 iNumProduct <= iIndexMaxProduct; iNumProduct++)
              ValueGridZtXtPsitProduct_t[iNumZ][iNumX][iNumPsi][iNumProduct] =
                  ValueGridZtXtPsitProduct_t1[iNumZ][iIndexMaxX][iNumPsi]
                                             [iNumProduct];
        } else if ((iIndexXplus > 1 + iIndexMinX) &&
                   (iIndexXplus < iIndexMaxX - 2) &&
                   (TypeInterpolation == LGMSV_MIXQUADRALINEAR)) {
          /* Quadratic Interpolation  */
          if (dX_t1 - GridX[iIndexXplus] >
              0.5 * (GridX[iIndexXplus + 1] - GridX[iIndexXplus])) {
            iIndex1 = iIndexXplus;
            ...
            for (iNumPsi = 0; iNumPsi < iNbPsi; iNumPsi++)
              for (iNumProduct = iIndexMinProduct;
                   iNumProduct <= iIndexMaxProduct; iNumProduct++) {
                ValueGridZtXtPsitProduct_t[iNumZ][iNumX][iNumPsi][iNumProduct] =
                    ValueGridZtXtPsitProduct_t1[iNumZ][iIndex1][iNumPsi]
                                               [iNumProduct] *
                        InterpQuadraCoef[iIndex1][0] *
                        (dX_t1 * dX_t1 - InterpQuadraCoef[iIndex1][1] * dX_t1 +
                         InterpQuadraCoef[iIndex1][2]) +
                    ValueGridZtXtPsitProduct_t1[iNumZ][iIndex2][iNumPsi]
                                               [iNumProduct] *
                        InterpQuadraCoef[iIndex1][3] *
                        (dX_t1 * dX_t1 - InterpQuadraCoef[iIndex1][4] * dX_t1 +
                         InterpQuadraCoef[iIndex1][5]) +
                    ValueGridZtXtPsitProduct_t1[iNumZ][iIndex3][iNumPsi]
                                               [iNumProduct] *
                        InterpQuadraCoef[iIndex1][6] *
                        (dX_t1 * dX_t1 - InterpQuadraCoef[iIndex1][7] * dX_t1 +
                         InterpQuadraCoef[iIndex1][8]);
              }
          } else {
          ...
          for (iNumPsi = 0; iNumPsi < iNbPsi; iNumPsi++)
            for (iNumProduct = iIndexMinProduct;
                 iNumProduct <= iIndexMaxProduct; iNumProduct++) {
              ValueGridZtXtPsitProduct_t[iNumZ][iNumX][iNumPsi][iNumProduct] =
                  (dX_t1 - GridX[iIndexXplus]) /
                      (GridX[iIndexXplus + 1] - GridX[iIndexXplus]) *
                      (ValueGridZtXtPsitProduct_t1[iNumZ][iIndexXplus + 1]
                                                  [iNumPsi][iNumProduct] -
                       ValueGridZtXtPsitProduct_t1[iNumZ][iIndexXplus][iNumPsi]
                                                  [iNumProduct]) +
                  ValueGridZtXtPsitProduct_t1[iNumZ][iIndexXplus][iNumPsi]
                                             [iNumProduct];
            }
              ...
              for (iNumProduct = iIndexMinProduct;
                ...
                ValueGridZtXtPsitProduct_t[iNumZ][iNumX][iNumPsi][iNumProduct] =
                    (dX_t1 - GridX[iIndexXplus]) /
                        ...
                        (ValueGridZtXtPsitProduct_t1[iNumZ][iIndexXplus + 1]
                                                    [iNumPsi][iNumProduct] -
                         ValueGridZtXtPsitProduct_t1[iNumZ][iIndexXplus]
                                                    [iNumPsi][iNumProduct]) +
                    ValueGridZtXtPsitProduct_t1[iNumZ][iIndexXplus][iNumPsi]
                                               [iNumProduct];
              }
...
void LGMSVCalculationNbSigMax(
    ...
    double *StdVt, double *ImpliedRhoTimesStdVt,

    /* Parameters */
    LGMSVParam *Params,

    /* Outputs */
    double *GridXNbSigmaMaxRight, double *GridZNbSigmaMaxRight,
    double *GridXNbSigmaMaxLeft, double *GridZNbSigmaMaxLeft) {
  /* Declaration of locals variables */
  int iNumTime;
  ...
  for (iNumTime = 0; iNumTime < iNbTime; iNumTime++) {
    ...
    if (Params->IsNbSigmaMaxXGridLeftAuto == LGMSV_TRUE) {
      GridXNbSigmaMaxLeft[iNumTime] =
          -(dC0_XLeft + dC1_XLeft * dStdVt + dC2_XLeft * dRhoTimesStdVt);
    ...
    if (Params->IsNbSigmaMaxXGridRightAuto == LGMSV_TRUE) {
      GridXNbSigmaMaxRight[iNumTime] =
          (dC0_XRight + dC1_XRight * dStdVt + dC2_XRight * dRhoTimesStdVt);
    ...
    if (Params->IsNbSigmaMaxZGridRightAuto == LGMSV_TRUE) {
      GridZNbSigmaMaxRight[iNumTime] =
          (dC0_ZRight + dC1_ZRight * dStdVt + dC2_ZRight * dRhoTimesStdVt);
...
Err lgmSV_adi_UtPieceWise(
  ...
  /* Declaration of locals variables */
  double *Sigt = NULL, *dLambdaVt = NULL, *dLvlVt = NULL, *dAlphaVt = NULL,
         *dRhot = NULL;
  ...
  Err err = NULL;
  clock_t t1, t2;
  double time_psi = 0.0, time_pde = 0.0, time_jmp = 0.0, time_pay = 0.0;
  ...
  ValueGridXtZtProduct =
      f3tensor(0, iNbZ - 1, 0, iNbX - 1, 0, iMaxNbProduct - 1);

  ValueTensor_t =
      f4tensor(0, iNbZ - 1, 0, iNbX - 1, 0, iNbPsi - 1, 0, iMaxNbProduct - 1);
  ...
  num_f_pde_init_2d_LGMSV_adi(pde, iNbPsi, iNbZ, iNbX, iMaxNbProduct);
  ...
  if (!pde || !Sigt || !dLambdaVt || !dLvlVt || !dAlphaVt || !dRhot || !GridX ||
      !GridZ || !GridPsi_t || !GridPsi_t1 || !PsiMean || !PsiStd || !XtMean ||
      !XtStd || !ZtMean || !ZtStd || !DXDrift || !DZDrift || !DXVar || !DZVar ||
      !r || !ValueGridXtZtProduct || !ValueTensor_t || !ValueTensor_t1 ||
      !InterpQuadraCoef || !InterpQuadraCoefGridX || !GridXNbSigmaMaxRight ||
      !GridZNbSigmaMaxRight || !GridXNbSigmaMaxLeft || !GridZNbSigmaMaxLeft) {
  ...
  if (Params->iUseOldExpect) {
    err = LGMSVPDEExpectations_UtPieceWise_OldVersion(
        ...
        /* For f(t        ,Tstar) diffusion and Xt*/
        dLambdaX, dSigma, dTstar,

        /* definition of u(t) piecewise constant */
        SigTime, Sig, iNbSigTime,

        /* For Vt diffusion */
        dLambdaV, dLvlV, dAlphaV, dRho,

        /* Outputs */
        Sigt, /* Value of u(t) = sig(t) for all t in dtime */
        dLambdaVt, dLvlVt, dAlphaVt, dRhot,

        PsiMean, PsiStd, XtMean, XtStd, ZtMean, ZtStd, ImpliedRhoTimesStdVt);
  } else {
  ...
  LGMSVCalculationNbSigMax(
      ...
      ZtStd, ImpliedRhoTimesStdVt,

      /* Parameters */
      Params,

      /* Outputs */
      GridXNbSigmaMaxRight, GridZNbSigmaMaxRight, GridXNbSigmaMaxLeft,
      GridZNbSigmaMaxLeft);

  /* Making the grid on X */
  LGMSVPDEGridX_UtPieceWise(/* Inputs */
                            iNbX, XtStd[iNbTime - 1],
  ...
  LGMSVPDEGridZ_UtPieceWise(/* Inputs */
                            iNbZ, ZtMean[iNbTime - 1], ZtStd[iNbTime - 1],
  ...
  LGMSVPDEMakePsiGrid_UtPieceWise(/* Inputs */
                                  iNbPsi, PsiMean[iNbTime - 1],
                                  PsiStd[iNbTime - 1], Params->iNbSigmaPhiGrid,
                                  ...
                                  /* Outputs */
                                  GridPsi_t1, &iIndexPsi);

  /* Final Payoff valuation   */
  err =
      payoff_func(/* Time Information */
                  ...
                  /* Martingale Mesure QTstar information */
                  dTstar, /* In time */
                  dAlphaVt[iNbTime - 1], dRhot[iNbTime - 1], Sigt[iNbTime - 1],

                  /* Grid data Information	*/
                  0, iNbPsi - 1, 0, iNbX - 1, 0, iNbZ - 1,

                  GridPsi_t1, GridX, GridZ,
      ...
      for (iNumZ = 0; iNumZ < iNbZ; iNumZ++)
        for (iNumX = 0; iNumX < iNbX; iNumX++)
          for (iNumPsi = 0; iNumPsi < iNbPsi; iNumPsi++)
            for (iNumProduct = 0; iNumProduct < 1; iNumProduct++)
              ValueTensor_t1[iNumZ][iNumX][iNumPsi][iNumProduct] *=
                  GridX[iNumX] + dRhot[iNbTime - 1] * Sigt[iNbTime - 1] /
                                     dAlphaVt[iNbTime - 1] *
                                     (GridZ[iNumZ] - 1.0);
    }
      ...
      for (iNumZ = 0; iNumZ < iNbZ; iNumZ++)
        for (iNumX = 0; iNumX < iNbX; iNumX++)
          for (iNumPsi = 0; iNumPsi < iNbPsi; iNumPsi++)
            for (iNumProduct = 0; iNumProduct < 1; iNumProduct++)
              ValueTensor_t1[iNumZ][iNumX][iNumPsi][iNumProduct] *=
                  pow(GridX[iNumX] + dRhot[iNbTime - 1] * Sigt[iNbTime - 1] /
                                         dAlphaVt[iNbTime - 1] *
                                         (GridZ[iNumZ] - 1.0),
                      2);
    }
  ...
  smessage("Phase 2 -convolution        , NbSigXRightMax: %f ",
           GridXNbSigmaMaxRight[iNbTime - 1]);
  smessage("Phase 2 -convolution        , NbSigXLeftMax: %f ",
           GridXNbSigmaMaxLeft[iNbTime - 1]);
  smessage("Phase 2 -convolution        , NbSigZRightMax: %f ",
           GridZNbSigmaMaxRight[iNbTime - 1]);
  ...
  if (Params->ModeExport != LGMSV_TRUE) {
    ...
    for (iNumTime = iNbTime - 2; iNumTime >= 0; iNumTime--) {
      ...
      iNbPsiGrid_t =
          ...
          (Params->GridPhiOptim == LGMSV_TRUE) *
              (int)((1 - iNumTime / ((double)(iNbTime - 1))) * (iNbPsi - 1) +
                    1.0e-08);
      iNbPsiGrid_t = ((int)(iNbPsiGrid_t / 2)) * 2 + 1;

      if (Params->GridXADIOptim == LGMSV_TRUE) {
        while ((iIndexMinX < iIndexX0 - 1) &&
               (GridX[iIndexMinX + 1] <
                -GridXNbSigmaMaxLeft[iNumTime + 1] * XtStd[iNumTime + 1]))
          iIndexMinX++;

        while ((iIndexMaxX > iIndexX0 + 1) &&
               (GridX[iIndexMaxX - 1] >
                GridXNbSigmaMaxRight[iNumTime + 1] * XtStd[iNumTime + 1]))
          iIndexMaxX--;
      }

      if (Params->GridZADIOptim == LGMSV_TRUE) {
        while ((iIndexMinZ < iIndexZ0 - 1) &&
               (GridZ[iIndexMinZ + 1] <
                1.0 - GridZNbSigmaMaxLeft[iNumTime + 1] * ZtStd[iNumTime + 1]))
          iIndexMinZ++;

        while ((iIndexMaxZ > iIndexZ0 + 1) &&
               (GridZ[iIndexMaxZ - 1] >
                1.0 + GridZNbSigmaMaxRight[iNumTime + 1] * ZtStd[iNumTime + 1]))
          iIndexMaxZ--;
      }
      ...
      if (Params->SaveTensor == LGMSV_TRUE) {
        ...
        LGMSVSaveTensor_UtPieceWise(/* Inputs */
                                    ...
                                    GridPsi_t1, GridX, GridZ, ValueTensor_t1);
      }
      ...
      LGMSVPDEMakePsiGrid_UtPieceWise(/* Inputs */
                                      iNbPsiGrid_t, PsiMean[iNumTime],
                                      PsiStd[iNumTime], Params->iNbSigmaPhiGrid,
      ...
      LGMSVPDECalculationValueMatrixfromPsit_UtPieceWise(
          /* Inputs */
          iIndexMinZ, iIndexMaxZ, iIndexMinX, iIndexMaxX,

          iNbPsiGrid_t, iNbPsiGrid_t1, iNbProduct,

          GridX, GridZ, GridPsi_t, GridPsi_t1,
          ...
          Sigt[iNumTime + 1], dLambdaX, dAlphaVt[iNumTime + 1],
          dRhot[iNumTime + 1],
      ...
      num_f_pde_one_step_backward_2f_save_LGMSV_adi(
          pde, iNbZ, GridZ, iNbX, GridX, 0, iNbProduct - 1, ValueTensor_t,
          DZDrift, DXDrift, DZVar, DXVar, r, ValueTensor_t1, 0,
          iNbPsiGrid_t - 1, iIndexMinZ, iIndexMaxZ, iIndexMinX, iIndexMaxX);
      ...
      if ((fabs(Sigt[iNumTime + 1] - Sigt[iNumTime]) > 1e-8) ||
        ...
        LGMSVPDEJumpUtTreatment(/* Inputs */
                                ...
                                0, iNbProduct - 1, iNbPsiGrid_t,

                                GridX, GridZ,

                                Sigt[iNumTime], Sigt[iNumTime + 1],
                                dAlphaVt[iNumTime], dAlphaVt[iNumTime + 1],
                                dRhot[iNumTime], dRhot[iNumTime + 1],

                                LGMSV_MIXQUADRALINEAR, /*LGMSV_LINEAR        ,
                                                    /*LGMSV_MIXQUADRALINEAR
                                                          , */
                                LGMSV_TRUE,

                                ValueTensor_t1, InterpQuadraCoefGridX,
      ...
      if (EvalEvent[iNumTime]) {
        ...
        err = payoff_func(/* Time Information */
                          ...
                          /* Martingale Mesure QTstar information */
                          dTstar, /* In time */
                          dAlphaVt[iNumTime], dRhot[iNumTime], Sigt[iNumTime],

                          /* Grid data Information	*/
                          0, iNbPsiGrid_t - 1, iIndexMinX, iIndexMaxX,
                          iIndexMinZ, iIndexMaxZ,

                          GridPsi_t, GridX, GridZ,

                          /* Tensor of results to be updated		*/
                          /* 4 dimensions : Psit        ,ft        ,Epst
                             ,Product	*/
                          &iNbProduct, ValueTensor_t1);
        if (err) {
          goto FREE_RETURN;
    ...
    memcpy(dProductArrayPv, ValueTensor_t1[iIndexZ0][iIndexX0][iIndexPsi],
...
FREE_RETURN:
  ...
  /* free all the arrays */
  if (pde)
    num_f_pde_free_2d_LGMSV_adi(pde, iNumPsi, iNbX, iNbZ, iNbProduct);
  ...
  if (ValueGridXtZtProduct)
    free_f3tensor(ValueGridXtZtProduct, 0, iNbZ - 1, 0, iNbX - 1, 0,
                  iNbProduct - 1);

  if (ValueTensor_t)
    free_f4tensor(ValueTensor_t, 0, iNbZ - 1, 0, iNbX - 1, 0, iNbPsi - 1, 0,
                  iNbProduct - 1);

  if (ValueTensor_t1)
    free_f4tensor(ValueTensor_t1, 0, iNbZ - 1, 0, iNbX - 1, 0, iNbPsi - 1, 0,
                  iNbProduct - 1);

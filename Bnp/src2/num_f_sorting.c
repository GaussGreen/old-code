...
void sort_simple(unsigned long n, 
				 double *arr, 
				 double *brr)
{
	...
	for (;;) {
		if (ir-l < MPAR) {
			...
			brr[j]=b;
			jstack += 2;
			if (jstack > NSTACK) goto end; //nrerror("NSTACK too small in sort2.");
			if (ir-i+1 >= j-l) {
				istack[jstack]=ir;
				istack[jstack-1]=i;
				ir=j-1;
...
void index_data(unsigned long n, 
				double *arr, 
				unsigned long *indx)
{
	...
	for (;;) {
		if (ir-l < MPAR) {
			...
			indx[j]=indxt;
			jstack += 2;
			if (jstack > NSTACK) goto end; //nrerror("NSTACK too small in indexx.");
			if (ir-i+1 >= j-l) {
				istack[jstack]=ir;
				istack[jstack-1]=i;
				ir=j-1;
...
///////////////////////////////////////////////////////////////////////////////////////
void SortSeriesDescending( long n, 
			     double  *ra)
{
	long i,ir,j,l;
	double rra;
...
void sort_simple2(unsigned long n, double **arr)
{
	...
	for (;;) {
		if (ir-l < MNUM) {
			...
			b=arr[l][1];
			for (;;) {
				do i++; while (arr[i][0] < a);
				do j--; while (arr[j][0] > a);
				if (j < i) break;
...
Err indexx_ll(long *f, long *pI, long n)
{
	...
	for (;;) {
		if (ir-l < MPAR) {		/* Insertion sort when subarray small enough */
			...
			k = (unsigned long)(l+ir) >> 1;		/* Choose median of left, center, and right */
			...
			{
				do i++; while (f[pI[i]] < a);	/* Scan up to find element > a */
				do j--;	while (f[pI[j]] > a);	/* Scan down to find element < a */
...
FREE_RETURN:
	if (istack) free_lvector(istack, 1, NSTACK);
	return err;
}

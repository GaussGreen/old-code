...
Err Dlist_func(Dlist l1, Dlist l2, char oper,Dlist *l3)
{
	...
	switch(oper){
		case '+':
		for(i=0;i<l;i++)l3->d[i] = l1.d[i] + l2.d[i];
		break;
		case '-':
		for(i=0;i<l;i++)l3->d[i] = l1.d[i] - l2.d[i];
		break;
...
/* OVE fix: a swap is described by theoretical payment dates: spot lag after fixing,
   vol is effective only until fixing date 
		tgt_exp_date = add_unit((long)period_start, -cmt_param->spot_lag, SRT_BDAY, SUCCEEDING);
*/   
		...
		{
			err = cmt_param->cmt_getvol((Date)period_start, (Date)period_end, strike, &swap_vol->d[i]);
			if (err)
				return err;
		}
	}
...
/* Interpolates all the vols, though the very last one is useless */
	...
	*/   
		...
		{
			err = cmt_param->cmt_getvol((Date)period_start, (Date)period_end, strike, &treas_vol->d[i]);
			if (err)
				return err;
		}

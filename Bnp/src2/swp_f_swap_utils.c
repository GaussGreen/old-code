...
Err fixing_list(DateList start_date, int spot_lag, DateList* fixing)
{
    ...
    *fixing = new_DateList(start_date.len);
    ...
    for (i = 0; i < start_date.len; i++)
        fixing->date[i] = add_unit(start_date.date[i], -spot_lag, SRT_BDAY, MODIFIED_SUCCEEDING);
...
/* ---------------------------------------------------------------------------
   ...
   --------------------------------------------------------------------------- */
Err fwd_cash_list(
    DateList start_date, DateList end_date, SrtBasisCode basiscode, String ycname, Dlist* fwd_cash)
{
    int i;
    Err err = NULL;
    ...
    for (i = 0; i < start_date.len; i++)
    {
        fwd_cash->d[i] = swp_f_fwdcash(start_date.date[i], end_date.date[i], basiscode, ycname);
...
/* ---------------------------------------------------------------------------
   ...
   Allocation is done inside
   --------------------------------------------------------------------------- */
Err spread_list(DateList start_date, DateList end_date, String ref_rate_code, Dlist* spread)
{
    int i;
    Err err = NULL;
    ...
    for (i = 0; i < start_date.len; i++)
    {
        spread->d[i] = swp_f_spread(start_date.date[i], end_date.date[i], ref_rate_code);
...
Err fwd_swap_list(DateList dl, SrtCurvePtr cmt_crv, Dlist* fwd_swap)
{
    ...
    for (i = 0; i < dl.len; i++)
    {
        ...
        swp_f_ForwardRate_SwapDP(&swapdp, yc_name, cmt_param->swap_ref_rate, &(fwd_swap->d[i]));
    }
...
Err swap_vol_list(DateList dl, Dlist swap_rate, SrtCurvePtr cmt_crv, Dlist* swap_vol)
{
    int    i;
    ...
    for (i = 0; i < dl.len; i++)
    {
        ...
        */
        ...
        {
            err = cmt_param->cmt_getvol(
                (Date)period_start, (Date)period_end, strike, &swap_vol->d[i]);
            if (err)
                return err;
        }
    }
...
Err treas_vol_list(
    DateList dl, Dlist swap_rate, Dlist treas_rate, SrtCurvePtr cmt_crv, Dlist* treas_vol)
{
    int    i;
    long   period_start, period_end;
    double strike;
   ...
   SUCCEEDING);
        */
        ...
        {
            err = cmt_param->cmt_getvol(
                (Date)period_start, (Date)period_end, strike, &treas_vol->d[i]);
            if (err)
                return err;
        }
    ...
    Dlist*      conv_fwd_treas)
{
    ...
    /* Computes all the convexity adjusted forwards, storing themon the next one */
    for (i = 0; i < dl.len - 1; i++)
    {
        /* OVE fix: a swap is described by theoretical payment dates: spot lag after fixing,
           vol is effective only until fixing date */
        fixing_date = add_unit(dl.date[i], -cmt_param->spot_lag, SRT_BDAY, SUCCEEDING);
        delay       = (double)coverage(dl.date[i], dl.date[i + 1], basis);
        maturity    = (double)(fixing_date - today) * YEARS_IN_DAY;
        err         = swp_f_cmsrate(
            fwd_rate.d[i],
            nfp,
            cmt_param->bond_compd,
            vol.d[i],
            maturity,
            delay,
            DEFAULT_CMS_DELTA,

...
Err free_resetableperiod_td(double **paramperiod, double *periodmat,
                            double *DRStsStart, double *DRSts, double *isfriday,
                            double nfixdays, int n_resetdates, double **mat,
  ...
  if (mat)
    free_dmatrix(mat, 1, (long)nfixdays, 1, n_resetdates);
  mat = NULL;

  if (paramaftereset)
    free_dmatrix(paramaftereset, 1, (long)nfixdays, 1, 5);
  paramaftereset = NULL;
...

{
  ...
  if (caliberror)
    free_dvector(caliberror, 1, n_resetdates);
  caliberror = NULL;
...
Err calibtosmile_td(long today, long dDate, char *cMarketId, char *cRefRateCode,
                    char *szVolCurveName, char *szYieldCurveName, int nperiods,
                    double *param, double *calerr, double *mat, long *ia) {
  Err err = NULL;
  int i;
  int spotlag;
  double atmvol;
  ...
  err = optcalibmertontimedep(dFra, NMAXSTRIKES, nperiods, strikes, vols, param,
                              mat, "Lognormal", calerr, ia, calibvols);
...
Err resetablecalib_td(long today, int n_resetdates, double *resetdates,
                      char *cMarketId, char *szVolCurveName, char *cRefRateCode,
                      char *szYieldCurveName, double *paraminitguess,
                      double **paramatrix, double *caliberr)

{
  ...
  err = calibtosmile_td(today, (long)resetdates[1], cMarketId, cRefRateCode,
                        szVolCurveName, szYieldCurveName, 1, paramatrix[1],
                        &(caliberr[1]), mat, freeze);
  ...
  for (j = 2; j <= n_resetdates; j++) {
    ...
    err = calibtosmile_td(today, (long)resetdates[j], cMarketId, cRefRateCode,
                          szVolCurveName, szYieldCurveName, j, paramatrix[j],
                          &(caliberr[j]), mat, freeze);
  }
...
Err DRSCalc_td(double *wn1n2, int nfixdays, double resetmat,
               double **periodparam, double **mat, int nperiods, double *DRSts)
  ...
  for (j = 1; j <= nfixdays; j++) {
    ...
    if (j == 1) {
      for (n = 1; n <= nperiods - 1; n++) {
        ...
        sig[j] += periodparam[j][5 * (n - 1) + 1] *
                  periodparam[j][5 * (n - 1) + 1] * weightsmat[n];
      }

      DRSts[j] *= exp(-sig[j] / 2 + sqrt(sig[j]) * wn1n2[1]);

      DRSts[j] *=
          exp(wn1n2[2] * log(1 + U1ave) -
              periodparam[j][3] * U1ave * resetmat) *
...
Err srt_f_resetable_td(int n_resetdates, double *resetdates, double *initparam,
  ...
  if (err) {
    free_resetablegeneral_td(weights, points, wn1n2, paramatrix, nherm,
                             n_resetdates, caliberror);
    return err;
  }
  ...
  err = resetablecalib_td(today, n_resetdates, resetdates, cMarketId,
                          szVolCurveName, cRefRateCode, szYieldCurveName,
                          initparam, paramatrix, caliberror);
  if (err) {
    free_resetablegeneral_td(weights, points, wn1n2, paramatrix, nherm,
                             n_resetdates, caliberror);
    return err;
  ...
  for (perindex = 1; perindex < n_resetdates; perindex++) {
    ...
    err = get_periodmatandisfriday(perindex, resetdates, nfixdays, periodmat,
                                   isfriday);
    if (err) {
      free_resetableperiod_td(paramperiod, periodmat, DRStsStart, DRSts,
                              isfriday, nfixdays, n_resetdates, mat,
                              paramaftereset);
      free_resetablegeneral_td(weights, points, wn1n2, paramatrix, nherm,
                               n_resetdates, caliberror);
      return err;
    ...
    err = get_DRSts_td(today, (long)resetdates[perindex + 1], (int)nfixdays,
    ...
    for (j = 1; j <= nfixdays; j++) {
      err = get_param_td(periodmat[j], perindex, resetdates, paramatrix,
                         paramperiod[j]);
    }
      ...
      for (n2 = 0; n2 <= N2max; n2++) {
        ...
        if (probajumps > probainf) {
          ...
          for (n = 1; n <= 2 * nherm; n++) {
            ...
            err = DRSCalc_td(wn1n2, (int)nfixdays, resetmat, paramperiod, mat,
                             perindex + 1, DRSts);
            ...
            if (err) {
              free_resetableperiod_td(paramperiod, periodmat, DRStsStart, DRSts,
                                      isfriday, nfixdays, n_resetdates, mat,
                                      paramaftereset);
              free_resetablegeneral_td(weights, points, wn1n2, paramatrix,
                                       nherm, n_resetdates, caliberror);
              return err;
            }
            ...
            if (strcmp(ResetType, "CENTERED") == 0) {
              ...
              prix = PrixTimeSwap(width / 2, DRSts, (int)nfixdays,
                                  periodmatinyears, isfriday, paramaftereset,
                                  width, callspread);
              ...
              err = NewtonJumps((int)nfixdays, DRSts, periodmat, isfriday,
                                paramaftereset, width, callspread, &eps, &prix);
            }
            ...
            if (strcmp(FloatCoupon, "YES") == 0) {
              pv[1][perindex] += weights[n] * probajumps * (DRSts[1] + margin) *
                                 prix / ntotaldays;
            }
    ...
    free_resetableperiod_td(paramperiod, periodmat, DRStsStart, DRSts, isfriday,
                            nfixdays, n_resetdates, mat, paramaftereset);

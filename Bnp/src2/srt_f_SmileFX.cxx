...
Err srt_f_SABW_Opt(Date eval_date, Date exp_date, double option_strike,
                   ...
                   double basevol, double beta, double gamma, double omega,

                   /* PDE PARAMETERS */
                   long min_node, long min_num_mesh, double X0, double X_min,
                   double X_max, double h_max,

                   double *option_pv) {
  Err err = NULL;
  long NumTimeStep, i, j, k, NumMesh;
  double yr_to_exp_date, TimeStep = 1.0 / 24, dX, PDEBoundary[2], *X = NULL,
                         *exp_X = NULL, *TerminalPayoff = NULL, **D = NULL,
                         **V = NULL, *U = NULL, **div_mat = NULL;
  ...
  init_fwd_val = exp(X0) *
                 srt_f_forward_from_fwdcrv(yr_to_exp_date, fwdcrv, NULL) /
                 df_exp;
  ...
  err = srt_f_feynmann_kac_pde(NumMesh, NumTimeStep, dX, TimeStep,

                               X, D, V, PDEType, PDEBoundaryCond, PDEBoundary,
                               TerminalPayoff, NULL, div_mat, &U);
  if (err)
    return err;

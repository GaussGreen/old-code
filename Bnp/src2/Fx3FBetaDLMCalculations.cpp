    ...
    double*                   premium)
{
    ...
    err = FxBetaDLM_FxOptionTS_struct(
        opt_settlmt_date,
        fwd_start_date,
        fixing_date,
        nb_strike,
...
Err FxBetaDLM_Allocate_FxOptInst(int iIndex, int iNbStrike, FxBetaDLM_FxOptInst* Inst)
{
    Err err = NULL;

    Inst->iInstIndex = iIndex;
    Inst->dStrike    = (double*)calloc(iNbStrike, sizeof(double));
    Inst->dLogStrike = (double*)calloc(iNbStrike, sizeof(double));
    Inst->dPrice     = (double*)calloc(iNbStrike, sizeof(double));
    Inst->dLogVol    = (double*)calloc(iNbStrike, sizeof(double));
    Inst->dVega      = (double*)calloc(iNbStrike, sizeof(double));
    ...
    double*                   Premium)
{
    ...
    else if (cNumParams->iMethod == 2 || cNumParams->iMethod == 3 || cNumParams->iMethod == 4)
    {
        if (Precalc->iIsCPD)
        {
            for (i = 1; i <= Hermite->iNbPoints; i++)
            ...
            for (i = 1; i <= Hermite->iNbPoints; i++)
            {
                for (j = 0; j < Inst->iNbStrike; j++)
                {
                    ...
                    else
                    {
                        ...
                        err = quadratic_bs(
                            Precalc->dQuadConst_down + Hermite->X[i] * Precalc->dQuadStdY_down,
    ...
    double*                   premium)
{
    ...
    premium1  = (double*)calloc(nb_strike, sizeof(double));
    premium2  = (double*)calloc(nb_strike, sizeof(double));
    SigFx     = (double*)calloc(model->iNbPWTime, sizeof(double));
    ...
    err = FxBetaDLM_Update_InstPrecalc_FromMoment(Inst, model, NumParams, InstConst);

    if (err)
        goto FREE_RETURN;

    err = FxBetaDLM_Price_FxOptInst(Inst, model, InstConst, hermite, NumParams, premium);
    if (err)
        goto FREE_RETURN;
...
void Fx3DBetaDLM_ExpectAndVarGrfn(
    long             nstp,
    double*          time,
    FxBetaDLM_model* model,
    ...
    double min_time)
{
    ...
    for (i = 1; i < nstp; i++)
    {
        ...
        for_var_std[i] =
            var_rates_for - 2.0 * model->dC0 * (1.0 + 2.0 * model->dC0 * var_0_Ti) * temp * temp;
...
void Fx3DBetaDLM_PrecalcGRFNTreeQBeta(
    long             nstp,
    double*          time,
    FxBetaDLM_model* model,
    ...
    double*                Premium)
{
    ...
    if (Precalc->dQuadVar == 0.0)
    {
        ...
        {
            *Premium =
                exp(constant_coef + Precalc->dQuadLin * mean + Precalc->dQuadQuad * mean * mean) -
                strike;
            if (*Premium < 0)
            {
                *Premium = 0;
    ...
    else
    {
        ...
        {
            err = serror(
                "quadratic_bs  The process has infinite Precalc->dQuadVar, C = %f > %f",
                Precalc->dQuadQuad,
        ...
        else if (Precalc->dQuadQuad == 0)
        {
            ...
            {
                J1 = exp(
                    constant_coef - mean * mean / (2.0 * Precalc->dQuadVar) +
                    (Precalc->dQuadLin * Precalc->dQuadVar + mean) *
                        (Precalc->dQuadLin * Precalc->dQuadVar + mean) / (2.0 * Precalc->dQuadVar));
                J1 *= norm(
                    (mean + Precalc->dQuadLin * Precalc->dQuadVar - root_1) / Precalc->dQuadStd);
            ...
            else
            {
                J1 = exp(
                    ...
                    (Precalc->dQuadLin * Precalc->dQuadVar + mean) *
                        (Precalc->dQuadLin * Precalc->dQuadVar + mean) / (2.0 * Precalc->dQuadVar));
                J1 *= norm(
                    (root_1 - mean - Precalc->dQuadLin * Precalc->dQuadVar) / Precalc->dQuadStd);
            ...
            if (delta < 0)
            {
                if (callput == SRT_CALL)
                {
                    ...
                    else
                    {
                        ...
                        *Premium = exp(constant_coef - mean * mean / (2.0 * Precalc->dQuadVar) +
                                       k1 * k1 / (2.0 * k2 * k2)) /
                                   Precalc->dQuadStd / k2;
                        *Premium += -strike;
                    }
...
Err FxBetaDLM_Price_ForwardFx(FxBetaDLM_InstPrecalc* Precalc, double* Premium)
{
    ...
    *Premium =
        Precalc->dFwdBeta *
        exp(Precalc->dQuadConst + Precalc->dFwdQuad2 * (Precalc->dQuadLin + Precalc->dFwdAlpha) *
    ...
    double**                  fx_cal_vols)
{
    ...
    fx_vols      = (double*)calloc(nb_fx, sizeof(double));
    settlmt_time = (double*)calloc(nb_fx, sizeof(double));

...
FxBetaDLM_Hermite	*hermite = NULL;
	...
	err	= FxBetaDLM_FxOptionTS_struct(	opt_settlmt_date,
										fwd_start_date,
										fixing_date,
										nb_strike,
										strikes,
...
Err	err = NULL;
	...
	if (!Inst->dStrike || !Inst->dLogStrike || !Inst->dPrice || !Inst->dLogVol || !Inst->dVega)
	{
...
void FxBetaDLM_Free_FxOptInst(FxBetaDLM_FxOptInst	*Inst)
{
	if (Inst)
	{
		if (Inst->dStrike) free(Inst->dStrike);
		Inst->dStrike = NULL;

		if (Inst->dLogStrike) free(Inst->dLogStrike);
		Inst->dLogStrike = NULL;

		if (Inst->dPrice) free(Inst->dPrice);
		Inst->dPrice = NULL;

		if (Inst->dLogVol) free(Inst->dLogVol);
		Inst->dLogVol = NULL;

		if (Inst->dVega) free(Inst->dVega);
		Inst->dVega = NULL;
	}
}
...
double	std_fx, temp;
	...
	{
		err = "fixing date must be before fwd_start_date in FxBetaDLM_Setup_FxOptInst";
		goto FREE_RETURN;
	}
	...
	if (nb_strike > 2 && strikes[2] < 0.0 && bs_vols)
	{
		for (i=1; i<nb_strike; i++)
		{
			...
			err = srt_f_optsarbvol(	Inst->dFwdFx,
									strikes[i],
									Inst->dExeTime,
									bs_vols[0],
									0.0,
	...
	{		
		...
		if (bs_vols)
		{
			...
			Inst->dPrice[i] = srt_f_optblksch(	Inst->dFwdFx,
												Inst->dStrike[i],
												Inst->dLogVol[i],
												Inst->dExeTime,
												Inst->dDfDom,
			...
			Inst->dVega[i] = srt_f_optblksch(	Inst->dFwdFx,
												Inst->dStrike[i],
												Inst->dLogVol[i],
												Inst->dExeTime,
												Inst->dDfDom,
...
double	fwd_fx, Val1, Val2, Val;
	...
	if(cNumParams->iMethod==1)
	{
		for (i=1; i<=Hermite->iNbPoints; i++)
		{
			fwd_fx = exp(Precalc->dConstCoef
				+ (Precalc->dLinCoef + Precalc->dQuadCoef * Hermite->X[i]) * Hermite->X[i]);		

			for (j=0; j<Inst->iNbStrike; j++)
			{
				Premium[j] += Hermite->W[i] * srt_f_optblksch(	fwd_fx,
																Inst->dStrike[j],
																Precalc->dStdRates,
																1.0,
																1.0,
	...
	if(cNumParams->iMethod==2 || cNumParams->iMethod==3 || cNumParams->iMethod==4)
	{
		if(Precalc->iIsCPD)
		{
			for (i=1; i<=Hermite->iNbPoints; i++)
		...
		else
		{
			for (i=1; i<=Hermite->iNbPoints; i++)
			{
				for (j=0; j<Inst->iNbStrike; j++)
				{
					...
					{
						err = quadratic_bs(	Precalc->dQuadConst + Hermite->X[i] * Precalc->dQuadStdY,
										Precalc->dQuadLin,
										Precalc->dQuadQuad,
					...
					else
					{
						...
						err = quadratic_bs(	Precalc->dQuadConst_down + Hermite->X[i] * Precalc->dQuadStdY_down,
										Precalc->dQuadLin_down,
										Precalc->dQuadQuad_down,
										Precalc->dQuadMean_down * Hermite->X[i] * Precalc->dQuadStdY_down,
										Precalc->dQuadVar_down,
										Inst->dStrike[j],
										Inst->sCallPutType,
										&Val2);
...
double					*premium2;
	...
	err = FxBetaDLM_Setup_FxOptInst(opt_settlmt_date,
									opt_settlmt_date,
									fwd_start_date,
									fixing_date,
									nb_strike,
...
FREE_RETURN:
	...
	if (SigFx) 	free(SigFx);
	if (premium1) free(premium1);
	if (premium2) free(premium2);
	return err;
}
...
double	last_dom_phi, last_for_phi, last_var_ffx;
	...
	for (i=1; i<nstp; i++)
	{
		...
		for_var_std[i] = var_rates_for - 2.0 * model->dC0 * (1.0 + 2.0 * model->dC0 * var_0_Ti) * temp * temp;
		...
		if (mc_tree == 0)
		{
			...
			dom_for_cov_corr[i] = covar_dom_for - 2.0 * model->dC0 * covar_dom_for_adjust;
			dom_fx_cov_corr[i] = covar_dom_ffx;
			for_fx_cov_corr[i] = - (1.0 + 2.0 * model->dC0 * var_0_Ti) * temp;
		}
		else
		{			
			dom_for_cov_corr[i] = (covar_dom_for - 2.0 * model->dC0 * covar_dom_for_adjust) / sqrt(dom_var_std[i] * for_var_std[i]);
...
double	expect_for, expect_dom, expect_lnZ;
	...
	for (i=1; i<nstp; i++)
	{
		...
		for (j=start_index; j<end_index+1; j++)
		{
			...
			covar_dom_for += sig_domfor * Phi_Func(dom_lam + for_lam, end_mat, t1, t2);

			covar_dom_lnZ += sig_domfx * Phi_Func(dom_lam, end_mat, t1, t2);
			covar_dom_lnZ -= sig_domfor * Gamma_Func(for_lam,dom_lam, end_mat, t1, t2);
			...
			covar_dom_lnZ += sig_domfor * Gamma_Func(dom_lam,for_lam, end_mat, t1, t2);
						...
						double					*Premium)
{
	...
	if(Precalc->dQuadVar==0.0)
	{
		if(callput == SRT_CALL)
		{
			*Premium = exp(constant_coef + Precalc->dQuadLin * mean + Precalc->dQuadQuad * mean * mean) - strike;
			if(*Premium<0)
			{
				*Premium = 0;
			}
		...
		{
			*Premium = strike - exp(constant_coef + Precalc->dQuadLin * mean + Precalc->dQuadQuad * mean * mean);
			if(*Premium<0)
			{
				*Premium = 0;
			}		
	...
	else
	{
		...
		{
			err = serror("quadratic_bs  The process has infinite Precalc->dQuadVar, C = %f > %f"
				, Precalc->dQuadQuad
				, 0.5 / Precalc->dQuadVar);
			goto FREE_RETURN;			
		}
		else
		if(Precalc->dQuadQuad == 0)
		{
			...
			if(callput == SRT_CALL)
			{
				J1 = exp( constant_coef - mean * mean / (2.0 * Precalc->dQuadVar) + (Precalc->dQuadLin * Precalc->dQuadVar + mean) * (Precalc->dQuadLin * Precalc->dQuadVar + mean) / (2.0 * Precalc->dQuadVar));
				J1 *= norm((mean + Precalc->dQuadLin * Precalc->dQuadVar - root_1)/ Precalc->dQuadStd);
			...
			{
				J1 = exp( constant_coef - mean * mean / (2.0 * Precalc->dQuadVar) + (Precalc->dQuadLin * Precalc->dQuadVar + mean) * (Precalc->dQuadLin * Precalc->dQuadVar + mean) / (2.0 * Precalc->dQuadVar));
				J1 *= norm((root_1 - mean - Precalc->dQuadLin * Precalc->dQuadVar )/ Precalc->dQuadStd);
		...
		{
			delta  = Precalc->dQuadLin*Precalc->dQuadLin-4*Precalc->dQuadQuad*(constant_coef-LogStrike);
			if(delta<0 )
			{
				if(callput == SRT_CALL)
				{
					...
					else
					{
						...
						*Premium = exp( constant_coef - mean * mean /  (2.0 * Precalc->dQuadVar) + k1 * k1 / (2.0 * k2 * k2)) / Precalc->dQuadStd /k2;
						*Premium += - strike;
					}
				}
				else
			...
			{
				root_1 = (-Precalc->dQuadLin + sqrt(delta)) / (2.0 * Precalc->dQuadQuad);
				root_2 = (-Precalc->dQuadLin - sqrt(delta)) / (2.0 * Precalc->dQuadQuad);
				...
				J1 *= exp( constant_coef - mean * mean /  (2.0 * Precalc->dQuadVar) + k1 * k1 / (2 * k2 * k2)) / Precalc->dQuadStd /k2;
				J2 *= strike; 
				if(callput == SRT_CALL)
				{
					*Premium = J1 - J2;
...
Err err = NULL;
	...
	*Premium = Precalc->dFwdBeta * exp(Precalc->dQuadConst + Precalc->dFwdQuad2 * (Precalc->dQuadLin + Precalc->dFwdAlpha) * (Precalc->dQuadLin + Precalc->dFwdAlpha));
...
FxBetaDLM_InstPrecalc	*InstConst	= NULL;
	...
	for (i=0; i<nb_fx; i++)
	{
		...
		err = FxBetaDLM_Setup_FxOptInst(settlmt_date,
										settlmt_date,
										Model->lToday,
										fixing_date,
										1,
	...
	err = Fx3DtsCalibration_corr(	fx_maturities,
									settlmt_time,
									fx_vols,
									nb_fx,
									Model->dPWTime,
									...
									Model->dPWTime,									
									Model->dCorrDomFor,
									Model->dCorrDomFxInput,
									Model->dCorrForFxInput,
									Model->iNbPWTime,
									fx_cal_vols);
...
FREE_RETURN:
	...
	if (fx_vols) free (fx_vols);
	if (settlmt_time) free (settlmt_time);

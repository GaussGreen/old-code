/* ==============================================================================
   ...
   FILE NAME:      srt_f_cheybeta_bck_pde.cxx
...
/*	Calculate the coefficients of a linear interpolation        ,
        i.e. find c1..cxxn such that f(x0) = sum ci * f(xi)	*/
static void lininter_quick_coef(double *x, int n, double x0, double *cup,
                                double *cdown, long *indexup, long *indexdown) {
  int i;
  ...
  *indexdown = i;
  ...
  /*	Calculate linear interpolation coefficients of x0 in terms of xi and
   * xi+1	*/
  *cdown = (x[i + 1] - x0) / (x[i + 1] - x[i]);
  *cup = 1.0 - (*cdown);
}

static void lininter_coef(double *x, int n, double x0, double *c) {
  int i;
...
void srt_f_chey_beta_bck_pde_alloc_temp(CHEYBETA_BCK_PDE_TEMP *tmp, int nx,
                                        int nphi, int num_prod) {
...
/*	Free temp vectors	*/
void srt_f_chey_beta_bck_pde_free_temp(CHEYBETA_BCK_PDE_TEMP *tmp) {
  free(tmp->coef);
...
void srt_f_chey_beta_bck_pde_one_step(CNPDE_TEMP *tmppde,
  ...
  for (xi = 0; xi < grid->nx; xi++) {
    ...
    nvar = chey_beta_mdl_norm_var_at_t(&p, x, 0.0, grid->fwdrt[t1],
                                       grid->maxvar[t1], grid->minvar[t1]);

    tmpcb->var[xi] = chey_beta_mdl_var(mdl, &p, x, 0.0, nvar);
  }
  ...
  for (phi_slice = 0; phi_slice < grid->nphi[t1]; phi_slice++) {
    ...
    for (xi = 0; xi < grid->nx; xi++) {
      ...
      /*	Calculate mu of x	*/
      tmpcb->mu[xi] = chey_beta_mdl_drift(mdl, &p, x, grid->phi[t1][phi_slice]);

      /*	Do explicit convolution in phi	*/
      lininter_quick_coef(grid->phi[t2], grid->nphi[t2],
                          chey_beta_mdl_phi(mdl, &p, x,
                                            grid->phi[t1][phi_slice],
                                            tmpcb->var[xi]),
                          &cup, &cdown, &indexup, &indexdown);
    ...
    num_f_pde_one_step_backward(tmppde, grid->nx, grid->x, grid->num_prod,
                                tmpcb->val, tmpcb->mu, tmpcb->var, tmpcb->r,
                                0.55, grid->vt1[phi_slice]);
...
/*	Go backward one step	*/
void srt_f_chey_beta_bck_pde_one_step2(CNPDE_TEMP *tmppde,
                                       CHEYBETA_BCK_PDE_TEMP *tmpcb,
                                       /*	Model	*/
                                       CHEYBETA_MDL *mdl,
                                       /*	Grid	*/
  ...
  for (phi_slice = 0; phi_slice < grid->nphi[t1]; phi_slice++) {
    ...
    num_f_pde_one_step_backward(tmppde, grid->nx, grid->x, grid->num_prod,
                                grid->vt2[phi_slice], tmpcb->mu, tmpcb->var,
                                tmpcb->r, 0.55, tmpcb->val);

    for (xi = 0; xi < grid->nx; xi++) {
      x = grid->x[xi];

      lininter_quick_coef(grid->phi[t1], grid->nphi[t1],
                          chey_beta_mdl_phi_back(mdl, &p, x,
...
/*	Go backward one step	*/
void srt_f_chey_beta_bck_pde_one_step1(CNPDE_TEMP *tmppde,
                                       CHEYBETA_BCK_PDE_TEMP *tmpcb,
                                       /*	Model	*/
                                       CHEYBETA_MDL *mdl,
                                       /*	Grid	*/
  ...
  for (phi_slice = 0; phi_slice < grid->nphi[t1]; phi_slice++) {
    ...
    for (xi = 0; xi < grid->nx; xi++) {
      ...
      nvar = chey_beta_mdl_norm_var_at_t(&p, x, grid->phi[t1][phi_slice],
                                         grid->fwdrt[t1], grid->maxvar[t1],
                                         grid->minvar[t1]);

      /*	Calculate mu and var of x	*/

      tmpcb->var[xi] =
          chey_beta_mdl_var(mdl, &p, x, grid->phi[t1][phi_slice], nvar);
      ...
      /*	Do explicit convolution in phi	*/
      lininter_coef(grid->phi[t2], grid->nphi[t2],
                    chey_beta_mdl_phi(mdl, &p, x, grid->phi[t1][phi_slice],
                                      tmpcb->var[xi]),
                    tmpcb->coef);
...
/*	Go backward n steps	*/
void srt_f_chey_beta_bck_pde_n_steps(CHEYBETA_BCK_PDE_TEMP *tmpcb,
                                     /*	Model	*/
                                     CHEYBETA_MDL *mdl,
                                     /*	Grid	*/
                                     CHEYBETA_GRID *grid,
  ...
  for (i = 0; i < n; i++) {
    srt_f_chey_beta_bck_pde_one_step(&tmppde, tmpcb, mdl, grid, *ti - i - 1,
                                     *ti - i);
  }

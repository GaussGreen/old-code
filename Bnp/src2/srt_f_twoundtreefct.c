/* ----------------------------------------------------------------------------------------------
   FILENAME:   srt_f_twoundtreefct.c
...
#define TREE_MESH_SPACING	  1.73205080756888 	   /* theoretically > 4  1.73205080756888 */
...
static long srt_minimum(long a,long b) { if (a>b) return b; else return a;}
static long srt_maximum(long a,long b) { if (a>b) return a; else return b;}
...
/*  At each time step:
	-	The dual basis is computed: eigenvectors of local cov matrix * sqrt (eigenvalues)
		GLOBAL standard deviations in each direction	
*/
Err twound_trelim(
					SrtStpPtr               stp, 
					SrtDiagTwoFacTreeInfo  *maxtrinf,  
...
double			 global_var[2], global_cov,	  /* Cov matrix seen from today for current step */
					global_var_dir[2],	             /* Global variance in direction of eigenvectors */
					local_var[2], local_cov, local_correl,     /* Cov matrix seen from previous step for current step */
					det,				        /* Det of dual basis used for inversion */
					std, lim,
					temp1, temp2, temp3, temp4, temp5,	temp6, temp7,   /* Temps used for diagonalisation */
					V1, V2, C, rho, dt;
int					i, 
					max_trunc_index,	/* Trimming */
...
/* Allocates tree info pointer ( 1 stands for  trinf (0=tminf), 0: index of the underlying ) */
...
/* Next steps: Loop on steps */
	...
	/* Compute Global and Local covariances in original basis */
		...
		/* For LGM, the global covariance of the short rate if Phi ... */
			...
			/* The local Covariance LGM/LGM from the previous step to this one */
				local_cov =	 rho * irmprvtminf1->ev.twof[0].sig * irmprvtminf2->ev.twof[0].sig * dt;
				
			/* For the Computation of the Global Covariance */
				GG += rho * (irmprvtminf1->ev.twof[0].sig * irmprvtminf2->ev.twof[0].sig) 
					/ (irmprvtminf1->ev.onef.F * irmprvtminf2->ev.onef.F) * dt;
			
			/* The Global Covariance : the previous sum multiplied by the product of F's */	
			...
			/* For a BS like model : the Local Variance from the previous step to this one */
			...
			/* The local covariance LGM/BS from the previous step to this one */
				local_cov =	 rho * irmprvtminf1->ev.twof[0].sig * logprvtminf2->int_sig_dt;

			/* For the computation of the Global covariance */
				GG += rho * (irmprvtminf1->ev.twof[0].sig * logprvtminf2->int_sig_dt) / irmprvtminf1->ev.onef.F;
		...
		if ((mdl_type1 == BLACK_SCHOLES)  || (mdl_type1 == EQ_STOCH_RATES))
		{
		...
		/* The global covariance is the simple integral of sigma^2 */
			...
			/* For the computation of the Global covariance */
				GG += 	rho * (prvtminf2->ev.twof[0].sig * logprvtminf1->int_sig_dt) / prvtminf2->ev.onef.F;
			
			/* The Global Covariance : the previous sum multiplied by the LGM F */	
				global_cov = tminf2->ev.onef.F * GG;
			}
			else if ((mdl_type2 == BLACK_SCHOLES) || (mdl_type1 == EQ_STOCH_RATES))
			{
			...
			/* The global variance is just the integral of the local variance (no mean reversion) */
	...
	/* Covariance is significative: diagonalise and multiply by eigenvalues */
		if (fabs (C) > EPS)
		{
			...
			temp7 = sqrt(2.0) * temp5;
		  	trinf->dual_basis[0][0] = sqrt (V1 + V2 - temp1) * (V1 - V2 - temp1) / temp3;
			trinf->dual_basis[1][0] = sqrt (V1 + V2 - temp1) / temp4;
			trinf->dual_basis[0][1] = sqrt (V1 + V2 + temp1) * (V1 - V2 + temp1) / temp6;
	...
	/* Compute Inverse basis = coordinates of x1 and x2 in terms of eigenvectors */
		det = trinf->dual_basis[0][0] * trinf->dual_basis[1][1] - trinf->dual_basis[0][1] * trinf->dual_basis[1][0];
	...
	/* Trimming */
		...
		/* Compute global variance in the direction of local eigenvectors */
			global_var_dir[i] = global_var[0] * trinf->inverse_basis[i][0] * trinf->inverse_basis[i][0]
								   + global_var[1] * trinf->inverse_basis[i][1] * trinf->inverse_basis[i][1]
								   + 2.0 * global_cov * trinf->inverse_basis[i][0] * trinf->inverse_basis[i][1];
			
		/* Compute standard deviations and TREE_LIM_IN_STDEVs (TREE_LIM_IN_STDEV * std) */
...
void populate_twound_tree_node( 
				  ...
				  SrtDiagTwoFacTreeInfo		   *nxttrinf)   /* Info about tree structure at next time step */
{
int         i;
double   x, y, xx, yy;
...
/* Compute forwards of original statevars */
	if (mdl_type1 == LGM)
		xx = sam_get(node->cur_sam, 0, STATEVAR) * exp( - ((SrtIRMTmInf*) stp->tminf[0])->ev.onef.lambda * dt);
	else
	{
		if (mdl_type2 == LGM)
		{
			...
			xx = sam_get(node->cur_sam, 0, STATEVAR)
				+ ( sam_get(node->cur_sam, 1, SHORT_RATE) - sam_get(tminf_irm->fwd_sam, 1, F_0_t) ) * dt; 
		}
		else
			xx = sam_get(node->cur_sam, 0, STATEVAR);
	}
	...
	else
	{
		if (mdl_type1 == LGM)
		{
			...
			yy = sam_get(node->cur_sam, 1, STATEVAR)
				+ ( sam_get(node->cur_sam, 0, SHORT_RATE) - sam_get(tminf_irm->fwd_sam, 0, F_0_t) ) * dt; 
		}
		else
			yy = sam_get(node->cur_sam, 1, STATEVAR); 
		yy += node->quanto_fwd;
...
/* Transfer in the dual basis: compute forwards of dual statevars */
	node->forward[0] = nxttrinf->inverse_basis[0][0] * xx + nxttrinf->inverse_basis[0][1] * yy;
	node->forward[1] = nxttrinf->inverse_basis[1][0] * xx + nxttrinf->inverse_basis[1][1] * yy;

/* Loops on the two dimensions */
	for (i = 0; i <= 1; i++)
	{
	/* Connects mid to the closest node to forward and rebuilds the statevar accordingly */
		xx = (node->forward[i] < 0.0) ? -0.5 : 0.5;
		
		node->son_index[i][NODE_MID] = (int) (node->forward[i] / nxttrinf->spacing[i] + xx);
		node->son_statevar[i][NODE_MID] = node->son_index[i][1] * nxttrinf->spacing[i];
		node->son_index[i][NODE_MID] = srt_maximum(srt_minimum(node->son_index[i][NODE_MID], nxttrinf->max_index[i]), -nxttrinf->max_index[i]); 

		node->son_index[i][NODE_UP] = (node->son_index[i][NODE_MID] < nxttrinf->max_index[i]) ? node->son_index[i][NODE_MID] + 1 : nxttrinf->max_index[i];
		node->son_statevar[i][NODE_UP] = node->son_statevar[i][NODE_MID] + nxttrinf->spacing[i]; 
			 
		node->son_index[i][NODE_DOWN] = (node->son_index[i][NODE_MID] > -nxttrinf->max_index[i]) ? node->son_index[i][NODE_MID] - 1 : -nxttrinf->max_index[i];
		node->son_statevar[i][NODE_DOWN] = node->son_statevar[i][NODE_MID] - nxttrinf->spacing[i]; 
	}
...
/* Points are like this : 	0: center ; 1 : right (x1 Up/x2 Mid) ; 2 : up ; 3 : left ; 4 : down */
	node->p[1] = ONE_HALF * (xx + x *TREE_MESH_SPACING)/TREE_MESH_SPACING_SQUARE;
	node->p[3] = ONE_HALF * (xx - x*TREE_MESH_SPACING)/TREE_MESH_SPACING_SQUARE;
	node->p[2] = ONE_HALF * (yy + y*TREE_MESH_SPACING)/TREE_MESH_SPACING_SQUARE;
	node->p[4] = ONE_HALF * (yy - y*TREE_MESH_SPACING)/TREE_MESH_SPACING_SQUARE;
				...
				long					  nb_assets)		  /* Number of assets */
{
	...
	{
		cur_assets[i] = node->df * 
			(  
			   node->p[0] * next_assets[node->son_index[0][NODE_MID]][node->son_index[1][NODE_MID]][i]
			+  node->p[1] * next_assets[node->son_index[0][NODE_UP]][node->son_index[1][NODE_MID]][i] 
			+  node->p[2] * next_assets[node->son_index[0][NODE_MID]][node->son_index[1][NODE_UP]][i] 
			+  node->p[3] * next_assets[node->son_index[0][NODE_DOWN]][node->son_index[1][NODE_MID]][i] 
			+  node->p[4] * next_assets[node->son_index[0][NODE_MID]][node->son_index[1][NODE_DOWN]][i]
			);
	}
}

...
*/
Err twound_trelim(
    SrtStpPtr stp, SrtDiagTwoFacTreeInfo* maxtrinf, SrtMdlType mdl_type1, SrtMdlType mdl_type2)
{
    Err                    err;
    ...
    while (cur)
    {
        ...
        else if ((mdl_type1 == BLACK_SCHOLES) || (mdl_type1 == EQ_STOCH_RATES))
        {
            ...
            else if ((mdl_type2 == BLACK_SCHOLES) || (mdl_type1 == EQ_STOCH_RATES))
            {
                ...
                /* The global variance is just the integral of the local variance (no mean
                 * reversion) */
        ...
        for (i = 0; i < 2; i++)
        {
            ...
            global_var_dir[i] =
                global_var[0] * trinf->inverse_basis[i][0] * trinf->inverse_basis[i][0] +
    ...
    SrtDiagTwoFacTreeInfo* nxttrinf) /* Info about tree structure at next time step */
{
    ...
    else
    {
        if (mdl_type2 == LGM)
        {
            ...
            xx                     = sam_get(node->cur_sam, 0, STATEVAR) +
                 (sam_get(node->cur_sam, 1, SHORT_RATE) - sam_get(tminf_irm->fwd_sam, 1, F_0_t)) *
                     dt;
        }
        else
            xx = sam_get(node->cur_sam, 0, STATEVAR);
    ...
    else
    {
        if (mdl_type1 == LGM)
        {
            ...
            yy                     = sam_get(node->cur_sam, 1, STATEVAR) +
                 (sam_get(node->cur_sam, 0, SHORT_RATE) - sam_get(tminf_irm->fwd_sam, 0, F_0_t)) *
                     dt;
        }
        else
            yy = sam_get(node->cur_sam, 1, STATEVAR);
    ...
    for (i = 0; i <= 1; i++)
    {
        ...
        node->son_index[i][NODE_UP]    = (node->son_index[i][NODE_MID] < nxttrinf->max_index[i])
                                             ? node->son_index[i][NODE_MID] + 1
                                             : nxttrinf->max_index[i];
        node->son_statevar[i][NODE_UP] = node->son_statevar[i][NODE_MID] + nxttrinf->spacing[i];

        node->son_index[i][NODE_DOWN]    = (node->son_index[i][NODE_MID] > -nxttrinf->max_index[i])
                                               ? node->son_index[i][NODE_MID] - 1
                                               : -nxttrinf->max_index[i];
    ...
    long                  nb_assets)                    /* Number of assets */
{
    ...
    for (i = 0; i < nb_assets; i++)
    {
        cur_assets[i] =
            node->df *
            (node->p[0] *
                 next_assets[node->son_index[0][NODE_MID]][node->son_index[1][NODE_MID]][i] +
             node->p[1] *
                 next_assets[node->son_index[0][NODE_UP]][node->son_index[1][NODE_MID]][i] +
             node->p[2] *
                 next_assets[node->son_index[0][NODE_MID]][node->son_index[1][NODE_UP]][i] +
             node->p[3] *
                 next_assets[node->son_index[0][NODE_DOWN]][node->son_index[1][NODE_MID]][i] +
             node->p[4] *
                 next_assets[node->son_index[0][NODE_MID]][node->son_index[1][NODE_DOWN]][i]);
    }
}

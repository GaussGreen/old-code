...
static double
proba_joint_spot_min_max(double mu, double sig, double mat, double k, double b,
                         double cp, /*1 if call:S>K  -1 if put:S<K */
                         double du  /*1 if down:min>B  -1 if up:Max<B*/
);

static double proba_min_max(double mu, double sig, double mat, double b,
                            double du /*1 if down:min>B  -1 if up:Max<B*/
);
...
double srt_f_optexting(double fwd, double spot, double strike, double barrier,
  ...
  if ((spot - barrier) * du < 0)
    ...
    if ((spot - strike) * cp > 0)
      ...
      /* if the vol is equal to 0  then the spot depends only from the constant
         rate */
  ...
  case DELTA_FWD: /*** DELTA FWD ***/
    shift = fwd / 10000;
    answer = (srt_f_optexting(fwd + shift, spot, strike, barrier, vol, mat,
  ...
  case DELTA: /*** DELTA SPOT + FWD ***/
    shift = spot / 10000;
    answer =
        (srt_f_optexting(fwd * (1 + shift / spot), spot + shift, strike,
  ...
  case GAMMA: /*** GAMMA ***/
    shift = spot / 10000;
    answer =
        srt_f_optexting(fwd * (1 + shift / spot), spot + shift, strike, barrier,
  ...
  case VEGA: /*** VEGA ***/
    shift = GVOPT.vol_add;
    answer = (srt_f_optexting(fwd, spot, strike, barrier, vol + shift, mat,
                              disc, call_put, down_up, PREMIUM) -
              premium) /
             shift;
    break;
  ...
  case THETA: /*** THETA  ***/
    shift = YEARS_IN_DAY;
    answer = srt_f_optexting(fwd, spot, strike, barrier, vol, mat - shift,

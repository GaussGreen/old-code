...
//***mbs_err_handler***
// for an error text  , if write==1  , try to open and append the text  , in any
// case  , return the error text exitmode = 1  , exit
//
...
long Dayofwk(long d) {
  ...
  if (MBS_Flags(_USESRTDATES, 0, 1))
    ...
    // FACT: 1/1/2000 is Sunday (Excel varifies it)  , between 20000101 and
    // 00000101 there are 2000x365 days  , plus (2000/4 - 20 + 5) leap days  ,
    // mod 7 gives 0  , i.e.  , 00000101 is Sunday

    // last days on consecutive years  , if the first year is non-leap  , are
    // off by one week day; if leap  , by 2 days e.g.  , 12/31/2001 is Monday
    // while 12/31/2002 is Tuesday
...
// Date_Add
// if unit = day  , bus is needed: if bus  , advance by len bus-days  , else by
// len calendar days if unit = month or year  , BOTH bus and eom needed: first
// advance by suitale number of months  ,
//             and eom affects how to handle end of month
//			then  , if bus  , then move to the next bus day if it is not
//a bus day
...
//          , i.e.  , there are at least two price_x values on each side of the
//          entire distribution of dens_x values.
// WARNING: Further  , the dens arrays must always be at least as fine as the
// price arrays:
...
// for each z_i  , associates with x_j  , where z_i in (x_(j-1)  , x_j] or j=0
// or j=(N-2) get the corresponding result of interpolating z_i using qinterps
// with x[(j-1)  ,(j+1)] (and let x_(-1)=x_0 x_N=x_(N-1)) say the result is t_i
// , then return sum_i z_i * t_i use: say we have density function r(x) with
...
double qinterpp(int price_n, double *price_x, double *price, int dens_nb,
  ...
  //
  // For each segment of density array  , add its contribution to the average
  // price.
...
// INPUTS to QINTERPI
//
// price_x is an array of 4 variable values ("x values")  , only the first 3 are
// used price is an array of 4 prices at x values given by price_x  , only the
// first 3 are used dens_x is an array of 2 (generally different) x values  ,
// only the first is used dens is an array of 2 densities for the x values given
// by dens_x  , only the first is used
//
// WARNING: price_x and dens_x must each be in numerical order (from lowest to
// highest).
//          Further  , the interpolation is meant to be applied only when the
//          dens_x values are both between price_x[1] and price_x[2].
//
// OUTPUT of QINTERPI
...
double qinterpi(double *price_x, double *price, double *dens_x, double *dens) {
  ...
  //
  // Quadratic interpolations: the first three values  , then the last three
  // values (out of four).
...
// quadratic passing through points (x{i]  , y[i])  , i=0  ,1  ,2 amd read off
// the value at x

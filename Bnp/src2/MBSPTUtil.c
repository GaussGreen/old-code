...
//if readOnly>0, then just store flags[FlagNames] = Value and return value
//else read it off
int MBS_Flags(MBS_FlagNames FlagNames, int value, int readOnly)
{
	...
	if(!readOnly) mbs_flag_values[FlagNames]=value;
	return(mbs_flag_values[FlagNames]);
}//MBS_Flags

//***mbs_err_handler***
//for an error text, if write==1, try to open and append the text, in any case, return the error text
//exitmode = 1, exit
//
...
//****mbspt_dmatrix***
//Allocation of memory for a matrix of doubles [rowStart, colStart] x [colStart, colEnd]

double **mbspt_dmatrix(
		 int    rowStart,
		 int    rowEnd,
		 int    colStart,
		 int    colEnd)
{
...
#ifdef CHECK_MEMALLOC_LIM
	if( (colStart < rowStart) || (colEnd < colEnd) ) return 0;
#endif//CHECK_MEMALLOC_LIM

	m = (double **) calloc ((unsigned) (rowEnd-rowStart+1), sizeof (double*));
	if (!m)
	{
		mbs_err_handler("Can't calloc (double **) for dmatrix", MBS_WRITE_FILE, EXIT_FAILURE);
		return(0);
	}//if
	m -= rowStart;
	...
	for (i=rowStart; i<=rowEnd; i++)
	{
		...
		if (!m[i])
		{
			mbs_err_handler("Can't calloc *(double *) in dmatrix", MBS_WRITE_FILE, EXIT_FAILURE);
			return(0);
		}//if
		else
			m[i] -= colStart;
...
/***mbsptfree_dmatrix***
//Free matrix of doubles m [rowStart, colStart] x [colStart, colEnd]
*/
void mbspt_free_dmatrix (     double  **m,
			int     rowStart,
			int     rowEnd,
			int     colStart,
			int     colEnd)
...
YBASIS readYBASIS(char * code)
{
	...
	if(strcmp("ACT", code)==0) return(_YACT);
	if(strcmp("360", code)==0) return(_360);
	if(strcmp("365", code)==0) return(_365);
	return(_BADYBASIS );
}//readYBASIS

DIFFBASIS readDIFFBASIS( char * code)
{
	if(strcmp("ACT",code)==0) return(_DACT);
	if(strcmp("30",code)==0) return(_30);
...
//Dayofwk
//Calculates day of week (0-6) of YYYYMMDD formatted date: Sunday <-> 0, Saturday 6
long Dayofwk(long d)
{
	...
	{
	//FACT: 1/1/2000 is Sunday (Excel varifies it), between 20000101 and 00000101 there are 2000x365 days, plus (2000/4 - 20 + 5) leap days, mod 7 gives 0, i.e., 00000101 is Sunday

	//last days on consecutive years, if the first year is non-leap, are off by one week day; if leap, by 2 days
	//e.g., 12/31/2001 is Monday while 12/31/2002 is Tuesday
		...
		if (yy > 0) //whether we count from from 19000101 or 00000101 makes no difference
		{
					--yy;
...
long Nxtwkday(long date, long advance)
{
	...
	if(MBS_Flags(_USESRTDATES,0,1))
		return add_unit( date, advance, SRT_BDAY/*no holiday!!*/, NO_BUSDAY_CONVENTION);
	else
	{
		work_date = date;
		if (advance == 0)
...
int isHoliday(long date_i)
{
	...
	if(MBS_Flags(_USESRTDATES,0,1))
	{
		for(i=0; i < holiday.numHolidays; ++i)
			if(holiday.Holidays[i] == date_i) return(TRUE);
	}
	else
	{
		for(i=0; i < holiday.numHolidays; ++i)
			if(holiday.Holidays[i] == YYYYMMDD2XlDate(date_i)) return(TRUE);
	}
...
int isBusDay(long date_i)
{
	...
	if(MBS_Flags(_USESRTDATES,0,1))
	{
		if(dd==SAT || dd==SUN) return(0);//if Sunday or Sat.
	}
	else
	{
		if(dd==6 || dd==0) return(0);//if Sunday or Sat.
	}
	if(isHoliday(date_i)) return(0);//if Holiday
	return(1);
}//isBusDay
...
//Date_Add
//if unit = day, bus is needed: if bus, advance by len bus-days, else by len calendar days
//if unit = month or year, BOTH bus and eom needed: first advance by suitale number of months,
//             and eom affects how to handle end of month
//			then, if bus, then move to the next bus day if it is not a bus day
long Date_Add( long Ref_Date, DATE_UNIT unit, int len, int bus, int eom )
{
	...
	if(unit==DAY)
	{
		if(bus) return(MBSNxtbusday( Ref_Date, len));
		else return(Nxtday( Ref_Date, len ));
	}
	else
	{
		mo = len;
		...
		if(bus && !isBusDay(ret)) ret = MBSNxtbusday(ret,1);
		return(ret);
	}
}//Date_Add
...
long DateDiff(long date1_i, long date2_i, DIFFBASIS base )
{
	...
	if( base == _DACT ) return(day_count_date( date1_i, date2_i, BASIS_ACT_USD));
	else return(day_count_date( date1_i, date2_i, BASIS_30_360));
}
...
double linear_interp(int n, double * xs, double * ys, double x, int linear_extrapolate)
{
	int j;
	j=0;
	if( n == 0 ) return ys[0];
	...
	return ys[j-1] + (ys[j] - ys[j-1])/(xs[j] - xs[j-1]) * (x - xs[j-1]);
}//linear_interp
...
double loglinear_interp(int n, double * xs, double * ys, double x, int loglinear_extrapolate)
{
	int j;
	j=0;
	if( n == 0 ) return ys[0];
...
//
 ...
 //
 // WARNING: price_x and dens_x must each be in numerical order (from lowest to highest).
 //          Further, the interpolation is meant to be applied only when price_x[1] <= dens_x[1]
 //          and price_x[price_n-2] >= dens_x[dens_nb-1], i.e., there are at least two price_x
 //          values on each side of the entire distribution of dens_x values.
 // WARNING: Further, the dens arrays must always be at least as fine as the price arrays:
 //          Between two adjacent dens_x values there should be at most one price_x value.
 //
 // OUTPUT of QINTERPI
 //
//if price_n = N, price_x is x[0..(N-1)] price y[0..(N-1)]
//dens_nb = M, dens_x z[0..(M-1)] dens d[0..(M-1)]
//for each z_i, associates with x_j, where z_i in (x_(j-1), x_j] or j=0 or j=(N-2)
//get the corresponding result of interpolating z_i using qinterps with x[(j-1),(j+1)] (and let x_(-1)=x_0 x_N=x_(N-1))
//say the result is t_i, then return sum_i z_i * t_i
//use:
//say we have density function r(x) with values r[1..m] on [z[1..m], with m >> 1
...
double qinterpp(int price_n,double *price_x, double *price, int dens_nb, double *dens_x, double *dens)
{
...
//
 ...
 // i-th value gives index of price array whose value is just below that of i-th in the density array.
 
	for (i=0;  i< dens_nb; i++)
	{
		while (price_x[jcurr] < dens_x[i]) jcurr++;
		dens2pri[i] = jcurr;
		if (jcurr == 0) dens2pri[i] =1;		// if price_x is less than smallest dens_x, use the first
		if (jcurr >= (price_n-1)) dens2pri[i] = price_n-2; // p_x is too large, use the last 3 points  
	}
//
 // For each segment of density array, add its contribution to the average price.
 

	for (i=0; i< dens_nb; i++) {
	   if (dens2pri[i] ==1) totval += dens[i]*(price[0]+(dens_x[i]-price_x[0])*(price[1]-price[0])/(price_x[1]-price_x[0]));					// linear interpolation for the first interval 
	   else {
		jcurr = dens2pri[i];
		for (j = 0; j<3; j++) { 
...
//
 // INPUTS to QINTERPI
 //
 // price_x is an array of 4 variable values ("x values"), only the first 3 are used
 // price is an array of 4 prices at x values given by price_x, only the first 3 are used
 // dens_x is an array of 2 (generally different) x values, only the first is used
 // dens is an array of 2 densities for the x values given by dens_x, only the first is used
 //
 // WARNING: price_x and dens_x must each be in numerical order (from lowest to highest).
 //          Further, the interpolation is meant to be applied only when the dens_x values
 //          are both between price_x[1] and price_x[2].
 //
 // OUTPUT of QINTERPI
 //get the result y of interpolating dens_x[0] using qinterps on price_x[0..2] and price[0..2]
// and return y * dens[0]
...
//
 // Quadratic interpolations: the first three values, then the last three values (out of four).
...
//else, use the Lagrange polynomial interpolation formula to get the quadratic passing through points (x{i], y[i]), i=0,1,2
//amd read off the value at x
...
double qinterps(double *x, double *y, double x1, double *coeff)
{
	...
	coeff[2] = 0.;
//
// If not degenerate, make the adjustment for full quadratic interpolation.
 
	if ((x[0] != x[1]) && (x[1] != x[2]))
	{
		intmed2 = (y[1] -(y[0]+intmed1*(x[1]-x[0])))/ ((x[1]-x[0])*(x[1]-x[2]));

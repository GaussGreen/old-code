...
//***mbs_err_handler***
// for an error text, if write==1, try to open and append the text, in any case, return the error
// text exitmode = 1, exit
//
...
*/
void mbspt_free_dmatrix(double** m, int rowStart, int rowEnd, int colStart, int colEnd)
{
    int i;
...
long Dayofwk(long d)
{
    ...
    {
        // FACT: 1/1/2000 is Sunday (Excel varifies it), between 20000101 and 00000101 there are
        // 2000x365 days, plus (2000/4 - 20 + 5) leap days, mod 7 gives 0, i.e., 00000101 is Sunday

        // last days on consecutive years, if the first year is non-leap, are off by one week day;
        // if leap, by 2 days e.g., 12/31/2001 is Monday while 12/31/2002 is Tuesday
...
double loglinear_interp(int n, double* xs, double* ys, double x, int loglinear_extrapolate)
{
    ...
    return ys[j - 1] * pow(ys[j] / ys[j - 1], (x - xs[j - 1]) / (xs[j] - xs[j - 1]));
}  // loglinear_interp
...
// get the corresponding result of interpolating z_i using qinterps with x[(j-1),(j+1)] (and let
// x_(-1)=x_0 x_N=x_(N-1)) say the result is t_i, then return sum_i z_i * t_i use: say we have
// density function r(x) with values r[1..m] on [z[1..m], with m >> 1
...
double qinterpp(
    int price_n, double* price_x, double* price, int dens_nb, double* dens_x, double* dens)
{
    ...
    // i-th value gives index of price array whose value is just below that of i-th in the density
    // array.
    ...
    for (i = 0; i < dens_nb; i++)
    {
        ...
        dens2pri[i] = jcurr;
        if (jcurr == 0)
            dens2pri[i] = 1;  // if price_x is less than smallest dens_x, use the first
        if (jcurr >= (price_n - 1))
            dens2pri[i] = price_n - 2;  // p_x is too large, use the last 3 points
    }
    //
    ...
    for (i = 0; i < dens_nb; i++)
    {
        if (dens2pri[i] == 1)
            totval += dens[i] *
                      (price[0] + (dens_x[i] - price_x[0]) * (price[1] - price[0]) /
                                      (price_x[1] -
...
// else, use the Lagrange polynomial interpolation formula to get the quadratic passing through
// points (x{i], y[i]), i=0,1,2 amd read off the value at x

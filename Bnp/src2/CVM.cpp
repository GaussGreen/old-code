...
Err cvm_fill_struct(
                                        ...
                                        double			shift  ,
                                        int
converging_sliding  , int				interpollation_mode  ,
                                        int nbOfDates  , double
*vol_dates  ,
...
NO_BUSDAY_CONVENTION  , &end); cvm->pillars_maturities[i] = end; if(err) goto
FREE_RETURN; cvm->pillars_times[i] = (end - lToday) / 365.0;
        }

        cvm->pillars_vols = (double***) calloc(nbOfDates  , sizeof(double**));
        cvm->pillars_vols_norm = (double**) calloc(nbOfDates  ,
...
, sizeof(double)); cvm->pillars_correls[i][j] = (double*) calloc(nbOfPillars  ,
sizeof(double)); cvm->pillars_angles[i][j] = (double*) calloc(nbOfPillars  ,
sizeof(double)); cvm->pillars_chol[i][j] = (double*) calloc(nbOfPillars  ,
sizeof(double)); for(k=0;k<nbOfPillars;++k)
                        {

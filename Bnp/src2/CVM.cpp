    ...
    cvm_str* cvm)
{
    ...
    err = srt_f_get_spot_lag_from_refrate(refRate, (int*)&cvm->spotLag);
    if (err)
        goto FREE_RETURN;
    ...
    for (i = 0; i < nbOfDates; ++i)
    {
        ...
        CholDec(&CholAlg, cvm->pillars_correls[i], cvm->pillars_chol[i], nbOfPillars);
    ...
    if ((sensitype == 0) || (sensitype == 2) ||
        ((sensitype == 1) &&
         ((nbOfSensiTenors == 0) || (sensi_tenors == NULL) || (sensitivities == NULL))))
    {
        ...
        for (i = 0; i < nbOfPillars; ++i)
        {
            spline(
                cvm->pillars_times - 1,
                cvm->sensitivities[i] - 1,
                nbOfPillars,
                1.0e30,
                1.0e30,
                cvm->sensitivities2[i] - 1);
        }
                                        ...
                                        cvm_str *cvm)
{
        ...
        for(i=0;i<nbOfPillars;++i)
        {
                spline(cvm->pillars_times-1, cvm->sensitivities[i]-1, nbOfPillars, 1.0e30, 1.0e30,
cvm->sensitivities2[i]-1);
        }
...
Err cvm_copy_struct(cvm_str* cvm_target, cvm_str* cvm_source)
{
    ...
    err = cvm_fill_struct(
        ...
        cvm_source->refRate,
        cvm_source->shift,
        cvm_source->converging_sliding,
        cvm_source->interpollation_mode,
        cvm_source->nbOfDates,
        cvm_source->vol_dates,
        cvm_source->nbOfPillars,
        pillars_tenors,
        cvm_source->pillars_correls,
        cvm_source->pillars_vols_norm,
        cvm_source->sensi_type,
        cvm_source->lambdas,
        cvm_source->nbOfSensiTenors,
        sensi_tenors,
        cvm_source->sensitivities,
...
Err cvm_set_pillars_vols(int dateIndex, int nPillars, double* pillarsVols, cvm_str* cvm)
{
    Err err = NULL;
    int i, k;
...
Err cvm_bump_pillars_correls(int dateIndex, int nPillarsMinusOne, double* correls, cvm_str* cvm)
{
    Err err = NULL;
    int i, k;
    ...
    for (i = 0; i < nPillarsMinusOne; ++i)
    {
        for (k = 1; k < nPillarsMinusOne - i; ++k)
        {
            cvm->pillars_correls[dateIndex][k][i + k] =
                DMAX(-0.9999, DMIN(0.9999, cvm->pillars_correls[dateIndex][k][i + k] + correls[i]));
        }
    }
    ...
    CholDec(
        &CholAlg, cvm->pillars_correls[dateIndex], cvm->pillars_chol[dateIndex], cvm->nbOfPillars);
    ...
    for (i = 0; i < cvm->nbOfPillars; ++i)
    {
        ...
        {
            cvm->pillars_vols[dateIndex][k][i] =
                cvm->pillars_chol[dateIndex][k][i] * cvm->pillars_vols_norm[dateIndex][k];
        }
    }
...
Err cvm_set_pillars_correls(int dateIndex, int nPillars, double** correls, cvm_str* cvm)
{
    Err err = NULL;
    int i, k;
...
Err cvm_compute_fwdvol_at_voldate(
    int dateIndex, double maturity, int nPillars, double* fwdvol, cvm_str* cvm)
{
    Err     err = NULL;
    int     i, k;
    double* sensi = NULL;
    ...
    if (time > maturity)
    {
        if (dateIndex > 0)
        {
            err = cvm_compute_fwdvol_at_voldate(dateIndex - 1, maturity, nPillars, fwdvol, cvm);
            return err;
        }
        else
        {
...
Err cvm_compute_pillarsvols(
    double time, double maturity, int nPillars, double** pillarsvols, cvm_str* cvm)
{
    Err err = NULL;
    int i, j, k;
...
Err cvm_compute_fwdvol_sliding(
    double time, double maturity, int nPillars, double* fwdvol, cvm_str* cvm)
{
    Err      err   = NULL;
    double** vol   = NULL;
    double*  sensi = NULL;
...
Err cvm_compute_fwdvol_converging(
    double time, double maturity, int nPillars, double* fwdvol, cvm_str* cvm)
{
    Err     err  = NULL;
    double* vol1 = NULL;
    ...
    {
        err = cvm_compute_fwdvol_converging(
            time + cvm->vol_times[0] - maturity, cvm->vol_times[0], cvm->nbOfPillars, fwdvol, cvm);
        return err;
    }
    ...
    else
    {
        ...
        else
        {
            ...
            for (k = 0; k < nPillars; ++k)
            {
                ...
                {
                    fwdvol[k] =
                        vol2[k];  // vol1[k] + (vol2[k] - vol1[k]) * (time - cvm->vol_times[i-1]) /
                                  // (cvm->vol_times[i] - cvm->vol_times[i-1]);
                }
                else
                {
                    fwdvol[k] = vol1[k] + (vol2[k] - vol1[k]) * (time - cvm->vol_times[i - 1]) /
                                              (cvm->vol_times[i] - cvm->vol_times[i - 1]);
                }
            }
        }
...
Err cvm_compute_fwdvol(double time, double maturity, int nPillars, double* fwdvol, cvm_str* cvm)
{
    Err err = NULL;
...
Err cvm_vol(double start, double end, double* vol, cvm_str* cvm)
{
    ...
    int     iNFixPayDates, iNFixDates;
    long *  lFixPayDates = NULL, *lFixStartDates = NULL, *lFixEndDates = NULL;
    double* dFixCoverages = NULL;

    int   iNFloatPayDates, iNFloatDates;
    long *lFloatFixingDates = NULL, *lFloatPayDates = NULL, *lFloatStartDates = NULL,
         *lFloatEndDates    = NULL;
    ...
    for (i = 0; i < iNFloatDates; ++i)
    {
        ...
        {
            weights[i] =
                dFloatCoverages[i] * swp_f_df(lToday, lFloatPayDates[i + 1], cvm->ycid) / level;
        }
        else
        {
    ...
    if (cvm->shift != -1)
    {
        fwdswap = (swp_f_df(lToday, lFloatPayDates[0], cvm->ycid) -
                   swp_f_df(lToday, lFloatPayDates[iNFloatDates], cvm->ycid) + spreadleg) /
                  level;
    }
    ...
    if (exercise <= cvm->vol_dates[0])
    {
        ...
        for (k = 0; k < cvm->nQuadLegendre; ++k)
        {
            ...
            for (i = 0; i < iNFloatDates; ++i)
            {
                err = cvm_compute_fwdvol(
                    x[k], (lFloatStartDates[i] - lToday) / 365.0, cvm->nbOfPillars, volloc, cvm);
                if (err)
                    goto FREE_RETURN;
                for (j = 0; j < cvm->nbOfPillars; ++j)
                {
        ...
        for (l = 1; l < dateIndex; ++l)
        {
            ...
            for (k = 0; k < cvm->nQuadLegendre; ++k)
            {
                ...
                for (i = 0; i < iNFloatDates; ++i)
                {
                    err = cvm_compute_fwdvol(
                        x[k],
                        (lFloatStartDates[i] - lToday) / 365.0,
                        cvm->nbOfPillars,
                        volloc,
                        cvm);
                    if (err)
...
Err cvm_partial_forward_vol(
    double date, double expiry, double start, double end, double* vol, cvm_str* cvm)
{
    Err    err = NULL;
    ...
    else
    {
        ...
        if ((dateIndex2 < cvm->nbOfDates) && (dateIndex2 < dateIndex))
        {
            ...
            {
                GaussLeg(
                    cvm->vol_times[l - 1], cvm->vol_times[l], x - 1, w - 1, cvm->nQuadLegendre);
                for (k = 0; k < cvm->nQuadLegendre; ++k)
                {
                    for (j = 0; j < cvm->nbOfPillars; ++j)
                    {
            ...
            GaussLeg(
                cvm->vol_times[dateIndex - 1], exercise_time, x - 1, w - 1, cvm->nQuadLegendre);
            for (k = 0; k < cvm->nQuadLegendre; ++k)
            {
                for (j = 0; j < cvm->nbOfPillars; ++j)
                {
    ...
    cvm_str* cvm)
{
    ...
    int     iNFixPayDates1, iNFixDates1;
    long *  lFixPayDates1 = NULL, *lFixStartDates1 = NULL, *lFixEndDates1 = NULL;
    double* dFixCoverages1 = NULL;

    int   iNFloatPayDates1, iNFloatDates1;
    long *lFloatFixingDates1 = NULL, *lFloatPayDates1 = NULL, *lFloatStartDates1 = NULL,
         *lFloatEndDates1    = NULL;
    double* dFloatCoverages1 = NULL;
    double* dFloatSpreads1   = NULL;

    int     iNFixPayDates2, iNFixDates2;
    long *  lFixPayDates2 = NULL, *lFixStartDates2 = NULL, *lFixEndDates2 = NULL;
    double* dFixCoverages2 = NULL;

    int   iNFloatPayDates2, iNFloatDates2;
    long *lFloatFixingDates2 = NULL, *lFloatPayDates2 = NULL, *lFloatStartDates2 = NULL,
         *lFloatEndDates2    = NULL;
    ...
    {
        weights1[i] =
            dFloatCoverages1[i] * swp_f_df(lToday, lFloatPayDates1[i + 1], cvm->ycid) / level1;
    }
    ...
    {
        weights2[i] =
            dFloatCoverages2[i] * swp_f_df(lToday, lFloatPayDates2[i + 1], cvm->ycid) / level2;
    }
    ...
    if (exercise <= cvm->vol_dates[0])
    {
        ...
        for (k = 0; k < cvm->nQuadLegendre; ++k)
        {
            ...
            {
                err = cvm_compute_fwdvol(
                    x[k], (lFloatStartDates1[i] - lToday) / 365.0, cvm->nbOfPillars, volloc1, cvm);
                if (err)
                    goto FREE_RETURN;
                for (j = 0; j < cvm->nbOfPillars; ++j)
                {
            ...
            {
                err = cvm_compute_fwdvol(
                    x[k], (lFloatStartDates2[i] - lToday) / 365.0, cvm->nbOfPillars, volloc2, cvm);
                if (err)
                    goto FREE_RETURN;
                for (j = 0; j < cvm->nbOfPillars; ++j)
                {
...
Err cvm_compute_sensitivities(
    double time, double maturity, int nPillars, double* sensi, cvm_str* cvm)
{
    Err     err = NULL;
...
{
    Err err = NULL;
    err     = cvm_free_struct((cvm_str*)pUndDesc->spec_desc);
...
Err cvm_free_und_struct(void* pUndDesc)
{
    return cvm_free_und_struct((SrtUndPtr)pUndDesc);
};
...
Err cvm_free_struct(cvm_str* cvm)
{
    ...
    if (cvm)
    {
        ...
        {
            free_dmatrix(
                cvm->market_vols, 0, cvm->nbOfOptionTenors - 1, 0, cvm->nbOfUnderlyingTenors - 1);
            cvm->market_vols = NULL;
        }
        ...
        {
            free_dmatrix(
                cvm->model_vols, 0, cvm->nbOfOptionTenors - 1, 0, cvm->nbOfUnderlyingTenors - 1);
            cvm->model_vols = NULL;
        }
    ...
    int nQuadLegendre)
{
    ...
    cvm = (cvm_str*)calloc(1, sizeof(cvm_str));
...
Err sortspline(int nbasis, double* xbasis, double* ybasis, int n, double* x, double* y)
{
    Err     err = NULL;
    int     i;
    double* ybasis2 = NULL;

...
#define FREE_STOCHVOLCALIB_MEMORY {\
	if (pdMarketPrices) free_dvector(pdMarketPrices, 1, (long) iNumInstruments);\
	if (beta)			free_dvector(beta, 1, (long) iNumInstruments);\
	if (pdPrice_hat)	 free_dvector(pdPrice_hat, 1, (long) iNumInstruments);\
	if (lambda)			 free_dvector(lambda, 1, (long) iNumInstruments);\
	if (previousbeta)	 free_dvector(previousbeta, 1, (long) iNumInstruments);\
	if (previouspdPrice_hat) free_dvector(previouspdPrice_hat, 1, (long) iNumInstruments);\
	if (pdCalibratedPrices)	 free_dvector(pdCalibratedPrices, 1, (long) iNumInstruments);\
	if (determin_model_resid_error) free_dvector(determin_model_resid_error, 1, (long) iNumInstruments);\
	if (stoch_model_resid_error)	free_dvector(stoch_model_resid_error, 1, (long) iNumInstruments);\
	if (stoch_error)  free_dvector(stoch_error, 1, 20);\
	if (impl_vol_mkt) free_dvector(impl_vol_mkt, 1, (long) iNumInstruments);\
	if (impl_vol_det) free_dvector(impl_vol_det, 1, (long) iNumInstruments);\
	if (impl_vol_stc) free_dvector(impl_vol_stc, 1, (long) iNumInstruments);\
	pdMarketPrices = NULL;\
	beta = NULL;\
	lambda = NULL;\
	pdPrice_hat = NULL;\
...
double   *previousbeta = NULL;       /* previous value of beta, used to compute the lambdas */
double   *pdPrice_hat = NULL;         /* adjusted "market" pdPrices */ 
double   *lambda = NULL;				/* "optimal" coefficient in the fixed point algorithm */ 
double   *determin_model_resid_error = NULL; /* residual error deterministic volatility model */
double   *pdCalibratedPrices = NULL;       /* pdPrices from deterministic volatility model calibrated 
					on market pdPrices - step "m=0" */
double   *stoch_model_resid_error = NULL; /* residual error stochastic volatility model */
double   deter_sum_error;    /* "XI" for the deterministic volatility model - step "m==0" */
...
double   *impl_vol_det = NULL;      /* BS implied volatility - deterministic volatility model*/
double   *impl_vol_stc = NULL;      /* BS implied volatility - stochastic volatility model*/
...
/* Compute the BS implied volatilities for the market pdPrices */
	...
	{   
		err = swp_f_SwaptionImpliedVol_SwapDP( pdMarketPrices[i], &psSwapDp[i], pdStrike[i], peRecPay[i],
...
/* Creating the Grfn spreadsheet that will be used to price the deals*/
	...
	err = new_grfn_SwapDParray_to_GrfnCells(&STATIC_SPRDSHT.lNumEventDates,&STATIC_SPRDSHT.pszEventDates,
								   &STATIC_SPRDSHT.lNumRows,
								   &STATIC_SPRDSHT.lNumCols,&STATIC_SPRDSHT.pshtSHEET,today,iNumInstruments,
								   psSwapDp,pdStrike,pdBondStrike,peRecPay,peOptionType,und_name,pszRefRateCode);	
    ...
    /*--------------------------------------------------------------------------------------------------*/
	/* Now starts the calibration and in particular, the fixed point algorithm*/
    /*--------------------------------------------------------------------------------------------------*/	
	...
	/* For Calibration, use the user defined Grfn Parameters for Discretisation */
	...
	/* pdPrice the deals with the stochastic volatility model */
		...
		/* Computing lambda, the "direction of the gradient" in our algorithm*/
			if (m==0)
			{
				lambda[i] = 1;
			}
			...
			{
			lambda[i] = (pdPrice_hat[i]-previouspdPrice_hat[i])/(beta[i]-previousbeta[i]);
			}
			else
			/* we force the lambda to decrease */
			{
				if ((pdPrice_hat[i]-previouspdPrice_hat[i])/(beta[i]-previousbeta[i])<=lambda[i])
					{
					lambda[i] = (pdPrice_hat[i]-previouspdPrice_hat[i])/(beta[i]-previousbeta[i]);
					}
			}
			
			/* in the case lambda <0, the monte carlo error is too big to provide any info so we 
			stop the algorithm by keeping the lambda at 0 */
		...
		/* Update implied "market pdPrices" using the fixed point principle */
			pdPrice_hat[i] = pdPrice_hat[i] - lambda[i]*(beta[i] - pdMarketPrices[i]);
		...
		/* Compute the BS implied volatilities for the stochastic volatility model */
			...
			err = swp_f_SwaptionImpliedVol_SwapDP( beta[i], &psSwapDp[i], pdStrike[i], 
				peRecPay[i], pszRefRateCode[i], yc_name, SRT_LOGNORMAL, &impl_vol_stc[i]);
			if (err)
			{
				FREE_STOCHVOLCALIB_MEMORY;
				return serror("error in implied vol");
		...
		smessage("Stochastic volatility model calibration. Finish iteration: %d",m);
...
/*	if (stoch_sum_error <= deter_sum_error)*/
	  if (stoch_sum_error <= 0.000001)
		...
		if (m == max_iter)
		{
			smessage("Convergence criterion not satisfied at max_iter = %d iteration",m) ;

    ...
    double*          pdChiSquare)
{
    ...
    while (m <= max_iter)
    {
        ...
        for (i = 0; i < iNumInstruments; i++)
        {
            ...
            {
                if ((pdPrice_hat[i] - previouspdPrice_hat[i]) / (beta[i] - previousbeta[i]) <=
                    lambda[i])
                {
                    lambda[i] =
                        (pdPrice_hat[i] - previouspdPrice_hat[i]) / (beta[i] - previousbeta[i]);
                }
            }
            ...
            stoch_sum_error =
                stoch_sum_error + stoch_model_resid_error[i] * stoch_model_resid_error[i];

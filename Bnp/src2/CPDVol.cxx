...
Err cpd_alloc_smile_vol_market(int num_vols, SMILE_VOL_MARKET smile_mkt) {
  ...
  if (!smile_mkt->forward || !smile_mkt->sigma || !smile_mkt->alpha ||
      !smile_mkt->beta || !smile_mkt->rho || !smile_mkt->pi ||
      !smile_mkt->times || !smile_mkt->dates) {
...
Err cpd_fill_smile_vol_market(
    long today,
    int smile_spec_type, // 0 SABR with ATMLOG        , 1 SABR with ATMBETA , 2
                         // BMM
                         // ...
...
Err cpd_vol_get_vol(double forward, double fix_time, double strike,
  ...
  if (smile_params->smile_spec_type < 2) {
    ...
    err = srt_f_optsarbvol(
        forward, strike, fix_time, smile_params->sigma, smile_params->alpha,
        smile_params->beta, smile_params->rho,
        smile_params->smile_spec_type ? SRT_BETAVOL : SRT_LOGNORMAL, output_vol,
        smile_std);
  } else if (smile_params->smile_spec_type == 2 ||
...
Err cpd_vol_get_price(int type, double Forward, double Maturity, double Strike,
  ...
  if (smile_params->smile_spec_type <
      3) // SABR Log        , SABR Beta or BMM Log
  {
    if (type >= 3) {
  ...
  } else if (smile_params->smile_spec_type == 3) // BMM beta
  {
    if (type >= 3) {
      if (!smile_params->Use_BetaQuick_in_MC) {
        err = BMM_Option_Price(Forward, Strike, Maturity, smile_params->sigma,
                               smile_params->alpha, smile_params->beta,
                               smile_params->rho, smile_params->pi,
                               type == 3 ? SRT_CALL : SRT_PUT, Value);
        if (err)
          goto FREE_RETURN;
      } else {
        err = BMM_Option_Price_quick(
            Forward, Strike, Maturity, smile_params->sigma, smile_params->alpha,
            smile_params->beta, smile_params->rho, smile_params->pi,
            type == 3 ? SRT_CALL : SRT_PUT, Value);
...
Err cpd_vol_get_smile_params(
  ...
  if (IsTime) {
    smile_params->alpha = interp(smile_mkt->times, smile_mkt->alpha,
                                 smile_mkt->num_vols, time, 0, NULL);
    smile_params->beta = interp(smile_mkt->times, smile_mkt->beta,
                                smile_mkt->num_vols, time, 0, NULL);
    smile_params->rho = interp(smile_mkt->times, smile_mkt->rho,
                               smile_mkt->num_vols, time, 0, NULL);

    if (smile_mkt->smile_spec_type == 2 || smile_mkt->smile_spec_type == 3)
      smile_params->pi = interp(smile_mkt->times, smile_mkt->pi,
                                smile_mkt->num_vols, time, 0, NULL);
  } else {
    ...
    smile_params->alpha =
        interp(dates, smile_mkt->alpha, smile_mkt->num_vols, date, 0, NULL);
    smile_params->beta =
        interp(dates, smile_mkt->beta, smile_mkt->num_vols, date, 0, NULL);
    smile_params->rho =
        interp(dates, smile_mkt->rho, smile_mkt->num_vols, date, 0, NULL);

    if (smile_mkt->smile_spec_type == 2 || smile_mkt->smile_spec_type == 3)
      smile_params->pi =
          interp(dates, smile_mkt->pi, smile_mkt->num_vols, date, 0, NULL);
  }
...
Err cpd_vol_get_linterpVol(
  ...
  if (IsTime)
    smile_params->sigma = interp(smile_mkt->times, smile_mkt->sigma,
                                 smile_mkt->num_vols, time, 0, NULL);
  else {
    dates = calloc(smile_mkt->num_vols, sizeof(double));
    for (j = 0; j < smile_mkt->num_vols; j++)
    ...
    smile_params->sigma =
        interp(dates, smile_mkt->sigma, smile_mkt->num_vols, date, 0, NULL);
  }

...
Err swp_f_CMT_swap(long start, long end_or_nfp, String cmt_freq_str,
                   ...
                   String info_str, SrtCurvePtr crv, double *value) {
  Err err;
  SwapDP cmt_swapdp;
  SwapDP swp_swapdp;
  Date today;
  ...
  if (err = swp_f_initSwapDP(start, end_or_nfp, cmt_freq_str, cmt_basis_str,
                             &cmt_swapdp)) {
    return err;
  }
  ...
  Arg_set(cmt_arg, ARG_COMPD, cmt_swapdp.cxxompd);
  ...
  err = swp_f_setSwapDP(start, end_or_nfp, cmt_param->swap_compd,
                        cmt_param->swap_basis_code, &swp_swapdp);
  if (err)
    return err;
  ...
  case COMPUTE_FWD_PV:
    ...
    /* Or compute separately the PV of the Fixed leg (with swap)*/
    {
      ...
      err = zcswap(&swp_swapdp, strike, 0.0, 0.0, COMPUTE_PV, yc_name, today,
                   &fixed_leg_price);
  ...
  case COMPUTE_FWD_RATE:
    ...
    err =
        zcswap(&swp_swapdp, 1.0, 0.0, 0.0, COMPUTE_PV, yc_name, today, &level);

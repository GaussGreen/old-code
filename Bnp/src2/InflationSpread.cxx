...
Err srt_InflationSpreadOption(double dRate, double dInflation,
                              struct sInflationArg *argPtr, double *pdOption);
...
double NAG_CALL inflationIntegrand(Integer iDim, double *x, Nag_User *comm) {
  ...
  // From the realized value of the spread        , calculate the forward
  // spreads

  // From the value of the short rate        , calculate the required discount
  // bonds At the same time compute the forward
...
Err srt_InflationSpreadOption(double dRate, double dInflation,
  ...
  *pdOption = 0.0;
  for (i = 0; i < argPtr->iNumDates; i++)
    *pdOption +=
        argPtr->dvForwardDF[i] * exp(argPtr->dvPsiDiff[i] * dRate / argPtr->dF -
...
Err setInflationStructure(char *csUndName, Date dtExpiry, Date *dtvMaturity,
                          int iNumDates, struct sInflationArg *sArg) {
  /* Variable declarations */
  ...
  sArg->dRateMean = 0.0; /* swp_f_zr( dtExpiry-7.0        , dtExpiry+7.0 ,
                            csYCName ); */
...
Err srt_InflationSpread(char *csUndName, Date dtExpiry, Date *dtvMaturity,
                        int iNumDates, double *dpIntArg, double *dpPrice) {
  // Variable declarations
  double dEps, dAcc;
  ...
  if (err = setInflationStructure(csUndName, dtExpiry, dtvMaturity, iNumDates,
                                  &sArg))
    return err;

  /* Set the upper and lower limits        , the number of points and the
   * desired accuracy (or use default values) */
  iDim = 2;
  ...
  nag_multid_quad_adapt_1(iDim, &inflationIntegrand, dvLowerLimits,
                          dvUpperLimits, &iMinPts, iMaxPts, dEps, dpPrice,
                          &dAcc, &nagArg, &neFail);
  *dpPrice *= sArg.dExpiryDF;

  if (neFail.cxxode != NE_NOERROR)
    err = neFail.message;
...
/* ----------------------------------------------------------------------------------------------------------------------------------
 */
/* Calculates the mean of a Vasicek underlying in a forward measure defined by
 * an LGM */
Err srt_get_vasicek_forward_measure_und_mean(double dTime, double dFwdTime,
                                             double dCorrelation,
  ...
  Err err;
  double dIntegral, dPsiFwdMeasure, dUpperTime, dLowerTime, dTimeDiff, lambda,
      a;

  /* The mean is equal to the mean in the risk neutral measure        , plus a
   * correction.  Begin with the risk-neutral mean */
  ...
  while ((plsLGM != NULL) &&
         (((IrmTermStructVal *)plsLGM->element->val.pval)->time < dTime))
    plsLGM = plsLGM->next;
  if (plsLGM == NULL)
    plsLGM = ptsLGM->tail;
  ...
  while (plsVasicek != ptsVasicek->head && plsLGM != ptsLGM->head) {
    ...
    dIntegral += pvalLGM->sig / pvalLGM->F * pvalVasicek->sig *
                 pvalVasicek->tau / pvalVasicek->F *
                 ((dPsiFwdMeasure - pvalLGM->Psi - pvalLGM->F / lambda) /
                      (a + lambda) * (exp((a + lambda) * dTimeDiff) - 1.0) +
 ...
 */
/* Calculates the inflation forward */
Err srt_InflationSpreadForward(char *csVasicekUnd, char *csLGMUnd,
                               Date dtExpiry, double dCorrelation,
                               double *pdForward) {
  /* Variable declarations */
...
Err srt_CalibrateInflationSpread(char *csYCname, double dSpotSpread,
                                 double dRevSpeed, double dCorrelation,
                                 double dSpreadVol, double dTau, Date dtToday,
                                 int iNumDates, double *dvFwdSpread,
                                 double *dvLGMvol, Date *dtvDates,
  ...
  /* Loop over the dates        , calculating the times and the difference of
   * the exponentials */
  ...
  /* Loop over the dates        , calcualating the reversion level up to that
   * time */
  dTimeM1 = 0.0;
  for (i = 0; i < iNumDates; i++) {
    dSum1 = dvLGMvol[0] * ((exp(dRevSpeed * dvTimes[0]) - 1.0) / dRevSpeed -
                           exp(dLambda * dvTimes[i]) *
                               (exp((dRevSpeed + dLambda) * dvTimes[0]) - 1.0) /
                               (dRevSpeed + dLambda));
    dSum2 = 0.0;
    for (j = 0; j < i; j++) {
      dSum1 +=
    ...
    dvCalibratedLevels[i] =
        (dvFwdSpread[i] / swp_f_df(dtToday, dtvDates[i], csYCname) -

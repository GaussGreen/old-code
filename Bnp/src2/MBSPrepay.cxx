...
#define ALSMM1FUNC(x, y, N)                                                    \
  ((y) * (x) - (1. - (x)) * (1. - pow((1. - (x)), (N - 1))))
#define ALSMM2FUNC(p, N)                                                       \
  ((1. - (p)-pow(1. - (p), (N) + 1)) / p - (N)*pow(1. - (p), (N)))
#define small 0.00001
///

// explicit initialization for one one S curve and NBPOINTS knot-points for
// S-curve call MBS_Prepay_Engine_Initialize
char *MBS_Prepay_Engine_Params_ReaderExplicit(
    char *data_dir, MBS_Prepay_Engine *prepay_engine, MTGPTACYPROG AgencyProg,
    long oterm, double gwac, double Speedup, double rational,
    double FebruaryEffect, double *IndexLevel, double *AmortLevel,
    double *SeasoningLevel, double *SeasoningMat, int SFSNb, double *Amorts,
    double Seasonality[12], int Delay, int NbFixedPrepay, double *FixedPrepay,
    int nuniform, int IndexMat, int EndIndexMat, int IndexFreq) {
  char *err;
  int i;
  double *triggerRates;
  ////
  ...
  MBS_Prepay_Engine_Initialize(
      prepay_engine, 1, NBPOINTS, IndexLevel, &(AmortLevel), Seasonality,
      FebruaryEffect, NUMRAMPS, SeasoningLevel, SeasoningMat, Speedup, rational,
      SFSNb, triggerRates, Amorts, Delay, NbFixedPrepay, FixedPrepay,
      AgencyProg, oterm, gwac, 0, 0, nuniform, IndexMat, EndIndexMat,
      IndexFreq);
...
char *MBS_Prepay_Engine_Params_Reader(char *data_dir,
  ...
  double IndexLevel[NBPOINTS], *AmortLevel, SeasoningLevel[NUMRAMPS],
      Seasonality[12];
  ...
  err = 0;
  //	if( !(stream = MBS_Sequential_File_Manager( data_dir        ,
  //"PrepayFunc.dat"
  //)) )
  ...
  if (err = MBS_Prepay_Engine_Params_ReaderExplicit(
          data_dir, prepay_engine, AgencyProg, oterm, gwac, Speedup, rational,
          FebEffect, IndexLevel, AmortLevel, SeasoningLevel, SeasoningMat,
          2 * SFSNb, Amorts, Seasonality, Delay, NbFixedPrepay, FixedPrepay,
          nuniform, IndexMat, EndIndexMat, IndexFreq))
    goto Free_Mem;
...
// various adjustments from mtg rate to 10Y rate and from ratio form to diff
// form of refi-incentive change from CPR to smm
char *MBS_Prepay_Engine_SetParams(MBS_Prepay_Engine *prepay_engine,
                                  double refiRate, double Rate10Y) {
  int i, j, k;
  ...
  adjust_density(&(prepay_engine->density),
                 (prepay_engine->Speedup) * (prepay_engine->rational),
                 prepay_engine->gwac, refiRate, Rate10Y,
                 prepay_engine->nuniform);
  adjust_history(&(prepay_engine->history), prepay_engine->oterm, refiRate,
                 Rate10Y);
  ...
  // tweaks
  for (i = 0; i < prepay_engine->NbSCurve; ++i)
    spdbase(prepay_engine->AmortLevel[i], prepay_engine->Speedup,
            prepay_engine->rational, prepay_engine->NbIndexLevel);
  // IndexLevel
  for (j = 0; j < prepay_engine->NbIndexLevel; ++j)
  ...
  for (j = 0; j < prepay_engine->NbSeasoningRamps; ++j)
    prepay_engine->SeasoningLevel[j] =
        transformIncentive(prepay_engine->SeasoningLevel[j],
                           prepay_engine->gwac, refiRate, Rate10Y);
  // triggers
  ...
  if ((0.96 * prepay_engine->gwac + REFI_THRESHOLD - refiRate) <= 0.01) {
    ...
    for (i = 0; i < half_SFSNb; i++)
      prepay_engine->TriggerRate[i] =
          refiRate - 1.0 +
          ratio1 * pow(1.0 - (double)i / (double)half_SFSNb * ratio2, 3);
  } // else
  for (i = 0; i < half_SFSNb; ++i) {
    prepay_engine->TriggerRate[i] -= refiRate;
  ...
  for (i = 0; i < prepay_engine->NbFixedPrepay; ++i) {
    ...
    prepay_engine->FixedPrepay[i] =
        1.0 - pow(1.0 - prepay_engine->FixedPrepay[i], 1.0 / 12.0);
  };
  return (0);
...
char *MBS_Prepay_Engine_Check(MBS_Prepay_Engine *prepay_engine) {
  ...
  if (prepay_engine->FebruaryEffect <= 0.0 ||
      prepay_engine->FebruaryEffect >= 1.0)
  ...
  for (i = 0; i < prepay_engine->NbFixedPrepay; ++i)
    if (prepay_engine->FixedPrepay[i] < 0.0 ||
        prepay_engine->FixedPrepay[i] > 1.0)
      return (nrerror("FixedPrepay must be in [0        ,1]!"));

  //		if( prepay_engine->agency_char != 'F' &&
  //prepay_engine->agency_char
  //!= 'G' ) return(nrerror( "agency_char: F or G" ));
...
void MBS_Prepay_Engine_Initialize(
    MBS_Prepay_Engine *prepay_engine, int NbSCurve, int NbIndexLevel,
    double *IndexLevel, double **AmortLevel, double Seasonality[],
    double FebruaryEffect, int NbSeasoningRamps, double *SeasoningLevel,
    double *SeasoningMat, double Speedup, double rational, int SFSNb,
    double *TriggerRate, double *Amort, int Delay, int NbFixedPrepay,
    double *FixedPrepay, MTGPTACYPROG AgencyProg, long oterm, double gwac,
    HISTORY *history, MBSDENSITY *density, long nuniform, int IndexMat,
    int EndIndexMat, int IndexFreq) {
  int i, j;
  ...
  if (NbIndexLevel > 0 && NbSCurve > 0) {
    ...
    prepay_engine->AmortLevel =
        mbspt_dmatrix(0, NbSCurve - 1, 0, NbIndexLevel - 1);
    for (i = 0; i < prepay_engine->NbIndexLevel; ++i) {
  ...
  if (NbSeasoningRamps > 0) {
    prepay_engine->SeasoningLevel =
        mbspt_dvector(0, prepay_engine->NbSeasoningRamps - 1);
  ...
  if (prepay_engine->NbFixedPrepay > 0) {
    prepay_engine->FixedPrepay =
        mbspt_dvector(0, prepay_engine->NbFixedPrepay - 1);
    for (i = 0; i < prepay_engine->NbFixedPrepay; ++i)
...
void MBS_Prepay_Engine_ReplaceTriggers(MBS_Prepay_Engine *new_engine,
                                       MBS_Prepay_Engine *prepay_engine,
                                       int num_buckets, double *triggerRate,
                                       double *amort) {
  MBS_Prepay_Engine_Initialize(
      new_engine, prepay_engine->NbSCurve, prepay_engine->NbIndexLevel,
      prepay_engine->IndexLevel, prepay_engine->AmortLevel,
      prepay_engine->Seasonality, prepay_engine->FebruaryEffect,
      prepay_engine->NbSeasoningRamps, prepay_engine->SeasoningLevel,
      prepay_engine->SeasoningMat, prepay_engine->Speedup,
      prepay_engine->rational, num_buckets, triggerRate, amort,
      prepay_engine->Delay, prepay_engine->NbFixedPrepay,
      prepay_engine->FixedPrepay, prepay_engine->AgencyProg,
      prepay_engine->oterm, prepay_engine->gwac, &(prepay_engine->history),
      &(prepay_engine->density), prepay_engine->nuniform,
      prepay_engine->IndexMat, prepay_engine->EndIndexMat,
...
void MBS_Prepay_Engine_Cleanup(MBS_Prepay_Engine *prepay_engine) {
  if (prepay_engine->NbIndexLevel > 0)
    mbspt_free_dvector(prepay_engine->IndexLevel, 0,
                       prepay_engine->NbIndexLevel - 1);
  if (prepay_engine->NbSCurve > 0 && prepay_engine->NbIndexLevel > 0)
    mbspt_free_dmatrix(prepay_engine->AmortLevel, 0,
                       prepay_engine->NbSCurve - 1, 0,
                       prepay_engine->NbIndexLevel - 1);
  if (prepay_engine->NbSeasoningRamps > 0)
    mbspt_free_dvector(prepay_engine->SeasoningLevel, 0,
                       prepay_engine->NbSeasoningRamps - 1);
  if (prepay_engine->NbSeasoningRamps > 0)
    mbspt_free_dvector(prepay_engine->SeasoningMat, 0,
                       prepay_engine->NbSeasoningRamps - 1);
  ...
  if (prepay_engine->NbFixedPrepay > 0)
    mbspt_free_dvector(prepay_engine->FixedPrepay, 0,
                       prepay_engine->NbFixedPrepay - 1);
}
...
char *MBS_Prepay_FactorHistory(MBS_Prepay_Engine *prepay_engine, int init_age,
                               int init_month, int num_rates,
                               double *input_rates, int num_buckets,
                               double *triggerRate, double *amort,
                               int useInitialSMM, double **factors)
// num_buckets <=0 means uses prepay_engine's SFSNb        , TriggerRate , and
// Amort; assumes there is no delay
{
  int i, j;
  ...
  MBS_Prepay_Engine_ReplaceTriggers(&new_engine, prepay_engine, newSFSNb,
                                    newTriggerRate, newAmort);
  ...
  // MBS_Prepay_Engine_SMM(factors[0]        , &new_engine        , age        ,
  // input_rates[0]        , monthOfYear        , monthfromsettle);
  for (j = 0; j < new_engine.SFSNb; ++j)
    factors[0][j] = 1.0;
  for (i = 1; i <= num_rates; ++i) {

    //#ifdef MBSPTCORRECTPPHISTORY
    MBS_Prepay_Engine_SMM(factors[i], &new_engine, age + 1, input_rates[i - 1],
                          monthOfYear, monthfromsettle);
    //#else

    //		MBS_Prepay_Engine_SMM(factors[i]        , &new_engine        , age
    //, input_rates[i-1]        , monthOfYear        , monthfromsettle);
...
char *MBS_Prepay_Path(MBS_Prepay_Engine *prepay_engine, long TradeYYYYMMDD,
                      long spotYYYYMMDD, long wam, int subgroup,
                      int num_future_rates, double *futureRates,
                      int useFixedPrepay, int *numProjections,
                      double rates[MAXOTERM], double cpr[MAXOTERM],
                      double principal_payments[MAXOTERM])
// subgroup == 0 <=> aggregated        , slow group comes first starting with 1
// then fast group principal_payments assume notional of $1 as of spotYYYYMMDD
{
  ...
  *numProjections = min(wam, num_future_rates);
  ...
  numHRates = Delay;
  if ((spot_position > prepay_engine->history.Number) ||
      (spot_position - Delay < 0))
    return (nrerror("Incomplete history"));
  // fill in future rates
  // recover mtg sprd and transform into 10YRate space
  sprd = (prepay_engine->density.IncentKnots[0] * prepay_engine->gwac -
  ...
  while (mo < 1)
    mo += 12;
  MBS_Prepay_FactorHistory(prepay_engine, wala - Delay, mo, *numProjections,
                           rates, -1, 0, 0, useFixedPrepay, factors);
  // get distribution as of spot
  weights = mbspt_dvector(0, prepay_engine->SFSNb - 1);
  if (!subgroup) {
    if (err = get_Trigger_Distribution(prepay_engine, TradeYYYYMMDD, wam,
                                       weights))
      return (err);
  } else {
    for (i = 0; i < prepay_engine->SFSNb; ++i)
  ...
  // for(i=0; i < prepay_engine->NbFixedPrepay; ++i) cpr[i] =
  // prepay_engine->FixedPrepay[i];
  // before turning smm to cpr        , calc factor history:
  if (err = get_initial_principal_payments(*numProjections, principal_payments,
                                           wam, prepay_engine->gwac,
                                           *numProjections, cpr))
    return (err);
  //
  for (i = 0; i < *numProjections; ++i)
  ...
  mbspt_free_dmatrix(factors, 0, *numProjections - 1, 0,
                     prepay_engine->SFSNb - 1);
...
void get_smm_weights(double *weights, MBS_Prepay_Engine *prepay_engine,
                     double *usweights, double *ufweights) {
  ...
  for (i = 0; i < subgroupSFSNb; ++i) {
    ...
    if (i)
      vec[i - 1] = 0.0;
    vec[i] = 1.0;
    swt = qinterpp(subgroupSFSNb, reversedTrigger, vec, nuniform,
                   prepay_engine->density.urates, usweights);
    fwt = qinterpp(subgroupSFSNb, reversedTrigger, vec, nuniform,
                   prepay_engine->density.urates, ufweights);
...
// monthOfYear = 2 for February prepay        , though the reset date for
// getting IndexValue may be March 1
int MBS_Prepay_Engine_SMM(
    double *output, MBS_Prepay_Engine *prepay_engine, int paymentNb,
    double IndexValue, int monthOfYear,
    int monthFromSettle) // this assumed there is NO prepay-delay i.e.        ,
                         // if there is delay        , the input paymentNb ,
                         // IndexValue
                         //       , monthOfYear and monthFromSetlle        ,
                         //       etc.        , have to
                         // adjusted properly
// paymentNb < 0 => base smm = 0.0        , only asmm
{
  ...
  if (prepay_engine->NbSCurve == 1)
    ...
    if (monthFromSettle > prepay_engine->NbSCurve + 6)
      SCurveIndex = prepay_engine->NbSCurve - 1;
    else if (monthFromSettle <= prepay_engine->NbSCurve + 6 &&
  ...
  smm = linterpFlat(prepay_engine->NbIndexLevel, prepay_engine->IndexLevel,
                    prepay_engine->AmortLevel[SCurveIndex], IndexValue);

  // Seasonality
  mo = monthOfYear;
  smm *= prepay_engine->Seasonality[mo - 1];
  // Seasoning:
  seasoningFactor = ((double)paymentNb) /
                    linterpFlat(prepay_engine->NbSeasoningRamps,
                                prepay_engine->SeasoningLevel,
                                prepay_engine->SeasoningMat, IndexValue);
  ...
  for (i = 0; i < prepay_engine->SFSNb; ++i) {
    ...
    //		if (monthOfYear == 2) output[i] *=
    // prepay_engine->FebruaryEffect; #endif//#ifndef CORRECTFEBEFFECT
  }
  return 0;
...
double alsmm1(double al, int N) /* al is average life in years        , N is
...
/*convert average life to smm*/
double
alsmm2(double al, double wac, int N, int wam,
       int pt) /* al -avg life        , wac -- gross coupon        , N --
                  original term wam -- remaining term        , pt -- N-wam*/
{
  ...
  for (i = 0; i < 30; i++) {
    ...
    y3 = al - 1. - (factor1 / factor2) * ALSMM2FUNC(x3, wam) -
         ALSMM2FUNC(x3 * (1. + wac / 1200.) - wac / 1200., wam) * factor3 /
             factor1;
    if (y3 > 0.)
      x1 = x3;
    else
...
// get dense representation of the density function of various trigger groups at
// the start (settle date or trade date?)

char *get_Trigger_Distribution(MBS_Prepay_Engine *prepay_engine,
                               long spotYYYYMMDD, long wam, double *weights) {
  int i;
  double sum, sum1;
  ...
  spot_position = month_diff(YYYYMM2FirstOfMon(prepay_engine->history.dates[0]),
                             spotYYYYMMDD);
  ...
  if ((spot_position - age - Delay < 0) ||
      (spot_position - age - Delay + num_rates - 1 >=
       prepay_engine->history.Number))
    return (nrerror("Incomplete History!"));
  ...
  for (i = 0; i < num_rates; ++i) {
    input_rates[i] =
        prepay_engine->history.hrates[spot_position - age + i - Delay];
  }

  Dsplit(
      Date_Add(YYYYMM2FirstOfMon(
                   prepay_engine->history.dates[spot_position - age - Delay]),
               MONTH, -Delay, 0, 1),
      &yy, &init_month, &dd);
  MBS_Prepay_FactorHistory(prepay_engine, -Delay, init_month, num_rates,
                           input_rates, num_buckets, urates, fastAmort, 0,
                           fastFactors);
  MBS_Prepay_FactorHistory(prepay_engine, -Delay, init_month, num_rates,
                           input_rates, num_buckets, urates, slowAmort, 0,
                           slowFactors);
...
char *get_initial_principal_payments(int num_payments, double *out, int wam,
  ...
  // out[0] = (1.0 - pow(u        , moving_wam-1))/(1.0 - pow(u ,moving_wam)) *
  // ( 1.0
  // - FixedPrepay[0] );
  ...
  for (i = 1; (i < num_payments) && (moving_wam > 1); ++i) {
    moving_wam--;
    out[i] = out[i - 1] *
             (1.0 - TOTAL_AMORT_RATE(moving_wam, gwac, FixedPrepay[i] * 100.0));
    // out[i] = out[i-1] * (1.0 - pow(u        ,moving_wam-1))/(1.0 - pow( u ,
    // moving_wam)) * (1.0 - FixedPrepay[i]);
  }
  if (i < num_payments) {
...
spotYYYYMMDD        , long  wam        , int num_future_rates)
{
        ...
        int i;
        double rates[MAXOTERM]        , cpr[MAXOTERM]        ,
principalPayDowns[MAXOTERM]; int numProjections        , spot_position;

        firstHistYYYYMM = prepay_engine->history.dates[0];
        spot_position = month_diff( firstHistYYYYMM*100+1        ,
spotYYYYMMDD); last_rate = prepay_engine->history.hrates[spot_position] +  (
prepay_engine->density.IncentKnots[0] * prepay_engine->gwac -
prepay_engine->density.irates[0] ); future_rates = mbspt_dvector(0        ,
num_future_rates-1); for(i=0; i < num_future_rates; ++i) future_rates[i] =
last_rate;

        MBS_Prepay_Path( prepay_engine        , spotYYYYMMDD        , wam , 0 ,
num_future_rates        , future_rates        , &numProjections        , rates
, cpr        , principalPayDowns); fp =
fopen("C:/workarea/mortgage/jymodel1/fixedpp.txt"
      ,"w"); for(i=0; i < numProjections; ++i)
        {
                fprintf(fp        , "rates[%d] = %f        , cpr = %f        ,
paydown=%f\n"        , i , rates[i]        , cpr[i]*100.0        ,
principalPayDowns[i]*100.0);
        }

        mbspt_free_dvector( future_rates        , 0        ,
num_future_rates-1); fclose(fp);
}
***/

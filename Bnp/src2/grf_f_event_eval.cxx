/******************************************************************************\
    ...
    RETURNS         :   An error
                                                Affect in the cash-flow input
the value of the entire event        , which will be the cashflow in the
rightmost of the tableau.

                        Result = f(sample        , fwd_sample        , gd ,
[tree information])
...
return NULL (no error).
                        ...
                        If pv_vect ==  NULL        , use an internal array to
take the place of pv_vect (For Monte-Carlo pv_vect can be NULL). COMLL_PTR
inside event will still contain nodes of type COMLL_T_INCREMENT        , because
it is unknown when an event is being constructed which numerical method will be
used.

                        In order to avoid numerical overflow when computing
                        swap rates when the value of the appropriate level
                        payment is too small        , it will approximate the
swap rate with the short rate.
...

{
  ...
  if (pv_vect != NULL) {
    ...
    /* In this case        , all the PV's will be computed on a local array
  ...
  while (comll != NULL) {
    ...
    if ((type_eval == 2) ||
        ((type_eval == 0) && (comll->evalstatus == FWDCELL)) ||
      ...
      case COMLL_F_PAYFROMPAST:
        ...
        /* Puts the discounted amount back into the stack (fro proper event
         * treatment) */
        PUSH(rubbish);
        comll = comll->next;
      ...
      case COMLL_F_CMS:
        ...
        /* Compute the floating leg value with all spreads (y): ini + fin +
         * spreads */
        y = event->df[ix][comll->dfind[0]] -
            event->df[ix][comll->dfind[comll->dfindlen - 1]];
        for (i = 1; i < comll->dfindfloatlen; i++) {
          y += event->df[ix][comll->dfindfloat[i]] * comll->spread[i - 1] *
               comll->cvgfloat[i - 1];
        }
        ...
        /*					error = swp_f_cmsratedp(
                                                                (SwapDP
           *)comll->ptr        , event->d        , x        , vol        ,
           DTOL(delay)        , DEFAULT_CMS_DELTA , MAX_CMS_SWAPS        ,
           difftype        , &x);	*/

        error = swp_f_cmsratedp_clsdfrm((SwapDP *)comll->ptr, event->d, x, vol,
                                        DTOL(delay), difftype, &x);
      ...
      case COMLL_A_BSSABR: /* Black-Scholes with SABR */
        ...
        rho = POP();
        y = POP(); /* time difference = mat */
        difftype = (SrtDiffusionType)
            POP(); /* 0 = Log        , 1 = Norm        , 2 = beta */
        call_put = (int)POP();

        /*					if (error =
           srt_f_optbetastochvoltoblkvol( f        , k        , betavol        ,
           alpha        , rho        , y        , beta        , &x)) return
           error;
        */

        if (error = srt_f_optsarbvol(f, k, y, betavol, alpha, beta, rho,
                                     difftype, SRT_LOGNORMAL, &x))
          return error;
        if (x <= 0.0) {
          if (call_put) /* put */
            x = k > f ? k - f : 0.0;
      ...
      case COMLL_A_NUMDAYS: /* number of days in a corridor */
        ...
        PUSH(prob_ExpectedDays(f, spot, dBarrier, vol, delay, mat, nstep, dPar,
                               i));
        comll = comll->next;
        break;
      ...
      case COMLL_C_ROWSUM:
      case COMLL_C_ROWAVG:
      case COMLL_C_ROWMAXIDX: /* Cyril Godart Regis Benichou fecit per Seculum
                                 Secolorum */
      ...
      case COMLL_C_ROWINTERP:
        ...
        x = POP();
        /* Interpolate y=f(x) using the row as Y values and the auxiliary range
         * as X values */
        grf_f_evlcllfct_interp(gd->aux[ind], gd->cells, r1, c1, c2, x,
                               comll->type, &y);
        PUSH(y);
        comll = comll->next;
        break;
      ...
      case COMLL_T_PVSOURCE_ASSIGN:
        ...
        return serror(
            "No mention of SOURCE is allowed with this GRFN function !");
        break;
      ...
      case COMLL_X_INTERP:
        ...
        x = POP();
        y = x;
        interp(gd->aux[r1], gd->aux[r2], IMIN(gd->auxlen[r1], gd->auxlen[r2]),
               y, 0, &x);
        PUSH(x);
        comll = comll->next;
        break;
      ...
      case COMLL_X_PVINTERP:
        ...
        /* Interpolate y=f(x) over the values of this row using the x_values
         * input */
        //					grf_f_evlcllfct_interp(        ,
        //gd->cells        , r1        , c1        , c2        , x        ,
        // comll->type        , &y);
        PUSH(y);
        comll = comll->next;
      ...
      case COMLL_PRINT:
        ...
        if (gd->outfileptr != NULL) {
          fprintf(gd->outfileptr, "%.6lf#%.6lf#%.6lf#\n",
                  samptr_get(sample, ix, 0), samptr_get(sample, ix, 1), *stack);
        }
      ...
      case COMLL_HIST:
        ...
        /* Add data to histogram: This function creates the required
           ...
           given value        , x        , in that structure (assuming POP has
           stored it Note: comll->sval = Histogram label
        */
    ...
    /* the cell cannot be evaluated in the current computing mode
       forward/backward */
    {
      comll = comll->nextcol;
    }
...

{
  ...
  return (grfn_eval_event_ammc(2, event, sample, gd, pv_vect, (double *)NULL,
                               (double *)NULL, evaldf, und_info, cashflow));
}
...
#define PVSOURCESET(c1, v)                                                     \
  { pvSource_vect[c1] = (v); } /* Set a new PVSource */
#define PVSOURCESTATUSSET(c1, v)                                               \
  { pvSource_status[c1] = (v); } /* Set a new PVSource status*/

...
{
	if(!rt)	return;
	if(rt->date) srt_free(rt->date);
	if(rt->cvg) srt_free(rt->cvg);
	if(rt->yp) srt_free(rt->yp);
	if(rt->t) srt_free(rt->t);
	if(rt->df) srt_free(rt->df);
	srt_free(rt);
}
...
/*
	...
	SrtSwapDP
*/
static Err srt_f_sefb_to_SwapDP(Date start,Date end_or_nfp, 
								char *freq,char *basis,SrtSwapDP *sdp)
{
Err err;
	...
	else return serror("bad end_or_nfp srt_f_rtdesc");
...
static SrtErr intpfutstr(char *futstr, SrtMonth *m, int *y, int *nm)
{
  ...
  if(len != 5 && len != 6)return serror("bad fut str %s",futstr);
  strupper(futstr);
  ...
  *y = atoi(futstr+3);
  ...
  else if(futstr[5] == 'M')*nm = 1;
  else return serror("bad fut str %s",futstr);
  return NULL;
}


SrtErr srt_f_rtmkimmswp(Date start, char *startfutstr, Date end_or_nfp,
  char *endfutstr, char *freqstr, char *basisstr, SrtRtFnc *rt)
{
  double *cvg, *t;
  Ddate *swpdate;
  ...
  *rt = srt_f_rtalloc();
  ...
  if(!(*rt) || !t || !cvg || !swpdate)return serror("imm_swp alloc failed");
  ...
  return NULL;
	...
	*/
static Err srt_f_rtmkswp(Date start,Date end_or_nfp, 
		char *freq,char *basis,SrtRtFnc *ret)
{
	SrtRtFnc rt;
	SrtDateList dl;
	SwapDP sdp;
	...
	rt = srt_f_rtalloc();
	if(!rt)return serror("srt_f_rtalloc failed");
...
static Err srt_f_rtmkfra(Date start,Date end_or_nfp,char *basis,SrtRtFnc *ret)
{
	...
	rt = srt_f_rtalloc();
	if(!rt)return serror("failed to alloc");
...
SrtErr srt_f_rtmk(char *rtname, Date start, char *stastr,
        Date end_or_nfp, char *endstr, 
	char *freq, char *basis,SrtRtFnc *ret)
{
	SrtErr err;	
	SrtRtFncType type;
	...
	switch(type)
	{
		...
		case 	SRTRTFNCIMMLVL:
		{
			err = srt_f_rtmkimmswp(start,stastr,end_or_nfp,
				endstr,freq,basis,ret);
			break;
		}
		case 	SRTRTFNCSWP:
		case 	SRTRTFNCLVL:
...
void srt_f_rtprint(FILE *out, SrtRtFnc rt)
{
	...
	for(i=0;i<rt->len;i++)
	{
		if(rt->date)
		{
			fprintf(out,"%s %.0lf  ",dtstr(DTOL(rt->date[i])),
				rt->date[i]);
		}
		if(rt->cvg)
		{

...
/*****  Manager  ************************************************************/
/*
 *       Manage input & output        , reading from ascii files into data
 * structures.
 */
              ...
              MBSDENSITY *density) // density function for prepay propensity
{
  ...
  for (i = 0; i < 8; i++) {
    fgets(string, 80, stream);
    //   fscanf (stream        , "%lf \n"        , &(spread)); /* Swap spread
    //   for this maturity */ if (term_data->TSdiscount ==1)
  ...
  fgets(string, 80, stream);
  fgets(string, 80, stream);
  //  fscanf (stream        , "%ld \n"        , &(deal_data->Exer[1])); /* The
  //  maturity of the deal is stored as the second exercise date */
  ...
  } /* if there is a shift of y.cxx.        , we should shift scenario */
  if (deal_data->Ppform ==
      'R') { // if ratio        , we change from refi/gwac ratio
  ...
  fgets(string, 80, stream);
  fgets(string, 80, stream);
  // fscanf (stream        , "%lf\n"        , &(deal_data->Speedup)); /* Read
  // speed up factor of PPF */
  fgets(string, 80, stream);
  fgets(string, 80, stream);
  //  fscanf (stream        , "%lf\n"        , &(deal_data->rational)); /* Read
  //  rational speed up */
  fgets(string, 80, stream);
  if (fscanf(stream, "%c \n", &(deal_data->Hedge)) != 1)
  ...
  // deal_data->FixedPrepay        , wam        , deal_data->Gwac        ,
  ...
  i = max(0, deal_data->NbFixedPrepay - deal_data->Delay) +
      1; /* Calculate the lockout date        , that is the date on which we do
            the last    */
                  ...
                  MBSDENSITY *density) // density function for prepay propensity
{
  ...
  if ((deal_data->Freq != 'A') && (deal_data->Freq != 'S') &&
    ...
    return (nrerror(
        "Specify a frequency for the underlying cash flows: 'A'        , "
        "'S'        , 'Q' or 'M'!"));
...
/*****  Conv_Freq  ******
 *       Convert the character Freq ('A'nnual        , 'S'emi-Annual        ,
 * 'Q'uarterly or 'M'onthly) into an integer (1        , 2        , 4 or 12).
 */
 ...
 * ************************************************************/
/*
 *       Allocation of memory for an array of integers starting at nl        ,
 * ending at nh.
 */
 ...
 * ************************************************************/
/*
 *       Allocation of memory for an array of doubles starting at nl        ,
 * ending at nh.
 */
...
double **mbs_dmatrix(int nrl, int nrh, int ncl, int nch) {
  ...
  for (i=nrl; i<=nrh; i++)
  {
          m[i] = (double *) calloc ((unsigned) (nch-ncl+1)        , sizeof
(double)); if (!m[i]) merror_register("Can't calloc *(double *) in dmatrix");
          else
                  m[i] -= ncl;
  }  // for i
 ...
 */
/* which are 3 io        , po prices        , at 3 rtn        ,spd (rationality
 * and speed) values that form a plane */

...
{
    double *dFras = NULL, *dLongLevel = NULL, *dShortLevel = NULL, *dExeTimes = NULL,

           *dLongForward = NULL, *dShortForward = NULL, *dLongOptions = NULL, *dShortOptions = NULL,

           *dATMLongForward = NULL, *dATMShortForward = NULL, *dATMLongOptions = NULL,
    ...
    *dCall = 0.0;
    ...
    if (!dFras || !dExeTimes || !dShortLevel || !dLongLevel || !dATMShortForward ||
        !dATMLongForward || !dShortForward || !dLongForward || !dATMShortOptions ||
        !dATMLongOptions || !dShortOptions || !dLongOptions)
    {
    ...
    for (i = i0; i < iNbFixedCoupon; i++)
    {
        ...
        if (sMidatParams->dShortVolShift && sMidatParams->dShortVolShiftTime)
        {
            ...
            constant_or_linear_interpolation_dlm(
                sMidatParams->dShortVolShiftTime,
        ...
        if (sMidatParams->iCalibATM)
        {
            ...
            if (power > 0.5)
            {
                dATMShortOptions[i] = srt_f_optblksch(
                    dShortForwardi,
                    dShortForwardi,
                    dShortVolatility,
                    dExeTimes[i],
        ...
        else
        {
            ...
            if (power > 0.5)
            {
                dShortOptions[i] = srt_f_optblksch(
                    dShortForwardi,
                    dStrike,
                    dShortVolatility,
                    dExeTimes[i],
            ...
            {
                dShortOptions[i] = srt_f_optblknrm(
                    dShortForwardi,
                    dStrike,
                    dShortVolatility,
                    dExeTimes[i],
        ...
        if (sMidatParams->iCalibATM)
        {
            ...
            if (power > 0.5)
            {
                dATMLongOptions[i] = srt_f_optblksch(
                    dLongForwardi,
                    dLongForwardi,
                    dLongVolatility,
                    dExeTimes[i],
            ...
            {
                dATMLongOptions[i] = srt_f_optblknrm(
                    dLongForwardi,
                    dLongForwardi,
                    dLongVolatility,
                    dExeTimes[i],
        ...
        if (sCalibParams->iShortIsCap == 0 && sMidatParams->iCalibATM)
        {
            if (i < iNbFixedCoupon - 1)
            {
                ...
                {
                    err = serror(
                        "Correlation %.2f between %.2f y %.2f y and %.2f y %.2f is too low",
                        dCorrel,
                        dExeTimes[i],
        ...
        err = swp_f_vol(
            cVolCurve, lCouponStartDate[i], lTheoEndDate, dStrike, &dLongVolatility, &power);
        if (err)
            goto FREE_RETURN;
        ...
        if (power > 0.5)
        {
            dLongOptions[i] = srt_f_optblksch(
                dLongForwardi,
                dStrike,
                dLongVolatility,
                dExeTimes[i],
        ...
        {
            dLongOptions[i] = srt_f_optblknrm(
                dLongForwardi,
                dStrike,
                dLongVolatility,
                dExeTimes[i],
                        ...
                        GENMIDAT_AUTOCALINFO sInfos)
{
    ...
    double *dFras = NULL, *dExoticPV = NULL, *dFundingPV = NULL, *dFlatFundingPV = NULL;

    double *dExeTimes = NULL, *dFwdIV = NULL, *dATMFwdIV = NULL, *dLongLevel = NULL,
           *dLongForward = NULL, *dATMShortOptions = NULL, *dATMLongOptions = NULL,
           *dATMLongVol = NULL, *dLongOptions = NULL;
    ...
    *dCall = 0.0;
    ...
    if (!dFras || !dExoticPV || !dFundingPV || !dFlatFundingPV || !dExeTimes || !dFwdIV ||
        !dATMFwdIV || !dLongLevel || !dLongForward || !dATMShortOptions || !dATMLongOptions ||
        !dATMLongVol || !dLongOptions)
    {
    ...
    *dIV = dPayRec * dMarketIV;
    ...
    for (i = iExeStartIdx; i < iNbExercise; i++)
    {
        ...
        {
            dLongLeveli +=
                (lCouponPayDate[iCouponIdx - 1] - lSettlementDate[i]) /
                (1.0 * (lCouponPayDate[iCouponIdx - 1] - lCouponStartDate[iCouponIdx - 1])) *
        ...
        /* Add the Fee */
        dLongForwardi = dLongForwardi * dPayRec -
        ...
        err = swp_f_vol(
            cVolCurve, lSettlementDate[i], lTheoEndDate, dLongForward[i], &dLongATMVoli, &dPower);
        if (err)
            goto FREE_RETURN;

        err = swp_f_vol(
            cVolCurve, lSettlementDate[i], lTheoEndDate, dLongStrike, &dLongStrikeVol, &dPower);
        if (err)
            goto FREE_RETURN;
        ...
        if (i > iExeStartIdx)
        {
            ...
            dShortForwardi =
                (dLongLevel[i - 1] * dLongForward[i - 1] - dLongLevel[i] * dLongForward[i]) /
                dShortLeveli;
            ...
            if (dPower > 0.5)
            {
                dATMShortOptions[i - 1] = srt_f_optblksch(
                    dShortForwardi,
                    dShortForwardi,
                    dShortATMVoli,
                    dExeTimes[i - 1],
            ...
            if (!sCalibParams->iShortIsCap)
            {
                ...
                dPartialVolatility =
                    dLongWeight * dLongWeight * dATMLongVol[i - 1] * dATMLongVol[i - 1] +
                ...
                if (dPartialVolatility < 1.0E-08)
                {
                    err = serror(
                        ...
                        dCorrel,
                        dExeTimes[i - 1],
                        (lSettlementDate[i] - lSettlementDate[i - 1]) * DAYS_IN_YEAR,
                        dExeTimes[i - 1],

...
Err op_sabr_calib_adi(double forward, double strike, double maturity,
  ...
  /*	If we have an error        , calculate a second point using SABR closed
   * form vega */
...
/* ---------------------------------------------------------------------------------------------------------------------------------
 */
/* Given a set of SABR parameters        , calculates the price        , and
 * then calculates the alpha necessary in the ADI model to match the price */
/* ---------------------------------------------------------------------------------------------------------------------------------
 */
#define R 0.61803399
#define C (1.0 - R)
...
Err srt_AlphaFromADI(double forward, double maturity, double ATMVol,
  ...
  */
  ...
  //      ,SRT_LOGNORMAL        ,alpha        ,beta        ,rho        ,nt ,nx
  //      ,nz        ,128        ,&dSigmaBeta);
  ...
  */
  ...
  /*
          ...
          ax = bx;
          err=op_sabr_calib_adi( forward        ,forward        ,maturity
     ,ATMVol ,SRT_LOGNORMAL        ,ax        ,beta        ,rho        ,nt ,nx
     ,nz        ,128        ,&dSigmaBeta); f1 = 0.0; for (i=1;i<num_strikes+1 &&
     !err;i++)
          {
                  dPrice = op_sabr_adi( forward        , strikes[i]        ,
     maturity        , 1.0 , 0        , dSigmaBeta        , ax        , beta ,
     rho        , nt        , nx        , nz ); err = srt_f_optimpvol( dPrice ,
     forward        , strikes[i]        , maturity        , 1.0        ,
     SRT_CALL        , SRT_LOGNORMAL        , &dTempVol ); f1 += (
     dTempVol-vols[i] ) * ( dTempVol-vols[i] );
          }
          f2 = f1 - 1.0;
          while ( f2 < f1 && !err)
          {
                  ...
                  else
                  {
                          ...
                          for (i=1;i<num_strikes+1 && !err;i++)
                          {
                                  dPrice = op_sabr_adi( forward        ,
     strikes[i]        , maturity        , 1.0        , 0        , dSigmaBeta ,
     ax        , beta        , rho        , nt        , nx        , nz
     ); err = srt_f_optimpvol( dPrice        , forward        , strikes[i] ,
     maturity       , 1.0 , SRT_CALL        , SRT_LOGNORMAL        , &dTempVol
     ); f2 += ( dTempVol-vols[i] ) * ( dTempVol-vols[i] );

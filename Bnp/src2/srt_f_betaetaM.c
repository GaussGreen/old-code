...
Err make_M_matrix(TermStruct* l)
{
    ...
    while (s < SMAX)
    {
        ...
        while (theta < THETAMAX)
        {
            ...
            /*						err = calc_bigM(M, beta, eta, lambda, delta_tau,
               x, s_index, theta_index); if (err) return err;
            */
            theta++;
        }
        s++;
...
static double integrand_1(double y_bar)
{
    ...
    ans =
        ...
        (pow(y * y / STATIC_DELTAU, nu - 1) *
         (STATIC_LAMBDA * pow(STATIC_BETA * (1 - STATIC_ETA) * y_bar, 2. * nu - 1.) +
          y_bar / STATIC_DELTAU) *
         exp(-STATIC_LAMBDA * (xb_yb - STATIC_X_K)) * pow(y_bar * y / STATIC_DELTAU, 1 - nu) *
         (.5 * i_nu + .5 * i_minus_nu) *
         /* exp(-y*y_bar/STATIC_DELTAU)*  taken out due to e^{-x} from N.R. I_nu */
         exp(-((y_bar - y) * (y_bar - y)) /
             (2 * STATIC_DELTAU))) - /* minus sign comes from integration by parts */
                                     /* 1b */
        ((pow(y * y / STATIC_DELTAU, nu - 1)) * (sin(SRT_PI * nu) / SRT_PI) *
         (-STATIC_LAMBDA * pow(STATIC_BETA * (1. - STATIC_ETA) * y_bar, 2 * nu - 1) +
          y_bar / STATIC_DELTAU) *
         exp(-STATIC_LAMBDA * (xt_yb - STATIC_X_K)) * pow(y * y_bar / STATIC_DELTAU, 1 - nu) *
         k_nu * exp(-(y_bar + y) * (y_bar + y) / (2 * STATIC_DELTAU)));

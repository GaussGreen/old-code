...
#include "math.h"
/* 
	This file contains the code to compute the M matrix for the beta-eta model.
	The routine make_M_cube should be called from init_ir_und in grfn.
...
double		   log(double);
		...
		{ 
			...
			{			
				...
				if (eta == 0)			/* Analytic soln for eta=.0  */
					M[s][theta] = (.5*delta_tau*lambda*lambda);
				else
				if (eta ==.5)			/* Analytic soln for eta=.5  */
					M[s][theta] = (.5*delta_tau*lambda*lambda)/(1.+.5*delta_tau*lambda);
				else
				{
				/* Use simpson's rule to perform integration over density for rest of cases */
					...
					{   /*  Case 2 Natural b.c.'s */
						...
						M[s][theta]= 
							...
							(gammq(nu,(y*y)/(2.*delta_tau)) - pow((y*y)/(2.*delta_tau),nu-1.)*
							exp(-(y*y)/(2.*delta_tau)))
							);
					}
					else
...
/*						err = calc_bigM(M, beta, eta, lambda, delta_tau, x,
								            s_index, theta_index);
				if (err)
					return err;
*/
...
double print_M(double beta, double eta, double lambda, double deltau, double x_k)
{

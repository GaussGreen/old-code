...
Err payoff_cheybeta_pde(double evt_date, double evt_time, void *func_parm,
  ...
  for (i = lx; i <= ux; i++) {
    for (j = lphi; j <= uphi; j++) {
      for (k = 0; k < total->num_df; k++) {
        local->evt->df[0][k] = total->dff[k] * exp(-total->gam[k] * x[i] -
                                                   total->gam_sqr[k] * phi[j]);
      }

      local->smp.und[0].sv[0] = x[i] + ifr;
      local->smp.und[0].sv[1] = phi[j];

      err = FIRSTEvalEvent(global, local, nprod, 2, NULL, NULL, prod_val[j][i],
                           &temp);
      if (err) {
        return err;
      }
...
Err payoff_cheybeta_mc(double evt_date, double evt_time, void *func_parm,
  ...
  for (k = 0; k < total->num_df; k++) {
    local->evt->df[0][k] =
        total->dff[k] * exp(-total->gam[k] * x - total->gam_sqr[k] * phi);
  }
  ...
  err = FIRSTEvalEvent(global, local, ncols, 2, NULL, NULL, cols_val, &temp);

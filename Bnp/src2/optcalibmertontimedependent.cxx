...
Err priceforcalibtimedep(double str, double param[], double *value,
                         double *Deriv, int n_param) {
  ...
  if (str == strikes[1]) {
    ...
    err = optmertonsmiletimedependent(dFwd, n_strikes, strikes, nperiods, param,
                                      mat, Lognorm, vols /*implied vols*/
    );
    if (err) {
    ...
    for (j = 1; j < (n_param + 1); ++j) {
      ...
      err = optmertonsmiletimedependent(dFwd, n_strikes, strikes, nperiods,
                                        param_shiftes, mat, Lognorm,
...
Err optcalibmertontimedep(double dFwd_loc, int n_strikes_loc, int n_periods_loc,
  ...
  */
  ...
  err = levenberg_marquardt_select(strikes, dvols, sig, n_strikes_loc, param,
                                   ia, nparam, 10, priceforcalibtimedep, chisq);
  if (err) {
    smessage("Error in levenberg_marquardt_select");
  } else {
    err = optmertonsmiletimedependent(dFwd, n_strikes, strikes, nperiods, param,
                                      mat, Logornorm, calibvols);
    if (err)

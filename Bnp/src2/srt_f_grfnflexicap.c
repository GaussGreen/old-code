			...
			String			  szCapFloorCell)
{
	if(eCapFloor == SRT_PAYER) 
	{
		sprintf(szCapFloorCell,
			"a[2,i]*df(now,a[1,i])*MAX(0,c[0,i]-%lf)",
			dStrike);
	}
	else 
	...
	{
		sprintf(szCapFloorCell,
			"a[2,i]*df(now,a[1,i])*MAX(0,%lf-c[0,i])",
			dStrike);
	}
}

void GrfnFlexiDigitalCell(double	dEps,
						  String	szDigitalCell)
{
	sprintf(szDigitalCell,
		"(c[2,i]-c[3,i])/(a[2,i]*df(now,a[1,i])*%lf)",
		2*dEps);
}
...
static void GrfnNewFlexiCapNumExerciseCell(
			String      szNumExerciseCell)
{
...
static void GrfnFlexiCapFirstCashFlowCell(
			long             lMaxNumExercise,
			String           szCashFlowCell)
{
	sprintf(szCashFlowCell,
		"if ( ( ( c[2,i] > 0 ) && (va<= %d) ) , c[5,i] , 0 ) ", 
		lMaxNumExercise);
}

static void GrfnFlexiCapSecondCashFlowCell(
			long             lMaxNumExercise,
			String           szCashFlowCell)
{
	sprintf(szCashFlowCell,
...
static void GrfnFlexiCapDiscPaymentCell(
			String           szDiscPaymentCell)
{
...
static void GrfnFlexiCapNumExerciseCell(
			String           szNumExerciseCell)
{
...
static void GrfnFlexiCapCashFlowCell(
			long             lMaxNumExercise,
			String           szCashFlowCell)
{
	sprintf(szCashFlowCell,
		"if ( ( ( c[1,i] > 0 ) && (va<= %d) ) , c[2,i] , 0 ) ", 
		lMaxNumExercise);
}
...
/* Allocate space for the Tableau (including strings) */	
	*pppsTableau 	= GrfnCellmatrix(*plNumRows,*plNumCols,GRFN_DEF_ARGBUFSZ);
	...
	for (i=0; i < *plNumRows; i++)
	{
		...
		if (i == 0 )
		{
			GrfnFlexiCapFirstCell(lNumAlreadyExercised,szCashBasis, szUndName, szRefRateCode, (*pppsTableau)[0][0].sval);
		}
...
/* Construct a Grfn Tableau that describes the Flexi Cap  */
	...
	if(dEps > 0)
	{
		err = NewGrfnFlexiCapMakeTableau(lNumDates,
				dStrike,
				dEps,
				lMaxNumExercise,
				lNumAlreadyExercised,
...
/* The Full Cap PV is in the third column */
	if( dEps > 0) dGrfnFullCapPrice =  pdColumnPvs[1];
	else dGrfnFullCapPrice =  pdColumnPvs[2];
...
/* Use some sort of control variate if it can be done with the full cap */
	*pdFlexiCapPrice = dGrfnFlexiCapPrice;
	...
	{
		*pdFlexiCapPrice = dGrfnFlexiCapPrice +
			(double) lMaxNumExercise / (double) lNumDates * ( dFullCapRealPrice - dGrfnFullCapPrice);
	}
	else
	{
		*pdFlexiCapPrice = dGrfnFlexiCapPrice;
...
/* Fills in the Fields for the instruments */
	for (i=0;i <iNumInstruments;i++)
	{
		...
		err = swp_f_CapFloor(plStartDates[i], plNfp[i],pdStrike[i], pfGetVol,
					...
					szVolType, &pdOptionPrice[i]);
	    err =  swp_f_ForwardRate(plStartDates[i], plNfp[i],pszFrequency[i],pszBasis[i],

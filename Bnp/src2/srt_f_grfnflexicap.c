...
void GrfnFlexiCapFraCell(
    String szCashBasis, String szUndName, String szRefRateCode, String szFraCell)
{
    sprintf(
        szFraCell,
        "FRA(a[0,i],a[1,i],\"%s\",\"%s\",\"%s\")",
        szCashBasis,
        szUndName,
        szRefRateCode);
}

void GrfnFlexiCapFloorCell(SrtReceiverType eCapFloor, double dStrike, String szCapFloorCell)
{
    if (eCapFloor == SRT_PAYER)
    {
        sprintf(szCapFloorCell, "a[2,i]*df(now,a[1,i])*MAX(0,c[0,i]-%lf)", dStrike);
    }
    else if (eCapFloor == SRT_RECEIVER)
    {
        sprintf(szCapFloorCell, "a[2,i]*df(now,a[1,i])*MAX(0,%lf-c[0,i])", dStrike);
    }
}

void GrfnFlexiDigitalCell(double dEps, String szDigitalCell)
{
    sprintf(szDigitalCell, "(c[2,i]-c[3,i])/(a[2,i]*df(now,a[1,i])*%lf)", 2 * dEps);
}
...
static void GrfnFlexiCapFirstCashFlowCell(long lMaxNumExercise, String szCashFlowCell)
{
    sprintf(
        szCashFlowCell, "if ( ( ( c[2,i] > 0 ) && (va<= %d) ) , c[5,i] , 0 ) ", lMaxNumExercise);
}

static void GrfnFlexiCapSecondCashFlowCell(long lMaxNumExercise, String szCashFlowCell)
{
    sprintf(
        szCashFlowCell,
        "if ( ( ( va > %d ) && (va - c[4,i] < %d) ) , c[5,i]*(%d-(va-c[4,i]))/c[4,i], 0 ) ",
        lMaxNumExercise,
        lMaxNumExercise,
        lMaxNumExercise);
}
...
static void GrfnFlexiCapFirstCell(
...
/* -------------------------------------------------------------------------- */
/* Cell in the second column of the flexi cap tableau : the discounted floating intrinsic */
static void GrfnFlexiCapDiscFloatCell(
    SrtReceiverType eCapFloor, double dStrike, String szDiscFloatCell)
{
    if (eCapFloor == SRT_RECEIVER)
    {
        sprintf(szDiscFloatCell, "(%f - c[0,i]) * a[2,i] * df(now, a[1,i])", dStrike);
    }
    else if (eCapFloor == SRT_PAYER)
    {
        sprintf(szDiscFloatCell, "(c[0,i] - %f) * a[2,i] * df(now, a[1,i])", dStrike);
    }
}

/* -------------------------------------------------------------------------- */
/* Cell in the third column of the flexi cap tableau : the discounted floating payment */
...
{
    sprintf(szNumExerciseCell, "if ( c[1,i] > 0 , va:=va + 1, 0 )");
}

/* -------------------------------------------------------------------------- */
/* Cell in the fifth and last column of the flexi cap tableau : the cashflow */
static void GrfnFlexiCapCashFlowCell(long lMaxNumExercise, String szCashFlowCell)
{
    sprintf(
        szCashFlowCell, "if ( ( ( c[1,i] > 0 ) && (va<= %d) ) , c[2,i] , 0 ) ", lMaxNumExercise);
}
    ...
    *pppsTableau = GrfnCellmatrix(*plNumRows, *plNumCols, GRFN_DEF_ARGBUFSZ);
    ...
    for (i = 0; i < *plNumRows; i++)
    {
        ...
        if (i > 0)
        {
            ...
            if (i == 0)
        {
            GrfnFlexiCapFirstCell(
                lNumAlreadyExercised,
                szCashBasis,
                szUndName,
                szRefRateCode,
                (*pppsTableau)[0][0].sval);
        }
        ...
        GrfnFlexiCapFloorCell(eCapFloor, (dStrike - dEps), (*pppsTableau)[i][2].sval);
        (*pppsTableau)[i][2].type = GRFNSCELL;

        /* The fourth column: the caplet struck at K+eps*/
        GrfnFlexiCapFloorCell(eCapFloor, (dStrike + dEps), (*pppsTableau)[i][3].sval);
    ...
    double***   pppdAuxRanges)
{
    ...
    *pdFlexiCapPrice = dGrfnFlexiCapPrice;
    ...
    {
        *pdFlexiCapPrice = dGrfnFlexiCapPrice + (double)lMaxNumExercise / (double)lNumDates *

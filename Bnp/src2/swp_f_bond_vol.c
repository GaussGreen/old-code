...
Err swp_f_bond_vol_yldbeta_to_yldbs(
    double  fwd_clean_price,
    double  clean_strike,
    double  yield,
    double  yieldmean,
    ...
    double* implied_vol)
{
    ...
    if ((1. - rho) < DBL_EPSILON * 100.)  // threshold has been increased to 100*epsilon to avoid
    ...
    else
    {
        if ((1. - beta) < DBL_EPSILON * 100.)
            ...
            ratio_integrand = log(yield / yieldstar) /
    ...
    /* OK now what is the implied volatility ? */
    *implied_vol =
        volatility * ratio_integrand * ratio_z_xz *
        (1. + (pow((1. - beta), 2) / 24. * pow(volatility, 2) * pow(yieldmean, 2. * (beta - 1.)) +
               1. / 4. * rho * gamma1 * alpha * volatility * PsiY * pow(yieldmean, beta) +
               (2. - 3. * rho * rho) / 24. * alpha * alpha) *
                  optmat);

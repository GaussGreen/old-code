...
Err swp_f_bond_vol(Date fut, SwapDP p, double coupon, double clean_price,
  ...
  if (err = duration_range(s.dl.date, s.cpn.d, s.cpn.len, s.dl.type, s.dl.prev,
                           s.sdp.basis_code, s.sdp.compd, yieldmean, &PsiY))
    return err;
  if (err = convexity_range(s.dl.date, s.cpn.d, s.cpn.len, s.dl.type, s.dl.prev,
                            s.sdp.basis_code, s.sdp.compd, yieldmean, &PsiYY))
    return err;
  if (err =
          thirdmoment_range(s.dl.date, s.cpn.d, s.cpn.len, s.dl.type, s.dl.prev,
                            s.sdp.basis_code, s.sdp.compd, yieldmean, &PsiYYY))
    return err;
  free_inSimpleSwap(&s);

  /* OK now what is the implied volatility ? */
  Tex = (add_unit(fut, -p.spot_lag, SRT_BDAY, NO_BUSDAY_CONVENTION) - today) /
        365.0;

  err = swp_f_bond_vol_yldbeta_to_pricebs(
      fwd_clean_price, strike, yield, yieldmean, yieldstar, -PsiY, PsiYY,
      -PsiYYY, volatility, Tex, alpha, beta, rho, implied_vol);
...
Err swp_f_bond_vol_yldbeta_to_pricebs(double fwd_clean_price,
  ...
  z = alpha / volatility * (fwd_clean_price - clean_strike) / PsiY /
      pow(yieldmean, beta);
  ...
  gamma1 = PsiYY / (PsiY * PsiY) + beta / (PsiY * yieldmean);

  gamma2 = PsiYYY / pow(PsiY, 3) - pow(PsiYY, 2) / pow(PsiY, 4) +
  ...
  if ((1. - beta) < DBL_EPSILON * 100.)
    ...
    integrand =
        (pow(yield, 1. - beta) - pow(yieldstar, 1. - beta)) / (1. - beta);
  ...
  CFactor = (2. * gamma2 - gamma1 * gamma1 + 1. / pmean / pmean) / 24. +
            1. / 4. * rho * gamma1 * alpha / volatility / PsiY /
                pow(yieldmean, beta) +
            (2. - 3. * rho * rho) / 24. * alpha * alpha /
                pow(volatility * PsiY * pow(yieldmean, beta), 2);

  (*implied_vol) = volatility * ratio_integrand * ratio_z_xz;
  (*implied_vol) *=
      (1. +
       CFactor * pow(volatility * PsiY * pow(yieldmean, beta), 2) * optmat);
...
Err swp_f_bond_vol_yldbeta_to_yldbs(double fwd_clean_price, double clean_strike,
  ...
  if ((1. - rho) <
      DBL_EPSILON *
          100.) // threshold has been increased to 100*epsilon to avoid
                // numerical problems on prices that are very close to ATM
    xz = 1.;
  else
    xz = log((sqrt(1. - 2. * rho * z + z * z) + z - rho) / (1. - rho));
  ...
  /* OK now what is the implied volatility ? */
  *implied_vol = volatility * ratio_integrand * ratio_z_xz *
                 (1. + (pow((1. - beta), 2) / 24. * pow(volatility, 2) *
                            pow(yieldmean, 2. * (beta - 1.)) +
                        1. / 4. * rho * gamma1 * alpha * volatility * PsiY *
                            pow(yieldmean, beta) +
                        (2. - 3. * rho * rho) / 24. * alpha * alpha) *
                           optmat);

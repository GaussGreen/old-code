...
if ( vol != 0  &&  mat != 0 )
{
	...
	caux = disc*rebate*(nprimexx1*(barrier/fwd_price
		+pow(spot/fwd_price,-2*chi+2))/(vol*sqrt(mat)*fwd_price) 
		+ ndxx2*pow(spot/barrier,-2*chi+2)*((-2*chi+2)/spot
		+2*vol/vol/mat*(1/spot-1/fwd_price)*log(spot/barrier)));
	       
	paux = disc*rebate*(-nprimexx1*(barrier/fwd_price
		+pow(spot/fwd_price,-2*chi+2))/(vol*sqrt(mat)*fwd_price) 
		- ndxx2*pow(spot/barrier,-2*chi+2)*((-2*chi+2)/spot
		+2*vol/vol/mat*(1/spot-1/fwd_price)*log(spot/barrier)));
};
...
{                                  /* DELTA FWD */
	...
	answer = ( srt_f_optrebate(	fwd_price+shift,
					spot,	
					barrier,
		       			rebate,
					vol,
...
{                                  /* GAMMA FWD */
	...
	answer = ( srt_f_optrebate(	fwd_price+shift,
					...
					call_put,
					PREMIUM)
		+ srt_f_optrebate(	fwd_price-shift,
					spot,
					barrier,
					rebate,
					vol,
...
{                                  /* VEGA */
	shift = 0.01;
	answer = ( srt_f_optrebate(	fwd_price,
					spot,
					barrier,
					rebate,
					vol + shift,

...
static double InterpPVratio(Date thedate,
...
LGMErr LGMMakeRefDeals(
	...
	double					*RatArr, /* [*,1,2,...,nEx] RATIO OF PV OF FIXED LEG TO PV OF STRIKE */
	...
	LGMErr					(*GetVol)(Date, Date, double, SRT_Boolean, double*),	/* SWAPTION CAP/VOL */
	LGMErr					(*GetBeta)(Date, Date, double*),			/* EXPONENT BETA TO INTERPRET VOLS */
...
after tNow and the last date before tLast */
	...
	error = checkinputarrays(nArr, tNow, tlast, TauArr, RatArr, CalReq,
								&firsti, &lasti);
	if (error!=NULL)
		return(error);
	...
	nEx = lasti - firsti + 1;
	tExArr = &(TauArr[firsti-1]);	/* tExArr[*,1,...,nEx] are relevent dates */
	FVArr = &(RatArr[firsti-1]);	/* FVArr[*,1,...,nEx] are relevent fwd values */

/* Create reference instruments */
/* Set tNow, n, nPay, the long swaption pay dates tPay, the coverages cvgpay, and the dis factors
	Dpay from tNow to the pay dates. Set nEx, and common exercise dates (tEx), start dates (tStart),
	and df to start date (DStart) */

	error = RefDealsCommon(tNow,nEx,tlast, tExArr, CalReq, conv, YcName, RefDealsPtr);
	if (error!=NULL)
...
/* determine which deals are needed */
	...
	if(CalReq->LGMOneTwoFactor==1)
	{
		...
		if (CalReq->calmeth == FixExp)			/* The fixed expiry method also uses the "1 into k" */
			make1intok = 1;						/* swaptions to find G(t) */
		
		if (CalReq->calmeth == TenorAndDiag)	/* TenorAndDiag uses long "k into n-k" swaptions for zeta(t) */
		{			
			makelong = 1;						/* It uses long "k into n-k" swaptions and either */
			if (CalReq->usecaps==1) makecap = 1; /* "k into 3mo caplets, or */
			else makeshort = 1;					/* short "k into 1" swaptions */
		}
	}
...
/* create long swaptions */
	if (makelong!=0)
	{	error = RefDealsLong(nEx, tExArr, FVArr,
							conv, YcName,
							CalReq, DealType, dealptr, GetVol, GetBeta, RefDealsPtr);
		if (error!=NULL)
		{	LGMFreeCalSet(&RefDealsPtr);
			return (error);
		}
...
/* create the short swaptions */
	if (makeshort!=0)
	{	error = RefDealsShort(nEx, tExArr, FVArr,
							conv, YcName,
							CalReq, DealType, dealptr, GetVol, GetBeta, RefDealsPtr);
			if (error!=NULL)
			{	LGMFreeCalSet(&RefDealsPtr);
...
/* create caplets */
	if (makecap!=0)
	{		
		error = RefDealsCap(DealType, dealptr, nEx, tExArr,
							FVArr, CalReq, GetVol, GetBeta, YcName, conv, RefDealsPtr);
...
/* create the "1 into k swaptions" */
	if (make1intok!=0)
	{	error = RefDeals1intok(nEx, tExArr, FVArr,
							conv, YcName,
							CalReq, DealType, dealptr, GetVol, GetBeta, RefDealsPtr);
			if (error!=NULL)
			{	LGMFreeCalSet(&RefDealsPtr);
...
tLast = tPay[n] to the spot of the first exercise date, tPay[0]. For simplicity, 
also extend the pay dates to either tPay[n+1] (for annual compounding) or tPay[n+2]
(for semiannual compounding) to ensure we can fit in a minimum length swap whose
start date is before tPay[n]. Construct
	the fixed leg pay dates tPay[i], i = 1,...,n.
	the discount factor Dpay[i] from tNow to the pay date
	the coverage cvgpay[i] between tPay[i-1] and tPay[i].
If ResExer = 1, then set the common exercise dates to be tEx[i], and set the 
start dates tStart[i] to be spot of tEx[i]. If use starts=0, then let the start dates be
tPay[i], i=1 to n-1, and set tEx[i] to be lag business days earlier. In either case
set DStart[i] to be the discount factor from tNow to tStart. 
*/
static LGMErr RefDealsCommon(
			Date tNow, long nEx,					/* evaluation date, number of exercises */
			Date tLast, Date *TauArr,				/* last pay date, exer dates */
...
/*******************************************/
/* Routine to construct the short swaptions */
/* Short swaption j has the same exercise dates and start dates as the long swaptions. 
The fixed leg date pay dates are tPay[i], i=ifirst[j] to i=nshort[j].
The discount factors to the pay dates tPay[i] are Dpay[i] like the long swaption,
the coverage between start and the first pay date is cvgfirst[j] like the long swaption,
and the coverages for the period between tPay[i-1] and tPay[i] is cvgpay[i] for i>ifirst[j].
	The only new variables we need are nshort[j]. */
/* Note: RefDealslong must be called first to set n, nPay, tPay[], ifirst[] */
...
/* Find end period nshort[j] */
	...
	{	i = ifirst[j];
		if (tPay[i] - tStart[j] > fullper)
			nshort[j] = i + conv->minswap - 1;	/* start to ifirst counts as full period */
		else									
			nshort[j] = i + conv->minswap;		/* start to ifirst too short to be full period */
	}
...
for n=1,...,nArr. By definition, the ratio is 1 at date tLast (last pay date of
the original deal)
Use GetVol and GetBeta to ensure that the strike is within two std deviations of par */
static LGMErr FindFixRateRefInst(
...
LGMErr LGMPriceRefDeals(
		LGMErr (*GetVol)(Date, Date, double,SRT_Boolean, double*),	/* function to provide swaption/cap vols */
		LGMErr (*GetBeta)(Date, Date, double*),			/* function to provide exponent (beta) to interpret vols */
		LGMErr (*RecVol)(Date, Date, double),			/* function to provide swaption/cap vols */
		LGMCalSet	*CSPtr,								/* Output */
...
/* find prices of long swaptions if needed */
	...
	{	
		if(lgmRefSwptnData !=NULL)
		{
			...
			lgmRefSwptnData->refLongSwptnArr = (SrtLgmRefSwptn *) srt_calloc(lgmRefSwptnData->NrefLongSwptn, sizeof(SrtLgmRefSwptn));
		...
		{	i = CSPtr->ifirst[j];					/* FIRST PAYMENT */
			...
			error = GetVol(tStart[j], tPay[iend], CSPtr->Rflong[j],SRT_FALSE, &CEVvol);	
			...
			for (CpnIndex=(i+1); CpnIndex<=iend; CpnIndex++) SwapLevel = SwapLevel + cvgpay[CpnIndex]*Dpay[CpnIndex];
			...
			CSPtr->Vlong[j] = LGMCEVSwaptionPrice(tNow, tEx[j],						
												  recpay, CSPtr->Rflong[j],					
												  DStart[j], CSPtr->cvgfirst[j], Dpay[i],			
												  (iend-i), &(cvgpay[i+1]), &(Dpay[i+1]),	
												  CEVvol, beta);

			CSPtr->VegaLong[j] = LGMCEVSwaptionPrice(tNow, tEx[j],						
												  recpay, CSPtr->Rflong[j],					
												  DStart[j], CSPtr->cvgfirst[j], Dpay[i],			
												  (iend-i), &(cvgpay[i+1]), &(Dpay[i+1]),	
			...
			CSPtr->StDLong[j] = (CSPtr->Rflong[j]-SwapRate)/(CEVvol*SwapRate*sqrt(Yrtoexp)) ;
...
/* find prices of short swaptions if needed */
	...
	{	
		if(lgmRefSwptnData !=NULL)
		{
			...
			lgmRefSwptnData->refShortSwptnArr = (SrtLgmRefSwptn *) srt_calloc(lgmRefSwptnData->NrefShortSwptn, sizeof(SrtLgmRefSwptn));
		...
		{	
			...
			error = GetVol(tStart[j], tPay[iend], CSPtr->Rfshort[j],SRT_FALSE, &CEVvol);	/* Get CEVvol */
...
/* find prices of caplets if needed */
	...
	{	
		if (lgmRefSwptnData != NULL)
		{
			...
			lgmRefSwptnData->refCapSwptnArr = (SrtLgmRefSwptn *) srt_calloc(lgmRefSwptnData->NrefCapSwptn, sizeof(SrtLgmRefSwptn));
		...
		{	
			...
			CSPtr->Vcap[j] = LGMCEVCapletGreek(recpay,SwapRate,CSPtr->Rfcap[j],SwapLevel,Yrtoexp,CEVvol,beta,PREMIUM);
			if (CSPtr->Vcap[j] < 0.) return("can't get cap price");
			CSPtr->VegaCap[j] = LGMCEVCapletGreek(recpay,SwapRate,CSPtr->Rfcap[j],SwapLevel,Yrtoexp,CEVvol,beta,VEGA);
			...
			*/
			...
			CSPtr->Vcap[j] =
					LGMCEVCapletPrice(
						tNow, tEx[j],						
						recpay,								
						CSPtr->Rfcap[j], CSPtr->cvgcap[j],	
						DStart[j], CSPtr->Dcap[j],			
						CEVvol, beta);
...
/* find prices of 1 into k swaptions if needed */ 
	...
	{	
		if (lgmRefSwptnData != NULL)
		{
			...
			lgmRefSwptnData->refFixSwptnArr = (SrtLgmRefSwptn *) srt_calloc(lgmRefSwptnData->NrefFixSwptn, sizeof(SrtLgmRefSwptn));
		...
		{	error = GetVol(CSPtr->ftStart, tPay[j], CSPtr->Rfix[j],SRT_FALSE, &CEVvol);	/* Get CEVvol */
			...
			for (CpnIndex=(i+1); CpnIndex<=j; CpnIndex++) SwapLevel = SwapLevel + cvgpay[CpnIndex]*Dpay[CpnIndex];
			...
			if (lgmRefSwptnData != NULL)
			{
				lgmRefSwptnData->refFixSwptnArr[j-CSPtr->nfirst].bgnDt = CSPtr->ftStart;
				lgmRefSwptnData->refFixSwptnArr[j-CSPtr->nfirst].endDt = tPay[j];
				lgmRefSwptnData->refFixSwptnArr[j-CSPtr->nfirst].strike = CSPtr->Rfix[j];
				lgmRefSwptnData->refFixSwptnArr[j-CSPtr->nfirst].bsPv = CSPtr->Vfix[j];

...
static double InterpPVratio(Date thedate, long n, double* FV, Date* Tau, Date tLast);
    ...
    LGMCalSet** RefDealsPtrPtr)             /* REFERENCE INSTRUMENTS (OUTPUT)*/
{
    ...
    *RefDealsPtrPtr = NULL;
    ...
    error = checkinputarrays(nArr, tNow, tlast, TauArr, RatArr, CalReq, &firsti, &lasti);
    if (error != NULL)
        return (error);
    ...
    /* Set tNow, n, nPay, the long swaption pay dates tPay, the coverages cvgpay, and the dis
       factors Dpay from tNow to the pay dates. Set nEx, and common exercise dates (tEx), start
    ...
    if (CalReq->LGMOneTwoFactor == 1)
    {
        ...
        if (CalReq->calmeth ==
            TenorAndDiag) /* TenorAndDiag uses long "k into n-k" swaptions for zeta(t) */
        {
        ...
        lgmRefSwptnData) /* ptr to reference swaption data structure (NULL => not req'd) */
{
    ...
    if (CSPtr->longflag != 0)
    {
        if (lgmRefSwptnData != NULL)
        {
            ...
            lgmRefSwptnData->refLongSwptnArr =
                (SrtLgmRefSwptn*)srt_calloc(lgmRefSwptnData->NrefLongSwptn, sizeof(SrtLgmRefSwptn));
    ...
    if (CSPtr->shortflag != 0)
    {
        ...
        for (j = 1; j <= nEx; j++)
        {
            ...
            error = GetVol(
                tStart[j], tPay[iend], CSPtr->Rfshort[j], SRT_FALSE, &CEVvol); /* Get CEVvol */
    ...
    if (CSPtr->capflag != 0)
    {
        if (lgmRefSwptnData != NULL)
        {
            ...
            lgmRefSwptnData->refCapSwptnArr =
                (SrtLgmRefSwptn*)srt_calloc(lgmRefSwptnData->NrefCapSwptn, sizeof(SrtLgmRefSwptn));
        ...
        for (j = 1; j <= nEx; j++)
        {
            ...
            CSPtr->Vcap[j] =
            LGMCEVCapletGreek(recpay,SwapRate,CSPtr->Rfcap[j],SwapLevel,Yrtoexp,CEVvol,beta,PREMIUM);
            if (CSPtr->Vcap[j] < 0.) return("can't get cap price");
            CSPtr->VegaCap[j] =
            LGMCEVCapletGreek(recpay,SwapRate,CSPtr->Rfcap[j],SwapLevel,Yrtoexp,CEVvol,beta,VEGA);

            */

            CSPtr->Vcap[j] = LGMCEVCapletPrice(
                tNow,
                tEx[j],
                recpay,
                CSPtr->Rfcap[j],
    ...
    if (CSPtr->fixflag != 0)
    {
        if (lgmRefSwptnData != NULL)
        {
            ...
            lgmRefSwptnData->refFixSwptnArr =
                (SrtLgmRefSwptn*)srt_calloc(lgmRefSwptnData->NrefFixSwptn, sizeof(SrtLgmRefSwptn));
        ...
        {
            error = GetVol(
                CSPtr->ftStart, tPay[j], CSPtr->Rfix[j], SRT_FALSE, &CEVvol); /* Get CEVvol */
            ...
            CSPtr->Vfix[j] = LGMCEVSwaptionPrice(
                tNow,
                CSPtr->ftEx,
                recpay,
                CSPtr->Rfix[j],

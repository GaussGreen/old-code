...
double srt_f_optameopt(double spot, double forward, double strike,
  ...
  if (call_put == SRT_STRADDLE) {
    answer = srt_f_optameopt(spot, forward, strike, disc_fact, vol, opt_mat,
                             SRT_CALL, step_num, greek) +
             srt_f_optameopt(spot, forward, strike, disc_fact, vol, opt_mat,
                             SRT_PUT, step_num, greek);
    return (answer);
  }
  ...
  if (opt_mat < 0) {
    ...
    if (bPayatTheExFlag) {
      BSprice =
          srt_f_optblksch(forward, strike, vol, opt_mat, 1, call_put, PREMIUM);
      BSdelta =
          srt_f_optblksch(forward, strike, vol, opt_mat, 1, call_put, DELTA);
      BSgamma =
          srt_f_optblksch(forward, strike, vol, opt_mat, 1, call_put, GAMMA);
      BStheta =
          srt_f_optblksch(forward, strike, vol, opt_mat, 1, call_put, THETA);
    } else {
      BSprice = srt_f_optblksch(forward, strike, vol, opt_mat, disc_fact,
                                call_put, PREMIUM);
      BSdelta = srt_f_optblksch(forward, strike, vol, opt_mat, disc_fact,
                                call_put, DELTA);
      BSgamma = srt_f_optblksch(forward, strike, vol, opt_mat, disc_fact,
                                call_put, GAMMA);
      BStheta = srt_f_optblksch(forward, strike, vol, opt_mat, disc_fact,
                                call_put, THETA);
    }
    ...
    for (n = step_num + 1; n >= 1; n--) {
      ...
      for (i = 0; i <= n; i++) {
        amer_disc_premium =
            (p_u * amer_premium[i] + p_d * amer_premium[i + 1]) * df_n;
        euro_disc_premium =
            (p_u * euro_premium[i] + p_d * euro_premium[i + 1]) * df_n;
        if (bPayatTheExFlag)
          factor_disc_premium =
              (p_u * disc_factor[i] + p_d * disc_factor[i + 1]) * df_n;
    ...
    case THETA_1D:
      ...
      theta_1d = srt_f_optameopt(spot, forward, strike, disc_fact, vol,
                                 opt_mat - YEARS_IN_DAY, call_put, old_step_num,
                                 PREMIUM);
      theta_1d -= amer_price;
      answer = theta_1d;
      break;
    ...
    case VEGA:
      ...
      amer_vega =
          srt_f_optameopt(spot, forward, strike, disc_fact, vol + GVOPT.vol_add,
                          opt_mat, call_put, old_step_num, PREMIUM);

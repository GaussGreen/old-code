 ...
 *		  theoretical delta        , gamma        , vega and theta for
 *an underlying which is lognormal
 *
...
double srt_f_optblknrm(double fwd_price, double strike, double vol, double mat,
  ...
  if (call_put == SRT_STRADDLE) {
    answer =
        srt_f_optblknrm(fwd_price, strike, vol, mat, disc, SRT_CALL, greek) +
        srt_f_optblknrm(fwd_price, strike, vol, mat, disc, SRT_PUT, greek);
    return answer;
  }
  ...
  /* Evaluate option-type dependent greeks (spot-deltas        , theta) and
   * premium */
  ...
  if ((mat == 0) && (greek != PREMIUM))
    ...
    case THETA: /*** THETA ***/
      answer = -(1 / 365.00) *
               (0.5 * disc * vol / sqrt(mat) * gd1 + disc * log(disc) / mat);
      break;
...
Err OptBlkNrm(double fwd_price, double strike, double vol, double mat,
  ...
  *result = srt_f_optblknrm(fwd_price, strike, vol, mat, disc, call_put_type,
                            greek_type);
...
double srt_f_optblknrm_accurate(double fwd_price, double strike, double vol,
  ...
  if (call_put == SRT_STRADDLE) {
    answer = srt_f_optblknrm_accurate(fwd_price, strike, vol, mat, disc,
                                      SRT_CALL, greek) +
             srt_f_optblknrm(fwd_price, strike, vol, mat, disc, SRT_PUT, greek);
    return answer;
  }

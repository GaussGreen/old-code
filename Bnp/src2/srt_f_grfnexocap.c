...
static void GrfnChooserCapletCell(
...
static void GrfnChooserMidatCapCell(SRT_Boolean StartMidatColumn, String szMidatCell)
{
...
static void GrfnBarrierCapletCell(SrtReceiverType eCapFloor, double dStrike, String szCapletCell)
{
    if (eCapFloor == SRT_RECEIVER)
    {
        sprintf(szCapletCell, "max(%f - c[0,i], 0) * a[2,i] * df(now, a[1,i])", dStrike);
    }
    else if (eCapFloor == SRT_PAYER)
    {
        sprintf(szCapletCell, "max(c[0,i] - %f, 0) * a[2,i] * df(now, a[1,i])", dStrike);
    }
}

static void GrfnDigitalConditionCell(
    SRT_Boolean bIsFirstCell,
    double      dBarrier,
...
static Err GrfnChooserMakeTableau(
    long            lNumEventDates,
    double          dStrike,
    long            lNumExercises,
    SrtReceiverType eCapFloor,
    ...
    GrfnCell***     pppsTableau)
{
    ...
    *pppsTableau = GrfnCellmatrix(*plNumRows, *plNumCols, GRFN_DEF_ARGBUFSZ);
    ...
    for (i = 0; i < *plNumRows; i++)
    {
        ...
        GrfnChooserCapletCell(
            eCapFloor, dStrike, szCashBasis, szUndName, szRefRateCode, (*pppsTableau)[i][0].sval);
...
static Err GrfnBarrierMakeTableau(
    long            lNumEventDates,
    double          dStrike,
    double          dBarrier,
    SrtReceiverType eCapFloor,
    ...
    GrfnCell***     pppsTableau)
{
    ...
    *pppsTableau = GrfnCellmatrix(*plNumRows, *plNumCols, GRFN_DEF_ARGBUFSZ);
    ...
    for (i = 0; i < *plNumRows; i++)
    {
        ...
        GrfnFlexiCapFraCell(szCashBasis, szUndName, szRefRateCode, (*pppsTableau)[i][0].sval);
        (*pppsTableau)[i][0].type = GRFNSCELL;

        /* The second column : c[1] - Condition */
        GrfnDigitalConditionCell(
            (i == 0 ? SRT_TRUE : SRT_NO),
            dBarrier,
            bIsInOrOut,
            bIsUpOrDown,
    ...
    double***   pppdAuxRanges)
{
    ...
    if (err = srt_f_set_GrfnParams(
            iNumGrfnParams, pszGrfnParamNames, pszGrfnParamValues, &sGrfnParams))
        return err;
...
void SrtGRFNQuickExoticCapGrfnOptions(
    String** grfn_param, String** grfn_value, long* lNumParams, SRT_Boolean UseTree)
{
    ...
    SRT_Boolean* pbReturnUnd)
{
    ...
    double *Coverage, *pdCapletRealPrices, **ppdLastPath, dFullCapRealPrice = 0, **ppdAuxRanges;
    ...
    if (err = swp_f_setSwapDP(StartDate, EndDate, SrtFloatFrequency, SrtFloatBasis, FloatLegDP))
        return err;
    ...
    if (bIsChooser)
        SrtGRFNQuickExoticCapGrfnOptions(
            &pszGrfnParamNames, &pszGrfnParamValues, &dNumberOfGrfnParams, SRT_YES);
    else
        SrtGRFNQuickExoticCapGrfnOptions(
            &pszGrfnParamNames, &pszGrfnParamValues, &dNumberOfGrfnParams, SRT_NO);

    /* to compute FullCapRealPrice... */

    if (err = swp_f_CapFloor(
            StartDate,
            EndDate,
            dStrike,
            pfGetVol,

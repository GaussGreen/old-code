...
/* Midat cell : if Startcolumn the max(PV[j]  ,c[0  ,i]) otherwise max(PV[j]
 * ,c[0  ,i]+PV[j-1]) */
...
GrfnDigitalConditionCell(SRT_Boolean bIsFirstCell, double dBarrier,
  ...
  if (bIsUpOrDown) /* Up case */
    sprintf(szFinalCashflowCell, "if(c[0  ,i]>%f  ,%d  ,%d)", dBarrier,
            (bIsInOrOut == SRT_YES ? 1 : 0), (bIsInOrOut == SRT_YES ? 0 : 1));
  else
    sprintf(szFinalCashflowCell, "if(c[0  ,i]<%f  ,%d  ,%d)", dBarrier,
            (bIsInOrOut == SRT_YES ? 1 : 0), (bIsInOrOut == SRT_YES ? 0 : 1));

  if (bIsFirstCell == SRT_NO)
    sprintf(szFinalCashflowCell, "if(c[1  ,i-1]=%d  ,%d  ,%s)",

...
void GenericMidat_OneFactor_ChangeBetaGrid(
    long lStart, long lEnd, long lStartProd, long lEndProd,

    double *dOldGridX, double dOldBeta, double *dNewGridX, double dNewBeta,

    GENMIDAT_MODEL sModel, GENMIDAT_PDEPAMS sParams,

    double **dOldValues, double **dNewValues) {
  Err err = NULL;
  double dOldX;
  int oldI;
  ...
  if (sParams->iJumpQuadra == 0) {
    ...
    for (i = lStart; i <= lEnd; i++) {
      ...
      dProba =
          (dOldX - dOldGridX[oldI]) / (dOldGridX[oldI + 1] - dOldGridX[oldI]);

      for (j = lStartProd; j <= lEndProd; j++) {
        dNewValues[i][j] = (1.0 - dProba) * dOldValues[oldI][j] +
                           dProba * dOldValues[oldI + 1][j];
      }
    }
  } else {
    ...
    for (i = lStart; i <= lEnd; i++) {
      dOldX = dRatio * dNewGridX[i];
      oldI = max(min(lStart + (int)((dOldX - dFirstX) / dx + 0.5), lEnd - 1),
                 lStart + 1);

      dProbaDown = (dOldX * (dOldX - dOldGridX[oldI] - dOldGridX[oldI + 1]) +
                    dOldGridX[oldI] * dOldGridX[oldI + 1]) /
                   (2.0 * dx2);
      dProbaMid = (dOldX * (dOldX - dOldGridX[oldI - 1] - dOldGridX[oldI + 1]) +
                   dOldGridX[oldI - 1] * dOldGridX[oldI + 1]) /
                  (-dx2);
      dProbaUp = (dOldX * (dOldX - dOldGridX[oldI - 1] - dOldGridX[oldI]) +
                  dOldGridX[oldI - 1] * dOldGridX[oldI]) /
                 (2.0 * dx2);
...
void GenericMidat_TwoFactor_ChangeBetaGrid(/* Old Grids */
                                           long lOldStartX, long lOldEndX,
                                           double *dOldGridX,

                                           long lOldStartZ, long lOldEndZ,
                                           double *dOldGridZ,

                                           /* New Grids */
                                           long lNewStartX, long lNewEndX,
                                           double *dNewGridX,

                                           long lNewStartZ, long lNewEndZ,
                                           double *dNewGridZ,

                                           /* Model parameters */
                                           int iIndex, GENMIDAT_MODEL sModel,
                                           GENMIDAT_PDEPAMS sParams,

                                           /* Product Values */
                                           long lStartProd, long lEndProd,
                                           double ***dTempValues,
                                           double ***dOldValues,
                                           double ***dNewValues) {
  Err err = NULL;
  ...
  double dRatio1, dRatio2, dCoefX, dCoefZ, dProba, dProbaDown, dProbaMid,
      dProbaUp;
  int i, j, k;
  ...
  dCoefX = 1.0 / sqrt(1.0 - sModel->dCorrel[iIndex] * sModel->dCorrel[iIndex]) *
           ...
           (sModel->dCorrel[iIndex - 1] * sModel->dSigma2[iIndex - 1] /
                sModel->dSigma[iIndex - 1] -
            sModel->dCorrel[iIndex] * sModel->dSigma2[iIndex] /
                sModel->dSigma[iIndex]);

  dCoefZ =
      dRatio2 *
      sqrt(1.0 - sModel->dCorrel[iIndex - 1] * sModel->dCorrel[iIndex - 1]) /
  ...
  if (sParams->iJumpQuadra == 0) {
    ...
    for (i = lNewStartX; i <= lNewEndX; i++) {
      ...
      oldI = max(min(lOldStartX + (int)((dOldX - dFirstX) / dx), lOldEndX - 1),
                 lOldStartX);
    ...
    for (i = lNewStartX; i <= lNewEndX; i++) {
      for (j = lNewStartZ; j <= lNewEndZ; j++) {
        ...
        oldZ =
            max(min(lOldStartZ + (int)((dOldZ - dFirstZ) / dz), lOldEndZ - 1),
                lOldStartZ);

        dProba =
            (dOldZ - dOldGridZ[oldZ]) / (dOldGridZ[oldZ + 1] - dOldGridZ[oldZ]);
    ...
    for (i = lNewStartX; i <= lNewEndX; i++) {
      dOldX = dRatio1 * dNewGridX[i];
      oldI = max(
          min(lOldStartX + (int)((dOldX - dFirstX) / dx + 0.5), lOldEndX - 1),
          lOldStartX + 1);
    ...
    for (i = lNewStartX; i <= lNewEndX; i++) {
      for (j = lNewStartZ; j <= lNewEndZ; j++) {
        dOldZ = dCoefX * dNewGridX[i] + dCoefZ * dNewGridZ[j];
        oldZ = max(
            min(lOldStartZ + (int)((dOldZ - dFirstZ) / dz + 0.5), lOldEndZ - 1),
            lOldStartZ + 1);

        dProbaDown = (dOldZ * (dOldZ - dOldGridZ[oldZ] - dOldGridZ[oldZ + 1]) +
                      dOldGridZ[oldZ] * dOldGridZ[oldZ + 1]) /
                     (2.0 * dz2);
        dProbaMid =
            (dOldZ * (dOldZ - dOldGridZ[oldZ - 1] - dOldGridZ[oldZ + 1]) +
             dOldGridZ[oldZ - 1] * dOldGridZ[oldZ + 1]) /
            (-dz2);
        dProbaUp = (dOldZ * (dOldZ - dOldGridZ[oldZ - 1] - dOldGridZ[oldZ]) +
                    dOldGridZ[oldZ - 1] * dOldGridZ[oldZ]) /
                   (2.0 * dz2);
...
Err GenericMidat_ConvertModelLinear(int iNbVols, double *dTimes,
                                    double *dSigmaTarget, double dSigma0,
                                    double *dCoefs, double *dSigma) {
  int i;
  ...
  for (i = 0; i < iNbVols; i++) {
    ...
    if (delta < 0.0) {
      ...
      dCoefs[i] =
          max(dCoefs[i], (dMinVol - dLastSigma) / (dTimes[i] - dLastTime));
    }
...
static Err GenericMidat_VarModelLinear(int iNbVols, double *dTimes,
                                       double *dSigmaTarget, double dSigma0,
                                       double *dCoefs, double *dSigma,
                                       double *dResult) {
  Err err = NULL;
  ...
  err = GenericMidat_ConvertModelLinear(iNbVols, dTimes, dSigmaTarget, dSigma0,
                                        dCoefs, dSigma);
...
Err GenericMidat_OptimalConvertModelLinear(int iNbVols, double *dTimes,
                                           double *dSigmaTarget,
                                           double *dSigma0, double *dCoefs,
                                           double *dSigma) {
  Err err = NULL;
  int i;
  double dLastTime;
  ...
  err = GenericMidat_VarModelLinear(iNbVols, dTimes, dSigmaTarget, sig2, dCoefs,
                                    dSigma, &var2);
  ...
  while (err && i < 4) {
    ...
    err = GenericMidat_VarModelLinear(iNbVols, dTimes, dSigmaTarget, sig2,
                                      dCoefs, dSigma, &var2);
...
static Err solve_integ_exponential(double t1, double t2, double target,
                                   double *lam) {
  Err err = NULL;
  double lam1, lam2;
...
Err GenericMidat_ConvertModelLambda(int iNbVols, double *dTimes,
                                    double *dSigmaTarget, double *dLambda,
                                    double *dSigma0, double *dSigma) {
  int i;
  ...
  *dSigma0 = sqrt(dVar / dTimes[iNbVols - 1]);
  ...
  err = GenericMidat_OptimalConvertModelLinear(iNbVols, dTimes, dSigmaTarget,
                                               dSigma0, dLambda, dSigma);
  ...
  for (i = 0; i < iNbVols; i++) {
    ...
    err = solve_integ_exponential(0, dTimes[i] - dLastTime, dTarget,
                                  &(dLambda[i]));
    ...
    if (err) {
      ...
      dLambda[i] /= 2.0;
      dLambda[i] =
          max(dLambda[i], log(dMinVol / dLastSigma) / (dTimes[i] - dLastTime));
    }
...
Err GenericMidat_OneFactor_PDE(/* Time Discretisation */
  ...
  double *dXtGrid = NULL, *dNextXtGrid = NULL, *dXtGridTemp, *dXtDrift = NULL,
         *dXtVar = NULL, *dXtVarGrid = NULL, *dShortR = NULL, *dXtTotStd = NULL,
         *dBeta = NULL, *dLambda = NULL, *dSigma = NULL;

  double dSigma0;

  double **dNextValues = NULL, **dValues = NULL, **dValuesTemp;
  ...
  if (!dXtGrid || !dNextXtGrid || !dXtDrift || !dXtVar || !dXtVarGrid ||
      !dShortR || !dXtTotStd || !dNextValues || !dValues || !dBeta ||
      !sReconsParams) {
  ...
  for (i = 1; i < lNbTime; i++) {
    ...
    dXtVar[i] =
        sModel->dSigma[Get_Index(dTimes[i], sModel->dTimes, sModel->iNbVols)];
  ...
  if (sParams->iDiffMethod == 2) {
    ...
    err = GenericMidat_ConvertModelLambda(sModel->iNbVols, sModel->dTimes,
                                          sModel->dSigma, dLambda, &dSigma0,
                                          dSigma);
    ...
    for (i = 1; i < lNbTime; i++) {
      ...
      dXtVar[i] = dt;
      dXtTotStd[i] /=
          dSigma[Get_Index(dTimes[i], sModel->dTimes, sModel->iNbVols)];
      dXtTotStd[i] = max(dXtTotStd[i], dXtTotStd[i - 1]);
    }
  }
  ...
  if (iEvalEvt[lNbTime - 1]) {
    ...
    case 2: {
      sReconsParams->dCoefX1 = dSigma[Get_Index(
          dTimes[lNbTime - 1], sModel->dTimes, sModel->iNbVols)];
      break;
    }
    ...
    err = payoff_func(dTimes[lNbTime - 1], sFuncParmTab[lNbTime - 1], 0,
                      sParams->lNbX - 1, dXtGrid, sReconsParams, iNbProd,
                      dNextValues);
  ...
  num_f_pde_init(pde, sParams->lNbX, iNbProd);
  ...
  for (step = lNbTime - 2; step >= 0; step--) {
    ...
    if (sParams->iDiffMethod == 2) {
      ...
      lambda =
          dLambda[Get_Index(dTimes[step + 1], sModel->dTimes, sModel->iNbVols)];
    ...
    num_f_pde_one_step_backward(pde, sParams->lNbX, dXtGrid, iNbProd,
                                dNextValues, dXtDrift, dXtVarGrid, dShortR,
                                sParams->dTheta, dValues);
    ...
    if (iEvalEvt[step]) {
      ...
      case 2: {
        sReconsParams->dCoefX1 =
            dSigma[Get_Index(dTimes[step], sModel->dTimes, sModel->iNbVols)];
        break;
      }
      ...
      err = payoff_func(dTimes[step], sFuncParmTab[step], 0, sParams->lNbX - 1,
                        dXtGrid, sReconsParams, iNbProd, dValues);
    ...
    if (sParams->iDiffMethod == 1 && step > 0 &&
        (fabs(dBeta[step + 1] - dBeta[step]) > 1.0E-04)) {
      if (iFirstEvent == 1) {
        iFirstEvent++;
      } else {
        if (sParams->iAdaptGrid) {
          /* Create the new grid */
          dx =
              sParams->dNbStd * dXtTotStd[step] / ((sParams->lNbX - 1.0) * 0.5);
        ...
        GenericMidat_OneFactor_ChangeBetaGrid(
            0, sParams->lNbX - 1, 0, iNbProd - 1, dXtGrid, dBeta[step + 1],
            dXtGridTemp, dBeta[step], sModel, sParams, dValues, dNextValues);
...
FREE_RETURN:
  ...
  if (dNextValues)
    free_dmatrix(dNextValues, 0, sParams->lNbX - 1, 0, iNbProd - 1);
  if (dValues)
    free_dmatrix(dValues, 0, sParams->lNbX - 1, 0, iNbProd - 1);
...
/* New Value in (Xt        , Zt) = Old Value in (Xt        , dCoefX * Xt +
 * dCoefZ * Zt) */
Err GenericMidat_ValueJump_PDE(long lStartX, long lEndX, long lStartZ,
                               long lEndZ, long lStartProd, long lEndProd,

                               double *dGridX, double *dGridZ,
                               double *dNextGridX, double *dNextGridZ,
                               ...
                               double ***dTempValues, double ***dOldValues,
                               double ***dNewValues) {
  Err err = NULL;
  ...
  if (fabs(sReconsParams->dCoefX1 - sReconsParams->dLastCoefX1) < 1.0E-10 &&
    ...
    dCoefX = (sReconsParams->dLastCoefY1 - sReconsParams->dCoefY1) /
             sReconsParams->dCoefY2;
    ...
    if (sParams->iJumpQuadra == 0) {
      ...
      for (i = lStartX; i <= lEndX; i++) {
        ...
        for (j = lStartZ; j <= lEndZ; j++) {
          ...
          newJ = max(min(lStartZ + (int)((dNewZ - dFirstZ) / dz), lEndZ - 1),
                     lStartZ);
          dProba = (dNewZ - dNextGridZ[newJ]) /
                   (dNextGridZ[newJ + 1] - dNextGridZ[newJ]);

          for (k = lStartProd; k <= lEndProd; k++) {
            dNewValues[i][j][k] = (1.0 - dProba) * dOldValues[i][newJ][k] +
                                  dProba * dOldValues[i][newJ + 1][k];
          }
        }
      }
    } else {
      ...
      for (i = lStartX; i <= lEndX; i++) {
        ...
        for (j = lStartZ; j <= lEndZ; j++) {
          ...
          newJ =
              max(min(lStartZ + (int)((dNewZ - dFirstZ) / dz + 0.5), lEndZ - 1),
                  lStartZ + 1);

          dProbaDown =
              (dNewZ * (dNewZ - dNextGridZ[newJ] - dNextGridZ[newJ + 1]) +
               dNextGridZ[newJ] * dNextGridZ[newJ + 1]) /
              (2.0 * dz2);
          dProbaMid =
              (dNewZ * (dNewZ - dNextGridZ[newJ - 1] - dNextGridZ[newJ + 1]) +
               dNextGridZ[newJ - 1] * dNextGridZ[newJ + 1]) /
              (-dz2);
          dProbaUp =
              (dNewZ * (dNewZ - dNextGridZ[newJ - 1] - dNextGridZ[newJ]) +
               dNextGridZ[newJ - 1] * dNextGridZ[newJ]) /
              (2.0 * dz2);
    ...
    if (sParams->iJumpQuadra == 0) {
      for (i = lStartX; i <= lEndX; i++) {
        ...
        newI = max(min(lStartX + (int)((dNewX - dFirstX) / dx), lEndX - 1),
                   lStartX);
        dProba = (dNewX - dNextGridX[newI]) /
                 (dNextGridX[newI + 1] - dNextGridX[newI]);

        for (j = lStartZ; j <= lEndZ; j++) {
          for (k = lStartProd; k <= lEndProd; k++) {
            dTempValues[i][j][k] = (1.0 - dProba) * dOldValues[newI][j][k] +
                                   dProba * dOldValues[newI + 1][j][k];
          }
        }
      }
    } else {
      ...
      for (i = lStartX; i <= lEndX; i++) {
        dNewX = dCoefX * dGridX[i];
        newI =
            max(min(lStartX + (int)((dNewX - dFirstX) / dx + 0.5), lEndX - 1),
                lStartX + 1);

        dProbaDown =
            (dNewX * (dNewX - dNextGridX[newI] - dNextGridX[newI + 1]) +
             dNextGridX[newI] * dNextGridX[newI + 1]) /
            (2.0 * dx2);
        dProbaMid =
            (dNewX * (dNewX - dNextGridX[newI - 1] - dNextGridX[newI + 1]) +
             dNextGridX[newI - 1] * dNextGridX[newI + 1]) /
            (-dx2);
        dProbaUp = (dNewX * (dNewX - dNextGridX[newI - 1] - dNextGridX[newI]) +
                    dNextGridX[newI - 1] * dNextGridX[newI]) /
                   (2.0 * dx2);
    ...
    err = GenericMidat_ValueJump_PDE(
        lStartX, lEndX, lStartZ, lEndZ, lStartProd, lEndProd, dGridX, dGridZ,
        dNextGridX, dNextGridZ, iIndex, sModel, sParams, sReconsParams,
        dTempValues, dTempValues, dNewValues);
  }
...
Err GenericMidat_TwoFactor_PDE(/* Time Discretisation */
  ...
  double *dXtGrid = NULL, *dNextXtGrid = NULL, *dXtGridTemp, *dZtGrid = NULL,
         *dNextZtGrid = NULL, *dZtGridTemp, *dXtVar = NULL, *dZtVar = NULL,
         *dCorrel = NULL, **dXtDrift = NULL, **dZtDrift = NULL,
         **dXtVarGrid = NULL, **dZtVarGrid = NULL, **dShortR = NULL,
         *dXtTotStd = NULL, *dZtTotStd = NULL, *dLambda1 = NULL,
         *dSigma1 = NULL, *dLambda2 = NULL, *dSigma2 = NULL;

  double dSigma01, dSigma02;

  double *dBeta1 = NULL, *dBeta2 = NULL;

  double ***dNextValues = NULL, ***dValues = NULL, ***dTempValues = NULL,
         ***dValuesTemp;
  ...
  if (!dXtVar || !dZtVar || !dCorrel || !dXtTotStd || !dZtTotStd ||
      !ValueJump || !dBeta1 || !dBeta2 || !sReconsParams) {
  ...
  if (sParams->iDiffMethod == 2) {
    ...
    /* We need to convert the model */
    err = GenericMidat_ConvertModelLambda(sModel->iNbVols, sModel->dTimes,
                                          sModel->dSigma, dLambda1, &dSigma01,
                                          dSigma1);
    ...
    err = GenericMidat_ConvertModelLambda(sModel->iNbVols, sModel->dTimes,
                                          sModel->dSigma2, dLambda2, &dSigma02,
                                          dSigma2);
    ...
    for (i = 1; i < lNbTime; i++) {
      ...
      if (index > 0) {
        dXtTotStd[i] /=
            0.5 * (sModel->dSigma[index] + sModel->dSigma[index - 1]);
        dZtTotStd[i] /=
            0.5 * (sModel->dSigma2[index] + sModel->dSigma2[index - 1]);
      } else {
  ...
  iNbX = (int)(sParams->lNbX *
                   sqrt(dXtTotStd[lNbTime - 1] / dZtTotStd[lNbTime - 1]) +
               0.5);
  iNbZ = (int)(sParams->lNbX *
                   sqrt(dZtTotStd[lNbTime - 1] / dXtTotStd[lNbTime - 1]) +
               0.5);
  ...
  if (!dXtGrid || !dNextXtGrid || !dZtGrid || !dNextZtGrid || !dXtDrift ||
      !dZtDrift || !dXtVarGrid || !dZtVarGrid || !dShortR || !dNextValues ||
      !dValues || !dTempValues) {
  ...
  if (iEvalEvt[lNbTime - 1]) {
    ...
    case 0: {
      ...
      sReconsParams->dCoefX1 = 1.0;
      sReconsParams->dCoefX2 = 0.0;
      sReconsParams->dCoefY1 = sModel->dCorrel[index] * sModel->dSigma2[index] /
                               sModel->dSigma[index];
      sReconsParams->dCoefY2 =
          sqrt(1.0 - sModel->dCorrel[index] * sModel->dCorrel[index]);
    ...
    case 2: {
      ...
      sReconsParams->dCoefX2 = 0.0;
      sReconsParams->dCoefY1 = sModel->dCorrel[index] * sModel->dSigma2[index] /
                               sModel->dSigma[index] * dSigma1[index];
      sReconsParams->dCoefY2 =
          sqrt(1.0 - sModel->dCorrel[index] * sModel->dCorrel[index]) *
          dSigma2[index];
    ...
    err =
        payoff_func(dTimes[lNbTime - 1], sFuncParmTab[lNbTime - 1], lx, ux, lz,
                    uz, dXtGrid, dZtGrid, sReconsParams, iNbProd, dNextValues);
  ...
  num_f_pde_init_2d_adi(pde, iNbX, iNbZ, iNbProd);
  ...
  for (step = lNbTime - 2; step >= 0; step--) {
    ...
    num_f_pde_one_step_backward_2f_adi(pde, iNbX, dXtGrid, iNbZ, dZtGrid, 0,
                                       iNbProd - 1, dNextValues, dXtDrift,
                                       dZtDrift, dXtVarGrid, dZtVarGrid,
                                       dShortR, dValues, lx, ux, lz, uz);

    /*	Jump Valuation */
    if (ValueJump[step] > 0 &&
        (sParams->iDiffMethod == 0 || sParams->iDiffMethod == 2)) {
      /* Fill Recons Param */
      switch (sParams->iDiffMethod) {
      case 0: {
        ...
        sReconsParams->dCoefX1 = 1.0;
        sReconsParams->dCoefX2 = 0.0;
        sReconsParams->dCoefY1 = sModel->dCorrel[index] *
                                 sModel->dSigma2[index] / sModel->dSigma[index];
        sReconsParams->dCoefY2 =
            sqrt(1.0 - sModel->dCorrel[index] * sModel->dCorrel[index]);

        sReconsParams->dLastCoefX1 = 1.0;
        sReconsParams->dLastCoefX2 = 0.0;
        sReconsParams->dLastCoefY1 = sModel->dCorrel[index - 1] *
                                     sModel->dSigma2[index - 1] /
                                     sModel->dSigma[index - 1];
        sReconsParams->dLastCoefY2 =
            sqrt(1.0 - sModel->dCorrel[index - 1] * sModel->dCorrel[index - 1]);
      ...
      case 2: {
        ...
        sReconsParams->dLastCoefY1 = sModel->dCorrel[index - 1] *
                                     sModel->dSigma2[index - 1] /
                                     sModel->dSigma[index - 1] * dSigma1[index];
        sReconsParams->dLastCoefY2 =
            sqrt(1.0 -
                 sModel->dCorrel[index - 1] * sModel->dCorrel[index - 1]) *
            dSigma2[index];
      ...
      err = GenericMidat_ValueJump_PDE(
          lx, ux, lz, uz, 0, iNbProd - 1, dXtGridTemp, dZtGridTemp, dXtGrid,
          dZtGrid, ValueJump[step] - 1, sModel, sParams, sReconsParams,
          dTempValues, dValues, dNextValues);
    ...
    if (iEvalEvt[step]) {
      ...
      err = payoff_func(dTimes[step], sFuncParmTab[step], lx, ux, lz, uz,
                        dXtGrid, dZtGrid, sReconsParams, iNbProd, dValues);
    ...
    if (sParams->iDiffMethod == 1 && step > 0 &&
        (fabs(dBeta1[step + 1] - dBeta1[step]) > 1.0E-04)) {
      if (iFirstEvent == 1) {
        iFirstEvent++;
      } else {
        if (sParams->iAdaptGrid) {
        ...
        GenericMidat_TwoFactor_ChangeBetaGrid(
            0, iNbX - 1, dXtGrid,

            0, iNbZ - 1, dZtGrid,

            0, iNextNbX - 1, dXtGridTemp,

            0, iNextNbZ - 1, dZtGridTemp,

            ValueJump[step] - 2,

            sModel, sParams, 0, iNbProd - 1, dTempValues, dValues, dNextValues);
...
FREE_RETURN:
  ...
  if (dXtDrift)
    free_dmatrix(dXtDrift, 0, iNbX - 1, 0, iNbZ - 1);
  if (dZtDrift)
    free_dmatrix(dZtDrift, 0, iNbX - 1, 0, iNbZ - 1);
  if (dXtVarGrid)
    free_dmatrix(dXtVarGrid, 0, iNbX - 1, 0, iNbZ - 1);
  if (dZtVarGrid)
    free_dmatrix(dZtVarGrid, 0, iNbX - 1, 0, iNbZ - 1);
  if (dShortR)
    free_dmatrix(dShortR, 0, iNbX - 1, 0, iNbZ - 1);

  if (dNextValues)
    free_f3tensor(dNextValues, 0, iNbX - 1, 0, iNbZ - 1, 0, iNbProd - 1);
  if (dValues)
    free_f3tensor(dValues, 0, iNbX - 1, 0, iNbZ - 1, 0, iNbProd - 1);
  if (dTempValues)
    free_f3tensor(dTempValues, 0, iNbX - 1, 0, iNbZ - 1, 0, iNbProd - 1);
...
Err GenericMidat_OneFactor_MC(/* Time Discretisation */
  ...
  double *dXt = NULL, *dInitGauss = NULL, *dBrownian = NULL, **dPayoff = NULL,
         **dResEvt = NULL, ***dSaveValues = NULL;
  ...
  if (!dXt || !dInitGauss || !dBrownian || !dPayoff || !dResEvt ||
      !sReconsParams) {
  ...
  if (iDoOptim) {
    dSaveValues =
        f3tensor(0, iNbEvent - 1, 0, sMCEBParams->iNbIndex, 0, iNumPaths - 1);
  ...
  for (j = 0; j < iNbEvent; j++) {
    ...
    for (k = iLastIndex + 1; k <= iIndex; k++) {
      dStdX += sModel->dSigma[k] * sModel->dSigma[k] *
               (sModel->dTimes[k] - dLastTime);
    ...
    if (sFuncParmTab[j]) {
      ...
      err = payoff_func(dTimes[j], sFuncParmTab[j], iNumPaths, dXt,
                        sReconsParams, iNbProd, dResEvt);
    ...
    if (iDoOptim) {
      for (i = 0; i < iNumPaths; i++) {
        ...
        if (iOptimise[j]) {
          dSaveValues[j][sMCEBParams->iNbIndex][i] =
              dResEvt[i][sMCEBParams->iColPay];
        } else {
  ...
  for (k = 0; k < iNbProd; k++) {
    dProdVal[k][1] =
        (dProdVal[k][1] - dProdVal[k][0] * dProdVal[k][0]) / iNumPaths;
  ...
  if (iDoOptim) {
    if (sMCEBParams->iFindBestOptim) {
      err = find_best_optim_dates(
          dSaveValues, iNbEvent, sMCEBParams->iFindBestOptim, iNumPaths,
          iOptimise, sMCEBParams, &(dProdVal[iNbProd][0]),
          &(dProdVal[iNbProd][1]));
    } else {
      err = find_and_optimise_boundary(
          dSaveValues, iNbEvent, iNumPaths, iOptimise, sMCEBParams,
          &(dProdVal[iNbProd][0]), &(dProdVal[iNbProd][1]));
    }
...
FREE_RETURN:
  ...
  if (dBrownian)
    free(dBrownian);
  if (dPayoff)
    free_dmatrix(dPayoff, 0, iNumPaths - 1, 0, iNbProd - 1);
  if (dResEvt)
    free_dmatrix(dResEvt, 0, iNumPaths - 1, 0, iNbProd - 1);

  if (sReconsParams)
    free(sReconsParams);

  if (dSaveValues)
    free_f3tensor(dSaveValues, 0, iNbEvent - 1, 0, sMCEBParams->iNbIndex, 0,
                  iNumPaths - 1);
...
Err GenericMidat_TwoFactor_MC(/* Time Discretisation */
  ...
  double *dXt = NULL, *dYt = NULL, *dInitGauss = NULL, **dBrownian = NULL,
         **dPayoff = NULL, **dResEvt = NULL, ***dSaveValues = NULL;
  ...
  if (!dXt || !dYt || !dInitGauss || !dBrownian || !dPayoff || !dResEvt ||
      !sReconsParams) {
    err = "Memory allocation (2) failure in GenericMidat_OneFactor_MC";
    goto FREE_RETURN;
  }
  ...
  for (j = 0; j < iNbEvent; j++) {
    ...
    for (k = iLastIndex + 1; k <= iIndex; k++) {
      dVarX += sModel->dSigma[k] * sModel->dSigma[k] *
               (sModel->dTimes[k] - dLastTime);
      dVarY += sModel->dSigma2[k] * sModel->dSigma2[k] *
               (sModel->dTimes[k] - dLastTime);
      dCovar += sModel->dCorrel[j] * sModel->dSigma[k] * sModel->dSigma2[k] *
                (sModel->dTimes[k] - dLastTime);
    ...
    dStdX = dCoef11 * dCoef11 * dVarX + dCoef12 * dCoef12 * dVarY +
    ...
    if (j < sModel->iNbVols - 1) {
      ...
      dStdY = dCoef21 * dCoef21 * dVarX + dCoef22 * dCoef22 * dVarY +
    ...
    if (j > 0) {
      ...
      dTransCoef11 =
          1.0 / dDelta * (dCoef11 * dLastCoef22 - dCoef12 * dLastCoef21);
      dTransCoef12 =
          1.0 / dDelta * (-dCoef11 * dLastCoef12 + dCoef12 * dLastCoef11);
      dTransCoef21 =
          1.0 / dDelta * (dCoef21 * dLastCoef22 - dCoef22 * dLastCoef21);
      dTransCoef22 =
          1.0 / dDelta * (-dCoef21 * dLastCoef12 + dCoef22 * dLastCoef11);
    } else {
      dTransCoef11 = 0.0;
      dTransCoef12 = 0.0;
      dTransCoef21 = 0.0;
    ...
    for (i = 0; i < iNumPaths; i++) {
      temp = dTransCoef11 * dXt[i] + dTransCoef12 * dYt[i] +
             dStdX * dBrownian[0][i];
      dYt[i] = dTransCoef21 * dXt[i] + dTransCoef22 * dYt[i] +
               dStdY * (dCorrel * dBrownian[0][i] + dCorrel2 * dBrownian[1][i]);
      dXt[i] = temp;
    }

    /* Payoff Evaluation */
    if (sFuncParmTab[j]) {
      err = payoff_func(dTimes[j], sFuncParmTab[j], iNumPaths, dXt, dYt,
                        sReconsParams, iNbProd, dResEvt);

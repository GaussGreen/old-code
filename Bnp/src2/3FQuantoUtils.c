									...
									int		*sig_merge_n)
{
	...
	*corr_merge_r3_fx = (double*) calloc (*sig_merge_n, sizeof (double));
	...
	if (!(*sig_merge_r1) || !(*sig_merge_r3) || !(*sig_merge_fx)
...
FREE_RETURN:
	...
	if (err)
	{
		...
		if (*sig_merge_r1) free (*sig_merge_r1);
		if (*sig_merge_r3) free (*sig_merge_r3);
								...
								double		*lgm1F_ifr)
{
	...
	double	correl_r1_r2, correl_r1_r3, correl_r1_fx,
		                  correl_r2_r3, correl_r2_fx,
						                correl_r3_fx;
	...
	for (i=1; i<nstp ; i++)
	{
		...
		for (j=0; j<i ; j++)
		{
			...
			/* R1 non quanto fwd */
			L1 = 1.0 / (lambda_R1 * lambda_R1) * (aux1_2 * (1.0 - 0.5 * aux1_2) - aux1_1 * (1.0 - 0.5 * aux1_1));
			L2 = 1.0 / lambda_R2 * (aux1_2 * (1.0 / lambda_R1 - aux2_2 / lambda_R1R2) 
				                  - aux1_1 * (1.0 / lambda_R1 - aux2_1 / lambda_R1R2));
			fwd_r1 += vol_r1 * vol_r1 * L1 + correl_r1_r2 * vol_r1 * vol_r2 * L2;

			/* R2 non quanto fwd */
			L1 = 1.0 / (lambda_R2 * lambda_R2) * (aux2_2 * (1.0 - 0.5 * aux2_2) - aux2_1 * (1.0 - 0.5 * aux2_1));
			L2 = 1.0 / lambda_R1 * (aux2_2 * (1.0 / lambda_R2 - aux1_2 / lambda_R1R2)
				                   -aux2_1 * (1.0 / lambda_R2 - aux1_1 / lambda_R1R2));
			fwd_r2 += vol_r2 * vol_r2 * L1 + correl_r1_r2 * vol_r1 * vol_r2 * L2;

			/* R3 non quanto fwd */
			L1 = 1.0 / (lambda_R3 * lambda_R3) * (aux3_2 * (1.0 - 0.5 * aux3_2) - aux3_1 * (1.0 - 0.5 * aux3_1));

...
Err quadr_best_fit(double **x, double *y, long n, long dim, double *a,
  ...
  double **X = dmatrix(1, n, 1, nvar), **Y = dmatrix(1, n, 1, 1),
         **transX = NULL, **transXX = NULL, **inv_transXX = NULL,
         **transXY = NULL, **inv_transXX_transXY = NULL, **inv_hess = NULL,
         **grad_matrix = dmatrix(1, dim, 1, 1), **inv_hess_grad = NULL,
         *eigen_val = dvector(1, dim), **eigen_vec = dmatrix(1, dim, 1, dim);

  Err err;

  /*	transform the problem into unidimensional one
          variables are 1        , x1        , ...        , xdim        , x1^2
     , x2*x1        , x2^2        , x3*x1 , x3*x2        , x3^2        , ...
     , xdim^2 */
  ...
  inv_transXX_transXY =
      product_matrix(inv_transXX, 1, nvar, 1, nvar, transXY, 1, nvar, 1, 1);
  if (!inv_transXX_transXY)
    return serror("error in inverse_matrix");

  /* interpret results in terms of a        , grad        , hess with y[i] = a +
   * <grad ,x[i]> + tx[i]*hess*x[i] */
  ...
  *a = inv_transXX_transXY[1][1];
  ...
  /* if hess is definite positive        , find minimum with formula min = - 0.5
   * * inv (hess) * grad */
  ...
  if (def > 0) {
    ...
    inv_hess_grad =
        product_matrix(inv_hess, 1, dim, 1, dim, grad_matrix, 1, dim, 1, 1);
    if (!inv_hess_grad)
      return serror("error in product_matrix");

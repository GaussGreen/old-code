...
Err srt_f_SABW_Opt(Date		eval_date,
				   Date		exp_date,
				   ...
				   double *option_pv)
{
	...
	double				yr_to_exp_date, TimeStep = 1.0/24,dX, PDEBoundary[2], *X = NULL,*exp_X = NULL,
						*TerminalPayoff = NULL, **D = NULL, **V = NULL, *U = NULL,**div_mat = NULL;
	...
	for(j = 1; j < NumMesh; j++)
	{
		X[j] = X_min + j*dX;
		exp_X[j] = exp(X[j]);
		if(RecPay == SRT_RECEIVER) 	TerminalPayoff[j] = max(option_strike-exp(X[j]),0);
		else if(RecPay == SRT_PAYER) TerminalPayoff[j] = max(exp(X[j])-option_strike,0);
		else if(RecPay == SRT_STRADDLE) TerminalPayoff[j] = max(option_strike-exp(X[j]),0) + max(exp(X[j])-option_strike,0);

		for(i = 0; i <= NumTimeStep; i++)
		{
			if(i == 0) time = yr_to_exp_date;
			else time = max(TimeStep,yr_to_exp_date-(i-1)*TimeStep);

			std = log(exp_X[j]/exp(X0))/(basevol*sqrt(time));

			V[i][j] = basevol*(1+beta*omega*tanh(std/omega)+gamma*omega*omega*tanh(std/omega)*tanh(std/omega));
	...
	err =  srt_f_feynmann_kac_pde(NumMesh,
								NumTimeStep,
								dX,
								TimeStep,
	...
	if(greek_type == PREMIUM) 	(*option_pv) = df_exp*U[k];
	else if(greek_type == DELTA) (*option_pv) = df_exp*(U[k+1]-U[k])/dX;
	else if(greek_type == GAMMA) (*option_pv) = df_exp*(U[k+1]-2*U[k]+U[k-1])/(dX*dX);

	if(X) free_dvector(X,1,NumMesh-1); X = NULL;
	if(exp_X) free_dvector(exp_X,1,NumMesh-1); exp_X = NULL;
	if(U) free_dvector(U,1,NumMesh-1); U = NULL;
	if(TerminalPayoff) free_dvector(TerminalPayoff,1,NumMesh-1); TerminalPayoff = NULL;
	if(V) free_matrix(V,0,NumTimeStep,1,NumMesh); V = NULL;
	if(D) free_matrix(D,0,NumTimeStep,1,NumMesh); D = NULL;
	if(div_mat) free_dmatrix(div_mat,0,NumTimeStep,1,NumMesh); div_mat = NULL;

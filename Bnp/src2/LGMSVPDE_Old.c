...
/* -----------------------------------------------------------------------------------------------
        ...
        Calculation of expectations for the definition of the grids
                ...
                in Epst (constant grid center in EpsTMean =E(EpsT / Eps0) and +- iNbSigmaEpsGrid
   EpsStd) in Phit (variable grid center at each t on PhitMean(t)
    ...
    double* ZtStd)
{
    ...
    for (iNumTime = 1; iNumTime < iNbTime; iNumTime++)
    {
        ...
        /* Calculation of Zt Mean */
        ZtStd[iNumTime] = sqrt(
            dVt2Mean - 1 + pow(dAlpha * dRho / (Sigt[iNumTime] * dDerBeta), 2) * dVarft -
        ...
        case LGMSV_TRUE:
        {
            ...
            else
            {
                ...
                default:
                {
                    ...
                    dPhiStep =
                        dPhiMean *
                        (exp(iNbSigmaPhiGrid * dLogPhiStd) - exp(-iNbSigmaPhiGrid * dLogPhiStd)) /
                        (iNbPhi - 1);
        ...
        case LGMSV_QUADRATIC:
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    /* Interpolation */
                    fx0 = fx[iIndex1][iNumX][iNumEps][iNumProduct] / (x[iIndex1] - x[iIndex2]) /
                              (x[iIndex1] - x[iIndex3]) *
                              (x0 * x0 - (x[iIndex2] + x[iIndex3]) * x0 + x[iIndex2] * x[iIndex3]) +
                          fx[iIndex2][iNumX][iNumEps][iNumProduct] / (x[iIndex2] - x[iIndex3]) /
                              (x[iIndex2] - x[iIndex1]) *
                              (x0 * x0 - (x[iIndex3] + x[iIndex1]) * x0 + x[iIndex3] * x[iIndex1]) +
                          fx[iIndex3][iNumX][iNumEps][iNumProduct] / (x[iIndex3] - x[iIndex1]) /
                              (x[iIndex3] - x[iIndex2]) *
    ...
    double*** ValueGridXtZtProduct)
{
    ...
    for (iNumX = iIndexMinX; iNumX <= iIndexMaxX; iNumX++)
    {
        ...
        for (iNumZ = iIndexMinZ; iNumZ <= iIndexMaxZ; iNumZ++)
        {
            ...
            /* dVt =
             * max((GridZCenter[iNumZ]+dZtMean)+dAlpha*dRho/dSigt/dDerBeta*GridftTstar[iNumX],dVFloor);
             */
            dVt =
                max((GridZCenter[iNumZ] + dZtMean) + dAlphaRho_SigtDerBeta * GridftTstar[iNumX],
                    dVFloor);
            ...
            if (iIndexPhi == -1)
                ...
                (iIndexPhi > 1) && (iIndexPhi < iNbPhi - 3) &&
    ...
    double* dProductArrayPv)
{
    ...
    if (Params.IsNbSigmaXGridRightAuto == LGMSV_TRUE)
    {
        ...
        else
        {
            Params.iNbSigmaXGridRight = 7.5 + 25 * max(dRho, 0) * dAlpha / 2.0 *
                                                  sqrt(
                                                      (1 - exp(-dLambdaEps * dTime[iNbTime - 1])) /
    ...
    ValueGridftZtProduct = f3tensor(0, iNbX - 1, 0, iNbEps - 1, 0, iNbProduct - 1);

    ValueTensor_t = f4tensor(0, iNbPhi - 1, 0, iNbX - 1, 0, iNbEps - 1, 0, iNbProduct - 1);

    ValueTensor_t1 = f4tensor(0, iNbPhi - 1, 0, iNbX - 1, 0, iNbEps - 1, 0, iNbProduct - 1);
    ...
    if (!pde || !Sigt || !GridftTstar || !GridZCenter || !GridPhi_t || !GridPhi_t1 || !PhiMean ||
        !PhiStd || !ftMean || !ftStd || !ZtMean || !ZtStd || !DfDrift || !DZDrift || !DfVar ||
        !DZVar || !r || !ValueGridftZtProduct || !ValueTensor_t || !ValueTensor_t1)
    {
    ...
    if (Params.ModeExport != LGMSV_TRUE)
    {
        ...
        for (iNumTime = iNbTime - 2; iNumTime >= 0; iNumTime--)
        {
            ...
            iNbPhiGrid_t =
                ...
                (Params.GridPhiOptim == LGMSV_TRUE) *
                    (int)((1 - iNumTime / ((double)(iNbTime - 1))) * (iNbPhi - 1) + 1.0e-08);
            iNbPhiGrid_t = ((int)(iNbPhiGrid_t / 2)) * 2 + 1;

            if (Params.GridXADIOptim == LGMSV_TRUE)
            {
                iIndexMinX =
                    (int)((-Params.iNbSigmaXGridLeft * ftStd[iNumTime + 1] - GridftTstar[0]) / (GridftTstar[1] - GridftTstar[0]) + 1.0e-08);
                iIndexMaxX = max(
                    (int)((Params.iNbSigmaXGridRight * ftStd[iNumTime + 1] - GridftTstar[0]) / (GridftTstar[1] - GridftTstar[0]) + 1.0e-08),
            ...
            {
                iIndexMinZ =
                    (int)((-Params.iNbSigmaZGridLeft * ZtStd[iNumTime + 1] - GridZCenter[0]) / (GridZCenter[1] - GridZCenter[0]) + 1.0e-08);
                iIndexMaxZ = max(
                    (int)((Params.iNbSigmaZGridRight * ZtStd[iNumTime + 1] - GridZCenter[0]) / (GridZCenter[1] - GridZCenter[0]) + 1.0e-08),
        ...
        memcpy(
            dProductArrayPv,
            ValueTensor_t1[iIndexPhi][iIndexf0][iIndexZ0],
    ...
    double* ZtStd)
{
    ...
    for (iNumTime = 1; iNumTime < iNbTime; iNumTime++)
    {
        ...
        ZtStd[iNumTime]  = sqrt(
            dVt2Mean + pow(dAlpha * dRho / Sigt[iNumTime], 2) * dXt2Mean -
            ...
            DZDrift[iNumX][iNumZ] =
                ...
                dAlpha * dRho *
                    ((1 / dSig_t1 - 1 / dSigt) * (dXt - dXtMean) +
                     ((dPhit - dPhitMean) - dLambdaX * (dXt - dXtMean)) / dSigt * dDt);
    ...
    double* dProductArrayPv)
{
    ...
    ValueGridXtZtProduct = f3tensor(0, iNbX - 1, 0, iNbEps - 1, 0, iNbProduct - 1);
    ...
    /* Gestion of allocation errors */
    if (!pde || !GridX || !GridXCenter || !Sigt || !GridZCenter || !GridPhi_t || !GridPhi_t1 ||
        !PhiMean || !PhiStd || !XtMean || !XtStd || !ZtMean || !ZtStd || !DXDrift || !DZDrift ||
        !DXVar || !DZVar || !r || !ValueGridXtZtProduct || !ValueTensor_t || !ValueTensor_t1)
    {
        err = "Memory allocation error (1) in LGMSVPDE";
        goto FREE_RETURN;
    }
    ...
    if (Params.ModeExport != LGMSV_TRUE)
    {
        ...
        for (iNumTime = iNbTime - 2; iNumTime >= 0; iNumTime--)
        {
            ...
            {
                iIndexMinX =
                    (int)((-Params.iNbSigmaXGridLeft * XtStd[iNumTime + 1] - GridXCenter[0]) / (GridXCenter[1] - GridXCenter[0]) + 1.0e-08);
                iIndexMaxX =
                    (int)((Params.iNbSigmaXGridRight * XtStd[iNumTime + 1] - GridXCenter[0]) / (GridXCenter[1] - GridXCenter[0]) + 1.0e-08);
            }
            else
            {
                iIndexMinX = 0;
        ...
        memcpy(
            dProductArrayPv,
            ValueTensor_t1[iIndexPhi][iIndexX0][iIndexZ0],
            iNbProduct * sizeof(double));
    }
    else
    {

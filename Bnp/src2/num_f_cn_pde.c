/* -------------------------------------------------------------------------
   ...
   PURPOSE		: convolve backward/forward using Cranck-Nicholson equations
   ------------------------------------------------------------------------- */
    ...
    double* f)
{
    ...
    {
        tmp->right_mem[i][0] =
            tmp->exp_pu[i] * f[i + 1] + tmp->exp_pc[i] * f[i] + tmp->exp_pd[i] * f[i - 1];
    }

    tmp->right_mem[0][0] = tmp->exp_pu[0] * f[1] + tmp->exp_pc[0] * f[0];

    tmp->right_mem[n - 1][0] = tmp->exp_pc[n - 1] * f[n - 1] + tmp->exp_pd[n - 1] * f[n - 2];
}

/*	Calculate right member of FDM equation	*/
/*	m functions	*/
static void calc_right_mem_m(
    CNPDE_TEMP* tmp,
    /*	Size of the array	*/
    int n,
    /*	Number of functions	*/
    ...
    double** f)
{
    ...
    for (j = 0; j < m; j++)
    {
        for (i = 1; i < n - 1; i++)
        {
            tmp->right_mem[i][j] = tmp->exp_pu[i] * f[i + 1][j] + tmp->exp_pc[i] * f[i][j] +
    ...
    int       uy)
{
    ...
    for (j = ly; j <= uy; j++)
    {
        ...
        for (i = lx + 1; i <= ux; i++)
        {
            ...
            for (k = 0; k < m; k++)
            {
                f_t[i][j][k] = (tmpx->right_mem[i][k] - tmpx->imp_pd[i] * f_t[i - 1][j][k]) / bet;
            }
        }
    ...
    for (i = lx; i <= ux; i++)
    {
        ...
        for (k = 0; k < m; k++)
        {
            for (j = ly + 1; j <= uy - 1; j++)
            {
                tmpy->right_mem[j][k] = tmpy->exp_pu[j] * fi[j + 1][k] +
                                        tmpy->exp_pc[j] * fi[j][k] + tmpy->exp_pd[j] * fi[j - 1][k];
            }
        ...
        for (j = ly + 1; j <= uy; j++)
        {
            ...
            for (k = 0; k < m; k++)
            {
                fi[j][k] = (tmpy->right_mem[j][k] - tmpy->imp_pd[j] * fi[j - 1][k]) / bet;
            }
        }
    ...
    int uy)
{
    ...
    static double *muxi, *varxi, *ri, **fi, **tmpfi, *xexp_pui, *xexp_pci, *xexp_pdi, *ximp_pui,
        *ximp_pci, *ximp_pdi, *yexp_pui, *yexp_pci, *yexp_pdi, *yimp_pui, *yimp_pci, *yimp_pdi;
    static double temp1, temp2;

    double **xexp_pu = tmp->xexp_pu, **xexp_pc = tmp->xexp_pc, **xexp_pd = tmp->xexp_pd,

           **ximp_pu = tmp->ximp_pu, **ximp_pc = tmp->ximp_pc, **ximp_pd = tmp->ximp_pd,

           **yexp_pu = tmp->yexp_pu, **yexp_pc = tmp->yexp_pc, **yexp_pd = tmp->yexp_pd,

           **yimp_pu = tmp->yimp_pu, **yimp_pc = tmp->yimp_pc, **yimp_pd = tmp->yimp_pd,
    ...
    for (j = ly; j <= uy; j++)
    {
        ...
        for (i = lx + 1; i <= ux; i++)
        {
            ...
            for (k = mstart; k <= mend; k++)
            {
                f_t[i][j][k] = (f_t_plus_1[i][j][k] - ximp_pd[i][j] * f_t[i - 1][j][k]) / bet;
            }
        }
    ...
    for (i = lx; i <= ux; i++)
    {
        ...
        for (k = mstart; k <= mend; k++)
        {
            for (j = ly + 1; j <= uy - 1; j++)
            {
                tmpfi[j][k - mstart] = yexp_pui[j] * fi[j + 1][k] + yexp_pci[j] * fi[j][k] +
                                       yexp_pdi[j] * fi[j - 1][k];
            }

            tmpfi[ly][k - mstart] = yexp_pui[ly] * fi[ly + 1][k] + yexp_pci[ly] * fi[ly][k];

            tmpfi[uy][k - mstart] = yexp_pci[uy] * fi[uy][k] + yexp_pdi[uy] * fi[uy - 1][k];
        }
    }
    ...
    for (i = lx; i <= ux; i++)
    {
        ...
        for (j = ly + 1; j <= uy; j++)
        {
            ...
            {
                tmpfi[j][k - mstart] =
                    (tmpfi[j][k - mstart] - yimp_pdi[j] * tmpfi[j - 1][k - mstart]) / bet;
            }
        }
    ...
    int  invuy)
{
    ...
    for (j = invly; j <= invuy; j++)
    {
        for (k = mstart; k <= mend; k++)
        {
            f_t[invlx[j]][j][k] = f_t_plus_1[invlx[j]][j][k] / (bet = ximp_pc[invlx[j]][j]);
        }
    ...
    for (j = invly; j <= invuy; j++)
    {
        for (k = mstart; k <= mend; k++)
        {
            ...
            f_t[invlx[j]][j][k] = xexp_pu[invlx[j]][j] * tmpf[invlx[j] + 1][j][k - mstart] +
                                  xexp_pc[invlx[j]][j] * tmpf[invlx[j]][j][k - mstart];

            f_t[invux[j]][j][k] = xexp_pc[invux[j]][j] * tmpf[invux[j]][j][k - mstart] +
    ...
    int uy)
{
    ...
    for (j = ly; j <= uy; j++)
    {
        for (k = 0; k < m; k++)
        {
            for (i = lx + 1; i <= ux - 1; i++)
            {
                f_t[i][j][k] = xexp_pu[i][j] * tmpf[i + 1][j][k] + xexp_pc[i][j] * tmpf[i][j][k] +
                               xexp_pd[i][j] * tmpf[i - 1][j][k];
            }

            f_t[lx][j][k] = xexp_pu[lx][j] * tmpf[lx + 1][j][k] + xexp_pc[lx][j] * tmpf[lx][j][k];

            f_t[ux][j][k] = xexp_pc[ux][j] * tmpf[ux][j][k] + xexp_pd[ux][j] * tmpf[ux - 1][j][k];
        }
    }
}
...
/*	Call before PDE	2D */
void num_f_pde_init_2d_LGMSV_adi(
...
/*	Call after PDE 2D */
void num_f_pde_free_2d_LGMSV_adi(
    CNPDE_TEMP_2D_LGMSV_ADI* tmp,
    /*	Size of the arrays	*/
    int npsi,
    int nx,
    ...
    int uy)
{
    static int     i, j, l, k;
    static double  dux, ddx, duy, ddy, bet;
    static double *muxi, *varxi, *ri, *xexp_pui, *xexp_pci, *xexp_pdi, *ximp_pui, *ximp_pci,
        *ximp_pdi, *yexp_pui, *yexp_pci, *yexp_pdi, *yimp_pui, *yimp_pci, *yimp_pdi;

    static double ***fi, ***fi1, ***fi2, ***fi3, **fij, **fij1, **fij2, **fij3, ***fi_plus1,
        **fij_plus1, ***tmpfi, ***tmpfi1, **tmpfij, **tmpfij1, **tmpfij2, **tmpfij3;
    ...
    for (i = lx + 1; i <= ux - 1; i++)
    {
        ...
        for (j = ly; j <= uy; j++)
        {
            ...
            for (k = lpsi; k <= upsi; k++)
            {
                for (l = mstart; l <= mend; l++)
                {
                    tmpfij[k][l] = xexp_pu[i][j] * fi1[j][k][l] + xexp_pc[i][j] * fij[k][l] +
                                   xexp_pd[i][j] * fij2[k][l];
                }
            }
        }
    }
    ...
    for (j = ly; j <= uy; j++)
    {
        ...
        for (k = lpsi; k <= upsi; k++)
        {
            for (l = mstart; l <= mend; l++)
            {
                ...
                tmpfij1[k][l] = xexp_pc[ux][j] * fij2[k][l] + xexp_pd[ux][j] * fij3[k][l];
            }
        }
    }
    ...
    for (j = ly; j <= uy; j++)
    {
        ...
        for (i = lx + 1; i <= ux; i++)
        {
            ...
            for (k = lpsi; k <= upsi; k++)
            {
                for (l = mstart; l <= mend; l++)
                {
                    tmpfij[k][l] = (tmpf[i][j][k][l] - ximp_pd[i][j] * tmpfij1[k][l]) / bet;
                }
            }
        }
    ...
    for (i = lx; i <= ux; i++)
    {
        ...
        for (k = lpsi; k <= upsi; k++)
        {
            for (l = mstart; l <= mend; l++)
            {
                ...
                fij1[k][l] = yexp_pci[uy] * tmpfij2[k][l] + yexp_pdi[uy] * tmpfij3[k][l];
            }
        }
    }

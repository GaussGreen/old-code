...
double						*f)
{
	...
	for (i=1; i<n-1; i++)
	{
		tmp->right_mem[i][0] =	tmp->exp_pu[i] * f[i+1]
								  + tmp->exp_pc[i] * f[i]
								  + tmp->exp_pd[i] * f[i-1];
			}

	tmp->right_mem[0][0] =	tmp->exp_pu[0] * f[1]
							  + tmp->exp_pc[0] * f[0];
	
	tmp->right_mem[n-1][0] =	tmp->exp_pc[n-1] * f[n-1]
								  + tmp->exp_pd[n-1] * f[n-2];
}
...
double						**f)
{
	...
	for (j=0; j<m; j++)
	{
		...
		tmp->right_mem[0][j] =	tmp->exp_pu[0] * f[1][j]
									  + tmp->exp_pc[0] * f[0][j];
		
		tmp->right_mem[n-1][j] =	tmp->exp_pc[n-1] * f[n-1][j]
									  + tmp->exp_pd[n-1] * f[n-2][j];
	}
}
...
int							uy)	
{
	...
	for (i=lx; i<=ux; i++)
	{
		...
		for (k=0; k<m; k++)
		{
			...
			tmpy->right_mem[ly][k] =	tmpy->exp_pu[ly] * fi[ly+1][k]
										  + tmpy->exp_pc[ly] * fi[ly][k];
			
			tmpy->right_mem[uy][k] =	tmpy->exp_pc[uy] * fi[uy][k]
										  + tmpy->exp_pd[uy] * fi[uy-1][k];
		}
...
int							uy)
{
	...
	static double			*muxi, *varxi, *ri, **fi, **tmpfi,
							*xexp_pui, *xexp_pci, *xexp_pdi, *ximp_pui, *ximp_pci, *ximp_pdi,
							*yexp_pui, *yexp_pci, *yexp_pdi, *yimp_pui, *yimp_pci, *yimp_pdi;
	static double			temp1, temp2;
	
	double					**xexp_pu = tmp->xexp_pu,
							**xexp_pc = tmp->xexp_pc,
							**xexp_pd = tmp->xexp_pd,
	...
	for (i=lx; i<=ux; i++)
	{
		...
		for (k=mstart; k<=mend; k++)
		{
			...
			tmpfi[ly][k-mstart] = yexp_pui[ly] * fi[ly+1][k]
						+ yexp_pci[ly] * fi[ly][k];
			
			tmpfi[uy][k-mstart] = yexp_pci[uy] * fi[uy][k]
							+ yexp_pdi[uy] * fi[uy-1][k];
		}
	}
	...
	for (i=lx; i<=ux; i++)
	{
		...
		for (j=ly+1; j<=uy; j++) 
		{
			...
			for (k=mstart; k<=mend; k++)
			{
				tmpfi[j][k-mstart] = (tmpfi[j][k-mstart] - yimp_pdi[j] * tmpfi[j-1][k-mstart]) / bet;
			}
		}
...
int							invuy)
{
	...
	for (i=lx; i<=ux; i++)
	{
		...
		for (k=mstart; k<=mend; k++)
		{
			...
			tmpfi[ly[i]][k-mstart] = yexp_pui[ly[i]] * fi[ly[i]+1][k]
						+ yexp_pci[ly[i]] * fi[ly[i]][k];
			
			tmpfi[uy[i]][k-mstart] = yexp_pci[uy[i]] * fi[uy[i]][k]
							+ yexp_pdi[uy[i]] * fi[uy[i]-1][k];
		}
	}
...
int							uy)
{
	...
	for (i=lx; i<=ux; i++)
	{
		...
		for (k=0; k<m; k++)
		{
			for (j=ly+1; j<=uy-1; j++)
			{
				tmpfi[j][k] = yexp_pui[j] * fi[j+1][k]
							+ yexp_pci[j] * fi[j][k]
							+ yexp_pdi[j] * fi[j-1][k];
			}

			tmpfi[ly][k] = yexp_pui[ly] * fi[ly+1][k]
						+ yexp_pci[ly] * fi[ly][k];
			
			tmpfi[uy][k] = yexp_pci[uy] * fi[uy][k]
							+ yexp_pdi[uy] * fi[uy-1][k];
		}
	}
	...
	for (j=ly; j<=uy; j++)
	{
		for (k=0; k<m; k++)
		{
			...
			f_t[lx][j][k] = xexp_pu[lx][j] * tmpf[lx+1][j][k]
						+ xexp_pc[lx][j] * tmpf[lx][j][k];
			
			f_t[ux][j][k] = xexp_pc[ux][j] * tmpf[ux][j][k]
						+ xexp_pd[ux][j] * tmpf[ux-1][j][k];
		}
	}
}
...
int							uy)
{
	static int				i, j, l, k;
	static double			dux, ddx, duy, ddy, bet;
	static double			*muxi, *varxi, *ri,
							*xexp_pui, *xexp_pci, *xexp_pdi, *ximp_pui, *ximp_pci, *ximp_pdi,
							*yexp_pui, *yexp_pci, *yexp_pdi, *yimp_pui, *yimp_pci, *yimp_pdi;

	static double			***fi, ***fi1, ***fi2, ***fi3, **fij, **fij1, **fij2, **fij3,
							***fi_plus1, **fij_plus1,
							***tmpfi, ***tmpfi1, **tmpfij, **tmpfij1, **tmpfij2, **tmpfij3;
...
/*	For each slice in x, implicit in y */
	...
	for (j=ly; j<=uy; j++)
	{
		...
		for (k=lpsi; k<=upsi; k++)
		{
			for (l=mstart; l<=mend; l++)
			{
				tmpfij[k][l] = xexp_pu[lx][j] * fij1[k][l]
									+ xexp_pc[lx][j] * fij[k][l];
				
				tmpfij1[k][l] = xexp_pc[ux][j] * fij2[k][l]
									+ xexp_pd[ux][j] * fij3[k][l];
			}
		}
	}
	...
	for (i=lx; i<=ux; i++)
	{
		...
		for (j=ly+1; j<=uy-1; j++)
		{
			...
			for (k=lpsi; k<=upsi; k++)
			{
				for (l=mstart; l<=mend; l++)
				{
					fij[k][l] = yexp_pui[j] * tmpfij1[k][l]
								+ yexp_pci[j] * tmpfij[k][l]
		...
		for (k=lpsi; k<=upsi; k++)
		{
			for (l=mstart; l<=mend; l++)
			{
				fij[k][l] = yexp_pui[ly] * tmpfij1[k][l]
							+ yexp_pci[ly] * tmpfij[k][l];
				
				fij1[k][l] = yexp_pci[uy] * tmpfij2[k][l]
								+ yexp_pdi[uy] * tmpfij3[k][l];
			}
		}
	}	

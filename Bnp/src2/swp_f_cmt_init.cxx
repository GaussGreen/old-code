...
static Err CMT_init(CMT_Obj *cmt, Date cmt_today_date,
  ...
  for (i = 0; i < num_spreads; i++) {
    /* Computes the end dates (a full CMTInstr) of the swap        , given the
     * tenor
     */
...
static Err string_to_CMTInstr(String iname, CMTInstr *instr, double *spread,
  ...
  /* Reads the tenor string and returns the number of days        , months ,
     years to be added */

  if (err = interp_tenor_string(iname, &ny, &nm, &nd))
    return err;

  instr->cms_dp.cxxashdates.start = spot;
  instr->cmt_dp.cxxashdates.start = spot;
  ...
  if (nd > 0) {
    ...
    instr->cms_dp.swapdates.cxxompd = cmt_param->cms_freq;
    ...
    instr->cmt_dp.swapdates.cxxompd = cmt_param->cmt_freq;
...
static Err CMT_set_Instr(CMT_Obj *cmt, CMTInstr *instr, int i) {
  ...
  case SWAPINSTR:
    ...
    CMT_field_Iset(cmt, CMT_SWAP_FREQ, i, instr->cmt_dp.swapdates.cxxompd);
    CMT_field_Iset(cmt, CMS_SWAP_FREQ, i, instr->cms_dp.swapdates.cxxompd);
  ...
  case CASHINSTR:
    ...
    /*
    See swp_f_ycinstr.cxx : Err YC_set_Instr
    */
  ...
  */
  ...
  if (num_swap > 0) {
    for (i = first_swap_index; i < num_swap; i++) {
      ...
      /* Populates the CMS-spread Leg of the CMS-Libor swap : fwds        ,
       * vols...*/
      crv = lookup_curve(fwd_T_name);
      ...
      /* Now that cms_libor_spread is known        , we can store cmt_libor
       * spread */
      ...
      swapdp.cxxompd = cmt_param->cmt_freq;
...

{
  ...
  /* Sets a few fields in the FwdT_obj (interp_method        , today        ,
   * spot)*/
  FwdT_Iset(fwdT, FwdT_INTERP_METHOD, cmt_param->interp_method);

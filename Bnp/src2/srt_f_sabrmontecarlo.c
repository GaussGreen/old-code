...

{
    ...
    if (sampletype == 3)
    {
        if (err = SpecTruncCube(
                cube, times_at_steps, 0.0001, 1, (long)num_paths, 1, 2, 1, (long)num_steps, &seed))
        {
    ...
    {
        blkbeta[i] =
            srt_f_optblkschbeta(forw, strike[i], sigma, maturity, beta, 1.0, SRT_CALL, SRT_PREMIUM);
    }

    for (cur_path = 1; cur_path <= num_paths; cur_path++)
    {
        if (err = monte_carlo_sabr(
                cube[cur_path],
                forw,
                vovol,
                beta,
    ...
    for (i = 1; i <= num_strikes; i++)
    {
        ...
        price[1][i] = price[1][i] / actnumpaths -
                      correlation * sqrt(stdev[1][i] - price[1][i] * price[1][i] / actnumpaths) /

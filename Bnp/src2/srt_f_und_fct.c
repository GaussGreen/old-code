...
SRT_Boolean is_model_Cheyette_type(SrtMdlType mdl_type)
{
    if ((mdl_type == LGM) || (mdl_type == CHEY) || (mdl_type == CHEY_BETA) ||
        (mdl_type == MIXED_BETA) || (mdl_type == LGM_STOCH_VOL) || (mdl_type == CHEY_STOCH_VOL) ||
...
Date get_today_from_underlying(SrtUndPtr und)
{
    ...
    if ((mdl_type == FX_STOCH_RATES) || (mdl_type == FX_LGMSV) || (mdl_type == FX_BETADLM))
    {
...
Err get_fx_underlying_currencies(SrtUndPtr fxund, String* dom_ccy, String* for_ccy)
{
    ...
    /* According to the model type, the names correspond to underlyings or curves */
...
/* creation of the bond object
  ...
  if ( und->underl_type == BOND_UND )
  {
        ...
        if (mdl_lbl!=0)
        {
                err =srt_f_interp_model(mdl_lbl, &(undbnd->mdl_type),&(undbnd->mdl_dim) );
                if (err)
                        return err;
                strcpy(undbnd->mdl_lbl,mdl_lbl) ;
        }
        else
        {

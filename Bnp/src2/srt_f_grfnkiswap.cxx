...
/* -------------------------------------------------------------------------------------------
 */
/* Cell in the first column (c[0        ,...]) of the knock in swap tableau :
 * the KI Index FRA (a[0]        ,a[1]) */
static void GrfnKiSwapIndexCell(String szIndexBasis, long lIndexNumMonths,
                                String szUndName, String szIndexRefRateCode,
                                String szIndexCell) {
  sprintf(szIndexCell,
          "FRA(a[0        ,i]        ,%d        ,\"%s\"        ,\"%s\"        "
          ",\"%s\")",
          lIndexNumMonths, szIndexBasis, szUndName, szIndexRefRateCode);
}

/* ------------------------------------------------------------------------------------------
 */
/* Cell in the second column (c[1        ,...]) of the knock in swap tableau :
 * the Call Spread for PV > 0 */
static void
GrfnKiSwapPositivePvCallSpreadCell(SrtBarrierType eKnockUpOrDown,
  ...
  if (eKnockUpOrDown == SRT_UP) {
    if (dCallSpreadWidth > 0.0) {
      sprintf(szPositivePvCallSpreadCell,
              "(MAX(c[0        ,i]-a[1        ,i]        ,0)-MAX(c[0        "
              ",i]-a[1        ,i]-%f        ,0))/%f",
              dCallSpreadWidth, dCallSpreadWidth);
    } else {
      sprintf(szPositivePvCallSpreadCell,
              "if ( c[0        ,i]> a[1        ,i]        , 1        , 0)");
    }
...
/* -----------------------------------------------------------------------------------------
 */
/* Cell in the third column (c[2        ,...]) of the knock in swap tableau :
 * the Call Spread for PV < 0 */
static void
GrfnKiSwapNegativePvCallSpreadCell(SrtBarrierType eKnockUpOrDown,
                                   double dCallSpreadWidth,
...
/* -----------------------------------------------------------------------------------------
 */
/* Cell in the fourth column (c[3        ,...]) of the knock in swap tableau :
 * the Fixed Leg Value */
static void GrfnKiSwapFixedLegPvCell(SRT_Boolean bStubPaidAtStart,
                                     String szFixedLegPvCell) {
  if (bStubPaidAtStart == SRT_NO)
    sprintf(szFixedLegPvCell, "PVRNG(6        ,7        ,a[2        ,i])-a[5   "
                              "     ,i]*df(now        ,a[3        ,i])");
  else if (bStubPaidAtStart == SRT_YES)
...
/* -----------------------------------------------------------------------------------------
 */
/* Cell in the fifth column (c[4        ,...]) of the knock in swap tableau :
 * the Float Leg Value */
static void GrfnKiSwapFloatLegPvCell(SrtReceiverType eRecPay,
                                     String szFloatLegPvCell) {
  if (eRecPay == SRT_RECEIVER)
    sprintf(szFloatLegPvCell,
            "PVRNG(8        ,9        ,a[2        ,i])-df(now        ,a[2      "
            "  ,i])*a[4        ,i]");
  else if (eRecPay == SRT_PAYER)
    sprintf(szFloatLegPvCell,
            "PVRNG(8        ,9        ,a[2        ,i])+df(now        ,a[2      "
            "  ,i])*a[4        ,i]");
}
...
/* -----------------------------------------------------------------------------------------
 */
/* Cell in the sixth column (c[5        ,...]) of the knock in swap tableau :
 * the Full Swap Value */
static void GrfnKiSwapFullSwapPvCell(String szFullSwapPvCell) {
  sprintf(szFullSwapPvCell, "c[3        ,i]+c[4        ,i]");
}

/* -----------------------------------------------------------------------------------------
 */
/* Cell in the seventh column (c[6        ,...]) of the knock in swap tableau :
 * have we knocked in ? */
static void GrfnKiSwapKnockedInCell(String szKnockedInCell) {
  sprintf(szKnockedInCell,
          "if(va>0        ,va:=2        ,va:= if(c[5        ,i]>0        ,c[1  "
          "      ,i]        ,c[2        ,i]))");
}

/* -----------------------------------------------------------------------------------------
 */
/* First Cell in the seventh column (c[6        ,...]) of the knock in swap
 * tableau : va initialisation */
static void GrfnKiSwapKnockedInFirstRowCell(String szKnockedInFirstRowCell) {
  sprintf(szKnockedInFirstRowCell, "va:= if(c[5        ,i]>0        ,c[1       "
                                   " ,i]        ,c[2        ,i])");
}
/* -----------------------------------------------------------------------------------------
 */
/* Cell in the eight column (c[7        ,...]) of the knock in swap tableau :
 * the Cash Flow */
static void GrfnKiSwapCashFlowCell(String szCashFlowCell) {
  sprintf(szCashFlowCell, "if(va>1.5        ,0        ,va*c[5        ,i])");
}
...
static Err GrfnKnockInSwapMakeTableau(
    long lNumEventDates, String szIndexBasis, long lIndexNumMonths,
    String szUndName, String szIndexRefRateCode, SrtReceiverType eRecPay,
    SrtBarrierType eKnockUpOrDown, double dCallSpreadWidth,
    SRT_Boolean bStubPaidAtStart, long *plNumRows, long *plNumCols,
  ...
  *pppsTableau = GrfnCellmatrix(*plNumRows, *plNumCols, GRFN_DEF_ARGBUFSZ);
  ...
  for (i = 0; i < *plNumRows; i++) {
    ...
    GrfnKiSwapIndexCell(szIndexBasis, lIndexNumMonths, szUndName,
                        szIndexRefRateCode, (*pppsTableau)[i][0].sval);
    (*pppsTableau)[i][0].type = GRFNSCELL;

    /* The second column: the call spread value to be used for a positive PV */
    GrfnKiSwapPositivePvCallSpreadCell(eKnockUpOrDown, dCallSpreadWidth,
                                       (*pppsTableau)[i][1].sval);
    (*pppsTableau)[i][1].type = GRFNSCELL;

    /* The third column: the call spread value to be used for a negative PV */
    GrfnKiSwapNegativePvCallSpreadCell(eKnockUpOrDown, dCallSpreadWidth,
...
static Err GrfnKnockInSwapMakeAuxiliary(
    double ***pppdAuxRanges, long **pplAuxRangesLength, long *plNumAuxColumns,
    long *plFixedStartDates, long *plFixedPayDates, long lNumFixedDates,
    double *pdFixedCoupons, double *pdNotionals, SrtBasisCode eFixedBasis,
    SrtReceiverType eRecPay, long *plFloatStartDates, long *plFloatPayDates,
    long lNumFloatDates, double dFloatMargin, String szFloatRefRate,
    SrtBasisCode eFloatBasis, double *pdKnockInLevels,
    long *plKnockInIndexStartDates, long *plKnockInSwapStartDates,
  ...
  (*pppdAuxRanges) =
      (double **)srt_malloc((*plNumAuxColumns) * sizeof(double **));
  for (i = 0; i < (*plNumAuxColumns); i++)
    (*pppdAuxRanges)[i] = NULL;
  ...
  /* Aux 4: the Corresponding Swap Notional (based on the notional for the
   * payment) */
  ...
  /* Aux 5: the Coupon Stub for the Fixed Payment (with Notional - from
   * SwapStart to KiSwapStart) */
  ...
  for (i = 0; i < (*pplAuxRangesLength)[4]; i++) {
    ...
    (*pppdAuxRanges)[5][i] = coverage(plFixedStartDates[j],
                                      plKnockInSwapStartDates[i], eFixedBasis) *
  ...
  for (i = 0; i < (*pplAuxRangesLength)[7]; i++) {
    (*pppdAuxRanges)[7][i] =
        pdFixedCoupons[i] * pdNotionals[i] *
  ...
  /* Aux 9: the equivalent Float Payments (Spreads + Margin + Notional Exchange
   * for each period) */
  (*pplAuxRangesLength)[9] = lNumFloatDates;
  (*pppdAuxRanges)[9] = dvector(0, (*pplAuxRangesLength)[9] - 1);
  if (plFixedPayDates[lNumFixedDates - 1] !=
      plFloatPayDates[lNumFloatDates - 1]) {
    FREE_GRFN_KNOCK_IN_SWAP_AUXILIARY_MEMORY;
  ...
  for (i = (*pplAuxRangesLength)[9] - 1; i >= 0; i--) {
    ...
    while (!bExitFlag) {
      if (plFixedPayDates[j] == plFloatPayDates[i]) {
        ...
        bExitFlag = SRT_YES;
      } else if (plFixedPayDates[j - 1] < plFloatPayDates[i]) {
        bExitFlag = SRT_YES;
      } else {
        j--;
      }
    ...
    dSpread =
        swp_f_spread(plFloatStartDates[i], plFloatPayDates[i], szFloatRefRate);
    dCoverage = coverage(plFloatStartDates[i], plFloatPayDates[i], eFloatBasis);

    /* The Floating Leg (REC) Representation: Notional at Beginning + Margin at
     * End - Notional at End */
    (*pppdAuxRanges)[9][i] =
        -dThisPeriodNotional +
...
Err SrtGrfnKnockInSwapPrice(
    long *plKnockInFixingDates, long *plKnockInIndexStartDates,
    long *plKnockInSwapStartDates, double *pdKnockInLevels,
    long lNumKnockInDates,

    String szIndexRefRate, String szKnockUpOrDown, double dCallSpreadWidth,
    SRT_Boolean bStubPaidAtStart,

    long *plFixedStartDates, long *plFixedPayDates, double *pdFixedCoupons,
    double *pdNotionals, long lNumFixedDates, String szFixedBasis,
    String szRecPay,

    long *plFloatStartDates, long *plFloatPayDates, long lNumFloatDates,
    double dFloatMargin, String szFloatRefRate,

    String szUndName,

    String *pszGrfnParamNames, String *pszGrfnParamValues, int iNumGrfnParams,

    /* Outputs from Grfn */
    double *pdKnockInSwapPrice, long *plNumRows, long *plNumCols,
  ...
  err = swp_f_get_ref_rate_details(szFloatRefRate, &eFloatBasis,
                                   &eFloatFrequency);
  if (err)
    return err;

  /* Gets the Index Reference Rate information : basis        , frequency */
  err = swp_f_get_ref_rate_details(szIndexRefRate, &eIndexBasis,
                                   &eIndexFrequency);
  if (err)
    return err;
  ...
  *pppsGrfnTableau = NULL;
  err = GrfnKnockInSwapMakeTableau(
      lNumKnockInDates, szIndexBasis, lIndexNumMonths, szUndName,
      szIndexRefRate, eRecPay, eKnockUpOrDown, dCallSpreadWidth,
      bStubPaidAtStart, plNumRows, plNumCols, pppsGrfnTableau);
  if (err) {
  ...
  err = GrfnKnockInSwapMakeAuxiliary(
      pppdAuxRanges, pplAuxRangesLength, plNumAuxColumns, plFixedStartDates,
      plFixedPayDates, lNumFixedDates, pdFixedCoupons, pdNotionals, eFixedBasis,
      eRecPay, plFloatStartDates, plFloatPayDates, lNumFloatDates, dFloatMargin,
      szFloatRefRate, eFloatBasis, pdKnockInLevels, plKnockInIndexStartDates,
      plKnockInSwapStartDates, lNumKnockInDates);
  if (err) {
  ...
  err = srt_f_grfn(sUndPtr, &sGrfnParams, lNumKnockInDates,
                   &plKnockInFixingDates, plNumRows, plNumCols, pppsGrfnTableau,
                   0, NULL, (*plNumAuxColumns), (*pplAuxRangesLength),
...
Err SrtGrfnKnockInSwapCalibrate(
    long *plIndexFixingDates, long *plIndexStartDates, double *pdKnockInLevels,
    long lNumDates, String szIndexRefRateCode, String szKnockUpOrDown,

    String szModelName, double dTau, SRT_Boolean bUseTwoFactor, double dAlpha,
    double dGamma, double dRho, String *pszGrfnParamNames,
    String *pszGrfnParamValues, int iNumGrfnParams, String szYieldCurveName,
    ...
    String szUndName, double **ppdCapletPrices, long *plNumCaplets) {
  Err err = NULL;
  int iNumSigmaRows;
  int iNumSigmaCols;
  ...
  for (i = 0; i < iNumInstruments; i++) {
    ...
    err = swp_f_ForwardRate(plIndexStartDates[i], plNfp[i], pszFrequency[i],
  ...
  err = SrtBootstrap(iNumGrfnParams, pszGrfnParamNames, pszGrfnParamValues,
                     &iNumInstruments, plIndexStartDates, plNfp, pszFrequency,

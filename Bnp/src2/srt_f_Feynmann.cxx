...
Err srt_f_feynmann_kac_pde(long num_mesh, long num_time_step, double dX,
                           double time_step, double *X,
                           double **conv_mat, /* CONVECTION MATRIX */
                           double **diff_mat, /* DIFFUSION MATRIX */
                           SrtPdePDEType pde_type,
                           SrtPdeBoundaryCond pde_bound_cond, double *pde_bound,
  ...
  double *diag_A = NULL, *diag_B = NULL, *diag_C = NULL, *vect_R = NULL,
         *vect_U = NULL, *PrevPrePtr = NULL, PDE_Param[4];

  if (pde_bound_cond ==
      DIRICHLET) /* FIRST CASE: THE BOUDARY CONDITIONS ARE DIRICHLET ONES */
  {
    ...
    for (i = 1; i <= num_time_step; i++) /* LOOP ON THE TIME STEP */
    {
      ...
      {
        diag_A[j] = -PDE_Param[2] * conv_mat[i][j] +
                    PDE_Param[0] * diff_mat[i][j] * diff_mat[i][j];
        diag_B[j] = -2 * PDE_Param[0] * diff_mat[i][j] * diff_mat[i][j] - 1.0;
        diag_C[j] = PDE_Param[2] * conv_mat[i][j] +
                    PDE_Param[0] * diff_mat[i][j] * diff_mat[i][j];
        ...
        if (j == 1) /* BOUNDARY CONDITION TO APPLY */
        {
          ...
          vect_R[j] = 0;
          vect_R[j] = (PDE_Param[1] * diff_mat[i - 1][j] * diff_mat[i - 1][j] +
                       PDE_Param[3] * conv_mat[i - 1][j]) *
                          PrevPrePtr[j + 1] -
                      (1.0 + 2.0 * PDE_Param[1] * diff_mat[i - 1][j] *
                                 diff_mat[i - 1][j]) *
                          PrevPrePtr[j] +
                      (PDE_Param[1] * diff_mat[i - 1][j] * diff_mat[i - 1][j] -
                       PDE_Param[3] * conv_mat[i - 1][j]) *
                          pde_bound[0] -
      ...
      for (j = 1; j < num_mesh; j++) {
        if (pde_type == LINEAR)
          PrevPrePtr[j] = vect_U[j];
        else
          PrevPrePtr[j] = max(vect_U[j], amer_payoff[i][j]);
      }
  ...
  if (diag_A)
    free_dvector(diag_A, 1, num_mesh - 1);
  diag_A = NULL;
  if (diag_B)
    free_dvector(diag_B, 1, num_mesh - 1);
  diag_B = NULL;
  if (diag_C)
    free_dvector(diag_C, 1, num_mesh - 1);
  diag_C = NULL;
  if (vect_R)
    free_dvector(vect_R, 1, num_mesh - 1);
  vect_R = NULL;
  if (vect_U)
    free_dvector(vect_U, 1, num_mesh - 1);
  vect_U = NULL;
  if (PrevPrePtr)
    free_dvector(PrevPrePtr, 1, num_mesh - 1);
  PrevPrePtr = NULL;
...
Err srt_f_black_scholes_pde(Date eval_date, Date exp_date, double spot,
                            double forward, double equity_strike,
                            double equity_vol, double min_node, double min_mesh,

                            /*OUTPUT */
                            double *bs_pv) {
  Err err = NULL;
  long i, j, k, num_mesh, num_time_step;
  double YrtoExpDate, X0, Xmin, Xmax, dXmax,
      time_step = MAX_TIME, dX, *X = NULL, *U = NULL, **diff_mat = NULL,
      **conv_mat = NULL, **div_mat = NULL, *term_payoff = NULL, pde_bound[2],
      drift;
  ...
  X0 = log(spot);
  Xmin = X0 - ONE_HALF * equity_vol * equity_vol * YrtoExpDate -
         LIM_IN_STDEV * equity_vol * sqrt(YrtoExpDate);
  Xmax = X0 - ONE_HALF * equity_vol * equity_vol * YrtoExpDate +
         LIM_IN_STDEV * equity_vol * sqrt(YrtoExpDate);
  ...
  err = srt_f_feynmann_kac_pde(num_mesh, num_time_step, dX, time_step, X,
                               conv_mat, diff_mat, pde_type, pde_bound_cond,
                               pde_bound, term_payoff, NULL, div_mat, &U);
  if (err)
    return err;
  ...
  if (X)
    free_dvector(X, 1, num_mesh - 1);
  X = NULL;
  if (U)
    free_dvector(U, 1, num_mesh - 1);
  U = NULL;
  if (diff_mat)
    free_dmatrix(diff_mat, 0, num_time_step, 1, num_mesh);
  diff_mat = NULL;
  if (conv_mat)
    free_dmatrix(conv_mat, 0, num_time_step, 1, num_mesh);
  conv_mat = NULL;
  if (div_mat)
    free_dmatrix(div_mat, 0, num_time_step, 1, num_mesh);
  div_mat = NULL;
  if (term_payoff)
    free_dvector(term_payoff, 1, num_mesh - 1);
  term_payoff = NULL;

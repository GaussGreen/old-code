...
/* Computaion of the Simpson integrand. The Simpson integrand is a U variable
 function. It is the result of a Gauss Legendre integration.  */

static double simp_func(double U, double FirstBSVol, double SecondBSVol,
                        double Gearing, double MeanLnFirstForward,
                        double MeanLnSecondForward, double Rho, double Maturity,
                        long NumPoints, SrtCallPutType Call_PutOne,
...

{
  ...
  if ((Call_PutOne == SRT_CALL) && (Call_PutTwo == SRT_CALL)) {
    ...
    if (YlimitSup < 0)
      ...
      VlimitSup = (log(YlimitSup) - MeanLnSecondForward) /
                  (sqrt(Maturity) * SecondBSVol);
    ...
    if (VlimitSup > s_VlimitInf) {
      ...
      gauleg(s_VlimitInf, VlimitSup, GaussLegAbscissas, GaussLegWeights,
             NumPoints);

      for (i = 1; i <= NumPoints; i++) {

        term = -0.5 * GaussLegAbscissas[i] * GaussLegAbscissas[i] /
                   (1 - Rho * Rho) +
...

  {
    ...
    if (YlimitInf < 0)
      ...
      VlimitInf = (log(YlimitInf) - MeanLnSecondForward) /
                  (sqrt(Maturity) * SecondBSVol);
    ...
    if (s_VlimitSup > VlimitInf) {
      ...
      gauleg(VlimitInf, s_VlimitSup, GaussLegAbscissas, GaussLegWeights,
             NumPoints);
  ...
  result = result * exp(-(0.5 / (1 - Rho * Rho)) * U * U) /
           (2 * SRT_PI * sqrt(1 - Rho * Rho));
...
{
  double result;
  result = simp_func(U, s_FirstBSVol, s_SecondBSVol, s_Gearing,
                     s_MeanLnFirstForward, s_MeanLnSecondForward, s_Rho,
                     s_Maturity, s_NumPoints, s_Call_PutOne, s_Call_PutTwo);
...

{
  ...
  /* computation of the mean of the logarithm of the forward FX rate under QT
   * (see paper) */
  ...
  /* computation of the mean of the logarithm of the forward FX rate under QX1
   * (see paper) */
  ...
  /* computation of the mean of the logarithm of the forward FX rate under QX2
   * (see paper) */
  ...
  B = (log(SecondStrike) - s_MeanLnSecondForward) /
      (SqrtMaturity * s_SecondBSVol);

  Ap = (log(FirstStrike) - MeanLnFirstForwardp) / (SqrtMaturity * s_FirstBSVol);
  Bp = (log(SecondStrike) - MeanLnSecondForwardp) /
       (SqrtMaturity * s_SecondBSVol);

  App =
      (log(FirstStrike) - MeanLnFirstForwardpp) / (SqrtMaturity * s_FirstBSVol);
  Bpp = (log(SecondStrike) - MeanLnSecondForwardpp) /
        (SqrtMaturity * s_SecondBSVol);
  ...
  if (Call_PutOne == SRT_CALL) {
    Premium = Df * srt_f_optblksch(FirstForward, FirstStrike, FirstBSVol,
                                   Maturity, 1.0, SRT_CALL, PREMIUM);
  }

  if (Call_PutOne == SRT_PUT) {
    Premium = Df * srt_f_optblksch(FirstForward, FirstStrike, FirstBSVol,
                                   Maturity, 1.0, SRT_PUT, PREMIUM);
  }
  ...
  if ((Call_PutOne == SRT_CALL) && (Call_PutTwo == SRT_CALL)) {
    ...
    Premium -=
        FirstForward * Df * (1 + bivar(Ap, Bp, Rho) - norm(Ap) - norm(Bp));
  }
  ...
  case PREMIUM:
    ...
    case SRT_BESTOFF:
      ...
      FirstOptionPrice =
          Df * srt_f_optblksch(FirstForward, FirstStrike, FirstBSVol, Maturity,
                               1.0, Call_PutOne, PREMIUM);

      SecondOptionPrice =
          Df * s_Gearing *
          srt_f_optblksch(SecondForward, SecondStrike, SecondBSVol, Maturity,
                          1.0, Call_PutTwo, PREMIUM);
  ...
  case DELTAX:
    ...
    answer = (srt_f_optworstbest(FirstForward + shift, SecondForward,
                                 FirstStrike, SecondStrike, Gearing, FirstBSVol,
                                 SecondBSVol, Rho, Df, Maturity, Call_PutOne,
                                 Call_PutTwo, Best_Worst, PREMIUM) -
              Premium) /
             shift;
    return answer;
    break;
  ...
  case DELTAY:
    ...
    answer = (srt_f_optworstbest(FirstForward, SecondForward + shift,
                                 FirstStrike, SecondStrike, Gearing, FirstBSVol,
                                 SecondBSVol, Rho, Df, Maturity, Call_PutOne,
                                 Call_PutTwo, Best_Worst, PREMIUM) -
              Premium) /
  ...
  case VEGAX:
    ...
    shift = GVOPT.vol_add;
    answer = (srt_f_optworstbest(FirstForward, SecondForward, FirstStrike,
                                 SecondStrike, Gearing, FirstBSVol + shift,
                                 SecondBSVol, Rho, Df, Maturity, Call_PutOne,
                                 Call_PutTwo, Best_Worst, PREMIUM) -
              Premium) /
             shift;
  ...
  case VEGAY:
    ...
    answer = (srt_f_optworstbest(
                  FirstForward, SecondForward, FirstStrike, SecondStrike,
                  Gearing, FirstBSVol, SecondBSVol + shift, Rho, Df, Maturity,
                  Call_PutOne, Call_PutTwo, Best_Worst, PREMIUM) -
              Premium) /
             shift;
    return (answer);
    break;
  ...
  case THETA:
    ...
    answer = (srt_f_optworstbest(
                  FirstForward, SecondForward, FirstStrike, SecondStrike,
                  Gearing, FirstBSVol, SecondBSVol + shift, Rho,
                  Df * exp(-shift * log(Df) / Maturity), Maturity - shift,
                  Call_PutOne, Call_PutTwo, Best_Worst, PREMIUM) -
              Premium) /
             shift;

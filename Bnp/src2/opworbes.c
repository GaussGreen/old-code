...
static	double simp_func(double				U,
						 double				FirstBSVol,
						 double				SecondBSVol,
						 double				Gearing,
...
double result;
	...
	if((Call_PutOne==SRT_CALL)&&(Call_PutTwo==SRT_CALL))
	{
		...
		if(YlimitSup<0)  VlimitSup=-GAUSS_LIMIT;
		else 
		{
		...
		if(VlimitSup>s_VlimitInf){
			...
			for(i=1;i<=NumPoints;i++) {
				...
				term = -0.5*GaussLegAbscissas[i]*GaussLegAbscissas[i]/(1-Rho*Rho)+(Rho/(1-Rho*Rho))*U*GaussLegAbscissas[i];
...

	{
			...
			if(YlimitInf<0)  VlimitInf = -GAUSS_LIMIT;
...
double  answer, shift;
	...
	MeanLnSecondForwardp = log(SecondForward)-0.5*Maturity*SqSecondBSVol+Rho*FirstBSVol*SecondBSVol*Maturity;
	...
	{
		Premium = Df*srt_f_optblksch(FirstForward,
								 FirstStrike,
								FirstBSVol,
								Maturity,
								1.0,
	...
	{	
		...
		case PREMIUM:
			...
			switch (Best_Worst)
			{
				...
				case SRT_BESTOFF :
					...
					FirstOptionPrice = Df*srt_f_optblksch(FirstForward,
															FirstStrike,
															FirstBSVol,
															Maturity,
															1.0,
					...
					SecondOptionPrice = Df*s_Gearing
								*srt_f_optblksch(SecondForward,
												SecondStrike,
												SecondBSVol,
												Maturity,
												1.0,
		...
		case DELTAX :
			...
			answer = (srt_f_optworstbest(FirstForward+shift,
											SecondForward,
											FirstStrike,
											SecondStrike,
											Gearing,
		...
		case DELTAY:
			...
			answer = (srt_f_optworstbest(FirstForward,
											SecondForward+shift,
											FirstStrike,
											SecondStrike,
											Gearing,

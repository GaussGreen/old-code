			 ...
			 double *vol)
{
  ...
  int exit = 0;
	...
	while ( array[j][0]<daysfromstart )
	{
        ...
        if (i==1) {
		if (j==1) *vol = array[1][1];
		else if (j>=nrow) *vol = array[nrow-1][1];
		else *vol = array[j-1][1]+(daysfromstart-array[j-1][0])/
			(array[j][0]-array[j-1][0])*
			(array[j][1]-array[j-1][1]); 
	}
	else if (i>=ncol) {
		if (j==1) *vol = array[1][ncol-1];
		else if (j>=nrow) *vol = array[nrow-1][ncol-1];
		else *vol = array[j-1][ncol-1]+(daysfromstart-array[j-1][0])/
			(array[j][0]-array[j-1][0])*
		...
		*vol = vola +
                      (exp_date - array[0][i-1])/(array[0][i]-array[0][i-1])*
		      (volb - vola);
		else              /* interpolate via (sigma^2 * t) */   
                *vol=sqrt( 
			   (  vola*vola*(array[0][i-1]-today)
						* (array[0][i]-exp_date)
			    + volb*volb*(array[0][i]-today)
						* (exp_date-array[0][i-1])
                            )
...
double vol_interpol_function(
		...
		double *und_mats_vec,
		long l_und_mats,
  		double **mkt_vol_2darray,long m_exp_dates,long m_und_mats)
{
...
**/
        if(i>0 && i<l_exp_dates && j>0 && j<l_und_mats)
        {
           ...
           interpolated_vol= tgt_vol_by_date_l + 
                             (tgt_vol_by_date_u-tgt_vol_by_date_l)*
                             (tgt_und_mat-und_mats_vec[j-1])/
                             (und_mats_vec[j]-und_mats_vec[j-1]);
        }             

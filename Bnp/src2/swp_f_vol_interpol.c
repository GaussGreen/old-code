    ...
    double*  vol)
{
    ...
    else if (i >= ncol)
    {
        if (j == 1)
            ...
            *vol = array[j - 1][ncol - 1] + (daysfromstart - array[j - 1][0]) /
                                                (array[j][0] - array[j - 1][0]) *
    ...
    else
    {
        ...
        if (method == 0) /* interpolate linearly */
            *vol = vola +
                   (exp_date - array[0][i - 1]) / (array[0][i] - array[0][i - 1]) * (volb - vola);
    ...
    long     m_und_mats)
{
    ...
    {
        interpolated_vol =
            mkt_vol_2darray[0][j - 1] + (mkt_vol_2darray[0][j] - mkt_vol_2darray[0][j - 1]) *
    ...
    if (i == l_exp_dates && j < l_und_mats && j > 0)
    {
        interpolated_vol =
            ...
            (mkt_vol_2darray[l_exp_dates - 1][j] - mkt_vol_2darray[l_exp_dates - 1][j - 1]) *
                (tgt_und_mat - und_mats_vec[j - 1]) / (und_mats_vec[j] - und_mats_vec[j - 1]);
    }

    /* boundary leftmost column, i.e. interpolating on exp_dates_vec only*/
    if (j == 0 && i < l_exp_dates && i > 0)
    {
        interpolated_vol =
            mkt_vol_2darray[i - 1][0] + (mkt_vol_2darray[i][0] - mkt_vol_2darray[i - 1][0]) *
    ...
    if (j == l_und_mats && i < l_exp_dates && i > 0)
    {
        interpolated_vol =
            ...
            (mkt_vol_2darray[i][l_und_mats - 1] - mkt_vol_2darray[i - 1][l_und_mats - 1]) *
                (tgt_exp_date - exp_dates_vec[i - 1]) / (exp_dates_vec[i] - exp_dates_vec[i - 1]);
    }

    /**     standard, interior solution, note interpolating on 2Dim array_grid
            using mat_date vector as root index vector, i.e. first interpolating
            across exp_dates_vec and then across und_mats_vec
    **/
    if (i > 0 && i < l_exp_dates && j > 0 && j < l_und_mats)
    {
        tgt_vol_by_date_l = mkt_vol_2darray[i - 1][j - 1] +
                            (mkt_vol_2darray[i][j - 1] - mkt_vol_2darray[i - 1][j - 1]) *
                                (tgt_exp_date - exp_dates_vec[i - 1]) /
                                (exp_dates_vec[i] - exp_dates_vec[i - 1]);
        tgt_vol_by_date_u =
            mkt_vol_2darray[i - 1][j] + (mkt_vol_2darray[i][j] - mkt_vol_2darray[i - 1][j]) *
                                            (tgt_exp_date - exp_dates_vec[i - 1]) /
                                            (exp_dates_vec[i] - exp_dates_vec[i - 1]);

        interpolated_vol = tgt_vol_by_date_l + (tgt_vol_by_date_u - tgt_vol_by_date_l) *
                                                   (tgt_und_mat - und_mats_vec[j - 1]) /
                                                   (und_mats_vec[j] - und_mats_vec[j - 1]);
    }

...
{
	const void *pvResult = upper_bound(
	pvKey,
	pvBegin,
	nElems,
	nSize,
	...
	if(nElems>0&&nSize==sizeof(double))
	{
		...
		_ASSERTE(pvResult == (char*)pvBegin+nElems*nSize? *(double*)((char*)pvBegin+(nElems-1)*nSize) <= *(double*)pvKey: 1);
		...
		)
{
	...
	unsigned n = ((char*)find_min_greater_than(pc,pcBegin,nElems,ncSize,dless) - (char*)pcBegin)/ncSize;
	...
	)
{
	...
	for(;(char*)pvBegin<=(char*)pvEnd-nSize;(char*)pvBegin+=nSize,(char*)pvEnd-=nSize)
	...
	)
{
	...
	for(;pv<(const char*)pvEnd;pv+=nSize)
	{
		...
		const double dR = nSize==sizeof(double)?*((double*)(pv-nSize)):*((int*)(pv-nSize));
		if(dL<dR ) return 0;
	}
	return 1;
}
...
void ** _alloca_matrix(void **pm, void *pdata, int nR_Begin, int nR_End, int nC_Begin, int nC_End,unsigned nSize)
{
...
Err _alloc_dvector_(int nBegin, int nEnd, double **ppd)
{
	...
	*ppd = dvector(nBegin, nEnd - 1);
	if (szErrMessage = check_null_ptr(*ppd)) return szErrMessage;
	return 0;
}
...
{
	const char *pInc = pcInBegin, *pIncEnd = (char*)pcInBegin + nInElems*ncSize;
	char *pInv = pInvOut;
...
{
	const char *psubc = psubcBegin, *psubcEnd = (char*)psubcBegin + nsubElems*ncSize;
		...
		)
{
	...
	for(nI = 0; nI < nMatrixSize - 1; ++nI) // NB: pdSubDiagL is one element less than nSize
	{
		...
		pdSubDiagL[nSubDiagL_Begin+nI] = pdSubDiag[nSubDiag_Begin + nI]/pdPivot[nPivot_Begin + nI];
			 ...
			 )
{
	...
	for(;plStart_Begin<plStart_End
		;++pdSwapCashRate_Begin,++plStart_Begin
		)
	{
		_ASSERTE(*plStart_Begin<=lEndDate);
		*pdSwapCashRate_Begin=swp_f_swapcashrate(*plStart_Begin,lEndDate,eBasis,eFreq,(char*)szYC,(char*)szRefRate);
	}

	return 0;
}

Err _fill_df( const char *szYieldCurve,
					 long lToday,
...
const double* _fill_tenor(long lBase,
					 const long *plDate_Begin,
					 const long *plDate_End,
					 // output
...
const long* _fill_date(long lBase,
...
const char * _alloc_dvector(
				   const double **ppd_Begin,
				   int nSize
				   )
{
	*ppd_Begin = 0;
...
{
	if(pPointer) free((void*)pPointer);
	pPointer = 0;
}

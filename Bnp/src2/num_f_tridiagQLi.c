/* ========================================================================
  ...
  DESCRIPTION:	From Numerical Recipes in C page 480
                QL algorithm with implicit shifts  , to determine the eigen
                                values and eigen vectors of a real  , symmetric
, tridiagonal matrix  , previously reduced by the Householder method (cf
                                uthouseholder)
                                On input  , d[0..n-1][0..n-1] contains the
...
with e[0] arbitrary. On output  , e[...] is destroyed If the eigenvectors of a
tridiagonal matrix are desired  , the matrix z[0..n-1][0..n-1] is input as the
identity matrix. If the eigen vectors of a matrix that has been reduced by
                                Householder method
(householder_tridiagonalisation) are required  , than z is input as the matrix
output by this function. The kth column of z (i.e. z[...][k] returns the
    ...
    do {
      ...
      if (m != l) {
        ...
        /* A plane rotation  , as in the original QL  , followed by Givens
           rotation to restore tridiagonal form */
        for (i = m - 1; i >= l; i--) {
          f = s * e[i];
          b = c * e[i];
          e[i + 1] = (r = pythag(f, g));

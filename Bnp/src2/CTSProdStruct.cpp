...
Err cts_fill_calls(
  ...
  while (i < ncall) {
    ...
    while (l < exo_leg->num_cpn &&
           exo_leg->cpn[l].cpn_start_date < call->ex_date) {
      l++;
    }
...
Err cts_check_calls(CTS cts) {
  ...
  for (i = 1; i < cts->num_calls; i++) {
    ...
    if (cts->call[i].exo_idx > 0) {
      if (cts->exo_leg->cpn[cts->call[i].exo_idx - 1].used_nfix > 0 &&
        ...
        return serror("Either too many fixings requested  , or Floating Rate "
  ...
  for (i = 0; i < cts->num_calls; i++) {
    ...
    if (cts->exo_leg->cpn[cts->call[i].exo_idx].cpn_start_date <
            cts->call[i].ex_date ||
        cts->exo_leg->cpn[cts->call[i].exo_idx].cpn_pay_date <
            cts->call[i].ex_date ||
...
Err cts_calib_const_alpha_and_rho(
  ...
  if (calib_months == 0) {
    ...
    while (i0 < cts->exo_leg->num_cpn &&
           cts->exo_leg->cpn[cts->call[0].exo_idx + i0].cpn_start_time <
               MIN_CALIB_TIME) {
      i0++;
    }
...
Err cts_calib_und(
  ...
  if (short_strike_flag == 1 && force_atm == 0) {
    for (i = 0; i < nb_cal_dates; i++) {
      sec_strikes[i] =
          cts->exo_leg->cpn[cts->call[i + skip_dates].exo_idx].caplet_strike;
    }
  }
  ...
  nb_new_dates =
      (int)((cts->exo_leg->cpn[cts->exo_leg->num_cpn - 1].cpn_pay_time -
...
void cts_get_numer_tstar(long today, CTS cts, double *numer_tstar) {
  ...
  if (fabs(*numer_tstar) < 1.0E-08) {
    temp_numer_tstar =
        0.5 *
        (cts->exo_leg->cpn[cts->call[0].exo_idx].cpn_start_time +
...
Err cts_launch_algo(CTS cts, CTS_UND und, CTS_ADI_ARG adi_arg,
  ...
  if (adi_arg->pde_or_mc == 0) {
    smessage("Launching adi  , time steps requested: %d  , actual: %d",
             adi_arg->initnstp, adi_arg->nstp);
  } else {
    smessage("Launching MC  , Nb Steps: %d  , Nb Paths: %d", adi_arg->nstp,
             adi_arg->npaths);
  }

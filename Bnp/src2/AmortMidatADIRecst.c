		...
		)
{
	...
	*pdConstant = -dHalfBondVar - *pdCoeff1*dExp_R1- *pdCoeff2*dExp_R2;
	...
	_ASSERTE(
		...
		(dPHI1_t>0. && dPHI2_t==0.&&dPHICross_t==0.) // 1f case with 2nd dimension suppressed
		) ;
}
		...
		)
{
	...
	_ASSERTE(dLAM1_t_T>0 && dLAM2_t_T>0.);
	_ASSERTE((dPHI1_t>0. && dPHI2_t>0.&&dPHICross_t>0.) || (dPHI1_t==0. && dPHI2_t==0.&&dPHICross_t==0.)) ;
		...
		)
{
	...
	_ASSERTE(dDF_0_t >= dDF_0_T);	
	if(dDF_0_t == dDF_0_T) return 1.;
	return __DF_nojump_(dX1,dX3,dDF_0_t,dDF_0_T,dLAM1_0_t,dLAM2_0_t,dLAM1_0_T,dLAM2_0_T,dEILam1_t,dEILam2_t,dPHI1_t,dPHI2_t,dPHICross_t,pNumeraire,dRho,dAlphaRho,dAlpha_Sqrt1mRhoSqrd);
}
		...
		)
{
...

	{
		...
		double dDF_Base = exp(dConstant-(*pdX1_Begin)*dCoeff1-dX2_Base*dCoeff2) * dBondRatio;

/* ===================================================================================
   FILENAME:      DoubleTriggerSwapQuantoLeg.cxx / LY J-M
...
// Prices a double put with payoff E[max(strikeX - X_fixed_at_timeX        ,0) *
// max(strikeY - Y_fixed_at_timeY        ,0) where X and Y are both lognormals
// with fwds fwdX and fwdY and vols sigmaX and sigmaY and instantaneous
// correlation rhoXY X is fixed at timeX and Y is fixed at time Y.
Err double_put_lognormal(double x0, double B, double sigma_x, double T_x,
                         double y0, double K, double sigma_y, double T_y,
                         double correl, double *result) {
  Err err;
  double e_x_1, e_y_1, e_x_2, e_y_2, e_x_3, e_y_3, e_x_4, e_y_4;
  double bivar1, bivar2, bivar3, bivar4, new_correl;

  if (T_x <= 0.0 || T_y <= 0.0 || B <= 0.0 || K <= 0.0 || x0 <= 0.0 ||
      y0 <= 0.0) {
    err = "double_put_lognormal: ERROR (maturities <= 0.0 OR strikes <= 0.0 OR "
          "fwds <= 0.0)";
    goto FREE_RETURN;
  }
  ...
  e_x_1 =
      1 / (sigma_x * sqrt(T_x)) * (log(B / x0) + 0.5 * sigma_x * sigma_x * T_x);
  e_y_1 =
      1 / (sigma_y * sqrt(T_y)) * (log(K / y0) + 0.5 * sigma_y * sigma_y * T_y);
  e_x_2 = 1 / (sigma_x * sqrt(T_x)) *
          (log(B / x0) - correl * sigma_x * sigma_y * min(T_x, T_y) +
           0.5 * sigma_x * sigma_x * T_x);
  e_y_2 =
      1 / (sigma_y * sqrt(T_y)) * (log(K / y0) - 0.5 * sigma_y * sigma_y * T_y);
  e_x_3 =
      1 / (sigma_x * sqrt(T_x)) * (log(B / x0) - 0.5 * sigma_x * sigma_x * T_x);
  e_y_3 = 1 / (sigma_y * sqrt(T_y)) *
          (log(K / y0) - correl * sigma_x * sigma_y * min(T_x, T_y) +
           0.5 * sigma_y * sigma_y * T_y);
  e_x_4 = 1 / (sigma_x * sqrt(T_x)) *
          (log(B / x0) - correl * sigma_x * sigma_y * min(T_x, T_y) -
           0.5 * sigma_x * sigma_x * T_x);
  e_y_4 = 1 / (sigma_y * sqrt(T_y)) *
          (log(K / y0) - correl * sigma_x * sigma_y * min(T_x, T_y) -
           0.5 * sigma_y * sigma_y * T_y);
...
Err double_inf_barrier_lognormal(double fwdX, double B1, double volX_B1,
                                 double B2, double volX_B2, double timeX,
                                 double fwdY, double K1, double volY_K1,
                                 double K2, double volY_K2, double timeY,
                                 double rhoXY, double *result) {
  Err err = NULL;
  ...
  err = double_put_lognormal(fwdX, B2, volX_B2, timeX, fwdY, K2, volY_K2, timeY,
                             rhoXY, &c1);
  if (err)
    goto FREE_RETURN;

  err = double_put_lognormal(fwdX, B2, volX_B2, timeX, fwdY, K1, volY_K1, timeY,
                             rhoXY, &c2);
  if (err)
    goto FREE_RETURN;

  err = double_put_lognormal(fwdX, B1, volX_B1, timeX, fwdY, K2, volY_K2, timeY,
                             rhoXY, &c3);
  if (err)
    goto FREE_RETURN;

  err = double_put_lognormal(fwdX, B1, volX_B1, timeX, fwdY, K1, volY_K1, timeY,
                             rhoXY, &c4);
  if (err)
    goto FREE_RETURN;
...
Err double_trigger_swap_quanto_cms_rate_quanto(
    ...
    char *cms_vc_name, int n_cms_strikes_in_vol, double *cms_strikes_in_vol,
    int cms_dom_for,
  ...
  Err err = NULL;
  // double cms        , cms_atm_log_vol        , cms_quanto_correlation ,
  // fwd_fx_vol        , flat_vol        , temp_vol;
  double cms = 0.0;

  if (use_CMSRate_CMSTEC == 0) {
    // Call the old functions to calculate the CMS Quanto
    digital_quanto_float_leg_cms_rate_quanto(
        fwd, cms_fixing_time, cms_start_time, cms_nfp,

        cms_compounding, cms_delay, cms_dRateConv,

        cms_vol_type, cms_flat_vol_or_smile_approx, cms_start_date,
        cms_theo_end_date, cms_cash_vol, fwd_spread,

        cms_vc_name, n_cms_strikes_in_vol, cms_strikes_in_vol, cms_dom_for,

        cms_quanto_corr_times, cms_quanto_corr_values, n_cms_quanto_corr,

        fwd_fx_vols_times, fwd_fx_vols_values, n_fwd_fx_vols,
...
Err double_trigger_swap_quanto_leg(
    ...
    double *exo_cms1_barriers, long *exo_cms2_fixing_dates,
    double *exo_cms2_barriers, double *exo_m12, double *exo_g2, double *exo_m2,
    double *exo_g3, double *exo_m3, double *exo_g4_X, double *exo_g4_Y,
    double *exo_m4, double *exo_cms1_past_fixings,
    ...
    int pay_rec, double call_spread, int use_CMSRate_CMSTEC, int eod_fix_flag,
    int eod_pay_flag,

    double *cpn1, double *cpn2, double *cpn3, double *cpn4,
  ...
  double cms1_ats_log_vol, cms1_B1_log_vol, cms1_B2_log_vol, cms2_ats_log_vol,
      cms2_K1_log_vol, cms2_K2_log_vol;
  double cms1_numeraire_adjustment = 1.0, cms2_numeraire_adjustment = 1.0,
  ...
  // Results
  double exotic_leg1_PV = 0.0, exotic_leg2_PV = 0.0, exotic_leg3_PV = 0.0,
         exotic_leg4_PV = 0.0;
  double exotic_coupon1_PV = 0.0, exotic_coupon2_PV = 0.0,
         exotic_coupon3_PV = 0.0, exotic_coupon4_PV = 0.0;
  ...
  while (min(exo_cms1_fixing_dates[i], exo_cms2_fixing_dates[i]) <
    ...
    if ((exo_cms1_fixing_dates[i] < today + eod_fix_flag) &&
      ...
      if ((exo_cms1_past_fixings[i] < exo_cms1_barriers[i]) &&
      ...
      } else if ((exo_cms1_past_fixings[i] >= exo_cms1_barriers[i]) &&
                 (exo_cms2_past_fixings[i] < exo_cms2_barriers[i])) {
        ...
        exotic_coupon4_PV =
            0.5 * (exo_g4_X[i] * exo_cms1_past_fixings[i] +
                   exo_g4_Y[i] * exo_cms2_past_fixings[i] + exo_m4[i]);
      }
    } else {
      err = "Handling of past fixings where one of the CMS is fixed in advance "
            "and the other one in arrears is not yet available";
      goto FREE_RETURN;
    }

    /*
            //One asset fixed in the case of a Digital Quanto Float TO BE
       REWRITTEN if DoubleTipTop with fixing in advance and in arrears else
            {
                    //CMS1 has fixed
                    if (exo_cms1_fixing_dates[i] < today + eod_fix_flag)
                    {
                            //Payoff is a forward CMS2 + margin (quanto
       corrected if needed) if (((exo_cms1_past_fixings[i] > exo_barriers[i]) &&
       (above_below == 0)) ||
                                    ((exo_cms1_past_fixings[i] <
       exo_barriers[i])  && (above_below == 1)) ||
                                    ((exo_cms1_past_fixings[i] ==
       exo_barriers[i]) && (accrue_on_barrier == 1)))
                            {
                                    //Calculates the start and theo end date of
       the CMS2 underlying cms2_start_date = add_unit(exo_cms2_fixing_dates[i]
             ,cms2_spot_lag        , SRT_BDAY        , MODIFIED_SUCCEEDING);
       if(cms2_tenor_unit == 'M')
                                    {
                                            cms2_theo_end_date =
       add_unit(cms2_start_date        , cms2_tenor_num        , SRT_MONTH ,
       NO_BUSDAY_CONVENTION);
                                    }
                                    else
                                    {
                                            cms2_theo_end_date =
       add_unit(cms2_start_date        , cms2_tenor_num        , SRT_YEAR ,
       NO_BUSDAY_CONVENTION);
                                    }

                                    //Calculates number of full periods for the
       underlying of CMS2 cms2_fixing_time = (exo_cms2_fixing_dates[i] - today)
       * YEARS_IN_DAY; cms2_start_time = (cms2_start_date - today) *
       YEARS_IN_DAY; cms2_delay = (exo_pay_dates[i] - cms2_start_date) *
       YEARS_IN_DAY; cms2_nfp = ((int) ((cms2_theo_end_date - cms2_start_date) *
       YEARS_IN_DAY * cms2_compounding + 0.1)) * 1.0; fwd_spread2 =
       swp_f_spread(cms2_start_date        , cms2_theo_end_date        ,
       cms2_refrate);
                                    ...
                                    err =
       digital_quanto_float_leg_cms_rate_quanto(fwd2        , cms2_fixing_time ,
                                                                                                                               cms2_start_time        ,
                                                                                                                               cms2_nfp        ,
                                                                                                                               cms2_compounding        ,
                                                                                                                               cms2_delay        ,
                                                                                                                               ...
                                                                                                                               &cms2);
                                    if(err) goto FREE_RETURN;
                                    exotic_coupon_PV = exo_gearings[i] * cms2 +
       exo_margins[i];
                            }
                            else
                    ...
                    {
                            //Calculates the start and theo end date of the CMS1
       underlying cms1_start_date = add_unit(exo_cms1_fixing_dates[i]
             ,cms1_spot_lag        , SRT_BDAY        , MODIFIED_SUCCEEDING);
       if(cms1_tenor_unit == 'M')
                            {
                                    cms1_theo_end_date =
       add_unit(cms1_start_date        , cms1_tenor_num        , SRT_MONTH ,
       NO_BUSDAY_CONVENTION);
                            }
                            else
                            {
                                    cms1_theo_end_date =
       add_unit(cms1_start_date        , cms1_tenor_num        , SRT_YEAR ,
       NO_BUSDAY_CONVENTION);
                            }

                            //Calculates number of full periods for the
       underlying of CMS1 cms1_fixing_time = (exo_cms1_fixing_dates[i] - today)
       * YEARS_IN_DAY; cms1_start_time = (cms1_start_date - today) *
       YEARS_IN_DAY; cms1_delay = (exo_pay_dates[i] - cms1_start_date) *
       YEARS_IN_DAY; cms1_nfp = ((int) ((cms1_theo_end_date - cms1_start_date) *
       YEARS_IN_DAY * cms1_compounding + 0.1)) * 1.0; fwd_spread1 =
       swp_f_spread(cms1_start_date        , cms1_theo_end_date        ,
       cms1_refrate);
       ...
       cms1_refrate        , &fwd1); if(err) goto FREE_RETURN;

                            //Calculates the CMS Rate
                            err = digital_quanto_float_leg_cms_rate_quanto(fwd1
             , cms1_fixing_time        , cms1_start_time        , cms1_nfp ,
       cms1_compounding        , cms1_delay        , cms1_dRateConv        ,
                                                                                                                       cms1_vol_type        ,
                                                                                                                       cms_flat_vol_or_smile_approx        ,
                                                                                                                       cms1_start_date        ,
                                                                                                                       cms1_theo_end_date        ,
                            ...
                            //CMS1 ATS B log vol
                            err =
       digital_quanto_float_leg_get_lognormal_vol(cms1_vc_name        , fwd1 ,
                                                                                                                             cms1_fixing_time        ,
                                                                                                                             cms1_start_date        ,
                                                                                                                             cms1_theo_end_date        ,
                                                                                                                             exo_barriers[i]        ,
                            ...
                            //Calculation of the strike for the shifted option
       (B+dB or B-dB) if(above_below == 1)
                            {
                                    if(pay_rec == 0)
                                            shifted_strike = exo_barriers[i] +
       call_spread; else shifted_strike = exo_barriers[i] - call_spread;
                            }
                            else
                            {
                                    if(pay_rec == 0)
       ...
       digital_quanto_float_leg_get_lognormal_vol(cms1_vc_name        , fwd1 ,
                            ...
                            exotic_coupon_PV = exo_gearings[i] *
       exo_cms2_past_fixings[i] + exo_margins[i]; if(above_below == 1) //CMS1<B
                            {
                                    if(pay_rec == 0) //PAY
                                            exotic_coupon_PV *= -
       (put_margin_B_shift -  put_margin_B) / call_spread; else //REC
                                            exotic_coupon_PV *= (put_margin_B -
       put_margin_B_shift) / call_spread;
                            }
                            else //CMS1>B
                            {
                                    if(pay_rec == 0) //PAY
                                            exotic_coupon_PV *= - 1 + (put_B +
       put_margin_B - put_B_shift - put_margin_B_shift) / call_spread; else
       //REC exotic_coupon_PV *= 1 - (put_B_shift + put_margin_B_shift - put_B -
       put_margin_B) / call_spread;
                            }
                    }
            }*/
  ...
  while (i < n_exo_coupons) {
    ...
    err = double_trigger_swap_quanto_cms_rate_quanto(
        ...
        cms1_delay, cms1_dRateConv, cms1_vol_type, 1, cms1_start_date,
        cms1_theo_end_date, cms1_cash_vol, fwd_spread1, cms1_vc_name,
        n_cms1_strikes_in_vol, cms1_strikes_in_vol, cms1_dom_for,
        cms1_quanto_corr_times, cms1_quanto_corr_values, n_cms1_quanto_corr,
        fwd_fx_vols_times, fwd_fx_vols_values, n_fwd_fx_vols,
        use_CMSRate_CMSTEC, &cms1);
    if (err)
      goto FREE_RETURN;

    err = double_trigger_swap_quanto_cms_rate_quanto(
        fwd2, cms2_fixing_time, cms2_start_time, cms2_nfp, cms2_compounding,
        cms2_delay, cms2_dRateConv, cms2_vol_type, 1, cms2_start_date,
        cms2_theo_end_date, cms2_cash_vol, fwd_spread2, cms2_vc_name,
        n_cms2_strikes_in_vol, cms2_strikes_in_vol, cms2_dom_for,
        cms2_quanto_corr_times, cms2_quanto_corr_values, n_cms2_quanto_corr,
        fwd_fx_vols_times, fwd_fx_vols_values, n_fwd_fx_vols,
        use_CMSRate_CMSTEC, &cms2);
    if (err)
      goto FREE_RETURN;
    ...
    err = digital_quanto_float_leg_get_lognormal_vol(
        ...
        cms1_theo_end_date, B1, &cms1_B1_log_vol);
    if (err)
      goto FREE_RETURN;

    err = digital_quanto_float_leg_get_lognormal_vol(
        cms1_vc_name, fwd1, fwd_spread1, cms1_fixing_time, cms1_start_date,
        cms1_theo_end_date, B2, &cms1_B2_log_vol);
    if (err)
      goto FREE_RETURN;

    err = digital_quanto_float_leg_get_lognormal_vol(
        cms2_vc_name, fwd2, fwd_spread2, cms2_fixing_time, cms2_start_date,
        cms2_theo_end_date, K1, &cms2_K1_log_vol);
    if (err)
      goto FREE_RETURN;

    err = digital_quanto_float_leg_get_lognormal_vol(
        cms2_vc_name, fwd2, fwd_spread2, cms2_fixing_time, cms2_start_date,
        cms2_theo_end_date, K2, &cms2_K2_log_vol);
    if (err)
      goto FREE_RETURN;
    ...
    err = constant_or_linear_interpolation(
        ...
        LINEAR_METHOD, min(cms1_fixing_time, cms2_fixing_time),
        &cms1_cms2_correlation);

    // Price the first coupon
    err = double_inf_barrier_lognormal(
        cms1, B1, cms1_B1_log_vol, B2, cms1_B2_log_vol, cms1_fixing_time, cms2,
        K1, cms2_K1_log_vol, K2, cms2_K2_log_vol, cms2_fixing_time,
        cms1_cms2_correlation, &exotic_coupon1_PV);
    if (err)
      goto FREE_RETURN;
    ...
    // For the numeraire adjustments        , the vols are taken at
    // exo_cms1_barriers and exo_cms2_barriers
    err = digital_quanto_float_leg_get_lognormal_vol(
        cms1_vc_name, fwd1, fwd_spread1, cms1_fixing_time, cms1_start_date,
        cms1_theo_end_date, exo_cms1_barriers[i], &cms1_ats_log_vol);
    if (err)
      goto FREE_RETURN;

    err = digital_quanto_float_leg_get_lognormal_vol(
        cms2_vc_name, fwd2, fwd_spread2, cms2_fixing_time, cms2_start_date,
        cms2_theo_end_date, exo_cms2_barriers[i], &cms2_ats_log_vol);
    if (err)
      goto FREE_RETURN;
    ...
    put2 = srt_f_optblksch(cms2_numeraire_adjustment, K2, cms2_K2_log_vol,
                           cms2_fixing_time, 1.0, SRT_PUT, SRT_PREMIUM);

    put1 = srt_f_optblksch(cms2_numeraire_adjustment, K1, cms2_K1_log_vol,
                           cms2_fixing_time, 1.0, SRT_PUT, SRT_PREMIUM);

    exotic_coupon2_PV = exo_g2[i] * cms1 * (put2 - put1) / call_spread;

    err = double_inf_barrier_lognormal(
        cms1_numeraire_adjustment, B1, cms1_B1_log_vol, B2, cms1_B2_log_vol,
        cms1_fixing_time, cms2_numeraire_adjustment, K1, cms2_K1_log_vol, K2,
        cms2_K2_log_vol, cms2_fixing_time, cms1_cms2_correlation,
        &double_inf_barrier);
    if (err)
      goto FREE_RETURN;
    ...
    put2 = srt_f_optblksch(cms2, K2, cms2_K2_log_vol, cms2_fixing_time, 1.0,
                           SRT_PUT, SRT_PREMIUM);

    put1 = srt_f_optblksch(cms2, K1, cms2_K1_log_vol, cms2_fixing_time, 1.0,
                           SRT_PUT, SRT_PREMIUM);
    ...
    err = double_inf_barrier_lognormal(
        cms1, B1, cms1_B1_log_vol, B2, cms1_B2_log_vol, cms1_fixing_time, cms2,
        K1, cms2_K1_log_vol, K2, cms2_K2_log_vol, cms2_fixing_time,
        cms1_cms2_correlation, &double_inf_barrier);
    if (err)
      goto FREE_RETURN;
    ...
    put2 = srt_f_optblksch(cms1_numeraire_adjustment, B2, cms1_B2_log_vol,
                           cms1_fixing_time, 1.0, SRT_PUT, SRT_PREMIUM);

    put1 = srt_f_optblksch(cms1_numeraire_adjustment, B1, cms1_B1_log_vol,
                           cms1_fixing_time, 1.0, SRT_PUT, SRT_PREMIUM);
    ...
    put2 = srt_f_optblksch(cms1, B2, cms1_B2_log_vol, cms1_fixing_time, 1.0,
                           SRT_PUT, SRT_PREMIUM);

    put1 = srt_f_optblksch(cms1, B1, cms1_B1_log_vol, cms1_fixing_time, 1.0,
                           SRT_PUT, SRT_PREMIUM);

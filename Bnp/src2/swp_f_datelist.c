...
SrtDateList SwapDP_to_DateList(SrtSwapDP* swap, SrtBusDayConv conv)
{
    ...
    case FWD:
        if (swap->start == swap->first_full_fixing)
        {
            ...
            cur_date =
                add_unit(count_date, (int)(12 / swap->compd), SRT_MONTH, NO_BUSDAY_CONVENTION);
            k = 1;
        }
        else
        {
        ...
        for (i = 1; i <= swap->nfp; i++)
        {
            ...
            cur_date = add_unit(
                count_date, (int)(12 / swap->compd) * (i + k), SRT_MONTH, NO_BUSDAY_CONVENTION);
        }
    ...
    case BKWD:
        ...
        for (i = 0; cur_date >= swap->start; i++)
        {
            ...
            cur_date = add_unit(
                count_date, -(int)(12 / swap->compd) * (i + 1), SRT_MONTH, NO_BUSDAY_CONVENTION);
        }
    ...
    {
        list.prev = add_unit(
            list.date[list.len - 1],
            -(12 / swap->compd) * list.len,
            SRT_MONTH,

...
/* ----------------------------------------------------------------------------- 
  PURPOSE:	From THEORETICAL (i.e. non holidays adjusted) start and
			end dates (as payment -or start of accrual period- dates),
			generates a list of EFFECTIVE payment (or start of accrual period)
			dates, that are business days adjusted according to SrtBusDayConv
			for a swap with periodicity compd

  AMENDMENT: E.Auld 5 Jan 94 --- added bus_day_start.
			previously, swap->start was used, which caused an error
			when swap->start was not a business day.
  ----------------------------------------------------------------------------- */
	...
	switch(swap->direction)
	{
		case FWD:
			if (swap->start==swap->first_full_fixing)
			{
				...
				cur_date=add_unit(count_date,(int)(12/swap->compd),SRT_MONTH,NO_BUSDAY_CONVENTION);
				k=1;
			}
			else
			{
			...
			{
				/* The effective period start (for accrual coupon..) is the modified theoretical date */
				list.date[i] = cur_date;
				/* The next theoretical payment date (== period start for accrual) is non modified */
				cur_date=add_unit(count_date,(int)(12/swap->compd)*(i+k),SRT_MONTH,NO_BUSDAY_CONVENTION);
			}
		...
		case BKWD:
		    ...
		    for(i=0;cur_date>=swap->start;i++)
			{
				...
				cur_date = add_unit(count_date,-(int)(12/swap->compd)*(i+1),SRT_MONTH,NO_BUSDAY_CONVENTION);
			}
		...
		/* Here, cur_date is before swap->start: check the next one to see
			...
			if (add_unit(cur_date,(int)(12/swap->compd),SRT_MONTH,NO_BUSDAY_CONVENTION)
							> bus_day_start )
			{
				d[i] = bus_day_start;
				length +=1;
		...
		/* Checks that applying business day rule will not set d[length-2] = d[length-1] */
			if (bus_date_method (d[length-2], conv) == bus_date_method (d[length-1], conv))
			{
				length--;
			}
...
/* Gets the theoretical previous payment date from start or first full coupon */
	...
	{
		list.prev = add_unit(list.date[list.len-1],-(12/swap->compd)*list.len,
							SRT_MONTH,NO_BUSDAY_CONVENTION);
	}
	else 
	{
		list.prev = add_unit(list.date[list.len-1],-(12/swap->compd)*(list.len-1),
							SRT_MONTH,NO_BUSDAY_CONVENTION);
	}
...
SrtDateList DateListafter(SrtDateList dl, Date after)
{
	...
	while(i<dl.len && dl.date[i] <= after)i++;	
	ndl.len = dl.len - i;

...
Err payoff_fx_sabr_adi(double evt_date, double evt_time, void *func_parm,
  ...
  df_ratio = swp_f_df(today, evt_date, (char *)for_yc) /
             swp_f_df(today, evt_date, (char *)dom_yc);

  if (total->num_fx_df > 0) {
    for (k = 0; k < total->num_fx_df; k++) {
      local->evt->df[total->fx_idx][k] =
          swp_f_df(evt_date, total->fx_df_dts[k], (char *)dom_yc);
    }
  }
  ...
  if (total->num_dom_df > 0) {
    for (k = 0; k < total->num_dom_df; k++) {
      local->evt->df[total->dom_idx][k] =
          swp_f_df(evt_date, total->dom_df_dts[k], (char *)dom_yc);
    }
  }
  ...
  if (total->num_for_df > 0) {
    for (k = 0; k < total->num_for_df; k++) {
      local->evt->df[total->for_idx][k] =
          swp_f_df(evt_date, total->for_df_dts[k], (char *)for_yc);
    }
  }
...
Err payoff_fx_sabr_adi_opt(double evt_date, double evt_time, void *func_parm,
  ...
  mat_date =
      add_unit((long)(evt_date + 1.0E-08), 2, SRT_BDAY, MODIFIED_SUCCEEDING);

  strike = (double *)func_parm;
  df_ratio = swp_f_df(today, mat_date, (char *)for_yc) /
             swp_f_df(today, mat_date, (char *)dom_yc);

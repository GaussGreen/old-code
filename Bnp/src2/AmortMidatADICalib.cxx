/*-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+--+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-
        ...
        Description	: implementation of the calibration of Callable
Amortizing/Accreting Swaption (CAS).
...
void _init_Calib_LamTS_DiagEuro_Comm(
  ...
  // pCalib_LamTS_DiagEuro->m_pParam_Swaption=calloc(1
  //       ,sizeof(cpd_diag_calib_param));
  // pCalib_LamTS_DiagEuro->m_pParam_Caplet=calloc(1
  //       ,sizeof(cpd_diag_calib_param));
  // pCalib_LamTS_DiagEuro->m_pCalibParam=calloc(1
  //       ,sizeof(diag_calib_lm_params));
  ...
  *(int *)(&pCalib_LamTS_DiagEuro->m_nOnefequi) = nOnefequi;
  ...
  Calibrate_MidAt_Preprocess(
      ...
      plFixStart_Begin, plFixStart_End, plFixEnd_Begin, plFixPay_Begin,
      plFltStart_Begin, plFltStart_End, plFltEnd_Begin, plFltPay_Begin,
      pdMargin_Begin, pdSpread_Begin,
...
void _free_Calib_LamTS_DiagEuro_Comm(
    _Calib_LamTS_DiagEuro_Comm *pCalib_LamTS_DiagEuro) {
  const int nSize_Ex =
      pCalib_LamTS_DiagEuro->m_plEx_End - pCalib_LamTS_DiagEuro->m_plEx_Begin;
  const char **pszSwapTenor = pCalib_LamTS_DiagEuro->m_pszSwapTenor;
  const char **pszCapTenor = pCalib_LamTS_DiagEuro->m_pszCapTenor;
  int nI;

  free((double *)pCalib_LamTS_DiagEuro
           ->m_pdSwapStrike); // = calloc(nSize_Ex        ,sizeof(double));
  free((double *)pCalib_LamTS_DiagEuro
           ->m_pdCapStrike); //=calloc(nSize_Ex        ,sizeof(double));
  // free((cpd_diag_calib_param*)pCalib_LamTS_DiagEuro->m_pParam_Swaption);//=calloc(1
  //       ,sizeof(cpd_diag_calib_param));
  // free((cpd_diag_calib_param*)pCalib_LamTS_DiagEuro->m_pParam_Caplet);//=calloc(1
  //       ,sizeof(cpd_diag_calib_param));
  // free((diag_calib_lm_params*)pCalib_LamTS_DiagEuro->m_pCalibParam);//=calloc(1
  //       ,sizeof(diag_calib_lm_params));
...
static int _initialize_ptr_TS_(const double *pdIn_Begin, const double *pdIn_End,
                               double dKey) {
  const double *pd = find_min_greater_than(
      &dKey, pdIn_Begin, pdIn_End - pdIn_Begin, sizeof(dKey), dless);
...
static void
_initialize_ptr_TS(const double *pdZetaT_Begin, const double *pdZetaT_End,
                   const double *pdGT_Begin, const double *pdGT_End,
  ...
  // zeta at exercise
  *(const double **)(&pInst->m_pdZeta_Ex) =
      pdZeta + _initialize_ptr_TS_(pdZetaT_Begin, pdZetaT_End, *pdKey);
  // G at exercise
  *(const double **)(&pInst->m_pdG_Ex) =
      pdG + _initialize_ptr_TS_(pdGT_Begin, pdGT_End, *pdKey);

  // G at cash flow dates
  for (pdKey = pInst->m_pdT_CashFlow_Begin; pdKey < pInst->m_pdT_CashFlow_End;
       ++pdKey, ++pdG_CashFlow) {
...
static const char *_Calibrate_(
    ...
    const double *pdGT_Begin, const double *pdGT_End,
    // Results
    double *pdZeta_Begin, double *pdG_Begin) {
  ...
  // global term structure
  double *pdTS = memset(_alloca(n * sizeof(double)), 0, n * sizeof(double)),
         dfsumsq = 0.;
  ...
  for (; ppInst < ppInst_end; ++ppInst) {
    for (pInst = *ppInst; pInst < *(ppInst + 1); ++pInst) {
      _initialize_ptr_TS(pdZetaT_Begin, pdZetaT_End, pdGT_Begin, pdGT_End, pdTS,
                         nZeta_Begin, nG_Begin, (_GenEuropean *)pInst);
    }
  }
  ...
  // delegate to nag ...
  nag_opt_lsq_no_deriv(m, n, _lsqfunc_, pdTS, &dfsumsq, pdf, pdjac, n,
                       E04_DEFAULT, &comm, &fail);
  ...
  // assign to results
  memcpy(pdZeta_Begin, pdTS + nZeta_Begin,
         (nNumZeta - nZeta_Begin) * sizeof(double));
...
static double _Price_GenEuroBlock(const _GenEuropean *pGenEuroBlock_Begin,
  ...
  for (; pInst < pGenEuroBlock_End; ++pInst) {
    dPrice +=
        lgmopval1F(pInst->m_pdT_CashFlow_End - pInst->m_pdT_CashFlow_Begin,
                   (double *)pInst->m_pdCashFlow_Begin,
                   (double *)pInst->m_pdG_CashFlow_Begin, *pInst->m_pdZeta_Ex,
                   *pInst->m_pdG_Ex);
  }
...
static void NAG_CALL _lsqfunc_(long m,        // number of equations
                               long n,        // number of unknowns
                               double xVec[], // x
                               double fVec[], Nag_Comm *pComm) {
...
static void
_Equi_EuroSwapStrike(long lToday, const char *szYC,
                     // exercise
                     const long *plEx_Begin, const long *plEx_End,
                     // fix
                     const long *plFixStart_Begin, const long *plFixStart_End,
                     const long *plFixEnd_Begin, const long *plFixPay_Begin,
                     const double *pdCoupon_Begin, SrtBasisCode eFixBasis,
                     // floating
                     const long *plFltStart_Begin, const long *plFltStart_End,
                     const long *plFltEnd_Begin, const long *plFltPay_Begin,
                     const double *pdMargin_Begin, const double *pdSpread_Begin,
  ...
  _memset(&dEqui_Notional, pdFixNotional_Begin, nSize_Fix,
          sizeof(dEqui_Notional));
  _memset(&dEqui_Notional, pdFltNotional_Begin, nSize_Flt,
          sizeof(dEqui_Notional));
  //_memset(&dCoupon        ,pdCoupon_Begin        ,nSize_Fix
  //,sizeof(*pdCoupon_Begin));

  for (; plEx_Begin < plEx_End; ++plEx_Begin, ++pdStrike_Begin) {
    /// locate indeces
    const long *plFixStart_Begin_ = find_min_greater_than(
        plEx_Begin, plFixStart_Begin, nSize_Fix, sizeof(*plEx_Begin), lless);
    int nFixOffSet = plFixStart_Begin_ - plFixStart_Begin;

    const long *plFltStart_Begin_ = find_min_greater_than(
        plEx_Begin, plFltStart_Begin, nSize_Flt, sizeof(*plEx_Begin), lless);
    int nFltOffSet = plFltStart_Begin_ - plFltStart_Begin;

    //_ASSERTE(nFixOffSet!=nSize_Fix);
    //_ASSERTE(nFltOffSet!=nSize_Flt);

    // NB: in case indeces are not located        , e.g. the last call in call
    // freq = q and fix freq = s
    *pdStrike_Begin = Equi_Strike(
        lToday, szYC, nFixOffSet != nSize_Fix ? plFixStart_Begin_ : plEx_Begin,
        nFixOffSet != nSize_Fix ? plFixStart_End : plEx_Begin + 1,
        nFixOffSet != nSize_Fix ? plFixEnd_Begin + nFixOffSet
                                : plFixEnd_Begin + nSize_Fix - 1,
        nFixOffSet != nSize_Fix ? plFixPay_Begin + nFixOffSet
                                : plFixPay_Begin + nSize_Fix - 1,
        nFixOffSet != nSize_Fix ? pdCoupon_Begin + nFixOffSet
                                : pdCoupon_Begin + nSize_Fix - 1,
        nFixOffSet != nSize_Fix ? pdFixNotional_Begin + nFixOffSet
                                : pdFixNotional_Begin + nSize_Fix - 1,
        eFixBasis, nFltOffSet != nSize_Flt ? plFltStart_Begin_ : plEx_Begin,
        nFltOffSet != nSize_Flt ? plFltStart_End : plEx_Begin + 1,
        nFltOffSet != nSize_Flt ? plFltEnd_Begin + nFltOffSet
                                : plFltEnd_Begin + nSize_Flt - 1,
        nFltOffSet != nSize_Flt ? plFltPay_Begin + nFltOffSet
                                : plFltPay_Begin + nSize_Flt - 1,
        nFltOffSet != nSize_Flt ? pdMargin_Begin + nFltOffSet
                                : pdMargin_Begin + nSize_Flt - 1,
        nFltOffSet != nSize_Flt ? pdSpread_Begin + nFltOffSet
                                : pdSpread_Begin + nSize_Flt - 1,
        nFltOffSet != nSize_Flt ? pdFltNotional_Begin + nFltOffSet
                                : pdFltNotional_Begin + nSize_Flt - 1,
        eFltBasis, dEqui_Notional);
  }
}

static void _ATM_3MStrike(long lToday, const char *szYC, const long *plEx_Begin,
                          const long *plEx_End, const long *plFltStart_Begin,
                          const long *plFltStart_End,
                          const double *pdMargin_Begin,
                          const double *pdSpread_Begin, SrtBasisCode eFltBasis,
                          /// results
                          double *pdStrike_Begin) {
  const int nSize_Flt = plFltStart_End - plFltStart_Begin;
  ...
  for (; plEx_Begin < plEx_End; ++plEx_Begin, ++pdStrike_Begin) {
    const long lMat_3M =
        add_unit(*plEx_Begin, 3, SRT_MONTH, NO_BUSDAY_CONVENTION);
    const long *plFltStart_Begin_ = find_min_greater_than(
        plEx_Begin, plFltStart_Begin, nSize_Flt, sizeof(*plEx_Begin), lless);
    const int nFltOffSet = plFltStart_Begin_ - plFltStart_Begin;

    *pdStrike_Begin =
        Swap_Rate(lToday, szYC, plEx_Begin,
                  1 + plEx_Begin, // size 1
                  &lMat_3M, &lMat_3M, eFltBasis, plEx_Begin, 1 + plEx_Begin,
                  &lMat_3M, &lMat_3M, pdMargin_Begin + nFltOffSet,
                  pdSpread_Begin + nFltOffSet, eFltBasis);
  }
}
...
void Calibrate_MidAt_Preprocess(
  ...
  _Equi_EuroSwapStrike(lToday, szYC, plEx_Begin, plEx_End, plFixStart_Begin,
                       plFixStart_End, plFixEnd_Begin, plFixPay_Begin,
                       pdCoupon_Begin, eFixBasis, plFltStart_Begin,
                       plFltStart_End, plFltEnd_Begin, plFltPay_Begin,
                       pdMargin_Begin, pdSpread_Begin, eFltBasis, pdSwapStrike);
  ...
  _ATM_3MStrike(lToday, szYC, plEx_Begin, plEx_End, plFltStart_Begin,
                plFltStart_End, pdMargin_Begin, pdSpread_Begin, eFltBasis,
                pdCapStrike);
...
const char *_Calibrate_MidAt(
  ...
  szErr = cpd_calib_diagonal_dlm(
      (char *)szYC, (char *)szVC, get_cash_vol, (char *)szRefRate,
      ///////////////////////
      // Diag Swaptions
      (char *)szFreq, (char *)szBasis, (char *)szRefRate, nSize_Ex,
      (long *)plEx_Begin,
      // all 1's to use exercise dates as calibration dates
      (int *)_memset(&nOne, _alloca(nSize_Ex * sizeof(int)), nSize_Ex,
                     sizeof(int)),
      ...
      "Q",  // Always 3M caplet ???
      "MM", // Always "MM"?
      (char *)szRefRate, nSize_Ex, (long *)plEx_Begin,
      // all 1's to use exercise dates as calibration dates
      (int *)_memset(&nOne, _alloca(nSize_Ex * sizeof(int)), nSize_Ex,
                     sizeof(int)),
      (char **)pszCapTenor_Begin, lTheoEnd, (double *)pdCapStrike_Begin,
      0, // pdOne        ,//weights
      (cpd_diag_calib_param *)pParam_Caplet, nCalibTauToCap ? 0 : 1, nOnefequi,
      nSize_Lam, (double *)pdLamT_Begin,
      // nSize_Ex==1?(double*)_memset(&dLam_Euro
      //       ,_alloca(nSize_Lam*sizeof(double))        ,nSize_Lam
      //       ,sizeof(double)):pdLam_Begin        ,
      pdLam_Begin,
      (double *)_memset(&dZero, _alloca(nSize_Lam * sizeof(double)), nSize_Lam,
                        sizeof(double)), // lambda shift
      nNumFactor, nNumFactor == 1 ? 1.e-5 : *pdAlpha,
      nNumFactor == 1 ? 1.e-5 : *pdGamma, nNumFactor == 1 ? 1.e-5 : *pdRho,
      // Shift parameters
      0, 0, 0, 0, 0, 0,
      //	Output
      &nSize_Sig, ppdSigT_Begin, ppdSig, (diag_calib_lm_params *)pCalibParam,
      0);
...
const char *Calibrate_MidAt(
    ...
    const double *pdMargin_Begin, const double *pdSpread_Begin,
    // term structure
    int nCalibTauToCap, // 2 - calibrate lambda ATM        , then prim ATS:  1 -
                        // calibrate Tau to cap        , 0 otherwise
    const double *pdAlpha, const double *pdGamma, const double *pdRho,
  ...
  Calibrate_MidAt_Preprocess(
      ...
      szFreq,    // frequency of the fixed leg -"M"        ,"Q"        ,"S" ,"A"
      pdCoupon_Begin, // coupon
      plEx_Begin, plEx_End, plFixStart_Begin, plFixStart_End, plFixEnd_Begin,
      plFixPay_Begin, plFltStart_Begin, plFltStart_End, plFltEnd_Begin,
      plFltPay_Begin, pdMargin_Begin, pdSpread_Begin, pszSwapTenor,
      pdSwapStrike, pszCapTenor, pdCapStrike);

  // delegate
  return _Calibrate_MidAt(
      szYC, szVC, get_cash_vol, lTheoEnd, szRefRate, szFreq, szBasis,
      plEx_Begin, plEx_End, pszSwapTenor, pdSwapStrike, pszCapTenor,
      pdCapStrike, nCalibTauToCap, pdAlpha, pdGamma, pdRho, &copy_SwapParam,
      &copy_CapParam, pCalibParam, nOnefequi, pdLamT_Begin, pdLamT_End,
      pdLam_Begin, ppdSigT_Begin, ppdSigT_End, ppdSig_Begin);
}
...
const char *Calibrate_GenMidAt(
    ...
    int nSkipLast, // If 1        , the last option is disregardedand the
  ...
  if (nEuroModel != 1)
    return "Only SMM model currently available for pricing "
           "amortizing/accreting european swaptions!";

  /// Delegate to compute diag generic european option prices ...
  szErr = Price_DiagGenEuro(
      lToday, szYC, szVC, get_correl, szCorrel, szRefRate, szFreq, szBasis,
      lTheoEnd, *plEx_Begin, plFixStart_Begin, plFixStart_End, plFixEnd_Begin,
      pdFixNotional_Begin, pdFixRate_Begin, pdFixFee_Begin, plFltStart_Begin,
      plFltStart_End, plFltEnd_Begin, pdFltNotional_Begin, pdMargin_Begin,
      pdSpread_Begin, dMinTime, dMinInterval, nUseVol, pnExBool, pdDiagPrice);

  if (szErr)
    return szErr;

  szErr = Decode_RefRate(szRefRate, (char **)(&szFreq_Flt),
                         (char **)(&szBasis_Flt));
  if (szErr)
    return szErr;
  ...
  if ((pdLamT_End - pdLamT_Begin) == 1) {
    szErr = amortMidat_cpd_calib_diagonal_new(
        lNotice, (char *)szYC, (char *)szVC, (char *)default_ref,
        (char *)default_swap_basis, (char *)default_swap_freq, get_cash_vol, 0,
        1, /// shift type
        pnExBool, pdDiagPrice,
        memcpy(_alloca(sizeof(double) * nSize_Fix), pdFixFee_Begin,
               sizeof(double) * nSize_Fix),
        (char *)szFreq, (char *)szBasis, nSize_Fix,
        memcpy(_alloca(sizeof(long) * nSize_Fix), plFixStart_Begin,
               sizeof(long) * nSize_Fix),
        memcpy(_alloca(sizeof(long) * nSize_Fix), plFixEnd_Begin,
               sizeof(long) * nSize_Fix),
        memcpy(_alloca(sizeof(long) * nSize_Fix), plFixPay_Begin,
               sizeof(long) * nSize_Fix), //(long*)plFixPay_Begin        ,
        memcpy(_alloca(sizeof(double) * nSize_Fix), pdFixRate_Begin,
               sizeof(double) * nSize_Fix), //(double*)pdFixRate_Begin        ,
        memcpy(_alloca(sizeof(double) * nSize_Fix), pdFixNotional_Begin,
               sizeof(double) * nSize_Fix), //(double*)pdFixNotional_Begin ,
        (char *)szFreq_Flt, (char *)szBasis_Flt, nSize_Flt,
        memcpy(_alloca(sizeof(long) * nSize_Flt), plFltStart_Begin,
               sizeof(long) * nSize_Flt),
        memcpy(_alloca(sizeof(long) * nSize_Flt), plFltEnd_Begin,
               sizeof(long) * nSize_Flt),
        memcpy(_alloca(sizeof(long) * nSize_Flt), plFltPay_Begin,
               sizeof(long) * nSize_Flt), //(long*)plFltPay_Begin        ,
        memcpy(_alloca(sizeof(double) * nSize_Flt), pdMargin_Begin,
               sizeof(double) * nSize_Flt), //(double*)pdMargin_Begin        ,
        memcpy(_alloca(sizeof(double) * nSize_Flt), pdSpread_Begin,
               sizeof(double) * nSize_Flt), //(double*)pdSpread_Begin        ,
        memcpy(_alloca(sizeof(double) * nSize_Flt), pdFltNotional_Begin,
               sizeof(double) * nSize_Flt), //(double*)pdFltNotional_Begin ,
        memcpy(_alloca(sizeof(double) * nSize_Fix), pdFixRate_Begin,
               sizeof(double) * nSize_Fix), //(double*)pdFixRate_Begin        ,
        nStrikeType, dMaxStdShort, nFixLam, n1FEqui, nSkipLast, nUseJump,
        dMaxVarJump,
        memcpy(_alloca((pdLamT_End - pdLamT_Begin) * sizeof(double)),
               pdLam_Begin, (pdLamT_End - pdLamT_Begin) * sizeof(double)),
        pdAlpha && pdGamma && pdRho ? 2 : 1, pdAlpha ? *pdAlpha : 1.e-5,
        pdGamma ? *pdGamma : 1.e-5, pdRho ? *pdRho : 1.e-5, &nSize_Sig,
        ppdSigT_Begin, ppdSig_Begin);
    ...
    *ppdSigT_End = *ppdSigT_Begin + nSize_Sig;
    ...
    szErr = amortMidat_cpd_calib_diagonal_new_ts(
        ...
        (char *)default_swap_basis, (char *)default_swap_freq, get_cash_vol, 0,
        1, /// shift type
        pnExBool, pdDiagPrice, (double *)pdFixFee_Begin, (char *)szFreq,
        (char *)szBasis, nSize_Fix, (long *)plFixStart_Begin,
        (long *)plFixEnd_Begin, (long *)plFixPay_Begin,
        (double *)pdFixRate_Begin, (double *)pdFixNotional_Begin,

        (char *)szFreq_Flt, (char *)szBasis_Flt, nSize_Flt,
        (long *)plFltStart_Begin, (long *)plFltEnd_Begin,
        (long *)plFltPay_Begin, (double *)pdMargin_Begin,
        (double *)pdSpread_Begin, (double *)pdFltNotional_Begin,
        ...
        nUseJump, dMaxVarJump,

        pdLamT_End - pdLamT_Begin, (double *)pdLam_Begin,
        (double *)pdLamT_Begin,

        pdAlpha && pdGamma && pdRho ? 2 : 1, pdAlpha ? *pdAlpha : 1.e-5,
        pdGamma ? *pdGamma : 1.e-5, pdRho ? *pdRho : 1.e-5,

        &nSize_Sig, ppdSigT_Begin, ppdSig_Begin);
  }
...
static const char *_nag_return(const char *szErr, Nag_Comm *pNagComm) {
  _ASSERTE(szErr);
...
static void _stdcall _OBJFUN_LAMTS_CONVERTSIG(const double *pdx, int nEle,
                                              double *pdsin) {
...
static void _stdcall _OBJFUN_LAMTS_CONVERTLAM(const double *pdx, int nEle,
                                              double *pdsin) {
  const double *pdxend = pdx + nEle;
  for (; pdx < pdxend; ++pdx, ++pdsin) {
    *pdsin = *pdx;
...
static void __stdcall _OBJFUN_LAMTS_CONVERT(const double *pdx, _Model *pModel) {
  ...
  _OBJFUN_LAMTS_CONVERTLAM(pdx + nSize_Sig, nSize_Lam,
                           (double *)pModel->m_pdLam_Begin);
}
...
static const char *_OBJFUN_LAMTS_CALIBDIAG(
    ...
    const double *pdGamma, const double *pdAlpha, const double *pdRho,
    // output
    double **ppdSigT_Begin, double **ppdSigT_End, double **ppdSig_Begin) {

  // first calibrate to its europeans
  return _Calibrate_MidAt(
      pCalibToEuro->m_szYC, pCalibToEuro->m_szVC,
      pCalibToEuro->m_get_cash_vol, /// get vol functor
      pCalibToEuro->m_lTheoEnd, pCalibToEuro->m_szRefRate,
      pCalibToEuro->m_szFreq, pCalibToEuro->m_szBasis,
      pCalibToEuro->m_plEx_Begin, pCalibToEuro->m_plEx_End,
      pCalibToEuro->m_pszSwapTenor, pCalibToEuro->m_pdSwapStrike,
      pCalibToEuro->m_pszCapTenor, pCalibToEuro->m_pdCapStrike,
      0, // nCalibTauToCap        ,
      pdAlpha, pdGamma, pdRho, pCalibToEuro->m_pParam_Swaption,
      pCalibToEuro->m_pParam_Caplet, pCalibToEuro->m_pCalibParam,
      pCalibToEuro->m_nOnefequi, pdLamT_Begin, pdLamT_End,
      (double *)pdLam_Begin, ppdSigT_Begin, ppdSigT_End, ppdSig_Begin);
}

static const char *_OBJFUN_LAMTS_BOUNDS_AUTOCAL(
    const _Calib_LamTS_DiagEuro_Comm *pCalibToEuro, const _GenMidAt *pOption,
  ...
  const char *szErr = _OBJFUN_LAMTS_CALIBDIAG(
      pCalibToEuro, pdLamT_Begin, pdLamT_End, pdLam_Begin, pdGamma, pdAlpha,
      pdRho, &pdSigT_Begin, &pdSigT_End, &pdSig_Begin);

  // create model that contains the sigma and lambda
  const _Model MyModel = {
      pdSigT_Begin, pdSigT_End, pdSig_Begin, pdLamT_Begin, pdLamT_End,
      pdLam_Begin, pdAlpha && pdGamma && pdRho ? 2 : 1, pdAlpha ? *pdAlpha : 0,
      pdGamma ? *pdGamma : 0, pdRho ? *pdRho : 0,
      MyModel.m_dAlpha * MyModel.m_dRho, // const double m_dAlphaRho;
      MyModel.m_dAlpha *
          sqrt(1 - MyModel.m_dRho *
                       MyModel.m_dRho), // const double m_dAlpha_Sqrt1mRhoSqrd;
      MyModel.m_dGamma * MyModel.m_dRho /
          sqrt(1. - MyModel.m_dRho *
                        MyModel.m_dRho), // const double
                                         // m_dGammaRho_Over_Sqrt1mRhoSqrd;
      0};
  ...
  // fill in models
  if (0) {
    _init_Model(pdSigT_Begin, pdSigT_End, pdSig_Begin, pdLamT_Begin, pdLamT_End,
                pdLam_Begin, pdAlpha, pdGamma, pdRho,
                0 //&MyModel
    );
  }
  ...
  /// price
  *pPrice = 0.;
  if (szErr = _GenMidAt_OptionPV_Core(nNumPointT, nNumPointX, pOption, &MyModel,
                                      pNumeraire, pPrice)) {
    // free(pdSig_Begin);
    // free(pdSigT_Begin);
    return szErr;
...
static void NAG_CALL _OBJFUN_LAMTS_BOUNDS(long n, double *pdx, double *pdf,
                                          double *pdg, Nag_Comm *pNagComm) {
  ...
  const _Calib_LamTS_DiagEuro_Comm *pCalib_LamTS_DiagEuro_Begin =
      pComm->m_pCalib_LamTS_DiagEuro_Begin;
  ...
  *pdf = 0.;
  ...
  for (; pOption_Begin < pOption_End; ++pOption_Begin, ++pOptionPrice_Begin,
                                      ++pNumeraire,
                                      ++pCalib_LamTS_DiagEuro_Begin, ++pdf_) {
    const char *szErr = _OBJFUN_LAMTS_BOUNDS_AUTOCAL(
        pCalib_LamTS_DiagEuro_Begin, pOption_Begin, pNumeraire, pdLamT_Begin,
        pdLamT_End,
        pdx, // unknowns        ,i.e. lambda
        pdGamma, pdAlpha, pdRho, nNumPointT, nNumPointX,
        // result
        pdf_);
...
static void _Set_NagFail(NagError *pfail) {
  SET_FAIL(*pfail);
  pfail->print = FALSE; // dont print!
}

static void NAG_CALL my_print_fun(const Nag_Search_State *st, Nag_Comm *comm) {
  /// do nothing
  st, comm;
}

static void _Set_NagOption(Nag_E04_Opt *poptions) {
...
_Calibrate_TS_(int nNumPointT, int nNumPointX, const _GenMidAt *pOption_Begin,
  ...
  if (m < n)
    return "_Calibrate_LamTS(...) : over-deterministic system!";
  ...
  /// delegate to nag solver ....
  {
    ...
    nag_opt_bounds_no_deriv(
        n, _OBJFUN_LAMTS_BOUNDS, Nag_Bounds, pdbl, pdbu, pdLam_InitGuess_Begin,
        &df, memset(_alloca(sizeof(double) * n), 0, sizeof(double) * n),
        &options, &commParams, &fail);

    if (fail.cxxode != NE_NOERROR && fail.cxxode != NE_USER_STOP)
      return fail.message;
  }
  ...
  _print_vector(&df, 1);
  _print_vector(Comm.m_pOptionPrice_Begin,
                Comm.m_pOption_End - Comm.m_pOption_Begin);
...
static const char *
_Calibrate_TS(int nNumPointT, int nNumPointX, const _GenMidAt *pOption_Begin,
              const _GenMidAt *pOption_End, const _PCQ_Seq *pNumeraire_Begin,
  ...
  /// delegate to nag solver ....
  {
    ...
    for (nI = 0; nI < m; ++nI) {
      const char *szErr = _OBJFUN_LAMTS_BOUNDS_AUTOCAL(
          pCalib_LamTS_DiagEuro_Begin + nI, pOption_Begin + nI,
          pNumeraire_Begin + nI, pdLamT_Begin + nI, pdLamT_Begin + nI + 1,
...
const char *Calibrate_TS_Diag_Euro_MidAt(
  ...
  /// options
  _GenMidAt *pOption_Begin = _alloca(m * sizeof(_GenMidAt)),
            *pOption_End = pOption_Begin + m, *pOption = pOption_Begin;
  ...
  const double *pdLamT_Begin =
      _fill_tenor(lToday, plEx_Begin, plEx_End, _alloca(m * sizeof(double)));
  // diag european swaption pricing
  const _Calib_LamTS_DiagEuro_Comm *pCalib_LamTS_DiagEuro_Begin =
      _alloca(sizeof(_Calib_LamTS_DiagEuro_Comm) * m);
  const _Calib_LamTS_DiagEuro_Comm *pCalib_LamTS_DiagEuro_End =
      pCalib_LamTS_DiagEuro_Begin + m;
  _Calib_LamTS_DiagEuro_Comm *pCalib_LamTS_DiagEuro =
      (_Calib_LamTS_DiagEuro_Comm *)pCalib_LamTS_DiagEuro_Begin;

  const int nSize_Fix = plFixPay_End - plFixPay_Begin;
  const int nSize_Flt = plFltPay_End - plFltPay_Begin;

  // ATM rate
  const char *szErr = _fill_swapcashrate_diag(
      szYC, plExStart_Begin, plExStart_Begin + m, lTheoEnd, szFreq, szBasis,
      szRefRate, pdCoupon_Begin);
  if (szErr)
    return szErr;

  // because we have a co-terminal system        , all options share the same
  // terminal measure numeraire
  if (szErr = _alloc_Seq(1, &Numeraire, 0, 0))
    return szErr;
  {
  ...
  for (; plEx_Begin < plEx_End;
       ++plEx_Begin, ++plExStart_Begin, ++plExFee_Begin, ++pdCoupon_Begin,
       ++pdExFee_Begin, ++pOption, ++pCalib_LamTS_DiagEuro) {
    const char *szErr = "GenMidAt_AutoCal_Diag(...): internal dates error!";

    // locate indeces
    const long *plFixPay_nI_Begin = (const long *)find_min_greater_than(
        plExStart_Begin, plFixPay_Begin, nSize_Fix, sizeof(long), lless);
    const long *plFltPay_nI_Begin = (const long *)find_min_greater_than(
        plExStart_Begin, plFltPay_Begin, nSize_Flt, sizeof(long), lless);
    ...
    if (szErr = _alloc_Option(plEx_End - plEx_Begin, nSize_Fix_nI, nSize_Flt_nI,
                              pOption))
      return szErr;

    // pre-initialize Diagonal MidAts
    _preinit_Option(
        szYC, lToday, plEx_Begin, plEx_End, plExFee_Begin, pdExFee_Begin,
        plExStart_Begin, plFixPay_Begin + nOffSet_Fix, plFixPay_End,
        plFixStart_Begin + nOffSet_Fix, plFixEnd_Begin + nOffSet_Fix,
        plFixFee_Begin + nOffSet_Fix, pdFixCoverage_Begin + nOffSet_Fix,
        pdFixNotional_Begin + nOffSet_Fix,
        (const double *)_memset(
            pdCoupon_Begin, _alloca(sizeof(double) * nSize_Fix_nI),
            nSize_Fix_nI, sizeof(double)), // pdFixNotional_Begin+nOffSet_Fix ,
        pdFixFee_Begin + nOffSet_Fix, plFltPay_Begin + nOffSet_Flt,
        plFltPay_End, plFltStart_Begin + nOffSet_Flt,
        plFltEnd_Begin + nOffSet_Flt, pdFltCoverage_Begin + nOffSet_Flt,
        pdFltNotional_Begin + nOffSet_Flt, pdMargin_Begin + nOffSet_Flt,
        pdSpread_Begin + nOffSet_Flt, dPayRec, pOption);

    // european diag calibration
    _init_Calib_LamTS_DiagEuro_Comm(
        lToday, szYC, szVC, get_cash_vol, lTheoEnd, szRefRate, szFreq, szBasis,
        plEx_Begin, plEx_End, nOnefequi,

        pdCoupon_Begin, plFixStart_Begin + nOffSet_Fix,
        plFixStart_Begin + nOffSet_Fix + nSize_Fix_nI,
        plFixEnd_Begin + nOffSet_Fix, plFixPay_Begin + nOffSet_Fix,

        plFltStart_Begin + nOffSet_Flt,
        plFltStart_Begin + nOffSet_Flt + nSize_Flt_nI,
        plFltEnd_Begin + nOffSet_Flt, plFltPay_Begin + nOffSet_Flt,
        pdMargin_Begin + nOffSet_Flt, pdSpread_Begin + nOffSet_Flt,
        pParam_Swaption, pParam_Caplet, pCalibParam, pCalib_LamTS_DiagEuro);
  }

  // delegate ...
  szErr = _Calibrate_TS(nNumPointT, nNumPointX, pOption_Begin,
                        pOption_Begin + m, pNumeraire_Begin, pdLambda_Begin,
                        pCalib_LamTS_DiagEuro_Begin, pdLamT_Begin,
                        pdLamT_Begin + m, pdAlpha, pdGamma, pdRho,
                        pdOptionPV_Begin, pdLam_InitGuess_Begin);
  ...
  pCalib_LamTS_DiagEuro =
      (_Calib_LamTS_DiagEuro_Comm *)pCalib_LamTS_DiagEuro_Begin;
  for (; pCalib_LamTS_DiagEuro < pCalib_LamTS_DiagEuro_End;
       ++pCalib_LamTS_DiagEuro)

    ...
    double** res)
{
            ...
            if (total->num_df > 0)
            {
                for (i = 0; i < total->num_df; i++)
                {
                    if (total->df_tms[i] >= YEARS_IN_DAY)
                    {
                        ...
                        total->gam[i] = Lambda_func(
                            time[t],
                            time[t] + total->df_tms[i],
                            ((SrtIrDesc*)(und->spec_desc))->ts);
    ...
    err = srt_f_New_gen_random(cube, 0, numpaths - 1, 0, nstept - 2, 1, gen_method, &seed, time);
    if (err)
        goto FREE_RETURN;

    if (method &&
        ((gen_method == BAL_ANTI) || (gen_method == RANDOM_GAUSS) || (gen_method == ANTITHETIC)))
    {
        /* indeed, it's ugly ! */

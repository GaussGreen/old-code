...
/*************************************************************************************/
  ...
  void FFTboundary_out(long int n, 
			double *yin, 
			double *yy,
			int iconv)
  {
...
/************************************************************************************/
void FFTboundary_in(long int n, 
		  double *y, 
		  double *ycompl)
{
	long int i;
...
//					stock1= rho*cos((n_conv+1) *atan(storage[2*j]/storage[2*j-1]))*corr_f;
...
void ConvolveMarginals(	double **AbscMarg,
						double **OrdMarg,
						unsigned long n_pts,
...
{
	double *avg,*PrDens,*ycompl,*FT_PrDens,*xPrDens,Norm,delta,Mean,StdDev,
		   MidPointX,MidPointY,DeltaX,InfBound,SupBound,dx,q,*xPrDens_new,
		   *PrDens_new,*Xdata,*Ydata,low_thresh,sigma2;
	...
	////////////////////////////////////////////////////////////////////////////////////
			...
			for (j=1;j<=n_conv;j++) {
				...
				for (k=1;k<=n_lagr_pts/2;k++) {
					Xdata[k+n_lagr_pts/2]=xPrDens[((n_pts-1)/2+1)+(n_kill_pts+1)+k];
					Ydata[k+n_lagr_pts/2]=PrDens[((n_pts-1)/2+1)+(n_kill_pts+1)+k];
				}

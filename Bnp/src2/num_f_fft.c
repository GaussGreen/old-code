...
void Conv1D(double* in_v, unsigned long n, unsigned long n_conv, double delta, int iconv)

{
    unsigned long i, j;
    double *storage, stock1, stock2, corr_f = 1. / sqrt(8. * n), corr_f2 = 1. / sqrt(2. * n), rho;
    int     k = 0;  // this choses the root by varying the integer k in [0,n_conv-1] as there are
                    // n_conv distinct roots
    ...
    case 1:
        ...
        for (i = 1; i <= n_conv; i++)
        {
            for (j = 1; j <= (2 * n) / 2; j++)
            {
                //					rho=pow(sqrt(storage[2*j-1]*storage[2*j-1]+storage[2*j]*storage[2*j]),(n_conv+1));
                //					stock1= rho*cos((n_conv+1)
                //*atan(storage[2*j]/storage[2*j-1]))*corr_f; 					stock2= rho*sin((n_conv+1)
                //*atan(storage[2*j]/storage[2*j-1]))*corr_f;

                stock1 = (storage[2 * j - 1] * in_v[2 * j - 1] - storage[2 * j] * in_v[2 * j]);
                stock2 = (storage[2 * j - 1] * in_v[2 * j] + storage[2 * j] * in_v[2 * j - 1]);
    ...
    case -1:
        ...
        for (j = 1; j <= n; j++)
        {
            rho = sqrt(storage[2 * j - 1] * storage[2 * j - 1] + storage[2 * j] * storage[2 * j]);
...
void ConvolveMarginals(
    double**      AbscMarg,
    double**      OrdMarg,
    unsigned long n_pts,
    unsigned long n_assets,
    unsigned long n_conv,
    int           conv_flag  // this is 1 if you want to convolve
                             // and -1 if you want to deconvolve
)

{
    double *avg, *PrDens, *ycompl, *FT_PrDens, *xPrDens, Norm, delta, Mean, StdDev, MidPointX,
        MidPointY, DeltaX, InfBound, SupBound, dx, q, *xPrDens_new, *PrDens_new, *Xdata, *Ydata,
        low_thresh, sigma2;
    ...
    PrDens  = dvector(0, n_pts - 1);  // NB. n_pts must be a  1 + a power of 2 to be used
    ...
    default:
        ...
        for (i = 0; i < n_assets; i++)
        {
            ...
            for (j = 1; j < n_pts; j++)
            {
                ...
                ////////////////////////////////////////////////////////////////////////////////////
                /////////// step 2: computes the centered Pr. density from each cumulative marginal
                ////////////////////////////////////////////////////////////////////////////////////

                PrDens[j] =
                    (OrdMarg[i][j] - OrdMarg[i][j - 1]) / (AbscMarg[i][j] - AbscMarg[i][j - 1]);
            ...
            for (j = 1; j <= n_conv; j++)
            {
                FFTboundary_in(n_pts - 1, PrDens, ycompl);  //  n_pts-1 must be a power a 2
                ...
                {
                    Xdata[k] = xPrDens[(n_pts - 1) / 2 - n_kill_pts - n_lagr_pts / 2 + (k - 1)];
                    Ydata[k] = PrDens[(n_pts - 1) / 2 - n_kill_pts - n_lagr_pts / 2 + (k - 1)];
                }
                for (k = 1; k <= n_lagr_pts / 2; k++)
                {
                ...
                for (k = 1; k <= 2 * n_kill_pts + 3; k++)
                    PrDens[(n_pts - 1) / 2 - n_kill_pts + (k - 1)] = InterpLagrange1D(
                        Xdata, Ydata, n_lagr_pts, xPrDens[(n_pts - 1) / 2 - n_kill_pts + (k - 1)]);
                ...
                StdDev = 0.0;
                for (k = 2; k < n_pts; k++)
                    Norm += (xPrDens[k] - xPrDens[k - 1]) * (PrDens[k] + PrDens[k - 1]) / 2.;

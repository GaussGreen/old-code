// FxSVCalib.cxx : FXSV underlying initialization and calibration
...
Err InitFxSVUnd(SFxSVUndDesc *und, char *dom_und, char *for_und, double spot,
  ...
  if (!und)
    return "Und must be allocated before calling InitFxSVUnd";
  domundptr = lookup_und(dom_und);
  if (!domundptr)
    return "Domestic underlying not found in InitFxSVUnd";
  ...
  if (!und->dates || !und->times || !und->beta || !und->alpha || !und->gamma ||
      !und->rho)
    return "Memory failure in InitFxSVUnd";

  memcpy(und->dates, dates, ndates * sizeof(long));
  for (i = 0; i < ndates; i++)
    und->times[i] = (dates[i] - und->today) * YEARS_IN_DAY;
...
char *FreeFxSVUnd(void *ptr) {
  ...
  free(und->gamma);
  if (und->rho)
    free_f3tensor(und->rho, 0, und->ntimes - 1, 0, 3, 0, 3);
...
Err FxSVFullFromUnd(SFxSVUndFull *o, SrtUndPtr und, long last_date) {
  ...
  if (!o->pmdl || strcmp(und->underl_lbl, "FXSV_UND"))
    return serror("Underlying %s improperly initialised or of incorrect type",
                  und->underl_name);

  T_last = (last_date - o->pmdl->today) * YEARS_IN_DAY;

  err = Get_LGM_TermStructure2(o->pmdl->dom_und, &o->sigtms_d, &o->sig_d,
                               &o->nsig_d, &tau_d);
  if (err)
    return err;
  o->lam_d = 1.0 / tau_d;

  err = Get_LGM_TermStructure2(o->pmdl->for_und, &o->sigtms_f, &o->sig_f,
                               &o->nsig_f, &tau_f);
  if (err)
    return err;
  o->lam_f = 1.0 / tau_f;
...
static void NAG_CALL EvalDerivatives(Integer neq, double t, double y[],
  ...
  // dy1/dt        , dy2/dt
  c1 = Gamma_d * Gamma_d / 2.0 + Gamma_f * Gamma_f / 2.0 -
       p->rho[0][1] * Gamma_d * Gamma_f;
  ...
  yp[0] = c1 * (u_re2 - u_im2 - p->u_im) - c2 * y[2] - c3 * y[4] +
          c4 * (p->u_re * y[3] + p->u_im * y[2]) -
          c3 * (y[2] * y[2] - y[3] * y[3]) / 2.0;
  ...
  for (i = 0; i < 4; i++)
    if (p->want[i]) {
      ypg[i][0] = -c2 * yg[i][2] - c3 * yg[i][4] +
                  c4 * (p->u_re * yg[i][3] + p->u_im * yg[i][2]) -
                  c3 * (yg[i][2] * y[2] - yg[i][3] * y[3]);
      ypg[i][1] = -c2 * yg[i][3] - c3 * yg[i][5] -
                  c4 * (p->u_re * yg[i][2] - p->u_im * yg[i][3]) -
                  c3 * (y[2] * yg[i][3] + yg[i][2] * y[3]);
    }
  ...
  if (p->is_cur_idx && p->want[1]) {
    ...
    ypg[1][0] += -c6 * y[2] - 2.0 * p->alpha * y[4] +
                 c7 * (p->u_re * y[3] + p->u_im * y[2]) -
                 p->alpha * (y[2] * y[2] - y[3] * y[3]);

    ypg[1][1] += -c6 * y[3] - 2.0 * p->alpha * y[5] -
                 c7 * (p->u_re * y[2] - p->u_im * y[3]) -
                 2.0 * p->alpha * y[2] * y[3];
  }
  ...
  yp[2] = c1 * (u_re2 - u_im2 - p->u_im) + p->gamma * y[2] - c2 * y[4] +
          c5 * (p->u_re * y[3] + p->u_im * y[2]) +
          c4 * (p->u_re * y[5] + p->u_im * y[4]) -
          c3 * (y[2] * y[4] - y[3] * y[5]);

  yp[3] = c1 * (p->u_re + u_re_u_im) + p->gamma * y[3] - c2 * y[5] -
          c5 * (p->u_re * y[2] - p->u_im * y[3]) -
          c4 * (p->u_re * y[4] - p->u_im * y[5]) -
          c3 * (y[2] * y[5] + y[3] * y[4]);

  for (i = 0; i < 4; i++)
    if (p->want[i]) {
      ypg[i][2] = p->gamma * yg[i][2] - c2 * yg[i][4] +
                  c5 * (p->u_re * yg[i][3] + p->u_im * yg[i][2]) +
                  c4 * (p->u_re * yg[i][5] + p->u_im * yg[i][4]) -
                  c3 * (y[2] * yg[i][4] - y[3] * yg[i][5] + yg[i][2] * y[4] -
                        yg[i][3] * y[5]);

      ypg[i][3] = p->gamma * yg[i][3] - c2 * yg[i][5] -
                  c5 * (p->u_re * yg[i][2] - p->u_im * yg[i][3]) -
                  c4 * (p->u_re * yg[i][4] - p->u_im * yg[i][5]) -
                  c3 * (y[2] * yg[i][5] + y[3] * yg[i][4] + yg[i][2] * y[5] +
                        yg[i][3] * y[4]);
    }
  ...
  if (p->is_cur_idx && p->want[0]) {
    ...
    ypg[0][2] +=
        c6 * (u_re2 - u_im2 - p->u_im) + c7 * (p->u_re * y[3] + p->u_im * y[2]);
    ypg[0][3] +=
        c6 * (p->u_re + u_re_u_im) - c7 * (p->u_re * y[2] - p->u_im * y[3]);
  }
  if (p->is_cur_idx && p->want[1]) {
    ...
    ypg[1][2] += -c6 * y[4] + c8 * (p->u_re * y[3] + p->u_im * y[2]) +
                 c7 * (p->u_re * y[5] + p->u_im * y[4]) -
                 4.0 * p->alpha * (y[2] * y[4] - y[3] * y[5]);

    ypg[1][3] += -c6 * y[5] - c8 * (p->u_re * y[2] - p->u_im * y[3]) -
                 c7 * (p->u_re * y[4] - p->u_im * y[5]) -
  ...
  yp[4] = c1 * (u_re2 - u_im2 - p->u_im) + c2 * y[4] +
          c5 * (p->u_re * y[5] + p->u_im * y[4]) -
          c3 * (y[4] * y[4] - y[5] * y[5]) / 2.0;

  yp[5] = c1 * (p->u_re + u_re_u_im) + c2 * y[5] -
          c5 * (p->u_re * y[4] - p->u_im * y[5]) - c3 * y[4] * y[5];

  for (i = 0; i < 4; i++)
    if (p->want[i]) {
      ypg[i][4] = c2 * yg[i][4] +
                  c5 * (p->u_re * yg[i][5] + p->u_im * yg[i][4]) -
                  c3 * (y[4] * yg[i][4] - y[5] * yg[i][5]);

      ypg[i][5] = c2 * yg[i][5] -
                  c5 * (p->u_re * yg[i][4] - p->u_im * yg[i][5]) -
  ...
  if (p->is_cur_idx && p->want[0]) {
    ...
    ypg[0][4] += p->beta * (u_re2 - u_im2 - p->u_im) +
                 c6 * (p->u_re * y[5] + p->u_im * y[4]);
    ypg[0][5] += p->beta * (p->u_re + u_re_u_im) -
                 c6 * (p->u_re * y[4] - p->u_im * y[5]);
  }
  if (p->is_cur_idx && p->want[1]) {
    c6 = 2.0 * p->rho[2][3] * p->beta;

    ypg[1][4] += c6 * (p->u_re * y[5] + p->u_im * y[4]) -
                 4.0 * p->alpha * (y[4] * y[4] - y[5] * y[5]);
    ypg[1][5] +=
        -c6 * (p->u_re * y[4] - p->u_im * y[5]) - 8.0 * p->alpha * y[4] * y[5];
  }
  if (p->is_cur_idx && p->want[2]) {
    ypg[2][4] += 2.0 * y[4];
    ypg[2][5] += 2.0 * y[5];
...
static Err FxSVCalcPhi(SFxSVComm_InvFT *q, double u_re, double u_im,
...
Err FxSVDensityFT(char *fxundname, long fix_date, long pay_date, double u_re,
  ...
  if (!und)
    return serror("Cannot find underlying %s", fxundname);
  ...
  //	err = FxSVCalcPhi(&comm_InvFT        , u_re        , u_im        , h_re
  //, h_im); 	if (err) goto FREE_RETURN;
             ...
             1e-16) // not yet calculated or no match
  {
        ...
        0.0) // point not yet calculated -> calculate it
    {
      ...
      for (i = 0; i < q->nk; i++) {
        ...
        for (j = 0; j < 5; j++)
          if (j == 0 || q->want[j - 1])
            q->c_cur->fn[q->c_cur->cur_pt][i][j] =
                dd1 * zeta_re[j] + dd2 * zeta_im[j];
      }
    }
  }
...
static Err FxSVDoIntegration(SFxSVComm_InvFT *q, int idx_k, int idx_fn,
                             double *res) {
  Err err = NULL;
...
static void NAG_CALL EvalDerivatives2(Integer neq, double t, double y[],
                                      double yp[], Nag_User *comm) {
  SFxSVComm_ODE *p = (SFxSVComm_ODE *)comm->p;
  double Gamma_d, Gamma_f;
  ...
  yp[4] = 2.0 * c1 + p->gamma * y[4] - c2 * y[5] + c5 * y[1] + c4 * y[2] +
          c3 * y[1] * y[2];
...
static Err FxSVCalcMoments(SFxSVComm_InvFT *q, double *mean, double *std) {
  ...
  for (i = o->ntimes - 2; i >= 0; i--) {
    ...
    nag_ode_ivp_rk_range(6, EvalDerivatives2, o->times[i], &tgot, y, yp, ymax,
                         &opt, &comm_Nag, &fail);
    if (fail.cxxode != NE_NOERROR) {
      err = serror(fail.message);
      goto FREE_RETURN;
...
Err FxSVOptions(char *fxundname, long fix_date, long pay_date, int nK,
  ...
  df_dom = 1.0; // swp_f_df(spot_date        , pay_date        , yc_dom);
                // affine
  df_for = 1.0; // swp_f_df(spot_date        , pay_date        , yc_for);
                // affine

  ffx0 = und_full.pmdl->spot * df_for / df_dom;

  for (i = 0; i < nK; i++)
    comm_InvFT.k[i] = log(K[i] / ffx0);
  ...
  for (i = 0; i < nK; i++) {
    ...
    for (j = 0; j < 5; j++)
      if (j == 0 || want[j - 1]) {
        ...
        res[i][j] = z / pi * ffx0;
        if (j == 0 && rec_pay == SRT_PUT && K[i] > ffx0)
          res[i][j] += K[i] - ffx0;
        if (j == 0 && rec_pay == SRT_CALL && K[i] < ffx0)
          res[i][j] += ffx0 - K[i];

        //			res[i][j] *= swp_f_df(und_full.pmdl->today ,
        // pay_date        , yc_dom);		affine
      }
  }
...
static Err CalibrateBeta(SFxSVComm_Calib *r) {
  ...
  for (i = r->idx_from; i <= r->idx_to; i = j + 1) {
    for (j = i; !r->calib[j][0]; j++)
      ; // Find next beta calibration date
    ...
    while (fabs(r->o->times[r->o->ntimes - 1] - pmdl->times[j]) > 1e-5)
      r->o->ntimes++;
    t1 = clock();
    ...
    do {
      ...
      if (fabs(f[1]) > 1e-16)
        pmdl->beta[i] -= f[0] / f[1];
      if (fabs(f[1]) <= 1e-16 || pmdl->beta[i] <= r->bl[0] ||
          pmdl->beta[i] >= r->bu[0]) {
      ...
      for (k = i + 1; k <= j; k++)
        pmdl->beta[k] = pmdl->beta[i];

    } while (fabs(f[0]) > tol);

    t2 = clock();
    smessage("Volatility calibration at step %d        , time in sec: %.2f",
             j + 1, (double)(t2 - t1) / CLOCKS_PER_SEC);
  }
  return NULL;
}

void NAG_CALL LsqFun(Integer m, Integer n, double x[], double fvec[],
                     Nag_Comm *comm) {
  Err err = NULL;
  ...
  smessage("Smile parameters calibration at step %d        , pass %d...",
           r->idx_to + 1, comm->nf);

  for (i = 1, n = 0; i < 4; i++)
    if (r->calib[r->idx_to][i]) // check bounds
    {
      xx[i] = x[n++];
      if (xx[i] <= r->bl[i])
        comm->flag = -2 * i;
  ...
  for (i = r->idx_from; i <= r->idx_to;
       i++) // modify the underlying according to x
  {
    if (r->calib[r->idx_to][1])
      pmdl->alpha[i] = xx[1];
    if (r->calib[r->idx_to][2])
      pmdl->gamma[i] = xx[2];
    if (r->calib[r->idx_to][3])
      pmdl->rho[i][2][3] = pmdl->rho[i][3][2] = xx[3];
  }
...
static Err CalcCorrBounds(double **rho, double *bl, double *bu, double tol) {
  ...
  if (fail.cxxode != NE_NOERROR)
    return serror(fail.message);
  if (r[0] <= 0.0)
    return serror("Input correlation matrix is not positive definite");
                  ...
                  double **vol) // Vols per exercise date per strike
{
  ...
  for (i = 0; i < pmdl->ntimes; i++) {
    ...
    if (nK[i] > MAXNK) {
      err = serror("Maximum %d strikes allowed per ex date", MAXNK);
      goto FREE_RETURN;
    }
    ...
    for (j = 0; j < nK[i]; j++) {
      ...
      comm_Calib.mkt_val[i][j] =
          srt_f_optblksch(ffx0, K[i][j], vol[i][j], pmdl->times[i], 1.0,
  ...
  for (i = 0; i < pmdl->ntimes; i = j + 1) {
    ...
    for (j = i;
         j < pmdl->ntimes && !calib[j][1] && !calib[j][2] && !calib[j][3]; j++)
      ;
    ...
    {                      // just calibrate beta then
      ...
      if (++j > 0)
        for (; j < pmdl->ntimes; j++)
          pmdl->beta[j] = pmdl->beta[j - 1];
      break; // nothing more to calibrate
    } else   // step j is a smile calibration date
    {
      if (!calib[j][0]) {
        err = serror("Error: Smile calibration date %d is not a volatility "
                     "calibration date",
                     j + 1);
        goto FREE_RETURN;
      }
      ...
      n = 0;
      if (calib[j][1])
        x[n++] = pmdl->alpha[i == 0 ? 0 : i - 1];
      if (calib[j][2])
        x[n++] = pmdl->gamma[i == 0 ? 0 : i - 1];
      if (calib[j][3]) {
        comm_Calib.bl[3] = -1.0;
        comm_Calib.bu[3] = 1.0;
        ...
        for (m = i; m <= j; m++) {
          ...
          if (rhobl > comm_Calib.bl[3])
            comm_Calib.bl[3] = rhobl;
          if (rhobu < comm_Calib.bu[3])
            comm_Calib.bu[3] = rhobu;
        }
        if (comm_Calib.bu[3] - comm_Calib.bl[3] < 0.01) {
          ...
          if (x[n] <= comm_Calib.bl[3])
            x[n] = comm_Calib.bl[3] + 0.001;
          if (x[n] >= comm_Calib.bu[3])
            x[n] = comm_Calib.bu[3] - 0.001;
          n++;
        }
      }
      ...
      if (n > 0) // if there's smth to calibrate
      {
        if (nK[j] - 1 < n) {
          err = serror(
              "Number of options must be greater or equal to the number of "
              "calibrated parameters at exercise date %d",
              j + 1);
          goto FREE_RETURN;
        }
        ...
        nag_opt_lsq_no_deriv(nK[j] - 1, n, LsqFun, x, &fsumsq, fvec,
                             &fjac[0][0], 3, &options, &comm, &fail);

        if (fail.cxxode == NE_USER_STOP) {
          switch (fail.errnum) {
          case -1:
            smessage("Alpha has hit the upper bound at step %d", j + 1);
            break;
          case -2:
            smessage("Alpha has hit the lower bound at step %d", j + 1);
            break;
          case -3:
            smessage("Gamma has hit the upper bound at step %d", j + 1);
            break;
          case -4:
            smessage("Gamma has hit the lower bound at step %d", j + 1);
            break;
          case -5:
            smessage("Rho has hit the upper bound at step %d", j + 1);
            break;
          case -6:
            smessage("Rho has hit the lower bound at step %d", j + 1);
            break;
          default:
            err = serror("Calibration failed at date %d", j + 1);
            goto FREE_RETURN;
          }
          smessage("Smile calibration skipped at step %d", j + 1);
          memcpy(x, x_save, 3 * sizeof(double));
        } else if (fail.cxxode != NE_NOERROR) {
          err = serror(fail.message);
          goto FREE_RETURN;
        }
      }
      ...
      for (m = i; m <= j; m++) {
        ...
        if (calib[j][2])
          pmdl->gamma[m] = x[n++];
        if (calib[j][3])
          pmdl->rho[m][2][3] = pmdl->rho[m][3][2] = x[n++];
      }

      t2 = clock();
      smessage("Smile calibration at step %d        , time in sec: %.2f", j + 1,
               (double)(t2 - t1) / CLOCKS_PER_SEC);

      if (n == 0 || fail.cxxode == NE_USER_STOP) {
        calib[j][1] = calib[j][2] = calib[j][3] =
            0;     // That means restart the calibration from i
...
FREE_RETURN:
  FxSVFullFree(&und_full);
  if (comm_Calib.k)
    free_dmatrix(comm_Calib.k, 0, pmdl->ntimes - 1, 0, MAXNK - 1);
  if (comm_Calib.mkt_val)
    free_dmatrix(comm_Calib.mkt_val, 0, pmdl->ntimes - 1, 0, MAXNK - 1);

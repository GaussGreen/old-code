...
// Selects the dates in r1_dates        , r3_dates        , fx_dates which are
// <= last_time and put them in a single array. (r1 then r3 then fx)
Err compute_vol_times_3FQuanto(double *r1_dates, double *r1_sigma,
  ...
  *vol_times =
      (double *)calloc(n_r1_dates + n_r3_dates + n_fx_dates, sizeof(double));
  if (!vol_times) {
...
// Merge rates        , fx and corr term structures and returns all variables in
// lists
Err merge_rates_fx_corr_ts_3FQuanto(
  ...
  *corr_merge_r3_fx = (double *)calloc(*sig_merge_n, sizeof(double));
  ...
  for (i = *sig_merge_n - 1; i >= 0; i--) {
    (*sig_merge_r1)[i] =
        sig_r1[Get_Index((*sig_merge_mat)[i], sig_r1_mat, sig_n_r1)];
    (*sig_merge_r3)[i] =
        sig_r3[Get_Index((*sig_merge_mat)[i], sig_r3_mat, sig_n_r3)];
    (*sig_merge_fx)[i] =
...
void fill_fwd_var_corr_3FQuanto(
  ...
  double correl_r1_r2, correl_r1_r3, correl_r1_fx, correl_r2_r3, correl_r2_fx,
      correl_r3_fx;

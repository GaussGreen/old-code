...
void genmidat_free_calibinfos(GENMIDAT_CALIBINFOS sCalibInfos) {
  if (sCalibInfos) {
    if (sCalibInfos->dLongVols)
      free(sCalibInfos->dLongVols);
    if (sCalibInfos->dShortVols)
      free(sCalibInfos->dShortVols);
  }
}

/* Interpolation in Sigma / (T - t) */
static Err generic_midat_interpolate_vol(int iNbExe, double *dMaturity,
                                         double *dVols, int *iCalibVols,
                                         int iInterpType) {
  ...
  dLastMaturity =
      dMaturity[iNbExe - 1] + (dMaturity[iNbExe - 1] - dMaturity[iNbExe - 2]);
  ...
  for (i = 0; i < iNbExe; i++) {
    if (!iCalibVols[i]) {
      ...
      /* Find the indexes */
      index_down = i - 1;
      while (index_down >= 0 && !iCalibVols[index_down])
        index_down--;
      index_up = i + 1;
      while (index_up < iNbExe && !iCalibVols[index_up])
        index_up++;
      ...
      case 0: {
        ...
        dVols[i] = dVols[index_down] +
                   (dVols[index_up] - dVols[index_down]) /
                       (dMaturity[index_up] - dMaturity[index_down]) *
      ...
      case 1: {
        ...
        dVols[i] =
            dVols[index_down] / (dLastMaturity - dMaturity[index_down]) +
...
Err generic_midat_calib(/* Input */
                        int iNbExe, double *dLongOption, double *dShortOption,
  ...
  Err err = NULL;
  double *dLongVols = NULL, *dShortVols = NULL;

  int *iCalibLong = NULL, *iCalibShort = NULL;
  ...
  for (i = 0; i < iNbExe; i++) {
    if (dLongOption[i] / sModel->dNumeraire >= sModel->dForward[i] &&
        fabs(dLongOption[i]) > 1.0E-10) {
      dShiftFwd = max(2.0 * fabs(sModel->dForward[i]), 1.0);

      err = srt_f_optimpvol(dLongOption[i], sModel->dForward[i] + dShiftFwd,
                            dShiftFwd, sModel->dTimes[i], sModel->dNumeraire,
                            SRT_CALL, SRT_NORMAL, &(dLongVols[i]));
      ...
      if (sParams->iInterpFailedVol &&
          (fabs(dLongOption[i]) < 1.0E-10 ||
           (dLongOption[i] / sModel->dNumeraire - sModel->dForward[i] >
            -dLongOption[i] / 1000.0))) {
        iCalibLong[i] = 0;
      } else {
        err = serror("Long Option %d: Option < Intrinseq or cannot solve for "
                     "volatility !",
                     i + 1);
        goto FREE_RETURN;
      }
  ...
  if (sParams->iInterpFailedVol) {
    err = generic_midat_interpolate_vol(iNbExe, sModel->dTimes, dLongVols,
                                        iCalibLong, sParams->iInterpVolType);
  ...
  if (sParams->iCalibLambda) {
    if (sParams->iShortIsCap) {
      for (i = 0; i < iNbExe - 1; i++) {
        ...
        if (dShortOption[i] / sModel->dNumeraire >= dShortFwd &&
          ...
          err =
              srt_f_optimpvol(dShortOption[i], dShortFwd + dShiftFwd, dShiftFwd,
                              sModel->dTimes[i], sModel->dNumeraire, SRT_CALL,
                              SRT_NORMAL, &(dShortVols[i]));

          if (err ||
              fabs((dShortVols[i] - (dShortFwd + dShiftFwd) * 1.0e-9)) <
                  1.0e-15) // To avoid the imput of misleading information
          {
            iCalibShort[i] = 0;
            err = NULL;
          } else {
          ...
          if (sParams->iInterpFailedVol &&
              (fabs(dShortOption[i]) < 1.0E-10 ||
               (dShortOption[i] / sModel->dNumeraire - dShortFwd >
                -dShortOption[i] / 1000.0))) {
            iCalibShort[i] = 0;
          } else {
            err = serror(
                "Short Option %d: Option < Intrinseq or cannot solve for vol !",
                i + 1);
            goto FREE_RETURN;
          }
        }
      ...
      for (i = 0; i < iNbExe - 1; i++) {
        if (dShortOption[i] / sModel->dNumeraire >= sModel->dForward[i + 1] &&
          ...
          err = srt_f_optimpvol(dShortOption[i],
                                sModel->dForward[i + 1] + dShiftFwd, dShiftFwd,
                                sModel->dTimes[i], sModel->dNumeraire, SRT_CALL,
                                SRT_NORMAL, &(dShortVols[i]));
          ...
          if (sParams->iInterpFailedVol &&
              (fabs(dShortOption[i]) < 1.0E-10 ||
               (dShortOption[i] / sModel->dNumeraire - sModel->dForward[i + 1] >
                -dShortOption[i] / 1000.0))) {
            iCalibShort[i] = 0;
          } else {
            err = serror(
    ...
    if (sParams->iInterpFailedVol) {
      err =
          generic_midat_interpolate_vol(iNbExe - 1, sModel->dTimes, dShortVols,
                                        iCalibShort, sParams->iInterpVolType);
  ...
  if (sModel->iNbFactor == 1) {
    err = generic_midat_calib_one_factor(iNbExe, iCalibLong, dLongVols,
                                         iCalibShort, dShortVols, sModel,
                                         sParams, sCalibInfos);
  } else if (sModel->iNbFactor == 2) {
    err = generic_midat_calib_two_factor_total(iNbExe, iCalibLong, dLongVols,
                                               iCalibShort, dShortVols, sModel,
                                               sParams, sCalibInfos);
  } else {
...
Err generic_midat_find_const_lambda(double dT0, double dT1, double dRatio,
                                    double *dLambda) {
...
Err generic_midat_calib_one_factor(/* Input */
                                   int iNbExe, int *iCalibLong,
                                   double *dLongVols, int *iCalibShort,
                                   double *dShortVols,
  ...
  for (i = 1; i < iNbExe; i++) {
    sModel->dSigma[i] =
        dLongVols[i] * dLongVols[i] * sModel->dTimes[i] -
        pow(sModel->dBeta[i] / sModel->dBeta[i - 1] * dLongVols[i - 1], 2.0) *
            sModel->dTimes[i - 1];

    if (sModel->dSigma[i] > 0.0) {
      sModel->dSigma[i] = sqrt(sModel->dSigma[i] /
                               (sModel->dTimes[i] - sModel->dTimes[i - 1])) /
                          sModel->dBeta[i];
    } else {
    ...
    /* Floor and Cap */
    sModel->dSigma[i] =
        max(sModel->dSigma[i], sParams->dMinChangeVol * sModel->dSigma[i - 1]);
    sModel->dSigma[i] =
        min(sModel->dSigma[i], sParams->dMaxChangeVol * sModel->dSigma[i - 1]);
  }
...
static double generic_midat_get_last_beta2(GENMIDAT_MODEL sModel) {
  ...
  if (sModel->iNbVols > 1) {
    dLastMat = 2.0 * sModel->dTimes[sModel->iNbVols - 1] -
               sModel->dTimes[sModel->iNbVols - 2];
    dLastBeta2 =
        sModel->dBeta2[sModel->iNbVols - 1] -
        exp(-sModel->dGamma * dLastMat) * sModel->dBeta[sModel->iNbVols - 1];
  } else {
    dLastBeta2 = 0.0;
  }
...
Err generic_midat_calib_two_factor_total(/* Input */
  ...
  double **dResIter = NULL, **dResIterBeta = NULL;
  ...
  if (!sCalibInfos) {
    ...
    if (!sUsedCalibInfos) {
      err =
          "Memory allocation faillure in generic_midat_calib_two_factor_total";
      goto FREE_RETURN;
    }
  } else {
  ...
  /* First Steps */
  err = generic_midat_calib_two_factor_new(iNbExe, iCalibLong, dLongVols,
                                           iCalibShort, dShortVols, sModel,
                                           sParams, sUsedCalibInfos);
  ...
  while (iter < sParams->iMaxBetaIterations) {
    ...
    if (!err) {
      ...
      if (fabs(dLastBeta2 - sParams->dLastBeta2) <
    ...
    if (dLastOpt > iOptMax ||
        (dLastOpt == iOptMax && dLastSigma2 > dSigma2Max)) {
      if (dLastOpt > iOptMax) {
        /* True improvement */
        iNbSameDir = 0;
        iNbBadIter = 0;
    ...
    if (err || lastiter == 1) {
      if (fabs(iNbSameDir) > sParams->iMaxSameDir) {
        ...
        if (iNbSameDir > 0) {
          ...
          dStartBeta2 = dResIter[0][iter - 1] +
                        (dResIter[0][iter - 1] - dResIter[0][iter - 2]);
          iNbSameDir++;
        }
      } else {
        if (iIndexMax > 0 && iIndexMax < iter - 1) {
          ...
          if (dResIter[1][iIndexMax - 1] > dResIter[1][iIndexMax + 1]) {
            if (iNbBadIter <= sParams->iMaxBadIter) {
              dStartBeta2 =
                  0.5 * (dResIter[0][iIndexMax - 1] + dResIter[0][iIndexMax]);

              iNbSameDir = min(iNbSameDir - 1, -1);
            } else {
              dStartBeta2 =
                  0.5 * (dResIter[0][iIndexMax] + dResIter[0][iIndexMax + 1]);
              ...
              if (iNbReallyBadIter > sParams->iMaxReallyBadIter) {
                ...
                if (l < 0) {
                  dStartBeta2 =
                      dResIter[0][0] - (dResIter[0][1] - dResIter[0][0]);
                } else {
            ...
            if (iNbBadIter <= sParams->iMaxBadIter) {
              ...
              if (iNbReallyBadIter > sParams->iMaxReallyBadIter) {
                ...
                l = iIndexMax + 1;
                while (l < iter &&
                       dResIter[1][iIndexMax + 1] == dResIter[1][l]) {
                  l++;
                }
          ...
          if (iNbBadIter <= sParams->iMaxBadIter) {
            dStartBeta2 = dResIter[0][iIndexMax] -
                          (dResIter[0][iIndexMax + 1] - dResIter[0][iIndexMax]);
          ...
          if (iNbBadIter <= sParams->iMaxBadIter) {
            dStartBeta2 = dResIter[0][iIndexMax] +
                          (dResIter[0][iIndexMax] - dResIter[0][iIndexMax - 1]);
      ...
      dStartBeta2 =
          solve_for_next_coef_gen(dResIterBeta, lastiter, sParams->dLastBeta2,
                                  sParams->dLastBeta2Tolerance, 2);
...
FREE_RETURN:
  ...
  if (!sCalibInfos && sUsedCalibInfos)
    free(sUsedCalibInfos);

  if (dResIter)
    free_dmatrix(dResIter, 0, 3, 0, sParams->iMaxBetaIterations);

  if (dResIterBeta)
    free_dmatrix(dResIterBeta, 0, sParams->iMaxBetaIterations, 0, 1);
...
Err generic_midat_calib_two_factor_new(/* Input */
  ...
  if (sParams->iCalibLambda) {
    /* Special case for the first Lambda */
    sModel->dBeta[0] = 1.0;
    sModel->dBeta2[0] = dStartBeta2 + (sModel->dBeta[0] - dStartBeta) *
                                          exp(-sModel->dGamma * dDeltaCalib);
  }
  ...
  for (i = 0; i < iNbExe; i++) {
    if (i > 0) {
      ...
      dFact0 = (1.0 / dExpSumLam / dExpSumLam -
                1.0 / dLastExpSumLam / dLastExpSumLam) /
               (2.0 * sModel->dLambda[i]);
      dFact1 =
          (exp(sModel->dGamma * sModel->dTimes[i]) / dExpSumLam / dExpSumLam -
           exp(sModel->dGamma * sModel->dTimes[i - 1]) / dLastExpSumLam /
               dLastExpSumLam) /
          (2.0 * sModel->dLambda[i] + sModel->dGamma);
      dFact2 = (exp(2.0 * sModel->dGamma * sModel->dTimes[i]) / dExpSumLam /
                    dExpSumLam -
                exp(2.0 * sModel->dGamma * sModel->dTimes[i - 1]) /
                    dLastExpSumLam / dLastExpSumLam) /
               (2.0 * (sModel->dLambda[i] + sModel->dGamma));
    } else {
      dt = sModel->dTimes[0];
      ...
      if (fabs(sModel->dLambda[0]) < 1.0E-10) {
        ...
        dFact0 = (exp(2.0 * sModel->dLambda[0] * sModel->dTimes[0]) - 1.0) /
                 (2.0 * sModel->dLambda[0]);
      }

      dFact1 = (exp((2.0 * sModel->dLambda[0] + sModel->dGamma) *
                    sModel->dTimes[0]) -
                1.0) /
               (2.0 * sModel->dLambda[0] + sModel->dGamma);
      dFact2 = (exp(2.0 * (sModel->dLambda[0] + sModel->dGamma) *
                    sModel->dTimes[0]) -
                1.0) /
               (2.0 * (sModel->dLambda[0] + sModel->dGamma));
    }
    ...
    dSigma2 /= sModel->dBeta[i] * sModel->dBeta[i] * dFact0 +
               2.0 * sModel->dBeta[i] * sModel->dBeta2[i] * sModel->dRhoAlpha *
                   dFact1 +
    ...
    if (i > 0) {
      dSigma2 =
          max(dSigma2, (sParams->dMinChangeVol * sModel->dSigma[i - 1]) *
                           (sParams->dMinChangeVol * sModel->dSigma[i - 1]) *
                           dt / dFact0);
      dSigma2 =
          min(dSigma2, (sParams->dMaxChangeVol * sModel->dSigma[i - 1]) *
                           (sParams->dMaxChangeVol * sModel->dSigma[i - 1]) *
                           dt / dFact0);
    }
    ...
    if (i < iNbExe - 1 && sParams->iCalibLambda) {
      ...
      dPhi0 = dVarX;
      dPhi1 =
          exp(-sModel->dGamma * (sModel->dTimes[i + 1] - sModel->dTimes[i])) *
              dPhi1 +
          exp(-sModel->dGamma * sModel->dTimes[i + 1]) * sModel->dCorrel[i];

      dPhi2 = exp(-2.0 * sModel->dGamma *
                  (sModel->dTimes[i + 1] - sModel->dTimes[i])) *
                  dPhi2 +
              exp(-2.0 * sModel->dGamma * sModel->dTimes[i + 1]) *
                  sModel->dSigma2[i];
      ...
      sModel->dLambda[i + 1] = (-log(sModel->dBeta[i + 1]) - dSumLam) /
                               (sModel->dTimes[i + 1] - sModel->dTimes[i]);
    ...
    if (i < iNbExe - 1) {
      ...
      sModel->dBeta2[i + 1] =
          sModel->dBeta2[i] + (sModel->dBeta[i + 1] - sModel->dBeta[i]) *
                                  exp(-sModel->dGamma * sModel->dTimes[i + 1]);
      dLastSumLam = dSumLam;
      dLastExpSumLam = dExpSumLam;
      dSumLam = dLastSumLam + sModel->dLambda[i + 1] *
                                  (sModel->dTimes[i + 1] - sModel->dTimes[i]);
      dExpSumLam = exp(-dSumLam);
    }
...
static double generic_midat_LocalVolInteg(double t1, double t2, double T1,
                                          double T2, GENMIDAT_MODEL sModel) {
  double res;
  ...
  res += sModel->dAlpha * sModel->dRho *
         (exp(-sModel->dGamma * (T1 - t2)) + exp(-sModel->dGamma * (T2 - t2)) -
          exp(-sModel->dGamma * (T1 - t1)) - exp(-sModel->dGamma * (T2 - t1))) /
         sModel->dGamma;

  res += sModel->dAlpha * sModel->dAlpha *
         (exp(-sModel->dGamma * (T1 + T2 - 2.0 * t2)) -
          exp(-sModel->dGamma * (T1 + T2 - 2.0 * t1))) /
         (2.0 * sModel->dGamma);
...
Err GenMidatCalib_GetTarget(void *Inst_, void *Params_, void *Model_,
...
Err GenMidatCalib_GetFirstGuess(void *Model_, void *Params_, int index_param,
...
Err GenMidatCalib_GetSecondGuess(void *Model_, void *Params_, int index_param,
...
Err GenMidatCalib_BumpParam(void *Model_, void *Params_, int index_param,
                            double param1) {
  GenMidatCalib_MODEL Model = (GenMidatCalib_MODEL)Model_;
...
Err GenMidatCalib_SetParam(void *Model_, CALIBGEN_PARAMS CalibConsts,
  ...
  /* Update Factors */
  Inst->dFact0 = (1.0 / (Model->dBeta * Model->dBeta) -
                  1.0 / (Model->dLastBeta * Model->dLastBeta)) /
                 (2.0 * Model->dLambda);

  Inst->dFact1 =
      (1.0 / (Inst->dExpFact1 * Model->dBeta * Model->dBeta) -
       1.0 / (Inst->dLastExpFact1 * Model->dLastBeta * Model->dLastBeta)) /
      (2.0 * Model->dLambda + Model->sModel->dGamma);

  Inst->dFact2 =
      (1.0 / (Inst->dExpFact2 * Model->dBeta * Model->dBeta) -
       1.0 / (Inst->dLastExpFact2 * Model->dLastBeta * Model->dLastBeta)) /
      (2.0 * (Model->dLambda + Model->sModel->dGamma));

  /* Calibrate the volatility */

  Model->dSigma2 = Inst->dLongVariance / (Model->dBeta * Model->dBeta) -
                   Model->dVarX - 2.0 * Model->dCoVar * Inst->dExpFact1 -
  ...
  Model->sModel->dCorrel[index_param] =
      Model->sModel->dRhoAlpha * Model->dSigma2 * Inst->dFact1;
  Model->sModel->dSigma2[index_param] =
      Model->sModel->dAlpha2 * Model->dSigma2 * Inst->dFact2;
  ...
  Model->sModel->dSigma[index_param] =
      sqrt(Model->sModel->dSigma[index_param] / Inst->dDt);
  Model->sModel->dSigma2[index_param] =
      sqrt(Model->sModel->dSigma2[index_param] / Inst->dDt);
  Model->sModel->dCorrel[index_param] /= Model->sModel->dSigma[index_param] *
                                         Model->sModel->dSigma2[index_param] *
                                         Inst->dDt;
...
Err GenMidatCalib_ExtrapolParam(void *Model_, void *Params_, int index_param) {
  return NULL;
}

Err GenMidatCalib_UpdateConstsAfterParam(void *Inst_, void *InstConst_,
  ...
  if (Inst->iIndex == 1) {
    ...
    err =
        GenMidatCalib_SetParam(Model_, CalibConsts, Params_, 0, Model->dLambda);
  }
...
Err GenMidatCalib_PriceInst(void *Inst_, void *InstConst_, void *Params_,
  ...
  diffbet2 =
      Model->dBeta * NxtInst->dExpFact1 - Model->dLastBeta * Inst->dExpFact1;
...
Err generic_midat_calib_two_factor(/* Input */
  ...
  if (sParams->iCalibLambda && iNbExe > 1) {
    ...
    if (ratio > 0.0) {
      sModel->dBeta[0] = pow(
          ratio, sModel->dTimes[0] / (sModel->dTimes[1] - sModel->dTimes[0]));
      sModel->dBeta[1] =
          pow(sModel->dBeta[0], sModel->dTimes[1] / sModel->dTimes[0]);
    } else {
  ...
  err = Initialise_CalibParams(1, 0.00001, 7, 1, sParams->iCalibLambda, 1, 0,
                               0.0, CalibParams);
  ...
  /* Launch the Newton */
  err = CalibrateParamTS(1, iNbExe - 1, AllInst, AllInst, Params, CalibModel,
                         CalibParams, AllFunctions);

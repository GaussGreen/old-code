...
static Err generic_midat_interpolate_vol(
    int iNbExe, double* dMaturity, double* dVols, int* iCalibVols, int iInterpType)
{
    ...
    for (i = 0; i < iNbExe; i++)
    {
        if (!iCalibVols[i])
        {
            ...
            case 0:
            {
                ...
                dVols[i] = dVols[index_down] + (dVols[index_up] - dVols[index_down]) /
                        ...
                        GENMIDAT_CALIBINFOS sCalibInfos)
{
    ...
    for (i = 0; i < iNbExe; i++)
    {
        ...
        else
        {
            ...
            {
                err = serror(
                    "Long Option %d: Option < Intrinseq or cannot solve for volatility !", i + 1);
                goto FREE_RETURN;
            }
        }
    }
    ...
    if (sParams->iInterpFailedVol)
    {
        err = generic_midat_interpolate_vol(
            iNbExe, sModel->dTimes, dLongVols, iCalibLong, sParams->iInterpVolType);
    ...
    if (sParams->iCalibLambda)
    {
        if (sParams->iShortIsCap)
        {
            for (i = 0; i < iNbExe - 1; i++)
            {
                    ...
                    fabs(dShortOption[i]) > 1.0E-10)
                {
                    ...
                    if (err || fabs((dShortVols[i] - (dShortFwd + dShiftFwd) * 1.0e-9)) <
                ...
                else
                {
                    ...
                    {
                        err = serror(
                            "Short Option %d: Option < Intrinseq or cannot solve for vol !", i + 1);
                        goto FREE_RETURN;
                    }
                }
            }
        ...
        {
            err = generic_midat_interpolate_vol(
                iNbExe - 1, sModel->dTimes, dShortVols, iCalibShort, sParams->iInterpVolType);
    ...
    {
        err = generic_midat_calib_one_factor(
            iNbExe, iCalibLong, dLongVols, iCalibShort, dShortVols, sModel, sParams, sCalibInfos);
    }
    else if (sModel->iNbFactor == 2)
    {
        err = generic_midat_calib_two_factor_total(
            iNbExe, iCalibLong, dLongVols, iCalibShort, dShortVols, sModel, sParams, sCalibInfos);
    }
    else
    {
...
Err generic_midat_find_const_lambda(double dT0, double dT1, double dRatio, double* dLambda)
{
                                   ...
                                   GENMIDAT_CALIBINFOS sCalibInfos)
{
    ...
    for (i = 1; i < iNbExe; i++)
    {
        ...
        {
            sModel->dSigma[i] =
                sqrt(sModel->dSigma[i] / (sModel->dTimes[i] - sModel->dTimes[i - 1])) /
                sModel->dBeta[i];
        }
        else
        {
...
static double generic_midat_get_last_beta2(GENMIDAT_MODEL sModel)
{
    ...
    if (sModel->iNbVols > 1)
    {
        ...
        dLastBeta2 = sModel->dBeta2[sModel->iNbVols - 1] -
                                         ...
                                         GENMIDAT_CALIBINFOS sCalibInfos)
{
    ...
    /* First Steps */
    err = generic_midat_calib_two_factor_new(
        iNbExe, iCalibLong, dLongVols, iCalibShort, dShortVols, sModel, sParams, sUsedCalibInfos);
    ...
    while (iter < sParams->iMaxBetaIterations)
    {
        ...
        if (err || lastiter == 1)
        {
            if (fabs(iNbSameDir) > sParams->iMaxSameDir)
            {
                ...
                {
                    dStartBeta2 =
                        dResIter[0][iter - 1] + (dResIter[0][iter - 1] - dResIter[0][iter - 2]);
                    iNbSameDir++;
                }
            }
            else
        ...
        else
        {
            ...
            dStartBeta2 = solve_for_next_coef_gen(
                dResIterBeta, lastiter, sParams->dLastBeta2, sParams->dLastBeta2Tolerance, 2);
                                       ...
                                       GENMIDAT_CALIBINFOS sCalibInfos)
{
    ...
    if (sParams->iCalibLambda)
    {
        ...
        sModel->dBeta[0] = 1.0;
        sModel->dBeta2[0] =
            dStartBeta2 + (sModel->dBeta[0] - dStartBeta) * exp(-sModel->dGamma * dDeltaCalib);
    }
    ...
    for (i = 0; i < iNbExe; i++)
    {
        if (i > 0)
        {
            ...
            dFact0 = (1.0 / dExpSumLam / dExpSumLam - 1.0 / dLastExpSumLam / dLastExpSumLam) /
                     (2.0 * sModel->dLambda[i]);
            dFact1 =
                (exp(sModel->dGamma * sModel->dTimes[i]) / dExpSumLam / dExpSumLam -
                 exp(sModel->dGamma * sModel->dTimes[i - 1]) / dLastExpSumLam / dLastExpSumLam) /
                (2.0 * sModel->dLambda[i] + sModel->dGamma);
            dFact2 = (exp(2.0 * sModel->dGamma * sModel->dTimes[i]) / dExpSumLam / dExpSumLam -
                      exp(2.0 * sModel->dGamma * sModel->dTimes[i - 1]) / dLastExpSumLam /
                          dLastExpSumLam) /
                     (2.0 * (sModel->dLambda[i] + sModel->dGamma));
        }
        else
        {
            ...
            dFact1 = (exp((2.0 * sModel->dLambda[0] + sModel->dGamma) * sModel->dTimes[0]) - 1.0) /
                     (2.0 * sModel->dLambda[0] + sModel->dGamma);
            dFact2 = (exp(2.0 * (sModel->dLambda[0] + sModel->dGamma) * sModel->dTimes[0]) - 1.0) /
                     (2.0 * (sModel->dLambda[0] + sModel->dGamma));
        }
            ...
            dSigma2 =
                max(dSigma2,
                    (sParams->dMinChangeVol * sModel->dSigma[i - 1]) *
                        (sParams->dMinChangeVol * sModel->dSigma[i - 1]) * dt / dFact0);
            dSigma2 =
                min(dSigma2,
                    (sParams->dMaxChangeVol * sModel->dSigma[i - 1]) *
                        (sParams->dMaxChangeVol * sModel->dSigma[i - 1]) * dt / dFact0);
        }
        ...
        if (i < iNbExe - 1 && sParams->iCalibLambda)
        {
            ...
            dPhi2 =
                exp(-2.0 * sModel->dGamma * (sModel->dTimes[i + 1] - sModel->dTimes[i])) * dPhi2 +
                exp(-2.0 * sModel->dGamma * sModel->dTimes[i + 1]) * sModel->dSigma2[i];

            sModel->dBeta[i + 1] =
                dShortVols[i] * dShortVols[i] * sModel->dTimes[i] / (dPhi0 + 2.0 * dPhi1 + dPhi2);
        ...
        if (i < iNbExe - 1)
        {
            ...
            dSumLam =
                dLastSumLam + sModel->dLambda[i + 1] * (sModel->dTimes[i + 1] - sModel->dTimes[i]);
            dExpSumLam = exp(-dSumLam);
        }
...
static double generic_midat_LocalVolInteg(
    double t1, double t2, double T1, double T2, GENMIDAT_MODEL sModel)
{
    double res;
...
Err GenMidatCalib_GetTarget(
    void* Inst_, void* Params_, void* Model_, CALIBGEN_PARAMS CalibConsts, double* target)
{
...
Err GenMidatCalib_GetFirstGuess(
    void* Model_, void* Params_, int index_param, double target, double* param1)
{
...
Err GenMidatCalib_GetSecondGuess(
    void*   Model_,
    void*   Params_,
    int     index_param,
    double  param1,
...
Err GenMidatCalib_GetLimitAndLastParam(
    void*           Model_,
...
Err GenMidatCalib_BumpParam(void* Model_, void* Params_, int index_param, double param1)
{
    GenMidatCalib_MODEL Model = (GenMidatCalib_MODEL)Model_;
...
Err GenMidatCalib_SetParam(
    void* Model_, CALIBGEN_PARAMS CalibConsts, void* Params_, int index_param, double param1)
{
    GenMidatCalib_MODEL  Model  = (GenMidatCalib_MODEL)Model_;
    ...
    /* Update Factors */
    Inst->dFact0 =
        (1.0 / (Model->dBeta * Model->dBeta) - 1.0 / (Model->dLastBeta * Model->dLastBeta)) /
        (2.0 * Model->dLambda);

    Inst->dFact1 = (1.0 / (Inst->dExpFact1 * Model->dBeta * Model->dBeta) -
                    1.0 / (Inst->dLastExpFact1 * Model->dLastBeta * Model->dLastBeta)) /
                   (2.0 * Model->dLambda + Model->sModel->dGamma);

    Inst->dFact2 = (1.0 / (Inst->dExpFact2 * Model->dBeta * Model->dBeta) -
                    1.0 / (Inst->dLastExpFact2 * Model->dLastBeta * Model->dLastBeta)) /
                   (2.0 * (Model->dLambda + Model->sModel->dGamma));

    /* Calibrate the volatility */

    Model->dSigma2 = Inst->dLongVariance / (Model->dBeta * Model->dBeta) - Model->dVarX -
                     2.0 * Model->dCoVar * Inst->dExpFact1 - Model->dVarY * Inst->dExpFact2;
    ...
    Model->sModel->dCorrel[index_param] /=
        Model->sModel->dSigma[index_param] * Model->sModel->dSigma2[index_param] * Inst->dDt;
...
Err GenMidatCalib_UpdateConstsAfterParam(
    void* Inst_, void* InstConst_, void* Params_, void* Model_, CALIBGEN_PARAMS CalibConsts)
{
    GenMidatCalib_PARAMS Params = (GenMidatCalib_PARAMS)Params_;
    GenMidatCalib_MODEL  Model  = (GenMidatCalib_MODEL)Model_;
    GenMidatCalib_INST   Inst   = (GenMidatCalib_INST)Inst_;
    ...
    if (Inst->iIndex == 1)
    {
        ...
        err = GenMidatCalib_SetParam(Model_, CalibConsts, Params_, 0, Model->dLambda);
    }
...
Err GenMidatCalib_PriceInst(
    void* Inst_, void* InstConst_, void* Params_, void* Model_, double* InstPrice)
{
    GenMidatCalib_PARAMS Params  = (GenMidatCalib_PARAMS)Params_;
    GenMidatCalib_MODEL  Model   = (GenMidatCalib_MODEL)Model_;
                                   ...
                                   GENMIDAT_CALIBINFOS sCalibInfos)
{
    ...
    if (sParams->iCalibLambda && iNbExe > 1)
    {
        ...
        {
            sModel->dBeta[0] =
                pow(ratio, sModel->dTimes[0] / (sModel->dTimes[1] - sModel->dTimes[0]));
    ...
    err = Initialise_CalibParams(1, 0.00001, 7, 1, sParams->iCalibLambda, 1, 0, 0.0, CalibParams);
    ...
    err = CalibrateParamTS(
        1, iNbExe - 1, AllInst, AllInst, Params, CalibModel, CalibParams, AllFunctions);

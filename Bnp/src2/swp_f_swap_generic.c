   ...
   (the coverage is stroed on the last date of the period) */
	...
	switch (leg_type)
	{
		...
		/* Sets the spreads RefRate (Libor,...) vs Cash + cash_fra + exchange of notionals */
			...
			if ( (leg->spread).len == 0 )
				return serror("Error in computing spreads in swap leg");
			err = fwd_cash_list( leg->start_date,leg->end_date, leg_sdp->basis_code, curve_name,&(leg->fwd));
		break;
		case DRS_LEG:
		break;
		case CMS_MARGIN_LEG:
...
/* According to the swap type, sets the legs accordingly */
	...
	/* Fixed leg vs exchange of notionals (initial & final) leg*/	
		case FIXED_NOTIONALS_SWAP:
			err = create_GenSwapLeg( today, rec_leg_sdp, fixed,
					0.0, 0.0, FIXED_LEG, SRT_RECEIVER, curve_name, NULL,
					&full_swap.leg[0] );
			if (err)
				return (err);
			err = create_GenSwapLeg( today, pay_leg_sdp, 0.0,
					1.0, 1.0, NOTIONALS_LEG, SRT_PAYER, curve_name, NULL,
					&full_swap.leg[1]);
			if (err)
				return (err);
		break;
	...
	/* Fixed leg vs floating (with cash spreads) leg */
		case FIXED_FLOATING_SWAP:
			...
			if (err)
				return (err);
			err = create_GenSwapLeg( today, pay_leg_sdp, 0.0,
					1.0, 1.0, FLOATING_LEG, SRT_PAYER, curve_name, NULL,
					&full_swap.leg[1]);
			if (err)
				return (err);
		break;
	...
	/* DRS leg minus margin vs floating (with cash spreads) */
		case DRS_MARGIN_FLOATING_SWAP:
			err = create_GenSwapLeg( today, rec_leg_sdp, 0.0,
					0.0, 0.0, DRS_MARGIN_LEG, SRT_RECEIVER, curve_name, NULL,
					&full_swap.leg[0] );
			if (err)
				return (err);
			err = create_GenSwapLeg( today, pay_leg_sdp, 0.0,
	...
	/* DRS leg minus margin vs fixed leg */
		case DRS_MARGIN_FIXED_SWAP:
			...
			if (err)
				return (err);
			err = create_GenSwapLeg( today, pay_leg_sdp, 0.0,
					1.0, 1.0, FIXED_LEG, SRT_PAYER, curve_name, NULL,
					&full_swap.leg[1]);
			if (err)
				return (err);
		break;
...
*/		
	...
	/* CMS minus margin leg vs Libor leg(with cash spreads) */
		case CMS_MARGIN_FLOATING_SWAP:
			err = create_GenSwapLeg( today, rec_leg_sdp, 0.0,
					0.0, 0.0, CMS_MARGIN_LEG, SRT_RECEIVER, curve_name, NULL,
					&full_swap.leg[0] );
			if (err)
				return (err);
			err = create_GenSwapLeg( today, pay_leg_sdp, 0.0,
	...
	/* CMS minus margin (the TEC way) leg vs fixed leg */
		case CMS_FOR_TEC_MARGIN_FLOATING_SWAP:
			err = create_GenSwapLeg( today, rec_leg_sdp, 0.0,
					0.0, 0.0, CMS_FOR_TEC_MARGIN_LEG, SRT_RECEIVER, curve_name, NULL,
					&full_swap.leg[0]);
			if (err)
				return (err);
			err = create_GenSwapLeg( today, pay_leg_sdp, 0.0,
	...
	/* CMS minus margin (the TEC way) leg vs fixed leg */
		case CMS_FOR_TEC_MARGIN_FIXED_SWAP:
			...
			if (err)
				return (err);
			err = create_GenSwapLeg( today, pay_leg_sdp, fixed,
					1.0, 1.0, FIXED_LEG, SRT_PAYER, curve_name, NULL,
					&full_swap.leg[1]);
			if (err)
				return (err);
	...
	/* CMT minus margin leg vs Libor leg (with cash spreads) */
		case CMT_MARGIN_FLOATING_SWAP:
			err = create_GenSwapLeg( today, rec_leg_sdp, 0.0,
					0.0, 0.0, CMT_MARGIN_LEG, SRT_RECEIVER, curve_name, NULL,
					&full_swap.leg[0]);
			if (err)
				return (err);
			err = create_GenSwapLeg( today, pay_leg_sdp, 0.0,
	...
	/* TEC minus margin (the TEC way) leg vs Libor leg (with cash spreads) */
		case TEC_MARGIN_FLOATING_SWAP:
			err = create_GenSwapLeg( today, rec_leg_sdp, 0.0,
					0.0, 0.0, TEC_MARGIN_LEG, SRT_RECEIVER, curve_name, NULL,
					&full_swap.leg[0]);
			if (err)
				return (err);
			err = create_GenSwapLeg( today, pay_leg_sdp, 0.0,
	...
	/* CMS minus margin leg vs fixed leg */
		case CMS_MARGIN_FIXED_SWAP:
			...
			if (err)
				return (err);
			err = create_GenSwapLeg( today, pay_leg_sdp, fixed,
					0.0, 0.0, FIXED_LEG, SRT_PAYER, curve_name, NULL,
					&full_swap.leg[1]);
			if (err)
				return (err);
		break;
...
*/   
static Err populate_GenSwapLeg( 
			double	          margin,
			SrtCurvePtr         crv,
			GenSwapLeg        *leg )
{
...
/* Sets all the payments on the leg depending on the type of leg */
	switch (leg->leg_type)
	{
		...
		case CMS_MARGIN_LEG:
			...
			{
				leg->payment.d[i] = (leg->cpn.d[i] + leg->margin)
						* leg->cvg.d[i-1];
			}			
		break;
		case CMT_MARGIN_LEG:
			...
			if (err)
				return (err);
			err = treas_vol_list(	leg->pay_date, 
										leg->temp_fwd,
										leg->fwd,
										crv,
										&(leg->vol));
		...
		case CMS_FOR_TEC_MARGIN_LEG:
			...
			if (err)
  			...
  			for (i=1;i<leg->pay_date.len;i++)
			{
				leg->payment.d[i] = exp ( 1.0 / (double)(leg->sdp.compd) 
										 * log( 1.0 + leg->cpn.d[i] ) ) - 1.0 ;
			}			
		break;
		case TEC_MARGIN_LEG:
			err = fwd_treas_list(leg->pay_date, crv,&(leg->fwd));
...
/* Create the fixed and notionals leg: dates, cvg, coupon, initial, final */
	err = create_GenSwapLeg( today, fixedsdp, fixed, initial, final,
			FIXED_AND_NOTIONALS_LEG, SRT_RECEIVER, NULL, NULL, fixedleg);
	if (err)
	{
		swp_f_freein_GenSwapLeg(fixedleg);
		return err;
...
/* Create the notionals leg: initial, final */
	err = create_GenSwapLeg( today, fixedsdp, 0.0, initial, final,
			NOTIONALS_LEG, SRT_RECEIVER, NULL, NULL, fixedleg);
	if (err)
	{
		swp_f_freein_GenSwapLeg(fixedleg);
		return err;
...
/* Create the floating leg: dates, cvg, spreads fra's */
	err = create_GenSwapLeg(today, &floatsdp, 0.0, 0.0, 0.0, 
				FLOATING_LEG, SRT_RECEIVER, get_curve_name(crv), ref_rate_code, floatleg);
	if (err)
	{
...
/* Create the floating leg: dates, cvg, spreads  */
	err =  create_GenSwapLeg(today, &spreadsdp, 0.0, 0.0, 0.0, 
				SPREAD_LEG, SRT_RECEIVER, NULL, ref_rate_code, spreadleg);
	if (err)
	{
...
/* Create the floating leg: dates, cvg, spreads  */
	err =  create_GenSwapLeg(today, &floatsdp, 0.0, initial, final,
				SPREAD_AND_NOTIONALS_LEG, SRT_RECEIVER, NULL, ref_rate_code, floatleg);
	if (err)
	{
		swp_f_freein_GenSwapLeg(floatleg);
		return err;
...
/* Create a fixed leg with dates and cvg  and a coupon of 1.0 */
	err = create_GenSwapLeg(today, swapdp, 1.0, 0.0, 0.0, FIXED_LEG,
				SRT_RECEIVER, NULL, NULL, &fixedleg);
	if (err)
	{
		swp_f_freein_GenSwapLeg(&fixedleg);

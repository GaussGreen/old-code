...
static Err create_GenSwapLeg(long today, SwapDP *leg_sdp, double fixed,
  ...
  case SPREAD_LEG:
    ...
    break;
  case FLOATING_LEG:
    /* Sets the spreads RefRate (Libor  ,...) vs Cash + cash_fra + exchange of
     * notionals */
    err = spread_list(leg->start_date, leg->end_date, ref_rate_code,
                      &(leg->spread));
    if (err)
...
Err swp_f_make_FixedAndNotionalsLeg(SwapDP *fixedsdp, double fixed,
  ...
  /* Create the fixed and notionals leg: dates  , cvg  , coupon  , initial  ,
   * final */
...
Err swp_f_make_SpreadLeg(long start, long end, Date today, String ref_rate_code,
  ...
  /* Set up the corresponding SwapDP: start  , end  ,... generated BACKWARD for
   * theo end date */
...
Err swp_f_make_FloatLegDatesAndCoverages(SwapDP *swapdp, Date today,
  ...
  /* Make a CASH floating leg (without computing the FRA's): dates  , times  ,
   * cvg */

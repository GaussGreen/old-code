...
void LGMSVComputeIntegralLaguerre(
    int     iNbX,
    double* X,
    double* W,
    double  Alpha,
...
void LGMSVComputeIntegralLegendre(
    int     iNbX,
    double* X,
    double* W,
    double* IntRe,
...
void LGMSVComputeIntegralHermite(
    int     iNbX,
    double* X,
    double* W,
    double* IntRe,
    ...
    double* result)
{
    ...
    for (j = 0; j < iNbStrike; j++)
    {
        ...
        result[j] =
            temp / PI / (exp(dIntegParam * dLogStrike[j]) - exp(-dIntegParam * dLogStrike[j]));
    }
}

void LGMSVConstructLegendreGrid(
    ...
    double* Price)
{
    ...
    LGMSVSolFunc FuncX_, *FuncX = &FuncX_, FuncX2_, *FuncX2 = &FuncX2_, FuncV_, *FuncV = &FuncV_,
                         FuncV2_, *FuncV2 = &FuncV2_, FuncXV_, *FuncXV = &FuncXV_;
    ...
    for (i = i; i <= endi; i++)
    {
        ...
        Coef2ReT[i] = dLambdaEps[i - do_switch] + dCoefMeanRev2 * dAlpha[i - do_switch] *
                                                      dRho[i - do_switch] * dSigma[i - do_switch];
                                    ...
                                    double* dCoefVol)
{
    ...
    err      = srt_f_optimpvol(
        price[1], swp_cash, strike[1], dExeTime, level, SRT_PUT, SRT_NORMAL, &(vol[1]));
    ...
    err      = srt_f_optimpvol(
        price[0], swp_cash, strike[0], dExeTime, level, SRT_PUT, SRT_NORMAL, &(vol[0]));
    ...
    err      = srt_f_optimpvol(
        price[2], swp_cash, strike[2], dExeTime, level, SRT_PUT, SRT_NORMAL, &(vol[2]));
    ...
    double* dFwd)
{
    ...
    if (NumerParams->iCalibLGM)
    {
        ...
        Calculate_CoefCMSandMeanRev_LGMSV(
            ncpn,
            cpn_time,
            cpn_cvg,
            cpn_df,
    ...
    err = swp_f_ForwardRate(
        lStartDate, lEndDate, swaption_freq, swaption_basis, yc_name, ref_rate_name, &swp_rate);
    ...
    if (CalibInst->dExeTime > 1.0E-08)
    {
        ...
        if (model->iOne2F == 2)
        {
            ...
            for (i = 0; i < index + 1; i++)
            {
                ...
                cum_var2 += model->dLGMAlpha[i] * model->dLGMAlpha[i] * model->dSigma[i] *
                            model->dSigma[i] * (t2 - t1);
                cum_var12 += model->dLGMAlpha[i] * model->dLGMRho[i] * model->dSigma[i] *
                             model->dSigma[i] * (t2 - t1);
            }
        ...
        if (CalibInst->iNbCoupon == 2)
        {
            ...
            HestonParam->dCoefVol =
                (exp(-(CalibCpnSchedule->dCpnTime[CalibInst->iStartCpn] - model->dTStar) /
                     model->dTau) -
                 exp(-(CalibCpnSchedule->dCpnTime[CalibInst->iEndCpn] - model->dTStar) /
                     model->dTau)) *
                model->dTau;
        ...
        else
        {
            ...
            else
            {
                ...
                HestonParam->dCoefVol =
                    HestonParam->dCoefVol /
                    sqrt(
                        (cum_var + cum_var2 * ratio2F * ratio2F + 2.0 * cum_var12 * ratio2F) /
                        CalibInst->dExeTime);
    ...
    LGMSV_HESTONINST HestonParam)
{
    ...
    {
        expTi1 = exp(
            -(CalibCpnSchedule->dCpnTime[CalibInst->iStartCpn + i] - model->dTStar) / model->dTau);
    ...
    if (model->iOne2F == 2)
    {
        ...
        {
            expTi1 =
                exp(-(CalibCpnSchedule->dCpnTime[CalibInst->iStartCpn + i] - model->dTStar) /
                    model->dTau2);
            dCoefMeanRev += CalibCpnSchedule->dCpnDf[CalibInst->iStartCpn + i] * (1.0 - expTi1);
        }
        ...
        {
            temp =
                (1.0 - exp(-(CalibInst->dPayTime - model->dTStar) / model->dTau2)) * model->dTau2;
            dCoefCMS     = dCoefMeanRev - temp;
            dCoefMeanRev = temp;
            ...
            if (CalibInst->iIsLiborOption)
            {
                temp = (exp(-(CalibInst->dLiborEndTime - model->dTStar) / model->dTau2) -
                        exp(-(CalibInst->dLiborStartTime - model->dTStar) / model->dTau2)) *
                       model->dTau2;
                dCoefCMS -= temp;
                dCoefMeanRev += temp;
    ...
    HestonParam->dCpnG1[0] =
        (1.0 - exp(-CalibCpnSchedule->dCpnTime[CalibInst->iStartCpn] / model->dTau)) * model->dTau;

    for (i = 1; i < CalibInst->iNbCoupon; i++)
    {
        HestonParam->dCpnG1[i] =
            (1.0 - exp(-CalibCpnSchedule->dCpnTime[CalibInst->iStartCpn + i] / model->dTau)) *
            model->dTau;
    }
    ...
    if (model->iOne2F == 2)
    {
        HestonParam->dCpnG2[0] =
            (1.0 - exp(-CalibCpnSchedule->dCpnTime[CalibInst->iStartCpn] / model->dTau2)) *
            model->dTau2;

        for (i = 1; i < CalibInst->iNbCoupon; i++)
        {
            HestonParam->dCpnG2[i] =
                (1.0 - exp(-CalibCpnSchedule->dCpnTime[CalibInst->iStartCpn + i] / model->dTau2)) *
                model->dTau2;
        }
    ...
    if (CalibInst->dExeTime > 1.0E-08)
    {
        ...
        if (model->iOne2F == 1)
        {
            ...
            for (i = 0; i <= InstNumer->iSwitchIndex; i++)
            {
                ...
                NumerParams->Coef2ReT[i] =
                    model->dLambdaEps[i] + HestonParam->dCoefMeanRev * model->dAlpha[i] *
                                               model->dRho[i] * model->dSigma[i];
                ...
                NumerParams->Coef3ImT[i] =
                    0.5 * temp_vol - HestonParam->dNewCoefCMS * model->dSigma[i] * model->dSigma[i];
            ...
            for (i = i; i <= InstNumer->iEndIndex; i++)
            {
                ...
                NumerParams->Coef2ReT[i] =
                    model->dLambdaEps[i2] + HestonParam->dCoefMeanRev2 * model->dAlpha[i2] *
                                                model->dRho[i2] * model->dSigma[i2];
        ...
        else
        {
            ...
            for (i = 0; i <= InstNumer->iSwitchIndex; i++)
            {
                ...
                temp_vol_tot = sqrt(
                    temp_vol * temp_vol + temp_vol2 * temp_vol2 +
                    2.0 * model->dLGMRho[i] * temp_vol * temp_vol2);
                correl_tot =
                    (model->dRho[i] * temp_vol + model->dRho2[i] * temp_vol2) / temp_vol_tot;
            ...
            for (i = i; i <= InstNumer->iEndIndex; i++)
            {
                ...
                correl_tot =
                    (model->dRho[i2] * temp_vol + model->dRho2[i2] * temp_vol2) / temp_vol_tot;
                ...
                NumerParams->Coef3ReT[i] = 0.5 * temp_vol_tot;
                NumerParams->Coef3ImT[i] =
                    0.5 * temp_vol_tot - model->dSigma[i2] * model->dSigma[i2] *
        ...
        {
            /* simple unifrom grid */
            XMax = (NumerParams->NumerParams->dParam1 + NumerParams->NumerParams->dParam2) *
                   HestonParam->dCoefVol *
    ...
    else
    {
        for (i = 0; i < InstNumer->iNbStrike - do_fwd; i++)
        {
            if (NumerParams->NumerParams->dIntegParam < 0.0)
            {
                /* Payer */
                Price[i] =
                    DMAX(CalibInst->dFwdCash + CalibInst->dSpread - CalibInst->dStrike[i], 0.0);
            }
            else
            {
                /* Receiver */
                Price[i] =
                    DMAX(CalibInst->dStrike[i] - CalibInst->dFwdCash - CalibInst->dSpread, 0.0);
            }
        }
    }
    ...
    CALIBEXESCHEDULEDLM CalibExeSchedule)
{
    ...
    err = swp_f_initSwapDP(lStartDate, lTheoEndDate, cInstFreq, cInstBasis, &Swap);
    ...
    for (k = 0; k < CalibCpnSchedule->iNCpn; k++)
    {
        ...
        if (k > 0)
        {
            CalibCpnSchedule->dCpnCvg[k] =
                coverage(CalibCpnSchedule->lCpnDate[k - 1], CalibCpnSchedule->lCpnDate[k], ibasis);
        }
        else
        {
    ...
    double* pSwaptionPrice)
{
    ...
    if (!CpnSchedule || !ExeSchedule || !Inst || !AllInst || !model || !HestonParam ||
        !PricingConst || !InstNumer)
    {
    ...
    */
    ...
    err = Calculate_HestonEquivalent_LGMSV_FromLGM_struct(Inst, CpnSchedule, model, HestonParam);

    if (err)
        goto FREE_RETURN;

    err = Calculate_ShiftAndVol_LGMSV_FromLGM_struct(Inst, CpnSchedule, model, HestonParam);
    ...
    /* pricing */
    LGMSVClosedFormApprox_struct(model, Inst, HestonParam, PricingConst, InstNumer, pSwaptionPrice);
    ...
    double* pSwaptionPrice)
{
    ...
    if (!CpnSchedule || !ExeSchedule || !Inst || !AllInst || !sModel || !HestonParam ||
        !PricingConst || !InstNumer)
    {
        err = "Memory allocation faillure in LGMSVOptionApprox_new";
        goto FREE_RETURN;
    ...
    */
    ...
    err = Calculate_HestonEquivalent_LGMSV_FromLGM_struct(Inst, CpnSchedule, sModel, HestonParam);

    if (err)
        goto FREE_RETURN;

    err = Calculate_ShiftAndVol_LGMSV_FromLGM_struct(Inst, CpnSchedule, sModel, HestonParam);
    ...
    LGMSVClosedFormApprox_struct(
        sModel, Inst, HestonParam, PricingConst, InstNumer, pSwaptionPrice);
    ...
    double* dCmsRate)
{
    ...
    /* pricing */
    LGMSVFwdClosedFormApprox_struct(model, Inst, HestonParam, PricingConst, InstNumer, dCmsRate);

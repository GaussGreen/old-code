...
{
	lgmsv_app_set_default_params(	&(NumerParams->iNbX),
									&(NumerParams->dParam1),
									&(NumerParams->dParam2),
									&(NumerParams->dIntegParam),
									&(NumerParams->iIntegMethod),
									&(NumerParams->dVolLimit),
									&(NumerParams->iCalibLGM),
									&(NumerParams->dMinStd),
									&(NumerParams->dMaxStd));
}
					...
					LGMSVSolFunc	*FuncXV)
{
	...
	/*	dV = (lam2 - (lam2 + rho * alpha * k2 * f(t)) * V) * dt + alpha * Sqr(V) * dW2 */
	...
	/*	dV2 = ((2 * lam2 + alpha^2) * V - 2 * (lam2 + rho * alpha * k2 * f(t)) * V^2) * dt + ... */
	...
	/*	dXV = -0.5 * (k1 * f(t))^2 * V^2 * dt + lam2 * X * dt + rho * alpha * k1 * f(t) * V * dt 
...
int		i;	
	...
	*/
	...
	{					
		...
		LGMSVUpdateADClosedForm(dt[i], CoefInt[i], Coef1ReT[i], Coef2Re, Coef2Im, Coef3Re, Coef3Im, &DRe, &DIm, &ARe, &AIm);		
	}
...
void LGMSVComputeIntegral(	int		iNbX,
							double	*X,
							double	*IntRe,
							double	*IntIm,
							int		iNbStrike,
...
void LGMSVComputeIntegralLaguerre(	int		iNbX,
									double	*X,
									double	*W,
									double	Alpha,
									double	*IntRe,
...
void LGMSVComputeIntegralLegendre(	int		iNbX,
									double	*X,
									double	*W,									
									double	*IntRe,
									double	*IntIm,
...
void LGMSVComputeIntegralHermite(	int		iNbX,
									double	*X,
									double	*W,
									double	*IntRe,
									double	*IntIm,
...
double	theta, temp, temp1;
	...
	{		
		...
		result[j] = temp / PI / (exp(dIntegParam * dLogStrike[j]) - exp(-dIntegParam * dLogStrike[j]));
	}	
}
...
LGMSVSolFunc	FuncX_, *FuncX = &FuncX_,
				FuncX2_, *FuncX2 = &FuncX2_,
				FuncV_, *FuncV = &FuncV_,
				FuncV2_, *FuncV2 = &FuncV2_,
				FuncXV_, *FuncXV = &FuncXV_;
...
int				iBreakNbX[4];
	...
	if (fabs(dSwitchTime - dPWTime[iSwitchIndex]) > 1.0E-08 && dSwitchTime < dExTime)
	{
		do_switch = 1;
	}
	else
	...
	for (i=i; i<=endi; i++)
	{
		...
		Coef2ReT[i] = dLambdaEps[i-do_switch] + dCoefMeanRev2 * dAlpha[i-do_switch] * dRho[i-do_switch] * dSigma[i-do_switch];
		Coef2ImT[i] = -dAlpha[i-do_switch] * dRho[i-do_switch] * temp_vol;
		temp_vol *= temp_vol;
		Coef3ReT[i] = 0.5 * temp_vol;
		Coef3ImT[i] = 0.5 * temp_vol - dCoefCMS2 * dSigma[i-do_switch] * dSigma[i-do_switch];
		
		LGMSVMomentCalculationApprox(	dLambdaX,
										dAlpha[i-do_switch],
										dLambdaEps[i-do_switch],
										dRho[i-do_switch],
										dSigma[i-do_switch],
										dCoefVol,
										dCoefMeanRev2,
	...
	if (NumerParams->iIntegMethod == 0)
	{
		...
		XMax = (NumerParams->dParam1 + NumerParams->dParam2) * dXStd / (NumerParams->iNbX - 1);
		XMax = 1.0 / XMax;
	...
	{
		LGMSVSolveODEGride(	NumerParams->iNbX,
							X,
							InitX,
							endi,
							dt,
	...
	{
		LGMSVSolveODEGrideTV(	NumerParams->iNbX,
								X,
								InitX,
								endi,
								dt,
	...
	if (NumerParams->iIntegMethod == 0)
	{
		LGMSVComputeIntegral(	NumerParams->iNbX,
								X,
								IntRe,
								IntIm,
								iNbStrike,
	...
	{
		LGMSVComputeIntegralLaguerre(	NumerParams->iNbX,
										X,
										W,
										0.0,
										IntRe,
	...
	{
		LGMSVComputeIntegralHermite(	NumerParams->iNbX,
										X,
										W,
										IntRe,
										IntIm,
	...
	{
		LGMSVComputeIntegralLegendre(	NumerParams->iNbX,
										X,
										W,
										IntRe,
										IntIm,
...
FREE_RETURN:
	...
	if (X) free_dvector(X, 0, NumerParams->iNbX - 1);
	if (W) free_dvector(W, 0, NumerParams->iNbX - 1);
	if (IntRe) free_dvector(IntRe, 0, NumerParams->iNbX - 1);
	if (IntIm) free_dvector(IntIm, 0, NumerParams->iNbX - 1);

	if (LogStrike) free_dvector(LogStrike, 0, iNbStrike - 1);
}
...
double			IntRe, IntIm;
	...
	if (fabs(dSwitchTime - dPWTime[iSwitchIndex]) > 1.0E-08 && dSwitchTime < dFixTime)
	{
		do_switch = 1;
	}
	else
...
Err		err		= NULL;
	...
	price[1] = lgmsopval1F(iNbCoupon, dCouponDf, dCouponCvg, cpn_G, ex_zeta, ex_G, strike[1]);
	err = srt_f_optimpvol(price[1], swp_cash, strike[1], dExeTime, level, SRT_PUT, SRT_NORMAL, &(vol[1]));
	...
	price[0] = lgmsopval1F(iNbCoupon, dCouponDf, dCouponCvg, cpn_G, ex_zeta, ex_G, strike[0]);
	err = srt_f_optimpvol(price[0], swp_cash, strike[0], dExeTime, level, SRT_PUT, SRT_NORMAL, &(vol[0]));
	...
	price[2] = lgmsopval1F(	iNbCoupon, dCouponDf, dCouponCvg, cpn_G, ex_zeta, ex_G, strike[2]);
	err = srt_f_optimpvol(	price[2], swp_cash, strike[2], dExeTime, level, SRT_PUT, SRT_NORMAL, &(vol[2]));
	...
	err = srt_f_optimpvol(	price[1],
							fabs(swp_cash + (*dShift)),
							fabs(strike[1] + (*dShift)),
							dExeTime,
							level,
							SRT_PUT,
							SRT_LOGNORMAL,
...
FREE_RETURN:
	...
	if (cpn_G) free_dvector(cpn_G, 0, iNbCoupon - 1);
...
int		i;
	...
	if (iIsCMS || iIsRatioNum)
	{
		...
		if (iIsRatioNum)
		{
			temp = (exp(-(dTREnd - dTStar) / dTau) - exp(-(dTRStart - dTStar) / dTau)) * dTau;
			coefCMS -= temp;
			coefMean += temp;
		}					
	}
				...
				double	*dFwd)
{
	...
	err = Construct_Swap_Schedule(	today,
									yc_name,
									lStartDate,
									lEndDate,
									ifreq,
	...
	{
		err = Calculate_CoefShiftAndVol_LGMSV(	dFixTime,
												ncpn,
												cpn_time,
												cpn_cvg,
												cpn_df,
		...
		Calculate_CoefCMSandMeanRev_LGMSV(	ncpn,
											cpn_time,
											cpn_cvg,
											cpn_df,
											dPayTime,
	...
	{	
		err = Calculate_HestonEquivalent_LGMSV(	ncpn,
												cpn_date,
												cpn_time,
												cpn_cvg,
												cpn_df,
	...
	LGMSVFwdClosedFormApprox(
					1.0 / dTau,
					iNbPWTime,		/* Term Structure of g(t) */
					dPWTime,		
					dSigmaTS,					
					dAlphaTS,		/* Alpha of V = Eps^2 */
...
FREE_RETURN:
	...
	if (cpn_date) free_lvector(cpn_date, 0, ncpn - 1);
	if (cpn_time) free_dvector(cpn_time, 0, ncpn - 1);
	if (cpn_cvg) free_dvector(cpn_cvg, 0, ncpn - 1);	
				...
				double	*pSwaptionPrice)
{
	...
	if (NumerParams->iCalibLGM)
	{
		err = Calculate_CoefShiftAndVol_LGMSV(	dExTime,
												ncpn,
												cpn_time,
												cpn_cvg,
												cpn_df,
	...
	if (NumerParams->iIntegMethod == 2)
	{
		...
		for (i=0; i<iNbStrike; i++)
		{
			...
			{
				pSwaptionPrice[i] = (pSwaptionPrice[i] + dShiftStrike[i] - 1.0) * dShiftFwd;
			}
		}
		...
		for (i=0; i<iNbStrike; i++)
		{
			...
			{
				/* call put parity */
				pSwaptionPrice[i] = (pSwaptionPrice[i] + dStrike[i] - swp_rate) * numeraire;
			}
		}
	}
	else
...
FREE_RETURN:
	...
	if (dShiftStrike) free_dvector(dShiftStrike, 0, iNbStrike - 1);
				...
				double	*pSwaptionPrice)
{
	...
	err = LGMSVCmsRateApprox_TS(yc_name,			
							libor_ref_rate_name,		
							libor_freq,		
							libor_basis,
							lLiborFixDate,			
...
FREE_RETURN:
	...
	if (dShiftStrike) free_dvector(dShiftStrike, 0, iNbStrike - 1);
	if (dPriceAdj) free_dvector(dPriceAdj, 0, iNbStrike - 1);
	if (dPriceNonAdj) free_dvector(dPriceNonAdj, 0, iNbStrike - 1);
...
int				one2F;
	...
	err = LGMSVOptionApprox_TS(	yc_name,			
								ref_rate_name,		
								swaption_freq,		
								swaption_basis,
								lExDate,			
...
int				one2F;
	...
	err = LGMSVLiborOptionApprox_TS(	yc_name,			
										swaption_ref_rate_name,		
										swaption_freq,		
										swaption_basis,
										libor_ref_rate_name,
	...
	if (CalibInst->dExeTime > 1.0E-08)
	{
		...
		if (model->iOne2F == 2)
		{
			...
			for (i=0; i<index+1; i++)
			{
				...
				cum_var2 += model->dLGMAlpha[i] * model->dLGMAlpha[i] * model->dSigma[i] * model->dSigma[i] * (t2 - t1);
				cum_var12 += model->dLGMAlpha[i] * model->dLGMRho[i] * model->dSigma[i] * model->dSigma[i] * (t2 - t1);
			}

			ex_zeta2 = cum_var2 * exp(2.0 / model->dTau2 * model->dTStar);
			ex_zeta12 = cum_var12 * exp((model->dLambdaX + model->dLambdaX2) * model->dTStar);
		}
		...
		if (model->iOne2F == 1)
		{
			price[1] = lgmsopval1F(CalibInst->iNbCoupon, &(CalibCpnSchedule->dCpnDf[CalibInst->iStartCpn]), &(CalibCpnSchedule->dCpnCvg[CalibInst->iStartCpn]), HestonParam->dCpnG1, ex_zeta1, HestonParam->dExG1, strike[1]);
		}
		else
		{
			price[1] = lgmsopval2F(CalibInst->iNbCoupon, &(CalibCpnSchedule->dCpnDf[CalibInst->iStartCpn]), &(CalibCpnSchedule->dCpnCvg[CalibInst->iStartCpn]), HestonParam->dCpnG1, HestonParam->dCpnG2, ex_zeta1, ex_zeta2, ex_zeta12, HestonParam->dExG1, HestonParam->dExG2, strike[1]);
		}

		err = srt_f_optimpvol(price[1], swp_cash, strike[1], CalibInst->dExeTime, CalibInst->dLevel, SRT_PUT, SRT_NORMAL, &(vol[1]));
		...
		HestonParam->dMinStrike = CalibInst->dFwdCash + CalibInst->dSpread - HestonParam->dMinStd * std;
		HestonParam->dMaxStrike = CalibInst->dFwdCash + CalibInst->dSpread + HestonParam->dMaxStd * std;
		...
		if (CalibInst->iNbCoupon == 2)
		{
			...
			HestonParam->dCoefVol = (exp(-(CalibCpnSchedule->dCpnTime[CalibInst->iStartCpn] - model->dTStar) / model->dTau) 
				- exp(-(CalibCpnSchedule->dCpnTime[CalibInst->iEndCpn] - model->dTStar) / model->dTau)) * model->dTau;		

			if (model->iOne2F == 2)
			{
				HestonParam->dCoefVol_2F = (exp(-(CalibCpnSchedule->dCpnTime[CalibInst->iStartCpn] - model->dTStar) / model->dTau2) 
				- exp(-(CalibCpnSchedule->dCpnTime[CalibInst->iEndCpn] - model->dTStar) / model->dTau2)) * model->dTau2;
			}
		...
		{		
			...
			if (model->iOne2F == 1)
			{
				price[0] = lgmsopval1F(CalibInst->iNbCoupon, &(CalibCpnSchedule->dCpnDf[CalibInst->iStartCpn]), &(CalibCpnSchedule->dCpnCvg[CalibInst->iStartCpn]), HestonParam->dCpnG1, ex_zeta1, HestonParam->dExG1, strike[0]);
			}
			else
			{
				price[0] = lgmsopval2F(CalibInst->iNbCoupon, &(CalibCpnSchedule->dCpnDf[CalibInst->iStartCpn]), &(CalibCpnSchedule->dCpnCvg[CalibInst->iStartCpn]), HestonParam->dCpnG1, HestonParam->dCpnG2, ex_zeta1, ex_zeta2, ex_zeta12, HestonParam->dExG1, HestonParam->dExG2, strike[0]);
			}

			err = srt_f_optimpvol(price[0], swp_cash, strike[0], CalibInst->dExeTime, CalibInst->dLevel, SRT_PUT, SRT_NORMAL, &(vol[0]));
			...
			if (model->iOne2F == 1)
			{
				price[2] = lgmsopval1F(CalibInst->iNbCoupon, &(CalibCpnSchedule->dCpnDf[CalibInst->iStartCpn]), &(CalibCpnSchedule->dCpnCvg[CalibInst->iStartCpn]), HestonParam->dCpnG1, ex_zeta1, HestonParam->dExG1, strike[2]);
			}
			else
			{
				price[2] = lgmsopval2F(CalibInst->iNbCoupon, &(CalibCpnSchedule->dCpnDf[CalibInst->iStartCpn]), &(CalibCpnSchedule->dCpnCvg[CalibInst->iStartCpn]), HestonParam->dCpnG1, HestonParam->dCpnG2, ex_zeta1, ex_zeta2, ex_zeta12, HestonParam->dExG1, HestonParam->dExG2, strike[2]);
			}

			err = srt_f_optimpvol(	price[2], swp_cash, strike[2], CalibInst->dExeTime, CalibInst->dLevel, SRT_PUT, SRT_NORMAL, &(vol[2]));
			...
			else
			{
				...
				HestonParam->dShift = (const_lin - coef_lin * swp_cash) / (2.0 * coef_lin);
			}
			...
			err = srt_f_optimpvol(	price[1],
									fabs(swp_cash + HestonParam->dShift),
			...
			if (model->iOne2F == 1)
			{
				HestonParam->dCoefVol = HestonParam->dCoefVol / sqrt(cum_var / CalibInst->dExeTime);
			}
			else
			{
				/* we need to find the ratio between the two brownian contributions */
				price1 = lgmsopval1F(CalibInst->iNbCoupon, &(CalibCpnSchedule->dCpnDf[CalibInst->iStartCpn]), &(CalibCpnSchedule->dCpnCvg[CalibInst->iStartCpn]), HestonParam->dCpnG1, ex_zeta1, HestonParam->dExG1, strike[1]);
				price2 = lgmsopval1F(CalibInst->iNbCoupon, &(CalibCpnSchedule->dCpnDf[CalibInst->iStartCpn]), &(CalibCpnSchedule->dCpnCvg[CalibInst->iStartCpn]), HestonParam->dCpnG2, ex_zeta2, HestonParam->dExG2, strike[1]);

				err = srt_f_optimpvol(price1, fabs(swp_cash + HestonParam->dShift), fabs(strike[1] + HestonParam->dShift), CalibInst->dExeTime, CalibInst->dLevel, SRT_PUT, SRT_LOGNORMAL, &vol1);

				if (err) goto FREE_RETURN;

				err = srt_f_optimpvol(price2, fabs(swp_cash + HestonParam->dShift), fabs(strike[1] + HestonParam->dShift), CalibInst->dExeTime, CalibInst->dLevel, SRT_PUT, SRT_LOGNORMAL, &vol2);

				if (err) goto FREE_RETURN;

				ratio2F = vol2 / vol1 * sqrt(cum_var / cum_var2);

				HestonParam->dCoefVol = HestonParam->dCoefVol / sqrt((cum_var + cum_var2 * ratio2F * ratio2F + 2.0 * cum_var12 * ratio2F) / CalibInst->dExeTime);
		...
		if (model->iOne2F == 2)
		{
			HestonParam->dNewCoefCMS_2F = HestonParam->dCoefCMS_2F * HestonParam->dCoefVol_2F;
			HestonParam->dNewCoefCMS2_2F = HestonParam->dCoefCMS2_2F * HestonParam->dCoefVol_2F;

			HestonParam->dNewCoefCMS_cross_2F = HestonParam->dCoefCMS_2F * HestonParam->dCoefVol
												+ HestonParam->dCoefCMS * HestonParam->dCoefVol_2F;

			HestonParam->dNewCoefCMS2_cross_2F = HestonParam->dCoefCMS2_2F * HestonParam->dCoefVol
...
Err		err		= NULL;				
	...
	{
		expTi1 = exp(-(CalibCpnSchedule->dCpnTime[CalibInst->iStartCpn + i] - model->dTStar) / model->dTau);		
		dCoefMeanRev += CalibCpnSchedule->dCpnDf[CalibInst->iStartCpn + i] * (1.0 - expTi1);
	}
	...
	{
		temp = (1.0 -exp(-(CalibInst->dPayTime - model->dTStar) / model->dTau)) * model->dTau;
		dCoefCMS = dCoefMeanRev - temp;
		dCoefMeanRev = temp;
		...
		{
			temp = (exp(-(CalibInst->dLiborEndTime - model->dTStar) / model->dTau)
				- exp(-(CalibInst->dLiborStartTime - model->dTStar) / model->dTau)) * model->dTau;
			dCoefCMS -= temp;
			dCoefMeanRev += temp;
	...
	if (model->iOne2F == 2)
	{
		...
		for (i=1; i<CalibInst->iNbCoupon; i++)
		{
			expTi1 = exp(-(CalibCpnSchedule->dCpnTime[CalibInst->iStartCpn + i] - model->dTStar) / model->dTau2);		
			dCoefMeanRev += CalibCpnSchedule->dCpnDf[CalibInst->iStartCpn + i] * (1.0 - expTi1);
		}
		...
		if (CalibInst->iIsCMS  || CalibInst->iIsLiborOption)
		{
			temp = (1.0 -exp(-(CalibInst->dPayTime - model->dTStar) / model->dTau2)) * model->dTau2;
			dCoefCMS = dCoefMeanRev - temp;
			dCoefMeanRev = temp;
			...
			if (CalibInst->iIsLiborOption)
			{
				temp = (exp(-(CalibInst->dLiborEndTime - model->dTStar) / model->dTau2)
					- exp(-(CalibInst->dLiborStartTime - model->dTStar) / model->dTau2)) * model->dTau2;
				dCoefCMS -= temp;
				dCoefMeanRev += temp;
	...
	HestonParam->dCpnG1[0] = (1.0 - exp (-CalibCpnSchedule->dCpnTime[CalibInst->iStartCpn] / model->dTau)) * model->dTau;

	for (i=1; i<CalibInst->iNbCoupon; i++)
	{		
		HestonParam->dCpnG1[i] = (1.0 - exp (-CalibCpnSchedule->dCpnTime[CalibInst->iStartCpn + i] / model->dTau)) * model->dTau;		
	}

	HestonParam->dExG1 = (1.0 - exp (-CalibInst->dExeTime / model->dTau)) * model->dTau;

	if (model->iOne2F == 2)
	{
		HestonParam->dCpnG2[0] = (1.0 - exp (-CalibCpnSchedule->dCpnTime[CalibInst->iStartCpn] / model->dTau2)) * model->dTau2;

		for (i=1; i<CalibInst->iNbCoupon; i++)
		{		
			HestonParam->dCpnG2[i] = (1.0 - exp (-CalibCpnSchedule->dCpnTime[CalibInst->iStartCpn + i] / model->dTau2)) * model->dTau2;
		}

		HestonParam->dExG2 = (1.0 - exp (-CalibInst->dExeTime / model->dTau2)) * model->dTau2;
	}
...
int				do_fwd;
	...
	if (CalibInst->dExeTime > 1.0E-08)
	{
		...
		{
			NumerParams->NumerParams->dIntegParam = -1.0 - NumerParams->NumerParams->dIntegParam;
		}		
		...
		if (model->iOne2F == 1)
		{
			...
			for (i=0; i<=InstNumer->iSwitchIndex; i++)
			{
				...
				NumerParams->Coef2ReT[i] = model->dLambdaEps[i] + HestonParam->dCoefMeanRev * model->dAlpha[i] * model->dRho[i] * model->dSigma[i];
				NumerParams->Coef2ImT[i] = -model->dAlpha[i] * model->dRho[i] * temp_vol;
				temp_vol *= temp_vol;
				temp_var += temp_vol * NumerParams->dt[i];
				NumerParams->Coef3ReT[i] = 0.5 * temp_vol;
				NumerParams->Coef3ImT[i] = 0.5 * temp_vol - HestonParam->dNewCoefCMS * model->dSigma[i] * model->dSigma[i];
			...
			for (i=i; i<=InstNumer->iEndIndex; i++)
			{
				...
				NumerParams->Coef2ReT[i] = model->dLambdaEps[i2] + HestonParam->dCoefMeanRev2 * model->dAlpha[i2] * model->dRho[i2] * model->dSigma[i2];
				NumerParams->Coef2ImT[i] = -model->dAlpha[i2] * model->dRho[i2] * temp_vol;
				temp_vol *= temp_vol;
				temp_var += temp_vol * NumerParams->dt[i];
				NumerParams->Coef3ReT[i] = 0.5 * temp_vol;
				NumerParams->Coef3ImT[i] = 0.5 * temp_vol - HestonParam->dNewCoefCMS2 * model->dSigma[i2] * model->dSigma[i2];
		...
		else
		{
			...
			for (i=0; i<=InstNumer->iSwitchIndex; i++)
			{
				...
				/* Precalculation on the option i*/				
				temp_vol = HestonParam->dCoefVol * model->dSigma[i];
				temp_vol2 = HestonParam->dCoefVol_2F * model->dSigma[i] * model->dLGMAlpha[i];
				temp_vol_tot = sqrt(temp_vol * temp_vol + temp_vol2 * temp_vol2 + 2.0 * model->dLGMRho[i] * temp_vol * temp_vol2);			
				correl_tot = (model->dRho[i] * temp_vol + model->dRho2[i] * temp_vol2) / temp_vol_tot;

				NumerParams->dt[i] = (t2 - t1);
				NumerParams->CoefIntT[i] = model->dLvlEps[i];
				NumerParams->Coef1ReT[i] = -0.5 * model->dAlpha[i] * model->dAlpha[i];
				NumerParams->Coef2ReT[i] = model->dLambdaEps[i] + model->dAlpha[i] * model->dSigma[i] * (HestonParam->dCoefMeanRev *  model->dRho[i] 
																										+ HestonParam->dCoefMeanRev_2F * model->dRho2[i] * model->dLGMAlpha[i]);
				NumerParams->Coef2ImT[i] = -model->dAlpha[i] * correl_tot * temp_vol_tot;
				temp_vol_tot *= temp_vol_tot;
				temp_var += temp_vol_tot * NumerParams->dt[i];
				NumerParams->Coef3ReT[i] = 0.5 * temp_vol_tot;
				NumerParams->Coef3ImT[i] = 0.5 * temp_vol_tot - model->dSigma[i] * model->dSigma[i]
											* (	HestonParam->dNewCoefCMS +
												HestonParam->dNewCoefCMS_2F * model->dLGMAlpha[i] * model->dLGMAlpha[i] + 
												model->dLGMRho[i] * model->dLGMAlpha[i] * HestonParam->dNewCoefCMS_cross_2F);
			...
			for (i=i; i<=InstNumer->iEndIndex; i++)
			{
				...
				/* Precalculation on the option i*/							
				temp_vol = HestonParam->dCoefVol * model->dSigma[i2];
				temp_vol2 = HestonParam->dCoefVol_2F * model->dSigma[i2] * model->dLGMAlpha[i2];
				temp_vol_tot = sqrt(temp_vol * temp_vol + temp_vol2 * temp_vol2 + 2.0 * model->dLGMRho[i2] * temp_vol * temp_vol2);
				correl_tot = (model->dRho[i2] * temp_vol + model->dRho2[i2] * temp_vol2) / temp_vol_tot;

				NumerParams->dt[i] = (t2 - t1);
				NumerParams->CoefIntT[i] = model->dLvlEps[i2];
				NumerParams->Coef1ReT[i] = -0.5 * model->dAlpha[i2] * model->dAlpha[i2];
				NumerParams->Coef2ReT[i] = model->dLambdaEps[i2] + model->dAlpha[i2] * model->dSigma[i2] * (HestonParam->dCoefMeanRev2 * model->dRho[i2]
																											+ HestonParam->dCoefMeanRev2_2F * model->dRho2[i2] * model->dLGMAlpha[i2]);
				NumerParams->Coef2ImT[i] = -model->dAlpha[i2] * correl_tot * temp_vol_tot;
				temp_vol_tot *= temp_vol_tot;
				temp_var += temp_vol_tot * NumerParams->dt[i];
				NumerParams->Coef3ReT[i] = 0.5 * temp_vol_tot;
				NumerParams->Coef3ImT[i] = 0.5 * temp_vol_tot - model->dSigma[i2] * model->dSigma[i2]
											* (	HestonParam->dNewCoefCMS2 +
												HestonParam->dNewCoefCMS2_2F * model->dLGMAlpha[i2] * model->dLGMAlpha[i2] + 
												model->dLGMRho[i2] * model->dLGMAlpha[i2] * HestonParam->dNewCoefCMS2_cross_2F);
		...
		{
			/* simple unifrom grid */	
			XMax = (NumerParams->NumerParams->dParam1 + NumerParams->NumerParams->dParam2) * HestonParam->dCoefVol * model->dSigma[InstNumer->iEndIndex - InstNumer->iDoSwitch] / (NumerParams->iNbX - 1);
			XMax = 1.0 / XMax;
		...
		if (iIntegMethod <= 1)
		{
			LGMSVSolveODEGride(	NumerParams->iNbX,
								NumerParams->X,
								InstNumer->dLogShiftFwd,
								InstNumer->iEndIndex ,
								NumerParams->dt,
								NumerParams->CoefIntT,
								NumerParams->Coef1ReT,
								NumerParams->Coef1ReT,
								NumerParams->Coef2ReT,
								NumerParams->Coef2ImT,
								NumerParams->Coef3ReT,
								NumerParams->Coef3ImT,
		...
		else if (iIntegMethod == 2)
		{
			...
			for (i=0; i<InstNumer->iNbStrike - do_fwd; i++)
			{
				...
				{
					Price[i] += CalibInst->dFwdCash + CalibInst->dSpread - CalibInst->dStrike[i];
				}
				else
				{
					Price[i] -= CalibInst->dFwdCash + CalibInst->dSpread - CalibInst->dStrike[i];
	...
	else
	{
		for (i=0; i<InstNumer->iNbStrike - do_fwd; i++)
		{
			...
			{
				/* Payer */
				Price[i] = DMAX(CalibInst->dFwdCash + CalibInst->dSpread - CalibInst->dStrike[i], 0.0);
			}
			else
			{
				/* Receiver */
				Price[i] = DMAX(CalibInst->dStrike[i] - CalibInst->dFwdCash - CalibInst->dSpread, 0.0);
			}
		}
	}
	...
	if (do_fwd)
	{
		if (CalibInst->dExeTime > 1.0E-08)
		{
			...
			{
				Price[InstNumer->iNbStrike-1] = exp(NumerParams->IntRe[0]) - HestonParam->dShift;
			}
			else
			{
				Price[InstNumer->iNbStrike-1] = -exp(NumerParams->IntRe[0]) - HestonParam->dShift;
		...
		{
			Price[InstNumer->iNbStrike-1] = CalibInst->dFwdCash + CalibInst->dSpread;
		}
	}
}
	...
	if (CalibInst->dExeTime > 1.0E-08)
	{
		...
		else
		{
			for (i=0; i<=InstNumer->iLiborIndex; i++)
			{
				...
				NumerParams->Coef1ReT[i] = -0.5 * model->dAlpha[i] * model->dAlpha[i];
				NumerParams->Coef2ReT[i] = model->dLambdaEps[i] + model->dAlpha[i] * model->dSigma[i] * (HestonParam->dCoefMeanRev * model->dRho[i]
																										+ HestonParam->dCoefMeanRev_2F * model->dAlpha[i] * model->dRho2[i]);
				NumerParams->Coef2ImT[i] = -model->dAlpha[i] * correl_tot * temp_vol_tot;
				temp_vol_tot *= temp_vol_tot;
				NumerParams->Coef3ReT[i] = 0.5 * temp_vol_tot;
				NumerParams->Coef3ImT[i] = 0.5 * temp_vol_tot - model->dSigma[i] * model->dSigma[i]
...
Err			err = NULL;
	...
	CalibExeSchedule->lActEndDates[0] = bus_date_method(lTheoEndDate, MODIFIED_SUCCEEDING);		
	...
	for (k=0; k<CalibCpnSchedule->iNCpn; k++)
	{
		...
		CalibCpnSchedule->dCpnTime[k] = (CalibCpnSchedule->lCpnDate[k] - lToday) * YEARS_IN_DAY;
		CalibCpnSchedule->dCpnDf[k] = swp_f_df(lToday, CalibCpnSchedule->lCpnDate[k], sYcName);

		if (k > 0)
		{
			CalibCpnSchedule->dCpnCvg[k] = coverage(CalibCpnSchedule->lCpnDate[k-1], CalibCpnSchedule->lCpnDate[k], ibasis);
		}
		else
		{
...
int					iIntegParam_needs_reset = 0;
	...
	if (!CpnSchedule || !ExeSchedule || !Inst || !AllInst ||
		!model || !HestonParam || !PricingConst || !InstNumer)
	{
	...
	/* Get the model */
	err = Get_LGMSV_model(	und_name,
							model);

	if (err) goto FREE_RETURN;

	/* Get the instrument informations */
	AllocateCalibExeSchedule(	&lExDate,
								&dExeTime,
								&lStartDate,
								&lTheoEndDate,
								&lEndDate,
	...
	*/
	...
	err = ConstructSingleInstrumentSchedule(yc_name,
											today,
											swaption_freq,
											swaption_basis,
											lExDate,	
	...
	err = Initialise_PricingConst(	model,
									NumerParams,
									PricingConst);
	...
	err = Calculate_HestonEquivalent_LGMSV_FromLGM_struct(	Inst,
															CpnSchedule,
															model,
															HestonParam);
	...
	err = Initialise_NumerInst(	model,
								Inst,
								HestonParam,
								InstNumer);
...
FREE_RETURN:
	...
	if (iIntegParam_needs_reset)  NumerParams->dIntegParam = -1 - NumerParams->dIntegParam;

	if (CpnSchedule) free(CpnSchedule);
	if (ExeSchedule) free(ExeSchedule);
	...
	if (HestonParam) free(HestonParam);
...
int					iIntegParam_needs_reset = 0;
	...
	if (!CpnSchedule || !ExeSchedule || !Inst || !AllInst ||
		!sModel || !HestonParam || !PricingConst || !InstNumer)
	{
		err = "Memory allocation faillure in LGMSVOptionApprox_new";
		goto FREE_RETURN;
	}	
	...
	*/
	...
	/* All the precalculation and memory allocation for pricing */
	err = Initialise_PricingConst(	sModel,
									NumerParams,
									PricingConst);
	...
	err = Initialise_NumerInst(	sModel,
								Inst,
								HestonParam,
								InstNumer);
...
int					iNbStrike;
	...
	/* Get the instrument informations */		
	AllocateCalibExeSchedule(	&lFixDate,
								&dFixTime,
								&lStartDate,
								&lTheoEndDate,
								&lEndDate,

	...
	)
{
	...
	const int nNumInterval=(int)(dNumInterval + 0.5)>1?(int) (dNumInterval + 0.5):1; /// truncate
...
//						and fills pdBegin with the result of the discretization
//
	...
	)
{
	...
	const int nMaxSize_X3 = *_max_nEle(pnSizeX3_Begin,pnSizeX3_Begin+(pnSizeX1_End-pnSizeX1_Begin));
						   ...
						   )
{
	...
	{
		*pnSizeX = (*pdDeltaX)==0.? 1:1+ (int)(2.*_MAX_NUM_STD_*sqrt(*pdVarX_Begin)/(*pdDeltaX));
	...
	)
{
	...
	{
		*pdVarX3_Begin = 
			_Variance_X3(*pdPHI1_Begin,*pdPHI2_Begin,*pdPHI12_Begin,dRho,dAlpha);
	}
}
		...
		)
{
	...
	_preprocess_grid_(pdVarX1_Begin,pdVarX1_Begin+nNumEx,nBase_NumX1,pnSizeX1_Begin,&dDeltaX1);
	_preprocess_grid_(pdVarX3_Begin,pdVarX3_Begin+nNumEx,nBase_NumX3,pnSizeX3_Begin,&dDeltaX3);
				...
				_Grid* pGrid)
{
	...
	*(double*)(&pGrid->m_dDeltaT) = dT/nNumPointT;
	...
	/// space direction	
	if (szErr = 
		_space_discretize_(nNumPointX1,dDeltaX1,(double**)(&pGrid->m_pdX1_Begin),(int*)(&pGrid->m_nX1_Zero)))
	return szErr;

	if (szErr = 
		_space_discretize_(nNumPointX3,dDeltaX3,(double**)(&pGrid->m_pdX3_Begin),(int*)(&pGrid->m_nX3_Zero)))
	return szErr;
					   ...
					   )
{
	...
	*pn_Zero = (nNumPoint - 1)/2;
	...
	if(szErr = _alloc_dvector(ppd_Begin,nNumPoint)) return szErr;
	pd_Begin = (*ppd_Begin);	

...
static double optfwdext_fn(double mu1, double mu2, double sig1, double sig2,
                           double mat1, double mat2, double k, double l,
                           int du);

static double srt_f_optfwdext_price(double fwd1, double fwd2, double strike,
                                    double barrier, double vol1, double vol2,
...
{ /* BEGIN srt_f_optfwdext() */
  ...
  premium = srt_f_optfwdext_price(fwd1, fwd2, strike, barrier, vol1, vol2, mat1,
                                  mat2, disc, call_put, down_up);
  ...
  case DELTA_FWD1: /*** DELTA FWD ***/
    shift = fwd1 / 10000;
    answer = (srt_f_optfwdext(fwd1 + shift, fwd2, strike, barrier, vol1, vol2,
                              mat1, mat2, disc, call_put, down_up, PREMIUM) -
              premium) /
             shift;
    break;
  case DELTA_FWD2: /*** DELTA FWD ***/
    shift = fwd2 / 10000;
    answer = (srt_f_optfwdext(fwd1, fwd2 + shift, strike, barrier, vol1, vol2,
                              mat1, mat2, disc, call_put, down_up, PREMIUM) -
              premium) /
             shift;
    break;
  ...
  case DELTA: /*** DELTA FWD1 + FWD2 ***/
    shift = fwd1 / 10000;
    answer =
        (srt_f_optfwdext(fwd1 * (1 + shift / fwd1), fwd2 * (1 + shift / fwd1),
                         strike, barrier, vol1, vol2, mat1, mat2, disc,
  ...
  case GAMMA: /*** GAMMA ***/
    shift = fwd1 / 10000;
    answer = srt_f_optfwdext(
        fwd1 * (1 + shift / fwd1), fwd2 * (1 + shift / fwd1), strike, barrier,
        vol1, vol2, mat1, mat2, disc, call_put, down_up, PREMIUM);
    answer += srt_f_optfwdext(
        fwd1 * (1 - shift / fwd1), fwd2 * (1 - shift / fwd1), strike, barrier,
        vol1, vol2, mat1, mat2, disc, call_put, down_up, PREMIUM);
    answer -= 2 * premium;
  ...
  case VEGA1: /*** VEGA AT T1***/
    shift = GVOPT.vol_add;
    answer = (srt_f_optfwdext(fwd1, fwd2, strike, barrier, vol1 + shift, vol2,
                              mat1, mat2, disc, call_put, down_up, PREMIUM) -
              premium) /
             shift;
    break;
  case VEGA2: /*** VEGA AT T2***/
    shift = GVOPT.vol_add;
    answer = (srt_f_optfwdext(fwd1, fwd2, strike, barrier, vol1, vol2 + shift,
                              mat1, mat2, disc, call_put, down_up, PREMIUM) -
              premium) /
             shift;
    break;
  case THETA: /*** THETA  ***/
    shift = YEARS_IN_DAY;
    answer =
        srt_f_optfwdext(fwd1, fwd2, strike, barrier, vol1, vol2, mat1 - shift,
                        mat2 - shift, disc * exp(shift * log(disc) / mat2),
                        call_put, down_up, PREMIUM) -
        premium;
    break;
...
{ /* BEGIN srt_f_optfwdext_price */
  ...
  if (mat1 >= mat2) {
    if (flag == -1) /* Call Up or Put Down */
    {
      ...
      premium =
          srt_f_optblksch(fwd1, strike, vol1, mat1, disc, call_put, PREMIUM);

      premium -=
          srt_f_optblksch(fwd1, barrier, vol1, mat1, disc, call_put, PREMIUM);

      premium -=
          du * (strike - barrier) *
          srt_f_opteurdig(fwd1, barrier, vol1, mat1, disc, down_up, PREMIUM);
  ...
  if (flag == -1) {
    measure = optfwdext_fn(mu1, mu2, sig1, sig2, mat1, mat2, l, l, du);
  }

  measure += flag * optfwdext_fn(mu1, mu2, sig1, sig2, mat1, mat2, k, l, du);
...
/******************************************************************************/
static double optfwdext_fn(double mu1, double mu2, double sig1, double sig2,
                           double mat1, double mat2, double k, double l, int du)

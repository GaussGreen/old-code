/* ========================================================================
   FILENAME:     pde_f_discretise.cxx

  PURPOSE:     all the functions to discretise a pde
   ======================================================================== */
/* -----------------------------------------------------------------------------------------------------------------------------------------------------
 */
/* Define a discrete mesh in space        , for each coordinate being within
 * min/max point according to PDE */
#include "pde_h_discretise.h"
#include "srt_h_all.h"

Err pde_discretise_space(SrtPdeObject *pde) {
  Err err = NULL;

  SrtPdeGridObject *grid = pde->grid;

  if (pde->dimension >= 1)
    pde_discretise_space_dim1(pde->mesh_type_x, grid->num_x, pde->min_x,
                              pde->max_x, grid->vec_x, grid->inc_x);
  if (pde->dimension >= 2)
    pde_discretise_space_dim1(pde->mesh_type_y, grid->num_y, pde->min_y,
                              pde->max_y, grid->vec_y, grid->inc_y);
  if (pde->dimension >= 3)
    pde_discretise_space_dim1(pde->mesh_type_z, grid->num_z, pde->min_z,
                              pde->max_z, grid->vec_z, grid->inc_z);
  return err;
}

Err pde_discretise_space_dim1(SrtPdeMeshType mesh_typ, int mx, double min,
                              double max, double *vec, double *inc) {
  Err err = NULL;
  int i;
  double dx, sigma;
...
Err pde_discretise_operator(SrtPdeObject *pde, double dt) {
  Err err = NULL;
  SrtPdeGridObject *grid = pde->grid;
  ...
  int mxx = grid->num_x, myy = grid->num_y;

  double dxd = 0.0, dx = 0.0, dyd = 0.0, dy = 0.0, c0 = 0.0, cx = 0.0,
         cxx = 0.0, cy = 0.0, cyy = 0.0, cxy = 0.0, aa, bb, cc;

  double alpha = 0.0, sumd = 0.0, sumd2 = 0.0;
  ...
  for (j = 0; j <= myy + 1; j++) {
    ...
    for (i = 1; i <= mxx + 1; i++) {
      ...
      grid->D1[i][j][0] =
          grid->BX[i][j][0] - grid->L1[i][j][0] * grid->CX[i - 1][j][0];
    }
  }
  ...
  for (i = 0; i <= mxx + 1; i++) {
    ...
    for (j = 1; j <= myy + 1; j++) {
      ...
      grid->D2[i][j][0] =
          grid->BY[i][j][0] - grid->L2[i][j][0] * grid->CY[i][j - 1][0];
    }
  }

...
/* Checks on the number of columns in Sigma : one value, or two/four(corr)/five(corr+smile)/six columns */
	...
	{	
		...
		(((TwoFacIrmTermStructVal*)l->element->val.pval)->val_origin == BOTH_DATE))
		{
			...
			*ppdTau2 = realloc ( *ppdTau2, (*plNumTaus) * sizeof (double));
			(*ppdTauDates)[*plNumTaus-1] = (long) (((TwoFacIrmTermStructVal*)l->element->val.pval)->date);
			(*ppdTau1)[*plNumTaus-1] = ((TwoFacIrmTermStructVal*)l->element->val.pval)->exp_ts[0].tau;
			(*ppdTau2)[*plNumTaus-1] = ((TwoFacIrmTermStructVal*)l->element->val.pval)->exp_ts[1].tau;
		}
		...
		(((TwoFacIrmTermStructVal*)l->element->val.pval)->val_origin == BOTH_DATE))
		{
			...
			*ppdRho = realloc ( *ppdRho, (*plNumSigmas) * sizeof (double));
			(*ppdSigmaDates)[*plNumSigmas-1] = (long) (((TwoFacIrmTermStructVal*)l->element->val.pval) -> date);
			(*ppdSigma1)[*plNumSigmas-1] = ((TwoFacIrmTermStructVal*)l->element->val.pval)->exp_ts[0].sig;
			(*ppdBeta1)[*plNumSigmas-1] = ((TwoFacIrmTermStructVal*)l->element->val.pval)->exp_ts[0].beta;
			(*ppdSigma2)[*plNumSigmas-1] = ((TwoFacIrmTermStructVal*)l->element->val.pval)->exp_ts[1].sig;
			(*ppdBeta2)[*plNumSigmas-1] = ((TwoFacIrmTermStructVal*)l->element->val.pval)->exp_ts[1].beta;
...
Err make_2f_G_H_vectors (TermStruct* l)
{
	...
	while (lc->next != NULL) 
	{
		...
		{
			tempc1 = (exp ((lambda[0] + lambda[1]) * dt) - 1.0) 
					 / (lambda[0] + lambda[1]);
		}
		else
		...
		for (i=0; i<=1; i++)
		{
			c->reb_ts[i][i].G = p->reb_ts[i][i].G 
								+ temp2[i] * var[i] / pow (p->exp_ts[i].F, 2.0);
		}
		c->reb_ts[0][1].G = c->reb_ts[1][0].G = p->reb_ts[0][1].G
												+ tempc1 * covar / p->exp_ts[0].F / p->exp_ts[1].F;	

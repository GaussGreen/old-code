/* ============================================================================
   ...
   PURPOSE:		1.-	Initialise a volatility term structure for a two
   factor model  , straight from raw data (dates + values). 2.-	Retrieve all
   useful quantities at any date from a given Term Struct.
...
Err srt_f_init_IRM_TwoFac_TermStruct(
    ...
    /* CORRELATION */
    double alpha, double gamma, double ppdRho,
  ...
  /* Checks on the number of columns in Sigma : one value  , or
   * two/four(corr)/five(corr+smile)/six columns */
  if ((sig_cols != 2) && (sig_cols != 4) && (sig_cols != 5) &&
      (sig_cols != 6)) {
    if ((sig_cols != 1) || (num_sig > 1))
      return serror("Need TWO/FOUR/FIVE/SIX columns or ONE SINGLE VALUE for "
                    "the Sigma Curve");
  }
  ...
  if ((tau_cols != 2) && (tau_cols != 3)) {
    if ((tau_cols != 1) || (num_tau > 1))
      return serror(
          "Need TWO/THREE columns or ONE SINGLE VALUE for the Tau Curve");
  }
  ...
  if (sig_cols == 1) {
    ...
    for (i = 0; i < num_sig; i++) {
      ...
      /* Sigma 2 and Rho */
      if (sig_cols <= 2) {
        /* If correlation parameters input directly to the function (alpha  ,
         * ppdRho) */
  ...
  while ((over_tau == SRT_FALSE) || (over_sig == SRT_FALSE)) {
    ...
    ts2 =
        (TwoFacIrmTermStructVal *)srt_calloc(1, sizeof(TwoFacIrmTermStructVal));
    ...
    ts2->gamma =
        1.0 / ppdTauValues[2][cur_tau] - 1.0 / ppdTauValues[1][cur_tau];
    ...
    else if ((over_tau == SRT_TRUE) ||
             (((over_sig == SRT_FALSE) && (over_tau == SRT_FALSE)) &&
              (ppdSigmaValues[0][cur_sig] < ppdTauValues[0][cur_tau]))) {
      /* Set up specific data */
    ...
    srt_f_lstins(*ts, "TwoFacTsAtom", ts2->date, OBJ_PTR_IRM2f_TermStruct,
                 (void *)ts2, &srt_f_irm2ftsvalfree, &ticker);
  }
...
Err srt_f_display_IRM_TwoFac_TermStruct(TermStruct *ts, double **ppdSigmaDates,
                                        double **ppdSigma1, double **ppdBeta1,
                                        double **ppdSigma2, double **ppdBeta2,
                                        double **ppdRho, long *plNumSigmas,
                                        double **ppdTauDates, double **ppdTau1,
                                        double **ppdTau2, long *plNumTaus) {
  SrtLst *l = ts->head;
  ...
  *ppdTau2 = (double *)malloc(sizeof(double));
  ...
  while (l != NULL) {
    if ((((TwoFacIrmTermStructVal *)l->element->val.pval)->val_origin ==
         TAU_DATE) ||
        (((TwoFacIrmTermStructVal *)l->element->val.pval)->val_origin ==
         BOTH_DATE)) {
    ...
    if ((((TwoFacIrmTermStructVal *)l->element->val.pval)->val_origin ==
      ...
      *ppdRho = realloc(*ppdRho, (*plNumSigmas) * sizeof(double));
      ...
      (*ppdRho)[*plNumSigmas - 1] =
          ((TwoFacIrmTermStructVal *)l->element->val.pval)->rho;
    }
...
Err make_2f_G_H_vectors(TermStruct *l) {
  ...
  double var[2], covar, lambda[2], dt, temp0[2], /* exp (-lambda * dt) */
      ...
      tempc2[2][2]; /* (exp (-lambda1 - lambda2) * dt - 1.0 )/ (lambda1 +
                       lambda2) */
  int i, j;
  Err err = NULL;
  ...
  while (lc->next != NULL) {
    ...
    for (i = 0; i <= 1; i++) {
      c->reb_ts[i][i].G =
          p->reb_ts[i][i].G + temp2[i] * var[i] / pow(p->exp_ts[i].F, 2.0);
    }
    ...
    for (i = 0; i <= 1; i++) {
      for (j = 0; j <= 1; j++) {
        c->reb_ts[i][j].H =
            ...
            temp0[i] * p->exp_ts[i].F * p->exp_ts[j].F * p->reb_ts[i][j].G *
                temp1[j] +

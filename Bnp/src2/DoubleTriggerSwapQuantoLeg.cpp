...
// max(strikeY - Y_fixed_at_timeY  ,0) where X and Y are both lognormals with
// fwds fwdX and fwdY and vols sigmaX and sigmaY and instantaneous correlation
// rhoXY X is fixed at timeX and Y is fixed at time Y.
...
Err double_trigger_swap_quanto_cms_rate_quanto(
  ...
  Err err = NULL;
  // double cms  , cms_atm_log_vol  , cms_quanto_correlation  , fwd_fx_vol  ,
  // flat_vol  , temp_vol;
  double cms = 0.0;
...
Err double_trigger_swap_quanto_leg(
  ...
  while (min(exo_cms1_fixing_dates[i], exo_cms2_fixing_dates[i]) <
    ...
    /*
       ...
       ,cms2_spot_lag  , SRT_BDAY  , MODIFIED_SUCCEEDING); if(cms2_tenor_unit ==
       'M')
                                    {
                                            cms2_theo_end_date =
       ...
       * YEARS_IN_DAY; cms2_start_time = (cms2_start_date - today) *
       ...
       ,cms1_spot_lag  , SRT_BDAY  , MODIFIED_SUCCEEDING); if(cms1_tenor_unit ==
       'M')
                            {
                                    cms1_theo_end_date =
       ...
       * YEARS_IN_DAY; cms1_start_time = (cms1_start_date - today) *
       ...
       swp_f_spread(cms1_start_date  , cms1_theo_end_date  , cms1_refrate);
                            ...
                            err = swp_f_ForwardRate(cms1_start_date  ,
                                                                            cms1_theo_end_date
       , cms1_frequency  , cms1_basis  , cms1_yc_name  , cms1_refrate  , &fwd1);
                            if(err) goto FREE_RETURN;

                            //Calculates the CMS Rate
                            err = digital_quanto_float_leg_cms_rate_quanto(fwd1
       , cms1_fixing_time  , cms1_start_time  , cms1_nfp  , cms1_compounding  ,
                                                                                                                       cms1_delay  ,
                                                                                                                       cms1_dRateConv  ,
                                                                                                                       cms1_vol_type  ,
                                                                                                                       cms_flat_vol_or_smile_approx  ,
  ...
  while (i < n_exo_coupons) {
    ...
    // For the numeraire adjustments  , the vols are taken at exo_cms1_barriers
    // and exo_cms2_barriers
    err = digital_quanto_float_leg_get_lognormal_vol(
        cms1_vc_name, fwd1, fwd_spread1, cms1_fixing_time, cms1_start_date,

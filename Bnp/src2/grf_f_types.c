...
static Err grf_check_and_rem_null_rows  (
			long        *nrows, 
			long        ncols, 
			GrfnCell  **sprdsht)
{
...
int i;
	...
	{   
        if (event[0].df[i]  != NULL) srt_free(event[0].df[i]);
        if (event[0].dft[i] != NULL) srt_free(event[0].dft[i]);
        if (event[0].dfd[i] != NULL) srt_free(event[0].dfd[i]);
        if (event[0].yp[i]  != NULL) srt_free(event[0].yp[i]);
    }

    if (event[0].icom != NULL) comll_free_list(event[0].icom);
...
{
	if(gd->event_dates)srt_free(gd->event_dates);

	if(gd->auxlen)free_lngvector(gd->auxlen,0,gd->auxwidth-1);
	if(gd->aux){ srt_free(gd->aux[0]); srt_free(gd->aux);}

	if(gd->rowstatus)srt_free(gd->rowstatus);       

	if(gd->cells)free_matrix(gd->cells,0,gd->sslength-1,0,gd->sswidth-1);
	...
	if(gd->outfileptr)fclose(gd->outfileptr);
...
Err grfn_check_tableau_and_dates(
		long        *nrows, 
		long         ncols, 
		GrfnCell  **sprdsht, 
		Date       *eventdates)

...
/* ------------------------------------------------------------------------------------------------
  ...
  ------------------------------------------------------------------------------------------------- */
double	Inflation_GetFixing(long					lValDate,
...
/* ------------------------------------------------------------------------------------------------
  ...
  ------------------------------------------------------------------------------------------------- */
double	Inflation_FwdCPI(long				lValDate,
...
long	lToday;
	...
	if (lValDate<lToday)
	{
		...
		dFwdCPI = Inflation_GetFixing(	lValDate,
										&CPIMkt->CPIFixing);
	}
	else
	{
...
double	dFwdDRI;
	...
	if (dAlpha!=0)
	{
		...
		dFwdCPI1 = Inflation_FwdCPI(lFixDate1,
									CPIMkt,
	...
	if (dBeta!=0)
	{
		...
		dFwdCPI2 = Inflation_FwdCPI(lFixDate2,
									CPIMkt,
									NomCurve,		/* Nominal Curve name*/
									RealCurve		/* Real curve name */);
...
/* -------------------------------------------------------------------------------------------------------
	...
	Description of the coupon 
		 at Tpay we receive if floored  Coupon*(max(alpha * CPI(Tf1) + beta * CPI(Tf2) + gamma, Floor))
							if not      Coupon*(alpha * CPI(Tf1) + beta * CPI(Tf2) + gamma)			  
  --------------------------------------------------------------------------------------------------------- */
double	Inflation_FwdCouponValue(InflationCoupon	*Coupon ,	/* Coupon */
								 Inflation_CPIMkt	*CPIMkt,	/* CPI Market */
								 const char			*NomCurve,	/* Nominal Curve name*/
								 WrapInfo			*RealCurve,	/* Real curve either a name if WestWrapper or  a c++ object ptr */
...
double	dFwdCPI1, dFwdCPI2;
	...
	/* = E (under Qtpay) (  DRI(Tfix1,Tfix2)/DRIRef + gamma + max((Floor-gamma) - DRI(Tfix1,Tfix2)/DRIRef , 0)*/

	/* Pricing of E (under Qtpay) max((Floor-gamma)*DRIRef - DRI(Tfix1,Tfix2), 0) */
	if (Coupon->iIsFloored)
	{
		dTimeToExpiry = ((double)(max(Coupon->lFixDateCPI1,Coupon->lFixDateCPI2)
						- lToday))/DAYSINYEAR;
...
InflationCoupon		*Coupon = NULL;
	...
	for (iNum=0; iNum<CashFlows->iNbCoupon; iNum++)
	{
		...
		dFwdCouponValue = Inflation_FwdCouponValue(	Coupon ,		/* Coupon */
													CPIMkt,			/* CPI Market */
													NomCurve,		/* Nominal Curve name*/
...
Err		err = NULL;
	...
	if (Coupon->dAlpha != 0.0)
	{
		...
		err = Inflation_GetAdjFwdCPI(	Coupon->lFixDateCPI1,	/*	Fixing date of the cash CPI */
	...
	{
		/* Calculation of the convexty adjustement */
		err = Inflation_GetAdjFwdCPI(	Coupon->lFixDateCPI2,	/*	Fixing date of the cash CPI */
										Coupon->lPayDate,		/*	Pay date  */
										Model,
	...
	if (Coupon->iIsFloored)
	{
		err = Inflation_GetCorrCPI1CPI2(	/* Inputs */
											 ...
											 max(Coupon->lFixDateCPI1, Coupon->lFixDateCPI2),	/* End date */
...
Err	err = NULL;
	...
	for (iNum=0; iNum<CashFlows->iNbCoupon; iNum++)
	{
		err = Inflation_OneCouponFillAdjVols(	/* Input */
												Model,							/*	Model used */
												/* Output */
												&(CashFlows->Coupon[iNum])		/* Description of the cash flows */);
		if (err)
			return err;
	}
...
double	dBondLegTodayPv,
		dAssetSwapLegTodayPV, 
		dSum, 
		dDirtyPrice,
		dfTi_m1;
	...
	case FlatIndexedNotional:
			...
			for (iNum=0; iNum<AssetSwapCashFlows->iNbCoupon; iNum++)	
			{
				AssetSwapCashFlows->Coupon[iNum].dCouponValue = dTodayCBP/BondCashFlows->Coupon[0].dDRIRef;
			}
	...
	case IndexedNotional:
			...
			for (iNum=0; iNum<AssetSwapCashFlows->iNbCoupon-1; iNum++)	
			{
				...
				{
					AssetSwapCashFlows->Coupon[iNum].dCouponValue = dDf_Tsettle * (BondCashFlows->Coupon[iNum].dFwdPV - BondCashFlows->Coupon[iNum].dFwdPVFloor) / BondCashFlows->Coupon[iNum].dCouponValue;
				}
			}

			/* Special Case for the last coupon containing the redemption */
			iNum = AssetSwapCashFlows->iNbCoupon - 1;
			if (BondCashFlows->Coupon[iNum].dCouponValue + BondCashFlows->Coupon[iNum+1].dCouponValue == 0.0)
			{
				AssetSwapCashFlows->Coupon[iNum].dCouponValue = 0.0;
			}
			else
			{
				AssetSwapCashFlows->Coupon[iNum].dCouponValue = dDf_Tsettle * (BondCashFlows->Coupon[iNum].dFwdPV + BondCashFlows->Coupon[iNum+1].dFwdPV - BondCashFlows->Coupon[iNum].dFwdPVFloor - BondCashFlows->Coupon[iNum+1].dFwdPVFloor)
					/ (BondCashFlows->Coupon[iNum].dCouponValue + BondCashFlows->Coupon[iNum+1].dCouponValue);
			}
	...
	case FlatDirtyNotional:
			...
			for (iNum=0; iNum<AssetSwapCashFlows->iNbCoupon; iNum++)	
			{
				dSum += AssetSwapCashFlows->Coupon[iNum].dGamma * BondCashFlows->Coupon[iNum].dDfPayDate;
			}
	 ...
	 case IndexedDirtyNotional:
			...
			dBondLegTodayPv = BondCashFlows->Coupon[AssetSwapCashFlows->iNbCoupon].dFwdPV*BondCashFlows->Coupon[AssetSwapCashFlows->iNbCoupon].dDfPayDate;
			...
			for (iNum=AssetSwapCashFlows->iNbCoupon-1; iNum>=0; iNum--)	
			{
				...
				dBondLegTodayPv += BondCashFlows->Coupon[iNum].dFwdPV*BondCashFlows->Coupon[iNum].dDfPayDate;
				...
				dDirtyPrice = (dBondLegTodayPv - dAssetSwapLegTodayPV)
					/(AssetSwapCashFlows->Coupon[iNum].dGamma* BondCashFlows->Coupon[iNum].dDfPayDate + dfTi_m1);

				/* Update the notional of the asset swap */
				AssetSwapCashFlows->Coupon[iNum].dCouponValue = dDirtyPrice;

				/* Update the today pv of the asset swap leg */
				dAssetSwapLegTodayPV += AssetSwapCashFlows->Coupon[iNum].dGamma*AssetSwapCashFlows->Coupon[iNum].dCouponValue*BondCashFlows->Coupon[iNum].dDfPayDate;
			}
...
Err		err = NULL;
	...
	/* Get dTodayCBP */
	dTodayCBP = Inflation_FwdCPI(CPIMkt->lTodayDate,
								 CPIMkt,
								 NomCurve,	/* Nominal Curve name*/
								 RealCurve	/* Real curve either a name if WestWrapper or  a c++ object ptr */
								 );
	...
	dPvBondLeg = Inflation_ValueCashFlow(	BondCashFlows,	/* Description of the cash flows */
											CPIMkt,			/* CPI Market */
											NomCurve,		/* Nominal Curve name*/
											RealCurve		/* Real curve either a name if WestWrapper or  a c++ object ptr */);
	...
	dPvAssetSwapLeg = Inflation_ValueCashFlow(	AssetSwapCashFlows,	/* Description of the cash flows */
												CPIMkt,				/* CPI Market */
												NomCurve,			/* Nominal Curve name*/
												RealCurve			/* Real curve either a name if WestWrapper or  a c++ object ptr */);

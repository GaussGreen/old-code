/* ------------------------------------------------------------------------
	...
	PURPOSE:	provide a full function that minimises an error
				criteria using the Levenberg-Marquardt algorithm.
				This function is intended as a generic wrapper to 
				the mrqmin function as described in Numerical Recipes in C.
	INPUTS:		
				data:     a vector of data (points of evaluation)
				target:   a vector of target for the function
				weight:   the relative weight of the associated data
				ndata:    number of data
				param:    a vector of parameters (they will be optimised)
				nparam:   number of parameters used in the optimisation
				niter:    number of successfull iterations
				funcs():  the f(x,a) function as described in mrqmin
...
/* Sets use_param to 1: optimisation is done on all */
	...
	for (i=1; i<=nparam; i++) use_param[i] = 1;

...
static Err set_static_structures_for_fx_calib(
    char*    szFXUndName,
    long*    plFXOptMDates,
    double*  pdFXOptMktVols,
    double*  pdWeights,
    ...
    int      iNumInstrs)
{
    ...
    STATIC_PARAMS.ppdInitCorrels =
        dmatrix(0, STATIC_PARAMS.iNumCorrel - 1, 0, STATIC_PARAMS.iNumCorrDates - 1);
...
static void from_optparam_to_fx_vol_curve(double* pdOptimParams, double** ppdFXVolCrv)
{
    int i, j;
...
static SRT_Boolean are_correl_matrix_definite_positive(double*** pppdCorrel)
{
    ...
    ppdCorrMatrixToTest =
        dmatrix(0, (STATIC_PARAMS.iNumCorrel - 1), 0, (STATIC_PARAMS.iNumCorrel - 1));
    ...
    for (k = 0; k < STATIC_PARAMS.iNumCorrDates; k++)
    {
        ...
        err = diagonalise_symmetric_matrix(
            ppdCorrMatrixToTest, STATIC_PARAMS.iNumCorrel, pdEigenval, ppdEigenvector);
    ...
    {
        free_dmatrix(
            ppdEigenvector, 0, STATIC_PARAMS.iNumCorrel - 1, 0, STATIC_PARAMS.iNumCorrel - 1);
        ppdEigenvector = NULL;
    }
    ...
    {
        free_dmatrix(
            ppdCorrMatrixToTest, 0, STATIC_PARAMS.iNumCorrel - 1, 0, STATIC_PARAMS.iNumCorrel - 1);
        ppdCorrMatrixToTest = NULL;
    }
...
static void get_correlation_from_correl_matrix(double*** pppdCorrTensor, double** ppdCorrelation)
{
    int i, j, k;
...
static Err from_volcorrel_to_optparam(double** ppdFXVolCurve, double* pdOptimParams)
{
    int i, j;
    int iIndex;
...
static Err from_optparam_to_fx_und(double* pdOptimParams)
{
    ...
    {
        free_dmatrix(
            ppdCorrelation, 0, STATIC_PARAMS.iNumCorrel - 1, 0, STATIC_PARAMS.iNumCorrDates - 1);
        ppdCorrelation = NULL;
    }
...
static Err levenberg_fxvol_funcs(int iInstrIndex, double* pdOptimParams, double* pdFXBSVol)
...
static Err levenberg_calib_funcs(
    double iInstrIndex, double* pdOptimParams, double* value, double deriv[], int iNumParams)
{
    long   i;
    double shift;
    Err    err = NULL;
...
static void free_calib_fx_core_memory(int iNumInstrs, int iNumLocVols, int iNumParams)
{
...
Err srt_f_calib_fx_core(
    char** pszFXCalibStrings,
...

{
    ...
    err = srt_f_set_FXCalibParams(
        pszFXCalibStrings, pszFXCalibValueStrings, iNumCalibParamss, &sFXCalibParams);
    if (err)
        return err;

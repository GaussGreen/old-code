...
static Err set_static_structures_for_fx_calib(char			*szFXUndName,
										  long			*plFXOptMDates,
										  double		*pdFXOptMktVols,
										  double		*pdWeights,
										  int			iNumParams,
										  ...
										  int			iNumInstrs)
{
	...
	if(! (STATIC_INSTRUMENT.pdFXOptMats && STATIC_INSTRUMENT.pdWeights && STATIC_INSTRUMENT.plFXOptMDates))	
	{
	...
	else
	{
		for(i=0;i<iNumInstrs;i++)
		{
			...
			STATIC_INSTRUMENT.pdFXOptMats[i] = (plFXOptMDates[i]-ltoday)*YEARS_IN_DAY;
		}
	}
...
/*-----------FILL IN STATIC_PARAMS STRUCTURE -------------------------*/
	...
	if(!STATIC_PARAMS.pdCorrDates)	return serror("STATIC structure list improperly initialised");
	...
	if(STATIC_PARAMS.iNumCorrel!=3) return serror("Wrong number of correlation in input.");
	...
	sFXUnd = lookup_und(szFXUndName);
	if(sFXUnd == NULL) return serror("Undefined Underlying %s",szFXUndName);
	...
	strcpy(STATIC_PARAMS.ppszUndNames[0][0],
			STATIC_PARAMS.szFXUndName);

	strcpy(STATIC_PARAMS.ppszUndNames[0][1],
			STATIC_PARAMS.szDomUndName);

	strcpy(STATIC_PARAMS.ppszUndNames[1][0],
			STATIC_PARAMS.szFXUndName);

	strcpy(STATIC_PARAMS.ppszUndNames[1][1],
			STATIC_PARAMS.szForUndName);

	strcpy(STATIC_PARAMS.ppszUndNames[2][0],
			STATIC_PARAMS.szDomUndName);

	strcpy(STATIC_PARAMS.ppszUndNames[2][1],
			STATIC_PARAMS.szForUndName);
	
	/*Get the Initial Correlations */

	STATIC_PARAMS.ppdInitCorrels = dmatrix(0,STATIC_PARAMS.iNumCorrel-1,
							0,STATIC_PARAMS.iNumCorrDates-1);

	if(!STATIC_PARAMS.ppdInitCorrels) return serror("STATIC structure list improperly initialised");

	for(i=0;i<STATIC_PARAMS.iNumCorrDates;i++)
	{	
		time = (STATIC_PARAMS.pdCorrDates[i]-ltoday)*YEARS_IN_DAY;
		
		err = srt_f_get_corr_from_CorrList( sCorrlist,		
											STATIC_PARAMS.szDomUndName,
											STATIC_PARAMS.szFXUndName,									    			 
											time,
...
static void from_optparam_to_correl(double *pdOptimParams,double ***pppdCorrTensor)
{
...
int	i,j,k;
		...
		{
			for(i=0;i<STATIC_PARAMS.iNumCorrel;i++) pppdCorrTensor[i][i][k] = 1.0;
		}
		...
		{	
			...
			{	
				for(j=0;j<i;j++) 
				{
					pppdCorrTensor[i][j][k] = pdOptimParams[k*STATIC_PARAMS.iNumCorrel+i+j];
...
/* This function tests if the different correlations matrix are definite positive */
...
Err			err=NULL;
	...
	ppdCorrMatrixToTest = dmatrix(0,(STATIC_PARAMS.iNumCorrel-1),
								  0,(STATIC_PARAMS.iNumCorrel-1));

	ppdEigenvector = dmatrix(0,(STATIC_PARAMS.iNumCorrel-1),0,(STATIC_PARAMS.iNumCorrel-1));
	...
	for(k=0;k<STATIC_PARAMS.iNumCorrDates;k++)
	{
		...
		err = diagonalise_symmetric_matrix(ppdCorrMatrixToTest, STATIC_PARAMS.iNumCorrel, pdEigenval,ppdEigenvector);
		for (j=0;j<STATIC_PARAMS.iNumCorrel;j++)
		{
			if (pdEigenval[j] < 0) 
			{
				if(pdEigenval) 
				free_dvector(pdEigenval,0,STATIC_PARAMS.iNumCorrel-1);pdEigenval=NULL;

				if(ppdEigenvector) 
				{
					free_dmatrix(ppdEigenvector,
								 0,STATIC_PARAMS.iNumCorrel-1,
								 0,STATIC_PARAMS.iNumCorrel-1);
					ppdEigenvector = NULL;
				}
				...
				{
					free_dmatrix(ppdCorrMatrixToTest,
						 0,STATIC_PARAMS.iNumCorrel-1,
						 0,STATIC_PARAMS.iNumCorrel-1);
	...
	if(ppdEigenvector) 
	{
		free_dmatrix(ppdEigenvector,0,STATIC_PARAMS.iNumCorrel-1,0,STATIC_PARAMS.iNumCorrel-1);
		ppdEigenvector = NULL;
	}
...
Err			err = NULL;
	...
	if(are_fx_vol_positive(ppdFXVolCrv)==SRT_NO) are_fxparams = SRT_NO;	

	/* Get the Correl from OptParams */
	
	if (STATIC_PARAMS.bCalibCorrel == SRT_YES)
	{
		pppdCorrTensor = f3tensor(0,STATIC_PARAMS.iNumCorrel-1,
							 0,STATIC_PARAMS.iNumCorrel-1,
							 0,STATIC_PARAMS.iNumCorrDates-1);

		from_optparam_to_correl(pdOptimParams,pppdCorrTensor);
		if(are_correl_matrix_definite_positive(pppdCorrTensor)==SRT_NO)  are_fxparams = SRT_NO;	
	}

	if(ppdFXVolCrv) free_dmatrix(ppdFXVolCrv,0,1,0,STATIC_PARAMS.iNumLocVols-1);
	if(pppdCorrTensor) free_f3tensor(pppdCorrTensor,
									 0,STATIC_PARAMS.iNumCorrel-1,
									 0,STATIC_PARAMS.iNumCorrel-1,
									 0,STATIC_PARAMS.iNumCorrDates-1);
...
int	i,j,k;
	...
	for(k=0;k<STATIC_PARAMS.iNumCorrDates;k++)
	{
		for(i=0;i<STATIC_PARAMS.iNumCorrel;i++)
		{
			for(j=0;j<i;j++)	ppdCorrelation[i+j-1][k]= pppdCorrTensor[i][j][k];
		} 
	}/* i=0 domund,i=1 forund, i=2 fxund */
...
double				**ppdFXVolCrv=NULL;
	...
	if(are_optparams_fxparams(pdOptimParams)==SRT_NO) return err;
	...
	if (STATIC_PARAMS.bCalibCorrel == SRT_YES)
	{
		...
		if(!pppdCorrTensor) return  serror("Allocation Failure ");
		
		from_optparam_to_correl(pdOptimParams,pppdCorrTensor);

		ppdCorrelation = dmatrix(0,STATIC_PARAMS.iNumCorrel-1,0,STATIC_PARAMS.iNumCorrDates-1);
		if(!ppdCorrelation) return  serror("Allocation Failure ");
		...
		err = SrtInitCorrelationMatrix( 
						STATIC_PARAMS.iNumCorrDates, 
						STATIC_PARAMS.iNumCorrel, 
						ppdCorrelation,
						STATIC_PARAMS.pdCorrDates, 
						STATIC_PARAMS.ppszUndNames);
		if(err) return err;	
	}
	...
	err = srt_f_init_FX_TermStruct(today,
									ppdFXVolCrv,2,STATIC_PARAMS.iNumLocVols,
	...
	if(und_list==NULL)	return serror("NO Underlying list");

	/* Puts the underlying in the Market List */

	err = srt_f_addundtolist(und_list,
								STATIC_PARAMS.szFXUndName,
								"FX_UND",
								dom_ccy,
								"FX_STOCH_RATES",
...
static Err levenberg_fxvol_funcs(int			iInstrIndex,
								 double			*pdOptimParams,
								 double			*pdFXBSVol)
...
double	dOptMat;
	...
	err = srt_f_get_fx_implied_vol(dOptMat,
								STATIC_PARAMS.szFXUndName,
								pdFXBSVol);
...
static Err levenberg_calib_funcs(double		iInstrIndex,
								 double		*pdOptimParams,
								 double		*value,
								 double		deriv[],
								 int		iNumParams)
{
	...
	err = levenberg_fxvol_funcs((int)iInstrIndex,
								pdOptimParams,
								value);
	if(err) 
		return err;
	...
	{ 
		if (pdOptimParams[i]==0)
			shift=FX_ZERO_SHIFT;
		else shift = FX_PROP_SHIFT*pdOptimParams[i];
...
static void free_calib_fx_core_memory(int	 iNumInstrs,
									  int	iNumLocVols,
									  int	 iNumParams)
{
	...
	{	free_dmatrix(STATIC_PARAMS.ppdInitCorrels,
					 0,STATIC_PARAMS.iNumCorrel-1,
					 0,STATIC_PARAMS.iNumCorrDates-1);
...
int					i;
	...
	err = srt_f_set_FXCalibParams(pszFXCalibStrings, 
								  pszFXCalibValueStrings,
								 iNumCalibParamss,
								 &sFXCalibParams);
	if(err) return err;
	...
	if (! (STATIC_PARAMS.bCalibCorrel == SRT_YES)) iNumParams = iNumLocVols;

	err	 = set_static_structures_for_fx_calib(szFXUndName,
										  	  plFXOptMDates,
										  	  pdFXOptMktVols,
										 	  pdWeights,
										      iNumParams,
...
/*Safety measure: free the TermStructure attached to the underlying */
	...
	if(sFXCalibParams.lNumIter>0)
	{
		...
		/* Sets a vector of x data: here it is 1,2,3....: the market pdData index */
			...
			err = levenberg_marquardt(
					...
					pdOptimParams,
					iNumParams,
					sFXCalibParams.lNumIter,
					levenberg_calib_funcs,
					ChiSquare);

			if(err)
			{	
				free_calib_fx_core_memory(iNumInstrs,
											  iNumLocVols,
											  iNumParams);
				
				if(pdOptimParams) 
				free_dvector(pdOptimParams,1,iNumParams);pdOptimParams=NULL;

				if(pdData) 
				free_dvector(pdData,1,iNumParams);pdData=NULL;
			}

								 ...
								 double  **dCpns)
{
	...
	if( (iNFixDates>1) 
			&&
	( (int)(12*(lFixEndDates[0]-lFixStartDates[0])/365.0 + 0.5)!=(int)(12*(lFixEndDates[1]-lFixStartDates[1])/365.0+0.5) ) )
	{
		shortstub = 1;//stub;
	}
	...
	FixFloatMult = (int)((double)(iNFloatDates-floatshortstub-shortstub)/(iNFixDates-shortstub)+0.5);
	...
	*dCpns = (double*) calloc (iNFixPayDates, sizeof(double));
	...
	for(i=1;i<=shortstub;++i)
	{
		...
		(*dCpns)[i] = dFixCoverages[i-1]*dFixRates[i-1]*dFixNotionals[i-1];
	...
	for(i=shortstub+1;i<iNFixPayDates-1;++i)
	{
		(*lCpnDates)[i] = lFixPayDates[i];
		(*dCpns)[i] = dFixCoverages[i-1]*dFixRates[i-1]*dFixNotionals[i-1];
		for(k=(i-1-shortstub)*FixFloatMult+shortstub+floatshortstub+1;k<(i-shortstub)*FixFloatMult+shortstub+floatshortstub+1;++k)
		{
			(*dCpns)[i] += - (dFloatNotionals[k] - dFloatNotionals[k-1]
								+ dFloatNotionals[k-1] 
									* dFloatCoverages[k-1]
	...
	(*dCpns)[iNFixPayDates-1] = dFixCoverages[iNFixPayDates-2]*dFixRates[iNFixPayDates-2]*dFixNotionals[iNFixPayDates-2];
	for(k=(iNFixPayDates-2-shortstub)*FixFloatMult+shortstub+floatshortstub+1;k<(iNFixPayDates-1-shortstub)*FixFloatMult+shortstub+floatshortstub;++k)
	{
		(*dCpns)[iNFixPayDates-1] += - ( dFloatNotionals[k]- dFloatNotionals[k-1]
							+ dFloatNotionals[k-1] 
								* dFloatCoverages[k-1]
								* (dMargins[k-1] + dFloatSpreads[k-1]))
						* (swp_f_df(lToday, lFloatPayDates[k], cYCname)
	...
	(*dCpns)[iNFixPayDates-1] += - ( - dFloatNotionals[iNFloatPayDates-2]
						+ dFloatNotionals[iNFloatPayDates-2] 
							* dFloatCoverages[iNFloatPayDates-2]
							* (dMargins[iNFloatPayDates-2] + dFloatSpreads[iNFloatPayDates-2]))
								 ...
								 int	*floatshortstub)
{
	...
	*floatshortstub = 0;
	...
	FixFloatMult = (int)((double)(lNFloatDates-*floatshortstub-*shortstub)/(lNFixDates-*shortstub)+0.5);
	...
	*dCpns = (double*) calloc (lNFixDates+1, sizeof(double));
	...
	(*dCpns)[lNFixDates] = dFixCoverages[lNFixDates-1]*dFixRates[lNFixDates-1]*dFixNotionals[lNFixDates-1];
	for(k=(lNFixDates-1-*shortstub)*FixFloatMult+*shortstub+*floatshortstub+1;k<(lNFixDates-*shortstub)*FixFloatMult+*shortstub+*floatshortstub;++k)
	{
		(*dCpns)[lNFixDates] += - ( dFloatNotionals[k]- dFloatNotionals[k-1]
							+ dFloatNotionals[k-1] 
								* dFloatCoverages[k-1]
								* (dMargins[k-1] + dFloatSpreads[k-1]) )
	...
	(*dCpns)[lNFixDates] += - ( - dFloatNotionals[lNFloatDates-1]
						+ dFloatNotionals[lNFloatDates-1] 
							* dFloatCoverages[lNFloatDates-1]
							 ...
							 double  **dCpns)
{
	...
	FixFloatMult = (int)((double)(iNFloatPayDates-1-floatshortstub-shortstub)/(iNFixPayDates-1-shortstub)+0.5);
...
//------------------------------------------------------------------------
Err PVBOND_IRR(char *cYCname, char *cVCname, long lToday, 
			   char *cFreq, char *cBasis, char *cRefRname,
			   int iNPayDates, long *lPayDates, double *dCpns, 
			   int UseVol, 
			   double R, double *Pv)
{
	Err err=NULL;
	...
	for(i=0;i<iNPayDates;++i)
	{
		if((i>0)&&(UseVol==1))
		{
			err = swp_f_ForwardRate(lPayDates[0], lPayDates[i], cFreq, cBasis, cYCname, cRefRname, &fwd);
			if(err)
			{
...
//------------------------------------------------------------------------
Err		ComputeBondIRR(char *cYCname, char *cVCname, long lToday,
					   char *cFreq, char *cBasis, char *cRefRname,
					   int nPayDates,
					   long *lCpnDates,
					   double *dCpns,
					   ...
					   int UseVol)
{
	...
	R = 0.05;
//	PV = PVBOND_IRR(cYCname, lToday, nPayDates, lCpnDates, dCpns, R);

	err = PVBOND_IRR(cYCname, cVCname, lToday, 
			   cFreq, cBasis, cRefRname,
			   nPayDates, lCpnDates, dCpns, 
			   UseVol, R, 
			   &PV);
	...
	{
		R2 = R + 0.0001;
//		PV2 = PVBOND_IRR(cYCname, lToday, nPayDates, lCpnDates, dCpns, R2);
		err = PVBOND_IRR(cYCname, cVCname, lToday, 
				   cFreq, cBasis, cRefRname,
				   nPayDates, lCpnDates, dCpns, 
				   UseVol, 
...
//	PV_Float = PVBOND_IRR(cYCname, lToday, iNFixPayDates, lFixPayDates, dCpns_Float, IRR);
	err = PVBOND_IRR(cYCname, cVCname, lToday, 
				   cFreq, cBasis, cRefRname,
				   iNFixPayDates, lFixPayDates, dCpns_Float, 
				   UseVol, 
				   IRR, 
				   &PV_Float);

//	PV_Fix = PVBOND_IRR(cYCname, lToday, iNFixPayDates, lFixPayDates, dCpns_Fix, IRR);
	err = PVBOND_IRR(cYCname, cVCname, lToday, 
				   cFreq, cBasis, cRefRname,
				   iNFixPayDates, lFixPayDates, dCpns_Fix,
				   UseVol, 
				   IRR, 
				   &PV_Fix);

//	*dSwapRate = -PVBOND_IRR(cYCname, lToday, iNFixPayDates, lFixPayDates, dCpns_Float, IRR);
//	*dSwapRate = *dSwapRate / PVBOND_IRR(cYCname, lToday, iNFixPayDates, lFixPayDates, dCpns_Fix, IRR);
	*dSwapRate = -PV_Float / PV_Fix;


FREE_RETURN :

	if(dCpns_Float) free_dvector(dCpns_Float, 0,iNFixPayDates-1);
					   ...
					   int UseVol)
{
	...
	for(i=0;i<iNFixDates;++i)
	{
		...
		err = ComputeBondIRR(cYCname, cVCname, lToday,
							cFreq, cBasis, cRefRname,
							lNCpns-i,
							lCpnDates+i,
							dCpns+i,
								 ...
								 int UseVol)
{
	...
	FixFloatMult = (int)(dFixCoverages[lNFix-1]/dFloatCoverages[lNFloat-1]+0.5);
...
Err ComputeCoInitStrikes(char *cYCname, char *cVCname,
					   char *cRefRname,
					   long StartDate,
					   long EndDate,
					   SrtCompounding srtFreq,
					   ...
					   int UseVol)
{
	...
	for(i=0;i<iNFixDates;++i)
	{
		ComputeSwapRateFromIRR(cYCname, cVCname,
								lToday,
								cRefRname,
								StartDate,
								lFixEndDates[i],
							   ...
							   )
{
	...
	dAmounts[NCoInitSwap-1] = dCpns[NCoInitSwap] / dCoupons[NCoInitSwap-1][NCoInitSwap];
...
Err		ConvertBondInCoInitalSwapsPortfolio(
								 ...
								 double *dCpns,
								 char *cRefRname,
								 SrtCompounding srtFreq, //Bond and coinit swaps should have the same frequency//
								 SrtBasisCode srtBasis,
								 double  *dStrikes,
								 double  *dAmounts,
								 int UseVol)
...
//	double tempoSwap;
	...
	for(i=0;i<iNFixDates;++i)
	{
		...
		end = add_unit(lFixStartDates[shortstub], nMonth * (i+1-shortstub), SRT_MONTH, NO_BUSDAY_CONVENTION);
//		end  = lFixEndDates[i];
		...
		err = ConvertVanillaSwapInBond(cYCname, 
									cRefRname, 
									start,
									end,
									srtFreq, 
...
FREE_RETURN :
	...
	if(lNCoupons) free(lNCoupons);
	for(i=0;i<iNFixDates;++i)
	{
		if(dCoupons[i]) free(dCoupons[i]);
		if(lCouponDates[i]) free(lCouponDates[i]);
	}
	if(dCoupons) free(dCoupons);
	if(lCouponDates) free(lCouponDates);
								 ...
								 int UseVol)
{
	...
	for(i=0;i<iNFixPayDates-1;++i)
	{
		ComputeSwapRateFromIRR2(cYCname, cVCname,
						  lToday,
						  cFreq, cBasis, cRefRname,
		...
		err = ConvertVanillaSwapInBond2(cYCname, 
						  lToday,
						  shortstub,
						  floatshortstub,
								 ...
								 int UseVol)
{
	...
	for(i=0;i<iNFixPayDates-1;++i)
	{
		ComputeSwapRateFromIRR3(cYCname, cVCname,
						  lToday,
						  cFreq, cBasis, cRefRname,
		...
		err = ConvertVanillaSwapInBond3(cYCname, 
						  lToday,
								 ...
								 int UseVol)
{
	...
	recpay = -1;
    if (srtCallPut == SRT_CALL) recpay=1;
...
FREE_RETURN:
	...
	if(lFixDates) free(lFixDates);
	if(lFloatDates) free(lFloatDates);

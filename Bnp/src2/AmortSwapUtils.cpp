    ...
    double**       dCpns)
{
    ...
    shortstub = 0;
    if ((iNFixDates > 1) && ((int)(12 * (lFixEndDates[0] - lFixStartDates[0]) / 365.0 + 0.5) !=
    ...
    FixFloatMult =
        (int)((double)(iNFloatDates - floatshortstub - shortstub) / (iNFixDates - shortstub) + 0.5);
    ...
    *dCpns     = (double*)calloc(iNFixPayDates, sizeof(double));
    ...
    for (i = shortstub + 1; i < iNFixPayDates - 1; ++i)
    {
        (*lCpnDates)[i] = lFixPayDates[i];
        (*dCpns)[i]     = dFixCoverages[i - 1] * dFixRates[i - 1] * dFixNotionals[i - 1];
        for (k = (i - 1 - shortstub) * FixFloatMult + shortstub + floatshortstub + 1;
    ...
    (*dCpns)[iNFixPayDates - 1] = dFixCoverages[iNFixPayDates - 2] * dFixRates[iNFixPayDates - 2] *
                                  dFixNotionals[iNFixPayDates - 2];
    for (k = (iNFixPayDates - 2 - shortstub) * FixFloatMult + shortstub + floatshortstub + 1;
         k < (iNFixPayDates - 1 - shortstub) * FixFloatMult + shortstub + floatshortstub;
         ++k)
    {
    ...
    (*dCpns)[iNFixPayDates - 1] +=
        -(-dFloatNotionals[iNFloatPayDates - 2] +
          dFloatNotionals[iNFloatPayDates - 2] * dFloatCoverages[iNFloatPayDates - 2] *
    ...
    int* floatshortstub)
{
    ...
    *shortstub = 0;
    if ((lNFixDates > 1) && ((int)(12 * (lFixEndDates[0] - lFixStartDates[0]) / 365.0 + 0.5) !=
                             (int)(12 * (lFixEndDates[1] - lFixStartDates[1]) / 365.0 + 0.5)))
    {
        *shortstub = 1;  // stub;
    }
    ...
    *floatshortstub = 0;
    ...
    FixFloatMult =
        (int)((double)(lNFloatDates - *floatshortstub - *shortstub) / (lNFixDates - *shortstub) + 0.5);
    ...
    *dCpns     = (double*)calloc(lNFixDates + 1, sizeof(double));
    ...
    (*dCpns)[lNFixDates] =
        dFixCoverages[lNFixDates - 1] * dFixRates[lNFixDates - 1] * dFixNotionals[lNFixDates - 1];
    for (k = (lNFixDates - 1 - *shortstub) * FixFloatMult + *shortstub + *floatshortstub + 1;
         k < (lNFixDates - *shortstub) * FixFloatMult + *shortstub + *floatshortstub;
         ++k)
    {
    ...
    double** dCpns)
{
    ...
    *dCpns     = (double*)calloc(lNFixDates + 1, sizeof(double));
    ...
    for (i = 0; i < lNFixDates - 1; ++i)
    {
        ...
        {
            (*dCpns)[i + 1] +=
                -(dFloatNotionals[k + 1] - dFloatNotionals[k] +
                  dFloatNotionals[k] * dFloatCoverages[k] * (dMargins[k] + dFloatSpreads[k])) *
                (swp_f_df(lToday, lFloatEndDates[k], cYCname) /
    ...
    (*dCpns)[i + 1] +=
        -(-dFloatNotionals[k] +
          dFloatNotionals[k] * dFloatCoverages[k] * (dMargins[k] + dFloatSpreads[k])) *
        (swp_f_df(lToday, lFloatEndDates[k], cYCname) / swp_f_df(lToday, lFixEndDates[i], cYCname));
    ...
    /*
       ...
       lFixPayDates[iNFixPayDates-1], cYCname));
            ...
            (*dCpns)[iNFixPayDates-1] += - ( - 1.0
                                            ...
                                            * (swp_f_df(lToday, lFloatPayDates[iNFloatPayDates-1], cYCname)
                                                    /swp_f_df(lToday, lFixPayDates[iNFixPayDates-1],
       cYCname));
    */
    for (i = 1; i <= shortstub; ++i)
    {
    ...
    for (k = (iNFixPayDates - 2 - shortstub) * FixFloatMult + shortstub + floatshortstub + 1;
         ...
         ++k)
    {
        (*dCpns)[iNFixPayDates - 1] += -(dFloatCoverages[k - 1] * dFloatSpreads[k - 1]) *
                                       (swp_f_df(lToday, lFloatPayDates[k], cYCname) /
                                        swp_f_df(lToday, lFixPayDates[iNFixPayDates - 1], cYCname));
    }
    (*dCpns)[iNFixPayDates - 1] +=
        -(-1.0 + dFloatCoverages[iNFloatPayDates - 2] * dFloatSpreads[iNFloatPayDates - 2]) *
        (swp_f_df(lToday, lFloatPayDates[iNFloatPayDates - 1], cYCname) /
         swp_f_df(lToday, lFixPayDates[iNFixPayDates - 1], cYCname));
    ...
    double** dCpns)
{
    ...
    FixFloatMult =
        (int)((double)(iNFloatPayDates - 1 - floatshortstub - shortstub) / (iNFixPayDates - 1 - shortstub) + 0.5);
    ...
    for (i = 0; i < iNPayDates; ++i)
    {
        if ((i > 0) && (UseVol == 1))
        {
            err = swp_f_ForwardRate(
                lPayDates[0], lPayDates[i], cFreq, cBasis, cYCname, cRefRname, &fwd);
            if (err)
            {
    ...
    double*        dSwapRate)
{
    ...
    */
    ...
    //	*dSwapRate = *dSwapRate / PVBOND_IRR(cYCname, lToday, iNFixPayDates, lFixPayDates,
    // dCpns_Fix, IRR);
    ...
    double* dSwapRate)
{
    ...
    dCpns_Float[iNFixPayDates - 1] += -(-1.0 + dFloatCoverages[k - 1] * dFloatSpreads[k - 1]) *
                                      (swp_f_df(lToday, lFloatPayDates[k], cYCname) /
                                       swp_f_df(lToday, lFixPayDates[iNFixPayDates - 1], cYCname));
    ...
    int            UseVol)
{
    ...
    lNCoupons    = (int*)calloc(iNFixDates, sizeof(int));
    ...
    for (i = 0; i < iNFixDates; ++i)
    {
        start = lFixStartDates[0];
            ...
            lFixStartDates[shortstub],
            nMonth * (i + 1 - shortstub),
            SRT_MONTH,
    ...
    err = ComputeEquivalentAmounts(
        nPayDates, lCpnDates, dCpns, iNFixDates, lNCoupons, lCouponDates, dCoupons, dAmounts);
    ...
    int UseVol)
{
    ...
    lNCoupons    = (int*)calloc(iNFixPayDates - 1, sizeof(int));
    ...
    for (i = 0; i < iNFixPayDates - 1; ++i)
    {
        ...
        ComputeLevel(cYCname, lToday, i + 2, lFixPayDates, dFixCoverages, &dLvls[i]);

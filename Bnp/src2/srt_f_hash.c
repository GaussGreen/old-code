...
Err InitHash(SHash *hash, int nfac, int maxsize)
{
	...
	if (!hash->hash_tree) return serror("Memory failure");
...
Err Hash(SHash *hash, int *imono, int *hmono)
{
	...
	for (i=0; i <= j; i++)
	{
		...
		if (!p->facpow[k])
		{
			...
			if (!p->facpow[k]) return serror("Memory failure");
			p->facpow[k]->idx = -1;
		}
		p = p->facpow[k];
	}
	if (p->idx < 0)
	{
		if (hash->pos >= hash->maxsize) return serror("Hash maxsize exceeded");
		p->idx = hash->pos++;
	}
	*hmono = p->idx;
...
{
	int				i;
	if (p) for (i=0; i < Hash_MAXNPOW; i++) FreeHashTree(p->facpow[i]);
	free(p);
}
...
Err Poly_Init(SPolynomial *poly, SHash *hash, int maxmono, int nstp)
{
	...
	if (!poly->mono || !poly->coef || !poly->pool) return serror("Memory failure");
...
{
	if (poly->mono) free_imatrix(poly->mono, 0, poly->maxmono-1, 0, poly->hash->nfac-1);
	if (poly->coef) free_dmatrix(poly->coef, 0, poly->maxmono-1, 0, poly->nstp-1);
	free(poly->pool);
}
...
void Poly_Copy(SPolynomial *poly, SPolynomial *p)
{
	...
	memcpy(&poly->mono[0][0], &p->mono[0][0], p->n_mono * p->hash->nfac * sizeof(int));
	memcpy(&poly->coef[0][0], &p->coef[0][0], p->n_mono * p->nstp * sizeof(double));
...
Err Poly_AddDPoly(SPolynomial *poly, SPolynomial *p, int ifac)
{
	...
	for (j=0; j < p->n_mono; j++)
	{
		if (p->mono[j][ifac] != 0)
		{
			...
			{
				if (poly->n_mono >= poly->maxmono) return serror("Max mono exceeded");
				poly->pool[hmono] = poly->n_mono++;
				memcpy(poly->mono[poly->n_mono-1], imono, poly->hash->nfac * sizeof(int));
			}
...
Err Poly_AddConstMono(SPolynomial *poly, double c, double *pc, int *mono_idx)
{
	...
	if (poly->pool[hmono] < 0)
	{
		if (poly->n_mono >= poly->maxmono) return serror("Max mono exceeded");
		poly->pool[hmono] = poly->n_mono++;
		memcpy(poly->mono[poly->n_mono-1], mono_idx, poly->hash->nfac * sizeof(int));
	}
	for (k=0; k < poly->nstp; k++)
									 ...
									 SPolynomial *p1, SPolynomial *p2)
{
	...
	for (i=0; i < p1->n_mono; i++)
	{
		...
		{
			for (k=0; k < poly->hash->nfac; k++)
				imono[k] = (mono_idx ? mono_idx[k] : 0) + p1->mono[i][k] + p2->mono[j][k];
			err = Hash(poly->hash, imono, &hmono);
			if (err) return err;
			...
			for (k=0; k < poly->nstp; k++)
				poly->coef[poly->pool[hmono]][k] += (pc ? c * pc[k] : c) * p1->coef[i][k] * p2->coef[j][k];
		}
	}
	return NULL;
}

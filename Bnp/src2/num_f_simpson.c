...
double sm_trapzd(double (*function)(double), double a, double b, int n, double accum)
{
...
/*additional parameters may be : SwapDP p,SwapDP pf,double coupon, double clean_price,double
   volatility, double myu, int T, double redemption, double first_coupon*/
{
    va_list argptr;
    int     j;
    ...
    for (j = 2; j < JMAX; j++)
    {
        ...
        sum   = (4.0 * sum2N - sumN) / 3.0;
        if (fabs(sum - oldsum) < EPS_SIMPSO * fabs(oldsum))
        {
            ret = sum;
            break;
        }
...
double sm_trapzd_list(
    double (*function)(double, va_list), double a, double b, int n, double accum, va_list argptr)
{
    double x, tnm, sum, del, ret;
    int    it, j;

...
/*<%%STA>-----------------------------------------------------------------
  ...
  DESCRIPTION     :returns pointer to new COMLL_STR        , or NULL if it
fails. Everything in the node will be set to zero except the index        ,
...
then 1        , 2        ,...; returns index of last element. Returns -1 if a is
NULL.
<%%END>---------------------------------------------------------------------*/
long comll_create_index(COMLL_PTR a) {
  ...
  for (i = 0, b = comll_gototop(a); b->next != NULL;
       b->index = i, b = b->next, i++)
    ;
...
/*<%%STA>-----------------------------------------------------------------
  ...
  DESCRIPTION     :joins together two lists        , so that a->b; returns a
unless a is NULL
<%%END>---------------------------------------------------------------------*/
...
/*<%%STA>-----------------------------------------------------------------
  ...
  DESCRIPTION     :copy a comll(even jumps        , and allocates new cvg arrays
      ,etc.)
<%%END>---------------------------------------------------------------------*/
/****************************************************************************
...
COMLL_PTR b=NULL        ,c = NULL;
        ...
        while(c)
        {
                ...
                if(b->dfindlen)
                {
                        ...
                        b->cvg = (double *)srt_calloc(b->dfindlen
      ,sizeof(double)); memcpy(b->dfind        ,c->dfind
,b->dfindlen*sizeof(int)); memcpy(b->cvg        ,c->cvg
,b->dfindlen*sizeof(double));
                }
                c = c->next;
        }

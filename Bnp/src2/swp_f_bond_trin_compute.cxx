...
double fwd_clean_fct(Ddate dstart, Ddate fut, String swap, String repo,
  ...
  fwd_acc_int += theta * contract.cxxoupon;
...
void add_step(Deal contract, Model model, int index_tree, int index_date,
  ...
  /*	useless  = interp(	&model.date_\cumvol[0][0]        ,
                                  ...
                                  &temp);
  */
  step[index_tree].cxxum_vol =
      eq_cum_vol_func((double)t_n_y, SRT_YES, model.ts);
  step[index_tree].time_start = t_n_y;
  step[index_tree].flag_date = 1;
  step[index_tree].cxxpn = contract.cxxpn[index_date];
      ...
      if ((j > 0) && ((j < *step_num) &&
        ...
        if (contract.quanto == 1) {
          ...
          step[j].expected_spot *=
              exp(contract.cxxorr_fx *
                  sqrt(step[j].cxxum_vol * step_fx[j].cxxum_vol));
  ...
  */
  ...
  for (j = 0; j <= model->step_num; j++) /***  at each step  ***/
  {
    ...
    /*        	useless_end  = interp(	&model->date_cumvol[0][0]        ,
                                            ...
                                            &temp_end);
    */
    step[j].cxxum_vol = eq_cum_vol_func((double)t_n_y, SRT_YES, model->ts);
...
void init_step(int n, Deal contract, Model model, Model model_fx, Step *step,
  ...
  if (contract.yield_strike_flag == 1) {
    ...
    step[n].cxxlean_strike =
        100 * clean_price_fct(p_temp, contract.cxxoupon / 100,
                              contract.yield_strike / 100, 1,
                              contract.cxxoupon / 100);
  }

  if (contract.yield_barrier_flag == 1) {
    p_temp = contract.p;
    p_temp.start = (int)(floor(contract.p.start + step[n].time_start * 365));
    step[n].cxxlean_barrier =
        100 * clean_price_fct(p_temp, contract.cxxoupon / 100,
                              contract.yield_barrier / 100, 1,
                              contract.cxxoupon / 100);
    ...
    if ((contract.type == EXTINGUISH) || (contract.type == AMER_DIGIT) ||
      ...
      step[n].yield_hit_level =
          (int)((2 * contract.do_up - 1) *
                floor(log(step[n].cxxlean_barrier / contract.spot) *
                      (2.0 * contract.do_up - 1.0) / model.delta_lns));
    }
  ...
  if (contract.quanto == 1) {
    ...
    step[n].expected_spot *= exp(
        contract.cxxorr_fx * sqrt(step[n].cxxum_vol * step_fx[n].cxxum_vol));

    H_DEBUG &&printf("init steps spot expected old =%f \n",
                     step[n].expected_spot);
    H_DEBUG &&printf("corr =%f cum_vol =%f cum_fx =%f \n ", contract.cxxorr_fx,
                     step[n].cxxum_vol, step_fx[n].cxxum_vol);
    H_DEBUG &&printf("init steps spot  new =%f \n", step[n].expected_spot);
  }

  if (n < model.step_num) {
    step[n].vol_h = step[n + 1].cxxum_vol - step[n].cxxum_vol;
    step[n].vol =
        sqrt((model.cxxum_vol_opt - step[n].cxxum_vol) / step[n].time_end);
  ...
  if ((n == model.step_num) &&
      (contract.sub_period !=
       contract.opt_mat)) { /****	Still need a vol for B-S at the end of
                               the tree	****/
    step[n].vol =
        sqrt((model.cxxum_vol_opt - step[n].cxxum_vol) / step[n].time_end);
  }
...
double bond_trinomial_fct(Deal contract, Model model, Model model_fx,
  ...
  if (contract.type == EURO_DIGIT) {
    ...
    for (i = 1; i < model.step_num; i++) {
      opt_num = (int)floor(i * i * temp * temp) + contract.cxxall_put;
  ...
  if (contract.yield_strike_flag == 1)
    contract.strike = steps[model.step_num].cxxlean_strike;
  if (contract.yield_barrier_flag == 1)
    contract.barrier = steps[model.step_num].cxxlean_barrier;
  ...
  for (i = 0; i <= 2 * model.step_num + 2; i++) {
    ...
    node.intrinsic =
        (node.asset - contract.strike) * (1 - 2 * contract.cxxall_put);
...
double endtree_partial(Deal contract, Model model, Step step, Node node) {
  ...
  result =
      srt_f_optblksch(node.forward, contract.strike, step.vol,
                      ...
                      step.df_swap_end, contract.cxxall_put, 0);
  return (result);
}
...
double payoff_instal(Deal contract, Model model, Step step, Node node) {
  ...
  if (step.flag_date == 1) {
    if (node.disc_premium > step.cxxpn)
      result = node.disc_premium - step.cxxpn;
    else
      result = 0;
  } else
    result = node.disc_premium;
...
double endtree_digit(Deal contract, Model model, Step step, Node node) {
  ...
  if ((contract.cxxall_put == 1) && (node.position == model.hit_level)) {

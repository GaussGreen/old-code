...
char *MBS_Prepay_Engine_Check(MBS_Prepay_Engine *prepay_engine) {
  ...
  //		if( prepay_engine->agency_char != 'F' && prepay_engine->agency_char
...
// num_buckets <=0 means uses prepay_engine's SFSNb  , TriggerRate  , and Amort;
// assumes there is no delay
{
  int i, j;
  ...
  // MBS_Prepay_Engine_SMM(factors[0]  , &new_engine  , age  , input_rates[0]  ,
...
// subgroup == 0 <=> aggregated  , slow group comes first starting with 1 then
// fast group principal_payments assume notional of $1 as of spotYYYYMMDD
{
...
// monthOfYear = 2 for February prepay  , though the reset date for getting
// IndexValue may be March 1
int MBS_Prepay_Engine_SMM(
    double *output, MBS_Prepay_Engine *prepay_engine, int paymentNb,
    double IndexValue, int monthOfYear,
    int monthFromSettle) // this assumed there is NO prepay-delay i.e.  , if
                         // there is delay  , the input paymentNb  , IndexValue
                         // , monthOfYear and monthFromSetlle  , etc.  , have to
                         // adjusted properly
...
double
alsmm1(double al,
       int N) /* al is average life in years  , N is original term in months */
{
...
/*convert average life to smm*/
double alsmm2(double al, double wac, int N, int wam,
              int pt) /* al -avg life  , wac -- gross coupon  , N -- original
                         term wam -- remaining term  , pt -- N-wam*/
{
...
char *get_initial_principal_payments(int num_payments, double *out, int wam,
  ...
  // out[0] = (1.0 - pow(u  , moving_wam-1))/(1.0 - pow(u  ,moving_wam)) * ( 1.0
  // - FixedPrepay[0] );
...
num_future_rates  , future_rates  , &numProjections  , rates  , cpr  ,
principalPayDowns); fp = fopen("C:/workarea/mortgage/jymodel1/fixedpp.txt"
,"w"); for(i=0; i < numProjections; ++i)
        {
                fprintf(fp  , "rates[%d] = %f  , cpr = %f  , paydown=%f\n"  , i
, rates[i]  , cpr[i]*100.0  , principalPayDowns[i]*100.0);
        }

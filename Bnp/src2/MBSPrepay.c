...
char * MBS_Prepay_Engine_Params_ReaderExplicit( char * data_dir, MBS_Prepay_Engine * prepay_engine,
									   MTGPTACYPROG AgencyProg, long oterm,
									   double gwac,
									   double Speedup, double rational,
									   double FebruaryEffect,
									   double * IndexLevel, double * AmortLevel,
									   double * SeasoningLevel, double * SeasoningMat,
...
////
	...
	if( err=history_reader(data_dir, &(prepay_engine->history))) return(err);
	if(err=density_reader(data_dir, &(prepay_engine->density))) return(err);
	triggerRates = mbspt_dvector(0, SFSNb-1);
	MBS_Prepay_Engine_Initialize( prepay_engine,
	1, NBPOINTS, IndexLevel, &(AmortLevel),
	Seasonality, FebruaryEffect,
	NUMRAMPS, SeasoningLevel, SeasoningMat,
	Speedup, rational,
	SFSNb, triggerRates, Amorts,
	Delay,
	NbFixedPrepay, FixedPrepay,
	AgencyProg, oterm, gwac,
	0, 0,
	nuniform,
	IndexMat, EndIndexMat, IndexFreq);
...
char * MBS_Prepay_Engine_Params_Reader( char * data_dir, MBS_Prepay_Engine * prepay_engine,
...
//	if( !(stream = MBS_Sequential_File_Manager( data_dir, "PrepayFunc.dat" )) )
	if( !(stream=_OpenMBSFile(data_dir, FilePPFUNCS)))
		...
		fgets(string, 80, stream);
		fscanf(stream, "%ld\n", &EndIndexMat);	
		if(EndIndexMat <= 0) EndIndexMat = IndexMat;
		fgets(string, 80, stream);
		fscanf(stream, "%ld\n", &IndexFreq);
		fgets(string, 80, stream);
		fscanf(stream, "%lf \n", &FebEffect);
		...
		fgets(string, 80, stream);
		for (i=0; i<12; i++) fscanf(stream,"%lf\n", Seasonality + i);
		fgets  (string, 80, stream);
		if( fscanf (stream, "%d \n", &(Delay)) != 1) return(nrerror("Can't read prepay Delay"));
		fgets  (string, 80, stream);
		if( fscanf (stream, "%d \n", &(NbFixedPrepay)) !=1 ) return(nrerror("Can't read prepay NbFixedPrepay"));
		fgets  (string, 80, stream);
	...
	if( err = MBS_Prepay_Engine_Params_ReaderExplicit( data_dir, prepay_engine,
		AgencyProg, oterm,
		 gwac,
		 Speedup, rational,
		 FebEffect,
		 IndexLevel, AmortLevel,
		 SeasoningLevel, SeasoningMat,
		 2 * SFSNb, Amorts,
		 Seasonality,
		 Delay, NbFixedPrepay, FixedPrepay,
		 nuniform,
		 IndexMat, EndIndexMat, IndexFreq)) goto Free_Mem;
...
//adjust_tweaks for different programs:
	if ( prepay_engine->AgencyProg == _FN15) prepay_engine->Speedup *= 1.15;
	if ( prepay_engine->AgencyProg == _GN ) prepay_engine->Speedup *= 0.8;
	if ( prepay_engine->AgencyProg == _GN15 ) prepay_engine->Speedup *= 0.7;

//density and history adjustments
	adjust_density( &(prepay_engine->density), (prepay_engine->Speedup)*(prepay_engine->rational), prepay_engine->gwac, refiRate, Rate10Y, prepay_engine->nuniform);
...
//convert from CPR to smm
	for(i=0; i < prepay_engine->NbSCurve; ++i)
		...
		{
			prepay_engine->AmortLevel[i][j] = 1.0 - pow(1.0 - prepay_engine->AmortLevel[i][j]/100.0, 1.0/12.0);
		}
//tweaks
	for(i=0; i < prepay_engine->NbSCurve; ++i)
		spdbase( prepay_engine->AmortLevel[i], prepay_engine->Speedup, prepay_engine->rational, prepay_engine->NbIndexLevel );
//IndexLevel
	for( j=0; j < prepay_engine->NbIndexLevel; ++j ) prepay_engine->IndexLevel[j] = transformIncentive( prepay_engine->IndexLevel[j], prepay_engine->gwac, refiRate, Rate10Y );

//seasoning ramp
	for(j=0; j < prepay_engine->NbSeasoningRamps; ++j) prepay_engine->SeasoningLevel[j] = transformIncentive( prepay_engine->SeasoningLevel[j], prepay_engine->gwac, refiRate, Rate10Y );
//triggers
	...
	if (( 0.96 * prepay_engine->gwac + REFI_THRESHOLD - refiRate) <= 0.01) {
		for (i=0; i< half_SFSNb; i++) {
			prepay_engine->TriggerRate[i]  = ratio1 +     //use linear spacing
				(ratio2-ratio1)*(double)i
				/(double) (prepay_engine->SFSNb-i);  
			...
			for(i=0; i < half_SFSNb; i++)
				prepay_engine->TriggerRate[i] = refiRate - 1.0 + ratio1*
					pow(1.0 - (double)i/(double)half_SFSNb * ratio2, 3);
	}//else	
	for(i=0; i < half_SFSNb; ++i)
	{
		prepay_engine->TriggerRate[i] -= refiRate;
	...
	{
		if( prepay_engine->FixedPrepay[i] > 1.0 ) return(nrerror("Some FixedPrepay > 1.0!"));
...
char * MBS_Prepay_Engine_Check( MBS_Prepay_Engine * prepay_engine )
{
	...
	char * err;
///
	if( prepay_engine->NbSCurve < 1) return(nrerror("NbSCurve >= 1!"));
	if( prepay_engine->NbIndexLevel < 1 ) return(nrerror("NbSCurve >= 1!"));
	prev = 0.0;
	for( i=0; i < prepay_engine->NbIndexLevel; ++i)
	{
		if(prepay_engine->IndexLevel[i] <= prev) return(nrerror("IndexLevel must be increasing!"));
	...
	for(i=0; i < 12; ++i)
		if(prepay_engine->Seasonality[i] <= 0.0) return(nrerror("Seasonality > 0.0!"));
	if(prepay_engine->FebruaryEffect <= 0.0 || prepay_engine->FebruaryEffect >= 1.0) return(nrerror("FebrurayEffect in (0,1)!"));
	if( prepay_engine->NbSeasoningRamps < 1) return(nrerror("NbSeasoningRamps >= 1 !"));
	prev = 0.0;
	for(i=0; i < prepay_engine->NbSeasoningRamps; ++i)
	{
		if( prepay_engine->SeasoningLevel[i] <=  prev ) return(nrerror("SeasoningLevel increasing!"));
		prev = prepay_engine->SeasoningLevel[i];
		if( prepay_engine->SeasoningMat[i] <= 0.0 ) return(nrerror("SeasoningMat > 0!"));
	}
	if( prepay_engine->Speedup <= 0.0 || prepay_engine->rational <= 0.0 ) return(nrerror("Speedup and rational > 0!"));
	if( prepay_engine->SFSNb < 1 ) return(nrerror("SFSNb >= 1!"));


	for(i=0; i < prepay_engine->SFSNb; ++i)
	{
		if( prepay_engine->Amort[i] < 0.0 || prepay_engine->Amort[i] > 1.0 ) return(nrerror("Amort must be in [0,1]!"));
	}
	if( prepay_engine->Delay < 0) return(nrerror("Delay >= 0!"));
	if( prepay_engine->NbFixedPrepay < 0 ) return(nrerror("NbFixedPrepay >= 0"));
	if(prepay_engine->NbFixedPrepay < prepay_engine->Delay ) return("NbFixedPrepay < PrepayDelay");
	for(i=0; i < prepay_engine->NbFixedPrepay; ++i)
		if( prepay_engine->FixedPrepay[i] < 0.0 || prepay_engine->FixedPrepay[i] > 1.0 ) return(nrerror("FixedPrepay must be in [0,1]!"));

//		if( prepay_engine->agency_char != 'F' && prepay_engine->agency_char != 'G' ) return(nrerror( "agency_char: F or G" ));
	if( prepay_engine->oterm < 1 || prepay_engine->oterm > MAXOTERM ) return(nrerror("oterm must be legal!"));
	if( prepay_engine->gwac < 0.0 ) return( nrerror("gwac > 0!" ));
	if( prepay_engine->nuniform < 1 || prepay_engine->nuniform > MAXNBWEIGHTS) return(nrerror("nuniform must be legal!"));
	if(prepay_engine->IndexFreq <= 0
			|| prepay_engine->IndexMat <=0 || prepay_engine->EndIndexMat <=0 
			|| prepay_engine->IndexMat < prepay_engine->EndIndexMat)
			return("Bad Index spec for prepay_engine");

	if( err = HISTORY_Check( &(prepay_engine->history) )) return(err);
	if( err = MBSDENSITY_Check( &(prepay_engine->density))) return(err);
	return(0);
}//MBS_Preppay_Engine_Check

//the ultimate initialization
void MBS_Prepay_Engine_Initialize( MBS_Prepay_Engine * prepay_engine,
	int NbSCurve, int NbIndexLevel, double * IndexLevel, double **AmortLevel,
	double Seasonality[], double FebruaryEffect,
	int NbSeasoningRamps, double * SeasoningLevel, double * SeasoningMat,
	double Speedup, double rational,
	int SFSNb, double * TriggerRate, double * Amort,
	int Delay,
	int NbFixedPrepay, double * FixedPrepay,
	MTGPTACYPROG AgencyProg,
	long oterm,
	double gwac,
	HISTORY *history,
	...
	int IndexFreq)
{
	...
	/////
	for(i=0; i < 12; ++i) prepay_engine->Seasonality[i] = Seasonality[i];
...
void MBS_Prepay_Engine_ReplaceTriggers( MBS_Prepay_Engine * new_engine, MBS_Prepay_Engine * prepay_engine, int num_buckets, double * triggerRate, double * amort)
{
		MBS_Prepay_Engine_Initialize( new_engine,
		prepay_engine->NbSCurve, prepay_engine->NbIndexLevel, prepay_engine->IndexLevel, prepay_engine->AmortLevel,
		prepay_engine->Seasonality, prepay_engine->FebruaryEffect,
		prepay_engine->NbSeasoningRamps, prepay_engine->SeasoningLevel, prepay_engine->SeasoningMat,
		prepay_engine->Speedup, prepay_engine->rational,
		num_buckets, triggerRate, amort,
		prepay_engine->Delay,
		prepay_engine->NbFixedPrepay, prepay_engine->FixedPrepay,
		prepay_engine->AgencyProg, prepay_engine->oterm, prepay_engine->gwac,
		&(prepay_engine->history), &(prepay_engine->density),
		prepay_engine->nuniform,
		prepay_engine->IndexMat, prepay_engine->EndIndexMat, prepay_engine->IndexFreq);
...
void MBS_Prepay_Engine_Cleanup( MBS_Prepay_Engine * prepay_engine )
{
	...
	if(  prepay_engine->NbSCurve > 0 &&  prepay_engine->NbIndexLevel > 0)
		mbspt_free_dmatrix( prepay_engine->AmortLevel, 0, prepay_engine->NbSCurve-1, 0, prepay_engine->NbIndexLevel-1);
...
char * MBS_Prepay_FactorHistory( MBS_Prepay_Engine * prepay_engine, int init_age, int init_month,
								int num_rates, double * input_rates,
								int num_buckets, double * triggerRate, double * amort, 
								int useInitialSMM, double ** factors)
								//num_buckets <=0 means uses prepay_engine's SFSNb, TriggerRate, and Amort; assumes there is no delay
{
	int i, j;
...
//
	...
	MBS_Prepay_Engine_ReplaceTriggers( &new_engine, prepay_engine, newSFSNb, newTriggerRate, newAmort);
...
//real work, special handling for initial month
	//MBS_Prepay_Engine_SMM(factors[0], &new_engine, age, input_rates[0], monthOfYear, monthfromsettle);
	for(j=0; j < new_engine.SFSNb; ++j) factors[0][j] = 1.0;
	for(i=1; i <= num_rates; ++i)
	{

//#ifdef MBSPTCORRECTPPHISTORY
		MBS_Prepay_Engine_SMM(factors[i], &new_engine, age+1, input_rates[i-1], monthOfYear, monthfromsettle);
//#else

		//		MBS_Prepay_Engine_SMM(factors[i], &new_engine, age, input_rates[i-1], monthOfYear, monthfromsettle);
...
char * MBS_Prepay_Path( MBS_Prepay_Engine * prepay_engine,
					   long TradeYYYYMMDD, long spotYYYYMMDD,
					   long  wam, int subgroup,
					   int num_future_rates, double * futureRates,
					   int useFixedPrepay,
						  ...
						  //principal_payments assume notional of $1 as of spotYYYYMMDD
{
	...
	if(subgroup > prepay_engine->SFSNb || subgroup < 0) return(nrerror("Illegal subgroup number"));
	...
	*numProjections = min(wam, num_future_rates);
	...
	sprd = ( prepay_engine->density.IncentKnots[0] * prepay_engine->gwac - prepay_engine->density.irates[0] );
	i = 0;
	if(useFixedPrepay>0)
	{
		for(i = 0; i < *numProjections; ++i)
		{
			...
			else
			{
				...
				if (rates[i] < 0.0 ) return(nrerror("Some future rates < 0 after transformation"));
			}
		}
	}
	else
	...
	//get factors
	if(*numProjections <=0) return(0);
	//future factors:
	factors = mbspt_dmatrix(0, *numProjections, 0, prepay_engine->SFSNb-1);
	mo = spot_month - Delay;
	while(mo < 1) mo += 12;
	MBS_Prepay_FactorHistory( prepay_engine, wala - Delay, mo, *numProjections, rates, - 1, 0, 0, useFixedPrepay, factors);
	//get distribution as of spot
	weights = mbspt_dvector(0,prepay_engine->SFSNb-1);
	if(!subgroup)
	{
		if( err = get_Trigger_Distribution(prepay_engine, TradeYYYYMMDD, wam, weights)) return(err);
	}
	else
	{
		for(i=0; i < prepay_engine->SFSNb; ++i)
...
//now aggregrate
	...
	cpr[0] = 1.0 - fsmm[0];
	if( *numProjections == wam ) cpr[ wam - 1 ] = 0.0;
...
//before turning smm to cpr, calc factor history:
	if( err = get_initial_principal_payments( *numProjections, principal_payments, wam, prepay_engine->gwac, *numProjections, cpr)) return(err);
//
	for(i=0; i < *numProjections; ++i)
		cpr[i] = 1.0 - pow( (1.0 -cpr[i]), 12.0);
	if(useFixedPrepay>0)
		for(i=0; i < prepay_engine->NbFixedPrepay; ++i) rates[i] = -100.0;
	//cleanup
...
void get_smm_weights(double * weights, MBS_Prepay_Engine *prepay_engine, double *usweights, double * ufweights)
{
	...
	for(i=0; i < subgroupSFSNb; ++i)
	{
		...
		if(i) vec[i-1]=0.0;
		vec[i] = 1.0;
		swt = qinterpp( subgroupSFSNb, reversedTrigger, vec, nuniform, prepay_engine->density.urates, usweights );
		fwt = qinterpp( subgroupSFSNb, reversedTrigger, vec, nuniform, prepay_engine->density.urates, ufweights );
...
int MBS_Prepay_Engine_SMM( double * output, MBS_Prepay_Engine * prepay_engine,
	int paymentNb, double IndexValue,
	int monthOfYear, int monthFromSettle)//this assumed there is NO prepay-delay i.e., if there is delay, the input paymentNb, IndexValue, monthOfYear and monthFromSetlle, etc., have to adjusted properly
...
//FIX: check NbSCurve > 0
	if( prepay_engine->NbSCurve == 1) SCurveIndex = prepay_engine->NbSCurve - 1;
	else
	{
		if( monthFromSettle > prepay_engine->NbSCurve + 6 ) SCurveIndex = prepay_engine->NbSCurve - 1;
		else if( monthFromSettle <= prepay_engine->NbSCurve + 6 && monthFromSettle >= prepay_engine->NbSCurve - 2 ) SCurveIndex = prepay_engine->NbSCurve - 2;
		else if( monthFromSettle >= 0 ) SCurveIndex = monthFromSettle;
		else SCurveIndex = 0; //FIX: should not have to come to this case
	}
	smm = linterpFlat(prepay_engine->NbIndexLevel,
			prepay_engine->IndexLevel, prepay_engine->AmortLevel[SCurveIndex],
			IndexValue );
...
//Seasoning:
	seasoningFactor = ((double) paymentNb)/linterpFlat( prepay_engine->NbSeasoningRamps,
			prepay_engine->SeasoningLevel, prepay_engine->SeasoningMat,
			IndexValue );
...
//trigger
	...
	{
		asmm = 0.0;
		if( IndexValue < prepay_engine->TriggerRate[i] ) asmm = prepay_engine->Amort[i];
...
void spdbase(double   *AmortLevel,
         double   Speedup,
         double   rational,
         int      numpts)
{
		...
		double S0;
			...
			for (i=0; i<numpts; i++) {
				if (AmortLevel[i] > S0) AmortLevel[i] += (rational -1.0)*(AmortLevel[i] - S0);
				else AmortLevel[i] *= pow((AmortLevel[i] / S0), (rational-1.0));
...
/*convert average life to smm*/
double alsmm2(double al,double wac,int N, int wam, int pt)		/* al -avg life, wac -- gross coupon, N -- original term wam -- remaining term, pt -- N-wam*/
{
	...
	for(i=0; i<30; i++) {
	x3 = (x1+x2)/2.;
	y3 = al-1. - (factor1/factor2)*ALSMM2FUNC(x3,wam) - ALSMM2FUNC(x3*(1.+wac/1200.)-wac/1200.,wam)*factor3/factor1;
	if (y3>0.) x1=x3;
		else  x2 = x3;
	} 
...
//get dense representation of the density function of various trigger groups at the start (settle date or trade date?)

char * get_Trigger_Distribution(MBS_Prepay_Engine * prepay_engine, long spotYYYYMMDD, long wam, double * weights)
{
	int i; 
	double sum, sum1;
...
///
	...
	if( (spot_position - age - Delay < 0) || (spot_position - age - Delay + num_rates - 1 >= prepay_engine->history.Number))
		return(nrerror("Incomplete History!"));
	...
	Dsplit(
		Date_Add(YYYYMM2FirstOfMon(prepay_engine->history.dates[ spot_position - age - Delay ]),MONTH, -Delay,0,1),
		&yy,&init_month,&dd);
	MBS_Prepay_FactorHistory( prepay_engine, -Delay,  init_month, num_rates, input_rates, num_buckets, urates, fastAmort, 0, fastFactors);
	MBS_Prepay_FactorHistory( prepay_engine, -Delay,  init_month, num_rates, input_rates, num_buckets, urates, slowAmort, 0, slowFactors);
...
char * get_initial_principal_payments( int num_payments, double * out, int wam, double gwac, int NbFixedPrepay, double *FixedPrepay )
{
	int i;
//	double u;
	int moving_wam;

	if( num_payments > NbFixedPrepay ) return(nrerror("Ask for too many principal paydowns"));
	if(num_payments<=0) return(0);//do nothing
	...
	for( i=1; (i < num_payments) && (moving_wam > 1); ++i)
	{
		...
		//out[i] = out[i-1] * (1.0 - pow(u,moving_wam-1))/(1.0 - pow( u, moving_wam)) * (1.0 - FixedPrepay[i]);
	}
	if(i < num_payments)
	{
...
/***
void test_MBS_Prepay_Path( MBS_Prepay_Engine * prepay_engine, long spotYYYYMMDD, long  wam, int num_future_rates)
{
	FILE * fp;
	...
	last_rate = prepay_engine->history.hrates[spot_position] +  ( prepay_engine->density.IncentKnots[0] * prepay_engine->gwac - prepay_engine->density.irates[0] );
	...
	for(i=0; i < numProjections; ++i)
	{
		fprintf(fp, "rates[%d] = %f, cpr = %f, paydown=%f\n", i, rates[i], cpr[i]*100.0, principalPayDowns[i]*100.0);
	}

 ...
 *				  analytically with approx price = disc * proba *
 *(fwd
 *+ mrg) * cvg
 *
 * DESCRIPTION  	: XX
 ...
 *				: CMSfixing		- forward fixing rate
 *CMS adjusted for pay date : CMSpay		- forward pay rate CMS adjusted
 *for pay date : cvgpay		- coverage of payment
 *				: discpay		- discount factor to
 *payment : margin		- margin on payment
 ...
 *				: in_out_flag	- proba to be in (SRT_IN) or out
 *(SRT_OUT)
 *				:
 *
 * RETURNS      	: *ans			- price
 ...
 *				: K1			- lower bound strike
 *				: K2			- upper bound strike
 *				: CMSfvol		- vol of Ffixing between
 *today and fixing : CMSpvol		- vol of Fpay between today and fixing :
 *rhofp
 *- correl of Ffixing and Fpay between today and fixing : fwdvol
 *- fwd vol of Fpay between fixing and pay : in_out_flag	- proba to be in
 *(SRT_IN) or out (SRT_OUT)
 *				:
 *
 * RETURNS      	: *ans			- price
 *
...
Err srt_f_sprrf_tok_mc(long today_long, long fixing_long, long pay_long,
  ...
  double slopevol = sqrt(CMSfvol * CMSfvol + CMSpvol * CMSpvol -
                         2 * rhofp * CMSfvol * CMSpvol);
  double slopevar, fwdvar;
  double CMSslope = CMSpay - CMSfixing;
  ...
  if (slopevar + fwdvar > 0) {
    ...
    *ans = 0.0;
    ...
    for (i = 1; i <= NSOBOL; i++) {
      ...
      f1 = CMSfixing + CMSfvol *
                           sqrt(YEARS_IN_DAY * (fixing_long - today_long)) *
                           brownians[1][1];

      correl_brownian =
          rhofp * brownians[1][1] + sqrt(1 - rhofp * rhofp) * brownians[1][2];

      f2 = CMSpay + CMSpvol * sqrt(YEARS_IN_DAY * (fixing_long - today_long)) *
                        correl_brownian;

      f2 += fwdvol * sqrt(YEARS_IN_DAY * (pay_long - fixing_long)) *
            brownians[1][3];

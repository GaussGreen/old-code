...
void transform_oldspec_into_newspec(
    double  alpha,
    double  beta,
    int     floored,
    double  floor,
...
Err cpd_caller(
    ...
    int     use_3F_interp, /*	0: interpolates the ATM market vols linearly, 1: uses the 3F for the
                              interpolation */
    int smile_spec_type,   //	0: lognormal vol + SABR params, 1: sigma-beta + SABR params, 2: BMM
                           //(not yet supported)
    /*		if no calilb */
    char* fx3dund,
    /*	The structure */
    ...
    int* pd_not_ref_nfxpts, /*	Number of notional interpolation points FOR EACH CALL DATE + in the
                               end */
    double** pd_not_ref_fxpts,
    double** pd_not_ref_cpn_at_pts,
    int*     pd_not_ref_lin_xtrpl_l, /*	0 = flat extrapolation to the left, 1 = linear w first 2 pts
                                        slope */
    int* pd_not_ref_lin_xtrpl_r,     /*	0 = flat extrapolation to the right, 1 = linear w last 2 pts
                                        slope */
    ...
    int*
        erased_call_list,  // list of 0 & 1 used only if erasing_call_done = 1 and if use_GMA = TRUE
    ...
    int use_GMA,
    int use_3f_optim_barrier,  // Optimize the boundary with the 3F and then price the smile
                               // adjustment
    double SSLstd,
    int    SSLniter,
    int    CopNsimul,
    ...
    CPD_UND   und_exp)            /*	TS to be exported */
{
    ...
    /*	Results */
    double** Results    = NULL;
    double * mergeTimes = NULL, *sigDom = NULL, *sigFor = NULL, *sigFx = NULL, *corDF = NULL,
           *corDFx = NULL, *corFFx = NULL;
    int mergeNtimes;
    int Height = ncall + 4;
    int Width  = 12;
    ...
    {
        err = serror(
            "Interpolated coupon is temporarily not supported with BetaDLM and AlphaBeta models");
        goto FREE_RETURN;
    }
    /*
    ...
    */
        ...
        smile_spec_type == 3)  // SABR Beta -> SABR Log || BMM Beta -> BMM Log
    {
        for (i = 0; i < num_fx_mkt_vol; i++)
        {
            ...
            temp = (add_unit(fx_mkt_vol_date[i], -2, SRT_BDAY, MODIFIED_SUCCEEDING) - today) *
                   YEARS_IN_DAY;
    ...
    if (!new_call_type || !new_ex_date || !new_set_date || !new_barrier || !new_bar_type ||
        !new_fees || !new_pd_not_ref_fix || !new_pd_not_ref_fix_fx ||
        (use_not_opt_str &&
         (!new_pd_not_ref_nfxpts || !new_pd_not_ref_fxpts || !new_pd_not_ref_cpn_at_pts ||
            ...
            if (ex_date[ncall - 1] > prune_date)
            {
                ...
                while (ex_date[i] <= prune_date)
                {
                    ...
                    if (use_not_opt_str)
                    {
                        ...
                        memcpy(
                            new_pd_not_ref_fxpts[i],
                            pd_not_ref_fxpts[i],
                            pd_not_ref_nfxpts[i] * sizeof(double));
                        memcpy(
                            new_pd_not_ref_cpn_at_pts[i],
                ...
                if (use_not_opt_str & (new_ncall != ncall))  // copy final redemption information:
                {
                    ...
                    memcpy(
                        new_pd_not_ref_fxpts[new_ncall],
                        pd_not_ref_fxpts[ncall],
                        pd_not_ref_nfxpts[ncall] * sizeof(double));
                    memcpy(
                        new_pd_not_ref_cpn_at_pts[new_ncall],
        ...
        for (i = 0; i < ncall; i++)
        {
            if (call_type[i] != -1)
            {
                ...
                if (use_not_opt_str)
                {
                    ...
                    memcpy(
                        new_pd_not_ref_fxpts[j],
                        pd_not_ref_fxpts[i],
                        pd_not_ref_nfxpts[i] * sizeof(double));
                    memcpy(
                        new_pd_not_ref_cpn_at_pts[j],
                        pd_not_ref_cpn_at_pts[i],
                        pd_not_ref_nfxpts[i] * sizeof(double));
    ...
    if (use_cpn_opt_str)
    {
        for (i = 0; i < pd_ncpn; i++)
        {
            ...
            if (ifst_pt > 0)
                memmove(pd_weights[i], pd_weights[i] + ifst_pt, pd_num_strikes[i] * sizeof(double));
        }
    }
    ...
    if (use_not_opt_str)
    {
        for (i = 0; i <= ncall; i++)
        {
            ...
            memcpy(
                pd_not_strikes[i],
                pd_not_ref_fxpts[i] + ifst_pt,
                pd_not_num_strikes[i] * sizeof(double));
            if (ifst_pt > 0)
                memmove(
                    pd_not_weights[i],
                    pd_not_weights[i] + ifst_pt,
                    pd_not_num_strikes[i] * sizeof(double));
        }
    }
    else
    {
        ...
        transform_oldspec_into_newspec(
            ...
            pd_not_ref_capped,
            pd_not_ref_cap,
            &pd_not_num_strikes[ncall],
            &pd_not_wcst[ncall],
            &pd_not_wspot[ncall],
            pd_not_strikes[ncall],
    ...
    if (cpd_dlm_params->use_beta_dlm && cpd_dlm_params->calib_smile && use_calib)
    {
        cpd_dlm_params->smile_settlmt_date = (long)(add_unit(
            fx_spot_date,
            (int)(12 * cpd_dlm_params->calib_smile_maturity + 0.5),
            SRT_MONTH,
        ...
        cpd_dlm_params->calib_smile_maturity =
            YEARS_IN_DAY *
            (add_unit(cpd_dlm_params->smile_settlmt_date, -2, SRT_BDAY, MODIFIED_SUCCEEDING) -
             today);
        ...
        cpd_dlm_params->smile_strikes[0] =
            forward_at_smile * exp(cpd_dlm_params->calib_smile_std_up * vol_at_smile *
                                   sqrt(cpd_dlm_params->calib_smile_maturity));
        cpd_dlm_params->smile_strikes[1] =
            forward_at_smile * exp(cpd_dlm_params->calib_smile_std_down * vol_at_smile *
                                   sqrt(cpd_dlm_params->calib_smile_maturity));
    ...
    //
    //=========================================
    smile_mkt        = (smile_vol_market*)calloc(1, sizeof(smile_vol_market));
    smile_params     = (smile_parameters*)calloc(1, sizeof(smile_parameters));
    fwd_at_vol_dates = (double*)calloc(num_fx_mkt_vol, sizeof(double));
    ...
    if (exercised)
    {
        ...
        for (i = 0; i < cpd->fund_leg->num_cpn && cpd->fund_leg->cpn[i].start_date < ex_date_ex;
             i++)
            ;
        fund_fut_ncpn = i;
        ...
        else
        {
            ...
            if (pd_not_num_strikes[ex_idx] > 0)
            {
                ...
                if (cpn->weights)
                    free(cpn->weights);
                cpn->strikes = (double*)calloc(pd_not_num_strikes[ex_idx], sizeof(double));
                cpn->weights = (double*)calloc(pd_not_num_strikes[ex_idx], sizeof(double));
    ...
    if (fund_fut_ncpn > 0)
    {
        ...
        else
        {
            ...
            if (exercised && cpd->fund_leg->cpn[fund_fut_ncpn - 1].pay_date != ex_date_set)
            {
                if (cpd->fund_leg->cpn[fund_fut_ncpn - 1].pay_date >= today + eod_pay_flag)
                {
                    fund_leg -=
                        swp_f_df(
                            und->today, cpd->fund_leg->cpn[fund_fut_ncpn - 1].pay_date, fund_yc) *
                        cpd->fund_leg->notional;
                }
                ...
                {
                    fund_leg +=
                        swp_f_df(und->today, ex_date_set, fund_yc) * cpd->fund_leg->notional;
                }
            }
        }
        ...
        {
            partial_fund_leg +=
                swp_f_df(und->today, cpd->fund_leg->cpn[partial_fund_idx].start_date, fund_yc) *
                cpd->fund_leg->notional;
        }
    }
    else
    {
        ...
        if (fin_not_date >= today + eod_pay_flag)
        {
            if (for_fund)
            {
                fund_leg += swp_f_df(und->today, fin_not_date, Copyfund_ccy_yc) * thirdCcyfundNot *
                            fx_fund_dom * swp_f_df(und->today, fx_fund_dom_spot_date, dom_yc) /
                            swp_f_df(und->today, fx_fund_dom_spot_date, Copyfund_ccy_yc);
            }
            else
            {
    ...
    if (start_date >= und->today + eod_pay_flag)
    {
        if (for_fund)
        {
            fund_leg -= swp_f_df(und->today, start_date, Copyfund_ccy_yc) * thirdCcyfundNot *
                        fx_fund_dom * swp_f_df(und->today, fx_fund_dom_spot_date, dom_yc) /
                        swp_f_df(und->today, fx_fund_dom_spot_date, Copyfund_ccy_yc);
        }
        else
    ...
    {
        partial_fund_leg -=
            swp_f_df(und->today, cpd->call[0].set_date, fund_yc) * cpd->call[0].fund_not_amt;
    }
    ...
    if (cpd_dlm_params->use_beta_dlm)
    {
        ...
        Inst      = (FxBetaDLM_FxOptInst*)calloc(1, sizeof(FxBetaDLM_FxOptInst));
        InstConst = (FxBetaDLM_InstPrecalc*)calloc(1, sizeof(FxBetaDLM_InstPrecalc));
    ...
    for (i = 0; i < pd_fut_ncpn; i++)
    {
        ...
        if (cpd_dlm_params->use_beta_dlm)
        {
            ...
            err = FxBetaDLM_Price_FxOptInst(
                Inst, und->model, InstConst, und->hermite, und->num_params, &dlm_fwd);
                ...
                if (fabs(cpn->weights[str_idx]) > 1e-16)
                {
                    ...
                    if (use_sabr)
                    {
                        ...
                        smile_opt_string +=
                            cpn->weights[str_idx] *
                            OPT_VAL_MACRO(
                                type, fwd, cpn->strikes[str_idx], smile_std, half_smile_std);
                    }
                }
            }
            if (use_sabr)
        ...
        else
        {
            ...
            if (use_sabr)
            {
                ...
                {
                    cpd_vol_get_vol(
                        fwd, cpn->fx_fix_time, str, smile_params, SRT_LOGNORMAL, &smile_std);
            ...
            if (cpd_dlm_params->use_beta_dlm)
            {
                if (type >= 3)
                {
                    ...
                    err = FxBetaDLM_Price_FxOptInst(
                        Inst, und->model, InstConst, und->hermite, und->num_params, &dlm_floor);
            ...
            if (cpd_dlm_params->use_beta_dlm)
            {
                if (type >= 3)
                {
                    ...
                    err = FxBetaDLM_Price_FxOptInst(
                        Inst, und->model, InstConst, und->hermite, und->num_params, &dlm_cap);
            ...
            if (use_sabr)
            {
                ...
                {
                    adj_fees[i] =
                        df * fabs(cpn->beta) * (smile_floor - dlm_floor - smile_cap + dlm_cap);
                }
                else
                {
    ...
    {
        partial_pd_leg -=
            swp_f_df(und->today, cpd->call[0].set_date, und->dom_yc) * cpd->call[0].pd_not_amt;
    }
            ...
            if (use_cpn_opt_str)
            {
                ...
                for (str_idx = 0; str_idx < pd_num_strikes[i]; str_idx++)
                {
                    if (fabs(pd_weights[i][str_idx]) > 1e-16 &&
                        ...
                        opt_string +=
                            pd_weights[i][str_idx] * (pd_fix_fx[i] - pd_strikes[i][str_idx]);
                }
    ...
    if (call_feat > 0 && use_sabr >= 1) /* First update fees in all preinitialized structures */
    {
        ...
        if (!und->fees)
            und->fees = (double*)calloc(und->nb_fees, sizeof(double));
        if (!und->fees_dates)
            und->fees_dates = (double*)calloc(und->nb_fees, sizeof(double));
    ...
    if (cpd_dlm_params->use_beta_dlm && fabs(und->model->dAlpha) > TINY)
    {
                ...
                case -1:
                    if (do_optim)
                    {
                        ...
                        err = cpd_launch_opti_mc(
                            cpd, und, mc_arg, &call, &call_std, do_infos, optim_bar);
                    }
                    else
                    {
                        smessage(
                            "Launching tree, time steps requested: %d, actual: %d",
                            req_stp,
                            tree_arg->nstp);
        ...
        *call_stdev =
            sqrt(0.5 * (*call_stdev * *call_stdev + call_val_alpha_std * call_val_alpha_std));
    }
    else
    {
        if (call_feat > 0 && use_sabr == 2)
        ...
        if (call_feat == 1)
        {
            ...
            case -1:
                if (do_optim)
                {
                    smessage("Launching optim MC");
                    err =
                        cpd_launch_opti_mc(cpd, und, mc_arg, &call, &call_std, do_infos, optim_bar);
                }
                else
                {
                    ...
                    {
                        err = serror(
                            "KO in a tree temporarily not supported with option string coupon or "
                            "redemption");
                        goto FREE_RETURN;
                    }
            ...
            case 0:
                ...
                smessage(
                    ...
                    tree_arg->nstp);
                err = cpd_launch_tree(
                    cpd, und, tree_arg, &call, &temp); /* Calculates the IV in the tree */
                break;
            }
    ...
    if (!exercised && use_GMA && cpd->num_calls > 0)
    {
        ...
        OTC_params = (otcpd_params*)calloc(1, sizeof(otcpd_params));
        ...
        cpd_init_otc_params(
            ...
            fwdSmileVisuNstd,
            /* otc */
            fwdVolMethod, /*	0=Sliding 3F vol; 1=Converging 3F vol; 2=Sliding Cvg Sbeta; 3=Cvg
                             Cvg Sbeta */
        ...
        Precalculations
        ================================== */
        OTC_precalc = (otcpd_precalc*)calloc(1, sizeof(otcpd_precalc));
        ...
        else
        {
            ...
            *smile_adjustment = -(Results[0][0] - Results[1][0]);
            ...
            if (OTC_precalc)
                cpd_free_otc_precalc(
                    OTC_precalc, cpd->num_calls, cpd->pd_leg->num_cpn, cpd->fund_leg->num_cpn);
            Results[0][0] = 0.0;
            Results[1][0] = 0.0;
...
FREE_RETURN:
    ...
    if (new_pd_not_ref_cpn_at_pts)
        free_dmatrix(new_pd_not_ref_cpn_at_pts, 0, alloc_ncall, 0, max_interp_pts - 1);

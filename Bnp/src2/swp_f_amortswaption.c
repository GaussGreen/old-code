...
Err ShiftedLogCopulaSimul(
    double** corr_mtx, long NPaths, long nFactorDim, int* pCutDim, double** res)
{
    int i, j, k;
...
void ShiftedLogCopulaSimul_NotOpt(
    double** corr_mtx,
    long     NPaths,
    long     Dim,
    double*  Fwd,
    ...
    double** res)
{
    ...
    for (i = 0; i < NPaths; i++)
    {
        ...
        for (j = 0; j < Dim; j++)
        {
            ...
            //				res[i][j] = sign * ( (sign * Fwd[j] + Shifts[j]) * exp(-0.5 * Vols[j] *
            //Vols[j] + Vols[j] * res[i][j]) - Shifts[j] );
            res[i][j] =
                ((Fwd[j] + Shifts[j]) * exp(-0.5 * Vols[j] * Vols[j] + Vols[j] * res[i][j])) -
                Shifts[j];
        }
    }
    ...
    double* dVol)
{
    ...
    */
    ...
    k1 = dFwdSwap;
    k2 = DMIN(
        dFwdSwap * (1 + 4 * sqrt(ex_time) * atmvol),
    ...
    */
    ...
    if (iCalibshift == 0)
    {
        err = find_shifted_log_params(
            ex_time, dFwdSwap, k1, vol1, k2, vol2, &shift, &shifted_vol, &equi_beta);
        *dShift = shift;
    }
    else
    {
        price2 = srt_f_optblksch(dFwdSwap, k2, vol2, ex_time, 1.0, SRT_CALL, PREMIUM);
    ...
    double*  dPrice)
{
    ...
    if (err = err = translate_compounding(&cFreq, srtFreq))
        return err;
    if (err = err = swp_f_get_ref_rate_details(cRefRname, &srtFloatBasis, &srtFloatFreq))
        return err;
    ...
    double*  dPrice)
{
    ...
    */
    ...
    for (i = 0; i < lNFloatDates; ++i)
    {
        //		dFloatCoverages[i] = coverage(lFloatStartDatesOut[i], lFloatEndDatesOut[i],
        //srtFloatBasis);
        dFloatCoverages[i] = coverage(lFloatStartDates[i], lFloatEndDates[i], srtFloatBasis);
    }
    ...
    ShiftedLogCopulaSimul_NotOpt(
        Correl, xlNumSim, lNFixDates, dFwdSwapRates, dShifts, dVols, dCopulaCube);
    ...
    for (j = 0; j < lNFixDates; ++j)
    {
        ...
        dFwdLvl  = dLvls[j];
        mean[j] =
            dCoInitNotionals[j] * dFwdLvl * (dFwdSwap - dStrikes[j]) - mean[j] / (xlNumSim - 2);
    }
    ...
    double*  dPrice)
{
    ...
    err = swp_f_make_FixedLegDatesAndCoverages(
        &Swap, lToday, &lPayDates, &iNPayDates, &lStartDates, &lEndDates, &dCoverages, &iNDates);
    if (err)
    {
        goto FREE_RETURN;
    }
    ...
    shortstub = 0;
    if ((iNDates > 1) && ((int)(12 * (lEndDates[0] - lStartDates[0]) / 365.0 + 0.5) !=
    ...
    ShiftedLogCopulaSimul_NotOpt(
        Correl, xlNumSim, iNDates, dFwdSwapRate, dShifts, dVols, dCopulaCube);
    ...
    int      iDistribMethod)
{
    ...
    err =
        "Function PriceAmortizedSwaptionShiftedLog no more supported, please use "
    ...
    int      iDistribMethod)
{
    ...
    err =
        "Function PriceAmortizedSwaptionSABR no more supported, please use "
        "AmortizedSwaptionShiftedLogForMAD";
                                           ...
                                           )
{
        ...
        double	***GaussSample=NULL,**UnifSample=NULL,
                ...
                //////////  Step3: Generates the matrix of r.v. compatible with the Copula and the
marginals for (i=0;i<p;i++)
                {
                        for (j=0; j<d; j++)
                        {
                                GaussSample[i][j][0]=inv_cumnorm_fast(UnifSample[i][j]);
...
//		err = srt_f_optimpvol(price2, shift+dFwdSwap, shift+k2, ex_time, 1, SRT_CALL,
SRT_LOGNORMAL, &shifted_vol);
//	}
...
// dCPDy,2*nClass+1,0,RANDOM_GAUSS,dCopulaCubeShiftedLog);
        ...
        for(j=0;j<lNFixNot;++j)
        {
//		err = swp_f_ForwardRate(lStartDates[0], lEndDates[j], cFreq, cBasis, cYCname,
cRefRname, &dFwdSwap); dFwdSwap = dFwdSwapRate[j]; dFwdLvl = dLvls[j]; mean[j] = dCoInitNotionals[j]
...
dFixNotionals[iNFixDates-i-1]
                                                                                ...
                                                                                -
dFixCoverages[iNFixDates-i-1] * sum ) / (1 + dFixCoverages[iNFixDates-i-1] *
dStrikes[iNFixDates-i-1] ); sum += dStrikes[iNFixDates-i-1] * (*dAmounts)[iNFixDates-i-1];
        }
...
/*
double PV_IRR(char *cYCname, long lToday, int iIndexStart, int iNPayDates, long *lPayDates, double
*dCpns, double R)
{
        int i;
        double PV;
...
*lFloatEndDates=NULL; double *dFloatCoverages=NULL; double *dFloatSpreads=NULL; int i, FixFloatMult;
        double *dCpns=NULL;
...
dFixNotionals[(int)(i/FixFloatMult)-1];
        ...
        for(i=1;i<iNFixPayDates;++i)
        {
                ...
                PV_Float = PV_IRR(cYCname, lToday, 0, i*FixFloatMult+1, lFloatPayDates, dCpns_Float,
R); PV_Fix = PV_IRR(cYCname, lToday, 0, i+1, lFixPayDates, dCpns_Fix, R);

                dStrikes[i-1] = -PV_IRR(cYCname, lToday, 0, i*FixFloatMult+1, lFloatPayDates,
dCpns_Float, R); dStrikes[i-1] = dStrikes[i-1] / PV_IRR(cYCname, lToday, 0, i+1, lFixPayDates,
dCpns_Fix, R);
...
//							&dCoInitStrikes,
        ...
        *dPrice=0.0;
        ...
        {
                mean[j] = 0.0;
                err = swp_f_LevelPayment(lStartDates[0], lEndDates[j], cFreq, cBasis, cYCname,
cRefRname, &(dLvls[j]));
        }
        ...
        for (i=3; i<xlNumSim; i++)
        {
                for(j=0;j<iNCoInitSwaps;++j)
                {
//			err = swp_f_LevelPayment(lStartDates[0], lEndDates[j], cFreq, cBasis,
cYCname, cRefRname, &dFwdLvl);
...
//		dFwdLvl = (1.-1./pow(1+dFwdSwap*dCov,j+1))/dFwdSwap;
                dFwdLvl = dLvls[j];
//		err = swp_f_LevelPayment(lStartDates[0], lEndDates[j], cFreq, cBasis, cYCname,
cRefRname, &dFwdLvl); mean[j] = dCoInitNotionals[j] * dFwdLvl * ( dFwdSwap - dStrikes[j]) - mean[j]
/ (xlNumSim-2);
        }
...
//      computes the Fwd swap rate
                ...
                err =
swp_f_ForwardRate(lPayDatesLong[0],lPayDatesLong[i],cFreq,cBasis,cYCname,cRefRname,&dFwd); err =
swp_f_LevelPayment(lPayDatesLong[0],lPayDatesLong[i],cFreq,cBasis,cYCname,cRefRname,&dLevel);

                err =
GetDensity_Lvl_Measure9_b(nClass,dMat,dFwd,SigmaHeston[i-1],AlphaHeston[i-1],1.0,LambdaHeston[i-1],
    ...
    err = swp_f_setSwapDP(lStartDate, lEndDate, srtFreq, srtBasis, &Swap);
        ...
        *HestonLambda = (double*) calloc (iNDates, sizeof (double));
        ...
        for(i=0;i<iNDates;++i)
        {
                ...
                dFwdCash = swp_f_swapcashrate( start, end, srtBasis, srtFreq, cYCname, cRefRname);
                err = swp_f_LevelPayment(start, end, cFreq, cBasis, cYCname, cRefRname, &dLvl);
                ex_time = (add_unit (start, - 2, SRT_BDAY, MODIFIED_SUCCEEDING) - lToday) *
YEARS_IN_DAY;
...
//	int NSpreads;
    ...
    err = swp_f_setSwapDP(xlStartDate, xlEndDate, srtFreq, srtBasis, &Swap);
        ...
        for(i=0;i<iNDates;++i)
        {
                err = swp_f_ForwardRate(lStartDates[0], lEndDates[i], cFreq, cBasis, cYCname,
cRefRname, &dFwdSwap); for(j=0;j<2*nClass+1;++j)
                {
...
// dCPDy,2*nClass+1,0,RANDOM_GAUSS,dCopulaCubeShiftedLog);
        ...
        {
                err = swp_f_ForwardRate(lStartDates[0], lEndDates[i], cFreq, cBasis, cYCname,
cRefRname, &dFwdSwap); for(j=0;j<xlNumSim+1;++j)
                {
//			shift = HESTON_CONST*(1.-BetaHeston[i])/BetaHeston[i];
                        shift = dFwdSwap*(1.-BetaHeston[i])/BetaHeston[i];
...
log((shift+dPPDx[nTen][i])/(shift+dFwdSwap)); shifted_vol = (x_PrMin*sqrt(ex_time) +
sqrt(discriminent))/ex_time; i=0;
                }
        }
...
//	shift = HESTON_CONST*(1.-beta)/beta;
        ...
        *dShifts = shift;
        ...
        temp_moins = srt_f_optblkschbetastochquick(
                                                                shift+dFwdSwap,
                                                                DMAX(0.0001, shift+dPPDx[nTen][0] -
dK), ex_time, shifted_vol, alpha, 1.0, rho, 1.0, SRT_BETAVOL, SRT_LOGNORMAL, SRT_CALL, PREMIUM);
...
//		{
                        err = GetAmortizedSwaptionPayoff5(dCopulaCube,
                                                                                        ...
                                                                                        dCoverages,
                                                                                        iNCoInitSwaps,
// dCoInitStrikes, dStrikes, dCoInitNotionals, i, &dPayoff, flag); if(err)
                        {
                                goto FREE_RETURN;
                        }
...
//		*dPrice += df_start * DMAX(0, flag * dPayoff) - flag * (dSwapPV + df_start *
dPayoff);

...
Err AmortizedSwaptionShiftedLogForMAD_NotOpt(
  ...
  */
  ...
  for (i = 0; i < lNFixDates; ++i) {
    //		dFixCoverages[i] = coverage(lFixStartDatesOut[i]  , lFixEndDatesOut[i]
    //, srtBasis);
    dFixCoverages[i] = coverage(lFixStartDates[i], lFixEndDates[i], srtBasis);
  }
  ...
  //	if( add_unit (lToday  , 2  , SRT_BDAY  , MODIFIED_SUCCEEDING) >=
  //lFixStartDatesOut[0])
  if (add_unit(lToday, 2, SRT_BDAY, MODIFIED_SUCCEEDING) >= lFixStartDates[0]) {
...
FREE_RETURN:
  ...
  //,lNFixDates); 	if(lFixEndDatesOut) free_lngvector(lFixEndDatesOut  , 0
  //,lNFixDates);

  //	if(lFloatStartDatesOut) free_lngvector(lFloatStartDatesOut  , 0
  //,lNFloatDates); 	if(lFloatEndDatesOut) free_lngvector(lFloatEndDatesOut  , 0
  //,lNFloatDates);
...
Err PriceAmortizedSwaptionShiftedLog(
  ...
  err = "Function PriceAmortizedSwaptionShiftedLog no more supported  , please "
...
Err PriceAmortizedSwaptionSABR(char *cYCname, char *cVCname, char *cRefRname,
  ...
  err = "Function PriceAmortizedSwaptionSABR no more supported  , please use "
...
correlation matrix corr_mtx CholDec	(&CholAlg  , corr_mtx  , sqrt_corr_mtx ,
                                 d);
...
/*
double amortSwap_PV(long lToday  , char *cYCname  , int iNCpns  , long
*lCpnDates  , double *dCpns)
{
        int i;
        double PV;
...
, SrtBasisCode srtBasis  , double	 *dCoverages  , int     iNCoInitSwaps  ,
                                                         double  *dStrikes  ,
                                                         double  *dNotionals  ,
                                                         int     iMC  ,
                                                         double  *dPayoff  ,
                                                         ...
                                                         int flag)
{
        ...
        *dPayoff = 0;
        ...
        {
                dLevelCash =(1.-1./pow(1+dCopulaCube[iMC][i]*dCov
,i+1))/dCopulaCube[iMC][i]; dLevelCash =dLvls[i]; *dPayoff += dNotionals[i] *
dLevelCash *(dCopulaCube[iMC][i]-dStrikes[i]) + controlvar[i];
        }
                                                ...
                                                )
{
        ...
        err = swp_f_ForwardRate(start  , end  , cFreq  , cBasis  , cYCname  ,
cRefRname  , &dFwdSwap); dFwdCash = swp_f_swapcashrate( start  , end  , srtBasis
, srtFreq  , cYCname  , cRefRname); err = swp_f_LevelPayment(start  , end  ,
cFreq  , cBasis  , cYCname  , cRefRname  , &dLvl); ex_time = (add_unit (start  ,
...
//	{
//		shift = 10000;
//		price2 = srt_f_optblksch(dFwdSwap  , k2  , vol2  , ex_time  , 1
, SRT_CALL  , SRT_PREMIUM);
//		err = srt_f_optimpvol(price2  , shift+dFwdSwap  , shift+k2  ,
ex_time  , 1  , SRT_CALL  , SRT_LOGNORMAL  , &shifted_vol);
//	}
        ...
        *dShift = shift;
        ...
        temp = srt_f_optblksch(shift + sgn * dFwdSwap  ,
                                                                shift + sgn *
dPPDx[nTen][0]  , shifted_vol  , ex_time  , 1.0  , callput  , PREMIUM);

        temp_plus = srt_f_optblksch(shift + sgn * dFwdSwap  ,
                                                                shift + sgn *
dPPDx[nTen][1]  , shifted_vol  , ex_time  , 1.0  , callput  , PREMIUM);
        ...
        for (i=1;i<2*nClass;i++)
        {
                ...
                temp_plus = srt_f_optblksch(shift + sgn * dFwdSwap  ,
                                                                shift + sgn *
dPPDx[nTen][i+1]  , shifted_vol  , ex_time  , 1.0  , callput  , PREMIUM);
...
,nTen+1))/dPPDx[nTen][2*nClass]; Df_x = 1;//./(1+dPPDx[nTen][2*nClass]*dCov);
...
get_spotlag_from_curve(pCurve);
        ...
        ///// defines the relevant swap objects
        ///////////////////////////////////////////////////////////////
    err = swp_f_setSwapDP(xlStartDate  , xlEndDate  , srtFreq  , srtBasis  ,
&Swap); if (err)
        {
                goto FREE_RETURN;
        }
        ...
        err = swp_f_make_FixedLegDatesAndCoverages(&Swap  ,lToday  ,&lPayDates
,&iNPayDates  , &lStartDates  ,&lEndDates  , &dCoverages  ,&iNDates); if (err)
        {
                goto FREE_RETURN;
        }
...
//---------------------------------------------------------------------------------
        ...
        err = ConvertAmortSwapWithMarginsInCoInitSwapPortfolio2(
                                                                 ...
                                                                 lNFloatNot  ,
                                                                 dFloatNotionals
, dMargins  , dStrikes  , dCoInitNotionals  , dLvls  , dFwdSwapRate  , UseVol
                                                                 );
...
,RANDOM_GAUSS  ,dCopulaCubeShiftedLog);
        ...
        CopulaSimul(Correl  ,xlNumSim  ,iNDates  ,dPPDShiftedLogx  ,dCPDy
,2*nClass+1  ,dCopulaCubeShiftedLog);
...
*(dCopulaCube[i][j]-dStrikes[j]);
        ...
        for(j=0;j<lNFixNot;++j)
        {
//		err = swp_f_ForwardRate(lStartDates[0]  , lEndDates[j]  , cFreq
, cBasis  , cYCname  , cRefRname  , &dFwdSwap); dFwdSwap = dFwdSwapRate[j];
                dFwdLvl = dLvls[j];
                mean[j] = dCoInitNotionals[j] * dFwdLvl * ( dFwdSwap -
dStrikes[j]) - mean[j] / (xlNumSim-2);
        }
...
,2*nClass);
        ...
        if(dCopulaCubeShiftedLog) free_dmatrix(dCopulaCubeShiftedLog  ,0
...
Err		ConvertAmortSwapInCoInitalSwapsPortfolio2(
                                                                 char *cYCname ,
                                                                 char *cRefRname
, long StartDate  , long EndDate  , SrtCompounding srtFreq  , SrtBasisCode
srtBasis  , long	lNFixNot  , double *dFixNotionals  , double *dFixRates ,
                                                                 double
*dStrikes  , long	*lNCoInitSwaps  , double  **dAmounts)
{
        Err err = NULL;
        int i;
        SrtCurvePtr	pCurve = lookup_curve (cYCname);
    ...
    err = swp_f_setSwapDP(StartDate  , EndDate  , srtFreq  , srtBasis  , &Swap);
        ...
        err = swp_f_make_FixedLegDatesAndCoverages(&Swap  ,lToday ,&lFixPayDates
,&iNFixPayDates  , &lFixStartDates  ,&lFixEndDates  , &dFixCoverages
...
/*
double PV_IRR(char *cYCname  , long lToday  , int iIndexStart  , int iNPayDates
, long *lPayDates  , double *dCpns  , double R)
{
        int i;
        double PV;
...
*lFloatStartDates=NULL  , *lFloatEndDates=NULL; double *dFloatCoverages=NULL;
...
cRefRname  , &lFloatPayDates  , &iNFloatPayDates  , &lFloatFixingDates  ,
                                                                                                &lFloatStartDates  ,
                                                                                                &lFloatEndDates  ,
                                                                                                &dFloatCoverages  ,
                                                                                                &dFloatSpreads  ,
...
dFixRates[iNFixPayDates-2]
        ...
        R = 0.05;
        PV = PV_IRR(cYCname  , lToday  , 0  , iNFloatPayDates  , lFloatPayDates
, dCpns  , R);
        ...
        {
                R2 = R + 0.0001;
                PV2 = PV_IRR(cYCname  , lToday  , 0  , iNFloatPayDates  ,
lFloatPayDates  , dCpns  , R2);
...
lFloatPayDates  , dCpns  , R);
        ...
        for(i=1;i<iNFixPayDates;++i)
        {
                ...
                PV_Float = PV_IRR(cYCname  , lToday  , 0  , i*FixFloatMult+1  ,
lFloatPayDates  , dCpns_Float  , R); PV_Fix = PV_IRR(cYCname  , lToday  , 0  ,
i+1  , lFixPayDates  , dCpns_Fix  , R);

                dStrikes[i-1] = -PV_IRR(cYCname  , lToday  , 0  ,
i*FixFloatMult+1  , lFloatPayDates  , dCpns_Float  , R); dStrikes[i-1] =
dStrikes[i-1] / PV_IRR(cYCname  , lToday  , 0  , i+1  , lFixPayDates  ,
dCpns_Fix  , R);
...
//							&dCoInitStrikes  ,
        ...
        *dPrice=0.0;
        ...
        {
                mean[j] = 0.0;
                err = swp_f_LevelPayment(lStartDates[0]  , lEndDates[j]  , cFreq
, cBasis  , cYCname  , cRefRname  , &(dLvls[j]));
        }
...
, cFreq  , cBasis  , cYCname  , cRefRname  , &dFwdLvl);
...
*(dCopulaCube[i][j]-dStrikes[j]);
        ...
        {
                err = swp_f_ForwardRate(lStartDates[0]  , lEndDates[j]  , cFreq
, cBasis  , cYCname  , cRefRname  , &dFwdSwap);
//		dFwdLvl = (1.-1./pow(1+dFwdSwap*dCov  ,j+1))/dFwdSwap;
                dFwdLvl = dLvls[j];
//		err = swp_f_LevelPayment(lStartDates[0]  , lEndDates[j]  , cFreq
, cBasis  , cYCname  , cRefRname  , &dFwdLvl); mean[j] = dCoInitNotionals[j] *
dFwdLvl * ( dFwdSwap - dStrikes[j]) - mean[j] / (xlNumSim-2);
        }


//	dSwapPV = amortSwap_PV(lToday  , cYCname  , iNCpns  , lCpnDates  ,
dCpns); df_start = swp_f_df(lToday  , xlStartDate  , cYCname); flag = -1; if
(srtCallPut == SRT_CALL) flag=1; for (i=3; i<xlNumSim; i++)
        {
//		if(iPayoffMethod==5)
//		{
...
dStdDev*dEqBSVol*sqrt(dMat);
        ...
        HestonPrice(dFwd  ,strikeVect  ,2*nClass+1  ,dMat  ,
                ...
                1.0  ,dUpBound  ,SRT_CALL  ,PREMIUM  ,SRT_TRUE  ,2  ,nSteps
,priceVect);
...
iNPayDatesLong  , Date *lPayDatesLong  , double *dCoveragesLong  ,
                                                                 ...
                                                                 double
*SigmaHeston  , double *AlphaHeston  , double *LambdaHeston  , double
*BetaHeston  , double *RhoHeston  ,
...
,cFreq  ,cBasis  ,cYCname  ,cRefRname  ,&dFwd); err =
swp_f_LevelPayment(lPayDatesLong[0]  ,lPayDatesLong[i]  ,cFreq  ,cBasis ,cYCname
,cRefRname  ,&dLevel);

                err = GetDensity_Lvl_Measure9_b(nClass  ,dMat  ,dFwd
,SigmaHeston[i-1]  ,AlphaHeston[i-1]  ,1.0  ,LambdaHeston[i-1]  ,
                                         BetaHeston[i-1]  ,RhoHeston[i-1]
,dLevel  ,dUpBound  ,iNPts  ,i-1  ,dCov  ,dPPDx  ,dPPDy  ,dCPDy);
        }
...
srtBasis); err = translate_compounding(&cFreq  , srtFreq);
        ...
        //-----Init and Fill
SWAPDP-------------------------------------------------------- err =
swp_f_setSwapDP(lStartDate  , lEndDate  , srtFreq  , srtBasis  , &Swap); if(err)
        {
                goto FREE_RETURN;
        }
        err = swp_f_make_FixedLegDatesAndCoverages(&Swap  ,lToday  ,&lPayDates
,&iNPayDates  , &lStartDates  ,&lEndDates  , &dCoverages  ,&iNDates); if(err)
        ...
        *HestonLambda = (double*) calloc (iNDates  , sizeof (double));
        ...
        for(i=0;i<iNDates;++i)
        {
                ...
                err = swp_f_ForwardRate(start  , end  , cFreq  , cBasis  ,
cYCname  , cRefRname  , &dFwdSwap); dFwdCash = swp_f_swapcashrate( start  , end
, srtBasis  , srtFreq  , cYCname  , cRefRname); err = swp_f_LevelPayment(start
, end  , cFreq  , cBasis  , cYCname  , cRefRname  , &dLvl); ex_time = (add_unit
(start  , - 2  , SRT_BDAY  , MODIFIED_SUCCEEDING) - lToday) * YEARS_IN_DAY;
                ...
                err = CalibrateHestonToSABR(dFwdSwap  ,
                                                                        ex_time
, SABRsigma  , SABRalpha  , SABRbeta  , SABRrho  , &HESTONsigma  , &HESTONalpha
, &HESTONsigmainfinity  , &HESTONlambda  , &HESTONbeta  , &HESTONrho  ,
                                                                        UpperBound
, nSteps  , iIntegerType  , nStdDev  , SRT_TRUE  , CHI2_MIN);
...
lToday  ,lSpotLag;
        ...
        //---Swap
Details------------------------------------------------------------------ char
*cBasis  , *cFreq; Date *lPayDates=NULL  ,*lStartDates=NULL  ,*lEndDates=NULL;
        double *dCoverages=NULL;
        int iNDates  , iNPayDates;

        //---Monte Carlo
Paths------------------------------------------------------------- double
**dCopulaCube=NULL  ,**dCopulaCubeShiftedLog=NULL  ,*dMean_v=NULL  ,*dAvg=NULL;

        //---Distribution
Discretization--------------------------------------------------- double
**dPPDx=NULL  ,**dPPDy=NULL  ,**dCPDy=NULL; double **dPPDShiftedLogx=NULL;
...
get_today_from_curve (pCurve); lSpotLag = (Date) get_spotlag_from_curve(pCurve);
        ...
        //-----Init and Fill
SWAPDP-------------------------------------------------------- err =
swp_f_setSwapDP(xlStartDate  , xlEndDate  , srtFreq  , srtBasis  , &Swap);
        if(err)
        {
                goto FREE_RETURN;
        }
...
,iNDates-1  ,0  ,2*nClass); dPPDy=dmatrix(0  ,iNDates-1  ,0  ,2*nClass);
        dCPDy=dmatrix(0  ,iNDates-1  ,0  ,2*nClass);
...
,2*nClass); for(i=0;i<iNDates;++i)
        {
                err = swp_f_ForwardRate(lStartDates[0]  , lEndDates[i]  , cFreq
, cBasis  , cYCname  , cRefRname  , &dFwdSwap); for(j=0;j<2*nClass+1;++j)
                {
...
log--------------------------- for(i=0;i<iNDates;++i)
        {
                err = swp_f_ForwardRate(lStartDates[0]  , lEndDates[i]  , cFreq
, cBasis  , cYCname  , cRefRname  , &dFwdSwap); for(j=0;j<xlNumSim+1;++j)
                {
//			shift = HESTON_CONST*(1.-BetaHeston[i])/BetaHeston[i];
                        shift = dFwdSwap*(1.-BetaHeston[i])/BetaHeston[i];
                        dCopulaCube[j][i] = exp(dCopulaCubeShiftedLog[j][i]) -
shift;
        ...
        //--------Convert AMort Swap in Coinit Swap
Portfolio------------------------------ dStrikes = dvector(0  ,iNDates-1); err =
ComputeIRRStrikes(cYCname  , lToday  , cRefRname  , xlStartDate  , xlEndDate  ,
                                                 srtFreq  ,srtBasis  ,
                                                 lNFixNot  ,dFixNotionals  ,
                                                 lNFloatNot  ,dFloatNotionals
,dFixRates  , dStrikes); if (err)
...
Portfolio------------------------------ dStrikes = dvector(0  ,iNDates-1);
        ...
        err = ConvertAmortSwapWithMarginsInCoInitSwapPortfolio(
                                                                 ...
                                                                 lNFloatNot  ,
                                                                 dFloatNotionals
, dMargins  , dStrikes  , dCoInitNotionals  , UseVol); if (err)
        {
                goto FREE_RETURN;
        }
...
,nTen+1))/dPPDx[nTen][0];
        ...
        for (i=1;i<2*nClass;i++)
        {
                ...
                temp_plus = srt_f_optblkschbetastochquick(
                                                                dFwdSwap  ,
                                                                dPPDx[nTen][i+1]
, ex_time  , sigma  , alpha  , beta  , rho  , 1.0  , SRT_BETAVOL  ,
                                                                SRT_LOGNORMAL  ,
                                                                SRT_CALL  ,
                                                                PREMIUM);
...
, shifted_vol  , ex_time  , 1.0  , SRT_CALL  , PREMIUM);
...
//	shift = HESTON_CONST*(1.-beta)/beta;
        ...
        *dShifts = shift;
        ...
        temp_moins = srt_f_optblkschbetastochquick(
                                                                shift+dFwdSwap ,
                                                                DMAX(0.0001  ,
shift+dPPDx[nTen][0] - dK)  , ex_time  , shifted_vol  , alpha  , 1.0  , rho  ,
                                                                1.0  ,
                                                                SRT_BETAVOL  ,
                                                                SRT_LOGNORMAL  ,
                                                                SRT_CALL  ,
        ...
        temp = srt_f_optblkschbetastochquick(
                                                                shift+dFwdSwap ,
                                                                shift+dPPDx[nTen][0]
, ex_time  , shifted_vol  , alpha  , 1.0  , rho  , 1.0  , SRT_BETAVOL  ,
                                                                SRT_LOGNORMAL  ,
                                                                SRT_CALL  ,
                                                                PREMIUM);
...
//		{
                        err = GetAmortizedSwaptionPayoff5(dCopulaCube  ,
                                                                                        ...
                                                                                        iNCoInitSwaps  ,
//
dCoInitStrikes  , dStrikes  , dCoInitNotionals  , i  , &dPayoff  , flag);
                        if(err)
                        {
                                goto FREE_RETURN;
                        }

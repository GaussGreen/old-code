...
static Err clsdfrm_FixedAndSpreadLegs(
    GenSwapLeg*     fixedleg,
...
/* -------------------------------------------------------------------------
   ...
   PURPOSE:  LGM model --- closed form prices for swaptions, bond options,
                 caps and floors.
    ...
    double*         answer)
{
    ...
    err = swp_f_make_FixedAndNotionalsLeg(sdp, strike, initial, final, today, &fixedleg);
    if (err)
        return err;
    ...
    err = swp_f_make_SpreadLeg(sdp->start, sdp->end, today, ref_rate_code, &spreadleg);
    if (err)
    {
    ...
    err = clsdfrm_FixedAndSpreadLegs(
        &fixedleg, &spreadleg, rec_pay, type, und, mdl_type, mdl_dim, answer);
    ...
    double*         answer)
{
    ...
    case CAPFLOOR:
        ...
        break;
        /*		case RESETCAPFLOOR:
                                fixing_date = srt_calloc (cfs->dl.len + 1, sizeof(double));
        */			/* No fixing for last pay date */
        /*			for (i = 0; i <df.len; i++)
           ...
           cfs->sdp.spot_lag, SRT_BDAY, SUCCEEDING);
                                ...
                                {
                                        free_inDlist(df);
                                        return serror ("Closed form: error in resetcap pricing");
                                }
                        break;
        */

    ...
    for (i = sAux->iStartIndex; i <= sAux->iEndIndex; i++)
    {
        ...
        if (sFunding->lFixDate[i] < sMarket->lToday + sMarket->iEODFixFlag)
        {
            sAux->dMarketValue[i] = (sFunding->dFixCoupon[i] + sFunding->dMargin[i]) *
                                    sFunding->dCoverage[i] * df_end * sFunding->dNot[i] *
                                    sAux->dCashFx;
        }
        else
        {
            ...
            sAux->dMarketValue[i] =
                (df_start - df_end +
                 (sFunding->dMargin[i] + sAux->dInitSpread[i]) * sFunding->dCoverage[i] * df_end) *
        ...
        if (sFunding->iCCY)
        {
            if (sFunding->lStartDate[i] > sMarket->lToday + sMarket->iEODPayFlag)
            {
                sAux->dMarketValue[i] -=
                    sFunding->dNot[0] *
                    swp_f_df(sMarket->lToday, sFunding->lStartDate[i], sMarket->cFundYcName) *
                    sFunding->sAux->dCashFx;
                sAux->dMarketValue[i] +=
                    sExotic->dNot[0] *
                    swp_f_df(sMarket->lToday, sFunding->lStartDate[i], sMarket->cYcName);
            }
    ...
    for (i = iStartIndex; i <= sExotic->sAux->iEndIndex; i++)
    {
        ...
        case REGULAR_CRIF:
        {
            ...
            else
            {
                /* Get the schedule */
                err = add_tenor(
                    sExotic->lStartDate[i],
                    sExotic->cPaidTenor[i],
                ...
                sAux->dCVStrike[i] = DMAX(
                    0.0001,
                    sExotic->dPaidMargin[i] + sExotic->dPrvCpngearing[i] * dPreviousCouponValue);
            }

...
Err	crif_calc_fund_value(	CRIF_DEAL		sDeal,
...
Err	err = NULL;
	...
	{
		df_end = swp_f_df(sMarket->lToday, sFunding->lPayDate[i], sMarket->cFundYcName);

		if (sFunding->lFixDate[i] < sMarket->lToday + sMarket->iEODFixFlag)
		{			
			sAux->dMarketValue[i] = (sFunding->dFixCoupon[i] + sFunding->dMargin[i]) * sFunding->dCoverage[i] * df_end * sFunding->dNot[i] * sAux->dCashFx;
		}
		else
		{
			/* Libor calculation */
			df_start = swp_f_df(sMarket->lToday, sFunding->lStartDate[i], sMarket->cFundYcName);

			sAux->dMarketValue[i] = (df_start - df_end + (sFunding->dMargin[i] + sAux->dInitSpread[i]) * sFunding->dCoverage[i] * df_end) * sFunding->dNot[i] * sAux->dCashFx;
		}

		/* for foreign funding add initial and final notional in Funding and Exotic leg */
		if (sFunding->iCCY)
		{
			if (sFunding->lStartDate[i] > sMarket->lToday + sMarket->iEODPayFlag)
			{
				sAux->dMarketValue[i] -= sFunding->dNot[0] * swp_f_df(sMarket->lToday, sFunding->lStartDate[i], sMarket->cFundYcName) * sFunding->sAux->dCashFx;
				sAux->dMarketValue[i] += sExotic->dNot[0] * swp_f_df(sMarket->lToday, sFunding->lStartDate[i], sMarket->cYcName);
			}

			if (sFunding->lPayDate[i] > sMarket->lToday + sMarket->iEODPayFlag)
			{
				sAux->dMarketValue[i] += sFunding->dNot[0] * swp_f_df(sMarket->lToday, sFunding->lPayDate[i], sMarket->cFundYcName) * sFunding->sAux->dCashFx;
				sAux->dMarketValue[i] -= sExotic->dNot[0] * swp_f_df(sMarket->lToday, sFunding->lPayDate[i], sMarket->cYcName);
			}
		}
	}
...
Err	crif_calc_exotic_value(		CRIF_DEAL				sDeal,
								CMSCTS_MARKET				sMarket,
								...
								CMSCTS_CALIB				sCalibration)	
{
	...
	for (i=iStartIndex; i<=sExotic->sAux->iEndIndex; i++)
	{
		...
		{
			dPaidDf = swp_f_df(sMarket->lToday, sExotic->lPayDate[i], sMarket->cYcName);
		}
		else
		{
			dPaidDf = 1.0;
		...
		case REGULAR_CRIF:
			{
					...
					sAux->iFloatingType[i] == 0)
				{
					...
					if (sExotic->lPaidFixingDate[i] < sMarket->lToday + sMarket->iEODFixFlag)
					{
						sAux->dFixedPV += dCouponValue * sExotic->dCoverage[i]* dPaidDf * sExotic->dNot[i];;
					}
				}
				else
				{
					/* Get the schedule */
					err = add_tenor(sExotic->lStartDate [i], sExotic->cPaidTenor[i], NO_BUSDAY_CONVENTION, &lTheoEndDate);
					if (err) goto FREE_RETURN;
					
					err = swp_f_initSwapDP(	sExotic->lStartDate [i],
											lTheoEndDate,
											sExotic->cPaidFreq,
											sExotic->cPaidBasis,
											&sFixedSwap);
					...
					sAux->dCVStrike[i] =  DMAX( 0.0001 , sExotic->dPaidMargin[i] + sExotic->dPrvCpngearing[i] * dPreviousCouponValue) ;

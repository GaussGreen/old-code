...
static Err compute_diffusion_generators(Date			ex_date,
								 SrtUndPtr		und,
								 SrtUndPtr		dom_und,
...
static Err compute_black_scholes_greeks(double				fwd,
								 double				strike,
								 double				volatility,
								 double				time,
								 double				df,
...
static Err  make_greeks_allocations(SrtBasicPdeInf	  pde_inf,
									double			  ***gamma_matrix,
									double			  ***vega_matrix,
									double			  ***volga_matrix,
									double			  ***vanna_matrix)
{
	...
	if( ((*gamma_matrix) == NULL) || ((*vega_matrix) == NULL) || ( (*volga_matrix) == NULL) || ( (*vanna_matrix) == NULL) ) 
...
static Err  make_greeks_desallocations(SrtBasicPdeInf		pde_inf,
										double			   ***gamma_matrix,
										double			   ***vega_matrix,
										double			   ***volga_matrix,
										double			   ***vanna_matrix)
{
	if((*gamma_matrix)) free_dmatrix((*gamma_matrix),0,pde_inf.num_step,1,pde_inf.num_mesh); (*gamma_matrix) = NULL;
	if((*vega_matrix)) free_dmatrix((*vega_matrix),0,pde_inf.num_step,1,pde_inf.num_mesh); (*vega_matrix) = NULL;
	if((*vanna_matrix)) free_dmatrix((*vanna_matrix),0,pde_inf.num_step,1,pde_inf.num_mesh); (*vanna_matrix) = NULL;
	if((*volga_matrix)) free_dmatrix((*volga_matrix),0,pde_inf.num_step,1,pde_inf.num_mesh); (*volga_matrix) = NULL;
...
Err srt_f_set_srvgs_pde_operators(Date				ex_date,
								  String			dom_und_name,
								  String			und_name,
								  SrtBasicPdeInf	pde_inf,
								  ...
								  double			****div_tensor)
{
	...
	double				basevol,beta,gamma,omega,voldrift,vovol,
						correlation_und_vol,correlation_und_rate,time,stdev,tanh_stdev,skew,wing;
	...
	if (!corr_list) return serror("Cannot get the correlation structure");
	...
	err = make_greeks_allocations(pde_inf,
								  &gamma_matrix,
								  &vega_matrix,
								  &volga_matrix,
								  &vanna_matrix);
	...
	err =  compute_diffusion_generators(ex_date,
									    und,
										dom_und,
										pde_inf,
										diff_mat,
	...
	for(step_index = 0; step_index <= pde_inf.num_step;step_index ++)
	{
		...
		for(mesh_index = 1; mesh_index < pde_inf.num_mesh ; mesh_index++) /* loop on each mesh point */
		{
			/* compute the greeks */
			err = compute_black_scholes_greeks(pde_inf.spots[mesh_index]*(pde_inf.forward[0]/pde_inf.forward[step_index]),
											   strike,
											   basevol,
			...
			gamma_matrix[step_index][mesh_index] *= pow(pde_inf.forward[0]/pde_inf.forward[step_index],2);
			vega_matrix[step_index][mesh_index] *= 100;
			vanna_matrix[step_index][mesh_index] *= (pde_inf.forward[0]/pde_inf.forward[step_index]);
		
			/* cost of hedge in presence of stochastic volatility */
			(*div_tensor)[0][step_index][mesh_index] = - voldrift*vega_matrix[step_index][mesh_index] 
									 - gamma_matrix[step_index][mesh_index]*pde_inf.spots[mesh_index]*pde_inf.spots[mesh_index]*basevol*voldrift*time
									 - 0.5*vovol*vovol*volga_matrix[step_index][mesh_index]
									 - vovol*correlation_und_vol*basevol*pde_inf.spots[mesh_index]*vanna_matrix[step_index][mesh_index];

			(*div_tensor)[0][step_index][mesh_index] *= (pde_inf.df[step_index]/pde_inf.df[0]);

			/* cost of hedge in presence of smile  */
			(*div_tensor)[1][step_index][mesh_index] = - gamma_matrix[step_index][mesh_index]*pde_inf.spots[mesh_index]*pde_inf.spots[mesh_index]*basevol*basevol*(skew + wing);
	...
	err = make_greeks_desallocations(pde_inf,
									  &gamma_matrix,
									  &vega_matrix,
									  &volga_matrix,
									  &vanna_matrix);
...
Err compute_black_scholes_greeks(double				fwd,
								 double				strike,
								 double				volatility,
								 double				time_to_exp,
								 double				df,
...
Err compute_diffusion_generators(Date			ex_date,
								 SrtUndPtr		und,
								 SrtUndPtr		dom_und,
								 SrtBasicPdeInf	pde_inf,
								 double			****diff_mat,
								 double			****conv_mat)
{
	...
	for(step_index = 0; step_index <= pde_inf.num_step; step_index ++)
	{
		if(step_index == 0)  time = (ex_date-date_today)*YEARS_IN_DAY;
		else time = max(pde_inf.time_step,pde_inf.time[step_index-1]-pde_inf.time_step);

		pde_inf.time[step_index] = time;
		date = (Date)(date_today + time*DAYS_IN_YEAR);
		
		err = srt_f_eq_forward(date,
						   und,
						   yc_name,
						   &pde_inf.forward[step_index]);
		...
		err = srt_f_get_corr_from_CorrList( srt_f_GetTheCorrelationList(),
											und->underl_name, 
											dom_und->underl_name,
											time,
	...
	{	
		for(mesh_index = 1; mesh_index < pde_inf.num_mesh ; mesh_index++) /* loop on each mesh point */
		{
			...
			stdev = log(pde_inf.spots[mesh_index]/spot)/(pde_inf.basevol[step_index]*sqrt(time));
			tanh_stdev = tanh(stdev/pde_inf.omega[step_index]);
	
			skew = pde_inf.beta[step_index]*pde_inf.omega[step_index]*tanh_stdev;
			wing = pde_inf.gamma[step_index]*pde_inf.omega[step_index]*pde_inf.omega[step_index]*tanh_stdev*tanh_stdev;
			
			(*diff_mat)[0][step_index][mesh_index] = pde_inf.basevol[step_index];
			(*diff_mat)[1][step_index][mesh_index] = pde_inf.basevol[step_index];

			if(step_index == pde_inf.num_step) drift = log(pde_inf.forward[step_index]/spot)/pde_inf.time_step;
			else drift = log(pde_inf.forward[step_index]/pde_inf.forward[step_index+1])/pde_inf.time_step;

			(*conv_mat)[0][step_index][mesh_index] = drift-0.5*pde_inf.basevol[step_index]*pde_inf.basevol[step_index];
				...
				SrtBasicPdeInf	*pde_inf)
{
	...
	yc_name = get_discname_from_underlying(dom_und);
	if(!yc_name) return serror("can not find yield curve");
	...
	err = srt_f_eq_implied_vol(exp_mat,
								und->underl_name,
								&black_vol);
	if(err) return err;

	log_spot_min = log(forward) - 0.5*black_vol*black_vol*exp_mat - LIM_IN_STDEV*black_vol*exp_mat;
				    
	log_spot_max = log(forward) - 0.5*black_vol*black_vol*exp_mat + LIM_IN_STDEV*black_vol*exp_mat;

	/* Build the discretisation scheme */
	max_h = black_vol*black_vol/fabs(swp_f_zr(ex_date, ex_date + 14.0, yc_name) - 0.5*black_vol*black_vol);

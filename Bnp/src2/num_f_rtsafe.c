/* ==========================================================
   ...
   DESCRIPTION: Using a combination of Newton-Rhapson and bissection  ,
                                find the root of a function bracketed between x1
   and x2. The root  , returned as the function value rtsafe  , will be refined
   until its accuracy is known within + or - xacc. funcd is a user-supplied
   routine that returns both the function value and the first derivative of the
   function
...
SrtErr rtsafe(Err (*func)(double, double *, double *), double x1, double x2,
              double xacc, int num_iter, double *answer) {
  Err err = NULL;
  int j;
  ...
  else if (fabs(fl) < DBL_EPSILON) {
    *answer = x1;
    return NULL;
  } else if (fabs(fh) < DBL_EPSILON) {
    *answer = x2;
    return NULL;
  }
...
// rtsafe_with_par
//
// Modified version of rtsafe that passes a double * to the function being
// zeroed
//
/**************************************************************************************************************************************/

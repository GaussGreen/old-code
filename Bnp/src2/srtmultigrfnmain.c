...
static long Get_Index_FxUnd(SrtUndPtr* und_ptr, int num_und, char* domname, char* forname)
{
    static int   i;
    ...
    for (i = 0; i < num_und_temp; i++)
    {
        switch
            ...
            case LGM:
                ...
                (*type)[i] = 1;
                index      = Get_Index_FxUnd(
                    *und_ptr, num_und_temp, domestic_name, ((*und_ptr)[i])->underl_name);
                if (index < 0)
                {
                    /* we have to add this underlying */
                    num_und_temp += 1;
    ...
    double*** prod_val)
{
    ...
    double *evt_tms = NULL, *sig_dates = NULL, **und_sig_dates = NULL, **und_sig_curve = NULL,
           **sig_curves = NULL, pay_time;

    int *type = NULL, *dom_forex = NULL, *for_forex = NULL, *fx_index = NULL, dom_index;
        ...
        for (i = 0; i < num_und; i++)
        {
            ...
            for (j = i + 1; j < num_und; j++)
            {
                err = srt_f_get_corr_from_CorrList(
                    corr_list,
                    und_ptr[i]->underl_name,
                    und_ptr[j]->underl_name,
                    sig_dates[l],
                    &(und_link->correlations[i][j][l]));
                if (err)
                {
                    goto FREE_RETURN;
                }
    ...
    for (i = 0; i < num_und; i++)
    {
        ...
        {
            sig_curves[i][j] =
                und_sig_curve[i][Get_Index(sig_dates[j], und_sig_dates[i], nb_und_sig[i])];
        }
        if (und_sig_dates[i])
        {
    ...
    err = fill_mc_multi_init_corr(pay_date, pay_time, evt_dts, evt_tms, nstp, und_link);
    ...
    for (i = num_evt - 1; i >= 0; i--)
    {
        if (xStr.evt[i].evt)
        {
            ...
            if (!grfn_prm->num_df || !grfn_prm->df_tms || !grfn_prm->df_dts || !grfn_prm->dff ||
                !grfn_prm->gam || !grfn_prm->gam2 || !grfn_prm->do_und)
            {
                err = "Memory allocation failure";
                goto FREE_RETURN;
            }
                ...
                if (grfn_prm->num_df[j] > 0)
                {
                    ...
                    if (und_link->type[j] < 2)
                    {
                        for (k = 0; k < grfn_prm->num_df[j]; k++)
                        {
                            ...
                            grfn_prm->gam[j][k] =
                                ...
                                und_link->lambda[j];
                            grfn_prm->gam2[j][k] = 0.5 * grfn_prm->gam[j][k] * grfn_prm->gam[j][k] *
                    ...
                    else
                    {
                        ...
                        for (k = 0; k < grfn_prm->num_df[j]; k++)
                        {
                            ...
                            grfn_prm->gam2[j][k] = 0.5 * grfn_prm->gam[und_link->dom_forex[j]][k] *

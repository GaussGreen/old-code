    ...
    MBSDENSITY*  density)      // density function for prepay propensity
{
    ...
    if (!stream)
        return (nrerror("Can't open Term.dat")); /* Open the yield curve data file (see termodel.h
    ...
    fgets(string, 80, stream);
    fgets(string, 80, stream);
    // fscanf(stream, "%lf\n", &(deal_data->Refirate));                                /* today's
    // refinance rate */
    fgets(string, 80, stream);
    if (fscanf(stream, "%ld\n", &(term_data->TStree)) != 1)
        return (nrerror("Can't read term_data.TStree")); /* treasury tree or swap tree */
    fgets(string, 80, stream);
    if (fscanf(stream, "%ld\n", &(term_data->TSdiscount)) != 1)
        return (
            nrerror("Can't read term_data.TSdiscount")); /* discount using treasury or swap rates?*/
    fgets(string, 80, stream);
    for (i = 0; i < 6; i++)
        fgets(string, 80, stream);
    //        fscanf (stream, "%lf \n", &(term_data->MMYield[i]));
    ...
    {
        fgets(string, 80, stream);
        //        fscanf (stream, "%lf \n", &(term_data->SwapYield[i]));          /* Treasury yield
        //        for this maturity */ term_data->YieldS[i] = term_data->SwapYield[i];
    } /* end for */
    fgets(string, 80, stream);
    for (i = 0; i < 8; i++)
    {
        fgets(string, 80, stream);
        //   fscanf (stream, "%lf \n", &(spread));                           /* Swap spread for this
        //   maturity */ if (term_data->TSdiscount ==1)
        //                   term_data->YieldS[i] += spread/100.; /* if Swap discount, add spread */
        //   if (term_data->TStree == 1) term_data->SwapYield[i] += spread / 100.;  /* if Swap tree,
        //   add spread to Yield in tree building */
    } /* for i */
    ...
    if (!stream)
        return (nrerror("Can't open Deal.dat")); /* open the deal data file (see struct.h for a
    ...
    fgets(string, 80, stream);
    fgets(string, 80, stream);
    // fscanf(stream, "%c\n", &(deal_data->Scenario));                 /* for calculating duration,
    // we parallel shift yield curve. */
    fgets(string, 80, stream);
    if (fscanf(stream, "%lf\n", &(deal_data->Shift)) != 1)
    ...
    deal_data->Maturity  = Nxtmth(
        deal_data
            ->Exer[1], /* The final maturity is the maturity of the deal plus the index maturity */
    ...
    fgets(string, 80, stream);
    // fscanf (stream, "%d \n", &(deal_data->Term));
    tree_data->NodeMax = (int)sqrt(20. * (double)tree_data->Ppy) *
    ...
    fgets(string, 80, stream);
    if (fscanf(stream, "%c\n", &(deal_data->Ppform)) != 1)
        return (nrerror("Can't read deal_data.Ppform")); /* ratio or absolute shift ppf */
    fgets(string, 80, stream);
    if (fscanf(stream, "%c\n", &(deal_data->AlorCPR)) != 1)
        return (nrerror("Can't read deal_data.AlorCPR")); /* avg life or annual CPR */
    ...
    { /* if CPR, there are MAXNUMP columns */
        for (i = 0; i < NBPOINTS; i++)
        {
    ...
    if (deal_data->Ppform == 'R')
    {                                   // if ratio, we change from refi/gwac ratio
        for (i = 0; i < NBPOINTS; i++)  // to shift in 10yr yield
            deal_data->IndexLevel[i] = transformIncentive(
                deal_data->IndexLevel[i],
                deal_data->Gwac,
                deal_data->Refirate,
    ...
    if (fscanf(stream, "%c\n", &(deal_data->Schedule)) != 1)
        return (nrerror("Can't read deal_data.Schedule")); /* scheduled amortization or not */
    fgets(string, 80, stream);
    if (fscanf(stream, "%c\n", &(deal_data->Balloon)) != 1)
    ...
    if (fscanf(stream, "%c\n", &(deal_data->Always)) != 1)
        return (nrerror("Can't read deal_data.BalloonSch"));
    if (deal_data->Always == 'N' && ((deal_data->Exer[1] - term_data->Today) / 10000 > 15.))
    ...
    prepay_data->Spot =
        0.; /* This is the one factor version of the model: we don't use the second dimension */
    prepay_data->SpotVol = 0.;
    prepay_data->Beta    = 0.;

    prepay_data->NbFwd =
        1; /* We don't use any forward curve for the prepayment so we put bogus values here */
    ...
    for (i = 0; i < NUMRAMPS; i++)
    {
        ...
        deal_data->SeasoningLevel[i] = transformIncentive(
            deal_data->SeasoningLevel[i],
            deal_data->Gwac,
            deal_data->Refirate,
            term_data->SwapYield[5]);
    ...
    //	if( err = get_initial_principal_payments( deal_data->NbFixedPrepay, deal_data->FixedPrepay,
    //wam, deal_data->Gwac, deal_data->NbFixedPrepay, deal_data->InitialSMM )) return(err); #endif
    ...
    term_data->ValueDate = Nxtbusday(
        term_data->Today, /* Value date is today shifted by (Spotdays) business days */
        term_data->Spotdays,
        term_data->Holidays,
    ...
    do
        Date = Nxtmth(
            Date, /* Calculate the coupon date falling before today by going backward from */
            (long)(-12 / F), /* maturity one coupon at a time. */
            (long)1);
    while (Date > term_data->ValueDate);

    i = max(0, deal_data->NbFixedPrepay - deal_data->Delay) +
        1; /* Calculate the lockout date, that is the date on which we do the last    */
    deal_data->Exer[0] = Nxtmth(
        Date, /* reading of the index. It is located NbFixedPrepay - Delay + 1 coupons   */
    ...
    MBSDENSITY*  density)      // density function for prepay propensity
{
    ...
    if ((deal_data->Freq != 'A') && (deal_data->Freq != 'S') && (deal_data->Freq != 'Q') &&
        (deal_data->Freq != 'M'))
        return (
            nrerror("Specify a frequency for the underlying cash flows: 'A', 'S', 'Q' or 'M'!"));
    ...
    if (deal_data->Hedge != 'Y' && deal_data->Hedge != 'N' && deal_data->Hedge != 'I' &&
        deal_data->Hedge != 'P')

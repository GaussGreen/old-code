    ...
    double** res)
{
    ...
    for (i = 0; i < nb_paths; i++)
    {
        ...
        for (j = 1; stop_path == 0 && j < nb_dates; j++)
        {
            ...
            Z += fx_fwd[j] - for_gam1_fwd[j - 1] * R1F - for_gam2_fwd[j - 1] * R2F +
                 dom_gam1_fwd[j - 1] * R1D + dom_gam2_fwd[j - 1] * R2D + matrixi[m + 4];
    ...
    if (do_optimisation)
    {
        ...
        time2 = clock();
        smessage(
            "Phase 5 -optimisation, time in sec: %.2f", (double)(time2 - time1) / CLOCKS_PER_SEC);
    }
    ...
    for (i = 0; i < nb_paths; i++)
    {
        ...
        for (j = 1; stop_path == 0 && j < nb_dates; j++)
        {
            ...
            R2F += (for_phi2[j - 1] + for_phi12[j - 1] - for_lam2 * R2F) * dt -
                   correl[3][4] * for_alpha * for_std[j] * fx_std[j] + matrix[i][l + 3];

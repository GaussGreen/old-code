...
Err mc_main_5dfx(
  ...
  for (i = 0; i < nb_paths; i++) {
    ...
    if (func_parm_tab[0]) {
      ...
      err = payoff_func(date[0], time[0], func_parm_tab[0], spot_fx, R1D, R2D,
                        R1F, R2F, exp(Z), nb_col, res_evt, &stop_path);
    ...
    for (j = 1; stop_path == 0 && j < nb_dates; j++) {
      ...
      Z += fx_fwd[j] - for_gam1_fwd[j - 1] * R1F - for_gam2_fwd[j - 1] * R2F +
      ...
      df =
          dom_bond_pay[j] * exp(-dom_gam1_pay[j] * R1D - dom_gam2_pay[j] * R2D);

      err = payoff_func(date[j], time[j], func_parm_tab[j], spot_fx, R1D, R2D,
                        R1F, R2F, exp(Z), nb_col, res_evt, &stop_path);
...
Err mc_main_5dfx_test(
  ...
  double **matrix = NULL, **covar = NULL, **chol = NULL;
  ...
  for (j = 1; j < nb_dates; j++) {
    ...
    covar[0][1] = covar[1][0] =
        correl[0][1] * dom_std[j] * dom_alpha * dom_std[j];
    covar[0][2] = covar[2][0] = correl[0][2] * dom_std[j] * for_std[j];
    covar[0][3] = covar[3][0] =
        correl[0][3] * dom_std[j] * for_alpha * for_std[j];
    covar[0][4] = covar[4][0] = correl[0][4] * dom_std[j] * fx_std[j];
    covar[1][1] = dom_alpha * dom_std[j] * dom_alpha * dom_std[j];
    covar[1][2] = covar[2][1] =
        correl[1][2] * dom_alpha * dom_std[j] * for_std[j];
    covar[1][3] = covar[3][1] =
        correl[1][3] * dom_alpha * dom_std[j] * for_alpha * for_std[j];
    covar[1][4] = covar[4][1] =
        correl[1][4] * dom_alpha * dom_std[j] * fx_std[j];
    covar[2][2] = for_std[j] * for_std[j];
    covar[2][3] = covar[3][2] =
        correl[2][3] * for_std[j] * for_alpha * for_std[j];
  ...
  for (i = 0; i < nb_paths; i++) {
    ...
    for (j = 1; stop_path == 0 && j < nb_dates; j++) {
      ...
      R1D += (dom_phi1[j - 1] + dom_phi12[j - 1] - dom_lam * R1D) * dt +
             matrix[i][l];
      R2D += (dom_phi2[j - 1] + dom_phi12[j - 1] - dom_lam2 * R2D) * dt +
             matrix[i][l + 1];

      R1F += (for_phi1[j - 1] + for_phi12[j - 1] - for_lam * R1F) * dt -
             correl[2][4] * for_std[j] * fx_std[j] + matrix[i][l + 2];
      ...
      if (func_parm_tab[j]) {
        err = payoff_func(date[j], t2, func_parm_tab[j], spot_fx, R1D, R2D, R1F,
                          R2F, exp(Z), nb_col, res_evt, &stop_path);

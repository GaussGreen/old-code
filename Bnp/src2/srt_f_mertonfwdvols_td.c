    ...
    long*   ia)
{
    ...
    err = optcalibmertontimedep(
        dFra, NMAXSTRIKES, nperiods, strikes, vols, param, mat, "Lognormal", &chisq, ia, calibvols);
...
Err get_DRStsfwdvols_td(
    long today, double smiledate, char* cRefRateCode, char* szYieldCurveName, double* DRSts)
{
    Err            err = NULL;
    long           FraSpot;
    ...
    double* DRSts)
{
    ...
    for (j = 1; j <= index + 1; j++)
    {
        *DRSts *= exp(-param[5 * (j - 1) + 1] * param[5 * (j - 1) + 1] * periodmat[j] / 2 +
...

{
    ...
    err = get_DRStsfwdvols_td(today, smiledate, cRefRateCode, szYieldCurveName, &DRStsStart);
    ...
    /* get parameters corresponding to the smiledate , with an interpolation for the first part of
     * the diffusion*/

    paramsmiledate[1] =
        paramatrix[intindex][1] + (smiledate - resetdates[intindex]) *
                                      (paramatrix[intindex + 1][1] - paramatrix[intindex][1]) /
                                      (resetdates[intindex + 1] - resetdates[intindex]);
    paramsmiledate[2] =
        paramatrix[intindex][2] + (smiledate - resetdates[intindex]) *
                                      (paramatrix[intindex + 1][2] - paramatrix[intindex][2]) /
                                      (resetdates[intindex + 1] - resetdates[intindex]);
    paramsmiledate[3] = paramatrix[intindex][3];
    paramsmiledate[4] =
        paramatrix[intindex][4] + (smiledate - resetdates[intindex]) *
    ...
    FREE_TD(
        n_resetdates, intindex, paramatrix, brownian, jumps1, jumps2, paramsmiledate, periodmat);

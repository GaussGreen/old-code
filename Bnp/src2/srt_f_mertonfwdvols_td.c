...
Err FREE_TD(int n_resetdates,int intindex, double **paramatrix,double *brownian,double *jumps1, double *jumps2, double *paramsmiledate, double *periodmat)
{
	if (paramatrix) free_dmatrix(paramatrix,1,n_resetdates, 1,5*n_resetdates);
	paramatrix = NULL;

	if (brownian) free_dvector(brownian, 1,n_resetdates);
	brownian = NULL;

	if (jumps1) free_dvector(jumps1,1,n_resetdates);
	jumps1 = NULL;

	if (jumps2) free_dvector(jumps2,1,n_resetdates);
	jumps2 = NULL;

	if (paramsmiledate) free_dvector(paramsmiledate, 1, 5*(intindex+1));
	paramsmiledate = NULL;

	if (periodmat) free_dvector(periodmat, 1,n_resetdates);
	periodmat = NULL;
...
Err calibtosmilefwdvols_td(  long today,
				   long dDate,
				   char *cMarketId,
				   char *szVolCurveName,
				   char *cRefRateCode,
...
err = optcalibmertontimedep(dFra,NMAXSTRIKES,nperiods,strikes,vols,param,mat,"Lognormal",&chisq,ia,calibvols);
...
Err resetablecalibfwdvols_td(long today, int n_resetdates, 
				   double *resetdates,
				   char *cMarketId,
				   char *szVolCurveName,
				   char *cRefRateCode,
...
Err get_DRStsfwdvols_td(long today,  double smiledate, 
...
/* compute the fra */
		...
		err = get_fra((long) smiledate, spotlag, cRefRateCode, szYieldCurveName, &dFra);
...
Err DRSCalcfwdvols_td(	double *brownian,
						double *jumps1,
						double *jumps2,
						int     index,
						double *periodmat,
...
for(j=1;j<=index+1;j++)
{
	...
	*DRSts *= exp(-param[5*(j-1)+1]*param[5*(j-1)+1]*periodmat[j]/2+param[5*(j-1)+1]*brownian[j]*sqrt(periodmat[j]))*
			exp(jumps1[j]*log(1+param[5*(j-1)+2])-param[5*(j-1)+3]*param[5*(j-1)+2]*periodmat[j])*
			exp(jumps2[j]*log(1+param[5*(j-1)+4])-param[5*(j-1)+5]*param[5*(j-1)+4]*periodmat[j]);
...
err = resetablecalibfwdvols_td(today,n_resetdates, 
						resetdates,
						cMarketId,
						szVolCurveName,
						cRefRateCode,
...
if (err)
{
FREE_TD(n_resetdates,intindex,paramatrix,brownian,jumps1, jumps2, paramsmiledate, periodmat);
...
/* get parameters corresponding to the smiledate , with an interpolation for the first part of the diffusion*/

		paramsmiledate[1] = paramatrix[intindex][1]+(smiledate - resetdates[intindex])*(paramatrix[intindex+1][1]-paramatrix[intindex][1])/(resetdates[intindex+1]-resetdates[intindex]);
		paramsmiledate[2] = paramatrix[intindex][2]+(smiledate - resetdates[intindex])*(paramatrix[intindex+1][2]-paramatrix[intindex][2])/(resetdates[intindex+1]-resetdates[intindex]);
		paramsmiledate[3] = paramatrix[intindex][3];
		paramsmiledate[4] = paramatrix[intindex][4]+(smiledate - resetdates[intindex])*(paramatrix[intindex+1][4]-paramatrix[intindex][4])/(resetdates[intindex+1]-resetdates[intindex]);
		...
		for (j=1;j<=5;j++)
		{
			paramleftout[j] = paramsmiledate[5*(intindex)+j];
				...
				err = DRSCalcfwdvols_td(brownian,jumps1,jumps2,intindex,periodmat,paramsmiledate,&DRSts);
				...
				if (err)
				{
					...
					optmertonsmile(DRSts,n_strikes, strikes, paramleftout, periodmat[1], "Lognormal",impvols);

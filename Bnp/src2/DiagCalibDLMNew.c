...
void free_LGM_model(LGM_MODEL model)
{
	...
	if (model->dSigma) free(model->dSigma);
	if (model->dLambda) free(model->dLambda);
	if (model->dLambdaTime) free(model->dLambdaTime);
	if (model->dInitLambda) free(model->dInitLambda);
	if (model->dNewLambda) free(model->dNewLambda);
	if (model->dNewLambda2) free(model->dNewLambda2);
}
						   ...
						   LGM_MODELFACTORS			sModelFactors)
{
	...
	if (!sModelFactors->dCpn_G1 || !sModelFactors->dExpFact1  || !sModelFactors->dEx_G1 || !sModelFactors->dZeta1)
	{
	...
	if (model->iNbFactor >= 2)
	{
		...
		sModelFactors->dExpFact12 = calloc(sCalibExeSchedule->iNExe, sizeof(double));
		...
		if (!sModelFactors->dCpn_G2 || !sModelFactors->dExpFact2  || !sModelFactors->dExpFact12  || !sModelFactors->dEx_G2 || !sModelFactors->dZeta2 || !sModelFactors->dZeta12)
		{
			return "Memory allocation faillure in Initialise_Factors_LGM";
		}
	}
...
{
	if (sModelFactors->dCpn_G1) free(sModelFactors->dCpn_G1);
	if (sModelFactors->dEx_G1) free(sModelFactors->dEx_G1);
	if (sModelFactors->dExpFact1) free(sModelFactors->dExpFact1);
	if (sModelFactors->dZeta1) free(sModelFactors->dZeta1);
	if (sModelFactors->dCpn_G2) free(sModelFactors->dCpn_G2);
	if (sModelFactors->dEx_G2) free(sModelFactors->dEx_G2);
	if (sModelFactors->dExpFact2) free(sModelFactors->dExpFact2);
	if (sModelFactors->dExpFact12) free(sModelFactors->dExpFact12);
	if (sModelFactors->dZeta2) free(sModelFactors->dZeta2);
	if (sModelFactors->dZeta12) free(sModelFactors->dZeta12);
}

Err Initialise_AllParams_LGM(	LGM_MODEL				model,
...
Err	err = NULL;
	...
	if (!AllParams->CalibParams || !AllParams->CalibParamsLambda || !AllParams->sLongFactors || !AllParams->sShortFactors )
	{
	...
	err = Initialise_Factors_LGM(	CalibLongCpnSchedule,
									AllCalibLongInst->sCalibExeSchedule,
									model,
...
double	t1, t2;
	...
	static_lgmsetupG_tauts(	model->iNbLam,
							model->dLambdaTime,
							model->dNewLambda,
							sSchedule->iNCpn,
							sSchedule->dCpnTime,
	...
	for (i=0; i<sCalibExeSchedule->iNExe; i++)
	{
		...
		sModelFactors->dExpFact1[i] = export_lgmcalcexpfact_tauts(	t1,
																	t2,
																	model->iNbLam,
																	model->dLambdaTime,
																	model->dLambda,
		...
		if (model->iNbFactor == 2)
		{
			sModelFactors->dExpFact2[i] = export_lgmcalcexpfact_tauts(	t1,
																		t2,
																		model->iNbLam,
																		model->dLambdaTime,
																		model->dLambda,
																		model->dGamma) / sModelFactors->dExpFact1[i];

			sModelFactors->dExpFact12[i] = export_lgmcalcexpfact_tauts(	t1,
																		t2,
																		model->iNbLam,
																		model->dLambdaTime,
																		model->dLambda,
																		0.5 * model->dGamma) / sModelFactors->dExpFact1[i];
		}
	}
}
...
clock_t			time1, time2;
	...
	*sig = NULL;
	...
	if (!CalibCpnLongSchedule || !CalibCpnShortSchedule || !CalibExeLongSchedule || !CalibExeShortSchedule ||
		!AllCalibLongInst || !AllCalibShortInst || !AllParams || !AllCalibShortInstOne || !model)
	{
	...
	if (!model->dPWTime || !model->dLambdaTime || !model->dInitLambda || !model->dNewLambda || !model->dNewLambda2)
	{
	...
	memcpy(model->dPWTime, CalibExeLongSchedule->dExeTimes, CalibExeLongSchedule->iNExe * sizeof(double));
	...
	if (!fix_lambda)
	{
		...
		num_f_concat_vector(&model->iNbPWTime, &model->dPWTime, CalibExeShortSchedule->iNExe, CalibExeShortSchedule->dExeTimes);
	}
	...
	for (i=0; i<CalibExeLongSchedule->iNExe; i++)
	{
		lSigLongIndex[i] = Get_Index(CalibExeLongSchedule->dExeTimes[i], model->dPWTime, model->iNbPWTime);
	}
	...
	if (!fix_lambda)
	{
		for (i=0; i<CalibExeShortSchedule->iNExe; i++)
		{
			lSigShortIndex[i] = Get_Index(CalibExeShortSchedule->dExeTimes[i], model->dPWTime, model->iNbPWTime);
		}
	}
	...
	if (params && fabs(params->lambda_shift) > 1.0E-08)
	{
		...
		if (fix_lambda)
		{
			...
			for (i=0; i<model->iNbLam; i++)
			{
				...
				if (model->iNbFactor == 2) model->dNewLambda2[i] += params->lambda_shift;
			}
	...
	err = Initialise_AllParams_LGM(	model,
									lSigLongIndex,
									CalibCpnLongSchedule,
									AllCalibLongInst,
									paraml,								
...
Err	GetTargetVol_LGM(	void				*Inst_,
						void				*GlobalParam,
						void				*Model,
...
double			last_zeta1, last_zeta2, last_zeta12, var;
	...
	AllParams->sLongFactors->dZeta1[AllParams->index_vol] = last_zeta1 + var * AllParams->sLongFactors->dExpFact1[AllParams->index_vol];

	if (model->iNbFactor == 2)
	{
		AllParams->sLongFactors->dZeta2[AllParams->index_vol] = last_zeta2 + var * AllParams->sLongFactors->dExpFact2[AllParams->index_vol];
		AllParams->sLongFactors->dZeta12[AllParams->index_vol] = last_zeta12 + var * AllParams->sLongFactors->dExpFact12[AllParams->index_vol];
	}
...
CALIBINSTRUMENTDLM Inst = (CALIBINSTRUMENTDLM) Inst_;
	...
	{
		*InstPrice = lgmopval1F(Inst->iNbCoupon,
								Inst->dCpnValues,
								&AllParams->sLongFactors->dCpn_G1[Inst->iStartCpn],
								AllParams->sLongFactors->dZeta1[AllParams->index_vol],
								AllParams->sLongFactors->dEx_G1[AllParams->index_vol]) * Inst->dWeight;
	}		
	else
	{
		*InstPrice = lgmopval2F(Inst->iNbCoupon,
								Inst->dCpnValues,
								&AllParams->sLongFactors->dCpn_G1[Inst->iStartCpn],
								...
								AllParams->sLongFactors->dEx_G2[AllParams->index_vol]) * Inst->dWeight;
	}
...
int					i;
	...
	{	
		if (model->iNbFactor == 1)
		{
			total += lgmopval1F(Inst->iNbCoupon,
								Inst->dCpnValues,
								&AllParams->sShortFactors->dCpn_G1[Inst->iStartCpn],
								AllParams->sShortFactors->dZeta1[i],
								AllParams->sShortFactors->dEx_G1[i]) * Inst->dWeight;
		}		
		else
		{
			total += lgmopval2F(Inst->iNbCoupon,
								Inst->dCpnValues,
								&AllParams->sShortFactors->dCpn_G1[Inst->iStartCpn],
								...
								AllParams->sShortFactors->dEx_G2[i]) * Inst->dWeight;
		}
...
CALIBINSTRUMENTDLM	Inst = (CALIBINSTRUMENTDLM) Inst_;
	...
	if (CalibConsts->do_calib)
	{
		...
		if (model->iNbFactor == 2)
		{
			static_lgmcalczeta2zeta12_tauts(AllParams->CalibExeShortSchedule->iNExe,
											AllParams->CalibExeShortSchedule->dExeTimes,
											AllParams->sShortFactors->dZeta1,
											model->iNbLam,
											model->dLambdaTime,

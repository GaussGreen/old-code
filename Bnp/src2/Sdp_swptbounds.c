...
Err build_instrument_with_cvgs(double *vecs, double **matres, int dim,
                               double *coverages) {
  double **matbuf = dmatrix(1, dim, 1, dim);
  double *vecbuf = dvector(1, dim);
  int i, j;
...
int is_bigger(double *caplet_a, double *caplet_b) {
  ...
  if (caplet_a[1] >= caplet_b[1])
    res = 0;
  if (caplet_a[2] >= caplet_b[2])
    res = 0;
  return res;
}
...
Err fudge_var(double *caplet_before, double *caplet_after,
              double *caplet_to_fudge) {
  double bufa = 0, bufb = 0;
  Err err = NULL;

  bufa = (caplet_after[1] - caplet_before[1]) /
         (caplet_after[2] - caplet_before[2]);
  if (bufa <= 0)
    err = serror("Smoothing impossible ...");
  if (err == NULL) {
    bufb = caplet_before[1];
    caplet_to_fudge[1] =
        (bufb + (caplet_to_fudge[2] - caplet_before[2]) * bufa);
  }
  return err;
}
...
/*  It tests for the presence of non-increasing sigma^2 * T and fudges the input
 * variance */
/*  in the cases where some Caplets are out of range. These ones are adapted so
 * that sigma^2 * T  */
/*  is closest to linear. */
/*  Succeeds if only isolated caplets are out of range  , fails if series of
 * caplets are out of range. */
Err smooth_caplets(double ***const_mat, double *const_val, int dim,
                   int num_const, int print_level) {
  char buffer[512];
  Err errbuf = NULL, err = NULL;
  int i, j, k = 0;
  int pos_before = 0, pos_after = num_const, buf_pos_before = 0,
      buf_pos_after = 0, is_last = 0;
...
/*  For DEBUG */
  ...
  for (i = 1; i <= num_const; i++) {
    ...
    if (vecpos[i][2] != 0)
      vecpos[i][1] = vecpos[i][2] * const_val[i];
  }
/*  ***** For DEBUG ******** */
#ifdef _DEBUG
  for (k = 1; k <= num_const; k++)
    vec_debug[k] = vecpos[k][1];
  ...
  for (i = 1; i <= num_const; i++) {
    pos_before = 0;
    pos_after = num_const + 1;
    /*  Then test with all shorter caplets */
    for (j = 1; j <= num_const; j++)
      if (vecpos[j][2] < vecpos[i][2]) {
        if (!(is_bigger(vecpos[j], vecpos[i])) && (j > pos_before))
          pos_before = j;
      }
    /*  If the caplet failed look for the nearest longer one. */
    if (pos_before != 0) {
      for (j = 1; j <= num_const; j++)
        if ((vecpos[i][2] < vecpos[j][2]) && (j < pos_after))
          pos_after = j;
      ...
      if (pos_after == num_const + 1) {
        ...
        pos_before = 0;
        for (j = 1; j <= num_const; j++) {
          if ((vecpos[j][2] < vecpos[pos_after][2]) &&
              (vecpos[j][2] > vecpos[pos_before][2]))
            pos_before = j;
        }
      }
      ...
      /*  than that of the Caplet before) then try to lower the caplet vol.
       * before. */
      if (errbuf != NULL) {
        /*  Grab another caplet immediately before (as above) */
        buf_pos_after = pos_after;
        pos_after = i;
        buf_pos_before = pos_before;
        pos_before = 0;
        for (j = 1; j <= num_const; j++) {
          if (vecpos[j][2] < vecpos[buf_pos_before][2] &&
              (vecpos[j][2] > vecpos[pos_before][2]))
            pos_before = j;
        }
        /*  if it has found one  , try to fudge... */
        if (vecpos[pos_before][2] != 0) {
          errbuf = fudge_var(vecpos[pos_before], vecpos[pos_after],
                             vecpos[buf_pos_before]);
          fudge_point = buf_pos_before;
          if (errbuf != NULL)
            err = errbuf;
        } else {
          /*  If not  , Interpolate backward. */
          errbuf = fudge_var(vecpos[i], vecpos[buf_pos_after],
                             vecpos[buf_pos_before]);
          fudge_point = buf_pos_before;
          if (errbuf != NULL)
            err = errbuf;
  ...
  for (i = 1; i <= num_const; i++)
    if (vecpos[i][2] != 0)
      const_val[i] = (vecpos[i][1]) / vecpos[i][2];
  if (print_level >= 1)
    smessage("\n");
/*  ***** For DEBUG ******** */
#ifdef _DEBUG
...
/*  It gets the instuments as one line per instrument (In puts have to be
 * properly transposed) */

Err compute_bounds(double **calib_inst, double *market_vars, int ninst,
                   double *vec_swpt, int dim, int minmax, double *result,
                   int *return_error, double *error, int printlevel,
  ...
  if (minmax == 0)
    minusma(dirmat, dirmat, dim);
  for (k = 1; k <= ninst; k++)
    build_instrument(calib_inst[k], calib_cube[k], dim);
  /*  Fudge Caplet vols. */
  ...
  if (err != NULL) {
    ...
    *return_error = 0;
    ...
    err = srt_sdp(calib_cube, market_vars, dirmat, dim, ninst, matbuf, result,
                  error, return_error, toler, printlevel, niter);
  }
  /*  Switch min-max. */
  if (minmax == 0)
    *result = (-(*result));

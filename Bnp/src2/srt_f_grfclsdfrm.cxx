/* -------------------------------------------------------------------------
   ...
   FILE NAME	: srt_f_grfclsdfrm.cxx

   PURPOSE		: functions to price swaptions        , caps or
   bondoptions without the Grfn interface. THese functions take simplified
...
/* ---------------------------------------------------------------------------
   ...
   DESCRIPTION	:   Prices a swaption        , a cap or a bond option using Grfn
                    If LGM can be used        , a switch send the calculation to
   a real closed form
   ---------------------------------------------------------------------------
 */
...
/* ---------------------------------------------------------------------------
   ...
   DESCRIPTION	:   Given a SwapDP (swaps date parameters)        , a strike and
   a type        , generate the GrfnCells and EventDates corresponding to a cap
   , a swaption or a bond option.
   ---------------------------------------------------------------------------
 */
...
Err grfn_SwapDP_to_GrfnCells(long *numeventdatesptr, Date **eventdatesptr,
  ...
  switch (type) {
    /* Need to calculate correct accrued interest for bond option        , which
       is then subtracted from the bond strike */
  case BOND_OPTION:
  ...
  case SIMPLEMIDAT:
    ...
    for (i = 0; i < nrows - 1; ++i) {
      if (rec_pay == SRT_PAYER) {
        sprintf(sprdsht[i][2].sval,
                "max(c[0        ,i] - c[1        ,i]        , PV[2])");
      }

      else {
        sprintf(sprdsht[i][2].sval,
                "max(c[1        ,i] - c[0        ,i]        , PV[2])");
      }
    }
    {
      //// last payoff
      if (rec_pay == SRT_PAYER) {
        sprintf(sprdsht[nrows - 1][2].sval,
                "max(c[0        ,i] - c[1        ,i]        , 0)");
      }
...
Err grfn_make_swaption_string(String swptn_str, SwapDP *sdp, double strike,
  ...
  if (rec_pay == SRT_PAYER) {
    sprintf(swptn_str,
            /* "max(df(now        ,%d)-df(now        ,%d)-%.10lf*lvl(%d ,%d
             * ,\"%s\"
             *       ,\"%s\")        ,0.0)"        ,
             */
            "max(swap(%d        ,%d        ,\"%s\"        ,\"%s\"        "
...
static void grfn_make_bondoption_string(String bndopt_str, SwapDP *sdp,
  ...
  if (acc_int == 0.0) {
    if (rec_pay == SRT_PAYER) {
      sprintf(bndopt_str,
              "max(%.10lf*df(now        ,%d)-df(now        ,%d)-%.10lf*lvl(%d  "
              "      ,%d  "
              "      ,\"%s\"        ,\"%s\")        ,0.0)",
...

{
  ...
  /* first caplet */
  sprintf(sprdsht[0][0].sval, "fra(%d        ,%d        ,\"%s\")", dfind[0],
          dfind[1], basis_str);
  sprdsht[0][0].type = GRFNSCELL;
  ...
  /* next caplets */
  for (i = 1; i < nrows; i++) {
    sprintf(sprdsht[i][0].sval, "fra(%d        ,%d        ,\"%s\")", dfind[i],
            dfind[i + 1], basis_str);
    sprdsht[i][0].type = GRFNSCELL;
    switch (rec_pay) {
    case SRT_PAYER:
      sprintf(sprdsht[i][1].sval,
              "max(c[0        ,i]-c[0        ,i-1]        ,0)*df(d[i]        "
              ",%d)*%g",
              dfind[i], cvg[i]);
      break;
    case SRT_RECEIVER:
...
Err grfn_SwapDP_to_BDT_GrfnCells(long *numeventdatesptr, Date **eventdatesptr,
  ...
  case BOND_OPTION:
    ...
    /* dfind[0] correponds to sdp->start        , dfind[num_pay_dates-1] to
     * sdp->end
     */
    swp_f_make_FixedLegDatesAndCoverages(sdp, today, &pay_dates, &num_pay_dates,
                                         &start_dates, &end_dates, &cvg,
...
static void grfn_mk_bdt_bnd_opt_sht(GrfnCell ***sprdshtptr, SwapDP *sdp,
  ...
  if (i_start) {
    ...
    if (rec_pay == SRT_PAYER) {
      sprintf(sprdsht[0][0].sval, "max( PV[0] - %.10lf         , 0.0 )",
              cash_flow);
    } else if (rec_pay == SRT_RECEIVER) {
      sprintf(sprdsht[0][0].sval, "max( %.10lf - PV[0]         , 0.0 )",
              cash_flow);
    }
    sprdsht[0][0].type = GRFNSCELL;
  }
...
static void grfn_mk_bdt_cap_flr_sht(GrfnCell ***sprdshtptr, SwapDP *sdp,
  ...
  for (j = 0; j < numcaplets; j++) {
    if (istart == 0) {
      sprintf(sprdsht[row][col].sval,
              "max( PV[0] + c[%d        , i]        , 0.0 )", j);
    } else if (istart == 1) {
...
/* ---------------------------------------------------------------------------
   ...
   DESCRIPTION	:   Given a SwapDP (swaps date parameters)        , a strike and
   a type        , generate the GrfnCells and EventDates corresponding to a cap
   , a swaption or a bond option - modification to include the cap.
   ---------------------------------------------------------------------------
 */

...
{
    return (Zero_Data_Construct(
        new_data,
        old_data->Frequency,
        old_data->YearBase,
        old_data->NbTimePoints,
        old_data->dates,
        old_data->DayPoints,
        old_data->Rates,
        old_data->Ref_Date));
...
char* MBSDF_Data_Copy(MBSDF_Data* new_struct, MBSDF_Data* old_struct)
{
    ...
    {
        if (err = MBSDF_Data_Construct_FromZeroData(new_struct, &(old_struct->zero_data)))
            return (err);
    }
    else
    {
    ...
    *nb = (term * yield_data->Frequency) / factor;
    ...
    for (i = 0; i < *nb; ++i)
    {
        ...
        if (yield_data->YearBase == _YACT)
            ...
            (*cvgs)[i] = ((double)DateDiff((*starts)[i], (*ends)[i], yield_data->diffBase)) /
...
char* MMYields_To_ZeroRates(Zero_Data* zero_data, MM_Data* mm_data, int frequency, YBASIS YearBase)
{
    ...
    df0 = 1.0 / (1.0 + mm_data->MMYields[0] / 100.0 *
                           ((double)DateDiff(zero_data->Ref_Date, start, mm_data->diffBase)) /
...
double zero_rate_from_t(Zero_Data* zero_data, double t, int extrapolate)
{
    return (linear_interp(
        zero_data->NbTimePoints, zero_data->TimePoints, zero_data->Rates, t, extrapolate));
}  // zero_rate_from_t

double zero_rate(Zero_Data* zero_data, long date, int extrapolate)
{
    return (zero_rate_from_t(
        zero_data,
...
// starting at Ref_Date + Delay + yield_data->yield_terms[0] (months), at intervals of
// interp_interval, and ending at the last yield_term if yeilds are provided, just use it, else, use
// linterp assume that last_date - 0-th date is a multiple of interp_interval
...
// k = index of zrs (zero rate) and day_points (number of days from yield_data->Ref_Date) to be
// filled in annuity, as input, is the value of an annuity at yield_data->Ref_Date, with frequency
// etc., as encoded in yield_data, 					with last payment on day_points[k-1] (from Ref_Date) yield = YTM
// with maturity day_points[k] = Ref_Date + period * 12/frequency (months) zrs[k] = the implied
// zero-rate annuity = value of the annuity with last payment on day_points[k] assume k >= 1

void MBS_ZR_BootStrap(
    int         k,
    int         period,
    double*     zrs,
    double*     annuity,
    ...
    Yield_Data* yield_data)
{
    ...
    // update day_points
    yield_date_gen(
        &nb, (&starts), (&ends), (&cvgs), yield_data, MONTH, period * 12 / yield_data->Frequency);
...
// also assume that mm_data has values at terms equal to all the coupon dates of a yield of 1Y
// maturity interp_freq is the frequency of interpolation in zero-rate-space
char* MBS_Strip(
    Zero_Data*  zero_data,
    MM_Data*    mm_data,
    Yield_Data* yield_data,
...
// offset is used because we woudl like to switch between the (wrong) case of treating 1Y as MMYield
// AND as swap
    ...
    Zero_Data_Destruct(zero_data);
    if (err = Zero_Data_Construct(
            zero_data, zero_cpd_freq, zero_ybase, zero_NbTimePoints, 0, 0, 0, zero_Ref_Date))
        return (err);
    ...
    if (err = Yield_Data_Construct(
            ...
            yield_data->Delay,
            yield_data->Frequency,
            yield_data->NbTimePoints + 1 - offset,
            0,
            0,
            0,
            yield_data->Ref_Date))
    ...
    for (i = 0; i < max_step - interp_freq; ++i)
    {
        ...
        date = Date_Add(
            zero_data->Ref_Date,
            DAY,
            (int)day_points[local_yield_data.Frequency - offset + 1 + i],
            0,
            0);
#endif
        ...
        zero_data->TimePoints[i + initial_zeros.NbTimePoints] =
            zero_data->DayPoints[i + initial_zeros.NbTimePoints] / ((double)zero_data->YearBase);
        zero_data->Rates[i + initial_zeros.NbTimePoints] = linterpFlat(
            max_step + 1, day_points, zrs, zero_data->DayPoints[i + initial_zeros.NbTimePoints]);
    }
    // clean up
    Zero_Data_Destruct(&initial_zeros);
    Yield_Data_Destruct(&local_yield_data);
...
{
    return (Vol_Surface_Construct(
        new_data,
        old_data->NbSwaptions,
        old_data->expiries,
        old_data->maturities,
        old_data->vols,
        old_data->Rate_Freq,
...
char* MBS_BKTree_Construct_FromFile(
    MBS_BKTree* tree,
    char*       data_dir,
    // Zero_Data * zero_data,
    MBSDF_Data* df_data,
...
char* BKTreeCachStruct_Reset(BKTreeCachStruct* cach, int NbZerosCached, int NbZerosWithOASCached)
{
    ...
    {
        cach->zeros_OAS = mbspt_dmatrix(0, cach->NbFwdZerosWithOAS - 1, 0, cach->NbXNodes - 1);
    }
    ///
    cach->currIdx_zeros = cach->currIdx_zeros_OAS = cach->currIdx_DF = cach->currIdx_DF_OAS =
        cach->currIdx_probs = lastIdx;  // cach->tree->NbTimeSlices;
    //
    return (0);
...
void BKTreeCachStruct_Destruct(BKTreeCachStruct* cach)
{
    if (cach)
    {
        ...
        {
            mbspt_free_dmatrix(
                cach->zeros_OAS, 0, cach->NbFwdZerosWithOAS - 1, 0, cach->NbXNodes - 1);
        }
        //
        cach->DF        = 0;
        cach->DF_OAS    = 0;
...
char* MBS_BKTree_ConstructHelper(
    MBS_BKTree* tree,
    long        RefDate,
    int         PPY,
    double      DX_COEFF,
    ...
    double RefiRate)
{
    ...
    if (err = MBS_BKTree_ConstructHelper(
            tree, RefDate, PPY, DX_COEFF, beta, OAS, XLimit_Coeff, NSigma, RefiBaseIndex, RefiRate))
        return (err);
    if (err = MBSDF_Data_Copy(&(tree->df_data), df_data))
        return (err);
    if (err = Vol_Surface_Construct(
            &(tree->vol_surface),
            NbSwaptions,
            expiries,
            maturities,
...
char* MBS_BKTree_FitVol(MBS_BKTree* tree)
{
    ...
    err = vol_surface_date_gen(
        &NbRow, &NbCol, &colNb, &resets, &starts, &ends, vol_surface);  // num == NbRow!!
    if (err)
        goto CLEANUP1;
    ...
    for (i = 0; i < num; ++i)
    {
        ...
        for (j = 0; j < NbCpn; ++j)
        {
            ...
            ann += df1;
            FwdZeroYield[j] = DFToZR(
                                  df1 / df0,
                                  ((double)j + 1) / ((double)vol_surface->Rate_Freq),
        ...
        y += pow(1.0 + FwdZeroYield[NbCpn - 1] / (double)vol_surface->Rate_Freq,
                 (double)-(NbCpn + 1)) *
             (double)NbCpn * FwdZeroYield[NbCpn - 1] * VolFactor(tree->beta, t2 - t1);
    ...
    for (i = 1; i <= num; i++)
    {
        t2 = starts[i - 1][0];
        SigmaValues[i] =
            (t2 * IntVol[i] - t1 * IntVol[i - 1] * exp(-2.0 * tree->beta * (t2 - t1))) / (t2 - t1) /
    ...
    if (tree->beta > MBS_SMALL)
    {
        for (i = 0; i < tree->NbTimeSlices - 1; ++i)
        {
            ...
            tree->sigmas[i] *=
                DF * log(DF) / (DF - 1.0) * (1.0 - exp(-tree->beta * DT)) / (tree->beta * DT);
        }
    }
    else
    {
...
// geometry of the lattice: as NbPer increases, (bottom, top) increases as (0, 2 * NbPer), with
// center the middle 		until NbPer = some predefined NodeMax, which may be lowered in the course of
//program 		thereaftet (bottom, top) = (NbPer - NodeMax, NbPer + NodeMax), again the middle being the
//center node Before the critical point NbPer = NodeMax is reached, always normal bramching, which
// means: 		(NbPer, i) --> (NbPer+1,i), (NbPer+1, i+1), (NbPer+1, i+2), [k being the time 		After the
//critical point, special branching at top and bottom 			Top:   i --> i-1, i, i+1 			Botom: i -->    i +
//1, i+2, i+3 At each point before the critical point is reached, we test for the up probability for
// top, and down for bottom 		whenever either of them is close to 1.0, we will decrease NodeMax so that
//the critical point is immediately reached
///////////////////////////////////////////////
char* MBS_BKTree_SolveDrift(MBS_BKTree* tree)
{
    ...
    tree->Top = mbspt_ivector(0, NbNodes - 1);  // add 2 more in case we want to price at 2 time
                                                // slices from now to make derivative calc
    ...
    currDiscount = mbspt_dvector(
        0,
        NbSpaceNodes -
            1);  // we need drift for up to NbPer=NbNodes-3, so need space for up to NbPer=NbNodes-1
    ...
    // we can calc Drift Only up to the Period (times[NbNodes - 3], times[NbNodes - 2]), i.e.,
    // Drift[NbNodes-3]; so the last 2 are bogus The last drift will let us calc transition prob from
    // times[NbNodes - 3] to times[NbNodes - 2]; this in turn, allows prices on times[NbNodes - 2] to
    // be backward inducted, but remember times[NbNodes-1] is only an artificial device to allow
    // transitional prob t be calc., s we have what we need
    for (NbPer = 0; NbPer < NbNodes - 2; ++NbPer)
    {
        ...
        nxtCenter  = NbPer + 1;
        MBS_BKTree_CalcVolAndDT(
            &Vol, &DT, NbPer, tree);  // Volatility over the period = sigma * sqrt(time step)

        // get next 1-Prd DF between [NbPer+1, NbPer+2]
        for (i = Bottom; i <= Top + 2; i++)  // if next time slice stays pre-critical it is [Bottom,
        ...
        for (i = Bottom; i <= Top; i++)
        {
            ...
            c += 0.5 *
                     (pow(Vol / tree->DX, 2.) * (x1 + y1 - 2.0 * z1) + tree->beta * DT * (y2 - x2) +
        ...
        for (i = Bottom; i <= Top; i++)  // Calculate state prices in one period
        {
            k = MBS_BKTree_CalcBranching(
                i, NbPer, tree);  //(NbPer + 1 > NodeMax) * ((i == Bottom) - (i == Top));
            MBS_BKTree_GetProb(probs, VolAndDT, tree, NbPer, i);
            nxtStatePr[i + 2 + k] += probs[2] * currStatePr[i] *
                                     currDiscount[i];  // There are 3 branches originating from node
                                                       // i and they contribute to 3 state
            nxtStatePr[i + 1 + k] +=
        ...
        for (i = Bottom; i <= Top + 4;
             i++)  // Permute the values so that they can be used for the next iteration of the
                   // loop,
                   // up to Top+4 because we want to be sure that that nxtStatePrc, etc, 2 steps
                   // from now, are all set to zero.
        ...
        // check probability
        if (!CutFlag)  // If tree has not been cut yet we check that the probabilities at the top
                       // and
...
char* MBS_BKTree_BackwardInduct(
    double*     values,  // both in and out
    int         useOAS,
    int         outTimeIdx,
    MBS_BKTree* tree)
{
    ...
    for (i = tree->Bottom[outTimeIdx]; i < tree->Top[outTimeIdx]; ++i)
    {
        ...
        values[i] =
            ...
            (values[i + 2 + k] * pu[i] + values[i + 1 + k] * p0[i] +
             values[i + k] * pd[i]);  // that the middle branch of normal branching is 1 higher than
                                      // the current node means there is no trouncing of values
    }
    // special handling for top, for if there is special branching, one node, i.e., Top-1, has been
    // trounced upon! i = tree->Top[outTimeIdx];
    k = MBS_BKTree_CalcBranching(i, outTimeIdx, tree);
    if (k == 0)
        tmp = values[i + k];
...
// if useOAS:
//(*zero_prices)[i][j] means the price at time <-> NbPer, with maturity times[NbPer + i], at space
//node j i=0, ... *lastFwdIndex else same but zero_prices have OAS
char* MBS_BKTree_ZeroPrices(
    int* lastFwdIndex, double*** zero_prices, int useOAS, int NbPer, MBS_BKTree* tree)
{
    char*    err;
    double** zeros = 0;
    ...
    if (useOAS && (tree->cach->currIdx_zeros_OAS != NbPer))  // if not yet cached
    {
        ...
        for (i = 0; i <= local_idx; ++i)
        {
            ...
            {
                OAS_Multiple =
                    OAS_scaled * (tree->tree_times[NbPer + i + 1] - tree->tree_times[NbPer + i]);
            }
            else
                OAS_Multiple = 0.0;
...
// zero_prices is shallow copied
//(*zero_prices)[i][j] means the price at time <-> NbPer, with maturity times[NbPer + i], at space
//node j i=0, ... *lastFwdIndex get zeros without OAS
char* MBS_BKTree_ZeroPricesHelper(
    int* lastFwdIndex, double*** zero_prices, int NbPer, MBS_BKTree* tree)
{
    int     i;
    double* d_ptr;
    int     prev_NbZeros;
    ...
    if (NbPer < tree->EndTimeSlices - 1 && tree->cach->currIdx_zeros != NbPer + 1 &&
    ...
    *lastFwdIndex = tree->EndTimeSlices - 1 - NbPer;
    ...
    {
        tree->cach->zeros[0] =
            tree->cach->zeros[prev_NbZeros];  // assign an unoccupied array, to be filled with 1
...
// if term exceeds the number of cached zeros available, automatically, shrink term to whatever is
// available
char* MBS_BKTree_Yield_Approx(double* yields, int freq, double term, int NbPer, MBS_BKTree* tree)
{
    double** zeros        = 0;
    int      lastFwdIndex = 0;
    char*    err;

...
#include "utdates.h"
//**********************//
// All rates and yields  , as input output  , will be quoted in %  , i.e.  , 1%
// <-> 1
//*********************//
...
// units have to be days first  , then months  , in each case the terms must be
// increasing if NbMMTerms > 0 but date_units  , MMTerms are NULL  , still alloc
// memory but don't copy
char *MM_Data_Construct(MM_Data *mm_data, YBASIS YearBase, DIFFBASIS diffBase,
  ...
  int hasFoundMo = 0;
  int prev = 0;
  // check: have to be days first  , then months  , in each the terms must be
  // increasing
  if (YearBase == _YACT)
...
// assume completely unitialized data  , in particular  , pointers are all NULL
// if NbMMTerms > 0 but date_units  , MMTerms are NULL  , still alloc memory but
// don't copy
...
// using basic info from Yield_Data such as basis and frequency  , and knowing
// the time to maturity generate the sets of start dates  , end dates  , and
// coverages
...
// rate  , also cvg memory to be alloc by user  , start is a single date
...
// starting at Ref_Date + Delay + yield_data->yield_terms[0] (months)  , at
// intervals of interp_interval  , and ending at the last yield_term if yeilds
// are provided  , just use it  , else  , use linterp assume that last_date -
// 0-th date is a multiple of interp_interval
...
// yield_data->Ref_Date) to be filled in annuity  , as input  , is the value of
// an annuity at yield_data->Ref_Date  , with frequency etc.  , as encoded in
// yield_data  , 					with last payment on day_points[k-1] (from Ref_Date) yield = YTM
// with maturity day_points[k] = Ref_Date + period * 12/frequency (months) zrs[k]
// = the implied zero-rate annuity = value of the annuity with last payment on
// day_points[k] assume k >= 1
...
// zero_data will be internally initialized  , including memory alloc  , in this
// routine mm_data and yield_data are inputs assume that yield_data starts with
// terms  1Y  , mm_data ends with  < 1Y also assume that mm_data has values at
// terms equal to all the coupon dates of a yield of 1Y maturity interp_freq is
// the frequency of interpolation in zero-rate-space
...
// treating 1Y as MMYield AND as swap
  ...
  if (offset ==
      0) // if 1Y is considered MM  , then we have to back out 1Y Swap Rate
  {
...
char *MBS_BKTree_GetProb(double *probs, double *VolAndDT, MBS_BKTree *tree,
  ...
  if ((!(tree->cach->cachOn)) ||
      (!(tree->cach->currIdx_probs == NbPer))) // if we have not cached  , calc
  {
    if (VolAndDT) {
      Vol = VolAndDT[0];
      DT = VolAndDT[1];
...
// geometry of the lattice: as NbPer increases  , (bottom  , top) increases as
// (0  , 2 * NbPer)  , with center the middle 		until NbPer = some predefined
//NodeMax  , which may be lowered in the course of program 		thereaftet (bottom  ,
//top) = (NbPer - NodeMax  , NbPer + NodeMax)  , again the middle being the
//center node Before the critical point NbPer = NodeMax is reached  , always
// normal bramching  , which means: 		(NbPer  , i) --> (NbPer+1  ,i)  , (NbPer+1  ,
//i+1)  , (NbPer+1  , i+2)  , [k being the time 		After the critical point  ,
//special branching at top and bottom 			Top:   i --> i-1  , i  , i+1 			Botom: i -->
//i + 1  , i+2  , i+3 At each point before the critical point is reached  , we
// test for the up probability for top  , and down for bottom 		whenever either of
//them is close to 1.0  , we will decrease NodeMax so that the critical point is
//immediately reached
///////////////////////////////////////////////
char *MBS_BKTree_SolveDrift(MBS_BKTree *tree) {
  ...
  currDiscount = mbspt_dvector(
      0, NbSpaceNodes - 1); // we need drift for up to NbPer=NbNodes-3  , so
                            // need space for up to NbPer=NbNodes-1
  ...
  // times[NbNodes - 2])  , i.e.  , Drift[NbNodes-3]; so the last 2 are bogus The
  // last drift will let us calc transition prob from times[NbNodes - 3] to
  // times[NbNodes - 2]; this in turn  , allows prices on times[NbNodes - 2] to
  // be backward inducted  , but remember times[NbNodes-1] is only an artificial
  // device to allow transitional prob t be calc.  , s we have what we need
  for (NbPer = 0; NbPer < NbNodes - 2; ++NbPer) {
    ...
    for (i = Bottom; i <= Top + 2;
         i++) // if next time slice stays pre-critical it is [Bottom  , Top+2];
              // if post-critical [Bottom+1  , Top+1]
    {
    ...
    // certain combination of currStatePrc  , prevStatePrc  , and curr
    // probabilities  equal to DF but the probabilities are expressible in terms
    // of drift
    ...
    for (i = Bottom; i <= Top; i++) {
      ...
      // the bottom node branches up (i.e. to bottom+1  , bottom+2  , bottom+3).
...
// calcs the 1-Prd Disc Factor  , with or without OAS for time slice NbPer to
// NbPer + 1 returned in the array staring with address *values and size *sz  ,
// which may be bigger than needed use tree->Bottom and Top to know which are not
// junk
    ...
    values[i] =
        ...
        (values[i + 2 + k] * pu[i] + values[i + 1 + k] * p0[i] +
         values[i + k] * pd[i]); // that the middle branch of normal branching
                                 // is 1 higher than the current node means there
                                 // is no trouncing of values
  }
  // special handling for top  , for if there is special branching  , one node
  // , i.e.  , Top-1  , has been trounced upon! i = tree->Top[outTimeIdx];
  k = MBS_BKTree_CalcBranching(i, outTimeIdx, tree);
  if (k == 0)
    tmp = values[i + k];
...
//times[NbPer + i]  , at space node j i=0  , ... *lastFwdIndex else same but
...
// zero_prices is shallow copied
//(*zero_prices)[i][j] means the price at time <-> NbPer  , with maturity
//times[NbPer + i]  , at space node j i=0  , ... *lastFwdIndex get zeros without
// OAS
char *MBS_BKTree_ZeroPricesHelper(int *lastFwdIndex, double ***zero_prices,
  ...
  *lastFwdIndex = tree->EndTimeSlices - 1 - NbPer;
  ...
  if (prev_NbZeros < tree->cach->NbTNodes) {
    tree->cach->zeros[0] =
        tree->cach->zeros[prev_NbZeros]; // assign an unoccupied array  , to be
                                         // filled with 1

...
// only the Year and Month of dealMatDate is used  , it will always operates as
// if it is the first of the month  , regardless of whether it is a holiday
// duePeriodEndDates starts with the 1st of the month following the first record
// date on or after settledate cachflowDates is the corresponding pay date their
// last are the last pay date and the corresponding 1st of month
...
// given sch_amort_rate  , vector of smm[0..nb-1]  , coupon (annualized)  , and
// df[0..nb_zeros-1][0..nb-1] assume io and po's memory have been alloc by user
// add the pv of underlying cashflow of io and pv to the io and pv
// user has to provide smm and sch_amort_rate in a special way at the beginning
// and end of the backward induction process to do it right 1% coupon <-> 0.01 1%
// smm    <-> 0.01 (similar for sch_amort_rate) zeros: 0          <-> maturity at
// first FNM/FH pay day following (i.e.  , the next 25/15 of a month)

    ...
    int               PayDelay)
{
    ...
    fgets(string, 80, stream);
    fscanf(stream, "%d\n", &cpn_freq);
    if (err = MBSPT_DealStruct_Construct(
            deal_struct, DealMaturityDate, AgcyProg, oterm, cpn_freq, cpn, PayDelay))
        return (err);
    //
    return (err);
...
char* MBSPT_DealStruct_SetTimes(MBSPT_DealStruct* deal_struct, long TradeDate, long SettleDate)
{
    ...
    return (
        MBSPT_CashFlowStruct_Construct(&(deal_struct->cashflow_struct), TradeDate, deal_struct));
}  // MBSPT_DealStruct_SetTimes

// this is a helper, not to be used directly
// assume security has wala as of settle
// only the Year and Month of dealMatDate is used, it will always operates as if it is the first of
// the month, regardless of whether it is a holiday duePeriodEndDates starts with the 1st of the
// month following the first record date on or after settledate cachflowDates is the corresponding
    ...
    MBSPT_CashFlowStruct* cashflow_struct, long spotSettleDate, MBSPT_DealStruct* deal_struct)
{
    ...
    //	cpnDate = DateYMMDD( yy, mm, 1);
    cpnDate =
        Date_Add(cpnDate, MONTH, 1, 0, 1);  // 1st duePeriodEndDate fir cashflows owned by buyer
    for (i = 0; i < cashflow_struct->numCashFlows; ++i)
    {
        cashflow_struct->duePeriodEndDates[i] = cpnDate;
        cashflow_struct->CashFlowDates[i] =
            Date_Add(cashflow_struct->duePeriodEndDates[i], MONTH, payDelayMo, 0, 0);
        cashflow_struct->CashFlowDates[i] =
        ...
        cashflow_struct->duePeriodEndTimes[i] =
            ((double)DateDiff(spotSettleDate, cashflow_struct->duePeriodEndDates[i], _DACT)) /
            365.0;
        cashflow_struct->CashFlowTimes[i] =
            ((double)DateDiff(spotSettleDate, cashflow_struct->CashFlowDates[i], _DACT)) / 365.0;
...
void MBSPT_CashFlowStruct_Destruct(MBSPT_CashFlowStruct* cashflow_struct)
{
    ...
    {
        mbspt_free_lvector(
            cashflow_struct->duePeriodEndDates, 0, cashflow_struct->numCashFlows - 1);
        mbspt_free_dvector(
            cashflow_struct->duePeriodEndTimes, 0, cashflow_struct->numCashFlows - 1);
...
// user has to provide smm and sch_amort_rate in a special way at the beginning and end of the
// backward induction process to do it right 1% coupon <-> 0.01 1% smm    <-> 0.01 (similar for
// sch_amort_rate) zeros: 0          <-> maturity at first FNM/FH pay day following (i.e., the next
// 25/15 of a month)
    ...
    int      NbZerosToSkip)
{
    ...
    for (i = start; i <= end; ++i)
    {
        ...
        // first po
        po[i] = 100.0 * (smm[i] * (1.0 - sch_amort_rate) * zeros[nb_zeros - 1][i]  // unscheduled
        ...
        tmp *= smm[i] *
               (1.0 - sch_amort_rate);  // coupon payments continue until unscheduled paydown occurs
        if (NbZerosToSkip == 0)

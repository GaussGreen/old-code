...
integer ipmpar_(integer *i__) {
  ...
  /*     POINT ARITHMETICS HAVE THE SAME BASE        , SAY B        , AND THAT
   * THE */
  /*     NONZERO NUMBERS ARE REPRESENTED IN THE FORM */

  /*               SIGN (B**E) * (X(1)/B + ... + X(M)/B**M) */

  /*               WHERE X(I) = 0        ,1        ,...        ,B-1 FOR I=1
   * ,...        ,M        , */
  ...
  /*     P.A. FOX        , A.D. HALL        , AND N.L. SCHRYER (BELL
   * LABORATORIES). */
...
doublereal spmpar_(integer *i__) {
  ...
  /*     I IS AN INTEGER HAVING ONE OF THE VALUES 1        , 2        , OR 3. IF
   * THE */
...
/* Subroutine */ int gratio_(doublereal *a, doublereal *x, doublereal *ans,
  ...
  /*     P(A        ,X) AND QANS THE VALUE Q(A        ,X). IND MAY BE ANY
   * INTEGER. */
  ...
  /*     6-TH SIGNIFICANT DIGIT        , AND IF IND .NE. 0        ,1 THEN
   * ACCURACY */
  /*     IS REQUESTED TO WITHIN 1 UNIT OF THE 3RD SIGNIFICANT DIGIT. */

  /*     ERROR RETURN ... */
  /*        ANS IS ASSIGNED THE VALUE 2 WHEN A OR X IS NEGATIVE        , */
  /*     WHEN A*X = 0        , OR WHEN P(A        ,X) AND Q(A        ,X) ARE
   * INDETERMINANT. */
  /*     P(A        ,X) AND Q(A        ,X) ARE COMPUTATIONALLY INDETERMINANT
   * WHEN */
...
/* Subroutine */ int cumchn_(doublereal *x, doublereal *df, doublereal *pnonc,
  ...
  /*     distribution        , i.e.        ,  the probability   that  a   random
   * variable
   */
  /*     which    follows  the  non-central chi-square  distribution        ,
   * with */
  ...
  /*     Uses  formula  26.4.25   of  Abramowitz  and  Stegun        , Handbook
   * of */
  /*     Mathematical    Functions        ,  US   NBS   (1966)    to calculate
   * the */
...
L20:
  ...
  /*     The following code calcualtes the weight        , chi-square        ,
   * and */

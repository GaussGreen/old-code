...
static double LL_computation(double* x)
{
    ...
    global_zeta_array[1] =
        log(garcharray[1].rate / garcharray[0].rate) / garcharray[1].s1 / global_s2_array[1];
    for (i = 2; i <= global_count; i++)
    {
        global_s2_array[i] =
            global_s2_array[i - 1] *
            exp(global_beta_0 + global_beta_1 * global_zeta_array[i - 1] +
                global_beta_2 * (fabs(global_zeta_array[i - 1]) - sqrt(2.0) / SRT_PI));
        if (garcharray[i].s1 == 0)
            garcharray[i].s1 = NULL_S1;
        if (global_s2_array[i] == 0)
            global_s2_array[i] = NULL_S1;
        global_zeta_array[i] = log(garcharray[i].rate / garcharray[i - 1].rate) / garcharray[i].s1 /
                               global_s2_array[i];
    }
...
)
{
    ...
    err = simulated_annealing(
        p, y, num_params, FTOL, num_iter, LL_computation, MAX_TEMP, MIN_TEMP, DEC_FAC);

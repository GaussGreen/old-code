				  ...
				  double	***prod_val)
{
	...
	err = jacobi_diagonalisation(	corr_temp,
									5,
									eigen_val,
									eigen_vec,
									&nrot);

	if (err || eigen_val[0] < 0.0 || eigen_val[1] < 0.0 || eigen_val[2] < 0.0 || eigen_val[3] < 0.0 ||eigen_val[4] < 0.0)
	{
		if (!err)
		{
	...
	err = Get_FX_StochRate_TermStructures5F(und3dfx,
										 &sigma_date_dom,  &sigma_dom,  &sigma_n_dom,
										 &tau_dom, &alpha_dom, &gamma_dom, &rho_dom,
										 &sigma_date_for,  &sigma_for,  &sigma_n_for,
										 &tau_for, &alpha_for, &gamma_for, &rho_for,
	...
	if (!dom_ifr || !dom_fwd1 || !dom_fwd2 || !dom_phi1 || !dom_phi2 || !dom_phi12 || !dom_exp1 || !dom_exp2
		|| !dom_gam1_fwd || !dom_gam2_fwd || !dom_bond_pay || !dom_gam1_pay ||!dom_gam2_pay
		|| !for_ifr || !for_fwd1 || !for_fwd2 || !for_phi1 || !for_phi2 || !for_phi12 || !for_exp1 || !for_exp2
	...
	for (i=xStr.num_evt-1; i>=0; i--)
	{
		...
		{			
			...
			if (grfn_prm->num_fx_df > 0 && grfn_prm->fx_idx != -1)
			{
				...
				if (!grfn_prm->fx_dff || !grfn_prm->fx_gam1 || !grfn_prm->fx_gam2 || !grfn_prm->fx_gam12)
				{
				...
				{
					grfn_prm->fx_dff[k] = swp_f_df (xStr.dts[i], grfn_prm->fx_df_dts[k], (char*) dom_yc);
					grfn_prm->fx_gam1[k] = (1.0 - exp (-lam_dom * grfn_prm->fx_df_tms[k] )) / lam_dom;
					grfn_prm->fx_gam2[k] = (1.0 - exp (-lam_dom2 * grfn_prm->fx_df_tms[k] )) / lam_dom2;
					grfn_prm->fx_gam12[k] =  -0.5 * (grfn_prm->fx_gam1[k] * grfn_prm->fx_gam1[k] * dom_phi1[i+flag] +
			...
			if (grfn_prm->num_dom_df > 0 && grfn_prm->dom_idx != -1)
			{
				...
				if (!grfn_prm->dom_dff || !grfn_prm->dom_gam1 || !grfn_prm->dom_gam2 || !grfn_prm->dom_gam12)
				{
				...
				{
					grfn_prm->dom_dff[k] = swp_f_df (xStr.dts[i], grfn_prm->dom_df_dts[k], (char*) dom_yc);
					grfn_prm->dom_gam1[k] = (1.0 - exp (-lam_dom * grfn_prm->dom_df_tms[k] )) / lam_dom;
					grfn_prm->dom_gam2[k] = (1.0 - exp (-lam_dom2 * grfn_prm->dom_df_tms[k] )) / lam_dom2;
					grfn_prm->dom_gam12[k] = -0.5 * (grfn_prm->dom_gam1[k] * grfn_prm->dom_gam1[k] * dom_phi1[i+flag] +
			...
			if (grfn_prm->num_for_df > 0 && grfn_prm->for_idx != -1)
			{
				...
				if (!grfn_prm->for_dff || !grfn_prm->for_gam1 || !grfn_prm->for_gam2 || !grfn_prm->for_gam12)
				{
				...
				{
					grfn_prm->for_dff[k] = swp_f_df (xStr.dts[i], grfn_prm->for_df_dts[k], (char*) for_yc);
					grfn_prm->for_gam1[k] = (1.0 - exp (-lam_for * grfn_prm->for_df_tms[k] )) / lam_for;
					grfn_prm->for_gam2[k] = (1.0 - exp (-lam_for2 * grfn_prm->for_df_tms[k] )) / lam_for2;
					grfn_prm->for_gam12[k] = -0.5 * (grfn_prm->for_gam1[k] * grfn_prm->for_gam1[k] * for_phi1[i+flag] +
...
FREE_RETURN:
	...
	if (eigen_val) free_dvector(eigen_val, 0, 4);
	if (eigen_vec) free_dmatrix(eigen_vec, 0, 4, 0, 4);
	if (corr_temp) free_dmatrix(corr_temp, 0, 4, 0, 4);
	...
	if (dom_fwd1) free (dom_fwd1);
	if (dom_fwd2) free (dom_fwd2);
	if (dom_exp1) free (dom_exp1);
	if (dom_exp2) free (dom_exp2);
	if (dom_phi1) free (dom_phi1);
	if (dom_phi2) free (dom_phi2);
	if (dom_phi12) free (dom_phi12);
	if (dom_gam1_fwd) free (dom_gam1_fwd);
	if (dom_gam2_fwd) free (dom_gam2_fwd);
	if (dom_bond_pay) free (dom_bond_pay);
	if (dom_gam1_pay) free (dom_gam1_pay);
	if (dom_gam2_pay) free (dom_gam2_pay);

	if (for_ifr) free (for_ifr);
	if (for_fwd1) free (for_fwd1);
	if (for_fwd2) free (for_fwd2);
	if (for_exp1) free (for_exp1);
	if (for_exp2) free (for_exp2);
	if (for_phi1) free (for_phi1);
	if (for_phi2) free (for_phi2);
	if (for_phi12) free (for_phi12);
	if (for_gam1_fwd) free (for_gam1_fwd);
	if (for_gam2_fwd) free (for_gam2_fwd);

	if (fx_fwd) free (fx_fwd);

	if (covar) free_f3tensor(covar, 0, nstp-1, 0, 4, 0, 4);
		...
		for (i=0; i<nstp; i++)
		{
			if (void_prm[i])
			{
					...
					&& grfn_prm->fx_idx != -1)
				{
					...
					if (grfn_prm->fx_gam1) free_dvector (grfn_prm->fx_gam1, 0, grfn_prm->num_fx_df-1);
					if (grfn_prm->fx_gam2) free_dvector (grfn_prm->fx_gam2, 0, grfn_prm->num_fx_df-1);
					if (grfn_prm->fx_gam12) free_dvector (grfn_prm->fx_gam12, 0, grfn_prm->num_fx_df-1);
				}
					...
					&& grfn_prm->dom_idx != -1)
				{
					...
					if (grfn_prm->dom_gam1) free_dvector (grfn_prm->dom_gam1, 0, grfn_prm->num_dom_df-1);
					if (grfn_prm->dom_gam2) free_dvector (grfn_prm->dom_gam2, 0, grfn_prm->num_dom_df-1);
					if (grfn_prm->dom_gam12) free_dvector (grfn_prm->dom_gam12, 0, grfn_prm->num_dom_df-1);
				}
					...
					&& grfn_prm->for_idx != -1)
				{
					...
					if (grfn_prm->for_gam1) free_dvector (grfn_prm->for_gam1, 0, grfn_prm->num_for_df-1);
					if (grfn_prm->for_gam2) free_dvector (grfn_prm->for_gam2, 0, grfn_prm->num_for_df-1);
					if (grfn_prm->for_gam12) free_dvector (grfn_prm->for_gam12, 0, grfn_prm->num_for_df-1);
				}
				  ...
				  double	***prod_val)
{
	...
	if (!dom_ifr || !dom_fwd || !dom_std || !dom_phi1 || !dom_phi2 || !dom_phi12 || !dom_beta || !dom_bond_pay || !dom_beta_pay 
		|| !for_ifr || !for_fwd || !for_std || !for_phi1 || !for_phi2 || !for_phi12 || !for_beta
		|| !fx_fwd || !fx_std)
	{
	...
	for (i=nstp-1; i>=0; i--)
	{
		...
		{			
			...
			if (grfn_prm->num_fx_df > 0 && grfn_prm->fx_idx != -1)
			{
				...
				for (k=0; k<grfn_prm->num_fx_df; k++)
				{
					grfn_prm->fx_dff[k] = swp_f_df (xStr.dts[j], grfn_prm->fx_df_dts[k], (char*) dom_yc);
					grfn_prm->fx_gam1[k] = (1.0 - exp (-lam_dom * grfn_prm->fx_df_tms[k] )) / lam_dom;
					grfn_prm->fx_gam2[k] = (1.0 - exp (-lam_dom2 * grfn_prm->fx_df_tms[k] )) / lam_dom2;
					grfn_prm->fx_gam12[k] =  -0.5 * (grfn_prm->fx_gam1[k] * grfn_prm->fx_gam1[k] * dom_phi1[i] +
			...
			if (grfn_prm->num_dom_df > 0 && grfn_prm->dom_idx != -1)
			{
				...
				for (k=0; k<grfn_prm->num_dom_df; k++)
				{
					grfn_prm->dom_dff[k] = swp_f_df (xStr.dts[j], grfn_prm->dom_df_dts[k], (char*) dom_yc);
					grfn_prm->dom_gam1[k] = (1.0 - exp (-lam_dom * grfn_prm->dom_df_tms[k] )) / lam_dom;
					grfn_prm->dom_gam2[k] = (1.0 - exp (-lam_dom2 * grfn_prm->dom_df_tms[k] )) / lam_dom2;
					grfn_prm->dom_gam12[k] = -0.5 * (grfn_prm->dom_gam1[k] * grfn_prm->dom_gam1[k] * dom_phi1[i] +
			...
			if (grfn_prm->num_for_df > 0 && grfn_prm->for_idx != -1)
			{
				...
				for (k=0; k<grfn_prm->num_for_df; k++)
				{
					grfn_prm->for_dff[k] = swp_f_df (xStr.dts[j], grfn_prm->for_df_dts[k], (char*) for_yc);
					grfn_prm->for_gam1[k] = (1.0 - exp (-lam_for * grfn_prm->for_df_tms[k] )) / lam_for;
					grfn_prm->for_gam2[k] = (1.0 - exp (-lam_for2 * grfn_prm->for_df_tms[k] )) / lam_for2;
					grfn_prm->for_gam12[k] = -0.5 * (grfn_prm->for_gam1[k] * grfn_prm->for_gam1[k] * for_phi1[i] +
...
FREE_RETURN:
	...
	if (dom_std) free (dom_std);
	if (dom_phi1) free (dom_phi1);
	if (dom_phi2) free (dom_phi2);
	if (dom_phi12) free (dom_phi12);
	if (dom_beta) free (dom_beta);
	if (dom_bond_pay) free (dom_bond_pay);
	if (dom_beta_pay) free (dom_beta_pay);

	if (for_ifr) free (for_ifr);
	if (for_fwd) free (for_fwd);
	if (for_std) free (for_std);
	if (for_phi1) free (for_phi1);
	if (for_phi2) free (for_phi2);
	if (for_phi12) free (for_phi12);
...
Err		err = NULL;
	...
	for (k=0; k<nb_dates-1; k++)
	{
		...
		for (i = StartIndex; i < EndIndex + 1; i++)
		{
			...
			adj_fx_pay += correl_dom_fx * sig_dom * sig_fx * (Etha_Func (lda_dom, end_mat, T1, T2)
															  -Etha_Func (lda_dom, mat_pay, T1, T2))
					   - correl_dom_for * sig_dom_for * (Psi_Func (lda_for, lda_dom, end_mat, T1, T2) 
		...
		dom_fx_cov[k+1] = exp(-lda_dom * end_mat) * 
													(correl13_1 / lda_dom
													- correl_dom_for * correl13_2 / lda_for
													+ correl_dom_fx * correl13_3);
						  
		for_fx_cov[k+1] = exp(-lda_for * end_mat) * 
													(-correl23_1 / lda_for
													+ correl_dom_for * correl23_2 / lda_dom
...
double	exp_dom_mat, exp_dom_mat2, exp_dom_pay, exp_dom_pay2,
		exp_for_mat, exp_for_mat2, exp_for_pay, exp_for_pay2;
int		i, k;
long	StartIndex, EndIndex;
...
Err		err = NULL;
	...
	{		
		...
		err = Fx5DtsImpliedVol(	
							end_mat, start_mat, end_mat,
							sig_dates, nb_sig_dates,
							sig_curve_dom, lda_dom, alpha_dom, gamma_dom,
							sig_curve_for, lda_for, alpha_for, gamma_for,
							sig_dates, sig_curve_fx, nb_sig_dates,
							correl,
							&fx_vol);	
		...
		dom_phi12[k+1] = dom_phi12[k] * exp_dom_mat * exp_dom_mat2 + correl[0][1] * alpha_dom * phi_dom12;

		dom_fwd1[k+1] =	dom_phi1[k] * exp_dom_mat * Phi_Func(-lda_dom, start_mat, start_mat, end_mat)
										+ expect_dom1 / lda_dom
		...
		dom_fwd2[k+1] = dom_phi2[k] * exp_dom_mat2 * Phi_Func(-lda_dom2, start_mat, start_mat, end_mat)
		...
		for_phi12[k+1] = for_phi12[k] * exp_for_mat * exp_for_mat2 + correl[2][3] * alpha_for * phi_for12;

		for_fwd1[k+1] =	for_phi1[k] * exp_for_mat * Phi_Func(-lda_for, start_mat, start_mat, end_mat) + expect_for1 / lda_for
		...
		for_fwd2[k+1] = for_phi2[k] * exp_for_mat2 * Phi_Func(-lda_for2, start_mat, start_mat, end_mat) + alpha_for2 * expect_for2 / lda_for2
		...
		fx_fwd[k+1] = expect_fx 
					+ correl[0][4] * adj_pay_fx1 + correl[1][4] * alpha_dom * adj_pay_fx2
		...
		cov[0][4] = cov[4][0] = correl[0][4] * covar_dom1_fx1
							- correl[0][2] * covar_dom1_fx2 / lda_for - correl[0][3] * alpha_for * covar_dom1_fx3 / lda_for2
							+ covar_dom1_fx4 / lda_dom + correl[0][1] * alpha_dom * covar_dom1_fx5 / lda_dom2; 
		...
		cov[1][4] = cov[4][1] = correl[1][4] * alpha_dom * covar_dom2_fx1
							- correl[1][2] * alpha_dom * covar_dom2_fx2 / lda_for - correl[1][3] * alpha_domfor * covar_dom2_fx3 / lda_for2
							+ correl[0][1] * alpha_dom * covar_dom2_fx4 / lda_dom + alpha_dom2 * covar_dom2_fx5 / lda_dom2;

		cov[2][2] = phi_for1;
		cov[2][3] = cov[3][2] = correl[2][3] * alpha_for * phi_for12;
		cov[2][4] = cov[4][2] = correl[2][4] * covar_for1_fx1
							- covar_for1_fx2 / lda_for - correl[2][3] * alpha_for * covar_for1_fx3 / lda_for2
							+ correl[0][2] * covar_for1_fx4 / lda_dom + correl[1][2] * alpha_dom * covar_for1_fx5 / lda_dom2;

		cov[3][3] = alpha_for2 * phi_for2;
		cov[3][4] = cov[4][3] = correl[3][4] * alpha_for * covar_for2_fx1
							- correl[2][3] * alpha_for * covar_for2_fx2 / lda_for - alpha_for2 * covar_for2_fx3 / lda_for2
							+ correl[0][3] * alpha_for * covar_for2_fx4 / lda_dom + correl[1][3] * alpha_domfor * covar_for2_fx5 / lda_dom2;

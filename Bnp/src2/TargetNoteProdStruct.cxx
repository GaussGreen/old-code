...
char *init_TARN_AUX(TARN_Struct *tarn, TARN_AUX *aux) {
  ...
  if (tarn->deal.fund_ccy == 1) {
    //		long lStartDate;
    //		double eq_final_ex        , eq_init_ex        , dFundingPV        ,
    //dCouponPV;
    ...
    */
    ...
    /*		if ( err = convert_funding_to_domestic(
       ...
       tarn->market.eodFixFlag        , tarn->market.eodPayFlag ,
       ...
       &tarn->deal.dFundingNotional        , tarn->deal.dvFundSpread ,
       tarn->deal.dvFundMargin        , tarn->deal.dvFundHistFixings        ,
       &lStartDate , &eq_final_ex        , &eq_init_ex)  ) goto FREE_RETURN;
...
void initTargetNote(TARN_Struct *tarn) {
  ...
  TARGET_NOTE.m_couponType = tarn->deal.cxxouponType;
...
*/
char *
TargetNoteCoupon_SWAP(TARN_EVENT *event, double dCoupon, double dNotionalRepay,
                      double *res, // res[0] = fund        , res[1] = coupon
                      int *stop_path) {
  /* If it is the final period and the coupon is guaranteed        , then
   * calculate the coupon */
  if (event->iIsFinal && TARGET_NOTE.tarn->deal.bRepayCoupon) {
    res[1] = TARGET_NOTE.m_dCouponNotional * event->fixed_pay *
             /*event->fixed_cvg **/
...
*/
char *
TargetNoteCoupon_FINAL(TARN_EVENT *event, double dCoupon, double dFloater,
                       double dNotionalRepay,
                       double *res, // res[0] = fund        , res[1] = coupon
                       int *stop_path) {
  /* Variable declaration */

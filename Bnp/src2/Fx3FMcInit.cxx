...
/* integral(s        , t        , sigB(u        , T        , x) * sigB(u , T
 * , y)) */
...
/* integral(s        , t        , sigB(u        , Tx        , x) * sigB(u , Ty
 * , y)) */
...
Err fill_mc_init(long pay_date, double pay_time, double *date, double *time,
  ...
  for (k = 0; k < nb_dates - 1; k++) {
    ...
    dom_fx_cov[k + 1] = exp(-lda_dom * end_mat) *
                        (correl13_1 / lda_dom -
                         correl_dom_for * correl13_2 / lda_for + correl13_3);
...
Err fill_mc_init_corr(long pay_date, double pay_time, double *date,
  ...
  for (k = 0; k < nb_dates - 1; k++) {
    ...
    for (i = StartIndex; i < EndIndex + 1; i++) {
      ...
      adj_pay_for += correl_dom_for * sig_domfor *
                     (x_for - exp(-lda_dom * mat_pay) * x_domfor);
      ...
      correl13_2 += correl_dom_for * sig_domfor *
                    (x_dom - exp(-lda_for * end_mat) * x_domfor);
      correl13_3 += correl_dom_fx * sig_dom * sig_fx * x_dom;

      /* foreign fx */
      correl23_1 += sig_for2 * (x_for - exp(-lda_for * end_mat) * y_for);
      correl23_2 += correl_dom_for * sig_domfor *
                    (x_for - exp(-lda_dom * end_mat) * x_domfor);

...
/*<%%STA-----------------------------------------------------------------
  ...
  AUTHOR          :O. Van Eyseren (from E. Auld code)
  DESCRIPTION     :calculate probabilities        , assuming that
node->drift_sam and node->var_at_sam are correctly set        , as well as
node->mid_son_index If the mid_son_index hits the top (bottom)        , it is
decremented (incremented).  Probabilities are bounded to be between PROBUPB and

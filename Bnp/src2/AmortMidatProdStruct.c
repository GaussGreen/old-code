...
AM_FUND_LEG	fund_leg)
{
	...
	if ((!fund_leg->cpn)||(!fund_leg->notional)||(!fund_leg->margin)||(!fund_leg->spread))
	{
					...
					AM_STR		am)
{
	...
	/*	Check that ex and set dates are strictly increasing
			Also check that funding and fix indices are strictly increasing,
...
//			return serror ("Calls %d and %d -indexed after today- are redundant", i-1, i);
	...
	for (i=0; i<am->num_calls; i++)
	{
		...
		if (	am->fund_leg->cpn[am->call[i].fund_idx].start_date < am->call[i].ex_date
			||	am->fund_leg->cpn[am->call[i].fund_idx].pay_date < am->call[i].ex_date)
		{
			return "A funding coupon starts before its exercise date";
		}
	
		if (	am->fix_leg->cpn[am->call[i].fix_idx].start_date < am->call[i].ex_date
...
FREE_RETURN:
	...
	if(lambda_time) free(lambda_time);
...
*/
	...
	*diag_prices = (double *) calloc(*nex, sizeof(double));
	...
	else
	{
		...
		if(model==1)
		{
			err = Compute_CoInitalSwaps_Correl(SpotDate,
											CorrelName,
											get_correl,
											StartDate,
											EndDate,
...
//	}
	...
	err = amortMidat_compute_diagonal_prices(yc, vc, ref, swap_freq, swap_basis, 
						today, eod_flag, mintime, mininterval, UseVol,
						&nex, &firstex, &ex_bool, &diag_prices, european_model, 
...
//	}
	...
	err = amortMidat_compute_diagonal_prices_new(yc, vc, ref, swap_freq, swap_basis, 
						today, eod_flag, mintime, mininterval, UseVol,
						&nex, &firstex, &ex_bool, &diag_prices, european_model, 
						get_correl, CorrelName, get_cash_vol, 
						/*GetVolForBadr, cVolType,*/ am);
...
*/
	...
	for(i = 0; i <und->lambda_n;++i)
	{
		...
		und->pdlambda_date[i] = today + und->pdlambda_time[i] * DAYS_IN_YEAR + 1.0e-08 ;
	}
...
AM_UND		und)
{
	...
	if(und->num_prices>0)
	{
		if (und->mkt_prices) free (und->mkt_prices);
		if (und->mdl_prices) free (und->mdl_prices);
		if (und->exercise_dates) free (und->exercise_dates);
		und->num_prices = 0;
	}
...
Err		err = NULL;
	...
	/* Phi Calculation */
	err = LGM2FCalcPhi(	am->num_calls+1, 
						time,
						und->sigma,
						und->sigma_time,
						und->sigma_n,
	...
	for (i=0; i<am->num_calls; i++)
	{
		...
		err = am_payoff_4_lgm2f_adi(call->ex_date,
									call->ex_time,
							...
							double		***prod_val)
{
	...
	{	
		...
		{		
			...
			fund_leg_pv += eval_const->df[eval_const->start_idx] * fund_leg->notional[call->fund_idx];
			...
			if (call->pay_rec == 0)
			{
				iv = fix_leg_pv - fund_leg_pv - eval_const->df[eval_const->start_idx] * fix_leg->fee[call->fix_idx];
			}
			else
			{
				iv = fund_leg_pv - fix_leg_pv + eval_const->df[eval_const->start_idx] * fix_leg->fee[call->fix_idx];
			}

/* ===============================================================================
   ...
   PURPOSE	: Performs the calibration of interest rate model.
                          ...
                          If the FixedPoint algorithm is not chosen        , the
   calibration is done immediately with the real model (call to srt_f_calibcore)
                          Otherwise        , the calibration is done using a
   FIXED POINT ALGORITHM principle.

                          The complex model price is approximated by a rough LGM
   price (including a rough Beta correction) The Algorithm works as follows:
                                - the approximate parameters LGM model is
   calibrated        , using the required method (LM        ,...)
                                - the exact error in prices is computed with the
   real model
                                - the price target is corrected by the error
   (symmetry)
                                - the approximate LGM model is re-calibrated on
   those new prices
                                - ...
                          Please note that the correlation is calibrated using
   LGM (exact) and is not modified after
...
Err srt_f_calib_main(SrtGrfnParam *psGrfnParams, SrtMdlType eModelType,
                     SrtMdlDim eModelDim, SwapDP *psSwapDp, double *pdStrike,
                     double *pdBondStrike, StructType *peOptionType,
                     SrtReceiverType *peRecPay, double *pdPrice, double *pdVega,
                     String *pszRefRateCode, long lNumInstruments,
                     double *dFraMaturities, double **ppdCorrelationMatrix,
                     long lNumTenors, SrtUndPtr sUndPtr,
                     SrtCalibParam *psCalibParams, double **ppdSigmaCurve,
                     long lNumSigmas, long lNumSigmaCols, double **ppdTauCurve,
                     long lNumTaus, long lNumTauCols, double *pdChiSquare) {
...
/* Send a message warning the user: calibration starts */
  ...
  /* If FIXED POINT algorithm is required        , LGM Starting point should be
   * there
   */
  ...
  if (psCalibParams->bLgmStartPoint == SRT_YES) {
    ...
    szErr = from_model_ts_to_LGM_ts(eModelDim, ppdSigmaCurve, lNumSigmas,
                                    szYieldCurveName, ppdLgmSigmaCurve);
    if (szErr) {
    ...
    szErr = srt_f_calib_core(
        psGrfnParams, eModelType, eModelDim, psSwapDp, pdStrike, pdBondStrike,
        peOptionType, peRecPay, pdPrice, pdVega, pszRefRateCode,
        lNumInstruments, dFraMaturities, ppdCorrelationMatrix, lNumTenors,
        sUndPtr, psCalibParams, ppdLgmSigmaCurve, lNumSigmas, lNumSigmaCols,
        ppdTauCurve, lNumTaus, lNumTauCols, pdChiSquare);
    ...
    /* Rebuilds a pseudo-equivalent TS from the calibrated LGM (inverse of
     * approximation) */
    szErr = from_LGM_ts_to_model_ts(eModelDim, ppdLgmSigmaCurve, lNumSigmas,
                                    szYieldCurveName, ppdSigmaCurve);
    if (szErr) {
      free_dmatrix(ppdLgmSigmaCurve, 0, lNumSigmaCols - 1, 0, lNumSigmas - 1);
      return szErr;
    }
  ...
  if (psCalibParams->eAlgoType != FIXED_POINT) {
    ...
    szErr = srt_f_calib_core(
        ...
        peOptionType, peRecPay, pdPrice, pdVega, pszRefRateCode,
        lNumInstruments, dFraMaturities, ppdCorrelationMatrix, lNumTenors,
        sUndPtr, psCalibParams, ppdSigmaCurve, lNumSigmas, lNumSigmaCols,
        ppdTauCurve, lNumTaus, lNumTauCols, pdChiSquare);
    if (szErr) {
      return szErr;
    }

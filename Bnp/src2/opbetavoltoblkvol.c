...
/* First term */
	first_term = pow(forward*strike, (1.0 - beta)/ 2.0) * ( 1.0 + 1.0 / 24.0 * pow((1.0 - beta) * log(forward/strike),2.0) );

/* Second Term */
	second_term = pow( 1.0 - beta , 2.0) / 24.0;
	second_term += 0.25 * rho * beta * vovol / betavol * pow(forward*strike, (1.0 - beta)/ 2.0);
	second_term += (2.0 - 3.0*rho*rho)/24.0 * vovol *vovol / (betavol*betavol) * pow(forward*strike, (1.0 - beta));

...
SigKapTSPtr LGMConvertZGtoSigKap(Date tNow, LGM_TS *input) {
  ...
  /*	Case 1: there is only two Gs  , thus a single mean-reversion that can be
     chosen arbitrarily  , so we choose 0	*/
...
Date *LGMFixLegSched(Date tEx, Date tEnd, long extraperiods, LGMMarkConv *conv,
  ...
  for (i = 1; i <= extraperiods; i++)
    tPay[n + i] =
        add_unit(tEnd, i * swmonth, SRT_MONTH, conv->sbdconv /*  , ccy */);

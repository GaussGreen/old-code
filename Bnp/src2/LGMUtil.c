...
static void GetSigFromZeta(
    double g0, long ntau, double* sig, Date* tau, double* zeta, long numK, Date* t, double* kap);
...
SrtSimMidAtPtr LGMCreateSimMidAt(long nEx, long nPay)
{
    ...
    if (ptr->Payment == NULL || ptr->tPay == NULL || ptr->tEx == NULL || ptr->tStart == NULL ||
        ptr->Strike == NULL || ptr->FirstPay == NULL || ptr->RedFirstPay == NULL)
    {
        LGMFreeSimMidAt(&ptr);
        return (NULL);
    }
...
SrtGenMidAtPtr LGMCreateGenMidAt(long nEx, long* nPayArr)
{
    ...
    if (ptr->tEx == NULL || ptr->nPay == NULL || ptr->Payment == NULL || ptr->tPay == NULL)
    {
        srt_free(ptr->tEx);
        srt_free(ptr->nPay);
        srt_free(ptr->Payment);
...
SrtSimAmerPtr LGMCreateSimAmer(long nfix, long nflt)
{
    ...
    if (ptr->tfixStart == NULL || ptr->tfixEnd == NULL || ptr->tfixPay == NULL ||
        ptr->fixCoupon == NULL || ptr->ExtraPrem == NULL || ptr->tfltFixing == NULL ||
        ptr->tfltPay == NULL)
    {
        LGMFreeSimAmer(&ptr);
        return (NULL);
...
LGMErr LGMValidSimAmer(SrtSimAmer* ptr, Date tfirst)
{
    ...
    for (i = 0;
         i < (nfix - 1) && ptr->tfixStart[i] < ptr->tfixEnd[i] &&
         ptr->tfixStart[i] < ptr->tfixStart[i + 1] && ptr->tfixEnd[i] < ptr->tfixEnd[i + 1] &&
         ptr->tfixPay[i] <= ptr->tfixPay[i + 1] && ptr->tfixStart[i] < ptr->tfixPay[i];
         i++)
        ;
    ...
    for (j = 0; j < (nflt - 1) && ptr->tfltPay[j] < ptr->tfltPay[j + 1] &&
                ptr->tfltFixing[j] < ptr->tfltFixing[j + 1] && ptr->tfltFixing[j] < ptr->tfltPay[j];
         j++)
        ;
...
SrtCallInvFltPtr LGMCreateCallInvFlt(long nEx, long n)
{
    ...
    if (!ptr->tEx || !ptr->iSet || !ptr->strike || !ptr->tCpnStart || !ptr->tCpnPay || !ptr->a ||
        !ptr->gear || !ptr->cvg || !ptr->lcvg || !ptr->cap_str)
    {
...
SrtCallTimeSwapPtr LGMCreateCallableTimeSwap(long nEx, long n, long observation_freq)
{
    ...
    */
    ...
    if (!ptr->tEx || !ptr->iSet || !ptr->tCpnStart || !ptr->tCpnPay || !ptr->tCpn || !ptr->gear ||
        !ptr->tCvgCpn || !ptr->barriers || !ptr->observationdays || !ptr->ratiodays_for_subperiod)
    {
...
LGM_TSPtr LGMCreateLGM_TS(long numZ, long NumG)
{
    ...
    if (ptr->zdate == NULL || ptr->zeta == NULL || ptr->Gdate == NULL || ptr->G == NULL)
...
SigKapTSPtr LGMCreateSigKapTS(long NumS, long NumK)
{
    ...
    if (ptr->sdate == NULL || ptr->sig == NULL || ptr->kdate == NULL || ptr->kap == NULL)
    {
        LGMFreeSigKapTS(&ptr);
    }
...
LGM_TSPtr LGMConvertSigKaptoZG(Date tNow, SigKapTS* SKtsPtr)
{
    ...
    for (j = 1; j <= numZ; j++)
    {
        ...
        LGMtsPtr->zeta[j] =
            LGMtsPtr->zeta[j - 1] + SKtsPtr->sig[j - j1 - 1] * SKtsPtr->sig[j - j1 - 1] * cumul;
    }
...
static double find_alpha(
    double dt12, double dt23, double dg12, double dg23, double* lambda, double* beta)
{
...
SigKapTSPtr LGMConvertZGtoSigKap(Date tNow, LGM_TS* input)
{
    ...
    /*	Case 2: there is more than two Gs	*/
    {
        ...
        alpha = find_alpha(
            (Gdate[numG - 2] - Gdate[numG - 3]) * YEARS_IN_DAY,
        ...
        for (i = numG - 1; i >= 1; i--)
        {
            ...
            output->kap[i - 1] =
                find_lambda((Gdate[i] - Gdate[i - 1]) * YEARS_IN_DAY, G[i - 1] - G[i], alpha, beta);
    ...
    lambda = interpolate_lambda(
        input->zdate[input->numZ - 1], Gdate[numG - 1], output->kdate, output->kap, output->numK);
    ...
    {
        lambda = interpolate_lambda(
            input->zdate[i], input->zdate[i - 1], output->kdate, output->kap, output->numK);

        output->sdate[i - 1] = input->zdate[i];
        output->sig[i - 1]   = find_sigma(
            (input->zdate[i] - input->zdate[i - 1]) * YEARS_IN_DAY,
...
LGMCalSetPtr LGMCreateCalSet(long nPay, long nEx, long nLast)
{
    ...
    if (ptr->tPay == NULL || ptr->cvgpay == NULL || ptr->Dpay == NULL || ptr->tEx == NULL ||
        ptr->tStart == NULL || ptr->DStart == NULL || ptr->ifirst == NULL || ptr->nlong == NULL ||
        ptr->cvgfirst == NULL || ptr->Rflong == NULL || ptr->Vlong == NULL || ptr->nshort == NULL ||
        ptr->Rfshort == NULL || ptr->Vshort == NULL || ptr->tEnd == NULL || ptr->cvgcap == NULL ||
        ptr->Dcap == NULL || ptr->Rfcap == NULL || ptr->Vcap == NULL || ptr->Rfix == NULL ||
        ptr->Vfix == NULL)
...
Date* LGMFixLegSched(Date tEx, Date tEnd, long extraperiods, LGMMarkConv* conv, long* nPayPtr)
{
    LGMErr  error;
    long    i, n, nPay;
    Date    tStart, *tPay;
...
LGMErr LGMGetRfixFromX(
    Date    tEx,
    Date    tEnd,
    double  x, /* definition of swaption */
    Date    tNow,
    String  ycName,
    LGM_TS* tsPtr, /* yield curve name */
    double* Rfix)  /* output */
{
    LGMErr      error;
    LGMMarkConv conv;
...
LGMErr LGMGetFloatRateFromX(
    Date tStart, Date tPay, double x, String ycName, LGM_TS* tsPtr, double* FloatRate)
{
    Err         err    = NULL;
    SrtCurvePtr yldcrv = NULL;
    LGMMarkConv conv;
    ...
    dDfStartPay = (dDfPay / dDfStart) * exp((dGPay - dGStart) * x -
...
/* On error, returns -1 */
double LGMNormOptPrice(
    double fwd, double strike, double NormVol, double mat, double dftoStart, SrtCallPutType callput)
{
...
/* On error, returns -1 */
double LGMNormImpVol(
    double price, double fwd, double strike, double mat, double dftoStart, SrtCallPutType callput)
{
    double x, stddev, vol;
    ...
    x = fabs(fwd - strike);
    stddev =
        (price + 0.5 * x) / LGMsafeGauss(0); /* guaranteed to converge from this initial guess */
    arg1   = -x / stddev;
    ...
    SrtCallPutType callput)
{
    ...
    {
        price =
            strike * LGMsafeNorm(theta + 0.5 * stddev) - fwd * LGMsafeNorm(theta - 0.5 * stddev);
        if (callput == SRT_CALL)
            price = price + fwd - strike;
        if (callput == SRT_STRADDLE)
    ...
    price = fwd * LGMsafeNorm(theta + 0.5 * stddev) - strike * LGMsafeNorm(theta - 0.5 * stddev);
    if (callput == SRT_PUT)
        price = price + strike - fwd;
    if (callput == SRT_STRADDLE)
...
Err update_autocal_midat_struct(
    SrtSimMidAt* MidAt,
    char*        YcName,
    ...
    Err (*GetBeta)(Date, Date, double*))
{
    ...
    double *CvgFirst, *CvgPay, *Ks, *StDLong, DtPay, DtStart, DtEnd, Payt, SwapLevel, SwapRate,
        YrtoExp, CEVVol, Beta, NORMVol;
    SrtCurvePtr      YldCrv;
    String           ccy_str;
    SrtCcyParam*     ccy_param = NULL;
    ...
    for (i = 1; i <= nEx; i++)
    {
        ...
        Ks[i - 1] = Payt / (MidAt->Strike[i - 1] * SwapLevel) +
        ...
        err = srt_f_optsarbvol(
            SwapRate, SwapRate, YrtoExp, CEVVol, 0.0, 0.0, 0.0, input_vol, SRT_NORMAL, &NORMVol);
        if (err)
            return err;

	...
	double		strike)								/*	Strike */								
{
	...
	return lgmopval2F (ncpn, coupon, cpn_G1, cpn_G2, ex_zeta1, ex_zeta2, ex_zeta12, ex_G1, ex_G2);
}


double ZCMidat_lgmZCsopval2F(
	int			ncpn,								/*	Number of cash-flow dates, including
															start and end date */
	double		not[],								/*	Notional */
	double		df[],								/*	Df to cash flow dates */
															...
															n-1 */
{
	...
	for (i=0; i<nex - (skip_last > 0); i++)
	{
		...
		coupon[j] = -cpn_df[j];
		coupon[ncpn - j - 1] = coupon[ncpn - j - 1] + exerFixCoupon[j] * cpn_df[ncpn - j - 1];
															...
															n-1 */
{
	...
	do
	{
		...
		if (err = ZCMidat_lgmprcapgivenlambda(
			...
			nex, ex_time, ex_cpn, ex_sncpn, ex_lstrike, ex_lprice, ex_sstrike, ex_sweight, ex_zeta,
			exerFixCoupon,
			lam1, one2F, alpha, gamma, rho, skip_last, 1, &pr1))
		{
			return err;
		}
	...
	{	
		...
		if (err = ZCMidat_lgmprcapgivenlambda(
			...
			nex, ex_time, ex_cpn, ex_sncpn, ex_lstrike, ex_lprice, ex_sstrike, ex_sweight, ex_zeta,
			exerFixCoupon,
			lam2, one2F, alpha, gamma, rho, skip_last, 1, &pr2))
		{
			return err;
		}
		...
		if (fabs (lam2 - lam1) < CALPRES || fabs (pr2 - pr1) < CALPRES) break;
...
//--------------------------------------------
	...
	for(k=1;k<nbcoupon;++k)
	{
		...
		{
			Cumcoupon = Cumcoupon * (1 + long_strike[fix_index-1] * fix_cvgs[fix_index-1]);
			Cumcoupon += long_strike[fix_index-1];
//			coupon[k] += long_strike[fix_index-1] * fix_cvgs[fix_index-1] * swp_f_df (today, fix_pay_dates[fix_index], yc_name);
		...
		else if((coupon_dates[k]!=fix_pay_times[fix_index])&&(coupon_dates[k]==float_pay_times[float_index]))
		{
			...
			coupon[k] += -( float_spreads[float_index-1] * float_cvgs[float_index-1] ) * swp_f_df (today, float_pay_dates[float_index], yc_name);
...
//				coupon[k] += long_strike[fix_index-1] * fix_cvgs[fix_index-1] * swp_f_df (today, fix_pay_dates[fix_index], yc_name);
				...
				exer_date[fix_index-1] = fix_pay_dates[fix_index-1];
				fix_index = fix_index + 1;
				coupon[k] += - ( - 1 + float_spreads[float_index-1] * float_cvgs[float_index-1] ) * swp_f_df (today, float_pay_dates[float_index], yc_name);
				float_index = float_index + 1;
			}
		}
	}
...
//			ex_lstrike[i] = ex_lfwd[i] - max_std_long * std;
	...
	if (!fix_lambda)
	{
		...
		for (i=0; i<nbexercise; i++)
		{
			...
			dff = swp_f_df (today, float_pay_dates[ex_coupon[i]+ex_sncpn[i]-1], yc_name);
	...
	for (i=1; i<nbexercise; i++)
	{
		...
		if (ex_zeta[i] > ex_zeta[i-1])
		{
			(*sig)[i] = sqrt ((ex_zeta[i] - ex_zeta[i-1]) * 2 * (*lambda)
								/ (exp (2 * (*lambda) * exer_time[i]) - exp ( 2 * (*lambda) * exer_time[i-1])));
		}
		else
		{
...
FREE_RETURN:
	...
	if(exerFixCoupon) free(exerFixCoupon);

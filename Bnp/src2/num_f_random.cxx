...
double gauss_sample(long *seed) { return inv_cumnorm_fast(uniform(seed)); }
...
Err BalSampCube(double ***rand, long pathindexstart, long pathindexend,
  ...
  /* If ODD number of paths        , make the last sample as a draw of 0
   * (central symmetry) */
  if ((numb_seg % 2) == 1) {
    numb_seg -= 1;
    sample[numb_seg] = 0;
  ...
  /* Makes a set of uniformaly distributed Gaussian sample (on [.5;1] and
   * symmetric on [0; .5] ) */
  numb_seg = (int)floor(numb_seg / 2);
  dx = 0.5 / (double)numb_seg;
  ...
  for (ibr = browindexstart; ibr <= browindexend; ibr++) {
    ...
    for (k = stepindexstart; k <= stepindexend; k++) {
      ...
      /* Loop on all paths */
      for (i = 0; i < path_num; i++) {
        /* Chooses randomly the sample that will be affected (uniform choice on
         * remaining ) */
        j = random_int(path_num - 1 - i, seed);

        /* Affects the randomly selected sample to the current path (storage in
         * the cube) */
        rand[i + pathindexstart][ibr][k] = temp_samp[j];

        /* Copies the last sample value to the chosen sample (for the proper
         * uniform choice) */
        temp_samp[j] = temp_samp[path_num - 1 - i];
...
/** compute from a correlation matrix the linear coefficients necessary
    ...
    in a second step        , in order to avoid allocating a new ***double , we
write random[i] = sum(j<i) coeff[i][j]*random[j] + alpha[i][i]*gauss[i] and
compute the coeff[i][j] using the invert of a triangle matrix
...
/** compute from a correlation matrix the linear coefficients necessary
    ...
    diagonalise_symmetric_matrix (in utdiagonalise.cxx)        , which is called
...
/* -----------------------------------------------------------------------
   ...
   correlated gaussian samples        , using the eigenvalues and eigenvectors
   of correlation matrix.. We know that X=MR where M'M = C        , the
   correlation matrix and R is the random matrix. Rewriting C=PDP' and
   M=sqrt(D)P. So: gauss_cor[i]= sum(k) m[i][k]*gauss_noncor[k] Now we should
   put m[i][k]=sqrt(d[i])*p[i][k] and this should be the value in coeff. A
   modification to the original programme below is needed as we cannot put the
   new  r matrix back into the old        , so we create rr. The r matrix is
   described as follows: r[brown][step] and the function will modify the
   following: r[0..nbr-1][sl..sh]
   ----------------------------------------------------------------------- */

Err correl_random_eigen(double **r, long sl, long sh, long nbr,
                        double **coeff) {
  Err err = NULL;
  long i, j, k;
  double sum;

...
Err cvm_vol_for_Levenberg_Marquardt(
    double end, double* pillarsVols, double* vol, double* gradient, int nPillars)
{
    Err     err = NULL;
    int     i;
...
/*
Err bump_correl_matrix(int nPillars, double **correlMat_source, double **correlMat_target, double
*bumpCorrel)
{
        Err err=NULL;
        int i,k;
...
correlMat_source[k][i+k] + bumpCorrel[i-1])); correlMat_target[i+k][k] = correlMat_target[k][i+k];
                }
        }
...
Err bump_correl_matrix(
    int nPillars, double** correlMat_source, double** correlMat_target, double* bumpCorrel)
{
    Err err = NULL;
    int i, k;
...
Err cvm_vol_for_Levenberg_Marquardt_for_correl(
    double end, double* bumpCorrel, double* vol, double* gradient, int nPillarsMinusOne)
{
    Err      err = NULL;
    int      i;
    double*  bump      = NULL;
    ...
    err       = bump_correl_matrix(
        cvm_static->nbOfPillars, cvm_static->pillars_correls[calDateIndex_static], correlLoc, bump);
    PositiveMatrix(correlLoc, cvm_static->nbOfPillars);

    err = cvm_set_pillars_correls(
        calDateIndex_static, cvm_static->nbOfPillars, correlLoc, cvm_static);
    if (err)
        goto FREE_RETURN;
    err = cvm_vol(calStart_static, end, vol, cvm_static);
    if (err)
    ...
    int      allow_negativeVols)
{
    ...
    err = cvm_calib_und(
        nbOptionTenors, option_tenors, nbUnderlyingTenors, underlying_tenors, market_vols, cvm);
    if (err)
        return err;

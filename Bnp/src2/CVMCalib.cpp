...
Err cvm_vol_for_Levenberg_Marquardt(double	end, 
									double	*pillarsVols, 
									double	*vol, 
									double	*gradient,
									int		nPillars)
{
	...
	else
	{
		...
		err = cvm_set_pillars_vols(calDateIndex_static, nPillars, pillarsVolsLoc, cvm_static);
...
/*
Err bump_correl_matrix(int nPillars, double **correlMat_source, double **correlMat_target, double *bumpCorrel)
{
	Err err=NULL;
	int i,k;
	...
	for(i=1;i<nPillars;++i)
	{
		...
		{
			correlMat_target[k][i+k] = DMAX(-0.9999, DMIN(0.9999 , correlMat_source[k][i+k] + bumpCorrel[i-1]));
			correlMat_target[i+k][k] = correlMat_target[k][i+k];
		}
	}
...
Err bump_correl_matrix(int nPillars, double **correlMat_source, double **correlMat_target, double *bumpCorrel)
{
	...
	for(i=0;i<nPillars;++i)
	{
		...
		{
			correlMat_target[k][i] = DMAX(-0.999, DMIN(0.999 , correlMat_source[k][i] + bumpCorrel[0]));
		}
	}
...
Err cvm_vol_for_Levenberg_Marquardt_for_correl(double	end, 
									double	*bumpCorrel, 
									double	*vol, 
									double	*gradient,
									int		nPillarsMinusOne)
{
	...
	err = bump_correl_matrix(cvm_static->nbOfPillars, cvm_static->pillars_correls[calDateIndex_static], correlLoc, bump);
	PositiveMatrix(correlLoc, cvm_static->nbOfPillars);

	err = cvm_set_pillars_correls(calDateIndex_static, cvm_static->nbOfPillars, correlLoc, cvm_static);
	if(err) goto FREE_RETURN;
	err = cvm_vol(calStart_static, end, vol, cvm_static);
	if(err) goto FREE_RETURN;
	*vol = *vol * 10000.0;
...
FREE_RETURN:
	if(correlLoc) 
	{
		free_dmatrix(correlLoc, 0, cvm_static->nbOfPillars-1, 0, cvm_static->nbOfPillars-1);
		correlLoc = NULL;
	}
...
Err cvm_calib_pillarsVols(cvm_str *cvm)
{
	...
	for(calDateIndex=0;calDateIndex<cvm->nbOfOptionTenors;++calDateIndex)
	{
		...
		err = add_tenor(cvm->spot, cvm->option_tenors[calDateIndex], MODIFIED_SUCCEEDING, &start);
		if(err) goto FREE_RETURN;
		calStart_static = start;

		for(i=0;i<cvm->nbOfUnderlyingTenors;++i)
		{
			err = add_tenor(start, cvm->underlying_tenors[i], NO_BUSDAY_CONVENTION, &end);
			if(err) goto FREE_RETURN;
			ends[i+1] = end;
		...
		err  = levenberg_marquardt_select(
								...
								market_vols, 
								weights, 
								cvm->nbOfUnderlyingTenors,
								pillarsVols,  
								use_param,
								cvm->nbOfPillars,
								nbr_iter,
								cvm_vol_for_Levenberg_Marquardt,
								&fitting_error);
		if(err) goto FREE_RETURN;
	}
...
FREE_RETURN:
	...
	if(ends) free_dvector(ends, 1, cvm->nbOfUnderlyingTenors);
	if(market_vols) free_dvector(market_vols, 1, cvm->nbOfUnderlyingTenors);
	if(model_vols) free_dvector(model_vols, 1, cvm->nbOfUnderlyingTenors);
	if(pillarsVols) free_dvector(pillarsVols, 1, cvm->nbOfPillars);
	if(weights) free_dvector(weights, 1, cvm->nbOfUnderlyingTenors);
	if(use_param) free_lngvector(use_param, 1, cvm->nbOfUnderlyingTenors);
	...
	for(calDateIndex=0;calDateIndex<cvm->nbOfOptionTenors;++calDateIndex)
	{
		...
		if((fabs(fitting_error) > 1e-10)&&(calibcorrel==0))
		{
			err  = levenberg_marquardt_select(
								...
								cvm->nbOfPillars-1,
								nbr_iter,
								cvm_vol_for_Levenberg_Marquardt_for_correl,
								&fitting_error);
			calibcorrel = 1;
...
FREE_RETURN:
	...
	if(pillarsVols) free_dvector(pillarsVols, 1, cvm->nbOfPillars);
	if(weights) free_dvector(weights, 1, cvm->nbOfUnderlyingTenors);
	if(use_param) free_lngvector(use_param, 1, cvm->nbOfPillars);

	return err;
}


Err cvm_calib_und(int nbOptionTenors, char **option_tenors, int nbUnderlyingTenors, char **underlying_tenors, double **market_vols, cvm_str *cvm)
{
	Err err = NULL;
	int i, j;
	...
	cvm->option_tenors = (char256*) calloc(cvm->nbOfOptionTenors, sizeof(char256));
	for(i=0;i<cvm->nbOfOptionTenors;++i)
	{
		strcpy(cvm->option_tenors[i], option_tenors[i]);
	}

	cvm->underlying_tenors = (char256*) calloc(cvm->nbOfUnderlyingTenors, sizeof(char256));
	for(i=0;i<cvm->nbOfUnderlyingTenors;++i)
	{
		strcpy(cvm->underlying_tenors[i], underlying_tenors[i]);
	}

	cvm->market_vols = dmatrix(0, cvm->nbOfOptionTenors-1, 0, cvm->nbOfUnderlyingTenors-1);
	cvm->model_vols = dmatrix(0, cvm->nbOfOptionTenors-1, 0, cvm->nbOfUnderlyingTenors-1);
	for(i=0;i<cvm->nbOfOptionTenors;++i)
	{
...
char *SrtCalibCVMUnd(
					...
					int nbOptionTenors, 
					char **option_tenors, 
					int nbUnderlyingTenors, 
					char **underlying_tenors, 
					double **market_vols, 
					...
					)
{
	...
	err = cvm_calib_und(nbOptionTenors, option_tenors, 
						nbUnderlyingTenors, underlying_tenors, 
						market_vols, cvm);
	if(err) return err;
	...
	if(vol_dates) free_dvector(vol_dates, 0, nbOptionTenors-1);
	if(pillars_vols) free_dmatrix(pillars_vols, 0, nbOptionTenors-1, 0, nbOfPillars-1);

...
int get_grf_und_index(String s)
{
    ...
    while (_grfn_und_list[i] != NULL) 
	{
        ...
        if (strcmp(_grfn_und_list[i], s) == 0) return (i);
        i++;
...
/*..Main Body..*/
    ...
    /* Check allocation */
    if (_grfn_und_list[i] == NULL) return (-1);
...
/*We start getting the underlying names at index 1 NOT 0 ( the domestic one is in position 0)*/
    ...
    while (1) 
	{
        ...
        if (s == NULL) 
		{
            ...
            /* We have reached the end of the list of names : make it a blank one (not to loose address) */
            strcpy(list[i],"");
            break;
        }
        else 

...
double h22(double x) {
  double result =
      -(3 * PolyHermite5(x) + 6 * PolyHermite3(x) + 2 * PolyHermite1(x)) / 384;
  return result;
}
...
double h112(double x) {
  double result =
      (14 * PolyHermite5(x) + 37 * PolyHermite3(x) + 8 * PolyHermite1(x)) / 288;
  return result;
}
...
double HermitePolynome(int n, double x) {
  ...
  if (n == 0 || n == 1)
    ...
    result =
        x * HermitePolynome(n - 1, x) - (n - 1) * HermitePolynome(n - 2, x);
...

{
        ...
        SimulationCIR(const BasicParameters & basicParameters0        , const
...
Polynom *ProductPolynoms(Polynom *P, Polynom *Q) {
  ...
  if ((Q->degree == 0 && P->coef[0] == 0) ||
      (Q->degree == 0 && P->coef[0] == 0)) {
...

    {
      if (P->degree < Q->degree) {
        ...
        if (maxPQ->degree < productDegree) {
          for (iPolynom = maxDegree + 1; iPolynom <= productDegree;
               iPolynom++) {
            minPQ->coef[iPolynom] = 0;
    ...
    for (iPolynom = 0; iPolynom <= maxDegree; iPolynom++) {
      ...
      for (iConvol = 0; iConvol <= iPolynom; iPolynom++)
        res->coef[iPolynom] +=
            minPQ->coef[iConvol] * maxPQ->coef[iPolynom - iConvol];
    }
  }
...
Sum.cxxoef[k]=P.cxxoef[k]+Q.cxxoef[k];
...
for(iSum=0; iSum<=Product.Degree; iSum++)

        {
                ...
                Product.cxxoef[iSum]=P.cxxoef[iConvol] * Q.cxxoef[iSum-iConvol];
        }
...
Err lgmSV_mc_balsam(
  ...
  double *sum_payoff = NULL, *sum_payoff2 = NULL, *path_payoff = NULL,
         *res_evt = NULL, **matrix = NULL, *matrixi = NULL, yInterpolated,

         *muf = NULL, *sigf = NULL, *sigf2 = NULL, *sigf3 = NULL, *sigf4 = NULL,
         *sigf5 = NULL, *sigf6 = NULL, *mupsi = NULL, *mupsi2 = NULL,
         *sigpsi = NULL, *muv1 = NULL, *muv2 = NULL, *sigv = NULL,
         *sigv2 = NULL, *sigv3 = NULL, *sigv4 = NULL, *sigv5 = NULL,
         *sigv6 = NULL, *sigv7 = NULL, *sigv8 = NULL, *sigv9 = NULL,
         *sigv10 = NULL, *sigv11 = NULL, *sigv12 = NULL, *sigv13 = NULL,
         *coef1 = NULL, *coef2 = NULL,

         *b1 = NULL, *b2 = NULL, *b3 = NULL,

         *c2 = NULL, *c3 = NULL, *c1 = NULL, *c4 = NULL, *c5 = NULL, *c6 = NULL,
         *c7 = NULL,

         *n0 = NULL, *n2 = NULL, *n4 = NULL, *d1 = NULL, **gamma = NULL,
         **cumulant1 = NULL, **cumulant2 = NULL, **cumulant3 = NULL, *nu = NULL,
         *xAux = NULL, **coefBesselApprox1 = NULL, **coefBesselApprox2 = NULL,
         **xValues = NULL, **TabulatedBesselFunction = NULL,
         **TabulatedBesselFunctionSecondDerivate = NULL,
         **ModifiedBesselFunctionValues = NULL, ***save_values = NULL;
  ...
  int NbCumulants = 7;
  double v0, sqrtV, sumV, difV, x, argGamma1, argGamma2, besselApproxNum,
      besselApproxDen, SimpleBesselFunction, BigBesselFunction;
  ...
  case 1: {
    for (j = 1; j < iNbTime; j++) {
      ...
      if (fabs(dLambdaEps[j]) > 1.0E-08) {
        ...
        sigv[j] =
            dAlpha[j] * sqrt(0.5 / dLambdaEps[j] * (1.0 - muv1[j] * muv1[j]));
        sigv[j] *= sigv[j];
      } else {
        muv1[j] = 1.0;
        muv2[j] = dLvlEps[j] * dt;
...

  {
    ...
    if (!muf || !sigf2 || !sigf3 || !sigf4 || !sigf5 || !sigf6 || !sigv2 ||
        !sigv3 || !mupsi || !mupsi2) {
      err = "Memory allocation failure in lgmSV_mc_balsam";
      goto FREE_RETURN;
    }
    ...
    for (j = 1; j < iNbTime; j++) {
      ...
      sigf2[j] = coef2[j] * dSigma[j] * 0.25 * dt *
                 (dLvlEps[j] - dAlpha[j] * dAlpha[j] / 4.0) * sqdt;
      sigf3[j] =
          coef1[j] * dSigma[j] * (1.0 - 0.25 * dLambdaEps[j] * dt) * sqdt;
      sigf4[j] = coef1[j] * dSigma[j] * 0.25 * dt *
                 (dLvlEps[j] - dAlpha[j] * dAlpha[j] / 4.0) * sqdt;
  ...
  case 3: {
    ...
    if (!muf || !sigf2 || !sigf3 || !sigf4 || !sigf5 || !sigf6 || !sigv2 ||
        !sigv3 || !sigv4 || !sigv5 || !sigv6 || !sigv7 || !sigv8 || !sigv9 ||
        !sigv10 || !sigv11 || !sigv12 || !sigv13 || !mupsi || !mupsi2) {
      err = "Memory allocation failure in lgmSV_mc_balsam";
      goto FREE_RETURN;
    }
    ...
    for (j = 1; j < iNbTime; j++) {
      ...
      sigv6[j] =
          (-11 * pow(dAlpha[j], 6) + 56 * pow(dAlpha[j], 4) * dLvlEps[j] -
           48 * pow(dAlpha[j] * dLvlEps[j], 2)) *
          dt * dt / (dAlpha[j] * 1536);
      sigv7[j] = (24 * pow(dAlpha[j], 4) * dLambdaEps[j] -
                  96 * pow(dAlpha[j], 2) * dLambdaEps[j] * dLvlEps[j]) *
                 dt * dt / (dAlpha[j] * 1536);
      sigv8[j] = 464 * pow(dAlpha[j] * dLambdaEps[j], 2) * dt * dt /
                 (dAlpha[j] * 1536);

      // w1: 0 1

      sigv4[j] =
          ((pow(dAlpha[j], 4) - 4.0 * pow(dAlpha[j], 2) * dLvlEps[j]) / 96.0) *
          dt;
      ...
      sigv9[j] =
          (1 / (36864 * pow(dAlpha[j], 2))) *
          (576 * dAlpha[j] * dAlpha[j] * dLvlEps[j] * dLvlEps[j] * dLvlEps[j] +
           364 * dLvlEps[j] * pow(dLvlEps[j], 6) -
           944 * dLvlEps[j] * dLvlEps[j] * pow(dAlpha[j], 4) -
           41 * pow(dAlpha[j], 8)) *
          dt * dt;
      sigv10[j] = (1 / (36864 * pow(dAlpha[j], 2))) *
                  (-576 * pow(dAlpha[j] * dLvlEps[j], 2) * dLambdaEps[j] -
                   132 * dLambdaEps[j] * pow(dAlpha[j], 6) +
                   672 * dLvlEps[j] * dLambdaEps[j] * pow(dAlpha[j], 4)) *
                  dt * dt;
      sigv11[j] = (1 / (36864 * pow(dAlpha[j], 2))) *
                  (1728 * dLvlEps[j] * pow(dAlpha[j] * dLambdaEps[j], 2) +
                   1104 * pow(dLambdaEps[j] * dAlpha[j] * dAlpha[j], 2)) *
                  dt * dt;
      sigv12[j] = (1 / (36864 * pow(dAlpha[j], 2))) *
                  (-6336 * dAlpha[j] * dAlpha[j] * pow(dLambdaEps[j], 3) -
                   3072 * dLvlEps[j] * pow(dLambdaEps[j], 3)) *
                  dt * dt;
      sigv13[j] = (1 / (36864 * pow(dAlpha[j], 2))) *
                  (3072 * pow(dLambdaEps[j], 4)) * dt * dt;
    }
  ...
  case 4: {
    ...
    if (!muf || !sigf2 || !sigf3 || !sigf4 || !sigf5 || !sigf6 || !sigv2 ||
        !sigv3 || !mupsi || !b1 || !b2 || !b3 || !c1 || !c2 || !c3 || !c4 ||
        !c5 || !c6 || !c7 || !nu || !xAux) {
      err = "Memory allocation failure in lgmSV_mc_balsam";
      goto FREE_RETURN;
    }
    ...
    for (j = 1; j < iNbTime; j++) {
      ...
      b1[j] = (-1 + exp(-dLambdaEps[j] * dt) +
               0.5 * dLambdaEps[j] * dt * (1 + exp(-dLambdaEps[j] * dt)));
      b2[j] = dAlpha[j] * dAlpha[j] /
              ((1 - exp(-dLambdaEps[j] * dt)) * dLambdaEps[j] * dLambdaEps[j]);
      b3[j] = 4 * exp(-0.5 * dLambdaEps[j] * dt) /
              (dLambdaEps[j] * pow(-1 + exp(-dLambdaEps[j] * dt), 2));
      ...
      c5[j] = b1[j] * b2[j];
      c6[j] = b1[j] * b3[j];
      c7[j] = -(2 * dLambdaEps[j] * dt * exp(-dLambdaEps[j] * dt) - 1 +
                exp(-2 * dLambdaEps[j] * dt)) /
              (dLambdaEps[j] * pow(1 - exp(-dLambdaEps[j] * dt), 2));
      nu[j] = 2 * dLvlEps[j] / pow(dAlpha[j], 2) - 1;
      xAux[j] = 4 * dLambdaEps[j] * exp(-0.5 * dLambdaEps[j] * dt) /
                (pow(dAlpha[j], 2) * (1 - exp(-dLambdaEps[j] * dt)));
    }
  ...
  case 5: {
    ...
    if (!muf || !sigf2 || !sigf3 || !sigf4 || !sigf5 || !sigf6 || !sigv2 ||
        !sigv3 || !mupsi || !b1 || !b2 || !b3 || !c1 || !c2 || !c3 || !c4 ||
        !c5 || !c6 || !c7 || !nu || !xAux || !coefBesselApprox1 ||
        !coefBesselApprox2) {
      err = "Memory allocation failure in lgmSV_mc_balsam";
      goto FREE_RETURN;
    }
    ...
    for (j = 1; j < iNbTime; j++) {
      ...
      for (iBesselApprox = 0; iBesselApprox < besselApproxOrder;
           iBesselApprox++)

      {
        argGamma1 = nu[j] + 1 + iBesselApprox + 1;
        coefBesselApprox1[j][iBesselApprox] =
            pow(0.5, nu[j] + 1) * pow(0.25, iBesselApprox) /
            (fact(iBesselApprox) * gamma_(&argGamma1));
        argGamma2 = nu[j] + iBesselApprox + 1;
        coefBesselApprox2[j][iBesselApprox] =
            pow(0.5, nu[j]) * pow(0.25, iBesselApprox) /
            (fact(iBesselApprox) * gamma_(&argGamma2));
      }
    }
  ...
  case 6: {
    ...
    if (xValues)
      free_dmatrix(xValues, 0, iNbTime - 1, 1, NbTabCount + 1);
    xValues = 0;
    if (TabulatedBesselFunction)
      free_dmatrix(TabulatedBesselFunction, 0, iNbTime - 1, 1, NbTabCount + 1);
    TabulatedBesselFunction = 0;
    if (TabulatedBesselFunctionSecondDerivate)
      free_dmatrix(TabulatedBesselFunctionSecondDerivate, 0, iNbTime - 1, 1,
                   NbTabCount + 1);
    TabulatedBesselFunctionSecondDerivate = 0;

    xValues = dmatrix(0, iNbTime - 1, 1, NbTabCount + 1);
    TabulatedBesselFunction = dmatrix(0, iNbTime - 1, 1, NbTabCount + 1);
    TabulatedBesselFunctionSecondDerivate =
        dmatrix(0, iNbTime - 1, 1, NbTabCount + 1);

    if (!muf || !sigf2 || !sigf3 || !sigf4 || !sigf5 || !sigf6 || !sigv2 ||
        !sigv3 || !sigv4 || !sigv5 || !sigv6 || !sigv7 || !sigv8 || !sigv9 ||
        !sigv10 || !sigv11 || !sigv12 || !sigv13 || !mupsi || !mupsi2 || !b1 ||
        !b2 || !b3 || !c1 || !c2 || !c3 || !c4 || !c5 || !c6 || !c7 || !nu ||
        !xAux)
    ...
    for (j = 1; j < iNbTime; j++) {
      ...
      if (nu[j] >= 0)
        ModifiedBesselFunctionValues =
            dmatrix(0, iNbTime - 1, 0, sizeNuMain + 1);
      if (nu[j] < 0)
      ...
      for (iTabCount = 1; iTabCount <= NbTabCount + 1; iTabCount++) {
        ...
        xValues[j][iTabCount] =
            xInfBound +
            ((xSupBound - xInfBound) / NbTabCount) * (iTabCount - 1);
        BesselArrays(nu[j], xValues[j][iTabCount],
                     ModifiedBesselFunctionValues[j]);

        if (nu[j] >= 0)
          TabulatedBesselFunction[j][iTabCount] =
              ModifiedBesselFunctionValues[j][sizeNuMain + 1] /
              ModifiedBesselFunctionValues[j][sizeNuMain];
      ...
      spline(xValues[j], TabulatedBesselFunction[j], NbTabCount + 1, 1.0e30,
             1.0e30, TabulatedBesselFunctionSecondDerivate[j]);

      if (nu[j] >= 0)
        free_dmatrix(ModifiedBesselFunctionValues, 0, iNbTime - 1, 0,
                     sizeNuMain + 1);
      if (nu[j] < 0)
        free_dmatrix(ModifiedBesselFunctionValues, 0, iNbTime - 1, 0,
                     sizeNuMain);
  ...
  for (i = 0; i < iNumPaths; i++) {
    ...
    if (func_parm_tab[0] && EvalEvent[0]) {
      /* Event at time 0.0 */
      err = payoff_func(0, dDate[0], dTime[0], func_parm_tab[0], ft, psi, v,
      ...
      if (do_optimisation) {
        mceb_fill_savevalues_from_GRFN(save_values[evtindex], res_evt, i, df,
                                       params);
      }
    ...
    for (j = 1; stop_path == 0 && j < iNbTime; j++) {
...

      {
        ...
        v0 = v;
        v = muv1[j] + muv2[j] * v0 -
            (sigv4[j] / v0 + sigv5[j] + sigv9[j] / (v0 * v0) + sigv10[j] / v0 +
             sigv11[j] + sigv12[j] * v0 + sigv13[j] * v0 * v0) *
                dt;
        v += (sigv[j] * sqv + sigv2[j] / sqv + sigv3[j] * brow2 +
              sqdt * (sigv6[j] / (v0 * sqv) + sigv7[j] / sqv + sigv8[j] * sqv) +
              (sqdt * (sigv4[j] / v0 + sigv5[j]) +
               dt * (sigv9[j] / (v0 * v0) + sigv10[j] / v0 + sigv11[j] +
                     sigv12[j] * v0 + sigv13[j] * v0 * v0)) *
                  brow2) *
             brow2;
        ...
        // Vol Projection + Expectation of Integral Volatility as a Bessel
        // Function
      ...
      case 4: {
        ...
        x = xAux[j] * sqrtV;
        SimpleBesselFunction = (ModifiedBesselFunction(nu[j] + 1, x) /
                                    ModifiedBesselFunction(nu[j], x) +
                                nu[j] / x);
        BigBesselFunction =
            c5[j] + c6[j] * sqrtV * SimpleBesselFunction + c7[j] * sumV;

        ft += c1[j] + c2[j] * difV + BigBesselFunction * c3[j] +
              c4[j] * sqrt(BigBesselFunction) * brow1;
      ...
      case 5: {
        ...
        for (iBesselApprox = 0; iBesselApprox < besselApproxOrder;
          ...
          besselApproxNum += coefBesselApprox1[j][iBesselApprox] *
                             pow(x, nu[j] + 1 + 2 * iBesselApprox);
          besselApproxDen += coefBesselApprox2[j][iBesselApprox] *
                             pow(x, nu[j] + 2 * iBesselApprox);
        }
        SimpleBesselFunction = (ModifiedBesselFunction(nu[j] + 1, x) /
                                    ModifiedBesselFunction(nu[j], x) +
                                nu[j] / x);
        ...
        // order 3  Volatility Projection + Bessel Approximation by cubic
        // splines
      case 6: {
        ...
        x = xAux[j] * sqrtV;
        splint(xValues[j], TabulatedBesselFunction[j],
               TabulatedBesselFunctionSecondDerivate[j], NbTabCount + 1, x,
               &yInterpolated);
      ...
      if (EvalEvent[j]) {
        ...
        err = payoff_func(i, dDate[j], dTime[j], func_parm_tab[j], ft, psi, v,
                          iNbProduct, res_evt, &stop_path);

        if (err)
          goto FREE_RETURN;

        df = exp(dff_star[evtindex] + gam_star[evtindex] * ft +
                 gam2_star[evtindex] * psi);
  ...
  if (do_optimisation) {
    ...
    if (matrix)
      free_dmatrix(matrix, 0, iNumPaths - 1, 0, 2 * (iNbTime - 1) - 1);
    matrix = NULL;
...
FREE_RETURN:
  ...
  if (coefBesselApprox1)
    free_dmatrix(coefBesselApprox1, 0, iNbTime - 1, 0, besselApproxOrder - 1);
  coefBesselApprox1 = 0;
  if (coefBesselApprox2)
    free_dmatrix(coefBesselApprox2, 0, iNbTime - 1, 0, besselApproxOrder - 1);
  coefBesselApprox2 = 0;

  if (cumulant1)
    free_dmatrix(cumulant1, 0, iNbTime - 1, 0, NbCumulants - 1);
  cumulant1 = 0;
  if (cumulant2)
    free_dmatrix(cumulant2, 0, iNbTime - 1, 0, NbCumulants - 1);
  cumulant2 = 0;
  if (cumulant3)
    free_dmatrix(cumulant3, 0, iNbTime - 1, 0, NbCumulants - 1);
  cumulant3 = 0;
  ...
  if (gamma)
    free_dmatrix(gamma, 0, iNbTime - 1, 0, NbCumulants - 1);
  gamma = 0;
  ...
  if (save_values)
    free_f3tensor(save_values, 0, iNbEvent - 1, 0,
                  params->iNbIndex + params->iHasNumeraire + params->iHasFees,
                  0, iNumPaths - 1);
...
Err lgmSV2F_mc_balsam(
    ...
    /*	Model data Information	*/
    double dLambdaX1, double dLambdaX2,

    double *dSigma, double *dAlphaLGM, double *dRhoLGM,
    ...
    Err (*payoff_func)(long path_index, double evt_date, double evt_time,
                       ...
                       /* Model data	*/
                       double ft1, double ft2, double phi1, double phi2,
                       double phi12, double v,
  ...
  double *sum_payoff = NULL, *sum_payoff2 = NULL, *path_payoff = NULL,
         ...
         *sigf1 = NULL, *sigf2 = NULL, *sigpsi1 = NULL, *sigpsi2 = NULL,
         *sigpsi12 = NULL, *muv1 = NULL, *muv2 = NULL, *sigv = NULL,
         *coef1 = NULL, *coef2 = NULL, *coef3 = NULL, *coef4 = NULL,
         *coef5 = NULL,
  ...
  if (!matrix || !path_payoff || !sum_payoff || !sum_payoff2 || !res_evt ||
      !sigf1 || !sigf2 || !sigpsi1 || !sigpsi2 || !sigpsi12 || !muv1 || !muv2 ||
      !sigv || !coef1 || !coef2 || !coef3 || !coef4 || !coef5) {
    err = "Memory allocation failure in lgmSV_mc_balsam";
    goto FREE_RETURN;
  }
  ...
  if (do_optimisation) {
    save_values =
        f3tensor(0, iNbEvent - 1, 0,
                 params->iNbIndex + params->iHasNumeraire + params->iHasFees, 0,
                 iNumPaths - 1);
  ...
  for (i = 0; i < iNumPaths; i++) {
    ...
    if (func_parm_tab[0] && EvalEvent[0]) {
      /* Event at time 0.0 */
      err = payoff_func(0, dDate[0], dTime[0], func_parm_tab[0], ft1, ft2, psi1,
                        psi2, psi12, v, iNbProduct, res_evt, &stop_path);

      if (err)
        goto FREE_RETURN;

      df = exp(dff_star[0] + gam1_star[0] * ft1 + gam2_star[0] * ft2 +
               gam1_2_star[0] * psi1 + gam2_2_star[0] * psi2 +
               gam12_star[0] * psi12);
    ...
    for (j = 1; stop_path == 0 && j < iNbTime; j++) {
      brow1 = matrixi[m];
      brow2 = coef1[j] * brow1 + coef2[j] * matrixi[m + 1];
      brow3 = coef3[j] * brow1 + coef4[j] * matrixi[m + 1] +
              coef5[j] * matrixi[m + 2];
      ...
      if (EvalEvent[j]) {
        /* Modification of the Payoff at t */
        err =
            payoff_func(i, dDate[j], dTime[j], func_parm_tab[j], ft1, ft2, psi1,
                        psi2, psi12, v, iNbProduct, res_evt, &stop_path);

        if (err)
          goto FREE_RETURN;

        df = exp(dff_star[evtindex] + gam1_star[evtindex] * ft1 +
                 gam2_star[evtindex] * ft2 + gam1_2_star[evtindex] * psi1 +
                 gam2_2_star[evtindex] * psi2 + gam12_star[evtindex] * psi12);
...
FREE_RETURN:
  ...
  if (sigf1)
    free_dvector(sigf1, 0, iNbTime - 1);
  if (sigf2)
    free_dvector(sigf2, 0, iNbTime - 1);
  if (sigpsi1)
    free_dvector(sigpsi1, 0, iNbTime - 1);
  if (sigpsi2)
    free_dvector(sigpsi2, 0, iNbTime - 1);
  if (sigpsi12)
    free_dvector(sigpsi12, 0, iNbTime - 1);
  if (muv1)
    free_dvector(muv1, 0, iNbTime - 1);
  if (muv2)
    free_dvector(muv2, 0, iNbTime - 1);
  if (sigv)
    free_dvector(sigv, 0, iNbTime - 1);
  if (coef1)
    free_dvector(coef1, 0, iNbTime - 1);
  if (coef2)
    free_dvector(coef2, 0, iNbTime - 1);
  if (coef3)
    free_dvector(coef3, 0, iNbTime - 1);
  if (coef4)
    free_dvector(coef4, 0, iNbTime - 1);
  if (coef5)
    free_dvector(coef5, 0, iNbTime - 1);
...
Err lgmSV_mc_balsam_rev(
    ...
    Err (*payoff_adjust_function)(long event_index, long npaths, long nprod,
                                  double ***saved_values, void *func_parm,
  ...
  clock_t time1, time2;
  long i, j, k;
  double **payoff = NULL, *res_evt = NULL, *init_gauss = NULL, *ft = NULL,
         *v = NULL, *psi = NULL, **matrix = NULL, *matrixk = NULL,
         ***save_values = NULL, **save_valuesj = NULL;
  ...
  if (!payoff || !res_evt || !init_gauss || !ft || !v || !psi || !matrix ||
      !stop_path) {
    err = "Memory allocation failure in lgmSV_mc_balsam";
    goto FREE_RETURN;
  }
  ...
  if (func_parm_tab[0] && EvalEvent[0]) {
    err = payoff_func(0, dDate[0], dTime[0], func_parm_tab[0], ft[0], psi[0],
                      v[0], iNbProduct, res_evt, &(stop_path[0]));
  ...
  for (j = 1; j < iNbTime; j++) {
    ...
    for (i = 0; i < iNumPaths; i++) {
      if (!stop_path[i]) {
        ...
        if (EvalEvent[j]) {
          /* Modification of the Payoff at t */
          err = payoff_func(i, dDate[j], dTime[j], func_parm_tab[j], ft[i],
                            psi[i], v[i], iNbProduct, res_evt, &(stop_path[i]));

          if (err)
            goto FREE_RETURN;

          if (dff_star) {
            df = exp(dff_star[evtindex] + gam_star[evtindex] * ft[i] +
                     gam2_star[evtindex] * psi[i]);
          ...
          if (do_optimisation) {
            mceb_fill_savevalues_from_GRFN(save_valuesj, res_evt, i, df,
                                           params);
          }
        }
      }
    ...
    if (EvalEvent[j] && payoff_adjust_function) {
      err = payoff_adjust_function(evtindex, iNumPaths, iNbProduct, save_values,
                                   func_parm_tab[j], params);
...
FREE_RETURN:
  ...
  if (payoff)
    free_dmatrix(payoff, 0, iNumPaths - 1, 0, iNbProduct - 1);

  if (res_evt)
    free_dvector(res_evt, 0, iNbProduct - 1);

  if (init_gauss)
    free_dvector(init_gauss, 0, iNumPaths - 1);

  if (ft)
    free_dvector(ft, 0, iNumPaths - 1);

  if (v)
    free_dvector(v, 0, iNumPaths - 1);

  if (psi)
    free_dvector(psi, 0, iNumPaths - 1);

  if (matrix)
    free_dmatrix(matrix, 0, 1, 0, iNumPaths - 1);

  if (stop_path)
    free_ivector(stop_path, 0, iNumPaths - 1);
...
Err lgmSV2F_mc_balsam_rev(
  ...
  clock_t time1, time2;
  long i, j, k;
  double **payoff = NULL, *res_evt = NULL, *init_gauss = NULL, *ft1 = NULL,
         *ft2 = NULL, *v = NULL, *psi1 = NULL, *psi2 = NULL, *psi12 = NULL,
         **matrix = NULL, *matrixk = NULL, ***save_values = NULL,
         **save_valuesj = NULL;
  ...
  if (!payoff || !res_evt || !init_gauss || !ft1 || !ft2 || !v || !psi1 ||
      !psi2 || !psi12 || !matrix || !stop_path) {
    err = "Memory allocation failure in lgmSV_mc_balsam";
    goto FREE_RETURN;
  }
  ...
  /* Event at time 0.0 */
  if (func_parm_tab[0] && EvalEvent[0]) {
    err = payoff_func(0, dDate[0], dTime[0], func_parm_tab[0], ft1[0], ft2[0],
                      psi1[0], psi2[0], psi12[0], v[0], iNbProduct, res_evt,
                      &(stop_path[0]));
    ...
    if (dff_star) {
      df = exp(dff_star[0] + gam1_star[0] * ft1[0] + gam2_star[0] * ft2[0] +
               gam1_2_star[0] * psi1[0] + gam2_2_star[0] * psi2[0] +
               gam12_star[0] * psi12[0]);
    } else {
      df = 1.0;
    }
    ...
    if (payoff_adjust_function) {
      err = payoff_adjust_function(0, iNumPaths, iNbProduct, save_values,
                                   func_parm_tab[0], params);
  ...
  for (j = 1; j < iNbTime; j++) {
    ...
    for (i = 0; i < iNumPaths; i++) {
      if (!stop_path[i]) {
        ...
        if (EvalEvent[j]) {
          /* Modification of the Payoff at t */
          err = payoff_func(i, dDate[j], dTime[j], func_parm_tab[j], ft1[i],
                            ft2[i], psi1[i], psi2[i], psi12[i], v[i],
                            iNbProduct, res_evt, &(stop_path[i]));

          if (err)
            goto FREE_RETURN;

          if (dff_star) {
            df = exp(dff_star[evtindex] + gam1_star[evtindex] * ft1[i] +
                     gam2_star[evtindex] * ft2[i] +
                     gam1_2_star[evtindex] * psi1[i] +
                     gam2_2_star[evtindex] * psi2[i] +
                     gam12_star[evtindex] * psi12[i]);
          } else {
            df = 1.0;
          }
        ...
        if (do_optimisation) {
          mceb_fill_savevalues_from_GRFN(save_valuesj, res_evt, i, df, params);
        }
      }
    }
...
FREE_RETURN:
  ...
  if (ft1)
    free_dvector(ft1, 0, iNumPaths - 1);

  if (ft2)
    free_dvector(ft2, 0, iNumPaths - 1);

  if (v)
    free_dvector(v, 0, iNumPaths - 1);

  if (psi1)
    free_dvector(psi1, 0, iNumPaths - 1);

  if (psi2)
    free_dvector(psi2, 0, iNumPaths - 1);

  if (psi12)
    free_dvector(psi12, 0, iNumPaths - 1);

  if (matrix)
    free_dmatrix(matrix, 0, 2, 0, iNumPaths - 1);
...
Err lgmSV2F_mc_balsam_optim_mem(
  ...
  clock_t time1, time2;
  long i, j, k;
  double *sum_payoff = NULL, *sum_payoff2 = NULL, *res_evt = NULL,
         *path_payoff = NULL, *init_gauss = NULL, **ft1 = NULL, **ft2 = NULL,
         **v = NULL, **psi1 = NULL, **psi2 = NULL, **psi12 = NULL,
         **matrix = NULL, *matrixk = NULL, ***save_values = NULL,
         **save_valuesj = NULL;
  ...
  if (!init_gauss || !matrix || !sum_payoff | !sum_payoff2 || !res_evt ||
      !path_payoff || !ft1 || !ft2 || !v || !psi1 || !psi2 || !psi12 ||
      !index_event) {
  ...
  for (j = 1; j < iNbTime; j++) {
    ...
    for (i = 0; i < iNumPaths; i++) {
      ...
      case 1: {
        ...
        if (log_drift > 0.0) {
          ...
          v[evtindex][i] =
              log_drift * exp(-0.5 * log_var + sqrt(log_var) * brow3);
        } else {
          v[evtindex][i] = 0.0;
        }
    ...
    if (EvalEvent[j]) {
      ...
      if (evtindex < iNbEvent - 1) {
        ...
        memcpy(psi1[evtindex + 1], psi1[evtindex], iNumPaths * sizeof(double));
        memcpy(psi2[evtindex + 1], psi2[evtindex], iNumPaths * sizeof(double));
        memcpy(psi12[evtindex + 1], psi12[evtindex],
               iNumPaths * sizeof(double));
        memcpy(v[evtindex + 1], v[evtindex], iNumPaths * sizeof(double));
      }
  ...
  /* Initialisation time display */
  time2 = clock();
  smessage("Phase 1 -Path Generation        , time in sec: %.2f",
           (double)(time2 - time1) / CLOCKS_PER_SEC);
  ...
  for (i = 0; i < iNumPaths; i++) {
    ...
    if (func_parm_tab[0] && EvalEvent[0]) {
      /* Event at time 0.0 */
      err = payoff_func(
          i, dDate[index_event[evtindex]], dTime[index_event[evtindex]],
          func_parm_tab[index_event[evtindex]], ft1[evtindex][i],
          ft2[evtindex][i], psi1[evtindex][i], psi2[evtindex][i],
          psi12[evtindex][i], v[evtindex][i], iNbProduct, res_evt, &stop_path);

      if (err)
        goto FREE_RETURN;

      df = exp(dff_star[0] + gam1_star[0] * ft1[evtindex][i] +
    ...
    for (evtindex; stop_path == 0 && evtindex < iNbEvent; evtindex++) {
      ...
      df = exp(dff_star[evtindex] + gam1_star[evtindex] * ft1[evtindex][i] +
...
FREE_RETURN:
  ...
  if (ft1)
    free_dmatrix(ft1, 0, iNbEvent - 1, 0, iNumPaths - 1);

  if (ft2)
    free_dmatrix(ft2, 0, iNbEvent - 1, 0, iNumPaths - 1);

  if (v)
    free_dmatrix(v, 0, iNbEvent - 1, 0, iNumPaths - 1);

  if (psi1)
    free_dmatrix(psi1, 0, iNbEvent - 1, 0, iNumPaths - 1);

  if (psi2)
    free_dmatrix(psi2, 0, iNbEvent - 1, 0, iNumPaths - 1);

  if (psi12)
    free_dmatrix(psi12, 0, iNbEvent - 1, 0, iNumPaths - 1);
...
Err lgmSV_mc(/*	Time Information  */
             ...
             /*	Model data Information	*/
             double dLambdaX, double *Sig, double *SigPsi, double *dAlpha,
             double *dLambdaEps, double *dLvlEps, double *dRho, double *dRho2,
             ...
             /*	Payoff function */
             Err (*payoff_func)(double evt_date, double evt_time,
                                void *func_parm,

                                double ft, double psi,
  ...
  if (Params.VerifExpectation = LGMSV_TRUE) {
    ...
    fprintf(stream, "Alpha	%f\n",
            dAlpha[iNbTime - 2] /
                sqrt(dTime[iNbTime - 1] - dTime[iNbTime - 2]));
  ...
  for (i = 0; i < iNumPaths; i++) {
    ...
    if (func_parm_tab[0] && EvalEvent[0]) {
      err = payoff_func(dDate[0], dTime[0], func_parm_tab[0], ft, psi,
                        iNbProduct, temp);
    ...
    for (j = 1; j < iNbTime; j++) {
      ...
      psi += SigPsi[j - 1] * v;
      v = max(v + dLvlEps[j - 1] - dLambdaEps[j - 1] * v +
                  dAlpha[j - 1] * sqv * brow2,
              0.0);

      /*	Case of evaluation events */
      if (EvalEvent[j]) {
        /* Modification of the Payoff at t */
        err = payoff_func(dDate[j], dTime[j], func_parm_tab[j], ft, psi,
                          iNbProduct, res_evt);
    ...
    if (Params.VerifExpectation = LGMSV_TRUE) {
      fprintf(stream, "%f	%f	%f	%f	%f\n", ft, v, log(psi),
              ft - ortho_fact * (v - 1.0), log(max(v, 1.0E-05)));
    }
...
Err lgmSV2F_mc(
    ...
    /*	Model data Information	*/
    double dLambdaX1, double dLambdaX2, double *Sig1, double *Sig2,
    double *SigPsi1, double *SigPsi2, double *SigPsi12,

    double *dAlpha, double *dLambdaEps, double *dLvlEps, double *dRho,
    double *dRho2,

    double *Coef_Ortho21, double *Coef_Ortho22, double *Coef_Ortho31,
    double *Coef_Ortho32, double *Coef_Ortho33,
  ...
  for (i = 0; i < iNumPaths; i++) {
    ...
    if (func_parm_tab[0] && EvalEvent[0]) {
      err = payoff_func(dDate[0], dTime[0], func_parm_tab[0], ft1, ft2, psi1,
                        psi2, psi12, iNbProduct, temp);
    ...
    for (j = 1; j < iNbTime; j++) {
      brow1 = gauss_sample(&seed);
      brow2 = Coef_Ortho21[j - 1] * brow1 +
              Coef_Ortho22[j - 1] * gauss_sample(&seed);
      brow3 = Coef_Ortho31[j - 1] * brow1 + Coef_Ortho32[j - 1] * brow2 +
              Coef_Ortho33[j - 1] * gauss_sample(&seed);
      ...
      v = max(v + dLvlEps[j - 1] - dLambdaEps[j - 1] * v +
                  dAlpha[j - 1] * sqv * brow3,
              0.0);

      /*	Case of evaluation events */
      if (EvalEvent[j]) {
        /* Modification of the Payoff at t */
        err = payoff_func(dDate[j], dTime[j], func_parm_tab[j], ft1, ft2, psi1,
                          psi2, psi12, iNbProduct, res_evt);
...
Err lgmSV_mc_cv(
    ...
    /*	Model data Information	*/
    double dLambdaX, double *Sig, double *SigPsi,
  ...
  if (!temp || !res_evt || !sum_price || !sum_2price || !temp_bs ||
      !res_evt_bs || !sum_price_bs || !sum_2price_bs || !cov_price_bs) {
  ...
  for (i = 0; i < iNumPaths; i++) {
    ...
    for (j = 1; j < iNbTime; j++) {
      ...
      if (EvalEvent[j]) {
        ...
        /* Modification of the Payoff at t */
        err = payoff_func(dDate[j], dTime[j], func_parm_tab[j], ft_bs, psi_bs,
                          iNbProduct, res_evt_bs);
  ...
  for (k = 0; k < iNbProduct; k++) {
    ...
    res[k + iNbProduct][1] = sqrt(
        (sum_2price_bs[k] - sum_price_bs[k] * sum_price_bs[k]) / iNumPaths);

    res[k + 2 * iNbProduct][0] =
        (cov_price_bs[k] - res[k][0] * res[k + iNbProduct][0]) / iNumPaths;
    res[k + 2 * iNbProduct][1] =
        res[k + iNbProduct][1] * res[k + iNbProduct][1];
  }
...
Err LGMSVOptionMC(
    ...
    long lExDate, long lEndDate, double *dStrike, int nb_strike,
    int pay_rec, /*	pay:1 rec:-1 */

    long iNumPaths, long nb_vol, int use_balsam,

    /* Output */
    double *pSwaptionPrice, double *pStd) {
  int i, j, ncpn;
  ...
  for (i = 1; i < ncpn; i++) {
    ...
    for (j = 0; j < nb_strike; j++) {
      Coupon[j][i] = -cpn_cvg[i] * pay_rec *
                     (dStrike[j] - (swp_rte - swp_cash)) *
                     swp_f_df(today, cpn_date[i], yc_name);
    }
  }
  ...
  for (j = 0; j < nb_strike; j++) {
    Coupon[j][ncpn - 1] -=
        pay_rec * swp_f_df(today, cpn_date[ncpn - 1], yc_name);
  }
  ...
  LGMSVOptionMCts(1.0 / dTau, dNbSig,     /* Term Structure of g(t) */
                  ...
                  cpn_time, cpn_date, Coupon, nb_strike,
                  yc_name, /* Yield Curve */
                  iNumPaths, nb_vol, pSwaptionPrice, pStd);
...
FREE_RETURN:
  ...
  if (dSig)
    free(dSig);
  if (Coupon)
    free_dmatrix(Coupon, 0, nb_strike - 1, 0, ncpn - 1);
...
Err LGMSVOptionMCts(
    ...
    double *CouponTime, long *CouponDate, double **Coupon, int nb_strike,
    char *cYieldCurve, /* Yield Curve */

    /* Parameter of grids */
    long iNumPaths, long nb_vol,

    /* Outputs */
    double *Price, double *Std) {
  Err err = NULL;
  int i, j, k, nbstep;
  double *time = NULL, *vol = NULL, *cpn = NULL, *lamdt = NULL, *dti = NULL,
         *beta = NULL, *volX = NULL, *volX2 = NULL, *sig = NULL, *var = NULL,
         *drift = NULL, *init_gauss = NULL, *sum = NULL, *sum2 = NULL,
         *covar = NULL;
  ...
  if (!vol || !lamdt || !nbpT || !cpn || !beta || !volX || !volX2 || !dti ||
      !sum || !sum2 || !covar || !init_gauss || !sig || !var || !drift) {
  ...
  for (i = 0; i < iNumPaths; i++) {
    for (j = 0; j < nb_strike; j++) {
      lgm_price = lgmopval_stochvol(iNbCoupon, Coupon[j], beta, var[i],
                                    dRho * drift[i], rho2, sqrho2);
...
FREE_RETURN:
  ...
  if (vol)
    free_dvector(vol, 0, endi);
  if (volX)
    free_dvector(volX, 0, endi);
  if (volX2)
    free_dvector(volX2, 0, endi);
  if (lamdt)
    free_dvector(lamdt, 0, endi);
  if (dti)
    free_dvector(dti, 0, endi);
  if (nbpT)
    free_lvector(nbpT, 0, endi);
  if (cpn)
    free_dvector(cpn, 0, iNbCoupon - 1);
  if (beta)
    free_dvector(beta, 0, iNbCoupon - 1);

  if (sum)
    free_dvector(sum, 0, nb_strike - 1);
  if (sum2)
    free_dvector(sum2, 0, nb_strike - 1);
  if (covar)
    free_dvector(covar, 0, nb_strike - 1);

  if (sig)
    free_dvector(sig, 0, nb_strike - 1);
  if (var)
    free_dvector(var, 0, nb_strike - 1);
  if (drift)
    free_dvector(drift, 0, nb_strike - 1);

  if (init_gauss)
    free_dvector(init_gauss, 0, nb_strike - 1);

  return err;
}

/* Put points between original points to ensure time between two points does not
 * exceed Max Time */
Err fill_time_vector_max_time(int iInitNbTimes, double *dInitTimes,
                              double dMaxTime, int *iNewNbTimes,
                              double **dNewTimes) {
  Err err = NULL;
  int i, j, k;
  int nstp;
  ...
  *iNewNbTimes = 1;
  ...
  for (i = 0; i < iInitNbTimes - 1; i++) {
    ...
    *iNewNbTimes +=
        max((int)((dInitTimes[i + 1] - dInitTimes[i]) / dMaxTime + 0.5), 1);
  }

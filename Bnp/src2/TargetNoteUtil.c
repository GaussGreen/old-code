...
char* TARN_calcHistoryOld(TARN_Struct* tarn, TARN_AUX* aux)
{
    ...
    /* Find the value of the cumulative coupon and see if it has knocked out.  If it hasn't knocked
       out, find the first event date */
           ...
           (tarn->deal.lvFltrFixingDates[aux->i1stCpn] < lFirstFixing))
    {
        ...
        if (tarn->deal.ivIsFloored[aux->i1stCpn])
            dCoupon = dCoupon < tarn->deal.dvFloor[aux->i1stCpn] ? tarn->deal.dvFloor[aux->i1stCpn]
                                                                 : dCoupon;
        if (tarn->deal.ivIsCapped[aux->i1stCpn])
            dCoupon =
                dCoupon > tarn->deal.dvCap[aux->i1stCpn] ? tarn->deal.dvCap[aux->i1stCpn] : dCoupon;
        if (tarn->output.dCumulCoupon + dCoupon * aux->dvCumulCvg[i] >= tarn->deal.dTarget)
        {
            tarn->output.iIsKnockedOut = aux->i1stCpn + 1;
            dCoupon =
                (tarn->deal.dTarget - tarn->output.dCumulCoupon) / aux->dvCouponCvg[aux->i1stCpn];
    ...
    if (tarn->output.iIsKnockedOut)
    {
        ...
        if (tarn->deal.couponType == TN_SWAP)
        {
            if (tarn->deal.lvCpnPayDates[aux->i1stCpn] >= lFirstPayment)
                aux->dHistCpnPV =
                    dCoupon * aux->dvCouponCvg[aux->i1stCpn] * aux->dvCouponPayDF[aux->i1stCpn];
        ...
        else if (tarn->deal.couponType == TN_FINAL)
        {
            if (tarn->deal.lvCpnPayDates[aux->i1stCpn] >= lFirstPayment)
                aux->dHistCpnPV =
                    dCoupon * aux->dvCouponCvg[aux->i1stCpn] * aux->dvCouponPayDF[aux->i1stCpn];
            aux->dHistCpnPV +=
                (tarn->deal.dTarget - tarn->output.dCumulCoupon) * aux->dvCouponPayDF[aux->i1stCpn];
        }
        ...
        for (i = aux->ivFundStartIndex[aux->i1stCpn]; i <= aux->ivFundEndIndex[aux->i1stCpn]; i++)
        {
            ...
            if (tarn->deal.lvFundFixingDates[i] >= lFirstFixing &&
                tarn->deal.lvFundEndDates[i] >= lFirstPayment)
            {
                double dFund_Start_DF = swp_f_df(
                    tarn->market.lToday, tarn->deal.lvFundStartDates[i], tarn->market.szYieldCurve);
                double dFund_End_DF = swp_f_df(
                    tarn->market.lToday, tarn->deal.lvFundEndDates[i], tarn->market.szYieldCurve);
    ...
    /* if the swap has not knocked out, calculate any residual funding PV up to the first fixing
     * date */
...
{
        double dCoupon = tarn->deal.dvCoupon[aux->i1stCpn] + tarn->deal.dvGearing[aux->i1stCpn] *
tarn->deal.dvFltrHistFixings[aux->i1stCpn]; if ( tarn->deal.ivIsFloored[aux->i1stCpn] ) dCoupon =
dCoupon < tarn->deal.dvFloor[aux->i1stCpn] ? tarn->deal.dvFloor[aux->i1stCpn] : dCoupon; if (
tarn->deal.ivIsCapped[aux->i1stCpn] ) dCoupon = dCoupon > tarn->deal.dvCap[aux->i1stCpn] ?
tarn->deal.dvCap[aux->i1stCpn] : dCoupon;
}
*/
...
char* TARN_calcHistory_ZC(TARN_Struct* tarn, TARN_AUX* aux)
{
    ...
    /* if there is only 1 coupon remaining, then the cash flows are deterministic, so it is as if we
     * have knocked out */
    if (aux->i1stCpn == (tarn->deal.nCouponDates - 1) && tarn->deal.bRepayCoupon)
    {
        tarn->output.iIsKnockedOut = tarn->deal.nCouponDates;
...
char* TARN_calcHistory_SWAP(TARN_Struct* tarn, TARN_AUX* aux)
{
    ...
    /* if there is only 1 coupon remaining and the coupon is guaranteed, then the cash flows are
     * deterministic, */
           ...
           (tarn->deal.lvFltrFixingDates[aux->i1stCpn] < lFirstFixing))
    {
        ...
        if (tarn->output.iIsKnockedOut)
        {
            tarn->output.dAccretedCoupon *=
                1.0 +
                aux->dvCouponCvg[aux->i1stCpn] * tarn->deal.dvFltrHistFixings[aux->i1stCpn - 1];
        }

...
/* 
	TargetNoteUtil.c 
	Routines:	
		TARN_calcHistory:  from the historical fixings, calculate what has happened.
*/
...
copy_TARN_Calibration_Struct( TARN_Calibration_Struct* in, TARN_Calibration_Struct* out )
{
...
free_TARN_Output_Struct( TARN_Output_Struct* tarn_output_struct )
{
	...
	if ( tarn_output_struct->dmKO_prob )
		free_dmatrix( tarn_output_struct->dmKO_prob, 0, tarn_output_struct->nProb, 0, 1 );
	tarn_output_struct->dmKO_prob = 0;
	tarn_output_struct->nProb = 0;
...
char* TARN_calcHistoryOld( TARN_Struct* tarn, 
						TARN_AUX* aux )
{
...
/* Find the value of the cumulative coupon and see if it has knocked out.  If it hasn't knocked out, find the first
	event date */
	while (  (! tarn->output.iIsKnockedOut) && ( aux->i1stCpn < tarn->deal.nCouponDates) 
					&& (tarn->deal.lvFltrFixingDates[aux->i1stCpn] < lFirstFixing)  )
	{
		dCoupon = tarn->deal.dvCoupon[aux->i1stCpn] + tarn->deal.dvGearing[aux->i1stCpn] * tarn->deal.dvFltrHistFixings[aux->i1stCpn];
		if ( tarn->deal.ivIsFloored[aux->i1stCpn] )
			dCoupon = dCoupon < tarn->deal.dvFloor[aux->i1stCpn] ? tarn->deal.dvFloor[aux->i1stCpn] : dCoupon;
		if ( tarn->deal.ivIsCapped[aux->i1stCpn] )
			dCoupon = dCoupon > tarn->deal.dvCap[aux->i1stCpn] ? tarn->deal.dvCap[aux->i1stCpn] : dCoupon;
		if ( tarn->output.dCumulCoupon + dCoupon * aux->dvCumulCvg[i] >= tarn->deal.dTarget )
		{
			tarn->output.iIsKnockedOut = aux->i1stCpn + 1;
			dCoupon = ( tarn->deal.dTarget - tarn->output.dCumulCoupon ) / aux->dvCouponCvg[aux->i1stCpn];
...
/* then the cash flows are deterministic, so it is as if we have knocked out */
	if (  aux->i1stCpn == (tarn->deal.nCouponDates - 1)  && tarn->deal.bRepayCoupon )
	{
...
/* If the swap has knocked out, see if there are any remaining cash flows and calculate their PV */
...
/* Calculate the coupon PV */
		if ( tarn->deal.couponType == TN_SWAP )
		{
			if ( tarn->deal.lvCpnPayDates[aux->i1stCpn] >= lFirstPayment )
				aux->dHistCpnPV = dCoupon * aux->dvCouponCvg[aux->i1stCpn]* aux->dvCouponPayDF[aux->i1stCpn];
//			aux->dHistCpnPV += ( tarn->deal.dTarget - tarn->output.dCumulCoupon ) * aux->dvCouponPayDF[aux->i1stCpn];
		}
		if ( tarn->deal.couponType == TN_SWAP )
		{
			if ( tarn->deal.lvCpnPayDates[aux->i1stCpn] >= lFirstPayment )
...
/* Calculate the funding PV */
		for ( i=aux->ivFundStartIndex[aux->i1stCpn]; i<=aux->ivFundEndIndex[aux->i1stCpn]; i++ )
		{
			if ( tarn->deal.lvFundFixingDates[i] < lFirstFixing && tarn->deal.lvFundEndDates[i] >= lFirstPayment )
				aux->dHistFundPV += ( tarn->deal.dvFundHistFixings[i] + tarn->deal.dvFundMargin[i] ) 
										* aux->dvFundCvg[i] * aux->dvFundDomPayDF[i];

			if ( tarn->deal.lvFundFixingDates[i] >= lFirstFixing && tarn->deal.lvFundEndDates[i] >= lFirstPayment )
			{
				double dFund_Start_DF = swp_f_df(tarn->market.lToday, tarn->deal.lvFundStartDates[i], tarn->market.szYieldCurve);
				double dFund_End_DF = swp_f_df(tarn->market.lToday, tarn->deal.lvFundEndDates[i], tarn->market.szYieldCurve);
				double dFund_cvgMargin = 1.0 - coverage( tarn->deal.lvFundStartDates[i], tarn->deal.lvFundEndDates[i], tarn->deal.basisFund ) 
...
/* if the swap has not knocked out, calculate any residual coupon cash flows */
	if ( tarn->deal.couponType == TN_SWAP )
	{
		if (  (aux->i1stCpn > 0) && (tarn->deal.lvCpnPayDates[aux->i1stCpn-1] >= lFirstPayment)  )
			aux->dHistCpnPV = ( tarn->deal.dvCoupon[aux->i1stCpn-1] + tarn->deal.dvGearing[aux->i1stCpn-1] * tarn->deal.dvFltrHistFixings[aux->i1stCpn-1] )
								* aux->dvCouponCvg[aux->i1stCpn-1] * aux->dvCouponPayDF[aux->i1stCpn-1];
	}


/* if the swap has not knocked out, calculate any residual funding PV up to the first fixing date */
	iTemp = aux->i1stCpn == 0 ? 0 : aux->i1stCpn - 1;
	for ( i=aux->ivFundStartIndex[iTemp]; i<=aux->ivFundEndIndex[aux->i1stCpn]; i++ )
	{
		if ( tarn->deal.lvFundFixingDates[i] < lFirstFixing && tarn->deal.lvFundEndDates[i] >= lFirstPayment )
			aux->dHistFundPV += ( tarn->deal.dvFundHistFixings[i] + tarn->deal.dvFundMargin[i] ) * aux->dvFundCvg[i] * aux->dvFundDomPayDF[i];
...
{
	double dCoupon = tarn->deal.dvCoupon[aux->i1stCpn] + tarn->deal.dvGearing[aux->i1stCpn] * tarn->deal.dvFltrHistFixings[aux->i1stCpn];
	if ( tarn->deal.ivIsFloored[aux->i1stCpn] )
		dCoupon = dCoupon < tarn->deal.dvFloor[aux->i1stCpn] ? tarn->deal.dvFloor[aux->i1stCpn] : dCoupon;
	if ( tarn->deal.ivIsCapped[aux->i1stCpn] )
		dCoupon = dCoupon > tarn->deal.dvCap[aux->i1stCpn] ? tarn->deal.dvCap[aux->i1stCpn] : dCoupon;
...
char* TARN_calcHistory_ZC( TARN_Struct* tarn, 
								TARN_AUX* aux )
{
/* Variable declaration */
	long lFirstFixing = tarn->market.lToday + ! tarn->market.eodFixFlag;
...
/* if there is only 1 coupon remaining, then the cash flows are deterministic, so it is as if we have knocked out */
	if (  aux->i1stCpn == (tarn->deal.nCouponDates - 1) && tarn->deal.bRepayCoupon )
	{
		tarn->output.iIsKnockedOut = tarn->deal.nCouponDates;
	}
...
char* TARN_calcHistory_SWAP( TARN_Struct* tarn, 
								TARN_AUX* aux )
{
/* Variable declaration */
	long lFirstFixing = tarn->market.lToday + ! tarn->market.eodFixFlag;
...
/* if there is only 1 coupon remaining and the coupon is guaranteed, then the cash flows are deterministic, */
...
/* if the swap has not knocked out, calculate any residual coupon cash flows */
	if (  (aux->i1stCpn > 0) && (tarn->deal.lvCpnPayDates[aux->i1stCpn-1] >= lFirstPayment)  )
		aux->dHistCpnPV = ( tarn->deal.dvCoupon[aux->i1stCpn-1] 
							+ tarn->deal.dvGearing[aux->i1stCpn-1] * tarn->deal.dvFltrHistFixings[aux->i1stCpn-1] )
						* aux->dvCouponCvg[aux->i1stCpn-1] * aux->dvCouponPayDF[aux->i1stCpn-1];
...
char* TARN_calcHistory_FINAL( TARN_Struct* tarn, 
								TARN_AUX* aux )
{
/* Variable declaration */
	long lFirstFixing = tarn->market.lToday + ! tarn->market.eodFixFlag;
...
/* Find the value of the cumulative coupon and see if it has knocked out.  If it hasn't knocked out, find the first
	...
	while (  ( aux->i1stCpn < tarn->deal.nCouponDates) && (tarn->deal.lvFltrFixingDates[aux->i1stCpn] < lFirstFixing)  )
	{
...
/* Check if we have previously knocked out */
		if ( tarn->output.iIsKnockedOut )  
		{
			tarn->output.dAccretedCoupon *= 
							1.0 + aux->dvCouponCvg[aux->i1stCpn]*tarn->deal.dvFltrHistFixings[aux->i1stCpn-1];
		}
/* or if we knocked out this time */
		else if ( tarn->output.dCumulCoupon + dCoupon * aux->dvCumulCvg[i] >= tarn->deal.dTarget )
		{
			tarn->output.iIsKnockedOut = aux->i1stCpn + 1;
...
/* if there is only 1 coupon remaining, only then is there a cash flow */
	if (  aux->i1stCpn == (tarn->deal.nCouponDates - 1)  )
	{
		...
		aux->dHistCpnPV = ( tarn->output.dAccretedCoupon - 1.0 ) * aux->dvCouponPayDF[aux->i1stCpn];
	}
...
char* TARN_calcHistory_SWAPKO( TARN_Struct* tarn, 
								TARN_AUX* aux )
{
/* Variable declaration */
	long lFirstFixing = tarn->market.lToday + ! tarn->market.eodFixFlag;
...
char* TARN_calcHistory( TARN_Struct* tarn, 
						TARN_AUX* aux )
{

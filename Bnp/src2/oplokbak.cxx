...
double srt_f_optlokbak(double fwd_price, double spot, double hist_extr,
                       ...
                       SrtGreekType greek) { /* BEGIN srt_f_optlokbak */
  ...
  **/
  ...
  if (greek == PREMIUM) {
    ...
    answer = (srt_f_optlokbak(fwd_price + shift, spot, hist_extr, vol, mat,
                              disc, call_put, PREMIUM) -
              premium) /
             shift;
  } else if (greek == DELTA) { /* DELTA */
    shift = spot / 10000;
    answer = (srt_f_optlokbak(fwd_price * (1 + shift / spot), spot + shift,
  ...
  */
    ...
    answer = (srt_f_optlokbak(fwd_price * (1 + shift / spot), spot + shift,
                              hist_extr, vol, mat, disc, call_put, PREMIUM) +
              srt_f_optlokbak(fwd_price * (1 - shift / spot), spot - shift,
                              hist_extr, vol, mat, disc, call_put, PREMIUM) -
              2 * premium) /
             (shift * shift);
  } else if (greek == VEGA) { /*   VEGA    */
    shift = GVOPT.vol_add;
    answer = (srt_f_optlokbak(fwd_price, spot, hist_extr, vol + shift, mat,
                              disc, call_put, PREMIUM) -
              premium) /
             shift;
  } else if (greek == THETA) { /*   THETA   */
    shift = YEARS_IN_DAY;
    answer = (srt_f_optlokbak(fwd_price, spot, hist_extr, vol, mat - shift,

...
Err ccf_check_exo_leg(CF_EXO_LEG exo_leg) {
  ...
  for (i = 1; i < exo_leg->num_cpn; i++) {
    ...
    if (exo_leg->cpn[i].cxxms_fix_date < exo_leg->cpn[i - 1].cxxms_fix_date) {
  ...
  for (i = 0; i < exo_leg->num_cpn; i++) {
    ...
    if (exo_leg->cpn[i].pay_date < exo_leg->cpn[i].cxxms_fix_date) {
...
Err ccf_check_calls(CCF_STR ccf) {
  ...
  if (nCheckCall) {
    ...
    for (i = 1; i < ccf->num_calls; i++) {
      ...
      if (ccf->call[i].cxxf_idx < ccf->call[i - 1].cxxf_idx) {
        return "Number of exotic coupons controlled by calls should be "
               "decreasing";
      }

      if (ccf->call[i].fund_idx <= ccf->call[i - 1].fund_idx &&
          ccf->call[i].cxxf_idx <= ccf->call[i - 1].cxxf_idx) {
    ...
    /*	Check that set dates are after ex dates
                    Also check that the call date is before the start        ,
       end and fixing dates of the coupons it controls */
    for (i = 0; i < ccf->num_calls; i++) {
      ...
      if (ccf->cf_leg->cpn[ccf->call[i].cxxf_idx].start_date <
              ccf->call[i].ex_date ||
          ccf->cf_leg->cpn[ccf->call[i].cxxf_idx].pay_date <
              ccf->call[i].ex_date ||
          ccf->cf_leg->cpn[ccf->call[i].cxxf_idx].cxxms_fix_date <
              ccf->call[i].ex_date) {
...
static Err find_strikes(
  ...
  for (i = 0; i < ccf->num_calls; i++) {
    ...
    if (fabs(fl_lvl) > 1.0e-08) {
      ...
      /*	Special case        , no sensitivity to level of rates        ,
       * do atm */
    } else {
      dfra = 0.0;
    }
        ...
        if (!exo_cpn->use_cfoptions) {
          ...
          if (eval_const->cf_spr_floor_type[l] == 0) {
            ...
            err = OptSpreadNew(
                ...
                (eval_const->cf_spr_floor_type[l] > 2
                     ? exo_cpn->cms[0].slvol
                     : 0.0),                          /* sigx        , */

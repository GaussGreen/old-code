/* ==========================================================================
   ...
   stochastic spread between the two interest rates - domestic and foreign
   ========================================================================== */
...
static double CalcVar(double vol, double spreadvol, double rho, double lambda,
  ...
  var += spreadvol * spreadvol *
         (T2 - T1 + K_func(2. * lambda, T1, T2, T) -
          2. * K_func(lambda, T1, T2, T)) /
         (lambda * lambda);

  var += 2. * rho * vol * spreadvol * (T2 - T1 - K_func(lambda, T1, T2, T)) /
         lambda;
...
Err GetImpVol(double optmat, double *mats, double *fxvols, long nbropts,
              double spreadvol, double rho, double lambda, double *impvol) {
  int i;
  ...
  for (i = 1; (i <= nbropts) && (mats[i] < optmat); i++) {
    ...
    (*impvol) += CalcVar(fxvols[i], spreadvol, rho, lambda, mats[i - 1],
                         mats[i], optmat);
  }
  ...
  (*impvol) +=
      CalcVar(fxvols[i], spreadvol, rho, lambda, mats[i - 1], optmat, optmat);
...
Err CalibFxOMModel(double *impvols, double *optmats, long nbropts,
  ...
  for (idxopt = 1; idxopt <= nbropts; idxopt++) {
    ...
    for (idxcumvar = 1; idxcumvar < idxopt; idxcumvar++) {
      cumvar +=
          CalcVar((*fxvols)[idxcumvar], spreadvol, rho, lambda,
    ...
    b = 2. * rho * spreadvol *
        (a - K_func(lambda, optmats[idxopt - 1], optmats[idxopt],
                    optmats[idxopt])) /
        lambda;

    c = spreadvol * spreadvol *
        (a +
         K_func(2. * lambda, optmats[idxopt - 1], optmats[idxopt],
                optmats[idxopt]) -
         2. * K_func(lambda, optmats[idxopt - 1], optmats[idxopt],
                     optmats[idxopt]));
    c /= (lambda * lambda);
...
/* Main Function */
Err ImpFxOpt(double optmat, double *calibimpvols, double *caliboptmats,
             long nbrcalibopts, double spreadvol, double rho, double lambda,
             double *impvol) {
  Err err = NULL;
  double *calibfxvols = NULL; /* calibrated fx spot vols */

  /* Calibration of the model */
  err = CalibFxOMModel(calibimpvols, caliboptmats, nbrcalibopts, spreadvol, rho,
                       lambda, &calibfxvols);
  ...
  /* Get the implied vol */
  err = GetImpVol(optmat, caliboptmats, calibfxvols, nbrcalibopts, spreadvol,
                  rho, lambda, impvol);

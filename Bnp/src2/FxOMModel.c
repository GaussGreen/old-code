...
static double CalcVar(
    double vol, double spreadvol, double rho, double lambda, double T1, double T2, double T)
{
    double var;

    var = vol * vol * (T2 - T1);
    var += spreadvol * spreadvol *
           (T2 - T1 + K_func(2. * lambda, T1, T2, T) - 2. * K_func(lambda, T1, T2, T)) /
           (lambda * lambda);

    var += 2. * rho * vol * spreadvol * (T2 - T1 - K_func(lambda, T1, T2, T)) / lambda;
    ...
    double* impvol)
{
    ...
    {
        (*impvol) += CalcVar(fxvols[i], spreadvol, rho, lambda, mats[i - 1], mats[i], optmat);
    }

    /* add the last part of the cumulative variance */
    /* need the previous index */
    (*impvol) += CalcVar(fxvols[i], spreadvol, rho, lambda, mats[i - 1], optmat, optmat);
    ...
    double** fxvols)
{
    ...
    for (idxopt = 1; idxopt <= nbropts; idxopt++)
    {
        ...
        b = 2. * rho * spreadvol *
            (a - K_func(lambda, optmats[idxopt - 1], optmats[idxopt], optmats[idxopt])) / lambda;

        c = spreadvol * spreadvol *
            (a + K_func(2. * lambda, optmats[idxopt - 1], optmats[idxopt], optmats[idxopt]) -
    ...
    double* impvol)
{
    ...
    err = CalibFxOMModel(
        calibimpvols, caliboptmats, nbrcalibopts, spreadvol, rho, lambda, &calibfxvols);
    ...
    /* Get the implied vol */
    err =
        GetImpVol(optmat, caliboptmats, calibfxvols, nbrcalibopts, spreadvol, rho, lambda, impvol);

...
char *WrapSrtBootstrap(int numParams, char **paramStrings, char **valueStrings,
  ...
  for (i = 0; i < *nInputs; i++) {
    err = swp_f_initSwapDP(startDates[i], endDates[i], cpdStr[i], basisStr[i],
                           &sdpArray[i]);
    if (err) {
      FREE_WRAP_BOOTSTRAP_MEMORY;
      return err;
    ...
    if (derivType[i] == CAPFLOOR) {
      ...
      lLastFixing =
          add_unit(sdpArray[i].start, -Spot_Lag, SRT_BDAY, MODIFIED_SUCCEEDING);
    }
    /* We only keep track of the instruments with a fixing date before or equal
     * to today */
    if (lLastFixing > clcndate) {
      /* Compute the forward rate and the spread for the forward period
       * underlying the instrument */
      err = swp_f_ForwardRate_SwapDP(&(sdpArray[i]), szYieldCurveName,
                                     refRateStr[i], &dForwardRate);
      /* Compute The ATM price */
      if (derivType[i] == CAPFLOOR) {
        err = swp_f_CapFloor_SwapDP(&(sdpArray[i]), dForwardRate, pfGetVol,
                                    rec_pay[i], refRateStr[i], szYieldCurveName,
                                    PREMIUM, eLogOrNorm, &ATMprice[i]);
        if (err) {
        ...
        dfEnd = swp_f_df(clcndate, date_list.date[date_list.len - 1],
                         szYieldCurveName);
        ...
        err = pfGetVol((double)dStart, (double)dEnd, dForwardRate, dForwardRate,
                       dSpread, &dATMVol);
        if (err) {
          srt_free(date_list.date);
          FREE_WRAP_BOOTSTRAP_MEMORY;
        ...
        err = swp_f_Swaption_SwapDP(&(sdpArray[i]), dATMVol, dForwardRate,
                                    rec_pay[i], refRateStr[i], szYieldCurveName,
                                    PREMIUM, eLogOrNorm, &ATMprice[i]);
        if (err) {
          srt_free(date_list.date);
  ...
  err = srt_f_bootstrap_calibrate(und, &grfnparam, sdpArray, strike, bondStrike,
                                  derivType, rec_pay, refRateStr, undPrice,
                                  ATMprice, tau, alpha, beta, rho, nInputs);

...
/* Loop throught the elements of the list until one with the same name is found */
	while (tmp!=NULL) 
	{
...
/* Free the Currency parameters if a Yield Curve */
	switch (curve_desc->curve_type )
	{
		...
		case DVD_CURVE:
			...
			err = srt_f_dvdobj_delete(& ((( SrtFwdDesc *) curve_desc->curve_desc)->dvd_obj));
			if (err)
				return err;
...
/* Insert (overwrite & update ticker) the object in the curve list (with the same name) */
	err = srt_f_lstins(curve_list, curve_name, 0.0, OBJ_PTR_CURVE, 
		curve, &srt_f_curvevalfree, &(curve->curve_ticker));
	if (err)

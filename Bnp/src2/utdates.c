...
double coverage_ACT_ACT(Date date1, Date date2, SrtBasisCode basis_code)
{
    ...
    if (y1 != y2)
    {
        ...
        begin_year = date(d_jan1, m_jan1, y2);  // Stanley Mrose (11.11.2002): have to update start
                                                // date when calculating final stub
        res += ((double)day_count_date(begin_year, date2, basis_code)) / num_days_year(date2);
    }
    else
        res = ((double)day_count_date(date1, date2, basis_code)) / num_days_year(date1);
...
Date bus_hld_fct(Date d, double hlds[30][2], int nrow, int ncol, int sun, int end_m)
{
    int  j, flag;
    Date dold;

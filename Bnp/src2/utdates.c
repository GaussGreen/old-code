...
int i,q,r;
	...
	d = day(date1); m = month(date1); y = year(date1);
	...
	switch(unit)
	{
		...
		case SRT_WEEK:
			...
			if (conv == SUCCEEDING)
			{
				if (week_day(date1)==SATURDAY) return date1 + 2;
				if (week_day(date1)==SUNDAY) return date1 + 1;
			}
		...
		case SRT_BDAY:
			...
			/* unless we just want the next bus day in modified succeeding (nb_unit == 0) */
			...
			/* other 'succeeding' */
			else if (week_day(date1)==SATURDAY) date1 = date1 + 2;
			else if (week_day(date1)==SUNDAY) date1 = date1 + 1;
...
/* third number and letter */        
	...
	{
		case 1:
			if(let1=='Y') num_years  = num1;
			if(let1=='M') num_months = num1;
			if(let1=='D') num_days   = num1;
			if(let1=='W') num_days   = num1*7;	
			break;                
		...
		case 2:
			...
			if(let2=='Y') num_years  = num2;
			if(let2=='M') num_months = num2;
			if(let2=='D') num_days   = num2;
			break;
		...
		case 3:
			...
			if(let3=='Y') num_years  = num3;
			if(let3=='M') num_months = num3;
			if(let3=='D') num_days   = num3;
			break;
...
double res=0; 
	...
	if(y1 != y2)
	{
		...
		res = ((double)day_count_date(date1,begin_year,basis_code))/ num_days_year(date1);
		...
		begin_year = date(d_jan1, m_jan1, y2);	// Stanley Mrose (11.11.2002): have to update start date when calculating final stub
		res += ((double)day_count_date(begin_year,date2,basis_code))/ num_days_year(date2); 
  	}
	else   
...
{
static int cummonth[]={0,0,31,59,90,120,151,181,212,243,273,304,334,365};
static int mlength[]={0,31,28,31,30,31,30,31,31,30,31,30,31};

	if (y>1899) y-=1900;

	if (d<1 || d>31 || m<1 || m>12 || y<0 || y>199) return -1;

	if ((m!=2 && d>mlength[m]) || (m==2 && d>28+(y%4?0:1))) return -1;

	return ((long)y/4)*1461+(y%4)*365+cummonth[m]+d+(y%4==0&&m<3?0:1);
}
...
int y;
	...
	if (n<1 || n>73050) return -1;

	y=(int)(((n-1)/1461)*4);
	y=y+(int)((n-(long)y/4*1461-2)/365);

	if (y<0 || y>199) return -1;
...
/* first day of month */
	...
	if(num_wed == 2)
		while(week_day(d1)!= WEDNESDAY) d1++; 
...
int j,flag;
    ...
    while (flag != 0)
	{
		...
		for(j=0;j<nrow;j++)
		{
        	...
        	if (  (week_day(d-1) == SUNDAY) 
           	   && (month(d-1) == (int )hlds[j][0] ) 
		       && (day(d-1) == (int )hlds[j][1] ) 
		       && (sun == 1) )  
			{ 
				d-=3;
				flag++; 
...
Date bus_hld_fct(Date d, double hlds[30][2], int nrow, int ncol, 
			int sun, int end_m)
{   
int j,flag;
Date	dold;
	...
	while (flag != 0)
	{
		...
		if (week_day(d)==SATURDAY) { d += 2; flag=1; }
	    if (week_day(d)==SUNDAY) { d++; flag=1; }
	}

	if (  (month(d) != month(dold)) 
       && (end_m) ) 
	{
...
*****************************************************************************/ 
Date add_days_hlds(Date d, int nbd, double hlds[30][2], int nrow, int ncol, int sun)
{
int 	i;
Date	dn;
...
/***************************************************************************** 
	Adds number of business days (excluding holidays) to date1	
*****************************************************************************/ 
Date less_days_hlds(Date d, int nbd, double hlds[30][2], int nrow, int ncol, int sun)
{
int 	i;
Date	dn;

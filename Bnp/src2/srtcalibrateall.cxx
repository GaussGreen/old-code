/* =======================================================================
        FILENAME:    srtnewcalibrateall.cxx
...
#define global_calib_free                                                      \
  {                                                                            \
    ...
    if (*pppdSigmaCurve)                                                       \
      free_dmatrix(*pppdSigmaCurve, 0, *plNumSigmaCols - 1, 0,                 \
                   *plNumSigmas - 1);                                          \
    *pppdSigmaCurve = NULL;                                                    \
    if (plDealTauDates)                                                        \
...
Err change_und_name(char *the_und) {
  ...
  if (!strstr(the_und, "_CAL")) {
    ...
    if (err) {
      return serror("Could not find %s underlying in list",
                    get_underlying_name(sUndPtr));
    }
...
/* ------------------------------------------------------------------------
        ...
        PURPOSE: calibrate model acoording to user's inputs:
                ...
                if FixedCalibration:
                        the correlation structure (alpha        , beta        ,
 rho) is frozen optimisation is made only on sig1(t)        , lam1(t) the
 criteria is just based on market prices USAGE: to get to a : Global
 Calibration: DEFAULT calib_param->type set to GLOBAL the FRD matrix has to be
 defined Blind Calibration: calib_param->type set to BLIND the FRD matrix is set
 to NULL Fixed Calibration calib_param->type set to FIXED the FRD matrix is set
 to NULL
 ------------------------------------------------------------------------- */
...
Err SrtNewCalibrateAll(
    String *pszGrfnParamNames, String *pszGrfnParamValues, long lNumGrfnParams,
    long *plStartDates, long *plEndDatesOrNfp, String *pszFrequency,
    String *pszBasis, double *pdStrike, double *pdBondStrike,
    String *pszOptionType, String *pszRecPay, String *pszRefRateCode,
    double *pdPrice, double *pdVega,
    Err (*pfGetVol)(double dStart, double dEnd, double dStrike, double dForward,
                    double dSpread, double *pdBsVol),
    String *LogNormStr, long lNumInstruments, String *pszFraTenors,
    double **pdCorrelationMatrix, long lNumTenors, String szUndName,
    String szNewName, String *pszCalibParamNames, String *pszCalibParamValues,
    long lNumCalibParams, double ***pppdSigmaCurve, long *plNumSigmas,
    long *plNumSigmaCols, double ***pppdTauCurve, long *plNumTaus,
    long *plNumTauCols, double *pdChiSquare, double **ppdTheoPrices,
    int *indexUsedInstr, int *InstrsIndexArrays) {
  Err err = NULL;
  int i;
  ...
  if (is_model_Cheyette_type(eModelType) == SRT_NO) {
    return serror("Can only calibrate LGM        , CHEY        , CHEYBETA "
                  "(with STOCHVOL) so far");
  }
  ...
  if (eModelDim == TWO_FAC) {
    if ((*plNumTauCols != 0) && (*plNumTauCols != 3)) {
      return serror(
          "Need a FULL Two Factor Tau TS for Calibration: Date - Tau1 - Tau2");
    }
    if ((*plNumSigmaCols != 0) && (*plNumSigmaCols != 6)) {
      return serror("Need a FULL Two Factor Sigma TS for Calibration: Date - "
                    "Sig1 - Bet1 - Sig2 - Bet2 - Rho");
    }
  } else if (eModelDim == ONE_FAC) {
    if ((*plNumTauCols != 0) && (*plNumTauCols != 2)) {
      return serror(
          "Need a FULL One Factor Tau TS for Calibration : Date - Tau");
    }
    if ((*plNumSigmaCols != 0) && (*plNumSigmaCols != 3)) {
      return serror("Need a FULL One Factor Sigma TS for Calibration : Date - "
                    "Sig - Beta");
    }
  } else {
  ...
  err = srt_f_set_CalibParams(pszCalibParamNames, pszCalibParamValues,
                              lNumCalibParams, &calib_param, eModelType);
  if (err) {
    return err;
  }

  /* Sets the Grfn Parameters  */
  err = srt_f_set_GrfnParams(lNumGrfnParams, pszGrfnParamNames,
  ...
  if (err = interp_diffusion_type(*LogNormStr, &eLogOrNorm)) {
    return serror(
        "Could not interpret the Type of the Vol in Market Vol Matrix");
  }

  /* Allocate an array of SwapDP and initialise them: start        , end , comp
   * , basis */
  ...
  if (!peOptionType) {
    srt_free(psSwapDpFree);
    return serror(
        "Memory allocation error (peOptionType) in srt_f_CalibrateAll");
  }
  ...
  for (i = 0; i < lNumInstruments; i++) {
    err = swp_f_initSwapDP(plStartDates[i], plEndDatesOrNfp[i], pszFrequency[i],
                           pszBasis[i], &psSwapDp[i]);
    if (err) {
  ...
  /* Allocate a vector of StructType        , SrtReceiverType and initialises
   * them */
  for (i = 0; i < lNumInstruments; i++) {
  ...
  for (i = 0; i < lNumInstruments; i++) {
    ...
    date_list = SwapDP_to_DateList(&(psSwapDp[iNumUsedInstruments]),
                                   MODIFIED_SUCCEEDING);
    ...
    if (peOptionType[iNumUsedInstruments] == CAPFLOOR) {
      ...
      lFirstFixing = add_unit(first_period_start, -Spot_Lag, SRT_BDAY,
                              MODIFIED_SUCCEEDING);
      /* Calibration on the last caplet of the cap */
      last_period_start = date_list.date[date_list.len - 2];
      lLastFixing =
          add_unit(last_period_start, -Spot_Lag, SRT_BDAY, MODIFIED_SUCCEEDING);
    } else if ((peOptionType[iNumUsedInstruments] == SWAPTION) ||
      ...
      lLastFixing = add_unit(psSwapDp[iNumUsedInstruments].start, -Spot_Lag,
                             SRT_BDAY, MODIFIED_SUCCEEDING);
    }
    /* We only keep track of the instruments with a fixing date before or equal
     * to today */
    if (lLastFixing > today) {

      /* In the case of the full Cap and if the first fixing is before today we
       * modify the start date and the relevant swapDP */
      if (peOptionType[iNumUsedInstruments] == CAPFLOOR) {
        int indexcap = 0;
        ...
        err = swp_f_initSwapDP(
            first_period_start, plEndDatesOrNfp[iNumUsedInstruments],
            pszFrequency[iNumUsedInstruments], pszBasis[iNumUsedInstruments],
        ...
        err = swp_f_CapFloor_SwapDP(
            &psSwapDp[iNumUsedInstruments], pdStrike[iNumUsedInstruments],
            pfGetVol, peRecPay[iNumUsedInstruments],
            pszRefRateCode[iNumUsedInstruments], szYieldCurveName, PREMIUM,
            eLogOrNorm, &pdPrice[iNumUsedInstruments]);
        if (err) {
          srt_free(date_list.date);
          global_new_calib_free;
          return err;
        ...
        err = swp_f_CapFloor_SwapDP(
            &psSwapDp[iNumUsedInstruments], pdStrike[iNumUsedInstruments],
            pfGetVol, peRecPay[iNumUsedInstruments],
            pszRefRateCode[iNumUsedInstruments], szYieldCurveName, DELTA,
            eLogOrNorm, &dDelta);
        if (err) {
          srt_free(date_list.date);
          global_new_calib_free;
        ...
        err = swp_f_Level_SwapDP(&(psSwapDp[iNumUsedInstruments]),
                                 szYieldCurveName, &dLevel);
      ...
      if ((peOptionType[iNumUsedInstruments] == SWAPTION)
        ...
        err = swp_f_ForwardRate_SwapDP(
            &(psSwapDp[iNumUsedInstruments]), szYieldCurveName,
            pszRefRateCode[iNumUsedInstruments], &dForwardRate);
        if (err) {
          srt_free(date_list.date);
          global_new_calib_free;
          return err;
        ...
        dfEnd = swp_f_df(today, date_list.date[date_list.len - 1],
                         szYieldCurveName);
        dForwardCash = (dfStart - dfEnd) / dLevel;
        dSpread = dForwardRate - dForwardCash;
        /*gets the ATM Vol */
        err = pfGetVol((double)DTOL(date_list.date[0]),
                       (double)DTOL(date_list.date[date_list.len - 1]),
                       dForwardRate, dForwardRate, dSpread, &dATMVol);
        if (err) {
          srt_free(date_list.date);
          global_new_calib_free;
          return err;
        ...
        if (eLogOrNorm == SRT_NORMAL) {
          dDistance =
              fabs(dForwardRate - pdStrike[iNumUsedInstruments]) / dStdDev;
        } else {
          dDistance =
              fabs(log(dForwardRate / pdStrike[iNumUsedInstruments])) / dStdDev;
        }
      }
      ...
      /* If the Strike is more than MAX_STDEV away from the Forward        ,
      ...
      if (dDistance > CALIBRATE_MAX_STDEV) {
        ...
        continue;
      } else { /* instrument is accepted for calibration */
  ...
  if ((!(*pppdSigmaCurve) || (*plNumSigmas == 0)) &&
    ...
    if (calib_param.bOneTau == SRT_YES) {
      *plNumTaus = 1;
      ...
      err =
          find_sigdates_from_deals(psSwapDp, peOptionType, iNumUsedInstruments,
                                   psCcyParams, &plDealSigmaDates, plNumSigmas);
      if (err) {
        global_new_calib_free;
        return (err);
      }
      ...
      if (eModelDim == ONE_FAC) {
        ...
        err = srt_f_display_IRM_OneFac_TermStruct(
            ts, &plTemSigmaDates, &pdTempSigma, &pdTempBeta, &pdTempVovol,
            &pdTempRho, &pdTempMeanVol, &plTempNumSigmas, &plTempTauDates,
            &pdTempTau, plNumTaus);
        ...
        err = srt_f_display_IRM_TwoFac_TermStruct(
            ts, &plTemSigmaDates, &pdTempSigma1, &pdTempBeta1, &pdTempSigma2,
            &pdTempBeta2, &pdTempRho, &plTempNumSigmas, &plTempTauDates,
            &pdTempTau1, &pdTempTau2, plNumTaus);
      ...
      err = find_dates_from_deals(psSwapDp, peOptionType, iNumUsedInstruments,
                                  psCcyParams, &plDealSigmaDates, plNumSigmas,
                                  &plDealTauDates, plNumTaus);
      if (err) {
        global_new_calib_free;
        return (err);
      }
    ...
    /* Allocation will be done inside with new plNumSigmas */
    err = find_sigdates_from_deals(psSwapDp, peOptionType, iNumUsedInstruments,
                                   psCcyParams, &plDealSigmaDates, plNumSigmas);
    if (err) {
      global_new_calib_free;
      return (err);
    ...
    /* Allocation will be done inside with new plNumSigmas */
    err = find_taudates_from_deals(psSwapDp, iNumUsedInstruments, psCcyParams,
                                   &plDealTauDates, plNumTaus);
    if (err) {
      global_new_calib_free;
      return (err);
  ...
  if (!(*pppdSigmaCurve) || (*plNumSigmas == 0)) {
    ...
    if (eModelDim == TWO_FAC) {
      /* Allocate memory for a FULL Sigma Curve : Dates - Sig1 - Beta1 - Sig2 -
       * Beta2- Rho */
      *plNumSigmaCols = 6;
      *pppdSigmaCurve = dmatrix(0, *plNumSigmaCols - 1, 0, *plNumSigmas - 1);
      if (!(*pppdSigmaCurve)) {
        global_new_calib_free;
        return serror(
            "Memory allocation error (*pppdSigmaCurve) in srt_f_CalibrateAll");
      }
      ...
      for (i = 0; i < *plNumSigmas; i++) {
        ...
        err = find_2f_sig((plDealSigmaDates[i] - today) * YEARS_IN_DAY, ts,
                          &(*pppdSigmaCurve)[1][i], &(*pppdSigmaCurve)[3][i]);
        if (err) {
          global_new_calib_free;
          return (err);
        }
        ...
        err = find_tf_beta((plDealSigmaDates[i] - today) * YEARS_IN_DAY, ts,
                           &(*pppdSigmaCurve)[2][i], &(*pppdSigmaCurve)[4][i]);
        if (err) {
          global_new_calib_free;
          return (err);
        }
        ...
        /* Set value for Rho */
        err = find_2f_rho((plDealSigmaDates[i] - today) * YEARS_IN_DAY, ts,
                          &(*pppdSigmaCurve)[5][i]);
        if (err) {
          global_new_calib_free;
          return (err);
    ...
    if (eModelDim == TWO_FAC) {
      ...
      *pppdTauCurve = dmatrix(0, *plNumTauCols - 1, 0, *plNumTaus - 1);
      if (!(*pppdTauCurve)) {
        global_new_calib_free;
        return serror(
            "Memory allocation error (*pppdTauCurve) in srt_f_CalibrateAll");
      }
      ...
      for (i = 0; i < *plNumTaus; i++) {
        ...
        err = find_2f_tau((plDealTauDates[i] - today) * YEARS_IN_DAY, ts,
                          &(*pppdTauCurve)[1][i], &(*pppdTauCurve)[2][i]);
        if (err) {
          global_new_calib_free;
          return (err);
        }
      ...
      *pppdTauCurve = dmatrix(0, *plNumTauCols - 1, 0, *plNumTaus - 1);
      ...
      for (i = 0; i < *plNumTaus; i++) {
        ...
        /* Set values for Tau */
        (*pppdTauCurve)[1][i] =
            find_tau((plDealTauDates[i] - today) * YEARS_IN_DAY, ts);
  ...
  if (eModelDim == ONE_FAC) {
    ...
    /* If calib is not global        , set num_tenors to 0 and frd weight to
     * 100% */
    if (calib_param.eCalibType != GLOBAL_CALIB) {
      ...
      if (calib_param.dOptionsWeight > 0.99999999) {
        ...
        calib_param.dOptionsWeight =
            sqrt(sin(0.50 * SRT_PI * calib_param.dOptionsWeight));
  ...
  if (eModelType == LGM_STOCH_VOL || eModelType == CHEY_STOCH_VOL ||
    ...
    err = srt_f_calib_stoch_vol(
        &sGrfnParams, eModelType, eModelDim, psSwapDp, pdStrike, pdBondStrike,
        peOptionType, peRecPay, pdPrice, pdVega, pszRefRateCode,
        iNumUsedInstruments, pdFraMaturities, pdCorrelationMatrix, lNumTenors,
        sUndPtr, &calib_param, *pppdSigmaCurve, *plNumSigmas, *plNumSigmaCols,
        *pppdTauCurve, *plNumTaus, *plNumTauCols, pdChiSquare);
    if (err) {
      global_new_calib_free;
      return err;
    }

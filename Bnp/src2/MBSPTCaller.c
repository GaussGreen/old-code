...
//Assume tree times coincide with mtg reset dates, with RefDate (whether settle or spot date does not matter as long as they differ by a small amount)
//and some dates at the end; this means that PPY == 11
//Assume security already seasoned (i.e.. wala >=0) on settle date 
int MBS_BKTree_create_times( MBS_BKTree * tree,
							MBSPT_DealStruct * deal_struct, MBS_Prepay_Engine * prepay_engine )
{
	...
	numTimeSlices = numDuePrdTimes
				+ prepay_engine->EndIndexMat * 12; //strictly, need 1 less than this
	...
	for(i=0; i < numTimeSlices - 1; ++i)
	{
		...
		if(numSubIntervals[i]==0) numSubIntervals[i]=1;
...
)
{
	...
	{
		if(err=MBS_BKTree_Yield_Approx( indices,
			prepay_engine->IndexFreq, prepay_engine->IndexMat,
			NbPer, tree))
		{
			goto CLEANUP;
		}
		...
		for(i=start; i <= end; ++i)
		{
			MBS_Prepay_Engine_SMM( smm_holder, prepay_engine,
				o_wala + NbPer,
				indices[i], mm, NbPer);
			...
			)
{
	...
	if(err=MBS_BKTree_BackwardInduct( io, 1, NbPer, tree )) return(err);
	if(err=MBS_BKTree_BackwardInduct( po, 1, NbPer, tree )) return(err);
	if(NbPer == 0) return(err);
	//
	o_wala = prepay_engine->oterm - cashflowstruct->numCashFlows;
	...
	if( NbPer <= prepay_engine->NbFixedPrepay )
		...
		sch_amort_rate
			= SCHEDULED_AMORT_RATE((prepay_engine->oterm)-(NbPer + o_wala)+1, (prepay_engine->gwac));
	...
	if(NbPer <= cashflowstruct->NbInitialCashFlowToSkip - prepay_engine->Delay) return(err);//has to return only after zeros have been calc and cached!
	NbZerosCached++;
	//arrange for the correct zeros with pp delay and pay delay
	if(NbZerosCached <= 1) return( "Not enough zeros for inter/extra-polate");
	NbZeros = prepay_engine->Delay+1;
	if(NbZeros > cashflowstruct->numCashFlows - NbPer + 1) NbZeros = cashflowstruct->numCashFlows - NbPer + 1;
	...
	for(i=0; i < NbZeros; ++i )
	{
		...
		pos = interp_search(NbZerosCached, tree->tree_times + NbPer, t);//position relative to NbPer
	...
	if(err=MBSPT_AddCashFlowPV( io, po, start, end, coupon/100.0, sch_amort_rate, smm, NbZeros, zeros, NbZerosToSkip ))
	{
...
char * mbs_iopo_initialize_zeros( MBS_BKTree * tree, int UntilNbPer )
{
	...
	{
		if(err=MBS_BKTree_ZeroPrices(&lastFwdIndex, &zeros, 0, i, tree)) return(err);
	}
	return(err);
...

{
	...
	bals[0] = 1.0;
	if(err=get_initial_principal_payments( NbCashFlow, bals + 1,
			wam,
			prepay_engine->gwac, 
	...
	if(NbCashFlow == cashflowstruct->numCashFlows )//if it is the case that after NbCashFlow exhaust all principals
	{
						...
						)
{
	...
	for(j=0; j < prepay_engine->SFSNb; ++j)
	{
		if(err=mbs_iopo_initialize_prices	( io_prices[j], po_prices[j],
...
//compute io po prices backward
	...
	{
		//pre-calc smm
		if(err = MBS_PricesGetSMM( smm,
					i,
					cashflowstruct, prepay_engine, tree ))
				goto CLEANUP;
		if(i== 1)
		...
		for(j=0; j < prepay_engine->SFSNb; ++j)
		{
			if(err=MBS_PricesBackwardInduct(io_prices[j], po_prices[j],
					i, coupon, smm[j], cashflowstruct, prepay_engine, tree ))
				goto CLEANUP;
		}
	}
//do weigted average
	...
	if(err=MBS_IOPO_Price_Adjust(io,po,
			deal_struct, prepay_engine, tree))
		goto CLEANUP;
	//
	...
	df *= exp(-tree->OAS /10000.0
		* (double) day_count_date(
			deal_struct->cashflow_struct.SettleDate,
...
//checks:
	if(Dateok(RefDate)) return("Bad RefDate in Zero_Prepare");
	if( (yield_freq < 0) || (zero_freq < 0) || (interp_freq <=0)) return("Illegal freq in Zero_Prepare");
	if( (mm_delay < 0) || (yield_delay<0)) return("Illegal delays in Zero_Prepare");
////////////////
	...
	//form mm_data
	for(i=0; i < NbMMTerms; ++i) MMYield[i] = rates[i];
	if(err=MM_Data_Construct(&mm_data,
		mm_ybase, mm_diffBase, mm_delay,
		NbMMTerms, mm_date_units, MMTerms, MMYield,
		ValueDate))
		goto CLEANUPMM;

	//form yield_data
	for(i=NbMMTerms; i < NbRates; ++i) yields[i-NbMMTerms] = rates[i];
	if(err=Yield_Data_Construct( &yield_data,
		yield_ybase, yield_diffBase, yield_delay, yield_freq,
		NbRates - NbMMTerms, yield_date_units, yield_terms, yields, ValueDate ))
		goto CLEANUPSWAP;
//strip:
	zero_freq = yield_freq;
	if(err = MBS_Strip((&zero_data),
		&mm_data, &yield_data,
		interp_freq, zero_freq, zero_ybase, ValueDate)) goto CLEANUPSWAP;
	if(err=MBSDF_Data_Construct_FromZeroData(df_data, (&zero_data))) goto CLEANUPSWAP;
	//clean up
CLEANUPSWAP:
						  ...
						  )
{
	...
	if(err=MBSPT_MBSDF_Prepare_Helper(df_data,
			NbRates, rates,
			NbMMTerms, mm_date_units, MMTerms,
			yield_date_units, yield_terms,
			mm_ybasis, mm_diffbasis, mm_delay,
			yield_ybasis, yield_diffbasis, yield_freq, yield_delay,
			zero_freq, interp_freq, RefDate))
		goto CLEANUP;
						  ...
						  )
{
	...
	for(i=0; i < 6; ++i) rates[i] = tsycurve[i];
	for(i=0; i < 8; ++i) rates[i + 6] = tsycurve[i+6] + swpSprd[i]/100.0;

	return(
		MBSPT_MBSDF_Prepare_FromRatesAndFile( df_data, data_dir,
						NbMMTerms + NbYields, rates,
						NbMMTerms, mm_date_units, MMTerms,
						yield_date_units, yield_terms,
						CrvDate));
...
//#endif//TREEREFDATEISSETTLEDATE
	...
	if(err=MBS_BKTree_Construct_FromFile( tree,
		data_dir,
		df_data,
		NbSwaptions, swapmat, swaptions_exp, volcurve, meanrev, inoas, Rate10Y, refisprd,
		TreeRefDate))
		goto CLEANUP;

	if(err=MBSPT_DealStruct_Construct_FromFile( deal_struct,
			data_dir,
			dealmat[0],
			dealmat[1], 
			coupon[0],
	...
	if(err = MBS_Prepay_Engine_Params_Reader( data_dir, prepay_engine, AgcyProg, dealmat[1], coupon[1],
				pptweaks[0], pptweaks[1],
				(int) swapmat) )
			goto CLEANUP;
	...
	if(err=BKTreeCachStruct_Reset( (tree->cach), 
		...
		)
{
	...
	while(it <= max_trial)
	{
		...
		if(err=MBSPT_Structures_Prepare_FromFile(
			&tree,&deal_struct, &prepay_engine,
			data_dir, valuation, FwdSettle, &df_data, refisprd, Rate10Y, AgcyProg,
			NbSwaptions, volcurve, volexp, meanrev,
			swapmat,dealmat, coupon, curr_oas, paydelays,
			pptweaks))
		{
			MBSPT_Structures_Cleanup( &df_data, &tree, &deal_struct, &prepay_engine);
			return(err);
		...
		if(err=MBSPT_Structures_Prepare_FromFile(
			...
			data_dir, valuation, FwdSettle, &df_data, refisprd, Rate10Y, AgcyProg,
			NbSwaptions, volcurve, volexp, meanrev,
			swapmat,dealmat, coupon, curr_oas + oas_inc, paydelays,
			pptweaks))
		{
			MBSPT_Structures_Cleanup( &df_data, &tree, &deal_struct, &prepay_engine);
			return(err);
	...
	if(it == max_trial) return("Solver fails");
	return(err);
...
char * MBS_Prepay_GetIndexValue(double ** indices,
								int endIndex,
								...
								)
{
	...
	{	
			if(err=MBS_BKTree_Yield_Approx( indices[i],
					prepay_engine->IndexFreq, prepay_engine->IndexMat,
					i, tree))
				return(err);
	}
	...
	int IndexMat)
{
	...
	err = MBS_Prepay_Path( &prepay_engine,
						TradeYYYYMMDD, spotYYYYMMDD,
						wam, subgroup,
						num_future_rates, futureRates,
						useFixedPrepay,
	...
	int IndexMat, int EndIndexMat, int IndexFreq)
{
	...
	err = MBS_Prepay_Path( &prepay_engine, 
						TradeYYYYMMDD, spotYYYYMMDD,
						wam, subgroup, num_future_rates, futureRates,
						useFixedPrepay,  
						numProjections, rates, cpr, principal_payments);
	MBS_Prepay_Engine_Cleanup(&prepay_engine);
	return(err);

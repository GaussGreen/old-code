...
// alloc memory for tree_times and assume times  , always tree_times[0] = 0.0
...
// Assume tree times coincide with mtg reset dates  , with RefDate (whether
// settle or spot date does not matter as long as they differ by a small amount)
// and some dates at the end; this means that PPY == 11
// Assume security already seasoned (i.e.. wala >=0) on settle date
int MBS_BKTree_create_times(MBS_BKTree *tree, MBSPT_DealStruct *deal_struct,
  ...
  // actually  , given the way we structure dates  , refisDuePrdEnd is always
  // 0!!
  ...
  numTimeSlices = numDuePrdTimes + prepay_engine->EndIndexMat *
...
// smm[0..prepay_engine->SFSNb-1][0  , tree->NbSpaceNodes-1]  , mem to be alloc
// by user 1% <-> 0.01
...
char *MBS_PricesBackwardInduct(double *io, double *po, int NbPer,
  ...
  // PrepayDelay sch_amort_rate = foreknown amortization rate  , without any
  // delay there is no smm for NbPer in [1..NbFixedPrepay-PrepayDelay] and
...
// filled with zeros
// then start with backward induct at the last slice  , i.e.  , at NbPer ==
// cashflowstruct->numCashFlows user alloc mem
...
// necessary adjustments due to user provided SMM the result are the dirty prices

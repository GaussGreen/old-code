    ...
    */
    ...
    while (iCpn < tarn->deal.nCouponDates)
    {
        ...
        /*		aux_mc->evt_dts[iEvt] = tarn->deal.lvFltrFixingDates[iCpn];	//today +
           DAYS_IN_YEAR * time[i]; aux_mc->evt_tms[iEvt] = ( tarn->deal.lvFltrFixingDates[iCpn] -
           tarn->market.lToday ) /365.0;
        */
        ++iEvt;
        ++iCpn;
    }
    ...
    int*    stop_path)
{
    ...
    */
    ...
    event->fixed_pay =
        exp(event->fixed_pay_logdff + event_2f->fixed_pay_gam12 - event_2f->fixed_pay_gam1 * R1D +
    ...
    event->fltr_end =
        exp(event->fltr_end_logdff + event_2f->fltr_end_gam12 - event_2f->fltr_end_gam1 * R1D +
            -event_2f->fltr_end_gam2 * R2D);

    /* Calculate the coupon */
    dFloater = (event->fltr_start / event->fltr_end - 1.0) / event->fltr_cvg + event->fltr_spread;
    ...
    for (l = 0; l < event->num_fund_df; l++)
    {
        event->fund_start[l] =
            exp(event->fund_start_logdff[l] + event_2f->fund_start_gam12[l] -
                event_2f->fund_start_gam1[l] * R1D + -event_2f->fund_start_gam2[l] * R2D);
        event->fund_end[l] =
            exp(event->fund_end_logdff[l] + event_2f->fund_end_gam12[l] -
...
char* fillTargetNoteFunding2F(
    int iEvt, int iStp, TARN_Struct* tarn, TARN_AUX* aux, TARN_MC_AUX* mc_aux, TARN_EVENT* event)
{
    /* Variable declaration */
    int             k, iCpn;
    char*           dom_yc;
    ...
    if (event->num_fund_df > 0)
    {
        event->fund_start_tms = dvector(
            0, event->num_fund_df - 1);  // xStr.evt[i].evt->dft[fund_idx]; // discount factor times
        event->fund_start_dts = dvector(
            0, event->num_fund_df - 1);  // xStr.evt[i].evt->dfd[fund_idx]; // discount factor dates
        ...
        */
        ...
        for (k = 0; k < event->num_fund_df; k++)
        {
            ...
            event_2f->fund_start_gam12[k] =
                -0.5 *
                    (event_2f->fund_start_gam1[k] * event_2f->fund_start_gam1[k] * dom_phi1[iStp] +
            ...
            event_2f->fund_end_gam12[k] =
                -0.5 * (event_2f->fund_end_gam1[k] * event_2f->fund_end_gam1[k] * dom_phi1[iStp] +
    ...
    TARN_EVENT*  event)
{
    ...
    /* period quantities */
    event->fltr_cvg =
        coverage((long)event->fltr_start_dts, (long)event->fltr_end_dts, tarn->deal.basisFltr);
    ...
    TARN_EVENT*  event)
{
    ...
    event_2f->fixed_pay_gam12 =
        -0.5 * (event_2f->fixed_pay_gam1 * event_2f->fixed_pay_gam1 * dom_phi1[iStp] +
    ...
    event_2f->fltr_start_gam12 =
        -0.5 * (event_2f->fltr_start_gam1 * event_2f->fltr_start_gam1 * dom_phi1[iStp] +
    ...
    event_2f->fltr_end_gam12 =
        -0.5 * (event_2f->fltr_end_gam1 * event_2f->fltr_end_gam1 * dom_phi1[iStp] +
...
char* set_TARN_EVENTS_2F(TARN_Struct* tarn, TARN_AUX* aux, TARN_MC_AUX* mc_aux)
{
    ...
    for (i = aux->i1stCpn; i < tarn->deal.nCouponDates; i++)
    {
        ...
        if (err =
                (*tarn->market
                      .getDF)(tarn->market.szYieldCurve, tarn->deal.lvFltrFixingDates[i], mc_aux_2f->pay_date, &DF))
            goto FREE_RETURN;
        event->numeraire_logdff = log(DF);
        event_2f->numeraire_gam1 =
            (1.0 - exp(-mc_aux_2f->lam * (mc_aux_2f->pay_date - tarn->deal.lvFltrFixingDates[i]) /
                       365.0)) /
            mc_aux_2f->lam;
        event_2f->numeraire_gam2 =
            (1.0 - exp(-mc_aux_2f->lam2 * (mc_aux_2f->pay_date - tarn->deal.lvFltrFixingDates[i]) /
                       365.0)) /
            mc_aux_2f->lam2;
        event_2f->numeraire_gam12 =
            -0.5 * (event_2f->numeraire_gam1 * event_2f->numeraire_gam1 * mc_aux_2f->phi1[iStp] +
...
char* TargetNoteMC_2F(char* szUnd, TARN_Struct* tarn, TARN_AUX* aux, double** prod_val)
{
    /* -----------------------------------------------------------------------------------------------------------------
     */
    /* Variable declaration */
    ...
    /* Add PV of the first funding coupon
            iNum_Fund_DF = aux->ivFundEndIndex[aux->i1stCpn] - aux->ivFundStartIndex[aux->i1stCpn] +
       1; dFundPV = 0.0; for ( i=0; i<iNum_Fund_DF; i++ )
            {
                    dFund_Start_DF = swp_f_df(tarn->market.lToday, tarn->deal.lvFundStartDates[i],
       tarn->market.szYieldCurve); dFund_End_DF = swp_f_df(tarn->market.lToday,
       tarn->deal.lvFundEndDates[i], tarn->market.szYieldCurve); dFund_cvgMargin = 1.0 - coverage(
       tarn->deal.lvFundStartDates[i], tarn->deal.lvFundEndDates[i], tarn->deal.basisFund )
...
char* TargetNoteMC_KO_2F(char* szUnd, TARN_Struct* tarn, TARN_AUX* aux, double** prod_val)
{
    /* -----------------------------------------------------------------------------------------------------------------
     */
    /* Variable declaration */

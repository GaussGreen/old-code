...
/* allocate the memory for the dates, times and event dates and times */
	...
	if ( ! (aux_mc->time = (double*) calloc (aux_mc->nStps, sizeof (double)))  )
		return "Memory allocation error in TARN_TimeSteps_2F";
/*
	if ( ! (aux_mc->evt_dts = (double*) calloc(aux_mc->nEvents, sizeof (double)))  )
		return "Memory allocation error in TARN_TimeSteps_2F";
	if ( ! (aux_mc->evt_tms = (double*) calloc(aux_mc->nEvents, sizeof (double)))  )
		return "Memory allocation error in TARN_TimeSteps_2F";
*/
...
/*		aux_mc->evt_dts[iEvt] = tarn->market.lToday;	//today + DAYS_IN_YEAR * time[i];
		aux_mc->evt_tms[iEvt] = 0.0;
		aux_mc->is_event[iEvt] = 0;
*/		++iEvt;
	}
...
/* Loop over the coupon fixing dates setting the dates and times */
	...
	{		
		aux_mc->time[iEvt] = ( tarn->deal.lvFltrFixingDates[iCpn] - tarn->market.lToday ) /365.0;
		aux_mc->date[iEvt] = tarn->deal.lvFltrFixingDates[iCpn];	//today + DAYS_IN_YEAR * time[i];
/*		aux_mc->evt_dts[iEvt] = tarn->deal.lvFltrFixingDates[iCpn];	//today + DAYS_IN_YEAR * time[i];
		aux_mc->evt_tms[iEvt] = ( tarn->deal.lvFltrFixingDates[iCpn] - tarn->market.lToday ) /365.0;
*/		++iEvt;
		++iCpn;
	}	
...
/* Reconstruct the coupon pay discount factors */
	event->fixed_pay = exp( event->fixed_pay_logdff + event_2f->fixed_pay_gam12 
																	- event_2f->fixed_pay_gam1 * R1D +
																	- event_2f->fixed_pay_gam2 * R2D	);

/* Reconstruct the floater discount factors */
	event->fltr_start = exp( event->fltr_start_logdff + event_2f->fltr_start_gam12 
																- event_2f->fltr_start_gam1 * R1D +
																- event_2f->fltr_start_gam2 * R2D	);
	event->fltr_end = exp( event->fltr_end_logdff + event_2f->fltr_end_gam12 
																- event_2f->fltr_end_gam1 * R1D +
																- event_2f->fltr_end_gam2 * R2D	);
...
/* Calculate the notional to be repaid */
	...
	dNotionalRepay = 1.0 - ( TARGET_NOTE.m_dTarget - dNextCumulCoupon ) / TARGET_NOTE.m_dSpread;
	dNotionalRepay = dNotionalRepay > 1.0 ? 1.0 : dNotionalRepay;
	dNotionalRepay = dNotionalRepay < TARGET_NOTE.m_dCouponNotionalRepaid ? TARGET_NOTE.m_dCouponNotionalRepaid 
																			: dNotionalRepay;
...
/* Call the relevant coupon calculation function */
	switch ( TARGET_NOTE.m_couponType )
	{
		...
		case TN_SWAP:
			...
			break;
		case TN_SWAPKO:
			err = TargetNoteCoupon_SWAPKO( event, dCoupon, dNotionalRepay, res, stop_path );  
			break;
		case TN_FINAL:
			err = TargetNoteCoupon_FINAL( event, dCoupon, dFloater, dNotionalRepay, res, stop_path );  
			break;
		default:
...
/* Reconstruct the start float discount factors */	
	...
	{
		event->fund_start[l] = exp( event->fund_start_logdff[l] + event_2f->fund_start_gam12[l] 
																	- event_2f->fund_start_gam1[l] * R1D +
																	- event_2f->fund_start_gam2[l] * R2D	);
		event->fund_end[l] = exp( event->fund_end_logdff[l] + event_2f->fund_end_gam12[l] 
																	- event_2f->fund_end_gam1[l] * R1D +
																	- event_2f->fund_end_gam2[l] * R2D	);
	}

/* Calculate the funding */
	dFloatPV = 0.0;
	for ( l=0; l<event->num_fund_df; l++ )
		dFloatPV += event->fund_start[l] - event->fund_cvgMargin[l] * event->fund_end[l];
...
/* check if it has knocked out */
	if ( TARGET_NOTE.m_dCumulCoupon >= TARGET_NOTE.m_dTarget )
	{
		...
		res[ event->iEventDate ] = exp( event->numeraire_logdff + event_2f->numeraire_gam12 
																	- event_2f->numeraire_gam1 * R1D +
																	- event_2f->numeraire_gam2 * R2D	);
		*stop_path = 1;
	}
...
/* Variable declaration */
	...
	long lEvent;
	long* lvFundEndIndex = aux->ivFundEndIndex, *lvFundStartIndex = aux->ivFundStartIndex;
...
/* Set up the target note */
	...
	{
		event->fund_start_tms = dvector (0, event->num_fund_df-1); //xStr.evt[i].evt->dft[fund_idx]; // discount factor times 
		event->fund_start_dts = dvector (0, event->num_fund_df-1); //xStr.evt[i].evt->dfd[fund_idx]; // discount factor dates
		...
		event->fund_end_tms = dvector (0, event->num_fund_df-1); //xStr.evt[i].evt->dft[fund_idx]; // discount factor times 
		event->fund_end_dts = dvector (0, event->num_fund_df-1); //xStr.evt[i].evt->dfd[fund_idx]; // discount factor dates
...
/* Add error handling in the even of memory problems		
		if (!event->dom_dff || !event->dom_gam1 || !event->dom_gam2 || !event->dom_gam12)
		{
...
/* start of the float period quantities */
			...
			event->fund_start_logdff[k] = log( swp_f_df(lEvent, event->fund_start_dts[k], (char*) dom_yc)  );

			event_2f->fund_start_gam1[k] = (1.0 - exp (-lam_dom * event->fund_start_tms[k] )) / lam_dom;
			event_2f->fund_start_gam2[k] = (1.0 - exp (-lam_dom2 * event->fund_start_tms[k] )) / lam_dom2;
			event_2f->fund_start_gam12[k] = -0.5 * (event_2f->fund_start_gam1[k] * event_2f->fund_start_gam1[k] * dom_phi1[iStp] +
											 event_2f->fund_start_gam2[k] * event_2f->fund_start_gam2[k] * dom_phi2[iStp])
										- event_2f->fund_start_gam1[k] * event_2f->fund_start_gam2[k] * dom_phi12[iStp];						
/* end of the float period quantities */
			...
			event->fund_end_logdff[k] = log( swp_f_df(lEvent, event->fund_end_dts[k], (char*) dom_yc)  );
			event_2f->fund_end_gam1[k] = (1.0 - exp (-lam_dom * event->fund_end_tms[k] )) / lam_dom;
			event_2f->fund_end_gam2[k] = (1.0 - exp (-lam_dom2 * event->fund_end_tms[k] )) / lam_dom2;

			event_2f->fund_end_gam12[k] = -0.5 * (event_2f->fund_end_gam1[k] * event_2f->fund_end_gam1[k] * dom_phi1[iStp] +
											 event_2f->fund_end_gam2[k] * event_2f->fund_end_gam2[k] * dom_phi2[iStp])
										- event_2f->fund_end_gam1[k] * event_2f->fund_end_gam2[k] * dom_phi12[iStp];						
/* period quantities */
			event->fund_cvgMargin[k] = 1.0  - aux->dvFundCvg[iFund] 
													* ( tarn->deal.dvFundMargin[iFund] + tarn->deal.dvFundSpread[iFund] );
		}
...
/* payment details */
	...
	event->fixed_pay_logdff = log( swp_f_df(dDFdate, event->fixed_pay_dts, (char*) dom_yc) );
...
/* floater details:  start date */
	...
	event->fltr_start_logdff = log( swp_f_df(dDFdate, event->fltr_start_dts, (char*) dom_yc) );

/* floater details:  end date */
	event->fltr_end_dts = tarn->deal.lvFltrEndDates[i]; 
	event->fltr_end_tms = ( event->fltr_end_dts - lEvent ) / 365.0;
	event->fltr_end_logdff = log( swp_f_df(dDFdate, event->fltr_end_dts, (char*) dom_yc) );
	event->fltr_spread = tarn->deal.dvFltrSpread[i];

/* period quantities */
	event->fltr_cvg = coverage( (long) event->fltr_start_dts, (long) event->fltr_end_dts, tarn->deal.basisFltr);
...
char* fillTargetNoteCoupon2F(
					int iStp, /* time step, not event index */
					TARN_MC_AUX* mc_aux,
					TARN_EVENT* event )
{
/* Variable declaration */
...
/* payment details */
	event_2f->fixed_pay_gam1 = (1.0 - exp (-lam_dom * event->fixed_pay_tms )) / lam_dom;
	event_2f->fixed_pay_gam2 = (1.0 - exp (-lam_dom2 * event->fixed_pay_tms )) / lam_dom2;

	event_2f->fixed_pay_gam12 = -0.5 * (event_2f->fixed_pay_gam1 * event_2f->fixed_pay_gam1 * dom_phi1[iStp] +
									 event_2f->fixed_pay_gam2 * event_2f->fixed_pay_gam2 * dom_phi2[iStp])
								- event_2f->fixed_pay_gam1 * event_2f->fixed_pay_gam2 * dom_phi12[iStp];						

	
/* floater details:  start date */
	event_2f->fltr_start_gam1 = (1.0 - exp (-lam_dom * event->fltr_start_tms )) / lam_dom;
	event_2f->fltr_start_gam2 = (1.0 - exp (-lam_dom2 * event->fltr_start_tms )) / lam_dom2;
	event_2f->fltr_start_gam12 = -0.5 * (event_2f->fltr_start_gam1 * event_2f->fltr_start_gam1 * dom_phi1[iStp] +
									 event_2f->fltr_start_gam2 * event_2f->fltr_start_gam2 * dom_phi2[iStp])
								- event_2f->fltr_start_gam1 * event_2f->fltr_start_gam2 * dom_phi12[iStp];						
/* floater details:  end date */
	event_2f->fltr_end_gam1 = (1.0 - exp (-lam_dom * event->fltr_end_tms )) / lam_dom;
	event_2f->fltr_end_gam2 = (1.0 - exp (-lam_dom2 * event->fltr_end_tms )) / lam_dom2;
	event_2f->fltr_end_gam12 = -0.5 * (event_2f->fltr_end_gam1 * event_2f->fltr_end_gam1 * dom_phi1[iStp] +
...
char* set_TARN_EVENTS_2F( TARN_Struct* tarn, TARN_AUX* aux, TARN_MC_AUX* mc_aux )
{
/* Variable declaration */
	int iStp, i;
	char* err = 0;
...
/* Fill the numeraire info */
		event_2f = (TARN_EVENT_2F*) event->ptrModel;
		if ( err = (*tarn->market.getDF)(tarn->market.szYieldCurve, tarn->deal.lvFltrFixingDates[i], mc_aux_2f->pay_date, &DF ) )
			goto FREE_RETURN;							
		event->numeraire_logdff = log(DF);
		event_2f->numeraire_gam1 = (1.0 
			- exp (-mc_aux_2f->lam * (mc_aux_2f->pay_date - tarn->deal.lvFltrFixingDates[i])/365.0 )) / mc_aux_2f->lam;
		event_2f->numeraire_gam2 = (1.0 
			- exp (-mc_aux_2f->lam2 * (mc_aux_2f->pay_date - tarn->deal.lvFltrFixingDates[i])/365.0 )) / mc_aux_2f->lam2;
		event_2f->numeraire_gam12 = -0.5 * (event_2f->numeraire_gam1 * event_2f->numeraire_gam1 * mc_aux_2f->phi1[iStp] +
									 event_2f->numeraire_gam2 * event_2f->numeraire_gam2 * mc_aux_2f->phi2[iStp])
								- event_2f->numeraire_gam1 * event_2f->numeraire_gam2 * mc_aux_2f->phi12[iStp];						
...
/* set up the MC aux structure */
	...
	if ( err = alloc_TARN_MC_AUX( tarn, aux, mc_aux ) ) goto FREE_RETURN;
...
/* Calculate the initial value of the numeraire and discount the payoffs */
	df = swp_f_zr (tarn->market.lToday, mc_aux_2f->pay_date, tarn->market.szYieldCurve );
...
/* Add PV of the first funding coupon
	iNum_Fund_DF = aux->ivFundEndIndex[aux->i1stCpn] - aux->ivFundStartIndex[aux->i1stCpn] + 1;
	dFundPV = 0.0;
	for ( i=0; i<iNum_Fund_DF; i++ )
	{
		dFund_Start_DF = swp_f_df(tarn->market.lToday, tarn->deal.lvFundStartDates[i], tarn->market.szYieldCurve);
		dFund_End_DF = swp_f_df(tarn->market.lToday, tarn->deal.lvFundEndDates[i], tarn->market.szYieldCurve);
		dFund_cvgMargin = 1.0 - coverage( tarn->deal.lvFundStartDates[i], tarn->deal.lvFundEndDates[i], tarn->deal.basisFund ) 
									* ( tarn->deal.dvFundMargin[i] + tarn->deal.dvFundSpread[i] );
		dFundPV += dFund_Start_DF - dFund_cvgMargin * dFund_End_DF;
	}
...
/* check the time */
	...
	err = mc_main_lgm2f(	
					/*	Time data */
					tarn->calibration.num_prob_paths,
					num_col,
					mc_aux->time,
					mc_aux->date,
					mc_aux->nStps,

...
double srt_f_optscdinn(double fwdx, double fwdy, double spoty, double strike,
  ...
  premium -= srt_f_optscdout(fwdx, fwdy, spoty, strike, barrier, sigx, sigy,
  ...
  case DELTAX:
    shift = fwdx / 10000;
    result =
        (srt_f_optscdinn(fwdx + shift, fwdy, spoty, strike, barrier, sigx, sigy,
                         rho, mat1, mat2, disc, call_put, down_up, PREMIUM) -
         premium) /
        shift;
    return (result);
  ...
  case DELTAY:
    shift = fwdy / 10000;
    result = (srt_f_optscdinn(fwdx, fwdy + shift, spoty + shift, strike,
  ...
  case GAMMAY:
    shift = fwdy / 1000;
    result = srt_f_optscdinn(fwdx, fwdy + shift, spoty * (1 + shift / fwdy),
                             strike, barrier, sigx, sigy, rho, mat1, mat2, disc,
                             call_put, down_up, PREMIUM);
    result += srt_f_optscdinn(fwdx, fwdy - shift, spoty * (1 - shift / fwdy),
                              strike, barrier, sigx, sigy, rho, mat1, mat2,
  ...
  case VEGAX:
    shift = GVOPT.vol_add;
    result =
        (srt_f_optscdinn(fwdx, fwdy, spoty, strike, barrier, sigx + shift, sigy,
                         rho, mat1, mat2, disc, call_put, down_up, PREMIUM) -
         premium) /
        shift;
    return (result);
  ...
  case VEGAY:
    shift = GVOPT.vol_add;
    result =
        (srt_f_optscdinn(fwdx, fwdy, spoty, strike, barrier, sigx, sigy + shift,
                         rho, mat1, mat2, disc, call_put, down_up, PREMIUM) -
         premium) /
        shift;
    return (result);
  ...
  case THETA:
    shift = YEARS_IN_DAY;
    result = srt_f_optscdinn(fwdx, fwdy, spoty, strike, barrier, sigx, sigy,

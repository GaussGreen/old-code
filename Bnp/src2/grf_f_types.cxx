...
/* Checks there is at least one cell in the tableau        , and that the dates
 * are increasing strictly */
...
/* Check from the bottom of the tableau that there are no empty rows; if YES:
 * remove them */
static Err grf_check_and_rem_null_rows(long *nrows, long ncols,
                                       GrfnCell **sprdsht) {
  long i, j;
  int null_row;
  Err err = NULL;
...
/* -------------------------------------------------------------------------------------
 FUNCNAME        :GrfnCellmatrix
  ...
  zero.  If the sval field is allocated        , the SRT_Boolean field
 str_alloced will be set to SRT_YES; otherwise it will be set to SRT_NO. Returns
 NULL if it fails. DESCRIPTION     : MODIFIES        : CALL            :
...
/* --------------------------------------------------------------------------
  ...
  DESCRIPTION:      checks the tableau dates        , null rows and at least one
  cell if NULL rows are detected: removes them
  --------------------------------------------------------------------------- */

Err grfn_check_tableau_and_dates(long *nrows, long ncols, GrfnCell **sprdsht,
                                 Date *eventdates) {
  Err err = NULL;
  ...
  /*  Check that the tableau is not empty        , check the dates are
   * increasing */
...
Err grfn_copy_to_GrfnDeal(GrfnDeal *gd, long nrows, long ncols,
                          GrfnCell **sprdsht, Date *eventdates, long numgrng,
                          GrfnRng *grng, long auxwidth, long *auxlen,
                          double **aux) {
  int i, j;
  long auxtoalloc = 0;
  ...
  for (i = 0; i < nrows; i++) {
    for (j = 0; j < ncols; j++) {
      if (gd->gcells[i][j].sval != NULL &&
          gd->gcells[i][j].str_alloced == SRT_YES) {
        gd->gcells[i][j].sval = new_string(gd->gcells[i][j].sval);
      }
    }
  }

  /* Allocates space for the GrfnCell values in the GrfnDeal (values of the
   * cells once evaluated)  */

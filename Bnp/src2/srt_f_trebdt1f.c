...
static Err bdttrelim(SrtStpPtr stp, SrtTreStpInf* maxtrinf)
{
    ...
    while (cur->next)
    {
        ...
        trinf->xmin = -cur->index * delta_x + log(sam_get(tminf->fwd_sam, 0, F_0_t));
        cur         = cur->next;
    }
...
static void bdtdrfatsam(SrtStpPtr stp, SrtSample* cur_sam, SrtSample* drift_sam, double theta)
{
    double       mu;
    SrtIRMTmInf* tminf;
    ...
    samptr_get(drift_sam, 0, STATEVAR) = samptr_get(cur_sam, 0, STATEVAR) + mu * stp->delta_t;
    return;
}
...
static void generate_prob(
    SrtStpPtr stp, SrtTrinTreNdInf* node, double varatsam, double adj, double cur_x, double* next_x)
{
    double dt, r;
...
static void calc_prob_and_Q(
    SrtStpPtr stp, double* cur_x, double* cur_Q, double adj, double* next_x, double* next_Q)
{
    ...
    SrtUndInfo*   und_info)
{
    ...
    cur_assets  = dmatrix(maxtrinf.min_x_index, maxtrinf.max_x_index, 0, grfnparam->node_dim - 1);
    next_assets = dmatrix(maxtrinf.min_x_index, maxtrinf.max_x_index, 0, grfnparam->node_dim - 1);
    ...
    for (stp = nxtstp, stp_num = 1; stp->next->next != NULL; stp = stp->next, stp_num++)
    {
        trinf     = stp->trinf;
        tminf     = stp->tminf[0];
        nxtstp    = stp->next;
    ...
    for (stp = stp->prev, stp_num--; stp != NULL; stp = stp->prev, stp_num--)
    {
        ...
        for (i = trinf->min_x_index; i <= trinf->max_x_index; i++)
        {
            ...
            srt_f_trintredcntvec(stp, cur_assets[i], &node, next_assets, grfnparam->node_dim);
            err = evalcf(
                (GrfnEvent*)stp->e,
                &node.cur_sam,
                gd,
    ...
    strncpy(io_request_val_p->computation_origin, "Tre_BDT_1F", strlen("Tre_BDT_1F"));
    io_request_val_p->dval = price;

    /** free the world **/

    free_dmatrix(
        cur_assets, maxtrinf.min_x_index, maxtrinf.max_x_index, 0, grfnparam->node_dim - 1);
    free_dmatrix(
        next_assets, maxtrinf.min_x_index, maxtrinf.max_x_index, 0, grfnparam->node_dim - 1);

...
Err srt_f_trebdt1f(
  ...
  for (i = trinf->min_x_index; i <= trinf->max_x_index; i++) {
    ...
    err =
        evalcf((GrfnEvent *)stp->e, &node.cur_sam, gd, (double *)cur_assets[i],
  ...
  for (stp = stp->prev, stp_num--; stp != NULL; stp = stp->prev, stp_num--) {
    ...
    gen_x(stp, cur_x);
    /* In BK model  , variance of X is only a function of t and not X  , hence
       it needs to be calculated only once per step */

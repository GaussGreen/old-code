...
/* Main call to the pricing function */
	err = swp_f_Swaption_SwapDP(&swapdp, vol, strike, recPay, refRateCode, 
				ycName, greek, logNorm, result);
	if (err)
		return err;
...
/* ----------------------------------------------------------------------------------- */
/* Compute the implied volatility given the price of a swaption (for use outside SROT) */

Err swp_f_SwaptionImpliedVol( 
	double     premium,
	long       start,
	long       end_nfp,
	String     compStr,
...
/* ----------------------------------------------------------------------------------- */
/* ----------------------------------------------------------------------------------- */
/* Compute the implied volatility given the price of a swaption (for use inside SROT) */
...
/* Compute the intrinsic value of the option, with a null volatility */
	err = swp_f_Swaption_SwapDP( swapdp, 0.0, strike, recPay, refRateCode,
					ycName, PREMIUM, logNorm, &intrinsic);
	if (err)
		return err;
...
/* Compute the BS implied vol of the renormalised premium */
	err = srt_f_optimpvol	( bs, forward, strike, mat, 1.0, call_put, logNorm, impvol);
	if (err)
...
/* ------------------------------------------------------------------------------
   Static function used to integrate numerically the cash settled swaption payoff
   value under Qlevel (in terms of level)
   ------------------------------------------------------------------------------ */
...
long      i;
	...
	if ( CS_NORM_LOG == SRT_NORMAL)
	{
		fwd_swap_rate = CS_DRIFT_FWD_SWAP_RATE + (CS_STDEV * x);
	}

/* The cash level is the sum of cf discounted with the swap rate (with broken period) */
...
/* The swap level is computed through the swap df and the zero rate (spread constant) */
...
Err swp_f_CashSettledSwaption_SwapDP(
		SwapDP             *swapdp, 
		double             vol, 
		double             strike,
		...
		double             *answer)
{
double          fwd_cash_level, swap_level, fwd_swap_rate, fwd_df, fwd_zero_rate, fwd_spread;
...
/* Get the forward swap rate */
	if (err = swp_f_ForwardRate_SwapDP( swapdp, ycname, refRateCode, &fwd_swap_rate))
		return err;
...
/*	swap = make_SimpleSwap(swapdp, 1.0, 0.0, 0.0, today, SWAP);
	...
	{
		alpha = coverage(swap.dl.date[0], swap.dl.date[1], swapdp->basis_code);
		alpha /= coverage(swap.dl.prev, swap.dl.date[1], swapdp->basis_code);  
		alpha *= (double) swapdp->compd; 
	}
	else
	{
		alpha = 1.0;
...
/* Compute the forward cash settled level, using the forward swap rate to discount */
...
/* The Zero Coupon rate: df = 1/(1+zr/freq) ^ ( alpha + n ) */
	...
	fwd_zero_rate = ( pow( fwd_df, -1.0 / (alpha + nfp) ) - 1.0 ) * (double)swapdp->compd;
...
/* Sets a few option parameters */
	fixing_date = add_unit(swapdp->start, -swapdp->spot_lag, SRT_BDAY, SUCCEEDING);
...
/*		CS_NFP = (long) swapdp->nfp; */
	...
	/* Integrate numerically E[(S-K)+ * fwd_cash_level/fwd_swap_level] */
		...
		integration = sm_qsimp	( cash_swaption_payoff, lower_bound, upper_bound, precision );
...
/* Gets the swap yield curve to extract some date information: spot lag... */
	...
	err = swp_f_CashSettledSwaption_SwapDP( &swapdp, vol, strike,  rec_pay,  
				refRateCode, ycname, greek,log_or_norm, precision, answer);
	if (err)
		return err;
...
/* Compute the CMSrate */
    if (err = swp_f_cmsratefwd(swapdp,
							refRateCode,
							vol, 
							settle_date, 
							DEFAULT_CMS_DELTA, 
...
/*		CS_NFP = (long) swapdp->nfp; */
		...
		{
			QCS_DRIFT_FWD_SWAP_RATE = fwd_swap_rate + correlationfxcms * volfx * vol * mat;
		}
	...
	/* Integrate numerically E[(S-K)+ * fwd_cash_level/fwd_swap_level] */
		...
		integration = sm_qsimp	( quanto_cash_swaption_payoff, lower_bound, upper_bound, precision );
		integration *= INV_SQRT_TWO_PI;
...
Err swp_f_QuantoCashSettledSwaption(
		long      start,
		long      nfp,
		String    strComp,
		String    strBasis,
...
/* Gets the swap yield curve to extract some date information: spot lag... */
	...
	err = swp_f_QuantoCashSettledSwaption_SwapDP( &swapdp, vol, strike, correlationfxcms, volfx, rec_pay,  
				refRateCode, ycname, greek,log_or_norm, precision, answer);
	if (err)
		return err;
...
Err swp_f_QuantoSwaptionPrice(double				dFwdSwapRate,
							  double				dStrike,
							  SrtCallPutType		SrtPayRec,
							  Date					dToday,
							  Date					dDomExerciceDate,
							  Date					dForFixingDate,
							  long					dStartDate,
							  Date					*pdEndDates,
							  double				*pdCvg,
							  int					iNumDates,
							  double				dCmsNumOfPeriods,				
							  SrtCompounding		SrtForFrequency,
							  SrtDiffusionType		SrtDomVolType,
							  SrtDiffusionType		SrtForVolType,
							  double				*pdForCmsVol,
							  double				*pdVolForwardFx,
							  double				*pdCorrelationCmsForwardFx,
							  long					lNumOfVolsAndCorrels,
							  char					*sDomYCName,
							  SRT_Boolean				bAdjustCmsVol,
							  double				*dQuantoSwaption)
{
Err					err    = NULL;
SrtCurvePtr			SrtDomYCurve;
double				dOptionMaturity, dCmsMaturity,
					dAdjRate = 0.0, dCoupon = 0.0, dCmsVol = 0.0,
					dCmsOptPremium = 0.0;
int 				i, iIndex;
	...
	for(i = 0; i < iNumDates; i++)
	{
		...
		if ( err= swp_f_cmsrate(dFwdSwapRate,
								dCmsNumOfPeriods, 
								SrtForFrequency,
								pdForCmsVol[iIndex], 
								dCmsMaturity,
		...
		{
			if (SrtPayRec == SRT_PAYER)
			{	/* Foreign Receiver CMS Option, */
				if(err= swp_f_cmsoption(dFwdSwapRate,
										dCmsNumOfPeriods, 
										SrtForFrequency,
										dStrike,
										pdForCmsVol[iIndex], 
				...
				/* Compute the correct vol for the Cms quanto adjustment */
					if (err = srt_f_optimpvol(dCmsOptPremium, dAdjRate, dStrike, 
											  dCmsMaturity, 1, SrtPayRec, 
											  SrtDomVolType, &dCmsVol))
							return err;
			}
			else
			{	/* Foreign Payer CMS Option, strike at Forward */
				if(err= swp_f_cmsoption(dFwdSwapRate,
										dCmsNumOfPeriods, 
										SrtForFrequency,
										dStrike,
				...
				/* Compute the correct vol for the Cms quanto adjustment */
				if (err = srt_f_optimpvol(dCmsOptPremium, dAdjRate, dStrike, dCmsMaturity, 
										  1, SrtPayRec, SrtDomVolType, &dCmsVol))
						return err;
				}
		}
		else
		...
		{
			dAdjRate *= exp(- pdCorrelationCmsForwardFx[iIndex] * pdVolForwardFx[iIndex] 
							* dCmsVol * dOptionMaturity);
...
Err swp_f_QuantoSwaption(double				dFwdSwapRate,
						 double				dStrike,
						 char				*sPayRec,
						 long				dStartDate,
						 long				dEndDate,
						 double				dCmsNumOfPeriods,				
						 char				*sDomCompounding,
						 char				*sDomBasis,
						 char				*sForCompounding,
						 SrtDiffusionType	SrtDomVolType,
						 SrtDiffusionType	SrtForVolType,
						 double				*pdForCmsVol,
						 double				*pdVolForwardFx,
...
SrtCompounding		SrtForFrequency;
double				*pdCvg;
Date				dCalcDate, dDomFixingDate, dForFixingDate,
					*pdStartDates, *pdEndDates, *pdPayDates;
...
SrtCallPutType		SrtPayRec;
	...
	/* interp all strings */
	if (err = interp_compounding(sForCompounding, &SrtForFrequency)) return err;
	if (err = interp_rec_pay(sPayRec, &SrtPayRec)) return err;

	/* Get today's date from the yield curve */
	dCalcDate = get_clcndate_from_yldcrv(SrtDomYCurve);

	/* Create the swap dp */
	if (err = swp_f_initSwapDP(dStartDate, dEndDate, 
							   sDomCompounding, sDomBasis, 
							   &SrtSwapdp))
		return err;
	...
	swp_f_make_FixedLegDatesAndCoverages(&SrtSwapdp, dCalcDate, &pdPayDates, &iNumPayDates,
										 &pdStartDates, &pdEndDates, &pdCvg, &iNumDates);
	...
	/* Get the maturity of the option: from today to fixing date */
	dDomFixingDate = add_unit(SrtSwapdp.start, - (SrtSwapdp.spot_lag), 
						   SRT_BDAY, SUCCEEDING);

	dForFixingDate = add_unit(SrtSwapdp.start, 
							  - get_spotlag_from_curve(SrtForYCurve),
							  SRT_BDAY, SUCCEEDING);

	err = swp_f_QuantoSwaptionPrice(dFwdSwapRate,
									dStrike,
									SrtPayRec,
									dCalcDate,
									dDomFixingDate,
									...
									pdForCmsVol,
									pdVolForwardFx,
									pdCorrelationCmsForwardFx,
									lNumOfVolsAndCorrels,
									sDomYCName,
									bAdjustCmsVol,
									dQuantoSwaption);

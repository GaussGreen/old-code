    ...
    double* result)
{
    ...
    err = swp_f_Swaption_SwapDP(
        &swapdp, vol, strike, recPay, refRateCode, ycName, greek, logNorm, result);
    if (err)
        return err;
    ...
    double* impvol)
{
    ...
    err = swp_f_SwaptionImpliedVol_SwapDP(
        premium, &swapdp, strike, recPay, refRateCode, ycName, logNorm, impvol);
    if (err)
        return err;
    ...
    double*          impvol)
{
    ...
    err = swp_f_Swaption_SwapDP(
        swapdp, 0.0, strike, recPay, refRateCode, ycName, PREMIUM, logNorm, &intrinsic);
    if (err)
        return err;
    ...
    double*          answer)
{
    ...
    swp_f_make_FixedLegDatesAndCoverages(
        swapdp, today, &pPay_date, &num_pay_date, &pStart_date, &pEnd_date, &pCvg, &num_date);
    ...
    double* answer)
{
    ...
    err = swp_f_CashSettledSwaption_SwapDP(
        &swapdp, vol, strike, rec_pay, refRateCode, ycname, greek, log_or_norm, precision, answer);
    if (err)
        return err;
    ...
    double*          dQuantoSwaption)
{
    ...
    double      dOptionMaturity, dCmsMaturity, dAdjRate = 0.0, dCoupon = 0.0, dCmsVol = 0.0,
                                          dCmsOptPremium = 0.0;
    int i, iIndex;
    ...
    for (i = 0; i < iNumDates; i++)
    {
        ...
        {
            dAdjRate -= pdCorrelationCmsForwardFx[iIndex] * pdVolForwardFx[iIndex] * dCmsVol *
                        dOptionMaturity;
            dCoupon = srt_f_optblknrm(
                dAdjRate, dStrike, dCmsVol, dOptionMaturity, 1.0, SrtPayRec, PREMIUM);
        }
        else
        {
            dAdjRate *=
                exp(-pdCorrelationCmsForwardFx[iIndex] * pdVolForwardFx[iIndex] * dCmsVol *
                    dOptionMaturity);
            dCoupon = srt_f_optblksch(
                dAdjRate, dStrike, dCmsVol, dOptionMaturity, 1.0, SrtPayRec, PREMIUM);
        }
    ...
    double*          dQuantoSwaption)
{
    ...
    SrtCompounding SrtForFrequency;
    double*        pdCvg;
    Date dCalcDate, dDomFixingDate, dForFixingDate, *pdStartDates, *pdEndDates, *pdPayDates;
    ...
    /* Create the swap dp */
    if (err = swp_f_initSwapDP(dStartDate, dEndDate, sDomCompounding, sDomBasis, &SrtSwapdp))
        return err;
    ...
    /* Get the maturity of the option: from today to fixing date */
    dDomFixingDate = add_unit(SrtSwapdp.start, -(SrtSwapdp.spot_lag), SRT_BDAY, SUCCEEDING);

    dForFixingDate =
        add_unit(SrtSwapdp.start, -get_spotlag_from_curve(SrtForYCurve), SRT_BDAY, SUCCEEDING);

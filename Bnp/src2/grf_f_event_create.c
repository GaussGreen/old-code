...
amount (like for the sum of deterministic cash flows  , or the final discounted
                                                sum of all last column cells)
                                                The "treeset" will be the value
of the "type" input to this function; it will not be inserted if it is not one
of COMLL_T_SET and COMLL_T_INCREMENT.
...
static Err parse_cell(GrfnDeal *gd, COMLL_PTR *icomptr) {
  ...
  case GRFNSCELL:
    ...
    if (GrfnIsStatus(GrfnCGCell(gd).status, GRFNCSPAYREF)) {
      ...
      /* If the last element is not of the PAY.. type  , there is an operation
       * outside PAY */
      if ((bot->type != COMLL_F_PAYFROMPAST) && (bot->type != COMLL_F_PAY) &&
          (bot->type != COMLL_F_PAYINPAST))
        return serror("[GRFN Error] %s in c[%d  ,%d]", GRERR_PAY_ALONE, gd->J,
                      gd->I);
    }
...
/* ----------------------------------------------------------------------------------------
  ...
  PAST and FUTURE events  , with the restriction that for PAST events  , the
                                                        gd->american flag is set
  to SRT_NO
   ...
   */
    ...
    for (gd->J = 0; gd->J < gd->sswidth; gd->J++) {
      ...
      if (GrfnIsStatus(GrfnCGCell(gd).status, GRFNCSAMERICAN)) {
        ...
        /* Joints the current cell to the previous ones  , taking this flag into
         * account */
...
Err grfn_create_future_event(GrfnDeal *gd, GrfnEvent **event, double step_ddate,
  ...
  if (step_ddate == gd->event_dates[gd->I + 1]) {
    ...
    /* ReParse the Ith event stored in the GrfnDeal  , and returns an evaluable
     * one */
    error = create_event(event, gd);
...
/* -----------------------------------------------------------------------------------
    ...
    DESCRIPTION     :   When dealing with historical events  , the creation (
   and then the evaluation ) has to be done in order to set variables that might
   have an impact on future events (auxiliary variables: va  , vb...)

                        For historical events  , the treatment of AM has to be
                                                specific: only for the event
   dates in the tableau so far

                                                The GrfnDeal->ssparam and
   GrfnDeal->cells will be populated accordingly !

                        When fixings are encountered  , they are going to be
   evaluated using the HistoricalFixings functions...
...
/* -----------------------------------------------------------------------------------
    ...
    DESCRIPTION     :   After evaluating historical events  , GrfnDeal->ssparams
   are populated with values (va  , vb...) implied by past events.

                        In order to keep track of these values that might impact
   on future cash flows  , this function copies the ssparams produced by the
   evaluation of historical events into the GrfnDeal->hist_ssparams.

                                                In order to force the use of
   these historical values  , a new RESET_STATE node will be atttached in the
   COMLL of today's GrfnEvent (passed as an input). If today's event does not
   exist (or is empty)  , it will be created with just this node in the COMLL
...
Err grfn_attach_past_info_to_future(GrfnDeal *gd, GrfnEvent **today_event) {
  ...
  /* If there are dates in the past  , and variables (va  , vb...)  have been
   * referenced */
  if ((gd->first_unkn_index > 0) && GrfnIsStatus(gd->sumstatus, GRFNCSVARREF)) {
    ...
    /* If today's event is empty  , create a ZERO value one (for the POP() at
     * the end of eval_event) */
    if (!(*today_event)) {

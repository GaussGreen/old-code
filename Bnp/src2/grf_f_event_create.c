...
*                       the same row of a GRFN Tableau.
                    ...
                    NB: The obscure rule of the GRFN Language says that if 
						...
						of the tableau (with the same column reference or NOT) 
						then the value being discounted through the tree is 
						REPLACED by the value at that cell after evaluation
						(like American style option to represent an exercise,
						or a choice depending on an otpion value)
						otherwise it is INCREMENTED by that amount (like for the
						sum of deterministic cash flows, or the final discounted
						sum of all last column cells)
						The "treeset" will be the value of the
                        "type" input to this function; it will not be inserted
                        if it is not one of COMLL_T_SET and COMLL_T_INCREMENT.
...
static COMLL_PTR join_adjacent_cells (  COMLL_PTR   c1, 
                                        COMLL_PTR   c2, 
                                        COMLLType   type, 
                                        int         i,
                                        int         j, 
...
/*..Main Body..*/
 ...
 /* Always ends the first cell COMLL by a POP comll to remove all value from the stack */
	if (c1 != NULL)
	{
        ...
        c2 = comll_join(c1, c2);
		temp = c2;
		while (temp->nextcol) temp = temp->nextcol;
		temp->nextcol = temp1;
    }
...
static Err parse_cell  (   GrfnDeal    *gd,
                           COMLL_PTR   *icomptr    )
{
...
/* Set the following GLOBAL variable to SRT_NO:
	...
	/* Cell contains nothing */
		...
		/* If parser found PAY in the cell, make sure nothing ele is hanging around outside it in the cell */
...
/* ----------------------------------------------------------------------------------------
    ...
    DESCRIPTION     :   For each event date do the following:
						...
						o   This function is called for PAST and FUTURE events,
						    with the restriction that for PAST events, the
							gd->american flag is set to SRT_NO

     Modification    :   Eric Fournie
						Modification de la liste comll grfn qui va maintenant contenir 
						un  champ permettant de se deplacer par colonne lors de l'evaluation.

  ---------------------------------------------------------------------------------------- */


static Err create_event (   GrfnEvent   **new_event_ptr, 
                            GrfnDeal    *gd             )
{
/*..Local variables..*/
...
/* If it is not an American event */
	...
	/* Parse and links all cells in the current row (i.e. column 0 to n) */
		...
		/* Adds the current cell COMLL to the already existing one of the event (for previous cells) */
			new_event->icom = join_adjacent_cells(	new_event->icom,
													icom,
													lattice_flg,
													gd->I,
													gd->J,
...
/* If it is an American event (this will not be called for historical events) */
	...
	/* Parse all cells in the current row (i.e. column 0 to n) */
		...
		/* Make sure that there is a NULL cash-flow for empty cell if in the CashFlow (last) column */
...
/* Set up and aggregates the dates (for events and df) in newly created event for the domestic underlying first (index = 0) */
    und_index = 0;
...
/* -----------------------------------------------------------------------------------
    ...
    DESCRIPTION     :   When dealing with future events, 
						...
						It is incremented only if the step date is on an EventDate
   ----------------------------------------------------------------------------------- */
...
/* If there is a call to AM in the previous row (already parsed) (if there is a previous row) */
...
/* -----------------------------------------------------------------------------------
    ...
    DESCRIPTION     :   When dealing with historical events, the creation ( and
						...
						specific: only for the event dates in the tableau so far
                        ...
                        GrfnDeal->I corresponds to the index of the previous event date
						and  tableau row associated with the past date we are looking at
   ----------------------------------------------------------------------------------- */
...
/* Move on to the next event date (in the tableau) (i.e. the one to treat) */
	...
	{
	/* The current step is on a discretised broken date ( during today's day with e.o.d. flag on!): no event to attach */
		event = NULL;
...
/* -----------------------------------------------------------------------------------
    ...
    DESCRIPTION     :   After evaluating historical events, GrfnDeal->ssparams are
						...
						future cash flows, this function copies the ssparams produced
						by the evaluation of historical events into the 
						GrfnDeal->hist_ssparams.
						
						In order to force the use of these historical values, a new
						RESET_STATE node will be atttached in the COMLL of today's
						GrfnEvent (passed as an input). 
						If today's event does not exist (or is empty), it will be 
						created with just this node in the COMLL

   ----------------------------------------------------------------------------------- */
Err grfn_attach_past_info_to_future(
			GrfnDeal     *gd,
			GrfnEvent   **today_event
			)
{
Err   err = NULL;
...
/* If there are dates in the past, and variables (va, vb...)  have been referenced */
	...
	{
     /* Copies the SsParams produced by the past evaluation into the historical ones for future use */
		memcpy(&gd->hist_ssparam,&gd->ssparam,sizeof(GrfnSSParam));

	/* If today's event is empty, create a ZERO value one (for the POP() at the end of eval_event) */
		if (!(*today_event))
		{

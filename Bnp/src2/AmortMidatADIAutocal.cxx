...
const char *MidAt_AutoCal_Diag(
  ...
  for (; plEx_Begin < plEx_End; ++plEx_Begin, ++plExStart_Begin,
                                ++pdOptionPV_Begin, ++pdCoupon_Begin,
                                ++pdLam_Begin) {
    ...
    szErr = MidAt_AutoCal(
        lToday, szYC, szVC, get_cash_vol, lTheoEnd,
        szRefRate, // Refrate
        ...
        // MidAt Fixed Leg
        plFixPay_Begin + nOffSet_Fix, plFixPay_End,
        plFixStart_Begin + nOffSet_Fix, plFixEnd_Begin + nOffSet_Fix,
        pdFixCvg_Begin + nOffSet_Fix,
        // MidAt Funding/Floating leg
        plFltPay_Begin + nOffSet_Flt, plFltPay_End,
        plFltStart_Begin + nOffSet_Flt, plFltEnd_Begin + nOffSet_Flt,
        pdFltCvg_Begin + nOffSet_Flt, pdMargin_Begin + nOffSet_Flt,
        pdSpread_Begin + nOffSet_Flt,
        // term structure
        nCalibTauToCap, // 1 - calibrate Tau to cap        , 0 otherwise
        (double *)&dLamT, 1 + (double *)&dLamT, (double *)pdLam_Begin, pdAlpha,
        pdGamma, pdRho,
        // calibration params
        nOnefequi,
        // grid
        nNumPointT, nNumPointX,
        /// parameters needed by diagcalib
        pParam_Swaption, pParam_Caplet, pCalibParam,
        /// results        ,
        pdOptionPV_Begin,
        0, // double *pdFixLegPV        ,
        0, // double *pdFltLegPV        ,
...
const char *GenMidAt_AutoCal(
  ...
  double *pdSigT_Begin = _alloca(nSize_Sig * sizeof(double)),
         *pdSigT_End = pdSigT_Begin + nSize_Sig;

  // lambda fixed
  const int nSize_Lam = plLamDate_End - plLamDate_Begin;
  double *pdLamT_Begin = _alloca(nSize_Lam * sizeof(double)),
  ...
  if (!(*pplSigDate_Begin) || !(*pplSigDate_End) || !(*ppdSig_Begin)) {
    szErr = Calibrate_GenMidAt(
        lToday, szYC, szVC, get_cash_vol, szCorrel, get_correl, eod_flag,
        lTheoEnd, szRefRate, szFreq, szBasis, plEx_Begin, plEx_End,
        plFixStart_Begin, plFixStart_End, plFixEnd_Begin, plFixPay_Begin,
        pdFixCoupon_Begin, pdFixNotional_Begin, pdFixFee_Begin,
        plFltStart_Begin, plFltStart_End, plFltEnd_Begin, plFltPay_Begin,
        pdMargin_Begin, pdSpread_Begin, pdFltNotional_Begin, pdLamT_Begin,
        pdLamT_Begin + nSize_Lam, pdLam_Begin, pdAlpha, pdGamma, pdRho,
        default_ref, default_swap_freq, default_swap_basis, dMinTime,
        dMinInterval, lNotice, dMaxStdShort, nFixLam, n1FEqui, nSkipLast,
        nUseJump, dMaxVarJump, nStrikeType, nEuroModel, &pdSigT_Begin,
        &pdSigT_End, ppdSig_Begin, pnBool_CalibInst, pdPrice_CalibInst);
  ...
  szErr = _Price_GenMidAt(
      nNumPointT, nNumPointX, szYC, lToday, pdSigT_Begin,
      pdSigT_Begin + nSize_Sig, *ppdSig_Begin, pdLamT_Begin,
      pdLamT_Begin + nSize_Lam, pdLam_Begin, pdAlpha, pdGamma, pdRho,
      plEx_Begin, plEx_End, plExStart_Begin, pdExFee_Begin, plExStart_Begin,
      plFixPay_Begin, plFixPay_Begin + (plFixStart_End - plFixStart_Begin),
      plFixStart_Begin, plFixEnd_Begin,
      plFixPay_Begin, //&vec_lFixFeePay[0]        ,
      pdFixCvg_Begin, pdFixNotional_Begin, pdFixCoupon_Begin, pdFixFee_Begin,
      plFltPay_Begin, plFltPay_Begin + (plFltStart_End - plFltStart_Begin),
      plFltStart_Begin, plFltEnd_Begin, pdFltCvg_Begin, pdFltNotional_Begin,
      pdMargin_Begin, pdSpread_Begin, _PayRec(szPayRec), pdFixLegPV, pdFltLegPV,
      pdOptionPV, pdExProb, pdExBoundary, nUse_Backward);

  if (!(*pplSigDate_Begin) || !(*pplSigDate_End) || !(*ppdSig_Begin)) {
    *pplSigDate_Begin = calloc((pdSigT_End - pdSigT_Begin), sizeof(long));
...
const char *_MidAt_AutoCal(
  ...
  const double *pdExFee =
      _memset(&dFee, _alloca(nSize_Ex * size), nSize_Ex, size);

  const double *pdFixFee =
      _memset(&dFee, _alloca(nSize_Fix * size), nSize_Fix, size);
  // const double *pdCoupon_Begin = 	_memset(&dCoupon
  //       ,_alloca(nSize_Fix*size)        ,nSize_Fix        ,size);
  const double *pdFixNotional =
      _memset(&dNotional, _alloca(nSize_Fix * size), nSize_Fix, size);

  const double *pdFltNotional =
      _memset(&dNotional, _alloca(nSize_Flt * size), nSize_Flt, size);
  ...
  const char *szErr = _Calibrate_MidAt(
      ...
      plEx_Begin, plEx_End, pszSwapTenor_Begin, pdSwapStrike_Begin,
      pszCapTenor_Begin, pdCapStrike_Begin, nCalibTauToCap, pdAlpha, pdGamma,
      pdRho, pParam_Swaption, pParam_Caplet, pCalibParam, nOnefequi,
      pdLamT_Begin, pdLamT_End, pdLam_Begin, &pdSigT_Begin, &pdSigT_End,
      &pdSig_Begin);
  ...
  return _Price_GenMidAt(
      nNumPointT, nNumPointX, szYC, lToday, pdSigT_Begin, pdSigT_End,
      pdSig_Begin, pdLamT_Begin, pdLamT_End, pdLam_Begin, pdAlpha, pdGamma,
      pdRho, plEx_Begin, plEx_End, plExStart_Begin,
      pdExFee, //&vec_dExFee[0]        ,
      plExStart_Begin, plFixPay_Begin, plFixPay_End, plFixStart_Begin,
      plFixEnd_Begin,
      plFixPay_Begin, //&vec_lFixFeePay[0]        ,
      pdFixCvg_Begin,
      ...
      plFltPay_Begin, plFltPay_End, plFltStart_Begin, plFltEnd_Begin,
      pdFltCvg_Begin,
...
const char *MIDAT_AUTOCAL(const Comm_MidAtAutocal *pComm) {
  return MidAt_AutoCal(
      pComm->m_lToday, pComm->m_szYC, pComm->m_szVC, pComm->m_get_cash_vol,
      pComm->m_lTheoEnd,
      ...
      pComm->m_plFixPay_Begin, pComm->m_plFixPay_End, pComm->m_plFixStart_Begin,
      pComm->m_plFixEnd_Begin, pComm->m_pdFixCvg_Begin, pComm->m_plFltPay_Begin,
      pComm->m_plFltPay_End, pComm->m_plFltStart_Begin, pComm->m_plFltEnd_Begin,
      pComm->m_pdFltCvg_Begin, pComm->m_pdMargin_Begin, pComm->m_pdSpread_Begin,
      pComm->m_nCalibTauToCap, // 1 - calibrate Tau to cap        , 0 otherwise
      (double *)pComm->m_pdLamT_Begin, (double *)pComm->m_pdLamT_End,
      (double *)pComm->m_pdLam_Begin, pComm->m_pdAlpha, pComm->m_pdGamma,
      pComm->m_pdRho, pComm->m_nOnefequi, pComm->m_nNumPointT,
      pComm->m_nNumPointX, pComm->m_pParam_Swaption, pComm->m_pParam_Caplet,
      pComm->m_pCalibParam, pComm->m_pdOptionPV, pComm->m_pdFixLegPV,
      pComm->m_pdFltLegPV, 0, 0, 0);
}
...
const char *MidAt_AutoCal(
  ...
  Calibrate_MidAt_Preprocess(
      ...
      szFreq,    // frequency of the fixed leg -"M"        ,"Q"        ,"S" ,"A"
      pdCoupon_Begin, // coupon
      plEx_Begin, plEx_End, plFixStart_Begin, plFixStart_Begin + nSize_Fix,
      plFixEnd_Begin, plFixPay_Begin, plFltStart_Begin,
      plFltStart_Begin + nSize_Flt, plFltEnd_Begin, plFltPay_Begin,
      pdMargin_Begin, pdSpread_Begin, pszSwapTenor, pdSwapStrike, pszCapTenor,
      pdCapStrike);

  return _MidAt_AutoCal(
      lToday, szYC, szVC, get_cash_vol, lTheoEnd, szRefRate, szFreq, szBasis,
      pdCoupon_Begin, szPayRec, plEx_Begin, plEx_End, plExStart_Begin,
      plFixPay_Begin, plFixPay_End, plFixStart_Begin, plFixEnd_Begin,
      pdFixCvg_Begin, plFltPay_Begin, plFltPay_End, plFltStart_Begin,
      plFltEnd_Begin, pdFltCvg_Begin, pdMargin_Begin, pdSpread_Begin,
      nCalibTauToCap, pdLamT_Begin, pdLamT_End, pdLam_Begin, pdAlpha, pdGamma,
      pdRho, pszSwapTenor, pdSwapStrike, pszCapTenor, pdCapStrike,
      pParam_Swaption, pParam_Caplet, pCalibParam, nOnefequi, nNumPointT,
      nNumPointX, pdOptionPV, pdFixLegPV, pdFltLegPV, pdExProb, pdExBoundary,
      nUse_Backward);
}

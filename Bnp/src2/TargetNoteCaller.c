...
/* if the deal is knocked out (but does not have a final payment)or has finished, record the remaining PV and return */
...
/* calculate LGM2FCV price */
	...
	{
	smessage("2F");
		if ( err = TargetNoteMC_2F(		tarn->output.szTARN_LGM2F_UND,
										tarn,
										aux,
	...
	smessage("1F");
		if ( err = TargetNoteMC_2F(		tarn->output.szTARN_LGM1F_UND,
										tarn,
										aux,
	...
	smessage("1FSV");
		if ( err = TargetNoteMC_SV(		tarn->output.szTARN_LGM1FSV_UND,
										tarn,
										aux,
...
/* calculate LGMSV price */
	...
	{
		smessage("SV");
		if ( err = TargetNoteMC_SV(		tarn->output.szTARN_LGMSV_UND,
										tarn,
										aux,

...
static double LGM1FQuantoFwdAdjustment(double mattime, double starttime,
                                       double endtime, double *sigtime,
                                       long nbsig, double *domsig,
                                       double *forsig, double *fxsig,
                                       double domlam, double forlam,
                                       double domforrho, double forfxrho) {
  long i;
  double I1, I2, I3;
  double I2_a, I2_b;
  double I3_a, I3_b;
  ...
  if (mattime <= sigtime[0]) {
    ...
    I3_b += domforrho * forsig[0] * domsig[0] *
            Ji0_func(domlam + forlam, 0, mattime);

    I1 =
        -I1 * exp(-forlam * starttime) * (1 - exp(-forlam * coverage)) / forlam;

    I2_a = I2_a * exp(-forlam * starttime) * (1 - exp(-forlam * coverage)) /
           (forlam * forlam);
    I2_b = I2_b * exp(-forlam * starttime) * exp(-forlam * endtime) *
           (1 - exp(-forlam * coverage)) / (forlam * forlam);
    I2 = I2_a - I2_b;

    I3_a = I3_a * exp(-forlam * starttime) * (1 - exp(-forlam * coverage)) /
           (forlam * domlam);
    ...
    I3_b += domforrho * forsig[0] * domsig[0] *
            Ji0_func(domlam + forlam, 0, sigtime[0]);

    while (sigtime[i + 1] < mattime && i < nbsig - 1) {
      I1 += forfxrho * fxsig[i + 1] * forsig[i + 1] *
            Ji0_func(forlam, sigtime[i], sigtime[i + 1]);

      I2_a += forsig[i + 1] * forsig[i + 1] *
              Ji0_func(forlam, sigtime[i], sigtime[i + 1]);
      I2_b += forsig[i + 1] * forsig[i + 1] *
              Ji0_func(2 * forlam, sigtime[i], sigtime[i + 1]);
    ...
    if (i == 0) {
      I1 += forfxrho * fxsig[1] * forsig[1] *
            Ji0_func(forlam, sigtime[0], mattime);

      I2_a += forsig[1] * forsig[1] * Ji0_func(forlam, sigtime[0], mattime);
      I2_b += forsig[1] * forsig[1] * Ji0_func(2 * forlam, sigtime[0], mattime);

      I3_a += domforrho * forsig[1] * domsig[1] *
              Ji0_func(forlam, sigtime[0], mattime);
      I3_b += domforrho * forsig[1] * domsig[1] *
              Ji0_func(domlam + forlam, sigtime[0], mattime);

      I1 = -I1 * exp(-forlam * starttime) * (1 - exp(-forlam * coverage)) /
           forlam;
      ...
      I1 += forfxrho * fxsig[i + 1] * forsig[i + 1] *
            Ji0_func(forlam, sigtime[i], mattime);

      I2_a +=
          forsig[i + 1] * forsig[i + 1] * Ji0_func(forlam, sigtime[i], mattime);
      I2_b += forsig[i + 1] * forsig[i + 1] *
              Ji0_func(2 * forlam, sigtime[i], mattime);
...
Err LGM1FQuantoFwd(char *und3dfx, long start, long end, char *freq, char *basis,
                   char *refrate, long *nbFwd, double **QuantoFwd) {
  double QAdj;
  ...
  err = Get_FX_StochRate_TermStructures(
      ...
      &fxsigtime, &fxsig, &fxsig_n, &domforcorr, &domfxcorr, &quantocorr);
  if (err) {
    goto FREE_RETURN;
  }
  ...
  *nbFwd = float_nb_dates;
  ...
  for (i = 0; i < float_nb_dates; ++i) {
    QAdj = LGM1FQuantoFwdAdjustment(
        ...
        (float_end_dates[i] - today) / 365.0, merge_dates, nb_merge_dates,
        sigdom, sigfor, sigfx, dom_lam, for_lam, domforcorr, quantocorr);

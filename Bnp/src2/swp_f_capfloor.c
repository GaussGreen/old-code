...
Err swp_f_CapFloor(
		...
		Err             (*GetBSVol)(double start, double end, double strike, 
						double dForward, double dSpread, double *vol),
		String           capFloorStr,
...
/* Main call to the pricing function for caps/floors */
	err = swp_f_CapFloor_SwapDP( &swapdp, strike, GetBSVol,  capFloor, szRefRateCode,
...
/* Make a floating leg corresponding to the cap */
	err = swp_f_make_FloatingLeg( swapdp->start, swapdp->end, today, 
			szRefRateCode, yccrv, &floatleg);
	if (err)
		return err;
...
/* Loop on all the caplets */
	*result = 0.0;
	...
	/* Get the volatility for the period (start, end) and the strike */
		err = GetBSVol(floatleg.start_date.date[i], floatleg.end_date.date[i], strike , 
				forward,floatleg.spread.d[i],  &vol);
	...
	/* Call to the BlackScholes type formulas */
		err = swp_f_Caplet_Struct( forward, strike, vol, mat, 
				capFloor, greek, logNorm, &caplet);
		if (err)
			return err;
...
/* Main call to the implied volatility function for caps/floors */
	err = swp_f_CapFloorImpVol_SwapDP( premium, &swapdp, strike, capFloor, refRateCode,
				ycName, logNorm, impvol );
	if (err)
		return err;
...
static Err NullVol (double start, double end, double strike, 
					double dForward, double dSpread, double *vol)
{
	*vol = 0.0;
	return NULL;
...
static Err ConstantVol (double start, double end, double strike, 
					double dForward, double dSpread, double *vol)
{
...
/* Start Newton like iterations (maximum of 20 iterations) */
	...
	/* Compute the premium with this new volatility */
		err = swp_f_CapFloor_SwapDP( swapdp, strike, ConstantVol,  capFloor, refRateCode,
				ycName, PREMIUM, logNorm, &prem_new);
    ...
    /* Gets a new point with a slighly shifted vol */
		err = swp_f_CapFloor_SwapDP( swapdp, strike, ConstantVol,  capFloor, refRateCode,
				ycName, PREMIUM, logNorm, &prem_shift);

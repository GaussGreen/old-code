...
{
	if( X > MAX_EXP_ARG ) X = MAX_EXP_ARG;
	else if( X < MIN_EXP_ARG) X = MIN_EXP_ARG;
...
typedef struct{
	...
	double				yr_to_exp,cms_rate,cmt_rate,
						atm_sigma_beta,sabr_alpha,sabr_beta,sabr_rho,
						spot_spread,fwd_spread,
...
static Err set_static_params_for_calib(String				swp_yc_name,
									   SrtDiffusionType		swp_vol_type,
									   double				yr_to_exp,
									   double               cms_rate,
									   double               cmt_rate,
...
static Err levenberg_CMT_VOL_funcs(double		instr_index,
									...
									int			lNumParams)
{
	...
	double				atm_sigma_beta = static_params.atm_sigma_beta,
						...
						sabr_beta = static_params.sabr_beta,
						sabr_rho = static_params.sabr_rho,
						spot_spread = static_params.spot_spread,
						*cmt_calibration_strikes = static_params.cmt_calibration_strikes,
						cms_rate = static_params.cms_rate,
						cmt_rate = static_params.cmt_rate;
	...
	if ( are_calib_parameters_within_band(pdOptimParams,lNumParams, static_params.ppdParamBounds) == SRT_NO)
	{
		*value = SRT_BIG;
		return NULL;
	}

	err = from_optparam_to_CMT_VOL_parameters(pdOptimParams,
	...
	err = srt_f_get_cmt_vol(swp_yc_name,
							swp_vol_type,
							yr_to_exp,
							cms_rate,
							cmt_rate,
								 ...
								 int          lNumParams)
{
	...
	err = levenberg_CMT_VOL_funcs(instr_index,
								  pdOptParams,
								  value,
								  lNumParams);
	if(err) return err;
...
Err srt_f_calibrate_CMT_CMS_spread_parameters(double			yr_to_exp,
											  String			swp_yc_name,
											  ...
											  double            **param)
{
	...
	err = set_static_params_for_calib(swp_yc_name,
									  swp_vol_type,
									  yr_to_exp,
									  cms_rate,
									  cmt_rate,
	...
	for( i = 1 ; i <= ndata; i++) data[i] = (double)(i);
	for( i = 1 ; i <= ndata; i++) target[i] = (cmt_vols[i]*cmt_vols[i]);
	for( i = 1 ; i <= ndata; i++) weight[i] = target[i];
	...
	(*param)[2] = (fwd_spread-spot_spread*trunc_exp(-opt_params[1]*yr_to_exp))/(1-trunc_exp(-opt_params[1]*yr_to_exp));
	(*param)[3] = opt_params[2];
	(*param)[4] = opt_params[3];


	if(data) free_dvector(data,1,ndata);data = NULL;
	if(target) free_dvector(target,1,ndata);target = NULL;
	if(weight) free_dvector(weight,1,ndata);weight = NULL;
	if(opt_params) free_dvector(opt_params,1,3);opt_params = NULL;
	if(static_params.ppdParamBounds) free_dmatrix(static_params.ppdParamBounds,1,2,1,3);static_params.ppdParamBounds = NULL;
...
/* COMPUTE THE NORMAL CMS VOLATILTY FROM THE CMS SABR PARAMATERS AND ADJUST IT BY THE CONVEXITY OF THE SPREAD OVER CMT */
Err srt_f_get_cmt_vol(String			swp_yc_name,
					  SrtDiffusionType	swp_vol_type,
					  ...
					  double			*cmt_vol)
{
	...
	if(use_mkt_fwd_spread == SRT_FALSE)	fwd_spread = spread_fwd_level + (spot_spread - spread_fwd_level)*trunc_exp(-yr_to_exp/spread_tau);
	else  fwd_spread = static_params.fwd_spread;

	cms_adjusted_strike = cmt_strike + fwd_spread;

	/* GET THE CMS MARKET VOL :SABR IMPLIED BLACK AND SCHOLES VOLATILITY */
	err = srt_f_optsarbvol(cms_rate,
						   cms_adjusted_strike,
						   yr_to_exp,		   
						   atm_sigma_beta,
						   sabr_alpha,
	...
	spread_vol = spread_loc_vol*sqrt(0.5*(1-trunc_exp(-2*yr_to_exp/spread_tau))*(spread_tau/yr_to_exp));
	cmt_norm_vol = sqrt(cms_norm_vol*cms_norm_vol-2*rho*cms_norm_vol*spread_vol+spread_vol*spread_vol);

	/* CONVERT THE CMT NORMAL VOLATILTY INTO swp_vol_type TYPE */
	if(swp_vol_type == SRT_NORMAL) 	(*cmt_vol) = cmt_norm_vol;
	else (*cmt_vol) = (cmt_norm_vol/cmt_rate);

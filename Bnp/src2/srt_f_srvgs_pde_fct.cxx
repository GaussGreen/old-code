...
static Err compute_diffusion_generators(Date ex_date, SrtUndPtr und,
                                        SrtUndPtr dom_und,
                                        SrtBasicPdeInf pde_inf,
                                        double ****diff_mat,
                                        double ****conv_mat);

static Err compute_black_scholes_greeks(double fwd, double strike,
                                        double volatility, double time,
                                        double df, SrtReceiverType rec_pay,

                                        double *gamma, double *vega,
                                        double *volga, double *vanna);
...
static Err make_greeks_allocations(SrtBasicPdeInf pde_inf,
  ...
  if (((*gamma_matrix) == NULL) || ((*vega_matrix) == NULL) ||
      ((*volga_matrix) == NULL) || ((*vanna_matrix) == NULL))
...
static Err allocate_pde_inf_ptr(SrtBasicPdeInf *pde_inf) {
  ...
  (*pde_inf).cxxorrelation_und_vol = dvector(0, num_time_step);
  (*pde_inf).cxxorrelation_und_rate = dvector(0, num_time_step);
...
Err srt_f_set_srvgs_pde_operators(Date ex_date, String dom_und_name,
                                  String und_name, SrtBasicPdeInf pde_inf,

                                  double strike, SrtReceiverType rec_pay,
                                  double ****diff_mat, double ****conv_mat,
                                  double ****div_tensor) {
  Err err = NULL;
  ...
  double basevol, beta, gamma, omega, voldrift, vovol, correlation_und_vol,
      correlation_und_rate, time, stdev, tanh_stdev, skew, wing;
  ...
  err = make_greeks_allocations(pde_inf, &gamma_matrix, &vega_matrix,
                                &volga_matrix, &vanna_matrix);
  if (err)
    return err;

  err = compute_diffusion_generators(ex_date, und, dom_und, pde_inf, diff_mat,
                                     conv_mat);
  if (err)
    return err;
  ...
  for (step_index = 0; step_index <= pde_inf.num_step; step_index++) {
    ...
    correlation_und_vol = pde_inf.cxxorrelation_und_vol[step_index];
    correlation_und_rate = pde_inf.cxxorrelation_und_rate[step_index];
         ...
         mesh_index++) /* loop on each mesh point */
    {
      ...
      err = compute_black_scholes_greeks(
          pde_inf.spots[mesh_index] *
              (pde_inf.forward[0] / pde_inf.forward[step_index]),
          strike, basevol, pde_inf.time[0] - pde_inf.time[step_index],
          pde_inf.df[0] / pde_inf.df[step_index],

          rec_pay, &gamma_matrix[step_index][mesh_index],
      ...
      stdev = log(pde_inf.spots[mesh_index] / pde_inf.spot) /
              (basevol * sqrt(time));
      ...
      (*div_tensor)[0][step_index][mesh_index] *=
          (pde_inf.df[step_index] / pde_inf.df[0]);
  ...
  err = make_greeks_desallocations(pde_inf, &gamma_matrix, &vega_matrix,
                                   &volga_matrix, &vanna_matrix);
  if (err)
    return err;
...
Err compute_black_scholes_greeks(double fwd, double strike, double volatility,
                                 double time_to_exp, double df,
                                 SrtReceiverType rec_pay,

                                 double *gamma, double *vega, double *volga,
                                 double *vanna)

{

  *gamma =
      srt_f_optblksch(fwd, strike, volatility, time_to_exp, df, rec_pay, GAMMA);
  *vega =
      srt_f_optblksch(fwd, strike, volatility, time_to_exp, df, rec_pay, VEGA);
  *volga =
      srt_f_optblksch(fwd, strike, volatility, time_to_exp, df, rec_pay, VOLGA);
  *vanna =
      srt_f_optblksch(fwd, strike, volatility, time_to_exp, df, rec_pay, VANNA);

  return NULL;
}

Err compute_diffusion_generators(Date ex_date, SrtUndPtr und, SrtUndPtr dom_und,
                                 SrtBasicPdeInf pde_inf, double ****diff_mat,
                                 double ****conv_mat) {
  TermStruct *ts;
  Date date_today;
  String yc_name;
  ...
  for (step_index = 0; step_index <= pde_inf.num_step; step_index++) {
    if (step_index == 0)
      ...
      time = max(pde_inf.time_step,
                 pde_inf.time[step_index - 1] - pde_inf.time_step);
    ...
    pde_inf.cxxorrelation_und_vol[step_index] = find_eq_rho(time, ts);

    err = srt_f_get_corr_from_CorrList(
        srt_f_GetTheCorrelationList(), und->underl_name, dom_und->underl_name,
        time, &pde_inf.cxxorrelation_und_rate[step_index]);
    if (err)
      return err;
  }
  ...
  for (step_index = 0; step_index <= pde_inf.num_step; step_index++) {
         ...
         mesh_index++) /* loop on each mesh point */
    {
      ...
      stdev = log(pde_inf.spots[mesh_index] / spot) /
              (pde_inf.basevol[step_index] * sqrt(time));
      ...
      if (step_index == pde_inf.num_step)
        ...
        drift =
            log(pde_inf.forward[step_index] / pde_inf.forward[step_index + 1]) /
            pde_inf.time_step;

      (*conv_mat)[0][step_index][mesh_index] =
          drift -
          0.5 * pde_inf.basevol[step_index] * pde_inf.basevol[step_index];
      (*conv_mat)[1][step_index][mesh_index] =
          (*conv_mat)[0][step_index][mesh_index];
...
Err und_pde_lim(Date ex_date, SrtUndPtr und, SrtUndPtr dom_und, double max_time,
                long min_node, long min_num_mesh, SrtBasicPdeInf *pde_inf) {
  Err err = NULL;
  ...
  log_spot_min = log(forward) - 0.5 * black_vol * black_vol * exp_mat -
                 LIM_IN_STDEV * black_vol * exp_mat;

  log_spot_max = log(forward) - 0.5 * black_vol * black_vol * exp_mat +
                 LIM_IN_STDEV * black_vol * exp_mat;

  /* Build the discretisation scheme */
  max_h = black_vol * black_vol /
          fabs(swp_f_zr(ex_date, ex_date + 14.0, yc_name) -
               0.5 * black_vol * black_vol);
  ...
  (*pde_inf).num_step =
      (long)((ex_date - date_today) * YEARS_IN_DAY / time_step);

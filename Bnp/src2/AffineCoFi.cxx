// AffineCoFi.cxx:  Cornish-Fisher expansion for affine models
  ...
  */
  ...
  for (iim = 1; iim <= affi.M; iim++) // order of the moment
  {
    ...
    for (;;) {
      ...
      coef = y[m_idx] + y[m_idx + 2]; // Heston specific	(later: force
                                      // all factors initial values = 0)
      for (k = 0; k < affi.n; k++)
        coef /= fact[ii[k]];
...
Err HestonDDensityFTExpansion(double sigma, double alpha, double rho,
  ...
  for (ii[0] = 1; ii[0] <= order; ii[0]++) {
    ...
    if (poly.pool[hii] >= 0) {
      err = Poly_AddConstMono(
          &pp, (ii[0] % 4 >= 2 ? -1.0 : 1.0) * poly.cxxoef[poly.pool[hii]][0],
  ...
  for (i = 0; i < nu; i++) {
    ...
    for (ii[0] = 1; ii[0] <= order; ii[0]++) {
      ...
      h_re[i] += uu_re * pp.cxxoef[pp.pool[hii]][0];
      h_im[i] += uu_im * pp.cxxoef[pp.pool[hii]][0];
    }
  }
...
Err HestonDCoFiMC(double sigma, double alpha, double rho, double lam,
  ...
  for (ii[0] = 1; ii[0] <= order; ii[0]++) {
    ...
    err = Poly_AddConstMono(&dd, g_pool[hii].cxxoef[0][0] / fact[ii[0]], NULL,
                            ii);
    if (err)
      goto FREE_RETURN;
  }
  ...
  for (ii[0] = 1; ii[0] <= order; ii[0]++) {
    ...
    err = Poly_AddConstMonoPoly(
        &a[0], (ii[0] % 2 ? -1.0 : 1.0) * dd.cxxoef[dd.pool[hii]][0], NULL,
        NULL, &herm[hii1]);
    if (err)
      goto FREE_RETURN;
  }
  ...
  for (i = 0; i < corr.n_mono; i++) {
    if (corr.mono[i][0] <= order) {
      ...
      corr_cut.cxxoef[corr_cut.n_mono][0] = corr.cxxoef[i][0];
      corr_cut.n_mono++;
    }
  }
  ...
  ii[0] = 1;
  Hash(&hash, ii, &hii);
  meany = poly.cxxoef[poly.pool[hii]][0];
  ii[0]++;
  Hash(&hash, ii, &hii);
  stdy = sqrt(-poly.cxxoef[poly.pool[hii]][0] * 2.0);

...
{
	answer = srt_f_optblksch(fwd_price, strike,vol,mat,disc,SRT_CALL,greek)+srt_f_optblksch(fwd_price, strike,vol,mat,disc,SRT_PUT,greek);
	return answer;
}
...
else
{
	...
	case GAMMA_FWD : 	/*** GAMMA_FWD UNDERLYING ***/
		answer = disc / ( fwd_price *  
				 vol * sqrt(mat ) ) * gauss(d1); 
		break ;

	case VEGA : 		/*** VEGA ***/
		answer =  0.01 * disc * fwd_price * 
				  sqrt( mat ) * gauss( d1 );
		break ;
	...
	case RHORHO:
		 answer = -disc * cp * strike * mat * mat* nd2 + cp * disc * mat * strike * gauss(d2)* (sqrt(mat)/vol); 
		 break;
    
	case RHODELTA:
		 answer = -cp * mat * disc * nd1 + cp * disc * gauss(d1) * (sqrt(mat)/vol);
		 break;
...
/* Compute the requested greek */
	*result = srt_f_optblksch(fwd_price, strike, vol, mat,
...
if (call_put == SRT_STRADDLE)
{
	answer = srt_f_optblksch_accurate(fwd_price, strike,vol,mat,disc,SRT_CALL,greek)+srt_f_optblksch(fwd_price, strike,vol,mat,disc,SRT_PUT,greek);
	return answer;
}
...
/* Compute the requested greek */
	*result = srt_f_optblksch_accurate(fwd_price, strike, vol, mat,
		disc, call_put_type, greek_type);

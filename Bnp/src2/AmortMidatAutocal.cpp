...
//
//	// margins and spreads
//	dResult = _leg_coupon_pv_(lToday,szYC,
//plStart_Begin,plStart_End,plEnd_Begin,plPay_Begin,pdMargin_Begin,pdNotional_Begin,eBasis); 	dResult
//+=
//_leg_coupon_pv_(lToday,szYC,plStart_Begin,plStart_End,plEnd_Begin,plPay_Begin,pdSpread_Begin,pdNotional_Begin,eBasis);
//
//	// first period
...
//{
//	const double dFltPV =
//		_flt_leg(lToday,szYC,plFltStart_Begin,plFltStart_End,plFltEnd_Begin,plFltPay_Begin,pdMargin_Begin,pdSpread_Begin,pdFltNotional_Begin,eFltBasis);
//
//	const double dFixPV =
//		_fix_leg(lToday,szYC,plFixStart_Begin,plFixStart_End,plFixEnd_Begin,plFixPay_Begin,pdFixCoupon_Begin,pdFixNotional_Begin,eFixBasis);
//
//	const double dMarginLevel =
//		_multiply_level_coupon(lToday,szYC,plFltStart_Begin,plFltStart_End,plFltEnd_Begin,plFltPay_Begin,eFltBasis,pdMargin_Begin);
//
//	const double dSpreadLevel =
//		_multiply_level_coupon(lToday,szYC,plFltStart_Begin,plFltStart_End,plFltEnd_Begin,plFltPay_Begin,eFltBasis,pdSpread_Begin);
//
//	const double dFixLevel =
//		_multiply_level_coupon(lToday,szYC,plFixStart_Begin,plFixStart_End,plFixEnd_Begin,plFixPay_Begin,eFixBasis,0);
//
//	// floating size
...
//	{
//		szErr =
//swp_f_ForwardRate(lStart,lEnd,(char*)szFreq,(char*)szBasis,(char*)szYC,(char*)szRefRate,&dFRA);
//		if(szErr) return szErr;
//		dPrice = srt_f_optblknrm(dFRA, dStrike, *pdVol, dEx, 1., SRT_CALL, SRT_PREMIUM);
//		szErr = srt_f_optimpvol(dPrice, dFRA, dStrike, dEx, 1, SRT_CALL, SRT_LOGNORMAL,
//pdVol); 		if(szErr) return szErr;
//	}
//	return 0;
//}
//
...
//	if((!dvPayDates)||(!dvReplicatingStrikes)||(!dvReplicatingNotionals)||(!dvReplicatingSwaptions)||(!pdMargins)||(!lStartDates)||(!lEndDates)||(!lPayDates)||(!dCoverages))
//	{
//		err = "Allocation failed in amortMidat_compute_diagonal_prices";
//		smessage("Allocation failed in amortMidat_compute_diagonal_prices");
//		goto FREE_RETURN;
...
//		while( (j < am->fund_leg->num_cpn - 1 ) && ( am->fund_leg->cpn[j].start_date <
...
//
//		while( (l < am->fund_leg->num_cpn) && (am->fund_leg->cpn[l].start_time <
...
//	if(dFixNots) free(dFixNots);
//	if(pdMargins) free(pdMargins);
//
//	if(dvPayDates) free(dvPayDates);
//	if(dvReplicatingStrikes) free(dvReplicatingStrikes);
//	if(dvReplicatingNotionals) free(dvReplicatingNotionals);
//	if(dvReplicatingSwaptions) free(dvReplicatingSwaptions);
//
//	if(dFixNotionals) free(dFixNotionals);
//	if(lFixStartDates) free(lFixStartDates);
//	if(lFixEndDates) free(lFixEndDates);
//	if(dFixRates) free(dFixRates);
//	if(dFee) free(dFee);
//	if(dFloatNotionals) free(dFloatNotionals);
//	if(lFloatStartDates) free(lFloatStartDates);
//	if(lFloatEndDates) free(lFloatEndDates);
//	if(dMargins) free(dMargins);
...
//	get_cash_vol;CorrelName;model;UseVol;eod_flag;get_correl;dFixedPV;dSwapRate;ExDate;dLGMVol;NDimCorrel;dFloatPV;
//
//
//	return err;
//
...
//		char		*yc,							/*	yc
//*/ 		char		*vc,							/*	vc */ 		char
//*default_ref,							/*	ref rate */ 		char
//*default_swap_freq,						/*	swap freq */ 		char
//*default_swap_basis,					/*	swap basis */
//
//		char		*ref,							/*	ref rate
...
//		int			one2F,
//		int			fix_lambda,						/*	0: calib
//lambda to cap, 1: fix lambda calib 																to diagonal */ 		int			one_f_equi,
///*	1F equivalent flag: 																if set to 1, then 2F lambda will calibrate 																to the cap priced within
//calibrated 1F 																with the given lambda */ 		int			skip_last,
///*	If 1, the last option is disregarded 																and the forward volatility is flat from option 																n-1 */
//		int			use_jump,
//		double		max_var_jump,
//
//		int			strike_type,
...
//		AM_STR		am,							/*	structure
//*/
//		Err			(*get_cash_vol)(				/*	function to get
//IR cash vol from the markets */ 						char	*vol_curve_name, 						double	start_date, 						double	end_date,
//						double	cash_strike,
//						int		zero,
//						char	*ref_rate_name,
//						double	*vol,
...
//
//	/// Previously err = amortMidat_compute_diagonal_prices_new(yc, vc, ref, swap_freq,
//swap_basis,
...
//	err = amortMidat_compute_diagonal_prices_new_equistrike(yc, vc, ref, swap_freq, swap_basis,
//						today, eod_flag, mintime, mininterval, UseVol,
//						&nex, &firstex, &ex_bool, &diag_prices, &dequinotional,
...
//	}
//
//	/// Calibrate term structure again based on the combination of amortizing and equivalent
//strike swaptions
...
//	if(fixNotional) free(fixNotional);
//	if(long_strikes) free(long_strikes);
//	if(ex_fee) free(ex_fee);
//
...
//	if(floatNotional) free(floatNotional);
//	if(float_margin) free(float_margin);
//	if(float_spread) free(float_spread);
//
//	if (lex) free (lex);
//	if (short_strikes) free (short_strikes);
//
//	if(ex_bool) free(ex_bool);
//	if(diag_prices) free(diag_prices);
//
//	return err;
//}
//
...
//		char		*vc,							/*	vc (only if
//calib) */
//		char		*ref,							/*	ref rate (only
//if calib) */
//		char		*swap_freq,						/*	swap freq (only
//if calib) */
//		char		*swap_basis,					/*	swap basis (only if
//calib) */
//		double		lambda,							/*	lambda if
//unique */
//		double		alpha,							/*	alpha
...
//
//		double		max_std_short,
//		int			fix_lambda,				/*	0: calib lambda to
//cap, 1: fix lambda calib 															to diagonal */
//		int			one_f_equi,				/*	1F equivalent
//flag: 														if set to 1, then 2F lambda will calibrate 														to the cap priced within calibrated 1F 														with the
//given lambda */
//		int			skip_last,				/*	If 1, the last option
//is disregarded 														and the forward volatility is flat from option 														n-1 */
//		/*	EOD Flags */
...
//		AM_STR		am,
//		AM_UND		und,
//		int			*call_feat,				/*	0: No callable feature
//to be valued 											1: Callable feature to be valued through adi */ 		AM_ADI_ARG	adi_arg)
//{
//	Err			szErr = NULL;
...
//
//			//	Intrinsic value
//			iv = call->pay_rec ? (fund_leg_pv - fix_leg_pv) : (fix_leg_pv -
//fund_leg_pv);
//
//			fee *= dCallFeeCoeff_R2; /// call fee
//			fee_total = fee + (dFixFee*=dFltInitCoeff_R2); // factor fixed fee into call
//fee
//
//			// update indicator function - exercise 1. and not exercise 0.
//			if(nCompExProb)
//			{
//				ppdExIndicator[i][j] = (prod_val[i][j][0] - iv) > (-fee_total) ? 0.
//: 1.;
//
...
//(i-1,j),(i-1,j+1),(i,j-1)
//					// then (i,j) sit right on the boundary
//					int nNumID= 0;
//					if(dExInd==dA) ++nNumID;
//					if(dExInd==dB) ++nNumID;
//					if(dExInd==dC) ++nNumID;
...
//
//							double
//_dFltInitCoeff_R1,_dFltInitCoeff_R2,_dFltInitBase,_dFltInitPayment; 							double
//_dFltTermCoeff_R1,_dFltTermCoeff_R2,_dFltTermBase,_dFltTermPayment;
//							const double __R1=	r1[i] - dDelta_R1 ; // offset by
//1 							const double __R2 = r2[i][j] - dDelta_R2;// offset by 1
//
//							double *_pdFixCoeff_R1 = _alloca( num_fix_cpn*
//sizeof(double)); 							double *_pdFixCoeff_R2 = _alloca(num_fix_cpn * sizeof(double)); 							double
//*_pdFltCoeff_R1 = _alloca( num_fund_cpn* sizeof(double)); 							double *_pdFltCoeff_R2 = _alloca(
//num_fund_cpn* sizeof(double)); 							double *_pdFixPayment = _alloca(num_fix_cpn * sizeof(double));
//							double *_pdFltPayment = _alloca(num_fund_cpn *
//sizeof(double)); 							double *_pdFixBase = _alloca( num_fix_cpn* sizeof(double)); 							double *_pdFltBase =
//_alloca(num_fund_cpn * sizeof(double));
//
//							// precompute constants for fund_coupons
//							for (l=0; l<num_fund_cpn; l++)
//							{
//								ndf = eval_const->fund_idx[l];
//								_pdFltCoeff_R1[l] = exp(- eval_const->df_beta[ndf]
//* dDelta_R1); 								_pdFltCoeff_R2[l] = exp(- eval_const->df_gamma[ndf] * dDelta_R2); 								_pdFltBase[l] =
//exp(-eval_const->df_alpha[ndf] - eval_const->df_beta[ndf] * __R1); 								_pdFltBase[l] *=
//(am->fund_leg->cpn + call->fund_idx + l)->cpn;
//							}
//
//							// precompute constants for "fund initial"
//							ndf = eval_const->start_idx;
//							_dFltInitCoeff_R1 = exp(- eval_const->df_beta[ndf] *
//dDelta_R1); 							_dFltInitCoeff_R2 = exp(- eval_const->df_gamma[ndf] * dDelta_R2); 							_dFltInitBase =
//exp(-eval_const->df_alpha[ndf] - eval_const->df_beta[ndf] *__R1); 							_dFltInitBase *=
//fund_leg->notional[call->fund_idx];
//
//							// precompute constants for "fund terminal"
//							ndf = eval_const->fund_idx[num_fund_cpn-1];
//							_dFltTermCoeff_R1 = exp(- eval_const->df_beta[ndf] *
//dDelta_R1); 							_dFltTermCoeff_R2 = exp(- eval_const->df_gamma[ndf] * dDelta_R2); 							_dFltTermBase =
//exp(-eval_const->df_alpha[ndf] - eval_const->df_beta[ndf] * __R1); 							_dFltTermBase *=
//(*func_params->pdInit_Prev);
//
//							// precompute constants for fix coupons
//							for (l=0; l<num_fix_cpn; l++)
//							{
//								ndf = eval_const->fix_disc_idx[l];
//								_pdFixCoeff_R1[l] = exp(- eval_const->df_beta[ndf]
//* dDelta_R1); 								_pdFixCoeff_R2[l] = exp(- eval_const->df_gamma[ndf] * dDelta_R2); 								_pdFixBase[l] =
//exp(-eval_const->df_alpha[ndf] - eval_const->df_beta[ndf] * __R1);
//
//								//fix_cpn = fix_leg->cpn + call->fix_idx +
//l; 								_pdFixBase[l] *= (fix_leg->cpn + call->fix_idx + l)->cpn;
//							}
//
//
//							// update funding legs
...
//							{
//								ndf = eval_const->fund_idx[l];
//								_pdFltPayment[l] = (_pdFltBase[l]*=
//_pdFltCoeff_R1[l]); 								_pdFltPayment[l] *= exp( - eval_const->df_gamma[ndf] * __R2);
//							}
//
//							/// fuding leg init
//							ndf = eval_const->start_idx;
//							_dFltInitPayment = (_dFltInitBase *=
//_dFltInitCoeff_R1); 							_dFltInitPayment *= exp( - eval_const->df_gamma[ndf] * __R2);
//
//							/// funding leg terminal
//							ndf = eval_const->fund_idx[num_fund_cpn-1];
//							_dFltTermPayment = (_dFltTermBase *=
//_dFltTermCoeff_R1); 							_dFltTermPayment *= exp(- eval_const->df_gamma[ndf] *__R2);
//
//							for (l=0; l<num_fix_cpn; l++)
//							{
//								ndf = eval_const->fix_disc_idx[l];
//								_pdFixPayment[l] = (_pdFixBase[l] *=
//_pdFixCoeff_R1[l]); 								_pdFixPayment[l] *= exp(- eval_const->df_gamma[ndf] *__R2);
//							}
//							//	Funding leg
//							dFltLeg = (_dFltInitPayment *=
//_dFltInitCoeff_R2);
//							//dFltLeg -= (_dFltTermPayment*=
//_dFltTermCoeff_R2); 							for (l=0; l<num_fund_cpn; l++) 								dFltLeg += (_pdFltPayment[l] *=
//_pdFltCoeff_R2[l]);
//
//							// Fixed leg
//							dFixLeg = 0.0;
//							for (l=0; l<num_fix_cpn; l++)
//								dFixLeg += (_pdFixPayment[l] *=
//_pdFixCoeff_R2[l]);
//
//						}
//
//						//const double dSwapRate = fix_cpn->cpn*
//fund_leg_pv/fix_leg_pv; 						*pdAvgSwapRate += dFltLeg/dFixLeg ;
//						++nNum_Boundary;
//					}
//				}
//			}
//
//			prod_val[i][j][0] = (prod_val[i][j][0] - iv) > (-fee_total) ? (prod_val[i][j][0]
//- iv) : (-fee_total);
//		}
//
...
//			int nFlip = 0;
//			for(pdExInd=
//ppdExIndicator[i]+l2+1;pdExInd<ppdExIndicator[i]+u2+1;++pdExInd)
//			{
...
//		{
//			fund_cpn = am->fund_leg->cpn + call->fund_idx + l;
//			fund_leg_pv +=  swp_f_df(func_params->lToday,fund_cpn->pay_date,yc) *
//fund_cpn->cpn;
//		}
//
//		//	PV of FIX leg
...
//		{
//			fix_cpn = fix_leg->cpn + call->fix_idx + l;
//			fix_leg_pv += swp_f_df(func_params->lToday,fix_cpn->pay_date,yc) *
//fix_cpn->cpn;
//		}
//
//		//	Intrinsic value
...
//	nprod;phi12;phi1;phi2;rho;gamma;nb_ts;ts_time;lam;yc;evt_time;evt_date;
//
//	return 0;
//}
//
...
//	double	t1, t2, ta, tb;
//	double	coef11, coef12, coef13,
//			coef21, coef22, coef23,
//			fact1, fact2, rhoalpha;
//
...
//
//		fwd1_mid[i-1] = coef11 * exp(-lam1 * t2) * H1 + coef12 * phi1[i] + coef13 *
//phi12[i]; 		fwd3_mid[i-1] = fact1 * (coef21 * exp(-lam2 * t2) * H2 + coef22 * phi2[i] + coef23 *
//phi12[i]) 					 -fact2 * fwd1_mid[i-1];
//
//		phi1[i] /= lam21;
...
//		fwd3[i] = fact1 * (coef21 * exp(-lam2 * t2) * H2 + coef22 * phi2[i] + coef23 *
//phi12[i]) 					 -fact2 * fwd1[i];
//
//		phi1[i] /= lam21;
//		phi2[i] *= alpha * alpha / lam22;
//		phi12[i] *= alpha / lam12;
...
//				prod_val[i][j][0] = ppdExIndicator[i][j] == 1. ? 0. : prod_val[i][j][0] /
//ppdCondiDF[i][j];
//				/// TEST TEST TEST TEST TEST
...
//
//	/// unreferenced variable
//	nprod;phi12;phi1;phi2;rho;gamma;nb_ts;ts_time;lam;yc;evt_time;evt_date;r1;r2;
//
//	return 0;
//}
//
...
//										double	*ts_time,
//										int
//nb_ts, 										double	gamma, 										double	rho, 										double	phi1, 										double	phi2, 										double	phi12,
//
//										/* Gride data	*/
//										int		l1,
//										int		u1,
...
//
//	//	Eval payoff
//	szErr = payoff_func (	date[nstept-1],
//						time[nstept-1],
...
//meshx + 0.5); 		uz = min(nstepz-1, index_z + tem); 		lz = max(0, index_z - tem);
//
//		if (uz - lz < 2)
//		{
//			tem += 1;
...
//					const double *pdLambdaTime, //// NB: calendar time, not year
//fraction !!!!!!! 					int nLambdaSize, 					double		*pdOptionPV
//					)
//{
//	double _pdOptionPV[2] = {0};
...
//						(double*)pdLambdaTime, //// NB: calendar time, not year
//fraction !!!!!!! 						nLambdaSize,
//
//						adi_arg->alpha,
//						adi_arg->gamma,
//						adi_arg->rho,
...
//			if(pdmatrix) free_dmatrix(pdmatrix,lx,ux,lz,uz);
...
//	//// that once the grid is set up - by the solver - at the termination, it remains fixed  as
//the solver
//	//// advances backwards in time; this variation is needed for
//	//// computing exercise probablities; bounds of the 2D grid are stored in plx,pux,plz,puz
...
//	int *pnExToTime = memset(_alloca(nNumEx * sizeof(int)),0,nNumEx*sizeof(int)); /// initialize
//to 0
//
//	*pnEx = nNumEx;
//
...
//	else
//		szErr =
//_price_genmidat_exprob_(adi_arg,ppAmortArg,pnExToTime,pdOptionPV,*ppdExProb);
//
//	und;am;
//
//	return szErr;
...
//					const double *pdLambdaValue,
//					const double *pdLambdaTime, //// NB: calendar time, not year
//fraction !!!!!!! 					int nLambdaSize, 					double		*pdOptionPV, 					double **ppdExProb, 					double
//**ppdAvgSwapRate, 					int *pnEx)
//{
//	// total number of exercise ;
//	// NB: is_event contains {0,1 sequence}, indicating occurrence of an event/exercise
//	const int nNumEx = _accumulate_(adi_arg->is_event,(adi_arg->is_event) + adi_arg->nstp);
...
//	if(!nCompExProb)
//		return
//_price_genmidat_ts_(adi_arg,ppAmortArg,pdLambdaValue,pdLambdaTime,nLambdaSize,pdOptionPV);
//	// else compute both option value and exercise probabilities
//	else
//		 szErr = "exercise probability not available with Tau TS yet !" ;  //
//_price_genmidat_exprob_(adi_arg,ppAmortArg,pnExToTime,pdOptionPV,*ppdExProb);
//
//	und;am;
//
...
//{
//	int iSum = 0;
//	for(;piBegin!=piEnd; ++piBegin) iSum+= (*piBegin);
//	return iSum;
//}
//
//
...
//
//							char					*dom_yc,
////	Domestic discount curve 							int						nb_dom_not,
////	Number of Domestic notional
//							double					*dom_not,
////	Domestic notional
//
//							int						fund_ncpn,
////	Number of coupon 							long					*fund_fix,
////	Fixing dates
//							long					*fund_start,
////	Start dates
//							long					*fund_pay,
////	Pay dates 							char					**fund_basis,			//
//Basis
...
//in funding ccy,
//																			//
//converted to domestic 							double					*fund_spr,
////	Spread in funding ccy,
//																			//
//put to 0
//							double					*fund_mrg,
////	Margin in funding ccy,
//																			//
//converted to margin over
//																			//
//cash libor in domestic currency 							double					*fund_fix_cpn,
////	Fixing: contains spread
//																			//
//but not margin,
//																			//
...
////	Start date of the funding 							double					*eq_final_ex,
////	Domestic cash-flow equivalent
//																			//
//to final exchange
//																			//
//(to be delivered
//																			//
//at funding start date) 							double					*eq_init_ex);
////	Domestic cash-flow equivalent
//																			//
//to initial exchange
//																			//
...
//			/*	The underlying */
//			int			use_calib,						/*	0:
//use lgm2fund, 1: calibrate */
//
//			/*		if calib */
//			char		*yc,							/*	yc
//*/ 			char		*vc,							/*	vc */ 			char
//*ref,							/*	ref rate (only if calib) */ 			char
//*swap_freq,						/*	swap freq (only if calib) */ 			char
//*swap_basis,					/*	swap basis (only if calib) */ 			double
//lambda,							/*	lambda if unique */ 			double
//alpha,							/*	alpha */ 			double
//gamma,							/*	gamma */ 			double rho,
///*	rho */
//
//			/*	End of calib params */
//			Err			(*get_cash_vol)(				/*	function to
//get IR cash vol from the markets */ 							char	*vol_curve_name, 							double	start_date, 							double
//end_date, 							double	cash_strike, 							int		zero, 							char	*ref_rate_name, 							double
//*vol, 							double	*power),
//
//			/*		if no calilb */
//			char		*lgm2dund,
//
//			/*	The structure */
//			long		start_date,				/*	Date at which initial
//notional exchange occurs */
//			long		theoEndDate,			/* End Date of the structure
//*/
//
//			/*		funding */
//			char		*fund_ref,
//			int			fund_ccy,				/*	0: domestic, 1:
//other */
//			double		*fund_not,				/*	If different from
//domestic or foreign (fund_ccy = 1) */ 			char		*fund_ccy_yc,			/* If
//different from domestic or foreign (fund_ccy = 1) */
//			double		fx_fund_dom,			/*	If different from
//domestic or foreign (fund_ccy = 1) 2 bd fwd */ 			long		fx_fund_dom_spot_date, 			int
//fund_ncpn, 			long		*fund_fix, 			long		*fund_start, 			long		*fund_end,
//			long		*fund_pay,
//			char		**fund_basis,
//			double		*fund_spr,
//			double		*fund_mrg,
//			double		*fund_fix_cpn,			/*	Past coupon fixing if
//relevant, 											includes spr, but not mrg, cvg and notional */
//			/*		fix */
//			double		*fix_not,
//			int			fix_ncpn,
//			long		*fix_start,
...
//			/*		calls */
//			int			ncall,
//			int			pay_rec,				/*	0: rec pd, 1: pay
//pd */ 			long		*ex_date, 			long		*set_date, 			double		*fee,
//
//			/*	Numerical params */
//			int			req_stp,
//			int			req_stpx,
...
//			double		max_std_short,
//			int			fix_lambda,						/*	0:
//calib lambda to cap, 1: fix lambda calib 																	to diagonal */ 			int			one_f_equi,
///*	1F equivalent flag: 																	if set to 1, then 2F lambda will calibrate 																	to the cap priced within
//calibrated 1F 																	with the given lambda */ 			int			skip_last,
///*	If 1, the last option is disregarded 																	and the forward volatility is flat from option 																	n-1 */
//
//			/*	EOD Flags */
//			int			eod_fix_flag,			/*	0: I, 1: E
//*/ 			int			eod_pay_flag,			/*	0: I, 1: E */ 			int
//eod_ex_flag,			/*	0: I, 1: E */
//
//			/*	Exercised flag */
//			int			exercised,				/*	Flag
//*/ 			long		ex_date_ex,				/*	Date when exercised */ 			long
//ex_date_set,			/*	Corresponding settlement date */ 			double		ex_fee,
///*	Corresponding fee */
//
...
//
//			/*	Results */
//			double		*fund_val,				/*	Value of the funding
//leg */ 			double		*fix_val,				/*	Value of the Power Dual leg
//*/
//			double		*call_val,				/*	Value of the
//callable feature */ 			double		**ppdExProb, 			double **ppdAvgSwapRate, 			int *pnEx, 			int
//export_ts,				/*	1: Export TS, 0: don't */ 			AM_UND
//und_exp)
//{
//	am_str			*am		= NULL;
//	am_und			*und		= NULL;
...
//			fix_leg_pv += fix_rate[i]
//				* coverage (fix_start[i], fix_pay[i], bas)
//				* fix_not[i]
...
//			{
//				temp = swp_f_df (today, fin_not_date, yc) *
//fix_not[fund_leg->num_cpn-1]; 				fix_leg_pv += temp;
//
//			}
//		}
//
...
//adi_arg->nstp);
//		// NB: most cases fix rates are the same! but might not be correct for some cases!!!
//shall come back later ... 		szErr = _price_genmidat(am,und,adi_arg,today,fix_rate[0],&call,
//(nOutPutExProb ? ppdExProb : 0),(nOutPutExProb ? ppdAvgSwapRate : 0),pnEx );
//
//		if (szErr)
//		{
//			goto FREE_RETURN;
...
//structure 			double *pdEquiExProb = _alloca((*pnEx+1)*sizeof(double)); 			double *pdAvgStrike =
//_alloca((*pnEx)*sizeof(double)); 			szErr = _price_genmidat(am,und,adi_arg,today,fix_rate[0],&call,
...
//*swap_freq,						/*	swap freq (only if calib) */ 			char
//*swap_basis,					/*	swap basis (only if calib) */ 			double
//*pdLambdaValue,							/*	lambda if unique */
//			double		*pdLambdaTime,
//			int			  nLambdaSize,
//			double		alpha,							/*	alpha
//*/ 			double		gamma,							/*	gamma */
...
//*/
//			double		*call_val,				/*	Value of the
//callable feature */ 			double		**ppdExProb, 			double **ppdAvgSwapRate, //averag swap rate at
//the boundary 			int *pnEx,
//			int			export_ts,				/*	1: Export TS, 0:
//don't */ 			AM_UND		und_exp)
//{
//	am_str			*am		= NULL;
//	am_und			*und		= NULL;
//	am_adi_arg		*adi_arg	= NULL;
...
//	{
//		smessage ("Launching adi, time steps requested: %d, actual: %d", req_stp,
//adi_arg->nstp); 		szErr = _price_genmidat_ts(am, und,
//adi_arg,today,fix_rate[0],pdLambdaValue,pdLambdaTime,nLambdaSize, &call, (nOutPutExProb ?
//ppdExProb : 0),(nOutPutExProb ? ppdAvgSwapRate : 0),pnEx);
//
//		if (szErr)
//		{
//			goto FREE_RETURN;

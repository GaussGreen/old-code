  ...
  *pdequinotional = dEquiv_Notional;
  ...
  if (am->fix_leg->cpn[j].start_time < DMAX(mintime, am->call[0].ex_time)) {
    ...
    /*		if(model==1)
       ...
       MODIFIED_SUCCEEDING); err = EuropeanAmortizingSwaption2(today  , ExDate ,
                                                                    EndDate  ,
                                                                    lNFixNot-j ,
                                                                    lStartDates+j
       , lEndDates+j  , lPayDates+j  , dCoverages+j  , dCoverages+j  ,
                                                                    dFixRates[0]
       , dFixNots+j  , yc  , vc  , cFreq  , cBasis  , cRec  , ref  ,
                                                                    get_cash_vol
       ,
       ...
       , dvReplicatingNotionals  , &dPrice  , &dFixedPV  , &dFloatPV  ,
                                                                    &dSwapRate ,
                                                                    &lgmSigKapTSPtrPtr
       , &dLGMVol  , dvReplicatingSwaptions  ,
  ...
  for (k = 1; k < *nex; ++k) {
    if ((am->fix_leg->cpn[j + k].start_time - lastcaltime < mininterval) ||
      ...
      /*if(model==1)
      {
              ...
              err = Compute_CoInitalSwaps_Correl2(SpotDate  ,
                                                                              CorrelName
      , get_correl  , StartDate  , EndDate  , srtFreq  , srtBasis  , NDimCorrel
      , &Correl); if(err)
              {
                      goto FREE_RETURN;
              }
...
Err am_calib_und_new_reduc(
  ...
  /// Previously err = amortMidat_compute_diagonal_prices_new(yc  , vc  , ref  ,
  /// swap_freq  , swap_basis  ,
    ...
    for (j = l2; j <= u2; j++) {
      ...
      if (nCompExProb) {
        ...
        if (i > l1 && j > l2 && j < u2) {
          ...
          // if dExInd is different from the ajacent 4 cells (i-1  ,j-1)  , (i-1
          // ,j)  ,(i-1  ,j+1)  ,(i  ,j-1) then (i  ,j) sit right on the
          // boundary
          int nNumID = 0;
          if (dExInd == dA)
            ++nNumID;
...
static Err _lgm2f_adi_shrinkgrid(
  ...
  */
  ...
  //volatility 	is the same  , the mesh has to be the same  , but the number of
  //steps has to be adjusted
...
static Err _price_genmidat(AM_STR am,          // structure
  ...
  // NB: is_event contains {0  ,1 sequence}  , indicating occurrence of an
  // event/exercise
  const int nNumEx =
      _accumulate_(adi_arg->is_event, (adi_arg->is_event) + adi_arg->nstp);
...
Err am_caller_AmortMidatAutocal(
   ...
   */
  if (call_feat == 1) {
    smessage("Launching adi  , time steps requested: %d  , actual: %d", req_stp,
             adi_arg->nstp);

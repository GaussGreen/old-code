...
static void disc_normal_center(
    double* x, long nstp, double xmin, double stdn, long* index, double* meshx);
static void disc_sqrt_center(
    double* x,
    long    nstpx,
    double* time,
    long    nstpt,
    ...
    double* phi12)
{
    ...
    for (i = 1; i < nstept; i++)
    {
        ...
        var3 = 1.0 / rho2 *
               (phi2[i] / alpha / alpha + rho * rho * phi1[i] - 2.0 * rho * rho / alpha * phi12[i]);
    ...
    double* std3der_mid)
{
    ...
    LGMSVSolFunc ExpectR1_, *ExpectR1 = &ExpectR1_, ExpectR2_, *ExpectR2 = &ExpectR2_, Phi1_,
                            *Phi1 = &Phi1_, Phi2_, *Phi2 = &Phi2_, Phi12_, *Phi12 = &Phi12_;
    ...
    for (i = 1; i < nstept; i++)
    {
        ...
        std3_mid[i - 1] = sqrt(std3_c1 * Phi2->dXt1 + std3_c2 * Phi1->dXt1 + std3_c3 * Phi12->dXt1);

        std1der_mid[i - 1] =
            (Phi1->a - Phi1->dLambda * std1_mid[i - 1] * std1_mid[i - 1]) / (2.0 * std1_mid[i - 1]);
        std3der_mid[i - 1] =
            (Phi1->a - std3_c1 * Phi2->dLambda * Phi2->dXt1 - std3_c2 * Phi1->dLambda * Phi1->dXt1 -
    ...
    double* res)
{
    ...
    double *r1_bar = NULL, *r3_bar = NULL, *r1_dim1 = NULL, **r1_dim2 = NULL, **r2 = NULL,
           ...
           *fwd1 = NULL, *fwd1_mid = NULL, *fwd3 = NULL, *fwd3_mid = NULL, *var1 = NULL,
    ...
    for (step = nstept - 2; step >= 0; step--)
    {
        ...
        tem =
            (int)(NSTD_LGM / rho2 * sqrt(phi2[step] / alpha / alpha + rho * rho * phi1[step] - 2.0 * rho * rho / alpha * phi12[step]) / meshx + 0.5);
    ...
    double* res)
{
    ...
    double *r1_bar = NULL, *r3_bar = NULL, *r1_dim1 = NULL, **r1_dim2 = NULL, **r2 = NULL,
           ***values = NULL, ***values_p1 = NULL, ***values_temp = NULL, **mu_r1 = NULL,
           **mu_r3 = NULL, **var_r1 = NULL, **var_r3 = NULL, **r = NULL, **r_init = NULL,
           *fwd1 = NULL, *fwd3 = NULL, *var1 = NULL, *quad_var = NULL, *avg_lam = NULL,
           *phi1 = NULL, *phi2 = NULL, *phi12 = NULL, *var1_max = NULL, *var3_max = NULL;
    ...
    if (!pde || !var1 || !avg_lam || !phi1 || !phi2 || !phi12 || !fwd1 || !fwd3 || !quad_var ||
        !var1_max || !var3_max)
    {
        err = "Memory allocation error (1) in lgm2fpde";
        goto FREE_RETURN;
    ...
    */
    ...
    {
        disc_sqrt_center(r1_bar, nstepx, time, nstept, NB_CHECK, var1_max, &index_x, &coefx);
        disc_sqrt_center(r3_bar, nstepz, time, nstept, NB_CHECK, var3_max, &index_z, &coefz);
    }
    ...
    for (step = nstept - 2; step >= 0; step--)
    {
        ...
        if (disc_method == 1)
        {
            tem = (int)((norm(NSTD_LGM * sqrt(var1_max[step]) / std1) - 0.5) / meshx + 0.5);
        }
        else if (disc_method == 0)
        {
        ...
        if (disc_method == 1)
        {
            tem = (int)((norm(NSTD_LGM * sqrt(var3_max[step]) / std3) - 0.5) / meshz + 0.5);
        }
        else if (disc_method == 0)
        {
    ...
    double* res)
{
    ...
    double mu_r1i, mu_r3i;
    double lam1, lam2;
    double fact1, rho2, rhoq, rhoalpha, rhoalpha_plus1, rho2alpha, std3_c1, std3_c2, std3_c3;
    double r_temp;

    double std1, std3, r2_temp, std1_next, std3_next, std1_mean, std3_mean, var1_temp, var3_temp,
        fact1_temp, rhoalpha_plus1_temp;

    double *r1_bar = NULL, *r3_bar = NULL, *r1_dim1 = NULL, **r2 = NULL, ***values = NULL,
           ***values_p1 = NULL, ***values_temp = NULL, **mu_r1 = NULL, **mu_r3 = NULL,
           **var_r1 = NULL, **var_r3 = NULL, **r = NULL, *fwd1 = NULL, *fwd2 = NULL,
           *std1_mid = NULL, *std3_mid = NULL, *std1der_mid = NULL, *std3der_mid = NULL,
           *var1 = NULL, *avg_lam = NULL, *phi1 = NULL, *phi2 = NULL, *phi12 = NULL;
    ...
    if (!pde || !var1 || !avg_lam || !phi1 || !phi2 || !phi12 || !fwd1 || !fwd2 || !std1_mid ||
        !std3_mid || !std1der_mid || !std3der_mid)
    {
        err = "Memory allocation error (1) in lgm2fpde";
        goto FREE_RETURN;
    }
    ...
    if (!r2 || !values || !values_p1 || !mu_r1 || !mu_r3 || !var_r1 || !var_r3 || !r || !r1_bar ||
        !r1_dim1 || !r3_bar)
    {
        err = "Memory allocation error (2) in lgm2fpde";
        goto FREE_RETURN;
    ...
    for (step = nstept - 2; step >= 0; step--)
    {
        ...
        std1   = sqrt(phi1[step]);
        std3   = sqrt(std3_c1 * phi2[step] + std3_c2 * phi1[step] + std3_c3 * phi12[step]);
...
void disc_normal_center(double* x, long nstp, double xmin, double stdn, long* index, double* meshx)
{
    long   i;
    double d;

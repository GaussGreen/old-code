...
static void disc_normal_center(double *x, long nstp, double xmin, double stdn, long *index, double *meshx);
static void	disc_sqrt_center(double	*x, long nstpx, double *time, long nstpt, int nb_check, double *varmax, long	*index, double *coefx);
...
{	
	...
	for (i=1; i<nstept; i++)
	{
		...
		var3 = 1.0 / rho2 * ( phi2[i] / alpha / alpha 
							 + rho * rho * phi1[i] 
...
{	
	...
	LGMSVSolFunc	ExpectR1_, *ExpectR1 = &ExpectR1_,
					ExpectR2_, *ExpectR2 = &ExpectR2_,
					Phi1_, *Phi1 = &Phi1_,
					Phi2_, *Phi2 = &Phi2_,
					Phi12_, *Phi12 = &Phi12_;
	...
	for (i=1; i<nstept; i++)
	{
		...
		std1der_mid[i-1] = (Phi1->a - Phi1->dLambda * std1_mid[i-1] * std1_mid[i-1]) / (2.0 * std1_mid[i-1]);
		std3der_mid[i-1] = (Phi1->a - std3_c1 * Phi2->dLambda * Phi2->dXt1 
			- std3_c2 * Phi1->dLambda * Phi1->dXt1 - std3_c3 * Phi12->dLambda * Phi12->dXt1) / (2.0 * std3_mid[i-1]);
...
CNPDE_TEMP_2D_ADI	pdestr, *pde = &pdestr;
	...
	if (!pde || !var1 || !avg_lam || !phi1 || !phi2 || !phi12 || !fwd1 || !fwd3 || !quad_var || !var1_max || !var3_max)
	{
		err = "Memory allocation error (1) in lgm2fpde";
		goto FREE_RETURN;
	}
...
FREE_RETURN:
	...
	if (phi12) free_dvector(phi12, 0, nstept-1);
	if (var1) free_dvector(var1, 0, nstept-1);
	if (avg_lam) free_dvector(avg_lam, 0, nstept-1);
	if (fwd1) free_dvector(fwd1, 0, nstept-1);
	if (fwd3) free_dvector(fwd3, 0, nstept-1);
	if (quad_var) free_dvector(quad_var, 0, nstept-1);
	if (var1_max) free_dvector(var1_max, 0, nstept-1);
	if (var3_max) free_dvector(var3_max, 0, nstept-1);
...
double			std1, std3, r2_temp, std1_next, std3_next,
				std1_mean, std3_mean, var1_temp, var3_temp,
...
CNPDE_TEMP_2D_ADI	pdestr, *pde = &pdestr;
	...
	if (!pde || !var1 || !avg_lam || !phi1 || !phi2 || !phi12 || !fwd1 || !fwd2 || !std1_mid || !std3_mid || !std1der_mid || !std3der_mid)
	{
		err = "Memory allocation error (1) in lgm2fpde";
		goto FREE_RETURN;
	}
	...
	if (!r2 || !values || !values_p1 || !mu_r1 || !mu_r3 || !var_r1 || !var_r3 || !r ||
		!r1_bar || !r1_dim1 || !r3_bar)
	{
		err = "Memory allocation error (2) in lgm2fpde";
		goto FREE_RETURN;
...
FREE_RETURN:
	...
	if (avg_lam) free_dvector(avg_lam, 0, nstept-1);
	if (fwd1) free_dvector(fwd1, 0, nstept-1);
	if (fwd2) free_dvector(fwd2, 0, nstept-1);
	if (std1_mid) free_dvector(std1_mid, 0, nstept-1);
	if (std3_mid) free_dvector(std3_mid, 0, nstept-1);
	if (std1der_mid) free_dvector(std1der_mid, 0, nstept-1);
	if (std3der_mid) free_dvector(std3der_mid, 0, nstept-1);
...
void	disc_sqrt_center(double	*x,
						 long	nstpx,
						 double *time,
						 long	nstpt,
						 int	nb_check,

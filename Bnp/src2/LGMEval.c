...
#define TREE_MESH_SPACING	1.73205080756888 	   /* 1.73205080756888 theoretically > 4  (1.414213562373) */
#define TREE_MESH_SPACING_SQUARE   3.0 
#define ONE_HALF  0.5
#define TREE_LIM_IN_STDEV	5

#define ALLOC_CONV_TREE1 {\
zeta11Deal = dvector(0,NumTimeStep); zeta12Deal = dvector(0,NumTimeStep);\
zeta22Deal = dvector(0,NumTimeStep);}

#define FREE_CONV_TREE1 {\
free_dvector(zeta11Deal, 0, NumTimeStep); free_dvector(zeta12Deal, 0, NumTimeStep);\
free_dvector(zeta22Deal, 0, NumTimeStep);\
srt_free(tDeal);}

#define ALLOC_CONV_TREE2 {\
max_index0 = lngvector(0, NumTimeStep); max_index1 = lngvector(0, NumTimeStep);\
dual_basis00 = dvector(0, NumTimeStep); dual_basis01 = dvector(0, NumTimeStep);\
dual_basis10 = dvector(0, NumTimeStep); dual_basis11 = dvector(0, NumTimeStep);\
inverse_basis00 = dvector(0, NumTimeStep); inverse_basis01 = dvector(0, NumTimeStep);\
inverse_basis10 = dvector(0, NumTimeStep); inverse_basis11 = dvector(0, NumTimeStep);\
spacing0 = dvector(0, NumTimeStep); spacing1 = dvector(0, NumTimeStep);\
IsVol1Null = lngvector(0, NumTimeStep); IsVol2Null = lngvector(0, NumTimeStep);}


#define FREE_CONV_TREE2 {\
free_lngvector(max_index0, 0, NumTimeStep); free_lngvector(max_index1, 0, NumTimeStep);\
free_dvector(dual_basis00, 0, NumTimeStep); free_dvector(dual_basis01, 0, NumTimeStep);\
free_dvector(dual_basis10, 0, NumTimeStep); free_dvector(dual_basis11, 0, NumTimeStep);\
free_dvector(inverse_basis00, 0, NumTimeStep); free_dvector(inverse_basis01, 0, NumTimeStep);\
free_dvector(inverse_basis10, 0, NumTimeStep); free_dvector(inverse_basis11, 0, NumTimeStep);\
free_dvector(spacing0, 0, NumTimeStep); free_dvector(spacing1, 0, NumTimeStep);\
free_lngvector(IsVol1Null, 0, NumTimeStep); free_lngvector(IsVol2Null, 0, NumTimeStep);}
				     ...
				     LGMErr	(*GetBeta)(Date, Date, double*))	
{
	...
	ZCtStart = DtStart*exp(H1tStart*X+H2tStart*Y
			      		-H1tStart*H2tStart*Zeta12tEx
						-0.5*(H1tStart*H1tStart*Zeta1tEx+H2tStart*H2tStart*Zeta2tEx));
	...
	for(k = 0; k <= MidAtnCpn; k++)
	{
		...
		ZCtPay = DtPay*exp(H1tPay*X+H2tPay*Y 
						   - H1tPay*H2tPay*Zeta12tEx
		...
		if( ( k ==0) && ( MidAtnCpn > 0)) SwapLevel += ZCtPay*MidAtCvgFirst;
		else if(( k > 0 ) && ( k < MidAtnCpn) )  SwapLevel += ZCtPay*MidAtCvgPay[MidAtiFirst+k-1];
		else if( k == MidAtnCpn)
		{
			if( MidAtnCpn == 0 ) SwapLevel += ZCtPay*MidAtCvgFirst;
			else  SwapLevel += ZCtPay*MidAtCvgPay[MidAtiFirst+k-1];
		}
...
long		j,
Date		*tDeal,
double		**zeta11Deal,		/* [0, 1, ..., j], values of zeta_kl the deal dates */
...
double		**zeta22Deal)		
{
	...
	while ( (k < j) ) 
	{
		...
		{
			while(tDeal[k] > tEx[index]) index++; /* tDeal[k] <= tEx[index] */

			Weight = ((double)(tDeal[k]-tEx[index-1]))/(tEx[index]-tEx[index-1]);
			(*zeta11Deal)[k] = zeta11[index-1]+Weight*(zeta11[index]-zeta11[index-1]);
			(*zeta12Deal)[k] = zeta12[index-1]+Weight*(zeta12[index]-zeta12[index-1]);
			(*zeta22Deal)[k] = zeta22[index-1]+Weight*(zeta22[index]-zeta22[index-1]);
		}
		...
		double		**xExBdry)		/* array of exercise points (in x) */
{
	...
	double		*dual_basis00, *dual_basis01, *dual_basis10, *dual_basis11,
				...
				*spacing0, *spacing1, payoff,
				**TheMatCur, **TheMatPrev, **TheMatTemp;	
	...
	double		local_var0,local_var1,local_cov, local_correl, V1, V2, C, 
				temp1, temp2, temp3, temp4, temp5, temp6, temp7, det,
				global_var_dir0, global_var_dir1, temp_global_var_dir0, 
				temp_global_var_dir1, std, lim, StateVar0, StateVar1, 
				forward0, forward1, vX0,vX1, dx, dxx, dy, dyy, xx,
				StatevarMid0, StatevarMid1, StatevarUp0, StatevarUp1,
				StatevarDown0, StatevarDown1;
	...
	double		Start, End;
	long		NumInterval;
	long		max_max_index0, max_max_index1, IndexEx = 0, IndexTemp = 0, 
				IndexCurrent = 0, i, j ,k, l, max_trunc_index,
				IndexMid0, IndexMid1, IndexUp0, IndexUp1, IndexDown0, IndexDown1, NumTimeStep;
	double		swap,l_stop;
	...
	if(xExBdry)
	{
		*xExBdry = (double*) srt_calloc(nEx, sizeof(double));
		if (*xExBdry==NULL) return("allocation failed in treeautocal2d");
	}

	/** FIRST STEP INITIALISATION OF THE DATES **/
	TimeStep =	max(1.0/DAYS_IN_YEAR,min((double)(tEx[nEx-1] - EvalDate)/DAYS_IN_YEAR / MinNode, TimeStep));
	...
	if (max_index0 == NULL || max_index1 == NULL ||
		dual_basis00 == NULL || dual_basis01 == NULL ||
		dual_basis10 == NULL || dual_basis11 == NULL ||
		inverse_basis00 == NULL || inverse_basis01 == NULL ||
		inverse_basis10 == NULL || inverse_basis11 == NULL ||
		spacing0 == NULL || spacing1 == NULL ||
		IsVol1Null == NULL || IsVol2Null== NULL)
	{
		smessage ("Warning: Allocation failed 2D Conv-Tree");
		FREE_CONV_TREE1
		FREE_CONV_TREE2
	...
	dual_basis00[0] = dual_basis11[0] = inverse_basis00[0] = inverse_basis11[0] = 1;
	dual_basis10[0] = dual_basis01[0] = inverse_basis01[0] = inverse_basis10[0] = 0;;
	...
	{	
		...
		V2 = local_var1;
		C = local_cov;
		/* Covariance is significative: need to diagonalise and multiply by eigenvalues */
		if (fabs (C) > EPS)
		{
		...
		/* DUAL BASIS : COORDINATES OF EIGENVECTORS IN TERMS OF X1, X2 MULTIPLIED BY EIGENVALUES 
		   DUAL BASIS  = SQRT(D)*P IN THE DECOMPOSITION PD(tP) */
		...
		temp_global_var_dir0 = zeta11Deal[j] * inverse_basis00[j] * inverse_basis00[j]
						+ zeta22Deal[j] * inverse_basis01[j] * inverse_basis01[j]
						+ 2.0 * zeta12Deal[j] * inverse_basis00[j] * inverse_basis01[j];
		
		/* Compute global variance in the direction of local eigenvectors */
		temp_global_var_dir1 = zeta11Deal[j] * inverse_basis10[j] * inverse_basis10[j]
		...
		if (V1 <= 1e-8 || V2 <= 1e-8)
		{
			if (fabs(temp_global_var_dir0 - global_var_dir0) < 1e-8) IsVol1Null[j] =1;
			if (fabs(temp_global_var_dir1 - global_var_dir1) < 1e-8) IsVol2Null[j] =1;
		}
	...
	TheMatCur = dmatrix(-max_max_index0, max_max_index0, -max_max_index1, max_max_index1);
	TheMatPrev = dmatrix(-max_max_index0, max_max_index0, -max_max_index1, max_max_index1);

	/** At the maturity date: initialisation of the Grid... **/
	/** (vX0,vX1)' IS A TWO DIM. GUAUSSIAN VECT.- P*SQRT(D)*(vX0,vX1)' HAS A VAR EQUAL TO  */
	for (k = -max_index0[NumTimeStep]; k <= max_index0[NumTimeStep]; k++)
		for (l = -max_index1[NumTimeStep]; l <= max_index1[NumTimeStep]; l++)
		{
			...
			StateVar0 = dual_basis00[NumTimeStep] * vX0 + dual_basis01[NumTimeStep] * vX1;
			StateVar1 = dual_basis10[NumTimeStep] * vX0 + dual_basis11[NumTimeStep] * vX1;
			/* Compute payoff for this date j */
			error = (*payofffunc)(&payoff, &swap, StateVar0,StateVar1,dealPtr,EvalDate,nEx-1,ycName,
								 zeta11,zeta22,zeta12,G1,G2,gamma,GetVol, GetBeta);
	...
	for (j = NumTimeStep - 1; j >= 0; j--)
	{
		...
		for (k = -max_index0[j]; k <= max_index0[j]; k++) /* X[k] */ 
		{
			...
			while(l_stop == 0)
			{
				...
				forward0 = inverse_basis00[j+1] * StateVar0 + inverse_basis01[j+1] * StateVar1;
				forward1 = inverse_basis10[j+1] * StateVar0 + inverse_basis11[j+1] * StateVar1;
				...
				IndexUp0 = (IndexMid0 < max_index0[j+1]) ? IndexMid0 + 1 : max_index0[j+1];
				StatevarUp0 = StatevarMid0 + spacing0[j+1]; 
					 
				IndexDown0 = (IndexMid0 > -max_index0[j+1]) ? IndexMid0 - 1 : -max_index0[j+1];
				...
				IndexUp1 = (IndexMid1 < max_index1[j+1]) ? IndexMid1 + 1 : max_index1[j+1];
				StatevarUp1 = StatevarMid1 + spacing1[j+1]; 
					 
				IndexDown1 = (IndexMid1 > -max_index1[j+1]) ? IndexMid1 - 1 : -max_index1[j+1];
				StatevarDown1 = StatevarMid1 - spacing1[j+1]; 

				/* Computes the 5 connection probabilities */
				/* Points are like this : 	0: center ; 1 : right (x1 Up/x2 Mid) ; 2 : up ; 3 : left ; 4 : down */
				dx = forward0 - StatevarMid0;
				dy = forward1 - StatevarMid1;
				dxx = 1.0 + dx * dx;
				dyy = 1.0 + dy * dy;
				p[1] = ONE_HALF * (dxx + dx*TREE_MESH_SPACING)/TREE_MESH_SPACING_SQUARE;
				p[3] = ONE_HALF * (dxx - dx*TREE_MESH_SPACING)/TREE_MESH_SPACING_SQUARE;
				p[2] = ONE_HALF * (dyy + dy*TREE_MESH_SPACING)/TREE_MESH_SPACING_SQUARE;
				p[4] = ONE_HALF * (dyy - dy*TREE_MESH_SPACING)/TREE_MESH_SPACING_SQUARE;

				if(IsVol2Null[j]) p[2] = p[4] = 0.0;
				if(IsVol1Null[j]) p[1] = p[3] = 0.0;
				...
				{
					error = (*payofffunc)(&payoff, &swap, StateVar0, StateVar1, dealPtr, EvalDate, IndexEx, ycName,
										zeta11, zeta22, zeta12, G1, G2,gamma, GetVol, GetBeta);
					...
					if ((payoff > TheMatCur[k][l])) 
					{
						...
						if((exercise == SRT_TRUE) && (xExBdry)) /* i.e 1( Y == f(X) ) == 1 */
						{
							X0 = inverse_basis00[j] * StateVar0 + inverse_basis01[j] * StateVar1;
							
							temp_global_var_dir0 = zeta11Deal[j] * inverse_basis00[j] * inverse_basis00[j]
								+ zeta22Deal[j] * inverse_basis01[j] * inverse_basis01[j]
								+ 2.0 * zeta12Deal[j] * inverse_basis00[j] * inverse_basis01[j];

							prob_vX0 = gauss(X0/sqrt(temp_global_var_dir0))/temp_global_var_dir0;
		...
		if ( (tDeal[j] == tEx[IndexEx]) && (IndexEx >= 0)) 
		{
			if( (sum_prob_vX0[IndexEx] > 0) && (xExBdry) ) (*xExBdry)[IndexEx] /= sum_prob_vX0[IndexEx];
			IndexEx--;  
		}
		
		TheMatTemp = TheMatPrev; TheMatPrev = TheMatCur; TheMatCur = TheMatTemp; 
	} /* End loop on time steps */

	(*answer) = TheMatPrev[0][0];

	/* Main Grid UnAllocation */
	free_dmatrix(TheMatCur, -max_max_index0, max_max_index0, -max_max_index1, max_max_index1);
	free_dmatrix(TheMatPrev, -max_max_index0, max_max_index0, -max_max_index1, max_max_index1);		
	if(sum_prob_vX0) free_dvector(sum_prob_vX0,0,nEx-1);sum_prob_vX0 = NULL;
		...
		double		**xExBdry)		/* array of exercise points (in x) */
{
	...
	{
			error = LGMAutoCal2DTree(nEx, tEx, zeta11[i], zeta12[i], zeta22[i], G1[i], G2[i], gamma,
									 EvalDate, ycName, GetVol, GetBeta,(void*)dealPtr, 
									 Midat2DPayoff, &(answer[i]), xExBdry);
			if (error) return error;
			sprintf(buf, "%lf\n", answer[i]);
			smessage(buf);
	}

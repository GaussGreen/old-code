    ...
    LGMErr (*GetBeta)(Date, Date, double*))
{
    ...
    ZCtStart =
        DtStart * exp(H1tStart * X + H2tStart * Y - H1tStart * H2tStart * Zeta12tEx -
    ...
    for (k = 0; k <= MidAtnCpn; k++)
    {
        ...
        ZCtPay = DtPay * exp(H1tPay * X + H2tPay * Y - H1tPay * H2tPay * Zeta12tEx -
    ...
    double** xExBdry) /* array of exercise points (in x) */
{
    ...
    double *dual_basis00, *dual_basis01, *dual_basis10, *dual_basis11, *inverse_basis00,
        *inverse_basis11, *inverse_basis01, *inverse_basis10, *spacing0, *spacing1, payoff,
    ...
    double local_var0, local_var1, local_cov, local_correl, V1, V2, C, temp1, temp2, temp3, temp4,
        temp5, temp6, temp7, det, global_var_dir0, global_var_dir1, temp_global_var_dir0,
        temp_global_var_dir1, std, lim, StateVar0, StateVar1, forward0, forward1, vX0, vX1, dx, dxx,
        dy, dyy, xx, StatevarMid0, StatevarMid1, StatevarUp0, StatevarUp1, StatevarDown0,
        StatevarDown1;
    ...
    double Start, End;
    long   NumInterval;
    long   max_max_index0, max_max_index1, IndexEx = 0, IndexTemp = 0, IndexCurrent = 0, i, j, k, l,
                                         max_trunc_index, IndexMid0, IndexMid1, IndexUp0, IndexUp1,
                                         IndexDown0, IndexDown1, NumTimeStep;
    double      swap, l_stop;
    ...
    TimeStep =
        max(1.0 / DAYS_IN_YEAR,
            min((double)(tEx[nEx - 1] - EvalDate) / DAYS_IN_YEAR / MinNode, TimeStep));
    ...
    if (max_index0 == NULL || max_index1 == NULL || dual_basis00 == NULL || dual_basis01 == NULL ||
        dual_basis10 == NULL || dual_basis11 == NULL || inverse_basis00 == NULL ||
        inverse_basis01 == NULL || inverse_basis10 == NULL || inverse_basis11 == NULL ||
        spacing0 == NULL || spacing1 == NULL || IsVol1Null == NULL || IsVol2Null == NULL)
    {
        smessage("Warning: Allocation failed 2D Conv-Tree");
        FREE_CONV_TREE1
        FREE_CONV_TREE2
    ...
    for (j = NumTimeStep - 1; j >= 0; j--)
    {
        ...
        for (k = -max_index0[j]; k <= max_index0[j]; k++) /* X[k] */
        {
            ...
            while (l_stop == 0)
            {
                ...
                /* Points are like this : 	0: center ; 1 : right (x1 Up/x2 Mid) ; 2 : up ; 3 :
                 * left ; 4 : down */

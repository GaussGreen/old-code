/* ==========================================================
   ...
   DESCRIPTION:
				...
				at the end x1 is assigned x2, x2 is assigned x3 and   
                x3 is assigned x3+dx2, y1 is assigned y2 and y2 is 
				assigned y3  
                if nstop=1, we do not proceed as x3-x2 is small                 
...
/* Check on the x values to see if convergence has been reached (RELATIVE precision)*/
    if (fabs(x[1]) != 0)
	{
...
/* Check on the y values to see if function is not locally flat (numerically null gradient)*/
	if (fabs(y[1]) != 0)
	{

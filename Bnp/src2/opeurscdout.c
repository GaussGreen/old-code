...
double srt_f_opteurscdout(double fwdx, double fwdy, double strike,
  ...
  if (matx <= 0) {
    ...
    /* Probability [ ( X <> K ) && ( Y <>B ) ] */
    prob_QT = bivar(cp * dx, du * dy, cp * du * corr);
    ...
    /* Probability [ ( X <> K ) && ( Y <>B ) ] */
    prob_QX = bivar(cp * dx, du * dy, cp * du * corr);
  ...
  case DELTAX:
    shift = fwdx / 10000;
    result =
        (srt_f_opteurscdout(fwdx + shift, fwdy, strike, barrier, sigx, sigy,
                            rho, maty, maty, disc, call_put, down_up, PREMIUM) -
         premium) /
        shift;
    return (result);
    break;
  ...
  case DELTAY:
    shift = fwdy / 10000;
    result =
        (srt_f_opteurscdout(fwdx, fwdy + shift, strike, barrier, sigx, sigy,
                            rho, maty, maty, disc, call_put, down_up, PREMIUM) -
         premium) /
        shift;
    return (result);
  ...
  case GAMMAX:
    shift = fwdx / 1000;
    result =
        srt_f_opteurscdout(fwdx + shift, fwdy, strike, barrier, sigx, sigy, rho,
                           maty, maty, disc, call_put, down_up, PREMIUM);
    result +=
        srt_f_opteurscdout(fwdx - shift, fwdy, strike, barrier, sigx, sigy, rho,
                           maty, maty, disc, call_put, down_up, PREMIUM);
    result -= 2 * premium;
  ...
  case GAMMAY:
    shift = fwdy / 1000;
    result =
        srt_f_opteurscdout(fwdx, fwdy + shift, strike, barrier, sigx, sigy, rho,
                           maty, maty, disc, call_put, down_up, PREMIUM);
    result +=
        srt_f_opteurscdout(fwdx, fwdy - shift, strike, barrier, sigx, sigy, rho,
                           maty, maty, disc, call_put, down_up, PREMIUM);
  ...
  case GAMMAXY:
    shifty = fwdy / 1000;
    shiftx = fwdx / 1000;
    result = srt_f_opteurscdout(fwdx + shiftx, fwdy + shifty, strike, barrier,
                                sigx, sigy, rho, maty, maty, disc, call_put,
                                down_up, PREMIUM);
    result += srt_f_opteurscdout(fwdx - shiftx, fwdy - shifty, strike, barrier,
                                 sigx, sigy, rho, maty, maty, disc, call_put,
                                 down_up, PREMIUM);
  ...
  case VEGAX:
    shift = GVOPT.vol_add;
    result =
        (srt_f_opteurscdout(fwdx, fwdy, strike, barrier, sigx + shift, sigy,
                            rho, maty, maty, disc, call_put, down_up, PREMIUM) -
         premium) /
        shift;
    return (result);
  ...
  case VEGAY:
    shift = GVOPT.vol_add;
    result =
        (srt_f_opteurscdout(fwdx, fwdy, strike, barrier, sigx, sigy + shift,
                            rho, maty, maty, disc, call_put, down_up, PREMIUM) -
         premium) /
        shift;
    return (result);
  ...
  case THETA:
    shift = YEARS_IN_DAY;
    result = srt_f_opteurscdout(fwdx, fwdy, strike, barrier, sigx, sigy, rho,
                                maty - shift, maty - shift,
                                disc * exp(-shift * log(disc) / maty), call_put,
                                down_up, PREMIUM) -
             premium;
    return (result);
    break;

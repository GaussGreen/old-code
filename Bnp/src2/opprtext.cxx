...
******************************************************************************/
static double
proba_joint_spot_part_min_max(double mu1, double mu2, double sig1, double sig2,
                              double mat1, double mat2, double k, double b,
  ...
  part1 = bivar(du * (-b + mu1 * mat1) / sig_sqrt1,
                cp * (-k + mu2 * mat2) / sig_sqrt2, du * cp * corr);

  part2 = bivar(du * (b + mu1 * mat1) / sig_sqrt1,
                cp * (-k + 2 * b + mu2 * mat2) / sig_sqrt2, du * cp * corr);
...
double srt_f_optprtext(double fwd1, double fwd2, double spot, double strike,
...
{ /* BEGIN srt_f_optprtext(11) */
  ...
  if ((spot - barrier) * du < 0)
    premium = 0.0;
      ...
      if (mat1 >= mat2) {
    premium = srt_f_optexting(fwd2, spot, strike, barrier, vol2, mat2, disc,
                              call_put, down_up, PREMIUM);
  } else if (mat2 == 0) /* so we don't care about mat1 */
  {
    if ((spot - strike) * cp > 0)
      premium = cp * (spot - strike);
    else
      premium = 0.0;
  } else if (mat1 <= 0) /* Standard European  */
  {
    premium =
    ...
    premium = fwd2 * proba_joint_spot_part_min_max(mu_1p, mu_2p, vol1, vol2,
                                                   mat1, mat2, k, b, cp, du);

    premium -= strike * proba_joint_spot_part_min_max(mu_1m, mu_2m, vol1, vol2,
                                                      mat1, mat2, k, b, cp, du);
    premium *= cp * disc;
  }
  ...
  case DELTA_FWD1: /*** DELTA FWD1 ***/
    shift = fwd1 / 10000;
    answer =
        (srt_f_optprtext(fwd1 + shift, fwd2, spot, strike, barrier, vol1, vol2,
  ...
  case DELTA_FWD2: /*** DELTA FWD2 ***/
    shift = fwd2 / 10000;
    answer =
        (srt_f_optprtext(fwd1, fwd2 + shift, spot, strike, barrier, vol1, vol2,
                         mat1, mat2, disc, call_put, down_up, PREMIUM) -
         premium) /
        shift;
    break;
  ...
  case DELTA: /*** DELTA FWD1 + FWD2 + SPOT ***/
    shift = spot / 10000;
    answer =
        (srt_f_optprtext(fwd1 * (1 + shift / spot), fwd2 * (1 + shift / spot),
                         spot + shift, strike, barrier, vol1, vol2, mat1, mat2,
  ...
  case VEGA1: /*** VEGA ***/
    shift = GVOPT.vol_add;
    answer =
        (srt_f_optprtext(fwd1, fwd2, spot, strike, barrier, vol1 + shift, vol2,
                         mat1, mat2, disc, call_put, down_up, PREMIUM) -
         premium) /
        shift;
    break;
  ...
  case VEGA2: /*** VEGA ***/
    shift = GVOPT.vol_add;
    answer =
        (srt_f_optprtext(fwd1, fwd2, spot, strike, barrier, vol1, vol2 + shift,
                         mat1, mat2, disc, call_put, down_up, PREMIUM) -
         premium) /
        shift;
    break;
  ...
  case THETA: /*** THETA ***/
    shift = YEARS_IN_DAY;
    answer = (srt_f_optprtext(fwd1, fwd2, spot, strike, barrier, vol1, vol2,

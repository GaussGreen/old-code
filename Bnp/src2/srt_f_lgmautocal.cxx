...
For american swaptions:
    ...
    two months apart        , and then extrapolates to get the value for
exercises every day. The value of the american swaption is *LGMamervalue.
...
semiannual markets (USD) and every year in annual markets (Europe)        ,
beginning with the initial start date of the mid-atlantic        , All these
"long" reference swaptions end at the approximate end date of mid-atlantic
"Short" swaptions: Vanilla swaptions of one year duration starting every six
months in semiannual markets (USD) and every year in annual markets (Europe) ,
beginning with the initial start date of the mid-atlantic. Caplets fitted:
Caplets that start every three or six months depending on the market
        ***If fix_tau_flag!=1        , then the code fits the long swaption and
, whenever possible        , the short swaptions. It is very rare that a deal
cannot have all short swaptions fitted exactly (4 out of 150) as well as all the
long swaptions.
        ***If fix_tau_flag==1 and usecaps!=1        , then only the long
swaptions are fitted
        ***If fix_tau_flag==1 and usecaps==1        , then only the caplets are
fitted Strikes of the long reference swaptions: The strikes are chosen so that
the swaption has the same ratio of PV(fixed leg)/PV(floating leg) as the swap
that would be recieved if the mid-atlantic was exercised at the starting date of
the reference swaption. Strikes for the short reference swaptions: The strike is
the "effective interest rate" of the mid-atlantic for that year        ,
including adjustments for exercise premiums        , etc. Strikes for the
caplets: The strikes are chosen like the short reference swaption strikes


    C. Numerical method used: Various global & local Newton routines for
fitting. Whenever fit        , swaption prices are matched to relative error
ofless than 1.e-8.
...
of 1.E-5. The midatlantic value is *LGMvalue.
 ...
 3. If the update_ts_flag is on        , the code will determine the sigma and
tau values which generate the integrals (zeta[j] and g[j] used by the code and
put them in the market denoted by SrtCurvePtr. IMPORTANT NOTE: Any method using
LGM calibration should NOT interpolate on sigmas and tau values. This is an
unstable procedure        , and may lead to inaccurate prices (it hasn't yet ,
however.) Should interpolate on the zeta(t) and G(t) values instead.
*/
...
Payment rules implemented:
 For mid-atlantics (LGMautocal)        , the payment rule is assumed to be:
    ...
    If the deal is exercised at It_ex[j]        , which has a settlement ("as
of") date It_start[j]        , then the first fixed leg payment is
fixpymnt[i]-Iredpay[j] paid on tpay[i]        , and the other fixed leg payments
are fixpymnt[i+1] on tpay[i+1]        , ...        , fixpymnt[npay-1] on
tpay[npay-1]. Here coupon i is the first coupon whose pay date tpay[i] is
strictly after the settlement date It_start[j].

    Note that the final payment should include the notional.

    The floating payments        , fees        , etc. are assumed to be
equivalent to a payment of Iprem[j] paid on the settlement date It_start[j].
Note that this includes the notional        , and should be equal to the
notional apart from fees and adjustments.
...
called. Any alterations in the floating side due to        , e.g. the floating
pay date being several days after the end date of the floating period need to be
...
end_day_flg is on.
    ...
    If the deal is exercised at T        , the theoretical start (settlement or
"as of") date S is lag_exer calendar or business days after T. The actual start
date is obtained from the theoretical start date using the Business Day Rule
conv_start.

    Let i be the first fixed coupon whose end date tfix_end[i] is strictly after
S. If early_flag_fix is off        , it is assumed that the first fixed coupon
will be reduced by the interest accrued between the coupon's start date
tfix_start[i] and the settlement date S (so that the first payment corresponds
to the interest for the period between S and tfix_end[i]). If early_fix_flag is
on        , it is assumed that the interest accrued between tfix_start[i] and S
is added to the floating leg at S        , effectively increasing the value of
the floating leg        , and the entire fixed coupon is paid at i.

        The last fixed rate payment full_pay_fix[npay-1] should include the
notional (which should be 1).

    On the floating side        , suppose that t_flt[k] is the first floating
pay date after the settlement date S        , and let coupon i be the first
fixed coupon whose pay date is strictly after S. The code assumes that all
floating leg payments that are made AFTER t_flt[k] plus any exercise premiums
are equivalent to a payment of Inprem[i] made at t_flt[k]. So like the mid-ats
, Inprem[i] should include the notional plus (for receivers) or minus (for
payers) any exercise premium.
...
floating leg. If the early_flag_flt is OFF        , then the code assumes that
the interest        , rflt*CVG(S        ,t_flt[k]) that accrues between
settlement and the next floating pay date t_flt[k] is paid on t_flt[k] in
addition to Inprem[i]. If the early_flag_flt is ON then the code assumes that
rflt*CVG(t_flt[k-1]        ,S) the interest that accrues between t_flt[k-1] and
S is paid at settlement (goes to reduce the floating leg's value) and that the
full floating coupon rflt*CVG(t_flt[k-1]        ,t_flt[k]) is paid at t_flt[k]
in addition to Inprem[i].

    In the above        , rflt is the floating rate for the period from
t_flt[k-1] to t_flt[k]. If t_flt[k-1] <= tnow < t_flt[k]        , so that today
is in the interval , then the code will use the input argument rflt_current for
this already-fixed interest rate. If rflt_current is zero or negative        ,
or if tnow does not fall in the interval        , then the code will calculate
rflt from the current discount curve.
        */

/******** WARNINGS ***********
1) The payment rules must be repected when entering deals with weird frequencies
      , long payment or start lags        , etc. 2) If the BS vols are bumped
one at a time for vega calculations        , best results are obtained if the
vols are bumped by a twentieth to a fiftieth of a per cent or less. If the BS
vols are all bumped simultaneously        , bumping by a per cent is easily OK.
...
double swp_disc_mkt(Date t1        , Date t2        , SrtCurvePtr yldcrv , int
1) discount factor between t1 and t2 for mkt double coverage(Date t1        ,
Date t2        , BasisCode basis) coverage from t1 to t2 according to basis
...
input if it is zero or negative. To implement        , need to uncomment and
delete some lines */
...
String LGMamerican(
    ...
    Date tfix_start[], /* [0        ,1        ,...        ,nfix-1]  start dates
                        * for fixed coupons
                        */
    Date tfix_end[], /* [0        ,1        ,...        ,nfix-1]  end dates for
                        fixed coupons   */
    Date tfix_pay[], /* [0        ,1        ,...        ,nfix-1]  pay dates for
                        fixed coupons   */
    double full_pay_fix[], /* [0        ,1        ,...        ,nfix-1]  fixed
                        coupons (with notional at end)   */
    double Inprem[],    /* [0        ,1        ,...        ,nfix-1]  premium for
                     exer. at j (with    notional) */
    ...
    Date t_flt[], /* [0        ,1        ,...        ,nflt-1] all flting dtes
               (all strt dtes plus final end dte) */
    ...
    int usecaps,   /* if fixed tau        , then 1=use caplets to calibrate   ,
                0=use   long swptns */
        ...
        *lgmExerBdryData) /* ptr to exercise boundary data structure         */
{
  ...
  /* subdivide pay periods into 1 mo and 2mo subdivisions        , but ensure
     that the first exercise start date and all fixed start dates are included
   */
...
String LGMautocal(
    ...
    Date It_ex[], /* notification (exercise) dates   T[0        ,1        ,...
                     ,J]    */
    ...
    String (*GetVol)(Date, Date, double, SRT_Boolean,
                     double *),              /* function to get swaption vols */
    char *char_vol_type, int update_ts_flag, /* 1=compte new sigs and taus &
                                          store in yldcrv; 0=don't bother */
    int find_exer_bdry, /* 1 = find swap rates at exercise bocrvary; 0 = don't
                     bother  */
    ...
    int skip_deal_evaluation) /* 0 for a noraml valuation        , 1 for a
                                 simple volatility points extraction */
{
  /* Declarations */
  ...
  /* sets natural_compnd to SRT_SEMIANNUAL or SRT_ANNUAL        , sets
     natural_basis; sets natural_conv        , and sets natural_spot_lag  */
  ...
  t_ex = (Date *)srt_calloc(
      nex + 1, sizeof(Date)); /* t_ex[0        ,1        ,...        ,nex]   */
  t_start = (Date *)srt_calloc(
      nex + 1, sizeof(Date)); /* t_start[0        ,..        ,nex]   */
  ifirst = (long *)srt_calloc(
      nex + 1, sizeof(long)); /* ifirst[0        ,..        ,nex]    */
  pvpay = (double *)srt_calloc(
      npay, sizeof(double)); /* pvpay[0        ,...        ,npay-1] */
  redpay = (double *)srt_calloc(
      nex + 1, sizeof(double)); /* redpay[0        ,...        ,nex]   */
  phi_cr = (double *)srt_calloc(
      nex + 1, sizeof(double)); /* phi_cr[0        ,...        ,nex]   */
  ...
  for (j = 0; j < In_ex; j++) {
    if (It_ex[j] >= nextdate) {
      nex++;
      t_ex[nex] = It_ex[j]; /* t_ex[0        ,1        ,2        , ...        ,
                               nex]    */
      t_start[nex] = It_start[j]; /* t_start[0        ,1        ,2        , ...
                                     , nex] */
      origprem[nex] = Iprem[j];   /* prem[0        ,1        ,2        , ...   ,
                                     nex]    */
      redpay[nex] =
          Iredpay[j]; /* redpay[0        ,1        , ...        , nex]    */
  ...
  /* This is the deal that will be evaluated:
          ...
          if exercised at t_ex[j]        , then fixed leg will receive the
     payments fixpymnt[i]-redpay[i] paid on tpay[i] for i=ifirst[j] fixpymnt[i]
     paid on tpay[i] for i=ifirst[j]+1        , ifirst[j]+2 , ...        ,
     npay-1 the floating leg will receive a single payment of origprem[j] paid
     on  t_start[j] ifirst[j] (calculated next) is first i for which
     tpay[i]>t_start[j] */
  ...
  /* Reference instruments:
          If fix_tau_flag != 1        , code uses swaptions with start dates are
     s[j] , j=0        ,1        ,...        ,nlast. End dates for short
     swaptions are s[j+1] (annual markets) or s[j+2] (semiannual markets). End
     dates for long swaptions are s[n]. Pay dates are s[i]        , i=j+1 ,...
     ,n. Exercise dates precede s[j] by natural_spot_lag. If fix_tau_flag == 1
     , and usecaps !=1 code uses long swaptions but not the short ones If
     fix_tau_flag == 1 and usecaps ==1        , code uses caps with start dates
     s[j] and end dates s[j+1]        , j=0        ,1        ,... ,nlast*/
  ...
  /*sets Rmin_sh[j]        , Rmax_sh[j]        , Rmin_lg[j]        , and
   * Rmax_lg[j] */
  ERROR_CHECK;
  ...
  /* To simplify the coding        , when we don't need both long and short
     rates        , we copy the needed one into the unneeded one */
  if ((fix_tau_flag == 1) && (usecaps != 1)) {
    for (j = 0; j <= nlast; j++) {
...
/* fit g[j] and zeta[j] by matching LGM values of vanilla swaptions to
   market (BS) values. Swaptions used:
      start s[j]        , end s[n]        , strike Ract_lg[j]        ,  (fit
   exactly) start s[j]        , end one year later        , strike Ract_sh[j]
   (fit whenever possible) for j = 0        ,1        , ...        ,nlast Uses
   global Newton routine (semiannual or annual) for fit                */
...
*/
  ...
  for (j = (nlast - 1); j >= 0; j--) {
    ...
    /* fit long swaption by changing either zeta or g        , keeping the other
     * at its extreme value */
...
/* fit zeta[j] by matching LGM values of vanilla long swaptions (if usecaps!=1)
   ...
   ... ,nlast Reference caplets start s[j]        , end s[j+1]        , strike
   Ract_sh[j]  for j = 0        ,1        , ...        ,nlast
...
static String mid_atl_fit_market_fix_tau(
  ...
  */
  ...
  /*    for (j=0; j<=nlast; j++)
      {   sly = s[j] - add_unit(s[j]        , -spot_lag        , SRT_BDAY ,
     SUCCEEDING); sly = sly/365.0; zeta[j] = zeta[j] +
     (zeta[j+1]-zeta[j])*sly/(sy[j+1]-sy[j]);
      }
  ...
  compute pv of the reduction in 1rst payment at each start date redpay[j] */
{
  ...
  /* for each exercise        , find first coupon whose pay date > settlement
   * date */
  i = 0;
  for (j = 0; j <= nex; j++) {
...
static void choose_dates(Date tnow, Date tstart, long *nptr, long npay,
  ...
  nmin =
      ((icmpnd == 1) ? 2
                     : 1); /* if icmpnd=1        , min tenor is two periods */
  ...
  */
  ...
  /*        tmaybe = add_unit(tnow        , num_months        , SRT_MONTH ,
     natural_conv) + 14; if (tmin < tmaybe)	*/	  /* Avoids calib. on instrmnts with less than 14 days to exer */
  /*           tmin = tmaybe;
  ...
  */
  ...
  /*    if (n<nmin)
      {   for (j=n+1; j<=nmin && j < MAX_REF_DATES; j++)
              s[j] = add_unit(t_ex[start_ex]        , num_months*n        ,
     SRT_MONTH        , natural_conv); n=nmin;
      }
  */
...
static void smooth_strikes(long nlast, double smth) {
  ...
  /* PH 2/18/98 If nlast < 1 (one period)        , there is no smoothing to be
   * done */
  if (nlast < 1)
    return;
   ...
   the delta irr  of the underlying swaps */
{
  ...
  /* interpolate on ratio[j] to find its value at s[j] and s[j+delay]        ,
     and choose the interest rate of the short swaption so that it is equivalent
     to buying a long swaption at s[j] with the first ratio and selling one at
  ...
  if (fix_tau_flag != 1) {
    rat = R_theor_lg[0]; /* if not enough exercise dates        , use R_lg for
                            R_sh */
...
/* Calculate the Black-Scholes price of swaption beginning at s[jst]
   and ending at s[nst]. Fixed rate r        , BS volatility sig        , expiry
   is s[jst]        , and pr=1 for a payer and pr=0 for reciever. */
...
/* Calculate the LGM value (val) of vanilla swaption starting at s[jst]        ,
   ...
   Also calculate the derivatives wrt sqrt(zeta[jst])        , g[j]        , and
   g[j+1] (v1        , v2        , and v3). phiact = initial guess for "at the
   money" phi needs to be set (at zero if nothing else)        , and the fixed
   rate Ract needs to be set */
  ...
  for (j = 0; j <= nlast; j++) {
        ...
        (usecaps == 1)) /*get par swap rate for short swaption */
    {
      ...
      /* PH: 2/18/98 Will this work for caplets? If so        , uncomment
                      if (err = srt_f_setSwapDP(s[j]        , s[j+dj]        ,
         compounding , basis        , &sdp)) return err;

                      err = swp_f_ForwardRate_SwapDP( &sdp        , s_yldcrv ,
         "CASH" , &fwd); if (err) return err;
      */
...

{
  ...
  zeta_ex = (double *)srt_calloc(
      nex + 1, sizeof(double)); /* zeta_ex[0        ,...        ,nex]   */
  G_start = (double *)srt_calloc(
      nex + 1, sizeof(double)); /* G_start[0        ,...        ,nex]   */
  G_pay = (double *)srt_calloc(
      npay, sizeof(double)); /* G_pay[0        ,...        ,npay-1]  */
  wgt = (double *)srt_calloc(
      nq + 1, sizeof(double)); /* wgt[0        ,...        ,nq] */
  wgt2 = (double *)srt_calloc(
      nq2 + 1, sizeof(double)); /* wgt[0        ,...        ,nq2] */
  Q0 = (double *)srt_calloc(
      nphi + 1, sizeof(double)); /* Q0[0        ,...        ,nphi] */
  Q1 = (double *)srt_calloc(
      nphi + 1, sizeof(double)); /* Q1[0        ,...        ,nphi] */
  ...
  for (j = nex; j >= 0; j--) {
    ...
    /* Evaluate mid-atlantic        , Q0        , at previous time step by
     * integration */
    if (j > 0)
...
static String Findexerbdry(Date tnow, long nex, Date t_ex[], Date tend,
  ...
  tnatst = (Date *)srt_calloc(
      nex + 1,
      sizeof(Date)); /* tnatst[0        , 1        , ...        , nex]     */
  exerBdryArr = srt_calloc(nex + 1, sizeof(SrtLgmExerBdry));
  zeta_ex = (double *)srt_calloc(
      nex + 1,
      sizeof(double)); /* zeta_ex[0        , 1        , ...        , nex]    */
  G_st = (double *)srt_calloc(
      nex + 1,
      sizeof(double)); /* G_st[0        , 1        , ...        , nex]       */
  B_st = (double *)srt_calloc(
      nex + 1,
      sizeof(double)); /* B_st[0        , 1        , ...        , nex]       */
  aB_first = (double *)srt_calloc(
      nex + 1,
      sizeof(double)); /* aB_first[0        , 1        , ...        , nex]   */
  ifirstpay = (long *)srt_calloc(
      nex + 1,
      sizeof(long)); /* ifirstpay[0        , 1        , ...        , nex]  */

  paydate = (Date *)srt_calloc(
      ncoup + 1,
      sizeof(Date)); /* paydate[0        , 1        , ...        , ncoup]  */
  G_pay = (double *)srt_calloc(
      ncoup + 1,
      sizeof(double)); /* G_pay[0        , 1        , ...        , ncoup]    */
  aB_pay = (double *)srt_calloc(
      ncoup + 1,
      sizeof(double)); /* aB_pay[0        , 1        , ...        , ncoup]   */
...
static Err get_ccy_defaults(SrtCurvePtr yldcrv, int fix_tau_flag, int usecaps,
  ...
  /* OVE: this will work as long as SrtInitYc is called        , that sets
   * default ccy_param */

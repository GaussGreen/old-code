...
static void disc_normal_center(
    double* x,
    long    nstp,
    double  fwd,
    double  xmin,
    ...
    double  nstd,
    long*   index);
static void disc_linleft_logright(
    double* x,
    long    nstp,
    double  fwd,
    double  xmin,
    ...
    double  nstd,
    long*   index);
static void disc_linleft_logright_strike(
    double* x,
    long    nstp,
    double  fwd,
    double  xmin,
    ...
    double* result)
{
    ...
    double *x = NULL, *z = NULL, **mux = NULL, **muz = NULL, **r = NULL, **varx = NULL,
           **varz = NULL, ***v1 = NULL, ***v2 = NULL, ***vtmp = NULL;
    ...
    if (fabs(beta) < 1.0e-08)
    {
        ...
        calculate_proba_backward_2f_adi(
            adi, nx, x, nz, z, 1, v1, mux, muz, varx, varz, r, v2, 0, nx - 1, 0, nz - 1);
    ...
    /*	Case 2: Beta != 0 Beta grid on F and normal grid on log[sigma/sigma0]*/
    {
        ...
        if (!x || !z || !mux || !muz || !r || !varx || !varz || !v1 || !v2 || !sig_init ||
            !varx_init || !muz_init || !exp_z || !drift_z)
        {
            err = "Memory allocation error in op_sabr_adi";
            goto FREE_RETURN;
        }
        ...
        for (i = 0; i < nx; i++)
        {
            ...
            varx_init[i] =
                tempi * tempi * dt; /*	F ^ (2 * beta) */
            muz_init[i] =
                temp2 * tempi2; /*	temp2 * F ^ (beta - 1)					*/
        ...
        for (stp = nt - 2; stp >= 0; stp--)
        {
            ...
            num_f_pde_one_step_backward_2f_adi(
                adi, nx, x, nz, z, 0, 0, v1, mux, muz, varx, varz, r, v2, 0, nx - 1, 0, nz - 1);
    ...
    double*          res)
{
    ...
    tgt_price = srt_f_optblksch(forward, strike, tgt_vol, maturity, 1, SRT_CALL, PREMIUM);
    ...
    {
        err = op_sabr_adi(
            forward, strike, maturity, 1.0, 0, vol1, alpha, beta, rho, nt, nx, nz, &price1);
        ...
        err = srt_f_optimpvol(
            price1, forward, strike, maturity, 1.0, SRT_CALL, input_vol_type, &impvol1);
    ...
    {
        err = op_sabr_adi(
            forward, strike, maturity, 1.0, 0, vol2, alpha, beta, rho, nt, nx, nz, &price2);
        ...
        err = srt_f_optimpvol(
            price2, forward, forward, maturity, 1.0, SRT_CALL, input_vol_type, &impvol2);
    ...
    double* pdNewRho)
{
    ...
    for (i = 1; i < num_strikes + 1 && !err; i++)
    {
        err = op_sabr_adi(
            ...
            &dPrice);
        err = srt_f_optimpvol(
            dPrice, forward, strikes[i], maturity, 1.0, SRT_CALL, SRT_LOGNORMAL, &vols[i]);
    }
   ...
   // err = srt_BGMNewRhoSABR(forward, maturity, ATMVol, alpha, beta, rho, newbeta, &bx);
    ...
    /* ----------------------------------------------------------------------------------------------------------------------------------*/
    /* 1-D Calibration using 'golden'.  Assumes that the error function is quadratic in rho.  We
     * also need a point that has an error     */
    /* less than the error at +/- 1  Try to bracket the minimum by expanding about the analytic
     * approximation */
    ...
    for (i = 1; i < num_strikes + 1 && !err; i++)
    {
        err = op_sabr_adi(
            ...
            &dPrice);
        err = srt_f_optimpvol(
            dPrice, forward, strikes[i], maturity, 1.0, SRT_CALL, SRT_LOGNORMAL, &dTempVol);
...
/* ---------------------------------------------------------------------------------------------------------------------------------
 */
/* Given a set of SABR parameters, calculates the price, and then calculates the alpha necessary in
 * the ADI model to match the price */
/* ---------------------------------------------------------------------------------------------------------------------------------
 */
#define R 0.61803399
#define C (1.0 - R)
    ...
    double* pdNewAlpha)
{
    ...
    /*
                    ...
                    strikeMin = forward * exp( - 0.5 * dApproxStdev * dApproxStdev - n_StdDev *
       dApproxStdev); strikeMax = forward * exp( - 0.5 * dApproxStdev * dApproxStdev + n_StdDev *
       dApproxStdev);
    */
    for (i = 1; i <= num_strikes; i++)
    ...
    /* Calculate the sigmabeta for the given ATM vol (using the old value of beta and rho */
    //	err=op_sabr_calib_adi(
    //forward,forward,maturity,ATMVol,SRT_LOGNORMAL,alpha,beta,rho,nt,nx,nz,128,&dSigmaBeta);
    ...
    /* ----------------------------------------------------------------------------------------------------------*/
    /* Step1: We try to "bracket the minimum of the objective function. We find three points
       ax<bx<cx such that the value of the objective function is minimum at f(bx). These points will
       be the inputs of the next function
    */
    ...
    /*
            ...
            ax = bx;
            err=op_sabr_calib_adi(
       forward,forward,maturity,ATMVol,SRT_LOGNORMAL,ax,beta,rho,nt,nx,nz,128,&dSigmaBeta); f1 =
       0.0; for (i=1;i<num_strikes+1 && !err;i++)
            {
                    dPrice = op_sabr_adi( forward, strikes[i], maturity, 1.0, 0, dSigmaBeta, ax,
       beta, rho, nt, nx, nz ); err = srt_f_optimpvol( dPrice, forward, strikes[i], maturity, 1.0,
       SRT_CALL, SRT_LOGNORMAL, &dTempVol ); f1 += ( dTempVol-vols[i] ) * ( dTempVol-vols[i] );
            }
            f2 = f1 - 1.0;
            while ( f2 < f1 && !err)
            {
                    ...
                    else
                    {
                            ...
                            for (i=1;i<num_strikes+1 && !err;i++)
                            {
                                    dPrice = op_sabr_adi( forward, strikes[i], maturity, 1.0, 0,
       dSigmaBeta, ax, beta, rho, nt, nx, nz ); err = srt_f_optimpvol( dPrice, forward, strikes[i],
       maturity, 1.0, SRT_CALL, SRT_LOGNORMAL, &dTempVol ); f2 += ( dTempVol-vols[i] ) * (
       dTempVol-vols[i] );
            ...
            while ( f2 < f1 && !err)
            {
                    ...
                    else
                    {
                            ...
                            {
                                    dPrice = op_sabr_adi( forward, strikes[i], maturity, 1.0, 0,
       dSigmaBeta, cx, beta, rho, nt, nx, nz ); err = srt_f_optimpvol( dPrice, forward, strikes[i],
       maturity, 1.0, SRT_CALL, SRT_LOGNORMAL, &dTempVol ); f2 += ( dTempVol-vols[i] ) * (
       dTempVol-vols[i] );
    ...
    */
    ...
    err = op_sabr_adi(
        forward, forward, maturity, Disc, SRT_CALL, ATMVol, x1, beta, rho, nt, nx, nz, &dPrice);
    err = srt_f_optimpvol(
        dPrice, forward, forward, maturity, Disc, SRT_CALL, SABR_LOGVOL, &atm_ADI_vol);
    ...
    for (i = 1; i <= num_strikes && !err; i++)
    {
        err = op_sabr_adi(
            ...
            &dPrice);
        err = srt_f_optimpvol(
            dPrice, forward, strikes[i], maturity, Disc, SRT_CALL, SRT_LOGNORMAL, &dTempVol);
        f1 += (dTempVol + gap - vols[i]) * (dTempVol + gap - vols[i]);
    }

    err = op_sabr_adi(
        forward, forward, maturity, Disc, SRT_CALL, ATMVol, x2, beta, rho, nt, nx, nz, &dPrice);
    err = srt_f_optimpvol(
        dPrice, forward, forward, maturity, Disc, SRT_CALL, SABR_LOGVOL, &atm_ADI_vol);
    gap = atm_SABR_vol - atm_ADI_vol;

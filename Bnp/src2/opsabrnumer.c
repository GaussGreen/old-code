...
static void disc_normal_center(double *x, long nstp, double fwd, double xmin,
                               double xmax, double stdn, double nstd,
                               long *index);
static void disc_linleft_logright(double *x, long nstp, double fwd, double xmin,
                                  double xmax, double stdln, double nstd,
                                  long *index);
static void disc_linleft_logright_strike(double *x, long nstp, double fwd,
...
Err op_sabr_adi(double forward, double strike, double maturity, double disc,
  ...
  double *x = NULL, *z = NULL, **mux = NULL, **muz = NULL, **r = NULL,
         **varx = NULL, **varz = NULL, ***v1 = NULL, ***v2 = NULL,
         ***vtmp = NULL;

  double *varx_init = NULL, *muz_init = NULL, **sig_init = NULL;

  double *exp_z = NULL, *drift_z = NULL;

  double *muxi, *muzi, *varxi, *varzi, *ri, *sig_initi, **v1i;
  ...
  if (fabs(beta) < 1.0e-08) {
    ...
    calculate_proba_backward_2f_adi(adi, nx, x, nz, z, 1, v1, mux, muz, varx,
                                    varz, r, v2, 0, nx - 1, 0, nz - 1);

    /*	Do the adi */
    for (stp = nt - 2; stp >= 0; stp--) {
      num_f_pde_one_step_backward_2f_adi_precalc_proba(
          adi, nx, x, nz, z, 1, v1, mux, muz, varx, varz, r, v2, 0, nx - 1, 0,
          nz - 1);
  ...
  /*	Case 2: Beta != 0 Beta grid on F and normal grid on log[sigma/sigma0]*/
  {
    ...
    fu = fl + 2.0 * NSTD * std;
    disc_linleft_logright_strike(x, nx, forward, exp(fl), exp(fu), std, NSTD,
                                 strike, &cx);

    /*	Limits of Z */
    z0 = rho * alpha / (1.0 - beta) * (pow(forward, 1.0 - beta) - 1.0) -
         sigma_beta;
    std = alpha * sigma_beta * sqrt(1.0 - rho * rho) * sqrt(maturity);
    ...
    const_exp1 = -0.5 * rho * alpha * beta * pow(forward, beta - 1.0) *
    ...
    for (stp = nt - 2; stp >= 0; stp--) {
      ...
      num_f_pde_one_step_backward_2f_adi(adi, nx, x, nz, z, 0, 0, v1, mux, muz,
                                         varx, varz, r, v2, 0, nx - 1, 0,
                                         nz - 1);
...
FREE_RETURN:
  ...
  if (sig_init)
    free_dmatrix(sig_init, 0, nx - 1, 0, nz - 1);
  if (varx_init)
    free_dvector(varx_init, 0, nx - 1);
  if (muz_init)
    free_dvector(muz_init, 0, nx - 1);

  if (exp_z)
    free_dvector(exp_z, 0, nt - 1);
  if (drift_z)
    free_dvector(drift_z, 0, nt - 1);
...
op_sabr_adi_simple(double forward, double strike, double maturity, double disc,
  ...
  /*	Case 2: Beta != 0 Beta grid on F and normal grid on log[sigma/sigma0]*/
  {
    ...
    if (beta <= 1.0 && beta >= 0.0) {
      err = srt_f_optsarbvol(forward, forward, maturity, sigma_beta, alpha,
                             beta, rho, SRT_BETAVOL, SRT_LOGNORMAL, &bs_vol);
    ...
    */
    ...
    std = sigma_beta * sqrt(1.0 - rho * rho) *
          sqrt(exp(alpha * alpha * maturity) - 1.0);
    ...
    for (i = 0; i < nx; i++) {
      F = fabs(x[i]) +
          1.0e-06;          /*	F has to be strictly greater than 0.0	*/
      tempi = pow(F, beta); /*	F ^ beta
                             */
      tempi2 =
          tempi /
  ...
  /*	Do the adi */
  for (stp = nt - 2; stp >= 0; stp--) {
    num_f_pde_one_step_backward_2f_adi_precalc_proba(adi, nx, x, nz, z, 1, v1,
                                                     mux, muz, varx, varz, r,
                                                     v2, 0, nx - 1, 0, nz - 1);
...
Err op_sabr_mc_ln(double forward, int nb_strike, double *strike,
  ...
  double *lnct = NULL, *res1 = NULL, *res2 = NULL;
  ...
  *fwdres = disc * res3;
  ...
  if (lnct)
    free_dvector(lnct, 0, nb_strike - 1);
  if (res1)
    free_dvector(res1, 0, nb_strike - 1);
  if (res2)
    free_dvector(res2, 0, nb_strike - 1);
...
Err op_sabr_mc_nor(double forward, int nb_strike, double *strike,
  ...
  double *diff = NULL, *res1 = NULL, *res2 = NULL;
  ...
  for (path = 0; path < npaths; path++) {
    ...
    for (i = 0; i < nb_strike; i++) {
      ...
      price =
          (diff[i] + drift) * norm(d1) + sqvar * exp(-d1 * d1 / 2.0) * pirac;
      res1[i] += price / npaths;
      res2[i] += price * price / npaths;
    }
  }
  ...
  if (diff)
    free_dvector(diff, 0, nb_strike - 1);
  if (res1)
    free_dvector(res1, 0, nb_strike - 1);
  if (res2)
    free_dvector(res2, 0, nb_strike - 1);
...
/* This routine currently uses a very simple Euler discretization of the SABR
 * equations */
...
Err op_sabr_calib_adi(double forward, double strike, double maturity,
  ...
  tgt_price =
      srt_f_optblksch(forward, strike, tgt_vol, maturity, 1, SRT_CALL, PREMIUM);
  ...
  while (err && k < nbiter) {
    ...
    err = op_sabr_adi(forward, strike, maturity, 1.0, 0, vol1, alpha, beta, rho,
                      nt, nx, nz, &price1);
    ...
    err = srt_f_optimpvol(price1, forward, strike, maturity, 1.0, SRT_CALL,
                          input_vol_type, &impvol1);
  ...
  /*	If we have an error  , calculate a second point using SABR closed form
   * vega */

  if (beta >= 0.0 && beta <= 1) {
    err = op_sabr_pricing(forward, strike, maturity, 1.0, SRT_CALL, alpha, beta,
                          rho, vol1, SABR_ATM_BETA, &Sabrp1);
    if (err) {
      return err;
    }
    ...
    err = op_sabr_pricing(forward, strike, maturity, 1.0, SRT_CALL, alpha, beta,
                          rho, (vol1 + shift), SABR_ATM_BETA, &Sabrp2);
    if (err) {
      return err;
    }
    ...
    err = op_sabr_adi(forward, strike, maturity, 1.0, 0, (vol1 + shift), alpha,
                      beta, rho, nt, nx, nz, &price2);
    if (err) {
      return err;
    }
  ...
  while ((k < nbiter) && (errorv > precision)) {
    err = op_sabr_adi(forward, strike, maturity, 1.0, 0, vol2, alpha, beta, rho,
                      nt, nx, nz, &price2);
    ...
    err = srt_f_optimpvol(price2, forward, forward, maturity, 1.0, SRT_CALL,
                          input_vol_type, &impvol2);
...
Err srt_ADINewRhoSABR(double forward, double maturity, double ATMVol,
  ...
  for (i = 1; i <= num_strikes; i++)
    strikes[i] =
        forward * exp((i - (iNumStrikes + 1) / 2) * ATMVol * sqrt(maturity));

  /* Calculate the sigmabeta for the given ATM vol (using the old value of beta
   * and rho */
  err =
      op_sabr_calib_adi(forward, forward, maturity, ATMVol, SRT_LOGNORMAL,
                        alpha, beta, rho, nt, nx, nz, 128, 1e-04, &dSigmaBeta);

  /* Calculates the vols at strikes */
  for (i = 1; i < num_strikes + 1 && !err; i++) {
    err = op_sabr_adi(forward, strikes[i], maturity, 1.0, 0, dSigmaBeta, alpha,
                      beta, rho, nt, nx, nz, &dPrice);
    err = srt_f_optimpvol(dPrice, forward, strikes[i], maturity, 1.0, SRT_CALL,
                          SRT_LOGNORMAL, &vols[i]);
  }

  /* Use the SABR vol approximation to get a good first guess */
  err = srt_BGMNewRhoSABR(forward, maturity, ATMVol, alpha, beta, rho, newbeta,
                          &bx);

  /* ----------------------------------------------------------------------------------------------------------------------------------*/
  /* 1-D Calibration using 'golden'.  Assumes that the error function is
   * quadratic in rho.  We also need a point that has an error     */
  /* less than the error at +/- 1  Try to bracket the minimum by expanding about
   * the analytic approximation */

  ax = bx;
  err = op_sabr_calib_adi(forward, forward, maturity, ATMVol, SRT_LOGNORMAL,
                          alpha, newbeta, ax, nt, nx, nz, 128, 1.0e-04,
                          &dSigmaBeta);
  f1 = 0.0;
  for (i = 1; i < num_strikes + 1 && !err; i++) {
    err = op_sabr_adi(forward, strikes[i], maturity, 1.0, 0, dSigmaBeta, alpha,
                      newbeta, ax, nt, nx, nz, &dPrice);
    err = srt_f_optimpvol(dPrice, forward, strikes[i], maturity, 1.0, SRT_CALL,
                          SRT_LOGNORMAL, &dTempVol);
  ...
  while (f2 < f1 && !err) {
    if ((ax -= dJump) < -1.0)
      ...
      f2 = 0.0;
      for (i = 1; i < num_strikes + 1 && !err; i++) {
        err = op_sabr_adi(forward, strikes[i], maturity, 1.0, 0, dSigmaBeta,
                          alpha, newbeta, ax, nt, nx, nz, &dPrice);
        err = srt_f_optimpvol(dPrice, forward, strikes[i], maturity, 1.0,
                              SRT_CALL, SRT_LOGNORMAL, &dTempVol);
  ...
  while (f2 < f1 && !err) {
    if ((cx += dJump) > 1.0)
      ...
      for (i = 1; i < num_strikes + 1 && !err; i++) {
        err = op_sabr_adi(forward, strikes[i], maturity, 1.0, 0, dSigmaBeta,
                          alpha, newbeta, cx, nt, nx, nz, &dPrice);
        err = srt_f_optimpvol(dPrice, forward, strikes[i], maturity, 1.0,
                              SRT_CALL, SRT_LOGNORMAL, &dTempVol);
        f2 += (dTempVol - vols[i]) * (dTempVol - vols[i]);
      }
  ...
  f1 = 0.0;
  err = op_sabr_calib_adi(forward, forward, maturity, ATMVol, SRT_LOGNORMAL,
                          alpha, newbeta, x1, nt, nx, nz, 128, 1.0e-04,
                          &dSigmaBeta);
  for (i = 1; i < num_strikes + 1 && !err; i++) {
    err = op_sabr_adi(forward, strikes[i], maturity, 1.0, 0, dSigmaBeta, alpha,
                      newbeta, x1, nt, nx, nz, &dPrice);
    err = srt_f_optimpvol(dPrice, forward, strikes[i], maturity, 1.0, SRT_CALL,
                          SRT_LOGNORMAL, &dTempVol);
    f1 += (dTempVol - vols[i]) * (dTempVol - vols[i]);
  }
  ...
  f2 = 0.0;
  err = op_sabr_calib_adi(forward, forward, maturity, ATMVol, SRT_LOGNORMAL,
                          alpha, newbeta, x2, nt, nx, nz, 128, 1.0e-04,
                          &dSigmaBeta);
  for (i = 1; i < num_strikes + 1 && !err; i++) {
    err = op_sabr_adi(forward, strikes[i], maturity, 1.0, 0, dSigmaBeta, alpha,
                      newbeta, x2, nt, nx, nz, &dPrice);
    err = srt_f_optimpvol(dPrice, forward, strikes[i], maturity, 1.0, SRT_CALL,
                          SRT_LOGNORMAL, &dTempVol);
    f2 += (dTempVol - vols[i]) * (dTempVol - vols[i]);
  }
  ...
  while (fabs(x3 - x0) > tol && !err) {
    if (f2 < f1) {
      ...
      for (i = 1; i < num_strikes + 1 && !err; i++) {
        err = op_sabr_adi(forward, strikes[i], maturity, 1.0, 0, dSigmaBeta,
                          alpha, newbeta, x2, nt, nx, nz, &dPrice);
        err = srt_f_optimpvol(dPrice, forward, strikes[i], maturity, 1.0,
                              SRT_CALL, SRT_LOGNORMAL, &dTempVol);
        f2 += (dTempVol - vols[i]) * (dTempVol - vols[i]);
      }
      ...
      for (i = 1; i < num_strikes + 1 && !err; i++) {
        err = op_sabr_adi(forward, strikes[i], maturity, 1.0, 0, dSigmaBeta,
                          alpha, newbeta, x1, nt, nx, nz, &dPrice);
        err = srt_f_optimpvol(dPrice, forward, strikes[i], maturity, 1.0,
                              SRT_CALL, SRT_LOGNORMAL, &dTempVol);
        f1 += (dTempVol - vols[i]) * (dTempVol - vols[i]);
      }
...
/* ---------------------------------------------------------------------------------------------------------------------------------
 */
/* Given a set of SABR parameters  , calculates the price  , and then calculates
 * the alpha necessary in the ADI model to match the price */
/* ---------------------------------------------------------------------------------------------------------------------------------
 */
#define R 0.61803399
#define C (1.0 - R)
...
Err srt_AlphaFromADI(double forward, double maturity, double ATMVol,
  ...
  /* Calculates the strikes */
  /*
                  dApproxStdev = ATMVol / pow(forward  , 1.0 - beta) *
     sqrt(maturity); n_StdDev=3;

                  strikeMin = forward * exp( - 0.5 * dApproxStdev * dApproxStdev
     - n_StdDev * dApproxStdev); strikeMax = forward * exp( - 0.5 * dApproxStdev
     * dApproxStdev + n_StdDev * dApproxStdev);
  */
  for (i = 1; i <= num_strikes; i++)
    strikes[i] =
        strikeMin + (i - 1) * (strikeMax - strikeMin) / (num_strikes - 1);

  /* Calculate the sigmabeta for the given ATM vol (using the old value of beta
   * and rho */
  //	err=op_sabr_calib_adi( forward  ,forward  ,maturity  ,ATMVol
  //,SRT_LOGNORMAL  ,alpha  ,beta  ,rho  ,nt  ,nx  ,nz  ,128  ,&dSigmaBeta);

  /* Calculates the SABR Log smile */
  for (i = 1; i < num_strikes + 1 && !err; i++)
    err = srt_f_optsarbvol(forward, strikes[i], maturity, ATMVol, alpha, beta,
                           rho, SABR_BETAVOL, SABR_LOGVOL, &vols[i]);

  /* ----------------------------------------------------------------------------------------------------------*/
  /* Step1: We try to "bracket the minimum of the objective function. We find
     three points ax<bx<cx such that the value of the objective function is
     minimum at f(bx). These points will be the inputs of the next function
  */
  ...
  /*
          ...
          ax = bx;
          err=op_sabr_calib_adi( forward  ,forward  ,maturity  ,ATMVol
     ,SRT_LOGNORMAL  ,ax  ,beta  ,rho  ,nt  ,nx  ,nz  ,128  ,&dSigmaBeta); f1 =
     0.0; for (i=1;i<num_strikes+1 && !err;i++)
          {
                  dPrice = op_sabr_adi( forward  , strikes[i]  , maturity  , 1.0
     , 0  , dSigmaBeta  , ax  , beta  , rho  , nt  , nx  , nz ); err =
     srt_f_optimpvol( dPrice  , forward  , strikes[i]  , maturity  , 1.0  ,
     SRT_CALL  , SRT_LOGNORMAL  , &dTempVol ); f1 += ( dTempVol-vols[i] ) * (
     dTempVol-vols[i] );
          }
          f2 = f1 - 1.0;
          while ( f2 < f1 && !err)
          {
                  ...
                  else
                  {
                          ...
                          for (i=1;i<num_strikes+1 && !err;i++)
                          {
                                  dPrice = op_sabr_adi( forward  , strikes[i]  ,
     maturity  , 1.0  , 0  , dSigmaBeta  , ax  , beta  , rho  , nt  , nx  , nz
     ); err = srt_f_optimpvol( dPrice  , forward  , strikes[i]  , maturity , 1.0
     , SRT_CALL  , SRT_LOGNORMAL  , &dTempVol ); f2 += ( dTempVol-vols[i] ) * (
     dTempVol-vols[i] );
          ...
          while ( f2 < f1 && !err)
          {
                  ...
                  else
                  {
                          ...
                          {
                                  dPrice = op_sabr_adi( forward  , strikes[i]  ,
     maturity  , 1.0  , 0  , dSigmaBeta  , cx  , beta  , rho  , nt  , nx  , nz
     ); err = srt_f_optimpvol( dPrice  , forward  , strikes[i]  , maturity , 1.0
     , SRT_CALL  , SRT_LOGNORMAL  , &dTempVol ); f2 += ( dTempVol-vols[i] ) * (
     dTempVol-vols[i] );
  ...
  */
  ...
  err = srt_f_optsarbvol(forward, forward, maturity, ATMVol, alpha, beta, rho,
                         SABR_BETAVOL, SABR_LOGVOL, &atm_SABR_vol);

  err = op_sabr_adi(forward, forward, maturity, Disc, SRT_CALL, ATMVol, x1,
                    beta, rho, nt, nx, nz, &dPrice);
  err = srt_f_optimpvol(dPrice, forward, forward, maturity, Disc, SRT_CALL,
                        SABR_LOGVOL, &atm_ADI_vol);
  gap = atm_SABR_vol - atm_ADI_vol;

  f1 = 0.0;
  for (i = 1; i <= num_strikes && !err; i++) {
    err = op_sabr_adi(forward, strikes[i], maturity, Disc, SRT_CALL, ATMVol, x1,
                      beta, rho, nt, nx, nz, &dPrice);
    err = srt_f_optimpvol(dPrice, forward, strikes[i], maturity, Disc, SRT_CALL,
                          SRT_LOGNORMAL, &dTempVol);
    f1 += (dTempVol + gap - vols[i]) * (dTempVol + gap - vols[i]);
  }

  err = op_sabr_adi(forward, forward, maturity, Disc, SRT_CALL, ATMVol, x2,
                    beta, rho, nt, nx, nz, &dPrice);
  err = srt_f_optimpvol(dPrice, forward, forward, maturity, Disc, SRT_CALL,
                        SABR_LOGVOL, &atm_ADI_vol);
  gap = atm_SABR_vol - atm_ADI_vol;
  ...
  f2 = 0.0;
  for (i = 1; i <= num_strikes && !err; i++) {
    err = op_sabr_adi(forward, strikes[i], maturity, Disc, SRT_CALL, ATMVol, x2,
                      beta, rho, nt, nx, nz, &dPrice);
    err = srt_f_optimpvol(dPrice, forward, strikes[i], maturity, Disc, SRT_CALL,
                          SRT_LOGNORMAL, &dTempVol);
  ...
  while (fabs(x3 - x0) > tol && !err) {
    if (f2 < f1) {
      ...
      for (i = 1; i <= num_strikes && !err; i++) {
        err = op_sabr_adi(forward, strikes[i], maturity, Disc, SRT_CALL, ATMVol,
                          x2, beta, rho, nt, nx, nz, &dPrice);
        err = srt_f_optimpvol(dPrice, forward, strikes[i], maturity, Disc,
                              SRT_CALL, SRT_LOGNORMAL, &dTempVol);
        f2 += (dTempVol + gap - vols[i]) * (dTempVol + gap - vols[i]);
      }
    } else {

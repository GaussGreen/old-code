...
/* Initialise the limits for the following steps */
	...
	/* Set up the relevant limits for the r discretisation*/
		trinf->delta_state_var        = delta_state_var;
		trinf->max_state_var_index    = IMIN (step->index, DTOL (trinf->max_state_var / delta_state_var) + 1);
		trinf->max_state_var          = trinf->max_state_var_index * delta_state_var;
		maxtrinf->max_state_var_index = IMAX (maxtrinf->max_state_var_index, trinf->max_state_var_index);
		trinf->min_state_var_index    = IMAX (-step->index, DTOL (trinf->min_state_var / delta_state_var) - 1);
		trinf->min_state_var          = trinf->min_state_var_index * delta_state_var;
		maxtrinf->min_state_var_index = IMIN (maxtrinf->min_state_var_index, trinf->min_state_var_index);
...
Err	srt_f_CheyBetaTree_make_state_var_grid (
...
Err	srt_f_CheyBetaTree_make_phi_grid(
	SrtLocTreInf		*trinf, 
	double				*cur_phi_grid)
{
...
Err	srt_f_CheyBetaTree_recombine_in_r(
...
Err	srt_f_CheyBetaTree_calc_prob (
		SrtTrinTreNdInf	*node
		)
{
double	Su;
...
double  V;
	...
	Sd = node->son_state_var_value[TREE_DOWN],
	E  = node->state_var_expectation,
...
/* Find the closest Phi node to the value of Phi for the next step (below it) */
	find_closest_strictly_below (next_phi, 
				  next_phi_grid,
...
/* Compute the expected value for each phi, than interpolate linearily */	
	for(i=0; i<nassets; i++)
	{
		...
		for(j=0; j<3; j++)
		{
			...
			asset_here_and_now += node->p[j] 
									* interp_linear (next_phi, 

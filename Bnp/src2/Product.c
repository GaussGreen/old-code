...
static Err GRFN_Payoff(SProductDesc *product, int idx, double time, SMktData *mkt_data, double *pv)
{
	Err				err = NULL;
	...
	for (i=0; i < product->nccy; i++)
		memcpy(local->evt->df[i], mkt_data->dfs[i], product->nmat[i][idx] * sizeof(double));
		...
		double		**aux )
{
	...
	if (!comm) return serror("Memory failure");

	err = FIRSTInitMktStruct(nevd, evd, ntabrows, ntabcols, tabstrs, tabmask,
	...
	if (product->ex[0] < 1e-5) return serror("Some event dates are in the past");
	...
	if (!product->nmat || !product->mat || !product->mat_d) return serror("Memory failure");
	...
	for (i=0; i < product->nccy; i++)
	{
		...
		if (!product->nmat[i] || !product->mat[i] || !product->mat_d[i]) return serror("Memory failure");

		for (j=0; j < product->nex; j++)
		{
			product->nmat[i][j] = comm->evt[j].evt->dflen[i];
			product->mat[i][j] = (double *) calloc(product->nmat[i][j], sizeof(double));
			product->mat_d[i][j] = (long *) calloc(product->nmat[i][j], sizeof(long));
			if (!product->mat[i][j] || !product->mat_d[i][j]) return serror("Memory failure");
...
Err ProductDesc_FreeGRFN(SProductDesc *product)
{
	...
	if (product->nmat) for (i=0; i < product->nccy; i++) free(product->nmat[i]);
	free(product->nmat);

	if (product->mat) for (i=0; i < product->nccy; i++)
	{
		if (product->mat[i]) for (j=0; j < product->nex; j++) free(product->mat[i][j]);
		free(product->mat[i]);
	}
	free(product->mat);

	if (product->mat_d) for (i=0; i < product->nccy; i++)
	{
		if (product->mat_d[i]) for (j=0; j < product->nex; j++) free(product->mat_d[i][j]);
		free(product->mat_d[i]);
	}
	free(product->mat_d);

	if (comm) FIRSTFreeMktStruct(comm);
	free(comm);

	return NULL;
}

static Err Swaption_Payoff(SProductDesc *product, int idx, double time, SMktData *mkt_data, double *pv)
{
	int		k;

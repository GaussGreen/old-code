 ...
 *                                                                    *
 * FYI
 ** (1)accrual swap:you get the fixed coupon times the number of	  * days libor is within a
 *range									  * (2)range floater: you
 *get the floating coupon fixed at t1 times	  * the number of days libor is within a range
 ** (3)resettable: you get the above where you get to				  * fix the band at
 *t1.												  *
 *--------------------------------------------------------------------*
 * Modification Record                                                *
 * Date     Inits   Comments                                          *
...
static double get_vol(
    double   vol_date,
    double   strike,
    double*  mat_dates_vec,
    double*  mkt_vol_1darray,
    double*  strikes_vec,
    ...
    double*  answer)
{
    ...
    ***/
    ...
    while ((bus_day_array[i] <= today) && (bus_day_array[i + lag + end_lag] < end_date))
    {
        ...
        {
            if (rate_array[i] > kao)
                optval += 1.0 * NUMD(bus_day_array[i + 1 + lag], bus_day_array[i + lag]);
        }
        else
        {
            if (rate_array[i] < kao)
        ...
        (bus_day_array[i] > today))
    {
        ...
        {
            vol = get_vol(
                (double)bus_day_array[i - 1],
                strike + reset_flg * float_fwd,
                vol_date_array,
                vol_array,
                strikes_vec,
                mkt_vol_2darray,

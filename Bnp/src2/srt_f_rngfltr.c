 ...
 *                                                                    *
 * FYI																  *
 *	(1)accrual swap:you get the fixed coupon times the number of	  *
 *	days libor is within a range									  *
 *	(2)range floater: you get the floating coupon fixed at t1 times	  *
 *	the number of days libor is within a range						  *
 *	(3)resettable: you get the above where you get to				  *
 *	fix the band at t1.												  *
 *--------------------------------------------------------------------*
 * Modification Record                                                *
 * Date     Inits   Comments                                          *
...
static double quadratic_minmax(double x[3], double y[3])
{
  ...
  if(fabs(y10) + fabs(y12) < EPS)return x[1];
...
static double get_vol(double vol_date, double strike,
    double *mat_dates_vec, double *mkt_vol_1darray, 
    double *strikes_vec, 
    double **mkt_vol_2darray, long m_mat_dates, long m_strikes,
    int method, int smile_flg)
{
...
printf("getvol: vol_date %lf strike %lf\n",vol_date,strike);
  ...
  if(smile_flg)
  {
    vol=vol_interpol_function(vol_date,strike,
           mat_dates_vec,m_mat_dates, strikes_vec,m_strikes,mkt_vol_2darray,
           m_mat_dates, m_strikes);
  }
  else
  {
    vol=interp(mat_dates_vec,mkt_vol_1darray,m_mat_dates,vol_date,method,&q); 
    vol=q;
  }
#ifdef RFLTR_DBG_PRINT
...
/*
get_d2(fwdy,fwdx,k,fvol,vol,float_set,y_set,today,correlation,float_tenor,
float_flg,reset_flg);
*/
static double get_d2(double fwdy, double fwdx, double k, double volx, 
  double voly, Date x_set, Date y_set, Date today, double p,double float_tenor,
  int float_flg, int reset_flg)
{
  double d2;
  double adj = 0;
  double tx,ty;
  ...
  p = DMAX(1.0-(1.0-p)*DMAX(ty-tx,0.0)*
          12.0/float_tenor,p);

#ifdef RFLTR_DBG_PRINT
printf("getd2: fwdy %lf fwdx %lf k %lf volx %lf voly %lf\n",
  fwdy,fwdx,k,volx,voly);
#endif
  ...
  else /* contingent on Y(t2) > X(t1)+k */
  {
    ...
    {
      d2 = (log(fwdy/fwdx) - 0.5*voly*voly*ty + 0.5*volx*volx*tx)/var;
    }
  }
#ifdef RFLTR_DBG_PRINT
printf("getd2: d2 %lf\n",d2);
...
INPUT KEY:
  ...
  int smile_flg--1 if we use smile effect of vol, 0 if not.  If 1, this means
    we are going to look at mkt_vol_2darray; if 0 we are going to look at
    vol_array.
...
SrtErr srt_f_rngfltr(
  ...
  char *cap_floor_code,char *win_lose,
  int lag,int end_lag,
  double strike, double float_fwd,
  Date float_set,double correlation, double float_tenor,
  Date *bus_day_array,
  double *rate_array,int ra_len,
  double *vol_date_array,
  double *vol_array, 
  double *strikes_vec,
  double **mkt_vol_2darray, long m_mat_dates, long m_strikes,
  int float_flg,int smile_flg,int FHLB_flg,int reset_flg,
  double *answer)
{   
...
/* COMPUTE THE APPROPRIATE STRIKE ADJUSTMENTS */
  cf=1.0;
  if(cap_floor_code[0]=='C')cf=(-1.0);
  wl=1.0;
  if(win_lose[0]=='L')wl=(-1.0);
  kao=strike+cf*wl*(1.0e-9);

/* WE MUST ROUND KA TO THE NEAREST 32ND IN THE RIGHT DIRECTION */
  if(!reset_flg)ka = nearest_32nd(strike,cf,wl);
  else ka = strike;
...
/* BUS_DAY_ARRAY IS THE RATE SET DATE */
  while(bus_day_array[i+lag]<dcnt_start_date)i++;
  total_bus_days = i;
  while(bus_day_array[i+lag]<start_date)i++;
...
/* THE START AND END DATES ARE SPECIAL BECAUSE THEY MAY BE ON WEEKENDS
  ...
  if((bus_day_array[i]<=today)&&(bus_day_array[i+lag]>start_date))
  {
    ...
    {
      if(rate_array[i-1]>kao)optval+=1.0*NUMD(bus_day_array[i+lag],start_date);
    }
    else
    {
      if(rate_array[i-1]<kao)optval+=1.0*NUMD(bus_day_array[i+lag],start_date);
  ...
  while((bus_day_array[i]<=today)&&(bus_day_array[i+lag+end_lag]<end_date))
  {  
    if(cap_floor_code[0]=='C')
    {
      if(rate_array[i]>kao)optval+=
...
/*  WE NEED >= END_DATE TO ACCOUNT FOR FHLB WEEKEND RULES */
  if((bus_day_array[i]<=today)&&(bus_day_array[i+lag+end_lag]>=end_date))
  {  
    if(cap_floor_code[0]=='C')
    {
      if(rate_array[i]>kao)optval+=1.0*NUMD(end_date,bus_day_array[i+lag]);
    }
    else
    {
      if(rate_array[i]<kao)optval+=1.0*NUMD(end_date,bus_day_array[i+lag]);
...
/*    i++;*/
  ...
  if(float_set>today)
  {
    ...
    vol_date_array, vol_array, strikes_vec,
    mkt_vol_2darray, m_mat_dates,
    m_strikes,method,smile_flg);
  }
/* NOW DEAL WITH DAYS TO VALUE OPTION BEFORE LAST DAY*/
/* THE START AND END DATES ARE SPECIAL BECAUSE THEY MAY BE ON WEEKENDS
   THANKS TO THE MEAN OLD FHLB */
  if((bus_day_array[i+lag]>start_date)&&(bus_day_array[i+lag-1]<start_date)&&
     (bus_day_array[i]>today))   
  {
/* THIS IS THE BUG FIX */
    ...
    {
      vol = get_vol((double)bus_day_array[i-1], strike + reset_flg * float_fwd,
        vol_date_array, vol_array, strikes_vec,
        mkt_vol_2darray, m_mat_dates,
        m_strikes,method,smile_flg);
      d2 = get_d2(rate_array[i-1],float_fwd,ka,fvol,vol,float_set,
        bus_day_array[i-1],today,correlation,float_tenor,float_flg,reset_flg);
...
/* END BUG FIX */
  ...
  {  
    ...
    d2 = get_d2(rate_array[i],float_fwd,ka,fvol,vol,float_set,
        bus_day_array[i],today,correlation,float_tenor,float_flg,reset_flg);

    if(cap_floor_code[0]=='C')
    {
      optval+=norm(d2)*NUMD(bus_day_array[i+1+lag],bus_day_array[i+lag]);
    }
    else
    {
      optval+=(1.0-norm(d2))*NUMD(bus_day_array[i+1+lag],bus_day_array[i+lag]);
    }
    i++;
  }                                
...
/*  WE NEED >= END_DATE TO ACCOUNT FOR FHLB WEEKEND RULES */
  ...
  if(FHLB_flg && (bus_day_array[i]>today)&&(bus_day_array[i+lag+end_lag]>=end_date))
  {  
    vol = get_vol((double)bus_day_array[i], strike+ reset_flg * float_fwd,
      vol_date_array, vol_array, strikes_vec,
      mkt_vol_2darray, m_mat_dates,
...
SrtErr srt_f_rngfltr_resettable(
  Date today,Date dcnt_start_date,Date start_date,Date end_date,
  char *win_lose_cap,char *win_lose_floor,
  int lag,int end_lag,
  ...
  double *strikes_vec,
  double **mkt_vol_2darray, long m_mat_dates, long m_strikes,
  int float_flg,int smile_flg,int FHLB_flg,
  double *answer)
{
  double val,val2;
  SrtErr err;
...
to be the bottom of the band */
  if(today > float_set)
  {
    ...
    for(j=0;j<float_flg+1;j++)
    {
     ...
     err=srt_f_rngfltr(
      today,dcnt_start_date,start_date,end_date,
      "CAP",win_lose_cap,
      lag,end_lag,
      strike+band_width, float_fwd,
      float_set,correlation,float_tenor,
      bus_day_array,
      rate_array,ra_len,
      vol_date_array,
      vol_array,
      strikes_vec,
      mkt_vol_2darray, m_mat_dates,m_strikes,
      j,smile_flg,FHLB_flg,0,
      &val);
     if(err)return err;

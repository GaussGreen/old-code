...
int	iIndex = 0;
	...
	/* Find the pair */
	iIndex = 0;
	while ((iIndex<ptrCorrMat->lNbPair) &&((strcmp(cName1,ptrCorrMat->cName1[iIndex]) !=0 ) || (strcmp(cName2,ptrCorrMat->cName2[iIndex]) !=0 ))
											&& ((strcmp(cName2,ptrCorrMat->cName1[iIndex]) !=0 ) || (strcmp(cName1,ptrCorrMat->cName2[iIndex]) !=0 )))
		iIndex++;
...
/* ---------------------------------------------------------------------------
  ...
  ---------------------------------------------------------------------------- */
char * SrtInitSABRUnd(	char		*cUndName, 
...
double	dValue;
	...
	for (iNum=0; iNum<iNbUnderlying; iNum++)
		ProductCallsParam->local->smp.und[ProductCallsParam->iIndex[iNum]].sv[SPOT] = UnderlyingValue[iNum];
	...
	for (iNumEvt = 0; iNumEvt<num_evt; iNumEvt ++)
	{
		if (xStr.evt[iNumEvt].evt)
		{
			...
			for (iNumUnd = 0; iNumUnd<xStr.und_info->no_of_underlyings; iNumUnd++)
			{
				...
				{
					GrfEvt->df[iNumUnd][iNum] = swp_f_df(GrfEvt->d, GrfEvt->dfd[iNumUnd][iNum], cDomYcName);
				}
			}
		}
	}
...
FREE_RETURN:
	...
	if (GenericModel) free(GenericModel);
	if (SpecificModel) free(SpecificModel);
	
	if (dForward) free(dForward);
	if (dVol) free(dVol);
	if (dAlpha) free(dAlpha);
	if (dBeta) free(dBeta);
	if (dRho) free(dRho);
	if (dCorrelation) free_dmatrix(dCorrelation, 0, iNbUnderlying-1, 0, iNbUnderlying-1);
	if (tabUndName) 

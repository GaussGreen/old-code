				  ...
				  double	**prod_val)
{
	...
	if (xStr.num_und > 3)
	{
		err = "Fx3DGrn can only accept 3 underlyings, please check the GRFN tableau or use the MultiGRFN functions";
		goto FREE_RETURN;
	}
	...
	if (!vol_change || !sig_dom || !sig_for || !sig_fx || !corr_dom_for || !corr_dom_fx || !corr_for_fx)
	{
				  ...
				  double	***prod_val)
{
	...
	err = Get_FX_StochRate_TermStructures_corr(und3dfx,
											 ...
											 &corr_date, &correl_dom_for,  &correl_dom_fx,  &correl_for_fx, &corr_n_date);
	if (err)
	{
		goto FREE_RETURN;
	}
	...
	if (!dom_ifr || !dom_fwd || !dom_std || !dom_phi || !dom_beta || !dom_bond_pay || !dom_beta_pay 
		|| !for_ifr || !for_fwd || !for_std || !for_phi || !for_beta
		|| !fx_fwd || !fx_std)
	{
...
FREE_RETURN:
	...
	if (opt_bar) free_dmatrix(opt_bar, 0, nstp-1, 0, 0);

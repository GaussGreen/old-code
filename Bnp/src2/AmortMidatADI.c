...
typedef struct _COMM_PFN_BI_
{
    const PFN_COEFF_3DPDE
                 m_FUNC_;  // tertiary function PFN_COEFF_3DPDE(double x,double aux,double t,void*)
    ...
    if (pdHalf_InvDeltaX_Mu_End - pdHalf_InvDeltaX_Mu_Begin >= 3)
    {
        ...
        *pdU_Begin = dOffDiag_Imp_front;  // NB : boundary
        ...
        // boundary
        {
            ...
            *pdL_Begin =
                (*(pdHalf_InvDeltaX_Mu_Begin + 1) - dHalf_InvDeltaXSqrd_SigSqrd) / dDiag_Imp_front;
...
//						at the boundary, the second derivative w.r.t to State X is
//0
//
//	Returns : ppd_To
//-------------------------------------------------------------------------------------------------------

...
LGMErr GetVol_SABRValues(long tNow, long value_date, long start, long end,
                         SrtCallTimeSwapPtr deal, double *alpha, double *beta,
                         double *sigma, double *rho);

LGMErr GetVol_FudgeTimeSwap_new(long tNow, long value_date, long start,
                                long end, double strike, double fra,
                                SrtCallTimeSwapPtr deal, double *result,
...
void NewConvolverTimeSwap(long nEx, /* number of exercises */
  ...
  /* Backward induction */
  for (j = (nEx - 1); j >= 0;
       j--) /* For each exercise date (starting from the last) */
  {
    if (j < nEx - 1 &&
        reduction[j] < 0.90 * reduction[j + 1]) /* reduce grid size if needed */
    {
      ratio = reduction[j] / reduction[j + 1];
      for (knew = 0; knew <= nx; knew++) /* transfer results */
        Qplus[knew] = Qarr[knew];
      for (knew = 0; knew <= nx;
           knew++) /* interpolate to get Qarr on new grid */
      {
    ...
    {
      alpha = h * varr[j] /
              (dx * reduction[j]); /* Qplus[k] = sum of weight[i]*max{Qarr[k*]
                                            , payoff[k*]} */
      for (i = 0; i <= nz; i++)    /*  at k* = k + (i-m)*alpha */
      {
        ...
        for (k = 0; k <= nx; k++) {
          knew = k + integershift; /* interpolate to get value at
                                      k+integershift+gamma */
    ...
    for (kk = 1; kk <= nx - 2; kk++) {
      ...
      a3 = 0.25 *
           (differ[kk + 2] + differ[kk - 1] - differ[kk] - differ[kk + 1]);
      a2 = a2 - a3;
      ...
      xkink = (exactkink - (double)n0) * dx *
              reduction[j]; /* store kink for output */
      ifoundkink = 1;
...
LGMErr ComputeBermudanAndEuropeanTimeSwap(
    ...
    double *zeta, /* [0        , 1        , ...        , nEx-1]        , values
               of zeta at the exercise dates */
...
startagain:
  ...
  /* unload the convolution parameters and set them to their default values if
   * needed */
  ...
  /* x[k] = (k-n0)*dx*reduction[j]        , k=0        ,1        ,...        ,
   * nx        , j=0        ,1        ,2        ,... ,nEx-1 */
  dx = 2.0 * totalstddev * gridwidth / ((double)nx);

  /* z[i] = (i-m)*h        , i = 0        , 1        , ...        , 2m=nz */
  /* integration intervals from z[i] to z[i+1]        , for i = 0        , 1 ,
   * ...        , 2m-1 = nz-1 */
  h = 0.9233333 * dx /
      vmax[nEx - 1]; /* 0.9233 to prevent staircase from roundoff */
  h = min(h, maxh);
  ...
  */
  ...
  if (x == NULL || payofftable == NULL || weights == NULL ||
      x_exerarr == NULL || payofftable_payers == NULL ||
      payofftable_receivers == NULL || receiver_european == NULL ||
      payer_european == NULL) {
    srt_free(varr);
    srt_free(vmax);
    srt_free(reduction);
  ...
  error = (*payofffunc)(payofftable, nx, x, reduction, dealtype, dealPtr,
                        CalReq, EvalDate, ycName, tsPtr, GetVol, GetBeta);
  /* gets the payoff of all european options */
  for (index_european = 0; index_european < nEx; index_european++) {
    for (k = 0; k <= nx; k++) {
      payofftable_receivers[index_european][0][k] =
          payofftable[index_european][k];
      payofftable_payers[index_european][0][k] =
          -payofftable[index_european][k];
    }
  }
  ...
  for (index_european = 0; index_european < nEx; index_european++) {
    ...
    NewConvolverTimeSwap(1, nx, n0, nz, dx, h, m, killkinks,
                         &(reduction[index_european]), weights, &stdev_european,
                         payofftable_receivers[index_european], &price);
  ...
  for (index_european = 0; index_european < nEx; index_european++) {
    ...
    NewConvolverTimeSwap(1, nx, n0, nz, dx, h, m, killkinks,
                         &(reduction[index_european]), weights, &stdev_european,
                         payofftable_payers[index_european], &price);
    payer_european[index_european] = price;
  }

  /* Call put parity for the forward */
  for (index_european = 0; index_european < nEx; index_european++)
    (ptr->tForwardTS)[1][index_european] =
        receiver_european[index_european] - payer_european[index_european];
  ...
  if (ptr->PayRec == SRT_RECEIVER) {
    for (j = 0; j < nEx; j++)
      for (k = 0; k <= nx; k++)
        payofftable[j][k] +=
            ((ptr->tForwardTS)[0][j] - (ptr->tForwardTS)[1][j]);
  } else {
    for (j = 0; j < nEx; j++)
      for (k = 0; k <= nx; k++)
        payofftable[j][k] -=
...
static double InterpfromArray(long k, double dk, long nk, double *TheArray) {
  ...
  if (k > 0 && k < (nk - 1))
    interpval =
        TheArray[k] + dk * (TheArray[k + 1] - TheArray[k]) -
        0.25 * dk * (1.0 - dk) *
    ...
    x = dk + (double)k;
    interpval =
        TheArray[0] + x * (TheArray[1] - TheArray[0]) -
    ...
    interpval = TheArray[nk - 1] + x * (TheArray[nk] - TheArray[nk - 1]) -
                0.5 * x * (1.0 - x) *
                    (TheArray[nk] - 2.0 * TheArray[nk - 1] + TheArray[nk - 2]);
  }
...
/* Interpolator for convolver
Function: Interpolates value at k+dk from an array of values
TheArray[k]        , k=0        , ...        , nk. For time swap        ,
extrapolates flat; this solves a problem for very small maturities  */
static double InterpfromArrayTS(long k, double dk, long nk, double *TheArray) {
  double interpval, x;
...
static double GetNormalVol(double mat, Date start, Date end, double fwd,
                           double strike, SRT_Boolean isCap,
                           LGMErr (*GetVol)(Date, Date, double, SRT_Boolean,
                                            double *),
  ...
  bs_price =
      srt_f_optblksch(fwd, strike, inp_vol, mat, 1.0, SRT_CALL, SRT_PREMIUM);

  error2 = srt_f_optimpvol(bs_price, fwd, strike, mat, 1.0, SRT_CALL,
                           SRT_NORMAL, &norm_vol);
...
/* ----------------------------------------------------------------------------------------------------------------------------------
 */
/* Computes the forward LOGNORMAL vols of LIBOR using a fudge for the autocal
 * timeswap */
    ...
    double *result)          /* output:  volatility */
{
  ...
  if (deal->timeswapvolmethod ==
      2) /* Converging:  tkaes vol from today's market assuming that the market
            in the future is shifted. */
  {
    startfudge = start - (value_date - tNow);
    endfudge = end - (value_date - tNow);
  } else if (deal->timeswapvolmethod ==
             3) /* Sliding: takes vol from today's market independent of where
  ...
  /*	if ( deal->dMaxStd > 0.0 )
          {
                  error = (*(deal->GetSABRParam))(startfudge        ,endfudge
           ,deal->vol_id        ,SABR_ATM_LOG        ,&sigma); if (error) return
     error; dStdDev = fra * exp( sigma*sqrt(maturity) ); if ( strike > fra *
     dStdDev ) strike = fra* dStdDev; else if ( strike < fra / dStdDev ) strike
     = fra / dStdDev;
          }
  */
  ...
  error = (*(deal->GetSABRParam))(startfudge, endfudge, deal->vol_id, atmtype,
                                  &sigma);
  if (error)
    return error;
  error = (*(deal->GetSABRParam))(startfudge, endfudge, deal->vol_id, ALPHA,
                                  &alpha);
  if (error)
    return error;
  error =
      (*(deal->GetSABRParam))(startfudge, endfudge, deal->vol_id, BETA, &beta);
  if (error)
    return error;
  error =
      (*(deal->GetSABRParam))(startfudge, endfudge, deal->vol_id, RHO, &rho);
  if (error)
    return error;
  ...
  if (fra <= 0.0 ||
      strike <= 0.0) /* Adds this condition to avoid the crash of MAD */
    volstrike = 0.0;
  else {
    if (deal->atmvolmethod == 1)
      error = srt_f_optsarbvol(fra, strike, maturity, sigma, alpha, beta, rho,
                               SRT_LOGNORMAL, SRT_LOGNORMAL, &volstrike);
    else if (deal->atmvolmethod == 2)
      error = srt_f_optsarbvol(fra, strike, maturity, sigma, alpha, beta, rho,
                               SRT_NORMAL, SRT_LOGNORMAL, &volstrike);
    else if (deal->atmvolmethod == 3)
      error = srt_f_optsarbvol(fra, strike, maturity, sigma, alpha, beta, rho,
                               SRT_BETAVOL, SRT_LOGNORMAL, &volstrike);
    else
  ...
  /* fudge to match the forward value of the underlying by using an input
   * volatility shift. */
  if (deal->shiftvol)
    volstrike += deal->shiftvol *
                 (1 - exp(-(value_date - tNow + 0.0) /
                          (deal->tCpnPay[0] - deal->tCpnStart[0] + 0.0)));
...
LGMErr GetVol_SABRValues(long tNow, long value_date, long start, long end,
  ...
  if (deal->timeswapvolmethod == 2) {
    ...
    ;
    endfudge = end - (value_date - tNow);
  } else if (deal->timeswapvolmethod == 3) {
    startfudge = start;
    endfudge = end;
  } else {
    return ("Unknown value of timeswapvolmethod parameter");
  ...
  if (error)
    return error;
  error =
      (*(deal->GetSABRParam))(startfudge, endfudge, deal->vol_id, ALPHA, alpha);
  if (error)
    return error;
  error =
      (*(deal->GetSABRParam))(startfudge, endfudge, deal->vol_id, BETA, beta);
    ...
    LGMErr (*GetBeta)(Date, Date, double *)) /* swaption/cap exponents (beta) */
{
  ...
  double vol_accrued_low,
      vol_accrued_up; /* vols for the floating digital adjustment */
  ...
  double proba_between, proba_between_adj, proba_below, proba_below_adj,
      proba_above, proba_above_adj;
  ...
  /* for debug
  clock_t counter1        ,counter2        ,counter3        ,counter4
  ,counter_observationdf1 ,counter_observationdf2;
  */
  double alpha, beta, sigma, rho, dfstart_init, dfend_init, dfstart, dfend,
      mult;
  double df_start_accrued_init, df_end_accrued_init, level_init, cvg,
      dfPay_init;
  double G1, G2;
  ...
  /* Memory allocation */
  zEx = (double *)srt_calloc(
      nEx,
      sizeof(double)); /* zeta[j0+j]        , j=0        ,...        ,nEx-1 */
  ...
  tvols_discret =
      f3tensor(0, num_subdiscret_obsfreq - 1, 0, num_subdiscret_statvar - 1, 0,
               3); /* low        ,up        ,lowshift        ,upshift */
  ...
  tvols_subperiod = f3tensor(0, observation_freq - 1, 0, nx, 0,
                             3); /*low        ,up        ,lowshift ,upshift */
  tindex_firstpositive = lngvector(0, observation_freq - 1);
  index_discret_statvar =
      lngmatrix(0, observation_freq - 1, 0, num_subdiscret_statvar - 1);
  index_discret_obsfreq = lngvector(0, num_subdiscret_obsfreq - 1);

  /* Computes indexes of discretisation for the observation days */
  for (current_index_obsfreq = 0;
       current_index_obsfreq < num_subdiscret_obsfreq;
       current_index_obsfreq++) {
    index_discret_obsfreq[current_index_obsfreq] =
        (int)(current_index_obsfreq * (observation_freq - 1.0) /
              (num_subdiscret_obsfreq - 1.0));
  }
  ...
  for (i = 0; i < ptr->nCpn; i++) {
    (observationdf)[i] =
        (double **)srt_calloc(observation_freq, sizeof(double *));
  ...
  {
    zeta = LGMZetaFromTS(ptr->tSet[j + j0], tNow,
                         tsPtr); /* Get zeta at exercise date */
    ...
    for (i = ifirst; i < nCpn; i++) /*Loop on coupons */
    {
      ...
      for (l = 0; l <= observation_freq - 1; l++) {
        ...
        tmaturities_subperiod[l] =
            (double)(tExp - ptr->tEx[j + j0] + 0.0) / 365.0;
        tindex_firstpositive[l] = -1;
        dfstart_init =
            observationdf[i + i0][l]
                         [0]; // swp_f_df(tNow        ,Start        ,ycName);
      ...
      for (k = 0; k <= nx; k++) {
        ...
        error = GetVol_FudgeTimeSwap_new(
            tNow, ptr->tEx[j + j0],
            ptr->observationdays[i + i0][index_for_adj_cms][0],
            ptr->observationdays[i + i0][index_for_adj_cms][1], fra_for_adj_cms,
            fra_for_adj_cms, ptr, &vol_for_adj_cms, alpha, beta, sigma, rho);
        if (error)
          return error;
          ...
          if (tfras_subperiod[l][k] > 0) {
            tcms_subperiod[l][k] =
                ...
                exp(vol_for_adj_cms * vol_for_adj_cms * fra_for_adj_cms *
                    tmaturities_subperiod[l] *
                    (ptr->observationdays[i + i0][l][1] - ptr->tCpnPay[i + i0] +
                     0.0) /
                    365.0);
          }
        }
      ...
      for (l = 0; l < observation_freq; l++) {
        ...
        if (0.5 * (strikeup + tfras_discr[10]) <
            1.5 * strikeup - 0.5 * strikelow) {
      ...
      for (current_index_obsfreq = 0;
           current_index_obsfreq < num_subdiscret_obsfreq;
           current_index_obsfreq++) {
        for (current_index_statvar = 0;
             current_index_statvar < num_subdiscret_statvar;
             current_index_statvar++) {
          ...
          error = GetVol_SABRValues(tNow, ptr->tEx[j + j0], Start, End, ptr,
          ...
          error = GetVol_FudgeTimeSwap_new(
              tNow, ptr->tEx[j + j0], Start, End, strikeup,
              tfras_subperiod[l][k], ptr,
              &(tvols_discret[current_index_obsfreq][current_index_statvar][1]),
              alpha, beta, sigma, rho);
          if (error)
            return error;

          error = GetVol_FudgeTimeSwap_new(
              tNow, ptr->tEx[j + j0], Start, End, strikeupshift,
              tfras_subperiod[l][k], ptr,
              &(tvols_discret[current_index_obsfreq][current_index_statvar][3]),
              alpha, beta, sigma, rho);
          if (error)
            return error;
          ...
          if (0 < strikelow) { /* gets vollow and vollowshift */
            ...
            error = GetVol_FudgeTimeSwap_new(
                tNow, ptr->tEx[j + j0], Start, End, strikelow,
                tfras_subperiod[l][k], ptr,
                &(tvols_discret[current_index_obsfreq][current_index_statvar]
                               [0]),
                alpha, beta, sigma, rho);
            if (error)
              return error;
            ...
            error = GetVol_FudgeTimeSwap_new(
                tNow, ptr->tEx[j + j0], Start, End, strikelowshift,
                tfras_subperiod[l][k], ptr,
                &(tvols_discret[current_index_obsfreq][current_index_statvar]
                               [2]),
                alpha, beta, sigma, rho);
      ...
      for (l = 0; l <= observation_freq - 1;
           l++) /* Loop on the observation days */
      {
        if (l > index_discret_obsfreq[current_index_obsfreq + 1]) {
          current_index_obsfreq += 1;
        }
        if (tmaturities_subperiod[index_discret_obsfreq[current_index_obsfreq +
                                                        1]] ==
            tmaturities_subperiod
                [index_discret_obsfreq[current_index_obsfreq]]) {
          xx = 0.5;
        } else {
          xx = (tmaturities_subperiod[l] -
                tmaturities_subperiod
                    [index_discret_obsfreq[current_index_obsfreq]]) /
               (tmaturities_subperiod
                    [index_discret_obsfreq[current_index_obsfreq + 1]] -
                tmaturities_subperiod
                    [index_discret_obsfreq[current_index_obsfreq]]);
        }
        ...
        for (k = tindex_firstpositive[l]; k <= nx;
             k++) /* Loop on the state variable */
        {
          if (tfras_subperiod[l][k] >
              tfras_subperiod
                  [l][index_discret_statvar[l][current_index_statvar + 1]]) {
            current_index_statvar += 1;
          }

          yy = (tfras_subperiod[l][k] -
                tfras_subperiod
                    [l][index_discret_statvar[l][current_index_statvar]]) /
               (tfras_subperiod
                    [l][index_discret_statvar[l][current_index_statvar + 1]] -
                tfras_subperiod
                    [l][index_discret_statvar[l][current_index_statvar]]);
          ...
          w4 = xx * yy;
          for (m = 0; m <= 3; m++) {
            tvols_subperiod[l][k][m] =
                w1 * tvols_discret[current_index_obsfreq][current_index_statvar]
                                  [m] +
                w2 * tvols_discret[current_index_obsfreq + 1]
                                  [current_index_statvar][m] +
                w3 * tvols_discret[current_index_obsfreq]
                                  [current_index_statvar + 1][m] +
                w4 * tvols_discret[current_index_obsfreq + 1]
                                  [current_index_statvar + 1][m];
      ...
      if (!(0 == ptr->gear[i + i0])) /* Floating digital computation */
      {
        ...
        end_accrued = add_unit(start_accrued, ptr->undAccrued, SRT_MONTH,
                               MODIFIED_SUCCEEDING);
        ...
        error =
            GetVol_SABRValues(tNow, ptr->tEx[j + j0], start_accrued,
                              end_accrued, ptr, &alpha, &beta, &sigma, &rho);
        if (error)
          return error;
      }

      /* computes probabilities to be between barriers        , and
      ...
      for (k = 0; k <= nx; k++) /* Loop on the state variable */
      {
        if (!(0 == ptr->gear[i + i0])) /* Floating digital computation */
        {
          ...
          error = GetVol_FudgeTimeSwap_new(
              tNow, ptr->tEx[j + j0], start_accrued, end_accrued, strikeup,
              fra_accrued, ptr, &vol_accrued_up, alpha, beta, sigma, rho);
          if (error)
            return error;

          error = GetVol_FudgeTimeSwap_new(
              tNow, ptr->tEx[j + j0], start_accrued, end_accrued, strikelow,
              fra_accrued, ptr, &vol_accrued_low, alpha, beta, sigma, rho);
          if (error)
            return error;
        }
        ...
        for (l = 0; l < observation_freq; l++) /* Loop on the number of days */
        {
          if (k < tindex_firstpositive[l]) /* case of negative fra */
          {
            ...
            } else /* Floating digital */
            {
              ...
              correl = (tmaturities_subperiod[l] - tmaturities_subperiod[0]) /
                           (tmaturities_subperiod[observation_freq - 1] -
                       ...
                       (tmaturities_subperiod[observation_freq - 1] -
                        tmaturities_subperiod[l]) /
                           ...
                           ptr->CorrelStart;
              adjustmentup =
                  exp(correl * vol_accrued_up * tvols_subperiod[l][k][1] *
                      tmaturities_subperiod[0]); /* Changed the adjustment */
              callup_adj = srt_f_optblksch(tcms_subperiod[l][k] * adjustmentup,
                                           strikeup, tvols_subperiod[l][k][1],
                                           tmaturities_subperiod[l], 1,
                                           SRT_CALL, PREMIUM);
              callupshift_adj = srt_f_optblksch(
                  tcms_subperiod[l][k] * adjustmentup, strikeupshift,
                  tvols_subperiod[l][k][3], tmaturities_subperiod[l], 1,
                  SRT_CALL, PREMIUM);
              proba_above_adj =
                  (callup_adj - callupshift_adj) / ptr->call_spread_up;
              if (ptr->barriers[i + i0][0] == 0) {
                proba_below_adj = 0;
              } else {
                adjustmentlow =
                    exp(correl * vol_accrued_low * tvols_subperiod[l][k][0] *
                        tmaturities_subperiod[0]); /* Changed the adjustment */
                putlow_adj = srt_f_optblksch(
                    tcms_subperiod[l][k] * adjustmentlow, strikelow,
                    tvols_subperiod[l][k][0], tmaturities_subperiod[l], 1,
                    SRT_PUT, PREMIUM);
                putlowshift_adj = srt_f_optblksch(
                    tcms_subperiod[l][k] * adjustmentlow, strikelowshift,
                    tvols_subperiod[l][k][2], tmaturities_subperiod[l], 1,
                    SRT_PUT, PREMIUM);
                proba_below_adj =
                    (putlowshift_adj - putlow_adj) / ptr->call_spread_low;
              }
            }
            /* UPPER BARRIER */
            ...
            callup = srt_f_optblksch(
                tcms_subperiod[l][k], strikeup, tvols_subperiod[l][k][1],
                tmaturities_subperiod[l], 1, SRT_CALL, PREMIUM);
            callupshift = srt_f_optblksch(
                tcms_subperiod[l][k], strikeupshift, tvols_subperiod[l][k][3],
                tmaturities_subperiod[l], 1, SRT_CALL, PREMIUM);
            ...
            if (0 == strikelow) {
              ...
              /* put spread */
              putlow = srt_f_optblksch(
                  tcms_subperiod[l][k], strikelow, tvols_subperiod[l][k][0],
                  tmaturities_subperiod[l], 1, SRT_PUT, PREMIUM);
              putlowshift = srt_f_optblksch(
                  tcms_subperiod[l][k], strikelowshift,
                  tvols_subperiod[l][k][2], tmaturities_subperiod[l], 1,
                  SRT_PUT, PREMIUM);
          ...
          expected_accrued_ratio +=
              proba_between * (ptr->ratiodays_for_subperiod)[i + i0][l];
          proba_between_adj = 1 - proba_below_adj - proba_above_adj;
          expected_accrued_ratio_adj +=
              proba_between_adj * (ptr->ratiodays_for_subperiod)[i + i0][l];
          expected_accrued_ratio_low +=
              proba_below * (ptr->ratiodays_for_subperiod)[i + i0][l];
          //					expected_accrued_ratio_high += (1.0
          //- proba_above) * (ptr->ratiodays_for_subperiod)[i+i0][l];
  ...
  /* Fills the payoff */
  for (j = 0; j < nEx; j++) {
    zeta = LGMZetaFromTS(ptr->tEx[j + j0], tNow,
                         tsPtr); /* Get zeta at exercise date */
...
cleanup:
  ...
  free_f3tensor(tvols_discret, 0, num_subdiscret_obsfreq - 1, 0,
                num_subdiscret_statvar - 1, 0, 3);
  ...
  free_lngmatrix(index_discret_statvar, 0, observation_freq - 1, 0,
                 num_subdiscret_statvar - 1);

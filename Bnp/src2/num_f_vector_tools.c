...
#define MY_DBL_EPSILON     (10.0 * DBL_EPSILON) /* a little above the smallest such that 1.0+DBL_EPSILON != 1.0 */
...
int							n0)
{
...

    {
        ...
        if (number_of_extra_points > 0)
        {
			...
			{
                v2[free_idx] = (*v)[i] +  k * ((*v)[i+1] - (*v)[i]) / (1 + number_of_extra_points);
				free_idx++;
			}
        }
    }
...
double						**matrix)
{
	...
	for (;;) 
	{
		...
		else 
		{
			...
			for (;;) 
			{
				do i++; while (v[i] < a[row]);
				do j--; while (v[j] > a[row]);

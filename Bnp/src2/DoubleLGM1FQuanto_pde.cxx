/* ==========================================================================
   FILE_NAME:	DoubleLGM1FQuanto_pde.cxx

   PURPOSE:		PDE implementation of the double 1 Factor LGM quanto
   model. Discretisation is LOD.

   DATE:		02/09/01

   AUTHOR:		B.B. (from L.C.'s LGM2Fpde.cxx)
   ========================================================================== */
...
static double Ji1_func(double lam, double t1, double t2) {
  return ((t2 * exp(lam * t2) - t1 * exp(lam * t1)) - Ji0_func(lam, t1, t2)) /
         lam;
}

static double Ji2_func(double lam, double t1, double t2) {
  return ((t2 * t2 * exp(lam * t2) - t1 * t1 * exp(lam * t1)) -
          2 * Ji1_func(lam, t1, t2)) /
         lam;
}

static double Ji3_func(double lam, double t1, double t2) {
  return ((t2 * t2 * t2 * exp(lam * t2) - t1 * t1 * t1 * exp(lam * t1)) -
          3 * Ji2_func(lam, t1, t2)) /
         lam;
}

static double Ji4_func(double lam, double t1, double t2) {
  return ((t2 * t2 * t2 * t2 * exp(lam * t2) -
           t1 * t1 * t1 * t1 * exp(lam * t1)) -
          4 * Ji3_func(lam, t1, t2)) /
         lam;
}

Err convertVol(double lam, int nstept, double *time, int nb_sig,
               double *sig_time, double *sig, double *sigtilda, double *contsig,
               double **ablinsig) {
  int i, j;
  double t1, t2;
  char *err = NULL;
  ...
  for (i = 0; i < nb_sig - 1; ++i) {
    ...
    B = 2 * Ji1_func(2 * lam, sig_time[i], sig_time[i + 1]) /
        (sig_time[i + 1] - sig_time[i]);
    C = Ji2_func(2 * lam, sig_time[i], sig_time[i + 1]) /
        ((sig_time[i + 1] - sig_time[i]) * (sig_time[i + 1] - sig_time[i]));
    D = A * (1 - e) * (1 - e) + B * (1 - e) + C;
    E = 2 * A * e * (1 - e) * sigtilda[i] - B * (1 - 2 * e) * sigtilda[i] -
        2 * C * sigtilda[i];
    F = (A * e * e - B * e + C) * sigtilda[i] * sigtilda[i] -
        sig[i] * sig[i] * A;
    Delta = E * E - 4 * D * F;
    if (Delta < 0) {
  ...
  for (i = 1; i < nb_sig; ++i) {
    ablinsigloc[0][i] = sigtilda[i - 1] - (sigtilda[i] - sigtilda[i - 1]) *
                                              sig_time[i - 1] /
                                              (sig_time[i] - sig_time[i - 1]);
    ablinsigloc[1][i] =
        (sigtilda[i] - sigtilda[i - 1]) / (sig_time[i] - sig_time[i - 1]);
  }
...
FREE_RETURN:
  ...
  if (ablinsigloc)
    free_dmatrix(ablinsigloc, 0, 1, 0, nb_sig);

  return err;
}

Err convertVol2(double lam, int nstept, double *time, int nb_sig,
                double *sig_time, double *sig, double *sigtilda,
                double *contsig, double **ablinsig) {
  int i, j;
  double t1, t2;
  char *err = NULL;
  double **ablinsigloc = NULL;
  ...
  for (i = 0; i < nb_sig - 1; ++i) {
    if ((sig_time[i + 1] - sig_time[i]) > 2.0 / 365.0) {
      ...
      A = Ji0_func(2 * lam, sig_time[i], sig_time[i + 1]) * sig_time[i] *
              sig_time[i] -
          2 * Ji1_func(2 * lam, sig_time[i], sig_time[i + 1]) * sig_time[i] +
          Ji2_func(2 * lam, sig_time[i], sig_time[i + 1]);

      B = -2 * sigtilda[i] *
          (Ji0_func(2 * lam, sig_time[i], sig_time[i + 1]) * sig_time[i + 1] *
               sig_time[i] -
           Ji1_func(2 * lam, sig_time[i], sig_time[i + 1]) *
               (sig_time[i + 1] + sig_time[i]) +
           Ji2_func(2 * lam, sig_time[i], sig_time[i + 1]));

      C = sigtilda[i] * sigtilda[i] *
              (Ji0_func(2 * lam, sig_time[i], sig_time[i + 1]) *
                   sig_time[i + 1] * sig_time[i + 1] -
               2 * Ji1_func(2 * lam, sig_time[i], sig_time[i + 1]) *
                   sig_time[i + 1] +
               Ji2_func(2 * lam, sig_time[i], sig_time[i + 1])) -
          sig[i + 1] * sig[i + 1] *
              Ji0_func(2 * lam, sig_time[i], sig_time[i + 1]) *
...
Err convertVol3(double lam, int nstept, double *time, int nb_sig,
                double *sig_time, double *sig, double *sigtilda,
                double *contsig, double **ablinsig) {
  int i, j;
  double t1, t2;
...
Err convertCorrelationWithTwoAffineVols(
    double domlam, double forlam, int nstept, double *time, int nb_sig,
    double *sig_time, double *correl, double *domsig, double *forsig,
    double *domsigtilda, double *forsigtilda, double *correltilda,
    double **ablincorrel, double *contcorrel) {
  int i, j;
  double t1, t2;
  char *err = NULL;
  ...
  for (i = 0; i < nb_sig - 1; ++i) {
    ...
    if (DeltaT > 30.0 / 365.0) {
      ...
      domsiga = (domsigtilda[i + 1] - domsigtilda[i]) /
                (sig_time[i + 1] - sig_time[i]);
      domsigb = domsigtilda[i] - domsiga * sig_time[i];

      forsiga = (forsigtilda[i + 1] - forsigtilda[i]) /
                (sig_time[i + 1] - sig_time[i]);
      forsigb = forsigtilda[i] - forsiga * sig_time[i];

      A = (J3 * domsiga * forsiga +
           J2 * (domsiga * forsigb + forsiga * domsigb -
                 sig_time[i] * domsiga * forsiga) +
           J1 * (domsigb * forsigb -
                 sig_time[i] * (domsiga * forsigb + forsiga * domsigb)) -
           J0 * sig_time[i] * domsigb * forsigb) /
          DeltaT;

      B = (-J3 * domsiga * forsiga +
           J2 * (sig_time[i + 1] * domsiga * forsiga -
                 (domsiga * forsigb + forsiga * domsigb)) +
           J1 * ((domsiga * forsigb + forsiga * domsigb) * sig_time[i + 1] -
                 domsigb * forsigb) +
           J0 * domsigb * forsigb * sig_time[i + 1]) /
          DeltaT;

      rho = (correl[i] * domsig[i] * forsig[i] * J0 - B * correltilda[i]) / A;

      if ((rho < -1) || (rho > 1)) {
        smessage("Cannot convert PW Const correl in continuous PW affine "
                 "correl        , Abs(correl)>1");
        //				err = "Cannot convert PW Const correl in
        // continuous PW affine correl        , Abs(correl)>1";
        // goto FREE_RETURN;
      }
  ...
  for (i = 1; i < nb_sig; ++i) {
    ablincorrelloc[0][i] =
        correltilda[i - 1] - (correltilda[i] - correltilda[i - 1]) *
                                 sig_time[i - 1] /
                                 (sig_time[i] - sig_time[i - 1]);
    ablincorrelloc[1][i] =
        (correltilda[i] - correltilda[i - 1]) / (sig_time[i] - sig_time[i - 1]);
  }
...
FREE_RETURN:
  ...
  if (ablincorrelloc)
    free_dmatrix(ablincorrelloc, 0, 1, 0, nb_sig);

  return err;
}

Err convertCorrelationWithOneAffineVol(double forlam, int nstept, double *time,
                                       int nb_sig, double *sig_time,
                                       double *correl, double *forsig,
                                       double *forsigtilda, double *correltilda,
                                       double **ablincorrel,
                                       double *contcorrel) {
  int i, j;
  double t1, t2;
  ...
  for (i = 0; i < nb_sig - 1; ++i) {
    ...
    if (DeltaT > 30.0 / 365.0) {
      ...
      A = forsiga * J2 + forsigb * J1 - forsiga * sig_time[i] * J1 -
          forsigb * sig_time[i] * J0;

      B = forsiga * J1 * sig_time[i + 1] - forsiga * J2 +
          forsigb * J0 * sig_time[i + 1] - forsigb * J1;

      rho = (correl[i] * forsig[i] * J0 * DeltaT - B * correltilda[i]) / A;
...
/*----------------------------------------------------------------------*/
Err convertVolQuadratic(double lam, int nstept, double *time, int nb_sig,
                        double *sig_time, double *sig, double *alpha,
                        double *contsig, double **abclinsig) {
  int i, j;
  double t1, t2;
  char *err = NULL;
  ...
  for (i = 0; i < nb_sig - 1; ++i) {
    ...
    D = (sig[i] * sig_time[i + 1] - sig[i + 1] * sig_time[i]) /
        (sig_time[i + 1] - sig_time[i]);
  ...
  for (i = 1; i < nb_sig; ++i) {
    abclinsigloc[0][i] = sig[i - 1] + alpha[i] * sig_time[i] * sig_time[i - 1] -
                         sig_time[i - 1] * (sig[i] - sig[i - 1]) /
                             (sig_time[i] - sig_time[i - 1]);
    abclinsigloc[1][i] =
        (sig[i] - sig[i - 1]) / (sig_time[i] - sig_time[i - 1]) -
  ...
  for (i = 0; i < nstept; ++i) {
    ...
    if (t2 > sig_time[nb_sig - 1]) {
      ...
      contsig[i] = abclinsigloc[0][j] + abclinsigloc[1][j] * t2 +
                   abclinsigloc[2][j] * t2 * t2;
    }
  }
...
FREE_RETURN:
  ...
  if (abclinsigloc)
    free_dmatrix(abclinsigloc, 0, 2, 0, nb_sig - 1);
...
void fillVol(int nstept, double *time, int nb_sig, double *sig_time,
             double *sig, double *sigall) {
  int i, j;
  double t1, t2;
...
void fillVolAndCorrel(int nstept, double *time, int nb_sig, double *sig_time,
                      double *domsig, double *forsig, double *fxsig,
                      double *domforrho, double *quantorho, double *domsigall,
                      double *forsigall, double *fxsigall, double *domforrhoall,
                      double *quantorhoall) {
  int i, j;
  double t1, t2;
...
static void DoubleLGM1FQuantoExpectations(
    int nstept, double *time, double domlam, double forlam,
    ...
    double domforrho, double *fwd1, double *fwd3, double *var1, double *var2,
    double *domphi, double *forphi, double *domforphi) {
  double t1, t2, ta, tb;
  ...
  for (i = 1; i < nstept; i++) {
    ...
    while (tb < t2 && j < nb_sig_minus1) {
      if (j > 0) {
        doma = domsig[j - 1] - (domsig[j] - domsig[j - 1]) * sig_time[j - 1] /
                                   (sig_time[j] - sig_time[j - 1]);
        domb = (domsig[j] - domsig[j - 1]) / (sig_time[j] - sig_time[j - 1]);

        fora = forsig[j - 1] - (forsig[j] - forsig[j - 1]) * sig_time[j - 1] /
                                   (sig_time[j] - sig_time[j - 1]);
      ...
      domforI2 +=
          doma * fora * Ji0_func(domlam + forlam, ta, tb) +
      ...
      std1 += doma * doma * (tb - ta) +
              domb * domb * (tb * tb * tb - ta * ta * ta) / 3.0 +
              doma * domb * (tb * tb - ta * ta);
    ...
    domforI2 +=
        doma * fora * Ji0_func(domlam + forlam, ta, t2) +
    ...
    std1 += doma * doma * (t2 - ta) +
            domb * domb * (t2 * t2 * t2 - ta * ta * ta) / 3.0 +
            doma * domb * (t2 * t2 - ta * ta);

    fwd1[i] =
        (exp(-domlam * t2) * domI1 - exp(-2 * domlam * t2) * domI2) / domlam;
  ...
  for (i = 1; i < nstept; i++) {
    ...
    while (tb < t2 && j < nb_sig_minus1) {
      if (j > 0) {
        ...
        quantorhoa = quantorho[j - 1] - (quantorho[j] - quantorho[j - 1]) *
                                            sig_time[j - 1] /
                                            (sig_time[j] - sig_time[j - 1]);
        quantorhob =
            (quantorho[j] - quantorho[j - 1]) / (sig_time[j] - sig_time[j - 1]);

        domforrhoa = domforrho[j - 1] - (domforrho[j] - domforrho[j - 1]) *
                                            sig_time[j - 1] /
                                            (sig_time[j] - sig_time[j - 1]);
        domforrhob =
            (domforrho[j] - domforrho[j - 1]) / (sig_time[j] - sig_time[j - 1]);
      }
      ...
      domforI2 += domforrhoa * doma * fora * Ji0_func(domlam + forlam, ta, tb) +
                  (domforrhoa * doma * forb + domforrhoa * fora * domb +
                   domforrhob * doma * fora) *
                      Ji1_func(domlam + forlam, ta, tb) +
                  (domforrhoa * domb * forb + domforrhob * doma * forb +
                   domforrhob * domb * fora) *
                      Ji2_func(domlam + forlam, ta, tb) +
                  domforrhob * domb * forb * Ji3_func(domlam + forlam, ta, tb);

      QAdj += fxsig[j] * (quantorhoa * fora * Ji0_func(forlam, ta, tb) +
                          (quantorhoa * forb + quantorhob * fora) *
                              Ji1_func(forlam, ta, tb) +
...
/*----Same function as DoubleLGM1FQuantoExpectations but with quadratic
 * vols---*/
...
static void DoubleLGM1FQuantoExpectationsQuadratic(
    ...
    double *sig_time, //	domsig        , forsig and fxsig must have
    double *domsig,   //	the same sig_time
    double *domalpha, double *forsig, double *foralpha, double *fxsig,
    int nb_sig, double quantorho, double domforrho, double *fwd1, double *fwd3,
    double *var1, double *var2, double *domphi, double *forphi,
    double *domforphi) {
  double t1, t2, ta, tb;
  int i, j, nb_sig_minus1;
  double domI1, domI2;
  ...
  for (i = 1; i < nstept; i++) {
    ...
    while (tb < t2 && j < nb_sig_minus1) {
      if (j > 0) {
        doma = domalpha[j];
        domb = (domsig[j] - domsig[j - 1]) / (sig_time[j] - sig_time[j - 1]) -
               domalpha[j] * (sig_time[j] + sig_time[j - 1]);
        domc = domsig[j - 1] + domalpha[j] * sig_time[j] * sig_time[j - 1] -
               sig_time[j - 1] * (domsig[j] - domsig[j - 1]) /
                   (sig_time[j] - sig_time[j - 1]);

        fora = foralpha[j];
        forb = (forsig[j] - forsig[j - 1]) / (sig_time[j] - sig_time[j - 1]) -
               foralpha[j] * (sig_time[j] + sig_time[j - 1]);
        forc = forsig[j - 1] + foralpha[j] * sig_time[j] * sig_time[j - 1] -
               sig_time[j - 1] * (forsig[j] - forsig[j - 1]) /
                   (sig_time[j] - sig_time[j - 1]);
      }
      ...
      domforI2 +=
          doma * fora * Ji4_func(domlam + forlam, ta, tb) +
          (doma * forb + domb * fora) * Ji3_func(domlam + forlam, ta, tb) +
          (doma * forc + domb * forb + domc * fora) *
              Ji2_func(domlam + forlam, ta, tb) +
      ...
      QAdj +=
          quantorho * fxsig[j] *
          (fora * Ji2_func(forlam, ta, tb) + forb * Ji1_func(forlam, ta, tb) +
           forc * Ji0_func(forlam, ta, tb));

      std1 += doma * doma * (tb * tb * tb * tb * tb - ta * ta * ta * ta * ta) /
                  5.0 +
              2 * doma * domb * (tb * tb * tb * tb - ta * ta * ta * ta) / 4.0 +
              (2 * doma * domc + domb * domb) * (tb * tb * tb - ta * ta * ta) /
                  3.0 +
    ...
    domforI2 +=
        doma * fora * Ji4_func(domlam + forlam, ta, t2) +
    ...
    QAdj += quantorho * fxsig[j] *
            (fora * Ji2_func(forlam, ta, t2) + forb * Ji1_func(forlam, ta, t2) +
             forc * Ji0_func(forlam, ta, t2));

    std1 +=
        doma * doma * (t2 * t2 * t2 * t2 * t2 - ta * ta * ta * ta * ta) / 5.0 +
        2 * doma * domb * (t2 * t2 * t2 * t2 - ta * ta * ta * ta) / 4.0 +
        (2 * doma * domc + domb * domb) * (t2 * t2 * t2 - ta * ta * ta) / 3.0 +
        2 * domb * domc * (t2 * t2 - ta * ta) / 2.0 + domc * domc * (t2 - ta);
...
Err doublelgm1fQuanto_adi(
    ...
    // Market data
    double *dom_ifr, double *for_ifr, char *dom_yc, char *for_yc,
    ...
    Err (*payoff_func)( // Event
        ...
        /* Model data	*/
        double domlam, double forlam, double domphi, double forphi,

        /* Grid data	*/
        int l1, int u1, int l2, int u2, double *r1, double **r2,
  ...
  double *r1_bar = NULL, *r3_bar = NULL, *r1_dim1 = NULL, **r1_dim2 = NULL,
         ...
         *fwd3 = NULL, *var1 = NULL, *var3 = NULL, *domphi = NULL,
         *forphi = NULL, *domforphi = NULL;
  ...
  if (!pde || !var1 || !var3 || !domphi || !forphi || !domforphi || !fwd1 ||
      !fwd3 || !domsigtilda || !forsigtilda || !allfxsig || !contdomsig ||
      !contforsig || !domabsig || !forabsig) {
    err = "Memory allocation error (1) in doublelgm1fQuanto_adi";
    goto FREE_RETURN;
  }

  err = convertVol2(domlam, nstept, time, nb_sig, sig_time, domsig, domsigtilda,
                    contdomsig, domabsig);
  if (err) {
    goto FREE_RETURN;
  }
  err = convertVol2(forlam, nstept, time, nb_sig, sig_time, forsig, forsigtilda,
                    contforsig, forabsig);
  if (err) {
    goto FREE_RETURN;
  }
  ...
  DoubleLGM1FQuantoExpectations(nstept, time, domlam, forlam, sig_time,
                                domsigtilda, forsigtilda, fxsig, nb_sig,
                                quantorho, domforrho, fwd1, fwd3, var1, var3,
                                domphi, forphi, domforphi);
  ...
  std3 =
      sqrt(((contdomsig[nstept - 1] / contforsig[nstept - 1]) *
                (contdomsig[nstept - 1] / contforsig[nstept - 1]) *
                forphi[nstept - 1] +
            domforrho * domforrho * domphi[nstept - 1] -
            2 * domforrho * (contdomsig[nstept - 1] / contforsig[nstept - 1]) *
                domforphi[nstept - 1]) /
  ...
  for (i = 0; i < nstepx; i++) {
    ...
    r2_temp = domforrho * r1_bar[i] *
              (contforsig[nstept - 1] / contdomsig[nstept - 1]);
    r_temp = domforrho *
             (1 + (contforsig[nstept - 1] / contdomsig[nstept - 1])) *
             r1_bar[i];

    for (j = 0; j < nstepz; j++) {
      r1_dim2[i][j] = r1_dim1[i];
      r2[i][j] = fwd3[nstept - 1] + r2_temp +
                 (contforsig[nstept - 1] / contdomsig[nstept - 1]) *
                     sqrt(1 - domforrho * domforrho) * r3_bar[j];
      r_init[i][j] = r1_bar[i];
    }
  }
  ...
  err = payoff_func(date[nstept - 1], time[nstept - 1],
                    func_parm_tab[nstept - 1], dom_yc, for_yc, domlam, forlam,
                    domphi[nstept - 1], forphi[nstept - 1], 0, nstepx - 1, 0,
  ...
  for (step = nstept - 2; step >= 0; step--) {
    ...
    for (i = lx; i <= ux; i++) {
      ...
      mu_r3i = -(forlam - domlam) * domforrho * r1_bar[i] /
               sqrt(1 - domforrho * domforrho);

      for (j = lz; j <= uz; j++) {
        mu_r1[i][j] = mu_r1i;
        mu_r3[i][j] = (mu_r3i - forlam * r3_bar[j] +
                       (r3_bar[j] + domforrho * r1_bar[i] /
                                        sqrt(1 - domforrho * domforrho)) *
                           (domabsig[1][step] / contdomsig[step] -
                            forabsig[1][step] / contforsig[step])) *
                      dt;
    ...
    if (eval_evt[step]) {
      ...
      for (i = lx; i <= ux; i++) {
        ...
        for (j = 0; j < nstepz; j++) {
          r1_dim2[i][j] = r1_dim1[i];
          r2[i][j] = fwd3[step] + r2_temp +
                     (contforsig[step] / contdomsig[step]) *
                         sqrt(1 - domforrho * domforrho) * r3_bar[j];
        }
      }

      err = payoff_func(date[step], time[step], func_parm_tab[step], dom_yc,
                        for_yc, domlam, forlam, domphi[step], forphi[step], lx,
                        ux, lz, uz, r1_dim1, r2, nprod, values);
...
FREE_RETURN:
  ...
  // allocation (1)
  if (domphi)
    free_dvector(domphi, 0, nstept - 1);
  if (forphi)
    free_dvector(forphi, 0, nstept - 1);
  if (domforphi)
    free_dvector(domforphi, 0, nstept - 1);
  if (var1)
  ...
  if (domsigtilda)
    free_dvector(domsigtilda, 0, nb_sig - 1);
  if (forsigtilda)
    free_dvector(forsigtilda, 0, nb_sig - 1);
  if (contdomsig)
    free_dvector(contdomsig, 0, nstept - 1);
  if (contforsig)
    free_dvector(contforsig, 0, nstept - 1);
  if (allfxsig)
    free_dvector(allfxsig, 0, nstept - 1);
  if (domabsig)
    free_dmatrix(domabsig, 0, 1, 0, nstept - 1);
  if (forabsig)
    free_dmatrix(forabsig, 0, 1, 0, nstept - 1);
...
Err doublelgm1fQuanto_adi2(
  ...
  if (!pde || !var1 || !var3 || !domphi || !forphi || !domforphi || !fwd1 ||
      !fwd3 || !domalpha || !foralpha || !allfxsig || !contdomsig ||
      !contforsig || !domabcsig || !forabcsig) {
    err = "Memory allocation error (1) in doublelgm1fQuanto_adi2";
    goto FREE_RETURN;
  }

  err = convertVolQuadratic(domlam, nstept, time, nb_sig, sig_time, domsig,
                            domalpha, contdomsig, domabcsig);
  if (err) {
    goto FREE_RETURN;
  }
  err = convertVolQuadratic(forlam, nstept, time, nb_sig, sig_time, forsig,
                            foralpha, contforsig, forabcsig);
  if (err) {
    goto FREE_RETURN;
  }
  fillVol(nstept, time, nb_sig, sig_time, fxsig, allfxsig);
  ...
  // Calculate the expecations of r1 and r3
  DoubleLGM1FQuantoExpectationsQuadratic(
      nstept, time, domlam, forlam, sig_time, domsig, domalpha, forsig,
      foralpha, fxsig, nb_sig, quantorho, domforrho, fwd1, fwd3, var1, var3,
      domphi, forphi, domforphi);
      ...
      for (j = lz; j <= uz; j++) {
        ...
        mu_r3[i][j] =
            (mu_r3i - forlam * r3_bar[j] +
             (r3_bar[j] +
              domforrho * r1_bar[i] / sqrt(1 - domforrho * domforrho)) *
                 ((domabcsig[1][step] + 2 * domabcsig[2][step] * time[step]) /
                      contdomsig[step] -
                  (forabcsig[1][step] + 2 * forabcsig[2][step] * time[step]) /
                      contforsig[step])) *
            dt;
...
FREE_RETURN:
  ...
  if (fwd3)
    free_dvector(fwd3, 0, nstept - 1);
  if (domalpha)
    free_dvector(domalpha, 0, nb_sig - 1);
  if (foralpha)
    free_dvector(foralpha, 0, nb_sig - 1);
  if (contdomsig)
    free_dvector(contdomsig, 0, nstept - 1);
  if (contforsig)
    free_dvector(contforsig, 0, nstept - 1);
  if (allfxsig)
    free_dvector(allfxsig, 0, nstept - 1);
  if (domabcsig)
    free_dmatrix(domabcsig, 0, 2, 0, nstept - 1);
  if (forabcsig)
    free_dmatrix(forabcsig, 0, 2, 0, nstept - 1);
...
Err doublelgm1fQuanto_adi_correl(
  ...
  if (!pde || !var1 || !var3 || !domphi || !forphi || !domforphi || !fwd1 ||
      !fwd3 || !domsigtilda || !forsigtilda || !domforcorreltilda ||
      !quantocorreltilda || !allfxsig || !contdomsig || !contforsig ||
      !contdomforcorrel || !contquantocorrel || !domabsig || !forabsig ||
      !ablindomforcorrel || !ablinquantocorrel) {
    err = "Memory allocation error (1) in doublelgm1fQuanto_adi";
    goto FREE_RETURN;
  }
  ...
  err = convertCorrelationWithTwoAffineVols(
      domlam, forlam, nstept, time, nb_sig, sig_time, domforrho, domsig, forsig,
      domsigtilda, forsigtilda, domforcorreltilda, ablindomforcorrel,
      contdomforcorrel);
  if (err) {
    goto FREE_RETURN;
  }
  ...
  err = convertCorrelationWithOneAffineVol(
      forlam, nstept, time, nb_sig, sig_time, quantorho, forsig, forsigtilda,
      quantocorreltilda, ablinquantocorrel, contquantocorrel);
  if (err) {
    goto FREE_RETURN;
  }
  ...
  // Calculate the expecations of r1 and r3
  DoubleLGM1FQuantoExpectationsWithCorrelTS(
      nstept, time, domlam, forlam, sig_time, domsigtilda, forsigtilda, fxsig,
      nb_sig, quantocorreltilda, domforcorreltilda, fwd1, fwd3, var1, var3,
      domphi, forphi, domforphi);
  ...
  std3 =
      sqrt(((contdomsig[nstept - 1] / contforsig[nstept - 1]) *
                (contdomsig[nstept - 1] / contforsig[nstept - 1]) *
                forphi[nstept - 1] +
            contdomforcorrel[nstept - 1] * contdomforcorrel[nstept - 1] *
                domphi[nstept - 1] -
            2 * contdomforcorrel[nstept - 1] *
                (contdomsig[nstept - 1] / contforsig[nstept - 1]) *
                domforphi[nstept - 1]) /
  ...
  for (i = 0; i < nstepx; i++) {
    r1_dim1[i] = r1_bar[i] + fwd1[nstept - 1];
    r2_temp = contdomforcorrel[nstept - 1] * r1_bar[i] *
              (contforsig[nstept - 1] / contdomsig[nstept - 1]);
    //		r_temp = contdomforcorrel[nstept-1] * (1 +
    //(contforsig[nstept-1]/contdomsig[nstept-1])) * r1_bar[i];

    for (j = 0; j < nstepz; j++) {
      r1_dim2[i][j] = r1_dim1[i];
      r2[i][j] = fwd3[nstept - 1] + r2_temp +
                 (contforsig[nstept - 1] / contdomsig[nstept - 1]) *
                     sqrt(1 - contdomforcorrel[nstept - 1] *
                                  contdomforcorrel[nstept - 1]) *
                     r3_bar[j];
      r_init[i][j] = r1_bar[i];
    }
  }
  ...
  for (step = nstept - 2; step >= 0; step--) {
    ...
    for (i = lx; i <= ux; i++) {
      mu_r1i = -domlam * r1_bar[i] * dt;
      mu_r3i = -(forlam - domlam) * contdomforcorrel[step] * r1_bar[i] /
               sqrt(1 - contdomforcorrel[step] * contdomforcorrel[step]);

      for (j = lz; j <= uz; j++) {
        mu_r1[i][j] = mu_r1i;
        temp = (contdomforcorrel[step] * r3_bar[j] /
                    sqrt(1 - contdomforcorrel[step] * contdomforcorrel[step]) -
                r1_bar[i]) *
               ablindomforcorrel[1][step] /
               sqrt(1 - contdomforcorrel[step] * contdomforcorrel[step]);

        mu_r3[i][j] =
            (mu_r3i - forlam * r3_bar[j] +
             (r3_bar[j] +
              contdomforcorrel[step] * r1_bar[i] /
                  sqrt(1 - contdomforcorrel[step] * contdomforcorrel[step])) *
                 (domabsig[1][step] / contdomsig[step] -
                  forabsig[1][step] / contforsig[step]) +
             (contdomforcorrel[step] * r3_bar[j] /
                  sqrt(1 - contdomforcorrel[step] * contdomforcorrel[step]) -
              r1_bar[i]) *
                 ablindomforcorrel[1][step] /
                 sqrt(1 - contdomforcorrel[step] * contdomforcorrel[step])) *
    ...
    if (eval_evt[step]) {
      ...
      for (i = lx; i <= ux; i++) {
        r1_dim1[i] = r1_bar[i] + fwd1[step];
        r2_temp = contdomforcorrel[step] * r1_bar[i] *
                  (contforsig[step] / contdomsig[step]);
...
void AdjustJumpingDrift(
  ...
  //	ftemp = f3tensor(0        , nx-1        , 0        , ny-1        , 0 ,
  //mend);
  ...
  for (i = lx; i <= ux; i++) {
    ...
    for (j = ly + 1; j <= uy; j++) {
      for (k = mstart; k <= mend; k++) {
        yderivative =
            (f_t_plus[i][j][k] - f_t_plus[i][j - 1][k]) / (y[j] - y[j - 1]);
        ...
        //								- f_t[i][j-1][k]
        //*
        // JumpYDividedByDeltaY 								) / (1 -
        // JumpYDividedByDeltaY);
      }
    }
  }

  //	free_f3tensor(ftemp        , 0        , nx-1        , 0        , ny-1 ,
  //0        , mend);
}
...
Err doublelgm1fQuanto_adi_correl2(
  ...
  double *r1_bar = NULL, *r3_bar = NULL, *r1_dim1 = NULL, **r1_dim2 = NULL,
         **r2 = NULL, ***values = NULL, ***values_p1 = NULL,
         ***values_temp = NULL, **mu_r1 = NULL, **mu_r3 = NULL, **var_r1 = NULL,
         **var_r3 = NULL, **zerovar1 = NULL, **zerovar3 = NULL, **r = NULL,
         **zeror = NULL, **r_init = NULL, *fwd1 = NULL, *fwd3 = NULL,
         *var1 = NULL, *var3 = NULL, *domphi = NULL, *forphi = NULL,
         *domforphi = NULL;
  ...
  if (!pde || !var1 || !var3 || !domphi || !forphi || !domforphi || !fwd1 ||
      !fwd3 || !allfxsig || !contdomsig || !contforsig || !contdomforcorrel ||
      !contquantocorrel) {
    err = "Memory allocation error (1) in doublelgm1fQuanto_adi";
    goto FREE_RETURN;
  }

  fillVolAndCorrel(nstept, time, nb_sig, sig_time, domsig, forsig, fxsig,
                   domforrho, quantorho, contdomsig, contforsig, allfxsig,
                   contdomforcorrel, contquantocorrel);

  // Calculate the expecations of r1 and r3
  DoubleLGM1FQuantoExpectationsWithCorrelationTS(
      nstept, time, domlam, forlam, sig_time, domsig, forsig, fxsig, nb_sig,
      quantorho, domforrho, fwd1, fwd3, var1, var3, domphi, forphi, domforphi);
  ...
  if (!r1_dim2 || !r2 || !values || !values_p1 || !mu_r1 || !mu_r3 ||
      !zerovar1 || !zerovar3 || !var_r1 || !var_r3 || !zeror || !r || !r_init ||
  ...
  for (step = nstept - 2; step >= 0; step--) {
      ...
      for (j = lz; j <= uz; j++) {
        ...
        Jump_r3[i][j] =
            ((contdomsig[step + 1] /
              (contforsig[step + 1] *
               sqrt(1 -
                    contdomforcorrel[step + 1] * contdomforcorrel[step + 1]))) -
             (contdomsig[step] /
              (contforsig[step] *
               sqrt(1 - contdomforcorrel[step] * contdomforcorrel[step])))) *
                ((0.5 * (contforsig[step + 1] / contdomsig[step + 1]) *
                      r3_bar[j] *
                      sqrt(1 - contdomforcorrel[step + 1] *
                                   contdomforcorrel[step + 1]) +
                  0.5 * (contforsig[step + 1] / contdomsig[step + 1]) *
                      contdomforcorrel[step + 1] * r1_bar[i]) +
                 (0.5 * (contforsig[step] / contdomsig[step]) * r3_bar[j] *
                      sqrt(1 -
                           contdomforcorrel[step] * contdomforcorrel[step]) +
                  0.5 * (contforsig[step] / contdomsig[step]) *
                      contdomforcorrel[step] * r1_bar[i])) -
            (contdomforcorrel[step + 1] /
                 sqrt(1 -
                      contdomforcorrel[step + 1] * contdomforcorrel[step + 1]) -
             contdomforcorrel[step] /
                 sqrt(1 - contdomforcorrel[step] * contdomforcorrel[step])) *
                r1_bar[i];
    ...
    if ((contforsig[step] != contforsig[step + 1]) ||
        (contdomsig[step] != contdomsig[step + 1]) ||
        (contdomforcorrel[step] != contdomforcorrel[step + 1])) {
      num_f_pde_one_step_backward_2f_adi(
          pde, nstepx, r1_bar, nstepz, r3_bar, 0, nprod - 1, values_p1, Jump_r1,
          Jump_r3, zerovar1, zerovar3, zeror, values, lx, ux, lz, uz);
      values_temp = values_p1;
      values_p1 = values;
      values = values_temp;
    }
...
FREE_RETURN:
  ...
  if (contforsig)
    free_dvector(contforsig, 0, nstept - 1);
  if (contdomforcorrel)
    free_dvector(contdomforcorrel, 0, nstept - 1);
  if (contquantocorrel)
    free_dvector(contquantocorrel, 0, nstept - 1);
  if (allfxsig)
    free_dvector(allfxsig, 0, nstept - 1);
  ...
  if (mu_r3)
    free_dmatrix(mu_r3, 0, nstepx - 1, 0, nstepz - 1);
  if (Jump_r1)
    free_dmatrix(Jump_r1, 0, nstepx - 1, 0, nstepz - 1);
  if (Jump_r3)
    free_dmatrix(Jump_r3, 0, nstepx - 1, 0, nstepz - 1);
  if (var_r1)
    free_dmatrix(var_r1, 0, nstepx - 1, 0, nstepz - 1);
  if (var_r3)
    free_dmatrix(var_r3, 0, nstepx - 1, 0, nstepz - 1);
  if (zerovar1)
    free_dmatrix(zerovar1, 0, nstepx - 1, 0, nstepz - 1);
  if (zerovar3)
    free_dmatrix(zerovar3, 0, nstepx - 1, 0, nstepz - 1);
  if (r)
    free_dmatrix(r, 0, nstepx - 1, 0, nstepz - 1);
  if (zeror)
    free_dmatrix(zeror, 0, nstepx - 1, 0, nstepz - 1);
  if (r_init)
    free_dmatrix(r_init, 0, nstepx - 1, 0, nstepz - 1);

...
Err Get_FX_TS(char* underlying, double** sigma_time, double** sigma, long* sigma_n)
{
    ...
    err = srt_f_display_FX_TermStruct(
        underlying, sigma_n, sigma_time, sigma, &corr_date_n, &corr_date, &corr);
...
Err fill_time_vector_without_barrier(
    double** time, int* nstp, int num_vol_times, double* vol_times, int target_nstp)
{
    Err err = NULL;
    ...
    double** prod_val)
{
    ...
    err = fill_time_vector_without_barrier(&time, &nstp, num_vol_times, vol_times, num_stp);
    if (err)
    {
        goto FREE_RETURN;
    }
    ...
    double** prod_val)
{
    ...
    for (i = nb_merge_dates - 1; i >= 0; i--)
    {
        ...
        err       = srt_f_get_corr_from_CorrList(
            sCorrlist, domname, forname, merge_dates[i], &(domforcorr[i]));
        err = srt_f_get_corr_from_CorrList(
            sCorrlist, forname, und3dfx, merge_dates[i], &(quantocorr[i]));
    }

...
void Fx3DBetaDLM_DriftVolAndCorr(
    long             nstp,
    double*          time,
        ...
        */
        ...
        if ((i == nstp - 1) ||
            (fabs(sig_dom[i] - sig_dom[i + 1]) + fabs(sig_for[i] - sig_for[i + 1]) +
                 fabs(sig_fx[i] - sig_fx[i + 1]) + fabs(corr_dom_for[i] - corr_dom_for[i + 1]) +
        ...
        if (i < nstp - 1)
        {
            ...
            dr_const_for[i] =
                for_phi[i] - (coef_B + 2.0 * coef_C * coef_D) * big_bracket + small_bracket;
    ...
    double**                   prod_val)
{
    ...
    double *dr_const_dom = NULL, *dr_coef_dom = NULL, *dr_const_for = NULL, *dr_coef1_for = NULL,
           *dr_coef2_for = NULL, *dr_coef3_for = NULL, *dr_const_fx = NULL, *dr_coef1_fx = NULL,
           *dr_coef2_fx = NULL, *dr_coef3_fx = NULL;

    double *dom_fwd = NULL, *dom_var = NULL, *dom_phi = NULL, *dom_ifr = NULL, *dom_beta = NULL,
           *for_fwd = NULL, *for_var = NULL, *for_phi = NULL, *for_ifr = NULL, *for_beta = NULL,
           *fx_fwd = NULL, *fx_var = NULL, *var_ffx = NULL, *global_corr_dom_for = NULL,
    ...
    {
        if (eventdates[i] > today && eventdates[i] <= xStr.dts[nstp - 1] && barrier[i] > 1.0e-08)
        {
    ...
    err = fill_time_vector(
        &time, &nstp, num_bar_times, bar_times, model->iNbPWTime, model->dPWTime, num_stp);
    ...
    if (!vol_change || !sig_dom || !sig_for || !sig_fx || !corr_dom_for || !corr_dom_fx ||
        !corr_for_fx || !dr_const_dom || !dr_coef_dom || !dr_const_for || !dr_coef1_for ||
        !dr_coef2_for || !dr_coef3_for || !dr_const_fx || !dr_coef1_fx || !dr_coef2_fx ||
        !dr_coef3_fx || !dom_ifr || !dom_fwd || !dom_var || !dom_phi || !dom_beta || !for_ifr ||
        !for_fwd || !for_var || !for_phi || !for_beta || !fx_fwd || !fx_var || !var_ffx ||
    ...
    double**                   prod_val)
{
    ...
    double *sig_dom = NULL, *mu_quanto_cons = NULL, *mu_quanto_lin = NULL, *sig_for = NULL,
           *sig_fx = NULL, *corr_dom_for = NULL, *corr_dom_fx = NULL, *corr_for_fx = NULL;

    double *dom_fwd = NULL, *dom_var = NULL, *dom_phi = NULL, *dom_beta = NULL, *for_fwd = NULL,
           *for_var = NULL, *for_phi = NULL, *for_beta = NULL, *fx_fwd = NULL, *fx_var = NULL,
           *ffx_var = NULL, *global_corr_dom_for = NULL, *global_corr_dom_fx = NULL,
    ...
    if (!vol_change || !sig_dom || !mu_quanto_cons || !mu_quanto_lin || !sig_for || !sig_fx ||
        !corr_dom_for || !corr_dom_fx || !corr_for_fx || !dom_fwd || !dom_var || !dom_phi ||
        !dom_beta || !for_fwd || !for_var || !for_phi || !for_beta || !fx_fwd || !fx_var ||
        !ffx_var || !global_corr_dom_for || !global_corr_dom_fx || !global_corr_for_fx)
    {
        err = "Memory allocation error (5) in SrtGrfn3DFXTree";
        goto FREE_RETURN;
    }
    ...
    for (UpOrDown = 0; UpOrDown <= alphaFudge; UpOrDown++)
    {
        ...
        for (i = nstp - 1; i >= 0; i--)
        {
            ...
            sig_fx[i] = pow(temp_vol_fx, 2) + pow(temp_vol_dom, 2) + pow(temp_vol_for, 2) -
            ...
            corr_for_fx[i] = (model->dCorrDomFor[index] * temp_vol_dom - temp_vol_for +
                              model->dCorrForFx[index] * temp_vol_fx) /
                             sig_fx[i];
        ...
        for (i = nstp - 1; i >= 0; i--)
        {
                ...
                if (grfn_prm->num_fx_df > 0 && grfn_prm->fx_idx != -1)
                {
                    ...
                    for (k = 0; k < grfn_prm->num_fx_df; k++)
                    {
                        ...
                        grfn_prm->df_fx_coef_const[k] =
                            log(swp_f_df(xStr.dts[j], grfn_prm->fx_df_dts[k], model->cYcDom));
                        grfn_prm->df_fx_coef_const[k] -=
                            0.5 * (beta_temp * beta_temp - dom_beta[i] * dom_beta[i]) * dom_phi[i];
                ...
                if (grfn_prm->num_dom_df > 0 && grfn_prm->dom_idx != -1)
                {
                    ...
                    for (k = 0; k < grfn_prm->num_dom_df; k++)
                    {
                        ...
                        grfn_prm->df_dom_coef_const[k] =
                            log(swp_f_df(xStr.dts[j], grfn_prm->dom_df_dts[k], model->cYcDom));
                        grfn_prm->df_dom_coef_const[k] -=
                            0.5 * (beta_temp * beta_temp - dom_beta[i] * dom_beta[i]) * dom_phi[i];
                ...
                if (grfn_prm->num_for_df > 0 && grfn_prm->for_idx != -1)
                {
                    ...
                    for (k = 0; k < grfn_prm->num_for_df; k++)
                    {
                        ...
                        grfn_prm->df_for_coef_const[k] =
                            log(swp_f_df(xStr.dts[j], grfn_prm->for_df_dts[k], model->cYcFor));
                        grfn_prm->df_for_coef_const[k] -=
                            0.5 * (beta_temp * beta_temp - for_beta[i] * for_beta[i]) * for_phi[i];
            ...
            if ((i > 0) && (fabs(next_bar) > 1.0e-08))
            {
                next_bar += (for_fwd[i - 1] - -dom_fwd[i - 1]) * (time[i] - time[i - 1]);
            }
        }
            ...
            if (void_prm[i])
            {
                ...
                if (grfn_prm->do_fx && grfn_prm->num_fx_df > 0 && grfn_prm->fx_idx != -1)
                {
                    if (grfn_prm->df_fx_coef_const)
                        free_dvector(grfn_prm->df_fx_coef_const, 0, grfn_prm->num_fx_df - 1);
                ...
                if (grfn_prm->do_dom && grfn_prm->num_dom_df > 0 && grfn_prm->dom_idx != -1)
                {
                    if (grfn_prm->df_dom_coef_const)
                        free_dvector(grfn_prm->df_dom_coef_const, 0, grfn_prm->num_dom_df - 1);
                    if (grfn_prm->df_dom_coef_lin)
                        free_dvector(grfn_prm->df_dom_coef_lin, 0, grfn_prm->num_dom_df - 1);
                }

                if (grfn_prm->do_for && grfn_prm->num_for_df > 0 && grfn_prm->for_idx != -1)
                {
                    if (grfn_prm->df_for_coef_const)
                        free_dvector(grfn_prm->df_for_coef_const, 0, grfn_prm->num_for_df - 1);
                    if (grfn_prm->df_for_coef_lin)
                        free_dvector(grfn_prm->df_for_coef_lin, 0, grfn_prm->num_for_df - 1);
                }
            }
        }
    ...
    double*** prod_val)
{
    ...
    double *time = NULL, *date = NULL, *dom_fwd = NULL, *dom_std = NULL, *dom_phi = NULL,
           *dom_beta = NULL, *bond_pay_const = NULL, *bond_pay_lin = NULL, *for_ifr = NULL,
           *for_fwd_const = NULL, *for_fwd_lin = NULL, *for_std = NULL, *for_phi = NULL,
           *for_beta = NULL, *fx_fwd = NULL, *fx_std = NULL, *ffx_var = NULL, *dom_for_cov = NULL,
           *dom_fx_cov = NULL, *for_fx_cov = NULL, **prod_val_temp = NULL;
    ...
    if (!dom_fwd || !dom_std || !dom_phi || !dom_beta || !for_fwd_const || !for_fwd_lin ||
        !for_std || !for_phi || !for_beta || !fx_fwd || !fx_std || !ffx_var || !bond_pay_const ||
        !bond_pay_lin || !dom_for_cov || !dom_fx_cov || !for_fx_cov)
    {
                ...
                if (grfn_prm->num_fx_df > 0 && grfn_prm->fx_idx != -1)
                {
                    ...
                    for (j = 0; j < grfn_prm->num_fx_df; j++)
                    {
                        ...
                        grfn_prm->df_fx_coef_const[j] =
                            log(swp_f_df(xStr.dts[i], grfn_prm->fx_df_dts[j], model->cYcDom));
                        grfn_prm->df_fx_coef_const[j] -=
                            0.5 *
                            (beta_temp * beta_temp - dom_beta[i + flag] * dom_beta[i + flag]) *
                            dom_phi[i + flag];
                ...
                if (grfn_prm->num_dom_df > 0 && grfn_prm->dom_idx != -1)
                {
                    ...
                    for (j = 0; j < grfn_prm->num_dom_df; j++)
                    {
                        ...
                        grfn_prm->df_dom_coef_const[j] =
                            log(swp_f_df(xStr.dts[i], grfn_prm->dom_df_dts[j], model->cYcDom));
                        grfn_prm->df_dom_coef_const[j] -=
                            0.5 *
                            (beta_temp * beta_temp - dom_beta[i + flag] * dom_beta[i + flag]) *
                            dom_phi[i + flag];
                ...
                if (grfn_prm->num_for_df > 0 && grfn_prm->for_idx != -1)
                {
                    ...
                    for (j = 0; j < grfn_prm->num_for_df; j++)
                    {
                        ...
                        grfn_prm->df_for_coef_const[j] =
                            log(swp_f_df(xStr.dts[i], grfn_prm->for_df_dts[j], model->cYcFor));
                        grfn_prm->df_for_coef_const[j] -=
                            0.5 *
                            (beta_temp * beta_temp - for_beta[i + flag] * for_beta[i + flag]) *
                            for_phi[i + flag];

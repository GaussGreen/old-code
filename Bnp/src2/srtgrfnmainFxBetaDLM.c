/**********************************************************************
 *      Name: SrtGrfnMainFxBetaDLM.c                                  * 
 *  Function: Entry point to GRFN with raw data                       *
 *            in the case of the 3Factor Beta DLM model				  *	
...
char *SrtGrfnFx3DFxBetaDLMTreeQBeta(
				  char						*und3dfx,
	              int						numeventdates, 
				  long						*eventdates,
                  long						tableauRows,
...
Err err = NULL;
	...
	{
		err = SrtGrfnFx3DFxBetaDLMTreeQBeta(und3dfx,
											numeventdates, 
											eventdates,
											tableauRows,
											tableauCols,
	...
	{
		err = SrtGrfnFx3DFxBetaDLMTreeQTStar(und3dfx,
											numeventdates, 
											eventdates,
											tableauRows,
											tableauCols,
...
double	small_bracket, big_bracket,
		coef_B, coef_C, coef_D;
	...
	for (i=nstp-1; i>=0; i--)
	{
		...
		*/
		...
		if (i < nstp-1)
		{
			...
			coef_B *= model->dB0;
			coef_D = 0.5 * (var_ffx[i]
				+ dom_beta[i] * dom_beta[i] * dom_phi[i] 
			...
			dr_const_for[i] = for_phi[i] - (coef_B + 2.0 * coef_C * coef_D) * big_bracket
				+ small_bracket;
				  ...
				  double					**prod_val)
{
	...
	double			*dom_fwd				= NULL,
					...
					*fx_var					= NULL,
					*var_ffx				= NULL,
					*global_corr_dom_for	= NULL,
					*global_corr_dom_fx		= NULL,
	...
	err = FxBetaDLM_Get_Model(	und3dfx,
								model);
	...
	if (!vol_change || !sig_dom || !sig_for || !sig_fx ||
		...
		!dom_ifr || !dom_fwd || !dom_var || !dom_phi || !dom_beta || 
		!for_ifr || !for_fwd || !for_var || !for_phi || !for_beta ||
		!fx_fwd || !fx_var || !var_ffx ||
	...
	for(UpOrDown=0; UpOrDown<=alphaFudge; UpOrDown++)
	{
		if(alphaFudge)
		{
			...
			if(!UpOrDown)
			{	
				memcpy(model->dSigmaFx, model->dSigmaFxUp, model->iNbPWTime * sizeof(double));
			}
			else
			{
				memcpy(model->dSigmaFx, model->dSigmaFxDown, model->iNbPWTime * sizeof(double));
				memcpy(model->dCorrDomFx, model->dCorrDomFxDown, model->iNbPWTime * sizeof(double));
				memcpy(model->dCorrForFx, model->dCorrForFxDown, model->iNbPWTime * sizeof(double));
			}
		}

		/* Fill the global expectations and variances */
		Fx3DBetaDLM_PrecalcGRFNTreeQBeta(	nstp,
											time,
											model,
											dom_fwd,
											dom_var,
		...
		Fx3DBetaDLM_DriftVolAndCorr(	nstp,
										time,
										model,
										dom_phi,
										dom_beta,
		...
		for (i=nstp-1; i>=0; i--)
		{
			...
			{
				next_bar += (for_ifr[i-1] + for_fwd[i-1] - dom_ifr[i-1] - dom_fwd[i-1]) 
					* (time[i] - time[i-1]);
			}
		...
		*num_prod = xStr.num_cols;
		...
		err = tree_main_3dBetaDLM_QBeta(
						...
						UpOrDown? void_prm_down:void_prm, 
						is_event,
						bar_lvl,
						bar_cl,
						is_bar,
						grfn_payoff_4_3dfxBetaDLMQBeta_Tree,
						*num_prod,
						Params->do_discount,
...
FREE_RETURN:
	...
	if (dr_const_dom) free(dr_const_dom);
	if (dr_coef_dom) free(dr_coef_dom);
	if (dr_const_for) free(dr_const_for);
	if (dr_coef1_for) free(dr_coef1_for);
	if (dr_coef2_for) free(dr_coef2_for);
	if (dr_coef3_for) free(dr_coef3_for);
	if (dr_const_fx) free(dr_const_fx);
	if (dr_coef1_fx) free(dr_coef1_fx);
	if (dr_coef2_fx) free(dr_coef2_fx);
	if (dr_coef3_fx) free(dr_coef3_fx);
	...
	if (global_corr_dom_for) free(global_corr_dom_for);
	if (global_corr_dom_fx) free(global_corr_dom_fx);
	if (global_corr_for_fx) free(global_corr_for_fx);
	...
	if(prod_val_temp) free(prod_val_temp);
				  ...
				  double					**prod_val)
{
	...
	double			t_mid;
	double			temp_beta_dom, temp_beta_for,
					temp_vol_dom, temp_vol_for, temp_vol_fx;
	...
	if (!vol_change || !sig_dom || !mu_quanto_cons || !mu_quanto_lin || !sig_for || !sig_fx ||
		!corr_dom_for || !corr_dom_fx || !corr_for_fx ||
		!dom_fwd || !dom_var || !dom_phi || !dom_beta || 
		!for_fwd || !for_var || !for_phi || !for_beta
		|| !fx_fwd || !fx_var || !ffx_var || !global_corr_dom_for || !global_corr_dom_fx || !global_corr_for_fx)
	{
		err = "Memory allocation error (5) in SrtGrfn3DFXTree";
		goto FREE_RETURN;
	}
	...
	for(UpOrDown=0; UpOrDown<=alphaFudge; UpOrDown++)
	{
		...
		/* Fill the global expectations and variances */
		Fx3DBetaDLM_ExpectAndVarGrfn(	nstp,
										time,
										model,
										dom_fwd,
										dom_var,
		...
		for (i=nstp-1; i>=0; i--)
		{
			...
			if (sig_fx[i] < 0.0)
			{
				err = "Correlation matrix is not positive definite in SrtgrfnMainFx3fBetaDLM";
				goto FREE_RETURN;
			}
			...
			corr_dom_fx[i] = (temp_vol_dom - model->dCorrDomFor[index] * temp_vol_for 
							+ model->dCorrDomFx[index] * temp_vol_fx) / sig_fx[i];

			corr_for_fx[i] = (model->dCorrDomFor[index] * temp_vol_dom - temp_vol_for 
							+ model->dCorrForFx[index] * temp_vol_fx) / sig_fx[i];

			if (i < nstp-1)
			{
				mu_quanto_cons[i] = -corr_for_fx[i] * sig_for[i] * sig_fx[i]
		...
		for (i=nstp-1; i>=0; i--)
		{
			if (j >= 0 && fabs (date[i] - xStr.dts[j]) < 1.0e-04)
			{
				...
				grfn_prm->bond_pay_const = log(swp_f_df(xStr.dts[j], pay_date, model->cYcDom));
				grfn_prm->bond_pay_const += 0.5 * dom_beta[i] * dom_beta[i] * dom_phi[i];
				...
				grfn_prm->fx_coef_const = log(model->dCashFx * 
											swp_f_df(today, xStr.dts[j], model->cYcFor) 
											/ swp_f_df(today, xStr.dts[j], model->cYcDom));

				grfn_prm->fx_coef_const += -0.5 * (model->dB0 * model->dB0 * var_ffx / temp + log(temp));

				grfn_prm->fx_coef_const += 0.5 * (dom_beta[i] * dom_beta[i] * dom_phi[i]
												- for_beta[i] * for_beta[i] * for_phi[i]);
				...
				if (grfn_prm->num_fx_df > 0 && grfn_prm->fx_idx != -1)
				{
					...
					for (k=0; k<grfn_prm->num_fx_df; k++)
					{
						...
						beta_temp = (1.0 - exp(-model->dLambdaDom * (T - model->dTStar))) / model->dLambdaDom;
						grfn_prm->df_fx_coef_const[k] = log(swp_f_df (xStr.dts[j], grfn_prm->fx_df_dts[k], model->cYcDom));
						grfn_prm->df_fx_coef_const[k] -= 0.5 * (beta_temp * beta_temp - dom_beta[i] * dom_beta[i]) * dom_phi[i];
				...
				if (grfn_prm->num_dom_df > 0 && grfn_prm->dom_idx != -1)
				{
					...
					for (k=0; k<grfn_prm->num_dom_df; k++)
					{
						...
						beta_temp = (1.0 - exp(-model->dLambdaDom * (T - model->dTStar))) / model->dLambdaDom;
						grfn_prm->df_dom_coef_const[k] = log(swp_f_df (xStr.dts[j], grfn_prm->dom_df_dts[k], model->cYcDom));
						grfn_prm->df_dom_coef_const[k] -= 0.5 * (beta_temp * beta_temp - dom_beta[i] * dom_beta[i]) * dom_phi[i];
				...
				if (grfn_prm->num_for_df > 0 && grfn_prm->for_idx != -1)
				{
					...
					for (k=0; k<grfn_prm->num_for_df; k++)
					{
						...
						beta_temp = (1.0 - exp(-model->dLambdaFor * (T - model->dTStar))) / model->dLambdaFor;					
						grfn_prm->df_for_coef_const[k] = log(swp_f_df (xStr.dts[j], grfn_prm->for_df_dts[k], model->cYcFor));
						grfn_prm->df_for_coef_const[k] -= 0.5 * (beta_temp * beta_temp - for_beta[i] * for_beta[i]) * for_phi[i];
			...
			if ((i > 0) && (fabs (next_bar) > 1.0e-08))
			{
				next_bar += (for_fwd[i-1] - - dom_fwd[i-1]) 
					* (time[i] - time[i-1]);
			}
		}
		...
		*num_prod = xStr.num_cols;
		...
		err = tree_main_3dBetaDLM_QTStar(
						...
						bar_cl,
						is_bar,
						grfn_payoff_4_3dfxBetaDLMQTStar_Tree,
						*num_prod,
						UpOrDown? prod_val_temp:(*prod_val));
	}
	/*	Add PV of Past */
...
FREE_RETURN:
	...
	if (sig_dom) free (sig_dom);
	if (sig_for) free (sig_for);
	if (mu_quanto_cons) free (mu_quanto_cons);
	if (mu_quanto_lin) free (mu_quanto_lin);
	if (sig_fx) free (sig_fx);
	if (corr_dom_for) free(corr_dom_for);
	if (corr_dom_fx) free(corr_dom_fx);
	if (corr_for_fx) free(corr_for_fx);
		...
		for (i=0; i<nstp; i++)
		{
			if (void_prm[i])
			{
				...
				if (	grfn_prm->do_fx 
					...
					&& grfn_prm->fx_idx != -1)
				{
					if (grfn_prm->df_fx_coef_const) free_dvector (grfn_prm->df_fx_coef_const, 0, grfn_prm->num_fx_df-1);
					if (grfn_prm->df_fx_coef_lin) free_dvector (grfn_prm->df_fx_coef_lin, 0, grfn_prm->num_fx_df-1);
				}
				...
				if (	grfn_prm->do_dom
					...
					&& grfn_prm->dom_idx != -1)
				{
					if (grfn_prm->df_dom_coef_const) free_dvector (grfn_prm->df_dom_coef_const, 0, grfn_prm->num_dom_df-1);
					if (grfn_prm->df_dom_coef_lin) free_dvector (grfn_prm->df_dom_coef_lin, 0, grfn_prm->num_dom_df-1);
				}
				...
				if (	grfn_prm->do_for
					...
					&& grfn_prm->for_idx != -1)
				{
					if (grfn_prm->df_for_coef_const) free_dvector (grfn_prm->df_for_coef_const, 0, grfn_prm->num_for_df-1);
					if (grfn_prm->df_for_coef_lin) free_dvector (grfn_prm->df_for_coef_lin, 0, grfn_prm->num_for_df-1);
				}
			}
		}
						...
						double		***prod_val)
{
	...
	if (!dom_fwd || !dom_std || !dom_phi || !dom_beta ||
		!for_fwd_const || !for_fwd_lin || !for_std || !for_phi || !for_beta ||
		!fx_fwd || !fx_std || !ffx_var || !bond_pay_const || !bond_pay_lin  ||
	...
	for(UpOrDown=0; UpOrDown<=alphaFudge; UpOrDown++)
	{
		...
		for (i=xStr.num_evt-1; i>=0; i--)
		{
			...
			{			
				...
				/* Pay Bond reconstruction */			
				bond_pay_const[i+flag] = log(swp_f_df(xStr.dts[i], pay_date, model->cYcDom));
				bond_pay_const[i+flag] += 0.5 * dom_beta[i+flag] * dom_beta[i+flag] * dom_phi[i+flag];
				...
				grfn_prm->fx_coef_const = log(model->dCashFx * 
											swp_f_df(today, xStr.dts[i], model->cYcFor) 
											/ swp_f_df(today, xStr.dts[i], model->cYcDom));

				grfn_prm->fx_coef_const += -0.5 * (model->dB0 * model->dB0 * var_ffx / temp + log(temp));

				grfn_prm->fx_coef_const += 0.5 * (dom_beta[i+flag] * dom_beta[i+flag] * dom_phi[i+flag]
				...
				if (grfn_prm->num_fx_df > 0 && grfn_prm->fx_idx != -1)
				{
					...
					for (j=0; j<grfn_prm->num_fx_df; j++)
					{
						...
						beta_temp = (1.0 - exp(-model->dLambdaDom * (T - model->dTStar))) / model->dLambdaDom;
						grfn_prm->df_fx_coef_const[j] = log(swp_f_df (xStr.dts[i], grfn_prm->fx_df_dts[j], model->cYcDom));
						grfn_prm->df_fx_coef_const[j] -= 0.5 * (beta_temp * beta_temp - dom_beta[i+flag] * dom_beta[i+flag]) * dom_phi[i+flag];
				...
				if (grfn_prm->num_dom_df > 0 && grfn_prm->dom_idx != -1)
				{
					...
					for (j=0; j<grfn_prm->num_dom_df; j++)
					{
						...
						beta_temp = (1.0 - exp(-model->dLambdaDom * (T - model->dTStar))) / model->dLambdaDom;
						grfn_prm->df_dom_coef_const[j] = log(swp_f_df (xStr.dts[i], grfn_prm->dom_df_dts[j], model->cYcDom));
						grfn_prm->df_dom_coef_const[j] -= 0.5 * (beta_temp * beta_temp - dom_beta[i+flag] * dom_beta[i+flag]) * dom_phi[i+flag];
				...
				if (grfn_prm->num_for_df > 0 && grfn_prm->for_idx != -1)
				{
					...
					for (j=0; j<grfn_prm->num_for_df; j++)
					{
						...
						beta_temp = (1.0 - exp(-model->dLambdaFor * (T - model->dTStar))) / model->dLambdaFor;					
						grfn_prm->df_for_coef_const[j] = log(swp_f_df (xStr.dts[i], grfn_prm->for_df_dts[j], model->cYcFor));
						grfn_prm->df_for_coef_const[j] -= 0.5 * (beta_temp * beta_temp - for_beta[i+flag] * for_beta[i+flag]) * for_phi[i+flag];
	...
	if(alphaFudge)
	{
		...
		if (do_optimisation)
		{
			(*prod_val)[num_col][0] = 0.5 * ((*prod_val)[num_col][0] + (prod_val_temp)[num_col][0]);
			(*prod_val)[num_col][1] = 0.5 * ((*prod_val)[num_col][1] + (prod_val_temp)[num_col][1]);
		}
		...
		{		
			...
			{												
				if (do_optimisation)
				{
					...
					for (k=0; k<params->iNbIndex; k++)
					{
						(*prod_val)[i][3+k] = 0.5 * ((*prod_val)[i][3+k] + (prod_val_temp)[i][3+k]);
					}
				}
			}
		}
...
FREE_RETURN:
	...
	if (bond_pay_const) free (bond_pay_const);
	if (bond_pay_lin) free (bond_pay_lin);

	if (for_fwd_const) free (for_fwd_const);
	if (for_fwd_lin) free (for_fwd_lin);

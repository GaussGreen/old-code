...
//	const int XL_MAX_ID_LEN=1234567890;	
	...
	double compute_sum_sigma(double t,
							 double *sigma,
							 double *sigma_date,
							 int nb_sigma,
							 double lambda)
	...
	double compute_sum_diagterm(double t,
								double *sigma_1,
								double *sigma_2,
								double *sigma_date,
								int nb_sigma,
								...
								double lambda_2)
	{
		...
		while((sigma_date[i]<t)&(i<nb_sigma))
		{
			...
			res+=sigma_1[i]*sigma_2[i]*(exp((lambda_1+lambda_2)*t2)-exp((lambda_1+lambda_2)*t1))/(lambda_1+lambda_2);
			i++;
		}
		t1=t2;
		t2=t;	
		...
		res+=sigma1_temp*sigma2_temp*(exp((lambda_1+lambda_2)*t2)-exp((lambda_1+lambda_2)*t1))/(lambda_1+lambda_2);
		return res;
	}


	double compute_A(double t,
					 double _T,
					 double sum_sigma_1,
					 double sum_sigma_2,
					 double sum_diagterm,
					 ...
					 double Tpay)
	{
		...
		Ares+=-(double)(1)/2*sum_sigma_1*exp(-2*lambda_1*Tpay)*(int_up1*int_up1-int_low1*int_low1);
		Ares+=-(double)(1)/2*sum_sigma_2*exp(-2*lambda_2*Tpay)*(int_up2*int_up2-int_low2*int_low2);
		Ares+=-sum_diagterm*rho*exp(-(lambda_1+lambda_2)*Tpay)*(int_up1*int_up2-int_low1*int_low2);
		return Ares;
	}



	double compute_c1(double t,
					  double _T,
					  double Tpay,
					  double *sigma_1,
					  double *sigma_2,
					  ...
					  double lambda_2)
	{
		...
		else
		{
			...
			while((sigma_date[i]<t)&(i<nb_sigma))
			{
				...
				temp1+=sigma_1[i]*sigma_1[i]*(exp(-2*lambda_1*(Tpay-t2))-exp(-2*lambda_1*(Tpay-t1)))/(2*lambda_1);
				temp2+=sigma_2[i]*sigma_2[i]*(exp(-2*lambda_2*(Tpay-t2))-exp(-2*lambda_2*(Tpay-t1)))/(2*lambda_2);
				temp3+=sigma_1[i]*sigma_2[i]*(exp(-(lambda_1+lambda_2)*(Tpay-t2))-exp(-(lambda_1+lambda_2)*(Tpay-t1)))/(lambda_1+lambda_2);
				i++;
			}
			t1=t2;
			t2=t;	
			...
			temp1+=sigma1_temp*sigma1_temp*(exp(-2*lambda_1*(Tpay-t2))-exp(-2*lambda_1*(Tpay-t1)))/(2*lambda_1);
			temp2+=sigma2_temp*sigma2_temp*(exp(-2*lambda_2*(Tpay-t2))-exp(-2*lambda_2*(Tpay-t1)))/(2*lambda_2);
			temp3+=sigma1_temp*sigma2_temp*(exp(-(lambda_1+lambda_2)*(Tpay-t2))-exp(-(lambda_1+lambda_2)*(Tpay-t1)))/(lambda_1+lambda_2);
					  ...
					  double lambda_2)
	{
		...
		else
		{
			...
			while((sigma_date[i]<t)&(i<nb_sigma))
			{
				...
				temp+=sigma_2[i]*sigma_2[i]*(exp(-2*lambda_2*(Tpay-t2))-exp(-2*lambda_2*(Tpay-t1)))/(2*lambda_2);
				i++;
			}
			t1=t2;
			t2=t;	
			...
			temp=temp+sigma2_temp*sigma2_temp*(exp(-2*lambda_2*(Tpay-t2))-exp(-2*lambda_2*(Tpay-t1)))/(2*lambda_2);
			temp=(1-rho*rho)*temp;
			res=sqrt(temp);
	...
	double compute_CMS_GaussLeg(double Tf,
								double Tpay,
								double *DF_fix,
								...
								double strike)
	{
		...
		sum_diagterm=compute_sum_diagterm(Tf,sigma_1,sigma_2,sigma_date,nb_sigma,lambda_1,lambda_2);
		
		for(i=0;i<nb_float_coupon;i++)
		{
			Afloat[i]=compute_A(Tf,float_coupon_date[i],sum_sigma_1,sum_sigma_2,sum_diagterm,rho,lambda_1,lambda_2,Tpay);
			c1float[i]=compute_c1(Tf,float_coupon_date[i],Tpay,sigma_1,sigma_2,sigma_date,nb_sigma,rho,lambda_1,lambda_2);
			c2float[i]=compute_c2(Tf,float_coupon_date[i],Tpay,sigma_2,sigma_date,nb_sigma,rho,lambda_2);
		}
		for(i=0;i<nb_fix_coupon;i++)
		{
			Afix[i]=compute_A(Tf,fix_coupon_date[i],sum_sigma_1,sum_sigma_2,sum_diagterm,rho,lambda_1,lambda_2,Tpay);
			c1fix[i]=compute_c1(Tf,fix_coupon_date[i],Tpay,sigma_1,sigma_2,sigma_date,nb_sigma,rho,lambda_1,lambda_2);
			c2fix[i]=compute_c2(Tf,fix_coupon_date[i],Tpay,sigma_2,sigma_date,nb_sigma,rho,lambda_2);
		}
		...
		for(l=0;l<nb_integration;l++)
		{
			...
			for(k=0;k<nb_integration;k++)
			{
				...
				{
					ZC_float[i+1]=DF_float[i+1]/DF_TF*exp(Afloat[i+1]+c1float[i+1]*y1+c2float[i+1]*y2);
					coupon_float[i]=((double)(1)/float_cvg[i+1]*(ZC_float[i]/ZC_float[i+1]-1)+spread[i])*float_cvg[i+1];
				...
				while((i<nb_float_coupon-1)&(j<nb_fix_coupon))
				{
					...
					else if((float_coupon_date[i]<fix_coupon_date[j])&(fix_coupon_date[j]<float_coupon_date[i+1]))
					{
						ZC_fix[j]=DF_fix[j]/DF_TF*exp(Afix[j]+c1fix[j]*y1+c2fix[j]*y2);
						j++;
					}
		...
		if(Afloat) free(Afloat);
		if(Afix) free(Afix);
		if(c1float) free(c1float);
		if(c2float) free(c2float);
		if(c1fix) free(c1fix);
		if(c2fix) free(c2fix);
		if(ZC_float) free(ZC_float);
		if(ZC_fix) free(ZC_fix);
		if(coupon_float) free(coupon_float);
		if(x) free(x);
		if(w) free(w);
	}


	double compute_CMScov_GL(double Tf1,
								double Tpay1,
								double *DF_fix1,
								...
								)
	{
		...
		sum_diagterm1=compute_sum_diagterm(Tf1,sigma_11,sigma_21,sigma_date1,nb_sigma1,lambda_11,lambda_21);
		...
		sum_diagterm2=compute_sum_diagterm(Tf2,sigma_12,sigma_22,sigma_date2,nb_sigma2,lambda_12,lambda_22);

		for(i=0;i<nb_float_coupon1;i++)
		{
			Afloat1[i]=compute_A(Tf1,float_coupon_date1[i],sum_sigma_11,sum_sigma_21,sum_diagterm1,rho1,lambda_11,lambda_21,Tpay1);
			c1float1[i]=compute_c1(Tf1,float_coupon_date1[i],Tpay1,sigma_11,sigma_21,sigma_date1,nb_sigma1,rho1,lambda_11,lambda_21);
			c2float1[i]=compute_c2(Tf1,float_coupon_date1[i],Tpay1,sigma_21,sigma_date1,nb_sigma1,rho1,lambda_21);
		}
		for(i=0;i<nb_fix_coupon1;i++)
		{
			Afix1[i]=compute_A(Tf1,fix_coupon_date1[i],sum_sigma_11,sum_sigma_21,sum_diagterm1,rho1,lambda_11,lambda_21,Tpay1);
			c1fix1[i]=compute_c1(Tf1,fix_coupon_date1[i],Tpay1,sigma_11,sigma_21,sigma_date1,nb_sigma1,rho1,lambda_11,lambda_21);
			c2fix1[i]=compute_c2(Tf1,fix_coupon_date1[i],Tpay1,sigma_21,sigma_date1,nb_sigma1,rho1,lambda_21);
		}

		for(i=0;i<nb_float_coupon2;i++)
		{
			Afloat2[i]=compute_A(Tf2,float_coupon_date2[i],sum_sigma_12,sum_sigma_22,sum_diagterm2,rho2,lambda_12,lambda_22,Tpay2);
			c1float2[i]=compute_c1(Tf2,float_coupon_date2[i],Tpay2,sigma_12,sigma_22,sigma_date2,nb_sigma2,rho2,lambda_12,lambda_22);
			c2float2[i]=compute_c2(Tf2,float_coupon_date2[i],Tpay2,sigma_22,sigma_date2,nb_sigma2,rho2,lambda_22);
		}
		for(i=0;i<nb_fix_coupon2;i++)
		{
			Afix2[i]=compute_A(Tf2,fix_coupon_date2[i],sum_sigma_12,sum_sigma_22,sum_diagterm2,rho2,lambda_12,lambda_22,Tpay2);
			c1fix2[i]=compute_c1(Tf2,fix_coupon_date2[i],Tpay2,sigma_12,sigma_22,sigma_date2,nb_sigma2,rho2,lambda_12,lambda_22);
			c2fix2[i]=compute_c2(Tf2,fix_coupon_date2[i],Tpay2,sigma_22,sigma_date2,nb_sigma2,rho2,lambda_22);
		}
		...
		for(l=0;l<nb_integration;l++)
		{
			...
			for(k=0;k<nb_integration;k++)
			{
				...
				swap_num2=0;
				swap_den2=0;
				ZC_float1[0]=DF_float1[0]/DF_TF1*exp(Afloat1[0]+c1float1[0]*y1+c2float1[0]*y2);
				ZC_float2[0]=DF_float2[0]/DF_TF2*exp(Afloat2[0]+c1float2[0]*y1+c2float2[0]*y2);

				for(i=0;i<nb_float_coupon1-1;i++)
				{
					ZC_float1[i+1]=DF_float1[i+1]/DF_TF1*exp(Afloat1[i+1]+c1float1[i+1]*y1+c2float1[i+1]*y2);
					coupon_float1[i]=((double)(1)/float_cvg1[i+1]*(ZC_float1[i]/ZC_float1[i+1]-1)+spread1[i])*float_cvg1[i+1];
					swap_num1+=coupon_float1[i]*ZC_float1[i+1];
				}
				for(i=0;i<nb_float_coupon2-1;i++)
				{
					ZC_float2[i+1]=DF_float2[i+1]/DF_TF2*exp(Afloat2[i+1]+c1float2[i+1]*y1+c2float2[i+1]*y2);
					coupon_float2[i]=((double)(1)/float_cvg2[i+1]*(ZC_float2[i]/ZC_float2[i+1]-1)+spread2[i])*float_cvg2[i+1];
		...
		if(Afloat1) free(Afloat1);
		if(Afix1) free(Afix1);
		if(c1float1) free(c1float1);
		if(c2float1) free(c2float1);
		if(c1fix1) free(c1fix1);
		if(c2fix1) free(c2fix1);
		if(ZC_float1) free(ZC_float1);
		if(ZC_fix1) free(ZC_fix1);
		if(coupon_float1) free(coupon_float1);
		if(Afloat2) free(Afloat2);
		if(Afix2) free(Afix2);
		if(c1float2) free(c1float2);
		if(c2float2) free(c2float2);
		if(c1fix2) free(c1fix2);
		if(c2fix2) free(c2fix2);
		if(ZC_float2) free(ZC_float2);
		if(ZC_fix2) free(ZC_fix2);
		if(coupon_float2) free(coupon_float2);
...
Err compute_CMScorrel_GL(double Tf1,
				    	 double Tpay1,
						 double *DF_fix1,
						 double *fix_coupon_date1,
						 double *fix_cvg1,//cvg[0]=0, the same size as fix_coupon_date
						 ...
						 )
{
	...
	{		
		...
		for(k=0;k<nb_integration;k++)
		{
			...
			for(i=0;i<nb_float_coupon1-1;i++)
			{
				ZC_float1[i+1] = exp(Afloat1[i+1]+c1float1[i+1]*y1+c2float1[i+1]*y2);
				coupon_float1[i] = ZC_float1[i] - ZC_float1[i+1] + spread1[i] * float_cvg1[i+1] * ZC_float1[i+1];
				swap_num1 += coupon_float1[i];
			}

			for(i=0;i<nb_float_coupon2-1;i++)
			{
				ZC_float2[i+1] = exp(Afloat2[i+1] + c1float2[i+1]*y1+c2float2[i+1]*y2);
				coupon_float2[i] = ZC_float2[i] - ZC_float2[i+1] + spread2[i] * float_cvg2[i+1] * ZC_float2[i+1];
	...
	(*output)[0] = (cov - cms1 * cms2) / (sqrt(varcms1 - cms1 * cms1) * sqrt(varcms2 - cms2 * cms2));
...
Err LGM2FCMS(
  ...
  char			*ProductTypeStr,
  double		strike,
  char *(GetCpdAutocalCashVol)(char* lpszVolCurveName, double dStart, double dEnd, double dStrike,
						  int bIsACap, char* lpszRefRateCode, double* pdVol, double* pdPower),
  double		*price)
{
	Err err=NULL;
...
FREE_RETURN:
		...
		if(float_coupon_date) free(float_coupon_date);
		if(fix_coupon_date) free(fix_coupon_date);
		if(DF_float) free(DF_float);
		if(DF_fix) free(DF_fix);
		if(float_cvg) free(float_cvg);
		if(spread) free(spread);
		if(fix_cvg) free(fix_cvg);
		if(sigma_1) free(sigma_1);
		if(sigma_2) free(sigma_2);

		if(sigma_date)free (sigma_date);
		if(sigma) free (sigma);
		if(nb_UNO) free (nb_UNO);
		if(primary_ex_date1) free(primary_ex_date1);
		if(primary_strike1) free (primary_strike1);
		if(end_tenor_primary1) free (end_tenor_primary1);
		if(secundary_strike) free (secundary_strike);
		if(secundary_ex_date) free (secundary_ex_date);
		if(end_tenor_secundary) free (end_tenor_secundary);
		if(lam_time1) free (lam_time1);
		if(lam1) free (lam1);							
		if(lam_shift1) free (lam_shift1);
		if(lam2) free (lam2);
		if(tau_date) free (tau_date);
		if(tau) free (tau);

		if(secundary_freq) free (secundary_freq);
		if(secundary_basis) free (secundary_basis);
  ...
  //1-CMS(Tp)-CMS(Ts) / 2-CMS(Tp) / 3-CMS(Ts)
{
	...
	return err;
	if(ProductType_loc) free(ProductType_loc);
}
...
Err LGM2FCMSCorrel(
  ...
  char *(GetCpdAutocalCashVol)(char* lpszVolCurveName, double dStart, double dEnd, double dStrike,
						  int bIsACap, char* lpszRefRateCode, double* pdVol, double* pdPower),
  double		**output)//dimension=7: OUTPUT:1-Correl / 2-CMS1 / 3-CMS2 / 4-VAR(CMS1) / 5-VAR(CMS2) / 6-sigma1 / 7-lambda1
{
	Err err=NULL;
	int	sigma_n;
	double *sigma_date=NULL;					
...
/* Gets Today from underlying */
	...
	err = LGM2FCMSCorrelBuild(yc_name,
							  und_name,
							  lTfixing,
							  lStartDate,
							  lEndDate1,
...
Err LGM2FCMSCorrelBuild(char *yc_name,
						char *und,
						long lTfixing,
						long lStartDate,
						long lEndDate1,
						...
						double **output)
{
	...
	long *float_fixing_dates1=NULL, *float_start_dates1=NULL, *float_end_dates1=NULL, *float_pay_dates1=NULL;
	...
	long *float_fixing_dates2=NULL, *float_start_dates2=NULL, *float_end_dates2=NULL, *float_pay_dates2=NULL;
...
//----------------------------------------------------------------------------
	...
	err = swp_f_make_FloatLegDatesCoveragesAndSpreads(&swapdp1, lToday, refRateName, 
															&float_pay_dates1,
															&float_nb_pay_dates1, 
															&float_fixing_dates1, 
															&float_start_dates1,
	...
	err = swp_f_make_FloatLegDatesCoveragesAndSpreads(&swapdp2, lToday, refRateName, 
															&float_pay_dates2,
															&float_nb_pay_dates2, 
															&float_fixing_dates2, 
															&float_start_dates2,
	...
	err = swp_f_make_FixedLegDatesAndCoverages(&swapdp1, lToday, 
												&fix_pay_dates1, 
												&fix_nb_pay_dates1, 
												&fix_start_dates1, 
												&fix_end_dates1, 
	...
	err = swp_f_make_FixedLegDatesAndCoverages(&swapdp2, lToday, 
												&fix_pay_dates2, 
												&fix_nb_pay_dates2, 
												&fix_start_dates2, 
												&fix_end_dates2, 
...
FREE_RETURN:
		...
		if(float_coupon_date1) free(float_coupon_date1);
		if(fix_coupon_date1) free(fix_coupon_date1);
		if(DF_float1) free(DF_float1);
		if(DF_fix1) free(DF_fix1);
		if(float_cvg1) free(float_cvg1);
		if(spread1) free(spread1);
		if(fix_cvg1) free(fix_cvg1);

		if(float_fixing_dates1) free(float_fixing_dates1);
		if(float_start_dates1) free(float_start_dates1);
		if(float_end_dates1) free(float_end_dates1);
		if(float_pay_dates1) free(float_pay_dates1);
	
		if(float_cvgs1) free(float_cvgs1);
		if(float_spreads1) free(float_spreads1);
		if(float_pay_times1) free_dvector(float_pay_times1, 0, float_nb_pay_dates1-1);

		if(fix_start_dates1) free(fix_start_dates1);
		if(fix_end_dates1) free(fix_end_dates1);
		if(fix_pay_dates1) free(fix_pay_dates1);
		if(fix_pay_times1) free_dvector(fix_pay_times1, 0, fix_nb_pay_dates1-1);
		if(fix_cvgs1) free(fix_cvgs1);
		
		if(float_coupon_date2) free(float_coupon_date2);
		if(fix_coupon_date2) free(fix_coupon_date2);
		if(DF_float2) free(DF_float2);
		if(DF_fix2) free(DF_fix2);
		if(float_cvg2) free(float_cvg2);
		if(spread2) free(spread2);
		if(fix_cvg2) free(fix_cvg2);

		if(float_fixing_dates2) free(float_fixing_dates2);
		if(float_start_dates2) free(float_start_dates2);
		if(float_end_dates2) free(float_end_dates2);
		if(float_pay_dates2) free(float_pay_dates2);
	
		if(float_cvgs2) free(float_cvgs2);
		if(float_spreads2) free(float_spreads2);
		if(float_pay_times2) free_dvector(float_pay_times2, 0, float_nb_pay_dates2-1);

		if(fix_start_dates2) free(fix_start_dates2);
		if(fix_end_dates2) free(fix_end_dates2);
		if(fix_pay_dates2) free(fix_pay_dates2);
		if(fix_pay_times2) free_dvector(fix_pay_times2, 0, fix_nb_pay_dates2-1);
		if(fix_cvgs2) free(fix_cvgs2);

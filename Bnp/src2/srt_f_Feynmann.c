    ...
    double** U)
{
    ...
    if (pde_bound_cond == DIRICHLET) /* FIRST CASE: THE BOUDARY CONDITIONS ARE DIRICHLET ONES */
    {
        ...
        for (i = 1; i <= num_time_step; i++) /* LOOP ON THE TIME STEP */
        {
            ...
            {
                diag_A[j] =
                    -PDE_Param[2] * conv_mat[i][j] + PDE_Param[0] * diff_mat[i][j] * diff_mat[i][j];
                    ...
                    vect_R[j] =
                        (PDE_Param[1] * diff_mat[i - 1][j] * diff_mat[i - 1][j] +
                         PDE_Param[3] * conv_mat[i - 1][j]) *
                            PrevPrePtr[j + 1] -
                        (1.0 + 2.0 * PDE_Param[1] * diff_mat[i - 1][j] * diff_mat[i - 1][j]) *
                            PrevPrePtr[j] +
                        (PDE_Param[1] * diff_mat[i - 1][j] * diff_mat[i - 1][j] -
                         PDE_Param[3] * conv_mat[i - 1][j]) *
                            pde_bound[0] -
    ...
    double* bs_pv)
{
    ...
    double YrtoExpDate, X0, Xmin, Xmax, dXmax,
        time_step = MAX_TIME, dX, *X = NULL, *U = NULL, **diff_mat = NULL, **conv_mat = NULL,
        **div_mat = NULL, *term_payoff = NULL, pde_bound[2], drift;

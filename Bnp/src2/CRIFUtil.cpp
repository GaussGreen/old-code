...
{	
	...
	if (!sMCArg->dDates || !sMCArg->dSigma || !sMCArg->dAlphaSV || !sMCArg->dLambdaSV || !sMCArg->dLevelSV || 
		!sMCArg->dRhoSV || !sMCArg->dRho2SV || !sMCArg->dLGMAlpha || !sMCArg->dLGMRho || !sMCArg->iEvalEvent || 
		!sMCArg->iIndexEvent || !sMCArg->vPayoffParams || !sMCArg->sMCEBParams || !sMCArg->iOptimise || !sMCArg->dMarketValue)
	{
...
static double crif_beta_func(	long		lStartDate,
								long		lEndDate,
								double		dLambda)
{
	return ((1.0 - exp(-dLambda * (lEndDate - lStartDate) * YEARS_IN_DAY)) / dLambda);
}


Err crif_fill_eval_const_coupon(	CMSCTS_MARKET		sMarket,
								CRIF_DEAL			sDeal,
								LGMSV_MODEL			sModel,
								int					iIndexCoupon,
								CRIF_COUPONARG	sCouponArg)
{
	...
	sCouponArg->tpay_tstar_alpha = swp_f_df(sModel->lTStarDate, sExotic->lPayDate[iIndexCoupon], sMarket->cYcName);
	sCouponArg->tpay_tstar_alpha = -log(sCouponArg->tpay_tstar_alpha);
	sCouponArg->tpay_tstar_beta = crif_beta_func(sModel->lTStarDate, sExotic->lPayDate[iIndexCoupon], sModel->dLambdaX);
	sCouponArg->tpay_tstar_gamma = 0.5 * sCouponArg->tpay_tstar_beta * sCouponArg->tpay_tstar_beta;
	
	if (sModel->iOne2F == 2)
	{
		sCouponArg->tpay_tstar_beta_2 = crif_beta_func(sModel->lTStarDate, sExotic->lPayDate[iIndexCoupon], sModel->dLambdaX2);
		sCouponArg->tpay_tstar_gamma_2 = 0.5 * sCouponArg->tpay_tstar_beta_2 * sCouponArg->tpay_tstar_beta_2;
		sCouponArg->tpay_tstar_gamma_12 = sCouponArg->tpay_tstar_beta * sCouponArg->tpay_tstar_beta_2;
	}
	...
	if (1.0 > 0)
	{
		...
		lStartDate = add_unit(lFixingDate, sExotic->iFixingLag, SRT_BDAY, MODIFIED_SUCCEEDING);
		
		if (fabs(sExotic->dPaidGearing[iIndexCoupon]) > 0)
		{
			/* Get the schedule */
			err = add_tenor(lStartDate, sExotic->cPaidTenor[iIndexCoupon], NO_BUSDAY_CONVENTION, &lTheoEndDate);
			if (err) goto FREE_RETURN;
			
			err = swp_f_initSwapDP(	lStartDate,
									lTheoEndDate,
									sExotic->cPaidFreq,
									sExotic->cPaidBasis,
									&sFixedSwap);
			...
			{
				dLevel += coverage(sFixedSwapDates.date[j-1], sFixedSwapDates.date[j], sFixedSwap.basis_code)
					* swp_f_df(sMarket->lToday, sFixedSwapDates.date[j], sMarket->cYcName);
			}
			
			dFloating = swp_f_df(sMarket->lToday, sFixedSwapDates.date[0], sMarket->cYcName) - swp_f_df(sMarket->lToday, sFixedSwapDates.date[sFixedSwapDates.len-1], sMarket->cYcName);
			...
			for (j=0; j<sFixedSwapDates.len; j++)
			{
				...
				if (j > 0)
				{
					sCouponArg->dCMSCoverage[j] = coverage(sFixedSwapDates.date[j-1], sFixedSwapDates.date[j], sFixedSwap.basis_code);
				}
			}
		...
		for (i=0; i<1; i++)
		{
			...
			{
				while (dAllDFDates[iIndexDF] < AllSchedules[i][j] - 0.5) iIndexDF++;
		...
		for (i=0; i<sCouponArg->iNbDF; i++)
		{
			...
			sCouponArg->tdf_tstar_alpha[i] = swp_f_df(sModel->lTStarDate, lPayDate, sMarket->cYcName);
			sCouponArg->tdf_tstar_alpha[i] = -log(sCouponArg->tdf_tstar_alpha[i]);
			sCouponArg->tdf_tstar_beta[i] = crif_beta_func(sModel->lTStarDate, lPayDate, sModel->dLambdaX);
			sCouponArg->tdf_tstar_gamma[i] = 0.5 * sCouponArg->tdf_tstar_beta[i] * sCouponArg->tdf_tstar_beta[i];
			
			if (sModel->iOne2F == 2)
			{
				sCouponArg->tdf_tstar_beta_2[i] = crif_beta_func(sModel->lTStarDate, lPayDate, sModel->dLambdaX2);
				sCouponArg->tdf_tstar_gamma_2[i] = 0.5 * sCouponArg->tdf_tstar_beta_2[i] * sCouponArg->tdf_tstar_beta_2[i];
				sCouponArg->tdf_tstar_gamma_12[i] = sCouponArg->tdf_tstar_beta[i] * sCouponArg->tdf_tstar_beta_2[i];
			}
		}
	}
...
FREE_RETURN:
	...
	if (dAllDFDates) free(dAllDFDates);
	if (AllSchedules)
	{
		for (i=0; i<1; i++)
		{
			if (AllSchedules[i]) free(AllSchedules[i]);
		}
...
Err crif_fill_eval_const_call(	CMSCTS_MARKET				sMarket,
								CRIF_DEAL				sDeal,
								LGMSV_MODEL				sModel,
								int						iIndexCall,
								int						iLastFundIdx,
								CRIF_CALLARG			sCallArg)
{
	...
	for (i=0; i<=sCallArg->iNbFundCoupon; i++)
	{
		...
		sCallArg->tpay_tstar_alpha[i] = swp_f_df(sModel->lTStarDate, lPayDate, sMarket->cYcName);
		sCallArg->tpay_tstar_alpha[i] = -log(sCallArg->tpay_tstar_alpha[i]);
		sCallArg->tpay_tstar_beta[i] = crif_beta_func(sModel->lTStarDate, lPayDate, sModel->dLambdaX);
		sCallArg->tpay_tstar_gamma[i] = 0.5 * sCallArg->tpay_tstar_beta[i] * sCallArg->tpay_tstar_beta[i];
		
		if (sModel->iOne2F == 2)
		{
			sCallArg->tpay_tstar_beta_2[i] = crif_beta_func(sModel->lTStarDate, lPayDate, sModel->dLambdaX2);
			sCallArg->tpay_tstar_gamma_2[i] = 0.5 * sCallArg->tpay_tstar_beta_2[i] * sCallArg->tpay_tstar_beta_2[i];
			sCallArg->tpay_tstar_gamma_12[i] = sCallArg->tpay_tstar_beta[i] * sCallArg->tpay_tstar_beta_2[i];
		}
	}	
	
	/* reconstruction of DF(t,Tset) / DF(t,Tstar) */
	sCallArg->tset_tstar_alpha = swp_f_df(sModel->lTStarDate, sCall->lSettlDate[iIndexCall], sMarket->cYcName);
	sCallArg->tset_tstar_alpha = -log(sCallArg->tset_tstar_alpha);
	sCallArg->tset_tstar_beta = crif_beta_func(sModel->lTStarDate, sCall->lSettlDate[iIndexCall], sModel->dLambdaX);
	sCallArg->tset_tstar_gamma = 0.5 * sCallArg->tset_tstar_beta * sCallArg->tset_tstar_beta;
	
	if (sModel->iOne2F == 2)
	{
		sCallArg->tset_tstar_beta_2 = crif_beta_func(sModel->lTStarDate, sCall->lSettlDate[iIndexCall], sModel->dLambdaX2);
		sCallArg->tset_tstar_gamma_2 = 0.5 * sCallArg->tset_tstar_beta_2 * sCallArg->tset_tstar_beta_2;
		sCallArg->tset_tstar_gamma_12 = sCallArg->tset_tstar_beta * sCallArg->tset_tstar_beta_2;
	}
	
	sCallArg->mceb_sCouponArg  = calloc(1, sizeof(crif_couponarg));

	err = crif_fill_eval_const_coupon(		sMarket,
											sDeal,
											sModel,
...
Err crif_fill_mc_arg(	CMSCTS_MARKET				sMarket,
						CRIF_DEAL				sDeal,
						...
						CRIF_PRICING_PARAMS		sPricingParams)
{
	...
	{
		if (sExotic->eCouponType[i] == REGULAR_CRIF && sExotic->lPaidFixingDate[i] >= sMarket->lToday + sMarket->iEODFixFlag)
		{
			/* Add Fixing of Paid CMS */
			dAllTimes[lNbAllTimes] = (sExotic->lPaidFixingDate[i] - sMarket->lToday) * YEARS_IN_DAY;
			lNbAllTimes++;
		}
	}
	...
	if (!sCall->iIsExercised)
	{
		...
		{
			dAllTimes[lNbAllTimes] = (sCall->lExeDate[i] - sMarket->lToday) * YEARS_IN_DAY;
			lNbAllTimes++;
		}
	}
	...
	err = fill_time_vector_max_time(lNbAllTimes,
									dNewAllTimes,
	...
	for (i=iIndexCoupon; i<sExotic->iNbCpn; i++)
	{
		...
		{
			sMCArg->dPathInfos = calloc(sPricingParams->sSimulParams->lNbPaths+1, sizeof(double));
			...
			/* Initialisation */
			if (sExotic->lPaidFixingDate[i] < sMarket->lToday + sMarket->iEODFixFlag)
			{
	...
	for (i=iIndexCoupon; i<sExotic->iNbCpn; i++)
	{
		if (sExotic->eCouponType[i] == REGULAR_CRIF)
		{
			sMCArg->dSavedCoupon = calloc(sPricingParams->sSimulParams->lNbPaths+1, sizeof(double));
			sMCArg->dSavedDF = calloc(sPricingParams->sSimulParams->lNbPaths+1, sizeof(double));
	...
	{
		/* Initialisation */
		sMCArg->dDates[i] = (long) (sMarket->lToday + sMCArg->dTimes[i] * DAYS_IN_YEAR + 0.5);		
			...
			fabs(sMCArg->dDates[i] - sCall->lExeDate[iIndexCall]) < 0.5)
		{
			...
			while (iLastFundIndex < iLastFundIndexCall && sFundingAux->iFundingControll[iLastFundIndex] != 2)
			{
				iLastFundIndex++;
			}			
			
			err = crif_fill_eval_const_call(	sMarket,
												sDeal,
												sModel,
												iIndexCall,
												iLastFundIndex,
			...
			{
				sMCArg->dMarketValue[i] += -sDeal->dPayRec * sFundingAux->dMarketValue[j];
			}
		...
		if (iIndexCoupon < sExotic->iNbCpn && 
			...
			fabs(sMCArg->dDates[i] - sExotic->lPaidFixingDate[iIndexCoupon]) < 0.5)
		{
			sPayArg->iIsCoupon = 1;
			...
			if (sExotic->eCouponType[iIndexCoupon] == REGULAR_CRIF)
			{
				sMCArg->dMarketValue[i] += sDeal->dPayRec * sExoticAux->dMarketValue[iIndexCoupon];
...
FREE_RETURN:
	...
	if (dNewAllTimes) free(dNewAllTimes);
				   ...
				   int			*stop_path)
{
	...
	if (sPayArg->iIsCall)
	{
		...
		for (i=0; i<sCallArg->iNbFundCoupon; i++)
		{
			DFR_pay = exp(-sCallArg->tpay_tstar_alpha[i] - sCallArg->tpay_tstar_beta[i] * ft - sCallArg->tpay_tstar_gamma[i] * psi);
			...
			{					
				...
				{
					dPartialFunding += sFundingAux->dCouponPlusEx[iStartFundIdx+i] * DFR_pay;
				}
				else
				{					
					dPartialFunding += sFundingAux->dCouponPlusExPartial[iStartFundIdx+i] * DFR_pay;
		...
		{		
			...
			{
				sCallArg->mceb_sCouponArg->dAllDF[i] = exp(-sCallArg->mceb_sCouponArg->tdf_tstar_alpha[i] - sCallArg->mceb_sCouponArg->tdf_tstar_beta[i] * ft - sCallArg->mceb_sCouponArg->tdf_tstar_gamma[i] * psi);
			}
			...
			for (i=0; i<1; i++)
			{
				if (sCallArg->mceb_sCouponArg->iNbCMSDF > 0)
				{
					...
					{
						dLevel += sCallArg->mceb_sCouponArg->dCMSCoverage[j] * sCallArg->mceb_sCouponArg->dAllDF[sCallArg->mceb_sCouponArg->iIndexCMSDF[j]];
					}

					dFloat = sCallArg->mceb_sCouponArg->dAllDF[sCallArg->mceb_sCouponArg->iIndexCMSDF[0]] - sCallArg->mceb_sCouponArg->dAllDF[sCallArg->mceb_sCouponArg->iIndexCMSDF[sCallArg->mceb_sCouponArg->iNbCMSDF-1]];
			...
			switch (sExotic->eCouponType[sCallArg->mceb_sCouponArg->iIndexCoupon])
			{
				case REGULAR_CRIF:
				{
					dCoupon = sCallArg->mceb_sCouponArg->dAlpha * sMCArg ->dSavedCoupon[path_index] + sCallArg->mceb_sCouponArg->dGamma  * dBasket + sCallArg->mceb_sCouponArg->dBeta;

					dCoupon = DMIN(DMAX(dCoupon, sCallArg->mceb_sCouponArg->dFloor),sCallArg->mceb_sCouponArg->dCap);
					DFR_pay = exp(-sCallArg->mceb_sCouponArg->tpay_tstar_alpha - sCallArg->mceb_sCouponArg->tpay_tstar_beta * ft - sCallArg->mceb_sCouponArg->tpay_tstar_gamma * psi);
					break;
				}
			}
			...
			{
				prod_val[4] = dCoupon  * DFR_pay * sCallArg->mceb_sCouponArg->dCoef;	// 1: le next coupon
		...
		if (sMCArg->iHasCallFee)
		{
			...
			DFR_set = exp(-sCallArg->tset_tstar_alpha - sCallArg->tset_tstar_beta * ft - sCallArg->tset_tstar_gamma * psi);
	...
	{		
		...
		for (i=0; i<sCouponArg->iNbDF; i++)
		{
			sCouponArg->dAllDF[i] = exp(-sCouponArg->tdf_tstar_alpha[i] - sCouponArg->tdf_tstar_beta[i] * ft - sCouponArg->tdf_tstar_gamma[i] * psi);
		}
		...
		for (i=0; i<1; i++)
		{
			if (sCouponArg->iNbCMSDF > 0)
			{
				...
				{
					dLevel += sCouponArg->dCMSCoverage[j] * sCouponArg->dAllDF[sCouponArg->iIndexCMSDF[j]];
				}

				dFloat = sCouponArg->dAllDF[sCouponArg->iIndexCMSDF[0]] - sCouponArg->dAllDF[sCouponArg->iIndexCMSDF[sCouponArg->iNbCMSDF-1]];
		...
		switch (sExotic->eCouponType[sCouponArg->iIndexCoupon])
		{
			case REGULAR_CRIF:
			{
				dCoupon = sCouponArg->dAlpha * sMCArg ->dSavedCoupon[path_index] + sCouponArg->dGamma  * dBasket + sCouponArg->dBeta;

				dCoupon = DMIN(DMAX(dCoupon, sCouponArg->dFloor),sCouponArg->dCap);
				sMCArg ->dSavedCoupon[path_index] = dCoupon;
				DFR_pay = exp(-sCouponArg->tpay_tstar_alpha - sCouponArg->tpay_tstar_beta * ft - sCouponArg->tpay_tstar_gamma * psi);
				   ...
				   int			*stop_path)
{
	...
	if (sPayArg->iIsCall)
	{
		...
		for (i=0; i<sCallArg->iNbFundCoupon; i++)
		{
			DFR_pay = exp(-sCallArg->tpay_tstar_alpha[i] - sCallArg->tpay_tstar_beta[i] * ft1 - sCallArg->tpay_tstar_beta_2[i] * ft2
				- sCallArg->tpay_tstar_gamma[i] * psi1 - sCallArg->tpay_tstar_gamma_2[i] * psi2 - sCallArg->tpay_tstar_gamma_12[i] * psi12);
		...
		{		
			...
			for (i=0; i<sCallArg->mceb_sCouponArg->iNbDF; i++)
			{
				sCallArg->mceb_sCouponArg->dAllDF[i] = exp(-sCallArg->mceb_sCouponArg->tdf_tstar_alpha[i] - sCallArg->mceb_sCouponArg->tdf_tstar_beta[i] * ft1 - sCallArg->mceb_sCouponArg->tdf_tstar_beta_2[i] * ft2 
					- sCallArg->mceb_sCouponArg->tdf_tstar_gamma[i] * psi1 - sCallArg->mceb_sCouponArg->tdf_tstar_gamma_2[i] * psi2 - sCallArg->mceb_sCouponArg->tdf_tstar_gamma_12[i] * psi12);
			}
			...
			switch (sExotic->eCouponType[sCallArg->mceb_sCouponArg->iIndexCoupon])
			{
				case REGULAR_CRIF:
				{
					...
					dCoupon = DMIN(DMAX(dCoupon, sCallArg->mceb_sCouponArg->dFloor),sCallArg->mceb_sCouponArg->dCap);
					DFR_pay = exp(-sCallArg->mceb_sCouponArg->tpay_tstar_alpha - sCallArg->mceb_sCouponArg->tpay_tstar_beta * ft1 - sCallArg->mceb_sCouponArg->tpay_tstar_beta_2 * ft2 
						- sCallArg->mceb_sCouponArg->tpay_tstar_gamma * psi1 - sCallArg->mceb_sCouponArg->tpay_tstar_gamma_2 * psi2 - sCallArg->mceb_sCouponArg->tpay_tstar_gamma_12 * psi12);
			...
			prod_val[5] = dCMS ;	// 2: le CMS
			prod_val[6] = (sCall->dPayRec * dPartialFunding -  dCoupon  * DFR_pay * sCallArg->mceb_sCouponArg->dCoef ) * (sCall->dPayRec * dPartialFunding -  dCoupon  * DFR_pay * sCallArg->mceb_sCouponArg->dCoef); // 3: le current coupon
	
			/* then calculate the fee */
			if (sMCArg->iHasCallFee)
			{
				DFR_set = exp(-sCallArg->tset_tstar_alpha - sCallArg->tset_tstar_beta * ft1 - sCallArg->tset_tstar_beta_2 * ft2
				- sCallArg->tset_tstar_gamma * psi1 - sCallArg->tset_tstar_gamma_2 * psi2 - sCallArg->tset_tstar_gamma_12 * psi12);
...
/*		if (sCallArg->iNbFundCoupon > sCallArg->iNbFundCouponPartial)
	...
	{		
		...
		for (i=0; i<sCouponArg->iNbDF; i++)
		{
			sCouponArg->dAllDF[i] = exp(-sCouponArg->tdf_tstar_alpha[i] - sCouponArg->tdf_tstar_beta[i] * ft1 - sCouponArg->tdf_tstar_beta_2[i] * ft2 
				- sCouponArg->tdf_tstar_gamma[i] * psi1 - sCouponArg->tdf_tstar_gamma_2[i] * psi2 - sCouponArg->tdf_tstar_gamma_12[i] * psi12);
		}
		...
		switch (sExotic->eCouponType[sCouponArg->iIndexCoupon])
		{
			case REGULAR_CRIF:
			{
				...
				dCoupon = DMIN(DMAX(dCoupon, sCouponArg->dFloor),sCouponArg->dCap);
				sMCArg ->dSavedCoupon[path_index] = dCoupon;
				DFR_pay = exp(-sCouponArg->tpay_tstar_alpha - sCouponArg->tpay_tstar_beta * ft1 - sCouponArg->tpay_tstar_beta_2 * ft2 
					- sCouponArg->tpay_tstar_gamma * psi1 - sCouponArg->tpay_tstar_gamma_2 * psi2 - sCouponArg->tpay_tstar_gamma_12 * psi12);
...
Err crif_adjust_payoff_2F(long		lEvtIndex,
						  long		lNbPaths,
						  long		lNbProd,
						  double		***dSavedValues,
						  void		*vFuncParam,
...
Err	crif_calculate_option(	CMSCTS_MARKET				sMarket,
							CRIF_DEAL				sDeal,
							CRIF_SIMULARG			sSimulArg,
							CRIF_PRICING_PARAMS		sPricingParams,
							CRIF_OUTPUTS			sOutputs)
{
	...
	sMCArg->sMCEBParams->dCallSpread = sPricingParams->sCouponPricingParams->dMCEBCallSpread;
	...
	sMCArg->sMCEBParams->iCalcOneTime = (sPricingParams->sReserveParams->iCalcSwitchAdjust > 0 ? 1 : 0);
	sMCArg->sMCEBParams->iCalcOneTimePartial = (sPricingParams->sReserveParams->iCalcSwitchAdjust > 0 ? 1 : 0);
	...
	{
		sMCArg->sMCEBParams->dMarketFwdIV[i] = sMCArg->dMarketValue[sMCArg->iIndexEvent[i]] / dDfStar;
		dMarketIV += sMCArg->dMarketValue[sMCArg->iIndexEvent[i]] / dDfStar;
	}	
	
	dTempVal = dmatrix(0, sMCArg->iNbEvent + 4+1, 0, 1+2 + sMCArg->sMCEBParams->iNbIndex + sMCArg->sMCEBParams->iHasNumeraire);
	...
	smessage ("Launching MC, Nb Steps: %d, Nb Paths: %d", sMCArg->iNbTimes, sPricingParams->sSimulParams->lNbPaths);
	
	if (sModel->iOne2F == 1)
	{
		err = lgmSV_mc_balsam_rev(	sMCArg->iNbTimes,
									sMCArg->iNbEvent,
									sMCArg->dTimes,
									sMCArg->dDates,
									...
									NULL,
									NULL,
									sPricingParams->sSimulParams->sLGMSVParams,
									sMCArg->vPayoffParams,
									sMCArg->iEvalEvent,
									sCall->iIsCallable ,
									sMCArg->iOptimise,
									sMCArg->sMCEBParams,
		...
		sOutputs->dExotic =  (dTempVal[1][0] - dTempVal[0][0])* dDfStar * sCall->dPayRec;
		if (sCall->iIsCallable)
			sOutputs->dCallInit = (dTempVal[7][0]- sCall->dPayRec * dTempVal[0][0]) * dDfStar;
		else
			sOutputs->dCallInit = 0;
	}
	else
	{
		err = lgmSV2F_mc_balsam_rev(sMCArg->iNbTimes,
									sMCArg->iNbEvent,
									sMCArg->dTimes,
									sMCArg->dDates,
									sPricingParams->sSimulParams->lNbPaths,
		...
		sOutputs->dExotic =  (dTempVal[1][0] - dTempVal[0][0])* dDfStar * sCall->dPayRec;
		if (sCall->iIsCallable)
			sOutputs->dCallInit = (dTempVal[8][0]- sCall->dPayRec * dTempVal[0][0]) * dDfStar;
		else
			sOutputs->dCallInit = 0;
	...
	{		
		...
		if (sMCArg->sMCEBParams->dMarketFwdIV) sMCArg->sMCEBParams->dMarketFwdIV[i] *= dDfStar;
		if (sMCArg->sMCEBParams->dModelFwdIV) sMCArg->sMCEBParams->dModelFwdIV[i] *= dDfStar;
		if (sMCArg->sMCEBParams->iCalcOneTime) sMCArg->sMCEBParams->dOneTimeCall[i] *= dDfStar;
		if (sMCArg->sMCEBParams->iCalcOneTimePartial) sMCArg->sMCEBParams->dOneTimePartial[i] *= dDfStar;		
	}	
	
FREE_RETURN:
	
	if (dTempVal) free_dmatrix(dTempVal, 0, sMCArg->iNbEvent + 4, 0, 2 + sMCArg->sMCEBParams->iNbIndex + sMCArg->sMCEBParams->iHasNumeraire);
	
	return err;
}

Err	crif_calibrate_model(	 CMSCTS_MARKET	sMarket,
							 CRIF_DEAL		sDeal,
							 CRIF_SIMULARG	sSimulArg,
							 ...
							 CRIF_OUTPUTS	sOutputs)
{
	...
	char	*cPrimFreq, *cPrimBasis, *cPrimRef,
		*cSecFreq, *cSecBasis,	*cSecRef;
	char	**cPrimTenors	= NULL,			
		**cSecTenors	= NULL;
	
	long	lNbPrim, lNbSec;
	long	*lPrimExeDates	= NULL,
		*lSecExeDates	= NULL;
...
/*	double	*tsv[2],
	...
	while(iStartCpnIndex<sExotic->iNbCpn && fabs(sExotic->dPaidGearing [iStartCpnIndex]) <1e-8)
	{
		iStartCpnIndex++;
	}
	...
	if (!cPrimTenors || !lPrimExeDates || !iPrimCalib || !dPrimStrike || !dPrimStrikeS1 || !dPrimStrikeS2 || 
		!cSecTenors || !lSecExeDates || !iSecCalib || !dSecStrike || !dSecStrikeS1 || !dSecStrikeS2 || !dSecWeights)
	{
		...
		case CMSCTS_DIAGCMS1:
			{
				for (i=0; i<lNbPrim+1; i++)
				{
					if (i < lNbPrim)
					{
						...
						{			
							lPrimExeDates[i] = add_unit(sExotic->lStartDate[i+iStartIndex], -sExotic->iFixingLag, SRT_BDAY, MODIFIED_SUCCEEDING);
						}
					}
					else
					{
						...
						case REGULAR_CRIF:
							{
								...
								if (sExotic->lPaidFixingDate[sExotic->iNbCpn-1] > lPrimExeDates[i-1] + 0.5 * (sExotic->lPayDate[sExotic->iNbCpn-1] - sExotic->lStartDate[sExotic->iNbCpn-1]))
								{
				...
				for (j=0; j<lNbSec+1; j++)
				{
					if (j < lNbSec)
					{
						...
						{			
							lSecExeDates[j] = add_unit(sExotic->lStartDate[j+iStartIndex], -sExotic->iFixingLag, SRT_BDAY, MODIFIED_SUCCEEDING);
						}
					}
					else
					{
						...
						case REGULAR_CRIF:
							{
								/* Add if In ARREARS fixing */
								if (sExotic->lPaidFixingDate[sExotic->iNbCpn-1] > lPrimExeDates[j-1] + 0.5 * (sExotic->lPayDate[sExotic->iNbCpn-1] - sExotic->lStartDate[sExotic->iNbCpn-1]))
								{
		...
		case CMSCTS_CMS1DIAG:
			{
				for (i=0; i<lNbPrim+1; i++)
				{
					if (i < lNbPrim)
					{
						...
						{			
							lPrimExeDates[i] = add_unit(sExotic->lStartDate[i+iIndexCpn], -sExotic->iFixingLag, SRT_BDAY, MODIFIED_SUCCEEDING);
						}
					}
					else
					{
	...
	if (!sCalibration->iUseInArrearsFixing)		iAddInArrears = 0;
		...
		case REGULAR_CRIF:
			{
				...
				{
					err = add_tenor(sDeal->lTheoEndDate, sExotic->cPaidTenor[i], NO_BUSDAY_CONVENTION, &temp);
	...
	err = cpd_calib_diagonal_LGMSV_new_dlm(	sMarket->cYcName,
											sMarket->cVcName,
											sMarket->GetCashVol,
											...
											sCalibration->iNbFactor,
											&dLambdaX,
											sCalibration->iNbTimesSV,
											sCalibration->dTimesSV,
											sCalibration->dAlphaSV,
											sCalibration->dLamSV,
											sCalibration->dRhoSV,
											sCalibration->dTStar,
											...
											sCalibration->dRho2SV,
											sCalibration->sLGMSVNumerParams,
	...
	iNbPWTimeNew = iNbPWTime +
		...
		- dPWTime[iNbPWTime-1]) / dStepCalib) + 1;
	...
	if (dRho2SV) dRho2SV = (double*) realloc(dRho2SV, iNbPWTimeNew * sizeof(double));
	
	if (!dPWTime || !dSigma || !dAlphaSV || !dLambdaSV || !dRhoSV || (!dRho2SV && sCalibration->iNbFactor == 2))
	{
	...
	for (i=iNbPWTime; i<iNbPWTimeNew; i++)
	{
		...
		if (dRho2SV) dRho2SV[i] = dRho2SV[iNbPWTime-1];
	}
	
	err = init_LGMSV_model(	sModel,
		sMarket->lToday,
		sCalibration->iNbFactor,
		iNbPWTimeNew,
		dLambdaX,
	...
	ConvertTS_LGM_to_LGMSV(	sModel->iNbPWTime,
		sModel->dPWTime,
		sModel->dSigma,
		sModel->dLambdaX,
		sModel->dTStar,
...
/*
	...
	err = cpd_calib_diagonal_dlm(	sMarket->cYcName,
									...
									sCalibration->sLGMSVCalibParams ->fix_lambda,
									sCalibration->iNbFactor,
									0,
									&dTime0,
									&dLambda,
	...
	err = SrtInitIRUnd(	sSimulArg->cLGM_fixedTau_ModelName,
						sMarket->cYcName,
						(sCalibration->iNbFactor == 1? "LGM": "LGM2F"), 
						iNbSigTime,
						2,
						tsv, 
						1,
...
FREE_RETURN:
	...
	if (cPrimTenors) free(cPrimTenors);
	if (lPrimExeDates) free(lPrimExeDates);
	if (iPrimCalib) free(iPrimCalib);
	if (dPrimStrike) free(dPrimStrike);
	if (dPrimStrikeS1) free(dPrimStrikeS1);
	if (dPrimStrikeS2) free(dPrimStrikeS2);
	
	if (cSecTenors) free(cSecTenors);
	if (lSecExeDates) free(lSecExeDates);
	if (iSecCalib) free(iSecCalib);
	if (dSecStrike) free(dSecStrike);
	if (dSecStrikeS1) free(dSecStrikeS1);
	if (dSecStrikeS2) free(dSecStrikeS2);
	if (dSecWeights) free(dSecWeights);
	
	if (dPWTime) free(dPWTime);
	if (dSigma) free(dSigma);
	if (dAlphaSV) free(dAlphaSV);
	if (dLambdaSV) free(dLambdaSV);
	if (dRhoSV) free(dRhoSV);
	if (dRho2SV) free(dRho2SV);
...
{
	if (sPayArg->sCallArg) free(sPayArg->sCallArg);
	if (sPayArg->sCouponArg) free(sPayArg->sCouponArg);
}
...
void crif_free_mc_arg(CRIF_MCARG	sMCArg)
{
	...
	if (sMCArg->dTimes) free(sMCArg->dTimes);
	if (sMCArg->dDates) free(sMCArg->dDates);
	if (sMCArg->dSigma) free(sMCArg->dSigma);
	if (sMCArg->dAlphaSV) free(sMCArg->dAlphaSV);
	if (sMCArg->dLambdaSV) free(sMCArg->dLambdaSV);
	if (sMCArg->dLevelSV) free(sMCArg->dLevelSV);
	if (sMCArg->dRhoSV) free(sMCArg->dRhoSV);
	if (sMCArg->dRho2SV) free(sMCArg->dRho2SV);
	if (sMCArg->dLGMAlpha) free(sMCArg->dLGMAlpha);
	if (sMCArg->dLGMRho) free(sMCArg->dLGMRho);
	
	if (sMCArg->iEvalEvent) free(sMCArg->iEvalEvent);
	if (sMCArg->iIndexEvent) free(sMCArg->iIndexEvent);
	...
	if (sMCArg->dPathInfos) free(sMCArg->dPathInfos);
	if (sMCArg->dSavedCoupon) free(sMCArg->dSavedCoupon);
	if (sMCArg->dSavedDF) free(sMCArg->dSavedDF);
	
	if (sMCArg->iOptimise) free(sMCArg->iOptimise);
	if (sMCArg->dMarketValue) free(sMCArg->dMarketValue);

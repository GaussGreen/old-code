...
//
//	if (!sMCArg->dDates || !sMCArg->dSigma || !sMCArg->dAlphaSV || !sMCArg->dLambdaSV ||
//!sMCArg->dLevelSV || 		!sMCArg->dRhoSV || !sMCArg->dRho2SV || !sMCArg->dLGMAlpha || !sMCArg->dLGMRho
//|| !sMCArg->iEvalEvent || 		!sMCArg->iIndexEvent || !sMCArg->vPayoffParams || !sMCArg->sMCEBParams
//|| !sMCArg->iOptimise || !sMCArg->dMarketValue)
//	{
...
//								CRIF_DEAL sDeal, 								LGMSV_MODEL
//sModel, 								int					iIndexCoupon, 								CRIF_COUPONARG	sCouponArg)
//{
//	Err				err = NULL;
//	CRIF_EXOTIC		sExotic;
...
//	sCouponArg->tpay_tstar_alpha = swp_f_df(sModel->lTStarDate, sExotic->lPayDate[iIndexCoupon],
//sMarket->cYcName); 	sCouponArg->tpay_tstar_alpha = -log(sCouponArg->tpay_tstar_alpha);
//	sCouponArg->tpay_tstar_beta = crif_beta_func(sModel->lTStarDate,
//sExotic->lPayDate[iIndexCoupon], sModel->dLambdaX); 	sCouponArg->tpay_tstar_gamma = 0.5 *
//sCouponArg->tpay_tstar_beta * sCouponArg->tpay_tstar_beta;
//
//	if (sModel->iOne2F == 2)
//	{
//		sCouponArg->tpay_tstar_beta_2 = crif_beta_func(sModel->lTStarDate,
//sExotic->lPayDate[iIndexCoupon], sModel->dLambdaX2); 		sCouponArg->tpay_tstar_gamma_2 = 0.5 *
//sCouponArg->tpay_tstar_beta_2 * sCouponArg->tpay_tstar_beta_2; 		sCouponArg->tpay_tstar_gamma_12 =
...
//NO_BUSDAY_CONVENTION, &lTheoEndDate); 			if (err) goto FREE_RETURN;
//
...
//			}
//
//			dFloating = swp_f_df(sMarket->lToday, sFixedSwapDates.date[0], sMarket->cYcName)
//- swp_f_df(sMarket->lToday, sFixedSwapDates.date[sFixedSwapDates.len-1], sMarket->cYcName);
//
...
//				if (j > 0)
//				{
//					sCouponArg->dCMSCoverage[j] = coverage(sFixedSwapDates.date[j-1],
...
//			sCouponArg->tdf_tstar_alpha[i] = swp_f_df(sModel->lTStarDate, lPayDate,
//sMarket->cYcName); 			sCouponArg->tdf_tstar_alpha[i] = -log(sCouponArg->tdf_tstar_alpha[i]);
//			sCouponArg->tdf_tstar_beta[i] = crif_beta_func(sModel->lTStarDate, lPayDate,
//sModel->dLambdaX); 			sCouponArg->tdf_tstar_gamma[i] = 0.5 * sCouponArg->tdf_tstar_beta[i] *
//sCouponArg->tdf_tstar_beta[i];
//
//			if (sModel->iOne2F == 2)
//			{
//				sCouponArg->tdf_tstar_beta_2[i] = crif_beta_func(sModel->lTStarDate,
//lPayDate, sModel->dLambdaX2); 				sCouponArg->tdf_tstar_gamma_2[i] = 0.5 *
//sCouponArg->tdf_tstar_beta_2[i] * sCouponArg->tdf_tstar_beta_2[i];
//				sCouponArg->tdf_tstar_gamma_12[i] = sCouponArg->tdf_tstar_beta[i] *
...
//								CRIF_DEAL
//sDeal, 								LGMSV_MODEL				sModel, 								int
//iIndexCall, 								int						iLastFundIdx, 								CRIF_CALLARG
//sCallArg)
//{
//	Err	err = NULL;
...
//		}
//
//		sCallArg->tpay_tstar_alpha[i] = swp_f_df(sModel->lTStarDate, lPayDate,
//sMarket->cYcName); 		sCallArg->tpay_tstar_alpha[i] = -log(sCallArg->tpay_tstar_alpha[i]);
//		sCallArg->tpay_tstar_beta[i] = crif_beta_func(sModel->lTStarDate, lPayDate,
//sModel->dLambdaX); 		sCallArg->tpay_tstar_gamma[i] = 0.5 * sCallArg->tpay_tstar_beta[i] *
//sCallArg->tpay_tstar_beta[i];
//
//		if (sModel->iOne2F == 2)
//		{
//			sCallArg->tpay_tstar_beta_2[i] = crif_beta_func(sModel->lTStarDate, lPayDate,
//sModel->dLambdaX2); 			sCallArg->tpay_tstar_gamma_2[i] = 0.5 * sCallArg->tpay_tstar_beta_2[i] *
//sCallArg->tpay_tstar_beta_2[i]; 			sCallArg->tpay_tstar_gamma_12[i] = sCallArg->tpay_tstar_beta[i] *
//sCallArg->tpay_tstar_beta_2[i];
//		}
//	}
//
//	/* reconstruction of DF(t,Tset) / DF(t,Tstar) */
//	sCallArg->tset_tstar_alpha = swp_f_df(sModel->lTStarDate, sCall->lSettlDate[iIndexCall],
//sMarket->cYcName); 	sCallArg->tset_tstar_alpha = -log(sCallArg->tset_tstar_alpha);
//	sCallArg->tset_tstar_beta = crif_beta_func(sModel->lTStarDate,
//sCall->lSettlDate[iIndexCall], sModel->dLambdaX); 	sCallArg->tset_tstar_gamma = 0.5 *
//sCallArg->tset_tstar_beta * sCallArg->tset_tstar_beta;
//
//	if (sModel->iOne2F == 2)
//	{
//		sCallArg->tset_tstar_beta_2 = crif_beta_func(sModel->lTStarDate,
//sCall->lSettlDate[iIndexCall], sModel->dLambdaX2); 		sCallArg->tset_tstar_gamma_2 = 0.5 *
//sCallArg->tset_tstar_beta_2 * sCallArg->tset_tstar_beta_2; 		sCallArg->tset_tstar_gamma_12 =
...
//											sDeal,
//											sModel,
//											sCallAux->iStartCpnIdx
//[iIndexCall], 											sCallArg->mceb_sCouponArg);
//
//	return err;
//}
//
...
//	{
//		if (sExotic->eCouponType[i] == REGULAR_CRIF && sExotic->lPaidFixingDate[i] >=
//sMarket->lToday + sMarket->iEODFixFlag)
//		{
//			/* Add Fixing of Paid CMS */
//			dAllTimes[lNbAllTimes] = (sExotic->lPaidFixingDate[i] - sMarket->lToday) *
//YEARS_IN_DAY; 			lNbAllTimes++;
//		}
//	}
//
//	/* Add the Call Dates */
...
//		{
//			dAllTimes[lNbAllTimes] = (sCall->lExeDate[i] - sMarket->lToday) *
//YEARS_IN_DAY; 			lNbAllTimes++;
//		}
//	}
//
...
//		{
//			sMCArg->dPathInfos = calloc(sPricingParams->sSimulParams->lNbPaths+1,
//sizeof(double));
//
...
//		if (sExotic->eCouponType[i] == REGULAR_CRIF)
//		{
//			sMCArg->dSavedCoupon = calloc(sPricingParams->sSimulParams->lNbPaths+1,
//sizeof(double)); 			sMCArg->dSavedDF = calloc(sPricingParams->sSimulParams->lNbPaths+1,
//sizeof(double)); 			sMCArg->iHasFloorCap = 1;
//
...
//	{
//		/* Initialisation */
//		sMCArg->dDates[i] = (long) (sMarket->lToday + sMCArg->dTimes[i] * DAYS_IN_YEAR +
//0.5); 		sMCArg->vPayoffParams[i]  = (void*) calloc(1, sizeof(crif_payarg)); 		sMCArg->dMarketValue[i]
//= 0.0; 		sMCArg->iEvalEvent[i] = 0; 		sMCArg->iOptimise[i] = 0;
//
...
//			{
//				sMCArg->dMarketValue[i] += -sDeal->dPayRec *
//sFundingAux->dMarketValue[j];
//			}
//
...
//			{
//				sMCArg->dMarketValue[i] += sDeal->dPayRec *
//sExoticAux->dMarketValue[iIndexCoupon];
//
...
//		for (i=0; i<sCallArg->iNbFundCoupon; i++)
//		{
//			DFR_pay = exp(-sCallArg->tpay_tstar_alpha[i] - sCallArg->tpay_tstar_beta[i] * ft
...
//				{
//					dPartialFunding += sFundingAux->dCouponPlusEx[iStartFundIdx+i] *
//DFR_pay;
//				}
//				else
//				{
...
//		/* first coupon */
//		DFR_pay = exp(-sCallArg->tpay_tstar_alpha[i] - sCallArg->tpay_tstar_beta[i] * ft -
//sCallArg->tpay_tstar_gamma[i] * psi); 		dFunding += sFunding->dNot[iStartFundIdx] * DFR_pay;
...
//exp(-sCallArg->mceb_sCouponArg->tdf_tstar_alpha[i] - sCallArg->mceb_sCouponArg->tdf_tstar_beta[i]
//* ft - sCallArg->mceb_sCouponArg->tdf_tstar_gamma[i] * psi);
//			}
//
//			dBasket = 0.0;
//
...
//sCallArg->mceb_sCouponArg->dAllDF[sCallArg->mceb_sCouponArg->iIndexCMSDF[j]];
//					}
//
//					dFloat =
//sCallArg->mceb_sCouponArg->dAllDF[sCallArg->mceb_sCouponArg->iIndexCMSDF[0]] -
//sCallArg->mceb_sCouponArg->dAllDF[sCallArg->mceb_sCouponArg->iIndexCMSDF[sCallArg->mceb_sCouponArg->iNbCMSDF-1]];
...
//				case REGULAR_CRIF:
//				{
//					dCoupon = sCallArg->mceb_sCouponArg->dAlpha * sMCArg
//->dSavedCoupon[path_index] + sCallArg->mceb_sCouponArg->dGamma  * dBasket +
//sCallArg->mceb_sCouponArg->dBeta;
//
//					dCoupon = DMIN(DMAX(dCoupon,
//sCallArg->mceb_sCouponArg->dFloor),sCallArg->mceb_sCouponArg->dCap); 					DFR_pay =
//exp(-sCallArg->mceb_sCouponArg->tpay_tstar_alpha - sCallArg->mceb_sCouponArg->tpay_tstar_beta * ft
//- sCallArg->mceb_sCouponArg->tpay_tstar_gamma * psi); 					break;
//				}
//			}
//
...
//			{
//				prod_val[4] = dCoupon  * DFR_pay * sCallArg->mceb_sCouponArg->dCoef;
//// 1: le next coupon
//				prod_val[5] = dCMS;
//// 2: le CMS
//			}
...
//		{
//
//			DFR_set = exp(-sCallArg->tset_tstar_alpha - sCallArg->tset_tstar_beta * ft -
//sCallArg->tset_tstar_gamma * psi); 			dFee = DFR_set * sCall->dFee[sCallArg->iIndexCall]; 			prod_val[6]
//= dFee;
//		}
//
//	}
...
//sCouponArg->tdf_tstar_beta[i] * ft - sCouponArg->tdf_tstar_gamma[i] * psi);
//		}
//
//		dBasket = 0.0;
//
...
//				}
//
//				dFloat = sCouponArg->dAllDF[sCouponArg->iIndexCMSDF[0]] -
//sCouponArg->dAllDF[sCouponArg->iIndexCMSDF[sCouponArg->iNbCMSDF-1]]; 				dCMS = dFloat / dLevel +
//sCouponArg->dSpread; 				dBasket += dCMS;
//			}
//		}
//
//
...
//				DFR_pay = exp(-sCouponArg->tpay_tstar_alpha - sCouponArg->tpay_tstar_beta
//* ft - sCouponArg->tpay_tstar_gamma * psi);
//
...
//		for (i=0; i<sCallArg->iNbFundCoupon; i++)
//		{
//			DFR_pay = exp(-sCallArg->tpay_tstar_alpha[i] - sCallArg->tpay_tstar_beta[i] *
//ft1 - sCallArg->tpay_tstar_beta_2[i] * ft2
//				- sCallArg->tpay_tstar_gamma[i] * psi1 - sCallArg->tpay_tstar_gamma_2[i] *
//psi2 - sCallArg->tpay_tstar_gamma_12[i] * psi12);
//
//			dFunding += sFundingAux->dCouponPlusEx[iStartFundIdx+i] * DFR_pay;
//
//			if (i < sCallArg->iNbFundCouponPartial)
...
//
//		/* first coupon */
//		DFR_pay = exp(-sCallArg->tpay_tstar_alpha[i] - sCallArg->tpay_tstar_beta[i] * ft1 -
//sCallArg->tpay_tstar_beta_2[i] * ft2
//			- sCallArg->tpay_tstar_gamma[i] * psi1 - sCallArg->tpay_tstar_gamma_2[i] * psi2
...
//				sCallArg->mceb_sCouponArg->dAllDF[i] =
//exp(-sCallArg->mceb_sCouponArg->tdf_tstar_alpha[i] - sCallArg->mceb_sCouponArg->tdf_tstar_beta[i]
//* ft1 - sCallArg->mceb_sCouponArg->tdf_tstar_beta_2[i] * ft2
...
//					dCoupon = DMIN(DMAX(dCoupon,
//sCallArg->mceb_sCouponArg->dFloor),sCallArg->mceb_sCouponArg->dCap); 					DFR_pay =
//exp(-sCallArg->mceb_sCouponArg->tpay_tstar_alpha - sCallArg->mceb_sCouponArg->tpay_tstar_beta *
//ft1 - sCallArg->mceb_sCouponArg->tpay_tstar_beta_2 * ft2
...
//			prod_val[6] = (sCall->dPayRec * dPartialFunding -  dCoupon  * DFR_pay *
//sCallArg->mceb_sCouponArg->dCoef ) * (sCall->dPayRec * dPartialFunding -  dCoupon  * DFR_pay *
//sCallArg->mceb_sCouponArg->dCoef); // 3: le current coupon
//
//			/* then calculate the fee */
//			if (sMCArg->iHasCallFee)
//			{
//				DFR_set = exp(-sCallArg->tset_tstar_alpha - sCallArg->tset_tstar_beta *
//ft1 - sCallArg->tset_tstar_beta_2 * ft2
//				- sCallArg->tset_tstar_gamma * psi1 - sCallArg->tset_tstar_gamma_2 * psi2
...
//		{
//			sCouponArg->dAllDF[i] = exp(-sCouponArg->tdf_tstar_alpha[i] -
//sCouponArg->tdf_tstar_beta[i] * ft1 - sCouponArg->tdf_tstar_beta_2[i] * ft2
//				- sCouponArg->tdf_tstar_gamma[i] * psi1 - sCouponArg->tdf_tstar_gamma_2[i]
//* psi2 - sCouponArg->tdf_tstar_gamma_12[i] * psi12);
//		}
//
//		dBasket = 0.0;
//
...
//				sMCArg ->dSavedCoupon[path_index] = dCoupon;
//				DFR_pay = exp(-sCouponArg->tpay_tstar_alpha - sCouponArg->tpay_tstar_beta
//* ft1 - sCouponArg->tpay_tstar_beta_2 * ft2
//					- sCouponArg->tpay_tstar_gamma * psi1 -
//sCouponArg->tpay_tstar_gamma_2 * psi2 - sCouponArg->tpay_tstar_gamma_12 * psi12);
//
//				prod_val[0] += -sCall->dPayRec * dCoupon * DFR_pay * sCouponArg ->dCoef
//;
//
...
//							CRIF_DEAL sDeal, 							CRIF_SIMULARG
//sSimulArg, 							CRIF_PRICING_PARAMS		sPricingParams, 							CRIF_OUTPUTS
//sOutputs)
//{
//	Err	err = NULL;
//	CMSCTS_FUNDING		sFunding;
...
//	sMCArg->sMCEBParams->iCalcOneTime = (sPricingParams->sReserveParams->iCalcSwitchAdjust > 0 ?
//1 : 0); 	sMCArg->sMCEBParams->iCalcOneTimePartial =
...
//	{
//		sMCArg->sMCEBParams->dMarketFwdIV[i] = sMCArg->dMarketValue[sMCArg->iIndexEvent[i]] /
//dDfStar; 		dMarketIV += sMCArg->dMarketValue[sMCArg->iIndexEvent[i]] / dDfStar;
//	}
//
//	dTempVal = dmatrix(0, sMCArg->iNbEvent + 4+1, 0, 1+2 + sMCArg->sMCEBParams->iNbIndex +
...
//			sOutputs->dCallInit = (dTempVal[7][0]- sCall->dPayRec * dTempVal[0][0]) *
//dDfStar; 		else 			sOutputs->dCallInit = 0;
//	}
//	else
//	{
...
//		sOutputs->dExotic =  (dTempVal[1][0] - dTempVal[0][0])* dDfStar * sCall->dPayRec;
//		if (sCall->iIsCallable)
//			sOutputs->dCallInit = (dTempVal[8][0]- sCall->dPayRec * dTempVal[0][0]) *
//dDfStar; 		else 			sOutputs->dCallInit = 0;
//
//	}
//
...
//
//		if (sMCArg->sMCEBParams->dMarketFwdIV) sMCArg->sMCEBParams->dMarketFwdIV[i] *=
//dDfStar; 		if (sMCArg->sMCEBParams->dModelFwdIV) sMCArg->sMCEBParams->dModelFwdIV[i] *= dDfStar; 		if
//(sMCArg->sMCEBParams->iCalcOneTime) sMCArg->sMCEBParams->dOneTimeCall[i] *= dDfStar; 		if
//(sMCArg->sMCEBParams->iCalcOneTimePartial) sMCArg->sMCEBParams->dOneTimePartial[i] *= dDfStar;
//	}
//
// FREE_RETURN:
//
//	if (dTempVal) free_dmatrix(dTempVal, 0, sMCArg->iNbEvent + 4, 0, 2 +
//sMCArg->sMCEBParams->iNbIndex + sMCArg->sMCEBParams->iHasNumeraire);
//
//	return err;
//}
//
...
//
//	if (!cPrimTenors || !lPrimExeDates || !iPrimCalib || !dPrimStrike || !dPrimStrikeS1 ||
//!dPrimStrikeS2 || 		!cSecTenors || !lSecExeDates || !iSecCalib || !dSecStrike || !dSecStrikeS1 ||
//!dSecStrikeS2 || !dSecWeights)
//	{
...
//					if (i < lNbPrim)
//					{
//						if (sCallAux->iNbUsedCall > 0 && iIndexCall <
//sCall->iNbCall)
//						{
//							lPrimExeDates[i] =
//sCall->lExeDate[iIndexCall]; 							iIndexCall++;
//						}
//						else
//						{
//							lPrimExeDates[i] =
//add_unit(sExotic->lStartDate[i+iStartIndex], -sExotic->iFixingLag, SRT_BDAY, MODIFIED_SUCCEEDING);
//						}
//					}
//					else
//					{
...
//lPrimExeDates[i-1] + 0.5 * (sExotic->lPayDate[sExotic->iNbCpn-1] -
//sExotic->lStartDate[sExotic->iNbCpn-1]))
//								{
//									lPrimExeDates[i] =
//sExotic->lPaidFixingDate[sExotic->iNbCpn-1]; 									iAddInArrears = 1;
//								}
//
//								break;
//							}
...
//					if (j < lNbSec)
//					{
//						if (sExotic->sAux->iNbUsedNumCpn > 0 && iIndexCpn <
//sExotic->iNbCpn)
//						{
//							lSecExeDates[j] = sExotic->lPaidFixingDate
//[iIndexCpn]; 							iIndexCpn++;
//						}
//						else
//						{
//							lSecExeDates[j] =
//add_unit(sExotic->lStartDate[j+iStartIndex], -sExotic->iFixingLag, SRT_BDAY, MODIFIED_SUCCEEDING);
//						}
//					}
//					else
//					{
...
//								/* Add if In ARREARS fixing */
//								if (sExotic->lPaidFixingDate[sExotic->iNbCpn-1] >
//lPrimExeDates[j-1] + 0.5 * (sExotic->lPayDate[sExotic->iNbCpn-1] -
//sExotic->lStartDate[sExotic->iNbCpn-1]))
//								{
//									lSecExeDates[j] =
//sExotic->lPaidFixingDate[sExotic->iNbCpn-1]; 									iAddInArrears = 1;
...
//sExotic->iNbCpn)
//						{
//							lPrimExeDates[i] = sExotic->lPaidFixingDate
//[iIndexCpn]; 							iIndexCpn++;
//						}
//						else
//						{
//							lPrimExeDates[i] =
//add_unit(sExotic->lStartDate[i+iIndexCpn], -sExotic->iFixingLag, SRT_BDAY, MODIFIED_SUCCEEDING);
//						}
//					}
//					else
//					{
...
//NO_BUSDAY_CONVENTION, &temp); 					lLongEndDate = max(lLongEndDate, temp); 					lShortEndDate =
//min(lShortEndDate, temp);
//				}
//
//				break;
//			}
...
//
//	if (!dPWTime || !dSigma || !dAlphaSV || !dLambdaSV || !dRhoSV || (!dRho2SV &&
...
//
//									sCalibration->sLGMSVCalibParams
//->fix_lambda, 									sCalibration->iNbFactor, 									0, 									&dTime0, 									&dLambda, 									0, 									2, 									sCalibration->dLGMAlpha,
//									sCalibration->dLGMGamma,
//									sCalibration->dLGMRho,
//
//

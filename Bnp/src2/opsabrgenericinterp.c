/* ==========================================================================
   ...
   PURPOSE:		MODIFIED IMPLEMENTATION OF SABR Generic with smile extrapolation using a beta model
...
static void NAG_CALL BMM2lsqfunqnopi(Integer m, Integer n, double x[], double fvec[],Nag_Comm *comm);
static void NAG_CALL BMM3lsqfunqnopi(Integer m, Integer n, double x[], double fvec[],Nag_Comm *comm);
static void NAG_CALL BMMGenlsqfunqnopi(Integer m, Integer n, double x[], double fvec[],Nag_Comm *comm);
static void NAG_CALL BMMlsqfunqwithpi(Integer m, Integer n, double x[], double fvec[],Nag_Comm *comm);
static void NAG_CALL BMM2lsqfunqwithzeta(Integer m, Integer n, double x[], double fvec[],Nag_Comm *comm);
...
Err CalibPiPi0Veta(double SigmaBeta,double alpha, double veta, double Pi0,double *ResPi0,double *ResPi,double *ResSig0,double *ResSig1,double *ResSig2)
{
	...
	//skew is always positive for log normal and Pi <0.5 so we know where to put which one is X1 and X2
...
Err BMMGetStates(	double forward,
					double maturity,
					double sigma,
					double alpha,
					double beta,
					...
					double* pFwd2)
{
	...
	if (forward	< 0.0)					{err = "Error: Forward should be positive";			goto FREE_RETURN;}
	if (sigma < 0.0)					{err = "Error: SigmaBeta should be positive";		goto FREE_RETURN;}
	if (alpha	< 0.0)					{err = "Error: Alpha should be positive";			goto FREE_RETURN;}
	if (maturity < 0.0)					{err = "Error: Maturity should better be positive";	goto FREE_RETURN;}
	if ((rho < -1.0) || (rho > 1.0))	{err = "Error: Rho should be > -1 and < 1";		goto FREE_RETURN;}
	if ((pi < 0.0) || (pi > 1.0))		{err = "Error: Pi should be between 0 and 1";		goto FREE_RETURN;}
...
//	pi = FindPiFromSkew(pi * SkewLog(alpha));
	...
	if (pi*(1-pi) != 0)
	{
		...
		*pSig2 = sigma - sqrt(pi / (1-pi)) * b;
		if (*pSig2 <= threshold*sigma)	{err = "Alpha too high";	goto FREE_RETURN;}
		if ((*pFwd1)*(*pFwd2) <= 0)		{err = "Forwards too far (Correlation to High or to Low)";	goto FREE_RETURN;}
	}
	else
	{
		*pFwd1	= forward;
...
Err BMM2GetStates(	double	forward,
					double	maturity,
					double	sigma,
					double	alpha,
					double	beta,
					...
					double* pPi)
{
	...
	if (alpha	< 0.0)					{err = "Error: Alpha should be positive";			goto FREE_RETURN;}
	if ((rho < -1.0) || (rho > 1.0))	{err = "Error: Rho should be > -1 and < 1";		goto FREE_RETURN;}
	if ((zeta < 0.0))						{err = "Error: Zeta should be positive";			goto FREE_RETURN;}
	...
	if (newpi*(1-newpi)*alpha !=0)
	{
		...
		*pFwd1	= max(forward * ( 1 + (1-newpi) * lambda), 1e-4); // new to handle Westminster newton
...
//3 states mapping
Err BMM3GetStates(	double	forward,
					double	maturity,
					double	sigma,
					double	alpha,
					double	beta,
					...
					double* pPi)
{
	...
	if (alpha	< 0.0)					{err = "Error: Alpha should be positive";			goto FREE_RETURN;}
	if ((rho < -1.0) || (rho > 1.0))	{err = "Error: Rho should be > -1 and < 1";		goto FREE_RETURN;}
	if ((veta < 0.0) || (veta > 1.0))	{err = "Error: Veta should be between 0 and 1";		goto FREE_RETURN;}
	if ((pi0 < 0.0) || (pi0 > 1.0))		{err = "Error: Pi0 should be between 0 and 1";		goto FREE_RETURN;}
	...
	else
	{
		...
		err = CalibPiPi0Veta(sigma,alpha*sqrt(maturity),veta,pi0,pPi0,pPi,pSig,pSig1,pSig2);
		if (err)		goto FREE_RETURN;
		pi =  *pPi;
		
		lambda = (rho  * UNSurSQRT2Pi * (*pSig) * pow (forward, beta - 1) * sqrt(maturity)) / ((1-pi0) * pi * (1- pi));
...
Err BMMGenGetStates(double	forward,
					double	maturity,
					double	sigma,
					double	alpha,
					double	a,
					...
					double* pFwd2)
{
	...
	if (pi*(1-pi) != 0)
	{
		...
		lambda = rho  * UNSurSQRT2Pi * unsurpiunmoinspi * sigma * temp / forward * sqrt(maturity)/sqrt(1-rho*rho);
...
Err BMM_Option_Price(	double	forward,
						double	strike,
						double	maturity,
						double	sigma,
						double	alpha,
						...
						double*	price)
{
	...
	if (beta>=0)
	{
		...
		//Fwd up   Vol Dn
		res +=  (1-pi) * srt_f_optblkschbeta(Fwd2,strike,Sig2,maturity,beta,1,optiontype,PREMIUM);
	}
	else
	{
		//Fwd up   Vol Up
...
Err BMM_Option_Price_quick(	double	forward,
							double	strike,
							double	maturity,
							double	sigma,
							double	alpha,
							...
							double*	price)
{
	...
	{
		//Fwd up   Vol Up
		res = pi * srt_f_optblkschbetaquick(Fwd1,strike,Sig1,maturity,beta,1,optiontype,PREMIUM);
		//Fwd up   Vol Dn
		res +=  (1-pi) * srt_f_optblkschbetaquick(Fwd2,strike,Sig2,maturity,beta,1,optiontype,PREMIUM);
	}
	else
	{
		//Fwd up   Vol Up
...
Err BMM_Option_Price_From_States(	double	Strike,
									double	Maturity,
									double	Beta,
									double	Fwd1,
									double	Fwd2,
									...
									double*	Price)
{
		...
		*Price = Pi * srt_f_optblkschbeta(Fwd1,Strike,Sig1,Maturity,Beta,1,optiontype,PREMIUM);
		//Fwd up   Vol Dn
		*Price +=  (1-Pi) * srt_f_optblkschbeta(Fwd2,Strike,Sig2,Maturity,Beta,1,optiontype,PREMIUM);
		return NULL;
}

Err BMM_Option_Price2(	double	forward,
						double	strike,
						double	maturity,
						double	sigma,
						double  alpha,
						...
						double*	price)
{
	...
	Err err = NULL;
	err = BMM2GetStates(forward,maturity,sigma,alpha,beta,rho,zeta,&Sig1,&Sig2,&Fwd1,&Fwd2,&Pi);
	if (err) goto FREE_RETURN;
	//Fwd up   Vol Up
	res = Pi * srt_f_optblkschbeta(Fwd1,strike,Sig1,maturity,beta,1,optiontype,PREMIUM);
	//Fwd up   Vol Dn
...
Err BMM_Option_Price3(	double	forward,
						double	strike,
						double	maturity,
						double	sigma,
						double  alpha,
						...
						double*	price)
{
	...
	err = BMM3GetStates(forward,maturity,sigma,alpha,beta,rho,veta,pi0,&Sig,&Sig1,&Sig2,&Fwd,&Fwd1,&Fwd2,&Pi0,&Pi);
	if (err) goto FREE_RETURN;
	//Fwd up   Vol Up
	res = (1-Pi0) * Pi * srt_f_optblkschbeta(Fwd1,strike,Sig1,maturity,beta,1,optiontype,PREMIUM);
	//Fwd up   Vol Dn
	res +=  (1-Pi0) *(1-Pi) * srt_f_optblkschbeta(Fwd2,strike,Sig2,maturity,beta,1,optiontype,PREMIUM);
	//Fwd Vol
	if(Pi0!=0)	res +=  Pi0 * srt_f_optblkschbeta(Fwd,strike,Sig,maturity,beta,1,optiontype,PREMIUM);
	*price = res;
...
Err BMM_Option_Price3_From_States(	double	forward,
									double	strike,
									double	maturity,
									double	beta,
									double	fwd,
...
Err BMMGen_Option_Price(	double	forward,
							double	strike,
							double	maturity,
							double	sigma,
							double	alpha,
							...
							SrtDiffusionType TypeVolLoc)
{
	...
	err = BMMGenGetStates(forward,maturity,sigma,alpha,a,b,c,rho,pi,TypeVolLoc,&Sig1,&Sig2,&Fwd1,&Fwd2);
	if (err) goto FREE_RETURN;
...
Err BMMCalibOnPrices(	double forward,
						double maturity,
						double atmvol,
						double alpha,
						double beta,
...
/*		options.print_level	= Nag_Soln_Iter_Full;  
	...
	BMMGetStates(forward,maturity,Sigma,alpha,beta,rho,pi,&tpSig1,&tpSig2,&tpFwd1,&tpFwd2);

	alphastep = alpha;
	while ((tpSig2 < 0.01 * Sigma) && (alphastep > 0.01 * alpha ))
	{
		alphastep = 0.9 * alphastep ;
		err = BMMGetStates(forward,maturity, Sigma, alphastep, beta,rho,pi,&tpSig1,&tpSig2,&tpFwd1,&tpFwd2);
	}
	...
	if (nCalibStrikes<=3)
	{
		nag_opt_lsq_no_deriv(nCalibStrikes, 3, BMMlsqfunqnopi, x, &res, fvec, (double*) fjac, tdj, &options, &nagcomm, &fail);
		*CalibBeta = beta;
		*CalibPi = pi;
	...
	else
	{
		...
		while ((exit == 0) && (i<10))
		{
			...
			nag_opt_lsq_no_deriv(nCalibStrikes, 4, BMMlsqfunqwithpi, x, &res, fvec, (double*) fjac, tdj, &options, &nagcomm, &fail);
			if (fail.errnum == 0) 
				exit = 1;
			else
			{
		...
		*Calibres = sqrt(fvec[0] * fvec[0] + fvec[1] * fvec[1] + fvec[2] * fvec[2] + fvec[3] * fvec[3]) / 4 /ATMVega;
	};
...
Err BMM2CalibOnPrices(	double forward,
						double maturity,
						double atmvol,
						double alpha,
						double beta,
...
/*		options.print_level	= Nag_Soln_Iter_Full;  
	...
	//Move alpha down if needed	
	Sigma = pow(forward, 1-beta) * atmvol;
	BMM2GetStates(forward,maturity,Sigma,alpha,beta,rho,zeta,&tpSig1,&tpSig2,&tpFwd1,&tpFwd2,&tpPi);

	alphastep = alpha;
	while ((tpSig2 < 0.01 * Sigma) && (alphastep > 0.01 * alpha ))
	{
		alphastep = 0.9 * alphastep ;
		err = BMM2GetStates(forward,maturity, Sigma, alphastep, beta,rho,zeta,&tpSig1,&tpSig2,&tpFwd1,&tpFwd2,&tpPi);
	}
	...
	if (nCalibStrikes<=3)
	{
		nag_opt_lsq_no_deriv(nCalibStrikes, 3, BMM2lsqfunqnopi, x, &res, fvec, (double*) fjac, tdj, &options, &nagcomm, &fail);
		*CalibBeta = beta;
		*CalibZeta = zeta;
		*Calibres = sqrt(fvec[0] * fvec[0] + fvec[1] * fvec[1] + fvec[2] * fvec[2]) / 3 / ATMVega;
	}
	else
	{
		...
		while ((exit == 0) && (i<10))
		{
			...
			nag_opt_lsq_no_deriv(nCalibStrikes, 4, BMM2lsqfunqwithzeta, x, &res, fvec, (double*) fjac, tdj, &options, &nagcomm, &fail);
			if (fail.errnum == 0) 
				exit = 1;
			else
			{
...
Err BMMCalibOnSabrStates(	double forward,
							double maturity,
							double sigma,
							double beta,
							double alpha,
							...
							double* Calibres)
{
	...
	if ((maturity ==0) || (pi ==0))	return NULL;
	if (beta<0) 
	{
		oldbeta = beta;
		beta = 0.0;
	...
	srt_f_optsarbvol(	forward, forward, maturity, sigma, alpha, beta, rho, TypeInput, SRT_LOGNORMAL, &sigma);
	...
	for (i=0;i<3;i++)	srt_f_optsarbvol(forward , CalibStrikes[i] , maturity, sigma, alpha, beta, rho, SRT_LOGNORMAL, SRT_LOGNORMAL, &CalibPrices[i]); // CalibPrices = LOG vols sabr 
	
	for (i=0;i<3;i++)	CalibPrices[i] = srt_f_optblksch(forward ,CalibStrikes[i] ,CalibPrices[i],maturity, 1.0, SRT_CALL, PREMIUM);
	...
	err = BMMCalibOnPrices(forward , maturity, sigma, alpha, beta, rho, pi, 3, CalibStrikes, CalibPrices, CalibWeights, &CalibSigma, &CalibBeta, &CalibAlpha, &CalibRho, &CalibPi, Calibres);
	if (err) return err;
	err = BMMGetStates(forward ,maturity,CalibSigma,CalibAlpha,CalibBeta,CalibRho,CalibPi,Sig1,Sig2,Fwd1,Fwd2);
	if (err) return err;
...
Err BMM2CalibOnSabrStates(	double forward,
							double maturity,
							double sigma,
							double beta,
							double alpha,
							...
							double* Calibres)
{
	...
	err = BMM2CalibOnSabr(forward , maturity, sigma, alpha, beta, rho, zeta, NStd, TypeInput, &CalibSigma, &CalibAlpha, &CalibBeta, &CalibRho, &CalibZeta, Calibres);
	if (err) return err;
	err = BMM2GetStates(forward ,maturity,CalibSigma,CalibAlpha,CalibBeta,CalibRho,CalibZeta,Sig1,Sig2,Fwd1,Fwd2, Pi);
	if (err) return err;
	return NULL;
};

Err BMMCalibOnSabr(	double forward,
					double maturity,
					double sigma, 
					double alpha,
					double beta,
...
Err BMM2CalibOnSabr(	double forward,
						double maturity,
						double atmvol,
						double alpha,
						double beta,
						...
						double* Calibres)
{
	...
	if (forward	< 0.0)					{err = "Error: Forward should be positive";			goto FREE_RETURN;}
	if (maturity < 0.0)					{err = "Error: Maturity should better be positive";	goto FREE_RETURN;}
	if (atmvol < 0.0)					{err = "Error: Sigma should be positive";			goto FREE_RETURN;}
	if (alpha	< 0.0)					{err = "Error: Alpha should be positive";			goto FREE_RETURN;}
	if ((rho < -1.0) || (rho > 1.0))	{err = "Error: Rho should be > -1 and < 1";		goto FREE_RETURN;}

	//Compute SigmaBeta vol and atmlogvol
	srt_f_optsarbvol(forward,forward,maturity,atmvol,alpha,beta,rho,TypeInput,SRT_BETAVOL,&sigma);
	srt_f_optsarbvol(forward,forward,maturity,atmvol,alpha,beta,rho,TypeInput,SRT_LOGNORMAL,&sigmaLog);

	//compute strikes and prices:
	for(i= 0; i<3;i++)
	{
		CalibStrikes[i] = forward * exp(vNStd[i] * NSTD * sqrt(maturity)*sigmaLog);
		srt_f_optsarbvol(forward,CalibStrikes[i],maturity,sigma,alpha,beta,rho,SRT_BETAVOL,SRT_LOGNORMAL,&tempvol);
		CalibPrices[i] = srt_f_optblksch(forward,CalibStrikes[i],tempvol,maturity,1,SRT_CALL,SRT_PREMIUM);
		CalibWeights[i] = 1;
	}
	e04xxc(&options);
	fail.print				= FALSE;
	...
	//Move alpha down if needed	
	i = 0;
	err = BMM2GetStates(forward,maturity,sigma,alpha,beta,rho,zeta,&pFwd1,&pFwd2,&pSig1,&pSig2,&pPi);
	while (err && i<15)
	{
		alpha = 0.9 * alpha;
		err = BMM2GetStates(forward,maturity,sigma,alpha,beta,rho,zeta,&pFwd1,&pFwd2,&pSig1,&pSig2,&pPi);
	...
	nag_opt_lsq_no_deriv(params.nb_strikes, 3, BMM2lsqfunqnopi, x, &res, fvec, (double*) fjac, 10, &options, &nagcomm, &fail);
...
Err BMM3CalibOnSabr(	double forward,
						double maturity,
						double atmvol,
						double alpha,
						double beta,
						...
						double* Calibres)
{
	...
	//Move alpha down if needed	
	i = 0;
	err = BMM3GetStates(forward,maturity,sigma,alpha,beta,rho,veta,pi0,&pFwd,&pFwd1,&pFwd2,&pSig,&pSig1,&pSig2,&pPi,&pPi0);
	while (err && i<15)
	{
		alpha = 0.9 * alpha;
		err = BMM3GetStates(forward,maturity,sigma,alpha,beta,rho,veta,pi0,&pFwd,&pFwd1,&pFwd2,&pSig,&pSig1,&pSig2,&pPi,&pPi0);
	...
	nag_opt_lsq_no_deriv(params.nb_strikes, 3, BMM3lsqfunqnopi, x, &res, fvec, (double*) fjac, 10, &options, &nagcomm, &fail);
...
Err BMMGenCalibOnSabrWithStrikesAndFirstGuess(	double forward,
												double maturity,
												double atmvol,
												double alpha,
												double beta,
												...
												double* Calibres)
{
	...
	if (nCalibStrikes<=3)
	{
		nag_opt_lsq_no_deriv(nCalibStrikes, 3, BMMGenlsqfunqnopi, x, &res, fvec, (double*) fjac, tdj, &options, &nagcomm, &fail);
		//modified for BMMGen
		*CalibA	= a;
		*CalibB	= b;
		*CalibC	= c;
...
static void NAG_CALL BMM2lsqfunqnopi(Integer m, Integer n, double x[], double fvec[],Nag_Comm *comm)
{
	...
	for (i=0; i< m; i++)
	{
		BMM_Option_Price2(forward,Vec_strikes[i],maturity,sigma,alpha,beta,rho,pi,SRT_CALL,&Price);
		fvec[i]= (Price - Vec_marketvols[i]) * Vec_weights[i];
	}
}
...
static void NAG_CALL BMM3lsqfunqnopi(Integer m, Integer n, double x[], double fvec[],Nag_Comm *comm)
{
	...
	for (i=0; i< m; i++)
	{
		BMM_Option_Price3(forward,Vec_strikes[i],maturity,sigma,alpha,beta,rho,veta,pi0,SRT_CALL,&Price);
...
double op_bmmgen_calib(	double				F,
						double				K,				
						double				T,
						double				sigma,	//ATMLOG vol
						double				alpha,
						...
						SrtDiffusionType	TypeVolLoc)
{
	...
	BMMGen_Option_Price(F,K,T,sigma_beta1,alpha,a,b,c,rho,pi,SRT_CALL,&pricetemp,TypeVolLoc);
	srt_f_optimpvol(pricetemp, F, K, T, 1, SRT_CALL, SRT_LOGNORMAL, &vol1);
	...
	BMMGen_Option_Price(F,K,T,sigma_beta2,alpha,a,b,c,rho,pi,SRT_CALL,&pricetemp,TypeVolLoc);
	srt_f_optimpvol(pricetemp, F, K, T, 1, SRT_CALL, SRT_LOGNORMAL, &vol2);
...
/////////////////////////////////////////////////////////////////////////////////////////////
//
//	srt_f_op_bmmvol(), Returns BSlog(@K) //copy from the sabr gen version..... should be improved
//
...
Err srt_f_optbmmvol(	double	Forward,
						double	Strike,
						double	Maturity,
						double	Sigma,
						double	Alpha,
						...
						double	*vol)
{
	...
	if (Maturity < 0.0)					{err = "Error: Maturity should better be positive";	goto FREE_RETURN;}
	if ((Rho < -1.0) || (Rho > 1.0))	{err = "Error: Rho should be > -1 and < 1";			goto FREE_RETURN;}
	if ((Pi < -1.0) || (Pi > 1.0))		{err = "Error: Zeta should be > -1 and < 1";		goto FREE_RETURN;}
	...
	if (( input == SRT_LOGNORMAL_ATS) || ( input == SRT_NORMAL_ATS) )
	{
		...
		{
				pricetemp = srt_f_optblksch(	Forward,
												Strike,
												Sigma,
												Maturity,
												1.0,
		...
		{
				pricetemp = srt_f_optblknrm(	Forward,
												Strike,
												Sigma,
												Maturity,
												1.0,
	...
	{
		err = srt_f_optsarbvol(	Forward, Forward, Maturity, Sigma, 0, 0, 0, SRT_BETAVOL, SRT_LOGNORMAL, &Sigma);
		if(err) goto FREE_RETURN;
	}
	...
	if (output == SRT_BETAVOL)
		...
		BMM_Option_Price(Forward,Strike,Maturity,SigmaBeta,Alpha,Beta,Rho,Pi,SRT_CALL,&pricetemp);
		...
		{
			err = srt_f_optsarbvol(	Forward, Strike, Maturity, res, 0, 1, 0, SRT_BETAVOL, SRT_NORMAL, &res);
			if(err) goto FREE_RETURN;
		}
	}
...
Err srt_f_optbmmprice(	double	Forward,
						double	Strike,
						double	Maturity,
						double	Sigma,
						double	Alpha,
						...
						double	*price)
{
	...
	BMM_Option_Price2(Forward,Strike,Maturity,SigmaBeta,Alpha,Beta,Rho,Pi,call_put,&pricetemp);
	*price = pricetemp;
...
Err srt_f_optbmmcalvol(	double	Forward,
						double	Strike,
						double	Maturity,
						double	Sigma,
						double	Alpha,
						...
						double	*vol)
{
	...
	if (Maturity < 0.0)					{err = "Error: Maturity should better be positive";	goto FREE_RETURN;}
	if ((Rho < -1.0) || (Rho > 1.0))	{err = "Error: Rho should be > -1 and < 1";			goto FREE_RETURN;}
	if ((Pi <=0) || (Pi>= 1.0))			{err = "Error: Pi should be > 0 and < 1";			goto FREE_RETURN;}
	if (NStd<=0)						{err = "Error: NStd should be > 0=";				goto FREE_RETURN;}
	
	err = BMMCalibOnSabr(Forward,Maturity,Sigma,Alpha,Beta,Rho,Pi,NStd,input,&CalibSigma,&CalibAlpha,&CalibBeta,&CalibRho,&CalibPi,&CalibRes);
	
	if (!err)
		err = srt_f_optbmmvol(Forward,Strike,Maturity,CalibSigma,CalibAlpha,CalibBeta,CalibRho,CalibPi,SRT_BETAVOL,output,vol);
						...
						double	*vol)
{
	...
	// Control of passed arguments 
	if ((Fwd1< 0.0) || (Fwd2 < 0.0))							{err = "Error: Forwards should be positive";			goto FREE_RETURN;}
	if ((Sig1 < 0.0) || (Sig2 < 0.0))							{err = "Error: Sigmas should be positive";		goto FREE_RETURN;}
	if (Maturity < 0.0)											{err = "Error: Maturity should better be positive";	goto FREE_RETURN;}
	if ((Pi < 0.0) || (Pi > 1.0))								{err = "Error: Pi should be > 0 and < 1";			goto FREE_RETURN;}
	if ((output != SRT_NORMAL) && (output != SRT_LOGNORMAL))	{err = "output should be NORMAL or LOGNORMAL";			goto FREE_RETURN;}
	
	Fwd = Pi * Fwd1 + (1-Pi) * Fwd2;
	BMM_Option_Price_From_States(Strike,Maturity,Beta,Fwd1,Fwd2,Sig1,Sig2,Pi,SRT_CALL,&res);
...
Err srt_f_optbmm2vol(	double	Forward,
						double	Strike,
						double	Maturity,
						double	Sigma,
						double	Alpha,
						...
						double	*vol)
{
	...
	if (Maturity < 0.0)					{err = "Error: Maturity should better be positive";	goto FREE_RETURN;}
	if ((Rho < -1.0) || (Rho > 1.0))	{err = "Error: Rho should be > -1 and < 1";			goto FREE_RETURN;}
	if ((Zeta < 0.0) )					{err = "Error: Zeta should be > 0";					goto FREE_RETURN;}
	...
	if (output == SRT_BETAVOL)
		...
		BMM_Option_Price2(Forward,Strike,Maturity,SigmaBeta,Alpha,Beta,Rho,Zeta,SRT_CALL,&pricetemp);
		srt_f_optimpvol(pricetemp, Forward, Strike, Maturity, 1, SRT_CALL, SRT_LOGNORMAL, &res);
...
Err srt_f_optbmm2calvol(	double	Forward,
							double	Strike,
							double	Maturity,
							double	Sigma,
							double	Alpha,
							...
							double	*vol)
{
	...
	if (Maturity < 0.0)					{err = "Error: Maturity should better be positive";	goto FREE_RETURN;}
	if ((Rho < -1.0) || (Rho > 1.0))	{err = "Error: Rho should be > -1 and < 1";			goto FREE_RETURN;}
	if ((Zeta <0))						{err = "Error: Zeta should be >= 0";				goto FREE_RETURN;}
	if (NStd<=0)						{err = "Error: NStd should be > 0";				goto FREE_RETURN;}
	
	err = BMM2CalibOnSabr(Forward,Maturity,Sigma,Alpha,Beta,Rho,Zeta,NStd,input,&CalibSigma,&CalibAlpha,&CalibBeta,&CalibRho,&CalibZeta,&CalibRes);
	if (!err)
		err = srt_f_optbmm2vol(Forward,Strike,Maturity,CalibSigma,CalibAlpha,CalibBeta,CalibRho,CalibZeta,input,output,vol);

FREE_RETURN:
	return err;
}

Err srt_f_optbmm3vol(	double	Forward,
						double	Strike,
						double	Maturity,
						double	Sigma,
						double	Alpha,
						...
						double	*vol)
{
	...
	if (output == SRT_BETAVOL)
		...
		BMM_Option_Price3(Forward,Strike,Maturity,SigmaBeta,Alpha,Beta,Rho,veta,pi0,SRT_CALL,&pricetemp);
		srt_f_optimpvol(pricetemp, Forward, Strike, Maturity, 1, SRT_CALL, SRT_LOGNORMAL, &res);
...
Err srt_f_optbmm3calvol(	double	Forward,
							double	Strike,
							double	Maturity,
							double	Sigma,
							double	Alpha,
							...
							double	*vol)
{
	...
	if (Maturity < 0.0)					{err = "Error: Maturity should better be positive";	goto FREE_RETURN;}
	if ((Rho < -1.0) || (Rho > 1.0))	{err = "Error: Rho should be > -1 and < 1";			goto FREE_RETURN;}
	if ((pi0 <=0) || (pi0>= 1.0))		{err = "Error: Pi0 should be > 0 and < 1";			goto FREE_RETURN;}
	if ((veta <=0) || (veta>= 1.0))		{err = "Error: Veta should be > 0 and < 1";			goto FREE_RETURN;}
	if (NStd<=0)						{err = "Error: NStd should be > 0=";				goto FREE_RETURN;}
	
	err = BMM3CalibOnSabr(Forward,Maturity,Sigma,Alpha,Beta,Rho,veta,pi0,NStd,input,&CalibSigma,&CalibAlpha,&CalibBeta,&CalibRho,&CalibVeta,&CalibPi0,&CalibRes);
	if (!err)
		err = srt_f_optbmm3vol(Forward,Strike,Maturity,CalibSigma,CalibAlpha,CalibBeta,CalibRho,CalibVeta,CalibPi0,input,output,vol);

FREE_RETURN:
	return err;
}

Err srt_f_optbmm3volfromstates(	double	Forward,
								double	Strike,
								double	Maturity,
								double	Beta,
								double  Fwd,
								...
								double	*vol)
{
	...
	BMM_Option_Price3_From_States(Forward,Strike,Maturity,Beta,Fwd,Fwd1,Fwd2,Sig,Sig1,Sig2,Pi0,Pi,SRT_CALL,&res);
	srt_f_optimpvol(res, Fwd, Strike, Maturity, 1, SRT_CALL, output, &res);
...
Err srt_f_optbmmgenvol(	double	Forward,
						double	Strike,
						double	Maturity,
						double	Sigma,
						double	Alpha,
						...
						SrtDiffusionType	TypeVolLoc)
{
	...
	SigmaBeta = op_bmmgen_calib(Forward,Forward,Maturity,Sigma,Alpha,a,b,c,Rho,Pi,TypeVolLoc);
	...
	if (output == SRT_BETAVOL)
		...
		BMMGen_Option_Price(Forward,Strike,Maturity,SigmaBeta,Alpha,a,b,c,Rho,Pi,SRT_CALL,&pricetemp,TypeVolLoc);
		srt_f_optimpvol(pricetemp, Forward, Strike, Maturity, 1, SRT_CALL, SRT_LOGNORMAL, &res);
				...
				double*	Price)
{
	...
	//Bessel = s18eec(Strike/pow(Forward,Theta)/Sigma02T,2.0/abs(Theta),&Fail);
 	//res = abs(Theta)/Sigma02T*sqrt(Forward)/pow(Strike,1.5+2*Theta)*exp(-(pow(Strike,-2*Theta)+pow(Forward,-2*Theta))/(2*Sigma02T))*Bessel;
				...
				double*	Price)
{
		...
		double vol;
		vol =	SigmaBeta*VolFav/Fav*(1
			+	Delta				/24*	(2	*Gam2		-1*	Gam1*Gam1		+1/pow(Fav,2.0))
			+	pow(Theta,2.0)		/24*	(1	*Gam2		-2*	Gam1*Gam1		+2/pow(Fav,2.0))

			+	pow(Delta,2.0)		/480*	(2		*Gam4	+4	*Gam1*Gam3	+3	*Gam2*Gam2	-3	*Gam1*Gam1*Gam2	+0.75	*(pow(Gam1,4.0)	-1/pow(Fav,4.0))	+0.5	/pow(Fav,2.0)*	(10	*Gam2	-5	*Gam1*Gam1	+5	/pow(Fav,2.0))	)
			+	Delta*pow(Theta,2.0)/2880*	(6		*Gam4	-18	*Gam1*Gam3	+14	*Gam2*Gam2	-29	*Gam1*Gam1*Gam2	+11		*(pow(Gam1,4.0)	-1/pow(Fav,4.0))	+1		/pow(Fav,2.0)*	(35	*Gam2	-40	*Gam1*Gam1	+40	/pow(Fav,2.0))	)
			+	pow(Theta,4.0)		/1440*	(0.75	*Gam4	-6	*Gam1*Gam3	-2	*Gam2*Gam2	+17	*Gam1*Gam1*Gam2	-8		*(pow(Gam1,4.0)	-1/pow(Fav,4.0))	+1		/pow(Fav,2.0)*	(5	*Gam2	-10	*Gam1*Gam1	+10	/pow(Fav,2.0))	)
			);
...
Err BMMGenPrice(double	Forward,
				double	Strike,
				double	SigmaBeta,
				double	Maturity,
				double	a,

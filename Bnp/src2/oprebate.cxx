...
double srt_f_optrebate(double fwd_price, double spot, double barrier,
                       ...
                       SrtGreekType greek) { /** BEGIN srt_f_optrebate() **/
  ...
  if (vol != 0 && mat != 0) {
    ...
    caux = disc * rebate *
           (nprimexx1 *
                (barrier / fwd_price + pow(spot / fwd_price, -2 * chi + 2)) /
                (vol * sqrt(mat) * fwd_price) +
            ndxx2 * pow(spot / barrier, -2 * chi + 2) *
                ((-2 * chi + 2) / spot + 2 * vol / vol / mat *
                                             (1 / spot - 1 / fwd_price) *
                                             log(spot / barrier)));
  ...
  if (greek == PREMIUM) {
    ...
    answer = (srt_f_optrebate(fwd_price + shift, spot, barrier, rebate, vol,
                              mat, disc, call_put, PREMIUM) -
              premium) /
             shift;
  } else if (greek == DELTA) { /* DELTA */
    shift = spot / 10000;
    answer = (srt_f_optrebate(fwd_price + shift, spot + shift, barrier, rebate,
    ...
    answer = (srt_f_optrebate(fwd_price + shift, spot, barrier, rebate, vol,
                              mat, disc, call_put, PREMIUM) +
              srt_f_optrebate(fwd_price - shift, spot, barrier, rebate, vol,
                              mat, disc, call_put, PREMIUM) -
              2 * premium) /
             (shift * shift);
    ...
    answer = (srt_f_optrebate(fwd_price + shift, spot + shift, barrier, rebate,
                              vol, mat, disc, call_put, PREMIUM) +
              srt_f_optrebate(fwd_price - shift, spot - shift, barrier, rebate,
                              vol, mat, disc, call_put, PREMIUM) -
              2 * premium) /
             (shift * shift);
  } else if (greek == VEGA) { /* VEGA */
    shift = 0.01;
    answer = (srt_f_optrebate(fwd_price, spot, barrier, rebate, vol + shift,
                              mat, disc, call_put, PREMIUM) -
              premium);
  } else if (greek == THETA) { /* THETA */
    shift = 1 / 365;
    answer = (srt_f_optrebate(fwd_price, spot, barrier, rebate, vol,

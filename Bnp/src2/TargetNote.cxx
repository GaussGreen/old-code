...
char *TARN_TimeSteps_2F(TARN_Struct *tarn, TARN_AUX *aux, TARN_MC_AUX *aux_mc) {
  ...
  if (!(aux_mc->time = (double *)calloc(aux_mc->nStps, sizeof(double))))
    return "Memory allocation error in TARN_TimeSteps_2F";
  /*
          if ( ! (aux_mc->evt_dts = (double*) calloc(aux_mc->nEvents        ,
     sizeof (double)))  ) return "Memory allocation error in TARN_TimeSteps_2F";
     if ( ! (aux_mc->evt_tms = (double*) calloc(aux_mc->nEvents        , sizeof
     (double)))  ) return "Memory allocation error in TARN_TimeSteps_2F";
  */
...
char *TargetNoteMC_2F(char *szUnd, TARN_Struct *tarn, TARN_AUX *aux,
  ...
  /* Dates and DF */
  //	double dFund_Start_DF        , dFund_End_DF        , dFund_cvgMargin ,
  //dFundPV; 	int iNum_Fund_DF;
  double df;
  ...
  /* Add PV of the first funding coupon
     ...
     tarn->deal.lvFundStartDates[i]        , tarn->market.szYieldCurve);
     dFund_End_DF = swp_f_df(tarn->market.lToday        ,
     tarn->deal.lvFundEndDates[i]        , tarn->market.szYieldCurve);

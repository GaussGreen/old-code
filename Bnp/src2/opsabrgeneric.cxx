...
//
// op_sabrgen_calib()	Given the ATMLOGVol        , solves for the SigmaBeta
// vol
//
// vol_bvm()				Specify your local vol of the form
// vol_mylocvol(double        , double        ,double        ,double        ,
// int)
//
...
double op_sabrgen(double F, double K, double T,
                  double sigma, // SigmaBeta Vol
                  double alpha, double a, double b, double c, double rho,
                  double (*vol_local)(double x, double a, double b, double c,
                                      int type)) {
...
double op_sabrgen_calib(double F, double K, double T,
...
//
/////////////////////////////////////////////////////////////////////////////////////////////
void GetLocVolFromDiffusionType(SrtDiffusionType TypeVolLoc,
...
// b: param2
// c: param3
// type: specify function(0)        , derivative(1)        , sec derivative(2)
// , prime of inverse(3)        , eq Beta (4)
//
/////////////////////////////////////////////////////////////////////////////////////////////
...
//
/////////////////////////////////////////////////////////////////////////////////////////////
double vol_sabr(double x, double beta, double not_used1, double not_used2,
                int type) {
  static double res;
...
/////////////////////////////////////////////////////////////////////////////////////////////
//
// local vol function: vol_log_quadra        , Vol = F * (a * lnF ^ 2 + b * lnF
// + c)
//
/////////////////////////////////////////////////////////////////////////////////////////////
...
//
/////////////////////////////////////////////////////////////////////////////////////////////
double vol_bvm(double x, double delay, double not_used1, double not_used2,
               int type) {
  static double res, x1, x2, delta;
...
/////////////////////////////////////////////////////////////////////////////////////////////
//
// local vol function: vol_bvm2        ,  Vol = x^beta (1 - exp((-x /
// x0)^(1-beta)))
//
/////////////////////////////////////////////////////////////////////////////////////////////
double vol_bvm2(double F, double F0, double Beta, double not_used2, int type) {
  ...
  case 1: {
    ...
    double o3 = pow(F, o2);
    res =
        (exp(-o3) * (F - Beta * F + Beta * (-1. + exp(o3)) * pow(F, Beta))) / F;
    res /= F0;
...
double vol_bvmh2(double F, double F0, double F1, double Beta, int type) {
  ...
  case 2: {
    ...
    res = (o4 * pow(F, -2.) *
           (-(F0 * o4 * o6 * Beta *
              (2. * F * o10 + F0 * o11 * (-1. + 3. * Beta))) +
            1 / tanh(o4) *
                (2. * F * o10 * (-(F * o11) + F0 * Beta) +
                 o1 * o11 * Beta *
                     (-1. + Beta + 2. * o6 * Beta * pow(o3, 2. * Beta))))) /
          o1;
  ...
  case 3: {
    ...
    double tempres = 0;
    for (i = 0; i < nbstep; i++) {
      // tempres += mesh / (tempvar / F0 * tanh(tempvar / F0) / tanh(tempvar /
      // F1));
...
/////////////////////////////////////////////////////////////////////////////////////////////
//
// local vol function: vol_bvmc        ,  Vol = (F/F0)^(betamax+betamin) /
// ((F/F0)^betamax + (F/F0)^betamin)
//
/////////////////////////////////////////////////////////////////////////////////////////////
double vol_bvmc(double F, double F0, double BetaMin, double BetaMax, int type) {
  ...
  if (BetaMin == BetaMax && BetaMax == 0) {
    ...
    case 2: {
      ...
      res = -(BetaMax * (1. - BetaMax + (1. + BetaMax) * o1) *
              pow(F, -2. + BetaMax) * pow(1. + o1, -3.));
      res = res / (F0 * F0);
      return res;
      break;
    }
    ...
    case 2: {
      double o1 = pow(F, BetaMax);
      res = -((-1. + BetaMax) * o1 * (-(BetaMax * F) + (-2. + BetaMax) * o1) *
              pow(F + o1, -3.));
      res = res / (F0 * F0);
      return res;
      break;
    ...
    case 2: {
      double o1 = pow(F, BetaMin);
      res = -((-1. + BetaMin) * o1 * (-(BetaMin * F) + (-2. + BetaMin) * o1) *
              pow(F + o1, -3.));
      res = res / (F0 * F0);
      return res;
      break;
  ...
  // Main Case
    ...
    case 1: {
      ...
      res = (BetaMin * o1 + BetaMax * o2) * pow(F, -1. + BetaMax + BetaMin) *
            pow(o1 + o2, -2.);
...
/////////////////////////////////////////////////////////////////////////////////////////////
//
// op_sabr_distri        ,  returns the density or cumulative distribution at
// strike K
//
/////////////////////////////////////////////////////////////////////////////////////////////
...
// check all : 30
double vol_parameters_check_from_SrtDiffusionType(double a, double b, double c,
                                                  int type,
                                                  SrtDiffusionType VolLocType) {
  double alb = -DBL_MAX, aub = DBL_MAX, blb = -DBL_MAX, bub = DBL_MAX,
         clb = -DBL_MAX, cub = DBL_MAX;
  ...
  case 22:
    return cub;
  case 30: {
    if ((a < aub) && (a > alb) && (b < bub) && (b > blb) && (c < cub) &&
        (c > clb))
      return 1; // ok
    else

...
double dCorrel(double dT, double beta)
{
    ...
    return 1. / (1. - e) * (1. - dRhoMin * e + (dRhoMin - 1.0) * exp(1. - dT / dTmax));
}
...
double dCrossIntegral(int indexP, int indexK, double* dPeriod)
{
    ...
    if (!(ppdFwdVol) && !(ppdOptFutVol))
    {
        for (i = 0; i < indexK; ++i)
        {
            dInteg += dVolStruct[indexK - i - 1] * dVolStruct[indexP - i - 1] * dPeriod[i];
        }
    }
    else if (ppdFwdVol)
    {
    ...
    else if (ppdOptFutVol)
    {
        for (i = 0; i < indexK; ++i)
        {
            dInteg += dVolStruct2[indexK - i - 1] * dVolStruct2[indexP - i - 1] * dPeriod[i];
        }
    }
...
/*=================================================================
    ...
    the market model
-------------------------------------------------------------------*/
double dDrift(int iNumFut, int indexK, double* dPeriod, double* dFwd, double beta)
{
    // fixed index K
    int    p;
    double drift = 0.0;
...
double dVar(int indexK, double* dPeriod)
{
    ...
    if (!ppdOptFutVol)
    {
        for (p = 0; p < indexK; ++p)
        {
            dInteg += dVolStruct[indexK - p - 1] * dVolStruct[indexK - p - 1] * dPeriod[p];
        }
    }
    else
    {
        for (p = 0; p < indexK; ++p)
        {
            dInteg += dVolStruct2[indexK - p - 1] * dVolStruct2[indexK - p - 1] * dPeriod[p];
        }
    }
    return exp(dInteg) - 1.;
}
...
------------------------------------------------------------------------*/
Err future_Rutkowski(long index, double* dPeriod, double* dParams, double* pdFuturesRate)
{
    double tmpProd = 1., tmpDen = 1., tmpSum = 0., dVarj, dFutRate;
    double beta;
    int    i, k, p;
    /*beta introduced as a parameter to fit the minimal correlation between 2 Libors
    ...
    for (k = 1; k < index; ++k)
    {
        tmpProd +=
            ...
            (exp(-dDrift(index, k, dPeriod, dL0T, beta)) +
             dCorrel(fabs(dDates[index] - dDates[k]), beta) * sqrt(dVarj * dVar(k, dPeriod)));
    }
    for (i = 1; i < index; ++i)
        for (p = 1; p < index; ++p)
        {
    ...
    for (k = 1; k < index; ++k)
    {
        tmpSum -= dPeriod[k] * dPeriod[k] * dL0T[k - 1] * dL0T[k - 1] *
                  exp(-2 * dDrift(index, k, dPeriod, dL0T, beta) + dCrossIntegral(k, k, dPeriod));
    }
...
Err future_HJM(
    long    index,
    double* dPeriod,
    double* dParams,  // first: short term rate, following: initial correlations
    double* pdFuturesRate)
{
    ...
    for (i = 1; i < index; ++i)
    {
        ...
        pdFinVol[i] = (pdFinVol[i - 1] * i + pdVol[i] * dFwd[i]) / (i + 1);  // short term vol
    }
    ...
    dBetaAdj = (1. + dL0T[index - 1] * dCoverage[index - 1]) *
               exp((pdVol[index - 1] * dFwd[index - 1]) * (pdVol[index - 1] * dFwd[index - 1]) *
                   dCumTime * dCoverage[index - 1] * dCoverage[index - 1]);
    dG = 8. * dCumTime / dTau / 105 - 4. / 15;
    dG *= dCumTime / dTau;
    dG += 2. / 3;
    dG *= dCumTime * sqrt(dCumTime);
    dBetaFut =
        dBetaAdj * exp(dRho * (pdVol[index - 1] * dFwd[index - 1]) *
                       (pdFinVol[index - 1] * dShort) * sqrt(dCumTime) * dG * dCoverage[index - 1]);
...
------------------------------------------------------------------------*/
Err future_Bloom(long index, double* dPeriod, double* dParams, double* pdFuturesRate)
{
    double dZ;
    double dCoeff;
    ...
    dFutRate = (dB0T[index - 1] / dB0T[index] * exp(dZ) - 1.) / dCoverage[index - 1];
...
Err future_HL(long index, double* dPeriod, double* pdFuturesRate)
{
    ...
    dInteg = 0.5 * dVol * dVol * dFwd[iNumFut - 1] * dFwd[iNumFut - 1] * pow(dCumTime, 2);
    dAdjConv =
        exp(dVol * dVol * dL0T[iNumFut - 1] * dL0T[iNumFut - 1] * dCumTime *
            dCoverage[iNumFut - 1] * dCoverage[iNumFut - 1]);
    dAdjConv *= exp(dInteg * dCoverage[iNumFut - 1]);
    dFutRate = ((1. + dL0T[iNumFut - 1] * dCoverage[iNumFut - 1]) * dAdjConv - 1.) /
               dCoverage[iNumFut - 1];
...
------------------------------------------------------------------------*/
Err eFutFunc_Rut(
    double dIndex, double* pdParams, double* pdConvexity, double* pdDerivative, int nParams)
{
    Err     err;
    double* dPeriod;
    double* dParams;

...
double dCrossIntegral(int indexP, int indexK, double *dPeriod)
{
    ...
    //forward volatility reconstruction
        ...
        free_dvector(pdVolp,0,indexP-1); pdVolp=NULL;
        free_dvector(pdVolk,0,indexK-1); pdVolk=NULL;
...
------------------------------------------------------------------------*/
Err future_Rutkowski(long    index,
                     double* dPeriod,
                     double* dParams,
...
/*beta introduced as a parameter to fit the minimal correlation between 2 Libors
  depending on the time to maturity*/
    ...
    for (k=1;k<index;++k){
        tmpProd+=dPeriod[k]*dL0T[k-1]*(exp(-dDrift(index, k, dPeriod, dL0T, beta))
               ...
               double* pdFuturesRate)
{
    ...
    dBetaAdj=(1.+dL0T[index-1]*dCoverage[index-1])*exp((pdVol[index-1]*dFwd[index-1])*
        (pdVol[index-1]*dFwd[index-1])*dCumTime*dCoverage[index-1]*dCoverage[index-1]);
              ...
              double* pdFuturesRate)
{
    ...
    dAdjConv=exp(dVol*dVol*dL0T[iNumFut-1]*dL0T[iNumFut-1]*dCumTime*
...
------------------------------------------------------------------------*/
Err eFutFunc_Rut(double  dIndex,
                 double* pdParams,
                 double* pdConvexity,
                 double* pdDerivative,
                 int     nParams)
...
------------------------------------------------------------------------*/
Err eFutFunc_HJM(double dIndex,
                 double* pdParams,
                 double* pdConvexity,
                 double* pdDerivative,
                 int     nParams)
...
------------------------------------------------------------------------*/
Err eFutFunc_Bloom(double  dIndex,
                   double* pdParams,
                   double* pdConvexity,
                   double* pdDerivative,
                   int     nParams)

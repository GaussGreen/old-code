...
static Err insert_expiry_inst(double expiry_time, int swapt_idx, SWAPTIONS_DATA* g)
{
    int i, j;
    ...
    char*  refrate)
{
    ...
    if (err = swp_f_make_FixedAndNotionalsLeg(&fix_sdp, strike, 1.0, 1.0, today, &fix_leg))
    {
        return err;
    }
    ...
    if (fabs(bond_strike - 1.0) > 1.0e-08)
    {
        swp_f_freein_GenSwapLeg(&big_leg);
        return serror(
            "Forward Chey Beta PDE can only price "
    ...
    for (i = 0; i < data->ncpn; i++)
    {
        data->pay_gamma[i] = Local_Lambda_func(expiry_time, data->pay_time[i], pmdl);
    }
...
Err chey_beta_initmem_swaption_data(int ninst, SWAPTIONS_DATA* g)
{
    ...
    if (!g->data || !g->ndata_i || !g->data_i || !g->ex || !g->ndata_e || !g->data_e)
    ...
    SWAPTIONS_DATA* g)
{
    ...
    for (i = 0; i < ninst; i++)
    {
        ...
        {
            if (err = swp_f_get_ref_rate_details(refrate_str[i], &rate_basis, &rate_freq))
            {
                chey_beta_fwd_free_swaption_data(g);
                return err;
            }
            ...
            if (err = swp_f_make_SpreadLeg(start[i], end[i], today, refrate_str[i], &float_leg))
            {
                chey_beta_fwd_free_swaption_data(g);
                return err;
            }
            ...
            {
                expiry = add_unit(
                    float_leg.start_date.date[j], -spot_lag, SRT_BDAY, MODIFIED_SUCCEEDING);
                if (err = add_new_swaption(
                        pmdl,
                        yc_name,
                        today,

...
Err srt_f_cpi_forward(char *price_index_und_name, Date fixing_date,
                      Date pay_date, double *fwd_cpi) {
  Err err = NULL;
  ...
  if (mdl_type != VASICEK)
    return serror("real rate und must be of VASICEK type");
  ...
  err = srt_f_vasicek_discount_factor(settlt_date, real_rate_und,
                                      &df_real_settdt);
  if (err)
    return err;

  err = srt_f_vasicek_discount_factor(fixing_date, real_rate_und,
                                      &df_real_fixingdt);
  if (err)
    return err;
  ...
  adj = 0.0;
  adj += (Psi_func(fixing_t, nom_ts) * V_dx_func(fixing_t, cpi_ts) -
          W_dx_func(fixing_t, cpi_ts));
  adj -= (Psi_func(pay_t, nom_ts) * V_dx_func(fixing_t, cpi_ts) -
          W_dx_func(fixing_t, cpi_ts));

  adj += O_fd_func(fixing_t, cpi_ts) * Psi_func(fixing_t, real_ts) *
         (Psi_func(pay_t, nom_ts) - Psi_func(fixing_t, nom_ts));
  adj -= P_fd_func(fixing_t, cpi_ts) *
         (Psi_func(pay_t, nom_ts) - Psi_func(fixing_t, nom_ts));

  adj += 2 * Psi_func(fixing_t, nom_ts) * L_func(fixing_t, nom_ts) -
         Psi_func(fixing_t, nom_ts) * Psi_func(pay_t, nom_ts) * nom_G +
         (Psi_func(fixing_t, nom_ts) + Psi_func(pay_t, nom_ts)) *
             (Psi_func(fixing_t, nom_ts) * nom_G - L_func(fixing_t, nom_ts)) -
...
Err srt_f_vasicek_mean_rev_lvl_and_initial_cond_calib_func(
    double settlt_date, double vasicek_init_cond,

    Date ipc_date_hm, Date ipc_date_lm, double interp_coeff,
    double daily_ref_index_target,

    long num_bonds, long *num_cpns, double **pay_dates, double **cpns,
    ...
    char *real_rate_und_name, char *price_und_name,

    double ***calibrated_mean_rev_lvl, double *model_daily_ref_index) {
  Err err = NULL;
  SrtUndPtr price_index_und, nom_rate_und, real_rate_und;
  char *nom_rate_und_name, *nom_rate_yc;
  double df_nom_ipc_date_hm, df_nom_ipc_date_lm, df_real_settlt,
      df_real_ipc_date_lm, df_real_ipc_date_hm;
  ...
  err = srt_f_vasicek_calibrated_mean_rev_lvl(
      settlt_date, vasicek_init_cond, num_bonds, num_cpns, pay_dates, cpns,

      bond_dirty_prices, discount_fwd_dirty_pr,

      price_floor, floor_gearing, pre_factor_fwd, floor_strike,
      floor_implied_vol,

      real_rate_und_name, calibrated_mean_rev_lvl);
  if (err)
    return err;
  ...
  real_rate_und = lookup_und(real_rate_und_name);
  if (!real_rate_und) {
    return serror("can not find vasicek underlying in "
                  "srt_f_vasicek_mean_rev_lvl_and_initial_cond_calib_func");
  }

  /* Ge the price index underlying pointer */
  price_index_und = lookup_und(price_und_name);
  if (!price_index_und) {
    return serror("can not find price index und in "
                  "srt_f_vasicek_mean_rev_lvl_and_initial_cond_calib_func");
  }
  ...
  /* get the forward real rate discount factor */
  err = srt_f_vasicek_discount_factor((long)settlt_date, real_rate_und,
                                      &df_real_settlt);
  if (err)
    return err;

  err = srt_f_vasicek_discount_factor(ipc_date_lm, real_rate_und,
                                      &df_real_ipc_date_lm);
  if (err)
    return err;

  err = srt_f_vasicek_discount_factor(ipc_date_hm, real_rate_und,
                                      &df_real_ipc_date_hm);
  if (err)
    return err;
  ...
  (*model_daily_ref_index) = get_spot_from_fxund(price_index_und) *
                             (ipc_lm + interp_coeff * (ipc_hm - ipc_lm));

  return NULL;
}

Err srt_f_vasicek_calibrated_mean_rev_lvl_and_initial_cond(
    Date settlt_date, Date ipc_date_hm, Date ipc_date_lm, double interp_coeff,
    double daily_ref_index_target,
    ...
    char *vasicek_und_name, char *price_und_name, char *nom_swap_market,
    SRT_Boolean price_floor_boolean,

    double ***calibrated_mean_rev_lvl, double *calibrated_vasicek_init_cond) {
  Err err = NULL;
  SrtUndPtr und;
  TermStruct *ts;
  ...
  if (!und)
    return serror("can not get underlying in "
                  "srt_f_vasicek_calibrated_mean_rev_lvl_and_initial_cond");
  ...
  err = srt_f_get_vasicek_init_cond((settlt_date - date_today) * YEARS_IN_DAY,
                                    ts, &vasicek_init_cond);

  a[0] = vasicek_init_cond;
  err = srt_f_vasicek_mean_rev_lvl_and_initial_cond_calib_func(
      settlt_date, a[0],

      ipc_date_hm, ipc_date_lm, interp_coeff, daily_ref_index_target,

      num_bonds, num_cpns, pay_dates, cpns, bond_dirty_prices,
      discount_fwd_dirty_pr,

      price_floor, floor_gearing, pre_factor_fwd, floor_strike,
      floor_implied_vol,

      vasicek_und_name, price_und_name, calibrated_mean_rev_lvl, &b[0]);
  if (err)
    return err;
  ...
  err = srt_f_vasicek_mean_rev_lvl_and_initial_cond_calib_func(
      ...
      vasicek_und_name, price_und_name, calibrated_mean_rev_lvl, &b[1]);
  if (err)
    return err;
  ...
  err = srt_f_vasicek_mean_rev_lvl_and_initial_cond_calib_func(
      ...
      vasicek_und_name, price_und_name, calibrated_mean_rev_lvl, &b[2]);
  if (err)
    return err;

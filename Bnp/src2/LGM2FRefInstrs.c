...
static Err make_swaptions_payts(long nEx, long nPay, long *first_cpn_index,
  ...
  for (j = 1; j <= nEx;
       j++) /* Loop on all the swaption  , i.e loop on all the exercise dates */
  {
...

{
  ...
  /* Compute the caplets end dates (caplet_end_dates)   ,the caplets coverages
    (cvg_cap)  , the long swaptions first coverages (cvg_first)  ,
    the discount factors at the start dates (df_start) and the discount factors
    at the caplets end dates (df_cap) */
  for (j = 1; j <= nEx; j++) {
  ...
  for (i = 1; i <= nEx; i++) {
    if ((SParms.cal_method == FullCapAndDiag) ||
        (SParms.cal_method == FixKappa)) {

/*	==========================================================================
        FILE_NAME:	Fx3FBetaDLM.cxx

        PURPOSE:	Modelling of the FFX(t        ,T*) as a log-quadratic
...
Err FxBetaDLM_get_C0_from_EquiBeta(double TStar, double B0, double beta,
  ...
  if (beta < C0[2]) {
    err = "FxBetaDLM_get_C0_from_EquiBeta: the beta required is too low for "
          "the TStar and B0 entered";
    C0[0] = 0;
    C0[1] = 0;
  }
  var = 0.01 * TStar;
  ...
  if (beta == 1.0) {
    ...
    counter = 0;
    Guess =
        (1.0 - sqrt(1.0 - 4.0 * var * B0 * B0 * (beta - 1.0) * (beta - 1.0))) /
        (4.0 * var * (beta - 1.0));
    do {
      if (1.0 + 2.0 * Guess * var < 0.0) {
        Guess = -1.0 / (2.0 * var) + 0.001;
      }
      Function = (B0 * B0 + 2.0 * Guess * log(1.0 + 2.0 * Guess * var)) *
                     (beta - 1.0) -
                 2.0 * Guess;
      Derivative =
          (beta - 1.0) * (2.0 * log(1.0 + 2.0 * Guess * var) +
...
Err FxBetaDLM_get_limit_beta(double TStar, double B0, double *BetaLimit) {
  ...
  if (TStar == 0.0)
    *BetaLimit = -10E6;
    ...
    C0 = 0.25 *
         (B0 * B0 * var -
          sqrt(B0 * B0 * var * B0 * B0 * var + 4 * B0 * B0 * var)) /
         var;
...
Err FxBetaDLM_fill_str(
  ...
  err = Get_LGM_TermStructure2(undname_dom, &(str->time_dom), &(str->sig_dom),
                               &(str->nb_dom), &(str->tau_dom));
  ...
  err = Get_LGM_TermStructure2(undname_for, &(str->time_for), &(str->sig_for),
                               &(str->nb_for), &(str->tau_for));
  ...
  if (compute_3F_corr && compute_DLM_corr) {
    err = "Need to provide at least one correlation set (3F or DLM) in "
          "FxBetaDLM_fill_str";
    goto FREE_RETURN;
  }
  ...
  if (!compute_3F_corr) {
    if (nb_3F_corr == 0) {
      ...
      for (i = 0; i < str->nb_3F_corr; i++) {
        ...
        err = srt_f_get_corr_from_CorrList(cls, name_dom, name_for,
                                           str->time_3F_corr[i],
                                           &(str->dom_for_3F_corr[i]));

        if (err)
          goto FREE_RETURN;

        err = srt_f_get_corr_from_CorrList(cls, name_dom, undname_fx,
                                           str->time_3F_corr[i],
                                           &(str->dom_fx_3F_corr[i]));

        if (err)
          goto FREE_RETURN;

        err = srt_f_get_corr_from_CorrList(cls, name_for, undname_fx,
                                           str->time_3F_corr[i],
                                           &(str->for_fx_3F_corr[i]));
  ...
  if (!compute_DLM_corr) {
    if (nb_DLM_corr > 0) {
      ...
      if (!str->time_DLM_corr || !str->dom_for_DLM_corr ||
          !str->dom_fx_DLM_corr || !str->for_fx_DLM_corr ||
...
Err SrtInitFXBetaDLMUnd(
  ...
  err = FxBetaDLM_fill_str(undname_fx,  /* und name */
                           ...
                           /*	FX Underlying	*/
                           tstar, B0, C0, alpha, lambda, spot_fx, nb_fx,
                           date_fx, sigma_fx,

                           /*	Correlations	*/
                           /*	3 Factor correlations */
                           compute_3F_corr, nb_3F_corr, date_3F_corr,
                           dom_for_3F_corr, dom_fx_3F_corr, for_fx_3F_corr,

                           /*	Model correlations */
                           compute_DLM_corr, 30.0, /* max_time_correl */
                           1.0,                    /* min_time_space_correl */
                           nb_DLM_corr, date_DLM_corr, dom_for_DLM_corr,
                           dom_fx_DLM_corr, for_fx_DLM_corr,
  ...
  err = srt_f_lstins(und_list, pUndDesc->underl_name, 0.0, OBJ_PTR_UND,
                     (void *)pUndDesc, &FxBetaDLM_free_und,
...
Err FxBetaDLM_Get_TermStruct(char *und, long *today, double *tau_dom,
  ...
  *for_fx_DLM_corr_down = calloc(*nb_DLM_corr, sizeof(double));
  ...
  if (!time_dom || !sig_dom || !time_for || !sig_for || !time_fx || !sig_fx ||
      !time_3F_corr || !time_DLM_corr || !dom_for_3F_corr || !dom_fx_3F_corr ||
      !for_fx_3F_corr || !dom_for_DLM_corr || !dom_fx_DLM_corr ||
      !for_fx_DLM_corr || !dom_fx_DLM_corr_down || !for_fx_DLM_corr_down) {
  ...
  memcpy(*dom_for_DLM_corr, str->dom_for_DLM_corr,
         *nb_DLM_corr * sizeof(double));
  ...
  memcpy(*dom_fx_DLM_corr_down, str->dom_fx_DLM_corr_down,
         *nb_DLM_corr * sizeof(double));
  memcpy(*for_fx_DLM_corr_down, str->for_fx_DLM_corr_down,
         *nb_DLM_corr * sizeof(double));
...
Err init_FxBetaDLM_model(double MinTimeCorrel, double MaxTimeCorrel, long today,
  ...
  model->dCashFx = spot_fx * swp_f_df(today, spot_date, yc_dom) /
  ...
  for (i = 0; i < model->iNbPWTime; i++) {
    if (!i) {
      if (model->dPWTime[i] > MinTimeCorrel) {
        ...
        t = dt;
        for (j = 0; j < n; j++) {
          num_f_add_number(&NbTime, &model->dPWTime,
  ...
  for (i = 0; i < model->iNbPWTime; i++) {
    ...
    if (fabs(model->dLambda) < TINY) {
      model->dSigmaFxUp[i] =
          sig_fx[index] * exp(model->dAlpha * sqrt(time_fx[index]));
      ...
      model->dSigmaFxUp[i] =
          sig_fx[index] *
          exp(model->dAlpha *
              sqrt((1.0 - exp(-2.0 * model->dLambda * time_fx[index])) /
                   (2.0 * model->dLambda)));
      model->dSigmaFxDown[i] =
          sig_fx[index] *
          exp(-model->dAlpha *
  ...
  memcpy(model->dCorrForFxDown, model->dCorrForFx,
         model->iNbPWTime * sizeof(double));
  memcpy(model->dCorrDomFxDown, model->dCorrDomFx,
         model->iNbPWTime * sizeof(double));
  memcpy(model->dCorrForFxInput, model->dCorrForFx,
         model->iNbPWTime * sizeof(double));
  memcpy(model->dCorrDomFxInput, model->dCorrDomFx,
         model->iNbPWTime * sizeof(double));
...
Err initialise_FxBetaDLM_Hermite(FxBetaDLM_OptNumerParams *NumParams,
  ...
  /* hermite standard
          err = HermiteStandard(hermite->X        , hermite->W        ,
     nb_points);
  */
  gauleg(NumParams->dLowerBound, NumParams->dUpperBound, hermite->X, hermite->W,
         hermite->iNbPoints);

  for (i = 1; i <= hermite->iNbPoints; i++) {
    hermite->W[i] *=
        exp(-0.5 * hermite->X[i] * hermite->X[i]) * INV_SQRT_TWO_PI;
  }
...
Err FxBetaDLM_Allocation_Precalculations(int iNbPWTime,
  ...
  if (!Precalc->dVarRatesDom || !Precalc->dVarRatesFor ||
      !Precalc->dVarRatesFor3D || !Precalc->dCovarRates ||
      !Precalc->dCovarRates3D || !Precalc->dVarFFx || !Precalc->dCovarFFxDom ||
      !Precalc->dCovarFFxFor || !Precalc->dCovarFFxFor3D || !Precalc->dVarX ||
      !Precalc->dExpectFor || !Precalc->dIntegral || !Precalc->dIntegral2 ||
      !Precalc->dCovarRatesAdjust || !Precalc->dVarRatesFor_down ||
      !Precalc->dVarRatesFor3D_down || !Precalc->dCovarRates_down ||
      !Precalc->dCovarRates3D_down || !Precalc->dVarFFx_down ||
      !Precalc->dCovarFFxDom_down || !Precalc->dCovarFFxFor_down ||
      !Precalc->dCovarFFxFor3D_down || !Precalc->dVarX_down ||
      !Precalc->dExpectFor_down || !Precalc->dIntegral_down ||
      !Precalc->dIntegral2_down || !Precalc->dCovarRatesAdjust_down ||
      !Precalc->dVarRatesFor_mid || !Precalc->dVarRatesFor3D_mid ||
      !Precalc->dCovarRates_mid || !Precalc->dCovarRates3D_mid ||
      !Precalc->dVarFFx_mid || !Precalc->dCovarFFxDom_mid ||
      !Precalc->dCovarFFxFor_mid || !Precalc->dCovarFFxFor3D_mid ||
      !Precalc->dVarX_mid || !Precalc->dExpectFor_mid ||
      !Precalc->dIntegral_mid || !Precalc->dIntegral2_mid ||
      !Precalc->dCovarRatesAdjust_mid) {
...
/*
Calculates E[(exp(constant_coef + linear_coef X + quadratic_coef X^2) -
strike)+] where X has a normal distribution with mean and variance
*/
...
Err quadratic_bs(double constant_coef, double linear_coef,
  ...
  if (variance == 0.0) {
    if (callput == SRT_CALL) {
      *Premium = exp(constant_coef + linear_coef * mean +
                     quadratic_coef * mean * mean) -
                 strike;
      if (*Premium < 0) {
        *Premium = 0;
      }
    } else {
      *Premium = strike - exp(constant_coef + linear_coef * mean +
                              quadratic_coef * mean * mean);
      if (*Premium < 0) {
        *Premium = 0;
      }
    ...
    if (quadratic_coef >= 0.5 / variance) {
      err = serror("quadratic_bs  The process has infinite variance        , C "
                   "= %f > %f",
                   quadratic_coef, 0.5 / variance);
      goto FREE_RETURN;
    } else if (quadratic_coef == 0) {
      root_1 = (log(strike) - constant_coef) / linear_coef;
      if (callput == SRT_CALL) {
        J1 = exp(constant_coef - mean * mean / (2.0 * variance) +
                 (linear_coef * variance + mean) *
                     (linear_coef * variance + mean) / (2.0 * variance));
      ...
      delta = linear_coef * linear_coef -
              4 * quadratic_coef * (constant_coef - log(strike));
      if (delta < 0) {
        if (callput == SRT_CALL) {
          if (quadratic_coef < 0) {
            *Premium = 0;
...
Err FxBetaDLM_correl_mapping_forward(
  ...
  double b, c;
  double delta;
  double RhoDX1, RhoFX1, RhoDomFFx1, RhoForFFx1, sigFFx1, varFFx1, B1, C1,
      volS1, volX2_1, RhoDS1, RhoFS1;
  double RhoDX2, RhoFX2, RhoDomFFx2, RhoForFFx2, sigFFx2, varFFx2, B2, C2,
      volS2, volX2_2, RhoDS2, RhoFS2;
  ...
  b = RhoDF * gammaFor - gammaDom +
      RhoDomFFx * (RhoDomFFx * gammaDom - RhoForFFx * gammaFor);
  ...
  sigFFx1 = SigmaDLM * SigmaDLM + gammaDom * gammaDom + gammaFor * gammaFor +
            2.0 * RhoFX1 * gammaFor - 2.0 * RhoDX1 * gammaDom -
            2.0 * RhoDF * gammaDom * gammaFor;
  ...
  sigFFx2 = SigmaDLM * SigmaDLM + gammaDom * gammaDom + gammaFor * gammaFor +
            2.0 * RhoFX2 * gammaFor - 2.0 * RhoDX2 * gammaDom -
            2.0 * RhoDF * gammaDom * gammaFor;
  ...
  if (admissible1 && admissible2) {
    ...
    varFFx1 +=
        -2.0 * RhoFX1 * SigmaFor * Etha_Func(LambdaFor, TStar, previous_t, t) +
        2.0 * RhoDX1 * SigmaDom * Etha_Func(LambdaDom, TStar, previous_t, t);
    varFFx1 += SigmaFor * SigmaFor *
                   Psi_Func(LambdaFor, LambdaFor, TStar, previous_t, t) +
               SigmaDom * SigmaDom *
                   Psi_Func(LambdaDom, LambdaDom, TStar, previous_t, t) -
    ...
    volX2_1 += gammaDom * gammaDom + gammaFor * gammaFor -
               2.0 * RhoDF * gammaDom * gammaFor;

    volS1 =
        B1 * B1 * SigmaDLM * SigmaDLM +
    ...
    varFFx2 +=
        -2.0 * RhoFX2 * SigmaFor * Etha_Func(LambdaFor, TStar, previous_t, t) +
        2.0 * RhoDX2 * SigmaDom * Etha_Func(LambdaDom, TStar, previous_t, t);
    varFFx2 += SigmaFor * SigmaFor *
                   Psi_Func(LambdaFor, LambdaFor, TStar, previous_t, t) +
               SigmaDom * SigmaDom *
                   Psi_Func(LambdaDom, LambdaDom, TStar, previous_t, t) -
               2.0 * RhoDF * SigmaDom * SigmaFor *
    ...
    volX2_2 += gammaDom * gammaDom + gammaFor * gammaFor -
               2.0 * RhoDF * gammaDom * gammaFor;

    volS2 =
        B2 * B2 * SigmaDLM * SigmaDLM +
  ...
  if (RhoDX1 != *RhoDX || RhoFX1 != *RhoFX) {
    ...
    if (sigFFx < 0.0 || fabs(RhoDomFFx1 - RhoDomFFx) > CorrelTolerance ||
        fabs(RhoForFFx1 - RhoForFFx) > CorrelTolerance) {
      err = "FxBetaDLM_correl_mapping: No solution for correlation mapping "
            "after adapting the correlations";
      goto FREE_RETURN;
    }
  }
...
Err FxBetaDLM_correl_mapping8(double t, double TStar, double SigmaDom,
                              double LambdaDom, double SigmaFor,
                              double LambdaFor, double Sigma3F, double SigmaDLM,
                              double RhoDF, double RhoDS, double RhoFS,
                              double *RhoDX, double *RhoFX) {
  double gammaDom, gammaFor;
  double sigFFx, RhoDomFFx, RhoForFFx;
...
Err FxBetaDLM_correl_mapping(double volX, double B0, double C0, double TStar,
  ...
  do {
    ...
    *RhoFX = RhoFX1;
    ...
    if (fabs(RhoDS) > 1.0E-08) {
      ...
      c = pow(RhoDF * vol_for + vol_dom, 2) -
          pow(RhoDS * volX, 2) * (B * B + Const1) -
          RhoDS * RhoDS * ((B - 1.0) * (B - 1.0) + Const1) *
              (volFor * volFor - 2.0 * RhoDF * volDom * volFor +
               volDom * volDom) -
          2.0 * RhoDS * vol_for * (B - 1.0 + Const2) *
              (RhoFS * (volFor * RhoDF - volDom) -
               RhoDS * (volFor - volDom * RhoDF));

      delta = b * b - a * c;

      if (delta < 0.0) {
        err = serror(
            "No solution for correlation mapping        , delta < 0 for t = %f",
            t);
        return err;
      }
      ...
      /*			if (fabs(RhoDX1) > vol_X * 0.999)
                              {
                                      err = "No solution for correlation mapping
               , correlation dom X not in [-1;1]"; return err;
                              }*/

      RhoFX1 = RhoFS / RhoDS * (vol_dom + RhoDF * vol_for + RhoDX1) -
               RhoDF * vol_dom - vol_for;

      /*			if (fabs(RhoFX1) > vol_X * 0.999)
                              {
                                      err = "No solution for correlation mapping
               , correlation for X not in [-1;1]"; return err;
                              }*/
      ...
      c = pow(RhoDF * vol_dom, 2) + vol_for * vol_for +
          RhoFS * RhoFS *
              (vol_dom * vol_dom - vol_for * vol_for - vol_X * vol_X) +
    ...
    VarFFx += -2.0 * RhoFX1 * SigmaFor * volX *
                  Etha_Func(LambdaFor, TStar, previous_t, middle_t) +
              2.0 * RhoDX1 * SigmaDom * volX *
                  Etha_Func(LambdaDom, TStar, previous_t, middle_t);
    VarFFx += SigmaFor * SigmaFor *
                  Psi_Func(LambdaFor, LambdaFor, TStar, previous_t, middle_t) +
              SigmaDom * SigmaDom *
                  Psi_Func(LambdaDom, LambdaDom, TStar, previous_t, middle_t) -
              2.0 * RhoDF * SigmaDom * SigmaFor *
  ...
  } while ((fabs(*RhoDX - RhoDX1) > tolerance &&
            fabs(*RhoFX - RhoFX1) > tolerance) ||
           iteration < 2);
...
/* Finds the Fix Point Solution */
Err FxBetaDLM_correl_mapping4(int index, double VarFFx, double volX,
                              double RhoDS, double RhoFS,
                              FxBetaDLM_model *model, double *RhoDX,
                              double *RhoFX) {
  Err err = NULL;
  ...
  do {
    ...
    *RhoFX = RhoFX1;
    ...
    vol_dom = -model->dSigmaDom[index] *
              (1.0 - exp(-model->dLambdaDom * (model->dTStar - t))) /
              model->dLambdaDom;
    vol_for = -model->dSigmaFor[index] *
              (1.0 - exp(-model->dLambdaFor * (model->dTStar - t))) /
              model->dLambdaFor;
    ...
    if (fabs(RhoDS) > 1.0E-08) {
      /* solve polynomial */
      a = 1.0;
      b = vol_dom + RhoDF * vol_for - RhoDS * RhoDS * vol_dom -
          RhoDS * RhoFS * vol_for;
      c = vol_dom * vol_dom + pow(RhoDF * vol_for, 2) +
          2.0 * RhoDF * vol_dom * vol_for - pow(RhoDS * vol_dom, 2) +
          pow(RhoDS * vol_for, 2) - pow(RhoDS * vol_X, 2) -
      ...
      if (fabs(RhoDX1) > vol_X * 0.999) {
        err = "No solution for correlation mapping        , correlation dom X "
              "not in "
              "[-1;1]";
        return err;
      }
      ...
      if (fabs(RhoFX1) > vol_X * 0.999) {
        err = "No solution for correlation mapping        , correlation For|X "
              "not in "
              "[-1;1]";
        return err;
      }
    ...
    VarFFx +=
        -2.0 * RhoFX1 * model->dSigmaFor[index] * volX *
    ...
    VarFFx += model->dSigmaFor[index] * model->dSigmaFor[index] *
                  Psi_Func(model->dLambdaFor, model->dLambdaFor, model->dTStar,
                           previous_t, next_t) +
              model->dSigmaDom[index] * model->dSigmaDom[index] *
                  Psi_Func(model->dLambdaDom, model->dLambdaDom, model->dTStar,
                           previous_t, next_t) -
              2.0 * RhoDF * model->dSigmaDom[index] * model->dSigmaFor[index] *
                  Psi_Func(model->dLambdaDom, model->dLambdaFor, model->dTStar,
                           previous_t, next_t);
...
Err FxBetaDLM_correl_mapping3(int index, double VarFFx, double volX,
                              double RhoDS, double RhoFS,
                              FxBetaDLM_model *model, double *RhoDX,
                              double *RhoFX) {
  Err err = NULL;
  ...
  for (step = 0; step < 2; step++) {
    if ((1.0 + 2.0 * model->dC0 * VarFFx) <= 0.0) {
      err = serror("FxBetaDLM_correl_mapping: The cumulative variance is too "
                   "high for t = %f",
                   t);
      return err;
    }
    ...
    if (fabs(RhoDS) > 1.0E-08) {
      ...
      if (fabs(RhoDX1) > vol_X * 0.999) {
        err = serror(
            "No solution for correlation mapping        , correlation dom_X "
            "not in [-1;1] for t = %f",
            t);
        return err;
      }
      ...
      if (fabs(RhoFX1) > vol_X * 0.999) {
        err = serror(
            "No solution for correlation mapping        , correlation for_X "
            "not in [-1;1] for t = %f",
            t);
        return err;
      }
    ...
    if (step == 0) {
      ...
      VarFFx += model->dSigmaFor[index] * model->dSigmaFor[index] *
                    Psi_Func(model->dLambdaFor, model->dLambdaFor,
                             model->dTStar, t, next_t) +
                model->dSigmaDom[index] * model->dSigmaDom[index] *
                    Psi_Func(model->dLambdaDom, model->dLambdaDom,
                             model->dTStar, t, next_t) -
                2.0 * RhoDF * model->dSigmaDom[index] *
                    model->dSigmaFor[index] *
  ...
  *RhoFX = 0.5 * (RhoFX1 + RhoFX2);
  ...
  /* check */
  err = Get_Correl_FromModel(index, VarFFx, volX, *RhoDX, *RhoFX, model,
                             &TestRhoDX, &TestRhoFX);
...
Err FxBetaDLM_correl_mapping2(int index, double VarFFx, double VolX,
                              double RhoSD, double RhoSF,
                              FxBetaDLM_model *model, double *RhoXD,
                              double *RhoXF) {
  Err err = NULL;
  double RhoSD1, RhoSD2, RhoSF1, RhoSF2, RhoXD1, RhoXD2, RhoXD3, RhoXF1, RhoXF2,
      RhoXF3;
  ...
  /* First Pricing */
  err = Get_Correl_FromModel(index, VarFFx, VolX, RhoXD1, RhoXF1, model,
                             &RhoSD1, &RhoSF1);
  ...
  while (nb_iter < 10 && error > 1.0E-03) {
    ...
    err = Get_Correl_FromModel(index, VarFFx, VolX, RhoXD2, RhoXF2, model,
                               &RhoSD2, &RhoSF2);
    ...
    if (fabs(RhoXD3) < 0 || fabs(RhoXF3) > 0.9999) {
      ...
      if (RhoXF3 < -0.9999) {
        coef2 =
            (RhoXF1 + 0.99) / ((-sens12 * error1 + sens11 * error2) / delta);
...
Err Get_Correl_FromModel(int index, double VarFFx, double VolX, double RhoXD,
  ...
  vol_X2 += gamma_dom * gamma_dom + gamma_for * gamma_for -
            2.0 * RhoDF * gamma_dom * gamma_for;
...
Err FxBetaDLM_GetFirstGuessFromB0(double B0, double Tstar, double *exercise_opt,
  ...
  for (i = 0; i < nbrOpt; i++) {
    ...
    for (j = start_index; j < end_index + 1; j++) {
      ...
      Integral -= B0minus1 * 2.0 * correl_dom_for[j] * sig_curve_dom[j] *
                  sig_curve_for[j] * Psi_Func(lda_dom, lda_for, Tstar, t1, t2);
    }

    fx_vol_curve[i] = (cal_vol_3f[i] * cal_vol_3f[i] +
                       B0minus1 * Integral / (end_mat - start_mat));
...
Err FxBetaDLM_GetModelFromStr(FXBETADLM_STR str, FxBetaDLM_model *model) {
  ...
  if (!model->dPWTime || !model->dSigmaDom || !model->dSigmaFor ||
      !model->dSigmaFx || !model->dSigmaFxUp || !model->dSigmaFxDown ||
      !model->dCorrDomFor || !model->dCorrDomFx || !model->dCorrForFx ||
      !model->dCorrDomFxDown || !model->dCorrForFxDown ||
      !model->dCorrDomFxInput || !model->dCorrForFxInput) {
  ...
  for (i = 0; i < model->iNbPWTime; i++) {
    ...
    if (fabs(model->dLambda) < TINY) {
      ...
      model->dSigmaFxUp[i] =
          str->sig_fx[index] *
          exp(model->dAlpha *
              sqrt((1.0 - exp(-2.0 * model->dLambda * str->time_fx[index])) /
                   (2.0 * model->dLambda)));
      model->dSigmaFxDown[i] =
          str->sig_fx[index] *
          exp(-model->dAlpha *
  ...
  memcpy(model->dCorrForFx, str->for_fx_DLM_corr,
         model->iNbPWTime * sizeof(double));
  memcpy(model->dCorrDomFx, str->dom_fx_DLM_corr,
         model->iNbPWTime * sizeof(double));
  memcpy(model->dCorrForFxDown, str->for_fx_DLM_corr_down,
         model->iNbPWTime * sizeof(double));
  memcpy(model->dCorrDomFxDown, str->dom_fx_DLM_corr_down,
         model->iNbPWTime * sizeof(double));
  memcpy(model->dCorrDomFor, str->dom_for_DLM_corr,
         model->iNbPWTime * sizeof(double));
...
Err FxBetaDLM_get3FCorrelFromDLM(FXBETADLM_STR str) {
  ...
  if (!str->dom_for_3F_corr || !str->time_3F_corr || !str->dom_fx_3F_corr ||
      !str->for_fx_3F_corr) {
    err = "Memory allocation faillure in FxBetaDLM_get3FCorrelFromDLM";
    goto FREE_RETURN;
  }

  memcpy(str->dom_for_3F_corr, str->dom_for_DLM_corr,
         str->nb_DLM_corr * sizeof(double));
  memcpy(str->time_3F_corr, str->time_DLM_corr,
         str->nb_DLM_corr * sizeof(double));
  ...
  for (i = 0; i < str->nb_DLM_corr; i++) {
    ...
    var_ffx +=
        sig_for2 * Psi_Func(for_lam, for_lam, tstar, t1, middle_t) +
    ...
    vol_X2 = sig_fx2;
    vol_X2 +=
        2.0 * RhoXF * gamma_for * sig_fx - 2.0 * RhoXD * gamma_dom * sig_fx;
    vol_X2 += gamma_dom * gamma_dom + gamma_for * gamma_for -
              2.0 * RhoDF * gamma_dom * gamma_for;
    ...
    str->dom_fx_3F_corr[i] =
        B * sig_fx * RhoXD + (B - 1.0) * (gamma_for * RhoDF - gamma_dom);
    str->dom_fx_3F_corr[i] /= volS;
    str->for_fx_3F_corr[i] =
        B * sig_fx * RhoXF + (B - 1.0) * (gamma_for - gamma_dom * RhoDF);
    ...
    var_ffx +=
        sig_for2 * Psi_Func(for_lam, for_lam, tstar, middle_t, t2) +
...
Err FxBetaDLM_getDLMCorrelFrom3F(FXBETADLM_STR str,
  ...
  num_f_concat_vector(&str->nb_DLM_corr, &str->time_DLM_corr, str->nb_dom,
                      str->time_dom);
  num_f_concat_vector(&str->nb_DLM_corr, &str->time_DLM_corr, str->nb_for,
                      str->time_for);
  num_f_concat_vector(&str->nb_DLM_corr, &str->time_DLM_corr, str->nb_3F_corr,
                      str->time_3F_corr);
  ...
  if (str->time_DLM_corr[str->nb_DLM_corr - 1] < NumParams->dMaxTimeCorrel) {
    num_f_add_number(&str->nb_DLM_corr, &str->time_DLM_corr,
  ...
  for (i = 0; i < str->nb_DLM_corr; i++) {
    if (!i) {
      if (str->time_DLM_corr[i] > NumParams->dMinTimeSpaceCorrel) {
        ...
        t = dt;
        for (j = 0; j < n; j++) {
          num_f_add_number(&NbTime, &str->time_DLM_corr,
                           (double)(((long)(t * DAYS_IN_YEAR)) * YEARS_IN_DAY));
          t += dt;
        }
      }
    } else {
      if (str->time_DLM_corr[i] - str->time_DLM_corr[i - 1] >
          NumParams->dMinTimeSpaceCorrel) {
        n = (int)((str->time_DLM_corr[i] - str->time_DLM_corr[i - 1]) /
                  NumParams->dMinTimeSpaceCorrel);
  ...
  if (!str->dom_for_DLM_corr || !str->dom_fx_DLM_corr ||
      !str->for_fx_DLM_corr || !str->dom_fx_DLM_corr_down ||
      !str->for_fx_DLM_corr_down) {
  ...
  for (i = 0; i < str->nb_DLM_corr; i++) {
    ...
    err = FxBetaDLM_correl_mapping(
        str->sig_fx[indexSigFx], str->b0, str->c0, str->tstar,
        str->sig_dom[indexSigDom], str->lam_dom, str->sig_for[indexSigFor],
        str->lam_for, str->dom_for_3F_corr[indexCorr],
        str->dom_fx_3F_corr[indexCorr], str->for_fx_3F_corr[indexCorr], t1, t2,
        varFFx, &(str->dom_fx_DLM_corr[i]), &(str->for_fx_DLM_corr[i]));
    if (err)
      goto FREE_RETURN;
    ...
    varFFx += -2.0 * str->for_fx_DLM_corr[i] * str->sig_for[indexSigFor] *
                  ...
                  Etha_Func(str->lam_for, str->tstar, t1, t2) +
              2.0 * str->dom_fx_DLM_corr[i] * str->sig_dom[indexSigDom] *
                  str->sig_fx[indexSigFx] *
                  Etha_Func(str->lam_dom, str->tstar, t1, t2);
    varFFx += str->sig_for[indexSigFor] * str->sig_for[indexSigFor] *
              ...
              2.0 * str->dom_for_3F_corr[indexCorr] *
                  str->sig_dom[indexSigDom] * str->sig_for[indexSigFor] *
...
Err FxBetaDLM_CheckOrMakePosMatrix2(double RhoDF, double *RhoDX,
                                    double *RhoFX) {
  Err err = NULL;
  double check;
  double vp = 1.0;
  ...
  if (check > det_tol || fabs(RhoDF - 1.0) < 1.0e-6)
    ...
    err =
        diagonalise_symmetric_matrix(correl_matrix, 3, eigen_val, eigen_vector);
...
Err FxBetaDLM_CheckOrMakePosMatrix(double RhoDF, double *RhoDX, double *RhoFX) {
  ...
  if (check > 0.0001 || fabs(RhoDF - 1.0) < 1.0e-6)
    ...
    if (*RhoDX * *RhoFX > 0.0) {
      ...
      delta =
          (*RhoDX + *RhoFX) * (*RhoDX + *RhoFX) - 2.0 * check / (RhoDF - 1.0);
      ...
      delta =
          (*RhoDX - *RhoFX) * (*RhoDX - *RhoFX) + 2.0 * check / (1.0 + RhoDF);
...
Err copy_FxBetaDLM_model(FxBetaDLM_model *model_source,
  ...
  if (!model_dest->dPWTime || !model_dest->dSigmaDom ||
      !model_dest->dSigmaFor || !model_dest->dSigmaFx3F ||
      !model_dest->dSigmaFx || !model_dest->dSigmaFxUp ||
      !model_dest->dSigmaFxDown || !model_dest->dCorrDomFor ||
      !model_dest->dCorrDomFx || !model_dest->dCorrForFx ||
      !model_dest->dCorrDomFxDown || !model_dest->dCorrForFxDown ||
  ...
  /* Copy the vectors */
  memcpy(model_dest->dPWTime, model_source->dPWTime,
         model_dest->iNbPWTime * sizeof(double));
  memcpy(model_dest->dSigmaDom, model_source->dSigmaDom,
         model_dest->iNbPWTime * sizeof(double));
  memcpy(model_dest->dSigmaFor, model_source->dSigmaFor,
         model_dest->iNbPWTime * sizeof(double));
  memcpy(model_dest->dSigmaFx, model_source->dSigmaFx,
         model_dest->iNbPWTime * sizeof(double));
  memcpy(model_dest->dSigmaFx3F, model_source->dSigmaFx3F,
         model_dest->iNbPWTime * sizeof(double));
  memcpy(model_dest->dSigmaFxUp, model_source->dSigmaFxUp,
         model_dest->iNbPWTime * sizeof(double));
  memcpy(model_dest->dSigmaFxDown, model_source->dSigmaFxDown,
         model_dest->iNbPWTime * sizeof(double));
  memcpy(model_dest->dCorrDomFor, model_source->dCorrDomFor,
         model_dest->iNbPWTime * sizeof(double));
  memcpy(model_dest->dCorrDomFx, model_source->dCorrDomFx,
         model_dest->iNbPWTime * sizeof(double));
  memcpy(model_dest->dCorrForFx, model_source->dCorrForFx,
         model_dest->iNbPWTime * sizeof(double));
  memcpy(model_dest->dCorrForFxDown, model_source->dCorrForFxDown,
         model_dest->iNbPWTime * sizeof(double));
  memcpy(model_dest->dCorrDomFxDown, model_source->dCorrDomFxDown,
         model_dest->iNbPWTime * sizeof(double));
  memcpy(model_dest->dCorrForFxInput, model_source->dCorrForFxInput,
         model_dest->iNbPWTime * sizeof(double));
  memcpy(model_dest->dCorrDomFxInput, model_source->dCorrDomFxInput,
         model_dest->iNbPWTime * sizeof(double));

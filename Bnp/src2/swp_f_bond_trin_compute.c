...
void add_step(Deal contract, Model model, int index_tree,int index_date ,
			Step *tree_steps);
void date_treatment(Deal contract,Model model,Model model_fx,Step *tree_steps,
			Step *tree_steps_fx, int *step_num, int *step_fx_num);
...
double fwd_clean_fct	(       Ddate		dstart,
				Ddate 		fut, 
				String 		swap, 
				String 		repo, 
				Deal        	contract)  
...
void shift_all_following_steps(Model model,int index, Step *step, int *step_num)
{
int i;
...
double t_n_y;
	...
	step[index_tree].df_swap =df_crv(
					(Ddate)(contract.today),
					(Ddate)(contract.date[index_date]),
					crv);
	t_n_y = (double)((contract.date[index_date]
				- contract.today)/365.0);
...
*/
	...
	step[index_tree].cpn=contract.cpn[index_date];
       	step[index_tree].expected_spot = 
				...
				(double)contract.today+t_n_y*365.0,
				model.swap,
				model.repo,
				contract);
...
void date_treatment(	Deal contract,
			Model model,
			Model model_fx,
			Step *step,
			Step *step_fx, 
...
int i,j;
	...
	for (i=0;i<contract.size_date_cpn;i++)
		...
		for (j=0;j<=*step_num;j++)
        	{
		if (contract.date[i]<=(double)(365.0*step[j].time_start+
						contract.today))
				break;
		}
		
		if ((j>0)&&((j<*step_num)&&(contract.date[i]<(double)
				(365.0*step[j].time_start+contract.today)))) 
		{	
		    ...
		    if (contract.quanto==1)
        	     {
		      	...
		      	add_step(contract,model_fx,j,i,step_fx);
         	      	step[j].expected_spot *= exp(contract.corr_fx
				*sqrt(step[j].cum_vol*step_fx[j].cum_vol));
		...
		{	
		    if ((j>0)&&((j<*step_num)&&(contract.date[i]==
...
double t_n,t_n_y,vol_max,interm;
	...
	model->date_cumvol[0][0] = (double) ( ( model->tsv[0][0] - 
					contract.today) / 365.0 );
	model->date_cumvol[1][0] = model->tsv[1][0] * model->tsv[1][0] *
					model->date_cumvol[0][0];
	vol_max = model->tsv[1][0];

	for (i=1;i<model->size_tsv;i++)
	{
		model->date_cumvol[0][i] = (double)((model->tsv[0][i] - 
					contract.today)/365);
		model->date_cumvol[1][i] = model->tsv[1][i] * model->tsv[1][i]
					* model->date_cumvol[0][i];
		vol_inst[i] = sqrt((model->date_cumvol[1][i] - 
					model->date_cumvol[1][i-1]) /
					(model->date_cumvol[0][i] - 
					model->date_cumvol[0][i-1]));

		if(vol_inst[i]>vol_max) 	vol_max=vol_inst[i];
...
DEBUG_VEGA && printf("1 vega=%f \n",*vega); 
	if ( (contract.ret_type == 3) || (contract.ret_type == 4) )
		{
		if (*vega > EPSILON)	model->delta_lns = *vega;
		else			*vega = interm;
...
*/
	...
	model->df_swap_all = df_crv((Ddate)(contract.today),
					(Ddate)(contract.opt_mat),
...
*/		step[j].cum_vol= eq_cum_vol_func((double)t_n_y,SRT_YES,model->ts);
		step[j].time_start=t_n_y;
	 	step[j].flag_date=0;
        	step[j].df_swap = df_crv((Ddate)(contract.today),
					(Ddate)t_n,
					lookup_curve(model->swap) );
	...
	/*******  for extinguish option hit_level position  *******/	  
	    ...
	    || (contract.type==EURO_DIGIT)	
	    || (contract.type==PAY_DIGIT) ) 

	{ 

	model->hit_level = (int)((2*contract.do_up-1)*
					floor(log(contract.barrier/
					contract.spot)*(2.0*contract.do_up-
					1.0)/model->delta_lns));
       
       /* Hit level in case of yield barriers  */
       /* will be defined in step_init (later) */

DEBUG_PRINT && printf("barrier=%f  spot=%f  ",contract.barrier,contract.spot); 
...
Step *test=step;
	...
	step->p_d = (m*m + var - model.delta_lns * m) / ( 2 * model.delta_lns *
					model.delta_lns );
	step->p_u = (m*m + var + model.delta_lns * m) / ( 2 * model.delta_lns *
					model.delta_lns );
...
SwapDP p_temp;
	...
	step[n].time_end=(double)((contract.opt_mat-contract.today)/365.0
					-step[n].time_start); 
	step[n].expected_spot = fwd_clean_fct((double)contract.p.start,
					(double)contract.today
					+step[n].time_start*365.0,
					model.swap,
					model.repo,
                                        contract);
    ...
    if(contract.yield_strike_flag==1)
        {
         ...
         step[n].clean_strike = 100*clean_price_fct ( p_temp,
                                                contract.coupon/100,
                                                contract.yield_strike/100,
                                                1,
                                                contract.coupon/100);
        }
    ...
    if(contract.yield_barrier_flag==1)
        {
         p_temp = contract.p;
         p_temp.start = (int)(floor(contract.p.start+step[n].time_start*365));  
         step[n].clean_barrier = 100*clean_price_fct ( p_temp,
                                                contract.coupon/100,
	    ...
	    || (contract.type==PAY_DIGIT) ) 
		{ 
		step[n].yield_hit_level = (int)((2*contract.do_up-1)*
					floor(log(step[n].clean_barrier/
					contract.spot)*(2.0*contract.do_up-
					1.0)/model.delta_lns));        
        	}
...
YIELD_DEBUG && printf("time start =%f \n",step[n].time_start);
    ...
    if(contract.quanto==1)
	   ...
	   step[n].expected_spot *= exp(contract.corr_fx
				*sqrt( step[n].cum_vol*step_fx[n].cum_vol));
...
H_DEBUG && printf("init steps spot  new =%f \n", step[n].expected_spot);
	...
	if (n<model.step_num)
	   {
		...
		step[n].vol=sqrt((model.cum_vol_opt-step[n].cum_vol)/
					step[n].time_end);
	...
	if ((n==model.step_num)&&(contract.sub_period!=contract.opt_mat))
	{   	/****	Still need a vol for B-S at the end of the tree	****/
		step[n].vol=sqrt((model.cum_vol_opt-step[n].cum_vol)/
					step[n].time_end);     
	}
...
if (   ( (contract.type==EXTINGUISH)
     ...
     || (contract.type==EURO_DIGIT)  
     || (contract.type==PAY_DIGIT) )
     && (((node->position==model.hit_level)&&(contract.yield_barrier_flag==0))
 ||((node->position==step->yield_hit_level)&&(contract.yield_barrier_flag==1))))
     
    {
        if(contract.yield_barrier_flag==1) contract.barrier=step->clean_barrier;
	...
	absorb_moment[0] = contract.barrier * (q2 (- do_up, m1,ft) - 
					fe * q2(-do_up, m2, ft));   
	absorb_moment[0]/= absorb_prob;
	absorb_moments[0] = absorb_moment[0] * absorb_prob + contract.barrier *
					 (1 - absorb_prob);  
	absorb_moments[0] /= node->asset;

	absorb_moment[1] = contract.barrier * contract.barrier * (q2 (-2 * 
					do_up, m1, ft) - fe * 
					q2 (-2*do_up, m2, ft)) / absorb_prob;  

	absorb_moments[1] = absorb_moment[1] * absorb_prob+ contract.barrier *
 					contract.barrier * (1 - absorb_prob); 
	absorb_moments[1] /= node->asset*node->asset;                  
                                                                               
        if (do_up == 1)	/* Up */
	{
		node->p_d = ( absorb_moments[1] - absorb_moments[0] * 
					absorb_moments[0])+ (contract.barrier /
					node->asset - absorb_moments[0]) * 
					(1 - absorb_moments[0]);
		node->p_d /= (1 - d) * (contract.barrier / node->asset - d);
		
		node->p_m  = (absorb_moments[1] - absorb_moments[0] * 
					absorb_moments[0])+ (contract.barrier /
					 node->asset- absorb_moments[0]) * 
					(d - absorb_moments[0]);
...
double bond_trinomial_fct(Deal contract,Model model,Model model_fx,double *vega)
{ 
...
prem_prev=calloc( 2*model.step_num+50 , sizeof(double) );
	if (!prem_prev)	printf("erreur chez prem_prev\n"); 
                       
prem_cur=calloc( 2*model.step_num+50 , sizeof(double) );
	if (!prem_cur) 	printf("erreur chez prem_cur\n"); 

steps=calloc( model.step_num+50 , sizeof( Step ) );  
	if (!steps) 	printf("erreur chez steps\n");
...
model.fwd_opt_mat = fwd_clean_fct(	(double)contract.p.start,
					contract.opt_mat,
					model.swap,
					model.repo,
					contract);
...
H_DEBUG && printf(" \n starts quantos with corr =%f \n ",contract.corr_fx);
	...
	if (!steps_fx) 	printf("erreur chez steps_fx\n");

       tree_init(contract,&model_fx,steps_fx,vega); 

H_DEBUG && printf("old forward mat =%f \n",model.fwd_opt_mat);

       model.fwd_opt_mat *= exp(contract.corr_fx
			 *sqrt(model.date_cumvol[1][model.mat_vol_nb]
...
date_treatment(contract,model,model_fx,steps,steps_fx,
	       	&model.step_num,&model_fx.step_num);
...
node.forward = node.asset*model.fwd_opt_mat/steps[model.step_num].expected_spot;
...
/*****	whereas in the bitrin it is   0 to 2*(n+1)		*****/
      	...
      	node.intrinsic = (node.asset - contract.strike) * 
					(1-2*contract.call_put);
...
/***** 	top node : +n  *****/
	...
	if  (d0>=0)  d0=0;	/* leads to undefined node because node -1 */
...
/***	prem_prev not changed for n=1 because of greeks calculation	***/
	...
	if (n>1)
	{                                             
	for(k=0; k<= 2*n; k++) prem_prev[k] = prem_cur[k];	 
	}
	
	if (n==1) 	price = prem_cur[1];
...
if (contract.ret_type==1) return(delta);
else 
	if (contract.ret_type==2) return(gamma);
	else
	    	if (contract.ret_type==5) return(theta_1d);
	    	else
			return(price);
...
result = srt_f_optblksch(node.forward,contract.strike,
					step.vol,
...
if(node.intrinsic > 0)
	...
	result += srt_f_optblksch(node.forward,
					node.asset,
					step.vol,
			(double)(contract.opt_mat-contract.sub_period)/365.0,
					step.df_swap_end,
...
if(node.intrinsic > 0)
	...
	prem_val += srt_f_optblksch(node.forward,
					node.asset,
					step.vol,
					step.time_end,
					step.df_swap_end,
...
else    
{
	...
	if (node.intrinsic > 0)
	{
		...
		{
			result = ((node.asset+contract.barrier)*0.5 - 
					contract.strike );
			result *= (contract.barrier-node.asset)/(node.asset*
					(1-exp(-model.delta_lns))); 
		}
		else	result = node.intrinsic;
	}
	else	result = 0.0;
    }
    else  
...
call_price = srt_f_optblksch(node.forward,
				contract.strike,
				step.vol,
			(double)(contract.opt_mat-contract.sub_period)/365.0,
				step.df_swap_end,
...
put_price = srt_f_optblksch(node.forward,
				contract.barrier,
				step.vol,
			(double)(contract.opt_mat-contract.sub_period)/365.0,
				step.df_swap_end,
...
/************** not a very good smoothing of payoff cf probas **/ 
	...
	{			
	 result = (node.asset-contract.barrier)* (1-2*contract.do_up);
         result += model.diff_do*contract.do_up 
		   + model.diff_up*(1-contract.do_up); 
         result /= model.diff_do*contract.do_up 
		   + model.diff_up*(1-contract.do_up); 

...
double fwd_clean_fct(Ddate dstart, Ddate fut, String swap, String repo, Deal contract);
...
void   shift_all_following_steps(Model model, int index, Step* tree_steps, int* step_num);
void   add_step(Deal contract, Model model, int index_tree, int index_date, Step* tree_steps);
void   date_treatment(
      Deal  contract,
      Model model,
      Model model_fx,
      ...
      int*  step_num,
      int*  step_fx_num);
void   init_step(int n, Deal contract, Model model, Model model_fx, Step* step, Step* step_fx);
...
double fwd_clean_fct(Ddate dstart, Ddate fut, String swap, String repo, Deal contract)
{
    ...
    {
        sum_coup +=
            (c / contract.p.compd) / df_crv((Ddate)(list.date[i]), (Ddate)fut, lookup_curve(swap));
        c = contract.coupon;
    }
...
void tree_choose(Deal* contract, Model* model, Func* func)
{
    if (contract->type == EUROPEAN)
    {
        ...
        model->h = (double)((contract->opt_mat - contract->today) / (model->step_num * 365.0));
    ...
    else if (contract->type == SHOUT_LOCK)
    {
        ...
        model->h = (double)((contract->sub_period - contract->today) / (model->step_num * 365.0));
        contract->size_date_cpn = 0;
    }
...
void add_step(Deal contract, Model model, int index_tree, int index_date, Step* step)
{
    double    t_n_y;
    SrtCrvPtr crv;
    ...
    int*  step_fx_num)
{
    ...
    if ((contract.type == INSTALMENT) || (contract.type == MID_ATLANTIC))
    {
        for (i = 0; i < contract.size_date_cpn; i++)
        {
            ...
            {
                if (contract.date[i] <= (double)(365.0 * step[j].time_start + contract.today))
                    break;
            }

            if ((j > 0) &&
                ((j < *step_num) &&
                 (contract.date[i] < (double)(365.0 * step[j].time_start + contract.today))))
            {
                ...
                if (contract.quanto == 1)
                {
                    ...
                    step[j].expected_spot *=
                        exp(contract.corr_fx * sqrt(step[j].cum_vol * step_fx[j].cum_vol));
...
void tree_init(Deal contract, Model* model, Step* step, double* vega)
{
    ...
    model->date_cumvol[0][0] = (double)((model->tsv[0][0] - contract.today) / 365.0);
    model->date_cumvol[1][0] = model->tsv[1][0] * model->tsv[1][0] * model->date_cumvol[0][0];
    vol_max                  = model->tsv[1][0];

    for (i = 1; i < model->size_tsv; i++)
    {
        model->date_cumvol[0][i] = (double)((model->tsv[0][i] - contract.today) / 365);
        model->date_cumvol[1][i] = model->tsv[1][i] * model->tsv[1][i] * model->date_cumvol[0][i];
        vol_inst[i]              = sqrt(
            (model->date_cumvol[1][i] - model->date_cumvol[1][i - 1]) /
    ...
    */
    ...
    model->df_swap_all =
        df_crv((Ddate)(contract.today), (Ddate)(contract.opt_mat), lookup_curve(model->swap));
    ...
    for (j = 0; j <= model->step_num; j++) /***  at each step  ***/
    {
        ...
        */
        ...
        step[j].flag_date  = 0;
        step[j].df_swap    = df_crv((Ddate)(contract.today), (Ddate)t_n, lookup_curve(model->swap));
...
void calc_proba(Model model, Step* step)
{
    ...
    step->p_d = (m * m + var - model.delta_lns * m) / (2 * model.delta_lns * model.delta_lns);
    step->p_u = (m * m + var + model.delta_lns * m) / (2 * model.delta_lns * model.delta_lns);
    step->p_m = 1 - step->p_u - step->p_d;

    DEBUG_PRINT&& printf("proba up:%f  mid:%f  down:%f \n", step->p_u, step->p_m, step->p_d);
}
...
void init_step(int n, Deal contract, Model model, Model model_fx, Step* step, Step* step_fx)
{
    ...
    step[n].time_end = (double)((contract.opt_mat - contract.today) / 365.0 - step[n].time_start);
    step[n].expected_spot = fwd_clean_fct(
        (double)contract.p.start,
    ...
    if (contract.yield_strike_flag == 1)
    {
        ...
        step[n].clean_strike = 100 * clean_price_fct(
                                         p_temp,
                                         contract.coupon / 100,
    ...
    if (contract.yield_barrier_flag == 1)
    {
        p_temp                = contract.p;
        p_temp.start          = (int)(floor(contract.p.start + step[n].time_start * 365));
        step[n].clean_barrier = 100 * clean_price_fct(
                                          p_temp,
                                          contract.coupon / 100,
    ...
    YIELD_DEBUG&& printf(
        "clean strike=%f barrier=%f \n", step[n].clean_strike, step[n].clean_barrier);
    YIELD_DEBUG&& printf("time start =%f \n", step[n].time_start);

    if (contract.quanto == 1)
    {
        step[n].df_swap_end = model_fx.df_swap_all / step_fx[n].df_swap;
        step[n].expected_spot *= exp(contract.corr_fx * sqrt(step[n].cum_vol * step_fx[n].cum_vol));

        H_DEBUG&& printf("init steps spot expected old =%f \n", step[n].expected_spot);
        H_DEBUG&& printf(
            "corr =%f cum_vol =%f cum_fx =%f \n ",
            contract.corr_fx,
            step[n].cum_vol,
            step_fx[n].cum_vol);
    ...
    if (n < model.step_num)
    {
        ...
        step[n].vol   = sqrt((model.cum_vol_opt - step[n].cum_vol) / step[n].time_end);
...
void smoothing(Deal contract, Model model, Step* step, Node* node)
{
...

    {
        ...
        absorb_moment[0] = contract.barrier * (q2(-do_up, m1, ft) - fe * q2(-do_up, m2, ft));
        absorb_moment[0] /= absorb_prob;
        absorb_moments[0] = absorb_moment[0] * absorb_prob + contract.barrier * (1 - absorb_prob);
        absorb_moments[0] /= node->asset;

        absorb_moment[1] = contract.barrier * contract.barrier *
                           (q2(-2 * do_up, m1, ft) - fe * q2(-2 * do_up, m2, ft)) / absorb_prob;
        ...
        if (do_up == 1) /* Up */
        {
            node->p_d =
                ...
                (contract.barrier / node->asset - absorb_moments[0]) * (1 - absorb_moments[0]);
            node->p_d /= (1 - d) * (contract.barrier / node->asset - d);

            node->p_m =
                (absorb_moments[1] - absorb_moments[0] * absorb_moments[0]) +
                (contract.barrier / node->asset - absorb_moments[0]) * (d - absorb_moments[0]);
        ...
        else
        {
            ...
            node->p_d /=
                (1 - contract.barrier / node->asset) * (u - contract.barrier / node->asset);

            node->p_m =
                (absorb_moments[1] - absorb_moments[0] * absorb_moments[0]) +
                (u - absorb_moments[0]) * (contract.barrier / node->asset - absorb_moments[0]);
...
double bond_trinomial_fct(Deal contract, Model model, Model model_fx, double* vega)
{
    ...
    model.fwd_opt_mat =
        fwd_clean_fct((double)contract.p.start, contract.opt_mat, model.swap, model.repo, contract);
    ...
    if (contract.quanto == 1)
    {
        ...
        model.fwd_opt_mat *=
            exp(contract.corr_fx * sqrt(
                                       model.date_cumvol[1][model.mat_vol_nb] *
                                       model_fx.date_cumvol[1][model_fx.mat_vol_nb]));

        H_DEBUG&& printf("    new forward mat =%f \n", model.fwd_opt_mat);
    }

    date_treatment(contract, model, model_fx, steps, steps_fx, &model.step_num, &model_fx.step_num);
    ...
    for (i = 0; i <= 2 * model.step_num + 2; i++)
    {
        ...
        node.intrinsic = (node.asset - contract.strike) * (1 - 2 * contract.call_put);
...
double endtree_digit(Deal contract, Model model, Step step, Node node)
{
    ...
    if ((contract.call_put == 1) && (node.position == model.hit_level))
    {
        ...
        result += model.diff_do * contract.do_up + model.diff_up * (1 - contract.do_up);
        result /= model.diff_do * contract.do_up + model.diff_up * (1 - contract.do_up);

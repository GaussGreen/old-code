...
/*                                                                            */
/*      AUTHORS:        E. Auld        , K.L. Chau        , J. Malhi        , O.
 * Van Eyseren */
/*                                                                            */
/******************************************************************************/
...
void srt_f_chetrerindex(SrtStpPtr stp,           /* current step */
...

{
  ...
  find_closest_strictly_below(sam_get(node->drift_sam, 0, SHORT_RATE), prev_r,
                              trinf->min_r_index, trinf->max_r_index,
                              &node->mid_son_index);
...
/*-----------------------------------------------------------------------
  ...
  DESCRIPTION     :calculate probabilities        ,
                                   assuming that node->mid_son_inde is correctly
set        , and also node->drift_sam and node->var_at_sam. If the mid_son_index
hits the top (bottom)        , it is decremented (incremented). Probabilities
are bounded to be between PROBUPB and PROBLUB MODIFIES		  :node->p CALL
:
...
void srt_f_chetreprob(SrtStpPtr stp, double *prev_r, SrtTrinTreNdInf *node,
...
/*-----------------------------------------------------------------------
  ...
  DESCRIPTION		: discounts three vectors in che tree
                                          i.e. cur will be discounted
probability weighted sum of three of the elements of assets MODIFIES
: cur
---------------------------------------------------------------------------*/
void srt_f_chetredcntvec(SrtStpPtr stp, double *cur, SrtTrinTreNdInf *node,
                         double ***assets, int node_dim, double **prev_phi) {
  ...
  for (j = 0; j < 3; j++) {
    find_closest_strictly_below(sam_get(node->drift_sam, 0, PHI),
                                prev_phi[r_i - 1 + j], trinf->min_phi_index,
                                trinf->max_phi_index, &phiind[j]);
  ...
  for (i = 0; i < node_dim; i++) {
    ...
    for (j = 0; j < 3; j++) {
      ...
      cur_val +=
          node->p[j] *
          interp_linear(sam_get(node->drift_sam, 0, PHI),
                        prev_phi[r_i - 1 + j][phiind[j]], asset_phi1,
                        prev_phi[r_i - 1 + j][phiind[j] + 1], asset_phi2);
    }
...
static void det_tre_lims(SrtStpPtr top, SrtStpPtr stp, SrtCheTreInf *trinf,
                         SrtIRMTmInf *tminf, SrtCheTreInf *prvtrinf,
                         SrtIRMTmInf *prvtminf, SrtGrfnParam *grfnparam);

/* --------------------------------------------------------------------------------
   FUNCNAME        :srt_f_cheytreelim
   DESCRIPTION     :allocates and populates SrtCheTreInf struct at each step
                                   population costs in calling det_tre_lims at
   each step maxtreeinf will contain the biggest maxr and maxphi        , and
   the smallest minr and minphi
   --------------------------------------------------------------------------------
 */
Err srt_f_cheytreelim(SrtStpPtr stp, SrtGrfnParam *grfnparam,
                      SrtCheTreInf *maxtrinf) {
  Err err;
  SrtStpPtr top;
  ...
  while (stp) {
    ...
    maxtrinf->max_phi_index =
        IMAX(maxtrinf->max_phi_index, trinf->max_phi_index);
    maxtrinf->min_r_index = IMIN(maxtrinf->min_r_index, trinf->min_r_index);
    maxtrinf->min_phi_index =
        IMIN(maxtrinf->min_phi_index, trinf->min_phi_index);
...
static void
det_tre_lims(SrtStpPtr top,          /* first step */
  ...
  /* i-th step: populate */
    ...
    /* r at top sam */
    sam_get(top_sam, 0, SHORT_RATE) = exp(
        trinf->logrmin + logu * (trinf->max_r_index - trinf->min_r_index + 1));
    /* maxphi and minphi and midphi at top sam */
    srt_f_chephilim(top, stp, tminf, &top_sam, &(sam_get(bot_drf_sam, 0, PHI)),
                    &(sam_get(top_drf_sam, 0, PHI)));
    sam_get(top_sam, 0, PHI) = sam_get(top_drf_sam, 0, PHI);

    /* same at bot sam */
    sam_get(bot_sam, 0, SHORT_RATE) = exp(trinf->logrmin);
    srt_f_chephilim(top, stp, tminf, &bot_sam, &(sam_get(bot_drf_sam, 0, PHI)),
                    &(sam_get(top_drf_sam, 0, PHI)));

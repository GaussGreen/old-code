...
Err CPBasisSwap(
  ...
  long *tPeriodDates = NULL; /*start dates of the different periods in the basis
                                swap with the last date at the end*/
  long TheoEndDate;
  long daycounter;
  ...
  long *TheoDates = NULL;
  double totalmargin, marginspread, marginslope, marginconvexity,
      marginsecondorder;
  ...
  if (err = swp_f_get_ref_rate_details(cshortRefRateCode, &CPbasis,
                                       &CPFrequency)) {
    return err;
  }

  /* get info from the RefRate Code of the Swap */
  if (err = swp_f_get_ref_rate_details(clongRefRateCode, &BasisSwapbasis,
                                       &BasisSwapFrequency)) {
    return err;
  }
  ...
  nbsperiod =
      (int)(Maturity *
            BasisSwapFrequency); /* Number of periods in the basis swap */
  numperiodCP =
      (int)(CPFrequency / BasisSwapFrequency); /* Number of CP periods in a
                                                  period of the basis swap */
  ...
  TheoEndDate = add_unit(StartDate, (int)(nbsperiod * 12 / BasisSwapFrequency),
                         SRT_MONTH, NO_BUSDAY_CONVENTION);

  /* Calculate the start dates of each period in the basis swap */
  for (i = 0; i <= nbsperiod; i++) {
    TheoDates[i] =
        add_unit(TheoEndDate, -(int)((nbsperiod - i) * 12 / BasisSwapFrequency),
                 SRT_MONTH, NO_BUSDAY_CONVENTION);
  ...
  for (period = 0; period < nbsperiod; period++) {
    ...
    err = swp_f_setSwapDP(
        ...
        tPeriodDates[period + 1], BasisSwapFrequency, BasisSwapbasis, &sdpFra);
    if (err)
      return err;
    ...
    err = swp_f_ForwardRate_SwapDP(&sdpFra, szYieldCurveName, clongRefRateCode,
                                   &(dlongFra[period]));
    if (err)
      return err;
    ...
    for (i = 0; i <= numperiodCP; i++) {
      tDatesCP[i] = add_unit(TheoDates[period + 1],
    ...
    for (periodCP = 0; periodCP < numperiodCP; periodCP++) {
      ...
      /* Calculates the spread between the CP and the Libor for all open days of
       * the period*/
      for (i = 0; i < numdaysopenperiodCP[periodCP]; i++) {
        EndFixingPeriodCP =
            add_unit(tEveryDayDates[i], 30, SRT_DAY, NO_BUSDAY_CONVENTION);
        EndFixingPeriodCP =
            add_unit(EndFixingPeriodCP, 0, SRT_BDAY, SUCCEEDING);
        EndFixingPeriodLibor = add_unit(
            add_unit(tEveryDayDates[i], spotlag, SRT_BDAY, MODIFIED_SUCCEEDING),
            1, SRT_MONTH, NO_BUSDAY_CONVENTION);
      ...
      MeanspreadperiodCP[periodCP] =
          MeanspreadperiodCP[periodCP] /
          (numdaysopenperiodCP[periodCP] +
           0.0); /* Mean of the spread during the CP period */

      /* Copies the values of the dates of the period and the spreads of the
       * period */
      for (i = 0; i < numdaysopenperiodCP[periodCP]; i++) {
      ...
      for (i = 0; i < numdaysopenperiodCP[periodCP]; i++) {
        ...
        ShortEndDates[i] = add_unit(
            add_unit(tEveryDayDates[i], spotlag, SRT_BDAY, MODIFIED_SUCCEEDING),
            (int)(CPFrequency / 12.0), SRT_MONTH, NO_BUSDAY_CONVENTION);
        err = swp_f_setSwapDP(
            add_unit(tEveryDayDates[i], spotlag, SRT_BDAY, MODIFIED_SUCCEEDING),
        ...
        err = swp_f_ForwardRate_SwapDP(&sdpFra, szYieldCurveName,
                                       cshortRefRateCode, &(dshortFra[i]));
        if (err)
          return err;
        ...
        DFs[i] = swp_f_df(
            today, add_unit(ShortEndDates[i], 0, SRT_BDAY, MODIFIED_SUCCEEDING),
            szYieldCurveName);
    ...
    /********************* Calculates the margin of the basis swap for the
     * period.***********************************/
    ...
    for (periodCP = 0; periodCP < numperiodCP; periodCP++) {
      ...
      marginsecondorder +=
          ((numdaysopenperiodCP[periodCP] + 0.0) / 360.0 -
           1.0 / (2.0 * CPFrequency)) *
    ...
    marginsecondorder = (BasisSwapFrequency + 0.0) / (CPFrequency + 0.0) *
                        (marginsecondorder + 1.0 / (2.0 * CPFrequency) *
                                                 (marginspread + marginslope) *
                                                 (marginspread + marginslope));
    marginspread =
        marginspread * (BasisSwapFrequency + 0.0) / (CPFrequency + 0.0);
    marginslope =
        marginslope * (BasisSwapFrequency + 0.0) / (CPFrequency + 0.0) -
        dlongFra[period];
    marginconvexity =
        marginconvexity * (BasisSwapFrequency + 0.0) / (CPFrequency + 0.0);

    /* Total margin of the period */
    totalmargin =
        marginspread + marginslope + marginconvexity + marginsecondorder;
...
Err srt_f_CPCap(long StartDate, long EndDate, double Strike, double chi,
  ...
  for (i = 1; i < nfixingdays; i++) {
    fixingdays[i] =
        add_unit(fixingdays[i - 1], 1, SRT_BDAY, MODIFIED_SUCCEEDING);
  ...
  for (i = 0; i < nfixingdays; i++) {
    get_fra(fixingdays[i], spotlag, cRefRateCode, szYieldCurveName,
            &(Libors[i]));
    enddate = add_unit(
        add_unit(fixingdays[i], spotlag, SRT_BDAY, MODIFIED_SUCCEEDING),
        12 / float_compounding, SRT_MONTH, MODIFIED_SUCCEEDING);
    err = swp_f_vol(
        szVolCurveName,
        add_unit(fixingdays[i], spotlag, SRT_BDAY, MODIFIED_SUCCEEDING),
        enddate, Strike - spreads[i], &(vols[i]), &lognorm);
  ...
  *price = coverage(StartDate, EndDate, float_basis) *
           srt_f_optblksch(sumCP / nfixingdays, Strike, vol, maturity, 1.0,
                           CallPut, PREMIUM);

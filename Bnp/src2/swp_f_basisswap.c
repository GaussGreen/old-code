...
Err CPBasisSwap(long StartDate, 
				double Maturity, 
...
long *tPeriodDates = NULL;/*start dates of the different periods in the basis swap with the last date at the end*/
long TheoEndDate;
long daycounter;
...
numperiodCP = (int) (CPFrequency/BasisSwapFrequency); /* Number of CP periods in a period of the basis swap */
maxnumofdaysperiodCP = (int) (360/CPFrequency);   /* Maximum number of open days in a CP period */
...
TheoEndDate = add_unit(StartDate, (int) (nbsperiod * 12 /  BasisSwapFrequency) , SRT_MONTH, NO_BUSDAY_CONVENTION);
	
/* Calculate the start dates of each period in the basis swap */
for (i=0; i<=nbsperiod; i++)
{
	TheoDates[i] = add_unit(TheoEndDate, - (int) (( nbsperiod - i) * 12 / BasisSwapFrequency) , SRT_MONTH, NO_BUSDAY_CONVENTION);
...
{		
	...
	err = swp_f_setSwapDP(add_unit(tPeriodDates[period],spotlag,SRT_BDAY,MODIFIED_SUCCEEDING),
							tPeriodDates[period+1],
							BasisSwapFrequency, BasisSwapbasis, &sdpFra);
	if (err) return err;
	...
	err = swp_f_ForwardRate_SwapDP( &sdpFra, 
									szYieldCurveName, 
									clongRefRateCode, 
									&(dlongFra[period]));
	if (err) return err;
	...
	PeriodDFs[period] = swp_f_df(today,add_unit(tPeriodDates[period+1],-1,SRT_BDAY,MODIFIED_SUCCEEDING),szYieldCurveName);


	/* Calculates the dates of CP periods within the swap period */
	for (i=0; i<=numperiodCP; i++)
	{
		tDatesCP[i] = add_unit(TheoDates[period+1], - (int) (( numperiodCP - i) * 12 / CPFrequency) , SRT_MONTH, NO_BUSDAY_CONVENTION);
	...
	/* Determines the NY open days in the CP period */
		...
		{
			tEveryDayDates[i] = add_unit(tEveryDayDates[i-1], 1, SRT_BDAY,  MODIFIED_SUCCEEDING);
		...
		for (i = 0; i<numdaysopenperiodCP[periodCP];i++)
		{
			...
			EndFixingPeriodLibor = add_unit(add_unit(tEveryDayDates[i],spotlag,SRT_BDAY,MODIFIED_SUCCEEDING),1,SRT_MONTH,NO_BUSDAY_CONVENTION);
			EndFixingPeriodLibor = add_unit(EndFixingPeriodLibor,0,SRT_BDAY,MODIFIED_SUCCEEDING);
		...
		MeanspreadperiodCP[periodCP]=MeanspreadperiodCP[periodCP]/(numdaysopenperiodCP[periodCP]+0.0); /* Mean of the spread during the CP period */
	...
	for (i = 0; i<numdaysopenperiodCP[periodCP];i++)
	{
		...
		for (i = 0; i<numdaysopenperiodCP[periodCP];i++)
		{
			...
			ShortEndDates[i] = add_unit(add_unit(tEveryDayDates[i],spotlag,SRT_BDAY,MODIFIED_SUCCEEDING),(int) (CPFrequency/12.0),SRT_MONTH,NO_BUSDAY_CONVENTION);
			err = swp_f_setSwapDP(add_unit(tEveryDayDates[i],spotlag,SRT_BDAY,MODIFIED_SUCCEEDING),
								ShortEndDates[i],CPFrequency, CPbasis, &sdpFra);
			if (err) return err;
		...
		/*	Get the Discount Factors */
			DFs[i] = swp_f_df(today,add_unit(ShortEndDates[i],0,SRT_BDAY,MODIFIED_SUCCEEDING),szYieldCurveName);	

			/* Computes the mean of the convexity effects within the CP period */
			MeanconvexityperiodCP[periodCP] += dshortFra[i]*(DFs[i]/PeriodDFs[period]-1)*(add_unit(ShortEndDates[i],0,SRT_BDAY,MODIFIED_SUCCEEDING)-today-0.0)/360.0;
		}

		/* Normalizes by the number of days to get the mean */
		MeanLiborperiodCP[periodCP]=MeanLiborperiodCP[periodCP]/(numdaysopenperiodCP[periodCP]+0.0);
		MeanconvexityperiodCP[periodCP] = Vol[period]*Vol[period]*MeanconvexityperiodCP[periodCP]/(numdaysopenperiodCP[periodCP]+0.0);
	}
...
/********************* Calculates the margin of the basis swap for the period.***********************************/
	...
	for (periodCP = 0; periodCP<numperiodCP;periodCP++)
	{
		...
		marginsecondorder += ((numdaysopenperiodCP[periodCP]+0.0)/360.0-1.0/(2.0*CPFrequency))*(MeanspreadperiodCP[periodCP]+MeanLiborperiodCP[periodCP])*(MeanspreadperiodCP[periodCP]+MeanLiborperiodCP[periodCP]);
	}

	/* Compounding adjustment */
	marginsecondorder=(BasisSwapFrequency+0.0)/(CPFrequency+0.0)*(marginsecondorder+1.0/(2.0*CPFrequency)*(marginspread+marginslope)*(marginspread+marginslope));
	marginspread = marginspread*(BasisSwapFrequency+0.0)/(CPFrequency+0.0);
	marginslope = marginslope*(BasisSwapFrequency+0.0)/(CPFrequency+0.0) - dlongFra[period];
...
Err srt_f_CPCap(long StartDate, 
		  long EndDate,
		  double Strike,
		  double chi,
...
spotlag = get_spotlag_from_curve(yldcrv);
	...
	for (i=0;i<nfixingdays;i++)
	{
		...
		enddate = add_unit(add_unit(fixingdays[i],spotlag,SRT_BDAY,MODIFIED_SUCCEEDING),12/float_compounding,SRT_MONTH,MODIFIED_SUCCEEDING);
		err = swp_f_vol(szVolCurveName,add_unit(fixingdays[i],spotlag,SRT_BDAY,MODIFIED_SUCCEEDING),enddate,Strike-spreads[i],&(vols[i]),&lognorm);
	...
	*price = coverage(StartDate, EndDate,float_basis)*srt_f_optblksch(sumCP/nfixingdays,Strike,vol,maturity,1.0,CallPut,PREMIUM);

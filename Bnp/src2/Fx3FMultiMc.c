...
double	sum;
	...
	if (temp) free_dmatrix (temp, 0, nb_paths-1, 0, n-1);
...
Err		err					= NULL;
	...
	correlate_underlyings (link_und -> covariance,
								nb_paths,
								link_und -> num_und,
								nb_dates-1,
								matrix);
	...
	for (i=0; i<nb_paths; i++)
	{
		...
		for (j=1; j<nb_dates; j++)
		{
			...
			df = link_und -> dom_bond_pay[j] * 
				exp(link_und -> dom_beta_pay[j] * (-0.5 * link_und -> dom_beta_pay[j] * 
				link_und -> phi[link_und -> dom_index][j] + sv[link_und -> dom_index]));
	...
	if (do_optimisation)
	{
		...
		time2 = clock();
		smessage ("Phase 6 -optimisation, time in sec: %.2f", (double) (time2 - time1) / CLOCKS_PER_SEC);
	}

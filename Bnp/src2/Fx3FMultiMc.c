...
static void nr_choldc_tolerance(
    int      n, /* 0.. n-1*/
    double** cov,
    double** chol)
{
...
static void correlate_underlyings(
    double*** cov, long nb_paths, int n, int nb_dates, double** matrix)
{
    long     i, j, l, k;
    double** chol;
    double** temp;
    ...
    double** res)
{
    ...
    double *temp = NULL, *res_evt = NULL, *sum_price = NULL, *sum_2price = NULL, **matrix = NULL,
           *sv = NULL, ***save_values = NULL;
    ...
    matrix     = dmatrix(0, nb_paths - 1, 0, (link_und->num_und) * (nb_dates - 1) - 1);
    ...
    if (do_optimisation)
    {
        err = mceb_allocate_savevalues_for_GRFN(nb_paths, nb_dates, params, &save_values);
    ...
    err = balsam_generation(nb_paths, (link_und->num_und) * (nb_dates - 1), matrix);
    if (err)
    {
        goto FREE_RETURN;
    }
    time2 = clock();
    smessage(
        "Phase 2 -BalSam generation, time in sec: %.2f", (double)(time2 - time1) / CLOCKS_PER_SEC);
    time1 = clock();
    ...
    time2 = clock();
    smessage(
        "Phase 3 -PECS adjustment, time in sec: %.2f", (double)(time2 - time1) / CLOCKS_PER_SEC);
    time1 = clock();

    correlate_underlyings(link_und->covariance, nb_paths, link_und->num_und, nb_dates - 1, matrix);
    if (err)
    {
        goto FREE_RETURN;
    }
    ...
    for (i = 0; i < nb_paths; i++)
    {
        ...
        if (func_parm_tab[0])
        {
            ...
            err = payoff_func(date[0], time[0], func_parm_tab[0], link_und, sv, nb_col, res_evt);
        ...
        for (j = 1; j < nb_dates; j++)
        {
            ...
            for (l = 0; l < link_und->num_und; l++)
            {
                if (link_und->type[l] == 2)
                {
                    sv[l] +=
                        link_und->fwd[l][j] +
                        link_und->beta[link_und->for_forex[l]][j - 1] * sv[link_und->for_forex[l]] -
                        link_und->beta[link_und->dom_forex[l]][j - 1] * sv[link_und->dom_forex[l]] +
            ...
            for (l = 0; l < link_und->num_und; l++)
            {
                if (link_und->type[l] < 2)
                {
                    sv[l] = sv[l] * exp(-link_und->lambda[l] * (t2 - t1)) + link_und->fwd[l][j] +
                            matrix[i][(link_und->num_und) * (j - 1) + l];
                }
            }
            ...
            df = link_und->dom_bond_pay[j] *
                 exp(link_und->dom_beta_pay[j] *
                     (-0.5 * link_und->dom_beta_pay[j] * link_und->phi[link_und->dom_index][j] +
                      sv[link_und->dom_index]));

            err = payoff_func(date[j], t2, func_parm_tab[j], link_und, sv, nb_col, res_evt);
    ...
    if (do_optimisation)
    {
        ...
        time2 = clock();
        smessage(
            "Phase 6 -optimisation, time in sec: %.2f", (double)(time2 - time1) / CLOCKS_PER_SEC);
    }
...
FREE_RETURN:
    ...
    if (matrix)
    {
        free_dmatrix(matrix, 0, nb_paths - 1, 0, (link_und->num_und) * (nb_dates - 1) - 1);
        matrix = NULL;
    }

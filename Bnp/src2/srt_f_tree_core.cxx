...
Err TreeCore(SrtUndPtr und, SrtGrfnParam *grfnparam, SrtStpPtr step,
             GrfnDeal *gd, EvalEventFct evalcf, void *iolist,
             SrtUndInfo *und_info) {
  Err err = NULL;
  ...
  if (und_info->no_of_underlyings == 1) {
    ...
    if (mdl_type == CHEY) {
      switch (mdl_dim) {
      case ONE_FAC:
        err = srt_f_vegatreche2dr(und, grfnparam, step, gd, evalcf, io_request,
                                  und_info);
        if (err) {
          return err;
        }
        break;
      case TWO_FAC:
        err = srt_f_cheybeta2dtree(und, grfnparam, step, gd, evalcf, io_request,
                                   und_info);
    ...
    /* -> 1 FACTOR and 2 FACTOR CHEY BETA TREE <- */
      ...
      case ONE_FAC:
        if ((grfnparam->lsm == SRT_BACK) || (grfnparam->lsm == SRT_FORBACK)) {
          /*
          err = srt_f_ammccheybeta (grfnparam        , step        , gd        ,
          evalcf        , io_request        , und_info); if (err) return err;
          */
        } else {
          err = srt_f_CheyBetaTree(und, grfnparam, step, gd, evalcf, io_request,
                                   und_info);
          if (err)
            return err;
        }
      ...
      case TWO_FAC:
        if ((grfnparam->lsm == SRT_BACK) || (grfnparam->lsm == SRT_FORBACK)) {
          /*
          err = srt_f_newbeta2ftree( und        , grfnparam        , step , gd
          , evalcf        , io_request        , und_info);
          */
        } else {
          err = srt_f_cheybeta2dtree(und, grfnparam, step, gd, evalcf,
                                     io_request, und_info);
        }
    ...
    /* -------------- 1 FACTOR and 2 FACTOR LGM TREES -------------------- */
      ...
      switch (mdl_dim) {
      case ONE_FAC:
        err = srt_f_vegashifttrelgm1d(und, grfnparam, step, gd, evalcf,
                                      io_request, und_info);
        if (err) {
          return err;
        }
        /*  we try to get the prices for the shift in the yield curve
                err = srt_f_yctrelgm1d(m        ,grfnparam        ,step ,gd
           ,evalcf ,iolist); not yet implemented */
        break;
      case TWO_FAC:
        if ((grfnparam->lsm == SRT_BACK) || (grfnparam->lsm == SRT_FORBACK)) {
          /*
          err = srt_f_ammclgm2f(grfnparam        , step        , gd        ,
          evalcf        , io_request , und_info); if (err) return err;
          */
        } else {
          if (grfnparam->minim == SRT_YES) {
            err = srt_f_lgm2dtree_proj(und, grfnparam, step, gd, evalcf,
                                       io_request, und_info);
            if (err)
              return err;
          } else {
            err = srt_f_lgm2dtree(und, grfnparam, step, gd, evalcf, io_request,
                                  und_info);
            if (err)
              return err;
          }
    ...
    else if (mdl_type == BDT) {
      switch (mdl_dim) {
      case ONE_FAC:
        err = srt_f_trebdt1f(und, grfnparam, step, gd, evalcf, io_request,
                             und_info);
        if (err) {
          return err;
        }
        break;
      case TWO_FAC:
        /*	Modified AS: BDT 2F Tree is unavailable for the moment        ,
         * The file is out of the project...  */
        /* err = srt_f_trebdt2f(und        ,mdl        ,step        ,gd ,evalcf
        ,io_request ,und_info); if (err)
        {
                return err;
        } */
      ...
      err =
          srt_f_logtree(und, grfnparam, step, gd, evalcf, io_request, und_info);
      if (err) {
        return err;
      }
    } /* END if mdl_type == BLACK_SCHOLES */

    else if (mdl_type == NORMAL_BS) {
      return serror("Normal Tree not yet implemented");
    }

    /*------------------ CHEY_STOCH_VOL ---------------*/
    else if (mdl_type == CHEY_BETA_STOCH_VOL) {
      switch (mdl_dim) {
      case ONE_FAC:
    ...
    if ((mdl_dim1 == ONE_FAC) && (mdl_dim2 == ONE_FAC) &&
        (mdl_type2 != EQ_STOCH_RATES_SRVGS)) {
      if (((mdl_type1 == LGM) &&
           ((mdl_type2 == LGM) || (mdl_type2 == BLACK_SCHOLES) ||
            (mdl_type2 == EQ_STOCH_RATES))) ||
          ((mdl_type1 == BLACK_SCHOLES) &&
           ((mdl_type2 == LGM) || (mdl_type2 == EQ_STOCH_RATES)))) {

        err =
            srt_f_twoundtree(grfnparam, step, gd, evalcf, io_request, und_info);
        if (err)
          return err;
      } else if ((mdl_type1 == CHEY_BETA) && (mdl_type2 == BLACK_SCHOLES)) {
        /*
        err = srt_f_cheybeta1f1etree(grfnparam        , step        , gd ,
        evalcf        , io_request        ,und_info);
        */
        if (err)
          return err;

		...
		Example p=2 
		  Calculate
			Integral between t1 and t of
				Integral between t1 and s of (u-t1)^n * exp(-lambda[p]*(s-u))*du
				times exp(-lambda[p-1]*(t-s))*ds
   ------------------------------------------------------------------------------------------------ */
...
/* ----------------------------------------------------------------------------------------------- 
	LGMSVFuncValue
		Evaluation of
			Xt  = Xt1*exp(-lambda*(t-t1))+integral between t1 and t of fs*exp(-lambda*(s-t1))
...
int iNumLevel;
	...
	accu = dXt1*exp(-dLambda*dDt)*LGMSVIntMult(iLevelIntegration, Lambda, 0, dDt);
	...
	if (Xt.c != 0)
	{
		...
		else
		{
			...
			accu += Xt.c*LGMSVFuncValue((*pht),dDt, Lambda,iLevelIntegration+1);		
		}
	}
...
int				iNumLevel, iNumSubFunction;
	...
	accu = Xt->dXt1 * exp(-dLambda * dDt) * LGMSVIntMult1(iLevelIntegration, Lambda, 0, dDt);
	...
	for (iNumSubFunction = 0; iNumSubFunction<Xt->iNbFunction; iNumSubFunction++)
	{
		if (fabs(Xt->a[iNumSubFunction]) > 1.0E-12)
		{
			if (Xt->bIsft1[iNumSubFunction] == 1)
			{
				accu += Xt->a[iNumSubFunction] * LGMSVIntMult1(iLevelIntegration+1, Lambda, 0, dDt);
			}
			else
			{
				/* Casting */
				...
				accu += Xt->a[iNumSubFunction] * LGMSVFuncValue1(pft, dDt, Lambda, iLevelIntegration + 1);
			}
		}
	}
...
int		i;
	...
	for (i=0; i<sigma_n; i++)
	{
		...
		new_alpha[i] = alpha * sqrt(lambda_ratio * (val2_new - val2_old) / (val1_new - val1_old));
		new_rho[i] = rho * sqrt(coef_lam * (val12_new - val12_old) * (val12_new - val12_old) / (val1_new - val1_old) / (val2_new - val2_old));
...
int		i;
	...
	if (one2F == 2)
	{
		ConvertAlphaRho_LGM_to_LGMSV(	sigma_n,
										sigma_time,
										Tstar,
										lambda,
										alpha,
...

{
	...
	*Tstar = LGMSV_Tstar;
	...
	ConvertTS_LGM_to_LGMSV(	*sigma_n,
							*sigma_time,
							*sigma,
							1.0 / tau,
							*Tstar,
...
void Convert_Tstar_LGMSV(	double	lambda,
							long	sigma_n,
							double	*sigma_time,
							double	*sigma,
							double	*alphaeps,
...
//			if (!sigma_datas[0][j] || !sigma_datas[1][j])
	...
	if (!irmsv->smile_dates || !irmsv->smile_times || !irmsv->alphaSV || !irmsv->lamSV || !irmsv->rhoSV)
	{
		err = "Memory allocation faillure in SrtInitIRMSVUnd";
		goto FREE_RETURN;
	}
...
void irm_sv_free_struct(IRM_SV irm_sv)
{
	if (irm_sv)
	{
		if (irm_sv->sigma_dates) free (irm_sv->sigma_dates);
		irm_sv->sigma_dates = NULL;

		if (irm_sv->sigma_times) free (irm_sv->sigma_times);
		irm_sv->sigma_times = NULL;

		if (irm_sv->sigma) free (irm_sv->sigma);
		irm_sv->sigma = NULL;

		if (irm_sv->tau_dates) free (irm_sv->tau_dates);
		irm_sv->tau_dates = NULL;

		if (irm_sv->tau_times) free (irm_sv->tau_times);
		irm_sv->tau_times = NULL;

		if (irm_sv->tau) free (irm_sv->tau);
		irm_sv->tau = NULL;

		if (irm_sv->smile_dates) free (irm_sv->smile_dates);
		irm_sv->smile_dates = NULL;

		if (irm_sv->smile_times) free (irm_sv->smile_times);
		irm_sv->smile_times = NULL;

		if (irm_sv->alphaSV) free(irm_sv->alphaSV);
		irm_sv->alphaSV = NULL;

		if (irm_sv->lamSV) free(irm_sv->lamSV);
		irm_sv->lamSV = NULL;

		if (irm_sv->rhoSV) free(irm_sv->rhoSV);
		irm_sv->rhoSV = NULL;

		if (irm_sv->rho2SV) free(irm_sv->rho2SV);
		irm_sv->rho2SV = NULL;
	}
}
...
Err	irm_sv_get_term_struct(	char	*und,
							int		*one2F,
							int		*num_sigma,
							double	**sigma_times,
							double	**sigma,
...
FREE_RETURN:
	...
	if (err)
	{
		if (*sigma_times) free(*sigma_times); 
		if (*sigma) free(*sigma);
		if (*tau_times) free(*tau_times);
		if (*tau) free(*tau);
		if (*smile_times) free(*smile_times);
		if (*alphaSV) free(*alphaSV);
		if (*lamSV) free(*lamSV);
		if (*rhoSV) free(*rhoSV);
		if (*rho2SV) free(*rho2SV);
	}

	return err;
}

Err	irm_sv_get_term_struct_date(	char	*und,
									int		*one2F,
									int		*num_sigma,
									long	**sigma_dates,
									double	**sigma,
...
FREE_RETURN:
	...
	if (err)
	{
		if (*sigma_dates) free(*sigma_dates); 
		if (*sigma) free(*sigma);
		if (*tau_dates) free(*tau_dates);
		if (*tau) free(*tau);
		if (*smile_dates) free(*smile_dates);
		if (*alphaSV) free(*alphaSV);
		if (*lamSV) free(*lamSV);
		if (*rhoSV) free(*rhoSV);
		if (*rho2SV) free(*rho2SV);
...
Err	init_LGMSV_model(	LGMSV_MODEL	model,
						long		lToday,
						int			iOne2F,
						int			iNbPWTime,
						double		dLambdaX,
...
Err	err = NULL;
	...
	model->lTStarDate = (long) (model->lToday + model->dTStar * DAYS_IN_YEAR + 1.0e-08);
	...
	{		
		...
		ConvertAlphaRho_LGM_to_LGMSV(	iNbPWTime,
										dPWTime,
										model->dTStar,
										model->dLambdaX,
										dLGMAlpha,
...
void free_LGMSV_model(LGMSV_MODEL	model)
{
	if (model)
	{
		...
		model->dSigma = NULL;
		if(model->dAlpha) free (model->dAlpha);
		model->dAlpha = NULL;
		if(model->dLambdaEps) free (model->dLambdaEps);
		model->dLambdaEps = NULL;
		if(model->dLvlEps) free (model->dLvlEps);
		model->dLvlEps = NULL;
		if(model->dRho) free (model->dRho);
		model->dRho = NULL;

		if(model->dRho2) free (model->dRho2);
		model->dRho2 = NULL;
		if(model->dLGMAlpha) free (model->dLGMAlpha);
		model->dLGMAlpha = NULL;
		if(model->dLGMRho) free (model->dLGMRho);
		model->dLGMRho = NULL;
...
Err	fill_LGMSV_model_from_irm_sv(	irm_sv		*irmsv,
									LGMSV_MODEL	model)
{
Err		err = NULL;
int		i, index;
	...
	{
		num_f_add_number(&(model->iNbPWTime), &(model->dPWTime), irmsv->smile_times[i]);
	}
	...
	if (!model->dSigma || !model->dAlpha || !model->dLambdaEps || !model->dLvlEps || !model->dRho)
	{
		err = "Memory allocation faillure in fill_LGMSV_model_from_irm_sv";
		goto FREE_RETURN;
	}
	...
	{
		model->dSigma[i] = irmsv->sigma[Get_Index(model->dPWTime[i], irmsv->sigma_times, irmsv->num_sigma)];
	...
	model->lTStarDate = (long) (model->lToday + model->dTStar * DAYS_IN_YEAR + 0.5);
	...
	if (model->iOne2F == 2)
	{
		...
		for (i=0; i<model->iNbPWTime; i++)
		{
			model->dRho2[i] = irmsv->rho2SV[Get_Index(model->dPWTime[i], irmsv->smile_times, irmsv->num_smile)];
		}
...
Err Get_LGMSV_model(char		*undname,
					LGMSV_MODEL	model)
{
irm_sv	*irmsv;
Err		err = NULL;
...
Err Check_Model_Input_Corr_Matrix(int		iOne2F,
								  int		iNbPWTime,
								  double	*dLGMRho,
								  double	*dRho,
								  double	*dRho2)
...
int		i;
	...
	{			
		for (i=0; i<iNbPWTime; i++)
		{
			...
			if (temp >= 1.0)
			{
				err = "LGMSV Correlation matrix is not positive definite, change Rho2 for example";
				return err;
			}
		}
	}
...
Err	err = NULL;
	...
	err = Check_Model_Input_Corr_Matrix(model->iOne2F,
										model->iNbPWTime,
										model->dLGMRho,
										model->dRho,
										model->dRho2);
...
void Convert_Tstar_model(LGMSV_MODEL	model,
						double			new_tstar)
{
...
int		i;
	...
	{				
		for (i=0; i<model->iNbPWTime; i++)
		{
			model->dLambdaEps[i] += adjust_mean * model->dAlpha[i] * model->dRho[i] * model->dSigma[i];
	...
	else if (model->iOne2F == 2)
	{
		...
		for (i=0; i<model->iNbPWTime; i++)
		{
			model->dLambdaEps[i] += model->dAlpha[i] * model->dSigma[i] * (
	...
	model->lTStarDate = (long) (model->lToday + DAYS_IN_YEAR * model->dTStar + 1.0E-08);
}

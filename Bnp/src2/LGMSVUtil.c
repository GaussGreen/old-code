    ...
    int            iLevelIntegration /* 0 for valuation */)
{
    ...
    for (iNumSubFunction = 0; iNumSubFunction < Xt->iNbFunction; iNumSubFunction++)
    {
        if (fabs(Xt->a[iNumSubFunction]) > 1.0E-12)
        {
            ...
            {
                accu +=
                    Xt->a[iNumSubFunction] * LGMSVIntMult1(iLevelIntegration + 1, Lambda, 0, dDt);
            }
            else
            {
                /* Casting */
    ...
    double* new_rho)
{
    ...
    for (i = 0; i < sigma_n; i++)
    {
        ...
        new_rho[i]   = rho * sqrt(
                               coef_lam * (val12_new - val12_old) * (val12_new - val12_old) /
                               (val1_new - val1_old) / (val2_new - val2_old));
    ...
    double* new_rho)
{
    ...
    if (one2F == 2)
    {
        ConvertAlphaRho_LGM_to_LGMSV(
            sigma_n, sigma_time, Tstar, lambda, alpha, gamma, rho, new_alpha, new_rho);
    }
}
...

{
    ...
    double *sigma2 = NULL, *beta = NULL, *beta2 = NULL, *rhoTS = NULL, *TauDates = NULL,
           *Tau = NULL, *Tau2 = NULL;
    ...
    if (*one2F == 1)
    {
        err = srt_f_display_IRM_OneFac_TermStruct(
            ts, sigma_time, sigma, &beta, alpha, rho, lambdaeps, sigma_n, &TauDates, &Tau, &nbTau);
    }
    else
    {
        /* to be implemented */
    ...
    double tstar)
{
    ...
    if (!irmsv->smile_dates || !irmsv->smile_times || !irmsv->alphaSV || !irmsv->lamSV ||
        !irmsv->rhoSV)
    {
        err = "Memory allocation faillure in SrtInitIRMSVUnd";
        goto FREE_RETURN;
    ...
    double*  tstar)
{
    ...
    *rhoSV       = calloc(*num_smile, sizeof(double));
    ...
    if (!*sigma_times || !*sigma_times || !*sigma_times || !*sigma_times || !*sigma_times ||
        !*sigma_times || !*sigma_times || !*sigma_times)
    {
    ...
    double*  tstar)
{
    ...
    *rhoSV       = calloc(*num_smile, sizeof(double));
    ...
    if (!*sigma_dates || !*sigma_dates || !*sigma_dates || !*sigma_dates || !*sigma_dates ||
        !*sigma_dates || !*sigma_dates || !*sigma_dates)
    {
        err = "Memomry allocation faillure in irm_sv_get_term_struct";
        goto FREE_RETURN;
    }
...
Err fill_LGMSV_model_from_irm_sv(irm_sv* irmsv, LGMSV_MODEL model)
{
    ...
    {
        model->dSigma[i] =
            irmsv->sigma[Get_Index(model->dPWTime[i], irmsv->sigma_times, irmsv->num_sigma)];
    ...
    if (model->iOne2F == 2)
    {
        ...
        {
            model->dRho2[i] =
                irmsv->rho2SV[Get_Index(model->dPWTime[i], irmsv->smile_times, irmsv->num_smile)];
        }
...
Err Check_Model_Corr_Matrix(LGMSV_MODEL model)
{
    ...
    err = Check_Model_Input_Corr_Matrix(
        model->iOne2F, model->iNbPWTime, model->dLGMRho, model->dRho, model->dRho2);

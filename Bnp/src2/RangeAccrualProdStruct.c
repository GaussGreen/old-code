    ...
    RangeAccrualStruct* RA)
{
    ...
    double vol_fx, rho_ffx, coef, fra_f, vol_f, fra_d, cvg_d, vol_d, power, vol_fra_cpn, vol_fx_cpn,
        rho_ffx_cpn, coef_cpn, fra_d_cpn, cvg_d_cpn, vol_d_cpn, cpn_float_spread, vol_f_lower_barr,
        vol_f_upper_barr;
    double quanto_corr, DRS_corr, quanto_corr_cpn, DRS_corr_cpn;
    double fra_float_cpn, cpn_float_upper_barrier_vol, cpn_float_lower_barrier_vol, n_std_dev,
        std_strike, correl;
    ...
    //		   eod_flag + ltoday > add_unit( dates[index], -RA->spotlag_f, SRT_BDAY,
    //MODIFIED_SUCCEEDING ); 		   index++ );
    // Changed on 12 Jan 2004 following Laurent's request
    for (index = 0; index < n_periods && eod_flag + ltoday > ra_fixingdates[index][0]; index++)
        ;
    ...
    if (RA->cpn_type != 0)
    {
        ...
        for (i = index; i < n_periods; i++)
        {
            ...
            RA->float_cpn_dates[j + 1] = add_unit(
                RA->float_cpn_dates[j], RA->tenor_float_cpn, SRT_MONTH, MODIFIED_SUCCEEDING);
            j += 2;
        }
    }
    ...
    */
    ...
    stp->date = d =
        add_unit(ra_fixingdates[index][0], RA->spotlag_f, SRT_BDAY, MODIFIED_SUCCEEDING);
    j = obs_freq;
    while (i < n_periods - index)
    {
        stp->next = (RangeAccrualObservStep*)malloc(sizeof(RangeAccrualObservStep));
        ...
        {
            d = add_unit(
                ra_fixingdates[i + index][j], RA->spotlag_f, SRT_BDAY, MODIFIED_SUCCEEDING);
        }
        else
        {
            i++;
    ...
    */
    ...
    for (i = index, stp = RA->obs_steps[0]; stp->next; stp = stp->next)
    {
        ...
        if (stp == RA->obs_steps[i + 1 - index] || stp == RA->obs_steps[0])
        {
            ...
            {
                d_cpn = add_unit(
                    ra_float_startl[i], RA->tenor_float_cpn, SRT_MONTH, MODIFIED_SUCCEEDING);
                ...
                fra_float_cpn = swp_f_fra(
                    ra_float_startl[i], d_cpn, RA->basis_float_cpn, yc_cpn_float, ra_float_refrate);
                ...
                {
                    // Get the FX ATM vol & FX correlations (using the same ones as for the quanto
                    // index
                ...
                if (ra_float_startl[i] == RA->dates[i - index] && d_cpn == RA->dates[i + 1 - index])
                    DRS_corr_cpn = 0;
                else
                {
                    ...
                    err       = swp_f_SABRvol(
                        volc_d, d1_cpn, d2_cpn, fra_d_cpn, &vol_d_cpn, &power, SABR_LOGVOL);
                    if (err)
                        return err;

                    DRS_corr_cpn = rho_df * vol_d_cpn * vol_fra_cpn * cvg_d_cpn * fra_d_cpn /
                                   (1.0 + cvg_d_cpn * fra_d_cpn);
        ...
        if (RA->cpn_type != 0)
        {
            ...
            stp->cvg =
                ((double)(stp->next->date - stp->date)) /
                ((double)(RA->obs_steps[i + 1 - index]->date - RA->obs_steps[i - index]->date)) *
        ...
        if (RA->cpn_type != 0 && float_adj_strike == 2)
        {
            if (vol_f > 0.0)
            {
                ...
                n_std_dev =
                    log(lower_barr[i] / fra_f) / (vol_f * sqrt((stp->date - today) / 365.0));
                std_strike = fra_float_cpn * exp(correl * vol_fra_cpn * n_std_dev *
                ...
                n_std_dev =
                    log(upper_barr[i] / fra_f) / (vol_f * sqrt((stp->date - today) / 365.0));
                std_strike = fra_float_cpn * exp(correl * vol_fra_cpn * n_std_dev *
                                                 sqrt((ra_float_startl[i] - today) / 365.0));
                err        = swp_f_SABRvol(
                    volc_cpn_float,
        ...
        {
            err = swp_f_SABRvol(
                volc_f, stp->date, d, lower_barr[i], &vol_f_lower_barr, &power, SABR_LOGVOL);
            if (err)
                goto RETURN_ERR;
            stp->cpn_float_lower_barrier_adj *= vol_f_lower_barr;

            err = swp_f_SABRvol(
                volc_f, stp->date, d, upper_barr[i], &vol_f_upper_barr, &power, SABR_LOGVOL);
            if (err)
                goto RETURN_ERR;
        ...
        if (RA->quanto) /* Calculate quanto adjustment */
        {
            ...
            {
                coef    = ((double)(d - fxvol_dates[j])) / (fxvol_dates[j + 1] - fxvol_dates[j]);
        ...
        err = swp_f_SABRvol(volc_f, stp->date, d, 0.05, &stp->betavol, &power, SABR_BETAVOL);
        if (err)
        {
            goto RETURN_ERR;
        }
        err = swp_f_SABRvol(volc_f, stp->date, d, 0.05, &stp->normvol, &power, SABR_ATMNORM);
        if (err)
        {
            goto RETURN_ERR;
        }
        err = swp_f_SABRvol(volc_f, stp->date, d, 0.05, &stp->lognvol, &power, SABR_ATMLOG);
        if (err)
        {
            goto RETURN_ERR;
        }
        err = swp_f_SABRvol(volc_f, stp->date, d, 0.05, &stp->alpha, &power, SABR_ALPHA);
        if (err)
        {
            goto RETURN_ERR;
        }
        err = swp_f_SABRvol(volc_f, stp->date, d, 0.05, &stp->beta, &power, SABR_BETA);
        if (err)
        {
            goto RETURN_ERR;
        }
        err = swp_f_SABRvol(volc_f, stp->date, d, 0.05, &stp->rho, &power, SABR_RHO);
        if (err)
        {
            goto RETURN_ERR;
        }
...
Err RA_RequestDfDates(RangeAccrualStruct* RA, long date, int* pn_unds, int** pn_dfs, long*** pdates)
{
    ...
    *pdates = (long**)calloc(*pn_unds, sizeof(long*));
    ...
    for (i = 0; i < RA->n_periods &&
                date > add_unit(RA->dates[i], -RA->spotlag_f, SRT_BDAY, MODIFIED_SUCCEEDING);
         i++)
        ;
...
Err RA_Payoff(RangeAccrualStruct* RA, long today, long date, double** dfs, double* payoff)
{
    Err    err = NULL;
    int    i, j, k, nfwds;
    long   d, d1;
    ...
    *payoff = 0.0;
    ...
    if (date != RA->last_date)
    {
        ...
        if (RA->cpn_type != 0)
        {
            ...
            {
                RA->cvg_float_cpn[k] =
                    coverage(dates_cpn_float[j], dates_cpn_float[j + 1], RA->basis_float_cpn);
                k++;
            }
        }
    ...
    if (RA->cpn_type != 0)
    {
        ...
        {
            fwds_cpn_float[k] =
                (dfs_cpn_float[j] / dfs_cpn_float[j + 1] - 1.0) / RA->cvg_float_cpn[k];
            k++;
        }
    }
    ...
    */
    ...
    if (RA->cpn_type != 0)
    {
        ...
        // Do not need to do RA->nb - i since RA->float_cpn_dates is the entire array (not a partial
        // array)
        fixing_cpn_float =
            add_unit(RA->float_cpn_dates[2 * i], -RA->spotlag_f, SRT_BDAY, MODIFIED_SUCCEEDING);
    ...
    for (k = 0, stp = RA->obs_steps[i = RA->period_idx]; stp->next; stp = stp->next)
    {
        if (stp == RA->obs_steps[i + 1])
        {
            ...
            if (RA->cpn_type != 0)
            {
                ...
                // Do not need to do RA->nb - i since RA->float_cpn_dates is the entire array (not a
                // partial array)
                fixing_cpn_float = add_unit(
                    RA->float_cpn_dates[2 * i], -RA->spotlag_f, SRT_BDAY, MODIFIED_SUCCEEDING);
                mat_cpn_float = (fixing_cpn_float - today) * YEARS_IN_DAY;
        ...
        coef = ((double)(stp->date - RA->dates_f[k])) / (RA->dates_f[k + 1] - RA->dates_f[k]);
        ...
        for (j = 0; j < 4; j++)
        {
            ...
            else if (fwd < K_TINY)
            {
                ...
                if (RA->cpn_type != 0)
                {
                    ...
                    {
                        fwd_lower_barr =
                            fwd * exp(stp->cpn_float_lower_barrier_adj * min(mat, mat_cpn_float));
                        bs_float_cpn[j] = df * (stp->K[j] - fwd_lower_barr);
                    }
                    else
                    {
                        fwd_upper_barr =
                            fwd * exp(stp->cpn_float_upper_barrier_adj * min(mat, mat_cpn_float));
            ...
            else
            {
                ...
                if (RA->cpn_type != 0)
                {
                    ...
                    if (j == 0 || j == 1)
                    {
                        fwd_lower_barr =
                            fwd * exp(stp->cpn_float_lower_barrier_adj * min(mat, mat_cpn_float));
                        bs_float_cpn[j] = srt_f_optblksch(
                            fwd_lower_barr, stp->K[j], vol, mat, df, SRT_PUT, PREMIUM);
                    }
                    else
                    {
                        fwd_upper_barr =
                            fwd * exp(stp->cpn_float_upper_barrier_adj * min(mat, mat_cpn_float));
                        bs_float_cpn[j] = srt_f_optblksch(
                            fwd_upper_barr, stp->K[j], vol, mat, df, SRT_PUT, PREMIUM);
                    }
                }
            }
        }

        sum = (bs[3] - bs[2]) / (stp->K[3] - stp->K[2]) - (bs[1] - bs[0]) / (stp->K[1] - stp->K[0]);
    ...
    double*             fwdPV)
{
    ...
    for (i = 0; i < nbDates; ++i)
    {
        //		date = add_unit( RA->dates[i], -RA->spotlag_f - 1, SRT_BDAY,
        //MODIFIED_SUCCEEDING );
        date = Dates[i];
...
Err init_ra_leg(
                        ...
                        int				*ra_nfixings,
                        long			**ra_fixingdates,
                        double			**ra_fixings,			// Past coupon
fixing if relevant double			*upper_barr, double			*lower_barr,
                        char			*recpay,
                        double			c_spread,
                        int				obs_freq,
                        double			rho_df,
                        ...
                        ra_leg *RA)
{
        ...
        for ( index=0; index < num_dates &&
                           eod_flag + ltoday > add_unit( start_dates[index], -RA->spotlag_f,
SRT_BDAY, MODIFIED_SUCCEEDING ); index++ );
        ...
        for(i=0;i < num_dates - index;++i)
        {
                ...
                {
                        RA->cpn[i].num_obs += 1;
                        RA->cpn[i].obs = (ra_obs*) realloc (RA->cpn[i].obs, RA->cpn[i].num_obs *
sizeof (ra_obs)); RA->cpn[i].obs[(int)(j/obs_freq)].fixing_date = ra_fixingdates[i+index][j];
                        RA->cpn[i].obs[(int)(j/obs_freq)].start_date = add_unit(
RA->cpn[i].obs[(int)(j/obs_freq)].fixing_date, RA->spotlag_f, SRT_BDAY, MODIFIED_SUCCEEDING );
                        RA->cpn[i].obs[(int)(j/obs_freq)].end_date = add_unit(
RA->cpn[i].obs[(int)(j/obs_freq)].start_date, RA->tenor_f, SRT_MONTH, NO_BUSDAY_CONVENTION);
...
ra_fixingdates[i+index][j];
        ...
        {
                if (add_unit( stp->date, -RA->spotlag_f, SRT_BDAY, MODIFIED_SUCCEEDING ) >=
RA->dates[i+1]) i++; d = add_unit(stp->date, RA->tenor_f, SRT_MONTH, NO_BUSDAY_CONVENTION);

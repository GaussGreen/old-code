//--------------------------------------------------------------------
//----------------From D.M.'s srt_f_rangeaccrual.c--------------------
//--------------------------------------------------------------------
#include "srt_h_all.h"
#include "math.h"
#include "OPFNCTNS.H"
...
//	RAVolBumpData *bump_data, *bump_tmp;
	...
	if (RA)
	{
		...
		if (RA->float_cpn_dates) free(RA->float_cpn_dates);
		if (RA->dates_f) free(RA->dates_f);
		if (RA->cvg_f) free(RA->cvg_f);
		if (RA->cvg_float_cpn) free(RA->cvg_float_cpn);
				   ...
				   RangeAccrualStruct *RA)
{
	...
	double fra_float_cpn, cpn_float_upper_barrier_vol, cpn_float_lower_barrier_vol, n_std_dev, std_strike, correl;
	...
	//		   eod_flag + ltoday > add_unit( dates[index], -RA->spotlag_f, SRT_BDAY, MODIFIED_SUCCEEDING );
	//		   index++ );
	// Changed on 12 Jan 2004 following Laurent's request
	for ( index=0; index < n_periods && 
			   eod_flag + ltoday > ra_fixingdates[index][0];
			   index++ );
	...
	RA->obs_steps =
		(RangeAccrualObservStep **) calloc(n_periods - index + 1, sizeof(RangeAccrualObservStep *));
	...
	if (RA->cpn_type != 0)
	{
		err = swp_f_get_ref_rate_details(ra_float_refrate, &basis_float_cpn, &freq_float_cpn);
		if (err) goto RETURN_ERR;
		...
		for (i = index; i < n_periods; i++)
		{
			...
			RA->float_cpn_dates[j+1] = add_unit(RA->float_cpn_dates[j], RA->tenor_float_cpn, SRT_MONTH, MODIFIED_SUCCEEDING);
			j+=2;
		}
	}
	...
	stp = RA->obs_steps[0] = (RangeAccrualObservStep *) malloc(sizeof(RangeAccrualObservStep));
	if (!stp)
	{
		err = "memory allocation error in ra_init";
		goto RETURN_ERR;
...
/*	stp->date = d = RA->dates[0];
	...
	{
		stp->next = (RangeAccrualObservStep *) malloc(sizeof(RangeAccrualObservStep));
		stp = stp->next;
...
*/
	...
	stp->date = d = add_unit( ra_fixingdates[index][0], 
								RA->spotlag_f,
								SRT_BDAY, 
								MODIFIED_SUCCEEDING );
	j=obs_freq;
	while (i < n_periods - index)
	{
		...
		{
			d = add_unit( ra_fixingdates[i+index][j], 
								RA->spotlag_f,
								SRT_BDAY, 
								MODIFIED_SUCCEEDING );
		}
...
*/
	...
	for (i = index, stp = RA->obs_steps[0]; stp->next; stp = stp->next)
	{
		...
		if (stp == RA->obs_steps[i+1-index] || stp == RA->obs_steps[0])
		{
			...
			if (stp != RA->obs_steps[0]) i++;

			// Calculates the RA floating coupon stuff 
			if (RA->cpn_type != 0)
			{
				
				d_cpn = add_unit(ra_float_startl[i], RA->tenor_float_cpn, SRT_MONTH, MODIFIED_SUCCEEDING);
				...
				fra_float_cpn = swp_f_fra(ra_float_startl[i], d_cpn, RA->basis_float_cpn, yc_cpn_float, ra_float_refrate);
				cpn_float_spread = swp_f_spread(ra_float_startl[i], d_cpn, ra_float_refrate);
				err = swp_f_SABRvol(volc_cpn_float, ra_float_startl[i], d_cpn, 0.05, &vol_fra_cpn, &power, SABR_ATMLOG);
				if (err) goto RETURN_ERR;

				// Quanto adjustment of the floating coupon
				if (RA->float_cpn_is_dom_for == 1)
				{
					// Get the FX ATM vol & FX correlations (using the same ones as for the quanto index
					for (; j < n_fxvol_dates-1 && fxvol_dates[j+1] < ra_float_startl[i]; j++);
					...
					else
					{
						...
						rho_ffx_cpn = coef_cpn * qtocorrel[j+1] + (1.0-coef_cpn) * qtocorrel[j];
					}
				...
				if (ra_float_startl[i] == RA->dates[i-index] && d_cpn == RA->dates[i+1-index]) DRS_corr_cpn = 0;
				else
				{
					d1_cpn = (RA->dates[i+1-index] < d_cpn ? RA->dates[i+1-index] : d_cpn);
					d2_cpn = (RA->dates[i+1-index] > d_cpn ? RA->dates[i+1-index] : d_cpn);
					...
					err = swp_f_SABRvol(volc_d, d1_cpn, d2_cpn, fra_d_cpn, &vol_d_cpn, &power, SABR_LOGVOL);
					if (err) return err;
						
					DRS_corr_cpn = rho_df * vol_d_cpn * vol_fra_cpn * cvg_d_cpn * fra_d_cpn / (1.0 + cvg_d_cpn * fra_d_cpn);
					if (RA->dates[i+1-index] > d_cpn) DRS_corr_cpn = -DRS_corr_cpn;
				}

				// Initially was getting the smile of the floating coupon
				// in order to get the correct LOGVOL when the LGM diffusion is done
				// and calculate the correct numeraire change. However, since Bertrand was already
				// freezing the logvol today for the index adjustment, we will do the same.

				// We now get the logvol of the floating coupon for the numeraire adjustment
				
				// Floating coupon volatility for the numeraire change if ATM or ATS
				if (float_adj_strike == 0)
				{
					// ATM
					err = swp_f_SABRvol( volc_cpn_float, ra_float_startl[i], d_cpn, 0.05,&cpn_float_lower_barrier_vol, &power, SABR_ATMLOG );
					if (err) goto RETURN_ERR;
				...
				{
					// ATS
					err = swp_f_SABRvol( volc_cpn_float, ra_float_startl[i], d_cpn, lower_barr[i],&cpn_float_lower_barrier_vol, &power, SABR_LOGVOL );
					if (err) goto RETURN_ERR;
					err = swp_f_SABRvol( volc_cpn_float, ra_float_startl[i], d_cpn, upper_barr[i],&cpn_float_upper_barrier_vol, &power, SABR_LOGVOL );
					if (err) goto RETURN_ERR;
				}
			}
		}
		...
		if (RA->cpn_type != 0)
		{
			...
			stp->cvg = ((double)(stp->next->date - stp->date)) / ((double)(RA->obs_steps[i+1-index]->date - RA->obs_steps[i-index]->date))
			...
			correl = (correl_end - correl_start) / (RA->dates[i+1-index] - RA->dates[i-index]) * (stp->date - RA->dates[i-index]) + correl_start;
		...
		// If floating coupon AND numeraire adjustment method is std dev logvol of the index
		// stdev depends on the index so has to be done for every index
		if (RA->cpn_type !=0 && float_adj_strike == 2)
		{
			if (vol_f > 0.0)
			{
				// Lower barrier stdev logvol
				n_std_dev = log (lower_barr[i] / fra_f) / (vol_f * sqrt((stp->date - today) / 365.0));
				std_strike = fra_float_cpn * exp(correl * vol_fra_cpn * n_std_dev * sqrt((ra_float_startl[i] - today) / 365.0));
				err = swp_f_SABRvol(volc_cpn_float, ra_float_startl[i], d_cpn, std_strike, &cpn_float_lower_barrier_vol, &power, SABR_LOGVOL);
				if (err) goto RETURN_ERR;
			
				// Upper barrier stdev logvol
				n_std_dev = log (upper_barr[i] / fra_f) / (vol_f * sqrt((stp->date - today) / 365.0));
				std_strike = fra_float_cpn * exp(correl * vol_fra_cpn * n_std_dev * sqrt((ra_float_startl[i] - today) / 365.0));
				err = swp_f_SABRvol(volc_cpn_float, ra_float_startl[i], d_cpn, std_strike, &cpn_float_upper_barrier_vol, &power, SABR_LOGVOL);
				if (err) goto RETURN_ERR;
		...
		if (RA->cpn_type != 0)
		{
			err = swp_f_SABRvol(volc_f, stp->date, d, lower_barr[i], &vol_f_lower_barr, &power, SABR_LOGVOL);
			if (err) goto RETURN_ERR;
			stp->cpn_float_lower_barrier_adj *= vol_f_lower_barr;

			err = swp_f_SABRvol(volc_f, stp->date, d, upper_barr[i], &vol_f_upper_barr, &power, SABR_LOGVOL);
			if (err) goto RETURN_ERR;
		...
		if (RA->dates[i+1-index] == d) DRS_corr = 0;
		else
		{
			...
			if (RA->dates[i+1-index] > d) DRS_corr = -DRS_corr;
		}
		...
		err = swp_f_SABRvol( volc_f, stp->date, d, 0.05,
		...
		err = swp_f_SABRvol( volc_f, stp->date, d, 0.05,
				&stp->normvol, &power, SABR_ATMNORM );
		if (err) 
		{
			goto RETURN_ERR;
		}
		err = swp_f_SABRvol( volc_f, stp->date, d, 0.05,
				&stp->lognvol, &power, SABR_ATMLOG );
		if (err) 
		{
			goto RETURN_ERR;
		}
...
Err RA_RequestDfDates(RangeAccrualStruct *RA, long date,
								int *pn_unds, int **pn_dfs, long ***pdates)
{
	...
	*pdates = (long **) calloc(*pn_unds, sizeof(long*));
	...
	if (RA->cpn_type != 0)
	{
		...
		if (RA->quanto) cpn_col = 2; // 0 and 1 are taken
		else cpn_col = 1; // 0 is taken
		...
		memcpy((*pdates)[cpn_col], RA->float_cpn_dates + 2*i, 2 * ndfs_float_cpn * sizeof(long));
	}
...
Err RA_Payoff(RangeAccrualStruct *RA, long today, long date,
						double **dfs, double *payoff)
{
	...
	double coef, fwd, fixing, mat, df, sum, vol, bs[4], bump_dir[4], bs_float_cpn[4];
	double fwd_cpn_float, mat_cpn_float, fwd_lower_barr, fwd_upper_barr, fixing_cpn_float;
...
//	RAVolBumpData *bump_tmp;
	...
	*payoff = 0.0;
	...
	/* If stored deterministic values are no more valid recalculate them (when event date changes)*/
	if (date != RA->last_date)
	{
		...
		if (RA->cpn_type != 0)
		{
			...
			memcpy(dates_cpn_float, RA->float_cpn_dates + 2 * i, 2*RA->ndfs_cpn_float * sizeof(long));
			...
			{
				RA->cvg_float_cpn[k] = coverage(dates_cpn_float[j], dates_cpn_float[j+1], RA->basis_float_cpn);
				k++;
			}
...
//				(RA->ndfs_d + RA->ndfs_f) * sizeof(long) );
	...
	if (RA->cpn_type != 0)
	{
		...
		for (j=0; j < 2*RA->ndfs_cpn_float; j+=2)
		{
			fwds_cpn_float[k] = (dfs_cpn_float[j] / dfs_cpn_float[j+1] - 1.0) / RA->cvg_float_cpn[k];
			k++;
		}
	}
...
/*	if (RA->bump_data->date != today)
	{
		...
		{
			bump_tmp->next = (RAVolBumpData *) malloc(sizeof(RAVolBumpData));
...
*/
	...
	// If floating coupon, setup the floating coupon params for the first period
	if (RA->cpn_type !=0)
	{
		i = RA->period_idx;
		stp = RA->obs_steps[i];
		...
		// Do not need to do RA->nb - i since RA->float_cpn_dates is the entire array (not a partial array)
		fixing_cpn_float = add_unit(RA->float_cpn_dates[2*i], -RA->spotlag_f, SRT_BDAY, MODIFIED_SUCCEEDING);
	...
	for (k=0, stp = RA->obs_steps[i = RA->period_idx]; stp->next; stp = stp->next)
	{
		...
		for (j=0; j < 4; j++)
		{
			...
			{
				bs[j] = 0.0;
				if (RA->cpn_type !=0) bs_float_cpn[j] = 0.0;
			}
			else if (fwd < K_TINY)
			{
				bs[j] = df * (stp->K[j] - fwd);
				if (RA->cpn_type !=0)
				{
					if (j == 0 || j == 1)
					{
						fwd_lower_barr = fwd * exp(stp->cpn_float_lower_barrier_adj * min(mat, mat_cpn_float));
						bs_float_cpn[j] = df * (stp->K[j] - fwd_lower_barr);
					}
					else
					{
						fwd_upper_barr = fwd * exp(stp->cpn_float_upper_barrier_adj * min(mat, mat_cpn_float));
			...
			else
			{
				...
				{
					normvol = stp->normvol;
					if (normvol < K_TINY) normvol = K_TINY;
					err = srt_f_optsarbvol(fwd, stp->K[j], mat, normvol, stp->alpha,
						stp->beta, stp->rho, SRT_NORMAL, SRT_LOGNORMAL, &vol);
					if (err) return err;
				}
				else if(RA->typeVol==2) //Lognormal
				{
					lognvol = stp->lognvol;
					if (lognvol < K_TINY) lognvol = K_TINY;
					err = srt_f_optsarbvol(fwd, stp->K[j], mat, lognvol, stp->alpha,
						stp->beta, stp->rho, SRT_LOGNORMAL, SRT_LOGNORMAL, &vol);
					if (err) return err;
				}
				else //Beta
				{
				...
				if (RA->cpn_type != 0)
				{
					...
					{
						fwd_lower_barr = fwd * exp(stp->cpn_float_lower_barrier_adj * min(mat, mat_cpn_float));
						bs_float_cpn[j] = srt_f_optblksch( fwd_lower_barr, stp->K[j], vol, mat, df,SRT_PUT, PREMIUM );
					}
					else 
					{
						fwd_upper_barr = fwd * exp(stp->cpn_float_upper_barrier_adj * min(mat, mat_cpn_float));
						bs_float_cpn[j] = srt_f_optblksch( fwd_upper_barr, stp->K[j], vol, mat, df,SRT_PUT, PREMIUM );
					}
				}
			}
		}
...
Err RA_FwdPV(char *yc_d, char *yc_f, 
			 RangeAccrualStruct *RA, 
			 long today, 
			 int nbDates, long *Dates, double *fwdPV)
{
	...
	for(i=0;i<nbDates;++i)
	{
//		date = add_unit( RA->dates[i], -RA->spotlag_f - 1, SRT_BDAY, MODIFIED_SUCCEEDING );
		date = Dates[i];
			...
			ra_leg *RA)
{
	...
	for ( index=0; index < num_dates && 
			   eod_flag + ltoday > add_unit( start_dates[index], -RA->spotlag_f, SRT_BDAY, MODIFIED_SUCCEEDING );
			   index++ );
	...
	for(i=0;i < num_dates - index;++i)
	{
		...
		RA->cpn[i].start_time = (start_dates[i] - ltoday) * YEARS_IN_DAY;
		RA->cpn[i].pay_date = end_dates[i];
		...
		RA->cpn[i].obs[0].end_date = add_unit( RA->cpn[i].obs[0].start_date, 
								RA->tenor_f,
								SRT_MONTH, 
								NO_BUSDAY_CONVENTION);
		RA->cpn[i].obs[0].spread = swp_f_spread(RA->cpn[i].obs[0].start_date, 
												RA->cpn[i].obs[0].end_date, 
												refrate_f);

		if(obs_freq < ra_nfixings[i+index])
		{
			weight = ra_fixingdates[i+index][obs_freq] - ra_fixingdates[i+index][0];
		}
		else
		{
		...
		RA->cpn[i].obs[0].cpn = cpns[i] 
				* weight
				/ (end_dates[i] - start_dates[i])
				* coverage(end_dates[i], start_dates[i], ibasis);

		for(j=obs_freq ; j < ra_nfixings[i+index]; j = j + obs_freq)
		{
			RA->cpn[i].num_obs += 1;
			RA->cpn[i].obs = (ra_obs*) realloc (RA->cpn[i].obs, RA->cpn[i].num_obs * sizeof (ra_obs));
			RA->cpn[i].obs[(int)(j/obs_freq)].fixing_date = ra_fixingdates[i+index][j];
			RA->cpn[i].obs[(int)(j/obs_freq)].start_date = add_unit( RA->cpn[i].obs[(int)(j/obs_freq)].fixing_date, 
								RA->spotlag_f,
								SRT_BDAY, 
								MODIFIED_SUCCEEDING );
			RA->cpn[i].obs[(int)(j/obs_freq)].end_date = add_unit( RA->cpn[i].obs[(int)(j/obs_freq)].start_date, 
								RA->tenor_f,
								SRT_MONTH, 
								NO_BUSDAY_CONVENTION);
			...
			{
				weight = ra_fixingdates[i+index][j + obs_freq ] - ra_fixingdates[i+index][j];
			}
			else
			{
				weight = end_dates[i] - ra_fixingdates[i+index][j];
			}
	...
	{
		if (add_unit( stp->date, -RA->spotlag_f, SRT_BDAY, MODIFIED_SUCCEEDING ) >= RA->dates[i+1]) i++;
		d = add_unit(stp->date, RA->tenor_f, SRT_MONTH, NO_BUSDAY_CONVENTION);
		stp->spread = swp_f_spread(stp->date, d, refrate_f);
		stp->cpn = cpns[i] * (stp->next->date - stp->date)
							/ (RA->dates[i+1] - RA->dates[i])
							* coverage(RA->dates[i], RA->dates[i+1], ibasis);
		...
		else
		{
			...
			DRS_corr = rho_df * vol_d * vol_f * cvg_d * fra_d / (1.0 + cvg_d * fra_d);
			if (RA->dates[i+1] > d) DRS_corr = -DRS_corr;
		}
		stp->correction = quanto_corr + DRS_corr;

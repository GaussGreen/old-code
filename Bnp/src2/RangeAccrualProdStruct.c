...
Err ra_init_struct(
  ...
  //		   eod_flag + ltoday > add_unit( dates[index]  , -RA->spotlag_f  ,
  ...
  for (i = index, stp = RA->obs_steps[0]; stp->next; stp = stp->next) {
    ...
    if (stp == RA->obs_steps[i + 1 - index] || stp == RA->obs_steps[0]) {
      ...
      if (RA->cpn_type != 0) {
        ...
        // and calculate the correct numeraire change. However  , since Bertrand
        // was already freezing the logvol today for the index adjustment  , we
        // will do the same.
...
Err init_ra_leg(
                        ...
                        long			**ra_fixingdates  ,
                        double			**ra_fixings  ,
//	Past coupon fixing if relevant double			*upper_barr  ,
                        double			*lower_barr  ,
                        char			*recpay  ,
                        double			c_spread  ,
                        int				obs_freq  ,
...
0: I  , 1: E ra_leg *RA)
{
        ...
        double vol_fx  , rho_ffx  , coef  , fra_f  , vol_f  , fra_d  , cvg_d  ,
vol_d  , power; double quanto_corr  , DRS_corr; SrtReceiverType irecpay;
        SrtCurvePtr ycd_ptr;
        long ltoday;
...
RA->cpn[i].obs[(int)(j/obs_freq)].start_date  , RA->tenor_f  , SRT_MONTH  ,
                                                                NO_BUSDAY_CONVENTION);
...
start_dates[i]  , ibasis);
        ...
        {
                if (add_unit( stp->date  , -RA->spotlag_f  , SRT_BDAY  ,
MODIFIED_SUCCEEDING ) >= RA->dates[i+1]) i++; d = add_unit(stp->date  ,
RA->tenor_f  , SRT_MONTH  , NO_BUSDAY_CONVENTION); stp->spread =
swp_f_spread(stp->date  , d  , refrate_f); stp->cpn = cpns[i] * (stp->next->date
- stp->date) / (RA->dates[i+1] - RA->dates[i])
                                                        * coverage(RA->dates[i]
, RA->dates[i+1]  , ibasis);

                fra_f = swp_f_fra(stp->date  , d  , basis_f  , yc_f  ,
refrate_f); err = swp_f_SABRvol(volc_f  , stp->date  , d  , fra_f  , &vol_f  ,
...
refrate_d); cvg_d = coverage(d1  , d2  , basis_d); err = swp_f_SABRvol(volc_d  ,
d1  , d2  , fra_d  , &vol_d  , &power  , SABR_LOGVOL); if (err)
                        {
                                goto RETURN_ERR;
                        }

...
char* TARN_set_calib_param(TARN_Struct* tarn, TARN_AUX* aux, TARN_CALIB_AUX* calib_aux)
{
    /* variable declaration */
    char* err = 0;
    ...
    // added to change the end date of the swaption calibration to be always the end date of the
    // deal
...
void alloc_tarn_lgm(TARN_Struct* tarn, TARN_lgm* pCal)
{
    ...
    {
        pCal->dmVol[0][i] = pCal->sigma_dates ? pCal->sigma_dates[i]
    ...
    {
        pCal->dmTau[0][i] =
            pCal->tau_dates ? pCal->tau_dates[i] : tarn->market.lToday + 365.0 * pCal->lam_time[i];
    ...
    if (pCal->calalpha)
    {
        ...
        {
            pCal->smile_datas[0][i] = pCal->smile_dates
                                          ? pCal->smile_dates[i]
                                          : tarn->market.lToday + 365.0 * pCal->sig_time[i];

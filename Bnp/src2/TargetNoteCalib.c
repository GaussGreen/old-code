...
/* -------------------------------------------------------------------------------- */
/* 
	Calibrate to caplets.  If the lambda value is not input, calibrate to diagonal
	swaptions.  The LGM2F parameters must be input.
*/
/* -------------------------------------------------------------------------------- */
...
/* set the strikes: if it is both capped and floored we calibrate ATM */
	...
	{	
		if ( fabs(tarn->deal.dvGearing[i]) <= 1.0e-8 || (tarn->deal.ivIsFloored[i] && tarn->deal.ivIsCapped[i])  )
			ptrInst->strike[i] = 0.0;
		else 
			ptrInst->strike[i] = fabs( tarn->deal.dvCoupon[i] / tarn->deal.dvGearing[i] );
...
void TARN_calib_freeInst( DiagCalibInstStruct* ptrInst )
{
	...
	if ( ptrInst->cal_date ) free( ptrInst->cal_date );
	if ( ptrInst->strike ) free( ptrInst->strike );
	if ( ptrInst->strikeP1 ) free( ptrInst->strikeP1 );
	if ( ptrInst->strikeM1 ) free( ptrInst->strikeM1 );
	if ( ptrInst->end_tenor )
	{
		for ( i=0; i<ptrInst->num_ex_dates; i++ )
			if ( ptrInst->end_tenor[i] ) free( ptrInst->end_tenor[i] );
		free( ptrInst->end_tenor );
	}

}

char* TARN_set_calib_param(
							TARN_Struct* tarn,
							TARN_AUX* aux,
...
/* set up the strikes and tenors */
	TARN_calib_setInst( tarn, 
						tarn->calibration.szPrimTenor,
						tarn->calibration.prim_param.strike_type,
						&calib_aux->Primary);

	TARN_calib_setInst( tarn, 
						tarn->calibration.szSecTenor,
						tarn->calibration.sec_param.strike_type,
						&calib_aux->Secondary );

	// added to change the end date of the swaption calibration to be always the end date of the deal
//	calib_aux->Secondary.end_date = tarn->deal.lvFundEndDates[ tarn->deal.nFundDates- 1 ];

/* if the KO calibration is requested, generate the probs and set the required instruments */
	if ( tarn->calibration.sec_param.strike_type == 4 )
	{
		if ( err = TARN_calc_KO_prob_2F(	tarn,
											aux,
											calib_aux	)	)
			goto FREE_RETURN;
...
void free_tarn_lgm( TARN_lgm* pCal )
{
	...
	if ( pCal->dmVol ) free_dmatrix( pCal->dmVol, 0, 1, 0, pCal->num_sig-1 );
	if ( pCal->dmTau ) free_dmatrix( pCal->dmTau, 0, 1, 0, pCal->num_tau-1 );
	if ( pCal->smile_datas ) free_dmatrix( pCal->smile_datas, 0, pCal->smile_col, 0, pCal->num_smile-1 );
}
...
void alloc_tarn_lgm( TARN_Struct* tarn, TARN_lgm* pCal )
{
	...
	{
		pCal->dmVol[0][i] = pCal->sigma_dates ? pCal->sigma_dates[i] : tarn->market.lToday + 365.0*pCal->sig_time[i];
	...
	for ( i=0; i<pCal->nlam; i++ )
	{
		pCal->dmTau[0][i] = pCal->tau_dates ? pCal->tau_dates[i] : tarn->market.lToday + 365.0*pCal->lam_time[i];
	...
	if ( pCal->calalpha )
	{
		...
		{
			pCal->smile_datas[0][i] = pCal->smile_dates ? pCal->smile_dates[i] : tarn->market.lToday + 365.0 * pCal->sig_time[i];
...
/* call the calibration routine */
		if ( err = cpd_calib_diagonal_dlm(
					/* Market info */
										...
										tarn->market.szVolCurve,
										tarn->market.getCashVol,
										tarn->market.szVolCurveRefRate,
					/* primary info */
										tarn->calibration.szPrimFreq,
										tarn->calibration.szPrimBasis,
										tarn->calibration.szPrimRefRate,				
										tarn->deal.nCouponDates,
										tarn->deal.lvFltrFixingDates,
										calib_aux->Primary.cal_date,
										calib_aux->Primary.end_tenor,
										calib_aux->Primary.end_date,
										calib_aux->Primary.strike,
										&(tarn->calibration.prim_param),
					/* secondary info */
										tarn->calibration.szSecFreq,
										tarn->calibration.szSecBasis,
										tarn->calibration.szSecRefRate,				
										tarn->deal.nCouponDates,
										tarn->deal.lvFltrFixingDates,
										calib_aux->Secondary.cal_date,
										calib_aux->Secondary.end_tenor,
										calib_aux->Secondary.end_date,
										calib_aux->Secondary.strike,
										0,/*calib_aux->Swap.weights,*/
										&(tarn->calibration.sec_param),
					/* Model info */
										tarn->calibration.fix_lambda,
										0,	/*	Force 2F lambda */
...
/* Instantiate the underlying */
	if ( err = SrtInitIRUnd( szTARN_LGM2F_UND, tarn->market.szYieldCurve, "LGM2F", cal.num_sig, 2, cal.dmVol,
				    cal.nlam, 2, cal.dmTau, 0.0, cal.alpha, cal.gamma, cal.rho, 
					0.0, 0.0, 0.0, 0, 0, 0)  )
		goto FREE_RETURN;
...
/* primary info */
			...
			calib_aux->Primary.end_date,
			calib_aux->Primary.strike,
			calib_aux->Primary.strikeP1,
			calib_aux->Primary.strikeM1,
			&tarn->calibration.prim_param,
/* secondary info */
			tarn->calibration.szSecFreq,
			tarn->calibration.szSecBasis,
			...
			calib_aux->Secondary.end_date,
			calib_aux->Secondary.strike,
			calib_aux->Secondary.strikeP1,
			calib_aux->Secondary.strikeM1,
			NULL,
			&tarn->calibration.sec_param,
/* Calibration */
			&tarn->calibration.lgmsv_calib_params,
			cal.nFactors,
			cal.lam,
			cal.nFactors==1 ? tarn->model.nsmilepar1F : tarn->model.nsmilepar,
			cal.nFactors==1 ? tarn->model.smilepartime1F : tarn->model.smilepartime,
			cal.nFactors==1 ? tarn->model.alphaepsts1F : tarn->model.alphaepsts,
			...
			tarn->model.gamma,
			tarn->model.rho,
			tarn->model.rho2epsts,
			&(calib_aux->NumerParams),
			&cal.num_sig,
			&cal.sig_time,
			&cal.sig,
			&cal.calalpha,
...
/* Initialize the new und */
	if ( err = SrtInitIRMSVUnd( szTARN_LGMSV_UND, 
								tarn->market.szYieldCurve, 
								cal.nFactors, 
								cal.dmVol, 
								cal.num_sig, 
...
/* LGM2F calibration */
	...
	{
		if ( err = TARN_calib_LGM2F(
										tarn->market.szLGM2FUnd,
										tarn->output.szTARN_LGM2F_UND,
										tarn,
										2,
										calib_aux,
										&tarn->output.inst_data_lgm2F	)	)
		goto FREE_RETURN;


/* LGM1F calibration */
		if ( err = TARN_calib_LGM2F(
										tarn->market.szLGM1FUnd,
										tarn->output.szTARN_LGM1F_UND,
										tarn,
										1,
										calib_aux,
										&tarn->output.inst_data_lgm1F	)	)
		goto FREE_RETURN;

/* LGM1FSV calibration */
		if ( err = TARN_calib_LGMSV(
										tarn->market.szLGM1FSVUnd,
										tarn->output.szTARN_LGM1FSV_UND,
										tarn,
										1,
										calib_aux,
										&tarn->output.inst_data_lgm1FSV			)	 )
		goto FREE_RETURN;
	}
...
/* LGMSV calibration */
	if ( tarn->pricing.iPriceSV )
		if ( err = TARN_calib_LGMSV(
										tarn->market.szLGMSVUnd,
										tarn->output.szTARN_LGMSV_UND,
										tarn,
										2,
										calib_aux,
										&tarn->output.inst_data_lgmSV			)	 )
		goto FREE_RETURN;
...
/* run a Monte-Carlo to calculate the KO probs */
	if ( err = TargetNoteMC_KO_2F( 
						szTempUnd,
						tarn,
						aux,
						tarn->output.dmKO_prob
					) )
		goto FREE_RETURN;

/*
Feb 2001. LTQLM Inc.
Wrapper Author: AdA (Alex d'Aspremont, aspremon@stanford.edu, +1 415 864 2962)
Base code: Miguel Sousa Lobo, Lieven Vandenberghe, and Stephen Boyd.
See code website: www.stanford.edu/~boyd/SOCP.html

WARNING: Matrixes are stored in flat BLAS-LAPACK (FORTRAN) format, column major order, arrays starting at 0,
the function ijtok(i,j,number_of_rows) makes the conversion.

This solves a quadratic optimisation program under LP constraints using an SOCP solver.
See header for source description.
*/
...
Err socp_smoothvol(int **index_set, double *upperbounds, double *lowerbounds, int numpoints, int num_dates, double centering_weight, double **result, int printlevel)
{
	...
	Err err;
	// Upper bound on vol. 
	double SOCP_MAXVOL=1.0; /// *** Warning: this must be set to an appropriate default value ***

	// Fill the objective matrix
	k=0;
	for (j=2;j<=num_dates;j++) for (i=1;i<=j-1;i++) 
	{
		k++;
	...
	for (i=2*numpoints+1;i<=2*numpoints+dim;i++) constraint_matrix[i][i-2*numpoints]=1.0;
	...
	err=socp_smooth_lp(objective_matrix,constraint_matrix,constraint_vector,2*dimobj,dim,2*numpoints+2*dim,centering_weight,resbuf,printlevel);
	// return result
	for (j=1;j<=num_dates;j++) for (i=1;i<=j;i++) result[i][j]=resbuf[toflat(i,j)];
	for (j=1;j<=num_dates;j++) for (i=j;i<=num_dates;i++) result[i][j]=resbuf[toflat(j,i)];
	// free and return
...
Err socp_bidask(double *upperbounds, double *lowerbounds, int numpoints, double centering_weight, double *result, int printlevel)
{
	...
	err=socp_smooth_lp(objective_matrix,constraint_matrix,constraint_vector,numpoints-1,numpoints,2*numpoints,centering_weight,result,printlevel);
	// free and return
...
Err socp_smooth_lp(double **objective_matrix, double **constraints_matrix, double *constraints_value, int nobjrows, int dim, int numconst, double alpha, double *result, int printlevel)
{
	Err err=NULL;
	...
	{
		alpha=defaut_alpha;
		if (printlevel>=2) smessage("SdpSOCP warning: Centering should be in [0,1], using default value... ");
	}
	// Make some tests
	if (numconst<2) err=serror("SdpSOCP error: Not enough constraints ....");
	if (dim<2) err=serror("SdpSOCP error: Problem is scalar.... ");
	// Start phase one and compute the center solution.
	if (err==NULL) err=phase_one(objective_matrix,constraints_matrix,constraints_value,nobjrows,dim,numconst,centersol,dualsol,printlevel);
	for (i=1;i<=dim;i++) result[i]=alpha*centersol[i-1];
	// Test for infeasibility 
	if (centersol[dim+1]>=0) err=serror("SdpSOCP error: Problem is infeasible.... ");
	// Start phase two if phase one OK and feasible, compute the smooth solution.
	if (err==NULL) err=phase_two(objective_matrix,constraints_matrix,constraints_value,nobjrows,dim,numconst,centersol,dualsol,printlevel);
	for (i=1;i<=dim;i++) result[i]+=(1-alpha)*centersol[i-1];
	// Free and return
...
// converts between (i,j) matrix storage and flat LAPACK matrix storage, column major order.
int ijtok(int i, int j, int numrow)
{
	return (i-1)+(j-1)*numrow;
}
...
{
	int i,j;
	for (i=1;i<=rowstofill;i++) for (j=1;j<=dimcol;j++)
	{
		if (i==j) gradmat[i][j]=-1.0;
		if (j==i+1) gradmat[i][j]=1.0;
	}
	return NULL;
}
...
Err phase_one(double **objective_matrix, double **constraints_matrix, double *constraints_value, int nobjrows, int dim, int numconst, double *centersol, double *dualsol, int printlevel)
{
	// Algo params
	int iter=80;
	int out_mode=1;
	...
	for (i=2;i<=numconst+2;i++) vecdimconstin[i-1]=1;
	// Set up the working arrays
	socp_getwork(numconst+2,vecdimconstin,dim+2,iter,out_mode,&mhist,&nhist,&ndbl,&nint);
	// Calls the core procedure
	err=phase_one_go(objective_matrix,constraints_matrix,constraints_value,nobjrows,dim,numconst,centersol,dualsol,vecdimconstin,ndbl,nint,mhist,nhist,iter,out_mode,printlevel);
	// Free and return
...
Err phase_two(double **objective_matrix, double **constraints_matrix, double *constraints_value, int nobjrows, int dim, int numconst, double *centersol, double *dualsol, int printlevel)
{
	// Algo params
	int iter=50;
	int out_mode=1;
	...
	for (i=2;i<=numconst+2;i++) vecdimconstin[i-1]=1;
	// Set up the working arrays
	socp_getwork(numconst+2,vecdimconstin,dim+1,iter,out_mode,&mhist,&nhist,&ndbl,&nint);
	// Calls the core procedure
	err=phase_two_go(objective_matrix,constraints_matrix,constraints_value,nobjrows,dim,numconst,centersol,dualsol,vecdimconstin,ndbl,nint,mhist,nhist,iter,out_mode,printlevel);
	// Free and return
	free_ivector(vecdimconstin,0,numconst+2);
	return err;
}
...
// Looks for the smoothest solution inside the inequalitites.
Err phase_one_go(double **objective_matrix, double **constraints_matrix, double *constraints_value, int nobjrows, int dim, int numconst, double *centersol, double *dualsol, int *vecdimconstin, int ndbl, int nint, int mhist, int nhist, int iter, int out_mode, int printlevel)
{
	// Algorithm params
	double abs_tol=-1.0;
	double rel_tol=1e-6;
	...
	for (i=1;i<=nobjrows;i++) for (j=1;j<=dim;j++) amatin[ijtok(i,j,numconst+nobjrows+2)]=objective_matrix[i][j];
	...
	for (i=nobjrows+2;i<=nobjrows+1+numconst;i++) for (j=1;j<=dim;j++) 
	{
		amatin[ijtok(i,j,numconst+nobjrows+2)]=constraints_matrix[i-(nobjrows+1)][j];
		// the big M constraint.
		amatin[ijtok(numconst+nobjrows+2,j,numconst+nobjrows+2)]-=constraints_matrix[i-(nobjrows+1)][j];
	}
	amatin[ijtok(numconst+nobjrows+2,dim+1,numconst+nobjrows+2)]=-1.0;
	// The feasibility variable column.
	for (i=nobjrows+2;i<=nobjrows+1+numconst;i++) amatin[ijtok(i,dim+2,numconst+nobjrows+2)]=1.0;
	// Big_M again.
	amatin[ijtok(numconst+nobjrows+2,dim+2,numconst+nobjrows+2)]=-(double)(numconst+1);
	...
	for (i=1;i<=numconst;i++) if (bufa<fabs(constraints_value[i])) bufa=fabs(constraints_value[i]);
	xvar[dim+1]=feasibility_margin*bufa+1;
	// The dual variables ...
	err=feasible_dual_vecs_one(fvecin,amatin,nobjrows,dim,numconst,zvar);
	// Check feasibility
	info=0;
	info=check_feasibility(amatin,fvecin,nobjrows,dim,numconst,zvar,vecdimconstin,bvecin,xvar);
	if (info>0) err=serror("SdpSOCP error: No strictly feasible solution has been found... ");
	// Calls the SOCP procedure
	if (err==NULL) output=socp(numconst+2,vecdimconstin,dim+2,fvecin,amatin,bvecin,xvar,zvar,abs_tol,rel_tol,target,&iter,nu,&info,out_mode,hist,dblwork,intwork);
	// Process the output
	for (i=1;i<=nobjrows+numconst+3;i++) dualsol[i-1]=zvar[i-1];
	for (i=1;i<=dim+2;i++) centersol[i-1]=xvar[i-1];
	if (output!=0) err=serror("SdpSOCP error: Error in socp core... ");
	// free and return
...
// Looks for the smoothest solution inside the inequalitites.
Err phase_two_go(double **objective_matrix, double **constraints_matrix, double *constraints_value, int nobjrows, int dim, int numconst, double *centersol, double *dualsol, int *vecdimconstin, int ndbl, int nint, int mhist, int nhist, int iter, int out_mode, int printlevel)
{
	// Algorithm params
	double abs_tol=-1.0;
	double rel_tol=1e-6;
	...
	fvecin[dim+1]=1.0;
	// The dual variables ...
	err=feasible_dual_vecs_two(fvecin,amatin,nobjrows,dim,numconst,dualsol);
	// Calls the SOCP procedure
	if (err==NULL) output=socp(numconst+2,vecdimconstin,dim+1,fvecin,amatin,bvecin,centersol,dualsol,abs_tol,rel_tol,target,&iter,nu,&info,out_mode,hist,dblwork,intwork);
	if (output!=0) err=serror("SdpSOCP error: Error in socp core... ");
	// free and return
...
double normvec(double *vec, int ilow, int ihigh)
{
	int i;
	double buf=0;
	for (i=ilow;i<=ihigh;i++) buf+=vec[i]*vec[i];
	return sqrt(buf);
}

// square
double ddsqr(double a)
{
	return a*a;
}
...
Err feasible_dual_vecs_one(double *fvecin, double *amatin, int nobjrows, int dim, int numconst, double *zvar)
{
	int i,j;
	double *abuf=dvector(0,(dim+2)*(numconst+nobjrows+2));
	double *bvecbuf=dvector(0,numconst+nobjrows+1);
	int mp=dim+2,np=numconst+nobjrows+1,nrhsp=1,ldap=dim+2,ldbp=numconst+nobjrows+1,infop;
	...
	for (i=1;i<=numconst+nobjrows+2;i++) for (j=1;j<=dim+2;j++) abuf[ijtok(j,i,dim+2)]=amatin[ijtok(i,j,numconst+nobjrows+2)];
	for (i=0;i<=dim+1;i++) bvecbuf[i]=fvecin[i];
	// Calls the LAPACK function.
	dgels_("N",&mp,&np,&nrhsp,abuf,&ldap,bvecbuf,&ldbp,dwork,&dimlwork,&infop);
	// Process the result
	bufa=normvec(bvecbuf,0,numconst+nobjrows);
	for (i=nobjrows+1;i<=numconst+nobjrows+1;i++) if (bvecbuf[i-1]<-bufa) bufa=-bvecbuf[i-1];
	for (i=nobjrows+1;i<=numconst+nobjrows+1;i++) bvecbuf[i-1]+=feasibility_margin*bufa+1;
	// return the result
	for (i=1;i<=numconst+nobjrows+1;i++) zvar[i-1]=bvecbuf[i-1];
...
// Computes a dual feasible starting point for phase one. z=inv(A)(f-c2), ....
Err feasible_dual_vecs_two(double *fvecin, double *amatin, int nobjrows, int dim, int numconst, double *zvar)
{
	int i,j;
	double *abuf=dvector(0,(dim+1)*(numconst+nobjrows+2));
	double *bvecbuf=dvector(0,numconst+nobjrows+1);
	int mp=dim+1,np=numconst+nobjrows+1,nrhsp=1,ldap=dim+1,ldbp=numconst+nobjrows+1,infop;
	double *sp=dvector(0,dim+1);
	double rcond=-1.0;
	int dimlwork=2*(dim+2+max(2*(dim+2),numconst+nobjrows+1)); // This is twice the minimum size in dgelss but improves speed.
	double *dwork=dvector(0,dimlwork);
	double bufa=0.0;
	double feasibility_margin=1.5; // Must be greater than 1...

	// Copy A-> At in abuf and f into bvec
	for (i=1;i<=numconst+dim+1;i++) for (j=1;j<=dim+1;j++) abuf[ijtok(j,i,dim+1)]=amatin[ijtok(i,j,numconst+nobjrows+2)];
	for (i=0;i<=dim;i++) bvecbuf[i]=fvecin[i];
	// Calls the LAPACK function.
	dgels_("N",&mp,&np,&nrhsp,abuf,&ldap,bvecbuf,&ldbp,dwork,&dimlwork,&infop);
	// Process the result
	bufa=normvec(bvecbuf,0,numconst+nobjrows);
...
// Checks for strict feasibility... (Possible trouble if initial point is to close to infeasible).
int check_feasibility(double *amatin, double *fvecin, int nobjrows, int dim, int numconst, double *zvar, int *vecdimconstin, double *bvecin, double *xvar)
{
	int i,j;
	double *abuf=dvector(0,(dim+2)*(numconst+nobjrows+2));
	...
	// Dual feasibility...
	for (i=1;i<=numconst+nobjrows+2;i++) for (j=1;j<=dim+2;j++) abuf[ijtok(j,i,dim+2)]=amatin[ijtok(i,j,numconst+nobjrows+2)];
	for (i=1;i<=dim+2;i++) vecbuf1[i-1]=fvecin[i-1];
	bufa=normvec(zvar,0,nobjrows-1);
	if (zvar[nobjrows]-bufa<minmargin) info=1;
	if (zvar[nobjrows+numconst+1]<minmargin) info=2;
	for (i=nobjrows+1;i<=numconst+nobjrows+2;i++) if (zvar[i-1]<minmargin) info=3;
	dgemv_("N",&mp,&np,&alpha,abuf,&ldap,zvar,&int1,&beta,vecbuf1,&int1);
	bufa=normvec(vecbuf1,0,dim+1);
	if ((bufa/(dim+1))>minmargin) info=4;
	// Primal feasibility ...
	beta=0.0;alpha=1.0;
	mp=nobjrows+numconst+2;np=dim+2;ldap=nobjrows+numconst+2;
	dgemv_("N",&mp,&np,&alpha,amatin,&ldap,xvar,&int1,&beta,vecbuf2,&int1);
	bufa=normvec(vecbuf2,0,nobjrows-1);
	if (vecbuf2[nobjrows]-bufa<minmargin) info=5;
	for (i=nobjrows+2;i<=nobjrows+numconst+1;i++) if ((vecbuf2[i-1]+bvecin[i-1])<minmargin) info=6;
	if ((vecbuf2[nobjrows+numconst+1]+bvecin[nobjrows+numconst+1])<minmargin) info=7;
	// free and return 
	free_dvector(abuf,0,(dim+2)*(numconst+nobjrows+2));

...
WARNING: Matrixes are stored in flat BLAS-LAPACK (FORTRAN) format, column major order, arrays
starting at 0, the function ijtok(i,j,number_of_rows) makes the conversion.
...
Err socp_smooth_lp(
    double** objective_matrix,
    double** constraints_matrix,
    double*  constraints_value,
    int      nobjrows,
    int      dim,
    int      numconst,
    double   alpha,
    ...
    int      printlevel)
{
    ...
    if ((alpha < 0.0) || (alpha > 1.0))
    {
        alpha = defaut_alpha;
        if (printlevel >= 2)
            smessage("SdpSOCP warning: Centering should be in [0,1], using default value... ");
    }
    // Make some tests
    if (numconst < 2)
    ...
    int      printlevel)
{
    ...
    socp_getwork(
        numconst + 2, vecdimconstin, dim + 2, iter, out_mode, &mhist, &nhist, &ndbl, &nint);
    ...
    int      printlevel)
{
    ...
    // Set up the working arrays
    socp_getwork(
        numconst + 2, vecdimconstin, dim + 1, iter, out_mode, &mhist, &nhist, &ndbl, &nint);
    // Calls the core procedure
    err = phase_two_go(
        objective_matrix,
        constraints_matrix,
    ...
    int      printlevel)
{
    ...
    // Check feasibility
    info = 0;
    info = check_feasibility(
        amatin, fvecin, nobjrows, dim, numconst, zvar, vecdimconstin, bvecin, xvar);
    if (info > 0)
        err = serror("SdpSOCP error: No strictly feasible solution has been found... ");
...
Err feasible_dual_vecs_one(
    double* fvecin, double* amatin, int nobjrows, int dim, int numconst, double* zvar)
{
    int     i, j;
    ...
    double* fvecin, double* amatin, int nobjrows, int dim, int numconst, double* zvar)
{
    ...
    int     dimlwork =
        2 * (dim + 2 +
             max(2 * (dim + 2),
                 numconst + nobjrows +
                     1));  // This is twice the minimum size in dgelss but improves speed.
    double* dwork              = dvector(0, dimlwork);
    double  bufa               = 0.0;
    double  feasibility_margin = 1.5;  // Must be greater than 1...
    ...
    double* xvar)
{
    ...
    for (i = nobjrows + 2; i <= nobjrows + numconst + 1; i++)
        if ((vecbuf2[i - 1] + bvecin[i - 1]) < minmargin)
            info = 6;
    if ((vecbuf2[nobjrows + numconst + 1] + bvecin[nobjrows + numconst + 1]) < minmargin)
        info = 7;
    // free and return
    free_dvector(abuf, 0, (dim + 2) * (numconst + nobjrows + 2));
...
void dgapdev(int m, int L, int* N, double* u, double* z, double* pgap, double* pdev)
/*
 ...
 */
{
    ...
    while (
        !((rel_tol < 0.0 && (xtarget =
                                 (ddot_(&n, f, &int1, x, &int1) < target ||
                                  -ddot_(&m, b, &int1, z, &int1) >= target))) ||
          (abs_tol > 0.0 && (xabs = (gap <= abs_tol))) ||
          (rel_tol > 0.0 &&
           (xrel =
                (((lbound = -ddot_(&m, b, &int1, z, &int1)) > 0.0 && gap / lbound <= rel_tol) ||
                 ((lbound = -ddot_(&n, f, &int1, x, &int1)) > 0.0 && gap / lbound <= rel_tol)))) ||
          (xiter = (iter_out >= *iter))))
    {
        ++iter_out;
        ...
        {                 /* solve linear system in least squares sense using SVD */
            ...
            dgelss_(
                ...
                dblwork,
                &lwork,
                &la_info);                    /* dblwork: lwork doubles (>= 5*n) */

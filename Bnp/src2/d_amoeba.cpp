...
y[0..ndim]  , whose components must be preinitialized to the values of funk
evaluated at the ndim+1 vertices (rows) of p; and ftol the fractional
convergence tolerance to be achieved in the function value (n.b.!). On output  ,
p and y will have been reset to ndim+1 new points all within ftol of a minimum
function value  , and nfunk gives the number of function evaluations taken. comm
is a pointer to a communication structure (external parameters that will be
passed to the objective function (3rd parameter)) */
{
  Err err = NULL;

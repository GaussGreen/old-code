...
static void GrfnKiSwapPositivePvCallSpreadCell(
    SrtBarrierType eKnockUpOrDown, double dCallSpreadWidth, String szPositivePvCallSpreadCell)
{
...
static void GrfnKiSwapNegativePvCallSpreadCell(
    SrtBarrierType eKnockUpOrDown, double dCallSpreadWidth, String szNegativePvCallSpreadCell)
{
    if (eKnockUpOrDown == SRT_UP)
    {
        if (dCallSpreadWidth > 0.0)
        ...
        {
            sprintf(szNegativePvCallSpreadCell, "if ( c[0,i] > a[1,i], 1, 0)");
        }
    }
    else
    {
...
static void GrfnKiSwapFixedLegPvCell(SRT_Boolean bStubPaidAtStart, String szFixedLegPvCell)
{
    if (bStubPaidAtStart == SRT_NO)
        sprintf(szFixedLegPvCell, "PVRNG(6,7,a[2,i])-a[5,i]*df(now,a[3,i])");
...
static void GrfnKiSwapFloatLegPvCell(SrtReceiverType eRecPay, String szFloatLegPvCell)
{
    if (eRecPay == SRT_RECEIVER)
        sprintf(szFloatLegPvCell, "PVRNG(8,9,a[2,i])-df(now,a[2,i])*a[4,i]");
...
{
    sprintf(szKnockedInCell, "if(va>0,va:=2,va:= if(c[5,i]>0,c[1,i],c[2,i]))");
}
    ...
    GrfnCell***     pppsTableau)
{
    ...
    *pppsTableau = GrfnCellmatrix(*plNumRows, *plNumCols, GRFN_DEF_ARGBUFSZ);
    ...
    for (i = 0; i < *plNumRows; i++)
    {
        ...
        GrfnKiSwapPositivePvCallSpreadCell(
            eKnockUpOrDown, dCallSpreadWidth, (*pppsTableau)[i][1].sval);
        (*pppsTableau)[i][1].type = GRFNSCELL;

        /* The third column: the call spread value to be used for a negative PV */
        GrfnKiSwapNegativePvCallSpreadCell(
            eKnockUpOrDown, dCallSpreadWidth, (*pppsTableau)[i][2].sval);
    ...
    long            lNumKnockInDates)
{
    ...
    for (i = 0; i < (*pplAuxRangesLength)[4]; i++)
    {
        ...
        (*pppdAuxRanges)[5][i] =
            coverage(plFixedStartDates[j], plKnockInSwapStartDates[i], eFixedBasis) *
    ...
    for (i = (*pplAuxRangesLength)[9] - 1; i >= 0; i--)
    {
        ...
        /* The Floating Leg (REC) Representation: Notional at Beginning + Margin at End - Notional
         * at End */

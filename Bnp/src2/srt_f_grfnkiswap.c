...
/* -------------------------------------------------------------------------------------------
 */
/* Cell in the first column (c[0  ,...]) of the knock in swap tableau : the KI
 * Index FRA (a[0]  ,a[1]) */
static void GrfnKiSwapIndexCell(String szIndexBasis, long lIndexNumMonths,
                                String szUndName, String szIndexRefRateCode,
                                String szIndexCell) {
  sprintf(szIndexCell, "FRA(a[0  ,i]  ,%d  ,\"%s\"  ,\"%s\"  ,\"%s\")",
          lIndexNumMonths, szIndexBasis, szUndName, szIndexRefRateCode);
}

/* ------------------------------------------------------------------------------------------
 */
/* Cell in the second column (c[1  ,...]) of the knock in swap tableau : the
 * Call Spread for PV > 0 */
static void
GrfnKiSwapPositivePvCallSpreadCell(SrtBarrierType eKnockUpOrDown,
  ...
  if (eKnockUpOrDown == SRT_UP) {
    if (dCallSpreadWidth > 0.0) {
      ...
      sprintf(szPositivePvCallSpreadCell, "if ( c[0  ,i]> a[1  ,i]  , 1  , 0)");
    }
  } else {
    if (dCallSpreadWidth > 0.0) {
      sprintf(szPositivePvCallSpreadCell,
...
/* -----------------------------------------------------------------------------------------
 */
/* Cell in the third column (c[2  ,...]) of the knock in swap tableau : the Call
 * Spread for PV < 0 */
static void
...
/* -----------------------------------------------------------------------------------------
 */
/* Cell in the fourth column (c[3  ,...]) of the knock in swap tableau : the
 * Fixed Leg Value */
static void GrfnKiSwapFixedLegPvCell(SRT_Boolean bStubPaidAtStart,
  ...
  if (bStubPaidAtStart == SRT_NO)
    ...
    sprintf(szFixedLegPvCell, "PVRNG(6  ,7  ,a[2  ,i])-a[5  ,i]");
}

/* -----------------------------------------------------------------------------------------
 */
/* Cell in the fifth column (c[4  ,...]) of the knock in swap tableau : the
 * Float Leg Value */
...
/* -----------------------------------------------------------------------------------------
 */
/* Cell in the sixth column (c[5  ,...]) of the knock in swap tableau : the Full
 * Swap Value */
...
/* -----------------------------------------------------------------------------------------
 */
/* Cell in the seventh column (c[6  ,...]) of the knock in swap tableau : have
 * we knocked in ? */
...
/* -----------------------------------------------------------------------------------------
 */
/* First Cell in the seventh column (c[6  ,...]) of the knock in swap tableau :
 * va initialisation */
static void GrfnKiSwapKnockedInFirstRowCell(String szKnockedInFirstRowCell) {
  sprintf(szKnockedInFirstRowCell, "va:= if(c[5  ,i]>0  ,c[1  ,i]  ,c[2  ,i])");
}
/* -----------------------------------------------------------------------------------------
 */
/* Cell in the eight column (c[7  ,...]) of the knock in swap tableau : the Cash
 * Flow */

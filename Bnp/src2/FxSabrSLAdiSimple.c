    ...
    double* res_at_point)
{
    ...
    double *x = NULL, *z = NULL, ***values = NULL, ***values_p1 = NULL, ***values_temp = NULL,
           **mux = NULL, **muz = NULL, **varx = NULL, **varz = NULL, *varxinit = NULL,
           *sigBeta = NULL, **r = NULL;
    ...
    {
        std1 = a * sigma_beta *
               exp(alpha * sqrt((1.0 - exp(-2.0 * lambda * maturity)) / (2.0 * lambda))) *
               sqrt(maturity);
    }
    ...
    logmin = ((a * forward + b) * exp(-0.5 * std1 * std1 - floorstd * std1) - b) / a;
    logmax = ((a * forward + b) * exp(-0.5 * std1 * std1 + floorstd * std1) - b) / a;
    ...
    if (nb_strike > 1)
    {
        if (floorstd > 5)
        {
            disc_linleft_logright_center(
                x, nstepx, a * forward + b, a * flx + b, a * fux + b, std1, NSTD_FX, &index_x);
        }
        else
        {
            disc_SL_center(x, nstepx, forward, flx, fux, a, b, std1 / a, NSTD_FX, &index_x);
        }
    }
    else
    {
        if (floorstd > 5)
        {
            disc_linleft_logright_center_strike(
                x,
                nstepx,
                a * forward + b,
                a * flx + b,
        ...
        {
            disc_SL_strike(
                x, nstepx, forward, flx, fux, a, b, std1 / a, NSTD_FX, strike[0], 1, &index_x);
        }
    }
    ...
    */
    ...
    disc_linleft_linright_center(
        z, nstepz, 0.0, -NSTD_VOL * std3, NSTD_VOL * std3, std3, NSTD_VOL, &index_z);
    ...
    double*          res)
{
    ...
    {
        premium_tgt = srt_f_optblksch(forward, strike, tgt_vol, maturity, 1.0, SRT_CALL, PREMIUM);
    }
    else
    {
    ...
    /* use the SABR guess */
    sig0 = op_sabrSLcalib(forward, forward, maturity, tgt_vol, alpha, beta, rho, lambda);
    ...
    err =
        srt_f_optimpvol(premium, forward, strike, maturity, 1.0, SRT_CALL, input_vol_type, &bsvol);
    ...
    if (errorv >= precision || err)
    {
        if (err || newSig0 < 0.0 || fabs(alpha < 0.001))
        {
            ...
            {
                newSig0 = sig0 * sqrt(premium_tgt * premium_tgt / premium / premium * maturity);
            }
        }
    ...
    while ((k < nbIter) && (errorv >= precision))
    {
        ...
        err = srt_f_optimpvol(
            premium, forward, strike, maturity, 1.0, SRT_CALL, input_vol_type, &bsvol);
    ...
    double lambda)
{
    ...
    {
        res =
            sigma * log(F / K) * coefa / (log(coefa * F + coefb) - log(coefa * K + coefb)) * Z / XZ;
    }
    ...
    res *=
        (1.0 + ((2.0 * gam2 - gam1 * gam1 + 1.0 / Fb / Fb) * Fb * Fb / 24.0 +
                alpha * Fb / sigma / (coefa * Fb + coefb) *
                    (0.25 * rho * gam1 * Fb +
                     (2.0 - 3.0 * rho * rho) / 24.0 * alpha * Fb / sigma / (coefa * Fb + coefb))) *
                   sigma * sigma * (coefa * Fb + coefb) * (coefa * Fb + coefb) / Fb / Fb * T);
    ...
    double** res)
{
    ...
    for (j = 0; j < nb_strike; j++)
    {
        ...
        res[j][1] = sqrt((varOpt + res[j][2] * (res[j][2] * varZ - 2.0 * covar)) / npaths);
    }

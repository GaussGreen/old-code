...
double				logx, logmax, logmin;
	...
	const_expect1 = rho * alpha * a * sigma_beta2
					* lambda2 / mean;

	const_expect2 = rho * alpha * a * sigma_beta2
					* alpha2 / mean / mean;
	...
	{
		std1 = a * sigma_beta * exp(alpha * sqrt((1.0 - exp(-2.0 * lambda * maturity)) / (2.0 * lambda))) * sqrt(maturity);
	}
	...
	if (nb_strike > 1)
	{
		if (floorstd > 5)
		{
			disc_linleft_logright_center(x, nstepx, a * forward + b, a * flx + b, a * fux + b, std1, NSTD_FX, &index_x);
		}
		else
		{
	...
	else
	{
		if (floorstd > 5)
		{
			disc_linleft_logright_center_strike(x, nstepx, a * forward + b, a * flx + b, a * fux + b, std1, NSTD_FX, a * strike[0] + b, &index_x);
		}
		else
		{
			disc_SL_strike(x, nstepx, forward, flx, fux, a, b, std1 / a, NSTD_FX, strike[0], 1, &index_x);
		}
	}
	...
	*/
	...
	disc_linleft_linright_center(z, nstepz, 0.0, -NSTD_VOL * std3, NSTD_VOL * std3, std3, NSTD_VOL, &index_z);
...
FREE_RETURN:
	...
	/* Allocation 2 */
	if (values) free_f3tensor (values, 0, nstepx-1, 0, nstepz-1, 0, nb_strike-1);
	if (values_p1) free_f3tensor (values_p1, 0, nstepx-1, 0, nstepz-1, 0, nb_strike-1);
	...
	/* Allocation 3 */
	if (pde) num_f_pde_free_2d_adi (pde, nstepx, nstepz, nb_strike);
...
time_t	t1, t2;
	...
	{
		premium_tgt = srt_f_optblksch(	forward,
										strike,
										tgt_vol,
										maturity,
										1.0,
...
double	res;
	...
	{
		res = sigma * log (F / K) * coefa / (log(coefa * F + coefb) - log(coefa * K + coefb)) * Z / XZ;
	}

	res *= (1.0 + ((2.0 * gam2 - gam1 * gam1 + 1.0 / Fb / Fb) * Fb * Fb / 24.0
					+ alpha * Fb / sigma / (coefa * Fb + coefb) * (0.25 * rho * gam1 * Fb +

/* ===============================================================================
   ...
   FILENAME:      swp_f_bond_cox_compute.c
   
   PURPOSE:       Provide a few function for Bond Option Pricing that duplicates
                  Murex mains functions (CoxClean, CoxFwd, CoxFwdYield, CoxFwdClean
   ================================================================================ */
#include "math.h"
#include "swp_h_all.h"
...
double *acc_int,*s,*price;                  
	...
	for (i=0;i<=step;i++) 
	{
		...
		p1.start = (int) ti;
		acc_int[i] =  acc_int_fct(p1,coup)
					+ (ti-p1.start)*coup/365.0;
	...
	for (i=(int)step-1;i>=0;i--) 
	{
		...
		{ 
			for (j=0;j<=i;j++) 
			{
				s[j]=s[j]*d; 
	        	proba = (  r_repo
						   -(acc_int[i+1]- r_repo*acc_int[i] +r_swap*coupon/p.compd)/s[j]
						   - d)
						/(u-d);
				
				price[j] = (proba*price[j]+(1-proba)*price[j+1])
							*df_swap;
			}
		}
		...
		/* this formula is false: 
		      gamma = (d*price[0]-(u+d)*price[1]+d*price[2])/(spot*spot*(u*u-2*u*d+d*d));
...
SrtCurvePtr   crv;                  
	...
	df_repo =exp(log(swp_f_df((Ddate)(p.start),(Ddate)(opt_mat+dval),repo))/ step ); 
    
    crv = lookup_curve(swap);                          
	df_swap =exp(log(swp_f_df((Ddate)(p.start),(Ddate)(opt_mat+dval),swap))/ step );   
	...
	for (i=(int)step-1;i>=0;i--) 
	{
		...
		if (i==2)	
			gamma = (price[2]-price[1])/(s[2]-s[1])-(price[1]-price[0])/(s[1]-s[0]);
      	else	
		if (i==1) 
		{
...
int i,j,dval,cp;
double yield, su, sd;
double u,d,proba,pstart,pend,ti,u1,d1,mat1,r_repo,df_repo,r_swap,df_swap;
double coup,result;
double delta, gamma;
	...
	for (i=(int)step-1;i>=0;i--) 
	{
		...
		else
		{
			for (j=0;j<=i;j++) 
			{
	        	...
	        	proba = (r_repo
						   -(acc_int[i+1]- r_repo*acc_int[i])/s[j]
						   - d1)
						/(u1-d1);

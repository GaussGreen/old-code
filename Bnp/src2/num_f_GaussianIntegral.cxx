/* ======================================================
   FILENAME:  num_f_GaussIntegral.cxx

   PURPOSE:   Gaussian quadrature : integration (a >0 to b>0) or (a to infinity)
   or (b to infinity) with weight function exp(-x*x/2)/(2*Pi)^0.5   (NUMC p158)
   ====================================================== */
...
jacobi (§11.1) or tqli (§11.3)        , this routine sorts the eigenvalues into
...
or of a real        , symmetric matrix previously reduced by tred2 §11.2. On
...

{
  ...
  if (Is_start_Infinity == 1) {
    ...
    if (Is_end_Infinity == 1) {
      ...
      moments[2] =
          INV_SQRT_TWO_PI * (exp(-start * start * 0.5) - exp(-0.5 * end * end));
      rec1 = INV_SQRT_TWO_PI * exp(-0.5 * start * start);
      rec2 = INV_SQRT_TWO_PI * exp(-0.5 * end * end);
    }
  }
  ...
  for (i = 3; i <= 2 * n; i++)
    moments[i] = (i - 2) * moments[i - 2] + rec1 * pow(start, i - 2) -
                 rec2 * pow(end, i - 2);

  /*computation of the recurrence coefficients to construct the orthonomial
     polynomials (NUM_C p 159) with coefficients alpha and beta =0 */
  ...
  for (i = 3; i <= n + 1; i++) {
    ...
    a[i - 1] =
        0 + sig[i][i + 1] / sig[i][i] - sig[i - 1][i] / sig[i - 1][i - 1];
...
void lanczos(double start_point, double end_point, int n, int k, double *x,
             double *w, int weight_function)

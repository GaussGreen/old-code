...
static Err CopyGrid(SCheyBetaPDEGrid* g1, SCheyBetaPDEGrid* g2)
{
    ...
    if (g1->ad)
    {
        ...
        if (!g2->ad)
            return serror("Memory failure");
        memcpy(&(g2->ad[0][0]), &(g1->ad[0][0]), g2->nphi * g2->nx * sizeof(double));
    }
    if (g1->pv)
    {
        ...
        if (!g2->pv)
            return serror("Memory failure");
        memcpy(
            &(g2->pv[0][0][0]), &(g1->pv[0][0][0]), g2->nphi * g2->nx * g2->ninst * sizeof(double));
    }
    return NULL;
}

Err CheyBetaPDE_Init(
    SCheyBetaPDE* pde,
    SProductDesc* g,
    int           nt,
    int           nx,
    ...
    SCheyBeta*    pmdl)
{
    ...
    while (pde->times[ntimes - 1] > g->ex[g->nex - 1] + 1e-5)
        ntimes--;
    num_f_fill_vector_newalgo(&ntimes, &pde->times, (int)(nt * pde->times[ntimes - 1] + 1e-5));
    pde->nt = ntimes;
    ...
    if (!pde->fr || !pde->gamT || !pde->df || !pde->phi_max || !pde->phi_min || !pde->phi_mid ||
        !pde->cumxvar)
        return serror("Memory failure");
    ...
    for (i = 0; i < ntimes - 1; i++)
    {
        ...
        pde->gamT[i] =
            (1.0 - exp(-pmdl->lambda * (pde->times[ntimes - 1] - pde->times[i]))) / pmdl->lambda;
        d1 = d2;
    }
...
static void ExpandBounds(SCheyBetaPDE* pde, int start, int end)
{
    ...
    for (i = start; i < end; i++)
    {
        ...
        while (isig < pde->pmdl->nsig - 1 && pde->pmdl->sigtms[isig] < pde->times[i] + 1e-5)
            isig++;
        ...
        v_mid =
            CheyBeta_Vol(0.0, pde->phi_mid[i], sig, beta, pde->gamT[i], pde->fr[i], xcut_u, xcut_d);
...
static Err InitGridAt_i(SCheyBetaPDE* pde, int i)
{
    ...
    {
        itmp = 3 + (int)((pde->nphi - 3) * pde->times[i] / pde->times[pde->nt - 1] + 1e-8);
    ...
    if (pde->isfwd)
    {
        pde->grids[!pde->dir].ad = dmatrix(0, pde->grids[!pde->dir].nphi - 1, 0, pde->nx - 1);
        if (!pde->grids[!pde->dir].ad)
            return serror("Memory failure");

        memset(
            &(pde->grids[!pde->dir].ad[0][0]),
            0,
    ...
    for (i = pde->last; i < new_last; i++)
    {
        ...
        else
        {
            ...
            for (k = 0; k < pde->grids[pde->dir].nphi; k++)
            {
                ...
                {
                    std = CheyBeta_Vol(
                        pde->x[xi], phi_k, sig, beta, pde->gamT[i], pde->fr[i], xcut_u, xcut_d);
                    var[xi] = std * std * dt;
                }

                // Convolve slice into the ad vector using Crank-Nicholson
                num_f_pde_one_step_forward(
                    pde->tmppde, pde->nx, pde->x, pde->grids[pde->dir].ad[k], mu, var, r, 0.55, ad);
                ...
                for (xi = 0; xi < pde->nx; xi++)
                {
                    ...
                    else if (phi_nxt >= pde->grids[!pde->dir].phi[pde->grids[!pde->dir].nphi - 1])
                        pde->grids[!pde->dir].ad[pde->grids[!pde->dir].nphi - 1][xi] += ad[xi];
...
static Err PayoffAt_i(SCheyBetaPDE* pde, int idx, int i, double*** pv)
{
    ...
    for (k = 0; k < pde->g->nmat[0][idx]; k++)
    {
        ...
        gams[k] = (1.0 - exp(-pde->pmdl->lambda * (pde->g->mat[0][idx][k] - pde->times[i]))) /
                  pde->pmdl->lambda;
    }
...
FREE_RETURN:
    if (payoff)
        free_f3tensor(
            payoff, 0, pde->grids[pde->dir].nphi - 1, 0, pde->nx - 1, 0, pde->g->ninst - 1);
    return err;
}
            ...
            for (xi = 0; xi < pde->nx; xi++)
            {
                ...
                else  // find j: phi[j] <= phi_nxt < phi[j+1]
                {
                    ...
                    for (m = 0; m < pde->g->ninst; m++)
                        pvinst[xi][m] = coef * pde->grids[!pde->dir].pv[j][xi][m] +

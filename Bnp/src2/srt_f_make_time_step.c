...
static Err insert_eventdates(SrtStpPtr step, Date* eventdates, long numeventdates, double today)
{
    Err  err = NULL;
    long i;
...
static Err insert_vol_dates(SrtUndInfo* und_info, SrtStpPtr step, double today)
{
...
static Err insert_constrained_dates(
    SrtGrfnParam* grfnparam, SrtStpPtr step, double today, SrtUndInfo* und_info)
{
    Err        err = NULL;
    long       i;
    ...
    if ((!und_info->jumping || grfnparam->imp_type != IMPMONTECARLO) &&
        (und_info->no_of_underlyings != 1) && ((grfnparam->insert_vol_dates == SRT_YES)))
...
static int insert_extra_pts_in_future(
    SrtGrfnParam* grfnparam, SrtStpPtr step, double today, TermStruct* ts)
{

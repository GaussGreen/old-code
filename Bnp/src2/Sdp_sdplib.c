...
/*  Computes the primal constraints operator (Tr(A1.X), ...,Tr(Ai.X), ....) operating on a matrix
 * mata. */
...
Err op_a_augm(
    double*** con_mat, int num_const, double** cmat, double** mata, int dim, double* vecres)
{
    int k;
    for (k = 1; k <= num_const; k++)
        vecres[k] = scal_mat(con_mat[k], mata, dim);
...
Err op_ta_augm(
    double*** con_mat, int num_const, double** cmat, double* veca, int dim, double** matres)
{
    int i, j, k;
    ...
    int      diag_switch)
{
    ...
    for (k = 1; k <= dim; k++)
        ...
        {
            if ((0.5 * (bufvec1[k] * bufvec2[l] + bufvec1[l] * bufvec2[k])) != 0)
                bufmat4[k][l] /= (0.5 * (bufvec1[k] * bufvec2[l] + bufvec1[l] * bufvec2[k]));
            else
...
/*  For direct method: */
            ...
            errbuf = fast_oper_inv_E(
                pmat, zmat, bufmat, dim, bufmat1, &errval, num_improve, bufmat2, bufvec2, (i - 1));
            if (errval > errmax)
                errmax = errval;
            if (err == NULL)
                err = errbuf;
...
Err left_matrix(
    double** matm, int num_const, double* bvec, double kappa, double tau, double** result)
{
    int i, j;
    for (i = 1; i <= (num_const); i++)
    ...
    double**  startz)
{
    ...
    for (k = 1; k <= num_const; k++)
    {
        if ((1 + fabs(const_val[k])) / (1 + sqrt(scal_mat(const_mat[k], const_mat[k], dim))) > bufb)
            bufb = (1 + fabs(const_val[k])) / (1 + sqrt(scal_mat(const_mat[k], const_mat[k], dim)));
    }
    xhi = dim * bufb;
    ...
    char*     step_type)
{
    ...
    infa = compute_infeas(
        const_mat, const_val, num_const, xmat, zmat, cmat, dim, yvec, tau, &bufa, &bufb);
    while (test_pos != 0)
    {
        boucl++;
    ...
    if ((compute_mu(matbuf1, matbuf2, dim, newtau, newkappa) / (newtau * newtau)) > buf)
        test_step = 0;
    ...
    double    num_error)
{
    ...
    bufphi = compute_infeas(
        const_mat, const_val, num_const, xmat, zmat, cmat, dim, yvec, tau, &pinf, &dinf);
    ...
    status_tracker[iter][11] =
        d_max(-12, -d_max(0, (1 / log(10)) * log(fabs(condition_mat(systmat, num_const + 1)))));
    /*  Output */
    if (printlevel == 3)
    {
    ...
    else if (printlevel >= 4)
    {
        sprintf(
            buffer,
            "%d  Mu: %.2e  Infeas: %.2e  Hmgen: %.2e  Centr: %.2e \n",
            iter,
            buferr,
            bufphi,
            tau,

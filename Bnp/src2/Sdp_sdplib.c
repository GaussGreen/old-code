...
/*  or the linear algebra fucntions provided for use in griffin can be used instead. */
...
{
	int i,j;
	for (i=1;i<=dim;i++) for (j=1;j<=dim;j++) matres[i][j]=mata[i][j]; 
	return NULL;
}
...
Err zero_mat(double **mata, int dim)
{
	...
	for (i=1;i<=dim;i++) for (j=1;j<=dim;j++) mata[i][j]=0;
	return NULL;
}
...
Err zero_vec(double *veca, int dim)
{
	...
	for (i=1;i<=dim;i++) veca[i]=0;
	return NULL;
}

/*  Stack a matrix of dimension dim into a [1,...,(dim*dim)] vector. */
Err stack_mat(double **mata, int dim, double *vecres)
{
	int i,j;
	for (i=1;i<=(dim);i++) for (j=1;j<=(dim);j++) vecres[(j-1)*dim + (i-1)+1]=mata[i][j];
	return NULL;
}

/*  Adds aplha*A+beta*B put it into C (if transpose="T", B is transposed) */
Err add_mat(double alpha, double**mata, double beta, double **matb, int dim, double **matc, char *transpose)
{
	int i,j;
	if (strncmp(transpose,"T",1)==0)
	{
		for (i=1;i<=(dim);i++) for (j=1;j<=(dim);j++) matc[i][j]=alpha*mata[i][j]+beta*matb[j][i];
	}
	else
	{
		for (i=1;i<=(dim);i++) for (j=1;j<=(dim);j++) matc[i][j]=alpha*mata[i][j]+beta*matb[i][j];
	}
	return NULL;
}
...
Err mat_mult_nobuf(double **mata, double **matb, int dim, double **result)
{
	...
	{
		buf=0;
		for (k=1;k<=dim;k++) buf+=mata[k][i]*matb[j][k];
		result[j][i]=buf;
	}
	return NULL;
}
...
Err mat_mult_buf(double **mata, double **matb, int dim, double **result)
{
	...
	{
		buf=0;
		for (k=1;k<=dim;k++) buf+=mata[i][k]*matb[k][j];
		matbuf1[i][j]=buf;
	}
...
Err mat_mult_buf(double **mata, double **matb, int dim, double **result)
{
	...
	for (i=1;i<=(dim);i++) for (j=1;j<=(dim);j++) tempmat1[(j-1)*dim+(i-1)]=mata[i][j];
	for (i=1;i<=(dim);i++) for (j=1;j<=(dim);j++) tempmat2[(j-1)*dim+(i-1)]=matb[i][j];
	cblas_dgemm(CblasRowMajor,CblasNoTrans,CblasNoTrans,dim,dim,dim,1.0,tempmat1,dim,tempmat2,dim,0.0,tempmat3,dim);
	for (i=1;i<=(dim);i++) for (j=1;j<=(dim);j++) result[i][j]=tempmat3[(j-1)*dim+(i-1)];
	/*  free */
...
{
	int i,j;
	for (i=1;i<=dim;i++) for (j=1;j<=dim;j++) result[i][j]=mata[j][i];
	return NULL;
}


/*  Fast image of a standard [1..n][1..n] matrix by a big operator [1..n^2][1..n^2] on matrixes. */
...
Err fast_imat(double **opera, double **mata, int dim, double **result)
{
	...
	for (i=1;i<=(dim*dim);i++) for (j=1;j<=(dim*dim);j++) opbuf[(j-1)*dim*dim+(i-1)]=opera[i][j];
	for (i=1;i<=(dim);i++) for (j=1;j<=(dim);j++) matbuf[(j-1)*dim+(i-1)]=mata[i][j];
	/*  Computes the image. */
	cblas_dgemv(CblasRowMajor,CblasNoTrans,dim*dim,dim*dim,1.0,opbuf,dim*dim,matbuf,1,0.0,resbuf,1);
	/*  Put the resbuf in the standard form result matrix. */
	for (i=1;i<=(dim);i++) for (j=1;j<=(dim);j++) result[i][j]=resbuf[(j-1)*dim+(i-1)];
...
/*  Defines the A(*)B tensor product of matrixes of size dim (A, B, must be symetric). */
...
Err tensor_oper(double **mata, double **matb, double **mato, int dim, double **result)
{
	double **tempmat1=dmatrix(1,dim,1,dim);
	double **tempmat2=dmatrix(1,dim,1,dim);
	double **tempmat3=dmatrix(1,dim,1,dim);
...
double scal_mat_nosym(double **mata, double **matb, int dim)
{
	double buf=0;
	int i,j;
	for (i=1;i<=(dim);i++) for (j=1;j<=(dim);j++) buf+=mata[i][j]*matb[i][j];
	return buf;
}
...
double scal_mat(double **mata, double **matb, int dim)
{
	double buf=0;
	int i,j;
	for (i=1;i<=(dim);i++) for (j=i+1;j<=(dim);j++) buf+=mata[i][j]*matb[i][j];
	buf*=2;
	for (i=1;i<=(dim);i++) buf+=mata[i][i]*matb[i][i];
	return buf;
}
...
double scal_vec(double *veca, double *vecb, int dim)
{
	...
	for (i=1;i<=dim;i++) buf+=veca[i]*vecb[i];
	return buf;
}


/*  Adds two scaled vectors alpha*VECA + beta*VECB = result */
Err add_vec(double alpha, double *veca, double beta, double  *vecb, int dim, double *vecres)
{
	int i;
	for (i=1;i<=dim;i++) vecres[i]=alpha*veca[i]+beta*vecb[i];
	return NULL;
}
...
Err slow_im_vec(double **mata, double *veca, int dim, double *vecres)
{
	...
	{
		buf=0;
		for (j=1;j<=dim;j++) buf+=mata[i][j]*(veca[j]);
		vecres[i]=buf;
	}
	return NULL;
}
...
int pd_test(double **mata, int dim)
{
	...
	for (i=1;i<=dim;i++) 
	{
		...
		{
			sum=bufm[i][j];
			for (k=i-1;k>=1;k--) sum-=bufm[i][k]*bufm[j][k];
			if (i == j) 
			{
				if (sum < 0.0 && ret==0) 
				{
				...
				else pbuf[i]=sqrt(sum);
			} 
			else bufm[j][i]=sum/pbuf[i];
		}
	}
	/*  free */
...
/*  Solves a linear system A.x=b, returns inv(A) and x. if algo_type is 0, x otherwise. */
/*  Algo_type gives the choice between 0:Gauss elimination, 1:Gauss Siedel. */
Err lsolve(double **mata, double *vecb, int dim, double *vecres, double **matres, int algo_type, double prec_obj, int num_improve)
{
	Err err=NULL;
	int i,j;
	...
	if (algo_type==0)
	{
		...
		for (i=1;i<=(dim);i++) matbuf[i][1]=vecb[i];
 		err=gaussj(matres,dim,matbuf,1);
		for (i=1;i<=(dim);i++) vecres[i]=matbuf[i][1];
		/*  Start improvement */
		bufrb=2;
		for (i=1;i<=4*num_improve;i++)
		{
			slow_im_vec(mata,vecres,dim,vecbuf);
			for (j=1;j<=dim;j++) vecbuf1[j]=(vecb[j]-vecbuf[j]);
		if (i==1) 
			{
			...
			for (j=1;j<=dim;j++) vecres[j]+=vecbuf[j];
		}
		if (sqrt(scal_vec(vecbuf1,vecbuf1,dim))>prec_obj) err=serror("Not enough precision in lsolve...");
	}
	else
	{
		err=serror("He He He...");
...
Err min_eigenval(double **mata, int dim, double *result, double *condition)
{
	...
	for (i=1;i<=(dim);i++) for (j=1;j<=(dim);j++) matbuf[i-1][j-1]=mata[i][j];
	err=diagonalise_symmetric_matrix(matbuf,dim,evals,evects);
	bufmin=evals[0];bufmax=evals[0];
	for (i=1;i<(dim-1);i++) if (bufmin>evals[i]) bufmin=evals[i];
	for (i=1;i<=dim;i++) if (bufmax<evals[i]) bufmax=evals[i];
	/*  free */
	free_dvector(evals,0,dim-1);
	free_dmatrix(evects,0,dim-1,0,dim-1);
	free_dmatrix(matbuf,0,dim-1,0,dim-1);
	*result=bufmin;
	if (bufmin!=0) *condition=bufmax/bufmin; else *condition=1e+308;
	return err;
}
...
Err min_eigenval_nosym(double **mata, int dim, double *result, int max1_min0)
{
	...
	for (i=1;i<=dim;i++) for (j=1;j<=dim;j++) matbuf[i][j]=mata[i][j];
	balanc(matbuf,dim);
	elmhes(matbuf,dim);
	...
	if (max1_min0==0)
	{
	for (i=1;i<=dim;i++) if (buf>evals[i]) buf=evals[i];
	}
	else
	{
	for (i=1;i<=dim;i++) if (buf<evals[i]) buf=evals[i];
...
Err diago(double **mata, int dim, double **eigenvec, double *eigenval)
{
	...
	for (i=1;i<=(dim);i++) for (j=1;j<=(dim);j++) matbuf[i-1][j-1]=mata[i][j];
	err=diagonalise_symmetric_matrix(matbuf,dim,evals,evects);
	for (i=1;i<=(dim);i++) eigenval[i]=evals[i-1];
	for (i=1;i<=(dim);i++) for (j=1;j<=(dim);j++) eigenvec[i][j]=evects[j-1][i-1];
	/*  free */
	free_dvector(evals,0,dim-1);
	free_dmatrix(evects,0,dim-1,0,dim-1);
	free_dmatrix(matbuf,0,dim-1,0,dim-1);
...
double condition_mat(double **mata, int dim)
{
	...
	if ((err==NULL) && (res*res!=1e+308)) return res; else return 1.0e+308;
}
...
Err fudge_eigenvals(double *evals, int dim)
{
	...
	bufmax=evals[1];
	for (i=2;i<=dim;i++) if (evals[i]>bufmax) bufmax=evals[i];
	for (i=1;i<=dim;i++) evals[i]+=5.0*bufmax*machine_epsilon;
...
/*  Computes the primal constraints operator (Tr(A1.X), ...,Tr(Ai.X), ....) operating on a matrix mata. */
Err op_a(double ***con_mat, int num_const, double **mata, int dim, double *vecres)
{
	int k;
	for (k=1;k<=num_const;k++) vecres[k]=scal_mat(con_mat[k],mata,dim);
	return NULL;
}

/*  Computes the augmented constraints operator (A,-C). */
/*  Returns a num_const+1 vector. */
Err op_a_augm(double ***con_mat, int num_const, double **cmat, double **mata, int dim, double *vecres)
{
	int k;
	for (k=1;k<=num_const;k++) vecres[k]=scal_mat(con_mat[k],mata,dim);
...
/*  Computes the transposition of the constraints operator operating on a vector veca. */
Err op_ta(double ***con_mat, int num_const, double *veca, int dim, double **matres)
{
	int i,j,k;
...
/*  Computes the transposition of the augmented constraints operator operating on a vector veca. */
Err op_ta_augm(double ***con_mat, int num_const, double **cmat, double *veca, int dim, double **matres)
{
	int i,j,k;
...
double step_length(double **mata, double **dmata, int dim, double tau, double dtau, double  kappa, double dkappa, double gamma)
{
	double res,buf=0;
	double **matbuf1=dmatrix(1,dim,1,dim);
	...
	if (err==NULL)
		...
		if((err==NULL) && (buf<0))
			{
			if ((-gamma/buf)<res) res=(-gamma/buf);
			}
		}

	if (tau*dtau<0) 
		{
		if (((-gamma*tau)/(dtau))<res) res=(-gamma*tau)/(dtau);
		}

	if (kappa*dkappa<0) 
		{
		if (((-gamma*kappa)/(dkappa))<res) res=(-gamma*kappa)/(dkappa);
		}
...
double compute_infeas(double ***const_mat, double *const_val, int num_const, double **xmat, double **zmat, double **cmat, int dim, double *yvec, double tau, double *prim, double *dual)
{
...
/*  This computes sigma */
double compute_sigma(double **xmat, double **dxmat, double **zmat, double **dzmat, int dim, double alpha, double beta, double tau, double dtau, double kappa, double dkappa, double expon)
{
	double **matbuf1=dmatrix(1,dim,1,dim);
	double **matbuf2=dmatrix(1,dim,1,dim);
	double buf=0,bufb;
...
/*  Computes mu. */
double compute_mu(double **xmat, double **zmat, int dim, double tau, double kappa)
{
...
/*  P and Z are supposed to be commuting symmetric. (It is the case for AHO and HKM). */
Err fast_compute_inv_E(double **pmat, double **zmat, int dim, double **big_inv_E)
{
	...
	/*  (for AHO, in general we would have to find a simultanous diag basis).  */
	errbuf=diago(pmat,dim,bufmat1,bufvec1);
	if (err==NULL) err=errbuf;
	...
	for (i=1;i<=dim;i++) for (j=1;j<=dim;j++)
	{
		...
		{
			if ((0.5*(bufvec1[k]*bufvec2[l]+bufvec1[l]*bufvec2[k]))!=0) bufmat4[k][l]/=(0.5*(bufvec1[k]*bufvec2[l]+bufvec1[l]*bufvec2[k]));
			else err=serror("Singular system matrix building");
		}
...
/*  This computes the inv(E) operator image of a matrix with the fast Kronecker product algo.  */
/*  P and Z are supposed to be commuting symmetric. (It is the case for AHO and HKM). */
Err fast_oper_inv_E(double **pmat, double **zmat, double **mata, int dim, double **result, double *error_ratio, int num_improve, double **bufmat2, double *bufvec2, int diag_switch)
{
	...
	{
		if ((0.5*(bufvec1[k]*bufvec2[l]+bufvec1[l]*bufvec2[k]))!=0) bufmat4[k][l]/=(0.5*(bufvec1[k]*bufvec2[l]+bufvec1[l]*bufvec2[k]));
		else err=serror("Singular system matrix operation");
	}
	mat_mult_nobuf(bufmat2,bufmat4,dim,bufmat1);
	...
	for (m=1;m<=num_improve;m++)
	{
		...
		if (m==1)
		{
			...
			if (buf!=0) error_improvement=1/buf; else break;
		}
		/*  Breaks if improved enough... */
		buf=sqrt(scal_mat(bufmat5,bufmat5,dim));
		if (error_improvement*buf<max_prec_improve) break;
		/*  ------------------------------- */
	...
	*error_ratio=error_improvement;
	...
	else *error_ratio=error_improvement=1;
	/*  Free. */
	free_dmatrix(bufmat1,1,dim,1,dim);
	free_dmatrix(bufmat3,1,dim,1,dim);
	free_dmatrix(bufmat4,1,dim,1,dim);
...
Err system_matrix(double ***const_mat, int num_const, double **xmat, double **zmat, double **cmat, int dim, char *srch, double **result, double **big_inv_E, double *errtrack, int num_improve)
{
	...
	if (err==NULL)
	{
		for (i=1;i<=(num_const+1);i++) 
		{
			...
			errbuf=fast_oper_inv_E(pmat,zmat,bufmat,dim,bufmat1,&errval,num_improve,bufmat2,bufvec2,(i-1));
			if (errval>errmax) errmax=errval;
			if (err==NULL) err=errbuf;
			#endif
...
Err left_matrix(double **matm, int num_const, double *bvec, double kappa, double tau, double **result)
{
	int i,j;
	for (i=1;i<=(num_const);i++) for (j=1;j<=(num_const);j++) result[i][j]=matm[i][j];
	for (i=1;i<=(num_const);i++) result[i][num_const+1]=matm[i][num_const+1]-bvec[i];
	for (i=1;i<=(num_const);i++) result[num_const+1][i]=matm[num_const+1][i]+bvec[i];
	result[num_const+1][num_const+1]=matm[num_const+1][num_const+1]+kappa/tau;
	return NULL;
}


/*  Computes the Rd matrix. */
Err Rd_mat(double ***const_mat, int num_const, double **zmat, double **cmat, int dim, double *yvec, double tau, double **result)
{
	double *vecbuf=dvector(1,num_const+1);
	double **bufmat=dmatrix(1,dim,1,dim);
	int i;

	for (i=1;i<=(num_const);i++) vecbuf[i]=yvec[i];
...
Err rp_vec(double ***const_mat, int num_const, double **xmat, double **cmat, int dim, double *yvec, double *bvec, double tau, double kappa, double *vecres)
{
	double buf=0;
	int i;

	op_a_augm(const_mat,num_const,cmat,xmat,dim,vecres);
	for (i=1;i<=(num_const+1);i++) vecres[i]=-vecres[i];
	for (i=1;i<=(num_const);i++) vecres[i]+=bvec[i]*tau;
	for (i=1;i<=(num_const);i++) buf+=bvec[i]*yvec[i];
...
Err Rc_mat(double **xmat, double **zmat, int dim, double sigma, double mu, char *srch_type, double **result)
{
	double **idbuf=dmatrix(1,dim,1,dim);
	double **matbuf=dmatrix(1,dim,1,dim);
	int i,j;

	for (i=1;i<=dim;i++) idbuf[i][i]=1.0;
	mat_mult_buf(xmat,zmat,dim,matbuf);
	/*  (ToDo) implement the HKM search direction */
	for (i=1;i<=dim;i++) for (j=1;j<=dim;j++) result[i][j]=0.5*(matbuf[i][j]+matbuf[j][i]);
...
Err Rq_mat(double **xmat, double **dxmat, double **zmat, double **dzmat, int dim, double sigma, double mu, char *srch, double **result)
{
	double **idbuf=dmatrix(1,dim,1,dim);
	double **matbuf=dmatrix(1,dim,1,dim);
	double **matbuf2=dmatrix(1,dim,1,dim);
...
Err right_vector(double ***const_mat, int num_const, double **big_inv_E, double **xmat, double **zmat, double **cmat, double **dpxmat, double **dpzmat, char *step_type, int dim, double *yvec, double *bvec, double eta, double tau, double sigma, double mu, double kappa, double dptau, double dpkappa, double *vecres, int num_improve)
{
	double **matbuf=dmatrix(1,dim,1,dim);
	double **matbuf2=dmatrix(1,dim,1,dim);
	...
	for (i=1;i<=(num_const+1);i++) vecres[i]*=eta;
	...
	fast_oper_inv_E(idbuf,zmat,matbuf,dim,matbuf4,&errval,num_improve,matbuf8,vecbuf2,0);
	#endif
	op_a_augm(const_mat,num_const,cmat,matbuf4,dim,vecbuf);
	for (i=1;i<=(num_const+1);i++) vecres[i]+=vecbuf[i];
	/*  free */
	free_dmatrix(matbuf,1,dim,1,dim);
	free_dmatrix(matbuf2,1,dim,1,dim);
...
/*  Computes dZ. */
Err delta_z(double ***const_mat, int num_const, double **zmat, double **cmat, int dim, double *yvec, double *dyvec, double tau, double eta, double dtau, double **result)
{
	double *vecbuf=dvector(1,num_const+1);
	...
	for (i=1;i<=(num_const);i++) vecbuf[i]=dyvec[i];
...
/*  Computes dX */
Err delta_x(double **big_inv_E, double **xmat, double **dpxmat, double **zmat, double **dpzmat, double **dczmat, int dim, double sigma, double mu, char *step_type, double **result, int num_improve)
{
	double **matbuf=dmatrix(1,dim,1,dim);
	double **matbuf2=dmatrix(1,dim,1,dim);
	double **matbuf8=dmatrix(1,dim,1,dim);
	...
	#else
	/*  For "on demand" method. for AHO method. */
	fast_oper_inv_E(idbuf,zmat,matbuf2,dim,result,&errval,num_improve,matbuf8,vecbuf2,0);
	#endif
	/*  Result should be symmetric but  */
	/*  symmetrize it anyway to improve precision. (CPU cost is zero) */
	add_mat(0.5,result,0.5,result,dim,result,"T");
...
Err init_sol(double ***const_mat, int num_const, double *const_val, double **cmat, int dim, double **startx, double **startz)
{
	double **idbuf=dmatrix(1,dim,1,dim);
	...
	for (k=1;k<=num_const;k++)
	{ 
		if (sqrt(scal_mat(const_mat[k],const_mat[k],dim))>bufa) bufa=sqrt(scal_mat(const_mat[k],const_mat[k],dim));
	}
	if (sqrt(scal_mat(cmat,cmat,dim))>bufa) bufa=sqrt(scal_mat(cmat,cmat,dim));
	eta=bufa/sqrt(dim);
	for (k=1;k<=num_const;k++)
	{ 
		if ((1+fabs(const_val[k]))/(1+sqrt(scal_mat(const_mat[k],const_mat[k],dim)))>bufb) bufb=(1+fabs(const_val[k]))/(1+sqrt(scal_mat(const_mat[k],const_mat[k],dim)));
	}
	xhi=dim*bufb;
	...
	/* add_mat(1.0,idbuf,0.0,startx,dim,startx,"No"); */
	/* add_mat(1.0,idbuf,0.0,startz,dim,startz,"No"); */
	/*  free */
	free_dmatrix(idbuf,1,dim,1,dim);
	return NULL;
}
...
double centrality(double **xmat, double **zmat, int dim, double tau, double kappa)
{
	double buf=0,bufb=0;
	double **matbuf=dmatrix(1,dim,1,dim);
	Err err=NULL;
	...
	if (err==NULL) buf=1-bufb*buf; else buf=0;
	/*  Free */
	free_dmatrix(matbuf,1,dim,1,dim);
	return buf;
}
...
Err set_steps(double ***const_mat,double *const_val, int num_const, double *yvec, double *dyvec, double **xmat, double **dxmat, double **zmat, double **dzmat, double **cmat, int dim, double kappa, double dkappa, double tau, double dtau, double eta, double *alpha, double *beta, double frac_search, char* step_type)
{
	double **matbuf1=dmatrix(1,dim,1,dim);
	double **matbuf2=dmatrix(1,dim,1,dim);
	double *vecbuf1=dvector(1,num_const);
	...
	infa=compute_infeas(const_mat,const_val,num_const,xmat,zmat,cmat,dim,yvec,tau,&bufa,&bufb);
	while (test_pos!=0)
	{
		boucl++;
		...
		if (taubuf1>taubuf2)
		{
			...
			add_mat(1.0,xmat,*alpha,dxmat,dim,matbuf1,"No");
			add_mat((taubuf1/taubuf2),zmat,(taubuf1/taubuf2)*(*beta),dzmat,dim,matbuf2,"No");
			add_vec((taubuf1/taubuf2),yvec,(taubuf1/taubuf2)*(*beta),dyvec,num_const,vecbuf1);
		}
		else 
		{
			newtau=taubuf2;
			newkappa=kappa+(*beta)*dkappa;
			add_mat((taubuf2/taubuf1),xmat,(taubuf2/taubuf1)*(*alpha),dxmat,dim,matbuf1,"No");
			add_mat(1.0,zmat,(*beta),dzmat,dim,matbuf2,"No");
			add_vec(1.0,yvec,(*beta),dyvec,num_const,vecbuf1);
		}
		/*  Test for positiveness. If not, retry with smaller, equal steps (Line search). */
		...
		if (boucl>max_reduc) test_pos=0;
	}
	/*  Test for decreasing total complementarity. */
	if((compute_mu(matbuf1,matbuf2,dim,newtau,newkappa)/(newtau*newtau))>buf) test_step=0;
	/*  Test for decreasing infesibility. */
	infb=compute_infeas(const_mat,const_val,num_const,matbuf1,matbuf2,cmat,dim,vecbuf1,newtau,&bufa,&bufb);
	if (infa<infb) test_step=0;
...
double global_error(double ***const_mat, double *const_val, int num_const, double **xmat, double **zmat, double **cmat, int dim, double *yvec, double tau, double kappa)
{
	double bufa,bufb,phib,mub,res;

	/*  Compute status. */
	phib=compute_infeas(const_mat,const_val,num_const,xmat,zmat,cmat,dim,yvec,tau,&bufa,&bufb);
	mub=(compute_mu(xmat,zmat,dim,tau,kappa)*(dim+1)-kappa*tau)/(tau*tau);
	res=mub;
	bufa*=(1+sqrt(scal_vec(const_val,const_val,num_const)));
	bufb*=(1+sqrt(scal_mat(cmat,cmat,dim)));
	if (bufa>mub) res=bufa;
	if (bufb>res) res=bufb;
	return res;

}

/*  Defines a step that only decrease primal infeasibility. */
Err take_feasibility_step(double ***const_mat, double *const_val, int num_const, double **xmat, double **dcxmat, double **zmat, double **dczmat, int dim, double *yvec, double *dcyvec, double *dctau, double tau, double *dckappa, double **inverse_gram)
{
	double *vecbuf1=dvector(1,num_const);
...
{
	int i,j;
	for (i=1;i<=dim1;i++) for (j=1;j<=dim2;j++) out_array[(j-1)*dim1+(i-1)]=in_array[i][j];
	return NULL;
}
...
Err status(int iter, double ***const_mat, double *const_val, int num_const, double **systmat, double **xmat, double **zmat, double **cmat, int dim, double *yvec, double tau, double kappa, double sigma, double eta, double alpha, double beta, double **status_tracker, int printlevel, int max_it, double num_error)
{
	char buffer[512];
	...
	buferr=(compute_mu(xmat,zmat,dim,tau,kappa)*(dim+1)-kappa*tau)/(tau*tau);
	bufphi=compute_infeas(const_mat,const_val,num_const,xmat,zmat,cmat,dim,yvec,tau,&pinf,&dinf);
	bufcent=centrality(xmat,zmat,dim,tau,kappa); 
	/*  Stores some scaling factors to plot everything in a homogenous way. */
	if (iter==1) 
	{
		for (k=1;k<=max_it;k++) for (l=1;l<=11;l++) status_tracker[k][l]=1e+308;
		if (buferr!=0) scaleerr=buferr; else scaleerr=1.0;
		if (bufphi!=0) scalephi=bufphi; else scalephi=1.0;
		if (tau!=0) scaletau=tau; else scaletau=1.0;
		if (pinf!=0) scalepinf=pinf; else scalepinf=1.0;
		if (dinf!=0) scaledinf=dinf; else scaledinf=1.0;
	}
	/*  Scale and store verything */
	status_tracker[iter][1]=d_max(-12,(1/log(10))*log(fabs(buferr/scaleerr)));
	status_tracker[iter][2]=d_max(-12,(1/log(10))*log(fabs(bufphi/scalephi)));
	status_tracker[iter][3]=d_max(-12,(1/log(10))*log(fabs(tau/scaletau)));
	...
	status_tracker[iter][9]=d_max(-12,(1/log(10))*log(fabs(pinf/scalepinf)));
	status_tracker[iter][10]=d_max(-12,(1/log(10))*log(fabs(dinf/scaledinf)));
	status_tracker[iter][11]=d_max(-12,-d_max(0,(1/log(10))*log(fabs(condition_mat(systmat,num_const+1)))));
	/*  Output */
	if (printlevel==3)
	{
...
void visu_vec(double *veca, int nrows, int monitorId)
{
	...
	long int monit;
	monit=(long int)(monitorId);
	for (i=1;i<=nrows;i++) flat_vec[i-1]=veca[i];

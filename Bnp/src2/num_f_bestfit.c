...
Err quadr_best_fit (double **x, double *y, long n, long dim, double *a, double *grad, double **hess,
					double *min)
{
	
	/* declaration and allocation */
	
	long	i, j, k, l, nrot, def,
			nvar = (dim + 1) * (dim + 2) / 2,
			*indx;
	
	double	**X = dmatrix (1, n, 1, nvar),
			**Y = dmatrix (1, n, 1, 1),
			**transX = NULL,
			**transXX = NULL,
			**inv_transXX = NULL,
			**transXY = NULL,
			**inv_transXX_transXY = NULL,
			**inv_hess = NULL,
			**grad_matrix = dmatrix (1, dim, 1, 1),
			**inv_hess_grad = NULL,
			*eigen_val = dvector (1, dim),
			**eigen_vec = dmatrix (1, dim, 1, dim);

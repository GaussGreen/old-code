...
Err quadr_best_fit(double **x, double *y, long n, long dim, double *a,
  ...
  /*	transform the problem into unidimensional one
          variables are 1  , x1  , ...  , xdim  , x1^2  , x2*x1  , x2^2  , x3*x1
     , x3*x2  , x3^2  , ...  , xdim^2 */
  ...
  /* interpret results in terms of a  , grad  , hess with y[i] = a + <grad
   * ,x[i]> + tx[i]*hess*x[i] */
  ...
  *a = inv_transXX_transXY[1][1];
  ...
  /* if hess is definite positive  , find minimum with formula min = - 0.5 * inv
   * (hess) * grad */

...
// ----------------------------------------------------------------------------------------------------------------
// ----------------------------------------------------------------------------------------------------------------
//                                 Calling routine for Callable Ratchet Inverse
//                                 Floater (CRIF)
// ----------------------------------------------------------------------------------------------------------------
// ----------------------------------------------------------------------------------------------------------------

Err CRIFAutocal(CMSCTS_MARKET sMarket, CRIF_DEAL sDeal,
                CMSCTS_CALIB sCalibration, CRIF_PRICING_PARAMS sPricingParams,
  ...
  /* Fill the structures */
  err = crif_fill_check_all_struct(sDeal, sMarket,
  ...
  if (sDeal->sExotic->sAux->iNbUsedNumCpn > 0) {
    ...
    err =
        crif_calibrate_model(sMarket, sDeal, sSimulArg, sCalibration, sOutputs);
    if (err)
      goto FREE_RETURN;

    time1 = clock();
    smessage("CRIF Calibration: %.2f",
             (double)(time1 - time2) / CLOCKS_PER_SEC);

    /* calculate the CV Exotic coupon value */
    err =
        crif_calc_cv_value(sMarket, sDeal, sSimulArg, sPricingParams, sOutputs);
    if (err)
      goto FREE_RETURN;

    /* Calculates the unadjusted option */
    err = crif_calculate_option(sMarket, sDeal, sSimulArg, sPricingParams,
                                sOutputs);
    if (err)
      goto FREE_RETURN;

    time2 = clock();
    smessage("CRIF Option Pricing: %.2f",
    ...
    if (sCallAux->iNbUsedCall > 0 && sFwdIVInfos) {
      ...
      while (sPayArg && !sPayArg->iIsCall) {
        ...
        if (iIndexEvt < sMCArg->iNbEvent) {
          sPayArg = (CRIF_PAYARG)(sMCArg->vPayoffParams
...
Err crif_allocate_fwdiv(CRIF_FWDIV sFwdIVInfos, long lNbCall) {
  ...
  if (!sFwdIVInfos->lExeDates || !sFwdIVInfos->dMarketIV ||
      !sFwdIVInfos->dModelIV || !sFwdIVInfos->dExeProba ||
      !sFwdIVInfos->dNewModelIV || !sFwdIVInfos->dFees ||

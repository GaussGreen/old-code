...
Err CRIFAutocal(	CMSCTS_MARKET			sMarket,
				CRIF_DEAL			sDeal,
				...
				CRIF_OUTPUTS		sOutputs)
{
	...
	/* Fill the structures */
	err = crif_fill_check_all_struct(	sDeal,
		sMarket,
	...
	if (sDeal->sExotic->sAux->iNbUsedNumCpn > 0)
	{
		...
		err = crif_calibrate_model(		sMarket,
										sDeal,
										sSimulArg,
										sCalibration,
										sOutputs);
		...
		err = crif_calc_cv_value(	sMarket,
									sDeal,
									sSimulArg,
									sPricingParams,
									sOutputs);
	if (err) goto FREE_RETURN;
		...
		err = crif_calculate_option(	sMarket,
										sDeal,
										sSimulArg,
										sPricingParams,
										sOutputs);
...
Err crif_fill_outputs(	CRIF_DEAL		sDeal,
	...
	if (sDeal->sExotic->sAux->iNbUsedNumCpn > 0)
	{
		...
		if (sCallAux->iNbUsedCall > 0 && sFwdIVInfos)
		{
			...
			for (i=0; i<sFwdIVInfos->lNbCall; i++)
			{
				...
				if (sMCArg->sMCEBParams->dExeProba) sFwdIVInfos->dExeProba [i] = sMCArg->sMCEBParams ->dExeProba[iIndexEvt];
						...
						long			lNbCall)
{
	...
	if (!sFwdIVInfos->lExeDates || !sFwdIVInfos->dMarketIV || !sFwdIVInfos->dModelIV  || !sFwdIVInfos->dExeProba || !sFwdIVInfos->dNewModelIV || !sFwdIVInfos->dFees  || 
...
void crif_free_fwdiv(	CRIF_FWDIV	sFwdIVInfos)
{
	...
	if (sFwdIVInfos->dExeProba	) free(sFwdIVInfos->dExeProba);

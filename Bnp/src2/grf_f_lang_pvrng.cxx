...
/******************************************************************************\
 ...
 pvrng(auxrng1        ,auxrng2        ,startdate).  As this is a financial
function        , we will end up with only one node. (The nodes after top will
be freed.) This assumes that auxrng1 contains dates        , auxrng2 contains
(known) cash flows; only the cashflows in auxrng2 corresponding to dates in
auxrng1 that are greater than startdate are included; pvrng is supposed to
calculate the pv of those cashflows; hence it is a sum of discount factors.  In
...
Err grfn_pvrange_setup(String name, GrfnDeal *gd, COMLL_PTR top, int index) {
  ...
  for (i = 0; i < gd->auxlen[auxrng1]; i++) {
    if (DTOL(gd->aux[auxrng1][i]) > startdate)
      break;
  }
  if (i < gd->auxlen[auxrng1])
    numdates = gd->auxlen[auxrng1] - i;
  else
    return serror("%s:nothing left to value", name);
  ...
  if (DTOL(gd->aux[auxrng1][gd->auxlen[auxrng1] - 1]) > 50000)
    return serror("%s:aux[%d][%d]:silly date %d", name, auxrng1,
                  gd->auxlen[auxrng1] - 1,
                  gd->aux[auxrng1][gd->auxlen[auxrng1] - 1]);
...
/******************************************************************************\
    ...
    DESCRIPTION     :   This function has not yet been tested.
                        For that reason        , it is not yet included in the
GRFN language. The reason one might want this is really to compute the CLEAN
price of a bond; for which we need an accrued interest function in the GRFN
function as well.

 Initializes top assuming that it is a call to
 drty(auxrng1        ,auxrng2        ,startdate        ,enddate).  As this is a
financial function , we will end up with only one node. (The nodes after top
will be freed.) This assumes that auxrng1 contains dates        , auxrng2
contains (known) cash flows; only the cashflows in auxrng2 corresponding to
dates in auxrng1 that are greater than startdate and <= enddate are included;
drty is supposed to calculate the pv of those cashflows (like the dirty price of
a bond); hence it is a sum of discount factors.  It would be calculated in
...
Err grfn_dirtyprice_setup(String name, GrfnDeal *gd, COMLL_PTR top, int index) {
  ...
  */
  ...
  for (j = i; j < gd->auxlen[auxrng1]; j++) {
    if (DTOL(gd->aux[auxrng1][j]) > enddate)
      break;
  }

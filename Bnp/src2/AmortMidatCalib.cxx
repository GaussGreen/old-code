    ...
    double strike[])  /*	Strikes */
{
  ...
  for (i = 0; i < nex; ++i) {
    price += lgmsopval1F(ncpn - ex_cpn[i], df + ex_cpn[i], cvg + ex_cpn[i],
                         cpn_G + ex_cpn[i], ex_zeta[i], ex_G[i], strike[i]);
  }
...
static double amort_static_lgmiv(double y,     /*	The y */
...
double amort_lgmopval2F(int ncpn,         /*	Number of cash-flows */
                        ...
                        double ex_G2)     /*	G2 at exercise date */
{
  ...
  amort_static_lgm2Fcalcexpfact(ncpn, cpn, cpn_G1, cpn_G2, ex_zeta1, sqz1,
                                ex_zeta2, sqz2, ex_zeta12, c1, c2, ex_G1, ex_G2,
                                cstfact, n1fact, n2fact, &highest);
  ...
  for (i = 0; i < NUM_HERMITE; i++) {
    ...
    for (j = 0; j < ncpn; j++) {
      ystar = amort_static_lgmystar2F(
          ncpn, cpn, cstfact, n1fact, n2fact, highest,
          X[i] - (highest == 1 ? n2fact[j] : n1fact[j]), &dir);
...
Err Get_LGM_TermStructureOneOrTwoFact(char *underlying, double **sigma_date,
                                      double **sigma, long *sigma_n,
                                      double **tau_date, double **tau,
                                      long *tau_n, double *alpha, double *gamma,
                                      double *rho)

{
  ...
  TermStruct *ts = NULL;
  SrtMdlDim mdl_dim;
  double *beta = NULL, *beta2 = NULL, *vovol = NULL, *rhots = NULL,
         *meanvol = NULL;
  double *sigma2 = NULL;
  double *rhovect = NULL;
  double *tau2 = NULL;
  ...
  if (mdl_dim == ONE_FAC) {
    ...
    *rho = 1.0;
    ...
    err = srt_f_display_IRM_TwoFac_TermStruct(
        ts, sigma_date, sigma, &beta, &sigma2, &beta2, &rhovect, sigma_n,
        tau_date, tau, &tau2, tau_n);
...
///*	Strike */
{
  ...
  return amort_lgmopval2F(ncpn, coupon, cpn_G1, cpn_G2, ex_zeta1, ex_zeta2,
                          ex_zeta12, ex_G1, ex_G2);
}

/*	Value of European Swap option within LGM 2F */
double amortMidat_lgmsopval2F(int ncpn,         /*	Number of cash-flow dates         ,
                                             including         start and end date */
                              double df[],      /*	Df to cash flow dates */
                              double cvg[],     /*	cvg from i-1 to i */
                              double cpn_G1[],  /*	G1 at cash-flow dates */
                              ...
                              double strike)    /*	Strike */
{
  ...
  return amort_lgmopval2F(ncpn, cpn, cpn_G1, cpn_G2, ex_zeta1, ex_zeta2,
                          ex_zeta12, ex_G1, ex_G2);
}
    ...
    double swaption_strike[]) /*	Standard Swaption Coupons */
{
  ...
  for (i = 0; i < nex; ++i) {
    price += amortMidat_lgmsopval2F(
        ncpn - ex_cpn[i], df + ex_cpn[i], cvg + ex_cpn[i], cpn_G1 + ex_cpn[i],
        cpn_G2 + ex_cpn[i], ex_zeta1[i], ex_zeta2[i], ex_zeta12[i], ex_G1[i],
        ex_G2[i], swaption_strike[i]);
  }
    ...
    double swaption_cpn[]) /*	Standard Swaption Coupons */
{
  ...
  for (i = 0; i < nex; ++i) {
    ...
    price += amort_lgmopval2F(ncpn - j, swaption_cpn + j, cpn_G1 + j,
                              cpn_G2 + j, ex_zeta1[i], ex_zeta2[i],
                              ex_zeta12[i], ex_G1[i], ex_G2[i]);
...
double amortMidat_lgmcapval1F(int ncpn,        /*	Number of cash-flow dates        ,
                                            including        start and end date */
                              double cpn[],    /*	Notional */
                              double df[],     /*	Df to cash flow dates */
                              double cvg[],    /*	cvg from i-1 to i */
...

{
  ...
  for (i = 0; i < nex; i++) {
    ...
    val += amortMidat_lgmamortsopval1F(
        ex_ncpn[i], &(cpn[ex_cpn[i]]), &(df[ex_cpn[i]]), &(cvg[ex_cpn[i]]),
        &(cpn_G1[ex_cpn[i]]), ex_zeta1[i], ex_G1[i]);
...

{
  ...
  for (i = 0; i < nex; i++) {
    ...
    coupon[ex_cpn[i] + ex_ncpn[i] - 1] =
        floatcoupon[j - 1] + df[ex_cpn[i] + ex_ncpn[i] - 1] +
        ex_sstrike[i] * df[ex_cpn[i] + ex_ncpn[i] - 1];

    val += amortMidat_lgmamortsopval1F(
        ex_ncpn[i], &(coupon[ex_cpn[i]]), &(df[ex_cpn[i]]), &(cvg[ex_cpn[i]]),
        &(cpn_G1[ex_cpn[i]]), ex_zeta1[i], ex_G1[i]);
  }
...

{
  ...
  for (i = 0; i < nex; i++) {
    ...
    val += amortMidat_lgmamortsopval2F(
        ex_ncpn[i], &(coupon[ex_cpn[i]]), &(df[ex_cpn[i]]), &(cvg[ex_cpn[i]]),
        &(cpn_G1[ex_cpn[i]]), &(cpn_G2[ex_cpn[i]]), ex_zeta1[i], ex_zeta2[i],
        ex_zeta12[i], ex_G1[i], ex_G2[i]);
  }
                              ...
                              double ex_G2[])     /*	G2 at exercise date */
{
  ...
  for (i = 0; i < nex; i++) {
    ...
    val += amortMidat_lgmamortsopval2F(
        ex_ncpn[i], &(cpn[ex_cpn[i]]), &(df[ex_cpn[i]]), &(cvg[ex_cpn[i]]),
        &(cpn_G1[ex_cpn[i]]), &(cpn_G1[ex_cpn[i]]), ex_zeta1[i], ex_zeta2[i],
        ex_zeta12[i], ex_G1[i], ex_G2[i]);
...
Err computeZeta(double exer_time, double lambda, double *sigma_date,
                double *sigma, int sigma_n, double *exer_zeta) {
  Err err = NULL;
  int i;
  double zeta;

  if (exer_time <= sigma_date[0]) {
    zeta = sigma[0] * sigma[0] * (exp(2 * lambda * exer_time) - 1.0) /
           (2 * lambda);
  } else {
    zeta = sigma[0] * sigma[0] * (exp(2 * lambda * sigma_date[0]) - 1.0) /
           (2 * lambda);

    i = 1;
    while ((exer_time > sigma_date[i]) && (i < sigma_n)) {
      zeta += sigma[i] * sigma[i] *
              (exp(2 * lambda * sigma_date[i]) -
               exp(2 * lambda * sigma_date[i - 1])) /
              (2 * lambda);
      ++i;
    }
    ...
    if (i < sigma_n) {
      ...
      zeta += sigma[sigma_n - 1] * sigma[sigma_n - 1] *
              (exp(2 * lambda * exer_time) -
               exp(2 * lambda * sigma_date[sigma_n - 1])) /
              (2 * lambda);
    }
  }
  *exer_zeta = zeta;
...
Err computeZeta_ts(double exer_time,

                   double *lambda_time, double *lambda, int lambda_n,
                   double gamma,

                   double *sigma_date, double *sigma, int sigma_n,
  ...
  export_lgmcalczeta1_tauts(sigma_n, sigma_date, sigma, lambda_n, lambda_time,
                            lambda, 0.0, gamma, 0.0, 1, &exer_time, exer_zeta);
...
Err europSwaption_clsdfrm(char *yc_name, SrtUndPtr und, int notperiod,
                          long lStartDate, long lEndDate, char *fixFreq,
                          char *fixBasis, char *refRateName, double exer_fee,
                          double strike, double margin, char *recPayStr,
                          double *price) {
  Err err = NULL;
  Date lToday;
  String und_name;
  ...
  if (mdl_type == LGM || mdl_type == NEWLGM) {
    ...
    err = swp_f_make_FloatLegDatesCoveragesAndSpreads(
        &swapdp, lToday, refRateName, &float_pay_dates, &float_nb_pay_dates,
    ...
    err = swp_f_make_FixedLegDatesAndCoverages(
        &swapdp, lToday, &fix_pay_dates, &fix_nb_pay_dates, &fix_start_dates,
    ...
    fix_index = 1;
    ivalue =
        -rec * (1 + exer_fee) * swp_f_df(lToday, float_pay_dates[0], yc_name);
    coupon[0] =
        -rec * (1 + exer_fee) * swp_f_df(lToday, float_pay_dates[0], yc_name);
    ...
    for (k = 1; k < nbcoupon; ++k) {
      ...
      if ((coupon_dates[k] == fix_pay_times[fix_index]) &&
        ...
        ivalue =
            ivalue + rec * strike * fix_cvgs[fix_index - 1] *
    ...
    //-----------------------------------------------------------------
    err = Get_LGM_TermStructureOneOrTwoFact(und_name, &sigma_date, &sigma,
                                            &sigma_n, &tau_date, &tau, &tau_n,
                                            &alpha, &gamma, &rho);
    if (err) {
      return err;
    }
    ...
    exer_time[0] =
        (add_unit(lStartDate, -notperiod, SRT_BDAY, MODIFIED_SUCCEEDING) -
         lToday) /
        365.0;

    if (mdl_dim == ONE_FAC) {
      export_lgmsetupG(lambda, nbcoupon, coupon_time, cpn_G, 1, exer_time,
                       ex_G);
      if (err) {
        return err;
      }

      err = computeZeta(exer_time[0], lambda, sigma_date, sigma, sigma_n,
                        &ex_zeta);
      if (err) {
        return err;
      }
      ...
      *price = lgmopval1F(nbcoupon, coupon, cpn_G, ex_zeta, ex_G[0]);
      ...
      export_lgmsetupG2(lambda, gamma, nbcoupon, coupon_time, cpn_G2, 1,
                        exer_time, ex_G2);
      if (err) {
        return err;
      }
      ...
      err = computeZeta(exer_time[0], lambda + gamma, sigma_date, sigma,
                        sigma_n, &ex_zeta2);
      if (err) {
        return err;
      }
      ...
      err = computeZeta(exer_time[0], lambda + 0.5 * gamma, sigma_date, sigma,
                        sigma_n, &ex_zeta12);
      if (err) {
        return err;
      }
      ...
      *price = amort_lgmopval2F(nbcoupon, coupon, cpn_G, cpn_G2, ex_zeta,
                                ex_zeta2, ex_zeta12, ex_G[0], ex_G2[0]);
...
Err europAmortSwaption_clsdfrm_new(
    char *yc_name, SrtUndPtr und, int notperiod,

    char *fixbasis, int NFix, long *FixStartDates, long *FixEndDates,
    double *FixRates, double *FixNotionals,

    char *refrate, int NFloat, long *FloatStartDates, long *FloatEndDates,
    double *FloatMargins, double *FloatSpreads, double *FloatNotionals,

    double exer_fee, char *recPayStr, double *price) {
  Err err = NULL;
  Date lToday;
  String und_name;
  SrtMdlType mdl_type;
  ...
  for (k = 0; k < NFloat; ++k) {
    FloatCoverages[k] =
        coverage(FloatStartDates[k], FloatEndDates[k], floatbas);
  }
  ...
  if (mdl_type == LGM || mdl_type == NEWLGM) {
    ...
    float_index = 1;
    fix_index = 1;
    ivalue = -(FloatNotionals[0] + exer_fee) *
             swp_f_df(lToday, FloatStartDates[0], yc_name);
    coupon[0] = -(FloatNotionals[0] + exer_fee) *
                swp_f_df(lToday, FloatStartDates[0], yc_name);
    coupon_time[0] = coupon_dates[0];
    ...
    for (k = 1; k < nbcoupon; ++k) {
      ...
      if ((coupon_dates[k] == fix_pay_times[fix_index]) &&
          (coupon_dates[k] != float_pay_times[float_index])) {
        ivalue =
            ivalue + FixRates[fix_index - 1] * FixNotionals[fix_index - 1] *
        ...
        ivalue =
            ...
            -(FloatNotionals[float_index] - FloatNotionals[float_index - 1] +
              FloatNotionals[float_index - 1] *
                  (FloatSpreads[float_index - 1] +
                   FloatMargins[float_index - 1]) *
        ...
        if (float_index < NFloat) {
          ivalue = ivalue +
                   FixRates[fix_index - 1] * FixNotionals[fix_index - 1] *
                       FixCoverages[fix_index - 1] *
                       swp_f_df(lToday, FixEndDates[fix_index - 1], yc_name);
          coupon[k] += FixRates[fix_index - 1] * FixNotionals[fix_index - 1] *
                       FixCoverages[fix_index - 1] *
          ...
          ivalue = ivalue + -(-FloatNotionals[float_index - 1] +
                              FloatNotionals[float_index - 1] *
                                  (FloatSpreads[float_index - 1] +
                                   FloatMargins[float_index - 1]) *
                                  FloatCoverages[float_index - 1]) *
                                swp_f_df(lToday, FloatEndDates[float_index - 1],
                                         yc_name);
          coupon[k] +=
              -(-FloatNotionals[float_index - 1] +
                FloatNotionals[float_index - 1] *
    ...
    if (tau_n == 1) {
      ...
      exer_time[0] = (add_unit(FixStartDates[0], -notperiod, SRT_BDAY,
                               MODIFIED_SUCCEEDING) -
                      lToday) /
                     365.0;
    ...
    else // nTau > 1
    {
      ...
      if (mdl_dim == ONE_FAC) {
        export_lgmsetupG_ts(lambda_n, pdlambda_time, pdlambda, nbcoupon,
                            coupon_time, cpn_G, 1, exer_time, ex_G);
        ...
        err = computeZeta_ts(exer_time[0], pdlambda_time, pdlambda, lambda_n,
                             0., sigma_date, sigma, sigma_n, &ex_zeta);
        if (err) {
          goto FREE_RETURN;
          return err;
        }
        ...
        *price = lgmopval1F(nbcoupon, coupon, cpn_G, ex_zeta, ex_G[0]);
        ...
        export_lgmsetupG2_ts(lambda_n, pdlambda_time, pdlambda, gamma, nbcoupon,
                             coupon_time, cpn_G2, 1, exer_time, ex_G2);
        if (err) {
          goto FREE_RETURN;
          return err;
        }
        ...
        err = computeZeta_ts(exer_time[0], pdlambda_time, pdlambda, lambda_n,
                             gamma, sigma_date, sigma, sigma_n, &ex_zeta2);
        if (err) {
          goto FREE_RETURN;
          return err;
        }
        ...
        err =
            computeZeta_ts(exer_time[0], pdlambda_time, pdlambda, lambda_n,
                           0.5 * gamma, sigma_date, sigma, sigma_n, &ex_zeta12);
        if (err) {
          goto FREE_RETURN;
          return err;
        }
...
Err europAmortSwaption_clsdfrm(char *yc_name, SrtUndPtr und, int notperiod,
                               long lStartDate, long lEndDate, char *fixFreq,
                               char *fixBasis, char *refRateName,
                               double exer_fee, int Nfix, double *fixNotional,
                               double *fixRates, int Nfloat,
                               double *floatNotional, double *margin,
  ...
  if (mdl_type == LGM || mdl_type == NEWLGM) {
    ...
    float_index = 1;
    fix_index = 1;
    ivalue = -(floatNotional[0] + exer_fee) *
             swp_f_df(lToday, float_pay_dates[0], yc_name);
    coupon[0] = -(floatNotional[0] + exer_fee) *
                swp_f_df(lToday, float_pay_dates[0], yc_name);
    coupon_time[0] = coupon_dates[0];
    coupon_df[0] = swp_f_df(lToday, float_pay_dates[0], yc_name);
    coupon_cvg[0] = 0;
    for (k = 1; k < nbcoupon; ++k) {
      ...
      if ((coupon_dates[k] == fix_pay_times[fix_index]) &&
          (coupon_dates[k] != float_pay_times[float_index])) {
        ivalue =
            ivalue + fixRates[fix_index - 1] * fixNotional[fix_index - 1] *
        ...
        ivalue =
            ivalue +
            -(floatNotional[float_index] - floatNotional[float_index - 1] +
    ...
    if (mdl_dim == ONE_FAC) {
      ...
      *price = lgmopval1F(nbcoupon, coupon, cpn_G, ex_zeta, ex_G[0]);
      ...
      /*			err = get_underlying_ts(und        , &ts);
                              ...
                              err = find_2f_rho (exer_time[0]        , ts ,
         &rho); if(err)
                              {
                                      return err;
                              }

                              err = find_2f_sig (exer_time[0]        , ts ,
         &sig1        , &sig2); if(err)
                              {
                                      return err;
                              }
                              ...
                              err = find_2f_tau (exer_time[0]        , ts ,
         &tau1        , &tau2); if(err)
                              {
                                      return err;
                              }
...
Err europZCSwaption_clsdfrm(char *yc_name, SrtUndPtr und, int notperiod,
                            long lStartDate, long lEndDate, char *fixFreq,
                            char *fixBasis, char *refRateName, int Nfix,
                            double *fixNotional, double *fixRates, int Nfloat,
                            double *floatNotional, double *margin,
                            double *price) {
  Err err = NULL;
  Date lToday;
  ...
  if (mdl_type == LGM || mdl_type == NEWLGM) {
    ...
    coupon[0] =
        -floatNotional[0] * swp_f_df(lToday, float_pay_dates[0], yc_name);
    coupon_time[0] = coupon_dates[0];
    coupon_df[0] = swp_f_df(lToday, float_pay_dates[0], yc_name);
    coupon_cvg[0] = 0;
    for (k = 1; k < nbcoupon; ++k) {
        ...
        if (float_index < float_nb_pay_dates - 1) {
          ivalue =
              ivalue + fixRates[fix_index - 1] * fixNotional[fix_index - 1] *
                           ...
                           swp_f_df(lToday, fix_pay_dates[fix_index], yc_name);
          //					coupon[k] += fixRates[fix_index-1]
          //* fixNotional[fix_index-1] * fix_cvgs[fix_index-1] * swp_f_df
          // (lToday
          //      , fix_pay_dates[fix_index]        , yc_name);
          fix_index = fix_index + 1;
    ...
    for (k = fix_nb_dates - 1; k >= 0; --k) {
      finalFixCoupon +=
          accruedTerm * fixRates[k] * fixNotional[k] * fix_cvgs[k];
      accruedTerm = accruedTerm * (1 + fixRates[k] * fix_cvgs[k]);
    }

    coupon[nbcoupon - 1] +=
        finalFixCoupon *
        swp_f_df(lToday, fix_end_dates[fix_nb_dates - 1], yc_name);
                      ...
                      from option n-1 */
{
  ...
  convert_ratio =
      sqrt(lambda / LAMBDA_REF * (1.0 - exp(-2.0 * LAMBDA_REF * cpn_time[i])) /
  ...
  for (i = 0; i < nex - (skip_last > 0); i++) {
    ...
    if ((ex_bool[i] == 1) || (i == 0)) {
      ...
      if (firstcalibdone == 0) {
        ...
        while (pr1 > mkt_price[i] && it < 6) {
          ...
          pr1 = amortMidat_lgmamortsopval2F(ncpn - j, coupon + j, cpn_df + j,
                                            cpn_cvg + j, cpn_G1 + j, cpn_G2 + j,
                                            z1, z2, z12, ex_G1[i], ex_G2[i]);
    ...
    //			zeta2 = zeta2 + s2 * exp_fact1 * alpha * alpha *
    // exp_fact2; 			zeta12 = zeta12 + s2 * exp_fact1 * alpha * rho *
    // exp_fact12; 			t = ex_time[i];
    //		}
  }
                      ...
                      from option n-1 */
{
  ...
  convert_ratio = sqrt(lambda[0] / LAMBDA_REF *
                       (1.0 - exp(-2.0 * LAMBDA_REF * cpn_time[i])) /
  ...
  for (i = 0; i < nex - (skip_last > 0); i++) {
    ...
    exp_fact1 = export_lgmcalcexpfact_tauts(t, ex_time[i], nlambda, lambda_time,
                                            lambda, 0.0);
    exp_fact2 = export_lgmcalcexpfact_tauts(t, ex_time[i], nlambda, lambda_time,
                                            lambda, gamma) /
                exp_fact1;
    exp_fact12 = export_lgmcalcexpfact_tauts(t, ex_time[i], nlambda,
                                             lambda_time, lambda, 0.5 * gamma) /
                 exp_fact1;
  ...
  if (skip_last) {
    exp_fact1 = export_lgmcalcexpfact_tauts(t, ex_time[nex - 1], nlambda,
                                            lambda_time, lambda, 0.0);

    dz1 = s_last * exp_fact1;

    // dz1 = s_last * (( exp (2 * l1 * ex_time[nex-1]) - exp (2 * l1 * t)) / 2 /
    // l1);
                      ...
                      from option n-1 */
{
  ...
  for (i = 0; i < nex - (skip_last > 0); i++) {
    ...
    if ((ex_bool[i] == 1) || (i == 0)) {
      ...
      //			ex_zeta[i] = zeta1 + s2 * exp_fact1;
      //			zeta2 = zeta2 + s2 * exp_fact1 * alpha * alpha *
      // exp_fact2; 			zeta12 = zeta12 + s2 * exp_fact1 * alpha
      // * rho * exp_fact12; 			t = ex_time[i];
    }
  }
    ...
    if ((ex_bool[i] == 1) || (i == 0)) {
      ...
      if (firstcalibdone == 0) {
        ...
        while (pr1 > mkt_price[i] && it < 6) {
          ...
          pr1 =
              amortMidat_lgmamortsopval1F(ncpn - j, coupon + j, cpn_df + j,
                                          cpn_cvg + j, cpn_G + j, z1, ex_G[i]);
        ...
        for (it = 1; it < niter; it++) {
          ...
          if (s2 < minvar) {
            ...
            */
            ...
            s2 =
                solve_for_next_coef_dlm(res_iter, nb_iter, mkt_price[i] + d, 2);
            ...
            */
            ...
            if (s2 < minvar) {
              ...
              */
              ...
              s2 = solve_for_next_coef_dlm(res_iter, nb_iter, mkt_price[i] + d,
                                           2);
  ...
  for (i = 0; i < nex - (skip_last > 0); i++) {
    ...
    if ((ex_bool[i] == 1) || (i == 0)) {
      ...
      if (fabs((mkt_price[i] + d) - pr2) > CALPRESNOT) {
        ...
        //				smessage ("Failed to calibrate for
        //exercise date %d"        , i+1);
        s2 = s_last;
        dz2 = s2 * exp_fact;
        z2 = zeta + dz2;
    ...
    double *ex_sprice) /*	Cap price as output */
{
  ...
  if (one2F == 1) {
    err = AmortMidat_lgmcalibzeta1F(ncpn, cpn, cpn_time, cpn_df, cpn_cvg, cpn_G,
                                    nex, ex_bool, ex_time, ex_cpn, ex_G,
                                    ex_lprice, ex_fee, ex_zeta, floatnotional,
                                    lambda, skip_last);
    ...
    if (price_cap == 1) {
      *ex_sprice = amortMidat_lgmcapval1F_b(ncpn, cpn, cpn_df, cpn_cvg, cpn_G,
                                            nex, ex_sstrike, floatcoupon,
                                            ex_cpn, ex_sncpn, ex_zeta, ex_G);

    } else if (price_cap == 2) {

      *ex_sprice = amortMidat_lgmsumsopval1F_b(ncpn, nex, ex_cpn, cpn_df, cpn_G,
                                               ex_zeta, ex_G, swaption_cpn);
    }
  } else {
    err = AmortMidat_lgmcalibzeta2F(
        ncpn, cpn, cpn_time, cpn_df, cpn_cvg, cpn_G, cpn_G2, nex, ex_bool,
        ex_time, ex_cpn, ex_G, ex_G2, ex_lprice, ex_fee, ex_zeta, floatnotional,
    ...
    if (price_cap == 1) {
      *ex_sprice = amortMidat_lgmcapval2F_b(
          ncpn, cpn, cpn_df, cpn_cvg, cpn_G, cpn_G2, nex, ex_sstrike,
          floatcoupon, ex_cpn, ex_sncpn, ex_zeta, zeta2, zeta12, ex_G, ex_G2);

    } else if (price_cap == 2) {
      *ex_sprice = amortMidat_lgmsumsopval2F_b(ncpn, nex, ex_cpn, cpn_df, cpn_G,
                                               cpn_G2, ex_zeta, zeta2, zeta12,
                                               ex_G, ex_G2, swaption_cpn);
    }
  }
    ...
    double *ex_sprice) /*	Cap price as output */
{
  ...
  if (one2F == 1) {
    return serror("AmortMidat_lgmprcapgivenlambda_ts(..) :: only 2 factors are "
                  "allowed at the moment !");
  }

  /*	Setup G function */
  export_lgmsetupG_ts(nNumLambdas, pdLambdaTime, pdLambdaValue, ncpn, cpn_time,
                      cpn_G, nex, ex_time, ex_G);

  if (one2F == 2) {
    export_lgmsetupG2_ts(nNumLambdas, pdLambdaTime, pdLambdaValue, gamma, ncpn,
                         cpn_time, cpn_G2, nex, ex_time, ex_G2);
  }
  ...
  if (one2F == 1) {
    ...
    err = AmortMidat_lgmcalibzeta2F_ts(
        ncpn, cpn, cpn_time, cpn_df, cpn_cvg, cpn_G, cpn_G2, nex, ex_bool,
        ex_time, ex_cpn, ex_G, ex_G2, ex_lprice, ex_fee, ex_zeta, floatnotional,
        nNumLambdas, pdLambdaTime, pdLambdaValue, alpha, gamma, rho, skip_last);

    if (err) {
      return err;
    }

    export_lgmcalczeta2zeta12_ts(nex, ex_time, ex_zeta, nNumLambdas,
                                 pdLambdaTime, pdLambdaValue, alpha, gamma, rho,
                                 zeta2, zeta12);
    ...
    double *ex_sprice) /*	Cap price as output */
{
  ...
  if (one2F == 1) {
    err = zcAmortMidat_lgmcalibzeta1F(ncpn, cpn, zc_last_cpn, cpn_time, cpn_df,
                                      cpn_cvg, cpn_G, nex, ex_bool, ex_time,
                                      ex_cpn, ex_G, ex_lprice, ex_zeta,
                                      floatnotional, lambda, skip_last);
    ...
    err = zcAmortMidat_lgmcalibzeta2F(
        ncpn, cpn, zc_last_cpn, cpn_time, cpn_df, cpn_cvg, cpn_G, cpn_G2, nex,
        ex_bool, ex_time, ex_cpn, ex_G, ex_G2, ex_lprice, ex_zeta,
        floatnotional, lambda, alpha, gamma, rho, skip_last);
...
Err amortMidat_lgmcalibzetalambda(
    ...
    int fix_lambda,      /*	0: calib lambda to cap        , 1: fix lambda calib
                                           to diagonal */
    int cap_or_swaption, /*	0: calib lambda to cap        , 1: calib lambda
                      to sum of diag swaptions */
                      ...
                      from option n-1 */
{
  ...
  err = AmortMidat_lgmprcapgivenlambda(
      ncpn, cpn, cpn_time, cpn_df, cpn_cvg, nex, ex_bool, ex_time, ex_cpn,
      ex_sncpn, ex_lprice, ex_fee, ex_sstrike, floatcoupon, swaption_cpn,
      ex_zeta, floatnotional, *lambda, one2F, alpha, gamma, rho, skip_last,
      (fix_lambda ? 0 : 1) + cap_or_swaption, &pr1);
  ...
  do {
    ...
    if (err = AmortMidat_lgmprcapgivenlambda(
            ncpn, cpn, cpn_time, cpn_df, cpn_cvg, nex, ex_bool, ex_time, ex_cpn,
            ex_sncpn, ex_lprice, ex_fee, ex_sstrike, floatcoupon, swaption_cpn,
            ex_zeta, floatnotional, lam1, one2F, alpha, gamma, rho, skip_last,
            1 + cap_or_swaption, &pr1)) {
      return err;
    }
  ...
  for (it = 1; it < 20; it++) {
    ...
    if (err = AmortMidat_lgmprcapgivenlambda(
            ncpn, cpn, cpn_time, cpn_df, cpn_cvg, nex, ex_bool, ex_time, ex_cpn,
            ex_sncpn, ex_lprice, ex_fee, ex_sstrike, floatcoupon, swaption_cpn,
            ex_zeta, floatnotional, lam2, one2F, alpha, gamma, rho, skip_last,
            1 + cap_or_swaption, &pr2)) {
      return err;
    }
    ...
    if (fabs(lam2 - lam1) < CALPRESAMORTMIDAT ||
        fabs(pr2 - pr1) < CALPRESAMORTMIDAT)
      break;
    ...
    while (pr1 > pr2 && it < 10) {
      ...
      if (err = AmortMidat_lgmprcapgivenlambda(
              ncpn, cpn, cpn_time, cpn_df, cpn_cvg, nex, ex_bool, ex_time,
              ex_cpn, ex_sncpn, ex_lprice, ex_fee, ex_sstrike, floatcoupon,
              swaption_cpn, ex_zeta, floatnotional, lam2, one2F, alpha, gamma,
              rho, skip_last, 1 + cap_or_swaption, &pr2)) {
        return err;
      }
                      ...
                      from option n-1 */
{
  ...
  if (fix_lambda != 1) {
    return serror("amortMidat_lgmcalibzetalambda_ts(...) :: this version only "
                  "works for fixed lambda !");
  }
  ...
  err = AmortMidat_lgmprcapgivenlambda_ts(
      ncpn, cpn, cpn_time, cpn_df, cpn_cvg, nex, ex_bool, ex_time, ex_cpn,
      ex_sncpn, ex_lprice, ex_fee, ex_sstrike, floatcoupon, swaption_cpn,
      ex_zeta, floatnotional, nNumLambda, pdLambdaValue, pdLambdaTime, one2F,
      alpha, gamma, rho, skip_last, (fix_lambda ? 0 : 1), &pr1);
  ...
  do {
    ...
    if (err = AmortMidat_lgmprcapgivenlambda_ts(
            ncpn, cpn, cpn_time, cpn_df, cpn_cvg, nex, ex_bool, ex_time, ex_cpn,
            ex_sncpn, ex_lprice, ex_fee, ex_sstrike, floatcoupon, swaption_cpn,
            ex_zeta, floatnotional, nNumLambda, pdLambdaValue, pdLambdaTime,
            one2F, alpha, gamma, rho, skip_last, (fix_lambda ? 0 : 1), &pr1)) {
      return err;
    }
  ...
  for (it = 1; it < 20; it++) {
    ...
    while (pr1 > pr2 && it < 10) {
      ...
      if (err = AmortMidat_lgmprcapgivenlambda_ts(
              ncpn, cpn, cpn_time, cpn_df, cpn_cvg, nex, ex_bool, ex_time,
              ex_cpn, ex_sncpn, ex_lprice, ex_fee, ex_sstrike, floatcoupon,
              swaption_cpn, ex_zeta, floatnotional, nNumLambda, pdLambdaValue,
              pdLambdaTime, one2F, alpha, gamma, rho, skip_last,
              (fix_lambda ? 0 : 1), &pr1)) {
        return err;
      }
                      ...
                      from option n-1 */
{
  ...
  err = zcAmortMidat_lgmprcapgivenlambda(
      ncpn, cpn, zc_last_cpn, cpn_time, cpn_df, cpn_cvg, nex, ex_bool, ex_time,
      ex_cpn, ex_sncpn, ex_lprice, ex_sstrike, floatcoupon, ex_zeta,
      floatnotional, *lambda, one2F, alpha, gamma, rho, skip_last,
      (fix_lambda ? 0 : 1), &pr1);
  ...
  do {
    ...
    if (err = zcAmortMidat_lgmprcapgivenlambda(
            ncpn, cpn, zc_last_cpn, cpn_time, cpn_df, cpn_cvg, nex, ex_bool,
            ex_time, ex_cpn, ex_sncpn, ex_lprice, ex_sstrike, floatcoupon,
            ex_zeta, floatnotional, lam1, one2F, alpha, gamma, rho, skip_last,
            1, &pr1)) {
      return err;
    }
    ...
    long *float_end_datesOut)   /*	End date for the amortised swap */
{
  ...
  if (Nfix <= Nfloat) {
    floatindex = 0;
    for (i = 0; i < Nfix; ++i) {
      while ((float_end_datesIn[floatindex] < fix_end_datesIn[i]) &&
             floatindex < Nfloat) {
...
Err amortMidat_modify_dates2(
    int Nfix, long *fix_pay_datesIn, /*	Pay date for the amortised swap */

    int Nfloat, long *float_pay_datesIn,
    ...
    long *float_pay_datesOut) /*	Pay date of the amortised swap */
{
  ...
  if (Nfix <= Nfloat) {
    floatindex = 0;
    for (i = 0; i < Nfix; ++i) {
      while ((float_pay_datesIn[floatindex] < fix_pay_datesIn[i]) &&
             floatindex < Nfloat) {
...
Err amortMidat_cpd_calib_diagonal_new(
    int notperiod, char *yc_name, char *vol_curve_name,

    char *default_ref_rate_name, char *swaption_basis, char *swaption_freq,
    ...
    double vol_shift, int shift_type,

    int *ex_bool, double *diag_prices, double *ex_fee,

    char *fix_freq, char *fix_basis, int Nfix, long *fix_start_dates,
    long *fix_end_dates, long *fixpaydates,

    double *fix_rates, double *fix_notionals,

    char *float_freq, char *float_basis, int Nfloat, long *float_start_dates,
    long *float_end_dates, long *floatpaydates,

    double *float_margins, double *float_spreads, double *float_notionals,

    // Calibration Parameters
    double *short_strike, int strike_type, double max_std_short,

    int fix_lambda, int one_f_equi, int skip_last,

    int use_jump, double max_var_jump,

    double *lambda, int one2F,
    //	Alpha        , Gamma        , Rho (2F only)
    double alpha, double gamma, double rho,

    int *num_sig, double **sig_time,
    double **sig) { /// start of amortMidat_cpd_calib_diagonal_new
  int i, j, k, n, float_index, fix_index;
  int nbcoupon;
  SrtCompounding fixfreq, floatfreq;
  SrtBasisCode fixbasis, floatbasis, swapbasis;
  double ex_lstrike[MAX_CPN], ex_lprice[MAX_CPN], ex_sfwd[MAX_CPN],
      ex_slvl[MAX_CPN], ex_sstrike[MAX_CPN], ex_svol[MAX_CPN],
  ...
  *sig = NULL;
  ...
  //	err = swp_f_get_ref_rate_details(ref_rate        , &floatbasis        ,
  //&floatfreq);
  ...
  for (i = 0; i < Nfloat; ++i) {
    float_cvgs[i] =
        coverage(float_start_dates[i], float_end_dates[i], floatbasis);
  }
  fix_float_mult = floatfreq / fixfreq;

  //-------------Find First Exercise Date----------------
  if (add_unit(fix_start_dates[0], -notperiod, SRT_BDAY, MODIFIED_SUCCEEDING) <=
      today) {
    ex_bool[0] = 0;
  }
  ...
  for (i = firstDoCalib; i < Nfix; ++i) {
    ...
    coupon_cvg_standard[i - firstDoCalib] =
        coverage(fix_start_dates[i - firstDoCalib],
                 fix_end_dates[i - firstDoCalib], swapbasis);
  }
  ...
  fix_index = 1;
  coupon[0] =
      -float_notionals[0] * swp_f_df(today, float_start_dates[0], yc_name);
  //	swaption_cpn[0] = - swp_f_df (today        , float_start_dates[0] ,
  //yc_name);
  ...
  for (k = 1; k < nbcoupon; ++k) {
    ...
    if ((fabs(coupon_times[k] - fix_pay_times[fix_index]) < 10.0 / 365.0) &&
      ...
      exer_date[fix_index - 1] =
          add_unit(fix_start_dates[fix_index - 1], -notperiod, SRT_BDAY,
                   MODIFIED_SUCCEEDING);
      exer_time[fix_index - 1] =
          (exer_date[fix_index - 1] - today) * YEARS_IN_DAY;
      fix_index = fix_index + 1;
    } else if ((fabs(coupon_times[k] - fix_pay_times[fix_index]) >
                10.0 / 365.0) &&
               (fabs(coupon_times[k] - float_pay_times[float_index]) <
                10.0 / 365.0)) {
      ...
      coupon[k] +=
          -(float_notionals[float_index] - float_notionals[float_index - 1] +
            float_notionals[float_index - 1] *
                (float_spreads[float_index - 1] +
                 float_margins[float_index - 1]) *
                float_cvgs[float_index - 1]) *
      ...
      if (float_index < nbcoupon - 1) {
        ...
        exer_time[fix_index - 1] =
            (exer_date[fix_index - 1] - today) * YEARS_IN_DAY;
        fix_index = fix_index + 1;
        coupon[k] += -(-float_notionals[float_index - 1] +
                       float_notionals[float_index - 1] *
                           (float_spreads[float_index - 1] +
                            float_margins[float_index - 1]) *
                           float_cvgs[float_index - 1]) *
                     swp_f_df(today, floatpaydates[float_index - 1], yc_name);
        floatcoupon[k] +=
            -(-1.0 + (float_spreads[float_index - 1] +
                      float_margins[float_index - 1]) *
                         float_cvgs[float_index - 1]) *
            swp_f_df(today, floatpaydates[float_index - 1], yc_name);
        float_index = float_index + 1;
  ...
  if ((strike_type == 4) || (strike_type == 5) || (strike_type == 6) ||
    ...
    err = ComputeAmortSwapDiagonalIRRsNew2(
        yc_name, vol_curve_name, default_ref_rate_name, fixfreq, fixbasis,

        Nfix - firstDoCalib, fix_start_dates, fix_end_dates, fix_cvgs,
        fix_rates, fix_notionals, ex_fee,

        Nfloat - firstDoCalibFloat, float_start_dates, float_end_dates,
        float_cvgs, float_margins, float_spreads, float_notionals,
    ...
    for (i = 0; i < nbexercise; i++) {
      ...
      if ((strike_type == 4) || (strike_type == 5)) {
        err = ComputeSwapRateFromIRR(
            yc_name, vol_curve_name, today, default_ref_rate_name,
            float_start_dates[ex_coupon[i]], float_end_dates[ex_coupon[i]],
            fixfreq, fixbasis, dIRRs[i], UseVol, &dIRRstrikes[i]);
        if (err) {
          goto FREE_RETURN;
        }
      }
      //---------Compute equivalent swaption strike using diagonal irrs-----
        ...
        err = ComputeSwapRateFromIRR(
            ...
            fix_end_dates[Nfix - firstDoCalib - 1], fixfreq, fixbasis, dIRRs[i],
            UseVol, &dIRRstrikes[i]);
        if (err) {
          goto FREE_RETURN;
        }
      }
      ...
      //---------Compute diagonal swap        , level and
      //vol---------------------
      if ((strike_type == 6) || (strike_type == 7) || (strike_type == 8)) {
        lvl = 0.0;
        for (k = i; k < Nfix - firstDoCalib; k++) {
          lvl += coupon_cvg_standard[k] *
                 swp_f_df(today, fix_end_dates[k], yc_name);
        }
        ...
        err = get_cash_vol(
            ...
            fix_end_dates[k - 1], ex_sfwd[i], 0, default_ref_rate_name, &std,
            &power);
        if (err) {
          goto FREE_RETURN;
        }
      ...
      //-----------Custom Swap Strike -------------
        ...
        if (err = swp_f_ForwardRate(fix_start_dates[i], fix_end_dates[k - 1],
                                    swaption_freq, swaption_basis, yc_name,
                                    default_ref_rate_name, &swp_rte)) {
          goto FREE_RETURN;
        }
      ...
      else if ((strike_type == 4) || (strike_type == 5) || (strike_type == 6) ||
               (strike_type == 7)) {
      ...
      err = get_cash_vol(
          vol_curve_name,
          add_unit(exer_date[i], notperiod, SRT_BDAY, MODIFIED_SUCCEEDING),
          fix_end_dates[k - 1], calibstrike, 0, default_ref_rate_name,
          &(ex_svol[i]), &power);
      if (err) {
        goto FREE_RETURN;
      }
      ...
      if (power > 0.5) {
        ex_sprice[i] =
            srt_f_optblksch(ex_sfwd[i], calibstrike, ex_svol[i], exer_time[i],
                            ex_slvl[i], SRT_PUT, PREMIUM);
      } else {
        ex_sprice[i] =
            srt_f_optblknrm(ex_sfwd[i], calibstrike, ex_svol[i], exer_time[i],
                            ex_slvl[i], SRT_PUT, PREMIUM);
      }
    ...
    for (k = 1; k < nbcoupon; ++k) {
      if ((coupon_times[k] == fix_pay_times[fix_index]) &&
        ...
        swaption_cpn[k] += ex_sstrike[fix_index - 1] *
                           ...
                           swp_f_df(today, fixpaydates[fix_index - 1], yc_name);
        fix_index = fix_index + 1;
      } else if ((coupon_times[k] != fix_pay_times[fix_index]) &&
                 (coupon_times[k] == float_pay_times[float_index])) {
        swaption_cpn[k] +=
            -((float_spreads[float_index - 1] +
               float_margins[float_index - 1]) *
        ...
        if (float_index < nbcoupon - 1) {
          swaption_cpn[k] +=
              ex_sstrike[fix_index - 1] * coupon_cvg_standard[fix_index - 1] *
              swp_f_df(today, fixpaydates[fix_index - 1], yc_name);
          fix_index = fix_index + 1;
          float_index = float_index + 1;
        } else {
          swaption_cpn[k] +=
              ex_sstrike[fix_index - 1] * fix_cvgs[fix_index - 1] *
              swp_f_df(today, fixpaydates[fix_index - 1], yc_name);
          fix_index = fix_index + 1;
          swaption_cpn[k] +=
              -(-1 + (float_spreads[float_index - 1] +
                      float_margins[float_index - 1]) *
                         float_cvgs[float_index - 1]) *
              swp_f_df(today, floatpaydates[float_index - 1], yc_name);
          float_index = float_index + 1;
  ...
  if (one2F == 2 && fix_lambda && one_f_equi && (strike_type < 6)) {
    ...
    err = amortMidat_lgmcalibzetalambda(
        ...
        exer_bool, exer_time, ex_coupon, ex_sncpn, ex_lprice, ex_fee,
        shortcoupon, floatcoupon, 0.0, ex_zeta, float_notionals, swaption_cpn,
        1, 0, lambda, 1, 0.0, 0.0, 0.0, skip_last);
    ...
    cap_price = amortMidat_lgmcapval1F_b(
        nbcoupon, coupon, coupon_df, coupon_cvg, cpn_G, nbexercise, shortcoupon,
        floatcoupon, ex_coupon, ex_sncpn, ex_zeta, ex_G);
  ...
  err = amortMidat_lgmcalibzetalambda(
      nbcoupon, coupon, coupon_time, coupon_df, coupon_cvg, nbexercise,
      exer_bool, exer_time, ex_coupon, ex_sncpn, ex_lprice, ex_fee, shortcoupon,
      floatcoupon, cap_price, ex_zeta, float_notionals, swaption_cpn,
      fix_lambda, cap_or_swaption, lambda, one2F, alpha, gamma, rho, skip_last);
...
Err amortMidat_cpd_calib_diagonal_new_ts(
    ...
    int *num_sig, double **sig_time,
    double **sig) { /// start of amortMidat_cpd_calib_diagonal_new_ts
  int i, j, k, n, float_index, fix_index;
  int nbcoupon;
  SrtCompounding fixfreq, floatfreq;
  SrtBasisCode fixbasis, floatbasis, swapbasis;
  ...
  *sig = NULL;
  ...
  if (one2F == 2 && fix_lambda && one_f_equi && (strike_type < 6)) {
    ...
    err = amortMidat_lgmcalibzetalambda_ts(
        ...
        exer_bool, exer_time, ex_coupon, ex_sncpn, ex_lprice, ex_fee,
        shortcoupon, floatcoupon, 0.0, ex_zeta, float_notionals, swaption_cpn,
        1, 0, nNum_lambda, pdLambdaValue, pdLambdaDate, 1, 0.0, 0.0, 0.0,
        skip_last);
    ...
    //--------Price the cap within the calibrated LGM1F model----------------
    export_lgmsetupG_ts(nNum_lambda, pdLambdaValue, pdLambdaDate, nbcoupon,
                        coupon_time, cpn_G, nbexercise, exer_time, ex_G);
  ...
  err = amortMidat_lgmcalibzetalambda_ts(
      ...
      exer_bool, exer_time, ex_coupon, ex_sncpn, ex_lprice, ex_fee, shortcoupon,
      floatcoupon, cap_price, ex_zeta, float_notionals, swaption_cpn,
      fix_lambda, cap_or_swaption, nNum_lambda, pdLambdaValue, pdLambdaDate,
      one2F, alpha, gamma, rho, skip_last);
  ...
  *sig = (double *)calloc(*num_sig, sizeof(double));
  ...
  //// previously
  //// (*sig)[0] = sqrt ( ex_zeta[0] * 2 * (*lambda) / (exp (2 * (*lambda) *
  /// exer_time[0]) - 1.0) );

  exp_fact = static_lgmcalcexpfact_tauts(0.0, exer_time[0], nNum_lambda,
                                         pdLambdaDate, pdLambdaValue);
  ...
  for (i = 1; i < nbexercise; i++) {
    if (exer_bool[i] > 0) {
      ...
      if (ex_zeta[i] > ex_zeta[last_i]) {
        ...
        /// previously
        ///(*sig)[k] = sqrt ((ex_zeta[i] - ex_zeta[last_i]) * 2 * (*lambda)
        ///				/ (exp (2 * (*lambda) * exer_time[i]) - exp ( 2
        ///*
        ///(*lambda) * exer_time[last_i])));

        exp_fact = static_lgmcalcexpfact_tauts(exer_time[last_i], exer_time[i],
                                               nNum_lambda, pdLambdaDate,
                                               pdLambdaValue);
...
Err amortMidat_cpd_calib_diagonal_new2(
    int notperiod, char *yc_name, /*	Name of the yield curve */
    ...
    char *fix_freq, char *fix_basis, int Nfix,
    ...
    char *float_freq, char *float_basis, int Nfloat,
  ...
  err = amortMidat_modify_dates2(Nfix, fix_pay_dates, Nfloat, float_pay_dates,
                                 fix_pay_datesOut, float_pay_datesOut);
  ...
  *sig = NULL;
  ...
  dff = swp_f_df(today, float_pay_datesOut[Nfloat - 1 - firstDoCalibFloat],
                 yc_name);
  for (i = 0; i < nbexercise; i++) {
    ex_lstrike[i] = fix_rates[i];
    ex_lprice[i] = diag_prices[i];
  ...
  if (!fix_lambda) {
    ...
    for (i = 0; i < nbexercise; i++) {
      ...
      if ((strike_type == 6) || (strike_type == 7) || (strike_type == 8)) {
        ...
        for (k = i; k < Nfix - firstDoCalib; k++) {
          lvl += coupon_cvg_standard[k] *
                 swp_f_df(today, fix_pay_datesOut[k], yc_name);
        }
        dfi = swp_f_df(today, fix_start_dates[i], yc_name);
        ...
        err = get_cash_vol(
            vol_curve_name,
            add_unit(exer_date[i], notperiod, SRT_BDAY, MODIFIED_SUCCEEDING),
            fix_pay_datesOut[k - 1], ex_sfwd[i], 0, default_ref_rate_name, &std,
            &power);
        if (err) {
          goto FREE_RETURN;
        }
      ...
      //-----------Custom Swap Strike -------------
        ...
        if (err = swp_f_ForwardRate(fix_start_dates[i], fix_pay_datesOut[k - 1],
                                    swaption_freq, swaption_basis, yc_name,
                                    default_ref_rate_name, &swp_rte)) {
          goto FREE_RETURN;
        }
      ...
      err = get_cash_vol(
          vol_curve_name,
          add_unit(exer_date[i], notperiod, SRT_BDAY, MODIFIED_SUCCEEDING),
          fix_pay_datesOut[k - 1], calibstrike, 0, default_ref_rate_name,
          &(ex_svol[i]), &power);
      if (err) {
        goto FREE_RETURN;
      }
        ...
        inst_data) /*	NULL = don't save calibration instrument data */
{
  ...
  *sig = NULL;
  ...
  //-------------Find Start Date----------------
  //--------------------------------------------
  if (add_unit(start_date, -notperiod, SRT_BDAY, MODIFIED_SUCCEEDING) <=
      today) {
    ex_bool[0] = 0;
  }
  ...
  for (i = (int)(0.5 + fix_cvgs[1] / float_cvgs[1]) * firstDoCalib;
       i < float_nb_dates; ++i) {
  ...
  fix_float_mult =
      (int)(fix_cvgs[fix_nb_dates - 1] / float_cvgs[float_nb_dates - 1] + 0.5);
  ...
  for (k = 1; k < nbcoupon; ++k) {
    ...
    if ((coupon_dates[k] == fix_pay_times[fix_index]) &&
      ...
      ex_coupon[fix_index - 1] = k - fix_float_mult;
      exer_date[fix_index - 1] =
          add_unit(fix_pay_dates[fix_index - 1], -notperiod, SRT_BDAY,
                   MODIFIED_SUCCEEDING);
      exer_time[fix_index - 1] =
          (exer_date[fix_index - 1] - today) * YEARS_IN_DAY;
      fix_index = fix_index + 1;
      ...
      if (float_index < float_nb_pay_dates - 1) {
        coupon[k] += long_strike[fix_index - 1] * fixNotional[fix_index - 1] *
                     fix_cvgs[fix_index - 1] *
                     swp_f_df(today, fix_pay_dates[fix_index], yc_name);
        swaption_cpn[k] += long_strike[fix_index - 1] *
                           fix_cvgs[fix_index - 1] *
                           swp_f_df(today, fix_pay_dates[fix_index], yc_name);
        ex_coupon[fix_index - 1] = k - fix_float_mult;
        exer_date[fix_index - 1] =
  ...
  if ((strike_type == 4) || (strike_type == 5) || (strike_type == 6)) {
    err = ComputeAmortSwapDiagonalIRRs(
        yc_name, vol_curve_name, default_ref_rate_name, start_date, end_date,
        ifreq, ibasis, fix_nb_dates, fixNotional, long_strike, ex_fee,
        float_nb_dates, floatNotional, margin, dIRRs, UseVol);
    if (err) {
      goto FREE_RETURN;
    }
  }
  ...
  if ((strike_type == 4) || (strike_type == 5) || (strike_type == 6)) {
    err = ComputeSwapRateFromIRR(yc_name, vol_curve_name, today,
                                 default_ref_rate_name, start_date, end_date,
                                 ifreq, ibasis, dIRRs[0], UseVol, &dtest);
    if (err) {
      goto FREE_RETURN;
    }
  }
    ...
    for (i = 0; i < nbexercise; i++) {
      if ((strike_type == 4) || (strike_type == 5)) {
        err = ComputeSwapRateFromIRR(
            yc_name, vol_curve_name, today, default_ref_rate_name,
            float_start_dates[ex_coupon[i]], float_end_dates[ex_coupon[i]],
            ifreq, ibasis, dIRRs[i], UseVol, &dIRRstrikes[i]);
        if (err) {
          goto FREE_RETURN;
        }
        ...
        err = ComputeSwapRateFromIRR(
            yc_name, vol_curve_name, today, default_ref_rate_name,
            float_start_dates[ex_coupon[i]], end_date, ifreq, ibasis, dIRRs[i],
            UseVol, &swaption_strikes[i]);
        if (err) {
          goto FREE_RETURN;
        }
      }
      ...
      if (strike_type == 6) {
        ...
        err = get_cash_vol(
            vol_curve_name,
            add_unit(exer_date[i], notperiod, SRT_BDAY, MODIFIED_SUCCEEDING),
            fix_pay_dates[k], ex_sfwd[i], 0, ref_rate_name, &std, &power);
        if (err) {
          goto FREE_RETURN;
        }
      } else {
      ...
      if ((!short_strike) || (!strike_type)) {
        ...
        if (err = swp_f_ForwardRate(
                float_pay_dates[ex_coupon[i]], fix_pay_dates[k], swaption_freq,
      ...
      err = get_cash_vol(
          ...
          fix_pay_dates[k],
          //				ex_sstrike[i]        ,
          calibstrike, 0, ref_rate_name, &(ex_svol[i]), &power);
      if (err) {
        goto FREE_RETURN;
      }
      ex_svol[i] += (shift_type == 1 ? ex_svol[i] * vol_shift : vol_shift);
      ...
      if (power > 0.5) {
        ex_sprice[i] =
            srt_f_optblksch(ex_sfwd[i],
                            //					ex_sstrike[i] ,
                            calibstrike, ex_svol[i], exer_time[i], ex_slvl[i],
                            SRT_PUT, PREMIUM);
      } else {
        ex_sprice[i] =
            srt_f_optblknrm(ex_sfwd[i],
                            //					ex_sstrike[i] ,
                            calibstrike, ex_svol[i], exer_time[i], ex_slvl[i],
                            SRT_PUT, PREMIUM);
      }
  ...
  if (one2F == 2 && fix_lambda && one_f_equi) {
    ...
    for (i = 0; i < nbexercise; i++) {
      ...
      lvl = 0.0;
      //			for
      //(k=(int)(ex_coupon[i]*(fix_nb_pay_dates-1)/(float_nb_pay_dates-1));
      // k<(ex_coupon[i]+ex_sncpn[i])*(fix_nb_pay_dates-1)/(float_nb_pay_dates-1);
      // k++)
      //			{
      //				lvl += fix_cvgs[k] * swp_f_df(today ,
      // fix_pay_dates[k+1]        , yc_name);
      //			}
      for (k = i; k < i + 1; k++) {
        lvl += fix_cvgs[k] * swp_f_df(today, fix_pay_dates[k + 1], yc_name);
      }
      //			dfi = swp_f_df (today        ,
      // float_pay_dates[ex_coupon[i]]        , yc_name);
      dfi = swp_f_df(today, fix_pay_dates[i], yc_name);
      dff = swp_f_df(today, fix_pay_dates[k], yc_name);
    ...
    err = amortMidat_lgmcalibzetalambda(
        nbcoupon, coupon, coupon_time, coupon_df, coupon_cvg, nbexercise,
        exer_bool, exer_time, ex_coupon, ex_sncpn, ex_lprice, ex_fee,
        shortcoupon, floatcoupon, 0.0, ex_zeta, floatNotional, swaption_cpn, 1,
        0, lambda, 1, 0.0, 0.0, 0.0, skip_last);
  ...
  err = amortMidat_lgmcalibzetalambda(
      nbcoupon, coupon, coupon_time, coupon_df, coupon_cvg, nbexercise,
      exer_bool, exer_time, ex_coupon, ex_sncpn, ex_lprice, ex_fee, shortcoupon,
      floatcoupon, cap_price, ex_zeta, floatNotional, swaption_cpn, fix_lambda,
      cap_or_swaption, lambda, one2F, alpha, gamma, rho, skip_last);
        ...
        inst_data) /*	NULL = don't save calibration instrument data */
{
  ...
  */
    ...
    for (i = 0; i < nbexercise; i++) {
      ...
      */
      ...
      err = get_cash_vol(
          vol_curve_name,
          add_unit(exer_date[i], notperiod, SRT_BDAY, MODIFIED_SUCCEEDING),
          fix_pay_dates[k], ex_sstrike[i], 0, ref_rate_name, &(ex_svol[i]),
          &power);
      if (err) {
        goto FREE_RETURN;
      }
  ...
  if (one2F == 2 && fix_lambda && one_f_equi) {
    ...
    err = zcamortMidat_lgmcalibzetalambda(
        nbcoupon, coupon, lastFixCoupon, coupon_time, coupon_df, coupon_cvg,
        nbexercise, exer_bool, exer_time, ex_coupon, ex_sncpn, ex_lprice,
        shortcoupon, floatcoupon, 0.0, ex_zeta, floatNotional, 1, lambda, 1,
        0.0, 0.0, 0.0, skip_last);
  ...
  err = zcamortMidat_lgmcalibzetalambda(
      nbcoupon, coupon, lastFixCoupon, coupon_time, coupon_df, coupon_cvg,
      nbexercise, exer_bool, exer_time, ex_coupon, ex_sncpn, ex_lprice,
      shortcoupon, floatcoupon, cap_price, ex_zeta, floatNotional, fix_lambda,
      lambda, one2F, alpha, gamma, rho, skip_last);

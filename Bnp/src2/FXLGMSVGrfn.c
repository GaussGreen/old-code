    ...
    if (total->do_dom)
    {
        for (k = 0; k < total->num_dom_df; k++)
        {
            local->evt->df[total->dom_idx][k] =
                exp(total->dom_dff[k] - total->dom_gam1[k] * domft1 - total->dom_gam2[k] * domft2 -
                    total->dom_gam1_2[k] * dompsi1 - total->dom_gam2_2[k] * dompsi2 -
                    total->dom_gam12[k] * dompsi12);
        }
    ...
    if (total->do_fx)
    {
        for (k = 0; k < total->num_fx_df; k++)
        {
            local->evt->df[total->fx_idx][k] =
                exp(total->fx_dff[k] - total->fx_gam1[k] * domft1 - total->fx_gam2[k] * domft2 -
                    total->fx_gam1_2[k] * dompsi1 - total->fx_gam2_2[k] * dompsi2 -
                    total->fx_gam12[k] * dompsi12);
        }
    }
    if (total->do_for)
    {
        for (k = 0; k < total->num_for_df; k++)
        {
            local->evt->df[total->for_idx][k] =
                exp(total->for_dff[k] - total->for_gam1[k] * forft1 - total->for_gam2[k] * forft2 -
                    total->for_gam1_2[k] * forpsi1 - total->for_gam2_2[k] * forpsi2 -
                    total->for_gam12[k] * forpsi12);
        }
    ...
    int*    stop_path)
{
    ...
    if (total->do_dom)
    {
        ...
        {
            local->evt->df[total->dom_idx][k] =
                exp(total->dom_dff[k] - total->dom_gam1[k] * domft - total->dom_gam1_2[k] * dompsi);
        }
    ...
    if (total->do_fx)
    {
        ...
        {
            local->evt->df[total->fx_idx][k] =
                exp(total->fx_dff[k] - total->fx_gam1[k] * domft - total->fx_gam1_2[k] * dompsi);
        }
    }
    if (total->do_for)
    {
        ...
        {
            local->evt->df[total->for_idx][k] =
                exp(total->for_dff[k] - total->for_gam1[k] * forft - total->for_gam1_2[k] * forpsi);
        }
    ...
    */
    ...
    if (total->do_for)
    {
        for (k = 0; k < total->num_for_df; k++)
        {
            ...
            smessage (" total->for_gam1_2[%d]=%f\n", k, (float)total->for_gam1_2[k]);
            smessage (" total->for_gam2_2[%d]=%f\n", k, (float)total->for_gam2_2[k]);
            smessage (" total->for_gam12[%d]=%f\n", k, (float)total->for_gam12[k]);
            smessage (" DF[%d]=%f\n", k, (float)local->evt->df[total->for_idx][k]);
            */
        }

...
Err Vasicek_Y_T_at_param(
    Ddate fix_date, Ddate* pay_dates, long dim_pay_date, SrtUndPtr und, YTatt_param* YT_sam)
{
    Err         err = NULL;
    TermStruct* ts;
    long        i;
    double      F_t, G_t, H_fix_t, Psi_t, S_t, O_t, vasicek_mean_sr_t, vasicek_mean_int_sr_t;
            ...
            if (mdl_type == ETABETA)
        {
            ...
            if (YT_sam[i].beta == 0.)
                ...
                theta = YT_sam[i].lambda_T *
                        (1. + YT_sam[i].beta * samptr_get(sam, index, STATEVAR)) / YT_sam[i].beta;

            M_stheta = M_eta_beta_func(s, theta, YT_sam[i].eta, YT_sam[i].M);

            /* Compute YTt_yield[i] from M_tT,A_t,A_T,lambda_t */
            YTt_yield[i] +=
                (YT_sam[i].lambda_T - YT_sam[i].lambda_t) * samptr_get(sam, index, STATEVAR);
...
SrtErr srt_f_rtevalyp(
    SrtRtFnc rt, SrtSample* sam, int index, SrtMdlDim mdl_dim, SrtMdlType mdl_type)
{
    int          i;
    double*      df;
    long         dim_Tdate;

...
static double NAG_CALL Z_func(double v, Nag_User *comm) {
             ...
             1e-16) // not yet calculated or no match
  {
    ...
    {
      //			err = FxHestonCalcPhi(q  , v  , -1.0  , &phi_re
      //, &phi_im);
...
Err FxHestonDCalibrate(char *yc_dom, char *yc_for, double spot_fx, long D_star,
  ...
  for (Calib.cal_from = 0; Calib.cal_from < ndates;
       Calib.cal_from = Calib.cal_to + 1) {
    for (Calib.cal_to = Calib.cal_from;
         Calib.cal_to < ndates - 1 &&
         (!calib_smile || !calib_smile[Calib.cal_to]);
         Calib.cal_to++)
      ;
    ...
    if (calib_smile && calib_smile[Calib.cal_to]) {
      ...
      smessage("Smile calibration at step %d  , time in sec: %.2f",
               Calib.cal_to, (double)(t2 - t1) / CLOCKS_PER_SEC);
    } else {
      err = CalibSigma(&Calib);
      if (err)
        goto FREE_RETURN;

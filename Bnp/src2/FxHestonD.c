...
typedef struct _SFxHestonComm_InvFT {
	...
	double			*k;			// Strikes ( log(K'+sh/FFX0+sh) )	K'= K*df_for(T_pay,T_star)/df_dom(T_pay,T_star)
...
typedef struct _SFxHestonComm_Calib {
	...
	double			*f0, *pay_to_star;			// forwards and T_star adjustments at each exercise date
...
static void NAG_CALL EvalDerivatives(Integer neq, double t, double y[], double yp[], Nag_User *comm)
{
	...
	yp[2] = -0.5 * c1 * (y[2]*y[2] - y[3]*y[3]) + p->lam * y[2] + c2 * (p->u_re * y[3] + p->u_im * y[2])
...
static Err FxHestonCalcPhi(SFxHestonComm_InvFT *q, double u_re, double u_im, double *h_re, double *h_im)
{
	Err					err = NULL;
	int					i, j, neq = 4;
	SFxHestonUnd		*o = q->o;
	...
	for (; i >= 0; i--)
	{
		...
		nag_ode_ivp_rk_range(neq, EvalDerivatives, (i ? o->t[i-1] : 0.0), &tgot, y, yp, ymax, &opt, &comm_Nag, &fail);
		if (fail.code != NE_NOERROR) { err = serror(fail.message);  goto FREE_RETURN; }
...
static Err FxHestonCalcPhiClosedForm(SFxHestonComm_InvFT *q, double u_re, double u_im, double *h_re, double *h_im)
{
	SFxHestonUnd		*o = q->o;
	int					i;
	...
	for (; i >= 0; i--)
	{
		...
		LGMSVUpdateADClosedForm((i ? t - o->t[i-1] : t), o->lam[i], -0.5 * o->alpha[i] * o->alpha[i],
			o->lam[i] + c1 * u_im, -c1 * u_re, c2 * (u_re * u_re - u_im * u_im - u_im), c2 * (u_re + 2.0 * u_re * u_im),
			&C_re, &C_im, &A_re, &A_im);
...
{
	if (cache->next) FxHestonFreeCache(cache->next, nk);
	free(cache->next);
	free(cache->v);
	if (cache->fn) free_dmatrix(cache->fn, 0, cache->maxpts-1, 0, nk-1);
	return NULL;
}
...
static double NAG_CALL Z_func(double v, Nag_User *comm)
{
	...
	double				phi_re, phi_im, zeta_re, zeta_im;//, phi_re_test, phi_im_test;
	double				dd1, dd2;
	int					i;
	...
	else if (fabs(v - q->c_cur->v[q->c_cur->cur_pt]) > 1e-16)		// not yet calculated or no match
	{
		...
		{
//			err = FxHestonCalcPhi(q, v, -1.0, &phi_re, &phi_im);
...
static Err FxHestonDoIntegration(SFxHestonComm_InvFT *q, int idx_k, double *res)
{
	Err				err = NULL;
...
static void NAG_CALL EvalDerivativesU(Integer neq, double t, double y[], double yp[], Nag_User *comm)
{
	SFxHestonComm_ODE	*p = (SFxHestonComm_ODE *) comm->p;
...
static Err FxHestonCalcMoments(SFxHestonComm_InvFT *q, double *mean, double *std)
{
	Err					err = NULL;
	SFxHestonUnd		*o = q->o;
	int					i, j;
	...
	for (j=0; j < 4; j++) RKthres[j] = thres_min;
	...
	for (; i >= 0; i--)
	{
		...
		nag_ode_ivp_rk_range(4, EvalDerivativesU, (i ? o->t[i-1] : 0.0), &tgot, y, yp, ymax, &opt, &comm_Nag, &fail);
		if (fail.code != NE_NOERROR) { err = serror(fail.message);  goto FREE_RETURN; }
...
Err FxHestonDOptions(SFxHestonUnd *und, long fix_date, long pay_date,
					 int nK, double *K, char **rec_pay_str, double *res)
{
	Err					err = NULL;
	...
	for (i=0; i < nK; i++)
		comm_InvFT.k[i] = log( (K[i] * pay_to_star + und->shift) / (f0 * pay_to_star + und->shift) );
	...
	for (i=0; i < nK; i++)
	{
		...
		if (rec_pay == SRT_PUT && K[i] > f0) res[i] += K[i] - f0;
		if (rec_pay == SRT_CALL && K[i] < f0) res[i] += f0 - K[i];
...
static Err CalibSigma(SFxHestonComm_Calib *Calib)
{
	...
	for (i = Calib->cal_from; i <= Calib->cal_to; i++)
	{
		...
		if (i > 0) sig_1st_guess = und->sigma[i-1];
		else sig_1st_guess = Calib->volATM[0] * Calib->f0[0] / (Calib->f0[0] + und->shift / Calib->pay_to_star[0]);

		err = NewtonD(sig_1st_guess, 1e-8, 2.0 / sqrt(und->t[i]), EvalATMdiff, 1e-4, 7, Calib);
		if (err) return err;
	}
	return NULL;
}
...
static Err Get_RR_BF(SFxHestonComm_Calib *Calib, double *RR, double *BF)
{
	...
	for (i=0; i < 2; i++)
	{
		...
		{ err = serror("Calibration strike is too far from the money");  goto FREE_RETURN; }
		...
		err = srt_f_optimpvol(z, Calib->f0[idx], KK[i], und->t[idx], 1.0,
			(KK[i] < Calib->f0[idx] ? SRT_PUT : SRT_CALL), SRT_LOGNORMAL, &vols[i]);
		if (err) goto FREE_RETURN;
	}
...
static Err EvalRRdiff(double x, double *diff, void *comm)
{
	...
	if (a_max < a_min) a_max = a_min;

	if (Calib->cal_from > 0) alpha_1st_guess = und->alpha[Calib->cal_from - 1];
	if (Calib->cal_from == 0 || alpha_1st_guess <= a_min) alpha_1st_guess = a_min + 0.05 * (a_max-a_min);

	err = NewtonD(alpha_1st_guess*alpha_1st_guess, a_min*a_min, a_max*a_max, EvalBFdiff, 1e-4, 7, comm);
	if (err) return err;
	...
	double			*rho )
{
	...
	und.S0 = spot_fx * swp_f_df(today, spotdate, yc_dom) / swp_f_df(today, spotdate, yc_for);
	und.D_star = D_star;
	und.today = today;
	und.shift = und.S0 * swp_f_df(today, D_star, yc_for) / swp_f_df(today, D_star, yc_dom) * (1.0 - beta) / beta;
	und.nt = ndates;
	und.sigma = sigma;
	und.alpha = alpha;
	und.rho = rho;
	...
	if (!und.t || !Calib.k1 || !Calib.k2 || !Calib.RR || !Calib.BF || !Calib.f0 || !Calib.pay_to_star)
	...
	for (i=0; i < ndates; i++)
	{
		...
		Calib.pay_to_star[i] = (df_star_for / df_pay_for) / (df_star_dom / df_pay_dom);
	...
	for (Calib.cal_from = 0; Calib.cal_from < ndates; Calib.cal_from = Calib.cal_to + 1)
	{
		for (Calib.cal_to = Calib.cal_from; Calib.cal_to < ndates-1 && (!calib_smile || !calib_smile[Calib.cal_to]);
			Calib.cal_to++);

		if (calib_smile && calib_smile[Calib.cal_to])
		{
			ra_max = 0.99 * 20.0 / sqrt(und.t[Calib.cal_to]);	// alpha limit
			ra_min = -ra_max;
			...
			if (err) goto FREE_RETURN;
			t2 = clock();
			smessage("Smile calibration at step %d, time in sec: %.2f", Calib.cal_to,

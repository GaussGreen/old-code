...
typedef struct _SFxHestonComm_InvFT
{
    ...
    double*
        k;  // Strikes ( log(K'+sh/FFX0+sh) )	K'= K*df_for(T_pay,T_star)/df_dom(T_pay,T_star)
...
static Err FxHestonCalcPhi(
    SFxHestonComm_InvFT* q, double u_re, double u_im, double* h_re, double* h_im)
{
    Err               err = NULL;
    int               i, j, neq = 4;
    SFxHestonUnd*     o = q->o;
...
static Err FxHestonCalcPhiClosedForm(
    SFxHestonComm_InvFT* q, double u_re, double u_im, double* h_re, double* h_im)
{
    SFxHestonUnd* o = q->o;
    int           i;
...
static Err EvalATMdiff(double x, double* diff, void* comm)
{
    ...
    err = srt_f_optimpvol(
        z, Calib->f0[idx], Calib->f0[idx], und->t[idx], 1.0, SRT_CALL, SRT_LOGNORMAL, &vol);
    if (err)
        goto FREE_RETURN;
...
static Err EvalRRdiff(double x, double* diff, void* comm)
{
    ...
    err = NewtonD(
        alpha_1st_guess * alpha_1st_guess, a_min * a_min, a_max * a_max, EvalBFdiff, 1e-4, 7, comm);
    if (err)
        return err;
    ...
    double* rho)
{
    ...
    und.D_star = D_star;
    und.today  = today;
    und.shift  = und.S0 * swp_f_df(today, D_star, yc_for) / swp_f_df(today, D_star, yc_dom) *
                (1.0 - beta) / beta;
    und.nt    = ndates;
    und.sigma = sigma;
    und.alpha = alpha;
    ...
    for (i = 0; i < ndates; i++)
    {
        ...
        {
            Calib.k1[i] =
                log((K1[i] * Calib.pay_to_star[i] + und.shift) /
                    (Calib.f0[i] * Calib.pay_to_star[i] + und.shift));
            Calib.k2[i] =
                log((K2[i] * Calib.pay_to_star[i] + und.shift) /
                    (Calib.f0[i] * Calib.pay_to_star[i] + und.shift));
    ...
    {
        for (Calib.cal_to = Calib.cal_from;
             Calib.cal_to < ndates - 1 && (!calib_smile || !calib_smile[Calib.cal_to]);
             Calib.cal_to++)
            ;

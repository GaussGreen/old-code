...
Err ConvexFx1F(char *underlying, long maturity, double barrier, double strike,
  ...
  for (i = 1; i < nstp; i++) {
    ...
    sig_quad[i - 1][0] =
        sqrt(fx_cum_vol_func(time[i], SRT_TRUE, fx_ts) - cum_vol);
    sig_quad[i - 1][1] = sqrt(fx_cum_vol_func(mat, SRT_TRUE, fx_ts) - cum_vol);

    drift[i - 1][0] = (swp_f_zr(date[i - 1], date[i], dom_yc) -
                       swp_f_zr(date[i - 1], date[i], for_yc)) *
                          dt -
                      0.5 * sig_quad[i - 1][0] * sig_quad[i - 1][0];
    drift[i - 1][1] = (swp_f_zr(date[i - 1], maturity, dom_yc) -
                       swp_f_zr(date[i - 1], maturity, for_yc)) *
                          (mat - time[i - 1]) -
...
double DeltaPIPS(double F, double T, double DfDom, double DfFor, double K,
...
double DeltaPercent(double F, double T, double DfDom, double DfFor, double K,
                    double Vol, int IsCall) {
  double d2, std;
...
double DeltaSimple(double F, double T, double DfDom, double DfFor, double K,
                   double Vol, int IsCall) {
  double d1, std;
...
Err SABR_to_Strikes(double Fwd, double T, double DfDom, double DfFor,
  ...
  switch (DeltaType) {
  case 0: {
    err = SABR_SolveDeltaNewton(Fwd, T, DfDom, DfFor, 0.25, 1, Sigma, Alpha,
                                Beta, Rho, VolType, DeltaSimple, IsForeign,
                                Precision, Strike1, Vol1);
    if (err) {
      return err;
    }

    err = SABR_SolveStraddleNewton(Fwd, T, DfDom, DfFor, Sigma, Alpha, Beta,
                                   Rho, VolType, DeltaSimple, IsForeign,
                                   Precision, Strike2, Vol2);
    if (err) {
      return err;
    }

    err = SABR_SolveDeltaNewton(Fwd, T, DfDom, DfFor, -0.25, 0, Sigma, Alpha,
                                Beta, Rho, VolType, DeltaSimple, IsForeign,
                                Precision, Strike3, Vol3);
    if (err) {
      return err;
    }
    break;
  ...
  case 1: {
    err = SABR_SolveDeltaNewton(Fwd, T, DfDom, DfFor, 0.25, 1, Sigma, Alpha,
                                Beta, Rho, VolType, DeltaPercent, IsForeign,
                                Precision, Strike1, Vol1);
    if (err) {
      return err;
    }
    ...
    err = SABR_SolveStraddleNewton(Fwd, T, DfDom, DfFor, Sigma, Alpha, Beta,
                                   Rho, VolType, DeltaPercent, IsForeign,
                                   Precision, Strike2, Vol2);
    if (err) {
      return err;
    }
  ...
  case 2: {
    err = SABR_SolveDeltaNewton(Fwd, T, DfDom, DfFor, 0.25, 1, Sigma, Alpha,
                                Beta, Rho, VolType, DeltaPIPS, IsForeign,
                                Precision, Strike1, Vol1);
    if (err) {
      return err;
    }
    ...
    err = SABR_SolveStraddleNewton(Fwd, T, DfDom, DfFor, Sigma, Alpha, Beta,
                                   Rho, VolType, DeltaPIPS, IsForeign,
                                   Precision, Strike2, Vol2);
    if (err) {
      return err;
    }
  ...
  err = srt_f_optsarbvol(Fwd, Fwd, T, Sigma, Alpha, Beta, Rho, VolType,
                         SRT_LOGNORMAL, VolATM);
...
Err SABR_SolveDeltaNewton(double F, double T, double DfDom, double DfFor,
  ...
  err = srt_f_optsarbvol(F, F, T, Sigma, Alpha, Beta, Rho, VolType,
                         SRT_LOGNORMAL, &volATM);
  if (err) {
    return err;
  }
  ...
  if (IsForeign) {
    ...
    if (IsCall) {
      ...
      err = srt_f_optsarbvol(F, 1.0 / K1, T, Sigma, Alpha, Beta, Rho, VolType,
                             SRT_LOGNORMAL, &Sig1);
      if (err)
        return err;
    ...
    err = srt_f_optsarbvol(F, 1.0 / K2, T, Sigma, Alpha, Beta, Rho, VolType,
                           SRT_LOGNORMAL, &Sig2);
    if (err)
      return err;
    ...
    if (IsCall) {
      ...
      err = srt_f_optsarbvol(F, K1, T, Sigma, Alpha, Beta, Rho, VolType,
                             SRT_LOGNORMAL, &Sig1);
      if (err)
        return err;
    ...
    err = srt_f_optsarbvol(F, K2, T, Sigma, Alpha, Beta, Rho, VolType,
                           SRT_LOGNORMAL, &Sig2);
    if (err)
      return err;
...
Err FindStrikes(double Fwd, double T, double DfDom, double DfFor, int DeltaType,
                int IsForeign, double Precision, double Vol25Put,
                double VolStraddle, double Vol25Call, double *Strike25Put,
                double *StrikeStraddle, double *Strike25Call) {
  Err err = NULL;

  switch (DeltaType) {
  case 0: {
    err = SolveStraddleNewton(Fwd, T, DfDom, DfFor, VolStraddle, DeltaSimple,
                              IsForeign, Precision, StrikeStraddle);
    if (err) {
      return err;
    }
    err = SolveDeltaNewton(Fwd, T, DfDom, DfFor, 0.25, 1, Vol25Call,
                           DeltaSimple, IsForeign, Precision, Strike25Call);
    if (err) {
      return err;
    }
    err = SolveDeltaNewton(Fwd, T, DfDom, DfFor, -0.25, 0, Vol25Put,
                           DeltaSimple, IsForeign, Precision, Strike25Put);
    if (err) {
      return err;
    }
    break;
  ...
  case 1: {
    err = SolveStraddleNewton(Fwd, T, DfDom, DfFor, VolStraddle, DeltaPercent,
                              IsForeign, Precision, StrikeStraddle);
    if (err) {
      return err;
    }
    err = SolveDeltaNewton(Fwd, T, DfDom, DfFor, 0.25, 1, Vol25Call,
                           DeltaPercent, IsForeign, Precision, Strike25Call);
    if (err) {
      return err;
    }
    err = SolveDeltaNewton(Fwd, T, DfDom, DfFor, -0.25, 0, Vol25Put,
                           DeltaPercent, IsForeign, Precision, Strike25Put);
    if (err) {
      return err;
    }
    break;
  ...
  case 2: {
    err = SolveStraddleNewton(Fwd, T, DfDom, DfFor, VolStraddle, DeltaPIPS,
                              IsForeign, Precision, StrikeStraddle);
    if (err) {
      return err;
    }
    err = SolveDeltaNewton(Fwd, T, DfDom, DfFor, 0.25, 1, Vol25Call, DeltaPIPS,
                           IsForeign, Precision, Strike25Call);
    if (err) {
      return err;
    }
    err = SolveDeltaNewton(Fwd, T, DfDom, DfFor, -0.25, 0, Vol25Put, DeltaPIPS,
                           IsForeign, Precision, Strike25Put);
    if (err) {
      return err;
    }
    break;
...
Err Strikes_to_SABR(double Fwd, double T, double DfDom, double DfFor,
  ...
  err = FindStrikes(Fwd, T, DfDom, DfFor, DeltaType, IsForeign, Precision,
                    Vol25Put, VolStraddle, Vol25Call, Strike25Put,
                    StrikeStraddle, Strike25Call);
  ...
  *volATM = VolStraddle;
  ...
  err =
      opsabrcalib(Fwd, T, 3, strike_sabr, vol_sabr, volATM, Alpha, FreezeAlpha,
                  Beta, FreezeBeta, Rho, FreezeRho, fitting_error);
...
Err BarrierOption_autocal(
    ...
    int is_digital,  /* 1: digital payoff        , 0        , regular option
                        payoff */
  ...
  double *bar_lvl_up = NULL, *bar_lvl_down = NULL, *prod_val = NULL,
         *prod_val_ki = NULL, *greeks_ki = NULL;

  long exercise_date_temp, date_temp;
  double cal_fwd, cal_mat, cal_dfdom, cal_dffor, cal_strike1, cal_strike2,
      cal_strike3, cal_sigma, cal_vol1, cal_vol2, cal_vol3, cal_volATM;
  ...
  spot_date = add_unit(today, 2, SRT_BDAY, MODIFIED_SUCCEEDING);
  cash_fx = spot_fx * swp_f_df(today, spot_date, dom_yc) /
            swp_f_df(today, spot_date, for_yc);
      ...
      *price = 0.0;
      ...
      if (today + eod_pay_flag > ex_date_set) {
        ...
        if (is_cvx) {
          /*	choto wrong */
          if (is_call) {
            *price = (1.0 / fwd_fx - strike) *
                     swp_f_df(today, ex_date_set, dom_yc) * notional;
          } else {
            *price = (strike - 1.0 / fwd_fx) *
                     swp_f_df(today, ex_date_set, dom_yc) * notional;
          }
        } else {
          if (is_call) {
            *price = (fwd_fx - strike) * swp_f_df(today, ex_date_set, dom_yc) *
                     notional;
          } else {
            *price = (strike - fwd_fx) * swp_f_df(today, ex_date_set, dom_yc) *
                     notional;
          }
        }
      }
      ...
      if (today + eod_pay_flag > ex_date_set) {
        ...
        *price = 0.0;
        ...
        if (has_knocked_up) {
          ...
          *price =
              rebate_down * swp_f_df(today, ex_date_set, dom_yc) * notional;
        }
      }
    } else {
      err = BarrierOption_autocal(
          today, dom_yc, for_yc, spot_fx, notional, exercise_date, settlmt_date,
          strike, 1000000 * spot_fx, -1000000 * spot_fx, rebate_up, rebate_down,
          is_call, 1, is_cvx, is_digital, is_american, mdl_alpha, mdl_beta,
          mdl_rho, mdl_lambda, mdl_gamma, mdl_floor, opt_matdates, opt_atmvols,
          nb_opt, calib_all_ts, do_smile_calib, cal_stochvol, cal_maturity,
          cal_alpha, cal_beta, cal_rho, pr_nstept, pr_nstepfx, pr_nstepvol,
          cal_nstept, cal_nstepfx, cal_nstepvol, cal_atmmaxiter, cal_atmprec,
          cal_smilemaxiter, cal_smileprec, cal_usetotalts, eod_fix_flag,
          eod_pay_flag, eod_ex_flag, exercised, 0, has_knocked_up, ex_date_ex,
          ex_date_set, spot_fx_fixing, price, greeks, export_ts, use_old_ts,
          calib_und);
    }
  ...
  if (!use_old_ts) {
    ...
    for (i = 0; i < nb_opt; i++) {
      exercise_date_temp =
          add_unit(opt_matdates[i], -2, SRT_BDAY, MODIFIED_SUCCEEDING);
    ...
    if (do_smile_calib) {
      ...
      cal_index = 0;
      while (fabs(opt_matdates[cal_index] - cal_maturity) > 1.0E-08 &&
             cal_index < nb_opt) {
        cal_index++;
      }

      if (cal_index == nb_opt) {
        err = serror("Maturity %d for calibration is not present in the market "
                     "ATM term struct",
                     cal_maturity);
        goto FREE_RETURN;
      }
      ...
      err = SABR_to_Strikes(cal_fwd, cal_mat, 1.0, 1.0, cal_sigma, cal_alpha,
                            cal_beta, cal_rho, SRT_LOGNORMAL, 0, 0, 0.0001,
                            &cal_strike1, &cal_strike2, &cal_strike3, &cal_vol1,
                            &cal_vol2, &cal_vol3, &cal_volATM);
    ...
    err = FxSabrQuadCalibSmile2(
        dom_yc, for_yc, today, spot_fx, &mdl_alpha, &mdl_gamma, &mdl_beta,
        &mdl_rho, mdl_lambda, mdl_floor, do_smile_calib, 1, cal_usetotalts,
        cal_stochvol, opt_exe, opt_mat, opt_atmvols, nb_opt, 0, cal_maturity,
        0.0, 0.0, cal_nstept, cal_nstepfx, cal_nstepvol, cal_atmmaxiter,
        cal_atmprec, cal_smilemaxiter, cal_smileprec, &cal_strike1, &cal_vol1,
        &cal_strike3, &cal_vol3, calib_ts);
    if (err) {
      goto FREE_RETURN;
    }
  } else {
  ...
  while (i < nstp) {
    if (index < nb_opt - 1) {
      /* middle part */
      coef = log(calib_ts[index + 1] / calib_ts[index]) /
             (opt_exe[index + 1] - opt_exe[index]);
  ...
  for (i = nstp - 1; i >= 0; i--) {
    ...
    date_temp =
        add_unit((long)(date[i] + 1.0E-08), 2, SRT_BDAY, MODIFIED_SUCCEEDING);
    bar_lvl_up[i] = barrier_up * swp_f_df(today, date_temp, dom_yc) /
                    swp_f_df(today, date_temp, for_yc);
    bar_lvl_down[i] = barrier_down * swp_f_df(today, date_temp, dom_yc) /
                      swp_f_df(today, date_temp, for_yc);
  }
  ...
  if (((cash_fx > barrier_up) || (cash_fx < barrier_down)) &&
      eod_fix_flag == 0) {
    prod_val[0] = 0.0;
  } else {
    err = FxSabrQuad_KOOption(
        nstp, time, date, pr_nstepfx, pr_nstepvol, calib_ts[0], drift,
        mdl_alpha, a, b, c, mdl_rho, mdl_lambda, mdl_floor, settlmt_date,
        strike, is_call, is_american, is_cvx, is_digital, bar_lvl_up,
        bar_lvl_down, rebate_up, rebate_down, cash_fx, dom_yc, for_yc,
        eod_fix_flag, eod_pay_flag, eod_ex_flag, prod_val, 1, greeks);
  }
  ...
  if (!is_ko) {
    ...
    err = FxSabrQuad_KOOption(
        ...
        mdl_alpha, a, b, c, mdl_rho, mdl_lambda, mdl_floor, settlmt_date,
        strike, is_call, is_american, is_cvx, is_digital, bar_lvl_up,
        bar_lvl_down, 0.0, 0.0, cash_fx, dom_yc, for_yc, eod_fix_flag,
        eod_pay_flag, eod_ex_flag, prod_val_ki, 1, greeks_ki);
...
Err FxSabrQuadSmilets(long today, double *sigma_time_fx, double *sigma_fx,
  ...
  *res = dvector(0, nb_strike - 1);
  ...
  err = FxSabrQuad_adi(nstp, time, date, nstpfx, nstpvol, sigma_fx[0], drift,
                       ...
                       payoff_fx_sabr_adi_opt, num_col, *res, 0, NULL, 0, 0, 0,
                       NULL, NULL);
  ...
  for (i = 0; i < nb_strike; i++) {
    err = srt_f_optimpvol((*res)[i], fwd_fx, strike[i], maturity, df, SRT_CALL,
                          SRT_LOGNORMAL, &bs_vol);
...
Err ConvexFwdSimple(
  ...
  if (strike < 1.0E-10) {
    ...
    if (today + eod_fix_flag > exercise_date &&
        today + eod_ex_flag > exercise_date) {
      /* Fx has been fixed and decision has been taken but deal is not paid yet
       */
      if (!exercised || spot_fx_fixing > barrier_up ||
          spot_fx_fixing < barrier_down) {
        *price = 0.0;
        *vol = 0.0;
        goto FREE_RETURN;
      } else {
  ...
  cash_fx = spot_fx * swp_f_df(today, spot_date, dom_yc) /
            swp_f_df(today, spot_date, for_yc);
  fwd_fx = cash_fx * swp_f_df(today, settlmt_date, for_yc) /
...
Err cvx_static_replication(double *strikes, long nb_strike, double *coefJ,
                           double *res) {
  int i;
  Err err = NULL;
  ...
  if (nb_strike == 1) {
    ...
    res[nb_strike - 1] = 0.0;
    res[nb_strike - 2] = 4.0 *
                         (strikes[nb_strike - 1] - strikes[nb_strike - 2]) /
                         strikes[nb_strike - 1] / strikes[nb_strike - 1];

    for (i = nb_strike - 3; i >= 0; i--) {
      res[i] =
          pow(res[i + 1] / (1.0 - sqrt(1 - res[i + 1] * strikes[i + 1])), 2) *
...
void init_static_logconvex(double mat, double df, double fwd,
                           double barrier_down, double barrier_up,
                           double log_vol, long nb_strike, double dig_spread,
...
Err static_logconvex(double param, double std[], double *price,
                     double *gradient, int nb_strike) {
  Err err = NULL;
  ...
  for (i = 0; i < nb_strike; i++) {
    ...
    logcvx_param->strikes[i] =
        logcvx_param->fwd * exp(sum * logcvx_param->log_std);
  }
  ...
  if (logcvx_param->barrier_up > 1.0E-10) {
    index_up = Get_Index(logcvx_param->barrier_up, logcvx_param->strikes,
                         logcvx_param->nb_strike);
  } else {
    index_up = logcvx_param->nb_strike - 1;
  }

  err = cvx_static_replication(logcvx_param->strikes, index_up + 1,
                               logcvx_param->coefJ, logcvx_param->coef);
  ...
  index_down = Get_Index(logcvx_param->barrier_down, logcvx_param->strikes,
                         index_up + 1);
  ...
  for (i = 0; i < index_up + 1; i++) {
    sum += logcvx_param->coef[i] *
           srt_f_optblksch(logcvx_param->fwd, logcvx_param->strikes[i],
                           logcvx_param->log_std, 1.0, logcvx_param->df,
                           SRT_PUT, PREMIUM);
  }
  ...
  if (logcvx_param->barrier_up > 1.0E-10) {
    ...
    sum += slope_up * srt_f_optblksch(logcvx_param->fwd, strike_up,
                                      logcvx_param->log_std, 1.0,
                                      logcvx_param->df, SRT_PUT, PREMIUM);
    ...
    sum += coef_up * srt_f_optblksch(logcvx_param->fwd, strike_up_up,
                                     logcvx_param->log_std, 1.0,
                                     logcvx_param->df, SRT_PUT, PREMIUM);

    sum -= coef_up * srt_f_optblksch(logcvx_param->fwd, strike_up_down,
                                     logcvx_param->log_std, 1.0,
                                     logcvx_param->df, SRT_PUT, PREMIUM);
  }
  ...
  if (logcvx_param->barrier_down > 1.0E-10) {
    ...
    /* remove option part */
    sum -= slope_down * srt_f_optblksch(logcvx_param->fwd, strike_down,
                                        logcvx_param->log_std, 1.0,
                                        logcvx_param->df, SRT_PUT, PREMIUM);
    ...
    sum -= coef_down * srt_f_optblksch(logcvx_param->fwd, strike_down_up,
                                       logcvx_param->log_std, 1.0,
                                       logcvx_param->df, SRT_PUT, PREMIUM);

    sum += coef_down * srt_f_optblksch(logcvx_param->fwd, strike_down_down,
                                       logcvx_param->log_std, 1.0,
                                       logcvx_param->df, SRT_PUT, PREMIUM);
  }
...
Err ConvexFwdSABR_static_replication(
    ...
    double *price, double *coef, double *vols,
    double **bar_adjust) /*	bar adjust is an array 5 * 3 */
{
  long spot_date, settlmt_fx_date;
  double cash_fx, fwd_fx, mat;
  double atm_vol, atm_std, sigma_beta, floor_vol, sum, df;
  double slope_down, const_down, strike_down, vol_down, strike_up, vol_up,
      slope_up, const_up;
  ...
  if (calib_strikes) {
    ...
    /* first we try to find the strikes which match as much as possible the log
     * price */
    ...
    if (sum < 1.0E-10 && nb_strike > 1) {
      ...
      for (i = 2; i <= nb_strike; i++) {
        param[i] =
            log(fwd_fx / strikes[i - 1]) / atm_vol / sqrt(mat) - strikes[i - 2];
      }
    } else {
    ...
    err = ConvexFwdSimple(today, dom_yc, for_yc, spot_fx, 1.0, exercise_date,
                          settlmt_date, 0.0, 0, barrier_up, barrier_down,
                          opt_matdates, opt_atmvols, nb_opt, 0, 0, 0, 0.0, 0,
                          &log_price, &log_vol);

    if (err)
      goto FREE_RETURN;

    init_static_logconvex(mat, df, fwd_fx, barrier_down, barrier_up, log_vol,
                          nb_strike, dig_spread, logcvx_param);
    ...
    err = levenberg_marquardt(data, target, weight, 1, param, nb_strike,
                              nb_iter, static_logconvex, &error);
  ...
  /* Get the sigma beta */
  err = srt_f_optsarbvol(fwd_fx, fwd_fx, mat, atm_vol, alpha, beta, rho,
                         SRT_LOGNORMAL, SRT_BETAVOL, &sigma_beta);

  if (err)
    goto FREE_RETURN;

  /* Calculate vol at floor */
  err = srt_f_optsarbvol(fwd_fx, floorvol_strike, mat, sigma_beta, alpha, beta,
                         rho, SRT_BETAVOL, SRT_LOGNORMAL, &floor_vol);
  ...
  for (i = 0; i < nb_strike; i++) {
    if (fabs(coef[i]) > 1.0E-10) {
      if (strikes[i] > floorvol_strike) {
        err = srt_f_optsarbvol(fwd_fx, strikes[i], mat, sigma_beta, alpha, beta,
                               rho, SRT_BETAVOL, SRT_LOGNORMAL, &(vols[i]));
      ...
      sum += coef[i] * srt_f_optblksch(fwd_fx, strikes[i], vols[i], mat, df,
                                       SRT_PUT, PREMIUM);
    }
  }
  ...
  if (barrier_up > 1.0E-10) {
    ...
    if (strike_up > floorvol_strike) {
      err = srt_f_optsarbvol(fwd_fx, strike_up, mat, sigma_beta, alpha, beta,
                             rho, SRT_BETAVOL, SRT_LOGNORMAL, &vol_up);

    } else {
      vol_up = floor_vol;
    }

    sum += digit_up * srt_f_optblksch(fwd_fx, strike_up, vol_up, mat, df,
                                      SRT_PUT, PREMIUM);
    ...
    if (strike_up_down > floorvol_strike) {
      err =
          srt_f_optsarbvol(fwd_fx, strike_up_down, mat, sigma_beta, alpha, beta,
                           rho, SRT_BETAVOL, SRT_LOGNORMAL, &vol_up_down);
    } else {
      vol_up_down = floor_vol;
    }

    sum -= digit_up * srt_f_optblksch(fwd_fx, strike_up_down, vol_up_down, mat,
                                      df, SRT_PUT, PREMIUM);
  ...
  if (barrier_down > 1.0E-10) {
    ...
    if (strike_down > floorvol_strike) {
      err = srt_f_optsarbvol(fwd_fx, strike_down, mat, sigma_beta, alpha, beta,
                             rho, SRT_BETAVOL, SRT_LOGNORMAL, &vol_down);
    } else {
      vol_down = floor_vol;
    }

    sum -= slope_down * srt_f_optblksch(fwd_fx, strike_down, vol_down, mat, df,
                                        SRT_PUT, PREMIUM);

    /* remove digital part */
    strike_down_up = strike_down + dig_spread;

    sum += digit_down * srt_f_optblksch(fwd_fx, strike_down, vol_down, mat, df,
                                        SRT_PUT, PREMIUM);

    if (strike_down_up > floorvol_strike) {
      err =
          srt_f_optsarbvol(fwd_fx, strike_down_up, mat, sigma_beta, alpha, beta,
                           rho, SRT_BETAVOL, SRT_LOGNORMAL, &vol_down_up);

    } else {
      vol_down_up = floor_vol;
    }

    sum -= digit_down * srt_f_optblksch(fwd_fx, strike_down_up, vol_down_up,
...
Err fwd_fxoption(
  ...
  double *fx_vol_curve = NULL, *sigma_date_dom = NULL, *sigma_dom = NULL,
         *sigma_date_for = NULL, *sigma_for = NULL, *merge_dates = NULL,
         *sig_dom = NULL, *sig_for = NULL, *opt_exe = NULL, *opt_mat = NULL;
  ...
  if (today + eod_ex_flag > exercise_date &&
      today + eod_fix_flag > fx_fixing_date) {
    /* Fx has been fixed and decision has been taken but deal is not paid yet */
    if (!exercised) {
      *price = 0.0;
      *vol = 0.0;
  ...
  opt_date_lim =
      add_unit(spot_date, 2 * call_nb_max, SRT_MONTH, MODIFIED_SUCCEEDING);
  ...
  err = cpd_calib_diagonal(dom_yc, dom_vc, dom_ref, get_cash_vol, dom_vol_shift,
                           1, num_calib_rates, ex_date, maturity_calib, NULL,
                           NULL, 0, 1.0, 1.0, dom_swap_freq, dom_swap_basis,
                           !calib_dom_tau, 0, 0, CALPRES, CALPRES, 0, 0, 0, 0,
                           NULL, &dom_lam, 1, 0.0, 0.0, 0.0, &sigma_n_dom,
                           &sigma_date_dom, &sigma_dom, NULL);

  if (err) {
    goto FREE_RETURN;
  }

  err = cpd_calib_diagonal(for_yc, for_vc, for_ref, get_cash_vol, for_vol_shift,
                           1, num_calib_rates, ex_date, maturity_calib, NULL,
                           NULL, 0, 1.0, 1.0, for_swap_freq, for_swap_basis,
                           !calib_for_tau, 0, 0, CALPRES, CALPRES, 0, 0, 0, 0,
                           NULL, &for_lam, 1, 0.0, 0.0, 0.0, &sigma_n_for,
                           &sigma_date_for, &sigma_for, NULL);
  ...
  err = Fx3DtsCalibration_corr(
      opt_exe, opt_mat, fx_mkt_vol, num_calib, merge_dates, nb_merge_dates,
      sig_dom, dom_lam, sig_for, for_lam, corr_times, correl_dom_for,
      correl_dom_fx, correl_for_fx, corr_n_times, &fx_vol_curve);
  ...
  err = Fx3DtsImpliedVol_corr(
      opt_maturity, 0.0, exe_maturity, merge_dates, nb_merge_dates, sig_dom,
      dom_lam, sig_for, for_lam, opt_exe, fx_vol_curve, num_calib, corr_times,
      correl_dom_for, correl_dom_fx, correl_for_fx, corr_n_times, vol);
  ...
  *price = notional * srt_f_optblksch(fwd_fx, strike, *vol, exe_maturity, df,
                                      CallPutFlag, PREMIUM);

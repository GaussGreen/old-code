 ...
 *--------------------------------------------------------------------*
 *    Author: Adam Litke        , Rishin Roy        , Eric Auld * Date: 10/02/95
 **
 *--------------------------------------------------------------------*
 *    Inputs:                                                         *
 *   Returns:                                                         *
 ...
 * Date     Inits   Comments                                          *
 * dd/mm/yy                                                           *
 * 10/02/95 AL      Started from acc_addin.cxx
 **
 **********************************************************************/
...
/* float_flg determines if we are going to receive x or k (if we are going
  to receive x        , we have to make a change of numeraire so there will be
  an adjustment); reset_flg determines whether or not the band is resettable; if
  it is then receiving will be contingent on y>x+k (for a cap); we value this by
  making a margrabe type assumption that x+k or y-k is lognormal.
*/

/*
get_d2(fwdy        ,fwdx        ,k        ,fvol        ,vol        ,float_set
,y_set        ,today        ,correlation ,float_tenor        , float_flg
,reset_flg);
*/
...
INPUT KEY:
  ...
  Date float_set--if you are receiving the floating rate        , when it is
fixed double correlation--correlation between floating rate you receive and
libor double float_tenor--length in months of floating rate you receive
(ifyoudo) Date *bus_day_array--array of all business days double
*rate_array--fixings and forward rates corresponding to bus_day_array int
ra_len--length of prev two arrays double *vol_date_array--maturities for which
we know vols double *vol_array--1d array of vols corresp. to
vol_date_array(maybe NULL) double *strikes_vec--strikes for which we know
vols(maybe NULL) double **mkt_vol_2darray--matrix of vols across mat and
strike(maybe NULL) long m_mat_dates--num of vol dates long m_strikes--num of vol
strikes int float_flg--1 if a floating rate is accruing        , 0 if a fixed
rate. Note that if a floating rate is accruing        , we make a change of
...
means we are going to look at mkt_vol_2darray; if 0 we are going to look at
  ...
  int FHLB_flg--1 this routine knows two rules for business days.
    1:{Friday        , Saturday         ,Sunday} are all considered business
days accruing off of Friday's Rate 0:we don't count Saturday and Sunday. double
*answer--answer = expected fraction of accruing days/business days
************************************************************************/
...
SrtErr srt_f_rngfltr_resettable(
  ...
  if (today > float_set) {
    ...
    *answer = val2;
    ...
    MINIMIZE THE NUMBER OF DAYS OUTSIDE THE BAND        , WHICH IS WHAT
    srt_f_rngfltr COMPUTES.*/

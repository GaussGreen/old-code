...
/* When you want to shift the bucket defined by dates d1 and d2        , you
really want to move the the sigma or tau given at the date d2. i.e. sigma(d2) is
correct between d1 and d2. If you want to shift sigma(d2)        , you don't
want to shift what is before d1. -> you don't want to shift d1. To do so , we
define the bucket by date d1+epsilon and d2 */
...
Err srt_f_readrequest(String *requests, int *buckets, int nr, TermStruct ts,
  ...
  for (j = 0; j < nr; j++) {
    ...
    if (strcmp(requests[j], "VEGA") == 0) {
      ...
      /* we create a first default request - since no shift value has been
       * specified by the user */
      /* default shift = +0.01 */

      if (err = srt_f_IOstructsetsigmashift(iolist, buckets[j], date1, date2,
                                            0.01, SH_ABSOLUTE, SRT_NO, 0, "")) {
        return err;
      }
    ...
    if (strcmp(requests[j], "TAU") == 0) {
      ...
      /* we create a default request - since no shift value has been specified
       * by the user */
      /* default shift = +1 */

      if (err = srt_f_IOstructsettaushift(iolist, buckets[j], date1, date2,
                                          0.01, SH_ABSOLUTE, SRT_NO, 0, "")) {
        return err;
      }
    ...
    if (strcmp(requests[j], "PARALLEL") == 0) {
      ...
      if (err = srt_f_IOstructsetrshift(iolist, "PARALLEL", 0.01, SH_ABSOLUTE,
                                        SRT_NO, 0, "", NULL)) {
        return err;
      }
      /* we create a rate request for a parallel shift whose shift is defined by
       * the user */
    }
...
Err srt_f_readresult(String greek, int bucket, double *ret_val) {
  ...
  if (!strcmp(greek, "PREMIUM")) {
    ...
    if (err = srt_f_IOstructgetsigmashiftval(*iolist, bucket, 0.01, SH_ABSOLUTE,
                                             &vega1)) {
      return err;
    }
    ...
    if (err = srt_f_IOstructgettaushiftval(*iolist, bucket, 0.01, SH_ABSOLUTE,
                                           &tau1)) {
      return err;
    }
...
Err srt_f_sig_ts_dates(int bucket_index, Date *date1, Date *date2,
                       TermStruct ts) {
  Err err = NULL;
  ...
  *****/
  ...
  for (ls = get_next_sigma_date(ts.head); ls != NULL;
  ...
  default:
    ...
    if (index == index_max + 1) {
      for (ls = get_next_sigma_date(ts.head), i = index_max - 1;
           ((ls != NULL) && (i > 0)); ls = get_next_sigma_date(ls), i--) {
        if (ls != NULL)
          ls = ls->next;
      }
      /* the last sigma date */
      *date1 =
          (Date)(((IrmTermStructVal *)(ls->element->val.pval))->date + 0.01);
    ...
    /* for the other buckets */
      ...
      for (ls = get_next_sigma_date(ts.head), i = index - 1;
           ((ls != NULL) && (i > 0)); ls = get_next_sigma_date(ls), i--) {
        if (ls != NULL)
          ls = ls->next;
      }
      *date1 =
          (Date)(((IrmTermStructVal *)(ls->previous->element->val.pval))->date +
                 0.01);
      *date2 = (Date)((IrmTermStructVal *)ls->element->val.pval)->date;
    }
    break;
...
Err srt_f_tau_ts_dates(int bucket_index, Date *date1, Date *date2,
                       TermStruct ts) {
  Err err = NULL;
  int index = bucket_index;
  int index_max = 0;
  ...
  *****/
  ...
  for (ls = get_next_tau_date(ts.head); ls != NULL;
  ...
  default:
    ...
    /* for the last bucket */
    if (index == index_max + 1) {
      for (ls = get_next_tau_date(ts.head), i = index_max - 1;
           ((ls != NULL) && (i > 0)); ls = get_next_tau_date(ls), i--) {
        if (ls != NULL)
          ls = ls->next;
      }
      /* the last tau date */
    ...
    /* for the other buckets */
      ...
      for (ls = get_next_tau_date(ts.head), i = index - 1;
           ((ls != NULL) && (i > 0)); ls = get_next_tau_date(ls), i--) {
        if (ls != NULL)
          ls = ls->next;
      }

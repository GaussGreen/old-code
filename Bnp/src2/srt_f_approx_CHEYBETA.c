...
typedef Err (*pricing_caplet)(
    Ddate, Ddate, Ddate, char*, char*, String, String, char*, double, double, double*);

typedef Err (*find_t_star)(
    Ddate, Ddate, Ddate, double, char*, char*, char*, String, String, char*, double*);
...
Err srt_f_beta_function(Ddate current_time, Ddate maturity, char* und_name, double* beta_t_T)
{
    Err         err;
    SrtUndPtr   und;
    TermStruct* ts;
...

{
    ...
    SrtCrvPtr    yldcrv;
    SrtBasisCode val;
    double       fra_coverage, df_fixing, df_end, beta_t_fixing, beta_t_end, IFR, spot_date;
    ...
    IFR_maturity = add_unit((Date)current_time, (long)IFR_epsilon, SRT_DAY, MODIFIED_SUCCEEDING);

    IFR = swp_f_fwdcash(current_time, IFR_maturity, val, yc_name);

    fra_coverage = coverage((Date)fixing_date, (Date)end_date, val);

    err = srt_f_beta_function(current_time, fixing_date, und_name, &beta_t_fixing);
    if (err)
    {
        return err;
    }
    ...
    *ans = (df_fixing / (fra_coverage * df_end)) *
               exp(0.5 * phi_t * (beta_t_end * beta_t_end - beta_t_fixing * beta_t_fixing) -
                   r_t * (beta_t_end - beta_t_fixing) + IFR * (beta_t_end - beta_t_fixing)) -
           1.0 / fra_coverage;
...
Err srt_f_bissection(
    pricing_caplet srt_f_caplet_function,
    Ddate          fixing_date,
    Ddate          end_date,
    double         strike,
    char*          cap_swaption,
    ...
    double* tau_m)
{
    ...
    *tau_m = 0.0;
    ...
    for (i = 1; i <= N; i++)
    {
        ...
        current_time = add_unit((Date)today, (long)temp, SRT_DAY, MODIFIED_SUCCEEDING);
...
Err srt_f_t_star_bissection(
    find_t_star srt_f_time_rescaling,
    Ddate       fixing_date,
    Ddate       end_date,
    double      strike,
    char*       cap_swaption,
    ...
    double* price)
{
    ...
    double fra, today, fra_coverage, df, beta, tau_m, X0, x_star, r_star, phi_star, K_star,
        first_der, second_der, ter_der, ordre_0, ordre_1, ordre_2, maturity, sigma_bs;

...
typedef Err(*find_t_star)(Ddate,Ddate,Ddate,double,char*,char*,char*,String,String,
					  char*,double*);

/*----------------------------------------------------------------------------------*/

/*-------------------------COMPUTES THE MID_POINT IN THE APPROX---------------------*/
						 ...
						 double *beta_t_T)
{
	...
	{
		*beta_t_T = (Psi_func(time1,ts)-Psi_func(time2,ts))/
		         F_func(time2,ts);
	}
...
/*-------------------------------CAPLET FUNCTION-----------------------------------*/
/* in the function, current time is the maturity of the IFR, marutity is set 14 days
   after */

Err srt_f_caplet_function(
						   Ddate   current_time,
						   Ddate   fixing_date,
						   Ddate   end_date,
						   char    *und_name,
...

{
	...
	df_fixing = swp_f_df( (Ddate)today,
		                  (Ddate)fixing_date,
						  yc_name);
	...
	IFR_maturity = add_unit( (Date)current_time,
		                     (long)IFR_epsilon,
							 SRT_DAY,
							 MODIFIED_SUCCEEDING);
	...
	fra_coverage = coverage( (Date)fixing_date,
		                     (Date)end_date,
						      val);
	...
	*ans = (df_fixing/(fra_coverage*df_end))
		    *exp(0.5*phi_t*(beta_t_end*beta_t_end-beta_t_fixing*beta_t_fixing)
			     -r_t*(beta_t_end-beta_t_fixing)
				 +IFR*(beta_t_end-beta_t_fixing))-1.0/fra_coverage;
...
/*-----------------------------------------------------------------------------------*/
/*-------------------FIRST & SECOND DERIVATIVES OF THE CAPLET FUNCTION---------------*/
...
Err srt_f_caplet_derivative_fct(
						              ...
						              double  phi_t,
						              double  r_t,
									  SrtDerType der_order,
						              double  *ans
						              )
{
	Err err;
	double beta_t_fixing,
		   beta_t_end,
		   rf_fra,
		   fra_coverage;
	...
	err = srt_f_caplet_function(
						         current_time,fixing_date,end_date,
						         und_name,yc_name,compStr,refRateCode,
						         basis_code,phi_t,r_t,&rf_fra
								 );
	if(err)
	{
		return err;
	...
	{
		return serror("Unknown derivatives order in srt_f_derivatives_caplet_function");
	}
	return NULL;
							 ...
							 )
{
	...
	*ans_phi = 0.5*pow((r),(2.0*beta))*F_func(time,ts)*F_func(time,ts)*val_G;
	
	return NULL;
}
/*---------------------------BISSECTION METHOD USED TO SEARCH MID_STATE_VAR----------*/

Err srt_f_bissection( pricing_caplet srt_f_caplet_function,
					  Ddate   fixing_date,
				      Ddate   end_date,
					  double  strike,
					  char    *cap_swaption,
...
double today,
	...
	err = srt_f_mid_point(fixing_date,end_date,strike,cap_swaption,compStr,refRateCode,
		                  basis_code,yc_name,&x_star);
	if(err)
	{
		return err;
	}
	...
	err = srt_f_phi_function_of_r(
						           fixing_date,end_date,strike,
							       cap_swaption,compStr,refRateCode,
							       basis_code,yc_name,und_name,x1,&phi_f_r);
	if(err)
	{
		return err;
	}

	err = srt_f_caplet_function( (Ddate)today,fixing_date,end_date,
									und_name,yc_name,compStr,refRateCode,
									basis_code,phi_f_r,x1,&f);
	if(err)
	{
		return err;
	}
	...
	err = srt_f_phi_function_of_r(
						           fixing_date,end_date,strike,cap_swaption,
							       compStr,refRateCode,basis_code,yc_name,und_name,
							       x2,&phi_f_r);
	if(err)
	{
		return err;
									  ...
									  double *phi_star)
{
	...
	err = srt_f_bissection( srt_f_caplet_function,
					        fixing_date,
				            end_date,
					        strike,
					        cap_swaption,
...
/*-----------------------------------------------------------------------------------*/
/*-------------------------COMPUTES THE MID VOLATILITY at current time----------------*/

Err srt_f_mid_volatility( Ddate   current_time,
						  Ddate   fixing_date,
						  Ddate   end_date,
				          double  strike,
			              char    *cap_swaption,
						  ...
						  double  *sigma)
{
	...
	err = srt_f_caplet_derivative_fct( current_time,
		                                    fixing_date,
											end_date,
											und_name,
											yc_name,
...
/*-----------------------------------------------------------------------------------*
/*-----------------------------tau_m: time_rescaling---------------------------------*/
Err srt_f_time_rescaling( Ddate   final_time,
						  Ddate   fixing_date,
						  Ddate   end_date,
				          double  strike,
			              char    *cap_swaption,
						  ...
						  double  *tau_m)
{
	...
	*tau_m = 0.0;
	...
	err = srt_f_mid_state_variables(fixing_date,end_date,strike,cap_swaption,und_name,
		               yc_name,compStr,refRateCode,basis_code,&r_star,&phi_star);
	if(err)
	{
		return err;
	}
	...
	for(i=1;i<=N;i++)
	{
		...
		current_time = add_unit( (Date)today,
			                     (long)temp,
								 SRT_DAY,
								 MODIFIED_SUCCEEDING);
...
/*-----------------------------------------------------------------------------------*/
/*---------------------------------Bissection method for t_star----------------------*/
Err srt_f_t_star_bissection( find_t_star srt_f_time_rescaling,
							 Ddate   fixing_date,
			                 Ddate   end_date,
				             double  strike,
			                 char    *cap_swaption,
				             ...
				             Ddate  *t_star)
{
	...
	err = srt_f_mid_state_variables(fixing_date,end_date,strike,cap_swaption,
		                             und_name,yc_name,compStr,refRateCode,basis_code,
									 &r_star,&phi_star);
	if(err)
	{
		return err;
	...
	f = 0.0;
	/*err = srt_f_time_rescaling(t1,fixing_date,end_date,strike,cap_swaption,
		                        und_name,yc_name,compStr,refRateCode,basis_code,&f);
	if(err)
	{
		return err;
	}*/
	...
	err = srt_f_time_rescaling(t2,fixing_date,end_date,strike,cap_swaption,
		                        und_name,yc_name,compStr,refRateCode,basis_code,&fmid);
	if(err)
	{
		return err;
	}
	...
	if(f*fmid>=0.0)
	{
		return serror("Root must be bracketed in function srt_f__t_star_bissection");
	}
	...
	while(i<=MAX_ITER && fabs(dt) >= 1.0 && fmid != 0.0 )
	{
		dt *= 0.5;
		tmid = *t_star + dt;
		err = srt_f_time_rescaling((Ddate)tmid,fixing_date,end_date,strike,cap_swaption,
		                        und_name,yc_name,compStr,refRateCode,basis_code,&fmid);
				  ...
				  Ddate  *t_star)
{
	...
	err = srt_f_t_star_bissection(srt_f_time_rescaling,fixing_date,end_date,strike,
		                          cap_swaption,und_name,yc_name,compStr,refRateCode,
								  basis_code,t_star);
	if(err)
	{
		return err;
...
Err srt_f_analytical_price( Ddate   fixing_date,
			                Ddate   end_date,
				            double  strike,
			                char    *cap_swaption,
				            char    *und_name,
				            ...
				            double  *price)
{
	...
	err = srt_f_time_rescaling(fixing_date,fixing_date,end_date,strike,cap_swaption,
		                       und_name,yc_name,compStr,refRateCode,basis_code,&tau_m);
	if(err)
	{
		return err;
	}
	...
	err = srt_f_t_star(fixing_date,end_date,strike,cap_swaption,
		                             und_name,yc_name,compStr,refRateCode,basis_code,
									 &t_star);
	if(err)
	{
	...
	err = srt_f_caplet_derivative_fct(t_star,fixing_date,end_date,und_name,
		                             yc_name,compStr,refRateCode,basis_code,phi_star,
									 r_star,SRT_FIRSTDER,&first_der);
	if(err)
	{
		return err;
	}
	...
	err = srt_f_caplet_derivative_fct(t_star,fixing_date,end_date,und_name,
		                             yc_name,compStr,refRateCode,basis_code,phi_star,
									 r_star,SRT_SECONDDER,&second_der);
	if(err)
	{
		return err;
	}

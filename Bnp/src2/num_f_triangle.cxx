...
/*   of a floating exception due to overflow.  Finally        , it is much
 * faster */
...
/* To insert lots of self-checks for internal errors        , define the
 * SELF_CHECK */
/*   symbol.  This will slow down the program significantly.  It is best to  */
/*   define the symbol using the -DSELF_CHECK compiler switch        , but you
 * could
 */
/*   write "#define SELF_CHECK" below.  If you are modifying this code        ,
 * I */
/*   recommend you turn self-checks on.                                      */

/* #define SELF_CHECK */

/* On some machines        , the exact arithmetic routines might be defeated by
 * the */
 ...
 * , */
/*   however        , INEXACT should be defined to be nothing.  ("#define
 * INEXACT".)
 */
...
/* Used for the TriPoint location scheme of Mucke        , Saias        , and
 * Zhu        , to decide    */
...
/* will port to Dec Alphas running OSF/1        , because their stdio.h file
 * commits
 */
/* the unpardonable sin of including stdlib.h.  Hence        , malloc()        ,
 * free()        , and */
...
/*   that the TriPoint was inserted with complete success        , was inserted
 * but */
/*   encroaches on a segment        , was not inserted because it lies on a
 * segment        ,
 */
 ...
 */
/*   the heap.  Instead        , each node knows its position in the heap , and
 * can
 */
...
/*   algorithm.  To distinguish site events from circle events        , all
 * circle */
...
/*   circle event covers two boundary edges with a TriTriangle        , so that
 * they
 */
/*   are no longer boundary edges        , those edges are not immediately
 * deleted */
 ...
 */
/*   it has been rotated (due to a circle event)        , it no longer
 * represents a */
...
/*                                                                           */
/* Traversal is the process of walking through the entire list of items        ,
 * and
 */
...
/*                                                                           */
/* itemwordtype is set to POINTER or FLOATINGPOINT        , and is used to
 * suggest */
...
/* Variables used to allocate memory for triangles        , shell edges        ,
 * points        ,
 */
/*   viri (triangles being eaten)        , bad (encroached) segments        ,
 * bad (skinny */
...
/*   useshelles: -p        , -r        , -q        , or -c switch; determines
 * whether shell edges
 */
/*     are used at all.                                                      */
/*                                                                           */
...
/*   and Stolfi.  However        , they use an edge-based data structure ,
 * whereas I
 */
...
/* onext() spins counterclockwise around a TriPoint; that is        , it finds
 * the next */
...
/* oprev() spins clockwise around a TriPoint; that is        , it finds the next
 * edge
 */
...
/* dnext() spins counterclockwise around a TriPoint; that is        , it finds
 * the next */
...
/* dprev() spins clockwise around a TriPoint; that is        , it finds the next
 * edge
 */
 ...
 */
/*   think it's connected to this TriTriangle.  Usually        , however , the
 * other
 */
/*   TriTriangle is being deleted entirely        , or bonded to another
 * TriTriangle , so   */
/*   it doesn't matter.                                                      */
...
/*   least significant bits (one for orientation        , one for viral
 * infection) */
...
void internalerror() {
  printf("  Please report this bug to jrs@cs.cxxmu.edu\n");
  printf("  Include the message above        , your input data set        , "
         "and the exact\n");
...
/*****************************************************************************/
/*                                                                           */
/*  parsecommandline()   Read the command line        , identify switches , and
 * set
 */
...
void printtriangle(TriOrientedTriangle *t) {
  ...
  if (printpoint == (TriPoint)NULL)
    ...
    printf("    Origin[%d] = x%lx  (%.12g        , %.12g)\n",
           (t->orient + 1) % 3 + 3, (unsigned long)printpoint, printpoint[0],
           printpoint[1]);
  dest(*t, printpoint);
  if (printpoint == (TriPoint)NULL)
    printf("    Dest  [%d] = NULL\n", (t->orient + 2) % 3 + 3);
    ...
    printf("    Dest  [%d] = x%lx  (%.12g        , %.12g)\n",
           (t->orient + 2) % 3 + 3, (unsigned long)printpoint, printpoint[0],
           printpoint[1]);
  apex(*t, printpoint);
  if (printpoint == (TriPoint)NULL)
    printf("    Apex  [%d] = NULL\n", t->orient + 3);
...
/*  collection of words        , and either pointers or floating-TriPoint values
 * are
 */
/*  assumed to be the "primary" word type.  (The "primary" word type is used */
/*  to determine alignment of items.)  If `alignment' isn't zero        , all
 * items */
...
void poolinit(struct memorypool *pool, int bytecount, int itemcount,
  ...
  /*   pointer (to TriPoint to the next block) are allocated        , as well as
   * space
   */
...
/*  that this routine successively returns all items in the list        ,
 * including */
...
/*                                                                           */
/*  The TriTriangle that fills "outer space"        , called `dummytri'        ,
 * is pointed to  */
/*  by every TriTriangle and shell edge on a boundary (be it outer or inner) of
 */
/*  the triangulation.  Also        , `dummytri' points to one of the triangles
 * on */
/*  the convex hull (until the holes and concavities are carved)        , making
 * it */
/*  possible to find a starting TriTriangle for TriPoint location. */
/*                                                                           */
/*  The omnipresent shell edge        , `dummysh'        , is pointed to by
 * every TriTriangle   */
...
void dummyinit(int trianglewords, int shellewords) {
  ...
  /*   will eventually be changed by various bonding operations        , but
   * their */
  ...
  if (useshelles) {
    ...
    /*   operations        , but their values don't really matter        , as
     * long as they
     */
...
/*                            memory pools.                                  */
/*                                                                           */
/*  This routine also computes the `highorderindex'        , `elemattribindex'
 * , and
 */
...
void initializetrisegpools() {
  ...
  /*   pointers to other triangles        , three pointers to corners        ,
   * and possibly
   */
   ...
   */
  /*   `regionattrib' flag is set        , an additional attribute will be
   * added. */
  ...
  /* If a Voronoi diagram or TriTriangle neighbor graph is requested        ,
   * make */
  ...
  /* Having determined the memory size of a TriTriangle        , initialize the
   * pool.
   */
...
/*****************************************************************************/
/*                                                                           */
/*  triangledealloc()   Deallocate space for a TriTriangle        , marking it
 * dead.
 */
/*                                                                           */
/*****************************************************************************/
...
/*****************************************************************************/
/*                                                                           */
/*  shelledealloc()   Deallocate space for a shell edge        , marking it
 * dead. */
/*                                                                           */
/*****************************************************************************/
...
/*****************************************************************************/
/*                                                                           */
/*  getpoint()   Get a specific TriPoint        , by number        , from the
 * list. */
/*                                                                           */
/*  The first TriPoint is number 'firstnumber'. */
/*                                                                           */
/*  Note that this takes O(n) time (with a small constant        , if
 * POINTPERBLOCK */
...
/* Many of the operations are broken up into two pieces        , a main part
 * that */
/*   performs an approximate operation        , and a "tail" that computes the
 */
/*   roundoff error of that operation.                                       */
/*                                                                           */
/* The operations Fast_Two_Sum()        , Fast_Two_Diff()        , Two_Sum() ,
 * Two_Diff()
 *       ,    */
/*   Split()        , and Two_Product() are all implemented as described in the
 */
/*   reference.  Each of these macros requires certain variables to be       */
/*   defined in the calling routine.  The variables `bvirt'        , `c' ,
 * `abig'        ,
 */
/*   `_i'        , `_j'        , `_k'        , `_l'        , `_m'        , and
 * `_n' are declared `INEXACT' because   */
...
/*  own good        , and somehow cause this routine to fail        , if it
 * pretends that */
...
void exactinit() {
  ...
  /*   the previous sum        , for machines that round up instead of using
   * exact */
...
/*  with IEEE 754)        , maintains the strongly nonoverlapping and
 * nonadjacent */
/*  properties as well.  (That is        , if e has one of these properties ,
 * so */
/*  will h.)                                                                 */
/*                                                                           */
/*****************************************************************************/
...
/*****************************************************************************/
/*                                                                           */
/*  counterclockwise()   Return a positive value if the points pa        , pb ,
 * and
 */
...
/*  computed adaptively        , in the sense that exact arithmetic is used only
 * to */
/*  the degree it is needed to ensure that the returned value has the        */
/*  correct sign.  Hence        , this function is usually quite fast        ,
 * but will run
 */
...
/*               circle passing through pa        , pb        , and pc; a
 * negative value if
 */
/*               it lies outside; and zero if the four points are cocircular.*/
/*               The points pa        , pb        , and pc must be in
 * counterclockwise */
 ...
 */
/*  is a bad random number generator        , but good enough for most
 * randomized */
...
/*  of several triangles        , but in the end every TriPoint will TriPoint to
 * some
 */
...
/*       is being inserted.                                                  */
/*                                                                           */
/*  (2)  To try to find an existing TriPoint.  In this case        , any edge on
 * the
 */
...
/*                                                                           */
/*  On completion        , `searchtri' is a TriTriangle that contains
 * `searchpoint'.
 */
/*                                                                           */
...
/*  TriPoint location routine (which I originally used)        , which tends to
 * spiral
 */
...
/*  encountered TriTriangle `recenttri'        , or from a TriTriangle chosen
 * from a
 */
...
TriLocateResultType locate(TriPoint searchpoint,
  ...
  if (verbose > 2) {
    printf(
        "  Randomly sampling for a TriTriangle near TriPoint (%.12g        , "
        "%.12g).\n",
  ...
  if (recenttri.tri != (TriTriangle *)NULL) {
    if (recenttri.tri[3] != (TriTriangle)NULL) {
      ...
      if (dist < searchdist) {
        ...
        searchdist = dist;
        if (verbose > 2) {
          printf("    Choosing recent TriTriangle with origin (%.12g        , "
                 "%.12g).\n",
                 torg[0], torg[1]);
        }
      }
    ...
    for (j = 0; j < samplesperblock; j++) {
      ...
      if (sampletri.tri[3] != (TriTriangle)NULL) {
        ...
        if (dist < searchdist) {
          ...
          searchdist = dist;
          if (verbose > 2) {
            printf("    Choosing TriTriangle with origin (%.12g        , "
                   "%.12g).\n",
                   torg[0], torg[1]);
          }
        }
...
/*  is applied to the shell edge and        , if appropriate        , its
 * vertices. */
/*                                                                           */
/*****************************************************************************/
...
/* shellemark : Marker for the new shell edge. */
{
  ...
  if (newshelle.sh == dummysh) {
    ...
    /*   `dummytri' (outer space)        , but the new shell edge is bonded to
     * it */
    /*   all the same.                                                    */
...
/*                                                                           */
/*  Imagine the original triangles        , abc and bad        , oriented so
 * that the */
/*  shared edge ab lies in a horizontal plane        , with the TriPoint b on
 * the left
 */
...
/*  of TriTriangle abc        , and is directed left        , from vertex a to
 * vertex b. */
/*                                                                           */
/*  The triangles abc and bad are deleted and replaced by the triangles cdb  */
/*  and dca.  The triangles that represent abc and bad are NOT deallocated;  */
/*  they are reused for dca and cdb        , respectively.  Hence        , any
 * handles that
 */
 ...
 * edge */
/*  dc of TriTriangle dca        , and is directed down        , from vertex d
 * to vertex c.
 */
...
/*  `searchtri.tri' is NULL        , a full TriPoint location procedure is
 * called. */
/*  If `insertpoint' is found inside a TriTriangle        , the TriTriangle is
 * split into  */
...
/*  existing vertex        , no action is taken        , and the value
 * DUPLICATEPOINT is */
...
/*  existing vertex.                                                         */
/*                                                                           */
/*  Normally        , the parameter `splitedge' is set to NULL        , implying
 * that no */
/*  segment should be split.  In this case        , if `insertpoint' is found to
 */
/*  lie on a segment        , no action is taken        , and the value
 * VIOLATINGPOINT is */
...
/*  `segmentflaws' is set.  If bad triangles are created        , these are
 * added */
/*  to the queue if `triflaws' is set.                                       */
/*                                                                           */
/*  If a duplicate TriPoint or violated segment does not prevent the TriPoint */
/*  from being inserted        , the return value will be ENCROACHINGPOINT if
 * the */
...
enum insertsiteresult insertsite(TriPoint insertpoint,
  ...
  if ((intersect == ONEDGE) || (intersect == OUTSIDE)) {
    ...
    /* Insert the TriPoint on an edge        , dividing one TriTriangle into two
     * (if
     */
  ...
  while (1) {
    ...
    if (doflip) {
      ...
      if (top.tri == dummytri) {
         ...
         */
        /*   `leftpoint'        , `rightpoint'        , and `farpoint' could be
         * vertices  */
        ...
        if ((leftpoint == infpoint1) || (leftpoint == infpoint2) ||
          ...
          /*   infinite as well        , but trust me        , this same
           * condition      */
          /*   should be applied.                                       */
        ...
        if (doflip) {
          ...
          if (vararea) {
            if ((areabound(top) <= 0.0) || (areabound(horiz) <= 0.0)) {
              ...
              /*   long        , long way from their original location due to
               * flips.
               */
...
/*  represented as edges in the mesh.  One side        , called the "base" ,
 * need not
 */
/*  be.                                                                      */
/*                                                                           */
 ...
 */
/*  polygon        , until `lastedge'        , the primary edge of the last
 * TriTriangle. */
...
/*  edge flip effectively removes one TriTriangle from the fan        ,
 * committing it
 */
/*  to the polygon.  The resulting polygon has one fewer edge.  If `doflip'  */
/*  is set        , the final flip will be performed        , resulting in a fan
 * of one */
/*  (useless?) TriTriangle.  If `doflip' is not set        , the final flip is
 * not */
/*  performed        , resulting in a fan of two triangles        , and an
 * unfinished */
 ...
 * It's */
/*  rarely a big deal for TriPoint deletion        , where n is rarely larger
 * than ten
 *       , */
/*  but it could be a big deal for segment insertion        , especially if
 * there's */
...
void triangulatepolygon(TriOrientedTriangle *firstedge,
  ...
  int bestnumber;
  int i;
  TriTriangle ptr; /* Temporary variable used by sym()        , onext()        ,
                      and oprev(). */
  ...
  if (verbose > 2) {
    ...
    printf("    (%.12g        , %.12g) (%.12g        , %.12g)\n",
           leftbasepoint[0], leftbasepoint[1], rightbasepoint[0],
           rightbasepoint[1]);
  }
...
/*****************************************************************************/
/*                                                                           */
/*  pointmedian()   An order statistic algorithm        , almost.  Shuffles an
 * array
 */
...
/*  if axis == 1.  Very similar to the pointsort() procedure        , but runs
 * in */
 ...
 * each */
/*  step        , whether the next "tooth" is a bounding TriTriangle of the left
 * hull
 */
...
/*                      conquer method.                                      */
/*                                                                           */
/*  Sorts the points        , calls a recursive procedure to triangulate them ,
 * and
 */
...
long divconqdelaunay() {
  ...
  for (j = 1; j < inpoints; j++) {
    if ((sortarray[i][0] == sortarray[j][0]) &&
      ...
      if (!quiet) {
        printf(
            "Warning:  A duplicate TriPoint at (%.12g        , %.12g) appeared "
            "and was ignored.\n",
...
/*                  into.                                                    */
/*                                                                           */
/*  The points at "infinity" are assigned finite coordinates        , which are
 * used
 */
/*  by the TriPoint location routines        , but (mostly) ignored by the
 * Delaunay */
/*  edge flip routines.                                                      */
/*                                                                           */
/*****************************************************************************/
...
/*****************************************************************************/
/*                                                                           */
/*  removebox()   Remove the "infinite" bounding TriTriangle        , setting
 * boundary
 */
...
/*  origin of `searchtri' to the TriPoint `endpoint'        , and returns the
 * result
 */
/*  in `searchtri'.  The origin of `searchtri' does not change        , even
 * though */
...
enum finddirectionresult finddirection(TriOrientedTriangle *searchtri,
  ...
  while (leftflag) {
    ...
    if (searchtri->tri == dummytri) {
      ...
      printf("  TriTriangle leading from (%.12g        , %.12g) to",
...
/*  intersection TriPoint as its origin        , and endpoint1 as its
 * destination. */
/*                                                                           */
/*****************************************************************************/
...
void segmentintersection(TriOrientedTriangle *splittri,
  ...
  if (verbose > 1) {
    printf("  Splitting edge (%.12g        , %.12g) (%.12g        , %.12g) at "
           "(%.12g        , "
           "%.12g).\n",
 ...
 */
/*                   second endpoint)        , a collinear TriPoint        , and
 * the */
 ...
 */
/*  the segment        , a shell edge is inserted connecting the first endpoint
 * to */
/*  the collinear TriPoint        , and the search is continued from the
 * collinear */
/*  TriPoint. */
/*                                                                           */
...
/*****************************************************************************/
/*                                                                           */
/*  delaunayfixup()   Enforce the Delaunay condition at an edge        , fanning
 * out
 */
...
/*                                                                           */
/*  The origin of fixuptri is treated as if it has just been inserted        ,
 * and */
/*  the local Delaunay condition needs to be enforced.  It is only enforced  */
/*  in one sector        , however        , that being the angular range defined
 * by */
/*  fixuptri.                                                                */
/*                                                                           */
/*  This routine also needs to make decisions regarding the "stacking" of    */
/*  triangles.  (Read the description of constrainededge() below before      */
/*  reading on here        , so you understand the algorithm.)  If the position
 * of */
/*  the new TriPoint (the origin of fixuptri) indicates that the vertex before
 */
/*  it on the polygon is a reflex vertex        , then "stack" the TriTriangle
 * by */
/*  doing nothing.  (fixuptri is an inverted TriTriangle        , which is how
 * stacked
 */
 ...
 */
/*  If so        , perform an edge flip        , thereby eliminating an inverted
 * TriTriangle
 */
/*  (popping it off the stack).  The edge flip may result in the creation    */
/*  of a new inverted TriTriangle        , depending on whether or not the new
 * vertex
 */
/*  is visible to the vertex three edges behind on the polygon.              */
/*                                                                           */
/*  If neither of the two vertices behind the new vertex are reflex          */
/*  vertices        , fixuptri and fartri        , the TriTriangle opposite it
 * , are not */
/*  inverted; hence        , ensure that the edge between them is locally
 * Delaunay. */
/*                                                                           */
...
void delaunayfixup(TriOrientedTriangle *fixuptri, int leftside) {
  ...
  if (counterclockwise(rightpoint, leftpoint, farpoint) > 0.0) {
    /* fartri is not an inverted TriTriangle        , and farpoint is not a
     * reflex */
    /*   vertex.  As there are no reflex vertices        , fixuptri isn't an */
    /*   inverted TriTriangle        , either.  Hence        , test the edge
     * between the */
...
/*  (which is not necessarily monotone        , but is close enough)        ,
 * which is */
/*  divided into two polygons by the new segment.  This routine's task is    */
/*  to generate the Delaunay triangulation of these two polygons.            */
/*                                                                           */
/*  You might think of this routine's behavior as a two-step process.  The   */
/*  first step is to walk from endpoint1 to endpoint2        , flipping each
 * edge */
/*  encountered.  This step creates a fan of edges connected to endpoint1 , */
/*  including the desired edge to endpoint2.  The second step enforces the   */
/*  Delaunay condition on each side of the segment in an incremental manner: */
/*  proceeding along the polygon from endpoint1 to endpoint2 (this is done   */
/*  independently on each side of the segment)        , each vertex is
 * "enforced" */
 ...
 */
/*                                                                           */
/*  In truth        , constrainededge() interleaves these two steps.  The
 * procedure */
/*  walks from endpoint1 to endpoint2        , and each time an edge is
 * encountered */
/*  and flipped        , the newly exposed vertex (at the far end of the flipped
 */
/*  edge) is "enforced" upon the previously flipped edges        , usually
 * affecting
 */
...
/*  polygon        , there will be an inverted TriTriangle formed by one of the
 * edge
 */
...
/*  thought of as a reflex vertex pushed on the stack        , waiting to be
 * fixed */
/*  later.                                                                   */
/*                                                                           */
...
void constrainededge(TriOrientedTriangle *starttri, TriPoint endpoint2,
    ...
    if ((farpoint[0] == endpoint2[0]) && (farpoint[1] == endpoint2[1])) {
      ...
      if (area == 0.0) {
        ...
        if (area > 0.0) { /* farpoint is to the left of the segment. */
          ...
          /*   flipped        , one of its endpoints is the fan vertex        ,
           * and the */
  ...
  /* If there was a collision with an interceding vertex        , install
   * another */
...
/*****************************************************************************/
/*                                                                           */
/*  formskeleton()   Create the shell edges of a triangulation        ,
 * including */
...
void plague() {
  ...
  while (virusloop != (TriTriangle **)NULL) {
    ...
    /*   edges        , setting it to an illegal value.  Hence        , we have
     * to       */
      ...
      if ((neighbor.tri == dummytri) || infected(neighbor)) {
        ...
        if (neighborshelle.sh == dummysh) {
          ...
          if (verbose > 2) {
            ...
            printf("    Marking (%.12g        , %.12g) (%.12g        , %.12g) "
                   "(%.12g        , "
                   "%.12g)\n",
    ...
    /* Remark the TriTriangle as infected        , so it doesn't get added to
     * the */
    /*   virus pool again.                                             */
    infect(testtri);
    virusloop = (TriTriangle **)traverse(&viri);
  ...
  while (virusloop != (TriTriangle **)NULL) {
    ...
    for (testtri.orient = 0; testtri.orient < 3; testtri.orient++) {
      sym(testtri, neighbor);
      if (neighbor.tri == dummytri) {
        /* There is no neighboring TriTriangle on this edge        , so this
         * edge */
        /*   is a boundary edge.  This TriTriangle is being deleted        , so
         * this
         */
...
void regionplague(REAL attribute, REAL area) {
  ...
  /*   and/or area constraint to their neighbors        , then to their
   * neighbors' */
  /*   neighbors.                                                          */
  traversalinit(&viri);
  virusloop = (TriTriangle **)traverse(&viri);
  while (virusloop != (TriTriangle **)NULL) {
    ...
    for (testtri.orient = 0; testtri.orient < 3; testtri.orient++) {
      ...
      /* Check for a shell between the TriTriangle and its neighbor. */
      tspivot(testtri, neighborshelle);
      /* Make sure the neighbor exists        , is not already infected        ,
       * and */
...
void carveholes(REAL *holelist, int holes, REAL *regionlist, int regions) {
  ...
  /*   (Incidentally        , this is the reason why regional attributes and
   * area */
...
void highorder() {
  ...
  traversalinit(&triangles);
  triangleloop.tri = triangletraverse();
  /* To loop over the set of edges        , loop over all triangles        , and
   * look at */
...
/*                                                                           */
/*  To save memory        , the TriPoint numbers are written over the shell
 * markers */
...
/*  triangles        , and the Voronoi edges are listed by traversing the
 * Delaunay */
/*  edges.                                                                   */
/*                                                                           */
 ...
 */
/*      hull (-c).                                                           */
/*  - Read the holes (-p)        , regional attributes (-pA)        , and
 * regional area */
/*      constraints (-pa).  Carve the holes and concavities        , and spread
 * the */

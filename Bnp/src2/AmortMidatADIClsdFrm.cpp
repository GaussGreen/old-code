/*-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+--+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-
        ...
        Description	: implementation of the Closed Form Solution of the
European Amortizing/Accreting Swaption.
...
static void _GenMidAt_CashFlow_(
    ...
    // results
    double *pdCashFlow_Begin, double *pdT_CashFlow_Begin) {
  ...
  const double *pdCashFlow_End = pdCashFlow_Begin + nSize_FixPay +
                                 nSize_FltPay + nSize_FltStart + nSize_FixFee +
                                 nSize_ExFee;
  ...
  // fixed leg
  {
    for (; plFixPay_Begin < plFixPay_End;
         ++pdCashFlow_Begin, ++pdT_CashFlow_Begin, ++plFixPay_Begin,
         ++pdFixCoverage_Begin, ++pdFixNotional_Begin) {
      const double dDF = swp_f_df(lToday, *plFixPay_Begin, szYC);
      *pdCashFlow_Begin =
          (*pdFixCoverage_Begin) * (*pdFixNotional_Begin) * dCoupon * dDF;
      *pdCashFlow_Begin *= dPayRec;
      _fill_tenor(lToday, plFixPay_Begin, 1 + plFixPay_Begin,
                  pdT_CashFlow_Begin);
    }
  }
  ...
  // floating leg
  {
    ...
    *pdCashFlow_Begin = -dPayRec * dDF * (*pdFltNotional_Begin);
    ...
    for (; plFltPay_Begin <
           plFltPay_End - 1; // NB: not inclusing the last cash flow
         ++pdCashFlow_Begin, ++pdT_CashFlow_Begin, ++plFltPay_Begin,
         ++pdFltCoverage_Begin, ++pdFltNotional_Begin, ++pdMargin_Begin,
         ++pdSpread_Begin) {
      const double dDF = swp_f_df(lToday, *plFltPay_Begin, szYC);
      *pdCashFlow_Begin = *(pdFltNotional_Begin + 1) - *pdFltNotional_Begin;
      *pdCashFlow_Begin += (*pdFltNotional_Begin) * (*pdFltCoverage_Begin) *
                           (*pdMargin_Begin + *pdSpread_Begin);
      *pdCashFlow_Begin *= -dDF * dPayRec;
      _fill_tenor(lToday, plFltPay_Begin, 1 + plFltPay_Begin,
                  pdT_CashFlow_Begin);
    }
...
const char *GenMidAt_ClsdFrm_1F(
  ...
  _GenMidAt_CashFlow_(szYC, lToday, dCoupon, plFixPay_Begin, plFixPay_End,
                      pdFixCoverage_Begin, pdFixNotional_Begin, lFltStart,
                      plFltPay_Begin, plFltPay_End, pdFltCoverage_Begin,
                      pdFltNotional_Begin, pdMargin_Begin, pdSpread_Begin,
                      dPayRec, plExFeePay, pdExFee, plFixFeePay, pdFixFee,
                      pdCashFlow, pdT_CashFlow);

  /// Covert Sigma to Zeta
  _fill_tenor(lToday, &lEx, 1 + &lEx, &dEx);
  szErr =
      Convert_SigToZeta_1F(pdSigT_Begin, pdSigT_End, pdSig_Begin, pdLamT_Begin,
                           pdLamT_End, pdLam_Begin, &dEx, 1 + &dEx, &dZeta);
  if (szErr)
    return szErr;

  /// Covert Lambda to G
  Convert_LamToG_1F(pdLamT_Begin, pdLamT_End, pdLam_Begin, pdT_CashFlow,
                    pdT_CashFlow + nSize_CashFlow,
                    pdG // G at cash flow dates
  );

  Convert_LamToG_1F(pdLamT_Begin, pdLamT_End, pdLam_Begin, &dEx, 1 + &dEx,
                    &dG_Ex // G at exercise date
  );

  /// price
  *pdOptionPV = lgmopval1F(nSize_CashFlow, pdCashFlow, pdG, dZeta, dG_Ex);
...
const char *GenMidAt_ClsdFrm_2F(
  ...
  _fill_tenor(lToday, &lEx, 1 + &lEx, &dEx);
  szErr =
      Convert_SigToZeta_2F(pdSigT_Begin, pdSigT_End, pdSig_Begin, pdLamT_Begin,
                           pdLamT_End, pdLam_Begin, &dEx, 1 + &dEx, dAlpha,
                           dGamma, dRho, &dZeta1, &dZeta2, &dZeta12);
  if (szErr)
    return szErr;

  /// Covert Lambda to G
  Convert_LamToG_2F(pdLamT_Begin, pdLamT_End, pdLam_Begin, pdT_CashFlow,
                    pdT_CashFlow + nSize_CashFlow, dGamma,
                    pdG1, // G1 at cash flow dates
                    pdG2  // G2 at cash flow dates
  );

  Convert_LamToG_2F(pdLamT_Begin, pdLamT_End, pdLam_Begin, &dEx, 1 + &dEx,
                    dGamma,
  ...
  /// price
  *pdOptionPV = lgmopval2F(nSize_CashFlow, pdCashFlow, pdG1, pdG2, dZeta1,
                           dZeta2, dZeta12, dG1_Ex, dG2_Ex);
...
void Convert_LamToG_1F(const double *pdLamT_Begin, const double *pdLamT_End,
  ...
  export_lgmsetupG_ts(pdLamT_End - pdLamT_Begin, (double *)pdLamT_Begin,
                      (double *)pdLam_Begin, pdGT_End - pdGT_Begin,
                      (double *)pdGT_Begin, pdG_Begin, 0, 0, 0);
  ...
  if (pdLamT_End - pdLamT_Begin == 1) {
    ...
    export_lgmsetupG(*pdLam_Begin, pdGT_End - pdGT_Begin, (double *)pdGT_Begin,
                     pdG, 0, 0, 0);
...
void Convert_LamToG_2F(const double *pdLamT_Begin, const double *pdLamT_End,
  ...
  // fill in pdG1_Begin
  Convert_LamToG_1F(pdLamT_Begin, pdLamT_End, pdLam_Begin, pdGT_Begin, pdGT_End,
                    pdG1_Begin);
  export_lgmsetupG2_ts(pdLamT_End - pdLamT_Begin, (double *)pdLamT_Begin,
                       (double *)pdLam_Begin, dGamma, pdGT_End - pdGT_Begin,
                       (double *)pdGT_Begin, pdG2_Begin, 0, 0, 0);

#ifdef _DEBUG
  // Check again export_lgmsetupG2 if there is a single lambda
  if (pdLamT_End - pdLamT_Begin == 1) {
    double *pdG2 = _alloca((pdGT_End - pdGT_Begin) * sizeof(double));
    export_lgmsetupG2(*pdLam_Begin, dGamma, pdGT_End - pdGT_Begin,
                      (double *)pdGT_Begin, pdG2, 0, 0, 0);
...
static const char *
_Convert_SigToZeta_(const double *pdSigT_Begin, const double *pdSigT_End,
                    const double *pdSig_Begin, const double *pdLamT_Begin,
                    const double *pdLamT_End, const double *pdLam_Begin,
                    ...
                    // results
                    double *pdZeta_Begin) {
  char *computeZeta_ts(double, double *, double *, int, double, double *,
                       double *, int, double *);
  ...
  for (; pdZetaT < pdZetaT_End; ++pdZetaT, ++pdZeta) {
    const char *szErr = computeZeta_ts(
        *pdZetaT, (double *)pdLamT_Begin, (double *)pdLam_Begin,
        pdLamT_End - pdLamT_Begin, dGamma, (double *)pdSigT_Begin,
        (double *)pdSig_Begin, pdSigT_End - pdSigT_Begin, pdZeta);
  ...
  if (pdLamT_End - pdLamT_Begin == 1) {
    ...
    for (; pdZetaT < pdZetaT_End; ++pdZeta, ++pdZetaT, ++pdZeta_Begin) {
      const char *szErr =
          computeZeta(*pdZetaT, *pdLam_Begin, (double *)pdSigT_Begin,
                      (double *)pdSig_Begin, pdSigT_End - pdSigT_Begin, pdZeta);
...
Convert_SigToZeta_1F(const double *pdSigT_Begin, const double *pdSigT_End,
  ...
  return _Convert_SigToZeta_(pdSigT_Begin, pdSigT_End, pdSig_Begin,
                             pdLamT_Begin, pdLamT_End, pdLam_Begin,
                             pdZetaT_Begin, pdZetaT_End, dGamma, pdZeta_Begin);
}
...
Convert_SigToZeta_2F(const double *pdSigT_Begin, const double *pdSigT_End,
  ...
  /// fill Zeta1
  szErr = _Convert_SigToZeta_(pdSigT_Begin, pdSigT_End, pdSig_Begin,
                              pdLamT_Begin, pdLamT_End, pdLam_Begin,
                              pdZetaT_Begin, pdZetaT_End, 0., pdZeta1_Begin);
  if (szErr)
    return szErr;
  ...
  szErr = _Convert_SigToZeta_(
      ...
      pdLam_Begin, pdZetaT_Begin, pdZetaT_End, dGamma, pdZeta2_Begin);
  if (szErr)
    return szErr;

  _multiply_vector2( // multiply by dAlpha^2
      pdZeta2_Begin, pdZeta2_Begin + (pdZetaT_End - pdZetaT_Begin),
      dAlpha * dAlpha, pdZeta2_Begin);

  /// fill Zeta12
  szErr = _Convert_SigToZeta_(
      pdSigT_Begin, pdSigT_End, pdSig_Begin, pdLamT_Begin, pdLamT_End,
      pdLam_Begin, pdZetaT_Begin, pdZetaT_End, 0.5 * dGamma, pdZeta12_Begin);
  if (szErr)
    return szErr;

  _multiply_vector2( // multiply by dAlpha*dRho
      pdZeta12_Begin, pdZeta12_Begin + (pdZetaT_End - pdZetaT_Begin),
      dRho * dAlpha, pdZeta12_Begin);

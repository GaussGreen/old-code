...
static double srt_f_opttpamdi_price(double fwd1, double fwd2, double barrier,
                                    double vol1, double vol2, double mat1,
                                    double mat2, double disc1, double disc2,
 ...
 *              	: vol1         	- volatility of underlying(BS) from 0 to
 *T1 : vol2         	- volatility of underlying(BS) from 0 to T2 : mat1
 *- first barrier date : mat2        	- maturity of option (last barrier date)
...
{ /* BEGIN srt_f_opttpamdi() */
  ...
  premium = srt_f_opttpamdi_price(fwd1, fwd2, barrier, vol1, vol2, mat1, mat2,
                                  disc1, disc2, below_above);
  ...
  case DELTA_FWD: /*** DELTA FWD ***/
    shift = fwd2 / 10000;
    answer = (srt_f_opttpamdi(fwd1, fwd2 + shift, barrier, vol1, vol2, mat1,
                              mat2, disc1, disc2, below_above, PREMIUM) -
              premium) /
             shift;
    break;
  ...
  case DELTA: /*** DELTA FWD1 + FWD2 ***/
    ...
    answer = (srt_f_opttpamdi(fwd1 * (1 + shift / fwd1),
                              fwd2 * (1 + shift / fwd2), barrier, vol1, vol2,
                              mat1, mat2, disc1, disc2, below_above, PREMIUM) -
              premium) /
             shift;
    break;
  ...
  case VEGA: /*** VEGA ***/
    shift = GVOPT.vol_add;
    answer = (srt_f_opttpamdi(fwd1, fwd2, barrier, vol1 + shift, vol2 + shift,
                              mat1, mat2, disc1, disc2, below_above, PREMIUM) -
              premium) /
             shift;
    break;
  ...
  case THETA: /*** THETA  ***/
    shift = YEARS_IN_DAY;
    answer =
        srt_f_opttpamdi(fwd1, fwd2, barrier, vol1, vol2, mat1 - shift,
                        mat2 - shift, disc1 * exp(-shift * log(disc1) / mat1),
                        disc2 * exp(-shift * log(disc2) / mat2), below_above,
                        PREMIUM) -
        premium;
    break;
...
static double srt_f_opttpamdi_price(
    double fwd1, double fwd2, double barrier, double vol1, double vol2,
    double mat1, double mat2, double disc1, double disc2,
    SrtBarrierType below_above) { /* BEGIN srt_f_opttpamdi_price */
  double premium = 0;
  double measure = 0;
  double mu12, mu12_p, r, b, sig1, sig1_sqrt, sig2, sig2_sqrt, alpha, a0, b0,
      a1, b1;
  int du;

...
/* -------------------------------------------------------------------------
    ...
    DESCRIPTION     :   Attach grfn events to steps in SrtStpLst inside gd.
                  ...
                  (this used to be in grf_f_get_event.c)

                        This function parses events in the past.
                        If there are any events in the past, it will augment
                        the event at today with a COMLL command to restore the
                        environment at the end history.
...
Err srt_f_attach_grfn_events_to_steps(GrfnDeal* gd, SrtStpPtr step)
{
    ...
    /* Start at the top left hand corner of tableau (minus one row for potential AM event )*/
    gd->I = -1;
    gd->J = 0;
    ...
    {
        /* Creates the historical event, taking fixings, cvg into account and evaluating what can be
         * done */
        error = grfn_create_historical_event(
            gd, (GrfnEvent**)(&(stp->e)), stp->ddate, (stp->next == NULL) ? 0.0 : stp->next->ddate);
        if (error)
            return error;
    ...
    /* Loop through all the time steps, one by one, starting from today  */
    while (stp)
    {
        /* Create and attach to the current time step the corresponding Grfn event */
        error = grfn_create_future_event(
            gd, (GrfnEvent**)(&(stp->e)), stp->ddate, (stp->next == NULL) ? 0.0 : stp->next->ddate);
        if (error)
            return error;
...
void srt_f_calc_grfn_event_dfs(GrfnEvent* event, SrtSample* sample, SrtUndInfo* und_info)
{
    int        i, j;
    SrtUndPtr  und;
    SrtMdlType mdl_type;
    ...
    for (i = 0; i < und_info->no_of_underlyings; i++)
    {
        ...
        if (mdl_type == VASICEK)
        {
            Vasicek_Y_T_at_t_compute(
                event->dflen[i], sample, event->yp[i], event->df[i], i, mdl_dim, mdl_type);
        }
        else
        {

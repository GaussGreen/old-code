...
/* While the next event date is before the first future (unknown) date */
    ...
    /* Creates the historical event, taking fixings, cvg into account and evaluating what can be done */
...
/* Loop through the underlyings in UndInfo one by one */
	...
	/* Get the Underlying and its model */	
		...
		if(mdl_type == VASICEK)
		{
			Vasicek_Y_T_at_t_compute(event->dflen[i], 
							 sample,
							 event->yp[i],
							 event->df[i],
							 i,
		...
		else
		{
			...
			Y_T_at_t_compute( event->dflen[i],
            		sample,
					event->yp[i],
					event->df[i], 
					i, 

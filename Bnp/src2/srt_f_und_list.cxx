...
SrtUndListPtr get_underlying_list(void) { return _srt_underlyings_list; }
...
Err srt_f_getundinlist(SrtUndListPtr und_list, String und_name,
                       SrtUndPtr *und) {
  int found = 0;
  Err err = NULL;
  SrtListObject *obj;
...
/* ---------------------------------------------------------------------------------
 */
/* Function to pass to srt_f_lstins to free an underlying object in a linked
 * list  */
...
Err srt_f_addundtolist(
    SrtUndListPtr und_list, String und_name, /* == und_name in SrtUndDesc */
    String und_lbl, /* FX_UND        , IR_UND        , EQ_UND */
    String und_ccy, String mdl_lbl, String crv_name1, /* == discount curve */
    String crv_name2,                                 /* == dividend curve */
    String crv_name3,                                 /* == repo curve */
    TermStruct *ts, double spot) {
  SrtUndDesc *und;
  SrtIrDesc *undint;
  SrtEqDesc *undeq;
  SrtFxDesc *undfx;
  ...
  case INTEREST_RATE_UND:
    ...
    if (mdl_lbl != 0) {
      if (err = srt_f_interp_model(mdl_lbl, &(undint->mdl_type),
                                   &(undint->mdl_dim)))
        return err;
  ...
  case EQUITY_UND:
    ...
    if (mdl_lbl != 0) {
      if (err = srt_f_interp_model(mdl_lbl, &(undeq->mdl_type),
                                   &(undeq->mdl_dim)))
        return err;
  ...
  case FOREX_UND:
    ...
    if (mdl_lbl != 0) {
      if (err = srt_f_interp_model(mdl_lbl, &(undfx->mdl_type),
                                   &(undfx->mdl_dim)))
        return err;
  ...
  /* Insert (overwrite & update ticker) the object in the underlying list (with
   * the same name) */
  err = srt_f_lstins(und_list, und_name, 0.0, OBJ_PTR_UND, und,
                     &srt_f_undvalfree, &(und->underl_ticker));
  if (err)
...
/* Get the FX Underlying  corresponding to the domname and forname in the
 * underlying list used */
  ...
  while (ls != NULL) {
    if (ls->element->type == OBJ_PTR_UND) {
      ...
      if (tempund->underl_type == FOREX_UND) {
        ...
        if (strcmp(temp_dom, copy_dom_name) == 0 &&
            strcmp(temp_for, copy_for_name) == 0) {
          srt_free(copy_dom_name);
          srt_free(copy_for_name);
          return tempund;
        }

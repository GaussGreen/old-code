...
void SabrQuadGetParams(double cash_fx, double gamma, double beta,
  ...
  */
  ...
  (*quad_param) =
      exp((beta - 2.0) * log(cash_fx)) * (gamma + beta * (beta - 1.0)) / 2.0;

  (*lin_param) =
      beta * exp((beta - 1.0) * log(cash_fx)) - 2.0 * (*quad_param) * cash_fx;
  (*const_param) = exp(beta * log(cash_fx)) -
                   cash_fx * ((*quad_param) * cash_fx + (*lin_param));
}
...
Err FxSabrQuadPrecalculations(
  ...
  for (i = 0; i < nstp - 1; i++) {
    ...
    if (fabs(drift[i]) > 1.0E-10) {
      exp_z += const1 * sig * sig / (2.0 * drift[i]) *
               (exp(2.0 * drift[i] * dt) - 1.0);
    } else {
    ...
    if (fabs(mean2) > 1.0E-10) {
      exp_z += const2 * sig * sig / mean2 *
               (exp(2.0 * drift[i] * dt - mean * tmid) - exp(-mean * time[i]));
    } else {
    ...
    drift_z[i] = (const3 * sig * sig *
                      (2.0 * lambda - alpha * alpha * exp(-mean * tmid)) +
                  (lambda - drift[i]) * sig0 * sig) *
                 2.0 * dt;
    ...
    if (fabs(mean2) > 1.0E-10) {
      exp_z +=
          const2 * sig * sig / mean2 *
          (exp(2.0 * drift[i] * dt - mean * time[i + 1]) - exp(-mean * tmid));
    } else {
...
Err FxSabrQuad_adi(
  ...
  double *expect_z = NULL, *drift_z = NULL, *x = NULL, *z = NULL,
         ...
         **mux = NULL, **muz = NULL, **varx = NULL, **varz = NULL,
         *varxinit = NULL, *muzinit = NULL, *sigBeta = NULL, **r = NULL;

  double *varxi, *varzi, *muzi, **valuesi, **valuesip, **valuesim, *valuesij;
  ...
  double bar_pres = 1.0E-08, bar_c1, bar_c2;
  long bar_index;
  ...
  if (bar_lvl) {
    disc_SL_center_barrier(&x, &nstepx, spot_fx, flx, fux, 0.9999, 0.00001,
                           stdln, NSTD_FX, bar_lvl, nstp, &index_x);
  } else {
    // disc_linleft_logright_center(x  , nstepx  , spot_fx  , flx  , fux  ,
    // stdln  , NSTD_FX  , &index_x); disc_linleft_linright_center(x  , nstepx  ,
    // spot_fx  , flx  , fux  , stdln  , NSTD_FX  , &index_x); disc_Quad_center(x
    // , nstepx  , spot_fx  , flx  , fux  , 0.9999  , 0.00001  , stdln  , NSTD_FX
    // , &index_x);
    disc_SL_center(x, nstepx, spot_fx, flx, fux, 0.9999, 0.00001, stdln,
                   NSTD_FX, &index_x);
  }

  /*
  disc_normal_center(z  , nstepz  , 0  , -NSTD_VOL * std3  , NSTD_VOL * std3  ,
  ...
  */
  ...
  if (bar_lvl && is_bar[nstp - 1]) {
    ...
    if (bar_index > 0 && bar_index < nstepx - 1) {
      ...
      bar_c1 = (x[bar_index] - x[bar_index - 1]) /
               (x[bar_index + 1] - x[bar_index - 1]);
      bar_c2 = 1.0 - bar_c1;

      for (j = 0; j < nstepz; j++) {
        valuesi[j][bar_col[nstp - 1]] =
            bar_c1 * valuesip[j][bar_col[nstp - 1]] +
            bar_c2 * valuesim[j][bar_col[nstp - 1]];
      }
    }
  }
  ...
  for (step = nstp - 2; step >= 0; step--) {
    ...
    /*	convolve							*/
    num_f_pde_one_step_backward_2f_adi(pde, nstepx, x, nstepz, z, 0, nprod - 1,
    ...
    if (eval_evt[step]) {
      err = payoff_func(date[step], time[step], func_parm_tab[step], today,
                        spot_fx, dom_yc, for_yc, lx, ux, lz, uz, x, nprod,
                        values);
      if (err) {
        goto FREE_RETURN;
      }
    ...
    if (bar_lvl && is_bar[step] && bar_index > lx && bar_index < ux) {
      ...
      for (j = lz; j <= uz; j++) {
        valuesi[j][bar_col[step]] = bar_c1 * valuesip[j][bar_col[step]] +
                                    bar_c2 * valuesim[j][bar_col[step]];
      }
    }
...
Err FxSabrQuadCalibration(
    ...
    /*	Model Parameters			*/
    double alpha, double coefa, double coefb, double coefc, double rho,
    double lambda,
    ...
    long nstpt_tgt, int nstpfx, int nstpvol, int nbIter, double precision,
  ...
  double *time = NULL, *date = NULL, *drift_res = NULL, *drift_adi = NULL,
         **res_iter = NULL;
  ...
  spot_fx = spot_fx * swp_f_df(today, spot_date, dom_yc) /
  ...
  if (!date || !drift_res || !drift_adi || !eval_evt || !func_parm ||
      !res_iter) {
  ...
  if (start_opt == 0) {
    ...
    if (fabs(lambda) > 1.0E-08) {
      alpha_eq *= sqrt((1.0 - exp(-2.0 * lambda * exercise[0])) /
                       (2.0 * lambda * exercise[0]));
    }
    ...
    if (is_straddle) {
      strike =
          spot_fx * exp(-0.5 * volatility[0] * volatility[0] * exercise[0]);
    ...
    nstpt = 2;
    err = fill_time_vector(&time, &nstpt, 0, NULL, 0, NULL, nstpt_tgt);
    ...
    premium_tgt = srt_f_optblksch(spot_fx, strike, volatility[0], exercise[0],
                                  df, SRT_CALL, PREMIUM);

    err = srt_f_optimpvol(premium_tgt * coef_linab, shift_spot, shift_strike,
                          exercise[0], df, is_put, SRT_LOGNORMAL, &sig0);

    sig0 /= coef_linab;
    vol2 = sig0;

    sig0 = op_sabrgen_calib(spot_fx, strike, exercise[0], volatility[0],
                            alpha_eq, coefa, coefb, coefc, rho, vol_quadra);
    ...
    err = FxSabrQuad_adi(nstpt, time, date, nstpfx, nstpvol, sig0, drift_adi,
                         alpha, coefa, coefb, coefc, rho, lambda, floormu,
                         func_parm, eval_evt, NULL, NULL, NULL, spot_fx, dom_yc,
                         for_yc, payoff_fx_sabr_adi_ATM_opt, 1, &premium, 0,
    ...
    err = srt_f_optimpvol(premium, spot_fx, strike, exercise[0], df, SRT_CALL,
                          SRT_LOGNORMAL, &bsvol);
    ...
    if (errorv >= precision || err) {
      if (err || newSig0 < 0.0 || fabs(alpha) < 0.001) {
        if (err) {
          ...
          newSig0 = sqrt(premium_tgt * premium_tgt / premium / premium *
                         newVar / exercise[0]);
        }
      }
    ...
    while ((k < nbIter) && (errorv >= precision)) {
      err = FxSabrQuad_adi(nstpt, time, date, nstpfx, nstpvol, newSig0,
                           drift_adi, alpha, coefa, coefb, coefc, rho, lambda,
                           floormu, func_parm, eval_evt, NULL, NULL, NULL,
                           spot_fx, dom_yc, for_yc, payoff_fx_sabr_adi_ATM_opt,
                           1, &premium2, 0, NULL, 0, 0, 0, NULL, NULL);
      if (err) {
        goto FREE_RETURN;
      }

      err = srt_f_optimpvol(premium2, spot_fx, strike, exercise[0], df,
                            SRT_CALL, SRT_LOGNORMAL, &bsvol);
  ...
  for (i = 1; i < start_opt; i++) {
    ...
    var += sigma[i - 1] * sigma[i - 1] / (2.0 * coef) *
           (exp(2.0 * coef * dt) - 1.0);
    drift_res[i] = coef;
  }
  ...
  if (start_opt > 0) {
    ...
    if (fabs(lambda) > 1.0E-08) {
      alpha_eq *= sqrt((1.0 - exp(-2.0 * lambda * exercise[start_opt - 1])) /
                       (2.0 * lambda * exercise[start_opt - 1]));
    }

    if (is_straddle) {
      strike =
          spot_fx * exp(-0.5 * volatility[start_opt - 1] *
                        volatility[start_opt - 1] * exercise[start_opt - 1]);
      shift_strike = coefa * strike + coefb;
    }

    df = swp_f_df(today,
                  (long)(today + exercise[start_opt - 1] * 365.0 + 1.0E-08),
                  dom_yc);

    premium_tgt =
        srt_f_optblksch(spot_fx, strike, volatility[start_opt - 1],
                        exercise[start_opt - 1], df, SRT_CALL, PREMIUM);

    err = srt_f_optimpvol(premium_tgt * coef_linab, shift_spot, shift_strike,
                          exercise[start_opt - 1], df, is_put, SRT_LOGNORMAL,
                          &vol2);

    vol2 /= coef_linab;

    volQuad2 = op_sabrgen_calib(spot_fx, strike, exercise[start_opt - 1],
                                volatility[start_opt - 1], alpha_eq, coefa,
                                coefb, coefc, rho, vol_quadra);
  }
  ...
  for (i = max(start_opt, 1); i < nbOpt; i++) {
    ...
    if (fabs(lambda) > 1.0E-08) {
      alpha_eq *= sqrt((1.0 - exp(-2.0 * lambda * exercise[i])) /
                       (2.0 * lambda * exercise[i]));
    }
    ...
    if (is_straddle) {
      strike =
          spot_fx * exp(-0.5 * volatility[i] * volatility[i] * exercise[i]);
      shift_strike = coefa * strike + coefb;
    }
    ...
    premium_tgt = srt_f_optblksch(spot_fx, strike, volatility[i], exercise[i],
                                  df, SRT_CALL, PREMIUM);

    /* Calculation of the first guess */

    var_diff = volatility[i - 1] * spot_fx /
               (spot_fx * (coefa * spot_fx + coefb) + coefc);
    vol2 =
        volatility[i] * spot_fx / (spot_fx * (coefa * spot_fx + coefb) + coefc);
    var_diff =
        (vol2 * vol2 * exercise[i] - var_diff * var_diff * exercise[i - 1]);

    /* Other guess of var_diff */

    err = srt_f_optimpvol(premium_tgt * coef_linab, shift_spot, shift_strike,
                          exercise[i], df, is_put, SRT_LOGNORMAL, &vol2);

    vol2 /= coef_linab;

    volQuad2 = op_sabrgen_calib(spot_fx, strike, exercise[i], volatility[i],
                                alpha, coefa, coefb, coefc, rho, vol_quadra);

    var_diff = (volQuad2 * volQuad2 * exercise[i] -
                volQuad1 * volQuad1 * exercise[i - 1]);
    ...
    newVar = var + sigma[i - 1] * sigma[i - 1] / (2.0 * coef) *
                       (exp(2.0 * coef * dt) - 1.0);
    ...
    err = FxSabrQuad_adi(nstpt, time, date, nstpfx, nstpvol, sig0, drift_adi,
                         ...
                         func_parm, eval_evt, NULL, NULL, NULL, spot_fx, dom_yc,
                         for_yc, payoff_fx_sabr_adi_ATM_opt, 1, &premium, 0,
                         NULL, 1, 0, premium_tgt, &newSig0, &temp);
    ...
    error = fabs(premium - premium_tgt);
    err = srt_f_optimpvol(premium, spot_fx, strike, exercise[i], df, SRT_CALL,
                          SRT_LOGNORMAL, &bsvol);
    ...
    if (errorv >= precision || err) {
      if (err) {
        ...
        if (newVar2 < var) {
          ...
          newCoef = solve_var_diff(dt, (newVar2 - var) /
                                           (sigma[i - 1] * sigma[i - 1]));
        }
      }
    ...
    while ((k < nbIter) && (errorv >= precision)) {
      err = FxSabrQuad_adi(nstpt, time, date, nstpfx, nstpvol, sig0, drift_adi,
                           alpha, coefa, coefb, coefc, rho, lambda, floormu,
                           func_parm, eval_evt, NULL, NULL, NULL, spot_fx,
                           dom_yc, for_yc, payoff_fx_sabr_adi_ATM_opt, 1,
      ...
      err = srt_f_optimpvol(premium2, spot_fx, strike, exercise[i], df,
                            SRT_CALL, SRT_LOGNORMAL, &bsvol);
      ...
      if (premium2 > 100 * premium_tgt || premium2 < 0.0 || err) {
        ...
        if (vega < 0) {
          ...
          */
          ...
          newCoef2 = solve_for_next_coef(res_iter, nb_iter, premium_tgt, 1);
    ...
    var += sigma[i - 1] * sigma[i - 1] / (2.0 * newCoef) *
           (exp(2.0 * newCoef * dt) - 1.0);
...
FREE_RETURN:
  ...
  if (time)
    free(time);
  if (date)
    free_dvector(date, 0, maxstep - 1);
  if (eval_evt)
    free_lvector(eval_evt, 0, maxstep - 1);
  if (func_parm)
    free_dmatrix(func_parm, 0, maxstep - 1, 0, 0);

  if (drift_adi)
    free_dvector(drift_adi, 0, maxstep - 1);
  if (drift_res)
    free_dvector(drift_res, 0, nbOpt - 1);
  if (res_iter)
...
static Err GetStrikesVol(double fwd_fx, double mat_smile, double df_dom,
                         double df_for, double atm_vol, double risk_reversal,
                         double butterfly, double *atm_strike, double *strike1,
                         double *vol1, double *strike2, double *vol2,
                         double *alpha, double beta, double *rho, double lambda,
                         int is_straddle, int use_input, double *fitting_error)

{
  ...
  if (*vol1 < 1.0E-08 || *vol2 < 1.0E-08 || *strike1 < 1.0E-08 ||
      *strike2 < 1.0E-08) {

    /* 25 PUT */
    *vol1 = butterfly + atm_vol - 0.5 * risk_reversal;
    //*strike1 = SolveForKPutPercent(-0.25  , fwd_fx  , *vol1 * sqrt(mat_smile)
    //, df_dom  , df_for);

    /* 25 CALL */
    *vol2 = *vol1 + risk_reversal;
    //*strike2 = SolveForKCallOutPercent(0.25  , fwd_fx  , *vol2 *
    //sqrt(mat_smile)  , df_dom  , df_for);
  } else {
    if (*strike1 > *strike2) {
      temp = *strike1;
      *strike1 = *strike2;
  ...
  if (!use_input) {
    ...
    *rho = 0.0;
    ...
    err = opsabrcalib(fwd_fx, mat_smile, 3, strike_sabr, vol_sabr, &atm_vol,
                      alpha, 1, &beta, 0, rho, 1, fitting_error);
    ...
    if (err) {
      ...
      *rho = 0.0;
      ...
      if (fabs(lambda) > 1.0E-10) {
        ...
        *alpha /= sqrt((1.0 - exp(-2.0 * lambda * mat_smile)) /
                       (2.0 * lambda * mat_smile));
      }
    }
  }
...
static Err FxSabrQuadCalibSmilePricer(
    /* Model params */
    double today, double spot_fx, double fwd_fx, double alpha, double a,
    double b, double c, double rho, double lambda, double floormu,

    char *dom_yc, char *for_yc,

    double *sigma_time_fx, double *sigma_fx, int sigma_n_fx,

    /* Discretisation params */
    double *time, double *date, double *drift, int nstp,

    double **func_parm, int *is_event,

    long nstpt_tgt, int nstpfx, int nstpvol,

    /* Product infos */
    double df, double mat_smile, double strike1, double strike2,

    double *res, double *impvol1, double *impvol2)

{
  ...
  err = FxSabrQuad_adi(nstp, time, date, nstpfx, nstpvol, sigma_fx[0], drift,
                       alpha, a, b, c, rho, lambda, floormu, func_parm,
                       is_event, NULL, NULL, NULL, spot_fx, dom_yc, for_yc,
                       payoff_fx_sabr_adi_opt, 2, res, 0, NULL, 0, 0, 0, NULL,
                       NULL);
  ...
  err = srt_f_optimpvol(res[0], fwd_fx, strike1, mat_smile, df, SRT_CALL,
                        SRT_LOGNORMAL, impvol1);

  err = srt_f_optimpvol(res[1], fwd_fx, strike2, mat_smile, df, SRT_CALL,
                        SRT_LOGNORMAL, impvol2);
...
Err FxSabrQuadCalibSmile(
    ...
    double *sigma) {
  double coefa, coefb;
  double alpha1, alpha2, alpha3, rho1, rho2, rho3, impvol11, impvol21, impvol12,
      impvol22, impvol13, impvol23;
  double sensalpha1, sensalpha2, sensrho1, sensrho2, delta, error1, error2,
      delta_alpha, delta_rho, fitting_error;
  double shift_spot;

  double mat_smile, setlt_smile, fwd_fx, df, df_for, spot_fx_cash;
  long exe_date, spot_date;

  double *time = NULL, *date = NULL, *drift = NULL, **func_parm = NULL,
         *res = NULL, *sigma_alpha = NULL, *sigma_rho = NULL, *guess = NULL;
  ...
  spot_date = add_unit(today, 2, SRT_BDAY, MODIFIED_SUCCEEDING);
  spot_fx_cash = spot_fx * swp_f_df(today, spot_date, dom_yc) /
                 swp_f_df(today, spot_date, for_yc);
  ...
  if (calib_smile) {
    ...
    if (i == nbOpt) {
      err = serror("Maturity %d of calibration is not present in the market "
                   "ATM term struct",
                   mat_date);
      goto FREE_RETURN;
    }
    ...
    err = GetStrikesVol(fwd_fx, mat_smile, df, df_for, atm_vol, risk_reversal,
                        butterfly, &atm_strike, &strike1, &vol1, &strike2,
                        &vol2, &alpha1, 0, &rho1, lambda, is_straddle,
                        use_input, &fitting_error);
    ...
    *vol2_out = vol2;
    ...
    /*	Fill the time vector */
    err = fill_time_vector(&time, &nstp, 0, NULL, 0, NULL, nstpt_tgt);
    if (err) {
      goto FREE_RETURN;
    }
    ...
    if (!date || !drift || !func_parm || !res || !sigma_alpha || !sigma_rho ||
        !guess) {
    ...
    /* calibration */
    err = FxSabrQuadCalibration(
        dom_yc, for_yc, today, spot_fx, alpha1, a, b, c, rho1, lambda, floormu,
    ...
    /* pricer */
    err = FxSabrQuadCalibSmilePricer(
        today, spot_fx_cash, fwd_fx, alpha1, a, b, c, rho1, lambda, floormu,
        dom_yc, for_yc, exercise_cal, sigma, nb_calib, time, date, drift, nstp,
        func_parm, is_event, nstpt_tgt, nstpfx, nstpvol, df, mat_smile, strike1,
        strike2, res, &impvol11, &impvol21);
    ...
    if (error > precisionSmile) {
       ...
       */
      if ((use_input || (!use_input && fabs(fitting_error) > 0.025)) &&
          alpha1 < 0.05) {
        alpha1 = 0.05;

        /* calibration */
        err = FxSabrQuadCalibration(dom_yc, for_yc, today, spot_fx, alpha1, a,
                                    b, c, rho1, lambda, floormu, exercise_cal,
                                    maturity_cal, volatility_cal, nb_calib, 0,
                                    is_straddle, nstpt_tgt, nstpfx, nstpvol,
                                    nbIterATM, precisionATM, NULL, sigma);
        ...
        err = FxSabrQuadCalibSmilePricer(
            today, spot_fx_cash, fwd_fx, alpha1, a, b, c, rho1, lambda, floormu,
            dom_yc, for_yc, exercise_cal, sigma, nb_calib, time, date, drift,
            nstp, func_parm, is_event, nstpt_tgt, nstpfx, nstpvol, df,
            mat_smile, strike1, strike2, res, &impvol11, &impvol21);
    ...
    while ((k < nbIterSmile) && (error >= precisionSmile)) {
      ...
      /* calibration */
      err = FxSabrQuadCalibration(dom_yc, for_yc, today, spot_fx, alpha2, a, b,
                                  c, rho1, lambda, floormu, exercise_cal,
                                  maturity_cal, volatility_cal, nb_calib, 0,
                                  is_straddle, nstpt_tgt, nstpfx, nstpvol,
                                  nbIterATM, precisionATM, sigma, sigma_alpha);
      ...
      /* pricer */
      err = FxSabrQuadCalibSmilePricer(
          today, spot_fx_cash, fwd_fx, alpha2, a, b, c, rho1, lambda, floormu,
          dom_yc, for_yc, exercise_cal, sigma_alpha, nb_calib, time, date,
          drift, nstp, func_parm, is_event, nstpt_tgt, nstpfx, nstpvol, df,
          mat_smile, strike1, strike2, res, &impvol12, &impvol22);
      ...
      /* calibration */
      err = FxSabrQuadCalibration(dom_yc, for_yc, today, spot_fx, alpha1, a, b,
                                  c, rho2, lambda, floormu, exercise_cal,
                                  maturity_cal, volatility_cal, nb_calib, 0,
                                  is_straddle, nstpt_tgt, nstpfx, nstpvol,
                                  nbIterATM, precisionATM, sigma, sigma_rho);
      ...
      /* pricer */
      err = FxSabrQuadCalibSmilePricer(
          today, spot_fx_cash, fwd_fx, alpha1, a, b, c, rho2, lambda, floormu,
          dom_yc, for_yc, exercise_cal, sigma_rho, nb_calib, time, date, drift,
          nstp, func_parm, is_event, nstpt_tgt, nstpfx, nstpvol, df, mat_smile,
          strike1, strike2, res, &impvol12, &impvol22);
      ...
      if (fabs(sensrho1) < 10E-06 || fabs(sensrho2) < 10E-06) {
        ...
        while (alpha3 < 0 || fabs(rho3) > 0.9999) {
          if (alpha3 < 0) {
            coef1 = (alpha1 - alpha1 * 0.5) /
                    ((sensrho2 * error1 - sensrho1 * error2) / delta);
          }

          if (rho3 < -0.9999) {
            coef2 = (rho1 + 0.99) /
                    ((-sensalpha2 * error1 + sensalpha1 * error2) / delta);
          } else if (rho3 > 0.9999) {
            coef2 = (rho1 - 0.99) /
                    ((-sensalpha2 * error1 + sensalpha1 * error2) / delta);
          }
          ...
          alpha3 =
              alpha1 - coef3 * (sensrho2 * error1 - sensrho1 * error2) / delta;
          rho3 = rho1 -
                 coef3 * (-sensalpha2 * error1 + sensalpha1 * error2) / delta;
        }
      }
      ...
      for (i = 0; i < nb_calib; i++) {
        guess[i] = sigma[i] +
                   (alpha3 - alpha1) * (sigma_alpha[i] - sigma[i]) /
                       (alpha2 - alpha1) +
                   (rho3 - rho1) * (sigma_rho[i] - sigma[i]) / (rho2 - rho1);
      }

      /* calibration */
      err = FxSabrQuadCalibration(dom_yc, for_yc, today, spot_fx, alpha3, a, b,
                                  c, rho3, lambda, floormu, exercise_cal,
                                  maturity_cal, volatility_cal, nb_calib, 0,
                                  is_straddle, nstpt_tgt, nstpfx, nstpvol,
                                  nbIterATM, precisionATM, guess, sigma);
      ...
      /* pricer */
      err = FxSabrQuadCalibSmilePricer(
          today, spot_fx_cash, fwd_fx, alpha3, a, b, c, rho3, lambda, floormu,
          dom_yc, for_yc, exercise_cal, sigma, nb_calib, time, date, drift,
          nstp, func_parm, is_event, nstpt_tgt, nstpfx, nstpvol, df, mat_smile,
          strike1, strike2, res, &impvol13, &impvol23);
  ...
  if (use_total_ts || nbOpt == 1) {
    ...
    err = FxSabrQuadCalibration(
        dom_yc, for_yc, today, spot_fx, alpha1, a, b, c, rho1, lambda, floormu,
        exercise, maturity, volatility, nbOpt, 0, is_straddle, nstpt_tgt,
        nstpfx, nstpvol, nbIterATM, precisionATM, NULL, sigma);
  }
...
FREE_RETURN:
  ...
  if (calib_smile) {
    smessage("Total calibration time in sec: %.2f",
             (double)(t2 - t1) / CLOCKS_PER_SEC);
  }
  ...
  if (drift)
    free_dvector(drift, 0, nstp - 1);
  if (func_parm)
    free_dmatrix(func_parm, 0, nstp - 1, 0, 1);
  if (res)
    free_dvector(res, 0, 1);
  if (is_event)
    free(is_event);

  if (sigma_alpha)
    free_dvector(sigma_alpha, 0, nbOpt - 1);
  if (sigma_rho)
    free_dvector(sigma_rho, 0, nbOpt - 1);
  if (guess)
    free_dvector(guess, 0, nbOpt - 1);
...
Err FxSabrQuadCalibSmile2(
    ...
    /*	Model Parameters				*/
    double *alpha_out, double *gamma_out, double *beta_out, double *rho_out,
    double lambda, double floormu,
    ...
    double *sigma) {
  double alpha1, alpha2, alpha3, rho1, rho2, rho3, beta1, beta2, beta3, gamma1,
      gamma2, gamma3, a1, a2, a3, b1, b2, b3, c1, c2, c3, impvol11, impvol21,
      impvol12, impvol22, impvol13, impvol23;
  double spotbeta1, spotbeta2, spotbeta3;
  double sensbt1, sensbt2, sensrr1, sensrr2, delta, error1, error2, delta_bt,
      delta_rr, fitting_error;
  ...
  if (calib_smile) {
    ...
    *vol2_out = vol2;
    ...
    /* calibration */
    err = FxSabrQuadCalibration(dom_yc, for_yc, today, spot_fx, alpha1, a1, b1,
                                c1, rho1, lambda, floormu, exercise_cal,
                                maturity_cal, volatility_cal, nb_calib, 0,
                                is_straddle, nstpt_tgt, nstpfx, nstpvol,
                                nbIterATM, precisionATM, NULL, sigma);
    ...
    /* pricer */
    err = FxSabrQuadCalibSmilePricer(
        today, spot_fx_cash, fwd_fx, alpha1, a1, b1, c1, rho1, lambda, floormu,
        dom_yc, for_yc, exercise_cal, sigma, nb_calib, time, date, drift, nstp,
        func_parm, is_event, nstpt_tgt, nstpfx, nstpvol, df, mat_smile, strike1,
        strike2, res, &impvol11, &impvol21);
    ...
    if (error > precisionSmile) {
       ...
       */
      if ((use_input || (!use_input && fabs(fitting_error) > 0.025)) &&
          ((use_alpharho == 1 && alpha1 < 0.05) ||
           (use_alpharho == 0 && gamma1 < 0.0005))) {
        if (use_alpharho) {
          alpha1 = 0.05;
        } else {
          gamma1 = 0.005;
        ...
        /* calibration */
        err = FxSabrQuadCalibration(dom_yc, for_yc, today, spot_fx, alpha1, a1,
                                    b1, c1, rho1, lambda, floormu, exercise_cal,
                                    maturity_cal, volatility_cal, nb_calib, 0,
                                    is_straddle, nstpt_tgt, nstpfx, nstpvol,
                                    nbIterATM, precisionATM, NULL, sigma);
        ...
        /* pricer */
        err = FxSabrQuadCalibSmilePricer(
            today, spot_fx_cash, fwd_fx, alpha1, a1, b1, c1, rho1, lambda,
            floormu, dom_yc, for_yc, exercise_cal, sigma, nb_calib, time, date,
            drift, nstp, func_parm, is_event, nstpt_tgt, nstpfx, nstpvol, df,
            mat_smile, strike1, strike2, res, &impvol11, &impvol21);
    ...
    while ((k < nbIterSmile) && (error >= precisionSmile)) {
      ...
      /* calibration */
      err = FxSabrQuadCalibration(dom_yc, for_yc, today, spot_fx, alpha2, a2,
                                  b2, c2, rho1, lambda, floormu, exercise_cal,
                                  maturity_cal, volatility_cal, nb_calib, 0,
                                  is_straddle, nstpt_tgt, nstpfx, nstpvol,
                                  nbIterATM, precisionATM, sigma, sigma_alpha);
      ...
      /* pricer */
      err = FxSabrQuadCalibSmilePricer(
          today, spot_fx_cash, fwd_fx, alpha2, a2, b2, c2, rho1, lambda,
          floormu, dom_yc, for_yc, exercise_cal, sigma_alpha, nb_calib, time,
          date, drift, nstp, func_parm, is_event, nstpt_tgt, nstpfx, nstpvol,
      ...
      /* calibration */
      err = FxSabrQuadCalibration(dom_yc, for_yc, today, spot_fx, alpha1, a2,
                                  b2, c2, rho2, lambda, floormu, exercise_cal,
                                  maturity_cal, volatility_cal, nb_calib, 0,
                                  is_straddle, nstpt_tgt, nstpfx, nstpvol,
                                  nbIterATM, precisionATM, sigma, sigma_rho);
      ...
      /* pricer */
      err = FxSabrQuadCalibSmilePricer(
          today, spot_fx_cash, fwd_fx, alpha1, a2, b2, c2, rho2, lambda,
          floormu, dom_yc, for_yc, exercise_cal, sigma_rho, nb_calib, time,
          date, drift, nstp, func_parm, is_event, nstpt_tgt, nstpfx, nstpvol,
          df, mat_smile, strike1, strike2, res, &impvol12, &impvol22);
      ...
      if (fabs(sensrr1) < 10E-06 || fabs(sensrr2) < 10E-06) {
        ...
        (*param_BT3) =
            (*param_BT1) - (sensrr2 * error1 - sensrr1 * error2) / delta;
        (*param_RR3) =
            (*param_RR1) - (-sensbt2 * error1 + sensbt1 * error2) / delta;

        while ((*param_BT3) < 0 || (use_alpharho && fabs(rho3) > 0.9999)) {
          if ((*param_BT3) < 0) {
            coef1 = ((*param_BT1) - (*param_BT1) * 0.5) /
                    ((sensrr2 * error1 - sensrr1 * error2) / delta);
          }

          if (use_alpharho && rho3 < -0.9999) {
            coef2 = (rho1 + 0.99) /
                    ((-sensbt2 * error1 + sensbt1 * error2) / delta);
          } else if (use_alpharho && rho3 > 0.9999) {
            coef2 = (rho1 - 0.99) /
                    ((-sensbt2 * error1 + sensbt1 * error2) / delta);
          } else {
            coef2 = (beta1 - delta_rr) /
                    ((-sensbt2 * error1 + sensbt1 * error2) / delta);
          }
          ...
          (*param_BT3) = (*param_BT1) -
                         coef3 * (sensrr2 * error1 - sensrr1 * error2) / delta;
          (*param_RR3) = (*param_RR1) -
                         coef3 * (-sensbt2 * error1 + sensbt1 * error2) / delta;
        }
      }
      ...
      for (i = 0; i < nb_calib; i++) {
        if (use_alpharho) {
          guess[i] = sigma[i] +
                     ((*param_BT3) - (*param_BT1)) *
                         (sigma_alpha[i] - sigma[i]) /
                         ((*param_BT2) - (*param_BT1)) +
                     ((*param_RR3) - (*param_RR1)) * (sigma_rho[i] - sigma[i]) /
                         ((*param_RR2) - (*param_RR1));
        } else {
          ...
          guess[i] = (sigma[i] * spotbeta1 +
                      ((*param_RR3) - (*param_RR1)) *
                          (sigma_rho[i] * spotbeta2 - sigma[i] * spotbeta1) /
                          ((*param_RR2) - (*param_RR1))) /
                     spotbeta3;

          guess[i] += ((*param_BT3) - (*param_BT1)) *
                      (sigma_alpha[i] - sigma[i]) /
                      ((*param_BT2) - (*param_BT1));
        }
      }
      ...
      /* calibration */
      err = FxSabrQuadCalibration(dom_yc, for_yc, today, spot_fx, alpha3, a3,
                                  b3, c3, rho3, lambda, floormu, exercise_cal,
                                  maturity_cal, volatility_cal, nb_calib, 0,
                                  is_straddle, nstpt_tgt, nstpfx, nstpvol,
                                  nbIterATM, precisionATM, guess, sigma);
      ...
      /* pricer */
      err = FxSabrQuadCalibSmilePricer(
          today, spot_fx_cash, fwd_fx, alpha3, a3, b3, c3, rho3, lambda,
          floormu, dom_yc, for_yc, exercise_cal, sigma, nb_calib, time, date,
          drift, nstp, func_parm, is_event, nstpt_tgt, nstpfx, nstpvol, df,
          mat_smile, strike1, strike2, res, &impvol13, &impvol23);
      ...
      /* Update alpha  , rho  , implied vols and the shift */
      if (use_alpharho) {
        delta_bt = max(min(fabs((*param_BT3) - (*param_BT1)), EPSALPHAMAX),
                       EPSALPHAMIN);
        delta_rr =
            max(min(fabs((*param_RR3) - (*param_RR1)), EPSRHOMAX), EPSRHOMIN);
      } else {
        delta_bt = max(min(fabs((*param_BT3) - (*param_BT1)), EPSCVXTYMAX),
                       EPSCVXTYMIN);
        delta_rr =
            max(min(fabs((*param_RR3) - (*param_RR1)), EPSBETAMAX), EPSBETAMIN);
      }
  ...
  if (use_total_ts || nbOpt == 1) {
    err = FxSabrQuadCalibration(
        dom_yc, for_yc, today, spot_fx, alpha1, a1, b1, c1, rho1, lambda,
        floormu, exercise, maturity, volatility, nbOpt, start_opt, is_straddle,
        nstpt_tgt, nstpfx, nstpvol, nbIterATM, precisionATM, NULL, sigma);
  } else {
    err = FxSabrQuadCalibration(
        dom_yc, for_yc, today, spot_fx, alpha1, a1, b1, c1, rho1, lambda,
        floormu, exercise, maturity, volatility, nbOpt, 0, is_straddle,
        nstpt_tgt, nstpfx, nstpvol, nbIterATM, precisionATM, NULL, sigma);
  }

    ...
    double* res_at_point)
{
    ...
    double *expect_z = NULL, *drift_z = NULL, *x = NULL, *z = NULL, ***values = NULL,
           ***values_p1 = NULL, ***values_temp = NULL, **mux = NULL, **muz = NULL, **varx = NULL,
           **varz = NULL, *varxinit = NULL, *muzinit = NULL, *sigBeta = NULL, **r = NULL;
    ...
    err = FxSabrQuadPrecalculations(
        nstp, time, sig0, drift, alpha, a, b, c, rho, lambda, spot_fx, expect_z, drift_z, &std1);

    /*	Boundaries */

    std1 /= sqrt(time[nstp - 1]);

    stdln = op_sabrgen(spot_fx, spot_fx, time[nstp - 1], std1, alpha, a, b, c, rho, vol_quadra);
    ...
    else
    {
        ...
        // disc_Quad_center(x, nstepx, spot_fx, flx, fux, 0.9999, 0.00001, stdln, NSTD_FX,
        // &index_x);
    ...
    */
    ...
    disc_linleft_linright_center(
        z, nstepz, 0, -NSTD_VOL * std3, NSTD_VOL * std3, std3, NSTD_VOL, &index_z);
    ...
    if (!x || !z || !values || !values_p1 || !mux || !muz || !sigBeta || !varx || !varxinit ||
        !muzinit || !varz || !r)
    {
    ...
    if (bar_lvl && is_bar[nstp - 1])
    {
        ...
        if (bar_index > 0 && bar_index < nstepx - 1)
        {
            ...
            for (j = 0; j < nstepz; j++)
            {
                valuesi[j][bar_col[nstp - 1]] = bar_c1 * valuesip[j][bar_col[nstp - 1]] +
    ...
    for (step = nstp - 2; step >= 0; step--)
    {
        ...
        if (bar_lvl && is_bar[step] && bar_index > lx && bar_index < ux)
        {
            ...
            for (j = lz; j <= uz; j++)
            {
                valuesi[j][bar_col[step]] =
                    bar_c1 * valuesip[j][bar_col[step]] + bar_c2 * valuesim[j][bar_col[step]];
            }
        }
    ...
    if (calc_greeks)
    {
        ...
        {
            greeks[0][k] =
                (values_p1[index_x + 1][index_z][k] - values_p1[index_x - 1][index_z][k]) /
                (dxu + dxd);
            greeks[1][k] = 2.0 *
                           ...
                           (dxu * dxd * (dxu + dxd));
            greeks[2][k] =
                (values[index_x][index_z][k] - values_p1[index_x][index_z][k]) / dt / DAYS_IN_YEAR;

            greeks[3][k] =
                (values_p1[index_x][index_z + 1][k] - values_p1[index_x][index_z - 1][k]) /
                (dzu + dzd);
            greeks[4][k] = 2.0 *
            ...
            greeks[5][k] =
                ((values_p1[index_x + 1][index_z + 1][k] - values_p1[index_x - 1][index_z + 1][k]) /
                     (dxu + dxd) -
                 (values_p1[index_x + 1][index_z - 1][k] - values_p1[index_x - 1][index_z - 1][k]) /
                     (dxu + dxd)) /
                (dzu + dzd);
    ...
    if (calc_at_point)
    {
        ...
        if ((target - res[column]) * is_up > 0.0)
        {
            ...
            vega = (values_p1[index_x][i][column] - values_p1[index_x][i - 1][column]) /
                   (z[i] - z[i - 1]);
            ...
            *vol = sig0 - newz;
            ...
            {
                res_at_point[k] =
                    coef * values_p1[index_x][i][k] + (1.0 - coef) * values_p1[index_x][i - 1][k];
            }
        }
        else
        {
            ...
            vega = (values_p1[index_x][i + 1][column] - values_p1[index_x][i][column]) /
                   (z[i + 1] - z[i]);
            ...
            *vol = sig0 - newz;
            ...
            {
                res_at_point[k] =
                    coef * values_p1[index_x][i + 1][k] + (1.0 - coef) * values_p1[index_x][i][k];
            }
        }
    }
    ...
    double* sigma)
{
    ...
    double *time = NULL, *date = NULL, *drift_res = NULL, *drift_adi = NULL, **res_iter = NULL;
    ...
    if (start_opt == 0)
    {
        ...
        {
            alpha_eq *=
                sqrt((1.0 - exp(-2.0 * lambda * exercise[0])) / (2.0 * lambda * exercise[0]));
        }
        ...
        premium_tgt =
            srt_f_optblksch(spot_fx, strike, volatility[0], exercise[0], df, SRT_CALL, PREMIUM);
        ...
        err   = srt_f_optimpvol(
            premium, spot_fx, strike, exercise[0], df, SRT_CALL, SRT_LOGNORMAL, &bsvol);
        ...
        if (errorv >= precision || err)
        {
            if (err || newSig0 < 0.0 || fabs(alpha) < 0.001)
            {
                ...
                {
                    newSig0 =
                        sqrt(premium_tgt * premium_tgt / premium / premium * newVar / exercise[0]);
                }
            }
        ...
        while ((k < nbIter) && (errorv >= precision))
        {
            ...
            err = srt_f_optimpvol(
                premium2, spot_fx, strike, exercise[0], df, SRT_CALL, SRT_LOGNORMAL, &bsvol);
    ...
    if (start_opt > 0)
    {
        ...
        {
            alpha_eq *= sqrt(
                (1.0 - exp(-2.0 * lambda * exercise[start_opt - 1])) /
                (2.0 * lambda * exercise[start_opt - 1]));
        }

        if (is_straddle)
        {
            strike = spot_fx * exp(-0.5 * volatility[start_opt - 1] * volatility[start_opt - 1] *
    ...
    for (i = max(start_opt, 1); i < nbOpt; i++)
    {
        ...
        {
            alpha_eq *=
                sqrt((1.0 - exp(-2.0 * lambda * exercise[i])) / (2.0 * lambda * exercise[i]));
        }
        ...
        premium_tgt =
            srt_f_optblksch(spot_fx, strike, volatility[i], exercise[i], df, SRT_CALL, PREMIUM);
        ...
        error = fabs(premium - premium_tgt);
        err   = srt_f_optimpvol(
            premium, spot_fx, strike, exercise[i], df, SRT_CALL, SRT_LOGNORMAL, &bsvol);
        ...
        while ((k < nbIter) && (errorv >= precision))
        {
            ...
            err = srt_f_optimpvol(
                premium2, spot_fx, strike, exercise[i], df, SRT_CALL, SRT_LOGNORMAL, &bsvol);
...

{
    ...
    err = srt_f_optimpvol(res[0], fwd_fx, strike1, mat_smile, df, SRT_CALL, SRT_LOGNORMAL, impvol1);

    err = srt_f_optimpvol(res[1], fwd_fx, strike2, mat_smile, df, SRT_CALL, SRT_LOGNORMAL, impvol2);
...
{
    double coefa, coefb;
    double alpha1, alpha2, alpha3, rho1, rho2, rho3, impvol11, impvol21, impvol12, impvol22,
        impvol13, impvol23;
    double sensalpha1, sensalpha2, sensrho1, sensrho2, delta, error1, error2, delta_alpha,
        delta_rho, fitting_error;
    double shift_spot;

    double mat_smile, setlt_smile, fwd_fx, df, df_for, spot_fx_cash;
    long   exe_date, spot_date;

    double *time = NULL, *date = NULL, *drift = NULL, **func_parm = NULL, *res = NULL,
           *sigma_alpha = NULL, *sigma_rho = NULL, *guess = NULL;
    ...
    fwd_fx = spot_fx_cash * swp_f_df(today, mat_date, for_yc) / swp_f_df(today, mat_date, dom_yc);
    ...
    if (calib_smile)
    {
        ...
        {
            err = serror(
                "Maturity %d of calibration is not present in the market ATM term struct",
                mat_date);
            goto FREE_RETURN;
        }
        ...
        while ((k < nbIterSmile) && (error >= precisionSmile))
        {
            ...
            else
            {
                ...
                while (alpha3 < 0 || fabs(rho3) > 0.9999)
                {
                    ...
                    {
                        coef2 =
                            (rho1 + 0.99) / ((-sensalpha2 * error1 + sensalpha1 * error2) / delta);
                    }
                    else if (rho3 > 0.9999)
                    {
                        coef2 =
    ...
    double* sigma)
{
    double alpha1, alpha2, alpha3, rho1, rho2, rho3, beta1, beta2, beta3, gamma1, gamma2, gamma3,
        a1, a2, a3, b1, b2, b3, c1, c2, c3, impvol11, impvol21, impvol12, impvol22, impvol13,
        impvol23;
    double spotbeta1, spotbeta2, spotbeta3;
    double sensbt1, sensbt2, sensrr1, sensrr2, delta, error1, error2, delta_bt, delta_rr,
        fitting_error;
        ...
        *vol2_out    = vol2;
        ...
        if (error > precisionSmile)
        {
            ...
            if ((use_input || (!use_input && fabs(fitting_error) > 0.025)) &&
                ((use_alpharho == 1 && alpha1 < 0.05) || (use_alpharho == 0 && gamma1 < 0.0005)))
            {
                if (use_alpharho)
                {
                    alpha1 = 0.05;
        ...
        while ((k < nbIterSmile) && (error >= precisionSmile))
        {
            ...
            else
            {
                ...
                while ((*param_BT3) < 0 || (use_alpharho && fabs(rho3) > 0.9999))
                {
                    ...
                    {
                        coef2 =
                            (beta1 - delta_rr) / ((-sensbt2 * error1 + sensbt1 * error2) / delta);
                    }

                    coef3 = min(coef1, coef2);

                    (*param_BT3) =
                        (*param_BT1) - coef3 * (sensrr2 * error1 - sensrr1 * error2) / delta;
                    (*param_RR3) =
                        (*param_RR1) - coef3 * (-sensbt2 * error1 + sensbt1 * error2) / delta;
                }
            }
            ...
            for (i = 0; i < nb_calib; i++)
            {
                ...
                {
                    guess[i] = sigma[i] +
                               ((*param_BT3) - (*param_BT1)) * (sigma_alpha[i] - sigma[i]) /
                                   ((*param_BT2) - (*param_BT1)) +
                ...
                else
                {
                    ...
                    guess[i] += ((*param_BT3) - (*param_BT1)) * (sigma_alpha[i] - sigma[i]) /
                                ((*param_BT2) - (*param_BT1));
                }
            }

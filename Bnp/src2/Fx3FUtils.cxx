...
Err get_transf_matrix(double *sigma, double *correl, double **transf_matrix,
  ...
  err = diagonalise_symmetric_matrix(covar_matrix, 3, eigen_val, eigen_vector);
  ...
  for (i = 0; i < 3; i++) {
    if (eigen_val[i] < 1.0e-16) {
      err = "Covariance matrix is NOT positive definite - check your "
            "correlations";
      return err;
    }
  ...
  prod_matrix_diago(eigen_vector, eigen_val, transf_matrix, gi, gj, gsum);

  /*	Calculation of the transformation matrix inverse : from Original to Dual
   * Base */
  for (i = 0; i < 3; i++) {
    eigen_val[i] = 1.0 / eigen_val[i];
  }

  transpo_matrix(eigen_vector, eigen_vector_transp, gi, gj);

  prod_diago_matrix(eigen_vector_transp, eigen_val, transf_matrix_inv, gi, gj,
                    gsum);
...
Err get_lambda_from_ir_ts(TermStruct *ts, double *lambda) {
  ...
  if (n != 1) {
    return "Term structure contains maore than one tau in "
           "get_lambda_from_ir_ts";
  }
...
Err compute_vol_times(char *und3dfx, int *num_vol_times, double **vol_times,
  ...
  double *sig_date_dom = NULL, *sig_dom = NULL, *tau_date_dom = NULL,
         *tau_dom = NULL, *sig_date_for = NULL, *sig_for = NULL,
         *tau_date_for = NULL, *tau_for = NULL, *sig_date_fx = NULL,
         *sig_fx = NULL, *mat_c = NULL, *c1 = NULL, *c2 = NULL, *c3 = NULL;

  int i;
  Err err = NULL;

  err = Get_FX_StochRate_TermStructures_corr(
      und3dfx, &sig_date_dom, &sig_dom, &sig_n_dom, &tau_date_dom, &tau_dom,
      &tau_n_dom, &sig_date_for, &sig_for, &sig_n_for, &tau_date_for, &tau_for,
      &tau_n_for, &sig_date_fx, &sig_fx, &sig_n_fx, &mat_c, &c1, &c2, &c3,
      &nb_corr);
  ...
  *vol_times =
      (double *)calloc(sig_n_dom + sig_n_for + sig_n_fx, sizeof(double));
  if (!vol_times) {
...
void adjust_emp_covar(double **g, int npth, int n) {
  int i, j, k;
  double **cov = NULL, **chol = NULL, *temp = NULL, **invchol = NULL;
  double sum;
...
FREE_RETURN:
  ...
  if (cov)
    free_dmatrix(cov, 0, n - 1, 0, n - 1);
  if (chol)
    free_dmatrix(chol, 0, n - 1, 0, n - 1);
  if (temp)
    free_dvector(temp, 0, n - 1);
  if (invchol)
    free_dmatrix(invchol, 0, n - 1, 0, n - 1);
}
...
Err correlate_variable_corr(double *dom_std, double *for_std, double *fx_std,
                            double *corr12, double *corr13, double *corr23,
                            long nbPaths, long nbSteps, double **matrix) {
  long i, j;
  double *a = NULL, *b = NULL, *c = NULL, *d = NULL, *e = NULL, *f = NULL;
  double x, y, z;
  ...
  for (j = 0; j < nbSteps; j++) {
    ...
    if (c[j] != 0) {
      ...
      if (corr13[j] * dom_std[j] * fx_std[j] ==
          corr23[j] * for_std[j] * fx_std[j]) {
...
Err correlate_variable_covar(double *dom_std, double *for_std, double *fx_std,
                             double *covar12, double *covar13, double *covar23,
                             long nbPaths, long nbSteps, double **matrix) {
  long i, j;
  double *a = NULL, *b = NULL, *c = NULL, *d = NULL, *e = NULL, *f = NULL;
  double x, y, z;

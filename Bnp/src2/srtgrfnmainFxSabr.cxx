...
char *SrtGrfnFxSabrAdi(char *underlying, double alpha, double beta, double rho,
                       double lambda, double floormu, int numeventdates,
                       long *eventdates, long tableauRows, long tableauCols,
                       char ***tableauStrings, int **tableauMask, long auxWidth,
                       long *auxLen, double **aux, double *barrier,
                       int *bar_col, int *is_cont_bar, int nstp, int nstpfx,
                       int nstpvol, int *nprod, double **prod_val) {
  int free_str = 0;
  FIRSTAllMkts xStr;
  ...
  double spot_fx;
  char *domname, *forname, *dom_yc, *for_yc;

  int fx_idx, dom_idx, for_idx;

  double *sigma_fx = NULL, *sigma_time_fx = NULL;
  ...
  err = display_FX1F_TermStruct(underlying, &sigma_n_fx, &sigma_time_fx,
                                &sigma_fx);
  ...
  if (!void_prm || !is_event || !sig || !date || !drift || !is_bar ||
      !bar_lvl || !bar_cl) {
  ...
  for (i = nstp - 1; i >= 0; i--) {
    ...
    if (j >= 0 && fabs(time[i] - evt_tms[j]) < 1.0E-08) {
      ...
      if (barrier[j] > 1.0e-08) {
        /* mat_date = add_unit ((long) (date[i] + 1.0E-08)        , 2        ,
         * SRT_BDAY        , MODIFIED_SUCCEEDING);*/
        next_bar = barrier[j] * swp_f_df(today, date[i], dom_yc) /
      ...
      if (is_cont_bar[j + 1]) {
        is_bar[i] = 1;
        bar_lvl[i] = barrier[j + 1] * swp_f_df(spot_date, date[i], dom_yc) /
                     swp_f_df(spot_date, date[i], for_yc);
  ...
  *prod_val = dvector(0, num_col - 1);
  ...
  err = FxSabr_adi(nstp, time, date, nstpfx, nstpvol, sigma_fx[0], drift, alpha,
                   beta, rho, lambda, floormu, void_prm, is_event, bar_lvl,
                   bar_cl, is_bar, spot_fx, dom_yc, for_yc, payoff_fx_sabr_adi,
                   num_col, *prod_val);
...
char *SrtGrfnFxSabrAdi2(char *underlying, double alpha, double beta, double rho,
                        double lambda, double floormu, int numeventdates,
                        long *eventdates, long tableauRows, long tableauCols,
                        char ***tableauStrings, int **tableauMask,
                        long auxWidth, long *auxLen, double **aux,
                        double *barrier, int *bar_col, int *is_cont_bar,
                        int *is_up_bar, int nstp, int nstpfx, int nstpvol,
  ...
  for (i = nstp - 1; i >= 0; i--) {
    ...
    if (j >= 0 && fabs(time[i] - evt_tms[j]) < 1.0E-08) {
      ...
      if (barrier[j - 1] > 1.0e-08) {
        next_bar = barrier[j - 1] * swp_f_df(today, date[i], dom_yc) /
                   swp_f_df(today, date[i], for_yc);
  ...
  *prod_val = dvector(0, num_col - 1);
  ...
  err = FxSabr_adi_bar(nstp, time, date, nstpfx, nstpvol, sigma_fx[0], drift,
                       alpha, beta, rho, lambda, floormu, void_prm, is_event,
                       bar_lvl, bar_cl, is_bar, is_up, spot_fx, dom_yc, for_yc,
                       payoff_fx_sabr_adi, num_col, *prod_val);
...
Err FxSabrAdiKO(char *underlying, double alpha, double beta, double rho,
  ...
  double *sigma_fx = NULL, *sigma_time_fx = NULL, *prod_val = NULL;
  ...
  double *bar_lvl = NULL, **func_parm = NULL;
  ...
  for (i = nstp - 1; i >= 0; i--) {
    date[i] = today + time[i] * DAYS_IN_YEAR;
    bar_lvl[i] = barrier * swp_f_df(today, date[i], dom_yc) /
                 swp_f_df(today, date[i], for_yc);
  }
  ...
  err = FxSabr_KOOption(nstp, time, date, nstpfx, nstpvol, sigma_fx[0], drift,
                        alpha, beta, rho, lambda, floormu, strike, is_call,
                        bar_lvl, is_up, is_cvx, spot_fx, dom_yc, for_yc,
                        prod_val);
  ...
  if (!is_ko) {
    ...
    err = FxSabr_adi(nstp, time, date, nstpfx, nstpvol, sigma_fx[0], drift,
                     alpha, beta, rho, lambda, floormu, func_parm, is_event,
                     NULL, NULL, NULL, spot_fx, dom_yc, for_yc,
                     payoff_fx_sabr_adi_opt, 1, &premium);
...
FREE_RETURN:
  ...
  if (func_parm)
    free_dmatrix(func_parm, 0, nstp - 1, 0, 0);
  if (is_event)
    free(is_event);
...
Err FxSabrSmile(char *underlying, double alpha, double beta, double rho,
                double lambda, double floormu,
                /*	Product data */
  ...
  *res = dvector(0, nb_strike - 1);
  ...
  err = FxSabr_adi(nstp, time, date, nstpfx, nstpvol, sigma_fx[0], drift, alpha,
                   beta, rho, lambda, floormu, func_parm, is_event, NULL, NULL,
                   NULL, spot_fx, dom_yc, for_yc, payoff_fx_sabr_adi_opt,
                   num_col, *res);
...
char *SrtGrfnFxSabrSLAdi(char *underlying, double alpha, double beta,
                         ...
                         int *is_cont_bar, int nstp, int nstpfx, int nstpvol,
                         int *nprod, double **prod_val) {
  int free_str = 0;
  FIRSTAllMkts xStr;
  SrtGrfnParam defParm;
  ...
  *prod_val = dvector(0, num_col - 1);
  ...
  err =
      FxSabrSL_adi(nstp, time, date, nstpfx, nstpvol, sigma_fx[0], drift, alpha,
                   beta, rho, lambda, floormu, void_prm, is_event, bar_lvl,
                   bar_cl, is_bar, spot_fx, dom_yc, for_yc, payoff_fx_sabr_adi,
                   num_col, *prod_val, 0, NULL, 0, 0, 0, NULL, NULL);
...
Err FxSabrSLSmile(char *underlying, double alpha, double beta, double rho,
                  double lambda, double floormu,
                  /*	Product data */
                  long mat_date, double *strike, int nb_strike, int nstp,
                  int nstpfx, int nstpvol, double **res) {
  ...
  *res = dvector(0, nb_strike - 1);
  ...
  err =
      FxSabrSL_adi(nstp, time, date, nstpfx, nstpvol, sigma_fx[0], drift, alpha,
                   beta, rho, lambda, floormu, func_parm, is_event, NULL, NULL,
                   NULL, spot_fx, dom_yc, for_yc, payoff_fx_sabr_adi_opt,
                   num_col, *res, 0, NULL, 0, 0, 0, NULL, NULL);
...
Err FxSabrSLAdiKO(char *underlying, double alpha, double beta, double rho,
  ...
  double *bar_lvl_up = NULL, *bar_lvl_down = NULL, *prod_val_ki = NULL,
         **greeks_ki = NULL;
  ...
  for (i = nstp - 1; i >= 0; i--) {
    date[i] = today + time[i] * DAYS_IN_YEAR;
    bar_lvl_up[i] = barrier_up * swp_f_df(today, date[i], dom_yc) /
                    swp_f_df(today, date[i], for_yc);
    bar_lvl_down[i] = barrier_down * swp_f_df(today, date[i], dom_yc) /
                      swp_f_df(today, date[i], for_yc);
  }
  ...
  *greeks = calloc(6, sizeof(double));
  ...
  if ((spot_fx > barrier_up) || (spot_fx < barrier_down)) {
    ...
    err = FxSabrSL_KOOption(nstp, time, date, nstpfx, nstpvol, sigma_fx[0],
                            drift, alpha, beta, rho, lambda, floormu, strike,
                            is_call, is_american, is_cvx, is_digital,
                            bar_lvl_up, bar_lvl_down, rebate_up, rebate_down,
                            spot_fx, dom_yc, for_yc, prod_val, 1, *greeks);
  }
  ...
  if (!is_ko) {
    ...
    err = FxSabrSL_KOOption(nstp, time, date, nstpfx, nstpvol, sigma_fx[0],
                            drift, alpha, beta, rho, lambda, floormu, strike,
                            is_call, is_american, is_cvx, is_digital,
                            bar_lvl_up, bar_lvl_down, 0.0, 0.0, spot_fx, dom_yc,
                            for_yc, prod_val_ki, 1, *greeks_ki);
...
FREE_RETURN:
  ...
  if (greeks_ki)
    free_dmatrix(greeks_ki, 0, 5, 0, 0);
  if (prod_val_ki)
...
char *SrtGrfnFxSabrQuadAdi(char *underlying, double alpha, double gamma,
                           double beta, double rho, double lambda,
                           double floormu, int numeventdates, long *eventdates,
  ...
  *prod_val = dvector(0, num_col - 1);
  ...
  err = FxSabrQuad_adi(nstp, time, date, nstpfx, nstpvol, sigma_fx[0], drift,
                       alpha, a, b, c, rho, lambda, floormu, void_prm, is_event,
                       bar_lvl, bar_cl, is_bar, spot_fx, dom_yc, for_yc,
                       payoff_fx_sabr_adi, num_col, *prod_val, 0, NULL, 0, 0, 0,
                       NULL, NULL);
...
Err FxSabrQuadSmile(char *underlying, double alpha, double gamma, double beta,
                    double rho, double lambda, double floormu,
                    /*	Product data */
                    long mat_date, double *strike, int nb_strike, int nstp,
                    int nstpfx, int nstpvol, double **res) {
...
Err FxSabrQuadAdiKO(char *underlying, double alpha, double gamma, double beta,
  ...
  settlmt_date = add_unit((long)(today + maturity * DAYS_IN_YEAR), 2, SRT_BDAY,
                          MODIFIED_SUCCEEDING);
  ...
  *greeks = calloc(6, sizeof(double));
  ...
  if ((spot_fx > barrier_up) || (spot_fx < barrier_down)) {
    ...
    err = FxSabrQuad_KOOption(
        nstp, time, date, nstpfx, nstpvol, sigma_fx[0], drift, alpha, a, b, c,
        rho, lambda, floormu, settlmt_date, strike, is_call, is_american,
        is_cvx, is_digital, bar_lvl_up, bar_lvl_down, rebate_up, rebate_down,
        spot_fx, dom_yc, for_yc, 0, 0, 0, prod_val, 1, *greeks);
  }
  ...
  if (!is_ko) {
    ...
    err = FxSabrQuad_KOOption(
        nstp, time, date, nstpfx, nstpvol, sigma_fx[0], drift, alpha, a, b, c,
        rho, lambda, floormu, settlmt_date, strike, is_call, is_american,
        is_cvx, is_digital, bar_lvl_up, bar_lvl_down, 0.0, 0.0, spot_fx, dom_yc,
        for_yc, 0, 0, 0, prod_val_ki, 1, *greeks_ki);
...
Err FxSabrQuadAdi_MultiKO(char *underlying, double alpha, double gamma,
                          ...
                          double floormu,
                          /*	Product data */
                          int nb_product, double *notional, long *exe_date,
                          long *settlmt_date, double *strike,
                          int *is_call,    /* 1 Call        , 0: Put */
                          int is_ko,       /* 1 KO        , 0: KI */
                          int *is_cvx,     /* 1 use 1 / Fx        , 0 use Fx */
                          int *is_digital, /* 1: digital payoff        , 0 ,
  ...
  double *maturity = NULL, *settlment = NULL;
  ...
  double *bar_lvl_up = NULL, *bar_lvl_down = NULL, *new_reb_down = NULL,
         *new_reb_up = NULL, *prod_val_ki = NULL, **greeks_ki = NULL;
  ...
  *greeks = calloc(6, sizeof(double));
  ...
  if ((spot_fx > barrier_up) || (spot_fx < barrier_down)) {
    ...
    err = FxSabrQuad_KO_MultiOption(
        nstp, time, date, eval_evt, nstpfx, nstpvol, sigma_fx[0], drift, alpha,
        a, b, c, rho, lambda, floormu, nb_product, notional, exe_date,
        settlmt_date, strike, is_call, is_cvx, is_digital, bar_lvl_up,
        bar_lvl_down, new_reb_up, new_reb_down, spot_fx, dom_yc, for_yc, 0, 0,
        0, prod_val, 1, *greeks);
  }

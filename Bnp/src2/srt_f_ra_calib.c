...
static Err try_bump(double bump, double* diff, void* static_data)
{
    ...
    for (i = 0, sum = 0.0; i < p->nx; i++)
    {
        ...
        {
            c        = p->ex_data->gam_d[j];
            dfs_d[j] = p->ex_data->dffs_d[j] * exp(-c * (p->x[i] + 0.5 * c * p->ex_data->phi_f));
        }
        /* Calculate payoff */
        ...
        sum += p->w[i] * c * INV_SQRT_TWO_PI * exp(-0.5 * p->x[i] * p->x[i] / p->ex_data->phi_f) /
               p->std;
    }
...
static Err try_bump_quanto(double bump, double* diff, void* static_data)
{
    ...
    for (i = 0, sum = 0.0; i < p->nx; i++)
    {
        ...
        {
            c        = p->ex_data->gam_f[j];
            dfs_f[j] = p->ex_data->dffs_f[j] * exp(-c * (p->x[i] + 0.5 * c * p->ex_data->phi_f));
        }
...
static Err calib_bump(SrtProduct* product, CalibRA_ExData* ex_data, int nx)
{
    ...
    gauleg(ex_data->R_ffx - 4.0 * data.std, ex_data->R_ffx + 4.0 * data.std, x - 1, w - 1, nx);
    ...
    /* Bracket the root */
    err = num_f_zbrac((quanto ? try_bump_quanto : try_bump), &x1, &x2, &f1, &f2, &data);
    if (err)
        return err;

    /* Calibrate */
    err = num_f_zbrent((quanto ? try_bump_quanto : try_bump), x1, x2, f1, f2, tol, &data, &result);
    if (err)
        return err;
    ...
    int         nx)
{
    ...
    for (i = 0, t1 = 0.0; i < nex; i++)
    {
        ...
        if (yc_f)
        {
            ...
            a_SdSf_Lf = a_SdSf_Lf * exp_Lf + sig_d[i] * sig_f[i] * (1.0 - exp_Lf) / lam_f;

            a_2Sf_Lf = a_2Sf_Lf * exp_Lf + sig_f[i] * sig_f[i] * (1.0 - exp_Lf) / lam_f;

            a_SfSx_Lf = a_SfSx_Lf * exp_Lf + sig_f[i] * sig_fx[i] * (1.0 - exp_Lf) / lam_f;
        ...
        for (j = 0; j < n_unds; j++)
            for (k = 0; k < n_dfs[j]; k++)
            {
                ...
                gam[j][k] =
                    (1.0 - exp(-lam[j] * (dates[j][k] - ex_dates[i]) * YEARS_IN_DAY)) / lam[j];
            }

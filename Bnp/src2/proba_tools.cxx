...
double proba_max_fct(double spot, double barrier, double vol, double mat,
                     double r_d, double r_f, int flag) {
...
double proba_gauss_fct(double spot, double b_do, double b_up, double vol,
                       double opt_mat, double r_d, double r_f, int nb_term,
                       int flag) {
...
static double int_g(double t) {
  ...
  result =
      (NI_var + NI_mu * t) / (2 * NI_vol * t * sqrt(2 * SRT_PI * t)) * expo;

  return (result);
}

double dens_gauss_n_explo(double var, double x1_n, double x2_n, double mu,
                          double mu2, double vol, double mat) {
  double result, coeff1, integ1, coeff2, integ2;
  double (*f)(double);

  coeff1 = exp(mu * x1_n / (vol * vol)) *
           exp((mu - mu2) * (var - x1_n) / (vol * vol));
  NI_var = var - x1_n;
  f = int_g;
  integ1 = sm_qsimp(f, 0.00001, mat, 1.0e-06);
  coeff2 = exp(mu * x2_n / (vol * vol)) *
           exp((mu - mu2) * (var - x2_n) / (vol * vol));
  NI_var = var - x2_n;
  f = int_g;
...
double proba_gauss_explo_fct(double spot, double b_do, double b_up, double vol,
                             double opt_mat, double r_d, double r_f,
                             int nb_term, int flag) {
  double x1_n, x2_n, a, b, k, mu, bound_do, bound_up, price;
  double price_k, mu2;
  int i, pl;
...
double proba_four_fct(double spot, double b_do, double b_up, double vol,
                      double opt_mat, double r_d, double r_f, int nb_term,
                      int flag)

{
  ...
  for (i = 1; i <= nb_term; i++) {
    ...
    price_k = four_n * exp(-lam_n * opt_mat) *
              (dens_four_n(i, bound_up, a, b, coeff1, coeff2) -
               dens_four_n(i, bound_do, a, b, coeff1, coeff2));
...
double probaf_yesno_fct(double spot, double b_yes, double b_no, double vol,
                        double mat, double r_d, double r_f, int nb_term,
                        int flag)

{
  ...
  for (i = 1; i <= nb_term; i++) {
    vn = -mu * mu / (2 * vol * vol) -
         i * i * SRT_PI * SRT_PI * vol * vol / (2 * (a + b) * (a + b));
    yn = exp(vn * mat) - 1;
    term = pow(-1, i + 1) * (SRT_PI * vol * vol / (a + b) / (a + b)) * i * yn /
           vn * sin(SRT_PI * a / (a + b));
    price += exp(-r_d * (1 - flag) * mat - r_f * flag * mat) *
             exp(-b * mu / vol / vol) * term;
  }
...
double probaf_yesno_explo_fct(double spot, double b_yes, double b_no,
                              double vol, double mat, double r_d, double r_f,
                              int nb_term)
...
double probag_yesno_fct(double spot, double b_yes, double b_no, double vol,
                        double mat, double r_d, double r_f, int nb_term,
                        int flag)

{
  ...
  if (b_yes < b_no) {
    if (b_yes >= spot)
      return (exp(-r_d * (1 - flag) * mat - r_f * flag * mat));
    if (b_no <= spot)
      return (0);
    mult2 = 1;
  } else {
  ...
  for (i = -nb_term; i <= nb_term; i++) {
    ...
    if (yn == 0)
      ...
      term = mult * (srt_f_optnrmdis(d1) + coeff * srt_f_optnrmdis(d2));
...
int proba_gauss_addin(int argc, UF_ARG *argv, UF_ARG *ret) {
  ...
  if (b_do > b_up)
    error_2020(" upper barrier must be > lower barrier");
...
int proba_gauss_explo_addin(int argc, UF_ARG *argv, UF_ARG *ret) {
  ...
  if (b_do > b_up)
    error_2020(" upper barrier must be > lower barrier");
  answer =
      proba_gauss_explo_fct(spot, b_do, b_up, vol, mat, rd, rf, num_term, flag);
...
int proba_four_addin(int argc, UF_ARG *argv, UF_ARG *ret) {
  double spot, strike, b_up, b_do, vol, mat, rd, rf;
  int opt_type, num_term, flag, dbg = 0;
  double premium;
...
int probaf_yesno_addin(int argc, UF_ARG *argv, UF_ARG *ret) {
  ...
  answer =
      probaf_yesno_fct(spot, b_yes, b_no, vol, mat, rd, rf, num_term, flag);
...
int probaf_yesno_explo_addin(int argc, UF_ARG *argv, UF_ARG *ret) {
  double spot, strike, b_yes, b_no, vol, mat, rd, rf;
  int opt_type, num_term, flag, dbg = 0;
  double premium;
  ...
  answer =
      probaf_yesno_explo_fct(spot, b_yes, b_no, vol, mat, rd, rf, num_term);
...
int probag_yesno_addin(int argc, UF_ARG *argv, UF_ARG *ret) {
  ...
  answer =
      probag_yesno_fct(spot, b_yes, b_no, vol, mat, rd, rf, num_term, flag);
...
int probag_yesno_explo_addin(int argc, UF_ARG *argv, UF_ARG *ret) {
  ...
  answer = probag_yesno_explo_fct(spot, b_yes, b_no, vol, mat, rd, rf, num_term,
                                  flag);

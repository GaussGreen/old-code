...
/* ---------------------------------------------------------------------------------
   ...
   abscissas and weights of the n-point Gauss-Hermite quadrature formula. The
   largest abscissa is returned in x[1]  , the most negative in x[n] Mind you  ,
   the integration function is exp(-x*x) and not the real Gaussian density
   ---------------------------------------------------------------------------------
 */
...
/* ---------------------------------------------------------------------------------
   Given n  , this routine returns arrays x[1..n] and w[1..n] containing the
   abscissas and weights of the n-point Gauss-Hermite quadrature formula. The
   largest abscissa is returned in x[1]  , the most negative in x[n]. If the
   polynomials roots have already been computed  , it does not recalculate them
   Mind you  , the integration function is exp(-x*x) and not the real Gaussian
   density
   ---------------------------------------------------------------------------------
 */
...
Err hermite_gauss_quick(int n, double x[], double w[]) {
  ...
  if (n < 0) {
    return serror(
        "Hermite polynomials of negative degree: U must be KIDDING !!!");
  }
  if (_hermite_prev_n == n) {
    ...
    if (_herm_x)
      free_dvector(_herm_x, 1, _hermite_prev_n);
    if (_herm_w)
      free_dvector(_herm_w, 1, _hermite_prev_n);

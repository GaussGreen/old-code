...
//computation of the canonic moments
	...
	if (Is_start_Infinity ==1) {
		if (Is_end_Infinity ==1) {
			...
			rec1=0;rec2=INV_SQRT_TWO_PI * exp(-0.5*end*end);
		}
	}
	else {
		if (Is_end_Infinity == 1) {
			...
			rec1=INV_SQRT_TWO_PI*exp(-0.5*start*start);rec2=0;
			}
		else {
...
void lanczos (double start_point, double end_point, int n, int k, double *x, double *w, int weight_function)

{
	...
	for (i=1 ; i<=n ; i++)
	{
		...
		case 1:
			b[i] = sqrt ( weight[i] * (1 + 
					(0.5*0.5 + 0.5 - y[i]*y[i]) /
					( exp(2.0*log(fabs(0.5*0.5 + 0.5 - y[i]*y[i]))) + exp(2.0*log(fabs((2*0.5+1.0)*y[i]))) ) ) );
			break;
		case 2 :
	...
	for (i=1;i<=k;i++) {
		for (j=1;j<=k;j++) z[i][j]=(double)(i == j);
	}

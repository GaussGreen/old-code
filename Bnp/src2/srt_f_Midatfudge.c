...
{   
	if (!str) return "Empty string in interp_MIDAFUDGE_model";
	strupper(str);
	strip_white_space(str);

	if(!strcmp(str,"SLIDDING"))
	{*val = SLIDDING; 	return 0;}
...
Err srt_f_initMIDAT_FUDGE(long nNumEx,
						  long *Ex,
						  long *ExPay,
						  double *ExPremium,
						  long lStart,
...
SWAPTION_MIDAT  **Swaption;
	...
	for (i=0; i<=date_list.len - 2; i++)
	{
		...
		plFixing[i] = add_unit(first_period_start, - spotlag, SRT_BDAY, MODIFIED_SUCCEEDING);
	}
	...
	for (i=1; i<=nNumEx; i++)
	{
		...
		else
		{
			...
			for (j=1; j<=nTempNum; j++)
			{
				...
				Swaption[i][j].coupon_rate = strike;
				
				/* compute the ForwardSwapRate today */
				err = swp_f_ForwardRate(
						date_list.date[index + j - 1],
						date_list.date[date_list.len -1],
						szSwapFreq,
						szSwapBasis,
						szYieldcurve,
						szRefRate,

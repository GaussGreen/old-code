...
static void NAG_CALL EvalDerivatives(Integer neq, double t, double y[], double yp[], Nag_User *comm)
{
	...
	for (i=0, offs = 4; i < 4; i++) if (p->want[i])
	{
		yg[i] = y + offs;
		ypg[i] = yp + offs;
		offs += 4;
	...
	if (p->want[1])
	{
		ypg[1][2] += -p->alpha * (y[2]*y[2] - y[3]*y[3]) + p->rho * p->sigma * (p->u_re * y[3] + p->u_im * y[2]);
		ypg[1][3] += -2.0 * p->alpha * y[2]*y[3] - p->rho * p->sigma * (p->u_re * y[2] - p->u_im * y[3]);
	}
	if (p->want[2])
	{
...
static Err HestonCalcPhi(SHestonComm_InvFT *q, double u_re, double u_im, double *h_re, double *h_im)
{
	...
	for (k=0, neq = 4; k < 4; k++) if (q->want[k])
	{
		yg[k] = y + neq;
		ypg[k] = yp + neq;
		neq += 4;
	...
	nag_ode_ivp_rk_setup(neq, q->mat, y, 0.0, RKtol, RKthres,
...
static Err HestonCalcPhiClosedForm(SHestonComm_InvFT *q, double u_re, double u_im, double *h_re, double *h_im)
{
	...
	LGMSVUpdateADClosedForm(q->mat, q->lam, -0.5 * q->alpha * q->alpha,
		q->lam + c1 * u_im, -c1 * u_re, c2 * (u_re * u_re - u_im * u_im - u_im), c2 * (u_re + 2.0 * u_re * u_im),
		&C_re, &C_im, &A_re, &A_im);
...
{
	if (cache->next) HestonFreeCache(cache->next, nk);
	free(cache->next);
	free(cache->v);
...
static double NAG_CALL Z_func(double v, Nag_User *comm)
{
	...
	else if (fabs(v - q->c_cur->v[q->c_cur->cur_pt]) > 1e-16)		// not yet calculated or no match
	{
		...
		if (q->c_cur->v[q->c_cur->cur_pt] == 0.0)	// point not yet calculated -> calculate it
		{
			...
			if (j < 4) err = HestonCalcPhi(q, v, -1.0, phi_re, phi_im);
			else err = HestonCalcPhiClosedForm(q, v, -1.0, phi_re, phi_im);
...
static Err HestonCalcMoments(SHestonComm_InvFT *q, double *mean, double *std)
{
	...
	nag_ode_ivp_rk_setup(4, q->mat, y, 0.0, RKtol, RKthres,
		Nag_RK_4_5, Nag_RK_range, Nag_ErrorAssess_off, 0.0, &opt, &fail);
	if (fail.code != NE_NOERROR) { err = serror(fail.message);  goto FREE_RETURN; }
...
Err HestonDOptions(double f0, double sigma, double alpha, double rho, double lam, double shift, double mat,
				  int nK, double *K, char **rec_pay_str, int *want, double **res)
{
	Err					err = NULL;
	...
	for (i=0; i < nK; i++) comm_InvFT.k[i] = log( (K[i]+shift) / (f0+shift) );
	...
	for (i=0; i < nK; i++)
	{
		...
		for (j=0; j < 5; j++) if (j==0 || want[j-1])
		{
			...
			if (j==0 && rec_pay == SRT_PUT && K[i] > f0) res[i][j] += K[i] - f0;
			if (j==0 && rec_pay == SRT_CALL && K[i] < f0) res[i][j] += f0 - K[i];
		}
	}
...
Err NewtonD(double x, double x_min, double x_max, Err (*func)(double, double *, void *),
			double tol, int maxiter, void *comm)
{
	Err					err = NULL;
	double				xx[3], ff[3];
	...
	while (1)
	{
		...
		if (fabs(ff[0]) < tol) return NULL;

		if (npts == 1) x = xx[0] + 0.1 * ff[0];
		else if (fabs(ff[1]-ff[0]) < 1e-16) return NULL;
		else if (npts == 2) x = (xx[0] * ff[1] - xx[1] * ff[0]) / (ff[1] - ff[0]);
		else {
			a = ( (b=(ff[2]-ff[1])/(xx[2]-xx[1])) - (ff[1]-ff[0])/(xx[1]-xx[0]) ) / (xx[2]-xx[0]);
			b -= a * (xx[1]+xx[2]);
...
static Err EvalATMdiff(double x, double *diff, void *comm)
{
	...
	if (Calib->alpha / Calib->lam < 1e-3)
		z = srt_f_optblksch(Calib->fwd + Calib->shift, Calib->fwd + Calib->shift, Calib->sigma,
			Calib->mat, 1.0, SRT_CALL, PREMIUM);
	else
	{
	...
	err = srt_f_optimpvol(z, Calib->fwd, Calib->fwd, Calib->mat, 1.0,
...
static Err Get_RR_BF(SHestonComm_Calib *Calib, double *RR, double *BF)
{
	...
	for (i=0; i < 2; i++)
	{
		...
		if (Calib->alpha / Calib->lam < 1e-3)
			z = srt_f_optblksch(Calib->fwd + Calib->shift, Calib->K[i] + Calib->shift, Calib->sigma,
				Calib->mat, 1.0, (Calib->K[i] < Calib->fwd ? SRT_PUT : SRT_CALL), PREMIUM);
		else
		{
		...
		err = srt_f_optimpvol(z, Calib->fwd, Calib->K[i], Calib->mat, 1.0,
			(Calib->K[i] < Calib->fwd ? SRT_PUT : SRT_CALL), SRT_LOGNORMAL, &vols[i]);
		if (err) goto FREE_RETURN;
	}
...
static Err EvalRRdiff(double x, double *diff, void *comm)
{
	...
	if (Calib->alpha <= a_min) Calib->alpha = a_min + 0.05 * (a_max-a_min);

	err = NewtonD(Calib->alpha * Calib->alpha, a_min*a_min, a_max*a_max, EvalBFdiff, 1e-4, 7, comm);
	if (err) return err;
					...
					int		calib_smile )
{
	...
	for (i=0; i < 2; i++) if (Calib.K[i] > 0.0) Calib.k[i] = log( (Calib.K[i]+shift) / (f0+shift) );
	...
	if (!calib_smile)
		...
		err = NewtonD(Calib.rho * (Calib.alpha + 1e-5), ra_min, ra_max, EvalRRdiff, 1e-4, 7, &Calib);
...
Err HestonDDensityFT(double sigma, double alpha, double rho, double lam, double mat,
					 double u_re, double u_im, double *h_re, double *h_im)
{
	double				A_re = 0.0, A_im = 0.0, C_re = 0.0, C_im = 0.0;
	double				c1, c2;
	...
	LGMSVUpdateADClosedForm(mat, lam, -0.5 * alpha * alpha,
		lam + c1 * u_im, -c1 * u_re, c2 * (u_re * u_re - u_im * u_im - u_im), c2 * (u_re + 2.0 * u_re * u_im),
		&C_re, &C_im, &A_re, &A_im);

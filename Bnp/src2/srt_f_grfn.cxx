...
/* Free all the memory used by Grfn : events        , dearls        , specific
 * corr and und
 */
...
/* The following two functions try to recreate what used to be in
   ...
   See logic in grfn_attach_event in file grf_f_get_event.cxx */
...
Err srt_f_grfn(
    ...
    double *
        *knownpayments) /* A report of known CF [2]        , paid on pay dates
                           [0] , according to event dates [1] ([0..*nrows-1])*/
{
  GrfnDeal gd;
  SrtStpPtr sptr = NULL;
  Err err = NULL;
  ...
  /* If calculation date is > last event date        , then extend tableau:
   * create a phantom event having a zero cash-flow. */
  ...
  /* Get info concerning :number of underlyings        , which ones.. (parses
   * the deal) */
  ...
  /*
   When two underlyings        , if they are lgm/lgm or lgm/bs        , one wil
   soon be able to can use tree as well and then the test will be if (
   und_info.no_of_underlyings > 2 )
  */

  /* ------------------ STEP 3: TIME DISCRETISATION ----------------------- */

  /* Build all the time steps        , for all the dates        , past and
   * future (according to event dates        , grfn params...) */
  ...
  /* Copy the results of the last MonteCarlo path (from gd.cxxells) */
  if ((!err) && (cellcontents)) {
    ...
    for (i = 0; i < ((init_nrows > *nrows) ? *nrows : init_nrows); i++) {
      for (j = 0; j < *ncols; j++)
        cellcontents[i][j] = gd.cxxells[i][j];
    }
  ...
  /* Sets the KNOWN Payments report (pay dates        , event dates and
   * cahsflow) as stored in the GrfnDeal */
...
/* ------------------------------------------------------------------------------------
   ...
   grfnparam->imp_type        , which was set in the function
   grfn_init_GrfnDeal.  One thing it does which ought to be done by MCCore is
   remove nodes which have no events in the case of Monte Carlo and LGM (in this
   case we can use the jumping numeraire.) This function also sets the value of
   grfnparam->step_num and grfnparam->node_dim for the tree functions.  The
   first of these is probably redundant because the tree function themselves
   will call the SrtStp function create_index(); the second of these is a
   legitimate piece of information that GRFN must somehow pass to the valuation
   function.
  ...
  if (grfnparam->skip_evaluation == SRT_NO) {
    ...
    if (sptr) {
      ...
      if (grfnparam->imp_type == IMPMONTECARLO) {
         ...
         * ==EQD_BLACK_SCHOLES) */
        if (und_info->jumping) {
          /* With Jumping Numeraire        , no need to save steps with no
           * evemts */
      ...
      /* Method is Backward: Finite Difference Method (Tree        , PDE ,
       * McTree...)
       */
      {
...
Err srt_f_grfn_val_historical_events(SrtStpPtr *step, GrfnDeal *gd,
  ...
  /* Makes sure all needed past variable information (va        ,vb...) is
   * attached to today's step */

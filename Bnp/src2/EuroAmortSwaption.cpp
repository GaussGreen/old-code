...
/*
        data for local volatility function defined to support old LGM1F pricer.
*/
Err (*LGM1F_GetVolNew)(
    char*   vol_curve_name,
    double  start_date,
...
char*       LGM1F_szYieldCurveName;
char*       LGM1F_szVolCurveName;
char*       LGM1F_szRefRate;
LGMMarkConv LGM1F_MarkConv;
long        LGM1F_lToday;

/*
        init function for local volatility function defined to support old LGM1F pricer.
*/
Err LGM1F_GetVolInit(
    Err (*getVolNew)(
        char*   vol_curve_name,
        double  start_date,
        double  end_date,
...
*/
LGMErr LGM1F_GetCashLogVol(
    Date lStart, Date lEnd, double dStrike, SRT_Boolean boolIsLog, double* dVol)
{
    /* local variables */
    SwapDP swap_dp;
    ...
    if (dPower == 0.0)
    {
        ...
        if (err = swp_f_setSwapDP(
                lStart, lEnd, LGM1F_MarkConv.sfreq, LGM1F_MarkConv.sbasis, &swap_dp))
            return err;
    ...
    double  dExerciseFee)
{
    // internal function declaration
     char* EuropeanAmortizingSwaption_(
        long,
        long,
        long,
        long*,
        ...
        char*,
        char*,
        LGMErr (*)(long, long, double, SRT_Boolean, double*),
        SRT_Boolean,
        LGM_TS*,
        LGMCalSet*,
        double*,
    ...
    LGM1F_GetVolInit(get_cash_vol, pszYieldCurveName, szVolCurveName, pszRefRate, AsOfDate);
    ...
    *dFloatPV = 0;
    ...
    else
    {
        for (ntime = 1; ntime <= NumPeriods; ++ntime)
        {
            // = Coupon*pFixCvgs[ntime]*dvPayDates[ntime] ;
            *dFixedPV +=
                pdNotionals[ntime] * pdCoupons[ntime] * pFixCvgs[ntime] * dvPayDates[ntime];
    ...
    */
    ...
    {
        ErrMessage =
            "Failure to instantiate term structure occurred in EuropeanAmortizingSwaption2()!";
        return ErrMessage;
    }

    // initialize calibration instruments
    Init_CalibInstruments(
        NumPeriods,
        AsOfDate,
        FixDate,
        plAcrlStartDates[1],
...
// temporarily commented out
    ...
    *dLGMVol = (pTermStruct->G[0] - pTermStruct->G[1]) * sqrt(2.0 * (pTermStruct->zeta[0])) *
               dLambda * sqrt(dLambda / (exp(2.0 * dLambda * dTex) - 1.0)) /
               (exp(-dLambda * dT0) - exp(-dLambda * dT1));
    ...
    LGMCalSet* pLGMCalSet)
{
    ...
    pLGMCalSet->cvgfix =
        pdCoverages[1]; /* cvgfix is the cvg for first period ftStart to tPay[ffirst] */
    pLGMCalSet->DfixStart = swp_f_df(
        AsOfDate,
        pLGMCalSet->ftStart,
    ...
    double      dExerciseFee)
{
    ...
    char* IsValidTermStructure(LGM_TS*, long);
    char* LGM1FZeroBond(long, long, long, char*, double, double, double, double, double*);
    ...
    double *pdA = vector(0, NumPeriods), *pdEta = vector(1, NumPeriods),
           *pdCoupons = vector(1, NumPeriods), *pdInitCoupons = vector(1, NumPeriods),
    ...
    if (ErrMessage = UpdateCalibInstr(
            NumPeriods,
            pdInitCoupons,
            pszFreq,
            pszBasis,
    ...
    if ((ErrMessage =
             FitAllGFwdSwap(pTermStruct, pCalibInstr, pTermStruct->zdate, pTermStruct->zeta)) ||
        (ErrMessage = IsValidTermStructure(pTermStruct, NumPeriods)))
    {
        // if calibration fails
        // return max(intrinsic value, 0.)

        *ASOPrice = AmortSwapPV(
            NumPeriods,
            AsOfDate,
            FixDate,
    ...
    else
    {
        ...
        while ((averageGDiffSqr > termstructtol) && (counter++ < maxcounter))
        {
            ...
            if ((ErrMessage = FitAllGFwdSwap(
                     pTermStruct, pCalibInstr, pTermStruct->zdate, pTermStruct->zeta)) ||
                (ErrMessage = IsValidTermStructure(pTermStruct, NumPeriods)))
            {
                // if calibration fails
                // return max(intrinsic value, 0.)
            ...
            {
                averageGDiffSqr +=
                    (ptmpG[ntime] - pTermStruct->G[ntime]) * (ptmpG[ntime] - pTermStruct->G[ntime]);
            }
...
char* IsValidTermStructure(LGM_TS* pTermStruct, long NumPeriods)
{
    int   tmpind;
    char* ErrMessage = 0;
...
{
    char* AmortSwapFV(double, EASData*, double*);
    double       result;
...
void LGM1FZeroBond_DetermineBounds(
    long    AsOfDate,
    long    StartDate,
    long    EndDate,
    char*   pszYieldCurveName,
...
// free memory
void free_all_vectors(
    long    NumPeriods,
    double* pdA,
    double* ptmpG,
    double* pdEta,
    double* pdCoupons,
    double* pdInitCoupons,
    double* pdESOs)
{
    ...
    double* ASOPrice)
{
    ...
    *ASOPrice = 0.;
    ...
    {
        if (ErrMessage =
                (*GetVol)(AcrlStart, plAcrlEndDates[ntime], pdCoupons[ntime], boolIsLog, &vol))
        {
            return ErrMessage;
        }
...
char* AmortSwapFV(double state, EASData* _pMyData, double* pASFV)  // vector(1,NumPeriods)
{
    // func declaration
    char* LGM1FZeroBond(long, long, long, char*, double, double, double, double, double*);
...

{
    ...
    {
        pdEta[ntime] =
            (pdA[ntime] - pdCoverages[ntime] * sum) / (1. + pdCoverages[ntime] * pdCoupons[ntime]);
    ...
    double* pdFltNotionals)
{
    ...
    if (pdFltNotionals)
    {
        if (pdFixLegCoupons)
        {
            ...
            pdA[NumPeriods] =
                pdNotionals[NumPeriods] * pdCoverages[NumPeriods] * pdFixLegCoupons[NumPeriods] +
            ...
            for (ntime = 1; ntime < NumPeriods; ++ntime)
            {
                ...
                // pdNotionals[ntime+1];
                pdA[ntime] = pdNotionals[ntime] * pdCoverages[ntime] * pdFixLegCoupons[ntime] +
    ...
    else
    {
        ...
        {
            pdA[0]          = pdNotionals[1];
            pdA[NumPeriods] = pdNotionals[NumPeriods] *
                              (1. + pdCoverages[NumPeriods] * pdFixLegCoupons[NumPeriods]);

            for (ntime = 1; ntime < NumPeriods; ++ntime)
            {
                pdA[ntime] =
                    pdNotionals[ntime] * (1. + pdCoverages[ntime] * pdFixLegCoupons[ntime]) -
                    pdNotionals[ntime + 1];
            }
        }
        else
...
// ------------------------------------------------------------------------------------------------------------------
// // finds ystar, the state variable at which a swaption is ATM.  Uses bisection because the
// existing Newton is unstable Note that the function is decreasing in y
char* FindYstar(
    long n, double* a, double DStart, double zeta, double* Gpay, double Gst, double* pdYstar)
{
...
// Local Functions
const char* GetStrikes(
    int     bIsConstNotional,
    double  dCoupon,
    long    NumPeriod,
    double* c,
    double  zeta,
    ...
    double* pdYstar)
{
    ...
    if (bIsConstNotional)
    {
        for (j = 1; j < NumPeriod + 1; j++) /*we want to find the same y star for all vanilla
                                               swaptions by changing the strike*/
                                               ...
                                               swaptions by changing the strike*/
        {
            ...
            for (i = 1; i <= j; i++) /*value of the vanilla swap at maturity date*/
            {
                level += tCvg[i - 1] * tDFStartDate[i - 1] *
                         exp(-(ex_G - coupon_G[i]) * (ystar + 0.5 * (ex_G - coupon_G[i]) * zeta));
            }
            strikevalue[j] =
                (1 - tDFStartDate[j - 1] *
                         exp(-(ex_G - coupon_G[j]) * (ystar + 0.5 * (ex_G - coupon_G[j]) * zeta))) /
                level;
        }
    }
    return 0;
...
void freeMemory(long* x1, long n1)
{
    if (x1)
        free_lngvector(x1, 0, n1);
}

// char* PriceConstantNotional( long lNumPeriod, double dCoupon, double* out_pdPrice
    ...
    double  dMargin)
{
    ...
    for (i = 0; i < NumPeriod; i++)
    {
        //		/*K =*/ tPay[i] = add_unit(TheoEndDate, - (int) (( NumPeriod - i-1 ) * 12 /
        //(frequency+0.0)) , SRT_MONTH, NO_BUSDAY_CONVENTION); 		if (i<NumPeriod-1) tStartDate[i+1] =
        //add_unit(tPay[i], 0 , SRT_DAY, Conv); 		tEndDate[i] = add_unit(tPay[i], 0 , SRT_DAY, Conv);
        ...
        lvTheoEndDates[i] =
            add_unit(StartDate, (i + 1) * lMonthsPerPeriod, SRT_MONTH, NO_BUSDAY_CONVENTION);

        // Calculate the spread.  For the moment, we assume that the fixed and floating frequencies
        // are the same.  Use it to adjust the fixed coverage
        ...
        tCvg[i] =
            coverage(lvFixedStartDates[i], lvFixedEndDates[i], basis) - dSpread * dFltCvg / Coupon;
    ...
    CS.tEx      = lngvector(0, NumPeriod + 1);
    CS.tStart   = lngvector(0, NumPeriod + 1);
    CS.DStart   = dvector(0, NumPeriod + 1);
    CS.tPay     = lngvector(0, NumPeriod + 1);
    CS.cvgpay   = dvector(0, NumPeriod + 1);
    CS.Dpay     = dvector(0, NumPeriod + 1);
    CS.ifirst   = lngvector(0, NumPeriod + 1);
    CS.nlong    = lngvector(0, NumPeriod + 1);
    ...
    k = frequency / FloatFrequency; /* to take into account the case where frequency different from
                                       float frequency*/
    if (k < 1)
    ...
    for (j = 1; j < NumPeriod; j++)
    {
        if (FloatFrequency >= frequency)
        {
            ...
            c[j] = (dvNotionals[j - 1] * (1 + Coupon * tCvg[j - 1]) - dvNotionals[j]) /
                   dvNotionals[0] * tDFStartDate[j - 1]; /*discounted cash-flows from start date*/
        ...
        else
        {
            if (fmod(j, k) == 0)
            {
                ...
                c[j] = (dvNotionals[j - 1] * (1 + Coupon * tCvg[j - 1]) - dvNotionals[j - 1 + k]) /
                       dvNotionals[k - 1] *
    ...
    c[NumPeriod] = dvNotionals[NumPeriod - 1] * (1 + Coupon * tCvg[NumPeriod - 1]) /
                   dvNotionals[k - 1] *
    ...
    /* if the expiry < 1M and the strike is more than 5 std dev away, simply return the intrinsic
     * value */
    ...
    *dFloatPV = floatinglegPV;
    ...
    for (j = 1; j < 10 && iNotFinished; j++)
    {
        ...
        {
            // Check to see if the error is caused by short maturity, and if so, return the
            // intrinsic
        ...
        // Check that the term structure is reasonable.  If not use the previous value (if no
        // previous value throw an error)
        ...
        else
        {
            ...
            // get the strikes, ystar value.  If it is -infinity, then use the previous strike
            // values
            if (GetStrikes(
                    bIsConstNotional,
                    Coupon,
    ...
    for (j = 1; j < NumPeriod + 1;
         j++) /* once we get the strikes and notionals, we compute the vanilla swaptions prices*/
    {
        ...
        //*/ 			sqrt(zeta), coupon_G, ex_G); 	dvDiscPay[j] -= tDFnow[j-1];
    ...
    *pdFltAdj = 0.;
    ...
    for (indfix = 0; indfix < NumFixPeriods; ++indfix)
    {
        ...
        for (indflt = indfix * tmpinc; indflt < (indfix + 1) * tmpinc; ++indflt)
        {
            ...
            tmp = (/*pdFltSpreads[indflt] + */ pdMargins[indflt]) * pdFltCvgs[indflt] *
        ...
        fixedleglevel += swp_f_df(plFixedDates[indfix], plFixedDates[indfix + 1], pcYCName) *
                         pdFixedCvgs[indfix];
    }
    ...
    double* pdFltAdj)
{
    ...
    *pdFltAdj = 0.;
    ...
    for (indfix = 0; indfix < NumFixPeriods; ++indfix)
    {
        ...
        // pdFixedCvgs_adjusted[indfix] = pdFixedCvgs[indfix];// - tmpsum/(Coupon *
        // swp_f_df(plFixedDates[indfix],plFixedDates[indfix+1], pcYCName));
        fixedleglevel += pdFixedNotionals[indfix] *
                         swp_f_df(plFixedDates[indfix], plFixedDates[indfix + 1], pcYCName) *
                         pdFixedCvgs[indfix];
    }
                                   ...
                                   char *)
{
                ...
                {
                        throw "AdjustMargins(...): Floating frequency must not be smaller than fixed
frequency!";
                }

                // get ptrSchedule for fixed and floating legs
                SPP::TPtr_Schedule ptrFixedSchedule =
SPP::XSchedule::Create(static_cast<long>(start_date),static_cast<long>(end_date),freqFixed,
basisFixed, ptrCurrency, false ); SPP::TPtr_Schedule ptrFloatingSchedule =
SPP::XSchedule::Create(static_cast<long>(start_date),static_cast<long>(end_date),freqFloating,
basisFloating, ptrCurrency, false );


                // get NumPeriod
                unsigned short int num_fix_notionals = ptrFixedSchedule->m_vecDates_Payment.size();
                unsigned short int num_float_notionals =
ptrFloatingSchedule->m_vecDates_Payment.size();


                // check to see if number of margins equal number of flt leg payment periods
                if(*num_margins != num_float_notionals)
                {
                        throw XSort("Number of margins must equal number of floating leg payment
periods!");
                }
        ...
        {
        pdSpread[tmpind] = swp_f_spread(lvFixedStartDates[tmpind], lvFixedEndDates[tmpind],
ref_name); pdFltCvg[tmpind] = coverage(lvFixedStartDates[tmpind], lvFixedEndDates[tmpind],
basisFloating); pdFixCvg[tmpind] = coverage(lvFixedStartDates[tmpind],

...
/*
	static data for local volatility function defined to support old LGM1F pricer.
*/
static Err (*LGM1F_GetVolNew)(	
							char	*vol_curve_name,	
							double	start_date, 
							double	end_date,
							double	cash_strike,
...
/*
	init function for local volatility function defined to support old LGM1F pricer.
*/
static Err LGM1F_GetVolInit( 
								Err (*getVolNew)(	
													char	*vol_curve_name,	
													double	start_date, 
													double	end_date,
...
*/
static LGMErr LGM1F_GetCashLogVol( Date lStart, Date lEnd, double dStrike, SRT_Boolean boolIsLog, double *dVol)
{
/* local variables */
	SwapDP swap_dp;
...
/* call the new vol function */
	if ( err  = (*LGM1F_GetVolNew)(	
							LGM1F_szVolCurveName,	
							(double)	lStart, 
							(double)	lEnd,
							dStrike,
							0,
...
/* if the returned volatility is normal, convert it to log */
	...
	{
/* Set up the swap DP */
		if ( err = swp_f_setSwapDP( lStart, lEnd, LGM1F_MarkConv.sfreq, LGM1F_MarkConv.sbasis, &swap_dp) )
			return err;
		swap_dp.spot_lag = LGM1F_MarkConv.lag;

/* Calculate the CASH forward swap rate */
	if  ( err = swp_f_ForwardRate_SwapDP( &swap_dp, LGM1F_szYieldCurveName, "CASH", &dForward )  )
		return err;

/* Get the log vol */
	lFix = add_unit( swap_dp.start, -swap_dp.spot_lag, SRT_BDAY, SUCCEEDING);
	dExpiry = (lFix - LGM1F_lToday ) * YEARS_IN_DAY ;
	if (  err = srt_f_optbetavoltoblkvol( dForward, dStrike, *dVol, dExpiry, 0.0, dVol )  )
		return err;
...
Err EuropeanAmortizingSwaption2(long AsOfDate,
								long FixDate,
								long TheoEndDate,
								long NumPeriods,
...
*/								
								...
								double *dvReplicatingStrikes, 
								double *dvReplicatingNotionals, 
								double *dPrice, // price of 
								double *dFixedPV,
								double *dFloatPV,
								double *dSwapRate,
								SigKapTS **lgmSigKapTSPtrPtr,
								double *dLGMVol,
								...
								double dExerciseFee)
{
	...
	static char* EuropeanAmortizingSwaption_(long,long,long,long*,long*,double*,double*,long*,double,double*,char*,char*,char*,char*,char*,char*,LGMErr(*)(long,long,double,SRT_Boolean,double*),SRT_Boolean,LGM_TS*,LGMCalSet*,double*,double*,double*,double*,double*,double*,double*,double);
	void Init_CalibInstruments(long,long,long,long,long*,double*,char*,LGMCalSet*);
...
/* Set up the vol function */
	...
	{
		dvPayDates[ntime] = swp_f_df(AsOfDate,plPayDates[ntime],pszYieldCurveName);	
	}
	...
	*dFloatPV = 0;
	...
	{	
		for(ntime =1;ntime<=NumPeriods; ++ntime)
		{
			...
			*dFixedPV += pdNotionals[ntime]*Coupon*pFixCvgs[ntime]*dvPayDates[ntime];
			...
			//{
				...
				//fltinc += (pdFltSpreads[indflt] + pdMargins[indflt]) * 
							//pdFltCvgs[indflt] * swp_f_df(plFltDates[indflt],plFltDates[indflt+1], pszYieldCurveName);
			

				*dFloatPV += pdNotionals[ntime]*(dvPayDates[ntime-1]-dvPayDates[ntime]);
			//}
		}
	}
	else
	{
		...
		{
			// = Coupon*pFixCvgs[ntime]*dvPayDates[ntime] ;
			*dFixedPV += pdNotionals[ntime]*pdCoupons[ntime]*pFixCvgs[ntime]*dvPayDates[ntime];
			...
			//{
				...
				*dFloatPV += pdFltNotionals[ntime]*(dvPayDates[ntime-1]-dvPayDates[ntime]);
			//}
		}
	}
...
*/	
	...
	if(pTermStruct==0)
	{
		ErrMessage = "Failure to instantiate term structure occurred in EuropeanAmortizingSwaption2()!";
		return ErrMessage;
	}

	// initialize calibration instruments
	Init_CalibInstruments(NumPeriods,
						  AsOfDate,
						  FixDate,
						  plAcrlStartDates[1],
						  plPayDates,
	...
	if(ErrMessage=EuropeanAmortizingSwaption_(AsOfDate,
												FixDate,
												NumPeriods,
												plAcrlStartDates, 
												plAcrlEndDates,
	...
	{	
		...
		(*LGM1F_GetCashLogVol)(plAcrlStartDates[1],
				  plAcrlEndDates[ntime],
				  dvReplicatingStrikes[ntime-1],
				  boolIsLog,
				  &vol);

		 // Get ESO Price
		if(ErrMessage = swp_f_Swaption(	plAcrlStartDates[1],
										plPayDates[ntime],
										pszFreq,
										pszBasis,
										vol,
...
// temporarily commented out
	...
	*dLGMVol = (pTermStruct->G[0] - pTermStruct->G[1]) * sqrt(2.0* (pTermStruct->zeta[0])) * dLambda 
						* sqrt(dLambda / (exp(2.0*dLambda*dTex) - 1.0) ) 
						/ ( exp(-dLambda*dT0) - exp(-dLambda*dT1) );
...
static char *UpdateCalibInstr(	long NumPeriods, 
								double *pdCoupons,						
								char *pszFreq,
								char *pszBasis,
								char *pszPayRec,
...
static char* SetInitCalibInstr(	long NumPeriods, 
								double Coupons,						
								char *pszFreq,
								char *pszBasis,
								char *pszPayRec,
...
void LGMFreeLGM_CalibInst(long NumPeriods,
						   LGMCalSet *pLGMCalSet)
{
...
void Init_CalibInstruments(	long NumPeriods,
							long AsOfDate,
							long FixDate,
							long AcrlStartDate,
							long *plPayDates,
							...
							LGMCalSet *pLGMCalSet)
{
	...
	pLGMCalSet->nfirst = 1;		/* the swaptions are j = nfirst, nfirst+1, ..., nlast */
	pLGMCalSet->nlast = NumPeriods;			
	pLGMCalSet->cvgfix = pdCoverages[1];		/* cvgfix is the cvg for first period ftStart to tPay[ffirst] */
	pLGMCalSet->DfixStart = swp_f_df(AsOfDate,pLGMCalSet->ftStart,pszYieldCurveName);		/* Dfixst is discount factor at ftStart[i] */	
	...
	for(ntime=1;ntime<=NumPeriods; ++ntime)
	{
		...
		pLGMCalSet->Dpay[ntime]= swp_f_df(AsOfDate,pLGMCalSet->tPay[ntime],pszYieldCurveName);	
...
static char*  EuropeanAmortizingSwaption_(  long AsOfDate,
											long FixDate,
											long NumPeriods,
											long *plAcrlStartDates, // vector(1,NumPeriods), 
											long *plAcrlEndDates, // vector(1,NumPeriods), 											
											...
											SRT_Boolean boolIsLog,
											LGM_TS *pTermStruct,
											LGMCalSet *pCalibInstr,
											double *pdInitZeroBonds,
											double *ASOPrice,
											double *dvReplicatingStrikes,
											double *dvReplicatingNotionals,
											double *dvReplicatingSwaptions,
											double *pdFixLegCoupons,
											double *pdFltNotionals,
											double dExerciseFee)
{	
	// internal function declaration
	...
	void ComputeBondWeights (long,double,double*,double*,double*,double*,double*);
	double AmortSwapPV(long,long,long,long*,char*,double*); 
	static char* AmortSwaption(long,long,long*,char*,char*,char*,char*,char*,char*,double*,double*,SRT_Boolean,LGMErr(*GetVol)(long,long,double,SRT_Boolean,double*),double*);
	static char* UpdateCalibInstr(long,double*,char*,char*,char*,char*,char*,char*,LGMErr(*)(long,long,double,SRT_Boolean,double*),SRT_Boolean,LGMCalSet*,double*);
	static char* SetInitCalibInstr(long,double,char*,char*,char*,char*,char*,char*,LGMErr(*)(long,long,double,SRT_Boolean,double*),SRT_Boolean,LGMCalSet*);
	static char* LGMFindStateStar(long,long,long,long,long*,char*,double*,double,double,double*,double,double*,double*,double*,double,double,double,double*,double);
	void DetermineStateBounds(long,long,long,long*,char*,double,double*,double,double*);	
	static char* IsValidTermStructure(LGM_TS*, long); 
	static char* LGM1FZeroBond(long,long,long,char*,double,double,double,double,double*);
	
	void free_all_vectors(long,double*,double*,double*,double*,double*,double*);
	...
	double	*pdA = vector(0,NumPeriods),
			...
			*pdASFZeroBond=vector(1,NumPeriods),
			*ptmpG = vector(0,NumPeriods),
	...
	ComputeBondWeights(NumPeriods,Coupon,pdA,pdNotionals,pdCoveragesUnAdjusted, pdFixLegCoupons, pdFltNotionals);
	...
	if(ErrMessage = UpdateCalibInstr(	NumPeriods,
										pdInitCoupons,
										pszFreq,
										pszBasis,
										pszPayRec,
										...
										pdESOs))
	{
		...
		free_all_vectors(NumPeriods,pdA,ptmpG,pdEta,pdCoupons,pdInitCoupons,pdESOs);
		return ErrMessage;
	}
	
	// Calibrate using ESOs with strikes equal pdInitCoupons
	// just so that we have an initial term structure to start off with
	if((ErrMessage = FitAllGFwdSwap(pTermStruct,pCalibInstr, pTermStruct->zdate,pTermStruct->zeta))||
	   (ErrMessage = IsValidTermStructure(pTermStruct,NumPeriods)))	
	{
		...
		*ASOPrice = AmortSwapPV(NumPeriods, 
								AsOfDate,
								FixDate,
	...
	DetermineStateBounds(	NumPeriods,
							AsOfDate,
							plAcrlStartDates[1],
							plPayDates,
							pszYieldCurveName,
	...
	if(LGMFindStateStar( NumPeriods,
						 AsOfDate,
						 plAcrlStartDates[1],
						 plAcrlStartDates[1],				   
						 plPayDates,
	...
	{		
		...
		while((averageGDiffSqr>termstructtol)&&(counter++<maxcounter))
		{
			...
			ComputeESOStrikes(NumPeriods,pdCoveragesUnAdjusted,pdASFZeroBond,pdCoupons); 
			...
			for(ntime=0;ntime<=NumPeriods;++ntime)
			{
				averageGDiffSqr += (ptmpG[ntime] - pTermStruct->G[ntime]) *(ptmpG[ntime] - pTermStruct->G[ntime]);
			}
		...
		if (ErrMessage = AmortSwaption(	NumPeriods,
										 plAcrlStartDates[1],
										 plAcrlEndDates,
										 pszFreq,
										 pszBasis,
...
void Init_MyData(	EASData *_pMyData,
					long NumPeriods,
					long AsOfDate,
					long FutureDate,
					long AcrlStartDate,				   
...
void DetermineStateBounds(	long NumPeriods,
							long AsOfDate,
							long AcrlStartDate,				   
							long *plPayDates,// vector(1,NumPeriods)
							char *pszYieldCurveName,				   
...
{
	// func declaration
	void LGM1FZeroBond_DetermineBounds(long,long,long,char*,double,double,double,double*);
	...
	for(ntime = 1; ntime <= NumPeriods; ++ntime)
	{
		...
		LGM1FZeroBond_DetermineBounds(	AsOfDate,
										AcrlStartDate,
										plPayDates[ntime],
										pszYieldCurveName, 
										G_AcrlStartDate,
...
void LGM1FZeroBond_DetermineBounds(	long AsOfDate,
									long StartDate,
									long EndDate,
									char *pszYieldCurveName, 
									double G_StartDate,
...
#ifdef _DEBUG		
		LGM1FZeroBond(AsOfDate,StartDate,EndDate,pszYieldCurveName,*state,G_StartDate,G_EndDate,zeta,&price);
#endif //_DEBUG
...
static char* LGMFindStateStar(long NumPeriods,
								long AsOfDate,
								long FutureDate,
								long AcrlStartDate,				   
								long *plPayDates,// vector(1,NumPeriods)
...

{
	...
	void Init_MyData(EASData*,long,long,long,long,long*,char*,double*,double,double,double*,double,double*,double*,double*,double);
	...
	Init_MyData(&data,
				...
				nag_zero_cont_func_bd_1(
					...
					tol,
					&commParams,	// common block including parameters passed to objective function
...
// free memory
void free_all_vectors(long NumPeriods,double*pdA,double*ptmpG,double*pdEta,double*pdCoupons,double*pdInitCoupons,double*pdESOs)
{
...
static char* AmortSwaption(long NumPeriods,
						   long AcrlStart,
						   long *plAcrlEndDates, //vector(1,NumPeriods)
						   char *szFreq,
						   char *szBasis,
						   ...
						   double *ASOPrice)
{
	...
	*ASOPrice=0.;
	for (ntime=1;ntime<=NumPeriods;++ntime)	
	{
		...
		if(ErrMessage = (*GetVol)(AcrlStart,plAcrlEndDates[ntime],pdCoupons[ntime],boolIsLog,&vol))
		{	
			return ErrMessage;
		}
		
		
		if(ErrMessage= swp_f_Swaption(	AcrlStart,
										plAcrlEndDates[ntime],
										szFreq,
										szBasis,
										vol,
...
static char* AmortSwapFV(double state, EASData *_pMyData,double *pASFV) //vector(1,NumPeriods)
{
	// func declaration
	static char* LGM1FZeroBond(long,long,long,char*,double,double,double,double,double*);
	...
	if (ErrorMessage = LGM1FZeroBond(	_pMyData->m_AsOfDate,
										_pMyData->m_FutureDate,
										_pMyData->m_AcrlStartDate,
										_pMyData->m_pszYieldCurveName,
										state,
										_pMyData->m_G_FutureDate,
										_pMyData->m_G_AcrlStartDate,
										_pMyData->m_zeta,
										&zerobond))
	{
		return ErrorMessage;
	...
	*_pMyData->m_pdASFLevel=0;
	...
	for(ntime = 1; ntime <= _pMyData->m_NumPeriods; ++ntime)
	{
		if (ErrorMessage = LGM1FZeroBond(	_pMyData->m_AsOfDate,
											_pMyData->m_FutureDate,
											_pMyData->m_plPayDates[ntime],
											_pMyData->m_pszYieldCurveName,
											state,
											_pMyData->m_G_FutureDate,
											_pMyData->m_pdG[ntime],
											_pMyData->m_zeta,
											&zerobond))
		{
			return ErrorMessage;
...
double AmortSwapPV(long NumPeriods,
					long AsOfDate,
					long AcrlStartDate,				   
					long *plPayDates,// vector(1,NumPeriods)
					char *pszYieldCurveName,				   
					double *pdA)// vector(0,NumPeriods)				   
{
	...
	for(ntime = 1; ntime <= NumPeriods; ++ntime)
	{
		ASPV += pdA[ntime]*swp_f_df(AsOfDate,plPayDates[ntime],pszYieldCurveName);	
	}

	return ASPV;
}

static char* LGM1FZeroBond(	long AsOfDate,
						    long StartDate,
							long EndDate,
							char *pszYieldCurveName, 
							double state,
...
void ComputeESOStrikes(long NumPeriods,
					   double *pdCoverages,
					   double *pdASFZeroBond,
					   double *pdCoupons) //vector(1,NumPeriods)
{
...
void ComputeESOWeights(long NumPeriods,
					   double *pdEta,
					   double *pdA,	// vector(0,NumPeriods)	
					   ...
					   
{
	...
	pdEta[NumPeriods] = pdA[NumPeriods]/(1.+pdCoverages[NumPeriods]*pdCoupons[NumPeriods]);
	sum = pdCoupons[NumPeriods]*pdEta[NumPeriods]; 

	for(ntime = NumPeriods-1; ntime>=1; --ntime)
	{
		pdEta[ntime] = (pdA[ntime] - pdCoverages[ntime]*sum)/(1.+pdCoverages[ntime]*pdCoupons[ntime]);
						 ...
						 double *pdFltNotionals)		
{
	...
	if(pdFltNotionals)
	{
		if(pdFixLegCoupons)
		{
			...
			pdA[NumPeriods] = pdNotionals[NumPeriods]*pdCoverages[NumPeriods]*pdFixLegCoupons[NumPeriods]
							  + pdFltNotionals[NumPeriods];
			
			for (ntime=1; ntime<NumPeriods; ++ntime)
			{
				//pdA[ntime] = pdNotionals[ntime]*(1.+ pdCoverages[ntime]*Coupon) - pdNotionals[ntime+1];
				pdA[ntime] = pdNotionals[ntime] * pdCoverages[ntime]*pdFixLegCoupons[ntime] + pdFltNotionals[ntime] - pdFltNotionals[ntime+1];
			}			
		}
		else
		{
			pdA[0] = pdFltNotionals[1];
			pdA[NumPeriods] = pdNotionals[NumPeriods]*pdCoverages[NumPeriods]*Coupon
							  + pdFltNotionals[NumPeriods];
			
			for (ntime=1; ntime<NumPeriods; ++ntime)
			{
				//pdA[ntime] = pdNotionals[ntime]*(1.+ pdCoverages[ntime]*Coupon) - pdNotionals[ntime+1];
				pdA[ntime] = pdNotionals[ntime] * pdCoverages[ntime]*Coupon + pdFltNotionals[ntime] - pdFltNotionals[ntime+1];
			}			
	...
	else
	{
		...
		{
			pdA[0] = pdNotionals[1];
			pdA[NumPeriods] = pdNotionals[NumPeriods]*(1.+ pdCoverages[NumPeriods]*pdFixLegCoupons[NumPeriods]);
			
			for (ntime=1; ntime<NumPeriods; ++ntime)
			{
				pdA[ntime] = pdNotionals[ntime]*(1.+ pdCoverages[ntime]*pdFixLegCoupons[ntime]) - pdNotionals[ntime+1];
			}
		}
		else
		{
			pdA[0] = pdNotionals[1];
			pdA[NumPeriods] = pdNotionals[NumPeriods]*(1.+ pdCoverages[NumPeriods]*Coupon);
...
// ------------------------------------------------------------------------------------------------------------------ //
// finds ystar, the state variable at which a swaption is ATM.  Uses bisection because the existing Newton is unstable
// Note that the function is decreasing in y
static char* FindYstar(long n, double *a, double DStart, double zeta, double *Gpay, double Gst, double* pdYstar )
{
...
// Local Functions
static const char* GetStrikes( int bIsConstNotional, double dCoupon, long NumPeriod, double* c, double zeta, double* coupon_G, double ex_G, 
						double *tCvg, double* tDFStartDate, double* strikevalue, double* pdYstar )
{
	double level, ystar;
	int i, j;
	char* err;
						   
/*Compute the valueof the state variable for which the amortising swap goes to 0*/	
	...
	if (bIsConstNotional ) 
	{
		for ( j=1;j<NumPeriod +1; j++) /*we want to find the same y star for all vanilla swaptions by changing the strike*/
	...
	else
	{
		for ( j=1;j<NumPeriod +1; j++) /*we want to find the same y star for all vanilla swaptions by changing the strike*/
		{
			...
			{
				level += tCvg[i-1] * tDFStartDate[i-1] * exp(-(ex_G-coupon_G[i])*(ystar + 0.5*(ex_G-coupon_G[i])*zeta));
			}
			strikevalue[j] = (1 - tDFStartDate[j-1]*exp(-(ex_G-coupon_G[j])*(ystar + 0.5*(ex_G-coupon_G[j])*zeta)))/level;
		}
	}
	return 0;
}
...
//static char* PriceConstantNotional( long lNumPeriod, double dCoupon, double* out_pdPrice

// We need to alter the pricer such that it takes in schedules.

Err EuropeanAmortizingSwaption( long today,
							    long StartDate,
								long TheoEndDate,
								long NumPeriod,
...
// Check to see if it is a constant notional swap, and if so, return the standard swaption value
...
// calculate the swaption value
		(*GetVol)( StartDate,lvFixedEndDates[NumPeriod-1],Coupon,boolIsLog,&vol);
		swp_f_Swaption(	StartDate, TheoEndDate, szFreq, szBasis, vol, Coupon, szPayRec, szRefRate, szYieldCurveName,
						"PREMIUM", szVolType, dPrice);
...
// get the Swap details
	...
	for (i = 0; i<NumPeriod; i++)
	{
//		/*K =*/ tPay[i] = add_unit(TheoEndDate, - (int) (( NumPeriod - i-1 ) * 12 / (frequency+0.0)) , SRT_MONTH, NO_BUSDAY_CONVENTION);
//		if (i<NumPeriod-1) tStartDate[i+1] =  add_unit(tPay[i], 0 , SRT_DAY, Conv);
//		tEndDate[i] = add_unit(tPay[i], 0 , SRT_DAY, Conv);
//		tPay[i] = add_unit(tPay[i], 0 , SRT_BDAY, MODIFIED_SUCCEEDING);

// Calculate the theoretical end dates for each of the underlying swaps
		lvTheoEndDates[i] = add_unit(StartDate, (i+1)*lMonthsPerPeriod, SRT_MONTH, NO_BUSDAY_CONVENTION );


// Calculate the spread.  For the moment, we assume that the fixed and floating frequencies are the same.  Use it to 
// adjust the fixed coverage

		dSpread = swp_f_spread(lvFixedStartDates[i], lvFixedEndDates[i], szRefRate ) + dMargin;
		dFltCvg = coverage(lvFixedStartDates[i], lvFixedEndDates[i], FloatBasis);

		tCvg[i] = coverage(lvFixedStartDates[i], lvFixedEndDates[i], basis) - dSpread * dFltCvg / Coupon; 
...
//		tEnd[i] = tEndDate[i];
//		tPremiumDates[i] = add_unit(tStart[i], 0 , SRT_BDAY, MODIFIED_SUCCEEDING);
		tEx[i] = add_unit(lvFixedStartDates[i],-NoticePeriod, SRT_BDAY, MODIFIED_SUCCEEDING);						
		tExPremium[i] = 0.0;
		tStrike[i] = 1.0;
		tExPremium[i]=0.0;
		tRedFirstPay[i] = 0.0;
...
// Calibrate
	...
	k = frequency / FloatFrequency;	/* to take into account the case where frequency different from float frequency*/
	if ( k < 1)
		k=1;								/* if float frequency > frequency, nothing changes */
	

	floatinglegPV = dvNotionals[0] * DStart - dvNotionals[NumPeriod-1] * tDFnow[NumPeriod-1] ;
	fixedlegPV = dvNotionals[NumPeriod-1] * tCvg[NumPeriod-1] * tDFnow[NumPeriod-1] ;
	...
	for (j=1;j<NumPeriod ; j++)
	{
		...
		{
			b[j] = dvNotionals[j-1]*(1+tCvg[j-1]*Coupon)-dvNotionals[j]; /*cash-flows*/
			c[j] = (dvNotionals[j-1]*(1 + Coupon * tCvg[j-1])-dvNotionals[j])/dvNotionals[0]* tDFStartDate[j-1];/*discounted cash-flows from start date*/
		...
		else
		{
			...
			if (fmod(j,k)==0)
			{
				b[j] = dvNotionals[j-1]*(1+tCvg[j-1]*Coupon)-dvNotionals[j-1+k]; /*cash-flows*/
				c[j] = (dvNotionals[j-1]*(1 + Coupon * tCvg[j-1])-dvNotionals[j-1+k])/dvNotionals[k-1]* tDFStartDate[j-1];/*discounted cash-flows from start date*/
				floatinglegPV += (dvNotionals[j-1+k]-dvNotionals[j-1])/dvNotionals[k-1]* tDFnow[j-1];
				fixedlegPV += dvNotionals[j-1]* tCvg[j-1]* tDFnow[j-1];
			}
			else
			{
				b[j] = dvNotionals[j-1]*tCvg[j-1]*Coupon; /*cash-flows*/
				c[j] = dvNotionals[j-1]* Coupon * tCvg[j-1]/dvNotionals[k-1]* tDFStartDate[j-1];/*discounted cash-flows from start date*/
				floatinglegPV += 0;
				fixedlegPV += dvNotionals[j-1]* tCvg[j-1]* tDFnow[j-1];
	...
	b[NumPeriod] = dvNotionals[NumPeriod-1]*(1+tCvg[NumPeriod-1]*Coupon); /*cash-flows*/
	c[NumPeriod] = dvNotionals[NumPeriod-1]*(1 + Coupon * tCvg[NumPeriod-1])/dvNotionals[k-1]* tDFStartDate[NumPeriod-1];/*discounted cash-flows from start date*/
	
/* Create term structure */
	tsPtr = LGMCreateLGM_TS( 2, NumPeriod+1 );

/* if the expiry < 1M and the strike is more than 5 std dev away, simply return the intrinsic value */
...
//	and fill numG, the G dates, and the G values 
	...
	for ( j = 1; j<10 && iNotFinished; j++ )
	{
		for ( i=1; i<= NumPeriod; i++ )
		{
			...
			GetVol( CSPtr->tStart[i], CSPtr->tPay[i], CSPtr->Rfix[i], SRT_FALSE, &CSPtr->CEVLong[i] ); 
			CSPtr->Vfix[i] = LGMCEVSwaptionPrice( today, CSPtr->tEx[i], SRT_RECEIVER, CSPtr->Rfix[i], DStart, 
												0.0, 0.0, i, &CSPtr->cvgpay[1], &CSPtr->Dpay[1],
												CSPtr->CEVLong[i], beta );
		}

	
		if (  error = FitAllGFwdSwap(tsPtr, CSPtr, &zdatefix, &zetafix)  )
		{	
// Check to see if the error is caused by short maturity, and if so, return the intrinsic
			if ( (tEx[0]-today)/365.0 < .083 ) /*&& (fabs(coupon-SwapRate) > 5*dVol ) */
			{
				*dPrice = PayRec==SRT_PAYER ? *dFloatPV - *dFixedPV : *dFixedPV - *dFloatPV;
				if ( *dPrice < 0.0 ) *dPrice = 0.0;
				return 0;
			} 
...
// Check that the term structure is reasonable.  If not use the previous value (if no previous value throw an error)
...
// get the strikes, ystar value.  If it is -infinity, then use the previous strike values
			if ( GetStrikes( bIsConstNotional, Coupon, NumPeriod, c, zeta, coupon_G, ex_G, tCvg, tDFStartDate, 
								strikevalue, &ystar ) )
				iNotFinished = 0;
	...
	{
		A[j]= 1/(1+tCvg[j-1]*strikevalue[j])*tCvg[j-1]*(A[j+1]/tCvg[j] + b[j]/tCvg[j-1]-b[j+1]/tCvg[j]);
	...
	for (j=1;j<NumPeriod +1; j++)	/* once we get the strikes and notionals, we compute the vanilla swaptions prices*/
	{
	
		(*GetVol)(StartDate,lvFixedEndDates[j-1],strikevalue[j],boolIsLog,&vol);
			
		swp_f_Swaption(	StartDate,
						lvTheoEndDates[j-1],
						szFreq,
						szBasis,
						vol,
	...
	//	vecLGMPrice[j] = LGMRecVal(&vecYstar[j], j, dvDiscPay, DStart,	/* receiver swaption */ 
	//			sqrt(zeta), coupon_G, ex_G);
	//	dvDiscPay[j] -= tDFnow[j-1];
...
char* ZeroCouponSwaption_ComputeCoupon(	  long Today,
...
/*double NAG_CALL _ObjectiveFunction_ZC_Price_(double dX,Nag_User *pComm)
{
	...
	SExcelRange xlLGMZCPrice = XLZCEuropeanAmortSwaption(	pData->m_und_name,
															pData->m_mkt_name,
															pData->m_notperiod,
															pData->m_ref_name,
															pData->m_freq_name,
...
/*	// call ComputeCoverages 
	...
	ComputeCoverages( pData->m_Today,
					  pData->m_pYieldCurve,
					  pData->m_pdFixCov, // 0, NumFixPeriods -1
					  pData->m_plFixedDates, //0,NumFixPeriods
					  pData->m_lNumPeriods,
					  pData->m_pdFltCov ,// 0, NumFltPeriods -1
					  pData->m_pdSpread,// 0, NumFltPeriods -1
					  pData->m_pdMargins,
					  pData->m_plFltDates,									  
...
void ComputeCoverages(long Today,
					  char *pcYCName,
	...
	*pdFltAdj=0.;
	...
	for(indfix=0;indfix<NumFixPeriods;++indfix)
	{
		...
		for(indflt=indfix*tmpinc;indflt<(indfix+1)*tmpinc;++indflt)
		{
			...
			tmp = (/*pdFltSpreads[indflt] + */pdMargins[indflt]) * pdFltCvgs[indflt] *swp_f_df(plFltDates[indflt],plFltDates[indflt+1], pcYCName);
			fltleglevel +=tmp	; 
		...
		//pdFixedCvgs_adjusted[indfix] = pdFixedCvgs[indfix];// - tmpsum/(Coupon * swp_f_df(plFixedDates[indfix],plFixedDates[indfix+1], pcYCName));
		fixedleglevel += swp_f_df(plFixedDates[indfix],plFixedDates[indfix+1], pcYCName) * pdFixedCvgs[indfix];
	}
...
// preliminary version
// will come back later ... 
void ComputeCoverages2(long Today,
					  char *pcYCName,
					  ...
					  double *pdFltAdj)
{
	...
	*pdFltAdj=0.;
	...
	for(indfix=0;indfix<NumFixPeriods;++indfix)
	{
		...
		{
			// looks though plFltDates might be out of bound, but this is not true 
			tmp = (/*pdFltSpreads[indflt] + */pdMargins[indflt]) * pdFltCvgs[indflt] * pdFixedNotionals[indfix]*swp_f_df(plFltDates[indflt],plFltDates[indflt+1], pcYCName);
			fltleglevel +=tmp	; 
			//*pdFltAdj += tmp*pdFltNotionals[indflt];
		...
		//pdFixedCvgs_adjusted[indfix] = pdFixedCvgs[indfix];// - tmpsum/(Coupon * swp_f_df(plFixedDates[indfix],plFixedDates[indfix+1], pcYCName));
		fixedleglevel += pdFixedNotionals[indfix]*swp_f_df(plFixedDates[indfix],plFixedDates[indfix+1], pcYCName) * pdFixedCvgs[indfix];
	}
				   ...
				   char *)
{
		...
		swp_f_get_ref_rate_details(ref_name, &basisFloating, &freqFloating);
		

		// 12/ ???
		if(freqFloating > freqFloating)
		{
			throw "AdjustMargins(...): Floating frequency must not be smaller than fixed frequency!";
		}

		// get ptrSchedule for fixed and floating legs 
		SPP::TPtr_Schedule ptrFixedSchedule = SPP::XSchedule::Create(static_cast<long>(start_date),static_cast<long>(end_date),freqFixed, basisFixed, ptrCurrency, false );
		SPP::TPtr_Schedule ptrFloatingSchedule = SPP::XSchedule::Create(static_cast<long>(start_date),static_cast<long>(end_date),freqFloating, basisFloating, ptrCurrency, false );


		// get NumPeriod
		unsigned short int num_fix_notionals = ptrFixedSchedule->m_vecDates_Payment.size();
		unsigned short int num_float_notionals = ptrFloatingSchedule->m_vecDates_Payment.size();


		// check to see if number of margins equal number of flt leg payment periods
		if(*num_margins != num_float_notionals)
		{
			throw XSort("Number of margins must equal number of floating leg payment periods!");
		}
		...
		// fixed leg dates
		long	*lvFixedStartDates = ptrFixedSchedule->m_vecDates_Calculation.data();
		long	*lvFixedEndDates = lvFixedStartDates + 1;

		// floating leg dates
		long	*lvFloatingStartDates = ptrFloatingSchedule->m_vecDates_Calculation.data();
	...
	{
	pdSpread[tmpind] = swp_f_spread(lvFixedStartDates[tmpind], lvFixedEndDates[tmpind], ref_name);
	pdFltCvg[tmpind] = coverage(lvFixedStartDates[tmpind], lvFixedEndDates[tmpind], basisFloating);
	pdFixCvg[tmpind] = coverage(lvFixedStartDates[tmpind], lvFixedEndDates[tmpind],basisFixed);
	}
}*/

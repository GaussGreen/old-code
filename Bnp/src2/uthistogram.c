...
Err srt_f_addhistdata(String name,SrtHistData *data)
{
	...
	err = srt_f_lstins(_srt_global_histlistptr, name, 0.0,
		OBJ_PTR_HIST, (void*)     data,		&srt_f_frehistval, &ticker);
		...
		double ***histo_values) /* [x][y] */
{
	...
	/* Gets max and min to make sure that there is more than one value for x (or y) */
	...
	/* Memory allocation */
		...
		{
			if (*histo_x) free_dvector((*histo_x), 0, *x_size - 1);

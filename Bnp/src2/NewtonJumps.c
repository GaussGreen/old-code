        ...
        for (n2 = 0; n2 < N2max; n2++)
        {
            ...
            if ((proba1p * proba2p > probainf) || (proba1 * proba2 > probainf))
            {
                d1up = (log(dFwd / strike1) - U1 * lambda1 * T - U2 * lambda2 * T +
                       ...
                       (sigma * sqrt(T));
                d1upsp = (log(dFwd / (strike1 + spread)) - U1 * lambda1 * T - U2 * lambda2 * T +
                          n1 * log(1 + U1) + n2 * log(1 + U2) + sigma * sigma * T / 2) /
                         (sigma * sqrt(T));
                d1dn = (log(dFwd / strike2) - U1 * lambda1 * T - U2 * lambda2 * T +
                        n1 * log(1 + U1) + n2 * log(1 + U2) + sigma * sigma * T / 2) /
                       (sigma * sqrt(T));
                d1dnsp = (log(dFwd / (strike2 - spread)) - U1 * lambda1 * T - U2 * lambda2 * T +
                          n1 * log(1 + U1) + n2 * log(1 + U2) + sigma * sigma * T / 2) /
                         (sigma * sqrt(T));
...
double PrixDoubleDigitMerton(
    double forward, double* param, double mat, double K, double width, double eps)

{
    ...
    double probasauts;
    double
        probainf; /* Threshold according to  which we consider that more jumps are very unlikely */
    double prix;
        ...
        for (N2 = 0; N2 <= N2max; N2++)
        {
            ...
            if (probasauts > probainf)
            {
                ...
                d1      = (param[3] * param[2] * mat + param[5] * param[4] * mat +
                      ...
                      log(strike1 /
                          (forward * exp(N1 * log(1 + param[2]) + N2 * log(1 + param[4]))))) /
                     (param[1] * sqrt(mat));
...
double DerivPrixDoubleCallSpreadMerton(
    double forward, double* param, double mat, double K, double width, double eps)

{
    ...
    for (n1 = 0; n1 < N1max; n1++)
    {
        for (n2 = 0; n2 < N2max; n2++)
        {
            ...
            if ((proba1p * proba2p > probainf) || (proba1 * proba2 > probainf))
            {
                ...
                premium1upsp += proba1p * proba2p * (-1 / ((strike1 + spread) * sigma * sqrt(T))) *
                                exp(-d1upsp * d1upsp / 2) / sqrt(2 * SRT_PI);
                premium2upsp += proba1 * proba2 * (-1 / ((strike1 + spread) * sigma * sqrt(T))) *
                ...
                premium1dnsp += proba1p * proba2p * (-1 / ((strike2 - spread) * sigma * sqrt(T))) *
                                exp(-d1dnsp * d1dnsp / 2) / sqrt(2 * SRT_PI);
                premium2dnsp += proba1 * proba2 * (-1 / ((strike2 - spread) * sigma * sqrt(T))) *
    ...
    deriv = (dFwd * premium1dnsp - (strike2 - spread) * premium2dnsp - premium3dnsp) -
...
double DerivPrixDoubleDigitMerton(
    double forward, double* param, double mat, double K, double width, double eps)
    ...
    for (N1 = 0; N1 <= N1max; N1++)
        for (N2 = 0; N2 <= N2max; N2++)
        {
            ...
            if (probasauts > probainf)
            {
                ...
                deriv += probasauts / (param[1] * sqrt(mat)) *
    ...
    double   callspread)
{
    ...
    for (i = 2; i <= nmat; i++) /* Computes the approximate price of the range digital everyday,
        ...
        prix +=
            PrixDoubleCallSpreadMerton(DRS[i], param[i], mat[i], DRS[1], width, callspread, eps) *
            (1 + 2 * IsFriday[i]);
    return prix;
}
...
double DerivPrixTimeSwap(
    double eps, double* DRS, int nmat, double* mat, double* IsFriday, double** param, double width)
{
    double deriv;
    int    i;
    ...
    deriv = 0;
    for (i = 2; i <= nmat; i++)
        deriv += DerivPrixDoubleCallSpreadMerton(DRS[i], param[i], mat[i], DRS[1], width, eps) *
                 (1 + 2 * IsFriday[i]);
    return (-deriv);
}

double Optimise_Time_Swap(
    int      nmat,
    double*  DRS,
    double   width,
    double   lowermin,

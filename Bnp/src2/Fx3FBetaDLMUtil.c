...
Err	FxBetaDLM_get_C0_from_beta(	double	TStar,
								double	beta,
								double	*C0)
{
	*C0 = beta;
...
Err	FxBetaDLM_get_beta_from_C0(	double	TStar,
								double	C0,
								double	*beta)
{
	*beta = C0;
...
Err	FxBetaDLM_get_C0_from_EquiBeta(	double	TStar,
									double	B0,
									double	beta,
									double	C0[3])
{
...
Err err;
	...
	err = FxBetaDLM_get_limit_beta(	TStar,
									B0,
									&C0[2]);

	if(beta < C0[2])
	{
		err = "FxBetaDLM_get_C0_from_EquiBeta: the beta required is too low for the TStar and B0 entered";
		C0[0]=0;
		C0[1]=0;
	}
	var = 0.01 * TStar;
	...
	else
	{
		/* First Solution */
		counter = 0;
		Guess = (1.0 - sqrt(1.0 - 4.0 * var * B0 * B0 * (beta - 1.0) * (beta - 1.0)))/(4.0 * var * (beta - 1.0));
		do
		{
			...
			Function = (B0 * B0 + 2.0 * Guess * log(1.0 + 2.0 * Guess * var)) * (beta - 1.0) - 2.0 * Guess;
			Derivative = (beta - 1.0) * (2.0 * log(1.0 + 2.0 * Guess * var) + 4.0 * Guess * var / (1.0 + 2.0 * Guess * var)) - 2.0;
		...
		C0[0] = Guess; 
		if(counter >= Limit)	err = "FxBetaDLM_get_C0_from_EquiBeta: did not converge";	
...
Err	FxBetaDLM_get_EquiBeta_from_C0(	double	TStar,
									double	B0,
									double	C0,
									double	*beta)
{
...
Err FxBetaDLM_get_limit_beta(	double	TStar,
								double	B0,
								double	*BetaLimit)
{
double var;
...
Err err;
	...
	if(TStar == 0.0) *BetaLimit = -10E6;
	else
	{
		C0 = 0.25 * (B0 * B0 * var - sqrt(B0 * B0 * var * B0 * B0 * var + 4 * B0 * B0 * var)) / var;
		err = FxBetaDLM_get_EquiBeta_from_C0(	TStar,
												B0,
												C0,
												BetaLimit);
	}
...
void FxBetaDLM_free_str(FXBETADLM_STR	str)
{
	if (str)
	{
		if (str->time_dom) free (str->time_dom);
		str->time_dom = NULL;

		if (str->sig_dom) free (str->sig_dom);
		str->sig_dom = NULL;

		if (str->time_for) free (str->time_for);
		str->time_for = NULL;

		if (str->sig_for) free (str->sig_for);
		str->sig_for = NULL;

		if (str->time_fx) free (str->time_fx);
		str->time_fx = NULL;

		if (str->sig_fx) free (str->sig_fx);
		str->sig_fx = NULL;

		if (str->time_3F_corr) free (str->time_3F_corr);
		str->time_3F_corr = NULL;

		if (str->dom_for_3F_corr) free (str->dom_for_3F_corr);
		str->dom_for_3F_corr = NULL;

		if (str->dom_fx_3F_corr) free (str->dom_fx_3F_corr);
		str->dom_fx_3F_corr = NULL;

		if (str->for_fx_3F_corr) free (str->for_fx_3F_corr);
		str->for_fx_3F_corr = NULL;
		
		if (str->time_DLM_corr) free (str->time_DLM_corr);
		str->time_DLM_corr = NULL;

		if (str->dom_for_DLM_corr) free (str->dom_for_DLM_corr);
		str->dom_for_DLM_corr = NULL;

		if (str->dom_fx_DLM_corr) free (str->dom_fx_DLM_corr);
		str->dom_fx_DLM_corr = NULL;
				
		if (str->for_fx_DLM_corr) free (str->for_fx_DLM_corr);
		str->for_fx_DLM_corr = NULL;

		if (str->dom_fx_DLM_corr_down) free (str->dom_fx_DLM_corr_down);
		str->dom_fx_DLM_corr_down = NULL;
				
		if (str->for_fx_DLM_corr_down) free (str->for_fx_DLM_corr_down);
		str->for_fx_DLM_corr_down = NULL;

		if (str->yc_dom) free (str->yc_dom);
		str->yc_dom = NULL;

		if (str->yc_for) free (str->yc_for);
		str->yc_for = NULL;
	}
}


Err FxBetaDLM_fill_str(	char			*undname_fx,		/* und name */
...
Err				err = NULL;
	...
	err = Get_LGM_TermStructure2(	undname_dom,
									&(str->time_dom),
									&(str->sig_dom),
									&(str->nb_dom),
									&(str->tau_dom));
	...
	err = Get_LGM_TermStructure2(	undname_for,
									&(str->time_for),
									&(str->sig_for),
									&(str->nb_for),
									&(str->tau_for));
	...
	if (!compute_3F_corr)
	{
		if (nb_3F_corr == 0)
		{
			...
			if (!str->time_3F_corr || !str->dom_for_3F_corr || !str->dom_fx_3F_corr || !str->for_fx_3F_corr)
			{
			...
			for (i=0; i<str->nb_3F_corr; i++)
			{
				...
				err = srt_f_get_corr_from_CorrList(cls, name_dom, name_for, str->time_3F_corr[i], &(str->dom_for_3F_corr[i]));

				if (err) goto FREE_RETURN;

				err = srt_f_get_corr_from_CorrList(cls, name_dom, undname_fx, str->time_3F_corr[i], &(str->dom_fx_3F_corr[i]));

				if (err) goto FREE_RETURN;

				err = srt_f_get_corr_from_CorrList(cls, name_for, undname_fx, str->time_3F_corr[i], &(str->for_fx_3F_corr[i]));
 	...
 	if (!compute_DLM_corr)
	{
		if(nb_DLM_corr > 0)
		{
			...
			if (!str->time_DLM_corr || !str->dom_for_DLM_corr || !str->dom_fx_DLM_corr || !str->for_fx_DLM_corr ||
	...
	if (compute_DLM_corr)
	{
		FxBetaDLM_getDLMCorrelFrom3F(	str,
										NumParams);
	}
...
Err SrtInitFXBetaDLMUnd(char			*undname_fx,		/* und name */					
						char			*undname_dom,		/* domestic underlying name */
						char			*undname_for,		/* foreign underlying name */
...
Err				err = NULL;
	...
	/* Fill the structure */
	err = FxBetaDLM_fill_str(	undname_fx,		/* und name */
...
Err FxBetaDLM_get_struct_from_und(char			*und,
								  FXBETADLM_STR	*str)
{
Err err = NULL;
SrtUndPtr pUndDesc;
...
Err	FxBetaDLM_Get_TermStruct(	char	*und,
								long	*today,
								double	*tau_dom,
								int		*nb_dom,
								double	**time_dom,
...
Err	err = NULL;
	...
	*for_fx_DLM_corr_down = calloc(*nb_DLM_corr, sizeof(double));
	...
	if (!time_dom || !sig_dom || !time_for || !sig_for || !time_fx || !sig_fx || 
		!time_3F_corr || !time_DLM_corr ||
		!dom_for_3F_corr || !dom_fx_3F_corr || !for_fx_3F_corr ||
		!dom_for_DLM_corr || !dom_fx_DLM_corr || !for_fx_DLM_corr ||
		!dom_fx_DLM_corr_down || !for_fx_DLM_corr_down)
	{
...
Err	init_FxBetaDLM_model(	double			MinTimeCorrel,
...
double	t, dt;
	...
	model->dCashFx = spot_fx 
		* swp_f_df (today, spot_date, yc_dom)
		/ swp_f_df (today, spot_date, yc_for);

	err = FxBetaDLM_get_beta_from_C0(	tstar,
										C0,
										&model->dEquiBeta);
	...
	for (i = 0; i < model->iNbPWTime; i++)
	{
		if (!i)
		{
			if ( model->dPWTime[i] > MinTimeCorrel )
			{
				...
				for(j = 0; j < n; j++)
				{
					num_f_add_number(&NbTime, &model->dPWTime, (double)(((long) (t * DAYS_IN_YEAR)) * YEARS_IN_DAY));
					t += dt;
				}
			}			
		}
	...
	if (!model->dSigmaDom || !model->dSigmaFor || !model->dSigmaFx3F || 
		!model->dSigmaFx || !model->dSigmaFxUp || !model->dSigmaFxDown || 
		!model->dCorrDomFor || !model->dCorrDomFx || !model->dCorrForFx ||
		!model->dCorrDomFxDown || !model->dCorrForFxDown ||
	...
	for (i=0; i<model->iNbPWTime; i++)
	{
		...
		{
			model->dSigmaFxUp[i] = sig_fx[index] * exp( model->dAlpha * sqrt((1.0 - exp(- 2.0 * model->dLambda * time_fx[index]))/(2.0 * model->dLambda)));
			model->dSigmaFxDown[i] = sig_fx[index] * exp(-model->dAlpha * sqrt((1.0 - exp(- 2.0 * model->dLambda * time_fx[index]))/(2.0 * model->dLambda)));				
		}
...
void free_FxBetaDLM_model(FxBetaDLM_model	*model)
{
	if (model)
	{
		if (model->dPWTime) free(model->dPWTime);
		model->dPWTime = NULL;

		if (model->dSigmaDom) free(model->dSigmaDom);
		model->dSigmaDom = NULL;

		if (model->dSigmaFor) free(model->dSigmaFor);
		model->dSigmaFor = NULL;

		if (model->dSigmaFx3F) free(model->dSigmaFx3F);
		model->dSigmaFx3F = NULL;

		if (model->dSigmaFx) free(model->dSigmaFx);
		model->dSigmaFx = NULL;

		if (model->dSigmaFxUp) free(model->dSigmaFxUp);
		model->dSigmaFxUp = NULL;

		if (model->dSigmaFxDown) free(model->dSigmaFxDown);
		model->dSigmaFxDown = NULL;

		if (model->dCorrDomFor) free(model->dCorrDomFor);
		model->dCorrDomFor = NULL;

		if (model->dCorrDomFx) free(model->dCorrDomFx);
		model->dCorrDomFx = NULL;

		if (model->dCorrForFx) free(model->dCorrForFx);
		model->dCorrForFx = NULL;

		if (model->dCorrDomFxDown) free(model->dCorrDomFxDown);
		model->dCorrDomFxDown = NULL;

		if (model->dCorrForFxDown) free(model->dCorrForFxDown);
		model->dCorrForFxDown = NULL;

		if (model->dCorrDomFxInput) free(model->dCorrDomFxInput);
		model->dCorrDomFxInput = NULL;

		if (model->dCorrForFxInput) free(model->dCorrForFxInput);
		model->dCorrForFxInput = NULL;
	}
}

Err	FxBetaDLM_Get_Model(char			*und,
						FxBetaDLM_model	*model)
{
Err	err = NULL;
FXBETADLM_STR str;

	err = FxBetaDLM_get_struct_from_und(und,
								 		&str);
...
void free_FxBetaDLM_Hermite(FxBetaDLM_Hermite *hermite)
{
	if (hermite)
	{
		if (hermite->W) free (hermite->W);
		hermite->W = NULL;

		if (hermite->X) free (hermite->X);
		hermite->X = NULL;
	}
}
...
*/
	gauleg(NumParams->dLowerBound, NumParams->dUpperBound, hermite->X, hermite->W, hermite->iNbPoints);	
...
Err err = NULL;
	...
	if (!Precalc->dVarRatesDom || !Precalc->dVarRatesFor || !Precalc->dVarRatesFor3D || !Precalc->dCovarRates || !Precalc->dCovarRates3D || 
		!Precalc->dVarFFx || !Precalc->dCovarFFxDom || !Precalc->dCovarFFxFor || !Precalc->dCovarFFxFor3D || !Precalc->dVarX ||
		!Precalc->dExpectFor || !Precalc->dIntegral || !Precalc->dIntegral2 || !Precalc->dCovarRatesAdjust ||
		!Precalc->dVarRatesFor_down || !Precalc->dVarRatesFor3D_down || !Precalc->dCovarRates_down || !Precalc->dCovarRates3D_down || 
		!Precalc->dVarFFx_down || !Precalc->dCovarFFxDom_down || !Precalc->dCovarFFxFor_down || !Precalc->dCovarFFxFor3D_down || !Precalc->dVarX_down ||
		!Precalc->dExpectFor_down || !Precalc->dIntegral_down || !Precalc->dIntegral2_down || !Precalc->dCovarRatesAdjust_down ||
		!Precalc->dVarRatesFor_mid || !Precalc->dVarRatesFor3D_mid || !Precalc->dCovarRates_mid || !Precalc->dCovarRates3D_mid || 
		!Precalc->dVarFFx_mid || !Precalc->dCovarFFxDom_mid || !Precalc->dCovarFFxFor_mid || !Precalc->dCovarFFxFor3D_mid || !Precalc->dVarX_mid ||
		!Precalc->dExpectFor_mid || !Precalc->dIntegral_mid || !Precalc->dIntegral2_mid || !Precalc->dCovarRatesAdjust_mid)
	{
...
Err quadratic_bs(	double				constant_coef,
					double				linear_coef,
					...
					double				*Premium)
{
	...
	if(variance==0.0)
	{
		if(callput == SRT_CALL)
		{
			*Premium = exp(constant_coef + linear_coef * mean + quadratic_coef * mean * mean) - strike;
			if(*Premium<0)
			{
				*Premium = 0;
			}
		...
		{
			*Premium = strike - exp(constant_coef + linear_coef * mean + quadratic_coef * mean * mean);
			if(*Premium<0)
			{
				*Premium = 0;
			}		
	...
	else
	{
		...
		{
			err = serror("quadratic_bs  The process has infinite variance, C = %f > %f"
				, quadratic_coef
				, 0.5 / variance);
			goto FREE_RETURN;			
		}
		...
		if(quadratic_coef == 0)
		{
			...
			if(callput == SRT_CALL)
			{
				J1 = exp( constant_coef - mean * mean / (2.0 * variance) + (linear_coef * variance + mean) * (linear_coef * variance + mean) / (2.0 * variance));
		...
		else
		{
			...
			if(delta<0 )
			{
				if(callput == SRT_CALL)
				{
					...
					else
					{
						...
						*Premium = exp( constant_coef - mean * mean /  (2.0 * variance) + k1 * k1 / (2 * k2 * k2)) / sqrt(variance) /k2;
						*Premium += - strike;
					}
				}
				else
			...
			else
			{
				...
				J1 *= exp( constant_coef - mean * mean /  (2.0 * variance) + k1 * k1 / (2 * k2 * k2)) / sqrt(variance) /k2;
				J2 *= strike; 
				if(callput == SRT_CALL)
				{
					*Premium = J1-J2;
...
Err FxBetaDLM_correl_mapping_forward(	double			CorrelTolerance,
										double			t,
										double			previous_t,
										double			B0,
										double			C0,
...
double	b, c;
double	delta;
double	RhoDX1, RhoFX1, RhoDomFFx1, RhoForFFx1, sigFFx1, varFFx1, B1, C1, volS1, volX2_1, RhoDS1, RhoFS1;
double	RhoDX2, RhoFX2, RhoDomFFx2, RhoForFFx2, sigFFx2, varFFx2, B2, C2, volS2, volX2_2, RhoDS2, RhoFS2;
...
Err err = NULL;
	...
	sigFFx = Sigma3F * Sigma3F + gammaDom * gammaDom + gammaFor * gammaFor 
		+ 2.0 * RhoFS * Sigma3F * gammaFor
		- 2.0 * RhoDS * Sigma3F * gammaDom
	...
	/* First solution */
	RhoDX1 = -b + sqrt(delta);
	RhoFX1 = RhoForFFx / RhoDomFFx * (RhoDX1 + RhoDF * gammaFor - gammaDom) - (gammaFor - RhoDF * gammaDom);
	...
	/* Second solution */
	RhoDX2 = -b - sqrt(delta);
	RhoFX2 = RhoForFFx / RhoDomFFx * (RhoDX2 + RhoDF * gammaFor - gammaDom) - (gammaFor - RhoDF * gammaDom);
	...
	if(admissible1 && (fabs(RhoDomFFx1-RhoDomFFx)> CorrelTolerance || fabs(RhoForFFx1-RhoForFFx)> CorrelTolerance))
		admissible1 =0;

	if(admissible2 && (fabs(RhoDomFFx2-RhoDomFFx)> CorrelTolerance || fabs(RhoForFFx2-RhoForFFx)> CorrelTolerance))
		admissible2 =0;
	...
	if(admissible1 && admissible2)
	{
		...
		varFFx1 += SigmaFor * SigmaFor * Psi_Func(LambdaFor, LambdaFor, TStar, previous_t, t)
				...
				- 2.0 * RhoDF * SigmaDom * SigmaFor * Psi_Func(LambdaDom, LambdaFor, TStar, previous_t, t);
		...
		volS1 = B1 * B1 * SigmaDLM * SigmaDLM + (B1 - 1.0) * (B1 - 1.0) * (gammaFor * gammaFor + gammaDom * gammaDom);
		volS1 += 2.0 * (B1 - 1.0) * (B1 * (RhoFX1 * gammaFor - RhoDX1 * gammaDom) - (B1 - 1.0) * RhoDF * gammaDom * gammaFor);
		...
		volS2 = B2 * B2 * SigmaDLM * SigmaDLM + (B2 - 1.0) * (B2 - 1.0) * (gammaFor * gammaFor + gammaDom * gammaDom);
		volS2 += 2.0 * (B2 - 1.0) * (B2 * (RhoFX2 * gammaFor - RhoDX2 * gammaDom) - (B2 - 1.0) * RhoDF * gammaDom * gammaFor);
	...
	err = FxBetaDLM_CheckOrMakePosMatrix(	RhoDF,
											RhoDX,
											RhoFX);
	...
	{
		sigFFx1 = SigmaDLM * SigmaDLM + gammaDom * gammaDom + gammaFor * gammaFor 
			+ 2.0 * *RhoFX * SigmaDLM * gammaFor
			- 2.0 * *RhoDX * SigmaDLM * gammaDom
			- 2.0 * RhoDF * gammaDom * gammaFor;
		...
		if(sigFFx < 0.0 || fabs(RhoDomFFx1-RhoDomFFx)> CorrelTolerance || fabs(RhoForFFx1-RhoForFFx)> CorrelTolerance)
		{
			err = "FxBetaDLM_correl_mapping: No solution for correlation mapping after adapting the correlations";
			goto FREE_RETURN;	
		}
	}
...
Err FxBetaDLM_correl_mapping8(	double			t,
								double			TStar,
								double			SigmaDom,
								double			LambdaDom,
								double			SigmaFor,
...
Err err = NULL;
	...
	*RhoDX = -b + sqrt(delta);
	*RhoFX = RhoForFFx / RhoDomFFx * (*RhoDX + RhoDF * gammaFor - gammaDom) - (gammaFor - RhoDF * gammaDom);
...
Err FxBetaDLM_correl_mapping(	double			volX,
								double			B0,
								double			C0,
								double			TStar,
								double			SigmaDom,
	...
	do
	{
		...
		*RhoFX = RhoFX1;
		...
		if (fabs(RhoDS) > 1.0E-08)
		{
			/* solve polynomial */
			a = 1.0;
			b = vol_dom + RhoDF * vol_for + RhoDS * (B - 1.0 + Const2) * (RhoDS * volDom - RhoFS * volFor);
			c = pow(RhoDF * vol_for + vol_dom,2) - pow(RhoDS * volX, 2) * (B * B + Const1)
				- RhoDS * RhoDS * ((B - 1.0) * (B - 1.0) + Const1) * (volFor * volFor - 2.0 * RhoDF * volDom * volFor + volDom * volDom)
				- 2.0 * RhoDS * vol_for * (B - 1.0 + Const2) * (RhoFS * (volFor * RhoDF - volDom) - RhoDS * (volFor - volDom * RhoDF));
...
/*			if (fabs(RhoDX1) > vol_X * 0.999)
			...
			RhoFX1 = RhoFS / RhoDS * (vol_dom + RhoDF * vol_for + RhoDX1) - RhoDF * vol_dom - vol_for;
...
/*			if (fabs(RhoFX1) > vol_X * 0.999)
		...
		VarFFx += -2.0 * RhoFX1 * SigmaFor * volX * Etha_Func(LambdaFor, TStar, previous_t, middle_t)
				+ 2.0 * RhoDX1 * SigmaDom * volX * Etha_Func(LambdaDom, TStar , previous_t, middle_t);
		VarFFx += SigmaFor * SigmaFor * Psi_Func(LambdaFor, LambdaFor, TStar, previous_t, middle_t)
				+ SigmaDom * SigmaDom * Psi_Func(LambdaDom, LambdaDom, TStar, previous_t, middle_t)
				- 2.0 * RhoDF * SigmaDom * SigmaFor * Psi_Func(LambdaDom, LambdaFor, TStar, previous_t, middle_t);
	...
	while((fabs(*RhoDX-RhoDX1) > tolerance && fabs(*RhoFX-RhoFX1) > tolerance) || iteration < 2);
...
/* Finds the Fix Point Solution */
Err FxBetaDLM_correl_mapping4(	int				index,
								double			VarFFx,
								double			volX,
								double			RhoDS,
								double			RhoFS,
...
double	iteration, iteration_limit;
	...
	do
	{
		...
		*RhoFX = RhoFX1;
		...
		vol_dom = - model->dSigmaDom[index] * (1.0 - exp(- model->dLambdaDom * (model->dTStar - t))) / model->dLambdaDom;
		vol_for = - model->dSigmaFor[index] * (1.0 - exp(- model->dLambdaFor * (model->dTStar - t))) / model->dLambdaFor;		
		...
		VarFFx += -2.0 * RhoFX1 * model->dSigmaFor[index] * volX * Etha_Func(model->dLambdaFor, model->dTStar, previous_t, next_t)
				+ 2.0 * RhoDX1 * model->dSigmaDom[index] * volX * Etha_Func(model->dLambdaDom, model->dTStar , previous_t, next_t);
		VarFFx += model->dSigmaFor[index] * model->dSigmaFor[index] * Psi_Func(model->dLambdaFor, model->dLambdaFor, model->dTStar, previous_t, next_t)
				+ model->dSigmaDom[index] * model->dSigmaDom[index] * Psi_Func(model->dLambdaDom, model->dLambdaDom, model->dTStar, previous_t, next_t)
				- 2.0 * RhoDF * model->dSigmaDom[index] * model->dSigmaFor[index] * Psi_Func(model->dLambdaDom, model->dLambdaFor, model->dTStar, previous_t, next_t);
...
Err FxBetaDLM_correl_mapping3(	int				index,
								double			VarFFx,
								double			volX,
								double			RhoDS,
								double			RhoFS,
...
int		step;
	...
	for (step=0; step<2; step++)
	{
		if( (1.0 + 2.0 * model->dC0 * VarFFx) <= 0.0)
		{
				err = serror("FxBetaDLM_correl_mapping: The cumulative variance is too high for t = %f", t);
				return err;		
		}
		B = model->dB0 / (1.0 + 2.0 * model->dC0 * VarFFx);
		vol_dom = - model->dSigmaDom[index] * (1.0 - exp(- model->dLambdaDom * (model->dTStar - t))) / model->dLambdaDom;
		...
		if (fabs(RhoDS) > 1.0E-08)
		{
			...
			if (fabs(RhoDX1) > vol_X * 0.999)
			{
				err = serror("No solution for correlation mapping, correlation dom_X not in [-1;1] for t = %f", t);
				return err;
			}
		...
		else
		{
			...
			if (fabs(RhoFX1) > vol_X * 0.999)
			{
				err = serror("No solution for correlation mapping, correlation for_X not in [-1;1] for t = %f", t);
				return err;
			}
		}
		...
		if (step == 0)
		{
			...
			VarFFx += -2.0 * RhoFX1 * model->dSigmaFor[index] * volX * Etha_Func(model->dLambdaFor, model->dTStar, t, next_t)
					+ 2.0 * RhoDX1 * model->dSigmaDom[index] * volX * Etha_Func(model->dLambdaDom, model->dTStar , t, next_t);
			VarFFx += model->dSigmaFor[index] * model->dSigmaFor[index] * Psi_Func(model->dLambdaFor, model->dLambdaFor, model->dTStar, t, next_t)
					+ model->dSigmaDom[index] * model->dSigmaDom[index] * Psi_Func(model->dLambdaDom, model->dLambdaDom, model->dTStar, t, next_t)
					- 2.0 * RhoDF * model->dSigmaDom[index] * model->dSigmaFor[index] * Psi_Func(model->dLambdaDom, model->dLambdaFor, model->dTStar, t, next_t);
	...
	*RhoFX = 0.5 * (RhoFX1 + RhoFX2);
	...
	VarFFxTest += -2.0 * *RhoFX * model->dSigmaFor[index] * volX * Etha_Func(model->dLambdaFor, model->dTStar, t, next_t)
				+ 2.0 * *RhoDX * model->dSigmaDom[index] * volX * Etha_Func(model->dLambdaDom, model->dTStar , t, next_t);
	VarFFxTest += model->dSigmaFor[index] * model->dSigmaFor[index] * Psi_Func(model->dLambdaFor, model->dLambdaFor, model->dTStar, t, next_t)
				+ model->dSigmaDom[index] * model->dSigmaDom[index] * Psi_Func(model->dLambdaDom, model->dLambdaDom, model->dTStar, t, next_t)
				- 2.0 * RhoDF * model->dSigmaDom[index] * model->dSigmaFor[index] * Psi_Func(model->dLambdaDom, model->dLambdaFor, model->dTStar, t, next_t);


	/* check */
	err = Get_Correl_FromModel(	index,
								VarFFx,
								volX, 
								*RhoDX,
								*RhoFX,
...
Err FxBetaDLM_correl_mapping2(	int				index,
												double			VarFFx,
												double			VolX,
												double			RhoSD,
												double			RhoSF,
...
{
Err err = NULL;
double	RhoSD1, RhoSD2,
			RhoSF1, RhoSF2,
			RhoXD1, RhoXD2, RhoXD3,
			RhoXF1, RhoXF2, RhoXF3;
...
Err Get_Correl_FromModel(	int				index,
							double			VarFFx,
							double			VolX,
							double			RhoXD,
							double			RhoXF,
...
Err err = NULL;
	...
	volS = B * B * VolX * VolX + (B - 1.0) * (B - 1.0) * (gamma_for * gamma_for + gamma_dom * gamma_dom);
	volS += 2.0 * (B - 1.0) * (B * (RhoXF * gamma_for - RhoXD * gamma_dom) * VolX - (B - 1.0) * RhoDF * gamma_dom * gamma_for);
...
Err FxBetaDLM_GetFirstGuessFromB0(	double	B0,
									double	Tstar,
									double	*exercise_opt,
									double	*maturity_opt,
									long	nbrOpt,
...
Err err = NULL;
	...
	for (i = 0; i < nbrOpt; i++)
	{
		...
		for (j=start_index; j<end_index+1; j++)
		{
			...
			Integral += B0minus1 * sig_curve_for[j] * sig_curve_for[j] * Psi_Func(lda_for, lda_for, Tstar, t1, t2);
			Integral += B0minus1 * sig_curve_dom[j] * sig_curve_dom[j] * Psi_Func(lda_dom, lda_dom, Tstar, t1, t2);
			Integral += 2.0 * correl_for_fx[j] * cal_vol_3f[i] * sig_curve_for[j] * Etha_Func(lda_for, Tstar, t1, t2);
			Integral -= 2.0 * correl_dom_fx[j] * cal_vol_3f[i] * sig_curve_dom[j] * Etha_Func(lda_dom, Tstar, t1, t2);
			Integral -= B0minus1 * 2.0 * correl_dom_for[j] * sig_curve_dom[j] * sig_curve_for[j] * Psi_Func(lda_dom, lda_for, Tstar, t1, t2);
		}

		fx_vol_curve[i] = (cal_vol_3f[i] * cal_vol_3f[i] + B0minus1 * Integral / (end_mat - start_mat));
...
Err	FxBetaDLM_GetModelFromStr(	FXBETADLM_STR str,
								FxBetaDLM_model	*model)
{
Err		err = NULL;
long	spot_date;
int		i, index;
	...
	model->dCashFx = str->spot_fx
		* swp_f_df (str->today, spot_date, str->yc_dom)
		/ swp_f_df (str->today, spot_date, str->yc_for);

	err = FxBetaDLM_get_beta_from_C0(	str->tstar,
										str->c0,
										&model->dEquiBeta);
	...
	if (!model->dPWTime || !model->dSigmaDom || !model->dSigmaFor || !model->dSigmaFx || 
		!model->dSigmaFxUp || !model->dSigmaFxDown || 
		!model->dCorrDomFor || !model->dCorrDomFx || !model->dCorrForFx ||
		!model->dCorrDomFxDown || !model->dCorrForFxDown ||
		!model->dCorrDomFxInput || !model->dCorrForFxInput)
	{
	...
	for (i=0; i<model->iNbPWTime; i++)
	{
		...
		if(fabs(model->dLambda) < TINY)
		{
			model->dSigmaFxUp[i] = str->sig_fx[index] * exp( model->dAlpha * sqrt(str->time_fx[index]));	
			model->dSigmaFxDown[i] = str->sig_fx[index] * exp(-model->dAlpha * sqrt(str->time_fx[index]));		
		}
		else
		{
			model->dSigmaFxUp[i] = str->sig_fx[index] * exp( model->dAlpha * sqrt((1.0 - exp(- 2.0 * model->dLambda * str->time_fx[index]))/(2.0 * model->dLambda)));
			model->dSigmaFxDown[i] = str->sig_fx[index] * exp(-model->dAlpha * sqrt((1.0 - exp(- 2.0 * model->dLambda * str->time_fx[index]))/(2.0 * model->dLambda)));				
		}
...
Err err = NULL;
	...
	for(i = 0; i<str->nb_DLM_corr; i++)
	{
		...
		volS = B * B * sig_fx * sig_fx + (B - 1.0) * (B - 1.0) * (gamma_for * gamma_for + gamma_dom * gamma_dom);
		volS += 2.0 * (B - 1.0) * (B * (RhoXF * gamma_for - RhoXD * gamma_dom) * sig_fx - (B - 1.0) * RhoDF * gamma_dom * gamma_for);
		volS += 4.0 * C * C * var_ffx * vol_X2;
...
Err err = NULL;
	...
	for (i = 0; i < str->nb_DLM_corr; i++)
	{
		if (!i)
		{
			if ( str->time_DLM_corr[i] > NumParams->dMinTimeSpaceCorrel)
			{
				...
				for(j = 0; j < n; j++)
				{
					num_f_add_number(&NbTime, &str->time_DLM_corr, (double)(((long) (t * DAYS_IN_YEAR)) * YEARS_IN_DAY));
					t += dt;
				}
			}			
		}
	...
	for (i=0; i<str->nb_DLM_corr; i++)
	{
		...
		err = FxBetaDLM_correl_mapping(	str->sig_fx[indexSigFx],
										str->b0,
										str->c0,
										str->tstar,
		...
		varFFx += -2.0 * str->for_fx_DLM_corr[i] * str->sig_for[indexSigFor] * str->sig_fx[indexSigFx] * Etha_Func(str->lam_for, str->tstar, t1, t2)
				+ 2.0 * str->dom_fx_DLM_corr[i] * str->sig_dom[indexSigDom] * str->sig_fx[indexSigFx] * Etha_Func(str->lam_dom, str->tstar , t1, t2);
		varFFx += str->sig_for[indexSigFor] * str->sig_for[indexSigFor] * Psi_Func(str->lam_for, str->lam_for, str->tstar, t1, t2)
				+ str->sig_dom[indexSigDom] * str->sig_dom[indexSigDom] * Psi_Func(str->lam_dom, str->lam_dom, str->tstar, t1, t2)
				- 2.0 * str->dom_for_3F_corr[indexCorr] * str->sig_dom[indexSigDom] * str->sig_for[indexSigFor] * Psi_Func(str->lam_dom, str->lam_for, str->tstar, t1, t2);
...
Err FxBetaDLM_CheckOrMakePosMatrix2(double RhoDF,
									double *RhoDX,
									double *RhoFX)
{
Err err = NULL;
...
int		iteration_limit = 10,
	...
	else
	{
		...
		err = diagonalise_symmetric_matrix(	correl_matrix,
											3,
											eigen_val,
											eigen_vector);
...
FREE_RETURN:	
	...
	if(correl_matrix)	free_dmatrix (correl_matrix, 0, 2, 0, 2);
	if(eigen_vector)	free_dmatrix (eigen_vector, 0, 2, 0, 2);

	return err;
}

Err FxBetaDLM_CheckOrMakePosMatrix(	double RhoDF,
										double *RhoDX,
										double *RhoFX)
{
Err err = NULL;
...
Err err = NULL;
	...
	if (!model_dest->dPWTime || !model_dest->dSigmaDom || !model_dest->dSigmaFor || !model_dest->dSigmaFx3F || 
		!model_dest->dSigmaFx || !model_dest->dSigmaFxUp || !model_dest->dSigmaFxDown || !model_dest->dCorrDomFor || 
		!model_dest->dCorrDomFx || !model_dest->dCorrForFx || !model_dest->dCorrDomFxDown || !model_dest->dCorrForFxDown ||
	...
	memcpy(model_dest->dSigmaFx3F, model_source->dSigmaFx3F, model_dest->iNbPWTime * sizeof(double));
	memcpy(model_dest->dSigmaFxUp, model_source->dSigmaFxUp, model_dest->iNbPWTime * sizeof(double));
	memcpy(model_dest->dSigmaFxDown, model_source->dSigmaFxDown, model_dest->iNbPWTime * sizeof(double));
	memcpy(model_dest->dCorrDomFor, model_source->dCorrDomFor, model_dest->iNbPWTime * sizeof(double));
	memcpy(model_dest->dCorrDomFx, model_source->dCorrDomFx, model_dest->iNbPWTime * sizeof(double));
	memcpy(model_dest->dCorrForFx, model_source->dCorrForFx, model_dest->iNbPWTime * sizeof(double));
	memcpy(model_dest->dCorrForFxDown, model_source->dCorrForFxDown, model_dest->iNbPWTime * sizeof(double));
	memcpy(model_dest->dCorrDomFxDown, model_source->dCorrDomFxDown, model_dest->iNbPWTime * sizeof(double));
	memcpy(model_dest->dCorrForFxInput, model_source->dCorrForFxInput, model_dest->iNbPWTime * sizeof(double));
	memcpy(model_dest->dCorrDomFxInput, model_source->dCorrDomFxInput, model_dest->iNbPWTime * sizeof(double));

...
/**	linear interpol on (log(y)/x) **/
	...
	if(method == 3)
	{
		yt = log(y[i-1])/x[i-1] + (xt - x[i-1]) * 
					(log(y[i])/x[i] - log(y[i-1])/x[i-1]) / 
					(x[i] - x[i-1]);
		yt = exp(yt * xt);
	}
/** 	quadratic interpol  on rate*rate times time **/
	...
	{   
        ...
        */
		...
		yt = y[i-1]*y[i-1]*x[i-1]+(xt-x[i-1])/(x[i]-x[i-1])
...
double interp_columns(	double **x, 
						double **y, 
						int l, 
						double xt, 
						int method, 
...
/**	linear interpol on (log(y)/x) **/
	...
	if(method == 3)
	{
		yt = log(y[n][i-1])/x[n][i-1] + (xt - x[n][i-1]) * 
...
/** 	quadratic interpol  on rate*rate times time **/
	...
	{   
        ...
        */
		...
		yt = y[n][i-1]*y[n][i-1]*x[n][i-1]+(xt-x[n][i-1])/(x[n][i]-x[n][i-1])
...
double lin_interp_2d (double x, double y, 
					 double *xa, double *ya, 
					 double **za, long num_xa, long num_ya)
...
double InterpLagrange1D( double *Xdata,
						 double *Ydata,
						 unsigned int n,
						 double x
					   )
...
double InterpLagrangeColumns1D(		double **Xdata,
									double **Ydata,
									unsigned int n,
									double x,
									int m

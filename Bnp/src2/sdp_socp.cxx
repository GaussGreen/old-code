/*
Feb 2001. LTQLM Inc.
Wrapper Author: AdA (Alex d'Aspremont        , aspremon@stanford.edu        , +1
415 864 2962) Base code: Miguel Sousa Lobo        , Lieven Vandenberghe        ,
and Stephen Boyd. See code website: www.stanford.edu/~boyd/SOCP.html

WARNING: Matrixes are stored in flat BLAS-LAPACK (FORTRAN) format        ,
column major order        , arrays starting at 0        , the function ijtok(i
,j       ,number_of_rows) makes the conversion.
...
//	Main function for vol surface smoothing        , solves the following
//program:
//
//		Minimize ||Ax||^2
...
Err socp_smooth_lp(double **objective_matrix, double **constraints_matrix,
  ...
  // Check alpha        , if incorrect input        , use default (this is
  // mostly harmless).
  if ((alpha < 0.0) || (alpha > 1.0)) {
    alpha = defaut_alpha;
    if (printlevel >= 2)
      smessage("SdpSOCP warning: Centering should be in [0        ,1]        , "
               "using "
               "default value... ");
  }
  // Make some tests
...
void dgrad(
    /* input args. */
    double w, int L, double c1, /* pre-computed constants        , dependent on:
                                   u        , z        , du        , dz */

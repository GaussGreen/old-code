...

{
        ...
        for (i = trinf->min_state_var_index; i <= trinf->max_state_var_index; i++)
        {
            ...
            for (j = trinf->min_phi_index; j <= trinf->max_phi_index; j++)
            {
                ...
                if (stp->next)
                {
                    ...
                    Y_T_at_t_compute(1, &node.cur_sam, &tminf->yp, &zc_yield, 0, ONE_FAC, mdl_type);
                    node.df = exp(-zc_yield);

                    /* Compute expected values for t+delta_t at this node */
                    err = srt_f_CheyBeta_drift_at_sam(
                        mdl_type, stp, &node.cur_sam, &node.drift_sam, 0);
                    if (err)
                        return err;
                    ...
                    err = srt_f_CheyBeta_var_at_sam(
                        mdl_type, stp, &node.cur_sam, &node.state_var_variance, 0);
                    if (err)
                        return err;

                    /* Recombine in r : find the closest node to the fwd + up & do */
                    err =
                        srt_f_CheyBetaTree_recombine_in_r(&node, state_var_grid, stp->next->trinf);
                    if (err)
                    {
                        FREE_CHEY_BETA_TREE_MEMORY;
                        return err;
    ...
    err = srt_f_IOstructsetcolpvs(
        iolist, SRT_NO, (double*)next_assets[0][0], grfnparam->node_dim, "");
    if (err)
    {
        FREE_CHEY_BETA_TREE_MEMORY;
        return err;

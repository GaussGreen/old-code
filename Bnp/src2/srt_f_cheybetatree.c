/* ==============================================================================
   ...
   FILE NAME:      srt_f_cheybetatree.c 
...
/* ------------------------------------------------------------------------------ */
#define PSWAP(X,Y) {void *_VPTR = X; X = Y; Y = _VPTR;}

#define FREE_CHEY_BETA_TREE_MEMORY {																	\
	free_f3tensor (cur_assets, maxtrinf.min_state_var_index, maxtrinf.max_state_var_index,			\
				   maxtrinf.min_phi_index, maxtrinf.max_phi_index,					\
				   0, grfnparam->node_dim - 1);								\
	free_f3tensor (next_assets, maxtrinf.min_state_var_index, maxtrinf.max_state_var_index,			\
				   maxtrinf.min_phi_index, maxtrinf.max_phi_index,					\
				   0, grfnparam->node_dim - 1);								\
	free_dvector (state_var_grid, maxtrinf.min_state_var_index, maxtrinf.max_state_var_index);				\
	free_dvector (cur_phi_grid, maxtrinf.min_phi_index, maxtrinf.max_phi_index);	\
	free_dvector (next_phi_grid, maxtrinf.min_phi_index, maxtrinf.max_phi_index);}
	
/* ------------------------------------------------------------------------------ */

/* ------------------------- Main function -------------------------------------- */

Err srt_f_CheyBetaTree		(
		SrtUndPtr 		und,			/* Market pointer: the underlying */
...
/* Set top (1st step), bot (last step) and stp (current step, init to top) */
	top = stp = gototop (stp);
	bot = gotobot (stp);
...
/* Memory Allocation */
	next_assets = f3tensor (maxtrinf.min_state_var_index, maxtrinf.max_state_var_index,
						    maxtrinf.min_phi_index, maxtrinf.max_phi_index,
							0, grfnparam->node_dim - 1);			   
	cur_assets = f3tensor (maxtrinf.min_state_var_index, maxtrinf.max_state_var_index,
						   maxtrinf.min_phi_index, maxtrinf.max_phi_index,
						   0, grfnparam->node_dim - 1);
	state_var_grid = dvector (maxtrinf.min_state_var_index, maxtrinf.max_state_var_index);
	...
	if (!next_assets || !cur_assets || !state_var_grid || !cur_phi_grid || !next_phi_grid)
	{
...
/* For each time stp, starting from the botom one */
	...
	/* Loop on r */
		...
		{
		/* Fill node info */
			sam_get (node.cur_sam, 0, STATEVAR) = state_var_grid[i];
			sam_get (node.cur_sam, 0, SHORT_RATE) = state_var_grid[i] + sam_get (tminf->fwd_sam, 0, F_0_t);

		/* Loop on phi */  
			for (j=trinf->min_phi_index; j<=trinf->max_phi_index; j++) 
			{
			/* Fill node info */
				sam_get (node.cur_sam, 0, PHI) = cur_phi_grid[j];
			...
			/* If not at the bottom: discount */
				...
				/* Compute expected values for t+delta_t at this node */
					err = srt_f_CheyBeta_drift_at_sam( mdl_type, stp, &node.cur_sam, 
							&node.drift_sam, 0);
					if (err)
						return err;
				...
				/* Compute the local variance at this node */
					err = srt_f_CheyBeta_var_at_sam( mdl_type, stp, &node.cur_sam, 
						&node.state_var_variance, 0);
					if (err)
						return err;

				/* Recombine in r : find the closest node to the fwd + up & do */
					err = srt_f_CheyBetaTree_recombine_in_r (&node, state_var_grid, stp->next->trinf);
					if (err)
					{
						FREE_CHEY_BETA_TREE_MEMORY;
						return err;
...
/* Stores the premium in the Input/Output list */
	err = srt_f_IOstructsetpremium(iolist,SRT_NO,next_assets[0][0][grfnparam->node_dim-1],"");
	if (err) 
	{
		FREE_CHEY_BETA_TREE_MEMORY;
		return (err);

    ...
    double*         price)
{
    ...
    /* Finds the index of the first date in the Sigma Term Struct that is on or after the Future
     * Date */
    next_sigma = 0;
    ...
    err = srt_f_grfn_clsdfrm(
        und, grfnparam, sdp, strike, bondstrike, rec_pay, type, ref_rate_code, &discprice);
    if (err)
    {
        return err;
    }

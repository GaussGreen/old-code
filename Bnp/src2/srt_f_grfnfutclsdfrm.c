/* -------------------------------------------------------------------------
   ...
   FILE NAME	: srt_f_grfnfutclsdfrm.c
	
   PURPOSE		: functions to price swaptions, caps or bondoptions without
                  the Grfn interface as of a date in the FUTURE, taking
				  todays forward curve as input. 
				  THese functions take simplified 
...
/* Finds the index of the first date in the Sigma Term Struct that is on or after the Future Date */ 
	next_sigma = 0;
	...
	if (sigma[0][next_sigma] == future_date)
	{
...
/* Calls Grfn Closed Form to price (as of today) this option with no vol until the future date*/
	err = srt_f_grfn_clsdfrm(und,grfnparam,sdp,strike,bondstrike,rec_pay,type,ref_rate_code,&discprice);
	if(err)
	{
		return err;
	}

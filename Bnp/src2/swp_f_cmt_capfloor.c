...
/*                                                                            */
/*      PURPOSE:  CAPS, FLOORS, IMPLIED_VOL CAP/FLOOR, CAPLETS, FLOORLETS     */
/*					on CMT MARKETS     										  */
/*                                                                            */
/******************************************************************************/
...
/* Populates a CMT vs Fixed swap to generate strike, vols... */
	err = swp_f_populate_GenFullSwap( 
						0.0,
						crvptr,
						&cmt_swap);
	if (err)
...
/* If cap has already started, first fixing used in the price is after spot */
...
/* Compute the value of each caplet independently (do not use last fixing)*/
	...
	/* is the next one in the swap*/
		...
		{
			fwd =  capfloor_swap->leg[0].cpn.d[i+1];  /* convexity adjusted fwd */
			strike = capfloor_swap->leg[1].cpn.d[i]; /* strike (2nd leg) */
		...
		vol is effective only until fixing date */
 			fixing_date = add_unit( capfloor_swap->leg[0].pay_date.date[i], -cmt_param->spot_lag, 
							SRT_BDAY, SUCCEEDING);
			mat= (double) (fixing_date - today) / 365.0;
			cvg = capfloor_swap->leg[0].cvg.d[i];
		/* Payment of the caplet is done on the next fixng date */
			...
			if (rec_pay == SRT_PAYER) /* Cap */
				call_put = SRT_CALL;  /* Call */

			caplet_grk = srt_f_optblksch(
								fwd, 
								strike, 
								vol, 
								mat,
...
Err swp_f_CMCapFloor(long TheoEnd,
				long numperiodcap,
				double Underlying,
				char *cCMTFrequency,
				char *cCMTBasis,
				...
				char *cRefRateCode,
				char *cCapFloor,
				char *method,/*BS: BS on CMS Full Smile + Fwd Spread - FS: Full Smile on Fwd Swap + Fwd Spread - FVBS: BS on CMS Flat Vol + Fwd Spread - FV: Flat Vol on Fwd Swap + Fwd Spread*/
...
SrtCallPutType CapFloor;
	...
	{
		PayDates[i]	   =	add_unit(add_unit(TheoEnd,-(numperiodcap-i-1)*12/CapFreq, SRT_MONTH, NO_BUSDAY_CONVENTION),0,SRT_BDAY,MODIFIED_SUCCEEDING);
		StartDates[i] =    add_unit(add_unit(TheoEnd,-(numperiodcap-i)*12/CapFreq, SRT_MONTH, NO_BUSDAY_CONVENTION),0,SRT_BDAY,MODIFIED_SUCCEEDING);
		FixingDates[i] = add_unit(StartDates[i], -spotlag, SRT_BDAY, MODIFIED_SUCCEEDING);
	}
	...
	for (i = 0;i<numperiodcap;i++)
	{
		...
		TheoEndCMT = add_unit(StartDates[i],numperiods*12/CMTFreq,SRT_MONTH,NO_BUSDAY_CONVENTION);
		err = swp_f_setSwapDP(StartDates[i], TheoEndCMT,CMTFreq, CMTBasis, &sdpSwap);
		if (err)
			return err;
		...
		err = swp_f_ForwardRate_SwapDP( &sdpSwap, 
										szYieldCurveName, 
										cRefRateCode, 
										&FwdSwapRate);   
		...
		{
			if (err=swp_f_Cms_Rate(FwdSwapRate, (FixingDates[i]-today)/365.0, numperiods,(double) CMTFreq, (PayDates[i]-StartDates[i])/365.0,
							   1.0, SRT_LOGNORMAL, 0.0 , 2 /* Full Smile Approx */, StartDates[i], TheoEndCMT,
							   0, 0.0, szSwaptionVolName, 
						       NumStrikesInVol, Strikes,
							   &CMS[i]))
			...
			CMT[i]	=	CMS[i]+dspread;
			swp_f_vol(szCMTVolName,StartDates[i], TheoEndCMT,Strike,&(CMTvol[i]),&lognorm);
			Caplet[i] = srt_f_optblksch(CMT[i],Strike,CMTvol[i],(FixingDates[i]-today)/365.25,1,CapFloor,PREMIUM);
	
		}

		else if (strcmp(method,"FVBS") == 0)
		{
			swp_f_vol(szSwaptionVolName,StartDates[i], TheoEndCMT,FwdSwapRate,&dFlatVol,&lognorm);
			if (err=swp_f_Cms_Rate(FwdSwapRate, (FixingDates[i]-today)/365.0, numperiods,(double) CMTFreq, (PayDates[i]-StartDates[i])/365.0,
							   1.0, SRT_LOGNORMAL, dFlatVol , 0 /* Flat Vol */, StartDates[i], TheoEndCMT,
							   0, 0.0, szSwaptionVolName, 
							   NumStrikesInVol, Strikes,
							   &CMS[i]))
		...
		else if (strcmp(method,"FV") == 0)
		{
			...
			swp_f_vol(szCMTVolName,StartDates[i], TheoEndCMT,FwdSwapRate,&dFlatVol,&lognorm);
			if (err=swp_f_Cms_Rate(FwdSwapRate+dspread, (FixingDates[i]-today)/365.0, numperiods,(double) CMTFreq, (PayDates[i]-StartDates[i])/365.0,
							   1.0, SRT_LOGNORMAL, dFlatVol , 0 /* Flat Vol */, StartDates[i], TheoEndCMT,
							   0, 0.0, szCMTVolName, 
							   NumStrikesInVol, Strikes,
							   &CMT[i]))
			...
			if (err=swp_f_Cms_Option(FwdSwapRate+dspread, (FixingDates[i]-today)/365.25, numperiods,Strike,(double) CMTFreq, CapFloor,(PayDates[i]-StartDates[i])/365.0,
							   1.0, SRT_LOGNORMAL, dFlatVol , 0 /* Flat vol */, StartDates[i], TheoEndCMT,
							   0, 0.0, szCMTVolName, 
							   NumStrikesInVol, Strikes,
							   &Caplet[i]))
		...
		Cap+=Caplet[i]*coverage(StartDates[i],PayDates[i],CapBasis)*swp_f_df(today,PayDates[i],szYieldCurveName);
	}
	...
	for (i = 0;i<numperiodcap;i++)
	{
		...
		result[i+1][3] = CMT[i];
		result[i+1][4] = Caplet[i]*coverage(StartDates[i],PayDates[i],CapBasis)*swp_f_df(today,PayDates[i],szYieldCurveName);
	}
...
Err swp_f_CMCapFloorNY(long TheoEnd,
				long numperiodcap,
				double Underlying,
				char *cCMTFrequency,
				char *cCMTBasis,
...
SrtCallPutType CapFloor;
	...
	for (i = 0;i<numperiodcap;i++)
	{
		...
		else
		{
			...
			if (err=swp_f_Cms_OptionNY(FwdSwapRate+dspread, (FixingDates[i]-today)/365.25, numperiods,Strike,(double) CMTFreq, CapFloor,(PayDates[i]-StartDates[i])/365.0,
							   1.0, SRT_LOGNORMAL, 0.0 , 2 /* Full Smile Approx */, StartDates[i], TheoEndCMT,
							   0, 0.0, szCMTVolName, 
							   NumStrikesInVol, Strikes,&(Caplet[i]),szYieldCurveName,
							   cRefRateCode))
...
Err swp_f_getcmsvoladjusted(double FwdSwapRate, 
							long StartSwap,
							long TheoEndSwap,
							double Mat,
							double CMS, 
...
double lognorm;
	...
	swp_f_vol(szSwaptionVolName,StartSwap, TheoEndSwap,FwdSwapRate,&ATMVolSwap,&lognorm);
	ATMVolCMS = ATMVolSwap*FwdSwapRate/CMS;

	err = srt_f_optsarbvol(	CMS,Strike-FwdSpread,Mat,ATMVolCMS,alpha,beta,rho,SRT_LOGNORMAL,SRT_LOGNORMAL,CMSVol);

return err;
}

Err swp_f_getcmtvol(double CMS,
					double FwdSpread, 
					double CMSVol,
					double Strike, 
					double dLambda, 
...
double FwdSpreadNormVol;
	...
	premium = srt_f_optblksch(CMS,Strike-FwdSpread, CMSVol,Mat, 1.0,SRT_CALL,PREMIUM);
	err = srt_f_optimpvol(premium,CMS, Strike-FwdSpread, Mat, 1.0,SRT_CALL, SRT_NORMAL,&CMSNormVol);
	
	FwdSpreadNormVol = dSpreadLocVol*sqrt((1-exp(-2*dLambda*Mat))/(2*dLambda*Mat));
	
	CMTNormVol = sqrt(CMSNormVol*CMSNormVol-2*dCorrel*CMSNormVol*FwdSpreadNormVol+FwdSpreadNormVol*FwdSpreadNormVol);

	premium = srt_f_optblknrm(CMS+FwdSpread,Strike,CMTNormVol,Mat,1.0,SRT_CALL,PREMIUM);
	err = srt_f_optimpvol(premium,CMS+FwdSpread,Strike,Mat,1.0,SRT_CALL,SRT_LOGNORMAL,CMTVol);
return err;
}



Err swp_f_getcmtvolMAD(double StartSwap,
					   double TheoEndSwap,
					   char *szSwaptionVolName,
					   double FwdSwapRate,
					   double CMS,
...
double lognorm;
	...
	{
		swp_f_vol(szSwaptionVolName,StartSwap, TheoEndSwap,Strikes[i]-FwdSpread,&SwapStrikeVol,&lognorm);
		CMTVol[i] = SwapStrikeVol*FwdSwapRate/(FwdSwapRate+FwdSpread);
/*		err = srt_f_optsarbvol(	CMS+FwdSpread,Strikes[i],Mat,ATMVolCMT,alpha,beta,rho,SRT_LOGNORMAL,SRT_LOGNORMAL,&(CMTVol[i]));*/
	}
return err;
}
		
Err swp_f_getcmtvolCHI(long StartSwap,
					   long TheoEndSwap,
					   char *szSwaptionVolName,
					   double FwdSwapRate,
					   double CMS,
...
rhocmt = rho;
	...
	for (i = 0;i<NumStrikes;i++)
	{
		err=swp_f_Cms_Option(FwdSwapRate, Mat, numperiods,Strikes[i]-FwdSpread,(double) CMTFreq, SRT_PAYER,0,
								   1.0, SRT_LOGNORMAL, 0.0 , 2 /* Full Smile Approx */, StartSwap, TheoEndSwap,
								   0, 0.0, szSwaptionVolName, 
								   NumStrikes, Strikes,
								   &(CMSOption[i]));

		err = srt_f_optimpvol(CMSOption[i], CMS+FwdSpread, Strikes[i],Mat, 1.0, SRT_PAYER,  SRT_LOGNORMAL,&(implied_vol[i]));

	}

	err = swp_f_Cms_Option(FwdSwapRate, Mat, numperiods,CMS,(double) CMTFreq, SRT_PAYER,0.0,
								   1.0, SRT_LOGNORMAL, 0.0 , 2 /* Full Smile Approx */, StartSwap, TheoEndSwap,
								   0, 0.0, szSwaptionVolName, 
								   NumStrikes, Strikes,
									&CMSATMOption);
	
	err = srt_f_optimpvol(CMSATMOption, CMS+FwdSpread, CMS+FwdSpread,Mat, 1.0, SRT_PAYER,  SRT_NORMAL,&ATMNormalimpvol);

	err = srt_f_optimpvol(CMSATMOption, CMS+FwdSpread, CMS+FwdSpread,Mat, 1.0, SRT_PAYER,  SRT_LOGNORMAL,&ATMVolCMS);
	...
	err = opsabrcalib(CMS+FwdSpread, Mat, NumStrikes, CMTStrikes, CMTVols,&ATMVolCMS,  &alphacmt,1,&betacmt,0,&rhocmt,1,&error);



	ATMNormalVolCMT = CHI*ATMNormalimpvol;
	
	ATMPrice = srt_f_optblknrm(CMS+FwdSpread,CMS+FwdSpread,ATMNormalVolCMT,Mat, 1.0, SRT_CALL,PREMIUM); 
		
	err = srt_f_optimpvol(ATMPrice, CMS+FwdSpread, CMS+FwdSpread,Mat, 1.0, SRT_PAYER,  SRT_LOGNORMAL,&ATMVolCMT);
	...
	for (i= 0;i<NumStrikes;i++)
	{
		...
		err = srt_f_optsarbvol(	CMS+FwdSpread,Strikes[i],Mat,ATMVolCMT,alphacmt,betacmt,rhocmt,SRT_LOGNORMAL,SRT_LOGNORMAL,&(CMTVol[i]));
	}
...
Err swp_f_GetCMVols(char *szYieldCurveName,
					char *szSwaptionVolName,
					char *cRefRateCode,
					double Underlying,
					char *cCMTFrequency,
					...
					double *beta,
					double *rho,
					char   *Method,/*MAD: CMT ATM Normal Vol = CMS ATM Normal Vol - CHI: CMT ATM Normal Vol = CHI * CMS ATM Normal Vol (In fact dLambda is taken to be CHI - ELSE: Use OU on the Spread with 3 parameters*/
					double *chi,
					double dLambda,
					double dSpreadLocVol,
					double dCorrel,
					double **CMTVols,
					double **SabrComponents)
{
Err err = NULL;
int i,j;
long   today, spotdate,spotlag;
...
/*Loop on all the different Maturities*/
	...
	/*First compute the CMS rate*/
		...
		TheoEndSwap = add_unit(spotdate,(long) (Mats[i]+Underlying)*12,SRT_MONTH,NO_BUSDAY_CONVENTION);
		StartSwap = add_unit(add_unit(TheoEndSwap,(long) -Underlying*12,SRT_MONTH,NO_BUSDAY_CONVENTION),0,
		...
		if (err=swp_f_Cms_Rate(FwdSwapRate, Mats[i],Underlying*CMTFreq,(double) CMTFreq, 0,
							   1.0, SRT_LOGNORMAL, 0.0 , 2 /* Full Smile Approx */, StartSwap, TheoEndSwap,
							   0, 0.0, szSwaptionVolName, 
							   NumStrikes, Strikes,
							   &CMS))
		...
		FwdSpread=swp_f_spread(add_unit(StartSwap,-spotlag,SRT_BDAY,MODIFIED_SUCCEEDING),StartSwap, cCMTRefRateCode);
		
		if (strcmp(Method,"MAD")== 0)
		{
			err = swp_f_getcmtvolMAD(StartSwap, TheoEndSwap,szSwaptionVolName,FwdSwapRate,CMS,FwdSpread,NumStrikes, Strikes,Mats[i],alpha[i],beta[i],rho[i],CMTVols[i]);

		}

		else if (strcmp(Method,"CHI")== 0)
		{
			err = swp_f_getcmtvolCHI(StartSwap, TheoEndSwap,szSwaptionVolName,FwdSwapRate,CMS,FwdSpread,(double) CMTFreq, Underlying*CMTFreq, NumStrikes, Strikes,Mats[i],alpha[i],beta[i],rho[i],chi[i],CMTVols[i],SabrComponents[i]);
		}
		...
		else
		{
			...
			{
				err = swp_f_getcmsvoladjusted(FwdSwapRate, StartSwap, TheoEndSwap,Mats[i],CMS, FwdSpread, Strikes[j],szSwaptionVolName, alpha[i], beta[i], rho[i],&CMSVol);
				err = swp_f_getcmtvol(CMS, FwdSpread, CMSVol,Strikes[j], dLambda, dSpreadLocVol, dCorrel, Mats[i],&CMTVols[i][j]);
			}
		}
	}
return err;

...
/*                                                                            */
/*      PURPOSE:  CAPS  , FLOORS  , IMPLIED_VOL CAP/FLOOR  , CAPLETS  ,
 * FLOORLETS     */
/*					on CMT MARKETS
 */
/*                                                                            */
...
Err swp_f_getcmtvolMAD(double StartSwap, double TheoEndSwap,
  ...
  for (i = 0; i < NumStrikes; i++) {
     ...
     * ,alpha  ,beta  ,rho  ,SRT_LOGNORMAL  ,SRT_LOGNORMAL  ,&(CMTVol[i]));*/
  }
  return err;
}
...
Err swp_f_GetCMVols(
    char *szYieldCurveName, char *szSwaptionVolName, char *cRefRateCode,
    double Underlying, char *cCMTFrequency, char *cCMTBasis,
    char *cCMTRefRateCode, int NumStrikes, double *Strikes, int NumMats,
    double *Mats, double *alpha, double *beta, double *rho,
    char *Method, /*MAD: CMT ATM Normal Vol = CMS ATM Normal Vol - CHI: CMT ATM
                     Normal Vol = CHI * CMS ATM Normal Vol (In fact dLambda is
                     taken to be CHI - ELSE: Use OU on the Spread with 3
                     parameters*/
    double *chi, double dLambda, double dSpreadLocVol, double dCorrel,
    double **CMTVols, double **SabrComponents) {
  Err err = NULL;
  int i, j;
  long today, spotdate, spotlag;
  SrtCurvePtr Crv;

    ...
    GenFullSwap* capfloor_swap)
{
    ...
    err = swp_f_initSwapDP(start, end_or_nfp, cap_freq_str, cap_basis_str, &cmt_swapdp);
    if (err)
        return err;
    ...
    err = swp_f_create_GenFullSwap(
        &cmt_swapdp, &cmt_swapdp, strike, 0.0, 0.0, CMT_MARGIN_FIXED_SWAP, cmt_name, &cmt_swap);
    ...
    double*         result)
{
    ...
    for (i = 0; i < list_length - 1; i++)
    {
        ...
        if (date_list.date[i] > spot)
        {
            ...
            fixing_date = add_unit(
                capfloor_swap->leg[0].pay_date.date[i], -cmt_param->spot_lag, SRT_BDAY, SUCCEEDING);
    ...
    double*     value)
{
    ...
    err = set_CMT_capfloor(
        start, end_or_nfp, cap_freq_str, cap_basis_str, strike, crvptr, &capfloor_swap);
    if (err)
        return err;

    /* Prices the CMT capfloor from the CMT swap */
    err = price_CMT_capfloor(&capfloor_swap, rec_pay_type, greek_type, crvptr, &result);
    if (err)
        return err;
    ...
    double*     imp_vol)
{
    ...
    /* Prices the CMT capfloor from the CMT swap */
    err = price_CMT_capfloor(&capfloor_swap, rec_pay_type, PREMIUM, crvptr, &result);
    if (err)
        return err;
    ...
    double**    result)
{
    ...
    for (i = 0; i < numperiodcap; i++)
    {
        PayDates[i] = add_unit(
            add_unit(
                TheoEnd, -(numperiodcap - i - 1) * 12 / CapFreq, SRT_MONTH, NO_BUSDAY_CONVENTION),
            0,
            SRT_BDAY,
            MODIFIED_SUCCEEDING);
        StartDates[i] = add_unit(
            add_unit(TheoEnd, -(numperiodcap - i) * 12 / CapFreq, SRT_MONTH, NO_BUSDAY_CONVENTION),
            0,
            SRT_BDAY,
            MODIFIED_SUCCEEDING);
    ...
    for (i = 0; i < numperiodcap; i++)
    {
        ...
        TheoEndCMT =
            add_unit(StartDates[i], numperiods * 12 / CMTFreq, SRT_MONTH, NO_BUSDAY_CONVENTION);
        ...
        err = swp_f_ForwardRate_SwapDP(&sdpSwap, szYieldCurveName, cRefRateCode, &FwdSwapRate);
        ...
        if (strcmp(method, "BS") == 0)
        {
            if (err = swp_f_Cms_Rate(
                    FwdSwapRate,
                    (FixingDates[i] - today) / 365.0,
                    numperiods,
                    (double)CMTFreq,
                    (PayDates[i] - StartDates[i]) / 365.0,
                    1.0,
                    SRT_LOGNORMAL,
                    0.0,
                    2 /* Full Smile Approx */,
            ...
            Caplet[i] = srt_f_optblksch(
                CMT[i], Strike, CMTvol[i], (FixingDates[i] - today) / 365.25, 1, CapFloor, PREMIUM);
        }

        else if (strcmp(method, "FVBS") == 0)
        {
            swp_f_vol(
                szSwaptionVolName, StartDates[i], TheoEndCMT, FwdSwapRate, &dFlatVol, &lognorm);
            if (err = swp_f_Cms_Rate(
                    FwdSwapRate,
                    (FixingDates[i] - today) / 365.0,
                    numperiods,
    ...
    for (i = 0; i < numperiodcap; i++)
    {
        ...
        result[i + 1][3] = CMT[i];
        result[i + 1][4] = Caplet[i] * coverage(StartDates[i], PayDates[i], CapBasis) *
                           swp_f_df(today, PayDates[i], szYieldCurveName);
    }
    ...
    double* CMTVol)
{
    ...
    err     = srt_f_optimpvol(
        premium, CMS, Strike - FwdSpread, Mat, 1.0, SRT_CALL, SRT_NORMAL, &CMSNormVol);

    FwdSpreadNormVol = dSpreadLocVol * sqrt((1 - exp(-2 * dLambda * Mat)) / (2 * dLambda * Mat));

    CMTNormVol = sqrt(
        CMSNormVol * CMSNormVol - 2 * dCorrel * CMSNormVol * FwdSpreadNormVol +
        FwdSpreadNormVol * FwdSpreadNormVol);

    premium = srt_f_optblknrm(CMS + FwdSpread, Strike, CMTNormVol, Mat, 1.0, SRT_CALL, PREMIUM);
    err     = srt_f_optimpvol(
        premium, CMS + FwdSpread, Strike, Mat, 1.0, SRT_CALL, SRT_LOGNORMAL, CMTVol);
    return err;
}

Err swp_f_getcmtvolMAD(
    double  StartSwap,
    double  TheoEndSwap,
    char*   szSwaptionVolName,
    double  FwdSwapRate,
    ...
    double* CMTVol)
{
    ...
    for (i = 0; i < NumStrikes; i++)
    {
        ...
        /*		err = srt_f_optsarbvol(
         * CMS+FwdSpread,Strikes[i],Mat,ATMVolCMT,alpha,beta,rho,SRT_LOGNORMAL,SRT_LOGNORMAL,&(CMTVol[i]));*/
    }
    return err;
}

Err swp_f_getcmtvolCHI(
    long    StartSwap,
    long    TheoEndSwap,
    char*   szSwaptionVolName,
    double  FwdSwapRate,
    ...
    double* SabrComp)
{
    ...
    ATMPrice = srt_f_optblknrm(
        CMS + FwdSpread, CMS + FwdSpread, ATMNormalVolCMT, Mat, 1.0, SRT_CALL, PREMIUM);

    err = srt_f_optimpvol(
        ATMPrice, CMS + FwdSpread, CMS + FwdSpread, Mat, 1.0, SRT_PAYER, SRT_LOGNORMAL, &ATMVolCMT);
...
Err swp_f_GetCMVols(
    ...
    double* beta,
    double* rho,
    char*   Method, /*MAD: CMT ATM Normal Vol = CMS ATM Normal Vol - CHI: CMT ATM Normal Vol = CHI *
                       CMS ATM Normal Vol (In fact dLambda is taken to be CHI - ELSE: Use OU on the
                       Spread with 3 parameters*/
    double*  chi,
    double   dLambda,
    double   dSpreadLocVol,
    double   dCorrel,
    ...
    double** SabrComponents)
{
    ...
    for (i = 0; i < NumMats; i++)
    {
        ...
        TheoEndSwap =
            add_unit(spotdate, (long)(Mats[i] + Underlying) * 12, SRT_MONTH, NO_BUSDAY_CONVENTION);
        StartSwap = add_unit(
            add_unit(TheoEndSwap, (long)-Underlying * 12, SRT_MONTH, NO_BUSDAY_CONVENTION),
            0,
            SRT_BDAY,
            MODIFIED_SUCCEEDING);
        ...
        FwdSpread = swp_f_spread(
            add_unit(StartSwap, -spotlag, SRT_BDAY, MODIFIED_SUCCEEDING),
            StartSwap,
            cCMTRefRateCode);

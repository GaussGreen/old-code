...
char *EurodollarOptionCaller( // For fwds and vols
    ...
    char *(*getSpread)(long start_date, long end_date, const char *szRefRate,
                       double *dSpread),
    long lOptionExpiry, double dStrike, long lFutureStart, double dFuturePrice,
    int iConvexityModel, SrtDiffusionType enumDiffType,
    SrtCallPutType enumCallPut, // Call = long
    SrtGreekType enumGreek, double dDifVol, double dJumpAvg, double dJumpVol,
    double dJumpInt, long lIterNo, double *out_dPrice, double *out_dVol) {
  ...
  *out_dVol = 0.0;
  ...
  if (err = (*getDF)(szYieldCurveName, (double)lToday, (double)lOptionExpiry,
                     &dOptionExpiryDf))
    return err;

  // Calculate the price
  return EurodollarOption(lToday, dOptionExpiry, dCashStrike, dCashFwd,
                          enumDiffType, enumCallPut, enumGreek, dDifVol,
                          dJumpAvg, dJumpVol, dJumpInt, lIterNo,
                          dOptionExpiryDf, out_dPrice, out_dVol);
}
...
char *EurodollarOption(long lToday, double dOptionExpiry, double dCashStrike,
                       double dCashFwd, SrtDiffusionType enumDiffType,
                       SrtCallPutType enumCallPut, // Call = long
                       SrtGreekType enumGreek, double dDifVol, double dJumpAvg,
                       double dJumpVol, double dJumpInt, long lIterNo,
                       double dOptionExpiryDf, double *out_dPrice,
                       double *out_dVol) {
  ...
  *out_dPrice = -1.0;
  ...
  for (n = 0; n <= lIterNo; n++) {
    ...
    dFwd_n = dCashFwd * exp(dJumpInt * (1 - dJumpAvg) * dOptionExpiry) *
             pow(dJumpAvg, n);

    bs = srt_f_optblksch(dFwd_n, dCashStrike, dVol_n, dOptionExpiry,
                         dOptionExpiryDf, enumCallPut, enumGreek);

    dPremium += 1 / n_f * exp(-dJumpInt * dOptionExpiry) *
                pow(dJumpInt * dOptionExpiry, n) * bs;
  }
  ...
  *out_dPrice = dPremium;
  ...
  if (err =
          srt_f_optimpvol(dPremium, dCashFwd, dCashStrike, dOptionExpiry,
                          dOptionExpiryDf, enumCallPut, enumDiffType, out_dVol))
    return err;
  ...
  //*out_dPrice; 			break; 		case DELTA: 			*out_dPrice
  //= - (*out_dPrice); 			break; 		default: 			break;
  //	}
...
char *PL_errorFunction(double index, double in_par[], double *out_y,
                       double *out_dyda, int in_nPar) {}

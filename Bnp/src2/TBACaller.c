...
char Benchmarks[14][5] = {
    ...
    "10yr",
    "20yr",
    "30yr"}; /* Benchmark instrument labels */
    ...
    double* outputs)
{
    ...
    for (i = 0; i < NTRIALS; i++)
    {
        ...
        if (err = Manager(
                data_dir, &term_data, &tree_data, &deal_data, &prepay_data, &history, &density))
            return (err);
        ...
        if (err = impp(
                &term_data, /* Main routine for SFS calculation */
                &tree_data,
                &deal_data,
                &history,
        ...
        if (i >= 1)
            multiplier[i + 1] =
                multiplier[i - 1] + (price0 - price[i - 1]) * (multiplier[i] - multiplier[i - 1]) /
    ...
    double*     HedgePrice)  // another output neededd if Hedge != 'N'
{
    ...
    N   = 0;  // FIX                                                               /* We add two
            // artificial periods at the beginning of the tree so that at time 0 */
    ...
    if (tree_data->ExEnd > 180 && deal_data->Always == 'N')
        TotPer = tree_data->ExEnd + F +
                 N; /* sometimes we need to only build trees 1 yr more than maturity */
    Lockout = tree_data->ExerPosition[0] + N;
    ExEnd   = tree_data->ExEnd + N;

    TotNbZero = term_data->IndexMat *
                F; /* If we have a 10yr index and monthly mortgage cash flows we need 10*12 zeros */
                   /* Note that the program cannot handle the case where the index has a higher   */

    Discount =
        mbs_dmatrix(0, 1, 0, 2 * TotPer); /* coupon frequency than the underlying mortgage. */
    ...
    Zero      = mbs_dmatrix(
        0, TotNbZero, 0, 2 * TotPer); /* Note that we allocate one more zero than needed */
    ParYield = mbs_dmatrix(0, 1, 0, 2 * ExEnd);

    Amort = mbs_dmatrix(
        0, 2 * deal_data->SFSNb - 1, 0, 2 * ExEnd); /* we allocate only that much memory. */
    IO = mbs_dmatrix(
        0, 2 * deal_data->SFSNb - 1, 0, 2 * ExEnd); /* We to price SFSNb * 2 structures */
    ...
    if (!ZeroDelay)
        ...
        for (i = 0; i <= 1; i++)
            ZeroDelay[i] = mbs_dmatrix(
                0, deal_data->Delay, 0, 2 * TotPer); /* allocate memory to ***ZeroDelay */
    ...
    for (q = 0; q < NTRIALS; q++)
    { /* do iteration if we have to find implied xxx */
        ...
        Date = deal_data->Maturity; /* This is the actual maturity date plus 1yr if actual maturity
if 15yr or more AND tree is 1yr beyond it, otherwise it is ten yr more from final maturity */
        NbZero = 1;
        ...
        /*************
                ...
                if( err = MBS_Prepay_Engine_SetParams( &prepay_engine, deal_data->Refirate,
        term_data->SwapYield[5])) return err;
        ************/
        ...
        MBS_Prepay_Engine_Initialize(
            ...
            MAXNUMP,
            NBPOINTS,
            deal_data->IndexLevel,
            tmpAmortLevel,
            deal_data->Seasonality,
            FEBEFFECT,
            NUMRAMPS,
            deal_data->SeasoningLevel,
            tmpSeasoningMat,
            deal_data->Speedup,
            deal_data->rational,
            2 * deal_data->SFSNb,
            deal_data->TriggerRate,
            deal_data->Amort,
            deal_data->Delay,
            deal_data->NbFixedPrepay,
            deal_data->InitialSMM,
            _FN,
            deal_data->Term,
            deal_data->Gwac,
            history,
        ...
        do
        {
            CpPaid = (int)floor(tree_data->Accrued[NbPer - N]); /* CpPaid is TRUE when a cash flow
                                                                   is paid i.e. Accrued = 1. */
            ...
            {
                Lattice(
                    Discount[0],  // chnage NbPer to 0                  /* Set up the lattice at the
                                  // current period */
                    DiscountS[0],
                    pu[0],
                    pd[0],
                    ...
                    tree_data);
                Zero_Calc(
                    Zero, /* Discounted expected value of the set of zeros maturing on each coupon
                             date */
                    &NbZero,
                    TotNbZero1,
                    CpPaid,
                ...
                //			*tree_data->Delta[NbPer-N]                      /* tree rates
                //are different from discount rates */
                ...
                for (i = 0; i <= deal_data->Delay; i++)
                {
                    for (j = Bottom; j <= Top; j++)
                        ZeroDelay[0][i][j] =
                            Zero[i][j] / (1. + term_data->OAS * (double)i * 30. / 365. /
            ...
            else if (casenumber >= 13 && casenumber <= 15)
            {  // if do oas analysis, we must redo discountS */
                for (i = Bottom; i <= Top; i++)
                {
                    if (term_data->TStree == term_data->TSdiscount)
                        ...
                        DiscountS[0][i] =
                            1. / (1. +
                                  (1. / Discount[0][i] - 1.) * tree_data->FwdRateS[NbPer - N] *
                                      tree_data->Delta[NbPer - N] /* tree rates are different from
                                                                     discount rates */
            ...
            if (NbPer <= ExEnd) /* If we sit before the maturity we do the SFS calculations */
            {
                ...
                if (CpPaid)
                {
                    ...
                    {
                        Par_Yield(
                            ParYield[0], /* Value of the par yield in the lattice. We need to
                                            calculate it */
                    ...
                    Main_Prepay(
                        Amort, ParYield[0], CouponNb, mo, NbPer, N, &prepay_engine, tree_data);
                ...
                {
                    IOPO_Price(
                        IO[i], /* Calculate the IO and PO prices in the lattice */
                        PO[i],
                        ZeroDelay[0],
                        DiscountS[0], /* Note that the discount factor used for the IO includes the
                                         OAS */
                        Discount[0],
                        pu[0],
                        pd[0],
                ...
                CouponNb -=
                    CpPaid; /* Decrement CouponNb if one coupon has been paid at this period */
            }

            Date = Nxtmth(
                Date, /* We reevaluate the current date on a coupon payment by going backward */
        ...
        for (i = 0; i < 2 * deal_data->SFSNb; i++)
        {
            ...
            {         /* Note that we have to go backward to do the calculations correctly */
                ...
                {  // We should not have amortized the coupons
                   // falling between today and the i coupon. Discounting should include OAS
                    IO[i][N] +=
                        deal_data->Coupon / F * deal_data->FixedPrepay[j - 1] * Zero[k][N] /
                        (1.0 + term_data->OAS *
                                   (tree_data->Length[0] + ((double)(k - 1)) * 30.0 / 365.0) /
                                   10000.0);
                }
                /* We correct it here.                      */
                PO[i][N] =
                    PO[i][N] * (1. - deal_data->FixedPrepay[j - 1]) +
            ...
            if (deal_data->invfloater == 'Y') /* inverse floater */
                ...
                IO[i][N] -= min(deal_data->Coupon / F, term_data->MMYield[0] / F) *
                            (tree_data->Accrued[0] - DiscountS[N][N]); /* last one is coupon*/
            else
                IO[i][N] -= deal_data->Coupon / F *
        ...
        if (deal_data->SFSNb > 1)
        { /* for the case we use new prepayment function */

            weights = mbs_dvector(0, prepay_engine.SFSNb - 1);
            if (err = (get_Trigger_Distribution(&prepay_engine, deal->ptime, wam, weights)))
                return (err);
        ...
        {
            io[qm] = IO[0][N]; /* for the case where we don't use new prepayment function */
            po[qm] = PO[0][N];
        }
        ...
        if (casenumber == 1)
        { /* calibrate both speedup and rationality */
            ...
            if (q == 0)
            {
                deal_data->rational =
                    rtn[0] *
                    (1.0 + (100. / deal_data->Coupon) *
                               ((io[0] + po[0]) / (deal_data->Inppo + deal_data->Inpio) -
                                1.)); /* guess, coupon number roughly equals to dColl/drtn */
                deal_data->Speedup =
            ...
            if (q == 1)
            {
                deal_data->rational =
                    rtn[1] *
                    (1.0 + (100. / deal_data->Coupon) *
                               ((io[1] + po[1]) / (deal_data->Inppo + deal_data->Inpio) - 1.));
            ...
            if (q >= 2)
                rs(io,
                   ...
                   &(IO_rtn),
                   &(PO_spd),
                   &(PO_rtn)); /* 2d linear interpolation when q>=2*/
        }
        if (casenumber == 2)
        { /* given io+po and rational, solve for speed */
            ...
            if (q == 0)
                deal_data->Speedup =
                    spd[0] *
                    (1.0 + 50. * ((po[0] + io[0]) / (deal_data->collatoral) - 1.)); /* guess work */

            if (q >= 1)
                deal_data->Speedup =
                    spd[0] + (deal_data->collatoral - io[0] - po[0]) * (spd[1] - spd[0]) /
                                 (io[1] + po[1] - io[0] - po[0]); /* linear interpolation */
        }
        if (casenumber == 3)
        { /* given Io+po and speed, solve for rational */
            ...
            if (q == 0)
                deal_data->rational =
                    rtn[0] *
                    (1.0 + 25. * ((io[0] + po[0]) / (deal_data->collatoral) - 1.)); /* guess */

            if (q >= 1)
                deal_data->rational =
                    rtn[0] + (deal_data->collatoral - io[0] - po[0]) * (rtn[1] - rtn[0]) /
                                 (io[1] + po[1] - io[0] - po[0]); /* linear interpolation */
        }
        if (casenumber == 4)
        { /* given io figure out speedup */
            ...
            if (q >= 1)
                deal_data->Speedup = spd[0] + (deal_data->Inpio - io[0]) * (spd[1] - spd[0]) /
                                                  (io[1] - io[0]); /* linear interpolation */
        }

        if (casenumber == 5)
        { /* given io figure out rational */

            if (fabs(io[qm] - deal_data->Inpio) <= ACURA / 3.)
                break; /* if good enuogh, get out early */

            if (q == 0)
                deal_data->rational = rtn[0] * (io[0] / deal_data->Inpio); /*guess work */

            if (q >= 1)
                deal_data->rational = rtn[0] + (deal_data->Inpio - io[0]) * (rtn[1] - rtn[0]) /
                                                   (io[1] - io[0]); /* linear interpolation */
        }

        if (casenumber == 6)
        { /* given po figure out speedup */
            ...
            if (q >= 1)
                deal_data->Speedup = spd[0] + (deal_data->Inppo - po[0]) * (spd[1] - spd[0]) /
                                                  (po[1] - po[0]); /* linear interpolation */
        }

        if (casenumber == 7)
        { /* given po figure out rational */
            ...
            if (q >= 1)
                deal_data->rational = rtn[0] + (deal_data->Inppo - po[0]) * (rtn[1] - rtn[0]) /
                                                   (po[1] - po[0]); /* linear interpolation */
        }

        if (casenumber == 8)
        { /* given po/io figure out speedup */
            ...
            if (q >= 1)
                deal_data->Speedup =
                    spd[0] + (deal_data->stripratio - po[0] / io[0]) * (spd[1] - spd[0]) /
                                 (po[1] / io[1] - po[0] / io[0]); /* linear interpolation */
        }

        if (casenumber == 9)
        { /* given po/io figure out rational */
            ...
            if (q >= 1)
                deal_data->rational =
                    rtn[0] + (deal_data->stripratio - po[0] / io[0]) * (rtn[1] - rtn[0]) /
                                 (po[1] / io[1] - po[0] / io[0]); /* linear interpolation */
        }

        if (casenumber == 10)
        { /* implied vol for collatoral */
            *price = io[0] + po[0];
            break;
        }
        ...
        { /* oas analysis */
            ...
            { /*first try */
                if ((io[0] + po[0]) > deal_data->collatoral)
                    term_data->OAS += 20.; /* if calculated price too large, increase oas */
                else
                    term_data->OAS -= 20.; /* if calculated price too small, decrease oas */
            }

            if (q >= 1)
                term_data->OAS =
                    oas[0] + (deal_data->collatoral - io[0] - po[0]) * (oas[1] - oas[0]) /
                                 (io[1] + po[1] - io[0] - po[0]); /* linear interpolation */
        }                                                         /* end case 13 */
        ...
        { /*  io oas analysis */
            ...
            if (q >= 1)
                term_data->OAS = oas[0] + (deal_data->Inpio - io[0]) * (oas[1] - oas[0]) /
                                              (io[1] - io[0]); /* linear interpolation */
        }                                                      /* end case 14 */
        ...
        { /* po  oas analysis */
            ...
            if (q >= 1)
                term_data->OAS = oas[0] + (deal_data->Inppo - po[0]) * (oas[1] - oas[0]) /
                                              (po[1] - po[0]); /* linear interpolation */
        }                                                      /* end case 15 */
    ...
    if (casenumber == 1)
    {
        //		printf("the gradient of IO with respect to speed up factor is %lf, to
        //steepness factor is %lf. \n", 			IO_spd, IO_rtn); 		printf("the gradient of PO with respect to
        //speed up factor is %lf, to steepness factor is %lf. \n", 			PO_spd, PO_rtn); 		printf("the
        //gradient of collatoral with respect to speed up factor is %lf \n", 			IO_spd+PO_spd);
    ...
    {                /* print out new prepayment */
            ...
            for (j = 0; j < NBPOINTS; j++)
            {
                if (deal_data->AlorCPR == 'C')
                    deal_data->AmortLevel[i][j] =
                        100. - 100 * pow(1. - deal_data->AmortLevel[i][j],
        ...
        for (j = 0; j < NBPOINTS; j++)
        {                          /* write old ppf */
                                   //			for (i=0; i<MAXNUMP; i++) fprintf(ofpios, "   %lf",
                                   //deal_data->AmortLevel1[i][j]);
    ...
    double*      price)
{
    ...
    long Date,                             /* Maturity of the current money market instrument */
        ...
        IndexSwap[] = {
            4, 6, 8, 10, 14, 20, 40, 60}; /* Maturities in half years of the benchmark swaps */
    double F, /* 'A'nnual or 'S'emi-Annual frequency of the yield curve expressed as a double */
    ...
    if (err = Build_Tree(
            term_data, /* Re-build the two dimension tree */
            tree_data,
            prepay_data))
        return (nrerror(err));

    if (err = impp(
            term_data, /* Re-calculate option price */
            tree_data,
            deal_data,
            history,
            density,
    ...
    do
    {
        ...
        RateTweak[i] = (RateTweak[i] - Option) / Duration[i] *
                       10000.; /* The Notional of the benchmark instrument we have to hold is
                                  determined by the */
        ...
        /* We stop if the last benchmark instrument has a maturity greater than the */
             ...
             (Daysact(Date, deal_data->Maturity) >
              0)); /* maturity of the underlying as the underlying is not sensitive to the     */
    ...
    //	PrintTweak (    Option,                                                 /* Print tweaks into
    //an Ascii file */ 			RateTweak, 			Duration, 			prepay_data);
    return (0);
...
)
{
    ...
    Bottom = (int)max(0., NbPer - tree_data->NodeMax); /* If NbPer>NodeMax the interest rate tree is
    ...
    for (i = Bottom; i <= Top; i++)
    {
        MBS_Prepay_Engine_SMM(
            smm, prepay_engine, CouponNb /*paymentNb*/, ParYield[i], Month, NbPer - N);
...
int TweakSCurve(DEAL_DATA* deal_data, long today)  // 1 means succcessful
{
    ...
    {  // if input is CPR
        ...
        for (i = 0; i < MAXNUMP; i++)
            spdbase(
                deal_data->AmortLevel[i],
                deal_data->Speedup,
                deal_data->rational,
                NBPOINTS);  // speed up and steepen
    }                       // end if
    else if (deal_data->Schedule == 'N')
    {  // if input average life and no scheduled amortization, suitable for busted PACs
        for (i = 0; i < MAXNUMP; i++)
        {
            for (j = 0; j < NBPOINTS; j++)
    ...
    else if (deal_data->Balloon == 'Y')
    { /*if avg life and sch amort and balloon, we left out non-balloon,if there is such a thing, set
        ...
        for (j = 0; j < 2 * deal_data->SFSNb; j++)
            deal_data->Amort[j] = alsmm2(
                12. * deal_data->Amort[j] - deal_data->NbFixedPrepay,
                deal_data->Gwac,
                deal_data->BalloonSch,
                effmat,
                deal_data->Term - effmat);

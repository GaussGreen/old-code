/**********************************************************************************************
Portfolio Optimisation for a certain amount of Contracts given their Average Daily Return 
and Daily St Dev and Correlation Matrix. We optimise the Average Return of the Portfolio 
for a given maximum loss and probaility of losing more than that maximum loss (15%, 2.5% and 0.5%).
Author : Ezra Nahum (May 2001)
*********************************************************************************************/
...
double ComputeFunction(double lambda, long size, double *mu, double *sigma, double **Correl, long ndays, double DrawDown, int level, double *PortfComp)
{
int i,j;
...
for (i=1;i<=size;i++)
{
	for (j=1;j<=size;j++)
	{
		if (i==j)
		{
			MatrixSystem[i][j] = (lambda-ndays)*mu[i]*mu[i]*ndays+level*level*ndays*sigma[i]*sigma[i];
		}

		else
		{
			MatrixSystem[i][j] = (lambda-ndays)*mu[i]*mu[j]*ndays+level*level*ndays*sigma[i]*sigma[j]*Correl[i][j];
		}
	}
}
...
/*Memory allocated within the function, make sure to free it!*/
systemsolution = product_matrix(MatrixSystemInverted,1,size,1,size,righthandvector,1,size,1,1);
...
for (i=1;i<=size;i++)
{
	for(j=1;j<=size;j++)
	{
		variance+=systemsolution[i][1]*systemsolution[j][1]*sigma[i]*sigma[j]*Correl[i][j];
	}
}
...
Err srt_f_PortfolioOpt(long size,double *mu, double *sigma, double **Correl,long ndays,double DrawDown,int level, double *PortfComp)
{
Err err = NULL;
int i,j;
double fx, dfx;
...
for (j=1;j<=1000;j++)
{
	...
	funcs[j] = ComputeFunction(lambdas[j],size,mu,sigma,Correl,ndays,DrawDown,level, PortfComp);
	if (funcs[j] > 0.0)
	{
		sgn[j] =1.0;
	}
...
fx = ComputeFunction(lambda,size,mu,sigma,Correl,ndays,DrawDown,level, PortfComp);

dfx = (ComputeFunction(lambda+bump,size,mu,sigma,Correl,ndays,DrawDown,level, PortfComp)-fx)/bump;
...
if (totalaverage1>totalaverage2)
{
	...
	fx = ComputeFunction(lambda1,size,mu,sigma,Correl,ndays,DrawDown,level, PortfComp);
}
...
precision = 1.0;
funcleft =  ComputeFunction(lambdaleft,size,mu,sigma,Correl,ndays,DrawDown,level, PortfComp);
funcright = ComputeFunction(lambdaright,size,mu,sigma,Correl,ndays,DrawDown,level, PortfComp);
	...
	while ((iter < maxiter)&&(precision>=0.0000001))
	{
		...
		funcmid = ComputeFunction(lambdamid,size,mu,sigma,Correl,ndays,DrawDown,level, PortfComp);

		lambdamidleft = (lambdaleft+lambdamid)/2;
		lambdamidright = (lambdaright+lambdamid)/2;

		funcmidleft = ComputeFunction(lambdamidleft,size,mu,sigma,Correl,ndays,DrawDown,level, PortfComp);
		funcmidright = ComputeFunction(lambdamidright,size,mu,sigma,Correl,ndays,DrawDown,level, PortfComp);

    ...
    double*  PortfComp)
{
    ...
    for (i = 1; i <= size; i++)
    {
        for (j = 1; j <= size; j++)
        {
            ...
            {
                MatrixSystem[i][j] = (lambda - ndays) * mu[i] * mu[j] * ndays +
    ...
    /*Memory allocated within the function, make sure to free it!*/
    systemsolution =
        product_matrix(MatrixSystemInverted, 1, size, 1, size, righthandvector, 1, size, 1, 1);
    ...
    for (i = 1; i <= size; i++)
    {
        ...
        {
            variance +=
                systemsolution[i][1] * systemsolution[j][1] * sigma[i] * sigma[j] * Correl[i][j];
        }
    }
    ...
    double*  PortfComp)
{
    ...
    */
    ...
    for (j = 1; j <= 1000; j++)
    {
        ...
        funcs[j] =
            ComputeFunction(lambdas[j], size, mu, sigma, Correl, ndays, DrawDown, level, PortfComp);
        if (funcs[j] > 0.0)
        {
            sgn[j] = 1.0;
        }
   ...
   */
    ...
    dfx = (ComputeFunction(
               lambda + bump, size, mu, sigma, Correl, ndays, DrawDown, level, PortfComp) -
           fx) /
          bump;
...
/*
Err srt_f_PortfolioOpt(long size,double *mu, double *sigma, double **Correl,long ndays,double
DrawDown,int level, double *PortfComp)
{
Err err = NULL;
int i;
...
funcright = ComputeFunction(lambdaright,size,mu,sigma,Correl,ndays,DrawDown,level, PortfComp);
        ...
        while ((iter < maxiter)&&(precision>=0.0000001))
        {
                ...
                funcmid = ComputeFunction(lambdamid,size,mu,sigma,Correl,ndays,DrawDown,level,
PortfComp);
                ...
                funcmidleft =
ComputeFunction(lambdamidleft,size,mu,sigma,Correl,ndays,DrawDown,level, PortfComp); funcmidright =
ComputeFunction(lambdamidright,size,mu,sigma,Correl,ndays,DrawDown,level, PortfComp);

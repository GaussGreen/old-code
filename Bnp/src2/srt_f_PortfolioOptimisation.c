...
than that maximum loss (15%  , 2.5% and 0.5%). Author : Ezra Nahum (May 2001)
*********************************************************************************************/
...
/*
Err srt_f_PortfolioOpt(long size  ,double *mu  , double *sigma  , double
**Correl  ,long ndays  ,double DrawDown  ,int level  , double *PortfComp)
{
Err err = NULL;
int i;
...
precision = 1.0;
funcleft =  ComputeFunction(lambdaleft  ,size  ,mu  ,sigma  ,Correl  ,ndays
,DrawDown  ,level  , PortfComp); funcright = ComputeFunction(lambdaright  ,size
,mu  ,sigma  ,Correl  ,ndays  ,DrawDown  ,level  , PortfComp);
        ...
        while ((iter < maxiter)&&(precision>=0.0000001))
        {
                ...
                funcmid = ComputeFunction(lambdamid  ,size  ,mu  ,sigma  ,Correl
,ndays  ,DrawDown  ,level  , PortfComp);

                lambdamidleft = (lambdaleft+lambdamid)/2;
                lambdamidright = (lambdaright+lambdamid)/2;

                funcmidleft = ComputeFunction(lambdamidleft  ,size  ,mu  ,sigma
,Correl  ,ndays  ,DrawDown  ,level  , PortfComp); funcmidright =
ComputeFunction(lambdamidright  ,size  ,mu  ,sigma  ,Correl  ,ndays  ,DrawDown
,level  , PortfComp);

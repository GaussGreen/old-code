...
Err cpd_alloc_smile_vol_market(	int					num_vols,
...
Err cpd_fill_smile_vol_market(	long					today,
...
Err err = NULL;
	...
	{
		if(times)	smile_mkt->times[i]		= times[i];
		if(dates)	smile_mkt->dates[i]		= dates[i];
		if(forward)	smile_mkt->forward[i]	= forward[i];
		if(sigma)	smile_mkt->sigma[i]		= sigma[i];
		if(alpha)	smile_mkt->alpha[i]		= alpha[i];
		if(beta)	smile_mkt->beta[i]		= beta[i];
		if(rho)		smile_mkt->rho[i]		= rho[i];
		if(pi)		smile_mkt->pi[i]		= pi[i];

		if(!dates && times)	smile_mkt->dates[i] = today + (long)(smile_mkt->times[i] * DAYS_IN_YEAR);
		if(!times && dates)	smile_mkt->times[i] = (smile_mkt->dates[i] - today) * YEARS_IN_DAY;
...
Err err = NULL;
	...
	if(smile_mkt->forward)  free(smile_mkt->forward);
	if(smile_mkt->sigma)  free(smile_mkt->sigma);
	if(smile_mkt->alpha)  free(smile_mkt->alpha);
	if(smile_mkt->beta)  free(smile_mkt->beta);
	if(smile_mkt->rho)  free(smile_mkt->rho);
	if(smile_mkt->pi)  free(smile_mkt->pi);
	if(smile_mkt->times)  free(smile_mkt->times);
	if(smile_mkt->dates)  free(smile_mkt->dates);

return err;
}

Err cpd_vol_get_vol(	double				forward,
						double				fix_time,
						double				strike,
...
Err err = NULL;
	...
	{
		//SABR
		err = srt_f_optsarbvol(	forward, strike, fix_time,
	...
	{
		//BMM
		err = srt_f_optbmmvol(	forward, strike, fix_time,
								smile_params->sigma, smile_params->alpha,
								smile_params->beta, smile_params->rho,
								smile_params->pi,
...
Err cpd_vol_get_price(	int					type,
						double				Forward,
						double				Maturity,
						double				Strike,
						SMILE_PARAMETERS	smile_params,
...
Err err = NULL;
	...
	else if(smile_params->smile_spec_type == 3) //BMM beta
	{
		if (type >= 3)
		{
			...
			{
				err = BMM_Option_Price(	Forward, Strike, Maturity, 
										smile_params->sigma, smile_params->alpha, smile_params->beta, 
										smile_params->rho, smile_params->pi, type==3?SRT_CALL:SRT_PUT, Value);
				if(err) goto FREE_RETURN;
			}
			else
			{
				err = BMM_Option_Price_quick(	Forward, Strike, Maturity, 
										smile_params->sigma, smile_params->alpha, smile_params->beta, 
										smile_params->rho, smile_params->pi, type==3?SRT_CALL:SRT_PUT, Value);
...
Err err = NULL;
	...
	if(IsTime)
	{
		smile_params->alpha = interp(smile_mkt->times, smile_mkt->alpha, smile_mkt->num_vols, time, 0, NULL);
		smile_params->beta = interp(smile_mkt->times, smile_mkt->beta, smile_mkt->num_vols, time, 0, NULL);
		smile_params->rho = interp(smile_mkt->times, smile_mkt->rho, smile_mkt->num_vols, time, 0, NULL);

		if(smile_mkt->smile_spec_type == 2 || smile_mkt->smile_spec_type == 3)
			smile_params->pi = interp(smile_mkt->times, smile_mkt->pi, smile_mkt->num_vols, time, 0, NULL);
	}
	else
	{
...
Err err = NULL;
	...
	if(IsTime)
		smile_params->sigma = interp(smile_mkt->times, smile_mkt->sigma, smile_mkt->num_vols, time, 0, NULL);
	else
	{
		dates = calloc(smile_mkt->num_vols, sizeof(double));
		for(j=0; j<smile_mkt->num_vols; j++)
...
Err cpd_vol_get_smile_params_from_otc(	int					smile_spec_type,
										int					row_idx,
										int					col_idx,

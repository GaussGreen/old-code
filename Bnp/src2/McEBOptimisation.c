...
void mceb_copy_nondynamic_params(MCEBPARAMS sParamsCpy, MCEBPARAMS sParamsSrc)
{
	...
	/* File Name where to save all infos */
	memcpy(sParamsCpy->sFilePath,sParamsSrc->sFilePath,sizeof(sParamsSrc->sFilePath) * sizeof(char));

}

Err	mceb_allocate_params(	MCEBPARAMS	sParams,
							long		lNbDates)
{
	if (sParams)
	{
...
void mceb_free_params(MCEBPARAMS sParams)
{
	if (sParams)
	{
		if (sParams->dBarrier) free(sParams->dBarrier);
		if (sParams->dCoefLin) free_dmatrix(sParams->dCoefLin, 0, sParams->lNbDates-1, 0, sParams->iNbIndex);
		if (sParams->dFee) free (sParams->dFee);
		if (sParams->dMarketFwdIV) free (sParams->dMarketFwdIV);
		if (sParams->dModelFwdIV) free (sParams->dModelFwdIV);
		if (sParams->dOneTimeCall) free (sParams->dOneTimeCall);
		if (sParams->dOneTimePartial) free (sParams->dOneTimePartial);
		if (sParams->dExeProba) free (sParams->dExeProba);		
	}
}

Err mceb_allocate_savevalues_for_GRFN(long			iNbPaths,
									  long			iNbEvent,
									  MCEBPARAMS	sParams,
									  double		****dSaveValues)
{
	*dSaveValues = f3tensor(0, iNbEvent - 1, 0, sParams->iNbIndex + sParams->iHasNumeraire + sParams->iHasFees, 0, iNbPaths - 1);
...
void mceb_free_savevalues_for_GRFN(	double		***dSaveValues,
									long		iNbPaths,
									long		iNbEvent,
									MCEBPARAMS	sParams)

{
	if (dSaveValues)
	{
		free_f3tensor(dSaveValues, 0, iNbEvent - 1, 0, sParams->iNbIndex + sParams->iHasNumeraire + sParams->iHasFees, 0, iNbPaths - 1);
	}
}

void mceb_fill_savevalues_from_GRFN(double		**dSaveValues,
									double		*dResEvt,
									long		lSimulIdx,
									double		dDF,
									MCEBPARAMS	sParams)
...
int	k;
	...
	{
		dSaveValues[sParams->iNbIndex+1][lSimulIdx] = dResEvt[sParams->iColNumeraire] / dDF;
	}

	if (sParams->iHasFees)
	{
		dSaveValues[sParams->iNbIndex+sParams->iHasNumeraire+1][lSimulIdx] = dResEvt[sParams->iColFees] / dDF;
	}
}


Err	mceb_adjust_fwdiv(	double		***save_values,
						long		start_date_idx,
						long		end_date_idx,
						long		nb_paths,
						int			*optimise,
		...
		for (j=start_date_idx; j<=end_date_idx; j++)
		{
			...
			if (params->iAdjustIV)
			{
				...
				if (params->iAddMultFee == 0)
				{
					...
					if (fabs(params->dMarketFwdIV[j] - value) < 1.0E-10)
					{
						fee = 0.0;
					}
					else
...
Err find_and_optimise_boundary(	double		***save_values,								
								long		nb_dates,
								long		nb_paths,
								int			*optimise,
								MCEBPARAMS	params,
...
Err		err = NULL;
	...
	/* Adjust Fwd IV */
	err = mceb_adjust_fwdiv(save_values,
							0,
							nb_dates-1,
							nb_paths,
							optimise,
	...
	if (!next_payoff || !cur_payoff || !mat || (params->iMultiIndex && (!x || !y)))
	{
	...
	if (params->iIsKO)
	{
		...
		{
			if (first_optim && params->iNbIndex > 1 && params->iRemoveLastOnLast == 1)
			{
			...
			{				
				...
				if (!first_optim && params->iCalcOneTimePartial)
				{
					...
					{					
						...
						{							
							...
							{
								/* Substract Paid Fees */
								cur_payoff[1][i] += save_valuesj[params->iNbIndex + params->iHasNumeraire + 1][i];
							}
				...
				if (params->iCalcOneTime || params->iCalcOneTimePartial)
				{
					...
					while (k<nb_dates-add_one_current && (k == j || !optimise[k]))
					{
						for (i=0; i<nb_paths; i++)
						{
							total_payoff[i] += save_values[k+add_one_current][params->iNbIndex][i];
						}
				...
				{															
					for (i=0; i<nb_paths; i++)
					{
						cur_payoff[1][i] = next_payoff[i] + save_values[j+add_one_current][params->iNbIndex][i];						
						y[i] = -cur_payoff[1][i];

						if (params->iHasFees)
						{
							/* Substract Paid Fees */
							y[i] -= save_valuesj[params->iNbIndex + params->iHasNumeraire + 1][i];
						}
				...
				if (iDoSmoothing)
				{
					...
					dStdCallSpread[j] = dValue2 / nb_paths - (dValue / nb_paths * dValue / nb_paths);
				...
				for (i=0; i<nb_paths; i++)
				{
					...
					if (params->iHasFees)
					{
						payoff += save_valuesj[params->iNbIndex + params->iHasNumeraire + 1][i];
					}
				...
				if (index >= nb_paths-1)
				{
					params->dBarrier[j] = cur_payoff[0][nb_paths-1] + 0.1 * fabs(cur_payoff[0][nb_paths-1]);
				}
				else if (index >= 0)
				{
					params->dBarrier[j] = 0.5 * (cur_payoff[0][index] + cur_payoff[0][index+1]);
				}
				else
				{
					params->dBarrier[j] = cur_payoff[0][0] - 0.000001 * fabs(cur_payoff[0][0]);
				}
				...
				if (params->iHasFees)
				{
					...
					{
						next_payoff[(int) cur_payoff[2][i]] = -save_valuesj[params->iNbIndex + params->iHasNumeraire + 1][i];
					}
				}
				else
				{
	...
	else
	{
		...
		for (j=nb_dates-1; j>=0; j--)
		{
			if (j == nb_dates - 1 && params->iNbIndex > 1 && params->iRemoveLastOnLast == 1)
			{
				/* Remove last index for last date */
				iNbIndex = params->iNbIndex - 1;
			}
			...
			if (optimise[j])
			{
				...
				if (params->iMultiIndex)
				{
					...
					for (i=0; i<nb_paths; i++)
					{
						cur_payoff[1][i] = save_values[j+add_one_current][params->iNbIndex][i];
		...
		for (i=0; i<nb_paths; i++)
		{
			...
			for (j=0; j<nb_dates; j++)
			{
				...
				{										
					...
					{
						dNotional *= (params->dBarrier[j] + dStdCallSpread[j] - dIndex) / (2.0 * dStdCallSpread[j]);
...
FREE_RETURN:
	...
	if (call_col) free (call_col);

	if (total_payoff) free(total_payoff);

	if (dStdCallSpread) free(dStdCallSpread);

	return err;
}

Err optimise_boundary_info(	double	***save_values,
							long	nb_dates,
							long	nb_paths,
							int		*optimise,
							int		iCallCurrent,
...
Err		err = NULL;
	...
	for (i=0; i<nb_paths; i++)
	{
		fprintf(stream, "%f	%f\n", save_values[i][1][0], save_values[i][2][0]);
	}
			...
			if (optimise[j])
			{
				...
				for (i=0; i<=index; i++)
				{
					...
					if (cur_payoff[1][i] > 0.0)
					{
						...
						{
							infos[call_date[pathnb]][3 + nb_dates + j] += cur_payoff[2][i] - cur_payoff[1][i];

							if (easy_date[pathnb])
							{
								infos[call_date[pathnb]][3 + nb_dates + call_date[pathnb]] -= cur_payoff[2][i];
				...
				for (i=index+1; i<nb_paths; i++)
				{
					...
					if (call_date[pathnb] > 0)
					{
						...
						infos[j][3 + nb_dates + call_date[pathnb]] += cur_payoff[1][i] - cur_payoff[2][i];
...
static void multiplic_matrix_vector(	long	nx,
										long	ny,
										double	**x,
										double	*y,
										double	*res)
...
static void multiplic_with_special(	long	nx,
									long	ny,
									double	**x,
									double	**y,
									double	**res)
...
static void square_matrix(	long	n,
							long	m,
							double	**x,
							double	**res)
{
...
Err	find_linear_coef(	long	p,	/* number of variables = number of row in Transp(X) */
						long	n,	/* number of observations */
						double	*y,
						double	**x,
						double	*b,
						double	**mat3 /* for memory saving */)
{
double	**mat1	= NULL,
		**mat2	= NULL;
...
FREE_RETURN:
	...
	if (mat1) free_dmatrix(mat1, 0, p - 1, 0, p  - 1);
	if (mat2) free_dmatrix(mat2, 1, p, 1, p);
...
Err find_best_optim_dates(		double		***save_values,								
								long		nb_dates,
								long		nb_target,
								long		nb_paths,
								int			*optimise,
...
FREE_RETURN:
	...
	if (optim_test) free(optim_test);
	if (best_optim) free(best_optim);
	if (p) free(p);

...
void mceb_copy_nondynamic_params(MCEBPARAMS sParamsCpy, MCEBPARAMS sParamsSrc)
{
    ...
    /* File Name where to save all infos */
    memcpy(
        sParamsCpy->sFilePath, sParamsSrc->sFilePath, sizeof(sParamsSrc->sFilePath) * sizeof(char));
}
...
void mceb_free_params(MCEBPARAMS sParams)
{
    if (sParams)
    {
        ...
        if (sParams->dCoefLin)
            free_dmatrix(sParams->dCoefLin, 0, sParams->lNbDates - 1, 0, sParams->iNbIndex);
        if (sParams->dFee)
            free(sParams->dFee);
...
Err find_and_optimise_boundary(
    ...
    double*    error)
{
    double *next_payoff = NULL, **cur_payoff = NULL, **x = NULL, *y = NULL, **mat = NULL,
           *total_payoff = NULL, *b, **save_valuesj;
    int* call_col        = NULL;
    ...
    /* Adjust Fwd IV */
    err = mceb_adjust_fwdiv(save_values, 0, nb_dates - 1, nb_paths, optimise, params);
        ...
        for (j = nb_dates - 1; j >= 0; j--)
        {
            ...
            if (optimise[j])
            {
                ...
                if (params->iMultiIndex)
                {
                    ...
                    {
                        cur_payoff[1][i] =
                            next_payoff[i] + save_values[j + add_one_current][params->iNbIndex][i];
                        y[i] = -cur_payoff[1][i];
                ...
                {
                    params->dBarrier[j] =
                        cur_payoff[0][nb_paths - 1] + 0.1 * fabs(cur_payoff[0][nb_paths - 1]);
                }
                else if (index >= 0)
                {
...
static void multiplic_matrix_vector(long nx, long ny, double** x, double* y, double* res)
{
    int     i, j;
    double  sum;
    double* xi;
...
static void multiplic_with_special(long nx, long ny, double** x, double** y, double** res)
{
    int     i, j, k;
    double  sum;
    double *xi, *resi;
    ...
    double*    error)
{
    ...
    err = find_and_optimise_boundary(
        save_values, nb_dates, nb_paths, optim_test, params, &best_res, error);
    ...
    while (!twiddle(&x, &y, &z, p))
    {
        ...
        err = find_and_optimise_boundary(
            save_values, nb_dates, nb_paths, optim_test, params, &res, error);
    ...
    err = find_and_optimise_boundary(
        save_values, nb_dates, nb_paths, best_optim, params, value, error);

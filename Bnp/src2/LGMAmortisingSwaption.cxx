...
static LGMErr FitAllZetaAmortisingSwaptions(LGM_TS *tsPtr, LGMCalSet *CSPtr,
                                            double NumPeriod, double *index,
                                            double *d, double *dstart,
                                            double *mktprice);
...
LGMErr LGMCalFixKapamortising(
    ...
    LGMCalSet *CSPtr, double NumPeriod, double *index, double *d,
    double *dstart, double *mktprice); /* Reference caplets */
...
double RecGDer(double ystar, long n, double *a,
               double DStart,                           /* receiver swaption */
...
LGMErr LGMamortisingCalibration(
    ...
    double NumPeriod, double *index, double *d, double *dstart,
    double *mktprice,
    /* info about the deal */
...
/* Routine to construct the "1 into k" swaptions. For all these swaptions , the
exercise date is ftEx        , which is the latter of the first exercise date
tEx[1] or MinMoEx months from today. The start dates of all these swaptions are
ftStart        , and the fixed leg pay dates of swaption j are tPay[i]        ,
for i=ffirst
      ,...        ,j. The swaptions run from j=nfirst to j=nlast. The coverage
between paydates ftStart and ftPay[ffirst] is cvgfix        , and the discount
factor to ftStart is DfixStart. */
/* Note: RefDealsCommon must be called first */
    ...
    LGMCalSet *CSPtr) /* Output */
{
  ...
  fullper =
      155; /* if period has more than fullper days        , count it as full*/
  if (conv->sfreq ==
...
/*******************************************/
/* Routine to construct the caplets */
/* Construct the caplets j        , j=1        ,2        , ...        , nEx.
Caplet j has the fixing date tEx[j]        , the start date as tStart[j] , and
the end date tEnd[j]. Discount factors to the start date and end date are
DStart[j] and Dcap[j]        , and coverage from the start to end date is
cvgcap[j]. The  new quantities we need are tEnd[]        , Dcap[]        , and
cvgcap[]. The rest are calculated in RefDealsCommon */
/* Note: RefDealsCommon must be called first to set nEx        , tEx[]        ,
 * tStart[]        , and DStart[] */
...
/*******************************************/
/* Routine to construct the long swaptions */
/* Construct the long swaption j. j=1        ,2        , ...        , nEx. Long
swaption j has exercise date tEx[j]        , start date tStart[j]        , and
fixed leg pay dates tPay[i]        , i=ifirst[j] to i=nlong[j]. The discount
factor to the start date is DStart[j]        , and the discount factors to the
pay dates tPay[i] are Dpay[i]. The coverage between start and the first pay date
is cvgfirst[j]        , and the coverages for the period between tPay[i-1] and
tPay[i] is cvgpay[i] for i>ifirst[j]
*/
/* Note: RefDealsCommon must be called first to set nEx        , n        , nPay
   , tEx[]        , tStart[]        , tPay        , cvgpay        , Dpay , n
   , and nPay */
    ...
    LGMCalSet *CSPtr) /* Output */
{
  ...
  /* for each tEx[j]        , find the first pay date tPay[i] which has
   * tPay[i]>tEx[j]
   */
  /* if a pay date tPay[i] lies between tEx[j] and tStart[j]        , change
   * tPay[i] to tStart[j] */
  i = 1;
  for (j = 1; j <= nEx; j++) {
  ...
  for (j = 1; j <= nEx; j++) {
    ...
    if (nlong[j] <
        n)          /* long swaption ends at paydate n        , unless can't */
  ...
  for (j = 1; j <= nEx; j++) {
    ...
    } else /* CALIBRATION AT THE EQUIVALENT STRIKE */
    {
      error = FindFixRateRefInst(
          ...
          DLast, /* df's for ref start and end        , actual end */
          tEx[j], tStart[j], tPay[iend], /* exer        , start        , and end
                                            date of ref swaption */
               ...
               LGMCalSet *CSPtr) /* Output */
{
  ...
  /* Find fixed leg pay dates tPay[1        ,...        ,nPay]; start date is
   * tPay[0] */
  ...
  /* Allocate space for common exer dates        , start dates        , and dis
   * factors to start dates */
  if (ResExer == 0)
    CSPtr->nEx = n;
  else
    CSPtr->nEx = nEx;
  ...
  /* Fill in common exercise dates        , start dates        , and dis factors
   * to start dates */
  if (ResExer != 0) {
    for (j = 1; j <= nEx; j++) {
      tEx[j] = TauArr[j];
...
static LGMErr FindFixRateRefInst(
  ...
  case IRR:
    ...
    /*
                    theta = InterpPVratio(tEx        , nArr        , FVArr ,
       TauArr        , tLast); if (DLast > DStart/(1.0 + 0.25*Rsw)) DLast =
       DStart/(1.0 + 0.25*Rsw); *rfix = Rsw*(1.0 +
       (theta-1.0)/(1.0-DLast/DStart)); break;
    */
    ...
    String outfile)            /* output file name for log file (unused) */
{
  ...
  if (skipCalib != 1) {
    error = LGMamortisingCalibration(
        ...
        NumPeriod, index, d, dstart, mktprice, dealtype, dealPtr, EffnEx,
    ...
    double *intrinsicValPtr) /* ptr to intrinsic value (NULL => not req'd) */
{
  ...
  error = LGMamortisingnewautocal(
      ...
      NumPeriod, index, d, dstart, mktprice, ItsAMidAt,
...
long swaption j to the market price Vlong[j] */
  for (j = 1; j <= nEx; j++) {
    ...
    if ((j == 1) ||
        ((index[j - 1] > index[j - 2]) && (j <= nEx - noswaption + 1))) {
      zeta[j] = FitOneZetaSwap(n, dispayment, dstart[ifirst + 2], recpay,
                               coupon_G, Gst, mktprice[ifirst + 1]);
    ...
    double *dstart, double *mktprice) /* Reference caplets */
{
  ...
  *LGMtsPtrPtr = NULL;
  ...
  if (usecaps == 1) {
    ...
    error = FitAllZetaAmortisingSwaptions(tsPtr, CSPtr, NumPeriod, index, d,
                                          dstart, mktprice);
  }
  ...
  */
    case FixKappa:
      error = LGMCalFixKapamortising(
          ...
          RefDealPtr, NumPeriod, index, d, dstart, mktprice);

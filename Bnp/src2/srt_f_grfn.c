...
static Err attach_ytat(double today, double amdt, GrfnEvent* ev, SrtUndPtr und, int und_index)
{
    ...
    if (ev->dflen[und_index] > 0)
    {
        ...
        if (ev->d + amdt > today)
        {
            if (mdl_type == VASICEK)
            {
                Vasicek_Y_T_at_param(
                    (Ddate)ev->d + amdt, df_ddates, ev->dflen[und_index], und, ev->yp[und_index]);
            }
            else
            {
                Y_T_at_t_param(
        ...
        else /* The parameters that we will need to rebuild a zero coupon for a PAST payment in the
                future (with today) */
        {
            if (mdl_type == VASICEK)
            {
                Vasicek_Y_T_at_param(
                    (Ddate)today, df_ddates, ev->dflen[und_index], und, ev->yp[und_index]);
            }
            else
                Y_T_at_t_param(
                    (Ddate)today, df_ddates, ev->dflen[und_index], und, ev->yp[und_index]);
...
Err srt_f_grfn(
    ...
    double** knownpayments) /* A report of known CF [2], paid on pay dates [0], according to event
                               dates [1] ([0..*nrows-1])*/
{
    GrfnDeal   gd;
    SrtStpPtr  sptr = NULL;
    ...
    /* If calculation date is > last event date, then extend tableau: create a phantom event having
     * a zero cash-flow. */
        ...
        ((grfnparam->end_of_day_flg == SRT_YES) && (today == (*eventdates)[*nrows - 1])))
    {
        ...
        /* Extend the tableau into a larger one with a zero on the last row (nrows will be
         * incremented) */
    ...
    err = grfn_copy_to_GrfnDeal(
        &gd, *nrows, *ncols, (*sprdsht), (*eventdates), numgrng, grng, auxwidth, auxlen, aux);
    if (err)
    {
        free_the_world(sptr, &gd, &und_info);
        return err;
    ...
    */
    ...
    /* Build all the time steps, for all the dates, past and future (according to event dates, grfn
     * params...) */
    ...
    /* Sets the KNOWN Payments report (pay dates, event dates and cahsflow) as stored in the
     * GrfnDeal */
    ...
    if (grfnparam->skip_evaluation == SRT_NO)
    {
        ...
        if (sptr)
        {
            ...
            if (grfnparam->imp_type == IMPMONTECARLO)
            {
                ...
                if (grfnparam->exfrontier != 100)
                {
                    err = MCCoreExFrontier(
                        grfnparam, sptr, gd, (EvalEventFct)grfn_eval_event, answer, und_info);
                    if (err)
                        return (err);
                }
            ...
            /* Method is Backward: Finite Difference Method (Tree, PDE, McTree...) */
            {
                ...
                err = TreeCore(
                    und, grfnparam, sptr, gd, (EvalEventFct)grfn_eval_event, answer, und_info);
                if (err)
                    return (err);
...
Err srt_f_grfn_val_historical_events(SrtStpPtr* step, GrfnDeal* gd, SrtUndInfo* und_info)
{
    Err       err = NULL;
    SrtStpPtr stp;
    double    cash_flow;
    ...
    /* Loop on all the steps until the step corresponding to today (+ eoday), and deletes them one
     * by one (but today)*/
    while (stp->ddate < gd->today + gd->end_of_day_index)
    {
        /* Evaluate the event attached to the current time step (we do not care about cashflow: only
         * PAY has an impact ) */
        err = grfn_eval_event(
            (GrfnEvent*)(stp->e),
        ...
        /* Frees the step (if before today) and moves on to the next one ( it should exist: today
         * has been created ) */

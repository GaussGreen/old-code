...
static Err attach_ytat(double today, double amdt, GrfnEvent *ev, SrtUndPtr und, int und_index)
{              
Ddate    *df_ddates ;
int      i ;
SrtMdlType	mdl_type;
Err			err;
	...
	/* The parameters that we will need to rebuild a zero coupon in the future*/	
		if (ev->d + amdt > today)
		{
			if(mdl_type == VASICEK)
			{
				Vasicek_Y_T_at_param( (Ddate)ev->d + amdt, df_ddates,ev->dflen[und_index],
							und, ev->yp[und_index]) ;
		...
		else /* The parameters that we will need to rebuild a zero coupon for a PAST payment in the future (with today) */	
		{
			if(mdl_type == VASICEK)
			{
				Vasicek_Y_T_at_param( (Ddate)today, df_ddates,ev->dflen[und_index],
						und, ev->yp[und_index]) ;

			}
			else Y_T_at_t_param( (Ddate)today, df_ddates,ev->dflen[und_index],
						und, ev->yp[und_index]) ;
		}
...
/* Attaches to GrfnDeal information necessary to rebuild zero-coupon (for ALL UND)*/
...
/* Loop through all the time steps */
	...
	/* Check there is an event attached to this date */
		...
		/* Increment the event date to look at if step is an event date */
			...
			if (i>0 && GrfnIsStatus(gd->rowstatus[i-1], GRFNCSAMERICAN))
			{
				if (stp->date<=0)
				{
		...
		/* Attaches at the time step all the info needed to compute zc (Y_T) */
			err = attach_ytat((double)gd->today, amdt, (GrfnEvent *) stp->e, und, und_index) ;
			if (err) 
				return err ;
		}
...
/* Parses the Grfn tableau, and collect all the names and info of the underlyings */
...
/* There should at least be one underlying returned from the call, the domestic one */
...
/* Parse all the cells row by row to create a list of underlyings,
	in the same order as stored in the static _grfn_und_list (grf_f_underlying) */
...
/* Add the different underlying necessary for STOCH RATES FX QUANTOS if they exist,
   and modify the Numeraire index if required */
...
/* Check the size of dates vs number of rows in tableau */
	if ( numeventdates != *nrows )
		return serror("# of Dates and Events do not match in Grfn: %d, %d", numeventdates,*nrows);
...
/* Removes all empty rows , and checks the dates are increasing */
 ...
 /* If calculation date is > last event date, then extend tableau: create a phantom event having a zero cash-flow. */
    if ((today > (*eventdates)[*nrows-1])
	   || ((grfnparam->end_of_day_flg == SRT_YES) && (today == (*eventdates)[*nrows-1])))
    {   
	...
	/* Extend the tableau into a larger one with a zero on the last row (nrows will be incremented) */
...
/* Initialise to 0 the payments (and their dates) of the GrfnDeal (cf PAY function) */
    gd.pay_dates  = NULL;
	gd.event_pay_dates  = NULL;
	gd.pay_amounts  = NULL;
	gd.numpay  = 0;

/* Set today (extracted from the yield curve via the underlying) in the GrfnDeal */
    gd.today        = today;
	
/* Store the domestic underlying name in gd.domestic_und, with the associated curve */
...
/* Get info concerning :number of underlyings, which ones.. (parses the deal) */
...
/*  When two underlyings or more: use MONTE CARLO so far */
	...
	{
		und_info.jumping = SRT_YES; /* If the model and the user agree then go for it */ 
	}
	else
	{
...
/* 
 When two underlyings, if they are lgm/lgm or lgm/bs, one wil soon be able to
 can use tree as well and then the test will be 
...
/* Build all the time steps, for all the dates, past and future (according to event dates, grfn params...) */
	err = srt_f_make_time_step(
			gd.sslength /*-first_unknown_index*/,
			&gd.event_dates[0] /*first_unknown_index]*/,
			und,
			&sptr,
			grfnparam,
			&und_info);
...
/* Loop through underlyings to attach relevant interest rate information: df, spreads, ...*/
	...
	/* Attach to GrfnDeal  information necessary to rebuild zero-coupon (for ALL UNDS) */
		err = attach_params_to_GrfnEvent(
				&gd,
				sptr,
				und,
...
/* ----------- STEP 5: CHOICE OF THE SOLVING DIRECTION : FWD/BKWD --------- */
	...
	{
	/* Set the Grfn direction depending on the results of the parsing on the full tableau */
...
/* Calculate the price ( + other info ) of the GRFN deal (if not skip_evaluation) */
...
/* Copy the results of the last MonteCarlo path (from gd.cells) */
	...
	/* The tableau could have been extended or shrunk (but only for rows) */
		...
		for ( i = ((init_nrows > *nrows ) ? *nrows : init_nrows );i <  init_nrows ; i ++)
		{
			for (j = 0 ; j < *ncols ; j++)
...
/* Sets the KNOWN Payments report (pay dates, event dates and cahsflow) as stored in the GrfnDeal */
...
/* ------------------------------------------------------------------------------------
   ...
   DESCRIPTION     :value a grfn deal initialized by grfn_init_GrfnDeal.
			        ...
			        MCCore or TreeCore depending on the value of grfnparam->imp_type,
  			        which was set in the function grfn_init_GrfnDeal.  One thing it does 
      			    which ought to be done by MCCore is remove nodes which have no events
	      		    in the case of Monte Carlo and LGM (in this case we can use the jumping 
		      	    numeraire.) 
			        This function also sets the value of grfnparam->step_num and
			        grfnparam->node_dim for the tree functions.  The first of these is
			        probably redundant because the tree function themselves will call
			        the SrtStp function create_index(); the second of these is a
      			    legitimate piece of information that GRFN must somehow pass to the 
	      		    valuation function.
		      	    
					The *answer field is passed to the appropriate 
			        valuation function will set it.  
...
/* If skip evaluation: do not evaluate the deal */
	...
	/* There are time steps left  */ 
		...
		/* Method is Forward: Monte-Carlo  **/
			...
			/* This will be executed for ( mdl_type ==LGM || mdl_type ==EQD_BLACK_SCHOLES) */
				if (und_info->jumping)
				{
		...
		/* Get the PV from the IO list of the discretisation method (for futures cash flows) */
		...
		/* Set the Premium in the IO list */
			err = srt_f_IOstructsetpremium((SrtIOStruct *)answer,status,premium,"");
			if (err) 
				return (err) ;   
...
/* This function immediately evaluates historical events and discrads them from the steps */
...
/* Loop on all the steps until the step corresponding to today (+ eoday), and deletes them one by one (but today)*/
	while (stp->ddate < gd->today + gd->end_of_day_index)
	{
	/* Evaluate the event attached to the current time step (we do not care about cashflow: only PAY has an impact ) */
		err = grfn_eval_event  (   
				(GrfnEvent*)(stp->e), 
				NULL,                                /* State variables                    */
				gd,
				NULL,                                /* Discounted Vector in tree          */
	...
	/* Frees the step (if before today) and moves on to the next one ( it should exist: today has been created ) */
...
/* Makes sure all needed past variable information (va,vb...) is attached to today's step */
	err = grfn_attach_past_info_to_future( 
			gd,
			(GrfnEvent**)(&(stp->e)) );
	if (err)
		return err;
	
/* Get the input step to point to the first step on or after today's (+ end of day) step */
	*step = gototop(stp);
...
for (n = numex-2; n>=0;n--)
{
	...
	gradient = 0.0;
	newgradient= 100;
	if (n == numex-2) aux[exfr][n] = storeexfr[n];
	else aux[exfr][n] = aux[exfr][n+1];
	...
	while (j<25)
	{
		...
		if ((j==0)&&(n==numex-2)) grfnparam->exfrontierfirst_time = SRT_YES;
	...
	if (gradient*newgradient<=0) s++;
	gamma = (GAMMACONST-(GAMMACONST-0.1*GAMMACONST)/(n+1))/s;  
	aux[exfr][n]+=gamma*newgradient;
	if (grfnparam->recpay == SRT_RECEIVER)
	{
		if (aux[exfr][n]>aux[exfr][n+1]) aux[exfr][n] = aux[exfr][n+1];
	}
	else
	{
		if (aux[exfr][n]<aux[exfr][n+1]) aux[exfr][n] = aux[exfr][n+1];

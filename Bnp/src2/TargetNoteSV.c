...
/* Reconstruct the discount factors */
	/* coupon pay */
		event->fixed_pay = exp( event->fixed_pay_logdff - event_sv->fixed_pay_gam1 * ft
																	- event_sv->fixed_pay_gam1_2 * psi );
	/* floater */
		event->fltr_start = exp( event->fltr_start_logdff - event_sv->fltr_start_gam1 * ft
...
/* Reconstruct the start float discount factors */	
	...
	{
		event->fund_start[l] = exp( event->fund_start_logdff[l] - event_sv->fund_start_gam1[l] * ft
																	- event_sv->fund_start_gam1_2[l] * psi );
		event->fund_end[l] = exp( event->fund_end_logdff[l] - event_sv->fund_end_gam1[l] * ft
...
/* Reconstruct the discount factors */
	/* coupon pay */
	event->fixed_pay = exp( event->fixed_pay_logdff - event_sv->fixed_pay_gam1 * ft1
																	- event_sv->fixed_pay_gam2 * ft2
																	- event_sv->fixed_pay_gam1_2 * psi1
																	- event_sv->fixed_pay_gam2_2 * psi2
																	- event_sv->fixed_pay_gam12 * psi12 );
	/* floater */
	event->fltr_start = exp( event->fltr_start_logdff - event_sv->fltr_start_gam1 * ft1
																	- event_sv->fltr_start_gam2 * ft2
																	- event_sv->fltr_start_gam1_2 * psi1
																	- event_sv->fltr_start_gam2_2 * psi2
																	- event_sv->fltr_start_gam12 * psi12 );
	event->fltr_end = exp( event->fltr_end_logdff - event_sv->fltr_end_gam1 * ft1
																	- event_sv->fltr_end_gam2 * ft2
																	- event_sv->fltr_end_gam1_2 * psi1
																	- event_sv->fltr_end_gam2_2 * psi2
																	- event_sv->fltr_end_gam12 * psi12 );
...
/* Reconstruct the start float discount factors */	
	for (l=0; l<event->num_fund_df; l++)
	{
		event->fund_start[l] = exp( event->fund_start_logdff[l] - event_sv->fund_start_gam1[l] * ft1
		...
		event->fund_end[l] = exp( event->fund_end_logdff[l] - event_sv->fund_end_gam1[l] * ft1
																		- event_sv->fund_end_gam2[l] * ft2
																		- event_sv->fund_end_gam1_2[l] * psi1
...
char* set_TARN_EVENTS_SV( TARN_Struct* tarn, TARN_AUX* aux, TARN_MC_AUX* mc_aux )
{
...
/* loop over the total number of time steps */
	...
	{
	/* calculate the date */
		mc_aux->date[i] = ((long) (tarn->market.lToday + mc_aux->time[i] * 365.0 + 1.0E-08)) * 1.0;


	/* Check that we are not at the last date and calculate the model parameters */
		if (i < mc_aux->nStps-1)
		{
			index = Get_Index(mc_aux->time[i+1], mc_aux_sv->model.dPWTime, mc_aux_sv->model.iNbPWTime);
...
//			event->do_dom = 1;
		...
		/* DF(t, T*) reconstruction */
			mc_aux_sv->logdff_star[j] = log(swp_f_df (mc_aux->evt_dts[j], mc_aux_sv->model.lTStarDate, tarn->market.szYieldCurve));
			mc_aux_sv->gam1_star[j] = (1.0 - exp(mc_aux_sv->model.dLambdaX * (mc_aux_sv->model.dTStar - mc_aux->evt_tms[j]))) 
										/ mc_aux_sv->model.dLambdaX;
			mc_aux_sv->gam1_2_star[j] = 0.5 * mc_aux_sv->gam1_star[j] * mc_aux_sv->gam1_star[j];
			if ( mc_aux->tnModel == TN_2F_SV )
			{
				mc_aux_sv->logdff_star[j] = log(swp_f_df (mc_aux->date[i], mc_aux_sv->model.lTStarDate, tarn->market.szYieldCurve));
				mc_aux_sv->gam1_star[j] = (1.0 - exp(mc_aux_sv->model.dLambdaX * (mc_aux_sv->model.dTStar - mc_aux->evt_tms[j]))) 
										/ mc_aux_sv->model.dLambdaX;			
				mc_aux_sv->gam2_star[j] = (1.0 - exp(mc_aux_sv->model.dLambdaX2 * (mc_aux_sv->model.dTStar - mc_aux->evt_tms[j]))) 
										/ mc_aux_sv->model.dLambdaX2;
				mc_aux_sv->gam12_star[j] = mc_aux_sv->gam1_star[j] * mc_aux_sv->gam2_star[j];
				mc_aux_sv->gam1_2_star[j] = 0.5 * mc_aux_sv->gam1_star[j] * mc_aux_sv->gam1_star[j];
				mc_aux_sv->gam2_2_star[j] = 0.5 * mc_aux_sv->gam2_star[j] * mc_aux_sv->gam2_star[j];
			}

/* Fill the legs */
			if ( err = fillTargetNoteFundingSV( j, tarn, aux, mc_aux, event ) )
				goto FREE_RETURN;
			if ( err = fillTargetNoteCoupon( j+aux->i1stCpn, tarn, aux, mc_aux, event ) )
				goto FREE_RETURN;
...
/* Parameters for DF(t,T*) reconstruction */
					mc_aux_sv->logdff_star,
					mc_aux_sv->gam1_star,
					mc_aux_sv->gam2_star,
					mc_aux_sv->gam1_2_star,
					mc_aux_sv->gam2_2_star,
...
/* calculate the initial value of the numeraire and hence discount */
	df = swp_f_df (tarn->market.lToday, mc_aux_sv->model.lTStarDate, tarn->market.szYieldCurve);
	for (i=0; i<num_col; i++)
	{
		(prod_val)[i][0] *= df;
		(prod_val)[i][1] *= df;

    ...
    int*    stop_path)
{
    ...
    */
    ...
    /* coupon pay */
    event->fixed_pay = exp(
        event->fixed_pay_logdff - event_sv->fixed_pay_gam1 * ft - event_sv->fixed_pay_gam1_2 * psi);
    /* floater */
    event->fltr_start =
        exp(event->fltr_start_logdff - event_sv->fltr_start_gam1 * ft -
            event_sv->fltr_start_gam1_2 * psi);
    event->fltr_end = exp(
        event->fltr_end_logdff - event_sv->fltr_end_gam1 * ft - event_sv->fltr_end_gam1_2 * psi);
    ...
    int*    stop_path)
{
    ...
    */
    ...
    event->fltr_end =
        exp(event->fltr_end_logdff - event_sv->fltr_end_gam1 * ft1 - event_sv->fltr_end_gam2 * ft2 -
            event_sv->fltr_end_gam1_2 * psi1 - event_sv->fltr_end_gam2_2 * psi2 -
    ...
    for (l = 0; l < event->num_fund_df; l++)
    {
        event->fund_start[l] =
            exp(event->fund_start_logdff[l] - event_sv->fund_start_gam1[l] * ft1 -
                event_sv->fund_start_gam2[l] * ft2 - event_sv->fund_start_gam1_2[l] * psi1 -
                event_sv->fund_start_gam2_2[l] * psi2 - event_sv->fund_start_gam12[l] * psi12);
        event->fund_end[l] =
            exp(event->fund_end_logdff[l] - event_sv->fund_end_gam1[l] * ft1 -
                event_sv->fund_end_gam2[l] * ft2 - event_sv->fund_end_gam1_2[l] * psi1 -
                event_sv->fund_end_gam2_2[l] * psi2 - event_sv->fund_end_gam12[l] * psi12);
    }
...
char* set_TARN_EVENTS_SV(TARN_Struct* tarn, TARN_AUX* aux, TARN_MC_AUX* mc_aux)
{
    ...
    for (i = mc_aux->nStps - 1; i >= 0; i--)
    {
        ...
        {
            index = Get_Index(
                mc_aux->time[i + 1], mc_aux_sv->model.dPWTime, mc_aux_sv->model.iNbPWTime);
        ...
        if (j >= 0 && fabs(mc_aux->time[i] - mc_aux->evt_tms[j]) < 1.0E-08)
        {
            ...
            mc_aux_sv->logdff_star[j] = log(swp_f_df(
                mc_aux->evt_dts[j], mc_aux_sv->model.lTStarDate, tarn->market.szYieldCurve));
            mc_aux_sv->gam1_star[j]   = (1.0 - exp(mc_aux_sv->model.dLambdaX *
            ...
            {
                mc_aux_sv->logdff_star[j] = log(swp_f_df(
                    mc_aux->date[i], mc_aux_sv->model.lTStarDate, tarn->market.szYieldCurve));
...
//
char* TargetNoteMC_SV(char* szUnd, TARN_Struct* tarn, TARN_AUX* aux, double** prod_val)
{
    /* -----------------------------------------------------------------------------------------------------------------
     */
    /* Variable declaration */
    ...
    if (mc_aux->tnModel == TN_2F_SV)
    {
        ...
        if (nUseMemOpt)
        {
            if (err = lgmSV2F_mc_balsam_optim_mem /*_rev*/ (
                    ...
                    NULL,
                    NULL,
                    /*	Initialisation function to be called at the beggining of each path or NULL
                       if none */
                    resetTargetNote,
                    /*	Payoff function */
                    TargetNotePayoffSV2,
    ...
    /*	Add PV of Past
       ...
       tarn->deal.lvFundStartDates[i], tarn->market.szYieldCurve); double dFund_End_DF =
       swp_f_df(tarn->market.lToday, tarn->deal.lvFundEndDates[i], tarn->market.szYieldCurve);
                    double dFund_cvgMargin = 1.0 - coverage( tarn->deal.lvFundStartDates[i],

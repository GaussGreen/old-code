...
/******************************************************************************\
    ...
    DESCRIPTION     :   Each part of the comll in the new_event->icom field
                        ...
                        array of dates, (in years) which goes into the dft
                        field of event.

                                                Df are gathered only underlying by underlying (there
is a loop in grf_f_get_event that calls this function und by und). It is therefore important to make
sure the COMLL we are working on is for the underlying we consider.

                        The dates in the dfind & dfindfloat fields are replaced
                                                with indices into the big dft array.
...
Err grfn_set_df_dates_in_event(GrfnEvent* new_event, GrfnDeal* gd, int und_index)

{
    ...
    /* Check if there is gathering work to be done : df's might be equired for this und */
    if (tsz > 0)
    {
       ...
       which they belong, in one array of Data (loop on COMLL for the transfer) */
        k           = 0;
        temp        = comll;
        comll_index = 0;
        while (temp != NULL)
        {
            ...
            if (temp->ivec[0] == und_index)
            {
                ...
                {
                    /* Add into data a new point (#k) with this df date, and the comll_index
                            (there is a cheat: even index for fixed leg: 2*index) */
                    data[k].date        = temp->dfind[j];
                    data[k].comll_index = 2 * comll_index;
                    k++;
                }
                ...
                for (j = 0; j < temp->dfindfloatlen; j++)
                {
                    ...
                    data[k].comll_index = 2 * comll_index + 1;
                    k++;
                }
            }
        ...
        /* Set df index in data s.t. no dates are counted twice (ndf is number of unique dates) */
        data[0].df_index = 0;
        ndf              = 1;
        for (i = 1; i < tsz; i++)
        {
        ...
        /* Change dates in the original COMLL (dfind & dfindfloat) to indices in big df array */
        for (temp = comll, comll_index = 0; temp != NULL; temp = temp->next, comll_index++)
        {
            k       = 0;
            k_float = 0;
            ...
            for (j = 0; j < tsz; j++)
            {
                ...
                if ((data[j].comll_index == 2 * comll_index) && (temp->dfindlen > 0))
                    temp->dfind[k++] = data[j].df_index;
                else
                    /* Reconciliate floating leg with odd comll_index in data */
                    if ((data[j].comll_index == 2 * comll_index + 1) && (temp->dfindfloatlen > 0))
    ...
    /*  Removed a chunk of code to srt_f_grfn.c */

...
static int compare(const void *ptr_a, const void *ptr_b)
{
 ...
 Data *b = (Data *)(ptr_b);
    ...
    if (a->date > b->date) return  1;
    if (a->comll_index < b->comll_index) return -1;
    if (a->comll_index > b->comll_index) return  1;
...
/******************************************************************************\
    ...
    DESCRIPTION     :   Each part of the comll in the new_event->icom field 
						...
						Df are gathered only underlying by underlying (there is
						a loop in grf_f_get_event that calls this function und by
						und).
						It is therefore important to make sure the COMLL we are
						working on is for the underlying we consider.

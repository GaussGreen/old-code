                                   ...
                                   double* dCross)
{
    ...
    *dCoef2_1 = -dSigma * dAlpha * (dSigma * dSigma0 / 4.0 + dRho * dAlpha / dSigma0 / 32.0) *
                dTime * dSqTime;
                            ...
                            double* dValue)
{
    ...
    dInitGauss = (double*)calloc(iNumPaths, sizeof(double));
    ...
    double* dLambdaH)
{
    ...
    srt_f_optsarbvol(
        dFwd, dFwd, dMaturity, dSigma, dAlpha, dBeta, dRho, eTypeInput, SRT_BETAVOL, &dSigma);
...
double copula_sv_get_SABR_get_cumul(
    double dMaturity,
    double dForward,
    double dStrike,
    double dSigmaBeta,
    ...
    double* dCumulative)
{
    ...
    if (iAdaptGrid && !iUsePoints)
    {
        ...
        dFirstGuessPoints   = (double*)calloc(lFirstGuessNbPoints, sizeof(double));
        dFirstGuessCumul    = (double*)calloc(lFirstGuessNbPoints, sizeof(double));
    ...
    for (i = 1; i < lFirstGuessNbPoints; i++)
    {
        ...
        dFirstGuessCumul[i] = copula_sv_get_SABR_get_cumul(
            dMaturity, dForward, dFirstGuessPoints[i], dSigmaBeta, dAlpha, dBeta, dRho, dShift);
    }
    ...
    if (i >= 0)
    {
        ...
        dNextCumul = copula_sv_get_SABR_get_cumul(
            dMaturity, dForward, dFirstGuessPoints[i + 1], dSigmaBeta, 0.0, dBeta, 0.0, dShift);
        for (i = i; i >= 0; i--)
        {
            ...
            dCurrentCumul = copula_sv_get_SABR_get_cumul(
                dMaturity, dForward, dFirstGuessPoints[i], dSigmaBeta, 0.0, dBeta, 0.0, dShift);
    ...
    if (iAdaptGrid && !iUsePoints)
    {
        ...
        dProbaStep =
            (dFirstGuessCumul[lFirstGuessNbPoints - 1] - dFirstGuessCumul[0]) / (lNumPoints - 1.0);

        for (i = 0; i < lNumPoints; i++)
        {
            dPoints[i] =
                interp(dFirstGuessCumul, dFirstGuessPoints, lFirstGuessNbPoints, dProba, 0, &dTemp);
    ...
    double*          dPremium)
{
    ...
    double *dShiftSV = NULL, *dSigmaSV = NULL, *dAlphaSV = NULL, *dRhoSV = NULL, *dVar0SV = NULL,
           *dLambdaSV = NULL;
    ...
    dX          = (double**)calloc(iNbFwd, sizeof(double*));
    dCumulative = (double**)calloc(iNbFwd, sizeof(double*));
    lNbPoints   = (long*)calloc(iNbFwd, sizeof(long*));

    dShiftSV  = (double*)calloc(iNbFwd, sizeof(double));
    dSigmaSV  = (double*)calloc(iNbFwd, sizeof(double));
    dAlphaSV  = (double*)calloc(iNbFwd, sizeof(double));
    dRhoSV    = (double*)calloc(iNbFwd, sizeof(double));
    dVar0SV   = (double*)calloc(iNbFwd, sizeof(double));
    dLambdaSV = (double*)calloc(iNbFwd, sizeof(double));

    if (!dResult || !dX || !dCumulative || !dShiftSV || !dSigmaSV || !dAlphaSV || !dRhoSV ||
        !dVar0SV || !dLambdaSV)
    {
    ...
    for (i = 0; i < iNbFwd; i++)
    {
        dX[i]          = (double*)calloc(sParams->lNbPoints, sizeof(double));
        dCumulative[i] = (double*)calloc(sParams->lNbPoints, sizeof(double));
...

{
    long     i, idum = -8935807;
    int      j, k, eff_d;
    double **GaussSample = NULL, **XtSample = NULL, *dBrownian = NULL, **dSortMatrix = NULL,
           **dSqCorrMatrix = NULL;
    ...
    dBrownian     = (double*)calloc(eff_d, sizeof(double));
    dSortMatrix   = dmatrix(0, 1, 0, sParams->lNumPaths);

    dConst   = (double*)calloc(iNbFwd, sizeof(double));
    dCoef1_1 = (double*)calloc(iNbFwd, sizeof(double));
    dCoef2_1 = (double*)calloc(iNbFwd, sizeof(double));
    dCross   = (double*)calloc(iNbFwd, sizeof(double));

    if (!GaussSample || !XtSample || !dSqCorrMatrix || !dBrownian || !dSortMatrix || !dConst ||
        !dCoef1_1 || !dCoef2_1 || !dCross)
    {
    ...
    double*          dPremium)
{
    ...
    /* Memory allocation */
    pdFwdsAtMaturity = (double*)calloc(nFwds, sizeof(double));
    dAvg             = (double*)calloc(nFwds, sizeof(double));

    dShiftSV  = (double*)calloc(nFwds, sizeof(double));
    dSigmaSV  = (double*)calloc(nFwds, sizeof(double));
    dAlphaSV  = (double*)calloc(nFwds, sizeof(double));
    dRhoSV    = (double*)calloc(nFwds, sizeof(double));
    dVar0SV   = (double*)calloc(nFwds, sizeof(double));
    dLambdaSV = (double*)calloc(nFwds, sizeof(double));
    ...
    if (!pdFwdsAtMaturity || !dAvg || !dShiftSV || !dSigmaSV || !dAlphaSV || !dRhoSV || !dVar0SV ||
        !dLambdaSV || !ppdPoints || !ppdPoints || !ppdCumProbaValue || !dCorrCube)
    {
    ...
    for (i = 0; i < nFwds; i++)
    {
        ...
        if (sParams->iUseOldGridMethod)
        {
            ...
            n_StdDev = 5.0 * dSqMat;
            ppdPoints[i][0] =
                dFwds[i] * exp(-0.5 * dApproxStdev * dApproxStdev - n_StdDev * dApproxStdev);
    ...
    double*          dPremium)
{
    ...
    double **dCorrMatrix = NULL, **dSqCorrMatrix = NULL, *dXtSample = NULL, *dIndBrownian = NULL,
           *dCorBrownian = NULL, *dCoefVol = NULL, *dStartVol = NULL, *dCoefVoVol = NULL;
    ...
    dSqCorrMatrix = dmatrix(0, eff_d - 1, 0, eff_d - 1);
    dXtSample     = (double*)calloc(eff_d, sizeof(double));
    dIndBrownian  = (double*)calloc(eff_d, sizeof(double));
    dCorBrownian  = (double*)calloc(eff_d, sizeof(double));
    dCoefVol      = (double*)calloc(nFwds, sizeof(double));
    dStartVol     = (double*)calloc(nFwds, sizeof(double));
    dCoefVoVol    = (double*)calloc(nFwds, sizeof(double));

    if (!dSqCorrMatrix || !dXtSample || !dIndBrownian || !dCorBrownian || !dCoefVol || !dStartVol ||
        !dCoefVoVol)
    {
    ...
    for (i = 0; i < sParams->lNumPaths; i++)
    {
        ...
        for (j = 0; j < lNumSteps; j++)
        {
            ...
            {
                dXtSample[k] =
                    max(dXtSample[k] + dXtSample[nFwds + k] * exp(dBeta[k] * log(dXtSample[k])) *
                                           sqdt * dCorBrownian[k],
                        0.0001);
                dXtSample[nFwds + k] = max(
                    dXtSample[nFwds + k] * (1.0 + dCoefVoVol[k] * dCorBrownian[nFwds + k]), 0.0001);
            }
        }
    ...
    double*          dResult)
{
    ...
    if (!dShiftSV || !dSigmaSV || !dAlphaSV || !dRhoSV || !dVar0SV || !dLambdaSV)
    {
    ...
    double*         dResult)
{
    ...
    dSqCorrMatrix = dmatrix(0, eff_d - 1, 0, eff_d - 1);
    dBrownian     = (double*)calloc(total_d, sizeof(double));
    ...
    double*  dRho)
{
    ...
    dCovar = (dCoef1[iIndex1] * dCholesky[3][0] + dCoef2[iIndex1] * dCholesky[1][0]) *
             ...
             3.0 * dCross[iIndex1] * dCholesky[3][0] * dCholesky[1][0] * dCross[iIndex2] *
                 dCholesky[2][0] * dCholesky[0][0];

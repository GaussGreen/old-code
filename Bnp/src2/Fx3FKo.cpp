    ...
    double* ans)
{
    ...
    double spotFx, paidFwdFx, ivol, df, *fwdFx = NULL, *logBar = NULL, **covar = NULL;
    ...
    double* ans)
{
    ...
    err = Fx3FKoFwd(undName, nKo, newKoDates, newKoLvl, newAb, fixDate, payDate, npth, &koFwd);
    if (err)
    {
        goto FREE_RETURN;
    }

    err = Fx3FKoDig(undName, nKo, newKoDates, newKoLvl, newAb, payDate, npth, &koDig);
    if (err)
    {
        goto FREE_RETURN;
    }

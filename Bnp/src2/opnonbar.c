...
static void absorbed_moment(
    double spot, double barrier, double dt, double vol, double rate, double absorb_moments[2])
...
/* + + + + + + + + + + + + + + + + + + + + + + + + + + + */
static void moments_stock_stock(
    double   fwd_S,
    double   nearest_S,
    double   fwd_S2,
    double   nearest_S2,
    ...
    MOMENTS* moments)
{
    ...
    moments->E_SS =
        fwd_S * fwd_S * exp(vol_s * vol_s * dt) - 2 * fwd_S * nearest_S + nearest_S * nearest_S;

    moments->E_R = fwd_S2 - nearest_S2;

    moments->E_RR = fwd_S2 * fwd_S2 * exp(vol_s2 * vol_s2 * dt) - 2 * fwd_S2 * nearest_S2 +
                    nearest_S2 * nearest_S2;

    moments->E_RS = fwd_S * fwd_S2 * exp(vol_s * vol_s2 * rho * dt) - fwd_S * nearest_S2 -
                    fwd_S2 * nearest_S + nearest_S * nearest_S2;
}

/* + + + + + + + + + + + + + + + + + + + + + + + + + + + */

static Err init_nono_tree(
    double**  R,
    double**  S,
    double**  new_R,
    double**  new_S,
    ...
    long      n_steps)
{
    ...
    *new_val     = dmatrix(-n_steps - 1, n_steps + 1, -n_steps - 1, n_steps + 1);
    ...
    if ((*mid_R == NULL) || (*gamma == NULL) || (*gamma_prime == NULL) || (*fwd_S == NULL) ||
        (*df_cve == NULL) || (*stoch_rate == NULL) || (*fwd_rate == NULL) || (*R == NULL) ||
        (*S == NULL) || (*new_R == NULL) || (*new_S == NULL) || (*val == NULL) ||
        (*new_val == NULL))
    {
...

{
    ...
    for (time_step = n_steps - 1; time_step >= 0; time_step--)
    {
        ...
        for (idx_R = -(time_step + 1); idx_R <= (time_step + 1); idx_R++)
        {
            ...
            {
                absorbed_moment(new_R[idx_R], barrier[2], dt, vol2, rate2, absorb_moments);
            ...
            {
                absorbed_moment(new_R[idx_R], barrier[3], dt, vol2, rate2, absorb_moments);
            ...
            if (smooth_R)
            {
                for (idx_S = -(time_step + 1); idx_S <= (time_step + 1); idx_S++)
                {
                    ...
                    {
                        absorbed_moment(new_S[idx_S], barrier[0], dt, vol1, rate1, absorb_moments);

                        fwd_s = new_S[idx_S] * absorb_moments[0];
                        vol_S = sqrt(
                            log(absorb_moments[1] / absorb_moments[0] / absorb_moments[0]) / dt);
                    ...
                    {
                        absorbed_moment(new_S[idx_S], barrier[1], dt, vol1, rate1, absorb_moments);
                    ...
                    {
                        moments_stock_stock(
                            fwd_s, S[idx_S], fwd_r, R[idx_R], vol_S, vol_R, rho, dt, &moments);

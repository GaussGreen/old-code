...
/*---------------------------------------------------------------------------
  ...
  DESCRIPTION     :values a structure according to a 2d tree discretization of
                                   the cheyette interest rate model  , by which
we mean the one where sigma(t  ,r) = sigma(t)*r
-----------------------------------------------------------------------------*/
    ...
    /* if yes */
      ...
      /* vega shifts */
        ...
        /* each request of the SrtIOStruct has now a field that points to a
           SrtStp structure. For this structure  , the parameters have been
           computed with the shift corresponding to the request */
    ...
    for (io_request = iolist->head; io_request != NULL;
      ...
      for (i = trinf->min_r_index; i <= trinf->max_r_index; i++) {
        ...
        for (j = trinf->min_phi_index; j <= trinf->max_phi_index; j++) {
          ...
          structure. The tree being frozen  , the tree info keeps being taken
          from stp.	*/
          ...
          if (stp->next) {
            ...
            Y_T_at_t_compute(1, &node.cur_sam, &tminf->yp, &zc_yield, 0,
                             ONE_FAC, CHEY);
          ...
          err = evalcf((GrfnEvent *)stp->e, &node.cur_sam, gd,

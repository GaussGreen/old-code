...
SRT_Boolean srt_f_treche_requestOK (int request_type)
{
    switch (request_type)
    {
		...
		case IO_SIGMA_SHIFT :   return SRT_NO; break;		/* set to yes when use it */
		case IO_TAU_SHIFT	:   return SRT_NO; break;		/* set to yes when use it */
	...
	for (io_request=iolist->head; io_request!=NULL; io_request=io_request->next)
	{
  		...
  		else
		{
			...
			{	
				...
				if (err =  srt_f_vegashiftirministp (stp, und, und_info,
													 sigma_bucket_start, sigma_bucket_end,
													 sigma_shift, sigma_shift_type,
													 tau_bucket_start, tau_bucket_end,
...
/* each request of the SrtIOStruct has now a field that points to a SrtStp structure.
	...
	/** Memory Allocation **/
	sz = xtra_calc_num 
		* (maxtrinf.max_r_index - maxtrinf.min_r_index + 1) *
	      (maxtrinf.max_phi_index - maxtrinf.min_phi_index + 1) *
	      grfnparam->node_dim;

	prev_assets = f4tensor (0, xtra_calc_num - 1, 
							maxtrinf.min_r_index, maxtrinf.max_r_index,
						    maxtrinf.min_phi_index, maxtrinf.max_phi_index,
							0, grfnparam->node_dim - 1);			   
	cur_assets = f4tensor (0, xtra_calc_num - 1, 
							maxtrinf.min_r_index, maxtrinf.max_r_index,
						    maxtrinf.min_phi_index, maxtrinf.max_phi_index,
							0, grfnparam->node_dim - 1);
	...
	for (stp=bot; stp!=NULL; stp=stp->prev) 
	{
		...
		for (io_request=iolist->head; io_request!=NULL; io_request=io_request->next)
		{
			...
			if  (srt_f_treche_requestOK (io_request_val_p->type) == SRT_NO) continue; 
...
/* Stores all the columns PV in the I/O list */
	...
	/** free the world **/
	free_f4tensor (prev_assets, 0, xtra_calc_num - 1,
				   maxtrinf.min_r_index, maxtrinf.max_r_index,
				   maxtrinf.min_phi_index, maxtrinf.max_phi_index,
				   0, grfnparam->node_dim - 1);			   
	free_f4tensor (cur_assets, 0, xtra_calc_num - 1,
				   maxtrinf.min_r_index, maxtrinf.max_r_index,
				   maxtrinf.min_phi_index, maxtrinf.max_phi_index,
				   0, grfnparam->node_dim - 1);			   

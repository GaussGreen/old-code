...
//1 SV	// 	factor, initialise a SV underlying then price a Grfn tableau using either PDE, MC or MC
//with		// 	optimal ex boundary.
////
//																										//
//						Author:  Paul McCallum				Date:  13th Oct
//2003						//
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~//
...
//values by	// 	repointing the pointers. These must be freed here since any possible client
//callnig routine may		// 	not pass preassigned pointers and therefore expect all memory
//management to happen here.			//
////
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~//
    ...
    double** pSecCalibratedModelPrices)
{
    ...
    if (err)
    {
        // we would like to think that this memory is freed in routine above, however just in
        // case...
        if (*pSigmaTimes)
            free(*pSigmaTimes);
        *pSigmaTimes = NULL;
    ...
    //	Now set up arrays we need for initialising the underlying. Note that we could have done what
    //we did		   // 	in the previous non-SV routine. ie had 2 x 2 row arrays of double
    //pointers and point them to the		   // 	Lambda and Sigma data, albeit this time
    //callocing the smile row size because of runtime 1 or 2 factor	   // 	decision. However it
    //is safer practice and is not much more time consuming, for small arrays at least,	   // 	to
    //copy values before passing them to a C routine (which of course could modify the values
    //unbeknownigly)  //
    //
    ...
    //  Before setting up underlying, take this opportunity to repoint the external smile value
    //  arrays and free     //
    //	the local ones. Believe that where we have originally passed a single set of values and not
    //recalibrated,	// 	or where we	calibrate but do not have enough information, there may well
    //be duplicate sets of smile values. //		      // 	Note that we are now pointing
    //pSmileTimes to *pSigmaTimes so there we must be careful to NULL after we		// 	free one
    //of them so as not to try to free for a second time! Eg the error handling we have below is a
    ...
    *pRho2SmileVals = calib_rho2_smile_vals;
    ...
    // want to free these arrays now that we are done with them (the values are copied into the
    // underlying)
    ...
    //	Now for the sake of the output at West level and in order to determine how good the
    //calibration is,	   // 	we now record the model prices. The market prices are already
    //contained in CalibrationInstrumentData   // 	returned by the calibration. Note the fact that we
    //hard code the option type in the calls below. This  // 	is because it is nigh impossible to
    //extract the types from the original code, which are hard coded to  // 	be "REC". If this should
    //ever change, the code below will become out of sync. PMc  1stDec03			   //
    ////
    //

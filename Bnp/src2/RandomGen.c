...
static Err RandomGen_Uniform(SRandomGen *rg, double *res)
{
	...
	if (rg->idum < 0) rg->idum += IM1;
	k = rg->idum2 / IQ2;
	rg->idum2 = IA2 * (rg->idum2 - k * IQ2) - k * IR2;		// Compute idum2=(IA2*idum) % IM2 likewise
	if (rg->idum2 < 0) rg->idum2 += IM2;
	j = rg->iy / NDIV;										// Will be in the range 0..SHUFFLE_NTAB-1
	rg->iy = rg->iv[j] - rg->idum2;							// Here idum is shuffled, idum and idum2 are
	rg->iv[j] = rg->idum;									// combined to generate output
	if (rg->iy < 1) rg->iy += IMM1;
	if ((*res = AM * rg->iy) > RNMX) *res = RNMX;			// Because users don't expect endpoint values
	return NULL;
}
...
Err RandomGen_Init(SRandomGen *rg, long seed)
{
	int j;
	long k;
	rg->idum = ( seed > 1 ? seed : (seed < -1 ? -seed : 1) );	// Be sure to prevent idum = 0
	rg->idum2 = rg->idum;
	for (j = SHUFFLE_NTAB + 7; j >= 0; j--) {	// Load the shuffle table (after 8 warm-ups)
		k = rg->idum / IQ1;
		rg->idum = IA1 * (rg->idum - k * IQ1) - k * IR1;
		if (rg->idum < 0) rg->idum += IM1;
		if (j < SHUFFLE_NTAB) rg->iv[j] = rg->idum;
	}
	rg->iy = rg->iv[0];
...
#define SWAP(a,b) ltemp=(a); (a)=(b); (b)=ltemp
...
static Err FillIndices(SRandomGen *rg)
{
	...
	if (!uni) { err = serror("Memory failure in FillIndices");	goto FREE_RETURN; }

	if (!desc->bStrat || desc->iStep == 0)
	{
		for (i=0; i < desc->TableSize; i++) uni[i] = i;		// Init uni
		...
		for (br=0; br < desc->nBrownians; br++)
		{
			...
			if (desc->bStrat) for (i=0; i < desc->TableSize; i++) desc->Spots[br][i] = desc->NormalTable[uni[i]];
		}
	}
	else
	{
		idx = (long *) calloc(desc->TableSize, sizeof(long));
		if (!idx) { err = serror("Memory failure in FillIndices");	goto FREE_RETURN; }
...
Err ABS_Init(SRandomGen *rg, long seed, long nPaths, int nBrownians, int bStrat)
{
	SABSDesc *desc = NULL;
	long	i;
	...
	if (!desc) return serror("Memory failure in ABS_Init");
	rg->spec_desc = desc;
	...
	if (bStrat)
	{
		if (nPaths < 10) return serror("Too few paths for ABS");

		if (desc->TableSize % 2) desc->TableSize++;

		desc->NBuck = 2 * (long)(sqrt(desc->TableSize) / 2 + 1e-8);
		while ( desc->TableSize % desc->NBuck ) desc->NBuck += 2;
		desc->NNode = desc->TableSize / desc->NBuck;

		desc->Spots = dmatrix(0, nBrownians-1, 0, desc->TableSize-1);
		if (!desc->Spots) return serror("Memory failure in ABS_Init");
	}

	desc->NormalTable = (double *) calloc(desc->TableSize, sizeof(double));
	if (!desc->NormalTable) return serror("Memory failure in ABS_Init");
...
Err ABS_Free(SRandomGen *rg)
{
	...
	if (desc)
	{
		...
		if (desc->Spots) free_dmatrix(desc->Spots, 0, desc->nBrownians-1, 0, desc->TableSize-1);
		if (desc->Index) for (i=0; i < desc->nBrownians; i++) free(desc->Index[i]);
		free(desc->Index);
	}
	free(desc);

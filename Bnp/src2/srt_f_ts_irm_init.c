...
Err srt_f_init_IRM_OneFac_TermStruct(
  ...
  /* Checks on the number of columns in Sigma : one value  , two columns or
   * three columns (with Beta) */
  ...
  /* If the third column (the Beta) is missing  , fill it in with the input beta
   * or according to model type */
  if (sig_cols < 3) {
    for (i = 0; i < num_sig; i++) {

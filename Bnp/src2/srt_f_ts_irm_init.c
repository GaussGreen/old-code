...
Err flag;
	...
	if (mdl_dim == TWO_FAC)
	{
		flag = srt_f_init_IRM_TwoFac_TermStruct(
					ts,
					today,
					sig, 
					sig_cols, 
...
/* Checks on the number of columns in Sigma : one value, two columns or three columns (with Beta) */
	if ( (sig_cols != 2) && (sig_cols != 3 ) && (sig_cols != 6))
	{
		if ((sig_cols != 1 )|| ( num_sig >1))
			return serror("Need TWO/THREE/SIX columns or ONE SINGLE VALUE for the Sigma Curve");
	}
...
/* If only one single value for sigma, create a fake Term Struct with constants */
...
/* If the third column (the Beta) is missing, fill it in with the input beta or according to model type */
	if (sig_cols <3)
	{
		for ( i =0; i < num_sig; i ++)
		{
...
/* Overwrites the Values of Beta for explicit models */
	if ((mdl_type == LGM) || (mdl_type == NEWLGM) || (mdl_type == LGM_STOCH_VOL ))
	{
		for ( i =0; i < num_sig; i ++)
			ppdSigmaValues[2][i] = 0.0;
	}
	else
	if ((mdl_type == CHEY) || (mdl_type == CHEY_STOCH_VOL))
	{	
		for ( i =0; i < num_sig; i ++)
			ppdSigmaValues[2][i] = 1.0;
	}

/* If only one single value for tau, create a fake Term Struct with constants */
...
/* If the data exists but is NULL: consider what follows as garbage */
	...
	{
		if( !vasicek_mean_rev_level_data[0][i] || !vasicek_mean_rev_level_data[1][i] )
			break;
	}
...
/* Current index for tau and sigma start at a date >= today */
	while ( (cur_sig < num_sig) &&
		 ((ppdSigmaValues[0][cur_sig]-today) <= EPSILON) ) cur_sig++;
	while ( (cur_tau < num_tau) &&
		 ((ppdTauValues[0][cur_tau]-today) <= EPSILON) ) cur_tau++;
	while ( (cur_mean_rev_level < num_mean_rev_level) &&
		 ((vasicek_mean_rev_level_data[0][cur_mean_rev_level]-today) <= EPSILON) ) cur_mean_rev_level++;
...
/* Loops to fill in the TS as long as there is something to put in */
	...
	/* 2nd case: date_sigma < date_tau */
		if ( (over_tau == 1) || ( ( (over_sig==0) && (over_tau==0) ) &&
			( ppdSigmaValues[0][cur_sig] < ppdTauValues[0][cur_tau] ) ) )
		{
	...
	/* 3rd case: date_tau < date_sigma */
		if ( (over_sig == 1) || ( ( (over_sig==0) && (over_tau==0) ) &&
			( ppdSigmaValues[0][cur_sig] > ppdTauValues[0][cur_tau] ) )  )
		{
		/* Transfer of specific information */
	...
	if(mdl_type == VASICEK)
	{
		/* loop on the mean reversion level dates to add them to the interest underlying one */
		...
		while(cur_mean_rev_level < num_mean_rev_level)     
		{
			...
			while((ls!=NULL) && (tsval->date < vasicek_mean_rev_level_data[0][cur_mean_rev_level]) )
			{
				ls = ls->next;
				if(ls) tsval = (IrmTermStructVal*)ls->element->val.pval;

			}
			if(ls == NULL) ls = (*ts)->tail;

			tsval = (IrmTermStructVal*)ls->element->val.pval;
			/* if not on the same date, we add the date onto the i.r term structure */
			if( tsval->date != vasicek_mean_rev_level_data[0][cur_mean_rev_level])
			{
				...
				tsval->time = (vasicek_mean_rev_level_data[0][cur_mean_rev_level]-today)*YEARS_IN_DAY;

				tsval->sig = find_sig(tsval->time,*ts);
				tsval->tau = find_tau(tsval->time,*ts);
				
				tsval->vasicek_init_cond = vasicek_init_cond;
				tsval->mean_rev_level =  vasicek_mean_rev_level_data[1][cur_mean_rev_level];
			...
			else /* If it tsval->date = vasicek_mean_rev_level_data[0][cur_mean_rev_level] */
			{
				/* adds the specific information */
				tsval->vasicek_init_cond = vasicek_init_cond;
				tsval->mean_rev_level =  vasicek_mean_rev_level_data[1][cur_mean_rev_level];				
		...
		while(ls!=NULL)
		{
			...
			j = 0;
			while((j < num_mean_rev_level) && (tsval->date > vasicek_mean_rev_level_data[0][j]) ) j++;
			if(j == num_mean_rev_level)	 tsval->mean_rev_level = vasicek_mean_rev_level_data[1][num_mean_rev_level-1];
			else tsval->mean_rev_level = vasicek_mean_rev_level_data[1][j];
...
/* -----------------------------------------------------------------------
   A function to output an existing TermStructure (Memory Allocation done inside)
   ----------------------------------------------------------------------- */
	...
	{	
		...
		(((IrmTermStructVal*)l->element->val.pval)->val_origin == BOTH_DATE))
		{
			...
			*tau = realloc (*tau, (*plNumTaus) * sizeof (double));
			(*tau_date)[*plNumTaus-1] =  (double) (((IrmTermStructVal*)l->element->val.pval) -> date);
			(*tau)[*plNumTaus-1] = ((IrmTermStructVal*)l->element->val.pval) -> tau;
		}

		if ((((IrmTermStructVal*)l->element->val.pval)->val_origin == SIGMA_DATE) ||
		(((IrmTermStructVal*)l->element->val.pval)->val_origin == BOTH_DATE))
		{
			...
			*meanvol = realloc (*meanvol, (*plNumSigmas) * sizeof (double));
			(*sigma_date)[*plNumSigmas-1] = (double) (((IrmTermStructVal*)l->element->val.pval) -> date);
			(*sigma)[*plNumSigmas-1] = ((IrmTermStructVal*)l->element->val.pval) -> sig;
			(*beta)[*plNumSigmas-1] = ((IrmTermStructVal*)l->element->val.pval) -> beta;
			(*vovol)[*plNumSigmas-1] = ((IrmTermStructVal*)l->element->val.pval) -> vovol;
			(*rho)[*plNumSigmas-1] = ((IrmTermStructVal*)l->element->val.pval) -> rho;
			(*meanvol)[*plNumSigmas-1] = ((IrmTermStructVal*)l->element->val.pval) -> meanvol;
		}
...
IrmTermStructVal* tsval;
	...
	while ( ( (done_sig!=1) || (done_tau!=1) ) && (ls!=NULL) )
	{
		...
		switch (tsval->val_origin)
	 	{
		case SIGMA_DATE : cur_sig = tsval->sig; done_sig = 1;
			break;
		case TAU_DATE	: cur_tau = tsval->tau; done_tau = 1;
			break;
		case BOTH_DATE	: cur_tau = tsval->tau; done_tau = 1;
				  cur_sig = tsval->sig; done_sig = 1;
			break;
		default		: break;  
		}
		ls = ls->previous;
...
long  ticker;
	...
	if (date<=today) return "Tau date < today";
...
long   ticker;
	...
	if (date<=today) return "Sigma date < today";

	if ( srt_f_lstgetobj( *ts, "", date, &objPtr ) == NULL )
	{
		tsval = (IrmTermStructVal*)objPtr->val.pval;
		tsval->sig = sig;
		if (tsval->val_origin == TAU_DATE) tsval->val_origin = BOTH_DATE;
		else tsval->val_origin = SIGMA_DATE;
...
Err	make_vasicek_mean_sr_vector(TermStruct	*ts)
{
	...
	while( lc->next != NULL)
	{
		...
		c->vasicek_mean_sr = p->vasicek_mean_sr	+ p->mean_rev_level*( exp(time / p->tau) - 1)/p->F;
...
Err	make_vasicek_mean_int_sr_vector(TermStruct	*ts)
{
	...
	while( lc->next != NULL)
	{
		...
		c->vasicek_mean_int_sr = p->vasicek_mean_int_sr + (p->vasicek_mean_sr*p->F-p->mean_rev_level)*exp_term
...
Description: in order to make interpolation easier, the value of G
		(resp. H) running up to date Ti-1 is stored on the ith 
		element of the ts for the new LGM model they are in fact equal to sig*sig*t and tau !!!. 
*/
static Err make_new_G_H_vector(TermStruct* l)
{
...
Err err=0;
	...
	while ( lc != NULL ) /* At least 2 elements in the ts */
	{
		...
		if (c->val_origin  ==  SIGMA_DATE)
		{
			c->G = c->sig * c->sig * c->time;
			
			ln = lc;
			while ( (ln!=NULL) 
		    &&  (((IrmTermStructVal*)ln->element->val.pval)->val_origin != TAU_DATE) 
			&& (((IrmTermStructVal*)ln->element->val.pval)->val_origin != BOTH_DATE) ) 
			{ 
				ln = ln->next;  
			}
			...
			lp = lc;
			while ( (lp!=NULL) 
		    &&  (((IrmTermStructVal*)lp->element->val.pval)->val_origin != TAU_DATE)
			&& (((IrmTermStructVal*)lp->element->val.pval)->val_origin != BOTH_DATE)) 
			{ 
				lp = lp->previous;  
			}
			...
			if ((lp != NULL) && (ln != NULL))
			{
				...
				c->H = p->tau + (n->tau - p->tau) * (c->time - p->time) / (n->time - p->time);
		
			}
			if (ln == NULL) ln = l->tail;
			if (lp == NULL) lp = l->head;

			if ( (((IrmTermStructVal*)ln->element->val.pval)->val_origin == TAU_DATE)
				|| (((IrmTermStructVal*)ln->element->val.pval)->val_origin == BOTH_DATE))
			{
				n = (IrmTermStructVal*)ln->element->val.pval;
			...
			if ((((IrmTermStructVal*)lp->element->val.pval)->val_origin == TAU_DATE)
				|| (((IrmTermStructVal*)lp->element->val.pval)->val_origin == BOTH_DATE))
			{
		...
		{ 
			...
			lp = lc;
			while ( (lp!=NULL) 
		    &&  (((IrmTermStructVal*)lp->element->val.pval)->val_origin != SIGMA_DATE) 
			&& (((IrmTermStructVal*)lp->element->val.pval)->val_origin != BOTH_DATE)) 
			{ 
				lp = lp->previous;  
			}
			...
			if ((lp != NULL) && (ln != NULL))
			{
				...
				c->G = p->sig * p->sig * p->time + (n->sig * n->sig * n->time - p->sig * p->sig * p->time) * (c->time - p->time) / (n->time - p->time);
	...
	/* G has to be increasing */
		if (diff < 0.0)
		{
			err = "Vol term Struct for New Model has to be cumulative vol :: check the input";
			return err;
		}
	}
...
Err err=0;
	...
	while ( lc->next != NULL ) 
	{
		...
		c->L =	p->L + p->F*p->G*(1-exp(-time/p->tau))*p->tau
				+ 0.5*pow(p->sig*p->tau,2)*(exp(-time/p->tau) + exp(time/p->tau) - 2)/p->F;
...
Err err = NULL;
	...
	while ( lc->next != NULL ) 
	{
		...
		c->O = p->O + p->F*p->G*p->Psi*(1-exp(-time/p->tau))*p->tau
			   + p->F*p->F*p->G*p->tau*( (1-exp(-time/p->tau))*p->tau - 0.5*(1-exp(-2*time/p->tau))*p->tau )
			   + 0.5*(p->Psi/p->F)*p->tau*p->sig*p->sig*( (exp(time/p->tau)-1)*p->tau - (1-exp(-time/p->tau))*p->tau )
			   + 0.5*pow(p->sig*p->tau,2)*( (exp(time/p->tau)-1)*p->tau - (1-exp(-time/p->tau))*p->tau   )
			   + 0.5*pow(p->sig*p->tau,2)*( 0.5*(1-exp(-2*time/p->tau))*p->tau - time);
...
static Err make_int_phi_vector(TermStruct *l)
{
	...
	while ( lc->next != NULL ) 
	{
		...
		c->int_phi = p->int_phi + p->F*p->F*p->G*temp + 0.5*p->sig*p->sig*p->tau*(time-temp);
	}
...
Err            err = NULL;
	...
	while ( lc->next != NULL ) /* At least 2 elements in the ts */
	{
		...
		c->Lambda = p->Lambda +  p->tau*(exp(-time_prev / p->tau) - exp(- p->time / p->tau)) ;
	}
	return err;
} 

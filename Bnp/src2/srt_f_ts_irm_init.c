    ...
    double**     vasicek_mean_rev_level_data)
{
    ...
    /* Checks on the number of columns in Sigma : one value, two columns or three columns (with
     * Beta) */
    ...
    /* If the third column (the Beta) is missing, fill it in with the input beta or according to
     * model type */
    if (sig_cols < 3)
    {
        for (i = 0; i < num_sig; i++)
    ...
    while ((cur_sig < num_sig) && ((ppdSigmaValues[0][cur_sig] - today) <= EPSILON))
        cur_sig++;
    ...
    if (mdl_type == VASICEK)
    {
        ...
        while (cur_mean_rev_level < num_mean_rev_level)
        {
            ...
            if (tsval->date != vasicek_mean_rev_level_data[0][cur_mean_rev_level])
            {
                ...
                tsval->date = vasicek_mean_rev_level_data[0][cur_mean_rev_level];
                tsval->time =
                    (vasicek_mean_rev_level_data[0][cur_mean_rev_level] - today) * YEARS_IN_DAY;
        ...
        while (ls != NULL)
        {
            ...
            j = 0;
            while ((j < num_mean_rev_level) && (tsval->date > vasicek_mean_rev_level_data[0][j]))
                j++;
            if (j == num_mean_rev_level)
                tsval->mean_rev_level = vasicek_mean_rev_level_data[1][num_mean_rev_level - 1];
            else
...
Err make_vasicek_mean_sr_vector(TermStruct* ts)
{
    ...
    while (lc->next != NULL)
    {
        ...
        c->vasicek_mean_sr =
            p->vasicek_mean_sr + p->mean_rev_level * (exp(time / p->tau) - 1) / p->F;
    }
...
Err make_vasicek_mean_int_sr_vector(TermStruct* ts)
{
    ...
    while (lc->next != NULL)
    {
        ...
        c->vasicek_mean_int_sr = p->vasicek_mean_int_sr +
...
Err make_G_H_vector(TermStruct* l)
{
    ...
    while (lc->next != NULL) /* At least 2 elements in the ts */
    {
        ...
        c->H =
            temp * p->H + temp2 * temp * p->F * p->F * p->G + 0.5 * p->sig * p->sig * temp2 * temp2;
    }
    return err;
}
/* ------------------------------------------------------------------------- */
...
static Err make_L_vector(TermStruct* l)
{
    ...
    while (lc->next != NULL)
    {
        ...
        c->L =
            p->L + p->F * p->G * (1 - exp(-time / p->tau)) * p->tau +
...
static Err make_O_vector(TermStruct* l)
{
    ...
    while (lc->next != NULL)
    {
        ...
        c->O =
            p->O + p->F * p->G * p->Psi * (1 - exp(-time / p->tau)) * p->tau +
            p->F * p->F * p->G * p->tau *
                ((1 - exp(-time / p->tau)) * p->tau -
                 0.5 * (1 - exp(-2 * time / p->tau)) * p->tau) +
            0.5 * (p->Psi / p->F) * p->tau * p->sig * p->sig *
                ((exp(time / p->tau) - 1) * p->tau - (1 - exp(-time / p->tau)) * p->tau) +
...
static Err make_int_phi_vector(TermStruct* l)
{
    ...
    while (lc->next != NULL)
    {
        ...
        c->int_phi =
            p->int_phi + p->F * p->F * p->G * temp + 0.5 * p->sig * p->sig * p->tau * (time - temp);
    }

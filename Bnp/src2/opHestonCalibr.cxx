...
Depending on the type of calibration that is specified as input        , it
performs three different algorithms

                   1) If CalibrType = "MATCH_3STRIKES"        , it calibrates on
the BS volatilties associated to an increasing set of (three) strikes that are
inpout through the pointer *CalibrDblArgs. In all cases        , the middle
strike HAS TO BE at-the-money for the routine to work. First        , the local
vol is fitted to match the ATM volatility        , then it is freezed and the
correlation is fitted to the volatility skew associated to the two expreme
strikes. Finally        , the first two parameters are frozem and the volvol
(alpha) is fitted to the smile convexity. Then the loop is repeated until
convergence.

  `		   2) If CalibrType = "MATCH_CONV"        , it minimizes the
distance between the Heston-generated smile and a set of volatility provided by
the user in input. Here        , a global measure of convexity is used to
determine the best fit. In this case none of the input volatilities is exactly
matched.
...
/*-----------------------------------------------------------------------------------------------
                At any call        , this function returns the chi2 distance
between SABR and Heston smiles on nK points. This        , in turn        ,  is
used in the calibration routine under the specifics "CHI2_MIN" where the
algorithm is in fact based on the minimization of the Chi2 distance
-----------------------------------------------------------------------------------------------*/

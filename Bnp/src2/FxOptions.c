...
{		
	...
	{		
		...
		drift[i-1][0] = (swp_f_zr(date[i-1], date[i], dom_yc) - swp_f_zr(date[i-1], date[i], for_yc)) * dt
						- 0.5 * sig_quad[i-1][0] * sig_quad[i-1][0];
		drift[i-1][1] = (swp_f_zr(date[i-1], maturity, dom_yc) - swp_f_zr(date[i-1], maturity, for_yc)) * (mat - time[i-1])
...
FREE_RETURN:
	...
	if (time) free_dvector (time, 0, nstp - 1);
	if (date) free_dvector (date, 0, nstp - 1);
	
	if (drift) free_dmatrix (drift, 0, nstp - 1, 0, 1);
	if (sig_quad) free_dmatrix (sig_quad, 0, nstp - 1, 0, 1);	
		
	return err;
}

double	DeltaPIPS(	double	F,
					double	T,
					double	DfDom,
					double	DfFor,
					double	K,
...
Err		err = NULL;
	...
	switch (DeltaType)
	{
		case 0:
		{
			err = SABR_SolveDeltaNewton(Fwd, T, DfDom, DfFor, 0.25, 1, Sigma, Alpha, Beta, Rho, VolType, DeltaSimple, IsForeign, Precision, Strike1, Vol1);
			if (err)
			{
				return err;
			}

			err = SABR_SolveStraddleNewton(Fwd, T, DfDom, DfFor, Sigma, Alpha, Beta, Rho, VolType, DeltaSimple, IsForeign, Precision, Strike2, Vol2);
			if (err)
			{
				return err;
			}

			err = SABR_SolveDeltaNewton(Fwd, T, DfDom, DfFor, -0.25, 0, Sigma, Alpha, Beta, Rho, VolType, DeltaSimple, IsForeign, Precision, Strike3, Vol3);
			if (err)
			{
				return err;
			}
		...
		case 1:
		{
			err = SABR_SolveDeltaNewton(Fwd, T, DfDom, DfFor, 0.25, 1, Sigma, Alpha, Beta, Rho, VolType, DeltaPercent, IsForeign, Precision, Strike1, Vol1);
			if (err)
			{
				return err;
			}

			err = SABR_SolveStraddleNewton(Fwd, T, DfDom, DfFor, Sigma, Alpha, Beta, Rho, VolType, DeltaPercent, IsForeign, Precision, Strike2, Vol2);
			if (err)
			{
				return err;
			}

			err = SABR_SolveDeltaNewton(Fwd, T, DfDom, DfFor, -0.25, 0, Sigma, Alpha, Beta, Rho, VolType, DeltaPercent, IsForeign, Precision, Strike3, Vol3);
			if (err)
			{
				return err;
			}
		...
		case 2:
		{
			err = SABR_SolveDeltaNewton(Fwd, T, DfDom, DfFor, 0.25, 1, Sigma, Alpha, Beta, Rho, VolType, DeltaPIPS, IsForeign, Precision, Strike1, Vol1);
			if (err)
			{
				return err;
			}

			err = SABR_SolveStraddleNewton(Fwd, T, DfDom, DfFor, Sigma, Alpha, Beta, Rho, VolType, DeltaPIPS, IsForeign, Precision, Strike2, Vol2);
			if (err)
			{
				return err;
			}

			err = SABR_SolveDeltaNewton(Fwd, T, DfDom, DfFor, -0.25, 0, Sigma, Alpha, Beta, Rho, VolType, DeltaPIPS, IsForeign, Precision, Strike3, Vol3);
			if (err)
			{
				return err;
			}
...
Err		err = NULL;
	...
	if (IsForeign)
	{
		...
		if (IsCall)
		{
			...
			err = srt_f_optsarbvol(F, 1.0/K1, T, Sigma, Alpha, Beta, Rho, VolType, SRT_LOGNORMAL, &Sig1);
			if (err) return err;
		...
		err = srt_f_optsarbvol(F, 1.0/K2, T, Sigma, Alpha, Beta, Rho, VolType, SRT_LOGNORMAL, &Sig2);
		if (err) return err;
...
Err	err = NULL;
	...
	switch (DeltaType)
	{
		case 0:
		{
			err = SolveStraddleNewton(Fwd, T, DfDom, DfFor, VolStraddle, DeltaSimple, IsForeign, Precision, StrikeStraddle); 
			if (err)
			{
				return err;
			}
			err = SolveDeltaNewton(Fwd, T, DfDom, DfFor, 0.25, 1, Vol25Call, DeltaSimple, IsForeign, Precision, Strike25Call);
			if (err)
			{
				return err;
			}
			err = SolveDeltaNewton(Fwd, T, DfDom, DfFor, -0.25, 0, Vol25Put, DeltaSimple, IsForeign, Precision, Strike25Put);
			if (err)
			{
				return err;
			}
		...
		case 1:
		{
			err = SolveStraddleNewton(Fwd, T, DfDom, DfFor, VolStraddle, DeltaPercent, IsForeign, Precision, StrikeStraddle); 
			if (err)
			{
				return err;
			}
			err = SolveDeltaNewton(Fwd, T, DfDom, DfFor, 0.25, 1, Vol25Call, DeltaPercent, IsForeign, Precision, Strike25Call);
			if (err)
			{
				return err;
			}
			err = SolveDeltaNewton(Fwd, T, DfDom, DfFor, -0.25, 0, Vol25Put, DeltaPercent, IsForeign, Precision, Strike25Put);
			if (err)
			{
				return err;
			}
		...
		case 2:
		{
			err = SolveStraddleNewton(Fwd, T, DfDom, DfFor, VolStraddle, DeltaPIPS, IsForeign, Precision, StrikeStraddle); 
			if (err)
			{
				return err;
			}
			err = SolveDeltaNewton(Fwd, T, DfDom, DfFor, 0.25, 1, Vol25Call, DeltaPIPS, IsForeign, Precision, Strike25Call);
			if (err)
			{
				return err;
			}
			err = SolveDeltaNewton(Fwd, T, DfDom, DfFor, -0.25, 0, Vol25Put, DeltaPIPS, IsForeign, Precision, Strike25Put);
			if (err)
			{
				return err;
			}
...
double	*strike_sabr	= NULL,
	...
	err = FindStrikes(Fwd, T, DfDom, DfFor, DeltaType, IsForeign, Precision,
					Vol25Put, VolStraddle, Vol25Call, Strike25Put, StrikeStraddle, Strike25Call);
...
Err	BarrierOption_autocal(
							...
							int				cal_usetotalts,		/* 1: when calibrating smile, consider total TS, 0: consider only the corresponding ATM vol */
...
double	cal_fwd, cal_mat, cal_dfdom, cal_dffor,
		cal_strike1, cal_strike2, cal_strike3,
		cal_sigma, cal_vol1, cal_vol2, cal_vol3, cal_volATM;
...
double	coef, a, b, c;
		...
		else
		{
			...
			if (today + eod_pay_flag > ex_date_set)
			{
				...
				if (is_cvx)
				{
					if (is_call)
					{
						*price = max(spot_fx_fixing - strike, 0.0) * swp_f_df (today, ex_date_set, dom_yc) * notional;
					}
					else
					{
						*price = max(strike - spot_fx_fixing, 0.0) * swp_f_df (today, ex_date_set, dom_yc) * notional;
					}
				}
				else
				{
					if (is_call)
					{
						*price = max(1.0 / spot_fx_fixing - strike, 0.0) * swp_f_df (today, ex_date_set, dom_yc) * notional;
					}
					else
					{
						*price = max(strike - 1.0 / spot_fx_fixing, 0.0) * swp_f_df (today, ex_date_set, dom_yc) * notional;
					}
				}
			}
			else
			{
				/* fx not yet fixed */
				fwd_fx = cash_fx * swp_f_df (today, ex_date_set, for_yc) / swp_f_df (today, ex_date_set, dom_yc);
				...
				if (is_cvx)
				{
					...
					if (is_call)
					{
						*price = (1.0 / fwd_fx - strike) * swp_f_df (today, ex_date_set, dom_yc) * notional;
					}
					else
					{
						*price = (strike - 1.0 / fwd_fx) * swp_f_df (today, ex_date_set, dom_yc) * notional;
					}
				}
				else
				{
					if (is_call)
					{
						*price = (fwd_fx - strike) * swp_f_df (today, ex_date_set, dom_yc) * notional;
					}
					else
					{
						*price = (strike - fwd_fx) * swp_f_df (today, ex_date_set, dom_yc) * notional;
					}
				}
			}
		}
	...
	if (!use_old_ts)
	{
		...
		if (do_smile_calib)
		{
			...
			if (cal_index == nb_opt)
			{
				err = serror("Maturity %d for calibration is not present in the market ATM term struct", cal_maturity);
				goto FREE_RETURN;
			}

			cal_sigma = opt_atmvols[cal_index];		
			
			err = SABR_to_Strikes(	cal_fwd,
									cal_mat,
									1.0,
									1.0,
									cal_sigma,
		...
		if ((fabs(mdl_alpha) < 1.0E-04 && (do_smile_calib == 0 || (do_smile_calib == 1 && cal_stochvol == 0)))
		...
		err = FxSabrQuadCalibSmile2(dom_yc,
									for_yc,
									today,
									spot_fx,
									&mdl_alpha,
	...
	while (i < nstp)
	{
		...
		{
			/* middle part */
			coef = log(calib_ts[index+1] / calib_ts[index]) / (opt_exe[index+1] - opt_exe[index]);
...
FREE_RETURN:
	...
	if (opt_mat) free_dvector(opt_mat, 0, nb_opt-1);
	if (opt_exe) free_dvector(opt_exe, 0, nb_opt-1);	
	if (calib_ts) free_dvector(calib_ts, 0, nb_opt-1);
	...
	if (greeks_ki) free_dvector(greeks_ki, 0, 5);
...
Err		err = NULL;
	...
	cash_fx = spot_fx * swp_f_df (today, spot_date, dom_yc) / swp_f_df (today, spot_date, for_yc);
	fwd_fx = cash_fx * swp_f_df (today, settlmt_date, for_yc) / swp_f_df (today, settlmt_date, dom_yc);	
	...
	if (nb_opt > 1 && settlmt_fx_date > opt_matdates[0])
	{
		...
		if (opt_matdates[i-1] > settlmt_fx_date)
		{
			...
			cvx_vol = (opt_atmvols[i-1] - opt_atmvols[i-2]) / (opt_matdates[i-1] - opt_matdates[i-2]) * (settlmt_fx_date - opt_matdates[i-2]) + opt_atmvols[i-2];
		}
		else
		{
	...
	*vol = cvx_vol;
	...
	if (fabs(strike) < 1.0E-10)
	{
		...
		*price = exp(cvx_std * cvx_std) / fwd_fx * (norm(dd - 2.0 * cvx_std) - norm(du - 2.0 * cvx_std));
	}
	else
	{
		if (is_call)
...
Err cvx_static_replication(	double	*strikes,
							long	nb_strike,
							double	*coefJ,
							double	*res)
{
...
Err		err = NULL;		
	...
	{
		res[nb_strike-1] = 0.0;
		res[nb_strike-2] = 4.0 * (strikes[nb_strike-1] - strikes[nb_strike - 2]) / strikes[nb_strike-1] / strikes[nb_strike-1];

		for (i=nb_strike-3; i>=0; i--)
		{
			res[i] = pow(res[i+1] / (1.0 - sqrt(1 - res[i+1] * strikes[i+1])), 2) * (strikes[i+1] - strikes[i]) + res[i+1];
		}
...
void free_static_logconvex(LOGCONVEX_PARAM	cvx_param)
{
	if (cvx_param)
	{
		if (cvx_param->coef) free_dvector(cvx_param->coef, 0, cvx_param->nb_strike-1);
		if (cvx_param->coefJ) free_dvector(cvx_param->coefJ, 0, cvx_param->nb_strike-1);
		if (cvx_param->strikes) free_dvector(cvx_param->strikes, 0, cvx_param->nb_strike-1);
...
Err	static_logconvex(	double	param,
						double	std[],
						double	*price,
						double	*gradient,
						int		nb_strike)
...
double	strike_up_down, strike_up_up, coef_up, digit_up;
	...
	{
		index_up = Get_Index(logcvx_param->barrier_up, logcvx_param->strikes, logcvx_param->nb_strike);
	}
	else
	{
	...
	err = cvx_static_replication(logcvx_param->strikes, index_up+1, logcvx_param->coefJ, logcvx_param->coef);
	...
	{		
		sum += logcvx_param->coef[i] * srt_f_optblksch(	logcvx_param->fwd,
	...
	{	
		...
		sum += slope_up * srt_f_optblksch(	logcvx_param->fwd,
											strike_up,
											logcvx_param->log_std,
											1.0,
											logcvx_param->df,
		...
		sum += coef_up * srt_f_optblksch(	logcvx_param->fwd,
											strike_up_up,
											logcvx_param->log_std,
											1.0,
											logcvx_param->df,
	...
	{		
		...
		/* remove option part */		
		sum -= slope_down * srt_f_optblksch(logcvx_param->fwd,
											strike_down,
											logcvx_param->log_std,
											1.0,
											logcvx_param->df,
		...
		sum -= coef_down * srt_f_optblksch(	logcvx_param->fwd,
											strike_down_up,
											logcvx_param->log_std,
											1.0,
											logcvx_param->df,
...
Err		err = NULL;
	...
	if (nb_opt > 1 && settlmt_fx_date > opt_matdates[0])
	{
		...
		{
			/* linear interpolation */
			atm_vol = (opt_atmvols[i-1] - opt_atmvols[i-2]) / (opt_matdates[i-1] - opt_matdates[i-2]) * (settlmt_fx_date - opt_matdates[i-2]) + opt_atmvols[i-2];
		}
		else
		{
	...
	for (i=0; i<nb_strike; i++)
	{
		if (fabs(coef[i]) > 1.0E-10)
		{
			...
			sum += coef[i] * srt_f_optblksch(	fwd_fx,
												strikes[i],
												vols[i],
												mat,
												df,
	...
	{	
		...
		sum += digit_up * srt_f_optblksch(	fwd_fx,
											strike_up,
											vol_up,
											mat,
											df,
	...
	if (barrier_down > 1.0E-10)
	{
		...
		sum -= slope_down * srt_f_optblksch(fwd_fx,
											strike_down,
											vol_down,
											mat,
											df,
		...
		sum += digit_down * srt_f_optblksch(fwd_fx,
											strike_down,
											vol_down,
											mat,
											df,
...
FREE_RETURN:
	...
	if (coefJ) free_dvector (coefJ, 0, nb_strike-1);
	if (data) free_dvector(data, 0, 1);
	if (weight) free_dvector(weight, 0, 1);
	if (target) free_dvector(target, 0, 1);
	if (param) free_dvector(param, 0, nb_strike);
...
Err		err = NULL;	
	...
	for (num_calib_rates=0 ; (num_calib_rates < nb_exe) && (ex_date[num_calib_rates] < exercise_date) ; num_calib_rates++);
	...
	err = Fx3DtsImpliedVol_corr(opt_maturity,
								0.0,
								exe_maturity,
								merge_dates,
								nb_merge_dates,
	...
	*price = notional * srt_f_optblksch(fwd_fx, strike, *vol, exe_maturity, df, CallPutFlag, PREMIUM);	
...
FREE_RETURN:
	...
	if (export_ts)
	{
		...
		if (fx_vol_curve)
		{
			...
			{
				und_exp->sigma_date_fx[i] = add_unit (fx_mkt_vol_date[i], -2, SRT_BDAY, MODIFIED_SUCCEEDING);
			}

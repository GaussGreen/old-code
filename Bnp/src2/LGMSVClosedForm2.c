...
void LGMSVSolveODELogTSEvalFunc(
    double  Coef1Re,
    double  Coef2Re,
    double  Coef2Im,
    double  Coef3Re,
...
void LGMSVCalculateDensityTPPoint(
    double dPhifreq,
    double dftFreq,
    ...
    double* LogTFIm)
{
    ...
    for (i = endi; i >= 0; i--)
    {
        ...
        for (j = 0; j < nbp; j++)
        {
            ...
            LGMSVSolveODELogTSEvalFunc(
                Coef1Re, Coef2Re, Coef2Im, Coef3Re, Coef3Im, expt2, DRe, DIm, &K1Re, &K1Im);
            expt2 *= coefexp;
            LGMSVSolveODELogTSEvalFunc(
                Coef1Re,
                Coef2Re,
...
void LGMSVCalculateDensityPrecalc(
    int    iNbPhi,
    int    iNbft,
    double dPhiFreqStep,
    double dftFreqStep,
    ...
    double**  DensitySpeq)
{
    ...
    double  CoefA;
    double *Coef1ReT = NULL, *Coef2ReT = NULL, *Coef2ImT = NULL, *Coef3ReT = NULL, *Coef3ImT = NULL,
           *CoefAT = NULL, *CoefexpT = NULL;
    ...
    if (!Coef1ReT || !Coef2ReT || !Coef2ImT || !Coef3ReT || !Coef3ImT || !CoefAT || !CoefexpT ||
        !nbpT)
    {
...
void LGMSVSolveODELogTSRungeKutta(
    double dPhifreq,
    double dFtFreq,
    double T,
    ...
    double* Price)
{
    ...
    if (!Density || !Densityspeq || !Coef1ReT || !Coef2ReT || !Coef2ImT || !Coef3ReT || !Coef3ImT ||
    ...
    if (SaveFile)
    {
        ...
        LGMSVSavePayOff(
            iNbPhi, iNbft, iIndexPhiMean, iIndexft0, dPhiStep, dftStep, dPhitMean, PayOff);
    }
    ...
    double* pSwaptionPrice)
{
    ...
    swp_cash =
        (swp_f_df(today, cpn_date[0], yc_name) - swp_f_df(today, cpn_date[ncpn - 1], yc_name)) /
        level;
    ...
    double* sigma)
{
    ...
    double *Coef1ReT = NULL, *Coef2ReT = NULL, *Coef2ImT = NULL, *Coef3ReT = NULL, *Coef3ImT = NULL,
           *CoefAT = NULL, *CoefexpT = NULL, **res_iter = NULL;
    ...
    for (i = 0; i < nbEx; i++)
    {
        ...
        {
            smessage(
                "Numerical problem at option %d, try to decrease MaxTime or number of Phi/ft",
                i + 1);
            for (j = i; j < nbEx; j++)
            {
                sigma[j] = 0.0;
        ...
        if (fabs(TargetPrice[i] - price1) > Precision)
        {
            ...
            err = srt_f_optimpvol(
                price1, Fwd[i], Strike[i], t2, Level[i], SRT_CALL, SRT_NORMAL, &imp_vol);
            ...
            err = op_sabr_pricing(
                ...
                0.0,
                dRho,
                beta_vol * (1.0 + factor * LGM_VEGA_SHIFT),
                SABR_ATM_BETA,
                &shift_price);
            ...
            sig2 =
                sqrt((newvar * pow(1.0 + (TargetPrice[i] - price1) / vega, 2) - var) / (t2 - t1));

            /*
            sig2 = sqrt((newvar * TargetPrice[i] * TargetPrice[i] / price1 / price1 - var) / (t2 -
            t1));
            */
            ...
            while (fabs(TargetPrice[i] - price1) > Precision && k < NbIterMax)
            {
                ...
                {
                    smessage(
                        "Numerical problem at option %d, try to increase MaxTime or number of "
                        "Phi/ft",
                        i + 1);
                    for (j = i; j < nbEx; j++)
                    {
    ...
    CPD_CALIB_INST_DATA inst_data) /*	NULL = don't save calibration instrument data */
{
    ...
    double         ex_time[MAX_CPN], ex_lfwd[MAX_CPN], ex_llvl[MAX_CPN], ex_lstrike[MAX_CPN],
        ex_lvol[MAX_CPN], ex_lprice[MAX_CPN], atm_price[MAX_CPN], ex_zeta[MAX_CPN];
    ...
    for (i = 0; i < nex; i++)
    {
        ...
        if (power > 0.5)
        {
            ex_lprice[i] = srt_f_optblksch(
                ex_lfwd[i], ex_lstrike[i], ex_lvol[i], ex_time[i], ex_llvl[i], SRT_CALL, PREMIUM);
        }
        else
        {
    ...
    */
    ...
    for (i = 0; i < nex; i++)
    {
        ...
        /*
        dDFFTexTstar = dDFTodayTstar / swp_f_df(lExDate, today + DAYS_IN_YEAR * ex_time[i],
        yc_name);
        */
    ...
    *sig      = (double*)calloc(nex, sizeof(double));
    ...
    if (power > 0.5)
    {
        for (i = 0; i < nex; i++)
        {
            ...
            {
                AlphaEq = dAlpha * sqrt(
                                       (1.0 - exp(-2.0 * dLambdaEps * ex_time[i])) /
            ...
            atm_price[i] = srt_f_optblksch(
                ex_lfwd[i], ex_lfwd[i], atm_vol, ex_time[i], ex_llvl[i], SRT_CALL, PREMIUM);
        }
    }
    else
    {
    ...
    for (i = 1; i < nex; i++)
    {
        ...
        {
            (*sig)[i] = sqrt(
                (ex_zeta[i] - ex_zeta[i - 1]) * 2 * (dLambdaX) /

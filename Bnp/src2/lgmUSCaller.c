...
LGMErr GetSABRParam(long			dstart, 
					long			dend,
					char			*vol_id,
					SABR_VOL_TYPE	component,
					double			*result)
{
LGMErr err=NULL;
int icomponent;
...
double power;
	...
	switch(component)
	{
	case SABR_ATM_LOG: icomponent = 0;
		break;
	case SABR_ATM_NORM: icomponent = 1;
		break;
	case SABR_ATM_BETA: icomponent = 2;
		break;
	case ALPHA: icomponent = 5;
		break;
	case BETA: icomponent = 6;
		break;
	case RHO: icomponent = 7;
		break;
	}
...
LGMErr SetUpStaticMarketIfo( LGMErr (*GetVol)(long, long, double, SRT_Boolean, double *),
							char* YCName, double dMaxStrikeStd, char* char_vol_type )
{
...
/* Wrapper to the input volatility function that cuts the volatility at +/- StdDev above and below the ATM */
...
/* Calculate the CASH forward swap rate */
	if  ( err = swp_f_ForwardRate_SwapDP( &swap_dp, StaticMarketInfo.YieldCurveName, "CASH", &dForward )  )
		return err;
...
//	Calibration
	...
	int nofunding,
	double dStrikeVolCap, /* Maximum lognormal std dev for smile around ATM (i.e., vol is flat outside) */
...
/* Accrual Period Day Rule -- not currently used:  1: only use business days, 3: count holidays using past setting */
	int iAccrualPeriodDayRule,
	
/* Coupon Index Lag -- not currently used:  number of business days before start that each index sets */
	int iCouponIndexLag,

/* End Lag -- not currently used:  number of business days before the end of the period that we stop checking the index */
	int iEndLag,
...
// Otherwise, calculate the PV of the fixed period that we are on
		return calcRangeAccrual( lPayDate, cpn[iLastStart-1] + tgear_floatdigital[iLastStart-1] * dFloatCouponFixing, eod_fix_flag, iAccrueOnBarrier, 
									nDaysInFirstCoupon, lvFirstCouponFixingDates, dvFirstCouponAccrualWeights, dvFirstCouponFixingOrSpread, 
									barriers[iLastStart-1][1], barriers[iLastStart-1][0], call_spread_up, call_spread_low, szIndexRefRate, cvgCpn[iLastStart-1],  
									FixedLeg );
	}
...
// Calculate the PV of the current period (0 if we haven't started yet)
		if ( iLastStart == 0 )
			*FixedLeg = 0.0;
			...
			if ( error = calcRangeAccrual( lPayDate, cpn[iLastStart-1] + tgear_floatdigital[iLastStart-1] * dFloatCouponFixing, eod_fix_flag, iAccrueOnBarrier, 
										nDaysInFirstCoupon, lvFirstCouponFixingDates, dvFirstCouponAccrualWeights, dvFirstCouponFixingOrSpread, 
										barriers[iLastStart-1][1], barriers[iLastStart-1][0], call_spread_up, call_spread_low, szIndexRefRate, cvgCpn[iLastStart-1],
										&dStubPV ) )
				return error;
...
//		tNow = get_clcndate_from_yldcrv (yldcrv);    /* get calculation date */
		...
		/* Get the exponent beta to use to interpret market vols. Useful only to cope with the generica autocal code */
...
/* get yield curve */
	...
	*nExleftPtr = nExEff;	/* Number of exercises after today */
	...
	i=n-1;
	while (i>=0 && tPayCpn[i] >= tNow + eod_pay_flag) i--;
	i++;
	if (i<n)
	{
		iFirstTS = i;
	...
	if (iFirstTS<iFirst)		/* then we create a time swap deal */
	{
		...
		{	
			...
			{
				/* for each subperiod ... */
				ptrTS->observationdays[i-iFirstTS][j-1][0] = (int) (tStartCpn[i] + (j-1)*(numdays_in_period+0.0)/(observation_freq - 1.0)); /* Start Date */
				ptrTS->observationdays[i-iFirstTS][j-1][1] = add_unit(ptrTS->observationdays[i-iFirstTS][j-1][0],ptrTS->undIndex,SRT_MONTH,NO_BUSDAY_CONVENTION);
				ptrTS->observationdays[i-iFirstTS][j-1][1]= add_unit(ptrTS->observationdays[i-iFirstTS][j-1][1],0,SRT_BDAY,MODIFIED_SUCCEEDING);
			}
			...
			else 
			{
				for (j=1;j <= observation_freq;j++)
				{
					...
					{
						ptrTS->ratiodays_for_subperiod[i-iFirstTS][j-1] = (double) (ptrTS->observationdays[i-iFirstTS][j][0] - ptrTS->observationdays[i-iFirstTS][j-1][0] + 0.0)/(numdays_in_period+0.0);
					}
					else 
					{
						ptrTS->ratiodays_for_subperiod[i-iFirstTS][observation_freq-1] = (double) (tPayCpn[i]-(ptrTS->observationdays)[i-iFirstTS][observation_freq-1][0]+0.0)/(numdays_in_period+0.0);
					}
				}
			}
		}
...
/*	ptrTS = LGMCreateCallableTimeSwap(1,n-iFirstTS,observation_freq); */
	...
	switch (freqIndex) {
	case SRT_ANNUAL:	ptr->undIndex = 12;
						break;
	case SRT_SEMIANNUAL:	ptr->undIndex = 6;
							break;
	case SRT_QUARTERLY:		ptr->undIndex = 3;
							break;
	case SRT_MONTHLY:		ptr->undIndex = 1;
							break;
	}
...
/*		switch (freqIndex) {
	...
	*/
		...
		switch (freqAccrued) {
	case SRT_ANNUAL:	ptr->undAccrued = 12;
						break;
	case SRT_SEMIANNUAL:	ptr->undAccrued = 6;
							break;
	case SRT_QUARTERLY:		ptr->undAccrued = 3;
							break;
	case SRT_MONTHLY:		ptr->undAccrued = 1;
							break;
		}
/*
		switch (freqAccrued) {
	...
	{	
		...
		{
			/* for each subperiod ... */
			ptr->observationdays[i-iFirst][j-1][0] = (int) (first_future_fixing + (j-1)*(remaining_numdays_in_period+0.0)/(observation_freq+0.0)); /* Start Date */
			ptr->observationdays[i-iFirst][j-1][1] = add_unit(ptr->observationdays[i-iFirst][j-1][0],ptr->undIndex,SRT_MONTH,NO_BUSDAY_CONVENTION);
			ptr->observationdays[i-iFirst][j-1][1]= add_unit(ptr->observationdays[i-iFirst][j-1][1],0,SRT_BDAY,MODIFIED_SUCCEEDING);
		}
		if (observation_freq==2) /* HARD CODE FOR THE PERIODIC CAP */
		{
		...
		else
		{
			for (j=1;j <= observation_freq;j++)
			{
				if (j<observation_freq)
				{
					ptr->ratiodays_for_subperiod[i-iFirst][j-1] = (double) (ptr->observationdays[i-iFirst][j][0] - ptr->observationdays[i-iFirst][j-1][0] + 0.0)/(numdays_in_period+0.0);
				}
				else 
				{
					ptr->ratiodays_for_subperiod[i-iFirst][observation_freq-1] = (double) (tPayCpn[i]-(ptr->observationdays)[i-iFirst][observation_freq-1][0]+0.0)/(numdays_in_period+0.0);
				}
			}
		}
	}
...
static char* BS( long lFixing, double dStrike, char* szRefRate, double dSpread, SrtCallPutType srtCallPut, double* out_pdPrice )
{
...
// Get the start and end dates
	lStart = add_unit( lFixing, StaticMarketInfo.Conventions.lag, SRT_BDAY, StaticMarketInfo.Conventions.cbdconv );
	lEnd = add_unit(lStart, (int)(12/Comp), SRT_MONTH, StaticMarketInfo.Conventions.cbdconv );

// Get the forward
	dFwd = swp_f_swapcashrate( lStart, lEnd, Basis, Comp, StaticMarketInfo.YieldCurveName, szRefRate );

// If the expiry is 0, return the intrinsic value
	if ( dExpiry == 0.0 )
	{
		*out_pdPrice = srt_f_optblksch( dFwd, dStrike - dSpread, 0.0, dExpiry, 1.0, srtCallPut, PREMIUM );
		return 0;
	}
...
// Price the option
	if ( StaticMarketInfo.IsLognormal )
		*out_pdPrice = srt_f_optblksch( dFwd, dStrike - dSpread, dVol, dExpiry, 1.0, srtCallPut, PREMIUM );
	else
		*out_pdPrice = srt_f_optblknrm( dFwd, dStrike - dSpread, dVol, dExpiry, 1.0, srtCallPut, PREMIUM );

// return
	return 0;
}

static char* putSpread( long lFixing, double dBarrier, double dSpread, char* szRefRate, double dPutSpread, double* out_pdProb )
{
...
static char* getSinglePV( long lFixing, double dUpperBarrier, double dLowerBarrier,  double dUpperSpread, double dLowerSpread, 
						 char* szRefRate, double dSpread,
							double* out_pdProb )
{
// Variable declarations
...
LGMErr calcRangeAccrual( long lPayDate, double dCoupon, int iEODFixFlag, int iAccrueOnBarrier, int nDaysInCoupon, 
							long* lvFixingDates, double* dvWeights, double* dvFixingOrSpread, 
							double dUpperBarrier, double dLowerBarrier,  double dUpperSpread, double dLowerSpread, 
							char* szRefRate, double dCvg,
							double* out_pdFixed )
{
// Variable declaration
	int i;
...
// Loop over the remaining dates, calculating the PV of each
	...
	{
		if ( err = getSinglePV( lvFixingDates[i], dUpperBarrier, dLowerBarrier, dUpperSpread, dLowerSpread, szRefRate, dvFixingOrSpread[i], &dProb )  )
			return err;
...
// return the value
	...
	*out_pdFixed = dCvg * dAccrual * dCoupon * swp_f_df( lToday, lPayDate, StaticMarketInfo.YieldCurveName ) / dTotalDays;
...
static LGMErr STATIC_GetVol( Date lStartDate, Date lEndDate, double dStrike, SRT_Boolean b, double *Vol )
{
...
/* today's volatilities */
		LGMErr (*GetVol)(Date, Date, Date, double, SRT_Boolean, double *), /* function to get swaption vols */		

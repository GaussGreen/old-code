...
- on the exotic side  , (cpn + gear*accrued_index)*accrued_ratio*coverage */
  ...
  int convertTS =
      1; /* 1 = cmpute new sigmas  , taus & store in ts; 0 = don't bother */
  int findExerBdry =
  ...
  *FixedLeg = 0.0;
  ...
  // Check to see if the option is exercised  , at which point we can return the
  // current PV
  if (isExercised) {
    ...
    while (iLastStart < nCpn && tCpnStart[iLastStart] < tNow)
      iLastStart++;
    // If the deal is exercised before the first start  , then there is nothing
    // to do
    if (iLastStart == 0) {
      return 0;
    }
...
//
/* Create two structures  , one for the option to call  , and one for the time
 * swap itself  */
...
static char *BS(long lFixing, double dStrike, char *szRefRate, double dSpread,
  ...
  lStart = add_unit(lFixing, StaticMarketInfo.Conventions.lag, SRT_BDAY,
                    StaticMarketInfo.Conventions.cbdconv);

    ...
    long*   ia)
{
    ...
    if (err = swp_f_vol(
            szVolCurveName, (double)dDatespot, (double)dDatespot3, dFra, &atmvol, &lognorm))
    {
    ...
    err = optcalibmertontimedep(
        dFra, NMAXSTRIKES, nperiods, strikes, vols, param, mat, "Lognormal", calerr, ia, calibvols);
...

{
    ...
    /*freeze the intensities of the jumps and the parameters that are determined by the stationarity
     * assumption*/
    freeze[3] = 0;
    freeze[5] = 0;
    ...
    for (j = 1; j <= nfixdays; j++)
    {
        ...
        if (j == 1)
        {
            for (n = 1; n <= nperiods - 1; n++)
            {
                ...
                sig[j] += periodparam[j][5 * (n - 1) + 1] * periodparam[j][5 * (n - 1) + 1] *
                          weightsmat[n];
            }
    ...
    double** pv)
{
    ...
    if (err)
    {
        free_resetablegeneral_td(
            weights, points, wn1n2, paramatrix, nherm, n_resetdates, caliberror);
        return err;
    }
    ...
    for (perindex = 1; perindex < n_resetdates; perindex++)
    {
        ...
        for (j = 1; j <= nfixdays; j++)
        {
            err = get_param_td(periodmat[j], perindex, resetdates, paramatrix, paramperiod[j]);
        }
            ...
            for (n2 = 0; n2 <= N2max; n2++)
            {
                ...
                if (probajumps > probainf)
                {
                    ...
                    for (n = 1; n <= 2 * nherm; n++)
                    {
                        ...
                        err = DRSCalc_td(
                            wn1n2, (int)nfixdays, resetmat, paramperiod, mat, perindex + 1, DRSts);
                        ...
                        {
                            pv[1][perindex] +=
                                weights[n] * probajumps * (DRSts[1] + margin) * prix / ntotaldays;
                        }

...
Err crif_allocate_mc_arg(CRIF_MCARG sMCArg)
{
    ...
    if (!sMCArg->dDates || !sMCArg->dSigma || !sMCArg->dAlphaSV || !sMCArg->dLambdaSV ||
        !sMCArg->dLevelSV || !sMCArg->dRhoSV || !sMCArg->dRho2SV || !sMCArg->dLGMAlpha ||
        !sMCArg->dLGMRho || !sMCArg->iEvalEvent || !sMCArg->iIndexEvent || !sMCArg->vPayoffParams ||
...
Err crif_fill_eval_const_coupon(
    ...
    CRIF_COUPONARG sCouponArg)
{
    ...
    sCouponArg->tpay_tstar_alpha =
        swp_f_df(sModel->lTStarDate, sExotic->lPayDate[iIndexCoupon], sMarket->cYcName);
    sCouponArg->tpay_tstar_alpha = -log(sCouponArg->tpay_tstar_alpha);
    sCouponArg->tpay_tstar_beta =
        crif_beta_func(sModel->lTStarDate, sExotic->lPayDate[iIndexCoupon], sModel->dLambdaX);
    sCouponArg->tpay_tstar_gamma = 0.5 * sCouponArg->tpay_tstar_beta * sCouponArg->tpay_tstar_beta;

    if (sModel->iOne2F == 2)
    {
        sCouponArg->tpay_tstar_beta_2 =
            crif_beta_func(sModel->lTStarDate, sExotic->lPayDate[iIndexCoupon], sModel->dLambdaX2);
    ...
    if (1.0 > 0)
    {
        ...
        if (fabs(sExotic->dPaidGearing[iIndexCoupon]) > 0)
        {
            /* Get the schedule */
            err = add_tenor(
                lStartDate, sExotic->cPaidTenor[iIndexCoupon], NO_BUSDAY_CONVENTION, &lTheoEndDate);
            if (err)
                goto FREE_RETURN;

            err = swp_f_initSwapDP(
                lStartDate, lTheoEndDate, sExotic->cPaidFreq, sExotic->cPaidBasis, &sFixedSwap);
            ...
            {
                dLevel += coverage(
                              sFixedSwapDates.date[j - 1],
                              sFixedSwapDates.date[j],
            ...
            for (j = 0; j < sFixedSwapDates.len; j++)
            {
                ...
                if (j > 0)
                {
                    sCouponArg->dCMSCoverage[j] = coverage(
                        sFixedSwapDates.date[j - 1],
                        sFixedSwapDates.date[j],
                        sFixedSwap.basis_code);
                }
        ...
        for (i = 0; i < sCouponArg->iNbDF; i++)
        {
            ...
            if (sModel->iOne2F == 2)
            {
                ...
                sCouponArg->tdf_tstar_gamma_2[i] =
                    0.5 * sCouponArg->tdf_tstar_beta_2[i] * sCouponArg->tdf_tstar_beta_2[i];
    ...
    CRIF_CALLARG  sCallArg)
{
    ...
    sCallArg->tset_tstar_alpha =
        swp_f_df(sModel->lTStarDate, sCall->lSettlDate[iIndexCall], sMarket->cYcName);
    sCallArg->tset_tstar_alpha = -log(sCallArg->tset_tstar_alpha);
    sCallArg->tset_tstar_beta =
        crif_beta_func(sModel->lTStarDate, sCall->lSettlDate[iIndexCall], sModel->dLambdaX);
    sCallArg->tset_tstar_gamma = 0.5 * sCallArg->tset_tstar_beta * sCallArg->tset_tstar_beta;

    if (sModel->iOne2F == 2)
    {
        sCallArg->tset_tstar_beta_2 =
            crif_beta_func(sModel->lTStarDate, sCall->lSettlDate[iIndexCall], sModel->dLambdaX2);
    ...
    err = crif_fill_eval_const_coupon(
        sMarket, sDeal, sModel, sCallAux->iStartCpnIdx[iIndexCall], sCallArg->mceb_sCouponArg);

    return err;
}

Err crif_fill_mc_arg(
    CMSCTS_MARKET       sMarket,
    CRIF_DEAL           sDeal,
    ...
    CRIF_PRICING_PARAMS sPricingParams)
{
    ...
    long   lNbAllTimes;
    int    iIndexFunding, iIndexCoupon, iIndexCouponForFixing, iIndexFixing, iIndexCall, iNbEvent;
    ...
    for (i = 0; i < sMCArg->iNbTimes; i++)
    {
            ...
            fabs(sMCArg->dDates[i] - sCall->lExeDate[iIndexCall]) < 0.5)
        {
            ...
            err = crif_fill_eval_const_call(
                sMarket, sDeal, sModel, iIndexCall, iLastFundIndex, sPayArg->sCallArg);
            ...
            fabs(sMCArg->dDates[i] - sExotic->lPaidFixingDate[iIndexCoupon]) < 0.5)
        {
            ...
            err = crif_fill_eval_const_coupon(
                sMarket, sDeal, sModel, iIndexCoupon, sPayArg->sCouponArg);
                   ...
                   int*    stop_path)
{
    ...
    if (sPayArg->iIsCall)
    {
        ...
        if (sCallArg->mceb_sCouponArg)
        {
            ...
            for (i = 0; i < 1; i++)
            {
                if (sCallArg->mceb_sCouponArg->iNbCMSDF > 0)
                {
                    ...
                    dFloat =
                        ...
                        sCallArg->mceb_sCouponArg
                            ->dAllDF[sCallArg->mceb_sCouponArg
                                         ->iIndexCMSDF[sCallArg->mceb_sCouponArg->iNbCMSDF - 1]];
            ...
            case REGULAR_CRIF:
            {
                dCoupon = sCallArg->mceb_sCouponArg->dAlpha * sMCArg->dSavedCoupon[path_index] +
                          sCallArg->mceb_sCouponArg->dGamma * dBasket +
                          sCallArg->mceb_sCouponArg->dBeta;

                dCoupon = DMIN(
                    DMAX(dCoupon, sCallArg->mceb_sCouponArg->dFloor),
                    sCallArg->mceb_sCouponArg->dCap);
                DFR_pay =
                    exp(-sCallArg->mceb_sCouponArg->tpay_tstar_alpha -
            ...
            {
                prod_val[4] =
                    dCoupon * DFR_pay * sCallArg->mceb_sCouponArg->dCoef;  // 1: le next coupon
    ...
    if (sPayArg->iIsCoupon)
    {
        ...
        {
            sCouponArg->dAllDF[i] =
                exp(-sCouponArg->tdf_tstar_alpha[i] - sCouponArg->tdf_tstar_beta[i] * ft -
        ...
        for (i = 0; i < 1; i++)
        {
            if (sCouponArg->iNbCMSDF > 0)
            {
                ...
                dFloat = sCouponArg->dAllDF[sCouponArg->iIndexCMSDF[0]] -
                         sCouponArg->dAllDF[sCouponArg->iIndexCMSDF[sCouponArg->iNbCMSDF - 1]];
    ...
    int*    stop_path)
{
    ...
    if (sPayArg->iIsCall)
    {
        ...
        {
            DFR_pay = exp(
                -sCallArg->tpay_tstar_alpha[i] - sCallArg->tpay_tstar_beta[i] * ft1 -
                sCallArg->tpay_tstar_beta_2[i] * ft2 - sCallArg->tpay_tstar_gamma[i] * psi1 -
                sCallArg->tpay_tstar_gamma_2[i] * psi2 - sCallArg->tpay_tstar_gamma_12[i] * psi12);
        ...
        /* first coupon */
        DFR_pay =
            exp(-sCallArg->tpay_tstar_alpha[i] - sCallArg->tpay_tstar_beta[i] * ft1 -
                sCallArg->tpay_tstar_beta_2[i] * ft2 - sCallArg->tpay_tstar_gamma[i] * psi1 -
                sCallArg->tpay_tstar_gamma_2[i] * psi2 - sCallArg->tpay_tstar_gamma_12[i] * psi12);
        ...
        if ((sCallArg->mceb_sCouponArg))
        {
            ...
            prod_val[6] =
                ...
                (sCall->dPayRec * dPartialFunding -
                 dCoupon * DFR_pay * sCallArg->mceb_sCouponArg->dCoef);  // 3: le current coupon

            /* then calculate the fee */
            if (sMCArg->iHasCallFee)
            {
                DFR_set = exp(
                    -sCallArg->tset_tstar_alpha - sCallArg->tset_tstar_beta * ft1 -
                    sCallArg->tset_tstar_beta_2 * ft2 - sCallArg->tset_tstar_gamma * psi1 -
                    sCallArg->tset_tstar_gamma_2 * psi2 - sCallArg->tset_tstar_gamma_12 * psi12);
    ...
    if (sPayArg->iIsCoupon)
    {
        ...
        {
            sCouponArg->dAllDF[i] =
                exp(-sCouponArg->tdf_tstar_alpha[i] - sCouponArg->tdf_tstar_beta[i] * ft1 -
                    sCouponArg->tdf_tstar_beta_2[i] * ft2 - sCouponArg->tdf_tstar_gamma[i] * psi1 -
        ...
        case REGULAR_CRIF:
        {
            ...
            sMCArg->dSavedCoupon[path_index] = dCoupon;
            DFR_pay                          = exp(
                -sCouponArg->tpay_tstar_alpha - sCouponArg->tpay_tstar_beta * ft1 -
                sCouponArg->tpay_tstar_beta_2 * ft2 - sCouponArg->tpay_tstar_gamma * psi1 -
    ...
    CRIF_OUTPUTS        sOutputs)
{
    ...
    dTempVal = dmatrix(
        ...
        sMCArg->iNbEvent + 4 + 1,
        0,
        1 + 2 + sMCArg->sMCEBParams->iNbIndex + sMCArg->sMCEBParams->iHasNumeraire);
    ...
    smessage(
        "Launching MC, Nb Steps: %d, Nb Paths: %d",
        sMCArg->iNbTimes,
...
FREE_RETURN:
    ...
    if (dTempVal)
        free_dmatrix(
            ...
            sMCArg->iNbEvent + 4,
            0,
            2 + sMCArg->sMCEBParams->iNbIndex + sMCArg->sMCEBParams->iHasNumeraire);

    return err;
}

Err crif_calibrate_model(
    CMSCTS_MARKET sMarket,
    CRIF_DEAL     sDeal,
    CRIF_SIMULARG sSimulArg,
    ...
    CRIF_OUTPUTS  sOutputs)
{
    ...
    double *dPrimStrike = NULL, *dPrimStrikeS1 = NULL, *dPrimStrikeS2 = NULL, *dSecStrike = NULL,
           *dSecStrikeS1 = NULL, *dSecStrikeS2 = NULL, *dSecWeights = NULL;

    int     iNbPWTime, iNbPWTimeNew;
    double  dLambdaX;
    double *dPWTime = NULL, *dSigma = NULL, *dAlphaSV = NULL, *dLambdaSV = NULL, *dRhoSV = NULL,
           *dRho2SV = NULL;
    ...
    if (!cPrimTenors || !lPrimExeDates || !iPrimCalib || !dPrimStrike || !dPrimStrikeS1 ||
        !dPrimStrikeS2 || !cSecTenors || !lSecExeDates || !iSecCalib || !dSecStrike ||
        !dSecStrikeS1 || !dSecStrikeS2 || !dSecWeights)
    {
    ...
    case CMSCTS_DIAGCMS1:
    {
        for (i = 0; i < lNbPrim + 1; i++)
        {
            if (i < lNbPrim)
            {
                ...
                {
                    lPrimExeDates[i] = add_unit(
                        sExotic->lStartDate[i + iStartIndex],
                        -sExotic->iFixingLag,
                        SRT_BDAY,
                        MODIFIED_SUCCEEDING);
                }
        ...
        for (j = 0; j < lNbSec + 1; j++)
        {
            if (j < lNbSec)
            {
                ...
                {
                    lSecExeDates[j] = add_unit(
                        sExotic->lStartDate[j + iStartIndex],
                        -sExotic->iFixingLag,
                        SRT_BDAY,
                        MODIFIED_SUCCEEDING);
                }
    ...
    case CMSCTS_CMS1DIAG:
    {
        for (i = 0; i < lNbPrim + 1; i++)
        {
            if (i < lNbPrim)
            {
                ...
                {
                    lPrimExeDates[i] = add_unit(
                        sExotic->lStartDate[i + iIndexCpn],
                        -sExotic->iFixingLag,
                        SRT_BDAY,
                        MODIFIED_SUCCEEDING);
                }
    ...
    for (i = iStartIndex; i < sExotic->iNbCpn; i++)
    {
        ...
        case REGULAR_CRIF:
        {
            ...
            {
                err = add_tenor(
                    sDeal->lTheoEndDate, sExotic->cPaidTenor[i], NO_BUSDAY_CONVENTION, &temp);
    ...
    iNbPWTimeNew =
        iNbPWTime +
        (int)(((sDeal->lTheoEndDate - sMarket->lToday) * YEARS_IN_DAY - dPWTime[iNbPWTime - 1]) / dStepCalib) +
        1;

...
#define set_lattice_flg                                                        \
  {                                                                            \
    lattice_flg = (GrfnIsStatus(GrfnCGCell(gd).status, GRFNCSPVREF)            \
                       ? COMLL_T_SET                                           \
                       : COMLL_T_INCREMENT);                                   \
  }
...
*                       the same row of a GRFN Tableau.
                    ...
                    NB: The obscure rule of the GRFN Language says that if
                        the PV[...] function occurs in a cell within ANY of
                                                of the tableau (with the same
column reference or NOT) then the value being discounted through the tree is
                                                REPLACED by the value at that
cell after evaluation (like American style option to represent an exercise , or
a choice depending on an otpion value) otherwise it is INCREMENTED by that
amount (like for the sum of deterministic cash flows        , or the final
discounted sum of all last column cells) The "treeset" will be the value of the
"type" input to this function; it will not be inserted if it is not one of
COMLL_T_SET and COMLL_T_INCREMENT.

                        COMLL_ASSIGN => Assign saved value of the expression in
                                        cell[j        ,i] i.e. c2.

    SPECIAL CASES   :   If c2 is an atom of type real        , skip COMLL_ASSIGN
and place its value directly into **cells.
...
static COMLL_PTR join_adjacent_cells(COMLL_PTR c1, COMLL_PTR c2, COMLLType type,
                                     int i, int j, double **cells)

{
  ...
  /* Always ends the first cell COMLL by a POP comll to remove all value from
   * the stack */
  if (c1 != NULL) {
...
/* -------------------------------------------------------------------------------
   ...
   DESCRIPTION     :   Parse a particular cell (GrfnCell) in the GRFN tableau.
                       ...
                       GRFN_INPUT_STRING is set to the cell's SVAL field , and
  YYPARSE is called.  If YYPARSE sets the global variable GRFN_LANG_CUR_AMCELL
  to be SRT_YES (meaning this cell contains the "AM" qualifier)        , this
  fact is recorded in the STATUS fields of the GrfnDeal for the cell        ,
  row and the entire deal.
...
static Err parse_cell(GrfnDeal *gd, COMLL_PTR *icomptr) {
  ...
  case GRFNSCELL:
    ...
    /* If parser found PAY in the cell        , make sure nothing ele is hanging
     * around outside it in the cell */
    if (GrfnIsStatus(GrfnCGCell(gd).status, GRFNCSPAYREF)) {
      bot = comll_gotobot(icom);

      /* If the last element is not of the PAY.. type        , there is an
       * operation outside PAY */
      if ((bot->type != COMLL_F_PAYFROMPAST) && (bot->type != COMLL_F_PAY) &&
          (bot->type != COMLL_F_PAYINPAST))
        return serror("[GRFN Error] %s in c[%d        ,%d]", GRERR_PAY_ALONE,
                      gd->J, gd->I);
    }
...
/* ----------------------------------------------------------------------------------------
    ...
    DESCRIPTION     :   For each event date do the following:
                        ...
                        o   If all the GrfnCells in the current row are of
                            type GRFNBCELL (blank)        , set *new_event_ptr =
  NULL

                        o   If gd->american==SRT_NO        , we are actually at
  the current row (and not between it and the next one) Then for each cell do
  the following:

                                * Create a comll        (parse_cell)
                                * Join adjacent comlls  (join_adjacent_cells)

                        o   If gd->american==SRT_YES        , we are generating
  an event between two actual event dates. ONLY call yyparse for cells that
  actually contain AM.

                            Since we are heading forwards in time        , we
  must have evaluated this row before when gd->american SRT_NO. Hence we can
  look at the status fields corresponding to this row within the GrfnDeal in
                        ...
                        o   Set up all the df dates for this event
                                                ...
                                                o   This function is called for
  PAST and FUTURE events        , with the restriction that for PAST events ,
  the gd->american flag is set to SRT_NO

     Modification    :   Eric Fournie
                                                Modification de la liste comll
  grfn qui va maintenant contenir un  champ permettant de se deplacer par
  colonne lors de l'evaluation.
...
static Err create_event(GrfnEvent **new_event_ptr, GrfnDeal *gd) {
  ...
  if (gd->american == SRT_NO) {
    ...
    for (gd->J = 0; gd->J < gd->sswidth; gd->J++) {
      ...
      if (!(GrfnCGCell(gd).status[GRFNCSPASTREF]) &&
          (GrfnCGCell(gd).status[GRFNCSPVREF] ||
           GrfnCGCell(gd).status[GRFNCSFUTUREREF]))
        while (icom) {
      ...
      /* Sets the correspondiong flag if GRFNCSPVREF : COMLL_T_INCREMENT or
       * COMLL_T_SET */
      set_lattice_flg;

      /* Adds the current cell COMLL to the already existing one of the event
       * (for previous cells) */
      new_event->icom = join_adjacent_cells(new_event->icom, icom, lattice_flg,
                                            gd->I, gd->J, gd->cells);
   ...
   */
    ...
    for (gd->J = 0; gd->J < gd->sswidth; gd->J++) {
      /* Check if there is a call to AM in the cell ( it has already been
       * analysed ) */
      if (GrfnIsStatus(GrfnCGCell(gd).status, GRFNCSAMERICAN)) {
        /* 	Reparse the cell        , to duplicate the event on each
         * discretisation date */
        err = parse_cell(gd, &icom);
        if (err)
          return err;
        ...
        /* Joints the current cell to the previous ones        , taking this
         * flag into account */
        new_event->icom = join_adjacent_cells(
            new_event->icom, icom, lattice_flg, gd->I, gd->J, gd->cells);

      } else
          /* Make sure that there is a NULL cash-flow for empty cell if in the
             CashFlow (last) column */
  ...
  /* Set up and aggregates the dates (for events and df) in newly created event
   * for the domestic underlying first (index = 0) */
  und_index = 0;
...
/* -----------------------------------------------------------------------------------
    ...
    DESCRIPTION     :   When dealing with future events        ,
                        ...
                        GrfnDeal->I corresponds to the index of the previous
   event date ( == previous row of the tableau ) It is incremented only if the
   step date is on an EventDate
   -----------------------------------------------------------------------------------
 */

Err grfn_create_future_event(GrfnDeal *gd, GrfnEvent **event, double step_ddate,
                             double next_step_ddate) {
  Err error = NULL;
  double x;

  /* If the current step date is the same as the next event date in the Grfn
   * tableau */
  if (step_ddate == gd->event_dates[gd->I + 1]) {
    ...
    error = create_event(event, gd);
      ...
      /* If there is a call to AM in the previous row (already parsed) (if there
         is a previous row) */
      if ((gd->I >= 0) &&
          (GrfnIsStatus(gd->rowstatus[gd->I], GRFNCSAMERICAN))) {
    ...
    /* ReParse the Ith event stored in the GrfnDeal        , and returns an
     * evaluable one */
    error = create_event(event, gd);
  ...
  /* This is just a discretisation date : no event in the tableau        , no AM
     before : nothing */
  else {
    event = NULL;
  }
...
/* -----------------------------------------------------------------------------------
    ...
    DESCRIPTION     :   When dealing with historical events        , the
   creation ( and then the evaluation ) has to be done in order to set variables
   that might have an impact on future events (auxiliary variables: va        ,
   vb...)

                        For historical events        , the treatment of AM has
   to be specific: only for the event dates in the tableau so far

                                                The GrfnDeal->ssparam and
   GrfnDeal->cells will be populated accordingly !

                        When fixings are encountered        , they are going to
   be evaluated using the HistoricalFixings functions...

                        GrfnDeal->I corresponds to the index of the previous
   event date and  tableau row associated with the past date we are looking at
   -----------------------------------------------------------------------------------
 */

Err grfn_create_historical_event(GrfnDeal *gd, GrfnEvent **event,
                                 double step_date, double next_step_date) {
  Err error = NULL;
  ...
  if (step_date == gd->event_dates[gd->I + 1]) {
    ...
    /* The current step is on a discretised broken date ( during today's day
     * with e.o.d. flag on!): no event to attach */
    event = NULL;
  ...
  /* Create the COMLL from the tableau and attaches it to the corresponding
   * event */
  error = create_event(event, gd);
  if (error)
    return error;
...
/* -----------------------------------------------------------------------------------
    ...
    DESCRIPTION     :   After evaluating historical events        ,
   GrfnDeal->ssparams are populated with values (va        , vb...) implied by
   past events.

                        In order to keep track of these values that might impact
   on future cash flows        , this function copies the ssparams produced by
   the evaluation of historical events into the GrfnDeal->hist_ssparams.

                                                In order to force the use of
   these historical values        , a new RESET_STATE node will be atttached in
   the COMLL of today's GrfnEvent (passed as an input). If today's event does
   not exist (or is empty)        , it will be created with just this node in
   the COMLL
...
Err grfn_attach_past_info_to_future(GrfnDeal *gd, GrfnEvent **today_event) {
  ...
  /* If there are dates in the past        , and variables (va        , vb...)
   * have been referenced */
  if ((gd->first_unkn_index > 0) && GrfnIsStatus(gd->sumstatus, GRFNCSVARREF)) {
    /* Copies the SsParams produced by the past evaluation into the historical
     * ones for future use */
    memcpy(&gd->hist_ssparam, &gd->ssparam, sizeof(GrfnSSParam));

    /* If today's event is empty        , create a ZERO value one (for the POP()
     * at the end of eval_event) */
    if (!(*today_event)) {
    ...
    /* Add a node in the COMLL of today's event to force the copy of
     * hist_ssparam */

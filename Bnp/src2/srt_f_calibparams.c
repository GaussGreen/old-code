...
static Err srt_f_set_default_calib_params(
...
/* If no psCalibParamseters entered, numpsCalibParamss has to be 0 */
   for (i=0;i<iNumCalibParamss;i++)
   {
	...
	{
		if ( (!(strcmp (pszParamValues[i], "BARC"))) && (!(strcmp (pszParamValues[i], "BESTFIT")))
	...
	/* LGM iterations */
	if ((!strcmp(pszParamNames[i],"LGMITER")) || (!strcmp(pszParamNames[i],"NUMLGMITER")) ||
	...
	/* LGM Algorithm */
	if ((!strcmp(pszParamNames[i],"LGMALGO")) || (!strcmp(pszParamNames[i],"LGMALGORITHM")) ||
...
Err srt_f_set_FXCalibParams(String					*pszFXCalibStrings, 
   ...
   for (i=0;i<iNumCalibParamss;i++)
   {
	/* Algorithm */  
				...
				(!strcmp(pszFXCalibStrings[i],"CORRELCALIB")) )
		{
			...
			if (!strcmp(pszFXCalibValues[i],"NO"))
			{
				psFXCalibParams->bCalibCorrel = SRT_NO;
			}
			else

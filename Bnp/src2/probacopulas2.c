...
double RankToLinCorr(const double SpRho, const int degree, const int acc_degree)
{
    int    iter;
    ...
    case 0:
        return 2. * sin(SRT_PI * SpRho /
                        6.);  // if degree=0 returns the gaussian case (NB, it is a convention,
                              // since the gaussian is recovered for degree -> 00 )
        break;
 ...
 *                                                                                     *
 **************************************************************************************/
double CopulaRhoSpearman(const double r, const double SpRho, const int degree, const int acc_degree)
{
    double Val, Int;
    ...
    for (i = 1; i <= N_SMALL; i++)
    {
        for (j = 1; j <= N_SMALL; j++)
        {
            integrand = EvalStudCpl(
                -100. * acc_degree, xin[i], -100. * acc_degree, yin[j], r, n, acc_degree);
...
double EvalStudCplIntegrand(const double x, const double y, const double r, const int n)
{
    int    i;
    double A, value = 1.;
...
// Stud_Deg_Fit
//
// Calculates the MLE value of the degree of a bivariate Student copula and the corresponding linear
// correlation coefficient using an input data series with fitted generalized Student parameters
//
// Outputs:
//			m						the MLE value of the Student degree for the
//bivariate copula
//			linear_corr				the MLE value of the linear correlation
//parameter for the bivariate copula
//
// Inputs:
//			data1					input historical data of changes in forward
//rate 1.  First element is data[1]
//			m1						fitted degree of generalized Student
//distribution for data1 			sigma1					fitted variance parameter of
//generalized Student distribution for data1
//			data2					input historical data of changes in forward
//rate 2.  First element is data[1]
//			m2						fitted degree of generalized Student
//distribution for data1 			sigma2					fitted variance parameter of
//generalized Student distribution for data1
//			N						number of data points
//			Inv_Stud_acc			the accuracy in the inverse Student distribution
//calculation 			Max_Stud_Degree			maximum value of the Student degree tested for the
//MLE
//			RankToLin_acc			accuracy parameter for calculating the linear
//correlation from the rank correlation.  Uses the 										Kendall tau approximation for the linear
//correlation if the value is set to 0.
//
// Return Value:
//			none
//
...
//{
//    // Local variables.  Because some routines start from 0 and some from 1, we need a pointer to
...
//
//    // Calculate the cumulative probability densities for the input data.  It currently only works
//    // for integer values, so round.
...
//    }
//
//    // Compute the rank correlation.  Then loop over all possible values of the student degree,
//    // calculating the implied linear correlation and then the modified log-likelihood value.  Keep
//    // the maximum log-likelihood values.
...
//        temp_max_llhd =
//            Stud_cop_llhd(i, temp_linear_corr, CumDensity1, CumDensity2, N, Inv_Stud_acc);
...
// Stud_Bivar_Copula
//
// Calculates all the likelihood values for a range of degrees of a bivariate Student copula with
// the corresponding linear correlation coefficients determined by a fit to the rank correlation of
// an input data series with fitted Student parameters.
//
// Needs to be altered to allow for a fit with a generalized Student distribution
//
// Outputs:
//			m						the degree of the Student bivariate copula.
//First element stored in data[1].  Assumes that memory 										is declared elsewhere.
//			llhd					the likelihood value for the corresponding
//degree.  Same memory and storage conventions as m.
//			linear_corr				the linear correlation parameter for the
//corresponding degree.  Same memory and storage conventions 										as m.
//
// Inputs:
//			data1					input historical data of changes in forward
//rate 1.  First element is data[1]
...
//MLE
//			RankToLin_acc			accuracy parameter for calculating the linear
//correlation from the rank correlation
//
// Return Value:
//			none
//
    ...
    unsigned long        RankToLin_acc)
{
    ...
    // Compute the rank correlation.  Then loop over all possible values of the student degree,
    // calculating the implied linear correlation and then the modified log-likelihood value.  Store
    // the relevant values.
    ...
    for (i = 1; i <= Max_Stud_Degree; i++)
    {
        ...
        llhd[i + 1] =
            Stud_cop_llhd(i, linear_corr[i + 1], CumDensity1, CumDensity2, N, Inv_Stud_acc);
    }
...
//		   m		degree of the distribution (0 is Gaussian, otherwise can be any
//positive integer)
...
//    else
//        return gammln(0.5 * (m + 2.0)) + gammln(0.5 * m) - 2.0 * gammln(0.5 * (m + 1.0)) -
//               0.5 * log(det) -
//               0.5 * (m + 2.0) * log(1 + (x1 * x1 - 2.0 * rho * x1 * x2 + x2 * x2) / det / m) +
...
double Student_Density(double x, double m, double sigma)
{
    ...
    return exp(
        gammln(0.5 * (m + 1.0)) - gammln(0.5 * m) - 0.5 * log(SRT_PI * m * sigma2) -
...
//
// Inputs:
//			data				the ordered or unordered input data.  The first
//element is data[1].  Is unchanged by the function
//			k					k+1 is the number of upper order statistics
//used
//          N					set to 0 if the data is ordered, otherwise is set equal to the
...
// Stud_Log_Likelihd
//
// Returns the derivative of the log-likelihood function for the Student distribution and and its
// derivative
//
// Inputs:
//			sigma				the standard deviation parameter
//			data				An array of input data.  data[0] is the Student
//degree.  data[1] is the number of observations, 									while data[2] onwards gives the actual data
// Outputs:
//			f					the value of the derivative of the log-likelihood
//function
//			df					the value of the 2nd derivative of the
...
// Stud_Deg_Diff
//
// Returns the difference between the model and historical StDev for a given Student degree.  Also
// gives the derivative
//
// Inputs:
//			m					the Student degree parameter
//parameter
//			data				An array of input data.  data[0] is the historical
//StDev.  data[1] is the number of 									observations, while data[2] onwards gives the actual data
// Outputs:
//			f					the value of the function
//			df					the value of the derivative of the
//function
//
// Return Value:
//			Null if no error
...
{
    // Local variables and switch around the array data, such that it can be used to call the
    // log-likelihood function
    Err    err       = NULL;
...
// The output vectors are assumed to already have memory allocated and run from 1 to N in good NR
// fashion
//
void Student_Simluation(
    double*            data1,
    double*            data2,
...
//		ABSCube( Unif_dev, 0, p-1, 0, eff_d-1, 0, 0, &idum );
                ...
                CholDec	(&CholAlg,
                                 corr_mtx,       // performs Choleski decomposition on the corr.
matrix sqrt_corr_mtx, d-degree);
...
chi2+=GaussSample[i+p][j]*GaussSample[i+p][j];
                        ...
                        for (j=0; j<d-degree; j++) {
                                if (degree > 0) {           // checks if we are in the gauassian
copula case (degree = 0) IntermRes=sqrt(degree/chi2)*res[i+p][j]; } else {                    // we
are in the gaussian copula case IntermRes=res[i+p][j];
                                }
...
// computes a draw of the Student variable
                        ...
                        for (j=0; j<d-degree; j++) {
                                if (degree > 0) {           // checks if we are in the gauassian
copula case (degree = 0) IntermRes=sqrt(degree/chi2)*res[i][j]; } else {                    // we
are in the gaussian copula case IntermRes=res[i][j];
                                }
...

{
    ...
    double chi2 = 0.0, ***GaussSample = NULL, **UnifSample = NULL, **sqrt_corr_mtx = NULL,
           IntermRes, q;
    ...
    case ABS:  // uses ABS cube
        ...
        for (i = 0; i < p; i++)
        {
            ...
            {
                if (degree > 0)
                {  // checks if we are in the gauassian copula case (degree = 0)
                    IntermRes = sqrt(degree / chi2) * res[i][j];
                }
                else
                {  // we are in the gaussian copula case
                    IntermRes = res[i][j];
                }
    ...
    case RANDOM_GAUSS:
        ...
        gauss_matrix(
            UnifSample, 0, p, 0, d - 1, &idum);  // note that in this case the function returns a

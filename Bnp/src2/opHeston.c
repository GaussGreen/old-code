    ...
    double*        result)
{
    ...
    case PREMIUM:
        ...
        case 1:
            ...
            ImAppr2 =
                -(log(Forward) + (esp2 - 1) * Sigma * Sigma / 2. / (b - Alpha * Rho) +
                  Maturity * a / 2. / (b - Alpha * Rho) +
                  a * (esp2 - 1.) / 2 / (b - Alpha * Rho) / (b - Alpha * Rho));
            ImAppr1 =
                -(log(Forward) + Sigma * Sigma / b * (1. - esp) - Maturity * a / 2. / b +
                  a / 2. / b / b * (1. - esp));
            ...
            for (j = 1; j <= nStrikes; j++)
            {
                ...
                do
                {
                    ...
                    NodeRight1 = HestonFindRightNode(
                        ImAppr2,
                        Strike[j],
                        NodeLeft,
                        isVolInfFix,
                    ...
                    for (k = 1; k <= nStp; k++)
                    {
                        ...
                        G22[j] +=
                            w1[k] / x1[k] * exp(RePsi2) * sin(ImPsi2 + x1[k] * log(Strike[j]));
                    }
            ...
            for (j = 1; j <= nStrikes; j++)
            {
                ...
                default:
                    result[j] =
                        DMAX(1.e-13, (n1 - Strike[j] * n2) * Disc - (Forward - Strike[j]) * Disc);
        ...
        case 0:
            ...
            /*
                                    ...
                                    PsiFunction
               (isVolInfFix,Maturity,1.,-UpperBound,a,b,Alpha,Rho,log(Forward),Sigma*Sigma,greek,&ReUpBound,&ImUpBound,&DRePsi1,&DImPsi1,&DDRePsi1,&DDImPsi1);
                                    PsiFunction
               (isVolInfFix,Maturity,1.,0.0,a,b,Alpha,Rho,log(Forward),Sigma*Sigma,greek,&ReLowBound,&ImLowBound,&DRePsi1,&DImPsi1,&DDRePsi1,&DDImPsi1);

                                    ReAppr1=(ReUpBound-ReLowBound)/(UpperBound-x[2]);
                                    ReAppr1c=ReLowBound;

                                    PsiFunction
               (isVolInfFix,Maturity,0.,-UpperBound,a,b,Alpha,Rho,log(Forward),Sigma*Sigma,greek,&ReUpBound,&ImUpBound,&DRePsi1,&DImPsi1,&DDRePsi1,&DDImPsi1);
                                    PsiFunction
               (isVolInfFix,Maturity,0.,0.0,a,b,Alpha,Rho,log(Forward),Sigma*Sigma,greek,&ReLowBound,&ImLowBound,&DRePsi1,&DImPsi1,&DDRePsi1,&DDImPsi1);
        ...
        case 3:
            ...
            for (j = 1; j <= nStrikes; j++)
            {
                ...
                // Here it branches following the rule: if Strike is larger than nStdDev from the
                // forward or smaller than -nStdDev, then it simply evaluates the price at +-
                // nStdDev

                dGear      = 2 + 3. / (Maturity * Maturity * Maturity);
                nOptSteps1 = IMAX(
                    IMIN(
                        (int)(dGear * UpperBound * (fabs(ImAppr1 + log(Strike[j]))) / SRT_PI),
                        nSteps),
                    22);
                nOptSteps2 = IMAX(
                    IMIN(
                        (int)(dGear * UpperBound * (fabs(ImAppr2 + log(Strike[j]))) / SRT_PI),
                        nSteps),
                    22);
        ...
        default:
            ...
            /* now I integrate with the optimised algorithm for strikes < breakpoint and with the
               parallel algorithm for strikes  > breakpoint */
    ...
    case GAMMA:
        ...
        for (i = 1; i <= nSteps; i++)
        {
            ...
            for (j = 1; j <= nStrikes; j++)
            {
                ...
                G12[j] +=
                    w[i] / x[i] * exp(RePsi1) *
                    (SinT1 * DRePsi1 * DRePsi1 + CosT1 * DImPsi1 * DRePsi1 + DDRePsi1 * SinT1 +
                     DRePsi1 * DImPsi1 * CosT1 - DImPsi1 * DImPsi1 * SinT1 + DDImPsi1 * CosT1);
                G22[j] +=
                    w[i] / x[i] * exp(RePsi2) *
                    (SinT2 * DRePsi2 * DRePsi2 + CosT2 * DImPsi2 * DRePsi2 + DDRePsi2 * SinT2 +
            ...
            for (j = 1; j <= nStrikes; j++)
            {
                ...
                do
                {
                    ...
                    for (k = 1; k <= nStp; k++)
                    {
                        ...
                        G12[j] +=
                            w[k] * exp(RePsi1) * cos(ImPsi1 + x[k] * log(Strike[j])) / Strike[j];
                        ...
                        G22[j] +=
                            w1[k] * exp(RePsi2) * cos(ImPsi2 + x1[k] * log(Strike[j])) / Strike[j];
                        GKK[j] +=
                            w1[k] / x1[k] * exp(RePsi2) * sin(ImPsi2 + x1[k] * log(Strike[j]));
                    }
...

{
    double Phi1, Phi2, Phi3, Phi4, Psi1, Psi2, Psi3, Psi4, NumRe, NumIm, DenRe, DenIm, Chi, Den,
        DDen, PhiG, PsiG, PhiG2, PsiG2, Phi3G, Psi3G, SinT, Sin2T, CosT, Cos2T,
        U2 = U * U, U3 = U * U * U, V2 = V * V, V3 = V * V * V, r = 0.0, NumRe2, NumIm2, Phi2G,
        Psi2G, Esp, Esp2, DNumRe, DNumIm, DLogArg;
    double alpha2 = alpha * alpha, alpharho = alpha * rho, alpharho2 = alpharho * alpharho,
           ...
           Ualpharho = U * alpharho, Valpharho = V * alpharho, PowG, ATanG, VPhiG, VPsiG,
           Den2ReDen2Im, SinTEsp, CosTEsp, Sin2TEsp2, Cos2TEsp2;

    Err err = NULL;

    //  Defines the Re and Im part of the square of the function gamma

    PhiG2 = b * b + alpha2 * (U - U2 + V2) - 2 * b * Ualpharho + alpharho2 * (U2 - V2);
    ...
    DenRe =
        b - Ualpharho + PhiG - Esp * (CosT * (b - Ualpharho - PhiG) - SinT * (Valpharho + PsiG));

    DenIm = -Valpharho + PsiG + Esp * (CosT * (Valpharho + PsiG) + SinT * (b - Ualpharho - PhiG));
    ...
    NumRe = PhiG * (b - Ualpharho) - Phi2G + 2 * Chi - Valpharho * PsiG - Psi2G -
            Esp * (CosT * (PhiG * (b - Ualpharho) - Phi2G - Valpharho * PsiG - Psi2G) -
                   SinT * (Valpharho * PhiG + PsiG * (b - Ualpharho)));

    NumIm = -Valpharho * PhiG - PsiG * (b - Ualpharho) +
    ...
    *ImPsi = Psi1 + Psi2 + Psi3 + Psi4;
    ...
    case VEGA:
        ...
        NumRe =
            ...
            U3 * x2 * alpha * rho + V2 * x2 * alpha * rho - U * V2 * x2 * alpha * rho -
            U * x2 * PhiG + U2 * x2 * PhiG - V2 * x2 * PhiG - V * x2 * PsiG +
            2 * U * V * x2 * PsiG + (2 * b * U * x2 * CosT) * Esp - (2 * b * U2 * x2 * CosT) * Esp +
            (2 * b * V2 * x2 * CosT) * Esp - (2 * U2 * x2 * alpha * rho * CosT) * Esp +
            (2 * U3 * x2 * alpha * rho * CosT) * Esp - (2 * V2 * x2 * alpha * rho * CosT) * Esp +
            (2 * U * V2 * x2 * alpha * rho * CosT) * Esp - (b * U * x2 * Cos2T) * Esp2 +
            (b * U2 * x2 * Cos2T) * Esp2 - (b * V2 * x2 * Cos2T) * Esp2 +
            (U2 * x2 * alpha * rho * Cos2T) * Esp2 - (U3 * x2 * alpha * rho * Cos2T) * Esp2 +
            (V2 * x2 * alpha * rho * Cos2T) * Esp2 - (U * V2 * x2 * alpha * rho * Cos2T) * Esp2 +
            (U * x2 * PhiG * Cos2T) * Esp2 - (U2 * x2 * PhiG * Cos2T) * Esp2 +
            (V2 * x2 * PhiG * Cos2T) * Esp2 + (V * x2 * PsiG * Cos2T) * Esp2 -
            (2 * U * V * x2 * PsiG * Cos2T) * Esp2 + (2 * V * x2 * PhiG * SinT) * Esp -
            (4 * U * V * x2 * PhiG * SinT) * Esp - (2 * U * x2 * PsiG * SinT) * Esp +
            (2 * U2 * x2 * PsiG * SinT) * Esp - (2 * V2 * x2 * PsiG * SinT) * Esp -
            (b * U * x2 * Sin2T) * Esp2 + (b * U2 * x2 * Sin2T) * Esp2 -
            (b * V2 * x2 * Sin2T) * Esp2 + (U2 * x2 * alpha * rho * Sin2T) * Esp2 -
            (U3 * x2 * alpha * rho * Sin2T) * Esp2 + (V2 * x2 * alpha * rho * Sin2T) * Esp2 -
            (U * V2 * x2 * alpha * rho * Sin2T) * Esp2 + (U * x2 * PhiG * Sin2T) * Esp2 -
            (U2 * x2 * PhiG * Sin2T) * Esp2 + (V2 * x2 * PhiG * Sin2T) * Esp2 +
            (V * x2 * PsiG * Sin2T) * Esp2 - (2 * U * V * x2 * PsiG * Sin2T) * Esp2;

        NumIm = -(b * V * x2) + 2 * b * U * V * x2 - U2 * V * x2 * alpha * rho -
                V3 * x2 * alpha * rho - V * x2 * PhiG + 2 * U * V * x2 * PhiG + U * x2 * PsiG -
                U2 * x2 * PsiG + V2 * x2 * PsiG + (2 * b * V * x2 * CosT) * Esp -
                (4 * b * U * V * x2 * CosT) * Esp + (2 * U2 * V * x2 * alpha * rho * CosT) * Esp +
                (2 * V3 * x2 * alpha * rho * CosT) * Esp - (b * V * x2 * Cos2T) * Esp2 +
                (2 * b * U * V * x2 * Cos2T) * Esp2 - (U2 * V * x2 * alpha * rho * Cos2T) * Esp2 -
                (V3 * x2 * alpha * rho * Cos2T) * Esp2 + (V * x2 * PhiG * Cos2T) * Esp2 -
                (2 * U * V * x2 * PhiG * Cos2T) * Esp2 - (U * x2 * PsiG * Cos2T) * Esp2 +
                (U2 * x2 * PsiG * Cos2T) * Esp2 - (V2 * x2 * PsiG * Cos2T) * Esp2 -
                (2 * U * x2 * PhiG * SinT) * Esp + (2 * U2 * x2 * PhiG * SinT) * Esp -
                (2 * V2 * x2 * PhiG * SinT) * Esp - (2 * V * x2 * PsiG * SinT) * Esp +
                (4 * U * V * x2 * PsiG * SinT) * Esp - (b * V * x2 * Sin2T) * Esp2 +
                (2 * b * U * V * x2 * Sin2T) * Esp2 - (U2 * V * x2 * alpha * rho * Sin2T) * Esp2 -
                (V3 * x2 * alpha * rho * Sin2T) * Esp2 + (V * x2 * PhiG * Sin2T) * Esp2 -
                (2 * U * V * x2 * PhiG * Sin2T) * Esp2 - (U * x2 * PsiG * Sin2T) * Esp2 +
                (U2 * x2 * PsiG * Sin2T) * Esp2 - (V2 * x2 * PsiG * Sin2T) * Esp2;

        DenRe = b - U * alpha * rho + PhiG - (b * CosT) * Esp + (U * alpha * rho * CosT) * Esp +
                (PhiG * CosT) * Esp + (V * alpha * rho * SinT) * Esp + (PsiG * SinT) * Esp;

        DenIm = -(V * alpha * rho) + PsiG + (V * alpha * rho * CosT) * Esp + (PsiG * CosT) * Esp +
                (b * SinT) * Esp - (U * alpha * rho * SinT) * Esp - (PhiG * SinT) * Esp;
    ...
    */
        ...
        if (isVolInfFix)
        {
            ...
            NumRe2 = b * PhiG - U * alpha * rho * PhiG - Phi2G + 2 * Chi - V * alpha * rho * PsiG -
                     Psi2G - (b * PhiG * CosT) * Esp + (U * alpha * rho * PhiG * CosT) * Esp +
                     (Phi2G * CosT) * Esp + (V * alpha * rho * PsiG * CosT) * Esp +
                     (Psi2G * CosT) * Esp + (V * alpha * rho * PhiG * SinT) * Esp +
                     (b * PsiG * SinT) * Esp - (U * alpha * rho * PsiG * SinT) * Esp;
            ...
            Phi4 =
                -a / alpha / alpha * log(NumRe2 * NumRe2 / Den / Den + NumIm2 * NumIm2 / Den / Den);
    ...
    case THETA:
            ...
            (2 * x2 *
             (PhiG *
                  (b * U - b * U2 + b * V2 - U2 * alpha * rho + U3 * alpha * rho -
                   V2 * alpha * rho + U * V2 * alpha * rho - U * PhiG + U2 * PhiG - V2 * PhiG -
                   V * PsiG + 2 * U * V * PsiG) *
                  Esp2 +

              (b * (-U + U2 - V2) * PhiG + U2 * alpha * rho * PhiG - U3 * alpha * rho * PhiG +
               V2 * alpha * rho * PhiG - U * V2 * alpha * rho * PhiG + V * PhiG * PsiG -
               2 * U * V * PhiG * PsiG - U * Psi2G + U2 * Psi2G - V2 * Psi2G) *
                  Esp * CosT +

              Esp *
                  (-(V * Phi2G) + 2 * U * V * Phi2G - b * U * PsiG + b * U2 * PsiG - b * V2 * PsiG +
                   U2 * alpha * rho * PsiG - U3 * alpha * rho * PsiG + V2 * alpha * rho * PsiG -
                   U * V2 * alpha * rho * PsiG + U * PhiG * PsiG - U2 * PhiG * PsiG +
                   V2 * PhiG * PsiG) *
                  SinT)); /* OK*/
        ...
        DNumIm =
            (2 * x2 *
             (PhiG *
                  (b * V - 2 * b * U * V + U2 * V * alpha * rho + V3 * alpha * rho - V * PhiG +
                   2 * U * V * PhiG + U * PsiG - U2 * PsiG + V2 * PsiG) *
                  Esp2 +

              Esp *
                  (b * (-1 + 2 * U) * V * PhiG - U2 * V * alpha * rho * PhiG -
                   V3 * alpha * rho * PhiG - U * PhiG * PsiG + U2 * PhiG * PsiG - V2 * PhiG * PsiG -
                   V * Psi2G + 2 * U * V * Psi2G) *
                  CosT -

              Esp *
                  (-(U * Phi2G) + U2 * Phi2G - V2 * Phi2G + b * V * PsiG - 2 * b * U * V * PsiG +
                   U2 * V * alpha * rho * PsiG + V3 * alpha * rho * PsiG - V * PhiG * PsiG +
                   2 * U * V * PhiG * PsiG) *
                  SinT)); /* OK */
        ...
        DDen =
            (-2 *
             (PhiG *
                  (b * b - 2 * b * U * alpha * rho + U2 * alpha * alpha * rho * rho +
                   V2 * alpha * alpha * rho * rho - 2 * b * PhiG + 2 * U * alpha * rho * PhiG +
                   Phi2G + 2 * V * alpha * rho * PsiG + Psi2G) *
                  Esp2 +

              Esp *
                  (-(b * b * PhiG) + 2 * b * U * alpha * rho * PhiG -
                   U2 * alpha * alpha * rho * rho * PhiG - V2 * alpha * alpha * rho * rho * PhiG +
                   Phi3G - 2 * V * alpha * rho * PhiG * PsiG - 2 * b * Psi2G +
                   2 * U * alpha * rho * Psi2G + PhiG * Psi2G) *
                  CosT +

              Esp *
                  (2 * V * alpha * rho * Phi2G - b * b * PsiG + 2 * b * U * alpha * rho * PsiG -
                   U2 * alpha * alpha * rho * rho * PsiG - V2 * alpha * alpha * rho * rho * PsiG +
                   2 * b * PhiG * PsiG - 2 * U * alpha * rho * PhiG * PsiG + Phi2G * PsiG + Psi3G) *
                  SinT)); /* OK */
        ...
        NumRe = b * PhiG - U * alpha * rho * PhiG - Phi2G + 2 * Chi - V * alpha * rho * PsiG -
                Psi2G - (b * PhiG * CosT) * Esp + (U * alpha * rho * PhiG * CosT) * Esp +
                (Phi2G * CosT) * Esp + (V * alpha * rho * PsiG * CosT) * Esp +
                (Psi2G * CosT) * Esp + (V * alpha * rho * PhiG * SinT) * Esp +
                (b * PsiG * SinT) * Esp - (U * alpha * rho * PsiG * SinT) * Esp; /* OK */

        NumIm = -(V * alpha * rho * PhiG) - b * PsiG + U * alpha * rho * PsiG +
                (V * alpha * rho * PhiG * CosT) * Esp + (b * PsiG * CosT) * Esp -
                (U * alpha * rho * PsiG * CosT) * Esp + (b * PhiG * SinT) * Esp -
                (U * alpha * rho * PhiG * SinT) * Esp - (Phi2G * SinT) * Esp -
                (V * alpha * rho * PsiG * SinT) * Esp - (Psi2G * SinT) * Esp; /* OK */
        ...
        Phi4 =
            -a / alpha / alpha / (NumRe * NumRe / Den / Den + NumIm * NumIm / Den / Den) * DLogArg;

        DNumRe = -((Phi2G + Psi2G) *
                   ((-b + U * alpha * rho + PhiG) * CosT + (V * alpha * rho + PsiG) * SinT)) *
                 Esp; /*OK */
        DNumIm = ((Phi2G + Psi2G) *
                  (-((V * alpha * rho + PsiG) * CosT) + (-b + U * alpha * rho + PhiG) * SinT)) *
                 Esp; /* OK */
    ...
    SrtCalibrationType CalibrType)
{
    ...
    */
    ...
    KMin =
        Forward * exp(-0.5 * dApproxStdev * dApproxStdev - nStdDev * dApproxStdev);  // modifier....
    ...
    slope = (BetaSABR - 1) * ATMVol / Forward *
            (1 - 0.5 / (1 + (1 - BetaSABR) * (1 - BetaSABR) / 24 * ATMVol * ATMVol * Maturity));
    volshift     = 2 * slope * Forward + ATMVol;
    *ShiftHeston = ATMVol * Forward / volshift * (1 - ATMVol * ATMVol * Maturity / 24) /
    ...
    */
    ...
    case MATCH_CONV:
        ...
        while ((alphamax - alphamin > precalpha) && (current_iter < n_iter_max))
        {
            ...
            err = srt_f_optimpvol(
                Prices[1], Forward, Strikes[1], Maturity, 1, SRT_CALL, SRT_LOGNORMAL, &(Vols[1]));
            err = srt_f_optimpvol(
                Prices[2], Forward, Strikes[2], Maturity, 1, SRT_CALL, SRT_LOGNORMAL, &(Vols[2]));
    ...
    double* shift)
{
    ...
    slope = (BetaSABR - 1) * ATMVol / Forward *
            (1 - 0.5 / (1 + (1 - BetaSABR) * (1 - BetaSABR) / 24 * ATMVol * ATMVol * Maturity));
    volshift = 2 * slope * Forward + ATMVol;
    *shift   = ATMVol * Forward / volshift * (1. - ATMVol * ATMVol * Maturity / 24.) /
                 (1. - volshift * volshift * Maturity / 24.) -
             Forward;
    ...
    double*          NewVols)
{
    ...
    case LOG_TO_HESTON:
        ...
        {
            BSPremium = srt_f_optblksch(Forward, Strikes[i], Vol, Maturity, 1.0, SRT_CALL, PREMIUM);

            fl = result_low[i] - BSPremium;
            fh = result_high[i] - BSPremium;

            if ((fl > 0.0 && fh > 0.0) ||
                (fl < 0.0 && fh < 0.0)) /* Throw exception: not bracketed */
                ;
    ...
    case HESTON_TO_LOG:
        ...
        {
            err = srt_f_optimpvol(
                result[j], Forward, Strikes[j], Maturity, 1.0, SRT_CALL, SRT_LOGNORMAL, &ImplVol);
    ...
    case HESTON_TO_NORMAL:
        ...
        {
            err = srt_f_optimpvol(
                result[j], Forward, Strikes[j], Maturity, 1.0, SRT_CALL, SRT_NORMAL, &ImplVol);
    ...
    case NORMAL_TO_HESTON:
        ...
        for (i = 1; i <= nStrikes; i++)
        {
            BSPremium = srt_f_optblknrm(Forward, Strikes[i], Vol, Maturity, 1.0, SRT_CALL, PREMIUM);
    ...
    case NORMAL_TO_LOG:
        ...
        for (j = 1; j <= nStrikes; j++)
        {
            ...
            err = srt_f_optimpvol(
                BSPremium, Forward, Strikes[j], Maturity, 1.0, SRT_CALL, SRT_LOGNORMAL, &ImplVol);
    ...
    case LOG_TO_NORMAL:
        ...
        for (j = 1; j <= nStrikes; j++)
        {
            ...
            err = srt_f_optimpvol(
                BSPremium, Forward, Strikes[j], Maturity, 1.0, SRT_CALL, SRT_NORMAL, &ImplVol);

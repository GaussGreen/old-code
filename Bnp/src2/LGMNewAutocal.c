...
strike. If deal is strictly positive or strictly negative  , ratio is set to 0
*/
...
/**** Comments ****/
/* Outputs */
/* double LGMValue  , double intrinsicValue be allocated in the calling routine
and their addresses passed to LGMnewautocal as LGMValPtr and intrinValPtr to
receive the value and intrinsic value of the deal
...
passed to LGMnewautocal On exit  , *ExerIntoPtrPtr and *ExerBdryPtrPtr will be
the first element of arrays of LGMSwptns (exer date  , final paydate  , fixed
rate) which are most similar to the underlying midatlantic and define the
exercise boundary  , respectively. *LGMtsPtrPtr will point to the calibrated
zeta-G term structure  , and if convertTS is on  , *SigKaptsPtrPtr will point to
the equivalent sigma-kappa term structure
*/
  ...
  for (k = exfreqmax; k >= 1; k--) {
    ...
    MidAtPtr =
        LGMCreateMidAtFromAmer(/* create MidAt with k months between exercises
                                */
                               tfirst,
                               rflt_current, ycName, &conventions, k, dealPtr);
    if (MidAtPtr != NULL) {
      error = LGMnewautocal(
    ...
    SrtSimAmer *AmerPtr) /* the American deal */
{
  ...
  /* start dates are tLastDate - n*dmos  , n=nfirst  ,...  , nlast (plus
   * adjustment for start & end) */
    ...
    SrtBasisCode basis)                      /* basis for rates */
{
  ...
  for (i = 1; i <= n; i++) /* find PV of coupon leg */
  {
    ...
    /* Vfloor = LGMCEVCapletGreek(SRT_RECEIVER  ,Forward  ,a[i]  ,Level
     * ,Yrtoexp  ,CEVvol  , beta  ,PREMIUM); */

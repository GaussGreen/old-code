...
static void testcompprice(LGM_TS* tsPtr, Date tNow, String ycname, SrtSimMidAt* dealPtr);
...
LGMErr LGMNewAmerican(
    ...
    String szRefRate) /* previously output file name for log file (unused):  now refrate for MUNI
                         case */
{
    /* declarations */
    LGMErr       error  = NULL;
...
starthere: /* for debugging */
    ...
    /* if a non-null refrate is passed in, set the defaults based on it, otherwise use the currency
     * defaults */
    ...
    for (k = exfreqmax; k >= 1; k--)
    {
        ...
        if (MidAtPtr != NULL)
        {
            error = LGMnewautocal(
                ...
                CalReq,
                EvalParms, /* calib & eval methods */
                &(MidAtValue[k]),
                intrinValPtr, /* value & intrinsic value of deal */
                ExerIntoPtrPtr,
                ExerBdryPtrPtr, /* swaptions representing underlying & exer boundary */
                lgmRefDeals,    /* reference swaptions */
                LGMtsPtrPtr,
                SigKaptsPtrPtr, /* calibrated term structures */
                szRefRate);     /* unused */
    ...
    SrtSimAmer*  AmerPtr)  /* the American deal */
{
    ...
    else
    {
        ...
        tlastStart =
            StFromEx(tlastEx, lag, calbus, bdconv); /* no value to exercising after last fixing */
        tlastStart =
            min(tlastStart, add_unit(AmerPtr->tfixEnd[nfix - 1], -1, SRT_MONTH, SUCCEEDING));
...

{
    ...
    case GenEur: /* Compute intrinsic value and PV ratio */
        nPay  = EurDeal->nPay;
        error = CompValGen(tNow, ycName, &ratio, &intVal, nPay, EurDeal->tPay, EurDeal->Payment);
        if (error != NULL)
        {
            srt_free(TauArr);
            srt_free(FVArr);
    ...
    if (dealtype != CallInvFlt && dealtype != CallTimeSwap)
    {
        for (j = 0; j < nEx; j++)
        {
            error = LGMGetFixRate(ExerInto->tEx[j], ExerInto->tEnd[j], FVArr[j + 1], ycName, &Rfix);
            if (error != NULL)
            {
                srt_free(TauArr);
                srt_free(FVArr);
    ...
    SrtBasisCode    basis)       /* basis for rates */
{
    ...
    for (i = 1; i <= n; i++) /* find PV of coupon leg */
    {
        ...
        Vfloor =
            LGMCEVCapletPrice(tNow, tEx, SRT_RECEIVER, a[i], cvgCap, dfSt, dfEnd, CEVvol, beta);
        if (Vfloor < 0.0)

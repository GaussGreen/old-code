...
static LGMErr CompValOpt(Date tNow, String ycName, double *ratio,
						 double *intValPtr, SrtReceiverType PayRec,
						 Date tStart, double Strike, double redfirstPay,
						 long nPay, Date *tPay, double *Payment);

/*	Returns intrinsic value and the ratio PV of cpn leg to  PV of the strike.
If deal is strictly positive or strictly negative, ratio is set to 0 */
static LGMErr CompValInvFlt(
	...
	double *a, double *marg, double lvg,			/* handle, margin, leverage */
...
/**** Comments ****/
/* Outputs */
/* double LGMValue, double intrinsicValue be allocated in the calling routine and
their addresses passed to LGMnewautocal as LGMValPtr and intrinValPtr to receive
the value and intrinsic value of the deal

The pointers *ExerIntoPtrPtr, *ExerBdryPtrPtr, *LGMtsPtrPtr, *SigKaptsPtrPtr
must be allocated in the calling routine and their addresses passed to LGMnewautocal
On exit, *ExerIntoPtrPtr and *ExerBdryPtrPtr will be the first element of arrays of
LGMSwptns (exer date, final paydate, fixed rate) which are most similar to the
underlying midatlantic and define the exercise boundary, respectively. *LGMtsPtrPtr
will point to the calibrated zeta-G term structure, and if convertTS is on,
*SigKaptsPtrPtr will point to the equivalent sigma-kappa term structure
*/
...
/* outputs */
		...
		String	szRefRate)			/* previously output file name for log file (unused):  now refrate for MUNI case */
{
/* declarations */
	LGMErr error = NULL;
...
/* if a non-null refrate is passed in, set the defaults based on it, otherwise use the currency defaults */
...
then with one month between exercise dates, and then extrapolate to find the
value of the American option */
	...
	{	if (k==exfreqmax)
		...
		MidAtPtr = LGMCreateMidAtFromAmer(			/* create MidAt with k months between exercises */
					tfirst, rflt_current, ycName, &conventions,
					k, dealPtr); 
		if (MidAtPtr!=NULL)
		{	error = LGMnewautocal(
					...
					ExerIntoPtrPtr, ExerBdryPtrPtr,			/* swaptions representing underlying & exer boundary */
...
/*****************************************************************************************/    
/* MAIN PROGRAM to calib zeta(t) & G(t) and evaluate a mid_atlantic or Bermudan Inv Flter*/
/*****************************************************************************************/    
...
/* find first exercise and start dates */
	...
	tfirstEx = ExFromSt(tfirstStart, lag, calbus, bdconv);	/* no reason to exercise earlier */
	tfirstEx = max(tfirstEx, AmerPtr->tFirstExer);			/* cannot exercise before allowed */
...
/* find last exercise and start dates */
	...
	{	tlastEx = AmerPtr->tfltFixing[nflt-1];
		tlastStart = StFromEx(tlastEx, lag, calbus, bdconv);/* no value to exercising after last fixing */
		tlastStart = min(tlastStart, add_unit(AmerPtr->tfixEnd[nfix-1], -1, SRT_MONTH, SUCCEEDING));
		tlastEx = ExFromSt(tlastStart, lag, calbus, bdconv);
		if (tlastEx<=tfirstEx || tlastStart<=tfirstStart)	/* effectively a vanilla swaption */
		{	tlastEx = tfirstEx;
			tlastStart = tfirstStart;
		}
	}
...
/* find start (upon exercise) dates */
	...
	n=1;
	while (n<=601 && tlastStart <= add_unit(tLastDate, -n*dmos, SRT_MONTH, conv->sbdconv))
		n++;
	nlast = n-1;
	while (n<=601 && tfirstStart < add_unit(tLastDate, -n*dmos, SRT_MONTH, conv->sbdconv))
		n++;
	nfirst = n;
	nEx = nfirst - nlast + 1;								/* this number may be reduced later */

/* start dates are tLastDate - n*dmos, n=nfirst,..., nlast (plus adjustment for start & end) */
...
/* find start (upon exercise) dates */
	for (n=nfirst; n>=nlast; n--)
		tStart[nfirst-n] = add_unit(tLastDate, -n*dmos, SRT_MONTH, conv->sbdconv);
...
/* adjust to match schedule (if possible) */
	...
	{	adjusted=0;
		for (; j<nfix &&
				AmerPtr->tfixPay[j] < tStart[i] - 10; j++)
				;
		...
		for (; k<nflt &&
				AmerPtr->tfltPay[k] < tStart[i] - 10; k++)
				;
...
/* first payment, strike */
	...
	{	for ( ; AmerPtr->tfixEnd[j] <= tStart[i]; j++)
		...
		RedFirstPay[i] = 0.;
		reduction = AmerPtr->fixCoupon[j]			/* compute accrued to settlement date */
		...
		if (AmerPtr->EarlyFlagFlt==0 && AmerPtr->ResetFlt==0 && rflt>0)
			Strike[i] = Strike[i] - 1.0 +
						(1.0 + rflt*coverage(tStart[i], tb, AmerPtr->fltBasis))* dfb/dfSt;
		else if (AmerPtr->EarlyFlagFlt!=0 && rflt>0)
			Strike[i] = Strike[i] - 1.0 - rflt*coverage(ta, tStart[i], AmerPtr->fltBasis)
...
/* output variables */
	...
	double *FVArr;				/* tlast, nEx, TauArr and FVArr go on to the calibrator */
	LGMSwptns *ExerInto; 		/* Underlying swaptions, returned to calling code */
...
/* allocate space */
	...
	case GenEur:						/* Compute intrinsic value and PV ratio */
		nPay = EurDeal->nPay;		
		error = CompValGen(tNow, ycName, &ratio, &intVal, 
							nPay, EurDeal->tPay, EurDeal->Payment);
		if (error!=NULL)
		{	srt_free(TauArr); srt_free(FVArr);
			LGMFreeSwptns(&ExerInto);
			return (error);
	...
	case SimpleMidAt:
		...
		{	if (tEx[j] >= nextdate)					/* which are at least 20 days apart */
			{	nEx++;
				...
				error = CompValOpt(
							tNow, ycName, &ratio, &tempIntVal,
							PayRec, tStart[j], Strike[j],
							redpay[j], nPay-i,
							&(tPay[i]), &(Payment[i]));
				...
				if (tempIntVal>intVal)		/* If this intrinsic value is larger, replace */
	...
	case GenMidAt:
		...
		{	if (tEx[j] >= nextdate)					/* which are at least 20 days apart */
			{	nEx++;
				...
				error = CompValGen(
							tNow, ycName, &ratio, &tempIntVal,	/* Compute PV ratio and intrinsic val */
		...
		{											/* which are at least 20 days apart */
			...
			{	
				...
				error = CompValOpt(
					...
					&ratio,
					&tempIntVal,
					BerInvFltDeal->PayRec,
					BerInvFltDeal->tCpnStart[i],
					BerInvFltDeal->strike[j],
					0.0,
					BerInvFltDeal->nCpn-i,
					&(BerInvFltDeal->tCpnPay[i]),
					&(BerInvFltDeal->a[i]));
	...
	case CallTimeSwap:
		...
		error = CompVal_ForwardsTimeSwaps(
						CallTimeSwapDeal,
						tNow,
						ycName,
						FVArr,
						&intVal,
						CallTimeSwapDeal->tForwardTS[0],
...
/*
		...
		{											
			...
			{	
				...
				CallTimeSwapDeal->tForwardTS[0][j] = tempIntVal;
				...
				if (1==nEx)
				{
					CallTimeSwapDeal->pv_fixedleg = tempIntVal;
				}
...
/* Compute PV ratio and intrin val */											
				...
				error = CompValCapFlt(
							...
							PayRec, tCpn[i], Strike[j],		/* pay/rec, settlement date, strike */
							nPay-i,	&(tCpn[i]),				/* number of periods left, cpn dates */					
...
/* Should actually be completely removed later */
	...
	{
		for (j=0; j<nEx; j++)
		{	error = LGMGetFixRate(ExerInto->tEx[j], ExerInto->tEnd[j],
											FVArr[j+1], ycName, &Rfix);
			if (error!=NULL)
			{	srt_free(TauArr); srt_free(FVArr);
				LGMFreeSwptns(&ExerInto);
				return (error);
...
for an inverse floater. If deal is strictly positive or strictly negative, ratio
is set to 0. */
static LGMErr CompValInvFlt(
	Date tNow, String ycName,						/* yield curve */
	LGMErr	(*GetVol)(Date, Date, double, SRT_Boolean, double*),	/* cap vols */
	...
	SrtBasisCode basis)								/* basis for rates */
{
	...
	{	dfEnd = swp_f_df(tNow, t[i], ycName);			/* calculate value of floor */
		...
		/* Vfloor = LGMCEVCapletGreek(SRT_RECEIVER,Forward,a[i],Level,Yrtoexp,CEVvol, beta,PREMIUM); */
		
		Vfloor = LGMCEVCapletPrice(tNow, tEx, SRT_RECEIVER,	a[i], cvgCap, dfSt, dfEnd, CEVvol, beta);
		if (Vfloor < 0.0) return("can't get floor price");
...
/********************************************************************************/
/*	Returns intrinsic value and the ratio PV of coupon leg to PV of the strike
for a cap floater. If deal is strictly positive or strictly negative, ratio
is set to 0. */
static LGMErr CompValCapFlt(
	Date tNow, String ycName,						/* yield curve */
	LGMErr	(*GetVol)(Date, Date, double, SRT_Boolean, double*),	/* cap vols */

...
Err fxlgmsv_mc_balsam(/*	Time Information  */
  ...
  brow = dvector(0, 6);
  // matrix = dmatrix (0        , iNumPaths - 1        , 0        , NbFactor *
  // (iNbTime - 1) - 1);
  ...
  /*
  err = balsam_generation (iNumPaths        , NbFactor * (iNbTime - 1)        ,
  matrix); if (err)
  {
          goto FREE_RETURN;
  }
...
path_index        , double	evt_date        , double	evt_time , void
*func_parm        ,
...
double	for_ft1        , for_ft2        , for_v        , for_sqv        ,
for_psi1        , for_psi2        , for_psi12; double	fwdfx; double	*brow;
double	dom_df        , for_df;
//double	domzcvol1        ,domzcvol2;
double	domzcvol1;
double	forzcvol1        , forzcvol2;
...
long	seed = -123456789;
        ...
        brow = dvector (0        , 3);
        matrix = dmatrix (0        , iNumPaths - 1        , 0        , 4 *
(iNbTime - 1) - 1); res_evt = dvector (0        , iNbProduct - 1); path_payoff =
dvector (0        , iNbProduct - 1); sum_payoff = dvector (0        , iNbProduct
- 1); sum_payoff2 = dvector (0        , iNbProduct - 1);
        ...
        cholmat = f3tensor(0        , iNbTime - 1        , 0        , 3        ,
0        , 3);
...
params->iNbIndex        , 0        , iNumPaths - 1); opt_bar = dvector(0 ,
iNbEvent - 1); coef_lin = dmatrix(0        , iNbEvent - 1        , 0        ,
params->iNbIndex);
...
(double) (time2 - time1) / CLOCKS_PER_SEC);
        ...
        err = balsam_generation (iNumPaths        , 4 * (iNbTime - 1)        ,
matrix); if (err)
        {
                goto FREE_RETURN;
        }
...
(double) (time2 - time1) / CLOCKS_PER_SEC);
        ...
        for (i=0; i<iNumPaths; i++)
        {
                ...
                matrixi = matrix[i];
                memset (path_payoff        , 0        , iNbProduct * sizeof
(double));
...
for_psi12);
      ...
      , dom_psi1        ,
                                                                        ...
                                                                        for_ft1
      , for_ft2        , for_psi1        , for_psi2        , for_psi12        ,
for_v        ,
...
- time2) / CLOCKS_PER_SEC);
        ...
        if (do_optimisation)
        {
                ...
                if (matrix) free_dmatrix (matrix        , 0        , iNumPaths -
1        , 0        , 4
...
FREE_RETURN:
        ...
        if (matrix) free_dmatrix (matrix        , 0        , iNumPaths - 1 , 0
, 4 * (iNbTime - 1) - 1); if (res_evt) free_dvector (res_evt        , 0        ,
iNbProduct - 1); if (path_payoff) free_dvector (path_payoff        , 0        ,
iNbProduct - 1); if (sum_payoff) free_dvector (sum_payoff        , 0        ,
iNbProduct - 1); if (sum_payoff2) free_dvector (sum_payoff2        , 0        ,
iNbProduct - 1);

        if (dom_sigf1) free_dvector (dom_sigf1        , 0        , iNbTime - 1);
        if (dom_sigpsi1) free_dvector (dom_sigpsi1        , 0        , iNbTime -
1);

        if (for_sigf1) free_dvector (for_sigf1        , 0        , iNbTime - 1);
        if (for_sigf2) free_dvector (for_sigf2        , 0        , iNbTime - 1);
        if (for_sigpsi1) free_dvector (for_sigpsi1        , 0        , iNbTime -
1); if (for_sigpsi2) free_dvector (for_sigpsi2        , 0        , iNbTime - 1);
        if (for_sigpsi12) free_dvector (for_sigpsi12        , 0        , iNbTime
- 1); if (for_muv1) free_dvector (for_muv1        , 0        , iNbTime - 1); if
(for_muv2) free_dvector (for_muv2        , 0        , iNbTime - 1); if
(for_sigv) free_dvector (for_sigv        , 0        , iNbTime - 1);

        if (fx_sig) free_dvector (fx_sig        , 0        , iNbTime - 1);

        if (brow) free_dvector (brow        , 0        , 3);

        if (cholmat) free_f3tensor(cholmat        , 0        , iNbTime - 1 , 0
, 3        , 0        , 3);

        if (CorrMatrix_loc) free_dmatrix(CorrMatrix_loc        , 0        , 3 ,
0        , 3);

        if (save_values) free_f3tensor(save_values        , 0        , iNbEvent
- 1        , 0        , 2        , 0        , iNumPaths - 1); if (opt_bar)
free_dvector(opt_bar        , 0        , iNbEvent - 1); if (coef_lin)
free_dmatrix(coef_lin        , 0        , iNbEvent - 1        , 0        ,
params->iNbIndex);
...
long	seed = -123456789;
        ...
        brow = dvector (0        , 6);
        matrix = dmatrix (0        , iNumPaths - 1        , 0        , 7 *
(iNbTime - 1) - 1); res_evt = dvector (0        , iNbProduct - 1); path_payoff =
dvector (0        , iNbProduct - 1); sum_payoff = dvector (0        , iNbProduct
- 1); sum_payoff2 = dvector (0        , iNbProduct - 1);
        ...
        cholmat = f3tensor(0        , iNbTime - 1        , 0        , 6        ,
0        , 6);
...
(double) (time2 - time1) / CLOCKS_PER_SEC);
        ...
        err = balsam_generation (iNumPaths        , 7 * (iNbTime - 1)        ,
matrix); if (err)
        {
                goto FREE_RETURN;
        }
...
FREE_RETURN:
        ...
        if (matrix) free_dmatrix (matrix        , 0        , iNumPaths - 1 , 0
, 7 * (iNbTime - 1) - 1); if (res_evt) free_dvector (res_evt        , 0        ,
iNbProduct - 1); if (path_payoff) free_dvector (path_payoff        , 0        ,
iNbProduct - 1); if (sum_payoff) free_dvector (sum_payoff        , 0        ,
iNbProduct - 1); if (sum_payoff2) free_dvector (sum_payoff2        , 0        ,
iNbProduct - 1);
...
, 6        , 0        , 6);
        ...
        if (CorrMatrix_loc) free_dmatrix(CorrMatrix_loc        , 0        , 6 ,
0        , 6);

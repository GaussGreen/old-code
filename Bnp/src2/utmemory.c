...
/* allocate a long vector with subscript range v[nl..nh] */
{
	...
	if (!v) nrerror("allocation failure in lngvector()");
	return v-nl+NR_END;
}
...
/* allocate a vector of Strings (length sz) with subscript range v[nl..nh] */
{
	...
	v=(char **)srt_calloc(nh-nl+1+NR_END,sizeof(char *));
	if (!v) nrerror("allocation failure in svector_size");
	v += NR_END;
	v -= nl;
...
/* point a submatrix [newrl..][newcl..] to a[oldrl..oldrh][oldcl..oldch] */
{
	...
	if (newrl != oldrh-oldrl+1) nrerror ("wrong no. of rows to submatrix");
	if (newcl != oldch-oldcl+1) nrerror ("wrong no. of cols to submatrix");
...
void free_l3tensor(long***t, long nrl, long nrh, long ncl, long nch,
	long ndl, long ndh)
/* free a double f3tensor allocated by f3tensor() */
{
	free((FREE_ARG) (t[nrl][ncl]+ndl-NR_END));

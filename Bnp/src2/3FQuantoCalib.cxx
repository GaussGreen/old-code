/* ==========================================================================
   ...
   PURPOSE:		Modelling of the spot FX vol by taking in consideration:
         ...
         , 1 spot FX) and all 3F quanto underlyings must be converted to 2 LGM2F
   ...
   quanto underlying        , taking a 4x4 correl tensor as input (index details
   in 3FQuantoCalib.h) In this case        , the 4x4 correl tensor is converted
   to a 5x5 correl tensor and the 5F calibration routine is called.
...
// Takes a domestic and a foreign LGM underlying (one being 1F        , the
// other 2F) and returns 2 LGM2F structures
Err convert_3FQuanto_to_2_LGM2F(
  ...
  if (dom_und_number_of_factors == 1 && for_und_number_of_factors == 2) {
    ...
    *lgm2F_is_dom_for = 1;
    ...
    // Setup the proper values for lambda        , alpha        , gamma &
    // correlation matrix
...
// on an interval [t1        , t2[ where ALL parameters (LGM1F        , LGM2F ,
// FX sigmas
//       , correls and lambdas) are CONSTANT DOMESTIC DYNAMICS: sigma1        ,
//       lambda1 +
// sigma2        , lambda2 FOREIGN DYNAMICS: sigma3        , lambda3 + sigma4 ,
// lambda4 Reminder: in the double** 5x5 correl_matrix        , the indices are:
// 0: DOM W1        , 1: DOM W2        , 3: FOR W3        , 4: FOR W4        ,
// 5: FX
...
// fx_option_maturity on an interval [t1        , t2[ where ALL parameters
// (LGM1F        , LGM2F        , FX sigmas        , correls and lambdas) are
// CONSTANT
...
Err Fx5FtsCalibrationCorr(
  ...
  for (i_fx_options = 0; i_fx_options < n_fx_options; i_fx_options++) {
    ...
    // T1 and T2 but we still need to split by sections [t1        , t2] where
    // LGM
    ...
    c = c2 +
        cum_var; // Add the cumulative variance calculated on [0        , T1]
...
Err Fx5FImpliedVolCorr(double fx_option_maturity, double start_date,
  ...
  for (i = start_index_fx; i < end_index_fx + 1; i++) {
    ...
    // That will correspond to a sum of intervals [t1        , t2] with the
    // leftmost t1 = T1 and the rightmost t2 = T2
    start_index_ir = Get_Index(T1, ir_dates, n_ir_dates);
    end_index_ir = Get_Index(T2, ir_dates, n_ir_dates);
...
Err Fx3FQuantoCalibrationCorr(
  ...
  // Setup the new 5x5 correlation matrix        , note that we put the depth in
  // first so that we can extract a 5x5 slice from it and pass it to our
  // double** functions.

...

{
	...
	if(mdl_dim == ONE_FAC)
	{
		err = srt_f_display_IRM_OneFac_TermStruct (
											ts, 
											sigma_date, sigma, &beta, &vovol, &rhots, &meanvol, sigma_n, 
											tau_date, tau, tau_n);
		*alpha = 1.0;
		*gamma = 0.0;
		*rho = 1.0;
...
//	double		strike)								/*	Strike */								
{
	...
	return amort_lgmopval2F(ncpn, coupon, cpn_G1, cpn_G2, ex_zeta1, ex_zeta2, ex_zeta12, ex_G1, ex_G2);
}
...
/*	NOTE: cap = sum of swaptions 
		each swaption has for underlyings all the coupons from one exercise date to the following */
...

{
	...
	for (i=0; i<nex; i++)
	{
		...
		if(i<nex-1)
		{
			cpn[ex_cpn[i]+ex_ncpn[i]-1] = cpn[ex_cpn[i]+ex_ncpn[i]-1]
										+ df[ex_cpn[i]+ex_ncpn[i]-1];
		}
...

{
	...
	for (i=0; i<nex; i++)
	{
		...
		coupon[ex_cpn[i]+ex_ncpn[i]-1] = floatcoupon[j-1]
										+ df[ex_cpn[i]+ex_ncpn[i]-1]
										+ ex_sstrike[i] 
											* df[ex_cpn[i]+ex_ncpn[i]-1];
...
Err computeZeta(double exer_time, double lambda, double *sigma_date, double *sigma, int sigma_n, double *exer_zeta)
{
	Err err=NULL;
	int i;
	double zeta;
				...
				double *exer_zeta)
{
	...
	export_lgmcalczeta1_tauts(sigma_n, sigma_date, sigma, lambda_n, lambda_time, lambda, 0.0, gamma, 0.0, 1, &exer_time, exer_zeta);
							   ...
							   double		*price)
{
	...
	long *float_fixing_dates=NULL, *float_start_dates=NULL, *float_end_dates=NULL, *float_pay_dates=NULL;
...
//----------------------------------------------------------------------------
		...
		err = swp_f_make_FloatLegDatesCoveragesAndSpreads(&swapdp, lToday, refRateName, 
													&float_pay_dates,
													&float_nb_pay_dates, 
													&float_fixing_dates, 
													&float_start_dates,
		...
		err = swp_f_make_FixedLegDatesAndCoverages(&swapdp, lToday, 
												&fix_pay_dates, 
												&fix_nb_pay_dates, 
												&fix_start_dates, 
												&fix_end_dates, 
		...
		for(k=1;k<nbcoupon;++k)
		{
			...
			if((coupon_dates[k]==fix_pay_times[fix_index])&&(coupon_dates[k]!=float_pay_times[float_index]))
			{
				ivalue = ivalue + rec * strike * fix_cvgs[fix_index-1] * swp_f_df (lToday, fix_pay_dates[fix_index], yc_name);
				coupon[k] += rec * strike * fix_cvgs[fix_index-1] * swp_f_df (lToday, fix_pay_dates[fix_index], yc_name);
				fix_index = fix_index + 1;
			}
			else if((coupon_dates[k]!=fix_pay_times[fix_index])&&(coupon_dates[k]==float_pay_times[float_index]))
			{
				ivalue = ivalue + - rec * ((float_spreads[float_index-1]+margin) * float_cvgs[float_index-1] ) * swp_f_df (lToday, float_pay_dates[float_index], yc_name);
				coupon[k] += -rec * ((float_spreads[float_index-1]+margin) * float_cvgs[float_index-1] ) * swp_f_df (lToday, float_pay_dates[float_index], yc_name);
			...
			else
			{
				...
				else
				{
					...
					ivalue = ivalue - rec * ( - 1 + (float_spreads[float_index-1]+margin) * float_cvgs[float_index-1] ) * swp_f_df (lToday, float_pay_dates[float_index], yc_name);
					coupon[k] += - rec * ( - 1 + (float_spreads[float_index-1]+margin) * float_cvgs[float_index-1] ) * swp_f_df (lToday, float_pay_dates[float_index], yc_name);
					float_index = float_index + 1;
				}
			}
		}
	...
	//-----------------------------------------------------------------
		err = Get_LGM_TermStructureOneOrTwoFact (und_name, 
									&sigma_date, &sigma, &sigma_n, 
									&tau_date, &tau, &tau_n, 
									&alpha, &gamma, &rho);
		if(err)
		{
		...
		exer_time[0] = (add_unit (lStartDate, -notperiod, SRT_BDAY, MODIFIED_SUCCEEDING) - lToday)/365.0;
		...
		else
		{
			...
			err = computeZeta(exer_time[0], lambda+0.5*gamma, sigma_date, sigma, sigma_n, &ex_zeta12);
			if(err)
			{
				return err;
			}
			...
			*price = amort_lgmopval2F(nbcoupon, coupon, cpn_G, cpn_G2, ex_zeta, ex_zeta2, ex_zeta12, ex_G[0], ex_G2[0]);
...
FREE_RETURN:
	...
	if(float_fixing_dates) free(float_fixing_dates);
	if(float_start_dates) free(float_start_dates);
	if(float_end_dates) free(float_end_dates);
	if(float_pay_dates) free(float_pay_dates);

	if(float_cvgs) free(float_cvgs);
	if(float_spreads) free(float_spreads);
	if(float_pay_times) free_dvector(float_pay_times, 0, float_nb_pay_dates-1);

	if(fix_start_dates) free(fix_start_dates);
	if(fix_end_dates) free(fix_end_dates);
	if(fix_pay_dates) free(fix_pay_dates);
	if(fix_pay_times) free_dvector(fix_pay_times, 0, fix_nb_pay_dates-1);
	if(fix_cvgs) free(fix_cvgs);

	if(sigma_date) free(sigma_date);
	if(sigma) free(sigma);

	if(tau_date) free(tau_date);
	if(tau) free(tau);

	if(coupon_dates) free(coupon_dates);
...
//----------------------------------------------------------------------------
		...
		for(k=1;k<nbcoupon;++k)
		{
			...
			if((coupon_dates[k]==fix_pay_times[fix_index])&&(coupon_dates[k]!=float_pay_times[float_index]))
			{
				ivalue = ivalue + FixRates[fix_index-1] * FixNotionals[fix_index-1] * FixCoverages[fix_index-1] * swp_f_df (lToday, FixEndDates[fix_index-1], yc_name);
				coupon[k] += FixRates[fix_index-1] * FixNotionals[fix_index-1] * FixCoverages[fix_index-1] * swp_f_df (lToday, FixEndDates[fix_index], yc_name);
				fix_index = fix_index + 1;
			}
			else if((coupon_dates[k]!=fix_pay_times[fix_index])&&(coupon_dates[k]==float_pay_times[float_index]))
			{
				ivalue = ivalue + -( FloatNotionals[float_index] - FloatNotionals[float_index-1] + FloatNotionals[float_index-1] * (FloatSpreads[float_index-1]+FloatMargins[float_index-1]) * FloatCoverages[float_index-1] ) * swp_f_df (lToday, FloatEndDates[float_index-1], yc_name);
				coupon[k] += -( FloatNotionals[float_index] - FloatNotionals[float_index-1] + FloatNotionals[float_index-1] * (FloatSpreads[float_index-1]+FloatMargins[float_index-1]) * FloatCoverages[float_index-1] ) * swp_f_df (lToday, FloatEndDates[float_index-1], yc_name);
				float_index = float_index + 1;
			}
			else
			{
				...
				{
					ivalue = ivalue + FixRates[fix_index-1] * FixNotionals[fix_index-1] * FixCoverages[fix_index-1] * swp_f_df (lToday, FixEndDates[fix_index-1], yc_name);
					coupon[k] += FixRates[fix_index-1] * FixNotionals[fix_index-1] * FixCoverages[fix_index-1] * swp_f_df (lToday, FixEndDates[fix_index-1], yc_name);
					fix_index = fix_index + 1;
				...
				else
				{
					...
					ivalue = ivalue + - ( - FloatNotionals[float_index-1] + FloatNotionals[float_index-1] * (FloatSpreads[float_index-1]+FloatMargins[float_index-1]) * FloatCoverages[float_index-1] ) * swp_f_df (lToday, FloatEndDates[float_index-1], yc_name);
					coupon[k] += - ( - FloatNotionals[float_index-1] + FloatNotionals[float_index-1] * (FloatSpreads[float_index-1]+FloatMargins[float_index-1]) * FloatCoverages[float_index-1] ) * swp_f_df (lToday, FloatEndDates[float_index-1], yc_name);
					float_index = float_index + 1;
				}
			}
		}
	...
	//-----------------------------------------------------------------
		...
		exer_time[0] = (add_unit (FixStartDates[0], -notperiod, SRT_BDAY, MODIFIED_SUCCEEDING) - lToday)/365.0;
		...
		if(mdl_dim == ONE_FAC)
		{
			export_lgmsetupG_ts(lambda_n,pdlambda_time,pdlambda,nbcoupon, coupon_time, cpn_G, 1, exer_time, ex_G);
			...
			err = computeZeta_ts(exer_time[0], pdlambda_time, pdlambda,lambda_n,0., sigma_date, sigma, sigma_n, &ex_zeta);
			if(err)
			{
				goto FREE_RETURN;
				return err;
		...
		else
		{
			...
			export_lgmsetupG2_ts(lambda_n,pdlambda_time,pdlambda, gamma, nbcoupon, coupon_time, cpn_G2, 1, exer_time, ex_G2);
			if(err)
			{
				goto FREE_RETURN;
				return err;
			...
			err = computeZeta_ts(exer_time[0], pdlambda_time, pdlambda,lambda_n,gamma, sigma_date, sigma, sigma_n, &ex_zeta2);
			if(err)
			{
				goto FREE_RETURN;
				return err;
			...
			err = computeZeta_ts(exer_time[0], pdlambda_time, pdlambda,lambda_n,0.5*gamma, sigma_date, sigma, sigma_n, &ex_zeta12);
			if(err)
			{
				goto FREE_RETURN;
				return err;
...
FREE_RETURN:
	...
	if(FixCoverages) free(FixCoverages);
	if(FloatCoverages) free(FloatCoverages);

	if(float_pay_times) free_dvector(float_pay_times, 0, NFloat);
	if(fix_pay_times) free_dvector(fix_pay_times, 0, NFix);
...
//----------------------------------------------------------------------------
		...
		for(k=1;k<nbcoupon;++k)
		{
			...
			if((coupon_dates[k]==fix_pay_times[fix_index])&&(coupon_dates[k]!=float_pay_times[float_index]))
			{
				ivalue = ivalue + fixRates[fix_index-1] * fixNotional[fix_index-1] * fix_cvgs[fix_index-1] * swp_f_df (lToday, fix_pay_dates[fix_index], yc_name);
				coupon[k] += fixRates[fix_index-1] * fixNotional[fix_index-1] * fix_cvgs[fix_index-1] * swp_f_df (lToday, fix_pay_dates[fix_index], yc_name);
				fix_index = fix_index + 1;
			}
			else if((coupon_dates[k]!=fix_pay_times[fix_index])&&(coupon_dates[k]==float_pay_times[float_index]))
			{
				ivalue = ivalue + -( floatNotional[float_index] - floatNotional[float_index-1] + floatNotional[float_index-1] * (float_spreads[float_index-1]+margin[float_index-1]) * float_cvgs[float_index-1] ) * swp_f_df (lToday, float_pay_dates[float_index], yc_name);
				coupon[k] += -( floatNotional[float_index] - floatNotional[float_index-1] + floatNotional[float_index-1] * (float_spreads[float_index-1]+margin[float_index-1]) * float_cvgs[float_index-1] ) * swp_f_df (lToday, float_pay_dates[float_index], yc_name);
				float_index = float_index + 1;
			}
			else
			{
				...
				else
				{
					...
					ivalue = ivalue + - ( - floatNotional[float_index-1] + floatNotional[float_index-1] * (float_spreads[float_index-1]+margin[float_index-1]) * float_cvgs[float_index-1] ) * swp_f_df (lToday, float_pay_dates[float_index], yc_name);
					coupon[k] += - ( - floatNotional[float_index-1] + floatNotional[float_index-1] * (float_spreads[float_index-1]+margin[float_index-1]) * float_cvgs[float_index-1] ) * swp_f_df (lToday, float_pay_dates[float_index], yc_name);
					float_index = float_index + 1;
				}
			}
		}
...
//					coupon[k] += fixRates[fix_index-1] * fixNotional[fix_index-1] * fix_cvgs[fix_index-1] * swp_f_df (lToday, fix_pay_dates[fix_index], yc_name);
		...
		coupon[nbcoupon-1] += finalFixCoupon * swp_f_df (lToday, fix_end_dates[fix_nb_dates-1], yc_name);
															...
															n-1 */
{
	...
	convert_ratio = sqrt(lambda / LAMBDA_REF * (1.0 - exp(-2.0 * LAMBDA_REF * cpn_time[i])) / (1.0 - exp(-2.0 * lambda * cpn_time[i])));
...
//				s2 -= (pr2 - (mkt_price[i] + d)) * (s2 - s1) / (pr2 - pr1);
				...
				s2 = solve_for_next_coef_dlm(	res_iter,
												k,
												mkt_price[i] + d,
												2);
				...
				if (fabs ((mkt_price[i] + d) - pr2) < CALPRESNOT) break;
			}
...
//		}
	...
	if (res_iter) free_dmatrix(res_iter, 0, 5*NITER, 0, 1);
															...
															n-1 */
{
	...
	convert_ratio = sqrt(lambda[0] / LAMBDA_REF * (1.0 - exp(-2.0 * LAMBDA_REF * cpn_time[i])) / (1.0 - exp(-2.0 * lambda[0] * cpn_time[i])));
	...
	for (i=0; i<nex - (skip_last > 0); i++)
	{
		...
		exp_fact2 = export_lgmcalcexpfact_tauts (t, ex_time[i], nlambda, lambda_time, lambda, gamma) / exp_fact1;
		exp_fact12 = export_lgmcalcexpfact_tauts (t, ex_time[i], nlambda, lambda_time, lambda, 0.5 * gamma) / exp_fact1;
...
//		}
	...
	if (skip_last)
	{		
		exp_fact1 = export_lgmcalcexpfact_tauts (t, ex_time[nex-1], nlambda, lambda_time, lambda, 0.0);
...
double			*ex_sprice)							/*	Cap price as output */
{
	...
	if (one2F == 1)
	{
		return serror ("AmortMidat_lgmprcapgivenlambda_ts(..) :: only 2 factors are allowed at the moment !");
	}

	/*	Setup G function */
	export_lgmsetupG_ts(nNumLambdas, pdLambdaTime, pdLambdaValue,ncpn, cpn_time, cpn_G, nex, ex_time, ex_G);
	
	if (one2F == 2)
	{
		export_lgmsetupG2_ts(nNumLambdas, pdLambdaTime, pdLambdaValue, gamma, ncpn, cpn_time, cpn_G2, nex, ex_time, ex_G2);
	}
															...
															n-1 */
{
	...
	err = AmortMidat_lgmprcapgivenlambda(
		ncpn, cpn, cpn_time, cpn_df, cpn_cvg,
		nex, ex_bool, ex_time, ex_cpn, ex_sncpn, ex_lprice, ex_fee, ex_sstrike, floatcoupon, swaption_cpn, ex_zeta,
		floatnotional, *lambda, one2F, alpha, gamma, rho, skip_last, (fix_lambda? 0: 1) + cap_or_swaption, &pr1);
...
//	}
	...
	do
	{
		...
		if (err = AmortMidat_lgmprcapgivenlambda(
			ncpn, cpn, cpn_time, cpn_df, cpn_cvg,
			nex, ex_bool, ex_time, ex_cpn, ex_sncpn, ex_lprice, ex_fee, ex_sstrike, floatcoupon, swaption_cpn, ex_zeta,
			floatnotional, lam1, one2F, alpha, gamma, rho, skip_last, 1 + cap_or_swaption, &pr1))
		{
			return err;
		}
	...
	{	
		...
		if (err = AmortMidat_lgmprcapgivenlambda(
			ncpn, cpn, cpn_time, cpn_df, cpn_cvg,
			nex, ex_bool, ex_time, ex_cpn, ex_sncpn, ex_lprice, ex_fee, ex_sstrike, floatcoupon, swaption_cpn, ex_zeta,
			floatnotional, lam2, one2F, alpha, gamma, rho, skip_last, 1 + cap_or_swaption, &pr2))
		{
			return err;
		}
		...
		if (fabs (lam2 - lam1) < CALPRESAMORTMIDAT || fabs (pr2 - pr1) < CALPRESAMORTMIDAT) break;
	...
	smessage 
		("Calibration of lambda, error in bp: %.00f", 10000 * min (fabs (ex_sprice - pr1), fabs (ex_sprice - pr2)));
															...
															n-1 */
{
	...
	if(fix_lambda != 1)
	{
		return serror ("amortMidat_lgmcalibzetalambda_ts(...) :: this version only works for fixed lambda !");
	}
	...
	err = AmortMidat_lgmprcapgivenlambda_ts(
		ncpn, cpn, cpn_time, cpn_df, cpn_cvg,
		nex, ex_bool, ex_time, ex_cpn, ex_sncpn, ex_lprice, ex_fee, ex_sstrike, floatcoupon, swaption_cpn, ex_zeta,
		floatnotional, nNumLambda, pdLambdaValue, pdLambdaTime, one2F, alpha, gamma, rho, skip_last, (fix_lambda? 0: 1), &pr1);
	...
	{	
		...
		if (fabs (lam2 - lam1) < CALPRESNOT || fabs (pr2 - pr1) < CALPRESNOT) break;
															...
															n-1 */
{
	...
	err = zcAmortMidat_lgmprcapgivenlambda(
		ncpn, cpn, zc_last_cpn, cpn_time, cpn_df, cpn_cvg,
		nex, ex_bool, ex_time, ex_cpn, ex_sncpn, ex_lprice, ex_sstrike, floatcoupon, ex_zeta,
		floatnotional, *lambda, one2F, alpha, gamma, rho, skip_last, (fix_lambda? 0: 1), &pr1);
	...
	do
	{
		...
		if (err = zcAmortMidat_lgmprcapgivenlambda(
			ncpn, cpn, zc_last_cpn, cpn_time, cpn_df, cpn_cvg,
			nex, ex_bool, ex_time, ex_cpn, ex_sncpn, ex_lprice, ex_sstrike, floatcoupon, ex_zeta,
			floatnotional, lam1, one2F, alpha, gamma, rho, skip_last, 1, &pr1))
		{
			return err;
		}
	...
	{	
		...
		if (err = zcAmortMidat_lgmprcapgivenlambda(
			ncpn, cpn, zc_last_cpn, cpn_time, cpn_df, cpn_cvg,
			nex, ex_bool, ex_time, ex_cpn, ex_sncpn, ex_lprice, ex_sstrike, floatcoupon, ex_zeta,
			floatnotional, lam2, one2F, alpha, gamma, rho, skip_last, 1, &pr2))
		{
			return err;
		}
...
//	err = swp_f_get_ref_rate_details(ref_rate, &floatbasis, &floatfreq);
	...
	for(i=firstDoCalib;i<Nfix;++i)
	{
		...
		coupon_cvg_standard[i-firstDoCalib] = coverage(fix_start_dates[i-firstDoCalib], fix_end_dates[i-firstDoCalib], swapbasis);
	}
...
//	swaption_cpn[0] = - swp_f_df (today, float_start_dates[0], yc_name);
	...
	for(k=1;k<nbcoupon;++k)
	{
		...
		coupon_cvg[k] = float_cvgs[k-1];
		if( (fabs(coupon_times[k]-fix_pay_times[fix_index])<10.0/365.0) && (fabs(coupon_times[k]-float_pay_times[float_index])>10.0/365.0) )
		{
			coupon[k] += fix_rates[fix_index-1] * fix_notionals[fix_index-1] * fix_cvgs[fix_index-1] * swp_f_df (today, fixpaydates[fix_index-1], yc_name);
			ex_coupon[fix_index-1] = k - fix_float_mult;
			exer_date[fix_index-1] = add_unit (fix_start_dates[fix_index-1], -notperiod, SRT_BDAY, MODIFIED_SUCCEEDING);
			exer_time[fix_index-1] = (exer_date[fix_index-1] - today) * YEARS_IN_DAY;
			fix_index = fix_index + 1;
		}
		else if( (fabs(coupon_times[k]-fix_pay_times[fix_index])>10.0/365.0) && (fabs(coupon_times[k]-float_pay_times[float_index])<10.0/365.0) )
		{
			temp = swp_f_df (today, floatpaydates[float_index-1], yc_name);
			coupon[k] += -( float_notionals[float_index] - float_notionals[float_index-1] + float_notionals[float_index-1] * (float_spreads[float_index-1] + float_margins[float_index-1]) * float_cvgs[float_index-1] ) * swp_f_df (today, floatpaydates[float_index-1], yc_name);
			floatcoupon[k] += - (float_spreads[float_index-1] + float_margins[float_index-1]) * float_cvgs[float_index-1] * swp_f_df (today, floatpaydates[float_index-1], yc_name);
			float_index = float_index + 1;
		}
		else
		{
			...
			else
			{
				...
				exer_time[fix_index-1] = (exer_date[fix_index-1] - today) * YEARS_IN_DAY;
				fix_index = fix_index + 1;
				coupon[k] += - ( - float_notionals[float_index-1] + float_notionals[float_index-1] * (float_spreads[float_index-1] + float_margins[float_index-1]) * float_cvgs[float_index-1] ) * swp_f_df (today, floatpaydates[float_index-1], yc_name);
				floatcoupon[k] += - ( - 1.0 + (float_spreads[float_index-1] + float_margins[float_index-1]) * float_cvgs[float_index-1] ) * swp_f_df (today, floatpaydates[float_index-1], yc_name);
				float_index = float_index + 1;
			}
		}
	}
...
//-----------------------------------------------------------------
	...
	{
		err = ComputeAmortSwapDiagonalIRRsNew2(yc_name, vol_curve_name,
...
//--------------------------------------------------------------
	if (!fix_lambda)
	{
		...
		for (i=0; i<nbexercise; i++)
		{
			...
			{
				err = ComputeSwapRateFromIRR(yc_name, vol_curve_name,
										today,
										default_ref_rate_name,
			...
			if ((strike_type == 1)||(strike_type == 8))
			{
				if (short_strike)
				{
	...
	if((strike_type>=6)&&(!fix_lambda))
	{
		...
		for(k=1;k<nbcoupon;++k)
		{
			if((coupon_times[k]==fix_pay_times[fix_index])&&(coupon_times[k]!=float_pay_times[float_index]))
			{
				swaption_cpn[k] += ex_sstrike[fix_index-1] * coupon_cvg_standard[fix_index-1] * swp_f_df (today, fixpaydates[fix_index-1], yc_name);
				fix_index = fix_index + 1;
			}
			else if((coupon_times[k]!=fix_pay_times[fix_index])&&(coupon_times[k]==float_pay_times[float_index]))
			{
				swaption_cpn[k] += -( (float_spreads[float_index-1] + float_margins[float_index-1]) * float_cvgs[float_index-1] ) * swp_f_df (today, floatpaydates[float_index-1], yc_name);
				float_index = float_index + 1;
			}
			else
			{
				...
				{
					swaption_cpn[k] += ex_sstrike[fix_index-1] * fix_cvgs[fix_index-1] * swp_f_df (today, fixpaydates[fix_index-1], yc_name);
					fix_index = fix_index + 1;
					swaption_cpn[k] += - ( - 1 + (float_spreads[float_index-1] + float_margins[float_index-1]) * float_cvgs[float_index-1] ) * swp_f_df (today, floatpaydates[float_index-1], yc_name);
					float_index = float_index + 1;
				}
			}
		}
	...
	*sig = (double*) calloc (*num_sig, sizeof (double));
	...
	for (i=1; i<nbexercise; i++)
	{
		if(exer_bool[i]>0)
		{
			...
			{
				smessage ("Diagonal calibration failed at exercise year %.2f - Calibration stopped", exer_time[i]);
				for (j=k; j<*num_sig; j++)
				{
...
FREE_RETURN:
	...
	if(fix_cvgs) free_dvector(fix_cvgs, 0, Nfix-1);
	if(float_cvgs) free_dvector(float_cvgs, 0, Nfloat-1);
	
	if(float_pay_dates) free_lngvector(float_pay_dates, 0, Nfloat);
	if(float_pay_times) free_dvector(float_pay_times, 0, Nfloat);

	if(fix_pay_dates) free_lngvector(fix_pay_dates, 0, Nfix);
	if(fix_pay_times) free_dvector(fix_pay_times, 0, Nfix);

	if(coupon_times) free(coupon_times);

	if(dIRRs) free(dIRRs);
	if(dIRRstrikes) free(dIRRstrikes);
...
//--------------------------------------------------------------
	...
	if (one2F == 2 && fix_lambda && one_f_equi && (strike_type < 6))
	{
		...
		//--------Price the cap within the calibrated LGM1F model----------------
		export_lgmsetupG_ts (nNum_lambda,pdLambdaValue,pdLambdaDate,nbcoupon, coupon_time, cpn_G, nbexercise, exer_time, ex_G);
	...
	*sig = (double*) calloc (*num_sig, sizeof (double));
	...
	exp_fact = static_lgmcalcexpfact_tauts (0.0, exer_time[0], nNum_lambda, pdLambdaDate, pdLambdaValue);
	...
	for (i=1; i<nbexercise; i++)
	{
		if(exer_bool[i]>0)
		{
			...
			if (ex_zeta[i] > ex_zeta[last_i])
			{
				...
				exp_fact = static_lgmcalcexpfact_tauts (exer_time[last_i], exer_time[i], nNum_lambda, pdLambdaDate, pdLambdaValue);
...
//-----------------------------------------------------
	...
	for(k=1;k<nbcoupon;++k)
	{
		...
		if((coupon_times[k]==fix_pay_times[fix_index])&&(coupon_times[k]!=float_pay_times[float_index]))
		{
			coupon[k] += fix_rates[fix_index-1] * fix_notionals[fix_index-1] * fix_cvgs[fix_index-1] * swp_f_df (today, fix_pay_datesOut[fix_index-1], yc_name);
			ex_coupon[fix_index-1] = k - fix_float_mult;
			exer_date[fix_index-1] = add_unit (fix_start_dates[fix_index-1], -notperiod, SRT_BDAY, MODIFIED_SUCCEEDING);
			exer_time[fix_index-1] = (exer_date[fix_index-1] - today) * YEARS_IN_DAY;
			fix_index = fix_index + 1;
		...
		else if((coupon_times[k]!=fix_pay_times[fix_index])&&(coupon_times[k]==float_pay_times[float_index]))
		{
			...
			coupon[k] += -( float_notionals[float_index] - float_notionals[float_index-1] + float_notionals[float_index-1] * (float_spreads[float_index-1] + float_margins[float_index-1]) * float_cvgs[float_index-1] ) * swp_f_df (today, float_pay_datesOut[float_index-1], yc_name);
			floatcoupon[k] += - (float_spreads[float_index-1] + float_margins[float_index-1]) * float_cvgs[float_index-1] * swp_f_df (today, float_pay_datesOut[float_index-1], yc_name);
			float_index = float_index + 1;
		}
		else
		{
			...
			else
			{
				...
				exer_time[fix_index-1] = (exer_date[fix_index-1] - today) * YEARS_IN_DAY;
				fix_index = fix_index + 1;
				coupon[k] += - ( - float_notionals[float_index-1] + float_notionals[float_index-1] * (float_spreads[float_index-1] + float_margins[float_index-1]) * float_cvgs[float_index-1] ) * swp_f_df (today, float_pay_datesOut[float_index-1], yc_name);
				floatcoupon[k] += - ( - 1.0 + (float_spreads[float_index-1] + float_margins[float_index-1]) * float_cvgs[float_index-1] ) * swp_f_df (today, float_pay_datesOut[float_index-1], yc_name);
				float_index = float_index + 1;
			}
		}
	}
...
//--------------------------------------------------------------
	...
	if((strike_type>=6)&&(!fix_lambda))
	{
		...
		for(k=1;k<nbcoupon;++k)
		{
			if((coupon_times[k]==fix_pay_times[fix_index])&&(coupon_times[k]!=float_pay_times[float_index]))
			{
				swaption_cpn[k] += ex_sstrike[fix_index-1] * coupon_cvg_standard[fix_index-1] * swp_f_df (today, fix_pay_datesOut[fix_index-1], yc_name);
				fix_index = fix_index + 1;
			}
			else if((coupon_times[k]!=fix_pay_times[fix_index])&&(coupon_times[k]==float_pay_times[float_index]))
			{
				swaption_cpn[k] += -( (float_spreads[float_index-1] + float_margins[float_index-1]) * float_cvgs[float_index-1] ) * swp_f_df (today, float_pay_datesOut[float_index-1], yc_name);
				float_index = float_index + 1;
			}
			else
			{
				...
				{
					swaption_cpn[k] += ex_sstrike[fix_index-1] * fix_cvgs[fix_index-1] * swp_f_df (today, fix_pay_datesOut[fix_index-1], yc_name);
					fix_index = fix_index + 1;
					swaption_cpn[k] += - ( - 1 + (float_spreads[float_index-1] + float_margins[float_index-1]) * float_cvgs[float_index-1] ) * swp_f_df (today, float_pay_datesOut[float_index-1], yc_name);
					float_index = float_index + 1;
				}
			}
		}
...
FREE_RETURN:
	...
	if(float_pay_datesOut) free(float_pay_datesOut);
	if(float_pay_times) free_dvector(float_pay_times, 0, Nfloat);

	if(fix_pay_datesOut) free(fix_pay_datesOut);
	if(fix_pay_times) free_dvector(fix_pay_times, 0, Nfix);
...
//--------------------------------------------
	...
	err = swp_f_make_FloatLegDatesCoveragesAndSpreads(&swapdp, today, ref_rate_name, 
												&float_pay_dates,
												&float_nb_pay_dates, 
												&float_fixing_dates, 
												&float_start_dates,
	...
	{
		floatNotional[i-(int)(0.5+fix_cvgs[1]/float_cvgs[1]) * firstDoCalib] = floatNotional[i];
	}
...
//-------------------------------------------
	...
	for(k=1;k<nbcoupon;++k)
	{
		...
		if((coupon_dates[k]==fix_pay_times[fix_index])&&(coupon_dates[k]!=float_pay_times[float_index]))
		{
			coupon[k] += long_strike[fix_index-1] * fixNotional[fix_index-1] * fix_cvgs[fix_index-1] * swp_f_df (today, fix_pay_dates[fix_index], yc_name);
			swaption_cpn[k] += long_strike[fix_index-1] * fix_cvgs[fix_index-1] * swp_f_df (today, fix_pay_dates[fix_index], yc_name);
			ex_coupon[fix_index-1] = k - fix_float_mult;
			exer_date[fix_index-1] = add_unit (fix_pay_dates[fix_index-1], -notperiod, SRT_BDAY, MODIFIED_SUCCEEDING);
			exer_time[fix_index-1] = (exer_date[fix_index-1] - today) * YEARS_IN_DAY;
			fix_index = fix_index + 1;
		}
		else if((coupon_dates[k]!=fix_pay_times[fix_index])&&(coupon_dates[k]==float_pay_times[float_index]))
		{
			...
			coupon[k] += -( floatNotional[float_index] - floatNotional[float_index-1] + floatNotional[float_index-1] * (float_spreads[float_index-1] + margin[float_index-1]) * float_cvgs[float_index-1] ) * swp_f_df (today, float_pay_dates[float_index], yc_name);
			swaption_cpn[k] += -( (float_spreads[float_index-1] + margin[float_index-1]) * float_cvgs[float_index-1] ) * swp_f_df (today, float_pay_dates[float_index], yc_name);
			floatcoupon[k] += - (float_spreads[float_index-1] + margin[float_index-1]) * float_cvgs[float_index-1] * swp_f_df (today, float_pay_dates[float_index], yc_name);
			float_index = float_index + 1;
		}
		else
		{
			...
			else
			{
				...
				exer_time[fix_index-1] = (exer_date[fix_index-1] - today) * YEARS_IN_DAY;
				fix_index = fix_index + 1;
				coupon[k] += - ( - floatNotional[float_index-1] + floatNotional[float_index-1] * (float_spreads[float_index-1] + margin[float_index-1]) * float_cvgs[float_index-1] ) * swp_f_df (today, float_pay_dates[float_index], yc_name);
				swaption_cpn[k] += - ( - 1 + (float_spreads[float_index-1] + margin[float_index-1]) * float_cvgs[float_index-1] ) * swp_f_df (today, float_pay_dates[float_index], yc_name);
				floatcoupon[k] += - ( - 1.0 + (float_spreads[float_index-1] + margin[float_index-1]) * float_cvgs[float_index-1] ) * swp_f_df (today, float_pay_dates[float_index], yc_name);
				float_index = float_index + 1;
			}
		}
	}
...
//-----------------------------------------------------------------
	...
	{
		err = ComputeAmortSwapDiagonalIRRs(yc_name, vol_curve_name,
						default_ref_rate_name,
						start_date,
						end_date,
						ifreq,
...
//					ex_sstrike[i],
	...
	if (one2F == 2 && fix_lambda && one_f_equi)
	{
		...
		for (i=0; i<nbexercise; i++)
		{
			...
			lvl = 0.0;
//			for (k=(int)(ex_coupon[i]*(fix_nb_pay_dates-1)/(float_nb_pay_dates-1)); k<(ex_coupon[i]+ex_sncpn[i])*(fix_nb_pay_dates-1)/(float_nb_pay_dates-1); k++)
//			{
//				lvl += fix_cvgs[k] * swp_f_df(today, fix_pay_dates[k+1], yc_name);
//			}
			for (k=i; k<i+1; k++)
...
FREE_RETURN:
	...
	if(lastFixCoupon) free(lastFixCoupon);

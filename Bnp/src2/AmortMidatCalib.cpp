...
/*	Precalculate exponential factors in the IV  , and decide what variable
   to use in integration  , and which one to use in y* solving  */
    ...
    if (mdl_dim == ONE_FAC) {
      ...
      *price = lgmopval1F(nbcoupon, coupon, cpn_G, ex_zeta, ex_G[0]);
      ...
      /*			err = get_underlying_ts(und  , &ts);
                              ...
                              err = find_2f_sig (exer_time[0]  , ts  , &sig1  ,
         &sig2); if(err)
                              {
                                      return err;
                              }
                              ...
                              err = find_2f_tau (exer_time[0]  , ts  , &tau1  ,
         &tau2); if(err)
                              {
                                      return err;
                              }
                      ...
                      from option n-1 */
{
  ...
  for (i = 0; i < nex - (skip_last > 0); i++) {
    ...
    //exp_fact2; 			zeta12 = zeta12 + s2 * exp_fact1 * alpha * rho * exp_fact12; 			t
    //= ex_time[i];
    //		}
  }
                      ...
                      from option n-1 */
{
  ...
  for (i = 0; i < nex - (skip_last > 0); i++) {
    ...
    if ((ex_bool[i] == 1) || (i == 0)) {
      ...
      //			ex_zeta[i] = zeta1 + s2 * exp_fact1;
      //			zeta2 = zeta2 + s2 * exp_fact1 * alpha * alpha *
      //exp_fact2; 			zeta12 = zeta12 + s2 * exp_fact1 * alpha * rho * exp_fact12;
      //			t = ex_time[i];
    }
  }
  ...
  for (i = 0; i < nex - (skip_last > 0); i++) {
    ...
    if ((ex_bool[i] == 1) || (i == 0)) {
      ...
      if (fabs((mkt_price[i] + d) - pr2) > CALPRESNOT) {
        ...
        //				smessage ("Failed to calibrate for exercise
        //date %d"  , i+1);
        s2 = s_last;
        dz2 = s2 * exp_fact;
        z2 = zeta + dz2;

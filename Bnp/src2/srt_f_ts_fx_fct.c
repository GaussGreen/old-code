...
SrtLst* ls;
	...
	while ( (ls!=NULL) &&	  
		( ((FxTermStructVal*)ls->element->val.pval)->time < time) ) ls = ls->next;
...

{
	...
	result = tsval->M_fx
			+tsval->rhofx*tsval->sigx*(sig/F)*temp;
	return  result;
...

{
	...
	result = tsval->R_fd
			+tsval->rhofd*dom_Psi*temp*temp1
...
double Q_fd_func(double time, TermStruct *l)
{
	...
	result = tsval->Q_fd
			...
			+tsval->rhofd*dom_sig*for_sig*dom_tau*(temp2/lambda-temp/lambda-time*for_tau+temp*for_tau);
...
Err srt_f_get_fx_implied_vol(double yr_to_exp,
					String        fx_und_names,
					double		 *fx_bs_vol)
{
...
/* Compute the Domestic BS Implied Volatility */
	...
	dom_bd_vol = (-sq_dom_psi*dom_G+2*dom_psi*dom_L+dom_G*sq_dom_psi-2*dom_O)/yr_to_exp;
...
/* Compute the Foreign BS Implied Volatility */
	...
	for_bd_vol = (-sq_for_psi*for_G+2*for_psi*for_L+for_G*sq_for_psi-2*for_O)/yr_to_exp;
...
/* Compute the Covariance between the  dom. zc bond and the for zc bond */
	...
	cov_dom_int_for_int = dom_psi*for_psi*Ofd
						-for_psi*Rfd
						-dom_psi*Pfd
						+Mfd;
...
Err srt_f_get_fx_stoch_rates_correl(double		start_date,
									double		end_date,
									String		fx_und_names,
									double		**corr_matrix,
...
/* Get the variance of log(Xt) */
	...
	var_int_dom_sr = 	sq_dom_psi*(dom_G-prev_dom_G)
					...
					+dom_G*sq_dom_psi-prev_dom_G*sq_prev_dom_psi
					-2*dom_O_dt;  
	...
	var_int_for_sr = 	sq_for_psi*(for_G-prev_for_G)
					...
					+for_G*sq_for_psi-prev_for_G*sq_prev_psi
					-2*for_O_dt; 
	...
	cov_int_dom_for = dom_psi*for_psi*Ofd_dt
					-for_psi*Rfd_dt
					-dom_psi*Pfd_dt
					+Mfd_dt;
...
Err srt_f_get_fx_stoch_rates_ind_correl(double		start_date,
										double		end_date,
										...
										double		*corr_or_var)
{
	...
	else if( ( ( !strcmp(first_und_name,fx_und_name) ) && ( !strcmp(second_und_name,for_und_name) ) )
		|| ( ( !strcmp(first_und_name,for_und_name) ) && ( !strcmp(second_und_name,fx_und_name) ) ) )
		   *corr_or_var = corr_matrix[1];
	else  if( ( ( !strcmp(first_und_name,dom_und_name) ) && ( !strcmp(second_und_name,for_und_name) ) )
	...
	if(corr_matrix) free_dvector(corr_matrix,0,2);corr_matrix = NULL;

...
/* The output nbrepuplets[i] is the number of p-uplets of integers whose sum is
   i-1 */
{
  double **u;
  int i, j, k;
...
/* CALCULATE AN UPPER BOUND OF THE TOTAL NUMBER OF JUMPS ON THE MATURITY OF
 * OPTION*/
Err FSautsMax(double **Poisson, /* Array of p Poissons */
              int p, double *NSautsMax) {
  int i;
  double intensmax, s;
  ...
  s = 0;
  i = 0;
  while (s < (1 - prec) * intensmax) /* Guaranteed that the relative error on
                                        the expectation is smaller than prec  */
  {
...
Err Probas_Poissons(double **Poissons, /* For i=1..p        , Poisson[i]
 has two parameters: intensity and amplitude */
                    int p, double *n_sauts,
                    double *proba, /* Conditional proba of the following
event: for all i        , the i=th Poisson jumps n_sauts[i] times */
                    double *amplitude_totale /* For each event        , value of
                                                Prod((1+U_i)^n_i) */
) {
  int i;
  ...
  *amplitude_totale = 1;
  for (i = 1; i < p + 1; i++) {
    *proba = *proba * exp(-Poissons[i][1] + n_sauts[i] * log(Poissons[i][1])) /
             fact((int)n_sauts[i]);
    *amplitude_totale =
        *amplitude_totale * pow(1 + Poissons[i][2], (double)n_sauts[i]);
  }
...
/* ITERATIVE FUNCTION*/
Err FLoop(double **enspuplets, double *pupletloc, int levelloc, int maxloc,
          int p, int *n) {
  int i, j;
...
has two parameters: intensity and amplitude*/
    int p, double NSautsMax, /* Upper bound ofthe total number of jumps*/
  ...
  *nbretotalpuplets = 0;
  ...
  for (n = 1; n <= *nbretotalpuplets; n++)
    Probas_Poissons(Poissons, p, enspuplets[n], &((*sauts).probas[n]),
                    &((*sauts).sizes[n]));
...
Err optmertonpremiumtimedependent(double dFwd, double Strike,
                                  int n_periods, /* The 6 following variables
                                  are vectors of length nperiods */
                                  double *sigma, double *U1, double *lambda1,
                                  double *U2, double *lambda2, double *mat,
  ...
  for (i = 1; i <= nevents; i++) {
    ...
    premium +=
        sauts.probas[i] * srt_f_optblksch(dFwdJump, Strike, sigma_eq, maturity,
                                          1, SRT_CALL, PREMIUM);
  }
...

{
  ...
  for (i = 1; i <= n_strikes; i++) {
    for (j = 1; j <= nevents; j++) {
      ...
      premium[i] +=
          sauts.probas[j] * srt_f_optblksch(dFwdJump, strikes[i], sigma_eq,
  ...
  for (i = 1; i <= n_strikes; i++) {
    ...
    srt_f_optimpvol(premium[i], dFwd, strikes[i], maturity, 1.0, SRT_CALL,
                    SRT_LOGNORMAL, &(impvols[i]));
  }
...
Err optmerton(double dFwd, double Strike, double sigma, double U1,
...

{
  ...
  switch (Greek) {
  case PREMIUM:
    *answer = optmertonpremium(dFwd, Strike, sigma, U1, lambda1, U2, lambda2,
                               maturity, Logornorm);

    break;
  case DELTA:
    shift = dFwd / 10000;
    *answer = (optmertonpremium(dFwd + shift, Strike, sigma, U1, lambda1, U2,
                                lambda2, maturity, Logornorm) -
               optmertonpremium(dFwd, Strike, sigma, U1, lambda1, U2, lambda2,
                                maturity, Logornorm)) /
              shift;
    break;
  case GAMMA:
    ...
    *answer += optmertonpremium(dFwd - shift, Strike, sigma, U1, lambda1, U2,
                                lambda2, maturity, Logornorm);
    *answer -= 2 * optmertonpremium(dFwd, Strike, sigma, U1, lambda1, U2,
                                    lambda2, maturity, Logornorm);
    *answer /= shift * shift;
  ...
  case VEGA:
    ...
    *answer = (optmertonpremium(dFwd, Strike, sigma + shift, U1, lambda1, U2,
                                lambda2, maturity, Logornorm) -
               optmertonpremium(dFwd, Strike, sigma, U1, lambda1, U2, lambda2,
                                maturity, Logornorm)) /
              shift;
  ...
  case DU1:
    ...
    *answer = (optmertonpremium(dFwd, Strike, sigma, U1 + shift, lambda1, U2,
                                lambda2, maturity, Logornorm) -
               optmertonpremium(dFwd, Strike, sigma, U1, lambda1, U2, lambda2,
                                maturity, Logornorm)) /
              shift;
  ...
  case DLAMBDA1:
    ...
    *answer = (optmertonpremium(dFwd, Strike, sigma, U1, lambda1 + shift, U2,
                                lambda2, maturity, Logornorm) -
               optmertonpremium(dFwd, Strike, sigma, U1, lambda1, U2, lambda2,
                                maturity, Logornorm)) /
              shift;
  ...
  case DU2:
    ...
    *answer = (optmertonpremium(dFwd, Strike, sigma, U1, lambda1, U2 - shift,
                                lambda2, maturity, Logornorm) -
               optmertonpremium(dFwd, Strike, sigma, U1, lambda1, U2, lambda2,
                                maturity, Logornorm)) /
              shift;
  ...
  case DLAMBDA2:
    ...
    *answer = (optmertonpremium(dFwd, Strike, sigma, U1, lambda1, U2,
                                lambda2 + shift, maturity, Logornorm) -
               optmertonpremium(dFwd, Strike, sigma, U1, lambda1, U2, lambda2,
                                maturity, Logornorm)) /
              shift;
    break;
...
Err optmertonsmile(double dFwd, int n_strikes, double *strikes,
  ...
  for (i = 1; i <= n_strikes; i++) {
    price = optmertonpremium(dFwd, strikes[i], parameters[1], parameters[2],
                             parameters[3], parameters[4], parameters[5],
                             maturity, Logornorm);

    if (strcmp(Logornorm, "Normal") == 0) {
      srt_f_optimpvol(price, dFwd, strikes[i], maturity, 1.0, SRT_CALL,
                      SRT_NORMAL, &(impvols[i]));
    } else {
      srt_f_optimpvol(price, dFwd, strikes[i], maturity, 1.0, SRT_CALL,
                      SRT_LOGNORMAL, &(impvols[i]));
    }

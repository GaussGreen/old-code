...
Err srt_f_get_vasicek_mean_sr(double time, TermStruct* ts, double* vasicek_mean_sr)
{
    SrtLst*           ls;
...
Err srt_f_get_vasicek_mean_int_sr(double time, TermStruct* ts, double* vasicek_mean_int_sr)
{
    SrtLst*           ls;
    IrmTermStructVal *tsval, *tsval_p;
    Err               err = NULL;
...
Err srt_f_get_vasicek_var_int_sr(double time, TermStruct* ts, double* vasicek_var_int_sr)
{
    SrtLst*           ls;
    IrmTermStructVal *tsval, *tsval_p;
    double            dom_psi;
    ...
    (*vasicek_var_int_sr) =
        (tsval->L + tsval->F * tsval->G * (1 - exp(-time / tsval->tau)) * tsval->tau +
         0.5 * pow(tsval->sig * tsval->tau, 2) * (exp(-time / tsval->tau) - 2) / tsval->F);

    (*vasicek_var_int_sr) *= dom_psi;

    (*vasicek_var_int_sr) -=
        (tsval->O + tsval->F * tsval->G * tsval->Psi * (1 - exp(-time / tsval->tau)) * tsval->tau +
...
Err srt_f_get_vasicek_init_cond(double time, TermStruct* ts, double* vasicek_init_cond)
{
    SrtLst* ls;
...
static Err srt_f_get_lgm_mean_int_sr(double time, TermStruct* ts, double* mean_int_nom_sr)
{
    (*mean_int_nom_sr) = 0.0;
...
static Err srt_f_get_lgm_var_int_sr(double time, TermStruct* ts, double* var_int_nom_sr)
{
...
Err srt_f_vasicek_discount_factor(Date discount_mat_date, SrtUndPtr und, double* discount_factor)
{
    TermStruct* ts;
    Date        date_today;
    ...
    err = srt_f_get_vasicek_init_cond(discount_factor_mat, ts, &vasicek_init_cond);
    if (err)
        return err;

    err = srt_f_get_vasicek_mean_int_sr(discount_factor_mat, ts, &vasicek_mean_int_sr);
    if (err)
        return err;
...
Err srt_f_vasicek_fra(Date start_date, Date end_date, SrtUndPtr und, double* fra)
{
    Err    err = NULL;
    ...
    double*   swap)
{
    ...
    /* Set the SwapDP */
    err = swp_f_setSwapDP(start_date, theo_end_date, swap_freq_code, swap_basis_code, &swp_dp);
    if (err)
        return err;
...
Err srt_f_vasicek_fut(double start_date, double pay_date, char* vasicek_und_name, double* fut_pr)
{
    Err         err = NULL;
    SrtUndPtr   und;
    TermStruct* und_ts;
    ...
    adjustement = 0.0;
    adjustement +=
        Psi_start_pay * Psi_start_pay * G_start + Psi_start_pay * L_func(start_time, und_ts);
...
Err srt_f_vasicek_market_libor_expectation(
    double  start_date,
    double  pay_date,
    double  risk_premuim,
    ...
    double* market_libor_expectation)
{
    ...
    (*market_libor_expectation) = ((df_start / df_pay) * exp(adjustement) - 1) /
                                  coverage((Date)start_date, (Date)pay_date, BASIS_ACT_360);
...
Err srt_f_vasicek_cont_fwd_zr(
    double fixing_date, double pay_date, SrtUndPtr und, double* cont_fwd_zr)
{
    Err    err = NULL;
    ...
    double* overnight_interest_rate_swap)
{
    ...
    start_t_cond_var +=
        (G_pay_T * Psi_func(pay_T, ts) * Psi_func(pay_T, ts) - 2 * O_func(pay_T, ts));
    ...
    (*overnight_interest_rate_swap) *=
        exp(sr_coeff * fwd_measure_sr_mean + 0.5 * sr_coeff * sr_coeff * sr_variance);
    ...
    double*    dirty_price)
{
    ...
    if (price_floor == SRT_YES)
    {
        err = srt_f_vasicek_discount_factor(
            (long)(pay_dates[bond_index][num_cpns[bond_index] - 1]), (*new_und), &real_df_fixing);
    ...
    double*** calibrated_mean_rev_lvl)
{
    ...
    for (i = 0; i < num_bonds; i++)
    {
        ...
        (*calibrated_mean_rev_lvl)[i][1] =
            find_mean_rev_level((pay_dates[i][num_cpns[i] - 1] - date_today) * YEARS_IN_DAY, ts);
    }
...
static Err init_static_params_struct(
    Date*   overnight_swap_end_dates,
...
static Err from_optparam_to_calibration_params(
    double* optim_params, long num_parms, struct calibration_struct* calibration_parms)
{
    long i;
    ...
    int     num_params)
{
    ...
    err = from_optparam_to_calibration_params(optim_params, num_params, &calibration_parms);
    if (err)
        return err;
...
static Err levenberg_calib_funcs(
    double instr_index, double optim_parms[], double* value, double deriv[], int num_params)
{
    long   i;
    double shift;
    Err    err;
    ...
    for (i = 1; i <= num_params; i++)
    {
        ...
        err = levenberg_price_funcs(instr_index, optim_parms, &(deriv[i]), num_params);
        if (err)
            return err;
...
Err srt_f_vasicek_overnights_calibrate(
    long    num_overnight_swap_rates,
...
Err srt_f_vasicek_money_market_account_vol(
    Date date, char* vasicek_und_name, double* money_market_account_vol)
{
    Err         err = NULL;
    double      date_today;
    SrtUndPtr   und;
    ...
    if (!und)
        return serror(
            "fatal: can not get the underlying in srt_f_vasicek_money_market_account_vol");
...
Err srt_f_inflation_fwd(
    long start_date, long end_date, char* price_index_und_name, double* inflation_fwd)
{
    Err         err = NULL;
    ...
    err = srt_f_vasicek_discount_factor(end_date, real_rate_und, &df_real_end_date);
    if (err)
        return err;

    err = srt_f_vasicek_discount_factor(start_date, real_rate_und, &df_real_start_date);
    if (err)
        return err;
    ...
    Psi_real_start_end =
        Psi_func(end_date_time, real_rate_ts) - Psi_func(start_date_time, real_rate_ts);
    Psi_nom_start_end =
        Psi_func(end_date_time, nom_rate_ts) - Psi_func(start_date_time, nom_rate_ts);

    adjustment += Psi_real_start_end * (Psi_func(start_date_time, real_rate_ts) * G_real_start -
    ...
    adjustment += Psi_real_start_end * (Psi_func(end_date_time, nom_rate_ts) *
                                            O_fd_func(start_date_time, price_index_ts) -
                                        R_fd_func(start_date_time, price_index_ts));
    adjustment -=
        Psi_real_start_end * Psi_nom_start_end * O_fd_func(start_date_time, price_index_ts);
...
double find_sig2_interp(double time1, double time2, TermStruct* l)
{
    ...
    while ((ls1 != NULL) && (((IrmTermStructVal*)ls1->element->val.pval)->time < time1))
    {
        ls1 = ls1->next;
    }
    if (ls1 == NULL)
    ...
    while ((ls2 != NULL) && (((IrmTermStructVal*)ls2->element->val.pval)->time < time2))
    {
...
void G_H_func(double t, TermStruct* l, double* val_G, double* val_H)
{
    ...
    while ((ls != NULL) && (((IrmTermStructVal*)ls->element->val.pval)->time < t))
        ls = ls->next;
    if (ls == NULL)
        ls = l->tail;
    ...
    *val_G = tsval->G + 0.5 * temp3 * (tsval->sig * tsval->sig) / (tsval->F * tsval->F);
...
double srt_f_lgm_cum_vol(
    TermStruct* ts, double fixing_time, double start_time, double end_time, double* answer)
{
...

{
    ...
    temp   = (1 - exp(-time / tsval->tau)) * tsval->tau;
    result = tsval->K + tsval->F * tsval->J * temp + tsval->sig * tsval->tau * (time - temp);
...

{
    ...
    result = tsval->L + tsval->F * tsval->G * (1 - exp(-time / tsval->tau)) * tsval->tau +
             0.5 * pow(tsval->sig * tsval->tau, 2) *
                 (exp(-time / tsval->tau) + exp(time / tsval->tau) - 2) / tsval->F;
...

{
    ...
    result = tsval->O +
             tsval->F * tsval->G * tsval->Psi * (1 - exp(-time / tsval->tau)) * tsval->tau +
             tsval->F * tsval->F * tsval->G * tsval->tau *
                 ((1 - exp(-time / tsval->tau)) * tsval->tau -
                  0.5 * (1 - exp(-2 * time / tsval->tau)) * tsval->tau) +
             0.5 * (tsval->Psi / tsval->F) * tsval->tau * tsval->sig * tsval->sig *
...
Err srt_f_extend_lgm_jumping_ts_for_quanto(
    TermStruct* lgm_ts, TermStruct* fx_ts, char* szLgmUndName, char* szFxUndName)
{
    ...
    err = srt_f_get_corr_from_CorrList(
        corrlist, szLgmUndName, szFxUndName, lgm_tsval->time, &dCorrelation);
    if (err)
        return err;
    ...
    /* Now that all FX and Correlation dates have been inserted, compute the M function on the LGM
     * Ts Dates */
    while (lgm_lc->next != NULL)
    {
        ...
        /* Gets the Correlation between the LGM und and the FX from the previous date to the current
         * one*/
        err = srt_f_get_corr_from_CorrList(
            corrlist, szLgmUndName, szFxUndName, lgm_tsval->time, &dCorrelation);
        if (err)
        ...
        lgm_tsval->M = lgm_tsval_p->M +
                       lgm_tsval_p->dFxVolTimesCor * (lgm_tsval_p->sig / lgm_tsval_p->F) * temp;
    }
...
double find_struct_interp(double time, LabelStruct label, TermStruct* l)
{
    ...
    /* Gets the closest (relevant) IrmTermStructVal that has a date bigger than t1 */
    while ((ls1 != NULL) && (((IrmTermStructVal*)ls1->element->val.pval)->time < time))
    {
        ls1 = ls1->next;
    }

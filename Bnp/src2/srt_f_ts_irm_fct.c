...
double M_eta_beta_func(double s, double theta, double dOmega, double **M) {
  ...
  */
  ...
  /*
     find nodal values for s  , theta. Currently hardwired so 0<s<1 delta_s =
     .01 and 0<theta<10  , delta_theta = .1
  ...
  */
  ...
  /*	M_stheta =  lq_inter_2d (s  , theta  , s_arr  , theta_arr  , M  , SMAX
   * , THETAMAX);
   */
  /* deallocate memory */
...
/* Descript: interpolates on (sig^2)*t  , computing the variance from the
                required dates up to the relevant IrmTermStructVal  , and then
   computing the variance from the first IrmTermStructVal to the second one
*/
...
Err srt_f_extend_lgm_jumping_ts_for_quanto(TermStruct *lgm_ts,
  ...
  /* Now that all FX and Correlation dates have been inserted  , compute the M
   * function on the LGM Ts Dates */

...
Err srt_f_get_vasicek_mean_sr(double		time,
							   TermStruct	*ts,
							   double		*vasicek_mean_sr)
{
	...
	while ( (ls!=NULL) &&
		( ((IrmTermStructVal*)ls->element->val.pval)->time < time) ) ls = ls->next;

	if ( ls == NULL ) ls = ts->tail;
	...
	(*vasicek_mean_sr) = tsval->vasicek_mean_sr + (tsval->mean_rev_level/tsval->F)*(exp(time / tsval->tau)-1);
...
Err  srt_f_get_vasicek_mean_int_sr(double		time,
								   TermStruct	*ts,
								   double		*vasicek_mean_int_sr)
{
	...
	(*vasicek_mean_int_sr) = tsval->vasicek_mean_int_sr + (tsval->vasicek_mean_sr*tsval->F - tsval->mean_rev_level)*exp_term
							 + tsval->mean_rev_level*time;

	return err;

}
Err srt_f_get_vasicek_var_int_sr(double			time,
								TermStruct		*ts,
								double			*vasicek_var_int_sr)
{
	...
	(*vasicek_var_int_sr) =  (tsval->L + tsval->F*tsval->G*(1- exp(-time/tsval->tau))*tsval->tau
							 + 0.5*pow(tsval->sig*tsval->tau,2)*(exp(-time/tsval->tau) - 2)/tsval->F);

	(*vasicek_var_int_sr) *= dom_psi;

	(*vasicek_var_int_sr) -= (tsval->O + tsval->F*tsval->G*tsval->Psi*(1-exp(-time/tsval->tau))*tsval->tau
							  + tsval->F*tsval->F*tsval->G*tsval->tau*( (1-exp(-time/tsval->tau))*tsval->tau - 0.5*(1-exp(-2*time/tsval->tau))*tsval->tau )
							  - 0.5*(tsval->Psi/tsval->F)*tsval->tau*tsval->sig*tsval->sig* (1-exp(-time/tsval->tau))*tsval->tau 
							  - 0.5*pow(tsval->sig*tsval->tau,2)*(1-exp(-time/tsval->tau))*tsval->tau
							  + 0.5*pow(tsval->sig*tsval->tau,2)*( 0.5*(1-exp(-2*time/tsval->tau))*tsval->tau - time) );
	
	(*vasicek_var_int_sr) += 0.5*pow(tsval->sig*tsval->tau,2)*tsval->Psi/tsval->F;
...
Err srt_f_get_vasicek_init_cond(double			time,
								TermStruct		*ts,
								double			*vasicek_init_cond)
{
	...
	if (ls!=NULL)  
		(*vasicek_init_cond) = ((IrmTermStructVal*)ls->element->val.pval)->vasicek_init_cond;
	
	else return serror("the vasicek term struct. is not initialised ");

	return NULL; 
	
}

static Err  srt_f_get_lgm_mean_int_sr(double		time,
									  TermStruct	*ts,
									  double        *mean_int_nom_sr)
{
	(*mean_int_nom_sr) = 0.0;
	(*mean_int_nom_sr) = - Psi_func(time,ts)*L_func(time,ts) + O_func(time,ts);

	return NULL;
}

static Err srt_f_get_lgm_var_int_sr(double		time,
									TermStruct	*ts,
									double		*var_int_nom_sr)
{
	(*var_int_nom_sr) = 2*(Psi_func(time,ts)*L_func(time,ts)-O_func(time,ts));

	return NULL;
}
	

Err srt_f_vasicek_discount_factor(Date		discount_mat_date,
								  SrtUndPtr	und,
								  double	*discount_factor)
{
	...
	err = srt_f_get_vasicek_var_int_sr(discount_factor_mat,
										ts,
										&var_int_sr);
	if(err) return err;
	...
	err = srt_f_get_vasicek_init_cond(discount_factor_mat,
									  ts,
									  &vasicek_init_cond);
	if(err) return err;

	err = srt_f_get_vasicek_mean_int_sr(discount_factor_mat,
										ts,
										&vasicek_mean_int_sr);
	if(err) return err;
...
Err srt_f_vasicek_fra(Date		start_date,
					  Date		end_date,
					  SrtUndPtr	und,
					  double	*fra)
{
	...
	err = srt_f_vasicek_discount_factor(start_date,
										und,
										&df_start);
	 if(err) return err;

	 err = srt_f_vasicek_discount_factor(end_date,
										 und,
										 &df_end);
	 if(err) return err;
...
Err srt_f_vasicek_swap(Date		start_date,
					   Date		theo_end_date,
					   char		*swap_freq_str,
					   char		*swap_basis_str,
					   SrtUndPtr und,
					   double	*swap)
{
	...
	/* Set the SwapDP */
	err = swp_f_setSwapDP(start_date, 
						  theo_end_date, 
						  swap_freq_code, 
						  swap_basis_code, 
						  &swp_dp);
	...
	err = srt_f_vasicek_discount_factor(pay_dates[num_dates],
										und,
										&df_end);
	if(err) return err;
	...
	{
		err = srt_f_vasicek_discount_factor(pay_dates[i],
											und,
											&df);
...
Err srt_f_vasicek_fut(double		start_date,
					  double		pay_date,
					  ...
					  double		*fut_pr)
{
	...
	adjustement = 0.0;
	adjustement += Psi_start_pay*Psi_start_pay*G_start + Psi_start_pay*L_func(start_time,und_ts);

	err = srt_f_vasicek_discount_factor((long)start_date,
										und,
										&df_start);
	if(err) return err;

	err = srt_f_vasicek_discount_factor((long)pay_date,
										und,
										&df_pay);
	if(err) return err;

	(*fut_pr) = ((df_start/df_pay)*exp(adjustement)-1)/coverage((Date)start_date,(Date)pay_date,BASIS_ACT_360);
...
Err srt_f_vasicek_market_libor_expectation(double	start_date,
										   double	pay_date,
										   double	risk_premuim,
										   char		*vasicek_und_name,
										   double   *market_libor_expectation)
{
	...
	(*market_libor_expectation) = ((df_start/df_pay)*exp(adjustement)-1)/coverage((Date)start_date,(Date)pay_date,BASIS_ACT_360);
...
Err srt_f_vasicek_cont_fwd_zr(double 	fixing_date,
							  double	pay_date,
							  SrtUndPtr und,
							  double	    *cont_fwd_zr)
{
	...
	err = srt_f_vasicek_discount_factor((long)fixing_date,
										und,
										&df_fixing);
	if(err) return err;
	...
	(*cont_fwd_zr) = -log(df_pay/df_fixing)/((pay_date-fixing_date)*YEARS_IN_DAY);

	return NULL;
}

Err  srt_f_vasicek_dirty_pr(SrtUndPtr	und,
							long	    num_dates,
							double      settlt_date,
							double	    *pay_dates,
							double	    *cpns,
							...
							double	    *dirty_price)
{
	...
	{
		err = srt_f_vasicek_discount_factor((long)pay_dates[i],
											und,
											&df);
		if(err) return err;
...
Err srt_f_vasicek_overnight_interest_rate_swap(char			*und_name,
											   long		    start_date,
										       long		    end_date,
											   ...
											   double		*overnight_interest_rate_swap)
{
	...
	double      Psi_t,vasicek_mean_sr_t,vasicek_mean_int_sr_t,vasicek_mean_int_sr_pay_T;
	double		G_t,H_t,G_pay_T,H_pay_T;
	double      start_t_cond_var,start_t_cond_mean,fwd_measure_sr_mean,vasicek_init_cond,sr_coeff;
	double		sr_variance;
	TermStruct	*ts;
	SrtUndPtr	und;

	und = lookup_und(und_name);
	if(!und) return serror("can not find the underlying");
	...
	err = srt_f_get_vasicek_mean_int_sr(start_t,
										ts,
	...
	err = srt_f_get_vasicek_mean_sr(start_t,
									ts,
									&vasicek_mean_sr_t);
	if(err) return err;
	...
	/* start_t_cond_mean is the start_date cond mean of the integral of the sr (forward measure) */
	...
	start_t_cond_mean -= Psi_func(pay_T,ts)*(I_func(pay_T,ts)-I_func(start_t,ts));
	start_t_cond_mean += Psi_func(pay_T,ts)*G_t*(Psi_func(pay_T,ts)-Psi_func(start_t,ts));
	start_t_cond_mean -= S_func(start_t,ts)*(Psi_func(pay_T,ts)-Psi_func(start_t,ts));
	...
	start_t_cond_var = Psi_func(pay_T,ts)*Psi_func(pay_T,ts)*(G_pay_T-G_t);

	start_t_cond_var -= 2*Psi_func(pay_T,ts)*(S_func(pay_T,ts)-S_func(start_t,ts));
	start_t_cond_var += (G_pay_T*Psi_func(pay_T,ts)*Psi_func(pay_T,ts)-2*O_func(pay_T,ts));
	start_t_cond_var -= (G_t*Psi_t*Psi_t-2*O_func(start_t,ts));

	/* compute the mean of the sr (fwd measure) */

	err = srt_f_get_vasicek_init_cond(start_t,
									  ts,
									  &vasicek_init_cond);
	if(err) return err;
	...
	fwd_measure_sr_mean -= F_func(start_t,ts)*Psi_func(start_t,ts)*(G_pay_T-G_t);
	fwd_measure_sr_mean += F_func(start_t,ts)*(S_func(pay_T,ts)-S_func(start_t,ts));


	/* compute the short rate coeff. */

	sr_coeff = 0.0;
	sr_coeff = (Psi_func(pay_T,ts)-Psi_func(start_t,ts))/F_func(start_t,ts);

	sr_variance = 0.0;
	sr_variance = F_func(start_t,ts)*F_func(start_t,ts)*G_t;


	(*overnight_interest_rate_swap) = exp(start_t_cond_mean + 0.5*start_t_cond_var);

	(*overnight_interest_rate_swap) *= exp(sr_coeff*fwd_measure_sr_mean + 0.5*sr_coeff*sr_coeff*sr_variance);

	(*overnight_interest_rate_swap) -= 1;

	(*overnight_interest_rate_swap) *= 1.0/coverage(start_date,end_date,BASIS_ACT_360);

	return NULL;

}

static Err one_bond_calib_func(char			*model,
							   SrtMdlType	mdl_type,
							   SrtMdlDim	mdl_dim,
							   char			*vasicek_und_name,
							   char			*yc_name,
							   ...
							   double		*dirty_price)
{
	...
	und = lookup_und(vasicek_und_name);
	if(!und) return serror("can not find underlying");
	...
	err = SrtInitIRUnd(vasicek_und_name,
					   yield_curve_name,
					   model,
					   num_sig,
					   2,
					   ...
					   num_bond,
					   2,
					   vasicek_mean_rev_level_data);
	
	if(err) return err;

	(*new_und) = lookup_und(vasicek_und_name);
	if(!(*new_und) ) return serror("can not find underlying");

	err = srt_f_vasicek_dirty_pr((*new_und),
								 num_cpns[bond_index],
								 settlt_date,
								 pay_dates[bond_index],
								 cpns[bond_index],
	...
	{
		err =  srt_f_vasicek_discount_factor((long)(pay_dates[bond_index][num_cpns[bond_index]-1]),
										     (*new_und),
										     &real_df_fixing);
		...
		err =  srt_f_vasicek_discount_factor(spot_date,
										     (*new_und),
										     &real_df_spot);
		...
		floor_price = srt_f_optblksch(forward,
									  floor_strike[bond_index],
									  floor_implied_vol[bond_index],
									  (pay_dates[bond_index][num_cpns[bond_index]-1]-date_today)*YEARS_IN_DAY,
									  1.0,
									  SRT_PUT,
									  PREMIUM);
...
Err srt_f_vasicek_calibrate(double	settlt_date,
							double	vasicek_init_cond,
							long	num_bonds,
							long	*num_cpns,
							double  **pay_dates,
							...
							char	**vasicek_und_name)
{
	...
	und = lookup_und((*vasicek_und_name));
	if(!und) return serror("fatal: can not get underlying");
	...
	for(i = 0; i < num_bonds; i++)
	{
		...
		if( i > 0) yc_name = get_discname_from_underlying(new_und);

		err = one_bond_calib_func("VASICEK",
								  mdl_type,
								  mdl_dim,
								  (*vasicek_und_name),
								  yc_name,
	...
	if(sig_data) free_dmatrix(sig_data,0,1,0,num_sig-1); sig_data = NULL;
	if(tau_data) free_dmatrix(tau_data,0,1,0,num_tau-1); tau_data = NULL;
	if(vasicek_mean_rev_level_data) free_dmatrix(vasicek_mean_rev_level_data,0,1,0,num_bonds-1); vasicek_mean_rev_level_data = NULL;
	
	return NULL;

}

Err srt_f_vasicek_calibrated_mean_rev_lvl(double		settlt_date,
										  double		vasicek_init_cond,
										  long			num_bonds,
										  long			*num_cpns,
										  double		**pay_dates,
										  ...
										  double		***calibrated_mean_rev_lvl)
{
	...
	err = srt_f_vasicek_calibrate(settlt_date,
								  vasicek_init_cond,
								  num_bonds,
								  num_cpns,
								  pay_dates,
	...
	for(i = 0; i < num_bonds; i++)
	{
		...
		(*calibrated_mean_rev_lvl)[i][1] = find_mean_rev_level((pay_dates[i][num_cpns[i]-1]-date_today)*YEARS_IN_DAY,ts);
									 ...
									 char		*und_name)
{
	...
	calibration_parms.levels_cols = calibration_parms.sig_cols = calibration_parms.tau_cols = 2;
	...
	err = srt_f_get_vasicek_init_cond(0.0,
									  ts,
									  &vasicek_init_cond);
	if(err) 
		return err;
...
static Err delete_static_params_struct(struct calibration_struct	*calibration_parms)
{
	...
	{
		free_dmatrix(tmp_calibration_parms.mean_rev_levels_data,0,1,0,num_levels-1);
...
static Err from_optparam_to_calibration_params(double						*optim_params,
											   long							num_parms,
			...
			int     num_params)
{
	...
	err = from_optparam_to_calibration_params(optim_params,
										  num_params,
										  &calibration_parms);
	if(err) 
		return err;
	...
	err = srt_f_init_IRM_TermStruct(calibration_parms.today,
									calibration_parms.sig_data,
									calibration_parms.sig_cols,
									calibration_parms.num_sig,
									calibration_parms.tau_data,
									calibration_parms.tau_cols,
									calibration_parms.num_tau,
									
									calibration_parms.mdl_type,
									calibration_parms.mdl_dim,
									...
									calibration_parms.vasicek_init_cond,
									calibration_parms.num_levels,
									calibration_parms.levels_cols,
									calibration_parms.mean_rev_levels_data,
	...
	err  = srt_f_vasicek_overnight_interest_rate_swap(calibration_parms.und_name,
													  calibration_parms.spot_date,
...
static Err levenberg_calib_funcs(double		instr_index,
								 double		optim_parms[],
								 double     *value,
								 double     deriv[],
								 int		num_params)
{
	...
	err = levenberg_price_funcs(instr_index,
								optim_parms,
								value,
								num_params);
	if(err) 
									   ...
									   char	    **und_name)
{
	...
	err = init_static_params_struct(overnights_swap_end_dates,
									 num_overnight_swap_rates,
									 market_overnight_swap_rates,
									 overnight_swap_rates_weights,
									 *und_name);
	if(err) 
		return err;
	...
	err = levenberg_marquardt(data,
							  market_overnight_swap_rates,
							  overnight_swap_rates_weights,
							  num_overnight_swap_rates,
							  params,
							  (num_overnight_swap_rates + 1),
							  niter,
	...
	if(data) free_dvector(data,1,num_overnight_swap_rates);data = NULL;
	if(params) free_dvector(params,1,num_overnight_swap_rates + 1); params = NULL;
...
Err srt_f_vasicek_money_market_account_vol(Date		date,
										   char		*vasicek_und_name,
										   double	*money_market_account_vol)
{
	...
	und = lookup_und(vasicek_und_name);
	if(!und) return serror("fatal: can not get the underlying in srt_f_vasicek_money_market_account_vol");
	...
	err = srt_f_get_vasicek_var_int_sr(date_exp,
										ts,
										&var_int_sr);
	if(err) return err;
...
Err  srt_f_inflation_fwd(long	start_date,
						 long	end_date,
						 char	*price_index_und_name,
						 double *inflation_fwd)
{
	Err			err = NULL;
	...
	double		df_nom_end_date,df_nom_start_date,df_real_end_date,df_real_start_date;
	double		adjustment,G_real_start,H_real_start,Psi_real_start_end,Psi_nom_start_end;
	...
	df_nom_end_date = swp_f_df(date_today,
								end_date,
								nom_rate_yc);

	df_nom_start_date = swp_f_df(date_today,
								start_date,
								nom_rate_yc);
	...
	Psi_real_start_end = Psi_func(end_date_time,real_rate_ts)-Psi_func(start_date_time,real_rate_ts);
	Psi_nom_start_end = Psi_func(end_date_time,nom_rate_ts)-Psi_func(start_date_time,nom_rate_ts);

	adjustment += Psi_real_start_end*(Psi_func(start_date_time,real_rate_ts)*G_real_start-L_func(start_date_time,real_rate_ts)-Psi_func(end_date_time,real_rate_ts)*G_real_start);
	...
	adjustment += Psi_real_start_end*(Psi_func(end_date_time,nom_rate_ts)*O_fd_func(start_date_time,price_index_ts)-R_fd_func(start_date_time,price_index_ts));
	adjustment -= Psi_real_start_end*Psi_nom_start_end*O_fd_func(start_date_time,price_index_ts);

	/*compute the fwd inflation */

	(*inflation_fwd) = (df_nom_start_date/df_nom_end_date)/(df_real_start_date/df_real_end_date)*exp(adjustment);
...
int             i;
	...
	if (dOmega == 1.)
	{
		M_stheta = .5*theta*theta*
		            (exp(s)-1.)*
					(1.-
					 .5*theta/3.*((exp(s)-1.)*(exp(s)+2.))+
	...
	/* allocate memory Probably should think of a better way
	   ...
	   precomputed, stored on the termstruct ts, and accessed with a
	   func
	*/
	...
	/*
	   find nodal values for s, theta. Currently hardwired so 0<s<1 delta_s = .01
	   and 0<theta<10, delta_theta = .1 
...
/*	M_stheta =  lq_inter_2d (s, theta, s_arr, theta_arr, M, SMAX, THETAMAX);
*/	
	/* deallocate memory */
...
/*	if (l == NULL) ENTER_DEBUG;  [MN] Not needed */
	...
	return ( tsval->Psi + tsval->F * tsval->tau *
		( 1 - exp( ( - time ) / tsval->tau )));
}
...
/* Descript: interpolates on (sig^2)*t, computing the variance from the 
		required dates up to the relevant IrmTermStructVal, and then computing
		the variance from the first IrmTermStructVal to the second one 
*/
...
/* Gets the closest (relevant) IrmTermStructVal that has a date bigger than t2 */
	...
	{ 
	/* ind represents the number of IrmTermStructVal that separates time1 and 2*/
		ind++; 
		ls2 = ls2->next; 
	}
...
/* H_func def: int(s = 0, s = t) [-s_ifr(s,t)*s_bond(s,t)] ds = F(t)*I(t) */

void G_H_func ( 	double t,
			TermStruct* l,
			double *val_G,
			double *val_H
	)
...
IrmTermStructVal* tsval, * tsval_p; /* _p for previous */
	...
	while ( (ls!=NULL) &&
	( ((IrmTermStructVal*)ls->element->val.pval)->time < t) ) ls = ls->next;
	if ( ls == NULL ) ls = l->tail;	
	...
	*val_G = tsval->G + 0.5 * temp3 * ( tsval->sig * tsval->sig ) 
				 / ( tsval->F * tsval->F );
...
double I_func(double time,TermStruct *l)
{
	...
	result = tsval->I+tsval->G*tsval->F*temp
			+(tsval->sig*tsval->tau)*(tsval->sig*tsval->tau)*temp1/(2*tsval->F);
...

{
	...
	result = tsval->L+tsval->F*tsval->G*(1- exp(-time/tsval->tau))*tsval->tau
			+ 0.5*pow(tsval->sig*tsval->tau,2)*(exp(-time/tsval->tau)+exp(time/tsval->tau)-2)/tsval->F;	   
...

{
	...
	result= tsval->O + tsval->F*tsval->G*tsval->Psi*(1-exp(-time/tsval->tau))*tsval->tau
			   + tsval->F*tsval->F*tsval->G*tsval->tau*( (1-exp(-time/tsval->tau))*tsval->tau - 0.5*(1-exp(-2*time/tsval->tau))*tsval->tau )
			   + 0.5*(tsval->Psi/tsval->F)*tsval->tau*tsval->sig*tsval->sig*( (exp(time/tsval->tau)-1)*tsval->tau - (1-exp(-time/tsval->tau))*tsval->tau )
			   + 0.5*pow(tsval->sig*tsval->tau,2)*( (exp(time/tsval->tau)-1)*tsval->tau - (1-exp(-time/tsval->tau))*tsval->tau   )
			   + 0.5*pow(tsval->sig*tsval->tau,2)*( 0.5*(1-exp(-2*time/tsval->tau))*tsval->tau - time);
...

{
	...
	if (ls != l->head) 
	{
		...
		result = tsval->Q+tsval->H*temp+tsval->F*tsval->F*tsval->G*tsval->tau*(temp-temp1)
				+0.5*tsval->sig*tsval->sig*tsval->tau*tsval->tau*(time-2*temp+temp1);
...

{
	...
	if (ls != l->head) 
	{
		...
		result = tsval->S+(tsval->sig*tsval->sig/(tsval->F*tsval->F))*tsval->Psi*temp1
...
double T_func(double time,TermStruct *l)
{
	...
	result = tsval->T + tsval->F*tsval->S*temp
			 + 0.5*tsval->sig*tsval->sig*tsval->Psi/tsval->F*tsval->tau*(temp1-temp)
...
Err srt_f_extend_lgm_jumping_ts_for_quanto( 
			TermStruct   *lgm_ts,
			TermStruct   *fx_ts,
			char         *szLgmUndName,
			char         *szFxUndName)
...
/* Loops on the Fx dates to insert them into the IRM Term Struct */
	...
	/* Moves on to the IRM date just after (or on) this date */
		while((lgm_lc!=NULL) &&
			( ((IrmTermStructVal*)lgm_lc->element->val.pval)->time < fx_tsval->time) ) 
			lgm_lc = lgm_lc->next;
		if(lgm_lc ==NULL) lgm_lc = lgm_ts->tail;
...
/* Gets the Correlation between the LGM und and the FX one */
	err = srt_f_get_corr_from_CorrList( corrlist,szLgmUndName, szFxUndName,lgm_tsval->time, &dCorrelation);
	if (err)
		return err;
...
/* Now that all FX and Correlation dates have been inserted, compute the M function on the LGM Ts Dates */
	...
	/* Gets the Correlation between the LGM und and the FX from the previous date to the current one*/
		err = srt_f_get_corr_from_CorrList( corrlist,szLgmUndName, szFxUndName,lgm_tsval->time, &dCorrelation);
		if (err)
			return err;
	...
	/* Computes and stores the M function (quanto adjustment term) (on the end of the period) */
		...
		lgm_tsval->M =  lgm_tsval_p->M + lgm_tsval_p->dFxVolTimesCor 
				* ( lgm_tsval_p->sig / lgm_tsval_p->F) * temp;
...
/*                 are not the same. We lineary interpolate those function between two dates
/*                 for t > tn   H(t) = Hn-1 + (Hn - Hn-1) (t - tn-1) / (tn - tn-1)

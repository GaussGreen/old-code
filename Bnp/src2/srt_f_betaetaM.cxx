...
Err make_M_matrix(TermStruct *l) {
  ...
  Since s and theta are a combo of lambda        ,beta        , etc. we can
  choose the easiest combo of lambda        ,beta        , etc. to compute and
  store M(s        ,theta). So we set  beta=1        , x=0 which makes s
  dependent only on delta_tau and theta dependent only on lambda. */
  ...
  while (s < SMAX) {
    ...
    while (theta < THETAMAX) {
      ...
      /* With beta = 1        , x = 0 -> delta_tau = s/SMAX        , lambda =
       * theta/THETAMAX
       */
      ...
      if ((s == 0) || (theta == 0)) /* zero soln on boundary     */
        ...
        if (0 <= eta && eta < .5)
          /* Case 1:  No barrier b.cxx.'s */
          M[s][theta] = log(sm_qsimp(integrand_1, a, b, EPS_SIMPSO * .0000001));
        else if (.5 <= eta && eta < 1) { /*  Case 2 Natural b.cxx.'s */
          nu = 1. / (2 * (1. - eta));
      ...
      /*						err = calc_bigM(M , beta
         , eta        , lambda        , delta_tau        , x        , s_index ,
         theta_index); if (err) return err;
      */
      theta++;
    }
    s++;
...
static double integrand_2(double y_bar) {
  ...
  /* Computes the integrand for Case 2: 0<=STATIC_ETA<.5  .5 <= nu < 1
     with reflecting b.cxx.'s  This routine is currently not used

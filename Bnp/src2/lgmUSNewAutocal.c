...
/* ----------------------------------------------------------------------------------------------------------------------------------
 */
/* Routine called by CompVal_ForwardsTimeSwaps in order to calculate the PV of one coupon of the
 * forward time swaps */
LGMErr CompValCouponTimeSwap(
    ...
    long*    index_discret_obsfreq, /* output:  indices between vol discreization dates and Libor
                                       observation dates */
    ...
    double* forward_pvPeriod)       /* output:  value of the accrual and funding legs */
{
    ...
    for (j = 0; j < obsfreq; j++)
        tcms_subperiod[j] =
            tfras_subperiod[j] *
            exp(vol_for_adj_cms * vol_for_adj_cms * fra_for_adj_cms * tmaturities_subperiod[j] *
                (deal->observationdays[index_coupon][j][1] - deal->tCpnPay[index_coupon] + 0.0) /
                365.0);
    ...
    for (index = 0; index < num_subdiscret_obsfreq; index++)
    {
        ...
        error = GetVol_FudgeTimeSwap(
            tNow, tNow, Start, End, strikeup, tfras_subperiod[l], deal, &(tvols_discr[index][1]));
        error = GetVol_FudgeTimeSwap(
            tNow,
            tNow,
            Start,
    ...
    {
        if (l > index_discret_obsfreq[index + 1]) /* if we have passed the vol observation point,
        ...
        if (tmaturities_subperiod[index_discret_obsfreq[index + 1]] ==
            ...
            xx = (tmaturities_subperiod[l] - tmaturities_subperiod[index_discret_obsfreq[index]]) /
                 (tmaturities_subperiod[index_discret_obsfreq[index + 1]] -
                  tmaturities_subperiod[index_discret_obsfreq[index]]);
        /* ratio of time between discretization and previous observation point and time between
    ...
    for (j = 1; j <= obsfreq; j++)
    {
        ...
        callupshift = srt_f_optblksch(
            fra_subperiod, strikeupshift, volupshift, maturity, 1, SRT_CALL, PREMIUM);
        ...
        else
        {
            ...
            /* put spread */
            putlow =
                srt_f_optblksch(fra_subperiod, strikelow, vollow, maturity, 1, SRT_PUT, PREMIUM);
            putlowshift = srt_f_optblksch(
                fra_subperiod, strikelowshift, vollowshift, maturity, 1, SRT_PUT, PREMIUM);
        ...
        else /* Floating digital */
        {
            ...
            callup_adj = srt_f_optblksch(
                fra_subperiod * adjustmentup, strikeup, volup, maturity, 1, SRT_CALL, PREMIUM);
            callupshift_adj = srt_f_optblksch(
                fra_subperiod * adjustmentup,
                strikeupshift,
                volupshift,
                maturity,
                1,
        ...
        expected_accrued_ratio_adj +=
            proba_between_adj * (deal->ratiodays_for_subperiod)[index_coupon][j - 1];
    }
    ...
    /* Floating Digital */
    pvCpn += dfPay * deal->tCvgCpn[index_coupon] * deal->gear[index_coupon] * fra_accrued *
             expected_accrued_ratio_adj;
    ...
    if (deal->tCpnPay[index_coupon] < tNow + eod_pay_flag)
    {
        ...
        pvCpn += dfPay * deal->tCvgCpn[index_coupon] * deal->gear[index_coupon] * floatingFixing *
                 (noDaysAccrued + 0.0) /
                 (deal->tCpnPay[index_coupon] - deal->tCpnStart[index_coupon] + 0.0);
    }
...
/* -----------------------------------------------------------------------------------------------------------------------------------
 */
/* Routine called by LGMExtractInfoFromDeals to get out forward value fo the time swaps for possible
 * use in determining the equivalent */
/* strikes */
    ...
    double             floatingfixing)                /* (0) */
{
    ...
    for (index_coupon = deal->nCpn - 1; index_coupon >= deal->iSet[0]; index_coupon--)
    {
        ...
        err = CompValCouponTimeSwap(
            ...
            tmaturities_subperiod, /* out: the time (from today) to the fixing for each Libor
                                      observation dates */
            tvols_subperiod, /* out: the log volatilities (interpolated) for each Libor observation
                                date */
            tvols_discr,     /* out: the log volatilities at the volatility discretization dates */
            index_discret_obsfreq, /* out: the map from the discretization dates to the observation
                                      dates */
        ...
        /* loop over the exercise dates before the current coupon adding the pv to the (earlier)
         * forward time swap */
        for (index_exo = 0; (deal->iSet[index_exo] < index_coupon) && (index_exo < nEx);
             index_exo++)
            forwardsTimeSwaps[index_exo] += forward_pvPeriod;

        /* if the exercise index is equal to the index coupon, then we have completed the forward
         * time swap */
        if (deal->iSet[index_exo] == index_coupon)
        {
            /* add in the pv.  The forwardTS is the sum of the accrual coupons, the float margins,
             * the final DF minus the start DF */
            forwardsTimeSwaps[index_exo] += forward_pvPeriod;

            /* calculate the discount factor at the start of the forward time swap */
            dfSt = swp_f_df(tNow, deal->tSet[index_exo], ycName);

            /* calculate the ratio (notice index is 1 higher) of the fixed PV to the floating PV
               (including the final but not the initial notional repayment). */
            ratios[index_exo + 1] = (forwardsTimeSwaps[index_exo] + dfSt) / dfSt;

            /* If the ratio is too high or too low, set it to 0 (not sure what the impact of this
             * is) */
    ...
    double floatingFixing /* fixing at the beginning of the period */) /* basis for rates */
{
    ...
    double       vollow, vollowshift, volup, volupshift;
    double       maturity;
    double       proba_above, proba_above_adj, proba_below, proba_below_adj, proba_between,
        proba_between_adj;
    double   expected_accrued_ratio, expected_accrued_ratio_adj;
    double   callup, callupshift, callup_adj, callupshift_adj;
    double   putlow, putlowshift, putlow_adj, putlowshift_adj;
    ...
    {
        dfPay = swp_f_df(
            tNow, deal->tCpnPay[i - 1], ycName); /* calculate discount factor to pay date */
        ...
        error             = GetVol_FudgeTimeSwap(
            tNow,
            tNow,
            deal->observationdays[i - 1][index_for_adj_cms][0],
            deal->observationdays[i - 1][index_for_adj_cms][1],
            fra_for_adj_cms,
            fra_for_adj_cms,
            deal,
            &vol_for_adj_cms);
        for (j = 1; j <= obsfreq; j++)
        {
            tcms_subperiod[j - 1] =
                ...
                exp(vol_for_adj_cms * vol_for_adj_cms * fra_for_adj_cms *
                    ...
                    (deal->observationdays[i - 1][j - 1][1] - deal->tCpnPay[i - 1] + 0.0) / 365.0);
        }
        ...
        for (j = 1; j <= obsfreq; j++)
        {
            ...
            /* call spread */
            callup =
                srt_f_optblksch(fra_subperiod, strikeup, volup, maturity, 1, SRT_CALL, PREMIUM);
            callupshift = srt_f_optblksch(
                fra_subperiod, strikeupshift, volupshift, maturity, 1, SRT_CALL, PREMIUM);
            ...
            else
            {
                ...
                putlow = srt_f_optblksch(
                    fra_subperiod, strikelow, vollow, maturity, 1, SRT_PUT, PREMIUM);
                putlowshift = srt_f_optblksch(
                    fra_subperiod, strikelowshift, vollowshift, maturity, 1, SRT_PUT, PREMIUM);
...
expected_accrued_ratio_adj = 1;   */
        ...
        {
            pvCpn += dfPay * deal->tCvgCpn[i - 1] * deal->tCpn[i - 1] * (noDaysAccrued + 0.0) /
                     (deal->tCpnPay[i - 1] - deal->tCpnStart[i - 1] + 0.0);
            pvCpn += dfPay * deal->tCvgCpn[i - 1] * deal->gear[i - 1] * floatingFixing *
                     (noDaysAccrued + 0.0) / (deal->tCpnPay[i - 1] - deal->tCpnStart[i - 1] + 0.0);
        }

...
LGMErr CashRate( Date tNow, Date Start, Date End, String ycName, double *outFwd )
{
	double dfSt, dfEnd;
...
/* ---------------------------------------------------------------------------------------------------------------------------------- */
/* Routine called by CompVal_ForwardsTimeSwaps in order to calculate the PV of one coupon of the forward time swaps */ 
LGMErr CompValCouponTimeSwap(
					...
					long				index_coupon,			/* the index of the coupon being calculated (actually it is index -1) */
					...
					double**			tvols_discr,			/* output:  market log vols at discretization points */
					long*				index_discret_obsfreq,	/* output:  indices between vol discreization dates and Libor observation dates */
					double*				forward_pvFixed,		/* output:  value of the accrual leg */
					double*				forward_pvPeriod)		/* output:  value of the accrual and funding legs */
{
	...
	double proba_above_adj, proba_below_adj, proba_between_adj;		/* Adjusted probs for floating component */
...
/* Check for coupons */
	if (deal->nCpn<1) return("no coupons");
...
/* Computes the Libor obsevation index corresponding to the vol discretisation */
	for (index=0; index<num_subdiscret_obsfreq; index++)
		index_discret_obsfreq[index] = (int) (index*(obsfreq-1.0)/(num_subdiscret_obsfreq-1.0));
...
/* if we have a floating digital component */
	if ( deal->gear[index_coupon] )		
	{
		...
		end_accrued = add_unit(start_accrued,deal->undAccrued,SRT_MONTH,MODIFIED_SUCCEEDING);
...
/* computes the fras at the observation days */ 
	for (j=1; j<=obsfreq; j++)
	{
		Start = deal->observationdays[index_coupon][j-1][0];	/* Start date of the fra */
		End = deal->observationdays[index_coupon][j-1][1];	/* End date of the fra */
...
/* Get the ATM log volatility for the fra at the final observation date */
	...
	error = GetVol_FudgeTimeSwap(
				tNow,
				tNow,
				deal->observationdays[index_coupon][index_for_adj_cms][0],
				deal->observationdays[index_coupon][index_for_adj_cms][1],
				fra_for_adj_cms,
				fra_for_adj_cms,deal,
				&vol_for_adj_cms);
/* For each FRA apply the correction CMS_j = FRA_j * exp[ sigma^2 FRA * tEx_j * (tEnd_j - tPay)  ] */
	for (j=0; j<obsfreq; j++)
		tcms_subperiod[j] = tfras_subperiod[j] * exp(  vol_for_adj_cms * vol_for_adj_cms * fra_for_adj_cms
									* tmaturities_subperiod[j] * ( deal->observationdays[index_coupon][j][1] -	
																		deal->tCpnPay[index_coupon]+0.0 )/365.0  );
...
/* Computes all vols for the period by interpolation of vols at discretisation points */
	...
	{
		if ( l>index_discret_obsfreq[index+1] ) /* if we have passed the vol observation point, move to the next one */
			index += 1;

		if ( tmaturities_subperiod[index_discret_obsfreq[index+1]] == tmaturities_subperiod[index_discret_obsfreq[index]])
			xx=0.5;								/* if the subperiods dates are the same.  This shouldn't really happen. */
		else
			xx = (tmaturities_subperiod[l] - tmaturities_subperiod[index_discret_obsfreq[index]])/
					(tmaturities_subperiod[index_discret_obsfreq[index+1]]-
					tmaturities_subperiod[index_discret_obsfreq[index]]);
				/* ratio of time between discretization and previous observation point and time between surrounding observation points */
		for (m=0;m<=3;m++) /* linear interpolation on vols */
			tvols_subperiod[l][m] = (1-xx) * tvols_discr[index][m] + xx * tvols_discr[index+1][m];
	}	
...
/* call spread */
		callup = srt_f_optblksch(fra_subperiod,strikeup,volup,maturity,1,SRT_CALL,PREMIUM);
		callupshift = srt_f_optblksch(fra_subperiod,strikeupshift,volupshift,maturity,1,SRT_CALL,PREMIUM);
...
/* put spread */
			putlow = srt_f_optblksch(fra_subperiod,strikelow,vollow,maturity,1,SRT_PUT,PREMIUM);
			putlowshift = srt_f_optblksch(fra_subperiod,strikelowshift,vollowshift,maturity,1,SRT_PUT,PREMIUM);
...
/* computes the correl (Libor,index) */
			correl = (maturity - tmaturities_subperiod[0])/(tmaturities_subperiod[obsfreq-1]-tmaturities_subperiod[0])*
							deal->CorrelEnd + 
						(tmaturities_subperiod[obsfreq-1] - maturity)/(tmaturities_subperiod[obsfreq-1]-tmaturities_subperiod[0])*
							deal->CorrelStart;
			adjustmentup = exp(correl*volLiborup*volup*tmaturities_subperiod[0]); /* Changes the adjustment */
			callup_adj = srt_f_optblksch(fra_subperiod*adjustmentup,strikeup,volup,maturity,1,SRT_CALL,PREMIUM);
			callupshift_adj = srt_f_optblksch(fra_subperiod*adjustmentup,strikeupshift,volupshift,maturity,1,SRT_CALL,PREMIUM);
			proba_above_adj = (callup_adj - callupshift_adj)/deal->call_spread_up;
			if (deal->barriers[index_coupon][0] == 0)
				proba_below_adj = 0;
			else
			{
				adjustmentlow = exp(correl*volLiborlow*vollow*tmaturities_subperiod[0]);  /* Changes the adjustment */
				putlow_adj = srt_f_optblksch(fra_subperiod*adjustmentlow,strikelow,vollow,maturity,1,SRT_PUT,PREMIUM);
				putlowshift_adj = srt_f_optblksch(fra_subperiod*adjustmentlow,strikelowshift,vollowshift,maturity,1,SRT_PUT,PREMIUM);
				proba_below_adj = (putlowshift_adj - putlow_adj)/deal->call_spread_low;   
			}
		}
		proba_between = 1 - proba_below - proba_above;
		proba_between_adj = 1 - proba_below_adj - proba_above_adj;		

		expected_accrued_ratio += proba_between * (deal->ratiodays_for_subperiod)[index_coupon][j-1];
		expected_accrued_ratio_adj += proba_between_adj * (deal->ratiodays_for_subperiod)[index_coupon][j-1];		
	}
		
/* Fix Coupon */
	pvCpn = dfPay * deal->tCvgCpn[index_coupon] * deal->tCpn[index_coupon] * expected_accrued_ratio;
		
/* Floating Digital */
	pvCpn += dfPay * deal->tCvgCpn[index_coupon] * deal->gear[index_coupon] * fra_accrued * expected_accrued_ratio_adj;
	
/* Past Fixings */
	if (deal->tCpnPay[index_coupon] <tNow + eod_pay_flag)
	{
		pvCpn += dfPay * deal->tCvgCpn[index_coupon]*deal->tCpn[index_coupon]*(noDaysAccrued+0.0)/(deal->tCpnPay[index_coupon] - deal->tCpnStart[index_coupon]+0.0);
		pvCpn += dfPay * deal->tCvgCpn[index_coupon]*deal->gear[index_coupon]*floatingFixing*(noDaysAccrued+0.0)/(deal->tCpnPay[index_coupon] - deal->tCpnStart[index_coupon]+0.0);
	}
...
/* ----------------------------------------------------------------------------------------------------------------------------------- */
/* Routine called by LGMExtractInfoFromDeals to get out forward value fo the time swaps for possible use in determining the equivalent */
/* strikes */
...
/* Memory allocation */
	...
	for ( index_coupon = deal->nCpn-1; index_coupon >= deal->iSet[0]; index_coupon-- )
	{
/* compute a number of quantities for each coupon */
		err = CompValCouponTimeSwap(
					...
					tcms_subperiod,					/* out: the adjusted fwd rates at the Libor observation dates */
					tmaturities_subperiod,			/* out: the time (from today) to the fixing for each Libor observation dates */
					tvols_subperiod,				/* out: the log volatilities (interpolated) for each Libor observation date */
					tvols_discr,					/* out: the log volatilities at the volatility discretization dates */
					index_discret_obsfreq,			/* out: the map from the discretization dates to the observation dates */
...
/* loop over the exercise dates before the current coupon adding the pv to the (earlier) forward time swap */
		for (  index_exo = 0; (deal->iSet[index_exo] < index_coupon) && (index_exo < nEx); index_exo++  )
			forwardsTimeSwaps[index_exo] += forward_pvPeriod;

/* if the exercise index is equal to the index coupon, then we have completed the forward time swap */			
		if (deal->iSet[index_exo] == index_coupon)		
		{
/* add in the pv.  The forwardTS is the sum of the accrual coupons, the float margins, the final DF minus the start DF */
			forwardsTimeSwaps[index_exo] += forward_pvPeriod;

/* calculate the discount factor at the start of the forward time swap */
			dfSt = swp_f_df(tNow,deal->tSet[index_exo],ycName);

/* calculate the ratio (notice index is 1 higher) of the fixed PV to the floating PV (including the final but not the initial
	notional repayment). */			
			ratios[index_exo+1] = (forwardsTimeSwaps[index_exo]+dfSt)/dfSt;

/* If the ratio is too high or too low, set it to 0 (not sure what the impact of this is) */			
			if (ratios[index_exo+1]>5.0 || ratios[index_exo+1] <0.2)
				ratios[index_exo+1] = 0.;
			
/* Calculate what the intrinsic value is (0 if OTM, otherwise the fwdTS value).  */
...
/********************************************************************************/
/*	Returns intrinsic value and the ratio PV of coupon leg to PV of the strike
for a timeswap. If deal is strictly positive or strictly negative, ratio
is set to 0. */
LGMErr CompValTimeSwap(
	...
	SrtCallTimeSwapPtr deal,
	int		bullet_or_option,		/* 0=bullet,1=option */
	int		eod_pay_flag,
	int		noDaysAccrued,		/* for period which has already started */
	double	floatingFixing		/* fixing at the beginning of the period */ )					/* basis for rates */
{
	SrtCurvePtr yldcrv = NULL;						/* yield curve pointer */
	LGMMarkConv conventions;						/* standard market conventions */
	LGMErr error=NULL;
	...
	double vollow,vollowshift,volup,volupshift;
	double maturity;
	double proba_above,proba_above_adj,proba_below,proba_below_adj,proba_between,proba_between_adj;
	double expected_accrued_ratio,expected_accrued_ratio_adj;
	double callup,callupshift,callup_adj,callupshift_adj;
	double putlow,putlowshift,putlow_adj,putlowshift_adj;
	double volLiborup,volLiborlow;
	...
	for (index=0; index<num_subdiscret_obsfreq; index++)
	{
		index_discret_obsfreq[index] = (int) (index*
			(obsfreq-1.0)/(num_subdiscret_obsfreq-1.0));
	}

 
	for (i=deal->iSet[indexexo]+1; i<=deal->nCpn; i++)	/* for each period */							/* find PV of coupon leg */
	{
		
		dfPay = swp_f_df(tNow,deal->tCpnPay[i-1] , ycName);		/* calculate discount factor to pay date */
	/*	if (bullet_or_option==0 && deal->tCpnPay[i-1] < (today + eod_pay_flag))
		...
		*/
		...
		if (dfSt==SRT_DF_ERROR || dfPay==SRT_DF_ERROR)
			return("no discount factor");
	/*	coverage_Libor = coverage(deal->tCpnStart[i-1],deal->tCpnPay[i-1],basis);
		level = coverage_Libor * dfPay;
		fra_period = (dfSt - dfPay)/level;   */
		...
		index_for_adj_cms = obsfreq-1;
		fra_for_adj_cms = tfras_subperiod[index_for_adj_cms];
		error = GetVol_FudgeTimeSwap(tNow,tNow,deal->observationdays[i-1][index_for_adj_cms][0],
			deal->observationdays[i-1][index_for_adj_cms][1],fra_for_adj_cms,fra_for_adj_cms,deal,
			&vol_for_adj_cms);
		for (j=1;j<=obsfreq;j++)
		{
			tcms_subperiod[j-1]=tfras_subperiod[j-1]*exp(vol_for_adj_cms*vol_for_adj_cms
				*fra_for_adj_cms*tmaturities_subperiod[j-1]*(deal->observationdays[i-1][j-1][1]-
				deal->tCpnPay[i-1]+0.0)/365.0);
		}
		...
		for (l=0; l<obsfreq;l++)
		{
			...
			{
				/* TEMPORARY */
				tvols_subperiod[l][m] = (1-xx)*tvols_discr[index][m] +
											xx*tvols_discr[index+1][m];
			}
...
/*				*/
		...
		/*		error = GetVol_FudgeTimeSwap(
			...
			expected_accrued_ratio += proba_between*(deal->ratiodays_for_subperiod)[i-1][j-1];
			expected_accrued_ratio_adj += proba_between_adj*(deal->ratiodays_for_subperiod)[i-1][j-1];		
		}
	...
	expected_accrued_ratio_adj = 1;   */
		...
		/* Fix Coupon */
		pvCpn = dfPay*deal->tCvgCpn[i-1]*deal->tCpn[i-1]*expected_accrued_ratio;
		/* Floating Digital */
		pvCpn += dfPay*deal->tCvgCpn[i-1]*deal->gear[i-1]*fra_accrued*expected_accrued_ratio_adj;
		/* Past Fixings */
		if (bullet_or_option == 0 && deal->tCpnPay[i-1] <tNow + eod_pay_flag)
		{
			pvCpn += dfPay * deal->tCvgCpn[i-1]*deal->tCpn[i-1]*(noDaysAccrued+0.0)/(deal->tCpnPay[i-1] - deal->tCpnStart[i-1]+0.0);
			pvCpn += dfPay * deal->tCvgCpn[i-1]*deal->gear[i-1]*floatingFixing*(noDaysAccrued+0.0)/(deal->tCpnPay[i-1] - deal->tCpnStart[i-1]+0.0);
		}
		...
		sum = sum + pvCpn;
		if (deal->nCpn==i && bullet_or_option==1)	/* final notional (only for option valuation)  */
		{
			sum += dfPay;
		}
...
*/
	...
	if (deal->PayRec == SRT_PAYER) intVal = -intVal;
	



	if (Strike==0.) ratio = 0.0;
	else ratio = sum/(dfSt*Strike);			/* ratio of PV's */

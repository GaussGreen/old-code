...
Err choldc (int n, double **a, double **chol)
{
	...
	for (i=0; i<n; i++)
	{
		for (j=i; j<n; j++)
		{
			...
			if (i == j)
			{
				if (sum <= 0.0) return serror("Matrix is not positive definite in choldc");
				chol[i][i] = sqrt(sum);
			}
			else a[j][i] = chol[j][i] = sum / chol[i][i];
		}
	}
	return NULL;
}
...

{
	Err err=NULL;
	int i,j,k,nrot;
	double *eigen_val,**eigen_vec,**AuxMatrix,Thresh=0.0,TraceOld=0.0,TraceNew=0.0;

	eigen_val=dvector(0,n-1);
	eigen_vec=dmatrix(0,n-1,0,n-1);
	AuxMatrix=dmatrix(0,n-1,0,n-1);

	err = jacobi_diagonalisation2(	Matrix, 
									n, 
									eigen_val, 
									eigen_vec, 
									&nrot);	
	...
	for (i=0;i<n;i++) {
		for (j=0;j<n;j++) {
			...
			for (k=0;k<n;k++){	
				...
				AuxMatrix[i][j]+=eigen_vec[i][k]*eigen_vec[j][k]*eigen_val[k]*TraceOld/TraceNew;
			}
		}
	}

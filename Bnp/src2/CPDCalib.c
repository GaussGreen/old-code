...
void cpd_calib_set_default_param(
...
void diag_calib_lm_params_set_default_param(
DIAG_CALIB_LM_PARAMS param)
{
	param->nb_iter = 10;
	param->use_moment = 1;
	param->vega_weight = 0;
...
Err diagcalib_interp_voltype(const char *constStr, DIAGCALIB_VOLTYPE *vol_type)
{   
	char str[30+1];
	if (!constStr) return "Empty string in diagcalib_interp_voltype";
	...
	if(!strcmp(str,"NORMAL")) 		{*vol_type = NORMAL_VOL; 		return 0;}
	if(!strcmp(str,"NORM")) 		{*vol_type = NORMAL_VOL; 		return 0;}
	if(!strcmp(str,"LOGNORMAL")) 	{*vol_type = LOGNORMAL_VOL;		return 0;}	
	if(!strcmp(str,"LOG")) 			{*vol_type = LOGNORMAL_VOL;		return 0;}	
	if(!strcmp(str,"LGM")) 			{*vol_type = LGM_VOL;			return 0;}	
	if(!strcmp(str,"CAL")) 			{*vol_type = LGM_VOL;			return 0;}
	
	if(!strcmp(str,"0")) 			{*vol_type = NORMAL_VOL; 		return 0;}
	if(!strcmp(str,"1")) 			{*vol_type = LOGNORMAL_VOL;		return 0;}
	if(!strcmp(str,"2")) 			{*vol_type = LGM_VOL; 			return 0;}
	 
	return serror("unknown diagcalib_interp_voltype. %s",str);
}

Err diagcalib_interp_shifttype(const char *constStr, DIAGCALIB_SHIFTTYPE *shift_type)
{   
	char str[30+1];
	if (!constStr) return "Empty string in diagcalib_interp_shifttype";
	...
	if(!strcmp(str,"ADDITIVE"))			{*shift_type = ADDITIVE; 		return 0;}
	if(!strcmp(str,"ADD"))				{*shift_type = ADDITIVE; 		return 0;}
	if(!strcmp(str,"MULTIPLICATIVE"))	{*shift_type = MULTIPLICATIVE;	return 0;}
	if(!strcmp(str,"MULT"))				{*shift_type = MULTIPLICATIVE;	return 0;}

	if(!strcmp(str,"0"))				{*shift_type = ADDITIVE; 		return 0;}
	if(!strcmp(str,"1"))				{*shift_type = MULTIPLICATIVE;	return 0;}
...
double	last_time, last_zeta, last_vol2;
	...
	/* Interpolation part */
	while (i < nb_new_zeta && new_zeta_time[i] <= old_zeta_time[nb_old_zeta - 1])
	{
		while (last_index < nb_old_zeta && new_zeta_time[i] > old_zeta_time[last_index])
		{
			last_index++;
		}		
		...
		else
		{
			...
			expfact2 = static_lgmcalcexpfact_tauts(t1, new_zeta_time[i], nlam, lam_time, lam);
	...
	if (i < nb_new_zeta)
	{
		...
		expfact1 = static_lgmcalcexpfact_tauts(last_time, old_zeta_time[nb_old_zeta-1], nlam, lam_time, lam);
		...
		while (i < nb_new_zeta)
		{
			expfact1 = static_lgmcalcexpfact_tauts(last_time, new_zeta_time[i], nlam, lam_time, lam);
...
double static_lgmcalcexpfact_tauts (
double			T1, 
double			T2, 
int				nlam, 
double			lam_time[], 
double			lam[])
{
	...
	while (i < nlam && lam_time[i] <= T2)
	{
		...
		{
			ans += exp (2.0 * li) * (exp (2.0 * lam[i] * (lam_time[i] - t0)) - 1.0) / 2.0 / lam[i];
		}
		else
		{
...
double			zeta[])								/*	Output */
{
	...
	for (i=0; i<nsig; i++)
	{		
		exp_fact = static_lgmcalcexpfact_tauts (t0, sig_time[i], nlam, lam_time, lam);
	...
	for (j=0; j<nzeta; j++)
	{
		...
		if (i < nsig)
		{
			...
			exp_fact = static_lgmcalcexpfact_tauts (t0, zeta_t[j], nlam, lam_time, lam);
			dz = sig[i] * sig[i] * exp_fact;
			z += dz;
		}
		else
...
double			zeta[])								/*	Output */
{
	static_lgmcalczeta1_tauts(nsig, sig_time, sig, nlam, lam_time, lam, alpha, gamma, rho, nzeta, zeta_t, zeta);
}
...
double			zeta12[])
{
	static_lgmcalczeta2zeta12(n, t, zeta1, lambda, alpha, gamma, rho, zeta2, zeta12);
}
...
double			zeta12[])
{
	...
	for (i=0; i<n; i++)
	{
		...
		//	/ ((exp (2 * l1 * t[i]) - exp (2 * l1 * t0)) / 2 / l1);
		dz12 = dz1 * alpha * rho
			* ( (exp ( dAvgl1_ti * t[i] +  dAvgl2_ti * t[i]  ) - exp ( ( dAvgl1_t0 + dAvgl2_t0) * t0)  ) / (dAvgl1 + dAvgl2))
			/ ( (exp ( 2 * dAvgl1_ti * t[i]) - exp (2 * dAvgl1_t0 * t0) ) / 2 / dAvgl1);
...
double			zeta12[])
{
	...
	for (i=0; i<n; i++)
	{
		...
		exp_fact2 = static_lgmcalcexpfact_tauts (t0, t[i], nlam, lam_time, lam2) / exp_fact1;
		exp_fact12 = static_lgmcalcexpfact_tauts (t0, t[i], nlam, lam_time, lam12) / exp_fact1;
	...
	double		strike)								/*	Strike */								
{
	...
	return lgmopval2F (ncpn, cpn, cpn_G1, cpn_G2, ex_zeta1, ex_zeta2, ex_zeta12, ex_G1, ex_G2);
}
...
void export_lgmsetupG_ts(
	...
	double		ex_G[])								/*	Output: G at exercise dates */
{
	static_lgmsetupG_tauts(nlambda, lambda_time, lambda, ncpn, cpn_time, cpn_G, nex, ex_time, ex_G);	
}
...
/*	Tau-TS enabled version */
static double static_lgmsetup_1G_tauts(
	int			nlam,
	double		lam_time[],
	double		lam[],
	double		T)
	...
	double		ex_G2[])							/*	Output: G2 at exercise dates */
{
	...
	static_lgmsetupG2_tauts(nlambda, lambda_time, lambda, gamma, ncpn, cpn_time, cpn_G2, nex, ex_time, ex_G2);	
}
...
int				use_jumps)							/*	Allow vol term structure to jump */
{
	...
	for (i=0; i<nex - (skip_last > 0); i++)
	{
		...
		for (it=1; it<niter; it++)
		{
			...
			if (fabs ((mkt_price[i] + d) - pr2) < prec) break;
		}
...
int				use_jumps)							/*	Allow vol term structure to jump */
{
	...
	for (i=0; i<nex - (skip_last > 0); i++)
	{
		...
		for (it=1; it<niter; it++)
		{
			...
			if (fabs ((mkt_price[i] + d) - pr2) < CALPRES) break;
		}
	...
	if (skip_last)
	{
		exp_fact = static_lgmcalcexpfact_tauts (t, ex_time[nex-1], nlam, lam_time, lam);
		dz2 = s_last * exp_fact;
		ex_zeta[nex-1] = zeta + dz2;
	}
...
/* if the price has converged then stop */
			if (fabs ((mkt_price[i] + d) - pr2) / vega < prec) break;
...
mkt_vega,
ex_zeta,							/*	Output: zetas */
nlam,
lam_time,
lam,
skip_last,							/*	If 1, the last option is disregarded and the forward volatility is flat from option n-1 */
prec,
vega_prec,
min_fact,							/*	Maximum down jump on variance */
max_fact,
use_jumps);							/*	Maximum up jump on variance */
...
int				use_jumps)							/*	Allow vol term structure to jump */
{
	...
	{
		j = ex_cpn[i];
		exp_fact1 = static_lgmcalcexpfact_tauts (t, ex_time[i], nlam, lam_time, lam1);
		exp_fact2 = static_lgmcalcexpfact_tauts (t, ex_time[i], nlam, lam_time, lam2) / exp_fact1;
		exp_fact12 = static_lgmcalcexpfact_tauts (t, ex_time[i], nlam, lam_time, lam12) / exp_fact1;
	...
	if (skip_last)
	{
		exp_fact1 = static_lgmcalcexpfact_tauts (t, ex_time[nex-1], nlam, lam_time, lam1);
...

{
	...
	/*	Setup G function */
	static_lgmsetupG_tauts (nlam, lam_time, lam, ncpn, cpn_time, cpn_G, nex, ex_time, ex_G);
	
	if (one2F == 2)
	{
		static_lgmsetupG2_tauts (nlam, lam_time, lam, gamma, ncpn, cpn_time, cpn_G2, nex, ex_time, ex_G2);
	}
...
double			rho,
int				skip_last,
DIAG_CALIB_LM_PARAMS ln_params)							/*	If 1, the last option is disregarded
															and the forward volatility is flat from option
															n-1 */
{
	NCPN_LM = ncpn;	
...
Err	static_lgmprcapgiventauts(double	index,
							  double	lam[],
							  double	*price,
							  double	*gradient,
							  int		nlam)
	...
	if (ind - SHIFT_FREQ_LM == 0)
	{
		...
		{			
			...
			for (j=0; j<nlam; j++)
			{
				...
				static_lgmsetupG_tauts (nlam, LAM_TIME_LM, lambda, NCPN_LM, CPN_TIME_LM, CPN_G_LM, NEX_LM, EX_TIME_LM, EX_G_LM);
				err = lgmcalibzeta1F_tauts2(
									NCPN_LM,
									CPN_TIME_LM,
									CPN_DF_LM,
		...
		else
		{
			...
			for (j=0; j<nlam; j++)
			{
				lambda[j] += BUMP_LAM_LM;	
				static_lgmsetupG_tauts (nlam, LAM_TIME_LM, lambda, NCPN_LM, CPN_TIME_LM, CPN_G_LM, NEX_LM, EX_TIME_LM, EX_G_LM);
				static_lgmsetupG2_tauts (nlam, LAM_TIME_LM, lambda, GAMMA_LM, NCPN_LM, CPN_TIME_LM, CPN_G2_LM, NEX_LM, EX_TIME_LM, EX_G2_LM);
...
Err	static_lgmprcapgiventauts_momentum(double	index,
									  double	lam[],
									  double	*price,
									  double	*gradient,
									  int		nlam)
	...
	if (ind == 0)
	{
				...
				for (i=0; i<NEX_LM; i++)
				{
					if (!fmod(i + SHIFT_FREQ_LM, FREQ_SHORT_LM))
					{
						SENSI_LM[i][j] = (lgmsopval1F(
											...
											EX_ZETA_LM[i], 
											EX_G_LM[i],
											EX_SSTRIKE_LM[i]) - EX_SPRICE_LM[i]);
					}
				}
		...
		for (j=0; j<nlam; j++)
		{
			for (k=0; k<NB_MOM_LM; k++)
			{
				...
				SENSI_LM[NEX_LM + k][j] = (temp / fabs(temp) * exp(log(fabs(temp)) / (k+1)) - PRICE_LM[NEX_LM + k]) / BUMP_LAM_LM;
			}
		}		
	}
...
double			ex_spriceres[])						/*	Cap price as output */
{
	...
	if (price_cap)
	{
		if (one2F == 1)
		{
			for (i=0; i<nex; i++)
			{
				ex_spriceres[i] = lgmsopval1F(
											ex_sendcpn[i] + 1 - ex_cpn[i], 
											&(cpn_df[ex_cpn[i]]), 
											&(cpn_cvg[ex_cpn[i]]), 
											&(cpn_G[ex_cpn[i]]), 
											ex_zeta[i], 
...
int				use_jumps)							/*	Allow vol term structure to jump */
{
	...
	err = lgmprcapgivenlambda(
		ncpn, cpn_time, cpn_df, cpn_cvg,
		nex, ex_time, ex_cpn, ex_sncpn, ex_lstrike, ex_lprice, ex_sstrike, ex_sweight, ex_zeta,
		*lambda, one2F, alpha, gamma, rho, skip_last, long_prec, min_fact, max_fact, use_jumps, (fix_lambda? 0: 1), &pr1);
	...
	do
	{
		...
		if (err = lgmprcapgivenlambda(
			ncpn, cpn_time, cpn_df, cpn_cvg,
			nex, ex_time, ex_cpn, ex_sncpn, ex_lstrike, ex_lprice, ex_sstrike, ex_sweight, ex_zeta,
			lam1, one2F, alpha, gamma, rho, skip_last, long_prec, min_fact, max_fact, use_jumps, 1, &pr1))
		{
			return err;
		}
...
//		lam2 = 0.5 * (lam1 + lam2);
		...
		if (err = lgmprcapgivenlambda(
			ncpn, cpn_time, cpn_df, cpn_cvg,
			nex, ex_time, ex_cpn, ex_sncpn, ex_lstrike, ex_lprice, ex_sstrike,ex_sweight, ex_zeta,
			lam2, one2F, alpha, gamma, rho, skip_last, long_prec, min_fact, max_fact, use_jumps, 1, &pr2))
		{
			return err;
		}
		...
		if (fabs (lam2 - lam1) < short_prec || fabs (pr2 - pr1) < short_prec) break;
															...
															n-1 */
{
	...
	if (fix_lambda)
	{
		err = lgmprcapgivenlambda_tauts2(
					ncpn, cpn_time, cpn_df, cpn_cvg,
					nex, ex_time, ex_cpn, ex_lncpn, ex_lstrike, ex_lprice, NULL, ex_sncpn, ex_sstrike, ex_zeta,
					one2F, nlam, lam_time, lam, alpha, gamma, rho, skip_last, CALPRES, 0, MAX_FACT, MAX_FACT, 0, 0, ex_spriceres);
	...
	else
	{
		...
		if (lm_params->use_moment)
		{
			...
			init_static_lgmprcapgiventauts(ncpn, cpn_time, cpn_df, cpn_cvg, nex, ex_time, ex_cpn, ex_lncpn,
											ex_lstrike, ex_lprice, ex_sncpn, ex_sstrike, ex_sprice, ex_zeta, one2F,
											nlam, lam_time, lambda, alpha, gamma, rho, skip_last, lm_params);
		...
		else
		{
			...
			init_static_lgmprcapgiventauts(ncpn, cpn_time, cpn_df, cpn_cvg, nex, ex_time, ex_cpn, ex_lncpn,
											ex_lstrike, ex_lprice, ex_sncpn, ex_sstrike, ex_sprice, ex_zeta, one2F,
											nlam, lam_time, lam, alpha, gamma, rho, skip_last, lm_params);

			err  = levenberg_marquardt(
								data,
								target,
								weight,
								nex / lm_params->freq_short,
								lambda,
								nlam,
								lm_params->nb_iter,
								static_lgmprcapgiventauts,
								&fitting_error);
		}
	...
	CPD_CALIB_INST_DATA	inst_data)					/*	NULL = don't save calibration instrument data */
{
	...
	*sig = NULL;
	...
	{
		ex_date++;
		if (long_strike) long_strike++;
		if (short_strike) short_strike++;
		if (proba) proba++;
		num_ex_dates--;
	}
	...
	for (i=num_ex_dates-1; i>=0; i--)
	{
		...
		if (j >= ncpn - 1 || j == l)
		{
			for (k=i-1; k>=0; k--)
			{
				...
				if (long_strike) long_strike[k+1] = long_strike[k];
				if (short_strike) short_strike[k+1] = short_strike[k];
				if (proba) proba[k+1] = proba[k];
			}
	...
	*sig = (double*) calloc (nex, sizeof (double));
	...
	(*sig)[0] = sqrt (ex_zeta[0] * 2 * (*lambda) / (exp (2 * (*lambda) * ex_time[0]) - 1.0));
	...
	CPD_CALIB_INST_DATA	inst_data)					/*	NULL = don't save calibration instrument data */
{
	...
	*sig = (double*) calloc (nex, sizeof (double));
	...
	for (i=1; i<nex; i++)
	{
		...
		if (ex_zeta[i] > ex_zeta[i-1])
		{
			exp_fact = static_lgmcalcexpfact_tauts (ex_time[i-1], ex_time[i], nlam, lam_time, lam);
...
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~//
//	All three routines below, initialisation, copying and freeing modified	//
//	to allow the handling of long and short market prices. PMc 17Nov03		//
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~//
void cpd_init_calib_inst_data(
...
void cpd_copy_calib_inst_data(
	CPD_CALIB_INST_DATA	dest,
	...
	if (dest && src)
	{
		if (src->num_inst > 0)
		{
			...
			if (src->exer_dates_long)
			{
				...
				memcpy (dest->exer_dates_long, src->exer_dates_long, dest->num_inst * sizeof (long));
			}
			else
			{
			...
			if (src->start_dates)
			{
				...
				memcpy (dest->start_dates, src->start_dates, dest->num_inst * sizeof (long));
			}
			else
			{
			...
			if (src->long_strikes)
			{
				...
				memcpy (dest->long_strikes, src->long_strikes, dest->num_inst * sizeof (double));
			}
			else
			{
			...
			{
				dest->market_prices_long = (double*) calloc (dest->num_inst, sizeof (double));
				memcpy (dest->market_prices_long, src->market_prices_long, dest->num_inst * sizeof (double));
			}
			else
			{
		...
		if (src->num_insts > 0)
		{
			...
			if (src->exer_dates_long)
			{
				...
				memcpy (dest->exer_dates_short, src->exer_dates_short, dest->num_insts * sizeof (long));
			}
			else
			{
			...
			if (src->start_datess)
			{
				...
				memcpy (dest->start_datess, src->start_datess, dest->num_insts * sizeof (long));
			}
			else
			{
			...
			if (src->end_datess)
			{
				...
				memcpy (dest->end_datess, src->end_datess, dest->num_insts * sizeof (long));
			}
			else
			{
			...
			if (src->short_strikes)
			{
				...
				memcpy (dest->short_strikes, src->short_strikes, dest->num_insts * sizeof (double));
			}
			else
			{
			...
			if (src->short_weights)
			{
				...
				memcpy (dest->short_weights, src->short_weights, dest->num_insts * sizeof (double));
			}
			else
			{
			...
			{
				dest->market_prices_short = (double*) calloc (dest->num_insts, sizeof (double));
				memcpy (dest->market_prices_short, src->market_prices_short, dest->num_insts * sizeof (double));
			}
			else
			{
		...
		if (src->num_inst_smile > 0)
		{
			...
			{
				dest->start_dates_smile = (long*) calloc (dest->num_inst_smile, sizeof (long));
				memcpy (dest->start_dates_smile, src->start_dates_smile, dest->num_inst_smile * sizeof (long));
			}
			else
			{
			...
			{
				dest->end_dates_smile = (long*) calloc (dest->num_inst_smile, sizeof (long));
				memcpy (dest->end_dates_smile, src->end_dates_smile, dest->num_inst_smile * sizeof (long));
			}
			else
			{
	...
	CPD_CALIB_INST_DATA	for_inst_data)				/*	NULL = don't save calibration instrument data */
{
	...
	double		*tmp_sig_time1 = NULL,
				*tmp_sig_time2 = NULL;
	double		*tmp_sig1 = NULL,
				*tmp_sig2 = NULL;
	int			i, n;
	double		*fx_mkt_vol_time = NULL;
	long		tmp_ex_date;
	int			free_dom_inst_data = 0,
				free_for_inst_data = 0;
	...
	*fx_vol = NULL;
	...
	if (dom_inst_data) free_dom_inst_data = 1;
	...
	err = cpd_calib_diagonal_2(
		...
		for_inst_data);
	if (err) goto FREE_RETURN;
	if (for_inst_data) free_for_inst_data = 1;
	...
	*fx_vol_time = (double*) calloc (num_fx_mkt_vol, sizeof (double));
	...
	for (i=0; i<num_fx_mkt_vol; i++)
	{
		...
		tmp_ex_date = add_unit (fx_mkt_vol_date[i], -2, SRT_BDAY, MODIFIED_SUCCEEDING);
...
FREE_RETURN:
	...
	if (tmp_sig_time1) free (tmp_sig_time1);
	if (tmp_sig1) free (tmp_sig1);
	if (tmp_sig_time2) free (tmp_sig_time2);
	if (tmp_sig2) free (tmp_sig2);
	...
	CPD_CALIB_INST_DATA	inst_data)					/*	NULL = don't save calibration instrument data */
{
	...
	*sig = NULL;
	...
	for (i=0; i<num_ex_dates; i++)
	{
		err = get_end_date (ex_date[i], end_date, end_tenorl[i], 0, &(theo_end_datesl[i]));
		if (err)
		{
			goto FREE_RETURN;
		}
		act_end_datesl[i] = bus_date_method (theo_end_datesl[i], MODIFIED_SUCCEEDING);

		err = get_end_date (ex_date[i], end_date, end_tenors[i], 0, &(theo_end_datess[i]));
		if (err)
		{
			goto FREE_RETURN;
		}
		act_end_datess[i] = bus_date_method (theo_end_datess[i], MODIFIED_SUCCEEDING);
	}
	for (i=0; i<num_ex_dates; i++)
	{
	...
	for (i=0; i<nex; i++)
	{
		...
		if (k > 0 && cpn_date[k] - act_end_datesl[i] > act_end_datesl[i] - cpn_date[k-1])
		{
			k--;
		}
		...
		if (k > 0 && cpn_date[k] - act_end_datess[i] > act_end_datess[i] - cpn_date[k-1])
		{
			k--;
		}
	...
	for (i=0; i<nex; i++)
	{
		...
		ex_svol[i] += (param->shift_type == 1? ex_svol[i] * param->vol_shift: param->vol_shift);

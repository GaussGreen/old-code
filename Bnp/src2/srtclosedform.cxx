...
char *SrtClosedForm(long start, long nfp_or_end, char *cpdStr, char *basisStr,
                    char *recPayStr, char *typeStr, double strike,
                    double bondStrike, char *refRateCodeStr, int numParams,
                    char **paramStrings, char **valueStrings, char *sUndPtrName,
  ...
  if (greek == PREMIUM) {
    ...
    *num_tau = 0;
    ...
    err = srt_f_grfn_clsdfrm(sUndPtr, &grfnparam, &sdp, strike, bondStrike,
                             rec_pay, type, refRateCodeStr, price);
  } else if (greek == VEGA) {
    err = srt_f_grfn_clsdfrm_vega(sUndPtr, &grfnparam, &sdp, strike, bondStrike,
                                  rec_pay, type, refRateCodeStr, price,
                                  sigma_vega, num_sig, tau_vega, num_tau);
  } else {
...
Err SrtFutureClosedForm(long lFutureDate, long lStartDate, long lNfpOrEnd,
                        ...
                        char *szsUndPtrName, double *pdFuturePrice) {
  Err err = NULL;
  int status = 0;
  SrtUndPtr sUndPtr;
  SrtGrfnParam grfnparam;
  ...
  /* Initialise and Overwrite Grfn defaults parameters with user defined
   * parameters */
  if (err = srt_f_set_GrfnParams(iNumParams, pszParamStrings, pszValueStrings,
                                 &grfnparam)) {
    return err;
  }
  ...
  err = srt_f_grfn_future_closedform(lFutureDate, sUndPtr, &grfnparam, &sdp,
                                     dStrike, dBondStrike, rec_pay, type,
                                     szRefRateCode, pdFuturePrice);
  if (err) {
    return err;
  }
...
Err SrtFutureVolatility(long lFutureDate, long lStartDate, long lNfpOrEnd,
                        char *szComp, char *szBasis, char *szRecPay,
                        char *szType, double dStrike, double dBondStrike,
                        char *szRefRateCode, int iNumParams,
                        char **pszParamStrings, char **pszValueStrings,
                        char *szsUndPtrName, char *szVolType,
  ...
  err = srt_f_grfn_future_closedform(lFutureDate, sUndPtr, &grfnparam, &sdp,
                                     dStrike, dBondStrike, eRecPay, type,
                                     szRefRateCode, &dFuturePrice);
  if (err) {
    return err;
  }
  ...
  /* Computes the forward rate for the relevant period (to compute an implied
   * volatility) */
  err = swp_f_ForwardRate(lStartDate, lNfpOrEnd, szComp, szBasis, szYcName,
                          szRefRateCode, &dForward);
  if (err) {
  ...
  /* Computes the Level Payment for the relevant period (to compute an implied
   * volatility) */
  err = swp_f_LevelPayment(lStartDate, lNfpOrEnd, szComp, szBasis, szYcName,
                           szRefRateCode, &dLevel);
  if (err) {
  ...
  if (err) {
    return serror("Error in SrtFutureVolatility (interp_diffusion_type): %s",
                  err);
  }

  /*  Compute the Implied Volatility for this Future Price */
  err = srt_f_optimpvol(dFuturePrice, dForward, dStrike,
                        (lFixingDate - lFutureDate) * YEARS_IN_DAY, dLevel,
                        eCallPut, eVolType, pdFutureVolatility);

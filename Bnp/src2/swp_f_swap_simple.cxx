/* =============================================================================
   ...
   FILENAME:    swp_f_swap_simple.cxx
...
Err zcswap(SwapDP *sdp, double strike, double ini, double fin, SwapOutput m,
           String ycname, Date value_date, double *answer)

{
  ...
  case COMPUTE_IRR:
    ...
    pv = value_SimpleSwap(&s, crv) / ldf;
    if (err = irr_range(s.dl.date, s.cxxpn.d, s.cxxpn.len, s.dl.type, s.dl.prev,
                        pv, s.sdp.basis_code, s.sdp.cxxompd, &irr))
      return err;
    free_inSimpleSwap(&s);
    *answer = irr;
    break;
  case COMPUTE_DURATION:
    ...
    if (err = irr_range(s.dl.date, s.cxxpn.d, s.cxxpn.len, s.dl.type, s.dl.prev,
                        pv, s.sdp.basis_code, s.sdp.cxxompd, &irr))
      return err;
    if (err = duration_range(s.dl.date, s.cxxpn.d, s.cxxpn.len, s.dl.type,
                             s.dl.prev, s.sdp.basis_code, s.sdp.cxxompd, irr,
                             &duration))
      return err;
    free_inSimpleSwap(&s);
    *answer = duration;
  ...
  case COMPUTE_CONVEXITY:
    ...
    if (err = irr_range(s.dl.date, s.cxxpn.d, s.cxxpn.len, s.dl.type, s.dl.prev,
                        pv, s.sdp.basis_code, s.sdp.cxxompd, &irr))
      return err;
    if (err = convexity_range(s.dl.date, s.cxxpn.d, s.cxxpn.len, s.dl.type,
                              s.dl.prev, s.sdp.basis_code, s.sdp.cxxompd, irr,
                              &convexity))
      return err;
    free_inSimpleSwap(&s);
  ...
  case COMPUTE_THIRDMOMENT:
    ...
    if (err = irr_range(s.dl.date, s.cxxpn.d, s.cxxpn.len, s.dl.type, s.dl.prev,
                        pv, s.sdp.basis_code, s.sdp.cxxompd, &irr))
      return err;
    if (err = thirdmoment_range(s.dl.date, s.cxxpn.d, s.cxxpn.len, s.dl.type,
                                s.dl.prev, s.sdp.basis_code, s.sdp.cxxompd, irr,
                                &thirdmoment))
      return err;
    free_inSimpleSwap(&s);
  ...
  case COMPUTE_MODIFIED_DURATION:
    ...
    if (err = irr_range(s.dl.date, s.cxxpn.d, s.cxxpn.len, s.dl.type, s.dl.prev,
                        pv, s.sdp.basis_code, s.sdp.cxxompd, &irr))
      return err;
    if (err = modified_duration_range(s.dl.date, s.cxxpn.d, s.cxxpn.len,
                                      s.dl.type, s.dl.prev, s.sdp.basis_code,
                                      s.sdp.cxxompd, irr, &duration))
      return err;
    free_inSimpleSwap(&s);
    *answer = duration;
    break;
  case COMPUTE_MODIFIED_CONVEXITY:
    ...
    if (err = irr_range(s.dl.date, s.cxxpn.d, s.cxxpn.len, s.dl.type, s.dl.prev,
                        pv, s.sdp.basis_code, s.sdp.cxxompd, &irr))
      return err;
    if (err = modified_convexity_range(s.dl.date, s.cxxpn.d, s.cxxpn.len,
                                       s.dl.type, s.dl.prev, s.sdp.basis_code,
                                       s.sdp.cxxompd, irr, &convexity))
      return err;
    free_inSimpleSwap(&s);
    *answer = convexity;
    break;
...
/* ---------------------------------------------------------------------------------
   ...
   ---------------------------------------------------------------------------------
 */
Err swp_f_SimpleSwap(long start, long end_nfp, String compStr, String basisStr,
                     double coupon, double initial, double final, String ycname,
                     String strMessage, long value_date, double *answer) {
  Err err = NULL;
  SwapDP swapdp;
  SwapOutput output;
  SrtCurvePtr yc_crv;
  ...
  if (!value_date)
    value_date = start;
  err = zcswap(&swapdp, coupon, initial, final, output, ycname, value_date,
               answer);
  if (err)
    return err;
...
Err zcsens(String tenor, SrtCurvePtr yldcrv, double *answer) {
  ...
  if (err = string_to_YCInstr(tenor, &instr, NULL, ccy_param,
                              add_unit(today, ccy_param->spot_lag, SRT_BDAY,
                                       MODIFIED_SUCCEEDING))) {
    return err;
  }
  ...
  switch (instr.type) {
  case SWAPINSTR:
    if (err = zcswap(&instr.dp.swapdates, 1.0, 0.0, 0.0, COMPUTE_PV,
                     get_curve_name(yldcrv), today, &sens)) {
      return err;
    }
    *answer = sens;
    break;
  default:
    dt = coverage(instr.dp.futdates.start, instr.dp.futdates.end,
                  instr.dp.futdates.basis_code);
    df = swp_f_df((Ddate)today, (Ddate)(instr.dp.futdates.end),
                  get_curve_name(yldcrv));
    *answer = df * dt;
    break;
  }
...
SimpleSwap make_SimpleSwap(SwapDP *sdp, double strike, double ini, double fin,
  ...
  err = cpn_list(sdp->basis_code, sdp->compd, start_date, end_date, cfs.dl,
                 strike, ini, fin, t, &(cfs.cxxpn));
...
/* ------------------------------------------------------------------------------
   ...
   cashflows ( a bond        , for instance) (we assume no initial exchange ,
   and 1.0 as the final one) PV is the pv of the swap at the swap start date All
   the calculations are done on the CLEAN price (which is not bond math)
   ------------------------------------------------------------------------------
 */
Err bond_modified_duration(SwapDP *sdp, double coupon, double pv,
                           double *answer) {
  SimpleSwap s;
  double irr;
  double duration;
...
Err bond_modified_convexity(SwapDP *sdp, double coupon, double pv,
                            double *answer) {
  SimpleSwap s;
  double irr;
  double convexity;

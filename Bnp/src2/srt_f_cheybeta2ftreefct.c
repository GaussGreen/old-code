/* ----------------------------------------------------------------------------------
   ...
   FILENAME:    srt_f_cheybeta2dtreefct.c
...
static Err MCTree_CHEYBETA_2f_evolve_in_Grid(
					double 		**rndm_mat,   
					SrtStpPtr 	   step, 
					MCTreePoint  **grid,
					long           path_index)
...
/* Start from the first time step */
	...
	/* Loops on all the time steps */
		...
		/* The jump corresponds to the Phi for the previous time step ... */
			grid[t][n].jump_x1 = (sam_get(sample, 0, PHI1) + sam_get(sample, 0, CROSSPHI)) * step->delta_t;
			grid[t][n].jump_x2 = (sam_get(sample, 0, PHI2) + sam_get(sample, 0, CROSSPHI)) * step->delta_t;
			
			/* Evolve the sample for one time step ( underlying index is 0: domestic in two factor...) */
		...
		/* Stores the variables without the Non Markovian part */	
			...
			grid[t][n].fw1 = (1.0 -  time_info->ev.twof[0].lambda * step->delta_t) * sam_get(sample, 0, X1); 
			grid[t][n].fw2 = (1.0 -  time_info->ev.twof[1].lambda * step->delta_t) * sam_get(sample, 0, X2);  
...
/*	Initialises mcrs according to inputs, makes memory allocation or
	...
	err = SrtMCRanSrc_start(&mcrs, 0, grfnparam->num_MCarlo_paths - 1, 1, last_index, 
		                    und_info->no_of_brownians, grfnparam->sample_type,	seed, top);
	if (err)
		return err;
...
/* Loop on all the paths used to generate the initial draw */
	...
	/* Generate the current path for the Chey Beta 2f underlying in the Grid */
		err = MCTree_CHEYBETA_2f_evolve_in_Grid( mcrs.r[mcrs.cur_path], top, grid, n);
		if (err)
		{
			SrtMCRanSrc_end(&mcrs);
			return err;
...
/* ------------------------------------------------------------------------------------------------
   PURPOSE:  Allocation of structures for points and probabilities	of transition
   ------------------------------------------------------------------------------------------------ */
						...
						SrtTriangulationIO   *triang)
{
	...
	for (n = 0; n < triang->numberofpoints; n++)
		for (k = 0; k < triang->numberofpointattributes; k++)
			triang->pointattributelist[n * triang->numberofpointattributes + k ] = next_assets[n][k];
					...
					SrtStpPtr     step)
{
	...
	nodes[1][1]  = 0.0;			nodes[1][2] = 0.0;
	nodes[2][1]  = a*x1;	   nodes[2][2] = a*x1;
	nodes[3][1]  = -a*x2;	   nodes[3][2] = a*x2;
	nodes[4][1]  = -a*x1;	   nodes[4][2] = -a*x1;
	nodes[5][1]  = a*x2;	   nodes[5][2] = -a*x2;
...
/* -------------------------------------------------------------------------------------------
   ...
   PURPOSE:   
 ------------------------------------------------------------------------------------------- */
Err rescal(TriPoint  pcur, double fw1, double fw2, double sig1, double sig2, int i,  SrtStpPtr     step)
{
	double   sqdt = sqrt(step->delta_t);
	SrtIRMTmInf  *info = (SrtIRMTmInf*) step->tminf[0];
	double	 v1 = sqrt(1.0 + info->correl_x)*sqdt, v2 = sqrt(1.0 - info->correl_x)*sqdt;
	double   a = 1.0/sqrt(2.0);
	double   xr = (pcur[0] - fw1)/sig1, yr = (pcur[1] - fw2)/sig2;
	double   x = a*(xr + yr)/v1, y =	a*(- xr + yr)/v2;

	/* build the matrix for transition probabilities for the standard gaussian law to specified points */
	coeffs[1][i] = 1.0;
	coeffs[2][i] = x;
	coeffs[3][i] = y;
	coeffs[4][i] = x*x;
...
#define  coords_bary(x, y, a1, b1, a2, b2, a3, b3) \
	((x - a2)*(b3 - b2) - (y - b2)*(a3 - a2)) / ((a1 - a2)*(b3 - b2) - (b1 - b2)*(a3 - a2))

int not_in_set(int i, TriPoint p) 
{
	int k, bb = 0; 
	for (k = 0; k < i-1; k++)	  bb += (p == ptx[k]);
	return(bb == 0); 
}
...
/* Compute the transition probas and conditionnal expectations to the current node */
	...
	/* Locate the position of node for quadrature formula */
		...
		dest(searchtri, p2);
		apex(searchtri, p3);
		l1 = coords_bary(pt_cur[0], pt_cur[1], p1[0], p1[1], p2[0], p2[1], p3[0], p3[1]);
		l2=  coords_bary(pt_cur[0], pt_cur[1], p2[0], p2[1], p3[0], p3[1], p1[0], p1[1]);
		l3 = 1.0 - l1 - l2;

	/* Determine the closest point and transfer its rescaled coordinates in the weight matrix
		as well as the value fonction */ 
		if (l1 >= l2)
		{
			if (l1 >= l3)
			{
		...
		if ((not_in_set(i, pt[0])) && (pt[0][2] != DBL_MAX))
			rescal(pt[0], grid[index_t][index_n].fw1, grid[index_t][index_n].fw2, sigma1, sigma2, i, step);
		else 
		if  ((not_in_set(i, pt[1]))	&& 	 (pt[1][2] != DBL_MAX))
			   rescal(pt[1], grid[index_t][index_n].fw1, grid[index_t][index_n].fw2, sigma1, sigma2, i, step);
		else
		if ((not_in_set(i, pt[2]))	&& 	 (pt[2][2] != DBL_MAX))
			rescal(pt[2], grid[index_t][index_n].fw1, grid[index_t][index_n].fw2, sigma1, sigma2,  i, step);
		else
		{
			rescal(pt_cur, grid[index_t][index_n].fw1, grid[index_t][index_n].fw2, sigma1, sigma2,  i, step);
			index_pt[i-1] = 1;
		}

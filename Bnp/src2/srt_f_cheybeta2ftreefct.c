...
static Err MCTree_CHEYBETA_2f_evolve_in_Grid(
    double** rndm_mat, SrtStpPtr step, MCTreePoint** grid, long path_index)

{
    ...
    if (step != NULL)
    {
        ...
        while (step->next != NULL)
        {
            ...
            grid[t][n].jump_x1 =
                (sam_get(sample, 0, PHI1) + sam_get(sample, 0, CROSSPHI)) * step->delta_t;
            grid[t][n].jump_x2 =
                (sam_get(sample, 0, PHI2) + sam_get(sample, 0, CROSSPHI)) * step->delta_t;

            /* Evolve the sample for one time step ( underlying index is 0: domestic in two
             * factor...) */
            err = srt_f_CheyBeta2f_evolve_sample(
                step, &sample, 0, rndm_mat[0][t], rndm_mat[1][t], &sample);
            if (err)
                return err;
            ...
            grid[t][n].fw1 =
                (1.0 - time_info->ev.twof[0].lambda * step->delta_t) * sam_get(sample, 0, X1);
            grid[t][n].fw2 =
                (1.0 - time_info->ev.twof[1].lambda * step->delta_t) * sam_get(sample, 0, X2);
...
Err srt_f_cheybeta2ftree_make_grid(
    SrtGrfnParam* grfnparam, SrtStpPtr step, SrtUndInfo* und_info, MCTreePoint** grid)
{
    Err        err = NULL;
    SrtSample* sam = NULL;
    SrtUndPtr  und = NULL;
...
------------------------------------------------------------------------------------------------*/
void transfer_info_nodes(MCTreePoint* mesh, double** next_assets, SrtTriangulationIO* triang)
{
    int    n, k;
...
Err rescal(TriPoint pcur, double fw1, double fw2, double sig1, double sig2, int i, SrtStpPtr step)
{
    ...
    /* build the matrix for transition probabilities for the standard gaussian law to specified
     * points */
    coeffs[1][i] = 1.0;
    coeffs[2][i] = x;
    coeffs[3][i] = y;
    ...
    SrtGrfnParam* grfnparam)
{
    ...
    for (i = 1; i <= NB_NEIGHBOURS; i++)
    {
        ...
        if ((not_in_set(i, pt[0])) && (pt[0][2] != DBL_MAX))
            rescal(
                pt[0],
                grid[index_t][index_n].fw1,
                grid[index_t][index_n].fw2,
                sigma1,
                sigma2,
                i,
                step);

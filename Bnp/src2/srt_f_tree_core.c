  ...
  if (und_info->no_of_underlyings == 1) {
    ...
    /* -> 1 FACTOR and 2 FACTOR CHEY BETA TREE <- */
      ...
      case ONE_FAC:
        if ((grfnparam->lsm == SRT_BACK) || (grfnparam->lsm == SRT_FORBACK)) {
          /*
          err = srt_f_ammccheybeta (grfnparam  , step  , gd  , evalcf  ,
          io_request  , und_info); if (err) return err;
          */
        } else {
      ...
      case TWO_FAC:
        if ((grfnparam->lsm == SRT_BACK) || (grfnparam->lsm == SRT_FORBACK)) {
          /*
          err = srt_f_newbeta2ftree( und  , grfnparam  , step  , gd  , evalcf  ,
          io_request  , und_info);
          */
        } else {
    ...
    /* -------------- 1 FACTOR and 2 FACTOR LGM TREES -------------------- */
      ...
      case ONE_FAC:
        ...
        /*  we try to get the prices for the shift in the yield curve
                err = srt_f_yctrelgm1d(m  ,grfnparam  ,step  ,gd  ,evalcf
           ,iolist); not yet implemented */
        break;
      case TWO_FAC:
        if ((grfnparam->lsm == SRT_BACK) || (grfnparam->lsm == SRT_FORBACK)) {
          /*
          err = srt_f_ammclgm2f(grfnparam  , step  , gd  , evalcf  , io_request
          , und_info); if (err) return err;
          */
        } else {
      ...
      case ONE_FAC:
        ...
        break;
      case TWO_FAC:
        /*	Modified AS: BDT 2F Tree is unavailable for the moment  , The
         * file is out of the project...  */
        /* err = srt_f_trebdt2f(und  ,mdl  ,step  ,gd  ,evalcf  ,io_request
        ,und_info); if (err)
        {
                return err;
        } */
    ...
    if ((mdl_dim1 == ONE_FAC) && (mdl_dim2 == ONE_FAC) &&
      ...
      if (((mdl_type1 == LGM) &&
        ...
        /*
        err = srt_f_cheybeta1f1etree(grfnparam  , step  , gd  , evalcf  ,
        io_request  ,und_info);
        */
        if (err)
          return err;

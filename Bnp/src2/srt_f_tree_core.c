    ...
    if (und_info->no_of_underlyings == 1)
    {
        ...
        else if (mdl_type == CHEY_BETA)
        {
            ...
            case ONE_FAC:
                ...
                {
                    err =
                        srt_f_CheyBetaTree(und, grfnparam, step, gd, evalcf, io_request, und_info);
                    if (err)
                        return err;
                }
                break;
            case TWO_FAC:
                ...
                {
                    /*
                    err = srt_f_newbeta2ftree( und, grfnparam, step, gd, evalcf, io_request,
                    und_info);
                    */
                }
                else
                {
                    err = srt_f_cheybeta2dtree(
                        und, grfnparam, step, gd, evalcf, io_request, und_info);
                }
        ...
        else if (mdl_type == LGM)
        {
            ...
            case ONE_FAC:
                err =
                    srt_f_vegashifttrelgm1d(und, grfnparam, step, gd, evalcf, io_request, und_info);
                if (err)
                {
                    return err;
                }
            ...
            case TWO_FAC:
                ...
                else
                {
                    ...
                    {
                        err =
                            srt_f_lgm2dtree(und, grfnparam, step, gd, evalcf, io_request, und_info);
                        if (err)
                            return err;
                    }
                }
        ...
        else if (mdl_type == BDT)
        {
            ...
            case ONE_FAC:
                ...
                break;
            case TWO_FAC:
                /*	Modified AS: BDT 2F Tree is unavailable for the moment, The file is out of
                 * the project...  */

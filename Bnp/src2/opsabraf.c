...
Err interp_sabraf_vol_type(
							char				*str,
							SrtDiffusionType	*val)
{   
	Err err = NULL;
	if (strcmp(str, "") != 0)
...
SABRAF_RISK_PARAM			*param)
{
	...
	for (i=0;i<num_param;i++)
	{
		...
		if (!strcmp (param_str[i], "ZETASHIFT"))
		{
			sscanf (value_str[i], "%lf", &d);
			param->zeta_shift = d;
		}
		...
		if (!strcmp (param_str[i], "ALPHAMULT"))
		{
			if (!strcmp (value_str[i], "YES"))
			{
				param->alpha_mult = 1;
		...
		if (!strcmp (param_str[i], "BETAMULT"))
		{
			if (!strcmp (value_str[i], "YES"))
			{
				param->alpha_mult = 1;
		...
		if (!strcmp (param_str[i], "ZETAMULT"))
		{
			if (!strcmp (value_str[i], "YES"))
			{
				param->alpha_mult = 1;
							...
							double	*price)
{
	...
	err = srt_f_optbmm2vol(Forward, Strike, Maturity, Sigma, Alpha, Beta,Rho,Zeta, input, SRT_BETAVOL, &SigmaBeta);
	if (err) goto FREE_RETURN;

	err = BMM_Option_Price2(Forward,Strike,Maturity,SigmaBeta,Alpha,Beta,Rho,Zeta,call_put,price);
	if (err) goto FREE_RETURN;
	*price *= Disc;
						...
						double					*sens)
{
	...
	if (err = srt_f_op_sabraf_price(	forward,
										strike,
										maturity,
										disc,
										call_put,
	...
	if (err = srt_f_op_sabraf_price (	forward + delta_shift,
										strike,
										maturity,
										disc,
										call_put,
	...
	*sens = fac * delta1;
	...
	if (delta_gamma == 1)
	{
		if (err = op_sabraf_delta_gamma (	forward + gamma_shift,
										strike,
										maturity,
										disc,
										call_put,
					...
					double					*sens)
{
	...
	*sens = fac * vega1;
	...
	if (vega_volga == 1)
	{
		if (err = op_sabraf_vega_volga (	forward,
										strike,
										maturity,
										disc,
										call_put,
					...
					double					*vanna)
{
	...
	/*	Calculate delta before shift	*/
	if (err = op_sabraf_delta_gamma (	forward,
										strike,
										maturity,
										disc,
										call_put,

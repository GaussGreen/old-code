...
/*	Setup parameters	*/
Err op_sabraf_set_param(int num_param, char** param_str, char** value_str, SABRAF_RISK_PARAM* param)
{
    int              i;
    double           d;
    SrtDiffusionType vt;
    ...
    double*          price)
{
    ...
    err = srt_f_optbmm2vol(
        Forward, Strike, Maturity, Sigma, Alpha, Beta, Rho, Zeta, input, SRT_BETAVOL, &SigmaBeta);
    if (err)
        goto FREE_RETURN;

    err = BMM_Option_Price2(
        Forward, Strike, Maturity, SigmaBeta, Alpha, Beta, Rho, Zeta, call_put, price);
    if (err)
        goto FREE_RETURN;
    *price *= Disc;

...
DESCRIPTION     :   Y_T_at_t_param computes 3 components in eqn. A.18        ,
   ...
   variables: t=date acting as origin for YTM Y_T_at_t with timetomat T ;
            ...
            Major changes for Two Factor Model
                    ...
                    (T-t) * Y(t        ,T) = - ln B(t        ,T) and not Y(t ,T)
anymore
---------------------------------------------------------------------------*/
...
Err Vasicek_Y_T_at_param(Ddate fix_date, Ddate *pay_dates, long dim_pay_date,
  ...
  TermStruct *ts;
  long i;
  double F_t, G_t, H_fix_t, Psi_t, S_t, O_t, vasicek_mean_sr_t,
  ...
  err = srt_f_get_vasicek_mean_sr(fix_t, ts, &vasicek_mean_sr_t);
  if (err)
    return err;

  err = srt_f_get_vasicek_mean_int_sr(fix_t, ts, &vasicek_mean_int_sr_t);
  if (err)
    return err;
  ...
  for (i = 0; i < dim_pay_date; i++) {
    if (pay_dates[i] > fix_date) {
      ...
      err =
          srt_f_get_vasicek_mean_int_sr(pay_T, ts, &vasicek_mean_int_sr_pay_T);
      if (err)
        return err;

      YT_sam[i].vasicek_mean_coeff =
          (vasicek_mean_int_sr_pay_T - vasicek_mean_int_sr_t);

      YT_sam[i].vasicek_mean_coeff -=
          (Psi_func(pay_T, ts) - Psi_t) * vasicek_mean_sr_t;

      YT_sam[i].vasicek_var_coeff =
          Psi_func(pay_T, ts) * Psi_func(pay_T, ts) * (G_pay_T - G_t);
      YT_sam[i].vasicek_var_coeff -=
          2 * Psi_func(pay_T, ts) * (S_func(pay_T, ts) - S_t);
      YT_sam[i].vasicek_var_coeff +=
          (G_pay_T * Psi_func(pay_T, ts) * Psi_func(pay_T, ts) -
           2 * O_func(pay_T, ts));
...
Err Y_T_at_t_param(Ddate t_date,  /* Simulation date */
                   Ddate *T_date, /* Discount Factor expiration dates (array) */
                   long dim_Tdate, SrtUndPtr und, YTatt_param *YT_sam) {
  long i;
  double a0, a1;
  double F_t, Psi_t;
  ...
  /* Compute the forward zero-rates from the yield curve for all the
   * zero-coupons */
  for (i = 0; i < dim_Tdate; i++) {
    ...
    if (T_date[i] > t_date) {
      ...
      if (df == SRT_DF_ERROR)
        return serror("Could not compute df(%f        ,%f) for %s", t_date,
                      T_date[i], yc_name);
    ...
    for (i = 0; i < dim_Tdate; i++) {
      if (T_date[i] > t_date) {
        ...
        if (is_irund_Cheyette_type(und)) {
          ...
          YT_sam[i].A_T = A_T;
          YT_sam[i].M = M;
        } else if (is_model_New_type(mdl_type)) {
          H_T = find_struct_interp(T_in_y, H, ts);
...
Err Vasicek_Y_T_at_t_compute(long dim_pay_date, SrtSample *sam,
                             YTatt_param *YT_sam, double *YTt_yield, int index,
                             SrtMdlDim mdl_dim, SrtMdlType mdl_type) {
  long i;

  for (i = 0; i < dim_pay_date; ++i) {
    YTt_yield[i] =
        YT_sam[i].vasicek_sr_coeff * samptr_get(sam, index, SHORT_RATE);
...
/*--------------------------------------------------------------------------
  ...
  DESCRIPTION     :
   ...
   NOTE: Since the actual data structure for "sample" will be a 2-dim array of
         structures        , containing r        , phi        , Z        , one
needs to pass one row of random numbers corresponding to one sample path of the
simulation to this routine        , which will o/p one sample path (i.e. one
vector) of the relevant state variables.
...
Err Y_T_at_t_compute(long dim_Tdate, SrtSample *sam, YTatt_param *YT_sam,
  ...
  for (i = 0; i < dim_Tdate; ++i) {
    /* For any underlying        , there is at least the forward zero-coupon
     * price */
    ...
    if (is_model_Cheyette_type(mdl_type)) {
      ...
      if (mdl_dim == TWO_FAC) {
        YTt_yield[i] +=
            YT_sam[i].x_coeff[1] * samptr_get(sam, index, X2) +
      ...
      s = YT_sam[i].beta * YT_sam[i].beta *
          (YT_sam[i].zeta_T - YT_sam[i].zeta_t) /
          pow(1. + YT_sam[i].beta * samptr_get(sam, index, STATEVAR),
              2. * (1. - YT_sam[i].eta));

      if (YT_sam[i].beta == 0.)
        theta = 0.;
      else
        theta = YT_sam[i].lambda_T *
                (1. + YT_sam[i].beta * samptr_get(sam, index, STATEVAR)) /
                YT_sam[i].beta;
       ...
       */
      YTt_yield[i] += (YT_sam[i].lambda_T - YT_sam[i].lambda_t) *
                      samptr_get(sam, index, STATEVAR);

...
#define FREE_LSMCSV_MEMORY {\
	for(nt = 0; nt < num_time_pts; nt++)\
		free(grid[nt]);\
	free(grid);\
	free_f3tensor(bwd_assets, 0, num_time_pts - 1, 0, grfnparams->num_MCarlo_paths - 1, 0, grfnparams->node_dim - 1);\
	free_f3tensor(fwd_assets, 0, num_time_pts - 1, 0, grfnparams->num_MCarlo_paths - 1, 0, grfnparams->node_dim - 1);\
	free_f3tensor(cur_assets, 0, num_time_pts - 1, 0, grfnparams->num_MCarlo_paths - 1, 0, grfnparams->node_dim - 1);\
	free_dvector(cvalue, 0, grfnparams->node_dim - 1);\
	free_imatrix(datex, 0, grfnparams->num_MCarlo_paths - 1, 0, grfnparams->node_dim - 1);\
	free(cur_val); }
	 ...
	 )
{
	...
	fwd_assets = f3tensor(0, num_time_pts - 1, 0, grfnparams->num_MCarlo_paths - 1, 0, grfnparams->node_dim -1);
	cur_assets = f3tensor(0, num_time_pts - 1, 0, grfnparams->num_MCarlo_paths - 1, 0, grfnparams->node_dim -1);
	bwd_assets = f3tensor(0, num_time_pts - 1, 0, grfnparams->num_MCarlo_paths - 1, 0, grfnparams->node_dim - 1); 
	cvalue = dvector(0, grfnparams->node_dim - 1); 
	datex = imatrix(0, grfnparams->num_MCarlo_paths - 1, 0, grfnparams->node_dim - 1); 

	/* Memory allocation for the grid */
	grid = (MCTreePointCheyBetaStochVol**) srt_calloc(num_time_pts, sizeof(MCTreePointCheyBetaStochVol*));
	for(nt = 0 ; nt < num_time_pts; nt++)
		grid[nt] = (MCTreePointCheyBetaStochVol*) calloc(grfnparams->num_MCarlo_paths, sizeof(MCTreePointCheyBetaStochVol));
	...
	err = cheybetastochvol_simul(grfnparams, top, und_info, grid, num_time_pts-1);
	if (err) 
	{
		srt_free(grid);
		return err;
	...
	if ((grfnparams->lsm == SRT_FOR) || (grfnparams->lsm == SRT_FORBACK))
	{
		for ( i = 0; i < grfnparams->num_MCarlo_paths; i++)
		{
			...
			{ 	
				...
				err = grfn_eval_event_ammc(0, (GrfnEvent *) stp->e, 
						&sam, 
						gd, 
						(double *) cur_assets[nt][i], 
						(double *) fwd_assets[nt][i], 
						(double *) NULL, 
				...
				*/
				...
				for (k = 0; k < grfnparams->node_dim; k++)
					fwd_assets[nt][i][k] = (nt == 0) ? cur_assets[nt][i][k]
						  : fwd_assets[nt-1][i][k]/grid[nt][i].df + cur_assets[nt][i][k];
	...
	if ((grfnparams->lsm == SRT_BACK) || (grfnparams->lsm == SRT_FORBACK))
	{
		...
		/* LOOP ON TIME STEPS */
		for (stp = bot, nt = num_time_pts - 1; stp->prev != NULL; stp = stp->prev, nt--)
		{
			/* LSM ALORITHM */
			if (stp->next != NULL)
			{
				double temp1, temp2, temp3, temp4, temp5, temp6, temp7,
					x1, x2, m1 = 0.0, m2 = 0.0, v11 = 0.0, v12 = 0.0, v22= 0.0, 
					d11, d12, d21, d22, s11, s12, s21, s22, det;
				...
				{
					err = srt_f_lsmcsv_regr( nreg, grfnparams->node_dim, grfnparams->num_MCarlo_paths, 
											 nt, grid, bwd_assets, m1, m2, s11, s12, s21, s22);
				}
				else
				{
					err = srt_f_lsmcsv_regr( nreg, grfnparams->node_dim, grfnparams->num_MCarlo_paths, 
											 nt, grid, cur_assets, m1, m2, s11, s12, s21, s22);
				}
				if (err)  
				{
					FREE_LSMCSV_MEMORY
			...
			for ( i = 0; i < grfnparams->num_MCarlo_paths; i++)
			{
				...
				if (grfnparams->lsm == SRT_FORBACK)
				{
					...
					err = grfn_eval_event_ammc(1, (GrfnEvent *) stp->e, 
						&sam, 
						gd, 
						(double *) bwd_assets[nt][i],
						(double *) fwd_assets[nt][i], 
						(double *) cur_assets[nt][i], 
						(EvalEventDfsFct) srt_f_calc_grfn_event_dfs, 
						und_info, &cash_flow);
		...
		for (k = 0; k < grfnparams->node_dim; k++) 	
		{
			...
			{
				for ( i = 0; i < grfnparams->num_MCarlo_paths; i++)
					cur_val[k] +=  bwd_assets[datex[i][k]][i][k] / grid[datex[i][k]][i].numeraire;
			}
			else
			{
				for ( i = 0; i < grfnparams->num_MCarlo_paths; i++)
	...
	err = srt_f_IOstructsetpremium(iolist,SRT_NO,cur_val[grfnparams->node_dim-1],"");
	if (err) 
	{
		FREE_LSMCSV_MEMORY
		return (err);
	...
	err = srt_f_IOstructsetcolpvs( iolist, SRT_NO, (double*) cur_val, grfnparams->node_dim,	"");
	if (err)
	{
		FREE_LSMCSV_MEMORY
		return err;

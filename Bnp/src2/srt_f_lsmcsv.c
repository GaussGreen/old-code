...
)
{
    ...
    fwd_assets = f3tensor(
        0, num_time_pts - 1, 0, grfnparams->num_MCarlo_paths - 1, 0, grfnparams->node_dim - 1);
    cur_assets = f3tensor(
        0, num_time_pts - 1, 0, grfnparams->num_MCarlo_paths - 1, 0, grfnparams->node_dim - 1);
    bwd_assets = f3tensor(
        0, num_time_pts - 1, 0, grfnparams->num_MCarlo_paths - 1, 0, grfnparams->node_dim - 1);
        ...
        for (stp = bot, nt = num_time_pts - 1; stp->prev != NULL; stp = stp->prev, nt--)
        {
            ...
            if (stp->next != NULL)
            {
                double temp1, temp2, temp3, temp4, temp5, temp6, temp7, x1, x2,
                    m1 = 0.0, m2 = 0.0, v11 = 0.0, v12 = 0.0, v22 = 0.0, d11, d12, d21, d22, s11,
                    s12, s21, s22, det;

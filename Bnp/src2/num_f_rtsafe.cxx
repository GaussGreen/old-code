/* ==========================================================
   ...
   and x2. The root        , returned as the function value rtsafe        , will
   be refined until its accuracy is known within + or - xacc. funcd is a
...
SrtErr rtsafe(Err (*func)(double, double *, double *), double x1, double x2,
  ...
  for (j = 1; j <= num_iter; j++) {
    if ((((rts - xh) * df - f) * ((rts - xl) * df - f) >= 0.0) ||
      ...
      /* bisect if Newton is out of range        , or not decreasing fast
       * enough. */
      dxold = dx;
      dx = 0.5 * (xh - xl);
      rts = xl + dx;

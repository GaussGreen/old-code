...
/* Integrand for the integration with respect to the joint probability distribution of
 * (DRST1T1,DRST1Tn)*/
...

{
    ...
    if (ValDiffusion == SRT_LOGNORMAL)
    {
        ...
        */
        Integrand =
            SDeltaN * sqrt(CorrResid) * (s_DRST1T1 + s_Margin) * exp(2 * Correl * u * v) / (SRT_PI);
    }
...

{
    ...
    SqDRSnVolT1 =
        (SqCapBsVolTn * MatT0Tn - MatT1Tn * s_FwdVol[s_N + 1] * s_FwdVol[s_N + 1]) / MatT0T1;
    if (SqDRSnVolT1 < 0)
    {
    ...
    if (ValDiffusion == SRT_LOGNORMAL)
    {
        ...
        DRSnVolT1 = sqrt(SqDRSnVolT1);
        LnXT1nVar =
            (SqCapBsVolT1 + SqDRSnVolT1 - 2 * Correlation * CapBsVolT1 * DRSnVolT1) * MatT0T1;
        LnXT1nStd  = sqrt(LnXT1nVar);
        ...
        for (i = 1; i <= NumHermitePoints; i++)
        {
            ...
            for (j = 1; j <= NumHermitePoints; j++)
            {
                if (ValResetType == SRT_OPTIMISED)
                {
                    ...
                    s_XT1Tn = exp(LnXT1Tn);
                    LnDRST1T1 =
                        sqrt(2 * CorrResid) * LnDRST1Var * HermiteAbscissas[j] + MeanLnDRST1T1;
                    s_DRST1T1 = exp(LnDRST1T1);
                ...
                /* Increment the integral by the current value (Hermite integration) (first term for
                 * Margin) */
                Term1 += HermiteWeights[j] * srt_f_accrual_gauss_hermite_integrand(
                                                 HermiteAbscissas[i],
                                                 HermiteAbscissas[j],
                                                 Correl,
                                                 LnXT1nVar,
    ...
    else if (ValDiffusion == SRT_NORMAL)
    {
        ...
        s_SlopeVarT0T1[s_N + 1] =
            MatT0T1 * (SqDRSnVolT1 - 2 * Correlation * CapBsVolT1 * DRSnVolT1 + SqCapBsVolT1);
        ...
        if (ValResetType == SRT_AUTORESET)
        {
            ...
            for (k = 2; k <= (s_N + 1); k++)
            {
                ...
                B = sqrt(MatT1Tk * s_FwdVol[k] * s_FwdVol[k] + (1 - SqCorrel) * s_SlopeVarT0T1[k]) /
        ...
        else if (ValResetType == SRT_OPTIMISED)
        {
            ...
            for (i = 1; i <= NumHermitePoints; i++)
            {
                ...
                for (j = 1; j <= NumHermitePoints; j++)
                {
                    ...
                    s_DRST1T1 =
                        DRS0T1 + HermiteAbscissas[i] * sqrt(2 * CorrResid * MatT0T1 * SqCapBsVolT1);
                    s_DRST1Tn =
                        DRS0Tn + HermiteAbscissas[j] * sqrt(2 * CorrResid * MatT0Tn * SqCapBsVolTn);

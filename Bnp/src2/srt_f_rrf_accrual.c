/* ==========================================================================================
   ...
   PURPOSE:        computation of the price of an Accrual Resettable Range Floater in a 
                Black-Scholes like framework (with input of Forward Volatility )
   ========================================================================================== */
#include "math.h"				
#include   "utallhdr.h"
#include   <OPFNCTNS.H>
...
#define FREE_ALL_ACCRUAL_RRF_MEMORY {\
	if (HermiteWeights)		free_dvector(HermiteWeights,1,NumHermitePoints);\
	if (HermiteAbscissas)	free_dvector(HermiteAbscissas,1,NumHermitePoints);\
	if (SlopeStdT0T1)		free_dvector(SlopeStdT0T1,2,(s_N+1));\
	if (OptStrike)			free_dmatrix(OptStrike,1,NumHermitePoints,1,NumHermitePoints);\
	if (s_Theta)            free_dvector(s_Theta,2,(s_N+1));\
	if (s_SlopeVarT0T1)	    free_dvector(s_SlopeVarT0T1,2,(s_N+1));\
	if (s_FwdVol)			free_dvector(s_FwdVol,2,(s_N+1));\
	if (s_FwdStDev)         free_dvector(s_FwdStDev,2,(s_N+1));\
	if (s_SlopeT0)			free_dvector(s_SlopeT0,2,(s_N+1));}
...
/* Function to minimise for the LOGNORMAL case (== k for band position centered around k*Libor)  */
...
/* Function to minimise for the NORMAL case (== k for band position centered around k*Libor)  */
...
/* Integrand for the integration with respect to the joint probability distribution of (DRST1T1,DRST1Tn)*/					  
...
int					i;
	...
	if(ValDiffusion==SRT_LOGNORMAL)
	{
		...
		for(i=2;i<=(s_N+1);i++)
		{
			...
			d = -log( 1 + s_Theta[i]*( (1.0/s_XT1Tn) -1.0) ) / s_FwdStDev[i] + 0.5*s_FwdStDev[i];
			d1 = d + log(Beta) / s_FwdStDev[i];
			d2 = d + log(Alpha) / s_FwdStDev[i];
		...
		*/
		Integrand = SDeltaN*sqrt(CorrResid)*( s_DRST1T1 + s_Margin )*exp( 2 * Correl * u * v)/(SRT_PI);
	...
	else if(ValDiffusion==SRT_NORMAL)
	{
		...
		for(i=2;i<=(s_N+1);i++)
		{
			...
			d = ( -s_Theta[i]*s_DRST1Tn+(OptStrike-1+s_Theta[i])*s_DRST1T1)/s_FwdStDev[i];
			d1 = d + s_BandWidth/2.0 / s_FwdStDev[i];
			d2 = d - s_BandWidth/2.0 / s_FwdStDev[i];
			SDeltaN+=(norm(d1)-norm(d2));
...
Err srt_f_rrf_accrual(String				ResetType,
						long				Today,
...
/* Compute Volatility of the DRStTn until T1 (caplet vol - forward vol ) */
	...
	SqDRSnVolT1 = (SqCapBsVolTn*MatT0Tn - MatT1Tn*s_FwdVol[s_N+1]*s_FwdVol[s_N+1]) / MatT0T1;
	if(SqDRSnVolT1<0)
	{
...
/* Computation of price */
	...
	/* ----------------- LOGNORMAL CASE ------------------------ */
		...
		DRSnVolT1 = sqrt(SqDRSnVolT1);		
		LnXT1nVar = (SqCapBsVolT1+SqDRSnVolT1-2*Correlation*CapBsVolT1*DRSnVolT1)*MatT0T1;
		LnXT1nStd = sqrt(LnXT1nVar);
		LnDRST1Var = SqCapBsVolT1*MatT0T1;
		LnDRST1Std = sqrt(LnDRST1Var);

		ChangeMeasureTerm = (SqCapBsVolT1-Correlation*CapBsVolT1*DRSnVolT1)*MatT0T1;
		MeanLnXT1n = log(DRS0T1/DRS0Tn) + 0.5*(SqCapBsVolT1-SqDRSnVolT1)*MatT0T1;
		...
		Correl = (LnDRST1Var-DRSnVolT1*CapBsVolT1*MatT0T1*Correlation)/(LnDRST1Std*LnXT1nStd);
		SqCorrel = 	Correl*Correl;
		CorrResid = 1-SqCorrel;
	...
	/* Double integration with respect to: ( DRST1Tn , XT1Tn = DRST1T1 / DRST1Tn) */
		for(i=1;i<=NumHermitePoints;i++)
		{
			...
			for(j=1;j<=NumHermitePoints;j++)
			{
				...
				/* OPTIMISED case */
					...
					LnXT1Tn = sqrt(2*CorrResid)*LnXT1nVar*HermiteAbscissas[i]+MeanLnXT1n;
					s_XT1Tn = exp(LnXT1Tn);
					LnDRST1T1 = sqrt(2*CorrResid)*LnDRST1Var*HermiteAbscissas[j]+MeanLnDRST1T1;
					s_DRST1T1 = exp(LnDRST1T1);
				...
				/* Finds the optimal position of the band */
					cx = (ax + bx) * 0.5;
					golden_section(ax,cx,bx,logdiff_num_f_golden,Tolerance,&OptStrike[i][j]);
				...
				{
				/* In the auto reset, the band is centerd on the initial Libor ( k = 1 ) */
					OptStrike[i][j] =1;
				}

					
			/* Increment the integral by the current value (Hermite integration) (first term for Margin) */
	...
	if(ValDiffusion== SRT_NORMAL)
	{
	/* ----------------- NORMAL CASE ------------------------ */

		DRST1Var = MatT0T1*SqCapBsVolT1;
		DRST1Std = sqrt(DRST1Var);
		s_SlopeVarT0T1[s_N+1] = MatT0T1*(SqDRSnVolT1-2*Correlation*CapBsVolT1*DRSnVolT1+SqCapBsVolT1);
		SlopeStdT0T1[s_N+1] = sqrt(s_SlopeVarT0T1[s_N+1]);
		Correl = (DRST1Var-DRSnVolT1*CapBsVolT1*MatT0T1*Correlation)/(DRST1Std*SlopeStdT0T1[s_N+1]);
		SqCorrel = Correl*Correl;
		CorrResid =1-SqCorrel;
		...
		/* AUTO_RESET case */ 
			...
			for(k=2;k<=(s_N+1);k++)
			{
				...
				if(k!=(s_N+1))
				{
					s_SlopeVarT0T1[k] = s_Theta[k]*s_Theta[k]*MatT0T1*(SqDRSnVolT1-2*Correlation*CapBsVolT1*DRSnVolT1+SqCapBsVolT1);
					SlopeStdT0T1[k] = sqrt(s_SlopeVarT0T1[k]);
				}
				
				MatT1Tk = (double)((k-1)*YEARS_IN_DAY);
				A = (s_BandWidth/2.0-s_SlopeT0[k])/(Correl*SlopeStdT0T1[k]);
				B = sqrt(MatT1Tk*s_FwdVol[k]*s_FwdVol[k]+(1-SqCorrel)*s_SlopeVarT0T1[k])/(Correl*SlopeStdT0T1[k]);
				C = -(s_BandWidth/2.0+s_SlopeT0[k])/(Correl*SlopeStdT0T1[k]);
			
			/* The nice part of the equations ... (see document) */
				MTerm = srt_f_intnrm_m2(A,B,0,1)-srt_f_intnrm_m1(A,B,0,1)-srt_f_intnrm_m2(C,B,0,1)
					+srt_f_intnrm_m1(C,B,0,1);

				BB = sqrt(MatT1Tk*s_FwdVol[k]*s_FwdVol[k]+s_SlopeVarT0T1[k])/(Correl*SlopeStdT0T1[k]);
		...
		/* Normal OPTIMISED case */
			...
			for(i=1;i<=NumHermitePoints;i++)
			{
				...
				/* Sets the Values of the DRS's on start date (T1) and end date (Tn) */
					s_DRST1T1 = DRS0T1+HermiteAbscissas[i]*sqrt(2*CorrResid*MatT0T1*SqCapBsVolT1);
					s_DRST1Tn = DRS0Tn+HermiteAbscissas[j]*sqrt(2*CorrResid*MatT0Tn*SqCapBsVolTn);
				...
				/* Finds the optimal band position (== OptStrike) using Golden section */
					cx = 0.5 * (ax + bx);
					golden_section(ax,cx,bx,normdiff_num_f_golden,Tolerance,&OptStrike[i][j]); 


				/* Computes the value with the optimal strike */
					Term1 += HermiteWeights[j]
								*srt_f_accrual_gauss_hermite_integrand(HermiteAbscissas[i],
																		HermiteAbscissas[j],
																		Correl,
																		MatT0T1*SqCapBsVolT1,
																		MatT0Tn*SqCapBsVolTn,
																		DRS0T1,
																		DRS0Tn,
																		OptStrike[i][j],
																		s_BandWidth,

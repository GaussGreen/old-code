...
#define infect(TriOrientedTriangle) \
    (TriOrientedTriangle).tri[6] =  \
        (TriTriangle)((unsigned long)(TriOrientedTriangle).tri[6] | (unsigned long)2l)
...
#define sencode(edge) (TriShell)((unsigned long)(edge).sh | (unsigned long)(edge).shorient)
...
#define sdest(edge, pointptr) pointptr = (TriPoint)(edge).sh[3 - (edge).shorient]

#define setsorg(edge, pointptr) (edge).sh[2 + (edge).shorient] = (TriShell)pointptr

#define setsdest(edge, pointptr) (edge).sh[3 - (edge).shorient] = (TriShell)pointptr
...
void printshelle(TriOrientedShell* s)
{
    ...
    printf(
        "shell edge x%lx with orientation %d and mark %d:\n",
        (unsigned long)s->sh,
        s->shorient,
        mark(*s));
...
void poolinit(
    struct memorypool* pool, int bytecount, int itemcount, enum wordtype wtype, int alignment)
{
    int wordsize;
    ...
    pool->itemwords =
        ((bytecount + pool->alignbytes - 1) / pool->alignbytes) * (pool->alignbytes / wordsize);
    ...
    pool->firstblock =
        (VOID**)malloc(pool->itemsperblock * pool->itembytes + sizeof(VOID*) + pool->alignbytes);
...
void poolrestart(struct memorypool* pool)
{
    ...
    pool->nextitem =
        (VOID*)(alignptr + (unsigned long)pool->alignbytes - (alignptr % (unsigned long)pool->alignbytes));
    ...
    else
    {
        ...
        if (pool->unallocateditems == 0)
        {
            ...
            if (*(pool->nowblock) == (VOID*)NULL)
            {
                ...
                newblock = (VOID**)malloc(
                    pool->itemsperblock * pool->itembytes + sizeof(VOID*) + pool->alignbytes);
...
void dummyinit(int trianglewords, int shellewords)
{
    ...
    dummytribase = (TriTriangle*)malloc(triwords * sizeof(TriTriangle) + triangles.alignbytes);
    ...
    if (useshelles)
    {
        ...
        /*   edge.                                                               */
        dummyshbase = (TriShell*)malloc(shwords * sizeof(TriShell) + shelles.alignbytes);
...
void initializepointpool()
{
    ...
    poolinit(
        ...
        pointsize,
        POINTPERBLOCK,
        (sizeof(REAL) >= sizeof(TriTriangle)) ? FLOATINGPOINT : POINTER,
        0);
}
...
void initializetrisegpools()
{
    ...
    trisize = ((order + 1) * (order + 2) / 2 + (highorderindex - 3)) * sizeof(TriTriangle);
    ...
    if ((voronoi || neighbors) && (trisize < 6 * sizeof(TriTriangle) + sizeof(int)))
    {
    ...
    if (useshelles)
    {
        ...
        poolinit(&shelles, 6 * sizeof(TriTriangle) + sizeof(int), SHELLEPERBLOCK, POINTER, 4);
...
TriPoint getpoint(int number)
{
    ...
    foundpoint =
        (TriPoint)(alignptr + (unsigned long)points.alignbytes - (alignptr % (unsigned long)points.alignbytes));
...
int fast_expansion_sum_zeroelim(
    int elen, REAL* e, int flen, REAL* f, REAL* h) /* h cannot be e or f. */
{
    REAL         Q;
    INEXACT REAL Qnew;
    INEXACT REAL hh;
...
void triangleinit()
{
    points.maxitems = triangles.maxitems = shelles.maxitems = viri.maxitems = badsegments.maxitems =
        badtriangles.maxitems = splaynodes.maxitems = 0l;
...
TriLocateResultType locate(TriPoint searchpoint, TriOrientedTriangle* searchtri)
{
    ...
    {
        printf(
            "  Randomly sampling for a TriTriangle near TriPoint (%.12g, %.12g).\n",
            searchpoint[0],
            searchpoint[1]);
    }
    ...
    for (i = 0; i < sampleblocks; i++)
    {
        ...
        for (j = 0; j < samplesperblock; j++)
        {
            ...
            sampletri.tri = (TriTriangle*)(firsttri + (samplenum * triangles.itemwords));
            if (sampletri.tri[3] != (TriTriangle)NULL)
            {
                ...
                if (dist < searchdist)
                {
                    ...
                    {
                        printf(
                            "    Choosing TriTriangle with origin (%.12g, %.12g).\n",
                            torg[0],
                            torg[1]);
                    }
                }
...
enum insertsiteresult insertsite(
    TriPoint             insertpoint,
    TriOrientedTriangle* searchtri,
    TriOrientedShell*    splitedge,
    int                  segmentflaws,
    int                  triflaws)
{
    ...
    if ((intersect == ONEDGE) || (intersect == OUTSIDE))
    {
        ...
        {
            printf("Internal error in insertsite():\n");
            printf("  Clockwise TriTriangle after edge TriPoint insertion (bottom right).\n");
        }
#endif /* SELF_CHECK */
        if (verbose > 2)
        {
        ...
        if (doflip)
        {
            ...
            else
            {
                    ...
                    (leftpoint == infpoint3))
                {
                    ...
                    (rightpoint == infpoint1) || (rightpoint == infpoint2) ||
                    (rightpoint == infpoint3))
                {
                    ...
                    (farpoint == infpoint1) || (farpoint == infpoint2) || (farpoint == infpoint3))
                {
...
void triangulatepolygon(
    TriOrientedTriangle* firstedge,
    TriOrientedTriangle* lastedge,
    int                  edgecount,
    int                  doflip,
    int                  triflaws)
{
...
void pointmedian(TriPoint* sortarray, int arraysize, int median, int axis)
{
    ...
    if (right < median - 1)
    {
        ...
        pointmedian(&sortarray[right + 1], arraysize - right - 1, median - right - 1, axis);
    }
}
...
void mergehulls(
    ...
    TriOrientedTriangle* innerleft,
    TriOrientedTriangle* innerright,
    ...
    int                  axis)
{
    ...
    while (1)
    {
        ...
        if (leftfinished ||
            (!rightfinished && (incircle(upperleft, lowerleft, lowerright, upperright) > 0.0)))
        {
...
enum finddirectionresult finddirection(TriOrientedTriangle* searchtri, TriPoint endpoint)
{
    ...
    while (leftflag)
    {
        ...
        if (searchtri->tri == dummytri)
        {
            ...
            printf("  TriTriangle leading from (%.12g, %.12g) to", startpoint[0], startpoint[1]);
...
void segmentintersection(
    TriOrientedTriangle* splittri, TriOrientedShell* splitshelle, TriPoint endpoint2)
{
    TriPoint                 endpoint1;
    TriPoint                 torg, tdest;
    TriPoint                 leftpoint, rightpoint;
...
void insertsegment(TriPoint endpoint1, TriPoint endpoint2, int newmark)
{
    ...
    if (checkpoint != endpoint1)
    {
        ...
        {
            printf("Internal error in insertsegment():  Unable to locate PSLG TriPoint\n");
    ...
    while (virusloop != (TriTriangle**)NULL)
    {
        ...
        for (testtri.orient = 0; testtri.orient < 3; testtri.orient++)
        {
            ...
            if (testpoint != (TriPoint)NULL)
            {
                ...
                while ((neighbor.tri != dummytri) && (!TriOrientedTriangleequal(neighbor, testtri)))
                {
...
void regionplague(REAL attribute, REAL area)
{
    ...
    while (virusloop != (TriTriangle**)NULL)
    {
        ...
        for (testtri.orient = 0; testtri.orient < 3; testtri.orient++)
        {
            ...
            if ((neighbor.tri != dummytri) && !infected(neighbor) && (neighborshelle.sh == dummysh))
            {
                if (verbose > 2)
                {
...
void carveholes(REAL* holelist, int holes, REAL* regionlist, int regions)
{
    ...
    if ((holes > 0) && !noholes)
    {
        ...
        for (i = 0; i < 2 * holes; i += 2)
        {
            ...
            if ((holelist[i] >= xmin) && (holelist[i] <= xmax) && (holelist[i + 1] >= ymin) &&
                (holelist[i + 1] <= ymax))
            {
...
enum circumcenterresult findcircumcenter(
    TriPoint torg, TriPoint tdest, TriPoint tapex, TriPoint circumcenter, REAL* xi, REAL* eta)
{
    ...
    while (triangleloop.tri != (TriTriangle*)NULL)
    {
        for (triangleloop.orient = 0; triangleloop.orient < 3; triangleloop.orient++)
        {
            ...
            if ((triangleloop.tri < trisym.tri) || (trisym.tri == dummytri))
            {
                ...
                triangleloop.tri[highorderindex + triangleloop.orient] = (TriTriangle)newpoint;
...
void writeelements(int** trianglelist, REAL** triangleattriblist)
{
    ...
    {
        *trianglelist =
            (int*)malloc(triangles.items * ((order + 1) * (order + 2) / 2) * sizeof(int));
        if (*trianglelist == (int*)NULL)
        {
            printf("Error:  Out of memory.\n");
            exit(1);
...
void statistics()
{
    ...
    if (verbose)
    {
        ...
        printf(
            ...
            points.maxitems * points.itembytes + triangles.maxitems * triangles.itembytes +
                shelles.maxitems * shelles.itembytes + viri.maxitems * viri.itembytes +

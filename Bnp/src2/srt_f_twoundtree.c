    ...
    SrtUndInfo*   und_info)
{
    ...
    /* Gets the models from the FIRST (== DOMESTIC) underlying and initialise the Time Info in the
     * steps accordingly */
    ...
    /* Gets the models from the SECOND underlying and initialise the Time Info in the steps
     * accordingly */
    ...
    for (stp = bot; stp != NULL; stp = stp->prev)
    {
        ...
        if (mdl_type2 == LGM)
        {
            ...
            node.quanto_fwd =
                tminf_irm2->quanto_adjustment * tminf_irm2->ev.onef.sig * stp->delta_t;
        }
        else if ((mdl_type2 == BLACK_SCHOLES) || (mdl_type2 == EQ_STOCH_RATES))
        {
        ...
        for (i = -trinf->max_index[0]; i <= trinf->max_index[0]; i++)
        {
            ...
            for (j = -trinf->max_index[1]; j <= trinf->max_index[1]; j++)
            {
                ...
                /* Sets the value of the SECOND state variable and rebuilds the underlying process
                 * value */
                if (mdl_type2 == LGM)
                {
                    sam_get(node.cur_sam, 1, STATEVAR)   = x2;
                    sam_get(node.cur_sam, 1, SHORT_RATE) = sam_get(node.cur_sam, 1, STATEVAR) +
                                                           tminf_irm2->rf.onef.H +
                ...
                if (stp->next)
                {
                    ...
                    populate_twound_tree_node(
                        mdl_type1, mdl_type2, stp, &node, stp->delta_t, stp->next->trinf);
    ...
    err = srt_f_IOstructsetpremium(
        (iolist), SRT_NO, next_assets[0][0][grfnparam->node_dim - 1], "Two Und Tree");

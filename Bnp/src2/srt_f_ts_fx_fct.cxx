...
double find_fx_sig(double time, TermStruct *l) {
  ...
  while ((ls != NULL) &&
         (((FxTermStructVal *)ls->element->val.pval)->time < time))
    ls = ls->next;
...
/* W_dx def: int(s = 0        , s = t)
 * [pdx(s)*s_x(s)*s_dom(s)*psi_dom(s)/F_dom(s)] ds
 */
...

{
  ...
  result = tsval->W_dx +
           tsval->rhodx * tsval->sigx * dom_sig * (dom_Psi / dom_F) * temp +
           tsval->rhodx * tsval->sigx * dom_sig * dom_tau * (temp - time);

  return result;
}

/*O_fd def: int(s = 0        , s = t)
 * [pfd(s)*s_dom(s)*s_for(s)/(F_dom(s)*F_for(s))] ds */
...
/* int ( s = 0        , s = t) [pfd(s)*s_ifr_dom(s        ,t)*s_ifr_for(s ,t)]
 * ds = F_dom(t)*F_for(t)*O_fd(t) */
double Phi_fd_func(double time, TermStruct *l) {
  ...
  temp =
      tsval->rhofd * tsval->StochRates_Ts[1].sig * tsval->StochRates_Ts[0].sig *
      (exp((tsval->StochRates_Ts[0].lambda + tsval->StochRates_Ts[1].lambda) *
           time) -
       1) *
      1.0 / (tsval->StochRates_Ts[0].lambda + tsval->StochRates_Ts[1].lambda);

  temp1 = exp(-time * (tsval->StochRates_Ts[0].lambda +
                       tsval->StochRates_Ts[1].lambda));
...
double H_fd_func(double time, TermStruct *domts, TermStruct *forts,
                 TermStruct *l) {
  double result;
...
/* R_fd_func: int (s = 0        , s = t)
 * [rho_fd(s)*s_dom(s)*s_for(s)*Psi_dom(s)/(F_for(s)*F_dom(s))] ds */
...

{
  SrtLst *ls;
  FxTermStructVal *tsval, *tsval_p;
  double dom_sig, for_sig, dom_tau, for_tau, dom_F, for_F, dom_Psi, forPsi,
      lambda;
  ...
  result =
      tsval->M_fd +
      tsval->rhofd * (temp / (dom_F * for_F)) * dom_Psi * forPsi * temp3 +
...
double Q_fd_func(double time, TermStruct *l) {
  ...
  result =
      tsval->Q_fd - for_F * tsval->O_fd * dom_Psi * temp -
      for_F * dom_F * tsval->O_fd * dom_tau * (temp - temp1) -
      tsval->rhofd * dom_sig * for_sig * dom_Psi / (dom_F * lambda) *
          (temp2 - temp) -
      tsval->rhofd * dom_sig * for_sig * dom_tau / lambda *
          (temp2 - temp - time + temp1) +
      for_F * tsval->P_fd * temp +
      tsval->rhofd * dom_Psi * dom_sig * for_sig / (lambda * dom_F) *
          (temp2 - temp) +
...
double T_fd_func(double time, TermStruct *l) {
  ...
  result = tsval->T_fd + for_F * tsval->S_fd * temp +
           tsval->rhofd * for_F * temp1 * (temp2 - temp3);
...
double U_fd_func(double time, TermStruct *l) {
  SrtLst *ls;
  FxTermStructVal *tsval, *tsval_p;
  double dom_tau, for_tau, dom_sig, for_sig, dom_F, for_F, dom_Psi, temp, temp1,
      temp2, lambda;
  double result;
  result = 0.0;
  ...
  result = tsval->U_fd + for_F * dom_Psi * tsval->O_fd * temp +
           ...
           dom_Psi * (tsval->rhofd / lambda) * (dom_sig * for_sig / (dom_F)) *
               (temp1 - temp) +
           tsval->rhofd * (dom_sig * for_sig * dom_tau / lambda) *
...
/* W_fd def: int(s=0        , s = t) [
 * p_fd(s)*s_dom(s)*s_for(s)*Psi_dom(s)/F_dom(s)] ds */
...

{
  ...
  result = tsval->W_fd +
           (tsval->rhofd * dom_sig * for_sig / dom_F) * dom_psi * dTemp +
...

{
  ...
  result = tsval->Y_dx +
           tsval->rhodx * tsval->sigx * dom_sig * (dom_Psi / SqdomF) * temp1 +
...

{
  ...
  result = tsval->Y_fx +
           tsval->rhofx * tsval->sigx * for_sig * (forPsi / sqforF) * temp1 +
...

{
  ...
  result = tsval->W_fx +
           tsval->rhofx * tsval->sigx * for_sig * (forPsi / for_F) * temp +
...
/* -------------------------------------------------------------------------------------------
   This Function computes the BS Implied Volatility of an FX Option in a
   stochastic world framework. The Domestic Underlying and the Foreign
   Underlying has to follow a LGM dynamic.

   The domestic        , the foreign and the FX Underlying must be first
   initialised
   -------------------------------------------------------------------------------------------
 */

Err srt_f_get_fx_implied_vol(double yr_to_exp, String fx_und_names,
                             double *fx_bs_vol) {
  ...
  if (err)
    return serror(
        "Can not get the fx term structure in srt_f_get_fx_implied_vol ");

  /* If only a Standard Fx Stoch Rates model        , return the Sqrt (cumVol /
   * T) */
  ...
  if (err)
    return serror(
        "Can not get the domestic term structure in srt_f_get_fx_implied_vol");

  err = get_underlying_ts(for_und, &for_ts);
  if (err)
    return serror(
        "Can not get the foreign term structure in srt_f_get_fx_implied_vol");
  ...
  dom_bd_vol = (-sq_dom_psi * dom_G + 2 * dom_psi * dom_L + dom_G * sq_dom_psi -
                2 * dom_O) /
               yr_to_exp;
  ...
  for_bd_vol = (-sq_for_psi * for_G + 2 * for_psi * for_L + for_G * sq_for_psi -
                2 * for_O) /
               yr_to_exp;

  /* Compute the BS Implied Volatility of the Spot FX */
  spot_fx_cum_vol =
      1.0 / yr_to_exp * fx_cum_vol_func(yr_to_exp, SRT_YES, fx_ts);

  /* Compute the Covariance between the Spot Fx and the Domestic zero coupon
   * bond */
  ...
  cov_dom_int_for_int =
      dom_psi * for_psi * Ofd - for_psi * Rfd - dom_psi * Pfd + Mfd;

  cov_for_int_fx = -for_psi * Vfx + Wfx;

  sq_fx_bs_vol = spot_fx_cum_vol + dom_bd_vol + for_bd_vol +
                 (2 / yr_to_exp) * cov_for_int_fx -
                 (2 / yr_to_exp) * cov_dom_int_fx -
                 (2 / yr_to_exp) * cov_dom_int_for_int;
...
Err srt_f_get_fx_stoch_rates_correl(double start_date, double end_date,
                                    String fx_und_names, double **corr_matrix,
  ...
  if (err)
    return serror(
        "Can not get the FX term structure in srt_f_get_fx_stoch_rates_correl");
  ...
  if (dom_model_type != LGM || for_model_type != LGM)
    return serror("srt_f_get_fx_stoch_rates_correl(...)  will only work for "
                  "LGM/LGM .... ");

  /* Get the dom & for & Fx ts */
  err = get_underlying_ts(dom_und, &dom_ts);
  if (err)
    return serror("Can not get the domestic term structure in "
                  "srt_f_get_fx_stoch_rates_correl");

  err = get_underlying_ts(for_und, &for_ts);
  if (err)
    return serror("Can not get the foreign term structure in "
                  "srt_f_get_fx_stoch_rates_correl");
  ...
  cov_int_dom_for =
      dom_psi * for_psi * Ofd_dt - for_psi * Rfd_dt - dom_psi * Pfd_dt + Mfd_dt;
...
Err srt_f_get_fx_stoch_rates_ind_correl(double start_date, double end_date,
  ...
  err = srt_f_get_fx_stoch_rates_correl((start_date - today) * YEARS_IN_DAY,
                                        (end_date - today) * YEARS_IN_DAY,
                                        fx_und_name, &corr_matrix, &var_ln_fx);
  if (err)
    return err;
  ...
  if (((!strcmp(first_und_name, fx_und_name)) &&
       (!strcmp(second_und_name, dom_und_name))) ||
      ((!strcmp(first_und_name, dom_und_name)) &&
       (!strcmp(second_und_name, fx_und_name))))
    *corr_or_var = corr_matrix[0];
  else if (((!strcmp(first_und_name, fx_und_name)) &&
            (!strcmp(second_und_name, for_und_name))) ||
           ((!strcmp(first_und_name, for_und_name)) &&
            (!strcmp(second_und_name, fx_und_name))))
    *corr_or_var = corr_matrix[1];
  else if (((!strcmp(first_und_name, dom_und_name)) &&
            (!strcmp(second_und_name, for_und_name))) ||
           ((!strcmp(first_und_name, for_und_name)) &&
            (!strcmp(second_und_name, dom_und_name))))

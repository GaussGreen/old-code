...
Err SrtDisplayUndTermStruct(char *UndName,
  ...
  if (!ISUNDTYPE(und, INTEREST_RATE_UND))
    return serror("%s is not an Interest Rate Underlying: cannot display TS",
                  UndName);
  err = get_underlying_mdldim(und, &mdl_dim);
  if (err)
    return err;
  ...
  /* Aloocate memory and populates the output TermStruct according to the Model
   * dimension */
  if (mdl_dim == ONE_FAC) {
    err = get_underlying_mdltype(und, &mdl_type);
    ...
    if (mdl_type == LGM_STOCH_VOL) {
      ...
      *NumTauCols = 2;
      ...
      err = srt_f_display_IRM_OneFac_TermStruct(
          ts, &((*SigmaCurve)[0]), &((*SigmaCurve)[1]), &((*SigmaCurve)[2]),
          &((*SigmaCurve)[3]), &((*SigmaCurve)[5]), &((*SigmaCurve)[4]),
          NumSigmaRows, &((*TauCurve)[0]), &((*TauCurve)[1]), NumTauRows);
      if (err) {
      ...
      *NumTauCols = 2;
      ...
      err = srt_f_display_IRM_OneFac_TermStruct(
          ts, &((*SigmaCurve)[0]), &((*SigmaCurve)[1]), &((*SigmaCurve)[2]),
          &temp1, &temp2, &temp3, NumSigmaRows, &((*TauCurve)[0]),
          &((*TauCurve)[1]), NumTauRows);
    ...
    *NumTauCols = 3;
    ...
    err = srt_f_display_IRM_TwoFac_TermStruct(
        ts, &((*SigmaCurve)[0]), &((*SigmaCurve)[1]), &((*SigmaCurve)[2]),
        &((*SigmaCurve)[3]), &((*SigmaCurve)[4]), &((*SigmaCurve)[5]),
        NumSigmaRows, &((*TauCurve)[0]), &((*TauCurve)[1]), &((*TauCurve)[2]),
        NumTauRows);
    if (err) {
      for (i = 0; i < (*NumSigmaCols); i++)
        srt_free(*SigmaCurve[i]);

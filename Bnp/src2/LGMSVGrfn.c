...
double dDerBeta, dBetaDerBeta, dBTTstar, dBeta, dBeta2, coef;
	...
	for (i=lphi; i<=uphi; i++)
	{
		...
		for (j=lx; j<=ux; j++)
		{
			...
			for (k=0; k<total->num_df; k++)
			{
				local->evt->df[0][k] = total -> dff[k] 
					* exp (- total->gam[k] * tmpx - total->gam2[k] * tmpphi);
			}			
	...
	for (i=lz; i<=uz; i++)
	{
		...
		for (j=lx; j<=ux; j++)
		{
			...
			{												
				...
				for (m=0; m<total->num_df; m++)
				{
					local->evt->df[0][m] =  exp (total -> dff[m] - total->gam[m] * tmpf + total->gam2[m] * psi[k]);
				}			
 ...
 *						Monte Carlo								*
 ****************************************************************/
Err payoff_lgmsv_mc(long	path_index,
					double	evt_date,
					double	evt_time,
					void	*func_parm, 					
...
static Err		err	= NULL;
	...
	for (k=0; k<total->num_df; k++)
	{
		local->evt->df[0][k] = exp (total->dff[k]
									- total->gam1[k] * ft1 
									- total->gam2[k] * ft2
									- total->gam1_2[k] * psi1
									- total->gam2_2[k] * psi2

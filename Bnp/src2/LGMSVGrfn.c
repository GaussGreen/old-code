    ...
    double**** prod_val)
{
    ...
    /* Multiplication of the old payoff by B(EventDate,Tstar) to  obtain the PV at the event date */
    dBTTstar = swp_f_df(evt_date, evt_date + (dTstar - evt_time) * 365.0, (char*)yc);
    for (i = lphi; i <= uphi; i++)
    {
        tmpphi = phi[i];
        for (j = lx; j <= ux; j++)
    ...
    for (i = lphi; i <= uphi; i++)
    {
        ...
        for (j = lx; j <= ux; j++)
        {
            ...
            {
                local->evt->df[0][k] =
                    total->dff[k] * exp(-total->gam[k] * tmpx - total->gam2[k] * tmpphi);
            }

            for (l = leps; l <= ueps; l++)
            {
                err = FIRSTEvalEvent(global, local, nprod, 2, NULL, NULL, prod_val[i][j][l], &temp);
                if (err)
                {
                    return err;
                }
    ...
    double**** prod_val)
{
    ...
    for (i = lphi; i <= uphi; i++)
    {
        ...
        for (j = lx; j <= ux; j++)
        {
            ...
            df = param->cpn_df[k] * exp(-param->gam[k] * tmpx - param->gam2[k] * tmpphi);
    ...
    double**** prod_val)
{
    ...
    static double dDerBetalamx, dLnBTTstar, dUtRhoDivAlpha, dlnCoef, coef, coef_tmp;
    ...
    dLnBTTstar = log(swp_f_df(evt_date, evt_date + (dTstar - evt_time) * DAYS_IN_YEAR, (char*)yc));
    ...
    for (i = lz; i <= uz; i++)
    {
        ...
        for (j = lx; j <= ux; j++)
        {
            ...
            for (k = lpsi; k <= upsi; k++)
            {
                /* Multiplication of the old payoff by B(EventDate,Tstar) to  obtain the PV at the
                 * event date */
                ...
                {
                    local->evt->df[0][m] =
                        exp(total->dff[m] - total->gam[m] * tmpf + total->gam2[m] * psi[k]);
                }

                err =
                    FIRSTEvalEvent(global, local, *nprod, 2, NULL, NULL, prod_val[i][j][k], &temp);
                if (err)
                {
                    return err;
                }
    ...
    double*** prod_val)
{
    ...
    for (iNumPhi = 1; iNumPhi <= iNbPhi; iNumPhi++)
    {
        ...
        for (iNumft = 1; iNumft <= iNbft; iNumft++)
        {
            ...
            err = FIRSTEvalEvent(
                global, local, nprod, 2, NULL, NULL, prod_val[iNumPhi][iNumft], &temp);
            if (err)
            {
                return err;
            }
    ...
    int*    stop_path)
{
    ...
    for (k = 0; k < total->num_df; k++)
    {
        local->evt->df[0][k] = exp(total->dff[k] - total->gam[k] * ft - total->gam2[k] * psi);
    }
    ...
    for (k = 0; k < total->num_df; k++)
    {
        local->evt->df[0][k] =
            exp(total->dff[k] - total->gam1[k] * ft1 - total->gam2[k] * ft2 -
                total->gam1_2[k] * psi1 - total->gam2_2[k] * psi2 - total->gam12[k] * psi12);
    }

...
Err swp_f_Cms_Option(
  ...
  /* Maturity is non zero */
    ...
    if (PayRec == SRT_RECEIVER) {
      /* If the Model is Lognormal        , we have to restrict to positive
       * strikes */
      if (CMS_SPACE_TYPE == SRT_LOGNORMAL) {
        ...
        /* If there is not enough space for two swaptions        , just do the
         * one and return */
...
Err swp_f_Tec_Option(
  ...
  if (dMaturity <= 0.0) {
    /* At maturity or zero vol        , so value is
                    MAX((1+CmsForward+m)^(1/k)-(1+dStrike)^(1/k)        ,0) for
       EPAY , etc */
...
Err CMSTECsetupvol(CMSVol *Volstruct, CMSGreeks *Greekstruct, double strike,
             ...
             Volstruct->input_method == 250) // 3 for BMM in test
  {
    ...
    Volstruct->end        , swaprate        , &yesno        ,  &power        ,
    SABR_BETAVOL);
    */
  ...
  if (Volstruct->input_SABR == 1) {
    ...
    /*	Calculate equivalent vol of the type to be frozen	only ATM types
     * or BETA*/
...
Err swp_f_FinalCms_Opts(           /* underlying */
  ...
  // double atmvol        ,newbetavol        , newbeta        , newalpha ,
  // newrho        , newprob; int i;
  ...
  /* for speed reasons and in order to be ready for the Heston vol        , we
   * sort by method */
...
Err swp_f_CMSTEC_Weights(              /* underlying */
                         ...
                         /* delay parameters */
                         double dCvgZC, /* cvg of the delay */
                         double dspdZC, /* spread to retreive B(0        ,Tpay)
                                           / B(0 ,Tstart) */
                         /* numerical param */
                         CMSParams *params,
                         /* Output */

/* ---------------------------------------------------------------------------------
   ...
   PURPOSE:          The main core for the random number generation when using 
					 ...
					 variables, that should be translated into Brownian increments
					 for each time step.
   --------------------------------------------------------------------------------- */
...
static Err srt_f_extract_time_at_steps(
								SrtStpPtr  top, 
...
Err     err = NULL;
	...
	/* BalSam returns a cube [path][brownian][step] */			
		err = BalSampCube( rand, first_path, last_path, 0, nbr-1,
	...
	/* ABS returns a cube [path][brownian][step] */			
		err = ABSCube( rand, first_path, last_path, 0, nbr-1,
							first_step, last_step, seed);
		if (err)
			return err;                                        	
...
/* Dynamic methods: just initialisation and memory allocation FOR ONE PATH is done */
	if (gen_method == RANDOM_GAUSS_DYNAMIC || 
		gen_method == ANTITHETIC_DYNAMIC|| 
		gen_method == SOBOL||
		gen_method == UWAN )
	{
		if (gen_method == SOBOL)
...
/* Non dynamic methods: full memory allocation and generation is done here*/
	...
	/* Generates and stores the numbers in the large mcrs->r[path][brownian][step] array */
		err = srt_f_New_gen_random(	mcrs->r, 
								mcrs->pl, mcrs->ph, 
								mcrs->sl, mcrs->sh, 
								mcrs->nbr, 
								gen_method, 
...
/* set the index to the first path */
	...
	/* DYNAMIC methods first: no initial storage of the full cube */
		...
		/* Fills in the random numbers for this path */	
			if (err = gauss_matrix(	mcrs->r[index], 
									0, mcrs->nbr-1, 
									mcrs->sl, mcrs->sh, 
									&mcrs->seed) ) 
				return err; 
		...
		/* Fills in the random numbers for this path */	
			if (err = sobol_matrix(	mcrs->r[index], 
									0, mcrs->nbr-1, 
									mcrs->sl, mcrs->sh) )
				return err;
		...
		if(mcrs->sample_type == UWAN )
		{

		/* Fills in the random numbers for this path */	
			if (err = uwan_matrix(	mcrs->r[index], 
									0, mcrs->nbr-1, 
									mcrs->sl, mcrs->sh, &mcrs->seed) )
				return err; 
		...
		if(mcrs->sample_type == ANTITHETIC_DYNAMIC)
		{
...
/* If the need_to_correl flag is set to SRT_NO, no work has to be done */
	...
	/* Test if stp is well defined */
		...
		while (stp->next)
		{
			...
			{
				err = correl_random(	mcrs->r[mcrs->cur_path],
										step_index,
										step_index,
										mcrs->nbr, 
					   					stp->coeff);
...
/* DYNAMIC methods first: no initial storage of the full cube */
	...
	/* Fills in the random numbers for this path */	
		if (err = gauss_matrix(	mcrs->r[mcrs->pl], 
								0, mcrs->nbr-1, 
								mcrs->sl, mcrs->sh, 
								&mcrs->seed) ) 
			return err; 
	...
	/* Fills in the random numbers for this path */	
		if (err = sobol_matrix(	mcrs->r[mcrs->pl], 
								0, mcrs->nbr-1, 
								mcrs->sl, mcrs->sh) )
			return err;
	...
	/* Fills in the random numbers for this path */	
		if (err = uwan_matrix(	mcrs->r[mcrs->pl], 
								0, mcrs->nbr-1, 
								mcrs->sl, mcrs->sh, &mcrs->seed) )
			return err; 

/* ---------------------------------------------------------------------------------
   ...
   PURPOSE:          The main core for the random number generation when using
                     Monte Carlo. The numbers finally output correspond to normal
                                         variables, that should be translated into Brownian
...
static Err srt_f_extract_time_at_steps(SrtStpPtr top, double** time_at_steps, long* size);

/* -----------------------------------------------------------------------
   PRIVATE FUNCTION : NOT TO BE CALLED FROM THE OUTSIDE
   Function which calls relevent routines, depending on generation method
   to fill a cube
    ...
    SrtStpPtr    top)
{
    ...
    if (gen_method == BAL_SAMPLING)
    {
        ...
        err = BalSampCube(rand, first_path, last_path, 0, nbr - 1, first_step, last_step, seed);
        if (err)
            return err;
    ...
    else if (gen_method == ABS)
    {
        ...
        err = ABSCube(rand, first_path, last_path, 0, nbr - 1, first_step, last_step, seed);
        if (err)
            return err;
    ...
    {
        err = gauss_cube(rand, first_path, last_path, 0, nbr - 1, first_step, last_step, seed);
        if (err)
            return err;
    ...
    else if (gen_method == ANTITHETIC)
    {
        ...
        err = gauss_cube(
            rand, first_path, first_path + segment, 0, nbr - 1, first_step, last_step, seed);
        if (err)
            return err;
        ...
        if (gen_method == BAL_ANTI)
    {
        ...
        /* Generates the first half ( + 1 point if odd) of rand[i] with Balance Sampling */
        err = BalSampCube(
            rand, first_path, first_path + segment, 0, nbr - 1, first_step, last_step, seed);
        if (err)
            return err;
...
/*---------------------------------------------------------------------------------------------------------*/
/*New function that does the same as the previous one but which does not take a SrtStpPtr as an
input but only the vector of times_at_steps  which is the only thing really needed*/
...
/* -------------------------------------------------------------------------
        ...
        USE THIS STRUCTURE AND IT'S FUNCTIONS TO GET YOUR RANDOM SAMPLES,
        FOR MONTE CARLO SIMULATION.
        RIGHT NOW IT GENERATES ALL SAMPLES AT ONCE, BUT
        IT COULD CONCEIVABLY BE READING NUMBERS FROM A FILE,
        OR GENERATING SAMPLES ON THE FLY.
   ------------------------------------------------------------------------- */
    ...
    SrtStpPtr    top)
{
    ...
    /* For the moment, the current path is one before the first one (increment after in MC...next)
     */
    mcrs->cur_path = mcrs->pl - 1;

    /* Check the inputs: something ha to be generated (for Evolve functions to work)  */
    if (mcrs->sl > mcrs->sh)
        mcrs->sl = mcrs->sh;

    /* Dynamic methods: just initialisation and memory allocation FOR ONE PATH is done */
    ...
    } /* END if method = RANDOM_GAUSS_DYNAMIC, SOBOL, UWAN, ANTI_DYNAMIC */

    else

    /* Non dynamic methods: full memory allocation and generation is done here*/
    {
        /* Allocate some initial memory for all the random numbers [path][brow][step] */
        mcrs->r = dcube(mcrs->pl, mcrs->ph, 0, mcrs->nbr - 1, mcrs->sl, mcrs->sh);
        if (!mcrs->r)
            return serror("Couldn't allocate space for random numbers");

        /* Sets all the values to 0.0 for safety reasons */
        memset(&(mcrs->r[0][0][0]), 0, nbr * (sh - sl + 1) * (ph - ph + 1) * sizeof(double));

        /* Generates and stores the numbers in the large mcrs->r[path][brownian][step] array */
        err = srt_f_gen_random(
            mcrs->r,
            mcrs->pl,
            mcrs->ph,
            mcrs->sl,
...
/*----------------------------------------------------------------------------------------------*/
/*New Function which is the same as the previous one only taking times_at_steps instead as a
SrtStpPtr as an input*/
...
/* ------------------------------------------------------------------------
   ...
   according to the method required, and the PATH NUMBER
   If numbers have been generated before (non DYNAMIC methods), this
   function makes sure that mcrs->r[i] points to the right path.
   Memory allocation has to be made before for the mcrs->r[pathindex][][]
   matrix,and for the mcrs
   Once this function has been called, mcrs->r[i] could be used for
   i = pl ... ph [paths].
   ------------------------------------------------------------------------ */
...
Err SrtMCRanSrc_init(SrtMCRanSrc* mcrs)
{
    ...
    for (index = mcrs->pl; index < mcrs->ph; index++)
    {
        ...
        if (mcrs->sample_type == RANDOM_GAUSS_DYNAMIC)
        {
            ...
            if (err =
                    gauss_matrix(mcrs->r[index], 0, mcrs->nbr - 1, mcrs->sl, mcrs->sh, &mcrs->seed))
                return err;
        ...
        {
            /* Fills in the random numbers for this path */
            if (err = sobol_matrix(mcrs->r[index], 0, mcrs->nbr - 1, mcrs->sl, mcrs->sh))
                return err;
        ...
        else if (mcrs->sample_type == UWAN)
        {
            /* Fills in the random numbers for this path */
            if (err =
                    uwan_matrix(mcrs->r[index], 0, mcrs->nbr - 1, mcrs->sl, mcrs->sh, &mcrs->seed))
                return err;
        ...
        else if (mcrs->sample_type == ANTITHETIC_DYNAMIC)
        {
            ...
            /* If last generated path number is even, generate a new path (Brow * steps) */
            {
                if (err = gauss_matrix(
                        mcrs->r[index], 0, mcrs->nbr - 1, mcrs->sl, mcrs->sh, &mcrs->seed))
                    return err;
            }
            else /* (mcrs->lastindex - mcrs->pl) % 2 == 1 */
            /* If path number is odd, use the previous one and make it symetrical*/
            {
...
/* ------------------------------------------------------------------------
   ...
   The correlated numbers overwrite the ones in mcrs->r[mcrs->cur_path][...][...]
   This function uses SrtStrPtr to associate the right correlation matrix to
   the right step, assuming that one step correspond to one move in the
   list and one indexation in the random numbers matrix
   The correlation is made for:
...
Err SrtMCRanSrc_correl(SrtMCRanSrc* mcrs, SrtStpPtr stp)
{
    ...
    if (mcrs->nbr == 1)
        ...
        /* If the need_to_correl flag is set to SRT_NO, no work has to be done */
    ...
    else
    {
        ...
        while (stp->next)
        {
            if (step_index > mcrs->sh)
                return serror("SrtStpPtr and mcrs do not correspond in SrtMCRanSrc_correl");
            if (!stp->coeff)
            ...
            if (1)
            {
                ...
                err = correl_random_eigen(
                    mcrs->r[mcrs->cur_path], step_index, step_index, mcrs->nbr, stp->coeff);
            }
            else
            {
                err = correl_random(
            ...
            /* Increment the step index by one, and move to next one in the list*/
            step_index++;
            stp = stp->next;
...
/* ------------------------------------------------------------------------
   ...
   Memory allocation has to be made before for the mcrs->r[pathindex][][]
   matrix,and for the mcrs
   Once this function has been called, mcrs->lastindex refers to the index
   of the path that has just been generated.
   ------------------------------------------------------------------------ */
...
Err SrtMCRanSrc_next(SrtMCRanSrc* mcrs)
{
    ...
    if (mcrs->sample_type == RANDOM_GAUSS_DYNAMIC)
    {
        ...
        /* Fills in the random numbers for this path */
        if (err =
                gauss_matrix(mcrs->r[mcrs->pl], 0, mcrs->nbr - 1, mcrs->sl, mcrs->sh, &mcrs->seed))
            return err;
    ...
    else if (mcrs->sample_type == SOBOL)
    {
        ...
        /* Fills in the random numbers for this path */
        if (err = sobol_matrix(mcrs->r[mcrs->pl], 0, mcrs->nbr - 1, mcrs->sl, mcrs->sh))
            return err;
    ...
    else if (mcrs->sample_type == UWAN)
    {
        ...
        /* Fills in the random numbers for this path */
        if (err = uwan_matrix(mcrs->r[mcrs->pl], 0, mcrs->nbr - 1, mcrs->sl, mcrs->sh, &mcrs->seed))
            return err;
    ...
    else if (mcrs->sample_type == ANTITHETIC_DYNAMIC)
    {
        ...
        {
            if (err = gauss_matrix(
                    mcrs->r[mcrs->pl], 0, mcrs->nbr - 1, mcrs->sl, mcrs->sh, &mcrs->seed))
                return err;
        }
        else /* (mcrs->lastindex - mcrs->pl) % 2 == 1 */
        /* If path number is odd, use the previous one and make it symetrical*/
...
Err SrtMCRanSrc_end(SrtMCRanSrc* mcrs)
{
    ...
    if (mcrs->r)
        free_dcube(mcrs->r, mcrs->pl, mcrs->ph, 0, mcrs->nbr - 1, mcrs->sl, mcrs->sh);
...
static Err srt_f_extract_time_at_steps(SrtStpPtr top, double** time_at_steps, long* size)
{
    ...
    /* Compute the number of time steps (hopefully, time is 0 at first step) */
    i   = 0;
    stp = top;
    while (stp)
    {
    ...
    *size = i;
    ...
    /* Sets all the times (from today to step date) into a single vector for all the step*/
    stp = top;
    for (i = 0; i < *size; i++)
    {

/* ---------------------------------------------------------------------------------
   ...
   normal variables  , that should be translated into Brownian increments for
   each time step.
   ---------------------------------------------------------------------------------
 */
...
Err SrtMCRanSrc_start(SrtMCRanSrc *mcrs, long pl, long ph, long sl, long sh,
  ...
  /* For the moment  , the current path is one before the first one (increment
   * after in MC...next) */
...
/* ------------------------------------------------------------------------
   ...
   , according to the method required  , and the PATH NUMBER If numbers have
   been generated before (non DYNAMIC methods)  , this function makes sure that
   mcrs->r[i] points to the right path. Memory allocation has to be made before
   for the mcrs->r[pathindex][][] matrix  ,and for the mcrs Once this function
   has been called  , mcrs->r[i] could be used for i = pl ... ph [paths].
   ------------------------------------------------------------------------ */
...
/* ------------------------------------------------------------------------
   ...
   the right correlation matrix to the right step  , assuming that one step
   correspond to one move in the list and one indexation in the random numbers
   matrix The correlation is made for:
...
/* ------------------------------------------------------------------------
   ...
   , according to the method required  , and the PATH NUMBER If numbers have
   been generated before (non DYNAMIC methods)  , this function makes sure that
   mcrs->r[mcrs->cur_path] points to the right path. Memory allocation has to be
   made before for the mcrs->r[pathindex][][] matrix  ,and for the mcrs Once
   this function has been called  , mcrs->lastindex refers to the index of the
   path that has just been generated.
   ------------------------------------------------------------------------ */

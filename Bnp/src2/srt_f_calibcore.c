...
{
#ifdef PVMPI
	if (!PRL_CONTEXT.bLocalComp) SendInstruments(
		lClcnDate,
		lNumInstruments,
		psSwapDp,
		pdStrike,
 ...
 long			today)
{
	Err	err = NULL;
#ifdef PVMPI
	if (!PRL_CONTEXT.bLocalComp) SendSprdsht(und_name, today);
#endif
	...
	if (!PRL_CONTEXT.bLocalComp) {
		...
		STATIC_MODELVALUES.pdModelHessian = dmatrix(0,STATIC_PARAMS.lNumData-1, 0,STATIC_PARAMS.lNumParams-1);  
#ifdef PVMPI
	}
#endif
		  return err;
 ...
 long          lNumCols)
{
		...
		if(STATIC_SPRDSHT.pshtSHEET)
			grfn_free_GrfnCellmatrix(STATIC_SPRDSHT.pshtSHEET,STATIC_SPRDSHT.lNumRows,STATIC_SPRDSHT.lNumCols);
		
#ifdef PVMPI
		if (!PRL_CONTEXT.bLocalComp) {
#endif
			if(STATIC_MODELVALUES.pdModelValue) free_dvector(STATIC_MODELVALUES.pdModelValue,0,STATIC_PARAMS.lNumData-1);
			if(STATIC_MODELVALUES.pdModelHessian) free_dmatrix(STATIC_MODELVALUES.pdModelHessian,0,STATIC_PARAMS.lNumData-1, 0,STATIC_PARAMS.lNumParams-1);
...
{
#ifdef PVMPI	
	if (!PRL_CONTEXT.bLocalComp) SendParams( 
		eCalibType,
		lNumData,
		pdMarketWeights,
		pdMarketTargets,
...
{
#ifdef PVMPI
	if (!PRL_CONTEXT.bLocalComp) SendParamToDeallocate(				
		eModelDim,
		lNumSigmas,
		lNumTaus);
#endif	
...
static double std_sig_value (void)
{
	...
	for (i=1; i<=STATIC_PARAMS.lNumSigmas; i++)
	{
		...
		exp_sigma2 += STATIC_PARAMS.ppdSigmaValues[1][i-1] * STATIC_PARAMS.ppdSigmaValues[1][i-1];
	}
					   ...
					   double      *value) 
{
	...
	else if (data_index <= STATIC_INSTRUMENT.lNumInstruments 
		...
		double      *pdOptParams) /* From [1] to [STATIC_PARAMS.lNumParams] */
{
	...
	if ( are_calib_parameters_within_band(
		...
		STATIC_PARAMS.ppdParamBounds) == SRT_NO)
	{
		criteria = SRT_BIG;
		return criteria;
	}
	...
	int          lNumParams)
{
	...
	if (bFirstTime)
	{
		...
		for (i=1; i<=STATIC_PARAMS.lNumParams;i++)
		{
			...
			{
				STATIC_MODELVALUES.pdModelHessian[j][i-1] = (pdTempPrice[j] - STATIC_MODELVALUES.pdModelValue[j]) / shift;
		...
		nb_retries=0;
		do { 
			err = ParallelComputation(STATIC_PARAMS.lNumData, STATIC_PARAMS.lNumParams, pdOptParams);
			if (!strcmp(err,"ok")) {
				for (i=0;i<STATIC_PARAMS.lNumParams;i++) {
					...
					for (j=0 ; j < STATIC_PARAMS.lNumData; j++) {
						STATIC_MODELVALUES.pdModelHessian[j][i] -=  STATIC_MODELVALUES.pdModelValue[j];
				...
				if (++nb_retries>3) return "Too many successive iterations failed";
				if (err) return err;
			}
		} while (1);
#endif
	...
	MyTRACE("");
	MyTRACE("Parameters: ");
	for (i=0;i<STATIC_PARAMS.lNumParams;i++) MyTRACE1(" ", pdOptParams[i+1]);
	MyTRACE("");
	MyTRACE("Prices: ");
	for (i=0;i<STATIC_PARAMS.lNumData;i++) MyTRACE1(" ", STATIC_MODELVALUES.pdModelValue[i]);
	MyTRACE("");
	MyTRACE("Hessian: ");
...
#define free_calib_core_memory {\
	if (pdMarketWeights) free_dvector(pdMarketWeights , 1, lNumData);\
	if (pdMarketTargets) free_dvector(pdMarketTargets , 1, lNumData);\
	if (ppdParamBounds) free_dmatrix(ppdParamBounds ,1, 2, 1, lNumParams);\
	if (pdOptParams) free_dvector(pdOptParams , 1, lNumParams);\
 ...
 double			  *pdChiSquare)
{
	...
	if (eModelDim == TWO_FAC)
	{
		...
		if (psCalibParams->eCalibType == GLOBAL_CALIB)
		{
			...
			if (psCalibParams->eCalibType == FIXED_CALIB)
			{
	...
	if (psCalibParams->lNumIter > 0)
	{
		...
		/* --------------------- SIMULATED_ANNEALING --------------------------  */
			...
			if ( psCalibParams->eAlgoType == SIMULATED_ANNEALING )
			{
			...
			/* --------------------- SOBENBERG -------------------------  */
				...
				if ( psCalibParams->eAlgoType == SOBENBERG )
				{
				...
				/* --------------------------- SIMPLEX --------------------------------  */
					...
					if ( psCalibParams->eAlgoType == SIMPLEX )
					{
	...
	int *piCPUs, int Version)
{
	...
	} else PRL_CONTEXT.szGroupName=(char *) NULL;
...
{
		if (STATIC_PARAMS.pdMarketWeights) free_dvector(STATIC_PARAMS.pdMarketWeights , 1, STATIC_PARAMS.lNumData);
		STATIC_PARAMS.pdMarketWeights=NULL;
    	if (STATIC_PARAMS.pdMarketTargets) free_dvector(STATIC_PARAMS.pdMarketTargets , 1, STATIC_PARAMS.lNumData);
		STATIC_PARAMS.pdMarketTargets=NULL;
    	if (STATIC_PARAMS.ppdParamBounds) free_dmatrix(STATIC_PARAMS.ppdParamBounds ,1, 2, 1, STATIC_PARAMS.lNumParams);
...
void free_instruments()
{
	if (STATIC_INSTRUMENT.lNumInstruments>0) {
		if (STATIC_INSTRUMENT.psSwapDp) srt_free (STATIC_INSTRUMENT.psSwapDp);
		STATIC_INSTRUMENT.psSwapDp=NULL;
		if (STATIC_INSTRUMENT.pdStrike) srt_free(STATIC_INSTRUMENT.pdStrike);
		STATIC_INSTRUMENT.pdStrike=NULL;
		if (STATIC_INSTRUMENT.pdBondStrike) srt_free(STATIC_INSTRUMENT.pdBondStrike);
		STATIC_INSTRUMENT.pdBondStrike=NULL;
		if (STATIC_INSTRUMENT.peRecPay) srt_free(STATIC_INSTRUMENT.peRecPay);
		STATIC_INSTRUMENT.peRecPay = NULL;	
		if (STATIC_INSTRUMENT.peProductType) srt_free(STATIC_INSTRUMENT.peProductType);
		STATIC_INSTRUMENT.peProductType = NULL;	
		if (STATIC_INSTRUMENT.pszRefRateCode) free_svector_size(STATIC_INSTRUMENT.pszRefRateCode, 0, 
			STATIC_INSTRUMENT.lNumInstruments-1,32);
		STATIC_INSTRUMENT.pszRefRateCode=NULL;
	}
	if(STATIC_INSTRUMENT.psGrfnParams) free(STATIC_INSTRUMENT.psGrfnParams);
	STATIC_INSTRUMENT.psGrfnParams=NULL;
	if (STATIC_INSTRUMENT.pdFraMaturities) free_vector(STATIC_INSTRUMENT.pdFraMaturities, 0, STATIC_INSTRUMENT.lNumTenors-1);

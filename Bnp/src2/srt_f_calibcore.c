...
(i.e.dOptionsWeight * varerr to frd + (1-dOptionsWeight) * varerr to correl )
  ...
  Also  , if the parameters go outside the ppdParamBounds  , the function
returns a SRT_BIG value that prevents the algorithm from using that point.
...
Err srt_f_calib_core(SrtGrfnParam *psGrfnParams, SrtModelType eModelType,
  ...
  /* If the sigma smoothing is also a criteria  , add one more pdData to "fit"
   * (the sigam variance) */
  ...
  /* Sets weigths for minimisation criteria using vegas  , options weights and
   * sizes */
  ...
  if (psCalibParams->lNumIter > 0) {
    ...
    if (psCalibParams->eAlgoType == LEVENBERG_MARQUARDT) {
      ...
      /* Sets a vector of x pdData: here  , it is 1  ,2  ,3..: the market pdData
       * index */

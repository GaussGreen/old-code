...
static void GrfnChooserCapletCell(SrtReceiverType eCapFloor, double dStrike,
                                  String szCashBasis, String szUndName,
                                  String szRefRateCode, String szCapletCell) {

  if (eCapFloor == SRT_RECEIVER) {
    sprintf(szCapletCell,
            "max(%f - FRA(a[0        ,i]        ,a[1        ,i]        ,\"%s\" "
            "       ,\"%s\"        ,\"%s\")  "
            "      ,0)*a[2        ,i]*df(now        ,a[1        ,i])",
            dStrike, szCashBasis, szUndName, szRefRateCode);
  } else if (eCapFloor == SRT_PAYER) {
    sprintf(szCapletCell,
            "max(FRA(a[0        ,i]        ,a[1        ,i]        ,\"%s\"      "
            "  ,\"%s\"        ,\"%s\") - %f  "
            "      ,0)*a[2        ,i]*df(now        ,a[1        ,i])",
            szCashBasis, szUndName, szRefRateCode, dStrike);
  }
...
/* Midat cell : if Startcolumn the max(PV[j]        ,c[0        ,i]) otherwise
 * max(PV[j] ,c[0        ,i]+PV[j-1]) */
...
/* Barrier cell final cash flow: c[0        ,i] * c[1        ,i] (condition *
 * caplet) */
...
static void GrfnBarrierCapletCell(SrtReceiverType eCapFloor, double dStrike,
                                  String szCapletCell) {
  if (eCapFloor == SRT_RECEIVER) {
    sprintf(szCapletCell,
            "max(%f - c[0        ,i]        , 0) * a[2        ,i] * df(now     "
            "   , a[1        ,i])",
            dStrike);
  } else if (eCapFloor == SRT_PAYER) {
    sprintf(szCapletCell,
            "max(c[0        ,i] - %f        , 0) * a[2        ,i] * df(now     "
            "   , a[1        ,i])",
            dStrike);
  }
}
...
static void
GrfnDigitalConditionCell(SRT_Boolean bIsFirstCell, double dBarrier,
  ...
  if (bIsUpOrDown) /* Up case */
    sprintf(szFinalCashflowCell, "if(c[0        ,i]>%f        ,%d        ,%d)",
            dBarrier, (bIsInOrOut == SRT_YES ? 1 : 0),
  ...
  if (bIsFirstCell == SRT_NO)
    sprintf(szFinalCashflowCell,
            "if(c[1        ,i-1]=%d        ,%d        ,%s)",
            (bIsInOrOut == SRT_YES ? 1 : 0), (bIsInOrOut == SRT_YES ? 1 : 0),
            szFinalCashflowCell);
}

/* Function that build the full grfn tableau for a Chooser cap */
static Err GrfnChooserMakeTableau(long lNumEventDates, double dStrike,
                                  long lNumExercises, SrtReceiverType eCapFloor,
                                  String szCashBasis, String szUndName,
                                  String szRefRateCode, long *plNumRows,
                                  long *plNumCols, GrfnCell ***pppsTableau) {
  Err err = NULL;
  int i, j;

  /* Sets the tableau dimensions : lNumExercices + 1  columns (c[0] ->
   * N[lNumExercices] */
  ...
  *pppsTableau = GrfnCellmatrix(*plNumRows, *plNumCols, GRFN_DEF_ARGBUFSZ);
  ...
  for (i = 0; i < *plNumRows; i++) {
    ...
    GrfnChooserCapletCell(eCapFloor, dStrike, szCashBasis, szUndName,
                          szRefRateCode, (*pppsTableau)[i][0].sval);
...
static Err GrfnBarrierMakeTableau(long lNumEventDates, double dStrike,
                                  double dBarrier, SrtReceiverType eCapFloor,
                                  SRT_Boolean bIsInOrOut,
                                  SRT_Boolean bIsUpOrDown, String szCashBasis,
  ...
  *pppsTableau = GrfnCellmatrix(*plNumRows, *plNumCols, GRFN_DEF_ARGBUFSZ);
  ...
  for (i = 0; i < *plNumRows; i++) {
    ...
    GrfnDigitalConditionCell((i == 0 ? SRT_TRUE : SRT_NO), dBarrier, bIsInOrOut,
                             bIsUpOrDown, (*pppsTableau)[i][1].sval);
    (*pppsTableau)[i][1].type = GRFNSCELL;

    /* The third column: c[2] - Caplet at strike = dStrike */
    GrfnBarrierCapletCell(eCapFloor, dStrike, (*pppsTableau)[i][2].sval);
    (*pppsTableau)[i][2].type = GRFNSCELL;

    /* The fourth column: c[3] - Caplet at strike = dBarrier */
    GrfnBarrierCapletCell(eCapFloor, dBarrier, (*pppsTableau)[i][3].sval);
...
/* the following function is based on the FlexiCap original code and price the
 * ChooserCap */
Err SrtGrfnChooserPrice(
    ...
    int iNumGrfnParams, double dFullCapRealPrice,
    /* Outputs from Grfn */
    double *pdChooserPrice, double *pdGrfnFullCapFloorPrice, long *plNumRows,
  ...
  if (err = GrfnChooserMakeTableau(
          lNumDates, dStrike, lMaxNumExercise, eCapFloor, szCashBasis,
          szUndName, szRefRateCode, plNumRows, plNumCols, pppsGrfnTableau)) {
  ...
  if (err = srt_f_grfn(sUndPtr, &sGrfnParams, lNumDates, &plFixingDates,
                       plNumRows, plNumCols, pppsGrfnTableau, 0, NULL,
                       (*plNumAuxColumns), (*pplAuxRangesLength),
                       (*pppdAuxRanges), psIOList, (*pppdLastPath), 0)) {
...
/* the following function is based on the FlexiCap original code and price the
 * BarrierCap */
Err SrtGrfnBarrierPrice(long *plFixingDates, long *plStartDates,
                        long *plPayDates, double *pdCoverages, long lNumDates,
                        String szRefRateCode, double dStrike, double dBarrier,
                        String szCapFloor, SRT_Boolean bIsInOrOut,
                        SRT_Boolean bIsUpOrDown, String szUndName,
                        String *pszGrfnParamNames, String *pszGrfnParamValues,
                        int iNumGrfnParams,
                        /* Outputs from Grfn */
                        double *pdBarrierPrice, double *pdGrfnFullCapFloorPrice,
                        double *pdGrfnCapFloorAtBarrier, long *plNumRows,
                        long *plNumCols, GrfnCell ***pppsGrfnTableau,
                        double ***pppdLastPath, long *plNumAuxColumns,
  ...
  if (err = GrfnBarrierMakeTableau(lNumDates, dStrike, dBarrier, eCapFloor,
                                   bIsInOrOut, bIsUpOrDown, szCashBasis,
...
void SrtGRFNQuickExoticCapGrfnOptions(String **grfn_param, String **grfn_value,
                                      long *lNumParams, SRT_Boolean UseTree) {
...
Err SrtGRFNQuickExoticCapPrice(
    ...
    double dGamma, double dRho, SRT_Boolean bIsChooser,
    /* Output of the function */
    String szUndName, double *pdGrfnFlexiCapPrice,
    double *pdGrfnFullCapFloorPrice, SRT_Boolean *pbReturnUnd) {
  Err err = NULL;
  ...
  int SpotLag, NumDates, NumPayDates;

  Date *FixingDates, *StartDates, *EndDates, *PayDates, Today, SpotDate;

  String FloatBasis, FloatFrequency, *pszGrfnParamNames, *pszGrfnParamValues;

  double *Coverage, *pdCapletRealPrices, **ppdLastPath, dFullCapRealPrice = 0,
                                                        **ppdAuxRanges;

  long lNumCaplets, lNumRows, lNumCols, lNumAuxColumns, *plAuxRangesLength,
  ...
  if (err = swp_f_get_ref_rate_details(szRefRateCode, &SrtFloatBasis,
                                       &SrtFloatFrequency))
    return err;
  ...
  if (err = swp_f_setSwapDP(StartDate, EndDate, SrtFloatFrequency,
                            SrtFloatBasis, FloatLegDP))
    return err;
  ...
  if (err = swp_f_make_FloatLegDatesAndCoverages(
          ...
          &EndDates, &Coverage, &NumDates))
    return err;

  /* Setup of GRFN options */

  /*	First Automatically detect if Two Factors case : if alpha is different
   * from 0 */
  ...
  if (bIsChooser)
    SrtGRFNQuickExoticCapGrfnOptions(&pszGrfnParamNames, &pszGrfnParamValues,
                                     &dNumberOfGrfnParams, SRT_YES);
  else
    SrtGRFNQuickExoticCapGrfnOptions(&pszGrfnParamNames, &pszGrfnParamValues,
                                     &dNumberOfGrfnParams, SRT_NO);

  /* to compute FullCapRealPrice... */

  if (err = swp_f_CapFloor(StartDate, EndDate, dStrike, pfGetVol, szCapFloor,
                           szRefRateCode, szYieldCurveName, "PREMIUM",
                           szVolType, &dFullCapRealPrice))
    return err;
  ...
  if (lNumExercices < NumDates) {
    if (!UndInList) {
      ...
      if (err = SrtGrfnFlexiCapCalibrate(
              FixingDates, StartDates, NumDates, szRefRateCode, dStrike,
              szCapFloor, "LGM", dTau, bUseTwoFactor, dAlpha, dGamma, dRho,
              pszGrfnParamNames, pszGrfnParamValues, dNumberOfGrfnParams,
              szYieldCurveName, pfGetVol, szVolType, szUndName,
              &pdCapletRealPrices, &lNumCaplets))
        return err;
    } else {
  ...
  if (!bReturnUnd) {
    if (bIsChooser) {
      if (err = SrtGrfnChooserPrice(
              FixingDates, StartDates, EndDates, Coverage, NumDates,
              szRefRateCode, dStrike, lNumExercices, 0, szCapFloor, szUndName,
              pszGrfnParamNames, pszGrfnParamValues, dNumberOfGrfnParams,
              dFullCapRealPrice, pdGrfnFlexiCapPrice, pdGrfnFullCapFloorPrice,
              &lNumRows, &lNumCols, &ppsGrfnTableau, &ppdLastPath,
              &lNumAuxColumns, &plAuxRangesLength, &ppdAuxRanges))
        return err;

    } else {
      if (err = SrtGrfnFlexiCapPrice(
              FixingDates, StartDates, EndDates, Coverage, NumDates,
              szRefRateCode, dStrike, dEps, lNumExercices, 0, szCapFloor,
              szUndName, pszGrfnParamNames, pszGrfnParamValues,
              dNumberOfGrfnParams, dFullCapRealPrice, pdGrfnFlexiCapPrice,
              pdGrfnFullCapFloorPrice, &lNumRows, &lNumCols, &ppsGrfnTableau,
              &ppdLastPath, &lNumAuxColumns, &plAuxRangesLength, &ppdAuxRanges))
        return err;
    }
...
Err SrtGRFNQuickBarrierCapPrice(
    ...
    /* Output of the function */
    String szUndName, double *pdGrfnBarrierPrice,
  ...
  GrfnCell **ppsGrfnTableau;
  SrtUndListPtr pSrtUndList;
  int NumDates, NumPayDates, SpotLag;

  Date *FixingDates, *StartDates, *EndDates, *PayDates, Today, SpotDate;

  String *pszGrfnParamNames, *pszGrfnParamValues, FloatBasis, FloatFrequency;

  double *pdCapletRealPrices, **ppdLastPath, **ppdAuxRanges, *Coverage;
  ...
  /*
          ...
          here follows the transformation from strings to booleans for
     in/out/up/down ...
  */
  ...
  if (!UndInList) {
    ...
    if (err = SrtGrfnFlexiCapCalibrate(
            FixingDates, StartDates, (long)NumDates, szRefRateCode, dStrike,
            szCapFloor, "LGM", dTau, bUseTwoFactor, dAlpha, dGamma, dRho,
            pszGrfnParamNames, pszGrfnParamValues, dNumberOfGrfnParams,
            szYieldCurveName, pfGetVol, szVolType, szUndName,
            &pdCapletRealPrices, &lNumCaplets))
  ...
  if (!bReturnUnd) {
    if (err = SrtGrfnBarrierPrice(
            FixingDates, StartDates, EndDates, Coverage, (long)NumDates,
            szRefRateCode, dStrike, dBarrier, szCapFloor, bIsInOrOut,
            bIsUpOrDown, szUndName, pszGrfnParamNames, pszGrfnParamValues,
            dNumberOfGrfnParams, pdGrfnBarrierPrice, pdGrfnFullCapFloorPrice,
            pdGrfnCapFloorAtBarrier, &lNumRows, &lNumCols, &ppsGrfnTableau,
            &ppdLastPath, &lNumAuxColumns, &plAuxRangesLength, &ppdAuxRanges))
      return err;
...
Err GrfnQuickDisplayTableau(String szProductName, long StartDate, long EndDate,
  ...
  int NumPayDates, NumDates, SpotLag;
  ...
  if (!strncmp(szProductName, "B", 1)) {
    ...
    err = GrfnBarrierMakeTableau((long)NumDates, dStrike, dBarrier, eCapFloor,
                                 bIsInOrOut, bIsUpOrDown, FloatBasis, szUndName,
                                 szRefRateCode, plNumRows, plNumCols,
                                 pppsGrfnTableau);
  }
  ...
  if (!strncmp(szProductName, "C", 1)) {
    ...
    err = GrfnChooserMakeTableau(NumDates, dStrike, lNumExercises, eCapFloor,
                                 FloatBasis, szUndName, szRefRateCode,
  ...
  if (!strncmp(szProductName, "F", 1)) {
    ...
    err = GrfnFlexiCapMakeTableau(
        NumDates, dStrike, lNumExercises, 0, eCapFloor, FloatBasis, szUndName,
        szRefRateCode, plNumRows, plNumCols, pppsGrfnTableau);
  }
  return err;
}

...
Err fill_mc_multi_init_corr(
    long pay_date, double pay_time, long* date, double* time, long nb_dates, LINK_UND link)
{
            ...
            for (k = 0; k < nb_dates - 1; k++)
            {
                ...
                for (i = StartIndex; i < EndIndex + 1; i++)
                {
                    ...
                    if (link->type[und] == 0)
                    {
                        ...
                        adj_pay +=
                            sig_und * link->sig_curve[dom_index][i] *
                                ...
                                correl_1[l] +=
                                    ...
                                    (x_und -
                                     exp(-link->lambda[link->dom_forex[l]] * end_mat) *
                                         Z_Func(
                                             lda_und + link->lambda[link->dom_forex[l]], T1, T2)) *
                                ...
                                correl_2[l] +=
                                    ...
                                    (x_und -
                                     exp(-link->lambda[link->for_forex[l]] * end_mat) *
                                         Z_Func(
                                             lda_und + link->lambda[link->for_forex[l]], T1, T2)) *
                ...
                else
                {
                    link->fwd[und][k + 1] =
                        1.0 / lda_und * expect +
                        exp(-lda_und * end_mat) * (-1.0 / lda_dom * adj_pay - 1.0 * adj_quanto) +
                        link->phi[und][k] * exp(-lda_und * mat) *
                            Phi_Func(-lda_und, start_mat, start_mat, end_mat);
                }
        ...
        if (link->type[und] == 2)
        {
            ...
            for (k = 0; k < nb_dates - 1; k++)
            {
                ...
                err = Fx3DtsImpliedVol_corr(
                    ...
                    link->sig_dates,
                    link->nb_sig_dates,
                    link->sig_curve[index_dom],
                    link->lambda[index_dom],
                    link->sig_curve[index_for],
                    link->lambda[index_for],
                    link->sig_dates,
                    link->sig_curve[und],
                    link->nb_sig_dates,
                    link->sig_dates,
                ...
                for (i = StartIndex; i < EndIndex + 1; i++)
                {
                    ...
                    adj_pay +=
                        ...
                        link->correlations[dom_index][index_for][i] * sig_dom *
                            ...
                            (Psi_Func(link->lambda[index_for], lda_dom, end_mat, T1, T2) -
                             Psi2_Func(
                                 link->lambda[index_for], lda_dom, end_mat, mat_pay, T1, T2)) +
                    ...
                    if (dom_index != index_dom)
                    {
                        /* NOT VALID */
                        /*
                        adj_quanto += sig_und * link -> sig_curve[link -> fx_index[index_dom]][i] *
                        (T2 - T1);
                        */
                    }
                    ...
                    for (l = 0; l < link->num_und; l++)
                    {
                        if (!do_covar[k][und][l] && und != l)
                        {
                            ...
                            case 2:
                                ...
                                correl_fxfx[l][1] +=
                                    sig_und * sig_l_dom *
                                    (Etha_Func(link->lambda[link->dom_forex[l]], end_mat, T1, T2)) *
                                    link->correlations[und][link->dom_forex[l]][i];

                                /* Z_und with For_l */
                                correl_fxfx[l][2] +=
                                    sig_und * sig_l_for *
                                    (Etha_Func(link->lambda[link->for_forex[l]], end_mat, T1, T2)) *
                                    link->correlations[und][link->for_forex[l]][i];
                                ...
                                correl_fxfx[l][3] +=
                                    sig_und_dom * sig_l *
                                    (Etha_Func(
                                        link->lambda[link->dom_forex[und]], end_mat, T1, T2)) *
                                ...
                                correl_fxfx[l][4] +=
                                    ...
                                    (Psi_Func(
                                        link->lambda[link->dom_forex[und]],
                                        link->lambda[link->dom_forex[l]],
                                        end_mat,
                                        T1,
                                        T2)) *
                                ...
                                correl_fxfx[l][6] +=
                                    sig_und_for * sig_l *
                                    (Etha_Func(
                                        link->lambda[link->for_forex[und]], end_mat, T1, T2)) *
                                ...
                                correl_fxfx[l][7] +=
                                    ...
                                    (Psi_Func(
                                        link->lambda[link->for_forex[und]],
                                        link->lambda[link->dom_forex[l]],
                                        end_mat,
                                        T1,
                                        T2)) *

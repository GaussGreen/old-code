...
#endif
    (long lClcnDate, long lNumInstruments, SwapDP *psSwapDp, double *pdStrike,
     double *pdBondStrike, SrtReceiverType *peRecPay, StructType *peProductType,
     String *pszRefRateCode, long lNumTenors, double *pdFraMaturities,
     SrtGrfnParam *psGrfnParams) {
#ifdef PVMPI
  if (!PRL_CONTEXT.bLocalComp)
    SendInstruments(lClcnDate, lNumInstruments, psSwapDp, pdStrike,
                    pdBondStrike, peRecPay, peProductType, pszRefRateCode,
                    lNumTenors, pdFraMaturities, psGrfnParams);
#endif
...
static Err set_static_sprdsht_for_calib
  ...
  err = new_grfn_SwapDParray_to_GrfnCells(
      ...
      &STATIC_SPRDSHT.pshtSHEET, today, STATIC_INSTRUMENT.lNumInstruments,
      STATIC_INSTRUMENT.psSwapDp, STATIC_INSTRUMENT.pdStrike,
      STATIC_INSTRUMENT.pdBondStrike, STATIC_INSTRUMENT.peRecPay,
      STATIC_INSTRUMENT.peProductType, und_name,
...
static Err free_space_for_static_sprdsht_for_calib
  ...
  if (STATIC_SPRDSHT.pshtSHEET)
    grfn_free_GrfnCellmatrix(STATIC_SPRDSHT.pshtSHEET, STATIC_SPRDSHT.lNumRows,
  ...
  if (!PRL_CONTEXT.bLocalComp) {
    ...
    if (STATIC_MODELVALUES.pdModelValue)
      free_dvector(STATIC_MODELVALUES.pdModelValue, 0,
                   STATIC_PARAMS.lNumData - 1);
    if (STATIC_MODELVALUES.pdModelHessian)
      free_dmatrix(STATIC_MODELVALUES.pdModelHessian, 0,
                   STATIC_PARAMS.lNumData - 1, 0, STATIC_PARAMS.lNumParams - 1);
...
/* -----------------------------------------------------------------------
Stores a few useful indexes and pdValues (mkt_price        ,
pdMarketWeightss...) in a static structure for calibration purposes.
----------------------------------------------------------------------- */
...
#ifdef PVMPI
  if (!PRL_CONTEXT.bLocalComp)
    SendParams(eCalibType, lNumData, pdMarketWeights, pdMarketTargets,
               lNumParams, lNumSigmas, lNumTaus, ppdParamBounds, bFreezeTau,
               bOneTau, dFixedTau, bFreezeBeta, bOneBeta, dFixedBeta,
               bFreezeOmega, bOneOmega, dFixedOmega, dFixedAlpha, dFixedGamma,
               dFixedRho, eModelType, eModelDim, bSmoothSigma, szUndName);
#endif /*PVMPI*/
...
static Err allocate_space_for_static_sigtau_for_calib
#endif
    (SrtMdlDim eModelDim, long lNumSigmas, double *pdSigmaDates, long lNumTaus,
     double *pdTauDates) {
  long i;
#ifdef PVMPI
  if (!PRL_CONTEXT.bLocalComp)
    SendParamForAllocation(eModelDim, lNumSigmas, pdSigmaDates, lNumTaus,
                           pdTauDates);
#endif
  ...
  if (eModelDim == TWO_FAC) {
    ...
    /* Allocate space for the static parameters Sigmas (6 cols) and Taus (3
     * cols) */
    ...
    /* Allocate space for the static parameters Sigmas (3 cols) and Taus (2
     * cols) */
...
corr_value(long corr_index,
  ...
  *corr = model_correl(STATIC_INSTRUMENT.pdFraMaturities[first_index],
...
Err aggregate_price_value
  ...
  err = from_optparam_to_sigtau(
      ...
      STATIC_PARAMS.ppdSigmaValues, STATIC_PARAMS.lNumSigmas,
      STATIC_PARAMS.ppdTauValues, STATIC_PARAMS.lNumTaus,
      STATIC_PARAMS.bFreezeTau, STATIC_PARAMS.bOneTau, STATIC_PARAMS.dFixedTau,
      STATIC_PARAMS.bFreezeBeta, STATIC_PARAMS.bOneBeta,
      STATIC_PARAMS.dFixedBeta, STATIC_PARAMS.eModelDim,
      STATIC_PARAMS.bFreezeOmega, STATIC_PARAMS.bOneOmega,
      STATIC_PARAMS.dFixedOmega, STATIC_PARAMS.eCalibType,
      STATIC_PARAMS.dFixedAlpha, STATIC_PARAMS.dFixedGamma,
  ...
  err = srt_f_init_IRM_TermStruct(
      STATIC_INSTRUMENT.lClcnDate, STATIC_PARAMS.ppdSigmaValues,
      STATIC_PARAMS.lNumSigmaCols, STATIC_PARAMS.lNumSigmas,
      STATIC_PARAMS.ppdTauValues, STATIC_PARAMS.lNumTauCols,
      STATIC_PARAMS.lNumTaus, STATIC_PARAMS.eModelType, STATIC_PARAMS.eModelDim,
      0.0, /* Beta */
      0.0, /* Alpha */
      0.0, /* Gamma */
      0.0, /* Rho */
  ...
  if ((mdl_type == ETABETA || mdl_type == LGM)) {
    for (i = 0; i < STATIC_INSTRUMENT.lNumInstruments; i++) {
      err = srt_f_closed_form(
          sUndPtr, mdl_type, mdl_dim, &(STATIC_INSTRUMENT.psSwapDp[i]),
          STATIC_INSTRUMENT.pdStrike[i], STATIC_INSTRUMENT.pdBondStrike[i],
          STATIC_INSTRUMENT.peRecPay[i], STATIC_INSTRUMENT.peProductType[i],
          STATIC_INSTRUMENT.pszRefRateCode[i], &(pdMktPrice[i]));
    ...
    if (!err) {
      err = srt_f_grfn(sUndPtr, STATIC_INSTRUMENT.psGrfnParams,
  ...
  for (i = 0; i < STATIC_INSTRUMENT.lNumInstruments; i++) {
    if (pdMktPrice[i] > DBL_MAX)
      return serror("Calibration failed: get a pdMktPrice overflow: %f",
                    *pdMktPrice);
  }
...
static Err price_value(long price_index, double *pdMktPrice) {
  ...
  err = srt_f_grfn_clsdfrm(sUndPtr, STATIC_INSTRUMENT.psGrfnParams,
...
static double std_sig_value(void) {
  ...
  for (i = 1; i <= STATIC_PARAMS.lNumSigmas; i++) {
    ...
    exp_sigma2 += STATIC_PARAMS.ppdSigmaValues[1][i - 1] *
                  STATIC_PARAMS.ppdSigmaValues[1][i - 1];
  }
...
(i.e.dOptionsWeight * varerr to frd + (1-dOptionsWeight) * varerr to correl )
  ...
  Also        , if the parameters go outside the ppdParamBounds        , the
function returns a SRT_BIG value that prevents the algorithm from using that
point.
    ...
    double *pdOptParams) /* From [1] to [STATIC_PARAMS.lNumParams] */
{
  ...
  if (are_calib_parameters_within_band(pdOptParams, STATIC_PARAMS.lNumParams,
                                       STATIC_PARAMS.ppdParamBounds) ==
      SRT_NO) {
    criteria = SRT_BIG;
    return criteria;
  }
  ...
  for (i = 1; i < STATIC_PARAMS.lNumData; i++) {
    ...
    criteria += (diff / STATIC_PARAMS.pdMarketWeights[i]) *
                (diff / STATIC_PARAMS.pdMarketWeights[i]);
  }
...
/* -----------------------------------------------------------------------
    For a given set of parameters pdOptParams[1..lNumParams]        , this
   function evaluates in *value the mkt_value required (correlation or
   pdMktPrice) for the data_index input (i.e pdMktPrice or correlation number).
    The derivatives are also returned in deriv[1..lNumParams] for the
    same data_index.

      This function calls in a loop levenberg_price_funcs        , that
   evaluates only the *value for a given set of parameters.
...
static Err levenberg_calib_funcs(
  ...
  /*	if (*value == SRT_BIG)
  {
  smessage("No valid pdMktPrice or correlation - check tau        ,alpha
  ,gamma");
  }
*/
...
static Err levenberg_aggregate_calib_funcs(
  ...
  if (bFirstTime) {
    ...
    for (i = 1; i <= STATIC_PARAMS.lNumParams; i++) {
      ...
      /* Checks the current value of the parameters is alright; if not: SRT_BIG
       */
      if (are_calib_parameters_within_band(pdOptParams, lNumParams,
                                           STATIC_PARAMS.ppdParamBounds) ==
          SRT_NO) {
        free_dvector(pdTempPrice, 0, STATIC_PARAMS.lNumData - 1);
        pdTempPrice = NULL;
    ...
    nb_retries = 0;
    do {
      err = ParallelComputation(STATIC_PARAMS.lNumData,
                                STATIC_PARAMS.lNumParams, pdOptParams);
      if (!strcmp(err, "ok")) {
...
#define free_calib_core_memory                                                 \
  {                                                                            \
    ...
    free_space_for_static_sprdsht_for_calib(STATIC_SPRDSHT.lNumRows,           \
                                            STATIC_SPRDSHT.lNumCols);          \
  }

Err srt_f_calib_core(SrtGrfnParam *psGrfnParams, SrtModelType eModelType,
                     SrtMdlDim eModelDim, SwapDP *psSwapDp, double *pdStrike,
                     double *pdBondStrike, StructType *peProductType,
                     SrtReceiverType *peRecPay, double *pdMktPrice,
                     double *pdMktVega, String *pszRefRateCode,
                     int lNumInstruments, double *pdFraMaturities,
                     double **ppdCorrelationMatrix, long lNumTenors,
                     SrtUndPtr sUndPtr, SrtCalibParam *psCalibParams,
                     double **ppdSigmaValues, long lNumSigmas,
                     long lNumSigmaCols, double **ppdTauValues, long lNumTaus,
  ...
  /* There is at least the Sigma Term Sturct to calibrate to the market prices
   * of options */
  ...
  /* If the sigma smoothing is also a criteria        , add one more pdData to
   * "fit" (the sigam variance) */
  ...
  /* Sets weigths for minimisation criteria using vegas        , options weights
   * and sizes */
  pdMarketWeights = dvector(1, lNumData);
  err = define_criteria_weights(
      lNumInstruments, lNumTenors, psCalibParams->bSmoothSigma,
      psCalibParams->dOptionsWeight, 1.0 - psCalibParams->dOptionsWeight,
      psCalibParams->dSmoothSigmaWeight, pdMktVega, pdMarketWeights);
  if (err) {
  ...
  err = define_market_targets(lNumInstruments, psCalibParams->bSmoothSigma,
                              pdMktPrice, ppdCorrelationMatrix, lNumTenors,
                              lNumData, pdMarketTargets);
  if (err) {
    free_calib_core_memory;
    return err;
  }
  ...
  err = set_calib_parameters_bounds(psCalibParams, ppdParamBounds, lNumSigmas,
                                    lNumUsedTaus, lNumUsedBetas, lNumUsedOmegas,
                                    eModelDim);
  if (err) {
    free_calib_core_memory;
    return err;
  ...
  err = from_sigtau_to_optparam(
      ppdSigmaValues, lNumSigmas, ppdTauValues, lNumUsedTaus, lNumUsedBetas,
      lNumUsedOmegas, psCalibParams->eCalibType, eModelDim, pdOptParams);
  if (err) {
    free_calib_core_memory;
    return err;
  }

  /* If Sobol is required        , gets the best initial value of the param
   * vector */
  if ((psCalibParams->lNumSobolPaths > 0) &&
      (psCalibParams->eAlgoType == LEVENBERG_MARQUARDT)) {
    /* Gets the starting point in a vector of parameters */
    err = sobol_startingpoint(pdOptParams, ppdParamBounds, lNumParams,
                              psCalibParams->lNumSobolPaths,
                              global_criteria_funcs, pdChiSquare);
    if (err) {
      free_calib_core_memory;
      return err;
    }
  ...
  /* Sets all useful statics parameters (for calibration routines from Numerical
   * Recipes) */
  szUndName = get_underlying_name(sUndPtr);
  err = set_static_params_for_calib(
      psCalibParams->eCalibType, lNumData, pdMarketWeights, pdMarketTargets,
      lNumParams, lNumSigmas, lNumTaus, ppdParamBounds,
      psCalibParams->bFreezeTau, psCalibParams->bOneTau, dFixedTau,
      psCalibParams->bFreezeBeta, psCalibParams->bOneBeta, dFixedBeta,
      psCalibParams->bFreezeOmega, psCalibParams->bOneOmega, dFixedOmega,
      dFixedAlpha, dFixedGamma, dFixedRho, eModelType, eModelDim,
      psCalibParams->bSmoothSigma, szUndName);
  if (err) {
    free_calib_core_memory;
    return err;
  }
  ...
  err = set_static_instr_for_calib(
      lClcnDate, lNumInstruments, psSwapDp, pdStrike, pdBondStrike, peRecPay,
      peProductType, pszRefRateCode, lNumTenors, pdFraMaturities, psGrfnParams);
  if (err) {
    free_calib_core_memory;
    return err;
  }

  /* If Model is not LGM (no closed form avaible) then build the static
   * SpreadSheet */
  if ((eModelType == LGM) || (psCalibParams->bAggregate == SRT_NO)) {
    ...
    err = set_static_sprdsht_for_calib(get_underlying_name(sUndPtr),
                                       get_today_from_underlying(sUndPtr));
    if (err) {
      free_calib_core_memory;
      return err;
    }
  ...
  if (psCalibParams->lNumIter > 0) {
    ...
    if (psCalibParams->eAlgoType == LEVENBERG_MARQUARDT) {
      ...
      /* Sets a vector of x pdData: here        , it is 1        ,2        ,3..:
       * the market pdData index */
      ...
      if ((eModelType == LGM) || (psCalibParams->bAggregate == SRT_NO)) {
        err = levenberg_marquardt(pdData, pdMarketTargets, pdMarketWeights,
                                  lNumData, pdOptParams, lNumParams,
                                  psCalibParams->lNumIter,
                                  levenberg_calib_funcs, pdChiSquare);
      } else {
        err = levenberg_marquardt(pdData, pdMarketTargets, pdMarketWeights,
                                  lNumData, pdOptParams, lNumParams,
                                  psCalibParams->lNumIter,
                                  levenberg_aggregate_calib_funcs, pdChiSquare);
      }
    ...
    /* --------------------- SIMULATED_ANNEALING --------------------------  */
      ...
      err = simulated_annealing(
          simplex_p, pdValues, lNumParams, psCalibParams->dSimplexTol,
          psCalibParams->lNumIter, global_criteria_funcs,
          psCalibParams->dMaxTemp, psCalibParams->dMinTemp,
    ...
    /* --------------------- SOBENBERG -------------------------  */
      ...
      /* Call the Levenberg-Marquardt main routine with the right function */
      err = sobenberg(pdData, pdMarketTargets, pdMarketWeights, lNumData,
                      pdOptParams, ppdParamBounds[1], ppdParamBounds[2],
                      lNumParams, psCalibParams->lNumSobenPoints,
                      psCalibParams->lNumBestSobenPoints,
                      psCalibParams->lNumClusters, psCalibParams->szSobenMethod,
                      psCalibParams->lNumIter, levenberg_price_funcs,
                      levenberg_calib_funcs, pdChiSquare);
    ...
    /* --------------------------- SIMPLEX --------------------------------  */
      ...
      err = amoeba(simplex_p, pdValues, (int)lNumParams,
                   psCalibParams->dSimplexTol, psCalibParams->lNumIter,
                   global_criteria_funcs, (int *)&n_func_eval);
  ...
  */
  ...
  err = from_optparam_to_sigtau(
      ...
      ppdSigmaValues, lNumSigmas, ppdTauValues, lNumTaus,
      psCalibParams->bFreezeTau, psCalibParams->bOneTau, (*dFixedTau),
      psCalibParams->bFreezeBeta, psCalibParams->bOneBeta, dFixedBeta,
      eModelDim, psCalibParams->bFreezeOmega, psCalibParams->bOneOmega,
      dFixedOmega, psCalibParams->eCalibType, dFixedAlpha, dFixedGamma,
      dFixedRho);
  if (err) {
    free_calib_core_memory;
    return err;
  ...
  err = srt_f_init_IRM_TermStruct(
      lClcnDate, ppdSigmaValues, lNumSigmaCols, lNumSigmas, ppdTauValues,
      lNumTauCols, lNumTaus, eModelType, eModelDim, 0.0, /* Beta */
      0.0,                                               /* Alpha */
      0.0,                                               /* Gamma */
      0.0,                                               /* Rho */
      0.0,                                               /* Vovol */
      0.0,                                               /* Eta */
      0.0, 0, 0, NULL, &psTermStruct);
  if (err) {
    free_calib_core_memory;
    return err;
  }
...
Err set_parallel_context(int bLocalComp, char *szGroupName, int iNumCPUs,
  ...
  if (szGroupName) {
    PRL_CONTEXT.szGroupName =
        (char *)calloc(strlen(szGroupName) + 1, sizeof(char));
...
void set_msg_tag(int iMsgTag) { PRL_CONTEXT.iMsgTag = iMsgTag; }
...
void free_weights_targets_and_bounds() {
  ...
  if (STATIC_PARAMS.ppdParamBounds)
    free_dmatrix(STATIC_PARAMS.ppdParamBounds, 1, 2, 1,
                 STATIC_PARAMS.lNumParams);
...
void free_instruments() {
  if (STATIC_INSTRUMENT.lNumInstruments > 0) {
    ...
    if (STATIC_INSTRUMENT.pszRefRateCode)
      free_svector_size(STATIC_INSTRUMENT.pszRefRateCode, 0,
                        STATIC_INSTRUMENT.lNumInstruments - 1, 32);
  ...
  if (STATIC_INSTRUMENT.pdFraMaturities)
    free_vector(STATIC_INSTRUMENT.pdFraMaturities, 0,
                STATIC_INSTRUMENT.lNumTenors - 1);

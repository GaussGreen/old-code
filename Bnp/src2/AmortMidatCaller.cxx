...
Err am_compute_american_fee(
    ...
    /*		calls */
    int ncall, int pay_rec, long *ex_date, long *set_date, double *fee) {
  Err err = NULL;
  int i;
  SrtBasisCode bas;
  double x;
...
Err am_caller(
    ...
    double *
        fund_not_ts, /*	If different from domestic or foreign (fund_ccy = 1) */
    char *
  ...
  if (exercised) {
    ...
    err = am_caller(today, use_calib, yc, vc, ref, swap_freq, swap_basis,
                    lambda, alpha, gamma, rho, get_cash_vol, lgm2dund,
                    start_date, theoEndDate, fund_ref, fund_ccy, fund_not_ts,
                    fund_ccy_yc, fx_fund_dom, fx_fund_dom_spot_date,

                    fund_ncpn, fund_fix, fund_start, fund_end, fund_pay,
                    fund_basis, fund_spr, fund_mrg, fund_fix_cpn,

                    fix_not, fix_ncpn, fix_start, fix_end, fix_pay, fix_basis,
                    fix_rate, fix_fee,
                    ...
                    mintime, mininterval, notperiod, one2F, use_jump,
                    max_var_jump, strike_type, european_model,
                    ...
                    exercised, ex_date_ex, ex_date_set, ex_fee, fund_val,
                    fix_val, call_val, export_ts, und_exp);
  ...
  if (fund_ccy == 1) {
    ...
    err = convert_funding_to_domestic_amort(
        ...
        fund_start, fund_pay, fund_basis, fund_ccy_yc, fund_not_ts2, fund_spr2,
        fund_mrg2, fund_fix_cpn2, &fund_start_date, &eq_final_ex, &eq_init_ex);
    if (err) {
      return err;
    }
  } else {
  ...
  err = am_fill_check_all_struct(
      ...
      fund_ref, fund_not_ts2, fund_ncpn, fund_fix, fund_start, fund_end,
      fund_pay, fund_basis, fund_spr2, fund_mrg2,
  ...
  while (i < fund_ncpn && fund_fix[i] < today + eod_fix_flag) {
    if (fund_pay[i] >= today + eod_pay_flag) {
      ...
      fund_leg_pv += (fund_fix_cpn2[i] + fund_mrg2[i]) *
                     ...
                     fund_not_ts2[0] * swp_f_df(today, fund_pay[i], yc);
    }
...
Err am_caller_ts(
  ...
  if (exercised) {
    ...
    err = am_caller_ts(today, use_calib, yc, vc, ref, swap_freq, swap_basis,
                       pdLambdaValue, pdLambdaTime, nLambdaSize,

                       alpha, gamma, rho, get_cash_vol, lgm2dund, start_date,
                       theoEndDate, fund_ref, fund_ccy, fund_not_ts,
                       fund_ccy_yc, fx_fund_dom, fx_fund_dom_spot_date,

                       fund_ncpn, fund_fix, fund_start, fund_end, fund_pay,
                       fund_basis, fund_spr, fund_mrg, fund_fix_cpn,

                       fix_not, fix_ncpn, fix_start, fix_end, fix_pay,
                       fix_basis, fix_rate, fix_fee,
  ...
  if (call_feat == 1) {
    ...
    err = am_launch_adi_ts(pdLambdaValue, pdLambdaTime, nLambdaSize, am, und,
                           adi_arg, &call);

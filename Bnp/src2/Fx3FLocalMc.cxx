...
Err mcLocal_main_3dfx(
    /*	Time data */
    long npaths, long nsteps, int num_col, double *time, double *date,
    double *dom_ifr, double *dom_std, double *dom_phi, double *for_ifr,
    double *for_std, double *for_phi, double *fx_fwd, double *fx_std,

    LOCAL_MODEL_FUNC *model_func, LOCAL_MODEL_PARAM *model_param,

    /*	Product data */
    void **func_parm_tab, int *has_evt,
    /*	Model data */
  ...
  double t1, t2;
  double df, dt, zc;
  double X, Y, Z, Z2, S, vol_ln, logSpot;
  ...
  err = correlate_variable_corr(&(dom_std[1]), &(for_std[1]), &(fx_std[1]),
                                &(corr_dom_for[1]), &(corr_dom_fx[1]),
                                &(corr_for_fx[1]), npaths, nsteps - 1, matrix);
  if (err) {
    goto FREE_RETURN;
  }
  time2 = clock();
  ...
  for (i = 0; i < npaths; i++) {
    ...
    for (j = 1; stop_path == 0 && j < nsteps; j++) {
      ...
      if (S > 1.0E-08) {
        ...
        vol_ln = fx_std[j] * model_func->vol_func(j - 1, Z2, model_param) / Z2;

        Z += (X + dom_ifr[j - 1] - Y - for_ifr[j - 1]) * dt -
             0.5 * vol_ln * vol_ln +
             vol_ln / fx_std[j] * matrix[i][3 * (j - 1) + 2];
      }

      X += (dom_phi[j - 1] - dom_lam * X) * dt + matrix[i][3 * (j - 1)];

      Y += (for_phi[j - 1] - for_lam * Y) * dt -
           corr_for_fx[j] * for_std[j] * vol_ln + matrix[i][3 * (j - 1) + 1];

      if (has_evt[j]) {
        err = payoff_func(date[j], t2, func_parm_tab[num_evt], spot_fx, dom_yc,
                          dom_lam, dom_phi[j], for_yc, for_lam, for_phi[j], X,
                          Y, Z, num_col, res_evt, &stop_path);
        num_evt += 1;
...
Err mcLocal_main_3dfx2(
  ...
  for (i = 0; i < npaths; i++) {
    ...
    for (j = 1; stop_path == 0 && j < nsteps; j++) {
      ...
      if (S > 1.0E-08) {
        vol_ln = fx_std[j] * vol_ln_func(t1, S, fx_fwd[j - 1], params);
      ...
      if (has_evt[j]) {
        err = payoff_func(date[j], t2, func_parm_tab[j], spot_fx, dom_yc,
                          dom_lam, dom_phi[j], for_yc, for_lam, for_phi[j], X,
                          Y, Z, num_col, res_evt, &stop_path);

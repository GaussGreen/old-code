    ...
    DEAL_DATA* deal_data) /* Structure of deal data */
{
    ...
    if (NbPer == ExEnd) /* Initialization of the IO price at the end of the tree */
    {
        for (i = Bottom; i <= Top; i++)
        {
            if (deal_data->invfloater == 'Y') /* inverse floator */
                ...
                IO[i] = Coupon / F * pow(Discount[i], (double)deal_data->pay_delay / 30.);

            PO[i] = 100. *
                    pow(Discount[i],
                        (double)deal_data->pay_delay / 30.); /* taking into account payment delay */
        }
        return;
    }
    ...
    if (deal_data->Schedule == 'N')
        TimeBalance = 1.0; /* if no scheduled amortization, Balance remains at 1.*/
    else
        TimeBalance =
            (pow(1. + deal_data->Gwac / F / 100., (double)MM - CouponNb + 1) - 1. -
             deal_data->Gwac / F / 100.) /* Balance is calculated so that it is 1 at onset */
    ...
    {
        if ((NbPer >= Lockout) &&
            (CouponNb < deal_data->Term - deal_data->Delay)) /* If we are after the lockout date and
                                                                "Delay" number of coupons */
        { /* before maturity we amortize according to the index level.      */
            for (i = Bottom; i <= Top; i++)
            {
                ...
                PO[i] = PO[i] * (1. - TotalAmort) +
                        100. * TotalAmort * Zero[deal_data->Delay][i] *
                            pow(Discount[i],
                                (double)deal_data->pay_delay /
                                    30.); /* If there is a delay, the par payment  is reduced, also
                                             there is a payment delay  */

                for (j = 1; j <= deal_data->Delay;
                     j++) /* If there is a delay in the index amortization we should not have */
                    IO[i] +=
                        Coupon / F * TotalAmort *
                        Zero[j][i]; /* amortized the next few coupons. We correct that here. */

                if (deal_data->invfloater == 'Y') /* inverse floater */
                    IO[i] += (1. - TotalAmort) *
                             max(0,
                                 Coupon / F - (1. / Discount1[i] - 1.) * 100. *
                    ...
                    IO[i] +=
                        (1. - TotalAmort) *
                        min(Coupon / F,
                            (1. / Discount1[i] - 1.) * 100. * (double)(tree_data->Ppy + 1.0) / F) *
                        pow(Discount[i], (double)deal_data->pay_delay / 30. + 1.0);
            } /* for i */
        }     /* if */
        ...
        for (i = Bottom; i <= Top; i++)
        {
            ...
            if (deal_data->invfloater != 'Y' &&
                ...
                IO[i] +=
                    Coupon / F *
                    pow(Discount[i],
                        (double)deal_data->pay_delay / 30.); /* The coupon is paid on balance left
            ...
            PO[i] =
                ...
                100. * (1. - TimeBalance) *
                    pow(Discount[i],
                        (double)deal_data->pay_delay /
                            30.); /* At a coupon date there always is a scheduled amortization. */

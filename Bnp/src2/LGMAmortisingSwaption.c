  ...
  for (j = 0; j < nEx; j++) {
    ...
    if (tEx >= tNow && tEx < tEnd - 30) {
      if (iLGMOneTwoFactor == 1) {
        // error = LGMGetRfixFromX(tEx  , tEnd  , xExBdry[j]  , tNow  , ycName
...
of the strike. If deal is strictly positive or strictly negative  , ratio is set
...
positive or strictly negative  , ratio is set to 0. Note that tNow is needed
only for computing the intrinsic value */
    ...
    String outfile)            /* output file name for log file (unused) */
{
  ...
  error = LGMExtractInfoFromDeal(
      ...
      &EffnEx, &LasttPay,
      /* NBR OF EXERCISE DATES  , THE LAST PAYT DATE  */ /* THE MID-AT DEAL THEO
                                                            END DATE. */

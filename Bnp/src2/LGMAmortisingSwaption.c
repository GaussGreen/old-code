...
static LGMErr FitAllZetaAmortisingSwaptions(LGM_TS *tsPtr, LGMCalSet *CSPtr,double NumPeriod, double *index, double *d, double *dstart, double *mktprice);
...
LGMErr LGMCalFixKapamortising(
		...
		LGMCalSet *CSPtr,
		double NumPeriod, double *index, double *d, double *dstart, double *mktprice);	/* Reference caplets */

static double RecSqZetaDer(double ystar, long n, double *a, double DStart,	/* receiver swaption */ 
						double sqzeta, double *Gpay, double Gst);			/* term structure */


double RecGDer(double ystar, long n, double *a, double DStart,		/* receiver swaption */ 
					double sqzeta, double *Gpay, double Gst,				/* term structure */
					double dGst, double *dGpay);	
...
/* info about today's market */
	...
	LGMErr	(*GetVol)(Date, Date, double, SRT_Boolean, double*),	/* function to provide swaption/cap vols */
	LGMErr	(*GetBeta)(Date, Date, double*),	/* function to provide exponent (beta) to interpret vols */
...
static double FitOneCap(double price, double cvg, double Rfix,
					double DStart, double Dend, SrtReceiverType recpay);

static LGMErr FitAllZetaCaps(LGM_TS	*tsPtr,			/* Return: zeta's in LGM term structure ts */
...
/* outputs */
		...
		SrtLgmRefSwptnData *lgmRefDeals, /* ptr to reference swaption data structure (NULL => not req'd) */
		LGM_TS	  **LGMtsPtrPtr,	/* calibrated zeta-G term structure */
...
static LGMErr RefDeals1intok( 
			long nArr, Date *TauArr, double *FVArr,		/* exercise date and relative PV arrays */
...
/* Note: RefDealsCommon must be called first */
static LGMErr RefDealsCap(LGMDealType			DealType,
						  void					*DealPtr,
						  long					nArr,
						  Date					*TauArr,
						  double				*FVArr,
...
static LGMErr RefDealsCommon(
			Date tNow, long nEx,						/* evaluation date, number of exercises */
...
static LGMErr checkinputarrays(long n, Date tNow, Date tLast, 
							   Date *t, double *RatArr, LGMCalParm *CR,
							   long *firstiPtr, long *lastiPtr);
...
static LGMErr FindFixRateRefInst(
		...
		double DStart, double DEnd, double DLast,		/* df's to start date and end dates */
		Date tEx, Date tStart, Date tEnd,				/* dates of refence swaption */
		long islong,									/* 1=use Rdata1, 0=use Rdata2 */
		long nArr, double *FVarr, Date *TauArr,			/* PV ratio & exer dates of orig deal */
...
double InterpPVratio(Date thedate,
			long n, double *FV, Date *Tau, Date tLast);

static double find_gear(
Date		thedate, 
Date		*d, 
double		*gear, 
int			n);
...
/* allocate space */
	...
	if (ExerBdryPtr==NULL) 	return (NULL);
	...
	ExerBdryPtr->n = k+1;
	if (k < 1) 	LGMFreeSwptns(&ExerBdryPtr);
...
static LGMErr CompValCapFlt(
	...
	double *amax, double *amin, double *marg,		/* handle max and min, margin*/ 
	double lvg,										/* leverage */ 
	...
	SrtBasisCode basis)								/* basis for rates */
{
	...
	{	dfEnd = swp_f_df(tNow, t[i], ycName);			/* calculate value of floor */
		...
		Vcapmax = LGMCEVCapletPrice(tNow, tEx, SRT_PAYER, amax[i], cvgCap, dfSt, dfEnd, CEVvolmax, beta);

		if (Vcapmax < 0.0) return("can't get cap max price");
		Vcapmin = LGMCEVCapletPrice(tNow, tEx, SRT_PAYER, amin[i], cvgCap, dfSt, dfEnd, CEVvolmin, beta);
	...
	if (df==SRT_DF_ERROR) return("no discount factor");

	intVal = sum-df*Strike;					/* intrinsic value */
	if (PayRec==SRT_PAYER) intVal = -intVal;
	intVal = max(0., intVal);

	if (Strike==0.) ratio = 0.0;
	else ratio = sum/(df*Strike);			/* ratio of PV's */
...
static LGMErr CompValOpt(Date tNow, String ycName, double *ratioPtr, double *intValPtr, 
					SrtReceiverType PayRec, Date tStart, double Strike, double redfirstPay,
					long nPay, Date *tPay, double *Payment)
{
	long j;
...
static LGMErr CompValGen(Date tNow, String ycName, double *ratioPtr, double *intValPtr, 
						long nPay, Date *tPay, double *Payment)
{
	long j;
...
{	double lbound, ubound, slope, dt, isign;
	...
	if (CR->keep1intok > 1) CR->keep1intok = 1;
	if (CR->keep1intok < -1) CR->keep1intok = -1;
...
/* check strike data */
	...
	if (CR->Rmeth == IRR || CR->Rmeth == dIRR || CR->Rmeth == EMK1 || CR->Rmeth == EMK2)	/* Nothing to check */
		return;
...
/* Rdata2 is used to construct strikes (Rfix) for all other instruments */
	...
	{	lbound = -0.03; ubound = 0.03;}		/* Rswap - 3% < Rfix < Rswap + 3% */
	if (CR->Rmeth == propshift)
	{	lbound = 0.5; ubound = 1.0;}		/* 0.5*Rswap < Rfix < 2.0*Rfix */
	if (CR->Rmeth == fracstd)
	{	lbound = -2.0; ubound = 2.0;}		/* Rswap - 2stddev < Rfix < Rswap + 2stddev */
	if (CR->Rmeth == givenR)
	{	lbound = 0.0010; ubound = 0.99;}	/* 10bps < Rfix < 99% */
...
/* Find fixed rates Rfix of 1 into k swaptions */
	...
	if (DealType == CallInvFlt)
	{
		level = cvgfix * Dpay[ffirst] * (1.0 + find_gear (tPay[ffirst], cinf->tCpnPay, cinf->gear, cinf->nCpn));
		for (i=ffirst+1; i<nfirst; i++)
		{
			level += cvgpay[i] * Dpay[i] * (1.0 + find_gear (tPay[i], cinf->tCpnPay, cinf->gear, cinf->nCpn));
		}
	}
	...
	{	
		...
		if (DealType == CallInvFlt)
		{
			level += cvgpay[j] * Dpay[j] * (1.0 + find_gear (tPay[j], cinf->tCpnPay, cinf->gear, cinf->nCpn));
			level /= DfixStart;
		}
		else
		{
		...
		error = FindFixRateRefInst(
				...
				level,								
				DfixStart, Dpay[j], DLast,			/* df's for ref start and end, actual end */
				ftEx, ftStart, tPay[j],				/* exer, start, and end date of ref swaption */
				islong,								/* which set of Rdata for strikes */
				nArr, FVArr, TauArr, tNow, tLast,	/* PV ratio, exer, and end date of orig deal */
				conv, CalReq, 
				GetVol, GetBeta,					/* Calibration method, market vol and smile */
...
/* Update Midat Structure for special methods of calibration */
	if (CalReq->LGMOneTwoFactor==2 && DealType == SimpleMidAt && ((CalReq->Rmeth == EMK1) || (CalReq->Rmeth == EMK2)))
	{
...
/* Fill in CSPtr->tEnd[], CSPtr->cvgcap[], CSPtr->Dcap[] */
	...
	{	
		CSPtr->tEnd[j] = add_unit(CSPtr->tStart[j], 
								  12/(int)conv->cfreq,
								  SRT_MONTH,
								  conv->cbdconv); /* NO_BUSDAY_CONVENTION or BUSDAY_CONVENTION...... */

		CSPtr->cvgcap[j] = coverage(CSPtr->tStart[j],
									CSPtr->tEnd[j],
		...
		switch(DealType)
		{
			...
			case CallInvFlt:
				...
				error = FindFixRateRefInst(
						...
						CSPtr->cvgcap[j]*CSPtr->Dcap[j]*(1.0+find_gear(CSPtr->tEnd[j],BerInvFlt->tCpnPay,BerInvFlt->gear, BerInvFlt->nCpn)),
						CSPtr->DStart[j], CSPtr->Dcap[j], CSPtr->Dpay[CSPtr->n],			
						CSPtr->tEx[j], CSPtr->tStart[j], CSPtr->tEnd[j],			
						islong,								
						nArr, FVArr, TauArr, CSPtr->tNow, CSPtr->tPay[CSPtr->n],	
						conv, CalReq, GetVol, GetBeta,		
						&CSPtr->Rfcap[j],
						NULL,
						CSPtr);
...
*/
/* Note: RefDealsCommon must be called first to set nEx, n, nPay, tEx[], tStart[], tPay, cvgpay, Dpay,
			n, and nPay */
...
/* Update Midat Structure for special methods of calibration */
	...
	{													
		...
		{
			i = ifirst[j];									
			level = cvgfirst[j] * Dpay[i] * (1.0 + find_gear (tPay[i], cinf->tCpnPay, cinf->gear, cinf->nCpn));	
			i++;
			for ( ; i<=iend; i++)
			{
				level += cvgpay[i] * Dpay[i] * (1.0 + find_gear (tPay[i], cinf->tCpnPay, cinf->gear, cinf->nCpn));
		...
		else		/* CALIBRATION AT THE EQUIVALENT STRIKE */
		{
			error = FindFixRateRefInst(
				refsw,									/* swap rate of ref swaption */
				level,
				DStart[j], Dpay[iend], DLast,			/* df's for ref start and end, actual end */
				tEx[j], tStart[j], tPay[iend],			/* exer, start, and end date of ref swaption */
		...
		LGMCalSet *CSPtr)
{
	...
	if (CalReq->Rmeth!=IRR && CalReq->Rmeth!=dIRR && CalReq->Rmeth!=EMK1 && CalReq->Rmeth!=EMK2)
	{	if (islong==1)
...
static double FitOneSwap(
		long n, double *a, double DStart, SrtReceiverType recpay,	/* the swaption */
		double zeta,												/* zeta */
	...
	{	newvalue = -1.0;							/* calculate func & der at C+dC */
		...
		{	arg = ratio*(Gst-Gpay[i] + (coef+dcoef)*(dGst-dGpay[i]));
			if (fabs(theta)<=1.e-5 || fabs(arg)<=1.e-4)
				newvalue = newvalue + a[i]*arg*(1. - 0.5*arg*theta*(1.0 - arg*theta/3.0));
			else
...
/* Use global Newton on coef to match LGM price to market price */
	...
	{	coef = coef + isign*dcoef;									/* update term structure */
		...
		if (fabs(newvalue)/(timevalue + 0.001) < 1.e-5 || fabs(dcoef)<1.e-6)	/* check convergence */
			success++;
		...
		String	outfile)			/* output file name for log file (unused) */
{
	...
	{	error = LGMVerifyLGM_TS(*LGMtsPtrPtr);
		if (error) skipCalib=0;						/* can't skip if no valid term structue */
	}

	if (skipCalib!=1 && *LGMtsPtrPtr != NULL)
	if(CalReq->LGMOneTwoFactor==1)	LGMFreeLGM_TS(LGMtsPtrPtr);
	else if(CalReq->LGMOneTwoFactor==2)	LGMFreeLGM2F_TS(LGMtsPtrPtr);
...
/* Step 1: Check validity of deal */
	...
	if (error) return(error);		/* problem with the deal */
...
and return these to the calling routine as "ExerIntoPtrPtr" */
	...
	error = LGMExtractInfoFromDeal(
				...
				&EffnEx, &LasttPay,         /* NBR OF EXERCISE DATES, THE LAST PAYT DATE  */					/* THE MID-AT DEAL THEO END DATE. */
				&TauArr, &FVArr,			/* info for calibrator */
				&intrinVal, &ExerIntoPtr);	/* other info to return as output */

	if (error)
	{	srt_free(TauArr); srt_free(FVArr);
...
/* Step 3: Create and calibrate a LGM ts */
	...
	{	error = LGMamortisingCalibration(
				...
				NumPeriod, index, d, dstart, mktprice,				
				dealtype,
				dealPtr,
				EffnEx,LasttPay,					/* info about the deal */
				TauArr, FVArr,
				/* info about the deal */
...
/* On error, returns a NULL pointer. We're treating this as non-fatal */
	...
	{
		if(CalReq->LGMOneTwoFactor==1) SigKaptsPtr = LGMConvertZGtoSigKap(tNow, LGMtsPtr);
		else 
		{
			...
			for(i = 1; i <=nEx; i++) 
			{
				...
				else
				{
					...
					kappa_term = (exp(2*one_kappa*yr_to_exp)-exp(2*one_kappa*prev_yr_to_exp))/(2*one_kappa);
					SigKaptsPtr->sig[i-1] = sqrt(LGMtsPtr->Zeta1[i-1]-LGMtsPtr->Zeta1[i-2])/kappa_term;
				}
			}
...
/* Step 5: Evaluate deal, create xExBdry array and fill it with exer bdry */
	if (skipEval!=1 || findExerBdry!=0)
		error = NewMidAtEval(
					...
					tNow, eod, ycName, 
					CalReq, LGMtsPtr,	/* Information about today */
					GetVol, GetBeta,				/* swaption/cap vols */
					dealtype, dealPtr,				/* the deal */
					&LGMVal,						/* value of the deal */
					&nExBdry, &tExBdry, (findExerBdry ? &xExBdry : NULL) );	/* array of exercise points (in x) */
	...
	{	LGMFreeSwptns(&ExerIntoPtr);
		...
		if (convertTS!=0)
		{
			if(CalReq->LGMOneTwoFactor==1)
			{
				...
				srt_free(xExBdry); srt_free(tExBdry);
			}
		}
		return(error);
	}
...
/* On error, returns a NULL pointer. We're treating this as non-fatal */
	...
	{
		ExerBdryPtr = FindBdry(CalReq->LGMOneTwoFactor,tNow, ycName, LGMtsPtr,nExBdry, tExBdry, xExBdry,LasttPay);
...
/* initialize */
	...
	zeta = dvector(0, nEx);
	if (tsPtr->numZ < nEx+1)
	{	srt_free(tsPtr->zeta); srt_free(tsPtr->zdate);
	...
	{	srt_free(dispayment); srt_free(Gpay);
		return ("allocation failed in FitAllZetaSwaps");
	}
...
long swaption j to the market price Vlong[j] */
	...
	{	
		...
		{
			zeta[j] = FitOneZetaSwap(n, dispayment, dstart[ifirst+2],recpay, coupon_G, Gst, mktprice[ifirst+1]);
		...
		if (zeta[j]<0)
		{	srt_free(Gpay); srt_free(dispayment);
...
/* Check calibration request for sensibility data */
	...
	/* Step 1: Construct reference instruments */
		error = LGMMakeRefDeals(
					...
					dealptr,
					tNow, tlast,
					nEx, TauArr, FVArr,		/* exer dates and relative PV of fixed leg of deal */
...
/*	Step 3: Calibrate */
	if(CalReq->LGMOneTwoFactor==1)
	{
		...
		if (error!=NULL)
		{	
			if(CalReq->LGMOneTwoFactor==1)	LGMFreeLGM_TS(&LGMtsPtr);		
			if(CalReq->LGMOneTwoFactor==2)	LGMFreeLGM2F_TS(&LGMtsPtr);		
			LGMFreeCalSet(&RefDealPtr);
		}
	...
	else if(CalReq->LGMOneTwoFactor==2)
	{
		...
		error  = lgm_2f_autocal_calibrate(dealptr,LGMtsPtrPtr,lgmRefSwptnData,CalReq,GetVol,GetBeta,ycname);
		if (error!=NULL)
		{	
			if(LGMtsPtr)
...
/* Create term structure, fill in numG and Gdates */
	...
	{	if (usecaps==1)											/* find last date to use */
		...
		for (i=1; i<=ndate && Gdate[i]<=CSPtr->tStart[1]; i++)	/* find number of dates to use */
			;
...
/* Use global Newton on sqzeta = sqrt(zeta(tEx)) to match LGM price to market price */
	...
	for (iter=1; iter<=itermax && success<=0; iter++)
	{	newvalue = LGMRecVal(&ystar, n, a, DStart, sqzeta+dsqzeta, Gpay, Gst)
						- price;
		if (fabs(newvalue)/price < 1.e-7 || fabs(dsqzeta)<1.e-7)	/* take three steps after OK convergence */
			success++;
